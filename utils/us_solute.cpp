...
void
US_Solute::init_solutes (double s_min, double s_max, int s_res, double ff0_min,
                         double ff0_max, int ff0_res, int grid_reps,
                         double cnstff0,
                         QList<QVector<US_Solute> > &solute_list)
{
  ...
  qDebug () << "InSo: nprs nprk" << nprs << nprk << "s_step k_step"
            << s_step * 1.e+13 << ff0_step << "s_grid k_grid"
      ...
      for (int jk = 0; jk < grid_reps; jk++)
        {
          ...
          solute_list << create_solutes (s_min_g, s_max, s_step, k_min_g,
                                         ff0_max, ff0_step, cnstff0);
        }
    }
}

QVector<US_Solute>
US_Solute::create_solutes (double s_min, double s_max, double s_step,
