...
int
US_CSV_Data::columnCount () const
{
  return m_header.size ();
}

int
US_CSV_Data::rowCount () const
{
  if (m_columns.size () == 0)
    return 0;
...
QStringList
US_CSV_Data::header () const
{
  return m_header;
}

QVector<double>
US_CSV_Data::columnAt (int id) const
{
  QVector<double> col;
...
void
US_CSV_Data::setFilePath (const QString &file_path)
{
  m_path = file_path;
}

bool
US_CSV_Data::setData (const QString &file_path, const QStringList &header,
...
bool
US_CSV_Data::setData (const QStringList &header,
                      const QVector<QVector<double> > &columns)
{
  m_columns.clear ();
  m_header.clear ();
  ...
  if (columns.size () != ncols)
    {
      m_error = "The number of columns does not match the length of the "
                "header list.";
      return false;
    }
  m_header << header;
      ...
      if (nr == 0 || nr != nrows)
        {
          m_columns.clear ();
          m_header.clear ();
          m_error = QObject::tr ("The number of rows in column 2 does not "
                                 "match the loaded columns.");
          return false;
        }
...
QString
US_CSV_Data::filePath () const
{
  return m_path;
}

QString
US_CSV_Data::error () const
{
  return m_error;
}

bool
US_CSV_Data::readFile (const QString &filePath, const QString &delimiter)
{
  QFile file (filePath);
  m_error.clear ();
  clear ();
  ...
  if (file.open (QIODevice::ReadOnly))
    {
      ...
      if (file_lines.size () == 0)
        ...
        {
          m_error = QObject::tr (
              "File has only one line! It must have at least two lines, "
              "including a header line and a data line.");
          return false;
        }
    }
  else
  ...
  if (!split_status)
    {
      m_error = QObject::tr ("Cannot split the lines with the given "
                             "separator. Error at the line: %1")
                    .arg (II);
      return false;
    }
      ...
      for (int ii = init; ii < n_rows; ii++)
        {
          ...
          if (ok)
            ...
            {
              m_error = QObject::tr ("Cannot convert the line to the floating "
                                     "numbers. Error at the line: %1")
                            .arg (ii + 1);
              return false;
            }
        }
  ...
  if (setData (filePath, header, columns))
    return true;
  else
    return false;
}
...
bool
US_CSV_Data::writeFile (const QString &delimiter)
{
  if (filePath ().isEmpty ())
    {
