...
int US_CSV_Data::columnCount() const { return m_header.size(); }
...
QStringList US_CSV_Data::header() const { return m_header; }
...
void US_CSV_Data::setFilePath(const QString &file_path) { m_path = file_path; }

bool US_CSV_Data::setData(const QString &file_path, const QStringList &header,
...
bool US_CSV_Data::setData(const QStringList &header,
  ...
  if (columns.size() != ncols) {
    m_error =
        "The number of columns does not match the length of the header list.";
    return false;
  }
  m_header << header;
  int nrows = -1;
  for (int ii = 0; ii < ncols; ii++) {
    ...
    if (nr == 0 || nr != nrows) {
      ...
      m_header.clear();
      m_error = QObject::tr(
          "The number of rows in column 2 does not match the loaded columns.");
      return false;
    }
...
QString US_CSV_Data::filePath() const { return m_path; }

QString US_CSV_Data::error() const { return m_error; }
...
bool US_CSV_Data::readFile(const QString &filePath, const QString &delimiter) {
  ...
  if (file.open(QIODevice::ReadOnly)) {
    ...
    if (file_lines.size() == 0) {
      ...
      m_error = QObject::tr(
          "File has only one line! It must have at least two lines, including "
          "a header line and a data line.");
      return false;
    }
  } else {
  ...
  if (!split_status) {
    m_error = QObject::tr(
                  "Cannot split the lines with the given separator. Error at "
                  "the line: %1")
                  .arg(II);
    return false;
  }
  ...
  for (int jj = 0; jj < n_columns; jj++) {
    ...
    for (int ii = init; ii < n_rows; ii++) {
      ...
      if (ok) {
        ...
        m_error = QObject::tr(
                      "Cannot convert the line to the floating numbers. Error "
                      "at the line: %1")
                      .arg(ii + 1);
        return false;
      }
