...
bool US_CSV_Data::setData(const QString &file_path, const QStringList &header, const QVector<QVector<double>> &columns) {
   setFilePath(file_path);
   return setData(header, columns);
}

bool US_CSV_Data::setData(const QStringList &header, const QVector<QVector<double>> &columns) {
   m_columns.clear();
   m_header.clear();
   m_error.clear();
...
bool US_CSV_Data::readFile(const QString &filePath, const QString &delimiter) {
   ...
   if (file.open(QIODevice::ReadOnly)) {
      ...
      if (file_lines.size() == 0) {
         ...
         m_error = QObject::tr(
            "File has only one line! It must have at least two lines, including a header line and a data line.");
         return false;
      }
   }
   else {
