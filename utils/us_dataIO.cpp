...
int
US_DataIO::RawData::xindex (double xvalue)
{
...
double
US_DataIO::RawData::radius (int radx)
{
  return xvalues[radx];
}
...
double
US_DataIO::RawData::scWavelength (int wavx)
{
  return xvalues[wavx];
}
...
double
US_DataIO::RawData::value (int scnx, int radx)
{
  return scanData[scnx].rvalues[radx];
}
// Get the readings value at given scan,radius indecies
double
US_DataIO::RawData::reading (int scnx, int radx)
{
  return scanData[scnx].rvalues[radx];
}
...
bool
US_DataIO::RawData::setValue (int scnx, int radx, double value)
{
  if (scnx < 0 || scnx >= scanData.size () || radx < 0
      || radx >= scanData[scnx].rvalues.size ())
    return false;
...
double
US_DataIO::RawData::std_dev (int scnx, int radx)
{
...
double
US_DataIO::RawData::average_temperature () const
{
...
double
US_DataIO::RawData::temperature_spread () const
{
  int scansize = scanData.size ();
  double smin = 99.0;
  double smax = 0.0;
...
// Return the count of readings points
int
US_DataIO::EditedData::pointCount ()
{
  return xvalues.size ();
}
...
// Return the count of scans
int
US_DataIO::EditedData::scanCount ()
{
  return scanData.size ();
}
...
// Return the index of an X (radius or wavelength) value
int
US_DataIO::EditedData::xindex (double xvalue)
{
  return index (xvalues, xvalue);
}
...
// Get the X value (radius) at a given index
double
US_DataIO::EditedData::radius (int radx)
{
  return xvalues[radx];
}
...
// Get the X value (wavelength) at a given index
double
US_DataIO::EditedData::scWavelength (int wavx)
{
  return xvalues[wavx];
}
...
// Get the readings value at given scan,radius indecies
double
US_DataIO::EditedData::value (int scnx, int radx)
{
  return scanData[scnx].rvalues[radx];
}
...
// Get the readings value at given scan,radius indecies
double
US_DataIO::EditedData::reading (int scnx, int radx)
{
  return scanData[scnx].rvalues[radx];
}
...
// Set the readings value at given scan,radius indecies
bool
US_DataIO::EditedData::setValue (int scnx, int radx, double value)
{
  if (scnx < 0 || scnx >= scanData.size () || radx < 0
      || radx >= scanData[scnx].rvalues.size ())
    return false;
...
// Get the standard deviation value at given scan,radius indecies
double
US_DataIO::EditedData::std_dev (int scnx, int radx)
{
  US_DataIO::Scan *scn = &scanData[scnx];
...
// Calculate the average temperature value across scans
double
US_DataIO::EditedData::average_temperature () const
{
  int scansize = scanData.size ();
  double sum = 0.0;
...
// Calculate the spread of temperature values across scans
double
US_DataIO::EditedData::temperature_spread () const
{
  int scansize = scanData.size ();
  double smin = 99.0;
  double smax = 0.0;
...
bool
US_DataIO::readLegacyFile (const QString &file, BeckmanRawScan &data)
{
...
int
US_DataIO::writeRawData (const QString &file, RawData &data)
{
...
void
US_DataIO::writeScan (QDataStream &ds, const Scan &data, quint32 &crc,
                      const Parameters &pp)
{
  union
  {
...
void
US_DataIO::write (QDataStream &ds, const char *c, int len, quint32 &crc)
{
...
int
US_DataIO::readRawData (const QString &file, RawData &data)
{
  QFile ff (file);
...
void
US_DataIO::read (QDataStream &ds, char *cc, int len, quint32 &crc)
{
...
int
US_DataIO::readEdits (const QString &filename, EditValues &parameters)
{
  QFile ff (filename);
  if (!ff.open (QIODevice::ReadOnly))
    return CANTOPEN;
...
void
US_DataIO::ident (QXmlStreamReader &xml, EditValues &parameters)
{
  while (!xml.atEnd ())
    {
      if (xml.isEndElement () && xml.name () == "identification")
        return;
...
void
US_DataIO::run (QXmlStreamReader &xml, EditValues &parameters)
{
  QXmlStreamAttributes a = xml.attributes ();
  ...
  while (!xml.atEnd ())
    {
      if (xml.isEndElement () && xml.name () == "run")
        return;
...
void
US_DataIO::all_lambdas (QXmlStreamReader &xml, EditValues &parameters)
{
  while (!xml.atEnd ())
    {
      if (xml.isEndElement () && xml.name () == "lambdas")
        return;
...
void
US_DataIO::params (QXmlStreamReader &xml, EditValues &parameters)
{
  ...
  while (!xml.atEnd ())
    {
      if (xml.isEndElement () && xml.name () == "parameters")
        return;
      ...
      else if (xml.name () == "air_gap")
        {
          ...
          parameters.gapTolerance
              = a.value ("tolerance").toString ().toDouble ();
        }
      ...
      else if (xml.name () == "linear_baseline_correction")
        {
          ...
          parameters.bl_corr_yintercept
              = a.value ("y_intercept").toString ().toDouble ();
        }
...
void
US_DataIO::operations (QXmlStreamReader &xml, EditValues &parameters)
{
  while (!xml.atEnd ())
    {
      if (xml.isEndElement () && xml.name () == "operations")
        return;
...
void
US_DataIO::excludes (QXmlStreamReader &xml, EditValues &parameters)
{
  while (!xml.atEnd ())
    {
      if (xml.isEndElement () && xml.name () == "excludes")
        return;
...
void
US_DataIO::do_edits (QXmlStreamReader &xml, EditValues &parameters)
{
  while (!xml.atEnd ())
    {
      if (xml.isEndElement () && xml.name () == "edited")
        return;
...
int
US_DataIO::loadData (const QString &directory, const QString &editFilename,
...
int
US_DataIO::loadData (const QString &directory, const QString &editFilename,
                     QVector<EditedData> &data, QVector<RawData> &raw)
{
  ...
  if (isMwl)
    {
      ...
      edtFileRead = editFilename.section (".", 0, -3) + "." + elambda + ".xml";
    }
  ...
  ed.ODlimit
      = (ed.dataType == "RA" || ed.dataType == "RI") ? ev.ODlimit : 1e+99;
  ...
  for (int ii = 0; ii < dd.scanData.size (); ii++)
    {
      if (ev.excludes.contains (ii))
        continue;

      Scan sc;
      copyRange (ev.rangeLeft, ev.rangeRight, dd.scanData[ii], sc, dd.xvalues);
...
void
US_DataIO::adjust_interference (RawData &data, const EditValues &ev)
{
  // Find first scan
  for (int ii = 0; ii < data.scanData.size (); ii++)
    {
...
void
US_DataIO::calc_integral (RawData &data, const EditValues &ee)
{
...
void
US_DataIO::copyRange (double left, double right, const Scan &orig, Scan &dest,
...
void
US_DataIO::copyxRange (double left, double right, const QVector<double> &origx,
...
bool
US_DataIO::spike_check (const US_DataIO::Scan &sc,
                        const QVector<double> &xvalues, int point, int start,
                        int end, double *value)
{
  ...
  US_Math2::linefit (&xx, &yy, &slope, &intercept, &sigma, &correlation,
                     count);
...
QList<double>
US_DataIO::calc_residuals (int order, const QVector<Scan> &sl)
{
...
int
US_DataIO::index (US_DataIO::RawData *rdata, double xvalue)
{
...
// Return index of X (radius or wavelength) value
int
US_DataIO::index (US_DataIO::EditedData *edata, double xvalue)
{
...
// Return index of X (radius or wavelength) value
int
US_DataIO::index (const QVector<double> &xvals, double xvalue)
{
  ...
  if (xvalue >= xvals[last])
    return last; // Return last index
...
QString
US_DataIO::errorString (int code)
{
  switch (code)
    {
    case OK:
    ...
    case NOT_USDATA:
      return QObject::tr ("The file was not valid scan data");
    case BADTYPE:
      return QObject::tr ("The filetype was not recognized");
    case BADXML:
      return QObject::tr ("The XML file was invalid");
    case NODATA:
      return QObject::tr (
