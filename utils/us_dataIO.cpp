...
int US_DataIO::RawData::xindex(double xvalue) { return index(xvalues, xvalue); }

// Get the X value (radius) at a given index
double US_DataIO::RawData::radius(int radx) { return xvalues[radx]; }

// Get the X value (wavelength) at a given index
double US_DataIO::RawData::scWavelength(int wavx) { return xvalues[wavx]; }
...
// Get the X value (radius) at a given index
double US_DataIO::EditedData::radius(int radx) { return xvalues[radx]; }

// Get the X value (wavelength) at a given index
double US_DataIO::EditedData::scWavelength(int wavx) { return xvalues[wavx]; }
...
bool US_DataIO::readLegacyFile(const QString& file, BeckmanRawScan& data) {
  ...
  while (!ts.atEnd()) {
    ...
    if (pp.size() == 1) pp = sc.split("\t", QString::SkipEmptyParts);
    if (pp.size() < 2) break;
    ...
    if (!interference_data && pp.size() > 2) {
      sval = pp[2].toDouble();
      if (sval != 0.0) data.nz_stddev = true;
    }
  ...
  if (!data.nz_stddev) data.stddevs.clear();
...
void US_DataIO::writeScan(QDataStream& ds, const Scan& data, quint32& crc,
                          const Parameters& pp) {
  union {
    quint32 u;
    float f;
...
void US_DataIO::run(QXmlStreamReader& xml, EditValues& parameters) {
  ...
  if (parameters.wavelength.contains("-")) all_lambdas(xml, parameters);
...
void US_DataIO::params(QXmlStreamReader& xml, EditValues& parameters) {
  ...
  while (!xml.atEnd()) {
    if (xml.isEndElement() && xml.name() == "parameters")
      ...
      if (xml.isEndElement() && xml.name() == "speed") spx++;
      ...
      if (isEquil) parameters.speedData[spx].meniscus = parameters.meniscus;
    }
...
int US_DataIO::loadData(const QString& directory, const QString& editFilename,
...
int US_DataIO::loadData(const QString& directory, const QString& editFilename,
  ...
  if (result == OK) data = editedData[0];
...
int US_DataIO::loadData(const QString& directory, const QString& editFilename,
  ...
  if (ed.expType == "Equilibrium") ed.speedData << ev.speedData;
...
void US_DataIO::adjust_interference(RawData& data, const EditValues& ev) {
  ...
  for (int ii = 0; ii < data.scanData.size(); ii++) {
    ...
    for (int kk = r_left; kk <= r_right; kk++) sum += data.reading(ii, kk);
    ...
    for (int jj = ii + 1; jj < data.scanData.size(); jj++) {
      ...
      for (int kk = 0; kk < sc->rvalues.size(); kk++) sc->rvalues[kk] -= delta;
    }
...
void US_DataIO::calc_integral(RawData& data, const EditValues& ee) {
  ...
  for (int scx = 1; scx < included.size(); scx++) {
    ...
    if (fabs(diff2 / diff1) < ee.gapTolerance) {
      ...
      for (int rr = r_left; rr <= r_right; rr++) sc->rvalues[rr] += 1.0;
...
void US_DataIO::copyRange(double left, double right, const Scan& orig,
  ...
  for (int ii = index_L; ii <= index_R; ii++) {
    ...
    if (orig.nz_stddev) dest.stddevs << orig.stddevs[ii];
  ...
  if (orig.nz_stddev) {  // Likely some stddevs non-zero, but double-check since
                         // range changed
    int nnz = 0;

    for (int ii = 0; ii < dest.stddevs.count(); ii++)
      if (dest.stddevs[ii] != 0.0) nnz++;
...
void US_DataIO::copyxRange(double left, double right,
  ...
  for (int ii = index(origx, left); ii <= index(origx, right);
       ii++) {  // Copy all X values within the specified range
    destx << origx[ii];
  }
}
...
bool US_DataIO::spike_check(const US_DataIO::Scan& sc,
                            const QVector<double>& xvalues, int point,
...
QList<double> US_DataIO::calc_residuals(int order, const QVector<Scan>& sl) {
  ...
  for (int ii = 0; ii < scan_count; ii++) scan_time[ii] = sl[ii].seconds;
...
int US_DataIO::index(const QVector<double>& xvals, double xvalue) {
  ...
  for (int ii = 1; ii < npoint; ii++) {
    ...
    if (rdiff < rdmin) {  // Save the minimum difference value; and its index
                          // for possible use
      rdmin = rdiff;
      mindx = ii;
    }
