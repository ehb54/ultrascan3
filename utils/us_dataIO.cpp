...
bool US_DataIO::RawData::setValue(int scnx, int radx, double value) {
   if (scnx < 0 || scnx >= scanData.size() || radx < 0 || radx >= scanData[ scnx ].rvalues.size())
      return false;
...
bool US_DataIO::readLegacyFile(const QString &file, BeckmanRawScan &data) {
...
void US_DataIO::writeScan(QDataStream &ds, const Scan &data, quint32 &crc, const Parameters &pp) {
   union {
         quint32 u;
         float f;
   } uf;
   ...
   bool stdDev = ((pp.min_data2 != 0.0 || pp.max_data2 != 0.0) && (data.stddevs.size() == data.rvalues.size()));
...
void US_DataIO::params(QXmlStreamReader &xml, EditValues &parameters) {
   ...
   while (!xml.atEnd()) {
      if (xml.isEndElement() && xml.name() == "parameters")
         ...
         parameters.speedData[ spx ].speed = a.value("value").toString().toDouble();
         parameters.speedData[ spx ].first_scan = a.value("scanStart").toString().toInt();
         parameters.speedData[ spx ].scan_count = a.value("scanCount").toString().toInt();
      }
...
int US_DataIO::loadData(const QString &directory, const QString &editFilename, QVector<EditedData> &data) {
   QVector<RawData> raw;

   return loadData(directory, editFilename, data, raw);
}

int US_DataIO::loadData(const QString &directory, const QString &editFilename, EditedData &data) {
   QVector<RawData> raw;
...
int US_DataIO::loadData(
   const QString &directory, const QString &editFilename, QVector<EditedData> &data, QVector<RawData> &raw) {
   ...
   ed.ODlimit = (ed.dataType == "RA" || ed.dataType == "RI") ? ev.ODlimit : 1e+99;
   ...
   if (ev.removeSpikes) {
      ...
      for (int ii = 0; ii < ed.scanData.size(); ii++) {
         ...
         for (int jj = start; jj < end; jj++) {
            if (spike_check(*sc, ed.xvalues, jj, start, end, &smoothed_value)) {
               ...
               while (cc & (1 << (7 - index % 8))) {
                  if (spike_check(*sc, ed.xvalues, index, start, end, &smoothed_value))
...
void US_DataIO::copyRange(double left, double right, const Scan &orig, Scan &dest, const QVector<double> &origx) {
   ...
   if (orig.nz_stddev) { // Likely some stddevs non-zero, but double-check since range changed
      int nnz = 0;
...
void US_DataIO::copyxRange(double left, double right, const QVector<double> &origx, QVector<double> &destx) {
   for (int ii = index(origx, left); ii <= index(origx, right); ii++) { // Copy all X values within the specified range
      destx << origx[ ii ];
   }
}

// Determine if a readings value is a spike
bool US_DataIO::spike_check(
   const US_DataIO::Scan &sc, const QVector<double> &xvalues, int point, int start, int end, double *value) {
   ...
   US_Math2::linefit(&xx, &yy, &slope, &intercept, &sigma, &correlation, count);
...
QList<double> US_DataIO::calc_residuals(int order, const QVector<Scan> &sl) {
...
int US_DataIO::index(const QVector<double> &xvals, double xvalue) {
   ...
   for (int ii = 1; ii < npoint; ii++) {
      ...
      if (rdiff < rdmin) { // Save the minimum difference value; and its index for possible use
         rdmin = rdiff;
         mindx = ii;
      }
   }
...
QString US_DataIO::errorString(int code) {
   switch (code) {
      ...
      case NODATA:
         return QObject::tr("No legacy data files suitable for import were found");
      case BAD_VERSION:
