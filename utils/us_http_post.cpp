...
US_HttpPost::US_HttpPost (const QString &url, const QString &request)
    : QObject ()
{
  ...
  connect (reply, SIGNAL (finished (void)), this, SLOT (postFinished (void)));

  connect (reply, SIGNAL (error (QNetworkReply::NetworkError)), this,
           SLOT (postError (QNetworkReply::NetworkError)));
}

void
US_HttpPost::postFinished (void)
{
...
void
US_HttpPost::postError (QNetworkReply::NetworkError error)
{
  ...
  if (count == 0)
    {
      count++;
      QString err = QString ("9-Network Error (%1): ").arg (error)
                    + reply->errorString ();
