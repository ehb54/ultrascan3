...
QString US_DataFiles::get_filename(
   const QString &path, const QString &guid, const QString &lfchar, const QString &lkupTag, const QString &lkupAtt,
   bool &newFile) {
   QDir f(path);
   ...
   for (int ii = 0; ii < f_names.size(); ii++) { // Browse all the existing files in the directory
      QString fname = f_names[ ii ];

      if (!guid.isEmpty()) { // If guid is not empty, search for a match in existing files
         ...
         while (!xml.atEnd()) { // Search for a matching tag and attribute
            xml.readNext();

            if (xml.isStartElement()) {
               if (xml.name() == lkupTag) { // Found the look-up tag
                  QXmlStreamAttributes a = xml.attributes();

                  if (a.value(lkupAtt).toString() == guid) { // There is a match of an attribute value to the GUID
      ...
      if (newFile) { // No match yet found:  look for a gap in numbering
         if (ofname.isEmpty()) { // No gap was previously found
   ...
   if (ofname.isEmpty()) { // No match and no numeric gap, so bump to number past last existing file
      if (f_names.size() > 0)
