...
#ifdef NO_DB
bool US_DB2::test_db_connection(const QString&, const QString&, const QString&,
                                const QString&, QString&) {
  return false;
}
#else
bool US_DB2::test_db_connection(const QString& host, const QString& dbname,
  ...
  bool status = mysql_real_connect(
      ...
      password.toLatin1().constData(), dbname.toLatin1().constData(), uport,
...
#ifdef NO_DB
bool US_DB2::test_secure_connection(const QString&, const QString&,
                                    const QString&, const QString&,
                                    const QString&, const QString&, QString&) {
  return false;
}
#else
bool US_DB2::test_secure_connection(const QString& host, const QString& dbname,
                                    const QString& user,
  ...
  mysql_ssl_set(db, keyFile.toLatin1(), certFile.toLatin1(), caFile.toLatin1(),
                NULL, CIPHER);
...
#else
bool US_DB2::connect(const QString& host, const QString& dbname,
                     const QString& user, const QString& password,
                     QString& error) {
  if (connected) return true;
...
QVariant US_DB2::value(unsigned index) {
  if (row && (index < mysql_field_count(db))) return row[index];

  return QVariant::Invalid;
}
#endif

bool US_DB2::isConnected(void) { return connected; }
...
#else
int US_DB2::writeBlobToDB(const QString& filename, const QString& procedure,
                          const int tableID) {
  ...
  if (tableID == 0) {
    error = QString("writeBlob: don't know which record data belongs to in ") +
            filename;
    db_errno = DBERROR;
    return DBERROR;
  }
  ...
  QByteArray sqlQuery(escaped_length + queryPart1.size() + checksum.size() + 7,
                      '\0');
  ...
  /*
     ...
     ); qDebug() << "check2 " << memcmp( sqlQuery.data(), sqlQuery2.data(),
     checkSize );
  */
  ...
  if (mysql_query(db, sqlQuery.data()) != 0) {
    error =
        QString("MySQL error: ") +
        mysql_error(
            db);  // ALEXEY: writing huge blob, MySql server has gone away...
...
#else
int US_DB2::readBlobFromDB(const QString& filename, const QString& procedure,
                           const int tableID) {
...
#ifdef NO_DB
unsigned long US_DB2::mysqlEscapeString(QByteArray&, QByteArray&,
                                        unsigned long) {
  return 0;
}
#else
unsigned long US_DB2::mysqlEscapeString(QByteArray& to, QByteArray& from,
