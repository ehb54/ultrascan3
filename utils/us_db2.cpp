...
#define CIPHER                                                                \
  "AES256-GCM-SHA384:AES256-SHA:AES256-SHA256:AES256-CCM:AES256-GCM-SHA384:"  \
  "AES128-SHA"
...
#ifdef NO_DB
bool
US_DB2::test_db_connection (const QString &, const QString &, const QString &,
...
#else
bool
US_DB2::test_db_connection (const QString &host, const QString &dbname,
                            ...
                            QString &error)
{
  ...
  bool status = mysql_real_connect (
      conn, uhost.toLatin1 ().constData (), user.toLatin1 ().constData (),
      password.toLatin1 ().constData (), dbname.toLatin1 ().constData (),
...
#ifdef NO_DB
bool
US_DB2::test_secure_connection (const QString &, const QString &,
...
#else
bool
US_DB2::test_secure_connection (const QString &host, const QString &dbname,
                                const QString &user, const QString &password,
                                ...
                                QString &err)
{
  ...
  mysql_ssl_set (db, keyFile.toLatin1 (), certFile.toLatin1 (),
                 caFile.toLatin1 (), NULL, CIPHER);

  QString uhost = host.section (":", 0, 0).simplified ();
  int uport = host.section (":", 1, 1).simplified ().toInt ();

  bool status = mysql_real_connect (
      db, uhost.toLatin1 ().constData (), user.toLatin1 ().constData (),
      password.toLatin1 ().constData (), dbname.toLatin1 ().constData (),
      uport, NULL, CLIENT_MULTI_STATEMENTS);
  ...
  if (!status)
    {
      error
          = QString ("Test secure connection open error\n") + mysql_error (db);
      err = mysql_error (db);
      return false;
    }
...
#ifdef NO_DB
bool
US_DB2::connect (const QString &, QString &)
{
  return false;
}
#else
bool
US_DB2::connect (const QString &masterPW, QString &err)
{
  if (connected)
    return true;
...
#ifdef NO_DB
bool
US_DB2::connect (const QString &, const QString &, const QString &,
                 const QString &, QString &)
{
  return false;
}
#else
bool
US_DB2::connect (const QString &host, const QString &dbname,
                 const QString &user, const QString &password, QString &error)
{
  if (connected)
    return true;
...
#ifdef NO_DB
void
US_DB2::rawQuery (const QString &)
{
}
#else
void
US_DB2::rawQuery (const QString &sqlQuery)
{
...
#ifdef NO_DB
int
US_DB2::statusQuery (const QString &)
{
  return 0;
}
#else
int
US_DB2::statusQuery (const QString &sqlQuery)
{
  db_errno = DBERROR;
...
int
US_DB2::statusQuery (const QStringList &arguments)
{
  return statusQuery (buildQuery (arguments));
}

int
US_DB2::functionQuery (const QStringList &arguments)
{
...
#ifdef NO_DB
void
US_DB2::query (const QString &)
{
}
#else
void
US_DB2::query (const QString &sqlQuery)
{
  this->rawQuery (sqlQuery);
  if (result)
    {
...
void
US_DB2::query (const QStringList &arguments)
{
  query (buildQuery (arguments));
}

QString
US_DB2::buildQuery (const QStringList &arguments)
{
  QString newquery
      = "CALL " + arguments[0] + "('" + guid + "', '" + userPW + "'";
...
QString
US_DB2::buildQuerySelect (const QStringList &arguments)
{
  QString newquery
      = "SELECT " + arguments[0] + "('" + guid + "', '" + userPW + "'";
...
#ifdef NO_DB
QVariant
US_DB2::value (unsigned)
{
  return QVariant::Invalid;
}
#else
QVariant
US_DB2::value (unsigned index)
{
...
#ifdef NO_DB
int
US_DB2::writeBlobToDB (const QString &, const QString &, const int)
{
  return 0;
}
#else
int
US_DB2::writeBlobToDB (const QString &filename, const QString &procedure,
                       const int tableID)
{
  ...
  if (tableID == 0)
    {
      error
          = QString ("writeBlob: don't know which record data belongs to in ")
            + filename;
      db_errno = DBERROR;
      return DBERROR;
    }
  ...
  ulong escaped_length
      = mysqlEscapeString (blobData_escaped, blobData, blobData.size ());
  ...
  QString queryPart1 = "CALL " + procedure + "('" + guid + "', '" + userPW
                       + "', " + QString::number (tableID) + ", '";
  QByteArray sqlQuery (
      escaped_length + queryPart1.size () + checksum.size () + 7, '\0');
  ...
  /*
     ...
     ulong checkSize = escaped_length + queryPart1.size() + checksum.size() +
     7; qDebug() << ( ( sqlQuery == sqlQuery2 ) ? "check equal" : "check not
     equal" ); qDebug() << "check2 " << memcmp( sqlQuery.data(),
     sqlQuery2.data(), checkSize );
  */
  ...
  if (mysql_query (db, sqlQuery.data ()) != 0)
    {
      error = QString ("MySQL error: ")
              + mysql_error (db); // ALEXEY: writing huge blob, MySql server
                                  // has gone away...
...
#ifdef NO_DB
int
US_DB2::readBlobFromDB (const QString &, const QString &, const int)
{
  return 0;
}
#else
int
US_DB2::readBlobFromDB (const QString &filename, const QString &procedure,
                        const int tableID)
{
  // First let's build the query
  QString sqlQuery = "CALL " + procedure + "('" + guid + "', '" + userPW
                     + "', " + QString::number (tableID) + ")";
  ...
  if (mysql_next_result (db) == 0)
    {
      ...
      QByteArray calculated
          = QCryptographicHash::hash (aucData, QCryptographicHash::Md5)
                .toHex ();
...
#ifdef NO_DB
unsigned long
US_DB2::mysqlEscapeString (QByteArray &, QByteArray &, unsigned long)
{
  return 0;
}
#else
unsigned long
US_DB2::mysqlEscapeString (QByteArray &to, QByteArray &from,
...
#ifdef NO_DB
int
US_DB2::writeAucToDB (const QString &, int)
{
  return 0;
}
#else
int
US_DB2::writeAucToDB (const QString &filename, int tableID)
{
...
#ifdef NO_DB
int
US_DB2::readAucFromDB (const QString &, int)
{
  return 0;
}
#else
int
US_DB2::readAucFromDB (const QString &filename, int tableID)
{
