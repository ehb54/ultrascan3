...
#ifdef NO_DB
bool US_DB2::test_db_connection(const QString &, const QString &, const QString &, const QString &, QString &) {
   return false;
}
#else
bool US_DB2::test_db_connection(
   const QString &host, const QString &dbname, const QString &user, const QString &password, QString &error) {
   error = "";
   ...
   bool status = mysql_real_connect(
      conn, uhost.toLatin1().constData(), user.toLatin1().constData(), password.toLatin1().constData(),
      dbname.toLatin1().constData(), uport, NULL, CLIENT_MULTI_STATEMENTS);
...
bool US_DB2::test_secure_connection(
   const QString &, const QString &, const QString &, const QString &, const QString &, const QString &, QString &) {
   return false;
}
#else
bool US_DB2::test_secure_connection(
   const QString &host, const QString &dbname, const QString &user, const QString &password, const QString &email,
   const QString &pw, QString &err) {
   error = "";
   ...
   mysql_ssl_set(db, keyFile.toLatin1(), certFile.toLatin1(), caFile.toLatin1(), NULL, CIPHER);

   QString uhost = host.section(":", 0, 0).simplified();
   int uport = host.section(":", 1, 1).simplified().toInt();

   bool status = mysql_real_connect(
      db, uhost.toLatin1().constData(), user.toLatin1().constData(), password.toLatin1().constData(),
      dbname.toLatin1().constData(), uport, NULL, CLIENT_MULTI_STATEMENTS);
...
#ifdef NO_DB
bool US_DB2::connect(const QString &, QString &) {
   return false;
}
#else
...
#ifdef NO_DB
bool US_DB2::connect(const QString &, const QString &, const QString &, const QString &, QString &) {
   return false;
}
#else
bool US_DB2::connect(
...
#ifdef NO_DB
int US_DB2::statusQuery(const QString &) {
   return 0;
}
#else
...
QString US_DB2::buildQuery(const QStringList &arguments) {
   QString newquery = "CALL " + arguments[ 0 ] + "('" + guid + "', '" + userPW + "'";
...
QString US_DB2::buildQuerySelect(const QStringList &arguments) {
   QString newquery = "SELECT " + arguments[ 0 ] + "('" + guid + "', '" + userPW + "'";
...
#ifdef NO_DB
QVariant US_DB2::value(unsigned) {
...
#ifdef NO_DB
int US_DB2::writeBlobToDB(const QString &, const QString &, const int) {
   return 0;
}
#else
int US_DB2::writeBlobToDB(const QString &filename, const QString &procedure, const int tableID) {
   ...
   QByteArray checksum = QCryptographicHash::hash(blobData, QCryptographicHash::Md5).toHex();

   // Now let's start building the query
   QString queryPart1 = "CALL " + procedure + "('" + guid + "', '" + userPW + "', " + QString::number(tableID) + ", '";
   QByteArray sqlQuery(escaped_length + queryPart1.size() + checksum.size() + 7, '\0');
...
#ifdef NO_DB
int US_DB2::readBlobFromDB(const QString &, const QString &, const int) {
   return 0;
}
#else
int US_DB2::readBlobFromDB(const QString &filename, const QString &procedure, const int tableID) {
   // First let's build the query
   QString sqlQuery = "CALL " + procedure + "('" + guid + "', '" + userPW + "', " + QString::number(tableID) + ")";
   ...
   if (mysql_next_result(db) == 0) {
      ...
      QByteArray calculated = QCryptographicHash::hash(aucData, QCryptographicHash::Md5).toHex();
...
#ifdef NO_DB
unsigned long US_DB2::mysqlEscapeString(QByteArray &, QByteArray &, unsigned long) {
   return 0;
}
#else
...
#ifdef NO_DB
int US_DB2::writeAucToDB(const QString &, int) {
   return 0;
}
#else
...
#ifdef NO_DB
int US_DB2::readAucFromDB(const QString &, int) {
   return 0;
}
#else
