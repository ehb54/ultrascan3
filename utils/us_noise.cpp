...
bool US_Noise::operator==(const US_Noise &n) const {
   ...
   if (description != n.description)
      return false;
   if (noiseGUID != n.noiseGUID)
      return false;
   if (modelGUID != n.modelGUID)
      return false;
   if (minradius != n.minradius)
      return false;
   if (maxradius != n.maxradius)
      return false;
   if (values.size() != n.values.size())
      return false;

   for (int ii = 0; ii < values.size(); ii++)
      if (values[ ii ] != n.values[ ii ])
         return false;
...
int US_Noise::write(US_DB2 *db) {
   ...
   if (db->lastErrno() != US_DB2::OK) {
      q << "new_noise" << noiseGUID << editID << modelID << modelGUID << typen << description << contents;
      ...
      q << "update_noise" << noiseID << noiseGUID << editID << modelID << modelGUID << typen << description << contents;
...
int US_Noise::apply_to_data(US_DataIO::EditedData &editdata, bool remove) {
   ...
   if (type == TI) { // Time-invariant:  subtract same value all scans at reading position
      ...
      for (jj = 0; jj < rCount; jj++) { // get constant noise value for each reading and apply to scans
         vnoise = (values[ jj ] * applyf);

         for (ii = 0; ii < sCount; ii++) { // apply to all scans at reading position
            editdata.scanData[ ii ].rvalues[ jj ] += vnoise;
         }
      }
   }

   else { // Radially-invariant:  subtract same value all readings at scan position
      ...
      for (ii = 0; ii < sCount; ii++) { // get constant noise value for each scan and apply to readings
         vnoise = (values[ ii ] * applyf);

         for (jj = 0; jj < rCount; jj++) { // apply to all readings at scan position
            editdata.scanData[ ii ].rvalues[ jj ] += vnoise;
         }
      }
   }
...
int US_Noise::apply_noise(US_DataIO::EditedData &editdata, US_Noise *noise, bool remove) {
   if (noise != 0) {
...
bool US_Noise::sum_noise(US_Noise noise2, bool always_sum) {
   ...
   else { // determine if they match in count and any radius range
      double difflo = qAbs(minradius - noise2.minradius);
      double diffhi = qAbs(maxradius - noise2.maxradius);
      if (count != noise2.count || (type == TI && (difflo > 0.0001 || diffhi > 0.0001))) {
         match = false;
         qDebug() << "SM-NOI: difflo diffhi" << difflo << diffhi;
      }
   }

   if (match) { // if they match, simply sum vector values;
      for (int ii = 0; ii < values.size(); ii++) {
   ...
   else if (always_sum) { // otherwise, we may need to interpolate or only sum a portion

      if (type == RI) { // for radially-invariant, just sum for minimum count
      ...
      else { // for time-invariant,  interpolate by radius location
         qDebug() << "SM-NOI: *NTRP* count count2" << count << noise2.count << "type" << type << "mnmx r" << minradius
                  << noise2.minradius << maxradius << noise2.maxradius;
         ...
         for (int ii = 0; ii < count; ii++) {
            ...
            double val1 = (j1 < 0) ? noise2.values[ 0 ] : ((j1 > limk2) ? noise2.values[ limk2 ] : noise2.values[ j1 ]);
            double val2 = (j2 < 0) ? noise2.values[ 0 ] : ((j2 > limk2) ? noise2.values[ limk2 ] : noise2.values[ j2 ]);
