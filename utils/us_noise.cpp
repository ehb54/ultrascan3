...
int US_Noise::apply_to_data(US_DataIO::EditedData& editdata, bool remove) {
  ...
  if (type == TI) {  // Time-invariant:  subtract same value all scans at
                     // reading position
    ...
    for (jj = 0; jj < rCount; jj++) {  // get constant noise value for each
                                       // reading and apply to scans
  ...
  else {  // Radially-invariant:  subtract same value all readings at scan
          // position
    ...
    for (ii = 0; ii < sCount; ii++) {  // get constant noise value for each scan
                                       // and apply to readings
...
int US_Noise::apply_noise(US_DataIO::EditedData& editdata, US_Noise* noise,
                          bool remove) {
  if (noise != 0) {
...
bool US_Noise::sum_noise(US_Noise noise2, bool always_sum) {
  ...
  else if (always_sum) {  // otherwise, we may need to interpolate or only sum a
                          // portion

    if (type == RI) {  // for radially-invariant, just sum for minimum count
      int mcount = qMin(count, noise2.count);

      for (int ii = 0; ii < mcount; ii++) values[ii] += noise2.values[ii];
