...
// equality operator
bool
US_Noise::operator== (const US_Noise &n) const
{
  if (type != n.type)
    return false;
...
int
US_Noise::load (bool db_access, const QString &guid, US_DB2 *db)
{
  if (db_access)
...
// load noise from db
int
US_Noise::load (const QString &id, US_DB2 *db)
{
  QStringList q;
...
int
US_Noise::load (const QString &filename)
{
  QFile file (filename);
...
int
US_Noise::load_stream (QXmlStreamReader &xml)
{
...
int
US_Noise::write (bool db_access, const QString &filename, US_DB2 *db)
{
  if (db_access)
    return write (db);
  else
...
US_Noise::write (US_DB2 *db)
{
  ...
  if (db->lastErrno () != US_DB2::OK)
    {
      q << "new_noise" << noiseGUID << editID << modelID << modelGUID << typen
...
int
US_Noise::write (const QString &filename)
{
  QFile file (filename);
...
int
US_Noise::apply_to_data (US_DataIO::EditedData &editdata, bool remove)
{
  ...
  if (type == TI)
    { // Time-invariant:  subtract same value all scans at reading position
      ...
      if (count != rCount)
        {
          message
              = QObject::tr ("Noise count does not equal data readings count");
          return -1;
        }
...
int
US_Noise::apply_noise (US_DataIO::EditedData &editdata, US_Noise *noise,
                       bool remove)
{
  if (noise != 0)
    {
...
bool
US_Noise::noise_path (QString &path)
{
  QDir dir;
...
int
US_Noise::load_disk (const QString &guid)
{
...
int
US_Noise::load_db (const QString &guid, US_DB2 *db)
{
  QStringList q;
...
void
US_Noise::write_stream (QXmlStreamWriter &xml)
{
  QString typen ("ti");
...
bool
US_Noise::sum_noise (US_Noise noise2, bool always_sum)
{
  bool ok = true;
  bool match = true;
      ...
      for (int ii = 0; ii < values.size (); ii++)
        {
          ...
          if (isNan (values[ii]))
            qDebug () << "SM-NOI: ii" << ii << "noi o i" << values[ii]
                      << noise2.values[ii];
        }
    }
...
bool
US_Noise::sum_noises (US_Noise &noise1, US_Noise noise2, bool always_sum)
{
