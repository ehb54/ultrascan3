...
US_dmGA_Constraints::US_dmGA_Constraints(US_Model* imodel) {
  ...
  if (imodel != NULL) {
    if (imodel->analysis ==
        US_Model::DMGA_CONSTR) {  // The input is a constraints model
      cmodel = *imodel;
...
US_dmGA_Constraints::~US_dmGA_Constraints() { attribs.clear(); }
...
int US_dmGA_Constraints::update_constraints(QVector<Constraint>& cnsv) {
  ...
  if (nupdc < 1) return 0;     // Nothing to do if no updates
  ...
  if (cnsv[0].atype <
      ATYPE_KD) {  // Count old constraints for specified component
  ...
  if (nupdc == ncompc) {  // Same number:  just replace existing elements
    ...
    for (int ii = 0; ii < nupdc; ii++, kk++) {
      ...
      DbgLv(1) << "dgC:upd_cns:   att" << kk << "flt" << attribs[kk].floats;
           ...
           ncompc) {  // Smaller number of updates:  replace, then compress
    int kk = fcx;

    for (int ii = 0; ii < nupdc; ii++, kk++) attribs[kk] = cnsv[ii];

    int frx = fcx + nupdc;
    int fmx = fcx + ncompc;
    kk = frx;

    for (int ii = fmx; ii < nctot; ii++, kk++) attribs[kk] = attribs[ii];
    ...
    attribs.resize(nctot);
           ...
           0) {  // If no previous components of this type, just append
    for (int ii = 0; ii < nupdc; ii++) attribs << cnsv[ii];
    ...
    for (int ii = ncold - 1; ii >= fmx; ii--, kk--) attribs[kk] = attribs[ii];

    for (int ii = 0, kk = fcx; ii < nupdc; ii++, kk++) attribs[kk] = cnsv[ii];
  }
...
int US_dmGA_Constraints::add_constraint(AttribType atype, int mcompx,
                                        double low, double high, bool floats,
                                        bool logscl) {
...
int US_dmGA_Constraints::comp_constraints(int compx, QVector<Constraint>* cnsvP,
                                          int* kfltP) {
  ...
  for (int ii = 0; ii < attribs.size(); ii++) {
    ...
    if (attribs[ii].atype >= ATYPE_KD) break;  // Done if to associations

    if (attribs[ii].mcompx != compx) continue;  // Skip if not specified comp

    kattrib++;                         // Bump comp's attrib count
    cnsv << attribs[ii];               // Save attribute for comp
    if (attribs[ii].floats) kfloat++;  // Bump attrib's floats count
  }
  DbgLv(1) << "dgC:cmp_cns: ka kf cnssz" << kattrib << kfloat << cnsv.size();

  if (cnsvP != NULL) *cnsvP = cnsv;  // Return constraints vector

  if (kfltP != NULL) *kfltP = kfloat;  // Return floats count
...
int US_dmGA_Constraints::assoc_constraints(int assox,
  ...
  for (int ii = 0; ii < attribs.size(); ii++) {
    if (attribs[ii].atype < ATYPE_KD) continue;  // Skip if not association

    if (attribs[ii].mcompx != assox) continue;  // Skip if not specified assoc

    kattrib++;                         // Bump assoc's attrib count
    cnsv << attribs[ii];               // Save attribute for assoc
    if (attribs[ii].floats) kfloat++;  // Bump attrib's floats count
    DbgLv(1) << "dgC:ass_cns:  ii" << ii << "kattrib kfloat" << kattrib
             << kfloat;
  }
...
void US_dmGA_Constraints::constraints_from_model() {
  ...
  for (int ii = 0; ii < cmodel.components.size();
       ii++) {  // Scan constraints model components (solutes)
    pcname = cname;
    ...
    DbgLv(1) << "dgC:cnsfrmo:  ii" << ii << "s k w d f v" << sval << kval
             << wval << dval << fval << vval;
    double xval = 0.0;
    double yval = 0.0;
    double zval = 0.0;
    ...
    if (tflag.contains("V")) {  // All fixed attributes
      ...
      if (is_low) {  // First of two:  just get low values
        ...
        attribs << attr;  // Save X attribute constraint
        DbgLv(1) << "dgC:cnsfrmo:      xattr xlow xhigh" << x_attr << xlow
                 << xval;
        ...
        attribs << attr;  // Save Y attribute constraint
        DbgLv(1) << "dgC:cnsfrmo:      yattr ylow yhigh" << y_attr << ylow
                 << yval;
        ...
        attribs << attr;  // Save Z attribute constraint
        DbgLv(1) << "dgC:cnsfrmo:      zattr zlow zhigh" << z_attr << zlow
                 << zval;
  ...
  for (int ii = 0; ii < nbassoc;
       ii++) {  // Loop to interpret pairs of constraints model associations
    massox = ii;
    int jj = ii * 2;
    int kk = jj + 1;
    ...
    if (dval == dhigh && oval == ohigh) {  // All fixed
      ...
      DbgLv(1) << "dgC:cnsfrmo: assoc float dlow dhigh olow ohigh" << dval
               << dhigh << oval << ohigh << "nfloat" << nfloat;
    }  // END: 1 or 2 floating
...
void US_dmGA_Constraints::model_from_constraints() {
  ...
  for (int ii = 0; ii < attribs.size(); ii++) {
    ...
    nfloat += (floats ? 1 : 0);
    DbgLv(1) << "dgC:modfrcn:scn:   ii" << ii << "  mcompx" << mcompx << "atype"
             << atype << "floats" << floats;
  ...
  nccomp = nbcomp + ncflt;
  ncassoc = nbassoc * 2;
  DbgLv(1) << "dgC:modfrcn: ncc nca" << nccomp << ncassoc << "nbc ncf" << nbcomp
           << ncflt << "nba" << nbassoc << "nf" << nfloat;
  mcompx = attribs[0].mcompx;
  DbgLv(1) << "dgC:modfrcn:  bmodel compsz assosz" << bmodel.components.size()
           << bmodel.associations.size();
  int kfloat = 0;
  int kcompo = 0;
  int kassoc = 0;
  int pmcomx = mcompx;
  ...
  if (bmodel.associations.size() > 0) as1 = bmodel.associations[0];
  ...
  for (int ii = 0; ii < attribs.size(); ii++) {
    ...
    if (mcompx !=
        pmcomx) {  // Initialize component or association for new base component
      if (atype < ATYPE_KD) {  // New component
        ...
        if (ptype < ATYPE_KD) {  // Save last component entry
          if (kfloat ==
              0) {  // If no floats in component, just output one record
            sc1.name = pname + QString(sc1.name);
            cmodel.components << sc1;
            DbgLv(1) << "dgC:modfrcn:cre:    ii" << ii << "sc1.name"
                     << sc1.name;
          }
            ...
            DbgLv(1) << "dgC:modfrcn:cre:    ii" << ii << "sc1.name"
                     << sc1.name;
            DbgLv(1) << "dgC:modfrcn:cre:    ii" << ii << "sc2.name"
                     << sc2.name;
          }
        }
...
void US_dmGA_Constraints::constraints_from_base() {
  ...
  for (int ii = 0; ii < bmodel.components.size();
       ii++) {  // Scan constraints model components (solutes)
  ...
  for (int ii = 0; ii < nbassoc;
       ii++) {  // Loop to interpret constraints model associations
...
int US_dmGA_Constraints::count_comp_constraints(int mcx, int* faxP,
  ...
  for (int ii = 0; ii < attribs.size(); ii++) {  // Scan all constraints
    ...
    if (mcompx != mcx) continue;  // Skip all non-matching component indecies

    kconstr++;                    // Bump constraints count
    if (fattrx < 0) fattrx = ii;  // Save first index of a match

    if (attribs[ii].floats) kfloat++;  // Bump count if this attribute floats
    DbgLv(1) << "dgC:ccc: ii" << ii << "kconstr" << kconstr << "fattrx kfloat"
             << fattrx << kfloat;
  }

  if (faxP != NULL) *faxP = fattrx;  // Return first constraints index of match
...
int US_dmGA_Constraints::count_asso_constraints(int msx, int* faxP,
  ...
  for (int ii = 0; ii < attribs.size(); ii++) {  // Scan all constraints
    ...
    if (massox != msx) continue;  // Skip all non-matching association indecies
...
bool US_dmGA_Constraints::init_work_model() {
  ...
  for (int ii = 0; ii < cmodel.components.size(); ii++) {
    ...
    if (kfloat > 0) {  // Floats:  loop to find values half-way through ranges
      ...
      for (int jj = 0; jj < kconstr; jj++) {
        if (cnsv[jj]
                .floats) {  // This is a floating attribute: store mid-way value
    ...
    DbgLv(1) << "dgC:inwkmdl:     out sc1  xval0"
             << fetch_attrib(sc1, cnsv[0].atype) << "mcompx" << mcompx;
  ...
  for (int ii = 0; ii < cmodel.associations.size(); ii += 2) {
    ...
    DbgLv(1) << "dgC:inwkmdl:     out as1  xval0"
             << fetch_attrib(as1, cnsv[0].atype) << "mcompx" << mcompx;
    mcompx++;
  }

  if (bmodel.components.size() == 0) bmodel = wmodel;
...
bool US_dmGA_Constraints::base_from_cmodel() {
  ...
  bmodel.description =
      QString(cmodel.description).replace("DMGA_Constraints", "DMGA_Base");
  bmodel.editGUID = "";
  ...
  for (int ii = 0; ii < cmodel.components.size(); ii++) {
    ...
    if (!aflg.contains("V")) {  // If float pair, get mid-way attribute values
      ...
      sc1.signal_concentration =
          (sc1.signal_concentration + sc2.signal_concentration) * 0.5;
    }
