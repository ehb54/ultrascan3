...
// Load a base model
void
US_dmGA_Constraints::load_base_model (US_Model *bmodelP)
{
  bmodel = *bmodelP;
  cmodel = bmodel;
  dbg_level = US_Settings::us_debug ();
...
void
US_dmGA_Constraints::load_constraints (US_Model *cmodelP)
{
  cmodel = *cmodelP;
  dbg_level = US_Settings::us_debug ();
...
int
US_dmGA_Constraints::update_constraints (QVector<Constraint> &cnsv)
{
  ...
  if (nupdc == ncompc)
    { // Same number:  just replace existing elements
      ...
      for (int ii = 0; ii < nupdc; ii++, kk++)
        {
          attribs[kk] = cnsv[ii];
          DbgLv (1) << "dgC:upd_cns:   att" << kk << "flt"
                    << attribs[kk].floats;
        }
    }
...
void
US_dmGA_Constraints::load_constraints (QVector<Constraint> &cnsv)
{
...
void
US_dmGA_Constraints::init_constraints ()
{
  dbg_level = US_Settings::us_debug ();
...
int
US_dmGA_Constraints::add_constraint (AttribType atype, int mcompx, double low,
                                     double high, bool floats, bool logscl)
{
...
bool
US_dmGA_Constraints::get_base_model (US_Model *bmodelP)
{
  bool is_ok = false;
...
bool
US_dmGA_Constraints::get_constr_model (US_Model *cmodelP)
{
  bool is_ok = false;
...
bool
US_dmGA_Constraints::get_work_model (US_Model *wmodelP)
{
  bool is_ok = false;
...
int
US_dmGA_Constraints::comp_constraints (int compx, QVector<Constraint> *cnsvP,
                                       int *kfltP)
{
...
int
US_dmGA_Constraints::assoc_constraints (int assox, QVector<Constraint> *cnsvP,
                                        int *kfltP)
{
...
int
US_dmGA_Constraints::float_constraints (QVector<Constraint> *cnsvP)
{
...
double
US_dmGA_Constraints::fetch_attrib (US_Model::SimulationComponent &sc,
...
double
US_dmGA_Constraints::fetch_attrib (US_Model::Association &as,
                                   const AttribType atype)
{
  double xval = 0.0;
...
void
US_dmGA_Constraints::store_attrib (US_Model::SimulationComponent &sc,
...
void
US_dmGA_Constraints::store_attrib (US_Model::Association &as,
                                   const AttribType atype, const double xval)
{
  if (atype == ATYPE_KD)
    as.k_d = xval;
...
void
US_dmGA_Constraints::constraints_from_model ()
{
  Constraint attr;
        ...
        { // 1 or 2 attributes are float
          ...
          if (is_low)
            ...
            { // Second of pair:  complete low,high and build constraints
              ...
              attribs << attr; // Save X attribute constraint
              DbgLv (1) << "dgC:cnsfrmo:      xattr xlow xhigh" << x_attr
                        << xlow << xval;
              ...
              attribs << attr; // Save Y attribute constraint
              DbgLv (1) << "dgC:cnsfrmo:      yattr ylow yhigh" << y_attr
                        << ylow << yval;
              ...
              attribs << attr; // Save Z attribute constraint
              DbgLv (1) << "dgC:cnsfrmo:      zattr zlow zhigh" << z_attr
                        << zlow << zval;
...
void
US_dmGA_Constraints::model_from_constraints ()
{
  ...
  for (int ii = 0; ii < attribs.size (); ii++)
    {
      ...
      DbgLv (1) << "dgC:modfrcn:cre:   ii" << ii << "mcx pmcx" << mcompx
                << pmcomx << "typ" << atype << "flt" << floats << "lo,hi"
                << xlow << xhigh;
...
void
US_dmGA_Constraints::constraints_from_base ()
{
...
int
US_dmGA_Constraints::count_comp_constraints (int mcx, int *faxP, int *countfP)
{
  ...
  for (int ii = 0; ii < attribs.size (); ii++)
    { // Scan all constraints
      ...
      DbgLv (1) << "dgC:ccc: ii" << ii << "kconstr" << kconstr
                << "fattrx kfloat" << fattrx << kfloat;
    }
...
int
US_dmGA_Constraints::count_asso_constraints (int msx, int *faxP, int *countfP)
{
...
bool
US_dmGA_Constraints::init_work_model ()
{
...
bool
US_dmGA_Constraints::base_from_cmodel ()
{
  if (cmodel.components.size () < 1)
    {
