...
void US_BufferComponent::getAllFromDB(
...
void US_BufferComponent::getInfoFromDB(US_DB2* db) {
  ...
  for (int i = 0; i < 6; i++) visc_coeff[i] = sl[i].toDouble();

  sl = density.split(" ");

  for (int i = 0; i < 6; i++) dens_coeff[i] = sl[i].toDouble();
...
void US_BufferComponent::getAllFromHD(
  ...
  while (!xml.atEnd()) {
    ...
    if (xml.isStartElement()) {
      if (xml.name() == "component") component(xml, componentList);
    }
  }
...
int US_BufferComponent::saveToDB(US_DB2* db) {
  ...
  for (int i = 0; i < 6; i++) {
    density +=
        QString(i > 0 ? "" : " ") + QString::number(dens_coeff[i], 'f', 5);
  }
  QString viscosity;
  for (int i = 0; i < 6; i++) {
    viscosity +=
        QString(i > 0 ? "" : " ") + QString::number(visc_coeff[i], 'f', 5);
  }
...
int US_Buffer::saveToDB(US_DB2* db, const QString private_buffer) {
  ...
  if (manx > 0) descrip = descrip.left(manx).simplified();
  ...
  else if (status == US_DB2::NOROWS) {  // There is no buffer with the given
                                        // GUID, so create a new one
    q.clear();
    q << "new_buffer" << GUID << descrip
      << QString::number(compressibility, 'e', 4) << QString::number(pH, 'f', 4)
      << QString::number(density, 'f', 6) << QString::number(viscosity, 'f', 5)
  ...
  for (int i = 0; i < ncomp; i++) {
    ...
    q << "add_buffer_component" << QString::number(idBuffer)
      << component[i].componentID << QString::number(concentration[i], 'f', 5);
    db->statusQuery(q);
  ...
  // if ( !extinction.isEmpty() )
  {
    if (!replace_spectrum) {
      ...
      US_ExtProfile::create_eprofile(db, bufferID.toInt(), compType, valType,
                                     extinction);
    } else {
      ...
      qDebug() << "Old Extinction keys: " << extinction.keys().count()
               << ", ProfileID: " << profileID;
      US_ExtProfile::update_eprofile(db, profileID, bufferID.toInt(), compType,
...
bool US_Buffer::readFromDisk(const QString& filename) {
  ...
  while (!xml.atEnd()) {
    ...
    if (xml.isStartElement()) {
      if (xml.name() == "buffer") readBuffer(xml);
    }
  }
...
QString US_Buffer::get_filename(const QString& path, const QString& guid,
                                bool& newFile) {
...
void US_Buffer::readBuffer(QXmlStreamReader& xml) {
  ...
  while (!xml.atEnd()) {
    ...
    if (xml.isStartElement() && xml.name() == "spectrum") readSpectrum(xml);
...
void US_Buffer::compositeCoeffs(double* d_coeff, double* v_coeff) {
  ...
  if (ncomp == 0) return;  // If 0, nothing to do
  ...
  for (int ii = 0; ii < ncomp;
       ii++) {                      // Sum concentration and coefficient values
