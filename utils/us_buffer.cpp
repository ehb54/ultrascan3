...
void US_BufferComponent::getAllFromDB(const QString &masterPW, QMap<QString, US_BufferComponent> &componentList) {
   US_DB2 db(masterPW);

   if (db.lastErrno() != US_DB2::OK) {
      qDebug() << "Database Error" << "US_BufferComponent ::connectDB: Could not open DB\n" << db.lastError();
      return;
   }
...
void US_BufferComponent::component(QXmlStreamReader &xml, QMap<QString, US_BufferComponent> &componentList) {
   ...
   bc.grad_form = US_Util::bool_flag(a.value("gradient_forming").toString());
...
void US_BufferComponent::putAllToHD(const QMap<QString, US_BufferComponent> &componentList) {
   ...
   for (int i = 0; i < keys.size(); i++) {
      ...
      xml.writeAttribute("gradient_forming", US_Util::bool_string(componentList[key].grad_form));
...
void US_CosedComponent::component(QXmlStreamReader &xml, QMap<QString, US_CosedComponent> &componentList) {
   US_CosedComponent bc;

   QXmlStreamAttributes a = xml.attributes();
   bc.id = a.value("id").toInt();
   bc.componentID = a.value("id").toString();
   bc.GUID = a.value("GUID").toString();
   bc.name = a.value("name").toString();
...
void US_CosedComponent::component(QXmlStreamReader &xml, QList<US_CosedComponent> &componentList,
                                  QList<QString> &componentIDList) {
   US_CosedComponent bc;
   ...
   while (!xml.atEnd()) {
      ...
      if (xml.isEndElement() && xml.name() == "cosed_component") {
         componentList << bc;
         componentIDList << bc.componentID;
         return;
      }
...
int US_CosedComponent::saveToDB(US_DB2 *db, const int buffer_ID) {
   ...
   if (status != US_DB2::OK && status != US_DB2::NOROWS) {
      ...
      return -9;
   } else if (status == US_DB2::NOROWS) {  // There is no cosed_component with the given GUID, so create a new one
      // construct density and viscosity
      QString density;
      for (int i = 0; i < 6; i++) {
         density += QString(i > 0 ? "" : " ") + QString::number(dens_coeff[i], 'f', 5);
...
US_CosedComponent US_CosedComponent::clone(void) {
   US_CosedComponent cloned;
   cloned.id = id;
   ...
   for (int i = 0; i < 6; i++) {
      cloned.visc_coeff[i] = visc_coeff[i];
   }
...
bool US_Buffer::writeToDisk(const QString &filename) const {
   ...
   xml.writeAttribute("compressibility", QString::number(compressibility, 'e', 4));
   xml.writeAttribute("manual", US_Util::bool_string(manual));

   for (int i = 0; i < component.size(); i++) {
      xml.writeStartElement("component");
      xml.writeAttribute("id", componentIDs[i]);
      xml.writeAttribute("concentration", QString::number(concentration[i], 'f', 5));
      xml.writeEndElement(); // component
   }
...
//qDebug() << "get_bufferID-stat" << status;
   ...
   if (status != US_DB2::OK && status != US_DB2::NOROWS) {
      ...
      return -9;
   } else if (status == US_DB2::NOROWS) {  // There is no buffer with the given GUID, so create a new one
      q.clear();
      q << "new_buffer" << GUID << descrip << QString::number(compressibility, 'e', 4) << QString::number(pH, 'f', 4)
        << QString::number(density, 'f', 6) << QString::number(viscosity, 'f', 5) << US_Util::bool_string(manual)
...
QString US_Buffer::get_filename(const QString &path, const QString &guid, bool &newFile) {
   return US_DataFiles::get_filename(path, guid, "B", "buffer", "guid", newFile);
}
...
void US_Buffer::readBuffer(QXmlStreamReader &xml) {
   ...
   while (!xml.atEnd()) {
      ...
      if (xml.isStartElement() && xml.name() == "cosed_component") {
         QXmlStreamAttributes a = xml.attributes();
         US_CosedComponent bc;
         bc.component(xml, cosed_component, cosed_componentIDs);
      }
...
void US_Buffer::readSpectrum(QXmlStreamReader &xml) {
   while (!xml.atEnd()) {
      ...
      if (xml.isStartElement() && xml.name() == "extinction") {
         QXmlStreamAttributes a = xml.attributes();
         extinction[a.value("wavelength").toString().toDouble()] = a.value("value").toString().toDouble();
      }

      if (xml.isStartElement() && xml.name() == "refraction") {
         QXmlStreamAttributes a = xml.attributes();
         refraction[a.value("wavelength").toString().toDouble()] = a.value("value").toString().toDouble();
      }

      if (xml.isStartElement() && xml.name() == "fluorescence") {
         QXmlStreamAttributes a = xml.attributes();
         fluorescence[a.value("wavelength").toString().toDouble()] = a.value("value").toString().toDouble();
      }
...
void US_Buffer::compositeCoeffs(double *d_coeff, double *v_coeff) {
   ...
   for (int ii = 0; ii < ncomp; ii++) { // Sum concentration and coefficient values
...
void US_Buffer::compositeCoeffs(QMap<QString, double> & conc, const double& dens_base, const double& visc_base,
                                double *d_coeff, double *v_coeff) {
   int ncomp = cosed_component.size();    // Number of buffer components

   if (ncomp == 0)
      return;                          // If 0, nothing to do

   // Pre-remove water value from components beyond first
   d_coeff[0] = (double) (1 - ncomp) * dens_base;
   v_coeff[0] = (double) (1 - ncomp) * visc_base;
   ...
   for (const US_CosedComponent& cosed_comp : cosed_component) { // Sum concentration and coefficient values
      if (!conc.contains(cosed_comp.componentID)) {
         continue;
      }
      double c1 = conc[cosed_comp.componentID];        // c ^ 1
      double cr = sqrt(c1);                         // c ^ 0.5
      double c2 = c1 * c1;                             // c ^ 2
      double c3 = c2 * c1;                             // c ^ 3
      double c4 = c3 * c1;                             // c ^ 4
      ...
      // Accumulate weighted coefficient terms
      d_coeff[0] += cosed_comp.dens_coeff[0];
      d_coeff[1] += cosed_comp.dens_coeff[1] * cr;
      d_coeff[2] += cosed_comp.dens_coeff[2] * c1;
      d_coeff[3] += cosed_comp.dens_coeff[3] * c2;
      d_coeff[4] += cosed_comp.dens_coeff[4] * c3;
      d_coeff[5] += cosed_comp.dens_coeff[5] * c4;

      v_coeff[0] += cosed_comp.visc_coeff[0];
      v_coeff[1] += cosed_comp.visc_coeff[1] * cr;
      v_coeff[2] += cosed_comp.visc_coeff[2] * c1;
      v_coeff[3] += cosed_comp.visc_coeff[3] * c2;
      v_coeff[4] += cosed_comp.visc_coeff[4] * c3;
      v_coeff[5] += cosed_comp.visc_coeff[5] * c4;
   }
