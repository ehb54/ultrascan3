...
void
US_BufferComponent::getAllFromDB (
    const QString &masterPW, QMap<QString, US_BufferComponent> &componentList)
{
  US_DB2 db (masterPW);
...
void
US_BufferComponent::getInfoFromDB (US_DB2 *db)
{
...
void
US_BufferComponent::getAllFromHD (
...
US_BufferComponent::component (
    QXmlStreamReader &xml, QMap<QString, US_BufferComponent> &componentList)
{
  ...
  if (!file.open (QIODevice::WriteOnly | QIODevice::Text))
    {
      qDebug () << "Cannot open file "
                << US_Settings::appBaseDir () + "/etc/bufferComponents.xml";
      return;
    }
...
int
US_BufferComponent::saveToDB (US_DB2 *db)
{
  QStringList q;
  ...
  for (int i = 0; i < 6; i++)
    {
      density += QString (i > 0 ? "" : " ")
                 + QString::number (dens_coeff[i], 'f', 5);
    }
  QString viscosity;
  for (int i = 0; i < 6; i++)
    {
      viscosity += QString (i > 0 ? "" : " ")
                   + QString::number (visc_coeff[i], 'f', 5);
    }
  ...
  qDebug () << "buffer_component_ID for new buffer component: "
            << idBufferComponent;
...
void
US_Buffer::getSpectrum (US_DB2 *db, const QString &type)
{
  QStringList q;
      ...
      if (type == "Extinction")
        {
          ...
          qDebug () << "Buffer->extinction details: " << lambda << " "
                    << value;
        }
...
void
US_Buffer::putSpectrum (US_DB2 *db, const QString &type) const
{
  QStringList q;
  q << "new_spectrum" << bufferID << "Buffer" << type << "" << "";

  // We have to delete previous buffer->extinction from DB if spectrum updated
  // from 'Edit Existing Buffer'
      ...
      for (int i = 0; i < keys.size (); i++)
        {
          ...
          qDebug () << "Buffer->extinction details: " << wavelength << " "
...
bool
US_Buffer::writeToDisk (const QString &filename) const
{
  QFile file (filename);
  if (!file.open (QIODevice::WriteOnly | QIODevice::Text))
    {
...
bool
US_Buffer::readFromDB (US_DB2 *db, const QString &bufID)
{
  ...
  US_ExtProfile::fetch_eprofile (db, bufferID.toInt (), compType, valType,
                                 extinction);
...
int
US_Buffer::saveToDB (US_DB2 *db, const QString private_buffer)
{
  int idBuffer = 0;
  QStringList q;
  q << "get_bufferID" << GUID;
  ...
  if (status != US_DB2::OK && status != US_DB2::NOROWS)
    ...
    { // There is no buffer with the given GUID, so create a new one
      q.clear ();
      q << "new_buffer" << GUID << descrip
        << QString::number (compressibility, 'e', 4)
        << QString::number (pH, 'f', 4) << QString::number (density, 'f', 6)
        << QString::number (viscosity, 'f', 5) << US_Util::bool_string (manual)
    ...
    {              // The buffer exists, so update it
      ...
      db->statusQuery (q);
      status = db->lastErrno ();
      // qDebug() << "delete_buffer_components status=" << status <<
      // US_DB2::NOROWS;
      if (status != US_DB2::OK && status != US_DB2::NOROWS)
        {
  ...
  for (int i = 0; i < ncomp; i++)
    {
      q.clear ();
      q << "add_buffer_component" << QString::number (idBuffer)
      ...
      if (db->lastErrno () != US_DB2::OK)
        {
          qDebug () << "add_buffer_component i,error=" << i
                    << db->lastErrno ();
          return -4;
        }
    }
      ...
      if (!replace_spectrum)
        {
          ...
          US_ExtProfile::create_eprofile (db, bufferID.toInt (), compType,
                                          valType, extinction);
        }
      else
        {
          ...
          int profileID = US_ExtProfile::fetch_eprofile (
              db, bufferID.toInt (), compType, valType, extinction);

          qDebug () << "Old Extinction keys: " << extinction.keys ().count ()
                    << ", ProfileID: " << profileID;
          US_ExtProfile::update_eprofile (db, profileID, bufferID.toInt (),
...
bool
US_Buffer::readFromDisk (const QString &filename)
{
  QFile file (filename);
...
QString
US_Buffer::get_filename (const QString &path, const QString &guid,
                         bool &newFile)
{
  return US_DataFiles::get_filename (path, guid, "B", "buffer", "guid",
                                     newFile);
}

void
US_Buffer::readBuffer (QXmlStreamReader &xml)
{
  QXmlStreamAttributes a = xml.attributes ();
  ...
  while (!xml.atEnd ())
    {
      if (xml.isEndElement () && xml.name () == "buffer")
        break;
...
void
US_Buffer::readSpectrum (QXmlStreamReader &xml)
{
  while (!xml.atEnd ())
    {
      if (xml.isEndElement () && xml.name () == "spectrum")
        break;
...
void
US_Buffer::compositeCoeffs (double *d_coeff, double *v_coeff)
{
...
void
US_Buffer::dumpBuffer (void) const
{
