...
void
US_ModelRecord::clear_data (void)
{
...
int
US_ModelRecord::compute_slines (double &xmin, double &xmax, double &ymin,
    ...
    { // Loop over k start values
      ...
      for (int jj = 0; jj < nypts; jj++)
        { // Loop over y end values
          ...
          // DbgLv(1) << "MR: ii jj" << ii << jj << "ys ye p1 p2" << ystr <<
          // yend
          //  << mrec.par1 << mrec.par2;
          mrecs << mrec;
...
int
US_ModelRecord::compute_sigmoids (int &ctype, double &xmin, double &xmax,
...
int
US_ModelRecord::compute_hlines (double &xmin, double &xmax, double &ymin,
                                double &ymax, int &nypts, int &nlpts,
                                double *parlims,
                                QVector<US_ModelRecord> &mrecs)
{
...
int
US_ModelRecord::compute_2ndorder (double &xmin, double &xmax, double &ymin,
                                  double &ymax, int &nypts, int &nlpts,
                                  double *parlims,
                                  QVector<US_ModelRecord> &mrecs)
{
        ...
        { // Loop over b values
          ...
          for (int kk = 0; kk < nypts; kk++)
            { // Loop over c values
              ...
              mrec.isolutes.clear ();
              US_ZSolute isol (0.0, 0.0, zval, 0.0);
              DbgLv (1) << "MR: a b c" << aval << bval << cval << " ii jj kk"
                        << ii << jj << kk;
...
int
US_ModelRecord::load_modelrecs (QXmlStreamReader &xml,
                                QVector<US_ModelRecord> &mrecs, QString &descr,
                                int &ctype, double &xmin, double &xmax,
          ...
          if (xmlname == "modelrecords")
            {
              ...
              ctype = sctypes.contains (s_type) ? ctype_flag (s_type)
                                                : s_type.toInt ();
              if (old_vers)
                {
          ...
          else if (xmlname == "modelrecord")
            {
              ...
              mrec.ctype = sctypes.contains (s_ctype) ? ctype_flag (s_ctype)
                                                      : s_ctype.toInt ();
              if (old_vers)
                {
              ...
              mrec.str_y
                  = stry.isEmpty () ? strk.toDouble () : stry.toDouble ();
              mrec.end_y
                  = endy.isEmpty () ? endk.toDouble () : endy.toDouble ();
          ...
          else if (xmlname == "c_solute")
            {
              ...
              csolute.x
                  = sval.isEmpty () ? xval.toDouble () : sval.toDouble ();
              csolute.y
                  = kval.isEmpty () ? yval.toDouble () : kval.toDouble ();
...
int
US_ModelRecord::write_modelrecs (QXmlStreamWriter &xml,
                                 QVector<US_ModelRecord> &mrecs,
...
void
US_ModelRecord::elite_limits (QVector<US_ModelRecord> &mrecs, int &ctype,
                              ...
                              double &minp3, double &maxp3)
{
  ...
  // DbgLv(1) << " ElLim: in min/max p1/p2" << minp1 << maxp1 << minp2 <<
  // maxp2; DbgLv(1) << " ElLim: in m0p1 m0p2" << m0p1 << m0p2; DbgLv(1) << "
  ...
  for (int ii = 0; ii < nmrec; ii++)
    {
      ...
      if (ii > nelite)
        DbgLv (1) << " ElLim:    minp1 maxp1 m0p1l m0p1h" << minp1 << maxp1
                  << m0p1l << m0p1h << "minp2 maxp2 m0p2l m0p2h" << minp2
                  << maxp2 << m0p2l << m0p2h << "minyv maxyv" << minyv
                  << maxyv;
...
int
US_ModelRecord::recompute_mrecs (int &ctype, double &xmin, double &xmax,
                                 double &ymin, double &ymax, int &nypts,
                                 int &nlpts, double *parlims,
                                 QVector<US_ModelRecord> &mrecs)
{
  ...
  if (LnType)
    { // Determine models for straight-line or horizontal-line curves
      ...
      if (ctype == CTYPE_SL)
        {
          nmrec = compute_slines (xmin, xmax, ymin, ymax, nypts, nlpts,
                                  parlims, mrecs);
        }

      else if (ctype == CTYPE_HL)
        {
          nmrec = compute_hlines (xmin, xmax, ymin, ymax, nypts, nlpts,
                                  parlims, mrecs);
        }
    }
    ...
    { // Determine models for sigmoid curves
      // DbgLv(1) << "RF: sigm:  mnmx p1 p2" << minp1 << maxp1 << minp2 <<
      // maxp2;
    ...
    { // Determine models for 2nd-order Power Law curves
      ...
      nmrec = compute_2ndorder (xmin, xmax, ymin, ymax, nypts, nlpts, parlims,
                                mrecs);
...
int
US_ModelRecord::ctype_flag (const QString s_ctype)
{
...
QString
US_ModelRecord::ctype_text (const int i_ctype)
{
...
int
US_ModelRecord::stype_flag (const QString s_stype)
{
  QString snum = QString (s_stype).section (".", 0, 0);
  int i_stype = (snum.mid (0, 1).toInt () << 6)
                + (snum.mid (1, 1).toInt () << 3) + (snum.mid (2, 1).toInt ());
...
QString
US_ModelRecord::stype_text (const int i_stype)
{
