...
int US_ModelRecord::compute_slines(
   double &xmin, double &xmax, double &ymin, double &ymax, int &nypts, int &nlpts, double *parlims,
   ...
   for (int ii = 0; ii < nypts; ii++) { // Loop over k start values
      double yend = yelo;

      for (int jj = 0; jj < nypts; jj++) { // Loop over y end values
         double xval = xmin;
         double yval = ystr;
...
int US_ModelRecord::compute_sigmoids(
   int &ctype, double &xmin, double &xmax, double &ymin, double &ymax, int &nypts, int &nlpts, double *parlims,
   QVector<US_ModelRecord> &mrecs) {
   if (parlims[ 0 ] < 0.0) {
      parlims[ 0 ] = 0.001;
      parlims[ 1 ] = 0.5;
   ...
   for (int ii = 0; ii < nypts; ii++) { // Loop over par1 values (logarithmic progression)
      double p1val = exp(p1vlg);
      double p1fac = sqrt(2.0 * p1val);
      double p2val = p2lo;

      for (int jj = 0; jj < nypts; jj++) { // Loop over par2 value (linear progression)
         double xoff = 0.0;
         double yval = ystr;
         double yval0 = yval;
         double xval = xmin;
         ...
         for (int kk = 0; kk < nlpts; kk++) { // Loop over points on a curve
...
int US_ModelRecord::compute_hlines(
   double &xmin, double &xmax, double &ymin, double &ymax, int &nypts, int &nlpts, double *parlims,
   QVector<US_ModelRecord> &mrecs) {
   US_ModelRecord mrec;
   mrec.ctype = CTYPE_HL;
   ...
   for (int ii = 0; ii < nypts; ii++) { // Loop over k start and end values
      double xval = xmin;
...
int US_ModelRecord::compute_2ndorder(
   double &xmin, double &xmax, double &ymin, double &ymax, int &nypts, int &nlpts, double *parlims,
   QVector<US_ModelRecord> &mrecs) {
   int dbg_level = US_Settings::us_debug();
   //double p1lo  = -1.0;
   ...
   if (parlims[ 0 ] > 0.0) { // Recomputing: Adjust A,B,C ranges so previous best in on the grid
      amin = parlims[ 0 ];
      amax = parlims[ 1 ];
      bmin = parlims[ 2 ];
      bmax = parlims[ 3 ];
 ...
 ***/
   ...
   for (int ii = 0; ii < nypts; ii++) { // Loop over a values
      double aval = avals[ ii ];

      for (int jj = 0; jj < nypts; jj++) { // Loop over b values
         double bval = bvals[ jj ];

         for (int kk = 0; kk < nypts; kk++) { // Loop over c values
            ...
            mrec.isolutes.clear();
            US_ZSolute isol(0.0, 0.0, zval, 0.0);
            DbgLv(1) << "MR: a b c" << aval << bval << cval << " ii jj kk" << ii << jj << kk;

            for (int ll = 0; ll < nlpts; ll++) { // Loop over points on a curve
...
int US_ModelRecord::load_modelrecs(
   QXmlStreamReader &xml, QVector<US_ModelRecord> &mrecs, QString &descr, int &ctype, double &xmin, double &xmax,
   double &ymin, double &ymax, int &stype) {
   //   int dbg_level   = US_Settings::us_debug();
   int nmrecs = 0;
   int nisols = 0;
   int ncsols = 0;
   ...
   while (!xml.atEnd()) {
      ...
      if (xml.isStartElement()) {
         ...
         if (xmlname == "modelrecords") {
            ...
            ctype = sctypes.contains(s_type) ? ctype_flag(s_type) : s_type.toInt();
            if (old_vers) {
            ...
            mrec.ctype = sctypes.contains(s_ctype) ? ctype_flag(s_ctype) : s_ctype.toInt();
            if (old_vers) {
            ...
            mrec.str_y = stry.isEmpty() ? strk.toDouble() : stry.toDouble();
            mrec.end_y = endy.isEmpty() ? endk.toDouble() : endy.toDouble();
            ...
            csolute.x = sval.isEmpty() ? xval.toDouble() : sval.toDouble();
            csolute.y = kval.isEmpty() ? yval.toDouble() : kval.toDouble();
...
int US_ModelRecord::write_modelrecs(
   QXmlStreamWriter &xml, QVector<US_ModelRecord> &mrecs, QString &descr, int &ctype, double &xmin, double &xmax,
   double &ymin, double &ymax, int &stype) {
   //   int dbg_level   = US_Settings::us_debug();
...
void US_ModelRecord::elite_limits(
   QVector<US_ModelRecord> &mrecs, int &ctype, double &minyv, double &maxyv, double &minp1, double &maxp1,
   double &minp2, double &maxp2, double &minp3, double &maxp3) {
   int dbg_level = US_Settings::us_debug();
   double efrac = 0.1;
   ...
   if (ctype == CTYPE_SL || ctype == CTYPE_HL) { // Possibly adjust initial par1,par2 limits for lines
      ln_type = true;
      m0p1 = mrecs[ 0 ].str_y;
      m0p2 = mrecs[ 0 ].end_y;
   ...
   if (ctype == CTYPE_IS || ctype == CTYPE_DS) { // Possibly adjust initial par1,par2 limits for sigmoids
   ...
   if (ctype == CTYPE_2O) { // Set 2nd-order par1,par2 limits so scan stops at elite cutoff point
      m0p1l = 1e+99;
      m0p1h = -1e+99;
      m0p2l = 1e+99;
      m0p2h = -1e+99;
   ...
   for (int ii = 0; ii < nmrec; ii++) {
      ...
      if (ii < 3 || (ii + 4) > nelite)
         DbgLv(1) << " ElLim:   ii" << ii << "par1 par2 par3" << par1 << par2 << par3 << "str_y end_y" << str_y << end_y
                  << "rmsd" << mrecs[ ii ].rmsd;
      ...
      if (ii > nelite)
         DbgLv(1) << " ElLim:    minp1 maxp1 m0p1l m0p1h" << minp1 << maxp1 << m0p1l << m0p1h
                  << "minp2 maxp2 m0p2l m0p2h" << minp2 << maxp2 << m0p2l << m0p2h << "minyv maxyv" << minyv << maxyv;
...
int US_ModelRecord::recompute_mrecs(
   int &ctype, double &xmin, double &xmax, double &ymin, double &ymax, int &nypts, int &nlpts, double *parlims,
   QVector<US_ModelRecord> &mrecs) {
   int dbg_level = US_Settings::us_debug();
   int nmrec = mrecs.size();
   ...
   elite_limits(mrecs, ctype, minyv, maxyv, minp1, maxp1, minp2, maxp2, minp3, maxp3);
   ...
   if (ctype != CTYPE_2O) { // Adjust par1,par2 limits
      minp1 = p1best - p1dif;
      minp2 = p2best - p2dif;
   ...
   if (LnType) { // Determine models for straight-line or horizontal-line curves
      ...
      if (ctype == CTYPE_SL) {
         nmrec = compute_slines(xmin, xmax, ymin, ymax, nypts, nlpts, parlims, mrecs);
      }

      else if (ctype == CTYPE_HL) {
         nmrec = compute_hlines(xmin, xmax, ymin, ymax, nypts, nlpts, parlims, mrecs);
      }
   }

   else if (SgType) { // Determine models for sigmoid curves
      ...
      nmrec = compute_sigmoids(ctype, xmin, xmax, ymin, ymax, nypts, nlpts, parlims, mrecs);
      //DbgLv(1) << "RF: sigm: nmrec" << nmrec;
   }

   else if (ctype == CTYPE_2O) { // Determine models for 2nd-order Power Law curves
      if (minp1 == maxp1) {
         minp1 *= 0.99;
         maxp1 *= 1.01;
      }
      ...
      ymin = minyv;
      ymax = maxyv;
      DbgLv(1) << "RF: 2ord: nmrec" << nmrec << "ymin ymax" << ymin << ymax << "minp1 maxp1" << minp1 << maxp1
               << "minp2 maxp2" << minp2 << maxp2 << "parlims0" << parlims[ 0 ];

      nmrec = compute_2ndorder(xmin, xmax, ymin, ymax, nypts, nlpts, parlims, mrecs);
      DbgLv(1) << "RF:   2ord:  parlims1-4" << parlims[ 0 ] << parlims[ 1 ] << parlims[ 2 ] << parlims[ 3 ];
   }
...
int US_ModelRecord::stype_flag(const QString s_stype) {
   ...
   int i_stype = (snum.mid(0, 1).toInt() << 6) + (snum.mid(1, 1).toInt() << 3) + (snum.mid(2, 1).toInt());
...
QString US_ModelRecord::stype_text(const int i_stype) {
   ...
   int iyv = (i_stype >> 3) & 7;
   int izv = i_stype & 7;
   QString s_stype = QString().sprintf("%03o", i_stype) + "." + QString(atyp[ ixv ]) + QString(atyp[ iyv ])
                     + QString(atyp[ izv ]);
