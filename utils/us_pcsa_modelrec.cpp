...
int US_ModelRecord::compute_slines(double& xmin, double& xmax, double& ymin,
                                   double& ymax, int& nypts, int& nlpts,
                                   double* parlims,
...
int US_ModelRecord::compute_sigmoids(int& ctype, double& xmin, double& xmax,
                                     double& ymin, double& ymax, int& nypts,
                                     int& nlpts, double* parlims,
                                     QVector<US_ModelRecord>& mrecs) {
  if (parlims[0] < 0.0) {
    parlims[0] = 0.001;
    parlims[1] = 0.5;
  ...
  for (int ii = 0; ii < nypts;
       ii++) {  // Loop over par1 values (logarithmic progression)
    double p1val = exp(p1vlg);
    double p1fac = sqrt(2.0 * p1val);
    double p2val = p2lo;

    for (int jj = 0; jj < nypts;
         jj++) {  // Loop over par2 value (linear progression)
      double xoff = 0.0;
      double yval = ystr;
      double yval0 = yval;
      double xval = xmin;
...
int US_ModelRecord::compute_hlines(double& xmin, double& xmax, double& ymin,
                                   double& ymax, int& nypts, int& nlpts,
                                   double* parlims,
                                   QVector<US_ModelRecord>& mrecs) {
  US_ModelRecord mrec;
...
int US_ModelRecord::compute_2ndorder(double& xmin, double& xmax, double& ymin,
                                     double& ymax, int& nypts, int& nlpts,
                                     double* parlims,
                                     QVector<US_ModelRecord>& mrecs) {
  int dbg_level = US_Settings::us_debug();
  ...
  // amin = xmin^-max(bvec)*(((ymax - ymin) - (ymax - ymin) / var) / var);   %
  // lower limit of variable a amax = xmin^-max(bvec)*(ymax - ymin - (ymax -
  // ymin) / var);             % upper limit of variable a
  DbgLv(1) << "MR: p1l p1u" << p1lo << p1up;

  if (parlims[0] > 0.0) {  // Recomputing: Adjust A,B,C ranges so previous best
                           // in on the grid
    amin = parlims[0];
    amax = parlims[1];
    bmin = parlims[2];
  ...
  for m = 1 : var                 % transfer of reference vector b to any
  x-range bvec(m) = bvec(m) * log10(5)/log10((xmax-xmin)/2); % (xmax - xmin)/2
  has to be larger than 1! end

  %% remark: bmax is required for the calculation of avec

  amin = xmin^-max(bvec)*(((ymax - ymin) - (ymax - ymin) / var) / var);   %
  lower limit of variable a amax = xmin^-max(bvec)*(ymax - ymin - (ymax - ymin)
  / var);             % upper limit of variable a avec =
  linspace(amin,amax,var);                                         % vector of
  variable a
  ...
  for (int ii = 0; ii < nypts; ii++) {  // Loop over a values
    ...
    for (int jj = 0; jj < nypts; jj++) {  // Loop over b values
      ...
      for (int kk = 0; kk < nypts; kk++) {  // Loop over c values
        ...
        mrec.isolutes.clear();
        US_ZSolute isol(0.0, 0.0, zval, 0.0);
        DbgLv(1) << "MR: a b c" << aval << bval << cval << " ii jj kk" << ii
                 << jj << kk;
...
int US_ModelRecord::load_modelrecs(QXmlStreamReader& xml,
                                   QVector<US_ModelRecord>& mrecs,
                                   QString& descr, int& ctype, double& xmin,
  ...
  while (!xml.atEnd()) {
    ...
    if (xml.isStartElement()) {
      ...
      if (xmlname == "modelrecords") {
        ...
        mrec.ctype =
            sctypes.contains(s_ctype) ? ctype_flag(s_ctype) : s_ctype.toInt();
        if (old_vers) {
...
int US_ModelRecord::write_modelrecs(QXmlStreamWriter& xml,
  ...
  if (!mrec.mrecGUID.isEmpty()) xml.writeAttribute("mrecGUID", mrec.mrecGUID);
  if (!mrec.editGUID.isEmpty()) xml.writeAttribute("editGUID", mrec.editGUID);
...
void US_ModelRecord::elite_limits(QVector<US_ModelRecord>& mrecs, int& ctype,
                                  double& minyv, double& maxyv, double& minp1,
                                  double& maxp1, double& minp2, double& maxp2,
  ...
  if (ctype == CTYPE_2O) {  // Set 2nd-order par1,par2 limits so scan stops at
                            // elite cutoff point
    m0p1l = 1e+99;
    m0p1h = -1e+99;
    m0p2l = 1e+99;
  ...
  for (int ii = 0; ii < nmrec; ii++) {
    ...
    if (ii < 3 || (ii + 4) > nelite)
      DbgLv(1) << " ElLim:   ii" << ii << "par1 par2 par3" << par1 << par2
               << par3 << "str_y end_y" << str_y << end_y << "rmsd"
               << mrecs[ii].rmsd;
    ...
    if (ii > nelite)
      DbgLv(1) << " ElLim:    minp1 maxp1 m0p1l m0p1h" << minp1 << maxp1
               << m0p1l << m0p1h << "minp2 maxp2 m0p2l m0p2h" << minp2 << maxp2
               << m0p2l << m0p2h << "minyv maxyv" << minyv << maxyv;
  ...
  // DbgLv(1) << " ElLim: out min/max p1/p2" << minp1 << maxp1 << minp2 <<
  // maxp2;
}
...
int US_ModelRecord::recompute_mrecs(int& ctype, double& xmin, double& xmax,
  ...
  elite_limits(mrecs, ctype, minyv, maxyv, minp1, maxp1, minp2, maxp2, minp3,
               maxp3);
  ...
  if (LnType) {  // Determine models for straight-line or horizontal-line curves
    ...
    if (ctype == CTYPE_SL) {
      nmrec =
          compute_slines(xmin, xmax, ymin, ymax, nypts, nlpts, parlims, mrecs);
    }

    else if (ctype == CTYPE_HL) {
      nmrec =
          compute_hlines(xmin, xmax, ymin, ymax, nypts, nlpts, parlims, mrecs);
    }
  }
    ...
    nmrec = compute_sigmoids(ctype, xmin, xmax, ymin, ymax, nypts, nlpts,
                             parlims, mrecs);
    ...
    DbgLv(1) << "RF: 2ord: nmrec" << nmrec << "ymin ymax" << ymin << ymax
             << "minp1 maxp1" << minp1 << maxp1 << "minp2 maxp2" << minp2
             << maxp2 << "parlims0" << parlims[0];

    nmrec =
        compute_2ndorder(xmin, xmax, ymin, ymax, nypts, nlpts, parlims, mrecs);
...
int US_ModelRecord::stype_flag(const QString s_stype) {
  ...
  int i_stype = (snum.mid(0, 1).toInt() << 6) + (snum.mid(1, 1).toInt() << 3) +
                (snum.mid(2, 1).toInt());
...
QString US_ModelRecord::stype_text(const int i_stype) {
  ...
  QString s_stype = QString().sprintf("%03o", i_stype) + "." +
                    QString(atyp[ixv]) + QString(atyp[iyv]) +
                    QString(atyp[izv]);
