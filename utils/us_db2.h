...
#undef ERROR
  enum dbStatus
  {
    ...
    BADOPERATOR
    = 202, //!< Operator does not have permission to use that instrument
    ...
    INSERTDUP = 402,  //!< Attempt to insert a duplicate value in a primary
                      //!<    or unique key field
    DUPFIELD
    = 403, //!< Attempt to insert duplicate value where one should not exist
    ...
    ROTOR_IN_USE
    = 513,            //!< The specified rotor is in use, and cannot be deleted
    ...
    NO_REPORT_DOCUMENT
    = 522, //!< No report document with the specified ID exists
  ...
  */
  ...
  /*! \brief Reads raw binary data from the database and writes it to the
             specified file. This makes reading a record with binary
     information in it a two-step process---first get the rest of the record
  ...
  */
  ...
  /*! \brief Returns the integer status code of the most recent query performed
      ...
      lastErrno() will return 0. If the status code is not 0, then you can use
     the status code itself to determine what went wrong, or you can call
     lastError() for a text string describing the error.
  */
  int
  lastErrno (void)
  {
  ...
  */
  ...
  /*! \brief Inserts escape codes into a string, preparing the string to be
     inserted into a mysql database

      \param to A reference to a data structure where the escaped string will
     go
