...
bool US_SolutionVals::values(
   US_DB2 *dbP, US_DataIO::EditedData *edata, QString &soluID, QString &cvbar20, QString &density, QString &viscosity,
   ...
   errmsg = QObject::tr(" Solution/Buffer value determination for data set\n   ") + edata->runID + "." + edata->editID
            + "." + edata->cell + "." + edata->channel + "." + edata->wavelength + "  --\n";

   if (dbP != 0) {
      got_ok = solinfo_db(dbP, edata, cvbar20, soluID, bufID, bguid, bdesc, errmsg);
      got_ok = got_ok ? got_ok : solinfo_disk(edata, cvbar20, soluID, bufID, bguid, bdesc, errmsg);
      got_ok = bufvals_db(dbP, bufID, bguid, bdesc, density, viscosity, compress, manual, errmsg);
      got_ok = got_ok ? got_ok : bufvals_disk(bufID, bguid, bdesc, density, viscosity, compress, manual, errmsg);
   }

   else {
      got_ok = solinfo_disk(edata, cvbar20, soluID, bufID, bguid, bdesc, errmsg);
      got_ok = bufvals_disk(bufID, bguid, bdesc, density, viscosity, compress, manual, errmsg);
   }
...
bool US_SolutionVals::solinfo_db(
   US_DB2 *dbP, US_DataIO::EditedData *edata, QString &cvbar20, QString &soluID, QString &bufId, QString &bufGuid,
   ...
   if (dbP->lastErrno() != US_DB2::OK) {
      ...
      if (dbP->lastErrno() != US_DB2::OK) {
         errmsg += QObject::tr("Unable to get solutionBuffer from solution ID") + soluID + "\n" + dbP->lastError()
                   + "\n";
      }
   }
   ...
   if (dbP->lastErrno() != US_DB2::OK) {
      errmsg += QObject::tr("Unable to get solution vbar from solution ID") + soluID + "\n" + dbP->lastError() + "\n";
   }
   else {
      dbP->next();
...
bool US_SolutionVals::solinfo_disk(
   US_DataIO::EditedData *edata, QString &cvbar20, QString &soluID, QString &bufId, QString &bufGuid, QString &bufDesc,
   QString &errmsg) {
   bool bufinfo = false;
   QString soluGUID = "";

   QString exppath = US_Settings::resultDir() + "/" + edata->runID + "/" + edata->runID + "." + edata->dataType.left(2)
                     + ".xml";
   ...
   if (!bufinfo && !soluGUID.isEmpty()) { // No buffer info yet, but solution GUID found:  get buffer from solution
...
bool US_SolutionVals::bufvals_db(
   US_DB2 *dbP, QString &bufId, QString &bufGuid, QString &bufDesc, QString &density, QString &viscosity,
   QString &compress, QString &manual, QString &errmsg) {
   bool bufvals = false;
   ...
   if (bufId.isEmpty() && !bufGuid.isEmpty()) {
      ...
      if (dbP->lastErrno() != US_DB2::OK) {
         errmsg += QObject::tr("Unable to get buffer ID from buffer GUID ") + bufGuid + "\n" + dbP->lastError() + "\n";
      }
      else {
         dbP->next();
   ...
   if (!bufId.isEmpty()) {
      ...
      dbP->query(query);
      if (dbP->lastErrno() != US_DB2::OK) {
         errmsg += QObject::tr("Unable to get buffer information from buffer ID ") + bufId + "\n" + dbP->lastError()
                   + "\n";
         return bufvals;
      }
      ...
      if (dbP->lastErrno() != US_DB2::OK) {
         errmsg += QObject::tr("Unable to get buffer description for investigator ") + invID + "\n" + dbP->lastError()
                   + "\n";
         return bufvals;
      }
      ...
      if (!bufId.isEmpty()) {
         ...
         if (dbP->lastErrno() != US_DB2::OK) {
            errmsg += QObject::tr("Unable to get buffer information for buffer ID ") + bufId + "\n" + dbP->lastError()
                      + "\n";
            return bufvals;
         }
...
bool US_SolutionVals::bufvals_disk(
   QString &bufId, QString &bufGuid, QString &bufDesc, QString &density, QString &viscosity, QString &compress,
   ...
   for (int ii = 0; ii < names.size(); ii++) {
      ...
      while (!xml.atEnd()) {
         ...
         if (xml.isStartElement() && xml.name() == "buffer") {
            ...
            if ((bufGuid != "EMPTY" && bguid == bufGuid) || (bufGuid == "EMPTY" && bid == bufId)) {
   ...
   if (!bufvals) {
      errmsg += QObject::tr("  Unable to find buffer values locally\n"
