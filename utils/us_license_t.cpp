...
int US_License_t::isValid(QString &ErrorMessage, const QStringList &newLicense) {
   ...
   if (license.size() < 12) {
      ErrorMessage = QString(qApp->translate(
                                "UltraScan", "You have not yet registered your copy of UltraScan.\n"
   ...
   if (platform != PLATFORM) {
      ErrorMessage = QString(qApp->translate(
                                "UltraScan", "You are running UltraScan on a %1 platform,\n"
   ...
   if (opSys != OS) {
      ErrorMessage = QString(qApp->translate(
                                "UltraScan", "You are running UltraScan with a %1 operating system,\n"
   ...
   if (license.size() == 12) {
      ErrorMessage = QString(qApp->translate(
         "UltraScan", "Your UltraScan registration is pending.\n"
   ...
   if (calculation != validation) {
      ErrorMessage = qApp->translate(
                        "UltraScan", "The registration is invalid.\n"
                                     "You will have to update your registration before\n"
   ...
   if (QDate::currentDate() > QDate::fromString(expiration)) {
      ErrorMessage = qApp->translate(
         "UltraScan", "The registration is expired.\n"
                      "You will have to update your registration before\n"
