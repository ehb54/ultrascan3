...
int US_ExtProfile::fetch_eprofile(
   US_DB2 *dbP, const int compID, QString &compType, QString &valType, QMap<double, double> &extinction) {
...
bool US_ExtProfile::parse_eprofile(
   QString &epxml, int *compID, QString *compType, QString *valType, QMap<double, double> &extinction) {
   int xCompID; // Values from XML
   QString xCompType;
   QString xValType;
   ...
   while (!xml.atEnd()) {
      ...
      if (xml.isStartElement()) { // Handle start elements
         QString tname = xml.name().toString();

         if (tname == "extinction") { // Get wavelength/value pairs
            netag++;
            attr = xml.attributes();
         ...
         else if (tname == "extinctionProfile") { // Get profile general attributes
            nptag++;
            attr = xml.attributes();
         ...
         else if (tname == "analyte") { // Flag that this is an analyte XML
            natag++;
         }
      }
   }
...
int US_ExtProfile::create_eprofile(
   US_DB2 *dbP, const int compID, QString &compType, QString &valueType, QMap<double, double> &extinction) {
   ...
   if (num_prof > 0) { // Already existing record:  update it
      profileID = dbP->value(0).toInt(); // Profile ID
      qry.clear();
      qry << "update_eprofile" << QString::number(profileID) << QString::number(compID) << compType << valueType
          << epxml;
      dbP->statusQuery(qry);
      ...
      qry.clear();
      qry << "new_eprofile" << QString::number(compID) << compType << valueType << epxml;
      dbP->statusQuery(qry);
...
bool US_ExtProfile::xml_eprofile(
   const int compID, QString &compType, QString &valueType, QMap<double, double> &extinction, QString &xmlout) {
   bool create_ok = true;
...
bool US_ExtProfile::update_eprofile(
   US_DB2 *dbP, const int profileID, const int compID, QString &compType, QString &valueType,
   ...
   QStringList qry;
   qry.clear();
   qry << "update_eprofile" << QString::number(profileID) << QString::number(compID) << compType << valueType << epxml;
   dbP->statusQuery(qry);
...
bool US_ExtProfile::delete_eprofile(US_DB2 *dbP, const int compID, QString &compType) {
   QStringList qry;
   qry.clear();
