...
int US_ExtProfile::fetch_eprofile(US_DB2* dbP, const int compID,
                                  QString& compType, QString& valType,
  ...
  if (dbP->lastErrno() != US_DB2::OK) return profileID;
...
bool US_ExtProfile::parse_eprofile(QString& epxml, int* compID,
    ...
    if (xml.isStartElement()) {  // Handle start elements
      ...
      if (tname == "extinction") {  // Get wavelength/value pairs
        ...
        extinction[wl] = va;
               ...
               "extinctionProfile") {  // Get profile general attributes
        nptag++;
        attr = xml.attributes();
  ...
  if (compType != NULL) *compType = xCompType;
  if (valType != NULL) *valType = xValType;
...
int US_ExtProfile::create_eprofile(US_DB2* dbP, const int compID,
                                   ...
                                   QMap<double, double>& extinction) {
  qDebug() << "compID: " << compID << ", compType: " << compType
           << ", valueType: " << valueType;
  ...
  if (num_prof > 0) {                   // Already existing record:  update it
    ...
    qry.clear();
    qry << "new_eprofile" << QString::number(compID) << compType << valueType
        << epxml;
    dbP->statusQuery(qry);
...
bool US_ExtProfile::update_eprofile(US_DB2* dbP, const int profileID,
                                    const int compID, QString& compType,
                                    QString& valueType,
                                    QMap<double, double>& extinction) {
  QString epxml;
