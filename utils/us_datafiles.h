...
class US_UTIL_EXTERN US_DataFiles {
 public:
  ...
  //! \param newFile Returned new-file flag. This will be set to true if a new
  //! file is created, or false if an existing file with a matching GUID is
  //! found.
  //! \return Full path name for new or old file to write
  //!
  //! The method searches for existing files in the specified directory that
  //! match the given GUID. It looks for a specific XML tag and attribute within
  //! each file. If a matching tag and attribute are found, the file is
  //! considered an existing file. If no matching file is found, the method will
  //! look for gaps in the numeric sequence of existing files. A gap is defined
  //! as a missing number in the sequence of file names (e.g., if "M0000002.xml"
  //! and "M0000004.xml" exist, there is a gap for "M0000003.xml"). If no gaps
  //! are found, it will create a new file name by incrementing the highest
  //! existing number by one.
