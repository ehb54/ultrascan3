  ...
  */
  ...
  for (int i = 1; i < order; i++) {
    for (int j = 0; j <= i; j++) {
      ...
      for (int k = 0; k < N; k++) A[i][j] += pow(x[k], i) * pow(x[k], j);
    }
  }
  ...
  for (int i = 0; i < order; i++) {
    ...
    for (int k = 0; k < N; k++) b[i] += y[k] * pow(x[k], i);
  }
...
bool US_Matrix::Cholesky_Invert(double** AA, double** AI, int nn) {
  ...
  if (!US_Matrix::Cholesky_Decomposition(AA, nn)) return false;
  ...
  for (int jj = 0; jj < nn; jj++) {
    ...
    for (int ii = 0; ii < nn; ii++) AI[ii][jj] = work[ii];
  }
...
double** US_Matrix::construct(QVector<double*>& QVm, QVector<double>& QVd,
  ...
  for (int ii = 0; ii < rows; ii++) {  // Construct the rows of the matrix
...
void US_Matrix::tmm(double** AA, double** CC, int rows, int columns) {
  ...
  for (int ii = 0; ii < columns;
       ii++) {  // Loop for A' rows, A columns and P columns
    double dotp = 0.0;
    ...
    for (int jj = 0; jj < ii; jj++) {  // Loop for P rows (lower triangle)
      dotp = 0.0;

      // Accumulate dot product of columns (A' row, A column)
      for (int kk = 0; kk < rows; kk++) dotp += (ATrow[kk] * AA[kk][jj]);
...
void US_Matrix::tmm(double** AA, double** CC, int rows, int columns,
  ...
  if (fill) {  // Duplicate values into the upper triangle
    for (int ii = 0; ii < columns - 1; ii++)
      for (int jj = ii + 1; jj < columns; jj++) CC[ii][jj] = CC[jj][ii];
  }
}

// Compute the vector product of a matrix and a vector
void US_Matrix::mvv(double** AA, double* bb, double* cc, int rows,
                    int columns) {
  for (int ii = 0; ii < rows; ii++) {
    double dotp = 0.0;

    for (int jj = 0; jj < columns; jj++) dotp += (AA[ii][jj] * bb[jj]);
...
void US_Matrix::tvv(double** AA, double* bb, double* cc, int rows,
                    int columns) {
  for (int jj = 0; jj < columns; jj++) {
    double dotp = 0.0;

    for (int ii = 0; ii < rows; ii++) dotp += (AA[ii][jj] * bb[ii]);
...
void US_Matrix::mmm(double** AA, double** BB, double** CC, int rows, int size,
                    int columns) {
  for (int ii = 0; ii < rows; ii++) {
    for (int jj = 0; jj < columns; jj++) {
      double dotp = 0.0;

      for (int kk = 0; kk < size; kk++) dotp += (AA[ii][kk] * BB[kk][jj]);
...
double US_Matrix::dotproduct(double* aa, double* bb, int size) {
  ...
  for (int ii = 0; ii < size; ii++) dotp += (aa[ii] * bb[ii]);
...
double US_Matrix::dotproduct(double* aa, int size) {
  ...
  for (int ii = 0; ii < size; ii++) dotp += sq(aa[ii]);
...
void US_Matrix::LU_Decomposition(double** matrix, int* index, bool parity,
                                 int n) {
  ...
  for (int j = 0; j < n; j++) {
    ...
    for (int i = 0; i < j; i++) {
      ...
      for (int k = 0; k < i; k++) dum -= matrix[i][k] * matrix[k][j];
    ...
    for (int i = j; i < n; i++) {
      ...
      for (int k = 0; k < j; k++) dum -= matrix[i][k] * matrix[k][j];
...
void US_Matrix::mmv(float** result, double** vector, float*** matrix, int row,
                    int column) {
  int i, j;
  for (i = 0; i < row; i++) {
    (*result)[i] = 0;
...
void US_Matrix::mmv(float** result, double** vector, double*** matrix, int row,
                    int column) {
  int i, j;
  for (i = 0; i < row; i++) {
    (*result)[i] = 0;
...
void US_Matrix::mmv(float** result, float** vector, double*** matrix, int row,
                    int column) {
  int i, j;
  for (i = 0; i < row; i++) {
    (*result)[i] = 0;
...
void US_Matrix::mmv(float** result, float** vector, float*** matrix, int row,
                    int column) {
  int i, j;
  for (i = 0; i < row; i++) {
    (*result)[i] = 0;
...
void US_Matrix::calc_A_transpose_A(double*** A, double*** product,
...
/*
  ...
  #else
*/
void US_Matrix::calc_A_transpose_A(float*** A, float*** product,
                                   unsigned int rows, unsigned int columns) {
  QVector<dpairs> data_pairs, m, n;
  QVector<QVector<dpairs> > dataarray;
  dpairs temp_pair;
