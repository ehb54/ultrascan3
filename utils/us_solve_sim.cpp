...
#define DbgLv(a) \
  if (dbg_level >= a) qDebug() << "SS-w:" << thrnrank << ":"
#endif
...
US_SolveSim::US_SolveSim(QList<DataSet*>& data_sets, int thrnrank,
  ...
  if (data_sets[0]->simparams.band_forming) {
    ...
    if (thrnrank == 1)
      DbgLv(1) << "CR:zthr lthr mxod mnzc mfac mfex bthr" << zerothr << linethr
               << maxod << minnzsc << mfactor << mfactex << banddthr;
  }
}
...
bool US_SolveSim::checkGridSize(QList<DataSet*>& data_sets, double s_max,
                                QString& smsg) {
  ...
  for (int dd = 0; dd < data_sets.size(); dd++) {
    ...
    // if(thrnrank==1) DbgLv(1) << "CK: tsteps" << tsteps << "tstep_max" <<
    // tstep_max << "dt omg rpm"
    //  << dt << omega_s << rpm_max << "bot men rat sfac spts tmax" << bottom <<
    //  meniscus << lgbmrat
    //  << somgfac << rsimpts << time_max;

    if (tsteps > tstep_max) {
      qDebug() << "CK: tsteps" << tsteps << "dt omg rpm" << dt << omega_s
               << rpm_max << "bot men rat sfac spts tmax" << bottom << meniscus
               << lgbmrat << somgfac << rsimpts << time_max;
      too_large = true;
...
void US_SolveSim::calc_residuals(int offset, int dataset_count,
  ...
  for (int ii = 0; ii < dbgtxt.count();
       ii++) {  // If debug text modifies norm_cutoff, apply it
    if (dbgtxt[ii].startsWith("normCutoff="))
      norm_cut = QString(dbgtxt[ii]).section("=", 1, 1).toDouble();
    if (thrnrank < 2)
      DbgLv(1) << "CR:   NORMCUT  ii" << ii << "dbgtii" << dbgtxt[ii]
               << "norm_cut" << norm_cut;
  }
...
US_Settings::set_us_debug( dbg_level );
  ...
  if (banddthr) {
    ...
    if (thrnrank < 2)
      DbgLv(1) << "CR:zthr lthr mxod mfac" << zerothr << linethr << maxod
               << mfactor;
  }
  //----------------------------------------------------------
  ...
  for (int ee = offset; ee < lim_offs;
       ee++) {  // Count scan,point totals for all data sets
  ...
  DbgLv(1) << "   CR:na nb nx" << navals << narows << nsolutes << "  nt ns"
           << ntotal << nsolutes;
  ...
  for (int ee = offset; ee < lim_offs; ee++) {
    ...
    for (int ss = 0; ss < nscans; ss++) {
      for (int rr = 0; rr < npoints;
           rr++) {  // Fill the B matrix with experiment data (or zero)
        double evalue = edata->value(ss, rr);
        // DbgLv(1)<<"subha_OD limit for scan "<< ss << rr << evalue << odlim;

        if (evalue >= odlim) {  // Where ODlimit is exceeded, substitute 0.0 and
                                // bump count
          evalue = 0.0;
          kodl++;
          DbgLv(1) << "subha_OD limit exceeded for scan no " << ss << rr
                   << odlim << kodl;
        }
#if 0
            else if ( ss == 0 )
  ...
  if (stype == 0) {
    DbgLv(1) << "stype == 0 case" << "nsolutes=" << nsolutes
             << "offset=" << offset << "lim_offs" << lim_offs;
    //======================================================================
      ...
      for (int ee = offset; ee < lim_offs; ee++) {  // Solve for each data set
        ...
        if (dset->simparams.tsobj == NULL ||
            dset->simparams.sim_speed_prof.count() <
                1) {  // Simparms does not yet have a loaded timestate
          DbgLv(0) << "solve_sim_calc_residuals_1 : TSOBJ"
                   << dset->simparams.tsobj << "ssprof count"
                   << dset->simparams.sim_speed_prof.count();
          dset->simparams.tsobj = NULL;  // Insure TimeState object and
          dset->simparams.sim_speed_prof
              .clear();  //  speed prof are both cleared.
          ...
          if ((check_file.exists()) &&
              (check_file.isFile())) {  // Timestate file exists
            bool intv_1sec =
                US_AstfemMath::timestate_onesec(tmst_fpath, simdat);
            DbgLv(0) << "solve_sim_calc_residuals_1 : intv_1sec" << intv_1sec;

            if (intv_1sec) {  // Timestate is already at 1-second interval, so
                              // load it
              dset->simparams.simSpeedsFromTimeState(tmst_fpath);
              DbgLv(0) << "solve_sim_calc_residuals_1 : sim_speed_prof count"
                       << dset->simparams.sim_speed_prof.count() << " intv_1sec"
                       << intv_1sec;
            }
            DbgLv(0) << "solve_sim_calc_residuals_1 : timestate file exists"
                     << tmst_fpath
                     << " timestateobject=" << dset->simparams.tsobj
                     << "sspknt=" << dset->simparams.sim_speed_prof.count();
            DbgLv(0) << "solve_sim_calc_residuals_1 : ntimes"
                     << dset->simparams.tsobj->time_count();
          } else {  // Timestate file does not exist (will be created in
                    // astfem_rsa)
            DbgLv(0) << "solve_sim_1: timestate file does not exist"
                     << tmst_fpath << dset->tmst_file;
          }
        } else
          DbgLv(2) << "solve_sim_1: timestate object exists  sspknt="
                   << dset->simparams.sim_speed_prof.count()
                   << " timestateobject=" << dset->simparams.tsobj;

        // DebugTime("BEG: clcr-NA-astfem");
        DbgLv(2) << "solve_sim_2: smeni sbott" << dset->simparams.meniscus
        ...
        if (banddthr) {  // If band forming, hold data within thresholds; skip
                         // if all-zero
        ...
        DbgLv(1) << "   CR: A fill kodl" << kodl << "bndthr ksol" << banddthr
                 << ksols << "  kacol" << kacol;
        ...
        if (kodl == 0) {  // Normal case of no ODlimit substitutions
          ...
          DbgLv(1) << " T0O0:norm_of_the_vector" << norm_a
                   << sim_vals.solutes[cc].s << sim_vals.solutes[cc].k;
          //------------------------------------------------

          // DbgLv(2) << "   CR:116  rss now" << US_Memory::rss_now() << "cc" <<
          // cc; if(lim_offs>1&&(thrnrank==1||thrnrank==11))
          ...
          for (int ss = 0; ss < nscans; ss++) {
            for (int rr = 0; rr < npoints;
                 rr++) {  // Fill A with simulations (or zero where B has zero)
                          // x  x  x  x  x  x  x  x
              if (nnls_b[bx++] != 0.0) {
                double sval = simdat.value(ss, rr);
                nnls_a[ka++] = sval;
          ...
          DbgLv(1) << " T0O1:norm_of_the_vector" << norm_a
                   << sim_vals.solutes[cc].s << sim_vals.solutes[cc].k;
        }
...
DbgLv(0)<< " norm_s norm_a norm_b" << norm_s << norm_a << norm_b
 << "enorms" << enorms;
      ...
      if (norm_a <
          norm_cut) {   // Norm for A column falls below cutoff:  skip column
      ...
      if (signal_wanted &&
          ++kstep ==
              increp) {  // If asked for and step at increment, signal progress
    ...
    for (int cc = 0; cc < ksolutes; cc++) {  // Solve for each solute
      ...
      for (int ee = offset; ee < lim_offs; ee++) {  // Solve for each data set
        ...
        DbgLv(1) << "CR:  simdat nsc npt" << simdat.scanCount()
                 << simdat.pointCount();
        ...
        if (dset->simparams.tsobj == NULL ||
            dset->simparams.sim_speed_prof.count() <
                1) {  // Simparms does not yet have a loaded timestate
          DbgLv(0) << "solve_sim_calc_residuals_2 : TSOBJ"
                   << dset->simparams.tsobj << "ssprof count"
                   << dset->simparams.sim_speed_prof.count();
          QString runID = edata->runID;
#ifdef NO_DB
          ...
          if ((check_file.exists()) &&
            ...
            DbgLv(1) << "solve_sim_calc_residuals_2 : timestate file exists"
                     ...
                     << " timestateobject=" << dset->simparams.tsobj
                     << "sspknt=" << dset->simparams.sim_speed_prof.count();
            DbgLv(1) << "solve_sim_calc_residuals_2 : ntimes"
                     << dset->simparams.tsobj->time_count();
          } else {  // Timestate file does not exist (will be created in
                    // astfem_rsa)
...
DbgLv(1) << "CR: sdat:"
 ...
 << simdat.value(ls, 0) << simdat.value(ls,mp) << simdat.value(ls,lp);
        ...
        // Populate the A matrix for the NNLS routine with simulation
        DbgLv(1) << "   CR: A-fill  bndthr" << banddthr << "kodl ksols" << kodl
                 << ksols;
        int ks = ka;
        ...
        //  << "nnA s...k" << nnls_a[ks] << nnls_a[ks+1] << nnls_a[ka-2] <<
        //  nnls_a[ka-1]
        //  << "cc ee" << cc << ee << "kodl" << kodl;
        DbgLv(1) << "CR: cc ee" << cc << ee << "ks ka" << ks << ka
                 << "nnls_a sz" << nnls_a.count() << "  sz-kd"
                 << (nnls_a.count() - ka);
      }  // Each data set
      ...
      if (norm_a < norm_cut) {
        ...
        count_cut++;
        cutsols << cc;
        // DbgLv(1)<< " norm is becoming zero -- norm_a" << norm_a <<
        // "count_cut" << count_cut;
      } else
        usesols << cc;
      if (signal_wanted &&
    ...
    for (int cc = 0; cc < ksolutes; cc++) {  // Solve for each solute
      ...
      for (int ee = offset; ee < lim_offs; ee++) {  // Solve for each data set
        ...
        if (dset->simparams.tsobj == NULL ||
            dset->simparams.sim_speed_prof.count() <
                1) {  // Simparms does not yet have a loaded timestate
          DbgLv(0) << "solve_sim_calc_residuals_3 : TSOBJ"
                   << dset->simparams.tsobj << "ssprof count"
                   << dset->simparams.sim_speed_prof.count();
          QString runID = edata->runID;
#ifdef NO_DB
          ...
          if ((check_file.exists()) &&
            ...
            DbgLv(1) << "solve_sim_calc_residuals_3 : timestate file exists"
                     << tmst_fpath
                     << " timestateobject=" << dset->simparams.tsobj
                     << "sspknt=" << dset->simparams.sim_speed_prof.count();
            DbgLv(1) << "solve_sim_calc_residuals_3 : ntimes"
                     << dset->simparams.tsobj->time_count();
          } else {  // Timestate file does not exist (will be created in
                    // astfem_rsa)
            DbgLv(1) << "solve_sim_3: timestate file does not exist"
                     << tmst_fpath << dset->tmst_file;
          }
        }
        ...
        DbgLv(1) << "CR: ks ka" << ks << ka << "nnA s...k" << nnls_a[ks]
                 << nnls_a[ks + 1] << nnls_a[ka - 2] << nnls_a[ka - 1]
                 << "cc ee" << cc << ee << "kodl" << kodl;
  ...
  if (count_cut >
      0) {  // Compress solutes down to those uncut by norm comparison
    int kk = 0;
    ...
    if (kk != nsolutes) {
      qDebug() << "KK!=NSOLUTES:  kk" << kk << "nsolutes" << nsolutes
               << "ksolutes" << ksolutes << "count_cut" << count_cut
               << cutsols.count();
    }
    DbgLv(1) << " norm cuts for #solutes=" << ksolutes << ":  count_cut"
             << count_cut;
  }

  if (tikreg &&
      (nsolutes != ksolutes)) {  // For regularization and norm-tolerance change
                                 // in solutes, redo A
  ...
  kstodo = max(kstodo, 2);
  DbgLv(1) << "   CR:200  rss now" << US_Memory::rss_now() << "thrn"
           << thrnrank;
  // DebugTime("BEG:clcr-cn");
  //------------------------------------------
  // Check linear independence of nnls_a matrix
  //------------------------------------------
  DbgLv(1) << "subha_nnls_a size: " << nnls_a.size() << nscans << npoints
           << "nsolutes=" << nsolutes;
  //------------------------------------------
  ...
  if (calc_ti) {  // Compute TI Noise (and, optionally, RI Noise)
    ...
    if (calc_ri) compute_a_tilde(a_tilde, nnls_b);
    ...
    if (calc_ri) compute_L_tildes(nrinois, nsolutes, L_tildes, nnls_a);

    // Compute L_bars
    compute_L_bars(nsolutes, nrinois, ntinois, ntotal, L_bars, nnls_a,
                   L_tildes);

    // Set up small_a, small_b for alternate nnls
    DbgLv(1) << "  set SMALL_A+B";
    ti_small_a_and_b(nsolutes, ntotal, ntinois, small_a, small_b, a_bar, L_bars,
                     nnls_a, nnls_b);
    if (abort) return;

    // Do NNLS to compute concentrations (nnls_x)
    DbgLv(1) << "  noise small NNLS";
    US_Math2::nnls(small_a.data(), nsolutes, nsolutes, nsolutes, small_b.data(),
                   nnls_x.data());
    ...
    if (calc_ri) compute_L_tilde(L_tilde, L);

    // Compute L_bar, the average model signal at each radius
    compute_L_bar(L_bar, L, L_tilde);

    // Compute ti noise
    for (int ii = 0; ii < ntinois; ii++) tinvec[ii] = a_bar[ii] - L_bar[ii];
    ...
    ri_small_a_and_b(nsolutes, ntotal, nrinois, small_a, small_b, a_tilde,
                     L_tildes, nnls_a, nnls_b);
    if (abort) return;
    ...
    for (int ii = 0; ii < nrinois; ii++) rinvec[ii] = a_tilde[ii] - L_tilde[ii];
    ...
    if (abort) return;
    DbgLv(2) << "   CR:210  rss now" << US_Memory::rss_now() << "thrn"
             << thrnrank;
    ...
    if (ASave != NULL && BSave != NULL) {
      ...
      sv_nnls_a = nnls_a;
      sv_nnls_b = nnls_b;
      DbgLv(1) << "no_ti_or_ri: CR: sv_nnls_a size" << sv_nnls_a.size()
               << nnls_a.size();
    }
    ...
    DbgLv(2) << "   CR:211  rss now" << US_Memory::rss_now() << "thrn"
             << thrnrank;
    if (lim_offs > 1 && (thrnrank == 1 || thrnrank == 11)) {
      DbgLv(1) << "CR: narows nsolutes" << narows << nsolutes;
      DbgLv(1) << "CR:  a0 a1 b0 b1" << nnls_a[0] << nnls_a[1] << nnls_b[0]
               << nnls_b[1];
    }
  ...
  // DbgLv(1) << "   CR:na  rss now,max" << US_Memory::rss_now() <<
  // sim_vals.maxrss;
  ...
  for (int ee = offset; ee < lim_offs; ee++) {
    ...
    if (ee == offset) {
      ...
      for (int ss = 0; ss < nscans;
           ss++) {  // Append zeroed-scans sim_data for multiple data sets
        DbgLv(1) << "CR:     ss jscan" << ss << jscan << "ee kscans nscans"
                 << ee << kscans << nscans;
        sim_vals.sim_data.scanData[jscan++] = tdata.scanData[ss];
      }
    }
    DbgLv(2) << "   CR:221  rss now" << US_Memory::rss_now() << "thrn"
             << thrnrank;
  }
  ...
  for (int cc = 0; cc < nsolutes; cc++) {
    ...
    if (soluval >
        0.0) {  // If concentration non-zero, need to sum in simulation data
      if (lim_offs > 1 && (thrnrank == 1 || thrnrank == 11))
        DbgLv(1) << "CR: cc soluval" << cc << soluval << "cc-old"
                 << usesols[cc];
      int scnx = 0;
      int sim_ix = usesols[cc] * dataset_count;
      if (soluval > 1.0)
        DbgLv(1) << thrnrank << ": SOLUVAL" << soluval << "cc" << cc << "sim_ix"
                 << sim_ix << "ksol nsol" << ksolutes << nsolutes;
      ...
      for (int ee = offset; ee < lim_offs; ee++, sim_ix++) {
        ...
        if (lim_offs > 1 && (thrnrank == 1 || thrnrank == 11))
          DbgLv(1) << "CR:    ee sim_ix np ns scnx" << ee << sim_ix << npoints
                   << nscans << scnx;
        if (soluval > 1.0)
          DbgLv(1) << thrnrank << ":  SOLUVAL>1  ee nscans npoints" << ee
                   << nscans << npoints << "idatamm"
        ...
        DbgLv(2) << "   CR:231  rss now" << US_Memory::rss_now() << "thrn"
                 << thrnrank;
        //*DEBUG*
        int ss = nscans / 2;
        int rr = npoints / 2;
        if (thrnrank == 1) {
          ...
          if (use_zsol) {
            DbgLv(1) << "CR:     x y z" << sim_vals.zsolutes[cc].x * 1.0e+13
                     << sim_vals.zsolutes[cc].y << sim_vals.zsolutes[cc].z
                     << "sval" << soluval << "idat sdat" << idata->value(ss, rr)
                     << sdata->value(ss, rr);
            if (soluval > 100.0) {
              double drval = 0.0;
              double dmax = 0.0;
              ...
              DbgLv(1) << "CR:B x y" << sim_vals.zsolutes[cc].x * 1.0e+13
                       << sim_vals.zsolutes[cc].y << "sval" << soluval
                       << "amax asum" << dmax << dsum;
            }
          } else {
            DbgLv(1) << "CR:     s k v" << sim_vals.solutes[cc].s * 1.0e+13
                     << sim_vals.solutes[cc].k << sim_vals.solutes[cc].v
                     << "sval" << soluval << "idat sdat" << idata->value(ss, rr)
                     << sdata->value(ss, rr);
            if (soluval > 100.0) {
              double drval = 0.0;
              ...
              DbgLv(1) << "CR:B s k" << sim_vals.solutes[cc].s * 1.0e+13
                       << sim_vals.solutes[cc].k << "sval" << soluval
                       << "amax asum" << dmax << dsum;
            }
          }
        }
  ...
  DbgLv(2) << "   CR:301  rss now" << US_Memory::rss_now() << "thrn"
           << thrnrank;
  ...
  for (int ee = offset; ee < lim_offs; ee++, kdsx++) {
    ...
    for (int ss = 0; ss < nscans;
         ss++, scnx++) {  // Create residuals dset:  exp - sim - noise(s)

      for (int rr = 0; rr < npoints; rr++) {
        double resval = edata->value(ss, rr) - sdata->value(scnx, rr) -
                        tinvec[rr + tinoffs] - rinvec[ss + rinoffs];
  ...
  for (int ee = offset; ee < lim_offs;
       ee++) {  // Scale variances for each data set
  ...
  DbgLv(2) << "   CR:310  rss now" << US_Memory::rss_now() << "thrn"
           << thrnrank;
  ...
  if (use_zsol) {
    ...
    DbgLv(1) << "CR:   jj solute-c" << 1
             << (kk > 1 ? sim_vals.zsolutes[1].c : 0.0);
    DbgLv(1) << "CR:   jj solute-c" << kk - 2
             << (kk > 1 ? sim_vals.zsolutes[kk - 2].c : 0.0);
    DbgLv(1) << "CR:   jj solute-c" << kk - 1
             << (kk > 0 ? sim_vals.zsolutes[kk - 1].c : 0.0);
  } else {
    sim_vals.solutes.resize(qMax(kk, 1));
    DbgLv(1) << "CR:   jj solute-c" << 0 << sim_vals.solutes[0].c;
    DbgLv(1) << "CR:   jj solute-c" << 1
             << (kk > 1 ? sim_vals.solutes[1].c : 0.0);
    DbgLv(1) << "CR:   jj solute-c" << kk - 2
             << (kk > 1 ? sim_vals.solutes[kk - 2].c : 0.0);
    DbgLv(1) << "CR:   jj solute-c" << kk - 1
             << (kk > 0 ? sim_vals.solutes[kk - 1].c : 0.0);
  }
  if (abort) return;
  // double sum3 = 0.0;
  ...
  DbgLv(2) << "   CR:320  rss now" << US_Memory::rss_now() << "thrn"
           << thrnrank;
  ...
  if (ASave != NULL && BSave != NULL) {
    ...
    if (padAB) {     // Return A and B with padding for regularization
      if (tikreg) {  // If regularization was used, return A and B with padding
                     // intact
        (*ASave) = sv_nnls_a;
        (*BSave) = sv_nnls_b;
      }
        ...
        for (int cc = 0; cc < nsolutes;
             cc++) {  // Save and pad A matrix, a column at a time
      ...
      if (tikreg) {  // If regularization was used, remove padding
        ...
        (*ASave) = sv_nnls_a;
        (*BSave) = sv_nnls_b;
        DbgLv(1) << "CR: ASv: sv_nnls_a size" << sv_nnls_a.size()
                 << ASave->size();
      }
    }
  }
  DbgLv(2) << "   CR:777  rss now" << US_Memory::rss_now() << "thrn"
           << thrnrank;
...
// Set abort flag
void US_SolveSim::abort_work() { abort = true; }
...
void US_SolveSim::compute_a_tilde(QVector<double>& a_tilde,
  ...
  for (int ss = 0; ss < nscans; ss++) {
    for (int rr = 0; rr < npoints; rr++) a_tilde[ss] += nnls_b[jb++];
...
// Compute L_tildes, the average signal at each radius
void US_SolveSim::compute_L_tildes(int nrinois, int nsolutes,
...
void US_SolveSim::compute_L_tilde(QVector<double>& L_tilde,
  ...
  for (int ss = 0; ss < nscans; ss++) {
    for (int rr = 0; rr < npoints; rr++) L_tilde[ss] += L[index++];

    L_tilde[ss] *= avgscale;
  }
}

void US_SolveSim::compute_L(int ntotal, int nsolutes, QVector<double>& L,
                            const QVector<double>& nnls_a,
...
void US_SolveSim::ri_small_a_and_b(int nsolutes, int ntotal, int nrinois,
  ...
  if (signal_wanted && kstodo > 0) emit work_progress(kstodo);
  DebugTime("END:ri_smab");
}

void US_SolveSim::ti_small_a_and_b(int nsolutes, int ntotal, int ntinois,
                                   QVector<double>& small_a,
                                   QVector<double>& small_b,
  ...
  int jstprg = (kstodo * incprg) / jsols;  // steps for each report
  int kstep = 0;                           // progress counter
  // DbgLv(1)<< "ti_small_ : np ns nn nso" << npoints << nscans << ntinois <<
  // nsolutes
  ...
  for (int cc = 0; cc < nsolutes; cc++) {
    ...
    for (int ss = 0; ss < nscans; ss++) {
      ...
      for (int rr = 0; rr < npoints; rr++) {
        sum_b +=
            ((nnls_b[jjnb++] - a_bar[rr]) * (nnls_a[jjna++] - L_bars[jjlb++]));
      }
    }
...
void US_SolveSim::compute_L_bars(int nsolutes, int nrinois, int ntinois,
                                 int ntotal, QVector<double>& L_bars,
                                 const QVector<double>& nnls_a,
                                 const QVector<double>& L_tildes) {
  US_DataIO::EditedData* edata = &data_sets[d_offs]->run_data;
  int npoints = edata->pointCount();
...
bool US_SolveSim::data_threshold(US_DataIO::RawData* sdata, double zerothr,
                                 double linethr, double maxod, double mfactor) {
  int nnzro = 0;
  int nzset = 0;
  int nntrp = 0;
  int nclip = 0;
  ...
  for (int ss = 0; ss < nscans; ss++) {
    for (int rr = 0; rr < npoints; rr++) {
      ...
      else if (avalue < linethr) {  // Between zero and linear threshold:  set
                                    // to interpolated value
        avalue *= ((avalue - zerothr) * thrfact);
        nntrp++;
      }

      else if (avalue <
               maxod) {  // Under maximum OD:  set to factor times input
        avalue *= mfactor;
      }
  ...
  // if(nnzro>0) {DbgLv(1) << "CR:THR: maxs" << maxs << maxsi << "mfact" <<
  // mfactor;} else        {DbgLv(1) << "CR:THz: maxs" << maxs << nnzro <<
  // "mfact" << mfactor;}
...
bool US_SolveSim::data_threshold(US_DataIO::EditedData* edata, double zerothr,
                                 double linethr, double maxod, double mfactor) {
  int nnzro = 0;
  int npoints = edata->pointCount();
  int nscans = edata->scanCount();
...
//-----------------------------------------
double US_SolveSim::angle_vectors(QVector<double>& v1, QVector<double>& v2,
                                  int nn) {
  double angle = 0.0;
  double sum1 = 0.0;
  double sum2 = 0.0;
  ...
  DbgLv(1) << "angle between two vectors: " << angle << "sum1=" << sum1
           << "sum2=" << sum2 << "sum3=" << sum3;
  return angle;
}
