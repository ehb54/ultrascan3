...
US_SolveSim::calc_residuals(int offset, int dataset_count, Simulation &sim_vals, bool padAB, QVector<double> *ASave,
   ...
   for (int ii = 0; ii < dbgtxt.count(); ii++) {  // If debug text modifies norm_cutoff, apply it
      ...
      if (thrnrank < 2)DbgLv(1) << "CR:   NORMCUT  ii" << ii << "dbgtii" << dbgtxt[ii] << "norm_cut" << norm_cut;
   }
...
US_Settings::set_us_debug( dbg_level );
   ...
   if (banddthr) {
      ...
      qDebug() << "SolveSim:calc_res: mfactor" << mfactor;
      if (mfactor < 0.0) {
         mfactor = 1.0;
         zerothr = 0.0;
         linethr = 0.0;
      ...
      if (thrnrank < 2)DbgLv(1) << "CR:zthr lthr mxod mfac" << zerothr << linethr << maxod << mfactor;
   }
   //----------------------------------------------------------
...
// << model.components[0].D << model.components[0].signal_concentration;
            ...
            if (dset->simparams.tsobj == NULL ||
               ...
               if ((check_file.exists()) && (check_file.isFile())) {  // Timestate file exists
                  ...
                  DbgLv(0) << "solve_sim_calc_residuals_1 : timestate file exists" << tmst_fpath << " timestateobject="
                           << dset->simparams.tsobj << "sspknt=" << dset->simparams.sim_speed_prof.count();
...
// << model.components[0].D << model.components[0].signal_concentration;
            ...
            if (dset->simparams.tsobj == NULL ||
               ...
               if ((check_file.exists()) && (check_file.isFile())) {  // Timestate file exists
                  ...
                  DbgLv(1) << "solve_sim_calc_residuals_2 : timestate file exists" << tmst_fpath << " timestateobject="
                           << dset->simparams.tsobj << "sspknt=" << dset->simparams.sim_speed_prof.count();
         ...
         for (int ee = offset; ee < lim_offs; ee++) {  // Solve for each data set
            ...
            if (dset->simparams.tsobj == NULL ||
               ...
               if ((check_file.exists()) && (check_file.isFile())) {  // Timestate file exists
                  ...
                  DbgLv(1) << "solve_sim_calc_residuals_3 : timestate file exists" << tmst_fpath << " timestateobject="
                           << dset->simparams.tsobj << "sspknt=" << dset->simparams.sim_speed_prof.count();
            ...
            if (thrnrank == 1) {
               ...
               if (use_zsol) {
                  ...
                  if (soluval > 100.0) {
                     ...
                     DbgLv(1) << "CR:B x y" << sim_vals.zsolutes[cc].x * 1.0e+13 << sim_vals.zsolutes[cc].y << "sval"
...
// Compute L_tildes, the average signal at each radius
void
US_SolveSim::compute_L_tildes(int nrinois, int nsolutes, QVector<double> &L_tildes, const QVector<double> &nnls_a) {
   US_DataIO::EditedData *edata = &data_sets[d_offs]->run_data;
   int npoints = edata->pointCount();
   int nscans = edata->scanCount();
   double avgscale = 1.0 / (double) npoints;
...
void US_SolveSim::compute_L(int ntotal, int nsolutes, QVector<double> &L, const QVector<double> &nnls_a,
...
void
US_SolveSim::ri_small_a_and_b(int nsolutes, int ntotal, int nrinois, QVector<double> &small_a, QVector<double> &small_b,
                              const QVector<double> &a_tilde, const QVector<double> &L_tildes,
                              const QVector<double> &nnls_a, const QVector<double> &nnls_b) {
...
void
US_SolveSim::ti_small_a_and_b(int nsolutes, int ntotal, int ntinois, QVector<double> &small_a, QVector<double> &small_b,
                              const QVector<double> &a_bar, const QVector<double> &L_bars,
                              const QVector<double> &nnls_a, const QVector<double> &nnls_b) {
...
void US_SolveSim::compute_L_bars(int nsolutes, int nrinois, int ntinois, int ntotal, QVector<double> &L_bars,
                                 const QVector<double> &nnls_a, const QVector<double> &L_tildes) {
   US_DataIO::EditedData *edata = &data_sets[d_offs]->run_data;
   int npoints = edata->pointCount();
   int nscans = edata->scanCount();
   double avgscale = 1.0 / (double) nscans;
...
bool
US_SolveSim::data_threshold(US_DataIO::RawData *sdata, double zerothr, double linethr, double maxod, double mfactor) {
   int nnzro = 0;
   int nzset = 0;
   int nntrp = 0;
   int nclip = 0;
...
bool US_SolveSim::data_threshold(US_DataIO::EditedData *edata, double zerothr, double linethr, double maxod,
                                 double mfactor) {
   int nnzro = 0;
   int npoints = edata->pointCount();
   int nscans = edata->scanCount();
