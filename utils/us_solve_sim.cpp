...
US_SolveSim::US_SolveSim (QList<DataSet *> &data_sets, int thrnrank,
                          bool signal_wanted)
    : QObject (), data_sets (data_sets), thrnrank (thrnrank),
  ...
  if (data_sets[0]->simparams.band_forming)
    {
      ...
      if (bfcfile.open (QIODevice::ReadOnly))
        {
          ...
          // Read in, in order: zero-thresh, linear-thresh, max-od,
          // min-nz-scale
          while (!tsi.atEnd ())
            {
              ...
              double val
                  = cmx > 0 ? fline.left (cmx).simplified ().toDouble () : 0.0;
              if (cmx <= 0)
                continue;
      ...
      if (thrnrank == 1)
        DbgLv (1) << "CR:zthr lthr mxod mnzc mfac mfex bthr" << zerothr
                  << linethr << maxod << minnzsc << mfactor << mfactex
                  << banddthr;
    }
}
...
bool
US_SolveSim::checkGridSize (QList<DataSet *> &data_sets, double s_max,
                            QString &smsg)
{
      ...
      if (tsteps > tstep_max)
        {
          qDebug () << "CK: tsteps" << tsteps << "dt omg rpm" << dt << omega_s
                    << rpm_max << "bot men rat sfac spts tmax" << bottom
                    << meniscus << lgbmrat << somgfac << rsimpts << time_max;
          too_large = true;
          smsg
              += tr ("The combination of rotor speed, the ratio of "
                     "sedimentation\n"
...
bool
US_SolveSim::check_grid_size (double s_max, QString &smsg)
{
...
void
US_SolveSim::calc_residuals (int offset, int dataset_count,
      ...
      for (int ss = 0; ss < nscans; ss++)
        {
          for (int rr = 0; rr < npoints; rr++)
            { // Fill the B matrix with experiment data (or zero)
              ...
              // DbgLv(1)<<"subha_OD limit for scan "<< ss << rr << evalue <<
              // odlim;
              ...
              if (evalue >= odlim)
                { // Where ODlimit is exceeded, substitute 0.0 and bump count
                  evalue = 0.0;
                  kodl++;
                  DbgLv (1) << "subha_OD limit exceeded for scan no " << ss
                            << rr << odlim << kodl;
                }
#if 0
            else if ( ss == 0 )
  ...
  if (stype == 0)
    {
      ...
      DbgLv (2) << "   CR:BF s20wcorr D20wcorr" << dset->s20w_correction
                ...
                << dset->solution_rec.buffer.manual << "vbar20"
                << dset->vbar20;
         ...
         QList< double > enorms;
          ...
          for (int ee = offset; ee < lim_offs; ee++)
            { // Solve for each data set
              ...
              DbgLv (2) << "   CR:111  rss now" << US_Memory::rss_now ()
                        << "cc" << cc;
              US_AstfemMath::initSimData (simdat, *edata, 0.0);

              DbgLv (2) << "   CR:112  rss now" << US_Memory::rss_now ()
                        << "cc" << cc;
                ...
                { // Simparms does not yet have a loaded timestate
                  ...
                  if ((check_file.exists ()) && (check_file.isFile ()))
                    { // Timestate file exists
                      bool intv_1sec = US_AstfemMath::timestate_onesec (
                          tmst_fpath, simdat);
                      DbgLv (0) << "solve_sim_calc_residuals_1 : intv_1sec"
                                << intv_1sec;

                      if (intv_1sec)
                        { // Timestate is already at 1-second interval, so load
                          // it
                          dset->simparams.simSpeedsFromTimeState (tmst_fpath);
                          DbgLv (0) << "solve_sim_calc_residuals_1 : "
                                       "sim_speed_prof count"
                                    << dset->simparams.sim_speed_prof.count ()
                                    << " intv_1sec" << intv_1sec;
                        }
                      DbgLv (0) << "solve_sim_calc_residuals_1 : timestate "
                                   "file exists"
                                << tmst_fpath
                    ...
                    { // Timestate file does not exist (will be created in
                      // astfem_rsa)
              ...
              DbgLv (2) << "   CR:114  rss now" << US_Memory::rss_now ()
                        << "cc" << cc;
              if (abort)
                return;

              if (banddthr)
                { // If band forming, hold data within thresholds; skip if
                  // all-zero
                  if (data_threshold (&simdat, zerothr, linethr, maxod,
                                      mfactor))
                    continue;
              ...
              DbgLv (2) << "   CR:115  rss now" << US_Memory::rss_now ()
                        << "cc" << cc;

              // Populate the A matrix for the NNLS routine with simulation
              DbgLv (1) << "   CR: A fill kodl" << kodl << "bndthr ksol"
                        << banddthr << ksols << "  kacol" << kacol;
              ...
              if (kodl == 0)
                { // Normal case of no ODlimit substitutions
                  ...
                  DbgLv (1)
                      << " T0O0:norm_of_the_vector" << norm_a
                      << sim_vals.solutes[cc].s << sim_vals.solutes[cc].k;
                  //------------------------------------------------

                  // DbgLv(2) << "   CR:116  rss now" << US_Memory::rss_now()
                  // << "cc" << cc; if(lim_offs>1&&(thrnrank==1||thrnrank==11))
                ...
                { // Special case where ODlimit substitutions are in B matrix
                  ...
                  DbgLv (1)
                      << " T0O1:norm_of_the_vector" << norm_a
                      << sim_vals.solutes[cc].s << sim_vals.solutes[cc].k;
                }
                ...
                { // Simparms does not yet have a loaded timestate
                  ...
                  if ((check_file.exists ()) && (check_file.isFile ()))
                    { // Timestate file exists
                      bool intv_1sec = US_AstfemMath::timestate_onesec (
                          tmst_fpath, simdat);
                      DbgLv (1) << "solve_sim_calc_residuals_2 : intv_1sec"
                                << intv_1sec;

                      if (intv_1sec)
                        {
                          dset->simparams.simSpeedsFromTimeState (tmst_fpath);
                          DbgLv (0) << "solve_sim_calc_residuals_2 : "
                                       "sim_speed_prof count"
                                    << dset->simparams.sim_speed_prof.count ()
                                    << " intv_1sec" << intv_1sec;
                        }
                      DbgLv (1) << "solve_sim_calc_residuals_2 : timestate "
                                   "file exists"
                                << tmst_fpath
                                << " timestateobject=" << dset->simparams.tsobj
                                << "sspknt="
...
DbgLv(1) << "CR: sdat:"
 ...
 << simdat.value(ls, 0) << simdat.value(ls,mp) << simdat.value(ls,lp);
              ...
              // Populate the A matrix for the NNLS routine with simulation
              DbgLv (1) << "   CR: A-fill  bndthr" << banddthr << "kodl ksols"
                        << kodl << ksols;
              int ks = ka;
                ...
                { // Simparms does not yet have a loaded timestate
                  ...
                  if ((check_file.exists ()) && (check_file.isFile ()))
                    { // Timestate file exists
                      bool intv_1sec = US_AstfemMath::timestate_onesec (
                          tmst_fpath, simdat);
                      DbgLv (1) << "solve_sim_calc_residuals_3 : intv_1sec"
                                << intv_1sec;

                      if (intv_1sec)
                        {
                          dset->simparams.simSpeedsFromTimeState (tmst_fpath);
                          DbgLv (0) << "solve_sim_calc_residuals_3 : "
                                       "sim_speed_prof count"
                                    << dset->simparams.sim_speed_prof.count ()
                                    << " intv_1sec" << intv_1sec;
                        }
                      DbgLv (1) << "solve_sim_calc_residuals_3 : timestate "
                                   "file exists"
                                << tmst_fpath
                                << " timestateobject=" << dset->simparams.tsobj
                                << "sspknt="
  ...
  if (calc_ti)
    { // Compute TI Noise (and, optionally, RI Noise)
      ...
      ti_small_a_and_b (nsolutes, ntotal, ntinois, small_a, small_b, a_bar,
                        L_bars, nnls_a, nnls_b);
      if (abort)
        return;
      ...
      if (lim_offs > 1 && (thrnrank == 1 || thrnrank == 11))
        {
          ...
          DbgLv (1) << "CR:  a0 a1 b0 b1" << nnls_a[0] << nnls_a[1]
                    << nnls_b[0] << nnls_b[1];
        }
  ...
  for (int cc = 0; cc < nsolutes; cc++)
    {
      ...
      if (soluval > 0.0)
        { // If concentration non-zero, need to sum in simulation data
          ...
          if (soluval > 1.0)
            DbgLv (1) << thrnrank << ": SOLUVAL" << soluval << "cc" << cc
                      << "sim_ix" << sim_ix << "ksol nsol" << ksolutes
                      << nsolutes;

          for (int ee = offset; ee < lim_offs; ee++, sim_ix++)
            {
              // Input sims (ea.dset, ea.solute); out sims (sum.solute,
              // ea.dset)
              ...
              if (lim_offs > 1 && (thrnrank == 1 || thrnrank == 11))
                DbgLv (1) << "CR:    ee sim_ix np ns scnx" << ee << sim_ix
                          << npoints << nscans << scnx;
              if (soluval > 1.0)
                DbgLv (1) << thrnrank << ":  SOLUVAL>1  ee nscans npoints"
                          << ee << nscans << npoints << "idatamm"
              ...
              DbgLv (2) << "   CR:231  rss now" << US_Memory::rss_now ()
                        << "thrn" << thrnrank;
              //*DEBUG*
              int ss = nscans / 2;
              int rr = npoints / 2;
                  ...
                  if (use_zsol)
                    {
                      DbgLv (1)
                          << "CR:     x y z"
                          << sim_vals.zsolutes[cc].x * 1.0e+13
                          << sim_vals.zsolutes[cc].y << sim_vals.zsolutes[cc].z
                          << "sval" << soluval << "idat sdat"
                      ...
                      if (soluval > 100.0)
                        {
                          ...
                          DbgLv (1)
                              << "CR:B s k" << sim_vals.solutes[cc].s * 1.0e+13
  ...
  for (int jj = 0; jj < nsolutes; jj++)
    {
      double cval
          = use_zsol ? sim_vals.zsolutes[jj].c : sim_vals.solutes[jj].c;
      xnorm += sq (cval);
    }
...
// Compute a_tilde, the average experiment signal at each time
void
US_SolveSim::compute_a_tilde (QVector<double> &a_tilde,
...
// Compute L_tildes, the average signal at each radius
void
US_SolveSim::compute_L_tildes (int nrinois, int nsolutes,
...
// Compute L_tilde, the average model signal at each radius
void
US_SolveSim::compute_L_tilde (QVector<double> &L_tilde,
...
void
US_SolveSim::compute_L (int ntotal, int nsolutes, QVector<double> &L,
                        const QVector<double> &nnls_a,
...
void
US_SolveSim::ri_small_a_and_b (int nsolutes, int ntotal, int nrinois,
          ...
          for (int ss = 0; ss < nscans; ss++)
            {
              ...
              for (int rr = 0; rr < npoints; rr++)
                {
                  sum_a
                      += ((nnls_a[jja1++] - Ltil1) * (nnls_a[jja2++] - Ltil2));
                }
            }
...
void
US_SolveSim::ti_small_a_and_b (int nsolutes, int ntotal, int ntinois,
                               QVector<double> &small_a,
                               QVector<double> &small_b,
...
void
US_SolveSim::compute_L_bar (QVector<double> &L_bar, const QVector<double> &L,
...
void
US_SolveSim::compute_a_bar (QVector<double> &a_bar,
                            const QVector<double> &a_tilde,
                            const QVector<double> &nnls_b)
{
  US_DataIO::EditedData *edata = &data_sets[d_offs]->run_data;
...
void
US_SolveSim::compute_L_bars (int nsolutes, int nrinois, int ntinois,
...
void
US_SolveSim::DebugTime (QString mtext)
{
  if (dbg_timing)
    {
...
bool
US_SolveSim::data_threshold (US_DataIO::RawData *sdata, double zerothr,
...
bool
US_SolveSim::data_threshold (US_DataIO::EditedData *edata, double zerothr,
                             double linethr, double maxod, double mfactor)
{
  int nnzro = 0;
  int npoints = edata->pointCount ();
...
void
US_SolveSim::set_comp_attr (US_Model::SimulationComponent &component,
...
//-----------------------------------------
double
US_SolveSim::angle_vectors (QVector<double> &v1, QVector<double> &v2, int nn)
{
  double angle = 0.0;
  double sum1 = 0.0;
  double sum2 = 0.0;
...
double
US_SolveSim::angle_vectors (double *v1, double *v2, int nn)
{
  double angle = 0.0;
  double sum1 = 0.0;
  double sum2 = 0.0;
