...
bool
US_Hardware::readRotorMap (QMap<QString, QString> &rotor_map)
{
  bool ok = false;
...
bool
US_Hardware::readRotorMap (US_DB2 *db, QMap<QString, QString> &rotor_map)
{
  bool ok = false;
  ...
  for (int ii = 0; ii < rotorIDs.size (); ii++)
    { // Add to the list of calibration IDs, from each rotor
      ...
      if (db->lastErrno () != US_DB2::OK)
        {
          qDebug () << "*WARNING* Unable to get calIDs for rotor ID"
                    << rotorID;
          continue;
        }
    ...
    { // Build coefficient pairs mapped to each calibration ID
      ...
      while (db->next ())
        { // Map this coefficient pair to a calibration ID
          QString coeffs
              = db->value (4).toString () + " " + db->value (5).toString ();
...
bool
US_Hardware::rotorValues (QString rCalID, QMap<QString, QString> rotor_map,
                          double *rotcoeffs)
{
...
bool
US_AbstractCenterpiece::read_centerpieces (
    US_DB2 *db, QList<US_AbstractCenterpiece> &centerpieces)
{
  ...
  while (db != NULL && db->isConnected ())
    { // Get centerpiece information from the database
      QList<int> cpids;
      QStringList query;
  ...
  if (!got_cp)
    { // Get centerpiece information from local (if db==NULL or db failed)
      QFile cp_file (US_Settings::appBaseDir ()
                     + "/etc/abstractCenterpieces.xml");
              ...
              if (xml.name () == "row")
                {
                  QXmlStreamAttributes a = xml.attributes ();
                  cp.path_length
                      << a.value ("pathlen").toString ().toDouble ();
                  cp.bottom_position
                      << a.value ("bottom").toString ().toDouble ();
                }
            }
        }
