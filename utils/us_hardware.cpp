   ...
   for (int ii = 0; ii < fnames.size(); ii++) { // Browse the calibration files
      ...
      while (!xml.atEnd()) { // Get ID and coefficients from a file
         ...
         if (xml.isStartElement() && xml.name() == "Calibration") {
            ...
            QString coeffs = att.value("coeff1").toString() + " " + att.value("coeff2").toString();
...
bool US_Hardware::readRotorMap(US_DB2 *db, QMap<QString, QString> &rotor_map) {
   ...
   for (int ii = 0; ii < labIDs.size(); ii++) { // Add to the list of rotor IDs, from each lab
   ...
   for (int ii = 0; ii < rotorIDs.size(); ii++) { // Add to the list of calibration IDs, from each rotor
   ...
   for (int ii = 0; ii < calibIDs.size(); ii++) { // Build coefficient pairs mapped to each calibration ID
...
bool US_Hardware::rotorValues(QString rCalID, QMap<QString, QString> rotor_map, double *rotcoeffs) {
...
bool US_AbstractCenterpiece::read_centerpieces(US_DB2 *db, QList<US_AbstractCenterpiece> &centerpieces) {
   centerpieces.clear();
   US_AbstractCenterpiece cp;
   bool got_cp = false;

   while (db != NULL && db->isConnected()) { // Get centerpiece information from the database
      QList<int> cpids;
      QStringList query;
   ...
   if (!got_cp) { // Get centerpiece information from local (if db==NULL or db failed)
...
bool US_AbstractCenterpiece::read_centerpieces(QList<US_AbstractCenterpiece> &centerpieces) {
