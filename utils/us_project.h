...
{
public:
  //! \brief   Some status codes to keep track of where project data has been
  //! saved to
  enum projectStatus
  {
  ...
  QString AUC_questions;   //!< Questions research is attempting to answer with
                           //!< the AUC
  ...
  */
  ...
  /*! \brief    Function to delete the current project from the db, if it
     exists
  ...
  */
  ...
  /*! \brief    Function to find the filename of a project on disk, if it
  ...
  */
  ...
  /*! \brief    Function to find out where the project xml files are stored on
