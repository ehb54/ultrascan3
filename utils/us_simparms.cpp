...
void
US_SimulationParameters::initFromData (US_DB2 *db, US_DataIO::RawData &rawdata,
              ...
              while (db->next ())
                {
                  ...
                  DbgLv (1) << "Sim parms: cell iecell ichan iechan" << cell
                            << iecell << ichan << iechan << "ccId" << cellCpId;
...
void
US_SimulationParameters::initFromData (US_DB2 *db,
...
int
US_SimulationParameters::readSpeedSteps (QString runID, QString dataType,
          ...
          if (xml.isStartElement () && xml.name () == "speedstep")
            {
              ...
              speedsteps << sp;
              nsteps++;
              DbgLv (1) << "SP:rSS:   step" << nsteps << "rotorspeed"
                        << sp.rotorspeed << "avg_speed" << sp.avg_speed;
            }
        }
...
void
US_SimulationParameters::computeSpeedSteps (QVector<US_DataIO::Scan> *scans,
                                            QVector<SpeedProfile> &speedsteps)
{
  SpeedProfile sp;
  speedsteps.clear ();
  ...
  DbgLv (1) << "SP:cSS: scan" << 1 << "rpm time omega2t" << rpm
            << qRound (time1) << (*scans)[0].omega2t << "ss_reso" << ss_reso;

  for (int ii = 1; ii < scanCount; ii++)
    { // Loop to build speed steps where RPM changes
      rpm = rpmnext;
      rpmnext = (*scans)[ii].rpm;
      // Get set_speeds, the speeds rounded to nearest 100 (or other
      // resolution)
      ...
      DbgLv (1) << "SP:cSS: scan" << (ii + 1) << "rpm srpm time omega2t"
                << rpmnext << ss_next << qRound ((*scans)[ii].seconds)
                << (*scans)[ii].omega2t;
      ...
      if (ss_curr != ss_next)
        { // RPM has changed, so need to create speed step for previous scans
          ...
          sp.delay_minutes
              = (delay_secs / 60.0) - ((double)sp.delay_hours * 60.0);
          sp.scans = ii - lscx;
          sp.w2t_first = w2t1;
          sp.w2t_last = w2t2;
...
void
US_SimulationParameters::computeSpeedSteps (
    QVector<US_DataIO::RawData> &allrData, QVector<SpeedProfile> &speedsteps)
{
  ...
  {
    bool
    operator() (const US_DataIO::Scan &a, const US_DataIO::Scan &b) const
    {
...
void
US_SimulationParameters::setHardware (US_DB2 *db, QString rCalID, int cp,
                                      int ch)
{
  int dbg_level = US_Settings::us_debug ();
  rotorCalID = rCalID;
...
void
US_SimulationParameters::setHardware (QString rCalID, int cp, int ch)
{
...
int
US_SimulationParameters::load_simparms (QString fname)
{
  int stat = 0;
  int kr = 0;
...
int
US_SimulationParameters::save_simparms (QString fname)
{
  int stat = 0;
      ...
      if (rotorcoeffs[0] != 0.0)
        {
          xml.writeAttribute ("rotorcoeffs",
                              QString ().sprintf ("%.3e %.3e", rotorcoeffs[0],
                                                  rotorcoeffs[1]));
        }
...
// Load simulation parameters from file
int
US_SimulationParameters::get_simparms (US_SimulationParameters &sparms,
                                       QString fname)
{
...
// Save simulation parameters to file
int
US_SimulationParameters::put_simparms (US_SimulationParameters &sparms,
                                       QString fname)
{
...
void
US_SimulationParameters::speedstepFromXml (QXmlStreamReader &xmli,
                                           SpeedProfile &spo)
{
...
void
US_SimulationParameters::speedstepToXml (QXmlStreamWriter &xmlo,
                                         SpeedProfile *spi)
{
...
int
US_SimulationParameters::speedstepsFromDB (US_DB2 *dbP, int expID,
...
int
US_SimulationParameters::speedstepToDB (US_DB2 *dbP, int expID,
                                        SpeedProfile *spi)
{
  int dbg_level = US_Settings::us_debug ();
  ...
  query << "new_speedstep" << idExp << QString::number (spi->scans)
        ...
        << QString::number (spi->time_last) << QString::number (spi->set_speed)
...
int
US_SimulationParameters::simSpeedsFromTimeState (const QString tmst_fpath)
{
...
int
US_SimulationParameters::ssProfFromTimeState (US_TimeState *tsobj,
  ...
  for (int tsx = tsx1; tsx < nrec; tsx++)
    {
      ...
      if (tm_c < tm_cep || (tsx + 5) > nrec || in_accel)
        DbgLv (1) << "Sim parms:ssProf:   tm_c" << tm_c << "rs_c" << rs_c
                  << "ss_c" << ss_c;
        ...
        { // In acceleration, looking for its end
          ...
          if (ss_c == ss_p && cspeeds.contains (ss_c))
            { // Found a constant speed:  out of acceleration
              DbgLv (1) << "Sim parms:ssProf: accel-end ss_p ss_c" << ss_p
                        << ss_c << "tm_c" << tm_c << "rs_p rs_c" << rs_p
                        << rs_c << "ss_c_ts" << ss_c_ts << "naintvs" << naintvs
                        << "iscan" << iscan;
              ssp.w2t_e_accel = w2_p;  // Accel end omega2t
              ssp.time_e_accel = tm_p; // Accel end time
              // qDebug()<<"accln times"<<ssp.time_e_accel<<
              // ss_c<<ss_p<<rs_c<<rs_p ;
              double tmi_accel = 1.0;
              // only back off one time intervall if there are more than one
              // time interval with acceleration
              if (naintvs > 1)
                {
        ...
        { // In constant speed, looking for its end
          ...
          if (ss_c != ss_p && !cspeeds.contains (ss_c))
            { // Set speeds unequal:  back into acceleration
              DbgLv (1) << "Sim parms:ssProf: const-end ss_p ss_c" << ss_p
                        << ss_c << "ndtimes" << ndtimes << "tm_c" << tm_c
                        << "rs_p rs_c" << rs_p << rs_c << "ss_c_ts" << ss_c_ts;

              // But, wait a minute! If the "constant zone" just ending is less
              // than 20 seconds long or contains no scans, then add it to the
              // current speed step's acceleration zone and resume looking for
              // an end to the acceleration zone
              if (ndtimes < ss_lo_sec || ssp.time_f_scan < 1)
                {
                  ...
                  DbgLv (1) << "Sim parms:ssProf: BACK-to-ACCEL: naintvs"
                            << naintvs << "ndtimes" << ndtimes << "sum_accel"
  ...
  for (int i1 = 0; i1 < ssps.count (); i1++)
    {
      DbgLv (1) << "Sim parms:ssProf:  i1" << i1 << "duration"
                << ssps[i1].duration;
      DbgLv (1) << "Sim parms:ssProf:   rpmsz"
                << ssps[i1].rpm_timestate.count () << "w2tsz"
                << ssps[i1].w2t_timestate.count ();
      for (int i2 = 0; i2 < ssps[i1].duration; i2++)
        {
          int rst = tsobj->read_record (rtimex);
          ssps[i1].rpm_timestate[i2]
              = rsfm == "F4" ? tsobj->time_dvalue ("RawSpeed")
                             : (double)tsobj->time_ivalue ("RawSpeed");
...
int
US_SimulationParameters::speedstepsFromSSprof ()
{
  int dbg_level = US_Settings::us_debug ();
  ...
  for (int ss = 0; ss < nspstep; ss++)
    {
      ...
      opp->set_speed
          = (int)qRound ((double)rspeed / (double)ss_reso) * ss_reso;
...
// Debug print
void
US_SimulationParameters::debug (void)
{
