...
void US_SimulationParameters::initFromData(US_DB2* db,
  ...
  rotorCalID = "0";
  QString fn = US_Settings::resultDir() + "/" + runID + "/" + runID + "." +
               dataType + ".xml";
  QFile file(fn);

  if (file.open(QIODevice::ReadOnly |
                QIODevice::Text)) {  // If experiment/run file exists, get
                                     // calibration,centerpiece IDs from it
    ...
    while (!xml.atEnd()) {
      ...
      if (xml.isStartElement() &&
          xml.name() == "calibration") {  // Pick up rotor calibration ID from
                                          // <calibration ... id=...
      ...
      if (xml.isStartElement() &&
          xml.name() == "dataset") {  // Pick up cell and channel for comparison
        QXmlStreamAttributes a = xml.attributes();
      ...
      if (xml.isStartElement() &&
          xml.name() == "centerpiece") {  // Pick up centerpiece ID from
                                          // <centerpiece ... id=...
        QXmlStreamAttributes a = xml.attributes();
        dcp_id = a.value("id").toString().toInt();
        DbgLv(1) << "SP:iFD:  dcell dchan" << dcell << dchan;
        if (dcell == iecell &&
            dchan == channel) {  // If cell,channel match edit, pick up CpID
          cp_id = dcp_id;
  ...
  if (incl_speed) computeSpeedSteps(&rawdata.scanData, speed_step);

#ifndef NO_DB
  double rpm = rawdata.scanData[0].rpm;
  if (db != NULL) {  // If reading from the database, get rotor,centerpiece info
                     // from DB
    int stat_db = 0;
    bool ok_db;
    QString expID;
    ...
    if (stat_db !=
        US_DB2::NOROWS) {  // Info by runID:  experiment and calibration IDs
      ok_db = db->next();
      ...
      if (ok_db) {
        ...
        // DbgLv(2) << "Sim parms: rotorCalID" << rotorCalID << "sv" <<
        // rcalIDsv;
      } else
        rotorCalID = "";

      if (rotorCalID.isEmpty() &&
          !expID.isEmpty()) {  // If still no calibration ID, try it another way
        query.clear();
      ...
      if (rotorCalID.isEmpty() || rotorCalID == "0") rotorCalID = rcalIDsv;
      // DbgLv(2) << "Sim parms(3):     rotorCalID" << rotorCalID;

      if (!expID.isEmpty()) {  // Get centerpiece ID from cell records for this
                               // experiment
        query.clear();
        ...
        while (db->next()) {
          ...
          if (cellCpId > 0 && cell == iecell &&
              ichan == iechan) {  // Valid CpID with cell,channel matching edit
            cp_id = cellCpId;
            break;
          }
        }
...
void US_SimulationParameters::initFromData(US_DB2* db,
  ...
  if (incl_speed) computeSpeedSteps(&editdata.scanData, speed_step);
  ...
  if (db != NULL) {  // If reading from the database, get rotor,centerpiece info
    ...
    // DbgLv(2) << "Sim parms:invID" << US_Settings::us_inv_ID();
    query << "get_experiment_info_by_runID" << editdata.runID
          << QString::number(US_Settings::us_inv_ID());
    db->query(query);
    stat_db = db->lastErrno();
    // DbgLv(2) << "Sim parms:query() stat" << stat_db;
...
void US_SimulationParameters::computeSpeedSteps(
    QVector<US_DataIO::Scan>* scans, QVector<SpeedProfile>& speedsteps) {
  SpeedProfile sp;
  speedsteps.clear();
  int dbg_level = US_Settings::us_debug();
  ...
  DbgLv(1) << "SP:cSS: scan" << 1 << "rpm time omega2t" << rpm << qRound(time1)
           << (*scans)[0].omega2t << "ss_reso" << ss_reso;

  for (int ii = 1; ii < scanCount;
       ii++) {  // Loop to build speed steps where RPM changes
    rpm = rpmnext;
    ...
    DbgLv(1) << "SP:cSS: scan" << (ii + 1) << "rpm srpm time omega2t" << rpmnext
             << ss_next << qRound((*scans)[ii].seconds) << (*scans)[ii].omega2t;

    if (ss_curr != ss_next) {  // RPM has changed, so need to create speed step
                               // for previous scans
      ...
      sp.duration_minutes =
          (step_secs / 60.0) - ((double)sp.duration_hours * 60.0);
      ...
      speedsteps << sp;
      DbgLv(1) << "SP:cSS:   speedsteps" << speedsteps.size() << "scans"
               << sp.scans << "duration h m" << sp.duration_hours
               << sp.duration_minutes << "delay h m" << sp.delay_hours
               << sp.delay_minutes << "rpm" << rpm;
      DbgLv(1) << "SP:cSS:      w2t1 w2t2 time1 time2" << sp.w2t_first
               << sp.w2t_last << sp.time_first << sp.time_last;
      DbgLv(1) << "SP:cSS:       sp set avg sdev" << sp.set_speed
               << sp.avg_speed << sp.speed_stddev;
...
void US_SimulationParameters::computeSpeedSteps(
  ...
  if (allrData.count() ==
      1) {  // For a single triple, compute the speed steps from it
...
void US_SimulationParameters::setHardware(US_DB2* db, QString rCalID, int cp,
                                          int ch) {
  int dbg_level = US_Settings::us_debug();
  rotorCalID = rCalID;
  bottom_position = 7.2;
  ...
  if (US_AbstractCenterpiece::read_centerpieces(db, cp_list)) {
    if (cp <
        0) {  // If cp given is negative, this means look for a serial number
      int cp_id = -cp;
      cp = 0;

      for (int jj = 0; jj < cp_list.size(); jj++) {
        if (cp_id == cp_list[jj].serial_number) {  // Replace cp value (serial)
                                                   // with an index in the list
          cp = jj;
          break;
        }
      }
  ...
  if (US_Hardware::readRotorMap(
          db,
          rotor_map)) {  // Get rotor coefficients by matching calibration ID
  ...
  if (xfile.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    while (!xml.atEnd()) {
      ...
      if (xml.isStartElement() && xml.name() == "params") {
        ...
        if (!astr.isEmpty()) simpoints = astr.toInt();
        astr = a.value("radialres").toString();
        if (!astr.isEmpty()) radial_resolution = astr.toDouble();
        astr = a.value("meniscus").toString();
        if (!astr.isEmpty()) meniscus = astr.toDouble();
        astr = a.value("bottom").toString();
        if (!astr.isEmpty()) bottom = astr.toDouble();
        astr = a.value("rnoise").toString();
        if (!astr.isEmpty()) rnoise = astr.toDouble();
        astr = a.value("lrnoise").toString();
        if (!astr.isEmpty()) lrnoise = astr.toDouble();
        astr = a.value("tinoise").toString();
        if (!astr.isEmpty()) tinoise = astr.toDouble();
        astr = a.value("rinoise").toString();
        if (!astr.isEmpty()) rinoise = astr.toDouble();
        astr = a.value("baseline").toString();
        if (!astr.isEmpty()) baseline = astr.toDouble();
        astr = a.value("temperature").toString();
        if (!astr.isEmpty()) temperature = astr.toDouble();
        ...
        if (!astr.isEmpty()) rotorCalID = astr;
        ...
        if (!astr.isEmpty()) cp_sector = astr.toInt();
        astr = a.value("pathlength").toString();
        if (!astr.isEmpty()) cp_pathlen = astr.toDouble();
        astr = a.value("angle").toString();
        if (!astr.isEmpty()) cp_angle = astr.toDouble();
        astr = a.value("width").toString();
        if (!astr.isEmpty()) cp_width = astr.toDouble();
      }
...
int US_SimulationParameters::save_simparms(QString fname) {
  ...
  if (xfile.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    if (!rotorCalID.isEmpty()) xml.writeAttribute("rotorCalID", rotorCalID);

    if (rotorcoeffs[0] != 0.0) {
      xml.writeAttribute(
          "rotorcoeffs",
          QString().sprintf("%.3e %.3e", rotorcoeffs[0], rotorcoeffs[1]));
    }
...
void US_SimulationParameters::speedstepFromXml(QXmlStreamReader& xmli,
  ...
  if (!astr.isEmpty()) spo.duration_hours = astr.toInt();
  astr = attr.value("duration_mins").toString();
  if (!astr.isEmpty()) spo.duration_minutes = astr.toDouble();
  astr = attr.value("delay_hrs").toString();
  if (!astr.isEmpty()) spo.delay_hours = astr.toInt();
  astr = attr.value("delay_mins").toString();
  if (!astr.isEmpty()) spo.delay_minutes = astr.toDouble();
  astr = attr.value("rotorspeed").toString();
  if (!astr.isEmpty()) spo.rotorspeed = astr.toInt();
  astr = attr.value("acceleration").toString();
  if (!astr.isEmpty()) spo.acceleration = astr.toInt();
  astr = attr.value("accelerflag").toString();
  if (!astr.isEmpty()) spo.acceleration_flag = (trueStr.indexOf(astr) > 0);
  astr = attr.value("scans").toString();
  if (!astr.isEmpty()) spo.scans = astr.toInt();
  astr = attr.value("w2tfirst").toString();
  if (!astr.isEmpty()) spo.w2t_first = astr.toDouble();
  astr = attr.value("w2tlast").toString();
  if (!astr.isEmpty()) spo.w2t_last = astr.toDouble();
  astr = attr.value("timefirst").toString();
  if (!astr.isEmpty()) spo.time_first = astr.toInt();
  astr = attr.value("timelast").toString();
  if (!astr.isEmpty()) spo.time_last = astr.toInt();

  // MWL enhancements
  astr = attr.value("set_speed").toString();
  if (!astr.isEmpty()) spo.set_speed = astr.toInt();
  astr = attr.value("avg_speed").toString();
  if (!astr.isEmpty()) spo.avg_speed = astr.toDouble();
  astr = attr.value("speed_stddev").toString();
  if (!astr.isEmpty()) spo.speed_stddev = astr.toDouble();
}
...
int US_SimulationParameters::speedstepsFromDB(US_DB2* dbP, int expID,
  ...
  if (dbP == NULL || expID < 1) return nspeeds;
...
int US_SimulationParameters::speedstepToDB(US_DB2* dbP, int expID,
  ...
  if (dbP == NULL || expID < 0 || spi == NULL) return sstepID;

  QString idExp = QString::number(expID);
  QStringList query;
  query << "new_speedstep" << idExp << QString::number(spi->scans)
        ...
        << QString::number(spi->time_first) << QString::number(spi->time_last)
        << QString::number(spi->set_speed) << QString::number(spi->avg_speed)
...
int US_SimulationParameters::simSpeedsFromTimeState(const QString tmst_fpath) {
  ...
  int dbg_level = US_Settings::us_debug();
  DbgLv(1) << "SP: ssFts: ispeed" << sim_speed_prof[0].rotorspeed << "ssp count"
           << sim_speed_prof.count();
...
int US_SimulationParameters::ssProfFromTimeState(US_TimeState* tsobj,
  ...
  if (!have_keys) return -1;  // Do not have needed keys
  ...
  for (int tsx = 1; tsx < nrec; tsx++) {
    ...
    if (ss3 == ss2 && ss2 == ss1 &&
        ss3 > ss_lo_rpm) {  // This non-zero set-speed and both previous are the
                            // same
  ...
  int tm_c = tmfm == "F4" ? (int)qRound(tsobj->time_dvalue("Time"))
  ...
  for (int tsx = tsx1; tsx < nrec; tsx++) {
    ...
    tm_c = tmfm == "F4" ? (int)qRound(tsobj->time_dvalue("Time"))
                        : tsobj->time_ivalue("Time");
    tm_c -= tm_off;
    ...
    if (tm_c < tm_cep || (tsx + 5) > nrec || in_accel)
      DbgLv(1) << "Sim parms:ssProf:   tm_c" << tm_c << "rs_c" << rs_c << "ss_c"
               << ss_c;
    ...
    if (in_accel) {  // In acceleration, looking for its end
      ...
      if (ss_c == ss_p &&
          cspeeds.contains(
              ss_c)) {  // Found a constant speed:  out of acceleration
        DbgLv(1) << "Sim parms:ssProf: accel-end ss_p ss_c" << ss_p << ss_c
                 << "tm_c" << tm_c << "rs_p rs_c" << rs_p << rs_c << "ss_c_ts"
                 << ss_c_ts << "naintvs" << naintvs << "iscan" << iscan;
        ...
        // only back off one time intervall if there are more than one time
        // interval with acceleration
        if (naintvs > 1) {
        ...
        if (iscan >
            0) {  // On a scan as we enter constant zone:  save 1st scan time
          ssp.time_f_scan = tm_c;
          DbgLv(1) << "Sim parms:ssProf:  f_scan(c)" << tm_c;
        } else if (pscan > 0) {  // On a scan as we enter constant zone:  save
                                 // 1st scan time
      ...
      if (ss_c != ss_p &&
          !cspeeds.contains(
              ss_c)) {  // Set speeds unequal:  back into acceleration
        DbgLv(1) << "Sim parms:ssProf: const-end ss_p ss_c" << ss_p << ss_c
                 << "ndtimes" << ndtimes << "tm_c" << tm_c << "rs_p rs_c"
                 << rs_p << rs_c << "ss_c_ts" << ss_c_ts;
        ...
        if (ndtimes < ss_lo_sec || ssp.time_f_scan < 1) {
          ...
          DbgLv(1) << "Sim parms:ssProf: BACK-to-ACCEL: naintvs" << naintvs
                   << "ndtimes" << ndtimes << "sum_accel" << sum_accel
                   << "sum_speed" << sum_speed;
          continue;
        }
  ...
  for (int i1 = 0; i1 < ssps.count(); i1++) {
    DbgLv(1) << "Sim parms:ssProf:  i1" << i1 << "duration"
             << ssps[i1].duration;
    ...
    for (int i2 = 0; i2 < ssps[i1].duration; i2++) {
      ...
      if (i2 < 4 || (i2 + 5) > ssps[i1].duration)
        DbgLv(1) << "Sim parms:ssProf:    i2" << i2 << "rtimex" << rtimex
                 << "rpm" << ssps[i1].rpm_timestate[i2] << "w2t"
                 << ssps[i1].w2t_timestate[i2] << "rd_stat" << rst;
...
int US_SimulationParameters::speedstepsFromSSprof() {
  ...
  for (int ss = 0; ss < nspstep; ss++) {
    ...
    DbgLv(1) << "SP: ssFssp:  ssp:" << ipp->acceleration << ipp->w2t_b_accel
             << ipp->w2t_e_accel << ipp->w2t_e_step << ipp->avg_speed
             << ipp->rotorspeed << ipp->duration << ipp->time_b_accel
             << ipp->time_e_accel << ipp->time_f_scan << ipp->time_l_scan
             << ipp->time_e_step;
    ...
    if ((ss + 1) ==
        nspstep) {  // Insure last step's end scan extends to last time
    ...
    DbgLv(1) << "SP: ssFssp:   tf tl wf wl" << time_first << time_last
             << w2t_first << w2t_last;
    DbgLv(1) << "SP: ssFssp:   ss:" << opp->duration_minutes
             << opp->delay_minutes << opp->w2t_first << opp->w2t_last
             << opp->avg_speed << opp->speed_stddev << opp->duration_hours
             << opp->delay_hours << opp->time_first << opp->time_last
             << opp->scans << opp->rotorspeed << opp->acceleration
             << opp->set_speed << opp->acceleration_flag;
  }
