...
void US_SimulationParameters::initFromData(
   US_DB2 *db, US_DataIO::RawData &rawdata, bool incl_speed, QString runID, QString dataType) {
   SpeedProfile sp;
   ...
   DbgLv(1) << "SP:iFD: cell chan ch" << ecell << channel << ch << rawdata.channel << (ch + 1);

   rotorCalID = "0";
   QString fn = US_Settings::resultDir() + "/" + runID + "/" + runID + "." + dataType + ".xml";
   QFile file(fn);

   if (file.open(
          QIODevice::ReadOnly
          | QIODevice::Text)) { // If experiment/run file exists, get calibration,centerpiece IDs from it
      ...
      while (!xml.atEnd()) {
         ...
         if (xml.isStartElement() && xml.name() == "calibration") { // Pick up rotor calibration ID from  <calibration ... id=...
         ...
         if (xml.isStartElement() && xml.name() == "dataset") { // Pick up cell and channel for comparison
            QXmlStreamAttributes a = xml.attributes();
            dcell = a.value("cell").toString().toInt();
            dchan = a.value("channel").toString();
         }

         if (xml.isStartElement() && xml.name() == "centerpiece") { // Pick up centerpiece ID from  <centerpiece ... id=...
            QXmlStreamAttributes a = xml.attributes();
            dcp_id = a.value("id").toString().toInt();
            DbgLv(1) << "SP:iFD:  dcell dchan" << dcell << dchan;
            if (dcell == iecell && dchan == channel) { // If cell,channel match edit, pick up CpID
               cp_id = dcp_id;
   ...
   if (db != NULL) { // If reading from the database, get rotor,centerpiece info from DB
      int stat_db = 0;
      bool ok_db;
      QString expID;
      QStringList query;
      ...
      if (stat_db != US_DB2::NOROWS) { // Info by runID:  experiment and calibration IDs
         ok_db = db->next();
         ...
         if (rotorCalID.isEmpty() && !expID.isEmpty()) { // If still no calibration ID, try it another way
            query.clear();
         ...
         if (!expID.isEmpty()) { // Get centerpiece ID from cell records for this experiment
            query.clear();
            ...
            while (db->next()) {
               ...
               DbgLv(1) << "Sim parms: cell iecell ichan iechan" << cell << iecell << ichan << iechan << "ccId"
                        << cellCpId;
               if (cellCpId > 0 && cell == iecell && ichan == iechan) { // Valid CpID with cell,channel matching edit
                  cp_id = cellCpId;
                  break;
               }
            }
...
void US_SimulationParameters::initFromData(US_DB2 *db, US_DataIO::EditedData &editdata, bool incl_speed) {
   SpeedProfile sp;
   ...
   DbgLv(1) << "SP:iFD: cell chan ch" << ecell << channel << ch << editdata.channel << (ch + 1);

   rotorCalID = "0";
   QString fn = US_Settings::resultDir() + "/" + editdata.runID + "/" + editdata.runID + "." + editdata.dataType
                + ".xml";
   QFile file(fn);
   ...
   if (db != NULL) { // If reading from the database, get rotor,centerpiece info from DB
      ...
      //DbgLv(2) << "Sim parms:invID" << US_Settings::us_inv_ID();
      query << "get_experiment_info_by_runID" << editdata.runID << QString::number(US_Settings::us_inv_ID());
      db->query(query);
      stat_db = db->lastErrno();
      //DbgLv(2) << "Sim parms:query() stat" << stat_db;
      if (stat_db != US_DB2::NOROWS) { // Info by runID:  experiment and calibration IDs
   ...
   DbgLv(1) << "SP:iFD: bottom" << bottom << "bottom_ed" << editdata.bottom;

   //-------------------------------------------
}

// Read the speed steps vector from runID file
int US_SimulationParameters::readSpeedSteps(QString runID, QString dataType, QVector<SpeedProfile> &speedsteps) {
   speedsteps.clear();
   SpeedProfile sp;
   int nsteps = 0;
   int dbg_level = US_Settings::us_debug();
   ...
   if (filei.open(QIODevice::ReadOnly | QIODevice::Text)) { // If experiment/run file exists, get speed steps from it
      ...
      while (!xml.atEnd()) {
         ...
         if (xml.isStartElement() && xml.name() == "speedstep") {
            ...
            speedsteps << sp;
            nsteps++;
            DbgLv(1) << "SP:rSS:   step" << nsteps << "rotorspeed" << sp.rotorspeed << "avg_speed" << sp.avg_speed;
         }
      }
...
void US_SimulationParameters::computeSpeedSteps(QVector<US_DataIO::Scan> *scans, QVector<SpeedProfile> &speedsteps) {
   SpeedProfile sp;
   speedsteps.clear();
   int dbg_level = US_Settings::us_debug();
   ...
   DbgLv(1) << "SP:cSS: scan" << 1 << "rpm time omega2t" << rpm << qRound(time1) << (*scans)[ 0 ].omega2t << "ss_reso"
            << ss_reso;

   for (int ii = 1; ii < scanCount; ii++) { // Loop to build speed steps where RPM changes
      rpm = rpmnext;
      ...
      DbgLv(1) << "SP:cSS: scan" << (ii + 1) << "rpm srpm time omega2t" << rpmnext << ss_next
               << qRound((*scans)[ ii ].seconds) << (*scans)[ ii ].omega2t;

      if (ss_curr != ss_next) { // RPM has changed, so need to create speed step for previous scans
         time2 = (*scans)[ ii - 1 ].seconds;
         w2t2 = (*scans)[ ii - 1 ].omega2t;
         step_secs = time2 - time1 + delay_secs;
         sp.duration_hours = ( int ) (step_secs / 3600.0);
         sp.duration_minutes = (step_secs / 60.0) - (( double ) sp.duration_hours * 60.0);
         sp.delay_hours = ( int ) (delay_secs / 3600.0);
         sp.delay_minutes = (delay_secs / 60.0) - (( double ) sp.delay_hours * 60.0);
         sp.scans = ii - lscx;
         sp.w2t_first = w2t1;
         sp.w2t_last = w2t2;
         ...
         speedsteps << sp;
         DbgLv(1) << "SP:cSS:   speedsteps" << speedsteps.size() << "scans" << sp.scans << "duration h m"
                  << sp.duration_hours << sp.duration_minutes << "delay h m" << sp.delay_hours << sp.delay_minutes
                  << "rpm" << rpm;
         DbgLv(1) << "SP:cSS:      w2t1 w2t2 time1 time2" << sp.w2t_first << sp.w2t_last << sp.time_first
                  << sp.time_last;
         DbgLv(1) << "SP:cSS:       sp set avg sdev" << sp.set_speed << sp.avg_speed << sp.speed_stddev;
   ...
   speedsteps << sp;
   DbgLv(1) << "SP:cSS:   speedsteps" << speedsteps.size() << "scans" << sp.scans << "duration h m" << sp.duration_hours
            << sp.duration_minutes << "delay h m" << sp.delay_hours << sp.delay_minutes << "rpm" << rpm;
   DbgLv(1) << "SP:cSS:      w2t1 w2t2 time1 time2" << sp.w2t_first << sp.w2t_last << sp.time_first << sp.time_last;
   DbgLv(1) << "SP:cSS:       sp set avg sdev" << sp.set_speed << sp.avg_speed << sp.speed_stddev;
}

// Compute the experiment speed steps vector from all data scans
void US_SimulationParameters::computeSpeedSteps(
   QVector<US_DataIO::RawData> &allrData, QVector<SpeedProfile> &speedsteps) {
   if (allrData.count() == 1) { // For a single triple, compute the speed steps from it
   ...
   struct SortBySeconds {
         bool operator()(const US_DataIO::Scan &a, const US_DataIO::Scan &b) const { return a.seconds < b.seconds; }
   };
...
void US_SimulationParameters::setHardware(US_DB2 *db, QString rCalID, int cp, int ch) {
   int dbg_level = US_Settings::us_debug();
   rotorCalID = rCalID;
   bottom_position = 7.2;
   ...
   if (US_AbstractCenterpiece::read_centerpieces(db, cp_list)) {
      if (cp < 0) { // If cp given is negative, this means look for a serial number
         int cp_id = -cp;
         cp = 0;

         for (int jj = 0; jj < cp_list.size(); jj++) {
            if (cp_id == cp_list[ jj ].serial_number) { // Replace cp value (serial) with an index in the list
               cp = jj;
               break;
            }
         }
   ...
   if (US_Hardware::readRotorMap(db, rotor_map)) { // Get rotor coefficients by matching calibration ID
...
int US_SimulationParameters::save_simparms(QString fname) {
   ...
   if (xfile.open(QIODevice::WriteOnly | QIODevice::Text)) {
      ...
      if (rotorcoeffs[ 0 ] != 0.0) {
         xml.writeAttribute("rotorcoeffs", QString().sprintf("%.3e %.3e", rotorcoeffs[ 0 ], rotorcoeffs[ 1 ]));
      }
      ...
      if (meshType == US_SimulationParameters::USER) {
         for (int ii = 0; ii < mesh_radius.size(); ii++) {
            ...
            xml.writeAttribute("radius", QString().sprintf("%11.5e", mesh_radius[ ii ]).simplified());
            xml.writeEndElement();
         }
      }
...
// Load simulation parameters from file
int US_SimulationParameters::get_simparms(US_SimulationParameters &sparms, QString fname) {
   return sparms.load_simparms(fname);
}

// Save simulation parameters to file
int US_SimulationParameters::put_simparms(US_SimulationParameters &sparms, QString fname) {
   return sparms.save_simparms(fname);
}

// Get a speed step from an XML portion
void US_SimulationParameters::speedstepFromXml(QXmlStreamReader &xmli, SpeedProfile &spo) {
...
void US_SimulationParameters::speedstepToXml(QXmlStreamWriter &xmlo, SpeedProfile *spi) {
   xmlo.writeStartElement("speedstep");
   xmlo.writeAttribute("rotorspeed", QString::number(spi->rotorspeed));
   xmlo.writeAttribute("scans", QString::number(spi->scans));
   xmlo.writeAttribute("timefirst", QString::number(spi->time_first));
   xmlo.writeAttribute("timelast", QString::number(spi->time_last));
   xmlo.writeAttribute("w2tfirst", QString::number(spi->w2t_first));
   xmlo.writeAttribute("w2tlast", QString::number(spi->w2t_last));
   xmlo.writeAttribute("duration_hrs", QString::number(spi->duration_hours));
   xmlo.writeAttribute("duration_mins", QString::number(spi->duration_minutes));
   xmlo.writeAttribute("delay_hrs", QString::number(spi->delay_hours));
   xmlo.writeAttribute("delay_mins", QString::number(spi->delay_minutes));
   xmlo.writeAttribute("acceleration", QString::number(spi->acceleration));
   xmlo.writeAttribute("accelerflag", (spi->acceleration_flag ? "1" : "0"));

   // Possible MWL enhancements
   if (spi->set_speed > 0) {
      xmlo.writeAttribute("set_speed", QString::number(spi->set_speed));
      xmlo.writeAttribute("avg_speed", QString::number(spi->avg_speed));
      xmlo.writeAttribute("speed_stddev", QString::number(spi->speed_stddev));
   }
...
int US_SimulationParameters::speedstepsFromDB(US_DB2 *dbP, int expID, QVector<SpeedProfile> &sps) {
   int nspeeds = 0;
   sps.clear();
...
int US_SimulationParameters::speedstepToDB(US_DB2 *dbP, int expID, SpeedProfile *spi) {
   int dbg_level = US_Settings::us_debug();
   ...
   QString idExp = QString::number(expID);
   QStringList query;
   query << "new_speedstep" << idExp << QString::number(spi->scans) << QString::number(spi->duration_hours)
         << QString::number(spi->duration_minutes) << QString::number(spi->delay_hours)
         << QString::number(spi->delay_minutes) << QString::number(spi->rotorspeed)
         << QString::number(spi->acceleration) << QString(spi->acceleration_flag ? "1" : "0")
         << QString::number(spi->w2t_first) << QString::number(spi->w2t_last) << QString::number(spi->time_first)
         << QString::number(spi->time_last) << QString::number(spi->set_speed) << QString::number(spi->avg_speed)
   ...
   DbgLv(1) << "SP:ssToDB:  w2t" << spi->w2t_first << spi->w2t_last << QString::number(spi->w2t_first)
...
int US_SimulationParameters::simSpeedsFromTimeState(const QString tmst_fpath) {
   ...
   int dbg_level = US_Settings::us_debug();
   DbgLv(1) << "SP: ssFts: ispeed" << sim_speed_prof[ 0 ].rotorspeed << "ssp count" << sim_speed_prof.count();
...
int US_SimulationParameters::ssProfFromTimeState(US_TimeState *tsobj, QVector<SimSpeedProf> &ssps) {
   int dbg_level = US_Settings::us_debug();
   ...
   bool have_keys = (tmkx >= 0) && (sskx >= 0) && (rskx >= 0) && (w2kx >= 0);
   ...
   for (int tsx = 1; tsx < nrec; tsx++) {
      ...
      tsobj->read_record();
      ss3 = ssfm == "F4" ? ( int ) qRound(tsobj->time_dvalue("SetSpeed"))
                         : tsobj->time_ivalue("SetSpeed"); // Current set speed

      if (ss3 == ss2 && ss2 == ss1 && ss3 > ss_lo_rpm) { // This non-zero set-speed and both previous are the same
   ...
   int tm_c = tmfm == "F4" ? ( int ) qRound(tsobj->time_dvalue("Time")) : tsobj->time_ivalue("Time");
   ...
   double rs_c = rsfm == "F4" ? tsobj->time_dvalue("RawSpeed")
                              : ( double ) tsobj->time_ivalue("RawSpeed"); // 1st raw speed
   int ss_c = ssfm == "F4" ? ( int ) qRound(tsobj->time_dvalue("SetSpeed"))
   ...
   while (ss_c == 0) { // Skipping until set speed is non-zero
      tsx1++;
      naintvs++;
      tm_p = tm_c;
      ...
      rs_c = rsfm == "F4" ? tsobj->time_dvalue("RawSpeed")
                          : ( double ) tsobj->time_ivalue("RawSpeed"); // Current raw speed
      ss_c = ssfm == "F4" ? ( int ) qRound(tsobj->time_dvalue("SetSpeed"))
                          : tsobj->time_ivalue("SetSpeed"); // Current set speed
   ...
   DbgLv(1) << "Sim parms:ssProf: initial tm_c" << tm_c << "naintvs" << naintvs << "tsx1" << tsx1 << "w2 rs ss" << w2_c
            << rs_c << ss_c;
   int tm_ci = tm_c;
   ...
   for (int tsx = tsx1; tsx < nrec; tsx++) {
      ...
      tm_c = tmfm == "F4" ? ( int ) qRound(tsobj->time_dvalue("Time")) : tsobj->time_ivalue("Time");
      tm_c -= tm_off;
      w2_c = tsobj->time_dvalue("Omega2T");
      rs_c = rsfm == "F4" ? tsobj->time_dvalue("RawSpeed") : ( double ) tsobj->time_ivalue("RawSpeed");
      ss_c = ssfm == "F4" ? ( int ) qRound(tsobj->time_dvalue("SetSpeed")) : tsobj->time_ivalue("SetSpeed");
      ...
      if (in_accel) { // In acceleration, looking for its end
         ss_c = ( int ) qRound(rs_c / ( double ) ss_reso) * ss_reso;
         if (ss_c == ss_p && cspeeds.contains(ss_c)) { // Found a constant speed:  out of acceleration
            DbgLv(1) << "Sim parms:ssProf: accel-end ss_p ss_c" << ss_p << ss_c << "tm_c" << tm_c << "rs_p rs_c" << rs_p
                     << rs_c << "ss_c_ts" << ss_c_ts << "naintvs" << naintvs << "iscan" << iscan;
            ...
            if (iscan > 0) { // On a scan as we enter constant zone:  save 1st scan time
               ssp.time_f_scan = tm_c;
               DbgLv(1) << "Sim parms:ssProf:  f_scan(c)" << tm_c;
            }
            else if (pscan > 0) { // On a scan as we enter constant zone:  save 1st scan time
               ssp.time_f_scan = tm_p;
               DbgLv(1) << "Sim parms:ssProf:  f_scan(p)" << tm_p;
            }
         }
         else { // We remain in acceleration
            naintvs++; // Bump accel intervals count
            sum_accel += accel_c; // Build acceleration sum
         }
      }

      else { // In constant speed, looking for its end
         ss_c = ( int ) qRound(rs_c / ( double ) ss_reso) * ss_reso;
         if (ss_c != ss_p && !cspeeds.contains(ss_c)) { // Set speeds unequal:  back into acceleration
            DbgLv(1) << "Sim parms:ssProf: const-end ss_p ss_c" << ss_p << ss_c << "ndtimes" << ndtimes << "tm_c"
                     << tm_c << "rs_p rs_c" << rs_p << rs_c << "ss_c_ts" << ss_c_ts;
            ...
            if (ndtimes < ss_lo_sec || ssp.time_f_scan < 1) {
               ...
               DbgLv(1) << "Sim parms:ssProf: BACK-to-ACCEL: naintvs" << naintvs << "ndtimes" << ndtimes << "sum_accel"
                        << sum_accel << "sum_speed" << sum_speed;
               continue;
            }
         ...
         else { // We remain in constant-speed
            sum_speed += rs_c; // Add to raw speed sum
            ndtimes++; // Bump constant speed times

            if (iscan > 0) { // On a scan:  save first,last scan times
   ...
   for (int i1 = 0; i1 < ssps.count(); i1++) {
      ...
      DbgLv(1) << "Sim parms:ssProf:   rpmsz" << ssps[ i1 ].rpm_timestate.count() << "w2tsz"
               << ssps[ i1 ].w2t_timestate.count();
      for (int i2 = 0; i2 < ssps[ i1 ].duration; i2++) {
         int rst = tsobj->read_record(rtimex);
         ssps[ i1 ].rpm_timestate[ i2 ] = rsfm == "F4" ? tsobj->time_dvalue("RawSpeed")
                                                       : ( double ) tsobj->time_ivalue("RawSpeed");
         ssps[ i1 ].w2t_timestate[ i2 ] = tsobj->time_dvalue("Omega2T");
         if (i2 < 4 || (i2 + 5) > ssps[ i1 ].duration)
            DbgLv(1) << "Sim parms:ssProf:    i2" << i2 << "rtimex" << rtimex << "rpm" << ssps[ i1 ].rpm_timestate[ i2 ]
...
int US_SimulationParameters::speedstepsFromSSprof() {
   ...
   for (int ss = 0; ss < nspstep; ss++) {
      ...
      DbgLv(1) << "SP: ssFssp:  ssp:" << ipp->acceleration << ipp->w2t_b_accel << ipp->w2t_e_accel << ipp->w2t_e_step
               << ipp->avg_speed << ipp->rotorspeed << ipp->duration << ipp->time_b_accel << ipp->time_e_accel
               << ipp->time_f_scan << ipp->time_l_scan << ipp->time_e_step;
      ...
      if ((ss + 1) == nspstep) { // Insure last step's end scan extends to last time
      ...
      DbgLv(1) << "SP: ssFssp:   ss:" << opp->duration_minutes << opp->delay_minutes << opp->w2t_first << opp->w2t_last
               << opp->avg_speed << opp->speed_stddev << opp->duration_hours << opp->delay_hours << opp->time_first
               << opp->time_last << opp->scans << opp->rotorspeed << opp->acceleration << opp->set_speed
