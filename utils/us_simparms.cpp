...
void US_SimulationParameters::setHardware( IUS_DB2* db, QString rCalID, int cp, int ch )
{
    int dbg_level    = US_Settings::us_debug();
    rotorCalID       = rCalID;
    bottom_position  = 7.2;
...
int US_SimulationParameters::save_simparms( QString fname )
{
    ...
    if ( xfile.open( QIODevice::WriteOnly | QIODevice::Text ) )
    {
        ...
        {
            xml.writeAttribute("rotorcoeffs", QString("%1 %2").arg(rotorcoeffs[0], 0, 'e', 3).arg(rotorcoeffs[1], 0, 'e', 3));
        }
