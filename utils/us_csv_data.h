...
class US_UTIL_EXTERN US_CSV_Data {
 public:
   ...
   * \param columns It is a nested array with the data type
   * `QVector<QVector<double>>`, where each inner `QVector` stores the data of a
   * separate column. Therefore, all inner vectors must have the same size,
   * representing the number of rows.
   * \return True if the data is set successfully, otherwise false.
   */
  bool setData(const QString &filePath, const QStringList &headers,
   ...
   * \return True if the data is set successfully, otherwise false.
   */
  bool setData(const QStringList &headers,
               const QVector<QVector<double>> &columns);
   ...
   * \param delimiter A delimiter string. If no separator is specified, the
   * following separators are checked in order: Tab, Comma, Semicolon, and
   * Space.
