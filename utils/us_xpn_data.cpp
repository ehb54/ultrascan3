...
bool US_XpnData::connect_data(const QString adbname, const QString xpnhost,
                              const int xpnport) {
...
bool US_XpnData::connect_data(const QString xpnhost, const int xpnport,
                              const QString adbname, const QString adbuser,
                              const QString adbpasw) {
  bool status = true;
...
int US_XpnData::checkExpStatus(QString runid) {
  ...
  QString qrytext =
      "SELECT \"RunStatus\" from " + qrytab + " WHERE \"RunId\"=" + runid + ";";
...
int US_XpnData::update_isysrec(const int runId) {
  ...
  qrytext =
      "SELECT count(*) from " + qrytab + " WHERE \"RunId\"=" + sRunId + ";";
  sqry = dbxpn.exec(qrytext);
  sqry.next();
  ...
  if (nnrows < 1) {  // If no data yet, return with negative speed
    isyrec.dataId = -1;
    isyrec.runId = runId;
    isyrec.exptime = -1;
    isyrec.stageNum = 0;
  ...
  qrytext = "SELECT * from " + qrytab + " WHERE \"RunId\"=" + sRunId +
  ...
  */
  ...
  DbgLv(1) << "XpDa:ussx: update_isysrec: lastrpm etimoff " << lastrpm
           << etimoff;
...
int US_XpnData::get_runid(QString expid) {
  ...
  QString qrytext =
      "SELECT * from " + qrytab + " WHERE \"ExperimentId\"=" + expid + ";";
  sqry = dbxpn.exec(qrytext);

  while (sqry.next())  // ALEXEY: does NOT seem to see all records with the same
                       // ExpId ???
  {
  ...
  for (int i = 0; i < RunIDs.size(); ++i) {
    if (RunIDs[i] > runid) runid = RunIDs[i];
  }
  qDebug() << "RunID: " << runid;
  return runid;
}
...
int US_XpnData::scan_runs(QStringList& runInfo) {
  ...
  QString stat_text =
      tr("Scanning %1 rows from %2 table...").arg(rows).arg(tabname);
  ...
  int row = 0;
  DbgLv(1) << "XpDa:scn: tabname" << tabname << "rows" << rows << "cols" << cols
           << "qtxt" << qtxt;
  DbgLv(1) << "XpDa:inforow: run exp 0 estart ename rname A F I W stat";

  while (sqry.next()) {  // Loop to pick up values from rows of the table
    row++;
    ...
    if (!(exprow.abscnf || exprow.flscnf || exprow.inscnf || exprow.wlscnf)) {
    ...
    QString inforow = delim + QString::number(exprow.runId) + delim +
                      QString::number(exprow.expId) + delim + "0" + delim +
                      exprow.expstart.toString(Qt::ISODate).replace("T", " ") +
                      delim + exprow.expname + delim + exprow.resname + delim +
                      QString(exprow.abscnf ? "1" : "0") + delim +
                      QString(exprow.flscnf ? "1" : "0") + delim +
                      QString(exprow.inscnf ? "1" : "0") + delim +
                      QString(exprow.wlscnf ? "1" : "0") + delim +
                      QString::number(exprow.runstat);
    runInfo << inforow;
...
// Scan an XPN database for run information
int US_XpnData::scan_runs_auto(QStringList& runInfo,
  ...
  QString qtxt =
      QString("SELECT * from ") + qrytab +
      QString(" WHERE \"RunId\" = %1")
          .arg(RunID);  // ALEXEY add WHERE ExperimentID=xxx for us_comproject
  const QString delim("^");
  runInfo.clear();
  runInfo.reserve(rows);
...
int US_XpnData::filter_runs(QStringList& runInfo) {
  ...
  for (int ii = 0; ii < kruns; ii++) {
    ...
    if (exprow.abscnf) {  // Count AbsorbanceScanData
    ...
    if (exprow.flscnf) {  // Count FluorescenceScanData
    ...
    if (exprow.inscnf) {  // Count InterferenceScanData
    ...
    if (exprow.wlscnf) {  // Count WavelengthScanData
    ...
    if (kdat > 0) {  // Recreate entry where data is non-zero
      QString inforow =
          delim + QString::number(exprow.runId) + delim +
          QString::number(exprow.expId) + delim + QString::number(kdat) +
          delim + exprow.expstart.toString(Qt::ISODate).replace("T", " ") +
          delim + exprow.expname + delim + exprow.resname + delim +
          QString(exprow.abscnf ? "1" : "0") + delim +
          QString(exprow.flscnf ? "1" : "0") + delim +
          QString(exprow.inscnf ? "1" : "0") + delim +
          QString(exprow.wlscnf ? "1" : "0");
      runInfo << inforow;
      tExprun << exprow;
      nruns++;
      DbgLv(1) << "XpDa:f_r:  ii nruns" << ii << nruns
               << "INCLUDED:" << inforow;
    }
...
bool US_XpnData::import_data(const int runId, const int scanMask) {
  ...
  if (ascnf) {  // Scan and build data for Absorbance Scan Data
    arows = scan_xpndata(runId, 'A');
  }

  if (fscnf) {  // Scan and build data for Fluorescence Scan Data
    frows = scan_xpndata(runId, 'F');
  }

  if (iscnf) {  // Scan and build data for Interference Scan Data
    irows = scan_xpndata(runId, 'I');
  }

  if (wscnf) {  // Scan and build data for Wavelength Scan Data
    wrows = scan_xpndata(runId, 'W');
  }

  // Scan and build data for Centrifuge Run Profile
  int crows = scan_xpndata(runId, 'C');
  DbgLv(1) << "XpDa:i_d: arows frows irows wrows srows crows" << arows << frows
           << irows << wrows << srows << crows;
...
bool US_XpnData::import_data_auto(const int runId, const int scanMask,
                                  bool& o_conn) {
  bool status = true;
  ntscan = 0;
  ntsrow = 0;
  ...
  if (!dbxpn.open()) {
    o_conn = false;
    qDebug() << "XPN: import_data_auto: !dbxpn.open() !!! runId, scanMask, "
                "o_conn -- "
  ...
  qDebug() << "[TIME] of scan_xpndata_auto( runId, 'S', srows_bool ) "
           << int(timer_srows.elapsed() / 1000) << " sec";

  if (ascnf) {  // Scan and build data for Absorbance Scan Data
    QElapsedTimer timer_arows;
    timer_arows.start();
    arows = scan_xpndata_auto(runId, 'A', arows_bool, timer_arows);
    qDebug() << "[TIME] of scan_xpndata_auto( runId, 'A', arows_bool ) "
             << int(timer_arows.elapsed() / 1000) << " sec";
  }
  ...
  if (fscnf) {  // Scan and build data for Fluorescence Scan Data
    ...
    qDebug() << "[TIME] of scan_xpndata_auto( runId, 'F', frows_bool ) "
             << int(timer_frows.elapsed() / 1000) << " sec";
  }
  ...
  if (iscnf) {  // Scan and build data for Interference Scan Data
    ...
    qDebug() << "[TIME] of scan_xpndata_auto( runId, 'I', irows_bool ) "
             << int(timer_irows.elapsed() / 1000) << " sec";
  }
  ...
  if (wscnf) {  // Scan and build data for Wavelength Scan Data
    ...
    qDebug() << "[TIME] of scan_xpndata_auto( runId, 'W', wrows_bool ) "
             << int(timer_wrows.elapsed() / 1000) << " sec";
  }
  ...
  qDebug() << "[TIME] of scan_xpndata_auto( runId, 'C', crows_bool ) "
           << int(timer_crows.elapsed() / 1000) << " sec";

  qDebug() << "XpDa:i_d: arows frows irows wrows srows crows" << arows << frows
           << irows << wrows << srows << crows;

  qDebug() << "XpDa:i_d: [BOOLS] arows_bool frows_bool irows_bool wrows_bool "
              "srows_bool crows_bool"
           << arows_bool << frows_bool << irows_bool << wrows_bool << srows_bool
           << crows_bool;

  ntsrow = arows + frows + irows + wrows;
  qDebug() << "in [import_data_auto]:ntsrow = " << ntsrow;

  if (!arows_bool || !frows_bool || !irows_bool || !wrows_bool || !srows_bool ||
      !crows_bool) {
    o_conn = false;
...
bool US_XpnData::reimport_data(const int runId, const int scanMask) {
  ...
  if (oarows > 1 && tAsdata[0].radPath != tAsdata[1].radPath) oarows /= 2;
  if (ofrows > 0 && tFsdata[0].radPath != tFsdata[1].radPath) ofrows /= 2;
  if (owrows > 0 && tWsdata[0].radPath != tWsdata[1].radPath) owrows /= 2;
  bool ascnf = scanMask & 1;
  bool fscnf = scanMask & 2;
  bool iscnf = scanMask & 4;
  bool wscnf = scanMask & 8;
  ...
  if (ascnf) {  // Scan and update data for Absorbance Scan Data
    arows = update_xpndata(runId, 'A');
    status = (status || arows > oarows);
    DbgLv(1) << "XpDa: rei_dat: arows oarows status" << arows << oarows
             << status;
    qDebug() << "XpDa: rei_dat: arows oarows status" << arows << oarows
             << status;
  }

  if (fscnf) {  // Scan and update data for Fluorescence Scan Data
    frows = update_xpndata(runId, 'F');
    status = (status || frows > ofrows);
  }

  if (iscnf) {  // Scan and update data for Interference Scan Data
    irows = update_xpndata(runId, 'I');
    status = (status || irows > oirows);
  }

  if (wscnf) {  // Scan and update data for Wavelength Scan Data
    wrows = update_xpndata(runId, 'W');
    status = (status || wrows > owrows);
  }

  DbgLv(1) << "XpDa: rei_dat: arows frows irows wrows" << arows << frows
           << irows << wrows << "status" << status;
  qDebug() << "XpDa: rei_dat: arows frows irows wrows" << arows << frows
           << irows << wrows << "status" << status;
  //*DEBUG*
  // status=true;
  //*DEBUG*
...
bool US_XpnData::reimport_data_auto(const int runId, const int scanMask,
                                    bool& o_conn) {
  bool status = false;
  ...
  if (!dbxpn.open()) {
    o_conn = false;
    qDebug() << "XPN: reimport_data_auto: !dbxpn.open() !!! runId, scanMask, "
                "o_conn -- "
             << runId << scanMask << o_conn;
...
int US_XpnData::scan_xpndata(const int runId, const QChar scantype) {
  ...
  QString qrytext =
      "SELECT count(*) from " + qrytab + " WHERE \"RunId\"=" + sRunId + ";";
  if (scantype == 'C') qrytext = "SELECT count(*) from " + qrytab + ";";
  ...
  emit status_text(
      tr(" Scanning %1 rows of %2 table...").arg(count).arg(tabname));

  // Get columns and determine indecies of fields
  qrytext = "SELECT * from " + qrytab + " WHERE \"RunId\"=" + sRunId +
            " ORDER BY \"DataId\" ;";
  if (scantype == 'C') qrytext = "SELECT * from " + qrytab + ";";
  ...
  DbgLv(1) << "XpDa:s_x:  isctyp scantype" << isctyp << scantype << "qrytext"
           << qrytext;
    ...
    switch (isctyp) {
      case 1: {
        ...
        if (rows < 21 || (rows + 21) > count) {
          ...
          DbgLv(1) << "XpDa:scn:     rads0 rads1 vals0 vals1" << asdrow.rads[0]
                   << asdrow.rads[1] << asdrow.vals[0] << asdrow.vals[1];
          DbgLv(1) << "XpDa:scn:      etim scn temp speed omg estr"
                   << asdrow.exptime << asdrow.scanSeqN << asdrow.tempera
                   << asdrow.speed << asdrow.omgSqT
                   << asdrow.expstart.toString();
        }
        //*DEBUG*
      ...
      case 3: {
        ...
        if (rows < 9 || (rows + 9) > count) {
          ...
          DbgLv(1) << "XpDa:scn:    row" << rows << "run" << isdrow.runId
                   << "dat" << isdrow.dataId;
          DbgLv(1) << "XpDa:scn:     rads0 rads1 vals0 vals1" << isdrow.rads[0]
                   << isdrow.rads[1] << isdrow.vals[0] << isdrow.vals[1];
        }
        //*DEBUG*
        break;
      }
      case 4: {
        ...
        if (rows < 9 || (rows + 9) > count) {
          ...
          DbgLv(1) << "XpDa:scn:    row" << rows << "run dat count"
                   << wsdrow.runId << wsdrow.dataId << wsdrow.count;
        }
        //*DEBUG*
        break;
      }
      case 5: {
        ...
        //*DEBUG*
        if (rows < 21 || (rows + 21) > count) {
          DbgLv(1) << "XpDa:scn:    row" << rows << "run dat" << sydrow.runId
                   << sydrow.dataId << "time" << sydrow.exptime << "temp"
                   << sydrow.tempera << "speed" << sydrow.speed << "omg2t"
                   << sydrow.omgSqT;
        }
        //*DEBUG*
        break;
  ...
  if (scantype ==
      'C') {  // Show mapping of RunId to ExperimentId to FugeRunProfileId
    ...
    while (sqry.next()) {  // Build a list of ExperimentRun expIds and runIds
    ...
    while (sqry.next()) {  // Build a list of ExperimentDefinition expIds and
                           // fugeIds
      int dExpId = sqry.value(jexpid).toInt();
      int dFugId = sqry.value(jfruid).toInt();
      dExpIds << dExpId;
      dFugIds << dFugId;
      DbgLv(1) << "XpDa:scn:  dExpId" << dExpId << "dFugId" << dFugId << "Name"
               << sqry.value(jexpnm).toString();
    }
    ...
    for (int ii = 0; ii < tCrprof.count();
         ii++) {  // Look for matching fuge Id in CentrifugeRunProfile values
      sstintl = tCrprof[ii].sstintv;

      if (tCrprof[ii].frunId ==
          iFugId) {  // Match found:  save corresponding SystemStatusInterval
        sstintv = sstintl;
        break;
      }
    }
  ...
  if (scantype ==
      'S') {  // Determine experiment time offset (time at last rms=0 point)
    int ntssda = tSydata.count();
    fnzstx = 0;

    for (int ii = 0; ii < ntssda;
         ii++) {  // Find the index to the first non-zero speed
    ...
    fnzstx = (fnzstx > 0)
                 ? fnzstx
                 : 1;  // ALEXEY: if tSydata.count() == 1 (so only tSydata[0]
                       // exists), AND fnzstx was 0, THEN
                       //         fnzstx = ( fnzstx > 0 ) ? fnzstx : 1; will
                       //         lead to fnzstx = 1 !!! THEN   int etime1 =
                       //         tSydata[ 1 ].exptime; WILL CRASH
    int t2tx = fnzstx + 1;
...
// Query and save data for a [AIFW]ScanData or [SC] table: [for autoflow
// framework]
int US_XpnData::scan_xpndata_auto(const int runId, const QChar scantype,
                                  bool& o_conn, QElapsedTimer elapse_t) {
  QSqlQuery sqry;
  QSqlRecord qrec;
  QString schname("AUC_schema");
  QString tabname("AbsorbanceScanData");
  ...
  qDebug() << "in [scan_xpndata_auto()]: scantype, qrytext -- " << scantype
           << qrytext;

  qDebug() << "[TIME 1] of scan_xpndata_auto(): "
           << int(elapse_t.elapsed() / 1000) << " sec";

  sqry = dbxpn.exec(qrytext);
  qDebug() << "sqry.isActive() ? " << sqry.isActive();

  qDebug() << "[TIME 2] of scan_xpndata_auto(): "
           << int(elapse_t.elapsed() / 1000) << " sec";

  bool scan_xpndata_query_success = sqry.next();

  qDebug() << "[TIME 3] of scan_xpndata_auto(): "
           << int(elapse_t.elapsed() / 1000) << " sec";

  count = sqry.value(0).toInt();
  DbgLv(1) << "XpDa:s_x: sRunId" << sRunId << "count" << count;
  qDebug() << "XpDa:s_x: sRunId" << sRunId
           << "scan_xpndata_query_success, count: "
           << scan_xpndata_query_success << count;

  qDebug() << "[TIME 4] of scan_xpndata_auto(): "
           << int(elapse_t.elapsed() / 1000) << " sec";

  if (!scan_xpndata_query_success || !sqry.isActive()) {
    qDebug()
        << "in [scan_xpndata()], scan_xpndata_query_success, NO CONNECTION? "
  ...
  qDebug() << "[TIME 5] of scan_xpndata_auto(): "
           << int(elapse_t.elapsed() / 1000) << " sec";
  ...
  qDebug() << "[TIME 6] of scan_xpndata_auto(): "
           << int(elapse_t.elapsed() / 1000) << " sec";
  ...
  qDebug() << "XpDa:s_x:  isctyp scantype" << isctyp << scantype << "qrytext"
           << qrytext;
  ...
  while (sqry.next()) {
    ...
    switch (isctyp) {
      ...
      case 5: {
        ...
        qDebug() << "[TIME 7 'S'] of scan_xpndata_auto(): "
                 << int(elapse_t.elapsed() / 1000) << " sec";
        //*DEBUG*
        if (rows < 21 || (rows + 21) > count) {
          DbgLv(1) << "XpDa:scn:    row" << rows << "run dat" << sydrow.runId
          ...
          qDebug() << "XpDa:scn:    row" << rows << "run dat" << sydrow.runId
                   << sydrow.dataId << "time" << sydrow.exptime << "temp"
                   << sydrow.tempera << "speed" << sydrow.speed << "omg2t"
                   << sydrow.omgSqT;
        }
      ...
      case 6: {
        ...
        qDebug() << "[TIME 7 'C'] of scan_xpndata_auto(): "
                 << int(elapse_t.elapsed() / 1000) << " sec";
        //*DEBUG*
        if (rows < 21 || (rows + 21) > count) {
          DbgLv(1) << "XpDa:scn:    row" << rows << "fugeRunId" << crprow.frunId
  ...
  if (scantype ==
    ...
    qDebug() << "[TIME 8 'C'] of scan_xpndata_auto(): "
             << int(elapse_t.elapsed() / 1000) << " sec";

    sqry = dbxpn.exec(qrytext);

    qDebug() << "[TIME 9 'C'] of scan_xpndata_auto(): "
             << int(elapse_t.elapsed() / 1000) << " sec";

    qrec = dbxpn.record(qrytab);

    qDebug() << "[TIME 10 'C'] of scan_xpndata_auto(): "
             << int(elapse_t.elapsed() / 1000) << " sec";
    ...
    qDebug() << "[TIME 11 'C'] of scan_xpndata_auto(): "
             << int(elapse_t.elapsed() / 1000) << " sec";
    ...
    qDebug() << "[TIME 12 'C'] of scan_xpndata_auto(): "
             << int(elapse_t.elapsed() / 1000) << " sec";

    sqry = dbxpn.exec(qrytext);

    qDebug() << "[TIME 13 'C'] of scan_xpndata_auto(): "
             << int(elapse_t.elapsed() / 1000) << " sec";

    qrec = dbxpn.record(qrytab);

    qDebug() << "[TIME 14 'C'] of scan_xpndata_auto(): "
             << int(elapse_t.elapsed() / 1000) << " sec";
    ...
    qDebug() << "[TIME 15 'C'] of scan_xpndata_auto(): "
             << int(elapse_t.elapsed() / 1000) << " sec";
  ...
  if (scantype ==
    ...
    qDebug() << "[TIME 8 'S'] of scan_xpndata_auto(): "
             << int(elapse_t.elapsed() / 1000) << " sec";
    ...
    qDebug() << "[TIME 9 'S'] of scan_xpndata_auto(): "
             << int(elapse_t.elapsed() / 1000) << " sec";
  }
...
int US_XpnData::update_xpndata(const int runId, const QChar scantype) {
  ...
  if (scantype == 'A') {  // Get Absorbance old and new counts
    int tknt = tAsdata.count();
    nradps = (tAsdata[0].radPath == tAsdata[1].radPath) ? 1 : 2;
    for (int ii = 0; ii < tknt; ii++) {
      if (tAsdata[ii].runId == runId) norows++;
    }
    norows /= nradps;
  ...
  qrytext = "SELECT count(*) from " + qrytab + " WHERE \"RunId\"=" + sRunId +
            " AND \"ExperimentTime\">=" + sExpTm + ";";
  sqry = dbxpn.exec(qrytext);
  sqry.next();
  int narows = sqry.value(0).toInt();
  DbgLv(1) << "XpDa:updx:  qrytext" << qrytext;
  DbgLv(1) << "XpDa:updx:  norows" << norows << "nnrows" << nnrows << "narows"
           << narows;
  ...
  if (nnrows > 0) {  // There are new rows, so update data tables
    qrytext = "SELECT * from " + qrytab + " WHERE \"RunId\"=" + sRunId +
              " AND \"ExperimentTime\">=" + sExpTm + ";";
    sqry = dbxpn.exec(qrytext);
    int cols = column_indexes(tabname, cnames, cxs);
...
void US_XpnData::load_auc(QVector<US_DataIO::RawData>& allData,
  ...
  for (int trx = 0; trx < nfile; trx++) {
    ...
    if (!cells.contains(cell)) cells << cell;  // Build unique cell list

    if (!chans.contains(chan)) chans << chan;  // Build unique channel list
  ...
  QString stat_text =
      tr("%1 triples loaded from %2 ...").arg(ntriple).arg(runID);
  emit status_text(stat_text);
}
...
int US_XpnData::indexOfLambda(int lambda) {
  ...
  if (wvx < 0) {  // If lambda is not in the list, find the nearest to a match
    int diflow = 9999;

    for (int ii = 0; ii < nlambda; ii++) {
      int difval = qAbs(lambda - wavelns[ii]);

      if (difval < diflow) {  // New low difference, so save index and new low
        wvx = ii;
        diflow = difval;
      }
    }
...
int US_XpnData::build_rawData(QVector<US_DataIO::RawData>& allData) {
  ...
  int wvx = 0;
  int scnnbr = 0;
  nscnn = scnnbrs.count();  // ALEXEY: is nscnn, the #scans, is the same for all
                            // stages ? NO!
  int stgnbr = 0;
  nstgn = stgnbrs.count();
  ntscan = 0;
  DbgLv(1) << "BldRawD szs: ccd" << ccdescs.size() << "wvs" << wavelns.size()
           << "nlambda" << nlambda << "nscnn" << nscnn;
  ...
  for (int trx = 0; trx < ntriple; trx++) {
    ...
    qDebug() << "BUILD_RAW_DATA trip: descr:  " << trnode << "descr"
             << rdata.description << "celchn" << celchn;

    QDateTime time10 = QDateTime::currentDateTime();
    for (int sgx = 0; sgx < nstgn; sgx++) {  // Set scan values
      stgnbr = stgnbrs[sgx];
      for (int scx = 0; scx < nscnn; scx++) {  // Set scan values
        scnnbr = scnnbrs[scx];
        ...
        if (scx < 3 || (scx + 4) > nscnn)
          DbgLv(1) << "BldRawD       trx wvx scx scnnbr" << trx << wvx << scx
                   << scnnbr << "wavl" << scan.wavelength;

        //*DEBUG*
        if (trx == 0) {
          DbgLv(1) << "BldRawD      scx" << scx << "trx" << trx << "seconds"
                   << scan.seconds << "rpm" << scan.rpm;
        }
        //*DEBUG*
        int kpoint = get_readings(scan.rvalues, trx, sgx, scx);

        if (kpoint < 0) continue;  // Skip output if no stage,scan match

        if (scx < 3 || (scx + 4) > nscnn)
          DbgLv(1) << "BldRawD        scx" << scx << "rvalues size"
                   << scan.rvalues.size() << "rvalues[mid]"
                   << scan.rvalues[scan.rvalues.size() / 2];
    ...
    DbgLv(1) << "BldRawD     trx" << trx << " saving allData... ndscan"
             << ndscan;
    allData << rdata;  // Append triple data to the array

    QString stat_text =
        tr("Of %1 raw AUCs, built %2").arg(ntriple).arg(trx + 1);
    emit status_text(stat_text);

    wvx++;

    if (wvx >= nlambda) {  // After final wavelength, reset at next cell/channel
      ccx++;
      wvx = 0;
...
int US_XpnData::rebuild_rawData(QVector<US_DataIO::RawData>& allData) {
  ...
  int wvx = 0;
  int scnnbr = 0;
  nscnn = scnnbrs.count();  // ALEXEY: is nscnn (# scan) is the same for all
                            // stages ? NO!
  int stgnbr = 0;
  nstgn = stgnbrs.count();
  DbgLv(1) << "rBldRawD ntriple nstgn nscnn" << ntriple << nstgn << nscnn
           << "st0 stn" << stgnbrs[0] << stgnbrs[nstgn - 1] << "sc0 scn"
           << scnnbrs[0] << scnnbrs[nscnn - 1];
  time20 = QDateTime::currentDateTime();
  timi1 += time10.msecsTo(time20);
  QDateTime time30 = time20;
  int timi7 = 0;

  for (int trx = 0; trx < ntriple; trx++) {  // Update scans for each triple
    ...
    for (int sgx = 0; sgx < nstgn; sgx++) {  // Set stage values
      stgnbr = stgnbrs[sgx];
      for (int scx = 0; scx < nscnn;
           scx++)  // ALEXEY: is nscnn (# scan) is the same for all stages ? NO!
      {            // Set scan values
        time10 = QDateTime::currentDateTime();
        scnnbr = scnnbrs[scx];
        ...
        if (scx < 3 || (scx + 4) > nscnn)
          DbgLv(1) << "rBldRawD       trx wvx scx scnnbr" << trx << wvx << scx
                   << scnnbr << "wavl" << scan.wavelength;

        //*DEBUG*
        if (trx == 0) {
          DbgLv(1) << "rBldRawD      scx" << scx << "trx" << trx << "seconds"
                   << scan.seconds << "rpm" << scan.rpm;
        }
        //*DEBUG*
        time20 = QDateTime::currentDateTime();
        ...
        if (scx < 3 || (scx + 4) > nscnn)
          DbgLv(1) << "rBldRawD        scx" << scx << "rvalues size"
                   << scan.rvalues.size() << "rvalues[mid]"
                   << scan.rvalues[scan.rvalues.size() / 2];

        // if ( ( scx + 1 ) == oscknt )
        if (ndscan == oscknt) {  // If last of old scans, update in case
                                 // readings were added

          // int ksc           = sgx * nscnn + scx; //ALEXEY: crashes after this
          // line: is this correct for ksc at multi-speed ? ALEXEY: is nscnn (#
          // scan) is the same for all stages ? NO!

          int ksc = oscknt - 1;

          qDebug() << "INSIDE xpn_data: AFTER get_readings() 2a: ksc = sgx * "
                      "nscnn + scx: "
                   << sgx << " * " << nscnn << " + " << scx;
          qDebug() << "INSIDE xpn_data: AFTER get_readings() 2: ksc: " << ksc
                   << ", rdata->scanData.size(): " << rdata->scanData.size();

          rdata->scanData[ksc] = scan;
          DbgLv(1) << "rBldRawD        ksc" << ksc << "++Scan Replace";
        } else {  // If new scan, append the scan to the triple
          rdata->scanData << scan;
          int nscknt = rdata->scanCount();
          DbgLv(1) << "rBldRawD        ndscan oscknt" << ndscan << oscknt
                   << "++Scan Add  nscknt" << nscknt;
        }
        time20 = QDateTime::currentDateTime();
    ...
    DbgLv(1) << "rBldRawD    trx" << trx << " saving allData... ndscan"
             << ndscan;

    QString stat_text =
        tr("Of %1 raw AUCs, updated %2").arg(ntriple).arg(trx + 1);
    emit status_text(stat_text);
    ...
    if (wvx >= nlambda) {  // After final wavelength, reset at next cell/channel
      ...
      if (memAv < low_memApc) {
        //            tscans.clear();
        int memAv2 = US_Memory::memory_profile();
        DbgLv(1) << "rBldRawD  memfree %: 1memAV" << memAv << "2memAV"
                 << memAv2;
      }
    }
    time20 = QDateTime::currentDateTime();
  ...
  DbgLv(1) << "rBldRawD   timi1" << timi1 << "timi2" << timi2 << "timi3"
           << timi3 << "timi4" << timi4 << "timi5" << timi5 << "timi6" << timi6
...
int US_XpnData::export_auc(QVector<US_DataIO::RawData>& allData) {
  ...
  QString ftype = QString(allData[0].type[0]) + QString(allData[0].type[1]);
  ...
  for (int ii = 0; ii < ntrips; ii++) {  // Create a file for each triple
    ...
    DbgLv(1) << "expA: ii" << ii << "trnodes[ii]" << trnodes[ii] << "trnode"
             << trnode;
    ...
    DbgLv(1) << "expA:   nf" << nfiles << "fname" << fname << "kscans" << kscans
             << "mxscans" << mxscans << "iiuse" << iiuse;
  }
  ...
  DbgLv(1) << "expA:   ntimes ntssda" << ntimes << ntssda << "e_utime e_stime"
           << e_utime << e_stime;
  ...
  if (e_utime > e_stime) {
    ...
    DbgLv(1) << "expA:    UPDATED ntssda" << ntssda << "e_stime" << e_stime
             << "idRun" << idRun;
  }
  //*DEBUG*
  ...
  US_Math2::linefit(&xa_scn, &ya_scn, &slope_s, &icept_s, &sigma_s, &corre_s,
                    nscnp);
  US_Math2::linefit(&xa_tms, &ya_tms, &slope_t, &icept_t, &sigma_t, &corre_t,
                    ntmsp);
  DbgLv(1) << "expA:DLT:SCN: npoint" << nscnp << "x1,y1,x2,y2" << xa_scn[0]
           << ya_scn[0] << xa_scn[nscnp - 1] << ya_scn[nscnp - 1];
  DbgLv(1) << "expA:DLT:TMS: npoint" << ntmsp << "x1,y1,x2,y2" << xa_tms[0]
           << ya_tms[0] << xa_tms[ntmsp - 1] << ya_tms[ntmsp - 1];
  ...
  US_Math2::linefit(&ya_scn, &xa_scn, &slope_s, &icept_s, &sigma_s, &corre_s,
                    nscnp);
  US_Math2::linefit(&ya_tms, &xa_tms, &slope_t, &icept_t, &sigma_t, &corre_t,
                    ntmsp);
  DbgLv(1) << "expA:DLT:SCN: npoint" << nscnp << "x1,y1,x2,y2" << ya_scn[0]
           << xa_scn[0] << ya_scn[nscnp - 1] << xa_scn[nscnp - 1];
  DbgLv(1) << "expA:DLT:TMS: npoint" << ntmsp << "x1,y1,x2,y2" << ya_tms[0]
           << xa_tms[0] << ya_tms[ntmsp - 1] << xa_tms[ntmsp - 1];
  DbgLv(1) << "expA:DLT:SCN:  slope" << slope_s << "intercept" << icept_s;
  DbgLv(1) << "expA:DLT:TMS:  slope" << slope_t << "intercept" << icept_t;
  DbgLv(1) << "expA:DLT:SCN:   stddev" << sigma_s << "correlation" << corre_s;
  DbgLv(1) << "expA:DLT:TMS:   stddev" << sigma_t << "correlation" << corre_t;
  ...
  if (ntssda < ntimes) {
    ...
    for (int ii = 0; ii < ntimes; ii++) {  // Create a record for each scan
  ...
  } else {  // Build TMST from System Status Data table values
    int ftx = 1;
    int stgoff = 0;
    ...
    DbgLv(1) << "expA: ftx" << ftx << "etimoff" << etimoff << "ntssda"
             << ntssda;
    QList<int> sctimes;

    for (int ii = 0; ii < ntimes; ii++) {  // Build a list of scan times
      US_DataIO::Scan* uscan = &udata->scanData[ii];
    ...
    for (int ii = ftx; ii < ntimes;
         ii++) {            // Create a record for each system status row
      ...
      if (time_e > ietime ||
          ((speed_n < 100) &&
           (ii > (ftx + 10)))) {  // Time well beyond last scan or speed
                                  // dropping back down to zero
        DbgLv(1) << "expA:   ii" << ii << "ftx" << ftx << "stage" << stage
                 << "time_n" << time_n << "speed_n" << speed_n << "omg2t_n"
                 << omg2t_n << "*BREAK*";
        break;
      }
      ...
      for (int jj = 0; jj < timeinc;
           jj++) {  // Expand values for each second in present range
        ...
        DbgLv(1) << "expA:      jj" << jj << "scan" << scannbr << "time_c"
                 << time_c << "speed" << rawSpeed << isSpeed << "omg2t(rec)"
                 << omega2t << "omg2t(sum)" << omg2t_sm;
        omega2t = omg2t_sm;
...
int US_XpnData::export_auc_auto(QVector<US_DataIO::RawData>& allData,
                                bool& tmstampOK) {
  int nfiles = 0;
#if 0
   if ( ! is_raw )
...
void US_XpnData::set_scan_data(const int datx, const QString arType) {
  ...
  if (datx < 5)
    DbgLv(1) << "XpDa:ssd: datx" << datx << "csd: dId rId scan" << csdrec.dataId
             << csdrec.runId << csdrec.scanSeqN << "rType" << rType;
}
...
#if 1
// Return a scan data index within the ScanData vector
int US_XpnData::scan_data_index(const QString trnode, const int mstage,
                                const int mscann) {
  static int pdatx = 0;
  QDateTime time10;
  QDateTime time20;
  ...
  if (runType == "RI") {
    ...
    for (int ii = 0; ii < sdknt; ii++) {
      ...
      if ((asdrec->scanSeqN == mscann) && (asdrec->stageNum == mstage) &&
          (asdrec->wavelen == mwavl) && (asdrec->radPath == mchan) &&
    ...
    for (int ii = 0; ii < sdknt; ii++) {
      ...
      if ((fsdrec->scanSeqN == mscann) && (fsdrec->stageNum == mstage) &&
          (fsdrec->wavelen == mwavl) && (fsdrec->radPath == mchan) &&
    ...
    for (int ii = 0; ii < sdknt; ii++) {
      ...
      if ((isdrec->scanSeqN == mscann) && (isdrec->stageNum == mstage) &&
          (isdrec->wavelen == mwavl) && (mchan == "A") &&
    ...
    for (int ii = 0; ii < sdknt; ii++) {
      ...
      if ((wsdrec->scanSeqN == mscann) && (wsdrec->stageNum == mstage) &&
          (wsdrec->scanPos == mwavl) && (wsdrec->radPath == mchan) &&
...
int US_XpnData::get_readings(QVector<double>& rvalues, const int trx,
                             const int sgx, const int scx) {
  QString trnode = trnodes[trx];
  ...
  if (datx < 0) {
    DbgLv(0) << "XpDa:grd: *** Unable to find data for" << trnode << stgnbr
             << scnnbr;
    return datx;
  }
...
void US_XpnData::interp_rvalues(QVector<double>& rads, QVector<double>& vals,
  ...
  for (int ii = 0; ii < npoint; ii++) {
    ...
    for (int jj = jjs; jj < radknt; jj++) {
      ...
      if (drad == 0.0) continue;
      valout = vals[jj];
      // if(valout<(-1e+66))
      // DbgLv(1) << "XpDa:irv: ii jj" << ii << jj << "radv2" << radv2 <<
      // "valout" << valout;
...
QString US_XpnData::runDetails(void) {
  ...
  if (is_raw) {
    msg += tr(" from Database at Host:Port %1:%2 .").arg(dbhost).arg(dbport);
    msg += tr("\nData Summary.\n");
  }
  ...
  DbgLv(1) << "XpDa:Det: sdknt ssknt" << sdknt << ssknt << "  e_dtime e_stime"
           << e_dtime << e_stime;
  if (e_stime < e_dtime) {  // For System Status last time less than last data
                            // time, rescan SysStat
    tSydata.clear();
...
QString US_XpnData::lastError(void) { return dbxpn.lastError().text(); }
...
void US_XpnData::build_internals() {
  ...
  // ncelchn       = 0;  //ALEXEY do we need to reset it fro automated flow and
  // in general ?
  ...
  for (int ii = 0; ii < sdknt;
       ii++) {  // Get cell/channel, triple, lambda lists
    set_scan_data(ii);
    ...
    QString darec = tnode + "." + QString().sprintf("%05i.%05i", stage, scnnbr);
    DbgLv(1) << "XpDa:b_i: ii" << ii << "schan cechn" << schan << cechn
             << "darec" << darec << "rad0 rad1" << csdrec.rads->at(0)
             << csdrec.rads->at(1);
    ...
    if (vcount != rknt || vcount != vknt) {
      qDebug() << "*** ii vcount rknt vknt tnode" << ii << vcount << rknt
               << vknt << tnode;
    }
  ...
  for (int ii = 0; ii < triples.count(); ii++) {
    ...
    QString cechn = scell + " / " + schan;
    DbgLv(1) << "XpDa:b_i:trip: ii" << ii << "icell ichan schan cechn" << icell
             << ichan << schan << cechn << "tripl" << tripl;
    ...
    if (!cellchans.contains(cechn)) {
      ...
      for (int jj = 0; jj < sdknt;
           jj++) {  // Use Samples data to get cell/channel description
        set_scan_data(jj);
        ...
        if (jcell == icell && jchan == ichan) {
          ...
          DbgLv(1) << "XpDa:b_i:     samplName" << csdrec.samplName
                   << "ccdescs sz" << ccdescs.count();
          qDebug() << "::build_internals(); csdrec.samplName -- "
                   << csdrec.samplName;
          break;
        }
      }
  ...
  DbgLv(1) << "XpDa:b_i:       kdarec" << datrecs.count() << "da0 dan"
           << datrecs[0] << datrecs[datrecs.count() - 1];
  DbgLv(1) << "XpDa:b_i:       kstgn " << stgnbrs.count() << "stg0 stgn"
           << stgnbrs[0] << stgnbrs[stgnbrs.count() - 1];
  DbgLv(1) << "XpDa:b_i:       kscnn " << scnnbrs.count() << "scn0 scnn"
           << scnnbrs[0] << scnnbrs[scnnbrs.count() - 1];
  qSort(datrecs);
  qSort(stgnbrs);
  qSort(scnnbrs);
  DbgLv(1) << "XpDa:b_i:       kdarec" << datrecs.count() << "da0 dan"
...
void US_XpnData::rebuild_internals() {
  ...
  for (int ii = 0; ii < sdknt;
    ...
    QString darec = tnode + "." + QString().sprintf("%05i.%05i", stage, scnnbr);
    DbgLv(1) << "XpDa:b_i: ii" << ii << "scnnbr" << scnnbr << "darec" << darec;
  ...
  DbgLv(1) << "XpDa:b_i:       kdarec" << datrecs.count() << "da0 dan"
           << datrecs[0] << datrecs[datrecs.count() - 1];
  DbgLv(1) << "XpDa:b_i:       kstgn " << stgnbrs.count() << "scn0 scnn"
           << scnnbrs[0] << scnnbrs[scnnbrs.count() - 1];
}
...
int US_XpnData::column_indexes(const QString tabname, QStringList& cnames,
  ...
  if (tabname == "AbsorbanceScanData") {
    ...
    flds << "RunId" << "ExperimentId" << "RotorSN" << "DataPath"
         << "ExperimentStart" << "InstrumentSN" << "ScienceModule1SN"
         << "ScienceModule2SN"
         << "ScienceModule3SN" << "RunStatus" << "ExperimentDefinition"
         << "ExperimentName"
         << "ResearcherName" << "AbsorbanceScan" << "FluorescenceScan"
         << "InterferenceScan"
         << "WavelengthScan";
  }
    ...
    flds << "FugeRunProfileId" << "Temperature" << "StepBraking"
         << "BrakingRate"
         << "HoldTempAfterFinal" << "HoldSpeedAfterFinal"
         << "SetTempBeforeStart"
  ...
  if (ncols != nflds) {
    qDebug() << "*WARNING:  for" << tabname << "field,column counts:" << nflds
             << ncols;
  }
  ...
  DbgLv(1) << "XpDa:cox: ncols" << ncols << "nflds" << nflds << "tabname"
           << tabname;
...
void US_XpnData::update_ATable(QSqlQuery& sqry, QList<int>& cxs) {
  ...
  // DbgLv(1) << "XpDa:updA: sPoss" << QString(sPoss).left(20) << "exptime
  // scanSeqN"
  ...
  for (int ii = strow; ii > enrow;
       ii--) {  // Look from end for any dataId match in already captured rows
    if (tAsdata[ii].dataId == asdrow.dataId) {  // Match: get index(es)
      if (mdx1 < 0)
        mdx1 = ii;
      else {
        mdx2 = ii;
  ...
  if (asdrow.radPath == " " ||
      asdrow.radPath.isEmpty()) {  // Store both an 'A' and 'B' channel record
    asdrow.radPath = "A";
    if (mdx1 < 0)
  ...
  else {  // Store either an 'A' or a 'B' channel record
    if (mdx1 < 0)
      tAsdata << asdrow;  // Update table with data entry
    else
      tAsdata[mdx1] = asdrow;  // Replace table data entry
...
void US_XpnData::update_FTable(QSqlQuery& sqry, QList<int>& cxs) {
  ...
  for (int ii = strow; ii > enrow;
       ii--) {  // Look from end for any dataId match in already captured rows
    if (tFsdata[ii].dataId == fsdrow.dataId) {  // Match: get index(es)
      if (mdx1 < 0)
        mdx1 = ii;
      else {
        mdx2 = ii;
  ...
  if (fsdrow.radPath == " " ||
      fsdrow.radPath.isEmpty()) {  // Store both an 'A' and 'B' channel record
    fsdrow.radPath = "A";
    if (mdx1 < 0)
...
void US_XpnData::update_ITable(QSqlQuery& sqry, QList<int>& cxs) {
  ...
  for (int ii = strow; ii > enrow;
       ii--) {  // Look from end for any dataId match in already captured rows
    if (tIsdata[ii].dataId == isdrow.dataId) {  // Match: get index(es)
      if (mdx1 < 0) {
        mdx1 = ii;
        break;
      }
...
void US_XpnData::update_WTable(QSqlQuery& sqry, QList<int>& cxs) {
  ...
  for (int ii = strow; ii > enrow;
       ii--) {  // Look from end for any dataId match in already captured rows
    if (tWsdata[ii].dataId == wsdrow.dataId) {  // Match: get index(es)
      if (mdx1 < 0)
        mdx1 = ii;
      else {
        mdx2 = ii;
  ...
  if (wsdrow.radPath == " " ||
      wsdrow.radPath.isEmpty()) {  // Store both an 'A' and 'B' channel record
    wsdrow.radPath = "A";
    if (mdx1 < 0)
