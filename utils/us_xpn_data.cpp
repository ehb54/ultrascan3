...
bool
US_XpnData::connect_data (const QString adbname, const QString xpnhost,
                          const int xpnport)
{
...
// Connect to a selected database server with XPN data
bool
US_XpnData::connect_data (const QString xpnhost, const int xpnport,
...
int
US_XpnData::checkExpStatus (QString runid)
{
  if (!dbxpn.open ())
    {
...
int
US_XpnData::checkExpStatus_auto (QString runid, bool &o_conn)
{
  ...
  if (!dbxpn.open ())
    {
      o_conn = false;
      qDebug ()
          << "XPN: checkExpStatus_auto: !dbxpn.open() !!! runid, o_conn -- "
          << runid << o_conn;
...
void
US_XpnData::setEtimOffZero (void)
{
  // etimoff  = 0;
  // fnzstx   = 0;
...
int
US_XpnData::update_isysrec (const int runId)
{
  int lastrpm = -1;
...
int
US_XpnData::get_runid (QString expid)
{
  // if ( !dbxpn.isOpen() )
  //    return 0;
  ...
  while (sqry.next ()) // ALEXEY: does NOT seem to see all records with the
                       // same ExpId ???
    {
...
int
US_XpnData::scan_runs (QStringList &runInfo)
{
  int nruns = 0;
  ...
  int row = 0;
  DbgLv (1) << "XpDa:scn: tabname" << tabname << "rows" << rows << "cols"
            << cols << "qtxt" << qtxt;
  ...
  while (sqry.next ())
    { // Loop to pick up values from rows of the table
      ...
      if (!(exprow.abscnf || exprow.flscnf || exprow.inscnf || exprow.wlscnf))
        {
          DbgLv (1) << "XpDa: NO AFIW:  run exp" << exprow.runId
                    << exprow.expId << "expname resname" << exprow.expname
                    << exprow.resname;
          continue;
        }
      ...
      QString inforow
          = delim + QString::number (exprow.runId) + delim
            ...
            + exprow.expstart.toString (Qt::ISODate).replace ("T", " ") + delim
            + exprow.expname + delim + exprow.resname + delim
...
// Scan an XPN database for run information
int
US_XpnData::scan_runs_auto (QStringList &runInfo, QString &RunID_to_retrieve)
{
  int nruns = 0;
  ...
  QString qtxt = QString ("SELECT * from ") + qrytab
                 + QString (" WHERE \"RunId\" = %1")
                       .arg (RunID); // ALEXEY add WHERE ExperimentID=xxx for
                                     // us_comproject
  const QString delim ("^");
  runInfo.clear ();
  runInfo.reserve (rows);
...
int
US_XpnData::filter_runs (QStringList &runInfo)
{
  int nruns = 0;
  ...
  for (int ii = 0; ii < kruns; ii++)
    {
      ...
      if (kdat > 0)
        { // Recreate entry where data is non-zero
          QString inforow
              = delim + QString::number (exprow.runId) + delim
                + QString::number (exprow.expId) + delim
                + QString::number (kdat) + delim
...
bool
US_XpnData::import_data (const int runId, const int scanMask)
{
  bool status = true;
  ntscan = 0;
  ntsrow = 0;
  ...
  DbgLv (1) << "XpDa:i_d: arows frows irows wrows srows crows" << arows
            << frows << irows << wrows << srows << crows;
...
bool
US_XpnData::import_data_auto (const int runId, const int scanMask,
                              bool &o_conn)
{
  bool status = true;
  ntscan = 0;
  ...
  qDebug () << "XpDa:i_d: arows frows irows wrows srows crows" << arows
            << frows << irows << wrows << srows << crows;

  qDebug () << "XpDa:i_d: [BOOLS] arows_bool frows_bool irows_bool wrows_bool "
               "srows_bool crows_bool"
            << arows_bool << frows_bool << irows_bool << wrows_bool
            << srows_bool << crows_bool;
...
bool
US_XpnData::reimport_data (const int runId, const int scanMask)
{
  bool status = false;
...
bool
US_XpnData::reimport_data_auto (const int runId, const int scanMask,
                                bool &o_conn)
{
  bool status = false;
  ...
  if (!dbxpn.open ())
    {
      o_conn = false;
      qDebug () << "XPN: reimport_data_auto: !dbxpn.open() !!! runId, "
                   "scanMask, o_conn -- "
                << runId << scanMask << o_conn;
...
int
US_XpnData::scan_xpndata (const int runId, const QChar scantype)
{
  QSqlQuery sqry;
  QSqlRecord qrec;
  QString schname ("AUC_schema");
            ...
            if (rows < 21 || (rows + 21) > count)
              {
                ...
                DbgLv (1) << "XpDa:scn:    row" << rows << "run"
                          << asdrow.runId << "dat" << asdrow.dataId << "count"
                          << asdrow.count;
                DbgLv (1) << "XpDa:scn:     rads0 rads1 vals0 vals1"
                          << asdrow.rads[0] << asdrow.rads[1] << asdrow.vals[0]
                          << asdrow.vals[1];
                DbgLv (1) << "XpDa:scn:      etim scn temp speed omg estr"
                          << asdrow.exptime << asdrow.scanSeqN
                          << asdrow.tempera << asdrow.speed << asdrow.omgSqT
            ...
            if (rows < 9 || (rows + 9) > count)
              {
                ...
                DbgLv (1) << "XpDa:scn:    row" << rows << "run"
                          << fsdrow.runId << "dat" << fsdrow.dataId;
            ...
            if (rows < 9 || (rows + 9) > count)
              {
                ...
                DbgLv (1) << "XpDa:scn:    row" << rows << "run"
                          << isdrow.runId << "dat" << isdrow.dataId;
                DbgLv (1) << "XpDa:scn:     rads0 rads1 vals0 vals1"
                          << isdrow.rads[0] << isdrow.rads[1] << isdrow.vals[0]
                          << isdrow.vals[1];
              }
            //*DEBUG*
            break;
        ...
        case 5:
          {
            ...
            if (rows < 21 || (rows + 21) > count)
              {
                DbgLv (1) << "XpDa:scn:    row" << rows << "run dat"
                          << sydrow.runId << sydrow.dataId << "time"
                          << sydrow.exptime << "temp" << sydrow.tempera
                          << "speed" << sydrow.speed << "omg2t"
                          << sydrow.omgSqT;
              }
            //*DEBUG*
            break;
        ...
        case 6:
          {
            ...
            if (rows < 21 || (rows + 21) > count)
              {
                DbgLv (1) << "XpDa:scn:    row" << rows << "fugeRunId"
                          << crprow.frunId << "ssInterval" << crprow.sstintv;
              }
            //*DEBUG*
            break;
          }
...
// framework]
int
US_XpnData::scan_xpndata_auto (const int runId, const QChar scantype,
            ...
            if (rows < 21 || (rows + 21) > count)
              {
                ...
                qDebug () << "XpDa:scn:    row" << rows << "run dat"
                          << sydrow.runId << sydrow.dataId << "time"
                          << sydrow.exptime << "temp" << sydrow.tempera
                          << "speed" << sydrow.speed << "omg2t"
                          << sydrow.omgSqT;
            ...
            if (rows < 21 || (rows + 21) > count)
              {
                DbgLv (1) << "XpDa:scn:    row" << rows << "fugeRunId"
                          << crprow.frunId << "ssInterval" << crprow.sstintv;
                qDebug () << "XpDa:scn:    row" << rows << "fugeRunId"
                          << crprow.frunId << "ssInterval" << crprow.sstintv;
              }
            //*DEBUG*
            break;
...
int
US_XpnData::update_xpndata (const int runId, const QChar scantype)
{
  QStringList cnames;
  QList<int> cxs;
  QSqlQuery sqry;
  ...
  if (scantype == 'A')
    { // Get Absorbance old and new counts
      ...
      DbgLv (1) << "XpDa:updx:  sdat count" << tknt << "last-exptime"
                << sExpTm;
      ;
    }
  else if (scantype == 'F')
...
int
US_XpnData::parse_doubles (const QString svals, QVector<double> &dvals)
{
...
void
US_XpnData::load_auc (QVector<US_DataIO::RawData> &allData,
                      const QStringList ifpaths)
{
  ...
  DbgLv (1) << "XpDa:LdMw: nfile" << nfile << "nscan npoint" << nscan
            << npoint;
  ...
  for (int trx = 0; trx < nfile; trx++)
    {
      ...
      if (!triples.contains (triple))
        ...
        { // Handle duplicate triple
          DbgLv (1) << "XpDa:LdMw:    DUPLICATE triple" << triple << "trx"
                    << trx;
              ...
              if (!triples.contains (triple))
                {
                  ...
                  DbgLv (1)
                      << "XpDa:LdMw:      NEW triple" << triple << trnode;
                  triples << triple;
                }
            }
        }
...
int
US_XpnData::lambdas_raw (QVector<int> &wls)
{
  wls.clear ();
  wls.reserve (nlambda);
...
int
US_XpnData::indexOfLambda (int lambda)
{
...
int
US_XpnData::cellchannels (QStringList &celchns)
{
  celchns = cellchans;
  return ncelchn;
}
...
int
US_XpnData::data_triples (QStringList &trips)
{
  trips = triples;
...
int
US_XpnData::build_rawData (QVector<US_DataIO::RawData> &allData)
{
  DbgLv (1) << "BldRawD IN";
  ...
  int wvx = 0;
  int scnnbr = 0;
  nscnn = scnnbrs.count (); // ALEXEY: is nscnn, the #scans, is the same for
                            // all stages ? NO!
  int stgnbr = 0;
  nstgn = stgnbrs.count ();
  ntscan = 0;
  DbgLv (1) << "BldRawD szs: ccd" << ccdescs.size () << "wvs"
            << wavelns.size () << "nlambda" << nlambda << "nscnn" << nscnn;
  ...
  for (int trx = 0; trx < ntriple; trx++)
    {
      ...
      rdata.channel
          = celchn.section ("/", 1, 1).simplified ().toLatin1 ().data ()[0];
      ...
      DbgLv (1) << "BldRawD         trip" << trnode << "descr"
          ...
          for (int scx = 0; scx < nscnn; scx++)
            { // Set scan values
              ...
              if (scx < 3 || (scx + 4) > nscnn)
                DbgLv (1) << "BldRawD       trx wvx scx scnnbr" << trx << wvx
                          << scx << scnnbr << "wavl" << scan.wavelength;
          ...
          if (memAv < low_memApc)
            {
              ...
              DbgLv (1) << "BldRawD  memfree %: 1memAV" << memAv << "2memAV"
                        << memAv2;
            }
        }
    } // END: triple loop
      ...
      for (int scx = 0; scx < rdata->scanCount (); scx++)
        {
          ...
          DbgLv (1) << "BldRawD trx" << trx << "scx" << scx << "speed"
                    << speed;
        }
    }
  //*DEBUG*
...
int
US_XpnData::rebuild_rawData (QVector<US_DataIO::RawData> &allData)
{
  DbgLv (1) << "rBldRawD IN";
  ...
  for (int trx = 0; trx < ntriple; trx++)
    { // Update scans for each triple
      DbgLv (1) << "rBldRawD     trx" << trx << " rebuilding scans... ccx"
                << ccx;
      time10 = QDateTime::currentDateTime ();
      US_DataIO::RawData *rdata = &allData[trx];
      QString triple = triples[trx].replace (" / ", "/");
      ...
      for (int sgx = 0; sgx < nstgn; sgx++)
        { // Set stage values
          ...
          for (int scx = 0; scx < nscnn;
               scx++) // ALEXEY: is nscnn (# scan) is the same for all stages ?
                      // NO!
            { // Set scan values
              time10 = QDateTime::currentDateTime ();
              scnnbr = scnnbrs[scx];
              ...
              if (scx < 3 || (scx + 4) > nscnn)
                DbgLv (1) << "rBldRawD       trx wvx scx scnnbr" << trx << wvx
                          << scx << scnnbr << "wavl" << scan.wavelength;
              ...
              if (ndscan == oscknt)
                { // If last of old scans, update in case readings were added
                  ...
                  // int ksc           = sgx * nscnn + scx; //ALEXEY: crashes
                  // after this line: is this correct for ksc at multi-speed ?
                  // ALEXEY: is nscnn (# scan) is the same for all stages ? NO!

                  int ksc = oscknt - 1;

                  qDebug () << "INSIDE xpn_data: AFTER get_readings() 2a: ksc "
                               "= sgx * nscnn + scx: "
                            << sgx << " * " << nscnn << " + " << scx;
                  qDebug () << "INSIDE xpn_data: AFTER get_readings() 2: ksc: "
                            << ksc << ", rdata->scanData.size(): "
                            << rdata->scanData.size ();

                  rdata->scanData[ksc] = scan;
                  DbgLv (1)
                      << "rBldRawD        ksc" << ksc << "++Scan Replace";
                }
              else
                { // If new scan, append the scan to the triple
                  rdata->scanData << scan;
                  int nscknt = rdata->scanCount ();
                  DbgLv (1) << "rBldRawD        ndscan oscknt" << ndscan
                            << oscknt << "++Scan Add  nscknt" << nscknt;
                }
              time20 = QDateTime::currentDateTime ();
  ...
  for (int trx = 0; trx < ntriple; trx++)
    {
      ...
      for (int scx = 0; scx < rdata->scanCount (); scx++)
        {
          double speed = rdata->scanData[scx].rpm;
          DbgLv (1) << "rBldRawD trx" << trx << "scx" << scx << "speed"
                    << speed;
        }
    }
  //*DEBUG*
  ...
  DbgLv (1) << "rBldRawD   timi1" << timi1 << "timi2" << timi2 << "timi3"
            << timi3 << "timi4" << timi4 << "timi5" << timi5 << "timi6"
            << timi6 << "   timi9" << timi9 << "  timi8 timi7" << timi8
            << timi7;
  return ntriple;
}
...
int
US_XpnData::export_auc (QVector<US_DataIO::RawData> &allData)
{
  int nfiles = 0;
#if 0
   if ( ! is_raw )
  ...
  int mxscans = 0;
  int iiuse = 0;
  DbgLv (1) << "expA: ktrips" << triples.count () << "ktnodes"
            << trnodes.count ();
  ...
  for (int ii = 0; ii < ntrips; ii++)
    { // Create a file for each triple
      ...
      DbgLv (1) << "expA:   nf" << nfiles << "fname" << fname << "kscans"
                << kscans << "mxscans" << mxscans << "iiuse" << iiuse;
    }
  ...
  if (ntssda < ntimes)
    {
      ...
      for (int ii = 0; ii < ntimes; ii++)
        { // Create a record for each scan
          ...
          double setSpeed
              = qRound (rawSpeed / (double)ss_reso) * (double)ss_reso;
          ...
          DbgLv (1) << "expA:   ii" << ii << "scan" << scannbr << "stage"
                    << istagen << "speed" << rawSpeed << isSpeed << "time"
                    << time;
    ...
    { // Build TMST from System Status Data table values
      ...
      for (int ii = ftx; ii < ntimes; ii++)
        {                      // Create a record for each system status row
          ...
          if (time_e > ietime || ((speed_n < 100) && (ii > (ftx + 10))))
            { // Time well beyond last scan or speed dropping back down to zero
              DbgLv (1) << "expA:   ii" << ii << "ftx" << ftx << "stage"
                        << stage << "time_n" << time_n << "speed_n" << speed_n
                        << "omg2t_n" << omg2t_n << "*BREAK*";
              break;
            }
          ...
          DbgLv (1) << "expA:   ii" << ii << "stage" << stage << "time_n"
                    << time_n << "speed_n" << speed_n << "omg2t_n" << omg2t_n;
...
int
US_XpnData::export_auc_auto (QVector<US_DataIO::RawData> &allData,
                             bool &tmstampOK)
{
  int nfiles = 0;
#if 0
...
int
US_XpnData::countOf (QString key)
{
  mapCounts ();
...
QString
US_XpnData::countOf_sysdata (QString key)
{
  mapCounts_sysdata ();
...
void
US_XpnData::mapCounts_sysdata (void)
{
...
QString
US_XpnData::cc_description (QString celchn)
{
...
void
US_XpnData::set_run_values (const QString arunid, const QString aruntype)
{
...
void
US_XpnData::run_values (QString &arunid, QString &aruntype)
{
  arunid = runID;
  aruntype = runType;
}
...
void
US_XpnData::set_scan_data (const int datx, const QString arType)
{
  ...
  if (datx < 5)
    DbgLv (1) << "XpDa:ssd: datx" << datx << "csd: dId rId scan"
              << csdrec.dataId << csdrec.runId << csdrec.scanSeqN << "rType"
              << rType;
}

// Private slot to map counts and sizes
void
US_XpnData::mapCounts (void)
{
  counts["file"] = nfile;
  counts["scan"] = nscan;
  counts["cell"] = ncell;
...
// Return a scan data index within the ScanData vector
int
US_XpnData::scan_data_index (const QString trnode, const int mstage,
                             const int mscann)
{
  static int pdatx = 0;
  QDateTime time10;
...
int
US_XpnData::get_readings (QVector<double> &rvalues, const int trx,
...
void
US_XpnData::interp_rvalues (QVector<double> &rads, QVector<double> &vals,
                            QVector<double> &rrads, QVector<double> &rvals)
{
  int jjs = 1;
          ...
          if (radv < radv2 || jj == lstrx)
            {
              ...
              jjs = jj;
              // if(valout<(-1e+66))
              // DbgLv(1) << "XpDa:irv: ii jj" << ii << jj << "valout" <<
              // valout
...
US_XpnData::build_internals ()
{
  ...
  for (int ii = 0; ii < sdknt; ii++)
    { // Get cell/channel, triple, lambda lists
      ...
      QString darec
          = tnode + "." + QString ().sprintf ("%05i.%05i", stage, scnnbr);
...
US_XpnData::rebuild_internals ()
{
  ...
  for (int ii = 0; ii < sdknt; ii++)
    { // Get cell/channel, triple, lambda lists
      ...
      QString darec
          = tnode + "." + QString ().sprintf ("%05i.%05i", stage, scnnbr);
      DbgLv (1) << "XpDa:b_i: ii" << ii << "scnnbr" << scnnbr << "darec"
                << darec;
...
int
US_XpnData::column_indexes (const QString tabname, QStringList &cnames,
                            QList<int> &cxs)
{
  ...
  if (tabname == "AbsorbanceScanData")
    {
      flds << "DataId" << "RunId" << "ExperimentStart" << "ExperimentTime"
           << "Temperature" << "RPM" << "OmegaSquaredT" << "StageNum"
           << "ScanSeqNum" << "SampleName" << "ScanTypeFlag"
           << "ModulePosition"
    ...
    {
      flds << "ExperimentId" << "FugeRunProfileId" << "CellCount"
           << "Researcher"
  ...
  if (ncols != nflds)
    {
      qDebug () << "*WARNING:  for" << tabname
                << "field,column counts:" << nflds << ncols;
    }
...
void
US_XpnData::update_ATable (QSqlQuery &sqry, QList<int> &cxs)
{
...
void
US_XpnData::update_FTable (QSqlQuery &sqry, QList<int> &cxs)
{
...
void
US_XpnData::update_ITable (QSqlQuery &sqry, QList<int> &cxs)
{
...
void
US_XpnData::update_WTable (QSqlQuery &sqry, QList<int> &cxs)
{
