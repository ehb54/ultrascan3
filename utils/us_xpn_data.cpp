...
bool US_XpnData::connect_data(const QString adbname, const QString xpnhost, const int xpnport) {
   return connect_data(xpnhost, xpnport, adbname);
}

// Connect to a selected database server with XPN data
bool US_XpnData::connect_data(
   const QString xpnhost, const int xpnport, const QString adbname, const QString adbuser, const QString adbpasw) {
   bool status = true;
   ...
   if (dbxpn.open()) {
      ...
      qDebug() << "Open Failure for db" << dbname << dbhost << dbport << dbxpn.lastError().text();
      return false;
   }
   ...
   if (
      ...
      && (sqtabs.contains("\"AUC_schema\".\"ExperimentRun\"") || sqtabs.contains("AUC_schema.ExperimentRun"))) {
      status = true;
      DbgLv(1) << "XpDa:cnc: QPSQL-in-qdrvrs && ExpRun-in-sqtabs";
   }
   else {
      DbgLv(1) << "XpDa:cnc: qdrvrs|sqtab content error:" << (qdrvrs.contains("QPSQL"))
...
int US_XpnData::checkExpStatus(QString runid) {
   ...
   QString qrytext = "SELECT \"RunStatus\" from " + qrytab + " WHERE \"RunId\"=" + runid + ";";
...
int US_XpnData::checkExpStatus_auto(QString runid, bool &o_conn) {
   ...
   if (!dbxpn.open()) {
      o_conn = false;
      qDebug() << "XPN: checkExpStatus_auto: !dbxpn.open() !!! runid, o_conn -- " << runid << o_conn;
...
int US_XpnData::update_isysrec(const int runId) {
   ...
   qrytext = "SELECT * from " + qrytab + " WHERE \"RunId\"=" + sRunId + " ORDER BY \"DataId\" DESC LIMIT 1;";
   /*
...
*/
   ...
   DbgLv(1) << "XpDa:ussx:  lastrpm" << lastrpm << "time temp rpm" << isyrec.exptime << isyrec.tempera << isyrec.speed;

   qDebug() << "XpDa:ussx:  lastrpm" << lastrpm << "time temp rpm" << isyrec.exptime << isyrec.tempera << isyrec.speed;
   return lastrpm;
}
...
int US_XpnData::get_runid(QString expid) {
   ...
   QString qrytext = "SELECT * from " + qrytab + " WHERE \"ExperimentId\"=" + expid + ";";
   sqry = dbxpn.exec(qrytext);
...
int US_XpnData::scan_runs(QStringList &runInfo) {
   ...
   QString stat_text = tr("Scanning %1 rows from %2 table...").arg(rows).arg(tabname);
   ...
   int row = 0;
   DbgLv(1) << "XpDa:scn: tabname" << tabname << "rows" << rows << "cols" << cols << "qtxt" << qtxt;
   ...
   while (sqry.next()) { // Loop to pick up values from rows of the table
      ...
      if (!(exprow.abscnf || exprow.flscnf || exprow.inscnf || exprow.wlscnf)) {
         DbgLv(1) << "XpDa: NO AFIW:  run exp" << exprow.runId << exprow.expId << "expname resname" << exprow.expname
                  << exprow.resname;
         continue;
      }
      ...
      QString inforow = delim + QString::number(exprow.runId) + delim + QString::number(exprow.expId) + delim + "0"
                        + delim + exprow.expstart.toString(Qt::ISODate).replace("T", " ") + delim + exprow.expname
                        + delim + exprow.resname + delim + QString(exprow.abscnf ? "1" : "0") + delim
                        + QString(exprow.flscnf ? "1" : "0") + delim + QString(exprow.inscnf ? "1" : "0") + delim
                        + QString(exprow.wlscnf ? "1" : "0") + delim + QString::number(exprow.runstat);
      runInfo << inforow;
...
int US_XpnData::scan_runs_auto(QStringList &runInfo, QString &RunID_to_retrieve) {
   ...
   QString qtxt = QString("SELECT * from ") + qrytab
                  + QString(" WHERE \"RunId\" = %1").arg(RunID); //ALEXEY add WHERE ExperimentID=xxx for us_comproject
   const QString delim("^");
   runInfo.clear();
   runInfo.reserve(rows);
...
int US_XpnData::filter_runs(QStringList &runInfo) {
   ...
   for (int ii = 0; ii < kruns; ii++) {
      ...
      if (exprow.abscnf) { // Count AbsorbanceScanData
         ...
         QString qrytab = "\"" + schname + "\".\"" + tabname + "\"";
         QString qrytext = "SELECT count(*) from " + qrytab + " WHERE \"RunId\"=" + sRunId + ";";
         sqry = dbxpn.exec(qrytext);
         sqry.next();
      ...
      if (kdat > 0) { // Recreate entry where data is non-zero
         QString inforow = delim + QString::number(exprow.runId) + delim + QString::number(exprow.expId) + delim
                           + QString::number(kdat) + delim + exprow.expstart.toString(Qt::ISODate).replace("T", " ")
                           + delim + exprow.expname + delim + exprow.resname + delim
                           + QString(exprow.abscnf ? "1" : "0") + delim + QString(exprow.flscnf ? "1" : "0") + delim
                           + QString(exprow.inscnf ? "1" : "0") + delim + QString(exprow.wlscnf ? "1" : "0");
         runInfo << inforow;
         tExprun << exprow;
         nruns++;
...
bool US_XpnData::import_data(const int runId, const int scanMask) {
   ...
   DbgLv(1) << "XpDa:i_d: arows frows irows wrows srows crows" << arows << frows << irows << wrows << srows << crows;
...
bool US_XpnData::import_data_auto(const int runId, const int scanMask, bool &o_conn) {
   ...
   if (!dbxpn.open()) {
      o_conn = false;
      qDebug() << "XPN: import_data_auto: !dbxpn.open() !!! runId, scanMask, o_conn -- " << runId << scanMask << o_conn;
      return false;
   }
   ...
   if (ascnf) { // Scan and build data for Absorbance Scan Data
      ...
      qDebug() << "[TIME] of scan_xpndata_auto( runId, 'A', arows_bool ) " << int(timer_arows.elapsed() / 1000)
               << " sec";
   }
   ...
   if (fscnf) { // Scan and build data for Fluorescence Scan Data
      ...
      qDebug() << "[TIME] of scan_xpndata_auto( runId, 'F', frows_bool ) " << int(timer_frows.elapsed() / 1000)
               << " sec";
   }
   ...
   if (iscnf) { // Scan and build data for Interference Scan Data
      ...
      qDebug() << "[TIME] of scan_xpndata_auto( runId, 'I', irows_bool ) " << int(timer_irows.elapsed() / 1000)
               << " sec";
   }
   ...
   if (wscnf) { // Scan and build data for Wavelength Scan Data
      ...
      qDebug() << "[TIME] of scan_xpndata_auto( runId, 'W', wrows_bool ) " << int(timer_wrows.elapsed() / 1000)
               << " sec";
   }
   ...
   qDebug() << "XpDa:i_d: arows frows irows wrows srows crows" << arows << frows << irows << wrows << srows << crows;

   qDebug() << "XpDa:i_d: [BOOLS] arows_bool frows_bool irows_bool wrows_bool srows_bool crows_bool" << arows_bool
            << frows_bool << irows_bool << wrows_bool << srows_bool << crows_bool;
...
bool US_XpnData::reimport_data(const int runId, const int scanMask) {
   ...
   if (!dbxpn.open()) {
      qDebug() << "XPN: reimport_data: !dbxpn.open() !!! runId, scanMask -- " << runId << scanMask;
      return false;
   }
   ...
   DbgLv(1) << "XpDa: rei_dat: arows frows irows wrows" << arows << frows << irows << wrows << "status" << status;
   qDebug() << "XpDa: rei_dat: arows frows irows wrows" << arows << frows << irows << wrows << "status" << status;
   //*DEBUG*
   //status=true;
   //*DEBUG*
...
bool US_XpnData::reimport_data_auto(const int runId, const int scanMask, bool &o_conn) {
   ...
   if (!dbxpn.open()) {
      o_conn = false;
      qDebug() << "XPN: reimport_data_auto: !dbxpn.open() !!! runId, scanMask, o_conn -- " << runId << scanMask
               << o_conn;
...
int US_XpnData::scan_xpndata(const int runId, const QChar scantype) {
   ...
   emit status_text(tr(" Scanning %1 rows of %2 table...").arg(count).arg(tabname));

   // Get columns and determine indecies of fields
   qrytext = "SELECT * from " + qrytab + " WHERE \"RunId\"=" + sRunId + " ORDER BY \"DataId\" ;";
   if (scantype == 'C')
      qrytext = "SELECT * from " + qrytab + ";";

   int cols = column_indexes(tabname, cnames, cxs);
   DbgLv(1) << "XpDa:s_x:  cols" << cols << "cnames" << cnames[ 0 ] << "..." << cnames[ cols - 1 ] << "tabname"
            << tabname;
   ...
   DbgLv(1) << "XpDa:s_x:  isctyp scantype" << isctyp << scantype << "qrytext" << qrytext;

   // Loop to read data and store in internal array

   while (sqry.next()) {
      rows++;
      emit status_text(tr("Of %1 ScanData(%2) rows, queried row %3").arg(count).arg(scantype).arg(rows));
      ...
      switch (isctyp) {
         case 1: {
            ...
            if (rows < 21 || (rows + 21) > count) {
               ...
               DbgLv(1) << "XpDa:scn:    row" << rows << "run" << asdrow.runId << "dat" << asdrow.dataId << "count"
                        << asdrow.count;
               DbgLv(1) << "XpDa:scn:     rads0 rads1 vals0 vals1" << asdrow.rads[ 0 ] << asdrow.rads[ 1 ]
                        << asdrow.vals[ 0 ] << asdrow.vals[ 1 ];
               DbgLv(1) << "XpDa:scn:      etim scn temp speed omg estr" << asdrow.exptime << asdrow.scanSeqN
                        << asdrow.tempera << asdrow.speed << asdrow.omgSqT << asdrow.expstart.toString();
            }
            //*DEBUG*
         ...
         case 2: {
            ...
            if (rows < 9 || (rows + 9) > count) {
               ...
               DbgLv(1) << "XpDa:scn:    row" << rows << "run" << fsdrow.runId << "dat" << fsdrow.dataId;
         ...
         case 3: {
            ...
            if (rows < 9 || (rows + 9) > count) {
               ...
               DbgLv(1) << "XpDa:scn:    row" << rows << "run" << isdrow.runId << "dat" << isdrow.dataId;
               DbgLv(1) << "XpDa:scn:     rads0 rads1 vals0 vals1" << isdrow.rads[ 0 ] << isdrow.rads[ 1 ]
                        << isdrow.vals[ 0 ] << isdrow.vals[ 1 ];
            }
            //*DEBUG*
            break;
         }
         case 4: {
            ...
            if (rows < 9 || (rows + 9) > count) {
               ...
               DbgLv(1) << "XpDa:scn:    row" << rows << "run dat count" << wsdrow.runId << wsdrow.dataId
                        << wsdrow.count;
            }
            //*DEBUG*
            break;
         ...
         case 5: {
            ...
            //*DEBUG*
            if (rows < 21 || (rows + 21) > count) {
               DbgLv(1) << "XpDa:scn:    row" << rows << "run dat" << sydrow.runId << sydrow.dataId << "time"
                        << sydrow.exptime << "temp" << sydrow.tempera << "speed" << sydrow.speed << "omg2t"
                        << sydrow.omgSqT;
            }
            //*DEBUG*
            break;
         ...
         case 6: {
            ...
            //*DEBUG*
            if (rows < 21 || (rows + 21) > count) {
               DbgLv(1) << "XpDa:scn:    row" << rows << "fugeRunId" << crprow.frunId << "ssInterval" << crprow.sstintv;
            }
            //*DEBUG*
            break;
         }
   ...
   if (scantype == 'C') { // Show mapping of RunId to ExperimentId to FugeRunProfileId
      ...
      while (sqry.next()) { // Build a list of ExperimentDefinition expIds and fugeIds
         int dExpId = sqry.value(jexpid).toInt();
         int dFugId = sqry.value(jfruid).toInt();
         dExpIds << dExpId;
         dFugIds << dFugId;
         DbgLv(1) << "XpDa:scn:  dExpId" << dExpId << "dFugId" << dFugId << "Name" << sqry.value(jexpnm).toString();
      }
      ...
      DbgLv(1) << "XpDa:scn:    RunId" << iRunId << "ExperimentId" << iExpId << "FugeRunProfileId" << iFugId;

      for (int ii = 0; ii < tCrprof.count(); ii++) { // Look for matching fuge Id in CentrifugeRunProfile values
         sstintl = tCrprof[ ii ].sstintv;

         if (tCrprof[ ii ].frunId == iFugId) { // Match found:  save corresponding SystemStatusInterval
            sstintv = sstintl;
            break;
         }
      }
   ...
   if (scantype == 'S') { // Determine experiment time offset (time at last rms=0 point)
      int ntssda = tSydata.count();
      fnzstx = 0;

      for (int ii = 0; ii < ntssda; ii++) { // Find the index to the first non-zero speed
      ...
      fnzstx = (fnzstx > 0) ? fnzstx
                            : 1; //ALEXEY: if tSydata.count() == 1 (so only tSydata[0] exists), AND fnzstx was 0, THEN
      ...
      //*DEBUG*
      DbgLv(1) << "XpDa:scn: fnzstx" << fnzstx << "etimoff" << etimoff << "  etime1 timeintv" << etime1 << timeintv
               << sstintv;
...
int US_XpnData::scan_xpndata_auto(const int runId, const QChar scantype, bool &o_conn, QElapsedTimer elapse_t) {
   ...
   qDebug() << "in [scan_xpndata_auto()]: scantype, qrytext -- " << scantype << qrytext;
   ...
   count = sqry.value(0).toInt();
   DbgLv(1) << "XpDa:s_x: sRunId" << sRunId << "count" << count;
   qDebug() << "XpDa:s_x: sRunId" << sRunId << "scan_xpndata_query_success, count: " << scan_xpndata_query_success
            << count;

   qDebug() << "[TIME 4] of scan_xpndata_auto(): " << int(elapse_t.elapsed() / 1000) << " sec";

   if (!scan_xpndata_query_success || !sqry.isActive()) {
      qDebug() << "in [scan_xpndata()], scan_xpndata_query_success, NO CONNECTION? " << scan_xpndata_query_success
               << !scan_xpndata_query_success;
   ...
   qDebug() << "XpDa:s_x:  cols" << cols << "cnames" << cnames[ 0 ] << "..." << cnames[ cols - 1 ] << "tabname"
            << tabname;
   ...
   qDebug() << "XpDa:s_x:  isctyp scantype" << isctyp << scantype << "qrytext" << qrytext;
      ...
      switch (isctyp) {
         ...
         case 5: {
            ...
            if (rows < 21 || (rows + 21) > count) {
               ...
               qDebug() << "XpDa:scn:    row" << rows << "run dat" << sydrow.runId << sydrow.dataId << "time"
                        << sydrow.exptime << "temp" << sydrow.tempera << "speed" << sydrow.speed << "omg2t"
                        << sydrow.omgSqT;
            }
            //*DEBUG*
         ...
         case 6: {
            ...
            if (rows < 21 || (rows + 21) > count) {
               DbgLv(1) << "XpDa:scn:    row" << rows << "fugeRunId" << crprow.frunId << "ssInterval" << crprow.sstintv;
               qDebug() << "XpDa:scn:    row" << rows << "fugeRunId" << crprow.frunId << "ssInterval" << crprow.sstintv;
            }
            //*DEBUG*
            break;
         }
...
int US_XpnData::update_xpndata(const int runId, const QChar scantype) {
   ...
   qrytext = "SELECT count(*) from " + qrytab + " WHERE \"RunId\"=" + sRunId + " AND \"ExperimentTime\">=" + sExpTm
             + ";";
   sqry = dbxpn.exec(qrytext);
   sqry.next();
   int narows = sqry.value(0).toInt();
   DbgLv(1) << "XpDa:updx:  qrytext" << qrytext;
   DbgLv(1) << "XpDa:updx:  norows" << norows << "nnrows" << nnrows << "narows" << narows;
   ...
   if (nnrows > 0) { // There are new rows, so update data tables
      qrytext = "SELECT * from " + qrytab + " WHERE \"RunId\"=" + sRunId + " AND \"ExperimentTime\">=" + sExpTm + ";";
      sqry = dbxpn.exec(qrytext);
      int cols = column_indexes(tabname, cnames, cxs);
...
void US_XpnData::load_auc(QVector<US_DataIO::RawData> &allData, const QStringList ifpaths) {
   QStringList chans;
   sctype = -1;
   is_absorb = false;
   is_raw = false;
   ...
   QString stat_text = tr("%1 triples loaded from %2 ...").arg(ntriple).arg(runID);
   emit status_text(stat_text);
}
...
int US_XpnData::build_rawData(QVector<US_DataIO::RawData> &allData) {
   ...
   nstgn = stgnbrs.count();
   ntscan = 0;
   DbgLv(1) << "BldRawD szs: ccd" << ccdescs.size() << "wvs" << wavelns.size() << "nlambda" << nlambda << "nscnn"
            << nscnn;
   ...
   for (int trx = 0; trx < ntriple; trx++) {
      ...
      DbgLv(1) << "BldRawD        channel" << rdata.channel << "str" << str_ch << "bar" << bar_ch << "chp" << chp_ch
               << "chr" << chr_ch;
      ...
      DbgLv(1) << "BldRawD         trip" << trnode << "descr" << rdata.description << "celchn" << celchn;

      qDebug() << "BUILD_RAW_DATA trip: descr:  " << trnode << "descr" << rdata.description << "celchn" << celchn;
      ...
      for (int sgx = 0; sgx < nstgn; sgx++) { // Set scan values
         ...
         for (int scx = 0; scx < nscnn; scx++) { // Set scan values
            ...
            if (scx < 3 || (scx + 4) > nscnn)
               DbgLv(1) << "BldRawD       trx wvx scx scnnbr" << trx << wvx << scx << scnnbr << "wavl"
                        << scan.wavelength;

            //*DEBUG*
            if (trx == 0) {
               DbgLv(1) << "BldRawD      scx" << scx << "trx" << trx << "seconds" << scan.seconds << "rpm" << scan.rpm;
            }
            //*DEBUG*
            ...
            if (scx < 3 || (scx + 4) > nscnn)
               DbgLv(1) << "BldRawD        scx" << scx << "rvalues size" << scan.rvalues.size() << "rvalues[mid]"
      ...
      DbgLv(1) << "BldRawD trx" << trx << "TIMEMS:scan loop time" << time10.msecsTo(time20);
      ...
      QString stat_text = tr("Of %1 raw AUCs, built %2").arg(ntriple).arg(trx + 1);
      emit status_text(stat_text);
   ...
   DbgLv(1) << "BldRawD  DONE ntriple" << ntriple << "mxscnn" << mxscnn << "mnscnn" << mnscnn;
   return ntriple;
}
...
int US_XpnData::rebuild_rawData(QVector<US_DataIO::RawData> &allData) {
   ...
   int stgnbr = 0;
   nstgn = stgnbrs.count();
   DbgLv(1) << "rBldRawD ntriple nstgn nscnn" << ntriple << nstgn << nscnn << "st0 stn" << stgnbrs[ 0 ]
            << stgnbrs[ nstgn - 1 ] << "sc0 scn" << scnnbrs[ 0 ] << scnnbrs[ nscnn - 1 ];
   ...
   for (int trx = 0; trx < ntriple; trx++) { // Update scans for each triple
      ...
      for (int sgx = 0; sgx < nstgn; sgx++) { // Set stage values
         ...
         { // Set scan values
            ...
            ndscan++;
            DbgLv(1) << "rBldRawD      sqx" << sgx << "scx" << scx << "ndscan oscknt" << ndscan << oscknt;
            if (ndscan < oscknt) {
               time20 = QDateTime::currentDateTime();
               timi3 += time10.msecsTo(time20);
            }
            if (ndscan < oscknt)
               continue; // Skip old AUC scan
            ...
            if (scx < 3 || (scx + 4) > nscnn)
               DbgLv(1) << "rBldRawD       trx wvx scx scnnbr" << trx << wvx << scx << scnnbr << "wavl"
                        << scan.wavelength;

            //*DEBUG*
            if (trx == 0) {
               DbgLv(1) << "rBldRawD      scx" << scx << "trx" << trx << "seconds" << scan.seconds << "rpm" << scan.rpm;
            }
            //*DEBUG*
            time20 = QDateTime::currentDateTime();
            timi3 += time10.msecsTo(time20);
            ...
            if (scx < 3 || (scx + 4) > nscnn)
               DbgLv(1) << "rBldRawD        scx" << scx << "rvalues size" << scan.rvalues.size() << "rvalues[mid]"
                        << scan.rvalues[ scan.rvalues.size() / 2 ];

            //if ( ( scx + 1 ) == oscknt )
            if (ndscan == oscknt) { // If last of old scans, update in case readings were added
               ...
               qDebug() << "INSIDE xpn_data: AFTER get_readings() 2a: ksc = sgx * nscnn + scx: " << sgx << " * "
                        << nscnn << " + " << scx;
      ...
      QString stat_text = tr("Of %1 raw AUCs, updated %2").arg(ntriple).arg(trx + 1);
      emit status_text(stat_text);
   ...
   DbgLv(1) << "rBldRawD  DONE ntriple" << ntriple << "mxscnn" << mxscnn << "mnscnn" << mnscnn;
   timi9 += time00.msecsTo(time20);
   DbgLv(1) << "rBldRawD   timi1" << timi1 << "timi2" << timi2 << "timi3" << timi3 << "timi4" << timi4 << "timi5"
            << timi5 << "timi6" << timi6 << "   timi9" << timi9 << "  timi8 timi7" << timi8 << timi7;
   return ntriple;
}
...
int US_XpnData::export_auc(QVector<US_DataIO::RawData> &allData) {
   ...
   DbgLv(1) << "expA: ntrips ftype fbase" << ntrips << ftype << fbase << "cur_dir" << cur_dir;
   QDir dir;
   ...
   for (int ii = 0; ii < ntrips; ii++) { // Create a file for each triple
      ...
      DbgLv(1) << "expA:   nf" << nfiles << "fname" << fname << "kscans" << kscans << "mxscans" << mxscans << "iiuse"
               << iiuse;
   }
   ...
   DbgLv(1) << "expA:   ntimes ntssda" << ntimes << ntssda << "e_utime e_stime" << e_utime << e_stime;
   ...
   DbgLv(1) << "expA:DLT:SCN: npoint" << nscnp << "x1,y1,x2,y2" << xa_scn[ 0 ] << ya_scn[ 0 ] << xa_scn[ nscnp - 1 ]
            << ya_scn[ nscnp - 1 ];
   DbgLv(1) << "expA:DLT:TMS: npoint" << ntmsp << "x1,y1,x2,y2" << xa_tms[ 0 ] << ya_tms[ 0 ] << xa_tms[ ntmsp - 1 ]
            << ya_tms[ ntmsp - 1 ];
   ...
   DbgLv(1) << "expA:DLT:SCN: npoint" << nscnp << "x1,y1,x2,y2" << ya_scn[ 0 ] << xa_scn[ 0 ] << ya_scn[ nscnp - 1 ]
            << xa_scn[ nscnp - 1 ];
   DbgLv(1) << "expA:DLT:TMS: npoint" << ntmsp << "x1,y1,x2,y2" << ya_tms[ 0 ] << xa_tms[ 0 ] << ya_tms[ ntmsp - 1 ]
            << xa_tms[ ntmsp - 1 ];
   DbgLv(1) << "expA:DLT:SCN:  slope" << slope_s << "intercept" << icept_s;
   DbgLv(1) << "expA:DLT:TMS:  slope" << slope_t << "intercept" << icept_t;
   DbgLv(1) << "expA:DLT:SCN:   stddev" << sigma_s << "correlation" << corre_s;
   ...
   if (ntssda < ntimes) {
      ...
      for (int ii = 0; ii < ntimes; ii++) { // Create a record for each scan
         ...
         DbgLv(1) << "expA:   ii" << ii << "scan" << scannbr << "stage" << istagen << "speed" << rawSpeed << isSpeed
                  << "time" << time;
      ...
      for (int ii = ftx; ii < ntimes; ii++) { // Create a record for each system status row
         ...
         if (time_e > ietime || ((speed_n < 100) && (ii > (ftx + 10)))) { // Time well beyond last scan or speed dropping back down to zero
            DbgLv(1) << "expA:   ii" << ii << "ftx" << ftx << "stage" << stage << "time_n" << time_n << "speed_n"
                     << speed_n << "omg2t_n" << omg2t_n << "*BREAK*";
            break;
         }
         ...
         DbgLv(1) << "expA:   ii" << ii << "stage" << stage << "time_n" << time_n << "speed_n" << speed_n << "omg2t_n"
                  << omg2t_n;

         for (int jj = 0; jj < timeinc; jj++) { // Expand values for each second in present range
            ...
            DbgLv(1) << "expA:      jj" << jj << "scan" << scannbr << "time_c" << time_c << "speed" << rawSpeed
                     << isSpeed << "omg2t(rec)" << omega2t << "omg2t(sum)" << omg2t_sm;
            omega2t = omg2t_sm;
...
void US_XpnData::set_scan_data(const int datx, const QString arType) {
   ...
   if (datx < 5)
      DbgLv(1) << "XpDa:ssd: datx" << datx << "csd: dId rId scan" << csdrec.dataId << csdrec.runId << csdrec.scanSeqN
               << "rType" << rType;
}
...
#if 1
// Return a scan data index within the ScanData vector
int US_XpnData::scan_data_index(const QString trnode, const int mstage, const int mscann) {
   static int pdatx = 0;
   QDateTime time10;
   QDateTime time20;
   QDateTime time30;
   ...
   if (runType == "RI") {
      ...
      for (int ii = 0; ii < sdknt; ii++) {
         ...
         if (
            (asdrec->scanSeqN == mscann) && (asdrec->stageNum == mstage) && (asdrec->wavelen == mwavl)
            && (asdrec->radPath == mchan) && (asdrec->cellPos == mcell)) {
            datx = jj;
            //time40=QDateTime::currentDateTime();
            //timi2+=time30.msecsTo(time40);
            break;
      ...
      for (int ii = 0; ii < sdknt; ii++) {
         ...
         if (
            (fsdrec->scanSeqN == mscann) && (fsdrec->stageNum == mstage) && (fsdrec->wavelen == mwavl)
            && (fsdrec->radPath == mchan) && (fsdrec->cellPos == mcell)) {
            datx = jj;
            break;
         }
         //time40=QDateTime::currentDateTime();
      ...
      for (int ii = 0; ii < sdknt; ii++) {
         ...
         if (
            (isdrec->scanSeqN == mscann) && (isdrec->stageNum == mstage) && (isdrec->wavelen == mwavl) && (mchan == "A")
      ...
      for (int ii = 0; ii < sdknt; ii++) {
         ...
         if (
            (wsdrec->scanSeqN == mscann) && (wsdrec->stageNum == mstage) && (wsdrec->scanPos == mwavl)
            && (wsdrec->radPath == mchan) && (wsdrec->cellPos == mcell)) {
            datx = jj;
            break;
         }
         //time40=QDateTime::currentDateTime();
   ...
   timi3 += time10.msecsTo(time20);
   timi9 += time00.msecsTo(time20);
   DbgLv(1) << "XpDa:scdX: trnode mstage mscann" << trnode << mstage << mscann << "datx" << datx << "ti's" << timi1
            << timi2 << timi3 << timi9 << "sdknt" << sdknt << "ntry" << ntry;
...
int US_XpnData::get_readings(QVector<double> &rvalues, const int trx, const int sgx, const int scx) {
   QString trnode = trnodes[ trx ];
   int stgnbr = stgnbrs[ sgx ];
   int scnnbr = scnnbrs[ scx ];
   DbgLv(1) << "XpDa:grd: trx sgx scx" << trx << sgx << scx << "trnode stgnbr scannbr" << trnode << stgnbr << scnnbr;
...
void US_XpnData::interp_rvalues(
   QVector<double> &rads, QVector<double> &vals, QVector<double> &rrads, QVector<double> &rvals) {
   int jjs = 1;
...
QString US_XpnData::runDetails(void) {
   ...
   DbgLv(1) << "XpDa:Det: sdknt ssknt" << sdknt << ssknt << "  e_dtime e_stime" << e_dtime << e_stime;
   if (e_stime < e_dtime) { // For System Status last time less than last data time, rescan SysStat
      tSydata.clear();
   ...
   QString tabname("SystemStatusData");
   msg += tr("\n  Table Name:  '") + tabname + "'  ( runID =" + QString::number(runID) + " )\n";
   msg += tr("    Count of Data Records:    %1\n").arg(ssknt);
...
void US_XpnData::build_internals() {
   ...
   for (int ii = 0; ii < sdknt; ii++) { // Get cell/channel, triple, lambda lists
      set_scan_data(ii);
      ...
      DbgLv(1) << "XpDa:b_i: ii" << ii << "schan cechn" << schan << cechn << "darec" << darec << "rad0 rad1"
               << csdrec.rads->at(0) << csdrec.rads->at(1);
      ...
      if (vcount != rknt || vcount != vknt) {
         qDebug() << "*** ii vcount rknt vknt tnode" << ii << vcount << rknt << vknt << tnode;
      }
   ...
   for (int ii = 0; ii < triples.count(); ii++) {
      ...
      QString cechn = scell + " / " + schan;
      DbgLv(1) << "XpDa:b_i:trip: ii" << ii << "icell ichan schan cechn" << icell << ichan << schan << cechn << "tripl"
               << tripl;
      ...
      if (!cellchans.contains(cechn)) {
         ...
         for (int jj = 0; jj < sdknt; jj++) { // Use Samples data to get cell/channel description
            set_scan_data(jj);
   ...
   DbgLv(1) << "XpDa:b_i:       kdarec" << datrecs.count() << "da0 dan" << datrecs[ 0 ]
            << datrecs[ datrecs.count() - 1 ];
   DbgLv(1) << "XpDa:b_i:       kstgn " << stgnbrs.count() << "stg0 stgn" << stgnbrs[ 0 ]
            << stgnbrs[ stgnbrs.count() - 1 ];
   DbgLv(1) << "XpDa:b_i:       kscnn " << scnnbrs.count() << "scn0 scnn" << scnnbrs[ 0 ]
            << scnnbrs[ scnnbrs.count() - 1 ];
   qSort(datrecs);
   qSort(stgnbrs);
   qSort(scnnbrs);
   DbgLv(1) << "XpDa:b_i:       kdarec" << datrecs.count() << "da0 dan" << datrecs[ 0 ]
   ...
   DbgLv(1) << "XpDa:b_i:       kscnn " << scnnbrs.count() << "scn0 scnn" << scnnbrs[ 0 ]
            << scnnbrs[ scnnbrs.count() - 1 ];
   DbgLv(1) << "XpDa:b_i:       rad0 radn" << a_radii[ 0 ] << a_radii[ rkntl - 1 ] << " nrad" << rkntl
            << a_radii.count();
...
void US_XpnData::rebuild_internals() {
   ...
   DbgLv(1) << "XpDa:b_i:       kdarec" << datrecs.count() << "da0 dan" << datrecs[ 0 ]
            << datrecs[ datrecs.count() - 1 ];
   DbgLv(1) << "XpDa:b_i:       kstgn " << stgnbrs.count() << "scn0 scnn" << scnnbrs[ 0 ]
            << scnnbrs[ scnnbrs.count() - 1 ];
}
...
int US_XpnData::column_indexes(const QString tabname, QStringList &cnames, QList<int> &cxs) {
   ...
   if (ncols != nflds) {
      qDebug() << "*WARNING:  for" << tabname << "field,column counts:" << nflds << ncols;
   }
   ...
   DbgLv(1) << "XpDa:cox: ncols" << ncols << "nflds" << nflds << "tabname" << tabname;
...
void US_XpnData::update_ATable(QSqlQuery &sqry, QList<int> &cxs) {
   ...
   for (int ii = strow; ii > enrow; ii--) { // Look from end for any dataId match in already captured rows
   ...
   if (asdrow.radPath == " " || asdrow.radPath.isEmpty()) { // Store both an 'A' and 'B' channel record
      asdrow.radPath = "A";
      if (mdx1 < 0)
...
void US_XpnData::update_FTable(QSqlQuery &sqry, QList<int> &cxs) {
   ...
   if (fsdrow.radPath == " " || fsdrow.radPath.isEmpty()) { // Store both an 'A' and 'B' channel record
      fsdrow.radPath = "A";
      if (mdx1 < 0)
...
void US_XpnData::update_WTable(QSqlQuery &sqry, QList<int> &cxs) {
   ...
   if (wsdrow.radPath == " " || wsdrow.radPath.isEmpty()) { // Store both an 'A' and 'B' channel record
      wsdrow.radPath = "A";
      if (mdx1 < 0)
