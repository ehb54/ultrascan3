...
int US_ProtocolUtil::list_all(QList<QStringList> &protdata, US_DB2 *dbP) {
   ...
   if (dbP != NULL) { // Read protocol records from the database
      ...
      while (dbP->next()) {
         ...
         QString pdate = US_Util::toUTCDatetimeText(date.toString(Qt::ISODate), true).section(":", 0, 1) + " UTC";
      ...
      for (int ii = 0; ii < nfiles; ii++) { // Examine each "R000*.xml" file in the directory
         ...
         //                                  .toString( Qt::ISODate ), true )
         //                                  .section( " ", 0, 0 ).simplified();
         QString fdate = US_Util::toUTCDatetimeText(QFileInfo(pfpath).lastModified().toUTC().toString(Qt::ISODate), true)
                            .section(":", 0, 1)
                         + " UTC";
         QFile pfile(pfpath);
...
int US_ProtocolUtil::update_list(const QStringList protentry, QList<QStringList> &protdata) {
   int nrecs = protdata.count(); // Count of entries in list
   int newpx = nrecs; // Default index of added entry

   for (int ii = 0; ii < nrecs; ii++) {
      if (protentry[ 0 ] == protdata[ ii ][ 0 ]) { // *ERROR* A name match already exists!
...
int US_ProtocolUtil::write_record(const QString xml, US_DB2 *dbP) {
   ...
   if (dbP != NULL) { // Write a record to the database
      ...
      qry << personID << protguid << protname << xml << ohost << rotorID << speed1 << QString::number(duration)
          << QString::number(nucells) << QString::number(escans) << solut1 << solut2 << QString::number(nwvlens);
      ...
      QString datdir = US_Settings::dataDir() + "/projects";
      datdir.replace("\\", "/"); // Possible Windows fix
      QString filename = US_DataFiles::get_filename(datdir, protguid, "R", "protocol", "guid");
      ...
      newpx = QString(filename).section("/", -1, -1).section(".", 0, 0).mid(1).toInt();
   }
...
int US_ProtocolUtil::read_record(const QString protname, QString *xml, QStringList *protentry, US_DB2 *dbP) {
   int idprot = -1;
   ...
   if (dbP != NULL) { // Find the record in the database with a matching name
      ...
      while (dbP->next()) {
         ...
         if (pname == protname) { // Name match:  store information and break out
            ...
            QString pguid = dbP->value(1).toString();
            QDateTime date = dbP->value(5).toDateTime().toUTC();
            QString pdate = US_Util::toUTCDatetimeText(date.toString(Qt::ISODate), true).section(" ", 0, 0).simplified();
      ...
      for (int ii = 0; ii < nfiles; ii++) { // Examine each "R000*.xml" file in the directory
         ...
         if (pname == protname) { // Name match:  store information and break out
            QString fdate
               = US_Util::toUTCDatetimeText(QFileInfo(pfpath).lastModified().toUTC().toString(Qt::ISODate), true)
                    .section(" ", 0, 0)
                    .simplified();
...
int US_ProtocolUtil::read_record_auto(
   const QString protname, int invID_passed, QString *xml, QStringList *protentry, US_DB2 *dbP) {
   int idprot = -1;
...
bool US_ProtocolUtil::delete_record(const QString protid, US_DB2 *dbP) {
   ...
   else { // Delete record from local disk, using name of file with matching GUID
      QString datdir = US_Settings::dataDir() + "/projects/";
      datdir.replace("\\", "/"); // Possible Windows fix
      bool newFile = false;
      QString filename = US_DataFiles::get_filename(datdir, protid, "R", "protocol", "guid", newFile);
