...
int
US_ProtocolUtil::list_all (QList<QStringList> &protdata, US_DB2 *dbP)
{
        ...
        { // Examine each "R000*.xml" file in the directory
          ...
          QString fdate
              = US_Util::toUTCDatetimeText (
                    QFileInfo (pfpath).lastModified ().toUTC ().toString (
                        Qt::ISODate),
                    true)
                    .section (":", 0, 1)
                + " UTC";
...
int
US_ProtocolUtil::list_all_auto (QList<QStringList> &protdata, US_DB2 *dbP)
{
  int nrecs = 0;     // Count of records found
  protdata.clear (); // Initialize list of entries
  ...
  if (dbP != NULL)
    { // Read protocol records from the database
      ...
      qDebug () << "[AUTO] PU:l_all:  qry errno, US_DB2::OK"
                << dbP->lastErrno () << US_DB2::OK;
...
int
US_ProtocolUtil::update_list (const QStringList protentry,
...
int
US_ProtocolUtil::write_record (const QString xml, US_DB2 *dbP)
{
  ...
  if (dbP != NULL)
    { // Write a record to the database
      ...
      qry << personID << protguid << protname << xml << ohost << rotorID
          << speed1 << QString::number (duration) << QString::number (nucells)
          << QString::number (escans) << solut1 << solut2
          << QString::number (nwvlens);

      dbP->query (qry);
      qDebug () << "PU:wrrec:  qry" << qry;
      qDebug () << "PU:wrrec:   stat" << dbP->lastErrno ()
                << dbP->lastError ();
...
int
US_ProtocolUtil::read_record (const QString protname, QString *xml,
        ...
        { // Examine each "R000*.xml" file in the directory
          ...
          if (pname == protname)
            { // Name match:  store information and break out
              ...
              // Return entry, xml, and protocol ID (negative of filename
              // numeric)
              if (protentry != NULL)
              ...
              idprot
                  = -(QString (pfname).section (".", 0, 0).mid (1).toInt ());
              break;
            }
        } // END: file loop
    } // END: local disk
...
int
US_ProtocolUtil::read_record_auto (const QString protname, int invID_passed,
                                   ...
                                   US_DB2 *dbP)
{
  ...
  if (dbP != NULL)
    { // Find the record in the database with a matching name
      ...
      qDebug ()
          << "In US_ProtocolUtil::read_record_auto: DB NOT NULL; query -- "
          << qry;
...
bool
US_ProtocolUtil::delete_record (const QString protid, US_DB2 *dbP)
{
  bool deleted = false;
  ...
  if (dbP != NULL)
    ...
    { // Delete record from local disk, using name of file with matching GUID
      ...
      datdir.replace ("\\", "/"); // Possible Windows fix
      bool newFile = false;
      QString filename = US_DataFiles::get_filename (
          datdir, protid, "R", "protocol", "guid", newFile);
      if (!newFile)
