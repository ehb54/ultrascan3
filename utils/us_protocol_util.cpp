...
int US_ProtocolUtil::list_all(QList<QStringList>& protdata, US_DB2* dbP) {
  int nrecs = 0;     // Count of records found
  protdata.clear();  // Initialize list of entries

  qDebug() << "PU:l_all: dbP" << dbP;
  if (dbP != NULL) {  // Read protocol records from the database
    ...
    qDebug() << "PU:l_all:  qry errno, US_DB2::OK" << dbP->lastErrno()
             << US_DB2::OK;
    ...
    while (dbP->next()) {
      ...
      QString pdate =
          US_Util::toUTCDatetimeText(date.toString(Qt::ISODate), true)
              .section(":", 0, 1) +
          " UTC";
  ...
  else {  // Read protocol records from local disk
    ...
    for (int ii = 0; ii < nfiles;
      ...
      //                                  QFileInfo( pfpath
      //                                  ).lastModified().toUTC() .toString(
      //                                  Qt::ISODate ), true ) .section( " ",
      //                                  0, 0 ).simplified();
      QString fdate =
          US_Util::toUTCDatetimeText(
              QFileInfo(pfpath).lastModified().toUTC().toString(Qt::ISODate),
              true)
              .section(":", 0, 1) +
          " UTC";
      QFile pfile(pfpath);
      ...
      while (!xmli.atEnd()) {  // Parse XML for description and guid
        xmli.readNext();
...
int US_ProtocolUtil::list_all_auto(QList<QStringList>& protdata, US_DB2* dbP) {
  int nrecs = 0;     // Count of records found
  protdata.clear();  // Initialize list of entries
  ...
  if (dbP != NULL) {  // Read protocol records from the database
    ...
    qDebug() << "[AUTO] PU:l_all:  qry errno, US_DB2::OK" << dbP->lastErrno()
             << US_DB2::OK;
...
int US_ProtocolUtil::update_list(const QStringList protentry,
  ...
  for (int ii = 0; ii < nrecs; ii++) {
    if (protentry[0] ==
        protdata[ii][0]) {  // *ERROR* A name match already exists!
...
int US_ProtocolUtil::write_record(const QString xml, US_DB2* dbP) {
  ...
  while (!xmli.atEnd()) {  // Parse XML
    ...
    if (xmli.isStartElement()) {
      ...
      if (ename == "protocol") {
        ...
        nspeeds++;
        double dura = attr.value("duration_minutes").toString().toDouble();
        double delay = attr.value("delay_seconds").toString().toDouble();
        dura *= 60.0;
        duration += (dura + delay);
        int sescans = qRound(dura * 0.1);
        escans += sescans;
        if (nspeeds == 1) speed1 = attr.value("rotorspeed").toString();
      } else if (ename == "cell") {
        nucells++;
  ...
  if (dbP != NULL) {  // Write a record to the database
    QStringList qry("new_protocol");
    qry << personID << protguid << protname << xml << ohost << rotorID << speed1
        << QString::number(duration) << QString::number(nucells)
        << QString::number(escans) << solut1 << solut2
    ...
    if (dbP->lastErrno() != US_DB2::OK) return newpx;

    newpx = dbP->lastInsertID();
    qDebug() << "PU:wrrec:    newpx" << newpx;
  }

  else {  // Write a record to local disk
    QString datdir = US_Settings::dataDir() + "/projects";
    datdir.replace("\\", "/");  // Possible Windows fix
    QString filename =
        US_DataFiles::get_filename(datdir, protguid, "R", "protocol", "guid");
    QFile pfile(filename);
    qDebug() << "PU:wrrec:    filename" << filename;

    if (!pfile.open(QIODevice::WriteOnly | QIODevice::Text)) return newpx;
...
int US_ProtocolUtil::read_record(const QString protname, QString* xml,
  ...
  if (xml != NULL) xml->clear();
  if (protentry != NULL) protentry->clear();

  if (dbP != NULL) {  // Find the record in the database with a matching name
    QStringList qry;
    ...
    while (dbP->next()) {
      ...
      if (pname == protname) {  // Name match:  store information and break out
        QString pdbid = dbP->value(0).toString();
        QString pguid = dbP->value(1).toString();
        QDateTime date = dbP->value(5).toDateTime().toUTC();
        QString pdate =
        ...
        if (protentry != NULL) *protentry << pname << pdate << pdbid << pguid;
        if (xml != NULL) *xml = dbP->value(3).toString();
  ...
  else {  // Find the record in a local file with a matching name
    QString datdir = US_Settings::dataDir() + "/projects/";
    datdir.replace("\\", "/");    // Possible Windows fix
    QStringList rfilt("R*.xml");  // "~/ultrascan/data/projects/R*.xml"
    QStringList pfiles = QDir(datdir).entryList(rfilt, QDir::Files, QDir::Name);
    ...
    for (int ii = 0; ii < nfiles;
      ...
      if (pname == protname) {  // Name match:  store information and break out
        ...
        if (protentry != NULL) *protentry << pname << fdate << pfname << pguid;
        if (xml != NULL) *xml = xmlstr;
...
int US_ProtocolUtil::read_record_auto(const QString protname, int invID_passed,
                                      QString* xml, QStringList* protentry,
                                      US_DB2* dbP) {
  int idprot = -1;
  ...
  if (dbP != NULL) {  // Find the record in the database with a matching name
    ...
    qDebug() << "In US_ProtocolUtil::read_record_auto: DB NOT NULL; query -- "
             << qry;
  ...
  if (idprot < 0) {  // Find the record in a local file with a matching name
...
bool US_ProtocolUtil::delete_record(const QString protid, US_DB2* dbP) {
  ...
  if (dbP != NULL) {  // Delete record from database, using the protocol db id
    QStringList qry("delete_protocol");
    qry << QString::number(US_Settings::us_inv_ID()) << protid;

    dbP->query(qry);

    if (dbP->lastErrno() == US_DB2::OK) deleted = true;
  }

  else {  // Delete record from local disk, using name of file with matching
          // GUID
    QString datdir = US_Settings::dataDir() + "/projects/";
    datdir.replace("\\", "/");  // Possible Windows fix
    bool newFile = false;
    ...
    if (!newFile) {  // File exists, so (attempt to) remove it
