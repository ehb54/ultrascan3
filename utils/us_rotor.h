...
public:
  ...
  enum Status
  {
    ...
    NOT_FOUND,  //!< The rotor, abstractRotor or RotorCalibration ID/GUID was
                //!< not found
    NOT_OPENED, //!< The rotor, abstractRotor or RotorCalibration file could
                //!< not be opened
  ...
  public:
    ...
    QList<Operator>
        operators; //!< A list of people authorized to use the instrument
  };
  ...
  public:
    ...
    QString material;     //!< What this rotor type is made of (Simulation,
                          //!< Titanium, CarbonFiber)
  ...
  //! \brief the RotorCalibration class describes a calibration experiment and
  //! associated data
  ...
  public:
    ...
    QString rotorGUID;           //!< The GUID of the rotor this calibration is
                                 //!< associated with
    int calibrationExperimentID; //!< The ID of the experiment that contains
                                 //!< the calibration data
    QString calibrationExperimentGUID; //!< The GUID of the experiment that
                                       //!< contains the calibration data
    double coeff1; //!< The first order coefficient for the second order
                   //!< polynomial
    double coeff2; //!< The second order coefficient for the second order
                   //!< polynomial the zeroth order coefficient is always zero
                   //!< (no stretch at 0 RPM)
    ...
    /*! \brief    Function to save the current calibration in the DB
        ...
        \param    rotorID The ID of the rotor this calibration is associated
       with
        \param    db For database access, an open database connection
    */
    int saveDB (int, US_DB2 * = 0);

    /*! \brief    Function to read an entire rotor calibration structure from
       the DB

        \param    calibrationID The database rotorCalibrationID of the desired
       calibration
        \param    db For database access, an open database connection
    */
    Status readDB (int, US_DB2 * = 0);

    /*! \brief    Function to delete the specified rotor calibration from the
       DB

        \param    calibrationID The database calibrationID of the rotor
       calibration to delete
        \param    db For database access, an open database connection
    */
    static int deleteCalibrationDB (int, US_DB2 * = 0);

    /*! \brief    Function to find the dummy calibration for the current rotor
       in the DB, and to replace it in all experiments with this one if it
       exists. Most likely it is the single calibration experiment that
       generated the dummy calibration in the first place.

        \param    oldCalibrationID Returns the database ID of the old dummy
       calibration here
        \param    db For database access, an open database connection
    */
    ...
    /*! \brief A function to read information about a rotor calibration from
       disk
  ...
  */
  ...
  /*! \brief A function to read information about all abstract rotors from DB
      ...
      \param    db  For database access, an open database connection
  */
  static Status readAbstractRotorsDB (QVector<US_Rotor::AbstractRotor> &,
                                      US_DB2 *db);
  ...
  */
  ...
  /*! \brief    Function to retrieve all the rotors in a particular lab from
     disk
  ...
  */
  static Status
  readCalibrationProfilesDB (QVector<US_Rotor::RotorCalibration> &, int,
                             US_DB2 *db);
  ...
  */
  static Status
  readCalibrationProfilesDisk (QVector<US_Rotor::RotorCalibration> &, int);
  ...
  /*! \brief    Function to find the filename of a rotor or calibration
      ...
      \param    filename  The function will return the filename here if it is
     found
  */
  static bool diskFilename (const QString &, const QString &, const int &,
                            QString &);

private:
  static bool diskPath (QString &);
  static QString get_filename (const QString &, const QString &,
                               const QString &, const int &, bool &);
