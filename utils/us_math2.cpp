...
double
US_Math2::box_muller (double m, double s)
{
...
double
US_Math2::linefit (double **x, double **y, double *slope, double *intercept,
...
int
US_Math2::nearest_curve_point (double *xs, double *ys, const int npoints,
...
bool
US_Math2::intersect (double &slope1, double &intcp1, double &slope2,
...
bool
US_Math2::intersect (double *x1s, double *y1s, int npoint1, double *x2s,
                     double *y2s, int npoint2, double *xisec, double *yisec)
{
  double slope1;
  double intcp1;
  double slope2;
...
void
US_Math2::calc_vbar (Peptide &pep, const QString &sequence, double temperature)
{
  pep.vbar_sum = 0.0;
  pep.mw = 0.0;
  pep.weight = 0.0;
  ...
  pep.residues = pep.a + pep.b + pep.c + pep.d + pep.e + pep.f + pep.g + pep.h
                 + pep.i + pep.j + pep.k + pep.l + pep.m + pep.n + pep.o
                 + pep.p + pep.q + pep.r + pep.s + pep.t + pep.v + pep.w
                 + pep.x + pep.y + pep.z + pep.dab + pep.dpr;
  ...
  pep.weight
      += pep.dpr * DIAMINOPROPANOIC_ACID_MW * DIAMINOPROPANOIC_ACID_VBAR;
...
void
US_Math2::data_correction (double t, SolutionData &d)
{
  ...
  */
  ...
  d.density_wt = xi_max * (c0 + c1 * t + c2 * t2 + c3 * t3 + c4 * t4 + c5 * t5)
                 / (1.0 + b * t);
  ...
  */
  ...
  double T = (t + K0) / 300.0;
  d.viscosity_wt = 1e-3
                   * (a1 * qPow (T, b1) + a2 * qPow (T, b2) + a3 * qPow (T, b3)
                      + a4 * qPow (T, b4));
#endif
  ...
  d.s20w_correction
      = (d.buoyancyw / d.buoyancyb) * (d.viscosity_tb / VISC_20W);
...
double
US_Math2::normal_distribution (double sigma, double mean, double x)
{
...
double
US_Math2::time_correction (const QVector<US_DataIO::EditedData> &dataList)
{
...
double
US_Math2::time_correction (const QVector<US_DataIO::RawData> &dataList)
{
  int size = dataList[0].scanData.size ();
...
int
US_Math2::nnls (double *a, int a_dim1, int m, int n, double *b, double *x,
  ...
  while (iz1 <= iz2 && nsetp < m)
    {
      ...
      while (true)
        {
          ...
          _nnls_h12 (1, npp1, npp1 + 1, m, &a[ja_dim1], 1, &up, &dummy, 1, 1,
                     0);
          ...
          if ((d2 - unorm) > 0.0)
            {
              ...
              _nnls_h12 (2, npp1, npp1 + 1, m, &a[ja_dim1], 1, &up, zz, 1, 1,
                         1);
      ...
      /* Index j = index[ iz ] has been selected to be moved from set Z to set
         P. Update B and indices, apply householder transformations to cols in
      ...
      if (iz1 <= iz2)
        for (jz = iz1; jz <= iz2; jz++)
          {
            jj = index[jz];
            _nnls_h12 (2, nsetp - 1, npp1, m, &a[ja_dim1], 1, &up,
 ...
 */
void
US_Math2::_nnls_g1 (double a, double b, double *cterm, double *sterm,
                    double *sig)
{
  double d1;
  double xr;
...
void
US_Math2::gaussian_smoothing (QVector<double> &array, int smooth)
{
  if (smooth <= 1)
    return;
...
double
US_Math2::calcCommonVbar (US_Solution &solution, double temperature)
{
  ...
  for (int ii = 0; ii < solution.analyteInfo.size (); ii++)
    {
      ...
      double vbar
          = solution.analyteInfo[ii].analyte.type == US_Analyte::PROTEIN
                ? US_Math2::adjust_vbar20 (vb20, temperature)
                : vb20;
      double wt = solution.analyteInfo[ii].analyte.mw
                  * solution.analyteInfo[ii].amount;
      vbsum += (vbar * wt);
      wtsum += wt;
    }
...
int
US_Math2::best_grid_reps (int &ngrid_s, int &ngrid_k)
{
  ...
  for (int ii = 0; ii < reps.count (); ii++)
    {
      ...
      qDebug () << "BGR:   ii" << ii << "jreps" << jreps << "jgrid_k"
                << jgrid_k;
...
double
US_Math2::norm_value (US_DataIO::RawData *simdat)
{
  double normval = 0.0;
...
double
US_Math2::norm_value (US_DataIO::EditedData *expdat)
{
  double normval = 0.0;
...
double
US_Math2::norm_value (double *datary, const int nval)
{
  double normval = 0.0;
...
double
US_Math2::norm_value (QVector<double> *datvec)
{
...
double
US_Math2::find_root (const double goal)
{
