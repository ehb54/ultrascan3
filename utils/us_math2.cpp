...
class libnnls {
   public:
      typedef int (*nnls_fn)(double *, int, int, int, double *, double *, double *, double *, double *, int *);
...
double US_Math2::linefit(
   ...
   *correlation = (arraysize * sumxy - sumx * sumy)
                  / (sqrt(arraysize * sumx_sq - sumx * sumx) * sqrt(arraysize * sumy_sq - sumy * sumy));
...
int US_Math2::nearest_curve_point(
   double *xs, double *ys, const int npoints, bool interp, double &xgiven, double &ygiven, double *xnear, double *ynear,
   double *zs, double *znear) {
   int jmin = npoints / 2;
   int jend = npoints - 1;
   ...
   if (interp) { // Interpolate a curve point between nearest point and an adjacent one
      ...
      if (jmin == 0) { // If nearest is first on curve, use the following point
      ...
      else if (jmin2 == npoints) { // If nearest is last on curve, use the preceding point
         jmin2 = jmin1;
         xdif = (xs[ jmin2 ] - xgiven) * xnorm;
         ydif = (ys[ jmin2 ] - ygiven) * ynorm;
         dmin2 = sqrt(sq(xdif) + sq(ydif));
         ...
         if (dmin2g > dmin1g) { // Where the test perpendicular towards the preceding point is
      ...
      if (xnear != NULL && ynear != NULL) { // Interpolate the curve point nearest to the given point
         *xnear = xs[ jmin ] * ratio + xs[ jmin2 ] * ratio2;
         *ynear = ys[ jmin ] * ratio + ys[ jmin2 ] * ratio2;
      }

      if (zs != NULL && znear != NULL) { // Interpolate the additional value associated with the nearest point
         *znear = zs[ jmin ] * ratio + zs[ jmin2 ] * ratio2;
      }
   }

   else { // Return the exact point found to be the nearest
      if (xnear != NULL && ynear != NULL) { // Return the curve point nearest to the given point
         *xnear = xs[ jmin ];
         *ynear = ys[ jmin ];
      }

      if (zs != NULL && znear != NULL) { // If desired, return the additional value associated with the point
         *znear = zs[ jmin ];
      }
   }
...
bool US_Math2::intersect(double &slope1, double &intcp1, double &slope2, double &intcp2, double *xisec, double *yisec) {
...
bool US_Math2::intersect(
   double *x1s, double *y1s, int npoint1, double *x2s, double *y2s, int npoint2, double *xisec, double *yisec) {
   double slope1;
   double intcp1;
   double slope2;
   double intcp2;
...
void US_Math2::calc_vbar(Peptide &pep, const QString &sequence, double temperature) {
   pep.vbar_sum = 0.0;
   pep.mw = 0.0;
   pep.weight = 0.0;
   pep.e280 = 0.0;
   ...
   pep.residues = pep.a + pep.b + pep.c + pep.d + pep.e + pep.f + pep.g + pep.h + pep.i + pep.j + pep.k + pep.l + pep.m
                  + pep.n + pep.o + pep.p + pep.q + pep.r + pep.s + pep.t + pep.v + pep.w + pep.x + pep.y + pep.z
                  + pep.dab + pep.dpr;
...
void US_Math2::data_correction(double t, SolutionData &d) {
   ...
   */
   ...
   d.density_wt = xi_max * (c0 + c1 * t + c2 * t2 + c3 * t3 + c4 * t4 + c5 * t5) / (1.0 + b * t);
...
uint US_Math2::randomize(void) {
   ...
   uint seed = now.msec() + now.second() * 1000 + now.minute() * 60000 + now.hour() * 3600000;
...
int US_Math2::nnls(
   double *a, int a_dim1, int m, int n, double *b, double *x, double *rnorm, double *wp, double *zzp, int *indexp) {
#ifdef _BF_NNLS_
   ...
   while (iz1 <= iz2 && nsetp < m) {
      ...
      if (iz1 <= iz2)
         for (jz = iz1; jz <= iz2; jz++) {
            jj = index[ jz ];
            _nnls_h12(2, nsetp - 1, npp1, m, &a[ ja_dim1 ], 1, &up, &a[ jj * a_dim1 ], 1, a_dim1, 1);
         }
         ...
         do {
            ...
            if (jj != (nsetp - 1)) {
               ...
               for (j = jj + 1; j < nsetp; j++) {
                  ...
                  _nnls_g1(a[ j - 1 + iia_dim1 ], a[ j + iia_dim1 ], &cc, &ss, &a[ j - 1 + iia_dim1 ]);
 ...
 */
void US_Math2::_nnls_g1(double a, double b, double *cterm, double *sterm, double *sig) {
   double d1;
   double xr;
   double yr;
...
int US_Math2::best_grid_reps(int &ngrid_s, int &ngrid_k) {
   ...
   for (int jreps = min_reps; jreps <= max_reps; jreps++) {
      ...
      if (jgrid_s == ngrid_s) { // Save a repetition that divides evenly into S grid points
         reps << jreps;
   ...
   qDebug() << "BGR:  nsubgs" << (nreps_g * nreps_g) << "subgsz" << (nsubg_s * nsubg_k) << "totgsz"
            << (ngrid_s * ngrid_k) << "nreps_g" << nreps_g;
...
qDebug() << "BGR: grfact" << grfact << "nreps_g" << nreps_g;
   ...
   while ((nsubg_s * nsubg_k) > max_subg || nreps_g < min_reps) { // Increase grid-reps and recompute sub-grid points
      nreps_g++;
   ...
   while ((nsubg_s * nsubg_k) < min_subg || nreps_g > max_reps) { // Decrease grid-reps and recompute sub-grid points
      nreps_g--;
      nsubg_s = ngrid_s / nreps_g;
      nsubg_k = ngrid_k / nreps_g;
   }
