...
class libnnls {
 public:
  typedef int (*nnls_fn)(double*, int, int, int, double*, double*, double*,
                         double*, double*, int*);
...
double US_Math2::linefit(double** x, double** y, double* slope,
  ...
  *slope =
      (arraysize * sumxy - sumx * sumy) / (arraysize * sumx_sq - sumx * sumx);
...
int US_Math2::nearest_curve_point(double* xs, double* ys, const int npoints,
                                  ...
                                  double* xnear, double* ynear, double* zs,
                                  double* znear) {
  int jmin = npoints / 2;
  int jend = npoints - 1;
  ...
  if (interp) {  // Interpolate a curve point between nearest point and an
                 // adjacent one
    ...
    if (jmin == 0) {  // If nearest is first on curve, use the following point
      ...
      dmin2 = sqrt(sq(xdif) + sq(ydif));
             ...
             npoints) {  // If nearest is last on curve, use the preceding point
      jmin2 = jmin1;
      xdif = (xs[jmin2] - xgiven) * xnorm;
      ydif = (ys[jmin2] - ygiven) * ynorm;
      dmin2 = sqrt(sq(xdif) + sq(ydif));
    ...
    else {  // If the nearest point is somewhere in the middle of the curve,
      ...
      if (dmin2g > dmin1g) {  // Where the test perpendicular towards the
                              // preceding point is
    ...
    if (xnear != NULL &&
        ynear !=
            NULL) {  // Interpolate the curve point nearest to the given point
      *xnear = xs[jmin] * ratio + xs[jmin2] * ratio2;
      *ynear = ys[jmin] * ratio + ys[jmin2] * ratio2;
    }

    if (zs != NULL && znear != NULL) {  // Interpolate the additional value
                                        // associated with the nearest point
      *znear = zs[jmin] * ratio + zs[jmin2] * ratio2;
    }
  }

  else {  // Return the exact point found to be the nearest
    if (xnear != NULL &&
        ynear != NULL) {  // Return the curve point nearest to the given point
      *xnear = xs[jmin];
      *ynear = ys[jmin];
    }

    if (zs != NULL && znear != NULL) {  // If desired, return the additional
...
bool US_Math2::intersect(double& slope1, double& intcp1, double& slope2,
                         double& intcp2, double* xisec, double* yisec) {
  bool isect = (slope1 != slope2);  // Parallel lines do not intersect!

  if (isect) {  // Return intersection point of non-parallel lines
...
bool US_Math2::intersect(double* x1s, double* y1s, int npoint1, double* x2s,
                         double* y2s, int npoint2, double* xisec,
                         double* yisec) {
  double slope1;
  double intcp1;
  double slope2;
...
void US_Math2::calc_vbar(Peptide& pep, const QString& sequence,
  ...
  pep.residues = pep.a + pep.b + pep.c + pep.d + pep.e + pep.f + pep.g + pep.h +
                 pep.i + pep.j + pep.k + pep.l + pep.m + pep.n + pep.o + pep.p +
                 pep.q + pep.r + pep.s + pep.t + pep.v + pep.w + pep.x + pep.y +
                 pep.z + pep.dab + pep.dpr;
...
void US_Math2::data_correction(double t, SolutionData& d) {
  ...
  \f[  \rho_t = \xi_{max}{{c_0 + c_1 * t + c_2 * t^2 + c_3 * t^3 + c_4 * t^4 +
  c_5 * t^5}
                \over{ 1 + bt }}
  \f]
  ...
  */
  ...
  /*!
     ...
     \f[
          \log_{10}{\eta_t\over\eta_{20}} = { {c_1(20 - t) -c_2(t-20)^2}\over
     {t+c_3}}
     \f]
  ...
  */
  ...
  /*
  Reference:
  Philo, J. S. (2023). SEDNTERP: a calculation and database utility to aid
  interpretation of analytical ultracentrifugation and light scattering data.
  European Biophysics Journal, 1-34. Equations 21 and 30
  */
  ...
  double b4 = -40.0;
  double T = (t + K0) / 300.0;
  d.viscosity_wt = 1e-3 * (a1 * qPow(T, b1) + a2 * qPow(T, b2) +
                           a3 * qPow(T, b3) + a4 * qPow(T, b4));
#endif
  ...
  DbgLv(1) << QObject::tr("dataCorr:  water: visc(20)=%1, visc(%2)=%3")
                  .arg(VISC_20W)
                  .arg(t)
                  .arg(d.viscosity_wt);
  DbgLv(1) << QObject::tr("dataCorr:  water: dens(20)=%1, dens(%2)=%3")
                  .arg(DENS_20W)
                  .arg(t)
                  .arg(d.density_wt);
  DbgLv(1) << QObject::tr("dataCorr: buffer: visc(20)=%1, visc(%2)=%3")
                  .arg(d.viscosity)
                  .arg(t)
                  .arg(d.viscosity_tb);
  DbgLv(1) << QObject::tr("dataCorr: buffer: dens(20)=%1, dens(%2)=%3")
                  .arg(d.density)
                  .arg(t)
                  .arg(d.density_tb);
  DbgLv(1) << QObject::tr("dataCorr: solute: vbar(20)=%1, vbar(%2)=%3")
                  .arg(d.vbar20)
                  .arg(t)
                  .arg(d.vbar);
  DbgLv(1) << QObject::tr("dataCorr: boyncy: water(20)=%1, buffer(%2)=%3")
                  .arg(d.buoyancyw)
                  .arg(t)
                  .arg(d.buoyancyb);
  DbgLv(1) << QObject::tr("dataCorr: factor: sed=%1, diff=%2")
                  .arg(d.s20w_correction)
                  .arg(d.D20w_correction)
           << "\n";

  // if ( qAbs(d.vbar-0.72) > 0.001 ) {
  // qDebug() << "M2:dacor:  denstb denswt" << d.density_tb << d.density_wt <<
  // "manual" << d.manual; qDebug() << "M2:dacor:  visctb viscwt" <<
  // d.viscosity_tb << d.viscosity_wt; qDebug() << "M2:dacor:  buoyb buoyw" <<
  // d.buoyancyb << d.buoyancyw; qDebug() << "M2:dacor:    vbtb vb20" << d.vbar
  // << d.vbar20; qDebug() << "M2:dacor:    dentb den20 den" << d.density_tb <<
  // DENS_20W << d.density; qDebug() << "M2:dacor:    vistb vis20 vis" <<
  // d.viscosity_tb << VISC_20W << d.viscosity; qDebug() << "M2:dacor:   scorr
  // dcorr" << d.s20w_correction << d.D20w_correction; }
}
...
double US_Math2::time_correction(const QVector<US_DataIO::RawData>& dataList) {
  int size = dataList[0].scanData.size();
...
uint US_Math2::randomize(void) {
  ...
  uint seed = now.msec() + now.second() * 1000 + now.minute() * 60000 +
              now.hour() * 3600000;
...
int US_Math2::nnls(double* a, int a_dim1, int m, int n, double* b, double* x,
                   double* rnorm, double* wp, double* zzp, int* indexp) {
#ifdef _BF_NNLS_
  ...
  while (iz1 <= iz2 && nsetp < m) {
    ...
    for (iz = iz1; iz <= iz2; iz++) {
      ...
      for (l = npp1; l < m; l++) sm += a[l + ja_dim1] * b[l];
    ...
    while (true) {
      ...
      _nnls_h12(1, npp1, npp1 + 1, m, &a[ja_dim1], 1, &up, &dummy, 1, 1, 0);
      ...
      if ((d2 - unorm) > 0.0) {
        ...
        _nnls_h12(2, npp1, npp1 + 1, m, &a[ja_dim1], 1, &up, zz, 1, 1, 1);
    ...
    if (iz1 <= iz2)
      for (jz = iz1; jz <= iz2; jz++) {
        jj = index[jz];
        _nnls_h12(2, nsetp - 1, npp1, m, &a[ja_dim1], 1, &up, &a[jj * a_dim1],
                  1, a_dim1, 1);
      }

    if (nsetp != m)
      for (l = npp1; l < m; l++) a[l + ja_dim1] = 0.0;
    ...
    for (l = 0; l < nsetp; l++) {
      ...
      if (l != 0)
        for (ii = 0; ii <= ip; ii++) zz[ii] -= a[ii + jj * a_dim1] * zz[ip + 1];
      ...
      do {
        ...
        if (jj != (nsetp - 1)) {
          ...
          for (j = jj + 1; j < nsetp; j++) {
            ...
            _nnls_g1(a[j - 1 + iia_dim1], a[j + iia_dim1], &cc, &ss,
                     &a[j - 1 + iia_dim1]);
 ...
 */
void US_Math2::_nnls_g1(double a, double b, double* cterm, double* sterm,
                        double* sig) {
  double d1;
  double xr;
  double yr;
...
double US_Math2::calcCommonVbar(US_Solution& solution, double temperature) {
  ...
  for (int ii = 0; ii < solution.analyteInfo.size(); ii++) {
    ...
    double vbar = solution.analyteInfo[ii].analyte.type == US_Analyte::PROTEIN
                      ...
                      : vb20;
    double wt =
        solution.analyteInfo[ii].analyte.mw * solution.analyteInfo[ii].amount;
    vbsum += (vbar * wt);
    wtsum += wt;
  }
...
int US_Math2::best_grid_reps(int& ngrid_s, int& ngrid_k) {
  ...
  for (int jreps = min_reps; jreps <= max_reps; jreps++) {
    ...
    if (jgrid_s ==
        ngrid_s) {  // Save a repetition that divides evenly into S grid points
      reps << jreps;
  ...
  for (int ii = 0; ii < reps.count(); ii++) {
    ...
    if (jdiff < kdiff) {  // Count and size of subgrid closely matched
      kdiff = jdiff;
      kreps = jreps;
      kgrid_k = jgrid_k;
  ...
  qDebug() << "BGR: ngrid_s" << ngrid_s << "ngrid_k ngrid_k_i" << ngrid_k
           << ngrid_k_i;
  qDebug() << "BGR:  nsubgs" << (nreps_g * nreps_g) << "subgsz"
           << (nsubg_s * nsubg_k) << "totgsz" << (ngrid_s * ngrid_k)
           << "nreps_g" << nreps_g;
...
qDebug() << "BGR: grfact" << grfact << "nreps_g" << nreps_g;
  ...
  while ((nsubg_s * nsubg_k) > max_subg ||
         nreps_g <
             min_reps) {  // Increase grid-reps and recompute sub-grid points
    nreps_g++;
  ...
  while ((nsubg_s * nsubg_k) < min_subg ||
         nreps_g >
             max_reps) {  // Decrease grid-reps and recompute sub-grid points
    nreps_g--;
    nsubg_s = ngrid_s / nreps_g;
    nsubg_k = ngrid_k / nreps_g;
  }
...
#define _FR_MXKNT 100         // Max find-root iteration count
  ...
  while (qAbs(test - goal) > tolerance) {
    ...
    if (test < goal) {  // At less than goal, adjust top (x2) limit
      x2 = xv;
      xv -= xdiff;
    }

    else {  // At greater than goal, adjust bottom (x1) limit
      x1 = xv;
      xv += xdiff;
    }
    ...
    xsqr = xv * xv;
    test = (1.0 + 2.0 * xsqr) * erfc(xv) - (2.0 * xv * exp(-xsqr)) * rsqr_pi;

    if ((++count) > _FR_MXKNT) break;
  }
