...
bool US_CfaData::import_data(QString &cfadir, const int sctypf) {
   ...
   if (sctype > 0) { // Scan type given:  data from .sqlite file
      if (QFileInfo(cur_dir).isFile()) { // Referenced "directory" is a file, so use that;
         cur_dir = cur_dir.section("/", 0, -2) + "/";
      }

      else { // Use referenced directory; find (largest) .sqlite file there
         if (cur_dir.right(1) != "/")
            cur_dir += "/";

         QDir ddir(cur_dir, "*", QDir::Name, QDir::Files | QDir::Readable);
         QStringList dbfs = ddir.entryList(QStringList("*.sqlite"), QDir::Files, QDir::Name);
         int ndfile = dbfs.count();

         if (ndfile == 1) { // Directory has single .sqlite file
            dbfile = cur_dir + dbfs[ 0 ];
         }

         else if (ndfile > 1) { // Directory has multiple .sqlfile files; use largest
            int szfile = 0;
         ...
         else { // Directory has no .sqlite file, ERROR!
            dbfile = QString("");
         }
...
void US_CfaData::load_auc(QVector<US_DataIO::RawData> &allData, const QStringList ifpaths) {
   QStringList chans;
   sctype = -1;
   is_absorb = false;
   is_raw = false;
...
int US_CfaData::build_rawData(QVector<US_DataIO::RawData> &allData) {
   ...
   a_radii = r_radii;
   double rad_inc = (a_radii[ npoint - 1 ] - a_radii[ 0 ]) / ( double ) (npoint - 1);
   ...
   int mnscan = nscnn;
   int mxscan = 0;
   DbgLv(1) << "BldRawD szs: ccd" << ccdescs.size() << "wvs" << wavelns.size() << "nlambda" << nlambda;
   ...
   for (int trx = 0; trx < ntriple; trx++) {
      ...
      for (int scx = 0; scx < nscnn; scx++) { // Set scan values
         ...
         if (scx < 3 || (scx + 4) > nscnn)
            DbgLv(1) << "BldRawD       trx wvx scx snx" << trx << wvx << scx << snx << "scnid" << scnid << "frec lrec"
                     << frec << lrec;
         ...
         if (rpm_set != rpm_setp) { // For a new speed step, re-do speeds in the previous one
            nspstep++;
            ...
            if (trx == 0) { // Save speeds at first triple
               a_rpms << rpm_avg;
               s_rpms << rpm_setp;
            }
         ...
         else { // Bump scan count in a speed step
            kscan++;
         }
         ...
         if (trx == 0) {
            DbgLv(1) << "BldRawD      scx" << scx << "trx" << trx << "seconds" << scan.seconds << "rpm" << scan.rpm
                     << "ss" << rpm_set << "speed step" << kspstep << nspstep + 1;
         }
         //*DEBUG*
         if (trx == 0)
            r_rpms << scan.rpm;
      ...
      //*DEBUG*
      if (trx == 0) {
         DbgLv(1) << "BldRawD trx=" << trx << "rpm_min rpm_max rpm_avg rpm_set" << rpm_min << rpm_max << rpm_avg
                  << rpm_set << "speed steps" << nspstep;
      }
      //*DEBUG*
   ...
   DbgLv(1) << "BldRawD  DONE ntriple" << ntriple << "mxscan" << mxscan << "mnscan" << mnscan;
   return ntriple;
}
...
int US_CfaData::export_auc(QVector<US_DataIO::RawData> &allData) {
   ...
   for (int ii = 0; ii < ntrips; ii++) { // Create a file for each triple
      ...
      QString trnode = QString::number(rdata->cell) + "." + QString(rdata->channel) + "."
                       + QString().sprintf("%03d", qRound(rdata->scanData[ 0 ].wavelength));
   ...
   for (int ii = 0; ii < ntimes; ii++) { // Create a record for each scan
      ...
      for (int jj = 0; jj < tcmetas.count(); jj++) { // Match scan ID in CFAMeta
      ...
      for (int jj = 0; jj < tdmetas.count(); jj++) { // Match scan ID in DIOMeta
         if (tdmetas[ jj ].scanId == scanid) {
            jjd = jj; // Index to ID in DIOMeta values
            break;
         }
      }
      DbgLv(1) << "expA:  ii" << ii << "scannbr scanid" << scannbr << scanid << "jjc" << jjc << "jjd" << jjd << "time"
               << tcmetas[ jjc ].time;
...
void US_CfaData::scan_cfafile(const QString dbpath, const int sctp) {
   ...
   DbgLv(1) << "CfDa:scn: ntabs" << ntabs << "tab0 tabn" << sqtabs[ 0 ] << sqtabs[ ntabs - 1 ];
   ...
   for (int ii = 0; ii < ntabs; ii++) { // Parse each table; save counts and many of the values
      QDateTime time10 = QDateTime::currentDateTime();
      stimes[ ii ] = time10;
      etimes[ ii ] = time10;
      ...
      if (rows < 1) {
         DbgLv(1) << "CfDa:scn: ii" << ii << "dbtab" << dbtab << "rows" << rows << "cols" << cols;
         rows = 0;
         totals[ dbtab ] = rows;
         counts[ dbtab ] = strows;
         continue;
      ...
      QString stat_text = tr("Scanning %1 rows from %2 table...").arg(rows).arg(dbtab);
      emit status_text(stat_text);

      QString qtxt = (dbtab != "ScanMeta") ? QString("SELECT * from ") + dbtab
      ...
      if (dbtab == "Scans") {
         ...
         int snx = tscans.count() == 0 ? scan_nbr_index(tripl, 1) : scan_nbr_index(tripl, lsc);
      ...
      int row = 0;
      DbgLv(1) << "CfDa:scn: ii" << ii << "dbtab" << dbtab << "rows" << rows << "cols" << cols << "qtxt" << qtxt;
      ...
      while (sqry.next()) { // Loop to pick up values from rows of the table
         ...
         if (dbtab == "AbsMeta") {
            ...
            datrow.triple = QString::number(datrow.cell) + "/" + QString('A' + datrow.channel) + "/"
                            + QString::number(datrow.wavelen);
            //if(row<15)
            DbgLv(1) << "CfDa:scn: scanID" << datrow.scanId << "scNbr" << datrow.scanNbr << "cell chan wvln"
                     << datrow.cell << datrow.channel << datrow.wavelen << "fRec lRec" << datrow.firstRec
                     << datrow.lastRec << "triple" << datrow.triple;
            tsmetas << datrow;
         }
            ...
            tsampls << datrow;
            if (row < 15)
               DbgLv(1) << "CfDa:scn: cell chann" << datrow.cell << datrow.channel << "concen" << datrow.concen
                        << "units" << datrow.units << "descript" << datrow.descript;
         }
   ...
   for (int jj = 0; jj < ntabs; jj++) {
      DbgLv(1) << "CfDa:scn: TIMEMS table scan " << stimes[ jj ].msecsTo(etimes[ jj ]) << "table" << sqtabs[ jj ];
   }
   ...
   for (int ii = 0; ii < tsmetas.count(); ii++) { // Use ScanMeta data to get cell/channel, triple, lambda lists
      ...
      DbgLv(1) << "CfDa:scn: ii" << ii << "icell ichan schan cechn" << icell << ichan << schan << cechn;
 ...
 << "scnnbr" << scnnbr << "tripl" << tripl;
   ...
   for (int ii = 0; ii < triples.count(); ii++) {
      ...
      if (!cellchans.contains(cechn)) {
         ...
         for (int jj = 0; jj < tsampls.count(); jj++) { // Use Samples data to get cell/channel description
   ...
   double ccpix = coef1 + coef2 * pix1 + coef3 * pix2 + coef4 * pix3;
   double rbase = 6.5;
   ...
   for (int ipix = 1; ipix <= npoint; ipix++) {
      ...
      pix2 = pix1 * pix1;
      pix3 = pix2 * pix1;
      double cppix = coef1 + coef2 * pix1 + coef3 * pix2 + coef4 * pix3;
   ...
   //*DEBUG*
   DbgLv(1) << "CfDa:scn: ncelchn" << ncelchn << "ntriple" << ntriple << "nlambda" << nlambda << "nscanids"
            << scanids.count() << "nscnnbrs" << scnnbrs.count() << "ndups" << ndups << "nccdescs" << ccdescs.count();
   for (int jj = 0; jj < ncelchn; jj++)
      DbgLv(1) << "CfDA:scn:  celchn" << jj + 1 << ":" << cellchans[ jj ] << "ccdesc" << ccdescs[ jj ];
   for (int jj = 0; jj < nlambda; jj++)
      DbgLv(1) << "CfDA:scn:   lambda" << jj + 1 << ":" << wavelns[ jj ];
   for (int jj = 0; jj < ntriple; jj++)
      DbgLv(1) << "CfDA:scn:  triple" << jj + 1 << ":" << triples[ jj ] << "trnode" << trnodes[ jj ];
   ...
   DbgLv(1) << "CfDa:scn: npoint" << npoint << "rad1,radn" << r_radii[ 0 ] << r_radii[ lpnt ] << "radinc avg" << ravg;
   ravg = 0.0;
   double rilo = 9999.0;
   double rihi = 0.0;
...
int US_CfaData::scan_nbr_index(const QString tripl, const int snbr) {
   ...
   for (int jj = 0; jj < tsmetas.count(); jj++) { // Match triple, scan type, scan number
      if (
         tsmetas[ jj ].triple == tripl && tsmetas[ jj ].scanType == sctype
         && tsmetas[ jj ].scanNbr == snbr) { // Return with index to matched triple,scan
         srx = jj;
         break;
      }
   }
   DbgLv(1) << "CfDa:srxS: tripl" << tripl << "snbr" << snbr << "sctp" << sctype << "tsm.count" << tsmetas.count()
            << "srx" << srx;
...
int US_CfaData::scan_id_index(const int scid) {
   ...
   for (int jj = 0; jj < tcmetas.count(); jj++) { // Match scan ID
      if (tcmetas[ jj ].scanId == scid) { // Return with index to matched scan ID
         srx = jj;
         break;
      }
   }
   DbgLv(1) << "CfDa:srxC: scid" << scid << "tcm.count" << tcmetas.count() << "srx" << srx;

   return srx;
}

// Get data readings from a range of Scans table records
int US_CfaData::get_readings(QVector<double> &readings, const int frec, const int lrec) {
   DbgLv(1) << "CfDa:grd: frec lrec" << frec << lrec;
   if (frec < 0) { // Return zeroed readings if scan data not present in DB
   ...
   while (sqry.next()) { // Loop to pick up values from rows of the table
      ...
      row++;
      if (row < 3 || row > 1022)
         DbgLv(1) << "CfDa:grd:   row" << row << "dbid scnId rval" << dbid << scnId << rval;
...
QString US_CfaData::runDetails(void) {
   ...
   if (is_raw) { // Input is raw CFA DB
      ...
      msg += tr("    First+Last Count:         %1").arg(kscans) + "   ( " + tsmetas[ 0 ].triple + " )\n";
   ...
   else { // Input is AUC files
      QDir tdir(cur_dir, "*", QDir::Name, QDir::Files | QDir::Readable);
      QStringList tmfs = tdir.entryList(QStringList("*.tmst"), QDir::Files, QDir::Name);
      ...
      if (ntfile > 0) { // TMST file exists, so report on it
         QString tmfn = tmfs[ 0 ];
         ...
         for (int jj = 0; jj < 2; jj++) {
            ...
            for (int kk = 0; kk < nkey; kk++) {
               ...
               QString cmsg = QString(ctitl + spad).left(flen) + cval + "\n";
               msg += cmsg;
            }
         }
      }
