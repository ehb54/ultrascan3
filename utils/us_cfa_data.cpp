...
bool
US_CfaData::import_data (QString &cfadir, const int sctypf)
{
  bool status = true;
  sctype = sctypf;
  cur_dir = cfadir;
...
void
US_CfaData::load_auc (QVector<US_DataIO::RawData> &allData,
                      const QStringList ifpaths)
{
  ...
  DbgLv (1) << "CfDa:LdMw: nfile" << nfile << "nscan npoint" << nscan
            << npoint;
  ...
  for (int trx = 0; trx < nfile; trx++)
    {
      ...
      if (!triples.contains (triple))
        ...
        { // Handle duplicate triple
          DbgLv (1) << "CfDa:LdMw:    DUPLICATE triple" << triple << "trx"
                    << trx;
              ...
              if (!triples.contains (triple))
                {
                  ...
                  DbgLv (1)
                      << "CfDa:LdMw:      NEW triple" << triple << trnode;
                  triples << triple;
                }
            }
        }
...
int
US_CfaData::lambdas_raw (QVector<int> &wls)
{
  wls.clear ();
  wls.reserve (nlambda);
...
int
US_CfaData::indexOfLambda (int lambda)
{
...
int
US_CfaData::cellchannels (QStringList &celchns)
{
  celchns = cellchans;
  return ncelchn;
}
...
int
US_CfaData::build_rawData (QVector<US_DataIO::RawData> &allData)
{
  DbgLv (1) << "BldRawD IN";
      ...
      for (int scx = 0; scx < nscnn; scx++)
        { // Set scan values
          ...
          if (scx < 3 || (scx + 4) > nscnn)
            DbgLv (1) << "BldRawD       trx wvx scx snx" << trx << wvx << scx
                      << snx << "scnid" << scnid << "frec lrec" << frec
                      << lrec;
...
int
US_CfaData::export_auc (QVector<US_DataIO::RawData> &allData)
{
  int nfiles = 0;
  if (!is_raw)
    return nfiles;
  ...
  for (int ii = 0; ii < ntrips; ii++)
    { // Create a file for each triple
      ...
      QString trnode = QString::number (rdata->cell) + "."
                       ...
                       + QString ().sprintf (
                           "%03d", qRound (rdata->scanData[0].wavelength));
...
int
US_CfaData::countOf (QString key)
{
  mapCounts ();
...
QString
US_CfaData::cc_description (QString celchn)
{
...
void
US_CfaData::run_values (QString &arunid, QString &aruntype)
{
  arunid = runID;
...
void
US_CfaData::scan_cfafile (const QString dbpath, const int sctp)
{
  ...
  for (int ii = 0; ii < ntabs; ii++)
    { // Parse each table; save counts and many of the values
      ...
      if (rows < 1)
        {
          DbgLv (1) << "CfDa:scn: ii" << ii << "dbtab" << dbtab << "rows"
                    << rows << "cols" << cols;
          rows = 0;
          totals[dbtab] = rows;
          counts[dbtab] = strows;
        ...
        { // Loop to pick up values from rows of the table
          ...
          if (dbtab == "AbsMeta")
          ...
          else if (dbtab == "Instrument")
            {
              ...
              // << jttm
              //  << jmag << jnpx << jtpx << jbpx << jcpx << jsrv << jsrc <<
              //  jdet
              //  << jmac << jdsc;
              datrow.id = dbid;
          ...
          else if (dbtab == "Samples")
            {
              ...
              tsampls << datrow;
              if (row < 15)
                DbgLv (1) << "CfDa:scn: cell chann" << datrow.cell
                          << datrow.channel << "concen" << datrow.concen
                          << "units" << datrow.units << "descript"
                          << datrow.descript;
            }
...
int
US_CfaData::scan_nbr_index (const QString tripl, const int snbr)
{
  ...
  DbgLv (1) << "CfDa:srxS: tripl" << tripl << "snbr" << snbr << "sctp"
            << sctype << "tsm.count" << tsmetas.count () << "srx" << srx;
...
int
US_CfaData::scan_id_index (const int scid)
{
...
int
US_CfaData::get_readings (QVector<double> &readings, const int frec,
                          const int lrec)
{
  ...
  while (sqry.next ())
    { // Loop to pick up values from rows of the table
      ...
      row++;
      if (row < 3 || row > 1022)
        DbgLv (1) << "CfDa:grd:   row" << row << "dbid scnId rval" << dbid
                  << scnId << rval;
              ...
              for (int kk = 0; kk < nkey; kk++)
                {
                  ...
                  QString cmsg
                      = QString (ctitl + spad).left (flen) + cval + "\n";
                  msg += cmsg;
                }
            }
        }
