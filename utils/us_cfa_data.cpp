...
bool US_CfaData::import_data(QString& cfadir, const int sctypf) {
  ...
  if (sctype > 0) {  // Scan type given:  data from .sqlite file
    if (QFileInfo(cur_dir)
            .isFile()) {  // Referenced "directory" is a file, so use that;
      ...
      QStringList dbfs =
          ddir.entryList(QStringList("*.sqlite"), QDir::Files, QDir::Name);
      ...
      if (ndfile == 1) {  // Directory has single .sqlite file
        dbfile = cur_dir + dbfs[0];
               ...
               1) {  // Directory has multiple .sqlfile files; use largest
        int szfile = 0;
...
int US_CfaData::build_rawData(QVector<US_DataIO::RawData>& allData) {
  ...
  for (int trx = 0; trx < ntriple; trx++) {
    ...
    for (int scx = 0; scx < nscnn; scx++) {  // Set scan values
      ...
      if (scx > 0 && scan.rpm != r_rpms[scx - 1]) kspstep++;

      if (scx < 3 || (scx + 4) > nscnn)
        DbgLv(1) << "BldRawD        rpm_set rpm_setp" << rpm_set << rpm_setp;
      if (rpm_set !=
          rpm_setp) {  // For a new speed step, re-do speeds in the previous one
        nspstep++;
      ...
      if (trx == 0) {
        DbgLv(1) << "BldRawD      scx" << scx << "trx" << trx << "seconds"
                 << scan.seconds << "rpm" << scan.rpm << "ss" << rpm_set
                 << "speed step" << kspstep << nspstep + 1;
      }
      //*DEBUG*
      if (trx == 0) r_rpms << scan.rpm;
    ...
    if (trx == 0) {
      DbgLv(1) << "BldRawD trx=" << trx << "rpm_min rpm_max rpm_avg rpm_set"
               << rpm_min << rpm_max << rpm_avg << rpm_set << "speed steps"
               << nspstep;
    }
    //*DEBUG*
...
void US_CfaData::scan_cfafile(const QString dbpath, const int sctp) {
  ...
  DbgLv(1) << "CfDa:scn: ntabs" << ntabs << "tab0 tabn" << sqtabs[0]
           << sqtabs[ntabs - 1];
  ...
  for (int ii = 0; ii < ntabs;
       ii++) {  // Parse each table; save counts and many of the values
    QDateTime time10 = QDateTime::currentDateTime();
    stimes[ii] = time10;
    etimes[ii] = time10;
    ...
    if (dbtab.startsWith("sqlite_stat")) continue;
    ...
    QString stat_text =
        tr("Scanning %1 rows from %2 table...").arg(rows).arg(dbtab);
    emit status_text(stat_text);

    QString qtxt = (dbtab != "ScanMeta") ? QString("SELECT * from ") + dbtab
                                         : QString(
                                               "SELECT * from ScanMeta where"
                                               " ScanType=%1")
                                               .arg(sctp);
    ...
    if (dbtab == "Scans") {
      ...
      int snx = tscans.count() == 0 ? scan_nbr_index(tripl, 1)
    ...
    while (sqry.next()) {  // Loop to pick up values from rows of the table
      ...
      if (dbtab == "AbsMeta") {
        ...
        // if(row<15)
        DbgLv(1) << "CfDa:scn: scanID" << datrow.scanId << "scNbr"
                 << datrow.scanNbr << "cell chan wvln" << datrow.cell
                 << datrow.channel << datrow.wavelen << "fRec lRec"
                 << datrow.firstRec << datrow.lastRec << "triple"
                 << datrow.triple;
        tsmetas << datrow;
      }
        ...
        tInstru datrow;
        // DbgLv(1) << "CfDa:scn:     js" << jtyp << jitm << jotm << jstm <<
        // jttm
  ...
  for (int jj = 0; jj < ntabs; jj++) {
    DbgLv(1) << "CfDa:scn: TIMEMS table scan " << stimes[jj].msecsTo(etimes[jj])
             << "table" << sqtabs[jj];
  }
  ...
  for (int ii = 0; ii < tsmetas.count();
       ii++) {  // Use ScanMeta data to get cell/channel, triple, lambda lists
    ...
    DbgLv(1) << "CfDa:scn: ii" << ii << "icell ichan schan cechn" << icell
             << ichan << schan << cechn;
...
DbgLv(1) << "CfDa:scn:   *DUP* scinx" << scinx << "scnid" << scnid
 << "scnnbr" << scnnbr << "tripl" << tripl;
  ...
  DbgLv(1) << "CfDa:scn: ncelchn" << ncelchn << "ntriple" << ntriple
           ...
           << "nscnnbrs" << scnnbrs.count() << "ndups" << ndups << "nccdescs"
           << ccdescs.count();
  ...
  for (int jj = 0; jj < ntriple; jj++)
    DbgLv(1) << "CfDA:scn:  triple" << jj + 1 << ":" << triples[jj] << "trnode"
             << trnodes[jj];
  ...
  DbgLv(1) << "CfDa:scn: npoint" << npoint << "rad1,radn" << r_radii[0]
           << r_radii[lpnt] << "radinc avg" << ravg;
  ravg = 0.0;
  double rilo = 9999.0;
  double rihi = 0.0;
...
int US_CfaData::scan_nbr_index(const QString tripl, const int snbr) {
  ...
  for (int jj = 0; jj < tsmetas.count();
       jj++) {  // Match triple, scan type, scan number
    if (tsmetas[jj].triple == tripl && tsmetas[jj].scanType == sctype &&
        tsmetas[jj].scanNbr ==
            snbr) {  // Return with index to matched triple,scan
      srx = jj;
      break;
    }
  }
...
int US_CfaData::get_readings(QVector<double>& readings, const int frec,
                             const int lrec) {
  ...
  while (sqry.next()) {  // Loop to pick up values from rows of the table
    ...
    row++;
    if (row < 3 || row > 1022)
      DbgLv(1) << "CfDa:grd:   row" << row << "dbid scnId rval" << dbid << scnId
               << rval;
...
QString US_CfaData::runDetails(void) {
  ...
  if (is_raw) {  // Input is raw CFA DB
    ...
    QStringList tmfs =
        tdir.entryList(QStringList("*.tmst"), QDir::Files, QDir::Name);
    ...
    if (ntfile > 0) {  // TMST file exists, so report on it
      ...
      for (int jj = 0; jj < 2; jj++) {
        ...
        for (int kk = 0; kk < nkey; kk++) {
          ...
          if (cfmt == "I") cval = QString::number(tsobj->time_ivalue(ckey));
          if (cfmt == "F") cval = QString::number(tsobj->time_dvalue(ckey));
          if (cfmt == "C") cval = tsobj->time_svalue(ckey);
