...
void
US_ZSolute::put_mcomp_attr (US_Model::SimulationComponent &comp, double &aval,
                            const int a_type)
{
  switch (a_type)
    {
...
void
US_ZSolute::put_solute_attr (US_ZSolute &solute, double &aval,
                             const int a_type)
{
  switch (a_type)
    {
...
void
US_ZSolute::get_mcomp_attr (US_Model::SimulationComponent &comp, double &aval,
                            const int a_type)
{
  switch (a_type)
    {
...
void
US_ZSolute::get_solute_attr (US_ZSolute &solute, double &aval,
                             const int a_type)
{
  switch (a_type)
    {
...
void
US_ZSolute::set_mcomp_values (US_Model::SimulationComponent &comp,
...
void
US_ZSolute::set_solute_values (US_Model::SimulationComponent &comp,
                               US_ZSolute &solute, const int s_type)
{
  int attr_x = (s_type >> 6) & 7;
  int attr_y = (s_type >> 3) & 7;
...
void
US_ZSolute::init_grid_solutes (double x_min, double x_max, int x_res,
  ...
  for (int ii = 0; ii < grid_reps; ii++)
    {
      for (int jj = 0; jj < grid_reps; jj++)
        {
          solutes << create_grid_solutes (x_min + x_grid * ii, x_max, x_step,
...
int
US_ZSolute::zcoeff_flag (double *zcoeffs)
{
...
QVector<US_ZSolute>
US_ZSolute::create_grid_solutes (double x_min, double x_max, double x_step,
      ...
      for (double xx = x_min; xx <= x_max; xx += x_step)
        {
          if (x_is_s && xx >= -1.0e-14 && xx <= 1.0e-14)
            continue;
          zz = z_coeffs[0] + z_coeffs[1] * xx + z_coeffs[2] * xx * xx
               + z_coeffs[3] * xx * xx * xx;
