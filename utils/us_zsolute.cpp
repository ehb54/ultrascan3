...
void US_ZSolute::put_mcomp_attr(US_Model::SimulationComponent &comp, double &aval, const int a_type) {
   switch (a_type) {
      default:
...
void US_ZSolute::put_solute_attr(US_ZSolute &solute, double &aval, const int a_type) {
   switch (a_type) {
      default:
      case 0: // X value
         solute.x = aval;
...
void US_ZSolute::get_mcomp_attr(US_Model::SimulationComponent &comp, double &aval, const int a_type) {
   switch (a_type) {
      default:
      case ATTR_S: // Sedimentation Coefficient
         aval = comp.s;
...
void US_ZSolute::get_solute_attr(US_ZSolute &solute, double &aval, const int a_type) {
   switch (a_type) {
      default:
      case 0: // X value
         aval = solute.x;
...
void US_ZSolute::set_mcomp_values(
   US_Model::SimulationComponent &comp, US_ZSolute &solute, const int s_type, const bool concv) {
   ...
   if (concv) { // Copy concentration value, too
...
void US_ZSolute::set_solute_values(US_Model::SimulationComponent &comp, US_ZSolute &solute, const int s_type) {
   int attr_x = (s_type >> 6) & 7;
   int attr_y = (s_type >> 3) & 7;
   int attr_z = s_type & 7;
   double aval = 0.0;
...
void US_ZSolute::init_grid_solutes(
   double x_min, double x_max, int x_res, double y_min, double y_max, int y_res, int grid_reps, double *z_coeffs,
   ...
   for (int ii = 0; ii < grid_reps; ii++) {
      for (int jj = 0; jj < grid_reps; jj++) {
         solutes << create_grid_solutes(x_min + x_grid * ii, x_max, x_step, y_min + y_grid * jj, y_max, y_step, z_coeffs);
      }
   }
}
...
QVector<US_ZSolute> US_ZSolute::create_grid_solutes(
   double x_min, double x_max, double x_step, double y_min, double y_max, double y_step, double *z_coeffs) {
   ...
   if (zcflg == 0) { // Z is constant
      for (double xx = x_min; xx <= x_max; xx += x_step) {
         if (x_is_s && xx >= -1.0e-14 && xx <= 1.0e-14)
            continue;

         for (double yy = y_min; yy <= y_max; yy += y_step) {
            if (y_is_s && yy >= -1.0e-14 && yy <= 1.0e-14)
               continue;
   ...
   else if (zcflg == 1) { // Z varies linearly with X
      for (double xx = x_min; xx <= x_max; xx += x_step) {
         if (x_is_s && xx >= -1.0e-14 && xx <= 1.0e-14)
            continue;
   ...
   else { // Z varies by ( a + bX + cX^2 + dX^3 )
      for (double xx = x_min; xx <= x_max; xx += x_step) {
         if (x_is_s && xx >= -1.0e-14 && xx <= 1.0e-14)
            continue;
         zz = z_coeffs[ 0 ] + z_coeffs[ 1 ] * xx + z_coeffs[ 2 ] * xx * xx + z_coeffs[ 3 ] * xx * xx * xx;
