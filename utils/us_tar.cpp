...
int US_Tar::create(const QString &archive, const QString &directory, QStringList *list) {
...
int US_Tar::create(const QString &archive, const QStringList &files, QStringList *list) {
...
void US_Tar::write_file(const QString &file) {
   ...
   int perms = TSUID | TSGID | TSVTX | TUREAD | TUWRITE | TUEXEC | TGREAD | TGWRITE | TGEXEC | TOREAD | TOWRITE
               | TOEXEC;
   ...
   memcpy(( void * ) (buffer + blocks_written * BLOCK_SIZE), ( void * ) tar_header.h, sizeof tar_header);
   ...
   if (!f.isDir()) {
      ...
      if (full_blocks > 0 && space >= full_blocks) {
         input = read(ifd, buffer + blocks_written * BLOCK_SIZE, full_blocks * BLOCK_SIZE);
         ...
         input = read(ifd, buffer + blocks_written * BLOCK_SIZE, space * BLOCK_SIZE);
      ...
      if (input > 0) {
         ...
         memset(buffer + blocks_written * BLOCK_SIZE + input, 0, BLOCK_SIZE - input);
         blocks_written++;
         if (blocks_written == BLOCKING_FACTOR)
            flush_buffer();
      }
...
void US_Tar::write_long_filename(const QString &filename) {
   ...
   for (int i = 0; i < full_blocks; i++) {
      ...
      memcpy(( void * ) tar_header.h, ( void * ) filename.mid(i * BLOCK_SIZE, BLOCK_SIZE).constData(), BLOCK_SIZE);
   ...
   if ((length) % BLOCK_SIZE) {
      ...
      memcpy(( void * ) tar_header.h, ( void * ) filename.mid(full_blocks * BLOCK_SIZE).constData(), length % BLOCK_SIZE);
...
int US_Tar::list(const QString &archive, QStringList &files) {
   ...
   try {
      while (true) {
         ...
         files << format_permissions(mode, directory) + " " + uname + "/" + gname + " " + s.sprintf("%10d", fsize) + " "
...
QString US_Tar::format_datetime(const unsigned int mtime) {
   ...
   QString s;
   s = s.sprintf("%04d-%02d-%02d %02d:%02d", t->tm_year + 1900, t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min);
   return s;
}
