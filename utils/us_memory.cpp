...
US_Memory::rss_now (void)
{
  ...
  int stat1 = task_info (task, TASK_BASIC_INFO, (task_info_t)&task_stats,
                         &inf_count);
...
long int
US_Memory::rss_max (long int &rssmax)
{
...
int
US_Memory::memory_profile (int *pMemA, int *pMemT, int *pMemU)
{
  int memavail;
  int memtotal;
  int memused;
  ...
  qDebug () << "  UsMEM:LINUX: fmtotal,used,free,buffer,cache" << fmtotal
            << fmused << fmfree << fmbuffer << fmcache;
  memtotal = fmtotal;
...
#endif
  qDebug () << "  UsMEM: memtotal,avail,used" << memtotal << memavail
            << memused;
