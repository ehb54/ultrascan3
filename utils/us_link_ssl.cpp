...
bool
Link::connectToServer (const QString &host, const int port)
{
  ...
  if (server.waitForEncrypted (15000))
    {
      // server.write("***qsslsocket_client_example sent this nothing
      // command***\n"); status_ok = true;
...
bool
Link::connectToServer_init_check (const QString &host, const int port)
{
  bool status_ok = false;
...
void
Link::disconnectFromServer (void)
{
...
void
Link::sslErrors (const QList<QSslError> &errors)
{
  foreach (const QSslError &error, errors)
    {
      qDebug () << error.errorString ();
      // if ( QString (error.errorString() ).contains("The certificate is
      // self-signed, and untrusted")  ) 	server.ignoreSslErrors(errors);
    }
  // server.ignoreSslErrors(errors);
}

void
Link::serverDisconnect (void)
{
  qDebug ()
      << "Server disconnected from inside LINK -- e.g. [DROPPED CONNECTION]";
...
Link::rx (void)
{
  ...
  // qDebug() << "In LINK: ready to read: connected_itself " <<
  // connected_itself;
  ...
  if (parseerror.error)
  ...
  else
    {
      ...
      if (rootObj.contains ("elapsedTime") && rootObj.contains ("omega2T")
          && rootObj.contains ("speed") && rootObj.contains ("temperature")
          && rootObj.contains ("vacuum") && rootObj.contains ("stage")
  ...
  // qDebug() << rootObj["elapsedTime"].toString() <<
  // rootObj["omega2T"].toString() << rootObj["speed"].toString() <<
  // rootObj["temperature"].toString();
