...
{
   // Using std::vector to handle temporary memory automatically.
   std::vector<double> D20( Ne );
   std::vector<double> D21( Ne );
   std::vector<double> D30( Nv, 0.0 );
   std::vector<double> D31( Nv, 0.0 );

   // Compute second derivatives on elements.
   for ( int i = 0; i < Ne; i++ )
   {
      const double h      = ( x[i + 1] - x[i] ) * 0.5;
      const int    i2     = i * 2;
      const double inv_h2 = 1.0 / ( h * h );
      D20[i]              = ( u0[i2 + 2] - 2.0 * u0[i2 + 1] + u0[i2] ) * inv_h2;
      D21[i]              = ( u1[i2 + 2] - 2.0 * u1[i2 + 1] + u1[i2] ) * inv_h2;
   }

   // Compute third derivative at nodes.
   for ( int i = 1; i < Nv - 1; i++ )
   {
      const double h = ( x[i + 1] - x[i - 1] ) * 0.5;
   ...
   {
      D20[i] = ( D30[i + 1] + D30[i] ) * 0.5;
      D21[i] = ( D31[i + 1] + D31[i] ) * 0.5;
   }
   // Extrapolate 3rd derivative to endpoints
   D20[0]                    = D20[1];
   D21[0]                    = D21[1];
   D20[Ne - 1]               = D20[Ne - 2];
   D21[Ne - 1]               = D21[Ne - 2];
   constexpr double onethird = 1.0 / 3.0;
   // Compute MeshDen based on absolute third derivatives.
   for ( int i = 0; i < Ne; i++ )
   {
      const double sum_abs = std::fabs( D20[i] ) + std::fabs( D21[i] );
      const double val     = 1.0 + MonScale * sum_abs;
      double       wd      = std::pow( val, onethird );
      if ( wd > MonCutoff )
      {
         wd = MonCutoff;
      }
...
void US_LammAstfvm::Mesh::Smoothing( const int n, double* y, const double Wt, const int Cycle )
{
   ...
   for ( s = 0; s < Cycle; s++ )
   {
      ...
      y[0] = Wt * next + Wt1 * curr; // 1st smoothed point

      for ( i = 1; i < n - 1; i++ )
      {
         // smooth all points except first and last
         prev = curr; // cycle the 3 points
         curr = next; // around the current one
         next = y[i + 1];

         y[i] = Wt * curr + Wt2 * ( prev + next ); // smoothed point
      }

      y[n - 1] = Wt * curr + Wt1 * next; // last smoothed point
   }
}
...
void US_LammAstfvm::Mesh::Unrefine( double alpha )
{
   ...
   while ( true )
   {
      ...
      const int newNv = Ne1 + 1;
      // Allocate new arrays for the coarsened mesh.
      int*  newEid     = new int[Ne1];
      int*  newRefLev  = new int[Ne1];
      auto* newMeshDen = new double[Ne1];
      auto* newX       = new double[newNv];

      newX[0]  = x[0];
      int iNew = 0;
      for ( i = 0; i < Ne; i++ )
      {
         if ( i < Ne - 1 && Mark[i] && Mark[i + 1] ) // combine two elements
         {
            newX[iNew + 1]   = x[i + 2];
            newEid[iNew]     = Eid[i] / 2;
            newRefLev[iNew]  = RefLev[i] - 1;
            newMeshDen[iNew] = 0.5 * ( MeshDen[i] + MeshDen[i + 1] );
            iNew++;
            i++; // skip the next element
         }
         else // no un-refinement
         {
            newX[iNew + 1]   = x[i + 1];
            newEid[iNew]     = Eid[i];
            newRefLev[iNew]  = RefLev[i];
            newMeshDen[iNew] = MeshDen[i];
            iNew++;
         }
      }
      ...
      RefLev  = newRefLev;
      MeshDen = newMeshDen;
      Mark    = new int[Ne]; // reallocate the marker array for the new mesh
   }
}
...
//
/////////////////////////
void US_LammAstfvm::Mesh::Refine( const double beta )
{
   while ( true )
   {
      ...
      for ( int k = 0; k < Ne - 1; k++ )
      {
         ...
         const int refinement_level_difference = RefLev[k] - RefLev[k + 1];

         if ( refinement_level_difference < ( -1 ) )
         {
            Mark[k] = 1;
         }
         else if ( refinement_level_difference > 1 )
         {
            Mark[k + 1] = 1;
         }
      }
      ...
      if ( Ne1 == Ne )
      {
         return; // no more elements need refining
      }
      ...
      for ( int k = 0; k < Ne; k++ )
      {
         if ( Mark[k] == 0 )
         {
            ...
            newX[ke + 1]   = x[k + 1];
            newEid[ke]     = Eid[k];
            newRefLev[ke]  = RefLev[k];
            newMeshDen[ke] = MeshDen[k];
            ke++;
         }
         else
         {
            // Refine: split the element into two.
            const double mid = ( x[k] + x[k + 1] ) * 0.5;
            newX[ke + 1]     = mid;
            newEid[ke]       = Eid[k] * 2;
            newRefLev[ke]    = RefLev[k] + 1;
            newMeshDen[ke]   = MeshDen[k];

            newX[ke + 2]       = x[k + 1];
            newEid[ke + 1]     = newEid[ke] + 1;
            newRefLev[ke + 1]  = newRefLev[ke];
            newMeshDen[ke + 1] = MeshDen[k];
            ke += 2;
         }
      }
      // Free old mesh arrays.
      delete[] x;
      delete[] Eid;
      delete[] RefLev;
      delete[] MeshDen;
...
void US_LammAstfvm::Mesh::RefineMesh( const double* u0, const double* u1, const double ErrTol )
{
   ...
   //double beta = pow( ErrTol * 6 / ( 2 * sqrt( 3 ) / 72 ), 1.0 / 3.0 );
   const double beta = 6.0 * std::pow( ErrTol / sqrt3, onethird );
...
void US_LammAstfvm::Mesh::InitMesh( const double s, const double D, const double w2 )
{
   ...
   for ( int i = 0; i < 10; i++ )
   {
      const double t  = ( i + 1 ) * 0.1;
      const double nu = std::pow( nu1, t ) * std::pow( nu0, 1.0 - t );
      // Allocate u0 and u1 for the current iteration.
      const int           size = 2 * Nv - 1;
      std::vector<double> u0( size );
      std::vector<double> u1( size );
      // Fill nodes values.
      for ( int j = 0; j < Nv; j++ )
      {
         const double x2 = x[j] * x[j];
         u0[2 * j]       = std::exp( nu * ( m2 - x2 ) ) * ( nu * nu * m2 );
         u1[2 * j]       = std::exp( nu * ( x2 - b2 ) ) * ( nu * nu * b2 );
      }
      // Fill midpoint values for each element.
      for ( int j = 0; j < Ne; j++ )
      {
         const double mid = ( x[j] + x[j + 1] ) * 0.5;
         const double x2  = mid * mid;
         u0[2 * j + 1]    = std::exp( nu * ( m2 - x2 ) ) * ( nu * nu * m2 );
         u1[2 * j + 1]    = std::exp( nu * ( x2 - b2 ) ) * ( nu * nu * b2 );
      }
      RefineMesh( u0.data(), u1.data(), 1.e-4 );
   }
}
                                     ...
                                     double* __restrict u1, const int* scan_hint ) const
{
   ...
   for ( int j = 1; j < M0; j++ )
   {
      ...
      const double duL           = ( u0[j2 - 2] - 4.0 * u0[j2 - 1] + u0[j2] ) / denominator_L;
   ...
   }
   {
      ...
      ux[M0]                   = ( u0[j2 - 2] - 4.0 * u0[j2 - 1] + u0[j2] ) / ( 2.0 * denominator );
   }
                              ...
                              double*      D_adj, const int* scan_hint = nullptr ) const
{
   ...
   switch ( NonIdealCaseNo )
   {
      ...
      // Initialize density and viscosity arrays with constant values
         ...
         if ( codiff_needed )
         {
            bandFormingGradient->interpolateCCodiff( Nv, x, t, Dens, Visc, scan_hint );
         }
         #ifdef DEBUG
            if (dbg_level > 2){
