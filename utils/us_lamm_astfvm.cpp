...
void US_LammAstfvm::Mesh::Unrefine(double alpha) {
   ...
   while (1) {
      ...
      for (i = 0; i < Ne - 1; i++) {
         if (
            RefLev[ i ] == RefLev[ i + 1 ] && (Eid[ i ] / 2) == (Eid[ i + 1 ] / 2)
...
US_LammAstfvm::SaltData::SaltData(US_Model amodel, US_SimulationParameters asparms, US_DataIO::RawData *asim_data) {
   dbg_level = US_Settings::us_debug();
   model = amodel;
   simparms = asparms;
   sa_data = *asim_data;
   ...
   simparms.radial_resolution = (sa_data.radius(Nx - 1) - sa_data.radius(0)) / ( double ) (Nx - 1);
...
void US_LammAstfvm::SaltData::InterpolateCSalt(int N, double *x, double t, double *Csalt) {
   double *tmp;
   DbgLv(2) << "SaltD:ntrp:  N t t1 Nt Nx" << N << t << t1 << Nt << Nx << "Cs0N Cs1N" << Cs0[ Nx - 1 ] << Cs1[ Nx - 1 ];
   ...
   for (int j = 0; j < N; j++) // loop for all x[m]
   {
      ...
      Csalt[ j ] = et0 * (xim * Cs0[ m ] + xik * Cs0[ k ]) + et1 * (xim * Cs1[ m ] + xik * Cs1[ k ]);
      //*DEBUG
      double csj = Csalt[ j ];
      if (csj != csj) {
         knan++;
         if (knan < 20)
            DbgLv(2) << "SaltD:ntrp: j,k,xj,xsm,xsk" << j << k << xj << xs[ m ] << xs[ k ] << "xim xik" << xim << xik
                     << "Csaltj" << Csalt[ j ] << csj;
      }
      //*DEBUG
   }

   DbgLv(2) << "SaltD:ntrp:  scn" << scn << "N Nx" << N << Nx << "Csalt0 CsaltM CsaltN" << Csalt[ 0 ] << Csalt[ N / 2 ]
            << Csalt[ N - 1 ];
...
US_LammAstfvm::US_LammAstfvm(US_Model &rmodel, US_SimulationParameters &rsimparms, QObject *parent /*=0*/) :
...
int US_LammAstfvm::solve_component(int compx) {
   ...
   if (NonIdealCaseNo == 2) { // co-sedimenting
      if (comp_x == model.coSedSolute) { // if this component is the salt, skip solving for it
         if (comp_x == 0) {
      ...
      else if (compx > model.coSedSolute) { // if beyond the salt component, adjust step count
         istep -= nts;
      }
   }
   ...
   for (jt = 0, kt = 0; jt < ntc; jt++) {
      ...
      if (ts >= t0 && ts <= t1) { // interpolate concentrations quadratically in x; linearly in time
...
#ifndef NO_DB
         if (((kt / ktinc) * ktinc) == kt || (kt + 1) == nts) { // signal progress at every "ktinc'th" scan or final one
...
void US_LammAstfvm::set_buffer(US_Buffer buffer) {
   ...
   DbgLv(2) << "buff d_coeff" << d_coeff[ 0 ] << d_coeff[ 1 ] << d_coeff[ 2 ] << d_coeff[ 3 ] << d_coeff[ 4 ]
            << d_coeff[ 5 ];
   DbgLv(2) << "buff v_coeff" << v_coeff[ 0 ] << v_coeff[ 1 ] << v_coeff[ 2 ] << v_coeff[ 3 ] << v_coeff[ 4 ]
            << v_coeff[ 5 ];
}
...
//
///////////////////////////////////////////////////////////////
void US_LammAstfvm::LammStepSedDiff_C(
   double t, double dt, int M0, double *x0, double *u0, int M1, double *x1, double *u1p, double *u1) {
   ...
   for (int j = 0; j < Ng; j++) {
      ...
      if ((xg0[ j ] <= (param_m + 1.e-14)) || (xg0[ j ] >= (param_b - 1.e-14))) {
         ...
         flux_u[ j ] = -(Sv[ j ] * param_w2 * xg0[ j ] + Dv[ j ] / xg0[ j ]) * ug0[ j ] + Dv[ j ] * uxs
                       + xt[ j ] * ug0[ j ];
      }
   }
...
//
///////////////////////////////////////////////////////////////
void US_LammAstfvm::AdjustSD(double t, int Nv, double *x, double *u, double *s_adj, double *D_adj) {
      ...
      case 2: // co-sedimenting
         ...
         kst1 += timer.restart();
         {
            ...
            for (jj = 0; jj < Nv; jj++) {
               ...
               rho = d_coeff[ 0 ] + Cmrt * d_coeff[ 1 ] + Cm * d_coeff[ 2 ] + Cmsq * d_coeff[ 3 ] + Cmcu * d_coeff[ 4 ]
                     + Cmqu * d_coeff[ 5 ];

               visc = v_coeff[ 0 ] + Cmrt * v_coeff[ 1 ] + Cm * v_coeff[ 2 ] + Cmsq * v_coeff[ 3 ] + Cmcu * v_coeff[ 4 ]
                      + Cmqu * v_coeff[ 5 ];
               visc = qAbs(visc);
...
int US_LammAstfvm::nonIdealCaseNo() {
   ...
   if (model.coSedSolute >= 0) { // co-sedimentation solute index not -1:  co-sedimenting
      if (NonIdealCaseNo != 0)
         rc = 2;
