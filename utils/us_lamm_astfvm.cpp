...
/////////////////////////
void
US_LammAstfvm::Mesh::ComputeMeshDen_D3 (double *u0, double *u1)
{
  int i;
  int i2;
  double h;
  ...
  for (i = 0; i < Ne; i++)
    {
      MeshDen[i]
          = pow (1 + MonScale * (fabs (D20[i]) + fabs (D21[i])), 0.33333);
...
////////////////////////////////////////////////////////////////////////////
void
US_LammAstfvm::Mesh::Smoothing (int n, double *y, double Wt, int Cycle)
{
  int s;
  int i;
  double Wt1;
...
/////////////////////////
void
US_LammAstfvm::Mesh::Unrefine (double alpha)
{
  int i;
  int i1;
  int Nv1;
...
/////////////////////////
void
US_LammAstfvm::Mesh::Refine (double beta)
{
  int k;
  int ke;
  int Nv1;
...
/////////////////////////
void
US_LammAstfvm::Mesh::RefineMesh (double *u0, double *u1, double ErrTol)
{
...
/////////////////////////
void
US_LammAstfvm::Mesh::InitMesh (double s, double D, double w2)
{
  int j;
  double D0;
  double nu0;
                                   ...
                                   US_DataIO::RawData *asim_data)
{
  ...
  DbgLv (2) << "SaltD: asim_data avg.temp."
            << asim_data->average_temperature ();
  ...
  DbgLv (2) << "SaltD:fem: m b  s D  rpm" << simparms.meniscus
            << simparms.bottom << model.components[0].s
            << model.components[0].D << simparms.speed_step[0].rotorspeed;
  DbgLv (2) << "SaltD: (0)Nt Nx" << Nt << Nx << "temperature"
            << sa_data.scanData[0].temperature;
  if (dbg_level > 0)
    {
      qDebug () << "SaltD: model";
      model.debug ();
  ...
  DbgLv (2) << "SaltD: (2)Nx" << Nx << "r0 r1 rm rn" << sa_data.radius (0)
            << sa_data.radius (1) << sa_data.radius (Nx - 2)
            << sa_data.radius (Nx - 1);
  ...
  DbgLv (2) << "SaltD: (3)Nx" << Nx << "r0 r1 rm rn" << sa_data.radius (0)
            << sa_data.radius (1) << sa_data.radius (Nx - 2)
            << sa_data.radius (Nx - 1);
  DbgLv (2) << "SaltD: Nt Nx" << Nt << Nx << "temp"
            << sa_data.scanData[0].temperature;
      ...
      for (int jj = 0; jj < Nx; jj++)
        {
          ...
          if (ii < 2 && (jj + 3) > Nx)
            DbgLv (2) << "SaltD:  ii jj" << ii << jj << "saltv salta" << saltv
                      << salta;
          ...
          if (saltv != saltv)
            { // Nan!!!
              ...
              DbgLv (2) << "SaltD:  salt *NaN* ii jj adj-saltv" << ii << jj
                        << saltv;
            }
  ...
  DbgLv (2) << "SaltD:  Nx" << Nx << "xs sme" << xs[0] << xs[1] << xs[2]
            << xs[Nx / 2 - 1] << xs[Nx / 2] << xs[Nx - 2 + 1] << xs[Nx - 3]
            << xs[Nx - 2] << xs[Nx - 1];
};
...
void
US_LammAstfvm::SaltData::initSalt ()
{
...
void
US_LammAstfvm::SaltData::InterpolateCSalt (int N, double *x, double t,
                                           double *Csalt)
{
  double *tmp;
  ...
  while ((t1 < t) && (Nt > 0))
    { // walk down salt scans until we are straddling desired time value
      ...
      scn++;
      DbgLv (3) << "SaltD:ntrp:      0 t 1" << t0 << t << t1 << "  N s" << Nt
                << scn;
    }
  DbgLv (2) << "SaltD:ntrp:   t0 t t1" << t0 << t << t1 << "  Nt scn" << Nt
            << scn;
      ...
      if (csj != csj)
        {
          knan++;
          if (knan < 20)
            DbgLv (2) << "SaltD:ntrp: j,k,xj,xsm,xsk" << j << k << xj << xs[m]
                      << xs[k] << "xim xik" << xim << xik << "Csaltj"
                      << Csalt[j] << csj;
        }
      //*DEBUG
    }
  ...
  DbgLv (2) << "SaltD:ntrp:  scn" << scn << "N Nx" << N << Nx
            << "Csalt0 CsaltM CsaltN" << Csalt[0] << Csalt[N / 2]
            << Csalt[N - 1];
  DbgLv (2) << "SaltD:ntrp:    Cs00 Cs0M Cs0N" << Cs0[0] << Cs0[Nx / 2]
            << Cs0[Nx - 1];
  DbgLv (2) << "SaltD:ntrp:    Cs10 Cs1M Cs1N" << Cs1[0] << Cs1[Nx / 2]
            << Cs1[Nx - 1];
}
    ...
    : QObject (parent), model (rmodel), simparams (rsimparms)
{
  ...
  DbgLv (2) << "LFvm: m b w2 strtch" << param_m << param_b << param_w2
            << stretch;
  param_m -= stretch;
  param_b += stretch;
...
int
US_LammAstfvm::calculate (US_DataIO::RawData &sim_data)
{
  auc_data = &sim_data;
...
int
US_LammAstfvm::solve_component (int compx)
{
  comp_x = compx;
  ...
  double dt = log (param_b / param_m) / (param_w2 * param_s * 100.0);
  ...
  DbgLv (2) << "LAsc:  CX=" << comp_x << "  ntcc ntc nts ncs nicase" << ntcc
            << ntc << nts << ncs << NonIdealCaseNo;
  DbgLv (2) << "LAsc:    tot_t dt sol_t" << total_t << dt << solut_t;
  DbgLv (2) << "LAsc:     m b s w2" << param_m << param_b << param_s
            << param_w2;
  ...
  DbgLv (2) << "LAsc:  u0 0,1,2...,N" << u0[0] << u0[1] << u0[2] << u0[N0u - 3]
            << u0[N0u - 2] << u0[N0u - 1];
  ...
  DbgLv (2) << "LAsc:  r0 rn ncs rsiz" << rads[0] << rads[ncs - 1] << ncs
            << af_data.radius.size () << rads.size ();
  ...
  for (jt = 0, kt = 0; jt < ntc; jt++)
    {
      ...
      if (ts >= t0 && ts <= t1)
        { // interpolate concentrations quadratically in x; linearly in time
          ...
          DbgLv (2) << "LAsc:  x0[0] x0[H] x0[N]" << x0[0] << x0[N0 / 2]
                    << x0[N0 - 1];
          DbgLv (2) << "LAsc:  x1[0] x1[H] x1[N]" << x1[0] << x1[N1 / 2]
                    << x1[N1 - 1];
          DbgLv (2) << "LAsc:   r[0]  r[H]  r[N]" << rads[0] << rads[ncs / 2]
                    << rads[ncs - 1];
          DbgLv (2) << "LAsc:  u0[0] u0[H] u0[N]" << u0[0] << u0[N0u / 2]
                    << u0[N1u - 1];
          DbgLv (2) << "LAsc:  u1[0] u1[H] u1[N]" << u1[0] << u1[N1u / 2]
                    << u1[N1u - 1];
          DbgLv (2) << "LAsc:  c0[0] c0[H] c0[N]" << conc0[0] << conc0[ncs / 2]
                    << conc0[ncs - 1];
          DbgLv (2) << "LAsc:  c1[0] c1[H] c1[N]" << conc1[0] << conc1[ncs / 2]
                    << conc1[ncs - 1];
          ...
          DbgLv (2) << "LAsc:   co[0] co[H] co[N]  kt"
                    << af_data.scan[kt].conc[0]
                    << af_data.scan[kt].conc[ncs / 2]
                    << af_data.scan[kt].conc[ncs - 1] << kt;
          ...
          for (int jj = 1; jj < af_data.scan[ii].conc.size (); jj++)
            {
              ...
              //  DbgLv(3) << "   jj cval dltr csum" << jj << cval << dltr <<
              //  csum;
            }
  ...
  DbgLv (2) << "compx" << comp_x << "times 1-6" << ktime1 << ktime2 << ktime3
            << ktime4 << ktime5 << ktime6;
  return 0;
}

void
US_LammAstfvm::set_buffer (US_Buffer buffer)
{
...
void
US_LammAstfvm::SetNonIdealCase_1 (double sigma_k, double delta_k)
{
...
void
US_LammAstfvm::SetNonIdealCase_2 ()
{
  if (comp_x == 0)
    {
...
void
US_LammAstfvm::SetNonIdealCase_3 (int &mropt, double &err_tol)
{
  mropt = 0;
  err_tol = 1.0e-5;
}
...
void
US_LammAstfvm::SetMeshSpeedFactor (double speed)
{
  MeshSpeedFactor = speed;
}

void
US_LammAstfvm::SetMeshRefineOpt (int Opt)
{
  MeshRefineOpt = Opt;
}
...
///////////////////////////////////////////////////////////////
void
US_LammAstfvm::LammStepSedDiff_P (double t, double dt, int M0, double *x0,
                                  double *u0, double *u1)
{
...
///////////////////////////////////////////////////////////////
void
US_LammAstfvm::LammStepSedDiff_C (double t, double dt, int M0, double *x0,
                                  double *u0, int M1, double *x1, double *u1p,
                                  double *u1)
{
  ...
  DbgLv (2) << "  xg1 0 1 M Nm N" << xg1[0] << xg1[1] << xg1[Ng / 2]
            << xg1[Ng - 2] << xg1[Ng - 1];
  DbgLv (2) << "  Sv 0 1 M Nm N" << Sv[0] << Sv[1] << Sv[Ng / 2] << Sv[Ng - 2]
            << Sv[Ng - 1];
  ...
  for (int j = 0; j < Ng; j++)
    {
      ...
      xg0[j]
          = xg1[j]
            - dt * MeshSpeedFactor * sw2 * xg1[j] * exp (-qAbs (sw2) * dt / 2);
  ...
  for (int j = 0; j < Ng; j++)
    {
      ...
      for (int jm = 0; jm < 3; jm++) // at xl
        {
          flux_p[jm][j]
              = (xt[j] - Sv[j] * param_w2 * xg1[j] - Dv[j] / xg1[j]) * phiL[jm]
                + Dv[j] * phiL[jm + 3] / h;
        }
      j++;
      ...
      for (int jm = 0; jm < 3; jm++) // at xr
        {
          flux_p[jm][j]
              = (xt[j] - Sv[j] * param_w2 * xg1[j] - Dv[j] / xg1[j]) * phiR[jm]
                + Dv[j] * phiR[jm + 3] / h;
        }
    }
  ...
  for (int j = 1; j < M0; j++) // internal nodes
    {
      int j2 = 2 * j;
      ux[j]
          = ((u0[j2 - 2] - 4. * u0[j2 - 1] + 3. * u0[j2]) / (x0[j] - x0[j - 1])
  ...
  ux[M0] = (u0[j2 - 2] - 4. * u0[j2 - 1] + 3. * u0[j2]) / (x0[M0] - x0[M0 - 1])
           / 2.;
  ...
  delete[] ke;
  delete[] MemDouble;
  DbgLv (2) << " Diff_C times 1-8" << ktim1 << ktim2 << ktim3 << ktim4 << ktim5
            << ktim6 << ktim7 << ktim8;
}
...
void
US_LammAstfvm::LocateStar (int N0, double *x0, int Ns, double *xs, int *ke,
                           double *xi)
{
  int eix = 1;
...
///////////////////////////////////////////////////////////////
void
US_LammAstfvm::AdjustSD (double t, int Nv, double *x, double *u, double *s_adj,
                         double *D_adj)
{
  ...
  switch (NonIdealCaseNo)
    ...
    case 2: // co-sedimenting
      ...
      kst1 += timer.restart ();
      {
        ...
        for (jj = 0; jj < Nv; jj++)
          {
            ...
            rho = d_coeff[0] + Cmrt * d_coeff[1] + Cm * d_coeff[2]
                  + Cmsq * d_coeff[3] + Cmcu * d_coeff[4] + Cmqu * d_coeff[5];

            visc = v_coeff[0] + Cmrt * v_coeff[1] + Cm * v_coeff[2]
                   + Cmsq * v_coeff[3] + Cmcu * v_coeff[4] + Cmqu * v_coeff[5];
            visc = qAbs (visc);
        ...
        DbgLv (2) << "AdjSD:   Csalt0 CsaltN Cm" << Csalt[0] << Csalt[Nv - 1]
                  << Cm;
        DbgLv (2) << "AdjSD:    sadj 0 m n" << s_adj[0] << s_adj[Nv / 2]
                  << s_adj[Nv - 1];
        DbgLv (2) << "AdjSD:    Dadj 0 m n" << D_adj[0] << D_adj[Nv / 2]
                  << D_adj[Nv - 1];
        DbgLv (2) << "AdjSD:     rho 0,m,e" << rho0 << rhom << rhoe;
        DbgLv (2) << "AdjSD:     visc 0,m,e" << vis0 << vism << vise;
        DbgLv (2) << "AdjSD:     Cm   0,m,e" << cms0 << cmsm << cmse;
        DbgLv (2) << "AdjSD:      vbar vbar_w rho_w" << vbar << vbar_w
                  << rho_w;
        DbgLv (2) << "AdjSD:       cmrt cm^2 cm^3 cm^4" << Cmrt << Cmsq << Cmcu
                  << Cmqu;
    ...
    case 3: // compressibility
      {
        ...
        DbgLv (3) << "AdjSD:   sadj 0 m n" << s_adj[0] << s_adj[Nv / 2]
                  << s_adj[Nv - 1];
        DbgLv (3) << "AdjSD:   Dadj 0 m n" << D_adj[0] << D_adj[Nv / 2]
                  << D_adj[Nv - 1];
      }
      break;
...
void
US_LammAstfvm::fun_phi (double x, double *y)
{
  double x2 = x * x;
  y[0] = 0.5 * (x2 - x);
  y[1] = 1.0 - x2;
...
void
US_LammAstfvm::fun_dphi (double x, double *y)
{
  // quadratic basis
  double x2 = x * x;
  y[0] = 0.5 * (x2 - x);
...
void
US_LammAstfvm::fun_Iphi (double x, double *y)
{
  double x2 = x * x;
  double x3 = x * x2 / 6.0;
  x2 *= 0.25;
...
////////////////////////////////////////////
double
US_LammAstfvm::IntQ (double *x, double *u, double xia, double xib)
{
  double intgrl;
  double phia[3];
  double phib[3];
  ...
  intgrl = (x[1] - x[0]) / 2.
           * (u[0] * (phib[0] - phia[0]) + u[1] * (phib[1] - phia[1])
              + u[2] * (phib[2] - phia[2]));
...
//////////////////////////////////////////////////////////////////
double
US_LammAstfvm::IntQs (double *x, double *u, int ka, double xia, int kb,
                      double xib)
{
  double intgrl;
      ...
      for (int k = ka + 1; k <= kb - 1; k++)
        {
          int k2 = k + k;
          intgrl
              += (x[k + 1] - x[k]) * (u[k2] + u[k2 + 2] + 4. * u[k2 + 1]) / 6.;
        }
    }
...
/////////////////////////////////////////////////////////////////
void
US_LammAstfvm::ProjectQ (int M0, double *x0, double *u0, int M1, double *x1,
                         double *u1)
{
  double intgrl;
  double phi[3];
  ...
  for (int j = 0; j <= M1; j++)
    {
      ...
      int idx = 2 * ke[j];
      u1[2 * j]
          = phi[0] * u0[idx] + phi[1] * u0[idx + 1] + phi[2] * u0[idx + 2];
    }
  ...
  for (int j = 0; j < M1; j++)
    {
      ...
      u1[j2 + 1]
          = 1.5 * intgrl / (x1[j + 1] - x1[j]) - 0.25 * (u1[j2] + u1[j2 + 2]);
    }
...
///////////////////////////////////////////////////////
void
US_LammAstfvm::LsSolver53 (int m, double **A, double *b, double *x)
{
...
void
US_LammAstfvm::quadInterpolate (double *x0, double *u0, int N0,
...
void
US_LammAstfvm::load_mfem_data (US_DataIO::RawData &edata,
                               US_AstfemMath::MfemData &fdata, bool zeroout)
{
  ...
  for (int ii = 0; ii < nscan; ii++)
   ...
   {  // copy all radius values
      ...
      if (zeroout)
        fscan->conc.fill (
            0.0, nconc); // if so specified, set concentrations to zero
  ...
  int mm = nn / 2;
  DbgLv (2) << "LdDa:  n r0 rm rn" << nn << fdata.radius[0] << fdata.radius[mm]
            << fdata.radius[nn];
#endif
}
...
void
US_LammAstfvm::store_mfem_data (US_DataIO::RawData &edata,
...
void
US_LammAstfvm::setStopFlag (bool flag)
{
  stopFlag = flag;
  qApp->processEvents ();
...
void
US_LammAstfvm::setMovieFlag (bool flag)
{
  movieFlag = flag;
  qApp->processEvents ();
