...
US_LammAstfvm::SaltData::SaltData(US_Model amodel,
  ...
  for (int ii = 0; ii < Nt; ii++)
    for (int jj = 0; jj < Nx; jj++) sa_data.setValue(ii, jj, 0.0);
  ...
  if (dbg_level >
      2) {  // save a copy of the salt data set so that it may be plotted for QC
...
void US_LammAstfvm::SaltData::initSalt() {
  ...
  for (int j = 0; j < Nx;
       j++) {  // get 1st two salt arrays from 1st two salt scans
...
void US_LammAstfvm::SaltData::InterpolateCSalt(int N, double* x, double t,
  ...
  while (
      (t1 < t) &&
      (Nt >
       0)) {  // walk down salt scans until we are straddling desired time value
    t0 = t1;
    tmp = Cs0;
    Cs0 = Cs1;
    ...
    for (int j = 0; j < Nx; j++) Cs1[j] = sa_data.value(scn, j);
    ...
    if (csj != csj) {
      ...
      if (knan < 20)
        DbgLv(2) << "SaltD:ntrp: j,k,xj,xsm,xsk" << j << k << xj << xs[m]
                 << xs[k] << "xim xik" << xim << xik << "Csaltj" << Csalt[j]
                 << csj;
    }
    //*DEBUG
  }
...
int US_LammAstfvm::calculate(US_DataIO::RawData& sim_data) {
  ...
  if (model.coSedSolute >=
      0) {  // for co-sedimenting, reduce total steps by one scan
...
int US_LammAstfvm::solve_component(int compx) {
  ...
  if (NonIdealCaseNo ==
      3) {  // compressibility:  8-fold smaller delta-t and greater time points
    dt /= 8.0;
  ...
  if (NonIdealCaseNo == 2) {            // co-sedimenting
    if (comp_x == model.coSedSolute) {  // if this component is the salt, skip
                                        // solving for it
      if (comp_x == 0) {
    ...
    else if (compx > model.coSedSolute) {  // if beyond the salt component,
                                           // adjust step count
      istep -= nts;
    }
  }
  ...
  if (nts > 10000) ktinc = qMax(2, ((nts + 5000) / 10000));
  if (nts < 100) ktinc = 1;
  ...
  for (jt = 0, kt = 0; jt < ntc; jt++) {
    ...
    if (dbg_level > 0 && ((jt / 10) * 10) == jt) {
      ...
      for (int j = 0; j < N0; j++) tso << QString().sprintf("%10.6e \n", x0[j]);
      tso << QString().sprintf("\n");
      for (int j = 0; j < N0u; j++)
    ...
    if (MeshRefineOpt == 1) {
      ...
      for (int jj = 0; jj < N1; jj++) x1[jj] = msh->x[jj];
    ...
    if (ts >= t0 && ts <= t1) {  // interpolate concentrations quadratically in
                                 // x; linearly in time
      ...
      // f0=1.0; f1=0.0;
      for (int jj = 0; jj < ncs; jj++) {  // update concentration vector with
      ...
      if (((kt / ktinc) * ktinc) == kt ||
          (kt + 1) ==
              nts) {  // signal progress at every "ktinc'th" scan or final one
    ...
    if (kt >= nts) break;  // if all scans updated, we are done
  ...
  if (dbg_level > 0) ftto.close();

  if (dbg_level > 0) {  // calculate and print the integral of scan curves
    double cimn = 9e+14;
    double cimx = 0.0;
    double ciav = 0.0;
...
void US_LammAstfvm::SetMeshRefineOpt(int Opt) { MeshRefineOpt = Opt; }
...
//
///////////////////////////////////////////////////////////////
void US_LammAstfvm::LammStepSedDiff_P(double t, double dt, int M0, double* x0,
                                      double* u0, double* u1) {
...
//
///////////////////////////////////////////////////////////////
void US_LammAstfvm::LammStepSedDiff_C(double t, double dt, int M0, double* x0,
                                      double* u0, int M1, double* x1,
                                      double* u1p, double* u1) {
  ...
  for (int j = 0; j < Ng; j++) {
    ...
    xg0[j] =
        xg1[j] - dt * MeshSpeedFactor * sw2 * xg1[j] * exp(-qAbs(sw2) * dt / 2);
  ...
  for (int j = 1; j < M0; j++)  // internal nodes
  {
    ...
    ux[j] =
        ((u0[j2 - 2] - 4. * u0[j2 - 1] + 3. * u0[j2]) / (x0[j] - x0[j - 1]) -
         (3. * u0[j2] - 4. * u0[j2 + 1] + u0[j2 + 2]) / (x0[j + 1] - x0[j])) /
        2.;
  }
  ...
  ux[M0] =
      (u0[j2 - 2] - 4. * u0[j2 - 1] + 3. * u0[j2]) / (x0[M0] - x0[M0 - 1]) / 2.;
  ...
  for (int i = 0; i <= Ng; i++) Mtx[i] = new double[5];
  ...
  i = Ng;
  rhs[i] =
      IntQs(x0, u0, ke[i - 1], xi[i - 1], M0 - 1, 1.) + dt2 * (-flux_u[i - 1]);

  ktim7 += timer.restart();
  LsSolver53(Ng, Mtx, rhs, u1);
  ktim8 += timer.restart();

  for (int i = 0; i <= Ng; i++) delete[] Mtx[i];
...
void US_LammAstfvm::LocateStar(int N0, double* x0, int Ns, double* xs, int* ke,
                               double* xi) {
  int eix = 1;
...
//
//////////////////////////////////////////////////////////////////
double US_LammAstfvm::IntQs(double* x, double* u, int ka, double xia, int kb,
                            double xib) {
  double intgrl;
...
//
/////////////////////////////////////////////////////////////////
void US_LammAstfvm::ProjectQ(int M0, double* x0, double* u0, int M1, double* x1,
                             double* u1) {
  double intgrl;
  double phi[3];
...
int US_LammAstfvm::nonIdealCaseNo() {
  ...
  if (sc->sigma != 0.0 ||
      sc->delta !=
          0.0) {  // non-zero sigma or delta given: concentration-dependent
    NonIdealCaseNo = 1;
  }

  if (model.coSedSolute >=
      0) {  // co-sedimentation solute index not -1:  co-sedimenting
    if (NonIdealCaseNo != 0) rc = 2;

    NonIdealCaseNo = 2;
  }

  if (compressib > 0.0) {  // compressibility factor positive: compressibility
    if (NonIdealCaseNo != 0) rc = 3;
...
void US_LammAstfvm::quadInterpolate(double* x0, double* u0, int N0,
  ...
  while (kk < nout) {  // loop to output interpolated concentrations
    ...
    while (xv > x3 &&
           ii < N0) {  // if need be, walk up input until between x values
...
void US_LammAstfvm::load_mfem_data(US_DataIO::RawData& edata,
  ...
  for (int ii = 0; ii < nscan; ii++) {  // copy over all scans
   ...
   {  // copy all radius values
    ...
    if (zeroout)
      fscan->conc.fill(0.0,
                       nconc);  // if so specified, set concentrations to zero
