...
bool
US_Analyte::operator== (const US_Analyte &a) const
{
  if (vbar20 != a.vbar20)
    return false;
  if (mw != a.mw)
  ...
  if (description != a.description)
    return false;
  if (analyteGUID != a.analyteGUID)
    return false;
  if (sequence != a.sequence)
    return false;
  if (type != a.type)
    return false;
  if (extinction != a.extinction)
    return false;
  if (refraction != a.refraction)
    return false;
  if (fluorescence != a.fluorescence)
    return false;
  if (grad_form != a.grad_form)
    return false;

  if (type == DNA || type == RNA)
    {
      if (doubleStranded != a.doubleStranded)
        return false;
      if (complement != a.complement)
        return false;
      if (_3prime != a._3prime)
        return false;
      if (_5prime != a._5prime)
        return false;
      if (sodium != a.sodium)
        return false;
      if (potassium != a.potassium)
        return false;
      if (lithium != a.lithium)
        return false;
      if (magnesium != a.magnesium)
        return false;
      if (calcium != a.calcium)
        return false;
    }
...
int
US_Analyte::load (bool db_access, const QString &guid, US_DB2 *db)
{
  if (db_access)
...
int
US_Analyte::load_db (const QString &load_guid, US_DB2 *db)
{
  int error = US_DB2::OK;
...
int
US_Analyte::load_disk (const QString &guid)
{
...
int
US_Analyte::read_analyte (const QString &filename)
{
  QFile file (filename);
              ...
              if (type == PROTEIN)
                {
                  ...
                  // The sequence tag comes before the extinction extinction
                  // tag so a value set there will override this setting, if it
                  // exists.  It's not the way xml is really supposed work, but
                ...
                {
                  doubleStranded
                      = US_Util::bool_flag (a.value ("stranded").toString ());
...
bool
US_Analyte::analyte_path (QString &path)
{
  QDir dir;
...
int
US_Analyte::write (bool db_access, const QString &filename, US_DB2 *db)
{
  if (db_access)
    return write_db (db);
  else
...
int
US_Analyte::write_disk (const QString &filename)
{
  QFile file (filename);
...
void
US_Analyte::set_spectrum (US_DB2 *db)
{
  QStringList q;
  ...
  if ((kntext != cntext) || (kntref != cntref) || (kntflu != cntflu))
    {
      qDebug () << "set_spectrum *ERROR* ext k c" << kntext << cntext
                << "ref k c" << kntref << cntref << "flu k c" << kntflu
                << cntflu;
    }
}

int
US_Analyte::write_db (US_DB2 *db)
{
      ...
      if (!replace_spectrum)
        {
          ...
          US_ExtProfile::create_eprofile (db, analyteID.toInt (), compType,
                                          valType, extinction);
        }
      else
        {
          ...
          // << ", ProfileID: " << profileID; US_ExtProfile::update_eprofile(
          // db, profileID, analyteID.toInt(), compType, valType,
          // new_extinction); qDebug() << "New Extinction keys: " <<
          // new_extinction.keys().count() << ", ProfileID: " << profileID;
          ...
          if (profileID == -1)
            ...
            {
              qDebug () << "Old Extinction keys: "
                        << extinction.keys ().count ()
                        << ", ProfileID: " << profileID;
              US_ExtProfile::update_eprofile (db, profileID,
                                              analyteID.toInt (), compType,
...
QString
US_Analyte::get_filename (const QString &path, const QString &guid)
{
  return US_DataFiles::get_filename (path, guid, "A", "analyte",
                                     "analyteGUID");
}

void
US_Analyte::write_nucleotide (US_DB2 *db)
{
  QStringList q;
