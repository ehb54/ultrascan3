...
#define DEBUG_QUERY \
   qDebug() << "Q" << q << "Err" << db->lastErrno() \
            << db->lastError(); //!< Debug print showing DB query string and error returns
...
int US_Analyte::load_db(const QString &load_guid, US_DB2 *db) {
   ...
   grad_form = (type == CARBOHYDRATE) ? US_Util::bool_flag(db->value(7).toString()) : false;
      ...
      if (xml.isStartElement()) {
         if (xml.name() == "analyte") {
            ...
            if (type == PROTEIN) {
               ...
               complement = US_Util::bool_flag(a.value("complement_only").toString());
               _3prime = US_Util::bool_flag(a.value("ThreePrimeHydroxyl").toString());
...
void US_Analyte::set_spectrum(US_DB2 *db) {
   ...
   if ((kntext != cntext) || (kntref != cntref) || (kntflu != cntflu)) {
      qDebug() << "set_spectrum *ERROR* ext k c" << kntext << cntext << "ref k c" << kntref << cntref << "flu k c"
               << kntflu << cntflu;
   }
}
