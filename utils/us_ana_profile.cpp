...
bool US_AnaProfile::toXml(QXmlStreamWriter& xmlo) {
  ...
  */
  ...
  qDebug() << "Sizes of lc_ratios, lc_tolers, l_volumes, lv_tolers, data_ends, "
              "analysis_run -- "
           << lc_ratios.count() << lc_tolers.count() << l_volumes.count()
           << lv_tolers.count() << data_ends.count() << analysis_run.count();
  ...
  for (int ii = 0; ii < pchans.count(); ii++) {
    ...
    if (chndescs[ii].contains("B:Interf")) continue;
    ...
    // Affected parms
    xmlo.writeAttribute("load_concen_ratio", QString::number(lc_ratios[kk]));
    ...
    xmlo.writeAttribute("abde_reference", QString::number(ref_channels[kk]));
    ...
    //  ch_report_ids[ chndescs_alt[ kk ] ]   [ i ], where i is for QList < int
    //  >, a list of reportIDs... ch_report_guids[ chndescs_alt[ kk ] ] [ i ]
    ...
    // scan_excl_begin[ ii ] )    ); xmlo.writeAttribute    ( "scan_excl_end",
    // QString::number( scan_excl_end[ ii ] ) );
    ...
    for (wvl_overlap = channdesc_to_overlapping_wvls_main.begin();
         ...
         ++wvl_overlap) {
      qDebug()
          << "In AProfile's toXML:  channdesc_to_overlapping_wvls_main QMap: "
          << wvl_overlap.key()
  ...
  while (!xmli.atEnd()) {
    ...
    if (xmli.isStartElement()) {
      ...
      if (ename == "analysis_profile") {
        ...
        else  // This will not be needed for newer analysis profiles; logic is
        ...
        //  "101,102,103,.." transform this string into QList < int > insert
        //  this QList into ch_report_ids[ channel_alt_desc ]
        ...
        if (attr.hasAttribute("replicate_wvl_overlap")) {
          channdesc_to_overlapping_wvls_main[channel_alt_desc] =
              attr.value("replicate_wvl_overlap").toString().split(",");
          qDebug() << "In AProfile's fromXML: channel_alt_desc:  "
...
bool US_AnaProfile::AnaProf2DSA::operator==(const AnaProf2DSA& ap) const {
  if (nchan != ap.nchan) return false;
    ...
    if (xmli.isStartElement()) {
      if (ename == "channel_parms") {
        ...
        job2run = US_Util::bool_flag(
            attr.value("run").toString());  // ALEXEY; shouldn't it be job2run ?
...
bool US_AnaProfile::AnaProf2DSA::toXml(QXmlStreamWriter& xmlo) {
  ...
  for (int ii = 0; ii < nchan; ii++) {
    ...
    xmlo.writeAttribute("s_gridpoints", QString::number(parms[ii].s_grpts));
    xmlo.writeAttribute("k_min", QString::number(parms[ii].k_min));
    xmlo.writeAttribute("k_max", QString::number(parms[ii].k_max));
    xmlo.writeAttribute("k_gridpoints", QString::number(parms[ii].k_grpts));
    xmlo.writeAttribute("vary_vbar", US_Util::bool_string(parms[ii].varyvbar));
    xmlo.writeAttribute("constant_ff0", QString::number(parms[ii].ff0_const));
  ...
  xmlo.writeEndElement();  // p_2dsa
                           /*
                                   <p_2dsa>
                                       ...
                                       <job_2dsa_fm run="1" noise="both" fit_range="0.03"
...
bool US_AnaProfile::AnaProf2DSA::Parm2DSA::operator==(const Parm2DSA& p) const {
  // bool same      = true;
...
bool US_AnaProfile::AnaProfPCSA::operator==(const AnaProfPCSA& ap) const {
...
QString US_AnaProfile::AnaProfPCSA::curve_type_fromXml(QString curve_param) {
  ...
  if (curve_param.contains("All")) curve_type = "All (IS + DS + SL)";
  if (curve_param.contains("IS")) curve_type = "Increasing Sigmoid";
  if (curve_param.contains("DS")) curve_type = "Decreasing Sigmoid";
  if (curve_param.contains("SL")) curve_type = "Straight Line";
  if (curve_param.contains("HL")) curve_type = "Horizontal Line";
  if (curve_param.contains("2O")) curve_type = "Second Order Power";
...
QString US_AnaProfile::AnaProfPCSA::curve_type_toXml(QString curve_param) {
  ...
  if (curve_param.contains("All")) curve_type = "All";
  if (curve_param.contains("Increasing Sigmoid")) curve_type = "IS";
  if (curve_param.contains("Decreasing Sigmoid")) curve_type = "DS";
  if (curve_param.contains("Straight Line")) curve_type = "SL";
  if (curve_param.contains("Horizontal Line")) curve_type = "HL";
  if (curve_param.contains("Second Order Power")) curve_type = "2O";
...
bool US_AnaProfile::AnaProfPCSA::toXml(QXmlStreamWriter& xmlo) {
  ...
  for (int ii = 0; ii < nchan; ii++) {
    ...
    xmlo.writeAttribute("mc_iterations", QString::number(parms[ii].mc_iters));
    xmlo.writeEndElement();  // channel_parms
  }
  // 3-------------------------------------------------------------------------->80
...
bool US_AnaProfile::AnaProfPCSA::ParmPCSA::operator==(const ParmPCSA& p) const {
  // bool same      = true;
  // same           = ( channel != p.channel ) ? false : same;
