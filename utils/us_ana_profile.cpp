...
bool
US_AnaProfile::operator== (const US_AnaProfile &ap) const
{
  qDebug () << "RP:AnaProfile COMPARISON!! ";

  // General
  if (aprofname != ap.aprofname)
    return false;
  if (aprofGUID != ap.aprofGUID)
    return false;
  if (protoname != ap.protoname)
    return false;
  if (protoGUID != ap.protoGUID)
    return false;
  ...
  if (lc_ratios != ap.lc_ratios)
    return false;
  if (lc_tolers != ap.lc_tolers)
    return false;
  if (l_volumes != ap.l_volumes)
    return false;
  if (lv_tolers != ap.lv_tolers)
    return false;
  if (data_ends != ap.data_ends)
    return false;

  qDebug () << "[ANA comp] Gen3 OK";

  // abde
  if (ld_dens_0s != ap.ld_dens_0s)
    return false;
  if (gm_vbars != ap.gm_vbars)
    return false;
  if (gm_mws != ap.gm_mws)
    return false;
  if (ref_channels != ap.ref_channels)
    return false;
  if (ref_use_channels != ap.ref_use_channels)
    return false;

  if (scan_excl_begin != ap.scan_excl_begin)
    return false;
  if (scan_excl_end != ap.scan_excl_end)
    return false;

  if (analysis_run != ap.analysis_run)
    return false;
  if (report_run != ap.report_run)
    return false;
  if (wvl_edit != ap.wvl_edit)
    return false;
  if (replicates != ap.replicates)
    return false;
  if (wvl_not_run != ap.wvl_not_run)
    return false;

  if (report_mask != ap.report_mask)
    return false;
  if (combPlots_parms != ap.combPlots_parms)
    return false;
...
bool
US_AnaProfile::toXml (QXmlStreamWriter &xmlo)
{
  ...
  */
  ...
  qDebug () << "Sizes of lc_ratios, lc_tolers, l_volumes, lv_tolers, "
               "data_ends, analysis_run -- "
            << lc_ratios.count () << lc_tolers.count () << l_volumes.count ()
            << lv_tolers.count () << data_ends.count ()
            << analysis_run.count ();
  ...
  qDebug () << "Size: pchans(), chndescs.size(),  scan_excl_begin.size(), "
               "scan_excl_end.size()  -- "
            << pchans.count () << chndescs.size () << scan_excl_begin.size ()
            << scan_excl_end.size ();

  for (int ii = 0; ii < pchans.count (); ii++)
    {
      qDebug () << "In toXML AProfile: Ch_desc, scan_beg, scan_end -- "
                << chndescs[ii] << scan_excl_begin[ii] << scan_excl_end[ii];
    }
  // END of DEBUG
  ...
  for (int ii = 0; ii < pchans.count (); ii++)
    {
      ...
      qDebug () << "WRITE: Ch description vs. chndescs_alt: " << chndescs[ii]
                << " vs. " << chndescs_alt[kk];
      ...
      xmlo.writeAttribute ("lcr_tolerance", QString::number (lc_tolers[kk]));
      xmlo.writeAttribute ("load_volume", QString::number (l_volumes[kk]));
      xmlo.writeAttribute ("lv_tolerance", QString::number (lv_tolers[kk]));
      xmlo.writeAttribute ("data_end", QString::number (data_ends[kk]));

      // ABDE parms
      xmlo.writeAttribute ("load_dens", QString::number (ld_dens_0s[kk]));
      xmlo.writeAttribute ("grad_vbar", QString::number (gm_vbars[kk]));
      xmlo.writeAttribute ("grad_mw", QString::number (gm_mws[kk]));

      // ALEXEY: analyse?
      xmlo.writeAttribute ("run", QString::number (analysis_run[kk]));

      // ALEXEY: run report ?
      xmlo.writeAttribute ("run_report", QString::number (report_run[kk]));
      ...
      // ALEXEY: wvl to edit
      xmlo.writeAttribute ("wvl_edit", QString::number (wvl_edit[kk]));
      ...
      //  "report_id" && "report_guid" will now be strings like
      //  "100,101,102,.." ch_report_ids[ chndescs_alt[ kk ] ]   [ i ], where i
      //  is for QList < int >, a list of reportIDs... ch_report_guids[
      //  chndescs_alt[ kk ] ] [ i ]
      ...
      // xmlo.writeAttribute    ( "scan_excl_begin", QString::number(
      // scan_excl_begin[ ii ] )    ); xmlo.writeAttribute    (
      // "scan_excl_end",   QString::number( scan_excl_end[ ii ] ) );

      // scan exclusions: ATTENTION -- Now use kk index, as for all other
      // attributes
      xmlo.writeAttribute ("scan_excl_begin",
                           QString::number (scan_excl_begin[kk]));
      xmlo.writeAttribute ("scan_excl_end",
                           QString::number (scan_excl_end[kk]));

      xmlo.writeAttribute ("replicate_group",
                           QString::number (replicates[kk]));

      // overlapping wvls:
      // Debug:
      QMap<QString, QStringList>::iterator wvl_overlap;
      for (wvl_overlap = channdesc_to_overlapping_wvls_main.begin ();
           wvl_overlap != channdesc_to_overlapping_wvls_main.end ();
           ++wvl_overlap)
        {
          qDebug () << "In AProfile's toXML:  "
                       "channdesc_to_overlapping_wvls_main QMap: "
                    << wvl_overlap.key ()
      ...
      QStringList repl_wvl_overlap
          = channdesc_to_overlapping_wvls_main[chndescs_alt[kk]];
...
bool
US_AnaProfile::fromXml (QXmlStreamReader &xmli)
{
  int chx = 0;
  pchans.clear ();
  chndescs.clear ();
  ...
  while (!xmli.atEnd ())
    {
      xmli.readNext ();
      // qDebug() << "AP:fX: next hasErr Err" << xmli.hasError() <<
      // xmli.errorString()
      //  << xmli.tokenString();
      ...
      if (xmli.isStartElement ())
        {
          ...
          if (ename == "analysis_profile")
          ...
          else if (ename == "channel_parms")
            {
              ...
              lc_ratios
                  << attr.value ("load_concen_ratio").toString ().toDouble ();
              lc_tolers
                  << attr.value ("lcr_tolerance").toString ().toDouble ();
              ...
              // ALEXEY: for now -- put all checked; later will be 'run="1"' OR
              // 'run="0"' field analysis_run << 1;
              analysis_run << attr.value ("run").toString ().toInt ();

              if (attr.hasAttribute ("run_report"))
                report_run << attr.value ("run_report").toString ().toInt ();
              else // This will not be needed for newer analysis profiles;
                   // logic is for backward compatibility..
                {
              ...
              if (attr.hasAttribute ("abde_reference"))
                ref_channels
                    << attr.value ("abde_reference").toString ().toInt ();
              else
                ref_channels << 0;

              if (attr.hasAttribute ("abde_use_reference"))
                ref_use_channels
                    << attr.value ("abde_use_reference").toString ().toInt ();
              else
                ref_use_channels << 0;
              ...
              QString channel_alt_desc
                  = attr.value ("chandesc_alt").toString ();
              chndescs_alt << channel_alt_desc;

              // ALEXEY_NEW_REPORT:
              //  attr.value( "report_id" ).toString() will be something like
              //  "101,102,103,.." transform this string into QList < int >
              //  insert this QList into ch_report_ids[ channel_alt_desc ]

              QString reportIDs_string = attr.value ("report_id").toString ();
              QString reportGIUDs_string
                  = attr.value ("report_guid").toString ();
              ...
              if (attr.hasAttribute ("scan_excl_begin"))
                scan_excl_begin
                    << attr.value ("scan_excl_begin").toString ().toInt ();
              else
                scan_excl_begin << 0;
              if (attr.hasAttribute ("scan_excl_end"))
                scan_excl_end
                    << attr.value ("scan_excl_end").toString ().toInt ();
              else
                scan_excl_end << 0;
              ...
              if (attr.hasAttribute ("replicate_wvl_overlap"))
                {
                  channdesc_to_overlapping_wvls_main[channel_alt_desc]
                      = attr.value ("replicate_wvl_overlap")
                            .toString ()
                            .split (",");
                  qDebug ()
                      << "In AProfile's fromXML: channel_alt_desc:  "
                      << channel_alt_desc << ", replicate_wvl_overlap -- "
                      << attr.value ("replicate_wvl_overlap")
                             .toString ()
                             .split (",");
                }
...
bool
US_AnaProfile::AnaProf2DSA::operator== (const AnaProf2DSA &ap) const
{
  if (nchan != ap.nchan)
    return false;
  ...
  if (fitrng != ap.fitrng)
    return false;
  if (grpoints != ap.grpoints)
    return false;
  if (j2rfiters != ap.j2rfiters)
    return false;
  if (rfiters != ap.rfiters)
    return false;
  if (mciters != ap.mciters)
    return false;
  if (fmb != ap.fmb)
    return false;

  if (job1run != ap.job1run)
    return false;
  if (job2run != ap.job2run)
    return false;
  if (job3run != ap.job3run)
    return false;
  if (job4run != ap.job4run)
    return false;
  if (job5run != ap.job5run)
    return false;
  if (job3auto != ap.job3auto)
    return false;
...
bool
US_AnaProfile::AnaProf2DSA::fromXml (QXmlStreamReader &xmli)
{
  nchan = 0;
  parms.clear ();
  Parm2DSA parm1;
          ...
          if (ename == "channel_parms")
            {
              ...
              int grreps
                  = attr.value ("grid_repetitions").toString ().toInt ();
              parm1.channel = chan;
              parm1.s_min = smin;
              parm1.s_max = smax;
              parm1.s_grpts = sgpts;
              ...
              parm1.have_custg = (!cgrid.isEmpty () && cgrid.length () == 36);
          ...
          else if (ename == "job_2dsa_fm")
            {
              ...
              // job1run        = US_Util::bool_flag( attr.value( "run"
              // ).toString() );
              job2run = US_Util::bool_flag (
                  attr.value ("run")
                      .toString ()); // ALEXEY; shouldn't it be job2run ?
              ...
              // ALEXEY -- check if the following 3 attributes exist (for older
              // ana_profiles): Meniscus Range
              attr_ref = attr.value ("meniscus_range");
              if (!attr_ref.isNull ())
                {
                  fitrng
                      = attr.value ("meniscus_range").toString ().toDouble ();
                  if (fitrng == 0)
                    fitrng = 0.03; // DEFAULT if 0 stored in xml (due old
                                   // ana_profiles)
                }
              else
              ...
              if (!attr_ref.isNull ())
                {
                  grpoints
                      = attr.value ("meniscus_points").toString ().toInt ();
                  if (!grpoints)
                    grpoints = 11; // DEFAULT if 0 stored in xml (due old
                                   // ana_profiles)
                }
              else
              ...
              if (!attr_ref.isNull ())
                {
                  j2rfiters
                      = attr.value ("max_iterations").toString ().toInt ();
                  if (!j2rfiters)
                    j2rfiters = 1; // DEFAULT if 0 stored in xml (due old
                                   // ana_profiles)
                }
              else
...
bool
US_AnaProfile::AnaProf2DSA::toXml (QXmlStreamWriter &xmlo)
{
  ...
  for (int ii = 0; ii < nchan; ii++)
    {
      ...
      xmlo.writeAttribute ("s_min", QString::number (parms[ii].s_min));
      xmlo.writeAttribute ("s_max", QString::number (parms[ii].s_max));
      xmlo.writeAttribute ("s_gridpoints",
                           QString::number (parms[ii].s_grpts));
      xmlo.writeAttribute ("k_min", QString::number (parms[ii].k_min));
      xmlo.writeAttribute ("k_max", QString::number (parms[ii].k_max));
      xmlo.writeAttribute ("k_gridpoints",
                           QString::number (parms[ii].k_grpts));
      xmlo.writeAttribute ("vary_vbar",
                           US_Util::bool_string (parms[ii].varyvbar));
      xmlo.writeAttribute ("constant_ff0",
                           QString::number (parms[ii].ff0_const));
      xmlo.writeAttribute ("custom_grid_guid", parms[ii].cust_grid);

      xmlo.writeEndElement (); // channel_parms
    }
  xmlo.writeStartElement ("job_2dsa");
  xmlo.writeAttribute ("run", US_Util::bool_string (job1run));
  xmlo.writeAttribute ("noise", job1nois);
  xmlo.writeEndElement (); // job_2dsa

  //<job_2dsa_fm run="1" noise="(TI+RI Noise)" fit_mb_select="1"
  //meniscus_range="0.03" meniscus_points="11"/>
  xmlo.writeStartElement ("job_2dsa_fm");
  xmlo.writeAttribute ("run", US_Util::bool_string (job2run));
  ...
  xmlo.writeAttribute ("run", US_Util::bool_string (job4run));
  xmlo.writeAttribute ("noise", job2nois);
  xmlo.writeAttribute ("max_iterations", QString::number (rfiters));
  xmlo.writeEndElement (); // job_2dsa_it

  xmlo.writeStartElement ("job_2dsa_mc");
  xmlo.writeAttribute ("run", US_Util::bool_string (job5run));
  xmlo.writeAttribute ("mc_iterations", QString::number (mciters));
  ...
  xmlo.writeEndElement (); // p_2dsa
                           /*
                                   <p_2dsa>
                                       <channel_parms channel="1A" s_min="1" s_max="10"
                              s_gridpoints="64"                          k_min="1" k_max="4" k_gridpoints="64"                          vary_vbar="false"
                              constant_ff0="2" grid_repetitions="8"                          custom_grid_guid="" />
                                       <channel_parms channel="1B" s_min="1" s_max="10"
                              s_gridpoints="64"                          k_min="1" k_max="4" k_gridpoints="64"                          vary_vbar="false"
                              constant_ff0="2" grid_repetitions="8"                          custom_grid_guid="" />                          <job_2dsa
                              run="1" noise="TI" />                          <job_2dsa_fm run="1" noise="both" fit_range="0.03"
                              grid_points="10" />                          <job_fitmen run="1" interactive="true" />                          <job_2dsa_it
                              run="1" noise="both" max_iterations="3" />                          <job_2dsa_mc run="1"
                              mc_iterations="100" />
                                   </p_2dsa>
                            */
...
bool
US_AnaProfile::AnaProf2DSA::Parm2DSA::operator== (const Parm2DSA &p) const
{
  // bool same      = true;
  ...
  if (k_max != p.k_max)
    return false;
  if (ff0_const != p.ff0_const)
    return false;
  if (s_grpts != p.s_grpts)
    return false;
  if (k_grpts != p.k_grpts)
    return false;
  if (gridreps != p.gridreps)
    return false;

  if (varyvbar != p.varyvbar)
    return false;
...
bool
US_AnaProfile::AnaProfPCSA::operator== (const AnaProfPCSA &ap) const
{
  qDebug () << "RP:AnaProfPCSA: nchan, ap.nchan -- " << nchan << ", "
            << ap.nchan;
  qDebug () << "RP:AnaProfPCSA: job_run, ap.job_run -- " << job_run << ", "
            << ap.job_run;

  if (nchan != ap.nchan)
    return false;
  if (job_run != ap.job_run)
    return false;
...
bool
US_AnaProfile::AnaProfPCSA::fromXml (QXmlStreamReader &xmli)
{
  parms.clear ();
  nchan = 0;
  ParmPCSA parm1;
      ...
      if (xmli.isStartElement ())
        {
          if (ename == "p_pcsa")
          ...
          else if (ename == "channel_parms")
            {
              ...
              // ALEXEY: get (modified)  curve type form XML & transform into
              // appropriate internal varialble QString ctype  = attr.value(
              // "curve_type" ).toString();
              ...
              int varcnt
                  = attr.value ("variations_count").toString ().toInt ();
              int griter
                  = attr.value ("gridfit_iterations").toString ().toInt ();
              int respts
                  = attr.value ("curve_reso_points").toString ().toInt ();
              ...
              // ALEXEY: change "regularization" to:
              // {"none","specified_alpha","auto_computed_alpha"}
...
QString
US_AnaProfile::AnaProfPCSA::curve_type_fromXml (QString curve_param)
{
...
QString
US_AnaProfile::AnaProfPCSA::curve_type_toXml (QString curve_param)
{
  QString curve_type = QString ("");
...
bool
US_AnaProfile::AnaProfPCSA::toXml (QXmlStreamWriter &xmlo)
{
  ...
  for (int ii = 0; ii < nchan; ii++)
    {
      ...
      xmlo.writeAttribute ("x_min", QString::number (parms[ii].x_min));
      xmlo.writeAttribute ("x_max", QString::number (parms[ii].x_max));
      xmlo.writeAttribute ("y_min", QString::number (parms[ii].y_min));
      xmlo.writeAttribute ("y_max", QString::number (parms[ii].y_max));
      xmlo.writeAttribute ("z_value", QString::number (parms[ii].z_value));
      xmlo.writeAttribute ("variations_count",
                           QString::number (parms[ii].varcount));
      xmlo.writeAttribute ("gridfit_iterations",
                           QString::number (parms[ii].grf_iters));
      xmlo.writeAttribute ("curve_reso_points",
                           QString::number (parms[ii].creso_pts));
      xmlo.writeAttribute ("noise", parms[ii].noise_type);

      xmlo.writeAttribute ("regularization", parms[ii].treg_type);

      xmlo.writeAttribute ("reg_alpha", QString::number (parms[ii].tr_alpha));
      xmlo.writeAttribute ("mc_iterations",
                           QString::number (parms[ii].mc_iters));
      xmlo.writeEndElement (); // channel_parms
    }
  // 3-------------------------------------------------------------------------->80
...
bool
US_AnaProfile::AnaProfPCSA::ParmPCSA::operator== (const ParmPCSA &p) const
{
  // bool same      = true;
  // same           = ( channel != p.channel ) ? false : same;
  ...
  if (y_max != p.y_max)
    return false;
  if (z_value != p.z_value)
    return false;
  if (tr_alpha != p.tr_alpha)
    return false;
  if (varcount != p.varcount)
    return false;
  if (grf_iters != p.grf_iters)
    return false;
  if (creso_pts != p.creso_pts)
    return false;
  if (noise_flag != p.noise_flag)
    return false;
  if (treg_flag != p.treg_flag)
    return false;
  if (mc_iters != p.mc_iters)
    return false;

  if (channel != p.channel)
    return false;
  if (curv_type != p.curv_type)
    return false;
  if (x_type != p.x_type)
    return false;
  if (y_type != p.y_type)
  ...
  if (z_type != p.z_type)
    return false;
  if (noise_type != p.noise_type)
    return false;
  if (treg_type != p.treg_type)
    return false;
