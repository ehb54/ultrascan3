...
bool US_AnaProfile::toXml(QXmlStreamWriter &xmlo) {
   ...
   */
   ...
   qDebug() << "Sizes of lc_ratios, lc_tolers, l_volumes, lv_tolers, data_ends, analysis_run -- " << lc_ratios.count()
            << lc_tolers.count() << l_volumes.count() << lv_tolers.count() << data_ends.count() << analysis_run.count();
   ...
   qDebug() << "Size: pchans(), chndescs.size(),  scan_excl_begin.size(), scan_excl_end.size()  -- " << pchans.count()
            << chndescs.size() << scan_excl_begin.size() << scan_excl_end.size();

   for (int ii = 0; ii < pchans.count(); ii++) {
      qDebug() << "In toXML AProfile: Ch_desc, scan_beg, scan_end -- " << chndescs[ ii ] << scan_excl_begin[ ii ]
               << scan_excl_end[ ii ];
   }
   //END of DEBUG
   ...
   for (int ii = 0; ii < pchans.count(); ii++) {
      ...
      xmlo.writeAttribute("abde_use_reference", QString::number(ref_use_channels[ kk ]));
      ...
      for (wvl_overlap = channdesc_to_overlapping_wvls_main.begin();
           wvl_overlap != channdesc_to_overlapping_wvls_main.end(); ++wvl_overlap) {
         qDebug() << "In AProfile's toXML:  channdesc_to_overlapping_wvls_main QMap: " << wvl_overlap.key()
...
bool US_AnaProfile::AnaProfPCSA::toXml(QXmlStreamWriter &xmlo) {
   ...
   for (int ii = 0; ii < nchan; ii++) {
      ...
      xmlo.writeAttribute("variations_count", QString::number(parms[ ii ].varcount));
      xmlo.writeAttribute("gridfit_iterations", QString::number(parms[ ii ].grf_iters));
      xmlo.writeAttribute("curve_reso_points", QString::number(parms[ ii ].creso_pts));
