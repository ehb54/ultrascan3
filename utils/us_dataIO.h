...
class US_UTIL_EXTERN US_DataIO {
 public:
  ...
  class Scan {
   public:
    ...
    QVector<double> stddevs;  //!< The scan readings standard deviations
    QByteArray
        interpolated;  //!< Bit array, 1 ea. Reading: 0=actual; 1=interpolated
  };
  ...
  */
  ...
  static bool spike_check(const Scan&, const QVector<double>&, int, int, int,
                          double*);
 ...
 private:
  ...
  static void copyxRange(double, double, const QVector<double>&,
                         QVector<double>&);
