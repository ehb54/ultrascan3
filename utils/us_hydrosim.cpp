...
void
US_Hydrosim::calculate (double temperature)
{
  ...
  //    << solution.viscosity << solution.viscosity_wt <<
  //    solution.viscosity_tb;
  double vol_per_molecule = solution.vbar * mw / AVOGADRO;
  double rad_sphere = pow (vol_per_molecule * 3.0 / (4.0 * M_PI), 1.0 / 3.0);
  ...
  prolate.f_f0 = pow (axial_ratio, -1.0 / 3.0) * sqrt (sq (axial_ratio) - 1.0)
  ...
  // Long rod:
  rod.f_f0 = pow (2.0 / 3.0, 1.0 / 3.0) * pow (axial_ratio, 2.0 / 3.0)
             / (log (2.0 * axial_ratio) - 0.3);

  double br
      = 1.0e+08 * pow (2.0 / (3.0 * axial_ratio), 1.0 / 3.0) * rad_sphere;
