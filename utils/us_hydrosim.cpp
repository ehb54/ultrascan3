...
void US_Hydrosim::calculate(double temperature) {
   ...
   // Oblate ellipsoid:
   oblate.f_f0 = sqrt(sq(axial_ratio) - 1.0) / (pow(axial_ratio, 2.0 / 3.0) * atan(sqrt(sq(axial_ratio) - 1.0)));
   ...
   // Long rod:
   rod.f_f0 = pow(2.0 / 3.0, 1.0 / 3.0) * pow(axial_ratio, 2.0 / 3.0) / (log(2.0 * axial_ratio) - 0.3);

   double br = 1.0e+08 * pow(2.0 / (3.0 * axial_ratio), 1.0 / 3.0) * rad_sphere;
