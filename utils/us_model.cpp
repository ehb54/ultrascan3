...
#define RT (R_GC * K20)
  ...
  // Adjust buoyancy to be consistent with s-value. Negative s-values require
  // negative buoyancy. Unless custom grid is used, we set the vbar to the
  // reciprocal of the positive s-value vbar.

  buoyancyb = 1.0 - (vbar * DENS_20W);

  if ((s < 0.0 && buoyancyb > 0.0) || (s > 0.0 && buoyancyb < 0.0)) {
    return false;
  }
  ...
  if (df >
      0.1) {  // Significant change in f:  replace and use old as concentration

    if (c == 0.0) component.signal_concentration = f;
...
QString US_Model::typeText(int subtype) {
  ...
  const typemap tmap[] = {{MANUAL, QObject::tr("Manual")},
  ...
  for (int jj = 0; jj < ntmap; jj++) {  // look for model type match

    if (analysis ==
        tmap[jj].typeval) {       // we have a match:  build type description
      ...
      {
        if (subtype > 0) {  // Append sub-type based on a given flag
        ...
        else {  // Append sub-type based on already-created description
          ...
          if (kk > 0)  // Grab type string from within model description
          {
            ...
            if (monteCarlo) tdesc = tdesc.replace("-MC", "");
            if (alphaRP != 0.0) tdesc = tdesc.replace("-TR", "");
...
int US_Model::load(const QString& filename) {
  ...
  if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) return US_DB2::DBERROR;

  QXmlStreamReader xml(&file);

  int result = load_stream(xml);

  if (result == US_DB2::NO_MODEL &&
      monteCarlo) {  // Handle a multi-model stream
    file.close();
  ...
  while (!xml.atEnd()) {
    ...
    if (xml.isStartElement()) {
      if (xml.name() == "model") {
        ...
        if (nmtag >
            1) {  // A second model tag:  return to handle multi-model stream
          monteCarlo = true;
        ...
        subGrids = subgs.isEmpty() ? subGrids : subgs.toInt();
        QString anal1 = a.value("type").toString();
        QString anal2 = a.value("analysisType").toString();
        analysis = anal1.isEmpty() ? analysis : (AnalysisType)anal1.toInt();
        analysis = anal2.isEmpty() ? analysis : (AnalysisType)anal2.toInt();
        global = (GlobalType)a.value("globalType").toString().toInt();
        optics = (OpticsType)a.value("opticsType").toString().toInt();
      }
        ...
        sc.shape = (ShapeType)a.value("shape").toString().toInt();
...
int US_Model::load_multi_model(QTextStream& tsi) {
  ...
  while (!tsi.atEnd()) {
    mline = tsi.readLine();
    if (mline.contains("</ModelData>")) break;

    if (mline.contains("<model ")) {  // At model tag, create initial contents
    ...
    else if (mline.contains("</model>")) {  // At end of model section, save a
                                            // model content and bump count
      mcont = mcont + mline + "\n</ModelData>\n";
      mcixmls << mcont;
      nmcixs++;
    }

    else {  // For any other type of line, add it to contents;
      mcont = mcont + mline + "\n";
    }
  }
  ...
  for (int ii = 0; ii < mmcomps.size();
       ii++) {  // Build list of solute point strings and list of unique ones
    ...
    sklist << skval;
    if (!skvals.contains(skval)) skvals << skval;
  }
  ...
  for (int ii = 0; ii < nskv;
       ii++) {  // Average concentration at each unique solute point
    QString skval = skvals[ii];  // Identifying solute point string
    double conc = 0.0;

    for (int jj = 0; jj < nskl; jj++) {  // Search all solute points
      if (skval == sklist[jj]) {         // If a match, sum the concentration
        scomp = mmcomps[jj];
  ...
  if (mmassos.size() > 0) associations << mmassos[0];

  QString mdsc1 = QString(mdesc).section(".", 0, -3);
  QString mdsc2 = QString(mdesc).section(".", -2, -2).section("_", 0, -2);
...
void US_Model::get_associations(QXmlStreamReader& xml, Association& as) {
  ...
  if (!skoff.isEmpty()) as.k_off = skoff.toDouble();
...
int US_Model::write(US_DB2* db) {
  ...
  qDebug() << "model writedb contsize tempsize" << contents.size()
           << temporary.size();
  ...
  if (db->lastErrno() != US_DB2::OK) {
    q.clear();
    q << "new_model" << modelGUID << description << contents << vari << meni
      << editGUID << QString::number(US_Settings::us_inv_ID());
    message = QObject::tr("created");
  } else {
    db->next();
    QString id = db->value(0).toString();
    q.clear();
    q << "update_model" << id << description << contents << vari << meni
      << editGUID;
...
void US_Model::write_stream(QXmlStreamWriter& xml) {
  ...
  if (bottom != 0.0) xml.writeAttribute("bottom", QString::number(bottom));
  if (alphaRP != 0.0) xml.writeAttribute("alphaRP", QString::number(alphaRP));
  ...
  if (requestGUID.length() > 0) xml.writeAttribute("requestGUID", requestGUID);

  if (monteCarlo) xml.writeAttribute("monteCarlo", "1");
  ...
  for (int i = 0; i < ncomps; i++) {
    ...
    if (cgmdata) {
      ...
      QString val =
          QString("g%1_r%2_c%3").arg(c.grid_id).arg(c.row).arg(c.column);
    ...
    if (notmany) {  // In most cases, don't test values to write
    ...
    } else {  // If many components, only write non-default values
      if (sc->vbar20 != TYPICAL_VBAR) xml.writeAttribute("vbar20", strVbar);
      if (sc->extinction != 0.0) xml.writeAttribute("extinction", strExtinc);
      if (sc->axial_ratio != 10.0) xml.writeAttribute("axial", strAxial);
      if (sc->sigma != 0.0) xml.writeAttribute("sigma", strSigma);
      if (sc->delta != 0.0) xml.writeAttribute("delta", strDelta);
      if (sc->oligomer != 1) xml.writeAttribute("oligomer", strOligo);
      if (sc->shape != SPHERE) xml.writeAttribute("shape", strShape);
      if (sc->analyte_type != 0) xml.writeAttribute("type", strType);
      if (sc->molar_concentration != 0.0) xml.writeAttribute("molar", strMolar);
    }
...
QString US_Model::get_filename(const QString& path, const QString& guid,
    ...
    while (!xml.atEnd()) {
      ...
      if (xml.isStartElement()) {
        if (xml.name() == "model") {
          ...
          if (a.value("modelGUID").toString() ==
              guid) {  // Break when we have found a file with a matching GUID
  ...
  if (newFile) {  // No matching-GUID file, so look for a break in the number
                  // sequence
    int number = (f_names.size() > 0) ?  // Last used file number
                     f_names.last().mid(1, 7).toInt()
                                      : 0;  // and default last sequence

    for (int ii = 0; ii < number; ii++) {
      QString fnamck = "M" + QString().sprintf("%07i", ii + 1) + ".xml";

      if (!f_names.contains(fnamck)) {  // There is a hole in the sequence, so
                                        // re-use this number
        number = ii;
        break;
      }
...
QString US_Model::composite_mc_file(QStringList& mcfiles, const bool rmvi) {
  ...
  if (mc_iters == 1) {
    if (cmfname.contains(".mcN") || cmfname.contains("_mcN")) return cmfname;
    if (!name_desc) {  // If no ".mc" in file name, must check description in
                       // contents
      QFile filei(cmfname);
      ...
      if (mdesc.contains("_mcN")) return cmfname;
    }
  }

  // Otherwise, scan the MC iteration file names
  for (int ii = 0; ii < mcfiles.size(); ii++) {
    if (name_desc) {  // Handle a file whose name tells that it is a composite
      if (mcfiles[ii].contains(".mcN") ||
          mcfiles[ii].contains("_mcN")) {  // For composite, bump composite
                                           // count, decrement iters, save name
        mc_comps++;
        mc_iters--;
        cmfname = mcfiles[ii];
      }
    } else {  // Handle a file whose description tells that it is a composite
      ...
      int jj = qMax(0, mcont.indexOf("description="));
      QString mdesc = QString(mcont).mid(jj, 99).section(dquo, 1, 1);
      if (mdesc.contains("_mcN")) {  // For composite, bump composite count,
                                     // decrement iters, save name
        mc_comps++;
        mc_iters--;
        cmfname = mcfiles[ii];
  ...
  if (mc_comps > 1 || mc_iters < 1) {  // Return now if we don't have 0 or 1
                                       // composite, plus some iter models
  ...
  if (mc_comps ==
      0) {  // No composite exists, so create one from the iteration models
    cmfname = mcfiles[0];
    ...
    if (!name_desc) {  // No description in name ("M00...xml"), so determine
                       // composite name
      if (rmvi) {      // Removing iteration files, so reuse first name for
                       // composite
        filei.remove();
      }

      else {  // If not removing iteration files, get a new name for composite
        bool newFile = true;
    ...
    else {  // Description in name, so create a new name with iters count in it
      ...
      cmfname =
          cmfname.replace(moiter, mditer).replace(".mdl.tmp", ".model.xml");
      // qDebug() << "MDL:CMF: moix1 moix2 moiter mditer" << moix1 << moix2 <<
      // moiter << mditer; qDebug() << "MDL:CMF: (C)cmfname" << cmfname;
    }
    ...
    if (miguid != mcguid) mcont.replace(miguid, mcguid);
    int flx = 0;
  ...
  else {  // A composite exists, so append to it from new iteration models
    QFile filei(cmfname);
    if (!filei.open(QIODevice::ReadOnly | QIODevice::Text)) {
      qDebug() << "**MC composite file open error**";
      return empty_str;
    ...
    if (name_desc) {  // Name has description, so see if it needs to be renamed
      int moix1 = cmfname.indexOf(".mc");
      int moix2 = cmfname.indexOf("_mc");
      int moix = (moix1 > 0) ? moix1 : qMax(0, moix2);
      QString moiter = QString(cmfname).mid(moix, 7);
      ...
      mditer =
          ((moix1 > 0) ? "." : "_") + QString().sprintf("mcN%03i", mc_ittot);
      // qDebug() << "MDL:CMF: (B)moix1 moix2 moiter mditer" << moix1 << moix2
      // << moiter << mditer;

      if (moiter != mditer) {  // Iterations changes (almost always), so rename
                               // is necessary
        cmfname =
            cmfname.replace(moiter, mditer).replace(".mdl.tmp", ".model.xml");
        filei.rename(cmfname);
        // qDebug() << "MDL:CMF: (D)cmfname" << cmfname;
      } else {  // No name change (unlikely), so must delete and re-create
        filei.remove();
      }
    }

    else {  // Name has no description, so no renaming necessary and just
            // recreate
      filei.remove();
    }
  ...
  if (rmvi) {             // If so requested, remove the MC iteration files
    if (mc_comps == 0) {  // If list is all iteration files, remove them all
      for (int ii = 0; ii < mcfiles.size();
           ii++) {  // Delete unless file name was reused for composite
        if (mcfiles[0] != cmfname) QFile(mcfiles[ii]).remove();
      }
    }

    else if (name_desc) {  // If list has composite, name has description;
                           // selectively delete
      for (int ii = 0; ii < mcfiles.size();
           ii++) {  // Delete iteration files (not composite)
        if (!mcfiles[ii].contains("mcN")) QFile(mcfiles[ii]).remove();
      }
    }

    else {  // If no description in file names, delete based on descr. content
      for (int ii = 0; ii < mcfiles.size();
           ii++) {  // Delete iteration files (not composite)
        if (mcfiles[ii] == cmfname) continue;
        QFile filei(mcfiles[ii]);
        if (!filei.open(QIODevice::ReadOnly | QIODevice::Text)) {
          qDebug() << "**MC iteration file open error**";
          continue;
        ...
        int jj = qMax(0, mcont.indexOf("description="));
        QString mdesc = QString(mcont).mid(jj, 99).section(dquo, 1, 1);
        if (!mdesc.contains("_mcN")) QFile(mcfiles[ii]).remove();
      }
    }
  }
...
void US_Model::debug(void) {
  ...
  for (int i = 0; i < components.size(); i++) {
    ...
    for (int j = 0; j < sc->c0.radius.size(); j++) {
      qDebug() << "   c0 r c" << sc->c0.radius[j] << sc->c0.concentration[j];
    }
  }
}
