...
bool US_Model::SimulationComponent::operator==(const SimulationComponent &sc) const {
   if (analyteGUID != sc.analyteGUID)
      return false;
   if (name != sc.name)
      return false;
   if (molar_concentration != sc.molar_concentration)
      return false;
   if (signal_concentration != sc.signal_concentration)
      return false;
   if (vbar20 != sc.vbar20)
      return false;
   if (mw != sc.mw)
   ...
   if (f_f0 != sc.f_f0)
      return false;
   if (extinction != sc.extinction)
      return false;
   if (sigma != sc.sigma)
      return false;
   if (delta != sc.delta)
      return false;
   if (oligomer != sc.oligomer)
      return false;
   if (shape != sc.shape)
      return false;
   if (axial_ratio != sc.axial_ratio)
      return false;
   if (analyte_type != sc.analyte_type)
      return false;

   if (c0.radius != sc.c0.radius)
      return false;
   if (c0.concentration != sc.c0.concentration)
      return false;
...
bool US_Model::operator==(const US_Model &m) const {
   if (monteCarlo != m.monteCarlo)
      return false;
   if (wavelength != m.wavelength)
      return false;
   if (variance != m.variance)
      return false;
   if (meniscus != m.meniscus)
      return false;
   if (bottom != m.bottom)
      return false;
   if (alphaRP != m.alphaRP)
      return false;
   if (modelGUID != m.modelGUID)
      return false;
   if (editGUID != m.editGUID)
      return false;
   if (requestGUID != m.requestGUID)
      return false;
   ...
   if (global != m.global)
      return false;
   if (coSedSolute != m.coSedSolute)
      return false;
   if (subGrids != m.subGrids)
      return false;
   if (dataDescrip != m.dataDescrip)
      return false;
   if (associations.size() != m.associations.size())
      return false;

   for (int i = 0; i < associations.size(); i++)
      if (associations[ i ] != m.associations[ i ])
         return false;

   if (components.size() != m.components.size())
      return false;

   for (int i = 0; i < components.size(); i++)
      if (components[ i ] != m.components[ i ])
         return false;
...
#define RT (R_GC * K20)
   ...
   if (df > 0.1) { // Significant change in f:  replace and use old as concentration
...
#else
   QDir dir;
   path = is_perm ? US_Settings::dataDir() + "/models" : US_Settings::tmpDir() + "/temp_models";
...
QString US_Model::typeText(int subtype) {
   ...
   for (int jj = 0; jj < ntmap; jj++) { // look for model type match
      ...
      if (analysis == tmap[ jj ].typeval) { // we have a match:  build type description
...
int US_Model::load(const QString &filename) {
   ...
   if (result == US_DB2::NO_MODEL && monteCarlo) { // Handle a multi-model stream
      file.close();
...
int US_Model::load_stream(QXmlStreamReader &xml) {
   ...
   while (!xml.atEnd()) {
      ...
      if (xml.isStartElement()) {
         if (xml.name() == "model") {
            ...
            if (nmtag > 1) { // A second model tag:  return to handle multi-model stream
               monteCarlo = true;
            ...
            monteCarlo = monteCarlo ? monteCarlo : (iter.isEmpty() ? false : (iter.toInt() > 1));
   ...
   bool cgmdata = !customGridData.grids.isEmpty() && !customGridData.components.isEmpty()
...
int US_Model::load_multi_model(QTextStream &tsi) {
   ...
   while (!tsi.atEnd()) {
      ...
      else if (mline.contains("</model>")) { // At end of model section, save a model content and bump count
         mcont = mcont + mline + "\n</ModelData>\n";
         mcixmls << mcont;
         nmcixs++;
      }
   ...
   for (int ii = 0; ii < mmcomps.size(); ii++) { // Build list of solute point strings and list of unique ones
   ...
   for (int ii = 0; ii < nskv; ii++) { // Average concentration at each unique solute point
...
int US_Model::write(US_DB2 *db) {
   ...
   if (db->lastErrno() != US_DB2::OK) {
      q.clear();
      q << "new_model" << modelGUID << description << contents << vari << meni << editGUID
      ...
      QString id = db->value(0).toString();
      q.clear();
      q << "update_model" << id << description << contents << vari << meni << editGUID;
...
QString US_Model::get_filename(const QString &path, const QString &guid, bool &newFile) {
      ...
      while (!xml.atEnd()) {
         ...
         if (xml.isStartElement()) {
            if (xml.name() == "model") {
               ...
               if (a.value("modelGUID").toString() == guid) { // Break when we have found a file with a matching GUID
   ...
   if (newFile) { // No matching-GUID file, so look for a break in the number sequence
      ...
      for (int ii = 0; ii < number; ii++) {
         ...
         if (!f_names.contains(fnamck)) { // There is a hole in the sequence, so re-use this number
            number = ii;
            break;
         }
      }
...
QString US_Model::composite_mc_file(QStringList &mcfiles, const bool rmvi) {
   ...
   if (mc_iters == 1) {
      if (cmfname.contains(".mcN") || cmfname.contains("_mcN"))
         return cmfname;
      if (!name_desc) { // If no ".mc" in file name, must check description in contents
         QFile filei(cmfname);
   ...
   for (int ii = 0; ii < mcfiles.size(); ii++) {
      if (name_desc) { // Handle a file whose name tells that it is a composite
         if (
            mcfiles[ ii ].contains(".mcN")
            || mcfiles[ ii ].contains("_mcN")) { // For composite, bump composite count, decrement iters, save name
            mc_comps++;
            mc_iters--;
            cmfname = mcfiles[ ii ];
         }
         ...
         int jj = qMax(0, mcont.indexOf("description="));
         QString mdesc = QString(mcont).mid(jj, 99).section(dquo, 1, 1);
         if (mdesc.contains("_mcN")) { // For composite, bump composite count, decrement iters, save name
            mc_comps++;
            mc_iters--;
            cmfname = mcfiles[ ii ];
   ...
   if (mc_comps > 1 || mc_iters < 1) { // Return now if we don't have 0 or 1 composite, plus some iter models
      qDebug() << "**" << mc_comps << "MC composites, and" << mc_iters << "MC iterations **";
      return empty_str;
   }
   ...
   if (mc_comps == 0) { // No composite exists, so create one from the iteration models
      cmfname = mcfiles[ 0 ];
      ...
      if (!name_desc) { // No description in name ("M00...xml"), so determine composite name
         if (rmvi) { // Removing iteration files, so reuse first name for composite
            filei.remove();
         }
         ...
         cmfname = cmfname.replace(moiter, mditer).replace(".mdl.tmp", ".model.xml");
      ...
      if (name_desc) { // Name has description, so see if it needs to be renamed
         ...
         mditer = ((moix1 > 0) ? "." : "_") + QString().sprintf("mcN%03i", mc_ittot);
         //qDebug() << "MDL:CMF: (B)moix1 moix2 moiter mditer" << moix1 << moix2 << moiter << mditer;

         if (moiter != mditer) { // Iterations changes (almost always), so rename is necessary
            cmfname = cmfname.replace(moiter, mditer).replace(".mdl.tmp", ".model.xml");
      ...
      else { // Name has no description, so no renaming necessary and just recreate
         filei.remove();
      }
   ...
   if (rmvi) { // If so requested, remove the MC iteration files
      if (mc_comps == 0) { // If list is all iteration files, remove them all
         for (int ii = 0; ii < mcfiles.size(); ii++) { // Delete unless file name was reused for composite
            if (mcfiles[ 0 ] != cmfname)
               QFile(mcfiles[ ii ]).remove();
         }
      }

      else if (name_desc) { // If list has composite, name has description;  selectively delete
         for (int ii = 0; ii < mcfiles.size(); ii++) { // Delete iteration files (not composite)
...
void US_Model::debug(void) {
   ...
   if (cgmdata) {
      for (int i = 0; i < customGridData.grids.size(); i++) {
         qDebug() << "Metadata: grid=" << i << "xType=" << customGridData.grids.at(i).xType
                  << "xMin=" << customGridData.grids.at(i).xMin << "xMax=" << customGridData.grids.at(i).xMax
                  << "xRes=" << customGridData.grids.at(i).xRes << "yType=" << customGridData.grids.at(i).yType
                  << "yMin=" << customGridData.grids.at(i).yMin << "yMax=" << customGridData.grids.at(i).yMax
                  << "yRes=" << customGridData.grids.at(i).yRes << "zVal=" << customGridData.grids.at(i).zVal;
      }
   }
