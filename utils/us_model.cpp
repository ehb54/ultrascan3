...
bool
US_Model::SimulationComponent::operator== (const SimulationComponent &sc) const
{
...
bool
US_Model::Association::operator== (const Association &a) const
{
  if (k_d != a.k_d)
    return false;
  if (k_off != a.k_off)
...
// Equality operator
bool
US_Model::operator== (const US_Model &m) const
{
...
bool
US_Model::calc_coefficients (SimulationComponent &component)
{
...
bool
US_Model::model_path (QString &path, bool is_perm)
{
#ifdef NO_DB
  path = is_perm ? "" : "";
#else
    ...
    { // look for model type match
      ...
      if (analysis == tmap[jj].typeval)
        { // we have a match:  build type description
          ...
          // if ( alphaRP != 0.0 )
          //    tdesc    = tdesc + "-TR";       // Tikhonov Regularization
          //    subtype
...
bool
US_Model::is_reactant (const int compx)
{
  bool is_react = false;
...
bool
US_Model::is_product (const int compx)
{
  bool is_prod = false;
...
int
US_Model::load (bool db_access, const QString &guid, US_DB2 *db)
{
  if (db_access)
...
int
US_Model::load_disk (const QString &guid)
{
...
int
US_Model::load (const QString &filename)
{
  QFile file (filename);
...
int
US_Model::load_string (const QString &mcont)
{
...
int
US_Model::load_stream (QXmlStreamReader &xml)
{
  QString coSedStr;
  QString comprStr;
          ...
          if (xml.name () == "model")
            {
              ...
              analysis
                  = anal1.isEmpty () ? analysis : (AnalysisType)anal1.toInt ();
              analysis
                  = anal2.isEmpty () ? analysis : (AnalysisType)anal2.toInt ();
          ...
          else if (xml.name () == "analyte")
            {
              ...
              if (a.hasAttribute ("grid"))
                {
                  QRegularExpressionMatch match
                      = re.match (a.value ("grid").toString ());
                  if (match.hasMatch ())
                    {
              ...
              sc.molar_concentration
                  = a.value ("molar").toString ().toDouble ();
              sc.signal_concentration
                  = a.value ("signal").toString ().toDouble ();
...
int
US_Model::load_multi_model (QTextStream &tsi)
{
...
void
US_Model::write_mm_stream (QTextStream &tso)
{
...
void
US_Model::mfem_scans (QXmlStreamReader &xml, SimulationComponent &sc)
{
  while (!xml.atEnd ())
    {
      xml.readNext ();
...
void
US_Model::get_associations (QXmlStreamReader &xml, Association &as)
{
  QXmlStreamAttributes a = xml.attributes ();
...
void
US_Model::get_metadata (QXmlStreamReader &xml)
{
  QXmlStreamAttributes a = xml.attributes ();
...
int
US_Model::load_db (const QString &guid, US_DB2 *db)
{
  QStringList q;
...
int
US_Model::load (const QString &id, US_DB2 *db)
{
  QStringList q;
...
int
US_Model::write (bool db_access, const QString &filename, US_DB2 *db)
{
  if (db_access)
    return write (db);
  else
...
// Write a model file
int
US_Model::write (const QString &filename)
{
  QFile file (filename);
...
void
US_Model::write_stream (QXmlStreamWriter &xml)
{
  if (modelGUID.size () != 36)
    modelGUID = US_Util::new_guid ();
      ...
      if (cgmdata)
        {
          ...
          QString val = QString ("g%1_r%2_c%3")
                            .arg (c.grid_id)
                            .arg (c.row)
                            .arg (c.column);
...
QString
US_Model::get_filename (const QString &path, const QString &guid,
                        bool &newFile)
{
  QDir f (path);
  QStringList filter ("M???????.xml");
...
QString
US_Model::composite_mc_file (QStringList &mcfiles, const bool rmvi)
{
  const QChar dquo ('"');
  QString empty_str ("");
  ...
  for (int ii = 0; ii < mcfiles.size (); ii++)
    {
      if (name_desc)
        { // Handle a file whose name tells that it is a composite
          if (mcfiles[ii].contains (".mcN") || mcfiles[ii].contains ("_mcN"))
            { // For composite, bump composite count, decrement iters, save
              // name
              mc_comps++;
              mc_iters--;
              cmfname = mcfiles[ii];
    ...
    { // A composite exists, so append to it from new iteration models
      ...
      if (name_desc)
        { // Name has description, so see if it needs to be renamed
          ...
          // qDebug() << "MDL:CMF: (B)moix1 moix2 moiter mditer" << moix1 <<
          // moix2 << moiter << mditer;
        ...
        { // Name has no description, so no renaming necessary and just
          // recreate
          filei.remove ();
        }
        ...
        { // If no description in file names, delete based on descr. content
          for (int ii = 0; ii < mcfiles.size (); ii++)
            { // Delete iteration files (not composite)
              ...
              int jj = qMax (0, mcont.indexOf ("description="));
              QString mdesc
                  = QString (mcont).mid (jj, 99).section (dquo, 1, 1);
              if (!mdesc.contains ("_mcN"))
                QFile (mcfiles[ii]).remove ();
            }
        }
...
int
US_Model::mc_iter_xmls (QStringList &mcixs)
{
...
bool
US_Model::subtype_match (const int subtype, const int stflag)
{
  ...
  for (int i = 0; i < components.size (); i++)
    {
      ...
      if (cgmdata)
        {
          qDebug () << "  grid info: id="
                    << customGridData.components.at (i).grid_id
      ...
      for (int j = 0; j < sc->c0.radius.size (); j++)
        {
          qDebug () << "   c0 r c" << sc->c0.radius[j]
                    << sc->c0.concentration[j];
        }
    }
}
