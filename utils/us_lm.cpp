 ...
 */
/*
 * Last Mod:   Emre Brookes   2012. Adaptations for UltraScan II.
...
US_LM::LM_Control::LM_Control (double ftol, double xtol, double gtol,
                               double epsilon, double stepbound, int maxcall,
                               int scale_diag, int printflags)
{
  this->ftol = ftol;
  this->xtol = xtol;
  this->gtol = gtol;
...
US_LM::LM_Status::LM_Status (double fnorm, int nfev, int info)
{
  this->fnorm = fnorm;
  this->nfev = nfev;
  this->info = info;
...
US_LM::LM_CurveData::LM_CurveData (double *t, double *y,
...
QString
US_LM::lm_statmsg (US_LM::LM_Status *status, bool longmsg)
{
  QString statmsg = longmsg ? QString (lm_infmsg[status->info])
...
void
US_LM::lm_printout_std (int n_par, double *par, int m_dat,
...
void
US_LM::lmmin (int n_par, double *par, int m_dat, const void *data,
                                ...
                                int printflags, int iflag, int iter, int nfev))
{
  ...
  lm_lmdif (m, n, par, fvec, control->ftol, control->xtol, control->gtol,
            ...
            &(status->nfev), fjac, ipvt, qtf, wa1, wa2, wa3, wa4, evaluate,
            printout, control->printflags, data);

  if (printout)
    (*printout) (n, par, m, data, fvec, control->printflags, -1, 0,
                 status->nfev);
...
void
US_LM::lm_lmdif (int m, int n, double *x, double *fvec, double ftol,
                 ...
                 int printflags, const void *data)
{
   ...
   *        m nonlinear functions. Implementations should be written as
   * follows:
   *
   ...
   *        Call with printout=lm_printout_std to use the default
   * implementation.
   *
   ...
   */
  int i, iter, j;
  double actred, delta, dirder, eps, fnorm, fnorm1, gnorm, par, pnorm, prered,
      ratio, step, sum, temp, temp1, temp2, temp3, xnorm;
  ...
  *nfev = 0; /* function evaluation counter */
  ...
  if (mode == 2)
    { /* scaling by diag[] */
      for (j = 0; j < n; j++)
        { /* check for nonpositive elements */
          if (diag[j] <= 0.0)
            {
              *info = 10; // invalid parameter
              return;
      ...
      do
        {
          ...
          lm_lmpar (n, fjac, m, ipvt, diag, qtf, delta, &par, wa1, wa2, wa4,
                    wa3);
          ...
          *info = 0;
          ...
          /*** inner: compute the ratio of the actual to the predicted
           * reduction. ***/
          ...
          printf ("lmdif/ actred=%.10e prered=%.10e ratio=%.10e"
                  ...
                  actred, prered, prered != 0 ? ratio : 0., SQR (temp1),
                  SQR (temp2), dirder);
#endif
...
void
US_LM::lm_lmpar (int n, double *r, int ldr, int *ipvt, double *diag,
...
void
US_LM::lm_qrfac (int m, int n, double *a, int pivot, int *ipvt, double *rdiag,
...
void
US_LM::lm_qrsolv (int n, double *r, int ldr, int *ipvt, double *diag,
...
double
US_LM::lm_rmsdnorm (int n, const double *x)
{
  double rmsd = 0e0;
  for (int i = 0; i < n; i++)
    {
...
double
US_LM::lm_enorm (int n, const double *x)
{
...
void
US_LM::lmcurve_evaluate (double *par, int m_dat, const void *data,
                         double *fvec, int * /* info */)
{
  int i;
  for (i = 0; i < m_dat; i++)
    fvec[i] = ((LM_CurveData *)data)->y[i]
              - ((LM_CurveData *)data)->f (((LM_CurveData *)data)->t[i], par);
  // *info = *info; /* to prevent a 'unused variable' warning */
}

void
US_LM::lmcurve_fit (int n_par, double *par, int m_dat, const double *t,
                    ...
                    const LM_Control *control, LM_Status *status)
{
  ...
  lmmin (n_par, par, m_dat, (const void *)&data, lmcurve_evaluate, control,
         status, lm_printout_std);
}

void
US_LM::lmcurve_fit_rmsd (int n_par, double *par, int m_dat, const double *t,
                         const double *y, double (*f) (double t, double *par),
                         const LM_Control *control, LM_Status *status)
{
  LM_CurveData data ((double *)t, (double *)y, f);
...
const US_LM::LM_Control lm_control_double (LM_USERTOL, LM_USERTOL, LM_USERTOL,
                                           LM_USERTOL, 100., 100, 1, 0);
const US_LM::LM_Control lm_control_float (1.e-7, 1.e-7, 1.e-7, 1.e-7, 100.,
                                          100, 0, 0);
