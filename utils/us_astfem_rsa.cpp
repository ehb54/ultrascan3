...
//!< care for non-reacting systems and calculate_ra2 takes care for reacting
//!< systems.
int US_Astfem_RSA::calculate(US_DataIO::RawData& exp_data) {
  ...
  for (int step = 0; step < simparams.speed_step.count(); step++) {
    if (fstime >= simparams.speed_step[step].time_first &&
        lstime <= simparams.speed_step[step]
                      .time_last) {  // Found step that this dataset fits
                                     // inside:  break out
      cdsx = step;
  ...
  af_params.cdset_speed =
      simparams.speed_step[cdsx].rotorspeed;  // Rotor speed of current dataset
  ...
  if (simparams.tsobj == NULL || simparams.sim_speed_prof.count() <
                                     1) {  // Timestate is not properly loaded
...
#else
    QString tmst_fpath =
        US_Settings::tmpDir() + "/" + temp_Id_name() + ".time_state.tmst";
    US_AstfemMath::writetimestate(tmst_fpath, simparams, exp_data);
#endif
    simparams.simSpeedsFromTimeState(tmst_fpath);
  } else {  // Timestate object and speed profile exist are properly loaded
  ...
  for (int istep = 0; istep < nspstep;
       istep++) {  // Fill time,omega2t work vectors for each step
      ...
      for (int jt = 0; jt < simparams.sim_speed_prof[istep].duration; jt++) {
        if (jt < 4 || (jt + 4) > simparams.sim_speed_prof[istep].duration)
          DbgLv(1) << "RSA:calc::  from_tmst:  rpm"
                   ...
                   << "time"
                   << simparams.sim_speed_prof[istep].time_b_accel + jt << "w2t"
  ...
  if (nstep > 0 && nstep < nspstep) {  // There are missing speed steps
    ...
    QList<int> speeds;
    DbgLv(1) << "RSA:calc: SSS:  nstep nspstep" << nstep << nspstep << "cdsx"
             << cdsx;
    ...
    for (int ss = 0; ss < nspstep; ss++) {
      ...
      DbgLv(1) << "RSA:calc: SSS:   ss" << ss << "ispeed" << ispeed << "ssox"
               << ssox;

      if (ssox >= 0) {  // If speed in old speed step vector, just copy it
      ...
      else {  // Speed not in old speed step:  build from sim_speed_prof
        ...
        DbgLv(1) << "RSA:calc: SSS:    tf tl wf wl" << time_first << time_last
                 << w2t_first << w2t_last;
      }
    }
  ...
  for (int js = 1; js < exp_data.scanCount(); js++) {
    if (exp_data.scanData[js].rpm !=
        srpm) {  // Flag at least one speed change within the raw data
      mspd_data = true;
      break;
    }
  }
  ...
  for (int cc = 0; cc < size_cv; cc++) {
    ...
    if (mspd_data) {  // For multi-speed data, use stretch radii just computed
    ...
    if (!reacting[cc])  // noninteracting
    {
      ...
      for (int speed_step = 0; speed_step < nstep; speed_step++) {
        ...
        if (speed_step > 0 && ed->scan[lscan].time < sp->time_first) {
          DbgLv(1) << "RSA:calc: break inside astfem_rsa (step-time beyond "
                      "last scan-time)";
          break;
        }
        ...
        DbgLv(1) << "RSA:2-calc: meniscus bottom" << af_params.current_meniscus
                 << af_params.current_bottom;
#if 1
        adjust_limits(sp->rotorspeed);  // Does rotor stretch
#endif
        DbgLv(1) << "RSA:3-calc: meniscus bottom" << af_params.current_meniscus
                 << af_params.current_bottom;

        ed->meniscus =
            af_params
                .current_meniscus;  // Update meniscus for experimental grid
        ed->bottom =
            af_params.current_bottom;  // Update bottom for experimental grid

        if (mspd_data &&
            strch_rot) {  // For multi-speed data, reset speed-appropriate radii
        ...
        if (sp->acceleration_flag)  // Accel flag is used to accelerate the
                                    // rotor speed.
        {
          ...
          // when acceleration process ended is obtained from timestate
          // readings. And simulation is done for acceleration zone for each
          // second.
          ...
          calculate_ni(current_speed, next_speed, speed_step, CT0, simdata,
                       true);
          ...
          omeg1 = current_om2t;
#ifndef NO_DB
          DbgLv(1) << "RSA:emit ctime: eoa time" << current_time << "component"
                   << cc + 1;
        ...
        DbgLv(1) << "RSA:calc:  dt" << af_params.dt << "rat omgf simp"
                 ...
                 << af_params.current_meniscus << af_params.omega_s << sc->s;
        if (af_params.dt < 0.0) break;
        ...
        DbgLv(2) << " CALC:_ni: speed" << avg_speed << "step" << speed_step
                 ...
                 << "af-men,bot" << af_params.current_meniscus
...
DbgLv(1) << "RSA:emit ccomp: component" << cc+1;
  ...
  for (int group = 0; group < rg.size(); group++) {
    ...
    else {  // Timestate exists and is loaded properly
    ...
    for (int step = 0; step < simparams.sim_speed_prof.size();
         step++) {  // Fill work time,omega2t vectors with values for each step
    ...
    if (simparams.sim_speed_prof.count() > 0) {
      //          time2        = qMax( 0.0, (double)simparams.sim_speed_prof[ 0
      //          ].time_b_accel - 1 );
    ...
    for (int step = 0; step < nstep; step++) {
      ...
      DbgLv(1) << "simulation_rsa:  dt" << af_params.dt << "rat omgf simp"
               ...
               << af_params.current_meniscus << af_params.omega_s << s_max;
      double dtsv = af_params.dt;
      ...
      if (af_params.simpoints > 10000) {
        DbgLv(1) << "RSA: ***COMPUTED simpoints:" << af_params.simpoints
                 ...
                 << af_params.dt << duration << "** simpts set to 10000 **";
...
DbgLv(2) << "RSA: TimeCorr   correction" << correction << "nscans" << nscans;
  ...
  if ((ncalls % TIMING_RA_INC) < 1) {
    DbgLv(1) << "  Elapsed fem-calc ms" << elapsedCalc << "nc totC" << ncalls
             << totTC << "  size_cv" << size_cv;
    DbgLv(1) << "   t1 t2 t3 t4 t5 t6 t7 t8" << totT1 << totT2 << totT3 << totT4
             << totT5 << totT6 << totT7 << totT8;
  }
#endif
...
void US_Astfem_RSA::adjust_limits(int speed) {
  ...
  if (icset_speed == speed &&
      simparams.bottom >
          simparams.bottom_position) {  // If the current speed step matches the
                                        // dataset speed step
    // and a specific bottom was given, then use that bottom value.
    af_params.current_bottom = simparams.bottom;
  } else {  // If no bottom was given or we are at a speed step preceding the
...
void US_Astfem_RSA::initialize_rg(void) {
  ...
  if (as->rcomps.size() > 2) tmp_rg.GroupComponent.append(as->rcomps[2]);
  ...
  for (int i = 0; i < system.associations.size(); i++) {
    ...
    for (int counter = 1; counter < system.associations.size(); counter++) {
      ...
      if (flag1) {
        ...
        for (int j = 0; j < tmp_rg.GroupComponent.size(); j++) {
          if (component1 == (int)tmp_rg.GroupComponent[j]) flag2 = true;
        }
        ...
        for (int j = 0; j < tmp_rg.GroupComponent.size(); j++) {
          if (component2 == (int)tmp_rg.GroupComponent[j]) flag2 = true;
        }
        ...
        if (ncomp > 2) {
          for (int j = 0; j < tmp_rg.GroupComponent.size(); j++) {
            if (component3 == (int)tmp_rg.GroupComponent[j]) flag2 = true;
          }
    ...
    if (flag3) {
      ...
      if (j < system.associations.size()) {
        ...
        // Only 2 components react in first reaction
        if (ncomp > 2) tmp_rg.GroupComponent.append(component3);
...
void US_Astfem_RSA::initialize_conc(int kk, US_AstfemMath::MfemInitial& CT0,
  ...
  } else  // af_c0.concentration.size() > 0
  {
    ...
    } else  // interpolation
    {
      ...
      for (int j = 0; j < nval; j++) CT0.concentration[j] += C.concentration[j];
    }
  }
}
...
int US_Astfem_RSA::calculate_ni(double rpm_start, double rpm_stop, int step,
  ...
  if (simparams.band_forming) z_tolerance = 0.0;

  if (!is_zero) {  // If not already into virtual-zero scans, test this one
    double rad_last = af_params.current_bottom - 0.04;

    for (int jr = C_init.radius.size() - 1; jr > 1;
         jr--) {  // Look for virtually-zero concentration from last radius back
      if (C_init.radius[jr] < rad_last) {
        if (C_init.concentration[jr] <
            z_tolerance) {  // Concentration very low, so flag simulation zero
                            // here on out
          is_zero = true;
        }
        DbgLv(1) << "jr" << jr << "C_init.conc[jr]" << C_init.concentration[jr]
                 << "z_toler" << z_tolerance << "rpm" << rpm_current << "radius"
  ...
  // is_zero=false;
  //*DEBUG*
  if (is_zero) {  // All sim scans will be zero: skip time loop and set last
                  // scan time
    ntsteps = 0;
    // ntsteps             = 1;
    ...
    DbgLv(1) << "C_ni:  IS_ZERO !!  1st step time" << last_time << "rpm_stop"
             << rpm_stop << "s D" << ss0 << dd0 << "ntsteps" << ntsteps;
  }
  ...
  for (int jt = 0; jt < ntsteps; jt++) {
    if ((jt == 0) && (accel == false)) {
      ...
      for (int jx = 0; jx < Nx; jx++) simscan.conc[jx] = C0[jx];
    ...
    //    simscan.omega_s_t   = simparams.sim_speed_prof[step].w2t_timestate[
    //    jt+2 ] ;
    // }
...
DbgLv(1) << "simulation_rsa: kt jt" << kt << jt << "scan_time=" << simscan.time
 ...
 << simparams.sim_speed_prof[step].w2t_timestate[kt];
      ...
      if (af_params.s[0] > 0) {
        ...
        for (int jx = 2; jx < Nx; jx++) {
          right_hand_side[jx] = -CB[0][jx] * C0[jx - 2] -
                                CB[1][jx] * C0[jx - 1] - CB[2][jx] * C0[jx];
        }
      } else {
        for (int jx = 0; jx < Nx - 2; jx++) {
          right_hand_side[jx] = -CB[0][jx] * C0[jx] - CB[1][jx] * C0[jx + 1] -
                                CB[2][jx] * C0[jx + 2];
    ...
    for (int jx = 0; jx < Nx; jx++) cA[jx] = C0[jx];
  ...
  if ((nccall % 1000) == 0) {
    ...
    qDebug() << "   ++Elapsed calc-ni ms" << elapsCalc << " nc totC totK"
             << nccall << ttTC << totTK << " Nx" << Nx << " acc fixG" << accel
             << fixedGrid;
...
void US_Astfem_RSA::mesh_gen(QVector<double>& nu, int MeshOpt) {
  ...
  switch (MeshOpt) {
    ...
    case (int)US_SimulationParameters::CLAVERIE:
      ...
      for (int i = 0; i < Np; i++) x.append(m + (b - m) * i / (Np - 1));
      break;
...
void US_Astfem_RSA::mesh_gen_s_pos(const QVector<double>& nuvec) {
  ...
  for (int i = 0; i < af_params.s.size(); i++)  // Markers for steep regions
  {
    ...
    DbgLv(1) << "RSA:  msg_pos: i " << i << "txc" << tmp_xc << "tNf" << tmp_Nf
             << "tHs" << tmp_Hstar;
  ...
  if (IndLayer > 0) {  // Steep region  (potentially)
    ...
    for (int i = 0; i < IndLayer; i++)  // Consider i-th steep region
    {
      ...
      } else  // Last layer, use sine distribution for grids
      {
        ...
        for (int j = 0; j <= Nf[i] - 1; j++) {
          indp++;
          y.append(xci + (bottom - xci) * sin(j / (Nf[i] - 1.0) * PIhalf));
  ...
  if (indp < 2) {  // IndLayer==0  or  indp count less than 2
    x.append(meniscus);

    // Add one more point to Schuck's grids
    for (int kk = 1; kk < Np - 1; kk++) {  // Schuck's mesh
  ...
  else {  // IndLayer>0  and  indp greater than 1
    int NfTotal = indp;
    ...
    for (int jj = NfTotal + Nm - 2; jj >= 0; jj--) x.append(yary[jj]);
    DbgLv(1) << "RSA:  msg_pos: IndL>0  y sz" << y.size() << "Mp" << Mp << "Nm"
             << Nm << "x sz" << x.size();
  }
...
void US_Astfem_RSA::mesh_gen_s_neg(const QVector<double>& nu) {
  ...
  if (b * (pow(mbratio, (Np - 3.5) / Npm1) - pow(mbratio, (Np - 2.5) / Npm1)) <
    ...
    DbgErr() << "Use exponential grid only!(1/10000 reported):  Np Nf Nm" << Np
             << Nf << Nm << "m b nu0" << m << b << nu0;
  } else {
    // Nf > 2
    double xcm = xc - m;
    ...
    // set x:
    for (jp = Nf - 1; jp >= 0; jp--) x.append(ysA[jp]);

    for (jp = 1; jp < Nm; jp++) x.append(ytA[jp]);

    for (jp = Js; jp >= 0; jp--) x.append(yrA[jp]);
...
void US_Astfem_RSA::mesh_gen_RefL(int N0, int M0) {
  ...
  if (US_AstfemMath::minval(af_params.s) > 0)  // All species with s>0
  {
    ...
    for (int jp = 0; jp < zz.size(); jp++) x.append(zA[jp]);
  } else if (US_AstfemMath::maxval(af_params.s) < 0)  //  All species with s<0
  {
    for (int jp = 0; jp < x.size() - N0; jp++) zz.append(xA[jp]);
...
void US_Astfem_RSA::ComputeCoefMatrixFixedMesh(double D, double sw2,
  ...
  if (Nx > Nsave) {
    if (Nsave > 0) US_AstfemMath::clear_3d(Nsave, 4, Stif);
  ...
  for (int k = 0; k < Nx - 1; k++) {  // loop for all elem
    xd[0][0] = xA[k];
    xd[0][1] = 0.0;
    xd[1][0] = xA[k + 1];
    xd[1][1] = 0.0;
  ...
  for (k = 1, m = 0; k < Nx - 1; k++, m++) {  // loop for all elem
    // elem k-1: i=1,2
...
void US_Astfem_RSA::ComputeCoefMatrixMovingMeshR(double D, double sw2,
                                                 double** CA, double** CB) {
  ...
  for (k = 2; k < Nx - 1; k++) {  // loop for all elem
    // elem k-1: i=1,2
    mm = k - 1;
...
void US_Astfem_RSA::ComputeCoefMatrixMovingMeshL(double D, double sw2,
                                                 double** CA, double** CB) {
  // compute local stiffness matrices
  double xd[4][2];  // coord for verices of quad elem
  xA = x.data();
  ...
  for (int k = 1; k < Nx - 2; k++) {  // loop for all elem
    // elem k:
...
void US_Astfem_RSA::decompose(US_AstfemMath::MfemInitial* C0) {
  ...
  DbgLv(2) << "RSA:  decompose k_min time_max timeStepSize" << k_min << time_max
           << timeStepSize;
...
//         qApp->processEvents();
  ...
  for (int i = 0; i < num_comp; i++) {
    for (int j = 0; j < Npts; j++) C0[i].concentration[j] = C1[i][j];
  }
...
void US_Astfem_RSA::ReactionOneStep_Euler_imp(int Npts, double** C1,
                                              double timeStep) {
  int num_comp = af_params.role.size();
...
int US_Astfem_RSA::calculate_ra2(double rpm_start, double rpm_stop,
  ...
  if (accel) {
    ...
    if (s_min > 0.0)  // all sediment towards bottom
    {
      ...
      DbgLv(1) << "RSA:_ra2: s_min s_max" << s_min << s_max << "xc xAj" << xc
               << xA[j] << "j" << j << "N0 M0" << (j + 1) << (j * 4);
    }
      ...
      if (s_min > 0)  // All components sedimenting
      {
        ...
        for (int i = 0; i < Mcomp; i++) {
          ...
          DbgLv(1) << "RSA: smin>0:GlStf: i" << i << "Si Di sw2 sw2D sqb s_max"
                   << af_params.s[i] << af_params.D[i] << sw2 << sw2D << sqb
                   << s_max;
          ...
          for (int j = 1; j < Nx; j++) {
            double dval =
                0.1 * exp(sw2D * (sq(0.5 * (xA[j - 1] + xA[j])) - sqb));

            double alpha = s_rat * (1.0 - dval) + dval;
            xbvec << (pow(xA[j - 1], alpha) * pow(xA[j], (1.0 - alpha)));
  ...
  DbgLv(1) << "RSA: newX3 Nx" << Nx << "C0[00] C0[m0] C0[0n] C0[mn]" << C0[0][0]
           << C0[Mcomp - 1][0] << C0[0][Nx - 1] << C0[Mcomp - 1][Nx - 1];
  ...
  for (int jj = 0; jj < Nx; jj++) {
    ...
    for (int ii = 0; ii < Mcomp; ii++) CT0[jj] += C0[ii][jj];
  ...
  for (int kkk = 0; kkk < Nt + 2; kkk += 2)  // two steps in together
  {
    ...
    for (int jj = 0; jj < Nx; jj++) simscan.conc.append(CT0[jj]);
    ...
    if (accel)  // need to reconstruct CA and CB by linear interpolation
    {
      ...
      for (int i = 0; i < Mcomp; i++) {
        for (int j1 = 0; j1 < 3; j1++) {
          for (int j2 = 0; j2 < Nx; j2++) {
            CA[i][j1][j2] =
                CA1[i][j1][j2] + dval * (CA2[i][j1][j2] - CA1[i][j1][j2]);

            CB[i][j1][j2] =
                CB1[i][j1][j2] + dval * (CB2[i][j1][j2] - CB1[i][j1][j2]);
          }
        }
      }
    }
    ...
    if (accel || fixedGrid)  // For fixed grid
    {
      for (int i = 0; i < Mcomp; i++) {
        ...
        US_AstfemMath::tridiag(CA[i][0], CA[i][1], CA[i][2], right_hand_side,
                               C1[i], Nx);
      }
    }
    ...
    else  // Moving grid
    {
      ...
      for (int i = 0; i < Mcomp; i++) {
        ...
        right_hand_side[1] = -CB[i][1][1] * C0[i][0] - CB[i][2][1] * C0[i][1] -
                             CB[i][3][1] * C0[i][2];

        for (int j = 2; j < Nx - 1; j++) {
          right_hand_side[j] =
              -CB[i][0][j] * C0[i][j - 2] - CB[i][1][j] * C0[i][j - 1] -
              CB[i][2][j] * C0[i][j] - CB[i][3][j] * C0[i][j + 1];
        }
        ...
        US_AstfemMath::QuadSolver(CA[i][0], CA[i][1], CA[i][2], CA[i][3],
                                  right_hand_side, C1[i], Nx);
        DbgLv(2) << "TMS:RSA:ra:   MovGrid:    i" << i << "C1[i]" << C1[i][0]
                 << C1[i][Nx - 4] << C1[i][Nx - 3] << C1[i][Nx - 2]
                 << C1[i][Nx - 1];
        DbgLv(2) << "TMS:RSA:ra:   MovGrid:    i" << i << "C0[i]" << C0[i][0]
                 << C0[i][Nx - 4] << C0[i][Nx - 3] << C0[i][Nx - 2]
                 << C0[i][Nx - 1];
      }
...
void US_Astfem_RSA::GlobalStiff(double* xb, double** ca, double** cb, double D,
                                double sw2) {
...
void US_Astfem_RSA::store_mfem_data(US_DataIO::RawData& edata,
  ...
  if (escan != nscan) edata.scanData.resize(nscan);
...
void US_Astfem_RSA::printspeedprofile(
    US_SimulationParameters::SpeedProfile& sp) {
