...
US_Astfem_RSA::US_Astfem_RSA (US_Model &model, US_SimulationParameters &params,
                              QObject *parent)
...
//!< for the output and the other radial grid needed for finite element
//!< simulation. Time grid spacing is different from radial grid spacing and
//!< defined in Ref : Cao W, Demeler B, Modeling Analytical Ultracentrifugation
//!< Experiments
//         with an Adaptive Space-Time Finite Element Solution of the Lamm
//         Equation, (2005) Biophys J. 89(3):1589-602.

//!< Concentrations can be obtained on either experimental grid or
//!< finite element simulation grid based on user's choice. "simout_flag" is
//!< the variable used to choose grid, true refers to simulation grid and false
...
//!< Both reacting and non-reacting systems are handled here. calculate_ni
//!< takes care for non-reacting systems and calculate_ra2 takes care for
//!< reacting systems.
int
US_Astfem_RSA::calculate (US_DataIO::RawData &exp_data)
{
  US_AstfemMath::MfemInitial *vC0
      = NULL; // Initial concentration for multiple components
  ...
  bool *reacting
      = reactVec.data ();  // Used to check reacting or non_reacting case
  ...
  US_SimulationParameters::SpeedProfile
      *sp; // Pointer used to current speed SpeedProfile
      ...
      if (fstime >= simparams.speed_step[step].time_first
        ...
        { // Found step that this dataset fits inside:  break out
          ...
          DbgLv (1) << "RSA:calc:   cdsx" << cdsx << "time_first time_last"
                    << simparams.speed_step[step].time_first
                    << simparams.speed_step[step].time_last;
          break;
        }
    }
  // DbgLv(0)<< "RSA:calc ===ST===";
  ...
  af_params.simpoints
      = simparams.simpoints; // Number of simulation i.e. radial grid points
  ...
  af_params.cdset_speed = simparams.speed_step[cdsx]
                              .rotorspeed; // Rotor speed of current dataset
  ...
  if (simparams.bottom > simparams.bottom_position)
    {
      double stretch_val
          = stretch (simparams.rotorcoeffs, (int)af_params.cdset_speed);
...
#endif
  DbgLv (1) << "RSA:calc: meniscus bottom" << af_params.current_meniscus
  ...
  int initial_npts
      = af_data.scan[0]
            .conc.size (); // Size of the concentration vector on radial grid
  ...
  if (simparams.tsobj == NULL || simparams.sim_speed_prof.count () < 1)
...
#else
      QString tmst_fpath = US_Settings::tmpDir () + "/" + temp_Id_name ()
                           + ".time_state.tmst";
      US_AstfemMath::writetimestate (tmst_fpath, simparams, exp_data);
#endif
      ...
      if (dbg_level > 0)
        {
          DbgLv (1) << "RSA:calc: readings:from tmst file: istep" << istep
                    ...
                    << "time_ea" << accel_times[istep] << "time_e"
                    << time_end[istep] << "w2t_ea" << accel_w2ts[istep]
                    << "w2t_e" << w2t_end[istep];
          for (int jt = 0; jt < simparams.sim_speed_prof[istep].duration; jt++)
            {
              if (jt < 4
                  || (jt + 4) > simparams.sim_speed_prof[istep].duration)
                DbgLv (1) << "RSA:calc::  from_tmst:  rpm"
                          << simparams.sim_speed_prof[istep].rpm_timestate[jt]
                          << "time"
                          << simparams.sim_speed_prof[istep].time_b_accel + jt
                          << "w2t"
                          << simparams.sim_speed_prof[istep].w2t_timestate[jt];
            }
        }
      //*DEBUG*
    }
  ...
  if (nstep > 0 && nstep < nspstep)
    { // There are missing speed steps
      ...
      QVector<US_SimulationParameters::SpeedProfile> ss_old
          = simparams.speed_step;
      QList<int> speeds;
      DbgLv (1) << "RSA:calc: SSS:  nstep nspstep" << nstep << nspstep
                << "cdsx" << cdsx;

      for (int ss = 0; ss < nstep; ss++)
        {
          DbgLv (1) << "RSA:calc: SSS:  old avgspeed rotspeed setspeed"
                    << ss_old[ss].avg_speed << ss_old[ss].rotorspeed
                    << ss_old[ss].set_speed;
      ...
      int kscan = sp0.scans;
      // Create the new full speed step vector by copy or
      // create-from-sim_speed_prof
      for (int ss = 0; ss < nspstep; ss++)
        {
          US_SimulationParameters::SimSpeedProf *ipp
              = &simparams.sim_speed_prof[ss];
          int ispeed = ipp->rotorspeed;
          int ssox = speeds.indexOf (ispeed);
          DbgLv (1) << "RSA:calc: SSS:   ss" << ss << "ispeed" << ispeed
                    << "ssox" << ssox;
          ...
          if (ssox >= 0)
            ...
            { // Speed not in old speed step:  build from sim_speed_prof
              US_SimulationParameters::SpeedProfile *opp
              ...
              double w2t_first
                  = w2t_eacc + (time_first - time_eacc) * delta_w / delta_t;
              double w2t_last
                  = w2t_eacc + (time_last - time_eacc) * delta_w / delta_t;
              ...
              DbgLv (1) << "RSA:calc: SSS:    tf tl wf wl" << time_first
                        << time_last << w2t_first << w2t_last;
            }
        }
  ...
  // Flag: any stretch?
  bool strch_rot
      = (simparams.rotorcoeffs[0] > 0.0 || simparams.rotorcoeffs[1] > 0.0);
  ...
  // experimental grid or simdata i.e. scans on the simulation grid as
  // required. Here size_cv refers to  number of components in the component
  // vector.
  // ---------------------------------------------------------------------------
      ...
      if (!reacting[cc]) // noninteracting
        {
          initialize_conc (
              cc, CT0,
              true); // Initialize the concentration vector on initial grid
          ...
          US_AstfemMath::MfemData
              *ed; // Pointer used for af_data i.e. on experimental grid
          is_zero = false;
          ...
          for (int speed_step = 0; speed_step < nstep; speed_step++)
            {
              ...
              if (speed_step > 0 && ed->scan[lscan].time < sp->time_first)
                {
                  DbgLv (1) << "RSA:calc: break inside astfem_rsa (step-time "
                               "beyond last scan-time)";
                  break;
                }
              ...
              DbgLv (1) << "RSA:2-calc: meniscus bottom"
                        << af_params.current_meniscus
                        << af_params.current_bottom;
#if 1
              adjust_limits (sp->rotorspeed); // Does rotor stretch
#endif
              DbgLv (1) << "RSA:3-calc: meniscus bottom"
                        << af_params.current_meniscus
                        << af_params.current_bottom;

              ed->meniscus = af_params.current_meniscus; // Update meniscus for
                                                         // experimental grid
              ed->bottom
                  = af_params
                        .current_bottom; // Update bottom for experimental grid

              if (mspd_data && strch_rot)
                { // For multi-speed data, reset speed-appropriate radii
                  int points = ed->radius.count ();
                  double radinc
                      = (ed->bottom - ed->meniscus) / (double)(points - 1);
              ...
              if (sp->acceleration_flag) // Accel flag is used to accelerate
                                         // the rotor speed.
                {
                  // Calculates number of time steps for acceleration
                  // simulation grid

                  // Number of time steps for acceleration zones is equal to
                  // time when acceleration ended minus last time of last
                  // timestep. Time when acceleration process ended is obtained
                  // from timestate readings. And simulation is done for
                  // acceleration zone for each second.

                  af_params.time_steps
                      = accel_times[speed_step] - (int)(time0);
                  af_params.dt = 1.0;
                  ...
                  calculate_ni (current_speed, next_speed, speed_step, CT0,
                                simdata, true);
                  ...
                  omeg1 = current_om2t;
#ifndef NO_DB
                  DbgLv (1) << "RSA:emit ctime: eoa time" << current_time
                            << "component" << cc + 1;
              ...
              DbgLv (1) << "RSA:calc:  dt" << af_params.dt << "rat omgf simp"
                        ...
                        << "b m omgs s" << af_params.current_bottom
                        << af_params.current_meniscus << af_params.omega_s
                        << sc->s;
              if (af_params.dt < 0.0)
                break;
              ...
              if (af_params.dt > duration)
                {
                  ...
                  af_params.simpoints
                      = 1 + (int)(lg_bm_rat / (s_omg_fac * af_params.dt));
                }

              af_params.time_steps = qCeil (duration / af_params.dt);

              af_params.start_time = current_time; // Update starting time for
                                                   // constant speed zone
              af_params.start_om2t = current_om2t; // Update starting w2t for
                                                   // constant speed zone
              ...
              double avg_speed
                  = simparams.sim_speed_prof[speed_step].avg_speed;

              // Calculate the simulation for constant speed zone
              DbgLv (2) << " CALC:_ni: speed" << avg_speed << "step"
                        << speed_step << "sp-men,bot" << simparams.meniscus
                        << simparams.bottom << "af-men,bot"
                        << af_params.current_meniscus
                        << af_params.current_bottom << "cdset_speed"
                        << af_params.cdset_speed;
              calculate_ni (avg_speed, avg_speed, speed_step, CT0, simdata,
                            false);
  ...
  for (int group = 0; group < rg.size (); group++)
    {
              ...
              for (int rr = 0; rr < as->rcomps.size (); rr++)
                {
                  ...
                  if (af_params.role[jj].comp_index == as->rcomps[rr])
                    {
      ...
      for (int aa = 0; aa < num_rule; aa++)
        {
          ...
          for (int rr = 0; rr < as->rcomps.size (); rr++)
            {
              as->rcomps[rr] = af_params.local_index[as->rcomps[rr]];
...
#endif
          simparams.simSpeedsFromTimeState (tmst_fpath);
          DbgLv (1) << "after writing timestate file"
      ...
      DbgLv (1) << "SS2: ss size" << simparams.speed_step.size () << "ssp size"
                << simparams.sim_speed_prof.size ();
      ...
      for (int step = 0; step < simparams.sim_speed_prof.size (); step++)
        { // Fill work time,omega2t vectors with values for each step
          ...
          DbgLv (1) << "subha: rsa readings: " << accel_times[step]
                    << accel_w2ts[step];
        }
      ...
      if (simparams.sim_speed_prof.count () > 0)
        {
          //          time2        = qMax( 0.0,
          //          (double)simparams.sim_speed_prof[ 0 ].time_b_accel - 1 );
      ...
      for (int step = 0; step < nstep; step++)
        {
          ...
          if (sp->acceleration_flag)
            {
              // Calculates number of time steps for acceleration simulation
              // grid
...
#ifndef NO_DB
              DbgLv (1) << "RSA:emit ctime: accel:current_time" << current_time
                        << "step" << step;
              emit new_time (current_time);
              qApp->processEvents ();
#endif
          ...
          DbgLv (2) << "RSA: ***COMPUTED(2) omega_s:" << af_params.omega_s
                    << "step_speed" << step_speed << "s.size s_max"
                    << af_params.s.size () << s_max;

          double lg_bm_rat
              = log (af_params.current_bottom / af_params.current_meniscus);
          double s_omg_fac = af_params.omega_s * s_max;
          af_params.dt = lg_bm_rat / (s_omg_fac * (simparams.simpoints - 1));
          DbgLv (1) << "simulation_rsa:  dt" << af_params.dt << "rat omgf simp"
                    << lg_bm_rat << s_omg_fac << simparams.simpoints
                    << "b m omgs smax" << af_params.current_bottom
                    << af_params.current_meniscus << af_params.omega_s
                    << s_max;
          double dtsv = af_params.dt;
          ...
          if (af_params.dt > duration)
            {
              ...
              af_params.simpoints
                  = 1 + (int)(lg_bm_rat / (s_omg_fac * af_params.dt));
              DbgLv (1) << "RSA: ***CORRECTED(2) dt:" << duration << dtsv
                        << af_params.simpoints;
            }
          ...
          if (af_params.simpoints > 10000)
            {
              DbgLv (1) << "RSA: ***COMPUTED simpoints:" << af_params.simpoints
                        << "omg2t s_max dt dura" << af_params.omega_s << s_max
                        << af_params.dt << duration
                        << "** simpts set to 10000 **";
          ...
          af_params.start_time
              = current_time; // Update starting time for constant speed zone
          af_params.start_om2t
              = current_om2t; // Update starting w2t for constant speed zone
          US_AstfemMath::MfemData *ed = &af_data;

          DbgLv (2) << "RSA:   tsteps sttime" << af_params.time_steps
                    << current_time;
...
DbgLv(2) << "RSA: TimeCorr   correction" << correction << "nscans" << nscans;
  ...
  if ((ncalls % TIMING_RA_INC) < 1)
    {
      DbgLv (1) << "  Elapsed fem-calc ms" << elapsedCalc << "nc totC"
                << ncalls << totTC << "  size_cv" << size_cv;
      DbgLv (1) << "   t1 t2 t3 t4 t5 t6 t7 t8" << totT1 << totT2 << totT3
                << totT4 << totT5 << totT6 << totT7 << totT8;
    }
#endif
...
//----------------
void
US_Astfem_RSA::update_assocv (void)
{
...
void
US_Astfem_RSA::adjust_limits (int speed)
{
  ...
  DbgLv (1) << "RSA:adjlim stretch1" << stretch_value << "speed"
            << af_params.cdset_speed;
  ...
  if (icset_speed == speed && simparams.bottom > simparams.bottom_position)
...
double
US_Astfem_RSA::stretch (double *rotorcoeffs, int rpm)
{
  double speed = (double)rpm;
  return (rotorcoeffs[0] * speed + rotorcoeffs[1] * sq (speed));
}

// Setup reaction groups
void
US_Astfem_RSA::initialize_rg (void)
{
      ...
      for (int counter = 1; counter < system.associations.size (); counter++)
        {
          ...
          while (reaction_used[counter])
            {
              counter++;
              if (counter == system.associations.size ())
                return;
            }

          // Check if any component already present in tmp_rg matches any of
          // the three components in current (*system).associations entry
          ...
          if (flag1)
            {
              // It is present (flag1=true) so add this rule to the tmp_rg
              // vector
              ...
              // Check if 1st component is already in the GroupVector from
              // tmp_rg
              for (int j = 0; j < tmp_rg.GroupComponent.size (); j++)
                {
                  if (component1 == (int)tmp_rg.GroupComponent[j])
              ...
              // Check if 2nd component is already in the GroupVector from
              // tmp_rg
              for (int j = 0; j < tmp_rg.GroupComponent.size (); j++)
                {
              ...
              // Check if 3rd component is already in the GroupVector from
              // tmp_rg (but only if non-zero)
...
void
US_Astfem_RSA::initialize_conc (int kk, US_AstfemMath::MfemInitial &CT0,
                                bool noninteracting)
{
      ...
      if (simparams.band_forming)
        {
          ...
          double plen
              = simparams.cp_pathlen != 0.0 ? simparams.cp_pathlen : 1.2;
          ...
          for (int j = 0; j < nval; j++)
            {
              ...
              if (j < 2 || j > (CT0.concentration.size () - 3)
                  || j == (CT0.concentration.size () / 40))
                DbgLv (2) << "RSA:  j base conc" << j << base
                          << CT0.concentration[j];
              if (mxct < CT0.concentration[j])
                {
                  mxct = CT0.concentration[j];
                }
            }
          DbgLv (1) << "RSA:BF:  mxct" << mxct << "lamella_width"
                    << lamella_width;
        }
          ...
          for (int j = 0; j < nval; j++)
            {
              ...
              // qDebug()<<"CT0_conc"<< CT0.concentration[ j ] <<
              // sc->signal_concentration  << j << kk ;
              if (mxct < CT0.concentration[j])
                {
                  mxct = CT0.concentration[j];
                }
...
int
US_Astfem_RSA::calculate_ni (double rpm_start, double rpm_stop, int step,
                             ...
                             US_AstfemMath::MfemData &simdata, bool accel)
{
  ...
  if (accel)
    {
      ...
      if (af_params.s[0] > 0)
        {
          // Radial distance from meniscus how far the boundary will move
          // during this acceleration step (without diffusion)
  ...
  double time_dif = (af_params.time_steps > 1 && rpm_stop != rpm_start)
                        ? (double)(af_params.time_steps)
                        : 1.0;

  double rpm_inc
      = (rpm_stop - rpm_start) / time_dif; // Increment in rotor speed
          ...
          if (C_init.radius[jr] < rad_last)
            {
              if (C_init.concentration[jr] < z_tolerance)
                { // Concentration very low, so flag simulation zero here on
                  // out
                  is_zero = true;
                }
              DbgLv (1) << "jr" << jr << "C_init.conc[jr]"
                        << C_init.concentration[jr] << "z_toler" << z_tolerance
                        << "rpm" << rpm_current << "radius"
                        << C_init.radius[jr] << "is_zero" << is_zero;
              break;
            }
        }
    }
  ...
  if (is_zero)
    { // All sim scans will be zero: skip time loop and set last scan time
      ...
      DbgLv (1) << "C_ni:  IS_ZERO !!  1st step time" << last_time
                << "rpm_stop" << rpm_stop << "s D" << ss0 << dd0 << "ntsteps"
                << ntsteps;
    }
  ...
  for (int jt = 0; jt < ntsteps; jt++)
    {
      if ((jt == 0) && (accel == false))
        ...
        //] ;
        //    simscan.omega_s_t   =
        //    simparams.sim_speed_prof[step].w2t_timestate[ jt+2 ] ;
        // }
...
#ifndef NO_DB
      // DbgLv(1) << "RSA: EMIT0 rpm_current" << rpm_current << "rpm inc" <<
      // rpm_inc;
      emit current_speed (
          (int)rpm_current); // Update current rotor speed in GUI window
      qApp->processEvents ();
#endif
      ...
      if (accel || fixedGrid)
        {
          ...
          int jx = Nx - 1;
          right_hand_side[jx] = -CB[0][jx] * C0[jx - 1] - CB[1][jx] * C0[jx];
        }
      else
        {
          if (af_params.s[0] > 0)
          ...
          if (jt < 10 || (jt + 10) > ntsteps || (jt % jti) == 0)
            {
              ...
              emit new_time (simscan.time);
              qApp->processEvents ();
              DbgLv (1) << "RSA:emit ntime: sscn time" << simscan.time << "jt"
                        << jt;
            }
        }
#endif
  ...
  if ((nccall % 1000) == 0)
    {
      ...
      qDebug () << "   ++Elapsed calc-ni ms" << elapsCalc << " nc totC totK"
                << nccall << ttTC << totTK << " Nx" << Nx << " acc fixG"
                << accel << fixedGrid;
      qDebug () << "     CN: t1 t2 t3 t4 t5 t6 t7 t8" << ttT1 << ttT2 << ttT3
                << ttT4 << ttT5 << ttT6 << ttT7 << ttT8;
      int p0 = totTK / 2;
...
void
US_Astfem_RSA::mesh_gen (QVector<double> &nu, int MeshOpt)
{
  ...
  //              = 2 Exponential mesh (Schuck's form., no refinement at
  //              bottom) = 3 input from data file: "mesh_data.dat" = 4 ASTFVM
  //              grid (Finite Volume Method) = 10, acceleration mesh (left and
  //              right refinement)
            ...
            if (qAbs (x[0] - m) > 1.0e7)
              {
                DbgErr () << "The meniscus from the mesh file does not"
                             " match the set meniscus - using Claverie Mesh "
                             "instead\n";
              }
  ...
  DbgLv (1) << "RSA: ***COMPUTED Nx" << Nx << "simpts" << af_params.simpoints
            << "omg2t" << af_params.omega_s << "s0" << af_params.s[0] << "dt"
            << af_params.dt;
}

// Generate exponential mesh and refine cell bottom (for s>0)
void
US_Astfem_RSA::mesh_gen_s_pos (const QVector<double> &nuvec)
{
  double tmp_Hstar;
  QVector<double> xc;
  ...
  DbgLv (1) << "RSA:  msg_pos: nu0" << nu[0] << "nu1" << nu[1] << "Np" << Np
            << "uth" << uth;
  DbgLv (1) << "RSA:  msg_pos:  m " << meniscus << " b " << bottom << " bmsqdf"
            << bmsqdf << "af_parms_size" << af_params.s.size ();

  for (int i = 0; i < af_params.s.size (); i++) // Markers for steep regions
    {
      double tmp_xc = bottom - (1.0 / (nu[i] * bottom)) * log (nu[i] * bmsqdf);

      // # of pts for i-th layer
      int tmp_Nf
          = (int)(PIhalf * (bottom - tmp_xc) * nu[i] * bottom / 2.0 + 0.5) + 1;

      // Step required by Pac(i) < 1
      tmp_Hstar = (bottom - tmp_xc) / tmp_Nf * PIhalf;

      DbgLv (1) << "RSA:  msg_pos: i " << i << "txc" << tmp_xc << "tNf"
                << tmp_Nf << "tHs" << tmp_Hstar;
              ...
              for (int j = 0; j <= Nf[i] - 1; j++)
                {
                  indp++;
                  y.append (
                      xci + (bottom - xci) * sin (j / (Nf[i] - 1.0) * PIhalf));
  ...
  if (indp < 2)
    ...
    { // IndLayer>0  and  indp greater than 1
      ...
      Nm = Mp;
      DbgLv (1) << "RSA:  msg_pos: IndL>0  Js" << Js << "NfTotal" << NfTotal
                << "Nm" << Nm;
      // Regular region
      x.append (meniscus);
      yary = y.data ();
      ...
      for (int jj = NfTotal + Nm - 2; jj >= 0; jj--)
        x.append (yary[jj]);
      DbgLv (1) << "RSA:  msg_pos: IndL>0  y sz" << y.size () << "Mp" << Mp
                << "Nm" << Nm << "x sz" << x.size ();
    }
  ...
  int mm = x.size () / 2;
  int ee = x.size () - 1;
  DbgLv (1) << "RSA:  mgs_pos: xA sme" << x[0] << x[1] << x[2] << x[mm - 1]
            << x[mm] << x[mm + 1] << x[mm + 2] << x[ee - 2] << x[ee - 1]
            << x[ee];
}

// Generate exponential mesh and refine cell meniscus (for  s<0)
void
US_Astfem_RSA::mesh_gen_s_neg (const QVector<double> &nu)
{
  ...
  if (b * (pow (mbratio, (Np - 3.5) / Npm1) - pow (mbratio, (Np - 2.5) / Npm1))
          < Hstar
      || Nf <= 2)
    {
      double *yrA = yr.data ();
...
void
US_Astfem_RSA::mesh_gen_RefL (int N0, int M0)
{
  const double PIhalf = M_PI / 2.0;
  ...
  int mm = x.size () / 2;
  int ee = x.size () - 1;
  DbgLv (1) << "RSA:mgR: Nx xA sme" << Nx << x[0] << x[1] << x[2] << x[mm - 1]
            << x[mm] << x[mm + 1] << x[mm + 2] << x[ee - 2] << x[ee - 1]
            << x[ee];
}
...
void
US_Astfem_RSA::ComputeCoefMatrixFixedMesh (double D, double sw2, double **CA,
                                           double **CB)
{
  if (Nx != x.size () || Nx < 1)
...
void
US_Astfem_RSA::ComputeCoefMatrixMovingMeshR (double D, double sw2, double **CA,
                                             double **CB)
{
...
void
US_Astfem_RSA::ComputeCoefMatrixMovingMeshL (double D, double sw2, double **CA,
                                             double **CB)
{
  // compute local stiffness matrices
  double xd[4][2]; // coord for verices of quad elem
...
void
US_Astfem_RSA::decompose (US_AstfemMath::MfemInitial *C0)
{
      ...
      for (int j = 0; j < Npts; j++)
        {
          ...
          if (st0 == 2 && st1 == -1) // mono <--> dimer
            ...
            {
              c1 = US_AstfemMath::cube_root (-ct / k_assoc, 1.0 / k_assoc,
                                             0.0);
          ...
          if (af_params.role[0].stoichs[0] > 0) // c1=reactant
            {
              ...
              DbgLv (1) << "reactant_case" << C0[0].concentration[j]
                        << C0[1].concentration[j]
  ...
  DbgLv (2) << "RSA:  decompose k_min time_max timeStepSize" << k_min
            << time_max << timeStepSize;
...
void
US_Astfem_RSA::ReactionOneStep_Euler_imp (int Npts, double **C1,
                                          double timeStep)
{
  int num_comp = af_params.role.size ();
  DbgLv (2) << "RSA:Eul: Npts timeStep" << Npts << timeStep << "num_comp"
            << num_comp;
      ...
      for (int j = 0; j < Npts; j++)
        {
          ...
          if (st0 == 2 && st1 == (-1)) // mono <--> dimer
            ...
            {
              uhat = US_AstfemMath::find_C1_mono_Nmer (st0, dva / dvb,
                                                       dvc / dvb);
            }
      ...
      for (int iter = 0; iter < iter_max; iter++) // Newton iteration
        {
          ...
          // qDebug() << "RSA:Eul: j" << j << "iter" << iter << " y0[0]" <<
          // y0[0];
...
void
US_Astfem_RSA::Reaction_dydt (double *y0, double *yt)
{
...
void
US_Astfem_RSA::Reaction_dfdy (double *y0, double **dfdy)
{
  double **QC;
      ...
      for (int j = 0; j < num_comp; j++)
        {
          ...
          for (int m = 0; m < cr->assocs.size (); m++)
            {
              dfdy[i][j] -= ((double)cr->stoichs[m] * QC[cr->assocs[m]][j]);
            }
...
int
US_Astfem_RSA::calculate_ra2 (double rpm_start, double rpm_stop,
                              US_AstfemMath::MfemInitial *C_init,
                              US_AstfemMath::MfemData &simdata, bool accel)
{
  ...
  DbgLv (1) << "RSA:_ra2: Mcomp Nx" << Mcomp << Nx << "x size" << x.size ()
            << "D0 nu0 D1 nu1" << af_params.D[0] << nu[0] << af_params.D[1]
            << nu[1];
              ...
              for (int i = 0; i < Mcomp; i++)
                {
                  ...
                  DbgLv (1) << "RSA: smin>0:GlStf: i" << i
                            << "Si Di sw2 sw2D sqb s_max" << af_params.s[i]
                            << af_params.D[i] << sw2 << sw2D << sqb << s_max;
                  ...
                  for (int j = 1; j < Nx; j++)
                    {
                      ...
                      if (j < 4 || j == (Nx / 2) || (j + 4) > Nx)
                        DbgLv (1) << "RSA: smin>0:GlStf:   j" << j
                                  << "dval alpha xbj xAj" << dval << alpha
                                  << xbvec[j] << xA[j];
                    }
                  ...
                  DbgLv (1) << "RSA: smin>0:GlStf: CA[i]:" << CA[i][0][0]
                            << CA[i][1][0] << CA[i][2][Nx - 1];
                  DbgLv (1) << "RSA: smin>0:GlStf: CB[i]:" << CB[i][0][0]
                            << CB[i][1][0] << CB[i][2][Nx - 1];
                }
            }
  ...
  for (int kkk = 0; kkk < Nt + 2; kkk += 2) // two steps in together
    {
      double rpm_current
          = rpm_start + (rpm_stop - rpm_start) * (kkk + 0.5) / Nt;

#ifndef NO_DB
      DbgLv (1) << "RSA: EMIT rpm_current" << rpm_current << "rpm start/stop"
                << rpm_start << rpm_stop;
      ...
      w2t_integral
          += ((simscan.time - last_time) * sq (rpm_current * M_PI / 30));

      last_time = simscan.time;
      simscan.omega_s_t = w2t_integral;

      DbgLv (2) << "TMS:RSA:ra: time omegast" << simscan.time
                << simscan.omega_s_t << "step-scan" << simdata.scan.size ();
      ...
      DbgLv (2) << "TMS:RSA:ra:  kkk" << kkk << "CT0[0] CT0[n]" << CT0[0]
                << CT0[Nx - 1] << "accel fixedGrid" << accel << fixedGrid;
                  ...
                  for (int j2 = 0; j2 < Nx; j2++)
                    {
                      CA[i][j1][j2]
                          = CA1[i][j1][j2]
                            + dval * (CA2[i][j1][j2] - CA1[i][j1][j2]);

                      CB[i][j1][j2]
                          = CB1[i][j1][j2]
                            + dval * (CB2[i][j1][j2] - CB1[i][j1][j2]);
                    }
                }
            }
        }
      ...
      if (accel || fixedGrid) // For fixed grid
        {
          for (int i = 0; i < Mcomp; i++)
            {
              right_hand_side[0]
                  = -CB[i][1][0] * C0[i][0] - CB[i][2][0] * C0[i][1];
              ...
              right_hand_side[j]
                  = -CB[i][0][j] * C0[i][j - 1] - CB[i][1][j] * C0[i][j];
        ...
        {
          // DbgLv(2) << "TMS:RSA:ra:   (0)MovGrid: C0[00] C0[0n] C0[m0]
          // C0[1n]"
          //  << C0[0][0] << C0[0][Nx-1] << C0[Mcomp-1][0] <<
          //  C0[Mcomp-1][Nx-1];
          // DbgLv(2) << "TMS:RSA:ra:   (0)MovGrid: C1[00] C1[0n] C1[m0]
          // C1[1n]"
          //  << C1[0][0] << C1[0][Nx-1] << C1[Mcomp-1][0] <<
          //  C1[Mcomp-1][Nx-1];
          for (int i = 0; i < Mcomp; i++)
            {
              // Calculate the right hand side vector
              right_hand_side[0]
                  = -CB[i][2][0] * C0[i][0] - CB[i][3][0] * C0[i][1];
              ...
              DbgLv (2) << "TMS:RSA:ra:   MovGrid:  i" << i << "CB[0]"
                        << CB[i][2][0] << CB[i][3][0] << CB[i][1][1]
                        << CB[i][2][1] << CB[i][3][1];
              DbgLv (2) << "TMS:RSA:ra:   MovGrid:   i" << i << "CA[0]"
                        << CA[i][0][0] << CA[i][1][0] << CA[i][0][0]
                        << CA[i][1][0] << CA[i][2][0];

              US_AstfemMath::QuadSolver (CA[i][0], CA[i][1], CA[i][2],
                                         CA[i][3], right_hand_side, C1[i], Nx);
              DbgLv (2) << "TMS:RSA:ra:   MovGrid:    i" << i << "C1[i]"
                        << C1[i][0] << C1[i][Nx - 4] << C1[i][Nx - 3]
                        << C1[i][Nx - 2] << C1[i][Nx - 1];
              DbgLv (2) << "TMS:RSA:ra:   MovGrid:    i" << i << "C0[i]"
                        << C0[i][0] << C0[i][Nx - 4] << C0[i][Nx - 3]
                        << C0[i][Nx - 2] << C0[i][Nx - 1];
            }
      ...
      DbgLv (2) << "TMS:RSA:ra:   C1[10] C1[11] C1[1k] C1[1n]" << C1[1][0]
                << C1[1][1] << C1[1][Nx - 2] << C1[1][Nx - 1];

      ReactionOneStep_Euler_imp (Nx, C1, 2 * af_params.dt);

      DbgLv (2) << "TMS:RSA:ra(2):   C1[10] C1[11] C1[1k] C1[1n]" << C1[1][0]
                << C1[1][1] << C1[1][Nx - 2] << C1[1][Nx - 1];
  ...
  for (int i = 0; i < Mcomp; i++)
    {
      ...
      DbgLv (2) << "TMS:RSA:ra2: comp:" << i << "C1[i]:" << C1[i][0]
                << C1[i][1] << C1[i][Nx / 2] << C1[i][Nx - 3] << C1[i][Nx - 2]
                << C1[i][Nx - 1];
    }
...
void
US_Astfem_RSA::GlobalStiff (double *xb, double **ca, double **cb, double D,
                            double sw2)
{
...
void
US_Astfem_RSA::load_mfem_data (US_DataIO::RawData &edata,
  ...
  for (int ii = 0; ii < nscan; ii++)
    {
      ...
      if (ii < 3)
        DbgLv (2) << "RSA:f  ii c0 cn" << ii << fscan->conc[0]
                  << fscan->conc[nconc - 1] << "d0 dn"
                  << edata.scanData[ii].rvalues[0]
                  << edata.scanData[ii].rvalues[nconc - 1];
#if 1
    }
#endif
#if 0
...
void
US_Astfem_RSA::store_mfem_data (US_DataIO::RawData &edata,
                                US_AstfemMath::MfemData &fdata)
{
...
void
US_Astfem_RSA::printspeedprofile (US_SimulationParameters::SpeedProfile &sp)
{
  DbgLv (1) << "---------------Speed profile parameters begins "
               "here------------------";
  ...
  DbgLv (1) << "speedprofile time first and last" << sp.time_first
            << sp.time_last;
  DbgLv (1) << "omega2t first and last" << sp.w2t_first << sp.w2t_last;
  DbgLv (1) << "----------------Speed profile parameters ends "
               "here--------------------";
}

// Prints simulation components
void
US_Astfem_RSA::printsimComponent (US_Model::SimulationComponent &sc)
{
  DbgLv (1) << "---------------Simulation component parameters begins "
               "here----------------";
  ...
  DbgLv (1) << "----------------Simulation component parameters ends "
               "here---------------";
...
void
US_Astfem_RSA::printsimparams (void)
{
...
// Sets the buffer
void
US_Astfem_RSA::set_buffer (US_Buffer buffer)
{
...
bool
US_Astfem_RSA::iszero (const double s, const double D, const double rpm,
                       const double t, const double meniscus,
                       const double bottom)
{
  ...
  double back_diffusion
      = tolerance * sqr_D / (s * omega_sq * (bottom + meniscus) * sqr_t);
  ...
  DbgLv (1) << "IS_Z: radD" << radD << "s_dist" << s_distance << "back_diffus"
            << back_diffusion << "xval" << xval << "cleared" << cleared
            << "is_zero" << is_zero;
  DbgLv (1) << "IS_Z:    s D rpm t men bot" << s << D << rpm << t << meniscus
            << bottom << "   omega_sq  sqr_t" << omega_sq << sqr_t;
