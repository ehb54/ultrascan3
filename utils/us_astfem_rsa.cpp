...
US_Astfem_RSA::US_Astfem_RSA(US_Model &model, US_SimulationParameters &params, QObject *parent) :
...
int US_Astfem_RSA::calculate(US_DataIO::RawData &exp_data) {
   ...
   for (int step = 0; step < simparams.speed_step.count(); step++) {
      if (
         fstime >= simparams.speed_step[ step ].time_first
         && lstime <= simparams.speed_step[ step ].time_last) { // Found step that this dataset fits inside:  break out
         cdsx = step;
         DbgLv(1) << "RSA:calc:   cdsx" << cdsx << "time_first time_last" << simparams.speed_step[ step ].time_first
   ...
   if (simparams.bottom > simparams.bottom_position) {
      double stretch_val = stretch(simparams.rotorcoeffs, ( int ) af_params.cdset_speed);
   ...
   if (simparams.tsobj == NULL || simparams.sim_speed_prof.count() < 1) { // Timestate is not properly loaded
      ...
      DbgLv(1) << "RSA:calc : timestate exists and timestateobject,sscount=" << simparams.tsobj
   ...
   for (int istep = 0; istep < nspstep; istep++) { // Fill time,omega2t work vectors for each step
      ...
      //*DEBUG*
      if (dbg_level > 0) {
         DbgLv(1) << "RSA:calc: readings:from tmst file: istep" << istep << "duration"
                  << simparams.sim_speed_prof[ istep ].duration << "time_ea" << accel_times[ istep ] << "time_e"
                  << time_end[ istep ] << "w2t_ea" << accel_w2ts[ istep ] << "w2t_e" << w2t_end[ istep ];
         for (int jt = 0; jt < simparams.sim_speed_prof[ istep ].duration; jt++) {
            if (jt < 4 || (jt + 4) > simparams.sim_speed_prof[ istep ].duration)
               DbgLv(1) << "RSA:calc::  from_tmst:  rpm" << simparams.sim_speed_prof[ istep ].rpm_timestate[ jt ]
                        << "time" << simparams.sim_speed_prof[ istep ].time_b_accel + jt << "w2t"
   ...
   if (nstep > 0 && nstep < nspstep) { // There are missing speed steps
      ...
      for (int ss = 0; ss < nstep; ss++) {
         DbgLv(1) << "RSA:calc: SSS:  old avgspeed rotspeed setspeed" << ss_old[ ss ].avg_speed
                  << ss_old[ ss ].rotorspeed << ss_old[ ss ].set_speed;
      ...
      for (int ss = 0; ss < nspstep; ss++) {
         ...
         if (ssox >= 0) { // If speed in old speed step vector, just copy it
            ...
            double w2t_first = w2t_eacc + (time_first - time_eacc) * delta_w / delta_t;
            double w2t_last = w2t_eacc + (time_last - time_eacc) * delta_w / delta_t;
   ...
   // Flag: any stretch?
   bool strch_rot = (simparams.rotorcoeffs[ 0 ] > 0.0 || simparams.rotorcoeffs[ 1 ] > 0.0);
   // Flag: multi-speed data?
   bool mspd_data = false;
   double srpm = exp_data.scanData[ 0 ].rpm;
   for (int js = 1; js < exp_data.scanCount(); js++) {
      if (exp_data.scanData[ js ].rpm != srpm) { // Flag at least one speed change within the raw data
         mspd_data = true;
         break;
      }
   }
   ...
   for (int cc = 0; cc < size_cv; cc++) {
      ...
      dr = (af_params.current_bottom - af_params.current_meniscus) / ( double ) (initial_npts - 1);
      ...
      if (!reacting[ cc ]) // noninteracting
      {
         ...
         for (int speed_step = 0; speed_step < nstep; speed_step++) {
            ...
            bool in_step = (ed->scan[ fscan ].time <= sp->time_last && ed->scan[ lscan ].time >= sp->time_first);
            ...
            if (mspd_data && strch_rot) { // For multi-speed data, reset speed-appropriate radii
            ...
            DbgLv(1) << "RSA:calc:  dt" << af_params.dt << "rat omgf simp" << lg_bm_rat << s_omg_fac
                     << simparams.simpoints << "b m omgs s" << af_params.current_bottom << af_params.current_meniscus
            ...
            DbgLv(2) << " CALC:_ni: speed" << avg_speed << "step" << speed_step << "sp-men,bot" << simparams.meniscus
                     << simparams.bottom << "af-men,bot" << af_params.current_meniscus << af_params.current_bottom
...
DbgLv(1) << "RSA:emit ccomp: component" << cc+1;
   ...
   for (int group = 0; group < rg.size(); group++) {
      ...
      for (int jj = 0; jj < num_comp; jj++) {
         ...
         for (int aa = 0; aa < rg[ group ].association.size(); aa++) {
            ...
            for (int rr = 0; rr < as->rcomps.size(); rr++) {
               DbgLv(2) << "RSA:      rr af-index as-react" << rr << af_params.role[ jj ].comp_index
                        << as->rcomps[ rr ];
      ...
      if (simparams.tsobj == NULL || simparams.sim_speed_prof.count() < 1) { // Timestate is not properly loaded
         ...
         DbgLv(1) << "rsa : timestate file exists and timestateobject = " << simparams.tsobj
                  << simparams.sim_speed_prof.count();
      }
      DbgLv(1) << "SS2: ss size" << simparams.speed_step.size() << "ssp size" << simparams.sim_speed_prof.size();
      ...
      for (int step = 0; step < nstep; step++) {
         ...
         DbgLv(2) << "RSA: ***COMPUTED(2) omega_s:" << af_params.omega_s << "step_speed" << step_speed << "s.size s_max"
                  << af_params.s.size() << s_max;

         double lg_bm_rat = log(af_params.current_bottom / af_params.current_meniscus);
         double s_omg_fac = af_params.omega_s * s_max;
         af_params.dt = lg_bm_rat / (s_omg_fac * (simparams.simpoints - 1));
         DbgLv(1) << "simulation_rsa:  dt" << af_params.dt << "rat omgf simp" << lg_bm_rat << s_omg_fac
                  << simparams.simpoints << "b m omgs smax" << af_params.current_bottom << af_params.current_meniscus
         ...
         if (af_params.simpoints > 10000) {
            DbgLv(1) << "RSA: ***COMPUTED simpoints:" << af_params.simpoints << "omg2t s_max dt dura"
                     << af_params.omega_s << s_max << af_params.dt << duration << "** simpts set to 10000 **";
         ...
         current_time = time2;
         current_om2t = omeg2;
         DbgLv(2) << "RSA:    current_time" << current_time << "fscan lscan" << fscan << lscan << "speed" << step_speed;

         // Interpolate the simulated data onto the experimental
         // time and radius grid

         if (ed->scan[ fscan ].time <= sp->time_last && ed->scan[ lscan ].time >= sp->time_first) {
...
DbgLv(2) << "RSA: TimeCorr   correction" << correction << "nscans" << nscans;
   ...
   if ((ncalls % TIMING_RA_INC) < 1) {
      ...
      DbgLv(1) << "   t1 t2 t3 t4 t5 t6 t7 t8" << totT1 << totT2 << totT3 << totT4 << totT5 << totT6 << totT7 << totT8;
   }
#endif
...
void US_Astfem_RSA::adjust_limits(int speed) {
   ...
   if (icset_speed == speed && simparams.bottom > simparams.bottom_position) { // If the current speed step matches the dataset speed step
   ...
   for (int i = 0; i < system.associations.size(); i++) {
      ...
      for (int counter = 1; counter < system.associations.size(); counter++) {
         ...
         for (int j = 0; j < tmp_rg.GroupComponent.size(); j++) {
            ...
            if (
               component1 == ( int ) tmp_rg.GroupComponent[ j ] || component2 == ( int ) tmp_rg.GroupComponent[ j ]
...
void US_Astfem_RSA::initialize_conc(int kk, US_AstfemMath::MfemInitial &CT0, bool noninteracting) {
   ...
   else // af_c0.concentration.size() > 0
   {
      ...
      else // interpolation
      {
         ...
         double dr = (af_params.current_bottom - af_params.current_meniscus) / ( double ) (nval - 1);
...
int US_Astfem_RSA::calculate_ni(
   double rpm_start, double rpm_stop, int step, US_AstfemMath::MfemInitial &C_init, US_AstfemMath::MfemData &simdata,
   bool accel) {
#ifdef NO_DB
   static int Nsave = 0;
   static int Nsavea = 0;
   ...
   double time_dif = (af_params.time_steps > 1 && rpm_stop != rpm_start) ? ( double ) (af_params.time_steps) : 1.0;
   ...
   if (!is_zero) { // If not already into virtual-zero scans, test this one
      ...
      for (int jr = C_init.radius.size() - 1; jr > 1;
         ...
         if (C_init.radius[ jr ] < rad_last) {
            if (C_init.concentration[ jr ] < z_tolerance) { // Concentration very low, so flag simulation zero here on out
               is_zero = true;
            }
   ...
   //is_zero=false;
   //*DEBUG*
   if (is_zero) { // All sim scans will be zero: skip time loop and set last scan time
      ntsteps = 0;
      //ntsteps             = 1;
      ...
      DbgLv(1) << "C_ni:  IS_ZERO !!  1st step time" << last_time << "rpm_stop" << rpm_stop << "s D" << ss0 << dd0
               << "ntsteps" << ntsteps;
   }
...
DbgLv(1) << "simulation_rsa: kt jt" << kt << jt << "scan_time=" << simscan.time
 ...
 << simparams.sim_speed_prof[step].w2t_timestate[kt];
      ...
      if (accel || fixedGrid) {
         ...
         if (af_params.s[ 0 ] > 0) {
            ...
            for (int jx = 2; jx < Nx; jx++) {
               right_hand_side[ jx ] = -CB[ 0 ][ jx ] * C0[ jx - 2 ] - CB[ 1 ][ jx ] * C0[ jx - 1 ]
                                       - CB[ 2 ][ jx ] * C0[ jx ];
            }
         }
         else {
   ...
   if ((nccall % 1000) == 0) {
      ...
      qDebug() << "   ++Elapsed calc-ni ms" << elapsCalc << " nc totC totK" << nccall << ttTC << totTK << " Nx" << Nx
               << " acc fixG" << accel << fixedGrid;
      qDebug() << "     CN: t1 t2 t3 t4 t5 t6 t7 t8" << ttT1 << ttT2 << ttT3 << ttT4 << ttT5 << ttT6 << ttT7 << ttT8;
      int p0 = totTK / 2;
      ...
      qDebug() << "      CN: p1 p2 p3 p4 p5 p6 p7 p8" << p1 << p2 << p3 << p4 << p5 << p6 << p7 << p8;
   }
#endif
   return 0;
}
...
void US_Astfem_RSA::mesh_gen(QVector<double> &nu, int MeshOpt) {
   ...
   DbgLv(1) << "RSA: ***COMPUTED Nx" << Nx << "simpts" << af_params.simpoints << "omg2t" << af_params.omega_s << "s0"
            << af_params.s[ 0 ] << "dt" << af_params.dt;
}
...
void US_Astfem_RSA::mesh_gen_s_pos(const QVector<double> &nuvec) {
   ...
   DbgLv(1) << "RSA:  msg_pos:  m " << meniscus << " b " << bottom << " bmsqdf" << bmsqdf << "af_parms_size"
            << af_params.s.size();
   ...
   for (int i = 0; i < af_params.s.size(); i++) // Markers for steep regions
   {
      ...
      int tmp_Nf = ( int ) (PIhalf * (bottom - tmp_xc) * nu[ i ] * bottom / 2.0 + 0.5) + 1;
   ...
   int mm = x.size() / 2;
   int ee = x.size() - 1;
   DbgLv(1) << "RSA:  mgs_pos: xA sme" << x[ 0 ] << x[ 1 ] << x[ 2 ] << x[ mm - 1 ] << x[ mm ] << x[ mm + 1 ]
            << x[ mm + 2 ] << x[ ee - 2 ] << x[ ee - 1 ] << x[ ee ];
}
...
void US_Astfem_RSA::mesh_gen_s_neg(const QVector<double> &nu) {
   ...
   if (b * (pow(mbratio, (Np - 3.5) / Npm1) - pow(mbratio, (Np - 2.5) / Npm1)) < Hstar || Nf <= 2) {
      double *yrA = yr.data();
      ...
      DbgErr() << "Use exponential grid only!(1/10000 reported):  Np Nf Nm" << Np << Nf << Nm << "m b nu0" << m << b
               << nu0;
   }
   else {
      // Nf > 2
...
void US_Astfem_RSA::mesh_gen_RefL(int N0, int M0) {
   ...
   int mm = x.size() / 2;
   int ee = x.size() - 1;
   DbgLv(1) << "RSA:mgR: Nx xA sme" << Nx << x[ 0 ] << x[ 1 ] << x[ 2 ] << x[ mm - 1 ] << x[ mm ] << x[ mm + 1 ]
            << x[ mm + 2 ] << x[ ee - 2 ] << x[ ee - 1 ] << x[ ee ];
}
...
void US_Astfem_RSA::ComputeCoefMatrixFixedMesh(double D, double sw2, double **CA, double **CB) {
   if (Nx != x.size() || Nx < 1)
      DbgErr() << "***FixedMesh ERROR*** Nx x.size" << Nx << x.size() << " params.s[0] D sw2" << af_params.s[ 0 ] << D
               << sw2;
   ...
   //*DEBUG
   int mm = Nx / 2;
   DbgLv(1) << "RSA:CCMFM: CA0 sme" << CA[ 0 ][ 0 ] << CA[ 0 ][ 1 ] << CA[ 0 ][ 2 ] << CA[ 0 ][ mm - 1 ]
            << CA[ 0 ][ mm ] << CA[ 0 ][ mm + 1 ] << CA[ 0 ][ mm + 2 ] << CA[ 0 ][ Nx - 3 ] << CA[ 0 ][ Nx - 2 ]
            << CA[ 0 ][ Nx - 1 ];
   DbgLv(1) << "RSA:CCMFM: CA1 sme" << CA[ 1 ][ 0 ] << CA[ 1 ][ 1 ] << CA[ 1 ][ 2 ] << CA[ 1 ][ mm - 1 ]
            << CA[ 1 ][ mm ] << CA[ 1 ][ mm + 1 ] << CA[ 1 ][ mm + 2 ] << CA[ 1 ][ Nx - 3 ] << CA[ 1 ][ Nx - 2 ]
            << CA[ 1 ][ Nx - 1 ];
   DbgLv(1) << "RSA:CCMFM: CB0 sme" << CB[ 0 ][ 0 ] << CB[ 0 ][ 1 ] << CB[ 0 ][ 2 ] << CB[ 0 ][ mm - 1 ]
            << CB[ 0 ][ mm ] << CB[ 0 ][ mm + 1 ] << CB[ 0 ][ mm + 2 ] << CB[ 0 ][ Nx - 3 ] << CB[ 0 ][ Nx - 2 ]
            << CB[ 0 ][ Nx - 1 ];
   DbgLv(1) << "RSA:CCMFM: CB1 sme" << CB[ 1 ][ 0 ] << CB[ 1 ][ 1 ] << CB[ 1 ][ 2 ] << CB[ 1 ][ mm - 1 ]
            << CB[ 1 ][ mm ] << CB[ 1 ][ mm + 1 ] << CB[ 1 ][ mm + 2 ] << CB[ 1 ][ Nx - 3 ] << CB[ 1 ][ Nx - 2 ]
            << CB[ 1 ][ Nx - 1 ];
   //*DEBUG
}
...
void US_Astfem_RSA::decompose(US_AstfemMath::MfemInitial *C0) {
   ...
   DbgLv(2) << "RSA:  decompose k_min time_max timeStepSize" << k_min << time_max << timeStepSize;
...
void US_Astfem_RSA::ReactionOneStep_Euler_imp(int Npts, double **C1, double timeStep) {
   int num_comp = af_params.role.size();
   DbgLv(2) << "RSA:Eul: Npts timeStep" << Npts << timeStep << "num_comp" << num_comp;
...
void US_Astfem_RSA::Reaction_dydt(double *y0, double *yt) {
   ...
   for (int m = 0; m < num_rule; m++) {
      ...
      for (int n = 0; n < as->rcomps.size(); n++) {
         ...
         //if(m==0)
         DbgLv(2) << "RSA:ReDydt:   m n ind_cn" << m << n << ind_cn << "rstoi react extn" << rstoi << react << extn;
...
int US_Astfem_RSA::calculate_ra2(
   double rpm_start, double rpm_stop, US_AstfemMath::MfemInitial *C_init, US_AstfemMath::MfemData &simdata, bool accel) {
   ...
   DbgLv(1) << "RSA:_ra2: Mcomp Nx" << Mcomp << Nx << "x size" << x.size() << "D0 nu0 D1 nu1" << af_params.D[ 0 ]
            << nu[ 0 ] << af_params.D[ 1 ] << nu[ 1 ];
   ...
   if (accel) {
      ...
      if (s_min > 0.0) // all sediment towards bottom
      {
         ...
         DbgLv(1) << "RSA:_ra2: s_min s_max" << s_min << s_max << "xc xAj" << xc << xA[ j ] << "j" << j << "N0 M0"
                  << (j + 1) << (j * 4);
      }
      ...
      else // Moving grid
      {
         if (s_min > 0) // All components sedimenting
         {
            ...
            for (int i = 0; i < Mcomp; i++) {
               ...
               DbgLv(1) << "RSA: smin>0:GlStf: i" << i << "Si Di sw2 sw2D sqb s_max" << af_params.s[ i ]
               ...
               for (int j = 1; j < Nx; j++) {
                  double dval = 0.1 * exp(sw2D * (sq(0.5 * (xA[ j - 1 ] + xA[ j ])) - sqb));
                  ...
                  if (j < 4 || j == (Nx / 2) || (j + 4) > Nx)
                     DbgLv(1) << "RSA: smin>0:GlStf:   j" << j << "dval alpha xbj xAj" << dval << alpha << xbvec[ j ]
                              << xA[ j ];
               }
   ...
   DbgLv(1) << "RSA: newX3 Nx" << Nx << "C0[00] C0[m0] C0[0n] C0[mn]" << C0[ 0 ][ 0 ] << C0[ Mcomp - 1 ][ 0 ]
            << C0[ 0 ][ Nx - 1 ] << C0[ Mcomp - 1 ][ Nx - 1 ];
   ...
   for (int kkk = 0; kkk < Nt + 2; kkk += 2) // two steps in together
   {
      ...
      DbgLv(2) << "TMS:RSA:ra: time omegast" << simscan.time << simscan.omega_s_t << "step-scan" << simdata.scan.size();

      simscan.conc.clear();
      simscan.conc.reserve(Nx);
      DbgLv(2) << "RSA: Nx CT0size" << Nx << CT0vec.size() << "step-scan" << simdata.scan.size() << "rss-now"
               << US_Memory::rss_now();

      for (int jj = 0; jj < Nx; jj++)
         simscan.conc.append(CT0[ jj ]);

      DbgLv(2) << "TMS:RSA:ra:  kkk" << kkk << "CT0[0] CT0[n]" << CT0[ 0 ] << CT0[ Nx - 1 ] << "accel fixedGrid"
               << accel << fixedGrid;
      ...
      if (accel || fixedGrid) // For fixed grid
      {
         for (int i = 0; i < Mcomp; i++) {
            ...
            US_AstfemMath::tridiag(CA[ i ][ 0 ], CA[ i ][ 1 ], CA[ i ][ 2 ], right_hand_side, C1[ i ], Nx);
         }
      }
      ...
      else // Moving grid
      {
         ...
         for (int i = 0; i < Mcomp; i++) {
            ...
            for (int j = 2; j < Nx - 1; j++) {
               right_hand_side[ j ] = -CB[ i ][ 0 ][ j ] * C0[ i ][ j - 2 ] - CB[ i ][ 1 ][ j ] * C0[ i ][ j - 1 ]
            ...
            DbgLv(2) << "TMS:RSA:ra:   MovGrid:  i" << i << "CB[0]" << CB[ i ][ 2 ][ 0 ] << CB[ i ][ 3 ][ 0 ]
                     << CB[ i ][ 1 ][ 1 ] << CB[ i ][ 2 ][ 1 ] << CB[ i ][ 3 ][ 1 ];
            DbgLv(2) << "TMS:RSA:ra:   MovGrid:   i" << i << "CA[0]" << CA[ i ][ 0 ][ 0 ] << CA[ i ][ 1 ][ 0 ]
                     << CA[ i ][ 0 ][ 0 ] << CA[ i ][ 1 ][ 0 ] << CA[ i ][ 2 ][ 0 ];

            US_AstfemMath::QuadSolver(
               CA[ i ][ 0 ], CA[ i ][ 1 ], CA[ i ][ 2 ], CA[ i ][ 3 ], right_hand_side, C1[ i ], Nx);
            DbgLv(2) << "TMS:RSA:ra:   MovGrid:    i" << i << "C1[i]" << C1[ i ][ 0 ] << C1[ i ][ Nx - 4 ]
                     << C1[ i ][ Nx - 3 ] << C1[ i ][ Nx - 2 ] << C1[ i ][ Nx - 1 ];
            DbgLv(2) << "TMS:RSA:ra:   MovGrid:    i" << i << "C0[i]" << C0[ i ][ 0 ] << C0[ i ][ Nx - 4 ]
                     << C0[ i ][ Nx - 3 ] << C0[ i ][ Nx - 2 ] << C0[ i ][ Nx - 1 ];
         }
      ...
      DbgLv(2) << "TMS:RSA:ra:   C1[10] C1[11] C1[1k] C1[1n]" << C1[ 1 ][ 0 ] << C1[ 1 ][ 1 ] << C1[ 1 ][ Nx - 2 ]
               << C1[ 1 ][ Nx - 1 ];

      ReactionOneStep_Euler_imp(Nx, C1, 2 * af_params.dt);

      DbgLv(2) << "TMS:RSA:ra(2):   C1[10] C1[11] C1[1k] C1[1n]" << C1[ 1 ][ 0 ] << C1[ 1 ][ 1 ] << C1[ 1 ][ Nx - 2 ]
               << C1[ 1 ][ Nx - 1 ];
   ...
   for (int i = 0; i < Mcomp; i++) {
      ...
      DbgLv(2) << "TMS:RSA:ra2: comp:" << i << "C1[i]:" << C1[ i ][ 0 ] << C1[ i ][ 1 ] << C1[ i ][ Nx / 2 ]
               << C1[ i ][ Nx - 3 ] << C1[ i ][ Nx - 2 ] << C1[ i ][ Nx - 1 ];
   }
...
void US_Astfem_RSA::GlobalStiff(double *xb, double **ca, double **cb, double D, double sw2) {
...
void US_Astfem_RSA::load_mfem_data(US_DataIO::RawData &edata, US_AstfemMath::MfemData &fdata) {
   ...
   for (int ii = 0; ii < nscan; ii++) {
      ...
      if (ii < 3)
         DbgLv(2) << "RSA:f  ii c0 cn" << ii << fscan->conc[ 0 ] << fscan->conc[ nconc - 1 ] << "d0 dn"
                  << edata.scanData[ ii ].rvalues[ 0 ] << edata.scanData[ ii ].rvalues[ nconc - 1 ];
#if 1
   }
#endif
#if 0
...
void US_Astfem_RSA::store_mfem_data(US_DataIO::RawData &edata, US_AstfemMath::MfemData &fdata) {
...
QString US_Astfem_RSA::temp_Id_name() {
   return ("p" + QString::number(getpid()) + "t" + QDateTime::currentDateTime().toUTC().toString("yyMMddhhmmss"));
}
...
bool US_Astfem_RSA::iszero(
   const double s, const double D, const double rpm, const double t, const double meniscus, const double bottom) {
   ...
   double back_diffusion = tolerance * sqr_D / (s * omega_sq * (bottom + meniscus) * sqr_t);
   ...
   DbgLv(1) << "IS_Z: radD" << radD << "s_dist" << s_distance << "back_diffus" << back_diffusion << "xval" << xval
            << "cleared" << cleared << "is_zero" << is_zero;
   DbgLv(1) << "IS_Z:    s D rpm t men bot" << s << D << rpm << t << meniscus << bottom << "   omega_sq  sqr_t"
            << omega_sq << sqr_t;
