...
int US_TimeState::set_key(QString key, QString format) {
  ...
  if (QString("FIC").indexOf(fl1) < 0) {
    ...
    set_error(status);
    error_msg += format;
    // DbgLv(1) << "TS: key format" << key << format << "stat errm" << status <<
    // error_msg;
    return status;
  }
    ...
    switch (rfmt) {
      ...
      case 5:  // Cnnn
        ...
        if (svalue.length() < rlen) {  // Pad with blanks
          for (int jj = svalue.length(); jj < rlen; jj++) wstr += QString(" ");
        }
...
int US_TimeState::write_defs(double timeinc, QString imptype) {
  ...
  if (!xfo.open(QIODevice::WriteOnly |
                QIODevice::Text)) {  // Error opening the file for write
    status = 700;
    set_error(status);
    return status;
  }
  ...
  for (int jj = 0; jj < nvalues;
       jj++) {  // Key,Format for each value field in the records
...
int US_TimeState::open_read_data(QString fpath, const bool pfetch) {
  ...
  if (strncmp(cdata, _TMST_MAGI_, 4) !=
      0) {  // Error in magic number (wrong kind of file?)
    status = 501;
    set_error(status);
  ...
  if (!xfi.open(QIODevice::ReadOnly |
                QIODevice::Text)) {  // Error opening xml definitions for read
    status = 505;
    set_error(status);
    return status;
  }
...
int US_TimeState::time_count() { return ntimes; }

// Get the set-speed resolution
int US_TimeState::ss_resolution() { return ss_reso; }
...
bool US_TimeState::origin(QString* dversP, QString* itypeP) {
  ...
  if (dversP != NULL) *dversP = fvers;

  if (itypeP != NULL) *itypeP = itype;
...
int US_TimeState::field_keys(QStringList* keysP, QStringList* fmtsP) {
  if (keysP != NULL) *keysP = keys;

  if (fmtsP != NULL) *fmtsP = fmts;
...
int US_TimeState::read_record(const int rtimex) {
  ...
  if (timex <
      0) {  // If current index indicates still in header, complete header calcs
    int lstv = nvalues - 1;
  ...
  if (rtimex < 0 || (rtimex - timex) == 1) {  // Set to next and read it in
    ...
    dsi->readRawData(cdata, rec_size);
           ...
           timex) {  // Read in records until we have reached the designated one
    while (timex < rtimex) {
      timex++;
      dsi->readRawData(cdata, rec_size);
    }
...
int US_TimeState::time_ivalue(const QString key, int* stat) {
  ...
  if (status == 0) {
    ...
    switch (rfmt) {  // Fetch the value in this key's format; get integer output
                     // value
      case 0:        // I4
        ivalue = iword(rdata);
        break;
...
double US_TimeState::time_dvalue(const QString key, int* stat) {
  ...
  if (status == 0) {
    ...
    switch (rfmt) {  // Fetch the value in this key's format; get double output
                     // value
      case 0:        // I4
        ivalue = iword(rdata);
        dvalue = (double)ivalue;
...
QString US_TimeState::time_svalue(const QString key, int* stat) {
  ...
  if (status == 0) {
    switch (rfmt) {  // Fetch the value in this key's format; get string output
                     // value
      case 0:        // I4
        ivalue = iword(rdata);
...
QString US_TimeState::error_message(int status) {
  ...
  for (int ii = 0; ii < netypes;
       ii++) {  // Find a match to the specified error status code
    if (emap[ii].estat == status) {  // A status code match is found, so get the
...
QString US_TimeState::last_error_message() { return error_msg; }
...
int US_TimeState::dbCreate(US_DB2* dbP, const int expID, const QString fpath) {
  ...
  if (stat == US_DB2::OK) {  // We have an already existing DB record, so check
                             // how it matches local
    if (cksumdb ==
        tmst_cksm) {  // Binary data appears to match, so check xml defs
      QByteArray defsdb = xdefs.toUtf8();
      QString cksumxd =
          QString(QCryptographicHash::hash(defsdb, QCryptographicHash::Md5)
                      .toHex()) +
          " " + QString::number(defsdb.size());

      if (tmstID > 0 &&
          cksumxd == defs_cksm) {     // Xml definitions characters also match,
                                      // return with existing DB
        if (fnamedb != tmst_fname) {  // If the file name differs, we must
                                      // update the record
          dbP->mysqlEscapeString(defs_ld, defs_da, defs_da.size());
          query.clear();
          query << "update_timestate" << idTmst << idExp << tmst_fname
                << defs_ld;
  ...
  //  << "idExp tmstID nsrtID dsiz" << idExp << tmstID << nsrtID <<
  //  defs_da.size();
...
int US_TimeState::dbExamine(US_DB2* dbP, int* tmstIdP, int* expIdP,
                            QString* fnameP, QString* xdefsP, QString* cksumP,
                            QDateTime* lastupdP) {
  int dbg_level = US_Settings::us_debug();
  DbgLv(1) << "dbExamine: dbP tmstID expID fname xdefs cksum lastupd" << dbP
           << tmstIdP << expIdP << fnameP << xdefsP << cksumP << lastupdP;
  QStringList query;
  ...
  if (tmstID > 0) {  // TimeState ID is given, get experimentID; then fall thru
                     // for the rest
    query.clear();
    query << "get_timestate" << QString::number(tmstID);
    dbP->query(query);
    status = dbP->lastErrno();
    DbgLv(1) << "dbExamine:  get_timestate tmstID" << tmstID << "status"
             << status;
    if (status != US_DB2::OK) return status;

    // int nrows=dbP->numRows();
    dbP->next();
    expID = dbP->value(0).toString().toInt();
    // DbgLv(1) << "dbExamine:  get_timestate expID" << expID << "nrows" <<
    // nrows;
    if (expIdP != NULL) *expIdP = expID;
  }

  else if (expID > 0) {  // Experiment ID is given, get timestateID; then fall
                         // thru for the rest
                         // query.clear();
    // query << "get_experiment2_timestate" << QString::number( expID );
    // dbP->query( query );
    // status=dbP->lastErrno();
    // DbgLv(1) << "dbExamine:  get_experiment2_timestate expID status" << expID
    // << status;
    query.clear();
    query << "get_experiment_timestate" << QString::number(expID);
    dbP->query(query);
    status = dbP->lastErrno();
    DbgLv(1) << "dbExamine:  get_experiment_timestate expID status" << expID
             << status;
    if (status != US_DB2::OK) return status;
    ...
    if (tmstIdP != NULL) *tmstIdP = tmstID;
  }
  ...
  if (lastupdP != NULL)  // Return last-updated datetime if requested
    *lastupdP =
        QDateTime::fromString(dbP->value(5).toString(), Qt::ISODate).toUTC();
...
int US_TimeState::dbDownload(US_DB2* dbP, const int tmstID,
  ...
  int status =
      dbP->readBlobFromDB(fpath, QString("download_timestate"), tmstID);
  return status;
}
...
bool US_TimeState::dbSyncToLF(US_DB2* dbP, const QString fpath,
  ...
  int statD =
      dbExamine(dbP, &tmstID, &expID1, &fpath1, &xdefs, &cksumD, &drecUpd);
  ...
  if (statD != US_DB2::OK) return newfile;

  // Now check any existing TimeState file
  bool needF = false;

  if (QFile(fpath).exists()) {  // The file exists, so we need to check its
                                // cksum and size
    cksumF = US_Util::md5sum_file(fpath);
    DbgLv(1) << "dbSTLF: LF exists : cksumF" << cksumF;

    if (cksumF == cksumD) {  // DB,Local records match in checksum and size, so
                             // no download
      return newfile;
    }
...
int US_TimeState::key_parameters(const QString key, int* rfmtP, int* rlenP,
                                 int* roffP) {
  int status = 0;
