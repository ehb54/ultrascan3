...
int
US_TimeState::open_write_data (QString fpath, double timeinc, double ftime)
{
  int status = 0;
...
int
US_TimeState::set_key (QString key, QString format)
{
  int status = 0;
...
int
US_TimeState::set_keys (QStringList &fkeys, QStringList &ffmts)
{
  int status = 0;
...
// Specify keys and formats for all data fields
int
US_TimeState::set_keys (QStringList &fkeys)
{
  QStringList wkeys;
  QStringList wfmts;
...
int
US_TimeState::set_value (QString key, int ivalue)
{
  int status = 0;
...
int
US_TimeState::set_value (QString key, double dvalue)
{
  int status = 0;
...
int
US_TimeState::set_value (QString key, QString svalue)
{
  int status = 0;
...
int
US_TimeState::write_defs (double timeinc, QString imptype)
{
  int status = 0;
...
int
US_TimeState::open_read_data (QString fpath, const bool pfetch)
{
  int status = 0;
  file_size = (qint64)0;
...
int
US_TimeState::time_range (bool *constti, double *timeinc, double *ftime)
{
  if (constti != NULL)
    *constti = const_ti;
  if (timeinc != NULL)
...
bool
US_TimeState::origin (QString *dversP, QString *itypeP)
{
...
int
US_TimeState::field_keys (QStringList *keysP, QStringList *fmtsP)
{
  if (keysP != NULL)
    *keysP = keys;
...
int
US_TimeState::read_record (const int rtimex)
{
  int status = 0;
...
int
US_TimeState::time_ivalue (const QString key, int *stat)
{
  int ivalue = 0;
  int rfmt = 0;
  int rlen = 4;
...
double
US_TimeState::time_dvalue (const QString key, int *stat)
{
  double dvalue = 0.0;
  int rfmt = 0;
  int rlen = 4;
...
QString
US_TimeState::time_svalue (const QString key, int *stat)
{
  QString svalue;
  int ivalue;
  double dvalue;
...
QString
US_TimeState::error_message (int status)
{
...
QString
US_TimeState::last_error_message ()
{
  return error_msg;
}
...
int
US_TimeState::dbCreate (US_DB2 *dbP, const int expID, const QString fpath)
{
  ...
  DbgLv (1) << "dbCreate: tmst_fpath" << tmst_fpath << "exists"
            << dfi.exists ();
  DbgLv (1) << "dbCreate: defs_fpath" << defs_fpath << "exists"
            << xfi.exists ();
...
int
US_TimeState::dbDelete (US_DB2 *dbP, const int tmstID)
{
  int dbg_level = US_Settings::us_debug ();
...
int
US_TimeState::dbExamine (US_DB2 *dbP, int *tmstIdP, int *expIdP,
                         ...
                         QDateTime *lastupdP)
{
  ...
  if (tmstID > 0)
    ...
    { // Experiment ID is given, get timestateID; then fall thru for the rest
      ...
      // dbP->query( query );
      // status=dbP->lastErrno();
      // DbgLv(1) << "dbExamine:  get_experiment2_timestate expID status" <<
      // expID << status;
      query.clear ();
      ...
      status = dbP->lastErrno ();
      // DbgLv(1) << "dbExamine:  get_experiment_timestate  next status" <<
      // status
      //  << "have_next" << havenx;
  ...
  if (lastupdP != NULL) // Return last-updated datetime if requested
    *lastupdP = QDateTime::fromString (dbP->value (5).toString (), Qt::ISODate)
                    .toUTC ();
...
int
US_TimeState::dbDownload (US_DB2 *dbP, const int tmstID, const QString fpath)
{
  int dbg_level = US_Settings::us_debug ();
...
int
US_TimeState::dbUpload (US_DB2 *dbP, const int tmstID, const QString fpath)
{
  int dbg_level = US_Settings::us_debug ();
  DbgLv (1) << "dbUpload: dbP tmstID fpath" << dbP << tmstID << fpath;
  int status
      = dbP->writeBlobToDB (fpath, QString ("upload_timestate"), tmstID);
  return status;
}

// Static function to synchronze a TMST in the DB to a local file
bool
US_TimeState::dbSyncToLF (US_DB2 *dbP, const QString fpath, const int expID)
{
  int dbg_level = US_Settings::us_debug ();
...
void
US_TimeState::store_hword (char *dba, int ival)
{
  unsigned char *cbuf = (unsigned char *)dba;
  unsigned char vbuf[8];
  int *vint = (int *)vbuf;
...
void
US_TimeState::store_iword (char *dba, int ival)
{
  unsigned char *cbuf = (unsigned char *)dba;
  unsigned char vbuf[8];
  int *vint = (int *)vbuf;
...
void
US_TimeState::store_fword (char *dba, double dval)
{
...
void
US_TimeState::store_dword (char *dba, double dval)
{
  double dwrk = dval;
  int *iptr = (int *)&dwrk;
...
void
US_TimeState::store_hwords (char *dba, int *ivals, int nvals)
{
  char *cptr = dba;
...
void
US_TimeState::store_iwords (char *dba, int *ivals, int nvals)
{
  char *cptr = dba;
...
void
US_TimeState::store_fwords (char *dba, double *dvals, int nvals)
{
  char *cptr = dba;
...
int
US_TimeState::set_error (int status)
{
...
int
US_TimeState::key_parameters (const QString key, int *rfmtP, int *rlenP,
                              int *roffP)
{
  int status = 0;
