...
int US_TimeState::open_write_data(QString fpath, double timeinc, double ftime) {
   int status = 0;
   ...
   if (status == 0) {
      ...
      switch (rfmt) {
         ...
         case 5: // Cnnn
            ...
            if (svalue.length() < rlen) { // Pad with blanks
...
int US_TimeState::write_defs(double timeinc, QString imptype) {
   ...
   if (!xfo.open(QIODevice::WriteOnly | QIODevice::Text)) { // Error opening the file for write
      status = 700;
      set_error(status);
      return status;
   }
   ...
   for (int jj = 0; jj < nvalues; jj++) { // Key,Format for each value field in the records
...
int US_TimeState::open_read_data(QString fpath, const bool pfetch) {
   ...
   if (!filei->open(QIODevice::ReadOnly)) { // Error opening file for read
      status = 500;
      set_error(status);
      error_msg += fpath;
      return status;
   ...
   filepath = fpath;
   filename = filepath.section("/", -1, -1);
   if (pre_fetch) { // If pre-fetch, read in all data bytes and close file
   ...
   else { // By default, create data stream from opened file
   ...
   if (strncmp(cdata, _TMST_MAGI_, 4) != 0) { // Error in magic number (wrong kind of file?)
      status = 501;
      set_error(status);
      error_msg += QString(_TMST_MAGI_) + " " + QString(cdata).left(4);
      return status;
   }
   ...
   if (!xfi.open(QIODevice::ReadOnly | QIODevice::Text)) { // Error opening xml definitions for read
      status = 505;
      set_error(status);
      return status;
   }
   ...
   while (!xml.atEnd()) { // Read definition elements
      QString xname;
      QString attv;
      ...
      if (xml.isStartElement()) {
         ...
         if (xname == "TimeState") { // Parse file/object version and import type
            fvers = attr.value("version").toString();
            imp_type = attr.value("import_type").toString();
         }

         else if (xname == "file") { // Parse overall file attributes
         ...
         else if (xname == "value") { // Parse the attributes of a record field
...
int US_TimeState::read_record(const int rtimex) {
   ...
   if (timex < 0) { // If current index indicates still in header, complete header calcs
      int lstv = nvalues - 1;
      QString fm = fmts[ lstv ];
      rec_size = offs[ lstv ] + fm.mid(1).toInt();
   }

   if (rtimex < 0 || (rtimex - timex) == 1) { // Set to next and read it in
      timex++;
      dsi->readRawData(cdata, rec_size);
   }

   else if (rtimex > timex) { // Read in records until we have reached the designated one
      while (timex < rtimex) {
         timex++;
         dsi->readRawData(cdata, rec_size);
      }
   ...
   else { // Error if designated time index is less than current
...
int US_TimeState::time_ivalue(const QString key, int *stat) {
   ...
   if (status == 0) {
      ...
      switch (rfmt) { // Fetch the value in this key's format; get integer output value
         case 0: // I4
            ivalue = iword(rdata);
            break;
         case 1: // I2
...
double US_TimeState::time_dvalue(const QString key, int *stat) {
   ...
   if (status == 0) {
      ...
      switch (rfmt) { // Fetch the value in this key's format; get double output value
         case 0: // I4
            ivalue = iword(rdata);
            dvalue = ( double ) ivalue;
            break;
...
QString US_TimeState::time_svalue(const QString key, int *stat) {
   ...
   if (status == 0) {
      switch (rfmt) { // Fetch the value in this key's format; get string output value
         case 0: // I4
            ivalue = iword(rdata);
...
QString US_TimeState::error_message(int status) {
   ...
   for (int ii = 0; ii < netypes; ii++) { // Find a match to the specified error status code
      if (emap[ ii ].estat == status) { // A status code match is found, so get the corresponding message
         errmsg = emap[ ii ].emsg;
         break;
      }
   }
...
int US_TimeState::dbCreate(US_DB2 *dbP, const int expID, const QString fpath) {
   ...
   int stat = dbExamine(dbP, &tmstID, &expIDdb, &fnamedb, &xdefs, &cksumdb);
   //DbgLv(1) << "dbCreate:  dbExam stat" << stat << "tmstID expID"
   // << tmstID << expIDdb << "cksumdb" << cksumdb;
   if (stat == US_DB2::OK) { // We have an already existing DB record, so check how it matches local
      if (cksumdb == tmst_cksm) { // Binary data appears to match, so check xml defs
         QByteArray defsdb = xdefs.toUtf8();
         QString cksumxd = QString(QCryptographicHash::hash(defsdb, QCryptographicHash::Md5).toHex()) + " "
                           + QString::number(defsdb.size());

         if (tmstID > 0 && cksumxd == defs_cksm) { // Xml definitions characters also match, return with existing DB
            if (fnamedb != tmst_fname) { // If the file name differs, we must update the record
               dbP->mysqlEscapeString(defs_ld, defs_da, defs_da.size());
               query.clear();
               query << "update_timestate" << idTmst << idExp << tmst_fname << defs_ld;
   ...
   tmstID = (stat == US_DB2::OK || stat == US_DB2::NOROWS) ? nsrtID : -4;
...
int US_TimeState::dbExamine(
   US_DB2 *dbP, int *tmstIdP, int *expIdP, QString *fnameP, QString *xdefsP, QString *cksumP, QDateTime *lastupdP) {
   int dbg_level = US_Settings::us_debug();
   DbgLv(1) << "dbExamine: dbP tmstID expID fname xdefs cksum lastupd" << dbP << tmstIdP << expIdP << fnameP << xdefsP
            << cksumP << lastupdP;
   QStringList query;
   ...
   if (tmstID > 0) { // TimeState ID is given, get experimentID; then fall thru for the rest
      query.clear();
   ...
   else if (expID > 0) { // Experiment ID is given, get timestateID; then fall thru for the rest
      //query.clear();
      ...
      DbgLv(1) << "dbExamine:  get_experiment_timestate   tmstID" << tmstID << dbP->value(0).toString()
               << dbP->value(1).toString();
      if (tmstIdP != NULL)
         *tmstIdP = tmstID;
   }

   else { // Neither tmstID nor expID given
   ...
   if (cksumP != NULL) // Return cksum+size string if requested
      *cksumP = dbP->value(3).toString() + " " + dbP->value(4).toString();
   //if(cksumP!=NULL)
   //DbgLv(1) << "dbExamine:  cksum-db" << *cksumP;

   if (lastupdP != NULL) // Return last-updated datetime if requested
      *lastupdP = QDateTime::fromString(dbP->value(5).toString(), Qt::ISODate).toUTC();

   return status;
}

// Static function to download a TMST binary data record from the DB
int US_TimeState::dbDownload(US_DB2 *dbP, const int tmstID, const QString fpath) {
   int dbg_level = US_Settings::us_debug();
   DbgLv(1) << "dbDownload: dbP tmstID fpath" << dbP << tmstID << fpath;
   int status = dbP->readBlobFromDB(fpath, QString("download_timestate"), tmstID);
   return status;
}

// Static function to upload a TMST binary data record to the DB
int US_TimeState::dbUpload(US_DB2 *dbP, const int tmstID, const QString fpath) {
   int dbg_level = US_Settings::us_debug();
   DbgLv(1) << "dbUpload: dbP tmstID fpath" << dbP << tmstID << fpath;
   int status = dbP->writeBlobToDB(fpath, QString("upload_timestate"), tmstID);
   return status;
}

// Static function to synchronze a TMST in the DB to a local file
bool US_TimeState::dbSyncToLF(US_DB2 *dbP, const QString fpath, const int expID) {
   int dbg_level = US_Settings::us_debug();
   ...
   int statD = dbExamine(dbP, &tmstID, &expID1, &fpath1, &xdefs, &cksumD, &drecUpd);
   DbgLv(1) << "dbSTLF: dbExamine return";
   DbgLv(1) << "dbSTLF: dbExamine statD=" << statD << "tmstID=" << tmstID << "cksumD" << cksumD << "drecUpd"
            << drecUpd.toString();
   ...
   if (QFile(fpath).exists()) { // The file exists, so we need to check its cksum and size
      cksumF = US_Util::md5sum_file(fpath);
      DbgLv(1) << "dbSTLF: LF exists : cksumF" << cksumF;

      if (cksumF == cksumD) { // DB,Local records match in checksum and size, so no download
         return newfile;
      }
      ...
      DbgLv(1) << "dbSTLF: sizeD sizeF" << sizeD << sizeF << "frecUpd" << frecUpd.toString();
      // Flag file update if DB record newer than local or larger
      needF = (drecUpd > frecUpd || sizeD > sizeF);
   }

   else { // The local file does not exist, so we need to create it
      needF = true;
   }
   DbgLv(1) << "dbSTLF:  NEEDF=" << needF;

   if (needF) { // We need a (new) local file copy
...
void US_TimeState::store_hword(char *dba, int ival) {
   ...
   *vint = ival;
   ...
   else { // Surprise! big-endian:  hi,lo order
...
int US_TimeState::key_parameters(const QString key, int *rfmtP, int *rlenP, int *roffP) {
   int status = 0;
   ...
   if (rfmt < 0 || rlen < 1 || roff < 0) {
      ...
      DbgLv(1) << "DtsF: keyx" << keyx << "fmt" << fmt << "ftst" << ftst << "keys sz" << keys.count() << "fmts sz"
               << fmts.count();
   }
