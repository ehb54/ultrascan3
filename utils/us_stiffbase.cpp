...
void
US_StiffBase::CompLocalStif (int NK, double xd[4][2], double D, double sw2,
                             double **Stif)
{
  double xg[2];
  double jcbv[5];
          ...
          for (int j = 0; j < NK; j++)
            {
              double tmp
                  = xg[0] * phiy[j] * phi[i]
                    + (D * xg[0] * phix[j] - sw2 * xg[0] * xg[0] * phi[j])
                          * phix[i];
...
void
US_StiffBase::LambdaG (uint kk, double lam1, double lam2, double w,
                       Gauss2D *Lm)
{
  ...
  switch (n_gaussQ)
    ...
    default:
      qDebug () << "Gauss Quadrature on Quad elem with "
                << QString::number (n_gaussQ) << " points not implemented";
      return;
    }
  ...
  switch (n_gaussT)
    ...
    default:
      qDebug () << "Gauss Quadrature on Triangular elem with "
                << QString::number (n_gaussT) << " not implemented";
    }
}

void
US_StiffBase::AffineMapping (int NK, double xd[4][2], int gauss_ind,
                             double x[5])
{
  x[0] = 0.0;
  x[1] = 0.0;
...
void
US_StiffBase::Jacobian (int NK, double xd[4][2], int gauss_ind, double jcbv[5])
{
  double J11 = 0.0;
  double J12 = 0.0;
  double J21 = 0.0;
