...
void US_RunProtocol::timeFromList(double& sectime, QList<int>& dhms) {
  sectime =
      dhms[0] * (24 * 60 * 60) + dhms[1] * (60 * 60) + dhms[2] * 60 + dhms[3];
}
...
void US_RunProtocol::timeToString(double& sectime, QString& strtime) {
  ...
  strtime = QString().sprintf("%dd %02d:%02d:%02d", dhms[0], dhms[1], dhms[2],
                              dhms[3]);
}
  ...
  while (!xmli.atEnd()) {
    ...
    if (xmli.isStartElement()) {
      if (ename == "speedstep") {
        ...
        if (!s_si.isEmpty()) US_RunProtocol::timeFromString(ss.scanintv, s_si);
  ...
  while (!xmli.atEnd()) {
    ...
    if (xmli.isStartElement()) {
      if (ename == "cells") {
        ...
        qDebug() << "fX:cel: cel cen win bal" << cu.cell << cu.centerpiece
                 << cu.windows << cu.cbalance;
        used << cu;
      }
    }
...
bool US_RunProtocol::RunProtoSolutions::operator==(
  ...
  if (nschan > 0 && rp.nschan > 0)
    qDebug() << "RP:SOL: chs" << chsols[0].channel << rp.chsols[0].channel
             << chsols[0].solution << rp.chsols[0].solution << chsols[0].sol_id
             << rp.chsols[0].sol_id << chsols[0].ch_comment
