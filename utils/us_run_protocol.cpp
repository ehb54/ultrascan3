...
bool
US_RunProtocol::operator== (const US_RunProtocol &rp) const
{
  qDebug () << "RP:eq?";
  if (investigator != rp.investigator)
    return false;
  qDebug () << "RP: EQ inv";
  if (protoname != rp.protoname)
    return false;
  qDebug () << "RP: EQ pnm";
  if (protoGUID != rp.protoGUID)
    return false;
  qDebug () << "RP: EQ pgu";
  if (optimahost != rp.optimahost)
    return false;
  qDebug () << "RP: EQ oho";
  if (temperature != rp.temperature)
    return false;
  qDebug () << "RP: EQ tem";
  if (temeq_delay != rp.temeq_delay)
    return false;
  qDebug () << "RP: EQ dly";

  if (rpRotor != rp.rpRotor)
    return false;
  qDebug () << "RP: EQ ROT";
  if (rpSpeed != rp.rpSpeed)
    return false;
  qDebug () << "RP: EQ SPD";
  if (rpCells != rp.rpCells)
    return false;
  qDebug () << "RP: EQ CEL";
  if (rpSolut != rp.rpSolut)
    return false;
  qDebug () << "RP: EQ SOL";
  if (rpOptic != rp.rpOptic)
    return false;
  qDebug () << "RP: EQ OPT";
  if (rpRange != rp.rpRange)
    return false;
  qDebug () << "RP: EQ RNG";
  if (rpAprof != rp.rpAprof)
    return false;
  qDebug () << "RP: EQ APR";
...
bool
US_RunProtocol::toXml (QXmlStreamWriter &xmlo)
{
...
bool
US_RunProtocol::fromXml (QXmlStreamReader &xmli)
{
  while (!xmli.atEnd ())
    {
      xmli.readNext ();
      ...
      if (xmli.isStartElement ())
        {
          ...
          if (ename == "protocol")
            ...
            {
              rpRange.fromXml (xmli);
            }
          else if (ename == "spectra")
            {
              rpRange.fromXml (xmli);
            }
          else if (ename == "aprofile")
            {
              rpAprof.fromXml (xmli);
            }
        }
    }
...
void
US_RunProtocol::timeToList (double &sectime, QList<int> &dhms)
{
...
// Function to convert from a time to a day,hour,minute,second list
void
US_RunProtocol::timeToList (QTime &timeobj, int &days, QList<int> &dhms)
{
  dhms.clear ();
  dhms << days << timeobj.hour () << timeobj.minute () << timeobj.second ();
}

// Function to convert to a time from a day,hour,minute,second list
void
US_RunProtocol::timeFromList (double &sectime, QList<int> &dhms)
{
  sectime = dhms[0] * (24 * 60 * 60) + dhms[1] * (60 * 60) + dhms[2] * 60
            + dhms[3];
}

// Function to convert to a time from a day,hour,minute,second list
void
US_RunProtocol::timeFromList (QTime &timeobj, int &days, QList<int> &dhms)
{
...
void
US_RunProtocol::timeToString (double &sectime, QString &strtime)
{
  QList<int> dhms;
  // timeToList( sectime, dhms );         //ALEXEY: bug!!!
  double sectime_to_list = sectime;
  timeToList (sectime_to_list, dhms);
  strtime = QString ().sprintf ("%dd %02d:%02d:%02d", dhms[0], dhms[1],
                                dhms[2], dhms[3]);
}

// Function to convert from a time to "0d 00:06:30" type string
void
US_RunProtocol::timeToString (QTime &timeobj, int &days, QString &strtime)
{
  strtime = QString ().sprintf ("%dd %02d:%02d:%02d", days, timeobj.hour (),
                                timeobj.minute (), timeobj.second ());
}

// Function to convert to a time from a "0d 00:06:30" type string
void
US_RunProtocol::timeFromString (double &sectime, QString &strtime)
{
  QList<int> dhms;
...
// Function to convert to a time from a "0d 00:06:30" type string
void
US_RunProtocol::timeFromString (QTime &timeobj, int &days, QString &strtime)
{
...
bool
US_RunProtocol::RunProtoRotor::operator== (const RunProtoRotor &rp) const
{
  ...
  if (labGUID != rp.labGUID)
    return false;
  qDebug () << "RP:ROT: EQ labG";
  if (rotGUID != rp.rotGUID)
    return false;
  qDebug () << "RP:ROT: EQ rotG";
  if (calGUID != rp.calGUID)
    return false;
  qDebug () << "RP:ROT: EQ calG";
  if (absGUID != rp.absGUID)
    return false;
...
bool
US_RunProtocol::RunProtoRotor::fromXml (QXmlStreamReader &xmli)
{
  while (!xmli.atEnd ())
    {
      QString ename = xmli.name ().toString ();
...
bool
US_RunProtocol::RunProtoRotor::toXml (QXmlStreamWriter &xmlo)
{
...
bool
US_RunProtocol::RunProtoSpeed::operator== (const RunProtoSpeed &rp) const
{
  if (nstep != rp.nstep)
    return false;
  if (ssteps != rp.ssteps)
...
bool
US_RunProtocol::RunProtoSpeed::fromXml (QXmlStreamReader &xmli)
{
  ssteps.clear ();
  nstep = 0;
          ...
          if (ename == "speedstep")
            {
              ...
              ss.scancount_int
                  = attr.value ("scancount_int").toString ().toInt ();

              double d_du
                  = attr.value ("duration_minutes").toString ().toDouble ();
              double d_dy
                  = attr.value ("delay_seconds").toString ().toDouble ();
          ...
          else if (ename == "speed")
            {
              ...
              int i_rcal
                  = attr.value ("radial_calibration").toString ().toInt ();
              spin_down = (i_spin != 0);
...
bool
US_RunProtocol::RunProtoSpeed::toXml (QXmlStreamWriter &xmlo)
{
  ...
  for (int ii = 0; ii < ssteps.count (); ii++)
    {
      qDebug () << "SPEED toXml ssteps[ii].duration, ssteps[ii].scanintv  0 : "
                << ssteps[ii].duration << ", " << ssteps[ii].scanintv;
      QString s_durat;
      QString s_stdelay;
      // Uv-vis
      QString s_delay;
      ...
      qDebug () << "SPEED toXml ssteps[ii].duration, ssteps[ii].scanintv  1 : "
                << ssteps[ii].duration << ", " << ssteps[ii].scanintv << ", "
                << ssteps[ii].delay_stage;
      ...
      xmlo.writeAttribute ("scancount",
                           QString::number (ssteps[ii].scancount));

      // interference
      xmlo.writeAttribute ("delay_int", s_delay_int);
      xmlo.writeAttribute ("scan_interval_int", s_sintv_int);
      xmlo.writeAttribute ("scancount_int",
                           QString::number (ssteps[ii].scancount_int));
...
US_RunProtocol::RunProtoSpeed::SpeedStep::SpeedStep ()
{
  ...
  // Uv-vis
  delay = 120.0;
  scanintv = 16.85; // ALEXEY: set default scanint (in secs) which corresponds
                    // to 45000 RPM
...
bool
US_RunProtocol::RunProtoSpeed::SpeedStep::operator== (
    const SpeedStep &ss) const
{
  ...
  if (accel != ss.accel)
    return false;
  if (duration != ss.duration)
    return false;
  if (delay != ss.delay)
    return false;
  if (scanintv != ss.scanintv)
    return false;
...
bool
US_RunProtocol::RunProtoCells::operator== (const RunProtoCells &rp) const
{
  if (ncell != rp.ncell)
    return false;
...
bool
US_RunProtocol::RunProtoCells::fromXml (QXmlStreamReader &xmli)
{
  used.clear ();
      ...
      if (xmli.isStartElement ())
        {
          ...
          if (ename == "cells")
          ...
          else if (ename == "cell")
            {
              ...
              qDebug () << "fX:cel: cel cen win bal" << cu.cell
                        << cu.centerpiece << cu.windows << cu.cbalance;
              used << cu;
            }
        }
...
bool
US_RunProtocol::RunProtoCells::toXml (QXmlStreamWriter &xmlo)
{
      ...
      if (used[ii].cell != ncell)
      ...
      else
        {
          ...
          if (!used[ii].cbalance.contains ("counterbalance"))
            ...
            xmlo.writeAttribute (
                "counterbalance",
                used[ii].cbalance); // ALEXEY: not always counterbalance
        }
...
bool
US_RunProtocol::RunProtoCells::CellUse::operator== (const CellUse &c) const
{
  ...
  if (cell != c.cell)
    return false;
  qDebug () << "RP: CU: EQ cel";
  if (centerpiece != c.centerpiece)
    return false;
  qDebug () << "RP: CU: EQ cen";
  if (windows != c.windows)
    return false;
  qDebug () << "RP: CU: EQ win";
  if (cbalance != c.cbalance)
    return false;
  qDebug () << "RP: CU: EQ bal";
...
bool
US_RunProtocol::RunProtoSolutions::operator== (
    const RunProtoSolutions &rp) const
{
  ...
  if (nschan > 0 && rp.nschan > 0)
    qDebug () << "RP:SOL: chs" << chsols[0].channel << rp.chsols[0].channel
...
bool
US_RunProtocol::RunProtoSolutions::fromXml (QXmlStreamReader &xmli)
{
  nschan = 0;
  chsols.clear ();
...
bool
US_RunProtocol::RunProtoSolutions::toXml (QXmlStreamWriter &xmlo)
{
  nschan = chsols.count ();
...
bool
US_RunProtocol::RunProtoSolutions::ChanSolu::operator== (
    const ChanSolu &c) const
{
  if (channel != c.channel)
    return false;
  if (solution != c.solution)
    return false;
  if (sol_id != c.sol_id)
    return false;
  if (ch_comment != c.ch_comment)
    return false;
...
bool
US_RunProtocol::RunProtoOptics::operator== (const RunProtoOptics &rp) const
{
  if (nochan != rp.nochan)
    return false;
...
bool
US_RunProtocol::RunProtoOptics::fromXml (QXmlStreamReader &xmli)
{
  nochan = 0;
  chopts.clear ();
          ...
          if (ename == "optical_system")
            {
              ...
              qDebug () << "RP:OPT: fX: ch,s1,s2,s3" << os.channel << os.scan1
                        << os.scan2 << os.scan3;
              chopts << os;
              nochan++;
            }
...
bool
US_RunProtocol::RunProtoOptics::toXml (QXmlStreamWriter &xmlo)
{
  nochan = chopts.count ();
  ...
  for (int ii = 0; ii < nochan; ii++)
    {
      ...
      qDebug () << "RP:OPT: tX: ch,s1,s2,s3" << chopts[ii].channel
                << chopts[ii].scan1 << chopts[ii].scan2 << chopts[ii].scan3;
    }
...
bool
US_RunProtocol::RunProtoOptics::OpticSys::operator== (const OpticSys &c) const
{
  if (channel != c.channel)
    return false;
  if (scan1 != c.scan1)
...
bool
US_RunProtocol::RunProtoRanges::operator== (const RunProtoRanges &rp) const
{
  if (nranges != rp.nranges)
    return false;
...
bool
US_RunProtocol::RunProtoRanges::fromXml (QXmlStreamReader &xmli)
{
  nranges = 0;
  chrngs.clear ();
  Ranges rng;
              ...
              if (attr.hasAttribute ("abde_mwl_deconv"))
                {
                  (attr.value ("abde_mwl_deconv").toString ().toInt ())
                      ? rng.abde_mwl_deconvolution = true
...
bool
US_RunProtocol::RunProtoRanges::toXml (QXmlStreamWriter &xmlo)
{
  ...
  for (int ii = 0; ii < nranges; ii++)
    {
      ...
      xmlo.writeAttribute ("end_radius", QString::number (chrngs[ii].hi_rad));
      // abde
      xmlo.writeAttribute (
          "abde_buffer",
...
bool
US_RunProtocol::RunProtoRanges::Ranges::operator== (const Ranges &s) const
{
...
bool
US_RunProtocol::RunProtoAProfile::operator== (const RunProtoAProfile &u) const
{
  if (aprofname != u.aprofname)
    return false;
  if (aprofGUID != u.aprofGUID)
    return false;
...
bool
US_RunProtocol::RunProtoAProfile::fromXml (QXmlStreamReader &xmli)
{
  while (!xmli.atEnd ())
    {
      QString ename = xmli.name ().toString ();
...
bool
US_RunProtocol::RunProtoAProfile::toXml (QXmlStreamWriter &xmlo)
{
...
// RunProtoUpload subclass Equality operator
bool
US_RunProtocol::RunProtoUpload::operator== (const RunProtoUpload &u) const
{
  if (us_xml != u.us_xml)
    return false;
  if (op_json != u.op_json)
    return false;
