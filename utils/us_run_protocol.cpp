...
void US_RunProtocol::timeFromList(double &sectime, QList<int> &dhms) {
   sectime = dhms[ 0 ] * (24 * 60 * 60) + dhms[ 1 ] * (60 * 60) + dhms[ 2 ] * 60 + dhms[ 3 ];
}

// Function to convert to a time from a day,hour,minute,second list
void US_RunProtocol::timeFromList(QTime &timeobj, int &days, QList<int> &dhms) {
...
void US_RunProtocol::timeToString(double &sectime, QString &strtime) {
   ...
   strtime = QString().sprintf("%dd %02d:%02d:%02d", dhms[ 0 ], dhms[ 1 ], dhms[ 2 ], dhms[ 3 ]);
}

// Function to convert from a time to "0d 00:06:30" type string
void US_RunProtocol::timeToString(QTime &timeobj, int &days, QString &strtime) {
   strtime = QString().sprintf("%dd %02d:%02d:%02d", days, timeobj.hour(), timeobj.minute(), timeobj.second());
}
...
// Function to convert to a time from a "0d 00:06:30" type string
void US_RunProtocol::timeFromString(QTime &timeobj, int &days, QString &strtime) {
...
bool US_RunProtocol::RunProtoRotor::operator==(const RunProtoRotor &rp) const {
...
bool US_RunProtocol::RunProtoSpeed::operator==(const RunProtoSpeed &rp) const {
   if (nstep != rp.nstep)
      return false;
   if (ssteps != rp.ssteps)
      return false;
...
bool US_RunProtocol::RunProtoSpeed::toXml(QXmlStreamWriter &xmlo) {
   ...
   for (int ii = 0; ii < ssteps.count(); ii++) {
      qDebug() << "SPEED toXml ssteps[ii].duration, ssteps[ii].scanintv  0 : " << ssteps[ ii ].duration << ", "
               << ssteps[ ii ].scanintv;
      QString s_durat;
      QString s_stdelay;
      //Uv-vis
      ...
      qDebug() << "SPEED toXml ssteps[ii].duration, ssteps[ii].scanintv  1 : " << ssteps[ ii ].duration << ", "
               << ssteps[ ii ].scanintv << ", " << ssteps[ ii ].delay_stage;
...
bool US_RunProtocol::RunProtoSpeed::SpeedStep::operator==(const SpeedStep &ss) const {
...
bool US_RunProtocol::RunProtoCells::operator==(const RunProtoCells &rp) const {
   if (ncell != rp.ncell)
      return false;
   ...
   while (!xmli.atEnd()) {
      ...
      if (xmli.isStartElement()) {
         if (ename == "cells") {
            ...
            qDebug() << "fX:cel: cel cen win bal" << cu.cell << cu.centerpiece << cu.windows << cu.cbalance;
            used << cu;
         }
      }
...
bool US_RunProtocol::RunProtoCells::CellUse::operator==(const CellUse &c) const {
...
bool US_RunProtocol::RunProtoSolutions::operator==(const RunProtoSolutions &rp) const {
   qDebug() << "RP:SOL: nch" << nschan << rp.nschan;
   qDebug() << "RP:SOL: nun" << nuniqs << rp.nuniqs;
   if (nschan > 0 && rp.nschan > 0)
      qDebug() << "RP:SOL: chs" << chsols[ 0 ].channel << rp.chsols[ 0 ].channel << chsols[ 0 ].solution
               << rp.chsols[ 0 ].solution << chsols[ 0 ].sol_id << rp.chsols[ 0 ].sol_id << chsols[ 0 ].ch_comment
               << rp.chsols[ 0 ].ch_comment;
...
bool US_RunProtocol::RunProtoSolutions::fromXml(QXmlStreamReader &xmli) {
   ...
   for (int ii = 0; ii < nschan; ii++) {
      ...
      if (!solus.contains(sdesc)) { // Update unique solutions list and correspond Id list
         solus << sdesc;
...
bool US_RunProtocol::RunProtoSolutions::ChanSolu::operator==(const ChanSolu &c) const {
...
bool US_RunProtocol::RunProtoOptics::operator==(const RunProtoOptics &rp) const {
   if (nochan != rp.nochan)
      return false;
   ...
   while (!xmli.atEnd()) {
      ...
      if (xmli.isStartElement()) {
         if (ename == "optical_system") {
            ...
            qDebug() << "RP:OPT: fX: ch,s1,s2,s3" << os.channel << os.scan1 << os.scan2 << os.scan3;
            chopts << os;
            nochan++;
         }
      }
...
bool US_RunProtocol::RunProtoOptics::toXml(QXmlStreamWriter &xmlo) {
   ...
   for (int ii = 0; ii < nochan; ii++) {
      ...
      qDebug() << "RP:OPT: tX: ch,s1,s2,s3" << chopts[ ii ].channel << chopts[ ii ].scan1 << chopts[ ii ].scan2
               << chopts[ ii ].scan3;
   }
...
bool US_RunProtocol::RunProtoOptics::OpticSys::operator==(const OpticSys &c) const {
   if (channel != c.channel)
      return false;
   if (scan1 != c.scan1)
      return false;
...
bool US_RunProtocol::RunProtoRanges::operator==(const RunProtoRanges &rp) const {
...
bool US_RunProtocol::RunProtoRanges::fromXml(QXmlStreamReader &xmli) {
   ...
   while (!xmli.atEnd()) {
      ...
      if (xmli.isStartElement()) {
         if (ename == "range" || ename == "spectrum") {
            ...
            if (attr.hasAttribute("abde_buffer")) {
               (attr.value("abde_buffer").toString().toInt()) ? rng.abde_buffer_spectrum = true
            ...
            if (attr.hasAttribute("abde_mwl_deconv")) {
               (attr.value("abde_mwl_deconv").toString().toInt()) ? rng.abde_mwl_deconvolution = true
      ...
      else if (xmli.isEndElement() && (ename == "range" || ename == "spectrum")) {
         chrngs << rng;
         nranges++;
      }
      ...
      if (was_end) {
         if (
            ename == "ranges" || // Break after "</ranges>"
...
bool US_RunProtocol::RunProtoRanges::toXml(QXmlStreamWriter &xmlo) {
   ...
   for (int ii = 0; ii < nranges; ii++) {
      ...
      xmlo.writeAttribute("start_radius", QString::number(chrngs[ ii ].lo_rad));
      xmlo.writeAttribute("end_radius", QString::number(chrngs[ ii ].hi_rad));
      //abde
      xmlo.writeAttribute("abde_buffer", QString::number(int(chrngs[ ii ].abde_buffer_spectrum)));
      xmlo.writeAttribute("abde_mwl_deconv", QString::number(int(chrngs[ ii ].abde_mwl_deconvolution)));

      for (int jj = 0; jj < chrngs[ ii ].wvlens.count(); jj++) {
         xmlo.writeStartElement("wavelength");
         xmlo.writeAttribute("lambda", QString::number(chrngs[ ii ].wvlens[ jj ]));
...
bool US_RunProtocol::RunProtoRanges::Ranges::operator==(const Ranges &s) const {
...
bool US_RunProtocol::RunProtoAProfile::operator==(const RunProtoAProfile &u) const {
...
// RunProtoUpload subclass Equality operator
bool US_RunProtocol::RunProtoUpload::operator==(const RunProtoUpload &u) const {
   if (us_xml != u.us_xml)
      return false;
