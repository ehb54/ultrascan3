...
class US_UTIL_EXTERN US_Archive : public QObject {
 ...
 public:
  ...
  //! \brief Method to extract archive file (Supported files: tar, tar.gz, tgz,
  //! tar.bz2, tar.xz, .zip).
  //! \param archivePath    Path to the archive file.
  //! \param outputPath     Path to where extracted data will be saved. Default
  //! is the path where the archive file is located.
  ...
  //! \param sourcePathList List of all files and folders need to be compressed.
  //! No need to list the contents of folders.
  //! \param archivePath    If only the filename is given, the output path is
  //! taken from the first one. Otherwise, the archive file will be saved in the
  //! given path. After compression is complete, this parameter stores the path
  //! to the archive file. Supported files: tar, tar.gz, tgz, tar.bz2, tar.xz,
  //! .zip
 ...
 signals:
  ...
  //! \brief A Signal sent when an entry is added to the archive file
  //! successfully
  //! \param relative_path
  //! \param absolute_path
