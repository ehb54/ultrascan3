...
int US_AstfemMath::writetimestate(const QString& tmst_fpath,
  ...
  int t_acc =
      1;  // Used for time when accelerated up to the specified rotor speed
  double rate = 1.0;
  double speed = 1000.0;
  int ss_reso = 100;
  ...
  for (int ii = 0; ii < nscans;
       ii++) {  // Accumulate the times and temperatures at scans
    scantimes << sim_data.scanData[ii].seconds;
    scantemps << sim_data.scanData[ii].temperature;
    DbgLv(1) << "AMATH:wrts:scantimes" << scantimes[ii]
             << sim_data.scanData[ii].omega2t << ii
             << sim_data.scanData[ii].rpm;
  }
  // DbgLv(1) << " writetimestate : no of scans" << nscans;

  if (simparams.sim == false) {  // Handle 1st acceleration zone for real data
    ...
    t_acc = (int)qRound((double)(simparams.speed_step[0].rotorspeed) / rate);
    DbgLv(0) << "AMATH:wrts: computed rate:" << rate;
  } else {  // Handle 1st acceleration zone for simulation (astfem_sim) data
    sp = &simparams.speed_step[0];
    // Use specified acceleration rate
    t_acc = (int)qCeil((double)sp->rotorspeed / (double)sp->acceleration);
    rate = (double)sp->acceleration;
    DbgLv(0) << "AMATH:wrts: rate is given by user : t_acc from timestate"
             << t_acc << rate;
  }
  ...
  for (int step = 0; step < nspeed; step++) {
    ...
    if (step == 0) {
      ...
      //         duration_prev += ( simparams.speed_step[ step-1
      //         ].duration_hours * 3600.0 )
      //                        + ( simparams.speed_step[ step-1
      ...
      if (simparams.sim == false)
        ...
        t_acc =
            (int)qRound(double(qAbs(speed - prvs_speed)) / sp->acceleration);
    ...
    for (int ii = d1; ii < d2; ii++) {
      ...
      // if(scan_nbr>0)
      if ((scan_nbr > 0) || (ii < (d1 + 2)) ||
          ((ii > (tacc - 2)) && (ii < (tacc + 2))) || ((ii + 3) > d2))
        DbgLv(1) << "AMATH:wrts: ii(time)" << ii << "scanx scan_nbr" << scanx
                 << scan_nbr << "rpm" << rpm << "omega2t" << omega2t;
...
bool US_AstfemMath::low_acceleration(
  ...
  DbgLv(0) << "AMATH:loac: om1t w2 w2t" << om1t << w2 << w2t << "t1w" << t1w
           << "tfac" << tfac;

  if (t1 >= t2) {  // Something wrong!  set t1 to a few seconds before t2
    t1 = t2 - 5.0;
  }
...
bool US_AstfemMath::timestate_onesec(const QString& tmst_fpath,
  ...
  if (timestate.open_read_data(tmst_fpath, false) != 0) return onesec_intv;
  ...
  if (constti &&
      timeinc ==
          1.0) {  // If constant increment and it is 1.0, one-second-interval
    onesec_intv = true;
  }

  else {  // Otherwise, compare timestate times count to last scan time
    ...
    if (ntimes >= lsctime) onesec_intv = true;
  }
...
void US_AstfemMath::interpolate_C0(MfemInitial& C0, double* C1,
  ...
  for (int j = 0; j < xvec.size(); j++) {
    ...
    if (i == 0)         // x[j] < C0.radius[0]
      ...
      C1[j] = conc[ja] * (1.0 - tmp) + conc[i] * tmp;
    }
  }
}
...
void US_AstfemMath::zero_2d(int val1, int val2, double** matrix) {
  for (int i = 0; i < val1; i++)
    for (int j = 0; j < val2; j++) matrix[i][j] = 0.0;
}
...
double US_AstfemMath::minval(const QVector<double>& value) {
  ...
  for (int i = 0; i < value.size(); i++) minimum = qMin(minimum, avalue[i]);

  return minimum;
}

double US_AstfemMath::minval(
    const QVector<US_Model::SimulationComponent>& value) {
  double minimum = 1.0e300;

  for (int i = 0; i < value.size(); i++) minimum = min(minimum, value[i].s);
...
double US_AstfemMath::maxval(const QVector<double>& value) {
  ...
  for (int i = 0; i < value.size(); i++) maximum = qMax(maximum, avalue[i]);

  return maximum;
}

double US_AstfemMath::maxval(
    const QVector<US_Model::SimulationComponent>& value) {
  double maximum = -1.0e300;

  for (int i = 0; i < value.size(); i++) maximum = max(maximum, value[i].s);

  return maximum;
}

void US_AstfemMath::initialize_3d(int val1, int val2, int val3,
                                  double**** matrix) {
  *matrix = new double**[val1];
...
void US_AstfemMath::tridiag(double* a, double* b, double* c, double* r,
                            double* u, int N) {
  double bet = b[0];
#ifdef NO_DB
  ...
  for (int jj = N - 1; jj >= 1; jj--) u[jj - 1] -= gam[jj] * u[jj];
}
...
double US_AstfemMath::find_C1_mono_Nmer(int n, double K, double CT) {
  ...
  if (CT <= 1.0e-12) return qMax(0.0, CT);

  for (ii = 1; ii < MaxNumIt; ii++) {
    x1 = (CT + zKn1 * pow(x0, zn)) / (1.0 + zKn * pow(x0, zn1));

    if ((qAbs(x1 - x0) / (1.0 + qAbs(x1))) < 1.e-12) break;
...
int US_AstfemMath::GaussElim(int n, double** a, double* b) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int ip = i + 1; ip < n; ip++) {
      ...
      for (int j = i + 1; j < n; j++) a[ip][j] -= tmp * a[i][j];
...
int US_AstfemMath::interpolate(MfemData& expdata, MfemData& simdata,
  ...
  if (use_time) {
    ...
    for (int expscan = 0; expscan < escans;
         expscan++) {  // First determine the scan range of current experiment
                       // data
      e_time = expdata.scan[expscan].time;
      if (fscan < 0 && e_time >= s_time) fscan = expscan;
      if (e_time > l_time) {  // Save scan upper limit (use scan count if close)
        DbgLv(2) << "MATHi: l_time e_time" << l_time << e_time << "expscan"
                 << expscan;
    ...
    //  << "fscan lscan" << fscan << lscan <<"expscansize=" <<  escans <<
    //  "simscansize="<< sscans;
    fscan = (fscan < 0) ? 0 : fscan;
    lscan = (lscan < 0) ? escans : lscan;
    // DbgLv(1) << "MATHi: s_time e_time" << s_time << e_time
    //  << "fscan lscan" << fscan << lscan << "escans=" <<  escans << "sscans="
    //  << sscans;
  } else  // Use omega^2t integral for interpolation
  {
    ...
    for (int expscan = 0; expscan < escans;
      ...
      if (fscan < 0 && e_omega >= s_omega) fscan = expscan;
      if (e_omega > l_omega) {
        lscan = (expscan > (escans - 5)) ? escans : expscan;
        break;
      }
...
int US_AstfemMath::interpolate(MfemData& expdata, MfemData& simdata,
  ...
  if (nescan == 0 || neconc == 0 || nsscan == 0 || nsconc == 0) return -1;
  ...
  if (use_time) {
    ...
    // DbgLv(1) << "MATHi2: eftime sltime" << eftime << sltime << "fscan lscan"
    // << fscan << lscan; DbgLv(1) << "MATHi2:  erad0 eradn" <<
    // expdata.radius[0] << expdata.radius[ner-1] << ner << nec; DbgLv(1) <<
    // "MATHi2:  srad0 sradn" << simdata.radius[0] << simdata.radius[nsr-1] <<
    // nsr << nsc;

    for (int expscan = fscan; expscan < lscan;
         expscan++) {  // Interpolate where needed to a range of experiment
                       // scans
      ...
      while (simdata.scan[simscan].time < e_time) {
        ...
        if (simscan == nsscan) {   // Sim scan count has exceeded limit
          if (sltime <= e_time) {  // Output a message if time ranges overlap
      ...
      if (s_time2 == e_time) {  // they are the same, so take this scan
                                // and push it onto the tmp_data array.
        tmp_data.scan << simdata.scan[simscan];
      } else  // interpolation is needed
      {
        // DbgLv(1) << "MATHi2:  sscm" << sscm << "s_time1 s_time2" << s_time1
        // << s_time2
        //  << "e_time" << e_time;
        double a;
        double b;
  ...
  } else  // Use omega^2t integral for interpolation
  {
    for (int expscan = fscan; expscan < lscan;
      ...
      e_omega = escan->omega_s_t;
      e_time = escan->time;
      // DbgLv(1) << "MATHi: somg eomg" << simdata.scan[simscan].omega_s_t <<
      // e_omega
      ...
      // Check to see if the time is equal or larger:
      if (s_omega2 == e_omega) {  // They are the same, so take this scan and
      ...
      } else  // Interpolation is needed
      {
        ...
        // Interpolate the concentration points:
        for (int ii = 0; ii < nsconc; ii++) {
          a = (sscan2->conc[ii] - sscan1->conc[ii]) / (s_omega2 - s_omega1);
  ...
  if (tmp_data.radius[0] > expdata.radius[0]) {
    qDebug() << "Radius comparison: " << tmp_data.radius[0] << " (simulated), "
             << expdata.radius[0] << " (experimental)";

    qDebug() << "jj = " << 0 << ", simdata radius: " << tmp_data.radius[0]
             << ", expdata radius: "
  ...
  for (int ii = 0; ii < neconc; ii++) {
    while (tdrad[jj] < exrad[ii]) {
      ...
      if (rrdif < 1.0e-4 && dbg_level > 0) {
        rrdk++;
        if (rrdk < 11)
          DbgLv(0) << "    jj ii" << jj << ii << "  trad erad diff" << tdrad[jj]
                   << exrad[ii] << rrdif;
      }
      if (rrdif < 1.0e-8) break;
      jj++;
      // make sure we don't overrun bounds:
      if (jj == tdradsz) {
        qDebug() << "The simulated data does not have enough "
                    ...
                    "and make sure the bottom of the cell is correctly defined "
                    "for this experiment!"
                    "exiting...";
        DbgLv(1) << "jj ii sztrad szerad" << jj << ii << tdradsz
                 << expdata.radius.size() << "trad erad"
                 << tmp_data.radius[jj - 1] << expdata.radius[ii] << rrdif;
#ifdef NO_DB
    ...
    if (qAbs(tdrad[jj] - exrad[ii]) <
        1.0e-8) {  // they are virtually the same, so simply update the
                   // concentration value:
      int ee = 0;
      for (int expscan = fscan; expscan < lscan; expscan++)
        expdata.scan[expscan].conc[ii] += tmp_data.scan[ee++].conc[jj];
    } else  // interpolation is needed
    {
      int ee = 0;
      int mm = jj - 1;
      ...
      // DbgLv(1) << "MATHi:   r1 r2 er rr" << radius1 << radius2 << eradius <<
      // radrange
      //  << "fscan lscan" << fscan << lscan;
  ...
  // if ((ncall%100)==1) DbgLv(1) << "interpolate() calls time-ms" << ncall <<
  // totMs; *TIMING*
  return 0;
}
...
void US_AstfemMath::QuadSolver(double* ai, double* bi, double* ci, double* di,
  ...
  //
  // Test
  // n=100; a=-1+rand(100,1); b=2+rand(200,1); c=-0.7*rand(100,1);
  // d=-0.3*rand(100,1); xs=rand(100,1); r(1)=b(1)*xs(1)+c(1)*xs(2)+d(1)*xs(3);
  // for i=2:n-2,
...
void US_AstfemMath::IntQT1(double* vx, double D, double sw2, double** Stif,
                           double dt) {
  ...
  for (k = 0; k < npts; k++) {
    ...
    for (i = 0; i < 3; i++) {
      dval = Integrand(x_gauss, D, sw2, phiL[i], phiLx[i], phiLy[i], 1. - phiC,
                       -phiCx);
      StifL[i][0] += Lam[k][3] * DJac * dval;

      dval =
          Integrand(x_gauss, D, sw2, phiL[i], phiLx[i], phiLy[i], phiC, phiCx);
  ...
  for (k = 0; k < npts; k++) {
    ...
    for (i = 0; i < 4; i++) {
      dval = Integrand(x_gauss, D, sw2, phiR[i], phiRx[i], phiRy[i], 1. - phiC,
                       -phiCx);
      StifR[i][0] += Lam[k][3] * DJac * dval;

      dval =
          Integrand(x_gauss, D, sw2, phiR[i], phiRx[i], phiRy[i], phiC, phiCx);
...
void US_AstfemMath::IntQTm(double* vx, double D, double sw2, double** Stif,
                           double dt) {
  // element to define basis functions
  //
  int npts, i, k;
...
void US_AstfemMath::IntQTn2(double* vx, double D, double sw2, double** Stif,
                            double dt) {
  // element to define basis functions
  //
  int npts, i, k;
...
void US_AstfemMath::IntQTn1(double* vx, double D, double sw2, double** Stif,
                            double dt) {
  // element to define basis functions
  //
  int npts, i, k;
...
void US_AstfemMath::DefineFkp(int npts, double** Lam) {
  ...
  switch (npts) {
    ...
    case 37:
      //   TOMS706_37, order 37, degree of precision 13, a rule from ACM TOMS
      //   algorithm #706.
      {
...
void US_AstfemMath::BasisTS(double xi, double et, double* phi, double* phi1,
                            double* phi2) {
...
void US_AstfemMath::BasisQS(double xi, double et, double* phi, double* phi1,
                            double* phi2) {
  phi[0] = (1.0 - xi) * (1.0 - et);
  phi1[0] = -1.0 * (1.0 - et);
  phi2[0] = -1.0 * (1.0 - xi);
...
void US_AstfemMath::BasisTR(double* vx, double* vy, double xs, double ys,
                            double* phi, double* phix, double* phiy) {
...
void US_AstfemMath::BasisQR(double* vx, double xs, double ts, double* phi,
  ...
  Jac[0] =
      vx[0] * phi1[0] + vx[1] * phi1[1] + vx[2] * phi1[2] + vx[3] * phi1[3];
  Jac[1] =
      vx[0] * phi2[0] + vx[1] * phi2[1] + vx[2] * phi2[2] + vx[3] * phi2[3];
...
double US_AstfemMath::Integrand(double x_gauss, double D, double sw2, double u,
                                double ux, double ut, double v, double vx) {
...
void US_AstfemMath::DefineGaussian(int nGauss, double** Gs2) {
  ...
  switch (nGauss) {
    ...
    case 5: {
      Gs1[0] = 0.906179845938664;
      w[0] = 0.236926885056189;
      Gs1[1] = 0.538469310105683;
      w[1] = 0.478628670499366;
      Gs1[2] = 0.000000000000000;
    ...
    case 7: {
      Gs1[0] = 0.949107912342759;
      w[0] = 0.129484966168870;
      Gs1[1] = 0.741531185599394;
      w[1] = 0.279705391489277;
      Gs1[2] = 0.405845151377397;
      w[2] = 0.381830050505119;
      Gs1[3] = 0.000000000000000;
    ...
    case 10: {
      Gs1[0] = 0.973906528517172;
      w[0] = 0.066671344308688;
      Gs1[1] = 0.865063366688985;
      w[1] = 0.149451349150581;
      Gs1[2] = 0.679409568299024;
      w[2] = 0.219086362515982;
      Gs1[3] = 0.433395394129247;
      w[3] = 0.269266719309996;
      Gs1[4] = 0.148874338981631;
...
void US_AstfemMath::initSimData(US_DataIO::RawData& simdata,
  ...
  for (int jj = 0; jj < 16; jj++) simdata.rawGUID[jj] = ' ';
  ...
  for (int ii = 0; ii < nscan; ii++) {  // Loop to copy scans
...
double US_AstfemMath::variance(US_DataIO::RawData& simdata,
  ...
  for (int ii = 0; ii < nscan;
       ii++) {  // accumulate sum of differences squared (readings in scans)
...
double US_AstfemMath::calc_bottom(double rpm, double bottom_chan,
                                  double* rotorcoefs) {
  return (bottom_chan + rotorcoefs[0] * rpm + rotorcoefs[1] * sq(rpm));
}
...
void IntQT1_ellam(QVector<double> vx, double D, double sw2, double** Stif,
                  double dt) {
  // element to define basis functions
  //
  int npts, i, k;
  ...
  //
  if ((vx[1] - vx[4]) / (vx[1] - vx[0]) <
      1.e-3)  // Q_{0,4,3,2} is almost degenerated into a triangle
  {
    ...
    for (k = 0; k < npts; k++) {
      ...
      xn1 = x_gauss +
            slope *
                (dt - y_gauss);  // trace-forward point at t_n+1 from (x_g, y_g)
      phiC = (xn1 - vx[2]) /
             hh;  // hat function on t_n+1, =1 at vx[3]; =0 at vx[2]
      phiCx = 1. / hh;
      ...
      for (i = 0; i < 4; i++) {
        ...
        dval = Integrand(x_gauss, D, sw2, phiR[i], phiRx[i], phiRy[i], phiC,
                         phiCx);
        StifR[i][1] += Lam[k][3] * DJac * dval;
      }
    }
...
void IntQTm_ellam(QVector<double> vx, double D, double sw2, double** Stif,
                  double dt) {
  // element to define basis functions
  //
  int npts, i, k;
  ...
  // integration over quadrilateral element Q :
  //
  if ((vx[7] - vx[1]) / (vx[2] - vx[1]) <
      1.e-3)  // Q_{1,7,5,4} is almost degenerated into a triangle
  {
...
void IntQTn1_ellam(QVector<double> vx, double D, double sw2, double** Stif,
                   double dt) {
  // element to define basis functions
  //
  int npts, i, k;
...
void QuadSolver_ellam(double* ai, double* bi, double* ci, double* di,
                      double* cr, double* solu, int N) {
  //
  // solve Quad-diagonal system [a_i, b_i, *c_i*, d_i]*[x]=[r_i]
...
double IntConcentration(QVector<double> r, double* u) {
  ...
  for (int j = 0; j < r.size() - 1; j++) {
    T +=
        (r[j + 1] - r[j]) * ((r[j + 1] - r[j]) * (u[j + 1] / 3.0 + u[j] / 6.0) +
                             r[j] * (u[j] + u[j + 1]) / 2.0);
  }
  return T;
}
...
int interpolate(MfemData* simdata, int scans, int points, float* scantimes,
   ...
   * simulated data. Then interpolate the radius points by linear
   *interpolation.*
   *                                                                            *
   ******************************************************************************/
  ...
  for (i = 0; i < scans; i++) {
    ...
    {
      for (j = 0; j < (*simdata).radius.size(); j++) {
        slope = ((*simdata).scan[count].conc[j] -
                 (*simdata).scan[count - 1].conc[j]) /
                ((*simdata).scan[count].time - (*simdata).scan[count - 1].time);
        intercept = (*simdata).scan[count].conc[j] -
                    slope * (*simdata).scan[count].time;
  ...
  for (i = 0; i < scans; i++) {
    ...
    for (j = 1; j < (int)points; j++) {
      while (radius[j] > (*simdata).radius[count] &&
             count < (*simdata).radius.size() - 1) {
        count++;
      }
      if (radius[j] == (*simdata).radius[count]) {
        c[i][j] += ip_array[i][count];
      } else {
        slope = (ip_array[i][count] - ip_array[i][count - 1]) /
                ((*simdata).radius[count] - (*simdata).radius[count - 1]);
