...
// Write time state
//----------------------------------------------------------------------
int US_AstfemMath::writetimestate(
   const QString &tmst_fpath, US_SimulationParameters &simparams, US_DataIO::RawData &sim_data) {
   ...
   DbgLv(1) << "AMATH:wrts: number of speeds=" << nspeed << "Time state file path=" << tmst_fpath;
   simparams.debug();
   ...
   for (int ii = 0; ii < nscans; ii++) { // Accumulate the times and temperatures at scans
      scantimes << sim_data.scanData[ ii ].seconds;
      scantemps << sim_data.scanData[ ii ].temperature;
      DbgLv(1) << "AMATH:wrts:scantimes" << scantimes[ ii ] << sim_data.scanData[ ii ].omega2t << ii
               << sim_data.scanData[ ii ].rpm;
   }
   //DbgLv(1) << " writetimestate : no of scans" << nscans;

   if (simparams.sim == false) { // Handle 1st acceleration zone for real data (compute acceleration rate)
   ...
   for (int step = 0; step < nspeed; step++) {
      ...
      for (int ii = d1; ii < d2; ii++) {
         ...
         if ((scan_nbr > 0) || (ii < (d1 + 2)) || ((ii > (tacc - 2)) && (ii < (tacc + 2))) || ((ii + 3) > d2))
            DbgLv(1) << "AMATH:wrts: ii(time)" << ii << "scanx scan_nbr" << scanx << scan_nbr << "rpm" << rpm
                     << "omega2t" << omega2t;
...
bool US_AstfemMath::low_acceleration(
   const QVector<US_SimulationParameters::SpeedProfile> &speedsteps, const double min_accel, double &rate) {
   int dbg_level = US_Settings::us_debug();
   ...
   DbgLv(0) << "AMATH:loac: om1t w2 w2t" << om1t << w2 << w2t << "t1w" << t1w << "tfac" << tfac;
...
bool US_AstfemMath::timestate_onesec(const QString &tmst_fpath, US_DataIO::RawData &sim_data) {
   ...
   if (constti && timeinc == 1.0) { // If constant increment and it is 1.0, one-second-interval
      onesec_intv = true;
   }
...
void US_AstfemMath::interpolate_C0(MfemInitial &C0, double *C1, QVector<double> &xvec) {
   int ja = 0;
   double *x = xvec.data();
...
void US_AstfemMath::initialize_3d(int val1, int val2, int val3, double ****matrix) {
   *matrix = new double **[ val1 ];
...
void US_AstfemMath::tridiag(double *a, double *b, double *c, double *r, double *u, int N) {
   double bet = b[ 0 ];
#ifdef NO_DB
...
int US_AstfemMath::interpolate(MfemData &expdata, MfemData &simdata, bool use_time) {
   ...
   if (
      expdata.scan.size() == 0 || expdata.scan[ 0 ].conc.size() == 0 || simdata.scan.size() == 0
      || simdata.radius.size() == 0)
      return -1;
   ...
   if (use_time) {
      ...
      for (int expscan = 0; expscan < escans; expscan++) { // First determine the scan range of current experiment data
...
// Interpolation routine By B. Demeler 041708
int US_AstfemMath::interpolate(MfemData &expdata, MfemData &simdata, bool use_time, int fscan, int lscan) {
   static int rrdk = 0;
   int dbg_level = US_Settings::us_debug();
   //*TIMING*
   //static int ncall=0;
   ...
   if (use_time) {
      ...
      for (int expscan = fscan; expscan < lscan; expscan++) { // Interpolate where needed to a range of experiment scans
         ...
         while (simdata.scan[ simscan ].time < e_time) {
            ...
            if (simscan == nsscan) { // Sim scan count has exceeded limit
               if (sltime <= e_time) { // Output a message if time ranges overlap
                  qDebug() << "simulation time scan[" << simscan << "]: " << simdata.scan[ simscan - 1 ].time
                           << ", expdata scan time[" << expscan << "]: " << expdata.scan[ expscan ].time;
      ...
      for (int expscan = fscan; expscan < lscan; expscan++) { // Interpolate where needed to a range of experiment scans
         ...
         while (simdata.scan[ simscan ].omega_s_t < e_omega) {
            ...
            if (simscan == ( int ) simdata.scan.size()) {
               qDebug() << "simulation omega^2t scan[" << simscan << "]: " << simdata.scan[ simscan - 1 ].omega_s_t
                        << ", expdata scan omega^2t[" << expscan << "]: " << expdata.scan[ expscan ].omega_s_t;
   ...
   if (tmp_data.radius[ 0 ] > expdata.radius[ 0 ]) {
      qDebug() << "Radius comparison: " << tmp_data.radius[ 0 ] << " (simulated), " << expdata.radius[ 0 ]
               << " (experimental)";

      qDebug() << "jj = " << 0 << ", simdata radius: " << tmp_data.radius[ 0 ]
               << ", expdata radius: " << expdata.radius[ 0 ]; // Changed from radius[ ii ]
         // ii out of scope
   ...
   for (int ii = 0; ii < neconc; ii++) {
      while (tdrad[ jj ] < exrad[ ii ]) {
         ...
         if (rrdif < 1.0e-4 && dbg_level > 0) {
            rrdk++;
            if (rrdk < 11)
               DbgLv(0) << "    jj ii" << jj << ii << "  trad erad diff" << tdrad[ jj ] << exrad[ ii ] << rrdif;
         }
         if (rrdif < 1.0e-8)
            break;
         jj++;
         ...
         if (jj == tdradsz) {
            qDebug() << "The simulated data does not have enough "
                        ...
                        "exiting...";
            DbgLv(1) << "jj ii sztrad szerad" << jj << ii << tdradsz << expdata.radius.size() << "trad erad"
      ...
      if (qAbs(tdrad[ jj ] - exrad[ ii ]) < 1.0e-8) { // they are virtually the same, so simply update the concentration value:
         int ee = 0;
...
void US_AstfemMath::QuadSolver(double *ai, double *bi, double *ci, double *di, double *cr, double *solu, int N) {
...
void US_AstfemMath::IntQT1(double *vx, double D, double sw2, double **Stif, double dt) {
...
void US_AstfemMath::IntQTm(double *vx, double D, double sw2, double **Stif, double dt) {
   // element to define basis functions
   //
   int npts, i, k;
   double x_gauss, y_gauss, dval;
   ...
   for (k = 0; k < npts; k++) {
      ...
      for (i = 0; i < 4; i++) {
         dval = Integrand(
            x_gauss, D, sw2, phiL[ i ], phiLx[ i ], phiLy[ i ], phiC[ 0 ] + phiC[ 3 ], phiCx[ 0 ] + phiCx[ 3 ]);
         StifL[ i ][ 0 ] += Gs[ k ][ 2 ] * DJac * dval;

         dval = Integrand(
            x_gauss, D, sw2, phiL[ i ], phiLx[ i ], phiLy[ i ], phiC[ 1 ] + phiC[ 2 ], phiCx[ 1 ] + phiCx[ 2 ]);
   ...
   for (k = 0; k < npts; k++) {
      ...
      for (i = 0; i < 4; i++) {
         dval = Integrand(
            x_gauss, D, sw2, phiR[ i ], phiRx[ i ], phiRy[ i ], phiC[ 0 ] + phiC[ 3 ], phiCx[ 0 ] + phiCx[ 3 ]);
         StifR[ i ][ 0 ] += Lam[ k ][ 3 ] * DJac * dval;

         dval = Integrand(
            x_gauss, D, sw2, phiR[ i ], phiRx[ i ], phiRy[ i ], phiC[ 1 ] + phiC[ 2 ], phiCx[ 1 ] + phiCx[ 2 ]);
         StifR[ i ][ 1 ] += Lam[ k ][ 3 ] * DJac * dval;
      }
   }
...
void US_AstfemMath::IntQTn2(double *vx, double D, double sw2, double **Stif, double dt) {
   // element to define basis functions
   //
   int npts, i, k;
   double x_gauss, y_gauss, dval;
...
void US_AstfemMath::IntQTn1(double *vx, double D, double sw2, double **Stif, double dt) {
   // element to define basis functions
   //
   int npts, i, k;
   double x_gauss, y_gauss, dval;
...
double US_AstfemMath::AreaT(double *xv, double *yv) {
   return (0.5 * ((xv[ 1 ] - xv[ 0 ]) * (yv[ 2 ] - yv[ 0 ]) - (xv[ 2 ] - xv[ 0 ]) * (yv[ 1 ] - yv[ 0 ])));
}

// Computer basis on standard element

void US_AstfemMath::BasisTS(double xi, double et, double *phi, double *phi1, double *phi2) {
...
void US_AstfemMath::BasisQS(double xi, double et, double *phi, double *phi1, double *phi2) {
   phi[ 0 ] = (1.0 - xi) * (1.0 - et);
   phi1[ 0 ] = -1.0 * (1.0 - et);
   phi2[ 0 ] = -1.0 * (1.0 - xi);
...
void US_AstfemMath::BasisTR(double *vx, double *vy, double xs, double ys, double *phi, double *phix, double *phiy) {
...
void US_AstfemMath::BasisQR(double *vx, double xs, double ts, double *phi, double *phix, double *phiy, double dt) {
   int i;
...
double US_AstfemMath::Integrand(
   double x_gauss, double D, double sw2, double u, double ux, double ut, double v, double vx) {
   return (x_gauss * ut * v + D * x_gauss * ux * vx - sw2 * sq(x_gauss) * u * vx);
}
...
void US_AstfemMath::initSimData(US_DataIO::RawData &simdata, US_DataIO::EditedData &editdata, double concval1) {
...
void US_AstfemMath::initSimData(US_DataIO::RawData &simdata, QList<US_DataIO::EditedData *> &edats, double concval1) {
...
//  experimental data.
double US_AstfemMath::variance(US_DataIO::RawData &simdata, US_DataIO::EditedData &editdata) {
   QList<int> escns;
...
// Calculate variance (average difference squared) between simulated and
//  experimental data.
double US_AstfemMath::variance(US_DataIO::RawData &simdata, US_DataIO::EditedData &editdata, QList<int> exclScans) {
   ...
   for (int ii = 0; ii < nscan; ii++) { // accumulate sum of differences squared (readings in scans)
...
double US_AstfemMath::calc_bottom(double rpm, double bottom_chan, double *rotorcoefs) {
...
double IntConcentration(QVector<double> r, double *u) {
   ...
   for (int j = 0; j < r.size() - 1; j++) {
      T += (r[ j + 1 ] - r[ j ])
           * ((r[ j + 1 ] - r[ j ]) * (u[ j + 1 ] / 3.0 + u[ j ] / 6.0) + r[ j ] * (u[ j ] + u[ j + 1 ]) / 2.0);
   }
   return T;
}
...
int interpolate(MfemData *simdata, int scans, int points, float *scantimes, double *radius, double **c) {
   /******************************************************************************
 * Interpolation:                                                             *
 *                                                                            *
 ...
 ******************************************************************************/
   ...
   for (i = 0; i < scans; i++) {
      while (count < (*simdata).scan.size() - 1 && scantimes[ i ] >= (*simdata).scan[ count ].time) {
         count++;
      }
