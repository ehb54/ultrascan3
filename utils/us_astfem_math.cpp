...
//----------------------------------------------------------------------
int
US_AstfemMath::writetimestate (const QString &tmst_fpath,
                               ...
                               US_DataIO::RawData &sim_data)
{
  ...
  if (simparams.sim == false)
    { // Handle 1st acceleration zone for real data (compute acceleration rate)
      ...
      t_acc
          = (int)qRound ((double)(simparams.speed_step[0].rotorspeed) / rate);
  ...
  for (int step = 0; step < nspeed; step++)
    {
      ...
      duration
          += (sp->duration_hours * 3600.0) + (sp->duration_minutes * 60.0);
      ...
      for (int ii = d1; ii < d2; ii++)
        {
          ...
          if ((scan_nbr > 0) || (ii < (d1 + 2))
            ...
            DbgLv (1) << "AMATH:wrts: ii(time)" << ii << "scanx scan_nbr"
                      << scanx << scan_nbr << "rpm" << rpm << "omega2t"
                      << omega2t;
...
bool
US_AstfemMath::low_acceleration (
...
bool
US_AstfemMath::timestate_onesec (const QString &tmst_fpath,
                                 US_DataIO::RawData &sim_data)
{
...
void
US_AstfemMath::interpolate_C0 (MfemInitial &C0, double *C1,
                               QVector<double> &xvec)
{
  int ja = 0;
  double *x = xvec.data ();
...
void
US_AstfemMath::interpolate_C0 (MfemInitial &C0, MfemInitial &C1)
{
  int ja = 0;
...
void
US_AstfemMath::zero_2d (int val1, int val2, double **matrix)
{
...
void
US_AstfemMath::initialize_2d (int val1, int val2, double ***matrix)
{
...
void
US_AstfemMath::clear_2d (int val1, double **matrix)
{
  for (int i = 0; i < val1; i++)
    delete[] matrix[i];
...
double
US_AstfemMath::minval (const QVector<double> &value)
{
...
double
US_AstfemMath::minval (const QVector<US_Model::SimulationComponent> &value)
{
  double minimum = 1.0e300;
...
double
US_AstfemMath::maxval (const QVector<double> &value)
{
  const double *avalue = value.data ();
...
double
US_AstfemMath::maxval (const QVector<US_Model::SimulationComponent> &value)
{
  double maximum = -1.0e300;
...
void
US_AstfemMath::initialize_3d (int val1, int val2, int val3, double ****matrix)
{
  *matrix = new double **[val1];
...
void
US_AstfemMath::clear_3d (int val1, int val2, double ***matrix)
{
  for (int i = 0; i < val1; i++)
    {
      for (int j = 0; j < val2; j++)
...
void
US_AstfemMath::tridiag (double *a, double *b, double *c, double *r, double *u,
                        int N)
{
  double bet = b[0];
#ifdef NO_DB
...
double
US_AstfemMath::cube_root (double a0, double a1, double a2)
{
  double x;
...
//////////////////////////////////////////////////////////////////
double
US_AstfemMath::find_C1_mono_Nmer (int n, double K, double CT)
{
...
/////////////////////////////////////////////////////////////////
int
US_AstfemMath::GaussElim (int n, double **a, double *b)
{
  // Elimination
  for (int i = 0; i < n; i++)
    {
...
int
US_AstfemMath::interpolate (MfemData &expdata, MfemData &simdata,
                            bool use_time)
{
  ...
  if (use_time)
    {
        ...
        { // First determine the scan range of current experiment data
          ...
          if (e_time > l_time)
            { // Save scan upper limit (use scan count if close)
              DbgLv (2) << "MATHi: l_time e_time" << l_time << e_time
                        << "expscan" << expscan;
      ...
      // DbgLv(1) << "MATHi: s_time e_time" << s_time << e_time
      //  << "fscan lscan" << fscan << lscan << "escans=" <<  escans <<
      //  "sscans=" << sscans;
    }
  ...
  if (use_time)
    {
      ...
      // DbgLv(1) << "MATHi2: eftime sltime" << eftime << sltime << "fscan
      // lscan" << fscan << lscan; DbgLv(1) << "MATHi2:  erad0 eradn" <<
      ...
      for (int expscan = fscan; expscan < lscan; expscan++)
        { // Interpolate where needed to a range of experiment scans
          ...
          if (s_time2 == e_time)
            ...
            {
              // DbgLv(1) << "MATHi2:  sscm" << sscm << "s_time1 s_time2" <<
              // s_time1 << s_time2
              //  << "e_time" << e_time;
              double a;
              double b;
              ...
              if (simscan == (int)simdata.scan.size ())
                {
                  ...
                  // DbgLv(1) << "e-lsc e_omega e_time" << lsc << e_omega <<
                  // e_time; lsc=simdata.scan.size()-1;
                  // e_omega=simdata.scan[lsc].omega_s_t;
                  // e_time=simdata.scan[lsc].time;
                  // DbgLv(1) << "s-lsc e_omega e_time" << lsc << e_omega <<
                  // e_time;
  ...
  if (tmp_data.radius[0] > expdata.radius[0])
    {
      qDebug () << "Radius comparison: " << tmp_data.radius[0]
                << " (simulated), " << expdata.radius[0] << " (experimental)";
  ...
  for (int ii = 0; ii < neconc; ii++)
    {
          ...
          if (jj == tdradsz)
            {
              qDebug () << "The simulated data does not have enough "
                           ...
                           "and make sure the bottom of the cell is correctly "
      ...
      if (qAbs (tdrad[jj] - exrad[ii]) < 1.0e-8)
        { // they are virtually the same, so simply update the concentration
          // value:
          int ee = 0;
          for (int expscan = fscan; expscan < lscan; expscan++)
      ...
      else // interpolation is needed
        {
          ...
          // DbgLv(1) << "MATHi:   r1 r2 er rr" << radius1 << radius2 <<
          // eradius << radrange
          //  << "fscan lscan" << fscan << lscan;
...
void
US_AstfemMath::QuadSolver (double *ai, double *bi, double *ci, double *di,
...
void
US_AstfemMath::IntQT1 (double *vx, double D, double sw2, double **Stif,
                       double dt)
{
      ...
      for (i = 0; i < 3; i++)
        {
          ...
          dval = Integrand (x_gauss, D, sw2, phiL[i], phiLx[i], phiLy[i], phiC,
                            phiCx);
...
void
US_AstfemMath::IntQTm (double *vx, double D, double sw2, double **Stif,
                       double dt)
{
  // element to define basis functions
  //
...
void
US_AstfemMath::IntQTn2 (double *vx, double D, double sw2, double **Stif,
                        double dt)
{
  // element to define basis functions
  //
...
void
US_AstfemMath::IntQTn1 (double *vx, double D, double sw2, double **Stif,
                        double dt)
{
  // element to define basis functions
  //
  ...
  for (k = 0; k < npts; k++)
    {
      ...
      for (i = 0; i < 3; i++)
        {
          dval = Integrand (x_gauss, D, sw2, phiL[i], phiLx[i], phiLy[i], 1.0,
                            0.0);
          StifR[i][0] += Lam[k][3] * DJac * dval;
        }
    }
...
void
US_AstfemMath::DefineFkp (int npts, double **Lam)
{
...
double
US_AstfemMath::AreaT (double *xv, double *yv)
{
  return (0.5
          * ((xv[1] - xv[0]) * (yv[2] - yv[0])
             - (xv[2] - xv[0]) * (yv[1] - yv[0])));
...
void
US_AstfemMath::BasisTS (double xi, double et, double *phi, double *phi1,
                        double *phi2)
{
...
void
US_AstfemMath::BasisQS (double xi, double et, double *phi, double *phi1,
                        double *phi2)
{
...
void
US_AstfemMath::BasisTR (double *vx, double *vy, double xs, double ys,
...
void
US_AstfemMath::BasisQR (double *vx, double xs, double ts, double *phi,
...
double
US_AstfemMath::Integrand (double x_gauss, double D, double sw2, double u,
...
void
US_AstfemMath::DefineGaussian (int nGauss, double **Gs2)
{
...
void
US_AstfemMath::initSimData (US_DataIO::RawData &simdata,
...
//  experimental data.
double
US_AstfemMath::variance (US_DataIO::RawData &simdata,
...
double
US_AstfemMath::variance (US_DataIO::RawData &simdata,
                         US_DataIO::EditedData &editdata, QList<int> exclScans)
{
...
double
US_AstfemMath::calc_bottom (double rpm, double bottom_chan, double *rotorcoefs)
{
...
static list<_created_matrices>::iterator
find_matrices_alloc (long addr)
{
  list<_created_matrices>::iterator Li;
...
void
IntQT1_ellam (QVector<double> vx, double D, double sw2, double **Stif,
              double dt)
{
  // element to define basis functions
  //
      ...
      for (k = 0; k < npts; k++)
        {
          ...
          xn1 = x_gauss + slope * (dt - y_gauss); // trace-forward point at
                                                  // t_n+1 from (x_g, y_g)
          phiC = (xn1 - vx[2])
...
void
IntQTm_ellam (QVector<double> vx, double D, double sw2, double **Stif,
              double dt)
{
  // element to define basis functions
  //
...
void
IntQTn1_ellam (QVector<double> vx, double D, double sw2, double **Stif,
               double dt)
{
  // element to define basis functions
  //
...
void
QuadSolver_ellam (double *ai, double *bi, double *ci, double *di, double *cr,
                  double *solu, int N)
{
  //
  // solve Quad-diagonal system [a_i, b_i, *c_i*, d_i]*[x]=[r_i]
...
double
IntConcentration (QVector<double> r, double *u)
{
...
int
interpolate (MfemData *simdata, int scans, int points, float *scantimes,
             double *radius, double **c)
{
          ...
          for (j = 0; j < (*simdata).radius.size (); j++)
            {
              slope = ((*simdata).scan[count].conc[j]
                      ...
                      / ((*simdata).scan[count].time
                         - (*simdata).scan[count - 1].time);
  ...
  for (i = 0; i < scans; i++)
    {
      c[i][0]
          += ip_array[i][0]; // meniscus position is identical for all scans
    }
          ...
          if (radius[j] == (*simdata).radius[count])
          ...
          else
            {
              slope = (ip_array[i][count] - ip_array[i][count - 1])
                      / ((*simdata).radius[count]
                         - (*simdata).radius[count - 1]);
              intercept
                  = ip_array[i][count] - (*simdata).radius[count] * slope;
...
void
interpolate_Cfinal (MfemInitial *C0, double *cfinal, QVector<double> *x)
{
  // This routine also considers cases where C0 starts before the meniscus or
  // stops after the bottom is reached. However, C0 needs to cover both
  // meniscus and bottom. In those cases it will fill the C0 vector with the
  // first and/or last value of C0, respectively, for the missing points.
