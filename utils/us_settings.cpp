...
// dataDir
QString US_Settings::dataDir(void) { return (workBaseDir() + "/data"); }

// archiveDir
QString US_Settings::archiveDir(void) { return (workBaseDir() + "/archive"); }

// resultDir
QString US_Settings::resultDir(void) { return (workBaseDir() + "/results"); }

// reportDir
QString US_Settings::reportDir(void) { return (workBaseDir() + "/reports"); }

// etcDir
QString US_Settings::etcDir(void) { return (workBaseDir() + "/etc"); }
...
QString US_Settings::appBaseDir(void) {
  ...
  if (base.contains(
          ".app/Contents")) {  // For Mac, move up path to where ./bin exists
    int ii = base.lastIndexOf("/bin/");

    if (ii > 0) base = base.left(ii);
  }
...
int US_Settings::us_debug(void) { return us_settings_debug; }

void US_Settings::set_us_debug(int level) { us_settings_debug = level; }
#endif
...
QStringList US_Settings::debug_text(void) { return us_sett_debug_text; }
#endif
...
void US_Settings::set_DA_status(const QString& status) {
  ...
  if (status_list[0] == "COM") settings.setValue("daComOpened", status_list[1]);
...
QString US_Settings::status() {
  ...
  switch (settings.status()) {
    ...
    default:
      return QString(
                 "Unknown settings error %1. Perhaps remove the file %2 and "
                 "try again.")
          .arg(settings.status())
          .arg(settings.fileName());
      break;
