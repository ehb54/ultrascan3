...
QString US_Settings::workBaseDir(void) {
   QSettings settings(US3, "UltraScan");
   return settings.value("workBaseDir", QDir::homePath() + "/ultrascan").toString();
}
...
QString US_Settings::appBaseDir(void) {
   ...
   if (base.contains(".app/Contents")) { // For Mac, move up path to where ./bin exists
...
QString US_Settings::status() {
   ...
   switch (settings.status()) {
      ...
      case QSettings::FormatError:
         ...
         break;
      default:
         return QString("Unknown settings error %1. Perhaps remove the file %2 and try again.")
            .arg(settings.status())
