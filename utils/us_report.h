...
class US_UTIL_EXTERN US_Report {
 public:
  ...
  enum Status {
    ...
    NOT_FOUND,  //!< The report, reportTriple or reportDocument ID/GUID was not
                //!< found
  ...
  class US_UTIL_EXTERN ReportTypes {
   public:
    ...
    //! \brief Displays the contents of the class variables in qDebug()
    //! statements
    void show(void);
  };

  /*! \brief      The ReportDocument class describes an individual report
     document. This document would be a png or an html snippet that could be
     included as one of many documents available to be a part of a report
  */
  class US_UTIL_EXTERN ReportDocument {
   public:
    ...
    QString
        filename;  //!< The filename of the disk file that stored this document
    QString analysis;  //!< The type of analysis that this document was derived
                       //!< from (2DSA, GA, dcdt, etc.)
    QString
        subAnalysis;  //!< Further information that helps to identify what type
                      //!< of content it is (report, residuals, rinoise, etc.)
    ...
    */
    ...
    /*! \brief    Function to read the entire current report document from the
       DB
  ...
  /*! \brief      The ReportTriple class describes an individual report. This
     report would ordinarily be associated with a particular runID and triple (
     c/c/w ) within the experiment, and could contain any number of report
     documents.
  */
  class US_UTIL_EXTERN ReportTriple {
   public:
    ...
    QString dataDescription;  //!< The data description from the first line of
    ...
    */
    ...
    /*! \brief    Function to add an empty report document record, both in the
       object and the DB

        \param    editedDataID       The edit profile in effect when the
       document was created
        \param    label              An identifying label
        \param    dir                The directory where the document can be
       found
        \param    filename           The filename of the document itself
        \param    analysis           The type of analysis (vHW, dcdt, 2DSA,
       etc.)
        \param    subAnalysis        The sub-analysis type (residuals, rinoise,
       sdistrib, etc.)
        \param    documentType       The type of document (png, csv, svg, html,
       etc.)
        \param    db For database access, an open database connection
        \return   One of the US_Report error codes
    */
    Status addDocument(int, QString, QString, QString, QString, QString,
                       QString, US_DB2* = 0);
    ...
    */
    ...
    /*! \brief    Function to delete a report document from the DB
        ...
        \param    ndx The index into the docs QVector of the report document to
       delete
        \param    db  For database access, an open database connection
    */
    Status removeDocument(int, US_DB2* = 0);

    /*! \brief    Function to find an existing report document record, based on
       the analysis, subAnalysis, and documentType fields

        \param    searchAnal         The type of analysis (vHW, dcdt, 2DSA,
       etc.)
        \param    searchSubanal      The sub-analysis type (residuals, rinoise,
       sdistrib, etc.)
        \param    searchType         The type of document (png, csv, svg, html,
       etc.)
        \return   The index into the QVector docs that matches all three
       analysis fields, or -1 if no match
    */
  ...
  //! \returns  One of the US_Report error codes
  Status saveDocumentFromFile(const QString&, const QString&, US_DB2*, int = 1,
                              const QString = "");

  Status saveDocumentFromFile_auto(int, const QString&, const QString&, US_DB2*,
                                   int = 1, const QString = "");
  ...
  //!                         the original file, in case adding new triple
  //! \returns  One of the US_Report error codes
  Status saveFileDocuments(const QString&, const QStringList&, US_DB2*, int = 1,
                           const QString = "");
