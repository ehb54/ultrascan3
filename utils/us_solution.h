...
public:
  ...
  //! \brief   Some status codes to keep track of where solution data has been
  //! saved to
  enum solutionStatus
  {
  ...
  double commonVbar20; //!< The weighted average vbar of all analytes present (
                       //!< 20 degrees C)
  ...
  */
  ...
  /*! \brief    Function to delete the current solution from the db, if it
     exists
  ...
  */
  ...
  /*! \brief    Function to find the filename of a solution on disk, if it
     exists. Returns true if successful, false otherwise
  ...
  */
  ...
  /*! \brief    Function to find out where the solution xml files are stored on
     disk Returns true if successful, false otherwise
