...
bool US_Archive::extract(const QString &filename, const QString &path, QStringList *file_list) {
   ...
   if (result != ARCHIVE_OK) {
      error = QObject::tr("US_Archive: Error: %1: %2").arg(archive_error_string(archive), filename);
         ...
         if (file.open(QIODevice::WriteOnly)) {
            ...
            while (archive_read_data_block(archive, &buff, &size, &offset) == ARCHIVE_OK) {
               size_t size_o = static_cast<size_t>(
                  dstream.writeRawData(static_cast<const char *>(buff), static_cast<qint64>(size)));
               if (size != size_o) {
...
bool US_Archive::compress(const QStringList &list, QString &filename) {
   ...
   if (flag && result != ARCHIVE_OK) {
      error = QObject::tr("US_Archive: Error: Failed to initialize archive data structure: %1")
