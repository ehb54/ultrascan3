...
bool
US_Archive::extract (const QString &filename, const QString &path,
                     QStringList *file_list)
{
  ...
  if (!dir.mkpath (dir.path ()))
    {
      error
          = "US_Archive: Error: Could not make the directory: " + dir.path ();
      return false;
    }
  ...
  archive_read_support_filter_all (
      archive); // Enable support for all compression filters

  int result;
  // Open archive file
  result = archive_read_open_filename (archive,
                                       filename.toUtf8 ().constData (), 10240);
  ...
  while (archive_read_next_header (archive, &entry) == ARCHIVE_OK)
    {
      ...
      QString entry_path
          = QObject::tr ("%1").arg (archive_entry_pathname (entry));
      ...
      if (entry_type == AE_IFDIR)
        {
          if (!dir.mkpath (target.absoluteFilePath ()))
            {
              error = "US_Archive: Error: Failed to make path: "
          ...
          if (file.open (QIODevice::WriteOnly))
            {
              ...
              while (archive_read_data_block (archive, &buff, &size, &offset)
                     == ARCHIVE_OK)
                {
                  size_t size_o = static_cast<size_t> (
                      dstream.writeRawData (static_cast<const char *> (buff),
                                            static_cast<qint64> (size)));
                  if (size != size_o)
                    {
                      error
                          = "US_Archive: Error: Failed to write data blocks: "
                            + target.absolutePath ();
                      file.close ();
                      archive_read_close (archive);
                      archive_read_free (archive);
            ...
            {
              error = "US_Archive: Error: Failed to open file: "
                      + target.absolutePath ();
              archive_read_close (archive);
              archive_read_free (archive);
              return false;
...
bool
US_Archive::compress (const QStringList &list, QString &filename)
{
  ...
  if (flag && result != ARCHIVE_OK)
    {
      error = QObject::tr ("US_Archive: Error: Failed to initialize archive "
                           "data structure: %1")
  ...
  // Create archive file
  result
      = archive_write_open_filename (archive, filename.toUtf8 ().constData ());
  if (result != ARCHIVE_OK)
    {
      error = QObject::tr (
                  "US_Archive: Error: Failed to create archive file: %1")
                  .arg (archive_error_string (archive));
      archive_write_free (archive);
      return false;
    }
...
void
US_Archive::list_files (const QString &abs_path, const QString &base_dir)
{
