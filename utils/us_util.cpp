...
QString US_Util::md5sum_file(QString filename) {
   ...
   QString hashandsize = QString(QCryptographicHash::hash(data, QCryptographicHash::Md5).toHex()) + " "
...
QString US_Util::toUTCDatetimeText(QString dttext, bool knownUTC) {
   ...
   if (knownUTC) { // If known to be UTC, just insure no single 'T' and " UTC" suffix
      ...
      if (!haveUTC) { // If no " UTC", convert to UTC and append " UTC"
         ...
         if (dttext.indexOf("-") == 4) { // If already in ISO form, get UTC DateTime from string in ISO form
...
QString US_Util::toISODatetimeText(QString dttext) {
   ...
   if (dttext.contains(" UTC")) { // If UTC form on input, convert by removing " UTC" and adding 'T'
      isotext.replace(" UTC", "").replace(dttext.indexOf(" "), 1, "T");
   }

   else if (dttext.indexOf("-") != 4) { // If not ISO (unknown), convert by back-and-forth from DateTime
...
QString US_Util::compressed_triple(const QString &ccw) {
   ...
   return (
      ccw.section("/", 0, 0).simplified() + ccw.section("/", 1, 1).simplified() + ccw.section("/", 2, 2).simplified());
}
...
int US_Util::listlistParse(QList<QStringList> &lsl, QString &llstring) {
   ...
   if (delim1.isEmpty() || delim2.isEmpty()) { // Ouch!!  Input string doesn't even have delimeters
      lsl.clear();
      return nlelem;
   }
