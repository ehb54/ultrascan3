...
#define SHM_KEY_BASE 0xe1000000
  ...
  if (sharedMemory.attach()) {
    ...
    switch (sharedMemory.error()) {
        ...
        // this is the expected path when shared memory doesn't already exist
        {
          if (sharedMemory.create(sizeof global)) {
            ...
            switch (sharedMemory.error()) {
              ...
              case QSharedMemory::AlreadyExists:  // bad ... possible on race
                                                  // condition ... could try
                                                  // attach again
              case QSharedMemory::
                  PermissionDenied:  // bad ... likely needs shmutil fix (or
                                     // system reboot if no one is available to
                                     // fix with shmutil)
              case QSharedMemory::OutOfResources:  // bad ... system out of
                                                   // resources, need admin help
      ...
      case QSharedMemory::UnknownError:      // bad
      default: {
        qDebug() << "US_Global() constructor : unexpected sharedMemory attach "
                    "error : "
...
#if !defined(Q_OS_WIN) && !defined(Q_OS_MACOS)
  // we are not locking, hopefully not an issue, if strange issues are apparent,
  // could add a separate locking semaphore
...
QString US_Global::errorString() { return errors.join("\n"); }
