...
#define SHM_KEY_BASE 0xe1000000
   ...
   if (sharedMemory.attach()) {
      ...
      switch (sharedMemory.error()) {
            ...
            // this is the expected path when shared memory doesn't already exist
            {
               if (sharedMemory.create(sizeof global)) {
                  ...
                  switch (sharedMemory.error()) {
                     ...
                     case QSharedMemory::
                        PermissionDenied: // bad ... likely needs shmutil fix (or system reboot if no one is available to fix with shmutil)
                     ...
                     default: {
                        qDebug() << "US_Global() constructor : unexpected sharedMemory create error : "
         ...
         case QSharedMemory::UnknownError: // bad
         default: {
            qDebug() << "US_Global() constructor : unexpected sharedMemory attach error : " << sharedMemory.error()
                     << " " << sharedMemory.errorString();
