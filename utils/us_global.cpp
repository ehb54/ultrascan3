...
US_Global::US_Global ()
{
  ...
  // the SHM_KEY_BASE can be adjusted if the current value clobbers some other
  // program's usage
#define SHM_KEY_BASE 0xe1000000
  ...
  // shm_open
  int shmid = shmget (key, sizeof (Global), IPC_CREAT | 0600);
  if (shmid == -1)
    {
      valid = false;
  ...
  if (sharedMemory.attach ())
  ...
  else
    {
      switch (sharedMemory.error ())
        ...
        case QSharedMemory::NotFound:
          // this is the expected path when shared memory doesn't already exist
          {
            if (sharedMemory.create (sizeof global))
              {
                // qDebug() << "us_global constructor sharedMemory.create()
                // ok";
                valid = true;
                set_global_position (QPoint (50, 50));
                setPasswd ("");
            ...
            else
              {
                ...
                switch (sharedMemory.error ())
                  ...
                  case QSharedMemory::AlreadyExists: // bad ... possible on
                                                     // race condition ...
                                                     // could try attach again
                  case QSharedMemory::
                      PermissionDenied: // bad ... likely needs shmutil fix (or
                                        // system reboot if no one is available
                                        // to fix with shmutil)
                  case QSharedMemory::OutOfResources: // bad ... system out of
                                                      // resources, need admin
                                                      // help
                  case QSharedMemory::InvalidSize:    // bad
                  case QSharedMemory::UnknownError:   // bad
                  default:
                    {
                      qDebug () << "US_Global() constructor : unexpected "
                                   "sharedMemory create error : "
        ...
        case QSharedMemory::PermissionDenied: // bad ... likely requires admin
                                              // intervention, shmutil to clear
        case QSharedMemory::KeyError:         // strange if key is invalid
        case QSharedMemory::UnknownError:     // bad
        default:
          {
            qDebug () << "US_Global() constructor : unexpected sharedMemory "
                         "attach error : "
                      << sharedMemory.error () << " "
                      << sharedMemory.errorString ();
...
void
US_Global::set_global_position (const QPoint &p)
{
  read_global ();
...
QPoint
US_Global::global_position (void)
{
  read_global ();
...
void
US_Global::setPasswd (const QString &pw)
{
  read_global ();
...
void
US_Global::read_global (void)
{
#if !defined(Q_OS_WIN) && !defined(Q_OS_MACOS)
  // we are not locking, hopefully not an issue, if strange issues are
  // apparent, could add a separate locking semaphore
  memcpy ((void *)&global, shmbuf, sizeof (Global));
#else
  sharedMemory.lock ();
  char *from = (char *)sharedMemory.data ();
  memcpy ((char *)&global, from,
          qMin (sharedMemory.size (), (int)sizeof global));
  sharedMemory.unlock ();
#endif
}

void
US_Global::write_global (void)
{
#if !defined(Q_OS_WIN) && !defined(Q_OS_MACOS)
  // we are not locking, hopefully not an issue, if strange issues are
  // apparent, could add a separate locking semaphore
