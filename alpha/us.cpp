...
void US_Win::launch(int index) {
  ...
  if (ln_time.secsTo(QDateTime::currentDateTime()) > trig_secs) notice_check();
...
void US_Win::help(int index) {
  ...
  switch (index) {
    ...
    case HELP_ABOUT:
      QMessageBox::information(this, tr("About UltraScan..."),
...
bool US_Win::notice_check() {
  ...
  if (db.lastErrno() != US_DB2::OK || db.numRows() == 0) {
    qDebug() << "US:NOTE: No DB notices" << db.lastError() << "numRows"
             << db.numRows();
    return do_abort;
  }
  ...
  QString srev =
      US_Version + "." + QString(REVISION).section(":", 1, 1).simplified();
  ...
  if (s_rev > i_rev) return do_abort;
