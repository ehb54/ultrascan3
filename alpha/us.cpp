...
US_Win::US_Win(QWidget *parent, Qt::WindowFlags flags) : QMainWindow(parent, flags) {
   ...
   addMenu(P_PCSA, tr("&Parametrically Constrained Spectrum Analysis"), velocity);
...
void US_Win::logo(int width) {
   ...
   QString version = "Version " + US_Version + " ( " REVISION " ) for " OS_TITLE; // REVISON is #define "Revision: xxx"
...
bool US_Win::notice_check() {
   ...
   if (db.lastErrno() != US_DB2::OK || db.numRows() == 0) {
      qDebug() << "US:NOTE: No DB notices" << db.lastError() << "numRows" << db.numRows();
      return do_abort;
   }
   ...
   QString srev = US_Version + "." + QString(REVISION).section(":", 1, 1).simplified();
   ...
   while (db.next()) {
      ...
      if (nnotice == 1) {
         ...
         time_d = time_m.secsTo(time_d) > 0 ? time_d : time_m;
      }
   ...
   QString msg_note = tr("UltraScan III notices posted  (") + time_d.toString("yyyy/MM/dd") + "):\n\n";

   for (int ii = 0; ii < nnotice; ii++) {
      // Skip messages for warn/crit same revision or any earlier than current
      if ((irevs[ ii ] == s_rev && types[ ii ] != "info") || irevs[ ii ] < s_rev)
         continue;

      // Add current message to full text
      msg_note += typeMap[ types[ ii ] ] + " for release " + revs[ ii ] + ":\n" + msgs[ ii ] + "\n";
