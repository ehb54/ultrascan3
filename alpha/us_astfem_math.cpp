                                   ...
                                   US_DataIO::RawData& sim_data )
{
  ...
  //  tmst_fname   = dirname;      // Write to a temporary file and update simparams 
   US_TimeState timestate;
   int     nspeed       = simparams.speed_step.size();
   
   //DbgLv(1)<<"AstfemMath::writetimestate : tmst fname=	"<< tmst_fname 
   //        <<"defs_fname   ="<< defs_fname 
   //        <<  "runid=	"<<run_id;
   //---------------------------
    if ( timestate.open_write_data( tmst_fname, 0.0, 0.0 ) != 0 )
  { 
     // DbgLv(1)<<"Unable to open"; 
      return 0;
   }
   
   //DbgLv(1)<< "Subha : Number of speeds=        " << nspeed <<
...
//   double t1 = (w2t - t2*c)/(d*c);
   ...
   {   
       scantimes.push_back( sim_data.scanData[i].seconds );
      
       //DbgLv(1)<<"scantimes"<<scantimes [i] << sim_data.scanData[i].omega2t << i << sim_data.scanData[i].rpm;
  ...
  //----------------------------------
   if ( simparams.sim == false ) 
   {
      ...
      double t1 = (w2t - (t2*c))/(d*c);     
      t_acc = ( qCeil ) (t1) ;
      rate  =( double) (simparams.speed_step[0].rotorspeed)/(t_acc);
      //--------------------------------------------------
   ...
   else
   {
      ...
      //rate = 400.0 ;
      t_acc = qCeil( sp->rotorspeed/rate );
      rate  = sp->rotorspeed/ double(t_acc) ;
  ...
  //----------------------------------
   ...
   for ( int step = 0; step < nspeed; step++)
   {
       ...
       {   duration_prev += (simparams.speed_step[step-1].duration_hours*3600.0)
           ...
           if ( simparams.sim == false )
              t_acc = ( qCeil )(double(qAbs (speed - prvs_speed ))/rate) ; // Use calculated rate when user doesn't know it
           else
              t_acc = ( qCeil )(double(qAbs (speed - prvs_speed ))/ sp->acceleration) ;// When user knows acceleration rate

           rate  = (double) (speed - prvs_speed ) / (double ) t_acc ;

           //if ( speed < prvs_speed ) rate = -rate ;
       }       

       for ( i = d1; i<= int(duration); i++ )
       {
           int tacc = d1 + t_acc  ;
                                ...
                                bool      use_time )
{
   ...
   if ( use_time )
   {
      ...
      //DbgLv(1) << "subha_MATHi: s_time e_time" << s_time << e_time
      //         << "fscan lscan" << fscan << lscan <<"expscansize=" <<  escans << "simscansize="<< sscans;

      //qDebug() << "subha_MATHi: s_time e_time" << s_time << e_time
      //         << "fscan lscan" << fscan << lscan <<"expscansize=" <<  escans << "simscansize="<< sscans;
...
/*   QString runID     = editdata.runID;
   ...
   if ( (check_file.exists()) && (check_file.isFile()) )
   {
      ...
      qDebug()<<"2dsa : timestate file exists"<< tmst_fpath << " timestateobject = "<<dset->simparams.tsobj;
      
   }
   else
   qDebug()<<"2dsa : timestate file does not exist";*/
  //-----------------------------
