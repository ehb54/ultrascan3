...
//   patm_dlay     = settings.value( "slideDelay", PA_TMDIS_MS ).toInt();
   ...
   //{   
   //   DbgLv(1)<<"show_norm_model_values"<<model->components[i].s << model->components[i].signal_concentration<<  nsolutes ;  
   //}
...
void US_show_norm::plot_data( void )
{
   ...
   //for ( int i= 0; i< sol_d->size() ; i++ )
   //{
   //    DbgLv(1)<<"sol_d values"<<sol_d->at(i).s << sol_d->at(i).k << sol_d->at(i).c <<  sol_d->count();
   //}
...
/*void US_show_norm::update_curr_distr( double dval )
{
...
/*
void US_show_norm::select_conloop()
{
...
void US_show_norm::load_distro( US_Model model, QString mdescr )
{
   DisSys      tsys;
   S_Solute    sol_sk;
   S_Solute    sol_xy;
...
DbgLv(1) << "LD:  analys_name" << tsys.analys_name;
   ...
   if ( tsys.distro_type != (int)US_Model::COFS )
   {
      ...
      for ( int jj = 0; jj < nsolmc; jj++ )
      {
         ...
         ol_sk.w  = model.components[ jj ].mw;
...
/*
void US_show_norm::timerEvent( QTimerEvent *event )
{
...
void US_show_norm::remove_distro( void )
{
...
void US_show_norm::build_xy_distro()
{
  ...
  /*
   ...
   for ( int ii = 0; ii < nsolsk; ii++ )
   {
      ...
      sol_xy.s  = ( plot_x == ATTR_K ) ? sol_sk.k : sol_xy.s;
      sol_xy.s  = ( plot_x == ATTR_W ) ? sol_sk.w : sol_xy.s;
      sol_xy.s  = ( plot_ == ATTR_V ) ? sol_sk.v : sol_xy.s;
      sol_xy.s  = ( plot_x == ATTR_D ) ? sol_sk.d : sol_xy.s;
      sol_xy.s  = ( plot_x == ATTR_F ) ? sol_sk.f : sol_xy.s;
      sol_xy.k  = ( plot_y == ATTR_S ) ? sol_sk.s : sol_xy.k;
      sol_xy.k  = ( plot_y == ATTR_K ) ? sol_sk.k : sol_xy.k;
...
QwtLinearColorMap* US_show_norm::ColorMapCopy( QwtLinearColorMap* colormap )
{
   ...
   QwtLinearColorMap* cmapcopy = new QwtLinearColorMap( colormap->color1(),
                                                        colormap->color2() );
   //DbgLv(1)<< "lstop_ColorMapCopy" << lstop << cstops.size();
