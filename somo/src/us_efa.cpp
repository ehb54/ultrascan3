   ...
   for ( int n_use = 0; n_use < n; ++n_use ) {
...
#if defined( DO_PRINTOUT )
      {
         ...
         ts << ( rightEfa ? n - n_use - 1 : n_use ) << " "
            ...
            << rpt[ 4 ]
            << "\n";
         // US_Vector::printvector( QString( "US_Efa::Efa result[ %1 ]" ).arg( n_use ), rpt );
      }
#endif
#if defined( SHOW_TIMINGS )
...
#endif
         efa_thr_threads[j]->efa_thr_setup(
                                           a_t,
                                           m,
                                           n_t,
                                           w_t,
