...
class GetRequest : public Q3Http
{
   ...
   GetRequest( QString sendtext, QObject *parent=0, const char *name=0 ) : Q3Http( parent, name ) 
   {
      ...
      connect( this, SIGNAL( readyRead ( const Q3HttpResponseHeader & ) ), SLOT( http_readyRead ( const Q3HttpResponseHeader & ) ) );
...
class ClientSocket : public Q3Socket
{
   ...
   ClientSocket( int sock, QObject *parent=0, const char *name=0 ) : Q3Socket( parent, name )
   {
...
class SimpleServer : public Q3ServerSocket
{
   Q_OBJECT
public:
   SimpleServer( unsigned int port, QObject* parent=0 ) : Q3ServerSocket( port, 1, parent )
   {
      if ( !ok() ) {
