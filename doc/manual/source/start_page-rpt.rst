...
.. list-table::
  ...
  * - **Speed**
    - The rotor speed used during the experiment. This is one of the most important experimental descriptors because sedimentation behavior depends directly on angular velocity.
  * - **Temperature**
...
.. list-table::
  ...
  *  - Time- and radially invariant noise profiles
...
Visual outputs are often the first means by which a user identifies sample heterogeneity, discrete species, broad distributions, or possible model artifacts.
...
For each simulated parameter or attribute, UltraScan3 can estimate the range within which the true value is expected to lie with 95% confidence. These intervals are critical for distinguishing well-resolved parameters from poorly constrained ones.
