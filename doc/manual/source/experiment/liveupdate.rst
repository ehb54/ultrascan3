...
.. Note::
  The **Skip Stage** bottom allows user to skip any of the pre-defined stage (for example: temperature delay) and the **STOP** bottom allows user to stop run. Usually this is to stop run when a lack(s) is notices. It is not recommended to stop a run before the pre-defined run time has finished. Use the `ASTFAM simulation module <../astfem_sim.html>`_ to estimate appropriate runtime for experiment).
