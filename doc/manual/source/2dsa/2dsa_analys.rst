...
*  **Step 1:** *Define the Grid and Grid points:* With *Fitting Controls*, define the overall solute grid by giving the lower and upper limits of s and f/f0, and the number of grid points for s and f/f0. The default of 64 grid points works well for typical sedimentation velocity datasets. Divide that total grid into subgrids using Grid Refinements.
*  **Step 2:** *Set Refinement Iterations:* Secondly, set a value for Maximum Iterations of refinement passes.
*  **Step 3:** *Set any Meniscus/Monte Carlo:* Thirdly, if desired, set parameters defining a Meniscus scan or a set of Monte Carlo iterations.
*  **Step 4:** *Set Threads:* Next, after control values are set, define a number of threads that is appropriate to the complexity of the run and the number of processors or cores available on your machine. Typically, 8-16 threads work best for single-wavelength datasets.
...
.. list-table::
  ...
  * - **Improvement:**
    - The difference between the variance value from the last iteration and the one preceding it. 
  * - **Status:**
...
.. list-table::
  ...
  * - **Custom Grid**
    - Check this box to load a custom grid as the preferred optimization method. A :doc:`Load Distribution dialog <../common_dialogs.html#load-distribution-model>`_ to load the custom grid generated by the `Custom Grid Editor (CG) <../grid_editor>`. 
  * - **Float Meniscus:**
  ...
  * - **Advanced Analysis Controls**
    - Click on this button to open the :doc:`2DSA Advanced Controls <2dsa_adv>` dialog with control parameters not of interest to the typical user.
...
Detailed information on the results of a 2DS analysis is available at :doc:`Simulated Finite Element Model Viewer <../fe_match/index>`
