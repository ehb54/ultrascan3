...
class US_GUI_EXTERN US_SelectItem : public US_WidgetsDialog {
 ...
 public:
  ...
  US_SelectItem(QList<QStringList>&, QStringList&, const QString, int*,
                const int = (-1));

  // Same as above but with Delete button and delete functionality for
  // us_experiment
  US_SelectItem(QList<QStringList>&, QStringList&, const QString, int*, QString,
                const int = (-1));
  ...
  US_SelectItem(QList<QStringList>&, QStringList&, const QString, QList<int>*,
                const int = (-2));
 ...
 private:
  ...
  bool autoflow_button;          // If autoflow, Cancel becomes define another
                         // Experiment
 ...
 private slots:
  ...
  void help(void) { showHelp.show_help("select_item.html"); };
