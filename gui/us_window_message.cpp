...
US_WindowMessage::Interpreter::Interpreter(ulong ntrpid, QString iname, QString fname) {
   interp_id = ntrpid;
   interp_name = iname;
   file_name = fname;
}
...
int US_WindowMessage::findAllInterps(void) {
   ...
   if (nchar > 0) { // Parse the character array to build a list of interpreter objects
...
int US_WindowMessage::findLiveInterps() {
   ...
   for (int ii = 0; ii < nintrp; ii++) { // Loop to generate the "live" list from the "all"
      Interpreter intrp = all_interps[ ii ];

      if (isLive(intrp)) { // If live, add to the list
         live_interps << intrp;
      }

      else { // Otherwise, add an identifying string to the zombie list
         zombie_list << QString().sprintf("%lx : ", intrp.interp_id) + intrp.interp_name;
      }
   }
...
ulong US_WindowMessage::addInterp(const QString filename) {
   ...
   for (int ii = 0; ii < nintrp; ii++) {
      ...
      if (!ids.contains(lvid)) { // This is a newly added interpreter, so save its index
         newx = ii;
         id = lvid;
      }
   }

   if (newx >= 0) { // Update the file name for the newly added interpreter
...
//  of interpreters.
int US_WindowMessage::fill_interps(int nchar, char *wiresult, QList<Interpreter> &intrps) {
   int niknt = 0;
   intrps.clear();
