...
int US_WindowMessage::findAllInterps(void) {
  ...
  if (nchar >
      0) {  // Parse the character array to build a list of interpreter objects
...
int US_WindowMessage::findLiveInterps() {
  ...
  for (int ii = 0; ii < nintrp;
       ii++) {  // Loop to generate the "live" list from the "all"
...
ulong US_WindowMessage::addInterp(const QString filename) {
  ...
  for (int ii = 0; ii < nintrp; ii++) {
    ...
    if (!ids.contains(
            lvid)) {  // This is a newly added interpreter, so save its index
      newx = ii;
      id = lvid;
    }
  }
...
int US_WindowMessage::interpIDs(QList<ulong>& ids) {
  ...
  for (int ii = 0; ii < nntrp; ii++) ids << live_interps[ii].interp_id;
...
int US_WindowMessage::interpNames(QStringList& names) {
  ...
  for (int ii = 0; ii < nntrp; ii++) names << live_interps[ii].interp_name;
...
int US_WindowMessage::interpFiles(QStringList& fnames) {
  ...
  for (int ii = 0; ii < nntrp; ii++) fnames << live_interps[ii].file_name;
