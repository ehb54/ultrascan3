...
QMap<QString, QString> US_Passwd::getPasswd_auditTrail(QString title,
                                                       QString i_text,
                                                       QString user_s) {
  // // If the pw is in global memory, return it
  // QString pw = g.passwd();
  ...
  while (true) {
    ...
    // break if Cancelled
    if (reject_chosen) break;

    // Check the hash
    QByteArray hash =
        QCryptographicHash::hash(pw.toLatin1(), QCryptographicHash::Sha1);

    if (hash == currentHash && !form_map["Comment:"].isEmpty()) break;

    QMessageBox::critical(
        this, tr("No Comment and/or Password Error"),
        tr("<font color='red'><b>ERROR:</b></font> The Comment field is empty, "
