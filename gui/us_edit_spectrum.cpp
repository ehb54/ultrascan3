    ...
    : US_Widgets ()
{
  ...
  QLabel *bn_info_new
      = us_banner (tr (" %1 has NO spectrum data: Choose among the options to "
                       "enter new spectrum")
                       .arg (type));
  bn_info_new->setSizePolicy (QSizePolicy::Preferred, QSizePolicy::Fixed);
  QLabel *bn_info_exists
      = us_banner (tr (" %1 has spectrum data: Choose among the options to "
                       "edit the spectrum")
                       .arg (type));
...
void
US_EditSpectrum::view_spectrum (void)
{
...
void
US_EditSpectrum::delete_spectrum (void)
{
  US_Passwd pw;
...
void
US_EditSpectrum::uploadDisk (void)
{
  add_spectrumDisk ();
}

void
US_EditSpectrum::uploadFit (void)
{
...
void
US_EditSpectrum::process_results (QMap<double, double> &xyz)
{
  if (type == "BUFFER")
...
void
US_EditSpectrum::add_spectrumDisk (void)
{
  QStringList files;
  QFile f;
...
void
US_EditSpectrum::readingspectra (const QString &fileName)
{
  QString str1;
  QStringList strl;
  float temp_x, temp_y;
...
void
US_EditSpectrum::editmanually (void)
{
  US_Table *sdiag;
