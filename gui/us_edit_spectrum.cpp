...
US_EditSpectrum::US_EditSpectrum(
   QString tmp_type, QString tmp_exists, const QString &tmp_text, const QString &tmp_text_e280, US_Buffer *tmp_buffer) :
    US_Widgets() {
   buffer = tmp_buffer;
   text = tmp_text;
   type = tmp_type;
   ...
   QLabel *bn_info_new = us_banner(
      tr(" %1 has NO spectrum data: Choose among the options to enter new spectrum").arg(type));
   bn_info_new->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Fixed);
   QLabel *bn_info_exists = us_banner(
      tr(" %1 has spectrum data: Choose among the options to edit the spectrum").arg(type));
...
US_EditSpectrum::US_EditSpectrum(
   QString tmp_type, QString tmp_exists, const QString &tmp_text, const QString &tmp_text_e280,
...
void US_EditSpectrum::delete_spectrum(void) {
   ...
   if (type == "BUFFER")
      q << "get_bufferID" << buffer->GUID;
   if (type == "ANALYTE")
      q << "get_analyteID" << analyte->analyteGUID;
   if (type == "SOLUTION")
   ...
   if (status == US_DB2::OK) {
      ...
      QMessageBox::information(this, tr("Deletion: Success"), tr("Spectrum was successfully deleted"));
...
void US_EditSpectrum::uploadDisk(void) {
   add_spectrumDisk();
}
...
void US_EditSpectrum::add_spectrumDisk(void) {
   ...
   dialog.setNameFilter(
      tr("Text files (*.[Tt][Xx][Tt] *.[Cc][Ss][Vv] *.[Dd][Aa][Tt] *.[Ww][Aa]* *.[Dd][Ss][Pp]);;All files (*)"));
...
void US_EditSpectrum::cancel(void) {
   this->close();
}
