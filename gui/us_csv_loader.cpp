...
void CSVTableView::contextMenuEvent(QContextMenuEvent *event) {
  ...
  QString styleSheet =
      "QMenu { background-color: rgb(255, 253, 208); }"
      "QMenu::item { background: transparent; }"
      "QMenu::item:selected { background-color: transparent; color: red; "
      "font-weight: bold; }";
...
#else
  connect(bg_delimiter, QOverload<int>::of(&QButtonGroup::buttonClicked), this,
          &US_CSV_Loader::fill_table);
#endif
}

QString US_CSV_Loader::error_message() { return error_msg; }
...
US_CSV_Data US_CSV_Loader::data() { return csv_data; }
...
void US_CSV_Loader::cancel() { reject(); }
...
bool US_CSV_Loader::parse_dsp_file(const QString &filepath) {
  ...
  for (int ii = 0; ii < ns; ++ii) {
    ...
    if (np != scans.at(ii).size()) {
      error_msg = tr("Multiple scans detected, but each has different number "
                     "data points!\n\n%1")
                      .arg(filepath);
      return false;
    }
