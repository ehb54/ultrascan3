...
bool
CSVSortFilterProxyModel::lessThan (const QModelIndex &left,
...
void
CSVTableView::contextMenuEvent (QContextMenuEvent *event)
{
...
US_CSV_Loader::set_UI ()
{
  ...
  connect (pb_add_header, &QPushButton::clicked, this,
           &US_CSV_Loader::add_header);
  connect (pb_ok, &QPushButton::clicked, this, &US_CSV_Loader::ok);
  connect (pb_cancel, &QPushButton::clicked, this, &US_CSV_Loader::cancel);
  connect (le_other, &QLineEdit::textChanged, this,
           &US_CSV_Loader::new_delimiter);
  connect (pb_save_csv, &QPushButton::clicked, this,
           &US_CSV_Loader::save_csv_clicked);
  connect (pb_reset, &QPushButton::clicked, this, &US_CSV_Loader::reset);
  connect (tv_data, &CSVTableView::column_deleted, this,
           &US_CSV_Loader::column_deleted);
  connect (tv_data, &CSVTableView::row_deleted, this,
           &US_CSV_Loader::row_deleted);
  connect (model, &QStandardItemModel::itemChanged, this,
           &US_CSV_Loader::item_changed);
  connect (pb_show_red, &QPushButton::clicked, this, &US_CSV_Loader::show_red);
#if QT_VERSION >= QT_VERSION_CHECK(5, 15, 0)
  connect (bg_delimiter, QOverload<int>::of (&QButtonGroup::idClicked), this,
           &US_CSV_Loader::fill_table);
#else
  connect (bg_delimiter, QOverload<int>::of (&QButtonGroup::buttonClicked),
           this, &US_CSV_Loader::fill_table);
#endif
}

QString
US_CSV_Loader::error_message ()
{
  return error_msg;
}
...
void
US_CSV_Loader::item_changed (QStandardItem *item)
{
  int row = item->row ();
  if (row == 0)
    {
...
US_CSV_Loader::add_header ()
{
  ...
  qDebug () << nr;
  qDebug () << nc;
  QFont font (US_Widgets::fixedFont ().family (), US_GuiSettings::fontSize ());
...
US_CSV_Loader::save_csv_clicked ()
{
  ...
  QString file_path = QFileDialog::getSaveFileName (
      this, "Save CSV File", US_Settings::workBaseDir (), "(CSV)(*.csv)");
  ...
  if (csv_data.writeFile (delimiter_str))
  ...
  else
    {
      QMessageBox::warning (
          this, "Error!",
          tr ("Error in saving the CSV file!\n%1").arg (csv_data.error ()));
      csv_data.clear ();
    }
}

void
US_CSV_Loader::cancel ()
{
  reject ();
}

bool
US_CSV_Loader::check_file (const QString &filepath)
{
  QFile file (filepath);
  error_msg.clear ();
...
bool
US_CSV_Loader::parse_file (const QString &filepath)
{
...
bool
US_CSV_Loader::parse_dsp_file (const QString &filepath)
{
  ...
  for (int ii = 0; ii < ns; ++ii)
    {
      ...
      if (np != scans.at (ii).size ())
        {
          error_msg = tr ("Multiple scans detected, but each has different "
                          "number data points!\n\n%1")
                          .arg (filepath);
          return false;
        }
...
void
US_CSV_Loader::new_delimiter (const QString &)
{
  if (delimiter == OTHER)
    {
      delimiter = NONE;
...
void
US_CSV_Loader::fill_table (int id)
{
  if (delimiter == id)
    {
      return;
  ...
  relabel ();
  check_table ();
  tv_data->horizontalHeader ()->setSectionResizeMode (
...
bool
US_CSV_Loader::make_csv_data (QString &error)
{
  csv_data.clear ();
  error.clear ();
  QStringList headers;
      ...
      for (int ii = 0; ii < nrows; ii++)
        {
          QModelIndex index = tv_data->model ()->index (ii, jj);
          QVariant item
              = tv_data->model ()->itemData (index).value (Qt::DisplayRole);
          if (ii == 0)
            {
...
QStringList
US_CSV_Loader::gen_alpha_list (int num)
{
