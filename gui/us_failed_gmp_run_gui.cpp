...
US_FailedRunGui::US_FailedRunGui (QMap<QString, QString> run_details)
    : US_Widgets ()
{
...
US_FailedRunGui::save_new (void)
{
  ...
  qDebug () << "Setting Info for FAILED run: stage, reason -- " << f_stage
            << f_reason;
  ...
  QStringList qry;
  qry << "new_autoflow_failed_record" << autoflowID << f_stage << f_reason;
  ...
  if (!autoflowFailedID)
    {
      QMessageBox::warning (
          ...
          tr ("autoflowFailed: There was a problem with creating a record in "
              "autoflowFailed table \n"));
  ...
  qry.clear ();
  qry << "update_autoflow_failedID" << autoflowID
  ...
  if (status == US_DB2::NO_AUTOFLOW_RECORD)
    {
      QMessageBox::warning (this, tr ("Autoflow's failedID field Not Updated"),
