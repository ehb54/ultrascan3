...
US_ReportGui::US_ReportGui(QMap<QString, US_ReportGMP *> report_map) : US_Widgets() {
   ...
   connect(le_tot_conc, SIGNAL(textChanged(const QString &)), this, SLOT(verify_text(const QString &)));
   connect(le_rmsd_limit, SIGNAL(textChanged(const QString &)), this, SLOT(verify_text(const QString &)));
   connect(le_av_intensity, SIGNAL(textChanged(const QString &)), this, SLOT(verify_text(const QString &)));
   connect(le_tot_conc_tol, SIGNAL(textChanged(const QString &)), this, SLOT(verify_text(const QString &)));
   connect(le_duration_tol, SIGNAL(textChanged(const QString &)), this, SLOT(verify_text(const QString &)));

   qDebug() << "Report params on load: tot_conc, conc_tol, duraiton, duration_tol -- " << report->tot_conc
            << report->tot_conc_tol << report->experiment_duration << report->experiment_duration_tol;
   ...
   connect(sb_durat_dd, SIGNAL(valueChanged(int)), this, SLOT(ssChgDuratTime_dd(int)));
...
void US_ReportGui::build_report_layout(void) {
  ...
  ***/
   ...
   for (int ii = 0; ii < r_item_num; ii++) {
      ...
      qDebug() << "ABDE reportGui replot: curr_item.type, type_ind -- " << curr_item.type << type_ind;
      ...
      //set connecitons btw textChanged() and slot
      connect(le_low, SIGNAL(textChanged(const QString &)), this, SLOT(verify_text(const QString &)));
      connect(le_high, SIGNAL(textChanged(const QString &)), this, SLOT(verify_text(const QString &)));
      connect(le_intval, SIGNAL(textChanged(const QString &)), this, SLOT(verify_text(const QString &)));
      connect(le_tol, SIGNAL(textChanged(const QString &)), this, SLOT(verify_text(const QString &)));
      connect(le_total, SIGNAL(textChanged(const QString &)), this, SLOT(verify_text(const QString &)));
   ...
   pseudo3d_box->setStyleSheet(
      QString("QGroupBox { font:bold; font-size: %1pt; background-color: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, "
              "stop: 0 #E0E0E0, stop: 1 #FFFFFF); border: 2px solid gray; border-radius: 10px; margin-top: 10px; "
...
void US_ReportGui::gui_to_report(void) {
   ...
   if (
      report->pseudo3d_2dsait_s_ff0 || report->pseudo3d_2dsait_s_d || report->pseudo3d_2dsait_mw_ff0
      || report->pseudo3d_2dsait_mw_d || report->pseudo3d_2dsamc_s_ff0 || report->pseudo3d_2dsamc_s_d
      || report->pseudo3d_2dsamc_mw_ff0 || report->pseudo3d_2dsamc_mw_d || report->pseudo3d_pcsa_s_ff0
      || report->pseudo3d_pcsa_s_d || report->pseudo3d_pcsa_mw_ff0 || report->pseudo3d_pcsa_mw_d) {
...
void US_ReportGui::apply_all_wvls(void) {
   ...
   QString msg_text = QString(
      "Do you want to apply current settings to all other triples?\n\n(Existing settings will be overwritten...)");
   msgBox.setText(msg_text);
   ...
   if (msgBox.clickedButton() == Accept) {
      ...
      qDebug() << "In apply_to_all: current_report_key, report->wavelength -- " << current_report_key
               << report->wavelength;
