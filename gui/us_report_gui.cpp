...
US_ReportGui::US_ReportGui (QMap<QString, US_ReportGMP *> report_map)
    : US_Widgets ()
{
  abde_mode = false;
  ...
  qDebug () << "Wvl_passed to ReportGui -- " << wvl_passed
            << ", wvl_passed[0].toInt() -- " << wvl_passed[0].toInt ();
  ...
  QLabel *bn_report
      = us_banner (QString (tr ("Report Parameters for channel: %1"))
                       .arg (report->channel_name),
                   1);
  ...
  le_tot_conc_tol
      = us_lineedit (QString::number (report->tot_conc_tol), 0, false);
  le_duration_tol = us_lineedit (
      QString::number (report->experiment_duration_tol), 0, false);

  le_rmsd_limit = us_lineedit (QString::number (report->rmsd_limit), 0, false);
  le_av_intensity
      = us_lineedit (QString::number (report->av_intensity), 0, false);
  ...
  connect (le_tot_conc, SIGNAL (textChanged (const QString &)), this,
           SLOT (verify_text (const QString &)));
  connect (le_rmsd_limit, SIGNAL (textChanged (const QString &)), this,
           SLOT (verify_text (const QString &)));
  connect (le_av_intensity, SIGNAL (textChanged (const QString &)), this,
           SLOT (verify_text (const QString &)));
  connect (le_tot_conc_tol, SIGNAL (textChanged (const QString &)), this,
           SLOT (verify_text (const QString &)));
  connect (le_duration_tol, SIGNAL (textChanged (const QString &)), this,
           SLOT (verify_text (const QString &)));

  qDebug () << "Report params on load: tot_conc, conc_tol, duraiton, "
               "duration_tol -- "
            << report->tot_conc << report->tot_conc_tol
            << report->experiment_duration << report->experiment_duration_tol;
  ...
  connect (cb_wvl, SIGNAL (currentIndexChanged (int)), this,
           SLOT (changeWvl (int)));
  /////////////////////////////////////////////////////////////////////////////
  ...
  connect (pb_prev_wvl, SIGNAL (clicked ()), this, SLOT (wvl_prev ()));
  connect (pb_next_wvl, SIGNAL (clicked ()), this, SLOT (wvl_next ()));
  ////////////////////////////////////////////////////////////////////////////

  pb_apply_all = us_pushbutton (tr ("Apply to all wvls"), true, 0);
  connect (pb_apply_all, SIGNAL (clicked ()), this, SLOT (apply_all_wvls ()));
...
// ABDE
void
US_ReportGui::abde_mode_passed (void)
{
  abde_mode = true;
  build_report_layout ();
}
...
// Exp. Durat. counters
void
US_ReportGui::ssChgDuratTime_dd (int val)
{
  report->exp_time_changed = true;
}
void
US_ReportGui::ssChgDuratTime_hh (int val)
{
  report->exp_time_changed = true;
}
void
US_ReportGui::ssChgDuratTime_mm (int val)
{
  report->exp_time_changed = true;
}
void
US_ReportGui::ssChgDuratTime_ss (int val)
{
  report->exp_time_changed = true;
}
...
// Layout build
void
US_ReportGui::build_report_layout (void)
{
  ...
  QHBoxLayout* lo_duratlay  = us_ddhhmmsslay( 0, 0,0,0,1, &sb_durat_dd,
  &sb_durat_hh, &sb_durat_mm, &sb_durat_ss ); // ALEXEY 0 - visible, 1 - hidden

  le_tot_conc      = us_lineedit( QString::number(report->tot_conc),     0,
  false  );

  le_tot_conc_tol  = us_lineedit( QString::number(report->tot_conc_tol),     0,
  false  ); le_duration_tol  = us_lineedit(
  QString::number(report->experiment_duration_tol),     0, false  );

  le_rmsd_limit    = us_lineedit( QString::number(report->rmsd_limit),   0,
  false  ); le_av_intensity  = us_lineedit(
  QString::number(report->av_intensity), 0, false  ); if (
  report->channel_name.contains("Interf.") ) le_av_intensity->setEnabled( false
  );
  ...
  **/
  ...
  // QSpacerItem* spacer1 = new QSpacerItem( 20, 0.75*ihgt,
  // QSizePolicy::Expanding ); params->setRowStretch( row, 1 );
  ...
  ***/
  ...
  if (!abde_mode)
    {
      sl_types << QString ("s") << QString ("D") << QString ("f/f0")
               << QString ("MW");
      sl_methods << QString ("2DSA-IT") << QString ("PCSA-SL/DS/IS")
                 << QString ("2DSA-MC");
    }
  else
    {
      sl_types << QString ("Radius") << QString ("vbar")
               << QString ("Density");
  ...
  for (int ii = 0; ii < r_item_num; ii++)
    {
      ...
      le_intval = us_lineedit (QString::number (curr_item.integration_val), 0,
                               false);
      le_total
          = us_lineedit (QString::number (curr_item.total_percent), 0, true);
      ...
      // set connecitons btw textChanged() and slot
      connect (le_low, SIGNAL (textChanged (const QString &)), this,
               SLOT (verify_text (const QString &)));
      connect (le_high, SIGNAL (textChanged (const QString &)), this,
               SLOT (verify_text (const QString &)));
      connect (le_intval, SIGNAL (textChanged (const QString &)), this,
               SLOT (verify_text (const QString &)));
      connect (le_tol, SIGNAL (textChanged (const QString &)), this,
               SLOT (verify_text (const QString &)));
      connect (le_total, SIGNAL (textChanged (const QString &)), this,
               SLOT (verify_text (const QString &)));
      ...
      connect (cb_type, SIGNAL (activated (int)), this,
               SLOT (type_changed (int)));
      connect (cb_method, SIGNAL (activated (int)), this,
               SLOT (method_changed (int)));
    }

  int ihgt = lb_low->height ();
  QSpacerItem *spacer2
      = new QSpacerItem (20, 1 * ihgt, QSizePolicy::Expanding);
  ...
  pseudo3d_box->setStyleSheet (
      QString ("QGroupBox { font:bold; font-size: %1pt; background-color: "
               "qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 #E0E0E0, "
               "stop: 1 #FFFFFF); border: 2px solid gray; border-radius: "
               "10px; margin-top: 10px; margin-bottom: 10px; padding-top: "
               "5px; } QGroupBox::title { subcontrol-origin: margin; "
               "subcontrol-position: top left; left: 10px; margin: 0 5px; "
               "background-color: black; color: white; padding: 0 3px;}  "
               "QGroupBox::indicator { width: 13px; height: 13px; border: 1px "
               "solid grey; background-color: rgba(204, 204, 204, 255);} "
               "QGroupBox::indicator:hover {background-color: rgba(235, 235, "
               "235, 255);} QLabel {background-color: rgb(105,105,105);}")
          .arg (f_size));
...
void
US_ReportGui::report_mask (void)
{
}

// Slot to update report
void
US_ReportGui::update_report (void)
{
  // Check for errors
  ...
  /**
  //now ask if you want to apply all settings for current report to all other
  channels' reports QMessageBox msgBox;
  msgBox.setIcon(QMessageBox::Information);
  msgBox.setWindowTitle(tr("Apply to Other Reports"));

  QString msg_text      = QString("Do you want to apply current report settings
  to all other channels?"); msgBox.setText( msg_text );
...
// Verify text
void
US_ReportGui::verify_text (const QString &text)
{
  ...
  if (curr_widget != NULL)
    {
      if (!rx_double.exactMatch (text))
      ...
      else
        {
          ...
          // ALEXEY: for a given row, compute 'Fraction of Total' (read-only)
          // as '(Int_Val / Total Conc.) * 100%'
          if (oname.contains (": intval") && !abde_mode)
            {
              ...
              QLineEdit *fraction_of_total_widget
                  = containerWidget->findChild<QLineEdit *> (
                      objName_fraction_of_total);

              double fraction
                  = (curr_widget->text ().toDouble () / tot_conc_val) * 100.0;
          ...
          // ALEXEY: check the same for tot_conc: apply to all ReportItems'
          // 'Fraction of Total'
          if (oname.contains ("tot_conc") && !abde_mode)
            {
              int r_item_num = report->reportItems.size ();

              qDebug () << "In Verify_text: report->reportItems.size() -- "
                        << r_item_num;

              for (int ii = 0; ii < r_item_num; ii++)
                {
                  QString objName_integration_val
                      = QString::number (ii) + QString (": intval");
                  QString objName_fraction_of_total
                      = QString::number (ii) + QString (": total");
                  QLineEdit *integration_val_widget
                      = containerWidget->findChild<QLineEdit *> (
                          objName_integration_val);
                  QLineEdit *fraction_of_total_widget
                      = containerWidget->findChild<QLineEdit *> (
                          objName_fraction_of_total);

                  double fraction
                      = (integration_val_widget->text ().toDouble ()
                         / text.toDouble ())
                        * 100.0;
                  fraction_of_total_widget->setText (
...
void
US_ReportGui::gui_to_report (void)
{
  // Main params
  ...
  qDebug () << "Gui-to-report: DURATION: dd hh mm ss -- " << t_day << t_hour
            << t_minute << t_second;
  ...
  qDebug () << "Gui-to-report: reportItems.size()  -- "
            << report->reportItems.size ();
  ...
  for (int ii = 0; ii < report->reportItems.size (); ii++)
    {
      ...
      // type
      QComboBox *cb_type
          = containerWidget->findChild<QComboBox *> (stchan + "type");
      qDebug () << "ii, cb_type->currentText()" << ii
                << cb_type->currentText ();
      ...
      if (!abde_mode)
        {
          QComboBox *cb_method
              = containerWidget->findChild<QComboBox *> (stchan + "method");
          qDebug () << "ii, cb_method->currentText()" << ii
                    << cb_method->currentText ();
      ...
      // range_low
      QLineEdit *le_low
          = containerWidget->findChild<QLineEdit *> (stchan + "low");
      qDebug () << "ii, le_low->text()" << ii << le_low->text ();
      report->reportItems[ii].range_low = le_low->text ().toDouble ();

      // range_high
      QLineEdit *le_high
          = containerWidget->findChild<QLineEdit *> (stchan + "high");
      ...
      if (!abde_mode)
        {
          QLineEdit *le_intval
              = containerWidget->findChild<QLineEdit *> (stchan + "intval");
          qDebug () << "ii, le_intval->text()" << ii << le_intval->text ();
          report->reportItems[ii].integration_val
              = le_intval->text ().toDouble ();
        }

      // tolerance
      QLineEdit *le_tol
          = containerWidget->findChild<QLineEdit *> (stchan + "tol");
      qDebug () << "ii, le_tol->text()" << ii << le_tol->text ();
      report->reportItems[ii].tolerance = le_tol->text ().toDouble ();

      // percent
      QLineEdit *le_total
          = containerWidget->findChild<QLineEdit *> (stchan + "total");
      qDebug () << "ii, le_total->text()" << ii << le_total->text ();
      report->reportItems[ii].total_percent = le_total->text ().toDouble ();

      // combined plot
      QCheckBox *ck_combinedplot
          = containerWidget->findChild<QCheckBox *> (stchan + "combined_plot");
      qDebug () << "ii, ck_combinedplot->isChecked()" << ii
                << ck_combinedplot->isChecked ();
      report->reportItems[ii].combined_plot
          = ck_combinedplot->isChecked () ? 1 : 0;

      // ind. combined plot
      QCheckBox *ck_combinedplot_ind
          = containerWidget->findChild<QCheckBox *> (stchan
                                                     + "ind_combined_plot");
      qDebug () << "ii, ck_combinedplot_ind->isChecked()" << ii
                << ck_combinedplot_ind->isChecked ();
      report->reportItems[ii].ind_combined_plot
          = ck_combinedplot_ind->isChecked () ? 1 : 0;
    }
  ...
  if (report->pseudo3d_2dsait_s_ff0 || report->pseudo3d_2dsait_s_d
      || report->pseudo3d_2dsait_mw_ff0 || report->pseudo3d_2dsait_mw_d
      || report->pseudo3d_2dsamc_s_ff0 || report->pseudo3d_2dsamc_s_d
      || report->pseudo3d_2dsamc_mw_ff0 || report->pseudo3d_2dsamc_mw_d
      || report->pseudo3d_pcsa_s_ff0 || report->pseudo3d_pcsa_s_d
      || report->pseudo3d_pcsa_mw_ff0 || report->pseudo3d_pcsa_mw_d)
    {
...
void
US_ReportGui::cancel_update (void)
{
  QMessageBox msgBox;
  msgBox.setIcon (QMessageBox::Critical);
  msgBox.setWindowTitle (tr ("Cancel Updates"));

  // QString msg_text      = QString("Attention! Do you want to cancel all
  // report updates for channel %1?").arg( channel_name );
  QString msg_text = QString ("Attention! Do you want to cancel all report "
                              "updates for the current channel?");
  ...
  if (msgBox.clickedButton () == Accept)
    {
      ...
      // qDebug() << "Sizes: report CHANGED,  report_original -- " <<
      // report->reportItems.size() << report_copy_original.reportItems.size();

      // report = &( report_copy_original );

      // qDebug() << "Sizes: report RESTORED, report_original -- " <<
      // report->reportItems.size() << report_copy_original.reportItems.size();
      // qDebug() << "Report_copy_original.channel_name -- " <<
      // report_copy_original.channel_name; emit cancel_changes(
      // report_copy_original );
...
US_ReportGui::add_row (void)
{
  ...
  double tot_conc_val = le_tot_conc->text ().toDouble ();
  initItem.total_percent
      = (abde_mode) ? 95 : (initItem.integration_val / tot_conc_val) * 100.0;
...
int
US_ReportGui::check_syntax (void)
{
  int syntax_errors = 0;
...
void
US_ReportGui::changeWvl (int ndx)
{
  // Check for errors
  int syntax_errors = check_syntax ();
  if (syntax_errors != 0)
  ...
  // first, save all changes for current | old (if clicked pb_prev | pb_next )
  // triple's report under consideration:
  gui_to_report ();
  ...
  // At this point, we need to temporarily disconnect upper-portion GUi widgets
  // from ::verify_Text()
...
// Slot to apply current triple's settings to the rest of triples (wvls) in a
// channel
void
US_ReportGui::apply_all_wvls (void)
{
  // Check for errors
  int syntax_errors = check_syntax ();
  if (syntax_errors != 0)
  ...
  QString msg_text
      = QString ("Do you want to apply current settings to all other "
  ...
  if (msgBox.clickedButton () == Accept)
    {
      ...
      qDebug ()
          << "In apply_to_all: current_report_key, report->wavelength -- "
          << current_report_key << report->wavelength;
      ...
      for (ri = report_map.begin (); ri != report_map.end (); ++ri)
        {
          ...
          report_map[ri.key ()]->experiment_duration
              = report->experiment_duration;

          report_map[ri.key ()]->tot_conc_tol = report->tot_conc_tol;
          report_map[ri.key ()]->experiment_duration_tol
              = report->experiment_duration_tol;
          ...
          report_map[ri.key ()]->experiment_duration_mask
              = report->experiment_duration_mask;
          report_map[ri.key ()]->integration_results_mask
              = report->integration_results_mask;
          report_map[ri.key ()]->plots_mask = report->plots_mask;

          // Pseuso 3D plots Mask params.
          report_map[ri.key ()]->pseudo3d_2dsait_s_ff0
              = report->pseudo3d_2dsait_s_ff0;
          report_map[ri.key ()]->pseudo3d_2dsait_s_d
              = report->pseudo3d_2dsait_s_d;
          report_map[ri.key ()]->pseudo3d_2dsait_mw_ff0
              = report->pseudo3d_2dsait_mw_ff0;
          report_map[ri.key ()]->pseudo3d_2dsait_mw_d
              = report->pseudo3d_2dsait_mw_d;
          report_map[ri.key ()]->pseudo3d_2dsamc_s_ff0
              = report->pseudo3d_2dsamc_s_ff0;
          report_map[ri.key ()]->pseudo3d_2dsamc_s_d
              = report->pseudo3d_2dsamc_s_d;
          report_map[ri.key ()]->pseudo3d_2dsamc_mw_ff0
              = report->pseudo3d_2dsamc_mw_ff0;
          report_map[ri.key ()]->pseudo3d_2dsamc_mw_d
              = report->pseudo3d_2dsamc_mw_d;
          report_map[ri.key ()]->pseudo3d_pcsa_s_ff0
              = report->pseudo3d_pcsa_s_ff0;
          report_map[ri.key ()]->pseudo3d_pcsa_s_d = report->pseudo3d_pcsa_s_d;
          report_map[ri.key ()]->pseudo3d_pcsa_mw_ff0
              = report->pseudo3d_pcsa_mw_ff0;
          report_map[ri.key ()]->pseudo3d_pcsa_mw_d
          ...
          for (int ic = 0; ic < report->reportItems.size (); ++ic)
            report_map[ri.key ()]->reportItems.push_back (
...
void
US_ReportGui::type_changed (int t)
{
  QObject *sobj = sender (); // Sender object
  QString oname = sobj->objectName ();
  ...
  // method
  QComboBox *cb_method
      = containerWidget->findChild<QComboBox *> (method_oname);
  // low
...
void
US_ReportGui::SetComboBoxItemEnabled (QComboBox *comboBox, int index,
                                      bool enabled)
{
...
void
US_ReportGui::method_changed (int m)
{
}
