...
US_ReportGui::US_ReportGui(QMap<QString, US_ReportGMP*> report_map)
  ...
  if (wvl_passed.size() == 1 && !wvl_passed[0].toInt()) wvl_passed.clear();
  ...
  QLabel* bn_report = us_banner(QString(tr("Report Parameters for channel: %1"))
  ...
  le_duration_tol =
      us_lineedit(QString::number(report->experiment_duration_tol), 0, false);
  ...
  qDebug()
      << "Report params on load: tot_conc, conc_tol, duraiton, duration_tol -- "
      << report->tot_conc << report->tot_conc_tol << report->experiment_duration
  ...
  connect(cb_wvl, SIGNAL(currentIndexChanged(int)), this, SLOT(changeWvl(int)));
  /////////////////////////////////////////////////////////////////////////////
  ...
  /////////////////////////////////////////////////////////////////////////////
  if (init_index == 0) pb_prev_wvl->setEnabled(false);
  if ((init_index + 1) == cb_wvl->count()) pb_next_wvl->setEnabled(false);
...
void US_ReportGui::build_report_layout(void) {
  ...
  **/
  ...
  // QSizePolicy::Expanding ); params->setRowStretch( row, 1 ); params->addItem(
  // spacer1,  row++,  0, 1, 1 );
  /***
  topContainerWidget = new QWidget;
  topContainerWidget->setLayout( params );
  ...
  ***/
  ...
  for (int ii = 0; ii < r_item_num; ii++) {
    ...
    le_intval =
        us_lineedit(QString::number(curr_item.integration_val), 0, false);
    ...
    connect(cb_type, SIGNAL(activated(int)), this, SLOT(type_changed(int)));
    connect(cb_method, SIGNAL(activated(int)), this, SLOT(method_changed(int)));
  }
  ...
  if (!report->channel_name.contains("Interf.")) {
    if (report->reportItems.size() < 2) pb_removeRow->setEnabled(false);
  } else {
    if (report->reportItems.size() < 1) pb_removeRow->setEnabled(false);
  }
  ...
  pseudo3d_box->setStyleSheet(
      QString("QGroupBox { font:bold; font-size: %1pt; background-color: "
              ...
              "stop: 1 #FFFFFF); border: 2px solid gray; border-radius: 10px; "
...
void US_ReportGui::update_report(void) {
  ...
  if (syntax_errors != 0) {
    ...
    QString message_error =
        QString(tr("There are %1 syntax errors!"
...
void US_ReportGui::verify_text(const QString& text) {
  ...
  if (curr_widget != NULL) {
    if (!rx_double.exactMatch(text)) {
      ...
      // ALEXEY: for a given row, compute 'Fraction of Total' (read-only) as
      // '(Int_Val / Total Conc.) * 100%'
      if (oname.contains(": intval") && !abde_mode) {
        ...
        QLineEdit* fraction_of_total_widget =
            containerWidget->findChild<QLineEdit*>(objName_fraction_of_total);
      ...
      if (oname.contains("tot_conc") && !abde_mode) {
        ...
        for (int ii = 0; ii < r_item_num; ii++) {
          ...
          QLineEdit* integration_val_widget =
              containerWidget->findChild<QLineEdit*>(objName_integration_val);
          QLineEdit* fraction_of_total_widget =
              containerWidget->findChild<QLineEdit*>(objName_fraction_of_total);

          double fraction =
              (integration_val_widget->text().toDouble() / text.toDouble()) *
              100.0;
          fraction_of_total_widget->setText(QString::number(fraction));
          palette->setColor(QPalette::Base, QColor(0xe0, 0xe0, 0xe0));
          fraction_of_total_widget->setPalette(*palette);
...
void US_ReportGui::gui_to_report(void) {
  ...
  for (int ii = 0; ii < report->reportItems.size(); ii++) {
    ...
    QCheckBox* ck_combinedplot_ind =
        containerWidget->findChild<QCheckBox*>(stchan + "ind_combined_plot");
...
void US_ReportGui::cancel_update(void) {
  ...
  QString msg_text = QString(
      "Attention! Do you want to cancel all report updates for the current "
      "channel?");
...
void US_ReportGui::wvl_next(void) {
  ...
  if ((row + 1) <= cb_wvl->count()) {
    ...
    if ((row + 1) == cb_wvl->count()) pb_next_wvl->setEnabled(false);
    qDebug() << "WVL_NEXT: 3";
  } else
...
void US_ReportGui::wvl_prev(void) {
  ...
  if (row >= 0) {
    ...
    if (row == 0) pb_prev_wvl->setEnabled(false);
...
void US_ReportGui::apply_all_wvls(void) {
  ...
  QString msg_text = QString(
      "Do you want to apply current settings to all other "
  ...
  if (msgBox.clickedButton() == Accept) {
    ...
    qDebug() << "In apply_to_all: current_report_key, report->wavelength -- "
             << current_report_key << report->wavelength;
    // iterate over channel's triples:
    QMap<QString, US_ReportGMP*>::iterator ri;
    for (ri = report_map.begin(); ri != report_map.end(); ++ri) {
      if (ri.key().contains(current_report_key)) continue;
...
void US_ReportGui::SetComboBoxItemEnabled(QComboBox* comboBox, int index,
                                          bool enabled) {
