...
US_ReportGenGui::US_ReportGenGui (QString reportMask) : US_Widgets ()
{
  ...
  QLabel *bn_detail = us_banner (
      QString (
          tr ("Define Report's Representation of the Protocol Settings: ")),
      1);
...
// ABDE
void
US_ReportGenGui::abde_mode_passed (void)
{
  abde_mode = true;
  build_layout ();
}
...
void
US_ReportGenGui::build_layout (void)
{
  ...
  for (int i = 0; i < topLevelItems.size (); ++i)
    {
      ...
      topItem[topItemName]
          = new QTreeWidgetItem (treeWidget, topItemNameList, wiubase);
  ...
  // QSizePolicy::Expanding); genL->setRowStretch( row, 1 ); genL->addItem(
  // spacer2,  row++,  0, 1, 1 );
...
// get Item's children
void
US_ReportGenGui::get_item_childs (QList<QTreeWidgetItem *> &children_list,
...
void
US_ReportGenGui::changedItem (QTreeWidgetItem *item, int col)
{
      ...
      if ( children_lev1 )
        {
          ...
          for( int i = 0; i < children_lev1; ++i )
            {
              item->child(i)->setCheckState( 0, (Qt::CheckState)
item->checkState(0) );
              ...
              if ( children_lev2 )
                {
                  for( int ii = 0; ii < children_lev2; ++ii )
                    {
                      item->child(i)->child(ii)->setCheckState( 0,
(Qt::CheckState) item->child(i)->checkState(0) );
                    }
                }
            }
      ...
      if ( parent_item )
        {
          ...
          //qDebug() << " Current item, " << item->text( 1 ) << ", has a
parent: " << parent_item->text( 1 );
...
void
US_ReportGenGui::gui_to_parms (void)
{
  // tree-to-json: genTree && json-to-genMask structure
  QString mask_edited = tree_to_json (topItem);
  // parse_edited_gen_mask_json( mask_edited, genMask_edited );  // <-- not
  // here, will be in the analysis program
...
//
void
US_ReportGenGui::get_children_to_json (QString &mask_edited,
                                       QTreeWidgetItem *item)
{
...
QString
US_ReportGenGui::tree_to_json (QMap<QString, QTreeWidgetItem *> topLevItems)
{
  QString mask_edited;
  mask_edited += "{";
...
void
US_ReportGenGui::cancel_selection (void)
{
  close ();
}

void
US_ReportGenGui::update_selection (void)
{
  gui_to_parms ();
  close ();
}
