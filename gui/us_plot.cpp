...
US_Plot::US_Plot (QwtPlot *&parent_plot, const QString &title,
                  ...
                  const bool cmEnab, const QString cmMatch,
  ...
  if (on)
    {
      ...
      connect (zoomer, SIGNAL (zoomed (QwtDoubleRect)), this,
               SLOT (scale_yRight (QwtDoubleRect)));
#else
      connect (zoomer, SIGNAL (zoomed (QRectF)),
               SIGNAL (zoomedCorners (QRectF)));
      connect (zoomer, SIGNAL (zoomed (QRectF)), this,
               SLOT (scale_yRight (QRectF)));
#endif
...
US_Plot::csv (void)
{
  ...
  QString fileName = QFileDialog::getSaveFileName (
      plot, tr ("Export File Name"), reportDir, tr ("CSV Documents (*.csv)"));
...
US_Plot::svg (void)
{
  ...
  QString fileName = QFileDialog::getSaveFileName (
      plot, tr ("Export File Name"), reportDir, tr ("SVG Documents (*.svgz)"));

  if (!fileName.isEmpty ())
    {
      if (fileName.right (5) != ".svgz")
        fileName += ".svgz";
...
US_Plot::png (void)
{
  ...
  QString fileName = QFileDialog::getSaveFileName (
      plot, tr ("Export File Name"), reportDir, tr ("PNG Documents (*.png)"));
...
void
US_Plot::colorMap (void)
{
  qDebug () << "UP:CM: colorMap chosen";
  // Get an xml file name for the color map
  QString filter = tr ("Color Map files (*cm-*.xml);;")
                   + tr ("Any XML files (*.xml);;") + tr ("Any files (*)");

  cmfpath = QFileDialog::getOpenFileName (
      (QWidget *)plot, tr ("Load Color Map File"), US_Settings::etcDir (),
      filter, 0, 0);
        ...
        {           // This is a curve
          ...
          if (!tmatch || ctitle.contains (cmMatch))
            {           // No matching or this curve's title matches
              ...
              // qDebug() << "UP:CM:   ii" << ii << "mcolx" << mcolx <<
              // "mcolor" << mcolors[mcolx];
...
int
US_Plot::map_colors (QList<QColor> &mcolors)
{
  int nmcol = 0;
...
#if QT_VERSION < 0x050000
void
US_Plot::scale_yRight (QwtDoubleRect)
{
}
#else
void
US_Plot::scale_yRight (QRectF rect)
{
  if (!plot->axisEnabled (QwtPlot::yRight))
    {
      return;
...
US_PlotPushbutton::US_PlotPushbutton (const QString &labelString, QWidget *w,
                                      int index)
    : QPushButton (labelString.toLatin1 (), w)
{
...
void
US_PlotPushbutton::us_plotClicked (void)
{
...
US_PlotConfig::US_PlotConfig (QwtPlot *current_plot, QWidget *p,
                              Qt::WindowFlags f)
    : US_WidgetsDialog (p, f, false) //( false, p, f )
{
  ...
  le_titleFont = us_lineedit (titleFont.family () + ", "
                                  + QString::number (titleFont.pointSize ())
                                  + tr (" points"),
                              1);
  ...
  connect (pb_showCanvasColor, SIGNAL (clicked ()),
           SLOT (selectCanvasColor ()));
  ...
  if (kk < 0)
  ...
  else
    {
      ...
      // qDebug() << " padding_v --         " <<  padding_v.left(
      // padding_v.indexOf( "px" ) ); qDebug() << " padding_v.toInt() -- " <<
      // padding_v.left( padding_v.indexOf( "px" ) ).toInt();
  ...
  connect (cmbb_margin, SIGNAL (activated (int)), SLOT (selectMargin (int)));
  ...
  connect (pb_legendFont, SIGNAL (clicked ()), SLOT (updateLegendFont ()));
  ...
  US_PlotPushbutton *pb_rightAxis = new US_PlotPushbutton (
      tr ("Update Right Axis"), this, QwtPlot::yRight);

  US_PlotPushbutton *pb_topAxis
      = new US_PlotPushbutton (tr ("Update Top Axis"), this, QwtPlot::xTop);

  US_PlotPushbutton *pb_bottomAxis = new US_PlotPushbutton (
      tr ("Update Bottom Axis"), this, QwtPlot::xBottom);
...
*/
void
US_PlotConfig::updateTitleText (const QString &newText)
{
...
void
US_PlotConfig::updateTitleFont (void)
{
  bool ok;
  ...
  if (ok)
    {
      ...
      le_titleFont->setText (newFont.family () + ", "
                             + QString::number (newFont.pointSize ())
                             + tr (" points"));
    }
}
...
US_PlotConfig::selectCanvasColor (void)
{
  ...
  qDebug () << "Canvas Color Selected in Plot(): "
            << plot->canvasBackground ();
}

/*!  \brief Change the frame color */
void
US_PlotConfig::selectFrameColor (void)
{
...
*/
void
US_PlotConfig::selectMargin (int index)
{
...
*/
void
US_PlotConfig::selectLegendPos (int index)
{
...
void
US_PlotConfig::setLegendFontString (void)
{
  ...
  if (cmbb_legendPos->currentIndex () == 0)
  ...
  else
    {
      ...
      legendFontString = legendFont.family () + ", "
                         + QString::number (legendFont.pointSize ())
                         + tr (" points");
    }
...
void
US_PlotConfig::updateLegendFont (void)
{
  if (cmbb_legendPos->currentIndex () == 0)
    {
      QMessageBox::warning (
          this, tr ("Plog Config Warning"),
  ...
  bool ok;
  QFont newFont
      = QFontDialog::getFont (&ok, font, this, tr ("Set Legend Font"), 0);
...
void
US_PlotConfig::updateAxis (int axis)
{
 ...
 */
QFont
US_PlotConfig::jsonToFont (QJsonObject font_obj)
{
  QFont font;
  foreach (const QString &key, font_obj.keys ())
    {
      ...
      if (key.compare (tr ("Family"), Qt::CaseInsensitive) == 0)
        ...
        font.setWeight (static_cast<QFont::Weight> (
            QMetaEnum::fromType<QFont::Weight> ().keyToValue (
                value.toUtf8 ())));
      else if (key.compare (tr ("Style"), Qt::CaseInsensitive) == 0)
        font.setStyle (static_cast<QFont::Style> (
            QMetaEnum::fromType<QFont::Style> ().keyToValue (
                value.toUtf8 ())));
  ...
  qDebug () << "Weight: "
            << QString::fromUtf8 (
                   QMetaEnum::fromType<QFont::Weight> ().valueToKey (
                       font.weight ()));
  qDebug () << "Style: "
            << QString::fromUtf8 (
                   QMetaEnum::fromType<QFont::Style> ().valueToKey (
                       font.style ()));
...
QMap<QString, bool>
US_PlotConfig::parseGridJson (QJsonObject grid_obj, QPen *pen)
{
  ...
  foreach (const QString &key, grid_obj.keys ())
    {
      ...
      if (key.compare (tr ("Style"), Qt::CaseInsensitive) == 0)
        pen->setStyle (static_cast<Qt::PenStyle> (
            QMetaEnum::fromType<Qt::PenStyle> ().keyToValue (
                value.toUtf8 ())));
...
void
US_PlotConfig::setTitleJson (QJsonObject title_obj)
{
  QFont font;
  QString text;
...
void
US_PlotConfig::setGridJson (QJsonObject grid_obj)
{
...
void
US_PlotConfig::setAxisJson (int axis_id, QJsonObject axis_obj)
{
  ...
  foreach (const QString &key, axis_obj.keys ())
    {
      ...
      if (key.compare (tr ("Enable"), Qt::CaseInsensitive) == 0)
        ...
        {
          //            QPalette axisPalette = plot->axisWidget( axis_id
          //            )->palette(); axisPalette.setColor( QPalette::Text,
          //            QColor(value.toString()) ); plot->axisWidget( axis_id
          //            )->setPalette( axisPalette );
          continue;
        }
      else if (key.compare (tr ("Scale"), Qt::CaseInsensitive) == 0)
        {
          ...
          //            QPalette palette = plot->axisWidget( axis_id
          //            )->palette(); bool linear = true; bool logarithmic =
          //            false; QFont font = plot->axisFont( axis_id );
          QFont font;
          foreach (const QString &kk, json_obj.keys ())
            {
              QJsonValue value = json_obj.value (kk);
              if (kk.compare (tr ("Autoscale"), Qt::CaseInsensitive) == 0)
                {
                  if (value.toString ().compare ("false", Qt::CaseInsensitive)
                      == 0)
                    auto_scale = false;
                }
              else if (kk.compare (tr ("Color"), Qt::CaseInsensitive) == 0)
              ...
              else if (kk.compare (tr ("Floating Endpoints"),
                                   Qt::CaseInsensitive)
                       == 0)
                {
                  if (value.toString ().compare ("true", Qt::CaseInsensitive)
                      == 0)
                    attributes |= (int)QwtScaleEngine::Floating;
                }
              else if (kk.compare (tr ("Include Reference"),
                                   Qt::CaseInsensitive)
                       == 0)
                {
                  if (value.toString ().compare ("true", Qt::CaseInsensitive)
              ...
              else if (kk.compare (tr ("Lower Bound"), Qt::CaseInsensitive)
                       == 0)
                {
                  lower_bound = value.toString ().toDouble ();
                }
              else if (kk.compare (tr ("Upper Bound"), Qt::CaseInsensitive)
                       == 0)
                {
              ...
              else if (kk.compare (tr ("Logarithmic"), Qt::CaseInsensitive)
                       == 0)
                {
                  if (value.toString ().compare ("true", Qt::CaseInsensitive)
                      == 0)
          ...
          plot->axisScaleEngine (axis_id)->setAttributes (
              (QwtScaleEngine::Attribute)attributes);
          plot->replot ();
        }
    }
}
...
void
US_PlotConfig::loadPlotProfile (void)
{
  ...
  QString p_filename
      = QFileDialog::getOpenFileName (this, tr ("Open Plot's Profile"),
                                      dirPath, tr ("Profile Files (*.json)"));
  ...
  if (json_plot_profile.open (QIODevice::ReadOnly))
    {
      ...
      if (jsonError.error != QJsonParseError::NoError)
        {
          qDebug () << "Reading JSON Plot profile failed: "
          ...
          foreach (const QString &key, json_obj.keys ())
            {
              ...
              else if (key.compare (tr ("Canvas Color"), Qt::CaseInsensitive)
                       == 0)
                canvas_color = value.toString ();
              else if (key.compare (tr ("Frame Color"), Qt::CaseInsensitive)
                       == 0)
                frame_color = value.toString ();
              else if (key.compare (tr ("Border Margin"), Qt::CaseInsensitive)
                       == 0)
                border_margin = value.toString ();
              else if (key.compare (tr ("Legend Position"),
                                    Qt::CaseInsensitive)
                       == 0)
  ...
  //	   << ", Title Font (Family, Size): (" << p_title_font_family << ", "
  //<< p_title_font_size << ")";
  ...
  // 2. Border Margin
  plot->setStyleSheet (
      QString ("QwtPlot{ padding: %1px }").arg (border_margin));
...
QJsonObject
US_PlotConfig::getAxisJson (int axis_id)
{

  QJsonObject axis_obj;

  QString axis_enable
      = plot->axisEnabled (axis_id) ? tr ("true") : tr ("false");
  ...
  QString tick_color = plot->axisWidget (axis_id)
                           ->palette ()
  ...
  scale_obj.insert (
      "Color",
      plot->axisWidget (axis_id)->palette ().color (QPalette::Text).name ());
  ...
  QString floating
      = (attributes & QwtScaleEngine::Floating) ? tr ("true") : tr ("false");
  QString inverted
      = (attributes & QwtScaleEngine::Inverted) ? tr ("true") : tr ("false");
  QString inc_ref = (attributes & QwtScaleEngine::IncludeReference)
                        ? tr ("true")
                        : tr ("false");
  QString symmetric
      = (attributes & QwtScaleEngine::Symmetric) ? tr ("true") : tr ("false");
  QString autoscale
      = (plot->axisAutoScale (axis_id)) ? tr ("true") : tr ("false");
...
QJsonObject
US_PlotConfig::getFontJson (QFont font)
{
  QJsonObject font_obj;
  QString font_size = QString::number (font.pointSize ());
  QString font_weight = QString::fromUtf8 (
      QMetaEnum::fromType<QFont::Weight> ().valueToKey (font.weight ()));
  QString font_style = QString::fromUtf8 (
      QMetaEnum::fromType<QFont::Style> ().valueToKey (font.style ()));
  QString font_underline = font.underline () ? "true" : "false";
  QString font_strikeout = font.strikeOut () ? "true" : "false";
...
void
US_PlotConfig::savePlotProfile (void)
{
  ...
  // frame & Canvas color
  QString frame_color = plot->palette ()
  ...
  if (kk < 0)
  ...
  else
    {
      ...
      padding_v.indexOf ("px");
      qDebug () << " padding_v --         "
                << padding_v.left (padding_v.indexOf ("px"));
      qDebug () << " padding_v.toInt() -- "
                << padding_v.left (padding_v.indexOf ("px")).toInt ();
  ...
  QString dirPath = US_Settings::etcDir ();
  QString p_filename
      = QFileDialog::getSaveFileName (this, tr ("Save Plot's Profile"),
                                      dirPath, tr ("Profile Files (*.json)"));
...
*/
void
US_PlotConfig::updateCurve (void)
{
  qDebug () << "1. In Update Selected Curve: plot's canvas backgound color -- "
            << plot->canvasBackground ();
  ...
  qDebug () << "2. In Update Selected Curve: plot's canvas backgound color -- "
            << plot->canvasBackground ();

  // curveWidget = new US_PlotCurveConfig( plot, selected );
  curveWidget = new US_PlotCurveConfig (
      plot, selected, this); // ALEXEY - call curves' Config widget with
                             // explicitly defining parent (::US_PlotConfig)
...
{
  plotConfigW
      = (US_PlotConfig *)parent; // ALEXEY: access to parent's US_PlotConfig
  qDebug () << "In parent US_PlotConfig widget, plot canvas Color is: "
            << plotConfigW->global_canvas_color;

  qDebug () << "0. In US_PlotCurveConfig() Constructor: (current)plot's "
               "canvas Color -- "
            << currentPlot->canvasBackground ();

  plot = currentPlot;
  // ALEXEY: for some reason, plot's canvas background color is lost here:
  // needs to reset to what has been selected in US_PlotConfig::
#if QT_VERSION > 0x050000
  ...
  qDebug ()
      << "1. In US_PlotCurveConfig() Constructor: plot's canvas Color -- "
      << plot->canvasBackground ();
  ...
  qDebug ()
      << "2. In US_PlotCurveConfig() Constructor: plot's canvas Color -- "
      << plot->canvasBackground ();
  ...
  connect (pb_showCurveColor, SIGNAL (clicked ()), SLOT (selectCurveColor ()));
  ...
  symbolStyles << QwtSymbol::NoSymbol << QwtSymbol::Rect << QwtSymbol::Diamond
               << QwtSymbol::Ellipse << QwtSymbol::DTriangle
               << QwtSymbol::UTriangle << QwtSymbol::LTriangle
               << QwtSymbol::RTriangle << QwtSymbol::Cross << QwtSymbol::XCross
               << QwtSymbol::HLine << QwtSymbol::VLine << QwtSymbol::Star1
...
*/
void
US_PlotCurveConfig::updateSample (int)
{
  lb_sample2->update ();
}

void
US_PlotCurveConfig::curveStyleChanged (int index)
{
...
void
US_PlotCurveConfig::symbolStyleChanged (int index)
{
  int style = cmbb_symbolStyle->itemData (index).toInt ();

  qDebug () << "Curve's SymbolStyle " << cmbb_symbolStyle->itemData (index)
            << ", Type: " << cmbb_symbolStyle->itemData (index).typeName ();
  qDebug () << "Curve's SymbolStyle (int): index, style -- " << index << ", "
            << style;
...
void
US_PlotCurveConfig::selectCurveColor (void)
{
  QPalette p = lb_showCurveColor->palette ();
  QColor current = p.color (QPalette::Active, QPalette::Window);
  QColor c = QColorDialog::getColor (current, this, tr ("Select curve color"));
...
void
US_PlotCurveConfig::selectSymbolInteriorColor (void)
{
...
void
US_PlotCurveConfig::selectSymbolOutlineColor (void)
{
...
US_PlotCurveConfig::apply (void)
{
  ...
  // QwtSymbol* newSymbol = new QwtSymbol( symbolStyle, symbolBrush, //ALEXEY::
  // new Symbol object will be initialized inside the cycle !!!
  ...
  // int j = 0;                                               //ALEXEY: that is
  // NOT true: user can select curves in arbitrary order!!!!

  qDebug ()
      << "Plot Curve Config: just before list() / selectedlist iteration:  ";
  ...
  for (int i = 0; i < selectedItems.size (); i++)
    {
      ...
      // There is no need to reiterate over the full list     //ALEXEY: that is
      // NOT true: user can select curves in arbitrary order!!!! This assumes
      // the selected list and the full list are in the same order

      // for ( /* no intitalizer */; j < list.size(); j++ )    //ALEXEY: that
      // is NOT true: user can select curves in arbitrary order!!!!
      for (int j = 0; j < list.size (); j++)
        {
          if (list[j]->rtti () != QwtPlotItem::Rtti_PlotCurve)
            continue;
          if (list[j]->title () == title)
            {
              ...
              qDebug () << "Plot Curve Config: for allItems -- "
                        << (list[j]->title ()).text ();
              if (curve->symbol () != NULL)
                qDebug () << "Current Symbol -- "
                          << curve->symbol ()->style ();

              QwtSymbol *newSymbol = new QwtSymbol (
                  symbolStyle,
                  symbolBrush, // ALEXEY: defining a new symbol must be witnin
                               // !!! caused crash before
                  symbolPen, symbolSize);
              ...
              if (symbolStyle == QwtSymbol::NoSymbol)
                {
                  qDebug () << "No symbolStyle selected in the first place, "
                               "setting to ::NoSymbol -- ";
                  newSymbol = NULL;
                }
              ...
              if (newSymbol != NULL)
                qDebug () << "newSymbol: pen, brush, size, style -- "
                          << newSymbol->pen () << newSymbol->brush ()
                          << newSymbol->size () << newSymbol->style ();

              curve->setSymbol (
                  newSymbol); // ALEXEY: this caused crash when
                              // plot->detachItems() from all apps (when
                              // ::NoSymbol style OR ...)
              // Also, crashed when multiple curves selected && pressing
              // "Apply" more than one time (even while changing the symbol
              // type...) ATTN: crashed only when applied to the 2nd curve in a
              // selected list! When only 1 curve selected, all good -- was
              // wrong (heap) memory allocation!!
              qDebug ()
                  << "Plot Curve Config: for allItems parms SET Symbol-- "
                  << (list[j]->title ()).text ();

              curve->setPen (curvePen);
              qDebug () << "Plot Curve Config: for allItems parms SET Pen-- "
                        << (list[j]->title ()).text ();

              curve->setStyle (curveStyle);
              qDebug () << "Plot Curve Config: for allItems parms SET Style-- "
                        << (list[j]->title ()).text ();

              qDebug () << "Plot Curve Config: for allItems parms SET ALL -- "
                        << (list[j]->title ()).text ();
              break;
            }
        }
...
US_PlotLabel::US_PlotLabel (US_PlotCurveConfig *caller, QWidget *p,
                            Qt::WindowFlags f)
    : QWidget (p, f)
{
  data = caller;
...
void
US_PlotLabel::paintEvent (QPaintEvent *e)
{
  QRect r = e->rect ();
...
US_PlotAxisConfig::US_PlotAxisConfig (int currentAxis, QwtPlot *currentPlot,
    ...
    : US_WidgetsDialog (parent, flags, false) //( false, parent, flags )
{
  plot = currentPlot;
  axis = currentAxis;
  ...
  switch (axis)
    {
    case QwtPlot::yLeft:
      axisLocation = tr ("Left");
      break;
    case QwtPlot::yRight:
      axisLocation = tr ("Right");
      break;
    case QwtPlot::xTop:
      axisLocation = tr ("Top");
      break;
    case QwtPlot::xBottom:
      axisLocation = tr ("Bottom");
      break;
    }
  ...
  Qt::CheckState checked
      = (plot->axisEnabled (axis)) ? Qt::Checked : Qt::Unchecked;
  ...
  le_axisTitleFont = us_lineedit (
      axisTitleFont.family () + ", "
          + QString::number (axisTitleFont.pointSize ()) + tr (" points"),
      1);
  ...
  le_axisScaleFont = us_lineedit (
      axisScaleFont.family () + ", "
          + QString::number (axisScaleFont.pointSize ()) + tr (" points"),
      1);
  ...
  le_scaleStep = us_lineedit (QString::number (plot->axisStepSize (axis)), 1);
  ...
  checked
      = (attributes & QwtScaleEngine::Floating) ? Qt::Checked : Qt::Unchecked;

  QGridLayout *floating
      = us_checkbox (QString ("Floating Endpoints"), cb_floating, checked);

  checked
      = (attributes & QwtScaleEngine::Inverted) ? Qt::Checked : Qt::Unchecked;
...
*/
void
US_PlotAxisConfig::selectTitleFont (void)
{
  bool ok;
...
void
US_PlotAxisConfig::selectAxisTitleColor (void)
{
  QPalette p = lb_showAxisTitleColor->palette ();
  QColor current = p.color (QPalette::Window);
  QColor c
      = QColorDialog::getColor (current, this, tr ("Select Axis Title Color"));
...
void
US_PlotAxisConfig::selectScaleFont (void)
{
  bool ok;
...
void
US_PlotAxisConfig::selectScaleColor (void)
{
  QPalette p = lb_showScaleColor->palette ();
  QColor current = p.color (QPalette::Window);
  QColor c = QColorDialog::getColor (current, this, tr ("Select Scale Color"));
...
void
US_PlotAxisConfig::selectTickColor (void)
{
  QPalette p = lb_showTickColor->palette ();
  QColor current = p.color (QPalette::Window);
  QColor c = QColorDialog::getColor (current, this, tr ("Select Tick Color"));
...
US_PlotAxisConfig::apply (void)
{
  ...
  if (cb_floating->isChecked ())
    attributes |= (int)QwtScaleEngine::Floating;
  if (cb_inverted->isChecked ())
    attributes |= (int)QwtScaleEngine::Inverted;
  plot->axisScaleEngine (axis)->setAttributes (
      (QwtScaleEngine::Attribute)attributes);
...
*/
US_PlotGridConfig::US_PlotGridConfig (QwtPlot *currentPlot, QWidget *parent,
...
void
US_PlotGridConfig::selectMajorColor (void)
{
...
void
US_PlotGridConfig::selectMinorColor (void)
{
...
US_PlotGridConfig::apply (void)
{
  ...
  int style;
  style
      = cmbb_majorStyle->itemData (cmbb_majorStyle->currentIndex ()).toInt ();
  ...
  style
      = cmbb_minorStyle->itemData (cmbb_minorStyle->currentIndex ()).toInt ();
  pen.setStyle (static_cast<Qt::PenStyle> (style));
...
void
US_PlotPicker::widgetMousePressEvent (QMouseEvent *e)
{
...
void
US_PlotPicker::widgetMouseReleaseEvent (QMouseEvent *e)
{
  static QTime last_click;
...
void
US_PlotPicker::widgetMouseMoveEvent (QMouseEvent *e)
{
  static QTime last_click;
