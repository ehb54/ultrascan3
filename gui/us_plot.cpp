...
US_Zoomer::US_Zoomer(int xAxis, int yAxis, QwtPlotCanvas *canvas) : QwtPlotZoomer(xAxis, yAxis, canvas) {
#if QT_VERSION < 0x050000
...
US_Plot::US_Plot(
   QwtPlot *&parent_plot, const QString &title, const QString &x_axis, const QString &y_axis, const bool cmEnab,
...
#if QT_VERSION > 0x050000
   plot->setStyleSheet(QString("QwtPlot{ padding: %1px }").arg(US_GuiSettings::plotMargin()));
#else
   ...
   if (!cmName.isEmpty()) { // Default Color Map Name is given
      cmfpath = cmName;
      if (!cmfpath.startsWith("cm-")) { // Prepend with "cm-" if missing
         cmfpath = "cm-" + cmfpath;
      }
      if (!cmfpath.endsWith(".xml-")) { // Append with ".xml" if missing
         cmfpath = cmfpath + ".xml";
      }
...
void US_Plot::zoom(bool on) {
   if (on) {
      // Set up for zooming
      zoomer = new US_Zoomer(QwtPlot::xBottom, QwtPlot::yLeft, ( QwtPlotCanvas * ) plot->canvas());
...
#if QT_VERSION < 0x050000
      connect(zoomer, SIGNAL(zoomed(QwtDoubleRect)), SIGNAL(zoomedCorners(QwtDoubleRect)));
      connect(zoomer, SIGNAL(zoomed(QwtDoubleRect)), this, SLOT(scale_yRight(QwtDoubleRect)));
#else
      connect(zoomer, SIGNAL(zoomed(QRectF)), SIGNAL(zoomedCorners(QRectF)));
      connect(zoomer, SIGNAL(zoomed(QRectF)), this, SLOT(scale_yRight(QRectF)));
#endif
      ...
      picker = new QwtPlotPicker(
         QwtPlot::xBottom, QwtPlot::yLeft, QwtPlotPicker::CrossRubberBand, QwtPicker::AlwaysOn, plot->canvas());
...
void US_Plot::csv(void) {
   ...
   QString fileName = QFileDialog::getSaveFileName(plot, tr("Export File Name"), reportDir, tr("CSV Documents (*.csv)"));
...
void US_Plot::svg(void) {
   ...
   QString fileName
      = QFileDialog::getSaveFileName(plot, tr("Export File Name"), reportDir, tr("SVG Documents (*.svgz)"));
...
void US_Plot::png(void) {
   ...
   QString fileName = QFileDialog::getSaveFileName(plot, tr("Export File Name"), reportDir, tr("PNG Documents (*.png)"));
...
void US_Plot::colorMap(void) {
   ...
   cmfpath
      = QFileDialog::getOpenFileName(( QWidget * ) plot, tr("Load Color Map File"), US_Settings::etcDir(), filter, 0, 0);
   ...
   for (int ii = 0; ii < list.size(); ii++) {
      if (list[ ii ]->rtti() == QwtPlotItem::Rtti_PlotCurve) { // This is a curve
         ntcurv++; // Total curves
         QwtPlotCurve *curve = dynamic_cast<QwtPlotCurve *>(list[ ii ]);
         QString ctitle = curve->title().text();
         //qDebug() << "UP:CM:   ii" << ii << "ctitle" << ctitle;
         if (!tmatch || ctitle.contains(cmMatch)) { // No matching or this curve's title matches
...
int US_Plot::map_colors(QList<QColor> &mcolors) {
   ...
   if (cmfpath.isEmpty()) { // No color map file was read:  revert to single color
      mcolors.clear();
      mcolors << US_GuiSettings::plotCurve();
      nmcol = 1;
   }

   else { // Get and return the colors from a map file
      US_ColorGradIO::read_color_gradient(cmfpath, mcolors);
      nmcol = mcolors.count();
   }
...
US_PlotPushbutton::US_PlotPushbutton(const QString &labelString, QWidget *w, int index) :
...
*/
US_PlotConfig::US_PlotConfig(QwtPlot *current_plot, QWidget *p, Qt::WindowFlags f) :
    US_WidgetsDialog(p, f, false) //( false, p, f )
{
   ...
   connect(le_titleText, SIGNAL(textChanged(const QString &)), SLOT(updateTitleText(const QString &)));
   ...
   le_titleFont = us_lineedit(titleFont.family() + ", " + QString::number(titleFont.pointSize()) + tr(" points"), 1);
   ...
   connect(cmbb_legendPos, SIGNAL(activated(int)), SLOT(selectLegendPos(int)));
   ...
   // Row 8
   US_PlotPushbutton *pb_leftAxis = new US_PlotPushbutton(tr("Update Left Axis"), this, QwtPlot::yLeft);

   US_PlotPushbutton *pb_rightAxis = new US_PlotPushbutton(tr("Update Right Axis"), this, QwtPlot::yRight);

   US_PlotPushbutton *pb_topAxis = new US_PlotPushbutton(tr("Update Top Axis"), this, QwtPlot::xTop);

   US_PlotPushbutton *pb_bottomAxis = new US_PlotPushbutton(tr("Update Bottom Axis"), this, QwtPlot::xBottom);

   connect(pb_leftAxis, SIGNAL(US_PlotPbPushed(int)), SLOT(updateAxis(int)));

   connect(pb_rightAxis, SIGNAL(US_PlotPbPushed(int)), SLOT(updateAxis(int)));

   connect(pb_topAxis, SIGNAL(US_PlotPbPushed(int)), SLOT(updateAxis(int)));

   connect(pb_bottomAxis, SIGNAL(US_PlotPbPushed(int)), SLOT(updateAxis(int)));
...
void US_PlotConfig::updateTitleFont(void) {
   ...
   QFont newFont = QFontDialog::getFont(&ok, currentFont, this, tr("Set Title Font"), 0);
   ...
   if (ok) {
      ...
      le_titleFont->setText(newFont.family() + ", " + QString::number(newFont.pointSize()) + tr(" points"));
   }
}
...
void US_PlotConfig::setLegendFontString(void) {
   ...
   if (cmbb_legendPos->currentIndex() == 0) {
      ...
      legendFontString = legendFont.family() + ", " + QString::number(legendFont.pointSize()) + tr(" points");
   }
...
void US_PlotConfig::updateLegendFont(void) {
   if (cmbb_legendPos->currentIndex() == 0) {
      QMessageBox::warning(
         this, tr("Plog Config Warning"), tr("The layout does not exist.  Set the legend position first."));
      return;
   }
#if QT_VERSION > 0x050000
   else {
   ...
   bool ok;
   QFont newFont = QFontDialog::getFont(&ok, font, this, tr("Set Legend Font"), 0);
...
QFont US_PlotConfig::jsonToFont(QJsonObject font_obj) {
   ...
   foreach (const QString &key, font_obj.keys()) {
      ...
      if (key.compare(tr("Family"), Qt::CaseInsensitive) == 0)
         ...
         font.setWeight(static_cast<QFont::Weight>(QMetaEnum::fromType<QFont::Weight>().keyToValue(value.toUtf8())));
      else if (key.compare(tr("Style"), Qt::CaseInsensitive) == 0)
         font.setStyle(static_cast<QFont::Style>(QMetaEnum::fromType<QFont::Style>().keyToValue(value.toUtf8())));
...
QMap<QString, bool> US_PlotConfig::parseGridJson(QJsonObject grid_obj, QPen *pen) {
   ...
   foreach (const QString &key, grid_obj.keys()) {
      ...
      if (key.compare(tr("Style"), Qt::CaseInsensitive) == 0)
         pen->setStyle(static_cast<Qt::PenStyle>(QMetaEnum::fromType<Qt::PenStyle>().keyToValue(value.toUtf8())));
...
void US_PlotConfig::setTitleJson(QJsonObject title_obj) {
   ...
   le_titleFont->setText(font.family() + ", " + QString::number(font.pointSize()) + tr(" points"));
...
void US_PlotConfig::loadPlotProfile(void) {
   ...
   QString p_filename
      = QFileDialog::getOpenFileName(this, tr("Open Plot's Profile"), dirPath, tr("Profile Files (*.json)"));
   ...
   qDebug() << "Loaded Plot Profile -- "
            //	   << "Title: "           << p_title
            << ", Canvas Color: " << canvas_color << ", Frame Color: " << frame_color
   ...
   //2. Border Margin
   plot->setStyleSheet(QString("QwtPlot{ padding: %1px }").arg(border_margin));
...
QJsonObject US_PlotConfig::getAxisJson(int axis_id) {
   ...
   scale_obj.insert("Color", plot->axisWidget(axis_id)->palette().color(QPalette::Text).name());
...
QJsonObject US_PlotConfig::getGridJson() {
   ...
   QPen pen = grid->majorPen();
   QString major_style = QString::fromUtf8(QMetaEnum::fromType<Qt::PenStyle>().valueToKey(pen.style()));
   QString major_width = QString::number(pen.width());
   pen = grid->minorPen();
   QString minor_style = QString::fromUtf8(QMetaEnum::fromType<Qt::PenStyle>().valueToKey(pen.style()));
...
QJsonObject US_PlotConfig::getFontJson(QFont font) {
   ...
   QString font_weight = QString::fromUtf8(QMetaEnum::fromType<QFont::Weight>().valueToKey(font.weight()));
   QString font_style = QString::fromUtf8(QMetaEnum::fromType<QFont::Style>().valueToKey(font.style()));
   QString font_underline = font.underline() ? "true" : "false";
   QString font_strikeout = font.strikeOut() ? "true" : "false";
...
void US_PlotConfig::savePlotProfile(void) {
   ...
   QString dirPath = US_Settings::etcDir();
   QString p_filename
      = QFileDialog::getSaveFileName(this, tr("Save Plot's Profile"), dirPath, tr("Profile Files (*.json)"));
...
void US_PlotConfig::updateCurve(void) {
   ...
   if (lw_curves->selectedItems().count() == 0) {
      QMessageBox::information(this, tr("Attention"), tr("No curves have been selected."));
   ...
   curveWidget = new US_PlotCurveConfig(
      plot, selected, this); //ALEXEY - call curves' Config widget with explicitly defining parent (::US_PlotConfig)
   ...
   curveWidget = NULL;
#ifdef Q_OS_MAC
   QMessageBox::information(this, tr("Curve Config"), tr("Curve Update Complete"));
#endif
   qApp->processEvents();
}
...
void US_PlotConfig::updateGrid(void) {
   ...
   gridWidget = NULL;
#ifdef Q_OS_MAC
   QMessageBox::information(this, tr("Grid Config"), tr("Grid Update Complete"));
   qApp->processEvents();
#endif
}
/*
...
*/
US_PlotCurveConfig::US_PlotCurveConfig(
   QwtPlot *currentPlot, const QStringList &selected, QWidget *parent, Qt::WindowFlags f) :
    US_WidgetsDialog(parent, f, false) //( false, parent, f )
{
   ...
   qDebug() << "0. In US_PlotCurveConfig() Constructor: (current)plot's canvas Color -- "
   ...
   for (int i = 0; i < list.size(); i++) {
      if (list[ i ]->rtti() == QwtPlotItem::Rtti_PlotCurve && list[ i ]->title().text() == firstSelectedText) {
   ...
   curveStyles << QwtPlotCurve::NoCurve << QwtPlotCurve::Lines << QwtPlotCurve::Sticks << QwtPlotCurve::Steps
               << QwtPlotCurve::Dots;
   ...
   connect(cmbb_curveStyle, SIGNAL(currentIndexChanged(int)), SLOT(curveStyleChanged(int)));
   ...
   connect(sb_curveWidth, SIGNAL(valueChanged(int)), SLOT(updateSample(int)));
   ...
   symbolStyles << QwtSymbol::NoSymbol << QwtSymbol::Rect << QwtSymbol::Diamond << QwtSymbol::Ellipse
   ...
   connect(cmbb_symbolStyle, SIGNAL(currentIndexChanged(int)), SLOT(symbolStyleChanged(int)));
   ...
   connect(sb_symbolWidth, SIGNAL(valueChanged(int)), SLOT(updateSample(int)));
   ...
   connect(pb_symOutlineColor, SIGNAL(clicked()), SLOT(selectSymbolOutlineColor()));
   ...
   connect(pb_symInteriorColor, SIGNAL(clicked()), SLOT(selectSymbolInteriorColor()));
...
void US_PlotCurveConfig::selectSymbolInteriorColor(void) {
   ...
   QColor current = p.color(QPalette::Active, QPalette::Window);
   QColor c = QColorDialog::getColor(current, this, tr("Select symbol interior color"));
...
void US_PlotCurveConfig::selectSymbolOutlineColor(void) {
   ...
   QColor current = p.color(QPalette::Active, QPalette::Window);
   QColor c = QColorDialog::getColor(current, this, tr("Select symbol outline color"));
      ...
      for (int j = 0; j < list.size(); j++) {
         ...
         if (list[ j ]->title() == title) {
            ...
            QwtSymbol *newSymbol = new QwtSymbol(
               symbolStyle, symbolBrush, //ALEXEY: defining a new symbol must be witnin !!! caused crash before
               symbolPen, symbolSize);
            ...
            if (newSymbol != NULL)
               qDebug() << "newSymbol: pen, brush, size, style -- " << newSymbol->pen() << newSymbol->brush()
                        << newSymbol->size() << newSymbol->style();

            curve->setSymbol(
               newSymbol); //ALEXEY: this caused crash when plot->detachItems() from all apps (when ::NoSymbol style OR ...)
...
US_PlotLabel::US_PlotLabel(US_PlotCurveConfig *caller, QWidget *p, Qt::WindowFlags f) : QWidget(p, f) {
   data = caller;
   label = new QLabel;
}
...
void US_PlotLabel::paintEvent(QPaintEvent *e) {
   ...
   if (data->symbolStyle != QwtSymbol::NoSymbol) {
      QSize size(data->sb_symbolWidth->value(), data->sb_symbolHeight->value());
...
*/
US_PlotAxisConfig::US_PlotAxisConfig(int currentAxis, QwtPlot *currentPlot, QWidget *parent, Qt::WindowFlags flags) :
    US_WidgetsDialog(parent, flags, false) //( false, parent, flags )
{
   ...
   Qt::CheckState checked = (plot->axisEnabled(axis)) ? Qt::Checked : Qt::Unchecked;
   ...
   le_axisTitleFont
      = us_lineedit(axisTitleFont.family() + ", " + QString::number(axisTitleFont.pointSize()) + tr(" points"), 1);
   ...
   connect(pb_axisTitleColor, SIGNAL(clicked()), SLOT(selectAxisTitleColor()));
   ...
   le_axisScaleFont
      = us_lineedit(axisScaleFont.family() + ", " + QString::number(axisScaleFont.pointSize()) + tr(" points"), 1);
...
#if QT_VERSION > 0x050000
   le_scaleFrom = us_lineedit(QString::number(plot->axisScaleDiv(axis).lowerBound()), 1);

   le_scaleTo = us_lineedit(QString::number(plot->axisScaleDiv(axis).upperBound()), 1);
#else
   le_scaleFrom = us_lineedit(QString::number(plot->axisScaleDiv(axis)->lowerBound()), 1);
...
#if QT_VERSION > 0x050000
#else
   if (plot->axisScaleEngine(axis)->transformation()->type() == QwtScaleTransformation::Log10)
   ...
   le_reference = us_lineedit(QString::number(plot->axisScaleEngine(axis)->reference()), 1);
   ...
   checked = (attributes & QwtScaleEngine::Floating) ? Qt::Checked : Qt::Unchecked;

   QGridLayout *floating = us_checkbox(QString("Floating Endpoints"), cb_floating, checked);

   checked = (attributes & QwtScaleEngine::Inverted) ? Qt::Checked : Qt::Unchecked;

   QGridLayout *inverted = us_checkbox(QString("Inverted Scale"), cb_inverted, checked);

   checked = (plot->axisAutoScale(axis)) ? Qt::Checked : Qt::Unchecked;
   QGridLayout *autoscale = us_checkbox(QString("Autoscale"), cb_autoscale, checked);
...
void US_PlotAxisConfig::selectTitleFont(void) {
   ...
   QFont newFont = QFontDialog::getFont(&ok, currentFont, this, tr("Set Axis Title Font"), 0);

   if (ok) {
      axisTitleFont = newFont;
      le_axisTitleFont->setText(newFont.family() + ", " + QString::number(newFont.pointSize()) + tr(" points"));
   }
}
...
void US_PlotAxisConfig::selectAxisTitleColor(void) {
   ...
   QColor c = QColorDialog::getColor(current, this, tr("Select Axis Title Color"));
...
void US_PlotAxisConfig::selectScaleFont(void) {
   ...
   QFont newFont = QFontDialog::getFont(&ok, currentFont, this, tr("Set Axis Scale Font"), 0);

   if (ok) {
      axisScaleFont = newFont;
      le_axisScaleFont->setText(newFont.family() + ", " + QString::number(newFont.pointSize()) + tr(" points"));
   }
}
...
void US_PlotAxisConfig::apply(void) {
   ...
   if (
      (plotType == QwtScaleTransformation::Log10 && rb_linear->isChecked())
...
*/
US_PlotGridConfig::US_PlotGridConfig(QwtPlot *currentPlot, QWidget *parent, Qt::WindowFlags flags) :
    US_WidgetsDialog(parent, flags, false) //( false, parent, flags )
{
   ...
   QStringList styles;
   styles << tr("Solid") << tr("Dash") << tr("Dot") << tr("DashDot") << tr("DashDotDot");

   QList<int> penStyle;
   penStyle << Qt::SolidLine << Qt::DashLine << Qt::DotLine << Qt::DashDotLine << Qt::DashDotDotLine;
   ...
   QGridLayout *enableXminor = us_checkbox(tr("Vertical"), cb_enableXminor, checked);

   checked = (grid->yMinEnabled()) ? Qt::Checked : Qt::Unchecked;
   QGridLayout *enableYminor = us_checkbox(tr("Horizontal"), cb_enableYminor, checked);
...
void US_PlotGridConfig::selectMajorColor(void) {
   ...
   QColor current = p.color(QPalette::Window);
   QColor c = QColorDialog::getColor(current, this, tr("Select major grid line color"));
   if (c.isValid()) {
      p.setColor(QPalette::Window, c);
      lb_showMajorColor->setPalette(p);
   }
...
void US_PlotGridConfig::selectMinorColor(void) {
   ...
   QColor current = p.color(QPalette::Window);
   QColor c = QColorDialog::getColor(current, this, tr("Select minor grid line color"));
   if (c.isValid()) {
      p.setColor(QPalette::Window, c);
      lb_showMinorColor->setPalette(p);
   }
...
US_PlotPicker::US_PlotPicker(QwtPlot *plot) : QwtPlotPicker(QwtPlot::xBottom, QwtPlot::yLeft, plot->canvas()) {
#if QT_VERSION < 0x050000
