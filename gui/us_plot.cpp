...
US_Plot::US_Plot(QwtPlot*& parent_plot, const QString& title,
                 ...
                 const bool cmEnab, const QString cmMatch, const QString cmName)
    : QHBoxLayout() {
  zoomer = NULL;
  setSpacing(0);
...
#if QT_VERSION > 0x050000
  plot->setStyleSheet(
      QString("QwtPlot{ padding: %1px }").arg(US_GuiSettings::plotMargin()));
#else
  ...
  if (plot->legend() != NULL) plot->legend()->setFont(font);
...
void US_Plot::colorMap(void) {
  ...
  cmfpath =
      QFileDialog::getOpenFileName((QWidget*)plot, tr("Load Color Map File"),
...
int US_Plot::map_colors(QList<QColor>& mcolors) {
  ...
  if (cmfpath
          .isEmpty()) {  // No color map file was read:  revert to single color
    mcolors.clear();
    mcolors << US_GuiSettings::plotCurve();
    nmcol = 1;
  }
    ...
    : US_WidgetsDialog(p, f, false)  //( false, p, f )
{
  ...
  le_titleFont =
      us_lineedit(titleFont.family() + ", " +
                      QString::number(titleFont.pointSize()) + tr(" points"),
                  1);
...
void US_PlotConfig::updateTitleFont(void) {
  ...
  QFont newFont =
      QFontDialog::getFont(&ok, currentFont, this, tr("Set Title Font"), 0);
...
void US_PlotConfig::updateLegendFont(void) {
  if (cmbb_legendPos->currentIndex() == 0) {
    ...
    QMessageBox::warning(this, tr("Legend Font Unchanged"),
...
QFont US_PlotConfig::jsonToFont(QJsonObject font_obj) {
  ...
  foreach (const QString& key, font_obj.keys()) {
    ...
    if (key.compare(tr("Family"), Qt::CaseInsensitive) == 0)
      ...
      font.setWeight(static_cast<QFont::Weight>(
          QMetaEnum::fromType<QFont::Weight>().keyToValue(value.toUtf8())));
    else if (key.compare(tr("Style"), Qt::CaseInsensitive) == 0)
      font.setStyle(static_cast<QFont::Style>(
          QMetaEnum::fromType<QFont::Style>().keyToValue(value.toUtf8())));
  ...
  qDebug() << "Weight: "
           << QString::fromUtf8(QMetaEnum::fromType<QFont::Weight>().valueToKey(
                  font.weight()));
  qDebug() << "Style: "
           << QString::fromUtf8(
...
QMap<QString, bool> US_PlotConfig::parseGridJson(QJsonObject grid_obj,
                                                 QPen* pen) {
  ...
  foreach (const QString& key, grid_obj.keys()) {
    ...
    if (key.compare(tr("Style"), Qt::CaseInsensitive) == 0)
      pen->setStyle(static_cast<Qt::PenStyle>(
          QMetaEnum::fromType<Qt::PenStyle>().keyToValue(value.toUtf8())));
    ...
    if (key.compare(tr("Enable"), Qt::CaseInsensitive) == 0) {
      ...
      foreach (const QString& kk, json_obj.keys()) {
        ...
        if (kk.compare(tr("Autoscale"), Qt::CaseInsensitive) == 0) {
          ...
          continue;
        } else if (kk.compare(tr("Floating Endpoints"), Qt::CaseInsensitive) ==
                   0) {
          if (value.toString().compare("true", Qt::CaseInsensitive) == 0)
            attributes |= (int)QwtScaleEngine::Floating;
        } else if (kk.compare(tr("Include Reference"), Qt::CaseInsensitive) ==
                   0) {
          if (value.toString().compare("true", Qt::CaseInsensitive) == 0)
...
void US_PlotConfig::loadPlotProfile(void) {
  ...
  QString p_filename = QFileDialog::getOpenFileName(
      this, tr("Open Plot's Profile"), dirPath, tr("Profile Files (*.json)"));
  ...
  // 1. Title
  if (!title_obj.isEmpty()) setTitleJson(title_obj);
  ...
  // 6. Grid
  if (!grid_obj.isEmpty()) setGridJson(grid_obj);
...
void US_PlotConfig::savePlotProfile(void) {
  ...
  // save to local file
  QString dirPath = US_Settings::etcDir();
  QString p_filename = QFileDialog::getSaveFileName(
      this, tr("Save Plot's Profile"), dirPath, tr("Profile Files (*.json)"));
...
void US_PlotConfig::updateCurve(void) {
  ...
  for (int i = 0; i < items.count(); i++) selected << items[i]->text();
    ...
    : US_WidgetsDialog(parent, f, false)  //( false, parent, f )
{
  ...
  qDebug() << "0. In US_PlotCurveConfig() Constructor: (current)plot's canvas "
              "Color -- "
           << currentPlot->canvasBackground();

  plot = currentPlot;
  // ALEXEY: for some reason, plot's canvas background color is lost here: needs
  // to reset to what has been selected in US_PlotConfig::
#if QT_VERSION > 0x050000
  ...
  qDebug() << "1. In US_PlotCurveConfig() Constructor: plot's canvas Color -- "
           << plot->canvasBackground();
  ...
  qDebug() << "2. In US_PlotCurveConfig() Constructor: plot's canvas Color -- "
           << plot->canvasBackground();
...
*/
void US_PlotCurveConfig::updateSample(int) { lb_sample2->update(); }
...
void US_PlotCurveConfig::selectSymbolInteriorColor(void) {
  ...
  QColor current = p.color(QPalette::Active, QPalette::Window);
  QColor c =
      QColorDialog::getColor(current, this, tr("Select symbol interior color"));
...
void US_PlotCurveConfig::selectSymbolOutlineColor(void) {
  ...
  QColor current = p.color(QPalette::Active, QPalette::Window);
  QColor c =
      QColorDialog::getColor(current, this, tr("Select symbol outline color"));
...
void US_PlotCurveConfig::apply(void) {
  ...
  for (int i = 0; i < selectedItems.size(); i++) {
    ...
    // NOT true: user can select curves in arbitrary order!!!! This assumes the
    // selected list and the full list are in the same order

    // for ( /* no intitalizer */; j < list.size(); j++ )    //ALEXEY: that is
    // NOT true: user can select curves in arbitrary order!!!!
    for (int j = 0; j < list.size(); j++) {
      ...
      if (list[j]->title() == title) {
        ...
        if (selectedItems.size() == 1) curve->setTitle(le_curveTitle->text());
        ...
        QwtSymbol* newSymbol = new QwtSymbol(
            symbolStyle, symbolBrush,  // ALEXEY: defining a new symbol must be
                                       // witnin !!! caused crash before
            symbolPen, symbolSize);

        // if ( !symbolStyle || symbolStyle ==  QwtSymbol::NoSymbol )
        if (symbolStyle == QwtSymbol::NoSymbol) {
          qDebug() << "No symbolStyle selected in the first place, setting to "
                      "::NoSymbol -- ";
          newSymbol = NULL;
        }
        ...
        curve->setSymbol(
            newSymbol);  // ALEXEY: this caused crash when plot->detachItems()
                         // from all apps (when ::NoSymbol style OR ...)
        // Also, crashed when multiple curves selected && pressing "Apply" more
        // than one time (even while changing the symbol type...) ATTN: crashed
        // only when applied to the 2nd curve in a selected list! When only 1
        // curve selected, all good -- was wrong (heap) memory allocation!!
        qDebug() << "Plot Curve Config: for allItems parms SET Symbol-- "
                 << (list[j]->title()).text();
...
void US_PlotLabel::paintEvent(QPaintEvent* e) {
  ...
  switch (data->curveStyle) {
    ...
    case QwtPlotCurve::Sticks:
      ...
      for (int i = p1.x(); i < p5.x(); i += 8) p.drawLine(i, y1, i, y2);
      break;
    ...
    case QwtPlotCurve::Dots:
      for (int i = p1.x(); i < p5.x(); i += 6) p.drawPoint(i, p1.y());
      break;
    ...
    : US_WidgetsDialog(parent, flags, false)  //( false, parent, flags )
{
...
#if QT_VERSION > 0x050000
  le_scaleFrom =
      us_lineedit(QString::number(plot->axisScaleDiv(axis).lowerBound()), 1);

  le_scaleTo =
      us_lineedit(QString::number(plot->axisScaleDiv(axis).upperBound()), 1);
#else
  le_scaleFrom =
      us_lineedit(QString::number(plot->axisScaleDiv(axis)->lowerBound()), 1);
  ...
  le_reference =
      us_lineedit(QString::number(plot->axisScaleEngine(axis)->reference()), 1);
...
void US_PlotAxisConfig::apply(void) {
  ...
  if (!cb_autoscale->isChecked()) plot->setAxisScale(axis, from, to, step);
    ...
    : US_WidgetsDialog(parent, flags, false)  //( false, parent, flags )
{
  ...
  for (int i = 0; i < styles.size(); i++) {
    ...
    if (currentStyle == penStyle[i]) cmbb_majorStyle->setCurrentIndex(i);
  }
  ...
  for (int i = 0; i < styles.size(); i++) {
    ...
    if (currentStyle == penStyle[i]) cmbb_minorStyle->setCurrentIndex(i);
  }
...
void US_PlotGridConfig::selectMajorColor(void) {
  ...
  QColor current = p.color(QPalette::Window);
  QColor c =
      QColorDialog::getColor(current, this, tr("Select major grid line color"));
  if (c.isValid()) {
    p.setColor(QPalette::Window, c);
    lb_showMajorColor->setPalette(p);
  }
...
void US_PlotGridConfig::selectMinorColor(void) {
  ...
  QColor current = p.color(QPalette::Window);
  QColor c =
      QColorDialog::getColor(current, this, tr("Select minor grid line color"));
  if (c.isValid()) {
    p.setColor(QPalette::Window, c);
    lb_showMinorColor->setPalette(p);
  }
...
void US_PlotPicker::widgetMousePressEvent(QMouseEvent* e) {
  ...
  if (last_click.isNull() || last_click.elapsed() > 300) {
    last_click.start();
    if (e->button() == Qt::LeftButton) emit mouseDown(invTransform(e->pos()));
...
void US_PlotPicker::widgetMouseReleaseEvent(QMouseEvent* e) {
  ...
  if (last_click.isNull() || last_click.elapsed() > 300) {
    last_click.start();
    if (e->button() == Qt::LeftButton) emit mouseUp(invTransform(e->pos()));
...
void US_PlotPicker::widgetMouseMoveEvent(QMouseEvent* e) {
  ...
  if (last_click.isNull() || last_click.elapsed() > 100) {
    last_click.start();
    if (e->button() == Qt::LeftButton) emit mouseDrag(invTransform(e->pos()));
