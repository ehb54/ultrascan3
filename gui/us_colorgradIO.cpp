...
int US_ColorGradIO::read_color_gradient(QString xmlfilename,
  ...
  if (rc == XFS_OK) {
    ...
    for (int jj = 0; jj < nvals;
         jj += 4) {  // read each quadruple as obtained from the XML
      ered = cparams.at(jj);
      ...
      if (npts > 0) {  // a step entry:  expand all color points
        ...
        for (int ii = 0; ii < npts;
             ii++) {     // output colors from interpolated RGB
...
int US_ColorGradIO::read_color_step_params(QString xmlfilename,
  ...
  if (filei.open(QIODevice::ReadOnly)) {  // able to open file
    ...
    while (!xmli.atEnd()) {  // read each element of the XML
      ...
      if (xmli.isComment()) {  // comment line:  verify that we have color steps
                               // type
        QString comm = xmli.text().toString();

        if (comm.contains(
                "UltraScanColorSteps")) {  // mark that we have the right type
          is_uscs = true;
        }
      }

      if (xmli.isStartElement() &&
          xmli.name() == "color") {  // start of "color" entry
