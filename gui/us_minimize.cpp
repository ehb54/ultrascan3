...
US_Minimize::US_Minimize (bool *temp_fitting_widget, bool temp_GUI)
    : US_Widgets ()
{
  GUI = temp_GUI;
...
US_Minimize::setup_GUI ()
{
  ...
  QGridLayout *monitor_layout
      = us_checkbox (tr ("Monitor Fit Graphically"), ck_monitor, false);
  bg1 = new QButtonGroup ();
  QGridLayout *plot1_layout
      = us_radiobutton (tr ("Plot All Data"), bt_plotAll);
  QGridLayout *plot2_layout
      = us_radiobutton (tr ("Plot Groups of 5"), bt_plotGroup);
  QGridLayout *plot3_layout
      = us_radiobutton (tr ("Plot Single Scans"), bt_plotSingle);
...
void
US_Minimize::update_lambdaStep (const QString &str)
{
  lambdaStep = str.toFloat ();
}
void
US_Minimize::update_lambdaStart (const QString &str)
{
  lambdaStart = str.toFloat ();
}
void
US_Minimize::update_maxIterations (const QString &str)
{
  maxIterations = str.toInt ();
}
void
US_Minimize::update_tolerance (const QString &str)
{
  tolerance = str.toFloat ();
}
void
US_Minimize::updateQN (float **gamma, float **delta)
{
  unsigned int i, j;
  float *hgamma, lambda, deltagamma, *v, *temp, **vvtranspose, **ddtranspose,
      **hgammatranspose;
  ...
  for (i = 0; i < parameters; i++)
    {
      for (j = 0; j < parameters; j++)
        {
          information_matrix[i][j] = information_matrix[i][j]
                                     - (double)hgammatranspose[i][j] / lambda
                                     + (double)ddtranspose[i][j] / deltagamma
                                     + (double)vvtranspose[i][j] * lambda;
        }
    }
...
US_Minimize::Fit ()
{
  // qDebug() << "converged: " << converged << ", completed: " << completed <<
  // ", aborted: " << aborted  ;
  float lambda = lambdaStart; // re-initialize Lambda
  float *search, alpha, *gamma, *delta;
  int step_counter = 0; // count how many times we try to shorten the step size
      ...
      if (GUI)
        {
          ...
          message.setText (tr (
              "No scans have been selected\nfor fitting or all scans "
              "have\nbeen excluded.\n\nPlease review the Scan "
  ...
  if (calc_model (guess) < 0)
    {
      if (GUI)
        {
          ...
          pb_close->setText (tr ("Close"));
          pb_fit->setEnabled (true);
          // lbl_status2->setText(tr("The fitting process didn't
          // converge..."));
      ...
      qDebug () << "The residuals from the initial guess are too large.\n"
                << "Please manually adjust the fitting parameters and retry "
                   "the fit.\n"
  ...
  qDebug () << "iter: " << iteration << ", maxiter: " << maxIterations
            << ", new_resid: " << new_residuals / points
            << ", tolerance: " << tolerance;
  ...
  while ((iteration < maxIterations) && (new_residuals / points > tolerance))
    {
      ...
      if ((nlsMethod < 3) || (nlsMethod == 3 && iteration == 1))
        {
          ...
          matrix->calc_A_transpose_A (
              &jacobian, &information_matrix, points, parameters,
              settings->threads ()); // ATTENTION /////////

          // matrix->calc_A_transpose_A(&jacobian, &information_matrix, points,
          // parameters, 20);

          // matrix_temp.calc_A_transpose_A(&jacobian, &information_matrix,
          // points, parameters, 15);
      ...
      if (nlsMethod == 3) // Quasi-Newton
        {
          ...
          */
          matrix->mmv (&search, &B, &information_matrix, parameters,
                       parameters);
          /*
            for (unsigned int i=0; i<parameters; i++)
            {
          ...
          */
          for (unsigned int i = 0; i < parameters; i++)
            {
              gamma[i] = (float)
                  B[i]; // B = J'y_delta = -gradient; in updateQN we need
          ...
          // qDebug() << "returning from calc_residuals in CP2 with " <<
          // new_residuals  ;
          calc_B ();
          for (unsigned int i = 0; i < parameters; i++)
            {
              ...
              // qDebug() << "Paramter " << i << ": gamma:" << gamma[i] << ",
              // delta: " << delta[i] << ", alpha: " << alpha << ", search: "
              // << search[i] << ", B: " << B[i]  ;
            }
      ...
      if (nlsMethod == 0 || nlsMethod == 2)
        {
          // Add lambda to the information matrix to make diagonal large to
          // make columns linearly independent:
          for (unsigned int i = 0; i < parameters; i++)
            {
      ...
      while (new_residuals >= old_residuals && nlsMethod != 3)
        {
          ...
          //   Problem: J * R = y_delta, where R = (a[i] - guess[i]) and we
          //   want to find a[i], so solve for R:
          // (1)   J'J * R = J' * y_delta
          ...
          // Alternative Solution: This can be solved using first forward and
          // then backward substitution, and Cholesky Decomposition: (1)   J'J
          // * R = J'*y_delta
          ...
          // save information matrix in LL_transpose in case chi-2 is larger,
          // in which case we would need to reset Lambda
          ...
          if (GUI)
            {
              // lbl_status2->setText(tr("Processing Cholesky
              // Decomposition..."));
                      ...
                      if (showGuiFit && !us_auto_mode)
                        {
                          ...
                          message.setText (tr (
                              ...
                              "matrix failed due to a\nsingularity in the "
                              "matrix.\n\nYou may achieve convergence "
          ...
          // Now the information matrix actually contains the decomposed
          // information matrix LL' (4)   L(L'*R) = B (5)    Substitute:  Z =
          // L'*R (6)    Solve for Z using forward substitution: L*Z = B (7)
          // Solve for R using backward substitution: L'*R = Z
          ...
          // 	 qDebug().noquote() << fit_timers.list_times();
          // Now B is changed to R (the difference between the current
          // parameter estimate with the "true" parameter)
          ...
          if (new_residuals < old_residuals)
                ...
                case 0:
                  {
                    ...
                    // qDebug() << "lambda = " << lambda << ", lambdaStep = "
                    // << lambdaStep  ;
                    break;
                  }
                case 1:
                    ...
                    if (lambda < 1.0)
                      {
                        ...
                        lambda = 0.0;
                        step *= 2.0;
                        step_counter = 0; // reset step counter to zero,
                                          // because we lengthened the step.
                        dostep = true;
                      }
                    break;
                  }
          ...
          else
            {
              ...
              if (GUI)
                {
                  str.sprintf ("%3.5e",
                               (old_residuals - new_residuals) / points);
                  le_improvement->setText (str);
                  qApp->processEvents ();
                }
              switch (nlsMethod)
      ...
      // qDebug() << "new_residuals = " << new_residuals << ", points = " <<
      // points << ", tolerance = " << tolerance  ; qDebug() << "GUI: " << GUI
      // ;
    }
  // qDebug() << "END ITER: "  ;
  return (0);
...
float
US_Minimize::linesearch (float **search, float f0)
{
  bool check_flag = true;
  double old_f0 = 0.0, old_f1 = 0.0, old_f2 = 0.0;
  // qDebug() << "before: " << errno  ;
  errno = 0; // clear old error state
  // look for the minimum residual. Residual values are f0, f1, f2, evaluated
  // at x0, x1, x2. x0, x1, x2 are multipliers for incremental change of the
  // parameter vector 'search' `search` contains the vector of parameters we
  // need to optimize calculate bracket: Assume the minimum is between x0=0 and
  // some stepsize x2 away from x0. then find an x1 in the middle between x0
  // and x2 and calculate f1(x1) and f2(x2), where shift to the right f is the
  ...
  if (f2 < 0)
    return (0.0);
  // qDebug() << "-2: " << errno << ", f0: " << f0 << ", f1: " << f1 << ", f2:
  // " << f2  ;

  while (errno != 0
         || (f0 >= 10000 || f0 < 0 || f1 >= 10000 || f1 < 0 || f2 >= 10000
             || f2 < 0)) // make the initial step size smaller if we have
                         // infinite residuals
    {
      x1 /= 10;
      x2 /= 10;
      ...
      if (x1 < FLT_MIN)
        {
          // qDebug() << "abandoned with: " << "x0: " << x0 << ", x1: " << x1
          // << ", x2: " << x2 << ", f0: " << f0 << ", f1: " << f1 << ", f2: "
          // << f2  ; qDebug() << FLT_MIN << ", " << FLT_MAX << ", errno: " <<
          // errno  ;
          errno = 0;
          return (-1); // couldn't do anything for this search direction - fit
                       // didn't converge
        }
    }
  check_flag = true;
  while (check_flag)
    {
      // qDebug() << "mathlib: "  << errno << ", x0: " << x0 << ", x1: " << x1
      // << ", x2: " << x2 << ", f0: " << f0 << ", f1: " << f1 << ", f2: " <<
      // f2 << ", d1: " << f1-f0 << ", d2: " << f2-f1 << ", d3: " << f2-f0  ;
      if ((qIsNaN (f0) && qIsNaN (f1)) || (qIsNaN (f1) && qIsNaN (f2))
          || (qIsNaN (f0)
              || qIsNaN (f2))) // at least two values are screwed up, exit.
        {
          // qDebug() << "error1 " << "x0: " << x0 << ", x1: " << x1 << ", x2:
          // " << x2 << ", f0: " << f0 << ", f1: " << f1 << ", f2: " << f2  ;
          errno = 0;
          return (-1);
        }
      // Check to see if the solution is really converging:

      if ((fabs (f2 - old_f2) < FLT_MIN) && (fabs (f1 - old_f1) < FLT_MIN)
          && (fabs (f0 - old_f0) < FLT_MIN)) // is the solution horizontal?
        {
      ...
      if (((fabs (f2 - f0) < FLT_MIN) && (fabs (f1 - f0) < FLT_MIN))
          || ((f0 > f1)
              && (fabs (f2 - f1) < FLT_MIN))) // is the solution horizontal?
        {
          // qDebug() << "error2 "  << "x0: " << x0 << ", x1: " << x1 << ", x2:
          // " << x2 << ", f0: " << f0 << ", f1: " << f1 << ", f2: " << f2  ;
          return (0); // we found the minimum, return alpha=0
        }
      if ((fabs (x0) < FLT_MIN) && (fabs (x1) < FLT_MIN)
          && (fabs (x2) < FLT_MIN)) // is the solution horizontal?
        {
          // qDebug() << "error3 "  << "x0: " << x0 << ", x1: " << x1 << ", x2:
          // " << x2 << ", f0: " << f0 << ", f1: " << f1 << ", f2: " << f2  ;
          return (0); // we found the minimum, return alpha=0
        }
      //      if ((f0 < f1) && (f1 == f2) || (f0 < f1) && (f1 > f2) || (fabs(f1
      //      - f0) < FLT_MIN) && f2 > f1)   // some weird cases can happen
      //      near the minimum
      if (((fabs (f0 - f1) < FLT_MIN) && (fabs (f1 - f2) < FLT_MIN))
          || ((fabs (f0 - f1) < FLT_MIN)
              && (f2 > f1))) // some weird cases can happen near the minimum
        {
          // qDebug() << "error4 "  << "x0: " << x0 << ", x1: " << x1 << ", x2:
          // " << x2 << ", f0: " << f0 << ", f1: " << f1 << ", f2: " << f2  ;
          return (0); // we found the minimum, return alpha=0
        }
      if (f0 > f1 && f2 > f1) // we have a bracket
        {
          // qDebug() << "bracket " << "x0: " << x0 << ", x1: " << x1 << ", x2:
          // " << x2 << ", f0: " << f0 << ", f1: " << f1 << ", f2: " << f2  ;
          check_flag = false;
          break;
        }
      else if ((f2 > f1 && f1 > f0) || (f1 > f0 && f1 > f2)
               || (f1 == f2 && f1 > f0)) // shift to the left
        {
          // qDebug() << "leftshift " << "x0: " << x0 << ", x1: " << x1 << ",
          // x2: " << x2 << ", f0: " << f0 << ", f1: " << f1 << ", f2: " << f2
          // ;
          x2 = x1;
          f2 = f1;
          x1 = (x2 + x0) / 2.0;
      ...
      else if (f0 > f1 && f1 > f2) // shift to the right
        {
          ...
          if (f2 < 0)
            return (0.0);
          // qDebug() << "rightshift #2" << "x0: " << x0 << ", x1: " << x1 <<
          // ", x2: " << x2 << ", f0: " << f0 << ", f1: " << f1 << ", f2: " <<
          // f2  ;
        }
      iter++;
  ...
  // search inside the bracket for the minimum and do a 2nd order polynomial
  // fit
  x1 = (x0 + x2) / 2.0;
  h = x1 - x0;
  ...
  while (true)
    {
      ...
      // qDebug() << "xmin: " << xmin << ", diff: " << (2 * (f0 - 2 * f1 + f2))
      // << ", h: " << h << ", f0: " << f0 << ", f1: " << f1 << ", f2: " << f2
...
float
US_Minimize::calc_testParameter (float **search, float step)
{
  float res;
  for (unsigned int i = 0; i < parameters; i++)
    {
      test_guess[i] = guess[i] + (double)step * (*search)[i];
      // qDebug() << "Step: " << step << ", Test-guess(" << i << "): " <<
      // test_guess[i] << ", guess: " << guess[i] << ", search: " <<
      // (*search)[i]  ;
    }
  /*
    QString s="", format;
    ...
    test_guess[i], i, (*search)[i]);
    }
    QMessageBox message;
               message.setWindowTitle(tr("Debug:"));
               message.setText(tr("The following values are passed to
    calc_model:\n" + s + "")); message.exec();
  */
  if (calc_model (test_guess) < 0)
    {
      qDebug () << "Attention: there was an error in the model calculation, "
...
void
US_Minimize::plot_overlays ()
{
  plotResiduals = false;
}
void
US_Minimize::plot_residuals ()
{
  plotResiduals = true;
}
float
US_Minimize::calc_residuals ()
{
  float residual = 0.0;
  errno = 0;
  for (unsigned int i = 0; i < points; i++)
    {
      ...
      if (errno != 0)
        {
          qDebug ()
              << "Floating point exception in the residuals calculation!\n";
          if (GUI)
            {
              QMessageBox message;
              message.setWindowTitle (tr ("UltraScan Error:"));
...
void
US_Minimize::closeEvent (QCloseEvent *e)
{
  emit fittingWidgetClosed ();
  e->accept ();
}
bool
US_Minimize::fit_init ()
{
  emit fitStarted ();
  return (false);
}
...
int
US_Minimize::calc_model (double *)
{
  return (-1);
}
void
US_Minimize::save_Fit ()
{
  QMessageBox mBox;
  ...
  mBox.setText (
      tr ("Please use the save function from the main extinction calculation "
          "module. It will allow you to save the coefficients of the fit as "
          "well as a copy of the plot window"));
  mBox.setIcon (QMessageBox::Information);
  mBox.exec ();
}
// if a fit was loaded, update all the dialogs and plot windows:
void
US_Minimize::update_fitDialog ()
{
  if (!GUI)
    {
      return;
  ...
  str.sprintf (" %ld", points);
  le_points->setText (str);
  str.sprintf ((tr ("%ld Runs (%2.2f")).toLatin1 ().data (), runs,
               runs_percent);
  str += " %)";
...
void
US_Minimize::update_nlsMethod (int item)
{
  nlsMethod = item;
  switch (item)
    {
    case 0:
      {
        ...
        if (GUI)
          {
            // lbl_status4->setText(tr("Levenberg-Marquardt Method
            // selected..."));
    ...
    case 1:
      {
        ...
        if (GUI)
          {
            // lbl_status4->setText(tr("Modified Gauss Newton Method
            // selected..."));
            le_lambdaStart->setText (" 20.0");
            le_lambdaStep->setText (" 1.05");
          }
    ...
    case 4:
      {
        ...
        if (GUI)
          {
            // lbl_status4->setText(tr("Generalized Linear Least Squares Method
            // selected..."));
      ...
      for (unsigned int j = 0; j < points; j++)
        {
          ...
          //         qDebug() << "jac: " << jacobian[j][i] << ", y_delta: " <<
          //         y_delta[j]  ;
        }
      //      qDebug() << "B["<<i<<"]: " << B[i]<<endl;
    }
