...
int US_Minimize::Fit() {
  ...
  if (calc_model(guess) < 0) {
    ...
    qDebug()
        << "The residuals from the initial guess are too large.\n"
  ...
  while ((iteration < maxIterations) && (new_residuals / points > tolerance)) {
    ...
    if ((nlsMethod < 3) || (nlsMethod == 3 && iteration == 1)) {
      ...
      // matrix_temp.calc_A_transpose_A(&jacobian, &information_matrix, points,
      // parameters, 15);
    ...
    if (nlsMethod == 3)  // Quasi-Newton
    {
      ...
      */
      for (unsigned int i = 0; i < parameters; i++) {
        gamma[i] =
            (float)B[i];  // B = J'y_delta = -gradient; in updateQN we need
      ...
      for (unsigned int i = 0; i < parameters; i++) {
        ...
        // qDebug() << "Paramter " << i << ": gamma:" << gamma[i] << ", delta: "
        // << delta[i] << ", alpha: " << alpha << ", search: " << search[i] <<
        // ", B: " << B[i]  ;
      }
    ...
    if (nlsMethod == 0 || nlsMethod == 2) {
      // Add lambda to the information matrix to make diagonal large to make
    ...
    while (new_residuals >= old_residuals && nlsMethod != 3) {
      ...
      //   Problem: J * R = y_delta, where R = (a[i] - guess[i]) and we want to
      //   find a[i], so solve for R:
      // (1)   J'J * R = J' * y_delta
      ...
      // Alternative Solution: This can be solved using first forward and then
      // backward substitution, and Cholesky Decomposition: (1)   J'J * R =
      // J'*y_delta
      ...
      // save information matrix in LL_transpose in case chi-2 is larger, in
      // which case we would need to reset Lambda
        ...
        if (GUI) {
          if (nlsMethod == 0) {
            ...
            if (showGuiFit && !us_auto_mode) {
              ...
              message.setText(
                  tr("The Cholesky Decomposition of the\nInformation matrix "
                     "failed due to a\nsingularity in the matrix.\n\nYou may "
                     "achieve convergence by\nre-fitting the current data "
      ...
      // Now the information matrix actually contains the decomposed information
      // matrix LL' (4)   L(L'*R) = B (5)    Substitute:  Z = L'*R (6)    Solve
      // for Z using forward substitution: L*Z = B (7)    Solve for R using
      // backward substitution: L'*R = Z
      ...
      // 	 qDebug().noquote() << fit_timers.list_times();
      // Now B is changed to R (the difference between the current parameter
      // estimate with the "true" parameter)
      ...
      switch (nlsMethod) {
        ...
        case 1: {
          ...
          for (unsigned int i = 0; i < parameters; i++) {
            ...
            //                  test_guess[i] = guess[i] + lambda * (float)
            //                  B[i];
          }
          break;
        }
        ...
        switch (nlsMethod) {
          ...
          case 2: {
            ...
            if (lambda < 1.0) {
              ...
              lambda = 0.0;
              step *= 2.0;
              step_counter = 0;  // reset step counter to zero, because we
                                 // lengthened the step.
              dostep = true;
            }
            break;
...
float US_Minimize::linesearch(float **search, float f0) {
  ...
  // look for the minimum residual. Residual values are f0, f1, f2, evaluated at
  // x0, x1, x2. x0, x1, x2 are multipliers for incremental change of the
  ...
  // some stepsize x2 away from x0. then find an x1 in the middle between x0 and
  // x2 and calculate f1(x1) and f2(x2), where shift to the right f is the
  ...
  while (errno != 0 ||
         (f0 >= 10000 || f0 < 0 || f1 >= 10000 || f1 < 0 || f2 >= 10000 ||
          f2 < 0))  // make the initial step size smaller if we have infinite
                    // residuals
  {
    x1 /= 10;
    x2 /= 10;
  ...
  while (check_flag) {
    ...
    //      f0) < FLT_MIN) && f2 > f1)   // some weird cases can happen near the
    //      minimum
    ...
    {
      // qDebug() << "leftshift " << "x0: " << x0 << ", x1: " << x1 << ", x2: "
      // << x2 << ", f0: " << f0 << ", f1: " << f1 << ", f2: " << f2  ;
      x2 = x1;
      f2 = f1;
      x1 = (x2 + x0) / 2.0;
    ...
    {
      // qDebug() << "rightshift " << "x0: " << x0 << ", x1: " << x1 << ", x2: "
      // << x2 << ", f0: " << f0 << ", f1: " << f1 << ", f2: " << f2  ;
      x0 = x1;
      f0 = f1;
      x1 = x2;
      ...
      f2 = calc_testParameter(search, x2);
      if (f2 < 0) return (0.0);
      // qDebug() << "rightshift #2" << "x0: " << x0 << ", x1: " << x1 << ", x2:
      // " << x2 << ", f0: " << f0 << ", f1: " << f1 << ", f2: " << f2  ;
    }
    iter++;
    // qDebug() << iter  ;
...
float US_Minimize::calc_testParameter(float **search, float step) {
  ...
  for (unsigned int i = 0; i < parameters; i++) {
    ...
    // test_guess[i] << ", guess: " << guess[i] << ", search: " << (*search)[i]
    // ;
  }
  /*
    QString s="", format;
...
void US_Minimize::plot_overlays() { plotResiduals = false; }
void US_Minimize::plot_residuals() { plotResiduals = true; }
...
int US_Minimize::calc_model(double *) { return (-1); }
...
int US_Minimize::calc_jacobian() { return (-1); }
