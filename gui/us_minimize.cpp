...
int US_Minimize::Fit() {
   ...
   qDebug() << "iter: " << iteration << ", maxiter: " << maxIterations << ", new_resid: " << new_residuals / points
            << ", tolerance: " << tolerance;
   ...
   while ((iteration < maxIterations) && (new_residuals / points > tolerance)) {
      ...
      if ((nlsMethod < 3) || (nlsMethod == 3 && iteration == 1)) {
         ...
         matrix->calc_A_transpose_A(
            &jacobian, &information_matrix, points, parameters, settings->threads()); // ATTENTION /////////
...
float US_Minimize::linesearch(float **search, float f0) {
   ...
   while (
      errno != 0
      || (f0 >= 10000 || f0 < 0 || f1 >= 10000 || f1 < 0 || f2 >= 10000 || f2 < 0)) //make the initial step size smaller if we have infinite residuals
   {
      x1 /= 10;
      x2 /= 10;
   ...
   while (check_flag) {
      ...
      if ((qIsNaN(f0) && qIsNaN(f1)) || (qIsNaN(f1) && qIsNaN(f2)) || (qIsNaN(f0) || qIsNaN(f2))) // at least two values are screwed up, exit.
      {
      ...
      if (
         ((fabs(f0 - f1) < FLT_MIN) && (fabs(f1 - f2) < FLT_MIN))
...
void US_Minimize::save_Fit() {
   ...
   mBox.setText(tr("Please use the save function from the main extinction calculation module. It will allow you to "
                   "save the coefficients of the fit as well as a copy of the plot window"));
