...
US_DataLoader::US_DataLoader(bool late, int local,
                             QVector<US_DataIO::RawData>& rData,
                             QVector<US_DataIO::EditedData>& eData,
                             QStringList& trips, QString& desc,
                             QMap<QString, QString>& prot_det, QString tfilt)
  ...
  connect(disk_controls, SIGNAL(changed(bool)), SLOT(update_disk_db(bool)));
  ...
  te_notes->setText(
      tr("Right-mouse-button-click on a list selection"
         " for details."));
...
US_DataLoader::US_DataLoader(bool late, int local,
                             QVector<US_DataIO::RawData>& rData,
                             QVector<US_DataIO::EditedData>& eData,
                             QStringList& trips, QString& desc, QString tfilt)
    : US_WidgetsDialog(0, 0),
      latest(late),
      rawData(rData),
      editedData(eData),
...
void US_DataLoader::search(const QString& search_string) {
  ...
  for (int i = 0; i < tw_data->topLevelItemCount(); i++) {
    ...
    bool hide = !twi->text(0).contains(search_string, Qt::CaseInsensitive);
    twi->setHidden(hide);
  }
}
...
bool US_DataLoader::load_edit(void) {
  ...
  if (selections.size() > 1 ||
      selections[0]->parent() !=
          NULL) {  // Multiple selections, so test that they are valid
    foreach (twi, selections) {
      index = twi->type();
  ...
  if (selections.size() == 1 && selections[0]->parent() == NULL) {
    ...
    if (sel_run) {
      ...
      for (int ii = 0; ii < dlabels.size(); ii++) indexes << ii;
    }
  } else
    qDebug() << "LdEd: selsz" << selections.size() << "dlabsz"
             << dlabels.size();

  if (indexes.size() < 1) {
    twi = selections[0];
    QMessageBox::warning(this, tr("No Edits Available"),
  ...
  if (!disk_controls->db())  // Load files from local disk
  {
    for (int ii = 0; ii < indexes.size(); ii++) {
      ...
      if (triple != ftriple &&
          ftriple.contains("-")) {  // Modify filename to signal MWL
  ...
  else  // Load data from database
  {
    ...
    for (int ii = 0; ii < indexes.size();
         ii++) {  // Read in each of the edits that was selected
      QString key = dlabels[indexes[ii]];
      ddesc = datamap[key];
      int idRec = ddesc.DB_id;
      QString dtriple = ddesc.tripID;
      QString triple = QString(dtriple).replace(".", " / ");
      qDebug() << "LdEd:  ii" << ii << "ndxii" << indexes[ii] << "triple"
               << triple;
      ...
      QString invID = (us_automode) ? protocol_details["invID_passed"]
                                    : QString::number(US_Settings::us_inv_ID());
      QString aucfn = ddesc.runID + "." + filename.section(".", -5, -3) + "." +
                      clambda + ".auc";
      ...
      if (QFile(afn).exists()) {  // AUC file exists, so only download if
                                  // checksum mismatch
        QString fcheck = US_Util::md5sum_file(afn);

        if (ddesc.acheck.isEmpty()) {  // No database checksum+size, so get it
          ...
          ddesc.acheck = db.value(8).toString() + " " + db.value(9).toString();
      ...
      if (dscfname != prvfname) {
        ...
        if (QFile(efn).exists()) {  // Edit XML file exists, so only download if
                                    // checksum mismatch
        ...
        if (dnld_edt) db.readBlobFromDB(efn, "download_editData", idRec);
      ...
      qDebug()
          << "In LOAD auc/edited data in fematch: uresdir, filename, afn, efn: "
          << uresdir << filename << afn << efn;
      qDebug() << "In LOAD auc/edited data in fematch: idRec(editID), "
                  "idAUC(rawData): "
               << idRec << idAUC;
...
void US_DataLoader::describe() {
  ...
  if (cdesc.contains(sep)) sep = "^";  // Use carat if semi-colon already in use

  // Create and return a composite description string
  description = sep + label + sep + descript + sep + dbID + sep + filename +
                sep + dataGUID + sep + aucID + sep + aucGUID;
...
void US_DataLoader::list_data() {
  ...
  if (sel_run) {              // A run has been selected, so scan for edits
    if (disk_controls->db())  // Scan database data
    {
      scan_dbase_edit();
    } else  // Scan local disk data
    ...
    te_notes->setText(
        tr("Select a run, then click on \"Show Edits\""
  ...
  QStringList hdrs;
  hdrs << (sel_run ? tr("Run|Triple|Edit") : tr("Run")) << tr("Date")
       << tr("DbID") << tr("Label");
  tw_data->clear();
  ...
  for (int ii = 0; ii < dlabels.size();
    ...
    if (crlabel != prlabel) {  // New runID:  add top-level item
      ...
      ndxe = 1;
               ...
               ptlabel) {  // New triple in same runID: add triple child of run
      twi_trip = new QTreeWidgetItem(QStringList(ctlabel), ii);
      twi_runi->addChild(twi_trip);
      ptlabel = ctlabel;
      ndxt++;
      ndxe = 1;
    } else {  // Same triple as before
      ndxe++;
    }
  ...
  for (int ii = dlabels.size() - 1; ii >= 0; ii--) {
    ...
    if (crlabel != prlabel) {  // New run:   get triple and edit count
      ndxt = ddesc.tripknt;
      ndxe = ddesc.editknt;
      prlabel = crlabel;
      ptlabel = "";
    ...
    else {  // Same run:  update triple count
      ddesc.tripknt = ndxt;
      update = true;
    }

    if (ctlabel != ptlabel) {  // New triple:  get edit count
      ndxe = ddesc.editknt;
      ptlabel = ctlabel;
    }

    else {  // Same triple:  update edit count
      ddesc.editknt = ndxe;
      update = true;
    }

    if (update) {  // One or other count has been updated
      datamap[cdescr] = ddesc;
    }
  }
  QApplication::restoreOverrideCursor();
...
void US_DataLoader::selected() {
  ...
  if (selitems.size() <
      1) {  // "Show Edits" with no run selected:  build full data tree
    runID_sel = "";
    te_notes->setText(
        tr("Reading edit information to fully populate"
           " the list data tree..."));
  }

  else {  // "Show Edits" with run selected:  build a data tree for the selected
          // run
...
void US_DataLoader::accepted() {
  if (us_automode) {
    tw_data->setCurrentItem(tw_data->topLevelItem(0),    // item
                            0,                           // column
...
void US_DataLoader::scan_dbase_edit() {
  ...
  while (db.next()) {  // Accumulate a mapping of AUC Filename to DB ID
  ...
  while (db.next()) {  // Accumulate edit record parameters from DB
    QString etype = db.value(8).toString().toLower();

    qDebug() << "etype -- " << etype;

    if (tfilter &&
        etype != etype_filt)  // ALEXEY < --- HERE, etype != etype_filt
                              // (etype_filt == "velocity")
      continue;
  ...
  for (int ii = 0; ii < nedit; ii++) {
    ...
    QString runID =
        descrip.isEmpty()
            ? filebase.section(".", 0,
                               -7)  // 'DubnauD_ComEA-high_121021-run1237'
            : descrip;
    ...
    // qDebug() << "ScDB:   isMwl idAUC" << isMwl << idAUC << "aucfname" <<
    // aucfname;
    ...
    if (isMwl) {  // For MWL we must generate multiple entries for a
                  // Cell/Channel edit
      ...
      while (!xml.atEnd()) {  // Build up a string list of Lambdas
        xml.readNext();
      ...
      for (int ii = 0; ii < tripknt;
           ii++) {  // Add an entry for each Lambda in the edit
        elambda = mwlambds[ii];
...
void US_DataLoader::scan_local_edit(void) {
  ...
  for (int ii = 0; ii < aucdirs.size(); ii++) {
    ...
    if (rfilter && runID != runID_sel) continue;
    ...
    for (int jj = 0; jj < edtfiles.size(); jj++) {
      ...
      if (tfilter && expType != etype_filt) continue;
...
void US_DataLoader::scan_dbase_runs() {
  ...
  while (db.next()) {  // Accumulate a mapping of data descriptions to runs
    ...
    if (us_automode) {
      qDebug() << "US_DATALOADER: runD, protocol_details[filename] -- " << runID
...
void US_DataLoader::scan_local_runs(void) {
  ...
  for (int ii = 0; ii < aucdirs.size(); ii++) {
    ...
    QString filename = subdir + "/" + filebase;
    QString editID = filebase.section(".", -6, -6);
    editID = (editID.length() == 12 && editID.startsWith("20")) ? editID.mid(2)
                                                                : editID;
    QString tripID = filebase.section(".", -4, -2);
    QString edtlamb = tripID.section(".", 2, 2);
    QString label = runID;
    ...
    QString date = US_Util::toUTCDatetimeText(
        QFileInfo(filename).lastModified().toUTC().toString(Qt::ISODate), true);
...
void US_DataLoader::pare_to_latest(void) {
  ...
  for (int kk = 0; kk < 2; kk++)  // May need two passes to pare down
  {
    ...
    for (int ii = 0; ii < keys.size() - 1; ii++) {
      ...
      if (crunid != frunid) {  // Likely difference in runid, but check for MWL
        QString cruncc = clabel.section(".", 0, -3);
        QString fruncc = flabel.section(".", 0, -3);

        if (cruncc == fruncc) kmwl++;  // Mark possible MWL case
      ...
      qDebug() << "PARE 1: vals.at( ii ).editID.left(10), vals.at( jj "
                  ").editID.left( 10 ) -- "
               << vals.at(ii).editID.left(10) << vals.at(jj).editID.left(10);
      qDebug() << "PARE 2: vals.at( ii ).editID.left(10).toInt(), vals.at( jj "
                  ").editID.left( 10 ).toLong() -- "
  ...
  if (kmwl > 0) pare_latest_mwl();  // More possible paring for MWL
}
...
void US_DataLoader::pare_latest_mwl(void) {
  ...
  for (int ii = 0; ii < keys.size(); ii++) {
    ...
    if (!cwavln.contains("-")) continue;  // Skip if not MWL

    QString cedit = fname.section(".", 0, -6);
    QString crunid = cedit.section(".", 0, -2);

    if (!mwruns.contains(crunid)) mwruns << crunid;

    if (!mwedts.contains(cedit)) mwedts << cedit;
  }

  // Go no further if there were less than 2 MWL edits
  //  or if the MWL edits count equals the MWL run count
  if (mwedts.count() < 2 || mwedts.count() == mwruns.count()) return;
  ...
  for (int ii = 1; ii < mwedts.count(); ii++) {
    ...
    if (currrun == prevrun) {  // If runs are the same, list the earlier edit
      mwrmvs << curredt;
    }
  ...
  for (int ii = 0; ii < keys.size(); ii++) {
    ...
    if (mwrmvs.contains(cedit)) {  // Remove an entry found in remove list
      QString clabel = keys.at(ii);
...
bool US_DataLoader::eventFilter(QObject* obj, QEvent* e) {
  if (obj == tw_data && e->type() == QEvent::ContextMenu) {
...
void US_DataLoader::show_data_info(QPoint pos) {
  ...
  QTreeWidgetItem* item =
      selitems.size() > 0 ? selitems.at(0) : tw_data->itemAt(pos);
  ...
  if (ddesc.DB_id < 0) {  // Local disk: separate name and directory
    filespec = filename.section("/", -1, -1) +
               tr("\n  File Directory:        ") + filename.section("/", 0, -2);
  }

  QString dtext =
      tr("Data Information:") + tr("\n  Label:                 ") + label +
...
void US_DataLoader::experiment_info(QString& rundir, QString& elabel,
  ...
  rundir.replace("\\", "/");
  if (QString(rundir).right(1) != "/") rundir += "/";

  QString runID = QString(rundir).section("/", -2, -2);
  QStringList ffilt(runID + ".??.xml");
  QStringList efiles = QDir(rundir).entryList(ffilt, QDir::Files, QDir::Name);
  if (efiles.size() != 1) return;
