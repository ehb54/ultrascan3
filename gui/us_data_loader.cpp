...
US_DataLoader::US_DataLoader(
   bool late, int local, QVector<US_DataIO::RawData> &rData, QVector<US_DataIO::EditedData> &eData, QStringList &trips,
   QString &desc, QMap<QString, QString> &prot_det, QString tfilt) :
    US_WidgetsDialog(0, 0), latest(late), rawData(rData), editedData(eData), triples(trips), description(desc),
    protocol_details(prot_det), etype_filt(tfilt) {
   ...
   QString inv_name = ((invlev > 0) ? QString::number(US_Settings::us_inv_ID()) + ": " : "")
...
bool US_DataLoader::load_edit(void) {
   ...
   if (selections.size() > 1 || selections[ 0 ]->parent() != NULL) { // Multiple selections, so test that they are valid
      foreach (twi, selections) {
         index = twi->type();
      ...
      foreach (index, indexes) {
         ...
         if (triples.contains(triple)) {
            QMessageBox::warning(this, tr("Invalid Selection"), tr("Only one edit from each triple may be selected."));
            return false;
         }
      }
   }
   ...
   QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
   te_notes->setText(tr("Loading data from ") + (disk_controls->db() ? "Database" : "Local Disk") + " ...");
   qApp->processEvents();
   ...
   if (!disk_controls->db()) // Load files from local disk
   {
      for (int ii = 0; ii < indexes.size(); ii++) {
         ...
         if (triple != ftriple && ftriple.contains("-")) { // Modify filename to signal MWL
         ...
         try {
            ...
            QApplication::restoreOverrideCursor();
            QMessageBox::warning(this, tr("Data Error"), US_DataIO::errorString(error));

            return false;
         } catch (int err) {
            QApplication::restoreOverrideCursor();
            QMessageBox::warning(this, tr("Data Error"), US_DataIO::errorString(err));
   ...
   else // Load data from database
   {
      ...
      for (int ii = 0; ii < indexes.size(); ii++) { // Read in each of the edits that was selected
         QString key = dlabels[ indexes[ ii ] ];
         ddesc = datamap[ key ];
         ...
         QString aucfn = ddesc.runID + "." + filename.section(".", -5, -3) + "." + clambda + ".auc";
         ...
         if (QFile(afn).exists()) { // AUC file exists, so only download if checksum mismatch
         ...
         if (dscfname != prvfname) {
            ...
            if (QFile(efn).exists()) { // Edit XML file exists, so only download if checksum mismatch
         ...
         qDebug() << "In LOAD auc/edited data in fematch: uresdir, filename, afn, efn: " << uresdir << filename << afn
                  << efn;
...
void US_DataLoader::describe() {
   ...
   description = sep + label + sep + descript + sep + dbID + sep + filename + sep + dataGUID + sep + aucID + sep
                 + aucGUID;
...
void US_DataLoader::list_data() {
   ...
   QStringList hdrs;
   hdrs << (sel_run ? tr("Run|Triple|Edit") : tr("Run")) << tr("Date") << tr("DbID") << tr("Label");
   tw_data->clear();
   ...
   for (int ii = 0; ii < dlabels.size(); ii++) { // Propagate list widget with labels
      QString cdescr = dlabels.at(ii);
      DataDesc ddesc = ddescrs.at(ii);
      QString dbID = fromDB ? QString().sprintf("%6d", ddesc.DB_id) : QString("");
      crlabels.clear();
      crlabels << ddesc.runID << QString(ddesc.date).section(" ", 0, 0).simplified() << dbID << ddesc.elabel;
      ...
      else if (ctlabel != ptlabel) { // New triple in same runID: add triple child of run
         twi_trip = new QTreeWidgetItem(QStringList(ctlabel), ii);
         twi_runi->addChild(twi_trip);
         ptlabel = ctlabel;
         ndxt++;
...
void US_DataLoader::selected() {
   ...
   if (selitems.size() < 1) { // "Show Edits" with no run selected:  build full data tree
      runID_sel = "";
      te_notes->setText(tr("Reading edit information to fully populate"
                           " the list data tree..."));
   }

   else { // "Show Edits" with run selected:  build a data tree for the selected run
...
void US_DataLoader::scan_dbase_edit() {
   ...
   while (db.next()) { // Accumulate edit record parameters from DB
      ...
      QString date = US_Util::toUTCDatetimeText(db.value(5).toDateTime().toString(Qt::ISODate), true);
   ...
   for (int ii = 0; ii < nedit; ii++) {
      ...
      if (isMwl) { // For MWL we must generate multiple entries for a Cell/Channel edit
         ...
         for (int ii = 0; ii < tripknt; ii++) { // Add an entry for each Lambda in the edit
            elambda = mwlambds[ ii ];
...
void US_DataLoader::scan_dbase_runs() {
   ...
   while (db.next()) { // Accumulate a mapping of data descriptions to runs
      ...
      if (us_automode) {
         qDebug() << "US_DATALOADER: runD, protocol_details[filename] -- " << runID << protocol_details[ "filename" ];
...
void US_DataLoader::scan_local_runs(void) {
   ...
   for (int ii = 0; ii < aucdirs.size(); ii++) {
      ...
      QString date = US_Util::toUTCDatetimeText(QFileInfo(filename).lastModified().toUTC().toString(Qt::ISODate), true);
   ...
   for (int kk = 0; kk < 2; kk++) // May need two passes to pare down
   {
      ...
      for (int ii = 0; ii < keys.size() - 1; ii++) {
         ...
         qDebug() << "PARE 2: vals.at( ii ).editID.left(10).toInt(), vals.at( jj ).editID.left( 10 ).toLong() -- "
                  << vals.at(ii).editID.left(10).toInt() << vals.at(jj).editID.left(10).toLong();
...
void US_DataLoader::show_data_info(QPoint pos) {
   ...
   QTreeWidgetItem *item = selitems.size() > 0 ? selitems.at(0) : tw_data->itemAt(pos);
   ...
   if (ddesc.DB_id < 0) { // Local disk: separate name and directory
      filespec = filename.section("/", -1, -1) + tr("\n  File Directory:        ") + filename.section("/", 0, -2);
   }

   QString dtext = tr("Data Information:") + tr("\n  Label:                 ") + label
                   + tr("\n  Description:           ") + descript + tr("\n  Edit Database ID:      ") + dbID
                   + tr("\n  Filename:              ") + filespec + tr("\n  Last Updated:          ") + ddesc.date
                   + tr("\n  Edit Global ID:        ") + ddesc.dataGUID + tr("\n  AUC Global ID:         ")
                   + ddesc.aucGUID + tr("\n  AUC DB ID:             ") + aucID + tr("\n  Experiment DB ID:      ")
                   + expID + tr("\n  Experiment Label:      ") + ddesc.elabel + tr("\n  Run ID:                ")
                   + ddesc.runID + tr("\n  Triple ID:             ") + ddesc.tripID + tr("\n  Edit ID:               ")
   ...
   edit->resize(700, 350);
   edit->e->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize()));
...
void US_DataLoader::experiment_info(QString &rundir, QString &elabel, QString &expID) {
   elabel = "";
   expID = "-1";
