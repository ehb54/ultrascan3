...
US_DataLoader::US_DataLoader (bool late, int local,
    ...
    : US_WidgetsDialog (0, 0), latest (late), rawData (rData),
      editedData (eData), triples (trips), description (desc),
...
void
US_DataLoader::search (const QString &search_string)
{
...
US_DataLoader::load_edit (void)
{
  ...
  if (!disk_controls->db ()) // Load files from local disk
  ...
  else // Load data from database
    {
      ...
      qDebug () << "LdEd: ndx0" << indexes[0] << "dlab0"
                << dlabels[indexes[0]];
      ...
      for (int ii = 0; ii < indexes.size (); ii++)
        { // Read in each of the edits that was selected
            ...
            { // AUC file exists, so only download if checksum mismatch
              ...
              if (ddesc.acheck.isEmpty ())
                { // No database checksum+size, so get it
                  ...
                  db.next ();
                  qDebug ()
                      << "LdEd: w-e, dberr" << db.lastErrno () << US_DB2::OK;
          ...
          if (dscfname != prvfname)
            {
              ...
              if (QFile (efn).exists ())
                { // Edit XML file exists, so only download if checksum
                  // mismatch
          ...
          qDebug () << "In LOAD auc/edited data in fematch: uresdir, "
                       "filename, afn, efn: "
          ...
          qDebug () << "LdEd: loadData uresdir filename" << uresdir
                    << filename;
      ...
      qDebug () << "AFTER In LOAD auc/edited data: edtatafiles -- "
                << edatafiles;
...
void
US_DataLoader::update_person (int ID)
{
...
US_DataLoader::list_data ()
{
  ...
  for (int ii = 0; ii < dlabels.size (); ii++)
    { // Propagate list widget with labels
      ...
      QString dbID
          = fromDB ? QString ().sprintf ("%6d", ddesc.DB_id) : QString ("");
      crlabels.clear ();
...
US_DataLoader::selected ()
{
  ...
  if (selitems.size () < 1)
    ...
    { // "Show Edits" with run selected:  build a data tree for the selected
      // run
...
US_DataLoader::scan_dbase_edit ()
{
  ...
  while (db.next ())
    { // Accumulate edit record parameters from DB
      ...
      QString runID
          = descrip.isEmpty () ? filebase.section (".", 0, -7) : descrip;
...
void
US_DataLoader::scan_local_edit (void)
{
      ...
      for (int jj = 0; jj < edtfiles.size (); jj++)
        {
          ...
          QString date = US_Util::toUTCDatetimeText (
              QFileInfo (filename).lastModified ().toUTC ().toString (
                  Qt::ISODate),
              true);
...
void
US_DataLoader::scan_local_runs (void)
{
...
void
US_DataLoader::pare_to_latest (void)
{
  int kmwl = 0;
      ...
      for (int ii = 0; ii < keys.size () - 1; ii++)
        {
          ...
          qDebug () << "PARE 2: vals.at( ii ).editID.left(10).toInt(), "
...
void
US_DataLoader::pare_latest_mwl (void)
{
  QStringList keys = datamap.keys ();
  QList<DataDesc> vals = datamap.values ();
  QStringList mwruns;
...
bool
US_DataLoader::eventFilter (QObject *obj, QEvent *e)
{
...
void
US_DataLoader::show_data_info (QPoint pos)
{
  QList<QTreeWidgetItem *> selitems = tw_data->selectedItems ();
...
void
US_DataLoader::update_disk_db (bool db)
{
  emit changed (db);
...
void
US_DataLoader::experiment_info (QString &rundir, QString &elabel,
                                QString &expID)
{
  elabel = "";
  expID = "-1";
  ...
  QStringList efiles
      = QDir (rundir).entryList (ffilt, QDir::Files, QDir::Name);
  if (efiles.size () != 1)
    return;
