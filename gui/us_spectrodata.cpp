...
// Get x range
QwtInterval US_SpectrogramData::xrange() { return QwtInterval(xmin, xmax); }

// Get y range
QwtInterval US_SpectrogramData::yrange() { return QwtInterval(ymin, ymax); }
...
double US_SpectrogramData::value(double x, double y) const {
  ...
  if (jr1 < 0 || jr1 > mxr) {  // for x,y outside raster, return z-minimum
    return zmin;
  }
  ...
  if (dx > 0.1 ||
      dy > 0.1) {  // if either fraction significant, interpolate output value
    int jr2 = jr1 + 1;      // x+1,y
...
void US_SpectrogramData::setRaster(QList<S_Solute>* solu) {
  ...
  if (xmin == xmax) {      // Auto-limits:  calculate x,y ranges
  ...
  if (resol != 100.0) {
    for (int kk = 0; kk < nsol;
         kk++) {              // spread z values for each distribution point
      ...
      for (int ii = fy; ii < ly;
           ii++) {  // calculate y-term and x range for each y
        ...
        for (int jj = fx; jj < lx; jj++) {  // calculate x-term then calculate
                                            // z-value for raster point
          ...
          if (zout > zin) rdata.replace(kr, zout);

          kr++;  // bump rast point index
        }
      }
    }
  } else {  // for resolution=100, make all points in circle have zval value
    for (int kk = 0; kk < nsol;
         kk++) {              // spread z values for each distribution point
      xval = solu->at(kk).s;  // x,y,z
      yval = solu->at(kk).k;
      ...
      for (int ii = fy; ii < ly;
        ...
        for (int jj = fx; jj < lx; jj++) {  // calculate x-term then calculate
          ...
          if (zdecay > LO_DTERM && zval > zin) rdata.replace(kr, zval);
...
void US_SpectrogramData::value(int x, int y, double& x_out, double& y_out,
                               double& z_out) const {
