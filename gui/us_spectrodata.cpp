...
void US_SpectrogramData::setRastRanges(double a_xres, double a_yres, double a_reso, double a_zfloor, QRectF a_drecti) {
   xreso = a_xres;
   yreso = a_yres;
   resol = a_reso;
...
double US_SpectrogramData::value(double x, double y) const {
   ...
   if (dx > 0.1 || dy > 0.1) { // if either fraction significant, interpolate output value
      int jr2 = jr1 + 1; // x+1,y
...
void US_SpectrogramData::setRaster(QList<S_Solute> *solu) {
   ...
   if (resol != 100.0) {
      for (int kk = 0; kk < nsol; kk++) { // spread z values for each distribution point
         ...
         for (int ii = fy; ii < ly; ii++) { // calculate y-term and x range for each y
            ...
            for (int jj = fx; jj < lx; jj++) { // calculate x-term then calculate z-value for raster point
