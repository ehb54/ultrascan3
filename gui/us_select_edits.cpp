...
void US_SelectEdits::accepted() {
  ...
  for (int ii = 0; ii < selitems.size(); ii++) {  // Loop thru selected runIDs
    ...
    for (int jj = 0; jj < elabs.size();
         jj++) {  // Return editIDs from each edit in a run
      QString editID = editmap[elabs[jj]].editID;

      if (!editIDs.contains(editID)) editIDs << editID;
    }
  }
...
void US_SelectEdits::scan_local_edit(void) {
  ...
  for (int ii = 0; ii < aucdirs.size(); ii++) {
    ...
    if (fileri.open(QIODevice::ReadOnly |
                    QIODevice::Text)) {  // Read the experiment XML and get
                                         // experiment label
