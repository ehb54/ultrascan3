...
void
US_SelectEdits::search (const QString &search_string)
{
...
void
US_SelectEdits::scan_dbase_edit ()
{
  // qDebug() << "ScDB:TM:00: " <<
  ...
  while (db.next ())
    {
      ...
      qDebug () << "edesc key" << key << "expID" << expID << "label"
                << edesc.label;
      edesc.editID = recID;
      edesc.date = date;
      edesc.DB_id = expID;
...
void
US_SelectEdits::scan_local_edit (void)
{
...
void
US_SelectEdits::build_runids (void)
{
  elabels = editmap.keys ();
...
void
US_SelectEdits::update_person (int ID)
{
...
// Slot to handle accept in investigator dialog
void
US_SelectEdits::update_disk_db (bool isDB)
{
