...
US_Properties::US_Properties (US_Model &mod, int access)
    : US_WidgetsDialog (0, 0), model (mod), db_access (access)
{
  ...
  // Row
  QGridLayout *lo_sigConc
      = us_checkbox (tr ("Signal Concentration:"), ck_sigConc, true);
  le_sigConc = us_lineedit ("");
  QGridLayout *lo_isProd
      = us_checkbox (tr ("Analyte is a Product"), ck_isProd, false);

  // Row
  QGridLayout *lo_molConc
      = us_checkbox (tr ("Molar Concentration:"), ck_molConc, false);
  ...
  // Row
  QGridLayout *lo_mw
      = us_checkbox (tr ("Molecular Wt. (mw,S20W)"), ck_mw, true);
  ...
  // Row
  QLabel *lb_sigma
      = us_label (tr ("Conc. Dependency of s (<span>&sigma;</span>):"));

  le_sigma = us_lineedit ("");

  // Row
  QLabel *lb_delta
      = us_label (tr ("Conc. Dependency of D (<span>&delta;</span>):"));
  le_delta = us_lineedit ("");

  // Row
  pb_load_c0 = us_pushbutton (tr ("Load C0 from File"));

  QGridLayout *lo_co_sed
      = us_checkbox (tr ("Co-sedimenting Solute"), ck_co_sed);
...
void
US_Properties::disable_gui (void)
{
  pb_accept->setEnabled (false);
}

void
US_Properties::clear_entries (void)
{
  le_guid->clear ();
  le_vbar->clear ();
  le_extinction->clear ();
...
void
US_Properties::update_analyte (US_Analyte new_analyte)
{
  analyte = new_analyte;
...
void
US_Properties::del_component (void)
{
  ...
  int response = QMessageBox::question (this, tr ("Delete Analyte?"),
...
void
US_Properties::set_oligomer (double oligomer)
{
  int row = lw_components->currentRow ();
  if (row < 0)
    return;
...
void
US_Properties::edit_component (void)
{
  int row = lw_components->currentRow ();
  if (row < 0)
    return;
...
US_Properties::load_c0 (void)
{
  ...
  if (!pb_load_c0->icon ().isNull ())
    {
      int response
          = QMessageBox::question (this, tr ("Remove C0 Data?"),
...
void
US_Properties::select_shape (int shape)
{
  if (inUpdate)
    return;
...
void
US_Properties::save_changes (int row)
{
  if (row < 0)
    return;
...
US_Properties::simulate (void)
{
  ...
  US_Predict1 *dialog
      = new US_Predict1 (working_data, analyte, db_access, true);
...
void
US_Properties::new_hydro (US_Analyte ad)
{
  int row = lw_components->currentRow ();
  if (row < 0)
...
void
US_Properties::enable (QLineEdit *le, bool status, const QPalette &p)
{
...
int
US_Properties::countChecks (void)
{
  int checked = 0;
  if (ck_mw->isChecked ())
    checked++;
...
void
US_Properties::co_sed (int new_state)
{
  if (inUpdate)
    return;
...
bool
US_Properties::keep_standard (void)
{
  if (le_guid->text ().isEmpty ())
    return false;
...
void
US_Properties::source_changed (bool db)
{
...
void
US_Properties::check_molar (bool chkd)
{
...
void
US_Properties::check_signal (bool chkd)
{
  // Set read-only states of concentration edits
  us_setReadOnly (le_sigConc, !chkd);
  us_setReadOnly (le_molConc, chkd);
