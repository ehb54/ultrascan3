...
US_Properties::US_Properties(US_Model &mod, int access) : US_WidgetsDialog(0, 0), model(mod), db_access(access) {
   ...
   // Row
   QGridLayout *lo_sigConc = us_checkbox(tr("Signal Concentration:"), ck_sigConc, true);
   le_sigConc = us_lineedit("");
   QGridLayout *lo_isProd = us_checkbox(tr("Analyte is a Product"), ck_isProd, false);

   // Row
   QGridLayout *lo_molConc = us_checkbox(tr("Molar Concentration:"), ck_molConc, false);
   ...
   // Row
   QGridLayout *lo_mw = us_checkbox(tr("Molecular Wt. (mw,S20W)"), ck_mw, true);
   ...
   // Row
   QGridLayout *lo_f_f0 = us_checkbox(tr("Frictional Ratio (f/f0,20W)"), ck_f_f0, true);

   le_f_f0 = us_lineedit("n/a");
   us_setReadOnly(le_f_f0, true);

   // Row
   QGridLayout *lo_s = us_checkbox(tr("Sedimentation Coeff. (s,20W)"), ck_s);
   le_s = us_lineedit("1e-13");

   // Row
   QGridLayout *lo_D = us_checkbox(tr("Diffusion Coeff. (D,20W)"), ck_D);

   le_D = us_lineedit("2e-7");

   // Row
   QGridLayout *lo_f = us_checkbox(tr("Frictional Coeff. (f,20W)"), ck_f);
   ...
   QGridLayout *lo_co_sed = us_checkbox(tr("Co-sedimenting Solute"), ck_co_sed);
   ...
   main->addWidget(lw_components, row, 0, 4, 6);
   row += 4;
   ...
   connect(pb_edit, SIGNAL(clicked()), SLOT(edit_analyte()));
   connect(le_description, SIGNAL(editingFinished()), SLOT(edit_component()));
   connect(lw_components, SIGNAL(currentRowChanged(int)), SLOT(update(int)));
   connect(le_vbar, SIGNAL(editingFinished()), SLOT(edit_vbar()));
   connect(le_extinction, SIGNAL(editingFinished()), SLOT(set_molar()));
   connect(ck_molConc, SIGNAL(toggled(bool)), SLOT(check_molar(bool)));
   connect(ck_sigConc, SIGNAL(toggled(bool)), SLOT(check_signal(bool)));
   connect(le_sigConc, SIGNAL(editingFinished()), SLOT(set_molar()));
   connect(pb_sim, SIGNAL(clicked()), SLOT(simulate()));
   connect(cb_shape, SIGNAL(currentIndexChanged(int)), SLOT(select_shape(int)));
   connect(ck_mw, SIGNAL(toggled(bool)), SLOT(calculate(bool)));
   connect(le_mw, SIGNAL(editingFinished()), SLOT(calculate()));
   connect(ct_oligomer, SIGNAL(valueChanged(double)), SLOT(set_oligomer(double)));
   connect(ck_f_f0, SIGNAL(toggled(bool)), SLOT(calculate(bool)));
   connect(le_f_f0, SIGNAL(editingFinished()), SLOT(calculate()));
   connect(ck_s, SIGNAL(toggled(bool)), SLOT(calculate(bool)));
   connect(le_s, SIGNAL(editingFinished()), SLOT(calculate()));
   connect(ck_D, SIGNAL(toggled(bool)), SLOT(calculate(bool)));
   connect(le_D, SIGNAL(editingFinished()), SLOT(calculate()));
   connect(ck_f, SIGNAL(toggled(bool)), SLOT(calculate(bool)));
...
void US_Properties::newAnalyte(void) {
   ...
   US_AnalyteGui *dialog = new US_AnalyteGui(true, QString(), db_access);

   connect(dialog, SIGNAL(valueChanged(US_Analyte)), SLOT(update_analyte(US_Analyte)));
...
void US_Properties::update_analyte(US_Analyte new_analyte) {
   ...
   if (model.wavelength == 0.0) { // If model has no wavelength, set it from the analyte
...
void US_Properties::del_component(void) {
   ...
   int response = QMessageBox::question(
      this, tr("Delete Analyte?"), tr("Delete the following analyte?\n") + item->text(), QMessageBox::Yes,
      QMessageBox::No);

   if (response == QMessageBox::No)
      return;

   if (model.coSedSolute == row)
      model.coSedSolute = -1;
...
void US_Properties::set_oligomer(double oligomer) {
   ...
   if (oligomer > 1.0) { // if oligomer count greater than 1, must check MW and frictional ratio
      if (!(ck_mw->isChecked() && ck_f_f0->isChecked())) { // mw,f_f0 not both checked, so make it so
   ...
   if (mwuck) { // if MW unchecked, fake it for re-calculate coeffs phase
      ck_mw->setChecked(true);
      ck_s->setChecked(false);
   }
   ...
   chgStoi = false;
   if (mwuck) { // if MW was unchecked, restore check state to pre-calc state
      inUpdate = true;
...
void US_Properties::edit_component(void) {
   ...
   if (desc == lw_components->item(row)->text())
      return;
...
void US_Properties::save_changes(int row) {
   ...
   switch (cb_shape->currentIndex()) {
      case US_Model::SPHERE:
         sc->shape = US_Model::SPHERE;
         break;

      case US_Model::PROLATE:
         sc->shape = US_Model::PROLATE;
         break;

      case US_Model::OBLATE:
         sc->shape = US_Model::OBLATE;
         break;

      default:
         sc->shape = US_Model::ROD;
         break;
   }
...
void US_Properties::update(int /* row */) {
   ...
   switch (sc->shape) {
      case US_Model::SPHERE:
         ...
         break;
      case US_Model::PROLATE:
         cb_shape->setCurrentIndex(1);
         break;
      case US_Model::OBLATE:
         cb_shape->setCurrentIndex(2);
         break;
      default:
   ...
   (sc->c0.radius.size() > 0) ? pb_load_c0->setIcon(check) : pb_load_c0->setIcon(QIcon());
   // Set co-sedimenting solute
   (row == model.coSedSolute) ? ck_co_sed->setChecked(true) : ck_co_sed->setChecked(false);
...
void US_Properties::simulate(void) {
   ...
   US_Predict1 *dialog = new US_Predict1(working_data, analyte, db_access, true);

   connect(dialog, SIGNAL(changed(US_Analyte)), SLOT(new_hydro(US_Analyte)));
...
bool US_Properties::keep_standard(void) {
   ...
   int response = QMessageBox::question(
      this, tr("Changing Standard Value"),
...
void US_Properties::calculate(void) {
   ...
   sc->mw = ck_mw->isChecked() ? le_mw->text().toDouble() : (chgStoi ? sc->mw : 0.0);
