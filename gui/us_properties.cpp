...
void US_Properties::disable_gui(void) { pb_accept->setEnabled(false); }
...
void US_Properties::update_analyte(US_Analyte new_analyte) {
  ...
  if (last < 0) last = model.components.count() - 1;

  update(0);

  if (model.wavelength ==
      0.0) {  // If model has no wavelength, set it from the analyte
    QList<double> keys = analyte.extinction.keys();
    if (keys.length() > 0) model.wavelength = keys[0];
  }
...
void US_Properties::set_oligomer(double oligomer) {
  ...
  if (oligomer > 1.0) {  // if oligomer count greater than 1, must check MW and
                         // frictional ratio
    if (!(ck_mw->isChecked() &&
          ck_f_f0->isChecked())) {  // mw,f_f0 not both checked, so make it so
...
void US_Properties::load_c0(void) {
  ...
  if (!pb_load_c0->icon().isNull()) {
    int response = QMessageBox::question(this, tr("Remove C0 Data?"),
...
void US_Properties::checkbox(void) {
  ...
  if ((int)ct_oligomer->value() > 1) us_setReadOnly(le_mw, true);
}
...
void US_Properties::calculate(void) {
  ...
  sc->mw =
      ck_mw->isChecked() ? le_mw->text().toDouble() : (chgStoi ? sc->mw : 0.0);
...
void US_Properties::edit_analyte() {
  ...
  if (aguid.isEmpty()) aguid = model.components[row].analyteGUID;
