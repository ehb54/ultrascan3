...
int US_GuiUtil::save_plot(const QString &filename, const QwtPlot *plot) {
   ...
   if (filename.contains(".svg")) { // Save the file as SVG, then save a PNG version
...
int US_GuiUtil::save_csv(const QString &filename, const QwtPlot *plot) {
   ...
   if (filename.endsWith(".csv")) { // Save the file as a CSV
      ...
      for (QwtPlotItem *it: plot->itemList(QwtPlotItem::Rtti_PlotCurve)) {
         ...
         if (data->size() > 0 || (x_data.size() > 1 && y_data.size() > 1)) { // Only add data if there is data to add
      ...
      for (QwtPlotItem *it: plot->itemList(QwtPlotItem::Rtti_PlotSpectrogram)) {
         ...
         if (x_data.size() > 1 && y_data.size() > 1 && z_data.size() > 1) { // Only add data if there is data to add
