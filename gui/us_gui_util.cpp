...
int US_GuiUtil::save_csv(const QString& filename, const QwtPlot* plot) {
  ...
  if (filename.endsWith(".csv")) {  // Save the file as a CSV
    ...
    for (QwtPlotItem* it : plot->itemList(QwtPlotItem::Rtti_PlotCurve)) {
      ...
      y_data.clear();
      x_data << QString("\"") + item->title().text() + " " + x_axis_title +
                    QString("\"");
      y_data << QString("\"") + item->title().text() + " " + y_axis_title +
                    QString("\"");
      ...
      if (data->size() > 0 ||
          (x_data.size() > 1 &&
           y_data.size() > 1)) {  // Only add data if there is data to add
    ...
    for (QwtPlotItem* it : plot->itemList(QwtPlotItem::Rtti_PlotSpectrogram)) {
      ...
      y_data << QString("\"") + item->title().text() + " " + y_axis_title +
                    QString("\"");
      z_data << QString("\"") + item->title().text() + " " + z_axis_title +
                    QString("\"");
      // Get the matrix data
      ...
      if (x_data.size() > 1 && y_data.size() > 1 &&
          z_data.size() > 1) {  // Only add data if there is data to add
  ...
  else {  // Mark error: filename does not end with ".csv"
    status = 1;
  }
