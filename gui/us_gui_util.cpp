...
int
US_GuiUtil::save_plot (const QString &filename, const QwtPlot *plot)
{
  int status = 0;
...
// Save SVG file
int
US_GuiUtil::save_svg (const QString &filename, const QwtPlot *plot)
{
  int status = 0;
...
// Save PNG file
int
US_GuiUtil::save_png (const QString &filename, const QwtPlot *plot)
{
  int status = 0;
...
int
US_GuiUtil::save_csv (const QString &filename, const QwtPlot *plot)
{
  int status = 0;
  ...
  if (filename.endsWith (".csv"))
    { // Save the file as a CSV
      ...
      for (QwtPlotItem *it : plot->itemList (QwtPlotItem::Rtti_PlotCurve))
        {
          ...
          y_data.clear ();
          x_data << QString ("\"") + item->title ().text () + " "
                        + x_axis_title + QString ("\"");
          y_data << QString ("\"") + item->title ().text () + " "
                        + y_axis_title + QString ("\"");
      ...
      for (QwtPlotItem *it :
           plot->itemList (QwtPlotItem::Rtti_PlotSpectrogram))
        {
          ...
          qDebug () << item->interval (Qt::XAxis).minValue ()
                    << item->interval (Qt::XAxis).maxValue ();
          qDebug () << item->interval (Qt::YAxis).minValue ()
                    << item->interval (Qt::YAxis).maxValue ();
          qDebug () << item->boundingRect ().bottomLeft ()
                    << item->boundingRect ().topRight ();
          US_SpectrogramData *data
              = dynamic_cast<US_SpectrogramData *> (item->data ());
          // create two Vectors for x and y respective
          QSize raster_size;
          QRectF rect;
          ...
          y_data << QString ("\"") + item->title ().text () + " "
                        + y_axis_title + QString ("\"");
          z_data << QString ("\"") + item->title ().text () + " "
                        + z_axis_title + QString ("\"");
          // Get the matrix data
