...
US_AnalysisBase2::US_AnalysisBase2() : US_Widgets() {
  ...
  // Plots
  plotLayout1 = new US_Plot(data_plot1, tr("Plot 1 Title"), tr("X-Axis Title"),
                            tr("Y-Axis Title"));

  data_plot1->setMinimumSize(600, 150);

  plotLayout2 = new US_Plot(data_plot2, tr("Plot 2 Title"), tr("X-Axis Title"),
...
double US_AnalysisBase2::smooth_point(int scan, int point, int type,
...
double US_AnalysisBase2::calc_baseline(void) const {
  ...
  for (int j = point - 5; j < point + 6; j++) sum += scan->rvalues[j];
...
void US_AnalysisBase2::load_noise(int index) {
  ...
  if (noiflags[index] >=
      0) {  // If noise already applied, ask user: retain? reselect?
  ...
  te_desc->setHtml(
      tr("<b>%1 noise(s) found for %2</b>").arg(nenois).arg(triples[index]));
  qApp->processEvents();
