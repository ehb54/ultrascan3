...
US_AnalysisBase2::US_AnalysisBase2() : US_Widgets() {
   ...
   // Plots
   plotLayout1 = new US_Plot(data_plot1, tr("Plot 1 Title"), tr("X-Axis Title"), tr("Y-Axis Title"));

   data_plot1->setMinimumSize(600, 150);

   plotLayout2 = new US_Plot(
      data_plot2, tr("Plot 2 Title"), tr("X-Axis Title"), tr("Y-Axis Title"), true, ".*in range", "rainbow");
   ...
   QLayout *lo_edlast = us_checkbox(tr("Latest Data Edit"), ck_edlast, true);
   ...
   connect(ct_smoothing, SIGNAL(valueChanged(double)), SLOT(smoothing(double)));
   ...
   connect(ct_boundaryPercent, SIGNAL(valueChanged(double)), SLOT(boundary_pct(double)));
   connect(ct_boundaryPos, SIGNAL(valueChanged(double)), SLOT(boundary_pos(double)));
...
void US_AnalysisBase2::load(void) {
   ...
   US_DataLoader *dialog = new US_DataLoader(edlast, dbdisk, rawList, dataList, triples, description, etype_filt);

   connect(dialog, SIGNAL(changed(bool)), SLOT(update_disk_db(bool)));
   connect(dialog, SIGNAL(progress(const QString)), SLOT(set_progress(const QString)));
...
void US_AnalysisBase2::update(int selection) {
   ...
   bool bufin = US_SolutionVals::values(dbP, d, solID, svbar, bdens, bvisc, bcomp, bmanu, errmsg);
   ...
   if (bufin) {
      ...
      if (solID.isEmpty()) {
         QMessageBox::warning(this, tr("Solution/Buffer Values Fetch"), tr("Empty solution ID value!"));
      }

      else if (solID.length() < 36 && dbP != NULL) { // Have DB solution ID
         solution_rec.readFromDB(solID.toInt(), dbP);
      }

      else { // Have Local solution GUID
      ...
      QMessageBox::warning(this, tr("Solution/Buffer Values Fetch"), errmsg);
...
void US_AnalysisBase2::details(void) {
   US_RunDetails2 *dialog = new US_RunDetails2(rawList, runID, directory, triples);
   dialog->exec();
   qApp->processEvents();
   delete dialog;
}
...
void US_AnalysisBase2::data_plot(void) {
   ...
   if (d->dataType == "RI" && !has_intensity_profile(d->runID, disk_controls->db()))
   ...
   QString header = tr("Velocity Data for\n") + d->runID + "  (" + d->cell + "/" + d->channel + "/" + d->wavelength
                    + ")";
...
void US_AnalysisBase2::smoothing(double smoothCount) {
   ...
   if (smoothPoints > 1) {
      ...
      for (int jj = 0; jj < smoothPoints; jj++) {
         ...
         y_weights[ jj ] = 0.7 * US_Math2::normal_distribution(0.7, 0.0, x_weights[ jj ]);
      }
      ...
      for (int ii = 0; ii < dat->scanData.size(); ii++) {
         ...
         for (int jj = scanPoints - smoothPoints - 1; jj < scanPoints; jj++) {
            scn->rvalues[ jj ] = smooth_point(ii, jj, 1, smoothPoints, scanPoints);
         }
      } // END: scan loop
...
double US_AnalysisBase2::smooth_point(int scan, int point, int type, int smoothPoints, int scanPoints) {
...
double US_AnalysisBase2::calc_baseline(void) const {
   ...
   int point = US_DataIO::index(dataList[ row ].xvalues, dataList[ row ].baseline);
...
QString US_AnalysisBase2::table_row(const QString &s1, const QString &s2, const QString &s3) const {
   return (indent(6) + "<tr><td>" + s1 + "</td><td>" + s2 + "</td><td>" + s3 + "</td></tr>\n");
}

// Table row HTML with 4 columns
QString US_AnalysisBase2::table_row(const QString &s1, const QString &s2, const QString &s3, const QString &s4) const {
   return (indent(6) + "<tr><td>" + s1 + "</td><td>" + s2 + "</td><td>" + s3 + "</td><td>" + s4 + "</td></tr>\n");
}

// Table row HTML with 5 columns
QString US_AnalysisBase2::table_row(
...
QString US_AnalysisBase2::html_header(const QString &title, const QString &head1) const {
...
// Compose HTML header string
QString US_AnalysisBase2::html_header(const QString &title, const QString &head1, US_DataIO::EditedData *edata) const {
   QString ss = QString("<?xml version=\"1.0\"?>\n");
   ss += "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Strict//EN\"\n";
   ss += "                      \"http://www.w3.org/TR/xhtml1/DTD"
         "/xhtml1-strict.dtd\">\n";
...
QString US_AnalysisBase2::run_details(void) const {
   ...
   QString s = "\n" + indent(4) + tr("<h3>Detailed Run Information:</h3>\n") + indent(4) + "<table>\n"
               + table_row(tr("Cell Description:"), d->description) + table_row(tr("Data Directory:"), directory)
   ...
   s += table_row(tr("Wavelength:"), d->wavelength + " nm")
        + table_row(tr("Baseline ") + dataType, QString::number(calc_baseline(), 'f', 6) + " OD")
...
QString US_AnalysisBase2::hydrodynamics(void) const {
   ...
   s += table_row(tr("Viscosity corrected:"), QString::number(solution.viscosity, 'f', 5))
        ...
        + table_row(tr("Vbar:"), QString::number(solution.vbar, 'f', 4) + " ccm/g")
        + table_row(tr("Vbar corrected for 20 ") + MLDEGC + ":", QString::number(solution.vbar20, 'f', 4) + " ccm/g")
...
QString US_AnalysisBase2::analysis(const QString &extra) const {
   ...
   s += table_row(tr("Smoothing Frame:"), QString::number(( int ) ct_smoothing->value()));
   s += table_row(tr("Analyzed Boundary:"), QString::number(( int ) ct_boundaryPercent->value()) + " %");
   s += table_row(tr("Boundary Position:"), QString::number(( int ) ct_boundaryPos->value()) + " %");
   s += table_row(tr("Early Non-Cleared Scans:"), le_skipped->text() + " scans");
   s += extra;
...
void US_AnalysisBase2::load_noise(int index) {
   ...
   if (noiflags[ index ] >= 0) { // If noise already applied, ask user: retain? reselect?
   ...
   te_desc->setHtml(tr("<b>Scanning noise for %1 ...</b>").arg(triples[ index ]));
   qApp->processEvents();
   ...
   te_desc->setHtml(tr("<b>%1 noise(s) found for %2</b>").arg(nenois).arg(triples[ index ]));
   qApp->processEvents();
   ...
   if (nenois > 0) { // There is/are noise(s):  ask user if she wants to load
      ...
      if (nenois > 1 && noisdf == 0) { // Noise exists and noise-dialog flag set to "Auto-load"
         ...
         if (kenois == 2) { // Second noise from same model:  get it, too
...
bool US_AnalysisBase2::query_noise_retain() {
   QString msg = tr("Noise has previously been applied to this triple.<br/>"
...
void US_AnalysisBase2::update_filelist(QStringList &flist, const QString fname) {
   flist << fname;
...
bool US_AnalysisBase2::has_intensity_profile(const QString &runID, const bool in_db) {
   QString ripxml;
