...
US_Investigator::US_Investigator(bool signal, int inv) : US_WidgetsDialog(0, 0) {
   ...
   connect(le_search, SIGNAL(textChanged(const QString &)), SLOT(limit_names(const QString &)));
   main->addWidget(le_search, row++, 1);

   // List widget
   lw_names = us_listwidget();
   connect(lw_names, SIGNAL(itemDoubleClicked(QListWidgetItem *)), SLOT(get_inv_data(QListWidgetItem *)));
...
US_Investigator::US_Investigator(QString auto_mode, bool signal, int inv) : US_WidgetsDialog(0, 0) {
...
void US_Investigator::update(void) {
   if (le_invID->text().isEmpty()) {
      QMessageBox::information(this, tr("Attention"), tr("You must select an Investigator from the DB to update."));
      return;
   }
   ...
   if (response == QMessageBox::Ok) {
      ...
      if (db.lastErrno() != US_DB2::OK) {
         QMessageBox::warning(
            this, tr("Database Error"), tr("US_Investigator::connectDB: Could not open DB\n") + db.lastError());
         return;
      }
      ...
      if (status == US_DB2::OK) {
         QMessageBox::information(this, tr("Success"), tr("The entry was updated.\n"));
      }
      else {
         QMessageBox::warning(
            this, tr("Database Error"), tr("US_Investigator::update_person: Could not update DB\n") + db.lastError());
      }
   }
}
...
void US_Investigator::get_inv_data(QListWidgetItem *item) {
   ...
   pb_update->setEnabled(US_Settings::us_inv_level() > 2 || user_permit);
}

void US_Investigator::close(void) {
   if (signal_wanted) {
      if (le_invID->text() == "") {
         QMessageBox::information(this, tr("Attention"), tr("Select an investigator before 'Accept':\n"));
...
bool US_Investigator::check_fields(void) {
   ...
   if (missing != "") {
      QMessageBox::information(
         this, tr("Attention"), tr("Update the following fields before committing data:\n") + missing);
...
bool US_Investigator::changed(void) {
   if (le_lname->text() != info.lastName)
      return true;
   if (le_fname->text() != info.firstName)
      return true;
   if (le_address->text() != info.address)
      return true;
   if (le_city->text() != info.city)
      return true;
   if (le_state->text() != info.state)
      return true;
   if (le_zip->text() != info.zip)
      return true;
   if (le_phone->text() != info.phone)
      return true;
   if (le_email->text() != info.email)
      return true;
   if (le_org->text() != info.organization)
      return true;

   QMessageBox::information(this, tr("Attention"), tr("Nothing has changed."));
