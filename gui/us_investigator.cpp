...
void US_Investigator::update(void) {
  ...
  int response =
      QMessageBox::question(this, tr("Update the entry?"),
...
void US_Investigator::close(void) {
  ...
  if (signal_wanted) emit investigator_accepted(le_invID->text().toInt());
...
bool US_Investigator::check_fields(void) {
  ...
  if (le_city->text().replace(strip, "\\1").isEmpty()) missing += tr("\nCity");
