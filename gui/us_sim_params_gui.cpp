...
US_SimParamsGui::US_SimParamsGui(US_SimulationParameters &params) :
   ...
   connect(cnt_speeds, SIGNAL(valueChanged(double)), SLOT(update_speeds(double)));
   // Speeds combo box
   ...
   for (int i = 0; i < simparams.speed_step.size(); i++) {
      ...
      cmb_speeds->addItem(
         "Speed Profile " + QString::number(i + 1) + ": " + QString::number(spi->duration_hours) + " hr "
         + QString::number(spi->duration_minutes) + " min, " + QString::number(spi->rotorspeed) + " rpm");
   }

   main->addWidget(cmb_speeds, row++, 0, 1, 4);
   connect(cmb_speeds, SIGNAL(activated(int)), SLOT(select_speed_profile(int)));
   ...
   connect(cnt_duration_mins, SIGNAL(valueChanged(double)), SLOT(update_duration_mins(double)));
   ...
   connect(cnt_delay_mins, SIGNAL(valueChanged(double)), SLOT(update_delay_mins(double)));
   ...
   QGridLayout *acceleration_flag = us_checkbox(tr("(Check to enable)"), cb_acceleration_flag, sp->acceleration_flag);
   ...
   connect(cnt_selected_speed, SIGNAL(valueChanged(double)), SLOT(update_speed_profile(double)));
   ...
   QGridLayout *rb1 = us_radiobutton(tr("Standard Centerpiece"), rb_standard, !simparams.band_forming);

   main->addLayout(rb1, row, 4, 1, 2);

   QGridLayout *rb2 = us_radiobutton(tr("Band-forming Centerpiece"), rb_band, simparams.band_forming);

   main->addLayout(rb2, row++, 6, 1, 2);

   connect(rb_standard, SIGNAL(toggled(bool)), SLOT(select_centerpiece(bool)));

   // Band loading
   QLabel *lb_lamella = us_label(tr("Band loading volume (") + QString(QChar(181)) + "l):");
   ...
   connect(cnt_radial_res, SIGNAL(valueChanged(double)), SLOT(update_radial_res(double)));
   ...
   connect(cnt_lrnoise, SIGNAL(valueChanged(double)), SLOT(update_lrnoise(double)));
   ...
   connect(cnt_tinoise, SIGNAL(valueChanged(double)), SLOT(update_tinoise(double)));
   ...
   connect(cnt_baseline, SIGNAL(valueChanged(double)), SLOT(update_baseline(double)));
   ...
   connect(cnt_temperature, SIGNAL(valueChanged(double)), SLOT(update_temp(double)));
...
void US_SimParamsGui::check_delay(void) {
   ...
   for (int i = 0; i < steps; i++) {
      ...
      int lower_limit = 1 + (abs((speed[ i + 1 ] - speed[ i ])) + 1) / ss->acceleration;
   ...
   if (sp->delay_hours == hours[ current_speed_step ] && sp->delay_minutes < mindmin) {
   ...
   else if (sp->duration_hours == 0 && sp->duration_minutes > sp->delay_minutes + 1) {
...
void US_SimParamsGui::save(void) {
   ...
   if (fn.endsWith(".")) { // ending with '.' signals no ".xml" to be added
      fn = fn.left(fn.length() - 1);
      fnam = fnam.left(fnam.length() - 1);
   }

   else if (!fn.endsWith(".xml")) { // if no .xml extension, add one
      fn = fn + ".xml";
      fnam = fnam + ".xml";
   }

   if (fnam.startsWith(".")) { // starting with '.' signals no "sp_" prefix
      fn = fdir + fnam.mid(1);
   }

   else if (!fnam.startsWith("sp_")) { // if no sp_ prefix, add one
      fn = fdir + "sp_" + fnam;
   }
