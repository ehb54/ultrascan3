...
US_SimParamsGui::US_SimParamsGui (US_SimulationParameters &params)
    : US_WidgetsDialog (nullptr, Qt::WindowFlags ()), simparams (params)
{
  ...
  for (int i = 0; i < simparams.speed_step.size (); i++)
    {
      ...
      cmb_speeds->addItem ("Speed Profile " + QString::number (i + 1) + ": "
  ...
  QGridLayout *acceleration_flag = us_checkbox (
      tr ("(Check to enable)"), cb_acceleration_flag, sp->acceleration_flag);
  ...
  connect (cmb_mesh, SIGNAL (activated (int)), SLOT (update_mesh (int)));

  // Right Column
  row = 1;
  // Centerpiece

  QGridLayout *rb1 = us_radiobutton (tr ("Standard Centerpiece"), rb_standard,
                                     !simparams.band_forming);

  main->addLayout (rb1, row, 4, 1, 2);

  QGridLayout *rb2 = us_radiobutton (tr ("Band-forming Centerpiece"), rb_band,
                                     simparams.band_forming);
  ...
  // Temperature
  QLabel *lb_temperature = us_label (tr ("Temperature (%1):").arg (DEGC));
  ...
  connect (cmb_moving, SIGNAL (activated (int)), SLOT (update_moving (int)));
...
bool
US_SimParamsGui::load_params (const QString &load_init,
...
void
US_SimParamsGui::backup_parms (void)
{
  ...
  for (int i = 0; i < simparams.speed_step.size (); i++)
    {
      ...
      US_SimulationParameters::SpeedProfile *ssbu
          = &simparams_backup.speed_step[i];
...
void
US_SimParamsGui::update_speeds (double value)
{
  ...
  for (int ii = old_size; ii < new_size; ii++)
    {
      ...
      US_SimulationParameters::SpeedProfile *ss_old
          = &simparams.speed_step[lx];
...
void
US_SimParamsGui::update_combobox (void)
{
...
void
US_SimParamsGui::update_speed_profile (double profile)
{
  select_speed_profile ((int)profile - 1);
}

void
US_SimParamsGui::select_speed_profile (int index)
{
...
void
US_SimParamsGui::check_delay (void)
{
  QVector<int> hours;
  ...
  for (int i = 0; i < steps; i++)
    {
      ...
      int lower_limit
          = 1 + (abs ((speed[i + 1] - speed[i])) + 1) / ss->acceleration;
  ...
  US_SimulationParameters::SpeedProfile *sp
      = &simparams.speed_step[current_speed_step];
...
void
US_SimParamsGui::update_duration_hours (double hours)
{
  US_SimulationParameters::SpeedProfile *sp
      = &simparams.speed_step[current_speed_step];
...
void
US_SimParamsGui::update_duration_mins (double minutes)
{
  US_SimulationParameters::SpeedProfile *sp
      = &simparams.speed_step[current_speed_step];
...
void
US_SimParamsGui::update_delay_hours (double hours)
{
  US_SimulationParameters::SpeedProfile *sp
      = &simparams.speed_step[current_speed_step];
...
void
US_SimParamsGui::update_delay_mins (double minutes)
{
  US_SimulationParameters::SpeedProfile *sp
      = &simparams.speed_step[current_speed_step];
...
void
US_SimParamsGui::update_rotorspeed (double speed)
{
  US_SimulationParameters::SpeedProfile *sp
      = &simparams.speed_step[current_speed_step];
...
void
US_SimParamsGui::acceleration_flag (void)
{
  US_SimulationParameters::SpeedProfile *sp
      = &simparams.speed_step[current_speed_step];
...
void
US_SimParamsGui::update_acceleration (double accel)
{
  US_SimulationParameters::SpeedProfile *sp
      = &simparams.speed_step[current_speed_step];
...
void
US_SimParamsGui::update_scans (double scans)
{
  US_SimulationParameters::SpeedProfile *sp
      = &simparams.speed_step[current_speed_step];
  sp->scans = (int)scans;
...
void
US_SimParamsGui::save (void)
{
  QString fn = QFileDialog::getSaveFileName (
      this, tr ("Save Simulation Parameters in:"), US_Settings::etcDir (),
...
void
US_SimParamsGui::load (void)
{
  QString fn = QFileDialog::getOpenFileName (
      this, tr ("Load Simulation Parameters from:"), US_Settings::etcDir (),
      tr ("SimulationParameters files (sp_*.xml);;"
          "All XML files (*.xml);;"
          "All files (*)"));
  ...
  if (simparams.load_simparms (fn) == 0)
    {
      ...
      for (int i = 0; i < steps; i++)
        {
          US_SimulationParameters::SpeedProfile *spi
              = &simparams.speed_step[i];

          cmb_speeds->addItem (
              "Speed Profile " + QString::number (i + 1) + ": "
              + QString::number (spi->duration_hours) + " hr "
              + QString::number (spi->duration_minutes) + " min, "
              + QString::number (spi->rotorspeed) + " rpm");
        }
      ...
      le_status->setText (tr ("Settings were loaded from:  ") + fn);
    }
...
void
US_SimParamsGui::update_mesh (int mesh)
{
...
void
US_SimParamsGui::update_lamella (double lamella)
{
...
void
US_SimParamsGui::update_meniscus (double meniscus)
{
  double rad_precis = simparams.radial_resolution * 0.1;
  simparams.meniscus = qRound (meniscus / rad_precis) * rad_precis;
  report_mods ();
}

void
US_SimParamsGui::update_bottom (double bottom)
{
  double rad_precis = simparams.radial_resolution * 0.1;
  simparams.bottom = qRound (bottom / rad_precis) * rad_precis;
  simparams.bottom_position = bottom;
  report_mods ();
}

void
US_SimParamsGui::update_simpoints (double simpoints)
{
...
void
US_SimParamsGui::update_radial_res (double radial_res)
{
...
void
US_SimParamsGui::update_rnoise (double rnoise)
{
...
void
US_SimParamsGui::update_lrnoise (double lrnoise)
{
...
void
US_SimParamsGui::update_tinoise (double tinoise)
{
...
void
US_SimParamsGui::update_rinoise (double rinoise)
{
...
void
US_SimParamsGui::update_baseline (double baseline)
{
...
void
US_SimParamsGui::update_moving (int grid)
{
...
void
US_SimParamsGui::select_centerpiece (bool)
{
...
void
US_SimParamsGui::update_temp (double temp)
{
...
void
US_SimParamsGui::disconnect_all ()
{
