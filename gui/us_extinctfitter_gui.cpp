...
void US_ExtinctFitter::saveFit() {
  ...
  message.setText(
      tr("Please use the save function from\nthe main extinction "
         "calculation\nmodule. It will allow you to save the ]ncoefficients of "
         "the fit as well as a\ncopy of the plot window."));
  message.exec();
}
            ...
            (pow(guess[(*wls_v).size() + (3 * k) + 2], -3) *
             (pow(guess[(*wls_v).size() + (3 * k) + 1], 2) -
              2 * guess[(*wls_v).size() + (3 * k) + 1] *
                  (*wls_v).at(i).v_readings.at(j).lambda +
              pow((*wls_v).at(i).v_readings.at(j).lambda, 2)));
      }
      point_counter++;
    }
  }
...
void US_ExtinctFitter::plot_overlays() {
  ...
  if (plotGroup) {
    if (datasets - firstScan == 0) {
      numScans = 1;
      s1.sprintf((tr("Overlays for fitted Scan %ld")).toLatin1().data(),
                 firstScan);
    } else if (datasets - firstScan < 5) {
      numScans = datasets - firstScan + 1;
      s1.sprintf((tr("Overlays for fitted Scans %ld - %ld")).toLatin1().data(),
...
void US_ExtinctFitter::plot_residuals() {
  ...
  if (plotGroup) {
    if (datasets - firstScan == 0) {
      numScans = 1;
      s1.sprintf((tr("Residuals from fitted Scan %ld")).toLatin1().data(),
                 firstScan);
    } else if (datasets - firstScan < 5) {
      numScans = datasets - firstScan + 1;
      s1.sprintf(
          (tr("Residuals from fitted Scans %ld - %ld")).toLatin1().data(),
          firstScan, firstScan + numScans - 1);
    } else {
      numScans = 5;
      s1.sprintf(
