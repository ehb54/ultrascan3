...
/*
US_ExtinctFitter::US_ExtinctFitter(QVector <struct WavelengthScan> *temp_wls_v,
double*& temp_guess, unsigned int& temp_order, unsigned int& temp_parameters,
QString& temp_projectName, bool *temp_fitting_widget, bool bufferfit) :
US_Minimize(temp_fitting_widget, true)
{
   guess = temp_guess;
   ...
   //pb_pause->hide();                // Test - edit what to show/hide,
depending on the General/Buffer/Analyte Global Fit
   //pb_resume->hide();
...
US_ExtinctFitter::US_ExtinctFitter (QVector<struct WavelengthScan> *temp_wls_v,
                                    ...
                                    unsigned int &temp_parameters,
                                    QString &temp_projectName,
                                    bool *temp_fitting_widget, bool auto_mode)
    : US_Minimize (temp_fitting_widget, true)
{
  guess = temp_guess;
  parameters = temp_parameters;
...
void
US_ExtinctFitter::saveFit ()
{
  QMessageBox message;
  message.setWindowTitle (tr ("Extinction Fitter:"));
  message.setText (
      tr ("Please use the save function from\nthe main extinction "
          "calculation\nmodule. It will allow you to save the ]ncoefficients "
          "of the fit as well as a\ncopy of the plot window."));
  message.exec ();
}

void
US_ExtinctFitter::startFit ()
{
  if (autoconverge)
    {
      iteration = 1000;
      while (iteration > 1)
        {
          ...
          if (nlsMethod
              == 3) // switch between Levenberg Marquardt and Quasi Newton
            {
              nlsMethod = 0;
              if (GUI)
                {
...
bool
US_ExtinctFitter::fit_init ()
{

  int i, point_counter;
  //
  // Calculate how many points there are in each dataset, sum them up for
  // "points" and leave out datasets that aren't fitted to keep the
  // vectors/matrices as small as possible.
  //
  points = 0;
  datasets = (*wls_v).size ();
...
int
US_ExtinctFitter::calc_model (double *guess_par)
{
  QString str;
      ...
      for (j = 0; j < points_per_dataset[i]; j++)
        {
          ...
          for (k = 0; k < order; k++)
            {
              gaussian += exp (
                  guess_par[(*wls_v).size () + (3 * k)]
                  - (pow (((*wls_v).at (i).v_readings.at (j).lambda
                           - guess_par[(*wls_v).size () + (3 * k) + 1]),
                          2)
                     / (2
                        * pow (guess_par[(*wls_v).size () + (3 * k) + 2],
                               2))));
            }
  ...
  /* //Polynomial fit:
     ...
     for (k=0; k<order; k++)
     {
     polynomial += guess_par[(*wls_v).size() + k] * pow((*wls_v)[i].lambda[j],
  k);
     }
     y_guess[point_counter] = guess_par[i] * polynomial;
     //qDebug() << "dataset: " << i << ", point: " << j << ", guessed y-value:
  " << y_guess[point_counter]  ; point_counter++;
     }
     }
     function_evaluations++;
...
int
US_ExtinctFitter::calc_jacobian ()
{
      ...
      for (j = 0; j < points_per_dataset[i]; j++)
        {
          for (k = 0; k < order; k++)
            {
              term[k] = exp (
                  guess[(*wls_v).size () + (3 * k)]
                  - (pow (((*wls_v).at (i).v_readings.at (j).lambda
                           - guess[(*wls_v).size () + (3 * k) + 1]),
                          2)
          ...
          for (k = 0; k < order; k++)
            {
              ...
              jacobian[point_counter][(*wls_v).size () + (3 * k)]
                  = guess[i] * term[k];

              jacobian[point_counter][(*wls_v).size () + (3 * k) + 1]
                  = guess[i] * term[k]
                    * (((*wls_v).at (i).v_readings.at (j).lambda
                        - guess[(*wls_v).size () + (3 * k) + 1])
                       / pow (guess[(*wls_v).size () + (3 * k) + 2], 2));

              jacobian[point_counter][(*wls_v).size () + (3 * k) + 2]
                  = guess[i] * term[k]
  ...
  /* Jacobian for polynomial fit:
     ...
     // y[i][j] = guess[i] * (a[0] + a[1] * x[i][j] + a[2] * x[i][j]^2 + ... +
  a[n] * x[i][j]^n) for (i=0; i<(*wls_v).size(); i++)
     {
     for (j=0; j<points_per_dataset[i]; j++)
     {
     polynomial = 0.0;
     ...
     // dy/dguess[i] = (a[0] + a[1] * x[i][j] + a[2] * x[i][j]^2 + ... + a[n] *
  x[i][j]^n) jacobian[point_counter][i] = polynomial;
     // dy/da[n] = guess[i] * x[i][j]^n
     jacobian[point_counter][(*wls_v).size()] = guess[i];
     for (k=1; k<order; k++)
     {
     jacobian[point_counter][(*wls_v).size() + k] = guess[i] *
  pow((*wls_v)[i].lambda[j], k);
     }
     point_counter++;
     }
     }
...
void
US_ExtinctFitter::view_report ()
{
  /*
     QString fileName, str;
     fileName = USglobal->config_list.result_dir + "/" + projectName +
     "-extinction.res";
     //view_file(fileName);
     TextEdit *e;
     e = new TextEdit();
     e->setPalette(QPalette(USglobal->global_colors.cg_normal,
     USglobal->global_colors.cg_normal, USglobal->global_colors.cg_normal));
...
void
US_ExtinctFitter::write_report ()
{
}

void
US_ExtinctFitter::plot_overlays ()
{
      ...
      if (datasets - firstScan == 0)
        {
          numScans = 1;
          s1.sprintf (
              (tr ("Overlays for fitted Scan %ld")).toLatin1 ().data (),
              firstScan);
        }
      else if (datasets - firstScan < 5)
        {
          ...
          s1.sprintf (
              (tr ("Overlays for fitted Scans %ld - %ld")).toLatin1 ().data (),
              firstScan, firstScan + numScans - 1);
        }
      else
        {
          numScans = 5;
  ...
  // QwtSymbol* symbol = new
  // QwtSymbol(QwtSymbol::Diamond,QBrush(Qt::red),QPen(Qt::red),QSize(5,5));
  ...
  //    data_plot->setAxisScale(QwtPlot::xBottom, -xmax/30.0, xmax + xmax/30.0,
  //    0);
  data_plot->replot ();
...
void
US_ExtinctFitter::plot_residuals ()
{
      ...
      if (datasets - firstScan == 0)
        {
          numScans = 1;
          s1.sprintf (
              (tr ("Residuals from fitted Scan %ld")).toLatin1 ().data (),
              firstScan);
        }
      else if (datasets - firstScan < 5)
        {
          numScans = datasets - firstScan + 1;
          s1.sprintf ((tr ("Residuals from fitted Scans %ld - %ld"))
                          .toLatin1 ()
                          .data (),
                      firstScan, firstScan + numScans - 1);
        }
...
void
US_ExtinctFitter::updateRange (double scan)
{
