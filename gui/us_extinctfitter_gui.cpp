...
US_ExtinctFitter::US_ExtinctFitter(
   QVector<struct WavelengthScan> *temp_wls_v, double *&temp_guess, unsigned int &temp_order,
   unsigned int &temp_parameters, QString &temp_projectName, bool *temp_fitting_widget) :
...
US_ExtinctFitter::US_ExtinctFitter(
   QVector<struct WavelengthScan> *temp_wls_v, double *&temp_guess, unsigned int &temp_order,
   unsigned int &temp_parameters, QString &temp_projectName, bool *temp_fitting_widget, bool auto_mode) :
    US_Minimize(temp_fitting_widget, true) {
   guess = temp_guess;
   parameters = temp_parameters;
   order = temp_order;
               ...
               = guess[ i ] * term[ k ]
                 * (pow(guess[ (*wls_v).size() + (3 * k) + 2 ], -3)
                    * (pow(guess[ (*wls_v).size() + (3 * k) + 1 ], 2)
                       - 2 * guess[ (*wls_v).size() + (3 * k) + 1 ] * (*wls_v).at(i).v_readings.at(j).lambda
