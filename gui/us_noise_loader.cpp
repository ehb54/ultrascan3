...
US_NoiseLoader::US_NoiseLoader(
   US_DB2 *db, QStringList &mieGUIDs, QStringList &nieGUIDs, US_Noise &ti_noise, US_Noise &ri_noise,
   US_DataIO::EditedData *edata) :
    US_WidgetsDialog(0, 0), db(db), mieGUIDs(mieGUIDs), nieGUIDs(nieGUIDs), ti_noise(ti_noise), ri_noise(ri_noise) {
   setWindowTitle((db == ( US_DB2 * ) 0 ? tr("Noise Vector Local Disk Load") : tr("Noise Vector Database Load")));
   ...
   QTreeWidgetItem *twi_curr = new QTreeWidgetItem(twi_null, QStringList(mititle));
   items.append(twi_curr); // first top-level item in tree

   for (int ii = 1; ii < mieGUIDs.size(); ii++) { // complete models list for top level
   ...
   for (int ii = 0; ii < nieGUIDs.size(); ii++) { // review noises-in-edit, adding children
   ...
   if (nieGUIDs.at(0).section(":", 2, 2).toInt() == 0) { // expand the loaded model tree if present
      tw_noises->expandItem(items.at(0));
   }

   connect(tw_noises, SIGNAL(itemSelectionChanged()), this, SLOT(itemsSelected()));
   connect(pb_detail, SIGNAL(clicked()), this, SLOT(view_details()));
   connect(pb_cancel, SIGNAL(clicked()), this, SLOT(cancelled()));
   connect(pb_load, SIGNAL(clicked()), this, SLOT(selected()));
...
void US_NoiseLoader::itemsSelected(void) {
   ...
   for (int ii = 0; ii < nsels; ii++) {
      ...
      if (itemtext.contains("_noise")) { // Select an individual noise
         lw_selects->addItem(itemtext);
      }

      else if (itemtext.contains("Model")) { // Select all the noises associated with a model
...
void US_NoiseLoader::selected() {
   ...
   if (darange > 0.0) {
      ...
      if (nurnois > 0) {
         QString msg = tr("Noise values exceed the range of experimental data.<br/>"
...
void US_NoiseLoader::view_details() {
   ...
   for (int ii = 0; ii < nieGUIDs.size(); ii++) { // add text for each noise vector entry
      ...
      mtxt += tr("  Values Count:   ") + QString::number(noise.count) + "\n";

      if (typ == "ti") {
         mtxt += tr("  Minimum Radius: ") + QString::number(noise.minradius) + "\n";
         mtxt += tr("  Maximum Radius: ") + QString::number(noise.maxradius) + "\n";
      }
   }
