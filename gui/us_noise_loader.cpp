...
US_NoiseLoader::US_NoiseLoader(US_DB2* db, QStringList& mieGUIDs,
                               QStringList& nieGUIDs, US_Noise& ti_noise,
                               US_Noise& ri_noise, US_DataIO::EditedData* edata)
    : US_WidgetsDialog(0, 0),
      ...
      ti_noise(ti_noise),
      ri_noise(ri_noise) {
  setWindowTitle((db == (US_DB2*)0 ? tr("Noise Vector Local Disk Load")
  ...
  for (int ii = 1; ii < mieGUIDs.size();
       ii++) {  // complete models list for top level
  ...
  for (int ii = 0; ii < nieGUIDs.size();
       ii++) {                      // review noises-in-edit, adding children
  ...
  if (nieGUIDs.at(0).section(":", 2, 2).toInt() ==
      0) {  // expand the loaded model tree if present
    tw_noises->expandItem(items.at(0));
  }

  connect(tw_noises, SIGNAL(itemSelectionChanged()), this,
          SLOT(itemsSelected()));
  ...
  for (int ii = 0; ii < nsels; ii++) {
    ...
    if (itemtext.contains("_noise")) {  // Select an individual noise
      lw_selects->addItem(itemtext);
                 ...
                 "Model")) {  // Select all the noises associated with a model
...
void US_NoiseLoader::cancelled() { close_all(); }
...
void US_NoiseLoader::selected() {
  ...
  for (int ii = 0; ii < lw_selects->count(); ii++) {  // browse select list
    ...
    for (int jj = 0; jj < nieGUIDs.size();
         jj++) {  // examine noises-in-edit list
      QString nie = nieGUIDs.at(jj);
      QString typ = nie.section(":", 1, 1);
      QString mdx = nie.section(":", 2, 2);

      if (typ == styptext &&
          mdx == snumtext) {  // found the selected noise: break with its GUID
  ...
  if (darange > 0.0) {
    ...
    if (nurnois > 0) {
      QString msg =
          tr("Noise values exceed the range of experimental data.<br/>"
...
void US_NoiseLoader::view_details() {
  ...
  for (int ii = 0; ii < nieGUIDs.size();
       ii++) {  // add text for each noise vector entry
    ...
    if (typ == "ti") {
      mtxt +=
          tr("  Minimum Radius: ") + QString::number(noise.minradius) + "\n";
      mtxt +=
          tr("  Maximum Radius: ") + QString::number(noise.maxradius) + "\n";
    }
  }
