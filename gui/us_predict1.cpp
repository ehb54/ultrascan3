...
US_Predict1::US_Predict1(US_Hydrosim &parm, const US_Analyte a_data, int disk_access, bool signal_wanted) :
    US_WidgetsDialog(0, 0), allparams(parm), base_analyte(a_data), access(disk_access), signal(signal_wanted) {
   ...
   if (signal) {
      ...
      connect(le_density, SIGNAL(textChanged(const QString &)), SLOT(density(const QString &)));
      controls->addWidget(le_density, c_row++, 1);
      ...
      connect(le_viscosity, SIGNAL(textChanged(const QString &)), SLOT(viscosity(const QString &)));
      controls->addWidget(le_viscosity, c_row++, 1);
   }
   ...
   connect(le_vbar, SIGNAL(textChanged(const QString &)), SLOT(vbar(const QString &)));
   ...
   QLabel *lb_temperature = us_label(tr("Temperature (") + DEGC + tr("):"));
   ...
   if (signal) {
      ...
      connect(le_temperature, SIGNAL(textChanged(const QString &)), SLOT(degC(const QString &)));
   }
   ...
   connect(le_axial, SIGNAL(editingFinished(void)), SLOT(update_ratio(void)));
   ...
   connect(le_max_x, SIGNAL(textChanged(const QString &)), SLOT(update_max_x(const QString &)));
   controls->addWidget(le_max_x, c_row++, 1);

   // Information
   QPalette p = US_GuiSettings::editColor();

   lb_info = us_banner(tr("Please select an axial ratio by\n"
   ...
   // Graph
   plotLayout = new US_Plot(plot, tr("f/f0 Dependence on Axial Ratios"), tr("Axial Ratio"), tr("f / f0"));
   ...
   connect(pick, SIGNAL(mouseDown(const QwtDoublePoint &)), SLOT(new_value(const QwtDoublePoint &)));
...
void US_Predict1::update_plot(void) {
   ...
   for (int i = 0; i < ARRAYSIZE; i++, x += max_x / ARRAYSIZE) {
      prolate[ i ] = pow(x, -1.0 / 3.0) * sqrt(sq(x) - 1.0) / log(x + sqrt(sq(x) - 1.0));

      oblate[ i ] = sqrt(sq(x) - 1.0) / (pow(x, 2.0 / 3.0) * atan(sqrt(sq(x) - 1.0)));

      rod[ i ] = pow(2.0 / 3.0, 1.0 / 3.0) * pow(x, 2.0 / 3.0) / (log(2.0 * x) - 0.3);
...
void US_Predict1::get_peptide(void) {
   US_AnalyteGui *dialog = new US_AnalyteGui(true, analyte.analyteGUID);

   connect(dialog, SIGNAL(valueChanged(US_Analyte)), SLOT(update_vbar(US_Analyte)));
...
void US_Predict1::get_buffer(void) {
   ...
   connect(dialog, SIGNAL(valueChanged(US_Buffer)), SLOT(update_buffer(US_Buffer)));
...
void US_Predict1::calc_column(
   const QString &name, const QString &unit, int column_index, double sphere_val, double prolate_val, double oblate_val,
   double rod_val, double multiplier) {
   ...
   if (power_of_ten != 0) {
      titles[ column_index ]->setText(
         name + " (" + QString("e%1%2").arg(power_of_ten > 0 ? '+' : '-').arg(qFabs(power_of_ten), 2, 'f', 0, '0')
         + ((unit.isEmpty()) ? "" : " " + unit) + ")");
   }
   lb_sphere[ column_index ]->setText(
      QString::number(sphere_val * multiplier * ( double ) pow(10, power_of_ten), format_char, 4));
   lb_prolate[ column_index ]->setText(
      QString::number(prolate_val * multiplier * ( double ) pow(10, power_of_ten), format_char, 4));
   lb_oblate[ column_index ]->setText(
      QString::number(oblate_val * multiplier * ( double ) pow(10, power_of_ten), format_char, 4));
   lb_rod[ column_index ]->setText(
      QString::number(rod_val * multiplier * ( double ) pow(10, power_of_ten), format_char, 4));
}
...
void US_Predict1::update() {
   ...
   calc_column(
      "s<sub>app</sub>", "S", 1, allparams.sphere.s, allparams.prolate.s, allparams.oblate.s, allparams.rod.s,
      1e+13 / solution.s20w_correction);
   calc_column(
      "D<sub>app</sub>", "cm<sup>2</sup>/sec", 2, allparams.sphere.D, allparams.prolate.D, allparams.oblate.D,
      allparams.rod.D, 1 / solution.D20w_correction);
   calc_column("f", "", 3, allparams.sphere.f, allparams.prolate.f, allparams.oblate.f, allparams.rod.f, 1);
   calc_column(
      "f / f0", "", 4, allparams.sphere.f_f0, allparams.prolate.f_f0, allparams.oblate.f_f0, allparams.rod.f_f0, 1);
   calc_column(
      "a", "<span>&Aring;</span>", 5, allparams.sphere.a, allparams.prolate.a, allparams.oblate.a, allparams.rod.a, 1);
   calc_column(
      "b", "<span>&Aring;</span>", 6, allparams.sphere.b, allparams.prolate.b, allparams.oblate.b, allparams.rod.b, 1);
   calc_column(
      "Volume", "<span>&Aring;<sup>3</sup></span>", 7, allparams.sphere.volume, allparams.prolate.volume,
      allparams.oblate.volume, allparams.rod.volume, 1);
   calc_column(
      "s<sub>20,w</sub>", "S", 8, allparams.sphere.s, allparams.prolate.s, allparams.oblate.s, allparams.rod.s, 1e+13);
   calc_column(
      "D<sub>20,w</sub>", "cm<sup>2</sup>/sec", 9, allparams.sphere.D, allparams.prolate.D, allparams.oblate.D,
      allparams.rod.D, 1);
...
void US_Predict1::get_solution(void) {
   ...
   connect(dialog, SIGNAL(updateSolutionGuiSelection(US_Solution)), SLOT(update_solution(US_Solution)));
...
void US_Predict1::update_solution(US_Solution soln) {
   ...
   if (soln.analyteInfo.size() == 0) {
      QMessageBox::warning(this, tr("No Analyte"), tr("There is no analyte in the solution"));
      return;
   }
      ...
      connect(dialog, SIGNAL(choice(int)), SLOT(choose(int)));
      dialog->exec();
      qApp->processEvents();
