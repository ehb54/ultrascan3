...
US_Predict1::US_Predict1 (US_Hydrosim &parm, const US_Analyte a_data,
                          int disk_access, bool signal_wanted)
    : US_WidgetsDialog (0, 0), allparams (parm), base_analyte (a_data),
      access (disk_access), signal (signal_wanted)
{
  ...
  if (signal)
    {
      ...
      le_viscosity
          = us_lineedit (QString::number (solution.viscosity, 'f', 4));
  ...
  // Graph
  plotLayout = new US_Plot (plot, tr ("f/f0 Dependence on Axial Ratios"),
                            tr ("Axial Ratio"), tr ("f / f0"));
...
void
US_Predict1::update_mw (const QString &s)
{
  mw = s.toDouble ();
  update ();
}
...
void
US_Predict1::update_max_x (const QString &s)
{
  max_x = s.toDouble ();
  update ();
  update_plot ();
...
void
US_Predict1::degC (const QString &s)
{
...
void
US_Predict1::update_vbar (const US_Analyte ad)
{
  analyte = ad;
  mw = ad.mw;
...
void
US_Predict1::update_buffer (const US_Buffer b)
{
  buffer = b;
...
void
US_Predict1::density (const QString &s)
{
...
void
US_Predict1::viscosity (const QString &s)
{
...
void
US_Predict1::vbar (const QString &s)
{
...
void
US_Predict1::new_value (const QwtDoublePoint &p)
{
  ratio = p.x ();
...
void
US_Predict1::mouseU (const QwtDoublePoint &p)
{
  ratio = p.x ();
...
void
US_Predict1::calc_column (const QString &name, const QString &unit,
                          int column_index, double sphere_val,
                          double prolate_val, double oblate_val,
                          double rod_val, double multiplier)
{
  double min_value
      = min (min (sphere_val, prolate_val), min (oblate_val, rod_val))
        * multiplier;
  double max_value
      = max (max (sphere_val, prolate_val), max (oblate_val, rod_val))
        * multiplier;
  ...
  if (qFabs (max_value * multiplier * (double)pow (10, power_of_ten)) < 1)
    {
      power_of_ten
          += (power_of_ten != 0) ? power_of_ten / abs (power_of_ten) : 1;
    }

  titles[column_index]->setText (
      name + ((unit.isEmpty ()) ? "" : " (" + unit + ")"));
  ...
  if (power_of_ten != 0)
    {
      titles[column_index]->setText (
          ...
          + QString ("e%1%2")
                .arg (power_of_ten > 0 ? '+' : '-')
                .arg (qFabs (power_of_ten), 2, 'f', 0, '0')
          + ((unit.isEmpty ()) ? "" : " " + unit) + ")");
    }
  lb_sphere[column_index]->setText (QString::number (
      sphere_val * multiplier * (double)pow (10, power_of_ten), format_char,
      4));
  lb_prolate[column_index]->setText (QString::number (
      prolate_val * multiplier * (double)pow (10, power_of_ten), format_char,
      4));
  lb_oblate[column_index]->setText (QString::number (
      oblate_val * multiplier * (double)pow (10, power_of_ten), format_char,
      4));
  lb_rod[column_index]->setText (QString::number (
      rod_val * multiplier * (double)pow (10, power_of_ten), format_char, 4));
}
...
US_Predict1::update ()
{
  ...
  calc_column ("s<sub>app</sub>", "S", 1, allparams.sphere.s,
               allparams.prolate.s, allparams.oblate.s, allparams.rod.s,
               1e+13 / solution.s20w_correction);
  calc_column ("D<sub>app</sub>", "cm<sup>2</sup>/sec", 2, allparams.sphere.D,
               allparams.prolate.D, allparams.oblate.D, allparams.rod.D,
               1 / solution.D20w_correction);
  calc_column ("f", "", 3, allparams.sphere.f, allparams.prolate.f,
               allparams.oblate.f, allparams.rod.f, 1);
  calc_column ("f / f0", "", 4, allparams.sphere.f_f0, allparams.prolate.f_f0,
               allparams.oblate.f_f0, allparams.rod.f_f0, 1);
  calc_column ("a", "<span>&Aring;</span>", 5, allparams.sphere.a,
               allparams.prolate.a, allparams.oblate.a, allparams.rod.a, 1);
  calc_column ("b", "<span>&Aring;</span>", 6, allparams.sphere.b,
               allparams.prolate.b, allparams.oblate.b, allparams.rod.b, 1);
  calc_column ("Volume", "<span>&Aring;<sup>3</sup></span>", 7,
               allparams.sphere.volume, allparams.prolate.volume,
               allparams.oblate.volume, allparams.rod.volume, 1);
  calc_column ("s<sub>20,w</sub>", "S", 8, allparams.sphere.s,
               allparams.prolate.s, allparams.oblate.s, allparams.rod.s,
               1e+13);
  calc_column ("D<sub>20,w</sub>", "cm<sup>2</sup>/sec", 9, allparams.sphere.D,
               allparams.prolate.D, allparams.oblate.D, allparams.rod.D, 1);
...
void
US_Predict1::source_changed (bool db)
{
  emit use_db (db);
  qApp->processEvents ();
}
...
void
US_Predict1::update_solution (US_Solution soln)
{
  ...
  if (soln.analyteInfo.size () == 0)
    {
      QMessageBox::warning (this, tr ("No Analyte"),
