...
US_Predict1::US_Predict1(US_Hydrosim& parm, const US_Analyte a_data,
  ...
  lb_info =
      us_banner(tr("Please select an axial ratio by\n"
...
void US_Predict1::update_plot(void) {
  ...
  for (int i = 0; i < ARRAYSIZE; i++, x += max_x / ARRAYSIZE) {
    prolate[i] =
        pow(x, -1.0 / 3.0) * sqrt(sq(x) - 1.0) / log(x + sqrt(sq(x) - 1.0));

    oblate[i] =
        sqrt(sq(x) - 1.0) / (pow(x, 2.0 / 3.0) * atan(sqrt(sq(x) - 1.0)));

    rod[i] =
        pow(2.0 / 3.0, 1.0 / 3.0) * pow(x, 2.0 / 3.0) / (log(2.0 * x) - 0.3);
...
void US_Predict1::calc_column(const QString& name, const QString& unit,
                              int column_index, double sphere_val,
                              double prolate_val, double oblate_val,
                              double rod_val, double multiplier) {
  double min_value =
      min(min(sphere_val, prolate_val), min(oblate_val, rod_val)) * multiplier;
  double max_value =
      max(max(sphere_val, prolate_val), max(oblate_val, rod_val)) * multiplier;
  ...
  titles[column_index]->setText(name +
                                ((unit.isEmpty()) ? "" : " (" + unit + ")"));
  ...
  lb_sphere[column_index]->setText(QString::number(
      sphere_val * multiplier * (double)pow(10, power_of_ten), format_char, 4));
  lb_prolate[column_index]->setText(
      QString::number(prolate_val * multiplier * (double)pow(10, power_of_ten),
                      format_char, 4));
  lb_oblate[column_index]->setText(QString::number(
      oblate_val * multiplier * (double)pow(10, power_of_ten), format_char, 4));
...
void US_Predict1::update() {
  ...
  calc_column("s<sub>20,w</sub>", "S", 8, allparams.sphere.s,
              allparams.prolate.s, allparams.oblate.s, allparams.rod.s, 1e+13);
...
void US_Predict1::choose(int value) { analyte_number = value; }
