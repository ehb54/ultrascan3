...
bool US_BufferGuiSelect::load_buffer(const QString& load_init,
                                     US_Buffer& buffer) {
  bool loaded = false;
...
void US_BufferGuiSelect::show_component(const QString& componentID,
                                        double conc) {
...
void US_BufferGuiSelect::read_from_disk(QListWidgetItem* item) {
  ...
  if (!buffer->readFromDisk(filenames[buf])) qDebug() << "read failed";
...
void US_BufferGuiSelect::search(QString const& text) {
  ...
  for (int ii = 0; ii < descriptions.size();
       ii++) {  // get list of filtered-description + index strings
  ...
  for (int jj = 0; jj < sortdesc.size();
       jj++) {  // build list of sorted meta data and ListWidget entries
    int ii = sortdesc[jj].section(sep, 1, 1).toInt();

    if (ii < 0 || ii >= dsize) {
      DbgLv(1) << "BufS-search:  *ERROR* ii" << ii << "jj" << jj << "sdesc"
...
void US_BufferGuiSelect::accept(void) { set_temp20(); }
...
void US_BufferGuiSelect::set_temp20() { sl_temp->setValue(20); }
...
US_BufferGuiNew::US_BufferGuiNew(int* invID, int* select_db_disk,
  ...
  QGridLayout* lo_manual =
      us_checkbox(tr("Manual unadjusted Density and Viscosity"), ck_manual);

  QLabel* bn_newbuf = us_banner(tr("Specify a new buffer to add"), -1);
  QLabel* lb_descrip = us_label(tr("Description:"));
  lb_bselect =
      us_label(tr("Please enter the concentration of\n"
                  " (component) in mM:"));
  ...
  if (false && US_Settings::us_inv_level() >
                   1) {  // "create new" disabled - see issue #275
...
void US_BufferGuiNew::select_water(QListWidgetItem* item) {
  ...
  if (lw_bufcomps->count() > 0) {
    QMessageBox::warning(this, tr("Warning!"),
                         tr("Please remove all the selected buffer components "
                            "before choosing [ Water (H2O) ]!"));
    return;
  }
...
void US_BufferGuiNew::add_component() {
  ...
  if (lw_bufcomps->count() > 0 && lw_bufcomps->item(0)->type() < 0) {
    QMessageBox::warning(this, tr("Warning!"),
                         tr("Please remove the water component before choosing "
  ...
  for (int ii = 0; ii < buffer->component.size(); ii++) {
    if (bcomp.name ==
        buffer->component[ii]
            .name) {  // Already a component: just change concentration
      buffer->concentration[ii] = concen;
      QString entext =
          bcomp.name + "  (" + QString::number(concen) + " " + bcomp.unit + ")";
...
void US_BufferGuiNew::create_new_buffer_component() {
  ...
  if (level < 2) {
    QMessageBox::critical(
        ...
        tr("You have not enough permissions on the currently selected database "
           "to create "
...
void US_BufferGuiNew::remove_bcomp(QListWidgetItem* item) {
  ...
  QMessageBox mBox;
  mBox.setText(
      tr("Are you sure you want to delete the buffer component you "
         "double-clicked?"));
    ...
    if (c1 > 0.0) {
      buffer->density +=
          (bc->dens_coeff[0] + bc->dens_coeff[1] * 1.0e-3 * sqrt(c1) +
           bc->dens_coeff[2] * 1.0e-2 * c1 + bc->dens_coeff[3] * 1.0e-3 * c2 +
           bc->dens_coeff[4] * 1.0e-4 * c3 + bc->dens_coeff[5] * 1.0e-6 * c4 -
           DENS_20W);
    }
  }
...
void US_BufferGuiNew::set_temp20() { sl_temp->setValue(20); }
...
void US_BufferGuiEdit::spectrum_class(void) {
  ...
  US_EditSpectrum* w = new US_EditSpectrum("BUFFER", ifexists,
                                           le_descrip->text(), "1.000", buffer);
...
void US_BufferGuiEdit::change_spectrum(void) { emit editBufAccepted(); }
...
US_BufferGuiSettings::US_BufferGuiSettings(int* invID, int* select_db_disk)
  ...
  QPushButton* pb_synchdb =
      us_pushbutton(tr("Synchronize buffer"
                       " components with DB"));
  QPushButton* pb_help = us_pushbutton(tr("Help"));
  ...
  QString number =
      ((*personID) > 0) ? QString::number(US_Settings::us_inv_ID()) + ": " : "";
...
void US_BufferGui::update_personID(int ID) { personID = ID; }
...
US_BufferComponentRequerster::US_BufferComponentRequerster(
  ...
  QString description_text =
      tr("To create a new buffer component, please enter a name, the unit for "
         "the range values.\n") +
...
void US_BufferComponentRequerster::accept(void) {
  ...
  if (le_lrange->text().toDouble() <= le_urange->text().toDouble()) {
    problems << QString(tr(
        "Invalid range, the upper limit is smaller or equal the lower limit."));
    le_urange->setText("");
    le_lrange->setText("");
  }
  foreach (US_BufferComponent bc, component_list) {
    if (bc.name == comp->name && comp->range == bc.range) {
      exists_already = true;
      problems << QString(tr(
          "A buffer component with the same name and range already exists."));
      break;
    }
  }
  QString msg = QString("");
  if (!problems.isEmpty()) {
    msg += QString(
        "The following problems were discovered while checking the input:\n");
  ...
  if (exists_already) {
    QMessageBox::critical(this, tr("Duplicate error"),
                          tr("A buffer component with the same name and range "
                             "already exists.\n\n") +
                              msg);
    return;
  } else {
