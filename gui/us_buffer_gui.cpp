...
US_BufferGuiSelect::US_BufferGuiSelect(int *invID, int *select_db_disk, US_Buffer *tmp_buffer) : US_Widgets() {
   ...
   QLabel *lb_viscosity = us_label(tr("Viscosity (20") + DEGC + tr(", cP):"));
...
void US_BufferGuiSelect::select_buffer() {
   if (lw_buffer_list->currentRow() < 0) {
      ...
      reset();
      return;
   } else DbgLv(1) << "BufS:  selbuf: ROW" << lw_buffer_list->currentRow();
...
bool US_BufferGuiSelect::buffer_path(QString &path) {
   ...
   if (!dir.exists(path)) {
      if (!dir.mkpath(path)) {
         QMessageBox::critical(this, tr("Bad Buffer Path"),
...
void US_BufferGuiSelect::info_buffer(void) {
   ...
   QString big_line("");
   int mxlch = 0;
   int nspec = buffer->extinction.count();
   qDebug() << "#NSPEC:  " << nspec;
...
//                        QString::number( extinc ) + "  ";
   ...
   int iwid = fmet.QFontMetrics::horizontalAdvance(big_line) + 40;
...
void US_BufferGuiSelect::delete_buffer(void) {
   ...
   int response = QMessageBox::question(this, tr("Confirmation"), tr("Delete this buffer?\n"
...
void US_BufferGuiSelect::delete_disk(void) {
   ...
   if (buffer_in_use(bufGUID)) {
      QMessageBox::warning(this, tr("Buffer Not Deleted"), tr("The buffer could not be deleted,\n"
...
void US_BufferGuiSelect::delete_db(void) {
   ...
   if (status == US_DB2::BUFFR_IN_USE) {
      QMessageBox::warning(this, tr("Buffer Not Deleted"), tr("This buffer could not be deleted since\n"
...
void US_BufferGuiNew::add_component() {
   ...
   if (concen < rlow || concen > rhigh) {
      QMessageBox::critical(this, tr("Concentration Out of Range"),
                            tr("Entered concentration %1 out of component range:\n"
                               "  %2 - %3 ").arg(concen).arg(rlow).arg(rhigh) + bcomp.unit);
      return;
   }
   ...
   if (mBox.clickedButton() == yesButton) {
      ...
      for (int ii = 0; ii < buffer->component.size(); ++ii) {
         if (item_name == buffer->component[ii].name) {
            index = ii;
            qDebug() << "TO DELETE::: NAME: " << buffer->component[ii].name << ", concentration:"
                     << buffer->concentration[ii] << ", compID: " << buffer->componentIDs[ii];
...
void US_BufferGuiNew::recalc_density(void) {
   ...
   for (int ii = 0; ii < bcsize; ii++) {
      ...
      if (c1 > 0.0) {
         buffer->density += (bc->dens_coeff[0] + bc->dens_coeff[1] * 1.0e-3 * sqrt(c1) +
                             bc->dens_coeff[2] * 1.0e-2 * c1 + bc->dens_coeff[3] * 1.0e-3 * c2 +
                             bc->dens_coeff[4] * 1.0e-4 * c3 + bc->dens_coeff[5] * 1.0e-6 * c4 - DENS_20W);
      }
   }
   ...
   for (int ii = 0; ii < bcsize; ii++) {
      ...
      if (c1 > 0.0) {
         buffer->viscosity += (bc->visc_coeff[0] + bc->visc_coeff[1] * 1.0e-3 * sqrt(c1) +
                               ...
                               bc->visc_coeff[4] * 1.0e-4 * c3 + bc->visc_coeff[5] * 1.0e-6 * c4 - VISC_20W);
      }
   }
...
// Write new buffer to database
   void US_BufferGuiNew::write_db() {
      ...
      qDebug() << "BufferID: " << buffer->bufferID << ", Description: " << buffer->description << ", Extintcion key #: "
               << buffer->extinction.count();
...
// Main Buffer window with panels
   US_BufferGui::US_BufferGui(bool
   signal_wanted,
   const US_Buffer &buf,
