    ...
    : US_Widgets ()
{
  ...
  lb_density
      = us_label (tr ("Density (20") + DEGC + tr (", g/cm<sup>3</sup>):"));
  ...
  connect (pb_accept, SIGNAL (clicked ()), this, SLOT (accept_buffer ()));
  connect (pb_info, SIGNAL (clicked ()), this, SLOT (info_buffer ()));
  connect (pb_help, SIGNAL (clicked ()), this, SLOT (help ()));
  connect (lw_buffer_list, SIGNAL (itemSelectionChanged ()), this,
           SLOT (select_buffer ()));
  connect (pb_spectrum, SIGNAL (clicked ()), this, SLOT (spectrum ()));
  connect (pb_delete, SIGNAL (clicked ()), this, SLOT (delete_buffer ()));
  connect (pb_temp20C, SIGNAL (clicked ()), this, SLOT (set_temp20 ()));
  connect (sl_temp, SIGNAL (valueChanged (int)), this,
           SLOT (calc_visc_dent_temp ()));
...
bool
US_BufferGuiSelect::load_buffer (const QString &load_init, US_Buffer &buffer)
{
  bool loaded = false;
  ...
  for (int i = 0; i < descriptions.size (); i++)
    {
      qDebug () << "BufS:  load_buffer:  i" << i << "desc" << descriptions[i]
                << "GUID" << GUIDs[i] << "bufferID" << bufferIDs[i];
...
void
US_BufferGuiSelect::select_buffer ()
{
  ...
  DbgLv (1) << "BufS: desc" << buffer->description << "compsize"
            << buffer->componentIDs.size ();
...
void
US_BufferGuiSelect::select_buffer (QListWidgetItem *tmp_item)
{
  ...
  DbgLv (1) << "BufS-item: desc" << buffer->description << "compsize"
            << buffer->componentIDs.size ();
  for (int ii = 0; ii < buffer->componentIDs.size (); ii++)
    {
      show_component (buffer->componentIDs[ii], buffer->concentration[ii]);
...
void
US_BufferGuiSelect::show_component (const QString &componentID, double conc)
{
  ...
  DbgLv (1) << "shco: name conc unit" << name << sconc << unit << "id"
            << componentID;
...
void
US_BufferGuiSelect::read_from_disk (QListWidgetItem *item)
{
...
void
US_BufferGuiSelect::read_from_db (QListWidgetItem *item)
{
  int row = lw_buffer_list->row (item);
...
void
US_BufferGuiSelect::read_from_db (const QString &bufferID)
{
  US_Passwd pw;
  ...
  if (buffer->component.size () > 0)
    DbgLv (1) << "BufS-rddb-bfid   comp0 nm,rng,un,id"
              << buffer->component[0].name << buffer->component[0].range
              << buffer->component[0].unit << buffer->component[0].componentID;
}

// Accept the currently selected buffer
void
US_BufferGuiSelect::accept_buffer (void)
{
  set_temp20 ();
  emit bufferAccepted ();
}
...
void
US_BufferGuiSelect::init_buffer (void)
{
  from_db = ((*db_or_disk) == 1);
  ...
  if ((from_db && idBuf < 0) || (!from_db && bguid.isEmpty ()))
    {
          ...
          for (int ii = 0; ii < buffer_metadata.size (); ii++)
            {
              if (bufid == buffer_metadata[ii].bufferID)
                {
                  DbgLv (1) << "BufS-initb:  ii" << ii << "match bufID"
                            << buffer->bufferID;
...
void
US_BufferGuiSelect::search (QString const &text)
{
  QString sep = ";";
  ...
  sortdesc.sort ();
  DbgLv (1) << "BufS-search:  descsize" << dsize << "sortsize"
            << sortdesc.size ();
...
bool
US_BufferGuiSelect::buffer_path (QString &path)
{
  QDir dir;
...
void
US_BufferGuiSelect::info_buffer (void)
{
  QStringList lines;
  QString inf_text;
  QString big_line ("");
  ...
  lines << tr ("pH:                       ") + QString::number (buffer->pH);
  lines << tr ("Compressibility:          ")
               + QString::number (buffer->compressibility);
  lines << tr ("Manual density,viscosity: ")
               + (buffer->manual ? tr ("ON") : tr ("off"));
  ...
  for (int ii = 0; ii < nspec; ii++)
    {
      ...
      DbgLv (1) << "BufS-info:  ii" << ii << "waveln extinc" << waveln
                << extinc;
  ...
  US_Editor *buf_info
      = new US_Editor (US_Editor::DEFAULT, true, QString (), this);
...
void
US_BufferGuiSelect::spectrum (void)
{
...
void
US_BufferGuiSelect::delete_buffer (void)
{
  ...
  int response = QMessageBox::question (this, tr ("Confirmation"),
...
void
US_BufferGuiSelect::delete_disk (void)
{
...
bool
US_BufferGuiSelect::buffer_in_use (QString &bufferGUID)
{
  bool in_use = false;
  QString soldir = US_Settings::dataDir () + "/solutions/";
  QStringList sfilt ("S*.xml");
  QStringList snames
      = QDir (soldir).entryList (sfilt, QDir::Files, QDir::Name);
...
void
US_BufferGuiSelect::delete_db (void)
{
  US_Passwd pw;
  US_DB2 db (pw.getPasswd ());
  ...
  if (status == US_DB2::BUFFR_IN_USE)
    {
      QMessageBox::warning (this, tr ("Buffer Not Deleted"),
...
void
US_BufferGuiSelect::read_buffer (void)
{
  QString path;
  if (!buffer_path (path))
    return;
  ...
  for (int ii = 0; ii < f_names.size (); ii++)
    {
      ...
      if (!b_file.open (QIODevice::ReadOnly | QIODevice::Text))
        continue;
...
void
US_BufferGuiSelect::read_db (void)
{
  US_Passwd pw;
  US_DB2 db (pw.getPasswd ());
...
void
US_BufferGuiSelect::connect_error (const QString &error)
{
...
void
US_BufferGuiSelect::accept (void)
{
  set_temp20 ();
}
...
void
US_BufferGuiSelect::reject (void)
{
  set_temp20 ();
...
void
US_BufferGuiSelect::calc_visc_dent_temp ()
{
  double temp = sl_temp->value ();
  lb_density->setText (
      tr ("Density (%1%2, g/cm<sup>3</sup>):").arg (temp).arg (DEGC));
  lb_viscosity->setText (tr ("Viscosity (%1%2, cP):").arg (temp).arg (DEGC));
  qApp->processEvents ();
  if (le_density->text ().isEmpty ())
    return;
  if (le_viscosity->text ().isEmpty ())
    return;
    ...
    : US_Widgets ()
{
  ...
  lb_density
      = us_label (tr ("Density (20") + DEGC + tr (", g/cm<sup>3</sup>):"));
  lb_viscos = us_label (tr ("Viscosity (20") + DEGC + tr (", cP):"));
  QLabel *lb_ph = us_label (tr ("pH:"));
  ...
  if (false && US_Settings::us_inv_level () > 1)
    { // "create new" disabled - see issue #275
      main->addWidget (bn_allcomps, row, 0, 1, 3);
      main->addWidget (pb_new_bcomp, row, 3, 1, 1);
      connect (pb_new_bcomp, SIGNAL (clicked ()), this,
               SLOT (create_new_buffer_component ()));
    }
  else
    {
  ...
  connect (le_descrip, SIGNAL (editingFinished ()), this,
           SLOT (new_description ()));
  connect (le_concen, SIGNAL (editingFinished ()), this,
           SLOT (add_component ()));
  connect (lw_allcomps, SIGNAL (itemSelectionChanged ()), this,
           SLOT (select_bcomp ()));
  connect (lw_allcomps, SIGNAL (itemDoubleClicked (QListWidgetItem *)), this,
           SLOT (select_water (QListWidgetItem *)));
  connect (lw_bufcomps, SIGNAL (itemDoubleClicked (QListWidgetItem *)), this,
           SLOT (remove_bcomp (QListWidgetItem *)));
  connect (le_density, SIGNAL (editingFinished ()), this, SLOT (density ()));
  connect (le_viscos, SIGNAL (editingFinished ()), this, SLOT (viscosity ()));
  connect (le_ph, SIGNAL (editingFinished ()), this, SLOT (ph ()));
  connect (le_compress, SIGNAL (editingFinished ()), this,
           SLOT (compressibility ()));
  connect (ck_manual, SIGNAL (toggled (bool)), this,
           SLOT (manual_flag (bool)));
  ...
  connect (this, SIGNAL (use_db (bool)), this, SLOT (update_db_disk (bool)));
  connect (sl_temp, SIGNAL (valueChanged (int)), this,
           SLOT (calc_visc_dent_temp ()));
  connect (pb_temp20C, SIGNAL (clicked ()), this, SLOT (set_temp20 ()));
}
...
void
US_BufferGuiNew::update_db_disk (bool state)
{
...
void
US_BufferGuiNew::init_buffer (void)
{
...
void
US_BufferGuiNew::new_description ()
{
  ...
  bool can_accept
      = (!le_descrip->text ().isEmpty () && !le_density->text ().isEmpty ()
...
void
US_BufferGuiNew::select_water (QListWidgetItem *item)
{
  ...
  if (lw_bufcomps->count () > 0)
    {
      QMessageBox::warning (
          this, tr ("Warning!"),
          tr ("Please remove all the selected buffer components before "
              "choosing [ Water (H2O) ]!"));
      return;
    }
...
void
US_BufferGuiNew::add_component ()
{
  DbgLv (1) << "BufN:SL: add_component()";
  if (lw_bufcomps->count () > 0 && lw_bufcomps->item (0)->type () < 0)
    {
      QMessageBox::warning (this, tr ("Warning!"),
                            tr ("Please remove the water component before "
                                "choosing any other components!"));
      return;
    }
  ...
  if (concen < rlow || concen > rhigh)
    {
      QMessageBox::critical (
          this, tr ("Concentration Out of Range"),
  ...
  for (int ii = 0; ii < buffer->component.size (); ii++)
    {
      if (bcomp.name == buffer->component[ii].name)
        { // Already a component: just change concentration
          ...
          QString entext = bcomp.name + "  (" + QString::number (concen) + " "
                           + bcomp.unit + ")";
  ...
  if (notfound)
    {
      DbgLv (1) << "BufN:SL: adco:  cname" << bcomp.name << "crange"
                << bcomp.range << "cunit" << bcomp.unit;
      QString entext = bcomp.name + "  (" + QString::number (concen) + " "
                       + bcomp.unit + ")";
      new QListWidgetItem (entext, lw_bufcomps,
                           lw_allcomps->currentItem ()->type ());
...
void
US_BufferGuiNew::create_new_buffer_component ()
{
  ...
  if (!from_db)
    { // if not, exceed
      QMessageBox::critical (
          ...
          tr ("Without a database you can create new buffer components."));
      return;
    }
  ...
  if (level < 2)
    {
      QMessageBox::critical (this, tr ("Insufficient permissions"),
                             tr ("You have not enough permissions on the "
                                 "currently selected database to create "
  ...
  US_BufferComponentRequerster *bcomp_dialog
      = new US_BufferComponentRequerster (&bc, component_list);
  bcomp_dialog->exec ();
  qApp->processEvents ();
  if (!bc.name.isEmpty ())
    return;
...
US_BufferGuiNew::select_bcomp ()
{
  ...
  DbgLv (1) << "BufN:SL:  selb:  cname" << bcomp.name << "crange"
            << bcomp.range << "cunit" << bcomp.unit;
...
void
US_BufferGuiNew::remove_bcomp (QListWidgetItem *item)
{
  DbgLv (1) << "BufN:SL: remove_bcomp()" << item->text ();

  QMessageBox mBox;
  mBox.setText (tr ("Are you sure you want to delete the buffer component you "
                    "double-clicked?"));
  ...
  if (mBox.clickedButton () == yesButton)
    {
      ...
      for (int ii = 0; ii < buffer->component.size (); ii++)
        {
          qDebug () << "NAME: " << buffer->component[ii].name
                    << ", concentration:" << buffer->concentration[ii]
                    << ", compID: " << buffer->componentIDs[ii];
        }
      ...
      for (int ii = 0; ii < buffer->component.size (); ++ii)
        {
          if (item_name == buffer->component[ii].name)
            {
              index = ii;
              qDebug () << "TO DELETE::: NAME: " << buffer->component[ii].name
                        << ", concentration:" << buffer->concentration[ii]
                        << ", compID: " << buffer->componentIDs[ii];
      ...
      qDebug () << "index: " << index
                << ", buf_size: " << buffer->component.size ();
...
void
US_BufferGuiNew::recalc_density (void)
{
  ...
  buffer->density = DENS_20W;
  DbgLv (1) << "BufN:SL: recalc_density()"
            << buffer->component[bcsize - 1].name;
  ...
  for (int ii = 0; ii < bcsize; ii++)
    {
      ...
      if (c1 > 0.0)
        {
          buffer->density
              += (bc->dens_coeff[0] + bc->dens_coeff[1] * 1.0e-3 * sqrt (c1)
                  + bc->dens_coeff[2] * 1.0e-2 * c1
                  + bc->dens_coeff[3] * 1.0e-3 * c2
                  + bc->dens_coeff[4] * 1.0e-4 * c3
                  + bc->dens_coeff[5] * 1.0e-6 * c4 - DENS_20W);
        }
    }
...
void
US_BufferGuiNew::recalc_viscosity (void)
{
  int bcsize = buffer->component.size ();
  if (bcsize < 1)
    {
  ...
  buffer->viscosity = VISC_20W;
  DbgLv (1) << "BufN:SL: recalc_viscosity()"
            << buffer->component[bcsize - 1].name;
  ...
  for (int ii = 0; ii < bcsize; ii++)
    {
      ...
      if (c1 > 0.0)
        {
          buffer->viscosity
              += (bc->visc_coeff[0] + bc->visc_coeff[1] * 1.0e-3 * sqrt (c1)
                  + bc->visc_coeff[2] * 1.0e-2 * c1
                  + bc->visc_coeff[3] * 1.0e-3 * c2
                  + bc->visc_coeff[4] * 1.0e-4 * c3 - 1.0)
                 * VISC_20W;
        }
    }
...
// Modify Viscosity and Density of Buffer for Temperature
void
US_BufferGuiNew::calc_visc_dent_temp ()
{
  double temp = sl_temp->value ();
  lb_density->setText (
      tr ("Density (%1%2, g/cm<sup>3</sup>):").arg (temp).arg (DEGC));
  ...
  if (le_density->text ().isEmpty ())
    return;
  if (le_viscos->text ().isEmpty ())
    return;
  if (ck_manual->isChecked ())
    return;
  double density_tb, viscosity_tb;
  if (temp == 20)
    {
...
void
US_BufferGuiNew::compressibility (void)
{
...
void
US_BufferGuiNew::viscosity (void)
{
...
// Slot for manually changed density
void
US_BufferGuiNew::manual_flag (bool is_on)
{
  ...
  if (is_on)
    {
      if (lw_bufcomps->count () > 0)
        {
          int qs = QMessageBox::question (
              this, "Warning!",
              "If you continue with YES, all selected buffer "
...
// Display a spectrum dialog for list/manage
void
US_BufferGuiNew::spectrum_class (void)
{
  US_NewSpectrum *w
      = new US_NewSpectrum ("BUFFER", le_descrip->text (), "", buffer);
...
US_BufferGuiNew::write_db ()
{
  ...
  if (idBuf < 0)
    {
      ...
      QString msg = tr ("(Return Code = %1 ) ").arg (idBuf) + db.lastError ();
      QMessageBox::critical (this, tr ("DB Buffer Write Error"),
  ...
  qDebug () << "BufferID: " << buffer->bufferID
            ...
            << ", Extintcion key #: " << buffer->extinction.keys ().count ();
}

// Write new buffer to local disk file
void
US_BufferGuiNew::write_disk ()
{
    ...
    : US_Widgets ()
{
  ...
  connect (pb_spectrum, SIGNAL (clicked ()), this, SLOT (spectrum_class ()));
  connect (pb_help, SIGNAL (clicked ()), this, SLOT (help ()));
  connect (pb_cancel, SIGNAL (clicked ()), this, SLOT (editCanceled ()));
  connect (pb_accept, SIGNAL (clicked ()), this, SLOT (editAccepted ()));
}

// Initialize buffer settings, possibly after re-entry to Edit panel
void
US_BufferGuiEdit::init_buffer (void)
{
  from_db = ((*db_or_disk) == 1);
  orig_buffer = *buffer;
...
void
US_BufferGuiEdit::spectrum_class (void)
{
  QString ifexists;
  if (buffer->extinction.isEmpty ())
    ifexists = "NEW";
  ...
  US_EditSpectrum *w = new US_EditSpectrum (
      "BUFFER", ifexists, le_descrip->text (), "1.000", buffer);

  connect (w, SIGNAL (change_spectrum (void)), this,
           SLOT (change_spectrum (void)));
  connect (w, SIGNAL (accept_enable (void)), this,
           SLOT (accept_enable (void)));
...
void
US_BufferGuiEdit::change_spectrum (void)
{
  emit editBufAccepted ();
}

void
US_BufferGuiEdit::accept_enable (void)
{
  pb_accept->setEnabled (!le_descrip->text ().isEmpty ());
}
    ...
    : US_Widgets ()
{
  ...
  int idb_or_disk
      = from_db ? US_Disk_DB_Controls::DB : US_Disk_DB_Controls::Disk;
  ...
  connect (disk_controls, SIGNAL (changed (bool)), this,
           SLOT (source_changed (bool)));
  connect (pb_investigator, SIGNAL (clicked ()), this,
           SLOT (sel_investigator ()));
  connect (pb_synchdb, SIGNAL (clicked ()), this, SLOT (synch_components ()));
  connect (pb_help, SIGNAL (clicked ()), this, SLOT (help ()));
}

// Select a new investigator
void
US_BufferGuiSettings::sel_investigator (void)
{
...
void
US_BufferGuiSettings::source_changed (bool db)
{
  emit use_db (db);
...
void
US_BufferGuiSettings::assign_investigator (int invID)
{
  (*personID) = invID;

  QString number = ((*personID) > 0) ? QString::number (invID) + ": " : "";
...
void
US_BufferGuiSettings::synch_components (void)
{
  US_Passwd pw;
    ...
    : US_WidgetsDialog (0, 0), signal (signal_wanted), buffer (buf)
{
  ...
  connect (tabWidget, SIGNAL (currentChanged (int)), this,
           SLOT (checkTab (int)));
  connect (selectTab, SIGNAL (bufferAccepted (void)), this,
           SLOT (bufferAccepted (void)));
  connect (selectTab, SIGNAL (selectionCanceled (void)), this,
           SLOT (bufferRejected (void)));
  connect (newTab, SIGNAL (newBufAccepted (void)), this,
           SLOT (newBufAccepted (void)));
  connect (newTab, SIGNAL (newBufCanceled (void)), this,
           SLOT (newBufCanceled (void)));
  connect (editTab, SIGNAL (editBufAccepted (void)), this,
           SLOT (editBufAccepted (void)));
  connect (editTab, SIGNAL (editBufCanceled (void)), this,
           SLOT (editBufCanceled (void)));
  connect (settingsTab, SIGNAL (use_db (bool)), this,
           SLOT (update_disk_or_db (bool)));
  connect (settingsTab, SIGNAL (investigator_changed (int)), this,
           SLOT (update_personID (int)));
}

bool
US_BufferGui::load_buffer (const QString &load_init, US_Buffer &bufferIn)
{
  bool loaded = false;
...
void
US_BufferGui::checkTab (int currentTab)
{
...
void
US_BufferGui::update_disk_or_db (bool choice)
{
...
void
US_BufferGui::update_personID (int ID)
{
  personID = ID;
}
...
void
US_BufferGui::editBufAccepted (void)
{
...
void
US_BufferGui::editBufCanceled (void)
{
...
void
US_BufferGui::newBufAccepted (void)
{
...
void
US_BufferGui::newBufCanceled (void)
{
...
void
US_BufferGui::bufferAccepted (void)
{
  emit valueChanged (buffer.density, buffer.viscosity);
...
void
US_BufferGui::bufferRejected (void)
{
  buffer = orig_buffer;
  reject ();
}
...
US_BufferComponentRequerster::US_BufferComponentRequerster (
    ...
    : US_WidgetsDialog (nullptr, nullptr), comp (comp_), component_list (list_)
{
  ...
  QString description_text
      = tr ("To create a new buffer component, please enter a name, the unit "
            "for the range values.\n")
        + tr ("Additionally you have to enter the density and viscosity "
              "coefficients for this component.\n")
        + tr ("Click on the check box if the buffer component is gradient "
              "forming.\n\n")
  ...
  lb_viscosity3
      = us_label (tr ("Viscosity coefficient for c<sup>2</sup> *1E3"));
  lb_viscosity4
      = us_label (tr ("Viscosity coefficient for c<sup>3</sup> *1E4"));
  lb_viscosity5
      = us_label (tr ("Viscosity coefficient for c<sup>4</sup> *1E6"));
...
void
US_BufferComponentRequerster::cancelled (void)
{
  comp->name = QString ("");
  this->close ();
}
...
void
US_BufferComponentRequerster::accept (void)
{
  comp->name = le_name->text ();
  comp->unit = le_unit->text ();
  comp->range = QString::number (le_lrange->text ().toDouble (), 'f', 3) + '-'
                + QString::number (le_urange->text ().toDouble (), 'f', 3)
                + ' M';
  ...
  if (!(comp->unit == QString ("M") || comp->unit == QString ("mM")))
    {
      problems << QString (
          tr ("Invalid unit, currently only mM and M are supported."));
      le_unit->setText ("");
    }
  if (le_lrange->text ().toDouble () <= le_urange->text ().toDouble ())
    {
      problems << QString (tr ("Invalid range, the upper limit is smaller or "
  ...
  foreach (US_BufferComponent bc, component_list)
    {
      if (bc.name == comp->name && comp->range == bc.range)
        {
          exists_already = true;
          problems << QString (tr ("A buffer component with the same name and "
  ...
  if (!problems.isEmpty ())
    {
      msg += QString ("The following problems were discovered while checking "
                      "the input:\n");
  ...
  if (exists_already)
    {
      QMessageBox::critical (this, tr ("Duplicate error"),
                             tr ("A buffer component with the same name and "
                                 "range already exists.\n\n")
                                 + msg);
      return;
    }
    ...
    {
      QMessageBox::information (this, tr ("Input error"), msg);
      return;
    }
  this->close ();
}
...
void
US_BufferComponentRequerster::edit (void)
{
  if (le_name->text ().isEmpty () || le_unit->text ().isEmpty ()
      || le_density0->text ().isEmpty () || le_density1->text ().isEmpty ()
      || le_density2->text ().isEmpty () || le_density3->text ().isEmpty ()
      || le_density4->text ().isEmpty () || le_density5->text ().isEmpty ()
      || le_viscosity0->text ().isEmpty () || le_viscosity1->text ().isEmpty ()
      || le_viscosity2->text ().isEmpty () || le_viscosity3->text ().isEmpty ()
      || le_viscosity4->text ().isEmpty () || le_viscosity5->text ().isEmpty ()
      || le_lrange->text ().isEmpty () || le_urange->text ().isEmpty ())
    {
      pb_accept->setEnabled (false);
      return;
    }
...
void
US_BufferComponentRequerster::gf_ck (bool)
{
}
