...
US_BufferGuiSelect::US_BufferGuiSelect(int *invID, int *select_db_disk, US_Buffer *tmp_buffer) : US_Widgets() {
   buffer = tmp_buffer;
   personID = invID;
   ...
   connect(le_search, SIGNAL(textChanged(const QString &)), this, SLOT(search(const QString &)));
   ...
   connect(lw_buffer_list, SIGNAL(itemSelectionChanged()), this, SLOT(select_buffer()));
...
bool US_BufferGuiSelect::load_buffer(const QString &load_init, US_Buffer &buffer) {
   ...
   if (QFile::exists(load_init)) { // Argument is a file path
   ...
   for (int i = 0; i < descriptions.size(); i++) {
      qDebug() << "BufS:  load_buffer:  i" << i << "desc" << descriptions[ i ] << "GUID" << GUIDs[ i ] << "bufferID"
               << bufferIDs[ i ];
...
void US_BufferGuiSelect::select_buffer(QListWidgetItem *tmp_item) {
   ...
   DbgLv(1) << "BufS-item: desc" << buffer->description << "compsize" << buffer->componentIDs.size();
   for (int ii = 0; ii < buffer->componentIDs.size(); ii++) {
      show_component(buffer->componentIDs[ ii ], buffer->concentration[ ii ]);
   }
...
void US_BufferGuiSelect::show_component(const QString &componentID, double conc) {
   ...
   DbgLv(1) << "shco: name conc unit" << name << sconc << unit << "id" << componentID;
...
void US_BufferGuiSelect::init_buffer(void) {
   ...
   if (!bguid.isEmpty()) { // There is a selected buffer, select a list item
   ...
   else { // There is no selected buffer, de-select a list item
      lw_buffer_list->setCurrentRow(-1);
...
void US_BufferGuiSelect::search(QString const &text) {
   ...
   for (int ii = 0; ii < descriptions.size(); ii++) { // get list of filtered-description + index strings
      if (descriptions[ ii ].contains(QRegExp(".*" + text + ".*", Qt::CaseInsensitive)) && !descriptions[ ii ].isEmpty()) {
   ...
   sortdesc.sort();
   DbgLv(1) << "BufS-search:  descsize" << dsize << "sortsize" << sortdesc.size();

   for (int jj = 0; jj < sortdesc.size(); jj++) { // build list of sorted meta data and ListWidget entries
      int ii = sortdesc[ jj ].section(sep, 1, 1).toInt();

      if (ii < 0 || ii >= dsize) {
         DbgLv(1) << "BufS-search:  *ERROR* ii" << ii << "jj" << jj << "sdesc" << sortdesc[ jj ].section(sep, 0, 0);
         continue;
      }
      BufferInfo info;
      info.index = ii;
...
bool US_BufferGuiSelect::buffer_path(QString &path) {
   ...
   if (!dir.exists(path)) {
      if (!dir.mkpath(path)) {
         QMessageBox::critical(this, tr("Bad Buffer Path"), tr("Could not create default directory for buffers\n") + path);
         return false;
      }
   }
...
void US_BufferGuiSelect::info_buffer(void) {
   ...
   QFont tfont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 1));
   ...
   lines << tr("Density:                  ") + QString::number(buffer->density);
   lines << tr("Viscosity:                ") + QString::number(buffer->viscosity);
   lines << tr("pH:                       ") + QString::number(buffer->pH);
   lines << tr("Compressibility:          ") + QString::number(buffer->compressibility);
   lines << tr("Manual density,viscosity: ") + (buffer->manual ? tr("ON") : tr("off"));
   ...
   for (int ii = 0; ii < nspec; ii++) {
      ...
      QString spair;
      if (extinc < 0)
         spair = QString::number(waveln) + " /" + QString::number(extinc, 'f', 4) + "  ";
      else
         spair = QString::number(waveln) + " / " + QString::number(extinc, 'f', 4) + "  ";
   ...
   US_Editor *buf_info = new US_Editor(US_Editor::DEFAULT, true, QString(), this);
...
void US_BufferGuiSelect::spectrum(void) {
   ...
   if (buffer->extinction.isEmpty()) {
      QMessageBox::information(this, tr("WARNING"), tr("Buffer does not have spectrum data!"));
   }
   else {
...
void US_BufferGuiSelect::delete_buffer(void) {
   if (buffer->GUID.size() == 0 || lw_buffer_list->currentRow() < 0) {
      QMessageBox::information(this, tr("Attention"), tr("First select the buffer to be deleted."));
      return;
   }
...
bool US_BufferGuiSelect::buffer_in_use(QString &bufferGUID) {
   ...
   QStringList snames = QDir(soldir).entryList(sfilt, QDir::Files, QDir::Name);
...
void US_BufferGuiSelect::delete_db(void) {
   ...
   if (status != US_DB2::OK) {
      QMessageBox::warning(this, tr("Attention"), tr("Delete failed.\n\n") + db.lastError());
   }
}
...
// New Buffer panel
US_BufferGuiNew::US_BufferGuiNew(int *invID, int *select_db_disk, US_Buffer *tmp_buffer) : US_Widgets() {
   buffer = tmp_buffer;
   personID = invID;
   db_or_disk = select_db_disk;
   from_db = ((*db_or_disk) == 1);
   ...
   connect(le_concen, SIGNAL(editingFinished()), this, SLOT(add_component()));
   connect(lw_allcomps, SIGNAL(itemSelectionChanged()), this, SLOT(select_bcomp()));
   connect(lw_allcomps, SIGNAL(itemDoubleClicked(QListWidgetItem *)), this, SLOT(select_water(QListWidgetItem *)));
   connect(lw_bufcomps, SIGNAL(itemDoubleClicked(QListWidgetItem *)), this, SLOT(remove_bcomp(QListWidgetItem *)));
   ...
   connect(le_compress, SIGNAL(editingFinished()), this, SLOT(compressibility()));
   connect(ck_manual, SIGNAL(toggled(bool)), this, SLOT(manual_flag(bool)));
...
void US_BufferGuiNew::new_description() {
   ...
   bool can_accept = (!le_descrip->text().isEmpty() && !le_density->text().isEmpty() && !le_viscos->text().isEmpty());
...
void US_BufferGuiNew::select_water(QListWidgetItem *item) {
   ...
   if (lw_bufcomps->count() > 0) {
      QMessageBox::warning(
         this, tr("Warning!"), tr("Please remove all the selected buffer components before choosing [ Water (H2O) ]!"));
      return;
   }
...
void US_BufferGuiNew::add_component() {
   ...
   if (lw_bufcomps->count() > 0 && lw_bufcomps->item(0)->type() < 0) {
      QMessageBox::warning(
         this, tr("Warning!"), tr("Please remove the water component before choosing any other components!"));
      return;
   }
   ...
   if (bcomp.unit != runit) { // Change limits to component (not range) units
      if (runit == "M") { // Range in M, Concentration in mM; so multiple by 1000
         rlow *= 1000.0;
         rhigh *= 1000.0;
      }
      DbgLv(1) << "BufN:SL: adco:  runit" << runit << "cunit" << bcomp.unit << "rlow rhigh" << rlow << rhigh;
   }
   ...
   for (int ii = 0; ii < buffer->component.size(); ii++) {
      if (bcomp.name == buffer->component[ ii ].name) { // Already a component: just change concentration
         buffer->concentration[ ii ] = concen;
         QString entext = bcomp.name + "  (" + QString::number(concen) + " " + bcomp.unit + ")";
   ...
   if (notfound) {
      DbgLv(1) << "BufN:SL: adco:  cname" << bcomp.name << "crange" << bcomp.range << "cunit" << bcomp.unit;
      QString entext = bcomp.name + "  (" + QString::number(concen) + " " + bcomp.unit + ")";
...
void US_BufferGuiNew::create_new_buffer_component() {
   ...
   if (!from_db) { // if not, exceed
      QMessageBox::critical(
         this, tr("No database connection"), tr("Without a database you can create new buffer components."));
      return;
   }
...
void US_BufferGuiNew::remove_bcomp(QListWidgetItem *item) {
   ...
   if (mBox.clickedButton() == yesButton) {
      ...
      for (int ii = 0; ii < buffer->component.size(); ii++) {
         qDebug() << "NAME: " << buffer->component[ ii ].name << ", concentration:" << buffer->concentration[ ii ]
      ...
      for (int ii = 0; ii < buffer->component.size(); ++ii) {
         if (item_name == buffer->component[ ii ].name) {
            ...
            qDebug() << "TO DELETE::: NAME: " << buffer->component[ ii ].name
                     << ", concentration:" << buffer->concentration[ ii ] << ", compID: " << buffer->componentIDs[ ii ];
...
void US_BufferGuiNew::recalc_density(void) {
   ...
   for (int ii = 0; ii < bcsize; ii++) {
      ...
      if (c1 > 0.0) {
         buffer->density
            += (bc->dens_coeff[ 0 ] + bc->dens_coeff[ 1 ] * 1.0e-3 * sqrt(c1) + bc->dens_coeff[ 2 ] * 1.0e-2 * c1 + bc->dens_coeff[ 3 ] * 1.0e-3 * c2 + bc->dens_coeff[ 4 ] * 1.0e-4 * c3 + bc->dens_coeff[ 5 ] * 1.0e-6 * c4 - DENS_20W);
      }
   }
...
void US_BufferGuiNew::recalc_viscosity(void) {
   ...
   for (int ii = 0; ii < bcsize; ii++) {
      ...
      if (c1 > 0.0) {
         buffer->viscosity += (bc->visc_coeff[ 0 ] + bc->visc_coeff[ 1 ] * 1.0e-3 * sqrt(c1)
                               + bc->visc_coeff[ 2 ] * 1.0e-2 * c1 + bc->visc_coeff[ 3 ] * 1.0e-3 * c2
...
void US_BufferGuiNew::write_db() {
   ...
   if (idBuf < 0) {
      ...
      QMessageBox::critical(this, tr("DB Buffer Write Error"), tr("Error updating buffer in the database:\n") + msg);
   }

   qDebug() << "BufferID: " << buffer->bufferID << ", Description: " << buffer->description
...
US_BufferGuiEdit::US_BufferGuiEdit(int *invID, int *select_db_disk, US_Buffer *tmp_buffer) : US_Widgets() {
   buffer = tmp_buffer;
   orig_buffer = *buffer;
   personID = invID;
   db_or_disk = select_db_disk;
   ...
   connect(le_descrip, SIGNAL(editingFinished()), this, SLOT(description()));
   connect(le_ph, SIGNAL(editingFinished()), this, SLOT(ph()));
   // connect( pb_spectrum, SIGNAL( clicked()  ),
   //          this,        SLOT  ( spectrum() ) );
   connect(pb_spectrum, SIGNAL(clicked()), this, SLOT(spectrum_class()));
...
void US_BufferGuiEdit::spectrum_class(void) {
   ...
   connect(w, SIGNAL(change_spectrum(void)), this, SLOT(change_spectrum(void)));
   connect(w, SIGNAL(accept_enable(void)), this, SLOT(accept_enable(void)));
...
void US_BufferGuiEdit::editAccepted() {
   ...
   if (from_db) { // Update buffer in database
      write_db();
      DbgLv(1) << "BufE:SL:  edtAcc: DB  id" << buffer->bufferID;
   }

   else { // Update buffer on local disk
...
// Settings panel
US_BufferGuiSettings::US_BufferGuiSettings(int *invID, int *select_db_disk) : US_Widgets() {
   personID = invID;
   db_or_disk = select_db_disk;
   from_db = ((*db_or_disk) == 1);
   dbg_level = US_Settings::us_debug();
   ...
   QString number = ((*personID) > 0) ? QString::number(US_Settings::us_inv_ID()) + ": " : "";

   le_investigator = us_lineedit(number + US_Settings::us_inv_name());
   le_syncstat = us_lineedit();
   int idb_or_disk = from_db ? US_Disk_DB_Controls::DB : US_Disk_DB_Controls::Disk;
   ...
   connect(disk_controls, SIGNAL(changed(bool)), this, SLOT(source_changed(bool)));
...
void US_BufferGuiSettings::synch_components(void) {
   ...
   US_BufferComponent::getAllFromDB(pw.getPasswd(), component_list);
   US_BufferComponent::putAllToHD(component_list);
   DbgLv(1) << "setB:synchc   synch complete:  components:" << component_list.size();
...
US_BufferGui::US_BufferGui(bool signal_wanted, const US_Buffer &buf, int select_db_disk) :
    US_WidgetsDialog(0, 0), signal(signal_wanted), buffer(buf) {
   personID = US_Settings::us_inv_ID();
   buffer = buf;
   orig_buffer = buf;
   disk_or_db = (select_db_disk == US_Disk_DB_Controls::Default) ? US_Settings::default_data_location()
                                                                 : select_db_disk;
   dbg_level = US_Settings::us_debug();
   ...
   connect(tabWidget, SIGNAL(currentChanged(int)), this, SLOT(checkTab(int)));
   connect(selectTab, SIGNAL(bufferAccepted(void)), this, SLOT(bufferAccepted(void)));
   connect(selectTab, SIGNAL(selectionCanceled(void)), this, SLOT(bufferRejected(void)));
   connect(newTab, SIGNAL(newBufAccepted(void)), this, SLOT(newBufAccepted(void)));
   connect(newTab, SIGNAL(newBufCanceled(void)), this, SLOT(newBufCanceled(void)));
   connect(editTab, SIGNAL(editBufAccepted(void)), this, SLOT(editBufAccepted(void)));
   connect(editTab, SIGNAL(editBufCanceled(void)), this, SLOT(editBufCanceled(void)));
   connect(settingsTab, SIGNAL(use_db(bool)), this, SLOT(update_disk_or_db(bool)));
   connect(settingsTab, SIGNAL(investigator_changed(int)), this, SLOT(update_personID(int)));
}
...
void US_BufferGui::checkTab(int currentTab) {
   ...
   if (currentTab == 0) {
      ...
      if (editTab->edit_buffer_description == "") {
         QMessageBox::information(this, tr("WARNING"), tr("No Buffer selected! Please select Buffer from the list"));
...
void US_BufferComponentRequerster::accept(void) {
   ...
   comp->range = QString::number(le_lrange->text().toDouble(), 'f', 3) + '-'
                 + QString::number(le_urange->text().toDouble(), 'f', 3) + ' M';
   ...
   if (exists_already) {
      QMessageBox::critical(
         this, tr("Duplicate error"), tr("A buffer component with the same name and range already exists.\n\n") + msg);
      return;
   }
   else {
...
void US_BufferComponentRequerster::edit(void) {
   if (
      le_name->text().isEmpty() || le_unit->text().isEmpty() || le_density0->text().isEmpty()
      || le_density1->text().isEmpty() || le_density2->text().isEmpty() || le_density3->text().isEmpty()
      || le_density4->text().isEmpty() || le_density5->text().isEmpty() || le_viscosity0->text().isEmpty()
      || le_viscosity1->text().isEmpty() || le_viscosity2->text().isEmpty() || le_viscosity3->text().isEmpty()
      || le_viscosity4->text().isEmpty() || le_viscosity5->text().isEmpty() || le_lrange->text().isEmpty()
      || le_urange->text().isEmpty()) {
      pb_accept->setEnabled(false);
      return;
   }
   pb_accept->setEnabled(true);
