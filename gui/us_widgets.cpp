...
// label
QLabel *US_Widgets::us_label(const QString &labelString, int fontAdjust, int weight) {
   ...
   newLabel->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() + fontAdjust, weight));
...
QLabel *US_Widgets::us_textlabel(const QString &labelString, int fontAdjust, int weight) {
...
QLabel *US_Widgets::us_banner(const QString &labelString, int fontAdjust, int weight) {
   QLabel *newLabel = us_label(labelString, fontAdjust, weight);
...
// pushbutton
QPushButton *US_Widgets::us_pushbutton(const QString &labelString, bool enabled, int fontAdjust) {
   QPushButton *button = new QPushButton(tr(labelString.toLatin1()), this);

   button->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() + fontAdjust));
...
QTextEdit *US_Widgets::us_textedit(void) {
   ...
   te->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() - 1));
...
// lineedit
QLineEdit *US_Widgets::us_lineedit(const QString &text, int fontAdjust, bool readonly) {
   QLineEdit *le = new QLineEdit(this);


   le->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() + fontAdjust));
...
QListWidget *US_Widgets::us_listwidget(int fontAdjust) {
   ...
   lw->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() + fontAdjust));

   return lw;
}

// checkbox
QGridLayout *US_Widgets::us_checkbox(const QString &text, QCheckBox *&cb, bool state) {
   QPalette p = US_GuiSettings::normalColor();
   QFont font = QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(), QFont::Bold);
...
// radiobutton
QGridLayout *US_Widgets::us_radiobutton(const QString &text, QRadioButton *&rb, bool state) {
   QPalette p = US_GuiSettings::normalColor();
   QFont font = QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(), QFont::Bold);
...
QProgressBar *US_Widgets::us_progressBar(int low, int high, int value) {
   ...
   pb->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(), QFont::Bold));
...
QComboBox *US_Widgets::us_comboBox(void) {
   ...
   cb->setAutoFillBackground(true);
   cb->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize()));
...
//QwtCounter
QwtCounter *US_Widgets::us_counter(int buttons, double low, double high, double value) {
   ...
   for (int jj = 0; jj < children.size(); jj++) { // Accumulate total width of button widgets
      QWidget *cwidg = ( QWidget * ) children.at(jj);
      QString clname = cwidg->metaObject()->className();
...
QwtPlot *US_Widgets::us_plot(const QString &title, const QString &x_axis, const QString &y_axis) {
...
QwtPlotPicker *US_Widgets::us_picker(QwtPlot *plot) {
   QwtPlotPicker *pick = new QwtPlotPicker(QwtPlot::xBottom, QwtPlot::yLeft, plot->canvas());
...
QFont US_Widgets::fixedFont() {
   ...
   bool ffixed;
   const char *preffam[] = {
      "DejaVu Sans Mono", "Nimbus Mono L",  "Liberation Mono", "FreeMono",       "DejaVu LGC Sans Mono",
      "Andale Mono",      "Menlo",          "Luxi Mono",       "Lucida Console", "Fixedsys",
      "Terminal",         "QuickType mono", "Monaco",          "Courier New",    "Courier 10 Pitch",
      "Courier",          "Monospace"
   };
   ...
   for (int ii = 0; ii < pfsize; ii++) {
      ...
      qDebug() << "fixf:   ii" << ii << "family" << family << "fmatch" << fmatch << "ffixed" << ffixed;
      if (fmatch && ffixed) {
         ffont = tfont;

         if (family.contains("New") || family.contains("FreeM"))
...
// tabWidget
QTabWidget *US_Widgets::us_tabwidget(int fontAdjust, int weight) {
   QTabWidget *newtw = new QTabWidget(this);

   newtw->setAutoFillBackground(true);

   newtw->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() + fontAdjust, weight));
...
// TimeEdit
QHBoxLayout *US_Widgets::us_timeedit(QTimeEdit *&tedt, const int fontAdjust, QSpinBox **sbox) {
   QPalette pal = US_GuiSettings::normalColor();
   QFont font = QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() + fontAdjust);
...
// day-hh-mm-ss box
QHBoxLayout *US_Widgets::us_ddhhmmss(const int fontAdjust, QSpinBox **dd, QSpinBox **hh, QSpinBox **mm, QSpinBox **ss) {
   QPalette pal = US_GuiSettings::normalColor();
   QFont font = QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() + fontAdjust);
...
QHBoxLayout *US_Widgets::us_ddhhmmsslay(
   const int fontAdjust, const int dd_hide, const int hh_hide, const int mm_hide, const int ss_hide, QSpinBox **dd,
   QSpinBox **hh, QSpinBox **mm, QSpinBox **ss) {
   QPalette pal = US_GuiSettings::normalColor();
   QFont font = QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() + fontAdjust);
...
QSpinBox *US_Widgets::us_spinbox(const int fontAdjust) {
   ...
   sbox->setAutoFillBackground(true);
   sbox->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() + fontAdjust));
...
int US_Widgets::clean_etc_dir(bool report) {
   ...
   if (lfile.open(QIODevice::ReadOnly | QIODevice::Text)) {
      ...
      while (!ts.atEnd()) { // Read the list of files that belong in install etc
         QString fline = ts.readLine();

         if (fline.startsWith("#")) { // Handle comment line
         ...
         else { // Actual file name:  move it to appropriate list
   ...
   qDebug() << "niefs nwefs" << niefs << nwefs << "nkeep/copy/link/dirf" << nkeepf << ncopyf << nlinkf << nsdir;
   ...
   for (int ii = 0; ii < niefs; ii++) {
      ...
      if (keep_files.contains(filename)) { // This file is to be kept in the install-etc directory
         qDebug() << "KEEP " << filename;
      }

      else if (copy_files.contains(filename)) { // This file is to be copied to the work-etc directory
         qDebug() << "COPY " << filename;
         if (in_wetc) { // But only copy if it is not already copied
            ...
            if (icksum != wcksum) { // They do not match in cksum+size, so copy
               nfcopy++;
               qDebug() << "       FILE COPY" << nfcopy;
               QFile(wetc_dname + filename).remove();

               QFile(ietc_dname + filename).copy(wetc_dname + filename);
            }
         }
         else { // Not present in work-etc, so copy
      ...
      else if (link_files.contains(filename)) { // This is a link, so copy the target file
         qDebug() << "LINK " << filename;
         QString sltarg = ie_files[ ii ].symLinkTarget();
         qDebug() << "   ietc sltarg " << sltarg;

         if (in_wetc) { // But only copy if not already copied
      ...
      else if (dir_names.contains(filename)) { // This is a directory, so ignore it
         qDebug() << "SDIR " << filename;
      }

      else if (filename.contains("~")) { // If name ends in tilde, delete it
      ...
      else { // File not in "belongs" list:  move it to work-etc
         qDebug() << "MOVE " << filename;
         nfmove++;
         qDebug() << "       FILE MOVE" << nfmove;

         if (in_wetc) { // First delete any version in work-etc
            QFile(wetc_dname + filename).remove();
         }

         QFile(ietc_dname + filename).rename(wetc_dname + filename);
      }
   }

   int nfmods = nfmove + nfcopy + nfdele;

   if (nfmods > 0 && report) { // If so flagged and any exist, pop up a message on modified files
...
US_Disk_DB_Controls::US_Disk_DB_Controls(int state) {
   ...
   if (state == Default)
      state = US_Settings::default_data_location();
...
QGridLayout *US_Disk_DB_Controls::us_radiobutton(const QString &text, QRadioButton *&rb, bool state) {
   QPalette p = US_GuiSettings::normalColor();
   QFont font = QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(), QFont::Bold);
   ...
   if (newText.size() < _mytext.size()) {
      ...
      if (reIdx >= 0) {
         ...
         if (newText.size() > _maxchrs) {
            QMessageBox::warning(
               this, tr("Warning!"), tr("The length of the text cannot exceed %1 characters!").arg(_maxchrs));
            this->_set_etext(_mytext);
