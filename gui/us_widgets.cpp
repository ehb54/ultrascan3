...
// checkbox
QGridLayout* US_Widgets::us_checkbox(const QString& text, QCheckBox*& cb,
                                     bool state) {
...
// radiobutton
QGridLayout* US_Widgets::us_radiobutton(const QString& text, QRadioButton*& rb,
                                        bool state) {
  QPalette p = US_GuiSettings::normalColor();
  QFont font = QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(),
                     QFont::Bold);
...
QProgressBar* US_Widgets::us_progressBar(int low, int high, int value) {
  ...
  pb->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(),
                    QFont::Bold));
...
QwtCounter* US_Widgets::us_counter(int buttons, double low, double high,
  ...
  for (int jj = 0; jj < children.size();
       jj++) {  // Accumulate total width of button widgets
    QWidget* cwidg = (QWidget*)children.at(jj);
    QString clname = cwidg->metaObject()->className();
...
QFont US_Widgets::fixedFont() {
  ...
  for (int ii = 0; ii < pfsize; ii++) {
    ...
    qDebug() << "fixf:   ii" << ii << "family" << family << "fmatch" << fmatch
             << "ffixed" << ffixed;
    if (fmatch && ffixed) {
      ffont = tfont;
...
// TimeEdit
QHBoxLayout* US_Widgets::us_timeedit(QTimeEdit*& tedt, const int fontAdjust,
                                     QSpinBox** sbox) {
...
// day-hh-mm-ss box
QHBoxLayout* US_Widgets::us_ddhhmmss(const int fontAdjust, QSpinBox** dd,
                                     QSpinBox** hh, QSpinBox** mm,
                                     QSpinBox** ss) {
  QPalette pal = US_GuiSettings::normalColor();
  QFont font = QFont(US_GuiSettings::fontFamily(),
                     US_GuiSettings::fontSize() + fontAdjust);
...
// day-hh-mm-ss layout
QHBoxLayout* US_Widgets::us_ddhhmmsslay(const int fontAdjust, const int dd_hide,
                                        const int hh_hide, const int mm_hide,
                                        const int ss_hide, QSpinBox** dd,
                                        QSpinBox** hh, QSpinBox** mm,
                                        QSpinBox** ss) {
  QPalette pal = US_GuiSettings::normalColor();
  QFont font = QFont(US_GuiSettings::fontFamily(),
...
int US_Widgets::clean_etc_dir(bool report) {
  ...
  for (int ii = 0; ii < niefs; ii++) {
    ...
    if (keep_files.contains(filename)) {  // This file is to be kept in the
                                          // install-etc directory
      qDebug() << "KEEP " << filename;
    }

    else if (copy_files.contains(filename)) {  // This file is to be copied to
                                               // the work-etc directory
    ...
    else if (link_files.contains(
                 filename)) {  // This is a link, so copy the target file
    ...
    else if (dir_names.contains(
                 filename)) {  // This is a directory, so ignore it
  ...
  if (nfmods > 0 && report) {  // If so flagged and any exist, pop up a message
                               // on modified files
...
bool US_Disk_DB_Controls::db(void) { return rb_db->isChecked(); }
...
void US_LineEdit_RE::setMaxChars(int n) { _maxchrs = n; }
