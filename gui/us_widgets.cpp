...
US_Widgets::US_Widgets (bool set_position, QWidget *w, Qt::WindowFlags f)
    : QFrame (w, f)
{
...
// label
QLabel *
US_Widgets::us_label (const QString &labelString, int fontAdjust, int weight)
{
  ...
  newLabel->setFont (QFont (US_GuiSettings::fontFamily (),
                            US_GuiSettings::fontSize () + fontAdjust, weight));
...
QLabel *
US_Widgets::us_textlabel (const QString &labelString, int fontAdjust,
                          int weight)
{
...
QLabel *
US_Widgets::us_banner (const QString &labelString, int fontAdjust, int weight)
{
  QLabel *newLabel = us_label (labelString, fontAdjust, weight);
...
// pushbutton
QPushButton *
US_Widgets::us_pushbutton (const QString &labelString, bool enabled,
                           int fontAdjust)
{
...
// lineedit
QLineEdit *
US_Widgets::us_lineedit (const QString &text, int fontAdjust, bool readonly)
{
...
void
US_Widgets::us_setReadOnly (QLineEdit *le, bool readonly)
{
  if (readonly)
    {
...
void
US_Widgets::us_setReadOnly (QTextEdit *te, bool readonly)
{
  if (readonly)
    {
...
// List Widget
QListWidget *
US_Widgets::us_listwidget (int fontAdjust)
{
...
// checkbox
QGridLayout *
US_Widgets::us_checkbox (const QString &text, QCheckBox *&cb, bool state)
{
...
// radiobutton
QGridLayout *
US_Widgets::us_radiobutton (const QString &text, QRadioButton *&rb, bool state)
{
  QPalette p = US_GuiSettings::normalColor ();
  QFont font = QFont (US_GuiSettings::fontFamily (),
                      US_GuiSettings::fontSize (), QFont::Bold);
...
// Progress Bar
QProgressBar *
US_Widgets::us_progressBar (int low, int high, int value)
{
...
// LCD
QLCDNumber *
US_Widgets::us_lcd (int digits, int value)
{
...
// QwtCounter
QwtCounter *
US_Widgets::us_counter (int buttons, double low, double high, double value)
{
...
QwtPlot *
US_Widgets::us_plot (const QString &title, const QString &x_axis,
...
QwtPlotGrid *
US_Widgets::us_grid (QwtPlot *plot)
{
...
QwtPlotCurve *
US_Widgets::us_curve (QwtPlot *plot, const QString &title)
{
...
QwtPlotPicker *
US_Widgets::us_picker (QwtPlot *plot)
{
  QwtPlotPicker *pick
      = new QwtPlotPicker (QwtPlot::xBottom, QwtPlot::yLeft, plot->canvas ());
...
void
US_ListWidget::mousePressEvent (QMouseEvent *event)
{
...
US_Widgets::fixedFont ()
{
  ...
  bool fmatch;
  bool ffixed;
  const char *preffam[] = { "DejaVu Sans Mono",
                            "Nimbus Mono L",
                            "Liberation Mono",
                            "FreeMono",
  ...
  for (int ii = 0; ii < pfsize; ii++)
    {
      ...
      qDebug () << "fixf:   ii" << ii << "family" << family << "fmatch"
                << fmatch << "ffixed" << ffixed;
      if (fmatch && ffixed)
        {
          ffont = tfont;
...
// tabWidget
QTabWidget *
US_Widgets::us_tabwidget (int fontAdjust, int weight)
{
  QTabWidget *newtw = new QTabWidget (this);

  newtw->setAutoFillBackground (true);

  newtw->setFont (QFont (US_GuiSettings::fontFamily (),
                         US_GuiSettings::fontSize () + fontAdjust, weight));
...
// TimeEdit
QHBoxLayout *
US_Widgets::us_timeedit (QTimeEdit *&tedt, const int fontAdjust,
                         QSpinBox **sbox)
{
...
// day-hh-mm-ss box
QHBoxLayout *
US_Widgets::us_ddhhmmss (const int fontAdjust, QSpinBox **dd, QSpinBox **hh,
                         QSpinBox **mm, QSpinBox **ss)
{
...
// day-hh-mm-ss layout
QHBoxLayout *
US_Widgets::us_ddhhmmsslay (const int fontAdjust, const int dd_hide,
                            const int hh_hide, const int mm_hide,
                            const int ss_hide, QSpinBox **dd, QSpinBox **hh,
                            QSpinBox **mm, QSpinBox **ss)
{
  QPalette pal = US_GuiSettings::normalColor ();
  QFont font = QFont (US_GuiSettings::fontFamily (),
...
// SpinBox
QSpinBox *
US_Widgets::us_spinbox (const int fontAdjust)
{
...
void
US_Widgets::write_plot (const QString &fname, const QwtPlot *plot)
{
...
int
US_Widgets::clean_etc_dir (bool report)
{
  int nfmove = 0;
  int nfcopy = 0;
  int nfdele = 0;
...
bool
US_Disk_DB_Controls::db (void)
{
  return rb_db->isChecked ();
}

void
US_Disk_DB_Controls::set_db (void)
{
  rb_db->disconnect ();
...
void
US_Disk_DB_Controls::set_disk (void)
{
  rb_db->disconnect ();
...
void
US_Disk_DB_Controls::rb_changed (bool /* state */)
{
...
QGridLayout *
US_Disk_DB_Controls::us_radiobutton (const QString &text, QRadioButton *&rb,
                                     bool state)
{
  QPalette p = US_GuiSettings::normalColor ();
  QFont font = QFont (US_GuiSettings::fontFamily (),
...
US_LineEdit_RE::US_LineEdit_RE (const QString &txt, int fontAdjust,
                                bool readonly)
    : QLineEdit ()
{
  _dfltchrs = 50;
  ...
  connect (this, SIGNAL (textEdited (const QString &)), this,
           SLOT (newEdit (const QString &)));
}

void
US_LineEdit_RE::setDefault ()
{
...
void
US_LineEdit_RE::setMaxChars (int n)
{
  _maxchrs = n;
}

void
US_LineEdit_RE::setText (const QString &inText)
{
      ...
      if (outText != inText)
        {
          QMessageBox::warning (this, tr ("Warning!"),
                                tr ("Special characters are removed from the "
                                    "input string and the "
                                    "length of the string is limited to the "
                                    "first %1 characters.\n"
                                    "input: %2\noutput: %3")
                                    .arg (_maxchrs)
                                    .arg (inText, outText));
...
void
US_LineEdit_RE::setText_auto (const QString &txt)
{
  _editstate = true;
  this->setText (txt);
}
...
void
US_LineEdit_RE::_set_etext (const QString &txt)
{
  _editstate = true;
  this->setText (txt);
}
...
void
US_LineEdit_RE::newEdit (const QString &newText)
{
  int reIdx;
          ...
          if (newText.size () > _maxchrs)
            {
              QMessageBox::warning (
                  this, tr ("Warning!"),
                  tr ("The length of the text cannot exceed %1 characters!")
                      .arg (_maxchrs));
              this->_set_etext (_mytext);
