...
US_RunDetails2::US_RunDetails2(
   const QVector<US_DataIO::RawData> &data, const QString &runID, const QString &dataDir,
   const QStringList &cell_ch_wl) : US_WidgetsDialog(0, 0), dataList(data), triples(cell_ch_wl) {
   ...
   QBoxLayout *plot = new US_Plot(
      data_plot, tr("Parameter Variation Throughout Run"), tr("Scan Number"), tr("RPM * 1000 / Temperature ") + DEGC);
   ...
   connect(lw_rpm, SIGNAL(itemClicked(QListWidgetItem *)), this, SLOT(show_rpm_details(QListWidgetItem *)));
}
...
void US_RunDetails2::setup(void) {
   ...
   for (int i = 0; i < scanCount_per_dataType.keys().size(); ++i) {
      ...
      qDebug() << "[US_RunDetails] dataType--scanCount: " << scanCount_per_dataType.keys()[ i ] << " -- "
...
void US_RunDetails2::show_all_data(void) {
   ...
   le_avgTemp->setText(QString::number(temp / scanCount, 'f', 1) + " " + DEGC);
...
void US_RunDetails2::draw_plot(const double *x, const double *t, const double *r, const double *m, int count) {
...
void US_RunDetails2::show_rpm_details(QListWidgetItem *item) {
   ...
   QString msg = tr("The following scans have been measured at ") + sspeed + ":\n\n";
   ...
   foreach (triple, triples) {
      ...
      if (cellChWl.size() > 2) { // Normal (c/c/w/s) data
         msg += tr("Cell: ") + cellChWl[ 0 ] + tr(", Channel: ") + cellChWl[ 1 ] + tr(", Wavelength: ") + cellChWl[ 2 ]
                + tr(", Scan Count: ");
         ...
         msg += tr("Cell: ") + cellChWl[ 0 ] + tr(", Channel: ") + cellChWl[ 1 ] + tr(", Scan Count: ");
      }
   ...
   QString fname = US_Settings::resultDir() + "/" + runID + "/" + runID + ".RI.xml";
   QFile filei(fname);
