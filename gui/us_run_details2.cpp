    ...
    : US_WidgetsDialog (0, 0), dataList (data), triples (cell_ch_wl)
{
  ...
  QBoxLayout *plot = new US_Plot (
      data_plot, tr ("Parameter Variation Throughout Run"), tr ("Scan Number"),
  ...
  setup ();
  connect (lw_triples, SIGNAL (currentRowChanged (int)), SLOT (update (int)));

  connect (lw_rpm, SIGNAL (itemClicked (QListWidgetItem *)), this,
           SLOT (show_rpm_details (QListWidgetItem *)));
}
...
US_RunDetails2::setup (void)
{
  ...
  qDebug () << "dtails: ds 1, scan 1: secs,omg2t,rpm" << s1tim << s1omg
            << s1rpm;
  //*DEBUG*
  ...
  for (int i = 0; i < scanCount_per_dataType.keys ().size (); ++i)
    {
      scanCount_per_dataType[scanCount_per_dataType.keys ()[i]]
          .removeDuplicates ();
      qDebug () << "[US_RunDetails] dataType--scanCount: "
                << scanCount_per_dataType.keys ()[i] << " -- "
                << scanCount_per_dataType[scanCount_per_dataType.keys ()[i]];
    }
...
void
US_RunDetails2::show_all_data (void)
{
  le_desc->setText ("");
  ...
  draw_plot (x.constData (), t.constData (), r.constData (), m.constData (),
             scanCount);
}

void
US_RunDetails2::check_temp (double dt)
{
...
void
US_RunDetails2::update_timer (void)
{
...
void
US_RunDetails2::draw_plot (const double *x, const double *t, const double *r,
                           const double *m, int count)
{
  ...
  switch (plotType)
    ...
    default:
      axisTitle.setText (tr ("RPM * 1000 / Temperature ") + DEGC);
      data_plot->setAxisTitle (QwtPlot::yLeft, axisTitle);
...
void
US_RunDetails2::update (int index)
{
  ...
  for (int i = 0; i < scanCount; i++)
    {
      ...
      if (i > 0)
        m[i] = (s->seconds - prior_seconds) / 60.0;
...
void
US_RunDetails2::show_rpm_details (QListWidgetItem *item)
{
  qDebug () << "show_rpm_details";
  QString sspeed = item->text ();
  QString msg
      = tr ("The following scans have been measured at ") + sspeed + ":\n\n";
  ...
  foreach (triple, triples)
    {
      ...
      if (cellChWl.size () > 2)
        { // Normal (c/c/w/s) data
          msg += tr ("Cell: ") + cellChWl[0] + tr (", Channel: ") + cellChWl[1]
                 + tr (", Wavelength: ") + cellChWl[2] + tr (", Scan Count: ");
  ...
  QString fname
      = US_Settings::resultDir () + "/" + runID + "/" + runID + ".RI.xml";
  QFile filei (fname);
...
#if 1
  QMessageBox::information (this, tr ("Speed Information"), msg);
#endif
#if 0
...
void
US_RunDetails2::plot_interval (void)
{
  plotType = INTERVAL;
  update (lw_triples->currentRow ());
}
...
void
US_RunDetails2::plot_combined (void)
{
  plotType = COMBINED;
  update (lw_triples->currentRow ());
}
...
QString
US_RunDetails2::moreSpeedInfo (double rpm)
{
  QString msg = "";
  QString runID = le_runID->text ();
  QString fname
              ...
              if (qAbs (rpm_x - rpm) < 200.0)
                {
                  ...
                  qDebug () << " srd:mSI:M:  rpm_s rpm_a rpm_d" << rpm_s
                            << rpm_a << rpm_d;
                  break;
                }
            }
...
US_RunDetails2::get_params_public ()
{
