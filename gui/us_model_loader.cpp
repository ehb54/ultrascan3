...
void
US_ModelLoader::build_dialog (void)
{
  ...
  le_investigator
      = us_lineedit (number + US_Settings::us_inv_name (), 0, true);
  ...
  do_unasgn = false;
  do_edlast = !can_edit;
  qDebug () << "Bld: do_edit" << do_edit << " do_edlast" << do_edlast
            << "edGUID" << edGUID;
  ...
  QGridLayout *lo_unasgn = us_checkbox (tr ("Edit-Unassigned Only"), ck_unasgn,
                                        do_unasgn || do_manual);
  QGridLayout *lo_edlast
      = us_checkbox (tr ("Last Edits Only"), ck_edlast, false);
...
void
US_ModelLoader::build_dialog_auto (QString invID_passed)
{
  qDebug () << "ML:BD: runIDs empty" << runIDs.isEmpty ();
  setWindowTitle (multi ? tr ("Load Distribution Model(s)")
                        : tr ("Load Distribution Model"));
...
int
US_ModelLoader::load_model (US_Model &model, int index)
{
  int rc = 0;
...
QString
US_ModelLoader::description (int index)
{
  ...
  if (model_descriptions[index].iterations > 1)
    cdesc
        = cdesc + sep + QString::number (model_descriptions[index].iterations);
...
void
US_ModelLoader::update_person (int ID)
{
...
US_ModelLoader::list_models ()
{
              ...
              while (dbP->next ())
                {
                  ...
                  // qDebug() << "   desc" << desc.description << "DB_id" <<
                  // desc.DB_id;
                  ...
                  if (do_manual)
                    { // If MANUAL, select only type Manual
                      ...
                      if (desc.description.contains ("2DSA")
                        ...
                        skip_it = true;
                      qDebug () << "   (m)desc" << desc.description
                                << "skip_it" << skip_it;
                    }
                    ...
                    { // If UnAssigned, select only type Manual/Custom/Global
                      skip_it = (desc.editGUID != uaGUID);
                      qDebug () << "   (u)desc" << desc.description
                                << "skip_it" << skip_it;
                    }
                      ...
                      if (desc.description.contains ("_mcN"))
                        {
                          ...
                          qDebug ()
                              << "     desc.iters(nimods)" << desc.iterations;
                        }
                ...
                { // Search XML elements until we find "model"
                  ...
                  if (xml.isStartElement () && xml.name () == "model")
                    { // Pick up model attributes for description
                      ...
                      qDebug () << "  nimods" << nimods << "edGUID" << edGUID
                                << "iters" << iters;
                      if (iters > 0)
                        {
                          nimods++;
                      ...
                      if (do_manual)
                        ...
                        { // If UnAssigned, select only type
                      ...
                      if (iters > 0)
                        { // Accumulate counts for MC models
                          ...
                          if (desc.description.contains ("_mc0001"))
                            kmmold++;
                          qDebug () << "      kmmmod" << kmmmod << "kmmold"
                                    << kmmold;
                          // if(kmmold==0) qDebug() << "       desc=" <<
                          // desc.description;
                        }
  ...
  if (kmmold > 0)
    {
      QString msg = tr ("%1 MC model sets are old-style separate models\n"
                        ...
                        .arg (kmmnew)
                        .arg (kmmmod);
      QMessageBox::information (this, tr ("Deprecated MC Model Types Exist"),
                                msg);
    }

  // possibly pare down models list based on search field
  QDateTime time5 = QDateTime::currentDateTime ();
  qDebug () << "Timing: Time5" << time0.msecsTo (time5)
            << time2.msecsTo (time5);
  ...
  qDebug () << "Timing: Time6" << time0.msecsTo (time6)
            << time2.msecsTo (time6);
...
// for GMP
void
US_ModelLoader::list_models_auto (QString invID_passed)
{
  qDebug () << "LIST_MODELS";
  QDateTime time0 = QDateTime::currentDateTime ();
  QDateTime time1 = QDateTime::currentDateTime ();
...
void
US_ModelLoader::accepted_auto (QStringList m_t_r_id)
{
  QList<ModelDesc> allmods = model_descriptions;
  // QList< QListWidgetItem* > selmods = lw_models->selectedItems();
      ...
      if (model_text.contains (model_passed)
          ...
          && model_text.contains (runid_passed) && modelID == modelid_passed)
        {
          ...
          qDebug () << "In accepted_auto(): model_passed, triple_passed, "
                    ...
                    << model_passed << triple_passed << runid_passed
                    << model_text << modelID;
        }
    }
...
bool
US_ModelLoader::eventFilter (QObject *obj, QEvent *e)
{
...
int
US_ModelLoader::modelIndex (QString mdesc, QList<ModelDesc> mds)
{
  // int mdx = 0;
  int mdx = -1;
...
void
US_ModelLoader::show_model_info (QPoint pos)
{
  ...
  if (modelsCount < 2)
    ...
    { // multiple rows selected:  build multiple-model information text
      ...
      dtext = tr ("Common Model Information   ( ")
              + QString::number (modelsCount) + tr (" models ):");
...
void
US_ModelLoader::records_list (void)
{
...
void
US_ModelLoader::singles_list (void)
{
...
void
US_ModelLoader::msearch (const QString &search_string)
{
...
void
US_ModelLoader::change_single (bool cksing)
{
  do_single = cksing;
...
void
US_ModelLoader::change_edit (bool ckedit)
{
  do_edit = ckedit;
...
void
US_ModelLoader::change_unasgn (bool ckunasgn)
{
  do_unasgn = ckunasgn;
...
void
US_ModelLoader::change_edlast (bool ckedla)
{
  do_edlast = ckedla;
...
QString
US_ModelLoader::alt_description (QString &descr, const bool from_mdesc)
{
  QString adescr;
  ...
  if (atimx < 11 || atimx > 21)
    ...
    { // Input is list form of description, output is the model description
      ...
      QString fiter = QString (fanal).section ("_", -2, -1);
      adescr = frun + "." + ftrip + "." + fedan + "_" + ftype + "_" + fiter
               + ".model";
