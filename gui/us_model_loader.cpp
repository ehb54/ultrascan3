...
US_ModelLoader::US_ModelLoader(bool dbSrc, QString& search, US_Model& amodel,
                               QString& adescr, const QString eGUID)
...
US_ModelLoader::US_ModelLoader(bool dbSrc, QString& search, US_Model& amodel,
                               QString& adescr, QStringList& arunIDs)
    : US_WidgetsDialog(0, 0),
      loadDB(dbSrc),
      dsearch(search),
      omodel(amodel),
...
void US_ModelLoader::build_dialog(void) {
  ...
  do_unasgn = false;
  do_edlast = !can_edit;
  qDebug() << "Bld: do_edit" << do_edit << " do_edlast" << do_edlast << "edGUID"
           << edGUID;

  if (!dsearch.isEmpty()) {  // If an input search string is given, look for
                             // special flags
  ...
  qDebug() << "Bld: single" << do_single << " manual" << do_manual << " edit"
           << do_edit << " unasgn" << do_unasgn << " dsearch" << dsearch;

  le_mfilter = us_lineedit(dsearch, -1, false);
  connect(le_mfilter, SIGNAL(returnPressed()), this, SLOT(list_models()));
  connect(le_mfilter, SIGNAL(textChanged(const QString&)), this,
          SLOT(msearch(const QString&)));
  ...
  if (multi) lw_models->setSelectionMode(QAbstractItemView::ExtendedSelection);
  ...
  QGridLayout* lo_single =
      us_checkbox(tr("Monte Carlo Singles"), ck_single, do_single);
  QGridLayout* lo_edit =
      us_checkbox(tr("Filter by Edit/Run"), ck_edit, do_edit || do_run);
  QGridLayout* lo_unasgn = us_checkbox(tr("Edit-Unassigned Only"), ck_unasgn,
                                       do_unasgn || do_manual);
...
int US_ModelLoader::load_model(US_Model& model, int index) {
  ...
  if (do_single) {  // If selecting single from MC composite, break it out now
    ...
    for (int jj = 0; jj < nxmls;
         jj++) {  // Loop through iteration contents looking for match
      QString mcont = xmls[jj];
      int kk = mcont.indexOf("description=");
      QString mdescr = QString(mcont).mid(kk, 100).section('"', 1, 1);
      if (mdescr ==
          sdescr) {  // Found a matching description:  load the single and break
        model.load_string(mcont);
        qDebug() << "LdM:     match at jj" << jj << "ncomp"
...
QString US_ModelLoader::description(int index) {
  ...
  QString cdesc = sep + model_descriptions[index].description + sep +
                  model_descriptions[index].filename + sep +
                  model_descriptions[index].modelGUID + sep +
                  model_descriptions[index].DB_id + sep +
...
void US_ModelLoader::list_models() {
  ...
  QString mflt1 = mfilt;
  if (!mfilt.isEmpty()) {
    if (!mfilt.startsWith("*")) mflt1 = "*" + mflt1;
    if (!mfilt.endsWith("*")) mflt1 = mflt1 + "*";
  }
  ...
  qDebug() << "LM: desc single edit" << listdesc << listsing << listedit
           << "editGUID" << editGUID << "nruns" << runIDs.size() << "mflt1"
           << mflt1;
  int kmmnew = 0;
  int kmmold = 0;
  int kmmmod = 0;
  ...
  if (listdesc) {  // filter is not empty
    ...
    if (listedit && !can_edit &&
        !can_run) {  // disallow edit filter if no edit GUID and no runs have
                     // been given
      QMessageBox::information(
          this, tr("Edit GUID Problem"),
  ...
  qDebug() << "  listdesc listedit listsing" << listdesc << listedit
           << listsing;
  ...
  if (loadDB) {  // Model list from DB
    ...
    if (countRD == 0 || kid1 != db_id1 ||
        kid2 !=
            db_id2) {  // only re-fetch all-models list if we don't yet have it
      ...
      for (int ii = 0; ii < kruns; ii++) {
        ...
        while (dbP->next()) {
          ...
          desc.filename.clear();
          desc.reqGUID =
              desc.description.section(".", -2, -2).section("_", 0, -2);
          desc.iterations = (desc.description.contains("-MC") &&
                             desc.description.contains("_mc"))
                                ? 1
                                : 0;
          bool skip_it = false;
          ...
          if (desc.description.simplified().length() < 2) {
            desc.description =
                " ( ID " + desc.DB_id + tr(" : empty description )");
          }
          ...
          if (do_manual) {  // If MANUAL, select only type Manual
            ...
            if (desc.description.contains("2DSA") ||
              ...
              skip_it = true;
            qDebug() << "   (m)desc" << desc.description << "skip_it"
                     << skip_it;
          }

          else if (do_unasgn) {  // If UnAssigned, select only type
                                 // Manual/Custom/Global
            skip_it = (desc.editGUID != uaGUID);
            qDebug() << "   (u)desc" << desc.description << "skip_it"
                     << skip_it;
          }
      ...
      qDebug() << "a_m size" << model_descrs_recs.size() << "m_d size"
               << model_descriptions.size();
      ...
      qDebug() << "Timing: DB-read" << time0.msecsTo(time3)
               << time2.msecsTo(time3);
      qDebug() << "Timing: Compress" << time3.msecsTo(time4)
               << time2.msecsTo(time4);
    }
    ...
    if (f_names.size() != model_descrs_recs.size() ||
        !listsing) {  // only re-fetch all-models list if we don't yet have it
      ...
      for (int ii = 0; ii < f_names.size(); ii++) {
        ...
        while (!xml.atEnd()) {  // Search XML elements until we find "model"
          ...
          if (xml.isStartElement() &&
              xml.name() ==
                  "model") {  // Pick up model attributes for description
            attr = xml.attributes();
            ...
            qDebug() << "  nimods" << nimods << "edGUID" << edGUID << "iters"
                     << iters;
            if (iters > 0) {
              nimods++;
            ...
            if (do_manual) {  // If MANUAL, select only type Manual
              ...
              if (iaType != US_Model::MANUAL) continue;
            }
            ...
            if (iters > 0) {  // Accumulate counts for MC models
              kmmmod++;
              if (desc.description.contains("_mc0001")) kmmold++;
            ...
            if (iters == 0) break;
                   ...
                   xml.name() ==
                       "ModelData") {  // Handle end of XML for possible
                                       // multi-model composite
            if (nimods >
                1) {  // If composite MC, get and delete last description
              ModelDesc desc = model_descrs_recs.takeLast();
              int mcndx = desc.description.indexOf("_mc0");
              desc.description = QString(desc.description).left(mcndx) +
  ...
  if (do_edlast) {  // Pare down models list to only the last edit of each run
                    // triple
    select_edlast();
  }
  ...
  if (model_descriptions.size() > 0) {
    for (int ii = 0; ii < model_descriptions.size();
         ii++) {  // propagate list widget with descriptions
...
// List model choices (disk/db and possibly filtered by search text): mod. copy
// for GMP
...
void US_ModelLoader::accepted() {
  ...
  if (modelsCount > 0) {  // loop through selections
    ...
    for (int ii = 0; ii < modelsCount;
         ii++) {  // get row of selection then index in original descriptions
                  // list
    ...
    QMessageBox::information(
        this, tr("No Model Selected"),
        tr("You have not selected a model.\nSelect+Accept or Cancel"));
    return;
  }

  qDebug() << "ACC: multi" << multi;
  if (!multi) {  // in single-select mode, load the model and set the
                 // description
    ...
    QDateTime time2 = QDateTime::currentDateTime();
    qDebug() << "Timing: accept-load: mcount" << modelsCount << "time(ms)"
             << time1.msecsTo(time2);
  }
  ...
  if (do_edit) dsearch = "=e " + dsearch;
  if (do_single) dsearch = "=s " + dsearch;
  if (do_unasgn) dsearch = "=u " + dsearch;
  if (do_manual) dsearch = "=m " + dsearch;
...
void US_ModelLoader::accepted_auto(QStringList m_t_r_id) {
  ...
  qDebug() << "In accepted_auto() 1: passed vals -- " << model_passed
           << triple_passed << runid_passed << modelid_passed;
  ...
  for (int i = 0; i < lw_models->count(); ++i) {
    ...
    if (model_text.contains(model_passed) &&
        ...
        model_text.contains(runid_passed) && modelID == modelid_passed) {
      selmods << item;
      qDebug() << "In accepted_auto(): model_passed, triple_passed, "
                  "runid_passed, model_text, modelID -- "
...
int US_ModelLoader::modelIndex(QString mdesc, QList<ModelDesc> mds) {
  ...
  for (int jj = 0; jj < mds.size();
       jj++) {  // search for matching description and save its index
...
void US_ModelLoader::show_model_info(QPoint pos) {
  ...
  if (modelsCount <
      2) {  // 1 or no rows selected:  build information for single model
    ...
    dtext = tr("Common Model Information   ( ") + QString::number(modelsCount) +
            tr(" models ):");
    ...
    for (int jj = 0; jj < modelsCount; jj++) {
      ...
      dtext =
          ...
          tr("\n  Description Global ID: ") +
          model_descriptions[mdx].modelGUID +
          tr("\n  Edit Global ID:        ") + model.editGUID +
          tr("\n  Request Global ID:     ") + model.requestGUID + lblid +
...
void US_ModelLoader::singles_list(void) {
  ...
  for (int ii = 0; ii < model_descrs_recs.size();
       ii++) {  // Duplicate/expand each model record to the unfiltered models
                // list
    ...
    if (mindx < 0) {  // For non-MC-composite, just duplicate
      ...
      for (int jj = 1; jj <= niters;
           jj++) {  // Create and append a description for each iteration model
...
void US_ModelLoader::select_edlast() {
  ...
  for (int jj = 0; jj < model_descriptions.size();
       jj++) {  // Accumulate last-edit triples and edit-times for list
    ...
    QString edstr =
        mdesc.section(".", 2, 2).section("_", 0, 0);  // Edit timestamp
    ...
    if (prx >= 0) {  // A previous match to run-triple exists -- compare times
      ...
      if (etimd >
          etiml) {  // Current desc time later, so replace list triple and time
        rtrips.removeAt(prx);
        edtims.removeAt(prx);
        lendxs.removeAt(prx);
        rtrips << rtrip;
        edtims << edtim;
        lendxs << jj;
      } else if (etimd == etiml) {  // Matching edit time, so assume different
                                    // analysis and save
        rtrips << rtrip;
        edtims << edtim;
        lendxs << jj;
  ...
  if (rtrips.count() <
      model_descriptions
          .count()) {  // Some earlier edits were removed, so re-do models list
    ...
    qDebug() << "select_edlast -- full,pared counts" << mdescrs_work.count()
             << model_descriptions.count();
  }
}
...
QString US_ModelLoader::alt_description(QString& descr, const bool from_mdesc) {
  ...
  if (atimx < 11 ||
      atimx >
          21) {  // Not standard analysis form, so input and output are the same
    adescr = descr;
  }

  else if (from_mdesc) {  // Input is the raw model description, output is list
                          // form
    //   i.e., "TYPE.TRIP.ETIME_ATIME_HPCID_ITER.RUN"
    ...
    QString fiter = QString(fanal).section("_", -2, -1);
    adescr =
        frun + "." + ftrip + "." + fedan + "_" + ftype + "_" + fiter + ".model";
