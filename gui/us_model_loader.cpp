...
US_ModelLoader::US_ModelLoader(bool dbSrc, QString &search, US_Model &amodel, QString &adescr, const QString eGUID) :
    US_WidgetsDialog(0, 0), loadDB(dbSrc), dsearch(search), omodel(amodel), odescr(adescr), omodels(wmodels),
    odescrs(mdescrs), runIDs(wrunIDs) {
   multi = false;
   editGUID = eGUID;
...
US_ModelLoader::US_ModelLoader(bool dbSrc, QString &search, US_Model &amodel, QString &adescr, QStringList &arunIDs) :
    US_WidgetsDialog(0, 0), loadDB(dbSrc), dsearch(search), omodel(amodel), odescr(adescr), omodels(wmodels),
    odescrs(mdescrs), runIDs(arunIDs) {
   multi = false;
   editGUID = "";
...
US_ModelLoader::US_ModelLoader(
   bool dbSrc, QString &search, QList<US_Model> &amodels, QStringList &adescrs, QStringList &arunIDs) :
    US_WidgetsDialog(0, 0), loadDB(dbSrc), dsearch(search), omodel(model), odescr(search), omodels(amodels),
    odescrs(adescrs), runIDs(arunIDs) {
   multi = true;
   editGUID = "";
...
US_ModelLoader::US_ModelLoader(bool dbSrc, QString &search, QList<US_Model> &amodels, QStringList &adescrs) :
    US_WidgetsDialog(0, 0), loadDB(dbSrc), dsearch(search), omodel(model), odescr(search), omodels(amodels),
    odescrs(adescrs), runIDs(wrunIDs) {
   multi = true;
   editGUID = "";
...
void US_ModelLoader::build_dialog(void) {
   ...
   setWindowTitle(multi ? tr("Load Distribution Model(s)") : tr("Load Distribution Model"));
   ...
   int iload = loadDB ? US_Disk_DB_Controls::DB : US_Disk_DB_Controls::Disk;
   dkdb_cntrls = new US_Disk_DB_Controls(iload);

   connect(dkdb_cntrls, SIGNAL(changed(bool)), this, SLOT(select_diskdb()));

   QPalette gray = US_GuiSettings::editColor();
   gray.setColor(QPalette::Base, QColor(0xe0, 0xe0, 0xe0));

   pb_investigator = us_pushbutton(tr("Select Investigator"));
   connect(pb_investigator, SIGNAL(clicked()), this, SLOT(get_person()));

   if (US_Settings::us_inv_level() < 3)
      pb_investigator->setEnabled(false);
   int inv = US_Settings::us_inv_ID();
   QString number = (inv > 0) ? QString::number(inv) + ": " : "";
   le_investigator = us_lineedit(number + US_Settings::us_inv_name(), 0, true);

   pb_filtmodels = us_pushbutton(tr("Search"));
   connect(pb_filtmodels, SIGNAL(clicked()), this, SLOT(list_models()));
   ...
   if (!dsearch.isEmpty()) { // If an input search string is given, look for special flags
   ...
   qDebug() << "Bld: single" << do_single << " manual" << do_manual << " edit" << do_edit << " unasgn" << do_unasgn
            << " dsearch" << dsearch;

   le_mfilter = us_lineedit(dsearch, -1, false);
   connect(le_mfilter, SIGNAL(returnPressed()), this, SLOT(list_models()));
   connect(le_mfilter, SIGNAL(textChanged(const QString &)), this, SLOT(msearch(const QString &)));
   ...
   QGridLayout *lo_single = us_checkbox(tr("Monte Carlo Singles"), ck_single, do_single);
   QGridLayout *lo_edit = us_checkbox(tr("Filter by Edit/Run"), ck_edit, do_edit || do_run);
   QGridLayout *lo_unasgn = us_checkbox(tr("Edit-Unassigned Only"), ck_unasgn, do_unasgn || do_manual);
   QGridLayout *lo_edlast = us_checkbox(tr("Last Edits Only"), ck_edlast, false);
   ...
   connect(ck_single, SIGNAL(toggled(bool)), SLOT(change_single(bool)));
   connect(ck_edit, SIGNAL(toggled(bool)), SLOT(change_edit(bool)));
   connect(ck_unasgn, SIGNAL(toggled(bool)), SLOT(change_unasgn(bool)));
   connect(ck_edlast, SIGNAL(toggled(bool)), SLOT(change_edlast(bool)));
...
int US_ModelLoader::load_model(US_Model &model, int index) {
   ...
   if (do_single) { // If selecting single from MC composite, break it out now
      US_Model model2 = model;
      QString sdescr = model_descriptions[ index ].description;
      QStringList xmls;
      int nxmls = model2.mc_iter_xmls(xmls);

      for (int jj = 0; jj < nxmls; jj++) { // Loop through iteration contents looking for match
         QString mcont = xmls[ jj ];
         int kk = mcont.indexOf("description=");
         QString mdescr = QString(mcont).mid(kk, 100).section('"', 1, 1);
         if (mdescr == sdescr) { // Found a matching description:  load the single and break
...
QString US_ModelLoader::description(int index) {
   ...
   QString cdesc = sep + model_descriptions[ index ].description + sep + model_descriptions[ index ].filename + sep
                   + model_descriptions[ index ].modelGUID + sep + model_descriptions[ index ].DB_id + sep
                   + model_descriptions[ index ].editGUID;

   if (model_descriptions[ index ].iterations > 1)
      cdesc = cdesc + sep + QString::number(model_descriptions[ index ].iterations);
...
void US_ModelLoader::list_models() {
   ...
   qDebug() << "LM: desc single edit" << listdesc << listsing << listedit << "editGUID" << editGUID << "nruns"
            << runIDs.size() << "mflt1" << mflt1;
   int kmmnew = 0;
   int kmmold = 0;
   int kmmmod = 0;

   if (listdesc) { // filter is not empty
      listedit = listedit | do_edit;

      if (listedit && !can_edit && !can_run) { // disallow edit filter if no edit GUID and no runs have been given
         QMessageBox::information(this, tr("Edit GUID Problem"), tr("No EditGUID/runIDs given.\nEdit filter turned off."));
         listedit = false;
         listdesc = listsing;
         listall = !listdesc;
      }

      if (listsing) { // if showing MC singles, re-check for filtering
         if (!singprev)
   ...
   if (loadDB) { // Model list from DB
      ...
      if (countRD == 0 || kid1 != db_id1 || kid2 != db_id2) { // only re-fetch all-models list if we don't yet have it
         ...
         for (int ii = 0; ii < kruns; ii++) {
            ...
            if (listedit && can_edit) {
               query << "get_model_desc_by_editID" << invID << editID;
            }

            else if (listedit && can_run) {
               query << "get_model_desc_by_runID" << invID << runIDs[ ii ];
            }
            ...
            while (dbP->next()) {
               ...
               desc.filename.clear();
               desc.reqGUID = desc.description.section(".", -2, -2).section("_", 0, -2);
               desc.iterations = (desc.description.contains("-MC") && desc.description.contains("_mc")) ? 1 : 0;
               bool skip_it = false;

               if (desc.description.simplified().length() < 2) {
                  desc.description = " ( ID " + desc.DB_id + tr(" : empty description )");
               }
               //qDebug() << "   desc" << desc.description << "DB_id" << desc.DB_id;

               if (do_manual) { // If MANUAL, select only type Manual
                  skip_it = (desc.editGUID != uaGUID);

                  if (
                     desc.description.contains("2DSA") || desc.description.contains("PCSA")
                     || desc.description.contains("GA") || desc.description.contains("-GL")
                     || desc.description.contains("Custom"))
                     skip_it = true;
                  qDebug() << "   (m)desc" << desc.description << "skip_it" << skip_it;
               }

               else if (do_unasgn) { // If UnAssigned, select only type Manual/Custom/Global
                  skip_it = (desc.editGUID != uaGUID);
         ...
         qDebug() << "a_m size" << model_descrs_recs.size() << "m_d size" << model_descriptions.size();
   ...
   else { // Models from local disk files
      QDir dir;
      ...
      if (f_names.size() != model_descrs_recs.size() || !listsing) { // only re-fetch all-models list if we don't yet have it
         ...
         for (int ii = 0; ii < f_names.size(); ii++) {
            ...
            while (!xml.atEnd()) { // Search XML elements until we find "model"
               xml.readNext();

               if (xml.isStartElement() && xml.name() == "model") { // Pick up model attributes for description
                  attr = xml.attributes();
                  ...
                  if ((do_edit && edGUID != editGUID) || (do_run && !runIDs.contains(runID)))
                     continue;
                  ...
                  if (desc.description.simplified().length() < 2) {
                     desc.description = " ( GUID " + desc.modelGUID.left(8) + "... : " + tr("empty description )");
                  }
                  //*DEBUG
                  //if (!listall) {
                  ...
                  if (do_manual) { // If MANUAL, select only type Manual
                     ...
                     //qDebug() << "   iaType igType" << iaType << igType;
                     if (
                        iaType != US_Model::MANUAL && iaType != US_Model::CUSTOMGRID && iaType != US_Model::DMGA_CONSTR
                        && igType != US_Model::GLOBAL && igType != US_Model::SUPERGLOBAL)
                        continue;
                  }

                  if (iters > 0) { // Accumulate counts for MC models
                     kmmmod++;
               ...
               else if (xml.isEndElement() && xml.name() == "ModelData") { // Handle end of XML for possible multi-model composite
                  if (nimods > 1) { // If composite MC, get and delete last description
                     ModelDesc desc = model_descrs_recs.takeLast();
                     int mcndx = desc.description.indexOf("_mc0");
                     desc.description = QString(desc.description).left(mcndx) + QString().sprintf("_mcN%03i", nimods)
   ...
   if (kmmold > 0) {
      QString msg = tr("%1 MC model sets are old-style separate models\n"
                       ...
                       .arg(kmmnew)
                       .arg(kmmmod);
      QMessageBox::information(this, tr("Deprecated MC Model Types Exist"), msg);
   }
   ...
   if (do_edlast) { // Pare down models list to only the last edit of each run triple
      select_edlast();
   }
   ...
   if (model_descriptions.size() > 0) {
      for (int ii = 0; ii < model_descriptions.size(); ii++) { // propagate list widget with descriptions
...
void US_ModelLoader::accepted() {
   ...
   if (modelsCount > 0) { // loop through selections
      model_descriptions.clear();

      for (int ii = 0; ii < modelsCount; ii++) { // get row of selection then index in original descriptions list
      ...
      QMessageBox::information(
         this, tr("No Model Selected"), tr("You have not selected a model.\nSelect+Accept or Cancel"));
      return;
   }

   qDebug() << "ACC: multi" << multi;
   if (!multi) { // in single-select mode, load the model and set the description
   ...
   else { // in multiple-select mode, load all models and descriptions
      omodels.clear();
      odescrs.clear();
      ...
      QDateTime time2 = QDateTime::currentDateTime();
      qDebug() << "Timing: accept-load: mcount" << modelsCount << "time(ms)" << time1.msecsTo(time2);
   }
...
void US_ModelLoader::accepted_auto(QStringList m_t_r_id) {
   ...
   for (int i = 0; i < lw_models->count(); ++i) {
      ...
      if (
         model_text.contains(model_passed) && model_text.contains(triple_passed) && model_text.contains(runid_passed)
...
bool US_ModelLoader::eventFilter(QObject *obj, QEvent *e) {
   if (obj == lw_models && e->type() == QEvent::ContextMenu) {
   ...
   else { // pass all other events to normal handler
...
int US_ModelLoader::modelIndex(QString mdesc, QList<ModelDesc> mds) {
   ...
   for (int jj = 0; jj < mds.size(); jj++) { // search for matching description and save its index
...
void US_ModelLoader::show_model_info(QPoint pos) {
   ...
   if (modelsCount < 2) { // 1 or no rows selected:  build information for single model

      if (modelsCount == 1) { // info on selected model
         row = lw_models->row(selmods[ 0 ]);
         lmdesc = selmods[ 0 ]->text();
      }

      else { // info on model at right-click row
      ...
      mdesc = mdesc.length() < 50 ? mdesc : (mdesc.left(50) + "\n                         " + mdesc.mid(50));
      ncomp = model.components.size();
      nassoc = model.associations.size();
      tdesc = model.typeText();
      iters = !model.monteCarlo ? 0 : model_descriptions[ mdx ].iterations;
      anlid = model_descriptions[ mdx ].reqGUID;
      runid = mdesc.section(".", 0, -4);
      int jts = runid.indexOf("_" + tdesc);
      runid = jts > 0 ? runid.left(jts) : runid;
      mdlid = frDisk ? model_descriptions[ mdx ].filename : // ID is filename
                 model_descriptions[ mdx ].DB_id; // ID is DB id
      mdlid = mdlid.length() < 50 ? mdlid : "*/" + mdlid.section("/", -3, -1); // short filename

      dtext = tr("Model Information:") + tr("\n  Description:           ") + mdesc + tr("\n  Implied RunID:         ")
              + runid + tr("\n  Type:                  ") + tdesc + "  (" + QString::number(( int ) mtype) + ")"
              + tr("\n  Model Global ID:       ") + model.modelGUID + tr("\n  Description Global ID: ")
              + model_descriptions[ mdx ].modelGUID + tr("\n  Edit Global ID:        ") + model.editGUID
              + tr("\n  Request Global ID:     ") + model.requestGUID + lblid + mdlid
              + tr("\n  Iterations:            ") + QString::number(iters) + tr("\n  Components Count:      ")
              + QString::number(ncomp) + tr("\n  Associations Count:    ") + QString::number(nassoc)
              + tr("\n  List Row:              ") + QString::number(row) + tr("\n  Analysis Run ID:       ") + anlid
              + "";
   }

   else { // multiple rows selected:  build multiple-model information text
      QString aruni;
      QString atype;
      QString aegid;
      QString eguid;
      ...
      dtext = tr("Common Model Information   ( ") + QString::number(modelsCount) + tr(" models ):");
      ...
      for (int jj = 0; jj < modelsCount; jj++) {
         ...
         mdlid = frDisk ? model_descriptions[ mdx ].filename : // ID is filename
                    model_descriptions[ mdx ].DB_id; // ID is DB id
         mdlid = mdlid.length() < 50 ? mdlid : "*/" + mdlid.section("/", -3, -1); // short filename
         mdlid = mdlid.length() < 50 ? mdlid : mdlid.left(23) + "..." + mdlid.right(24); // short filename
         iters = !model.monteCarlo ? 0 : model_descriptions[ mdx ].iterations;
         anlid = model_descriptions[ mdx ].reqGUID;

         dtext = dtext + tr("\n\nModel Information: (") + QString::number((jj + 1))
                 + "):" + tr("\n  Description:           ") + mdesc + tr("\n  Implied RunID:         ") + runid
                 + tr("\n  Type:                  ") + tdesc + "  (" + QString::number(( int ) mtype) + ")"
                 + tr("\n  Model Global ID:       ") + model.modelGUID + tr("\n  Description Global ID: ")
                 + model_descriptions[ mdx ].modelGUID + tr("\n  Edit Global ID:        ") + model.editGUID
...
void US_ModelLoader::singles_list(void) {
   ...
   for (int ii = 0; ii < model_descrs_recs.size();
        ii++) { // Duplicate/expand each model record to the unfiltered models list
      ModelDesc mdesc = model_descrs_recs[ ii ];
      mdesc.rec_index = ii;
      int mindx = mdesc.description.indexOf("_mcN");
      qDebug() << "SL: ii" << ii << "mindx" << mindx;

      if (mindx < 0) { // For non-MC-composite, just duplicate
         model_descrs_ufilt << mdesc;
      }

      else { // For MC-composite, expand to all the single iteration models
         ...
         for (int jj = 1; jj <= niters; jj++) { // Create and append a description for each iteration model
...
void US_ModelLoader::change_unasgn(bool ckunasgn) {
   ...
   if (do_unasgn) { // If unassigned now checked, turn off edit (unless id=1)
      do_edit = false;
...
void US_ModelLoader::select_edlast() {
   ...
   for (int jj = 0; jj < model_descriptions.size(); jj++) { // Accumulate last-edit triples and edit-times for list
      ...
      QString edstr = mdesc.section(".", 2, 2).section("_", 0, 0); // Edit timestamp
      QString rtrip = runid + "." + mtype + "." + tripl;
      QString edtim = edstr.mid(1, 10); // Edit numeric time value
      int prx = rtrips.indexOf(rtrip);
      if (prx >= 0) { // A previous match to run-triple exists -- compare times
         int etiml = edtims[ prx ].toInt();
         int etimd = edtim.toInt();
         if (etimd > etiml) { // Current desc time later, so replace list triple and time
            rtrips.removeAt(prx);
            edtims.removeAt(prx);
            lendxs.removeAt(prx);
            rtrips << rtrip;
         ...
         else if (etimd == etiml) { // Matching edit time, so assume different analysis and save
            rtrips << rtrip;
            edtims << edtim;
            lendxs << jj;
         }
      ...
      else { // No match to run-triple, so add it to "latest" list
         rtrips << rtrip;
         edtims << edtim;
         lendxs << jj;
      }
   ...
   if (rtrips.count() < model_descriptions.count()) { // Some earlier edits were removed, so re-do models list
      QList<ModelDesc> mdescrs_work;
      mdescrs_work = model_descriptions;
      model_descriptions.clear();

      for (int jj = 0; jj < mdescrs_work.count(); jj++) {
         if (lendxs.contains(jj)) { // Copy a latest-edit to the model list
            model_descriptions << mdescrs_work[ jj ];
         }
      }
      qDebug() << "select_edlast -- full,pared counts" << mdescrs_work.count() << model_descriptions.count();
   }
}
...
QString US_ModelLoader::alt_description(QString &descr, const bool from_mdesc) {
   ...
   if (atimx < 11 || atimx > 21) { // Not standard analysis form, so input and output are the same
      adescr = descr;
   }

   else if (from_mdesc) { // Input is the raw model description, output is list form
      //   i.e., "TYPE.TRIP.ETIME_ATIME_HPCID_ITER.RUN"
      ...
      adescr = ftype + "." + ftrip + "." + fedan + "_" + fiter + "." + frun;
      qDebug() << "ALT_DESC:Native-Form: " << descr;
      qDebug() << "ALT_DESC:List-Form: " << adescr;
   }

   else { // Input is list form of description, output is the model description
      //   i.e., "RUN.TRIP.ETIME_ATIME_TYPE_HPCID_ITER.model"
      ...
      QString fiter = QString(fanal).section("_", -2, -1);
      adescr = frun + "." + ftrip + "." + fedan + "_" + ftype + "_" + fiter + ".model";
