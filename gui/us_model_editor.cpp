...
US_ModelEditor::US_ModelEditor (ModelSystem &current_model, QWidget *p,
                                Qt::WindowFlags f)
    : US_WidgetsDialog (p, f), model (current_model)
{
  ...
  // Label + line edit
  QLabel *lb_diffusion
      = us_label (tr ("Diffusion Coeff. (cm<sup>2</sup>/sec):"));
  ...
  QGridLayout *prolate
      = us_radiobutton ("Prolate Ellipsoid", rb_prolate, true);
...
void
US_ModelEditor::change_component1 (int index)
{
  component = index;
...
void
US_ModelEditor::change_spinbox (int value)
{
  component = value - 1;
...
void
US_ModelEditor::update_component (void)
{
  rb_prolate->setEnabled (false);
  rb_oblate->setEnabled (false);
  rb_rod->setEnabled (false);
...
void
US_ModelEditor::update_vbar (double vbar)
{
  if (vbar > 0.0)
...
void
US_ModelEditor::simulate_component (void)
{
...
void
US_ModelEditor::select_shape (int new_shape)
{
  shape = new_shape;
  update_shape ();
}
...
void
US_ModelEditor::update_shape (void)
{
  struct SimulationComponent *sc = &model.component_vector[component];
...
US_ModelEditor::load_c0 (void)
{
  ...
  QString fn
      = QFileDialog::getOpenFileName (this, US_Settings::resultDir (), "*");
...
void
US_ModelEditor::load_model ()
{
  QString fn = QFileDialog::getOpenFileName (
      this, tr ("Load Model File"), US_Settings::resultDir (),
      "*.model.?? *.model-?.??  *model-??.??");
      ...
      if (code == 0)
        {
          ...
          QMessageBox::information (this, tr ("Simulation Module"),
...
void
US_ModelEditor::save_model (void)
{
  if (!verify_model ())
    return;

  QString fn = QFileDialog::getSaveFileName (
      this, tr ("Save model as"), US_Settings::resultDir (),
      "*.model.?? *.model-?.?? *model-??.??");
...
void
US_ModelEditor::accept_model (void)
{
  if (verify_model ())
    accept ();
}
...
void
US_ModelEditor::update_sed (const QString &text)
{
  if (text == "")
    return;
...
void
US_ModelEditor::update_diff (const QString &text)
{
  if (text == "")
    return;
...
void
US_ModelEditor::update_extinction (const QString &text)
{
  if (text == "")
    return;
...
void
US_ModelEditor::update_sigma (const QString &text)
{
  if (text == "")
    return;
...
void
US_ModelEditor::update_delta (const QString &text)
{
  if (text == "")
    return;
...
// to combine them.
void
US_ModelEditor::update_vbar (const QString &text)
{
  struct SimulationComponent *sc = &model.component_vector[component];
...
void
US_ModelEditor::update_mw (const QString &text)
{
  struct SimulationComponent *sc = &model.component_vector[component];
...
void
US_ModelEditor::update_f_f0 (const QString &text)
{
  if (text == "")
    return;
...
void
US_ModelEditor::update_conc (const QString &text)
{
  struct SimulationComponent *sc = &model.component_vector[component];
...
// to combine them
void
US_ModelEditor::update_keq (const QString &text)
{
  if (text == "")
    return;
...
void
US_ModelEditor::update_koff (const QString &text)
{
  if (text == "")
    return;
...
void
US_ModelEditor::error (const QString &message)
{
  QMessageBox::warning (this, tr ("Model Error"), message);
}

bool
US_ModelEditor::verify_model (void)
{
  bool flag = true;
      ...
      if (as->stoichiometry2 > 0 && as->stoichiometry3 != 1)
        {
          if (fabs ((*cv)[as->component2].mw
                    - ((*cv)[as->component1].mw * as->stoichiometry2
                       / as->stoichiometry1))
              > 1.0) // MWs don't match within 1 dalton
            {
              str = tr ("The molecular weights of the reacting species\n"
                    ...
                    + QString::number (as->stoichiometry2)
                    + tr (" = MW(2)\n\n")

                    + tr ("Please adjust either MW(1) or MW(2) before "
                          "proceeding...");
          ...
          if (fabs (model.component_vector[as->component3].mw
            ...
            // MWs don't match within 10 dalton
            {
              str = tr ("The molecular weights of the reacting species\n"
                    ...
                    + tr ("Please adjust the molecular weight of the "
                          "appropriate\n"
