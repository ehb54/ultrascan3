...
void US_ModelEditor::update_component(void) {
  ...
  if (sc->show_keq) {
    for (uint i = 0; i < assoc->size();
         i++) {  // only check the dissociating species
  ...
  if (sc->show_stoich > 0) {
    ...
    sc->mw =
        (*scl)[sc->show_component[0]].mw + (*scl)[sc->show_component[1]].mw;
...
void US_ModelEditor::update_sD(void) {
  ...
  sc->D =
      R * K20 / (AVOGADRO * sc->f_f0 * 9.0 * VISC_20W * M_PI * pow(base, 0.5));
...
void US_ModelEditor::save_model(void) {
  ...
  if (!fn.isEmpty()) {
    ...
    if (f.exists()) {
      int answer = QMessageBox::question(this, tr("Warning"),
                                         tr("Attention:\n"
    ...
    if (as->stoichiometry2 > 0 && as->stoichiometry3 != 1) {
      if (fabs((*cv)[as->component2].mw -
               ((*cv)[as->component1].mw * as->stoichiometry2 /
                as->stoichiometry1)) > 1.0)  // MWs don't match within 1 dalton
      {
        str = tr("The molecular weights of the reacting species\n"
              ...
              + tr("Stoichiometry of reaction ") + QString::number(i + 1) +
              tr(": MW(1) *") + QString::number(as->stoichiometry2) +
              tr(" = MW(2)\n\n")
