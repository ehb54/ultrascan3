...
US_Plot3D::US_Plot3D(QWidget *p, US_Model *m, QVector<QVector3D> *d) : QMainWindow(p, Qt::Dialog) {
   model = m;
   xyzdat = d;
   reverse_y = false;
   ...
   QFont font = QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(), QFont::Normal);
   ...
   dataWidget->setTitleFont(titleFont.family(), titleFont.pointSize(), titleFont.weight());
   if (model != nullptr) {
...
// Public function to set dimension value types and calculate data ranges
void US_Plot3D::setTitles(QString wndt, QString pltt, QString xat, QString yat, QString zat) {
   const double VROUND = 10.0;
   const double MAX_ANNO = (99.9 / VROUND);
//   const double MAX_ANNO = (999.0/VROUND);
#if 0
...
void US_Plot3D::setParameters(
   double z_scale_in, double a_gridr_in, double a_alpha_in, double a_beta_in, double x_scale_in, double y_scale_in) {
...
// Public function to set internal variables from plot control parameters
void US_Plot3D::setParameters(
   int a_gridy, int a_gridx, double x_scale_in, double y_scale_in, double z_scale_in, double a_alpha, double a_beta) {
   // Get the variables set in the plot control dialog
...
void US_Plot3D::reloadData(QVector<QVector3D> *d) {
   ...
   for (int ii = 0; ii < nidpt; ii++) { // Accumulate unique X values so that we can count rows
   ...
   DbgLv(1) << "P3D:rD:  x0 x1 xm xn" << xvals[ 0 ] << xvals[ 1 ] << xvals[ nrows - 2 ] << xvals[ nrows - 1 ];
...
void US_Plot3D::calculatePoints() {
   ...
   double zfact = zscale;
   DbgLv(1) << "cP:  dfac xdif ydif" << dfac << xdif << ydif << "xpinc ypinc zfact" << xpinc << ypinc << zfact;
   ...
   xdif = acos(pow(1e-18, (1.0 / alpha))) * beta / (M_PI * 0.5 * sqrt(2.0)); // max xy-diff at small zpkf
   ...
   for (int kk = 0; kk < nidpt; kk++) { // calculate spread of each data point to a radius of raster points
      ...
      for (int ii = fx; ii < lx; ii++) { // find square of difference of y-raster and y-data
         xdif = sq(( double ) ii / xpinc - xval);
         //ydif       = (double)sq( ( ii - ry ) );

         //for ( int jj = fx; jj < lx; jj++ )
         for (int jj = fy; jj < ly; jj++) { // find square of difference of y-raster and y-data
...
void US_Plot3D::calculateData(QVector<QVector<double>> &zdat) {
   ...
   for (int kk = 0; kk < data_points; kk++) { // calculate spread of each model point to a radius of raster points
      ...
      for (int ii = fx; ii < lx; ii++) { // find square of difference of x-raster and x-model
         xdif = sq(( double ) ii / xpinc - xval);

         for (int jj = fy; jj < ly; jj++) { // find square of difference of y-raster and y-model
            ydif = sq(( double ) jj / ypinc - yval);
...
void US_Plot3D::replot() {
   ...
   for (int ii = 0; ii < ncols; ii++) { // copy data to work 2D vector and get new z-max
      if ((ii & 63) == 1)
   ...
   dataWidget->createCoordinateSystem(Triple(xmin, ymin, zmin), Triple(xmax, ymax, zmax));
   ...
   dataWidget->coordinates()->setLabelFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(), QFont::Bold);
   dataWidget->coordinates()->setLabelColor(Qwt3D::RGBA(0, 0, 0, 1));
   dataWidget->coordinates()->setNumberFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize());
   ...
   DbgLv(2) << "P3D:rP:  xmin xmax" << xmin << xmax << " ymin ymax" << ymin << ymax << " xscl yscl" << x_scale
            << y_scale;
   if (x_scale > 4.0) {
      x_scale /= 4.0;
   }
...
void US_Plot3D::replot(bool hold_color) {
   ...
   if (triples_in) {
      ...
      if (hold_color) { // If hold-colors option is on, use point with max to hold colors
      ...
      int m = nrows - 1;
      int n = ncols - 1;
      DbgLv(1) << "P3D:replot: wdata[0][0]" << wdata[ 0 ][ 0 ].x << wdata[ 0 ][ 0 ].y << wdata[ 0 ][ 0 ].z;
      DbgLv(1) << "P3D:replot: wdata[n][0]" << wdata[ n ][ 0 ].x << wdata[ n ][ 0 ].y << wdata[ n ][ 0 ].z;
      DbgLv(1) << "P3D:replot: wdata[0][m]" << wdata[ 0 ][ m ].x << wdata[ 0 ][ m ].y << wdata[ 0 ][ m ].z;
      DbgLv(1) << "P3D:replot: wdata[n][m]" << wdata[ n ][ m ].x << wdata[ n ][ m ].y << wdata[ n ][ m ].z;
      ...
      dataWidget->createCoordinateSystem(Triple(xcmin, ycmin, zcmin), Triple(xcmax, ycmax, zcmax));
      DbgLv(1) << "P3D:replot:  xmin xmax ycmin ycmax" << xmin << xmax << ycmin << ycmax << "zcmin zcmax" << zcmin
               << zcmax;
   }
      ...
      DbgLv(1) << "P3D:ld:  xmin xmax ycmin ycmax" << xmin << xmax << ycmin << ycmax << "zmin zmax zfac zdmx" << zmin
               << zmax << zfac << zdmx;

      // Load the widget raster data
      dataWidget->loadFromData(wddat, kcols, krows, xmin, xmax, ycmin, ycmax);
      dataWidget->createCoordinateSystem(Triple(xmin, ycmin, zmin), Triple(xmax, ycmax, zmax));
   }
   ...
   DbgLv(2) << "P3D:rP:  xmin xmax" << xmin << xmax << " ymin ymax" << ymin << ymax << " zmin zmax" << zmin << zmax
...
void US_Plot3D::createActions() {
   ...
   openffAct = new QAction(QIcon(":/images/filecell.png"), tr("Open FEM File"), this);
   saveAct = new QAction(QIcon(":/images/savecontent.png"), tr("Dump Contents"), this);
   ...
   wirefrAct = new QAction(QIcon(":/images/wireframe.png"), tr("Wire Frame"), this);
   hiddlnAct = new QAction(QIcon(":/images/hiddenline.png"), tr("Hidden Line"), this);
   polygnAct = new QAction(QIcon(":/images/polygon.png"), tr("Polygon only"), this);
   fdmeshAct = new QAction(QIcon(":/images/filledmesh.png"), tr("Filled Mesh"), this);

   nodataAct = new QAction(QIcon(":/images/nodata.png"), tr("No Data"), this);
   fldataAct = new QAction(QIcon(":/images/floordata.png"), tr("Floor Data"), this);
   flisolAct = new QAction(QIcon(":/images/flooriso.png"), tr("Floor Isolines"), this);
   flemptAct = new QAction(QIcon(":/images/floorempty.png"), tr("Floor Empty"), this);

   normsAct = new QAction(QIcon(":/images/normals.png"), tr("Normal Vectors"), this);
...
double US_Plot3D::comp_value(US_Model::SimulationComponent *sc, int type, double normscl) {
   double xyval = sc->s;
...
void US_Plot3D::rotate() {
   if (dataWidget) {
      dataWidget->setRotation(
         ( int ) (dataWidget->xRotation() + 1) % 360, ( int ) (dataWidget->yRotation() + 1) % 360,
...
void US_Plot3D::pick_back_co() {
   QColor cc = QColorDialog::getColor(Qt::white, this, tr("Select Background Color"));
   Qwt3D::RGBA rgb = Qt2GL(cc);
...
void US_Plot3D::pick_numb_co() {
   QColor cc = QColorDialog::getColor(Qt::black, this, tr("Select Number Color"));
   Qwt3D::RGBA rgb = Qt2GL(cc);
...
void US_Plot3D::pick_capt_co() {
   QColor cc = QColorDialog::getColor(Qt::black, this, tr("Select Caption Color"));
   Qwt3D::RGBA rgb = Qt2GL(cc);
...
#ifndef Q_OS_MAC
   QString mapfname = QFileDialog::getOpenFileName(this, tr("Load Color Map File"), US_Settings::etcDir(), filter, 0, 0);
#else
...
void US_Plot3D::pick_numb_fn() {
   ...
   QFont newfont = QFontDialog::getFont(&ok, oldfont, this, tr("Select Scale Numbers Font"));
...
void US_Plot3D::pick_axes_fn() {
   ...
   QFont newfont = QFontDialog::getFont(&ok, oldfont, this, tr("Select Axes Labels Font"));
...
void US_Plot3D::pick_capt_fn() {
   bool ok;
   QFont newfont = QFontDialog::getFont(&ok, titleFont, this, tr("Select Title Caption Font"));

   if (!ok)
      return;

   dataWidget->setTitleFont(newfont.family(), newfont.pointSize(), newfont.weight());
   dataWidget->updateGL();
   titleFont = newfont;
}
// rset fonts
void US_Plot3D::reset_fonts() {
   dataWidget->setTitleFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize());
   dataWidget->coordinates()->setLabelFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(), QFont::Bold);
   dataWidget->coordinates()->setNumberFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize());
   dataWidget->updateGL();
}
...
void US_Plot3D::dump_contents() {
   ...
   QString ofname = US_Settings::reportDir() + "/" + modldesc + "_" + datetime + "_plot3d." + fileext;
   ...
   if (fileext == "csv") {
      ...
      if (!myFile.open(QIODevice::WriteOnly)) {
         ...
         if (triples_in) {
            ...
            for (int ii = 0; ii < ncols; ii++) {
               for (int jj = 0; jj < nrows; jj++) {
                  out << QString::number(wdata[ ii ][ jj ].x) << ", " << QString::number(wdata[ ii ][ jj ].y) << ", "
            ...
            for (int ii = 0; ii < tdata.size(); ii++) {
               for (int jj = 0; jj < tdata.first().size(); jj++) {
                  out << QString::number(tdata[ ii ][ jj ].x) << ", " << QString::number(tdata[ ii ][ jj ].y) << ", "
...
// Allocate 2-D vector
void US_Plot3D::alloc_2dvect(QVector<QVector<double>> &vec, int ncol, int nrow) {
   vec.resize(ncol);
...
// Allocate 2-D vector
void US_Plot3D::alloc_2dvect(QVector<QVector<Triple>> &vec, int ncol, int nrow) {
   vec.resize(ncol);
