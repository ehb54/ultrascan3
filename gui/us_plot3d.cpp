    ...
    : QMainWindow (p, Qt::Dialog)
{
  ...
  QFont font = QFont (US_GuiSettings::fontFamily (),
                      US_GuiSettings::fontSize (), QFont::Normal);
  ...
  connect (ck_light, SIGNAL (stateChanged (int)), this,
           SLOT (light_check (int)));
  connect (ck_ortho, SIGNAL (stateChanged (int)), this,
           SLOT (ortho_check (int)));
  connect (ck_legend, SIGNAL (stateChanged (int)), this,
           SLOT (legnd_check (int)));
  connect (ck_autosc, SIGNAL (stateChanged (int)), this,
           SLOT (autsc_check (int)));
  connect (ck_mouse, SIGNAL (stateChanged (int)), this,
           SLOT (mouse_check (int)));
  connect (ck_shade, SIGNAL (stateChanged (int)), this,
           SLOT (shade_check (int)));

  connect (poffsSlider, SIGNAL (valueChanged (int)), this,
           SLOT (poffs_slide (int)));
  connect (resolSlider, SIGNAL (valueChanged (int)), this,
           SLOT (resol_slide (int)));
  connect (normlSlider, SIGNAL (valueChanged (int)), this,
           SLOT (norml_slide (int)));
  connect (normqSlider, SIGNAL (valueChanged (int)), this,
           SLOT (normq_slide (int)));

  timer = new QTimer (this);
  connect (movieAct, SIGNAL (toggled (bool)), this,
           SLOT (movie_toggle (bool)));
  ...
  connect (frameAct, SIGNAL (toggled (bool)), this,
           SLOT (frame_axes_on (bool)));
  ...
  connect (gridfrAct, SIGNAL (toggled (bool)), this,
           SLOT (grid_front_on (bool)));
  connect (gridbAct, SIGNAL (toggled (bool)), this,
           SLOT (grid_back_on (bool)));
  connect (gridrAct, SIGNAL (toggled (bool)), this,
           SLOT (grid_right_on (bool)));
  connect (gridlAct, SIGNAL (toggled (bool)), this,
           SLOT (grid_left_on (bool)));
  connect (gridcAct, SIGNAL (toggled (bool)), this,
           SLOT (grid_ceil_on (bool)));
  connect (gridfAct, SIGNAL (toggled (bool)), this,
           SLOT (grid_floor_on (bool)));
  connect (gridbAct, SIGNAL (toggled (bool)), this,
           SLOT (grid_back_on (bool)));
  connect (gridbAct, SIGNAL (toggled (bool)), this,
           SLOT (grid_back_on (bool)));

  connect (scattdAct, SIGNAL (toggled (bool)), this,
           SLOT (data_points_on (bool)));
  connect (wirefrAct, SIGNAL (toggled (bool)), this,
           SLOT (data_wirefr_on (bool)));
  connect (hiddlnAct, SIGNAL (toggled (bool)), this,
           SLOT (data_hidden_on (bool)));
  connect (polygnAct, SIGNAL (toggled (bool)), this,
           SLOT (data_polygn_on (bool)));
  connect (fdmeshAct, SIGNAL (toggled (bool)), this,
           SLOT (data_fimesh_on (bool)));
  connect (nodataAct, SIGNAL (toggled (bool)), this,
           SLOT (data_none_on (bool)));

  connect (fldataAct, SIGNAL (toggled (bool)), this,
           SLOT (floor_data_on (bool)));
  connect (flisolAct, SIGNAL (toggled (bool)), this,
           SLOT (floor_isol_on (bool)));
  connect (flemptAct, SIGNAL (toggled (bool)), this,
           SLOT (floor_empty_on (bool)));
...
void
US_Plot3D::setTypes (int tx, int ty, int tz)
{
  const double VROUND = 10.0;
  const double VNEGOF = (1.0 / VROUND);
  const double MAX_ANNO = (99.9 / VROUND);
...
// Public function to set dimension value types and calculate data ranges
void
US_Plot3D::setTitles (QString wndt, QString pltt, QString xat, QString yat,
                      QString zat)
{
  const double VROUND = 10.0;
  const double MAX_ANNO = (99.9 / VROUND);
...
void
US_Plot3D::setPlotTitle (QString pltt)
{
  dataWidget->setTitle (pltt);
}
...
void
US_Plot3D::setParameters (double z_scale_in, double a_gridr_in,
...
// Public function to set internal variables from plot control parameters
void
US_Plot3D::setParameters (int a_gridy, int a_gridx, double x_scale_in,
                          double y_scale_in, double z_scale_in, double a_alpha,
                          double a_beta)
{
  // Get the variables set in the plot control dialog
...
void
US_Plot3D::reloadData (QVector<QVector3D> *d)
{
  xyzdat = d;
  int nidpt = xyzdat->count ();
  QList<double> xvals;
  ...
  DbgLv (1) << "P3D:rD:  x0 x1 xm xn" << xvals[0] << xvals[1]
            << xvals[nrows - 2] << xvals[nrows - 1];
        ...
        { // find square of difference of y-raster and y-data
          ...
          for (int jj = fy; jj < ly; jj++)
            { // find square of difference of y-raster and y-data
              ...
              dist = sqrt (xdif + ydif);
              // DbgLv(0) << "cP:tif:   kk ii jj" << kk << ii << jj << "dist
              // beta_d"
              //  << dist << beta_d;
              ...
              //   Scale = Cosine( Dist * PI/2 / Beta ) raised to the Alpha
              //   power OutZ  = InZ + ( DataZ * Scale * Zfact )
              // if ( dist <= beta_d )
              if (dist <= beta)
                {
                  // zdata[ jj ][ ii ] += ( ( zval *
                  zdata[ii][jj]
                      += ((zval * (pow (cos (dist * dfac), alpha))) * zfact);
                  // DbgLv(0) << "cP:tif: ii jj" << ii << jj << "zval dist dfac
                  // alpha zfact beta_d"
                  //  << zval << dist << dfac << alpha << zfact << beta_d
                  //  << "zdataij" << zdata[ii][jj];
                }
              // else {DbgLv(3) << "  *dist>beta* dist beta iijj" << dist <<
              // beta << ii << jj;} *DBG* if ( kk>(nidpt/2-2) && kk<(nidpt/2+2)
              // ) { if ( ii>3 && ii<9 && jj>5 && jj<11 ) {
              //  DbgLv(3) << "kk" << kk << "rx ry ii jj" << rx << ry << ii <<
              //  jj
              //   << "zout"  << zdata[ii][jj] << " zfac" <<
              //   (pow(cos(dist*dfac),alpha));
              //  DbgLv(3) << "  dist" << dist << "dfac alpha beta zval" <<
              //  dfac
              //   << alpha << beta << zval;
              // }
              // }
...
void
US_Plot3D::calculateData (QVector<QVector<double> > &zdat)
{
  US_Model::SimulationComponent *sc;
        ...
        { // find square of difference of x-raster and x-model
          ...
          for (int jj = fy; jj < ly; jj++)
            { // find square of difference of y-raster and y-model
              ...
              //       Zfact, the user-specified Z scaling factor;
              //   Scale = Cosine( Dist * PI/2 / Beta ) raised to the Alpha
              //   power OutZ  = InZ + ( ModlZ * Scale * Zfact )
              if (dist <= beta)
                {
                  zdat[ii][jj]
                      += ((zval * (pow (cos (dist * dfac), alpha))) * zfact);
              ...
              // else {DbgLv(3) << "  *dist>beta* dist beta iijj" << dist <<
              // beta << ii << jj;} *DBG* if ( kk>(ncomp/2-2) && kk<(ncomp/2+2)
              // ) { if ( ii>3 && ii<9 && jj>5 && jj<11 ) {
              //  DbgLv(3) << "kk" << kk << "rx ry ii jj" << rx << ry << ii <<
              //  jj
...
void
US_Plot3D::replot (bool hold_color)
{
  unsigned int kcols = (unsigned int)ncols;
  unsigned int krows = (unsigned int)nrows;
  ...
  if (triples_in)
  ...
  else
    {
      ...
      DbgLv (1) << "P3D:ld:  xmin xmax ycmin ycmax" << xmin << xmax << ycmin
                << ycmax << "zmin zmax zfac zdmx" << zmin << zmax << zfac
                << zdmx;
...
bool
US_Plot3D::save_plot (const QString filename, const QString imagetype)
{
  dataWidget->updateData ();
  dataWidget->updateGL ();
...
QString
US_Plot3D::xyAxisTitle (int type, double sclnorm)
{
  QString atitle = tr ("s");
...
void
US_Plot3D::createActions ()
{
  openAct
      = new QAction (QIcon (":/images/fileopen.png"), tr ("&Open File"), this);
  ...
  movieAct
      = new QAction (QIcon (":/images/movie.png"), tr ("Animation"), this);

  frameAct
      = new QAction (QIcon (":/images/frame.png"), tr ("Frame Axes"), this);
  boxAct = new QAction (QIcon (":/images/box.png"), tr ("Box Axes"), this);
  noneAct = new QAction (QIcon (":/images/none.png"), tr ("No Axes"), this);

  gridfrAct
      = new QAction (QIcon (":/images/gridfr.png"), tr ("Front Grid"), this);
  gridbAct
      = new QAction (QIcon (":/images/gridb.png"), tr ("Back Grid"), this);
  gridrAct
      = new QAction (QIcon (":/images/gridr.png"), tr ("Right Grid"), this);
  gridlAct
      = new QAction (QIcon (":/images/gridl.png"), tr ("Left Grid"), this);
  gridcAct
      = new QAction (QIcon (":/images/gridc.png"), tr ("Ceiling Grid"), this);
  gridfAct
      = new QAction (QIcon (":/images/gridf.png"), tr ("Floor Grid"), this);

  scattdAct
      = new QAction (QIcon (":/images/scattered.png"), tr ("Points"), this);
  wirefrAct = new QAction (QIcon (":/images/wireframe.png"), tr ("Wire Frame"),
                           this);
  hiddlnAct = new QAction (QIcon (":/images/hiddenline.png"),
                           tr ("Hidden Line"), this);
  polygnAct = new QAction (QIcon (":/images/polygon.png"), tr ("Polygon only"),
                           this);
  fdmeshAct = new QAction (QIcon (":/images/filledmesh.png"),
                           tr ("Filled Mesh"), this);

  nodataAct
      = new QAction (QIcon (":/images/nodata.png"), tr ("No Data"), this);
  fldataAct = new QAction (QIcon (":/images/floordata.png"), tr ("Floor Data"),
                           this);
  ...
  qwtpltAct
      = new QAction (QIcon (":/images/qwtplot.png"), tr ("QwtPlot"), this);
...
double
US_Plot3D::comp_value (US_Model::SimulationComponent *sc, int type,
                       double normscl)
{
  double xyval = sc->s;
...
void
US_Plot3D::ifmt_chosen (int index)
{
...
// lighting checked
void
US_Plot3D::light_check (int state)
{
...
// ortho checked
void
US_Plot3D::ortho_check (int state)
{
...
// legend checked
void
US_Plot3D::legnd_check (int state)
{
...
// autoscale checked
void
US_Plot3D::autsc_check (int state)
{
...
// mouse checked
void
US_Plot3D::mouse_check (int state)
{
...
// shading checked
void
US_Plot3D::shade_check (int state)
{
...
void
US_Plot3D::movie_toggle (bool isOn)
{
...
// frame axes on/off
void
US_Plot3D::frame_axes_on (bool isOn)
{
  if (isOn)
    {
      dataWidget->setCoordinateStyle (FRAME);
...
// box axes on/off
void
US_Plot3D::box_axes_on (bool isOn)
{
  if (isOn)
    {
...
// no axes on/off
void
US_Plot3D::no_axes_on (bool isOn)
{
  if (isOn)
    {
...
// grid front on/off
void
US_Plot3D::grid_front_on (bool isOn)
{
...
// grid back on/off
void
US_Plot3D::grid_back_on (bool isOn)
{
...
// grid right on/off
void
US_Plot3D::grid_right_on (bool isOn)
{
...
// grid left on/off
void
US_Plot3D::grid_left_on (bool isOn)
{
...
// grid ceiling on/off
void
US_Plot3D::grid_ceil_on (bool isOn)
{
...
// grid floor on/off
void
US_Plot3D::grid_floor_on (bool isOn)
{
...
// set grid(s) on/off
void
US_Plot3D::set_grid_onoff (Qwt3D::SIDE side, bool isOn)
{
...
void
US_Plot3D::data_points_on (bool isOn)
{
  if (isOn)
    {
...
void
US_Plot3D::data_wirefr_on (bool isOn)
{
  if (isOn)
    {
...
void
US_Plot3D::data_hidden_on (bool isOn)
{
  if (isOn)
    {
...
void
US_Plot3D::data_polygn_on (bool isOn)
{
  if (isOn)
    {
...
void
US_Plot3D::data_fimesh_on (bool isOn)
{
  if (isOn)
    {
      dataWidget->setPlotStyle (FILLEDMESH);
...
void
US_Plot3D::data_none_on (bool isOn)
{
  if (isOn)
    {
...
void
US_Plot3D::floor_data_on (bool isOn)
{
  if (isOn)
    {
...
void
US_Plot3D::floor_isol_on (bool isOn)
{
  if (isOn)
    {
...
void
US_Plot3D::floor_empty_on (bool isOn)
{
  if (isOn)
    {
      dataWidget->setFloorStyle (NOFLOOR);
...
// set show-normals on
void
US_Plot3D::normals_on (bool isOn)
{
...
// pick axes color
void
US_Plot3D::pick_axes_co ()
{
  QColor cc
      = QColorDialog::getColor (Qt::black, this, tr ("Select Axes Color"));
...
// pick mesh color
void
US_Plot3D::pick_mesh_co ()
{
  QColor cc
      = QColorDialog::getColor (Qt::black, this, tr ("Select Mesh Color"));
  Qwt3D::RGBA rgb = Qt2GL (cc);
...
// pick label color
void
US_Plot3D::pick_labl_co ()
{
  QColor cc
      = QColorDialog::getColor (Qt::black, this, tr ("Select Label Color"));
  Qwt3D::RGBA rgb = Qt2GL (cc);
                      ...
                      for (int jj = 0; jj < nrows; jj++)
                        {
                          ...
                          double yval = (yroff - tdata[kk][jj].y);
                          double zval = tdata[kk][jj].z;
                          wdata[ii][jj] = Triple (xval / x_norm, yval / y_norm,
                                                  zval / z_norm);
                        }
                    }
                }
...
void
US_Plot3D::clean_title (QString &atitle)
{
...
// Clear 2-D vector
void
US_Plot3D::clear_2dvect (QVector<QVector<double> > &vec)
{
  int ncol = vec.size ();
  if (ncol < 1)
    return;
...
// Clear 2-D vector
void
US_Plot3D::clear_2dvect (QVector<QVector<Triple> > &vec)
{
  int ncol = vec.size ();
  if (ncol < 1)
    return;
...
// Allocate 2-D vector
void
US_Plot3D::alloc_2dvect (QVector<QVector<double> > &vec, int ncol, int nrow)
{
  vec.resize (ncol);
...
// Allocate 2-D vector
void
US_Plot3D::alloc_2dvect (QVector<QVector<Triple> > &vec, int ncol, int nrow)
{
  vec.resize (ncol);
...
void
US_Plot3D::closeEvent (QCloseEvent *e)
{
  emit has_closed ();
  e->accept ();
}
