...
US_Plot3D::US_Plot3D(QWidget* p, US_Model* m, QVector<QVector3D>* d)
  ...
  QFont font = QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(),
                     QFont::Normal);
  ...
  connect(pb_std, SIGNAL(clicked()), this, SLOT(std_button()));
  connect(pb_light, SIGNAL(clicked()), this, SLOT(light_button()));

  connect(cb_ifmt, SIGNAL(activated(int)), this, SLOT(ifmt_chosen(int)));

  connect(ck_light, SIGNAL(stateChanged(int)), this, SLOT(light_check(int)));
  connect(ck_ortho, SIGNAL(stateChanged(int)), this, SLOT(ortho_check(int)));
  connect(ck_legend, SIGNAL(stateChanged(int)), this, SLOT(legnd_check(int)));
  connect(ck_autosc, SIGNAL(stateChanged(int)), this, SLOT(autsc_check(int)));
  connect(ck_mouse, SIGNAL(stateChanged(int)), this, SLOT(mouse_check(int)));
  connect(ck_shade, SIGNAL(stateChanged(int)), this, SLOT(shade_check(int)));

  connect(poffsSlider, SIGNAL(valueChanged(int)), this, SLOT(poffs_slide(int)));
  connect(resolSlider, SIGNAL(valueChanged(int)), this, SLOT(resol_slide(int)));
  connect(normlSlider, SIGNAL(valueChanged(int)), this, SLOT(norml_slide(int)));
  connect(normqSlider, SIGNAL(valueChanged(int)), this, SLOT(normq_slide(int)));

  timer = new QTimer(this);
  connect(movieAct, SIGNAL(toggled(bool)), this, SLOT(movie_toggle(bool)));
  connect(timer, SIGNAL(timeout()), this, SLOT(rotate()));
  connect(openAct, SIGNAL(triggered(bool)), this, SLOT(open_file()));
  connect(saveAct, SIGNAL(triggered(bool)), this, SLOT(dump_contents()));
  connect(exitAct, SIGNAL(triggered(bool)), this, SLOT(close_all()));

  connect(coaxesAct, SIGNAL(triggered(bool)), this, SLOT(pick_axes_co()));
  connect(cobackAct, SIGNAL(triggered(bool)), this, SLOT(pick_back_co()));
  connect(comeshAct, SIGNAL(triggered(bool)), this, SLOT(pick_mesh_co()));
  connect(conumbAct, SIGNAL(triggered(bool)), this, SLOT(pick_numb_co()));
  connect(colablAct, SIGNAL(triggered(bool)), this, SLOT(pick_labl_co()));
  connect(cocaptAct, SIGNAL(triggered(bool)), this, SLOT(pick_capt_co()));
  connect(codataAct, SIGNAL(triggered(bool)), this, SLOT(pick_data_co()));
  connect(corsetAct, SIGNAL(triggered(bool)), this, SLOT(reset_colors()));

  connect(fnnumbAct, SIGNAL(triggered(bool)), this, SLOT(pick_numb_fn()));
  connect(fnaxesAct, SIGNAL(triggered(bool)), this, SLOT(pick_axes_fn()));
  connect(fncaptAct, SIGNAL(triggered(bool)), this, SLOT(pick_capt_fn()));
  connect(fnrsetAct, SIGNAL(triggered(bool)), this, SLOT(reset_fonts()));

  connect(frameAct, SIGNAL(toggled(bool)), this, SLOT(frame_axes_on(bool)));
  connect(boxAct, SIGNAL(toggled(bool)), this, SLOT(box_axes_on(bool)));
  connect(noneAct, SIGNAL(toggled(bool)), this, SLOT(no_axes_on(bool)));

  connect(gridfrAct, SIGNAL(toggled(bool)), this, SLOT(grid_front_on(bool)));
  connect(gridbAct, SIGNAL(toggled(bool)), this, SLOT(grid_back_on(bool)));
  connect(gridrAct, SIGNAL(toggled(bool)), this, SLOT(grid_right_on(bool)));
  connect(gridlAct, SIGNAL(toggled(bool)), this, SLOT(grid_left_on(bool)));
  connect(gridcAct, SIGNAL(toggled(bool)), this, SLOT(grid_ceil_on(bool)));
  connect(gridfAct, SIGNAL(toggled(bool)), this, SLOT(grid_floor_on(bool)));
  connect(gridbAct, SIGNAL(toggled(bool)), this, SLOT(grid_back_on(bool)));
  connect(gridbAct, SIGNAL(toggled(bool)), this, SLOT(grid_back_on(bool)));

  connect(scattdAct, SIGNAL(toggled(bool)), this, SLOT(data_points_on(bool)));
  connect(wirefrAct, SIGNAL(toggled(bool)), this, SLOT(data_wirefr_on(bool)));
  connect(hiddlnAct, SIGNAL(toggled(bool)), this, SLOT(data_hidden_on(bool)));
  connect(polygnAct, SIGNAL(toggled(bool)), this, SLOT(data_polygn_on(bool)));
  connect(fdmeshAct, SIGNAL(toggled(bool)), this, SLOT(data_fimesh_on(bool)));
  connect(nodataAct, SIGNAL(toggled(bool)), this, SLOT(data_none_on(bool)));

  connect(fldataAct, SIGNAL(toggled(bool)), this, SLOT(floor_data_on(bool)));
  connect(flisolAct, SIGNAL(toggled(bool)), this, SLOT(floor_isol_on(bool)));
  connect(flemptAct, SIGNAL(toggled(bool)), this, SLOT(floor_empty_on(bool)));

  connect(normsAct, SIGNAL(toggled(bool)), this, SLOT(normals_on(bool)));
  ...
  titleFont = QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize());
...
void US_Plot3D::calculateAxes() {
  ...
  double xavg;
  double yavg;
  // If there is only one data point, ensure that the minimum and maximum of
  // each axis is not the same
  if (data_points == 1) {
    zmin *= 0.90;
    zmax *= 1.10;
    ymin *= 0.90;
  ...
  // If vbar is selected, change the scale to be between 0.1 and 1 instead of 1
  // and 10
  ...
  DbgLv(2) << "P3D:cA: xmin xmax ymin ymax zmin zmax" << xmin << xmax << ymin
           << ymax << zmin << zmax;
}
...
// Public function to set dimension value types and calculate data ranges
void US_Plot3D::setTitles(QString wndt, QString pltt, QString xat, QString yat,
                          QString zat) {
  const double VROUND = 10.0;
  const double MAX_ANNO = (99.9 / VROUND);
//   const double MAX_ANNO = (999.0/VROUND);
...
void US_Plot3D::setPlotTitle(QString pltt) { dataWidget->setTitle(pltt); }
...
void US_Plot3D::setParameters(double z_scale_in, double a_gridr_in,
                              double a_alpha_in, double a_beta_in,
                              double x_scale_in, double y_scale_in) {
  ...
  for (int ii = 0; ii < ncols; ii++) zdata[ii].resize(nrows);
...
// Public function to set internal variables from plot control parameters
void US_Plot3D::setParameters(int a_gridy, int a_gridx, double x_scale_in,
                              double y_scale_in, double z_scale_in,
...
void US_Plot3D::reloadData(QVector<QVector3D>* d) {
  ...
  for (int ii = 0; ii < nidpt;
       ii++) {  // Accumulate unique X values so that we can count rows
    double xval = xyzdat->at(ii).x();
    if (!xvals.contains(xval)) xvals << xval;
  }
...
void US_Plot3D::calculateContour() {
  ...
  if (!tdata.empty()) DbgLv(1) << "P3D:cC: tdata M" << tdata[0].size();
  int kk = 0;
  clear_2dvect(tdata);
...
void US_Plot3D::calculatePoints() {
  ...
  xdif = acos(pow(1e-18, (1.0 / alpha))) * beta /
         (M_PI * 0.5 * sqrt(2.0));  // max xy-diff at small zpkf
  ...
  for (int ii = 0; ii < ncols; ii++)  // initialize raster to zmin
    for (int jj = 0; jj < nrows; jj++) zdata[ii][jj] = zval;

  for (int kk = 0; kk < nidpt; kk++) {  // calculate spread of each data point
                                        // to a radius of raster points
    ...
    for (int ii = fx; ii < lx;
         ii++) {  // find square of difference of y-raster and y-data
      xdif = sq((double)ii / xpinc - xval);
      // ydif       = (double)sq( ( ii - ry ) );

      // for ( int jj = fx; jj < lx; jj++ )
      for (int jj = fy; jj < ly;
           jj++) {  // find square of difference of y-raster and y-data
        ...
        if (dist <= beta) {
          // zdata[ jj ][ ii ] += ( ( zval *
          zdata[ii][jj] += ((zval * (pow(cos(dist * dfac), alpha))) * zfact);
          // DbgLv(0) << "cP:tif: ii jj" << ii << jj << "zval dist dfac alpha
          // zfact beta_d"
          //  << zval << dist << dfac << alpha << zfact << beta_d
          //  << "zdataij" << zdata[ii][jj];
        }
        // else {DbgLv(3) << "  *dist>beta* dist beta iijj" << dist << beta <<
        // ii << jj;} *DBG* if ( kk>(nidpt/2-2) && kk<(nidpt/2+2) ) { if ( ii>3
        // && ii<9 && jj>5 && jj<11 ) {
        //  DbgLv(3) << "kk" << kk << "rx ry ii jj" << rx << ry << ii << jj
        //   << "zout"  << zdata[ii][jj] << " zfac" <<
        //   (pow(cos(dist*dfac),alpha));
...
void US_Plot3D::calculateData(QVector<QVector<double> >& zdat) {
  ...
  for (int ii = 0; ii < nrows; ii++)  // initialize raster to zmin
    for (int jj = 0; jj < ncols; jj++) zdat[ii][jj] = zval;

  for (int kk = 0; kk < data_points;
       kk++) {  // calculate spread of each model point to a radius of raster
                // points
    ...
    for (int ii = fx; ii < lx;
         ii++) {  // find square of difference of x-raster and x-model
      xdif = sq((double)ii / xpinc - xval);

      for (int jj = fy; jj < ly;
           jj++) {  // find square of difference of y-raster and y-model
        ydif = sq((double)jj / ypinc - yval);
        ...
        if (dist <= beta) {
          zdat[ii][jj] += ((zval * (pow(cos(dist * dfac), alpha))) * zfact);
        }
        // else {DbgLv(3) << "  *dist>beta* dist beta iijj" << dist << beta <<
        // ii << jj;} *DBG* if ( kk>(ncomp/2-2) && kk<(ncomp/2+2) ) { if ( ii>3
        // && ii<9 && jj>5 && jj<11 ) {
        //  DbgLv(3) << "kk" << kk << "rx ry ii jj" << rx << ry << ii << jj
        //   << "zout"  << zdat[ii][jj] << " zfac" <<
        //   (pow(cos(dist*dfac),alpha));
        //  DbgLv(3) << "  dist" << dist << "dfac alpha beta zval" << dfac
        //   << alpha << beta << zval;
        // }
...
void US_Plot3D::replot() {
  ...
  for (int ii = 0; ii < ncols;
       ii++) {  // copy data to work 2D vector and get new z-max
  ...
  dataWidget->coordinates()->setLabelFont(
      US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(), QFont::Bold);
  ...
  DbgLv(2) << "P3D:rP:  xmin xmax" << xmin << xmax << " ymin ymax" << ymin
           << ymax << " xscl yscl" << x_scale << y_scale;
  if (x_scale > 4.0) {
    x_scale /= 4.0;
  }
  ...
  for (int ii = 0; ii < nrows; ii++) delete[] wdata[ii];
...
void US_Plot3D::replot(bool hold_color) {
  unsigned int kcols = (unsigned int)ncols;
  unsigned int krows = (unsigned int)nrows;
  DbgLv(1) << "P3D:replot: ncols nrows" << ncols << nrows << "triples-in"
           << triples_in;
  double xcmin = xmin;
  double xcmax = xmax;
  ...
  if (triples_in) {
    ...
    if (reverse_y) {
      ...
      for (int ii = 0; ii < ncols; ii++) {
        wdata[ii] = new Triple[nrows];
        for (int jj = 0; jj < nrows; jj++) wdata[ii][jj] = tdata[ii][jj];
      }
    }

    if (hold_color) {  // If hold-colors option is on, use point with max to
                       // hold colors
    ...
    DbgLv(1) << "P3D:replot: ldDa  rev_y ycmin ycmax" << reverse_y << ycmin
             << ycmax;
    // Load the widget raster data
    bool uperio = true;
    bool vperio = true;
    ...
    DbgLv(1) << "P3D:replot:  xmin xmax ycmin ycmax" << xmin << xmax << ycmin
             << ycmax << "zcmin zcmax" << zcmin << zcmax;
  }
    ...
    DbgLv(1) << "P3D:ld:  xmin xmax ycmin ycmax" << xmin << xmax << ycmin
             << ycmax << "zmin zmax zfac zdmx" << zmin << zmax << zfac << zdmx;
  ...
  if (x_norm != 1.0) xatitle = xatitle + " * " + QString::number(x_norm);
  xatitle = annopad + xatitle;
  if (y_norm != 1.0) yatitle = yatitle + " * " + QString::number(y_norm);
  yatitle = yatitle + annopad;
  if (z_norm != 1.0) zatitle = zatitle + " * " + QString::number(z_norm);

  DbgLv(2) << "P3D:rP:  xmin xmax" << xmin << xmax << " ymin ymax" << ymin
           << ymax << " zmin zmax" << zmin << zmax << " xscl yscl zscl"
  ...
  if (wdata != NULL) {
    for (int ii = 0; ii < ncols; ii++) delete[] wdata[ii];
  ...
  if (wddat != NULL) {
    for (int ii = 0; ii < ncols; ii++) delete[] wddat[ii];
...
QString US_Plot3D::xyAxisTitle(int type, double sclnorm) {
  ...
  if (sclnorm != 1.0) atitle = atitle + " * " + QString::number(sclnorm);
...
QString US_Plot3D::zAxisTitle(int type) {
  ...
  if (type == 2) atitle = tr("Mol.Concentr. ");
...
void US_Plot3D::createActions() {
  ...
  openffAct =
      new QAction(QIcon(":/images/filecell.png"), tr("Open FEM File"), this);
  saveAct =
      new QAction(QIcon(":/images/savecontent.png"), tr("Dump Contents"), this);
  movieAct = new QAction(QIcon(":/images/movie.png"), tr("Animation"), this);

  frameAct = new QAction(QIcon(":/images/frame.png"), tr("Frame Axes"), this);
  boxAct = new QAction(QIcon(":/images/box.png"), tr("Box Axes"), this);
  noneAct = new QAction(QIcon(":/images/none.png"), tr("No Axes"), this);

  gridfrAct = new QAction(QIcon(":/images/gridfr.png"), tr("Front Grid"), this);
  gridbAct = new QAction(QIcon(":/images/gridb.png"), tr("Back Grid"), this);
  gridrAct = new QAction(QIcon(":/images/gridr.png"), tr("Right Grid"), this);
  gridlAct = new QAction(QIcon(":/images/gridl.png"), tr("Left Grid"), this);
  gridcAct = new QAction(QIcon(":/images/gridc.png"), tr("Ceiling Grid"), this);
  gridfAct = new QAction(QIcon(":/images/gridf.png"), tr("Floor Grid"), this);

  scattdAct = new QAction(QIcon(":/images/scattered.png"), tr("Points"), this);
  wirefrAct =
      new QAction(QIcon(":/images/wireframe.png"), tr("Wire Frame"), this);
  hiddlnAct =
      new QAction(QIcon(":/images/hiddenline.png"), tr("Hidden Line"), this);
  polygnAct =
      new QAction(QIcon(":/images/polygon.png"), tr("Polygon only"), this);
  fdmeshAct =
      new QAction(QIcon(":/images/filledmesh.png"), tr("Filled Mesh"), this);

  nodataAct = new QAction(QIcon(":/images/nodata.png"), tr("No Data"), this);
  fldataAct =
      new QAction(QIcon(":/images/floordata.png"), tr("Floor Data"), this);
  flisolAct =
      new QAction(QIcon(":/images/flooriso.png"), tr("Floor Isolines"), this);
  flemptAct =
      new QAction(QIcon(":/images/floorempty.png"), tr("Floor Empty"), this);

  normsAct =
      new QAction(QIcon(":/images/normals.png"), tr("Normal Vectors"), this);
  iconAct = new QAction(QIcon(":/images/icon.png"), tr("Icon"), this);
  qwtpltAct = new QAction(QIcon(":/images/qwtplot.png"), tr("QwtPlot"), this);
...
double US_Plot3D::comp_value(US_Model::SimulationComponent* sc, int type,
  ...
  switch (type) {  // get appropriate value, based on type
...
void US_Plot3D::std_button() { setStandardView(); }

// lighting button clicked
void US_Plot3D::light_button() { DbgLv(2) << "light_button"; }
...
// grid front on/off
void US_Plot3D::grid_front_on(bool isOn) { set_grid_onoff(FRONT, isOn); }

// grid back on/off
void US_Plot3D::grid_back_on(bool isOn) { set_grid_onoff(BACK, isOn); }

// grid right on/off
void US_Plot3D::grid_right_on(bool isOn) { set_grid_onoff(RIGHT, isOn); }

// grid left on/off
void US_Plot3D::grid_left_on(bool isOn) { set_grid_onoff(LEFT, isOn); }

// grid ceiling on/off
void US_Plot3D::grid_ceil_on(bool isOn) { set_grid_onoff(CEIL, isOn); }

// grid floor on/off
void US_Plot3D::grid_floor_on(bool isOn) { set_grid_onoff(FLOOR, isOn); }
...
// open file selected
void US_Plot3D::open_file() { DbgLv(2) << "open_file"; }
...
void US_Plot3D::pick_axes_co() {
  QColor cc = QColorDialog::getColor(Qt::black, this, tr("Select Axes Color"));
...
void US_Plot3D::pick_mesh_co() {
  QColor cc = QColorDialog::getColor(Qt::black, this, tr("Select Mesh Color"));
  Qwt3D::RGBA rgb = Qt2GL(cc);
...
void US_Plot3D::pick_numb_co() {
  QColor cc =
      QColorDialog::getColor(Qt::black, this, tr("Select Number Color"));
  Qwt3D::RGBA rgb = Qt2GL(cc);
...
void US_Plot3D::pick_labl_co() {
  QColor cc = QColorDialog::getColor(Qt::black, this, tr("Select Label Color"));
  Qwt3D::RGBA rgb = Qt2GL(cc);
...
void US_Plot3D::pick_capt_co() {
  QColor cc =
      QColorDialog::getColor(Qt::black, this, tr("Select Caption Color"));
  Qwt3D::RGBA rgb = Qt2GL(cc);
...
#ifndef Q_OS_MAC
  QString mapfname = QFileDialog::getOpenFileName(
...
void US_Plot3D::pick_numb_fn() {
  ...
  QFont newfont =
      QFontDialog::getFont(&ok, oldfont, this, tr("Select Scale Numbers Font"));
...
void US_Plot3D::pick_axes_fn() {
  ...
  QFont newfont =
      QFontDialog::getFont(&ok, oldfont, this, tr("Select Axes Labels Font"));
...
void US_Plot3D::dump_contents() {
  ...
  QString ofname = US_Settings::reportDir() + "/" + modldesc + "_" + datetime +
                   "_plot3d." + fileext;
  ...
  if (fileext == "csv") {
    ...
    if (!myFile.open(QIODevice::WriteOnly)) {
      qDebug() << "Could not write to file:" << ofname
               << "Error string:" << myFile.errorString();
    } else {
        ...
        if (reverse_y) {
          ...
          for (int ii = 0; ii < ncols; ii++) {
            ...
            for (int jj = 0; jj < nrows; jj++) {
              ...
              double zval = tdata[kk][jj].z;
              wdata[ii][jj] =
                  Triple(xval / x_norm, yval / y_norm, zval / z_norm);
            }
          }
        } else {
          for (int ii = 0; ii < ncols; ii++) {
        ...
        for (int ii = 0; ii < ncols; ii++) {
          for (int jj = 0; jj < nrows; jj++) {
            out << QString::number(wdata[ii][jj].x) << ", "
                ...
                << QString::number(wdata[ii][jj].z) << Qt::endl;
          }
        }
        myFile.flush();
      } else {
        ...
        for (int ii = 0; ii < tdata.size(); ii++) {
          for (int jj = 0; jj < tdata.first().size(); jj++) {
            out << QString::number(tdata[ii][jj].x) << ", "
                ...
                << QString::number(tdata[ii][jj].z) << Qt::endl;
          }
        }
        myFile.flush();
      }
  ...
  if (ok) {
    statusBar()->showMessage(tr("Successful dump to ") + ofname);
  }

  else {
    statusBar()->showMessage(tr("*ERROR* Unable to create ") + ofname);
  }
}
...
void US_Plot3D::clear_2dvect(QVector<QVector<double> >& vec) {
  ...
  for (int ii = 0; ii < ncol; ii++) vec[ii].clear();
...
// Allocate 2-D vector
void US_Plot3D::alloc_2dvect(QVector<QVector<double> >& vec, int ncol,
                             int nrow) {
  vec.resize(ncol);

  for (int ii = 0; ii < ncol; ii++) vec[ii].resize(nrow);
}

// Allocate 2-D vector
void US_Plot3D::alloc_2dvect(QVector<QVector<Triple> >& vec, int ncol,
                             int nrow) {
  vec.resize(ncol);
