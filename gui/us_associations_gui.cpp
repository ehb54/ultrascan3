...
US_PushButton::US_PushButton(const QString &text, int i) : QPushButton(text), index(i) {
...
US_AssociationsGui::US_AssociationsGui(US_Model &current_model) : US_WidgetsDialog(0, 0), model(current_model) {
   ...
   te_help->setText(tr("* Drag a component from the upper list and drop it in an"
...
void US_AssociationsGui::complete(void) {
   ...
   for (int i = 0; i < tw->rowCount() - 1; i++) {
      ...
      item = tw->item(i, 5);
      if (item != 0)
         association.k_d = item->text().toDouble();

      item = tw->item(i, 6);
      if (item != 0)
         association.k_off = item->text().toDouble();
      ...
      if (moles_right != moles_left) {
         QMessageBox::information(this, tr("Equations do not balance"), tr("Equation %1 does not balance").arg(i + 1));
         return;
      }

      if (combine_reactants(&association)) {
         QMessageBox::information(
            this, tr("Combined Identical Reactants"),
...
bool US_AssociationsGui::combine_reactants(US_Model::Association *as) {
   ...
   if (as->rcomps.size() == 3 && as->stoichs[ 1 ] > 0) { // 2 reactants and a product:  check if reactants are identical
