...
US_NewSpectrum::US_NewSpectrum (QString tmp_type, const QString &tmp_text,
    ...
    : US_Widgets ()
{
  ...
  connect (pb_manual, SIGNAL (clicked ()), this, SLOT (entermanually ()));
...
void
US_NewSpectrum::uploadDisk (void)
{
  add_spectrumDisk ();
}

void
US_NewSpectrum::uploadFit (void)
{
...
void
US_NewSpectrum::process_results (QMap<double, double> &xyz)
{
  if (type == "BUFFER")
  ...
  if (type == "ANALYTE")
    {
      ...
      // US_AnalyteMgrNew::le_protein_e280->setText(
...
void
US_NewSpectrum::add_spectrumDisk (void)
{
  QStringList files;
  QFile f;
  ...
  dialog.setNameFilter (
      tr ("Text files (*.[Tt][Xx][Tt] *.[Cc][Ss][Vv] *.[Dd][Aa][Tt] "
          "*.[Ww][Aa]* *.[Dd][Ss][Pp]);;All files (*)"));
...
void
US_NewSpectrum::readingspectra (const QString &fileName)
{
  QString str1;
  QStringList strl;
  float temp_x, temp_y;
...
void
US_NewSpectrum::entermanually (void)
{
  US_Table *sdiag;
...
void
US_NewSpectrum::cancel (void)
{
  this->close ();
}

/// View Spectrum ///////////////////////////
US_ViewSpectrum::US_ViewSpectrum (QMap<double, double> &tmp_extinction)
    : US_Widgets ()
{
  ...
  // changedCurve = NULL;
  plotLayout
      = new US_Plot (data_plot, tr (""), tr ("Wavelength(nm)"), tr (""));
...
void
US_ViewSpectrum::plot_extinction ()
{
  QVector<double> x;
  QVector<double> y;
...
void
US_ViewSpectrum::save_csv ()
{
  QString fileName = QFileDialog::getSaveFileName (
      this, tr ("Save File"), US_Settings::reportDir (), tr ("CSV (*.csv)"));
  if (fileName.isEmpty ())
    return;
