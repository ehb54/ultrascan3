...
US_RotorGui::US_RotorGui (US_Rotor::RotorCalibration &calibration,
                          bool new_calibration, bool signal_wanted,
                          int select_db_disk)
  ...
  if (savingCalibration)
    {
      ...
      QList<QListWidgetItem *> items = lw_rotors->findItems (
          QString::number (rotorID), Qt::MatchStartsWith);
      ...
      lw_calibrations->addItem (
          currentCalibration.lastUpdated.toString ("d MMMM yyyy"));
...
US_RotorGui::US_RotorGui (bool signal_wanted, int select_db_disk,
                          const US_Rotor::Rotor &rotorIn,
                          const US_Rotor::RotorCalibration &calibrationIn)
    : US_WidgetsDialog (nullptr, Qt::WindowFlags ()), currentRotor (rotorIn),
  ...
  if (rotorID > 0 && calibrationID > 0)
    {
      ...
      if (status == US_Rotor::ROTOR_OK)
        items = lw_rotors->findItems (QString::number (rotorID),
                                      Qt::MatchStartsWith);
      if (items.count () == 1) // should be exactly 1
        lw_rotors->setCurrentItem (items[0]);
      ...
      items.clear ();
      items = lw_calibrations->findItems (QString::number (calibrationID),
                                          Qt::MatchStartsWith);
      if (items.count () == 1)
...
bool
US_RotorGui::load_rotor (QString &load_init, double &coeff1, double &coeff2)
{
  bool ok;
  ...
  for (auto &lab : labList)
    {
      if (disk_controls->db ())
          ...
          while (db.next ())
            {
              ...
              rotorList << (rotorID + ":" + rotorDesc + ":"
                            + QString::number (lab.ID));
            }
        }
      ...
      else
        {
          ...
          if (US_Rotor::readRotorsFromDisk (rotors, labID)
              == US_Rotor::NOT_FOUND)
            {
              QMessageBox::warning (
                  this, tr ("Disk Problem"),
          ...
          for (auto &rotor : rotors)
            {
              rotorList << (QString::number (rotor.ID) + ":" + rotor.name + ":"
                            + QString::number (rotor.labID));
            }
        }
    }
          ...
          if (parts.contains (load_init))
            {
              ...
              US_Rotor::Status status
                  = readRotor (disk_controls->db (), rotorID);
...
void
US_RotorGui::setupGui (int select_db_disk)
{
  ...
  QLabel *lbl_bannerCalibration
      = us_banner (tr ("Click to select a Rotor Calibration: "), -1);
  lbl_bannerCalibration->setSizePolicy (QSizePolicy::Preferred,
                                        QSizePolicy::Fixed);
  top->addWidget (lbl_bannerCalibration, row, 0, 1, 2);

  pb_saveCalibration = us_pushbutton (tr ("Save Calibration Data"));
  connect (pb_saveCalibration, SIGNAL (clicked ()), this,
  ...
  connect (pb_deleteCalibration, SIGNAL (clicked ()), this,
           SLOT (deleteCalibration ()));
  ...
  QLabel *lbl_coefficients
      = us_label (tr (" Rotor Stretch Coefficient 1:"), -1);
...
US_RotorGui::reset (void)
{
  ...
  bool enabled
      = lw_rotors->count () > 0 && lw_rotors->currentIndex ().row () != -1;
...
void
US_RotorGui::addRotor (void)
{
  int dbdisk = (disk_controls->db ()) ? US_Disk_DB_Controls::DB
                                      : US_Disk_DB_Controls::Disk;

  currentRotor.reset ();
  US_AbstractRotorGui *arDialog
      = new US_AbstractRotorGui (true, dbdisk, &currentRotor);
  connect (arDialog, SIGNAL (accepted ()), this, SLOT (newRotor ()));
  connect (arDialog, SIGNAL (use_db (bool)), SLOT (update_disk_db (bool)));
  arDialog->show ();
}

void
US_RotorGui::update_disk_db (bool db)
{
  (db) ? disk_controls->set_db () : disk_controls->set_disk ();
}

void
US_RotorGui::newRotor (void)
{
...
void
US_RotorGui::db_error (const QString &error)
{
  QMessageBox::warning (this, tr ("Database Problem"),
...
void
US_RotorGui::selectRotor (QListWidgetItem *item)
{
...
US_Rotor::Status
US_RotorGui::readRotor (int disk_db, int rotorID)
{
  // Let's see if the user passed a rotorID, and
  // find out the rest of the rotor info
  US_Rotor::Status status;
...
bool
US_RotorGui::readCalibrationProfiles (int rotorID)
{
  ...
  if (disk_controls->db ())
      ...
      while (db.next ())
        {
          ...
          calibrationDescriptions
              << (ID + ": " + label + dateUpdated.toString (" (d MMMM yyyy)"));
        }
    }
  ...
  else
    {
      ...
      if (US_Rotor::readCalibrationProfilesDisk (rc, rotorID)
          == US_Rotor::NOT_FOUND)
        {
          QMessageBox::warning (
              this, tr ("Disk Problem"),
...
void
US_RotorGui::selectCalibration (QListWidgetItem *item)
{
  QString selected = item->text ();
  QStringList parts = selected.split (":");
...
US_Rotor::Status
US_RotorGui::readCalibration (int disk_db, int calibrationID)
{
...
void
US_RotorGui::deleteCalibration (void)
{
  ...
  if (disk_controls->db ())
    {
      ...
      int status = US_Rotor::RotorCalibration::deleteCalibrationDB (
          calibrationID, &db);
      if (status == US_DB2::CALIB_IN_USE)
        {
          QString error
              = tr ("This rotor calibration is in use, and can't be deleted");
          db_error (error);
          return;
        }
  ...
  else
    {
      QMessageBox::warning (this, tr ("Attention"),
                            tr ("Deleting rotor calibrations on the local "
                                "disk is not implemented yet."));
      return;
    }
...
void
US_RotorGui::updateName (const QString &name)
{
  currentRotor.name = name;
}

void
US_RotorGui::updateSerialNumber (const QString &number)
{
  currentRotor.serialNumber = number;
}

void
US_RotorGui::updateLabel (const QString &label)
{
  currentCalibration.label = label;
}

void
US_RotorGui::updateOmega2t (const QString &omega2t)
{
  currentCalibration.omega2t = omega2t.toFloat ();
}

void
US_RotorGui::saveCalibration ()
{
...
//  the dummy.
void
US_RotorGui::replaceDummyCalibration (void)
{
  ...
  if (status == US_DB2::NO_CALIB)
    {
      QString error
          = tr ("The replacement rotor calibration could not be found.");
      db_error (error);
      return;
    }
  ...
  // If we get here then a dummy calibration was found and replaced without
  // error
  status = US_Rotor::RotorCalibration::deleteCalibrationDB (oldCalibrationID,
                                                            &db);
  if (status == US_DB2::CALIB_IN_USE)
    {
      QString error = tr ("The database shows the dummy calibration is still "
...
void
US_RotorGui::source_changed (bool db)
{
  QStringList DB = US_Settings::defaultDB ();
...
bool
US_RotorGui::loadRotors (const int labID)
{
      ...
      for (int i = 0; i < rotors.size (); i++)
        {
          if (rotors[i].labID == labID)
            rotorDescriptions
                << (QString::number (rotors[i].ID) + ": " + rotors[i].name);
        }
    }
...
void
US_RotorGui::connect_error (const QString &error)
{
