...
US_RotorGui::US_RotorGui(
   US_Rotor::RotorCalibration &calibration, bool new_calibration, bool signal_wanted, int select_db_disk) :
   ...
   if (savingCalibration) {
      ...
      QList<QListWidgetItem *> items = lw_rotors->findItems(QString::number(rotorID), Qt::MatchStartsWith);
...
US_RotorGui::US_RotorGui(
   bool signal_wanted, int select_db_disk, const US_Rotor::Rotor &rotorIn,
   const US_Rotor::RotorCalibration &calibrationIn) :
    US_WidgetsDialog(nullptr, Qt::WindowFlags()), currentRotor(rotorIn), currentCalibration(calibrationIn),
...
bool US_RotorGui::load_rotor(QString &load_init, double &coeff1, double &coeff2) {
   ...
   if (disk_controls->db()) {
      ...
      if (US_Rotor::readLabsDB(labList, &db) == US_Rotor::NOT_FOUND) {
         QMessageBox::warning(this, tr("Database Problem"), tr("Could not read lab information \n"));
      ...
      if (US_Rotor::readLabsDisk(labList) == US_Rotor::NOT_FOUND) {
         QMessageBox::warning(this, tr("Disk Problem"), tr("Could not read lab information \n"));
   ...
   for (auto &lab: labList) {
      if (disk_controls->db()) {
         ...
         if (US_Rotor::readRotorsFromDisk(rotors, labID) == US_Rotor::NOT_FOUND) {
            QMessageBox::warning(this, tr("Disk Problem"), tr("Could not read rotor information \n"));
            return (false);
         }
...
void US_RotorGui::setupGui(int select_db_disk) {
   ...
   connect(
      cb_lab, SIGNAL(activated(int)), // Only if the user has changed it
      SLOT(changeLab(int)));
   ...
   connect(lw_rotors, SIGNAL(itemClicked(QListWidgetItem *)), SLOT(selectRotor(QListWidgetItem *)));
   ...
   connect(le_name, SIGNAL(textEdited(const QString &)), SLOT(updateName(const QString &)));
   ...
   connect(le_serialNumber, SIGNAL(textEdited(const QString &)), SLOT(updateSerialNumber(const QString &)));
   ...
   connect(lw_calibrations, SIGNAL(itemClicked(QListWidgetItem *)), SLOT(selectCalibration(QListWidgetItem *)));
   ...
   connect(le_calibrationLabel, SIGNAL(textEdited(const QString &)), SLOT(updateLabel(const QString &)));
   ...
   connect(le_omega2t, SIGNAL(textEdited(const QString &)), SLOT(updateOmega2t(const QString &)));
...
void US_RotorGui::reset(void) {
   ...
   pb_addRotor->setEnabled((US_Settings::us_inv_level() > 2) && (disk_controls->db()));

   // buttons to enable if a rotor is selected
   bool enabled = lw_rotors->count() > 0 && lw_rotors->currentIndex().row() != -1;
   pb_deleteRotor->setEnabled(enabled);

   // buttons to enable if a calibration is selected
   enabled = lw_calibrations->count() > 0 && lw_calibrations->currentIndex().row() != -1;
...
void US_RotorGui::deleteRotor(void) {
   ...
   if (disk_controls->db()) {
      ...
      QMessageBox::warning(this, tr("Attention"), tr("Deleting rotors on the local disk is not implemented yet."));
      return;
   }
...
bool US_RotorGui::readCalibrationProfiles(int rotorID) {
   ...
   if (disk_controls->db()) {
      ...
      if (US_Rotor::readCalibrationProfilesDisk(rc, rotorID) == US_Rotor::NOT_FOUND) {
         QMessageBox::warning(this, tr("Disk Problem"), tr("Could not find rotor calibration profiles \n"));
         return (false);
      }

      for (int i = 0; i < rc.size(); i++) {
         if (rc[ i ].rotorID == rotorID)
            calibrationDescriptions << (QString::number(rc[ i ].ID) + ": " + rc[ i ].lastUpdated.toString("d MMMM yyyy"));
      }
   }
...
void US_RotorGui::viewReport(void) {
   ...
   edit->resize(500, 400);
   edit->editor->e->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize()));
...
void US_RotorGui::deleteCalibration(void) {
   ...
   if (disk_controls->db()) {
      ...
      QMessageBox::warning(
         this, tr("Attention"), tr("Deleting rotor calibrations on the local disk is not implemented yet."));
      return;
   }
...
void US_RotorGui::saveCalibration() {
   ...
   if (disk_controls->db()) {
      ...
      if (status == US_DB2::OK) {
         ...
         QMessageBox::information(this, tr("Attention"), tr("Calibration was saved to the DB."));
      }
   }
      ...
      QMessageBox::information(this, tr("Attention"), tr("Calibration was saved to the Disk."));
   }
