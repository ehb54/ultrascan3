...
US_RotorGui::US_RotorGui(bool signal_wanted, int select_db_disk,
  ...
  if (rotorID > 0 && calibrationID > 0) {
    ...
    items = lw_calibrations->findItems(QString::number(calibrationID),
                                       Qt::MatchStartsWith);
    if (items.count() == 1) lw_calibrations->setCurrentItem(items[0]);
  }

  reset();
}

bool US_RotorGui::load_rotor(QString& load_init, double& coeff1,
                             double& coeff2) {
  bool ok;
...
US_Rotor::Status US_RotorGui::readRotor(int disk_db, int rotorID) {
  ...
  if (rotorID <= 0) return (US_Rotor::NOT_FOUND);
...
US_Rotor::Status US_RotorGui::readCalibration(int disk_db, int calibrationID) {
  ...
  if (calibrationID <= 0) return (US_Rotor::NOT_FOUND);
...
void US_RotorGui::deleteCalibration(void) {
  ...
  if (disk_controls->db()) {
    ...
    int status =
        US_Rotor::RotorCalibration::deleteCalibrationDB(calibrationID, &db);
    ...
    QMessageBox::warning(this, tr("Attention"),
                         tr("Deleting rotor calibrations on the local disk is "
                            "not implemented yet."));
    return;
  }
...
void US_RotorGui::updateName(const QString& name) { currentRotor.name = name; }
...
void US_RotorGui::replaceDummyCalibration(void) {
  ...
  // error
  status =
      US_Rotor::RotorCalibration::deleteCalibrationDB(oldCalibrationID, &db);
  if (status == US_DB2::CALIB_IN_USE) {
    QString error =
        tr("The database shows the dummy calibration is still in use, and "
           "can't be deleted");
    db_error(error);
    return;
  }
...
bool US_RotorGui::load(void) {
  ...
  foreach (US_Rotor::Lab lab, labList) {
    ...
    if (lab.ID == currentRotor.labID) index = cb_lab->currentIndex();
  }
  if (cb_lab->count() > 0) cb_lab->setCurrentIndex(index);
...
bool US_RotorGui::loadRotors(const int labID) {
  ...
  if (!savingCalibration) currentCalibration.reset();
  ...
  if (disk_controls->db()) {
    ...
    for (int i = 0; i < rotors.size(); i++) {
      if (rotors[i].labID == labID)
        rotorDescriptions << (QString::number(rotors[i].ID) + ": " +
                              rotors[i].name);
    }
  }
