...
US_Sassoc::US_Sassoc (double eq0, double eq1, double stoich1, double stoich2,
                      ...
                      bool position_win, QWidget *parent,
                      Qt::WindowFlags flags)
    : US_Widgets (position_win, parent, flags)
{
  eq[0] = eq0;
  eq[1] = eq1;
  model = t_model;
  ...
  // Graph
  plot1 = new US_Plot (plot, tr ("Self-Association Profile for") + project,
  ...
  QLineEdit *le_conc = us_lineedit (
      tr ("Drag mouse through plot area to see relative concentrations"));
  le_species3->setReadOnly (true);
...
double
US_Sassoc::monomer_root (double total)
{
...
double
US_Sassoc::polynomial (double monomer, double total)
{
...
void
US_Sassoc::new_value (const QwtDoublePoint &p)
{
  update_legend (p.x ());
}

void
US_Sassoc::mouseU (const QwtDoublePoint & /* p */)
{
  update_legend (-1.0);
}

void
US_Sassoc::mouseD (const QwtDoublePoint &p)
{
  updating = true;
  update_legend (p.x ());
}
...
void
US_Sassoc::update_legend (const double total)
{
...
void
US_Sassoc::update_stoich1 (const QString &s)
{
  stoich[0] = s.toDouble ();
  recalc ();
}
...
void
US_Sassoc::update_stoich2 (const QString &s)
{
  stoich[1] = s.toDouble ();
  recalc ();
}
...
void
US_Sassoc::update_eq1 (const QString &s)
{
  eq[0] = s.toDouble ();
...
void
US_Sassoc::update_eq2 (const QString &s)
{
  eq[1] = s.toDouble ();
...
void
US_Sassoc::update_eq1Count (double value)
{
  eq[0] = value;
...
void
US_Sassoc::update_eq2Count (double value)
{
  eq[1] = value;
...
US_Sassoc::save (void)
{
  QString filename = US_Settings::resultDir () + "/" + project + "-"
                     + QString::number (model) + ".dis";
  ...
  if (f.open (QIODevice::WriteOnly | QIODevice::Text))
    {
      QTextStream ts (&f);
      ts << tr ("\"Total Concentration\"\t\"% Monomer\"\t\"% ") << stoich[0]
         << "-mer\"\t\"% " << stoich[1] << "-mer\"\n";

      for (int i = 0; i < ARRAY_SIZE; i++)
        {
          ts << x[i] << "\t" << species1[i] << "\t" << species2[i] << "\t"
             << species3[i] << endl;
        }
    }
  ...
  filename = reportDir + "/" + project + "-" + QString::number (model)
             + ".distribution.png";
