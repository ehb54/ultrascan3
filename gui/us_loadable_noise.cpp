...
int US_LoadableNoise::count_noise(
   bool ondisk, US_DataIO::EditedData *edata, US_Model *model, QStringList &mieGUIDs, QStringList &nieGUIDs) {
   ...
   for (int ii = 0; ii < noiIDs.size(); ii++) {
      DbgLv(2) << "LaNoi:allNoi nID eID mID type" << noiIDs.at(ii) << noiEdIDs.at(ii) << noiMoIDs.at(ii)
               << noiTypes.at(ii);
   }
...
int US_LoadableNoise::noises_in_model(bool ondisk, QString mGUID, QStringList &nGUIDs) {
   QString xnGUID;
   QString xmGUID;
   QString xntype;
...
int US_LoadableNoise::id_list_disk(QString daEditGUID) {
   ...
   for (int ii = 0; ii < f_names.size(); ii++) {
      ...
      while (!xml.atEnd()) { // Search XML elements until we find "noise"
         xml.readNext();

         if (xml.isStartElement() && xml.name() == "noise") { // test for desired editGUID
            attr = xml.attributes();
   ...
   for (int ii = 0; ii < f_names.size(); ii++) {
      ...
      while (!xml.atEnd()) { // Search XML elements until we find "model"
         ...
         if (xml.isStartElement() && xml.name() == "model") { // test for desired editGUID
            attr = xml.attributes();
            ...
            if (noiMoIDs.contains(modelID) && editID == daEditGUID) { // Only list models that have associated noise that match edit
               if (mCarlo) { // Treat monte carlo in a special way (1 composite model)
