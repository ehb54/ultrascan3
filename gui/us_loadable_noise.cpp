...
int US_LoadableNoise::count_noise(bool ondisk, US_DataIO::EditedData* edata,
  ...
  for (int ii = 1; ii < nemods; ii++) {  // Search through models in edit
    ...
    if (kenois > 0) {  // if we have 1 or 2 noises, add to noise-in-edit list
      ...
      DbgLv(2) << "LaNoi: kenois nenois niesz" << kenois << nenois
               << nieGUIDs.size();
    ...
    while (!xml.atEnd()) {  // Search XML elements until we find "model"
      ...
      if (xml.isStartElement() &&
        ...
        if (noiMoIDs.contains(modelID) &&
            editID == daEditGUID) {  // Only list models that have associated
                                     // noise that match edit
          if (mCarlo) {  // Treat monte carlo in a special way (1 composite
                         // model)
