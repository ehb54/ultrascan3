...
int
US_LoadableNoise::count_noise (bool ondisk, US_DataIO::EditedData *edata,
                               ...
                               QStringList &nieGUIDs)
{
  ...
  for (int ii = 0; ii < modIDs.size (); ii++)
    {
      DbgLv (2) << "LaNoi:allMod mID eID" << modIDs.at (ii)
                << modEdIDs.at (ii);
    }
...
int
US_LoadableNoise::models_in_edit (bool ondisk, QString eGUID,
                                  QStringList &mGUIDs)
{
  QString xmGUID;
  QString xeGUID;
...
int
US_LoadableNoise::noises_in_model (bool ondisk, QString mGUID,
                                   QStringList &nGUIDs)
{
  QString xnGUID;
  QString xmGUID;
  ...
  for (int ii = 0; ii < noiIDs.size (); ii++)
    { // Examine noises list; Save to this list if model GUID matches
      ...
      if (!ondisk)
        xntype
            = xntype.contains ("ri_nois", Qt::CaseInsensitive) ? "ri" : "ti";
...
int
US_LoadableNoise::id_list_db (QString daEditGUID)
{
  QStringList query;
...
int
US_LoadableNoise::id_list_disk (QString daEditGUID)
{
  noiIDs.clear ();
  noiEdIDs.clear ();
  noiMoIDs.clear ();
          ...
          if (xml.isStartElement () && xml.name () == "model")
            { // test for desired editGUID
              ...
              if (noiMoIDs.contains (modelID) && editID == daEditGUID)
                { // Only list models that have associated noise that match
                  // edit
                  if (mCarlo)
                    { // Treat monte carlo in a special way (1 composite model)
                      if (reqIDs.contains (reqGUID))
                        ...
                        reqIDs
                            << reqGUID; // Record that this request ID is used
                    }
