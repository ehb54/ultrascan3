...
US_ScanExclGui::US_ScanExclGui(QStringList channels_desc, QList<int> scan_beg,
  ...
  QLabel* bn_excl_scans =
      us_banner(QString(tr("Selection of the Scans Excluded from Beginnig and "
                           "End of the Scan Sets: ")),
                1);
  ...
  // QLabel* lb_begin          = us_label( tr( "Number of scans at the beginning
  // of run:" ) ); QLabel* lb_end            = us_label( tr( "Number of scans at
  // the end of run:" ) );
...
void US_ScanExclGui::build_layout(void) {
  ...
  qDebug() << "Sizes of channels_desc, scan_beg | end: " << channels_desc.size()
           << scan_beg.size() << scan_end.size();

  for (int ii = 0; ii < channels_desc.size(); ii++) {
    QString chan_desc = channels_desc[ii];

    qDebug() << "Build layout: chan_Desc -- " << chan_desc;
    qDebug() << "Build layout: scan_beg[ii], scan_end[ii]  -- " << scan_beg[ii]
             << scan_end[ii];
    ...
    if (ii == 0) {
      ...
      connect(pb_applyall, SIGNAL(clicked()), this, SLOT(applied_to_all()));
    } else
      row++;
...
void US_ScanExclGui::scan_excl_changed(int) {
  ...
  int rem_scans_num = maxScans_map[row_num] -
...
void US_ScanExclGui::gui_to_parms(void) {
  ...
  for (int ii = 0; ii < channels_desc.size(); ii++) {
    ...
    QString range_pair =
        QString::number(scan_beg[ii]) + ":" + QString::number(scan_end[ii]);
...
// cancel
void US_ScanExclGui::cancel_update(void) { close(); }
