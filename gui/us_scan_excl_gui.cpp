...
US_ScanExclGui::US_ScanExclGui(
   QStringList channels_desc, QList<int> scan_beg, QList<int> scan_end, int scanCount, int scanCount_int) :
    US_Widgets() {
   ...
   QLabel *bn_excl_scans
      = us_banner(QString(tr("Selection of the Scans Excluded from Beginnig and End of the Scan Sets: ")), 1);
...
void US_ScanExclGui::build_layout(void) {
   ...
   for (int ii = 0; ii < channels_desc.size(); ii++) {
      ...
      connect(sb_begin, SIGNAL(valueChanged(int)), this, SLOT(scan_excl_changed(int)));
      connect(sb_end, SIGNAL(valueChanged(int)), this, SLOT(scan_excl_changed(int)));
...
void US_ScanExclGui::scan_excl_changed(int) {
   ...
   qDebug() << "Clicked couner:        oname, value -- " << oname << sb_widget->value();
   qDebug() << "Counterpart's widget:  oname, value -- " << objName_for_sb_widget_counterpart
