...
US_ScanExclGui::US_ScanExclGui (QStringList channels_desc, QList<int> scan_beg,
                                QList<int> scan_end, int scanCount,
                                int scanCount_int)
    : US_Widgets ()
{
  ...
  QLabel *bn_excl_scans
      = us_banner (QString (tr ("Selection of the Scans Excluded from "
                                "Beginnig and End of the Scan Sets: ")),
                   1);
  ...
  // QLabel* lb_begin          = us_label( tr( "Number of scans at the
  // beginning of run:" ) ); QLabel* lb_end            = us_label( tr( "Number
  // of scans at the end of run:" ) );
...
// build Layout
void
US_ScanExclGui::build_layout (void)
{
  ...
  QLabel *lb_channame = us_label (tr ("Channel Description"));
  QLabel *lb_begin
      = us_label (tr ("Number of scans at\nthe beginning of run:"));
  QLabel *lb_end = us_label (tr ("Number of scans at\nthe end of run:"));
  QLabel *lb_remaining_scans
      = us_label (tr ("Total number of \nremaining scans:"));
  ...
  qDebug () << "Sizes of channels_desc, scan_beg | end: "
            << channels_desc.size () << scan_beg.size () << scan_end.size ();
  ...
  for (int ii = 0; ii < channels_desc.size (); ii++)
    {
      ...
      qDebug () << "Build layout: scan_beg[ii], scan_end[ii]  -- "
                << scan_beg[ii] << scan_end[ii];
      ...
      le_remaining_scans
          = us_lineedit (QString::number (remaining_scans_num), 0, true);
      ...
      connect (sb_begin, SIGNAL (valueChanged (int)), this,
               SLOT (scan_excl_changed (int)));
      connect (sb_end, SIGNAL (valueChanged (int)), this,
               SLOT (scan_excl_changed (int)));
      ...
      if (ii == 0)
        {
          ...
          connect (pb_applyall, SIGNAL (clicked ()), this,
                   SLOT (applied_to_all ()));
        }
      else
        row++;
...
void
US_ScanExclGui::scan_excl_changed (int)
{
  QObject *sobj = sender ();
  ...
  if (oname.contains (": begin"))
    {
      ...
      sb_widget_counterpart = containerWidget->findChild<QSpinBox *> (
  ...
  qDebug () << "Clicked couner:        oname, value -- " << oname
            << sb_widget->value ();
  ...
  QLineEdit *le_remainScans
      = containerWidget->findChild<QLineEdit *> (objName_for_le_remainScans);
  int rem_scans_num
      = maxScans_map[row_num]
        - (sb_widget->value () + sb_widget_counterpart->value ());
...
void
US_ScanExclGui::applied_to_all (void)
{
  ...
  for (int ii = 0; ii < channels_desc.size (); ii++)
    {
      ...
      QSpinBox *sb_b
          = containerWidget->findChild<QSpinBox *> (stchan + "begin");
...
// save gui to parms
void
US_ScanExclGui::gui_to_parms (void)
{
  ...
  for (int ii = 0; ii < channels_desc.size (); ii++)
    {
      ...
      qDebug () << "In gui_to_parms: scna_beg, scan_end -- " << scan_beg[ii]
                << scan_end[ii];
...
// cancel
void
US_ScanExclGui::cancel_update (void)
{
  close ();
}
...
// update
void
US_ScanExclGui::update_scans_excl (void)
{
  gui_to_parms ();
  close ();
}
