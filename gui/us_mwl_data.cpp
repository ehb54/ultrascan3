...
bool US_MwlData::import_data(QString& mwldir, QLineEdit* lestat) {
  ...
  // Read in the data
  QStringList mwrfs =
      ddir.entryList(QStringList("*.mwrs"), QDir::Files, QDir::Name);
  QStringList chans;
  QVector<int> ccscans;
  cells.clear();
  cellchans.clear();
  ...
  cellchans.clear();
  le_status->setText(
      QString("%1 cells and %2 channels ...").arg(ncell).arg(nchan));
  qApp->processEvents();
  ...
  for (int ii = 0; ii < nfile; ii++) {
    ...
    if (ii == 0) {  // At the first file, read in the wavelengths
      ...
      for (int jj = 1; jj < nlamb_i; jj++) {
        if (ri_wavelns[jj] ==
            ri_wavelns[jj - 1]) {  // Mark dup with zero value for now; and bump
                                   // dupl. count
          ri_wavelns[jj - 1] = 0;
          nlmb_dup++;
          DbgLv(1) << "MwDa:   jj" << jj << "ndup lmb" << nlmb_dup
                   << ri_wavelns[jj];
      ...
      DbgLv(1) << "MwDa:    w0 w1 w3 wi wj wk" << ri_wavelns[0] << ri_wavelns[1]
               << ri_wavelns[2] << ri_wavelns[ww - 2] << ri_wavelns[ww - 1]
               << ri_wavelns[ww];
      if (nlmb_dup == 0) {
    ...
    int ccx = cellchans.indexOf(celchn);
    DbgLv(1) << "MwDa:  cell chann celchn ccx" << acell << chann << celchn
             << ccx;
  ...
  DbgLv(1) << "MwDa: wv0 wvm wvn" << ri_wavelns[0] << ri_wavelns[nlamb_i / 2]
           << ri_wavelns[nlamb_i - 1];
#if 0
DbgLv(1) << "MwDa: da20,40" << ri_readings[20][40] << "m+40 n-40"
 << ri_readings[20][npointt/2+40] << ri_readings[20][npointt-41];
  ...
  if (nlmb_dup == 0) {
    le_status->setText(
        tr("Initial MWL import from %1 files is complete.").arg(nfile));
  } else {
    ...
    for (int ii = 0; ii < nlamb_i; ii++) {
      if (ri_wavelns[ii] > 0) ri_wavelns[kk++] = ri_wavelns[ii];
    }
...
DbgLv(1) << "MwDa:L  trx" << trx << "kwvl iwvl_p iwvl cech cech_p"
 << kwvl << iwvl_p << iwvl << celchn << cech_p;
    ...
    if (!chans.contains(chan)) chans << chan;
  ...
  if (nlambda > (klam + 2))
    DbgLv(1) << "MwDa:LdMw:   wl k.." << ri_wavelns[klam - 2]
             << ri_wavelns[klam - 1] << ri_wavelns[klam] << ri_wavelns[klam + 1]
             << ri_wavelns[klam + 2];

  if ((nlambda * ncelchn) ==
      ntriple) {  // If all cells have same wavelengths, just duplicate list
    ...
    for (int trx = 0; trx < ntriple;
         trx++) {  // Fill each cells list with only the wavelengths it has
      QString triple = triples[trx];
      QString celchn = triple.section(" / ", 0, 1);
      int iwvl = triple.section(" / ", 2, 2).toInt();
      int ccx = cellchans.indexOf(celchn);
      // DbgLv(1) << "MwDa:LdMw:    trx" << trx << "ccx" << ccx << "celchn" <<
      // celchn;
...
float US_MwlData::fword(char* cbuf) {
  ...
  if (sizeof(long) == 8) {
    static int kk = 0;
    if (((++kk) % 1000) == 1) qDebug() << "fval" << *fptr << "   kk" << kk;
  }
#endif
  return *fptr;
}
...
void US_MwlData::read_header(QDataStream& ds, DataHdr& hd) {
  ...
  if (evers >
      1.0) {  // Current version that includes both set_speed and rotor_speed
...
void US_MwlData::read_lambdas(QDataStream& ds, QVector<int>& wvs,
  ...
  for (int ii = 0; ii < klambda;
       ii++) {  // Pick up each 2-byte value and store as integer
...
void US_MwlData::read_rdata(QDataStream& ds, QVector<double>& rvs, int& scnx,
                            int& npoint) {
  char cbuf[4];
  int kk = scnx;
  ...
  for (int ii = 0; ii < npoint; ii++) {  // Pick up each 4-byte value, convert
                                         // to double and possibly scale
...
int US_MwlData::set_lambdas(int start, int end, int ccx) {
  ...
  if (ex_wavelns[curccx].count() ==
      0) {  // If out list is empty, build from input
    ...
    ex_wavelns[curccx].clear();
    DbgLv(1) << "SetLamb  (3)n" << nlambda << slambda << elambda << "ccx"
             << curccx;
    ...
    if (slambda < old_start) {  // If start is before old list, grab some from
                                // the original input
    ...
    while (wvx <=
           wvxe) {  // Duplicate lambdas from the range of previous lambdas
      ex_wavelns[curccx] << wkwaves[wvx++];
    }

    DbgLv(1) << "SetLamb   elambda old_end" << elambda << old_end;
    if (elambda > old_end) {  // If end is after old list, append some from the
                              // original list
...
int US_MwlData::build_rawData(QVector<US_DataIO::RawData>& allData) {
  ...
  DbgLv(1) << "BldRawD szs: ccd" << ccdescs.size() << "hdrs" << headers.size()
           << "rds" << ri_readings.size() << ri_readings[0].size() << "wvs"
           << ri_wavelns.size() << "nli nlo" << nlamb_i << nlambda;
  ...
  for (int trx = 0; trx < ntrip_i; trx++) {
    ...
    for (int scx = 0; scx < nscan; scx++) {  // Set scan values
      ...
      if (rpm_set != rpm_setp) {  // A new speed step is reached, re-do speeds
                                  // in the previous one
        nspstep++;
      ...
      //*DEBUG*
      if (trx == 0) {
        DbgLv(1) << "BldRawD      scx" << scx << "jhx" << jhx << "seconds"
      ...
      if (scx < 3 || (scx + 4) > nscan)
        DbgLv(1) << "BldRawD        riread[trx] size"
                 << ri_readings[trx].size();
    ...
    if (trx == 0) {
      ...
      if (evers >
          1.0) {  // In newer data, a set_speed may differ from the average
        if (qAbs(rpm_avg - rpm_set) > signif_dif) {
          QMessageBox::warning(0, tr("Set/Average Speed Difference"),
...
void US_MwlData::read_runxml(QDir ddir, QString curdir) {
  QStringList mwrfs =
      ddir.entryList(QStringList("*.mwrs.xml"), QDir::Files, QDir::Name);
  ...
  while (!xml.atEnd()) {
    ...
    if (xml.isStartElement()) {
      ...
      if (xml.name() == "runID") {
        ...
        DbgLv(1) << "MwDa:RdXML take_i"
                 << att.value("take_intensity").toString() << "evers absorb"
                 << evers << is_absorb;
...
int US_MwlData::set_celchnx(int ccx) {
  // DbgLv(1) << "SetCCX" << ccx;
  if (ccx != -1) curccx = qMax(0, qMin(ccx, (ncelchn - 1)));
...
int US_MwlData::data_index(int waveln, int ccx) {
  ...
  for (int ii = 0; ii < curccx; ii++) datax += ex_wavelns[ii].count();
...
int US_MwlData::update_speedsteps(QVector<SP_SPEEDPROFILE>& speedsteps) {
  ...
  DbgLv(1) << "MwDa:uSS: nsteps" << nsteps << speedsteps.size()
           << s_rpms.size();
  if (nsteps > 0) {
    int kdecr = 0;
    const int mxstep = 10;
