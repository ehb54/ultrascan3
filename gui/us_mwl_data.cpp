...
bool US_MwlData::import_data(QString &mwldir, QLineEdit *lestat) {
   ...
   // Read in the data
   QStringList mwrfs = ddir.entryList(QStringList("*.mwrs"), QDir::Files, QDir::Name);
   QStringList chans;
   QVector<int> ccscans;
   cells.clear();
   cellchans.clear();
   ...
   cellchans.clear();
   le_status->setText(QString("%1 cells and %2 channels ...").arg(ncell).arg(nchan));
   qApp->processEvents();
   ...
   for (int ii = 0; ii < nfile; ii++) {
      ...
      if (ii == 0) { // At the first file, read in the wavelengths
         nlamb_i = hd.nlambda;
         ...
         for (int jj = 1; jj < nlamb_i; jj++) {
            if (ri_wavelns[ jj ] == ri_wavelns[ jj - 1 ]) { // Mark dup with zero value for now; and bump dupl. count
               ri_wavelns[ jj - 1 ] = 0;
               nlmb_dup++;
               DbgLv(1) << "MwDa:   jj" << jj << "ndup lmb" << nlmb_dup << ri_wavelns[ jj ];
            }
            else if (slambda == 0) { // Save start non-duplicate lambda
         ...
         int ww = nlamb_i - 1;
         DbgLv(1) << "MwDa:    w0 w1 w3 wi wj wk" << ri_wavelns[ 0 ] << ri_wavelns[ 1 ] << ri_wavelns[ 2 ]
                  << ri_wavelns[ ww - 2 ] << ri_wavelns[ ww - 1 ] << ri_wavelns[ ww ];
         if (nlmb_dup == 0) {
            le_status->setText(tr("%1 wavelengths ...").arg(nlamb_i));
         }
         else {
      ...
      else { // Otherwise, skip past wavelengths
         ds.skipRawData(nlamb_i * 2);
      }

      int ccx = cellchans.indexOf(celchn);
      DbgLv(1) << "MwDa:  cell chann celchn ccx" << acell << chann << celchn << ccx;
      DbgLv(1) << "MwDa:   celchn size cc0 ccn" << cellchans.size() << cellchans[ 0 ]
               << cellchans[ cellchans.size() - 1 ];
      ...
      DbgLv(1) << "MwDa:  PREPARE   icell ichan nchan" << hd.icell << hd.ichan << nchan << "channel" << hd.channel;
      ...
      le_status->setText(QString("Data in for triple %1, scan %2 ...").arg(tripx + 1).arg(scnx + 1));
      qApp->processEvents();
      //DbgLv(1) << "MwDa:    read_data COMPLETE";
   } // END: header read loop

   DbgLv(1) << "MwDa: wv0 wvm wvn" << ri_wavelns[ 0 ] << ri_wavelns[ nlamb_i / 2 ] << ri_wavelns[ nlamb_i - 1 ];
#if 0
DbgLv(1) << "MwDa: da20,40" << ri_readings[20][40] << "m+40 n-40"
 << ri_readings[20][npointt/2+40] << ri_readings[20][npointt-41];
#endif
   if (nlmb_dup == 0) {
      le_status->setText(tr("Initial MWL import from %1 files is complete.").arg(nfile));
   }
   else {
...
DbgLv(1) << "MwDa:L  trx" << trx << "kwvl iwvl_p iwvl cech cech_p"
 << kwvl << iwvl_p << iwvl << celchn << cech_p;
      ...
      DbgLv(1) << "MwDa:LdMw:  trx" << trx << "triple" << triple << "nscan npoint npointt" << nscan << npoint
               << npointt;
      ...
      if (!ri_wavelns.contains(iwvl)) {
         ...
         DbgLv(1) << "MwDa:L  trx" << trx << "duplicate iwvl" << iwvl << triple << "dup wx" << wx << "trip[wx]"
                  << tripd;
      }
   ...
   if (nlambda > (klam + 2))
      DbgLv(1) << "MwDa:LdMw:   wl k.." << ri_wavelns[ klam - 2 ] << ri_wavelns[ klam - 1 ] << ri_wavelns[ klam ]
               << ri_wavelns[ klam + 1 ] << ri_wavelns[ klam + 2 ];

   if ((nlambda * ncelchn) == ntriple) { // If all cells have same wavelengths, just duplicate list
   ...
   else { // If wavelength lists vary, we must build them carefully
      QVector<int> wvs;
      DbgLv(1) << "MwDa:LdMw:   All Cells NOT Same #Wavelengths";

      for (int ccx = 0; ccx < ncelchn; ccx++) { // First fill with empty vectors
         ex_wavelns << wvs;
      }

      for (int trx = 0; trx < ntriple; trx++) { // Fill each cells list with only the wavelengths it has
...
void US_MwlData::read_header(QDataStream &ds, DataHdr &hd) {
   ...
   if (evers > 1.0) { // Current version that includes both set_speed and rotor_speed
   ...
   else { // Older version with only rotor_speed (use that as set_speed)
...
void US_MwlData::read_lambdas(QDataStream &ds, QVector<int> &wvs, int &klambda) {
   char cbuf[ 4 ];

   wvs.reserve(klambda);

   for (int ii = 0; ii < klambda; ii++) { // Pick up each 2-byte value and store as integer
...
void US_MwlData::read_rdata(QDataStream &ds, QVector<double> &rvs, int &scnx, int &npoint) {
   char cbuf[ 4 ];
   int kk = scnx;
   ...
   for (int ii = 0; ii < npoint; ii++) { // Pick up each 4-byte value, convert to double and possibly scale
...
int US_MwlData::set_lambdas(int start, int end, int ccx) {
   ...
   if (ex_wavelns[ curccx ].count() == 0) { // If out list is empty, build from input
      ...
      while (wvx <= wvxe) { // Duplicate lambdas from the range of raw lambdas
         ex_wavelns[ curccx ] << ri_wavelns[ wvx++ ];
      }
      DbgLv(1) << "SetLamb  (2)n" << nlambda << wvxs << wvxe << wvx;
   }

   else { // If out list exists, pair it down to the new range
      ...
      if (slambda < old_start) { // If start is before old list, grab some from the original input
      ...
      while (wvx <= wvxe) { // Duplicate lambdas from the range of previous lambdas
         ex_wavelns[ curccx ] << wkwaves[ wvx++ ];
      }

      DbgLv(1) << "SetLamb   elambda old_end" << elambda << old_end;
      if (elambda > old_end) { // If end is after old list, append some from the original list
...
int US_MwlData::build_rawData(QVector<US_DataIO::RawData> &allData) {
   ...
   a_rpms.clear();
   d_rpms.clear();
   DbgLv(1) << "BldRawD radv radi" << rad_val << rad_inc << "npoint" << npoint << "  evers" << evers;
   ...
   int wvx = 0;
   int hdx = 0;
   DbgLv(1) << "BldRawD szs: ccd" << ccdescs.size() << "hdrs" << headers.size() << "rds" << ri_readings.size()
            << ri_readings[ 0 ].size() << "wvs" << ri_wavelns.size() << "nli nlo" << nlamb_i << nlambda;
   ...
   for (int trx = 0; trx < ntrip_i; trx++) {
      ...
      for (int scx = 0; scx < nscan; scx++) { // Set scan values
         US_DataIO::Scan scan;
         ...
         if (rpm_set != rpm_setp) { // A new speed step is reached, re-do speeds in the previous one
            nspstep++;
         ...
         //*DEBUG*
         if (trx == 0) {
            DbgLv(1) << "BldRawD      scx" << scx << "jhx" << jhx << "seconds" << scan.seconds << "rpm" << scan.rpm
         ...
         for (int kk = 0; kk < npoint; kk++) { // Set readings values
      ...
      if (evers < 1.2) { // For an old data version, get the average of all scans
         rpm_min = 1.e+9;
         rpm_max = 1.e-9;
         rpm_sum = 0.0;
      ...
      if (trx == 0) {
         ...
         if (evers > 1.0) { // In newer data, a set_speed may differ from the average
            if (qAbs(rpm_avg - rpm_set) > signif_dif) {
               QMessageBox::warning(
                  0, tr("Set/Average Speed Difference"),
      ...
      le_status->setText(tr("Of %1 raw AUCs, built %2").arg(ntriple).arg(trx + 1));
      qApp->processEvents();
      wvx++;
   ...
   le_status->setText(tr("All %1 raw AUCs have been build.").arg(ntriple));
   qApp->processEvents();
...
void US_MwlData::read_runxml(QDir ddir, QString curdir) {
   QStringList mwrfs = ddir.entryList(QStringList("*.mwrs.xml"), QDir::Files, QDir::Name);
   ...
   while (!xml.atEnd()) {
      ...
      if (xml.isStartElement()) {
         ...
         if (xml.name() == "runID") {
            ...
            DbgLv(1) << "MwDa:RdXML take_i" << att.value("take_intensity").toString() << "evers absorb" << evers
                     << is_absorb;
         }
...
int US_MwlData::update_speedsteps(QVector<SP_SPEEDPROFILE> &speedsteps) {
   ...
   if (nsteps > 0) {
      ...
      for (int jj = 0; jj < nsteps; jj++) {
         ...
         DbgLv(1) << "MwDa:uSS:   jj" << jj << "set_speed avg_speed speed_stddev" << s_rpms[ jj ] << a_rpms[ jj ]
                  << d_rpms[ jj ];
         if (jj > 0 && speedsteps[ jj ].set_speed < speedsteps[ jj - 1 ].set_speed)
            kdecr++;
      }
