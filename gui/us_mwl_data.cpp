...
bool
US_MwlData::import_data (QString &mwldir, QLineEdit *lestat)
{
  bool status = true;
  int nlmb_dup = 0;
  cur_dir = mwldir;
  ...
  le_status->setText (
      QString ("%1 files from %2 ...").arg (nfile).arg (runID));
  qApp->processEvents ();
  ...
  for (int ii = 0; ii < nfile; ii++)
    {
      ...
      if (ii == 0)
        { // At the first file, read in the wavelengths
          ...
          int ww = nlamb_i - 1;
          DbgLv (1) << "MwDa:    w0 w1 w3 wi wj wk" << ri_wavelns[0]
                    << ri_wavelns[1] << ri_wavelns[2] << ri_wavelns[ww - 2]
                    << ri_wavelns[ww - 1] << ri_wavelns[ww];
          if (nlmb_dup == 0)
            {
          ...
          DbgLv (1) << "MwDa:   ri_readings CREATED size"
                    << ri_readings.size ();
        }
      else
      ...
      DbgLv (1) << "MwDa:  PREPARE rdata ccx tripx scnx" << ccx << tripx
                << scnx;
...
void
US_MwlData::load_mwl (QVector<US_DataIO::RawData> &allData)
{
  QStringList chans;
  nfile = allData.size ();
  ntriple = nfile;
  ...
  npointt = npoint * nscan;
  DbgLv (1) << "MwDa:LdMw: nfile" << nfile << "nscan npoint" << nscan
            << npoint;
 ...
 << kwvl << iwvl_p << iwvl << celchn << cech_p;
      ...
      if (!ri_wavelns.contains (iwvl))
      ...
      else if (chans.size () < 2)
        {
          ...
          DbgLv (1) << "MwDa:L  trx" << trx << "duplicate iwvl" << iwvl
                    << triple << "dup wx" << wx << "trip[wx]" << tripd;
        }
  ...
  if (nlambda > (klam + 2))
    DbgLv (1) << "MwDa:LdMw:   wl k.." << ri_wavelns[klam - 2]
              << ri_wavelns[klam - 1] << ri_wavelns[klam]
    ...
    { // If wavelength lists vary, we must build them carefully
      ...
      for (int trx = 0; trx < ntriple; trx++)
        { // Fill each cells list with only the wavelengths it has
          ...
          if (ccx < 0)
            {
              qDebug () << "load_mwl:*ERROR* unexpected missing cell"
                        << celchn;
              continue;
            }
...
int
US_MwlData::rvalues (int &tripx, int &scanx, QVector<double> &rvs)
{
  int jj = scanx * npoint;
  rvs.clear ();
  rvs.reserve (npoint);
...
int
US_MwlData::lambdas (QVector<int> &wls, int ccx)
{
  set_celchnx (ccx);
...
int
US_MwlData::lambdas_raw (QVector<int> &wls)
{
  wls.clear ();
  wls.reserve (nlamb_i);
...
void
US_MwlData::read_header (QDataStream &ds, DataHdr &hd)
{
  char chary[28];
  ...
  if (evers > 1.0)
    ...
    { // Older version with only rotor_speed (use that as set_speed)
      ...
      // DbgLv(1) << "EVER=1.0:  rspeed sspeed" << hd.rotor_speed <<
      // hd.set_speed;
...
void
US_MwlData::read_lambdas (QDataStream &ds, QVector<int> &wvs, int &klambda)
{
  char cbuf[4];
...
void
US_MwlData::read_rdata (QDataStream &ds, QVector<double> &rvs, int &scnx,
                        int &npoint)
{
  char cbuf[4];
  int kk = scnx;
...
int
US_MwlData::set_lambdas (int start, int end, int ccx)
{
  set_celchnx (ccx);
    ...
    { // If out list exists, pair it down to the new range
      ...
      if (elambda > old_end)
        { // If end is after old list, append some from the original list
          ...
          DbgLv (1) << "SetLamb     wvx s,e" << wvxs << wvxe << "old_end"
                    << old_end;
...
int
US_MwlData::set_lambdas (QVector<int> &wls, int ccx)
{
  set_celchnx (ccx);
...
int
US_MwlData::indexOfLambda (int lambda)
{
...
int
US_MwlData::cellchannels (QStringList &celchns)
{
  celchns = cellchans;
  return ncelchn;
}
...
int
US_MwlData::build_rawData (QVector<US_DataIO::RawData> &allData)
{
  ...
  DbgLv (1) << "BldRawD   xout size ntrip" << xout.size () << npoint
            << ntrip_i;
  ...
  int wvx = 0;
  int hdx = 0;
  DbgLv (1) << "BldRawD szs: ccd" << ccdescs.size () << "hdrs"
            << headers.size () << "rds" << ri_readings.size ()
            << ri_readings[0].size () << "wvs" << ri_wavelns.size ()
            << "nli nlo" << nlamb_i << nlambda;
  ...
  for (int trx = 0; trx < ntrip_i; trx++)
    {
      ...
      for (int scx = 0; scx < nscan; scx++)
        { // Set scan values
          ...
          if (scx < 3 || (scx + 4) > nscan)
            DbgLv (1) << "BldRawD       scx" << scx << "jhx" << jhx << "wvx"
                      << wvx;
          ...
          if (scx < 3 || (scx + 4) > nscan)
            DbgLv (1) << "BldRawD        rpm_set rpm_setp" << rpm_set
                      << rpm_setp;
          if (rpm_set != rpm_setp)
      ...
      if (trx == 0)
        {
          DbgLv (1) << "BldRawD trx=" << trx
                    << "rpm_min rpm_max rpm_avg rpm_set" << rpm_min << rpm_max
                    << rpm_avg << rpm_set << "speed steps" << nspstep;
        }
      //*DEBUG*
      ...
      DbgLv (1) << "BldRawD   ccx wvx hdx" << ccx << wvx << hdx
                << headers.size ();
    } // END: triple loop
...
int
US_MwlData::countOf (QString key)
{
  mapCounts ();
...
QString
US_MwlData::cc_description (QString celchn)
{
  int ccx = cellchans.indexOf (celchn);
...
void
US_MwlData::run_values (QString &arunid, QString &aruntype)
{
  arunid = runID;
...
void
US_MwlData::read_runxml (QDir ddir, QString curdir)
{
  QStringList mwrfs
          ...
          if (xml.name () == "runID")
            {
              ...
              DbgLv (1) << "MwDa:RdXML take_i"
                        << att.value ("take_intensity").toString ()
...
int
US_MwlData::set_celchnx (int ccx)
{
  // DbgLv(1) << "SetCCX" << ccx;
  if (ccx != -1)
...
int
US_MwlData::data_index (int waveln, int ccx)
{
  // Set the current cell/channel index
  set_celchnx (ccx);
...
// Return the output data index of a wavelength in a channel
int
US_MwlData::data_index (QString clambda, int ccx)
{
...
// Return the output data index of a wavelength in a channel
int
US_MwlData::data_index (QString clambda, QString celchn)
{
  int ccx = cellchans.indexOf (celchn);
  return data_index (clambda.toInt (), ccx);
}
...
int
US_MwlData::update_speedsteps (QVector<SP_SPEEDPROFILE> &speedsteps)
{
      ...
      for (int jj = 0; jj < nsteps; jj++)
        {
          ...
          DbgLv (1) << "MwDa:uSS:   jj" << jj
                    << "set_speed avg_speed speed_stddev" << s_rpms[jj]
                    << a_rpms[jj] << d_rpms[jj];
          if (jj > 0
...
int
US_MwlData::raw_speeds (QVector<double> &rrpms)
{
