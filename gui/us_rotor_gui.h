...
class US_GUI_EXTERN US_RotorGui : public US_WidgetsDialog {
 ...
 public:
  /*! \brief Overloaded constructor for the US_RotorGui class. This one is
      ...
      \param calibrationIn A reference to a structure that contains the
     currently selected calibration information
  */
  US_RotorGui(bool = false, int = US_Disk_DB_Controls::Default,
  ...
  /*! \brief Overloaded constructor for the US_RotorGui class. In this case,
      ...
      \param calibration The calibration structure passed in from the
     calibration program
      \param new_calibration  A boolean value indicating whether the caller is
                              US_RotorCalibration with new calibration data or
     not
      \param signal_wanted A boolean value indicating whether the caller
                     wants a signal to be emitted
      \param select_db_disk Indicates whether the default search is on
                     the local disk or in the DB
  */
  US_RotorGui(US_Rotor::RotorCalibration&, bool = false, bool = false,
  ...
  US_Rotor::RotorCalibration
      currentCalibration;  //!< Current calibration structure

 signals:

  /*! \brief    Signal to pass the accepted rotor calibration to the calling
     program
      \param    Rotor the rotor structure selected by the user
      \param    RotorCalibration the rotor calibration structure selected by the
     user
  */
  void RotorCalibrationSelected(US_Rotor::Rotor& Rotor,
                                US_Rotor::RotorCalibration& RotorCalibration);

  /*! \brief    Signal to pass to the calling program if the operation was
   * canceled
   */
 ...
 private slots:
  ...
  void help(void) { showHelp.show_help("manual/rotor.html"); };
