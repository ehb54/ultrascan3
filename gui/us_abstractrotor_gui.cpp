...
US_AbstractRotorGui::US_AbstractRotorGui (bool signal_wanted,
                                          int select_db_disk,
...
void
US_AbstractRotorGui::setupGui (int select_db_disk)
{
  ...
  connect (cb_rotors,
           SIGNAL (activated (int)), // Only if the user has changed it
...
void
US_AbstractRotorGui::reset (void)
{
  le_name->setText ("");
  le_serialNumber->setText ("");
...
void
US_AbstractRotorGui::source_changed (bool db)
{
  QStringList DB = US_Settings::defaultDB ();
...
bool
US_AbstractRotorGui::loadAbstractRotors (void)
{
  ...
  if (disk_controls->db ())
    {
      ...
      if (US_Rotor::readAbstractRotorsDB (abstractRotorList, &db)
          == US_Rotor::NOT_FOUND)
        {
          QMessageBox::warning (
              this, tr ("Database Problem"),
              tr ("Could not read abstract rotor information \n"));
          return (false);
        }
    }
  ...
  else
    {
      ...
      if (US_Rotor::readAbstractRotorsDisk (abstractRotorList)
          == US_Rotor::NOT_FOUND)
        {
          QMessageBox::warning (
              this, tr ("Local Disk Problem"),
              tr ("Could not read abstract rotor information\n"));
          return (false);
        }
    }
...
// Get the current logical index into the abstractRotorList vector from the
// rotor cb
int
US_AbstractRotorGui::getIndex (void)
{
...
void
US_AbstractRotorGui::select ()
{
  // First double-check if the user has filled out name and serial
  if (le_name->text ().isEmpty () || le_serialNumber->text ().isEmpty ())
    {
      QMessageBox::warning (
          this, tr ("Please provide the missing information:"),
          tr ("Please provide both a rotor name and serial number"));
      return;
    }
...
void
US_AbstractRotorGui::connect_error (const QString &error)
{
...
void
US_AbstractRotorGui::showDetails (int)
{
  ...
  text += "Magnet Offset:   " + QString::number (ar.magnetOffset)
          + " degrees\n";
