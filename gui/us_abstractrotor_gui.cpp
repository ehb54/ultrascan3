...
US_AbstractRotorGui::US_AbstractRotorGui(bool signal_wanted, int select_db_disk,
...
bool US_AbstractRotorGui::loadAbstractRotors(void) {
  ...
  if (disk_controls->db()) {
    ...
    if (US_Rotor::readAbstractRotorsDisk(abstractRotorList) ==
        US_Rotor::NOT_FOUND) {
      QMessageBox::warning(this, tr("Local Disk Problem"),
                           tr("Could not read abstract rotor information\n"));
      return (false);
    }
  }
...
int US_AbstractRotorGui::getIndex(void) {
  ...
  for (int i = 0; i < abstractRotorList.size(); i++) {
    if (abstractRotorList[i].ID == arID) return i;
  }
...
void US_AbstractRotorGui::showDetails(int) {
  if (abstractRotorList.size() == 0) return;  // nothing to display
