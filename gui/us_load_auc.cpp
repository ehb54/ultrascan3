...
US_LoadAUC::US_LoadAUC(bool local, QMap<QString, QString>& prot_det,
                       QVector<US_DataIO::RawData>& rData, QStringList& trips,
                       QString& wdir)
...
US_LoadAUC::US_LoadAUC(bool local, QString auto_mode,
                       QVector<US_DataIO::RawData>& rData, QStringList& trips,
                       QString& wdir)
...
US_LoadAUC::US_LoadAUC(bool local, QVector<US_DataIO::RawData>& rData,
                       QStringList& trips, QString& wdir)
    : US_WidgetsDialog(0, 0), rawList(rData), triples(trips), workingDir(wdir) {
  int ddstate;
...
void US_LoadAUC::load(void) {
  if (us_auto_mode_abde) {
    tree->setCurrentItem(tree->topLevelItem(0),       // item
                         0,                           // column
        ...
        for (int i = 0; i < datamap.keys().size(); ++i) {
          ...
          if (db.lastErrno() != US_DB2::OK) {
            QMessageBox::warning(this, tr("Connection Problem"),
                                 tr("Assessing EditedData IDs: Could not "
                                    "connect to database \n") +
                                     db.lastError());
            return;
          }
          ...
          QStringList qry;
          qry.clear();
          qry << "get_editedDataIDs"
              << QString::number(datamap[datamap.keys()[i]].DB_id);
          db.query(qry);
  ...
  for (int ii = 0; ii < nitems;
       ii++) {  // Review and record selected list items
    item = items.at(ii);
  ...
  qDebug() << "Ed:Ld: ntops" << ntops << "trips" << tripls.count() << "nitems"
           << nitems << "nruns" << nruns;
  ...
  if (ntops ==
      1) {  // If a single top-level item was chosen, get all its triples
    QString runID = sdescs.at(0).runID;  // We have the first triple
    item = items.at(0);

    qDebug() << "[in US_LoadAUC() load() ], runID -- " << runID;

    for (int ii = 1; ii < item->childCount();
         ii++) {  // Save any run triples beyond first
...
void US_LoadAUC::populate_tree(void) {
  ...
  if (sel_run) {              // A run has been selected, so scan for AUCs
    ...
    te_notes->setText(
        tr("Select a run, then click on \"Show Triples\""
           " to fill in triples;\n"
  ...
  if (naucf == 0) {
    QMessageBox::information(this, tr("No AUC Files"),
  ...
  QStringList headers;
  headers << (sel_run ? tr("Run|Triple") : tr("Run")) << tr("Date")
          << tr("DbID") << tr("Label");
  tree->setColumnCount(4);
  tree->setHeaderLabels(headers);
  tree->setSortingEnabled(false);
  ...
  for (int ii = 0; ii < naucf; ii++) {
    ...
    QStringList topItem;
    topItem << ddesc.runID << QString(ddesc.date).section(" ", 0, 0)
            << QString().sprintf("%6d", ddesc.DB_id) << ddesc.label;

    if (!runIDs.contains(ddesc.runID)) {  // First entry for a run, so preceed
                                          // with top-level item
    ...
    if (sel_run) new QTreeWidgetItem(top, item);
  }
...
void US_LoadAUC::fill_in(void) {
  ...
  if (selitems.size() <
      1) {  // "Show Triples" with no run selected:  build full data tree
    runID_sel = "";
    te_notes->setText(
        tr("Reading AUC information to fully populate"
           " the list data tree..."));
  }

  else {  // "Show Triples" with run selected:  build a data tree for selected
          // run
...
bool US_LoadAUC::eventFilter(QObject* obj, QEvent* e) {
  if (obj == tree &&
      e->type() == QEvent::ContextMenu) {  // It is right-mouse-button-click, so
...
void US_LoadAUC::show_data_info(QPoint pos) {
  ...
  QTreeWidgetItem* item =
      selitems.size() > 0 ? selitems.at(0) : tree->itemAt(pos);
  ...
  for (int ii = 0; ii < aucdirs.size(); ii++) {
    ...
    for (int jj = 0; jj < aucfiles.size(); jj++) {
      ...
      QString date = US_Util::toUTCDatetimeText(
          QFileInfo(aucfname).lastModified().toUTC().toString(Qt::ISODate),
          true);
      ...
      filei.close();
      QString idata = label + "^" + runID + "^" + tripID + "^" + aucfbase +
                      "^" + rawGUID + "^" + rawDataID + "^" + date;
      runIDs << runID;  // Save each run
      infoDs << idata;  // Save concatenated description string
      naucf++;
      // qDebug() << "ScDk: run" << runID << "naucf" << naucf << "label" <<
      // label;
    }
  }
...
int US_LoadAUC::scan_run_disk() {
  ...
  for (int ii = 0; ii < aucdirs.size(); ii++) {
    ...
    QString date = US_Util::toUTCDatetimeText(
        QFileInfo(aucfname).lastModified().toUTC().toString(Qt::ISODate), true);
