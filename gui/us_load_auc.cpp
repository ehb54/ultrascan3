...
US_LoadAUC::US_LoadAUC(
   bool local, QMap<QString, QString> &prot_det, QVector<US_DataIO::RawData> &rData, QStringList &trips, QString &wdir) :
    US_WidgetsDialog(0, 0), protocol_details(prot_det), rawList(rData), triples(trips), workingDir(wdir) {
   int ddstate;

   us_auto_mode = false;
   us_auto_mode_abde = true;
   ...
   personID = protocol_details[ "invID_passed" ].toInt();
   ...
   QString number = (id > 0) ? QString::number(personID) + ": " : "";
   ...
   connect(le_search, SIGNAL(textChanged(const QString &)), SLOT(limit_data(const QString &)));
   search->addWidget(le_search);

   // Tree
   QFont tr_font(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize());
   ...
   QStringList headers;
   headers << tr("Run|Triple") << tr("Date") << tr("DbID") << tr("Label");
   ...
   //for GMP
   populate_tree(); //another populate with sel_run==true;
   load(); //in load(), specify selected items (a single top-level one)

   resize(800, 500);
}

US_LoadAUC::US_LoadAUC(
   bool local, QString auto_mode, QVector<US_DataIO::RawData> &rData, QStringList &trips, QString &wdir) :
    US_WidgetsDialog(0, 0), rawList(rData), triples(trips), workingDir(wdir) {
   int ddstate;

   us_auto_mode = true;
   us_auto_mode_abde = false;
...
US_LoadAUC::US_LoadAUC(bool local, QVector<US_DataIO::RawData> &rData, QStringList &trips, QString &wdir) :
    US_WidgetsDialog(0, 0), rawList(rData), triples(trips), workingDir(wdir) {
   int ddstate;
...
void US_LoadAUC::load(void) {
   if (us_auto_mode_abde) {
      tree->setCurrentItem(
         tree->topLevelItem(0), // item
         0, // column
   ...
   if (nitems == 0) {
      QMessageBox::warning(this, tr("Invalid Selection"), tr("No selection has been made"));
      return;
   }
   ...
   if (!sel_run) {
      ...
      if (dkdb_cntrls->db()) { // Scan database for AUC then load all of them
         scan_db();
         ...
         if (us_auto_mode) {
            ...
            for (int i = 0; i < datamap.keys().size(); ++i) {
               qDebug() << datamap.keys()[ i ] << ", rawDataGUID, rawDataID -- "
                        << datamap[ datamap.keys()[ i ] ].rawGUID << datamap[ datamap.keys()[ i ] ].DB_id;
            ...
            if (editDataIDs.size() == 0) {
               QMessageBox::warning(
                  this, tr("No Edits Available"),
                  tr("No Edit children exist for selected run"
                     " '%1'. The run will be hidden.")
                     .arg(runID_sel));
      ...
      else { // Scan local files for AUC then load all of them
         scan_disk();
   ...
   for (int ii = 0; ii < nitems; ii++) { // Review and record selected list items
      item = items.at(ii);
      QString lkey = item->text(0);
      DataDesc ddesc;

      if (item->parent() != NULL) { // Not top-level
      ...
      if (!runIDs.contains(runID)) { // New runID:  save it and bump run count
         runIDs << runID;
         nruns++;
      }
   ...
   qDebug() << "Ed:Ld: ntops" << ntops << "trips" << tripls.count() << "nitems" << nitems << "nruns" << nruns;

   // Verify that selections are consistent
   if (ntops > 1 || nruns > 1) {
      QMessageBox::warning(this, tr("Invalid Selection"), tr("Only items from one run may be selected."));
      return;
   }

   if (ntops != nitems && tripls.count() != nitems) {
      QMessageBox::warning(this, tr("Invalid Selection"), tr("A single run or items from a single run must be selected."));
      return;
   }

   if (ntops == 1) { // If a single top-level item was chosen, get all its triples
      QString runID = sdescs.at(0).runID; // We have the first triple
      item = items.at(0);

      qDebug() << "[in US_LoadAUC() load() ], runID -- " << runID;

      for (int ii = 1; ii < item->childCount(); ii++) { // Save any run triples beyond first
   ...
   QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
   te_notes->setText(tr("Loading data from ") + (dkdb_cntrls->db() ? tr("Database") : tr("Local Disk")) + " ...");
...
void US_LoadAUC::populate_tree(void) {
   ...
   if (sel_run) { // A run has been selected, so scan for AUCs
      if (dkdb_cntrls->db()) { // Scan database for AUC
         naucf = scan_db();
         qDebug() << "in populate, second scan_db(), naucf -- " << naucf;
      }

      else { // Scan local files for AUC
         naucf = scan_disk();
      }
   ...
   else { // No run is selected, so scan for runs
      if (dkdb_cntrls->db()) { // Scan database for runs
         naucf = scan_run_db();
         qDebug() << "in populate, first scan_run_db(), naucf -- " << naucf;
      }

      else { // Scan local files for runs
         naucf = scan_run_disk();
      }
   ...
   if (naucf == 0) {
      QMessageBox::information(this, tr("No AUC Files"), tr("No AUC files were found."));
   ...
   QStringList headers;
   headers << (sel_run ? tr("Run|Triple") : tr("Run")) << tr("Date") << tr("DbID") << tr("Label");
   tree->setColumnCount(4);
   tree->setHeaderLabels(headers);
   tree->setSortingEnabled(false);
   ...
   for (int ii = 0; ii < naucf; ii++) {
      ...
      QStringList topItem;
      topItem << ddesc.runID << QString(ddesc.date).section(" ", 0, 0) << QString().sprintf("%6d", ddesc.DB_id)
              << ddesc.label;

      if (!runIDs.contains(ddesc.runID)) { // First entry for a run, so preceed with top-level item
...
void US_LoadAUC::limit_data(const QString &search) {
   ...
   for (int i = 0; i < tree->topLevelItemCount(); i++) {
      ...
      item->setHidden(have_search && !lkey.contains(search, Qt::CaseInsensitive));
   }
}
...
void US_LoadAUC::fill_in(void) {
   ...
   if (selitems.size() < 1) { // "Show Triples" with no run selected:  build full data tree
      runID_sel = "";
      te_notes->setText(tr("Reading AUC information to fully populate"
                           " the list data tree..."));
   }

   else { // "Show Triples" with run selected:  build a data tree for selected run
...
void US_LoadAUC::update_disk_db(bool db) {
   ...
   setWindowTitle(tr("Load AUC Data from ") + (db ? tr("DB") : tr("Local Disk")));
...
bool US_LoadAUC::eventFilter(QObject *obj, QEvent *e) {
   if (obj == tree && e->type() == QEvent::ContextMenu) { // It is right-mouse-button-click, so show data information
...
void US_LoadAUC::show_data_info(QPoint pos) {
   QList<QTreeWidgetItem *> selitems = tree->selectedItems();
   QTreeWidgetItem *item = selitems.size() > 0 ? selitems.at(0) : tree->itemAt(pos);
   ...
   dtext += toplev ? tr("the first triple of run %1").arg(runID) : tr("item %1").arg(lkey);
   dtext = dtext + "\n" + tr("\n  List Item Key:          ") + lkey + tr("\n  AUC Database ID:        ")
           + QString::number(ddesc.DB_id) + tr("\n  AUC Global ID:          ") + ddesc.rawGUID
           + tr("\n  Filename:               ") + ddesc.filename + tr("\n  Last Updated:           ") + ddesc.date
           + tr("\n  Label:                  ") + ddesc.label + tr("\n  Run ID:                 ") + runID
           + tr("\n  Triple ID:              ") + tripID + tr("\n  Triple Index:           ")
           + QString::number(ddesc.tripndx) + tr("\n  Triples per Run:        ") + QString::number(ddesc.tripknt) + "";
   ...
   eddiag->e->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize()));
...
int US_LoadAUC::scan_disk() {
   ...
   for (int ii = 0; ii < aucdirs.size(); ii++) {
      ...
      for (int jj = 0; jj < aucfiles.size(); jj++) {
         ...
         QString date
            = US_Util::toUTCDatetimeText(QFileInfo(aucfname).lastModified().toUTC().toString(Qt::ISODate), true);
         ...
         filei.close();
         QString idata = label + "^" + runID + "^" + tripID + "^" + aucfbase + "^" + rawGUID + "^" + rawDataID + "^"
                         + date;
         runIDs << runID; // Save each run
         infoDs << idata; // Save concatenated description string
         naucf++;
...
int US_LoadAUC::scan_run_db() {
   ...
   while (db.next()) { // Accumulate data description objects from database information
      ...
      else if (us_auto_mode_abde) {
         qDebug() << "[in US_LoadAUC: scan_run_db() ]";
         if (runID == protocol_details[ "filename_abde" ]) {
            qDebug() << "runID, protocol_details[\"filename_abde\"] " << runID << ","
...
int US_LoadAUC::scan_run_disk() {
   ...
   for (int ii = 0; ii < aucdirs.size(); ii++) {
      ...
      QString date = US_Util::toUTCDatetimeText(QFileInfo(aucfname).lastModified().toUTC().toString(Qt::ISODate), true);
...
void US_LoadAUC::create_descs(QStringList &runIDs, QStringList &infoDs, int naucf) {
