...
US_LoadAUC::US_LoadAUC (bool local, QMap<QString, QString> &prot_det,
                        ...
                        QString &wdir)
    : US_WidgetsDialog (0, 0), protocol_details (prot_det), rawList (rData),
      triples (trips), workingDir (wdir)
{
  int ddstate;
...
US_LoadAUC::US_LoadAUC (bool local, QString auto_mode,
                        QVector<US_DataIO::RawData> &rData, QStringList &trips,
                        QString &wdir)
    : US_WidgetsDialog (0, 0), rawList (rData), triples (trips),
      workingDir (wdir)
{
  int ddstate;
          ...
          if (us_auto_mode)
            {
              ...
              for (int i = 0; i < datamap.keys ().size (); ++i)
                {
                  qDebug ()
                      << datamap.keys ()[i] << ", rawDataGUID, rawDataID -- "
                  ...
                  if (db.lastErrno () != US_DB2::OK)
                    {
                      QMessageBox::warning (
                          ...
                          tr ("Assessing EditedData IDs: Could not connect to "
                              "database \n")
                              + db.lastError ());
                      return;
                    }
...
void
US_LoadAUC::sel_investigator (void)
{
  if (!dkdb_cntrls->db ())
...
void
US_LoadAUC::assign_investigator (int invID)
{
  personID = invID;
...
void
US_LoadAUC::limit_data (const QString &search)
{
...
void
US_LoadAUC::update_disk_db (bool db)
{
  emit changed (db);
...
bool
US_LoadAUC::eventFilter (QObject *obj, QEvent *e)
{
...
void
US_LoadAUC::show_data_info (QPoint pos)
{
  QList<QTreeWidgetItem *> selitems = tree->selectedItems ();
  ...
  dtext = dtext + "\n" + tr ("\n  List Item Key:          ") + lkey
          ...
          + tr ("\n  Triple ID:              ") + tripID
          + tr ("\n  Triple Index:           ")
          + QString::number (ddesc.tripndx)
          + tr ("\n  Triples per Run:        ")
          + QString::number (ddesc.tripknt) + "";
...
US_LoadAUC::scan_db ()
{
  ...
  while (db.next ())
    { // Accumulate data description objects from database information
      ...
      naucf++;
      // if(rfilter)
      // qDebug() << "LdA: naucf" << naucf << "runID" << runID << "idata" <<
      // idata
      ...
      for (int jj = 0; jj < aucfiles.size (); jj++)
        {
          ...
          QString date = US_Util::toUTCDatetimeText (
              QFileInfo (aucfname).lastModified ().toUTC ().toString (
                  Qt::ISODate),
              true);
...
US_LoadAUC::scan_run_db ()
{
  ...
  while (db.next ())
    { // Accumulate data description objects from database information
      ...
      bool expIsBuoyancy
          = (etype.compare ("Buoyancy", Qt::CaseInsensitive) == 0);
      ...
      if (us_auto_mode)
      ...
      else if (us_auto_mode_abde)
        {
          ...
          if (runID == protocol_details["filename_abde"])
            {
              qDebug () << "runID, protocol_details[\"filename_abde\"] "
                        << runID << "," << protocol_details["filename_abde"];
              runIDs << runID;
              infoDs << idata;
              runID_sel = runID;
              sel_run = true; //!!!
...
void
US_LoadAUC::create_descs (QStringList &runIDs, QStringList &infoDs, int naucf)
{
...
void
US_LoadAUC::load_db (QList<DataDesc> &sdescs)
{
  int nerr = 0;
  QString emsg;
  QString rdir = US_Settings::resultDir ();
...
void
US_LoadAUC::load_disk (QList<DataDesc> &sdescs)
{
  int nerr = 0;
  QString emsg;
  QString runID = sdescs.count () > 0 ? sdescs.at (0).runID : "";
