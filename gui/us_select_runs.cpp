...
US_SelectRuns::US_SelectRuns(bool dbase, QStringList &runIDs) : US_WidgetsDialog(0, 0), runIDs(runIDs) {
   sel_db = dbase;

   setWindowTitle(tr("Select Run(s) as Models Pre-Filter (%1)").arg(sel_db ? "DB" : "Local"));
...
void US_SelectRuns::scan_local_run(void) {
   ...
   for (int ii = 0; ii < aucdirs.size(); ii++) {
      ...
      if (fileri.open(QIODevice::ReadOnly | QIODevice::Text)) {
         ...
         date = US_Util::toUTCDatetimeText(QFileInfo(fileri).lastModified().toUTC().toString(Qt::ISODate), true)
                   .section("", 0, 0);
      }
