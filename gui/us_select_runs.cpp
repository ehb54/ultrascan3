...
void
US_SelectRuns::search (const QString &search_string)
{
  ...
  if (sel_db)
    {
      ...
      qDebug () << "Acpt: runID" << runIDs[0] << "count_models_by_runID"
                << nmodl;
    }
...
void
US_SelectRuns::scan_local_run (void)
{
      ...
      if (fileri.open (QIODevice::ReadOnly | QIODevice::Text))
        {
          ...
          date = US_Util::toUTCDatetimeText (
                     QFileInfo (fileri).lastModified ().toUTC ().toString (
                         Qt::ISODate),
                     true)
                     .section ("", 0, 0);
        }
...
void
US_SelectRuns::build_runids (void)
{
  rlabels = runmap.keys ();
...
void
US_SelectRuns::update_person (int ID)
{
...
// Slot to handle accept in investigator dialog
void
US_SelectRuns::update_disk_db (bool isDB)
{
