    ...
    : US_Widgets ()
{
  ...
  QGridLayout *carbo = us_radiobutton (tr ("Carbohydrate/Other"), rb_carbo);
  ...
  lb_vbar20 = us_label (tr ("VBar (<small>cm<sup>3</sup>/g at 20") + DEGC
                        + "</small>):");
  ...
  for (int ii = 0; ii < 12; ii++)
    main->setColumnStretch (ii, 99);
  for (int ii = 0; ii < 12; ii++)
    main->setRowStretch (ii, 99);

  connect (typeButtons, SIGNAL (buttonClicked (int)), this,
  ...
  connect (pb_accept, SIGNAL (clicked ()), this, SLOT (accept_analyte ()));
  connect (pb_info, SIGNAL (clicked ()), this, SLOT (info_analyte ()));
  connect (pb_help, SIGNAL (clicked ()), this, SLOT (help ()));
  connect (lw_analyte_list, SIGNAL (itemSelectionChanged ()), this,
           SLOT (select_analyte ()));
  connect (pb_spectrum, SIGNAL (clicked ()), this, SLOT (spectrum ()));
  connect (pb_sequence, SIGNAL (clicked ()), this, SLOT (sequence ()));
  connect (pb_delete, SIGNAL (clicked ()), this, SLOT (delete_analyte ()));
...
void
US_AnalyteMgrSelect::select_analyte ()
{
...
void
US_AnalyteMgrSelect::select_analyte (QListWidgetItem *tmp_item)
{
  ...
  qDebug () << "Select ANALYTE 1:: -- extcount, extMAP -- " << extcount
            << analyte->extinction;
  ...
  if (extcount == 0)
    {
      ...
      //  It uses incorrect logic for proteins in ::calc_vbar() when DNA/RNA
      //  contains Cytosin (mistakes for Cysteine)
  ...
  qDebug () << "Select ANALYTE 2:: -- extcount, extMAP -- " << extcount
            << analyte->extinction;
  ...
  if (analyte->type == US_Analyte::CARBOHYDRATE)
    ...
    // if ( analyte->type == US_Analyte::DNA || analyte->type ==
    // US_Analyte::RNA ) //ALEXEY: bug fixed
    {
      QString strand = "";
      ...
      // ALEXEY: shouldn't we reset analyte->extinction[ 280.0 ] && entire
      // analyte_extinciton to 0 / empty .clear()?;

      qDebug () << "Select ANALYTE 3:: -- extcount, extMAP -- " << extcount
                << analyte->extinction;
    }

  qDebug () << "Select ANALYTE 4:: -- extcount, extMAP -- " << extcount
            << analyte->extinction;
  qDebug () << "Analyte->type: " << analyte->type;
  te_analyte_smry->setText (analyte_smry (analyte));
  qDebug () << "Select ANALYTE 5:: -- extcount, extMAP -- " << extcount
            << analyte->extinction;
...
void
US_AnalyteMgrSelect::read_from_disk (QListWidgetItem *item)
{
...
void
US_AnalyteMgrSelect::read_from_db (QListWidgetItem *item)
{
  int row = lw_analyte_list->row (item);
  QString analyGUID = analyte_metadata[row].guid;
...
void
US_AnalyteMgrSelect::read_from_db (const QString &analyGUID)
{
...
void
US_AnalyteMgrSelect::accept_analyte (void)
{
...
void
US_AnalyteMgrSelect::init_analyte (void)
{
  DbgLv (1) << "agS: init_a";
  from_db = ((*db_or_disk) == 1);
  ...
  if ((from_db && idAna < 0) || (!from_db && aguid.isEmpty ()))
    {
          ...
          for (int ii = 0; ii < analyte_metadata.size (); ii++)
            {
              if (anaid == analyte_metadata[ii].analyteID)
                {
                  DbgLv (1) << "agS-initb:  ii" << ii << "match anaID"
                            << analyte->analyteID;
...
void
US_AnalyteMgrSelect::search (QString const &text)
{
  DbgLv (1) << "agS: search : text" << text;

  // ALEXEY: modified separator to something random which is unlikely to be
  // encountered in analyte description:
  ...
  sortdesc.sort ();
  DbgLv (1) << "agS-search:  descsize" << dsize << "sortsize"
            << sortdesc.size ();
  ...
  for (int jj = 0; jj < sortdesc.size (); jj++)
    { // build list of sorted meta data and ListWidget entries
      ...
      if (ii < 0 || ii >= dsize)
        {
          DbgLv (1) << "agS-search:  *ERROR* ii" << ii << "jj" << jj << "sdesc"
                    << sortdesc[jj].section (sep, 0, 0);
          continue;
        }
      AnalyteInfo info;
      info.index = ii;
...
bool
US_AnalyteMgrSelect::analyte_path (QString &path)
{
  QDir dir;
...
void
US_AnalyteMgrSelect::sequence (void)
{
  QString inf_text;
  QString big_line ("");
...
void
US_AnalyteMgrSelect::info_analyte (void)
{
  ...
  atype = analyte->type == (US_Analyte::PROTEIN) ? tr ("Protein") : atype;
  ...
  lines << tr ("Type:                     ") + atype;
  lines << tr ("Molecular Weight:         ") + QString::number (analyte->mw)
               + " D";
  ;
  lines << tr ("Vbar (20 deg. C):         ")
               + QString::number (analyte->vbar20);
...
void
US_AnalyteMgrSelect::spectrum (void)
{
...
void
US_AnalyteMgrSelect::delete_analyte (void)
{
#if 0
...
void
US_AnalyteMgrSelect::delete_disk (void)
{
#if 0
...
bool
US_AnalyteMgrSelect::analyte_in_use (QString &analyteGUID)
{
  bool in_use = false;
...
void
US_AnalyteMgrSelect::delete_db (void)
{
  US_Passwd pw;
  US_DB2 db (pw.getPasswd ());
  ...
  if (status == US_DB2::ANALY_IN_USE)
    {
      QMessageBox::warning (this, tr ("Analyte Not Deleted"),
...
void
US_AnalyteMgrSelect::query (void)
{
...
void
US_AnalyteMgrSelect::read_analyte (void)
{
  QString path;
  ...
  for (int ii = 0; ii < f_names.size (); ii++)
    {
      ...
      if (!a_file.open (QIODevice::ReadOnly | QIODevice::Text))
        continue;
...
void
US_AnalyteMgrSelect::read_db (void)
{
  US_Passwd pw;
  US_DB2 db (pw.getPasswd ());
  ...
  while (db.next ())
    {
      ...
      if (a_type == "Protein")
        current = US_Analyte::PROTEIN;
      else if (a_type == "DNA")
        current = US_Analyte::DNA;
      else if (a_type == "RNA")
        current = US_Analyte::RNA;
      else if (a_type == "Other")
        current = US_Analyte::CARBOHYDRATE;
      DbgLv (1) << "agS-rddb:    current atype_set" << current << atype_set;

      if (current != atype_set)
        continue;
...
void
US_AnalyteMgrSelect::connect_error (const QString &error)
{
...
void
US_AnalyteMgrSelect::set_analyte_type (int type)
{
...
QString
US_AnalyteMgrSelect::analyte_info (US_Analyte *ana)
{
...
QString
US_AnalyteMgrSelect::analyte_smry (US_Analyte *ana)
{
  ...
  QString atype = tr ("Other");
  atype = ana->type == (US_Analyte::PROTEIN) ? tr ("Protein") : atype;
  ...
  // ALEXEY: do not include absorbing residues information for non-protein
  // analytes ??
  ...
  if (ana->type == 0)
    {
      ...
      // mlines << tr( "E280:                    " ) +  QString::number(
      // analyte->extinction[ 280.0 ]); ALEXEY: bug fix for showing analyte's
      // extinciton at 280.0 when it doesn't exist
      if (analyte->extinction.contains (280.0))
        {
          mlines << tr ("E280:                    ")
                        + QString::number (analyte->extinction[280.0]);
        }
      else
...
US_AnalyteMgrNew::US_AnalyteMgrNew (int *invID, int *select_db_disk,
                                    US_Analyte *tmp_analyte,
                                    double temperature, bool signal)
    : US_Widgets ()
{
  ...
  QLabel *lb_protein_temp
      = us_label (tr ("Temperature <small>(") + DEGC + ")</small>:");
  ...
  QLabel *lb_protein_e280
      = us_label (tr ("E280 <small>(OD/(mol*cm))</small>:"));
  ...
  connect (ck_mw_only, SIGNAL (toggled (bool)), SLOT (update_mw_only (bool)));
  ...
  QLabel *lb_nucle_vbar
      = us_label (tr ("VBar<small>(cm<sup>3</sup>/g)</small>:"));
  ...
  QGridLayout *box7
      = us_checkbox (tr ("Gradient-Forming"), ck_grad_form, false);
  ...
  connect (typeButtons, SIGNAL (buttonClicked (int)), this,
           SLOT (set_analyte_type (int)));
  connect (pb_sequence, SIGNAL (clicked ()), this, SLOT (manage_sequence ()));
...
// Display a spectrum dialog for list/manage
void
US_AnalyteMgrNew::spectrum_class (void)
{
  US_NewSpectrum *w = new US_NewSpectrum ("ANALYTE", le_descrip->text (),
                                          le_protein_e280->text (), analyte);

  connect (w, SIGNAL (change_prot_e280 (void)), this,
           SLOT (change_prot_e280 (void)));
...
// Display a spectrum dialog for list/manage
void
US_AnalyteMgrNew::change_prot_e280 (void)
{
...
void
US_AnalyteMgrNew::manage_sequence (void)
{
...
void
US_AnalyteMgrNew::update_sequence (QString seq)
{
  seq = seq.toLower ().remove (QRegExp ("[\\s0-9]"));
  QString check = seq;

  if (seq == analyte->sequence)
    return;
  ...
  switch (analyte->type)
    ...
    case US_Analyte::DNA:
    case US_Analyte::RNA:
      qDebug () << "DNA/RNA extrinction BEFORE:-- "
                << analyte->extinction.count ();
      update_nucleotide ();
      qDebug () << "DNA/RNA extrinction AFTER: -- "
                << analyte->extinction.count ();
      break;
...
void
US_AnalyteMgrNew::value_changed_e280 (const QString &)
{
...
void
US_AnalyteMgrNew::value_changed (const QString &)
{
...
void
US_AnalyteMgrNew::temp_changed (const QString &text)
{
...
void
US_AnalyteMgrNew::update_stranded (bool checked)
{
  // if ( inReset ) return;
  if (checked)
...
void
US_AnalyteMgrNew::update_mw_only (bool checked)
{
  // if ( inReset ) return;
  if (checked)
    ck_stranded->setChecked (false);
  update_nucleotide ();
}

// Update Nucleotides ////////////////////
void
US_AnalyteMgrNew::update_nucleotide (bool /* value */)
{
  // if ( inReset ) return;
  update_nucleotide ();
}
...
void
US_AnalyteMgrNew::update_nucleotide (double /* value */)
{
  // if ( inReset ) return;
  update_nucleotide ();
}
...
void
US_AnalyteMgrNew::update_nucleotide (void)
{
  // if ( inReset ) return;
...
void
US_AnalyteMgrNew::parse_dna (void)
{
...
void
US_AnalyteMgrNew::init_analyte (void)
{
  DbgLv (1) << "agN: init_a";
  from_db = ((*db_or_disk) == 1);
...
void
US_AnalyteMgrNew::set_analyte_type (int type)
{
  DbgLv (1) << "agN: sAtype" << type;
#if 0
   if ( inReset ) return;
...
void
US_AnalyteMgrNew::new_description ()
{
...
void
US_AnalyteMgrNew::add_component ()
{
...
void
US_AnalyteMgrNew::remove_bcomp (QListWidgetItem *item)
{
...
void
US_AnalyteMgrNew::recalc_density (void)
{
#if 0
...
void
US_AnalyteMgrNew::recalc_viscosity (void)
{
#if 0
   int bcsize        = buffer->component.size();
   if ( bcsize < 1 )
...
void
US_AnalyteMgrNew::compressibility (void)
{
#if 0
...
void
US_AnalyteMgrNew::viscosity (void)
{
#if 0
...
// Slot for manually changed density
void
US_AnalyteMgrNew::manual_flag (bool is_on)
{
  ...
  if (analyte->type == US_Analyte::DNA || analyte->type == US_Analyte::RNA)
    {
      ...
      QStringList mw
          = le_nucle_mw->text ().split (" ", QString::SkipEmptyParts);

      if (mw.empty ())
        {
          QMessageBox::warning (
              this, tr ("Analyte Error"),
  ...
  if (idAna < 0)
    {
      ...
      QString msg = tr ("(Return Code = %1 ) ").arg (idAna) + db.lastError ();
      QMessageBox::critical (this, tr ("DB Analyte Write Error"),
...
bool
US_AnalyteMgrNew::data_ok (void)
{
  // Check to see if a sequence is entered
  if (analyte->sequence.isEmpty ())
    {
      QMessageBox question (QMessageBox::Question, tr ("Attention"),
  ...
  if (analyte->type == US_Analyte::PROTEIN && (vbar <= 0.0 || vbar > 2.0))
    {
      QMessageBox::information (
          this, tr ("Attention"),
          tr ("The vbar entry (%1) is not a reasonable value.").arg (vbar));
      return false;
    }
  ...
  if (analyte->type == US_Analyte::PROTEIN)
    {
      ...
      if (p.mw == 0.0)
        ...
        {
          QString msg
              = tr ("There is a difference between<br/>"
    ...
    : US_Widgets ()
{
  ...
  //          this,        SLOT  ( spectrum() ) );
  connect (pb_spectrum, SIGNAL (clicked ()), this, SLOT (spectrum_class ()));
  connect (le_descrip, SIGNAL (editingFinished ()), SLOT (description ()));
}

// Slot for manually changed description
void
US_AnalyteMgrEdit::description ()
{
...
void
US_AnalyteMgrEdit::spectrum_class (void)
{
  QString ifexists;
  if (analyte->extinction.isEmpty ())
    ifexists = "NEW";
  ...
  US_EditSpectrum *w = new US_EditSpectrum (
      "ANALYTE", ifexists, le_descrip->text (), "1.000", analyte);
...
void
US_AnalyteMgrEdit::change_spectrum (void)
{
  emit editAnaAccepted ();
}

void
US_AnalyteMgrEdit::accept_enable (void)
{
  pb_accept->setEnabled (!le_descrip->text ().isEmpty ());
}
...
// Initialize analyte settings, possibly after re-entry to Edit panel
void
US_AnalyteMgrEdit::init_analyte (void)
{
  DbgLv (1) << "agE: init_a";
  from_db = ((*db_or_disk) == 1);
...
// Slot to cancel edited analyte
void
US_AnalyteMgrEdit::editCanceled ()
{
  DbgLv (1) << "AnaE:SL: editCanceled()  origAnaGUID"
            << orig_analyte.analyteGUID;
...
// Slot to accept edited analyte
void
US_AnalyteMgrEdit::editAccepted ()
{

  qDebug () << "In EditSpectrum 1: analyte-extinction -- "
            << analyte->extinction;
...
void
US_AnalyteMgrSettings::sel_investigator (void)
{
...
void
US_AnalyteMgrSettings::db_changed (bool db)
{
  emit use_db (db);
...
void
US_AnalyteMgrSettings::assign_investigator (int invID)
{
  (*personID) = invID;
...
US_AnalyteGui::US_AnalyteGui (bool signal, const QString &GUID, int accessf,
                              double temper)
    : US_WidgetsDialog (0, 0), signal (signal), guid (GUID),
      temperature (temper)
{
  ...
  newTab = new US_AnalyteMgrNew (&personID, &disk_or_db, &analyte, temperature,
                                 signal);
  ...
  connect (tabWidget, SIGNAL (currentChanged (int)), this,
           SLOT (checkTab (int)));
  connect (selectTab, SIGNAL (analyteAccepted (void)), this,
           SLOT (analyteAccepted (void)));
  connect (selectTab, SIGNAL (selectionCanceled (void)), this,
           SLOT (analyteRejected (void)));
  connect (selectTab, SIGNAL (analytesAccepted (QList<US_Analyte> &)), this,
           SLOT (analytesAccepted (QList<US_Analyte> &)));
  connect (newTab, SIGNAL (newAnaAccepted (void)), this,
           SLOT (newAnaAccepted (void)));
  connect (newTab, SIGNAL (newAnaCanceled (void)), this,
           SLOT (newAnaCanceled (void)));
  connect (editTab, SIGNAL (editAnaAccepted (void)), this,
           SLOT (editAnaAccepted (void)));
  connect (editTab, SIGNAL (editAnaCanceled (void)), this,
           SLOT (editAnaCanceled (void)));
...
void
US_AnalyteGui::value_changed (const QString &)
{
  // This only is activated by changes to vbar20
  // (either protein or dna/rna) but vbar is not saved.
#if 0
...
void
US_AnalyteGui::checkTab (int currentTab)
{
...
void
US_AnalyteGui::update_disk_or_db (bool choice)
{
...
void
US_AnalyteGui::update_personID (int ID)
{
  personID = ID;
}
...
void
US_AnalyteGui::editAnaAccepted (void)
{
...
void
US_AnalyteGui::editAnaCanceled (void)
{
...
void
US_AnalyteGui::newAnaAccepted (void)
{
...
void
US_AnalyteGui::newAnaCanceled (void)
{
...
void
US_AnalyteGui::analyteAccepted (void)
{
#if 0
...
// Exit and signal caller that changes and selected were accepted
void
US_AnalyteGui::analytesAccepted (QList<US_Analyte> &analytes)
{
  DbgLv (1) << "AnaM: aaAcc:";
...
void
US_AnalyteGui::analyteRejected (void)
{
  analyte = orig_analyte;
