...
US_SequenceEditor::US_SequenceEditor(const QString &sequence) : US_WidgetsDialog(0, 0) {
...
US_AnalyteMgrSelect::US_AnalyteMgrSelect(int *invID, int *select_db_disk, US_Analyte *tmp_analyte) : US_Widgets() {
   analyte = tmp_analyte;
   personID = invID;
   ...
   lb_vbar20 = us_label(tr("VBar (<small>cm<sup>3</sup>/g at 20") + DEGC + "</small>):");
   ...
   te_analyte_smry->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize()));
   ...
   connect(typeButtons, SIGNAL(buttonClicked(int)), this, SLOT(set_analyte_type(int)));
   ...
   connect(lw_analyte_list, SIGNAL(itemSelectionChanged()), this, SLOT(select_analyte()));
...
void US_AnalyteMgrSelect::init_analyte(void) {
   ...
   if (!aguid.isEmpty()) { // There is a selected analyte, select a list item
   ...
   else { // There is no selected analyte, de-select a list item
      lw_analyte_list->setCurrentRow(-1);
...
void US_AnalyteMgrSelect::search(QString const &text) {
   ...
   for (int ii = 0; ii < dsize; ii++) { // get list of filtered-description + index strings
   ...
   sortdesc.sort();
   DbgLv(1) << "agS-search:  descsize" << dsize << "sortsize" << sortdesc.size();
   ...
   for (int jj = 0; jj < sortdesc.size(); jj++) { // build list of sorted meta data and ListWidget entries
      ...
      if (ii < 0 || ii >= dsize) {
         DbgLv(1) << "agS-search:  *ERROR* ii" << ii << "jj" << jj << "sdesc" << sortdesc[ jj ].section(sep, 0, 0);
         continue;
      }
      AnalyteInfo info;
      info.index = ii;
   ...
   if (!dir.exists(path)) {
      if (!dir.mkpath(path)) {
         QMessageBox::critical(
            this, tr("Bad Analyte Path"), tr("Could not create default directory for analytes\n") + path);
         return false;
      }
   }
...
void US_AnalyteMgrSelect::sequence(void) {
   ...
   US_Editor *ana_info = new US_Editor(US_Editor::DEFAULT, true, QString(), this);
   ...
   if (seqsmry.count() != 0) {
      ...
      QMessageBox::information(this, tr("WARNING"), tr("Analyte does not have sequence!"));
   }
}
...
void US_AnalyteMgrSelect::info_analyte(void) {
   ...
   lines << tr("Type:                     ") + atype;
   lines << tr("Molecular Weight:         ") + QString::number(analyte->mw) + " D";
   ;
   lines << tr("Vbar (20 deg. C):         ") + QString::number(analyte->vbar20);
...
void US_AnalyteMgrSelect::spectrum(void) {
   ...
   if (analyte->extinction.isEmpty()) {
      QMessageBox::information(this, tr("WARNING"), tr("Analyte does not have spectrum data!"));
   }
   else {
...
QString US_AnalyteMgrSelect::analyte_smry(US_Analyte *ana) {
   ...
   if (extknt == 0)
      ...
      lambda = ekeys[ 0 ];
      extsmry = QString::number(lambda) + " / " + QString::number(ana->extinction[ lambda ]);
   }
   ...
   mlines << tr("Molecular Weight:        ") + QString::number(ana->mw) + " D";
   ...
   for (int ii = 0; ii < mlines.count(); ii++) { // Create a single string from the lines
      amsg += mlines[ ii ] + "\n";
   }
...
// New Analyte panel
US_AnalyteMgrNew::US_AnalyteMgrNew(
   int *invID, int *select_db_disk, US_Analyte *tmp_analyte, double temperature, bool signal) : US_Widgets() {
   ...
   QLabel *lb_protein_vbar20 = us_label(tr("VBar <small>(cm<sup>3</sup>/g at 20") + DEGC + ")</small>:");
   protein_info->addWidget(lb_protein_vbar20, prow, 2);

   le_protein_vbar20 = us_lineedit("0.0000", 0, false);
   connect(le_protein_vbar20, SIGNAL(textChanged(const QString &)), SLOT(value_changed(const QString &)));
   ...
   QLabel *lb_protein_temp = us_label(tr("Temperature <small>(") + DEGC + ")</small>:");
   ...
   if (signal_tmp) {
      ...
      connect(le_protein_temp, SIGNAL(textChanged(const QString &)), SLOT(temp_changed(const QString &)));
   }

   protein_info->addWidget(le_protein_temp, prow, 1);

   QLabel *lb_protein_vbar = us_label(tr("VBar <small>(cm<sup>3</sup>/g at T ") + DEGC + ")</small>:");
   ...
   QLabel *lb_protein_e280 = us_label(tr("E280 <small>(OD/(mol*cm))</small>:"));
   protein_info->addWidget(lb_protein_e280, prow, 2);

   //le_protein_e280 = us_lineedit( "0", 0, true );
   le_protein_e280 = us_lineedit("0", 0, false);
   connect(le_protein_e280, SIGNAL(textChanged(const QString &)), SLOT(value_changed_e280(const QString &)));
   ...
   connect(ck_stranded, SIGNAL(toggled(bool)), SLOT(update_stranded(bool)));
   ...
   connect(rb_3_hydroxyl, SIGNAL(toggled(bool)), SLOT(update_nucleotide(bool)));
   ...
   connect(rb_5_hydroxyl, SIGNAL(toggled(bool)), SLOT(update_nucleotide(bool)));
   ...
   connect(ct_sodium, SIGNAL(valueChanged(double)), SLOT(update_nucleotide(double)));
   ...
   connect(ct_potassium, SIGNAL(valueChanged(double)), SLOT(update_nucleotide(double)));
   ...
   connect(ct_lithium, SIGNAL(valueChanged(double)), SLOT(update_nucleotide(double)));
   ...
   connect(ct_magnesium, SIGNAL(valueChanged(double)), SLOT(update_nucleotide(double)));
   ...
   connect(ct_calcium, SIGNAL(valueChanged(double)), SLOT(update_nucleotide(double)));
   ...
   QLabel *lb_nucle_vbar = us_label(tr("VBar<small>(cm<sup>3</sup>/g)</small>:"));
   nucle_data->addWidget(lb_nucle_vbar, 1, 0);

   le_nucle_vbar = us_lineedit("0.5500");
   connect(le_nucle_vbar, SIGNAL(textChanged(const QString &)), SLOT(value_changed(const QString &)));
   ...
   QGridLayout *box7 = us_checkbox(tr("Gradient-Forming"), ck_grad_form, false);
   ...
   QLabel *lb_carbs_vbar = us_label(tr("VBar<small>(cm<sup>3</sup>/g)</small>:"));
...
void US_AnalyteMgrNew::spectrum_class(void) {
   ...
   connect(w, SIGNAL(change_prot_e280(void)), this, SLOT(change_prot_e280(void)));
...
void US_AnalyteMgrNew::manage_sequence(void) {
   ...
   connect(edit, SIGNAL(sequenceChanged(QString)), SLOT(update_sequence(QString)));
   edit->exec();
}
...
void US_AnalyteMgrNew::update_nucleotide(void) {
   ...
   if (analyte->doubleStranded) {
      s.sprintf(" %2.5e kD (%d A, %d G, %d C, %d U, %d T, %d bp)", MW / 1000.0, A, G, C, U, T, total / 2);
   }
   else {
      s.sprintf(" %2.5e kD (%d A, %d G, %d C, %d U, %d T, %d bases)", MW / 1000.0, A, G, C, U, T, total);
   }
...
void US_AnalyteMgrNew::new_description() {
   ...
   switch (anatype) {
      case US_Analyte::PROTEIN:
         can_accept
            = (!le_descrip->text().isEmpty() && !le_protein_mw->text().isEmpty()
...
void US_AnalyteMgrNew::spectrum() {
   ...
   QMessageBox::information(this, tr("INCOMPLETE"), tr("A new Spectrum dialog is under development."));
}
...
void US_AnalyteMgrNew::newAccepted() {
   ...
   if (analyte->type == US_Analyte::DNA || analyte->type == US_Analyte::RNA) {
      ...
      if (mw.empty()) {
         QMessageBox::warning(this, tr("Analyte Error"), tr("Molecular weight is empty.  Define a sequence."));
         return;
      }
   ...
   if (from_db) { // Add analyte to database
      write_db();
      DbgLv(1) << "AnaN:SL:  newAcc: DB  id" << analyte->analyteID;
   }

   else { // Add analyte to local disk
...
void US_AnalyteMgrNew::write_db() {
   ...
   if (idAna < 0) {
      ...
      QMessageBox::critical(this, tr("DB Analyte Write Error"), tr("Error updating analyte in the database:\n") + msg);
   }
}
...
bool US_AnalyteMgrNew::data_ok(void) {
   ...
   if (analyte->sequence.isEmpty()) {
      QMessageBox question(
         QMessageBox::Question, tr("Attention"),
   ...
   if (analyte->type == US_Analyte::PROTEIN && (vbar <= 0.0 || vbar > 2.0)) {
      QMessageBox::information(this, tr("Attention"), tr("The vbar entry (%1) is not a reasonable value.").arg(vbar));
      return false;
   }
   ...
   if (mwvl <= 0.0) {
      QMessageBox::information(
         this, tr("Attention"), tr("The Molecular Weight entry (%1) is not a reasonable value.").arg(mwvl));
      return false;
   }
...
void US_AnalyteMgrNew::verify_vbar() {
   if (analyte->type == US_Analyte::PROTEIN) {
      ...
      if (p.mw == 0.0)
         ...
         QString msg = tr("There is a difference between<br/>"
...
US_AnalyteMgrEdit::US_AnalyteMgrEdit(int *invID, int *select_db_disk, US_Analyte *tmp_analyte) : US_Widgets() {
   analyte = tmp_analyte;
...
void US_AnalyteMgrEdit::editAccepted() {
   ...
   if (from_db) { // Update analyte in database
      write_db();
      DbgLv(1) << "AnaE:SL:  edtAcc: DB  id" << analyte->analyteID;
   }

   else { // Update analyte on local disk
...
// Settings panel
US_AnalyteMgrSettings::US_AnalyteMgrSettings(int *invID, int *select_db_disk) : US_Widgets() {
   personID = invID;
   db_or_disk = select_db_disk;
   from_db = ((*db_or_disk) == 1);
   dbg_level = US_Settings::us_debug();
   ...
   connect(disk_controls, SIGNAL(changed(bool)), this, SLOT(db_changed(bool)));
...
US_AnalyteGui::US_AnalyteGui(bool signal, const QString &GUID, int accessf, double temper) :
    US_WidgetsDialog(0, 0), signal(signal), guid(GUID), temperature(temper) {
   personID = US_Settings::us_inv_ID();
   analyte = US_Analyte();
   orig_analyte = analyte;
   disk_or_db = (accessf == US_Disk_DB_Controls::Default) ? US_Settings::default_data_location() : accessf;
   dbg_level = US_Settings::us_debug();
   ...
   connect(selectTab, SIGNAL(analyteAccepted(void)), this, SLOT(analyteAccepted(void)));
   connect(selectTab, SIGNAL(selectionCanceled(void)), this, SLOT(analyteRejected(void)));
   connect(selectTab, SIGNAL(analytesAccepted(QList<US_Analyte> &)), this, SLOT(analytesAccepted(QList<US_Analyte> &)));
   connect(newTab, SIGNAL(newAnaAccepted(void)), this, SLOT(newAnaAccepted(void)));
   connect(newTab, SIGNAL(newAnaCanceled(void)), this, SLOT(newAnaCanceled(void)));
   connect(editTab, SIGNAL(editAnaAccepted(void)), this, SLOT(editAnaAccepted(void)));
   connect(editTab, SIGNAL(editAnaCanceled(void)), this, SLOT(editAnaCanceled(void)));
