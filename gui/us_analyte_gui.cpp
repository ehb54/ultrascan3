...
US_AnalyteMgrSelect::US_AnalyteMgrSelect(int* invID, int* select_db_disk,
  ...
  lb_vbar20 =
      us_label(tr("VBar (<small>cm<sup>3</sup>/g at 20") + DEGC + "</small>):");
...
void US_AnalyteMgrSelect::reject(void) { emit selectionCanceled(); }
...
QString US_AnalyteMgrSelect::analyte_smry(US_Analyte* ana) {
  ...
  for (int ii = 0; ii < mlines.count();
       ii++) {  // Create a single string from the lines
    amsg += mlines[ii] + "\n";
  }
...
US_AnalyteMgrNew::US_AnalyteMgrNew(int* invID, int* select_db_disk,
                                   US_Analyte* tmp_analyte, double temperature,
                                   bool signal)
    : US_Widgets() {
  ...
  QLabel* lb_protein_vbar =
      us_label(tr("VBar <small>(cm<sup>3</sup>/g at T ") + DEGC + ")</small>:");
...
void US_AnalyteMgrNew::update_sequence(QString seq) {
  ...
  for (int i = 0; i < segments; i++) {
    ...
    if (i % lsize == 0) s += t.sprintf("%04i ", i * gsize + 1);
  ...
  switch (analyte->type) {
    case US_Analyte::PROTEIN: {
      ...
      // If spectrum is empty, set to 280.0/e280
         ...
         {
#endif
      if (analyte->extinction.count() == 0) analyte->extinction[280.0] = p.e280;
#if 0
         }
...
void US_AnalyteMgrNew::update_nucleotide(void) {
  ...
  if (analyte->doubleStranded) {
    s.sprintf(" %2.5e kD (%d A, %d G, %d C, %d U, %d T, %d bp)", MW / 1000.0, A,
              G, C, U, T, total / 2);
  } else {
    s.sprintf(" %2.5e kD (%d A, %d G, %d C, %d U, %d T, %d bases)", MW / 1000.0,
              A, G, C, U, T, total);
  }
...
void US_AnalyteMgrNew::spectrum() {
  ...
  QMessageBox::information(this, tr("INCOMPLETE"),
...
void US_AnalyteMgrNew::write_db() {
  ...
  if (idAna < 0) {
    ...
    QMessageBox::critical(
        this, tr("DB Analyte Write Error"),
...
bool US_AnalyteMgrNew::data_ok(void) {
  ...
  if (analyte->sequence.isEmpty()) {
    ...
    if (question.exec() == QMessageBox::No) return false;
  }
...
void US_AnalyteMgrNew::verify_vbar() {
  if (analyte->type == US_Analyte::PROTEIN) {
    ...
    if (p.mw == 0.0)
      ...
      if (msgBox.exec() == QMessageBox::No) mwval = p.mw;
    }
    ...
    if (qAbs(vbar20 - pvbar) > 1e-4) {
      ...
      if (msgBox.exec() == QMessageBox::No) vbar20 = pvbar;
...
void US_AnalyteMgrEdit::change_spectrum(void) { emit editAnaAccepted(); }
...
void US_AnalyteGui::update_personID(int ID) { personID = ID; }
