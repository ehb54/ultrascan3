...
US_TmstPlot::US_TmstPlot(QWidget* parent, const QString tspath)
  ...
  QGridLayout* box1 = us_checkbox(tr("X axis scans only"), ck_xscans, x_scans);
  QGridLayout* box2 = us_checkbox(tr("X start at scan 1"), ck_sstart, s_start);

  buttonsLayout->addWidget(pb_detail);
  buttonsLayout->addWidget(pb_close);

  // Lay out the right-side plots
  US_Plot* plotLayout1 = new US_Plot(data_plot1, tr("Time Data"),
                                     tr("Scan Number"), tr("Time (minutes)"));

  US_Plot* plotLayout2 =
      new US_Plot(data_plot2, tr("Time State Data:  Combined"),
                  tr("Scan Number"), tr("Time (minutes)"));
  ...
  connect(cb_pltkey, SIGNAL(currentIndexChanged(int)), this,
          SLOT(plot_kdata()));
  connect(pb_prev, SIGNAL(clicked()), this, SLOT(prev_plot()));
  connect(pb_next, SIGNAL(clicked()), this, SLOT(next_plot()));
  connect(pb_detail, SIGNAL(clicked()), this, SLOT(details()));
  connect(pb_close, SIGNAL(clicked()), this, SLOT(close()));
  connect(ck_xscans, SIGNAL(toggled(bool)), this, SLOT(plot_data()));
  connect(ck_sstart, SIGNAL(toggled(bool)), this, SLOT(plot_data()));
...
QStringList US_TmstPlot::timestamp_data_dkeys() { return dkeys; }

QVector<QVector<double>> US_TmstPlot::timestamp_data_dvals() { return dvals; }

QMap<QString, double> US_TmstPlot::timestamp_data_mins() { return dmins; }

QMap<QString, double> US_TmstPlot::timestamp_data_maxs() { return dmaxs; }

QMap<QString, double> US_TmstPlot::timestamp_data_avgs() { return davgs; }

QMap<QString, QMap<QString, double>>
US_TmstPlot::timestamp_data_avgs_stdd_first_scan() {
  ...
  for (int i = 0; i < dkeys.size(); ++i) {
    ...
    for (int jj = 0; jj < nplpts; jj++) {
      ...
      if (jj == 0) yy_first = dvals[ky][jt];
      if (jj == nplpts - 1) yy_last = dvals[ky][jt];
    ...
    for (int jj = 0; jj < nplpts; jj++) {
      ...
      sum_mean_dev_sq +=
          (dvals[ky][jt] - y_avg_val) * (dvals[ky][jt] - y_avg_val);
    }
...
void US_TmstPlot::plot_data() {
  if (ck_xscans->isChecked()) {  // For scans-only, disable scan1-start checkbox
                                 // and set checked
  ...
  } else {  // If not scans-only, insure scan1-start checkbox is enabled
...
void US_TmstPlot::plot_kdata() {
  ...
  // qDebug() << "Plot for: " << pkey  << "; Last value of Y: "  << yy[ nplpts -
  // 1 ]; qDebug() << "Plot for: " << pkey  << "; # of points: "      << nplpts;
  // qDebug() << "Plot for: " << pkey  << "; Average: "          << y_avg_val;

  DbgLv(1) << "TP:plkd   smin smax" << smin << smax << "xkey" << xkey << "kx ks"
           << kx << ks;
  data_plot1->setAxisScale(QwtPlot::xBottom, smin, smax);

  if (ymin < ymax) {  // Auto-scale where Y is normal range type
    data_plot1->setAxisAutoScale(QwtPlot::yLeft);
  }

  else {  // Add min,max padding where Y is constant
...
void US_TmstPlot::plot_cdata() {
  ...
  for (int jk = 0; jk < npkeys; jk++) {
    ...
    for (int jj = 0; jj < nplpts; jj++) {  // Get data Y min,max
      int jt = pntxs[jj];
      double yval = dvals[ky][jt];
      ymin = qMin(ymin, yval);
      ymax = qMax(ymax, yval);
    }
    DbgLv(1) << "TP:plcd:   jk ky" << jk << ky << "color" << ccolr << "key"
             << pkey << "ymin ymax" << ymin << ymax;

    if (ymin == ymax) {  // Adjust the range when Y is constant
      ymin = (ymin > 0.0) ? (ymin * 0.95) : (ymin * 1.05);
      ymax = (ymax > 0.0) ? (ymax * 1.05) : (ymax * 0.95);
    }
...
void US_TmstPlot::details() {
  ...
  for (int jk = 0; jk < ndkeys; jk++) {  // Compose a brief summary for each key
  ...
  for (int js = 0; js < nstep; js++) {
    ...
    DbgLv(1) << "TP: det: step speed" << stepnum << ispeed
             << "scan_0 time_0 scan_n time_n" << stsscans[js] << ststimes[js]
             << stescans[js] << stetimes[js];
  }
...
void US_TmstPlot::read_data() {
  ...
  for (int jt = 0; jt < ntimes;
       jt++) {  // Build data vectors for each time position
    tsobj.read_record();

    for (int jk = 0; jk < nfkeys; jk++) {  // Set the value for each key
      QString fkey = fkeys[jk];
      QString ffmt = ffmts[jk];
      // Get value as double, regardless of file format
      double fval = ffmt.contains("F") ? tsobj.time_dvalue(fkey)
                                       : (double)tsobj.time_ivalue(fkey);
      // Convert any time value from seconds to minutes
      fval *= (fkey == "Time" ? tscl : 1.0);
      // DbgLv(1) << "TP:rdda:  jt jk" << jt << jk << "fkey" << fkey << "ffmt"
      // << ffmt
      //  << "fval" << fval;
  ...
  nscans = scnxs.count();
  DbgLv(1) << "TP:rdda: tmdx" << tmdx << "dtdx" << dtdx << "dvals[tmdx]size"
           << dvals[tmdx].size() << "nscans" << nscans;

  // Build the vector of TimeInterval-between-scans values
  int jt1 = scnxs[0];
  int jt2 = scnxs[1];
  double delta_t = (dvals[tmdx][jt2] - dvals[tmdx][jt1]);
  for (int jj = 0; jj < jt1; jj++) dvals[dtdx] << delta_t;
  ...
  DbgLv(1) << "TP:rdda:  dvals[dtdx]size" << dvals[dtdx].size() << "ntimes"
           << ntimes;
  ...
  for (int jk = 0; jk < ndkeys; jk++) {
    ...
    for (int jt = 0; jt < ntimes; jt++) {  // Accumulate min, max, and sum
    ...
    DbgLv(1) << "TP:rdda:  key" << dkey << "min,max,avg" << dmin << dmax
             << davg;
  }
  //*DEBUG*
  for (int jk = 0; jk < ndkeys; jk++) {
    QString dkey = dkeys[jk];
    DbgLv(1) << "TP:rdda:    key" << dkey << "   min,max,avg" << dmins[dkey]
             << dmaxs[dkey] << davgs[dkey];
  }
  //*DEBUG*
  ...
  for (int jk = 0; jk < ndkeys; jk++) {
    ...
    if (dkey == "Scan" || dkey == "TimeInterval") continue;
...
QColor US_TmstPlot::curve_color(const int keyx) {
  const QColor ccolors[] = {
      // Curve colors that differ by key
      QColor(255, 0, 0),   QColor(0, 255, 0),    QColor(0, 0, 255),
      QColor(255, 255, 0), QColor(255, 0, 255),  QColor(0, 255, 255),
      QColor(122, 0, 255), QColor(0, 255, 122),  QColor(0, 122, 255),
      QColor(255, 122, 0), QColor(122, 255, 0),  QColor(80, 0, 255),
      QColor(0, 255, 80),  QColor(0, 80, 255),   QColor(255, 80, 0),
      QColor(80, 255, 0),  QColor(40, 255, 40),  QColor(40, 40, 255),
      QColor(80, 80, 80),  QColor(122, 122, 122)};
...
int US_TmstPlot::point_indexes() {
  ...
  if (x_scans)
    ...
    for (int jj = scnxs[0]; jj < ntimes; jj++) pntxs << jj;
  } else {
    for (int jj = 0; jj < ntimes; jj++) pntxs << jj;
  }
