    ...
    : US_WidgetsDialog (parent, 0)
{
  ...
  QGridLayout *box1
      = us_checkbox (tr ("X axis scans only"), ck_xscans, x_scans);
  QGridLayout *box2
      = us_checkbox (tr ("X start at scan 1"), ck_sstart, s_start);

  buttonsLayout->addWidget (pb_detail);
  buttonsLayout->addWidget (pb_close);

  // Lay out the right-side plots
  US_Plot *plotLayout1 = new US_Plot (
      data_plot1, tr ("Time Data"), tr ("Scan Number"), tr ("Time (minutes)"));
  ...
  resize (p2size);
  adjustSize ();
  DbgLv (1) << "TP:mn:   resized" << size () << "lw size"
            << lw_datinfo->size ();
}

// public funcitons to access GMP-related parms
QStringList
US_TmstPlot::timestamp_data_dkeys ()
{
  return dkeys;
}

QVector<QVector<double> >
US_TmstPlot::timestamp_data_dvals ()
{
  return dvals;
}

QMap<QString, double>
US_TmstPlot::timestamp_data_mins ()
{
  return dmins;
}

QMap<QString, double>
US_TmstPlot::timestamp_data_maxs ()
{
  return dmaxs;
}

QMap<QString, double>
US_TmstPlot::timestamp_data_avgs ()
{
  return davgs;
}
      ...
      if (!ck_sstart->isChecked ())
        {
          ...
          connect (ck_sstart, SIGNAL (toggled (bool)), this,
                   SLOT (plot_data ()));
        }
    }
  else
...
US_TmstPlot::plot_kdata ()
{
  ...
  // - 1 ]; qDebug() << "Plot for: " << pkey  << "; # of points: "      <<
  // nplpts; qDebug() << "Plot for: " << pkey  << "; Average: "          <<
  // y_avg_val;

  DbgLv (1) << "TP:plkd   smin smax" << smin << smax << "xkey" << xkey
            << "kx ks" << kx << ks;
...
QwtPlot *
US_TmstPlot::rp_data_plot1 (QString p_type)
{
  ...
  for (int jk = 0; jk < npkeys; jk++)
    {
      ...
      DbgLv (1) << "TP:plcd:   " << pkey << ccolr << "  ymin ymax" << ymin
                << ymax << "yoff" << yoff << "yscl" << yscl;
  ...
  for (int js = 0; js < nstep; js++)
    {
      ...
      DbgLv (1) << "TP: det: step speed" << stepnum << ispeed
                << "scan_0 time_0 scan_n time_n" << stsscans[js]
...
US_TmstPlot::read_data ()
{
  ...
  if (tsobj.open_read_data (tmstpath) != 0)
    {
      DbgLv (1) << "TP:rdda: *ERROR* openread :"
                << tsobj.last_error_message ();
      ndkeys = 0;
      return;
    }
    ...
    { // Build data vectors for each time position
      ...
      for (int jk = 0; jk < nfkeys; jk++)
        { // Set the value for each key
          ...
          // DbgLv(1) << "TP:rdda:  jt jk" << jt << jk << "fkey" << fkey <<
          // "ffmt" << ffmt
          //  << "fval" << fval;
...
QColor
US_TmstPlot::curve_color (const int keyx)
{
