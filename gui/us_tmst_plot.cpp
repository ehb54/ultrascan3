...
US_TmstPlot::US_TmstPlot(QWidget *parent, const QString tspath) : US_WidgetsDialog(parent, 0) {
   ...
   US_Plot *plotLayout1 = new US_Plot(data_plot1, tr("Time Data"), tr("Scan Number"), tr("Time (minutes)"));

   US_Plot *plotLayout2
      = new US_Plot(data_plot2, tr("Time State Data:  Combined"), tr("Scan Number"), tr("Time (minutes)"));
   ...
   connect(cb_pltkey, SIGNAL(currentIndexChanged(int)), this, SLOT(plot_kdata()));
...
void US_TmstPlot::plot_data() {
   if (ck_xscans->isChecked()) { // For scans-only, disable scan1-start checkbox and set checked
...
void US_TmstPlot::plot_cdata() {
   ...
   DbgLv(1) << "TP:plcd:  ndkeys npkeys npltps" << ndkeys << npkeys << nplpts << "smin smax" << smin << smax;
   ...
   for (int jk = 0; jk < npkeys; jk++) {
      ...
      DbgLv(1) << "TP:plcd:   jk ky" << jk << ky << "color" << ccolr << "key" << pkey << "ymin ymax" << ymin << ymax;
      ...
      DbgLv(1) << "TP:plcd:   " << pkey << ccolr << "  ymin ymax" << ymin << ymax << "yoff" << yoff << "yscl" << yscl;
...
void US_TmstPlot::details() {
   ...
   DbgLv(1) << "TP: det: lostep histep nstep" << lostep << histep << nstep << "tmdx scdx stpx" << tmdx << scdx << stpx;
   ...
   for (int js = 0; js < nstep; js++) {
      ...
      dtext += QString().sprintf(
         "%5d   %5d %5d to %5d  %9.3f to %9.3f\n", stepnum, ispeed, stsscans[ js ], stescans[ js ], ststimes[ js ],
         stetimes[ js ]);
      DbgLv(1) << "TP: det: step speed" << stepnum << ispeed << "scan_0 time_0 scan_n time_n" << stsscans[ js ]
               << ststimes[ js ] << stescans[ js ] << stetimes[ js ];
   }
...
void US_TmstPlot::read_data() {
   ...
   for (int jt = 0; jt < ntimes; jt++) { // Build data vectors for each time position
      tsobj.read_record();
      ...
      for (int jk = 0; jk < nfkeys; jk++) { // Set the value for each key
         ...
         double fval = ffmt.contains("F") ? tsobj.time_dvalue(fkey) : ( double ) tsobj.time_ivalue(fkey);
   ...
   nscans = scnxs.count();
   DbgLv(1) << "TP:rdda: tmdx" << tmdx << "dtdx" << dtdx << "dvals[tmdx]size" << dvals[ tmdx ].size() << "nscans"
            << nscans;
   ...
   for (int jk = 0; jk < ndkeys; jk++) {
      ...
      for (int jt = 0; jt < ntimes; jt++) { // Accumulate min, max, and sum
         ...
         if (jk == 9 && dval < 0.0) {
            int j2 = qMax(1, jt);
            int j1 = jt - 1;
            DbgLv(1) << "TP:rdda:      DT" << dval << "jt" << jt << "T2,T1" << dvals[ 1 ][ j2 ] << dvals[ 1 ][ j1 ]
                     << "   +++++++";
         }
      }
   ...
   for (int jk = 0; jk < ndkeys; jk++) {
      QString dkey = dkeys[ jk ];
      DbgLv(1) << "TP:rdda:    key" << dkey << "   min,max,avg" << dmins[ dkey ] << dmaxs[ dkey ] << davgs[ dkey ];
   }
   //*DEBUG*
   ...
   for (int jk = 0; jk < ndkeys; jk++) {
      ...
      infotxt << tr("  %1 Range:  %2 to %3").arg(dkey).arg(dmins[ dkey ]).arg(dmaxs[ dkey ]);
   }
...
QColor US_TmstPlot::curve_color(const int keyx) {
   const QColor ccolors[] = { // Curve colors that differ by key
                              ...
                              QColor(255, 0, 255), QColor(0, 255, 255), QColor(122, 0, 255), QColor(0, 255, 122),
                              QColor(0, 122, 255), QColor(255, 122, 0), QColor(122, 255, 0), QColor(80, 0, 255),
                              QColor(0, 255, 80),  QColor(0, 80, 255),  QColor(255, 80, 0),  QColor(80, 255, 0),
                              QColor(40, 255, 40), QColor(40, 40, 255), QColor(80, 80, 80),  QColor(122, 122, 122)
   };
