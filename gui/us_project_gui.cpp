...
void US_ProjectGui::cancel(void) {
  if (signal) emit cancelProjectGuiSelection();
...
void US_ProjectGui::newProject(void) {
  ...
  goalsTab->setGoals(
      tr("Please replace with an introduction to your "
...
void US_ProjectGuiGeneral::reset(void) {
  ...
  if (*investigatorID == 0) *investigatorID = US_Settings::us_inv_ID();
...
QString US_ProjectGuiGeneral::getDesc(void) { return (le_projectDesc->text()); }
...
QString US_ProjectGuiGoals::getGoals(void) { return (te_goals->toPlainText()); }
...
QString US_ProjectGuiPurity::getPurity(void) { return (le_purity->text()); }
...
US_ProjectGuiExpense::US_ProjectGuiExpense(void) : US_Widgets() {
  ...
  QLabel* lb_expense =
      us_label(tr("How much material (volume, concentration) is"
                  ...
                  " absorbance optics,\n"
                  "provide an absorbance scan against buffer from 215-700 nm "
                  "for each sample:"));
...
void US_ProjectGuiBufferComponents::setBufferComponents(
...
void US_ProjectGuiSaltInformation::setSaltInformation(
    QString newSaltInformation) {
...
US_ProjectGuiNotes::US_ProjectGuiNotes(void) : US_Widgets() {
  ...
  QLabel* lb_notes =
      us_label(tr("Special instructions, questions, and notes (optional):"));
...
QString US_ProjectGuiNotes::getNotes(void) { return (te_notes->toPlainText()); }
