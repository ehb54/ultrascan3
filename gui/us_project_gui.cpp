...
US_ProjectGui::US_ProjectGui(bool signal_wanted, int select_db_disk, const US_Project &dataIn) :
   ...
   // Global signals
   connect(generalTab, SIGNAL(source_changed(bool)), SLOT(source_changed(bool)));

   connect(generalTab, SIGNAL(newProject()), SLOT(newProject()));

   connect(generalTab, SIGNAL(load()), SLOT(load()));

   connect(generalTab, SIGNAL(selectProject(QListWidgetItem *)), SLOT(selectProject(QListWidgetItem *)));

   connect(generalTab, SIGNAL(saveDescription(const QString &)), SLOT(saveDescription(const QString &)));
   ...
   connect(moleculesTab, SIGNAL(moleculesTabChanged()), SLOT(tabTextChanged()));

   connect(purityTab, SIGNAL(purityTabChanged(const QString &)), SLOT(tabTextChanged(const QString &)));

   connect(expenseTab, SIGNAL(expenseTabChanged()), SLOT(tabTextChanged()));

   connect(bufferComponentsTab, SIGNAL(bufferComponentsTabChanged()), SLOT(tabTextChanged()));

   connect(saltInformationTab, SIGNAL(saltInformationTabChanged()), SLOT(tabTextChanged()));

   connect(auc_questionsTab, SIGNAL(AUC_questionsTabChanged()), SLOT(tabTextChanged()));

   connect(expDesignTab, SIGNAL(expDesignTabChanged()), SLOT(tabTextChanged()));
   ...
   if (project.projectID > 0) {
      QList<QListWidgetItem *> items = generalTab->lw_projects->findItems(project.projectDesc, Qt::MatchExactly);
...
void US_ProjectGui::saveProject(void) {
   ...
   if (generalTab->disk_controls->db()) {
      ...
      if (status == US_DB2::NO_PROJECT) {
         QMessageBox::information(this, tr("Attention"), tr("There was a problem saving the project to the database.\n"));
         return;
      }
...
void US_ProjectGui::deleteProject(void) {
   ...
   QMessageBox::information(this, tr("Delete results"), tr("Project Deleted"));
}
...
US_ProjectGuiGeneral::US_ProjectGuiGeneral(int *invID, int select_db_disk) : US_Widgets() {
   ...
   connect(disk_controls, SIGNAL(changed(bool)), SIGNAL(source_changed(bool)));
   general->addLayout(disk_controls, row++, 1, 1, 2);

   // Row 3
   lw_projects = us_listwidget();
   lw_projects->setSortingEnabled(true);
   connect(lw_projects, SIGNAL(itemClicked(QListWidgetItem *)), SIGNAL(selectProject(QListWidgetItem *)));
   ...
   connect(le_projectDesc, SIGNAL(textEdited(const QString &)), SIGNAL(saveDescription(const QString &)));
...
void US_ProjectGuiGeneral::reset(void) {
   ...
   QString number = (*investigatorID > 0) ? QString::number(*investigatorID) + ": " : "";
...
US_ProjectGuiMolecules::US_ProjectGuiMolecules(void) : US_Widgets() {
   ...
   QLabel *lb_molecules = us_label(tr("For each analyte to be measured,"
   ...
   connect(te_molecules, SIGNAL(textChanged()), SIGNAL(moleculesTabChanged()));
...
US_ProjectGuiPurity::US_ProjectGuiPurity(void) : US_Widgets() {
   ...
   QLabel *lb_purity = us_label(tr("Please indicate the approximate purity of your sample(s).\n"
                                   "You can express it in percent:"));

   purity->addWidget(lb_purity);

   le_purity = us_lineedit();
   connect(le_purity, SIGNAL(textChanged(const QString &)), SIGNAL(purityTabChanged(const QString &)));
...
US_ProjectGuiExpense::US_ProjectGuiExpense(void) : US_Widgets() {
   ...
   QLabel *lb_expense = us_label(tr("How much material (volume, concentration) is"
...
US_ProjectGuiBufferComponents::US_ProjectGuiBufferComponents(void) : US_Widgets() {
   ...
   QLabel *lb_bufferComponents = us_label(tr("Please list the molar concentrations of each component"
                                             " in your buffer,\n"
   ...
   connect(te_bufferComponents, SIGNAL(textChanged()), SIGNAL(bufferComponentsTabChanged()));
...
US_ProjectGuiSaltInformation::US_ProjectGuiSaltInformation(void) : US_Widgets() {
   ...
   QLabel *lb_saltInformation = us_label(tr("Please indicate the ideal storage conditions"
                                            " (room temperature,\n"
   ...
   connect(te_saltInformation, SIGNAL(textChanged()), SIGNAL(saltInformationTabChanged()));
...
US_ProjectGuiAUC_questions::US_ProjectGuiAUC_questions(void) : US_Widgets() {
   ...
   QLabel *lb_auc_questions = us_label(tr("What questions are you trying to answer with AUC?\n"
   ...
   connect(te_auc_questions, SIGNAL(textChanged()), SIGNAL(AUC_questionsTabChanged()));
...
US_ProjectGuiExpDesign::US_ProjectGuiExpDesign(void) : US_Widgets() {
   ...
   connect(te_exp_design, SIGNAL(textChanged()), SIGNAL(expDesignTabChanged()));
