...
US_ProjectGui::US_ProjectGui (bool signal_wanted, int select_db_disk,
    ...
    : US_WidgetsDialog (0, 0), project (dataIn)
{
  ...
  connect (generalTab, SIGNAL (newProject ()), SLOT (newProject ()));

  connect (generalTab, SIGNAL (load ()), SLOT (load ()));
  ...
  connect (generalTab, SIGNAL (save ()), SLOT (saveProject ()));

  connect (generalTab, SIGNAL (deleteProject ()), SLOT (deleteProject ()));

  connect (goalsTab, SIGNAL (goalsTabChanged ()), SLOT (tabTextChanged ()));
  ...
  connect (notesTab, SIGNAL (notesTabChanged ()), SLOT (tabTextChanged ()));
  ...
  if (project.projectID > 0)
    {
      QList<QListWidgetItem *> items = generalTab->lw_projects->findItems (
          project.projectDesc, Qt::MatchExactly);
...
void
US_ProjectGui::enableButtons (void)
{
  ...
  else if (!generalTab->disk_controls->db ()
...
US_ProjectGui::newProject (void)
{
  ...
  goalsTab->setGoals (tr ("Please replace with an introduction to your "
                          ...
                          "of this research."));
  moleculesTab->setMolecules (
      tr ("Please include their approximate molecular weights"));
  purityTab->setPurity (tr ("%"));
  expenseTab->setExpense (
      tr ("Would this expense be acceptable? If not, what amount "
          "would you feel comfortable with?"));
  bufferComponentsTab->setBufferComponents (
      tr ("What buffers do you plan to use? Is phosphate or "
  ...
  saltInformationTab->setSaltInformation (
      tr ("If not, please explain why not."));
  auc_questionsTab->setAUC_questions (
      tr ("How do you propose to approach the research with "
          "AUC experiments?"));
  expDesignTab->setExpDesign (
      tr ("Please enter any notes about the experiment design."));
  notesTab->setNotes (
      tr ("Special instructions, questions, and notes (optional)"));
...
void
US_ProjectGui::loadDisk (void)
{
  QString path;
  if (!project.diskPath (path))
    return;
...
void
US_ProjectGui::loadProjects (void)
{
  generalTab->lw_projects->clear ();
  info.clear ();
  projectMap.clear ();
  ...
  for (int i = 0; i < descriptions.size (); i++)
    {
      ...
      QListWidgetItem *item
          = new QListWidgetItem (descriptions[i], generalTab->lw_projects);
      projectMap[item] = i;
...
void
US_ProjectGui::selectProject (QListWidgetItem *item)
{
...
void
US_ProjectGui::saveDescription (const QString &)
{
...
void
US_ProjectGui::saveProject (void)
{
  ...
  if (generalTab->disk_controls->db ())
    {
      ...
      if (status == US_DB2::NO_PROJECT)
        {
          QMessageBox::information (this, tr ("Attention"),
                                    tr ("There was a problem saving the "
  ...
  QMessageBox::information (this, tr ("Save results"), tr ("Project saved"));
...
void
US_ProjectGui::deleteProject (void)
{
  if (generalTab->disk_controls->db ())
    {
      US_Passwd pw;
...
void
US_ProjectGui::source_changed (bool db)
{
  QStringList DB = US_Settings::defaultDB ();
...
void
US_ProjectGui::tabTextChanged (const QString &)
{
  text_changed = true;
  generalTab->pb_save->setEnabled (true);
}
...
void
US_ProjectGui::db_error (const QString &error)
{
...
US_ProjectGuiGeneral::US_ProjectGuiGeneral (int *invID, int select_db_disk)
    : US_Widgets ()
{
  ...
  if (US_Settings::us_inv_level () > 2)
    {
      QPushButton *pb_investigator
          = us_pushbutton (tr ("Select Investigator"));
...
void
US_ProjectGuiGeneral::reset (void)
{
  ...
  QString number
      = (*investigatorID > 0) ? QString::number (*investigatorID) + ": " : "";
...
void
US_ProjectGuiGeneral::sel_investigator (void)
{
...
void
US_ProjectGuiGeneral::assign_investigator (int invID)
{
  *investigatorID = invID;
...
void
US_ProjectGuiGeneral::setGUID (QString newGUID)
{
  le_guid->setText (newGUID);
}

void
US_ProjectGuiGeneral::setLastUpd (QString lastupd)
{
  le_lastupd->setText (lastupd);
}

void
US_ProjectGuiGeneral::setDesc (QString newDesc)
{
  le_projectDesc->setText (newDesc);
}

QString
US_ProjectGuiGeneral::getDesc (void)
{
...
US_ProjectGuiGoals::US_ProjectGuiGoals (void) : US_Widgets ()
{
  ...
  QLabel *lb_goals = us_label (tr ("Please provide a detailed description"
                                   " of your research.\n"
  ...
  connect (te_goals, SIGNAL (textChanged ()), SIGNAL (goalsTabChanged ()));
...
void
US_ProjectGuiGoals::setGoals (QString newGoals)
{
  te_goals->setText (newGoals);
}

QString
US_ProjectGuiGoals::getGoals (void)
{
...
US_ProjectGuiMolecules::US_ProjectGuiMolecules (void) : US_Widgets ()
{
  ...
  QLabel *lb_molecules = us_label (
      tr ("For each analyte to be measured,"
...
void
US_ProjectGuiMolecules::setMolecules (QString newMolecules)
{
  te_molecules->setText (newMolecules);
}

QString
US_ProjectGuiMolecules::getMolecules (void)
{
...
US_ProjectGuiPurity::US_ProjectGuiPurity (void) : US_Widgets ()
{
  ...
  QLabel *lb_purity = us_label (
      tr ("Please indicate the approximate purity of your sample(s).\n"
...
void
US_ProjectGuiPurity::setPurity (QString newPurity)
{
  le_purity->setText (newPurity);
}

QString
US_ProjectGuiPurity::getPurity (void)
{
...
US_ProjectGuiExpense::US_ProjectGuiExpense (void) : US_Widgets ()
{
  ...
  QLabel *lb_expense = us_label (
      tr ("How much material (volume, concentration) is"
          ...
          " absorbance optics,\n"
          "provide an absorbance scan against buffer from 215-700 nm for each "
          "sample:"));

  expense->addWidget (lb_expense);

  te_expense = us_textedit ();
  connect (te_expense, SIGNAL (textChanged ()), SIGNAL (expenseTabChanged ()));
...
void
US_ProjectGuiExpense::setExpense (QString newExpense)
{
  te_expense->setText (newExpense);
}

QString
US_ProjectGuiExpense::getExpense (void)
{
  return (te_expense->toPlainText ());
}

US_ProjectGuiBufferComponents::US_ProjectGuiBufferComponents (void)
    : US_Widgets ()
{
  QVBoxLayout *bufferComponents = new QVBoxLayout;

  QLabel *lb_bufferComponents = us_label (
      tr ("Please list the molar concentrations of each component"
          " in your buffer,\n"
...
void
US_ProjectGuiBufferComponents::setBufferComponents (
    QString newBufferComponents)
{
  te_bufferComponents->setText (newBufferComponents);
}

QString
US_ProjectGuiBufferComponents::getBufferComponents (void)
{
  return (te_bufferComponents->toPlainText ());
}

US_ProjectGuiSaltInformation::US_ProjectGuiSaltInformation (void)
    : US_Widgets ()
{
  QVBoxLayout *saltInformation = new QVBoxLayout;

  QLabel *lb_saltInformation = us_label (
      tr ("Please indicate the ideal storage conditions"
          " (room temperature,\n"
...
void
US_ProjectGuiSaltInformation::setSaltInformation (QString newSaltInformation)
{
  te_saltInformation->setText (newSaltInformation);
}

QString
US_ProjectGuiSaltInformation::getSaltInformation (void)
{
...
void
US_ProjectGuiAUC_questions::setAUC_questions (QString newAUC_questions)
{
  te_auc_questions->setText (newAUC_questions);
}

QString
US_ProjectGuiAUC_questions::getAUC_questions (void)
{
...
US_ProjectGuiExpDesign::US_ProjectGuiExpDesign (void) : US_Widgets ()
{
  ...
  QLabel *lb_exp_design = us_label (tr ("The AUC experimental design:"));
...
void
US_ProjectGuiExpDesign::setExpDesign (QString newExpDesign)
{
  te_exp_design->setText (newExpDesign);
}

QString
US_ProjectGuiExpDesign::getExpDesign (void)
{
...
US_ProjectGuiNotes::US_ProjectGuiNotes (void) : US_Widgets ()
{
  ...
  QLabel *lb_notes = us_label (
      tr ("Special instructions, questions, and notes (optional):"));
  notes->addWidget (lb_notes);

  te_notes = us_textedit ();
  connect (te_notes, SIGNAL (textChanged ()), SIGNAL (notesTabChanged ()));
...
void
US_ProjectGuiNotes::setNotes (QString newNotes)
{
  te_notes->setText (newNotes);
}

QString
US_ProjectGuiNotes::getNotes (void)
{
