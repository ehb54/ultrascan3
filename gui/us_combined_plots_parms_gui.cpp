...
US_CombPlotsGui::US_CombPlotsGui (QString combPlotsMask,
    ...
    : US_Widgets ()
{
  ...
  QLabel *bn_excl_scans = us_banner (
      QString (tr ("Define Combined Plots' Global Parameters: ")), 1);
...
void
US_CombPlotsGui::parse_json (void)
{
  combPlots_map.clear ();
      ...
      for (int i = 0; i < json_array.size (); ++i)
        {
          foreach (const QString &array_key, json_array[i].toObject ().keys ())
            {
              combPlots_typeMethod_map[array_key]
  ...
  foreach (const QString &key, json.keys ())
    {
      ...
      for (top = c_map.begin (); top != c_map.end (); ++top)
        {
          qDebug () << "Type, Method: Param, Value -- " << key << top.key ()
                    << top.value ();
        }
    }
  // end of debug
...
// build Layout
void
US_CombPlotsGui::build_layout (void)
{
  ...
  foreach (const QString &key, json.keys ())
    {
      ...
      // hide (or disable ?) rows for which type-method is not in the
      // type_method_list:
...
// gui to parms
void
US_CombPlotsGui::gui_to_parms (void)
{
  ...
  foreach (const QString &key, json.keys ())
    {
      ...
      // sigma
      QLineEdit *le_sig
          = containerWidget->findChild<QLineEdit *> (stmeth + "sigma");

      // xmin
      QLineEdit *le_min
          = containerWidget->findChild<QLineEdit *> (stmeth + "min");

      // xmax
      QLineEdit *le_max
          = containerWidget->findChild<QLineEdit *> (stmeth + "max");
      ...
      for (top = c_map.begin (); top != c_map.end (); ++top)
        {
          if (top.key ().contains ("Sigma"))
            {
              qDebug () << "Sigma for type,method -- " << key
                        << le_sig->text ();
          ...
          if (top.key ().contains ("Minimum"))
            {
              qDebug () << "X Min for type,method -- " << key
                        << le_min->text ();
          ...
          if (top.key ().contains ("Maximum"))
            {
              qDebug () << "X Max for type,method -- " << key
                        << le_max->text ();
  ...
  foreach (const QString &key, json.keys ())
    {
      ...
      for (top = c_map.begin (); top != c_map.end (); ++top)
        {
          qDebug () << "In Gui-to-parm: Type, Method: Param, Value -- " << key
                    << top.key () << top.value ();
        }
    }
  // end of debug
...
// cancel
void
US_CombPlotsGui::cancel_update (void)
{
  close ();
}
...
// update
void
US_CombPlotsGui::update_parms (void)
{
  gui_to_parms ();
  close ();
}
...
void
US_CombPlotsGui::applied_to_all (void)
{
}
