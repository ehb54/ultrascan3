...
US_Extinction::US_Extinction (QString buffer, const QString &text,
                              const QString &text_e280, QWidget *parent)
    : US_Widgets (parent, 0)
{
  ...
  // connect(this, SIGNAL( get_results(QMap < double, double > & )), parent,
  ...
  pb_perform_buffer = us_pushbutton (
      tr ("Perform Buffer Fit")); // New button for 'Fit Buffer...'
  // if (buffer_temp.toStdString() == "ANALYTE")
  pb_perform_analyte = us_pushbutton (
      tr ("Perform Analyte Fit")); // New button for 'Fit Analyte...'

  pb_perform_solution = us_pushbutton (
      tr ("Perform Solution Fit")); // New button for 'Fit Solution...'

  connect (pb_perform_buffer, SIGNAL (clicked ()),
           SLOT (perform_global_buffer ()));
  connect (pb_perform_analyte, SIGNAL (clicked ()),
           SLOT (perform_global_analyte ()));
  connect (pb_perform_solution, SIGNAL (clicked ()),
           SLOT (perform_global_solution ()));
  if (buffer_temp.toStdString () == "BUFFER")
    {
  ...
  connect (lw_file_names, SIGNAL (itemSelectionChanged ()),
           SLOT (listToCurve ()));
  connect (lw_file_names, SIGNAL (itemDoubleClicked (QListWidgetItem *)),
           SLOT (deleteCurve ()));
  ...
  connect (ct_gaussian, SIGNAL (valueChanged (double)),
           SLOT (update_order (double)));
  ...
  changedCurve = NULL;
  plotLayout = new US_Plot (data_plot, tr (""), tr ("Wavelength(nm)"),
                            tr ("Optical Density"));
  ...
  /*  if (buffer_temp.toStdString() == "BUFFER")
    {
      pb_perform->hide();              // Tell what to show/hide, or edit
    Layout pb_perform_buffer->show();
...
//   dialog.setNameFilter(tr("Text files (*.[Tt][Xx][Tt] *.[Cc][Ss][Vv]
//   *.[Dd][Aa][Tt] *.[Ww][Aa]* *.[Dd][Ss][Pp]);;All files (*)"));
...
bool
US_Extinction::isComment (const QString &str)
{
...
bool
US_Extinction::loadScan (US_CSV_Data &csv_data)
{
  ...
  if (lw_file_names->count () > 0)
    {
      QString text
          = lw_file_names->item (lw_file_names->count () - 1)->text ();
      re.setPattern ("^[(](\\d+)[)].+");
      match = re.match (text);
      if (match.hasMatch ())
        {
      ...
      for (int jj = 0; jj < nrows; jj++)
        {
          ...
          if (xt >= lambdaLimitLeft && yt <= odCutoff
              && xt <= lambdaLimitRight)
            {
              Reading r = { xt, yt };
  ...
  qDebug () << "Size of the Wvs_to_descr_map: " << Wvs_to_descr_map.keys ()
            << Wvs_to_descr_map.keys ().size ();
  ...
  for (mm = Wvs_to_descr_map.begin (); mm != Wvs_to_descr_map.end (); ++mm)
    {
      ...
      if (wls.v_readings.at (0).lambda
          > wls.v_readings.at (wls.v_readings.size () - 1).lambda)
        { // we need to reverse the order of entries
          WavelengthScan wls2;
          wls2.v_readings.clear ();
          qDebug () << "Inside LAOD 1aa, wls.v_readings.size(), "
                    << wls.v_readings.size () - (unsigned int)1;
          for (int i = (wls.v_readings.size () - (unsigned int)1); i >= 0; i--)
            {
              qDebug () << "i: " << i << wls.v_readings.at (i).lambda << ", "
                        << wls.v_readings.at (i).od;
              Reading temp
                  = { wls.v_readings.at (i).lambda, wls.v_readings.at (i).od };
              wls2.v_readings.push_back (temp);
            }
          qDebug () << "Inside LAOD 1bb, wls2.v_readings.size(), "
                    << (unsigned int)wls2.v_readings.size ();
          wls.v_readings.clear ();

          for (unsigned int i = 0; i < (unsigned int)wls2.v_readings.size ();
               i++)
            {
              qDebug () << "i: " << i;
              Reading temp2 = { wls2.v_readings.at (i).lambda,
                                wls2.v_readings.at (i).od };
...
US_Extinction::plot ()
{
  ...
  for (int i = 0; i < v_wavelength.size (); i++)
    {
      xmax = max (
          xmax, (double)v_wavelength.at (i)
                    .v_readings.at (v_wavelength.at (i).v_readings.size () - 1)
                    .lambda);
  ...
  for (int j = 0; j < v_wavelength.size (); j++)
    {
      ...
      for (int i = 0; i < v_wavelength.at (j).v_readings.size (); i++)
        {
          xtemp.push_back (
              (double)v_wavelength.at (j).v_readings.at (i).lambda);
...
void
US_Extinction::reset_scanlist (void)
{
  v_wavelength.clear ();
...
void
US_Extinction::update_data (void)
{
  fitted = false;
  ...
  float minimum = le_lambdaLimitLeft->text ().toFloat (),
        maximum = le_lambdaLimitRight->text ().toFloat (),
      ...
      for (int j = 0; j < v_wavelength.at (i).v_readings.size (); j++)
        {
          ...
          //  v_wavelength.at(i).v_readings.size() - j);

          if (v_wavelength.at (i).v_readings.at (j).lambda >= maximum)
            {
              // v_wavelength[i].v_readings.remove(j,
              // v_wavelength.at(i).v_readings.size() - j); break;
              v_wavelength[i].v_readings.remove (j);
              --j;
            }
          ...
          if (v_wavelength.at (i).v_readings.at (j).lambda < minimum)
            {
              // qDebug() << "Inside MIN: " <<
              // v_wavelength.at(i).v_readings.at(j).lambda;
  ...
  if (ct_coefficient->value () < lambda_min
      || ct_coefficient->value () > lambda_max)
    {
      int wvlnewref = int (lambda_min + (lambda_max - lambda_min) / 2);
      QMessageBox msg;
      msg.setWindowTitle ("Reference Wavelength");
      QString text
          = QString ("A value of the default reference wavelength, %1 nm, is "
                     "outside of the selected wavelength range! It will be "
                     "adjusted to the middle of the wavelength domain, %2 nm.")
                .arg (ct_coefficient->value ())
                .arg (wvlnewref);
      msg.setText (text);
      msg.setInformativeText (
          "Reference wavelength can also be ajdusted manually.");

      // msgBox.setText("Buffer does not have spectrum data!\n You can Upload
      // and fit buffer spectrum, or Enter points manually");
      QPushButton *pContinue = msg.addButton (tr ("OK"), QMessageBox::YesRole);
      // QPushButton* pNewval = msg.addButton(tr("Enter New Value"),
      // QMessageBox::YesRole);
  ...
  qDebug () << "Orig., Updated: "
            << v_wavelength_original.at (0).v_readings.size () << ", "
            << v_wavelength.at (0).v_readings.size ();
  plot ();
}

void
US_Extinction::listToCurve (void)
{
  ...
  foreach (QwtPlotCurve *c, v_curve)
    {
      if (selectedName.contains (c->title ().text ()))
        {
          qDebug () << "selectedName, c->title().text() -- " << selectedName
                    << ", " << c->title ().text ();
          c_select = c;
        }
    }
...
bool
US_Extinction::deleteCurve (void)
{
  QMessageBox mBox;
  mBox.setText (
      tr ("Are you sure you want to delete the curve you double-clicked?"));
...
void
US_Extinction::perform_global_buffer (void)
{
  if (v_wavelength.size () < 1)
    {
      QMessageBox message;
      ...
      message.setText (
          tr ("You will need at least 1 scan \nto perform a global "
...
////////////////////////////////////////////////////////////////////////////
void
US_Extinction::perform_global_solution (void)
{
  if (v_wavelength.size () < 1)
    {
      QMessageBox message;
...
void
US_Extinction::perform_global_analyte (void)
{
  if (v_wavelength.size () < 1)
    {
      QMessageBox message;
  ...
  QString text = QString ("A value of %1 for the molar extinction coefficient "
                          "at %2 nm will be used.")
  ...
  // msgBox.setText("Buffer does not have spectrum data!\n You can Upload and
  // fit buffer spectrum, or Enter points manually");
  QPushButton *pContinue
      = msg.addButton (tr ("Continue"), QMessageBox::YesRole);
  QPushButton *pNewval
      = msg.addButton (tr ("Enter New Value"), QMessageBox::YesRole);
...
void
US_Extinction::perform_global (void)
{
  if (v_wavelength.size () < 1)
    {
      QMessageBox message;
  ...
  qDebug () << "LAMBDAs: min, max, step: " << lambda_min << ", " << lambda_max
            << ", " << lambda_step;
  for (unsigned int i = 0; i < order; i++)
    {
      fitparameters[v_wavelength.size () + (i * 3)] = 1;
      ...
      fitparameters[v_wavelength.size () + (i * 3) + 1]
          = lambda_min + lambda_step * i;
      fitparameters[v_wavelength.size () + (i * 3) + 2] = 10;
      // fitparameters[v_wavelength.size() + (i * 3) + 2] = 0.015;           //
      // Sigma
    }
  // opens the fitting GUI

  // DEBUG
  qDebug () << "order, parameters, projectName, fitting_widget " << order
            << parameters << projectName << fitting_widget;
      ...
      for (int j = 0; j < w_t.v_readings.size (); j++)
        {
          qDebug () << "Raw Data [SET " << i + 1 << " ]: X, Y -- "
                    << w_t.v_readings[j].lambda << w_t.v_readings[j].od;
        }
    }
  // END DEBUG
      ...
      for (j = 0; j < order; j++)
        {
          od += exp (
              ...
              - (pow ((lambda[i]
                      ...
                      2)
                 / (2
                    * pow (fitparameters[v_wavelength.size () + (3 * j) + 2],
                           2))));
          qDebug () << "J:" << j
                    << fitparameters[v_wavelength.size () + (3 * j)]
                    << fitparameters[v_wavelength.size () + (3 * j) + 1]
      ...
      if (od_wavelength != 0)
        {
          for (int i = 0; i < extinction.size (); i++)
            {
              extinction[i]
                  = extinction_coefficient * (extinction[i] / od_wavelength);
            }
        }
    }
}
...
void
US_Extinction::process_yfit (QVector<QVector<double> > &x,
...
void
US_Extinction::update_order (double new_order)
{
  order = (int)new_order;
}
void
US_Extinction::calculateE280 (void)
{
  ...
  connect (analyte_dialog, SIGNAL (valueChanged (US_Analyte)), this,
           SLOT (accessAnalyteExtinc (US_Analyte)));
...
void
US_Extinction::accessAnalyteExtinc (US_Analyte data)
{
...
void
US_Extinction::accept (void)
{
  if (!fitted)
    {
      QMessageBox mBox;
      mBox.setWindowTitle (tr ("Ultrascan Error:"));
      mBox.setText (tr ("You have not yet performed the Buffer fit. There is "
                        "no data to save."));
      mBox.exec ();
      return;
    }
...
void
US_Extinction::save (void)
{
  if (!fitted)
    {
      QMessageBox mBox;
      mBox.setWindowTitle (tr ("Ultrascan Error:"));
      mBox.setText (tr ("You have not yet performed the global fit. There is "
                        "no data to save."));
      mBox.exec ();
      return;
    }
  ...
  // "/home/minji/ultrascan/results/", "*.extinction.dat");
  ...
  QString filename = QFileDialog::getSaveFileName (
      this, "Save File", filename_passed, "*.extinction.dat");
  if (filename.isEmpty ())
    return;
  ...
  if (f2.open (QIODevice::WriteOnly | QIODevice::Text))
    {
      QTextStream ts (&f2);
      ts << tr ("Results for global wavelength scan/extinction coefficient "
                "fit:\n\n");
      ts << tr ("Number of Gaussian terms: ") << order << "\n\n";
      ts << tr ("Extinction coeffcient at ") << selected_wavelength
         << tr (" nm used for normalization of Data: ")
      ...
      for (unsigned int i = 0; i < order; i++)
        {
          ts << "\n" << (i + 1) << tr (". Gaussian:\n");
          ts << tr ("Peak position: ")
             << fitparameters[v_wavelength.size () + (i * 3) + 1] << " nm\n";
          ts << tr ("Amplitude of peak: ")
             << exp (fitparameters[v_wavelength.size () + (i * 3)])
             << tr (" extinction\n");
          ts << tr ("Peak width: ")
             << fitparameters[v_wavelength.size () + (i * 3) + 2] << " nm\n";
        }
      ts << "\n";
      ts << "Equation to be multiplied by the relative concentration of each "
            "scan (printed below)\n";
      ts << "to re-generate the curve for each scan:\n\n";
      ts << "Concentration = Relative_Conc * SUM amplitude_i * exp((-(lambda "
            "- position_i)^2) / (2*width_i^2))\n";
      ts << "\n";
      for (int i = 0; i < v_wavelength.size (); i++)
        {
          ts << tr ("Relative Concentration of Scan ") << (i + 1) << ": "
             << fitparameters[i] << "\n";
        }
      ts << "\n";
      ts << "/* A small C++ program to calculate the concentration curve\n";
      ts << "   compile with \"g++ myprog.cpp\"\n";
      ts << "   run with \"./a.out relative_concentration wavelength_start "
            "wavelength_end\"\n";
      ts << "*/\n";
      ts << "#include <iostream>\n";
      ts << "#include <math.h>\n";
      ...
      ts << "{\n";
      ts << "  float p[" << order << "], w[" << order << "], a[" << order
         << "], c=0.0, rel_conc;\n";
      ts << "  int start, end;\n";
      for (unsigned int i = 0; i < order; i++)
        {
      ...
      ts << "  if(argc < 4)\n";
      ts << "  {\n";
      ts << "     cout << \"\\nUsage: a,out relative_concentration "
      ...
      ts << "     {\n";
      ts << "        c += rel_conc * a[i] * exp(-pow(j-p[i], "
            "2.0)/(2.0*w[i]*w[i]));\n";
      ts << "     }\n";
      ts << "     cout << j << \", \" << c << endl;\n";
      ts << "  }\n";
...
void
US_Extinction::view_result (void)
{
  save ();
  QTextEdit *e1;
  e1 = new QTextEdit ();
...
void
US_Extinction::help (void)
{
}

bool
CustomListWidgetItem::operator< (const QListWidgetItem &other) const
{
  QRegularExpression re;
  re.setPatternOptions (QRegularExpression::CaseInsensitiveOption);
  QRegularExpressionMatch match;
