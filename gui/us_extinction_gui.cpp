...
US_Extinction::US_Extinction(QString buffer, const QString& text,
  ...
  if (buffer_temp.toStdString() == "BUFFER") lbl_peptide->hide();
  ...
  if (buffer_temp.toStdString() == "BUFFER") lbl_coefficient->hide();

  lbl_wavelengthref = us_label(tr("Wavelength:"));
  if (buffer_temp.toStdString() == "BUFFER") lbl_wavelengthref->hide();
  ...
  if (text_e280.toStdString() != "0") le_coefficient->setText(text_e280);

  if (buffer_temp.toStdString() == "BUFFER") le_coefficient->hide();
  ...
  if (buffer_temp.toStdString() == "BUFFER") ct_coefficient->hide();
...
bool US_Extinction::loadScan(US_CSV_Data& csv_data) {
  ...
  for (mm = Wvs_to_descr_map.begin(); mm != Wvs_to_descr_map.end(); ++mm) {
    ...
    if (wls.v_readings.at(0).lambda >
        wls.v_readings.at(wls.v_readings.size() - 1)
            .lambda) {  // we need to reverse the order of entries
      WavelengthScan wls2;
...
void US_Extinction::plot() {
  ...
  for (int i = 0; i < v_wavelength.size(); i++) {
    xmax =
        max(xmax, (double)v_wavelength.at(i)
...
void US_Extinction::update_data(void) {
  ...
  if (ct_coefficient->value() < lambda_min ||
    ...
    QString text =
        QString(
            "A value of the default reference wavelength, %1 nm, is outside of "
            "the selected wavelength range! It will be adjusted to the middle "
            "of the wavelength domain, %2 nm.")
...
void US_Extinction::perform_global_buffer(void) {
  ...
  connect(
      fitter,
      SIGNAL(get_yfit(QVector<QVector<double> >&, QVector<QVector<double> >&)),
      this,
...
void US_Extinction::perform_global_analyte(void) {
  ...
  QString text = QString(
                     "A value of %1 for the molar extinction coefficient at %2 "
                     "nm will be used.")
  ...
  for (i = 0; i < maxrange; i++) {
    ...
    for (j = 0; j < order; j++) {
      od += exp(
          ...
          (pow((lambda[i] - fitparameters[v_wavelength.size() + (3 * j) + 1]),
               2) /
...
void US_Extinction::process_yfit(QVector<QVector<double> >& x,
...
void US_Extinction::update_order(double new_order) { order = (int)new_order; }
...
void US_Extinction::accept(void) {
  if (!fitted) {
    ...
    mBox.setText(
        tr("You have not yet performed the Buffer fit. There is no data to "
           "save."));
    mBox.exec();
    return;
  }
...
void US_Extinction::save(void) {
  if (!fitted) {
    ...
    mBox.setWindowTitle(tr("Ultrascan Error:"));
    mBox.setText(
        tr("You have not yet performed the global fit. There is no data to "
           "save."));
    mBox.exec();
    return;
  }
  ...
  if (f2.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    ts << tr("Extinction coeffcient at ") << selected_wavelength
       << tr(" nm used for normalization of Data: ") << extinction_coefficient
       << "\n\n";
