...
void US_License::help(void) { online_help.show_help("manual/register.html"); }
...
void US_License::update(void) {
  if (firstname == "" || lastname == "" || institution == "" || state == "" ||
      email == "") {
    QMessageBox::information(this, tr("Request Error"),
  ...
  if (license.size() > 8) {
    ...
    if (le_email->text() != license[8]) {
      int result =
          QMessageBox::question(this, tr("License Change"),
                                tr("Your email address is changing.\n"
                                   "You must validate your new address using "
