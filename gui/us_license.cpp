...
US_License::US_License(QWidget *parent, Qt::WindowFlags flags) : US_Widgets(true, parent, flags) {
   ...
   // Banner
   QLabel *banner = us_banner(tr("Please enter all fields"));

   banner->setGeometry(QRect(xpos, ypos, width, spacing + rowHeight));
   ...
   connect(cbb_state, SIGNAL(currentIndexChanged(const QString &)), SLOT(update_state(const QString &)));
   ...
   connect(cbb_licensetype, SIGNAL(currentIndexChanged(const QString &)), SLOT(update_licensetype(const QString &)));
...
void US_License::update(void) {
   if (firstname == "" || lastname == "" || institution == "" || state == "" || email == "") {
      QMessageBox::information(
         this, tr("Request Error"),
   ...
   // Form request
   QString req = "firstname=" + firstname + "&lastname=" + lastname + "&institution=" + institution + "&address="
                 + address + "&city=" + city + "&state=" + state + "&zip=" + zip + "&phone=" + phone + "&email=" + email
                 + "&os=" + OS + "&platform=" + PLATFORM + "&version=" + US_Version + "&licensetype=" + licensetype;
   ...
   connect(transmit, SIGNAL(US_Http_post_response(const QString &)), this, SLOT(update_response(const QString &)));
}
...
void US_License::update_response(const QString &response) {
   ...
   if (error == 0) {
      ...
      QMessageBox::information(this, tr("Update Error"), response);
}

void US_License::request(void) {
   //qDebug() << "Starting Req";
   // Form request
   QString request = "firstname=" + firstname + "&lastname=" + lastname + "&institution=" + institution
                     + "&address=" + address + "&city=" + city + "&state=" + state + "&zip=" + zip + "&phone=" + phone
                     + "&email=" + email + "&os=" + OS + "&platform=" + PLATFORM + "&version=" + US_Version
                     + "&licensetype=" + licensetype;

   // Save partial request
   QStringList license;
   license << lastname << firstname << institution << address << city << state << zip << phone << email
           << platform.toLower() << os.toLower() << version;
   ...
   connect(transmit, SIGNAL(US_Http_post_response(const QString &)), this, SLOT(request_response(const QString &)));
}
...
bool US_License::save(void) {
   ...
   license << lastname << firstname << institution << address << city << state << zip << phone << email
           << platform.toLower() << os.toLower() << version << validation.toUpper() << expiration;
