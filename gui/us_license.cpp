...
void
US_License::help (void)
{
  online_help.show_help ("manual/register.html");
}

QString
US_License::titleCase (const QString &phrase)
{
...
void
US_License::update (void)
{
  if (firstname == "" || lastname == "" || institution == "" || state == ""
      || email == "")
    {
      ...
      if (le_email->text () != license[8])
        {
          int result = QMessageBox::question (
              ...
              tr ("Your email address is changing.\n"
                  "You must validate your new address using new registration "
                  "procedures.\n"
                  "Continue?"),
  ...
  // Form request
  QString req = "firstname=" + firstname + "&lastname=" + lastname
                + "&institution=" + institution + "&address=" + address
                + "&city=" + city + "&state=" + state + "&zip=" + zip
                + "&phone=" + phone + "&email=" + email + "&os=" + OS
                + "&platform=" + PLATFORM + "&version=" + US_Version
  ...
  connect (transmit, SIGNAL (US_Http_post_response (const QString &)), this,
           SLOT (update_response (const QString &)));
}

// Process response
void
US_License::update_response (const QString &response)
{
...
void
US_License::request (void)
{
  // qDebug() << "Starting Req";
  // Form request
  QString request = "firstname=" + firstname + "&lastname=" + lastname
                    + "&institution=" + institution + "&address=" + address
                    + "&city=" + city + "&state=" + state + "&zip=" + zip
                    + "&phone=" + phone + "&email=" + email + "&os=" + OS
                    + "&platform=" + PLATFORM + "&version=" + US_Version
  ...
  QStringList license;
  license << lastname << firstname << institution << address << city << state
          << zip << phone << email << platform.toLower () << os.toLower ()
          << version;
  ...
  connect (transmit, SIGNAL (US_Http_post_response (const QString &)), this,
           SLOT (request_response (const QString &)));
}

void
US_License::request_response (const QString &response)
{
  QStringList data = response.split ("-");
  // qDebug() << data;
  int error = data[0].toInt ();
  ...
  if (error == 0)
    {
      ...
      QMessageBox::information (
          ...
          tr ("You have been sent an email to confirm your request.\n"
              "After you respond to the email, select 'Finish "
              "Registration'."));
    }
  else
    {
...
US_License::save (void)
{
  ...
  license << lastname << firstname << institution << address << city << state
          << zip << phone << email << platform.toLower () << os.toLower ()
          << version << validation.toUpper () << expiration;
...
QString
US_License::trim (const QString &st)
{
  QString s = st;
  QRegExp blanks ("(^\\s+)|(\\s+$)");
