    ...
    : US_WidgetsDialog (0, 0), model (current_model)
{
  ...
  connect (dkdb_cntrls, SIGNAL (changed (bool)), SLOT (upd_disk_db (bool)));
  connect (le_mlfilt, SIGNAL (editingFinished ()), SLOT (filter_changed ()));
...
bool
US_ModelGui::load_model (const QString &load_init, US_Model &modelIn)
{
  bool result = false;
  bool loaded = false;
      ...
      if (QFile::exists (load_init) && QFileInfo (load_init).isReadable ())
      ...
      else
        {
          ...
          if (!US_Model::model_path (path))
            {
              QMessageBox::information (this, tr ("Model Path Error"),
          ...
          if (!dkdb_cntrls->db ())
            {
              ...
              QStringList f_names
                  = f.entryList (filter, QDir::Files, QDir::Name);
          ...
          for (auto &model_description : model_descriptions)
            {
              if (model_description.modelGUID == load_init
                  || model_description.DB_id == load_init)
                {
                  if (dkdb_cntrls->db ())
                    { // Load from database
                      US_Passwd pw;
...
void
US_ModelGui::show_model_desc (void)
{
  lw_models->clear ();
...
bool
US_ModelGui::ignore_changes (void)
{
  if (working_model == model)
    return true;

  // check if leaving GMP produced model after potential changes in
  // Components..
...
void
US_ModelGui::edit_description (void)
{
  int row = lw_models->currentRow ();
  if (row < 0 || model_descriptions.size () == 0)
    return;
  ...
  if (desc == lw_models->item (row)->text ())
    return;
...
void
US_ModelGui::select_model (QListWidgetItem *item)
{
  qDebug () << "SelMdl: IN";
  if (!ignore_changes ())
    {
...
bool
US_ModelGui::is_modelIDs_from_autoflow (QString mID)
{
...
bool
US_ModelGui::parse_models_desc_json (QStringList modelInfos_autoflow,
                                     QString mID)
{
...
US_ModelGui::delete_model (void)
{
  ...
  // 				 tr( "The Model:\n\n"
  // 				     "\"%1\"\n\n"
  // 				     "can NOT be deleted since it was generated
  // within GMP framework!" ) 				 .arg( mdesc ) );
  ...
  if (!dkdb_cntrls->db ())
  ...
  else // Remove from DB
    {
      ...
      if (status_query (q))
        QMessageBox::information (
            this, tr ("Model Deleted"),
            tr ("The model has been deleted from the database."));
    }
}

bool
US_ModelGui::status_query (const QStringList &q)
{
  US_Passwd pw;
  US_DB2 db (pw.getPasswd ());
...
US_ModelGui::check_db (void)
{
  ...
  QString number
      = (investigator > 0) ? QString::number (investigator) + ": " : "";

  le_investigator->setText (number + US_Settings::us_inv_name ());
}

void
US_ModelGui::connect_error (const QString &error)
{
...
bool
US_ModelGui::database_ok (US_DB2 &db)
{
  if (db.lastErrno () == US_DB2::OK)
    return true;
...
void
US_ModelGui::manage_components (void)
{
  ...
  if (index < 0)
    {
      QMessageBox::information (this, tr ("Model not selected"),
  ...
  connect (this, SIGNAL (disable_components_gui ()), dialog,
           SLOT (disable_gui ()));
...
US_ModelGui::save_model (void)
{
  ...
  //     QMessageBox::information( this, tr( "Selected Model Cannot be
  //     Saved/Updated" ),
  // 				tr( "The Model:\n\n"
  // 				    "\"%1\"\n\n"
  // 				    "can NOT be saved/updated since it was
  // generated within GMP framework!" ) 				 .arg( mdesc ) );
  //     return;
  //   }
  ...
  if (!dkdb_cntrls->db ())
  ...
  else // Save/update in DB
    {
      ...
      if (model.write (&db) == US_DB2::OK)
        {
          QMessageBox::information (
              this, tr ("Model Written"),
              tr ("The model has been %1 in the database.")
                  .arg (model.message));
...
bool
US_ModelGui::verify_model (void)
{
  if (model.components.size () == 0)
    {
      QMessageBox::information (this, tr ("Model Error"),
...
int
US_ModelGui::modelIndex (QString mdesc, QList<ModelDesc> mds)
{
...
void
US_ModelGui::source_changed (bool db)
{
  qDebug () << "SrcChg: db" << db;
  if (db)
    dkdb_cntrls->set_db ();
...
void
US_ModelGui::upd_disk_db (bool db)
{
  qDebug () << "UpdDDb: db" << db;
  model_descriptions.clear ();
