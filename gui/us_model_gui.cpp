...
US_ModelGui::US_ModelGui(US_Model &current_model) : US_WidgetsDialog(0, 0), model(current_model) {
   ...
   connect(le_description, SIGNAL(editingFinished()), SLOT(edit_description()));
   ...
   connect(lw_models, SIGNAL(itemClicked(QListWidgetItem *)), SLOT(select_model(QListWidgetItem *)));

   if (!model.description.isEmpty() && model.description != "New Model") { // if re-loading a previous model, list that model
      ModelDesc desc;
...
bool US_ModelGui::load_model(const QString &load_init, US_Model &modelIn) {
   ...
   if (!load_init.isEmpty()) {
      if (QFile::exists(load_init) && QFileInfo(load_init).isReadable()) { // Argument is a file path
         ...
         if (!US_Model::model_path(path)) {
            QMessageBox::information(this, tr("Model Path Error"), tr("The model path is not set."));
            return false;
         }
         ...
         {
            if (investigator < 0) {
               QMessageBox::information(this, tr("Investigator not set"), tr("Please select an investigator first."));
               return false;
            }
         ...
         for (auto &model_description: model_descriptions) {
            if (model_description.modelGUID == load_init || model_description.DB_id == load_init) {
               if (dkdb_cntrls->db()) { // Load from database
                  US_Passwd pw;
                  US_DB2 db(pw.getPasswd());
...
bool US_ModelGui::ignore_changes(void) {
   ...
   int response = QMessageBox::question(
      this, tr("Model Changed"), tr("The model has changed.  Do you want to ignore the changes?"), QMessageBox::Cancel,
      QMessageBox::Yes);
...
void US_ModelGui::select_model(QListWidgetItem *item) {
   ...
   if (is_modelIDs_from_autoflow(model_descriptions[ modlx ].DB_id)) {
      ...
      QMessageBox::information(
         this, tr("Selected Model GMP produced!"),
         tr("Selected Model:\n\n"
            "\"%1\"\n\n"
...
void US_ModelGui::delete_model(void) {
   ...
   else // Remove from DB
   {
      ...
      if (status_query(q))
         QMessageBox::information(this, tr("Model Deleted"), tr("The model has been deleted from the database."));
   }
}
...
void US_ModelGui::check_db(void) {
   ...
   QString number = (investigator > 0) ? QString::number(investigator) + ": " : "";
...
bool US_ModelGui::database_ok(US_DB2 &db) {
   ...
   QMessageBox::information(this, tr("Database Error"), tr("The following error was returned:\n") + db.lastError());
...
void US_ModelGui::save_model(void) {
   ...
   if (!dkdb_cntrls->db()) {
      ...
      QMessageBox::information(this, tr("Model Saved"), tr("The model has been %1 in the disk.").arg(save_type));
   ...
   else // Save/update in DB
   {
      ...
      if (model.write(&db) == US_DB2::OK) {
         QMessageBox::information(
            this, tr("Model Written"), tr("The model has been %1 in the database.").arg(model.message));

         working_model = model;
         le_guid->setText(model.modelGUID);
      }
      else {
         QMessageBox::information(this, tr("Database Error"), tr("The model could not be saved:\n") + model.message);
         return;
      }
   }
...
bool US_ModelGui::verify_model(void) {
   if (model.components.size() == 0) {
      QMessageBox::information(this, tr("Model Error"), tr("The model has no components."));
      return false;
   }

   if (model.description.size() == 0 || model.description.trimmed() == "New Model") {
      QMessageBox::information(
...
void US_ModelGui::list_models(void) {
   ...
   if (model_descriptions.count() == 0) { // No model descriptions, so get them all
      QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
   ...
   le_nlines->setText(nlines == 1 ? tr("1 list item") : tr("%1 list items").arg(nlines));
   qApp->processEvents();

   if (nlines == 0)
      lw_models->addItem("No models found.");

   else { // Resize to show reasonable portion of list
