...
US_ModelGui::US_ModelGui(US_Model& current_model)
  ...
  if (!model.description.isEmpty() &&
      model.description !=
          "New Model") {  // if re-loading a previous model, list that model
    ModelDesc desc;
...
bool US_ModelGui::load_model(const QString& load_init, US_Model& modelIn) {
  ...
  if (!load_init.isEmpty()) {
    if (QFile::exists(load_init) &&
        QFileInfo(load_init).isReadable()) {  // Argument is a file path
      ...
      {
        if (investigator < 0) {
          QMessageBox::information(this, tr("Investigator not set"),
...
void US_ModelGui::show_model_desc(void) {
  ...
  for (int ii = 0; ii < model_descriptions.size(); ii++) {
    ...
    if (listdesc && !desc.contains(mpart)) continue;
...
bool US_ModelGui::parse_models_desc_json(QStringList modelInfos_autoflow,
                                         QString mID) {
...
void US_ModelGui::accept_model(void) {
  ...
  if (recent_row >= 0) emit valueChanged(model);
...
void US_ModelGui::save_model(void) {
  ...
  if (!dkdb_cntrls->db()) {
    ...
    if (le_guid->text().size() != 36) le_guid->setText(US_Util::new_guid());
...
int US_ModelGui::modelIndex(QString mdesc, QList<ModelDesc> mds) {
  for (int ii = 0; ii < mds.size(); ii++) {
    if (mdesc == mds[ii].description) return ii;
  }
...
void US_ModelGui::list_models(void) {
  ...
  if (model_descriptions.count() ==
      0) {  // No model descriptions, so get them all
    QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
