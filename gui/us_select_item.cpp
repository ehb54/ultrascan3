...
US_SelectItem::US_SelectItem (QList<QStringList> &items, QStringList &hdrs,
                              const QString titl, int *aselxP,
                              QString add_label, const int def_sort)
    : US_WidgetsDialog (0, 0), items (items), hdrs (hdrs)
{
  multi_sel = false;
  deleted_button = false;
...
void
US_SelectItem::build_layout (const QString titl)
{
  setWindowTitle (titl);
      ...
      if (autoflow_dev)
        {
          le_info->setText (
              tr ("Information on one or more developmental runs based on "
                  ...
                  "<ul><li>You can reattach to a specific job by selecting it "
                  "from the list below and clicking \"Select Development Run "
                  "to Follow\"</ul></li>"
                  "<ul><li>Alternatively, you can click \"Setup New "
                  "Development Run\" to modify an existing analysis protocol "
                  "and re-analyze data</ul></li>"
                  "<ul><li>Records can be removed form the list (\"Remove "
                  "from the List\"). NOTE that such runs cannot be monitored "
                  "with this program</ul></li>"
                  "<ul><li>Finally, selected record can be marked as "
                  "\"Failed\". The program will re-initialize the run from "
                  "the 3. IMPORT stage</ul></li>"));
        }
      else
        {
          le_info->setText (
              tr ("Information on one or more experimental methods submitted "
                  "to Beckman Optima AUC Instruments is available."
                  "<ul><li>You can reattach to a specific job by selecting it "
                  "from the list below and clicking \"Select Optima Run to "
                  "Follow\"</ul></li>"
                  "<ul><li>Alternatively, you can click \"Define Another "
                  "Experiment\" to design and/or submit a new experimental "
                  "method to the availabale Optima instrument(s)</ul></li>"
                  "<ul><li>Records can be removed form the list (\"Remove "
                  "from the List\"). NOTE that such runs cannot be monitored "
                  "with this program</ul></li>"
                  "<ul><li>Finally, selected record can be marked as "
  ...
  QPushButton *pb_delete_autoflow
      = us_pushbutton (tr ("Remove from the List"));
  ...
  connect (pb_delete_autoflow, SIGNAL (clicked ()),
           SLOT (deleted_autoflow ()));
...
void
US_SelectItem::search (const QString &search_string)
{
...
US_SelectItem::list_data ()
{
  ...
  for (int ii = 0; ii < mxrows; ii++)
    { // Propagate list widget with labels
      ...
      if (have_search && !iname.contains (dsearch, Qt::CaseInsensitive))
        continue;
      ...
      // qDebug() << "4a: autoflow_da, items[ ii ][ ncols -1 ]: " <<
      // autoflow_da << "," <<  items[ ii ][ ncols -1 ];
          ...
          if (autoflow_da && items[ii][ncols - 2] == "YES")
            {
              ...
              // //tw_data->item( kk,
              // 0)->setForeground(QBrush(QColor(250,0,0)));
...
*/
void
US_SelectItem::deleted_autoflow ()
{
  QList<QTableWidgetItem *> selitems = tw_data->selectedItems ();
  ...
  msgBox.setInformativeText (
      "<font color='red'><b>NOTE:</b> if deleted, this run cannot be "
      "monitored with this program anymore!</font>");
      ...
      if (status == US_DB2::NO_AUTOFLOW_RECORD)
        {
          ...
          emit accept_autoflow_deletion (); // Signal to pass to us_comproject
                                            // to update (re-read reduced)
                                            // autoflow records
...
US_SelectItem::deleted ()
{
  ...
  // Here, check if the protocol part of the autoflow || autoflowHistory
  // tables: If YES, inform of inability to delete, return:
  ...
  emit accept_deletion (); // Signal to pass to us_experiment to update
                           // (re-read reduced) protdata
}

// Check if the protocol part of GMP framework
bool
US_SelectItem::check_protocol_for_autoflow (QString pID, QString pName)
{
...
void
US_SelectItem::do_refresh_state_autoflow ()
{
...
void
US_SelectItem::set_unset_failed_autoflow ()
{
  QList<QTableWidgetItem *> selitems = tw_data->selectedItems ();
  ...
  QString add_msg = "This implies the run will be re-started from stage <b>3. "
                    "IMPORT</b> upon next re-attachment.";
  bool isFailed = false;
  if (failedID.toInt () != 0)
    {
  ...
  QMessageBox msgBox;
  msgBox.setText (tr ("You have selected to change the status for the "
                      "following run:<br><br>")
                  + tr ("<b>ID:&emsp;</b>") + items[AutoflowRow][0]
                  + tr ("<br>") + tr ("<b>Name:&emsp;</b>")
                  + items[AutoflowRow][1] + tr ("<br>")
  ...
  QPushButton *Confirm
      = msgBox.addButton (tr ("Proceed"), QMessageBox::YesRole);
  QPushButton *Cancel
      = msgBox.addButton (tr ("Cancel"), QMessageBox::RejectRole);
  ...
  if (msgBox.clickedButton () == Cancel)
  ...
  else if (msgBox.clickedButton () == Confirm)
    {
      ...
      if (!isFailed)
        {
          // Setting as FAILED: additional info (reason, stage) can be
          // specified:
          if (protocol_details["gmpRun"] == "YES ")
               ...
               // autoflowFailed record & attach to the autoflow
            {
              ...
              if (!autoflowFailedID)
                {
                  QMessageBox::warning (
                      ...
                      tr ("autoflowFailed: There was a problem with creating "
                          "a record in autoflowFailed table \n"));
              ...
              if (status == US_DB2::NO_AUTOFLOW_RECORD)
                {
                  QMessageBox::warning (
                      this, tr ("Autoflow's failedID field Not Updated"),
...
void
US_SelectItem::show_autoflow_run_as_failed (void)
{
  int last_col = ncols - 1;
  qDebug () << "Setting as Failed: failed_run_row, ncols-1 -- "
            << failed_run_row << last_col;
