...
US_SelectItem::US_SelectItem(QList<QStringList>& items, QStringList& hdrs,
                             const QString titl, int* aselxP,
                             const int def_sort)
  ...
  selxP = aselxP;
  selxsP = NULL;
  sort_ord = (def_sort < 0) ? Qt::DescendingOrder : Qt::AscendingOrder;
...
US_SelectItem::US_SelectItem(QList<QStringList>& items, QStringList& hdrs,
                             const QString titl, int* aselxP, QString add_label,
                             const int def_sort)
    : US_WidgetsDialog(0, 0), items(items), hdrs(hdrs) {
  multi_sel = false;
  deleted_button = false;
  ...
  if (!add_label.isEmpty()) {
    if (add_label == "DELETE") deleted_button = true;
...
US_SelectItem::US_SelectItem(QList<QStringList>& items, QStringList& hdrs,
                             const QString titl, QList<int>* aselxsP,
                             const int def_sort)
    : US_WidgetsDialog(0, 0), items(items), hdrs(hdrs) {
  multi_sel = true;
  deleted_button = false;
...
void US_SelectItem::build_layout(const QString titl) {
  ...
  if (autoflow_button) {
    ...
    if (autoflow_dev) {
      le_info->setText(
          tr("Information on one or more developmental runs based on "
             "previously completed experiments is available."
             "<ul><li>You can reattach to a specific job by selecting it from "
             "the list below and clicking \"Select Development Run to "
             "Follow\"</ul></li>"
             "<ul><li>Alternatively, you can click \"Setup New Development "
             "Run\" to modify an existing analysis protocol and re-analyze "
             "data</ul></li>"
             "<ul><li>Records can be removed form the list (\"Remove from the "
             "List\"). NOTE that such runs cannot be monitored with this "
             "program</ul></li>"
             "<ul><li>Finally, selected record can be marked as \"Failed\". "
             "The program will re-initialize the run from the 3. IMPORT "
             "stage</ul></li>"));
    } else {
      le_info->setText(tr(
          "Information on one or more experimental methods submitted to "
          "Beckman Optima AUC Instruments is available."
          "<ul><li>You can reattach to a specific job by selecting it from the "
          "list below and clicking \"Select Optima Run to Follow\"</ul></li>"
          "<ul><li>Alternatively, you can click \"Define Another Experiment\" "
          "to design and/or submit a new experimental method to the availabale "
          "Optima instrument(s)</ul></li>"
          "<ul><li>Records can be removed form the list (\"Remove from the "
          "List\"). NOTE that such runs cannot be monitored with this "
          "program</ul></li>"
          "<ul><li>Finally, selected record can be marked as \"Failed\". The "
          "program will re-initialize the run from the 3. IMPORT "
          "stage</ul></li>"));
    }
  ...
  QPushButton* pb_accept =
      us_pushbutton(multi_sel ? tr("Select Item(s)") : accept_pb_label);

  QPushButton* pb_delete = us_pushbutton(tr("Delete Item"));
  QPushButton* pb_delete_autoflow = us_pushbutton(tr("Remove from the List"));

  QPushButton* pb_mark_unmark_failed_autoflow =
      us_pushbutton(tr("Mark/Unmark Run as Failed"));

  QPushButton* pb_refresh_state_autoflow =
      us_pushbutton(tr("Refresh Optima Run States"));
  ...
  connect(pb_mark_unmark_failed_autoflow, SIGNAL(clicked()),
          SLOT(set_unset_failed_autoflow()));
  connect(pb_refresh_state_autoflow, SIGNAL(clicked()),
          SLOT(do_refresh_state_autoflow()));

  if (!deleted_button) pb_delete->hide();

  if (!deleted_button_autoflow) pb_delete_autoflow->hide();

  if (!set_unset_failed_button_autoflow) pb_mark_unmark_failed_autoflow->hide();

  if (!refresh_state_autoflow) pb_refresh_state_autoflow->hide();
...
void US_SelectItem::list_data() {
  ...
  for (int ii = 0; ii < nitems;
       ii++) {  // Save column0 description in a separate list
    itemlist << items[ii][0];
  }
  //////////////////
  ...
  if (mxrows == 0) {  // Return now if there are no list items
  ...
  if (have_search) {  // We have search text, so we must count the filtered
                      // items
    nrows = 0;
    for (int ii = 0; ii < mxrows;
         ii++) {  // Bump therow count for each item name matching the filter
      ...
      if (iname.contains(dsearch, Qt::CaseInsensitive)) nrows++;
    }
  } else {  // There is no filter, so the row count is the full list count
    nrows = mxrows;
  }
  ...
  for (int ii = 0; ii < mxrows;
    ...
    qDebug() << "LIST DATA 4a";
    qDebug() << "4a: autoflow_da, items[ ii ][ ncols -1 ]: " << autoflow_da
             << "," << items[ii][ncols - 1];
    qDebug() << "LIST DATA 4ab";

    qDebug() << "have_search, iname, dsearch -- " << have_search << iname
             << dsearch;

    // Skip where name does not match the filter
    if (have_search && !iname.contains(dsearch, Qt::CaseInsensitive)) continue;
    ...
    // ALEXEY: if GMP run ("YES") & open with DA software (autoflow_da == true),
    ...
    for (int jj = 1; jj < ncols;
         jj++) {  // Set fields for remaining columns of the present row
      tw_data->setItem(kk, jj, new QTableWidgetItem(items[ii][jj]));

      // ALEXEY: if GMP run ("YES") & open with DA software (autoflow_da ==
      // true), make item unselectable:
      if (autoflow_da && items[ii][ncols - 2] == "YES") {
...
void US_SelectItem::accepted() {
  ...
  if (multi_sel &&
      selxsP != NULL) {  // Possibly multiple selections:  return their indexes
    selxsP->clear();

    for (int ii = 0; ii < selitems.size();
         ii++) {  // Return indexes to selected items
      ...
      (*selxsP) << qMax(0, itemlist.indexOf(twi->text()));
           ...
           selxP != NULL) {  // Return the index to the selected item
...
/*  ALEXEY: For future Delete() function:
    ...
    delete (stored procedure)
           + emit signal to us_experiment to update protdata BEFORE any
   selection; then remove items[selxP] from QList; then rebuild list by running
   list_data(); [move part of ]
*/
void US_SelectItem::deleted_autoflow() {
  ...
  if (selitems.size() == 0) {
    QMessageBox::information(
        this, tr("No Autoflow Record Selected"),
  ...
  qDebug() << "Autoflow ID to delete: ID, name, run status: " << AutoflowID
           << ", " << items[AutoflowRow][1] << ", " << items[AutoflowRow][4];

  QMessageBox msgBox;
  msgBox.setText(
      tr("You have selected the following Record to delete:<br><br>") +
      tr("<b>ID:&emsp;</b>") + items[AutoflowRow][0] + tr("<br>") +
      tr("<b>Name:&emsp;</b>") + items[AutoflowRow][1] + tr("<br>") +
      tr("<b>Status:&emsp;</b> ") + items[AutoflowRow][4] +
      tr("<br><br>Proceed?"));
  msgBox.setInformativeText(
      "<font color='red'><b>NOTE:</b> if deleted, this run cannot be monitored "
      "with this program anymore!</font>");
  ...
  if (msgBox.clickedButton() == Cancel)
    ...
    q.clear();
    q << QString("delete_autoflow_record_by_id") << AutoflowID;
    ...
    if (status == US_DB2::NO_AUTOFLOW_RECORD) {
      ...
      emit
      accept_autoflow_deletion();  // Signal to pass to us_comproject to update
                                   // (re-read reduced) autoflow records
    ...
    q.clear();
    q << "delete_autoflow_stages_record" << AutoflowID;
    ...
    for (int i = 0; i < analysisIDs_list.size(); ++i) {
      ...
      q << "update_autoflow_analysis_record_at_deletion"
        << QString("Canceled by user, top-level") << requestID;
    ...
    QMessageBox::information(this, tr("Autoflow Record Deleted"), msg);
...
void US_SelectItem::deleted() {
  ...
  qDebug() << "Prtotcol ID to delete: ID, name: " << ProtID << ", "
           << items[ProtRow][0];
  ...
  if (isRequired) {
    QMessageBox::information(
        this, tr("Protocol Cannot be Deleted"),
        tr("The Protocol:\n\n"
           "\"%1\"\n\n"
  ...
  QStringList q("");
  q.clear();
  q << QString("delete_protocol") << ProtID;

  int status = db->statusQuery(q);

  if (status == US_DB2::PROTOCOL_IN_USE) {
    QMessageBox::warning(this, tr("Protocol Not Deleted"),
  ...
  QMessageBox::information(this, tr("Protocol Deleted"), msg);

  emit accept_deletion();  // Signal to pass to us_experiment to update (re-read
                           // reduced) protdata
}
...
bool US_SelectItem::check_protocol_for_autoflow(QString pID, QString pName) {
  ...
  QStringList q;
  q << QString("check_protocol_for_autoflow") << pName;
...
void US_SelectItem::do_refresh_state_autoflow() {
  emit accept_refresh_states();  // Signal to pass to us_comproject to update
                                 // Run List
}
...
void US_SelectItem::set_unset_failed_autoflow() {
  ...
  //
  qDebug() << "Autoflow ID to set/uset as FAILED: AutoflowRow, ID, name, run "
              "status: "
           << AutoflowRow << ", " << AutoflowID << ", " << items[AutoflowRow][1]
           << ", " << items[AutoflowRow][4];
  ...
  QString add_msg =
      "This implies the run will be re-started from stage <b>3. IMPORT</b> "
      "upon next re-attachment.";
  bool isFailed = false;
  if (failedID.toInt() != 0) {
  ...
  QMessageBox msgBox;
  msgBox.setText(tr("You have selected to change the status for the following "
                    "run:<br><br>") +
                 tr("<b>ID:&emsp;</b>") + items[AutoflowRow][0] + tr("<br>") +
                 tr("<b>Name:&emsp;</b>") + items[AutoflowRow][1] + tr("<br>") +
                 tr("<b>Current Status:&emsp;</b> ") + current_mark +
                 tr("<br><br> If proceeded, it will be marked as %1. %2")
                     .arg(new_mark)
                     .arg(add_msg) +
                 tr("<br><br>Proceed?"));
  // msgBox.setInformativeText("<font color='red'><b>NOTE:</b> if deleted, this
  // run cannot be monitored with this program anymore!</font>");
  ...
  if (msgBox.clickedButton() == Cancel)
    ...
    if (!isFailed) {
      ...
      if (protocol_details["gmpRun"] == "YES ") {
        ...
        connect(fdiag, SIGNAL(failed_status_set()), this,
                SLOT(show_autoflow_run_as_failed()));
        fdiag->show();
      } else  // For non-GMP: just repeat as for GMP, but create "empty"
              // autoflowFailed record & attach to the autoflow
      {
        int autoflowFailedID = 0;
        q.clear();
        q << "new_autoflow_failed_record" << AutoflowID << "N/A"
          << "reason: non-GMP";
        ...
        if (!autoflowFailedID) {
          QMessageBox::warning(
              this, tr("AutoflowFailed Record Problem"),
              tr("autoflowFailed: There was a problem with creating a record "
                 "in autoflowFailed table \n"));
        ...
        q.clear();
        q << "update_autoflow_failedID" << AutoflowID
      ...
      // Reverting to NOT FAILED (deleting autoflowFailed record && setting
      // 'failedID' in autoflow to DEFAULT):
      q.clear();
      q << "delete_autoflow_failed_record" << AutoflowID << failedID;
      db->query(q);

      // set Run Manager's record back to black, last item to "NO"
      int last_col = ncols - 1;
      qDebug() << "Resetting Failed to NOT Failed: irow, ncols-1 -- " << irow
               << last_col;
...
void US_SelectItem::show_autoflow_run_as_failed(void) {
  int last_col = ncols - 1;
  qDebug() << "Setting as Failed: failed_run_row, ncols-1 -- " << failed_run_row
           << last_col;
