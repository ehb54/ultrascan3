...
US_SelectItem::US_SelectItem(
   QList<QStringList> &items, QStringList &hdrs, const QString titl, int *aselxP, const int def_sort) :
...
US_SelectItem::US_SelectItem(
   QList<QStringList> &items, QStringList &hdrs, const QString titl, int *aselxP, QString add_label,
   const int def_sort) : US_WidgetsDialog(0, 0), items(items), hdrs(hdrs) {
   multi_sel = false;
   deleted_button = false;
...
US_SelectItem::US_SelectItem(
   QList<QStringList> &items, QStringList &hdrs, const QString titl, QList<int> *aselxsP, const int def_sort) :
    US_WidgetsDialog(0, 0), items(items), hdrs(hdrs) {
   multi_sel = true;
   deleted_button = false;
   selxP = NULL;
...
void US_SelectItem::build_layout(const QString titl) {
   ...
   QFont le_info_font(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize());
   ...
   if (autoflow_button) {
      ...
      if (autoflow_dev) {
         le_info->setText(
            tr("Information on one or more developmental runs based on previously completed experiments is available."
               "<ul><li>You can reattach to a specific job by selecting it from the list below and clicking \"Select "
               "Development Run to Follow\"</ul></li>"
               "<ul><li>Alternatively, you can click \"Setup New Development Run\" to modify an existing analysis "
               "protocol and re-analyze data</ul></li>"
               "<ul><li>Records can be removed form the list (\"Remove from the List\"). NOTE that such runs cannot be "
               "monitored with this program</ul></li>"
               "<ul><li>Finally, selected record can be marked as \"Failed\". The program will re-initialize the run "
               "from the 3. IMPORT stage</ul></li>"));
      }
      else {
         le_info->setText(tr(
            "Information on one or more experimental methods submitted to Beckman Optima AUC Instruments is available."
            "<ul><li>You can reattach to a specific job by selecting it from the list below and clicking \"Select "
            "Optima Run to Follow\"</ul></li>"
            "<ul><li>Alternatively, you can click \"Define Another Experiment\" to design and/or submit a new "
            "experimental method to the availabale Optima instrument(s)</ul></li>"
            "<ul><li>Records can be removed form the list (\"Remove from the List\"). NOTE that such runs cannot be "
            "monitored with this program</ul></li>"
            "<ul><li>Finally, selected record can be marked as \"Failed\". The program will re-initialize the run from "
   ...
   QPushButton *pb_accept = us_pushbutton(multi_sel ? tr("Select Item(s)") : accept_pb_label);
...
void US_SelectItem::list_data() {
   ...
   for (int ii = 0; ii < nitems; ii++) { // Save column0 description in a separate list
      itemlist << items[ ii ][ 0 ];
   }
   //////////////////
   ...
   if (have_search) { // We have search text, so we must count the filtered items
      nrows = 0;
      for (int ii = 0; ii < mxrows; ii++) { // Bump therow count for each item name matching the filter
   ...
   int kk = 0;
   for (int ii = 0; ii < mxrows; ii++) { // Propagate list widget with labels
      // QString iname    = itemlist.at( ii );
      ...
      for (int jj = 1; jj < ncols; jj++) { // Set fields for remaining columns of the present row
...
void US_SelectItem::accepted() {
   ...
   if (multi_sel && selxsP != NULL) { // Possibly multiple selections:  return their indexes
      selxsP->clear();

      for (int ii = 0; ii < selitems.size(); ii++) { // Return indexes to selected items
   ...
   else if (!multi_sel && selxP != NULL) { // Return the index to the selected item
...
void US_SelectItem::deleted_autoflow() {
   ...
   if (selitems.size() == 0) {
      QMessageBox::information(
         this, tr("No Autoflow Record Selected"), tr("You have not selected any auflow record.\nSelect or Cancel"));
      return;
   }
   ...
   qDebug() << "Autoflow ID to delete: ID, name, run status: " << AutoflowID << ", " << items[ AutoflowRow ][ 1 ]
            << ", " << items[ AutoflowRow ][ 4 ];

   QMessageBox msgBox;
   msgBox.setText(
      tr("You have selected the following Record to delete:<br><br>") + tr("<b>ID:&emsp;</b>")
      + items[ AutoflowRow ][ 0 ] + tr("<br>") + tr("<b>Name:&emsp;</b>") + items[ AutoflowRow ][ 1 ] + tr("<br>")
      + tr("<b>Status:&emsp;</b> ") + items[ AutoflowRow ][ 4 ] + tr("<br><br>Proceed?"));
   msgBox.setInformativeText(
      "<font color='red'><b>NOTE:</b> if deleted, this run cannot be monitored with this program anymore!</font>");
   ...
   if (msgBox.clickedButton() == Cancel)
      ...
      for (int i = 0; i < analysisIDs_list.size(); ++i) {
         ...
         q.clear();
         q << "update_autoflow_analysis_record_at_deletion" << QString("Canceled by user, top-level") << requestID;
...
void US_SelectItem::set_unset_failed_autoflow() {
   ...
   //
   qDebug() << "Autoflow ID to set/uset as FAILED: AutoflowRow, ID, name, run status: " << AutoflowRow << ", "
            << AutoflowID << ", " << items[ AutoflowRow ][ 1 ] << ", " << items[ AutoflowRow ][ 4 ];
   ...
   QMessageBox msgBox;
   msgBox.setText(
      tr("You have selected to change the status for the following run:<br><br>") + tr("<b>ID:&emsp;</b>")
      + items[ AutoflowRow ][ 0 ] + tr("<br>") + tr("<b>Name:&emsp;</b>") + items[ AutoflowRow ][ 1 ] + tr("<br>")
      + tr("<b>Current Status:&emsp;</b> ") + current_mark
      + tr("<br><br> If proceeded, it will be marked as %1. %2").arg(new_mark).arg(add_msg) + tr("<br><br>Proceed?"));
   ...
   if (msgBox.clickedButton() == Cancel)
      ...
      if (!isFailed) {
         ...
         else // For non-GMP: just repeat as for GMP, but create "empty" autoflowFailed record & attach to the autoflow
         {
            ...
            q.clear();
            q << "update_autoflow_failedID" << AutoflowID << QString::number(autoflowFailedID);
         ...
         qDebug() << "Resetting Failed to NOT Failed: irow, ncols-1 -- " << irow << last_col;
...
void US_SelectItem::show_autoflow_run_as_failed(void) {
   int last_col = ncols - 1;
   qDebug() << "Setting as Failed: failed_run_row, ncols-1 -- " << failed_run_row << last_col;
