...
int
HandleIPCError (Display *dpy, XErrorEvent *ptr)
{
...
int
ShowInterpNames (char *inames, int mxchar)
{
  Display *dpy;
  Atom InterpAtom;
  Atom type;
  ...
  result = XGetWindowProperty (dpy, RootWindow (dpy, 0), InterpAtom, 0L, 300L,
                               False, XA_STRING, &type, &format, &len, &left,
                               &registry);
...
int
CheckInterpName (char *name, unsigned long *interpid)
{
  unsigned char *registry;
  ...
  *interpid = 0;
  handler = XSetErrorHandler (HandleIPCError);
  result = XGetWindowProperty (dpy, RootWindow (dpy, 0), InterpAtom, 0, 100000,
                               False, XA_STRING, &type, &format, &len, &left,
                               &registry);
      ...
      while (*ptr != ' ')
        {
          ...
          if (isxdigit (*ptr))
            {
              *interpid
                  = ((*interpid) << 4) | (unsigned long)digittoint (*ptr);
            }
          else
            {
              break;
...
int
SendCommand (long unsigned int interpid, char *interp, char *command)
{
  static int Serial = 0;
  Display *dpy;
  Atom CommAtom;
  ...
  XChangeProperty (dpy, commWindow, CommAtom, XA_STRING, 8, PropModeAppend,
...
int
SendQuery (long unsigned int interpid, char *interp, char *command,
           char *response, int maxrch)
{
  ...
  if (result != 0)
    { /* If the send had an error, return an error message */
  ...
  result
      = XGetWindowProperty (dpy, MainWin, CommAtom, 0, 1024, True, XA_STRING,
                            &type, &format, &len, &left, &propstr);
  ...
  if (ptr != NULL)
    fprintf (stderr, "SQ: r0-8 %x %x %x %x %x %x %x %x\n", ptr[0], ptr[1],
             ptr[2], ptr[3], ptr[4], ptr[5], ptr[6], ptr[7]);
  fprintf (stderr, "SQ: r0-8 %c %c %c %c %c %c %c %c\n", ptr[0], ptr[1],
           ptr[2], ptr[3], ptr[4], ptr[5], ptr[6], ptr[7]);
  ...
  if ((result != Success) || (type != XA_STRING) || (format != 8))
    { /* Return immediately if error or unexpected type/format */
      if (command != NULL)
        XFree (ptr);
      return result;
    }
  ...
  /* Parse the response */
  if (ptr[0] == '\0')
    { /* Response is Tcl/Tk4 type */
      ptr++;

      while (ptr < ptr_last)
        { /* Walk through response characters */

          if (ptr[0] == 'r' && ptr[1] == '\0')
            { // Handle a response string
              ptr += 2;
