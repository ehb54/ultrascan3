...
char* GetLastError() { return ErrorBuffer() + 1; }

/* Return the last IPC error code */
int GetErrorCode() { return (int)ErrorBuffer()[0]; }
...
int ShowInterpNames(char* inames, int mxchar) {
  ...
  result =
      XGetWindowProperty(dpy, RootWindow(dpy, 0), InterpAtom, 0L, 300L, False,
                         XA_STRING, &type, &format, &len, &left, &registry);
...
int CheckInterpName(char* name, unsigned long* interpid) {
  ...
  *interpid = 0;
  handler = XSetErrorHandler(HandleIPCError);
  result =
      XGetWindowProperty(dpy, RootWindow(dpy, 0), InterpAtom, 0, 100000, False,
                         XA_STRING, &type, &format, &len, &left, &registry);
...
int SendCommand(long unsigned int interpid, char* interp, char* command) {
  ...
  if (interpid == 0L) CheckInterpName(interp, &interpid);
  ...
  jj = 6;
  len = strlen(interp);
  for (ii = 0; ii < len; ii++) buffer[jj++] = interp[ii];
  buffer[jj++] = '\0';
  ...
  buffer[jj++] = ' ';
  len = strlen(command);
  for (ii = 0; ii < len; ii++) buffer[jj++] = command[ii];
  buffer[jj++] = '\0';
  len = jj;
  ClearLastError();
...
int SendQuery(long unsigned int interpid, char* interp, char* command,
  ...
  result = XGetWindowProperty(dpy, MainWin, CommAtom, 0, 1024, True, XA_STRING,
  ...
  if ((result != Success) || (type != XA_STRING) ||
      (format !=
       8)) { /* Return immediately if error or unexpected type/format */
    if (command != NULL) XFree(ptr);
    return result;
  }
