                      ...
                      )
{
   ...
   {
      cerr 
         << "Warning: maximum dimension requested " << max_dimension 
   ...
   if ( use_errors )
   {
      ...
      {
         cout << "Notice: SD's provided but some were zero, so SD fitting is turned off" << endl;
      }
   }
...
bool PM_WORKER::create_model( vector < double > params, set < pm_point > & model, bool only_last_model )
{
   model.clear();
...
bool PM_WORKER::rotation_matrix( double l, double m, double n, double theta, vector < vector < double > > &rm )
{
   rm.clear();
   rm.resize( 3 );
   rm[ 0 ].resize( 3 );
...
bool PM_WORKER::apply_rotation_matrix( vector < vector < double > > &rm, int x, int y, int z, double & newx, double & newy, double & newz )
{
...
bool PM_WORKER::split( vector < double > & params, vector < int > & types, vector < double > & fparams )
{
   types  .clear();
   fparams.clear();
...
bool PM_WORKER::join( vector < double > & params, vector < int > & types, vector < double > & fparams )
{
   params.clear();
