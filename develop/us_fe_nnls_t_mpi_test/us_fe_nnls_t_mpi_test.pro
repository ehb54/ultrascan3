...
unix:contains (UNAME, ia64) {
	DEFINES += ITANIUM
	message ("Configuring for the Intel Itanium Platform...")
}
...
DEPENDPATH      +=../src \
                  ../include
SOURCES		+= main.cpp
SOURCES		+= ../src/us_fe_nnls_t_mpi_test.cpp
...
unix:contains (UNAME,x86_64) {
	TARGET		= ../../bin64/us_fe_nnls_t_mpi_test
        DEFINES         += BIN64
} else {
	TARGET		= ../../bin/us_fe_nnls_t_mpi_test
}


unix:contains(UNAME,x86_64) {
    LIBS +=  -L$(ULTRASCAN)/lib
}
