...
unix:DEFINES   += UNIX
win32:DEFINES  += WIN32
INCLUDEPATH    = $(QWTDIR)/include $(QWT3DDIR)/include $(QTDIR)/include \
               $(ADOLCDIR) \
               $(ADOLCDIR)/drivers \
               $(ADOLCDIR)/sparse \
               $(ADOLCDIR)/tapedoc \
...
DEPENDPATH      += ../src \
                   ../include
SOURCES		+= main.cpp
HEADERS		= ../include/us_db_rst_equilproject.h

unix:contains (UNAME,x86_64) {
	TARGET		= ../../bin64/us_db_rst_equilproject
} else {
	TARGET		= ../../bin/us_db_rst_equilproject
}
