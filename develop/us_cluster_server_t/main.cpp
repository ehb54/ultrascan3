...
class US_Cluster_Server_Config
{
 public:
   ...
   bool init( QString configfile ) {
      ...
      if ( ts.atEnd() )
      {
         errormsg = QString( "Error: %1 line %2: premature end of file" ).arg( configfile ).arg( line );
         return false;
      }
      ...
      {
         errormsg = QString( "Error: %1 line %2: port must be between 1 and 65535, was %3" ).arg( configfile ).arg( line ).arg( port );
      }
...
class GetRequest : public QHttp
{
   ...
   GetRequest( QString sendtext, QObject *parent=0, const char *name=0 ) : QHttp( parent, name ) 
   {
      // connect( this, SIGNAL( stateChanged ( int ) ), SLOT( http_stateChanged ( int ) ) );
      // connect( this, SIGNAL( responseHeaderReceived ( const QHttpResponseHeader & ) ),SLOT( http_responseHeaderReceived ( const QHttpResponseHeader & ) ) );
      connect( this, SIGNAL( readyRead ( const QHttpResponseHeader & ) ), SLOT( http_readyRead ( const QHttpResponseHeader & ) ) );
      // connect( this, SIGNAL( dataSendProgress ( int, int ) ), SLOT( http_dataSendProgress ( int, int ) ) );
      // connect( this, SIGNAL( dataReadProgress ( int, int ) ), SLOT( http_dataReadProgress ( int, int ) ) );
      // connect( this, SIGNAL( requestStarted ( int ) ), SLOT( http_requestStarted ( int ) ) );
      // connect( this, SIGNAL( requestFinished ( int, bool ) ),SLOT( http_requestFinished ( int, bool ) ) );
   ...
   void http_done ( bool /* error */ )
   {
      ...
      // disconnect( this, SIGNAL( responseHeaderReceived ( const QHttpResponseHeader & ) ), 0, 0 );
      disconnect( this, SIGNAL( readyRead ( const QHttpResponseHeader & ) ), 0, 0 );
   ...
   {
      std::cout << "http_responseHeaderReceived: " << resp.reasonPhrase() << std::endl;
   }
...
class SimpleServer : public QServerSocket
{
   Q_OBJECT
public:
   SimpleServer( unsigned int port, QObject* parent=0 ) : QServerSocket( port, 1, parent )
   {
      if ( !ok() ) {
...
public:
   ServerInfo( unsigned int port )
   {
     ...
     QObject::connect( server, SIGNAL(newConnect(ClientSocket*)), SLOT(newConnect(ClientSocket*)) );
