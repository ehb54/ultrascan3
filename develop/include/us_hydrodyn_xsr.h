...
#define GAUSSIAN(value,mean,stdDev) (1.0/sqrt(2.0*PI*(stdDev)*(stdDev))*exp(-((value) - (mean))*((value) - (mean))/(2.0*(stdDev)*(stdDev))))
...
typedef struct{
   ...
   double q[NPEXP], Iq[NPEXP], sigIq[NPEXP], *sigq, *qbar, *sF, A, L, *slit, *qSlit, **resFunction, **qResFunction;
...
int getScaleBackground(calc_t *model, exp_t *data);   /* determines scale and background corrections for calculated scattering profiles */
...
int updatePrGaussian(double *pr, double mean, int bin, double nSymmOps);  /* smears the contribution to the pr according to a gaussian distribution */

int getPartialPr(pr_t **pr, grid_t *grid, grid_t *point);   /* calculates the contribution to the pr of a single grid element */
