...
class US_MovingFEM : public QObject
{
	Q_OBJECT

	public:

		US_MovingFEM(struct mfem_data *, bool, QObject * parent=0, const char * name=0);
		US_MovingFEM(int, double, double, double, double, double, double, double, 
		             struct mfem_data *, bool, struct mfem_initial *, QObject * parent=0, const char * name=0 );
		~US_MovingFEM();
	...
	signals:
		...
		void scan_updated(struct mfem_scan, std::vector <double>);
#endif
};
