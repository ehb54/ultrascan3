...
void calc_vbar(struct peptide *pep, QString *sequence, float *temperature)
{
   ...
   pep->weight = 0;
   pep->e280 = 0;
   pep->r = sequence->contains("r", false);
   pep->l = sequence->contains("l", false);
   pep->k = sequence->contains("k", false);
   pep->a = sequence->contains("a", false);
   pep->v = sequence->contains("v", false);
   pep->g = sequence->contains("g", false);
   pep->e = sequence->contains("e", false);
   pep->n = sequence->contains("n", false);
   pep->i = sequence->contains("i", false);
   pep->j = sequence->contains("j", false);
   pep->o = sequence->contains("o", false);
   pep->q = sequence->contains("q", false);
   pep->y = sequence->contains("y", false);
   pep->p = sequence->contains("p", false);
   pep->s = sequence->contains("s", false);
   pep->h = sequence->contains("h", false);
   pep->t = sequence->contains("t", false);
   pep->d = sequence->contains("d", false);
   pep->f = sequence->contains("f", false);
   pep->b = sequence->contains("b", false);
   pep->c = sequence->contains("c", false);
   pep->m = sequence->contains("m", false);
