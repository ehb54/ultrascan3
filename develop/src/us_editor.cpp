...
US_Editor::US_Editor(int flag, QWidget * parent , const char * name )
	: QFrame( parent, name /*, WStyle_Customize | WStyle_MinMax  | WStyle_SysMenu*/)
{
	if (flag == 0)
	{
...
void US_Editor::load(const QString &fileName)
{
	QFile f(fileName);
	e->clear();
...
{
    if ( tabWidget->currentPage() && tabWidget->currentPage()->inherits( "QTextEdit" ) )
	{
...
void TextEdit::fileSaveAs()
{
    if ( !currentEditor() )
	return;
    QString fn = QFileDialog::getSaveFileName( QString::null, tr( "All Files (*)" ), this );
    if ( !fn.isEmpty() ) {
...
void TextEdit::fileSaveDB()
{
	...
	QSqlQuery query;
	str="UPDATE tblRequest SET Notes = '"+notes+"' WHERE RequestID ="+QString::number(tableID);
