...
#include "../include/us_fefitter_ad.h"

US_FeFitter_AD::US_FeFitter_AD(	bool *temp_fitting_widget, 
											bool temp_GUI, 
																						...
																						name)
{
	...
	if (GUI)
	{
		...
		//residuals_plot->setPlotBackground(USglobal->global_colors.plot);		//old version
...
US_FeFitter_AD::~US_FeFitter_AD()
{
	cleanup();
}

void US_FeFitter_AD::assign_orig()
{
	veloc_par.sed.clear();
...
void US_FeFitter_AD::resizeEvent(QResizeEvent *e)
{
	if (!GUI)
	{
		return;
...
int US_FeFitter_AD::calc_jacobian()
{
	unsigned int i, j, k, count;
	double **identity, **jac;
	jac = new double * [all_sim_points];
	identity = new double *	[parameters];
	...
	for (i=0; i<parameters; i++)
	{
		identity[i] = new double [parameters];
		for (j=0; j<parameters; j++)
		{
			if (j == i)
			{
	...
	forward(tag, (int) all_sim_points, (int) parameters, (int) parameters, guess, identity, y_guess, jac);
...
bool US_FeFitter_AD::fit_init()
{
	...
	*/
	parameters = 0;
	adParameterIndex.conc.clear();
	adParameterIndex.sed.clear();
	adParameterIndex.diff.clear();
	adParameterIndex.sigma.clear();
	adParameterIndex.delta.clear();
	adParameterIndex.vbar.clear();
	...
	{															// list which keeps track of the indices of each
		...
		{
			adParameterIndex.conc.push_back(parameters);
			parameters += 1;
		}
		else
		{
			adParameterIndex.conc.push_back(-1);
		}
		if ((*component_vector)[i].sed_fit)
		{
			adParameterIndex.sed.push_back(parameters);
			parameters += 1;
		}
		else
		{
			adParameterIndex.sed.push_back(-1);
		}
		if ((*component_vector)[i].diff_fit)
		{
			adParameterIndex.diff.push_back(parameters);
			parameters += 1;
		}
		else
		{
			adParameterIndex.diff.push_back(-1);
		}
		if ((*component_vector)[i].sigma_fit)
		{
			adParameterIndex.sigma.push_back(parameters);
			parameters += 1;
		}
		else
		{
			adParameterIndex.sigma.push_back(-1);
		}
		if ((*component_vector)[i].delta_fit)
		{
			adParameterIndex.delta.push_back(parameters);
			parameters += 1;
		}
		else
		{
			adParameterIndex.delta.push_back(-1);
		}
		if ((*component_vector)[i].vbar_fit)
		{
			adParameterIndex.vbar.push_back(parameters);
			parameters += 1;
		}
		else
		{
			adParameterIndex.vbar.push_back(-1);
		}
	}
	if ((*runData).baseline_fit)
	{
		adParameterIndex.baseline = parameters;
		parameters += 1;
	}
	else
	{
		adParameterIndex.baseline = -1;
	}
	if ((*runData).slope_fit)
	{
		adParameterIndex.slope = parameters;
		parameters += 1;
	}
	else
	{
		adParameterIndex.slope = -1;
	}
	if ((*runData).stray_fit)
	{
		adParameterIndex.stray = parameters;
		parameters += 1;
	}
	else
	{
		adParameterIndex.stray = -1;
	}
...
void US_FeFitter_AD::parameter_map_to_guess()
{
	unsigned int i, j=0;
...
void US_FeFitter_AD::parameter_map_to_component_vector(double *par)
{
	unsigned int i, j=0;
	for (i=0; i<(*component_vector).size(); i++)
	{
...
void US_FeFitter_AD::check_constraints(double *par)
{
	unsigned int i, j=0;
	for (i=0; i<(*component_vector).size(); i++)
	{
...
int US_FeFitter_AD::calc_model(double *par)
{
	QString str;
	tag=0;
	bool ldu_flag = true;
	...
	if (tape_construct)
	{
		if (GUI)
		{
			lbl_status4->setText(tr("Writing Automatic Differentiation Tape..."));
		}
		trace_on(tag);
		pgb_progress->reset();
		pgb_progress->setTotalSteps(10+components);
	}
...
//	cout << "time: " << (*run_inf).time[cell][lambda][(*run_inf).scans[cell][lambda] - 1]<<", Time steps: " <<time_steps << ", rpm: " << (*run_inf).rpm[cell][0][0]<<endl;
	init_finite_element();
	concentration = new adouble *[(*run_inf).scans[cell][lambda]];
	for (i=0; i<(*run_inf).scans[cell][lambda]; i++)
	{
		concentration[i] = new adouble [sim_points];
	}
	for (i=0; i<(*run_inf).scans[cell][lambda]; i++)
	{
...
//	cout << endl;
	if (conc_dep_flag)
	{
		c_current = new adouble [sim_points];
	}
	adouble *temp_val;
	temp_val = new adouble [2];
	right = new adouble [sim_points];
	right_old = new adouble [sim_points];
	left = new adouble* [sim_points];
	left_temp = new adouble* [sim_points];
	for (i=0; i<sim_points; i++)
	{
		left[i] = new adouble [3];
		left_temp[i] = new adouble [3];
	}
	for (k=0; k<components; k++)
	{
		...
		if (GUI)
		{
			if (tape_construct)
			{
				pgb_progress->setProgress(k+1);
				qApp->processEvents();
			}
			lbl_status2->setText(tr("Fitting Finite Element Solution, Component ") + str.sprintf("%d", k+1));
		}
		scan_timer=0.0;
		if (adParameterIndex.sed[k] == -1)
		{
			temp_val[0] = runData->delta_t * (*component_vector)[k].sed * omega_s;
		}
		else
		{
			temp_val[0] = runData->delta_t * ad_par[adParameterIndex.sed[k]] * omega_s;
		}
		if (adParameterIndex.diff[k] == -1)
		{
			temp_val[1] = runData->delta_t * (*component_vector)[k].diff;
		}
		else
		{
			temp_val[1] = runData->delta_t * ad_par[adParameterIndex.diff[k]];
		}
		if (adParameterIndex.conc[k] == -1)
		{
			for (i=0; i<sim_points; i++)
			{
		...
		else
		{
			for (i=0; i<sim_points; i++)
			{
				right[i] = ad_par[adParameterIndex.conc[k]];
				for (j=0; j<3; j++)
				{
					left[i][j] = temp_val[1] * a1[i][j] - temp_val[0] * a2[i][j];
				}
		...
		for (j=0; j<time_steps; j++)
		{
			...
			{
				if (adParameterIndex.sigma[k] == -1)
				{
					conc_dep_s((*component_vector)[k].sigma, &right, &c_current, &a2, temp_val[0], sim_points);
				}
				else
				{
					conc_dep_s(ad_par[adParameterIndex.sigma[k]], &right, &c_current, &a2, temp_val[0], sim_points);
				}
			}
			if ((*component_vector)[k].delta != 0)
			{
				conc_dep_d((*component_vector)[k].delta, &right, &c_current, &u, &v, &w, temp_val[1], sim_points,
				&cu, &cv, &cw);
			}
...
// before exiting this function, add the baseline
	...
	if (adParameterIndex.baseline == -1)
	{
		for (i=0; i<(*run_inf).scans[cell][lambda]; i++)
		{
			for (j=0; j<sim_points; j++)
	...
	else
	{
		for (i=0; i<(*run_inf).scans[cell][lambda]; i++)
		{
			for (j=0; j<sim_points; j++)
			{
				concentration[i][j] += ad_par[adParameterIndex.baseline];
			}
		}
	}
	if (adParameterIndex.slope == -1)
	{
		for (i=0; i<(*run_inf).scans[cell][lambda]; i++)
		{
			for (j=0; j<sim_points; j++)
	...
	else
	{
		for (i=0; i<(*run_inf).scans[cell][lambda]; i++)
		{
			for (j=0; j<sim_points; j++)
			{
				concentration[i][j] += ad_par[adParameterIndex.slope] * (sim_radius[j] - (*run_inf).range_left[cell][lambda][0]);
			}
		}
	}
	if (adParameterIndex.stray == -1)
	{
		for (i=0; i<(*run_inf).scans[cell][lambda]; i++)
		{
			for (j=0; j<sim_points; j++)
	...
	else
	{
		for (i=0; i<(*run_inf).scans[cell][lambda]; i++)
		{
			for (j=0; j<sim_points; j++)
			{
				concentration[i][j] += ad_par[adParameterIndex.stray] * concentration[i][j] * (sim_radius[j] - (*run_inf).range_left[cell][lambda][0]);
			}
		}
	}
	count=0;
	for (unsigned int i=0; i<(*run_inf).scans[cell][lambda]; i++)
	{
		...
		{
			concentration[i][j] >>= y_guess[count];
			count ++;
		}
	}
	cleanup_finite_element();
	...
	{
		trace_off();
		tape_construct = false;
		tapestats(tag, tape_stats);
		if (GUI)
		{
			lbl_status4->setText(tr("Writing Tape to Disk..."));
		}
		for (i=0; i<10; i++)
		{
			if (GUI)
			{
				pgb_progress->setProgress(components + i + 1);
				qApp->processEvents();
			}
			system("sleep 1");
		}
		if (GUI)
		{
			lbl_status4->setText(tr("Finished writing Automatic Differentiation Tape..."));
		}
/*
		for (unsigned int i=0; i<11; i++)
		{
			cout << "Tapestats[" << i << "]: " << tape_stats[i] << endl;
		} 
*/
	}
...
void US_FeFitter_AD::cleanup_finite_element()
{
	unsigned int i;
	delete [] ad_par;
	for (unsigned int i=0; i<(*run_inf).scans[cell][lambda]; i++)
	...
	if (conc_dep_flag) //we got concentration dependency, de-allocate memory
	{ 
		delete [] cu; 
		delete [] cv;
		delete [] cw;
...
void US_FeFitter_AD::init_finite_element()
{
	unsigned int i, j;
	init_simulation = true;
	if (model == 4 || model == 20)
	{
		s_bar = new adouble [sim_points];
		D_bar = new adouble [sim_points];
		partial_c = new adouble* [2];
		partial_c[0] = new adouble [sim_points];
		partial_c[1] = new adouble [sim_points];
	}

	if (conc_dep_flag) //we got concentration dependency, allocate memory and initialize:
	{
//cout << "Initializing with sim_points: " << sim_points << endl;
		cu = new adouble [sim_points]; 
		cv = new adouble [sim_points];
		cw = new adouble [sim_points];
		...
		{			
			v[i][0] = -(sim_radius)[i-1]/(2.0*runData->delta_r)-1.0/6.0;
		}
		v[0][1] = sim_radius[0]/(2.0*runData->delta_r)+1.0/6.0;
		for (i=1; i<(unsigned int) (sim_points-1); i++)
		{
			v[i][1] = sim_radius[i]/runData->delta_r;
		}
		v[sim_points-1][1] = sim_radius[sim_points-1]/(2.0*runData->delta_r)-1.0/6.0;
		for (i=0; i< (unsigned int) sim_points-1; i++)
		{
			v[i][2] = -(sim_radius)[i]/(2.0*runData->delta_r)-1.0/3.0;
		}
...
void US_FeFitter_AD::view_report()
{
	if(!GUI)
	{
		return;
...
void US_FeFitter_AD::write_report()
{
	if(!GUI)
	{
		return;
...
bool US_FeFitter_AD::createHtmlDir()
{
...
void US_FeFitter_AD::saveFit(const QString &filename)
{
...
float US_FeFitter_AD::calc_residuals()
{
