...
nsa_sga_individual nsa_sga_individual::random( unsigned int size /* in char's */ )
{
...
bool US_Saxs_Util::nsa_sga( double & nrmsd )
{
   ...
   {
      errormsg = "Error: nsa_sga requires nsasgaincrement to be greater than zero";
      return false;
   }
   ...
   for ( unsigned int g = 0; g < control_parameters[ "nsagenerations" ].toUInt(); g++ )
   {
      ...
      for ( unsigned int i = 0; i < control_parameters[ "nsapopulation" ].toUInt(); i++ )
      {
         ...
         {
            // cout << "mutate\n";
            nsa_sga_individual individual = last_pop[ nsa_pop_selection( last_pop.size() ) ];
              ...
              drand48() < control_parameters[ "nsacrossover" ].toDouble() )
         {
            // cout << "crossover\n";
            nsa_sga_individual individual  = last_pop[ nsa_pop_selection( last_pop.size() ) ];
            nsa_sga_individual individual2 = last_pop[ nsa_pop_selection( last_pop.size() ) ];
            unsigned int pos  = ( unsigned int )( drand48() * ( double ) individual .v.size() );
            unsigned int pos2 = ( unsigned int )( drand48() * ( double ) individual2.v.size() );
         ...
         {
            nsa_sga_individual individual = nsa_sga_individual::random( last_pop[ pos ].v.size() );
            individual.fitness    = nsa_sga_fitness( individual );
            nsa_pop.push_back( individual );
            random_count++;
         } else {
