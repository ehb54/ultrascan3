...
void US_Hydrodyn_Bead_Output::setupGUI()
{
   ...
   cb_beams_output->setPalette( PALET_NORMAL );
   AUTFBACK( cb_beams_output );
   connect(cb_beams_output, SIGNAL(clicked()), this, SLOT(select_output_beams()));
   ...
   rb_pdb_sequence->setFont(QFont( USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
   ...
   rb_chain_sequence->setText(us_tr(" exposed side chain -> exposed main chain -> buried "));
   rb_chain_sequence->setEnabled(true);
   rb_chain_sequence->setMinimumHeight(minHeight1);
   rb_chain_sequence->setFont(QFont( USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
   ...
   case 0 : rb_pdb_sequence->setChecked( true ); break;
...
void US_Hydrodyn_Bead_Output::select_output_somo() {
   select_output( 0 );
}

void US_Hydrodyn_Bead_Output::select_output_beams() {
   select_output( 1 );
}
