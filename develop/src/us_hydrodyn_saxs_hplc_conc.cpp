                                                           ...
                                                           ) : QFrame(p, name)
{
   ...
   setCaption( tr( "US-SOMO: SAXS Hplc: File Concentrations"));
...
void US_Hydrodyn_Saxs_Hplc_Conc::cancel()
{
   close();
}
...
void US_Hydrodyn_Saxs_Hplc_Conc::help()
{
   ...
   online_help->show_help("manual/somo_saxs_hplc_conc.html");
}
...
void US_Hydrodyn_Saxs_Hplc_Conc::load()
{
   ...
   if ( *(((US_Hydrodyn_Saxs_Hplc *)saxs_hplc_window)->saxs_widget) )
   {
      ((US_Hydrodyn_Saxs_Hplc *)saxs_hplc_window)->saxs_window->select_from_directory_history( use_dir, this );
      raise();
   }
   ...
   if ( !QFile::exists( fname ) )
   {
      QMessageBox::warning( this, 
                            tr( "US-SOMO: SAXS Hplc Subtraction Utility: Concentrations" ),
   ...
   if ( QFileInfo( fname ).extension( false ).contains( QRegExp( "^(sbc|SBC)$" ) ) )
   {
      ...
      US_Hydrodyn_Saxs_Hplc_Conc_Load *hbscl = 
         new US_Hydrodyn_Saxs_Hplc_Conc_Load(
                                               qsl_lines,
                                               csv1,
                                               this );
      if ( hbscl->disp_csv.data.size() )
      {
         ...
         QMessageBox::warning( this, 
                               tr( "US-SOMO: SAXS Hplc: File Concentrations"),
...
void US_Hydrodyn_Saxs_Hplc_Conc::save()
{
   ...
   if ( ((US_Hydrodyn_Saxs_Hplc *)saxs_hplc_window)->saxs_widget )
   {
      ((US_Hydrodyn_Saxs_Hplc *)saxs_hplc_window)->saxs_window->add_to_directory_history( fname );
   }
