...
US_FeMatchRa_W::US_FeMatchRa_W(QWidget *p, const char *name) : Data_Control_W(1, p, name)
{
...
void US_FeMatchRa_W::second_plot(int val)
{
	if (resplot == NULL)
	{
		resplot = new US_ResidualPlot(0,0);
...
void US_FeMatchRa_W::update_component(double val)
{
...
void US_FeMatchRa_W::update_s(const QString &str)
{
	s20w[current_component-1] = str.toDouble();
}

void US_FeMatchRa_W::update_D(const QString &str)
{
	D20w[current_component-1] = str.toDouble();
}

void US_FeMatchRa_W::update_C(const QString &str)
{
	partial_concentration[current_component-1] = str.toDouble();
}

void US_FeMatchRa_W::update_baseline(const QString &str)
{
	cout << "updating baseline...\n";
	baseline = str.toFloat();
}

void US_FeMatchRa_W::update_labels()
{
...
void fematch_ra_thr_t::fematch_ra_thr_setup(QProgressBar *a_progress,
...
//	    printf("z_exp.scan %d %lx %lx\n", j, &(z_experiment[j].scan), &(experiment.scan));
	  ...
	  for(j = 0; j < threads; j++) {
	    ...
	    fematch_ra_thr_threads[j]->fematch_ra_thr_setup(progress,
						      &fem_data[j],
						      &z_experiment[j],
						      &z_residuals[j],
						      &initCVector,
