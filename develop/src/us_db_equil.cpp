...
US_DB_Equil::US_DB_Equil(QWidget *p, const char *name) : US_DB(p, name)
{
	exp_rst.expRstID = -1;
	exp_rst.invID = -1;
	from_HD = false;
...
void US_DB_Equil::sel_runrequest()
{
...
void US_DB_Equil::load_HD()
{
	...
	if(htmlDir !="")
	{
		...
		report_equil = new US_Report_Equil(false);
...
*/
void US_DB_Equil::sel_investigator()
{
...
//Insert Compress data into tblEquilResult and tblEquilResultData
...
bool US_DB_Equil::insertCompressData()
{
...
void US_DB_Equil::endCompressProcess()
{
	QDir work_dir;
	...
	switch(c_step)
	{
		case 0:			//tar report tar file	
		{
         ...
         compress_proc->setWorkingDirectory(work_dir);
			...
			{
				QMessageBox::message("UltraScan Error:", "Unable to start process to tar equilibrium report data archive.");
				return;
			}
			break;
		...
		case 1:			//gzip report tar file	
		{
         ...
         compress_proc->setWorkingDirectory(work_dir);
			...
			if(!compress_proc->start())
			{
				QMessageBox::message("UltraScan Error:", "Unable to start process to gzip equilibrium report data archive.");
				return;
			}
			break;
...
/*		
		case 2:			//tar raw data tar file 
		{
         ...
         compress_proc->setWorkingDirectory(work_dir);
			...
			if(!compress_proc->start())
			{
				QMessageBox::message("UltraScan Error:", "Unable to start process to tar equil result archive.");
				return;
			}
			break;
		}
		case 3:			//gzip raw data tar file 
		{
         ...
         compress_proc->setWorkingDirectory(work_dir);
			...
			if(!compress_proc->start())
			{
				QMessageBox::message("UltraScan Error:", "Unable to start process to gzip equil result archive.");
				return;
			}
			break;
		}
*/
		...
		case 6:	//insert to DB table tblEquilResultData
		{
			db_connect();
			QSqlCursor cur("tblEquilResultData");
	...
	//		write_blob(fileName, buffer, "Rawdata_tar");
			...
			if(!compress_proc->start())
			{
				QMessageBox::message("UltraScan Error:", "Unable insert data to DB table tblEquilResultData.");
				return;
			}
			break;
		}
		case 7: //Insert data to DB table tblEquilResult
		{
			QString STR = "INSERT INTO tblEquilResult (EquilRstID, Date, RunID, InvestigatorID, DataID";
      	if(runrequestID > 0)
      	{
         	STR += ", RunRequestID";
      	}
      	...
      	if(!finished)
      	{
         	...
         	QMessageBox::message(tr("Attention:"), 
				...
				QSqlQuery del_data;
				str.sprintf("DELETE FROM tblEquilResultData WHERE tableID = %d;", DataID);
				del_data.exec(str);
			...
			QMessageBox::message(tr("Congratulation:"), tr("Sucessfully save data to database.\n"));						
			delete compress_proc;
		}//end case 7
	}
}

void US_DB_Equil::cleanCompressFile()
{
...
void US_DB_Equil::query_db()
{
	...
	if(exp_rst.invID>0)
	{
		str.sprintf("SELECT EquilRstID, RunID FROM tblEquilResult WHERE InvestigatorID = %d;", exp_rst.invID);
	}
	else
	{
		str.sprintf("SELECT EquilRstID, RunID FROM tblEquilResult;");
	}
	QSqlQuery query(str);
	if(query.isActive())
	{
...
void US_DB_Equil::endUncompressProcess()
{
...
*/
void US_DB_Equil::check_permission()
{
