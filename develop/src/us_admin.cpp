...
US_Admin::US_Admin(QWidget *p, const char *name) : QFrame( p, name)
{
	...
	lbl_psswd1->setFont(QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1, QFont::Bold));
	...
	le_psswd1->setFont(QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1, QFont::Bold));
	le_psswd1->setEchoMode(QLineEdit::Password);
	le_psswd1->setMinimumHeight(buttonh);
	connect (le_psswd1, SIGNAL(textChanged(const QString &)), SLOT(update_psswd1(const QString &)));
	...
	lbl_psswd2->setFont(QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1, QFont::Bold));
	...
	le_psswd2->setFont(QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1, QFont::Bold));
	le_psswd2->setMinimumHeight(buttonh);
	le_psswd2->setEchoMode(QLineEdit::Password);
	connect (le_psswd2, SIGNAL(textChanged(const QString &)), SLOT(update_psswd2(const QString &)));
...
void US_Admin::save()
{
	if(QString::compare(password1, password2) != 0 )
	{
		...
		QMessageBox::message(tr("Attention:"), 
									tr("The entered two password is not same, please type again.\n"));
		return;
	}
	...
	if(!pdfile.isEmpty())
	{
		...
		{
			QMessageBox::message(tr("Attention:"), 
									tr("You have no permission to change adminstrator password.\n"));
			return;
		}
	}
	else
	{															
		QMessageBox::message(tr("Attention:"), 
									tr("Can not create file for saving your new password.\n"));
		return;				
