...
void US_Hydrodyn_Saxs_Hplc::setupGUI()
{
   ...
   le_gauss_pos_dist1->setPalette(QPalette(USglobal->global_colors.cg_normal, USglobal->global_colors.cg_normal, USglobal->global_colors.cg_normal));
   ...
   le_gauss_pos_dist2->setPalette(QPalette(USglobal->global_colors.cg_normal, USglobal->global_colors.cg_normal, USglobal->global_colors.cg_normal));
...
void US_Hydrodyn_Saxs_Hplc::gauss_new()
{
   ...
   gaussians.push_back( use_max_height / 2 );
...
void US_Hydrodyn_Saxs_Hplc::gauss_pos_dist1_focus( bool hasFocus )
{
   ...
   if ( hasFocus )
   {
      ...
      update_gauss_pos();
      // fix for gauss dist
      qwtw_wheel->setRange( -100e0,
                            100e0,
                            201e0 / UHSH_WHEEL_RES );
...
/*
  ...
  // fix for gauss dist
  for ( unsigned int i = 0; i < lm_fit_gauss_size; i += gaussian_type_size )
  {
...
void US_Hydrodyn_Saxs_Hplc::update_gauss_mode()
{
   ...
   int percharwidth = 1 + ( 7 * ( USglobal->config_list.fontSize - 1 ) / 10 );
   pb_gauss_start->setMaximumWidth( percharwidth * ( pb_gauss_start->text().length() + 2 ) );
}
