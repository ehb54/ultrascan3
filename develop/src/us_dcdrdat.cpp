...
dcdr_dat_W::dcdr_dat_W(QWidget *p, const char *name) : Data_Control_W(1, p, name)
{
	baseline_flag = true;
	bd_range=90;
	bd_position = 5;
	...
	pb_second_plot->setText(tr("Avg. dC/dr Plot"));
	xpos = border;
	ypos = 438;
	setup_already = false;
	pm = new US_Pixmap();
	...
	sval_counter->setRange(1, 1000, 1);
	sval_counter->setValue(50);
...
dcdr_dat_W::~dcdr_dat_W()
{
}

void dcdr_dat_W::setup_GUI()
{
	int j=0;
...
void dcdr_dat_W::update_s_button()
{
	if (step == 0)
	{
...
void dcdr_dat_W::update_r_button()
{
	if (step == 0)
	{
		QMessageBox::message(tr("Attention:\n"),tr("You need to load some data first!\n\n	   Click on \"Load Data\"\n"));
...
void dcdr_dat_W::second_plot()
{
	...
	int temp_scans = run_inf.scans[selected_cell][selected_lambda] - exclude;
	avg = new US_Average(points-1, temp_scans, &sval, &dcdr_points, "dcdr", htmlDir, title, selected_cell, selected_lambda);
	avg->setCaption(tr("Radial Derivative (dC/dr): Average S Distribution:"));
	avg->show();
}

void dcdr_dat_W::save()
{
	if (step == 0)
	{
		QMessageBox::message(tr("Attention:\n"),tr("You need to load some data first!\n\n	   Click on \"Load Data\"\n"));
	...
	QPixmap p;
	QString fileName;
	fileName.sprintf(htmlDir + "/dcdr_%d%d.", selected_cell + 1, selected_lambda +1);
	p = QPixmap::grabWidget(analysis_plot, 2, 2, analysis_plot->width() - 4, analysis_plot->height() - 4);
	pm->save_file(fileName, p);
	fileName.sprintf(htmlDir + "/dcdr_edited_%d%d.", selected_cell + 1, selected_lambda +1);
	p = QPixmap::grabWidget(edit_plot, 2, 2, edit_plot->width() - 4, edit_plot->height() - 4);
	pm->save_file(fileName, p);
}

void dcdr_dat_W::help()
{
	US_Help *online_help; online_help = new US_Help(this);
	online_help->show_help("manual/dcdr.html");
}

int dcdr_dat_W::plot_analysis()
{
	double s_step;
	already = true;
	if (setup_already)
	...
	analysis_plot->clear();
	title.sprintf(tr("Run ") + run_inf.run_id + tr(": Cell %d (%d nm) - Radial Derivative (dC/dr)"), 
	...
	for (j=exclude; j<run_inf.scans[selected_cell][selected_lambda]; j++)
	{
		for (i=1; i<points; i++)
		{
//
...
//
//			dcdr_points[j-exclude][i-1] = (absorbance[j][i]
//			/ run_inf.plateau[selected_cell][selected_lambda][j])
//			- (absorbance[j][i-1] / run_inf.plateau[selected_cell][selected_lambda][j]);
			dcdr_points[j-exclude][i-1] = (absorbance[j][i] - absorbance[j][i-1]) / (radius[i] - radius[i-1]);//run_inf.point_density[selected_cell][selected_lambda][selected_channel]);
			sval[j-exclude][i-1] = correction * 1e13 * log (radius[i] / run_inf.meniscus[selected_cell])
			/ (omega_s * run_inf.time[selected_cell][selected_lambda][j]);
		}
		progress->setProgress(j-exclude+1);
		plots[j-exclude] = analysis_plot->insertCurve("dcdr");
		analysis_plot->setCurveStyle(plots[j-exclude], QwtCurve::Lines);
		analysis_plot->setCurvePen(plots[j-exclude], yellow);
		if(dcdr_plots)
		{
			analysis_plot->setCurveData(plots[j-exclude], radius, dcdr_points[j-exclude], points-1);
			analysis_plot->setAxisTitle(QwtPlot::xBottom, tr("Radius (cm)"));
		}
		else
		{
			analysis_plot->setCurveData(plots[j-exclude], sval[j-exclude], dcdr_points[j-exclude], points-1);
...
void dcdr_dat_W::update_sval(double newValue)
{
	smax = newValue;
...
//
	...
	if ((run_inf.scans[selected_cell][selected_lambda] - exclude) < 1)
	{
		QMessageBox::message( tr("Warning"), tr("Attention:\n\n"
		"Not enough scans have cleared the meniscus.\n\n"
		"In order to perform a dC/dr Analysis, you need\n"
		"at least one scan that has cleared the meniscus."));
		return(-1);
	}
	...
	dcdr_points = new double* [run_inf.scans[selected_cell][selected_lambda] - exclude];
	Q_CHECK_PTR(dcdr_points);
	sval = new double* [run_inf.scans[selected_cell][selected_lambda] - exclude];
	Q_CHECK_PTR(sval);
	...
	plots = new uint [run_inf.scans[selected_cell][selected_lambda] - exclude];
	Q_CHECK_PTR(plots);
	setup_already = true;
	old_local_scans = run_inf.scans[selected_cell][selected_lambda] - exclude;
	return(0);
}

void dcdr_dat_W::cleanup()
{
	delete [] frequency;
...
void dcdr_dat_W::write_dcdr_avg()
{
	QString filestr, filename, temp;
	filename = USglobal->config_list.result_dir.copy();
	filename.append("/");
	...
	temp.sprintf(".%d%d", selected_cell+1, selected_lambda+1);
	filestr = filename.copy();
	filestr.append(".dcdr_avg");
	filestr.append(temp);
	QFile dcdr_f(filestr);
	int temp_scans = run_inf.scans[selected_cell][selected_lambda] - exclude;
	US_Average *avg1;
	avg1 = new US_Average(points, temp_scans, &sval, &dcdr_points, "dcdr", htmlDir, title, selected_cell, selected_lambda);
	unsigned int i;	
	if (dcdr_f.open(IO_WriteOnly | IO_Translate))
	{
		QTextStream ts(&dcdr_f);
...
void dcdr_dat_W::write_dcdr_scans()
{

	QString filestr, filename, temp;
// subtract one since it is going to be used as an index (0 -> scans)
	unsigned int temp_scans = run_inf.scans[selected_cell][selected_lambda] - exclude - 1;
	filename = USglobal->config_list.result_dir.copy();
	filename.append("/");
	filename.append(run_inf.run_id);
	temp.sprintf(".%d%d", selected_cell+1, selected_lambda+1);
	filestr = filename.copy();
	filestr.append(".dcdr_scans");
	filestr.append(temp);
	...
	if (scans_f.open(IO_WriteOnly | IO_Translate))
	{
		...
		for (i=0; i<points-1; i++)
		{
			ts << (radius[i]+radius[i+1])/2 << "\t";
			for (j=0; j<temp_scans; j++)
			{
				ts << dcdr_points[j][i] << "\t";
			}
			ts << dcdr_points[temp_scans][i] << "\n";
		}
		scans_f.close();
	}
}

void dcdr_dat_W::write_dcdr_sval()
{
	...
	if (scans_f.open(IO_WriteOnly | IO_Translate))
	{
		...
		for (i=0; i<points-1; i++)
		{
			for (j=0; j<temp_scans; j++)
			{
				ts << sval[j][i] << "\t" << dcdr_points[j][i] << "\t";
			}
			ts << sval[temp_scans][i] << "\t" << dcdr_points[temp_scans][i] << "\n";
		}
		scans_f.close();
	}
}

void dcdr_dat_W::write_res()
{
	QString filestr, filename, temp;
	filename = USglobal->config_list.result_dir.copy();
	filename.append("/");
	...
	if (res_f.open(IO_WriteOnly | IO_Translate))
	{
		...
		ts << "***************************************************\n";
		ts << tr("*      Radial - Derivative (dC/dr) Analysis       *\n");
		ts << "***************************************************\n\n\n";
