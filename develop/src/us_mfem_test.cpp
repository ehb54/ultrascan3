...
#include "../include/us_mfem_test.h"
                              ...
                              double m, double b, double c0, struct mfem_initial *tmp_initCvector)
{
   ...
   this->b = b;
   this->c0 = c0;
   //   printf("%d: N: %d s: %e D: %e rpm: %e total_time: %e m: %e b: %e c0: %e\n", myrank, N, s, D, rpm, total_t, m, b, c0); fflush(stdout);
...
US_MovingFEM::US_MovingFEM(int N, double s, double D, double rpm, double total_t, 
                           ...
                           struct mfem_initial *tmp_initCvector)
{
   initCvector = tmp_initCvector;
   stop_now = false;
   GUI = temp_GUI;
...
int US_MovingFEM::interpolate(struct mfem_data *expdata, struct mfem_data *simdata) {
   ...
   //  printf("%d: interpolate 1\n", myrank); fflush(stdout);
  
   if ((*expdata).scan.size() == 0 || 
       (*expdata).scan[0].conc.size() == 0 ||
       (*simdata).scan.size() == 0) {
      return -1;
   }
