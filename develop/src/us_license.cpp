...
void US_License::save()
{
	...
	if(f.open(IO_WriteOnly | IO_Translate))
	{
		...
		QMessageBox::message("Thanks!", "Your license information was saved to the following file:\n\n"+lcfile);
		pb_cancel->setText("Close");
	}
	else
	{
		QMessageBox::message("Error", "Could not save license file - please make sure you have write permission");
		return;
	}
	f.close();
}
...
void US_License::import()
{
	...
	filename=fd.getOpenFileName("", "", this, "", "Please select an e-mail file containing an UltraScan license...");
	...
	if (texfile.open(IO_ReadOnly))
	{
		...
		{
			QMessageBox::message("Attention:", "This file does not seem to be an E-mail text file.\n"
			"It is missing the\n\n\"_____________________CUT HERE_____________________\"\n\ntext string.\n"
			"Please load an e-mail license message file or add the string exactly as shown to the\n"
