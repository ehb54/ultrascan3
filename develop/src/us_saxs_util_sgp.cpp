...
bool US_Saxs_Util::sgp_run()
{
   ...
   for ( unsigned int g = 1; g <= control_parameters[ "sgpgenerations" ].toUInt(); g++ )
   {
      ...
      double pop_size_sd  = sqrt( ( pop_size * sum2 - sum * sum ) / ( pop_size * ( pop_size - 1 ) ) );
      ...
      cout << QString( 
         ...
         .arg( duplicate_count )
         .arg( random_count )
         .arg( elitism_count + mutate_count + crossover_count + plague_count + duplicate_count + random_count );
