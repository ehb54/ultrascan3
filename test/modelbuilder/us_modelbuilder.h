...
private slots:
  ...
  */
  US_DataIO::RawData *
  simulateComponent (US_Model::SimulationComponent component);
  ...
  */
  ...
  /*!
   \brief Calculates a diffusion value from molecular weight and frictioal
   ratio
  ...
  */
  ...
  /*!
   \brief Calculates the RMSD between two simulated solutes, excluding
   simulated scans that take place after either solute has pelleted.
  ...
  */
  QVector<double>
  findListRMSD (QVector<US_Model::SimulationComponent> components);
  ...
  /*!
   ...
   \param approximateToMidpoint whether or give resultant values s, D, f/f0,
   and mw values of the average of the two contributing points
   \return QVector<QVector<double> > a list of length 5 qvectors containing s,
   D, f/f0, mw, and RMSD
  */
  QVector<QVector<double> >
  checkLineRMSDParalell (QVector<QVector<US_Model::SimulationComponent> > grid,
  ...
  */
  QVector<double>
  calculateGridStatistics (QVector<QVector<double> > processedGrid);
  ...
  */
  ...
  testRegularGrid (QVector<QVector<US_Model::SimulationComponent> > grid);
  ...
  */
  ...
  switchRegularGridOrientation (
      QVector<QVector<US_Model::SimulationComponent> > regular);

  /*!
   \brief Very inefficiently finds RMSD levels for every point in an
   irregularly spaced grid (one in which order does not provide additional
   information).

   Operates by simulating every single point, storing simulations in memory,
   and finding RMSDs between every point. An arbitrary threshold, numNeighbors,
   ...
   each other. As a consequence, passing large grids to this method will
   require large amounts of memory and extremely large amounts of time to
   produce results.
  ...
  */
  ...
  /*!
   \brief Generates a Faxen grid over the specified area, with a particular
   size parameter.
  ...
  */
  QVector<QVector<US_Model::SimulationComponent> >
  generateFaxenGrid (double sMin, double maxS, double kMin, double kMax,
                     int grids);
  ...
  */
  QVector<QVector<US_Model::SimulationComponent> >
  generateSKGrid (double sMin, double sMax, double kMin, double kMax, int sDim,
                  int kDim, QChar constantAxis);
  ...
  /*!
   ...
   \param sDim The number of distinct sedimentaion lines to include in the
   grid.
   \param dDim The number of distinct diffusion lines to include in the grid.
   \param constantAxis The first axis to be iterated down. Must be either 's'
   or 'd'.
   \return QVector<QVector<US_Model::SimulationComponent> > A 2D QVector
   containing the grid, oriented based on the selected constantAxis.
  */
  QVector<QVector<US_Model::SimulationComponent> >
  generateSDGrid (double sMin, double sMax, double dMin, double dMax, int sDim,
                  int dDim, QChar constantAxis);
  ...
  /*!
   ...
   \param kDim The number of distinct frictional ratio lines to include in the
   grid.
   \param constantAxis The first axis to be iterated down. Must be either 'm'
   or 'k'.
   \return QVector<QVector<US_Model::SimulationComponent> > A 2D QVector
   containing the grid, oriented based on the selected constantAxis.
  */
  QVector<QVector<US_Model::SimulationComponent> >
  generateMKGrid (double mMin, double mMax, double kMin, double kMax, int mDim,
                  int kDim, QChar constantAxis);
  ...
  /*!
   ...
   \return QVector<US_Model::SimulationComponent> A QVector of solutes,
   starting with the startCoord, and proceeding to points that differ from the
   previous by the specified RMSD.
  */
  ...
  /*!
   \brief Generates a solute grid of either constant sedimentation or f/f0
   lines with constant RMSD between adjacent points.
   ...
   function once on its major axis, and then along the minor axis for each
   point produced.
   ...
   Must be 's' or 'k'.
   \return QVector<QVector<US_Model::SimulationComponent> > A QVector
   containing each line produced for the grid.
  */
  QVector<QVector<US_Model::SimulationComponent> >
  createNumericalGrid (double tolerance, double targetRMSD, double minS,
                       double maxS, double minK, double maxK, QChar majorAxis);
  ...
  */
  ...
  findConstantRMSDNeighbors (double s, double k, double degreeIncrement,
                             double target);
  ...
  /*!
   ...
   \return QPair<double, double> A double-precision representation of the
   point.
  */
  ...
  */
  ...
  getSamplePoints (US_Model::SimulationComponent origin,
                   US_DataIO::RawData *originSim, int numSamples,
                   double degree);
  ...
  */
  ...
  /*!
   \brief Produces a set of isoRMSD level sets for a list of solute points,
  ...
  */
   ...
   * fitted to; for each QPair the first element represents the x value, and
   * the second represents the y value of the point
   * \return a double value representing the slope of the line
   */
  QVector<double>
  linefitThroughOrigin (QVector<QPair<double, double> > points);
};
