...
void US_ModelBuilder::updateParameters(void) { simparams = working_simparams; }
...
void US_ModelBuilder::runIsoRMSDTest(
    QVector<US_Model::SimulationComponent> points, QVector<double> rmsdTargets,
    double increment, QString outputname) {
  ...
  for (int i = 0; i < points.size(); i++) {
    ...
    futures.append(QtConcurrent::run(this, &US_ModelBuilder::getLevelSets,
                                     points.at(i), rmsdTargets, increment));
  }
  ...
  for (int i = 0; i < points.size(); i++) {
    ...
    for (int j = 0; j < rmsdTargets.size(); j++) {
      if (resultPoints.size() != 0) {
        // output file
        QFile outfileIsoLines("output/isoRMSD_" + outputname + "/RMSD_" +
                              QString::number(rmsdTargets.at(j)) + "/" +
                              outputname + "_" +
                              QString::number(rmsdTargets.at(j)) + "_" +
        ...
        for (int k = 0; k < resultPoints.at(j).size(); k++) {
          ...
          outstreamIsoLines << currentComponent.s << " \t" << currentComponent.D
  ...
  for (int i = 0; i < centerpoints.size(); i++) {
    ...
    for (int j = 0; j < targets.size(); j++) {
      ...
      QFile outfile(outputDirectory.path() + "/RMSD_" +
                    QString::number(targets.at(j)) + "/" + outputname + "_" +
                    QString::number(targets.at(j)) + "_" +
                    QString::number(centerpoint.s) + "s_" +
                    QString::number(centerpoint.f_f0) + "k.tsv");
      outfile.open(QFile::WriteOnly | QFile::Truncate);
      QTextStream outstream(&outfile);
      outstream << "#Sed. Coeff. \t Diff. Coeff. \t Frict. Ratio \t Mol. Wt."
                << endl;
...
US_ModelBuilder::levelSetFromJson(QJsonArray slopeJson,
    ...
    foreach (double target, targets) {
      ...
      for (int j = 0; j < currentSlopes.size(); j++) {
        ...
        US_Model::SimulationComponent projected = componentFromSKPair(
            projectPolar(s * 1e13, k, projectionDistance, degree), true);
  ...
  return QPair<QVector<QVector<QVector<US_Model::SimulationComponent> > >,
               QVector<US_Model::SimulationComponent> >(gridSets, centerpoints);
}

// interpolates the RMSD of the point 'target' using the RMSDs of 4 points on a
// regular grid
double US_ModelBuilder::interpolatePoint(QVector2D target, QVector3D p11,
                                         QVector3D p12, QVector3D p21,
                                         QVector3D p22) {
...
double US_ModelBuilder::calculateDistance(US_Model::SimulationComponent first,
...
QVector<double> US_ModelBuilder::findListRMSD(
  ...
  if (components.size() >= 2) {
    ...
    for (int i = 1; i < components.size(); i++) {
      ...
      rmsdList.append(
          calculateScaledRMSD(firstPoint, component1, secondPoint, component2));
...
US_DataIO::RawData* US_ModelBuilder::simulateComponent(
...
double US_ModelBuilder::calculateScaledRMSD(
    US_DataIO::RawData* simulation1, US_Model::SimulationComponent component1,
    US_DataIO::RawData* simulation2, US_Model::SimulationComponent component2) {
...
US_ModelBuilder::switchRegularGridOrientation(
  ...
  for (int i = 0; i < regular.size(); i++) {
    if (regular.size() != regular.at(i).size()) {
      qDebug() << "Grid orientation cannot proceed if dimensions are not all "
                  "equivalent!";
      return rotated;
    }
  }
    ...
    for (int j = 0; j < grid.at(i).size(); j++) {
      ...
      if ((i) >= 0 && (i) < grid.size() && (j + 1) >= 0 &&
        ...
        if (simulationData.at(i).at(j + 1) == NULL) {
          ...
          simulationData[i].replace(j + 1,
                                    simulateComponent(grid.at(i).at(j + 1)));
        }

        // calculate rmsd between current point and point to the right
        rmsdSum += calculateScaledRMSD(simulationData.at(i).at(j), current,
                                       simulationData.at(i).at(j + 1),
                                       grid.at(i).at(j + 1));
      }
...
// Accomplished by simulating every solute, storing the simulations in the heap,
// and then finding RMSD relative to every point A point's RMSD is determined by
// taking the average RMSD of the 'numNeighbors' lowest values NOTE: UNDER NO
// CONDITIONS PASS A LARGE GRID IN HERE
...
// qvector of sedimentation, diffusion, f/f0, MW, and RMSD; this is done because
// std::tuple is unavailable on Alamo
QVector<QPair<US_Model::SimulationComponent, double> >
US_ModelBuilder::calculateIrregularGridRMSD(
  ...
  for (int i = 0; i < grid.size(); i++) {
    futures.append(
        QtConcurrent::run(this, &US_ModelBuilder::simulateBatch, grid.at(i)));
  }
      ...
      for (int j = 0; j < jValues.size(); j++) {
        ...
        if (x_ij >= sMin && x_ij <= sMax) {
          ...
          if (US_Model::calc_coefficients(current)) {
            ...
            qDebug() << "calc_coefficients failed in Faxen generation. s, d, "
                        "f/f0, mw: "
                     << current.s << " , " << current.D << " , " << current.f_f0
                     << " , " << current.mw;
          }
        }
      }
...
US_Model::SimulationComponent US_ModelBuilder::componentFromSK(double s,
                                                               double k) {
...
QVector<US_Model::SimulationComponent> US_ModelBuilder::calculateLine(
    double tolerance, double targetRMSD, QVector2D startCoord,
    QVector2D endCoord) {
  ...
  do {
    ...
    if (calculateScaledRMSD(currentPointSim, currentComponent, terminusSim,
      ...
      do {
        ...
        // calculate testPoint
        testPoint = componentFromSK((lowPoint.s + highPoint.s) / 2.0,
        ...
        // calculate rmsd
        testRMSD = calculateScaledRMSD(currentPointSim, currentComponent,
                                       testPointSim, testPoint);
        ...
        if (countInner > maxDepthInner) {
          qDebug()
              << "Loop depth exceeded in inner loop for numerical grid gen!";
          finished = true;
        }
      ...
      if (calculateDistance(start, testPoint, false) > distanceLimit) {
        ...
        // qDebug() << "test RMSD value: " << testRMSD<< " s: " << testPoint.s
        // << " ff0: " << testPoint.f_f0;
...
US_ModelBuilder::createNumericalGrid(double tolerance, double targetRMSD,
  ...
  for (int i = 0; i < firstLineValues.size(); i++) {
    ...
    if (majorAxis == 's') {
      futures.append(
          QtConcurrent::run(this, &US_ModelBuilder::calculateLine, tolerance,
                            targetRMSD, QVector2D(currentS, minK),
                            QVector2D(currentS, maxK)));  // for s-major
    } else if (majorAxis == 'k') {
      futures.append(
          QtConcurrent::run(this, &US_ModelBuilder::calculateLine, tolerance,
                            targetRMSD, QVector2D(minS, currentK),
  ...
  for (int i = 0; i < numFutures; i++) {
    // add result, blocking when calculation is incomplete (so that ordering is
    // preserved)
...
// finds the RMSDs between adjacent points in each column of the 2d qvector grid
// and returns them as a qvector of sedimentation, diffusion, f/f0, MW, and RMSD
QVector<QVector<double> > US_ModelBuilder::checkLineRMSDParalell(
  ...
  for (int i = 0; i < grid.size(); i++) {
    ...
    // fetch qFutures in the order that they were added, derefrerencing as we go
    // to save memory
...
// Note: Major axis is the axis along which we set constant lines; increments on
// outer QVector, other increments on internal QVector
QVector<QVector<US_Model::SimulationComponent> >
...
QVector<double> US_ModelBuilder::calculateGridStatistics(
...
QPair<double, double> US_ModelBuilder::projectPolar(double originX,
                                                    double originY,
                                                    double distance,
                                                    double degree) {
  double rad = degree * (M_PI / 180);
...
US_Model::SimulationComponent US_ModelBuilder::componentFromSKPair(
    QPair<double, double> sk, bool scaleS) {
  if (scaleS) {
...
// refer to RMSD deviation from the target; use 0.01 as value for most use cases
US_Model::SimulationComponent US_ModelBuilder::findConstantRMSDPointInDirection(
  ...
  double delta =
      2 * sqrt(sq(deltaS * direction.first) + sq(deltaK * direction.second));
  ...
  while (lambdaDifference <= lastDifference ||
         bracket.size() < 3)  // we assume this always runs; can guarantee if
                              // stepsize is dynamically set
  {
    count++;
...
US_ModelBuilder::findConstantRMSDNeighbors(double s, double k,
  ...
  for (double deg = 0.0; deg < 360; deg += degreeIncrement) {
    ...
    try {
      // result = (findConstantRMSDPointInDirection(origin, originSim, target,
      // 0.000001, deg));
      result = (findConstantRMSDPointInDirection(origin, originSim, target, 0.1,
                                                 deg));
    } catch (std::invalid_argument)  // catch invalid argument exception
    {
      // the point here tried to test a value that was illegal, cancelling the
      // operation
    }
...
US_ModelBuilder::getLevelSets(US_Model::SimulationComponent center,
  ...
  for (double degree = 0; degree <= 360; degree += degreeIncrement) {
    ...
    try {
      ...
      //         qDebug() << "sigma, correlation: " << sigma << "," <<
      //         correlation; qDebug() << "s, k, degree: " << center.s << "," <<
      //         center.f_f0 << "," << degree;
      ...
      if (regressionLine.at(2) < 0.9) {
        ...
        qWarning() << "\tR^2, correlation: " << regressionLine.at(2) << ","
                   << regressionLine.at(3);
        qWarning() << "\ts, k, degree: " << center.s << "," << center.f_f0
                   << "," << degree;
      }
  ...
  for (int i = 0; i < targets.size(); i++) {
    ...
    for (double degree = 0; degree <= 360; degree += degreeIncrement) {
      ...
      US_Model::SimulationComponent projected =
          componentFromSKPair(projectPolar(center.s * 1e13, center.f_f0,
                                           calculatedDistance, degree),
                              true);
      ...
      calculatedDistance << "," << currentLastSampleDistance; qWarning() <<
      "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=";
      }
      */
    }
...
// corr. coeff
QVector<double> US_ModelBuilder::linefitThroughOrigin(
