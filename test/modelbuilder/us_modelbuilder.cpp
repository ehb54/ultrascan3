...
#include "main1.inc"
  ...
  // Uncomment to enable GUI display (also uncomment the entire constructor
  // below) w.show(); //!< \memberof QWidget
...
US_ModelBuilder::US_ModelBuilder () : US_Widgets ()
{
  ...
  //TODO: Add control for maximum s/d values to check, control for RMSD search
  tolerance
...
void
US_ModelBuilder::collectParameters (void)
{
...
void
US_ModelBuilder::updateParameters (void)
{
...
void
US_ModelBuilder::startSimulation (void)
{
  /*
  double sMin = 1e-13;
  double sMax = 10e-13;
  ...
  */
  ...
  /*
  QVector<QVector<US_Model::SimulationComponent> > fg =
  generateFaxenGrid(1.1e-13, 9.9e-13, 1.2, 3.8, 270);
  //QVector<QVector<US_Model::SimulationComponent> > fg =
  generateFaxenGrid(1.1e-13, 9.99e-13, 1.05, 3.95, 270);
  ...
  for(int i = 0; i < fg.size(); i++)
  {
     for(int j = 0; j < fg.at(i).size(); j++)
     {
        ...
        else if(current.s >= 1.2e-13 && current.s <= 1.5e-13 && current.f_f0
  <= 1.5)
        {
  ...
  QVector<QVector<US_Model::SimulationComponent> > sk =
  generateSKGrid(1.1e-13, 9.9e-13, 1.2, 3.8, 340, 62, 's');
  ...
  */
  ...
  foreach(double speed, speeds)
  {
     ...
     //simparams.speed_step[0].duration_minutes = (int) ((time - ((int) time))
  * 60);
     ...
     QVector<QVector<US_Model::SimulationComponent> > fg =
  generateFaxenGrid(1.1e-13, 9.9e-13, 1.2, 3.8, 26);
     ...
     runIsoRMSDTest(faxenPoints, targets, 2, "Faxen_speed" +
  QString::number(speed));

     qDebug()<< "\tMaking sk grid";
     QVector<QVector<US_Model::SimulationComponent> > sk =
  generateSKGrid(1.1e-13, 9.9e-13, 1.2, 3.8, 35, 6, QChar('s'));
     QVector<US_Model::SimulationComponent> skPoints;
     for(int i = 0; i < sk.size(); i++)
     {
        skPoints.append(sk.at(i));
     }
     runIsoRMSDTest(skPoints, targets, 2, "skGrid_speed" +
  QString::number(speed));

     qDebug()<< "\tMaking sd grid";
     QVector<QVector<US_Model::SimulationComponent> > sd =
  generateSDGrid(1.1e-13, 9.9e-13, 8.55883e-08, 1.4469e-06, 46, 8, QChar('s'));
     QVector<US_Model::SimulationComponent> sdPoints;
     for(int i = 0; i < sd.size(); i++)
     {
        sdPoints.append(sd.at(i));
     }
     runIsoRMSDTest(sdPoints, targets, 2, "sdGrid_speed" +
  QString::number(speed));
     //qDebug() << "sdsize:" << sdPoints.size();

     qDebug()<< "\tMaking mk grid";
     QVector<QVector<US_Model::SimulationComponent> > mk =
  generateMKGrid(6587.98, 1.00235e+06, 1.2, 3.8, 32, 12, QChar('m'));
     QVector<US_Model::SimulationComponent> mkPoints;
     for(int i = 0; i < mk.size(); i++)
     {
        mkPoints.append(mk.at(i));
     }
     runIsoRMSDTest(mkPoints, targets, 2, "mkGrid_speed" +
  QString::number(speed));
  }
  */
  ...
  QJsonArray jsonSlopeArray =
  QJsonDocument::fromJson(jsonString.toUtf8()).array();
  ...
  QVector<US_Model::SimulationComponent> > sets =
  levelSetFromJson(jsonSlopeArray, targets); processJsonSlopes(sets.first,
  sets.second, targets, "jsonFaxenOut");
  */
  ...
  */
  ...
  /*
     ...
     {
        QVector<QVector<US_Model::SimulationComponent> > fg =
     generateFaxenGrid(1.25e-13, 9.75e-12, 1.1, 3.9, i); int count = 0; for(int
     j = 0; j < fg.size(); j++)
        {
           count += fg.at(j).size();
        }
...
void
US_ModelBuilder::runIsoRMSDTest (QVector<US_Model::SimulationComponent> points,
                                 ...
                                 QString outputname)
{
  ...
  for (int i = 0; i < rmsdTargets.size (); i++)
    {
      QDir outputRMSDDirectory ("output/isoRMSD_" + outputname + "/RMSD_"
                                + QString::number (rmsdTargets.at (i)));
  ...
  QVector<QFuture<QPair<QVector<QVector<US_Model::SimulationComponent> >,
  ...
  for (int i = 0; i < points.size (); i++)
    {
      // queue up job
      futures.append (QtConcurrent::run (this, &US_ModelBuilder::getLevelSets,
                                         points.at (i), rmsdTargets,
                                         increment));
    }
  ...
  for (int i = 0; i < points.size (); i++)
    {
      ...
      if ((i + 1) % 10 == 0)
        {
          qDebug () << "\t\tgetting result " << (i + 1) << " of "
                    << points.size ();
        }

      QPair<QVector<QVector<US_Model::SimulationComponent> >,
            QVector<QPair<double, double> > >
          results = futures.at (0).result ();
      futures.remove (0);

      QVector<QVector<US_Model::SimulationComponent> > resultPoints
          = results.first;
          ...
          if (resultPoints.size () != 0)
            {
              ...
              QFile outfileIsoLines (
                  "output/isoRMSD_" + outputname + "/RMSD_"
                  + QString::number (rmsdTargets.at (j)) + "/" + outputname
                  + "_" + QString::number (rmsdTargets.at (j)) + "_"
                  + QString::number (points.at (i).s) + "s_"
              ...
              outstreamIsoLines
                  << "#Sed. Coeff. \t Diff. Coeff. \t Frict. Ratio \t Mol. Wt."
                  << endl;

              // iterate over rmsd levels
              for (int k = 0; k < resultPoints.at (j).size (); k++)
                {
                  US_Model::SimulationComponent currentComponent
                      = resultPoints.at (j).at (k);
                  outstreamIsoLines << currentComponent.s << " \t"
                                    << currentComponent.D << " \t"
                                    << currentComponent.f_f0 << " \t"
...
void
US_ModelBuilder::processJsonSlopes (
    ...
    QVector<double> targets, QString outputname)
{
  ...
  foreach (double target, targets)
    {
      QDir outputRMSDDirectory (outputDirectory.path () + "/RMSD_"
  ...
  for (int i = 0; i < centerpoints.size (); i++)
    {
      ...
      for (int j = 0; j < targets.size (); j++)
        {
          // output
          QFile outfile (outputDirectory.path () + "/RMSD_"
                         + QString::number (targets.at (j)) + "/" + outputname
                         + "_" + QString::number (targets.at (j)) + "_"
                         + QString::number (centerpoint.s) + "s_"
          ...
          QVector<US_Model::SimulationComponent> currentSet
              = sets.at (i).at (j);
          foreach (US_Model::SimulationComponent component, currentSet)
            {
              outstream << component.s << " \t" << component.D << " \t"
                        << component.f_f0 << " \t" << component.mw << endl;
            }
        }
    }
}

// reads a QJsonObject containing stored slopes and uses them to project RMSD
// levels in each direction for every point with stored data NOTE: top level
// qvector has 1 index / centerpoint, mid level has 1 index / rmsd target,
// bottom has 1 index / projected point
QPair<QVector<QVector<QVector<US_Model::SimulationComponent> > >,
      QVector<US_Model::SimulationComponent> >
US_ModelBuilder::levelSetFromJson (QJsonArray slopeJson,
                                   QVector<double> targets)
{
  ...
  for (int i = 0; i < slopeJson.size (); i++)
    {
      ...
      QJsonArray currentDegrees
          = currentJsonValue.value ("degrees").toArray ();
          ...
          for (int j = 0; j < currentSlopes.size (); j++)
            {
              ...
              US_Model::SimulationComponent projected = componentFromSKPair (
                  projectPolar (s * 1e13, k, projectionDistance, degree),
                  true);
  ...
  return QPair<QVector<QVector<QVector<US_Model::SimulationComponent> > >,
               QVector<US_Model::SimulationComponent> > (gridSets,
                                                         centerpoints);
}

// interpolates the RMSD of the point 'target' using the RMSDs of 4 points on a
// regular grid
double
US_ModelBuilder::interpolatePoint (QVector2D target, QVector3D p11,
                                   QVector3D p12, QVector3D p21, QVector3D p22)
{
  ...
  double xy1
      = ((x2 - x) / (x2 - x1)) * p11.z () + ((x - x1) / (x2 - x1)) * p21.z ();
  double xy2
      = ((x2 - x) / (x2 - x1)) * p12.z () + ((x - x1) / (x2 - x1)) * p22.z ();

  // calculate approximate interpolation - uses abbreviated formula, possibly
  // replace in future
...
void
US_ModelBuilder::initSimparams ()
{
  /*if (simparams.load_simparms("Resources/sp_defaultProfle.xml") != 0) {
       startButton->setEnabled(false);
       qDebug() << "Error: Simulation Parameters cannot be loaded. Manual
   selection required.";
   }*/
...
// this function produces an initalized RawData sim object using given
// simparams
US_DataIO::RawData *
US_ModelBuilder::initRawData (US_Model *system)
{
  ...
  for (int i = 0; i < points; i++)
    {
      working_simdata->xvalues[i]
          = simparams.meniscus + i * simparams.radial_resolution;
    }
...
double
US_ModelBuilder::calculateDistance (US_Model::SimulationComponent first,
                                    US_Model::SimulationComponent second,
                                    bool scaleS)
{
  if (scaleS)
    {
      return sqrt (pow ((1e13 * (first.s - second.s)), 2)
                   + pow (first.f_f0 - second.f_f0, 2));
    }
  else
    {
      return sqrt (pow ((first.s - second.s), 2)
                   + (pow (first.f_f0 - second.f_f0, 2)));
    }
}
...
double
US_ModelBuilder::calculateFrictionalRatioSD (double s, double D)
{
  // constants
...
double
US_ModelBuilder::calculateDiffusionSK (double s, double k)
{
  double R = 8.314e7;
  double T = 293.15;
  double vbar = 0.72;
  ...
  D = (R * T)
      / (N * k * 9.0 * eta * M_PI
         * pow ((2.0 * s * k * vbar * eta) / (1.0 - vbar * rho), 0.5));
...
double
US_ModelBuilder::calculateDiffusionMK (double M, double k)
{
  double R = 8.314e7;
  double T = 293.15;
  double vbar = 0.72;
...
// calculates sedimentation coefficient from molecular weight and diffusion
// coefficient
double
US_ModelBuilder::calculateSedimentationMD (double M, double D)
{
  double R = 8.314e7;
  double T = 293.15;
  double vbar = 0.72;
...
// Takes an ordered list of SCs, simulates each, and returns the list of
// resulting RMSD values from (0,1),(1,2), etc.
QVector<double>
US_ModelBuilder::findListRMSD (
  ...
  if (components.size () >= 2)
    {
      ...
      for (int i = 1; i < components.size (); i++)
        {
          ...
          rmsdList.append (calculateScaledRMSD (firstPoint, component1,
                                                secondPoint, component2));
      ...
      // works because at this point secondpoint and firstpoint have the same
      // pointer
      delete firstPoint;
...
// Creates a model from a single component and performs ASTFEM simulation to
// produce raw data
// TODO: remove dependence on non-updated method init_simData
US_DataIO::RawData *
US_ModelBuilder::simulateComponent (US_Model::SimulationComponent component)
{
  ...
  if (component.s >= 1e-14 && component.f_f0 >= 1)
    ...
    {
      // qDebug() << "Simulator called with invalid component params!
      // Aborting..."; qDebug() << "s, k: " << component.s << ", " <<
      // component.f_f0;
...
double
US_ModelBuilder::calculateScaledRMSD (US_DataIO::RawData *simulation1,
                                      US_Model::SimulationComponent component1,
                                      US_DataIO::RawData *simulation2,
                                      US_Model::SimulationComponent component2)
{
  // TODO: check that all the params are the same between the simulatons
  // (especially mensiscus position)
  ...
  if (simulation1->scanCount () != simulation2->scanCount ())
    {
      ...
      throw std::invalid_argument (
          "Passed simulation scancounts do not match");
    }
  ...
  if (simparams.speed_step.size () != 1)

    {
      qDebug () << "Error: simulations do not match";
      throw std::invalid_argument (
          "Passed simulations have different speedprofiles");
    }

  // time check
  for (int i = 0; i < simulation1->scanCount (); i++)
    {
      if (simulation1->scanData.at (i).seconds
          != simulation2->scanData.at (i).seconds)
        {
  ...
  int startIndex = min (US_DataIO::index (simulation1, left),
                        US_DataIO::index (simulation2, left));
      ...
      if (deltaTMin >= scan1.seconds && deltaTMin >= scan2.seconds)
        {
          ...
          double plateauConcentration1
              = C01 * pow (M_E, -2.0 * s1 * omega2 * scan1.seconds);
          double plateauConcentration2
              = C02 * pow (M_E, -2.0 * s1 * omega2 * scan2.seconds);

          // qDebug() << "plateau con1, con2: " << plateauConcentration1 << " ,
          // " << plateauConcentration2;

          double minPlateauConcentration
              = min (plateauConcentration1, plateauConcentration2);

          // do calculations for scan - run until values reach 1/2 of the
          // minPlateauConcentration
          ...
          for (int j = startIndex; j < endIndex && !pastHalfPlateau; j++)
            {
              ...
              // outstream << val1 << " , " << val2 << " , " << scan1.seconds
              // << " , " << minPlateauConcentration << endl;

              if (val1 * 2 < minPlateauConcentration
                  && val2 * 2 < minPlateauConcentration)
                {
                  pointCount++;
                  rmsd += sq (val1 - val2);
                }
...
double
US_ModelBuilder::calculateScaledRMSD (
    QPair<US_Model::SimulationComponent, US_DataIO::RawData *> pair1,
    QPair<US_Model::SimulationComponent, US_DataIO::RawData *> pair2)
{
  return calculateScaledRMSD (pair1.second, pair1.first, pair2.second,
                              pair2.first);
}

// switches row-column orientation of a 2d simcomponent qvector; requires that
// the r-c dimensions be equal
QVector<QVector<US_Model::SimulationComponent> >
US_ModelBuilder::switchRegularGridOrientation (
  ...
  for (int i = 0; i < regular.size (); i++)
    {
      if (regular.size () != regular.at (i).size ())
        {
          qDebug () << "Grid orientation cannot proceed if dimensions are not "
                       "all equivalent!";
          return rotated;
        }
    }
...
// Creates a paralell grid of simulation data that is populated as needed to
// ensure that each simulation is done only once
QVector<QPair<US_Model::SimulationComponent, double> >
US_ModelBuilder::testRegularGrid (
      ...
      for (int j = 0; j < grid.at (i).size (); j++)
        {
          ...
          if (simulationData.at (i).at (j) == NULL)
            {
              ...
              delete simulationData.at (i).at (
                  j); // ensure we don't memory leak here
              ...
              if (simulationData.at (i - 1).at (j) == NULL)
                {
                  ...
                  simulationData[i - 1].replace (
                      j, simulateComponent (grid.at (i - 1).at (j)));
                }

              // calculate rmsd between current point and point above
              rmsdSum += calculateScaledRMSD (
                  simulationData.at (i).at (j), current,
              ...
              if (simulationData.at (i).at (j + 1) == NULL)
                {
                  ...
                  simulationData[i].replace (
                      j + 1, simulateComponent (grid.at (i).at (j + 1)));
                }
          ...
          // add to results data
          QPair<US_Model::SimulationComponent, double> currentResult (current,
                                                                      rmsdSum);
...
// Accomplished by simulating every solute, storing the simulations in the
// heap, and then finding RMSD relative to every point A point's RMSD is
// determined by taking the average RMSD of the 'numNeighbors' lowest values
// NOTE: UNDER NO CONDITIONS PASS A LARGE GRID IN HERE
// TODO: Optimize by selecting nearest 20 or so points in sk or sd grid for
// comparison instead of calculating RMSD for everything returns them as a
// qvector of sedimentation, diffusion, f/f0, MW, and RMSD; this is done
// because std::tuple is unavailable on Alamo
QVector<QPair<US_Model::SimulationComponent, double> >
US_ModelBuilder::calculateIrregularGridRMSD (
    QVector<QVector<US_Model::SimulationComponent> > grid, int numNeighbors)
{
  ...
  for (int i = 0; i < grid.size (); i++)
    {
      futures.append (QtConcurrent::run (this, &US_ModelBuilder::simulateBatch,
                                         grid.at (i)));
    }
  ...
  for (int i = 0; i < simulations.size (); i++)
    {
      ...
      QPair<US_Model::SimulationComponent, US_DataIO::RawData *> current
          = simulations.at (i);
      ...
      for (int j = i + 1; j < simulations.size (); j++)
        {
          ...
          // add to the current point's differences array and other point's
          // differences array
      ...
      QPair<US_Model::SimulationComponent, double> currentPair (current.first,
                                                                average);
      // qDebug() << current.first.s << " , " << current.first.f_f0 << " , " <<
      // current.second;
    ...
    QVector<US_Model::SimulationComponent> components)
{
  ...
  for (int i = 0; i < components.size (); i++)
    {
      QPair<US_Model::SimulationComponent, US_DataIO::RawData *> value (
          components.at (i), simulateComponent (components.at (i)));
...
QVector<QVector<US_Model::SimulationComponent> >
US_ModelBuilder::generateFaxenGrid (double sMin, double sMax, double kMin,
                                    double kMax, int grids)
{
  QVector<QVector<US_Model::SimulationComponent> > grid;
  ...
  for (int j = 0; j <= M; j++)
    {
      double xi_j = pow ((1 - ((double)j) / M) * pow (mu_1, -0.25)
                             + (((double)j) / M) * pow (mu_2, -0.25),
                         -4.0);
  ...
  for (int i = 0; i <= M; i++)
    {
      ...
      double xi_i = pow ((1 - (((double)i) / M)) * pow (mu_1, -0.25)
                             + (((double)i) / M) * pow (mu_2, -0.25),
                         -4.0);
              ...
              if (x_ij >= sMin && x_ij <= sMax)
                {
                  ...
                  // use calc_coefficients to populate the rest of the
                  // simcomponent
                  if (US_Model::calc_coefficients (current))
                    ...
                    {
                      qDebug () << "calc_coefficients failed in Faxen "
                                   "generation. s, d, f/f0, mw: "
                                << current.s << " , " << current.D << " , "
                                << current.f_f0 << " , " << current.mw;
                    }
                }
            }
        }
...
US_ModelBuilder::componentFromSK (double s, double k)
{
  ...
  if (US_Model::calc_coefficients (newModel))
    ...
    {
      qDebug () << "calc_coefficients failed in SK model generation. s, f/f0: "
                << newModel.s << " , " << newModel.f_f0;
      return newModel;
    }
}
...
QVector<US_Model::SimulationComponent>
US_ModelBuilder::calculateLine (double tolerance, double targetRMSD,
                                QVector2D startCoord, QVector2D endCoord)
{
  ...
  do
    {
      ...
      if (calculateScaledRMSD (currentPointSim, currentComponent, terminusSim,
                               end)
          > targetRMSD)
        {
          double testRMSD;
          ...
          do
            {
              ...
              // calculate testPoint
              testPoint
                  = componentFromSK ((lowPoint.s + highPoint.s) / 2.0,
                                     ((lowPoint.f_f0 + highPoint.f_f0) / 2.0));
              ...
              // calculate rmsd
              testRMSD = calculateScaledRMSD (
                  currentPointSim, currentComponent, testPointSim, testPoint);
              ...
              if (countInner > maxDepthInner)
                {
                  qDebug () << "Loop depth exceeded in inner loop for "
                               "numerical grid gen!";
                  finished = true;
                }
            }
          while (qAbs (targetRMSD - testRMSD) > tolerance
                 && maxDepthInner > countInner);
          ...
          if (calculateDistance (start, testPoint, false) > distanceLimit)
            ...
            {
              // qDebug() << "test RMSD value: " << testRMSD<< " s: " <<
              // testPoint.s << " ff0: " << testPoint.f_f0;
...
// uses the constant RMSD line generator to generate a brute-force grid in the
// form specified by brookes100915a12.pdf
QVector<QVector<US_Model::SimulationComponent> >
US_ModelBuilder::createNumericalGrid (double tolerance, double targetRMSD,
                                      double minS, double maxS, double minK,
                                      double maxK, QChar majorAxis)
{
  QVector<QVector<US_Model::SimulationComponent> > grid;
  ...
  if (majorAxis == 's')
    {
      ...
      firstLineValues = calculateLine (
          tolerance, targetRMSD, QVector2D (minS, minK),
          QVector2D (maxS, minK)); // for s-major, constant k line first
    }
  else if (majorAxis == 'k')
    {
      ...
      firstLineValues = calculateLine (
          tolerance, targetRMSD, QVector2D (minS, minK),
          QVector2D (minS, maxK)); // for k-major, constant s line first
    }
  else
    {
  ...
  for (int i = 0; i < firstLineValues.size (); i++)
    {
      ...
      if (majorAxis == 's')
        {
          futures.append (QtConcurrent::run (
              this, &US_ModelBuilder::calculateLine, tolerance, targetRMSD,
              QVector2D (currentS, minK),
      ...
      // grid.append(calculateLine(tolerance, targetRMSD, QVector2D(minS,
      // currentK) , QVector2D(maxS, currentK)));
    }
  ...
  for (int i = 0; i < numFutures; i++)
    {
      // add result, blocking when calculation is incomplete (so that ordering
      // is preserved)
...
// finds the RMSDs between adjacent points in each column of the 2d qvector
// grid and returns them as a qvector of sedimentation, diffusion, f/f0, MW,
// and RMSD
QVector<QVector<double> >
US_ModelBuilder::checkLineRMSDParalell (
    QVector<QVector<US_Model::SimulationComponent> > grid,
  ...
  for (int i = 0; i < grid.size (); i++)
    {
      ...
      futures.append (
          QtConcurrent::run (this, &US_ModelBuilder::findListRMSD, column));
    }
  ...
  for (int i = 0; i < grid.size (); i++)
    {
      ...
      // fetch qFutures in the order that they were added, derefrerencing as we
      // go to save memory
...
// Note: Major axis is the axis along which we set constant lines; increments
// on outer QVector, other increments on internal QVector
QVector<QVector<US_Model::SimulationComponent> >
US_ModelBuilder::generateSKGrid (double sMin, double sMax, double kMin,
                                 double kMax, int sDim, int kDim,
                                 QChar constantAxis)
{
  QVector<QVector<US_Model::SimulationComponent> > grid;
  ...
  // otherwise, error
    ...
    {
      throw std::invalid_argument (
          "ConstatnAxis passed to grid generator was invalid");
    }
...
QVector<QVector<US_Model::SimulationComponent> >
US_ModelBuilder::generateSDGrid (double sMin, double sMax, double dMin,
                                 double dMax, int sDim, int dDim,
                                 QChar constantAxis)
{
  QVector<QVector<US_Model::SimulationComponent> > grid;
          ...
          for (int j = 0; j < dDim; j++)
            {
              ...
              if (US_Model::calc_coefficients (current) && current.f_f0 >= 1
                  && current.s <= 10e-13 && current.f_f0 <= 4)
                {
                  // qDebug() << "s: " << current.s << " , D: " << current.D <<
                  // " , f_f0: " << current.f_f0 << endl;
                  column.append (current);
                }
            }
          grid.append (column);
...
QVector<QVector<US_Model::SimulationComponent> >
US_ModelBuilder::generateMKGrid (double mMin, double mMax, double kMin,
                                 double kMax, int mDim, int kDim,
                                 QChar constantAxis)
{
  QVector<QVector<US_Model::SimulationComponent> > grid;
          ...
          for (int j = 0; j < kDim; j++)
            {
              ...
              // do coefficient calculation, and proceed only if successful
              if (US_Model::calc_coefficients (current) && current.f_f0 <= 4.0
                  && current.f_f0 >= 1 && current.s < 10e-13)
                {
                  // qDebug() << "s: " << current.s << " , D: " << current.D <<
                  // " , f_f0: " << current.f_f0 << endl;
                  column.append (current);
...
// calculates distribution statistics on a specified grid, calculating along
// inner QVectors Returns data in a size 3 qvector in the form: pointcount,
// average RMSD, standard deviation
QVector<double>
US_ModelBuilder::calculateGridStatistics (
...
QPair<double, double>
US_ModelBuilder::convertPolar (double degree)
{
...
// finds a point along a given direction vector from a source point in degrees
// with the specified distance
QPair<double, double>
US_ModelBuilder::projectPolar (double originX, double originY, double distance,
                               double degree)
{
  double rad = degree * (M_PI / 180);
  QPair<double, double> newPoint (originX + distance * cos (rad),
                                  originY + distance * sin (rad));
...
US_Model::SimulationComponent
US_ModelBuilder::componentFromSKPair (QPair<double, double> sk, bool scaleS)
{
  if (scaleS)
    {
...
// Finds a solute point differing from the origin by a specified target RMSD in
// the polar direction given NOTE: tolerance, in this context does not directly
// refer to RMSD deviation from the target; use 0.01 as value for most use
// cases
US_Model::SimulationComponent
US_ModelBuilder::findConstantRMSDPointInDirection (
    ...
    double target, double tolerance, double degree)
{
  ...
  // double delta = deltaS * direction.first + deltaK * direction.second;
  // //scalar projection of default grid step delta /= 25;

  // TODO: properly fix if we actually need this
  double delta = 2
                 * sqrt (sq (deltaS * direction.first)
                         + sq (deltaK * direction.second));

  // NOTE: This assumes that delta is a valid, legal stepsize
  // this may not be true for very low RMSDs, and probably should be set
  // dynamically

  // dynamic stepsize setting
  US_Model::SimulationComponent stepsizeTest = componentFromSKPair (
      projectPolar (origin.s * 1e13, origin.f_f0, delta, degree), true);
  US_DataIO::RawData *stepsizeTestSim = simulateComponent (stepsizeTest);
  double stepsizeTestDifference = qAbs (
      calculateScaledRMSD (originSim, origin, stepsizeTestSim, stepsizeTest)
      - target);
  ...
  while (stepsizeTestDifference > target)
    {
      ...
      // redo calcs
      stepsizeTest = componentFromSKPair (
          projectPolar (origin.s * 1e13, origin.f_f0, delta, degree), true);
      stepsizeTestSim = simulateComponent (stepsizeTest);
      stepsizeTestDifference
          = qAbs (calculateScaledRMSD (originSim, origin, stepsizeTestSim,
                                       stepsizeTest)
                  - target);
      delete stepsizeTestSim;
    }
  ...
  US_Model::SimulationComponent lambda
      = componentFromSK (origin.s, origin.f_f0);
  ...
  while (lambdaDifference <= lastDifference
         || bracket.size () < 3) // we assume this always runs; can guarantee
                                 // if stepsize is dynamically set
    {
      count++;

      // reassign point
      bracket.append (lambda);
      lambda = componentFromSKPair (
          projectPolar (lambda.s * 1e13, lambda.f_f0, delta, degree), true);
      ...
      lambdaDifference = qAbs (
          calculateScaledRMSD (originSim, origin, lambdaSim, lambda) - target);
      delete lambdaSim;

      // qDebug() << "bracket testpoint: " << lambda.s << " , " << lambda.f_f0
      // << " , " << lambdaDifference;
    }
  ...
  US_Model::SimulationComponent bracketMidpoint = componentFromSKPair (
      projectPolar (origin.s * 1e13, origin.f_f0,
                    (calculateDistance (origin, bracket.at (1), true)
                     + calculateDistance (origin, bracket.at (2), true))
                        / 2.0,
                    degree),
      true);
  ...
  if (qAbs (calculateScaledRMSD (originSim, origin, bracketMidpointSim,
                                 bracketMidpoint)
            - target)
      >= qAbs (calculateScaledRMSD (originSim, origin, bracketMedianSim,
                                    bracket.at (1))
               - target))
    {
      // median is smallest
      // now find which of k-1 or k+1 is closer
      if (calculateDistance (bracket.at (1), bracket.at (2), true)
          >= calculateDistance (bracket.at (1), bracket.at (0), true))
        {
  ...
  else
    {
      ...
      // now find which of k-1 or k+1 is closer
      if (calculateDistance (bracketMidpoint, bracket.at (2), true)
          >= calculateDistance (bracketMidpoint, bracket.at (0), true))
        {
          // we want to discard k+1, keep k-1
          bracket.replace (2, bracketMidpoint);
        }
  ...
  double stepDifference1
      = qAbs (calculateScaledRMSD (originSim, origin, stepSim1, x1) - target);
  double stepDifference2
      = qAbs (calculateScaledRMSD (originSim, origin, stepSim2, x2) - target);
      ...
      if (stepDifference1 < stepDifference2)
        {
          ...
          x1 = componentFromSKPair (
              projectPolar (origin.s * 1e13, origin.f_f0,
                            alphaDistance
                                + (1 - tau) * (betaDistance - alphaDistance),
                            degree),
              true);
          stepSim1 = simulateComponent (x1);
          stepDifference1 = qAbs (
              calculateScaledRMSD (originSim, origin, stepSim1, x1) - target);
          delete stepSim1;
      ...
      else
        {
          ...
          x2 = componentFromSKPair (
              projectPolar (origin.s * 1e13, origin.f_f0,
                            alphaDistance
                                + tau * (betaDistance - alphaDistance),
                            degree),
              true);
          stepSim2 = simulateComponent (x2);
          stepDifference2 = qAbs (
              calculateScaledRMSD (originSim, origin, stepSim2, x2) - target);
          delete stepSim2;
  ...
  // double mdptDifference = qAbs(calculateScaledRMSD(originSim, origin,
  // mdptSim, midpoint) - target); delete mdptSim; qDebug() << "result
  // midpoint:"; qDebug() << midpoint.s << "," << midpoint.f_f0 << "," <<
  // mdptDifference;
...
QVector<US_Model::SimulationComponent>
US_ModelBuilder::findConstantRMSDNeighbors (double s, double k,
  ...
  for (double deg = 0.0; deg < 360; deg += degreeIncrement)
    {
      ...
      try
        {
          // result = (findConstantRMSDPointInDirection(origin, originSim,
          // target, 0.000001, deg));
          result = (findConstantRMSDPointInDirection (origin, originSim,
                                                      target, 0.1, deg));
        }
        ...
        {
          // the point here tried to test a value that was illegal, cancelling
          // the operation
        }
...
// Gets a list of sample points (in terms of distance from the origin point
// along the given line) and RMSDs relative to the specified origin in a given
// direction
QVector<QPair<double, double> >
US_ModelBuilder::getSamplePoints (US_Model::SimulationComponent origin,
                                  US_DataIO::RawData *originSim,
                                  int numSamples, double degree)
{
  ...
  double delta
      = (deltaS * deltaK)
        / sqrt (sq (deltaK) * sq (cos (rad)) + sq (deltaS) * sq (sin (rad)));
  ...
  for (int i = 1; i <= numSamples && inBounds; i++)
    {
      ...
      US_Model::SimulationComponent current = componentFromSKPair (
          projectPolar (origin.s * 1e13, origin.f_f0, delta * (i + 1), degree),
          true);
      ...
      try
        {
          ...
          double currentRMSD
              = calculateScaledRMSD (originSim, origin, currentSim, current);
          delete currentSim;

          // qDebug() << current.s << "\t" << current.f_f0 << "\t" <<
          // currentRMSD;

          // make pair
          QPair<double, double> currentPair (
              calculateDistance (origin, current, true), currentRMSD);
...
// gets RMSD level sets around the specified center point for the RMSD levels
// given
QPair<QVector<QVector<US_Model::SimulationComponent> >,
      QVector<QPair<double, double> > >
US_ModelBuilder::getLevelSets (US_Model::SimulationComponent center,
                               QVector<double> targets, double degreeIncrement)
{
  // level set structure
  QVector<QVector<US_Model::SimulationComponent> > levelSets;
      ...
      try
        {
          ...
          QVector<QPair<double, double> > samples = getSamplePoints (
              center, centerSim, 3,
              degree); // change the number of points to make this go faster
          ...
          //         qDebug() << "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n";
          //         qDebug() << "slope, intercept: " << slope << "," <<
          //         intercept; qDebug() << "sigma, correlation: " << sigma <<
          //         "," << correlation; qDebug() << "s, k, degree: " <<
          //         center.s << "," << center.f_f0 << "," << degree;

          // check that correlation is high
          if (regressionLine.at (2) < 0.9)
            {
              qWarning () << "R^2 less than 0.9 at:";
              qWarning () << "\tR^2, correlation: " << regressionLine.at (2)
                          << "," << regressionLine.at (3);
              qWarning () << "\ts, k, degree: " << center.s << ","
                          << center.f_f0 << "," << degree;
            }

          // store line
          slopes.append (
              QPair<double, double> (regressionLine.first (), degree));
        }
      catch (std::invalid_argument) // catch invalid argument exception
        {
      ...
      for (double degree = 0; degree <= 360; degree += degreeIncrement)
        {
          ...
          // QPair<double, double> currentLine = lines.at((int) (degree /
          // degreeIncrement));
          double currentSlope
              = slopes.at ((int)(degree / degreeIncrement)).first;

          // get current last distance
          // double currentLastSampleDistance = lastDistances.at((int) (degree
          // / degreeIncrement));
          ...
          US_Model::SimulationComponent projected = componentFromSKPair (
              projectPolar (center.s * 1e13, center.f_f0, calculatedDistance,
                            degree),
              true);
          ...
          if(currentLastSampleDistance < calculatedDistance)
          {
             ...
             qWarning() << "degree, distance, sampledistance: " << degree <<
          "," << calculatedDistance << "," << currentLastSampleDistance;
             qWarning() << "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=";
          }
          */
        }
  ...
  return QPair<QVector<QVector<US_Model::SimulationComponent> >,
               QVector<QPair<double, double> > > (levelSets, slopes);
}

// Performs an OLS regression through the origin (without intercept), returning
// a qvector containing in this order, the slope, coeff of det, and pearson
// corr. coeff
QVector<double>
US_ModelBuilder::linefitThroughOrigin (QVector<QPair<double, double> > points)
{
  double sumXY = 0.0;
