...
void TestUS_SimulationParameters::test_save_simparms_user_mesh() {
  ...
  while (!xml.atEnd()) {
    ...
    if (xml.isStartElement() && xml.name() == "usermesh") {
      ...
      QCOMPARE(attr.value("radius").toString(),
               QString()
...
void TestUS_SimulationParameters::
    test_speedstepFromXml_AccelerationFlagFalse() {
  ...
  for (const QString& val : falseValues) {
    ...
    US_SimulationParameters::SpeedProfile spo;
    spo.acceleration_flag = true;  // preset to true; expect function to set it
