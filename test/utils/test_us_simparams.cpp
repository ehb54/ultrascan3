...
void
TestUS_SimulationParameters::test_load_simparams_missing_file ()
{
  US_SimulationParameters simParams;
  QString filename = "sp_nonexistent_file.xml";
  QVERIFY2 (!QFileInfo (filename).exists (),
            qPrintable ("File " + filename + " already exists"));
...
void
TestUS_SimulationParameters::test_load_simparams_empty_file ()
{
...
void
TestUS_SimulationParameters::test_speedstepToXml_without_set_speed ()
{
  // Create a SpeedProfile instance with set_speed == 0 (will not write extra
  // attributes)
  ...
  QCOMPARE (attributes.value ("rotorspeed").toString (),
            QString::number (spi.rotorspeed));
  QCOMPARE (attributes.value ("scans").toString (),
            QString::number (spi.scans));
  QCOMPARE (attributes.value ("timefirst").toString (),
            QString::number (spi.time_first));
  QCOMPARE (attributes.value ("timelast").toString (),
            QString::number (spi.time_last));
  QCOMPARE (attributes.value ("w2tfirst").toString (),
            QString::number (spi.w2t_first));
  QCOMPARE (attributes.value ("w2tlast").toString (),
            QString::number (spi.w2t_last));
  QCOMPARE (attributes.value ("duration_hrs").toString (),
            QString::number (spi.duration_hours));
  QCOMPARE (attributes.value ("duration_mins").toString (),
            QString::number (spi.duration_minutes));
  QCOMPARE (attributes.value ("delay_hrs").toString (),
            QString::number (spi.delay_hours));
  QCOMPARE (attributes.value ("delay_mins").toString (),
            QString::number (spi.delay_minutes));
  QCOMPARE (attributes.value ("acceleration").toString (),
            QString::number (spi.acceleration));
  // acceleration_flag is written as "1" for true
  QCOMPARE (attributes.value ("accelerflag").toString (),
...
void
TestUS_SimulationParameters::test_speedstepToXml_with_set_speed ()
{
  // Create a SpeedProfile instance with set_speed > 0 (will write the extra
  // attributes)
  US_SimulationParameters::SpeedProfile spi;
  spi.rotorspeed = 1500;
  spi.scans = 20;
  ...
  QCOMPARE (attributes.value ("set_speed").toString (),
            QString::number (spi.set_speed));
  QCOMPARE (attributes.value ("avg_speed").toString (),
            QString::number (spi.avg_speed));
  QCOMPARE (attributes.value ("speed_stddev").toString (),
            QString::number (spi.speed_stddev));
}

// Test that a basic set of parameters are written correctly.
void
TestUS_SimulationParameters::test_save_simparms_default ()
{
  ...
  simParams.meshType
      = US_SimulationParameters::ASTFEM; // Assuming enum starts at 0
      ...
      if (xml.isStartElement () && xml.name () == "params")
        {
          ...
          // Verify mesh and grid types (first entries in the respective
          // arrays).
          QCOMPARE (attr.value ("meshType").toString (), QString ("ASTFEM"));
          QCOMPARE (attr.value ("gridType").toString (), QString ("Fixed"));
          QCOMPARE (attr.value ("simpoints").toString (),
                    QString::number (simParams.simpoints));
          QCOMPARE (attr.value ("radialres").toString (),
                    QString::number (simParams.radial_resolution));
          QCOMPARE (attr.value ("meniscus").toString (),
                    QString::number (simParams.meniscus));
          QCOMPARE (attr.value ("bottom").toString (),
                    QString::number (simParams.bottom));
          QCOMPARE (attr.value ("rnoise").toString (),
                    QString::number (simParams.rnoise));
          QCOMPARE (attr.value ("lrnoise").toString (),
                    QString::number (simParams.lrnoise));
          QCOMPARE (attr.value ("tinoise").toString (),
                    QString::number (simParams.tinoise));
          QCOMPARE (attr.value ("rinoise").toString (),
                    QString::number (simParams.rinoise));
          QCOMPARE (attr.value ("temperature").toString (),
                    QString::number (simParams.temperature));
          QCOMPARE (attr.value ("bandform").toString (),
                    QString (simParams.band_forming ? "1" : "0"));
          QCOMPARE (attr.value ("sector").toString (),
                    QString::number (simParams.cp_sector));
          QCOMPARE (attr.value ("pathlength").toString (),
                    QString::number (simParams.cp_pathlen));
          QCOMPARE (attr.value ("angle").toString (),
                    QString::number (simParams.cp_angle));
          QCOMPARE (attr.value ("width").toString (),
                    QString::number (simParams.cp_width));
...
void
TestUS_SimulationParameters::test_save_simparms_user_mesh ()
{
  QTemporaryFile tempFile;
  QVERIFY (tempFile.open ());
  QString fileName = tempFile.fileName ();
      ...
      if (xml.isStartElement () && xml.name () == "usermesh")
        {
          ...
          QCOMPARE (
              ...
              QString ()
                  .asprintf ("%11.5e", simParams.mesh_radius[userMeshCount])
                  .simplified ());
          ++userMeshCount;
        }
    }
...
// Test that optional attributes (rotorCalID, rotorcoeffs, bandvolume) are
// written.
void
TestUS_SimulationParameters::test_save_simparms_optionalAttributes ()
{
  QTemporaryFile tempFile;
  QVERIFY (tempFile.open ());
  QString fileName = tempFile.fileName ();
      ...
      if (xml.isStartElement () && xml.name () == "params")
        {
          ...
          if (attr.hasAttribute ("rotorCalID"))
            {
              QCOMPARE (attr.value ("rotorCalID").toString (),
                        QString ("CAL123"));
          ...
          if (attr.hasAttribute ("bandvolume"))
            {
              QCOMPARE (attr.value ("bandvolume").toString (),
                        QString::number (42.0));
...
void
TestUS_SimulationParameters::test_save_simparms_file_open_fail ()
{
...
void
TestUS_SimulationParameters::test_speedstepFromXml_AllAttributes ()
{
  ...
  // According to the implementation, accelerflag uses indexOf – "yes" is
  // present, so it should be true.
...
void
TestUS_SimulationParameters::test_speedstepFromXml_MissingAttributes ()
{
  US_SimulationParameters simParams;
  ...
  US_SimulationParameters::SpeedProfile spo;
  // Initialize some members with non-default values to check they persist if
  // not overwritten.
...
void
TestUS_SimulationParameters::test_speedstepFromXml_AccelerationFlagTrue ()
{
  // Test different variations of a "true" string and verify the flag is set.
  // The internal test searches in " 1YesyesTruetrue", so any substring that is
  // found at an index > 0 should yield true.
  ...
  for (const QString &val : trueValues)
    {
      ...
      QVERIFY2 (spo.acceleration_flag,
                QString ("accelerflag value '%1' should yield true")
                    .arg (val)
                    .toUtf8 ()
                    .constData ());
    }
...
void
TestUS_SimulationParameters::test_speedstepFromXml_AccelerationFlagFalse ()
{
  // For values not in the string " 1YesyesTruetrue" at appropriate index,
  // the flag should remain false. For example, "false" is not a substring that
  // triggers condition.
  ...
  for (const QString &val : falseValues)
    {
      ...
      US_SimulationParameters::SpeedProfile spo;
      spo.acceleration_flag = true; // preset to true; expect function to set
                                    // it to false if not found
      US_SimulationParameters::speedstepFromXml (reader, spo);
      // The flag should be false.
      QVERIFY2 (!spo.acceleration_flag,
                QString ("accelerflag value '%1' should yield false")
                    .arg (val)
                    .toUtf8 ()
                    .constData ());
    }
...
void
TestUS_SimulationParameters::test_load_simparams_minimum_file ()
{
  US_SimulationParameters simParams;
...
void
TestUS_SimulationParameters::test_load_simparams_all ()
{
  ...
  QCOMPARE (static_cast<int> (simparms.meshType),
            2); // "MovingHat" index (0: ASTFEM,1: Claverie,2: MovingHat,...)
  QCOMPARE (static_cast<int> (simparms.gridType),
            1); // "Moving" index (0: Fixed, 1: Moving)
...
// Test case 2: Minimum file (only <params> with required attributes) but
// missing <speedstep>
void
TestUS_SimulationParameters::test_load_simparams_missing_speedstep ()
{
  ...
  out << "<?xml version=\"1.0\"?>\n"
      ...
      << "rnoise=\"0.05\" lrnoise=\"0.1\" tinoise=\"0.15\" rinoise=\"0.2\" "
         "temperature=\"20.0\" "
  ...
  // As speed_step is missing, the function should revert to the old speed_step
  // and return an error.
  QVERIFY (stat != 0);
