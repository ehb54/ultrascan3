...
protected:
    ...
    US_Model build(const US_SimSpecies::Component& component) {
        US_Model model;
        QString error;
        EXPECT_TRUE(US_SimSpecies::model(component, model, error))
            << error.toStdString();
        return model;
    }

    US_Model build(const QVector<US_SimSpecies::Component>& components) {
        US_Model model;
        QString error;
        EXPECT_TRUE(US_SimSpecies::model(components, model, error))
            << error.toStdString();
        return model;
    }
};
...
TEST_F(US_SimSpeciesTest, ComponentAcceptsEverySolvablePair) {
    ...
    for (const auto& entry : pairs) {
        ...
        US_Model model = build(entry.second);
        ASSERT_EQ(model.components.count(), 1);
        const US_Model::SimulationComponent& sc = model.components[0];
        EXPECT_GT(sc.mw, 0.0);
        EXPECT_GT(sc.D, 0.0);
...
TEST_F(US_SimSpeciesTest, ComponentRejectsZeroValuedCoefficients) {
    ...
    // Negative s remains meaningful for a floating species, but zero does not.
    US_SimSpecies::Component zero_s;
    zero_s.s    = 0.0;
    zero_s.f_f0 = 1.25;
    EXPECT_FALSE(US_SimSpecies::validateComponent(zero_s).isEmpty());
}
...
TEST_F(US_SimSpeciesTest, ComponentDefaultsToTheLibraryConcentration) {
    ...
    EXPECT_DOUBLE_EQ(c.signal_concentration, sc_defaults.signal_concentration);
...
TEST_F(US_SimSpeciesTest, ComponentRejectsNonPositiveConcentration) {
    ...
    c.signal_concentration = 0.0;
    EXPECT_FALSE(US_SimSpecies::validateComponent(c).isEmpty());

    c.signal_concentration = -0.5;
    EXPECT_FALSE(US_SimSpecies::validateComponent(c).isEmpty());
}

TEST_F(US_SimSpeciesTest, ComponentDefaultsToNoSpectrum) {
    US_SimSpecies::Component c;
    US_Model::SimulationComponent sc_defaults;
...
TEST_F(US_SimSpeciesTest, ExtinctionReachesTheModelPerComponent) {
    US_SimSpecies::Component monomer;
    monomer.mw         = 66430.0;
    monomer.f_f0       = 1.2;
    monomer.extinction = 43824.0;

    US_SimSpecies::Component dimer;
    dimer.mw         = 132860.0;
    dimer.f_f0       = 1.4;
    dimer.extinction = 87648.0;

    US_Model model;
    QString  error;
    const QVector<US_SimSpecies::Component> mixture{ monomer, dimer };
    ASSERT_TRUE(US_SimSpecies::model(mixture, model, error)) << error.toStdString();

    ASSERT_EQ(model.components.count(), 2);
    EXPECT_DOUBLE_EQ(model.components[0].extinction, 43824.0);
    EXPECT_DOUBLE_EQ(model.components[1].extinction, 87648.0);
}

TEST_F(US_SimSpeciesTest, ComponentDefaultsToProtein) {
    US_SimSpecies::Component c;
    US_Model::SimulationComponent sc_defaults;

    EXPECT_EQ(c.analyte_type, sc_defaults.analyte_type);
    EXPECT_EQ(c.analyte_type, static_cast<int>(US_Analyte::PROTEIN));
}

TEST_F(US_SimSpeciesTest, AnalyteTypeOutsideTheEnumIsRejected) {
    US_SimSpecies::Component c = US_SimSpecies::defaultComponent();

    for (int type = US_Analyte::PROTEIN; type <= US_Analyte::CARBOHYDRATE; type++) {
        c.analyte_type = type;
...
TEST_F(US_SimSpeciesTest, AnalyteTypeReachesTheModelPerComponent) {
    US_SimSpecies::Component dna;
    dna.mw           = 128396.0;
    dna.f_f0         = 2.86;
    dna.analyte_type = US_Analyte::DNA;

    // Left at the default, to show one component's type does not set another's.
    US_SimSpecies::Component protein;
    protein.mw   = 66430.0;
    protein.f_f0 = 1.2;

    US_Model model;
    QString  error;
    const QVector<US_SimSpecies::Component> mixture{ dna, protein };
    ASSERT_TRUE(US_SimSpecies::model(mixture, model, error)) << error.toStdString();

    ASSERT_EQ(model.components.count(), 2);
    EXPECT_EQ(model.components[0].analyte_type, static_cast<int>(US_Analyte::DNA));
    EXPECT_EQ(model.components[1].analyte_type, static_cast<int>(US_Analyte::PROTEIN));
}
...
TEST_F(US_SimSpeciesTest, AnalyteGuidMustBeAFullUuidIfGiven) {
    US_SimSpecies::Component c = US_SimSpecies::defaultComponent();

    c.analyte_guid = US_Util::new_guid();
...
TEST_F(US_SimSpeciesTest, AnalyteGuidReachesTheModelPerComponent) {
    const QString first_guid  = US_Util::new_guid();

    US_SimSpecies::Component identified;
    identified.mw           = 128396.0;
    identified.f_f0         = 2.86;
    identified.analyte_guid = first_guid;

    // Left empty, to show one component's identity does not leak into another's.
    US_SimSpecies::Component anonymous;
    anonymous.mw   = 66430.0;
    anonymous.f_f0 = 1.2;

    US_Model model;
    QString  error;
    const QVector<US_SimSpecies::Component> mixture{ identified, anonymous };
    ASSERT_TRUE(US_SimSpecies::model(mixture, model, error)) << error.toStdString();

    ASSERT_EQ(model.components.count(), 2);
    EXPECT_EQ(model.components[0].analyteGUID, first_guid);
    EXPECT_TRUE(model.components[1].analyteGUID.isEmpty());
}
...
TEST_F(US_SimSpeciesTest, ExtinctionIsIndependentOfLoadingConcentration) {
    ...
    c.mw                   = 66430.0;
    c.f_f0                 = 1.2;
    c.signal_concentration = 0.75;
    c.extinction           = 43824.0;

    US_Model model;
    QString  error;
    const QVector<US_SimSpecies::Component> one{ c };
    ASSERT_TRUE(US_SimSpecies::model(one, model, error)) << error.toStdString();
...
TEST_F(US_SimSpeciesTest, MixtureKeepsEveryComponentInOrder) {
    ...
    monomer.s             = 4.58e-13;
    monomer.mw            = 66430.0;
    monomer.signal_concentration = 0.75;
    ...
    dimer.s             = 6.50e-13;
    dimer.mw            = 132860.0;
    dimer.signal_concentration = 0.25;
...
TEST_F(US_SimSpeciesTest, ModelBuilderRejectsInvalidInputWithoutPartialOutput) {
    US_SimSpecies::Component invalid;
    US_Model model;
    model.description = "unchanged";
    QString error;

    EXPECT_FALSE(US_SimSpecies::model(invalid, model, error));
    EXPECT_FALSE(error.isEmpty());
    EXPECT_EQ(model.description, QString("unchanged"));
    EXPECT_TRUE(model.components.isEmpty());
}

TEST_F(US_SimSpeciesTest, ConcentrationsAreAbsoluteAndSumToTheModelTotal) {
    US_SimSpecies::Component first = US_SimSpecies::defaultComponent();
    US_SimSpecies::Component second = US_SimSpecies::defaultComponent();
    first.signal_concentration = 2.0;
    second.signal_concentration = 3.5;
