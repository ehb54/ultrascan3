...
void TestUSDataFiles::test_no_existing_files() {
  ...
  QString filename =
      US_DataFiles::get_filename(path, guid, lfchar, lkupTag, lkupAtt, newFile);
...
void TestUSDataFiles::test_existing_file_with_matching_guid() {
  ...
  // Test case 2: Existing file with a matching GUID, should return the existing
  // file
  QString filename =
      US_DataFiles::get_filename(path, guid, lfchar, lkupTag, lkupAtt, newFile);
  QCOMPARE(filename, path + QDir::separator() + "M0000001.xml");
...
void TestUSDataFiles::test_existing_files_without_matching_guid() {
  ...
  // Test case 3: Existing files without a matching GUID, should create the next
  // file
  QString filename =
      US_DataFiles::get_filename(path, guid, lfchar, lkupTag, lkupAtt, newFile);
