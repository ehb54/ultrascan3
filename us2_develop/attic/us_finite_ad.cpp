...
US_Finite_ad::US_Finite_ad(QWidget *p, const char *name) : Data_Control_W(13, p, name)
{
...
US_Finite_ad::~US_Finite_ad()
{
	cleanup();
}

void US_Finite_ad::closeEvent(QCloseEvent *e)
{
	e->accept();
	global_Xpos -= 30;
	global_Ypos -= 30;
...
void US_Finite_ad::updateButtons()
{
	pb_save_fit->setEnabled(false);
	pb_save->setEnabled(false);
	pb_fit_control->setEnabled(false);
...
void US_Finite_ad::select_model_control()
{
	if (model_widget)
	{
	...
	run_inf.cell_id[selected_cell], run_inf.baseline[selected_cell][selected_lambda],
	run_inf.meniscus[selected_cell], run_inf.plateau[selected_cell][selected_lambda][0]
	- run_inf.baseline[selected_cell][selected_lambda], 
	&viscosity_tb, &density_tb, run_inf.avg_temperature);
...
void US_Finite_ad::change_tape_status()
{
	velocFitter->tape_construct = false;
}

void US_Finite_ad::select_fit_control()
{
	...
	{
		fit_init();
		velocFitter = new US_FeFitter_AD(&fitting_widget, 
													true /* show GUI */,
													components,
													model,
													&component_vector,
...
void US_Finite_ad::updateModelWindow()
{
	model_control->update_labels();
}

void US_Finite_ad::updateMonteCarlo()
{
	pb_second_plot->setEnabled(true);
}

void US_Finite_ad::suspend_fit()
{
	suspend_flag = true;
...
void US_Finite_ad::fit_converged()
{
	converged = true;
...
void US_Finite_ad::create_model()
{
	if (step == 0)
	{
		QMessageBox::message(tr("Attention:\n"),tr("You need to load some data first!\n\n	   Click on \"Load Data\"\n"));
...
void US_Finite_ad::start_fit()
{
	...
	&viscosity_tb, &density_tb, run_inf.avg_temperature);
	model_control->setPalette( QPalette(USglobal->global_colors.cg_frame, USglobal->global_colors.cg_frame, USglobal->global_colors.cg_frame));
	model_control->ct_delta_r->setEnabled(false);
	model_control->show();
	model_defined = true;
}

void US_Finite_ad::update_residuals(double val)
{
	if (step == 0)
	{
		QMessageBox::message(tr("Attention:\n"),tr("You need to load some data first!\n\n	   Click on \"Load Data\"\n"));
...
void US_Finite_ad::help()
{
	US_Help *online_help; online_help = new US_Help(this);
	online_help->show_help("manual/finite-ad.html");
}

int US_Finite_ad::plot_analysis()
{
	return(0);
}

void US_Finite_ad::update_plot(const unsigned int first_residual)
{
...
void US_Finite_ad::load_fit(const QString &filename)
{
	QFile f(filename);
	fitName = filename;
	QString str;
