...
void
setCheckable (QActionGroup *ag)
{
  QList<QAction *> tmplist = ag->actions ();
  for (QList<QAction *>::iterator it = tmplist.begin (); it != tmplist.end ();
       ++it)
...
void
DesignerWorkaround::setupWorkaround (QMainWindow *mw)
{
  // actions

  openFile
      = new QAction (QIcon (":/images/fileopen.png"), tr ("&Open File"), mw);
  openFile->setShortcut (QKeySequence (tr ("CTRL+O")));
  openMeshFile = new QAction (QIcon (":/images/filecell.png"),
                              tr ("Open FEM File"), mw);
  ...
  psurfaceCB->setToolTip (
      tr ("Display parametric surface. Turn Lighting on for better quality."));
