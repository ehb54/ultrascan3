...
bool
Mesh2MainWindow::connectA (const QObject *sender, const char *slot)
{
#if QT_VERSION < 0x040000
...
bool
Mesh2MainWindow::connectAG (const QObject *sender, const char *slot)
{
#if QT_VERSION < 0x040000
...
Mesh2MainWindow::Mesh2MainWindow (QWidget *parent) : DummyBase (parent)
{
...
#if QT_VERSION < 0x040000 // todo - restore, when Qt4 re-implements preview
                          // functionality
  ...
  if (!pmhandler)
    pmhandler = (PixmapWriter *)IO::outputHandler (
        "jpeg"); // Qt4 naming scheme change
  if (pmhandler)
...
#else
  QString s = QFileDialog::getOpenFileName (this, "", "../../data",
                                            "GridData Files (*.mes *.MES)");
#endif
...
void
Mesh2MainWindow::createFunction (QString const &name)
{
...
void
Mesh2MainWindow::createPSurface (QString const &name)
{
  dataWidget->makeCurrent ();
...
#if QT_VERSION < 0x040000
  QString data (
      QFileDialog::getOpenFileName ("../../data", "nodes (*.nod)", this));
  QString edges (QFileDialog::getOpenFileName (
      "../../data", "connectivities (*.cel)", this));
#else
  QString data (
      QFileDialog::getOpenFileName (this, "", "../../data", "nodes (*.nod)"));
  QString edges (QFileDialog::getOpenFileName (this, "", "../../data",
                                               "connectivities (*.cel)"));
#endif
