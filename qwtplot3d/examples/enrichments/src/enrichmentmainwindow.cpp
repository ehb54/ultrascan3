...
public:
  ...
  name () const
  {
    return "$\\frac{1}{x^2+y^2+\\frac{1}{2}}$";
  }
};
    ...
    : DummyBase (parent)
{
  ...
  level_ = 0.5;
  width_ = 0.007;
  connect (levelSlider, SIGNAL (valueChanged (int)), this,
           SLOT (setLevel (int)));
...
void
EnrichmentMainWindow::setLevel (int i)
{
  level_ = 1 - i / 100.;
...
void
EnrichmentMainWindow::setWidth (int i)
{
  width_ = i / 20000.;
  bar->configure (width_, level_);
  plot->updateData ();
