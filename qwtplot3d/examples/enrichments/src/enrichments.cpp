...
void
Bar::configure (double rad, double level)
{
  plot = 0;
  radius_ = rad;
  level_ = level;
...
Bar::drawBegin ()
{
  diag_ = (plot->hull ().maxVertex - plot->hull ().minVertex).length ()
          * radius_;
  glLineWidth (0);
...
void
Bar::draw (Qwt3D::Triple const &pos)
{
...
void
Label3D::draw (Qwt3D::Triple const &pos, double w, double h)
{
  double gap = 0.3;
  glColor3d (1, 1, 1);
  glBegin (GL_QUADS);
