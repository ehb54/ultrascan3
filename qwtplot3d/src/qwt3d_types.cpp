...
int ccw(coordinate_type** P, int i, int j, int k) {
  coordinate_type a = P[i][0] - P[j][0], b = P[i][1] - P[j][1],
                  c = P[k][0] - P[j][0], d = P[k][1] - P[j][1];
...
int make_chain(coordinate_type** V, int n,
...
void Qwt3D::convexhull2d(std::vector<unsigned>& idx,
...
unsigned Qwt3D::tesselationSize(CellField const& t) {
  ...
  for (unsigned i = 0; i != t.size(); ++i) ret += t[i].size();
