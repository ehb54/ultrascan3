...
IO::Functor *
VectorWriter::clone () const
{
  return new VectorWriter (*this);
}

VectorWriter::VectorWriter ()
    : gl2ps_format_ (GL2PS_EPS), formaterror_ (false),
#ifdef GL2PS_HAVE_ZLIB
      compressed_ (true),
#else
      compressed_ (false),
#endif
      sortmode_ (SIMPLESORT), landscape_ (VectorWriter::AUTO),
      textmode_ (VectorWriter::PIXEL), texfname_ ("")
{
}
...
/*!
  ...
  TEX - high quality and exact positioning, arbitrary TeX strings as content
  for the saved labels are possible. The disadvantage is the need for an
  additionally TeX run to get the final output.\n
  ...
  will be generated, where "OUTPUT.FOR" describes the file name argument for
  IO::save().\n\n (04/05/27: On Linux platforms, pdflatex seems a file named
  'dump_0.pdf.tex' mistakenly to identify as PDF file.)
*/
void
VectorWriter::setTextMode (TEXTMODE val, QString fname)
{
  textmode_ = val;
...
#ifdef GL2PS_HAVE_ZLIB
//! Turns compressed output on or off (no effect if zlib support has not been
//! set)
void
VectorWriter::setCompressed (bool val)
{
  compressed_ = val;
}
#else
...
//! set)
void
VectorWriter::setCompressed (bool)
{
  compressed_ = false;
}
#endif
...
*/
bool
VectorWriter::setFormat (QString const &format)
{
...
bool
VectorWriter::operator() (Plot3D *plot, QString const &fname)
{
  if (formaterror_)
    return false;
  ...
  GLint options = GL2PS_SIMPLE_LINE_OFFSET | GL2PS_SILENT
                  | GL2PS_DRAW_BACKGROUND | GL2PS_OCCLUSION_CULL
                  | GL2PS_BEST_ROOT;
  ...
  while (state == GL2PS_OVERFLOW)
    {
      ...
      gl2psBeginPage ("---", QWT3DLOCAL8BIT (producer), viewport,
                      gl2ps_format_, sortmode, options, GL_RGBA, 0, NULL, 0, 0,
      ...
      while (state == GL2PS_OVERFLOW)
        {
          bufsize += 2 * 1024 * 1024;
          gl2psBeginPage ("---", QWT3DLOCAL8BIT (producer), viewport,
                          GL2PS_TEX, sortmode, options, GL_RGBA, 0, NULL, 0, 0,
                          0, bufsize, fp, QWT3DLOCAL8BIT (fn));
...
GLint
Qwt3D::setDevicePointSize (GLfloat val)
{
  if (val < 0)
    val = 0;
...
GLint
Qwt3D::drawDevicePixels (GLsizei width, GLsizei height, GLenum format,
                         GLenum type, const void *pixels)
{
  ...
  GLfloat *convertedpixel
      = (GLfloat *)malloc (3 * (long int)width * height * sizeof (GLfloat));
  if (!convertedpixel)
    return GL2PS_ERROR;
  ...
  GLint ret = gl2psDrawPixels (width, height, 0, 0, GL_RGB, GL_FLOAT,
                               convertedpixel);
  free (convertedpixel);
  return ret;
}
...
GLint
Qwt3D::drawDeviceText (const char *str, const char *fontname, int fontsize,
                       Triple pos, RGBA /*rgba*/, ANCHOR align, double gap)
{
  double vp[3];
