...
/*!
  ...
  the saved labels are possible. The disadvantage is the need for an
...
bool VectorWriter::operator()(Plot3D* plot, QString const& fname) {
  ...
  while (state == GL2PS_OVERFLOW) {
    bufsize += 2 * 1024 * 1024;
    gl2psBeginPage("---", QWT3DLOCAL8BIT(producer), viewport, gl2ps_format_,
                   sortmode, options, GL_RGBA, 0, NULL, 0, 0, 0, bufsize, fp,
  ...
  if (textmode_ == TEX) {
    ...
    while (state == GL2PS_OVERFLOW) {
      bufsize += 2 * 1024 * 1024;
      gl2psBeginPage("---", QWT3DLOCAL8BIT(producer), viewport, GL2PS_TEX,
                     sortmode, options, GL_RGBA, 0, NULL, 0, 0, 0, bufsize, fp,
                     QWT3DLOCAL8BIT(fn));
...
GLint Qwt3D::drawDevicePixels(GLsizei width, GLsizei height, GLenum format,
  ...
  GLint ret =
      gl2psDrawPixels(width, height, 0, 0, GL_RGB, GL_FLOAT, convertedpixel);
  free(convertedpixel);
  return ret;
}

GLint Qwt3D::drawDeviceText(const char* str, const char* fontname, int fontsize,
                            Triple pos, RGBA /*rgba*/, ANCHOR align,
                            double gap) {
  double vp[3];
