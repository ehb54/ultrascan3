...
void CrossHair::configure(double rad, double linewidth, bool smooth,
                          bool boxed) {
  plot = 0;
  radius_ = rad;
...
void CrossHair::draw(Qwt3D::Triple const& pos) {
  ...
  double diag =
      (plot->hull().maxVertex - plot->hull().minVertex).length() * radius_;
...
*/
void Arrow::configure(int segs, double relconelength, double relconerad,
                      double relstemrad) {
  plot = 0;
  segments_ = segs;
  oldstate_ = GL_FALSE;
