...
void
CrossHair::configure (double rad, double linewidth, bool smooth, bool boxed)
{
  plot = 0;
  radius_ = rad;
...
void
CrossHair::draw (Qwt3D::Triple const &pos)
{
  RGBA rgba = (*plot->dataColor ()) (pos);
  glColor4d (rgba.r, rgba.g, rgba.b, rgba.a);

  double diag = (plot->hull ().maxVertex - plot->hull ().minVertex).length ()
                * radius_;
...
void
Dot::configure (double pointsize, bool smooth)
{
  plot = 0;
...
void
Dot::draw (Qwt3D::Triple const &pos)
{
  RGBA rgba = (*plot->dataColor ()) (pos);
  glColor4d (rgba.r, rgba.g, rgba.b, rgba.a);
...
void
Cone::configure (double rad, unsigned quality)
{
  plot = 0;
  radius_ = rad;
  quality_ = quality;
...
void
Cone::draw (Qwt3D::Triple const &pos)
{
  RGBA rgba = (*plot->dataColor ()) (pos);
  glColor4d (rgba.r, rgba.g, rgba.b, rgba.a);
...
/**
\param segs number of faces for the fields arrows (see the gallery for
examples)
...
*/
void
Arrow::configure (int segs, double relconelength, double relconerad,
                  double relstemrad)
{
  plot = 0;
  segments_ = segs;
...
void
Arrow::draw (Qwt3D::Triple const &pos)
{
  Triple end = top_;
  Triple beg = pos;
...
//! transform a vector on the z axis with length |beg-end|, to get them in
//! coincidence with the vector(beg,end)
/**
...
*/
double
Arrow::calcRotation (Triple &axis, FreeVector const &vec)
{
