...
Scale::Scale ()
    : start_p (0.), stop_p (0.), majorintervals_p (0), minorintervals_p (0),
...
index, an empty QString else.
*/
QString
Scale::ticLabel (unsigned int idx) const
{
...
void
Scale::setLimits (double start, double stop)
{
  if (start < stop)
    {
      start_p = start;
...
void
Scale::setMajorLimits (double start, double stop)
{
  if (start < stop)
    {
      mstart_p = start;
...
*/
int
Scale::autoscale (double &a, double &b, double start, double stop, int ivals)
{
  a = start;
  b = stop;
  return ivals;
...
int
LinearScale::autoscale (double &a, double &b, double start, double stop,
                        int ivals)
{
  ...
  for (i = 1; i <= majorintervals_p; ++i)
    {
      ...
      if (runningval > stop_p)
        break;
      if (isPracticallyZero (mstart_p,
                             -t * interval)) // prevent rounding errors near 0
        runningval = 0.0;
...
void
LogScale::setupCounter (double &k, int &step)
{
...
are accepted as arguments. They will produce mantissa sets of
{2,3,4,5,6,7,8,9}, {2,4,6,8}, {2,5} or {5} respectively.
*/
void
LogScale::setMinors (int val)
{
...
QString
LogScale::ticLabel (unsigned int idx) const
{
  if (idx < majors_p.size ())
    {
