...
/**
        The returned value is not affected by resolution(). The pair gives
(columns,rows) for grid data , (number of cells,1) for free formed data
(datatype() == POLYGON) and (0,0) else
*/
pair<int, int>
...
void
SurfacePlot::createEnrichment (Enrichment &p)
{
  if (!actualData_p)
    return;
