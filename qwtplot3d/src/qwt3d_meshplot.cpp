          ...
          for (unsigned j = 0; j != actualDataC_->cells[i].size (); ++j)
            {
              ...
              glVertex3d (actualDataC_->nodes[idx].x,
                          actualDataC_->nodes[idx].y,
                          actualDataC_->nodes[idx].z);
              glNormal3d (actualDataC_->normals[idx].x,
                          actualDataC_->normals[idx].y,
...
void
SurfacePlot::setColorFromVertexC (int node, bool skip)
{
  if (skip)
    return;

  RGBA col = (*datacolor_p) (actualDataC_->nodes[node].x,
                             actualDataC_->nodes[node].y,
              ...
              if ((val >= nodes[m].z && val <= nodes[mm].z)
                  || (val >= nodes[mm].z && val <= nodes[m].z))
                {
                  ...
                  intersection.push_back (Triple (
                      nodes[m].x + lambda * (nodes[mm].x - nodes[m].x),
...
SurfacePlot::createNormalsC ()
{
  ...
  double diag
      = (actualData_p->hull ().maxVertex - actualData_p->hull ().minVertex)
            .length ()
        * normalLength ();
...
*/
bool
SurfacePlot::loadFromData (TripleField const &data, CellField const &poly)
{
  actualDataG_->clear ();
          ...
          for (unsigned j = 0; j < poly[i].size (); ++j)
            {
              ...
              u = actualDataC_->nodes[poly[i][jj]]
                  - actualDataC_->nodes[poly[i][j]];
              v = actualDataC_->nodes[poly[i][pjj]]
                  - actualDataC_->nodes[poly[i][j]];
