...
void
Label::setColor (double r, double g, double b, double a)
{
...
void
Label::setColor (Qwt3D::RGBA rgba)
{
...
Label::update ()
{
  ...
  QRect r = QRect (
      QPoint (0, 0),
      fm.size (Qwt3D::SingleLine,
               text_)); // fm.boundingRect(text_)  misbehaviour under linux;
  ...
  if (pm_.isNull ()) // else crash under linux
    {
      r = QRect (QPoint (0, 0),
                 fm.size (Qwt3D::SingleLine,
                          QString (" "))); // draw empty space else //todo
#if QT_VERSION < 0x040000
      r.moveBy (0, -r.top ());
#else
      r.translate (0, -r.top ());
...
/**
Adds an additional shift to the anchor point. This happens in a more or less
intelligent manner depending on the nature of the anchor:
\verbatim
anchor type         shift
...
\endverbatim
The unit is user space dependend (one pixel on screen - play around to get
satisfying results)
*/
void
...
Label::convert2screen ()
{
  ...
  switch (anchor_)
    ...
    case TopLeft:
      ...
      break;
    case TopCenter:
      beg_ = ViewPort2World (start
                             - Triple (width () / 2, height () + gap_, 0));
      break;
    case CenterLeft:
...
Label::draw ()
{
  ...
  if (devicefonts_)
    {
      drawDeviceText (QWT3DLOCAL8BIT (text_), "Courier", font_.pointSize (),
                      pos_, color, anchor_, gap_);
    }
  else
    {
