...
void Label::init(const QString& family, int pointSize, int weight,
                 bool italic) {
  init();
...
void Label::setFont(const QString& family, int pointSize, int weight,
                    bool italic) {
  font_ = QFont(family, pointSize, weight, italic);
  flagforupdate_ = true;
}
...
void Label::draw() {
  ...
  if (devicefonts_) {
    drawDeviceText(QWT3DLOCAL8BIT(text_), "Courier", font_.pointSize(), pos_,
                   color, anchor_, gap_);
  } else {
