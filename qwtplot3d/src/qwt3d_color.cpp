...
void
StandardColor::reset (unsigned size)
{
...
/**
        Assigns a new ColorVector (Also overwrites the constructors size
   argument)
*/
void
StandardColor::setColorVector (ColorVector const &cv)
{
  colors_ = cv;
}
...
RGBA
StandardColor::operator() (double, double, double z) const
{
  Q_ASSERT (data_);
  int index
      = (int)((colors_.size () - 1) * (z - data_->hull ().minVertex.z)
