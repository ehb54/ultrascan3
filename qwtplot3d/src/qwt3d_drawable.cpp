...
Drawable::detach (Drawable *dr)
{
  std::list<Drawable *>::iterator it
      = std::find (dlist.begin (), dlist.end (), dr);
...
Drawable::ViewPort2World (Triple win, bool *err)
{
  ...
  int res = gluUnProject (win.x, win.y, win.z, modelMatrix, projMatrix,
                          viewport, &obj.x, &obj.y, &obj.z);
...
Drawable::World2ViewPort (Triple obj, bool *err)
{
  ...
  getMatrices (modelMatrix, projMatrix, viewport);
  int res = gluProject (obj.x, obj.y, obj.z, modelMatrix, projMatrix, viewport,
                        &win.x, &win.y, &win.z);
...
Drawable::draw ()
{
  ...
  for (std::list<Drawable *>::iterator it = dlist.begin (); it != dlist.end ();
       ++it)
    {
      (*it)->draw ();
    }
