...
Axis::drawLabel ()
{
  ...
  Triple pos = ViewPort2World (
      World2ViewPort (center + ticOrientation () * lmaj_) + bnumber);
...
bool
Axis::prepTicCalculation (Triple &startpoint)
{
  ...
  if (autoScale ())
    {
      setMajors (
          scale_->autoscale (autostart_, autostop_, start_, stop_, majors ()));
...
void
Axis::drawTicLabel (Triple pos, int mtic)
{
  if (!drawNumbers_ || (mtic < 0))
    return;

  markerLabel_[mtic].setFont (numberfont_.family (), numberfont_.pointSize (),
                              numberfont_.weight (), numberfont_.italic ());
...
void
Axis::setNumberFont (QString const &family, int pointSize, int weight,
                     bool italic)
{
...
void
Axis::setLabelFont (QString const &family, int pointSize, int weight,
                    bool italic)
{
...
{
  setLabelFont (font.family (), font.pointSize (), font.weight (),
                font.italic ());
}
...
Axis::biggestNumberString ()
{
  ...
  for (unsigned i = 0; i != size; ++i)
    {
      width = fabs ((World2ViewPort (markerLabel_[i].second ())
                     - World2ViewPort (markerLabel_[i].first ()))
                        .x);
      height = fabs ((World2ViewPort (markerLabel_[i].second ())
                      - World2ViewPort (markerLabel_[i].first ()))
                         .y);
...
*/
void
Axis::setScale (Qwt3D::SCALETYPE val)
{
  switch (val)
    {
