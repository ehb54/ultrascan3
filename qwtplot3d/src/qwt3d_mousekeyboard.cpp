...
*/
void
Plot3D::mouseReleaseEvent (QMouseEvent *)
{
  mpressed_ = false;
}
...
void
Plot3D::setRotationMouse (MouseState bstate, double accel, QPoint diff)
{
  // Rotation
...
void
Plot3D::setScaleMouse (MouseState bstate, double accel, QPoint diff)
{
  // Scale
  double w = max (1, width ());
  double h = max (1, height ());
...
void
Plot3D::setShiftMouse (MouseState bstate, double accel, QPoint diff)
{
  // Shift
  double w = max (1, width ());
  double h = max (1, height ());
...
/**
        Sets the key/mousebutton combination for data/coordinatesystem moves
   inside the widget\n\n default behaviour:\n
        ...
        zoom:                 Qt::LeftButton | Qt::AltButton |
   Qt::ControlButton shifting along x:     Qt::LeftButton | Qt::ControlButton
        shifting along y:     Qt::LeftButton | Qt::ControlButton
        \endverbatim

        mouseMoveEvent() evaluates this function - if overridden, their
   usefulness becomes somehow limited
*/
void
Plot3D::assignMouse (MouseState xrot, MouseState yrot, MouseState zrot,
...
/**
The function has no effect if you derive from Plot3D and overrides the mouse
Function too careless. In this case check first against mouseEnabled() in your
version of mouseMoveEvent() and wheelEvent(). A more fine grained input control
can be achieved by combining assignMouse() with enableMouse().
*/
void
Plot3D::enableMouse (bool val)
{
...
*/
void
Plot3D::disableMouse (bool val)
{
  mouse_input_enabled_ = !val;
}
bool
Plot3D::mouseEnabled () const
{
  return mouse_input_enabled_;
}

void
Plot3D::keyPressEvent (QKeyEvent *e)
{
  if (!keyboardEnabled ())
    {
      e->ignore ();
...
void
Plot3D::setRotationKeyboard (KeyboardState kseq, double speed)
{
  // Rotation
  double w = max (1, width ());
  double h = max (1, height ());
...
void
Plot3D::setScaleKeyboard (KeyboardState kseq, double speed)
{
  // Scale
  double w = max (1, width ());
  double h = max (1, height ());
...
void
Plot3D::setShiftKeyboard (KeyboardState kseq, double speed)
{
  // Shift
  double w = max (1, width ());
  double h = max (1, height ());
...
/**
        Sets the keybutton combination for data/coordinatesystem moves inside
   the widget\n\n default behaviour:\n
...
*/
void
Plot3D::assignKeyboard (KeyboardState xrot_n, KeyboardState xrot_p,
...
/**
The function has no effect if you derive from Plot3D and overrides the keyboard
Functions too careless. In this case check first against keyboardEnabled() in
your version of keyPressEvent() A more fine grained input control can be
achieved by combining assignKeyboard() with enableKeyboard().
*/
void
Plot3D::enableKeyboard (bool val)
{
...
*/
void
Plot3D::disableKeyboard (bool val)
{
  kbd_input_enabled_ = !val;
}
bool
Plot3D::keyboardEnabled () const
{
...
*/
void
Plot3D::setKeySpeed (double rot, double scale, double shift)
{
  if (rot > 0)
    kbd_rot_speed_ = rot;
  if (scale > 0)
...
void
Plot3D::keySpeed (double &rot, double &scale, double &shift) const
{
  rot = kbd_rot_speed_;
