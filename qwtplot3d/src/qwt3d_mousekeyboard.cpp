...
void Plot3D::setRotationMouse(MouseState bstate, double accel, QPoint diff) {
  ...
  if (bstate == xrot_mstate_) new_xrot = round(xRotation() + relyz) % 360;
  if (bstate == yrot_mstate_) new_yrot = round(yRotation() + relx) % 360;
  if (bstate == zrot_mstate_) new_zrot = round(zRotation() + relx) % 360;
...
void Plot3D::assignMouse(MouseState xrot, MouseState yrot, MouseState zrot,
                         MouseState xscale, MouseState yscale,
                         MouseState zscale, MouseState zoom, MouseState xshift,
                         MouseState yshift) {
  xrot_mstate_ = xrot;
  yrot_mstate_ = yrot;
  zrot_mstate_ = zrot;
...
void Plot3D::setRotationKeyboard(KeyboardState kseq, double speed) {
  ...
  if (kseq == xrot_kstate_[0]) new_xrot = round(xRotation() + relyz) % 360;
  if (kseq == xrot_kstate_[1]) new_xrot = round(xRotation() - relyz) % 360;
  if (kseq == yrot_kstate_[0]) new_yrot = round(yRotation() + relx) % 360;
  if (kseq == yrot_kstate_[1]) new_yrot = round(yRotation() - relx) % 360;
  if (kseq == zrot_kstate_[0]) new_zrot = round(zRotation() + relx) % 360;
  if (kseq == zrot_kstate_[1]) new_zrot = round(zRotation() - relx) % 360;
...
/**
   ...
   the widget\n\n default behaviour:\n
        ...
        \endverbatim
*/
void Plot3D::assignKeyboard(KeyboardState xrot_n, KeyboardState xrot_p,
