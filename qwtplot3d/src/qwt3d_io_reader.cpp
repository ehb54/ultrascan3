...
int
read_char (FILE *fp, bool skipcomments = true)
{
  int c;
...
char *
read_field (FILE *fp, bool skipcomments = true)
{
  static char buf[71];
  int c, i;
...
//! set to data begin
bool
extract_info (FILE *fp, unsigned int &xmesh, unsigned int &ymesh, double &xmin,
...
bool
check_magic (FILE *fp, const char *val)
{
  char *p;
...
bool
check_type (FILE *fp, const char *val)
{
  char *p;
  if ((p = read_field (fp)) == 0)
    return false;
...
double **
allocateData (int columns, int rows)
{
...
void
deleteData (double **data, int columns)
{
  for (int i = 0; i < columns; i++)
    {
      delete[] data[i];
...
bool
NativeReader::collectInfo (FILE *&file, QString const &fname, unsigned &xmesh,
                           unsigned &ymesh, double &minx, double &maxx,
...
bool
NativeReader::operator() (Plot3D *plot, QString const &fname)
{
      ...
      for (unsigned int i = 0; i < xmesh; i++)
        {
          if (fscanf (file, "%lf", &data[i][j]) != 1)
            {
              fprintf (stderr,
                       "NativeReader::read: error in data file \"%s\"\n",
                       QWT3DLOCAL8BIT (fname));
              return false;
            }
  ...
  ((SurfacePlot *)plot)
      ->loadFromData (data, xmesh, ymesh, minx, maxx, miny, maxy);
