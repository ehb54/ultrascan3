...
void
IO::Entry::operator= (IO::Entry const &e)
{
  if (this == &e)
    return;
...
bool
IO::FormatCompare::operator() (IO::Entry const &e)
{
  return (e.fmt == e_.fmt);
}
...
bool
IO::FormatCompare2::operator() (IO::Entry const &e)
{
  return (e.fmt == s_);
}

bool
IO::add_unique (Container &l, Entry const &e)
{
...
IO::IT
IO::find (Container &l, QString const &fmt)
{
...
/*!
  ...
  Every call overwrites a formerly registered handler for the same format
  string (case sensitive).
*/
bool
IO::defineInputHandler (QString const &format, IO::Function func)
{
...
*/
bool
IO::defineInputHandler (QString const &format, IO::Functor const &func)
{
  return add_unique (rlist (), Entry (format, func));
}
 ...
 */
bool
IO::defineOutputHandler (QString const &format, IO::Function func)
{
...
*/
bool
IO::defineOutputHandler (QString const &format, IO::Functor const &func)
{
  return add_unique (wlist (), Entry (format, func));
}
...
*/
bool
IO::load (Plot3D *plot, QString const &fname, QString const &format)
{
...
*/
bool
IO::save (Plot3D *plot, QString const &fname, QString const &format)
{
...
*/
IO::Functor *
IO::inputHandler (QString const &format)
{
...
*/
IO::Functor *
IO::outputHandler (QString const &format)
{
...
bool
PixmapWriter::operator() (Plot3D *plot, QString const &fname)
{
...
void
PixmapWriter::setQuality (int val)
{
  quality_ = val;
}
...
/*!
  ...
  Writes vector data supported by gl2ps. The corresponding format types are
  "EPS","PS"or "PDF". If zlib has been configured this will be extended by
  "EPS_GZ" and "PS_GZ".
        \b Beware: BSPSORT turns out to behave very slowly and memory
  consuming, especially in cases where many polygons appear. It is still more
  exact than SIMPLESORT.
*/
bool
Plot3D::saveVector (QString const &fileName, QString const &format,
...
/*!
  ...
  Saves the framebuffer to the file fileName using one of the image file
  formats supported by Qt.
*/
bool
Plot3D::savePixmap (QString const &fileName, QString const &format)
{
  if (format == "EPS" || format == "EPS_GZ" || format == "PS"
      || format == "PS_GZ" || format == "PDF")
    return false;
...
*/
bool
Plot3D::save (QString const &fileName, QString const &format)
{
  return IO::save (this, fileName, format);
}
