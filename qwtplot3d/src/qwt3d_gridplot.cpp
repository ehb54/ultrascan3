...
void SurfacePlot::setColorFromVertexG(int ix, int iy, bool skip) {
   ...
   RGBA col = (*datacolor_p)(
      actualDataG_->vertices[ ix ][ iy ][ 0 ], actualDataG_->vertices[ ix ][ iy ][ 1 ],
...
void SurfacePlot::createNormalsG() {
   ...
   for (int i = 0; i <= actualDataG_->columns() - step; i += step) {
      for (int j = 0; j <= actualDataG_->rows() - step; j += step) {
         basev = Triple(
            actualDataG_->vertices[ i ][ j ][ 0 ], actualDataG_->vertices[ i ][ j ][ 1 ],
...
void SurfacePlot::readIn(
   GridData &gdata, double **data, unsigned int columns, unsigned int rows, double minx, double maxx, double miny,
   double maxy) {
...
void SurfacePlot::Data2FloorG() {
   ...
   for (int i = 0; i < actualDataG_->columns() - step; i += step) {
      ...
      for (int j = 0; j < actualDataG_->rows() - step; j += step) {
         ...
         glVertex3d(
            actualDataG_->vertices[ i + step ][ j + step ][ 0 ], actualDataG_->vertices[ i + step ][ j + step ][ 1 ],
            zshift);
      }
      glEnd();
   }
   ...
   for (int k = 0; k != isolines(); ++k) {
      ...
      for (int i = 0; i < cols - step; i += step) {
         for (int j = 0; j < rows - step; j += step) {
            ...
            t[ 1 ] = Triple(
               actualDataG_->vertices[ i + step ][ j ][ 0 ], actualDataG_->vertices[ i + step ][ j ][ 1 ],
               actualDataG_->vertices[ i + step ][ j ][ 2 ]);
            t[ 2 ] = Triple(
               actualDataG_->vertices[ i + step ][ j + step ][ 0 ], actualDataG_->vertices[ i + step ][ j + step ][ 1 ],
               actualDataG_->vertices[ i + step ][ j + step ][ 2 ]);
            t[ 3 ] = Triple(
               actualDataG_->vertices[ i ][ j + step ][ 0 ], actualDataG_->vertices[ i ][ j + step ][ 1 ],
