...
SurfacePlot::createDataG ()
{
  ...
  if (plotStyle () == FILLEDMESH || plotStyle () == WIREFRAME
      || plotStyle () == HIDDENLINE)
    {
      glColor4d (meshColor ().r, meshColor ().g, meshColor ().b,
                 meshColor ().a);
...
void
SurfacePlot::setColorFromVertexG (int ix, int iy, bool skip)
{
  if (skip)
    return;

  RGBA col = (*datacolor_p) (actualDataG_->vertices[ix][iy][0],
...
SurfacePlot::createNormalsG ()
{
  ...
  double diag
      = (actualDataG_->hull ().maxVertex - actualDataG_->hull ().minVertex)
            .length ()
        * normalLength ();
  ...
  for (int i = 0; i <= actualDataG_->columns () - step; i += step)
    {
      for (int j = 0; j <= actualDataG_->rows () - step; j += step)
        {
          basev = Triple (actualDataG_->vertices[i][j][0],
                          actualDataG_->vertices[i][j][1],
                          actualDataG_->vertices[i][j][2]);
          topv = Triple (
              actualDataG_->vertices[i][j][0] + actualDataG_->normals[i][j][0],
              actualDataG_->vertices[i][j][1] + actualDataG_->normals[i][j][1],
              actualDataG_->vertices[i][j][2]
                  + actualDataG_->normals[i][j][2]);
...
void
SurfacePlot::readIn (GridData &gdata, Triple **data, unsigned int columns,
                     unsigned int rows)
{
...
void
SurfacePlot::readIn (GridData &gdata, double **data, unsigned int columns,
                     unsigned int rows, double minx, double maxx, double miny,
                     double maxy)
{
...
void
SurfacePlot::calcNormals (GridData &gdata)
{
...
void
SurfacePlot::sewPeriodic (GridData &gdata)
{
  // sewing
...
*/
bool
SurfacePlot::loadFromData (Triple **data, unsigned int columns,
                           unsigned int rows, bool uperiodic, bool vperiodic)
{
  actualDataC_->clear ();
...
*/
bool
SurfacePlot::loadFromData (double **data, unsigned int columns,
                           unsigned int rows, double minx, double maxx,
  ...
  for (int i = 0; i < actualDataG_->columns () - step; i += step)
    {
      ...
      glVertex3d (actualDataG_->vertices[i][0][0],
                  actualDataG_->vertices[i][0][1], zshift);

      setColorFromVertexG (i + step, 0);
      glVertex3d (actualDataG_->vertices[i + step][0][0],
                  actualDataG_->vertices[i + step][0][1], zshift);
      for (int j = 0; j < actualDataG_->rows () - step; j += step)
        {
...
void
SurfacePlot::Isolines2FloorG ()
{
  if (isolines () <= 0 || actualData_p->empty ())
    return;

  double count
      = (actualData_p->hull ().maxVertex.z - actualData_p->hull ().minVertex.z)
        / isolines ();
          ...
          for (int j = 0; j < rows - step; j += step)
            {
              ...
              for (int m = 0; m != 4; ++m)
                {
                  int mm = (m + 1) % 4;
                  if ((val >= t[m].z && val <= t[mm].z)
                      || (val >= t[mm].z && val <= t[m].z))
                    {
                      diff = t[mm].z - t[m].z;
                      ...
                      intersection.push_back (
                          Triple (t[m].x + lambda * (t[mm].x - t[m].x),
                  ...
                  if (intersection.size () > 2)
                    {
                      ...
                      for (unsigned dd = 0; dd != intersection.size (); ++dd)
                        {
                          glVertex3d (intersection[dd].x, intersection[dd].y,
                                      zshift);
                        }
                      glEnd ();
                      glBegin (GL_POINTS);
                      glVertex3d (intersection[0].x, intersection[0].y,
                                  zshift);
                      glEnd ();
                    }
                  else if (intersection.size () == 2)
                    {
                      ...
                      glVertex3d (intersection[0].x, intersection[0].y,
                                  zshift);
                      glVertex3d (intersection[1].x, intersection[1].y,
                                  zshift);
