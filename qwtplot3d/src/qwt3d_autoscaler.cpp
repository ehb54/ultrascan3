...
double
floorExt (int &exponent, double x, std::vector<double> &sortedmantissi)
{
  if (x == 0.0)
    {
      exponent = 0;
...
*/
double
floor125 (int &exponent, double x)
{
...
/**
        Switchs start and stop, if stop < start and sets intervals = 1 if ivals
   < 1
*/
void
LinearAutoScaler::init (double start, double stop, int ivals)
{
  start_ = start;
  stop_ = stop;
  intervals_ = ivals;
...
*/
double
LinearAutoScaler::anchorvalue (double start, double m, int n)
{
...
\verbatim
                          -l_intervals * i    -2 * i    -i +r_intervals * i
                                                                                   |
|______|_______ _ _ _ ____|____|___ _ _ _ _ _ _ _|_______|_______|_ _ _ _ _ _
_____|__|_____ |                  |                                      | | 0
i := m*10^n         start                                  anchor stop

c 'minimal' (anchor-start < m*10^n)
\endverbatim
*/
int
LinearAutoScaler::segments (int &l_intervals, int &r_intervals, double start,
                            double stop, double anchor, double m, int n)
{
...
/*!
        ...
        \return Number of intervals after rescaling. This will in the most
  cases differ from the requested interval number!  Always >0.
...
*/
int
LinearAutoScaler::execute (double &a, double &b, double start, double stop,
                           int ivals)
{
