...
CoordinateSystem::init (Triple first, Triple second)
{
  ...
  axes[Z1].setPosition (first + Triple (0, dv.y, 0),
                        first + Triple (0, dv.y, dv.z)); // back left z
  ...
  // remaining x axes
  axes[X2].setPosition (first + Triple (0, 0, dv.z),
                        first + Triple (dv.x, 0, dv.z));         // front top x
  ...
  axes[Y4].setPosition (first + Triple (0, 0, dv.z),
                        first + Triple (0, dv.y, dv.z)); // top left y
  ...
  axes[Z4].setPosition (first + Triple (dv.x, dv.y, 0),
                        second); // back right z
...
//! build convex hull (6 axes: 2 x, 2 y, 2 z) and choose one of them at a time
//! for scales, labels etc.
void
CoordinateSystem::chooseAxes ()
{
  ...
  for (unsigned k = 0; k != idx.size (); ++k)
    {
      ...
      unsigned int next
          = idx[(k + 1) % idx.size ()]; // next point in cv (considered as ring
                                        // buffer of points)
                  ...
                  if (rem_x >= 0) // schon zweite Achse der konvexen Huelle ?
                    {
                      ...
                      double y = min (min (end[rem_x].y, end[i].y),
                                      min (beg[rem_x].y, beg[i].y));
                  ...
                  if (rem_y >= 0)
                    {
                      ...
                      double y = min (min (end[rem_y].y, end[i].y),
                                      min (beg[rem_y].y, beg[i].y));
                  ...
                  if (rem_z >= 0)
                    {
                      ...
                      double z = max (max (end[rem_z].z, end[i].z),
                                      max (beg[rem_z].z, beg[i].z));
  ...
  // fit z axis in - the onthewall axis if the decorated axes build a continous
  // line, the opposite else
  if (choice_x >= 0 && choice_y >= 0 && choice_z >= 0)
    {
      left = (beg[choice_z].x < beg[other_z].x
              || end[choice_z].x < end[other_z].x)
                 ? true
                 : false;
...
CoordinateSystem::autoDecorateExposedAxis (Axis &ax, bool left)
{
  ...
  if (left) // leftmost (compared with antagonist in CV)  axis -> draw
...
void
CoordinateSystem::recalculateAxesTics ()
{
  for (unsigned i = 0; i != axes.size (); ++i)
...
void
CoordinateSystem::setNumberFont (QString const &family, int pointSize,
...
void
CoordinateSystem::setStandardScale ()
{
  for (unsigned i = 0; i != axes.size (); ++i)
    axes[i].setScale (LINEARSCALE);
}
...
void
CoordinateSystem::setLabelFont (QString const &family, int pointSize,
                                int weight, bool italic)
{
...
void
CoordinateSystem::setStyle (COORDSTYLE s, AXIS frame_1, AXIS frame_2,
                            AXIS frame_3)
{
  style_ = s;
...
*/
void
CoordinateSystem::setGridLines (bool majors, bool minors, int sides)
{
  sides_ = sides;
...
CoordinateSystem::drawMajorGridLines ()
{
  ...
  glColor4d (gridlinecolor_.r, gridlinecolor_.g, gridlinecolor_.b,
             gridlinecolor_.a);
...
void
CoordinateSystem::drawMajorGridLines (Axis &a0, Axis &a1)
{
  Triple d = a1.begin () - a0.begin ();

  for (unsigned int i = 0; i != a0.majorPositions ().size (); ++i)
    {
      glVertex3d (a0.majorPositions ()[i].x, a0.majorPositions ()[i].y,
                  a0.majorPositions ()[i].z);
      glVertex3d (a0.majorPositions ()[i].x + d.x,
                  a0.majorPositions ()[i].y + d.y,
                  a0.majorPositions ()[i].z + d.z);
    }
}

void
CoordinateSystem::drawMinorGridLines (Axis &a0, Axis &a1)
{
  Triple d = a1.begin () - a0.begin ();

  for (unsigned int i = 0; i != a0.minorPositions ().size (); ++i)
    {
      glVertex3d (a0.minorPositions ()[i].x, a0.minorPositions ()[i].y,
                  a0.minorPositions ()[i].z);
      glVertex3d (a0.minorPositions ()[i].x + d.x,
                  a0.minorPositions ()[i].y + d.y,
