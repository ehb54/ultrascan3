...
void CoordinateSystem::chooseAxes() {
      ...
      if ((one == beg[i] && two == end[i]) ||
        ...
        if (i == X1 || i == X2 || i == X3 || i == X4)  // x Achsen
        {
          if (rem_x >= 0)  // schon zweite Achse der konvexen Huelle ?
          {
            ...
            double y =
                min(min(end[rem_x].y, end[i].y), min(beg[rem_x].y, beg[i].y));
          ...
          if (rem_y >= 0) {
            ...
            double y =
                min(min(end[rem_y].y, end[i].y), min(beg[rem_y].y, beg[i].y));
          ...
          if (rem_z >= 0) {
            ...
            double z =
                max(max(end[rem_z].z, end[i].z), max(beg[rem_z].z, beg[i].z));
  ...
  if (choice_x >= 0 && choice_y >= 0 && choice_z >= 0) {
    left =
        (beg[choice_z].x < beg[other_z].x || end[choice_z].x < end[other_z].x)
            ? true
            : false;
...
void CoordinateSystem::setNumberFont(QString const& family, int pointSize,
...
void CoordinateSystem::setLabelFont(QString const& family, int pointSize,
                                    int weight, bool italic) {
...
void CoordinateSystem::setStyle(COORDSTYLE s, AXIS frame_1, AXIS frame_2,
                                AXIS frame_3) {
  style_ = s;
