...
static void gl2psListRealloc(GL2PSlist *list, GLint n) {
   ...
   if (!list->array) {
      ...
      if (n > list->nmax) {
         ...
         list->array = ( char * ) gl2psRealloc(list->array, ( long int ) list->nmax * list->size);
      }
   }
}
...
static void gl2psConvertPixmapToPNG(GL2PSimage *pixmap, GL2PSlist *png) {
   ...
   png_set_IHDR(
      png_ptr, info_ptr, pixmap->width, pixmap->height, 8, PNG_COLOR_TYPE_RGB, PNG_INTERLACE_NONE,
...
static GLboolean gl2psSamePosition(GL2PSxyz p1, GL2PSxyz p2) {
   if (!GL2PS_ZERO(p1[ 0 ] - p2[ 0 ]) || !GL2PS_ZERO(p1[ 1 ] - p2[ 1 ]) || !GL2PS_ZERO(p1[ 2 ] - p2[ 2 ]))
      return GL_FALSE;
   return GL_TRUE;
}
...
static void gl2psGetPlane(GL2PSprimitive *prim, GL2PSplane plane) {
   ...
   switch (prim->type) {
      ...
      case GL2PS_QUADRANGLE:
         ...
         if (
            (GL2PS_ZERO(v[ 0 ]) && GL2PS_ZERO(v[ 1 ]) && GL2PS_ZERO(v[ 2 ]))
            ...
            plane[ 3 ] = -plane[ 0 ] * prim->verts[ 0 ].xyz[ 0 ] - plane[ 1 ] * prim->verts[ 0 ].xyz[ 1 ]
...
static void gl2psCreateSplitPrimitive(
   GL2PSprimitive *parent, GL2PSplane plane, GL2PSprimitive *child, GLshort numverts, GLshort *index0, GLshort *index1) {
   GLshort i;
...
static void gl2psTraverseBspTree(
   GL2PSbsptree *tree, GL2PSxyz eye, GLfloat epsilon, GLboolean (*compare)(GLfloat f1, GLfloat f2),
...
static void gl2psAddPolyPrimitive(
   GLshort type, GLshort numverts, GL2PSvertex *verts, GLint offset, GLushort pattern, GLint factor, GLfloat width,
   char boundary) {
   GL2PSprimitive *prim;
...
static void gl2psPrintPostScriptHeader(void) {
   ...
   if (gl2ps->options & GL2PS_DRAW_BACKGROUND) {
      gl2psPrintf(
         ...
         "closepath fill\n",
         gl2ps->bgcolor[ 0 ], gl2ps->bgcolor[ 1 ], gl2ps->bgcolor[ 2 ], ( int ) gl2ps->viewport[ 0 ],
         ( int ) gl2ps->viewport[ 1 ], ( int ) gl2ps->viewport[ 2 ], ( int ) gl2ps->viewport[ 1 ],
         ( int ) gl2ps->viewport[ 2 ], ( int ) gl2ps->viewport[ 3 ], ( int ) gl2ps->viewport[ 0 ],
         ( int ) gl2ps->viewport[ 3 ]);
   }
}
...
static void gl2psPrintPostScriptPrimitive(void *data) {
   ...
   switch (prim->type) {
      ...
      case GL2PS_LINE:
         if (
            !gl2psSamePosition(gl2ps->lastvertex.xyz, prim->verts[ 0 ].xyz)
      ...
      case GL2PS_TRIANGLE:
         if (!gl2psVertsSameColor(prim)) {
            ...
            gl2psPrintf(
               ...
               prim->verts[ 2 ].xyz[ 1 ], prim->verts[ 2 ].rgba[ 0 ], prim->verts[ 2 ].rgba[ 1 ],
               prim->verts[ 2 ].rgba[ 2 ], prim->verts[ 1 ].xyz[ 0 ], prim->verts[ 1 ].xyz[ 1 ],
               prim->verts[ 1 ].rgba[ 0 ], prim->verts[ 1 ].rgba[ 1 ], prim->verts[ 1 ].rgba[ 2 ],
               prim->verts[ 0 ].xyz[ 0 ], prim->verts[ 0 ].xyz[ 1 ], prim->verts[ 0 ].rgba[ 0 ],
      ...
      case GL2PS_IMAGEMAP:
         if (prim->data.image->type != GL2PS_IMAGEMAP_WRITTEN) {
            ...
            gl2psPrintPostScriptImagemap(
               prim->data.image->pixels[ 0 ], prim->data.image->pixels[ 1 ], prim->data.image->width,
      ...
      case GL2PS_TEXT:
         ...
         gl2psPrintf(
            "%g %g %d /%s ", prim->verts[ 0 ].xyz[ 0 ], prim->verts[ 0 ].xyz[ 1 ], prim->data.text->fontsize,
...
static void gl2psPrintTeXPrimitive(void *data) {
   ...
   switch (prim->type) {
      case GL2PS_TEXT:
         ...
         fprintf(
            gl2ps->stream, "\\textcolor[rgb]{%g,%g,%g}{{%s}}", prim->verts[ 0 ].rgba[ 0 ], prim->verts[ 0 ].rgba[ 1 ],
...
static void gl2psPDFgroupListInit(void) {
   ...
   for (i = 0; i < gl2psListNbr(gl2ps->pdfprimlist); ++i) {
      ...
      switch (p->type) {
         ...
         case GL2PS_LINE:
            if (
               lasttype != p->type || lastwidth != p->width || lastpattern != p->pattern || lastfactor != p->factor
      ...
      switch (prim->type) {
         case GL2PS_POINT:
            ...
            for (j = 0; j <= lastel; ++j) {
               ...
               gl2ps->streamlength += gl2psPrintf(
                  "%f %f m %f %f l\n", prim->verts[ 0 ].xyz[ 0 ], prim->verts[ 0 ].xyz[ 1 ], prim->verts[ 0 ].xyz[ 0 ],
                  prim->verts[ 0 ].xyz[ 1 ]);
            }
            ...
            if (t.prop & T_CONST_COLOR && t.prop & T_ALPHA_1) {
               ...
               for (j = 0; j <= lastel; ++j) {
                  ...
                  gl2ps->streamlength += gl2psPrintf(
                     ...
                     "%f %f l\n"
                     "h f\n",
                     t.vertex[ 0 ].xyz[ 0 ], t.vertex[ 0 ].xyz[ 1 ], t.vertex[ 1 ].xyz[ 0 ], t.vertex[ 1 ].xyz[ 1 ],
...
static int gl2psPrintPDFShaderStreamDataCoord(
   GL2PSvertex *vertex, size_t (*action)(unsigned long data, size_t size), GLfloat dx, GLfloat dy, GLfloat xmin,
   GLfloat ymin) {
   int offs = 0;
   unsigned long imap;
   GLfloat diff;
...
static int gl2psPrintPDFShaderStreamDataAlpha(
   GL2PSvertex *vertex, size_t (*action)(unsigned long data, size_t size), int sigbyte) {
   int offs = 0;
   unsigned long imap;
   double dmax = ~1UL;
...
static int gl2psPrintPDFShaderStreamData(
   ...
   size_t (*action)(unsigned long data, size_t size), int gray) {
   int i, offs = 0;
   GL2PSvertex v;
...
static int gl2psPrintPDFShader(int obj, GL2PStriangle *triangles, int size, int gray) {
   ...
   offs += fprintf(
      ...
      obj, (gray) ? "/DeviceGray" : "/DeviceRGB", (gray) ? gray : 8, xmin, xmax, ymin, ymax, (gray) ? "" : "0 1 0 1");
   ...
   if (gl2ps->options & GL2PS_COMPRESS) {
      ...
      for (i = 0; i < size; ++i)
         gl2psPrintPDFShaderStreamData(
            &triangles[ i ], xmax - xmin, ymax - ymin, xmin, ymin, gl2psWriteBigEndianCompress, gray);
   ...
   if (!done) {
      ...
      offs += fprintf(
         ...
         vertexbytes * 3 * size);
      for (i = 0; i < size; ++i)
         offs += gl2psPrintPDFShaderStreamData(
            &triangles[ i ], xmax - xmin, ymax - ymin, xmin, ymin, gl2psWriteBigEndian, gray);
   }
...
static int gl2psPrintPDFShaderMask(int obj, int childobj) {
   ...
   offs += fprintf(
      ...
      ">>\n",
      obj, ( int ) gl2ps->viewport[ 0 ], ( int ) gl2ps->viewport[ 1 ], ( int ) gl2ps->viewport[ 2 ],
      ( int ) gl2ps->viewport[ 3 ]);
...
static void gl2psPrintSVGSmoothTriangle(GL2PSxyz xyz[ 3 ], GL2PSrgba rgba[ 3 ]) {
   ...
   if (gl2psSameColorThreshold(3, rgba, gl2ps->threshold)) {
      ...
      gl2psPrintf(
         "points=\"%g,%g %g,%g %g,%g\"/>\n", xyz[ 0 ][ 0 ], xyz[ 0 ][ 1 ], xyz[ 1 ][ 0 ], xyz[ 1 ][ 1 ], xyz[ 2 ][ 0 ],
         xyz[ 2 ][ 1 ]);
   }
   else {
...
#else
   gl2psMsg(
      GL2PS_WARNING, "GL2PS has to be compiled with PNG support in "
   ...
   switch (prim->type) {
      ...
      case GL2PS_TEXT:
         ...
         gl2psPrintf(
            ...
            col, xyz[ 0 ][ 0 ], xyz[ 0 ][ 1 ], prim->data.text->fontsize, prim->data.text->fontname,
            prim->data.text->str);
         break;
      case GL2PS_SPECIAL:
         /* alignment contains the format for which the special output text
...
static void gl2psPrintSVGBeginViewport(GLint viewport[ 4 ]) {
   ...
   gl2psPrintf(
      "  <polygon points=\"%d,%d %d,%d %d,%d %d,%d\"/>\n", x, gl2ps->viewport[ 3 ] - y, x + w, gl2ps->viewport[ 3 ] - y,
      x + w, gl2ps->viewport[ 3 ] - (y + h), x, gl2ps->viewport[ 3 ] - (y + h));
...
static void gl2psPrintPGFPrimitive(void *data) {
   ...
   switch (prim->type) {
      ...
      case GL2PS_TEXT:
         ...
         fprintf(
            gl2ps->stream, "\\pgfnode{rectangle}{%s}{\\fontsize{%d}{0}\\selectfont",
...
GL2PSDLL_API GLint gl2psBeginPage(
   const char *title, const char *producer, GLint viewport[ 4 ], GLint format, GLint sort, GLint options,
   GLint colormode, GLint colorsize, GL2PSrgba *colormap, GLint nr, GLint ng, GLint nb, GLint buffersize, FILE *stream,
   const char *filename) {
   GLint index;
   int i;
   ...
   if (!gl2ps->viewport[ 2 ] || !gl2ps->viewport[ 3 ]) {
      gl2psMsg(
         GL2PS_ERROR, "Incorrect viewport (x=%d, y=%d, width=%d, height=%d)", gl2ps->viewport[ 0 ],
...
GL2PSDLL_API GLint gl2psDrawPixels(
   ...
   if ((format != GL_RGB && format != GL_RGBA) || type != GL_FLOAT) {
      gl2psMsg(
         GL2PS_ERROR, "gl2psDrawPixels only implemented for "
...
GL2PSDLL_API GLint
gl2psDrawImageMap(GLsizei width, GLsizei height, const GLfloat position[ 3 ], const unsigned char *imagemap) {
   int size, i;
