...
static void
gl2psMsg (GLint level, const char *fmt, ...)
{
  va_list args;
...
static void *
gl2psRealloc (void *ptr, size_t size)
{
  if (!size)
    return (NULL);
...
static size_t
gl2psWriteBigEndian (unsigned long data, size_t bytes)
{
  size_t i;
...
static int
gl2psAllocCompress (unsigned int srcsize)
{
  gl2psFreeCompress ();
...
static void *
gl2psReallocCompress (unsigned int srcsize)
{
  if (!gl2ps->compress || !srcsize)
    return NULL;
...
static size_t
gl2psWriteBigEndianCompress (unsigned long data, size_t bytes)
{
  size_t i;
  size_t size = sizeof (unsigned long);
  for (i = 1; i <= bytes; ++i)
  ...
  if (gl2ps->options & GL2PS_COMPRESS)
    {
      if (Z_OK != gl2psDeflate ())
      ...
      else
        {
          ...
          fwrite (gl2ps->compress->dest + n,
                  gl2ps->compress->destLen - (n + 4), 1, gl2ps->stream);
...
static void
gl2psListRealloc (GL2PSlist *list, GLint n)
{
  if (!list)
    {
      ...
      if (n > list->nmax)
        {
          ...
          list->array = (char *)gl2psRealloc (list->array, (long int)list->nmax
                                                               * list->size);
        }
    }
}
...
static GL2PSlist *
gl2psListCreate (GLint n, GLint incr, GLint size)
{
  GL2PSlist *list;
...
static void
gl2psListDelete (GL2PSlist *list)
{
  if (!list)
    return;
...
static void
gl2psListAdd (GL2PSlist *list, void *data)
{
  if (!list)
    {
...
static void *
gl2psListPointer (GL2PSlist *list, GLint index)
{
  if (!list)
    {
...
static void
gl2psListSort (GL2PSlist *list, int (*fcmp) (const void *a, const void *b))
{
  if (!list)
    return;
...
static void
gl2psListAction (GL2PSlist *list, void (*action) (void *data))
{
  GLint i;
...
static void
gl2psListActionInverse (GL2PSlist *list, void (*action) (void *data))
{
  GLint i;
...
static void
gl2psListRead (GL2PSlist *list, int index, void *data)
{
  if ((index < 0) || (index >= list->n))
...
static void
gl2psEncodeBase64Block (unsigned char in[3], unsigned char out[4], int len)
{
...
static void
gl2psListEncodeBase64 (GL2PSlist *list)
{
...
static GLboolean
gl2psSameColor (GL2PSrgba rgba1, GL2PSrgba rgba2)
{
...
static GLboolean
gl2psVertsSameColor (const GL2PSprimitive *prim)
{
  int i;
...
static GLboolean
gl2psSameColorThreshold (int n, GL2PSrgba rgba[], GL2PSrgba threshold)
{
  int i;
...
static void
gl2psSetLastColor (GL2PSrgba rgba)
{
  int i;
  for (i = 0; i < 3; ++i)
    {
...
static GLfloat
gl2psGetRGB (GL2PSimage *im, GLuint x, GLuint y, GLfloat *red, GLfloat *green,
             GLfloat *blue)
{
...
static void
gl2psUserWritePNG (png_structp png_ptr, png_bytep data, png_size_t length)
{
  unsigned int i;
...
static void
gl2psUserFlushPNG (png_structp png_ptr)
{
}

static void
gl2psConvertPixmapToPNG (GL2PSimage *pixmap, GL2PSlist *png)
{
  png_structp png_ptr;
  png_infop info_ptr;
  ...
  png_set_write_fn (png_ptr, (void *)png, gl2psUserWritePNG,
                    gl2psUserFlushPNG);
  ...
  row_data = (unsigned char *)gl2psMalloc (3 * pixmap->width
...
static GLint
gl2psAddText (GLint type, const char *str, const char *fontname,
              GLshort fontsize, GLint alignment, GLfloat angle)
{
  ...
  prim->data.text->str
      = (char *)gl2psMalloc ((strlen (str) + 1) * sizeof (char));
...
static void
gl2psFreeText (GL2PSstring *text)
{
  if (!text)
    return;
  gl2psFree (text->str);
...
static GLboolean
gl2psSupportedBlendMode (GLenum sfactor, GLenum dfactor)
{
...
static void
gl2psAdaptVertexForBlending (GL2PSvertex *v)
{
...
static void
gl2psAssignTriangleProperties (GL2PStriangle *t)
{
  /* int i; */
...
static void
gl2psFillTriangleFromPrimitive (GL2PStriangle *t, GL2PSprimitive *p,
...
static void
gl2psInitTriangle (GL2PStriangle *t)
{
  int i;
  GL2PSvertex vertex
...
static GL2PSprimitive *
gl2psCopyPrimitive (GL2PSprimitive *p)
{
  GL2PSprimitive *prim;
  ...
  prim->width = p->width;
  prim->verts
      = (GL2PSvertex *)gl2psMalloc (p->numverts * sizeof (GL2PSvertex));
...
static GLboolean
gl2psSamePosition (GL2PSxyz p1, GL2PSxyz p2)
{
...
static GLfloat
gl2psComparePointPlane (GL2PSxyz point, GL2PSplane plane)
{
...
static void
gl2psPvec (GLfloat *a, GLfloat *b, GLfloat *c)
{
  c[0] = a[1] * b[2] - a[2] * b[1];
  c[1] = a[2] * b[0] - a[0] * b[2];
  c[2] = a[0] * b[1] - a[1] * b[0];
...
static void
gl2psGetNormal (GLfloat *a, GLfloat *b, GLfloat *c)
{
  GLfloat norm;
...
static void
gl2psGetPlane (GL2PSprimitive *prim, GL2PSplane plane)
{
...
static void
gl2psCutEdge (GL2PSvertex *a, GL2PSvertex *b, GL2PSplane plane, GL2PSvertex *c)
{
  GL2PSxyz v;
  GLfloat sect;
...
static void
gl2psCreateSplitPrimitive (GL2PSprimitive *parent, GL2PSplane plane,
...
static void
gl2psAddIndex (GLshort *index0, GLshort *index1, GLshort *nb, GLshort i,
               GLshort j)
{
  GLint k;
...
static GLshort
gl2psGetIndex (GLshort i, GLshort num)
{
  return (i < num - 1) ? i + 1 : 0;
}

static GLint
gl2psTestSplitPrimitive (GL2PSprimitive *prim, GL2PSplane plane)
{
...
static GLint
gl2psSplitPrimitive (GL2PSprimitive *prim, GL2PSplane plane,
...
static void
gl2psDivideQuad (GL2PSprimitive *quad, GL2PSprimitive **t1,
                 GL2PSprimitive **t2)
{
  ...
  *t2 = (GL2PSprimitive *)gl2psMalloc (sizeof (GL2PSprimitive));
  ...
  (*t1)->boundary
      = ((quad->boundary & 4) ? 2 : 0) | ((quad->boundary & 4) ? 2 : 0);
}

static int
gl2psCompareDepth (const void *a, const void *b)
{
  GL2PSprimitive *q, *w;
...
static int
gl2psTrianglesFirst (const void *a, const void *b)
{
  GL2PSprimitive *q, *w;
...
static void
gl2psFreeImagemap (GL2PSimagemap *list)
{
  GL2PSimagemap *next;
  while (list != NULL)
    {
...
static void
gl2psAddPrimitiveInList (GL2PSprimitive *prim, GL2PSlist *list)
{
...
static void
gl2psFreeBspTree (GL2PSbsptree **tree)
{
  if (*tree)
    {
      if ((*tree)->back)
...
static GLboolean
gl2psGreater (GLfloat f1, GLfloat f2)
{
  if (f1 > f2)
    return GL_TRUE;
  else
...
static void
gl2psBuildBspTree (GL2PSbsptree *tree, GL2PSlist *primitives)
{
      ...
      if (i != index)
        {
          ...
          switch (
              gl2psSplitPrimitive (prim, tree->plane, &frontprim, &backprim))
            {
...
static void
gl2psTraverseBspTree (GL2PSbsptree *tree, GL2PSxyz eye, GLfloat epsilon,
...
static GLint
gl2psGetPlaneFromPoints (GL2PSxyz a, GL2PSxyz b, GL2PSplane plane)
{
  GLfloat n;
...
static void
gl2psFreeBspImageTree (GL2PSbsptree2d **tree)
{
  if (*tree)
    {
      if ((*tree)->back)
...
static GLint
gl2psCheckPoint (GL2PSxyz point, GL2PSplane plane)
{
  GLfloat pt_dis;
...
static void
gl2psAddPlanesInBspTreeImage (GL2PSprimitive *prim, GL2PSbsptree2d **tree)
{
  GLint ret = 0;
  GLint i;
  GLint offset = 0;
      ...
      switch (ret)
        ...
        case GL2PS_POINT_INFRONT:
          ...
          for (i = 1 + offset; i < prim->numverts - 1; i++)
            {
              if (cur->front == NULL)
                {
                  cur->front = (GL2PSbsptree2d *)gl2psMalloc (
          ...
          if (cur->front == NULL)
            {
              cur->front
                  = (GL2PSbsptree2d *)gl2psMalloc (sizeof (GL2PSbsptree2d));
            }
          if (gl2psGetPlaneFromPoints (prim->verts[i].xyz,
        ...
        case GL2PS_POINT_BACK:
          ...
          if (gl2psGetPlaneFromPoints (prim->verts[offset].xyz,
                                       prim->verts[i].xyz, cur->front->plane))
            {
              cur->front->front = NULL;
              cur->front->back = NULL;
            }
...
static GLint
gl2psCheckPrimitive (GL2PSprimitive *prim, GL2PSplane plane)
{
  GLint i;
  GLint pos;
...
static GL2PSprimitive *
gl2psCreateSplitPrimitive2D (GL2PSprimitive *parent, GLshort numverts,
                             GL2PSvertex *vertx)
{
  GLint i;
  GL2PSprimitive *child
      = (GL2PSprimitive *)gl2psMalloc (sizeof (GL2PSprimitive));
...
static void
gl2psSplitPrimitive2D (GL2PSprimitive *prim, GL2PSplane plane,
                       GL2PSprimitive **front, GL2PSprimitive **back)
{
      ...
      if (((prev == -1) || (prev == cur) || (prev == 0) || (cur == 0))
          && (i < prim->numverts))
        {
          if (cur == GL2PS_POINT_INFRONT)
            ...
            {
              back_count++;
              back_list = (GL2PSvertex *)gl2psRealloc (
                  back_list, sizeof (GL2PSvertex) * back_count);
...
static GLint
gl2psAddInBspImageTree (GL2PSprimitive *prim, GL2PSbsptree2d **tree)
{
  GLint ret = 0;
  ...
  if (*tree == NULL)
    {
      if ((prim->type != GL2PS_IMAGEMAP)
          && (GL_FALSE == gl2ps->zerosurfacearea))
        {
...
static void
gl2psAddBoundaryInList (GL2PSprimitive *prim, GL2PSlist *list)
{
  GL2PSprimitive *b;
  GLshort i;
  GL2PSxyz c;
  ...
  for (i = 0; i < prim->numverts; i++)
    {
      ...
      b->verts[1].xyz[2] = prim->verts[gl2psGetIndex(i, prim->numverts)].xyz[2];
          ...
          b->verts[0].xyz[2] = prim->verts[i].xyz[2];
          b->verts[1].xyz[0]
              = prim->verts[gl2psGetIndex (i, prim->numverts)].xyz[0];
          b->verts[1].xyz[1]
              = prim->verts[gl2psGetIndex (i, prim->numverts)].xyz[1];
          b->verts[1].xyz[2]
              = prim->verts[gl2psGetIndex (i, prim->numverts)].xyz[2];
#endif
...
static void
gl2psBuildPolygonBoundary (GL2PSbsptree *tree)
{
  GLint i;
  GL2PSprimitive *prim;
...
static void
gl2psAddPolyPrimitive (GLshort type, GLshort numverts, GL2PSvertex *verts,
                       GLint offset, GLushort pattern, GLint factor,
...
static GLint
gl2psGetVertex (GL2PSvertex *v, GLfloat *p)
{
  GLint i;
...
static void
gl2psParseFeedbackBuffer (GLint used)
{
  char flag;
  GLushort pattern = 0;
  GLboolean boundary;
  ...
  while (used > 0)
    {
      ...
      switch ((GLint)*current)
        ...
        case GL_POINT_TOKEN:
          ...
          current += i;
          used -= i;
          gl2psAddPolyPrimitive (GL2PS_POINT, 1, vertices, 0, pattern, factor,
                                 psize, 0);
          break;
        case GL_LINE_TOKEN:
        case GL_LINE_RESET_TOKEN:
          ...
          current += i;
          used -= i;
          gl2psAddPolyPrimitive (GL2PS_LINE, 2, vertices, 0, pattern, factor,
                                 lwidth, 0);
          break;
        ...
        case GL_PASS_THROUGH_TOKEN:
          switch ((GLint)current[1])
            ...
            case GL2PS_IMAGEMAP_TOKEN:
              ...
              prim->numverts = 4;
              prim->verts
                  = (GL2PSvertex *)gl2psMalloc (4 * sizeof (GL2PSvertex));
              prim->culled = 0;
              prim->offset = 0;
              prim->pattern = 0;
              prim->factor = 0;
              ...
              prim->verts[0].xyz[0] = prim->verts[0].xyz[0]
                                      - (int)(node->image->width / 2) + 0.5;
              prim->verts[0].xyz[1] = prim->verts[0].xyz[1]
                                      - (int)(node->image->height / 2) + 0.5;
              for (i = 1; i < 4; i++)
                {
                  for (v = 0; v < 3; v++)
                    {
              ...
              prim->verts[1].xyz[0]
                  = prim->verts[1].xyz[0] + node->image->width;
              prim->verts[2].xyz[0] = prim->verts[1].xyz[0];
              prim->verts[2].xyz[1]
                  = prim->verts[2].xyz[1] + node->image->height;
              ...
              for (i = 0; i < vtot; i += sizeoffloat)
                {
                  ...
                  used -= 2;
                  if ((vtot - i) >= 4)
                    memcpy (&(((char *)(node->image->pixels))[i + v]),
...
static void
gl2psWriteByte (unsigned char byte)
{
...
static void
gl2psPrintPostScriptPixmap (GLfloat x, GLfloat y, GL2PSimage *im)
{
...
static void
gl2psPrintPostScriptImagemap (GLfloat x, GLfloat y, GLsizei width,
                              GLsizei height, const unsigned char *imagemap)
{
  int i, size;
...
static void
gl2psPrintPostScriptHeader (void)
{
  time_t now;
  ...
  gl2psPrintf ("/BD { bind def } bind def\n"
               "/C  { setrgbcolor } BD\n"
               "/G  { 0.082 mul exch 0.6094 mul add exch 0.3086 mul add neg "
               "1.0 add setgray } BD\n"
               "/W  { setlinewidth } BD\n");
  ...
  gl2psPrintf (
      ...
      "/SCLR{ gsave FCT moveto rotate 0 SH -2 div rmoveto show grestore} "
      "BD\n");
  gl2psPrintf (
      "/SCCR{ gsave FCT moveto rotate SW -2 div SH -2 div rmoveto show "
      "grestore} BD\n"
  ...
  gl2psPrintf ("/STshfill {\n"
               "      /b1 exch def /g1 exch def /r1 exch def /y1 exch def /x1 "
               "exch def\n"
               "      /b2 exch def /g2 exch def /r2 exch def /y2 exch def /x2 "
               "exch def\n"
               "      /b3 exch def /g3 exch def /r3 exch def /y3 exch def /x3 "
               "exch def\n"
               "      gsave << /ShadingType 4 /ColorSpace [/DeviceRGB]\n"
               "      /DataSource [ 0 x1 y1 r1 g1 b1 0 x2 y2 r2 g2 b2 0 x3 y3 "
               "r3 g3 b3 ] >>\n"
               "      shfill grestore } BD\n");
...
static void
gl2psPrintPostScriptColor (GL2PSrgba rgba)
{
...
static void
gl2psResetPostScriptColor (void)
{
  gl2ps->lastrgba[0] = gl2ps->lastrgba[1] = gl2ps->lastrgba[2] = -1.;
}

static void
gl2psEndPostScriptLine (void)
{
  int i;
...
static void
gl2psParseStipplePattern (GLushort pattern, GLint factor, int *nb,
                          int array[10])
{
  int i, n;
...
static int
gl2psPrintPostScriptDash (GLushort pattern, GLint factor, char *str)
{
...
static void
gl2psPrintPostScriptPrimitive (void *data)
{
  int newline;
  GL2PSprimitive *prim;
  ...
  switch (prim->type)
    ...
    case GL2PS_TEXT:
      ...
      gl2psPrintf ("%g %g %d /%s ", prim->verts[0].xyz[0],
                   prim->verts[0].xyz[1], prim->data.text->fontsize,
                   prim->data.text->fontname);
...
static void
gl2psPrintPostScriptFooter (void)
{
  gl2psPrintf ("grestore\n"
               "showpage\n"
               "cleartomark\n"
...
static void
gl2psPrintPostScriptBeginViewport (GLint viewport[4])
{
  GLint index;
  GLfloat rgba[4];
...
static GLint
gl2psPrintPostScriptEndViewport (void)
{
  GLint res;
...
static void
gl2psPrintPostScriptFinalPrimitive (void)
{
...
static void
gl2psPrintTeXPrimitive (void *data)
{
  GL2PSprimitive *prim;
...
static void
gl2psPrintTeXBeginViewport (GLint viewport[4])
{
  glRenderMode (GL_FEEDBACK);
...
static GLint
gl2psPrintTeXEndViewport (void)
{
  return gl2psPrintPrimitives ();
}

static void
gl2psPrintTeXFinalPrimitive (void)
{
}
...
static int
gl2psPrintPDFCompressorType (void)
{
#if defined(GL2PS_HAVE_ZLIB)
  if (gl2ps->options & GL2PS_COMPRESS)
    {
...
static int
gl2psPrintPDFStrokeColor (GL2PSrgba rgba)
{
  int i, offs = 0;
...
static int
gl2psPrintPDFFillColor (GL2PSrgba rgba)
{
  int i, offs = 0;
...
static int
gl2psPrintPDFLineWidth (GLfloat lw)
{
  if (GL2PS_ZERO (lw))
...
static void
gl2psPutPDFText (GL2PSstring *text, int cnt, GLfloat x, GLfloat y)
{
...
static void
gl2psPutPDFImage (GL2PSimage *image, int cnt, GLfloat x, GLfloat y)
{
  gl2ps->streamlength
      += gl2psPrintf ("q\n"
                      "%d 0 0 %d %f %f cm\n"
...
static void
gl2psPDFgroupObjectInit (GL2PSpdfgroup *gro)
{
  if (!gro)
    return;
...
static void
gl2psPDFgroupListWriteMainStream (void)
{
  int i, j, lastel;
  ...
  for (i = 0; i < gl2psListNbr (gl2ps->pdfgrouplist); ++i)
    {
      ...
      switch (prim->type)
        ...
        case GL2PS_POINT:
          ...
          gl2ps->streamlength
              += gl2psPrintPDFStrokeColor (prim->verts[0].rgba);
          for (j = 0; j <= lastel; ++j)
            {
          ...
          if (t.prop & T_CONST_COLOR && t.prop & T_ALPHA_1)
            {
              ...
              for (j = 0; j <= lastel; ++j)
                {
                  prim
                      = *(GL2PSprimitive **)gl2psListPointer (gro->ptrlist, j);
          ...
          /* Const alpha < 1 and const color: Simple PDF draw orders
          ...
          else if (t.prop & T_CONST_COLOR && t.prop & T_ALPHA_LESS_1)
            {
              gl2ps->streamlength += gl2psPrintf ("q\n"
                                                  ...
                                                  gro->gsno);
              gl2ps->streamlength
                  += gl2psPrintPDFFillColor (prim->verts[0].rgba);
              for (j = 0; j <= lastel; ++j)
                {
                  prim
                      = *(GL2PSprimitive **)gl2psListPointer (gro->ptrlist, j);
        ...
        case GL2PS_PIXMAP:
          for (j = 0; j <= lastel; ++j)
            {
              prim = *(GL2PSprimitive **)gl2psListPointer (gro->ptrlist, j);
              gl2psPutPDFImage (prim->data.image, gro->imno,
                                prim->verts[0].xyz[0], prim->verts[0].xyz[1]);
            }
          break;
        case GL2PS_TEXT:
          for (j = 0; j <= lastel; ++j)
            {
              ...
              gl2ps->streamlength
                  += gl2psPrintPDFFillColor (prim->verts[0].rgba);
              gl2psPutPDFText (prim->data.text, gro->fontno,
                               prim->verts[0].xyz[0], prim->verts[0].xyz[1]);
            }
          break;
        default:
...
static int
gl2psPDFgroupListWriteGStateResources (void)
{
  GL2PSpdfgroup *gro;
  int offs = 0;
  int i;
...
static int
gl2psPDFgroupListWriteShaderResources (void)
{
  GL2PSpdfgroup *gro;
  int offs = 0;
  int i;
...
static int
gl2psPDFgroupListWriteXObjectResources (void)
{
  int i;
  GL2PSprimitive *p = NULL;
  GL2PSpdfgroup *gro;
...
static int
gl2psPDFgroupListWriteFontResources (void)
{
  int i;
  GL2PSpdfgroup *gro;
  int offs = 0;
...
static int
gl2psOpenPDFDataStreamWritePreface (void)
{
  int offs;
...
static void
gl2psPrintPDFPrimitive (void *data)
{
  GL2PSprimitive *prim = *(GL2PSprimitive **)data;
  ...
  if (gl2ps->options & GL2PS_COMPRESS)
    {
      if (Z_OK != gl2psDeflate ())
        ...
        fwrite (gl2ps->compress->dest, gl2ps->compress->destLen, 1,
                gl2ps->stream);
...
static int
gl2psPrintPDFDataStreamLength (int val)
{
...
static int
gl2psPDFgroupListWriteVariableResources (void)
{
  int offs = 0;
...
gl2psPrintPDFShaderStreamDataCoord (
    GL2PSvertex *vertex, size_t (*action) (unsigned long data, size_t size),
    GLfloat dx, GLfloat dy, GLfloat xmin, GLfloat ymin)
{
  int offs = 0;
  unsigned long imap;
  GLfloat diff;
...
static int
gl2psPrintPDFShaderStreamDataRGB (GL2PSvertex *vertex,
...
static int
gl2psPrintPDFShaderStreamDataAlpha (GL2PSvertex *vertex,
                                    size_t (*action) (unsigned long data,
                                                      size_t size),
                                    int sigbyte)
{
...
gl2psPrintPDFShaderStreamData (
    GL2PStriangle *triangle, GLfloat dx, GLfloat dy, GLfloat xmin,
    GLfloat ymin, size_t (*action) (unsigned long data, size_t size), int gray)
{
  int i, offs = 0;
  GL2PSvertex v;
  ...
  for (i = 0; i < 3; ++i)
    {
      ...
      if (gray)
        ...
        {
          offs += gl2psPrintPDFShaderStreamDataRGB (&triangle->vertex[i],
                                                    action);
        }
    }
...
static void
gl2psPDFRectHull (GLfloat *xmin, GLfloat *xmax, GLfloat *ymin, GLfloat *ymax,
...
static int
gl2psPrintPDFShader (int obj, GL2PStriangle *triangles, int size, int gray)
{
  ...
  offs += fprintf (gl2ps->stream,
                   ...
                   obj, (gray) ? "/DeviceGray" : "/DeviceRGB",
                   (gray) ? gray : 8, xmin, xmax, ymin, ymax,
                   (gray) ? "" : "0 1 0 1");
  ...
  if (!done)
    {
      ...
      offs += fprintf (gl2ps->stream,
                       ...
                       vertexbytes * 3 * size);
      for (i = 0; i < size; ++i)
        offs += gl2psPrintPDFShaderStreamData (&triangles[i], xmax - xmin,
                                               ymax - ymin, xmin, ymin,
...
static int
gl2psPrintPDFShaderMask (int obj, int childobj)
{
  int offs = 0, len;
...
static int
gl2psPrintPDFShaderExtGS (int obj, int childobj)
{
  int offs = 0;
  ...
  offs += fprintf (gl2ps->stream, "/SMask << /S /Alpha /G %d 0 R >> ",
                   childobj);
...
static int
gl2psPrintPDFShaderSimpleExtGS (int obj, GLfloat alpha)
{
  int offs = 0;
...
static int
gl2psPrintPDFPixmapStreamData (GL2PSimage *im,
                               size_t (*action) (unsigned long data,
                                                 size_t size),
                               int gray)
{
...
static int
gl2psPrintPDFPixmap (int obj, int childobj, GL2PSimage *im, int gray)
{
...
static int
gl2psPrintPDFText (int obj, GL2PSstring *s, int fontnumber)
{
  int offs = 0;
...
static int
gl2psPDFgroupListWriteObjects (int entryoffs)
{
  int i, j;
  GL2PSprimitive *p = NULL;
  GL2PSpdfgroup *gro;
  ...
  for (i = 0; i < gl2psListNbr (gl2ps->pdfgrouplist); ++i)
    {
      ...
      switch (p->type)
        ...
        case GL2PS_TRIANGLE:
          ...
          triangles
              = (GL2PStriangle *)gl2psMalloc (sizeof (GL2PStriangle) * size);
          for (j = 0; j < size; ++j)
            {
          ...
          if (triangles[0].prop & T_ALPHA_LESS_1)
            {
              ...
              offs += gl2psPrintPDFShaderSimpleExtGS (
                  gro->gsobjno, triangles[0].vertex[0].rgba[3]);
            }
          if (triangles[0].prop & T_VAR_ALPHA)
            {
              gl2ps->xreflist[gro->gsobjno] = offs;
              offs += gl2psPrintPDFShaderExtGS (gro->gsobjno,
                                                gro->trgroupobjno);
              gl2ps->xreflist[gro->trgroupobjno] = offs;
              offs += gl2psPrintPDFShaderMask (gro->trgroupobjno,
                                               gro->maskshno);
              gl2ps->xreflist[gro->maskshobjno] = offs;
              offs += gl2psPrintPDFShader (gro->maskshobjno, triangles, size,
                                           8);
            }
          gl2psFree (triangles);
          break;
        case GL2PS_PIXMAP:
          ...
          offs += gl2psPrintPDFPixmap (gro->imobjno, gro->imobjno + 1,
                                       p->data.image, 0);
          if (p->data.image->format == GL_RGBA)
            {
              gl2ps->xreflist[gro->imobjno + 1] = offs;
              offs += gl2psPrintPDFPixmap (gro->imobjno + 1, -1, p->data.image,
                                           8);
            }
          break;
        case GL2PS_TEXT:
          ...
          offs
              += gl2psPrintPDFText (gro->fontobjno, p->data.text, gro->fontno);
          break;
        case GL2PS_SPECIAL:
          /* alignment contains the format for which the special output text
             is intended */
...
static void
gl2psPrintPDFBeginViewport (GLint viewport[4])
{
  int offs = 0;
  GLint index;
  GLfloat rgba[4];
...
static void
gl2psPrintPDFFinalPrimitive (void)
{
}
...
static void
gl2psSVGGetCoordsAndColors (int n, GL2PSvertex *verts, GL2PSxyz *xyz,
                            GL2PSrgba *rgba)
{
  int i, j;
...
static void
gl2psSVGGetColorString (GL2PSrgba rgba, char str[32])
{
...
static void
gl2psPrintSVGSmoothTriangle (GL2PSxyz xyz[3], GL2PSrgba rgba[3])
{
  int i;
  GL2PSxyz xyz2[3];
  GL2PSrgba rgba2[3];
...
static void
gl2psPrintSVGDash (GLushort pattern, GLint factor)
{
  int i, n, array[10];
...
static void
gl2psPrintSVGPixmap (GLfloat x, GLfloat y, GL2PSimage *pixmap)
{
#if defined(GL2PS_HAVE_LIBPNG)
  GL2PSlist *png;
  unsigned char c;
...
static void
gl2psPrintSVGPrimitive (void *data)
{
  GL2PSprimitive *prim;
  GL2PSxyz xyz[4];
  GL2PSrgba rgba[4];
  ...
  switch (prim->type)
    ...
    case GL2PS_TEXT:
      ...
      gl2psPrintf ("<text fill=\"%s\" x=\"%g\" y=\"%g\" "
                   ...
                   prim->data.text->fontname, prim->data.text->str);
      break;
    case GL2PS_SPECIAL:
      /* alignment contains the format for which the special output text
         is intended */
...
static void
gl2psPrintSVGBeginViewport (GLint viewport[4])
{
  GLint index;
  char col[32];
  GLfloat rgba[4];
...
static void
gl2psPrintSVGFinalPrimitive (void)
{
  /* End any remaining line, if any */
  gl2psEndSVGLine ();
}
...
static void
gl2psPrintPGFColor (GL2PSrgba rgba)
{
  if (!gl2psSameColor (gl2ps->lastrgba, rgba))
    {
      gl2psSetLastColor (rgba);
...
static void
gl2psPrintPGFDash (GLushort pattern, GLint factor)
{
  int i, n, array[10];
...
static const char *
gl2psPGFTextAlignment (int align)
{
  switch (align)
    {
    case GL2PS_TEXT_C:
...
static void
gl2psPrintPGFPrimitive (void *data)
{
  GL2PSprimitive *prim;
...
static void
gl2psPrintPGFFooter (void)
{
  fprintf (gl2ps->stream, "\\end{pgfpicture}\n");
}

static void
gl2psPrintPGFBeginViewport (GLint viewport[4])
{
  GLint index;
  GLfloat rgba[4];
  int x = viewport[0], y = viewport[1], w = viewport[2], h = viewport[3];
...
static void
gl2psPrintPGFFinalPrimitive (void)
{
}
...
static void
gl2psComputeTightBoundingBox (void *data)
{
  GL2PSprimitive *prim;
  int i;
...
gl2psPrintPrimitives (void)
{
  ...
  switch (gl2ps->sort)
    ...
    case GL2PS_BSP_SORT:
      ...
      gl2ps->primitives
          = gl2psListCreate (500, 500, sizeof (GL2PSprimitive *));
      break;
    }
...
GL2PSDLL_API GLint
gl2psBeginPage (const char *title, const char *producer, GLint viewport[4],
                GLint format, GLint sort, GLint options, GLint colormode,
                GLint colorsize, GL2PSrgba *colormap, GLint nr, GLint ng,
                GLint nb, GLint buffersize, FILE *stream, const char *filename)
{
  GLint index;
  int i;
  ...
  if (gl2ps->colormode == GL_RGBA)
  ...
  else if (gl2ps->colormode == GL_COLOR_INDEX)
    {
      ...
      memcpy (gl2ps->colormap, colormap,
              gl2ps->colorsize * sizeof (GL2PSrgba));
      glGetIntegerv (GL_INDEX_CLEAR_VALUE, &index);
  ...
  if (!title)
    ...
    {
      gl2ps->title
          = (char *)gl2psMalloc ((strlen (title) + 1) * sizeof (char));
...
GL2PSDLL_API GLint
gl2psBeginViewport (GLint viewport[4])
{
  if (!gl2ps)
    return GL2PS_UNINITIALIZED;
...
GL2PSDLL_API GLint
gl2psTextOpt (const char *str, const char *fontname, GLshort fontsize,
              GLint alignment, GLfloat angle)
{
  return gl2psAddText (GL2PS_TEXT, str, fontname, fontsize, alignment, angle);
}

GL2PSDLL_API GLint
gl2psText (const char *str, const char *fontname, GLshort fontsize)
{
  return gl2psAddText (GL2PS_TEXT, str, fontname, fontsize, GL2PS_TEXT_BL,
                       0.0F);
}

GL2PSDLL_API GLint
gl2psSpecial (GLint format, const char *str)
{
  return gl2psAddText (GL2PS_SPECIAL, str, "", 0, format, 0.0F);
}

GL2PSDLL_API GLint
gl2psDrawPixels (GLsizei width, GLsizei height, GLint xorig, GLint yorig,
      ...
      if (gl2ps->options & GL2PS_NO_BLENDING || !gl2ps->blending)
        {
          ...
          prim->data.image->pixels
              = (GLfloat *)gl2psMalloc (size * sizeof (GLfloat));
          piv = (GLfloat *)pixels;
...
GL2PSDLL_API GLint
gl2psDrawImageMap (GLsizei width, GLsizei height, const GLfloat position[3],
