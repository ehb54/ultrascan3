...
static void gl2psListRealloc(GL2PSlist *list, GLint n) {
  ...
  if (!list->array) {
    ...
    if (n > list->nmax) {
      ...
      list->array =
          (char *)gl2psRealloc(list->array, (long int)list->nmax * list->size);
    }
  }
}
...
static void gl2psAddPolyPrimitive(GLshort type, GLshort numverts,
                                  ...
                                  GLushort pattern, GLint factor, GLfloat width,
                                  char boundary) {
  GL2PSprimitive *prim;
        ...
        while (count > 0 && used > 0) {
          ...
          if (v == 2) {
            if (GL_TRUE == boundary) {
              ...
              flag = 0;
            gl2psAddPolyPrimitive(GL2PS_TRIANGLE, 3, vertices, offset, pattern,
                                  factor, 1, flag);
      ...
      case GL_PASS_THROUGH_TOKEN:
        switch ((GLint)current[1]) {
          ...
          case GL2PS_IMAGEMAP_TOKEN:
            ...
            prim->verts[0].xyz[0] =
                prim->verts[0].xyz[0] - (int)(node->image->width / 2) + 0.5;
            prim->verts[0].xyz[1] =
                prim->verts[0].xyz[1] - (int)(node->image->height / 2) + 0.5;
            for (i = 1; i < 4; i++) {
              for (v = 0; v < 3; v++) {
...
static void gl2psPrintPostScriptPixmap(GLfloat x, GLfloat y, GL2PSimage *im) {
  ...
  } else if (nbit == 2) { /* color, 2 bits for r and g and b; rgbs following
                             each other */
    nrgb = width * 3;
    nbits = nrgb * nbit;
    nbyte = nbits / 8;
  ...
  } else if (nbit == 4) { /* color, 4 bits for r and g and b; rgbs following
                             each other */
    nrgb = width * 3;
    nbits = nrgb * nbit;
    nbyte = nbits / 8;
    ...
    switch (p->type) {
      case GL2PS_PIXMAP:
        ...
        if (GL_RGBA ==
          ...
          gl2ps->objects_stack++;
        offs +=
            fprintf(gl2ps->stream, "/Im%d %d 0 R\n", gro->imno, gro->imobjno);
      case GL2PS_TRIANGLE:
        if (gro->trgroupno >= 0)
...
static int gl2psPrintPDFPixmap(int obj, int childobj, GL2PSimage *im,
                               int gray) {
  ...
  switch (prim->type) {
    ...
    case GL2PS_LINE:
      ...
      if (newline) {
        gl2psSVGGetColorString(rgba[0], col);
        gl2psPrintf(
            "<polyline fill=\"none\" stroke=\"%s\" stroke-width=\"%g\" ", col,
            prim->width);
        if (rgba[0][3] < 1.0F)
...
static void gl2psPrintSVGBeginViewport(GLint viewport[4]) {
  ...
  if (gl2ps->options & GL2PS_DRAW_BACKGROUND) {
    ...
    gl2psPrintf("<polygon fill=\"%s\" points=\"%d,%d %d,%d %d,%d %d,%d\"/>\n",
                col, x, gl2ps->viewport[3] - y, x + w, gl2ps->viewport[3] - y,
                x + w, gl2ps->viewport[3] - (y + h), x,
