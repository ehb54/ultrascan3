...
namespace Qwt3D
{
#define QWT3DLOCAL8BIT(qstring) ((const char *)(qstring.local8Bit ()))
...
//! This class creates a (mouse-button,modifier) pair (ordinary typedef for int
//! if Qt3 is used)
class MouseState
{
public:
  MouseState (Qt::MouseButtons mb = Qt::NoButton,
...
//! This class creates a (key-button,modifier) pair (ordinary typedef for int
//! if Qt3 is used)
class KeyboardState
{
public:
  KeyboardState (int key = Qt::Key_unknown,
                 Qt::KeyboardModifiers km = Qt::NoModifier)
      : key_ (key), km_ (km)
  {
  }
  ...
  bool
  operator== (const KeyboardState &ms)
  {
