...
class QWT3D_EXPORT AutoScaler {
 ...
 protected:
  ...
  virtual int execute(double& a, double& b, double start, double stop,
                      int ivals) = 0;
  virtual ~AutoScaler() {}
...
class QWT3D_EXPORT LinearAutoScaler : public AutoScaler {
 ...
 private:
  ...
  int segments(int& l_intervals, int& r_intervals, double start, double stop,
               double anchor, double mantissa, int exponent);
