...
//! A flat color legend
/**
        The class visualizes a ColorVector together with a scale (axis)  and a
   caption. ColorLegends are vertical or horizontal
*/
...
public:
  ...
  enum ORIENTATION
  {
    BottomTop, //!< Positionate the legend vertically, the lowest color index
               //!< is on the bottom
    LeftRight  //!< Positionate the legend horizontally, the lowest color index
               //!< is on left side
  };

  ColorLegend (); //!< Standard constructor

  void draw (); //!< Draws the object. You should not use this explicitely -
                //!< the function is called by updateGL().

  void setRelPosition (Qwt3D::Tuple relMin,
                       Qwt3D::Tuple relMax); //!< Sets the relative position of
                                             //!< the legend inside widget
  void setOrientation (
      ORIENTATION,
      SCALEPOSITION); //!< Sets legend orientation and scale position
  ...
  void
  drawNumbers (bool val)
  {
    axis_.setNumbers (val);
  } //!< Sets whether the scale will have scale numbers.
  void
  setAutoScale (bool val); //!< Sets, whether the axis is autoscaled or not.
  ...
  void setTitleFont (QString const &family, int pointSize,
