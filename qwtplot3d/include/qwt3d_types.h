...
struct QWT3D_EXPORT Tuple
{
  ...
  Tuple (double X, double Y)
      : x (X), y (Y) {} //!< Initialize Tuple with x and y
                        //! Tuple coordinates
  double x, y;
};
...
inline const Triple
operator+ (const Triple &t, const Triple &t2)
{
  return Triple (t) += t2;
}
inline const Triple
...
inline const Triple
operator* (double d, const Triple &t)
{
  return Triple (t) *= d;
}
inline const Triple
operator* (const Triple &t, double d)
{
  return Triple (t) *= d;
}
inline const Triple
...
//! Free vector
/**
        FreeVectors represent objects like normal vectors and other vector
   fields inside R^3
*/
...
//! Holds indices in a TripleField interpreted as counterclockwise node
//! numbering for a convex polygon
...
QWT3D_EXPORT QColor GL2Qt (GLdouble r, GLdouble g,
                           GLdouble b);      //!< RGB -> QColor
...
public:
  ...
  void
  setHull (Qwt3D::ParallelEpiped const &h)
  {
    hull_p = h;
  }
...
public:
  ...
  void setSize (unsigned int columns,
                unsigned int rows); //!< destroys content and set new size,
                                    //!< elements are uninitialized

  DataMatrix vertices; //!< mesh vertices
  DataMatrix normals;  //!< mesh normals
  void
  setPeriodic (bool u, bool v)
  {
    uperiodic_ = u;
    vperiodic_ = v;
  }
...
inline Triple
normalizedcross (Triple const &u, Triple const &v)
{
  Triple n;
...
inline double
dotProduct (Triple const &u, Triple const &v)
{
  return u.x * v.x + u.y * v.y + u.z * v.z;
}

void convexhull2d (std::vector<unsigned> &idx,
