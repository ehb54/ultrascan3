...
inline void
SaveGlDeleteLists (GLuint &lstidx, GLsizei range)
{
  if (glIsList (lstidx))
...
*/
inline void
getMatrices (GLdouble *modelMatrix, GLdouble *projMatrix, GLint *viewport)
{
...
//! simplified glut routine (glUnProject): windows coordinates_p --> object
//! coordinates_p
/**
        Don't rely on (use) this in display lists !
*/
inline bool
ViewPort2World (double &objx, double &objy, double &objz, double winx,
                double winy, double winz)
{
  ...
  int res = gluUnProject (winx, winy, winz, modelMatrix, projMatrix, viewport,
                          &objx, &objy, &objz);

  return (res == GL_FALSE) ? false : true;
}

//! simplified glut routine (glProject): object coordinates_p --> windows
//! coordinates_p
/**
        Don't rely on (use) this in display lists !
*/
inline bool
World2ViewPort (double &winx, double &winy, double &winz, double objx,
                double objy, double objz)
{
  ...
  getMatrices (modelMatrix, projMatrix, viewport);
  int res = gluProject (objx, objy, objz, modelMatrix, projMatrix, viewport,
                        &winx, &winy, &winz);
