...
public:
  ...
  enum LANDSCAPEMODE
  {
    ...
    AUTO //!< The output orientation depends on the plot widgets aspect ratio
         //!< (default)
  };
  ...
  void
  setLandscape (LANDSCAPEMODE val)
  {
    landscape_ = val;
  } //!< Sets landscape mode.
  LANDSCAPEMODE
  ...
  //! Turns compressed output on or off (no effect if zlib support is not
  //! available)
...
GLint drawDevicePixels (GLsizei width, GLsizei height, GLenum format,
                        GLenum type, const void *pixels);
GLint drawDeviceText (const char *str, const char *fontname, int fontsize,
                      Qwt3D::Triple pos, Qwt3D::RGBA rgba, Qwt3D::ANCHOR align,
                      double gap);
