...
main (int argc, char *argv[])
{
  ...
  parser.addOption (
      { { "t", "test-attach" }, "test attaching to the users shm" });
  ...
  if (testattach)
    {
      ...
      // attach to existing
      {
        shmid = shmget (key, sizeof (Global), 0);
        if (shmid == -1)
          {
        ...
        if (shmds.shm_segsz != sizeof (Global))
          {
            TSO << "shmctl (attach) size mismatch " << shmds.shm_segsz
                << " vs " << sizeof (Global) << "\n";
            exit (-1);
          }
  ...
  if (createshm)
    {
      ...
      {
        // shm_open
        shmid = shmget (key, sizeof (Global), IPC_CREAT | 0600);
        if (shmid == -1)
          {
