...
bool
us_semaphore_exists (key_t key, int nsems, int &semid)
{
...
bool
us_semaphore_init (key_t key, int nsems, int &semid)
{
  int i;
  union semun arg;
  struct semid_ds buf;
  ...
  if (!strncmp (argv[1], "exists", 6))
    {
      printf ("semaphore 0x%x %d %s\n", key, semid,
              us_semaphore_exists (key, 1, semid) ? "exists"
                                                  : "does not exist");
      exit (0);
    }
