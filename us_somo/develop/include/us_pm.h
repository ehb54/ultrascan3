...
class US_PM {
 ...
 private:
  ...
  bool best_vary_one_param(unsigned int param_to_vary, vector<double> &params,
  ...
  bool best_vary_two_param(unsigned int param_to_vary_1,
                           unsigned int param_to_vary_2, vector<double> &params,
                           set<pm_point> &model, double &best_fitness);
  ...
  // refine the low_fparams, high_fparams based upon top_count population
  // members
  bool ga_refine_limits(unsigned int top_count, bool needs_sort_unique = true,
                        unsigned int extend_deltas = 1);

  bool ga_delta_to_fparams(vector<int> &delta, vector<double> &fparams);
  ...
  bool rescale_params(vector<int> &types, vector<double> &fparams,
                      double new_conversion_factor);

  bool rescale_params(vector<double> &params, vector<double> &low_fparams,
  ...
  void random_normal(double mean1, double sd1, double mean2, double sd2,
                     double &x, double &y);
  ...
  bool ga_fparams_to_individual(vector<double> &fparams,
                                pm_ga_individual &individual);
  bool ga_params_to_individual(vector<double> &params,
                               pm_ga_individual &individual);
  ...
  bool expand_types(vector<vector<int> > &types_vector, QString types,
                    bool incrementally, bool allcombinations);

  bool expand_types(vector<vector<double> > &types_vector, QString types,
                    bool incrementally, bool allcombinations);
 ...
 public:
  ...
  US_PM(double grid_conversion_factor, int max_dimension,
        unsigned int max_harmonics, vector<double> F, vector<double> q,
        vector<double> I, vector<double> e, unsigned int max_mem_in_MB = 2048,
        int debug_level = 0, bool quiet = false, US_Log *us_log = (US_Log *)0,
  ...
  void set_best_delta(double best_delta_start = 1e0,
                      double best_delta_divisor = 10e0,
                      double best_delta_min = 1e-2);
  // find best model at base 0
  bool best_md0(vector<double> &params, set<pm_point> &model,
                double finest_conversion = 1e0, double coarse_conversion = 10e0,
                double refinement_range_pct = 5e0,
                double conversion_divisor = 2e0);
  bool best_md0(vector<double> &params, vector<double> &low_fparams,
                vector<double> &high_fparams, set<pm_point> &model);

  // quick "approximate" max_d
  bool approx_max_dimension(vector<double> &params, double coarse_conversion,
  ...
  bool best_sphere(vector<double> &params, vector<double> &low_fparams,
                   vector<double> &high_fparams, set<pm_point> &model);
  bool best_cylinder(vector<double> &params, vector<double> &low_fparams,
                     vector<double> &high_fparams, set<pm_point> &model);
  bool best_spheroid(vector<double> &params, vector<double> &low_fparams,
                     vector<double> &high_fparams, set<pm_point> &model);
  bool best_ellipsoid(vector<double> &params, vector<double> &low_fparams,
                      vector<double> &high_fparams, set<pm_point> &model);
  bool best_torus(vector<double> &params, vector<double> &low_fparams,
                  vector<double> &high_fparams, set<pm_point> &model);
  bool best_torus_segment(vector<double> &params, vector<double> &low_fparams,
                          vector<double> &high_fparams, set<pm_point> &model);

  bool write_model(QString &filename, set<pm_point> &model,
                   bool overwrite = true);
  bool write_model(QString &filename, set<pm_point> &model,
                   vector<double> &params, bool overwrite = true);
  ...
  bool random_params(vector<double> &params, vector<int> &types,
                     double max_d = 0e0);
  ...
  bool grid_search(vector<double> &params, vector<double> &delta,
                   vector<double> &low_fparams, vector<double> &high_fparams,
                   set<pm_point> &model);
  ...
  bool ga_run(vector<int> &types, pm_ga_individual &best_individual,
  ...
  void ga_set_params(unsigned int ga_population = 100,
                     unsigned int ga_generations = 100,
                     double ga_mutate = 0.1e0, double ga_sa_mutate = 0.5e0,
                     double ga_crossover = 0.4e0, unsigned int ga_elitism = 2,
                     unsigned int ga_early_termination = 10);
  void ga_set_params(map<QString, QString> control_parameters);

  bool best_md0_ga(vector<double> &params, set<pm_point> &model,
                   unsigned int steps_to_ga = 1, unsigned int points_max = 100,
                   double finest_conversion = 1e0,
                   double coarse_conversion = 10e0,
                   double refinement_range_pct = 5e0,
                   double conversion_divisor = 2e0);

  bool best_ga(vector<double> &params, set<pm_point> &model,
               unsigned int points_max = 100, double finest_conversion = 1e0,
               double coarse_conversion = 10e0,
               double refinement_range_pct = 5e0,
               double conversion_divisor = 2e0);
