...
struct centerpieceInfo {
  ...
  float width;  // width of channel, if rectangular, or radius if sector==2, 0
                // otherwise.
};
...
struct Config {
  ...
  QString help_dir;     // root directory for html help files (usually ../html
                        // subdir of system_dir)
...
struct SA2d_control_variables {
  ...
  double minx, miny, maxx, maxy, maxz, xscaling, yscaling, zscaling, alpha,
      beta;
};
...
class US_EXTERN US_Static {
 public:
...
#if QT_VERSION >= 0x040000
  static double getDouble(const QString &title, const QString &label,
                          double value = 0, double min = -2147483647,
                          double max = 2147483647, int decimals = 1,
                          bool *ok = 0, QWidget *parent = 0,
                          const char *name = 0);
  static int getInteger(const QString &title, const QString &label,
                        int value = 0, int min = -2147483647,
                        int max = 2147483647, int step = 1, bool *ok = 0,
  ...
  static int us_message(const QString &title, const QString &text,
