 ...
 public:
  ...
  QString cinfo() {
    ...
    for (int i = 0; i < (int)header.size(); ++i) {
      ...
      if (data.size()) {
        ...
        qs +=
            QString("column %1: '%2' (data empty)\n").arg(i + 1).arg(header[i]);
      }
    }
    return qs;
  }
  ...
  bool read() {
    ...
    while (!ts.atEnd()) {
      ...
      QStringList qsl =
          qs.split(QRegExp(is_csv ? "," : "\\s+"), QString::SkipEmptyParts);
  ...
  bool split_on_peak(US_Multi_Column &asc, US_Multi_Column &des,
    ...
    if (peak_pos == 0 || peak_pos == data.size() - 1) {
      errormsg =
          QString("Error: %1 peak position (%2, value %3) occured at an end")
              .arg(filename)
              .arg(peak_pos)
              .arg(peak);
      return false;
  ...
  bool avg(vector<double> &result, vector<double> &a, vector<double> &b) {
    errormsg = "";
    if (a.size() != b.size()) {
  ...
  bool join(US_Multi_Column &part1, US_Multi_Column &part2,
            QString use_filename = "", unsigned int overlap = 1) {
    errormsg = "";
    if (overlap > part1.data.size() || overlap > part2.data.size()) {
      errormsg =
          QString("Error: overlap %1 is greater than row sizes ( %2 and %3 )")
              .arg(overlap)
              .arg(part1.data.size())
              .arg(part2.data.size());
      return false;
  ...
  bool spline(US_Multi_Column &part1, US_Multi_Column &part2, unsigned int dep1,
    ...
    if (!ascending && part2_min_dep < part2_max_dep) {
      errormsg = QString(
                     "Error: range reversal over dependent range: %1 [%2:%3] "
                     "vs %4 [%5:%6]")
                     .arg(part1.filename)
  ...
  // make spline dep2->indep2
  // compute values for indep2( dep1 ) and append with header to part1
  bool quadratic(US_Multi_Column &part1, US_Multi_Column &part2,
                 unsigned int dep1, unsigned int indep2, unsigned int dep2,
  ...
  // make spline dep2->indep2
  // compute values for indep2( dep1 ) and append with header to part1
  bool linear(US_Multi_Column &part1, US_Multi_Column &part2, unsigned int dep1,
              unsigned int indep2, unsigned int dep2, QString use_filename) {
    errormsg = "";
    if (!part1.data.size()) {
      errormsg = QString("Error: %1 has no data").arg(part1.filename);
    ...
    if (!x2.size()) {
      errormsg = QString(
                     "In linear: part1 %1 part2 %2 dep1 %3 indep2 %4 dep2 %5 "
                     "range min %6 max %7:  data in range")
                     .arg(part1.filename)
                     .arg(part2.filename)
                     .arg(dep1)
                     .arg(indep2)
  ...
  bool smooth(unsigned int col, unsigned int points,
    ...
    if (!col) {
      errormsg =
          QString("Error: smooth: %1, col must be nonzero").arg(filename);
      return false;
    }
    ...
    if (data[0].size() < col) {
      errormsg =
          QString("Error: smooth: %1 has %2 columns and column %3 requested")
              .arg(filename)
              .arg(data[0].size())
              .arg(col);
      return false;
  ...
  bool repeak(unsigned int col) {
    ...
    if (!col) {
      errormsg =
          QString("Error: repeak: %1, col must be nonzero").arg(filename);
      return false;
    }
    ...
    if ((unsigned int)data[0].size() < col) {
      errormsg =
          QString("Error: repeak: %1 has %2 columns and column %3 requested")
              .arg(filename)
              .arg(data[0].size())
              .arg(col);
      return false;
    ...
    if (max <= 0e0) {
      errormsg = QString(
                     "Error: repeak: %1 column %2 max is %3, but must be "
                     "greater than zero")
                     .arg(filename)
                     .arg(col + 1)
                     .arg(max);
  ...
  bool range(unsigned int col, double min, double max) {
    ...
    if ((unsigned int)data[0].size() < col) {
      errormsg =
          QString("Error: range: %1 has %2 columns and column %3 requested")
              .arg(filename)
              .arg(data[0].size())
              .arg(col);
      return false;
  ...
  bool scale(unsigned int col, double mult) {
    ...
    if ((unsigned int)data[0].size() < col) {
      errormsg =
          QString("Error: scale: %1 has %2 columns and column %3 requested")
              .arg(filename)
              .arg(data[0].size())
              .arg(col);
      return false;
  ...
  bool clip(unsigned int col, double min, double max) {
    ...
    if ((unsigned int)data[0].size() < col) {
      errormsg =
          QString("Error: clip: %1 has %2 columns and column %3 requested")
              .arg(filename)
              .arg(data[0].size())
              .arg(col);
      return false;
  ...
  bool ijoin(US_Multi_Column &mc1, US_Multi_Column &mc2, unsigned int col1,
             unsigned int col2, unsigned int col2add) {
    errormsg = "";
    if (!col1 || !col2 || !col2add) {
      errormsg =
          QString("Error: ijoin: %1, cols must be nonzero").arg(filename);
      return false;
    }
    ...
    if ((unsigned int)mc1.data[0].size() < col1) {
      errormsg =
          QString("Error: ijoin: %1 has %2 columns and column %3 requested")
              .arg(mc1.filename)
              .arg(mc1.data[0].size())
              .arg(col1);
      return false;
    ...
    if (col2 == col2add) {
      errormsg =
          QString(
              "Error: ijoin: %1 col2 %2 and col2add %3 can not be identical")
              .arg(mc2.filename)
              .arg(col2)
              .arg(col2add);
      return false;
  ...
  bool sort(unsigned int col) {
    ...
    if ((unsigned int)data[0].size() < col) {
      errormsg =
          QString("Error: sort: %1 has %2 columns and column %3 requested")
              .arg(filename)
              .arg(data[0].size())
              .arg(col);
      return false;
  ...
  bool uniquify(unsigned int col, unsigned int sumcol) {
    errormsg = "";
    if (!col) {
      errormsg =
          QString("Error: uniquify: %1, col must be nonzero").arg(filename);
      return false;
    }
    ...
    if ((unsigned int)data[0].size() < col ||
      ...
      errormsg = QString(
                     "Error: uniquify: %1 has %2 columns and columns %3 and %4 "
                     "requested")
                     .arg(filename)
                     .arg(data[0].size())
                     .arg(col)
