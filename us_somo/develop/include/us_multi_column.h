...
public:
  ...
  unsigned int index;
  bool
  operator< (const umc_sortable_double &objIn) const
  {
    //      unsigned int i;
    return d < objIn.d;
  }
...
public:
  ...
  {
    return QString (
               "US_Multi_Column filename %1: header %2 rows %3 columns %4\n")
        .arg (filename)
        .arg (header.size ())
        .arg (data.size ())
    ...
    while (!ts.atEnd ())
      {
        ...
        QStringList qsl = qs.split (QRegExp (is_csv ? "," : "\\s+"),
                                    QString::SkipEmptyParts);
        if (!pos && qsl.size () && qsl[0].contains (QRegExp ("^\\D")))
          {
            // take as header;
  ...
  bool
  split_on_peak (US_Multi_Column &asc, US_Multi_Column &des,
                 unsigned int indep, unsigned int dep)
  {
    errormsg = "";
    if (!data.size ())
      {
    ...
    if (data.size () < 3)
      {
        errormsg
            = QString ("Error: %1 has only %2 rows and it needs at least 3")
                  .arg (filename)
                  .arg (data.size ());
        return false;
      }
    ...
    if (data[0].size () < indep || data[0].size () < dep)
      {
        errormsg
            = QString (
                  "Error: %1 has %2 columns and columns %3 %4 requested  ")
                  .arg (filename)
                  .arg (data[0].size ())
                  .arg (indep)
                  .arg (dep);
    ...
    if (peak_pos == 0 || peak_pos == data.size () - 1)
      {
        errormsg
            = QString (
                  "Error: %1 peak position (%2, value %3) occured at an end")
                  .arg (filename)
                  .arg (peak_pos)
                  .arg (peak);
        return false;
  ...
  bool
  monotonize (US_Multi_Column &mono, unsigned int dep)
  {
    errormsg = "";
    if (!data.size ())
      {
    ...
    if (data[0].size () < dep)
      {
        errormsg
            = QString ("Error: %1 has %2 columns and column %3 requested  ")
                  .arg (filename)
                  .arg (data[0].size ())
                  .arg (dep);
        return false;
  ...
  bool
  write (QString use_filename = "", bool overwrite = false)
  {
    errormsg = "";
    ...
    bool is_write_csv
        = use_filename.right (4).contains (".csv", Qt::CaseInsensitive);
    ...
    if (!f.open (QIODevice::WriteOnly | QIODevice::Text))
      {
        errormsg = QString ("Error: file %1 can not be opened for writing")
                       .arg (use_filename);
        return false;
      }
  ...
  bool
  avg (vector<double> &result, vector<double> &a, vector<double> &b)
  {
    errormsg = "";
    if (a.size () != b.size ())
      {
  ...
  bool
  join (US_Multi_Column &part1, US_Multi_Column &part2,
        QString use_filename = "", unsigned int overlap = 1)
  {
    errormsg = "";
    if (overlap > part1.data.size () || overlap > part2.data.size ())
      {
        errormsg
            = QString (
                  "Error: overlap %1 is greater than row sizes ( %2 and %3 )")
                  .arg (overlap)
                  .arg (part1.data.size ())
                  .arg (part2.data.size ());
        return false;
  ...
  bool
  spline (US_Multi_Column &part1, US_Multi_Column &part2, unsigned int dep1,
          unsigned int indep2, unsigned int dep2, QString use_filename)
  {
    errormsg = "";
    if (!part1.data.size ())
      {
    ...
    if (part2.data[0].size () < dep2 || part2.data[0].size () < indep2)
      {
        errormsg
            = QString ("Error: %1 has %2 columns and columns %3 %4 requested")
                  .arg (part2.filename)
    ...
    if (!ascending && part2_min_dep < part2_max_dep)
      {
        errormsg = QString ("Error: range reversal over dependent range: %1 "
                            "[%2:%3] vs %4 [%5:%6]")
                       .arg (part1.filename)
    ...
    for (double pos = part2_min_dep; pos <= part2_max_dep;
         pos += (part2_max_dep - part2_min_dep) / 256)
      {
        vector<double> tmp_data;
        double new_val;
    ...
    for (unsigned int i = 0; i < (unsigned int)data.size (); i++)
      {
        double new_val = 0e0;
        if (data[i][dep1] >= part2_min_dep && data[i][dep1] <= part2_max_dep)
          {
            ...
            // cout << QString( "spline: compute point dest value %1, new value
  ...
  // compute values for indep2( dep1 ) and append with header to part1
  bool
  quadratic (US_Multi_Column &part1, US_Multi_Column &part2, unsigned int dep1,
             unsigned int indep2, unsigned int dep2, QString use_filename)
  {
    errormsg = "";
    if (!part1.data.size ())
  ...
  // compute values for indep2( dep1 ) and append with header to part1
  bool
  linear (US_Multi_Column &part1, US_Multi_Column &part2, unsigned int dep1,
          unsigned int indep2, unsigned int dep2, QString use_filename)
  {
    errormsg = "";
    if (!part1.data.size ())
    ...
    if (!x2.size ())
      {
        errormsg = QString ("In linear: part1 %1 part2 %2 dep1 %3 indep2 %4 "
                            "dep2 %5 range min %6 max %7:  data in range")
                       .arg (part1.filename)
                       .arg (part2.filename)
                       .arg (dep1)
                       .arg (indep2)
  ...
  bool
  smooth (unsigned int col, unsigned int points, bool smooth_endpoints = false)
  {
    errormsg = "";
    if (!points)
      {
    ...
    if (!col)
      {
        errormsg = QString ("Error: smooth: %1, col must be nonzero")
                       .arg (filename);
        return false;
      }
    ...
    if (data[0].size () < col)
      {
        errormsg
            = QString (
                  "Error: smooth: %1 has %2 columns and column %3 requested")
                  .arg (filename)
                  .arg (data[0].size ())
                  .arg (col);
        return false;
  ...
  bool
  repeak (unsigned int col)
  {
    errormsg = "";
    if (!col)
      {
        errormsg = QString ("Error: repeak: %1, col must be nonzero")
                       .arg (filename);
        return false;
      }
    ...
    if ((unsigned int)data[0].size () < col)
      {
        errormsg
            = QString (
                  "Error: repeak: %1 has %2 columns and column %3 requested")
                  .arg (filename)
                  .arg (data[0].size ())
                  .arg (col);
        return false;
    ...
    if (max <= 0e0)
      {
        errormsg = QString ("Error: repeak: %1 column %2 max is %3, but must "
  ...
  bool
  range (unsigned int col, double min, double max)
  {
    errormsg = "";
    if (!col)
      {
        errormsg
            = QString ("Error: range: %1, col must be nonzero").arg (filename);
        return false;
      }
    ...
    if ((unsigned int)data[0].size () < col)
      {
        errormsg
            = QString (
                  "Error: range: %1 has %2 columns and column %3 requested")
                  .arg (filename)
                  .arg (data[0].size ())
                  .arg (col);
        return false;
    ...
    for (unsigned int i = 0; i < (unsigned int)data.size (); i++)
      {
        if (data[i][col] >= min && data[i][col] <= max)
          {
  ...
  bool
  scale (unsigned int col, double mult)
  {
    errormsg = "";
    if (!col)
      {
        errormsg
            = QString ("Error: scale: %1, col must be nonzero").arg (filename);
        return false;
      }
    ...
    if ((unsigned int)data[0].size () < col)
      {
        errormsg
            = QString (
                  "Error: scale: %1 has %2 columns and column %3 requested")
                  .arg (filename)
                  .arg (data[0].size ())
                  .arg (col);
        return false;
  ...
  bool
  clip (unsigned int col, double min, double max)
  {
    errormsg = "";
    if (!col)
      {
        errormsg
            = QString ("Error: clip: %1, col must be nonzero").arg (filename);
        return false;
      }
    ...
    if ((unsigned int)data[0].size () < col)
      {
        errormsg
            = QString (
                  "Error: clip: %1 has %2 columns and column %3 requested")
                  .arg (filename)
                  .arg (data[0].size ())
                  .arg (col);
        return false;
  ...
  bool
  ijoin (US_Multi_Column &mc1, US_Multi_Column &mc2, unsigned int col1,
         unsigned int col2, unsigned int col2add)
  {
    errormsg = "";
    if (!col1 || !col2 || !col2add)
      {
        errormsg = QString ("Error: ijoin: %1, cols must be nonzero")
                       .arg (filename);
        return false;
      }
    ...
    if ((unsigned int)mc1.data[0].size () < col1)
      {
        errormsg
            = QString (
                  "Error: ijoin: %1 has %2 columns and column %3 requested")
                  .arg (mc1.filename)
                  .arg (mc1.data[0].size ())
                  .arg (col1);
        return false;
    ...
    if (col2 == col2add)
      {
        errormsg = QString ("Error: ijoin: %1 col2 %2 and col2add %3 can not "
                            "be identical")
                       .arg (mc2.filename)
                       .arg (col2)
                       .arg (col2add);
    ...
    // US_Vector::printvector3( "to grid, from grid, from data", to_grid,
    // from_grid, from_data );
  ...
  bool
  sort (unsigned int col)
  {
    errormsg = "";
    if (!col)
      {
        errormsg
            = QString ("Error: sort: %1, col must be nonzero").arg (filename);
        return false;
      }
    ...
    if ((unsigned int)data[0].size () < col)
      {
        errormsg
            = QString (
                  "Error: sort: %1 has %2 columns and column %3 requested")
                  .arg (filename)
                  .arg (data[0].size ())
                  .arg (col);
        return false;
  ...
  bool
  uniquify (unsigned int col, unsigned int sumcol)
  {
    errormsg = "";
    if (!col)
      {
        errormsg = QString ("Error: uniquify: %1, col must be nonzero")
                       .arg (filename);
        return false;
      }
    ...
    if ((unsigned int)data[0].size () < col
      ...
      {
        errormsg = QString ("Error: uniquify: %1 has %2 columns and columns "
                            "%3 and %4 requested")
                       .arg (filename)
                       .arg (data[0].size ())
                       .arg (col)
