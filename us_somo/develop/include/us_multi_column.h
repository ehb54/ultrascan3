...
class US_Multi_Column {
   public:
      ...
      bool split_on_peak(US_Multi_Column &asc, US_Multi_Column &des, unsigned int indep, unsigned int dep) {
         errormsg = "";
         if (!data.size()) {
         ...
         if (data.size() < 3) {
            errormsg = QString("Error: %1 has only %2 rows and it needs at least 3").arg(filename).arg(data.size());
            return false;
         }
      ...
      bool avg(vector<double> &result, vector<double> &a, vector<double> &b) {
         errormsg = "";
         if (a.size() != b.size()) {
            errormsg = QString("Error: avg size incompatibility %1 vs %2").arg(a.size()).arg(b.size());
            return false;
         }
      ...
      bool join(US_Multi_Column &part1, US_Multi_Column &part2, QString use_filename = "", unsigned int overlap = 1) {
         errormsg = "";
      ...
      bool spline(
         US_Multi_Column &part1, US_Multi_Column &part2, unsigned int dep1, unsigned int indep2, unsigned int dep2,
      ...
      bool linear(
         ...
         if (!x2.size()) {
            errormsg = QString(
                          "In linear: part1 %1 part2 %2 dep1 %3 indep2 %4 dep2 %5 range min %6 max %7:  data in range")
                          .arg(part1.filename)
                          .arg(part2.filename)
                          .arg(dep1)
                          .arg(indep2)
      ...
      bool smooth(unsigned int col, unsigned int points, bool smooth_endpoints = false) {
         errormsg = "";
         if (!points) {
      ...
      bool ijoin(US_Multi_Column &mc1, US_Multi_Column &mc2, unsigned int col1, unsigned int col2, unsigned int col2add) {
         errormsg = "";
      ...
      bool sort(unsigned int col) {
         ...
         for (list<umc_sortable_double>::iterator it = lumcsd.begin(); it != lumcsd.end(); it++) {
      ...
      bool uniquify(unsigned int col, unsigned int sumcol) {
         ...
         if (( unsigned int ) data[ 0 ].size() < col || ( unsigned int ) data[ 0 ].size() < sumcol) {
   ...
   private:
      double normal(double x) { return exp(-(x * x)) / sqrt(2e0 * 3.14159265358978323e0); }
