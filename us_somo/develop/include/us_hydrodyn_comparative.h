...
struct csv
{
  ...
  vector<QString> prepended_names; // list of "model names" with
  ...
  map<QString, unsigned int>
      nd_key; // used to map parents of data items to below
...
public:
  US_Hydrodyn_Comparative (comparative_info *comparative, void *us_hydrodyn,
                           bool *comparative_widget
                           = 0, // no comparative widget implies non-gui
...
#pragma warning(disable : 4251)
  ...
  void csv_write (QString filename,
                  csv &csv1);              // use to save any csv in the loaded
  ...
  void csv_merge_loaded_selected (); // takes all loaded selected pdbs, merges
                                     // them and puts it on the loaded list and
                                     // selects it

  bool csv_merge_selected_selected (
      csv &csv_merged); // does not add to csvs, returns false if not created
  //                            ^^^^^^ note: this function disables_updates()
  //                            and you must enable_updates() afterwards.
  //                            (there are potential gui issues without this
  //                            behaviour)
  ...
  QString csv_info (
      csv &csv1); // returns readable summary info (primarily for debugging)
  QStringList
  csv_model_names (csv &csv1); // returns a list of the names (primarily for
                               // updating lb_selected)
  bool csv_contains (comparative_entry ce,
                     csv &csv1); // checks the csv for the column names by the
                                 // comparative entry
  bool all_selected_csv_contain (
      comparative_entry ce); // checks all selected csvs for the column names
                             // by the comparative entry

  bool csv_get_min_max (double &min, double &max,
                        unsigned int &rows_used_count, comparative_entry ce,
                        csv &csv1, bool ignore_selected_selected = true);

  bool csv_get_loaded_min_max (double &min, double &max, comparative_entry ce);

  bool csv_get_selected_min_max (double &min, double &max,
                                 comparative_entry ce);

  void csv_make_unique_header_name (csv &csv1, QString name);
  //                            finds a unique name that does equal the
  //                            originally supplied name and then rename the
  //                            header to that unique name

  void update_selected_map ();
  map<QString, bool> selected_map; // needed for quick lookups, maps
                                   // lb_selected_selected.text()
  ...
  map<QString, bool> csv_premerge_missing_header_map; // keeps track of columns
                                                      // that will be added
