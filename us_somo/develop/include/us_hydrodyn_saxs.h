...
public:
  US_Hydrodyn_Saxs (bool *saxs_widget, saxs_options *our_saxs_options,
                    QString pdb_file, QString pdb_filepath,
                    ...
                    bool create_native_saxs, void *us_hydrodyn, QWidget *p = 0,
                    const char *name = 0);
  ~US_Hydrodyn_Saxs ();

  void refresh (QString pdb_file, QString pdb_filepath,
                vector<residue> residue_list, vector<PDB_model> model_vector,
                vector<vector<PDB_atom> > bead_models,
                vector<unsigned int> selected_models,
                map<QString, vector<int> > multi_residue_map,
                map<QString, QString> residue_atom_hybrid_map, int source,
...
public:
  ...
  map<unsigned int, QwtPlotCurve *>
      plotted_cs_Gp_curves_full; // cs guinier points
  map<unsigned int, QwtPlotCurve *>
      plotted_Rt_Gp_curves_full; // Rt guinier points
...
private:
  ...
  bool log_rebin (int intervals, const vector<double> &q,
  ...
  bool last_pr_rebin_save (const QString &header, const QString &rxstr,
  ...
  void create_shd (vector<saxs_atom> &atoms, vector<double> &q,
  ...
  void do_plot_resid ();
  void do_plot_resid (vector<double> &x, vector<double> &y, vector<double> &e,
                      QColor qc);

  void do_plot_resid (vector<double> &x, vector<double> &y, vector<double> &e,
                      map<double, double> &pts_removed, QColor qc);

  void do_plot_resid (vector<double> &x_d, vector<double> &e_d, QColor qc);

  void set_plot_pr_range (double &minx, double &maxx, double &miny,
                          double &maxy);
  ...
  // map < QString, bool > saxs_guinier_frames_widgets; map < QString,
  // US_Hydrodyn_Saxs_Guinier_Frames *> saxs_guinier_frames_windows;
  ...
  QString info_remember_mw (const QString &msg
                            = ""); // report contents of remember_mw maps
  ...
  vector<double> interpolate (vector<double> to_r, vector<double> from_r,
                              vector<double> from_pr);

  bool interpolate (vector<double> to_grid, vector<double> from_grid,
                    vector<double> from_data, vector<double> &to_data);
  ...
  void calc_best_fit (QString title, QString save_to_csv_name = "");
  void plot_one_pr (vector<double> r, vector<double> pr, QString name,
                    bool skip_mw = false);

  void plot_one_pr (vector<double> r, vector<double> pr,
                    vector<double> pr_error, QString name,
                    bool skip_mw = false, bool do_replot = true);

  void calc_iqq_nnls_fit (QString title, QString save_to_csv_name = "");
  void calc_iqq_best_fit (QString title, QString save_to_csv_name = "");

  void plot_one_iqq (vector<double> q, vector<double> I, QString name);
  void plot_one_iqq (vector<double> q, vector<double> I,
  ...
  void crop_iq_data (vector<double> &q, vector<double> &I);
  ...
  void display_iqq_residuals (QString title, vector<double> q,
                              vector<double> I1, vector<double> I2,
                              QColor plot_color, vector<double> I_errors);
  double last_rescaling_multiplier;
  double last_rescaling_offset;
  double last_rescaling_chi2;

  void rescale_iqq_curve_using_last_rescaling (vector<double> &I,
                                               bool use_offset = false);

  void rescale_iqq_curve (QString scaling_target, vector<double> &q,
                          vector<double> &I, bool do_plot_residuals = true);
  ...
  void rescale_iqq_curve (QString scaling_target, vector<double> &q,
                          vector<double> &I, vector<double> &I2,
                          QColor plot_color, bool do_plot_resduals = true);
  ...
  void sync_conc_csv (); // removes deleted curves, adds non-extant curves with
                         // default
  ...
  bool get_conc_csv_values (QString name, double &conc, double &psv,
                            double &I0_std);
  ...
  void set_pr_sd (
      vector<double> &r, vector<double> &pr,
      vector<double> &pr_error); // sets a min sd value if pr has non-zero sds

  void crop_pr_tail (vector<double> &r, vector<double> &pr,
                     vector<double> &pr_error); // crops the tail of zeros
                                                // values, used for plotting

  void
  prop_pr_sd_tail (vector<double> &pr_error,
                   unsigned int len); // propagate the last pr sd to the tail
...
private:
  ...
  bool mw_from_I0 (QString name, double I0_exp, double &MW,
                   double &internal_contrast);
  bool ml_from_qI0 (QString name, double I0_exp, double &ML,
                    double &internal_contrast);
  bool ma_from_q2I0 (QString name, double I0_exp, double &MA,
                     double &internal_contrast);
  ...
  bool compute_rg_to_progress (const vector<double> &r,
...
private slots:
  ...
  void pr_info (const QString &msg = "",
                bool detail = false);      // stdout report of pr vectors
  ...
  double compute_ff_bead_model (saxs &s, double q_o_4pi2);
...
private slots:
  ...
  void load_saxs (QString filename = "", bool just_plotted_curves = false,
                  QString scaleto = "", bool no_scaling = false);
  ...
  void set_curve ();
  void set_curve (int);
  void load_pr (bool just_plotted_curves = false, QString load_this = "",
                bool skip_mw = false);
  void load_plot_pr ();
  ...
  void normalize_pr (vector<double>, vector<double> *, vector<double> *,
                     double mw = 1e0);
  ...
  void plot_saxs_item_clicked (const QVariant &iteminfo, int index);
  void plot_pr_item_clicked (const QVariant &iteminfo, int index);
  void saxs_legend ();
  void pr_legend ();
...
public:
  saxs_Iq_thr_t (int);
  void saxs_Iq_thr_setup (vector<saxs_atom> *atoms, vector<vector<double> > *f,
                          vector<vector<double> > *fc,
                          vector<vector<double> > *fp, vector<double> *I,
                          vector<double> *Ia, vector<double> *Ic,
                          vector<double> *q, unsigned int threads,
                          mQProgressBar *progress, QLabel *lbl_core_progress,
                          bool *stopFlag);
...
public:
  saxs_pr_thr_t (int);
  void saxs_pr_thr_setup (vector<saxs_atom> *atoms, vector<float> *hist,
                          double delta, unsigned int threads,
                          mQProgressBar *progress, QLabel *lbl_core_progress,
                          bool *stopFlag, float b_bar_inv2);
