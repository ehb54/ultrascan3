...
class US_EXTERN US_Hydrodyn_Saxs : public QFrame {
   ...
   public:
      US_Hydrodyn_Saxs(
         bool *saxs_widget, saxs_options *our_saxs_options, QString pdb_file, QString pdb_filepath,
         vector<residue> residue_list, vector<PDB_model> model_vector, vector<vector<PDB_atom>> bead_models,
         vector<unsigned int> selected_models, map<QString, vector<int>> multi_residue_map,
         map<QString, QString> residue_atom_hybrid_map, int source, bool create_native_saxs, void *us_hydrodyn,
         QWidget *p = 0, const char *name = 0);
      ~US_Hydrodyn_Saxs();

      void refresh(
         QString pdb_file, QString pdb_filepath, vector<residue> residue_list, vector<PDB_model> model_vector,
         vector<vector<PDB_atom>> bead_models, vector<unsigned int> selected_models,
         map<QString, vector<int>> multi_residue_map, map<QString, QString> residue_atom_hybrid_map, int source,
   ...
   private:
      ...
      bool log_rebin(
         int intervals, const vector<double> &q, const vector<double> &I, const vector<double> &e,
         vector<double> &rebin_q, vector<double> &rebin_I, vector<double> &rebin_e, QString &errors);

      bool log_rebin(
         int intervals, const vector<double> &q, const vector<double> &I, vector<double> &rebin_q,
         vector<double> &rebin_I, QString &errors);

      bool last_pr_rebin_save(const QString &header, const QString &rxstr, QStringList &created_files);
      ...
      void create_shd(vector<saxs_atom> &atoms, vector<double> &q, vector<double> &q2, vector<double> &q_over_4pi_2);
      ...
      void do_plot_resid();
      void do_plot_resid(vector<double> &x, vector<double> &y, vector<double> &e, QColor qc);

      void do_plot_resid(
         vector<double> &x, vector<double> &y, vector<double> &e, map<double, double> &pts_removed, QColor qc);

      void do_plot_resid(vector<double> &x_d, vector<double> &e_d, QColor qc);

      void set_plot_pr_range(double &minx, double &maxx, double &miny, double &maxy);
      ...
      vector<double> interpolate(vector<double> to_r, vector<double> from_r, vector<double> from_pr);

      bool interpolate(
         vector<double> to_grid, vector<double> from_grid, vector<double> from_data, vector<double> &to_data);

      bool natural_spline_interpolate(
         vector<double> to_grid, vector<double> from_grid, vector<double> from_data, vector<double> &to_data);
      ...
      void plot_one_pr(vector<double> r, vector<double> pr, QString name, bool skip_mw = false);

      void plot_one_pr(
         vector<double> r, vector<double> pr, vector<double> pr_error, QString name, bool skip_mw = false,
         bool do_replot = true);
      ...
      void crop_iq_data(vector<double> &q, vector<double> &I, vector<double> &I_errors);
      ...
      void display_iqq_residuals(
         QString title, vector<double> q, vector<double> I1, vector<double> I2, QColor plot_color,
      ...
      void rescale_iqq_curve(QString scaling_target, vector<double> &q, vector<double> &I, bool do_plot_residuals = true);

      void rescale_iqq_curve(
         QString scaling_target, vector<double> &q, vector<double> &I, vector<double> &I2,
         bool do_plot_residuals = true);

      void rescale_iqq_curve(
         QString scaling_target, vector<double> &q, vector<double> &I, QColor plot_color, bool do_plot_residuals = true);

      void rescale_iqq_curve(
         QString scaling_target, vector<double> &q, vector<double> &I, vector<double> &I2, QColor plot_color,
      ...
      void set_pr_sd(
         vector<double> &r, vector<double> &pr, vector<double> &pr_error); // sets a min sd value if pr has non-zero sds

      void crop_pr_tail(
         vector<double> &r, vector<double> &pr,
         vector<double> &pr_error); // crops the tail of zeros values, used for plotting

      void prop_pr_sd_tail(vector<double> &pr_error, unsigned int len); // propagate the last pr sd to the tail
   ...
   private:
      ...
      bool pvalue(
         const vector<double> &q, vector<double> &I, vector<double> &G, double &P,
         QString &errormsg); // compute pvalue comparing 2 curves

      bool compute_rg_to_progress(const vector<double> &r, const vector<double> &pr, const QString &filename);
   ...
   private slots:
      ...
      void show_plot_saxs();
      void load_saxs(
         QString filename = "", bool just_plotted_curves = false, QString scaleto = "", bool no_scaling = false);
      ...
      void normalize_pr(vector<double>, vector<double> *, vector<double> *, double mw = 1e0);
...
class saxs_Iq_thr_t : public QThread {
   public:
      ...
      void saxs_Iq_thr_setup(
         vector<saxs_atom> *atoms, vector<vector<double>> *f, vector<vector<double>> *fc, vector<vector<double>> *fp,
         vector<double> *I, vector<double> *Ia, vector<double> *Ic, vector<double> *q, unsigned int threads,
         mQProgressBar *progress, QLabel *lbl_core_progress, bool *stopFlag);
...
class saxs_pr_thr_t : public QThread {
   public:
      ...
      void saxs_pr_thr_setup(
         vector<saxs_atom> *atoms, vector<float> *hist, double delta, unsigned int threads, mQProgressBar *progress,
         QLabel *lbl_core_progress, bool *stopFlag, float b_bar_inv2);
