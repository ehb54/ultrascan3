...
inline i_refvec<T>::i_refvec(const i_refvec<T> &V)
  ...
  if (V.ref_count_ != NULL) (*(V.ref_count_))++;
}
...
Array2D<T> Array2D<T>::copy() const {
  ...
  for (int i = 0; i < m_; i++)
    for (int j = 0; j < n_; j++) A[i][j] = v_[i][j];
...
Array2D<T> &Array2D<T>::inject(const Array2D &A) {
  if (A.m_ == m_ && A.n_ == n_) {
    for (int i = 0; i < m_; i++)
      for (int j = 0; j < n_; j++) v_[i][j] = A[i][j];
  }
  return *this;
}
...
Array2D<T> matmult(const Array2D<T> &A, const Array2D<T> &B) {
  if (A.dim2() != B.dim1()) return Array2D<T>();
...
Array3D<T> operator+(const Array3D<T> &A, const Array3D<T> &B) {
  ...
  if (B.dim1() != m || B.dim2() != n || B.dim3() != p)
    ...
    for (int i = 0; i < m; i++)
      for (int j = 0; j < n; j++)
        for (int k = 0; k < p; k++) C[i][j][k] = A[i][j][k] + B[i][j][k];
...
Array3D<T> &operator+=(Array3D<T> &A, const Array3D<T> &B) {
  ...
  if (B.dim1() == m && B.dim2() == n && B.dim3() == p) {
    for (int i = 0; i < m; i++)
      for (int j = 0; j < n; j++)
        for (int k = 0; k < p; k++) A[i][j][k] += B[i][j][k];
  }
...
class Vector {
 ...
 protected:
  ...
  void copy(const T *v) {
    ...
    for (i = N4; i < N; i++) v_[i] = v[i];
#else

    for (i = 0; i < N; i++) v_[i] = v[i];
#endif
  }
  ...
  void set(const T &val) {
    ...
    for (i = N4; i < N; i++) v_[i] = val;
#else

    for (i = 0; i < N; i++) v_[i] = val;
...
class Matrix {
 ...
 public:
  ...
  Matrix<T> &newsize(Subscript M, Subscript N) {
    if (num_rows() == M && num_cols() == N) return *this;
...
Matrix<T> operator+(const Matrix<T> &A, const Matrix<T> &B) {
  ...
  for (i = 0; i < M; i++)
    for (j = 0; j < N; j++) tmp[i][j] = A[i][j] + B[i][j];
...
inline Matrix<T> matmult(const Matrix<T> &A, const Matrix<T> &B) {
  ...
  for (Subscript i = 0; i < M; i++)
    for (Subscript k = 0; k < K; k++) {
      sum = 0;
      for (Subscript j = 0; j < N; j++) sum = sum + A[i][j] * B[j][k];
...
Vector<T> matmult(const Matrix<T> &A, const Vector<T> &x) {
  ...
  for (Subscript i = 0; i < M; i++) {
    sum = 0;
    const T *rowi = A[i];
    for (Subscript j = 0; j < N; j++) sum = sum + rowi[j] * x[j];
...
Fortran_Array2D<T> &Fortran_Array2D<T>::inject(const Fortran_Array2D &A) {
  if (m_ == A.m_ && n_ == A.n_) copy_(data_, A.data_, m_ * n_);
...
Fortran_Array3D<T>::Fortran_Array3D(int m, int n, int k, const T &val)
    : v_(m * n * k), m_(m), n_(n), k_(k), data_(v_.begin()) {
  for (T *p = data_; p < data_ + m * n * k; p++) *p = val;
}
...
Fortran_Array3D<T> operator+(const Fortran_Array3D<T> &A,
  ...
  if (B.dim1() != m || B.dim2() != n || B.dim3() != p)
    ...
    for (int i = 1; i <= m; i++)
      for (int j = 1; j <= n; j++)
        for (int k = 1; k <= p; k++) C(i, j, k) = A(i, j, k) + B(i, j, k);
...
Fortran_Array3D<T> &operator+=(Fortran_Array3D<T> &A,
  ...
  if (B.dim1() == m && B.dim2() == n && B.dim3() == p) {
    for (int i = 1; i <= m; i++)
      for (int j = 1; j <= n; j++)
        for (int k = 1; k <= p; k++) A(i, j, k) += B(i, j, k);
  }
