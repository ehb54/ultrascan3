...
template <class T>
inline const T &
i_refvec<T>::operator[] (int i) const
{
  return data_[i];
}
...
protected:
  ...
  void
  initialize (Subscript M, Subscript N)
  {
    mn_ = M * N;
    m_ = M;
    n_ = N;
...
public:
  ...
  //
  Matrix<T> &
  newsize (Subscript M, Subscript N)
  {
  ...
  inline const_reference
  operator() (Subscript i, Subscript j) const
  {
#ifdef TNT_BOUNDS_CHECK
    assert (1 <= i);
    assert (i <= m_);
...
public:
          ...
          case 3:
            {
              ...
              Real scale = max (max (max (max (abs (s[p - 1]), abs (s[p - 2])),
                                          abs (e[p - 2])),
                                     abs (s[k])),
                                abs (e[k]));
              Real sp = s[p - 1] / scale;
  ...
  void
  getSingularValues (Array1D<Real> &x)
  {
    x = s;
  }
