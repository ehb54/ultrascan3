#ifndef US_HYDRODYN_MALS_SVD_H
#define US_HYDRODYN_MALS_SVD_H
...
class US_EXTERN US_Hydrodyn_Mals_Svd : public QFrame {
  Q_OBJECT

 public:
  US_Hydrodyn_Mals_Svd(US_Hydrodyn_Mals *mals_win,
                       vector<QString> mals_selected_files, QWidget *p = 0);

  ~US_Hydrodyn_Mals_Svd();
 ...
 private:
  ...
  QPushButton *pb_efa_decomp_to_mals_saxs;
 ...
 private:
  ...
  // ------ norm utils
  enum norms { NO_NORM, NORM_NOT, NORM_PW, NORM_AVG };
  ...
  bool plot_file(QString file, double &minx, double &maxx, double &miny,
                 double &maxy, norms norm_mode = NO_NORM);
  ...
  void add_i_of_q_or_t(QString source, QStringList files,
                       bool do_update_enables = true);
  ...
  void vvd_cnorm(vector<vector<double> > &A, vector<vector<double> > &B);
  ...
  enum modes { MODE_IQIT, MODE_SVD, MODE_EFA, MODE_EFA_DECOMP };
  ...
  bool convert_it_to_iq(QStringList files, QStringList &created_files,
                        QString &error_msg);

  // efa decomp
  enum efa_decomp_method { METHOD_HYBRID, METHOD_ITERATIVE, METHOD_EXPLICIT };

  void initHybridEFA(vector<vector<int> > &M, int num_sv,
                     vector<vector<double> > &D, vector<vector<double> > &C,
                     bool &converged, vector<vector<double> > &V_bar, int niter,
                     double tol, vector<int> &force_pos,
                     // returns
                     bool &failed, vector<vector<double> > &C_ret,
                     vector<vector<double> > &T_ret);

  void initIterativeEFA(vector<vector<int> > &M, int num_sv,
                        vector<vector<double> > &D, vector<vector<double> > &C,
                        bool &converged, vector<vector<double> > &V_bar,
                        // returns
                        bool &failed, vector<vector<double> > &C_ret);

  void initExplicitEFA(vector<vector<int> > &M, int num_sv,
                       vector<vector<double> > &D, vector<vector<double> > &C,
                       bool &converged, vector<vector<double> > &V_bar,
                       // returns
                       bool &failed, vector<vector<double> > &T_ret);

  void runExplicitEFARotation(vector<vector<int> > &M,
  ...
  void runIterativeEFARotation(vector<vector<int> > &M,
                               vector<vector<double> > &D,
                               bool &failed,  // also a return
                               vector<vector<double> > &C,
                               vector<vector<double> > &V_bar,
                               vector<vector<double> > &T, int niter,
                               double tol, vector<int> &force_pos,
                               // returns
                               vector<vector<double> > &C_ret, bool &converged,
                               vector<double> &dc, int &k);

  void EFAUpdateRotation(vector<vector<int> > &M, vector<vector<double> > &C,
                         vector<vector<double> > &D, vector<int> &force_pos,
                         // returns
                         vector<vector<double> > &Cnew);

  void EFAFirstRotation(vector<vector<int> > &M, vector<vector<double> > &C,
                        vector<vector<double> > &D,
                        // returns
                        vector<vector<double> > &Cnew);
