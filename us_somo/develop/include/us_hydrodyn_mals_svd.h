...
class US_EXTERN US_Hydrodyn_Mals_Svd : public QFrame {
   ...
   public:
      US_Hydrodyn_Mals_Svd(US_Hydrodyn_Mals *mals_win, vector<QString> mals_selected_files, QWidget *p = 0);
   ...
   private:
      ...
      bool plot_file(QString file, double &minx, double &maxx, double &miny, double &maxy, norms norm_mode = NO_NORM);

      bool get_min_max(QString file, double &minx, double &maxx, double &miny, double &maxy, norms norm_mode = NO_NORM);
      ...
      vector<vector<double>> vivd_pwmult(vector<vector<int>> &A, vector<vector<double>> &B);
      ...
      void initHybridEFA(
         vector<vector<int>> &M, int num_sv, vector<vector<double>> &D, vector<vector<double>> &C, bool &converged,
         vector<vector<double>> &V_bar, int niter, double tol, vector<int> &force_pos,
         // returns
         bool &failed, vector<vector<double>> &C_ret, vector<vector<double>> &T_ret);
      ...
      void runExplicitEFARotation(
         ...
         vector<vector<double>> &C, vector<vector<double>> &V_bar, vector<vector<double>> &T, int niter, double tol,
      ...
      void runIterativeEFARotation(
         ...
         vector<int> &force_pos,
         // returns
         vector<vector<double>> &C_ret, bool &converged, vector<double> &dc, int &k);

      void EFAUpdateRotation(
         vector<vector<int>> &M, vector<vector<double>> &C, vector<vector<double>> &D, vector<int> &force_pos,
         // returns
         vector<vector<double>> &Cnew);

      void EFAFirstRotation(
         vector<vector<int>> &M, vector<vector<double>> &C, vector<vector<double>> &D,
         // returns
         vector<vector<double>> &Cnew);
