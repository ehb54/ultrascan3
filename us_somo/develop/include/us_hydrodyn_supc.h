...
int us_hydrodyn_supc_main(
   hydro_results *hydro_results, hydro_options *hydro, double use_overlap_tolerance,
   vector<vector<PDB_atom>> *bead_models, vector<int> *somo_processed, vector<PDB_model> *model_vector,
   QListWidget *lb_model, const char *filename, const char *res_filename, vector<QString> model_names,
   QProgressBar *progress, QTextEdit *editor, US_Hydrodyn *us_hydrodyn);
...
class supc_thr_t : public QThread {
   public:
      supc_thr_t(int);
      void supc_thr_setup(
         unsigned int threads, int mode, float *p, float *rRis, float *rRi, float *a, int N, int i, int nat, float *sum);
