...
class US_EXTERN US_Hydrodyn : public QFrame {
   ...
   public:
      ...
      US_Hydrodyn(
         vector<QString>, QString gui_script_file = "", bool init_configs_silently = false, QWidget *p = 0,
         const char *name = 0);
      ...
      static QString fileNameCheck2(
         QString *path, QString *base, QString *ext, bool &cancel, bool &overwrite_all, int mode = 0, QWidget *p = 0);
      ...
      void write_dati1_pat_bead_model(QString filename, int size, void *dtp, void *dts);
   ...
   private:
      ...
      void SS_change_residue( // change the residue of an entry to target_residue
         struct PDB_model &model, const QString &line, const QString target_residue);
      ...
      void info_model_vector(
         const QString &msg, const vector<struct PDB_model> &models, const set<QString> only_atoms = {});
      ...
      bool atom_align(
         vector<point> transform_from, vector<point> transform_to, vector<point> apply_from, vector<point> &result,
   ...
   public slots:
      ...
      int calc_iqq(
         bool bead_model, bool create_native_saxs = true,
         bool do_raise = true); // bring up saxs window if needed and compute iqq curve
      int calc_prr(
         bool bead_model, bool create_native_saxs = true,
         bool do_raise = true); // bring up saxs window if needed and compute prr curve
      ...
      void visualize(
         bool movie_frame = false, QString dir = "", float scale = 1.0, bool black_background = false,
         bool do_pat = false);
   ...
   private slots:
      ...
      int overlap_check(
         bool sc, bool mc, bool buried, double tolerance, int limit = 0,
      ...
      void bead_check(
         bool use_threshold = false, bool message_type = false, bool vdw = false,
         bool only_asa = false); // recheck beads
      void load_config();
      void write_config();
      ...
      void write_bead_spt(
         QString, vector<PDB_atom> *, bool movie_frame = false, float scale = 1, bool black_background = false);
      ...
      int browflex_get_no_of_beads(
         QString filename); // open main file, read molec file, return # of beads or zero if not found
      ...
      bool write_pdb_from_model(
         const PDB_model &model, QString &errors, QString &writtenname, const QString &headernote = "",
         const QString &suffix = "", const QString &filename = "");
...
class radial_reduction_thr_t : public QThread {
   public:
      ...
      void radial_reduction_thr_setup(
         unsigned int, vector<PDB_atom> *, vector<bool> *, vector<bool> *, vector<BPair> *, unsigned int, double);
