...
class US_EXTERN US_Hydrodyn : public QFrame {
 ...
 public:
  ...
  static QString fileNameCheck2(QString *path, QString *base, QString *ext,
                                bool &cancel, bool &overwrite_all, int mode = 0,
                                QWidget *p = 0);
  ...
  void model_viewer(QString file, QString prefix = "", bool nodisplay = false);
 ...
 private:
  ...
  bool model_vector_has_hydration_differences(
  ...
  QString model_summary_msg(
      const QString &msg, struct PDB_model *model);  // summary info about model
  ...
  QString visc_dens_msg(
      bool only_used = false);  // return density and viscosity message
  ...
  void SS_apply(
      ...
      const QString
          &ssbondfile);    // called during load pdb to process any adjustments
                           // (CYS->CYH etc) *** per model! ***
  ...
  void update_model_chain_ionization(
      struct PDB_model &model, bool quiet = true);  // progagate ionization
  ...
  // it uses these to reset the values in lieu of re-reading the residue file...
  ...
  void calc_bead_mw(struct residue *);  // calculate the molecular weight of all
                                        // beads in residue
  ...
  double ionized_residue_atom_mw(vector<double> &fractions, struct residue *res,
                                 struct atom *atom);
 ...
 public slots:
  ...
  void reload_pdb();
  int calc_somo(
      bool no_ovlp_removal = false);  // compute asa and then refine bead_model
  int calc_somo_o();                  // somo model with overlaps
  int calc_grid_pdb(
      bool no_ovlp_removal = false);  // compute grid model from pdb
  ...
  int calc_iqq(bool bead_model, bool create_native_saxs = true,
               bool do_raise = true);  // bring up saxs window if needed and
                                       // compute iqq curve
  int calc_prr(bool bead_model, bool create_native_saxs = true,
               bool do_raise = true);  // bring up saxs window if needed and
                                       // compute prr curve
  ...
  void visualize(bool movie_frame = false, QString dir = "", float scale = 1.0,
                 bool black_background = false, bool do_pat = false);
 ...
 private slots:
  ...
  int compute_pb_normals();  // compute the normal vectors for each peptide bond
                             // plane
