...
public:
  ...
  US_Hydrodyn (vector<QString>, QString gui_script_file = "",
               bool init_configs_silently = false, QWidget *p = 0,
               const char *name = 0);
  ...
  US_Hydrodyn_Save *save_util; // to enable calling of save routines regardless
                               // of whether or not widget is available
  bool batch_widget;
  ...
  static QString fileNameCheck (QString filename, int mode = 0,
                                QWidget *p = 0);
  static QString fileNameCheck (QString *path, QString *base, QString *ext,
                                int mode = 0, QWidget *p = 0);
  static QString fileNameCheck2 (QString filename, bool &cancel,
                                 bool &overwrite_all, int mode = 0,
                                 QWidget *p = 0);
  static QString fileNameCheck2 (QString *path, QString *base, QString *ext,
  ...
  // auto increment,
  void setSomoGridFile (bool); // checks for file existance and resets suffix
                               // accordingly, true for somo
  void
  setHydroFile (); // checks for file existance and resets suffix accordingly
  static void sizeArrows (QwtCounter *);
  static void fixWinButtons (QWidget *);
  QCheckBox *cb_overwrite;
  QString last_hydro_res;
  void bd_anaflex_enables (
      bool flag); // turns buttons on/off based upon current status
  ...
  QString saxs_sans_ext (); // returns correct extension based upon current
                            // saxs/sans mode
  ...
  QString gparam_value (const QString &param); // convenience function, returns
  ...
  void model_viewer (QString file, QString prefix = "",
                     bool nodisplay = false);

  double
  use_solvent_visc (); // temperature solvent viscosity - checks manual flag
  double
  use_solvent_dens (); // temperature solvent density   - checks manual flag
  double use_vbar (double vbar); // temperature vbar

  struct asa_options asa;

  void write_dati1_supc_bead_model (QString filename, int size, void *dt);

  void write_dati1_pat_bead_model (QString filename, int size, void *dtp,
                                   void *dts);
...
private:
  ...
  void compute_residues_with_atomic_vs_bead_hydration_differences (
  ...
  bool
  model_vector_has_hydration_differences (const vector<struct PDB_model> &m);

  bool gui_script;
  QString gui_script_file;
  void gui_script_msg (int line, QString arg, QString msg);
  void gui_script_error (int line, QString arg, QString msg,
                         bool doexit = true);
  ...
  QString
  model_summary_msg (const QString &msg,
                     struct PDB_model *model); // summary info about model
  bool model_summary_csv (struct PDB_model *model,
                          const QString &filename = "model_load_summary.csv");
  ...
  QString visc_dens_msg (bool only_used
                         = false); // return density and viscosity message
  ...
  void
  SS_apply (struct PDB_model &model,
            const QString
                &ssbondfile); // called during load pdb to process any
  ...
  void update_model_chain_ionization (struct PDB_model &model,
                                      bool quiet
                                      = true); // progagate ionization
  ...
  void info_bead_models_mw (const QString &msg,
                            const vector<vector<PDB_atom> > &b_models);
  void info_bead_models_mw (const QString &msg,
                            const vector<PDB_atom> &b_model);
  void info_model_vector (const QString &msg,
                          const vector<struct PDB_model> &models,
                          const set<QString> only_atoms = {});
  void info_model_residues (const QString &msg, struct PDB_model &model);
  void info_model_vector_mw (const QString &msg,
                             const vector<struct PDB_model> &models,
                             bool detail = false);
  void info_model_vector_vbar (const QString &msg,
                               const vector<struct PDB_model> &models);
  void info_residue_p_residue (struct PDB_model &model); // consistency report
  void info_compare_residues (struct residue *res1,
                              struct residue *res2); // compare residues
  void info_residue_vector (const QString &msg,
                            vector<struct residue> &residue_v,
                            bool only_pKa_dependent = true);
  void info_model_p_residue (const QString &msg, const struct PDB_model &model,
                             bool only_pKa_dependent);
  void info_mw (const QString &msg, const vector<struct PDB_model> &models,
                bool detail = false);
  void info_mw (const QString &msg, const struct PDB_model &model,
                bool detail = false);
  void info_residue_protons_electrons_at_pH (double pH,
                                             const struct PDB_model &model);
  ...
  void set_ionized_residue_vector (
      vector<struct residue>
          &residue_v); // apply ionization to complete residue vector
  void reset_ionized_residue_vectors (); // for saxs hplc options

  // in attic: vector < struct residue * > model_residue_v( const struct
  // PDB_model & model ); // returns a residue * vector based upon PDB_atoms's
  // p_residue
  ...
  QString getExtendedSuffix (bool prerun = true, bool somo = true,
                             bool no_ovlp_removal = false, bool vdw = false);
  ...
  map<QString, vector<int> >
      multi_residue_map; // maps residue to index of residue_list
  map<QString, vector<int> > valid_atom_map; // maps resName|atomName|pos
  //                                                  in multi_residue_map to
  //                                                  index of atoms
  ...
  //                                       maps atomName to hybrid_name for abb
  //                                       and nc usage
  ...
  // save_residue_list and save_multi_residue_map contain the results of
  // 'read_residue_file' the automatic bead builder adds temporary residues, so
  // it uses these to reset the values in lieu of re-reading the residue
  // file...
  ...
  // checks for new versions of somo.config, .atom, .saxs_atom, .hybrid,
  // .residue and backs up previous versions
  ...
  map<QString, vector<float> >
      connection_dist_stats;              // 0 = min, 1 = max, 2 = avg
  ...
  void dna_rna_resolve (); // often DNA is encoded without the D, check each
                           // chain and ask about & adding it

  map<QString, bool> cluster_additional_methods_options_active;
  map<QString, map<QString, QString> >
      cluster_additional_methods_options_selected;

  void calc_mw (); // after pdb (bead_model?) loaded, go through and compute
                   // PDB_model.mw (& later PDB_chain.mw?)
  void calc_vol_for_saxs ();            // run when there are errors
  void calc_bead_mw (struct residue *); // calculate the molecular weight of
                                        // all beads in residue
  ...
  bool atom_align (vector<point> transform_from, vector<point> transform_to,
                   ...
                   QString &error_msg, bool force_zero_center = false);
  ...
  map<QString, vector<vector<QString> > >
      state_molecules_residue_missing_atoms;
  map<QString, vector<vector<unsigned int> > >
      state_molecules_residue_missing_atoms_beads;
  ...
  double ionized_residue_atom_mw (vector<double> &fractions,
                                  struct residue *res, struct atom *atom);
  double ionized_residue_atom_radius (vector<double> &fractions,
                                      struct residue *res, struct atom *atom);
  double ionized_residue_atom_hydration (vector<double> &fractions,
                                         struct residue *res,
                                         struct atom *atom);
  double ionized_residue_atom_protons (vector<double> &fractions,
                                       struct residue *res, struct atom *atom);
...
private slots:
  ...
  void fractal_dimension (bool from_parameters = false,
                          save_info *fd_save_info = (save_info *)0);
...
public slots:
  ...
  void clear_display ();
  void reload_pdb ();
  int calc_somo (bool no_ovlp_removal
                 = false); // compute asa and then refine bead_model
  int calc_somo_o ();      // somo model with overlaps
  int calc_grid_pdb (bool no_ovlp_removal
                     = false); // compute grid model from pdb
  ...
  int
  calc_iqq (bool bead_model, bool create_native_saxs = true,
            bool do_raise
            = true); // bring up saxs window if needed and compute iqq curve
  int
  calc_prr (bool bead_model, bool create_native_saxs = true,
            bool do_raise
            = true); // bring up saxs window if needed and compute prr curve
  ...
  void visualize (bool movie_frame = false, QString dir = "",
                  float scale = 1.0, bool black_background = false,
                  bool do_pat = false);
...
private slots:
  ...
  // The next function returns a boolean to indicate whether the atom that was
  // just assigned belongs to a new residue or is part of a previously assigned
  // residue true: new residue sequence number, false: still the same the
  // integer argument contains the last sequence number (last_resSeq)
  ...
  int create_beads (QString *error_string,
                    bool quiet = false); // turn pdb/atom model into bead_model
  int create_vdw_beads (QString &error_string,
                        bool quiet
                        = false); // turn pdb/atom model into bead_model
  ...
  int overlap_check (bool sc, bool mc, bool buried, double tolerance,
                     int limit = 0,
                     bool from_overlap_hydro = false); // check for overlaps
  int compute_asa (bool bd_mode = false,
  ...
  void view_file (const QString &,
                  QString title = "SOMO editor"); // call editor to view a file
  void bead_check (bool use_threshold = false, bool message_type = false,
                   bool vdw = false, bool only_asa = false); // recheck beads
  void load_config ();
  void write_config ();
  ...
  void write_bead_spt (QString, vector<PDB_atom> *, bool movie_frame = false,
                       float scale = 1, bool black_background = false);
  void write_bead_model (QString, vector<PDB_atom> *, QString extra_text = "");
  void write_bead_model (QString, vector<PDB_atom> *, int bead_model_output,
                         QString extra_text = "");
  ...
  void update_vbar (); // update the results.vbar everytime something changes
                       // the vbar in options or calculation
  ...
  int run_browflex ();
  int browflex_get_no_of_beads (
      QString filename); // open main file, read molec file, return # of beads
                         // or zero if not found

  int compute_pb_normals (); // compute the normal vectors for each peptide
                             // bond plane
  ...
  int compute_bd_connections (); // computes intersected pb beads within
                                 // tolerance map
  int write_pdb (QString fname, vector<PDB_atom> *model);

  bool write_pdb_from_model (const PDB_model &model, QString &errors,
                             QString &writtenname,
  ...
  int write_contact_plot (QString fname, vector<PDB_atom> *model,
                          float thresh);
...
public:
  ...
  void radial_reduction_thr_setup (unsigned int, vector<PDB_atom> *,
                                   vector<bool> *, vector<bool> *,
                                   vector<BPair> *, unsigned int, double);
