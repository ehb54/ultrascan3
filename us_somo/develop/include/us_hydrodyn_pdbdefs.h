...
public:
  ...
  bool
  operator< (const point &objIn) const
  {
    return axis[0] < objIn.axis[0]
           || (axis[0] == objIn.axis[0] && axis[1] < objIn.axis[1])
           || (axis[0] == objIn.axis[0] && axis[1] == objIn.axis[1]
               && axis[2] < objIn.axis[2]);
  }
  ...
  operator== (const point &objIn) const
  {
    return axis[0] == objIn.axis[0] && axis[1] == objIn.axis[1]
           && axis[2] == objIn.axis[2];
  }
};
...
struct PDB_atom
{
  ...
  float bead_actual_radius; // used for radial reduction % computation, does
                            // not get reduced
  int placing_method; // baric method (see struct bead->placing method, -1
                      // undefined

  //        float vol_intersection            // temporary value used in
  //        popping
  vector<struct PDB_atom *> all_beads; // this is used to keep track of beads
                                       // that have been popped together
  QString residue_list;                // for loaded bead models
  int group;            // used in surfracer for breaking up groups of atoms
  QString count_idx;    // used in us_hydrodyn for backtracking on bead/atom
                        // exceptions
  ...
  // should be in a separate header, but without a rewrite of bead model
  // functions, we'll kludge into bead0
  QString is_vdw;
...
struct PDB_model
{
  ...
  vector<struct residue> residue; // keep track of amino acid sequence in
                                  // molecule for vbar calculation
  float vbar;
...
struct bead
{
  ...
  unsigned int placing_method; // baric method
                               // 0: place the bead at the center of gravity of
                               // all atoms 1: placing the bead at the position
                               // of the furthest atom specified in the p3 file
                               // 2: no positioning necessary (only one atom to
                               // place)
  ...
  float mw;                // bead mw
  float ionized_mw_delta;
  bool hydration_flag;  // false = use sum of atom's hydrations, true = bead
                        // hydration overrides
  float atom_hydration; // number of waters bound based upon sum of atoms'
                        // hydrations
  // used only in residue editor
  float atom_hydration2; // number of waters bound based upon sum of atoms'
                         // hydrations at pH 14
...
struct atom
{
  ...
  int ionization_index; // map into the pKa sequence number for ionizaztion
                        // changes
  unsigned int bead_assignment; // which bead this atom belongs to
  bool positioner; // does this atom control position? (yes=1, no=0)
  unsigned int
      serial_number;   // the serial number the atom occupies in the residue
...
struct residue
{
  ...
  map<int, struct atom> r_atom_0; // protonated values for multi-pKa, mapping
                                  // atom position in r_atom
  ...
  float ionized_mw_delta; // computed ionized_mw_delta ... sum of those in the
                          // atoms which equals sum in the beads
  float mw;
...
public:
  ...
  unsigned int index;
  bool
  operator< (const sortable_float &objIn) const
  {
    return (f < objIn.f);
  }
};
