...
class pointmass {
   public:
      ...
      bool operator<(const pointmass &objIn) const {
         return axis[ 0 ] < objIn.axis[ 0 ] || (axis[ 0 ] == objIn.axis[ 0 ] && axis[ 1 ] < objIn.axis[ 1 ])
                || (axis[ 0 ] == objIn.axis[ 0 ] && axis[ 1 ] == objIn.axis[ 1 ] && axis[ 2 ] < objIn.axis[ 2 ]);
      }

      bool operator==(const pointmass &objIn) const {
         return axis[ 0 ] == objIn.axis[ 0 ] && axis[ 1 ] == objIn.axis[ 1 ] && axis[ 2 ] == objIn.axis[ 2 ];
      }
};
...
class US_Fractal_Dimension {
   public:
      ...
      bool compute(
         enum US_Fractal_Dimension::methods method, const vector<pointmass> &points, double angstrom_start,
         double angstrom_end, double angstrom_steps, double enright_ca_pct_start, double enright_ca_pct_end,
      ...
      static QString method_name(enum US_Fractal_Dimension::methods method) {
         switch (method) {
            case USFD_BOX_MODEL:
               ...
               break;
            case USFD_BOX_ALT:
               return "Box Alternate";
               break;
            case USFD_BOX_MASS:
               return "Box Mass";
               break;
            case USFD_ENRIGHT:
               return "Mass fractal D (slice)";
               break;
            case USFD_ENRIGHT_FULL:
               return "Mass fractal D (full)";
               break;
            case USFD_ROLL_SPHERE:
               return "Surface fractal D";
               break;
            default:
   ...
   private:
      ...
      bool compute_box_counting(
         const vector<pointmass> &points, double angstrom_start, double angstrom_end, double angstrom_steps,
         double &fd // computed fractal dimension
         ,
         vector<vector<vector<double>>> &x // x coordinate of plots
         ,
      ...
      bool compute_enright(
         const vector<pointmass> &points, double angstrom_start, double angstrom_end, double angstrom_steps,
         double enright_ca_pct_start, double enright_ca_pct_end, double &fd // computed fractal dimension
         ,
         vector<vector<vector<double>>> &x // x coordinate of plots
         ,
         vector<vector<vector<double>>> &y // y coordinate of plots
