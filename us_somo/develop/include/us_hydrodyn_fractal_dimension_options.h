...
public:
  US_Hydrodyn_Fractal_Dimension_Options (
      ...
      bool *fractal_dimension_options_widget, void *us_hydrodyn,
      QWidget *p = 0);
  ~US_Hydrodyn_Fractal_Dimension_Options ();

  static QString
  options (map<QString, QString> &parameters, double xmin = DBL_MAX,
           double xmax
           = DBL_MAX); // returns current options in a string format
