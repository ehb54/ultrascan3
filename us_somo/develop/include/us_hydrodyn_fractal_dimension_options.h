...
class US_EXTERN US_Hydrodyn_Fractal_Dimension_Options : public QFrame {
   ...
   public:
      US_Hydrodyn_Fractal_Dimension_Options(
         map<QString, QString> *parameters, bool *fractal_dimension_options_widget, void *us_hydrodyn, QWidget *p = 0);
      ~US_Hydrodyn_Fractal_Dimension_Options();

      static QString options(
         map<QString, QString> &parameters, double xmin = DBL_MAX,
         double xmax = DBL_MAX); // returns current options in a string format
   ...
   private:
      ...
      enum WidgetType : int { QLINEEDIT = 0, QCOMBOBOX = 1, QCHECKBOX = 2, QLABEL = 3, WT_UNKNOWN = 4 };
