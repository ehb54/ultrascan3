...
class US_Eigen {
   public:
      ...
      bool polyfit(
         const vector<double> &x, const vector<double> &y, const vector<double> &e, const int degree,
         vector<double> &coeff // the polynomial coefficients
         ,
         double &chi2, enum fit_methods fit_method = EIGEN_HOUSEHOLDER_QR_PIVOT_COL,
         enum weight_methods weight_method = EIGEN_1_OVER_SD_SQ);

      bool polyfit(
         const vector<double> &x, const vector<double> &y, const int degree,
         vector<double> &coeff // the polynomial coefficients
         ,
         double &chi2, enum fit_methods fit_method = EIGEN_HOUSEHOLDER_QR_PIVOT_COL,
      ...
      bool evaluate_polynomial(
         const vector<double> &coeff, const double x_start, const double x_end, const int points, vector<double> &x,
         vector<double> &y);
