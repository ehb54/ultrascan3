...
// As of this writing, the Eigen software is available from
// https://gitlab.com/libeigen/eigen and further information about Eigen is
// available from https://eigen.tuxfamily.org The Eigen Source Code is subject
...
public:
  ...
  enum weight_methods : int
  {
    ...
    // The relative standard deviation (RSD) is often times more convenient. It
    // is expressed in percent and is obtained by multiplying the standard
  ...
  bool polyfit (const vector<double> &x, const vector<double> &y,
                const vector<double> &e, const int degree,
  ...
  bool evaluate_polynomial (const vector<double> &coeff, const double x_start,
                            const double x_end, const int points,
                            vector<double> &x, vector<double> &y);

  QString errors ();

  static QString gnuplot_poly (const vector<double> &coeff,
                               QString fname = "f");
