#ifndef US_HYDRODYN_MALS_FIT_GLOBAL_H
#define US_HYDRODYN_MALS_FIT_GLOBAL_H
...
class US_EXTERN US_Hydrodyn_Mals_Fit_Global : public QDialog {
  Q_OBJECT

  friend class US_Hydrodyn_Mals;

 public:
  US_Hydrodyn_Mals_Fit_Global(US_Hydrodyn_Mals *mals_win, QWidget *p = 0,
                              const char *name = 0);
  ~US_Hydrodyn_Mals_Fit_Global();
 ...
 private:
  ...
  US_Hydrodyn_Mals::gaussian_types gaussian_type;
  ...
  unsigned int mals_maxfpk_restart_tries;
  double mals_maxfpk_restart_pct;
...
extern vector<unsigned int>
    param_pos;  // index into fixed params or variable params
...
extern map<unsigned int, unsigned int>
