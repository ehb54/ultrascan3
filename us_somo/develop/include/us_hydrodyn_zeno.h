...
class US_Hydrodyn_Zeno {
 public:
  US_Hydrodyn_Zeno(hydro_options* options, hydro_results* results,
                   mQProgressBar* use_progress, US_Hydrodyn* us_hydrodyn);

  static bool test();
  bool run(QString filename, vector<PDB_atom>* bead_model, double& sum_mass,
           double& sum_volume, const double& Rg, mQProgressBar* use_progress,
           bool keep_files = false, bool zeno_cxx = false, int threads = 1);
  QString error_msg;
