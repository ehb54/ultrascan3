...
class US_EXTERN US_Hydrodyn_Pdb_Tool_Merge : public QFrame {
 ...
 public:
  US_Hydrodyn_Pdb_Tool_Merge(void *us_hydrodyn, void *pdb_tool_window,
 ...
 private:
  ...
  void sel_to_range(QTreeWidget *lv, vector<range_entry> &ranges,
...
#pragma warning(default : 4251)
  ...
  void update_t_csv_range(vector<range_entry> &ranges, unsigned int col_start,
  ...
  unsigned int residue_length(bool use_from, QString chain,
                              unsigned int start_residue,
                              unsigned int end_residue);

  unsigned int residue_offset_position(bool use_from, QString chain,
                                       unsigned int start_residue, int offset);
