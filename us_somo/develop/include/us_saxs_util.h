...
public:
  ...
  bool scalesum (QString outtag, vector<QString> tags,
  ...
  bool merge (QString outtag, QString tag1, double w1, QString tag2,
              double w2);

  bool interpolate (vector<double> &results, vector<double> &to_r,
                    vector<double> &from_r, vector<double> &from_pr);

  bool subbackground (QString outtag, QString solutiontag, QString buffertag,
                      double alpha);

  bool subcellwave (QString outtag, QString solutiontag, QString buffertag,
                    QString emptytag, double alpha);

  bool waxsguidedsaxsbuffersub (double &nrmsd, QString outtag,
                                QString solutiontag, QString buffertag,
                                QString waxstag, double alpha, double beta,
                                double dconst, double low, double high,
                                QString &waxscrop, QString &waxscropinterp,
                                bool prechecked = false,
                                bool waxsprecropinterp = false);

  bool waxsguidedsaxsbuffersubgridsearch (
      double &nrmsd, double &alphamin, double &betaamin, double &dconstmin,
  ...
  bool waxsguidedsaxsbuffersubgsmsearch (
      double &nrmsd, double &alphamin, double &betaamin, double &dconstmin,
      QString &log, QString outtag, QString solutiontag, QString buffertag,
      QString waxstag, int gsm_type, double alphalow, double alphahigh,
      double alphainc, double betalow, double betahigh, double betainc,
      double dconstlow, double dconsthigh, double dconstinc, double low,
      double high);
  bool compute_wgsbs (QString outfile, QString solutiontag, QString buffertag,
                      QString waxstag, int grids, int gsm_type,
                      long max_iterations, double gsmpercent, double alphalow,
                      double alphahigh, double alphaincg, double alphaincgsm,
                      double betalow, double betahigh, double betaincg,
                      double betaincgsm, double dconstlow, double dconsthigh,
                      double dconstincg, double dconstincgsm, double low,
                      double high, double &nrmsd, double &alphamin,
                      double &betamin, double &dconstmin, QString &cliperrors);

  bool setup_wgsbs_gsm_f_df (QString outtag, QString solutiontag,
                             QString buffertag, QString waxstag,
                             double alphalow, double alphahigh,
                             double alphainc, double betalow, double betahigh,
                             double betainc, double dconstlow,
                             double dconsthigh, double dconstinc, double low,
                             double high);

  bool wgsbs_gsm (double &nrmsd, double &alpha, double &beta, double &dconst,
                  int gsm_type, long max_iterations);

  bool guinier_plot (QString outtag, QString tag);

  bool
  guinier_fit (QString &log,
               QString tag, // tag needs to be preprocessed with guinierplot
               unsigned int startpos, unsigned int endpos, double &a,
               double &b, double &siga, double &sigb, double &chi2, double &Rg,
               double &I0, double &smaX, double &smin, double &sRgmin,
               double &sRgmax, bool compute_Rc = false,
               bool compute_Rt = false);

  bool guinier_remove_points (QString outtag, QString tag,
                              map<double, double> &removed,
                              unsigned int &pts_removed,
                              unsigned int &startpos, unsigned int &endpos,
                              double a, double b, double sd_limit);

  bool guinier_fit_with_removal (
      QString &log,
      QString tag, // tag needs to be preprocessed with guinierplot
      unsigned int startpos, unsigned int endpos, double &a, double &b,
      double &siga, double &sigb, double &chi2, double &Rg, double &I0,
      double &smax, double &smin, double &sRgmin, double &sRgmax,
      double sd_limit, map<double, double> &removed, unsigned int &pts_removed,
      bool compute_Rc = false, bool compute_Rt = false);
  ...
  bool guinier_fit2 ( // find best guinier fit
      ...
      double pointweightpower, // the exponent ofnumber of points when
                               // computing the best one
      //                          i.e. fitness = chi2 / ( number_of_points **
      //                          pointweightpower )
      double maxq, // the maximum q value allowed
      double &a, double &b, double &siga, double &sigb, double &chi2,
      double &Rg, double &I0, double &smaX, double &smin, double &sRgmin,
      double &sRgmax, unsigned int &beststart, unsigned int &bestend,
      bool compute_Rc = false, bool compute_Rt = false);
  ...
  bool merge_projects (QString outfile, double reference_mw_multiplier,
                       vector<QString> projects, bool run_gnom = false);
  bool build_wiki ();

  // build averages and wiki page for subdirectory 1d, create pngsplits # of
  // zoom (none if 1 or less)
  bool project_1d (QString wikitag, unsigned int pngsplits);

  // linear fit code, solves  y = a + bx, returing sigmas & chi2
  static void linear_fit (vector<double> x, vector<double> y, double &a,
                          double &b, double &siga, double &sigb, double &chi2);

  static void linear_fit (vector<double> x, vector<double> y, vector<double> e,
                          double &a, double &b, double &siga, double &sigb,
                          double &chi2);

  static bool pearsonpmcc (const vector<double> &x, const vector<double> &y,
                           double &r);

  static bool pearsonpmcc (const vector<double> &x, const vector<double> &y,
                           double &r, QString &error_msg);

  // linear fit code, solves  y = kx, returing chi2
  bool scaling_fit (vector<double> x, vector<double> y, double &a,
                    double &chi2);

  bool scaling_fit (vector<double> x, vector<double> y, vector<double> sd,
                    double &a, double &chi2);

  static bool sscaling_fit (vector<double> x, vector<double> y,
                            vector<double> sd, double &a, double &chi2);
  void nnls_fit (vector<double> x, vector<double> y, double &k,
                 double &nnls_rmsd);

  bool nnls_fit (vector<vector<double> > A, vector<double> y,
                 vector<double> &x, double &nnls_rmsd);

  static double calc_rmsd (vector<double> v1, vector<double> v2);
  static double
  calc_nrmsd (vector<double> v1,
              vector<double> v2); // normalized rmsd is rmsd divided by the
                                  // range (of v2) time 100 (for percent)

  static bool calc_chisq (const vector<double> &x, const vector<double> &y,
                          const vector<double> &sds, int parameters,
                          double &chisq, double &nchi, QString &errors);

  static bool calc_chisq1 (vector<double> bins, vector<double> ebins,
                           unsigned int &df, double &chisq, double &prob);
  static bool calc_chisq2 (vector<double> bins1, vector<double> bins2,
                           unsigned int &df, double &chisq, double &prob);
  static double calc_gammaln (double val);
  static bool calc_chisq_prob (double a, double x, double &prob);

  static bool calc_mychi2 (vector<double> x, vector<double> y,
                           vector<double> sds, double &chi2);

  static bool calc_myrmsd (vector<double> x, vector<double> y, double &rmsd);

  static bool calc_chisqshannon (vector<double> &q, vector<double> &Iexp,
                                 vector<double> &sexp, vector<double> &Icalc,
                                 double dmax, unsigned int k, QString &errors,
                                 double &ns, double &chi2shannon);

  bool setup_saxs_maps (QString atom_file, QString hybrid_file,
                        QString saxs_file);
  ...
  bool set_excluded_volume (PDB_atom &this_atom, double &vol,
                            double &scaled_vol, saxs_options &our_saxs_options,
  ...
  bool
  iqq_sphere (QString tag, // creates iqq for a sphere based on Rayleigh (1911)
              double radius, double delta_rho, double min_q, double max_q,
              double delta_q);

  bool iqq_sphere_grid (QString outtag, // build sphere I(q) on intag's grid
                        QString intag, double radius, double delta_rho);

  bool iqq_sphere_fit (QString outfile, QString infile, double min_radius,
                       double max_radius, double delta_radius,
                       double min_delta_rho, double max_delta_rho,
                       double delta_delta_rho, double min_q, double max_q,
  ...
  bool iqq_sphere_fit (
      ...
      double &best_fit_delta_rho, double &average_fit_radius,
      double &average_fit_delta_rho, double &start_q, double &end_q,
      double &delta_q, unsigned int &points_q, QString &best_tag,
      QString &nnls_tag, bool do_normalize = true);

  bool compute_rayleigh_structure_factors (double radius, double delta_rho,
                                           vector<double> &q,
                                           vector<double> &F);
  ...
  point plane (PDB_atom *a1, PDB_atom *a2,
               PDB_atom *a3);                 //( a3 - a2 ) x ( a1 - a2 )
  ...
  bool get_quadratic_interpolation_coefficients (vector<double> x,
                                                 vector<double> y,
                                                 vector<double> &c);

  // takes x1 on grid q1 and interpolates to x2 on use_q indexed points of q,
  // putting the result in r (on q2)
  bool linear_interpolate_iq_curve (vector<double> &q,
                                    vector<unsigned int> &use_q,
                                    vector<double> &x1, vector<double> &x2,
                                    vector<double> &r);

  bool quadratic_interpolate_iq_curve (vector<double> &q,
                                       vector<unsigned int> &use_q,
                                       vector<double> &x1, vector<double> x2,
                                       vector<double> &r);

  bool cubic_spline_interpolate_iq_curve (vector<double> &q,
                                          vector<unsigned int> &use_q,
                                          vector<double> &x1,
                                          vector<double> x2,
                                          vector<double> &r);

  bool quadratic_interpolate (vector<double> &x1, vector<double> &y1,
                              vector<double> &x2, vector<double> &y2);

  bool linear_interpolate (vector<double> &x1, vector<double> &y1,
                           vector<double> &x2, vector<double> &y2);

  // computes 2nd derivatives on y=f(x) and distributes n points
  // proportionately to abs(f'') and returns in r
  bool create_adaptive_grid (vector<double> &x, vector<double> &y,
                             unsigned int n, vector<unsigned int> &r);

  bool apply_natural_spline (vector<double> &xa, vector<double> &ya,
  ...
  static void natural_spline (vector<double> &x, vector<double> &y,
                              vector<double> &y2);

  static void natural_spline (vector<double> &x, vector<double> &y,
                              vector<double> &y2, double yp1, double ypn);

  // takes from_data with from_errors and places on to_grid resulting in
  // to_data & to_errors for systems without errors, these are assumed to be
  // computed iq curves so a natural spline interpolation is performed for
  // systems with errors, these are assumed to be experimental data it is only
  // acceptible going to a coarser grid, otherwise an error will result with
  // error_msg set

  static bool
  is_nonzero_vector (const vector<double> &v); // returns true if all non-zero
  static bool
  is_zero_vector (const vector<double> &v); // returns true if all zero

  bool
  interpolate_iqq_by_case (vector<double> from_grid, vector<double> from_data,
                           vector<double> from_errors, vector<double> to_grid,
                           vector<double> &to_data, vector<double> &to_errors);

  bool crop (vector<double> from_grid, vector<double> from_data,
             vector<double> from_errors, double min_q, double max_q,
             vector<double> &to_data, vector<double> &to_errors);

  static bool crop (const vector<double> &x, const vector<double> &y,
                    double x_min, double x_max, vector<double> &x_cropped,
                    vector<double> &y_cropped, QString &errormsg);

  static bool transpose_vvd (const vector<vector<double> > &x,
                             vector<vector<double> > &y, QString &errormsg);
  ...
  static map<QString, unsigned int>
  get_atom_summary_counts (PDB_model *model,
                           map<QString, QString> &residue_atom_hybrid_map,
                           saxs_options *use_saxs_options);
  static QString
  list_atom_summary_counts (PDB_model *model,
                            map<QString, QString> &residue_atom_hybrid_map,
                            saxs_options *use_saxs_options);
  bool compute_exponential (vector<double> &q, vector<double> &I,
                            vector<double> &coeff4, vector<double> &coeff5,
                            double &norm4, double &norm5);

  bool compute_exponential (vector<double> &q, vector<double> &I,
                            vector<double> &coeff4, vector<double> &coeff5,
                            vector<double> &coeffv, double &norm4,
                            double &norm5, double &normv,
                            // compute increased coeff size (when maxv > 5)
                            unsigned int maxv, bool use_gsm_fitting = false,
  ...
  bool grid (vector<PDB_atom> &bm, vector<PDB_atom> &result, double dR,
             bool accum_overlaps, // if true, beads will increase in size
             bool allow_partials
             = false // if true attempt "partial" smaller beads for edges
  );

  bool smooth (vector<double> &x, vector<double> &result, unsigned int points,
               bool smooth_endpoints = false);

  bool average (vector<double> &q, vector<double> &I, vector<double> &e,
                double &avg_q, double &avg_I, double &avg_e);

  bool bin (vector<double> &q, vector<double> &I, vector<double> &e,
            vector<double> &new_q, vector<double> &new_I,
            vector<double> &new_e, unsigned int points);

  bool align_test ();

  static bool average (const vector<vector<double> > &ys,
                       vector<double> &y_avg, vector<double> &y_sd,
                       QString &error_msg);

  static bool read_sas_data (QString filename, vector<double> &q,
                             vector<double> &I, vector<double> &e,
                             QString &error_msg);
  static void clip_data (double minq, double maxq, vector<double> &q,
                         vector<double> &I, vector<double> &e);
  static void bin_data (int bins, bool log_bin, vector<double> &q,
                        vector<double> &I, vector<double> &e);
  static bool bin_data (int bins, bool log_bin, vector<double> &q,
                        vector<double> &I, vector<double> &e,
                        QString &error_msg, QString &notice_msg);

  bool flush_pm_csv (vector<QString> &csv_name, vector<double> &csv_q,
  ...
  static void compute_tau (double drrev1, double drrev2, double drrev3,
                           double fconv, vector<double> &tau);

  bool pdb_mw (QString file, double &mw);
  bool pdb_mw (QStringList &qsl, double &mw);

  static bool pdb2fasta (QString outfile, QStringList &files,
                         int max_line_size = 120);

  static bool mwt (const vector<double> &q, const vector<double> &I, double Rg,
                   double sigRg, double I0, double sigI0, double k, double c,
                   double qmax, double &Vc, double &Qr, double &mwt,
                   double &sigmwt, QString &messages, QString &notes,
                   QString &warning);

  bool mwc (const vector<double> &q, const vector<double> &I, double Rg,
            double sigRg, double I0, double sigI0, double mw_per_N, double &qm,
            double &Vc, double &Qr, double &mwc, double &sigmwc,
            QString &messages, QString &notes);
  ...
  bool cormap (const vector<double> &q, const vector<vector<double> > &I,
               vector<vector<double> > &rkl, int &N, int &S, int &C,
               double &P);
  ...
  bool run_hydro (map<QString, QString> &parameters,
                  map<QString, QString> &results);

  bool run_pat (map<QString, QString> &parameters,
                map<QString, QString> &results);

  bool run_align (map<QString, QString> &parameters,
                  map<QString, QString> &results);

  // bool screen_pdb( QString, bool display_pdb = false, bool skip_clear_issue
  // = false );
  void read_residue_file ();
  bool screen_pdb (QString filename,
                   bool parameters_set_first_model); //, bool display_pdb, bool
                                                     //skipclearissue );
  int check_for_missing_atoms_hydro (QString *error_string, PDB_model *model,
                                     bool parameters_set_first_model);
  void reload_pdb (bool parameters_set_first_model);

  bool hard_coded_defaults (map<QString, QString> &results,
                            map<QString, QString> &parameters);
  bool set_default (map<QString, QString> &results,
                    map<QString, QString> &parameters);
  ...
  QString getExtendedSuffix_somo (bool prerun = true, bool somo = true,
                                  bool no_ovlp_removal = false);

  void bead_check (bool use_threshold = false,
                   bool message_type = false); // recheck beads
  int radial_reduction (bool from_grid = false, int use_ppos = 0,
                        int mppos = 0);
  int overlap_check (bool sc, bool mc, bool buried, double tolerance,
                     int limit);
  int calc_hydro ();
  int do_calc_hydro ();
  ...
  QString project; // name of the current project - derived from the prefix of
                   // the pdb filename

  QStringList list_of_models;

  int calc_somo (bool no_ovlp_removal = false,
                 bool parameters_set_first_model = false);
  bool
  calc_somo_o (bool parameters_set_first_model); // somo model with overlaps
...
private:
  ...
  bool run_ssbond (map<QString, QString> &parameters,
                   map<QString, QString> &results);

  void SS_setup (); // called once to setup any persistant distance threshold
                    // structures
  void SS_init (); // called during load pdb to setup any processing structures
  void SS_apply (
      struct PDB_model &model,
      QString &ssbond_data); // called during load pdb to process any
                             // adjustments (CYS->CYH etc) *** per model! ***
  void SS_change_residue ( // change the residue of an entry to target_residue
      struct PDB_model &model, const QString &line,
  ...
  int compute_asa (bool bd_mode = false, bool no_ovlp_removal = false,
                   bool parameters_set_first_model
                   = false); // calculate maximum accessible surface area
  ...
  bool run_gnom (QString project, QString prefix, vector<QString> files,
                 vector<double> use_rmax_start, vector<double> use_rmax_end,
                 vector<double> use_rmax_inc, vector<double> use_crop_low,
  ...
  map<QString, vector<QString> >
      wave_buffer_names; // maps names to assoc buffer names
  map<QString, vector<QString> >
      wave_empty_names;                // maps names to assoc empty cell names
  map<QString, QString> wave_comments; // maps names to comments
  map<QString, double>
      wave_alpha_starts;                // maps names to alpha start (for waxs)
  ...
  // computed:
  map<QString, QString>
      wave_sb; // maps wave to standard background (and empty cell) subtraction
  map<QString, QString>
      wave_wgsbs; // maps saxs wave to waxs guided saxs background subtraction
  map<QString, QString> wave_wgsbs_unscaled; // maps saxs wave to waxs guided
                                             // saxs background subtraction
  map<QString, QString> wave_join; // maps saxs wave to waxs guided saxs
                                   // background subtraction join
  ...
  static bool compute_rg_from_pr (const vector<double> &r,
                                  const vector<double> &pr, double &Rg,
                                  QString &errormsg);
...
private:
  ...
  bool compute_gsm_exponentials (vector<double> &par, vector<double> &t,
                                 vector<double> &y, QString method);

  void compute_gsm_exponentials_setup (vector<double> &t, vector<double> &y);
  // other gsm stuff
  ...
  bool assign_atom (const QString &str1, PDB_chain *temp_chain,
                    QString *last_resSeq);
  ...
  map<QString, int>
      new_residues; // maps resName|atom_count to {0,1} for duplicate checks
  ...
  map<QString, vector<int> >
      save_multi_residue_map; // maps residue to index of residue_list
  ...
  //                                       counts how many atoms are in each
  //                                       residue
  ...
  //                                       1 == ok
  //                                       2 == skip
  //                                       3 == use automatic bead builder for
  //                                       residue 4 == use automatic bead
  //                                       builder for missing atom 5 == not
  //                                       ok, duplicate or non-coded atom
  map<QString, bool> broken_chain_end;
  //                                  maps molecule #|resSeq to flag indicating
  //                                  broken end of chain
  map<QString, bool> broken_chain_head;
  //                                  maps molecule #|resSeq to flag indicating
  //                                  broken head of chain
  map<QString, vector<QString> > molecules_residues_atoms;
  //                                  maps molecule #|resSeq to vector of atom
  //                                  names
  ...
  //                                  maps molecule #|resSeq to vector of
  //                                  errors each element in the vector
  //                                  corresponds to the dup_residue_map pos
  //                                  for the residue
  map<QString, vector<int> > molecules_residue_missing_counts;
  //                                  maps molecule #|resSeq to vector of
  //                                  missing count if any atoms errors that
  //                                  are "non-missing" i.e. duplicate or
  //                                  non-coded, then the value is set to -1
  map<QString, vector<vector<QString> > > molecules_residue_missing_atoms;
  //                                  maps molecule #|resSeq to vector of
  //                                  vector of missing atoms each vector in
  //                                  the vector corresponds to the
  //                                  dup_residue_map pos for the residue
  map<QString, vector<vector<unsigned int> > >
      molecules_residue_missing_atoms_beads;
  //                                  maps molecule #|resSeq to vector of
  //                                  vector of missing atoms beads each vector
  //                                  in the vector corresponds to the
  //                                  dup_residue_map pos for the residue
  map<QString, bool> molecules_residue_missing_atoms_skip;
  //                                  maps molecule
  //                                  #|resSeq|multiresmappos|atom_no to flag
  //                                  if true, the atom should be skipped
  map<QString, int> use_residue;
  //                                  maps molecule #|resSeq to correct residue
  //                                  in residue_list to use
  map<QString, bool> skip_residue;
  //                                  flags molecule "#|resSeq|residue_list
  //                                  entry" to be ignored
  map<QString, int> molecules_residue_min_missing;
  //                                  maps molecule #|resSeq to pos of entry
  //                                  with minimum missing count
  ...
  static bool write_iq (QString &name, QString &msg, vector<double> &q,
                        vector<double> &I);
  bool flush_output ();
  ...
  bool atom_align (vector<point> transform_from, vector<point> transform_to,
                   vector<point> apply_from, vector<point> &result);
  ...
  map<QString, QString>
      dmd_mol2_res; // newly assigned residue name to original residue name
  map<QString, map<int, int> >
      dmd_chain; // maps chain id and residue number to dmd's chain number
  map<QString, map<int, int> >
      dmd_res; // maps chain id and residue number to dmd's residue number
  map<QString, map<int, int> >
      dmd_res_link; // maps chain id and residue number to dmd's residue number
                    // for links
  map<QString, map<int, QString> >
      dmd_org_chain; // maps chain id and residue number to pdb's original
                     // chain number
  map<QString, map<int, int> > dmd_org_res; // maps chain id and residue number
                                            // to pdb's original residue number
  QStringList dmd_pdb_add_back; // lines to restore to pdbs - REMARKs etc, not
                                // ATOMs or HETATMs
  map<QString, QStringList> dmd_pdb_prepare_reports; // for easy GUI display
  map<QString, set<int> >
      dmd_chain_is_hetatm; // maps chain id and residue number to hetatm status
  ...
  QString dmd_next_res (const QString &source); // returns a unique residuename
                                                // and updates dmd_mol2
  bool dmd_pdb_prepare (QStringList &qsl_pdb, QStringList &qsl_pdb_removed,
  ...
  bool dmd_pdb_restore (const QStringList &qsl_pdb,
                        QStringList &qsl_pdb_restored, bool add_back = true);
  ...
  bool strip_pdb (QString &pdb_stripped, const QString &pdb,
  ...
  bool dmd_run_with_log (const QString &tag, const QString &cmd,
                         const QString &log);
  ...
  // each atom is assigned to one cubelet and then saxs curves are computed on
  // each, then 1sa spheres are created for each which becomes the a2sb model
  ...
  long nsa_min_gsm_5_1 (our_vector *i, double epsilon, long max_iter);

  long nsa_min_fr_pr_cgd (our_vector *i, double epsilon, long max_iter);

  long nsa_min_hessian_bfgs (our_vector *ip, double epsilon, long max_iter);
  ...
  bool bspline_basis (unsigned int basis, double q, double &value);
  // prepare the net for bspline calcs
  bool bspline_prepare (unsigned int knots, unsigned int degree);
  bool bspline_test ();

  bool iqq_cuda (vector<double> &q, vector<saxs_atom> &atoms,
  ...
  double compute_ff (saxs &sa,     // gaussian decomposition for the main atom
  ...
  bool load_rotations_mpi (unsigned int number,
                           vector<vector<double> > &rotations);
  void
  hypercube_rejection_drand_rotations (unsigned int number,
                                       vector<vector<double> > &rotations);
#endif
  ...
  // From (private) US_Hydrodyn:: in an order as it goes in
  // 'read_residue_file()' //////// */
