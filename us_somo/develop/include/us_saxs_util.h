...
class US_EXTERN US_Saxs_Util {
 public:
  ...
  bool setup_wgsbs_gsm_f_df(QString outtag, QString solutiontag,
                            QString buffertag, QString waxstag, double alphalow,
                            double alphahigh, double alphainc, double betalow,
                            double betahigh, double betainc, double dconstlow,
  ...
  bool guinier_remove_points(QString outtag, QString tag,
                             ...
                             unsigned int &pts_removed, unsigned int &startpos,
                             unsigned int &endpos, double a, double b,
                             double sd_limit);
  ...
  bool scaling_fit(vector<double> x, vector<double> y, double &a, double &chi2);
  ...
  bool nnls_fit(vector<vector<double> > A, vector<double> y, vector<double> &x,
                double &nnls_rmsd);

  static double calc_rmsd(vector<double> v1, vector<double> v2);
  static double calc_nrmsd(
      vector<double> v1,
      vector<double> v2);  // normalized rmsd is rmsd divided by the range (of
                           // v2) time 100 (for percent)
  ...
  bool set_excluded_volume(PDB_atom &this_atom, double &vol, double &scaled_vol,
  ...
  bool iqq_sphere_fit(QString tag, vector<double> q, vector<double> I,
                      vector<double> I_errors, double min_radius,
                      double max_radius, double delta_radius,
                      double min_delta_rho, double max_delta_rho,
                      double delta_delta_rho, double min_q, double max_q,
                      vector<double> &by_radius, vector<double> &by_delta_rho,
                      vector<double> &val_radius, vector<double> &val_delta_rho,
                      map<double, unsigned int> &index_radius,
                      map<double, unsigned int> &index_delta_rho,
                      double &best_fit_radius, double &best_fit_delta_rho,
                      double &average_fit_radius, double &average_fit_delta_rho,
                      double &start_q, double &end_q, double &delta_q,
                      unsigned int &points_q, QString &best_tag,
                      QString &nnls_tag, bool do_normalize = true);

  bool compute_rayleigh_structure_factors(double radius, double delta_rho,
                                          vector<double> &q, vector<double> &F);
  ...
  // computes 2nd derivatives on y=f(x) and distributes n points proportionately
  // to abs(f'') and returns in r
  ...
  static bool is_nonzero_vector(
      const vector<double> &v);  // returns true if all non-zero
  static bool is_zero_vector(
      const vector<double> &v);  // returns true if all zero

  bool interpolate_iqq_by_case(vector<double> from_grid,
                               vector<double> from_data,
                               vector<double> from_errors,
                               vector<double> to_grid, vector<double> &to_data,
  ...
  static map<QString, unsigned int> get_atom_summary_counts(
      PDB_model *model, map<QString, QString> &residue_atom_hybrid_map,
      saxs_options *use_saxs_options);
  static QString list_atom_summary_counts(
      PDB_model *model, map<QString, QString> &residue_atom_hybrid_map,
      saxs_options *use_saxs_options);
  ...
  bool compute_exponential(vector<double> &q, vector<double> &I,
                           vector<double> &coeff4, vector<double> &coeff5,
                           vector<double> &coeffv, double &norm4, double &norm5,
                           double &normv,
  ...
  bool bin(vector<double> &q, vector<double> &I, vector<double> &e,
           vector<double> &new_q, vector<double> &new_I, vector<double> &new_e,
           unsigned int points);

  bool align_test();

  static bool average(const vector<vector<double> > &ys, vector<double> &y_avg,
                      vector<double> &y_sd, QString &error_msg);
  ...
  static bool bin_data(int bins, bool log_bin, vector<double> &q,
                       vector<double> &I, vector<double> &e, QString &error_msg,
                       QString &notice_msg);
  ...
  bool cormap(const vector<double> &q, const vector<vector<double> > &I,
              vector<vector<double> > &rkl, int &N, int &S, int &C, double &P);
  ...
  bool calc_somo_o(
      bool parameters_set_first_model);  // somo model with overlaps
 ...
 private:
  ...
  //                                       builder for missing atom 5 == not ok,
  //                                       duplicate or non-coded atom
  ...
  //                                  missing count if any atoms errors that are
  //                                  "non-missing" i.e. duplicate or non-coded,
  //                                  then the value is set to -1
  map<QString, vector<vector<QString> > > molecules_residue_missing_atoms;
  //                                  maps molecule #|resSeq to vector of vector
  //                                  of missing atoms each vector in the vector
  //                                  corresponds to the dup_residue_map pos for
  //                                  the residue
  map<QString, vector<vector<unsigned int> > >
      molecules_residue_missing_atoms_beads;
  //                                  maps molecule #|resSeq to vector of vector
  //                                  of missing atoms beads each vector in the
  //                                  vector corresponds to the dup_residue_map
  //                                  pos for the residue
  map<QString, bool> molecules_residue_missing_atoms_skip;
  //                                  maps molecule
  //                                  #|resSeq|multiresmappos|atom_no to flag if
  //                                  true, the atom should be skipped
  ...
  bool set_control_parameters_from_experiment_file(
      QString filename, bool load_without_interp = false);
  ...
  bool load_rotations(unsigned int number, vector<vector<double> > &rotations);
  bool compute_1d();
  ...
  void hypercube_rejection_drand_rotations(unsigned int number,
                                           vector<vector<double> > &rotations);
#endif
