...
class US_EXTERN US_Hydrodyn_Cluster_Dmd : public QDialog {
 ...
 public:
  US_Hydrodyn_Cluster_Dmd(csv &csv1, void *us_hydrodyn, QWidget *p = 0,
                          const char *name = 0);
 ...
 private:
  ...
  map<QString, map<QString, map<int, int> > >
  ...
  bool dmd_native_range(const QString &filename, const QString &chainid,
                        const int resseq_start, const int resseq_end,
                        QString &dmd_static);
