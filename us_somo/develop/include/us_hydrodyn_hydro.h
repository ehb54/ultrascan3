...
struct hydro_options {
  int unit;  // exponent from units in meter (example: -10 = Angstrom, -9 =
             // nanometers)
  ...
  bool reference_system;     // false: diffusion center, true: cartesian origin
                             // (default false)
  bool boundary_cond;        // false: stick, true: slip (default false)
  bool volume_correction;    // false: Automatic, true: manual (provide value)
  bool use_avg_for_volume;   // true: if volume_correction true, will replace
                             // volumes with averages
  ...
  bool rotational;  // false: include beads in volume correction for rotational
                    // diffusion, true: exclude
  bool viscosity;   // false: include beads in volume correction for intrinsic
                    // viscosity, true: exclude
  bool
      overlap_cutoff;  // false: same as in model building, true: enter manually
...
class US_EXTERN US_Hydrodyn_Hydro : public QFrame {
 ...
 public:
  US_Hydrodyn_Hydro(struct hydro_options *, bool *, void *, QWidget *p = 0,
                    const char *name = 0);
  ~US_Hydrodyn_Hydro();
