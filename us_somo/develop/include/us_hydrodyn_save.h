...
class US_EXTERN US_Hydrodyn_Save : public QFrame {
   ...
   public:
      ...
      US_Hydrodyn_Save(
         save_info *save, void *us_hydrodyn,
      ...
      static save_data save_data_initialized_from_bead_model(
         const vector<PDB_atom *> model, bool bead_exclusion = true); // returns initialized save_data from model info
      static save_data save_data_initialized_from_bead_model(
         const vector<PDB_atom> &model, bool bead_exclusion = true); // returns initialized save_data from model info
      ...
      QString hydroFormatStats(
         vector<save_data>,
         enum HydroTypes hydrotype = HYDRO_UNKNOWN); // returns a 'hydro_res' format data string of the avg, st.dev.
