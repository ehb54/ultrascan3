...
struct save_data {
  ...
  QString
      hydro_res;  // copy of what is written to hydro_res file (model specific)
  ...
  double tra_fric_coef_sd;  // sd of "Translational frictional coefficient
                            // [g*cm^2/s]"
...
class US_EXTERN US_Hydrodyn_Save : public QFrame {
 ...
 public:
  enum HydroTypes { HYDRO_UNKNOWN, HYDRO_SMI, HYDRO_ZENO, HYDRO_GRPY };

  US_Hydrodyn_Save(save_info *save, void *us_hydrodyn,
  ...
  static save_data save_data_initialized_from_bead_model(
      const vector<PDB_atom *> model,
      bool bead_exclusion =
          true);  // returns initialized save_data from model info
  static save_data save_data_initialized_from_bead_model(
      const vector<PDB_atom> &model,
      bool bead_exclusion =
          true);  // returns initialized save_data from model info
  ...
  QString hydroFormatStats(
      vector<save_data>, enum HydroTypes hydrotype =
                             HYDRO_UNKNOWN);  // returns a 'hydro_res' format
                                              // data string of the avg, st.dev.
  vector<save_data> stats(
      vector<save_data> *);  // returns a 2 vector containing avg, st.dev.
