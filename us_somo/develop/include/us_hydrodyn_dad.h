...
class DAD_Angle {
 public:
  ...
  QString list_rich(
      double lambda = 0,
      double n = 0);  // set lambda & n non-zero to include q values
...
class US_EXTERN US_Hydrodyn_Dad : public QFrame {
 ...
 private:
  ...
  void powerfit_add_marker(QwtPlot *plot, double pos, QColor color,
                           QString text,
 ...
 private:
  ...
  void baseline2_add_marker(QwtPlot *plot, double pos, QColor color,
                            QString text,
 ...
 private:
  ...
  void guinier_add_marker(QwtPlot *plot, double pos, QColor color, QString text,
                          Qt::Alignment align = Qt::AlignRight | Qt::AlignTop);
  ...
  bool gg_fit_vector(vector<double> &fit,
                     vector<double> &pfit);  // returns a vector of the chi^2 or
                                             // rmsd and their q values &
 ...
 private:
  ...
  vector<double> conc_curve(vector<double> &t, unsigned int peak, double conv);
  ...
  enum gaussian_types { GAUSS, EMG, GMG, EMGGMG };
