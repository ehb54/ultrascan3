#ifndef US_HYDRODYN_DAD_H
#define US_HYDRODYN_DAD_H
...
#define DAD_LAMBDA_EXTC_MSG                                                   \
  QString (                                                                   \
      "<center>Selected lambda %1, extinction coefficient %2</br></center>")  \
      .arg (dad_param_lambda)                                                 \
      .arg (dad_param_g_extinction_coef)
...
public:
  ...
  QString list ();
  QString summary ();
  QString summary_rich ();
  bool load (const QString &filename, const QStringList &filelines,
             QString &errormsg);
...
public:
  ...
  QString list ();
  QString list_csv ();
  QString list_rich (double lambda = 0,
                     double n
                     = 0); // set lambda & n non-zero to include q values
...
public:
  ...
  bool load (const QString &filename, const QStringList &csvlines,
             QString &errormsg);
  bool load (const QString &filename, QString &errormsg);
...
class US_EXTERN US_Hydrodyn_Dad : public QFrame
{
  Q_OBJECT
  ...
  friend class US_Hydrodyn_Dad_Fit;
  friend class US_Hydrodyn_Dad_Fit_Global;
  friend class US_Hydrodyn_Dad_Options;
  friend class US_Hydrodyn_Dad_Gauss_Mode;
  friend class US_Hydrodyn_Dad_Nth;
  friend class US_Hydrodyn_Dad_Svd;
  friend class US_Hydrodyn_Dad_Movie;
  friend class US_Hydrodyn_Dad_Baseline_Best;
  friend class US_Hydrodyn_Dad_Simulate;
  friend class US_Hydrodyn_Dad_Parameters;

public:
  US_Hydrodyn_Dad (csv csv1, void *us_hydrodyn, QWidget *p = 0);
  ~US_Hydrodyn_Dad ();

  void add_plot (QString name, vector<double> q, vector<double> I,
                 bool is_time = false, bool replot = true);

  void add_plot (QString name, vector<double> q, vector<double> I,
                 vector<double> errors, bool is_time = false,
                 bool replot = true);

  void add_plot_gaussian (const QString &file, const QString &tag);
...
private:
  DAD_Lambdas dad_lambdas;
  DAD_Angles dad_angles;
  bool dad_load (const QString &filename, const QStringList &qsl,
                 QString &errormsg);
  ...
  QPushButton *pb_dad_lambdas_save;
  QLabel *lbl_dad_lambdas_data;
...
private:
  ...
  void
  powerfit_add_marker (QwtPlot *plot, double pos, QColor color, QString text,
  ...
  vector<double> powerfit_curve (const vector<double> &x, double A, double B,
                                 double C);

  vector<double> curve_trim (const vector<double> x, double x_min,
                             double x_max);
...
private:
  ...
  void
  baseline2_add_marker (QwtPlot *plot, double pos, QColor color, QString text,
                        Qt::Alignment align = Qt::AlignRight | Qt::AlignTop);
...
private:
  ...
  void guinier_range ();
  void guinier_range (double minq2, double maxq2, double minI, double maxI);

  void
  guinier_add_marker (QwtPlot *plot, double pos, QColor color, QString text,
                      Qt::Alignment align = Qt::AlignRight | Qt::AlignTop);
  ...
  vector<dad_stack_data> stack_data;
  ...
  void add_smoothed (const QString &name, const vector<double> &q,
  ...
  void add_oldstyle (const QString &name, const vector<double> &q,
  ...
  vector<double> unified_ggaussian_params; // global centers first, then
                                           // height, width for each file
  ...
  vector<unsigned int>
      unified_ggaussian_q_start; // the start q pos - one for each curve
  vector<unsigned int>
      unified_ggaussian_q_end; // the end q pos   - one for each curve
  ...
  bool gg_fit_vector (vector<double> &fit,
                      vector<double> &pfit); // returns a vector of the chi^2
                                             // or rmsd and their q values &
  ...
  bool pvalue (const vector<double> &q, vector<double> &I, vector<double> &G,
               double &P,
               QString &errormsg); // compute pvalue comparing 2 curves
  bool pvalue (const QString &file, double &P,
               QString &errormsg); // compute pvalue comparing I with Gaussians

  map<QString, bool> all_files_map ();

  void update_plot_errors (vector<double> &grid, vector<double> &target,
                           vector<double> &fit, vector<double> &errors,
...
private:
  ...
  vector<double> conc_curve (vector<double> &t, unsigned int peak,
                             double conv);
  ...
  bool ggaussian_compatible (QStringList &files,
                             bool check_against_global = true);
  bool ggaussian_compatible (set<QString> &selected,
                             bool check_against_global = true);
  ...
  bool plot_file (QString file, double &minx, double &maxx, double &miny,
                  double &maxy);

  bool get_min_max (QString file, double &minx, double &maxx, double &miny,
                    double &maxy);
  ...
  bool create_i_of_q (QStringList files, double t_min = -1e99,
                      double t_max = 1e99);
  bool create_i_of_q (set<QString> &fileset, double t_min = -1e99,
                      double t_max = 1e99);
  bool create_i_of_q_ng (QStringList files, double t_min = -1e99,
                         double t_max = 1e99);
  bool create_i_of_q_ng (set<QString> &fileset, double t_min = -1e99,
                         double t_max = 1e99);
  bool create_ihash_t (QStringList files);
  bool create_istar_q (QStringList files, double t_min = -1e99,
                       double t_max = 1e99);
  bool create_istar_q_ng (QStringList files, double t_min = -1e99,
                          double t_max = 1e99);
  ...
  void gauss_add_marker (double pos, QColor color, QString text,
                         Qt::Alignment align = Qt::AlignRight | Qt::AlignTop);
  ...
  void baseline_apply (QStringList files, bool integral = false,
                       int smoothing = 0, bool save_bl = false,
                       unsigned int reps = 1, bool do_replot = true,
                       bool quiet = false);
  ...
  bool compute_f_gaussians (QString file, QWidget *dad_fit_widget);
  bool compute_f_gaussians_trial (QString file, QWidget *dad_fit_widget);
  ...
  dad_stack_data current_data (bool selected_only = false);
  dad_stack_data clipboard;
  void set_current_data (dad_stack_data &data);

  bool errors_were_on;

  void redo_plot_errors ();

  void stack_join (dad_stack_data &);
  ...
  vector<bool> is_common; // is common maps the offsets to layout of the
                          // regular file specific gaussians
  ...
  void check_discard_it_sd_mult (QStringList &files,
                                 bool optionally_discard = false);

  bool started_in_expert_mode;

  bool height_wheel_file (double &val,
                          double pos); // computes local height in gauss mode
  ...
  double wyatt_errors (QString file, double start_q, double end_q);

  double wyatt_errors (QString file, double start_q, double end_q,
                       vector<double> &new_q, vector<double> &new_I,
                       vector<double> &new_y);

  double wyatt_errors (const vector<double> &q, const vector<double> &I,
  ...
  int slice (const vector<double> &q, const vector<double> &I, double start_q,
             double end_q, vector<double> &new_q, vector<double> &new_I);
