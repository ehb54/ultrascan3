...
#define DAD_LAMBDA_EXTC_MSG \
   QString("<center>Selected lambda %1, extinction coefficient %2</br></center>") \
...
class DAD_Lambdas {
   public:
      ...
      bool load(const QString &filename, const QStringList &filelines, QString &errormsg);
...
class US_EXTERN US_Hydrodyn_Dad : public QFrame {
   ...
   public:
      ...
      void add_plot(QString name, vector<double> q, vector<double> I, bool is_time = false, bool replot = true);

      void add_plot(
         QString name, vector<double> q, vector<double> I, vector<double> errors, bool is_time = false,
         bool replot = true);
   ...
   private:
      ...
      void powerfit_add_marker(
         QwtPlot *plot, double pos, QColor color, QString text, Qt::Alignment align = Qt::AlignRight | Qt::AlignTop);
   ...
   private:
      ...
      void add_smoothed(const QString &name, const vector<double> &q, const vector<double> &I, int best_smoothing);
      ...
      void add_oldstyle(const QString &name, const vector<double> &q, const vector<double> &I);
      ...
      bool gg_fit_vector(
         vector<double> &fit,
         vector<double> &pfit); // returns a vector of the chi^2 or rmsd and their q values &
      ...
      void update_plot_errors(
         vector<double> &grid, vector<double> &target, vector<double> &fit, vector<double> &errors,
   ...
   private:
      ...
      bool create_i_of_q(QStringList files, double t_min = -1e99, double t_max = 1e99);
      bool create_i_of_q(set<QString> &fileset, double t_min = -1e99, double t_max = 1e99);
      bool create_i_of_q_ng(QStringList files, double t_min = -1e99, double t_max = 1e99);
      bool create_i_of_q_ng(set<QString> &fileset, double t_min = -1e99, double t_max = 1e99);
      bool create_ihash_t(QStringList files);
      bool create_istar_q(QStringList files, double t_min = -1e99, double t_max = 1e99);
      bool create_istar_q_ng(QStringList files, double t_min = -1e99, double t_max = 1e99);
      ...
      void gauss_add_marker(double pos, QColor color, QString text, Qt::Alignment align = Qt::AlignRight | Qt::AlignTop);
      ...
      void baseline_apply(
         QStringList files, bool integral = false, int smoothing = 0, bool save_bl = false, unsigned int reps = 1,
         bool do_replot = true, bool quiet = false);
      ...
      double wyatt_errors(
         QString file, double start_q, double end_q, vector<double> &new_q, vector<double> &new_I,
         vector<double> &new_y);

      double wyatt_errors(const vector<double> &q, const vector<double> &I, double start_q, double end_q);
      double wyatt_errors(
         const vector<double> &q, const vector<double> &I, double start_q, double end_q, vector<double> &new_q,
