...
class uhs_index_pair {
   public:
      int r;
      int c;
      bool operator<(const uhs_index_pair &objIn) const { return r < objIn.r || (r == objIn.r && c < objIn.c); }
      bool operator==(const uhs_index_pair &objIn) const { return r == objIn.r && c == objIn.c; }
};

class US_Hydrodyn_Saxs_Cormap_Cluster_Analysis {
   public:
      bool run(
         vector<vector<double>> &pvaluepairs, map<QString, QString> &parameters, map<QString, QString> &csv_report,
         QWidget *parent = 0);

      bool sliding(
         vector<vector<double>> &pvaluepairs, map<QString, QString> &parameters, map<QString, double> &sliding_results,
         QWidget *parent = 0, QProgressBar *progress = 0);

      bool sliding(
         vector<vector<double>> &pvaluepairs, map<QString, QString> &parameters, map<QString, double> &sliding_results,
         map<QString, double> &hb_sliding_results, QWidget *parent = 0, QProgressBar *progress = 0);
...
class US_EXTERN US_Hydrodyn_Saxs_Cormap : public QFrame {
   ...
   public:
      US_Hydrodyn_Saxs_Cormap(
         void *us_hydrodyn, map<QString, QString> parameters, vector<vector<double>> pvaluepairs,
         vector<vector<double>> adjpvaluepairs, vector<QString> selected_files, QWidget *p = 0, const char *name = 0);

      ~US_Hydrodyn_Saxs_Cormap();

      static bool load_csv(
         QString filename, map<QString, QString> &parameters, vector<vector<double>> &pvaluepairs,
         vector<vector<double>> &adjpvaluepairs, vector<QString> &selected_files, QWidget *parent = ( QWidget * ) 0);
   ...
   private:
      ...
      bool streak_check(vector<int> &row1, vector<int> &row2, int &N, int &S, int &C, double &P);
