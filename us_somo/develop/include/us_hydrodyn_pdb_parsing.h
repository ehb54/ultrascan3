...
struct pdb_parsing {
  ...
  bool alternate;  // true = Skip alternate conformations (default: true), else
                   // prompt for action
  ...
  int missing_residues;  // 0 = List them and stop operation
                         ...
                         // 2 = use automatic bead builder (approximate method,
                         // default)
  int missing_atoms;     // 0 = List them and stop operation (fix with WHATIF)
                         // 1 = List them, skip residue and proceed
                         // 2 = use approximate method to generate beads
                         // (approximate method, default)
};
...
class US_EXTERN US_Hydrodyn_PDB_Parsing : public QFrame {
 ...
 public:
  US_Hydrodyn_PDB_Parsing(struct pdb_parsing *, bool *, void *, QWidget *p = 0,
                          const char *name = 0);
