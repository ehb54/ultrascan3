#ifndef US_HYDRODYN_MALS_H
#define US_HYDRODYN_MALS_H
...
class US_EXTERN US_Hydrodyn_Mals : public QFrame {
  ...
  friend class US_Hydrodyn_Mals_Fit;
  friend class US_Hydrodyn_Mals_Fit_Global;
  friend class US_Hydrodyn_Mals_Options;
  friend class US_Hydrodyn_Mals_Gauss_Mode;
  friend class US_Hydrodyn_Mals_Nth;
 ...
 public:
  US_Hydrodyn_Mals(csv csv1, void *us_hydrodyn, QWidget *p = 0);
  ~US_Hydrodyn_Mals();
 ...
 private:
  ...
  bool mals_load(const QString &filename, const QStringList &qsl,
                 QString &errormsg);
 ...
 private:
  ...
  vector<mals_stack_data> stack_data;
 ...
 private:
  ...
  csv csv_conc;
  US_Hydrodyn_Mals_Conc *conc_window;
  bool conc_widget;
  ...
  bool compute_f_gaussians(QString file, QWidget *mals_fit_widget);
  bool compute_f_gaussians_trial(QString file, QWidget *mals_fit_widget);
  ...
  mals_stack_data current_data(bool selected_only = false);
  mals_stack_data clipboard;
  void set_current_data(mals_stack_data &data);

  bool errors_were_on;

  void redo_plot_errors();

  void stack_join(mals_stack_data &);
