...
class US_Container : public QObject {
 ...
 public:
  ...
  bool installed();
  bool test();
  // QStringList prepend( const QStringList & args ); // takes an argument list
  // and prepends proper type to run
  static QString unique_name(
      QString basename = "");  // convenience to name containers
  QString type();
