...
int us_hydrodyn_supc_main_hydro(
    bool bead_model_from_file, misc_options misc, hydro_results *hydro_results,
    hydro_options *hydro, double use_overlap_tolerance,
    vector<vector<PDB_atom> > *bead_models, vector<int> *somo_processed,
    ...
    US_Log *us_log, US_Udp_Msg *us_udp_msg, QString *accumulated_msgs);
...
class supc_thr_t : public QThread {
 public:
  supc_thr_t(int);
  void supc_thr_setup(unsigned int threads, int mode, float *p, float *rRis,
                      float *rRi, float *a, int N, int i, int nat, float *sum);
