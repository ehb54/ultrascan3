...
template <size_t Ndims> struct arr_index_data
{
  ...
  explicit arr_index_data (ssize_t i1) : actual_number_of_dimensions (1)
  {
    elems[0] = i1;
  }
  ...
  arr_index_data (ssize_t i1, ssize_t i2, ssize_t i3)
      : actual_number_of_dimensions (3)
  {
    elems[0] = i1;
    elems[1] = i2;
    elems[2] = i3;
  ...
  arr_index_data (ssize_t i1, ssize_t i2, ssize_t i3, ssize_t i4)
      : actual_number_of_dimensions (4)
  {
    elems[0] = i1;
    elems[1] = i2;
    elems[2] = i3;
  ...
  arr_index_data (ssize_t i1, ssize_t i2, ssize_t i3, ssize_t i4, ssize_t i5)
      : actual_number_of_dimensions (5)
  {
    elems[0] = i1;
    elems[1] = i2;
    elems[2] = i3;
...
template <size_t Ndims> struct arr_and_str_indices
{
  ...
  arr_and_str_indices (arr_index_data<Ndims> const &arr_ix_,
...
inline arr_and_str_indices<Ndims>
arr_index_data<Ndims>::operator() (int first, int last) const
{
...
inline bool
show_if_possible (std::ostream &ostream, int n_frames_skip = 0)
{
  ...
  ostream << "libc backtrace (" << size - n_frames_skip
          << " frames, most recent call last):" << std::endl;
                  ...
                  if (demangled != 0)
                    {
                      ...
                      char *b = static_cast<char *> (std::malloc (
                          static_cast<size_t> (n1 + n2 + n3 + 1)));
                      if (b != 0)
                        {
                          std::strncpy (b, s, n1);
...
inline std::string
file_and_line_as_string (const char *file, long line)
{
  std::ostringstream o;
  o << file << "(" << line << ")";
  return o.str ();
...
inline std::string
file_and_line_as_string_with_backtrace (const char *file, long line)
{
  std::ostringstream o;
...
template <size_t Ndims> struct dims_base_1
{
  size_t all[Ndims];

  size_t
  size_1d (size_t n_dims = Ndims) const
  {
    size_t result = 1;
  ...
  void
  set_dims (dims_base_1<BufferNdims> const &source)
  {
...
template <size_t Ndims> struct dim_data : dims_base_1<Ndims>
{
  ssize_t origin[Ndims];

  template <size_t BufferNdims>
  void
  set_dims (dim_data<BufferNdims> const &source)
  {
...
template <size_t Ndims> struct dims;

template <> struct dims<1> : dim_data<1>
{
  size_t
  index_1d (ssize_t i1) const
  {
    return i1 - this->origin[0];
  }
};

template <> struct dims<2> : dim_data<2>
{
  size_t
  index_1d (ssize_t i1, ssize_t i2) const
  {
    return (i2 - this->origin[1]) * this->all[0] + (i1 - this->origin[0]);
  }
};

template <> struct dims<3> : dim_data<3>
{
  size_t
  index_1d (ssize_t i1, ssize_t i2, ssize_t i3) const
  {
    return ((i3 - this->origin[2]) * this->all[1] + (i2 - this->origin[1]))
               * this->all[0]
           + (i1 - this->origin[0]);
  }
};

template <> struct dims<4> : dim_data<4>
{
  size_t
  index_1d (ssize_t i1, ssize_t i2, ssize_t i3, ssize_t i4) const
  {
    return (((i4 - this->origin[3]) * this->all[2] + (i3 - this->origin[2]))
                * this->all[1]
            + (i2 - this->origin[1]))
               * this->all[0]
           + (i1 - this->origin[0]);
...
template <> struct dims<5> : dim_data<5>
{
  size_t
  index_1d (ssize_t i1, ssize_t i2, ssize_t i3, ssize_t i4, ssize_t i5) const
  {
    return ((((i5 - this->origin[4]) * this->all[3] + (i4 - this->origin[3]))
                 * this->all[2]
             + (i3 - this->origin[2]))
                * this->all[1]
            + (i2 - this->origin[1]))
...
template <> struct dims<6> : dim_data<6>
{
  size_t
  index_1d (ssize_t i1, ssize_t i2, ssize_t i3, ssize_t i4, ssize_t i5,
            ssize_t i6) const
  {
    return (((((i6 - this->origin[5]) * this->all[4] + (i5 - this->origin[4]))
                  * this->all[3]
              + (i4 - this->origin[3]))
                 * this->all[2]
             + (i3 - this->origin[2]))
...
struct dim_buffer : dims<arr_dim_max>
{
  ...
  dim_buffer (dim_buffer const &other)
      : actual_number_of_dimensions (other.actual_number_of_dimensions)
  {
    ...
    std::copy (other.origin, other.origin + actual_number_of_dimensions,
               origin);
  }

  template <size_t BufferNdims>
  dim_buffer (dims<BufferNdims> const &source)
  ...
  template <unsigned I>
  void
  set_origin_all (ssize_t first, star_type const &)
  {
    actual_number_of_dimensions = (I + 1);
    all[I] = size_t_max - 1;
    origin[I] = first;
  ...
  template <unsigned I>
  void
  set_origin_all (ssize_t first, ssize_t last)
  {
    actual_number_of_dimensions = (I + 1);
    all[I] = last - first + 1;
    origin[I] = first;
  ...
  size_t
  actual_index_1d (arr_index_data<MaxNdims> const &arr_index) const
  {
    TBXX_ASSERT (arr_index.actual_number_of_dimensions
                 == actual_number_of_dimensions);
...
struct dim_chain : dim_buffer
{
  ...
  dim_chain (ssize_t first, star_type const &)
  {
    this->set_origin_all<0> (first, star_type ());
  }

  dim_chain (ssize_t first, ssize_t last)
  {
    this->set_origin_all<0> (first, last);
  }

  dim_chain &
  dim2 (star_type const &)
  {
  ...
  dim_chain &
  dim2 (ssize_t first, star_type const &)
  {
  ...
  dim_chain &
  dim2 (ssize_t first, ssize_t last)
  {
  ...
  dim_chain &
  dim3 (ssize_t first, star_type const &)
  {
  ...
  dim_chain &
  dim3 (ssize_t first, ssize_t last)
  {
  ...
  dim_chain &
  dim4 (ssize_t first, star_type const &)
  {
  ...
  dim_chain &
  dim4 (ssize_t first, ssize_t last)
  {
  ...
  dim_chain &
  dim5 (ssize_t first, star_type const &)
  {
  ...
  dim_chain &
  dim5 (ssize_t first, ssize_t last)
  {
  ...
  dim_chain &
  dim6 (ssize_t first, star_type const &)
  {
  ...
  dim_chain &
  dim6 (ssize_t first, ssize_t last)
  {
...
inline dim_chain
dim1 (ssize_t first, star_type const &)
{
  return dim_chain (first, star_type ());
}

inline dim_chain
dim1 (ssize_t first, ssize_t last)
{
  return dim_chain (first, last);
}

inline dims<1>
dimension (star_type const &)
{
  dims<1> result;
...
inline dims<2>
dimension (size_t n1, star_type const &)
{
  dims<2> result;
  result.all[0] = n1;
...
inline dims<3>
dimension (size_t n1, size_t n2, star_type const &)
{
  dims<3> result;
  result.all[0] = n1;
  result.all[1] = n2;
...
inline dims<3>
dimension (size_t n1, size_t n2, size_t n3)
{
  dims<3> result;
  result.all[0] = n1;
  result.all[1] = n2;
...
inline dims<4>
dimension (size_t n1, size_t n2, size_t n3, star_type const &)
{
  dims<4> result;
  result.all[0] = n1;
  result.all[1] = n2;
...
inline dims<4>
dimension (size_t n1, size_t n2, size_t n3, size_t n4)
{
  dims<4> result;
  result.all[0] = n1;
  result.all[1] = n2;
...
inline dims<5>
dimension (size_t n1, size_t n2, size_t n3, size_t n4, star_type const &)
{
  dims<5> result;
  result.all[0] = n1;
  result.all[1] = n2;
...
inline dims<5>
dimension (size_t n1, size_t n2, size_t n3, size_t n4, size_t n5)
{
  dims<5> result;
  result.all[0] = n1;
  result.all[1] = n2;
...
inline dims<6>
dimension (size_t n1, size_t n2, size_t n3, size_t n4, size_t n5, size_t n6)
{
  dims<6> result;
  result.all[0] = n1;
  result.all[1] = n2;
...
*/
template <typename T> struct zero_impl;
...
template <typename T, size_t Ndims = 1> struct arr_cref
{
protected:
  T const *elems_;
  dims<Ndims> dims_;
...
public:
  ...
  arr_cref (arr_cref<T, OtherNdims> const &other,
  ...
  template <size_t BufferNdims>
  void
  operator() (dims<BufferNdims> const &dims)
  {
    dims_.set_dims (dims);
  }
  ...
  size_1d () const
  {
    return dims_.size_1d ();
  }

  T const &
  operator() (ssize_t i1) const
  {
    return elems_[dims_.index_1d (i1)];
  }

  T const &
  operator() (ssize_t i1, ssize_t i2) const
  {
    return elems_[dims_.index_1d (i1, i2)];
  }

  T const &
  operator() (ssize_t i1, ssize_t i2, ssize_t i3) const
  {
    return elems_[dims_.index_1d (i1, i2, i3)];
  }

  T const &
  operator() (ssize_t i1, ssize_t i2, ssize_t i3, ssize_t i4) const
  {
    return elems_[dims_.index_1d (i1, i2, i3, i4)];
  }

  T const &
  operator() (ssize_t i1, ssize_t i2, ssize_t i3, ssize_t i4, ssize_t i5) const
  {
    return elems_[dims_.index_1d (i1, i2, i3, i4, i5)];
  }

  T const &
  operator() (ssize_t i1, ssize_t i2, ssize_t i3, ssize_t i4, ssize_t i5,
              ssize_t i6) const
  {
...
template <typename T, size_t Ndims = 1> struct arr_ref : arr_cref<T, Ndims>
{
protected:
  arr_ref () {}
...
public:
  ...
  template <size_t BufferNdims>
  arr_ref (T &val, dims<BufferNdims> const &dims)
      : arr_cref<T, Ndims> (val, dims)
  {
  }

  template <size_t BufferNdims>
  arr_ref (T &val, dims<BufferNdims> const &dims, no_fill0_type const &)
      : arr_cref<T, Ndims> (val, dims)
  {
  }

  template <size_t BufferNdims>
  arr_ref (T &val, dims<BufferNdims> const &dims, fill0_type const &)
      : arr_cref<T, Ndims> (val, dims)
  {
  ...
  begin () const
  {
    return const_cast<T *> (this->elems_);
  }
...
template <typename T, size_t Ndims = 1> struct arr : arr_ref<T, Ndims>
{
private:
  arr (arr const &);
...
public:
  template <size_t BufferNdims>
  explicit arr (dims<BufferNdims> const &dims)
...
template <typename T> struct hide
{
  T hidden;
};
...
template <typename T> struct slick_ptr
{
protected:
  mutable T *ptr;
...
public:
  ...
  slick_ptr &
  operator= (slick_ptr const &other)
  {
    reset (other.release ());
    return *this;
  }
  ...
  void
  swap (slick_ptr const &other) const
  {
    T *tmp = other.ptr;
    other.ptr = ptr;
    ptr = tmp;
...
inline bool
starts_with (char const *str, unsigned start, unsigned stop,
             char const *substr)
{
...
inline bool
ends_with_char (std::string const &str, int c)
{
  unsigned i = str.size ();
  if (i == 0)
    return false;
...
inline int
unsigned_integer_scan (char const *code, unsigned start, unsigned stop)
{
  unsigned i = start;
  for (; i < stop; i++)
    {
...
//! Assumes ASCII or similar.
inline unsigned
unsigned_integer_value (char const *str, unsigned start, unsigned stop)
{
  unsigned result = 0;
  unsigned i = start;
  for (; i < stop; i++)
...
inline unsigned
unsigned_integer_value (char const *str, unsigned stop)
{
  return unsigned_integer_value (str, 0, stop);
}

inline int
signed_integer_value (char const *str, unsigned start, unsigned stop)
{
  bool negative;
  if (str[start] == '-')
    {
...
inline void
copy_with_blank_padding (char const *src, size_t src_size, char *dest,
                         size_t dest_size)
{
...
inline void
copy_with_blank_padding (char const *src, char *dest, size_t dest_size)
{
  size_t i;
...
inline bool
string_eq (char const *lhs, size_t lhs_size, char const *rhs, size_t rhs_size)
{
...
inline bool
string_eq (char const *lhs, size_t lhs_size, char const *rhs)
{
  static const char blank = ' ';
...
inline int
string_compare_lexical (char const *lhs, size_t lhs_size, char const *rhs,
                        size_t rhs_size)
{
...
inline size_t
find_leading_blank_padding (char const *str, size_t stop)
{
  size_t i = 0;
  while (i != stop)
    {
...
inline size_t
find_trailing_blank_padding (char const *str, size_t stop)
{
  size_t i = stop;
  while (i != 0)
    {
...
inline size_t_2
find_leading_and_trailing_blank_padding (char const *str, size_t stop)
{
  return size_t_2 (find_leading_blank_padding (str, stop),
                   find_trailing_blank_padding (str, stop));
}

inline std::string
strip_leading_and_trailing_blank_padding (std::string const &str)
{
  size_t_2 indices
      = find_leading_and_trailing_blank_padding (str.data (), str.size ());
  if (indices.elems[0] == 0 && indices.elems[1] == str.size ())
    {
      return str;
    }
  return std::string (str.data () + indices.elems[0],
...
inline int
keyword_index (char const *valid_vals[], std::string const &val,
               char const *throw_info = 0)
{
  std::string val_norm
      = to_lower (strip_leading_and_trailing_blank_padding (val));
...
inline void
string_reverse_in_place (char *s, size_t s_size)
{
  if (s_size == 0)
    return;
  size_t i = 0;
...
//! Assumes ASCII or similar.
inline int
int_to_string (char *buffer, int buffer_size, int width, int value,
...
unsigned
split_comma_separated (VectorOfStringType &result, char const *c_str)
{
  for (unsigned i = 0;; i++)
    {
      char c = c_str[i];
...
public:
  ...
  str_cref (str_cref const &other, int len)
  ...
  bool
  operator== (str_cref const &rhs) const
  {
  ...
  operator< (str_cref const &rhs) const
  {
    return (utils::string_compare_lexical (elems (), len (), rhs.elems (),
                                           rhs.len ())
            < 0);
  }
...
bool inline
operator== (char const *lhs, str_cref const &rhs)
{
  return (rhs == lhs);
}
...
struct str_addends
{
  ...
  str_addends (str_cref const &lhs_, str_cref const &rhs_)
      : lhs (lhs_), rhs (rhs_)
  {
  }
...
inline str_addends
operator+ (str_cref const &lhs, str_cref const &rhs)
{
...
struct str_ref : str_cref
{
  ...
  {
    utils::copy_with_blank_padding (rhs.data (), rhs.size (), elems (),
                                    len ());
  }

  void
  operator= (str_cref const &rhs)
  {
    utils::copy_with_blank_padding (rhs.elems (), rhs.len (), elems (),
                                    len ());
  }

  void
  operator= (str_ref const &rhs)
  {
    (*this) = static_cast<str_cref> (rhs);
  }

  void
  operator= (str_addends const &addends)
  {
    ...
    if (n_from_rhs <= 0)
    ...
    else
      {
        ...
        utils::copy_with_blank_padding (addends.rhs.elems (),
                                        addends.rhs.len (), buffer.space + ll,
                                        n_from_rhs);
...
template <int StrLen> struct str
{
  ...
  str (str_cref const &other)
  {
    utils::copy_with_blank_padding (other.elems (), other.len (), elems,
                                    StrLen);
  }

  template <int OtherStrLen> str (str<OtherStrLen> const &other)
  {
  ...
  void
  operator= (str<OtherStrLen> const &rhs)
  {
...
template <int StrLen>
inline void
str_ref::operator= (str<StrLen> const &rhs)
{
  utils::copy_with_blank_padding (rhs.elems, StrLen, elems (), len ());
}

template <int StrLen> struct zero_impl<str<StrLen> >
{
  static str<StrLen>
  get ()
  {
...
template <size_t Ndims = 1> struct str_arr_cref
{
protected:
  char const *elems_;
  int len_;
...
public:
  ...
  template <size_t BufferNdims>
  str_arr_cref (char const *elems, int len, dims<BufferNdims> const &dims)
      : elems_ (elems), len_ (len)
  {
    (*this) (dims);
  }

  str_arr_cref (str_cref const &other)
      : elems_ (other.elems ()), len_ (other.len ())
  {
  }

  template <size_t BufferNdims>
  str_arr_cref (str_cref const &other, dims<BufferNdims> const &dims)
      : elems_ (other.elems ()), len_ (other.len ())
  {
    (*this) (dims);
  }

  str_arr_cref (str_cref const &other, int len)
      : elems_ (other.elems ()), len_ (len)
  {
  }
  ...
  str_arr_cref (str<StrLen> const &other, dims<BufferNdims> const &dims)
      : elems_ (other.elems), len_ (StrLen)
  {
    (*this) (dims);
  }

  template <int StrLen, size_t OtherNdims>
  str_arr_cref (arr_cref<str<StrLen>, OtherNdims> const &other)
  ...
  template <size_t OtherNdims, size_t BufferNdims>
  str_arr_cref (str_arr_cref<OtherNdims> const &other,
                dims<BufferNdims> const &dims)
  ...
  operator() (ssize_t i1, ssize_t i2, ssize_t i3, ssize_t i4, ssize_t i5) const
  {
    return str_cref (&elems_[len_ * dims_.index_1d (i1, i2, i3, i4, i5)],
                     len_);
  }
  ...
  operator() (ssize_t i1, ssize_t i2, ssize_t i3, ssize_t i4, ssize_t i5,
              ssize_t i6) const
  {
    return str_cref (&elems_[len_ * dims_.index_1d (i1, i2, i3, i4, i5, i6)],
                     len_);
  }
};

template <size_t Ndims = 1> struct str_arr_ref : str_arr_cref<Ndims>
{
protected:
  str_arr_ref () {}
...
public:
  ...
  template <size_t BufferNdims>
  str_arr_ref (char const *elems, int len, dims<BufferNdims> const &dims)
  ...
  template <size_t BufferNdims>
  str_arr_ref (str_ref const &other, dims<BufferNdims> const &dims)
      : str_arr_cref<Ndims> (other, dims)
  {
  }

  str_arr_ref (str_ref const &other, int len)
  ...
  template <int StrLen, size_t OtherNdims>
  str_arr_ref (arr_ref<str<StrLen>, OtherNdims> const &other)
      : str_arr_cref<Ndims> (other)
  {
  }

  template <int StrLen, size_t BufferNdims>
  str_arr_ref (str<StrLen> const &other, int len,
               dims<BufferNdims> const &dims)
  ...
  template <int StrLen, size_t BufferNdims>
  str_arr_ref (str<StrLen> const &other, dims<BufferNdims> const &dims)
      : str_arr_cref<Ndims> (other, dims)
  {
  }
  ...
  template <size_t OtherNdims, size_t BufferNdims>
  str_arr_ref (str_arr_ref<OtherNdims> const &other,
               dims<BufferNdims> const &dims)
      : str_arr_cref<Ndims> (other, dims)
  {
  }
  ...
  begin () const
  {
    return const_cast<char *> (this->elems_);
  }
  ...
  operator[] (size_t i) const
  {
    return str_ref (&this->begin ()[this->len () * i], this->len ());
  }

  str_ref
  operator() (ssize_t i1) const
  {
    return str_ref (&this->begin ()[this->len () * this->dims_.index_1d (i1)],
                    this->len ());
  }
  ...
  operator() (ssize_t i1, ssize_t i2, ssize_t i3, ssize_t i4, ssize_t i5) const
  {
    return str_ref (
        &this->begin ()[this->len ()
                        * this->dims_.index_1d (i1, i2, i3, i4, i5)],
        this->len ());
  }
              ...
              ssize_t i6) const
  {
    return str_ref (
        &this->begin ()[this->len ()
                        * this->dims_.index_1d (i1, i2, i3, i4, i5, i6)],
        this->len ());
  }
};
...
template <typename T> struct data_of_type : utils::noncopyable
{
  T const *values;
  size_t values_size;
  size_t value_index;
...
public:
  ...
  tokenizer (char const *fmt, unsigned fmt_stop)
  {
    ...
    if (stop == 0)
      {
        throw std::runtime_error (
            "Invalid FORMAT specification: empty string");
      }
    stop--;
    if (code[0] != '(')
      raise_invalid ();
...
protected:
  ...
  bool
  starts_with (char const *substr, unsigned start)
  {
    return utils::starts_with (code, start, stop, substr);
  }

  int
  unsigned_integer_scan (unsigned start)
  {
    return utils::unsigned_integer_scan (code, start, stop);
  }

  void
  add_token (char const *type, unsigned start)
  {
    tokens.push_back (
        utils::token (type, std::string (&code[start], &code[i])));
  }
        ...
        if (c == opening_quote)
          {
            ...
            if (i == stop || code[i] != opening_quote)
              {
                tokens.push_back (
                    utils::token ("string", std::string (str_buf, str_size)));
                return;
              }
          }
        str_buf[str_size++] = c;
...
struct repeat_point
{
  ...
  repeat_point (unsigned i_fmt_, unsigned n_,
...
struct token_loop
{
  ...
  utils::token const *
  next_executable_token (bool final = false)
  {
    if (simple_repeat != 0)
      {
        simple_repeat--;
        ...
        if (i_fmt == fmt_tokens.size ())
          {
            ...
            if (fmt_tokens.size () == 0)
              {
                throw io_err (
                    "Empty format string but data editing requested.");
              }
            i_fmt = i_fmt_wrap;
        ...
        if (t->type == "integer")
          {
            ...
            unsigned n
                = utils::unsigned_integer_value (tv.data (), tv.size ());
            if (n == 0)
              {
                throw std::runtime_error (
                ...
                if (tv0 == '(')
                ...
                else if (tv0 == ')')
                  {
                    if (repeat_points.size () == 0)
                      {
                        throw std::runtime_error (
                            "Unexpected closing parenthesis in format "
                            "string.");
                      }
...
inline V
sign (V const &value, S const &sign_source)
{
  if (sign_source < 0)
    {
      if (value > 0)
...
inline double
dsign (double const &value, double const &sign_source)
{
  return sign (value, sign_source);
}

inline int
isign (int const &value, int const &sign_source)
{
  return sign (value, sign_source);
}
...
inline double
dsqrt (double const &x)
{
  return std::sqrt (x);
}

inline float
cos (float const &angle)
{
  return std::cos (angle);
}

inline double
cos (double const &angle)
{
  return std::cos (angle);
}

inline float
acos (float const &angle)
{
  return std::acos (angle);
}

inline double
dacos (double const &angle)
{
  return std::acos (angle);
}

inline double
acos (double const &angle)
{
  return dacos (angle);
}

inline float
sin (float const &angle)
{
  return std::sin (angle);
}

inline double
sin (double const &angle)
{
  return std::sin (angle);
}

inline float
asin (float const &angle)
{
  return std::asin (angle);
}

inline double
dasin (double const &angle)
{
  return std::asin (angle);
}

inline double
asin (double const &angle)
{
  return dasin (angle);
}

inline float
tan (float const &angle)
{
  return std::tan (angle);
}

inline double
dtan (double const &angle)
{
  return std::tan (angle);
}

inline double
tan (double const &angle)
{
  return dtan (angle);
}

inline float
atan (float const &angle)
{
  return std::atan (angle);
}

inline double
datan (double const &angle)
{
  return std::atan (angle);
}

inline double
atan (double const &angle)
{
  return datan (angle);
}

inline float
atan2 (float const &y, float const &x)
{
  return std::atan2 (y, x);
}

inline double
datan2 (double const &y, double const &x)
{
  return std::atan2 (y, x);
}

inline double
atan2 (double const &y, double const &x)
{
  return datan2 (y, x);
}
...
inline float
alog10 (float const &x)
{
  return std::log10 (x);
}

inline double
alog10 (double const &x)
{
  return std::log10 (x);
}
...
inline int
mod (int const &v1, int const &v2)
{
  return v1 % v2;
}

inline float
amod (float const &v1, float const &v2)
{
  return std::fmod (v1, v2);
}

inline float
mod (float const &v1, float const &v2)
{
  return std::fmod (v1, v2);
}

inline double
dmod (double const &v1, double const &v2)
{
  return std::fmod (v1, v2);
}

inline double
mod (double const &v1, double const &v2)
{
  return std::fmod (v1, v2);
}
...
inline int
min0 (int const &v1, int const &v2)
{
  return std::min (v1, v2);
}

inline int
min (int const &v1, int const &v2)
{
  return std::min (v1, v2);
}

inline float
amin1 (float const &v1, float const &v2)
{
  return std::min (v1, v2);
}

inline float
amin1 (float const &v1, float const &v2, float const &v3)
{
  return amin1 (amin1 (v1, v2), v3);
}

inline float
amin1 (float const &v1, float const &v2, float const &v3, float const &v4)
{
...
inline float
min (float const &v1, float const &v2)
{
  return std::min (v1, v2);
}

inline double
dmin1 (double const &v1, double const &v2)
{
  return std::min (v1, v2);
}

inline double
min (double const &v1, double const &v2)
{
  return std::min (v1, v2);
}

inline float
amin0 (int const &v1, float const &v2)
{
  return std::min (static_cast<float> (v1), v2);
}

inline float
min (int const &v1, float const &v2)
{
  return std::min (static_cast<float> (v1), v2);
}

inline float
min1 (float const &v1, int const &v2)
{
  return std::min (v1, static_cast<float> (v2));
}

inline float
min (float const &v1, int const &v2)
{
  return std::min (v1, static_cast<float> (v2));
}

inline int
min (int const &v1, int const &v2, int const &v3)
{
  return min (v1, min (v2, v3));
}

inline int
min (int const &v1, int const &v2, int const &v3, int const &v4)
{
  return min (v1, min (v2, v3, v4));
}

inline float
min (float const &v1, float const &v2, float const &v3)
{
  return min (v1, min (v2, v3));
}

inline float
min (float const &v1, float const &v2, float const &v3, float const &v4)
{
  return min (v1, min (v2, v3, v4));
}

inline double
min (double const &v1, double const &v2, double const &v3)
{
  return min (v1, min (v2, v3));
}

inline double
min (double const &v1, double const &v2, double const &v3, double const &v4)
{
  return min (v1, min (v2, v3, v4));
}

inline int
max0 (int const &v1, int const &v2)
{
  return std::max (v1, v2);
}

inline int
max (int const &v1, int const &v2)
{
  return std::max (v1, v2);
}

inline float
amax1 (float const &v1, float const &v2)
{
  return std::max (v1, v2);
}

inline float
amax1 (float const &v1, float const &v2, float const &v3)
{
  return amax1 (amax1 (v1, v2), v3);
}

inline float
amax1 (float const &v1, float const &v2, float const &v3, float const &v4)
{
...
inline float
max (float const &v1, float const &v2, float const &v3, float const &v4)
{
  return amax1 (v1, v2, v3, v4);
}

inline float
max (float const &v1, float const &v2)
{
  return std::max (v1, v2);
}

inline double
dmax1 (double const &v1, double const &v2)
{
  return std::max (v1, v2);
}

inline double
max (double const &v1, double const &v2)
{
  return std::max (v1, v2);
}

inline float
amax0 (int const &v1, float const &v2)
{
  return std::max (static_cast<float> (v1), v2);
}

inline float
max (int const &v1, float const &v2)
{
  return std::max (static_cast<float> (v1), v2);
}

inline float
max1 (float const &v1, int const &v2)
{
  return std::max (v1, static_cast<float> (v2));
}

inline float
max (float const &v1, int const &v2)
{
  return std::max (v1, static_cast<float> (v2));
}

inline double
max (float const &v1, double const &v2)
{
  return std::max (static_cast<double> (v1), v2);
}

inline int
max (int const &v1, int const &v2, int const &v3)
{
  return max (max (v1, v2), v3);
}

inline float
max (float const &v1, float const &v2, float const &v3)
{
  return max (max (v1, v2), v3);
}

inline double
max (double const &v1, double const &v2, double const &v3)
{
  return max (max (v1, v2), v3);
}
...
inline int
pow (int const &base, int const &exponent)
{
  if (exponent < 0)
    return 0;
  int result = 1;
...
inline float
pow (int const &base, float const &exponent)
{
  return std::pow (static_cast<float> (base), exponent);
}

inline double
pow (int const &base, double const &exponent)
{
  return std::pow (static_cast<double> (base), exponent);
}

inline float
pow (float const &base, int const &exponent)
{
  return std::pow (base, exponent);
}

inline double
pow (double const &base, int const &exponent)
{
  return std::pow (base, exponent);
}

inline float
pow (float const &base, float const &exponent)
{
  return std::pow (base, exponent);
}

inline double
pow (double const &base, float const &exponent)
{
  return std::pow (base, static_cast<double> (exponent));
}

inline double
pow (float const &base, double const &exponent)
{
  return std::pow (static_cast<double> (base), exponent);
}

inline double
pow (double const &base, double const &exponent)
{
  return std::pow (base, exponent);
}
...
inline int
index (str_cref sentence, str_cref word)
{
...
inline int
nint (float const &value)
{
  return static_cast<int> (std::floor (value + 0.5));
}

inline int
idnint (float const &value)
{
  return static_cast<int> (std::floor (value + 0.5));
}

inline int
nint (double const &value)
{
  return static_cast<int> (std::floor (value + 0.5));
}

inline std::complex<float>
cmplx (float const &re, float const &im)
{
  return std::complex<float> (re, im);
}

inline std::complex<double>
dcmplx (double const &re, double const &im)
{
  return std::complex<double> (re, im);
}

inline std::complex<double>
cmplx (double const &re, double const &im)
{
  return dcmplx (re, im);
}
...
inline void
getenv (str_cref key, str_ref result)
{
...
date (str_ref result)
{
  ...
  str<10> buf;
  std::sprintf (buf.elems, "%02d-%s-%02d", tm->tm_mday, months[tm->tm_mon],
                tm->tm_year % 100);
  result = buf;
}
...
time (str_ref result)
{
  ...
  std::tm const *tm (std::localtime (&now));
  str<10> buf;
  std::sprintf (buf.elems, "%02d:%02d:%02d", tm->tm_hour, tm->tm_min,
                tm->tm_sec);
  result = buf;
}
...
public:
  stop_info (const char *source_file_, size_t source_line_,
             ssize_t digits_) throw ()
      : source_file (source_file_), source_line (source_line_),
        digits (digits_)
  {
  }
  ...
  stop_info (const char *source_file_, size_t source_line_,
      ...
      : source_file (source_file_), source_line (source_line_), digits (-1),
        message (message_)
  {
  }
...
#undef FABLE_LOC
  if (!result)
    {
      std::fprintf (stderr, "NOTE: fem/utils/int_types.hpp"
...
inline int
main_with_catch (int argc, char const *argv[],
...
static const std::string
    dynamic_parameters_option ("--fem-dynamic-parameters");
...
struct dynamic_parameters_from
{
  ...
  dynamic_parameters_from (command_line_arguments const &command_line_args_,
      ...
      : command_line_args (command_line_args_), i_fld (0)
  {
    ...
    if (n_flds > max_number_of_flds)
      {
        std::ostringstream o;
        o << "Too many " << dynamic_parameters_option
          << " fields (given: " << n_flds
  ...
  reset_if_given (T &value)
  {
    int n_flds = static_cast<int> (
        command_line_args.dynamic_parameters_fields.size ());
    if (i_fld < n_flds)
      {
        std::string const &fld
            = command_line_args.dynamic_parameters_fields[i_fld];
...
public:
  ...
  template <size_t BufferNdims>
  explicit arr_size (dims<BufferNdims> const &dims)
  ...
  template <size_t BufferNdims>
  arr_size (dims<BufferNdims> const &dims, fill0_type const &)
...
template <size_t Size, typename T> struct arr_1d : arr_size<Size, T, 1>
{
  ...
  template <size_t BufferNdims>
  arr_1d (dims<BufferNdims> const &dims, no_fill0_type const &)
      : base_t (dims, no_fill0)
  {
  }

  template <size_t BufferNdims>
  arr_1d (dims<BufferNdims> const &dims, fill0_type const &)
      : base_t (dims, fill0)
  {
  }
};
...
struct arr_2d : arr_size<Size1 * Size2, T, 2>
{
  ...
  explicit arr_2d (no_fill0_type const &)
      : base_t (dimension (Size1, Size2), no_fill0)
  {
  }

  explicit arr_2d (fill0_type const &)
  ...
  template <size_t BufferNdims>
  arr_2d (dims<BufferNdims> const &dims, no_fill0_type const &)
      : base_t (dims, no_fill0)
  {
  }

  template <size_t BufferNdims>
  arr_2d (dims<BufferNdims> const &dims, fill0_type const &)
      : base_t (dims, fill0)
  {
  }
};
...
struct arr_3d : arr_size<Size1 * Size2 * Size3, T, 3>
{
  ...
  explicit arr_3d (no_fill0_type const &)
      : base_t (dimension (Size1, Size2, Size3), no_fill0)
  {
  }

  explicit arr_3d (fill0_type const &)
  ...
  template <size_t BufferNdims>
  arr_3d (dims<BufferNdims> const &dims, no_fill0_type const &)
      : base_t (dims, no_fill0)
  {
  }

  template <size_t BufferNdims>
  arr_3d (dims<BufferNdims> const &dims, fill0_type const &)
      : base_t (dims, fill0)
  {
  }
};
...
struct datum
{
  ...
  datum (double const &value)
      : content (new holder<double> (value)), repeats (1)
  {
  }

  template <typename T>
  datum (T const &value)
      : content (new holder<std::string> (std::string (value))), repeats (1)
  {
  }
  ...
  datum const &
  operator= (datum const & /* rhs */)
  {
    throw TBXX_UNREACHABLE_ERROR ();
  }

  void
  set_repeats (unsigned value) const
  {
    repeats = value;
  }

  void
  throw_type_mismatch (char const *target_type) const
  {
    throw std::runtime_error (std::string ("DATA type mismatch: target type ")
                              + target_type + " vs. "
                              + content->type ().fileName () + " value");
  }
};

inline datum const &
operator* (unsigned repeats, datum const &value)
{
...
struct data_values
{
  ...
  data_values (data_buffer const &values_)
      : values (values_), value_index (0), repeat_index (0)
  {
  }
                ...
                str_ref val)
  {
    ...
    if (tab_val.content->type () == typeid (std::string))
      {
        std::string const &tab_str
            = static_cast<datum::holder<std::string> *> (tab_val.content)
                  ->held;
        val = tab_str.c_str ();
      }
    else
...
inline bool
is_absolute (char const *path, bool drive_split_already = false)
{
...
inline bool
truncate_file_at_current_position (std::FILE *fp)
{
...
inline std::string
random_name_simple (size_t size)
{
...
struct simple_ostream_to_char_ptr_and_size : simple_ostream
{
  ...
  simple_ostream_to_char_ptr_and_size (char *string, size_t size)
      : s (string), sz (size), i (0)
  {
  }
...
struct simple_istream_from_char_ptr_and_size : simple_istream
{
  ...
  simple_istream_from_char_ptr_and_size (char const *string, size_t size)
      : s (string), sz (size), i (0)
  {
  }
...
struct simple_istream_from_std_string : simple_istream
{
  ...
  simple_istream_from_std_string (std::string const &string)
      : s (string), i (0)
  {
  }
...
struct io_unit
{
  ...
  */
  ...
  io_unit (int number_, std::string file_name_ = std::string (""),
           std_file stream_ = std_file (0))
      : number (number_), file_name (file_name_), stream (stream_),
        prev_op_was_write (false), access (ac_undef), form (fm_undef),
        recl (0), blank (bl_undef), status (st_undef), iostat (0)
  {
  }
        ...
        if (status != st_scratch)
          {
            char buffer[64];
            int actual_width
                = utils::int_to_string (buffer, sizeof (buffer),
                                        /*width*/ 3,
                                        /*value*/ number,
        ...
        else
          {
            ...
            while (true)
              {
                file_name
                    = "fem_io_unit_scratch_" + utils::random_name_simple (8);
  ...
  void
  close (int *iostat_ptr = 0, bool status_delete = false)
  {
    if (iostat_ptr != 0)
      *iostat_ptr = 0; // XXX
  ...
  void
  backspace (int * /* iostat_ptr */)
  {
    throw TBXX_NOT_IMPLEMENTED ();
  }

  void
  endfile (int * /* iostat_ptr */)
  {
    if (is_std_io_unit (number))
      {
        throw TBXX_NOT_IMPLEMENTED ();
...
struct io : utils::noncopyable
{
  ...
  io_unit *
  unit_ptr (int unit, bool auto_open = false)
  {
    typedef std::map<int, io_unit>::iterator it;
  ...
  inline struct inquire_chain
  inquire_file (std::string file, bool blank_padding_removed_already = false);

  inline struct open_chain open (int unit, std::string file = std::string (),
  ...
  inline bool
  is_opened_simple (std::string const &file_name) const
  {
        ...
        if (!is_std_io_unit (unit))
          {
            if (!utils::path::truncate_file_at_current_position (sf.ptr))
              {
                throw io_err ("Cannot truncate file for writing: "
                              + u_ptr->file_name);
              }
          }
        u_ptr->prev_op_was_write = true;
      }
    return utils::slick_ptr<utils::simple_ostream> (
        new utils::simple_ostream_to_c_file (sf.ptr));
  }

  utils::slick_ptr<utils::simple_istream>
  simple_istream (int unit)
  {
    io_unit *u_ptr = unit_ptr (unit, /*auto_open*/ true);
    u_ptr->prev_op_was_write = false;
    return utils::slick_ptr<utils::simple_istream> (
        new utils::simple_istream_from_c_file (u_ptr->stream.ptr));
  }
...
public:
  close_chain (io *io_ptr_, int unit_)
      : io_ptr (io_ptr_), unit (unit_), iostat_ptr (0), status_delete (false)
  {
  }

  close_chain (close_chain const &other)
      : io_ptr (other.io_ptr), unit (other.unit),
        iostat_ptr (other.iostat_ptr), status_delete (other.status_delete)
  {
    other.io_ptr = 0;
  }
...
private:
  file_positioning_chain const &
  operator= (file_positioning_chain const &); // not implemented
...
public:
  ...
  file_positioning_chain (file_positioning_chain const &other)
      : io_function (other.io_function), u_ptr (other.u_ptr),
...
inline file_positioning_chain
io::backspace (int unit)
{
  return file_positioning_chain ("backspace",
                                 unit_ptr (unit, /*auto_open*/ true));
}

inline file_positioning_chain
io::endfile (int unit)
{
  return file_positioning_chain ("endfile",
                                 unit_ptr (unit, /*auto_open*/ true));
}
...
io::rewind (int unit)
{
  return file_positioning_chain ("rewind",
                                 unit_ptr (unit, /*auto_open*/ true));
}
...
public:
  inquire_chain (io *io_ptr_, int unit_)
      : io_ptr (io_ptr_), unit (unit_), have_file (false)
  {
  }

  inquire_chain (io *io_ptr_, std::string const &file_)
      : io_ptr (io_ptr_), unit (0),
  ...
  inquire_chain (inquire_chain const &other)
      : io_ptr (other.io_ptr), unit (other.unit), file (other.file),
...
inline inquire_chain
io::inquire_unit (int unit)
{
  return inquire_chain (this, unit);
}

inline inquire_chain
io::inquire_file (std::string file, bool blank_padding_removed_already)
{
...
public:
  ...
  open_chain (open_chain const &other)
      : u_ptr (other.u_ptr), iostat_ptr (other.iostat_ptr)
  {
    other.u_ptr = 0;
  }
...
inline open_chain
io::open (int unit, std::string file, bool blank_padding_removed_already)
{
  if (!blank_padding_removed_already)
    {
      file = utils::strip_leading_and_trailing_blank_padding (file);
...
struct common
{
  ...
  void
  getarg (int pos, str_ref value) const
  {
...
public:
  ...
  void
  construct (D const &dynamic_parameters)
  {
...
template <typename T>
void
no_operation_to_avoid_unused_variable_warning (const T &)
{
}
...
public:
  ...
  explicit optional_copy (ValueType const &value)
...
public:
  ...
  optional_container (optional_container const &other)
      : optional_copy<ContainerType> (other)
  {
  }

  optional_container &
  operator= (optional_container const &other)
  {
    delete this->ptr_;
  ...
  explicit optional_container (ContainerType const &container)
      : optional_copy<ContainerType> (container)
  {
  }

  optional_container &
  operator= (ContainerType const &container)
  {
    delete this->ptr_;
...
struct string_to_double
{
  ...
  string_to_double (simple_istream &inp, unsigned decimal_point_substitute = 0,
  ...
  void
  convert (simple_istream &inp,
           unsigned decimal_point_substitute
           = 0,                          // "d" part of Fortran "w.d" format
...
struct string_to_double_fmt : string_to_double
{
  ...
  string_to_double_fmt (simple_istream &inp, unsigned w, unsigned d,
                        bool blanks_zero, int exp_scale)
  {
    reset ();
    if (w == 0)
      return;
...
public:
  read_loop (common &cmn, int const &unit, unformatted_type const &)
      : inp (cmn.io.simple_istream (unit)), first_inp_get (true),
        blanks_zero (false), exp_scale (0), io_mode (io_unformatted),
        iostat_ptr (0)
  {
  }
  ...
  read_loop (common &cmn, int const &unit, star_type const &)
      : inp (cmn.io.simple_istream (unit)), first_inp_get (true),
        blanks_zero (false), exp_scale (0), io_mode (io_list_directed),
        iostat_ptr (0)
  {
  }
  ...
  read_loop (common &cmn, int const &unit, str_cref fmt)
      : inp (cmn.io.simple_istream (unit)), first_inp_get (true),
        fmt_loop (fmt), blanks_zero (false), exp_scale (0),
  ...
  read_loop (str_cref const &internal_file, star_type const &)
      : inp (utils::slick_ptr<utils::simple_istream> (
            new utils::simple_istream_from_char_ptr_and_size (
                internal_file.elems (), internal_file.len ()))),
        first_inp_get (true), blanks_zero (false), exp_scale (0),
  ...
  read_loop (str_cref const &internal_file, str_cref fmt)
      : inp (utils::slick_ptr<utils::simple_istream> (
            new utils::simple_istream_from_char_ptr_and_size (
                internal_file.elems (), internal_file.len ()))),
        first_inp_get (true), fmt_loop (fmt), blanks_zero (false),
        exp_scale (0), io_mode (io_formatted), iostat_ptr (0)
  {
  }
            ...
            if (utils::ends_with_char (tv, 'x'))
            ...
            else if (utils::ends_with_char (tv, 'p'))
              {
                if (tv.size () == 1)
                  ...
                  {
                    exp_scale = utils::signed_integer_value (tv.data (), 0,
                                                             tv.size () - 1);
                  }
              }
            else if (tv[0] == 't')
                      ...
                      bool& val)
  {
    if (io_mode == io_unformatted)
      {
        from_stream_unformatted (reinterpret_cast<char *> (&val),
                                 sizeof (bool));
      }
                      ...
                      float& val)
  {
    if (io_mode == io_unformatted)
      ...
      {
        val = static_cast<float> ((io_mode == io_formatted
                                       ? read_fmt_double ()
                                       : read_star_double ()));
      }
    return *this;
                      ...
                      double& val)
  {
    if (io_mode == io_unformatted)
      ...
      {
        val = (io_mode == io_formatted ? read_fmt_double ()
                                       : read_star_double ());
      }
    return *this;
  }
                      ...
                      std::complex<float>& val)
  {
    if (io_mode == io_unformatted)
      {
        float re, im;
        from_stream_unformatted (reinterpret_cast<char *> (&re),
                                 sizeof (float));
        from_stream_unformatted (reinterpret_cast<char *> (&im),
                                 sizeof (float));
  ...
  {
    while (true)
      { // loop scanning for first non-whitespace
        int c = inp_get ();
        if (utils::is_stream_end (c))
          {
            inp.reset ();
        ...
        if (!utils::is_whitespace (c))
          {
            ...
            while (true)
              { // loop collecting digits
                ...
                result += utils::digit_as_int (c);
                c = inp_get ();
                if (utils::is_stream_end (c) || utils::is_whitespace (c)
                    || c == ',')
                  {
                    if (negative)
                      result *= -1;
  ...
  void
  throw_if_conv_error_message (utils::string_to_double const &conv)
  {
  ...
  read_star_double ()
  {
    ...
    if (this->iostat_ptr != 0)
      *iostat_ptr = IOSTAT_ERROR;
    throw io_err ("Invalid character while reading floating-point value: "
                  + utils::format_char_for_display (c));
  }

  void
  from_stream_unformatted (char *target, unsigned target_size)
  {
...
struct read_from_string : public read_loop
{
  read_from_string (std::string const &internal_file, str_cref fmt)
      : read_loop (str_cref (internal_file.data (),
...
struct common_read
{
  ...
  read_loop
  operator() (int unit, unformatted_type const &)
  {
  ...
  read_loop
  operator() (int unit, star_type const &)
  {
  ...
  read_loop
  operator() (int const &unit, str_cref fmt)
  {
  ...
  read_loop
  operator() (str_cref const &internal_file, star_type const &)
  {
  ...
  read_loop
  operator() (str_cref const &internal_file, str_cref fmt)
  {
...
struct array_alignment
{
  ...
  msg_prefix ()
  {
    return "equivalence::array_alignment: ";
  }

  void
  add_anchor (size_t i0, ssize_t a0, size_t i1, ssize_t a1)
  {
        ...
        for (size_t i1 = i0 + 1; i1 < n; i1++)
          {
            ...
            if (d != ssize_t_max && diffs0[i1] - diffs0[i0] != d)
              {
                throw std::runtime_error (msg_prefix ()
                                          + "indirectly conflicting input");
              }
          }
      }
  }
...
struct variant_member
{
  ...
  variant_member (size_t type_size_, DimsType const &dims_)
      : type_size (type_size_), dims (dims_)
  {
  }
  ...
  size_t
  actual_index_1d (arr_index const &arr_ix)
  {
...
template <typename T> struct mbr : variant_member
{
...
template <int StrLen> struct mbr<str<StrLen> > : variant_member
{
...
struct equivalence
{
  ...
  equivalence (variant_member const &mbr_1, variant_member const &mbr_2)
      : members_size (2), array_alignment (members_size),
        align_i_mbr (size_t_max), align_byte_offset (size_t_max)
  {
    members[0] = mbr_1;
    members[1] = mbr_2;
  }
  ...
  equivalence (variant_member const &mbr_1, variant_member const &mbr_2,
      ...
      : members_size (3), array_alignment (members_size),
        align_i_mbr (size_t_max), align_byte_offset (size_t_max)
  {
    members[0] = mbr_1;
    members[1] = mbr_2;
  ...
  equivalence (variant_member const &mbr_1, variant_member const &mbr_2,
               variant_member const &mbr_3, variant_member const &mbr_4)
      : members_size (4), array_alignment (members_size),
        align_i_mbr (size_t_max), align_byte_offset (size_t_max)
  {
    members[0] = mbr_1;
    members[1] = mbr_2;
  ...
  equivalence (variant_member const &mbr_1, variant_member const &mbr_2,
      ...
      : members_size (5), array_alignment (members_size),
        align_i_mbr (size_t_max), align_byte_offset (size_t_max)
  {
    members[0] = mbr_1;
    members[1] = mbr_2;
  ...
  equivalence (variant_member const &mbr_1, variant_member const &mbr_2,
               variant_member const &mbr_3, variant_member const &mbr_4,
               variant_member const &mbr_5, variant_member const &mbr_6)
      : members_size (6), array_alignment (members_size),
        align_i_mbr (size_t_max), align_byte_offset (size_t_max)
  {
    members[0] = mbr_1;
    members[1] = mbr_2;
  ...
  equivalence (variant_member const &mbr_1, variant_member const &mbr_2,
      ...
      : members_size (7), array_alignment (members_size),
        align_i_mbr (size_t_max), align_byte_offset (size_t_max)
  {
    members[0] = mbr_1;
    members[1] = mbr_2;
  ...
  equivalence &
  align (size_t index_1d = 0, size_t str_offset = 0)
  {
    align_i_mbr = Jmbr - 1;
  ...
  template <size_t Jmbr>
  equivalence &
  align (arr_and_str_indices<arr_dim_max> const &indices)
  {
    align<Jmbr> (members[Jmbr - 1].actual_index_1d (indices.arr_ix),
  ...
  template <size_t Jmbr>
  equivalence &
  with (size_t index_1d = 0, size_t str_offset = 0)
  {
    array_alignment.add_anchor (align_i_mbr, align_byte_offset, Jmbr - 1,
                                members[Jmbr - 1].type_size * index_1d
                                    + str_offset);
    return *this;
  }
  ...
  template <size_t Jmbr>
  equivalence &
  with (arr_and_str_indices<arr_dim_max> const &indices)
  {
    with<Jmbr> (members[Jmbr - 1].actual_index_1d (indices.arr_ix),
                static_cast<size_t> (indices.str_ix.first - 1));
    return *this;
  }
...
struct variant_core : utils::noncopyable
{
  ...
  void
  grow_if_necessary (size_t new_size)
  {
    if (size < new_size)
      {
...
struct variant_allocate_chain
{
  ...
  variant_allocate_chain (variant_core *core_, variant_bindings *bindings_,
                          bool is_common_variant_)
      : core (core_), bindings (bindings_),
        is_common_variant (is_common_variant_), curr_bytes (0), end_bytes (0)
  {
  }
        ...
        for (size_t i_mbr = 0; i_mbr < e.members_size; i_mbr++)
          {
            ...
            if (diff0 < 0)
              {
                origin_bytes
                    = std::max (origin_bytes, static_cast<size_t> (-diff0));
              }
          }
      }
    for (size_t i_mbr = 0; i_mbr < e.members_size; i_mbr++)
...
struct variant_bind_chain
{
  ...
  variant_bind_chain (variant_core *core_, variant_bindings *bindings_,
                      bool is_common_variant_)
      : core (core_), bindings (bindings_),
        is_common_variant (is_common_variant_), bind_index (0)
  {
    core->use_count++;
  }
...
struct save_equivalences : variant_bind_chain
{
  save_equivalences (variant_core_and_bindings &core_and_bindings)
      : variant_bind_chain (&core_and_bindings.core,
...
struct local_equivalences : utils::hide<variant_core_and_bindings>,
                            variant_bind_chain
{
  local_equivalences ()
      : variant_bind_chain (&hidden.core, &hidden.bindings,
                            /*is_common_variant*/ false)
  {
  }
};
...
struct double_to_string_scientific_notation
{
      ...
      char e_or_d)
  {
    ...
    static const pow_tab_entry pow_tab[] = {
      { 256, 1e256, 1e-256 }, { 128, 1e128, 1e-128 }, { 64, 1e064, 1e-064 },
      { 32, 1e032, 1e-032 },  { 16, 1e016, 1e-016 },  { 8, 1e008, 1e-008 },
      { 4, 1e004, 1e-004 },   { 2, 1e002, 1e-002 },   { 1, 1e001, 1e-001 }
    };
...
struct write_loop_base
{
  ...
  write_loop_base (bool write_crlf_ = false)
      : write_crlf (write_crlf_), pos (0), prev_was_string (false),
        exp_scale (0), number_of_x_held (0), suppress_new_line_at_end (false)
  {
  }
};
...
public:
  write_loop (common &cmn, int const &unit, unformatted_type const &)
      : write_loop_base (FEM_WRITE_CRLF && !is_std_io_unit (unit)),
        out (cmn.io.simple_ostream (unit)), internal_file_len (-1),
        io_mode (io_unformatted), terminated_by_colon (false)
  {
  }
  ...
  write_loop (common &cmn, int const &unit, star_type const &)
      : write_loop_base (FEM_WRITE_CRLF && !is_std_io_unit (unit)),
        out (cmn.io.simple_ostream (unit)), internal_file_len (-1),
        io_mode (io_list_directed), terminated_by_colon (false)
  {
  }
  ...
  write_loop (common &cmn, int const &unit, str_cref fmt)
      : write_loop_base (FEM_WRITE_CRLF && !is_std_io_unit (unit)),
        out (cmn.io.simple_ostream (unit)), internal_file_len (-1),
        io_mode (io_formatted), fmt_loop (fmt), terminated_by_colon (false)
  {
  }

  write_loop (str_ref const &internal_file, star_type const &)
      : out (utils::slick_ptr<utils::simple_ostream> (
            new utils::simple_ostream_to_char_ptr_and_size (
                internal_file.elems (), internal_file.len ()))),
        internal_file_len (internal_file.len ()), io_mode (io_list_directed),
  ...
  write_loop (str_ref const &internal_file, str_cref fmt)
      : out (utils::slick_ptr<utils::simple_ostream> (
            new utils::simple_ostream_to_char_ptr_and_size (
                internal_file.elems (), internal_file.len ()))),
        internal_file_len (internal_file.len ()), io_mode (io_formatted),
        fmt_loop (fmt), terminated_by_colon (false)
  {
  }
  ...
  std::string const &
  next_edit_descriptor (bool final = false)
  {
    while (true)
      {
                      ...
                      bool const& val)
  {
    if (io_mode == io_unformatted)
      {
        to_stream_unformatted (reinterpret_cast<char const *> (&val),
                               sizeof (bool));
      }
    else if (io_mode == io_list_directed)
      {
...
protected: // implementation detail
  void
  to_stream_fmt_double (double const &val)
  {
    std::string const &ed = next_edit_descriptor ();
    if (ed[0] == 'f')
      {
    ...
    if (io_mode == io_unformatted)
      {
        float re = val.real ();
        float im = val.imag ();
        to_stream_unformatted (reinterpret_cast<char const *> (&re),
                               sizeof (float));
        to_stream_unformatted (reinterpret_cast<char const *> (&im),
                               sizeof (float));
      }
    else if (io_mode == io_list_directed)
      {
        ...
        to_stream_star_complex (conv_re.begin, conv_re.n, conv_im.begin,
                                conv_im.n);
      }
    else
      {
...
private:
  void
  to_stream (char const *buf, unsigned n, bool space = true)
  {
    switch (io_mode)
      {
      case io_unformatted:
  ...
  void
  to_stream_unformatted (char const *buf, unsigned n)
  {
    for (unsigned i = 0; i < n; i++)
      {
        char c = buf[i];
  ...
  void
  to_stream_fmt (char const *buf, unsigned n)
  {
  ...
  void
  to_stream_star (char const *buf, unsigned n, bool space = true)
  {
    if (pos == 0)
      {
        out->put (' ');
      ...
      char const *buf, unsigned n, bool /* space=true */)
  {
    ...
    to_stream_star (buf + indices.elems[0],
                    indices.elems[1] - indices.elems[0],
                    /*space*/ false);
  }

  void
  to_stream_star_complex (char const *buf_re, unsigned n_re,
...
struct common_write
{
  ...
  write_loop
  operator() (str_ref const &internal_file, star_type const &)
  {
  ...
  write_loop
  operator() (str_ref const &internal_file, str_cref fmt)
  {
  ...
  template <size_t Ndims>
  write_loop
  operator() (str_arr_ref<Ndims> const &internal_file, star_type const &)
  {
    write_loop result (internal_file, star);
    return result;
  }
  ...
  template <size_t Ndims>
  write_loop
  operator() (str_arr_ref<Ndims> const &internal_file, str_cref fmt)
  {
    write_loop result (internal_file, fmt);
    return result;
  }
