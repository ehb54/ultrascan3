...
public:
  ...
  EIGEN_DEVICE_FUNC inline CoeffReturnType
  x () const
  {
   ...
   * lvalue) */
  EIGEN_DEVICE_FUNC inline NonConstCoeffReturnType
  x ()
  {
  ...
  // errors when compiling test_stdvector with GCC 4.4.2. This looks like a GCC
  // bug though, so feel free to re-enable it if it's useful; however notice
  // that we already have the templated operator= above and e.g. in MatrixBase
   ...
   * rather use a fuzzy comparison such as isApprox()
...
#ifndef EIGEN_NO_IO
  friend std::ostream &
  operator<< (std::ostream &s, const QuaternionBase<Derived> &q)
  {
    s << q.x () << "i + " << q.y () << "j + " << q.z () << "k" << " + "
      << q.w ();
    return s;
...
QuaternionBase<Derived>::toRotationMatrix (void) const
{
  ...
  // +35%, however, not inlining this function is an order of magnitude slower,
  // so it has to be inlined, and so the return by value is not an issue
  Matrix3 res;
