...
namespace Eigen {
   ...
   namespace internal {
      ...
      template<
         typename Other, int Mode, int Options, int Dim, int HDim, int OtherRows = Other::RowsAtCompileTime,
  ...
  */
   ...
   class Transform {
      public:
         ...
         typedef typename internal::conditional<
            int(Mode) == int(AffineCompact), MatrixType &, Block<MatrixType, Dim, HDim>>::type AffinePart;
         /** type of read reference to the affine part of the transformation */
         typedef typename internal::conditional<
            int(Mode) == int(AffineCompact), const MatrixType &, const Block<const MatrixType, Dim, HDim>>::type
            ConstAffinePart;
         /** type of a vector */
    ...
    * If Mode==Affine or Mode==Isometry, then the last row is set to [0 ... 0 1] */
         ...
         EIGEN_DEVICE_FUNC inline Transform(const Transform<Scalar, Dim, OtherMode, OtherOptions> &other) {
            ...
            if (EIGEN_CONST_CONDITIONAL(ModeIsAffineCompact == OtherModeIsAffineCompact)) {
               ...
               internal::transform_construct_from_matrix<OtherMatrixType, Mode, Options, Dim, HDim>::run(
                  this, other.matrix());
            }
            else {
    ...
    */
         template<typename OtherDerived>
         friend EIGEN_DEVICE_FUNC inline const typename internal::transform_left_product_impl<
   ...
   */
    ...
    *          and the HDim x Dim part for projective transformations.
    */
         EIGEN_DEVICE_FUNC inline const Block<MatrixType, int(Mode) == int(Projective) ? HDim : Dim, Dim> linearExt()
            const {
            return m_matrix.template block < int(Mode) == int(Projective) ? HDim : Dim, Dim > (0, 0);
         }
...
**************************/
  ...
  */
   ...
   QMatrix Transform<Scalar, Dim, Mode, Options>::toQMatrix(void) const {
      ...
      EIGEN_STATIC_ASSERT(Dim == 2, YOU_MADE_A_PROGRAMMING_MISTAKE)
      return QMatrix(
         m_matrix.coeff(0, 0), m_matrix.coeff(1, 0), m_matrix.coeff(0, 1), m_matrix.coeff(1, 1), m_matrix.coeff(0, 2),
         m_matrix.coeff(1, 2));
   }
  ...
  */
   ...
   QTransform Transform<Scalar, Dim, Mode, Options>::toQTransform(void) const {
      ...
      if (EIGEN_CONST_CONDITIONAL(Mode == int(AffineCompact)))
         return QTransform(
            m_matrix.coeff(0, 0), m_matrix.coeff(1, 0), m_matrix.coeff(0, 1), m_matrix.coeff(1, 1),
            m_matrix.coeff(0, 2), m_matrix.coeff(1, 2));
      else
         return QTransform(
            m_matrix.coeff(0, 0), m_matrix.coeff(1, 0), m_matrix.coeff(2, 0), m_matrix.coeff(0, 1),
...
*********************/
  ...
  */
   ...
   EIGEN_DEVICE_FUNC Transform<Scalar, Dim, Mode, Options> &Transform<Scalar, Dim, Mode, Options>::preshear(
      ...
      EIGEN_STATIC_ASSERT(int(Dim) == 2, YOU_MADE_A_PROGRAMMING_MISTAKE)
      EIGEN_STATIC_ASSERT(Mode != int(Isometry), THIS_METHOD_IS_ONLY_FOR_SPECIFIC_TRANSFORMATIONS)
      m_matrix.template block<Dim, HDim>(0, 0) = LinearMatrixType(1, sx, sy, 1)
                                                 * m_matrix.template block<Dim, HDim>(0, 0);
      return *this;
   }
  ...
  */
   ...
   EIGEN_DEVICE_FUNC Transform<Scalar, Dim, Mode, Options> Transform<Scalar, Dim, Mode, Options>::inverse(
      ...
      if (hint == Projective) {
         ...
         res.matrix().template topRightCorner<Dim, 1>() = -res.matrix().template topLeftCorner<Dim, Dim>()
                                                          * translation();
...
**********************************************************/
      ...
      struct transform_transform_product_impl<
         Transform<Scalar, Dim, LhsMode, LhsOptions>, Transform<Scalar, Dim, RhsMode, RhsOptions>, false> {
      ...
      template<typename Scalar, int Dim, int LhsMode, int LhsOptions, int RhsMode, int RhsOptions>
      struct transform_transform_product_impl<
         Transform<Scalar, Dim, LhsMode, LhsOptions>, Transform<Scalar, Dim, RhsMode, RhsOptions>, true> {
            typedef Transform<Scalar, Dim, LhsMode, LhsOptions> Lhs;
            typedef Transform<Scalar, Dim, RhsMode, RhsOptions> Rhs;
      ...
      struct transform_transform_product_impl<
         Transform<Scalar, Dim, AffineCompact, LhsOptions>, Transform<Scalar, Dim, Projective, RhsOptions>, true> {
      ...
      template<typename Scalar, int Dim, int LhsOptions, int RhsOptions>
      struct transform_transform_product_impl<
         Transform<Scalar, Dim, Projective, LhsOptions>, Transform<Scalar, Dim, AffineCompact, RhsOptions>, true> {
