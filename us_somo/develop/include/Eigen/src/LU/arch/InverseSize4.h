...
// inverse of M, adjugate of M and determinant of M respectively. M# is
// computed block-wise using specific formulae. For proof, see:
                             ...
                             ResultType>
{
  ...
  static void
  run (const MatrixType &mat, ResultType &result)
  {
    ...
    iD = padd (
        iD, pmul (vec4f_swizzle2 (C, C, 1, 1, 3, 3), vec4f_movehl (AB, AB)));
    iD = psub (pmul (D, vec4f_duplane (dA, 0)), iD);

    // iA = A*|D| - B*D#*C
    iA = pmul (vec4f_swizzle2 (B, B, 0, 0, 2, 2), vec4f_movelh (DC, DC));
    iA = padd (
        iA, pmul (vec4f_swizzle2 (B, B, 1, 1, 3, 3), vec4f_movehl (DC, DC)));
