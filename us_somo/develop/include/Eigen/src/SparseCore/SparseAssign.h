...
template <typename Derived>
inline Derived &
SparseMatrixBase<Derived>::operator= (const Derived &other)
{
  internal::call_assignment_no_alias (derived (), other.derived ());
  return derived ();
}
...
void
assign_sparse_to_sparse (DstXprType &dst, const SrcXprType &src)
{
...
EIGEN_CATCH_ASSIGN_DENSE_OP_SPARSE (add_assign_op, scalar_sum_op,
                                    add_assign_op);
EIGEN_CATCH_ASSIGN_DENSE_OP_SPARSE (sub_assign_op, scalar_sum_op,
                                    sub_assign_op);
