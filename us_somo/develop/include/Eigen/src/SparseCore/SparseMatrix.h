   ...
   class SparseMatrix : public SparseCompressedBase<SparseMatrix<_Scalar, _Options, _StorageIndex>> {
      ...
      * \sa insertBack, startVec */
         inline Scalar &insertBackByOuterInner(Index outer, Index inner) {
            ...
            eigen_assert(
               (m_outerIndex[ outer + 1 ] - m_outerIndex[ outer ] == 0 || m_data.index(m_data.size() - 1) < inner)
         ...
         void conservativeResize(Index rows, Index cols) {
            ...
            if (m_innerNonZeros) {
               ...
               StorageIndex *newInnerNonZeros = static_cast<StorageIndex *>(
                  std::realloc(m_innerNonZeros, (m_outerSize + outerChange) * sizeof(StorageIndex)));
               if (!newInnerNonZeros)
                  internal::throw_std_bad_alloc();
            ...
            StorageIndex *newOuterIndex = static_cast<StorageIndex *>(
               std::realloc(m_outerIndex, (m_outerSize + outerChange + 1) * sizeof(StorageIndex)));
            if (!newOuterIndex)
               internal::throw_std_bad_alloc();
...
#include EIGEN_SPARSEMATRIX_PLUGIN
            ...
            if (m_data.size() == 0 || overwrite) {
               ...
               if (n_entries > 0) {
                  ...
                  for (Index k = 0; k < n_entries; ++k) {
                     ...
                     internal::smart_copy(
                        m_data.valuePtr() + prev_p, m_data.valuePtr() + p, newData.valuePtr() + prev_p + k);
                     internal::smart_copy(
                        m_data.indexPtr() + prev_p, m_data.indexPtr() + p, newData.indexPtr() + prev_p + k);
                  ...
                  {
                     internal::smart_copy(
                        m_data.valuePtr() + prev_p, m_data.valuePtr() + m_data.size(),
                        newData.valuePtr() + prev_p + n_entries);
                     internal::smart_copy(
                        m_data.indexPtr() + prev_p, m_data.indexPtr() + m_data.size(),
   ...
   namespace internal {
      ...
      void set_from_triplets(
         const InputIterator &begin, const InputIterator &end, SparseMatrixType &mat, DupFunctor dup_func) {
   ...
   SparseMatrix<Scalar, _Options, _StorageIndex>::operator=(const SparseMatrixBase<OtherDerived> &other) {
      ...
      if (needToTranspose) {
         ...
         typedef typename internal::nested_eval<
            OtherDerived, 2, typename internal::plain_matrix_type<OtherDerived>::type>::type OtherCopy;
