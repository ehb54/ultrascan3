   ...
   class SparseCompressedBase : public SparseMatrixBase<Derived> {
      ...
      */
         internal::LowerBoundIndex lower_bound(Index row, Index col) const {
            ...
            eigen_assert(
               end >= start && "you are using a non finalized sparse matrix or written coefficient does not exist");
            internal::LowerBoundIndex p;
            p.value = std::lower_bound(this->innerIndexPtr() + start, this->innerIndexPtr() + end, inner)
                      - this->innerIndexPtr();
