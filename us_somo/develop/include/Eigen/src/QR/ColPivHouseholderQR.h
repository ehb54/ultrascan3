 ...
 * and improve numerical stability. It is slower than HouseholderQR, and faster
 * than FullPivHouseholderQR.
 *
...
public:
   ...
   * invertible.
   */
  inline const Inverse<ColPivHouseholderQR>
  inverse () const
  {
    eigen_assert (m_isInitialized
                  && "ColPivHouseholderQR is not initialized.");
      ...
      for (Index j = k + 1; j < cols; ++j)
        {
          ...
          if (m_colNormsUpdated.coeffRef (j) != RealScalar (0))
            {
              RealScalar temp = abs (m_qr.coeffRef (k, j))
...
template <typename RhsType, typename DstType>
void
ColPivHouseholderQR<_MatrixType>::_solve_impl (const RhsType &rhs,
                                               DstType &dst) const
{
...
template <bool Conjugate, typename RhsType, typename DstType>
void
ColPivHouseholderQR<_MatrixType>::_solve_impl_transposed (const RhsType &rhs,
                                                          DstType &dst) const
{
  const Index nonzero_pivots = nonzeroPivots ();
