 ...
 * rank-revealing and achieve optimal numerical stability. The trade-off is
 * that it is slower than HouseholderQR and ColPivHouseholderQR.
 *
...
public:
  ...
  FullPivHouseholderQR ()
      : m_qr (), m_hCoeffs (), m_rows_transpositions (),
        m_cols_transpositions (), m_cols_permutation (), m_temp (),
   ...
   * transpositions */
  const IntDiagSizeVectorType &
  rowsTranspositions () const
  {
    eigen_assert (m_isInitialized
                  && "FullPivHouseholderQR is not initialized.");
   ...
   * invertible.
   */
  inline const Inverse<FullPivHouseholderQR>
  inverse () const
  {
    eigen_assert (m_isInitialized
                  && "FullPivHouseholderQR is not initialized.");
  ...
  for (Index k = 0; k < size; ++k)
    {
      ...
      RealScalar beta;
      m_qr.col (k).tail (rows - k).makeHouseholderInPlace (
...
template <typename RhsType, typename DstType>
void
FullPivHouseholderQR<_MatrixType>::_solve_impl (const RhsType &rhs,
                                                DstType &dst) const
{
...
template <bool Conjugate, typename RhsType, typename DstType>
void
FullPivHouseholderQR<_MatrixType>::_solve_impl_transposed (const RhsType &rhs,
                                                           DstType &dst) const
{
  const Index l_rank = rank ();
...
public:
  ...
  void
  evalTo (ResultType &result) const
  {
  ...
  template <typename ResultType>
  void
  evalTo (ResultType &result, WorkVectorType &workspace) const
  {
    using numext::conj;
