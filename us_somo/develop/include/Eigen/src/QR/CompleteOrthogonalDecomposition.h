...
public:
   ...
   * this->solve(rhs).
   */
  inline const Inverse<CompleteOrthogonalDecomposition>
  pseudoInverse () const
  {
...
protected:
  ...
  void
  _check_solve_assertion (const Rhs &b) const
  {
    ...
    eigen_assert ((Transpose_ ? derived ().cols () : derived ().rows ())
                      == b.rows ()
                  && "CompleteOrthogonalDecomposition::solve(): invalid "
...
template <typename MatrixType>
void
CompleteOrthogonalDecomposition<MatrixType>::computeInPlace ()
{
  check_template_parameters ();
          ...
          if (k != rank - 1)
            {
              ...
              // (zero-based) to form the matrix X_k = [X(0:k, k), X(0:k, r:n)]
...
template <bool Conjugate, typename Rhs>
void
CompleteOrthogonalDecomposition<MatrixType>::applyZOnTheLeftInPlace (
    Rhs &rhs) const
{
  ...
  for (Index k = 0; k < rank; ++k)
    {
      ...
      rhs.middleRows (rank - 1, cols - rank + 1)
          .applyHouseholderOnTheLeft (
              matrixQTZ ().row (k).tail (cols - rank).adjoint (),
              zCoeffs () (k), &temp (0));
      if (k != rank - 1)
        {
          rhs.row (k).swap (rhs.row (rank - 1));
...
template <typename RhsType, typename DstType>
void
CompleteOrthogonalDecomposition<_MatrixType>::_solve_impl (const RhsType &rhs,
                                                           DstType &dst) const
{
  const Index rank = this->rank ();
  if (rank == 0)
...
template <bool Conjugate, typename RhsType, typename DstType>
void
CompleteOrthogonalDecomposition<_MatrixType>::_solve_impl_transposed (
...
const CompleteOrthogonalDecomposition<
