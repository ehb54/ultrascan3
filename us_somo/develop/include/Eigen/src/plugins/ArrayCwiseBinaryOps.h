...
/** \returns an expression of the coefficient-wise min of \c *this and scalar
 * \a other
 *
 * \sa max()
 */
...
/** \returns an expression of the coefficient-wise max of \c *this and scalar
 * \a other
 *
 * \sa min()
 */
...
(absolute_difference)
    ...
    (const Scalar &other) const
{
  return (absolute_difference)(Derived::PlainObject::Constant (
      rows (), cols (), other));
}
 ...
 * with floating-point types. In order to check for equality between two
 * vectors or matrices with floating-point coefficients, it is generally a far
 * better idea to use a fuzzy comparison as provided by isApprox() and
 * isMuchSmallerThan().
 *
...
const CwiseBinaryOp<internal::scalar_difference_op<Scalar, T>, Derived,
                    Constant<T> >
operator- (const T &scalar) const;
/** \returns an expression of the constant matrix of value \a scalar
 * decremented by the coefficients of \a expr
 *
 * \tparam T is the scalar type of \a scalar. It must be compatible with the
 * scalar type of the given expression.
 */
