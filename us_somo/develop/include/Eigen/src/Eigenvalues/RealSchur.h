   ...
   class RealSchur {
      ...
      */
         ...
         explicit RealSchur(const EigenBase<InputType> &matrix, bool computeU = true) :
             ...
             m_workspaceVector(matrix.rows()), m_hess(matrix.rows()), m_isInitialized(false), m_matUisUptodate(false),
             m_maxIters(-1) {
   ...
   inline void RealSchur<MatrixType>::initFrancisQRStep(
      ...
      for (im = iu - 2; im >= il; --im) {
         ...
         const Scalar rhs = v.coeff(0)
                            * (abs(m_matT.coeff(im - 1, im - 1)) + abs(Tmm) + abs(m_matT.coeff(im + 1, im + 1)));
