...
namespace Eigen {
   ...
   class SelfAdjointEigenSolver {
      ...
      */
         ...
         SelfAdjointEigenSolver() :
             m_eivec(), m_eivalues(), m_subdiag(), m_hcoeffs(), m_info(InvalidInput), m_isInitialized(false),
      ...
      */
         ...
         explicit SelfAdjointEigenSolver(Index size) :
             ...
             m_hcoeffs(size > 1 ? size - 1 : 1), m_isInitialized(false), m_eigenvectorsOk(false) {}
      ...
      */
         ...
         EIGEN_DEVICE_FUNC explicit SelfAdjointEigenSolver(
            const EigenBase<InputType> &matrix, int options = ComputeEigenvectors) :
             ...
             m_subdiag(matrix.rows() > 1 ? matrix.rows() - 1 : 1), m_hcoeffs(matrix.cols() > 1 ? matrix.cols() - 1 : 1),
      ...
      */
         SelfAdjointEigenSolver &computeFromTridiagonal(
            const RealVectorType &diag, const SubDiagonalType &subdiag, int options = ComputeEigenvectors);
  ...
  */
   ...
   EIGEN_DEVICE_FUNC SelfAdjointEigenSolver<MatrixType> &SelfAdjointEigenSolver<MatrixType>::compute(
      ...
      eigen_assert(
         (options & ~(EigVecMask | GenEigMask)) == 0 && (options & EigVecMask) != EigVecMask
