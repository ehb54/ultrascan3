...
under the Mozilla Public License v. 2.0, as stated at the top of this file.
*/
...
inline bool
amd_marked (const T0 *w, const T1 &j)
{
  return w[j] < 0;
}
template <typename T0, typename T1>
inline void
amd_mark (const T0 *w, const T1 &j)
{
...
static StorageIndex
cs_wclear (StorageIndex mark, StorageIndex lemax, StorageIndex *w,
           StorageIndex n)
{
  StorageIndex k;
...
template <typename StorageIndex>
StorageIndex
cs_tdfs (StorageIndex j, StorageIndex k, StorageIndex *head,
      ...
      for (pk = pk1; pk < pk2; pk++) /* scan2: degree update */
        {
          ...
          if (d == 0) /* check for mass elimination */
            ...
            {
              degree[i] = std::min<StorageIndex> (degree[i],
                                                  d); /* update degree(i) */
      ...
      for (p = pk1, pk = pk1; pk < pk2; pk++) /* finalize Lk */
        {
          ...
          last[i] = -1;
          head[d] = i;
          mindeg = std::min<StorageIndex> (mindeg,
                                           d); /* find new minimum degree */
          degree[i] = d;
