...
cholmod_sparse
viewAsCholmod (const SparseSelfAdjointView<
...
/** Returns a view of the Cholmod sparse matrix \a cm as an Eigen sparse
 * matrix. The data are not copied but shared. */
...
template <>
inline cholmod_dense *
cm_solve<SuiteSparse_long> (int sys, cholmod_factor &L, cholmod_dense &B,
...
cm_spsolve (int sys, cholmod_factor &L, cholmod_sparse &B,
            cholmod_common &Common)
{
...
template <typename _StorageIndex>
inline int
cm_factorize_p (cholmod_sparse *A, double beta[2], _StorageIndex *fset,
...
public:
   ...
   * full control over the performed operations. See the Cholmod user guide for
   * details. */
  cholmod_common &
  cholmod ()
  {
  ...
  void
  _solve_impl (const SparseMatrixBase<RhsDerived> &b,
