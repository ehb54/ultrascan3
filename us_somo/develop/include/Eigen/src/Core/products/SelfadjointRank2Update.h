 ...
 */
      ...
      struct selfadjoint_rank2_update_selector<Scalar, Index, UType, VType, Lower> {
            static EIGEN_DEVICE_FUNC void run(
               Scalar *mat, Index stride, const UType &u, const VType &v, const Scalar &alpha) {
   ...
   EIGEN_DEVICE_FUNC SelfAdjointView<MatrixType, UpLo> &SelfAdjointView<MatrixType, UpLo>::rankUpdate(
      ...
      internal::selfadjoint_rank2_update_selector<
         Scalar, Index, UType, VType, (IsRowMajor ? int(UpLo == Upper ? Lower : Upper) : UpLo)>::
         run(
            _expression().const_cast_derived().data(), _expression().outerStride(), UType(actualU), VType(actualV),
            actualAlpha);
