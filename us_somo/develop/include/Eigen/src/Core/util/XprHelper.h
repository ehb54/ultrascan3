...
EIGEN_DEVICE_FUNC inline IndexDest
...
// promote_scalar_arg is an helper used in operation between an expression and
// a scalar, like:
//    expression * scalar
...
// internal::has_ReturnType<ScalarBinaryOpTraits<ExprScalar,T,op> >::value
// using the proper order for ExprScalar and T. Then the logic is as follows:
//  - if the operation is natively supported as defined by IsSupported, then
//  the scalar type is not promoted, and T is returned.
//  - otherwise, NumTraits<ExprScalar>::Literal is returned if T is implicitly
//  convertible to NumTraits<ExprScalar>::Literal AND that this does not imply
//  a float to integer conversion.
...
// classes inheriting no_assignment_operator don't generate a default
// operator=.
...
public:
  ...
  EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE EIGEN_CONSTEXPR T
  value ()
  {
    return T (Value);
  }
...
public:
  ...
  EIGEN_DEVICE_FUNC T EIGEN_STRONG_INLINE
  value () const
  {
    return m_value;
  }
...
/** \internal Adds the const qualifier on the value-type of T2 if and only if
 * T1 is a const type */
...
/** \class ScalarBinaryOpTraits
  * \ingroup Core_Module
  *
  * \brief Determines whether the given binary operation of two numeric types
  is allowed and what the scalar return type is.
  *
  * This class permits to control the scalar return type of any binary
  operation performed on two different scalar types through (partial) template
  specializations.
  *
  ...
  * Or customize the return type for individual operation:
    \code
    template<>
    struct ScalarBinaryOpTraits<U1,U2,internal::scalar_sum_op<U1,U2> > {
  typedef U1 ReturnType; };
    \endcode
  *
  * By default, the following generic combinations are supported:
  ...
  </td></tr> <tr            ><td>\c T </td><td>\c NumTraits<T>::Real
  </td><td>\c * </td><td>\c T </td><td>Only if \c NumTraits<T>::IsComplex
  </td></tr>
  </table>
  *
...
// types gives very unexpected errors when enabling vectorization, when the
// user tries to add together a float matrix and a double matrix.
