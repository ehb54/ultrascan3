...
// For the record, here is a table summarizing the possible values for
// EIGEN_COMP_MSVC_LANG: MSVC option                          Standard
// MSVC_LANG /std:c++14 (default as of VS 2019)   C++14     201402L /std:c++17
// C++17     201703L /std:c++latest                       >C++17    >201703L
...
/// \internal EIGEN_GNUC_STRICT set to 1 if the compiler is really GCC and not
/// a compatible compiler (e.g., ICC, clang, mingw, etc.)
#if EIGEN_COMP_GNUC                                                           \
...
/// \internal EIGEN_ARCH_ARMV8 set to 1 if the architecture is armv8 or
/// greater.
...
/// \internal EIGEN_HAS_ARM64_FP16 set to 1 if the architecture provides an
/// IEEE compliant Arm fp16 type
#if EIGEN_ARCH_ARM64
...
// attribute value to 1024. The compiler assigns a default value of 256 when
// the attribute is not specified. This results in failures on the HIP
// platform, for cases when a GPU kernel without an explicit launch_bounds
// attribute is called with a threads_per_block value greater than 256.
//
...
// both the HIP and CUDA kernels. For those cases, the corresponding code
// should be guarded with
//      #if defined(EIGEN_GPUCC)
// instead of
//      #if defined(EIGEN_CUDACC) || defined(EIGEN_HIPCC)
//
// For cases where the tweak is specific to HIP, the code should be guarded
// with
...
//
// If either EIGEN_CUDA_ARCH or EIGEN_HIP_DEVICE_COMPILE is defined, then
...
// compilation phase needs to be guarded with a macro that indicates the
...
// The macros EIGEN_HAS_CXX?? defines a rough estimate of available c++
// features but in practice we should not rely on them but rather on the
// availabilty of individual features as defined later. This is why there is no
// EIGEN_HAS_CXX17.
...
// ^^ Disable the use of variadic templates when compiling with versions of
// nvcc older than 8.0 on ARM devices:
    ...
    && ((EIGEN_COMP_CXXVER > 11)                                              \
        || (EIGEN_COMP_CXXVER == 11)                                          \
               && (EIGEN_COMP_GNUC_STRICT || EIGEN_COMP_CLANG                 \
                   || EIGEN_COMP_MSVC || EIGEN_COMP_ICC)                      \
...
// While available already with c++11, this is useful mostly starting with
// c++14 and relaxed constexpr rules
#if defined(__NVCC__)
...
// Does the compiler support the __int128 and __uint128_t extensions for
...
// EIGEN_ALWAYS_INLINE is the stronget, it has the effect of making the
// function inline and adding every possible attribute to maximize inlining.
// This should only be used when really necessary: in particular, it uses
...
inline void
assert_fail (const char *condition, const char *function, const char *file,
             int line)
{
...
ignore_unused_variable (const T &)
{
}
}
}
...
#define EIGEN_USING_STD(FUNC) using std::FUNC;
    ...
    && (EIGEN_COMP_MSVC < 1900                                                \
        || (EIGEN_COMP_MSVC == 1900 && EIGEN_COMP_NVCC))
...
// EIGEN_SIZE_MIN_PREFER_DYNAMIC gives the min between compile-time sizes. 0
