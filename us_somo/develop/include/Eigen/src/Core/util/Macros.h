...
#define EIGEN_HAS_CONSTEXPR 1
   ...
   && (__has_feature(cxx_relaxed_constexpr) || (EIGEN_COMP_CXXVER >= 14) || (EIGEN_GNUC_AT_LEAST(4, 8) && (EIGEN_COMP_CXXVER >= 11)) || (EIGEN_COMP_CLANG >= 306 && (EIGEN_COMP_CXXVER >= 11)))
#define EIGEN_HAS_CONSTEXPR 1
#endif
...
#if EIGEN_MAX_CPP_VER >= 11 \
   && ((EIGEN_COMP_CXXVER > 11) || (EIGEN_COMP_CXXVER == 11) && (EIGEN_COMP_GNUC_STRICT || EIGEN_COMP_CLANG || EIGEN_COMP_MSVC || EIGEN_COMP_ICC) && (EIGEN_ARCH_i386_OR_x86_64) && (EIGEN_OS_GNULINUX || EIGEN_OS_WIN_STRICT || EIGEN_OS_MAC))
...
#if EIGEN_MAX_CPP_VER >= 17 && EIGEN_COMP_CXXVER >= 17 \
   && ((EIGEN_COMP_MSVC >= 1912) || (EIGEN_GNUC_AT_LEAST(7, 0)) || ((!defined(__apple_build_version__)) && (EIGEN_COMP_CLANG >= 500)) || ((defined(__apple_build_version__)) && (__apple_build_version__ >= 10000000)))
...
#define EIGEN_GENERIC_PUBLIC_INTERFACE(Derived) \
   ...
   typedef typename Eigen::NumTraits<Scalar>::Real \
      RealScalar; /*!< \brief The underlying numeric type for composed scalar types. \details In cases where Scalar is e.g. std::complex<T>, T were corresponding to RealScalar. */ \
   typedef typename Base::CoeffReturnType \
      CoeffReturnType; /*!< \brief The return type for coefficient access. \details Depending on whether the object allows direct coefficient access (e.g. for a MatrixXd), this type is either 'const Scalar&' or simply 'Scalar' for objects that do not allow direct coefficient access. */ \
...
#define EIGEN_CWISE_BINARY_RETURN_TYPE(LHS, RHS, OPNAME) \
   CwiseBinaryOp< \
      EIGEN_CAT(EIGEN_CAT(internal::scalar_, OPNAME), _op) < typename internal::traits<LHS>::Scalar, \
...
#define EIGEN_EXPR_BINARYOP_SCALAR_RETURN_TYPE(EXPR, SCALAR, OPNAME) \
   CwiseBinaryOp<EIGEN_CAT(EIGEN_CAT(internal::scalar_, OPNAME), _op) < typename internal::traits<EXPR>::Scalar, SCALAR>, \
   ...
   EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE EIGEN_MSVC10_WORKAROUND_BINARYOP_RETURN_TYPE( \
      const EIGEN_EXPR_BINARYOP_SCALAR_RETURN_TYPE( \
         ...
         typename internal::promote_scalar_arg< \
            Scalar EIGEN_COMMA T EIGEN_COMMA EIGEN_SCALAR_BINARY_SUPPORTED(OPNAME, Scalar, T)>::type, \
