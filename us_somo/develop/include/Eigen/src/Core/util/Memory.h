...
{
  eigen_assert (
      false && "heap allocation is forbidden (EIGEN_NO_MALLOC is defined)");
}
...
aligned_malloc (std::size_t size)
{
  ...
  eigen_assert ((size < 16 || (std::size_t (result) % 16) == 0)
                && "System's malloc returned an unaligned pointer. Compile "
                   "with EIGEN_MALLOC_ALREADY_ALIGNED=0 to fallback to "
                   "handmade aligned memory allocator.");
#endif
#else
...
template <bool Align>
inline void *
conditional_aligned_realloc (void *ptr, std::size_t new_size,
                             std::size_t old_size)
{
...
template <>
inline void *
conditional_aligned_realloc<false> (void *ptr, std::size_t new_size,
                                    std::size_t)
{
 ...
 * pointer is undefined, but a std::bad_alloc is thrown. The default
 * constructor of T is called.
 */
template <typename T>
EIGEN_DEVICE_FUNC inline T *
...
template <typename T, bool Align>
inline T *
conditional_aligned_realloc_new_auto (T *pts, std::size_t new_size,
                                      std::size_t old_size)
{
  check_size_for_overflow<T> (new_size);
  check_size_for_overflow<T> (old_size);
 ...
 * is not a multiple of a scalar, the size of the array is returned. For
 * example with SSE, the requested alignment is typically 16-bytes. If packet
 * size for the given scalar type is 1, then everything is considered
 * well-aligned.
 *
 * \note Otherwise, if the Alignment is larger that the scalar size, we rely on
 * the assumptions that sizeof(Scalar) is a power of 2. On the other hand, we
 * do not assume that the array address is a multiple of sizeof(Scalar), as
 * that fails for example with Scalar=double on certain 32-bit platforms, see
 * bug #79.
 *
...
// intelligent memmove. falls back to std::memmove for POD types, uses
// std::copy otherwise.
 ...
 * \endcode
 * with the advantage of using aligned stack allocation even if the maximal
 * size of XPR at compile time is unknown. This is accomplished through alloca
 * if this later is supported and if the required number of bytes is below
...
// If alloca is already aligned, the compiler should be smart enough to
// optimize away the re-alignment.
      ...
      = (BUFFER) != 0                                                         \
            ...
            : reinterpret_cast<TYPE *> (                                      \
                  (sizeof (TYPE) * SIZE <= EIGEN_STACK_ALLOCATION_LIMIT)      \
                      ...
                      : Eigen::internal::aligned_malloc (sizeof (TYPE)        \
                                                         * SIZE));            \
  ...
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW_IF (bool (                                  \
      ...
      && (((EIGEN_MAX_ALIGN_BYTES >= 16)                                      \
          ...
          || ((EIGEN_MAX_ALIGN_BYTES >= 32)                                   \
              && ((sizeof (Scalar) * (Size)) % (EIGEN_MAX_ALIGN_BYTES / 2)    \
                  == 0))                                                      \
          || ((EIGEN_MAX_ALIGN_BYTES >= 64)                                   \
              && ((sizeof (Scalar) * (Size)) % (EIGEN_MAX_ALIGN_BYTES / 4)    \
                  == 0)))))
 ...
 *
 * This can be controlled using the \c EIGEN_MAX_ALIGN_BYTES macro as
 * documented
...
queryCacheSizes (int &l1, int &l2, int &l3)
{
  ...
  const int AMDisbetter_[]
      = { 0x69444d41, 0x74656273, 0x21726574 }; // "AMDisbetter!"
