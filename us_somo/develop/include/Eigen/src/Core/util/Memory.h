     ...
     **/
      ...
      struct local_nested_eval_wrapper<Xpr, NbEvaluations, true> {
            ...
            local_nested_eval_wrapper(const Xpr &xpr, Scalar *ptr) :
                object(
                   ptr == 0 ? reinterpret_cast<Scalar *>(Eigen::internal::aligned_malloc(sizeof(Scalar) * xpr.size()))
                            : ptr,
                   xpr.rows(), xpr.cols()),
                m_deallocate(ptr == 0) {
...
#define ei_declare_aligned_stack_constructed_variable(TYPE, NAME, SIZE, BUFFER) \
   ...
   EIGEN_CAT(NAME, _stack_memory_destructor)((BUFFER) == 0 ? NAME : 0, SIZE, true)
...
#define EIGEN_MAKE_ALIGNED_OPERATOR_NEW_IF_VECTORIZABLE_FIXED_SIZE(Scalar, Size) \
   EIGEN_MAKE_ALIGNED_OPERATOR_NEW_IF(bool( \
      ((Size) != Eigen::Dynamic) \
      && (((EIGEN_MAX_ALIGN_BYTES >= 16) && ((sizeof(Scalar) * (Size)) % (EIGEN_MAX_ALIGN_BYTES) == 0)) || ((EIGEN_MAX_ALIGN_BYTES >= 32) && ((sizeof(Scalar) * (Size)) % (EIGEN_MAX_ALIGN_BYTES / 2) == 0)) || ((EIGEN_MAX_ALIGN_BYTES >= 64) && ((sizeof(Scalar) * (Size)) % (EIGEN_MAX_ALIGN_BYTES / 4) == 0)))))
