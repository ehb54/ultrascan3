          ...
          : evaluator_base<Reshaped<ArgType, Rows, Cols, Order>> {
            ...
            inline RowCol index_remap(Index rowId, Index colId) const {
               if (Order == ColMajor) {
                  ...
                  return RowCol(
                     nth_elem_idx % m_xpr.nestedExpression().rows(), nth_elem_idx / m_xpr.nestedExpression().rows());
               }
               else {
                  const Index nth_elem_idx = colId + rowId * m_xpr.cols();
                  return RowCol(
                     nth_elem_idx / m_xpr.nestedExpression().cols(), nth_elem_idx % m_xpr.nestedExpression().cols());
               }
            }
  ...
  inline void writePacket(Index index, const PacketScalar& val)
  {
    ...
    return m_argImpl.template packet<Unaligned>(row_col.first, row_col.second, val);
      ...
      struct reshaped_evaluator<ArgType, Rows, Cols, Order, /* HasDirectAccess */ true>
          : mapbase_evaluator<
               Reshaped<ArgType, Rows, Cols, Order>, typename Reshaped<ArgType, Rows, Cols, Order>::PlainObject> {
            typedef Reshaped<ArgType, Rows, Cols, Order> XprType;
            typedef typename XprType::Scalar Scalar;
