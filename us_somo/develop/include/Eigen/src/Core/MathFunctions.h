 ...
 * cases, like the Array expressions. When you do sin(array1+array2), the
 * object array1+array2 has a complicated expression type, all what you want to
 * know is that it inherits ArrayBase. So we implement a partial specialization
 * of sin_impl for ArrayBase<Derived>. So we must make sure to use
...
{
  EIGEN_DEVICE_FUNC
  static EIGEN_ALWAYS_INLINE Scalar
  run (const Scalar &x)
  {
...
{
  EIGEN_DEVICE_FUNC
  static EIGEN_ALWAYS_INLINE std::complex<T>
  run (const std::complex<T> &x)
  {
...
template <typename OldType, typename NewType>
EIGEN_DEVICE_FUNC inline NewType
...
template <typename Scalar> struct random_default_impl<Scalar, false, true>
{
  ...
  run (const Scalar &x, const Scalar &y)
  {
    ...
    // types and arithmetic and signed overflows (which are undefined
    // behavior).
    ...
    // are two's complement and have the same number of padding bits in signed
    // and unsigned variants. This is the case in most modern implementations
    // of C++.
...
isnan_impl (const long double &x)
{
  return _isnan (x) != 0;
}
EIGEN_DEVICE_FUNC inline bool
isnan_impl (const double &x)
{
  return _isnan (x) != 0;
}
EIGEN_DEVICE_FUNC inline bool
isnan_impl (const float &x)
{
  return _isnan (x) != 0;
}

EIGEN_DEVICE_FUNC inline bool
isinf_impl (const long double &x)
{
  return isinf_msvc_helper (x);
}
EIGEN_DEVICE_FUNC inline bool
isinf_impl (const double &x)
{
  return isinf_msvc_helper (x);
}
EIGEN_DEVICE_FUNC inline bool
isinf_impl (const float &x)
{
  return isinf_msvc_helper (x);
}
...
//      while the second prevent too aggressive optimizations in fast-math
//      mode:
#define EIGEN_TMP_NOOPT_ATTRIB                                                \
...
template <>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE float
fmod (const float &a, const float &b)
{
  return ::fmodf (a, b);
}
...
template <>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE double
fmod (const double &a, const double &b)
{
  return ::fmod (a, b);
}
#endif
...
template <typename T>
EIGEN_DEVICE_FUNC bool
isfinite_impl (const std::complex<T> &x)
{
...
template <typename T>
EIGEN_DEVICE_FUNC bool
isnan_impl (const std::complex<T> &x)
{
  return (numext::isnan) (numext::real (x))
         || (numext::isnan) (numext::imag (x));
}

template <typename T>
EIGEN_DEVICE_FUNC bool
isinf_impl (const std::complex<T> &x)
{
