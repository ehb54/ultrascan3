 ...
 * vector, arrays, and related expression types). The common Eigen API for
 * dense objects is contained in this class.
 *
...
public:
  ...
  enum
  {
     ...
     * provided by the \a Derived type. If a value is not known at
     * compile-time, it is set to the \a Dynamic constant.
     ...
     * high number of columns, this value is set to \a Dynamic.
     *
     * This value is useful to know when evaluating an expression, in order to
     * determine whether it is possible to avoid doing a dynamic memory
     * allocation.
     ...
     * with a row-vector (if there is only one row). */
   ...
   * is the minor dimension with respect to the \ref TopicStorageOrders
   * "storage order", i.e., the number of rows for a column-major matrix, and
   * the number of columns for a row-major matrix. */
  EIGEN_DEVICE_FUNC EIGEN_CONSTEXPR Index
  innerSize () const
  {
   ...
   * The present method only asserts that the new size equals the old size, and
   * does nothing else.
   */
  EIGEN_DEVICE_FUNC
  void
  ...
  EIGEN_DEPRECATED const Derived &
  flagged () const
  {
    return derived ();
  }
  ...
  template <typename OtherDerived>
  EIGEN_DEVICE_FUNC bool
  isMuchSmallerThan (const DenseBase<OtherDerived> &other,
                     const RealScalar &prec
                     = NumTraits<Scalar>::dummy_precision ()) const;

  EIGEN_DEVICE_FUNC bool
  isApproxToConstant (const Scalar &value,
                      const RealScalar &prec
                      = NumTraits<Scalar>::dummy_precision ()) const;
  EIGEN_DEVICE_FUNC bool
  isConstant (const Scalar &value,
              ...
              = NumTraits<Scalar>::dummy_precision ()) const;
  EIGEN_DEVICE_FUNC bool
  isZero (const RealScalar &prec
          = NumTraits<Scalar>::dummy_precision ()) const;
  EIGEN_DEVICE_FUNC bool
  isOnes (const RealScalar &prec
          = NumTraits<Scalar>::dummy_precision ()) const;
