...
template <typename Scalar> struct hypot_impl
{
  ...
  static EIGEN_DEVICE_FUNC inline RealScalar
...
complex_sqrt (const std::complex<T> &z)
{
  ...
  const T zero = T (0);
  const T w
      = numext::sqrt (T (0.5) * (numext::abs (x) + numext::hypot (x, y)));
...
template <typename T>
EIGEN_DEVICE_FUNC std::complex<T>
complex_rsqrt (const std::complex<T> &z)
{
...
template <typename T>
EIGEN_DEVICE_FUNC std::complex<T>
complex_log (const std::complex<T> &z)
{
