...
// Define the threshold value to fallback from the generic matrix-matrix
// product implementation (heavy) to the lightweight coeff-based product one.
...
// Cons: this could be a problem if in a meta unrolled algorithm a
// matrix-matrix product ends up to a row-vector times col-vector product... To
// tackle this use case, we could have a specialization for
 ...
 * direction. Therefore we need a lower level meta selector. Furthermore, if
 * the matrix is the rhs, then the product has to be transposed.
 */
namespace internal
{
...
struct gemv_dense_selector<OnTheLeft, StorageOrder, BlasCompatible>
{
  ...
  static void
  run (const Lhs &lhs, const Rhs &rhs, Dest &dest,
    ...
    if (!MightCannotUseDest)
      {
        ...
        // this ease the compiler to generate cleaner and more optimzized code
...
template <> struct gemv_dense_selector<OnTheRight, RowMajor, false>
{
       ...
       const typename Dest::Scalar &alpha)
  {
    ...
    for (Index i = 0; i < rows; ++i)
      dest.coeffRef (i)
          += alpha
             * (lhs.row (i).cwiseProduct (actual_rhs.transpose ())).sum ();
  }
};
 ...
 * coefficients of the product will be computed once at a time as requested.
 * This might be useful in some extremely rare cases when only a small and no
 * coherent fraction of the result's coefficients have to be computed.
 *
