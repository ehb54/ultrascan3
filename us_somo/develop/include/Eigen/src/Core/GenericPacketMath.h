...
template <typename T, int unique_id = 0> struct eigen_packet_wrapper
{
  ...
  EIGEN_ALWAYS_INLINE
  eigen_packet_wrapper (const T &v)
      : m_val (v)
  {
  }
...
// Although this is technically not a valid bitmask, the scalar path for
// pselect uses a comparison to zero, so this should still work in most cases.
// We don't have another option, since the scalar type requires initialization.
template <typename T>
struct ptrue_impl<
    T, typename internal::enable_if<
           is_scalar<T>::value && NumTraits<T>::RequireInitialization>::type>
{
  static EIGEN_DEVICE_FUNC inline T
  run (const T & /*a*/)
  {
    return T (1);
  }
...
// For scalars, explicitly set to Scalar(0), since the underlying
// representation for zero may not consist of all-zero bits.
template <typename T>
...
{
  EIGEN_DEVICE_FUNC static inline T
  bitwise_and (const T &a, const T &b)
  {
    return bit_and<T> () (a, b);
  }
  EIGEN_DEVICE_FUNC static inline T
  bitwise_or (const T &a, const T &b)
  {
    return bit_or<T> () (a, b);
  }
  EIGEN_DEVICE_FUNC static inline T
  bitwise_xor (const T &a, const T &b)
  {
    return bit_xor<T> () (a, b);
  }
  EIGEN_DEVICE_FUNC static inline T
  bitwise_not (const T &a)
  {
    return bit_not<T> () (a);
  }
};
...
private:
  ...
  template <typename Op>
  EIGEN_DEVICE_FUNC static inline T
  binary (const T &a, const T &b, Op op)
  {
    const unsigned char *a_ptr = reinterpret_cast<const unsigned char *> (&a);
...
template <int NaNPropagation> struct pminmax_impl
{
  ...
  static EIGEN_DEVICE_FUNC inline Packet
  run (const Packet &a, const Packet &b, Op op)
  {
    return op (a, b);
  }
};
...
/** \internal \returns a packet with constant coefficients \a a, e.g.:
 * (a,a,a,a) */
template <typename Packet>
EIGEN_DEVICE_FUNC inline Packet
...
/** \internal \returns a packet with constant coefficients \a a, e.g.: (x, 0,
   x, 0), where x is the value of all 1-bits. */
template <typename Packet>
EIGEN_DEVICE_FUNC inline Packet
 ...
 * returns a half packet {a0+a4, a1+a5, a2+a6, a3+a7} For packet-size smaller
 * or equal to 4, this boils down to a noop.
 */
template <typename Packet>
EIGEN_DEVICE_FUNC inline
...
/***************************************************************************
 * The following functions might not have to be overwritten for vectorized
 *types
 ***************************************************************************/
...
template <typename Packet>
inline void
pstore1 (typename unpacket_traits<Packet>::type *to,
