...
public:
  ...
  struct virtual_pointer_t
  {
    ...
    bool
    operator< (virtual_pointer_t rhs) const
    {
   ...
   */
  static inline bool
  is_nullptr (virtual_pointer_t ptr)
  {
   ...
   */
  typename pointerMap_t::iterator
  get_insertion_point (size_t requiredSize)
  {
   ...
   */
  typename pointerMap_t::iterator
  get_node (const virtual_pointer_t ptr)
  {
    if (this->count () == 0)
      {
        m_pointerMap.clear ();
  ...
  get_buffer (const virtual_pointer_t ptr)
  {
    ...
    // `buffer<>`. We can do this without the `buffer_mem` being a pointer, as
    // we only declare member variables in the base class (`buffer_mem`) and
    // not in the child class (`buffer<>).
  ...
  get_offset (const virtual_pointer_t ptr)
  {
    // The previous element to the lower bound is the node that
    // holds this memory address
    auto node = get_node (ptr);
  ...
  inline size_t
  get_element_offset (const virtual_pointer_t ptr)
  {
   ...
   */
  inline virtual_pointer_t
  add_pointer (const buffer_t &b)
  {
   ...
   */
  void
  fuse_forward (typename pointerMap_t::iterator &node)
  {
   ...
   */
  void
  fuse_backward (typename pointerMap_t::iterator &node)
  {
  ...
  void
  remove_pointer (const virtual_pointer_t ptr)
  {
    if (is_nullptr (ptr))
      {
        return;
...
private:
  ...
  {
    bool
    operator() (typename pointerMap_t::iterator a,
...
template <>
inline void
PointerMapper::remove_pointer<false> (const virtual_pointer_t ptr)
{
  if (is_nullptr (ptr))
    {
      return;
 ...
 */
inline void *
SYCLmalloc (size_t size, PointerMapper &pMap)
{
  if (size == 0)
    {
      return nullptr;
...
inline void
SYCLfree (void *ptr, PointerMapper &pMap)
{
...
inline void
SYCLfreeAll (PointerMapper &pMap)
{
  pMap.clear ();
}
...
template <cl::sycl::access::mode AcMd, typename T> struct RangeAccess
{
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE ptr_t
  get_pointer () const
  {
    return (access_.get_pointer ().get () + offset_);
  }
  template <typename Index>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE self_t &
  operator+= (Index offset)
  {
    offset_ += (offset);
    return *this;
  }
  ...
  set_offset (std::ptrdiff_t offset)
  {
    offset_ = offset;
  }

  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE ref_t
  operator* () const
  {
    return *get_pointer ();
  }
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE scalar_t *
  get_virtual_pointer () const
  {
