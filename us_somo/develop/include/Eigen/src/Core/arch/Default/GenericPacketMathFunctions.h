...
pfrexp_generic (const Packet &a, Packet &exponent)
{
  ...
  const Packet sign_mantissa_mask = pset1frombits<Packet> (
      static_cast<ScalarUI> (scalar_sign_mantissa_mask));
  ...
  const Scalar scalar_exponent_offset = -Scalar (
      (ScalarUI (1) << (int (ExponentBits) - 1)) - ScalarUI (2)); // -126
  ...
  const Packet m
      = pselect (is_zero_or_not_finite, a,
                 por (pand (normalized_a, sign_mantissa_mask), half));
  exponent = pselect (is_zero_or_not_finite, zero,
  ...
  if (base2)
    {
      const Packet cst_log2e
          = pset1<Packet> (static_cast<float> (EIGEN_LOG2E));
...
inline float
trig_reduce_huge (float xf, int *quadrant)
{
  ...
  static const uint32_t two_over_pi[]
      = { 0x00000028, 0x000028be, 0x0028be60, 0x28be60db, 0xbe60db93,
          0x60db9391, 0xdb939105, 0x9391054a, 0x91054a7f, 0x054a7f09,
          0x4a7f09d5, 0x7f09d5f4, 0x09d5f47d, 0xd5f47d4d, 0xf47d4d37,
          0x7d4d3770, 0x4d377036, 0x377036d8, 0x7036d8a5, 0x36d8a566,
          0xd8a5664f, 0xa5664f10, 0x664f10e4, 0x4f10e410, 0x10e41000,
          0xe4100000 };
...
psqrt_complex (const Packet &a)
{
  ...
  RealPacket eta = pandnot (pmul (cst_half, pdiv (a.v, pcplxflip (rho).v)),
                            a_max_zero_mask);
  ...
  Packet result = pselect (negative_real_mask, negative_real_result,
                           positive_real_result);
  ...
  real_inf_result.v = pmul (
      a_abs, pset1<Packet> (Scalar (RealScalar (1.0), RealScalar (0.0))).v);
...
absolute_split (const Packet &x, Packet &n, Packet &r)
{
  n = pround (x);
  r = psub (x, n);
}
...
template <typename Packet>
EIGEN_STRONG_INLINE void
veltkamp_splitting (const Packet &x, Packet &x_hi, Packet &x_lo)
{
  typedef typename unpacket_traits<Packet>::type Scalar;
...
template <typename Packet>
EIGEN_STRONG_INLINE void
twosum (const Packet &x_hi, const Packet &x_lo, const Packet &y_hi,
...
template <typename Packet>
EIGEN_STRONG_INLINE void
fast_twosum (const Packet &x_hi, const Packet &x_lo, const Packet &y_hi,
             const Packet &y_lo, Packet &s_hi, Packet &s_lo)
{
  Packet r_hi, r_lo;
...
template <typename Packet>
EIGEN_STRONG_INLINE void
fast_twosum (const Packet &x, const Packet &y_hi, const Packet &y_lo,
...
template <typename Packet>
EIGEN_STRONG_INLINE void
twoprod (const Packet &x_hi, const Packet &x_lo, const Packet &y, Packet &p_hi,
         Packet &p_lo)
{
  Packet c_hi, c_lo1;
...
template <typename Packet>
EIGEN_STRONG_INLINE void
twoprod (const Packet &x_hi, const Packet &x_lo, const Packet &y_hi,
...
template <typename Packet>
void
doubleword_reciprocal (const Packet &x, Packet &recip_hi, Packet &recip_lo)
{
  typedef typename unpacket_traits<Packet>::type Scalar;
...
template <> struct fast_accurate_exp2<float>
{
  ...
  operator() (const Packet &x)
  {
    ...
    // evaluated in single precision, and the remaining steps are evaluated
    // with extra precision using double word arithmetic. C is an extra precise
...
// TODO(rmlarsen): We should probably add this as a packet up 'ppow', to make
// it easier to specialize or turn off for specific types and/or backends.x
template <typename Packet>
EIGEN_STRONG_INLINE Packet
generic_pow_impl (const Packet &x, const Packet &y)
{
  typedef typename unpacket_traits<Packet>::type Scalar;
...
generic_pow (const Packet &x, const Packet &y)
{
  ...
  const Packet abs_y_is_huge
      = pcmp_le (pset1<Packet> (huge_exponent), pabs (y));
  ...
  const Packet pow_is_zero = por (
      por (por (pand (x_is_zero, y_is_pos), pand (abs_x_is_inf, y_is_neg)),
           pand (pand (abs_x_is_lt_one, abs_y_is_huge), y_is_pos)),
      pand (pand (abs_x_is_gt_one, abs_y_is_huge), y_is_neg));
  const Packet pow_is_inf = por (
      por (por (pand (x_is_zero, y_is_neg), pand (abs_x_is_inf, y_is_pos)),
