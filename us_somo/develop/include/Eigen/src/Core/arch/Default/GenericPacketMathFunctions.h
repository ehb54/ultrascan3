...
namespace Eigen {
   namespace internal {
      ...
      EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Packet pldexp_generic(const Packet &a, const Packet &exponent) {
         ...
         const Packet max_exponent = pset1<Packet>(
            Scalar((ScalarI(1) << int(ExponentBits)) + ScalarI(int(MantissaBits) - 1))); // 278
 ...
 */
      ...
      inline float trig_reduce_huge(float xf, int *quadrant) {
         ...
         static const uint32_t two_over_pi[] = { 0x00000028, 0x000028be, 0x0028be60, 0x28be60db, 0xbe60db93, 0x60db9391,
...
// The coefficients are: 0xbfc90f80, 0xb7354480, 0x2e74b9ee
      ...
      generic_pow(const Packet &x, const Packet &y) {
         ...
         EIGEN_CONSTEXPR Scalar huge_exponent = (NumTraits<Scalar>::max_exponent() * Scalar(EIGEN_LN2))
         ...
         const Packet pow_is_one = por(
            por(x_is_one, y_is_zero), pand(x_is_neg_one, por(abs_y_is_inf, pandnot(y_is_even, invalid_negative_x))));
         const Packet pow_is_nan = por(invalid_negative_x, por(x_is_nan, y_is_nan));
         const Packet pow_is_zero = por(
            por(
               por(pand(x_is_zero, y_is_pos), pand(abs_x_is_inf, y_is_neg)),
         ...
         const Packet pow_is_inf = por(
            por(
               por(pand(x_is_zero, y_is_neg), pand(abs_x_is_inf, y_is_pos)),
