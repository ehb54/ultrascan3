...
pfrexp (const Packet16h &a, Packet16h &exponent)
{
  Packet16f fexponent;
  const Packet16h out
...
template <>
EIGEN_STRONG_INLINE Packet16h
pldexp (const Packet16h &a, const Packet16h &exponent)
{
  return float2half (
      pldexp<Packet16f> (half2float (a), half2float (exponent)));
}

template <>
EIGEN_STRONG_INLINE Packet16bf
pfrexp (const Packet16bf &a, Packet16bf &exponent)
{
  Packet16f fexponent;
...
template <>
EIGEN_STRONG_INLINE Packet16bf
pldexp (const Packet16bf &a, const Packet16bf &exponent)
{
...
psqrt<Packet16f> (const Packet16f &x)
{
...
psqrt<Packet8d> (const Packet8d &x)
{
...
template <>
EIGEN_STRONG_INLINE Packet16f
prsqrt<Packet16f> (const Packet16f &x)
{
...
prsqrt<Packet16f> (const Packet16f &_x)
{
  ...
  Packet16f y_newton
      = pmul (y_approx, pmadd (y_approx, pmul (neg_half, y_approx),
                               p16f_one_point_five));
  ...
  // will return rsqrt(+inf) = 0, rsqrt(x) = NaN if x < 0, and rsqrt(0) = +inf.
...
prsqrt<Packet8d> (const Packet8d &_x)
{
  ...
  // convergence and roughly doubles the number of correct digits for each
  // step. This uses the formula y_{n+1} = y_n * (1.5 - y_n * (0.5 * x) * y_n).
  // It is essential to evaluate the inner term like this because forming
  // y_n^2 may over- or underflow.
  Packet8d y_newton
      = pmul (y_approx,
              pmadd (neg_half, pmul (y_approx, y_approx), p8d_one_point_five));
#if !defined(EIGEN_VECTORIZE_AVX512ER)
...
template <>
EIGEN_STRONG_INLINE Packet8d
prsqrt<Packet8d> (const Packet8d &x)
{
