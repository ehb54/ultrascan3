   ...
   namespace internal {
      ...
      EIGEN_DEVICE_FUNC inline Packet4cf pgather<std::complex<float>, Packet4cf>(
         ...
         return Packet4cf(_mm256_set_ps(
            std::imag(from[ 3 * stride ]), std::real(from[ 3 * stride ]), std::imag(from[ 2 * stride ]),
      ...
      EIGEN_DEVICE_FUNC inline void pscatter<std::complex<float>, Packet4cf>(
         ...
         to[ stride * 2 ] = std::complex<float>(
            _mm_cvtss_f32(_mm_shuffle_ps(high, high, 0)), _mm_cvtss_f32(_mm_shuffle_ps(high, high, 1)));
         to[ stride * 3 ] = std::complex<float>(
            _mm_cvtss_f32(_mm_shuffle_ps(high, high, 2)), _mm_cvtss_f32(_mm_shuffle_ps(high, high, 3)));
      }
      ...
      EIGEN_DEVICE_FUNC inline Packet2cd pgather<std::complex<double>, Packet2cd>(
         ...
         return Packet2cd(_mm256_set_pd(
            std::imag(from[ 1 * stride ]), std::real(from[ 1 * stride ]), std::imag(from[ 0 * stride ]),
