...
// The main advantage of this approach is not just speed, but also the fact
// that it can be inlined and pipelined with other computations, further
// reducing its effective latency. This is similar to Quake3's fast inverse
// square root. For detail see here:
...
prsqrt<Packet8f> (const Packet8f &_x)
{
  ...
  Packet8f y_newton
      = pmul (y_approx,
              pmadd (y_approx, pmul (neg_half, y_approx), p8f_one_point_five));
...
pfrexp (const Packet8h &a, Packet8h &exponent)
{
  Packet8f fexponent;
  const Packet8h out
      = float2half (pfrexp<Packet8f> (half2float (a), fexponent));
...
template <>
EIGEN_STRONG_INLINE Packet8h
pldexp (const Packet8h &a, const Packet8h &exponent)
{
...
pfrexp (const Packet8bf &a, Packet8bf &exponent)
{
  Packet8f fexponent;
  const Packet8bf out
      = F32ToBf16 (pfrexp<Packet8f> (Bf16ToF32 (a), fexponent));
...
template <>
EIGEN_STRONG_INLINE Packet8bf
pldexp (const Packet8bf &a, const Packet8bf &exponent)
{
