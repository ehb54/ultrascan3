...
class US_Efa {
 public:
  bool Efa(vector<vector<double> > &A, vector<vector<double> > &result,
           bool rightEfa = false);
  bool Efa_t(unsigned int numThreads, vector<vector<double> > &A,
             vector<vector<double> > &result, bool rightEfa = false);
  QString errors;
...
class efa_thr_t : public QThread {
 public:
  ...
  void efa_thr_setup(vector<double **> &a_t, int m, vector<int> &n_t,
