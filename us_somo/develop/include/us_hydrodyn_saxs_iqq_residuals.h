...
class US_EXTERN US_Hydrodyn_Saxs_Iqq_Residuals : public QFrame {
 ...
 public:
  US_Hydrodyn_Saxs_Iqq_Residuals(
      ...
      bool plot_as_percent, double avg_std_dev_frac,
      vector<double> std_dev_frac, unsigned int pen_width, QWidget *p = 0,
      const char *name = 0);
  ~US_Hydrodyn_Saxs_Iqq_Residuals();

  void add(unsigned int width, vector<double> q, vector<double> difference,
           vector<double> difference_no_errors, vector<double> target,
           vector<double> log_difference, vector<double> log_target,
           QColor plot_color);
