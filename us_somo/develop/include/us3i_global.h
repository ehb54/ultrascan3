...
public:
  /*! \brief Create or attach to shared memory
     ...
     Right now there is only one value stored in shared memory: the location of
     the upper left corner of the last program that started.  It is initialized
     to QPoint( 50, 50 ).
  */
  US3i_Global ();
  ...
  */
  ...
  /*! \brief Sets the point in shared memory
     ...
     This function sets the point that the next program should use for its
     upper left corner.  The calling program should increnment this point after
     it retrives the current point and decrement it when terminating.
  */
