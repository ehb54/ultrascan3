#ifndef US_HYDRODYN_MALS_SIMULATE_H
#define US_HYDRODYN_MALS_SIMULATE_H
...
class US_EXTERN US_Hydrodyn_Mals_Simulate : public QFrame {
 ...
 public:
  US_Hydrodyn_Mals_Simulate(void* us_hydrodyn, void* mals_win,
                            QStringList files, map<QString, vector<double> > q,
                            map<QString, vector<double> > I,
                            map<QString, vector<double> > e, QWidget* p = 0,
                            const char* name = 0);

  ~US_Hydrodyn_Mals_Simulate();
