...
int main(int argc, char** argv) {
  ...
  if (argc < 2) {
    printf(
        ...
        "wgsbs     \toutfile solutionfile bufferfile waxsfile alpha beta const "
        "low high"
        "          \tWAXS guided SAXS buffer subtraction\n"
        "wgsbsg    \toutfile logfile solutionfile bufferfile waxsfile alphamin "
        "alphamax alphainc betamin betamax betainc constmin constmax constinc "
        "low high\n"
        "          \tWAXS guided SAXS buffer subtraction grid search\n"
        "wgsbsgsm  \toutfile logfile solutionfile bufferfile waxsfile gsm_type "
        "max_iterations alphainit alphamin alphamax alphainc betainit betamin "
        "betamax betainc constinit constmin constmax constinc low high\n"
        "          \tWAXS guided SAXS buffer subtraction gsm search, bounded "
        "by *min,*man, starting at *init, deltas *inc\n"
        "wgsbsggsm \toutfile logfile solutionfile bufferfile waxsfile gsm_type "
        "max_iterations gsmpercent alphamin alphamax alphaincg alphaincgsm "
        "betamin betamax betaincg betaincgsm constmin constmax constincg "
        "constincgsm low high\n"
        "          \tWAXS guided SAXS buffer subtraction grid then gsm search, "
        "bounded by *min,*man, deltas *incg (grid) *incgsm (gsm), gsmpercent "
        "is the pct range delta for gsm search from min\n"
        "wgsbsnggsm \toutfile logfile solutionfile bufferfile waxsfile grids "
        ...
        "files\n"
        "project_rebuild\n"
        "              \treads the project file and produces wiki pages & pngs "
        "describing the analysis\n"
        "project_merge \toutfile { gnom }\n"
        "              \treads project files and produces a Rg/Io summary wiki "
        "page and optionally produce gnom Dmax series\n"
        "project_1d    \twikiprefix pngsplits\n"
        "              \tbuilds wiki page and computes averages for files in "
        "1d directory\n"
        "iqqshere      \toutfile radius delta_rho minq maxq deltaq\n"
        "              \tproduces a .dat file containing the I(q) vs q curve, "
        "min,max,deltaq in 1/angstrom\n"
        ...
        "              \tfind peak, compute splines and join infile2 indep2 to "
        "infile1 and output\n"
        ...
        "              \nselects rows whose 'col' has values between min and "
        "max inclusive\n"
        "scale         \toutfile infile col mult\n"
        "              \tmultiplies column 'col' by multiplier\n"
        "ijoin         \toutfile infile1 infile2 col1 col2 col2add\n"
        "              \tinterpolates data in infile2 col2 to grid of infile1 "
        "col1 and joins col2add to outfile\n"
        "clip          \toutfile infile col min max\n"
        "              \tremove rows whose 'col' has values less or equal to "
        "min or greater than or equal to max\n"
        "info          \tinfile1\n"
        ...
        "outputfile\n"
        "json          \tjson\n"
        "              \tprocess commands from json provided as command line "
        "argument or as a json file\n"
        "              \tinternal json commands support: align, pmrun, hydro, "
        "pat, ssbond as top level key\n",
        argv[0]);
    exit(-1);
  }
  vector<QString> cmds;
  ...
  if (cmds[0].toLower() == "sbg") {
    ...
    outfile +=
        "a" + QString("%1").arg(alpha).replace(QRegExp("\\."), "") + ".dat";
  ...
  if (cmds[0].toLower() == "wgsbs") {
    if (cmds.size() != 10) {
      printf(
          "usage: %s %s outfile solutionfile bufferfile waxsfile alpha beta "
          "const low high\n",
          argv[0], argv[1]);
      exit(errorbase);
    }
    ...
    outfile += "a" + QString("%1").arg(alpha).replace(QRegExp("\\."), "") +
               "b" + QString("%1").arg(beta).replace(QRegExp("\\."), "") + "c" +
               QString("%1").arg(dconst).replace(QRegExp("\\."), "") + ".dat";
    ...
    if (!usu.read(solutionfile, "sol") || !usu.read(bufferfile, "buf") ||
        ...
        !usu.waxsguidedsaxsbuffersub(nrmsd, "out", "sol", "buf", "waxs", alpha,
                                     beta, dconst, low, high, waxscrop,
                                     waxscropinterp) ||
        !usu.write(outfile, "out")) {
      cout << usu.errormsg << endl;
      exit(errorbase - 1);
  ...
  if (cmds[0].toLower() == "wgsbsg") {
    ...
    if (!usu.read(solutionfile, "sol") || !usu.read(bufferfile, "buf") ||
        ...
        !usu.waxsguidedsaxsbuffersubgridsearch(
            ...
            "waxs", alphalow, alphahigh, alphainc, betalow, betahigh, betainc,
            dconstlow, dconsthigh, dconstinc, low, high) ||
        !usu.write(outfile, "out")) {
      cout << usu.errormsg << endl;
      exit(errorbase - 1);
    }
  ...
  if (cmds[0].toLower() == "wgsbsgsm") {
    ...
    if (!usu.read(solutionfile, "sol") || !usu.read(bufferfile, "buf") ||
        ...
        !usu.setup_wgsbs_gsm_f_df(
            "out", "sol", "buf", "waxs", alphalow, alphahigh, alphainc, betalow,
            betahigh, betainc, dconstlow, dconsthigh, dconstinc, low, high) ||
    ...
    outfile.replace(QRegExp("\\.dat$"), "");
    outfile += "a" + QString("%1").arg(alphainit).replace(QRegExp("\\."), "") +
  ...
  if (cmds[0].toLower() == "wgsbsggsm") {
    ...
    if (!usu.read(solutionfile, "sol") || !usu.read(bufferfile, "buf") ||
        ...
        !usu.waxsguidedsaxsbuffersubgridsearch(
            nrmsd, alphamin, betamin, dconstmin, log, "out", "sol", "buf",
            "waxs", alphalow, alphahigh, alphaincg, betalow, betahigh, betaincg,
            dconstlow, dconsthigh, dconstincg, low, high)) {
      cout << usu.errormsg << endl;
      exit(errorbase - 1);
    }

    cout << QString("minimum from grid nrmsd %1 alpha %2 beta %3 dconst %4\n")
                .arg(nrmsd)
                .arg(alphamin)
                .arg(betamin)
                .arg(dconstmin);
    ...
    outfile.replace(QRegExp("\\.dat$"), "");
    outfile += "a" + QString("%1").arg(alphamin).replace(QRegExp("\\."), "") +
  ...
  if (cmds[0].toLower() == "wgsbsnggsm") {
    if (cmds.size() != 24) {
      printf(
          "usage: %s %s outfile logfile solutionfile bufferfile waxsfile grids "
          "gsm_type max_iterations gsmpercent alphamin alphamax alphaincg "
          "alphaincgsm betamin betamax betaincg betaincgsm constmin constmax "
          "constincg constincgsm low high\n",
          argv[0], argv[1]);
    ...
    for (int g = 0; g < grids; g++) {
      cout << QString(
                  "ranges,inc for next grid alpha %1:%2 %3 beta %4:%5 %6 "
                  "dconst %7:%8 %9\n")
                  .arg(alphalow)
                  .arg(alphahigh)
                  .arg(alphaincg)
  ...
  if (cmds[0].toLower() == "guinier_fit") {
    if (cmds.size() != 7) {
      printf("usage: %s %s logfile infile startpos endpos minlen maxlen\n",
             argv[0], argv[1]);
      exit(errorbase);
    }
    ...
    QString log =
        "|| startpos || endpos || s_min || s_max || sRg_min || sRg_max || Rg "
        "|| Io || a || b || sigma a || sigma b || chi2 ||\n";
    double a;
    double b;
    double siga;
    double sigb;
    ...
    for (unsigned int pos = startpos; pos <= endpos; pos++) {
      for (unsigned int len = minlen; len <= maxlen; len++) {
        if (!usu.guinier_fit(log, "guinier", pos, pos + len, a, b, siga, sigb,
                             chi2, Rg, Io, smin, smax, sRgmin, sRgmax)) {
          cout << usu.errormsg << endl;
          exit(errorbase - 2);
        }
      }
  ...
  if (cmds[0].toLower() == "guinier_fit2") {
    if (cmds.size() != 7) {
      printf(
          "usage: %s %s logfile infile pointsmin pointsmax sRgmaxlimit "
          "pointweightpower\n",
          argv[0], argv[1]);
      exit(errorbase);
    }
    ...
    if (!usu.guinier_fit2(log, tag, pointsmin, pointsmax, sRgmaxlimit,
                          pointweightpower, .05, a, b, siga, sigb, chi2, Rg, Io,
                          smin, smax, sRgmin, sRgmax, beststart, bestend)) {
      cout << usu.errormsg << endl;
      exit(errorbase - 2);
    }
  ...
  if (cmds[0].toLower() == "project_init") {
    ...
    QString pngdir = QString("%1%2pngs").arg(projectdir).arg(QDir::separator());
    ...
    QString tmpdir = QString("%1%2tmp").arg(projectdir).arg(QDir::separator());

    QString logdir = QString("%1%2log").arg(projectdir).arg(QDir::separator());
    ...
    cout << QString(
                ...
                "edit %3/project and run \"us_saxs_cmd_t project_rebuild\" in "
                "the %4 directory\n")
                .arg(projectdir)
                .arg(projectname)
                .arg(projectname)
  ...
  if (cmds[0].toLower() == "project_merge") {
    ...
    QFile f("merge");
    if (!f.exists()) {
      cout << "You must first create a file called 'merge' which contains the "
              "reference MW multiplier\n"
    ...
    if (!f.open(QIODevice::ReadOnly)) {
      cout << "Could not open file 'merge' for reading.  Check permissions\n";
      exit(errorbase);
    }
    errorbase--;
  ...
  if (cmds[0].toLower() == "iqqspherefit") {
    if (cmds.size() != 12) {
      printf(
          "usage: %s %s outfile infile minradius maxradius deltaradius "
          "mindeltarho maxdeltarho deltadeltarho minq maxq normalize{y|n}\n",
          argv[0], argv[1]);
      exit(errorbase);
    }
    errorbase--;
  ...
  if (cmds[0].toLower() == "sjoin") {
    if (cmds.size() != 8) {
      printf("usage: %s %s outfile infile1 infile2 indep1 dep1 indep2 dep2\n",
             argv[0], argv[1]);
      exit(errorbase);
    }
    errorbase--;
    ...
    //                                      "spline-des.txt"
    //                                      ) ||
    //           !result_spline_join.join( result_spline_asc, result_spline_des,
    //           outfile + "-spline.txt" )
    //           )
    //       {
    ...
    if (!mc1_asc.write("", true) || !mc1_des.write("", true) ||
        ...
        !mc1_mono_joined.write("", true) || !mc2_mono_joined.write("", true)) {
      cout << mc1.errormsg << endl;
      cout << mc2.errormsg << endl;
      exit(errorbase);
    }
    ...
    if (!result_linear_asc.linear(mc1_asc, mc2_asc_mono, dep1, indep2, dep2,
                                  "linear-asc.txt") ||
        !result_linear_des.linear(mc1_des, mc2_des_mono, dep1, indep2, dep2,
                                  "linear-des.txt") ||
  ...
  if (cmds[0].toLower() == "uniquify") {
    ...
    US_Multi_Column mc(infile);
    if (!mc.read() || !mc.uniquify(col, sumcol) || !mc.write(outfile, true)) {
      cout << mc.errormsg << endl;
      exit(errorbase);
    }
    errorbase--;
  ...
  if (cmds[0].toLower() == "range") {
    ...
    US_Multi_Column mc(infile);
    if (!mc.read() || !mc.range(col, min, max) || !mc.write(outfile, true)) {
      cout << mc.errormsg << endl;
      exit(errorbase);
    }
    errorbase--;
