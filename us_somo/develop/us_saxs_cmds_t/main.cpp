...
main (int argc, char **argv)
{
  ...
  // {
  //    triangles << QString( outfiles[ i ] ).replace( QRegExp( QString( "^%1_"
  //    ).arg( inputbase ) ), "" ).replace( QRegExp( "^0*" ) , "" );
  //    one_over_triangles.push_back( triangles.back().toDouble() != 0e0 ?
  //                                  1e0 / triangles.back().toDouble() : -1e0
  //                                  );
  //    us_qdebug( QString( "%1 %2\n" ).arg( triangles.back() ).arg(
  //    one_over_triangles.back() ) );
  // }
  ...
  if (argc < 2)
    {
      printf (
          ...
          "wgsbs     \toutfile solutionfile bufferfile waxsfile alpha beta "
          "const low high"
          "          \tWAXS guided SAXS buffer subtraction\n"
          "wgsbsg    \toutfile logfile solutionfile bufferfile waxsfile "
          "alphamin alphamax alphainc betamin betamax betainc constmin "
          "constmax constinc low high\n"
          "          \tWAXS guided SAXS buffer subtraction grid search\n"
          "wgsbsgsm  \toutfile logfile solutionfile bufferfile waxsfile "
          "gsm_type max_iterations alphainit alphamin alphamax alphainc "
          "betainit betamin betamax betainc constinit constmin constmax "
          "constinc low high\n"
          "          \tWAXS guided SAXS buffer subtraction gsm search, "
          "bounded by *min,*man, starting at *init, deltas *inc\n"
          "wgsbsggsm \toutfile logfile solutionfile bufferfile waxsfile "
          "gsm_type max_iterations gsmpercent alphamin alphamax alphaincg "
          "alphaincgsm betamin betamax betaincg betaincgsm constmin constmax "
          "constincg constincgsm low high\n"
          "          \tWAXS guided SAXS buffer subtraction grid then gsm "
          "search, bounded by *min,*man, deltas *incg (grid) *incgsm (gsm), "
          "gsmpercent is the pct range delta for gsm search from min\n"
          "wgsbsnggsm \toutfile logfile solutionfile bufferfile waxsfile "
          "grids gsm_type max_iterations gsmpercent alphamin alphamax "
          "alphaincg alphaincgsm betamin betamax betaincg betaincgsm constmin "
          "constmax constincg constincgsm low high\n"
          "           \tWAXS guided SAXS buffer subtraction n grids then gsm "
          "search, bounded by *min,*man, deltas *incg (grid) *incgsm (gsm), "
          "gsmpercent is the pct range delta for subsequent grid searches and "
          "then gsm search from min\n"
          "join       \toutfile infile1 infile2 q-value\n"
          "           \tjoin two files\n"
          "guinier_plot  \toutfile infile\n"
          "              \treplace q with q*q, r with log(r), don't plot in "
          "logscale\n"
          "guinier_fit   \tlogfile infile startpos endpos minlen maxlen\n"
          "              \tcompute guinier fit\n"
          "guinier_fit2  \tlogfile infile pointsmin pointsmax sRgmaxlimit "
          "pointweightpower\n"
          "              \tcompute guinier fit\n"
          "project_init  \tprojectname\n"
          "              \tcreates a new blank project directory & template "
          "files\n"
          "project_rebuild\n"
          "              \treads the project file and produces wiki pages & "
          "pngs describing the analysis\n"
          "project_merge \toutfile { gnom }\n"
          "              \treads project files and produces a Rg/Io summary "
          "wiki page and optionally produce gnom Dmax series\n"
          "project_1d    \twikiprefix pngsplits\n"
          "              \tbuilds wiki page and computes averages for files "
          "in 1d directory\n"
          "iqqshere      \toutfile radius delta_rho minq maxq deltaq\n"
          "              \tproduces a .dat file containing the I(q) vs q "
          "curve, min,max,deltaq in 1/angstrom\n"
          "iqqspherefit  \toutfile     infile\n"
          "              \tminradius   maxradius   deltaradius\n"
          "              \tmindeltarho maxdeltarho deltadeltrarho\n"
          "              \tminq        maxq\n"
          "              \tnormalize{y|n}\n"
          "              \tbuild a collection of iqqsphere's and compute best "
          "fit & nnls distribution\n"
          "              \toutputs datafiles: outfile-radius.txt, "
          "outfile-delta_rho.txt, outfile.txt\n"
          "merge         \toutfile     infile1     weight1     infile2     "
          "weight2\n"
          ...
          "              \tfind peak, compute splines and join infile2 indep2 "
          "to infile1 and output\n"
          ...
          "              \nselects rows whose 'col' has values between min "
          "and max inclusive\n"
          "scale         \toutfile infile col mult\n"
          "              \tmultiplies column 'col' by multiplier\n"
          "ijoin         \toutfile infile1 infile2 col1 col2 col2add\n"
          "              \tinterpolates data in infile2 col2 to grid of "
          "infile1 col1 and joins col2add to outfile\n"
          "clip          \toutfile infile col min max\n"
          "              \tremove rows whose 'col' has values less or equal "
          "to min or greater than or equal to max\n"
          "info          \tinfile1\n"
          ...
          "              \tconvert listed .pdb's to a fasta formatted "
          "outputfile\n"
          "json          \tjson\n"
          "              \tprocess commands from json provided as command "
          "line argument or as a json file\n"
          "              \tinternal json commands support: align, pmrun, "
          "hydro, pat, ssbond as top level key\n",
          argv[0]);
      exit (-1);
    }
  vector<QString> cmds;
  ...
  if (cmds[0].toLower () == "avg")
    {
      if (cmds.size () < 4)
        {
          printf ("usage: %s %s outfile infile1 infile2 {infile3 etc}\n",
                  argv[0], argv[1]);
          exit (errorbase);
        }
      ...
      if (!usu.avg (outfile, tags) || !usu.write (outfile, outfile))
        {
          cout << usu.errormsg << endl;
          exit (errorbase - 2);
        }
  ...
  if (cmds[0].toLower () == "sbg")
    {
      if (cmds.size () != 5)
        {
          printf ("usage: %s %s outfile solutionfile bufferfile alpha\n",
                  argv[0], argv[1]);
          exit (errorbase);
        }
      ...
      outfile += "a" + QString ("%1").arg (alpha).replace (QRegExp ("\\."), "")
                 + ".dat";

      US_Saxs_Util usu;
      if (!usu.read (solutionfile, "sol") || !usu.read (bufferfile, "buf")
  ...
  if (cmds[0].toLower () == "scw")
    {
      if (cmds.size () != 6)
        {
          printf ("usage: %s %s outfile solutionfile bufferfile emptycellfile "
                  "alpha\n",
                  argv[0], argv[1]);
          exit (errorbase);
        }
  ...
  if (cmds[0].toLower () == "wgsbs")
    {
      if (cmds.size () != 10)
        {
          printf ("usage: %s %s outfile solutionfile bufferfile waxsfile "
                  "alpha beta const low high\n",
                  argv[0], argv[1]);
          exit (errorbase);
        }
      ...
      if (!usu.read (solutionfile, "sol") || !usu.read (bufferfile, "buf")
          ...
          || !usu.waxsguidedsaxsbuffersub (nrmsd, "out", "sol", "buf", "waxs",
                                           alpha, beta, dconst, low, high,
                                           waxscrop, waxscropinterp)
          || !usu.write (outfile, "out"))
        {
          cout << usu.errormsg << endl;
          exit (errorbase - 1);
  ...
  if (cmds[0].toLower () == "wgsbsg")
    {
      if (cmds.size () != 17)
        {
          printf ("usage: %s %s outfile logfile solutionfile bufferfile "
                  "waxsfile alphamin alphamax alphainc betamin betamax "
                  "betainc constmin constmax constinc low high",
                  argv[0], argv[1]);
          exit (errorbase);
        }
      ...
      if (!usu.read (solutionfile, "sol") || !usu.read (bufferfile, "buf")
          || !usu.read (waxsfile, "waxs")
          || !usu.waxsguidedsaxsbuffersubgridsearch (
              nrmsd, alphamin, betamin, dconstmin, log, "out", "sol", "buf",
              "waxs", alphalow, alphahigh, alphainc, betalow, betahigh,
              betainc, dconstlow, dconsthigh, dconstinc, low, high)
          || !usu.write (outfile, "out"))
        {
          cout << usu.errormsg << endl;
          exit (errorbase - 1);
  ...
  if (cmds[0].toLower () == "wgsbsgsm")
    {
      if (cmds.size () != 22)
        {
          printf ("usage: %s %s outfile logfile solutionfile bufferfile "
                  "waxsfile gsm_type max_iterations alphainit alphamin "
                  "alphamax alphainc betainit betamin betamax betainc "
                  "constinit constmin constmax constinc low high\n",
                  argv[0], argv[1]);
          exit (errorbase);
        }
      ...
      if (!usu.read (solutionfile, "sol") || !usu.read (bufferfile, "buf")
          || !usu.read (waxsfile, "waxs")
          || !usu.setup_wgsbs_gsm_f_df ("out", "sol", "buf", "waxs", alphalow,
                                        alphahigh, alphainc, betalow, betahigh,
                                        betainc, dconstlow, dconsthigh,
                                        dconstinc, low, high)
          || !usu.wgsbs_gsm (nrmsd, alphainit, betainit, dconstinit, gsm_type,
                             max_iterations))
        {
          cout << usu.errormsg << endl;
          exit (errorbase - 1);
      ...
      outfile
          += "a" + QString ("%1").arg (alphainit).replace (QRegExp ("\\."), "")
             + "b"
             + QString ("%1").arg (betainit).replace (QRegExp ("\\."), "")
             + "c"
             + QString ("%1").arg (dconstinit).replace (QRegExp ("\\."), "")
             + ".dat";
  ...
  if (cmds[0].toLower () == "wgsbsggsm")
    {
      if (cmds.size () != 23)
        {
          printf (
              "usage: %s %s outfile logfile solutionfile bufferfile waxsfile "
              "gsm_type max_iterations gsmpercent alphamin alphamax alphaincg "
              "alphaincgsm betamin betamax betaincg betaincgsm constmin "
              "constmax constincg constincgsm low high\n",
              argv[0], argv[1]);
          exit (errorbase);
        }
      ...
      if (!usu.read (solutionfile, "sol") || !usu.read (bufferfile, "buf")
          ...
          || !usu.waxsguidedsaxsbuffersubgridsearch (
              nrmsd, alphamin, betamin, dconstmin, log, "out", "sol", "buf",
              "waxs", alphalow, alphahigh, alphaincg, betalow, betahigh,
              betaincg, dconstlow, dconsthigh, dconstincg, low, high))
        {
          cout << usu.errormsg << endl;
          exit (errorbase - 1);
        }
      ...
      cout << QString (
                  "minimum from grid nrmsd %1 alpha %2 beta %3 dconst %4\n")
                  .arg (nrmsd)
                  .arg (alphamin)
                  .arg (betamin)
                  .arg (dconstmin);
      ...
      cout << QString ("ranges for gsm alpha %1:%2 beta %3:%4 dconst %5:%6\n")
                  .arg (alphalow)
                  .arg (alphahigh)
                  .arg (betalow)
                  .arg (betahigh)
      ...
      if (!usu.setup_wgsbs_gsm_f_df ("out", "sol", "buf", "waxs", alphalow,
                                     alphahigh, alphaincgsm, betalow, betahigh,
                                     betaincgsm, dconstlow, dconsthigh,
                                     dconstincgsm, low, high)
          || !usu.wgsbs_gsm (nrmsd, alphamin, betamin, dconstmin, gsm_type,
                             max_iterations))
        {
          cout << usu.errormsg << endl;
          exit (errorbase - 1);
      ...
      outfile
          += "a" + QString ("%1").arg (alphamin).replace (QRegExp ("\\."), "")
             ...
             + "c"
             + QString ("%1").arg (dconstmin).replace (QRegExp ("\\."), "")
             + ".dat";
  ...
  if (cmds[0].toLower () == "wgsbsnggsm")
    {
      if (cmds.size () != 24)
        {
          printf (
              "usage: %s %s outfile logfile solutionfile bufferfile waxsfile "
              "grids gsm_type max_iterations gsmpercent alphamin alphamax "
              "alphaincg alphaincgsm betamin betamax betaincg betaincgsm "
              "constmin constmax constincg constincgsm low high\n",
              argv[0], argv[1]);
          exit (errorbase);
        }
      ...
      double alphagridpoints
          = alphaincg ? (alphahigh - alphalow) / alphaincg : 0;
      ...
      for (int g = 0; g < grids; g++)
        {
          cout << QString ("ranges,inc for next grid alpha %1:%2 %3 beta "
                           "%4:%5 %6 dconst %7:%8 %9\n")
                      .arg (alphalow)
                      .arg (alphahigh)
                      .arg (alphaincg)
          ...
          if (!usu.waxsguidedsaxsbuffersubgridsearch (
                  nrmsd, alphamin, betamin, dconstmin, log, "out", "sol",
                  "buf", "waxs", alphalow, alphahigh, alphaincg, betalow,
                  betahigh, betaincg, dconstlow, dconsthigh, dconstincg, low,
                  high))
            {
              cout << usu.errormsg << endl;
              exit (errorbase - 2);
            }
          cout << QString ("minimum from grid %1 nrmsd %2 alpha %3 beta %4 "
                           "dconst %5\n")
                      .arg (g)
                      .arg (nrmsd)
                      .arg (alphamin)
          ...
          double alphadelta
              = (alphahigh - alphalow) * 0.5 * gsmpercent / 100e0;
          ...
          double dconstdelta
              = (dconsthigh - dconstlow) * 0.5 * gsmpercent / 100e0;
      ...
      cout << QString (
                  "ranges for next gsm alpha %1:%2 beta %3:%4 dconst %5:%6\n")
                  .arg (alphalow)
                  .arg (alphahigh)
                  .arg (betalow)
                  .arg (betahigh)
      ...
      if (max_iterations)
        {
          if (!usu.setup_wgsbs_gsm_f_df ("out", "sol", "buf", "waxs", alphalow,
                                         alphahigh, alphaincgsm, betalow,
                                         betahigh, betaincgsm, dconstlow,
                                         dconsthigh, dconstincgsm, low, high)
              || !usu.wgsbs_gsm (nrmsd, alphamin, betamin, dconstmin, gsm_type,
                                 max_iterations))
            {
              cout << usu.errormsg << endl;
  ...
  if (cmds[0].toLower () == "join")
    {
      if (cmds.size () != 5)
        {
          printf ("usage: %s %s outfile infile1 infile2 q-value\n", argv[0],
                  argv[1]);
          exit (errorbase);
        }
      ...
      if (!usu.read (infile1, infile1) || !usu.read (infile2, infile2)
  ...
  if (cmds[0].toLower () == "guinier_plot")
    {
      if (cmds.size () != 3)
        {
          printf ("usage: %s %s outfile infile\n", argv[0], argv[1]);
          exit (errorbase);
        }
      ...
      if (!usu.read (infile, infile) || !usu.guinier_plot (outfile, infile)
          || !usu.write (outfile, outfile))
        {
          cout << usu.errormsg << endl;
          exit (errorbase - 1);
  ...
  if (cmds[0].toLower () == "guinier_fit")
    {
      ...
      if (!usu.read (infile, infile) || !usu.guinier_plot ("guinier", infile))
        {
          cout << usu.errormsg << endl;
          exit (errorbase - 1);
        }
      ...
      QString log
          = "|| startpos || endpos || s_min || s_max || sRg_min || sRg_max || "
            "Rg || Io || a || b || sigma a || sigma b || chi2 ||\n";
      double a;
      double b;
      double siga;
      double sigb;
      ...
      for (unsigned int pos = startpos; pos <= endpos; pos++)
        {
          for (unsigned int len = minlen; len <= maxlen; len++)
            {
              if (!usu.guinier_fit (log, "guinier", pos, pos + len, a, b, siga,
                                    sigb, chi2, Rg, Io, smin, smax, sRgmin,
                                    sRgmax))
                {
                  cout << usu.errormsg << endl;
                  exit (errorbase - 2);
  ...
  if (cmds[0].toLower () == "guinier_fit2")
    {
      if (cmds.size () != 7)
        {
          printf ("usage: %s %s logfile infile pointsmin pointsmax "
      ...
      if (!usu.read (infile, infile) || !usu.guinier_plot (tag, infile)
      ...
      if (!usu.guinier_fit2 (log, tag, pointsmin, pointsmax, sRgmaxlimit,
                             pointweightpower, .05, a, b, siga, sigb, chi2, Rg,
                             Io, smin, smax, sRgmin, sRgmax, beststart,
                             bestend))
        {
          cout << usu.errormsg << endl;
          exit (errorbase - 2);
  ...
  if (cmds[0].toLower () == "project_init")
    {
      ...
      // make directory
      if (!projectdir.contains (QRegExp ("^(\\|/)")) // doesn't start at root
      )
        {
      ...
      QString pngdir
          = QString ("%1%2pngs").arg (projectdir).arg (QDir::separator ());

      QString sourcewavedir
          = QString ("%1%2wave").arg (projectdir).arg (QDir::separator ());

      QString computedwavedir
          = QString ("%1%2cwave").arg (projectdir).arg (QDir::separator ());

      QString tmpdir
          = QString ("%1%2tmp").arg (projectdir).arg (QDir::separator ());

      QString logdir
          = QString ("%1%2log").arg (projectdir).arg (QDir::separator ());
      ...
      if (pd.exists ())
      ...
      else
        {
          if (!pd.mkdir (projectdir))
            {
              cerr << "error: could not create directory: " << projectdir
                   << endl;
              exit (errorbase);
            }
        }
      ...
      if (pswd.exists ())
        {
          cout
              << "project source wave directory already exists, not created\n";
        }
      else
        {
          if (!pswd.mkdir (sourcewavedir))
            {
              cerr << "error: could not create directory: " << sourcewavedir
                   << endl;
              exit (errorbase);
            }
        }
      ...
      if (pcwd.exists ())
        {
          cout << "project computed wave directory already exists, not "
                  "created\n";
        }
      else
        {
          if (!pcwd.mkdir (computedwavedir))
            {
              cerr << "error: could not create directory: " << computedwavedir
                   << endl;
              exit (errorbase);
            }
        }
      ...
      ts << QString (
                ...
                "# guinierMaxQ          .05\n"
                "# the following paramaters for waxs guided saxs background "
                "subtraction default [0.98:1.00]\n"
                "# alphaMin            .98\n"
                "# alphaMax            1.00\n"
                ...
                "# waveName starts a new concentration or buffer (i.e. zero "
                "concentration) entry\n"
                "waveName            \n"
                ...
                "exposureTime        \n"
                "waveConc            \n"
                "# waveBufferName must match a previously defined wave file "
                "name\n"
                "# multiple waveBufferNames for a wave ok, they will be "
                "averaged\n"
                "waveBufferName      \n"
                "# waveEmptyName must match a previously defined wave file "
                "name\n"
                "# multiple waveEmptyNames for a wave ok, they will be "
                "averaged\n"
                "waveEmptyName       \n"
                "# manually set waveAlpha, Beta, Const (Beta,Const for waxs "
                "guided saxs background subtraction)\n"
                "# waveAlpha           \n"
                "# waveBeta            \n"
                "# waveConst           \n"
      ...
      pf.close ();
      cout << QString ("project %1 created\n"
                       "now put wave files in to %2/wave and\n"
                       "edit %3/project and run \"us_saxs_cmd_t "
                       "project_rebuild\" in the %4 directory\n")
                  .arg (projectdir)
                  .arg (projectname)
                  .arg (projectname)
                  .arg (projectdir);
  ...
  if (cmds[0].toLower () == "project_rebuild")
    {
      ...
      US_Saxs_Util usu;
      if (!usu.read_project () || !usu.build_wiki ())
        {
          cout << usu.errormsg << endl;
          exit (errorbase);
        }
  ...
  if (cmds[0].toLower () == "project_merge")
    {
      ...
      if (!f.exists ())
        {
          cout << "You must first create a file called 'merge' which contains "
                  "the reference MW multiplier\n"
      ...
      if (!f.open (QIODevice::ReadOnly))
        {
          cout << "Could not open file 'merge' for reading.  Check "
                  "permissions\n";
          exit (errorbase);
        }
      errorbase--;
      ...
      US_Saxs_Util usu;
      if (!usu.merge_projects (outfile, reference_mw_multiplier, projects,
                               gnom_run))
        {
          cout << usu.errormsg << endl;
          exit (errorbase);
  ...
  if (cmds[0].toLower () == "project_1d")
    {
      if (cmds.size () != 3)
        {
          printf ("usage: %s %s wikiprefix pngsplits\n", argv[0], argv[1]);
          exit (errorbase);
        }
      errorbase--;
  ...
  if (cmds[0].toLower () == "iqqsphere")
    {
      if (cmds.size () != 7)
        {
          printf ("usage: %s %s outfile radius delta_rho minq maxq deltaq\n",
                  argv[0], argv[1]);
          exit (errorbase);
        }
      errorbase--;
      ...
      US_Saxs_Util usu;
      if (!usu.iqq_sphere (outfile, radius, delta_rho, minq, maxq, deltaq))
        {
          cout << usu.errormsg << endl;
          exit (errorbase - 1);
        }
  ...
  if (cmds[0].toLower () == "iqqspherefit")
    {
      if (cmds.size () != 12)
        {
          printf ("usage: %s %s outfile infile minradius maxradius "
                  "deltaradius mindeltarho maxdeltarho deltadeltarho minq "
                  "maxq normalize{y|n}\n",
                  argv[0], argv[1]);
          exit (errorbase);
        }
      ...
      US_Saxs_Util usu;
      if (!usu.iqq_sphere_fit (outfile, infile, min_radius, max_radius,
                               delta_radius, min_delta_rho, max_delta_rho,
                               delta_delta_rho, min_q, max_q, do_normalize))
        {
          cout << usu.errormsg << endl;
          exit (errorbase - 1);
        }
  ...
  if (cmds[0].toLower () == "merge")
    {
      if (cmds.size () != 6)
        {
          printf ("usage: %s %s outfile infile1 weight1 infile2 weight2\n",
                  argv[0], argv[1]);
          exit (errorbase);
        }
      errorbase--;
      ...
      US_Saxs_Util usu;
      if (!usu.read (infile1, infile1) || !usu.read (infile2, infile2)
          || !usu.merge (outfile, infile1, weight1, infile2, weight2)
          || !usu.write (outfile, outfile))
        {
          cout << usu.errormsg << endl;
          exit (errorbase - 1);
  ...
  if (cmds[0].toLower () == "sjoin")
    {
      ...
      if (!mc1.split_on_peak (mc1_asc, mc1_des, indep1, dep1)
          || !mc2.split_on_peak (mc2_asc, mc2_des, indep2, dep2))
        {
          cout << mc1.errormsg << endl;
          cout << mc2.errormsg << endl;
          exit (errorbase);
      ...
      if (!mc1_mono_joined.join (mc1_asc_mono, mc1_des_mono,
                                 "mc1_mono_joined.txt")
          || !mc2_mono_joined.join (mc2_asc_mono, mc2_des_mono,
                                    "mc2_mono_joined.txt"))
        {
      ...
      //                                      "spline-des.txt"
      //                                      ) ||
      //           !result_spline_join.join( result_spline_asc,
      //           result_spline_des, outfile + "-spline.txt" )
      //           )
      //       {
      ...
      //                                            "quadratic-des.txt"
      //                                            ) ||
      //           !result_quadratic_join.join( result_quadratic_asc,
      //           result_quadratic_des, outfile + "-quadratic.txt" )
      //           )
      //       {
      ...
      if (!mc1_asc.write ("", true) || !mc1_des.write ("", true)
          || !mc2_asc.write ("", true) || !mc2_des.write ("", true)
          || !mc1_asc_mono.write ("", true) || !mc1_des_mono.write ("", true)
          || !mc2_asc_mono.write ("", true) || !mc2_des_mono.write ("", true)
      ...
      if (!result_linear_asc.linear (mc1_asc, mc2_asc_mono, dep1, indep2, dep2,
                                     "linear-asc.txt")
          || !result_linear_des.linear (mc1_des, mc2_des_mono, dep1, indep2,
                                        dep2, "linear-des.txt")
          || !result_linear_join.join (result_linear_asc, result_linear_des,
                                       outfile + "-linear.txt"))
        {
  ...
  if (cmds[0].toLower () == "smooth")
    {
      if (cmds.size () != 5)
        {
          printf ("usage: %s %s outfile infile col points\n", argv[0],
                  argv[1]);
          exit (errorbase);
        }
      errorbase--;
      ...
      if (!mc.read () || !mc.smooth (col, points) || !mc.write (outfile, true))
        {
  ...
  if (cmds[0].toLower () == "repeak")
    {
      if (cmds.size () != 4)
        {
          printf ("usage: %s %s outfile infile col\n", argv[0], argv[1]);
          exit (errorbase);
        }
      errorbase--;
      ...
      US_Multi_Column mc (infile);
      if (!mc.read () || !mc.repeak (col) || !mc.write (outfile, true))
        {
          cout << mc.errormsg << endl;
          exit (errorbase);
        }
  ...
  if (cmds[0].toLower () == "sort")
    {
      ...
      US_Multi_Column mc (infile);
      if (!mc.read () || !mc.sort (col) || !mc.write (outfile, true))
        {
          cout << mc.errormsg << endl;
          exit (errorbase);
        }
  ...
  if (cmds[0].toLower () == "reverse")
    {
      ...
      US_Multi_Column mc (infile);
      if (!mc.read () || !mc.reverse () || !mc.write (outfile, true))
        {
          cout << mc.errormsg << endl;
          exit (errorbase);
        }
  ...
  if (cmds[0].toLower () == "uniquify")
    {
      if (cmds.size () != 5)
        {
          printf ("usage: %s %s outfile infile col sumcol\n", argv[0],
                  argv[1]);
          exit (errorbase);
        }
      errorbase--;
      ...
      US_Multi_Column mc (infile);
      if (!mc.read () || !mc.uniquify (col, sumcol)
  ...
  if (cmds[0].toLower () == "range")
    {
      if (cmds.size () != 6)
        {
          printf ("usage: %s %s outfile infile col min max\n", argv[0],
                  argv[1]);
          exit (errorbase);
        }
      errorbase--;
      ...
      US_Multi_Column mc (infile);
      if (!mc.read () || !mc.range (col, min, max)
          || !mc.write (outfile, true))
        {
          cout << mc.errormsg << endl;
          exit (errorbase);
  ...
  if (cmds[0].toLower () == "scale")
    {
      if (cmds.size () != 5)
        {
          printf ("usage: %s %s outfile infile col mult\n", argv[0], argv[1]);
          exit (errorbase);
        }
      errorbase--;
      ...
      US_Multi_Column mc (infile);
      if (!mc.read () || !mc.scale (col, mult) || !mc.write (outfile, true))
        {
          cout << mc.errormsg << endl;
          exit (errorbase);
        }
  ...
  if (cmds[0].toLower () == "ijoin")
    {
      if (cmds.size () != 7)
        {
          printf ("usage: %s %s outfile infile1 infile2 col1 col2 col2add\n",
                  argv[0], argv[1]);
          exit (errorbase);
        }
      errorbase--;
  ...
  if (cmds[0].toLower () == "clip")
    {
      ...
      US_Multi_Column mc (infile);
      if (!mc.read () || !mc.clip (col, min, max) || !mc.write (outfile, true))
        {
          cout << mc.errormsg << endl;
          exit (errorbase);
        }
  ...
  if (cmds[0].toLower () == "fasta")
    {
      if (cmds.size () <= 3)
        {
          printf ("usage: %s %s max_line_length outfile pdb1 {pdb2 ... }\n",
                  argv[0], argv[1]);
          exit (errorbase);
        }
      errorbase--;
  ...
  if (cmds[0].toLower () == "json")
    {
      if (cmds.size () != 2)
        {
          printf ("{\"errors\":\"%s %s incorrect number of arguments\"}\n",
                  argv[0], argv[1]);
          exit (errorbase);
        }
      errorbase--;
