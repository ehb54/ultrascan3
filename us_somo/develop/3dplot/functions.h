...
class C_s_k : public Function {
 public:
  ...
  C_s_k(SA2d_control_variables *tmp_sa2d_ctrl_vars,
        vector<struct element_3D> tmp_solutes, SurfacePlot &pw)
      : Function(pw) {
    sa2d_ctrl_vars = tmp_sa2d_ctrl_vars;
    solutes = tmp_solutes;
    setMesh((*sa2d_ctrl_vars).meshx, (*sa2d_ctrl_vars).meshy);
    setDomain((*sa2d_ctrl_vars).minx, (*sa2d_ctrl_vars).maxx,
  ...
  double operator()(double x, double y) {
    ...
    for (i = 0; i < solutes.size(); i++) {
      dist = sqrt(((x - solutes[i].x) * (x - solutes[i].x)) +
                  ((y - solutes[i].y) * (y - solutes[i].y)));
      if (dist <= (*sa2d_ctrl_vars).beta) {
        retval +=
            solutes[i].z * pow(cos(dist * M_PI / ((*sa2d_ctrl_vars).beta * 2)),
                               (*sa2d_ctrl_vars).alpha);
      }
    }
