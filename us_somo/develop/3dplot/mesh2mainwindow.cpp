    ...
    : Mesh2MainWindowBase (parent, name, f)
{
  ...
  */
  ...
  connect (backgroundcolor, SIGNAL (activated ()), this,
           SLOT (pickBgColor ()));
  ...
  connect (numbercolor, SIGNAL (activated ()), this,
           SLOT (pickNumberColor ()));
  ...
  connect (animation, SIGNAL (toggled (bool)), this,
  ...
  connect (autoscale, SIGNAL (toggled (bool)), this,
...
void
Mesh2MainWindow::setParameters (QString xtitle, QString ytitle, QString ztitle,
                                double **data,
...
void
Mesh2MainWindow::setParameters (QString title_3d, QString xtitle,
                                QString ytitle, QString ztitle, double **data,
                                SA2d_control_variables *sa2d_ctrl_vars)
{
  this->sa2d_ctrl_vars = sa2d_ctrl_vars;
  this->xtitle = xtitle;
...
void
Mesh2MainWindow::setParameters (QString xtitle, QString ytitle,
...
void
Mesh2MainWindow::closeEvent (QCloseEvent *e)
{
  *widget_flag = false;
  e->accept ();
}
...
void
Mesh2MainWindow::createFunction ()
{
  C_s_k *c_s_k = (C_s_k *)0;
  dataWidget->makeCurrent ();
  ...
  dataWidget->coordinates ()->axes[Y3].setLabelString (QString (ytitle));
  dataWidget->coordinates ()->axes[Y4].setLabelString (QString (ytitle));
  dataWidget->coordinates ()->axes[Z1].setLabelString (
      QString ("Concentration"));
  dataWidget->coordinates ()->axes[Z2].setLabelString (
      QString ("Concentration"));
  dataWidget->coordinates ()->axes[Z3].setLabelString (
      QString ("Concentration"));
  dataWidget->coordinates ()->axes[Z4].setLabelString (
      QString ("Concentration"));
  pickCoordSystem (activeCoordSystem);
...
void
Mesh2MainWindow::pickCoordSystem (QAction *action)
{
  ...
  if (!dataWidget->hasData ())
    {
      double l = 0.6;
      dataWidget->createCoordinateSystem (Triple (-l, -l, -l),
                                          Triple (l, l, l));
      for (unsigned i = 0; i != dataWidget->coordinates ()->axes.size (); ++i)
        {
          dataWidget->coordinates ()->axes[i].setMajors (4);
...
void
Mesh2MainWindow::pickPlotStyle (QAction *action)
{
  if (!action || !dataWidget)
    return;
...
void
Mesh2MainWindow::setLeftGrid (bool b)
{
  setGrid (Qwt3D::LEFT, b);
}
void
Mesh2MainWindow::setRightGrid (bool b)
{
  setGrid (Qwt3D::RIGHT, b);
}
void
Mesh2MainWindow::setCeilGrid (bool b)
{
  setGrid (Qwt3D::CEIL, b);
}
void
Mesh2MainWindow::setFloorGrid (bool b)
{
  setGrid (Qwt3D::FLOOR, b);
}
void
Mesh2MainWindow::setFrontGrid (bool b)
{
  setGrid (Qwt3D::FRONT, b);
}
void
Mesh2MainWindow::setBackGrid (bool b)
{
  setGrid (Qwt3D::BACK, b);
}

void
Mesh2MainWindow::setGrid (Qwt3D::SIDE s, bool b)
{
  if (!dataWidget)
    return;
...
void
Mesh2MainWindow::pickNumberColor ()
{
...
void
Mesh2MainWindow::pickLabelColor ()
{
  QColor c = QColorDialog::getColor (Qt::white, this);
  if (!c.isValid ())
    return;
...
void
Mesh2MainWindow::pickTitleColor ()
{
  QColor c = QColorDialog::getColor (Qt::white, this);
  if (!c.isValid ())
    return;
...
void
Mesh2MainWindow::pickDataColor ()
{
  datacolordlg_->show ();
}

void
Mesh2MainWindow::pickLighting ()
{
  lightingdlg_->show ();
}

void
Mesh2MainWindow::adaptDataColors (const QString &fileName)
{
  ColorVector cv;
...
void
Mesh2MainWindow::pickNumberFont ()
{
  bool ok;
...
void
Mesh2MainWindow::dumpImage (QString filename, QString imagetype)
{
  if (!dataWidget)
    return;
...
*/
void
Mesh2MainWindow::toggleAnimation (bool val)
{
  if (val)
    {
...
void
Mesh2MainWindow::openMesh ()
{
  QString data (
      Q3FileDialog::getOpenFileName ("../../data", "nodes (*.nod)", this));
  QString edges (Q3FileDialog::getOpenFileName (
      "../../data", "connectivities (*.cel)", this));
...
void
Mesh2MainWindow::setFileType (QString const &name)
{
  filetype_ = name;
}

void
Mesh2MainWindow::enableLighting (bool val)
{
