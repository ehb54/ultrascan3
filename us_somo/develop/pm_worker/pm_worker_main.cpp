        ...
        case PM_NEW_PM:
          {
            ...
            if (MPI_SUCCESS
              ...
              {
                cout << myrank
                     << ":MPI PM_NEW_PM Receive failed in pm_mpi_worker()\n"
                     << flush;
                MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
                exit (errorno - myrank);
              }
            ...
            pm = new PM_WORKER (msg.grid_conversion_factor, msg.max_dimension,
                                msg.max_harmonics, F, q, I, e,
                                msg.max_mem_in_MB, 0, true);
          }
          break;
        ...
        case PM_NEW_GRID_SIZE:
          {
            ...
            if (!pm)
              {
                cout << myrank
                     << ":MPI PM_NEW_GRID_SIZE Receive called before "
        ...
        case PM_CALC_FITNESS:
          {
            ...
            if (MPI_SUCCESS
              ...
              {
                cout << myrank
                     << ":MPI PM_CALC_FITNESS Receive failed in "
                        "pm_mpi_worker()\n"
                     << flush;
                MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
                exit (errorno - myrank);
            ...
            if (!pm->create_model (params, model)
              ...
              {
                cout << myrank
                     << ":MPI PM_CALC_FITNESS error in pm_mpi_worker()\n"
                     << pm->error_msg << endl
                     << flush;
                MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
                exit (errorno - myrank);
            ...
            if (MPI_SUCCESS
              ...
              {
                cout << myrank
                     << ":MPI PM_CALC_FITNESS Send 1 failed in "
                        "pm_mpi_worker()\n"
                     << flush;
                MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
                exit (errorno - myrank);
            ...
            if (MPI_SUCCESS
              ...
              {
                cout << myrank
                     << ":MPI PM_CALC_FITNESS Send 2 failed in "
                        "pm_mpi_worker()\n"
                     << flush;
                MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
                exit (errorno - myrank);
