...
bool PM_WORKER::compute_I(set<pm_point> &model, vector<double> &I_result) {
  if (!model.size()) {
  ...
  if (use_CYJ) {
    ...
    if (model.size() > max_beads_CA) {
      cout << "switching to CYJ mode" << endl;
      pcdata.clear();
      use_CYJ = true;
      return compute_CYJ_I(model, I_result);
    } else {
...
double PM_WORKER::model_fit(vector<double> &params, set<pm_point> &model,
...
bool PM_WORKER::compute_CYJ_I(set<pm_point> &model, vector<double> &I_result) {
  ...
  for (set<pm_point>::iterator it = model.begin(); it != model.end(); it++) {
    if (!pdata.count(*it)) {
      ...
      if (tmp_pm_data.rtp[0] == 0e0) {
        ...
        if (it->x[0] == 0 && it->x[1] == 0) {
          tmp_pm_data.rtp[2] = 0e0;
        } else {
          if (it->x[0] < 0) {
            tmp_pm_data.rtp[2] =
                M_PI -
                asin(tmp_pm_data.x[1] /
                     sqrt((double)(tmp_pm_data.x[0] * tmp_pm_data.x[0] +
                                   tmp_pm_data.x[1] * tmp_pm_data.x[1])));
          } else {
            if (it->x[1] < 0) {
              tmp_pm_data.rtp[2] =
                  M_2PI + asin(tmp_pm_data.x[1] /
                               sqrt(tmp_pm_data.x[0] * tmp_pm_data.x[0] +
                                    tmp_pm_data.x[1] * tmp_pm_data.x[1]));
            } else {
              tmp_pm_data.rtp[2] = asin(
                  tmp_pm_data.x[1] / sqrt(tmp_pm_data.x[0] * tmp_pm_data.x[0] +
                                          tmp_pm_data.x[1] * tmp_pm_data.x[1]));
            }
          }
        }
      ...
      */
...
#if defined(ALT_SH)
      if (!sh::alt_conj_sh(max_harmonics, tmp_pm_data->rtp[1],
                           tmp_pm_data->rtp[2], Yp)) {
      ...
      for (unsigned int l = 0; l <= max_harmonics; ++l) {
        for (int m = -(int)l; m <= (int)l; ++m) {
          ...
          if (!sh::conj_spherical_harmonic(l, m, tmp_pm_data->rtp[1],
                                           tmp_pm_data->rtp[2], result)) {
            error_msg = "sh::spherical_harmonic failed";
            return false;
          }
          (*Yp) = result;
...
bool PM_WORKER::compute_delta_I(set<pm_point> &model, set<pm_point> &prev_model,
  ...
  set_difference(model.begin(), model.end(), prev_model.begin(),
                 prev_model.end(), inserter(model_adds, model_adds.end()));

  // and what we need to subtract
  set_difference(prev_model.begin(), prev_model.end(), model.begin(),
                 model.end(), inserter(model_subtracts, model_subtracts.end()));
  ...
  for (set<pm_point>::iterator it = model_adds.begin(); it != model_adds.end();
       it++) {
    if (!pdata.count(*it)) {
      tmp_pm_data.x[0] = grid_conversion_factor * (double)it->x[0];
      tmp_pm_data.x[1] = grid_conversion_factor * (double)it->x[1];
  ...
  for (set<pm_point>::iterator it = model_subtracts.begin();
       it != model_subtracts.end(); it++) {
    if (!pdata.count(*it)) {
      cerr << "pdata does not contain: " << it->x[0] << it->x[1] << it->x[2]
           << endl;
      exit(-1);
    }
...
#if defined(ALT_SPHBES)
        if (!shs->shs_compute_sphbes(q[j] * v_pdata[i]->rtp[0],
                                     &(v_pdata[i]->J[J_ofs]))) {
          error_msg = "nr::alt_shbes failed";
          return false;
        }
#else
        for (unsigned int l = 0; l <= max_harmonics; ++l) {
          if (!nr::sphbes(l, q[j] * v_pdata[i]->rtp[0],
                          v_pdata[i]->J[J_ofs + l])) {
            error_msg = "nr::shbes failed";
            return false;
          }
...
bool PM_WORKER::compute_CA_I(set<pm_point> &model, vector<double> &I_result) {
  // model should already be centered
    ...
    if (!pcdata.count(*it)) {
      ...
      if (pm_datap->rtp[0] == 0e0) {
        ...
        if (it->x[0] == 0 && it->x[1] == 0) {
          ...
          if (it->x[0] < 0) {
            pm_datap->rtp[2] =
                M_PI - asin(pm_datap->x[1] /
                            sqrt((double)(pm_datap->x[0] * pm_datap->x[0] +
                                          pm_datap->x[1] * pm_datap->x[1])));
          } else {
            if (it->x[1] < 0) {
              pm_datap->rtp[2] =
                  M_2PI +
                  asin(pm_datap->x[1] / sqrt(pm_datap->x[0] * pm_datap->x[0] +
                                             pm_datap->x[1] * pm_datap->x[1]));
            } else {
              pm_datap->rtp[2] =
                  asin(pm_datap->x[1] / sqrt(pm_datap->x[0] * pm_datap->x[0] +
...
#if defined(ALT_SH)
      if (!sh::alt_conj_sh(max_harmonics, pm_datap->rtp[1], pm_datap->rtp[2],
                           Yp)) {
        error_msg = "sh::spherical_harmonic failed";
        return false;
      }
      ...
      for (unsigned int l = 0; l <= max_harmonics; ++l) {
        for (int m = -(int)l; m <= (int)l; ++m) {
          if (!sh::conj_spherical_harmonic(l, m, pm_datap->rtp[1],
                                           pm_datap->rtp[2], tmp_cd)) {
            error_msg = "sh::spherical_harmonic failed";
            return false;
          }
          (*Yp) = tmp_cd;
...
double PM_WORKER::fitness2(vector<double> &I_result) {
  double chi2 = 0e0;
  double tmp;
