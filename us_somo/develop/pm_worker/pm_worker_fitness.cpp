...
bool
PM_WORKER::compute_I (set<pm_point> &model, vector<double> &I_result)
{
  if (!model.size ())
    {
...
double
PM_WORKER::model_fit (vector<double> &params, set<pm_point> &model,
...
bool
PM_WORKER::compute_CYJ_I (set<pm_point> &model, vector<double> &I_result)
{
...
bool
PM_WORKER::compute_delta_I (set<pm_point> &model, set<pm_point> &prev_model,
                            ...
                            vector<double> &I_result)
{
  ...
  for (set<pm_point>::iterator it = model_subtracts.begin ();
       it != model_subtracts.end (); it++)
    {
      if (!pdata.count (*it))
        {
          cerr << "pdata does not contain: " << it->x[0] << it->x[1]
               << it->x[2] << endl;
          exit (-1);
        }
...
bool
PM_WORKER::compute_CA_I (set<pm_point> &model, vector<double> &I_result)
{
  // model should already be centered
...
double
PM_WORKER::fitness2 (vector<double> &I_result)
{
  double chi2 = 0e0;
  double tmp;
