...
int
US_Hydrodyn::pdb_hydrate_for_saxs (bool quiet)
{
  QString error_msg;
  if (!load_rotamer (error_msg))
    {
      QMessageBox::warning (
          this, us_tr ("Error reading Hydrated Rotamer file"), error_msg);
      return -1;
    }
          ...
          if (!pdb_asa_for_saxs_hydrate ())
            {
              ...
              if (!compute_to_hydrate_dihedrals (error_msg))
                {
                  QMessageBox::warning (
                      this, us_tr ("Error computing to-hydrate dihedrals"),
                      error_msg);
                  any_errors = true;
                }
#if defined(DEBUG_TO_HYDRATE_DIHEDRAL)
              ...
              if (!compute_best_fit_rotamer (error_msg))
                {
                  QMessageBox::warning (
                      this, us_tr ("Error finding best fit rotamer"),
                      error_msg);
                  any_errors = true;
                }
              ...
              if (!setup_pointmap_rotamers (error_msg))
                {
                  QMessageBox::warning (
                      this, us_tr ("Error setting up pointmaps"), error_msg);
                  any_errors = true;
                }
              ...
              if (saxs_options.alt_hydration)
                {
                  editor_msg ("blue",
                              us_tr ("NOTICE: using alternate hydration "
                                     "(experimental) method"));
                  if (!compute_waters_to_add_alt (error_msg, quiet))
                    {
                      QMessageBox::warning (
                          this, us_tr ("Error trying to add waters"),
                          error_msg);
                      any_errors = true;
                    }
                }
              ...
              if (!write_pdb_with_waters (error_msg, quiet))
                {
                  QMessageBox::warning (
                      this, us_tr ("Error trying to write pdb with waters"),
                      error_msg);
                  any_errors = true;
                }
              if (!any_errors && !screen_pdb (last_hydrated_pdb_name, !quiet))
                {
                  QMessageBox::warning (
                      this, us_tr ("Error trying to reload hydrated pdb"),
                      error_msg);
                  any_errors = true;
                }
              if (!any_errors)
      ...
      if (stopFlag)
        {
          ...
          bd_anaflex_enables (
              ((browflex && browflex->state () == QProcess::Running)
...
int
US_Hydrodyn::pdb_asa_for_saxs_hydrate ()
{
  ...
  if (asa.method == 1)
    {
      ...
      int retval = us_hydrodyn_asab1_main (active_atoms, &asa, &results, false,
                                           progress, editor, this);
      ...
      if (retval)
        {
          ...
          switch (retval)
            ...
            case US_HYDRODYN_ASAB1_ERR_MEMORY_ALLOC:
              {
                printError (
                    "US_HYDRODYN_ASAB1 encountered a memory allocation error");
                        ...
                        && !(misc.pb_rule_on && this_atom->chain == 0
                             ...
                             && !broken_chain_head.count (
                                 QString ("%1|%2")
                                     ...
                                     .arg (this_atom->resName)))))
                  {
                    if (advanced_config.debug_1 || advanced_config.debug_2)
                      {
                        printf ("pass 2 active %s %s %d new bead chain %d\n",
                                this_atom->name.toLatin1 ().data (),
                                this_atom->resName.toLatin1 ().data (),
                                this_atom->serial, this_atom->chain);
                        fflush (stdout);
                      }
                        ...
                        if (sidechain_N && this_atom->chain == 1)
                          {
                            ...
                            if (advanced_config.debug_1
                              ...
                              {
                                printf (
                                    "adding sidechain N %f %f to this_atom\n",
                                    sidechain_N->asa, sidechain_N->mw);
                              }
                ...
                if (advanced_config.debug_1)
                  {
                    printf ("atom %s %s p_atom.hybrid.mw %f atom.mw %f\n",
                            this_atom->name.toLatin1 ().data (),
                            this_atom->resName.toLatin1 ().data (),
                            this_atom->p_atom->hybrid.mw, this_atom->mw);
                  }
                // accum
                if (!create_beads_normally || this_atom->bead_positioner)
                  {
                    if (advanced_config.debug_3)
                      {
                        printf ("adding cog from %d to %d mw %f totmw %f "
                                "(this pos [%f,%f,%f], org pos [%f,%f,%f])\n",
                                this_atom->serial, use_atom->serial,
                                this_atom->mw, use_atom->bead_cog_mw,
      ...
      if (advanced_config.debug_3)
        {
          printf ("molecular cog [%f,%f,%f] mw %f\n", molecular_cog[0],
                  molecular_cog[1], molecular_cog[2], molecular_mw);
        }
    }
  else
    {
            ...
            if (advanced_config.debug_1)
              {
                printf ("pass 2b active %d is_bead %d chain %d atom %s %s %d "
                        "pm %d\n",
                        this_atom->active, this_atom->is_bead,
                        this_atom->chain, this_atom->name.toLatin1 ().data (),
                        this_atom->resName.toLatin1 ().data (),
                        this_atom->serial, this_atom->placing_method);
                fflush (stdout);
              }
            ...
            if (placed_N1
                                               ...
                                               .arg (this_atom->resName)))
              {
                ...
                if (advanced_config.debug_1)
                  {
                    printf (
                        "placed N1, found break, turned off placed N1 %s %s\n",
                        this_atom->resName.toLatin1 ().data (),
            ...
            if (this_atom->active && this_atom->is_bead
                && this_atom->chain == 0)
              {
                if (advanced_config.debug_1)
                  {
                    printf (
                        "pass 2b active, bead, chain == 0 %s %s %d pm %d\n",
                        this_atom->name.toLatin1 ().data (),
                        this_atom->resName.toLatin1 ().data (),
                        this_atom->serial, this_atom->placing_method);
                    fflush (stdout);
                ...
                if (multi_residue_map["PBR-NO-OXT"].size () == 1)
                  {
                    ...
                    if (advanced_config.debug_1)
                      {
                        printf ("pass 2b broken end adjustment %s %s %s new "
                                "mw %.4f vol %.4f\n",
                                this_atom->name.toLatin1 ().data (),
                                this_atom->resName.toLatin1 ().data (),
                                this_atom->resSeq.toLatin1 ().data (),
                ...
                else
                  {
                    ...
                    editor->append (
                        "Chain has broken end and PBR-NO-OXT isn't uniquely "
                    ...
                    .arg (last_main_chain_bead->resName)))
          {
            ...
            if (multi_residue_map["PBR-NO-OXT"].size () == 1)
            ...
            else
              {
                QColor save_color = editor->textColor ();
                editor->setTextColor ("red");
                editor->append ("Chain has no terminating OXT and PBR-NO-OXT "
            ...
            if (this_atom->active && this_atom->is_bead)
              {
                ...
                this_atom->visibility
                    = (this_atom->bead_asa >= asa.hydrate_threshold);
...
bool
US_Hydrodyn::compute_to_hydrate_dihedrals (QString &error_msg)
{
  ...
  for (map<QString, map<QString, point> >::iterator it = to_hydrate.begin ();
       it != to_hydrate.end (); it++)
    {
      if (rx_expand_mapkey.indexIn (it->first) == -1)
        {
          error_msg
              = QString (us_tr ("Internal error: could not expand mapkey %1"))
                    .arg (it->first);
          return false;
        }
      ...
      if (!dihedral_atoms.count (resName))
        {
          error_msg
              = QString (us_tr ("No dihedral group found for residue %1."))
                    .arg (resName);
          return false;
        }
#if defined(DEBUG_TO_HYDRATE_DIHEDRAL)
      ...
      for (unsigned int i = 0; i < dihedral_atoms[resName].size (); i++)
        {
          if (dihedral_atoms[resName][i].size () != 4)
            {
              error_msg = QString ("Dihedral table size incorrect, should be "
                                   "4 but is %1 for %2th dihedral group of %3")
          ...
          for (unsigned int j = 0; j < dihedral_atoms[resName][i].size (); j++)
            {
              if (!(it->second.count (dihedral_atoms[resName][i][j])))
                {
                  error_msg = QString ("Side chain %1 is missing required "
                                       "dihedral atom %2")
                                  .arg (it->first)
...
QString
US_Hydrodyn::list_to_hydrate_dihedrals ()
{
  QString out = "";
...
QString
US_Hydrodyn::list_to_hydrate (bool coords)
{
...
US_Hydrodyn::view_exposed ()
{
              ...
              if (this_atom->chain == 1)
                {
                  ...
                  use_color[mapkey] = rotamers.count (this_atom->resName)
                                          ? "green"
                                          : "yellow";
                }
              if (pointmap_atoms.count (this_atom->resName))
                {
                  ...
                  use_color[mapkey] = pointmap_atoms.count (this_atom->resName)
                                          ? "cyan"
                                          : "yellow";
                }
            }
  ...
  QString fname = somo_dir + SLASH + "tmp" + SLASH
                  + QFileInfo (pdb_file).baseName () + ".spt";
  QFile f (fname);
...
bool
US_Hydrodyn::load_rotamer (QString &error_msg)
{
  puts ("load_rotamer");
  if (!rotamer_changed)
    {
  ...
  if (!f.exists ())
    {
      error_msg = us_tr (
          "Hydrated Rotamer file not found:\n"
  ...
  for (unsigned int i = 0; i < (unsigned int)qsl.size (); i++)
    {
      ...
      if (qsl_line[0] == "multiple-rotate:")
        {
          if (in_rotamer)
            {
              error_msg
                  = QString (us_tr ("Error in hydrated rotamer file line %1.  "
                                    "multiple-rotate token can not occur "
                                    "within a rotamer definition"))
                        .arg (i + 1);
              return false;
            }
          ...
          if (rotated_rotamers.count (name))
            {
              error_msg = QString (us_tr ("Error in hydrated rotamer file "
                                          "line %1.  multiple-rotate: %2 "
                                          "previously multiple-rotate'd"))
                              .arg (i + 1)
                              .arg (name);
              return false;
            }
          ...
          if (rotate <= 1)
            {
              error_msg = QString (us_tr ("Error in hydrated rotamer file "
                                          "line %1.  multiple-rotate: rotate "
                                          "count %2, must be 2 or greater"))
                              .arg (i + 1)
                              .arg (rotate);
              return false;
            }
          ...
          //          {
          //             error_msg =
          //                QString( us_tr( "Error in hydrated rotamer file
          //                line %1.  multiple-rotate: difference in water
          //                count (%2) with reference rotamer water count (%3)"
          //                ) ) .arg( i + 1 ) .arg( water_count ) .arg(
          //                rotamers[ residue ][ 0
          //                ].water_positioning_atoms.size() );
          //             return false;
          //          }
          ...
          for (unsigned int water_number = 0; water_number < water_count;
               water_number++)
            {
              ...
              for (unsigned int j = 0;
                   j < (unsigned int)rotamers[residue].size (); j++)
                {
              ...
              if (!found_match)
                {
                  error_msg
                      = QString (us_tr ("Error in hydrated rotamer file line "
              ...
              for (unsigned int j = 0; j < 2; j++)
                {
                  ...
                  for (unsigned int k = 0;
                       k < (unsigned int)ref_rotamer.side_chain.size (); k++)
                    {
                  ...
                  if (!found_sc_atom)
                    {
                      error_msg
                          = QString (
                                us_tr ("Error in hydrated rotamer file line "
                                       "%1.  multiple-rotate: residue: %2 "
                                       "found, atom %3 missing"))
                                .arg (i + 1)
                                .arg (residue)
                                .arg (atoms[j]);
                      return false;
              ...
              if ((unsigned int)ref_rotamer.waters.size () <= water_number)
                {
                  error_msg
                      = QString (us_tr ("Error in hydrated rotamer file line "
                                        "%1.  multiple-rotate: residue: %2 "
                                        "found reference rotamer, but it has "
                                        "%3 waters, less than %4 requested"))
                            .arg (i + 1)
                            .arg (residue)
              ...
              cout << QString (
                          us_tr ("new: Angle %1,%2,%3 = %4 or %5 degrees\n"))
                          .arg (atoms[0])
                          .arg (atoms[1])
                          .arg (atoms[2])
                          .arg (na)
              ...
              for (unsigned int j = 0;
                   j < (unsigned int)ref_rotamer.side_chain.size (); j++)
                {
                  ref_rotamer_p.push_back (
                      ref_rotamer.side_chain[j].coordinate);
                }
              for (unsigned int j = 0;
                   j < (unsigned int)ref_rotamer.waters.size (); j++)
                {
          ...
          for (unsigned int jj = 0;
               jj < (unsigned int)rotated_rotamers[name].size (); jj++)
            {
              out += QString ("REMARK  water %1\n").arg (jj + 1);
              for (unsigned int j = 0;
                   j < (unsigned int)rotated_rotamers[name][jj].size (); j++)
                {
                  out += QString ("MODEL     %1\n").arg (j + 1);
                  ...
                  for (unsigned int k = 0;
                       k < (unsigned int)rotated_rotamers[name][jj][j]
                               .side_chain.size ();
                       k++)
                    {
                      out += QString ("").sprintf (
              ...
              if (points < 3)
                {
                  error_msg
                      = QString (us_tr ("Error in hydrated rotamer file line "
                                        "%1.  Invalid number of points for a "
                                        "map on pointmap line"))
                            .arg (i + 1);
                  return false;
                }
                  ...
                  if (!qsl_line.size ())
                    {
                      error_msg
                          = QString (us_tr ("Error in hydrated rotamer file "
                                            "line %1.  Invalid number of "
                                            "tokens on pointmap line"))
                                .arg (i + 1);
                      return false;
                    }
              ...
              for (unsigned int j = 0; j < (unsigned int)AAs.size (); j++)
                {
                  ...
                  if (pointmap_atoms.count (AAs[j]))
                    {
                      error_msg
                          = QString (
                                us_tr (
                                    "Error in hydrated rotamer file line %1. "
                                    "Pointmap defined for AA, but preexisting "
                                    "pointmap defined for Amino Acid residue "
                                    "%2."))
                                .arg (i + 1)
                                .arg (AAs[j]);
                      return false;
                  ...
                  pointmap_atoms_ref_residue[AAs[j]]
                      = pointmap_atoms_ref_residue[res];
                }
            }
          continue;
        }
          ...
          if (four_atoms.size ())
            {
              error_msg
                  = QString (
                        us_tr ("Error in hydrated rotamer file line %1.  "
      ...
      if (qsl_line[0] != "ATOM")
        {
          error_msg = QString (us_tr ("Error in hydrated rotamer file line "
                                      "%1.  Unrecognized token %2"))
                          .arg (i + 1)
                          .arg (qsl_line[0]);
          return false;
        }
  ...
  for (map<QString, vector<rotamer> >::iterator it = rotamers.begin ();
       it != rotamers.end (); it++)
    {
      for (unsigned int i = 0; i < (unsigned int)it->second.size () && i < 1;
           i++)
        {
          if (hydrate_count.count (it->first))
            {
...
bool
US_Hydrodyn::compute_rotamer_dihedrals (QString &error_msg)
{
      ...
      for (unsigned int i = 0; i < (unsigned int)it->second.size (); i++)
        {
          ...
          for (unsigned int j = 0;
               j < (unsigned int)dihedral_atoms[it->first].size (); j++)
            {
#if defined(DEBUG_DIHEDRAL)
              ...
              for (unsigned int k = 0;
                   k < (unsigned int)dihedral_atoms[it->first][j].size (); k++)
                {
#if defined(DEBUG_DIHEDRAL)
                  cout << QString ("  %1: ").arg (
...
QString
US_Hydrodyn::list_rotamer_dihedrals ()
{
  QString out;
      ...
      for (unsigned int i = 0; i < (unsigned int)it->second.size (); i++)
        {
          ...
          for (unsigned int j = 0;
               j < (unsigned int)it->second[i].dihedral_angles.size (); j++)
            {
      ...
      for (unsigned int i = 0; i < (unsigned int)it->second.size (); i++)
        {
          ...
          if (pointmap_atoms_ref_residue.count (it->first)
            ...
            {
              for (unsigned int j = 0; j < (unsigned int)it->second[i].size ();
                   j++)
                {
                  out += QString ("    %1 -> %2\n")
                             .arg (it->second[i][j])
                             .arg (
                                 pointmap_atoms_dest.count (it->first)
                                     ? ((unsigned int)pointmap_atoms_dest
                                                    [it->first]
                                                        .size ()
                                                > i
                                            ? ((unsigned int)
...
QString
US_Hydrodyn::list_rotated_rotamers ()
{
  QString out = "no";
...
QString
US_Hydrodyn::list_rotamers (bool coords)
{
  QString out;
      ...
      for (unsigned int i = 0; i < (unsigned int)it->second.size (); i++)
        {
          out += QString (" %1: sc").arg (it->second[i].extension);
          for (unsigned int j = 0;
               j < (unsigned int)it->second[i].side_chain.size (); j++)
            {
          ...
          out += " w";
          for (unsigned int j = 0;
               j < (unsigned int)it->second[i].waters.size (); j++)
            {
...
bool
US_Hydrodyn::compute_best_fit_rotamer (QString &error_msg)
{
  ...
  for (map<QString, vector<float> >::iterator it
       ...
       it != to_hydrate_dihedrals.end (); it++)
    {
      ...
      if (!rotamers.count (resName))
        {
          error_msg
              = QString (us_tr ("No rotamer group found for residue %1."))
                    .arg (resName);
          return false;
        }
      ...
      for (unsigned int i = 0; i < (unsigned int)rotamers[resName].size ();
           i++)
        {
          if (it->second.size ()
            ...
            {
              error_msg
                  = QString (us_tr ("Number of dihedrals do not match between "
                                    "side chain %1 (%2) and rotamer %3 (%4)"))
                        .arg (it->first)
                        .arg (it->second.size ())
...
QString
US_Hydrodyn::list_best_fit_rotamer ()
{
  QString out;
...
bool
US_Hydrodyn::setup_pointmap_rotamers (QString &error_msg)
{
  ...
  for (map<QString, map<QString, point> >::iterator it
       ...
       it != to_hydrate_pointmaps.end (); it++)
    {
      ...
      if (!pointmap_atoms_ref_residue.count (resName))
        {
          error_msg = QString (us_tr ("Internal error: could not find rotamer "
                                      "for pointmap key %1"))
                          .arg (it->first);
          return false;
        }
      ...
      for (unsigned int i = 0;
           i < (unsigned int)pointmap_atoms_ref_residue[resName].size (); i++)
        {
          if (!rotamers.count (pointmap_atoms_ref_residue[resName][i]))
            {
              error_msg
                  = QString (us_tr ("No rotamer group found for pointmap key "
                                    "%1 reference residue %2."))
                        .arg (it->first)
...
QString
US_Hydrodyn::list_pointmap_rotamers ()
{
  QString out;
...
bool
US_Hydrodyn::compute_water_positioning_atoms (QString &error_msg)
{
  editor->append (
      us_tr ("Calculating water positioning atoms for each rotamer\n"));
  qApp->processEvents ();
      ...
      for (unsigned int i = 0; i < (unsigned int)it->second.size (); i++)
        {
          ...
          for (unsigned int j = 0;
               j < (unsigned int)it->second[i].waters.size (); j++)
            {
              ...
              for (unsigned int k = 0;
                   k < (unsigned int)it->second[i].side_chain.size (); k++)
                {
  ...
  editor->append (
      us_tr ("Done calculating water positioning atoms for each rotamer\n"));
  qApp->processEvents ();
  puts ("Done calculating water positioning atoms for each rotamer");

  editor->append (
      us_tr ("Copy over water positioning info for rotated rotamers\n"));
  qApp->processEvents ();
  ...
  for (map<QString, vector<vector<rotamer> > >::iterator it
       ...
       it != rotated_rotamers.end (); it++)
    {
      if (!back_ref.count (it->first))
        {
          error_msg = QString (us_tr ("Error: rotated_rotamer wth name %1 not "
                                      "found it rotamers"))
                          .arg (it->first);
          return false;
        }
      ...
      if (water_count != back_ref[it->first]->water_positioning_atoms.size ())
        {
          error_msg
              = QString (us_tr ("Error: rotated_rotamer wth name %1 found but "
                                "water count differs %2 vs %3"))
                    .arg (it->first)
                    .arg (water_count)
                    .arg (
...
QString
US_Hydrodyn::list_water_positioning_atoms ()
{
      ...
      for (unsigned int i = 0; i < (unsigned int)it->second.size (); i++)
        {
          out += QString ("  %1:\n").arg (it->second[i].extension);
          for (unsigned int j = 0;
               j < (unsigned int)it->second[i].water_positioning_atoms.size ();
               j++)
            {
              out += QString ("    water %1 :").arg (j);
              for (unsigned int k = 0; k < (unsigned int)it->second[i]
                                               .water_positioning_atoms[j]
                                               .size ();
                   k++)
                {
                  out += QString (" %1").arg (
...
bool
US_Hydrodyn::compute_waters_to_add (QString &error_msg, bool quiet)
{
  ...
  for (map<QString, rotamer>::iterator it = best_fit_rotamer.begin ();
       it != best_fit_rotamer.end (); it++)
    {
      if (!to_hydrate.count (it->first))
        {
          error_msg = QString (us_tr ("Internal error: best_fit_rotamer key "
                                      "%1 not found in to_hydrate"))
                          .arg (it->first);
          return false;
        }
      ...
      if (it->second.water_positioning_atoms.size ()
        ...
        {
          error_msg = QString (us_tr ("Internal error: water positioning atom "
      ...
      for (unsigned int i = 0;
           i < (unsigned int)it->second.water_positioning_atoms.size (); i++)
        {
          // get coordinates of
          ...
          for (unsigned int j = 0;
               j < (unsigned int)it->second.water_positioning_atoms[i].size ();
               j++)
            {
              if (!it->second.atom_map.count (
                ...
                {
                  error_msg
                      = QString (us_tr ("Internal error: water positioning "
              ...
              if (!to_hydrate[it->first].count (
                      it->second.water_positioning_atoms[i][j]))
                {
                  error_msg
                      = QString (
                            us_tr ("Internal error: water positioning atom %1 "
  ...
  for (map<QString, vector<rotamer> >::iterator it
       ...
       it != pointmap_rotamers.end (); it++)
    {
      if (!to_hydrate_pointmaps.count (it->first))
        {
          error_msg = QString (us_tr ("Internal error: pointmap_rotamers key "
                                      "%1 not found in to_hydrate_pointmaps"))
                          .arg (it->first);
          return false;
        }
      ...
      if (!pointmap_atoms.count (resName)
        ...
        {
          error_msg
              = QString (us_tr ("Internal error: could not find pointmap "
      ...
      if (it->second.size () != pointmap_atoms[resName].size ()
        ...
        {
          error_msg
              = QString (us_tr ("Internal error: could not find pointmap size "
      ...
      for (unsigned int i = 0; i < (unsigned int)it->second.size (); i++)
        {
          ...
          if (it->second[i].residue != pointmap_atoms_ref_residue[resName][i])
            {
              error_msg
                  = QString (
                        us_tr ("Internal error: inconsistancy for reference "
          ...
          for (unsigned int j = 0; j < (unsigned int)p1.size (); j++)
            {
              ...
              if (!it->second[i].atom_map.count (
                ...
                {
                  error_msg
                      = QString (us_tr ("Internal error: atom %1 not found in "
          ...
          for (unsigned int j = 0;
               j < (unsigned int)it->second[i].waters.size (); j++)
            {
              ...
              for (unsigned int k = 0;
                   k < (unsigned int)rotamer_waters.size (); k++)
                {
              ...
              for (unsigned int k = 0; k < (unsigned int)new_waters.size ();
                   k++)
                {
                  cout << new_waters[k];
                }
              ...
              else
                {
                              ...
                              .arg (it->second[i].name);
                  {
                    ...
                    double dist_threshold
                        = 1e0 - (saxs_options.steric_clash_distance / 100e0);
                    double water_radius
                    ...
                    // check structure:
                    for (unsigned int j = 0;
                         j < (unsigned int)model_vector[i].molecule.size ();
                         j++)
                      {
                        for (unsigned int k = 0;
                             k < (unsigned int)model_vector[i]
                                     .molecule[j]
                                     .atom.size ();
                             k++)
                          {
                    ...
                    for (map<QString, vector<point> >::iterator it
                      ...
                      {
                        for (unsigned int i = 0;
                             i < (unsigned int)it->second.size (); i++)
                          {
  ...
  editor->append (
      us_tr ("Done transforming waters to add to pdb coordinates\n"));
  editor->append (
      QString (
          us_tr ("%1 waters added. %2 not added due to steric clashes \n"))
...
QString
US_Hydrodyn::list_waters_to_add ()
{
  QString out;
...
US_Hydrodyn::list_steric_clash ()
{
  ...
  for (map<QString, unsigned int>::iterator it = steric_clash_summary.begin ();
       it != steric_clash_summary.end (); it++)
    {
...
float
US_Hydrodyn::min_dist_to_struct_and_waters (point p)
{
  unsigned int i = current_model;
...
bool
US_Hydrodyn::has_steric_clash (point p, bool summary)
{
  unsigned int i = current_model;
  double dist_threshold = 1e0 - (saxs_options.steric_clash_distance / 100e0);
  double water_radius
  ...
  // cout << QString( "using %1 for water radius %2\n" ).arg( water_radius
  // ).arg(  multi_residue_map.count( "WAT" ) ? "has WAT" : "no WAT" );
...
float
US_Hydrodyn::min_dist_to_waters (point p)
{
  // check already added waters:
...
QString
US_Hydrodyn::list_steric_clash_recheck (bool quiet)
{
  unsigned int i = current_model;
  double dist_threshold
      = 1e0 - (saxs_options.steric_clash_recheck_distance / 100e0);
  double water_radius
      = multi_residue_map.count ("WAT")
            ? residue_list[multi_residue_map["WAT"][0]].r_atom[0].hybrid.radius
            : 1.401;
      ...
      for (unsigned int pos = 0; pos < (unsigned int)it->second.size (); pos++)
        {
          ...
          for (unsigned int j = 0;
            ...
            {
              for (unsigned int k = 0;
                   k < (unsigned int)model_vector[i].molecule[j].atom.size ();
                   k++)
                {
                  PDB_atom *this_atom = &(model_vector[i].molecule[j].atom[k]);
                  if (dist (this_atom->coordinate, p)
          ...
          for (map<QString, vector<point> >::iterator it2 = it;
               it2 != waters_to_add.end (); it2++)
            {
              for (unsigned int j = (it2 == it ? pos + 1 : 0);
                   j < (unsigned int)it2->second.size (); j++)
                {
  ...
  editor_msg (
      ...
      QString ("Maximum possible water to place %1, degree of hydration %2\n")
              .arg (hydrate_max_waters_no_asa)
              .arg (QString ("").sprintf (
                  "%.2f", (double)hydrate_max_waters_no_asa * 18e0
                              / model_vector[current_model].mw))
          + QString ("Maximum exposed possible water to place %1, degree of "
                     "hydration %2\n")
                .arg (count_waters_added + count_waters_not_added)
                .arg (QString ("").sprintf (
                    "%.2f",
                    (double)(count_waters_added + count_waters_not_added)
                        * 18e0 / model_vector[current_model].mw))
          + QString ("Waters placed %1, degree of hydration %2\n")
                .arg (count_waters_added)
                .arg (QString ("").sprintf (
                    "%.2f", (double)count_waters_added * 18e0
                                / model_vector[current_model].mw)));
  ...
  if (f.open (QIODevice::WriteOnly))
    ...
    {
      editor->append (
          QString (us_tr ("Error: could not create %1")).arg (fname));
    }
...
bool
US_Hydrodyn::write_pdb_with_waters (QString &error_msg, bool quiet)
{
#if defined(OLD_WAY)
  if (saxs_options.alt_hydration)
    {
      ...
      for (unsigned int i = 0; i < (unsigned int)it->second.size (); i++)
        {
          last_hydrated_pdb_text += QString ("").sprintf (
              ...
              it->second[i].axis[2]);
          if (hydrate_clash_map_rtmr_water.count (
                  QString ("%1~%2").arg (it->first).arg (i)))
            {
              hydrate_clash_waters_rtmr_list
                  << QString ("swh%1:%2").arg (residue_number).arg (chainID);
            }
          if (hydrate_clash_map_pm_water.count (
                  QString ("%1~%2").arg (it->first).arg (i)))
            {
              hydrate_clash_waters_pm_list
                  << QString ("swh%1:%2").arg (residue_number).arg (chainID);
            }
        }
    }
  ...
  if (!quiet)
    {
      ...
      for (map<QString, bool>::iterator it
           = hydrate_clash_map_structure.begin ();
           it != hydrate_clash_map_structure.end (); it++)
        {
          out += QString ("select %1\ncolor cyan\n").arg (it->first);
        }
      for (unsigned int i = 0;
           i < (unsigned int)hydrate_clash_waters_rtmr_list.size (); i++)
        {
          out += QString ("select %1\ncolor yellow\n")
                     .arg (hydrate_clash_waters_rtmr_list[i]);
        }
      for (unsigned int i = 0;
           i < (unsigned int)hydrate_clash_waters_pm_list.size (); i++)
        {
          out += QString ("select %1\ncolor green\n")
      ...
      QString fname = somo_dir + SLASH + "tmp" + SLASH
                      + QFileInfo (last_hydrated_pdb_name).baseName ()
                      + ".spt";
      QFile f (fname);
      if (!f.open (QIODevice::WriteOnly))
        {
...
bool
US_Hydrodyn::selected_models_contain (QString residue)
{
  for (unsigned int i = 0; i < (unsigned int)lb_model->count (); i++)
    {
      if (lb_model->item (i)->isSelected ())
                  ...
                  if (!rotamers.count (it->second[i]))
                  ...
                  else
                    {
                      if (!rotamers[it->second[i]].size ())
                        {
                          out += QString ("Pointmap %1 contains empty target "
                                          "rotamer %2, removing from loaded "
                                          "pointmaps\n")
                                     .arg (it->first)
                                     .arg (it->second[i]);
                          to_erase[it->first] = true;
...
#if defined(OLD_WAY)
bool
US_Hydrodyn::compute_waters_to_add_alt (QString &error_msg, bool quiet)
{
  editor->append (us_tr ("Transforming waters to add to pdb coordinates\n"));
  qApp->processEvents ();
  puts ("Transforming waters to add to pdb coordinates");
  ...
  editor_msg ("blue",
              us_tr ("NOTICE: only main chain CA-C-N currenly implemented"));
  ...
  for (map<QString, rotamer>::iterator it = best_fit_rotamer.begin ();
       it != best_fit_rotamer.end (); it++)
    {
      ...
      cout << QString ("toh: asa %1 tot=%2 sc=%3 mc=%4\n")
                  .arg (it->first)
                  .arg (residue_asa.count (it->first) ? residue_asa[it->first]
                                                      : -9.99)
                  .arg (residue_asa_sc.count (it->first)
                            ? residue_asa_sc[it->first]
                            : 0.0)
      ...
      for (unsigned int i = 0; i < (unsigned int)it->second.size (); i++)
        {
          ...
          cout << QString ("Note: in compute_waters_to_add_alt(), current "
                      ...
                      .arg (rotated_rotamers.count (ref_rotamer.name) ? "yes"
                                                                      : "no");
#endif
          ...
          if (rotated_rotamers.count (ref_rotamer.name))
          ...
          else
            {
              ...
              if (p1.size () != p2.size ())
                {
                  error_msg
                      = QString (us_tr ("Internal error: size inconsistancy "
              ...
              for (unsigned int j = 0; j < (unsigned int)p1.size (); j++)
                {
                  ...
                  if (!ref_rotamer.atom_map.count (
                          pointmap_atoms_dest[resName][i][j]))
                    {
                      error_msg
                          = QString (
                                us_tr ("Internal error: atom %1 not found in "
                                       "to_hydrate_pointmaps atoms"))
                                .arg (pointmap_atoms_dest[resName][i][j]);
                      return false;
                    }
  ...
  // we need to build up a list sorted by % asa ascending possibilities of both
  // pointmap and best fit rotamers then handle them in a uniform way one water
  // at a time for now we are going to compute % asa of complete mc and sc
  // another possiblity would be to compute asa % of mapping atoms for each
  // individual water
          ...
          if (rotated_rotamers.count (ref_rotamer.name))
            {
              ...
              for (unsigned int k = 0;
                   k < (unsigned int)rotated_rotamers[it->second[i].name]
                           .size ();
                   k++)
                {
                  ...
                  if (ref_rotamer.residue
                    ...
                    {
                      error_msg
                          = QString (us_tr ("Internal error: inconsistancy "
                                            "for reference residue name for "
                                            "key %1 pos %2 (%3 != %4)"))
                                .arg (it->first)
                                .arg (i)
                                .arg (ref_rotamer.residue)
                  ...
                  if (p1.size () != p2.size ())
                    {
                      error_msg
                          = QString (
                                us_tr ("Internal error: size inconsistancy "
                                       "for reference residue name for key %1 "
                                       "pos %2 (%3 != %4)"))
                                .arg (it->first)
                                .arg (i)
                                .arg (pointmap_atoms[resName][i].size ())
                  ...
                  for (unsigned int j = 0; j < (unsigned int)p1.size (); j++)
                    {
                      ...
                      if (!ref_rotamer.atom_map.count (
                              pointmap_atoms_dest[resName][i][j]))
                        {
                          error_msg
                              = QString (
                                    us_tr ("Internal error: atom %1 not found "
                      ...
                      p2[j] = to_hydrate_pointmaps
                          [it->first][pointmap_atoms[resName][i][j]];
                  ...
                  if (ref_rotamer.waters.size () != 1)
                    {
                      error_msg
                          = QString (
                                us_tr ("Error: currently only one water "
                                       "supported for multiple rotated "
                                       "rotamer waters, found %1 for %2\n"))
                                .arg (ref_rotamer.waters.size ())
                                .arg (ref_rotamer.name);
                      return false;
                    }

                  // now we are going to save them all and create a nmr style
                  // set of options
                      ...
                      if (!has_steric_clash (new_waters[0]))
                        {
                          ...
                          water_choices_source[it->first][i].push_back (
                              QString ("PM:%1").arg (ref_rotamer.name));
                          // count_waters_added++;
                          // waters_to_add[ it->first ].push_back( new_waters[
                          // 0 ] ); waters_source [ it->first ].push_back(
                          // QString( "PM:%1" ).arg( ref_rotamer.name ) );
                          found_rotated_rotamer_without_clash = true;
#if defined(DEBUG_POINTMAP_ROTATE)
                          if (k)
                            {
                              cout << QString ("found clash, resolved with "
                                               "rotamer pos %1\n")
                                          .arg (k);
                            }
#endif
...
#if defined(DEBUG_POINTMAP_ROTATE)
                  cout << QString (
                      "found clash, exhausted rotated rotamers\n");
#endif
                  count_waters_not_added++;
                }
#endif // NOT_YET
  ...
  // ok, now we potentially have a bunch of choices in water_choices for each
  // water, let's list them:

  cout << "water choices:\n";
  vector<QString> water_choices_v;
  for (map<QString, vector<vector<vector<point> > > >::iterator it
       = water_choices.begin ();
       it != water_choices.end (); it++)
    {
      QString out
          = QString ("%1 %2 pointmap rotamers rejected %3\n")
                .arg (it->first)
                .arg (it->second.size ())
  ...
  for (int i = 0; i < (int)water_choices_v.size (); i++)
    {
      if (rx_split_key.indexIn (water_choices_v[i]) == -1)
        {
          error_msg
              = QString (
                    "Internal error: unexpected key in water_choices_v <%1>")
                    .arg (water_choices_v[i]);
          return false;
        }
      ...
      // water_choices_v[ i ] ).arg( resname ).arg( pm_rot ) << flush;

      if ((int)water_choices[resname].size () <= pm_rot)
        {
          error_msg = QString ("Internal error: unexpected key pos in "
                               "water_choices_v <%1>")
                          .arg (water_choices_v[i]);
          return false;
        }
      ...
      for (int jj = 0; jj < (int)water_choices_v.size (); jj++)
        {
          ...
          if (rx_split_key.indexIn (water_choices_v[j]) == -1)
            {
              error_msg = QString ("Internal error: unexpected key in "
                                   "water_choices_v 2 <%1>")
          ...
          // water_choices_v[ j ] ).arg( resname ).arg( pm_rot ) << flush;
          ...
          if (water_choices[resname][pm_rot].size () == 1)
            {
              ...
              // cout << QString( "for key %1: only one, no alternatives
              // available\n" ) .arg( water_choices_v[ j ] );

              bool any_steric_clash = false;
              for (int l = 0;
                   l < (int)water_choices[resname][pm_rot][0].size (); l++)
                {
              ...
              for (int l = 0;
                   l < (int)water_choices[resname][pm_rot][0].size (); l++)
                {
                  if (!any_steric_clash)
                    {
                      count_waters_added++;
                      waters_to_add[resname].push_back (
                          water_choices[resname][pm_rot][0][l]);
                      waters_source[resname].push_back (QString ("PM:%1").arg (
                          water_choices_source[resname][pm_rot][0]));
                    }
                  else
                    {
                      count_waters_not_added++;
          ...
          for (int k = 0; k < (int)water_choices[resname][pm_rot].size (); k++)
            {
              ...
              for (int l = 0;
                   l < (int)water_choices[resname][pm_rot][k].size (); l++)
                {
                  // cout << QString( "min dist to waters called k %1 l %2\n"
                  // ).arg( k ).arg( l ) << flush;
                  float tmp_dist = min_dist_to_waters (
                      water_choices[resname][pm_rot][k][l]);
                  // cout << QString( "back min dist to waters called k %1 l
                  // %2\n" ).arg( k ).arg( l ) << flush;
          ...
          bool any_steric_clash = false;
          for (int l = 0;
               l < (int)water_choices[resname][pm_rot][best_k].size (); l++)
            {
              ...
              if (!any_steric_clash)
                {
                  count_waters_added++;
                  waters_to_add[resname].push_back (
                      water_choices[resname][pm_rot][best_k][l]);
                  waters_source[resname].push_back (QString ("PM:%1").arg (
                      water_choices_source[resname][pm_rot][best_k]));
                }
              else
                {
                  count_waters_not_added++;
  ...
  for (int i = 0; i < (int)alt_waters_to_add.size (); i++)
    {
      editor->append (QString (us_tr ("Model %1: %2 waters added. %3 not "
                                      "added due to steric clashes \n"))
                          .arg (i + 1)
      ...
      alt_hydrate_clash_map_rtmr_water.push_back (
          hydrate_clash_map_rtmr_water);
...
bool
US_Hydrodyn::alt_write_pdb_with_waters (QString &error_msg, bool /* quiet */)
{
  if (alt_waters_to_add.size () != alt_steric_clash_summary.size ()
    ...
    {
      error_msg
          = QString ("Internal error: vector size mismatch %1 %2 %3 %4 %5 %6")
  ...
  vector<QStringList> alt_hydrate_clash_waters_rtmr_list (
      alt_waters_to_add.size ());
  vector<QStringList> alt_hydrate_clash_waters_pm_list (
      alt_waters_to_add.size ());
          ...
          for (unsigned int i = 0; i < (unsigned int)it->second.size (); i++)
            {
              ...
              if (hydrate_clash_map_rtmr_water.count (
                      QString ("%1~%2").arg (it->first).arg (i)))
                {
                  alt_hydrate_clash_waters_rtmr_list[m]
                      << QString ("swh%1:%2")
                             .arg (residue_number)
                             .arg (chainID);
                }
              if (hydrate_clash_map_pm_water.count (
                      QString ("%1~%2").arg (it->first).arg (i)))
                {
                  alt_hydrate_clash_waters_pm_list[m]
                      << QString ("swh%1:%2")
                             .arg (residue_number)
                             .arg (chainID);
                }
  ...
  if ( !quiet )
  {
     QString out;
     out = QString( "load %1\ncpk\nselect all\ncolor white\nselect swh\ncolor
  red\n" ).arg( fname ); for ( map < QString, bool >::iterator it =
  hydrate_clash_map_structure.begin(); it != hydrate_clash_map_structure.end();
           it++ )
     {
        out += QString( "select %1\ncolor cyan\n" ).arg( it->first );
     }
     for ( unsigned int i = 0; i < (unsigned int)
  hydrate_clash_waters_rtmr_list.size(); i++ )
     {
        out += QString( "select %1\ncolor yellow\n" ).arg(
  hydrate_clash_waters_rtmr_list[ i ] );
     }
     for ( unsigned int i = 0; i < (unsigned int)
  hydrate_clash_waters_pm_list.size(); i++ )
     {
        out += QString( "select %1\ncolor green\n" ).arg(
  hydrate_clash_waters_pm_list[ i ] );
     }

     QString fname =
        somo_dir + SLASH + "tmp" + SLASH +
  QFileInfo(last_hydrated_pdb_name).baseName() + ".spt"; QFile f(fname); if (
  !f.open( IO_WriteOnly ) )
     {
        editor->append("Error creating file " + fname + "\n");
     } else {
...
US_Hydrodyn::compute_waters_to_add_alt (QString &error_msg, bool quiet)
{
  ...
  // editor_msg( "blue", us_tr( "NOTICE: only main chain CA-C-N currenly
  // implemented" ) );
  ...
  progress->setValue (residues_in_order.size ());
  progress->setMaximum (residues_in_order.size () * 2);

  for (unsigned int i = 0; i < (unsigned int)residues_in_order.size (); i++)
    {

      progress->setValue (residues_in_order.size () + i);
      progress->setMaximum (residues_in_order.size () * 2);
      ...
      if (best_fit_rotamer.count (this_residue))
        {
          ...
          if (!to_hydrate.count (this_residue))
            {
              error_msg = QString (us_tr ("Internal error: best_fit_rotamer "
                                          "key %1 not found in to_hydrate"))
                              .arg (this_residue);
              return false;
            }
          ...
          if (best_fit.water_positioning_atoms.size ()
            ...
            {
              error_msg
                  = QString (us_tr ("Internal error: water positioning atom "
                                    "size %1 does not match waters size %2"))
          ...
          if (rotated_rotamers.count (best_fit.name))
            {
              ...
              if (rotated_rotamers[best_fit.name].size ()
                  != best_fit.water_positioning_atoms.size ())
                {
                  error_msg
                      = QString (
                            us_tr (
                                "Internal error: water positioning atom size "
              ...
              for (unsigned int ii = 0;
                   ii < (unsigned int)best_fit.water_positioning_atoms.size ();
                   ii++)
                {
                  ...
                  for (unsigned int m = 0;
                       m < (unsigned int)rotated_rotamers[best_fit.name][ii]
                               .size ();
                       m++)
                    {
                      ...
                      if (best_fit.water_positioning_atoms[ii].size ()
                        ...
                        {
                          error_msg
                              = QString (us_tr ("Internal error: best fit "
                                                "waters count %1 does not "
                                                "match reference %2"))
                      ...
                      for (unsigned int jj = 0;
                           jj < (unsigned int)this_ref_rotamer
                                    .water_positioning_atoms[0]
                                    .size ();
                           jj++)
                        {
                          if (!this_ref_rotamer.atom_map.count (
                            ...
                            {
                              error_msg
                                  = QString (us_tr ("Internal error: water "
                                                    "positioning atom %1 not "
                                                    "found in atom_map"))
                                        .arg (
                                            this_ref_rotamer
                                                .water_positioning_atoms[0]
          ...
          else
            {
              ...
              //                  best_fit.waters.size() )
              //             {
              //                error_msg = QString( us_tr( "Internal error:
              //                water positioning atom size %1 does not match
              //                waters size %2" ) )
              //                   .arg(
              //                   best_fit.water_positioning_atoms.size() )
              //                   .arg( best_fit.waters.size() );
              ...
              for (unsigned int i = 0;
                   i < (unsigned int)best_fit.water_positioning_atoms.size ();
                   i++)
                {
                  // get coordinates of
                  ...
                  for (unsigned int j = 0;
                       j < (unsigned int)best_fit.water_positioning_atoms[i]
                               .size ();
                       j++)
                    {
                      if (!best_fit.atom_map.count (
                              best_fit.water_positioning_atoms[i][j]))
                        {
                          error_msg
                              = QString (
                                    us_tr ("Internal error: water positioning "
                                           "atom %1 not found in atom_map"))
                                    .arg (best_fit
      ...
      if (pointmap_rotamers.count (this_residue))
        {
          if (!to_hydrate_pointmaps.count (this_residue))
            {
              error_msg
                  = QString (us_tr ("Internal error: pointmap_rotamers key %1 "
          ...
          if (rx_expand_mapkey.indexIn (this_residue) == -1)
            {
              error_msg
                  = QString (
                        us_tr ("Internal error: could not expand mapkey %1"))
                        .arg (this_residue);
              return false;
            }
          QString resName = rx_expand_mapkey.cap (1);
          ...
          for (unsigned int i = 0;
               i < (unsigned int)pointmap_rotamers[this_residue].size (); i++)
            {
              // for each water to add:
              ...
              if (rotated_rotamers.count (ref_rotamer.name))
                {
                  ...
                  if (rx_expand_mapkey.indexIn (this_residue) == -1)
                    {
                      error_msg = QString (us_tr ("Internal error: could not "
                                                  "expand mapkey %1"))
                                      .arg (this_residue);
                      return false;
                    }
                  ...
                  if (!pointmap_atoms.count (resName)
                      ...
                      || !pointmap_atoms_ref_residue.count (resName))
                    {
                      error_msg
                          = QString (
                                us_tr (
                                    "Internal error: could not find pointmap "
                                    "entries for residue of key %1"))
                                .arg (resName);
                      return false;
                    }
                  ...
                  for (unsigned int k = 0;
                       k < (unsigned int)rotated_rotamers[ref_rotamer.name][i]
                               .size ();
                       k++)
                    {
                      rotamer this_ref_rotamer
                      ...
                      if (this_ref_rotamer.residue
                          != pointmap_atoms_ref_residue[resName][i])
                        {
                          error_msg
                              = QString (
                                    us_tr ("Internal error: inconsistancy for "
                      ...
                      if (p1.size () != p2.size ())
                        {
                          error_msg
                              = QString (
                                    us_tr (
                                        "Internal error: size inconsistancy "
                                        "for reference residue name for key "
                                        "%1 pos %2 (%3 != %4)"))
                                    .arg (this_residue)
                                    .arg (i)
                      ...
                      for (unsigned int j = 0; j < (unsigned int)p1.size ();
                           j++)
                        {
                          // cout << QString("mapping: %1 %2 dest %3\n")
                          // .arg(this_residue)
                          ...
                          if (!this_ref_rotamer.atom_map.count (
                                  pointmap_atoms_dest[resName][i][j]))
                            {
                              error_msg
                                  = QString (
                                        us_tr ("Internal error: atom %1 not "
                      ...
                      for (unsigned int m = 0;
                           m < (unsigned int)
                                   rotated_rotamers[this_ref_rotamer.name][0]
                                       .size ();
                           m++)
                        {
                          vector<point> rotamer_waters;
