...
int US_Hydrodyn::pdb_hydrate_for_saxs(bool quiet) {
  ...
  if (!load_rotamer(error_msg)) {
    QMessageBox::warning(this, us_tr("Error reading Hydrated Rotamer file"),
                         error_msg);
    return -1;
  }
    ...
    if (!any_errors && lb_model->item(current_model)->isSelected()) {
      ...
      if (!pdb_asa_for_saxs_hydrate()) {
        ...
        if (!compute_to_hydrate_dihedrals(error_msg)) {
          QMessageBox::warning(
              this, us_tr("Error computing to-hydrate dihedrals"), error_msg);
          any_errors = true;
        }
#if defined(DEBUG_TO_HYDRATE_DIHEDRAL)
        ...
        if (!compute_best_fit_rotamer(error_msg)) {
          QMessageBox::warning(this, us_tr("Error finding best fit rotamer"),
                               error_msg);
          any_errors = true;
        }
        ...
        if (!setup_pointmap_rotamers(error_msg)) {
          QMessageBox::warning(this, us_tr("Error setting up pointmaps"),
                               error_msg);
          any_errors = true;
        }
        ...
        if (saxs_options.alt_hydration) {
          editor_msg(
              "blue",
              us_tr("NOTICE: using alternate hydration (experimental) method"));
          if (!compute_waters_to_add_alt(error_msg, quiet)) {
            QMessageBox::warning(this, us_tr("Error trying to add waters"),
                                 error_msg);
            any_errors = true;
          }
        } else {
        ...
        if (!write_pdb_with_waters(error_msg, quiet)) {
          QMessageBox::warning(
              this, us_tr("Error trying to write pdb with waters"), error_msg);
          any_errors = true;
        }
        if (!any_errors && !screen_pdb(last_hydrated_pdb_name, !quiet)) {
          QMessageBox::warning(
              this, us_tr("Error trying to reload hydrated pdb"), error_msg);
          any_errors = true;
        }
        if (!any_errors) {
          pdb_saxs(true, !quiet);
...
bool US_Hydrodyn::compute_to_hydrate_dihedrals(QString &error_msg) {
  ...
  for (map<QString, map<QString, point> >::iterator it = to_hydrate.begin();
    ...
    if (rx_expand_mapkey.indexIn(it->first) == -1) {
      error_msg = QString(us_tr("Internal error: could not expand mapkey %1"))
                      .arg(it->first);
      return false;
    }
    ...
    if (!dihedral_atoms.count(resName)) {
      error_msg = QString(us_tr("No dihedral group found for residue %1."))
                      .arg(resName);
      return false;
    }
#if defined(DEBUG_TO_HYDRATE_DIHEDRAL)
    ...
    for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size(); k++) {
      ...
      if (this_atom->exposed_code == 1) {
        ...
        if (this_atom->chain == 1) {
          ...
          use_color[mapkey] =
              rotamers.count(this_atom->resName) ? "green" : "yellow";
        }
        if (pointmap_atoms.count(this_atom->resName)) {
          exposed_for_pointmap[mapkey] = true;
          use_color[mapkey] =
              pointmap_atoms.count(this_atom->resName) ? "cyan" : "yellow";
        }
      }
    }
  }
...
bool US_Hydrodyn::load_rotamer(QString &error_msg) {
  ...
  if (!f.open(QIODevice::ReadOnly)) {
    error_msg = us_tr(
        "Hydrated Rotamer file is not readable:\n"
  ...
  for (unsigned int i = 0; i < (unsigned int)qsl.size(); i++) {
    ...
    if (qsl_line[0] == "multiple-rotate:") {
      ...
      for (unsigned int water_number = 0; water_number < water_count;
        ...
        unsigned int pos;
        for (unsigned int j = 0; j < (unsigned int)rotamers[residue].size();
             j++) {
        ...
        if (!found_match) {
          error_msg = QString(us_tr("Error in hydrated rotamer file line %1.  "
                                    "multiple-rotate: residue: %2 found, but "
                                    "full name %3 missing"))
                          .arg(i + 1)
                          .arg(residue)
                          .arg(name);
            ...
            error_msg =
                QString(
                    us_tr(
                        ...
                        "multiple-rotate: residue: %2 found, atom %3 missing"))
                    .arg(i + 1)
                    .arg(residue)
                    .arg(atoms[j]);
            return false;
        ...
        if ((unsigned int)ref_rotamer.waters.size() <= water_number) {
          error_msg =
              QString(
                  us_tr("Error in hydrated rotamer file line %1.  "
                        "multiple-rotate: residue: %2 found reference rotamer, "
                        "but it has %3 waters, less than %4 requested"))
                  .arg(i + 1)
                  .arg(residue)
        ...
        if (ref_rotamer.waters[water_number].name != atoms[2]) {
          error_msg = QString(us_tr("Error in hydrated rotamer file line %1.  "
                                    "multiple-rotate: residue: %2 found, atom "
                                    "%3 missing from waters"))
                          .arg(i + 1)
                          .arg(residue)
                          .arg(atoms[2]);
          return false;
        ...
        for (unsigned int j = 0; j < (unsigned int)ref_rotamer.waters.size();
             j++) {
      ...
      for (unsigned int jj = 0;
        ...
        for (unsigned int j = 0;
          ...
          for (unsigned int k = 0;
               k <
               (unsigned int)rotated_rotamers[name][jj][j].side_chain.size();
               k++) {
            out += QString("").sprintf(
          ...
          out += "TER\n";
          for (unsigned int k = 0;
               k < (unsigned int)rotated_rotamers[name][jj][j].waters.size();
               k++) {
            out += QString("").sprintf(
                "ATOM  %5d%5s%4s %1s%4d    %8.3f%8.3f%8.3f%6.2f%6.2f          "
                "%2s\n",
    ...
    if (qsl_line[0] == "pointmap:") {
      ...
      if (pointmap_atoms.count(res)) {
        error_msg = QString(us_tr("Error in hydrated rotamer file line %1.  %2 "
                                  "was previously defined as a pointmap"))
                        .arg(i + 1)
                        .arg(res);
        return false;
      }
      ...
      while (qsl_line.size()) {
        ...
        if (points < 3) {
          error_msg =
              QString(us_tr("Error in hydrated rotamer file line %1.  Invalid "
          ...
          if (pointmap_atoms.count(AAs[j])) {
            error_msg =
                QString(us_tr("Error in hydrated rotamer file line %1. "
                              ...
                              "pointmap defined for Amino Acid residue %2."))
                    .arg(i + 1)
                    .arg(AAs[j]);
            return false;
          }
      ...
      if (qsl_line[0] == "end-rotamer") {
        ...
        if (!tmp_rotamer.side_chain.size() || !tmp_rotamer.waters.size()) {
          error_msg =
              QString(
                  us_tr(
                      "Error in hydrated rotamer file line %1.  Empty rotamer"))
                  .arg(i + 1);
          return false;
        }
    ...
    for (unsigned int i = 0; i < (unsigned int)it->second.size(); i++) {
      ...
      for (unsigned int j = 0;
        ...
        if (dihedral_atoms[it->first][j].size() != 4) {
          error_msg = QString(
                          "Dihedral table size incorrect, should be 4 but is "
                          "%1 for %2th dihedral group of %3")
        ...
        for (unsigned int k = 0;
          ...
          if (!(it->second[i].atom_map.count(
            ...
            error_msg = QString("Rotamer for %1 is missing dihedral atom %2")
                            .arg(it->first)
          ...
          out +=
              QString("    %1 -> %2\n")
                  ...
                  .arg(pointmap_atoms_dest.count(it->first)
                           ? ((unsigned int)pointmap_atoms_dest[it->first]
                                          .size() > i
...
QString US_Hydrodyn::list_rotamers(bool coords) {
  ...
  for (map<QString, vector<rotamer> >::iterator it = rotamers.begin();
    ...
    for (unsigned int i = 0; i < (unsigned int)it->second.size(); i++) {
      ...
      out += " w";
      for (unsigned int j = 0; j < (unsigned int)it->second[i].waters.size();
           j++) {
...
bool US_Hydrodyn::compute_best_fit_rotamer(QString &error_msg) {
  ...
  for (map<QString, vector<float> >::iterator it = to_hydrate_dihedrals.begin();
    ...
    if (!rotamers.count(resName)) {
      error_msg =
          QString(us_tr("No rotamer group found for residue %1.")).arg(resName);
      return false;
    }
    ...
    for (unsigned int i = 0; i < (unsigned int)rotamers[resName].size(); i++) {
      if (it->second.size() != rotamers[resName][i].dihedral_angles.size()) {
        error_msg = QString(us_tr("Number of dihedrals do not match between "
  ...
  for (map<QString, map<QString, point> >::iterator it =
    ...
    if (!pointmap_atoms_ref_residue.count(resName)) {
      error_msg =
          QString(
              us_tr(
                  "Internal error: could not find rotamer for pointmap key %1"))
              .arg(it->first);
      return false;
    }
    ...
    for (unsigned int i = 0;
      ...
      if (!rotamers.count(pointmap_atoms_ref_residue[resName][i])) {
        error_msg = QString(us_tr("No rotamer group found for pointmap key %1 "
                                  "reference residue %2."))
                        .arg(it->first)
...
bool US_Hydrodyn::compute_water_positioning_atoms(QString &error_msg) {
  ...
  for (map<QString, vector<vector<rotamer> > >::iterator it =
    ...
    if (!back_ref.count(it->first)) {
      error_msg =
          QString(
              us_tr("Error: rotated_rotamer wth name %1 not found it rotamers"))
              .arg(it->first);
      return false;
    }
    ...
    if (water_count != back_ref[it->first]->water_positioning_atoms.size()) {
      error_msg = QString(us_tr("Error: rotated_rotamer wth name %1 found but "
  ...
  for (map<QString, vector<rotamer> >::iterator it = rotamers.begin();
    ...
    for (unsigned int i = 0; i < (unsigned int)it->second.size(); i++) {
      ...
      for (unsigned int j = 0;
        ...
        for (unsigned int k = 0;
             k < (unsigned int)it->second[i].water_positioning_atoms[j].size();
             k++) {
          out +=
...
bool US_Hydrodyn::compute_waters_to_add(QString &error_msg, bool quiet) {
  ...
  for (map<QString, rotamer>::iterator it = best_fit_rotamer.begin();
    ...
    if (!to_hydrate.count(it->first)) {
      error_msg = QString(us_tr("Internal error: best_fit_rotamer key %1 not "
                                "found in to_hydrate"))
                      .arg(it->first);
      return false;
    }
    ...
    if (it->second.water_positioning_atoms.size() != it->second.waters.size()) {
      error_msg = QString(us_tr("Internal error: water positioning atom size "
                                "%1 does not match waters size %2"))
    ...
    for (unsigned int i = 0;
      ...
      for (unsigned int j = 0;
        ...
        if (!it->second.atom_map.count(
          ...
          error_msg = QString(us_tr("Internal error: water positioning atom %1 "
  ...
  for (map<QString, vector<rotamer> >::iterator it = pointmap_rotamers.begin();
    ...
    if (!to_hydrate_pointmaps.count(it->first)) {
      error_msg = QString(us_tr("Internal error: pointmap_rotamers key %1 not "
    ...
    if (!pointmap_atoms.count(resName) || !pointmap_atoms_dest.count(resName) ||
      ...
      error_msg = QString(us_tr("Internal error: could not find pointmap "
    ...
    if (it->second.size() != pointmap_atoms[resName].size() ||
      ...
      error_msg = QString(us_tr("Internal error: could not find pointmap size "
    ...
    for (unsigned int i = 0; i < (unsigned int)it->second.size(); i++) {
      ...
      if (it->second[i].residue != pointmap_atoms_ref_residue[resName][i]) {
        error_msg = QString(us_tr("Internal error: inconsistancy for reference "
      ...
      if (p1.size() != p2.size()) {
        error_msg =
            QString(us_tr("Internal error: size inconsistancy for reference "
                          "residue name for key %1 pos %2 (%3 != %4)"))
                .arg(it->first)
                .arg(i)
      ...
      for (unsigned int j = 0; j < (unsigned int)p1.size(); j++) {
        ...
        if (!it->second[i].atom_map.count(pointmap_atoms_dest[resName][i][j])) {
          error_msg = QString(us_tr("Internal error: atom %1 not found in "
  ...
  editor->append(
      QString(us_tr("%1 waters added. %2 not added due to steric clashes \n"))
  ...
  editor_msg(
      ...
      QString("Maximum possible water to place %1, degree of hydration %2\n")
          ...
          QString("Maximum exposed possible water to place %1, degree of "
              ...
              .arg(QString("").sprintf(
                  "%.2f",
                  (double)(count_waters_added + count_waters_not_added) * 18e0 /
                      model_vector[current_model].mw)) +
          QString("Waters placed %1, degree of hydration %2\n")
              .arg(count_waters_added)
              .arg(QString("").sprintf("%.2f",
...
bool US_Hydrodyn::write_pdb_with_waters(QString &error_msg, bool quiet) {
  ...
  if (!quiet) {
    ...
    QString fname = somo_dir + SLASH + "tmp" + SLASH +
                    QFileInfo(last_hydrated_pdb_name).baseName() + ".spt";
    QFile f(fname);
    if (!f.open(QIODevice::WriteOnly)) {
      editor->append("Error creating file " + fname + "\n");
    } else {
...
bool US_Hydrodyn::compute_waters_to_add_alt(QString &error_msg, bool quiet) {
    ...
    for (unsigned int i = 0; i < (unsigned int)it->second.size(); i++) {
...
#if defined(DEBUG_POINTMAP)
      cout << QString(
                  "Note: in compute_waters_to_add_alt(), current name '%1' has "
                  "alternates %2\n")
      ...
      if (rotated_rotamers.count(ref_rotamer.name)) {
        ...
        if (ref_rotamer.residue != pointmap_atoms_ref_residue[resName][i]) {
          error_msg =
              QString(us_tr("Internal error: inconsistancy for reference "
                            "residue name for key %1 pos %2 (%3 != %4)"))
                  .arg(it->first)
                  .arg(i)
...
#endif
          p2[j] =
              to_hydrate_pointmaps[it->first][pointmap_atoms[resName][i][j]];
  ...
  for (map<QString, vector<rotamer> >::iterator it = pointmap_rotamers.begin();
    ...
    for (unsigned int i = 0; i < (unsigned int)it->second.size(); i++) {
      ...
      if (rotated_rotamers.count(ref_rotamer.name)) {
        ...
        for (unsigned int k = 0;
             k < (unsigned int)rotated_rotamers[it->second[i].name].size();
             k++) {
          ...
          if (p1.size() != p2.size()) {
            error_msg =
                QString(
                    us_tr("Internal error: size inconsistancy for reference "
                          "residue name for key %1 pos %2 (%3 != %4)"))
                    .arg(it->first)
                    .arg(i)
                    .arg(pointmap_atoms[resName][i].size())
          ...
          if (ref_rotamer.waters.size() != 1) {
            error_msg =
                QString(
                    us_tr("Error: currently only one water supported for "
                          "multiple rotated rotamer waters, found %1 for %2\n"))
                    .arg(ref_rotamer.waters.size())
                    .arg(ref_rotamer.name);
            return false;
          }

          // now we are going to save them all and create a nmr style set of
          // options
          ...
          for (unsigned int j = 0; j < (unsigned int)ref_rotamer.waters.size();
            ...
            if (!has_steric_clash(new_waters[0])) {
              ...
              // count_waters_added++;
              // waters_to_add[ it->first ].push_back( new_waters[ 0 ] );
              // waters_source [ it->first ].push_back( QString( "PM:%1" ).arg(
              // ref_rotamer.name ) );
  ...
  for (int i = 0; i < (int)water_choices_v.size(); i++) {
    if (rx_split_key.indexIn(water_choices_v[i]) == -1) {
      error_msg =
          QString("Internal error: unexpected key in water_choices_v <%1>")
    ...
    if ((int)water_choices[resname].size() <= pm_rot) {
      error_msg =
          QString("Internal error: unexpected key pos in water_choices_v <%1>")
              .arg(water_choices_v[i]);
      return false;
    }
    ...
    for (int jj = 0; jj < (int)water_choices_v.size(); jj++) {
      ...
      if (rx_split_key.indexIn(water_choices_v[j]) == -1) {
        error_msg =
            QString("Internal error: unexpected key in water_choices_v 2 <%1>")
      ...
      if ((int)water_choices[resname].size() <= pm_rot) {
        error_msg =
            QString(
                "Internal error: unexpected key pos in water_choices_v 2 <%1>")
                .arg(water_choices_v[j]);
        return false;
      }
      ...
      if (water_choices[resname][pm_rot].size() == 1) {
        ...
        for (int l = 0; l < (int)water_choices[resname][pm_rot][0].size();
             l++) {
        ...
        for (int l = 0; l < (int)water_choices[resname][pm_rot][0].size();
          ...
          if (!any_steric_clash) {
            ...
            waters_source[resname].push_back(
                QString("PM:%1").arg(water_choices_source[resname][pm_rot][0]));
          } else {
            count_waters_not_added++;
          }
        }
      ...
      for (int k = 0; k < (int)water_choices[resname][pm_rot].size(); k++) {
        ...
        for (int l = 0; l < (int)water_choices[resname][pm_rot][k].size();
             l++) {
          // cout << QString( "min dist to waters called k %1 l %2\n" ).arg( k
          // ).arg( l ) << flush;
          float tmp_dist =
              min_dist_to_waters(water_choices[resname][pm_rot][k][l]);
      ...
      bool any_steric_clash = false;
      for (int l = 0; l < (int)water_choices[resname][pm_rot][best_k].size();
           l++) {
  ...
  for (int i = 0; i < (int)alt_waters_to_add.size(); i++) {
    editor->append(QString(us_tr("Model %1: %2 waters added. %3 not added due "
                                 "to steric clashes \n"))
                       .arg(i + 1)
  ...
  for (unsigned int i = 0; i < (unsigned int)residues_in_order.size(); i++) {
    ...
    if (best_fit_rotamer.count(this_residue)) {
      ...
      if (rotated_rotamers.count(best_fit.name)) {
        ...
        if (rotated_rotamers[best_fit.name].size() !=
          ...
          error_msg =
              QString(us_tr("Internal error: water positioning atom size %1 "
                            "does not match best fit rotamer size %2"))
                  .arg(best_fit.water_positioning_atoms.size())
                  .arg(best_fit.waters.size());
          return false;
        }
        ...
        for (unsigned int ii = 0;
             ii < (unsigned int)best_fit.water_positioning_atoms.size(); ii++) {
          ...
          for (unsigned int m = 0;
               m < (unsigned int)rotated_rotamers[best_fit.name][ii].size();
               m++) {
            ...
            if (best_fit.water_positioning_atoms[ii].size() !=
              ...
              error_msg =
                  QString(us_tr("Internal error: best fit waters count %1 does "
                                "not match reference %2"))
            ...
            for (unsigned int jj = 0;
                 jj < (unsigned int)this_ref_rotamer.water_positioning_atoms[0]
                          .size();
                 jj++) {
              if (!this_ref_rotamer.atom_map.count(
                      this_ref_rotamer.water_positioning_atoms[0][jj])) {
                error_msg =
                    QString(us_tr("Internal error: water positioning atom %1 "
                                  "not found in atom_map"))
        ...
        //             {
        //                error_msg = QString( us_tr( "Internal error: water
        //                positioning atom size %1 does not match waters size
        //                %2" ) )
        //                   .arg( best_fit.water_positioning_atoms.size() )
        //                   .arg( best_fit.waters.size() );
        //                return false;
        ...
        for (unsigned int i = 0;
             i < (unsigned int)best_fit.water_positioning_atoms.size(); i++) {
          // get coordinates of
          p1.resize(best_fit.water_positioning_atoms[i].size());
          p2.resize(best_fit.water_positioning_atoms[i].size());

          for (unsigned int j = 0;
               j < (unsigned int)best_fit.water_positioning_atoms[i].size();
               j++) {
            if (!best_fit.atom_map.count(
                    best_fit.water_positioning_atoms[i][j])) {
              error_msg = QString(us_tr("Internal error: water positioning "
            ...
            if (!to_hydrate[this_residue].count(
                    best_fit.water_positioning_atoms[i][j])) {
              error_msg =
                  QString(us_tr("Internal error: water positioning atom %1 not "
    ...
    if (pointmap_rotamers.count(this_residue)) {
      if (!to_hydrate_pointmaps.count(this_residue)) {
        error_msg = QString(us_tr("Internal error: pointmap_rotamers key %1 "
      ...
      for (unsigned int i = 0;
        ...
        if (rotated_rotamers.count(ref_rotamer.name)) {
          ...
          for (unsigned int k = 0;
               k < (unsigned int)rotated_rotamers[ref_rotamer.name][i].size();
               k++) {
            ...
            for (unsigned int j = 0; j < (unsigned int)p1.size(); j++) {
              ...
              if (!this_ref_rotamer.atom_map.count(
                      pointmap_atoms_dest[resName][i][j])) {
                error_msg = QString(us_tr("Internal error: atom %1 not found "
            ...
            for (unsigned int m = 0;
                 m < (unsigned int)rotated_rotamers[this_ref_rotamer.name][0]
                         .size();
                 m++) {
              vector<point> rotamer_waters;
              // this is probably not correct:
