...
void US_Hydrodyn_Dad_Baseline_Best::cancel()
{
   close();
}
...
void US_Hydrodyn_Dad_Baseline_Best::editor_msg( QString color, QString msg, QTextEdit *e )
{
   if ( !e ) {
      e = editor;
   }
...
void US_Hydrodyn_Dad_Baseline_Best::set_best() {
   if ( ((US_Hydrodyn *)us_hydrodyn)->dad_widget ) {
      ...
      if ( dad_win->current_mode == US_Hydrodyn_Dad::MODE_BASELINE &&
         ...
         dad_win->le_baseline_end_e->setText( QString( "%1" ).arg( bestpos + dparameters[ "width" ] - 1 ) );
         dad_win->le_baseline_width->setText( QString( "%1" ).arg( dparameters[ "width" ] ) );
