...
bool
US_PM::best_sphere (vector<double> &params, vector<double> &low_fparams,
...
bool
US_PM::best_sphere (set<pm_point> &model)
{
  // start with 1 sphere, slowly increase size until fitness drops
    ...
    while (delta > min_delta)
      {
        ...
        for (params[1] = low_limit; params[1] <= high_limit;
             params[1] += delta)
          {
            bool skip = false;
            create_model (params, this_model);
                ...
                if (prev_model.size () != this_model.size ())
                ...
                else
                  {
                    ...
                    if (us_log)
                      {
                        us_log->log (
                            QString ("skipped radius %1 beads %2 fitness %3\n")
                                .arg (params[1])
                                .arg (model.size ())
                                .arg (this_fit, 0, 'g', 8)
                                .toLatin1 ()
