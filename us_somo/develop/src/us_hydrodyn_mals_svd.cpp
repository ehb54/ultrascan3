...
US_Hydrodyn_Mals_Svd::US_Hydrodyn_Mals_Svd (
    US_Hydrodyn_Mals *mals_win, vector<QString> mals_selected_files,
    QWidget *p)
    : QFrame (p)
{
  ...
  this->plot_colors = mals_win->plot_colors;
  this->use_line_width = mals_win->use_line_width;

  USglobal = new US_Config ();
  setPalette (PALET_FRAME);
  setWindowTitle (us_tr ("US-SOMO: MALS SVD"));
  ...
  if (mals_win->f_is_time[this->mals_selected_files[0]])
    {
      QStringList qsl_mals_selected_files;
      for (int i = 0; i < (int)this->mals_selected_files.size (); ++i)
        {
          qsl_mals_selected_files << this->mals_selected_files[i];
          ;
        }

      QString error_msg;
      if (!convert_it_to_iq (qsl_mals_selected_files, original_data,
                             error_msg))
        {
          QMessageBox::warning (
              this, windowTitle (),
      ...
      this->mals_selected_files.clear ();
      for (int i = 0; i < (int)original_data.size (); ++i)
        {
          this->mals_selected_files.push_back (original_data[i]);
        }
    }
  else
    {
      for (int i = 0; i < (int)this->mals_selected_files.size (); ++i)
        {
          QString this_name = this->mals_selected_files[i];
          original_data.push_back (this_name);

          f_pos[this_name] = f_pos.size ();
          f_qs_string[this_name] = mals_win->f_qs_string[this_name];
          f_qs[this_name] = mals_win->f_qs[this_name];
          f_Is[this_name] = mals_win->f_Is[this_name];
          f_errors[this_name] = mals_win->f_errors[this_name];
  ...
  if (!norm_ok)
    {
      editor_msg ("dark red", us_tr ("Missing or zero associated errors so "
...
US_Hydrodyn_Mals_Svd::setupGUI ()
{
  ...
  AUTFBACK (lbl_data);
  lbl_data->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  AUTFBACK (lv_data);
  lv_data->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize - 1, QFont::Bold));
  lv_data->setEnabled (true);
  // lv_data->setMinimumWidth();

  lv_data->setColumnCount (1);
  lv_data->setHeaderLabels (QStringList () << us_tr ("Source"));
  ...
  connect (lv_data, SIGNAL (itemSelectionChanged ()),
           SLOT (data_selection_changed ()));

  QTreeWidgetItem *element
      = new QTreeWidgetItem (QStringList () << us_tr ("Original data"));
  lv_data->addTopLevelItem (element);
  QTreeWidgetItem *iq = new QTreeWidgetItem (
      QStringList () << QString (mode_i_of_t ? "I(t)" : "I(q)"));
  element->addChild (iq);
  ...
  for (int i = 0; i < (int)mals_selected_files.size (); ++i)
    {
      lvi = new QTreeWidgetItem (iq, lvi);
      lvi->setText (0, mals_selected_files[i]);
    }
  ...
  pb_clear->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  pb_to_mals = new QPushButton (us_tr ("To HPLC window"), this);
  pb_to_mals->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize - 1));
  pb_to_mals->setMinimumHeight (minHeight3);
  pb_to_mals->setPalette (PALET_PUSHB);
  connect (pb_to_mals, SIGNAL (clicked ()), SLOT (to_mals ()));
  data_widgets.push_back (pb_to_mals);

  pb_save_plots = new QPushButton (us_tr ("Save Plots"), this);
  pb_save_plots->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  ...
  pb_replot->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize - 1));
  ...
  AUTFBACK (lbl_modes);
  lbl_modes->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize - 1));

  bg_modes = new QButtonGroup (this);

  rb_mode_iqit = new QRadioButton ("Data", this);
  rb_mode_iqit->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize - 1));
  ...
  rb_mode_svd->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  ...
  rb_mode_efa->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  ...
  rb_mode_efa_decomp->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize - 1));
  rb_mode_efa_decomp->setMinimumHeight (minHeight3);
  rb_mode_efa_decomp->setPalette (PALET_NORMAL);
  AUTFBACK (rb_mode_efa_decomp);
  connect (rb_mode_efa_decomp, SIGNAL (clicked ()),
           SLOT (set_mode_efa_decomp ()));
  ...
  connect ((QWidget *)plot_data->titleLabel (),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_data (const QPoint &)));
  ((QWidget *)plot_data->titleLabel ())
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)plot_data->axisWidget (QwtPlot::yLeft),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_data (const QPoint &)));
  ((QWidget *)plot_data->axisWidget (QwtPlot::yLeft))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)plot_data->axisWidget (QwtPlot::xBottom),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_data (const QPoint &)));
  ((QWidget *)plot_data->axisWidget (QwtPlot::xBottom))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  ...
  plot_data->setAxisFont (QwtPlot::yLeft,
                          QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  plot_data->setAxisFont (QwtPlot::xBottom,
                          QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  plot_data->setAxisFont (QwtPlot::yRight,
                          QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  ...
  // connect( plot_data_zoomer, SIGNAL( zoomed( const QRectF & ) ), SLOT(
  // plot_data_zoomed( const QRectF & ) ) );
  ...
  // us_tr("q^2 (1/Angstrom^2)") : */  us_tr("q [1/Angstrom]" )); // or Time or
  // Frame"));
  ...
  plot_errors->setAxisFont (QwtPlot::xBottom,
                            QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize - 1));
  plot_errors->setAxisFont (QwtPlot::yRight,
                            QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize - 1));
  ...
  // plot_errors_zoomed( const QRectF & ) ) );
  ...
  cb_plot_errors->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize - 1));
  ...
  cb_plot_errors_ref->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize - 1));
  cb_plot_errors_ref->setPalette (PALET_NORMAL);
  AUTFBACK (cb_plot_errors_ref);
  connect (cb_plot_errors_ref, SIGNAL (clicked ()),
           SLOT (set_plot_errors_ref ()));
  ...
  connect ((QWidget *)plot_svd->titleLabel (),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_svd (const QPoint &)));
  ((QWidget *)plot_svd->titleLabel ())
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)plot_svd->axisWidget (QwtPlot::yLeft),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_svd (const QPoint &)));
  ((QWidget *)plot_svd->axisWidget (QwtPlot::yLeft))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)plot_svd->axisWidget (QwtPlot::xBottom),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_svd (const QPoint &)));
  ((QWidget *)plot_svd->axisWidget (QwtPlot::xBottom))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  grid_data = new QwtPlotGrid;
  grid_data->enableXMin (true);
  grid_data->enableYMin (true);
  ...
  plot_svd->setAxisFont (QwtPlot::yLeft,
                         QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize - 1));
  plot_svd->setAxisFont (QwtPlot::xBottom,
                         QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize - 1));
  plot_svd->setAxisFont (QwtPlot::yRight,
                         QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize - 1));
  ...
  // connect( plot_svd_zoomer, SIGNAL( zoomed( const QRectF & ) ), SLOT(
  // plot_svd_zoomed( const QRectF & ) ) );
  ...
  connect ((QWidget *)plot_ac->titleLabel (),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_ac (const QPoint &)));
  ((QWidget *)plot_ac->titleLabel ())
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)plot_ac->axisWidget (QwtPlot::yLeft),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_ac (const QPoint &)));
  ((QWidget *)plot_ac->axisWidget (QwtPlot::yLeft))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)plot_ac->axisWidget (QwtPlot::xBottom),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_ac (const QPoint &)));
  ((QWidget *)plot_ac->axisWidget (QwtPlot::xBottom))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  ...
  plot_ac->setAxisFont (QwtPlot::yLeft,
                        QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  plot_ac->setAxisFont (QwtPlot::xBottom,
                        QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  plot_ac->setAxisFont (QwtPlot::yRight,
                        QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  ...
  // connect( plot_ac_zoomer, SIGNAL( zoomed( const QRectF & ) ), SLOT(
  // plot_ac_zoomed( const QRectF & ) ) );
  ...
  connect ((QWidget *)plot_lefa->titleLabel (),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_lefa (const QPoint &)));
  ((QWidget *)plot_lefa->titleLabel ())
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)plot_lefa->axisWidget (QwtPlot::yLeft),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_lefa (const QPoint &)));
  ((QWidget *)plot_lefa->axisWidget (QwtPlot::yLeft))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)plot_lefa->axisWidget (QwtPlot::xBottom),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_lefa (const QPoint &)));
  ((QWidget *)plot_lefa->axisWidget (QwtPlot::xBottom))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  ...
  AUTFBACK (plot_lefa);
  grid_lefa->setMajorPen (
      QPen (USglobal->global_colors.major_ticks, 0, Qt::DotLine));
  grid_lefa->setMinorPen (
      QPen (USglobal->global_colors.minor_ticks, 0, Qt::DotLine));
  ...
  plot_lefa->setAxisFont (QwtPlot::yLeft,
                          QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  plot_lefa->setAxisFont (QwtPlot::xBottom,
                          QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  plot_lefa->setAxisFont (QwtPlot::yRight,
                          QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  ...
  // connect( plot_lefa_zoomer, SIGNAL( zoomed( const QRectF & ) ), SLOT(
  // plot_lefa_zoomed( const QRectF & ) ) );
  ...
  connect ((QWidget *)plot_refa->titleLabel (),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_refa (const QPoint &)));
  ((QWidget *)plot_refa->titleLabel ())
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)plot_refa->axisWidget (QwtPlot::yLeft),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_refa (const QPoint &)));
  ((QWidget *)plot_refa->axisWidget (QwtPlot::yLeft))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)plot_refa->axisWidget (QwtPlot::xBottom),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_refa (const QPoint &)));
  ((QWidget *)plot_refa->axisWidget (QwtPlot::xBottom))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  ...
  AUTFBACK (plot_refa);
  grid_refa->setMajorPen (
      QPen (USglobal->global_colors.major_ticks, 0, Qt::DotLine));
  grid_refa->setMinorPen (
      QPen (USglobal->global_colors.minor_ticks, 0, Qt::DotLine));
  ...
  plot_refa->setAxisFont (QwtPlot::yLeft,
                          QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  plot_refa->setAxisFont (QwtPlot::xBottom,
                          QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  plot_refa->setAxisFont (QwtPlot::yRight,
                          QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  ...
  // connect( plot_refa_zoomer, SIGNAL( zoomed( const QRectF & ) ), SLOT(
  // plot_refa_zoomed( const QRectF & ) ) );
  efa_widgets.push_back (plot_refa);

  lbl_efas = new QLabel (
      us_tr ("Number of SV's for EFA plots and decomposition:"), this);
  lbl_efas->setAlignment (Qt::AlignCenter | Qt::AlignVCenter);
  lbl_efas->setPalette (PALET_NORMAL);
  AUTFBACK (lbl_efas);
  lbl_efas->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize - 1));
  ...
  qwtc_efas->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  AUTFBACK (qwtc_efas);
  connect (qwtc_efas, SIGNAL (valueChanged (double)),
           SLOT (update_efas (double)));
  ...
  for (int i = 0; i < UHSHS_MAX_SV_FOR_EFA; ++i)
    {
      {
        QLabel *tmp_lbl = new QLabel (
            QString (us_tr ("S.V. %1 Start:")).arg (i + 1), this);
        tmp_lbl->setAlignment (Qt::AlignCenter | Qt::AlignVCenter);
        tmp_lbl->setPalette (PALET_NORMAL);
        AUTFBACK (tmp_lbl);
        tmp_lbl->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
        tmp_lbl->hide ();
...

      {
        ...
        tmp_qwtc->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize));
        ...
        AUTFBACK (tmp_qwtc);
        tmp_qwtc->hide ();
        connect (tmp_qwtc, SIGNAL (valueChanged (double)),
                 SLOT (update_efa_range_start (double)));
      }
...

      {
        ...
        tmp_qwtc->hide ();
        connect (tmp_qwtc, SIGNAL (valueChanged (double)),
                 SLOT (update_efa_range_end (double)));
      }
    }
  ...
  connect ((QWidget *)plot_efa_decomp->titleLabel (),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_efa_decomp (const QPoint &)));
  ((QWidget *)plot_efa_decomp->titleLabel ())
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)plot_efa_decomp->axisWidget (QwtPlot::yLeft),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_efa_decomp (const QPoint &)));
  ((QWidget *)plot_efa_decomp->axisWidget (QwtPlot::yLeft))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)plot_efa_decomp->axisWidget (QwtPlot::xBottom),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_efa_decomp (const QPoint &)));
  ((QWidget *)plot_efa_decomp->axisWidget (QwtPlot::xBottom))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  ...
  grid_efa_decomp->setMajorPen (
      QPen (USglobal->global_colors.major_ticks, 0, Qt::DotLine));
  grid_efa_decomp->setMinorPen (
      QPen (USglobal->global_colors.minor_ticks, 0, Qt::DotLine));
  ...
  plot_efa_decomp->setAxisFont (QwtPlot::yLeft,
                                QFont (USglobal->config_list.fontFamily,
                                       USglobal->config_list.fontSize - 1));
  plot_efa_decomp->setAxisFont (QwtPlot::xBottom,
                                QFont (USglobal->config_list.fontFamily,
                                       USglobal->config_list.fontSize - 1));
  plot_efa_decomp->setAxisFont (QwtPlot::yRight,
                                QFont (USglobal->config_list.fontFamily,
                                       USglobal->config_list.fontSize - 1));
  //    plot_efa_decomp->setMargin(USglobal->config_list.margin);
  set_title (plot_efa_decomp, "EFA Deconvolution");

  plot_efa_decomp->setAxisScaleEngine (QwtPlot::yLeft,
                                       new QwtLinearScaleEngine);
  plot_efa_decomp->setCanvasBackground (USglobal->global_colors.plot);

  // connect( plot_efa_decomp_zoomer, SIGNAL( zoomed( const QRectF & ) ), SLOT(
  // plot_efa_decomp_zoomed( const QRectF & ) ) );
  ...
  cb_efa_decomp_force_positive->setFont (QFont (
  ...
  connect (cb_efa_decomp_force_positive, SIGNAL (clicked ()),
           SLOT (set_efa_decomp_force_positive ()));
  efa_decomp_widgets.push_back (cb_efa_decomp_force_positive);

  pb_efa_decomp_to_mals_saxs
      = new QPushButton (us_tr ("To HPLC window"), this);
  pb_efa_decomp_to_mals_saxs->setFont (QFont (
      USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1));
  pb_efa_decomp_to_mals_saxs->setMinimumHeight (minHeight1);
  pb_efa_decomp_to_mals_saxs->setPalette (PALET_PUSHB);
  connect (pb_efa_decomp_to_mals_saxs, SIGNAL (clicked ()),
           SLOT (efa_decomp_to_mals_saxs ()));
  efa_decomp_widgets.push_back (pb_efa_decomp_to_mals_saxs);

  iq_it_state = mode_i_of_t;
  pb_iq_it = new QPushButton (us_tr (mode_i_of_t ? "Show I(q)" : "Show I(t)"),
                              this);
  pb_iq_it->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize - 1));
  ...
  lbl_process->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1,
                               QFont::Bold));
  ...
  cb_norm_pw->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize - 1));
  ...
  cb_norm_avg->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  ...
  lbl_q_range->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  // process_widgets.push_back( lbl_q_range );
  lbl_q_range->hide ();

  le_q_start = new mQLineEdit (this);
  le_q_start->setObjectName ("le_q_start Line Edit");
  le_q_start->setText ("");
  le_q_start->setAlignment (Qt::AlignCenter | Qt::AlignVCenter);
  le_q_start->setPalette (PALET_NORMAL);
  AUTFBACK (le_q_start);
  le_q_start->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize - 1));
  le_q_start->setEnabled (false);
  le_q_start->setValidator (new QDoubleValidator (le_q_start));
  connect (le_q_start, SIGNAL (textChanged (const QString &)),
           SLOT (q_start_text (const QString &)));
  // process_widgets.push_back( le_q_start );
  le_q_start->hide ();

  le_q_end = new mQLineEdit (this);
  le_q_end->setObjectName ("le_q_end Line Edit");
  le_q_end->setText ("");
  le_q_end->setAlignment (Qt::AlignCenter | Qt::AlignVCenter);
  le_q_end->setPalette (PALET_NORMAL);
  AUTFBACK (le_q_end);
  le_q_end->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize - 1));
  le_q_end->setEnabled (false);
  le_q_end->setValidator (new QDoubleValidator (le_q_end));
  connect (le_q_end, SIGNAL (textChanged (const QString &)),
           SLOT (q_end_text (const QString &)));
  ...
  //    lbl_t_range->setFont(QFont( USglobal->config_list.fontFamily,
  //    USglobal->config_list.fontSize - 1)); process_widgets.push_back(
  //    lbl_t_range );

  //    le_t_start = new mQLineEdit( this );     le_t_start->setObjectName(
  //    "le_t_start Line Edit" ); le_t_start->setText( "" );
  //    le_t_start->setAlignment(Qt::AlignCenter|Qt::AlignVCenter);
  //    le_t_start->setPalette( PALET_NORMAL );
  //    le_t_start->setFont(QFont( USglobal->config_list.fontFamily,
  //    USglobal->config_list.fontSize - 1)); le_t_start->setEnabled( false );
  //    le_t_start->setValidator( new QDoubleValidator( le_t_start ) );
  //    connect( le_t_start, SIGNAL( textChanged( const QString & ) ), SLOT(
  //    t_start_text( const QString & ) ) ); process_widgets.push_back(
  //    le_t_start );

  //    le_t_end = new mQLineEdit( this );     le_t_end->setObjectName(
  //    "le_t_end Line Edit" ); le_t_end->setText( "" );
  //    le_t_end->setAlignment(Qt::AlignCenter|Qt::AlignVCenter);
  //    le_t_end->setPalette( PALET_NORMAL );
  //    le_t_end->setFont(QFont( USglobal->config_list.fontFamily,
  //    USglobal->config_list.fontSize - 1)); le_t_end->setEnabled( false );
  //    le_t_end->setValidator( new QDoubleValidator( le_t_end ) );
  //    connect( le_t_end, SIGNAL( textChanged( const QString & ) ), SLOT(
  //    t_end_text( const QString & ) ) ); process_widgets.push_back( le_t_end
  //    );
  ...
  AUTFBACK (lbl_ev);
  lbl_ev->setFont (QFont (USglobal->config_list.fontFamily,
                          USglobal->config_list.fontSize));
  ...
  pb_svd->setToolTip (us_tr (
      "SVD is only available in I(q) mode, adjust the plot type to use."));
  connect (pb_svd, SIGNAL (clicked ()), SLOT (svd ()));
  process_widgets.push_back (pb_svd);

  pb_efa = new QPushButton (us_tr ("Compute EFA"), this);
  pb_efa->setFont (QFont (USglobal->config_list.fontFamily,
                          USglobal->config_list.fontSize - 1));
  ...
  pb_efa_decomp->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  ...
  // pb_svd_plot->setFont(QFont( USglobal->config_list.fontFamily,
  // USglobal->config_list.fontSize - 1 ));
  ...
  pb_svd_save->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  ...
  pb_recon->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize - 1));
  pb_recon->setMinimumHeight (minHeight3);
  pb_recon->setPalette (PALET_PUSHB);
  pb_recon->setToolTip (us_tr ("Reconstruction is only enabled when all "
                               "normalization off and SVs are selected"));
  ...
  pb_indiv_recon->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize - 1));
  pb_indiv_recon->setMinimumHeight (minHeight3);
  pb_indiv_recon->setPalette (PALET_PUSHB);
  pb_indiv_recon->setToolTip (
      us_tr ("Reconstruction is only enabled when all normalization off and "
             "SVs are selected"));
  ...
  pb_inc_recon->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize - 1));
  pb_inc_recon->setMinimumHeight (minHeight3);
  pb_inc_recon->setPalette (PALET_PUSHB);
  pb_inc_recon->setToolTip (us_tr ("Reconstruction is only enabled when all "
                                   "normalization off and SVs are selected"));
  ...
  pb_inc_chi_plot->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize - 1));
  ...
  pb_inc_rmsd_plot->setFont (QFont (USglobal->config_list.fontFamily,
                                    USglobal->config_list.fontSize - 1));
  ...
  pb_rmsd_save->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize - 1));
  ...
  // ----- left side
  {
    ...
    }
    {
      ...
      bl_buttons->setContentsMargins (0, 0, 0, 0);
      bl_buttons->setSpacing (0);
      bl_buttons->addWidget (pb_to_mals);
    ...
    bl->addWidget (lbl_process);
    {
      ...
      // gl->addWidget( lb_ev         , 3 , 1 , 1 + ( 3 ) - ( 3 ) , 1 + ( 2  )
      // - ( 1 ) );
  ...
  // ----- right side
  {
    ...
    bl->addWidget (plot_efa_decomp);
    {
      ...
      bl_buttons->addWidget (pb_efa_decomp_to_mals_saxs);
      bl->addLayout (bl_buttons);
    }

    //       {
    //          QGridLayout * gl = new QGridLayout( 0 );
    //          gl->setContentsMargins( 0, 0, 0, 0 ); gl->setSpacing( 0 ); gl
    //          -> addWidget( cb_plot_errors       , 0, 0 ); gl -> addWidget(
    //          cb_plot_errors_sd    , 0, 1 ); gl -> addWidget(
    //          cb_plot_errors_pct   , 0, 2 ); gl -> addWidget(
    //          cb_plot_errors_rev   , 1, 0 ); gl -> addWidget(
    //          cb_plot_errors_ref   , 1, 1 ); gl -> addWidget(
    //          cb_plot_errors_group , 1, 2 );
  ...
  hide_widgets (data_widgets,
                !ush_win->gparams.count ("mals_svd_data_widgets")
                        || ush_win->gparams["mals_svd_data_widgets"] == "false"
                    ? false
                    : true);
  hide_widgets (editor_widgets,
                !ush_win->gparams.count ("mals_svd_editor_widgets")
                        || ush_win->gparams["mals_svd_editor_widgets"]
                               == "false"
                    ? false
                    : true);
  hide_widgets (process_widgets,
                !ush_win->gparams.count ("mals_svd_process_widgets")
                        || ush_win->gparams["mals_svd_process_widgets"]
                               == "false"
                    ? false
                    : true);
...
void
US_Hydrodyn_Mals_Svd::cancel ()
{
  close ();
}

void
US_Hydrodyn_Mals_Svd::help ()
{
  US_Help *online_help;
  online_help = new US_Help (this);
  online_help->show_help ("manual/somo/somo_mals_svd.html");
}

void
US_Hydrodyn_Mals_Svd::closeEvent (QCloseEvent *e)
{
  e->accept ();
}

void
US_Hydrodyn_Mals_Svd::clear_display ()
{
  editor->clear ();
  editor->append ("\n\n");
}
...
void
US_Hydrodyn_Mals_Svd::update_font ()
{
  bool ok;
  QFont newFont;
...
void
US_Hydrodyn_Mals_Svd::editor_msg (QString color, QString msg)
{
...
void
US_Hydrodyn_Mals_Svd::plot_files ()
{
  // puts( "plot_files" );
  plot_data->detachItems (QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
...
void
US_Hydrodyn_Mals_Svd::plot_files (QStringList add_files, norms norm_mode)
{
  // puts( "plot_files" );
  plot_data->detachItems (QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems (QwtPlotItem::Rtti_PlotMarker);
...
bool
US_Hydrodyn_Mals_Svd::plot_file (QString file, double &minx, double &maxx,
                                 double &miny, double &maxy, norms norm_mode)
{
  ...
  if (!f_qs_string.count (file) || !f_qs.count (file)
      || !(*f_Iuse).count (file) || !f_pos.count (file))
    {
      editor_msg (
          "red",
          QString (
              us_tr (
                  "Internal error: request to plot %1, but not found in data"))
              .arg (file));
      return false;
    }
  ...
  if (!axis_y_log)
    {
      curve->setSamples ((double *)&(f_qs[file][0]),
                         (double *)&((*f_Iuse)[file][0]), q_points);

      curve->setPen (QPen (plot_colors[f_pos[file] % plot_colors.size ()],
  ...
  else
    {
      ...
      curve->setSamples (
          ...
          (double *)&(q[0]), (double *)&(I[0]), q_points);
...
bool
US_Hydrodyn_Mals_Svd::get_min_max (QString file, double &minx, double &maxx,
                                   double &miny, double &maxy, norms norm_mode)
{
  map<QString, vector<double> > *f_Iuse = &f_Is;
  ...
  if (!(*f_Iuse).count (file) || !f_pos.count (file))
    {
      // editor_msg( "red", QString( us_tr( "Internal error: requested %1, but
      // not found in data" ) ).arg( file ) );
      return false;
    }
...
// void US_Hydrodyn_Mals_Svd::plot_data_zoomed( const QRectF & /* rect */ )
// {
...
// void US_Hydrodyn_Mals_Svd::plot_errors_zoomed( const QRectF & /* rect */ )
// {
//    //   cout << QString( "zoomed: %1 %2 %3 %4\n" )
//    // .arg( rect.x1() )
//    // .arg( rect.x2() )
...
void
US_Hydrodyn_Mals_Svd::disable_all ()
{
  lv_data->setEnabled (false);
  pb_clear->setEnabled (false);
  pb_to_mals->setEnabled (false);
...
void
US_Hydrodyn_Mals_Svd::q_start_text (const QString &)
{
}

void
US_Hydrodyn_Mals_Svd::q_end_text (const QString &)
{
}

// void US_Hydrodyn_Mals_Svd::t_start_text( const QString & )
// {
// }

// void US_Hydrodyn_Mals_Svd::t_end_text( const QString & )
// {
// }

void
US_Hydrodyn_Mals_Svd::replot (bool keep_mode)
{
  disable_all ();
  ...
  if (plot_data_zoomer)
    {
      ...
      // plot_data_zoomer->zoomRectIndex() );
  ...
  if (plot_errors_zoomer)
    {
      // cout << QString( "plot zoomer stack size %1\n" ).arg(
      // plot_errors_zoomer->zoomRectIndex() );
  ...
  if ((mode_i_of_t && !iq_it_state) || (!mode_i_of_t && iq_it_state))
    {
...
void
US_Hydrodyn_Mals_Svd::to_mals ()
{
  if (!ush_win->mals_widget)
    {
      editor_msg ("red", us_tr ("US-SOMO MALS window has been closed"));
      return;
    }
  ...
  for (int i = 0; i < (int)files.size (); ++i)
    {
      mals_win->add_plot (files[i], f_qs[files[i]], f_Is[files[i]],
                          f_errors[files[i]], iq_it_state, false);
    }
  update_enables ();
}

void
US_Hydrodyn_Mals_Svd::axis_x ()
{
...
void
US_Hydrodyn_Mals_Svd::hide_data ()
{
  cout << QString ("hide data size %1\n").arg (data_widgets.size ());
  hide_widgets (data_widgets, data_widgets[0]->isVisible ());

  ush_win->gparams["mals_svd_data_widgets"]
      = data_widgets[0]->isVisible () ? "visible" : "hidden";
}

void
US_Hydrodyn_Mals_Svd::hide_editor ()
{
  ...
  ush_win->gparams["mals_svd_editor_widgets"]
      = editor_widgets[0]->isVisible () ? "visible" : "hidden";
}

void
US_Hydrodyn_Mals_Svd::hide_process ()
{
  ...
  ush_win->gparams["mals_svd_process_widgets"]
      = process_widgets[0]->isVisible () ? "visible" : "hidden";
}

void
US_Hydrodyn_Mals_Svd::hide_widgets (vector<QWidget *> widgets, bool hide)
{
...
void
US_Hydrodyn_Mals_Svd::update_enables ()
{
  // count selections
  if (running)
    {
  ...
  pb_to_mals->setEnabled (files.size ()
                          && !sources.count (us_tr ("Original data"))
                          && ush_win->mals_widget);
  ...
  pb_svd->setEnabled (files.size () && mode_i_of_t == iq_it_state
                      && sources.size () == 1
                      && sources.count (us_tr ("Original data")));
  ...
  if (sources.size () == 1 && sources.begin ()->contains ("reconstruction"))
    {
      cb_plot_errors->show ();
      if (cb_plot_errors->isChecked () && !errors_widgets[0]->isVisible ())
        {
...
void
US_Hydrodyn_Mals_Svd::data_selection_changed ()
{
  update_enables ();
}

void
US_Hydrodyn_Mals_Svd::sv_selection_changed ()
{
  update_enables ();
}

void
US_Hydrodyn_Mals_Svd::clear ()
{
...
QStringList
US_Hydrodyn_Mals_Svd::selected_files ()
{
  QStringList result;
  ...
  while ((*it))
    {
      ...
      if (is_selected (item) && US_Static::lvi_depth (item) == 2
...
set<QString>
US_Hydrodyn_Mals_Svd::get_current_files ()
{
  set<QString> result;
...
set<QString>
US_Hydrodyn_Mals_Svd::get_sources ()
{
  set<QString> result;
...
set<QString>
US_Hydrodyn_Mals_Svd::get_selected_sources ()
{
  set<QString> result;
  QString iq_or_it = iq_it_state ? "I(t)" : "I(q)";
...
QTreeWidgetItem *
US_Hydrodyn_Mals_Svd::get_source_item (QString source)
{
  QTreeWidgetItemIterator it (lv_data);
  while ((*it))
    {
      QTreeWidgetItem *item = (*it);
      if (US_Static::lvi_depth (item) == 0 && item->text (0) == source)
        {
          return item;
        }
      ++it;
...
int
US_Hydrodyn_Mals_Svd::selected_sources ()
{
  QString iq_or_it = iq_it_state ? "I(t)" : "I(q)";
...
void
US_Hydrodyn_Mals_Svd::clean_selected ()
{
...
bool
US_Hydrodyn_Mals_Svd::all_children_selected (QTreeWidgetItem *lvi)
{
...
bool
US_Hydrodyn_Mals_Svd::is_selected (QTreeWidgetItem *lvi)
{
  if (lvi->isSelected ())
    {
      return true;
...
void
US_Hydrodyn_Mals_Svd::axis_x_title ()
{
  ...
  if (axis_x_log)
    {
      plot_data->setAxisTitle (QwtPlot::xBottom,
                               title + us_tr (" (log scale)"));
      plot_data->setAxisScaleEngine (QwtPlot::xBottom,
...
void
US_Hydrodyn_Mals_Svd::axis_y_title ()
{
  ...
  if (axis_y_log)
    {
      ...
      plot_data->setAxisScaleEngine (QwtPlot::yLeft,
                                     new QwtLogScaleEngine (10));
    }
  else
    {
...
bool
US_Hydrodyn_Mals_Svd::plotted_matches_selected ()
{
  set<QString> plotted;
  ...
  for (map<QString, QwtPlotCurve *>::iterator it = plotted_curves.begin ();
       it != plotted_curves.end (); ++it)
    {
      plotted.insert (it->first);
    }

  if (cb_plot_errors->isChecked () && cb_plot_errors_ref->isChecked ()
      && plot_errors->isVisible ())
    {
      for (set<QString>::iterator it = plot_errors_reference.begin ();
           it != plot_errors_reference.end (); ++it)
        {
          selected.insert (*it);
        }
    }
...
void
US_Hydrodyn_Mals_Svd::add_i_of_q_or_t (QString source, QStringList files,
                                       bool do_update_enables)
{
  editor_msg ("blue", QString (us_tr ("Making I(%1) for source %2"))
                          .arg (mode_i_of_t ? "q" : "t")
                          .arg (source));
  ...
  if (!source_item)
    {
      editor_msg (
          "red", QString (us_tr ("Internal error: source item not defined %1"))
                     .arg (source));
      return;
    }
  ...
  QString tail = mals_win->qstring_common_tail (files, true);
  ...
  for (unsigned int i = 0; i < (unsigned int)q.size (); i++)
    {
      QString basename = QString ("%1_I%2_%3%4")
                             .arg (head)
                             .arg (mode_i_of_t ? "q" : "t")
                             .arg (mode_i_of_t ? "t" : "q")
                             .arg (q[i]);
      ...
      for (map<double, map<double, double> >::iterator it = I_values.begin ();
           it != I_values.end (); it++)
        {
          ...
          if (e_values.count (it->first) && e_values[it->first].count (q[i]))
            {
  ...
  editor_msg ("blue", QString (us_tr ("Done making I(%1) for source %2"))
                          .arg (mode_i_of_t ? "q" : "t")
                          .arg (source));
}

void
US_Hydrodyn_Mals_Svd::rescale (bool do_update_enables)
{
  // bool any_selected = false;
  double minx = 0e0;
  double maxx = 1e0;
  ...
  for (int i = 0; i < (int)files.size (); ++i)
    {
      if (1)
        {
          // any_selected = true;
          if (get_min_max (files[i], file_minx, file_maxx, file_miny,
                           file_maxy))
            {
              if (first)
                {
...
void
US_Hydrodyn_Mals_Svd::svd_plot (bool axis_change)
{
...

  {
    ...
    curve->setSamples ((double *)&(svd_x[0]), (double *)&(svd_y[0]),
                       svd_x.size ());

    curve->setSymbol (new QwtSymbol (
        QwtSymbol::Cross, QBrush (QColor (plot_colors[0]), Qt::SolidPattern),
    ...
    plot_svd->setAxisScale (QwtPlot::yLeft, svd_y.back () * 0.95e0,
                            svd_y[0] * 1.1e0);
  ...
  if (svd_autocor_U.size () < svd_x.size ())
    {
      editor_msg ("red", us_tr ("Internal error: size mismatch autocorr U. "
    ...
    }
  {
    ...
    QwtPlotCurve *curve
        = new QwtPlotCurve ("q-Space Singular Vectors (U, Left)");
    curve->setStyle (QwtPlotCurve::Lines);

    curve->setSamples ((double *)&(svd_x[0]), (double *)&(svd_autocor_U[0]),
                       svd_x.size ());
  ...
  if (svd_autocor_V.size () < svd_x.size ())
    {
      editor_msg ("red", us_tr ("Internal error: size mismatch autocorr V. "
                                "Please inform the developers"));
      update_enables ();
      return;
    }
  {
    ...
    curve->setSamples ((double *)&(svd_x[0]), (double *)&(svd_autocor_V[0]),
                       svd_x.size ());

    curve->setSymbol (new QwtSymbol (
        QwtSymbol::Cross, QBrush (QColor (plot_colors[1]), Qt::SolidPattern),
...
US_Hydrodyn_Mals_Svd::svd ()
{
  ...
  lbl_ev->setText (
      QString (us_tr ("Singular value list for %1")).arg (last_svd_name));
...
#define DEBUG_MN
#if defined(DEBUG_MN)
  qDebug () << "US_Hydrodyn_Mals_Svd::svd rows " << m << " cols " << n;
  ...
  US_Vector::printvector3 ("SVD x autocor U, V", svd_x, svd_autocor_U,
                           svd_autocor_V);
#endif
...
QTreeWidgetItem *
US_Hydrodyn_Mals_Svd::lvi_last_depth (int d)
{
...
void
US_Hydrodyn_Mals_Svd::color_rotate ()
{
...
void
US_Hydrodyn_Mals_Svd::inc_rmsd_plot (bool axis_change)
{
  plot_data->detachItems (QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
  ...
  curve->setSamples ((double *)&(rmsd_x[0]), (double *)&(rmsd_y[0]),
                     rmsd_x.size ());
  ...
  plot_data->setAxisScale (QwtPlot::yLeft, vmin (rmsd_y) * 0.9e0,
                           vmax (rmsd_y) * 1.1e0);
...
void
US_Hydrodyn_Mals_Svd::inc_chi_plot (bool axis_change)
{
  plot_data->detachItems (QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
  ...
  curve->setSamples ((double *)&(chi_x[0]), (double *)&(chi_y[0]),
                     chi_x.size ());
  ...
  plot_data->setAxisScale (QwtPlot::yLeft, vmin (chi_y) * 0.9e0,
                           vmax (chi_y) * 1.1e0);
...
void
US_Hydrodyn_Mals_Svd::inc_recon ()
{
  ...
  last_recon_tag = QString (us_tr ("\"Incremental TSVD on SVD of %1\",\"RMSD "
                                   "of fit\",\"Singular values\""))
                       .arg (last_svd_name);
  ...
  for (int i = 0; i < (int)evs_selected.size (); ++i)
    {
      ...
      progress->setValue (i);
      progress->setMaximum (evs_selected.size ());
      ...
      for (int j = 0; j <= i; ++j)
        {
          ...
          evs_used
              += QString ("%1 ").arg (lb_ev->item (evs_selected[j])->text ());
        }
      qApp->processEvents ();
...
US_Hydrodyn_Mals_Svd::do_recon ()
{
  ...
  QString name = QString ("TSVD %1reconstruction %2 SVs%3")
                     .arg (recon_mode)
                     .arg (sv_count)
                     .arg (sv_count == 1 ? QString (" %1").arg (last_ev)
                                         : QString (""));

  {
    ...
    while (sources.count (name))
      {
        name = QString ("TSVD %1reconstruction %2 SVs%3 Trial %4")
                   .arg (recon_mode)
                   .arg (sv_count)
                   .arg (sv_count == 1 ? QString (" %1").arg (last_ev)
                                       : QString (""))
  ...
  QTreeWidgetItem *iqs
      = new QTreeWidgetItem (lvi, US_Static::lv_lastItem (lv_data));
  ...
  QString tag
      = QString ("TSVD%1_%2")
            .arg (sv_count)
            .arg (sv_count == 1
                      ? QString ("%1_").arg (last_ev).replace (".", "_")
                      : QString (""));
  // QTextStream( stdout ) <<
...
double
US_Hydrodyn_Mals_Svd::vmin (vector<double> &x)
{
  double min = x[0];
...
double
US_Hydrodyn_Mals_Svd::vmax (vector<double> &x)
{
  if (!x.size ())
    {
      editor_msg ("red", us_tr ("Internal error: vmax called with empty "
                                "vector. Please inform the developers."));
      return -1e99;
    }
...
void
US_Hydrodyn_Mals_Svd::indiv_recon ()
{
  recon_mode = "Indiv. ";
  ...
  last_recon_tag = QString (us_tr ("\"Individual TSVD on SVD of %1\",\"RMSD "
                                   "of fit\",\"Singular value\""))
                       .arg (last_svd_name);
  last_recon_evs.clear ();
  ...
  for (int i = 0; i < (int)evs_selected.size (); ++i)
    {
      ...
      last_recon_evs << QString ("%1").arg (
          lb_ev->item (evs_selected[i])->text ());
      rmsd_x.push_back (i + 1e0);
      rmsd_y.push_back (last_recon_rmsd);
...
void
US_Hydrodyn_Mals_Svd::set_plot_errors ()
{
...
void
US_Hydrodyn_Mals_Svd::set_plot_errors_rev ()
{
  update_plot_errors ();
}

void
US_Hydrodyn_Mals_Svd::set_plot_errors_sd ()
{
  if (plot_errors_zoomer)
    {
      plot_errors_zoomer->zoom (0);
  ...
  if (cb_plot_errors_sd->isChecked () && cb_plot_errors_pct->isChecked ())
    {
      cb_plot_errors_pct->setChecked (false);
    }
  update_plot_errors ();
...
void
US_Hydrodyn_Mals_Svd::set_plot_errors_pct ()
{
  if (plot_errors_zoomer)
    {
      plot_errors_zoomer->zoom (0);
...
void
US_Hydrodyn_Mals_Svd::set_plot_errors_group ()
{
  if (plot_errors_zoomer)
    {
      plot_errors_zoomer->zoom (0);
...
void
US_Hydrodyn_Mals_Svd::set_plot_errors_ref ()
{
...
void
US_Hydrodyn_Mals_Svd::update_plot_errors (bool do_update_enables)
{
  ...
  if (cb_plot_errors->isChecked ())
    {
      ...
      cb_plot_errors_group->isChecked () ? do_plot_errors_group ()
                                         : do_plot_errors ();
    }
  if (do_update_enables)
    {
...
QStringList
US_Hydrodyn_Mals_Svd::add_subset_data (QStringList files)
{
  QStringList result;
  ...
  if (sel_sources.size () != 1)
    {
      editor_msg ("red", QString (us_tr ("Internal error: add_subset_data "
                                         "with more than one source (%1)"))
                             .arg (sel_sources.size ()));
      return result;
    }

  QString name
      = QString (us_tr ("Subset of %1")).arg (*(sel_sources.begin ()));

  {
    int ext = 0;
    while (sources.count (name))
      {
        name = QString (us_tr ("Subset %1 of %2"))
                   .arg (++ext)
                   .arg (*(sel_sources.begin ()));
      }
  }

#if QT_VERSION < 0x040000
  QTreeWidgetItem *lvi
      = new QTreeWidgetItem (lv_data, lvi_last_depth (0), name);

  // copy over I(q), ignore SVs, rmsd since these are not computed

  QTreeWidgetItem *iqs = new QTreeWidgetItem (
      lvi, US_Static::lv_lastItem (lv_data), mode_i_of_t ? "I(t)" : "I(q)");
#else
  QTreeWidgetItem *lvi = new QTreeWidgetItem (lv_data, lvi_last_depth (0));
  lvi->setText (0, name);
  // copy over I(q), ignore SVs, rmsd since these are not computed
      ...
      for (int i = 0; i < (int)files.size (); ++i)
        {
          QString this_name
              = tag + (ext ? QString ("%1_").arg (ext) : QString (""))
                + last_svd_data[i].mid (head.length (),
                                        files[i].length () - head.length ());
          if (current_files.count (this_name))
            {
              ext++;
              any_found = true;
...
QString
US_Hydrodyn_Mals_Svd::get_related_source_name (QString name)
{
...
QStringList
US_Hydrodyn_Mals_Svd::get_files_by_name (QString name)
{
...
bool
US_Hydrodyn_Mals_Svd::get_name_norm_mode (QString fullname, QString &name,
                                          norms &norm_mode)
{
  ...
  for (int i = NORM_NOT; i <= NORM_AVG; ++i)
    {
      ...
      // QTextStream( stdout ) << "get_name_norm_mode: checking for '" <<
      // norm_name_map[ static_cast<norms>(i) ] << "'\n";
      if (fullname.contains (rx))
        {
          norm_mode = static_cast<norms> (i);
          name = fullname.replace (rx, "");
          // QTextStream( stdout ) << "get_name_norm_mode() matched, returning
          // true, name is now: '" << name << "'\n";
          return true;
        }
    }
  name = fullname;
  norm_mode = NO_NORM;
  // QTextStream( stdout ) << "get_name_norm_mode() dropping out, returning
  // false, name is now: '" << name << "'\n";
  return false;
}

bool
US_Hydrodyn_Mals_Svd::get_plot_files (QStringList &use_list,
                                      ...
                                      norms &ref_norm_mode)
{
  ...
  if (sources.size () > 1)
    {
      // editor_msg( "red", QString( us_tr( "Internal error: get_plot_files()
      // with more than one source (%1)" ) ).arg( sources.size() ) );
      return false;
    }

  QString source = *(sources.begin ());
  QString ref_source;
  (void)get_name_norm_mode (get_related_source_name (source), ref_source,
                            ref_norm_mode);
  // QTextStream( stdout ) << "ref source:" << ref_source << "  ref_norm_mode:"
  // << norm_name_map[ ref_norm_mode ] << "\n";
  ...
  if (ref_source.isEmpty ())
    {
      if (source != us_tr ("Original data"))
        {
          editor_msg ("red",
                      QString (us_tr ("Internal error: get_plot_files() could "
                                      "not find related name for source %1"))
                          .arg (source));
        }
      return false;
    }
  ...
  if (full_list.size () != full_ref_list.size ())
    {
      editor_msg ("red", QString (us_tr ("Internal error: get_plot_files() "
                                         "file list count mismatch %1 %2"))
                             .arg (full_list.size ())
                             .arg (full_ref_list.size ()));
      return false;
    }
...
void
US_Hydrodyn_Mals_Svd::do_plot_errors ()
{
  ...
  for (int i = 0; i < (int)use_list.size (); ++i)
    {
      // QTextStream( stdout )
      //    <<
      //    QString( "use %1 list name '%2' fe %4 qs %5 qss %6 ref list name
      //    '%3' fe %7 qs %8 qss %9" ) .arg( i ) .arg( use_list[ i ] ) .arg(
      //    use_ref_list[ i ] ) .arg( f_errors.count( use_list[ i ] ) ? "yes" :
      //    "NO" ) .arg( f_qs.count( use_list[ i ] ) ? "yes" : "NO" ) .arg(
      //    f_qs_string.count( use_list[ i ] ) ? "yes" : "NO" ) .arg(
      //    f_errors.count( use_ref_list[ i ] ) ? "yes" : "NO" ) .arg(
      //    f_qs.count( use_ref_list[ i ] ) ? "yes" : "NO" ) .arg(
      //    f_qs_string.count( use_ref_list[ i ] ) ? "yes" : "NO" )
      //    <<
      ...
      if (f_qs_string[use_ref_list[i]] != f_qs_string[use_list[i]])
        {
          QTextStream (stdout)
              << QString ("do_plot_errors() i=%1 use_ref_list[i] = %2 "
                          "use_list[i] = %3\n")
                     .arg (i)
                     .arg (use_ref_list[i])
                     .arg (use_list[i])

              << US_Vector::qs_vector2 (
                     "f_qs[use_ref_list[i]] f_qs[ref_list][i]",
                     f_qs[use_ref_list[i]], f_qs[use_list[i]])

              << US_Vector::qs_vector2 (
                     "f_qs_string[use_ref_list[i]] f_qs_string[ref_list][i]",
                     f_qs_string[use_ref_list[i]], f_qs_string[use_list[i]])

              << "\n";

          editor_msg (
              "red",
              QString (us_tr ("Error: plot residuals: curve grid mismatch")));
          return;
        }

      for (int j = 0; j < (int)f_qs[use_list[i]].size (); ++j)
        {
          grid.push_back (f_qs[use_list[i]][j] - f_qs[use_list[i]][0]
                          + grid_ofs);
      ...
      // QTextStream( stdout )
      //    <<
      //    QString( "grid_ofs for i = %1 is %2; f_pos is %3 color pos %4
      //    colors size %5\n" ) .arg( i ) .arg( grid_ofs ) .arg( f_pos[
      //    use_list[ i ] ] ) .arg( f_pos[ use_list[ i ] ] % plot_colors.size()
  ...
  for (int i = 1; i < (int)grid.size (); ++i)
    {
      if (grid[i - 1] > grid[i])
        {
          QTextStream (stdout)
              << QString ("grid error grid[%1]=%2 > grid[%3]=%4\n")
                     .arg (i - 1)
                     .arg (grid[i - 1])
                     .arg (i)
                     .arg (grid[i]);
  ...
  if (cb_plot_errors_pct->isChecked ())
    {
      if (cb_plot_errors_sd->isChecked () && errors.size () == target.size ()
          && US_Saxs_Util::is_nonzero_vector (errors))
        {
              ...
              if (target[i] != 0e0)
                {
                  ...
                  e[use_pos[i]].push_back (100.0 * (target[i] - fit[i])
                                           / target[i]);
                }
            }
        }
  ...
  // QTextStream( stdout ) << US_Vector::qs_vector2( "use_pos, grid", use_pos,
  // grid ) << "\n";
  ...
  for (int j = 0; j < (int)x.size (); ++j)
    {
      // US_Vector::printvector2( "x[j], e[j]", x[j], e[j] );
      {
        ...
        curve->setSamples ((double *)&x[j][0], (double *)&y[j][0],
                           x[j].size ());
...

      {
        ...
        curve->setSamples ((double *)&x[j][0], (double *)&e[j][0],
                           x[j].size ());
      ...
      plot_errors->setAxisTitle (
          ...
          QString (
              us_tr (cb_plot_errors_pct->isChecked ()
                         ? "% difference %1"
                         : (cb_plot_errors_sd->isChecked () ? "delta I%1/sd"
                                                            : "delta I%1")))
              .arg (iq_it_state ? "I(t)" : "I(q)"));

      // plot_errors->setAxisScale( QwtPlot::xBottom, x[ 0 ][ 0 ],
      // x.back().back() );
...
void
US_Hydrodyn_Mals_Svd::plot_errors_jump_markers ()
{
  if (cb_plot_errors_group->isChecked () || plot_errors_jumps.size () > 25)
    {
      return;
    }
...
void
US_Hydrodyn_Mals_Svd::do_plot_errors_group ()
{
  ...
  for (int j = 0; j < (int)use_list.size (); ++j)
    {
                  ...
                  if ((*ref_f_Iuse)[this_ref_file][i] != 0e0)
                    {
                      ...
                      y.push_back (0e0);
                      e.push_back (100.0
                                   * ((*ref_f_Iuse)[this_ref_file][i]
                                      - (*org_f_Iuse)[this_file][i])
                                   / (*ref_f_Iuse)[this_ref_file][i]);
                    }
                }
            }
              ...
              for (int i = 0; i < (int)f_qs[this_file].size (); ++i)
                {
                  x.push_back (f_qs[this_file][i]);
                  y.push_back (0e0);
                  e.push_back (((*ref_f_Iuse)[this_ref_file][i]
                                - (*org_f_Iuse)[this_file][i])
                               / f_errors[this_ref_errors][i]);
                }
            }
          else
            {
...

      {
        ...
        curve->setStyle (QwtPlotCurve::Lines);
        curve->setPen (
            QPen (plot_colors[f_pos[this_color_file] % plot_colors.size ()],
                  use_line_width, Qt::SolidLine));
        curve->setSamples ((double *)&x[0], (double *)&e[0], x.size ());
        curve->setStyle (QwtPlotCurve::Sticks);
        curve->attach (plot_errors);
      }
...
bool
US_Hydrodyn_Mals_Svd::setup_save (QString tag, QString &fname)
{
  ...
  fname = QFileDialog::getSaveFileName (
      this, us_tr ("Select a file name to ") + tag, use_dir,
      "Text files (*.txt *.TXT);;"
      "CSV files (*.csv *.CSV);;"
      "All Files (*)");
...
void
US_Hydrodyn_Mals_Svd::rmsd_save ()
{
  QString fname;
  ...
  if (!f.open (QIODevice::WriteOnly))
    {
      editor_msg (
          "red",
          QString (us_tr ("Error: can not open %1 for writing")).arg (fname));
      return;
    }
...
US_Hydrodyn_Mals_Svd::svd_save ()
{
  ...
  QString out = QString (us_tr ("\"SVD of %1\",\"Singular value\""))
                    .arg (last_svd_name)
                + "\n";
  for (int i = 0; i < (int)lb_ev->count (); ++i)
    {
...
void
US_Hydrodyn_Mals_Svd::usp_config_plot_data (const QPoint &)
{
...
void
US_Hydrodyn_Mals_Svd::usp_config_plot_svd (const QPoint &)
{
...
void
US_Hydrodyn_Mals_Svd::usp_config_plot_errors (const QPoint &)
{
...
void
US_Hydrodyn_Mals_Svd::usp_config_plot_lefa (const QPoint &)
{
...
void
US_Hydrodyn_Mals_Svd::usp_config_plot_refa (const QPoint &)
{
...
void
US_Hydrodyn_Mals_Svd::usp_config_plot_ac (const QPoint &)
{
...
void
US_Hydrodyn_Mals_Svd::usp_config_plot_efa_decomp (const QPoint &)
{
...
void
US_Hydrodyn_Mals_Svd::set_norm_pw ()
{
  if (cb_norm_avg->isChecked ())
    {
      cb_norm_avg->setChecked (false);
...
void
US_Hydrodyn_Mals_Svd::set_norm_avg ()
{
  if (cb_norm_pw->isChecked ())
    {
      cb_norm_pw->setChecked (false);
...
void
US_Hydrodyn_Mals_Svd::clear_svd ()
{
  plot_svd->detachItems (QwtPlotItem::Rtti_PlotCurve);
...
void
US_Hydrodyn_Mals_Svd::clear_efa ()
{
...
void
US_Hydrodyn_Mals_Svd::clear_efa_decomp ()
{
...
US_Hydrodyn_Mals_Svd::efa ()
{
      ...
      if ((int)A[i].size () != m)
        {
          editor_msg (
              "red",
              QString (us_tr ("Inconsistent data length (e.g. 1st file has %1 "
                              "data points but file %2 has %3 data points")
                           .arg (m)
                           .arg (i)
                           .arg ((int)A[i].size ())));
          update_enables ();
  ...
  if (efa_lsv.size () != efa_rsv.size ())
    {
      editor_msg (
          "red",
          us_tr (
              "Unexpected error: Forward EFA and Backward EFA size mismatch"));
    }
  ...
  int max_range
      = (int)(efa_rsv.size () > UHSHS_MAX_SV_FOR_EFA ? UHSHS_MAX_SV_FOR_EFA
                                                     : efa_rsv.size ());
...
void
US_Hydrodyn_Mals_Svd::make_plotted_efas ()
{
...
void
US_Hydrodyn_Mals_Svd::update_efas (double val)
{
  // qDebug() << "US_Hydrodyn_Mals_Svd::update_efas( " << val << " )";
  ...
  if (n != (int)efa_rsv.size ())
    {
      editor_msg ("red", us_tr ("Unexpected error: Forward EFA and Backward "
                                "EFA size mismatch in plot"));
      return;
    }
  ...
  if (ival > n)
    {
      efa_plot_count = n;
      editor_msg ("red", us_tr ("Unexpected error: requested greater than "
                                "available SVs for EFA plot"));
      return;
    }
...
US_Hydrodyn_Mals_Svd::efa_plot ()
{
  ...
  for (int j = 0; j < efa_plot_count; ++j)
    {
      US_Vector::printvector (QString ("lefa %1").arg (j + 1),
                              plotted_efa_lsv[j]);
      US_Vector::printvector (QString ("refa %1").arg (j + 1),
                              plotted_efa_rsv[j]);
    }
#endif
  ...
  for (int i = 0; i < efa_plot_count; ++i)
    {
      {
        QwtPlotCurve *curve
            = new QwtPlotCurve (QString ("Forward EFA:%1").arg (i + 1));
        curve->setStyle (QwtPlotCurve::Lines);

        curve->setSamples ((double *)&(plotted_efa_x[0]),
                           (double *)&(plotted_efa_lsv[i][0]), n);

        curve->setPen (QPen (plot_colors[i % plot_colors.size ()],
                             use_line_width, Qt::SolidLine));
        curve->attach (plot_lefa);
      }
      {
        QwtPlotCurve *curve
            = new QwtPlotCurve (QString ("Backward EFA:%1").arg (i + 1));
        curve->setStyle (QwtPlotCurve::Lines);

        curve->setSamples ((double *)&(plotted_efa_x[0]),
                           (double *)&(plotted_efa_rsv[i][0]), n);
...

      {
        if ((int)efa_range_start.size () <= i)
          {
            editor_msg ("red",
                        us_tr ("Internal error: start range point value not "
                               "found. please inform the developers"));
          }
        else
          {
            ...
            if ((int)plotted_efa_lsv[i].size () <= pos)
              {
                editor_msg (
                    "red",
                    us_tr ("Internal error: start range point value exceeds "
                           "known values. please inform the developers"));
              }
            else
              {
                ...
                m->setSymbol (new QwtSymbol (
                    ...
                    QPen (plot_colors[i % plot_colors.size ()],
                          use_line_width + 3),
                    QSize (4 * (use_line_width + 2),
                           4 * (use_line_width + 2))));

                m->setValue (
                    QPointF (plotted_efa_x[pos], plotted_efa_lsv[i][pos]));
                m->attach (plot_lefa);
      ...
      }
      {
        if ((int)efa_range_end.size () <= i)
          {
            editor_msg ("red",
                        us_tr ("Internal error: end range point value not "
                               "found. please inform the developers"));
          }
        else
          {
            ...
            if ((int)plotted_efa_rsv[i].size () <= pos)
              {
                editor_msg (
                    "red",
                    us_tr ("Internal error: end range point value exceeds "
                           "known values. please inform the developers"));
              }
            else
              {
                ...
                m->setSymbol (new QwtSymbol (
                    ...
                    QSize (4 * (use_line_width + 2),
                           4 * (use_line_width + 2))));
                m->setValue (
                    QPointF (plotted_efa_x[pos], plotted_efa_rsv[i][pos]));
                m->attach (plot_refa);
...
void
US_Hydrodyn_Mals_Svd::setup_norm ()
{
  unsigned int q_points = (unsigned int)f_qs[mals_selected_files[0]].size ();

  vector<double> avg_errors (q_points);

  for (int i = 0; i < (int)mals_selected_files.size (); ++i)
    {
      QString this_name = mals_selected_files[i];
      ...
      if (this_q_points != q_points)
        {
          QMessageBox::warning (this, windowTitle (),
                                us_tr ("Internal error: HPLC SVD called but "
                                       "curves have varying lengths"));
          close ();
          return;
        }
  ...
  double recip_fc = 1e0 / (double)mals_selected_files.size ();
  ...
  for (int i = 0; i < (int)mals_selected_files.size (); ++i)
    {
      ...
      for (unsigned j = 0; j < q_points; ++j)
        {
          f_Is_norm_avg[this_name].push_back (f_Is[this_name][j]
                                              * avg_errors[j]);
        }
    }
  ...
  US_Vector::printvector3 (
      "file 0 I, errors, I_norm_pw", f_Is[mals_selected_files[0]],
      f_errors[mals_selected_files[0]], f_Is_norm_pw[mals_selected_files[0]]);

  US_Vector::printvector3 ("file 0 I, avg_errors, I_norm_avg",
                           f_Is[mals_selected_files[0]], avg_errors,
                           f_Is_norm_avg[mals_selected_files[0]]);
...
void
US_Hydrodyn_Mals_Svd::check_norm (QStringList &files)
{
  ...
  if (f_Is_norm_pw.count (files[0]) && f_Is_norm_avg.count (files[0]))
    {
      return;
    }
  ...
  for (int i = 0; i < (int)files.size (); ++i)
    {
      ...
      if (f_Is_norm_pw.count (this_name) || f_Is_norm_avg.count (this_name))
        {
          editor_msg (
              "red",
              us_tr (
                  "Internal error: unexpected condition 1 - inconsistency, "
                  "turning off normalization, please inform the developers"));
          norm_ok = false;
          return;
        }
      ...
      if (this_q_points != q_points)
        {
          editor_msg (
              "red",
              us_tr (
                  "Internal error: unexpected condition 2 - length mismatch, "
                  "turning off normalization, please inform the developers"));
          norm_ok = false;
          return;
        }
      ...
      for (unsigned j = 0; j < q_points; ++j)
        {
          if (!f_errors[this_name][j])
            {
              editor_msg ("red",
                          us_tr ("Internal error: unexpected condition 3 - "
                                 "zero error, turning off normalization, "
                                 "please inform the developers"));
          ...
          f_Is_norm_pw[this_name].push_back (f_Is[this_name][j]
                                             * org_avg_errors[j]);
          f_Is_norm_avg[this_name].push_back (f_Is[this_name][j]
                                              / f_errors[this_name][j]);
        }
    }
}

double
US_Hydrodyn_Mals_Svd::autocor1 (vector<double> &x)
{
  int len = (int)x.size ();

#if defined(DEBUG_AUTOCOR_DETAIL)
  US_Vector::printvector ("US_Hydrodyn_Mals_Svd::autocor1 x", x);
#endif
...
vector<double>
US_Hydrodyn_Mals_Svd::autocor (vector<vector<double> > &A)
{
  ...
  {
    QTextStream ts (stdout);
    ts << "US_Hydrodyn_Mals_Svd::autocor A" << Qt::endl;
...
vector<vector<int> >
US_Hydrodyn_Mals_Svd::transpose (vector<vector<int> > &A)
{

  //   qDebug() << "US_Hydrodyn_Mals_Svd::transpose( vvi ) start";
  ...
  //   qDebug() << "US_Hydrodyn_Mals_Svd::transpose( vvi ) rows " << rows << "
  //   cols " << cols;
  ...
  //   qDebug() << "US_Hydrodyn_Mals_Svd::transpose( vvi ) return";
  return result;
}

vector<vector<double> >
US_Hydrodyn_Mals_Svd::transpose (vector<vector<double> > &A)
{

  //   qDebug() << "US_Hydrodyn_Mals_Svd::transpose( vvd ) start";
  ...
  //   qDebug() << "US_Hydrodyn_Mals_Svd::transpose( vvd ) rows " << rows << "
  //   cols " << cols;
  ...
  //   qDebug() << "US_Hydrodyn_Mals_Svd::transpose( vvd ) return";

  return result;
}

double
US_Hydrodyn_Mals_Svd::vvd_min (vector<vector<double> > &A)
{
  double result = -1e99;
  int rows = (int)A.size ();
  if (!rows)
    {
      editor_msg ("red", us_tr ("Internal error: vvd_min called with an empty "
                                "vvd, please inform the developers"));
      return result;
    }
  int cols = (int)A[0].size ();
  if (!cols)
    {
      editor_msg ("red",
                  us_tr ("Internal error: vvd_min called with a columnless "
                         "vvd, please inform the developers"));
      return result;
    }
...
double
US_Hydrodyn_Mals_Svd::vvd_max (vector<vector<double> > &A)
{
  double result = 1e99;
  int rows = (int)A.size ();
  if (!rows)
    {
      editor_msg ("red", us_tr ("Internal error: vvd_max called with an empty "
                                "vvd, please inform the developers"));
      return result;
    }
  int cols = (int)A[0].size ();
  if (!cols)
    {
      editor_msg ("red",
                  us_tr ("Internal error: vvd_max called with a columnless "
                         "vvd, please inform the developers"));
      return result;
    }
...
void
US_Hydrodyn_Mals_Svd::vvd_smult (vector<vector<double> > &A, double x)
{
  int rows = (int)A.size ();
  if (!rows)
    {
      editor_msg ("red", us_tr ("Internal error: vdd_smult called with an "
                                "empty vvd, please inform the developers"));
      return;
    }
  int cols = (int)A[0].size ();
  if (!cols)
    {
      editor_msg ("red",
                  us_tr ("Internal error: vdd_smult called with a columnless "
                         "vvd, please inform the developers"));
      return;
    }
...
vector<vector<double> >
US_Hydrodyn_Mals_Svd::vivd_pwmult (vector<vector<int> > &A,
                                   vector<vector<double> > &B)
{
  ...
  if (!arows || !brows)
    {
      editor_msg ("red", us_tr ("Internal error: vivd_pwmult called with an "
                                "empty vvd, please inform the developers"));
      return result;
    }
  if (arows != brows)
    {
      ...
      editor_msg (
          "red", us_tr ("Internal error: vivd_pwmult called with incompatible "
                        "vi & vd, please inform the developers"));
      return result;
    }
  ...
  if (acols != bcols)
    {
      editor_msg (
          "red", us_tr ("Internal error: vivd_pwmult called with incompatible "
                        "vi & vd colwise, please inform the developers"));
      return result;
    }
...
void
US_Hydrodyn_Mals_Svd::vvd_cnorm (vector<vector<double> > &A)
{
  int rows = (int)A.size ();
  if (!rows)
    {
      editor_msg ("red", us_tr ("Internal error: vvd_cnorm called with an "
                                "empty vvd, please inform the developers"));
      return;
    }
  int cols = (int)A[0].size ();
  if (!cols)
    {
      editor_msg ("red",
                  us_tr ("Internal error: vvd_cnorm called with a columnless "
                         "vvd, please inform the developers"));
      return;
    }
  ...
  for (int i = 0; i < rows; ++i)
    {
      ...
      if (csum[i])
        ...
        {
          editor_msg ("red",
                      us_tr ("Internal error: vvd_cnorm called with a zero "
                             "sum column, please inform the developers"));
          return;
        }
    }
  for (int i = 0; i < rows; ++i)
                                 ...
                                 vector<vector<double> > &B)
{
  ...
  if (arows != brows)
    {
      editor_msg ("red",
                  us_tr ("Internal error: vvd_cnorm called with incompatible "
                         "vvds, please inform the developers"));
      return;
    }
...
vector<vector<double> >
US_Hydrodyn_Mals_Svd::dot (vector<vector<double> > &A,
                           vector<vector<double> > &B)
{
  vector<vector<double> > result;
  int arows = (int)A.size ();
  ...
  if (!arows || !brows)
    {
      editor_msg ("red", us_tr ("Internal error: dot called with an empty "
                                "vvd's, please inform the developers"));
      return result;
    }
  ...
  if (acols != brows)
    {
      editor_msg ("red", us_tr ("Internal error: dot called with incompatible "
                                "vvd's, please inform the developers"));
      return result;
    }
...
void
US_Hydrodyn_Mals_Svd::matrix_info (QString qs, vector<vector<double> > &A)
{
  if (!A.size ())
    {
      qDebug () << "US_Hydrodyn_Mals_Svd::matrix_info() " << qs << " empty!";
      return;
    }
  qDebug () << "US_Hydrodyn_Mals_Svd::matrix_info() " << qs << " rows "
            << A.size () << " cols " << A[0].size ();
}

void
US_Hydrodyn_Mals_Svd::set_title (QwtPlot *plot, QString title)
{
  QwtText qwtt_title (title);
  qwtt_title.setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize));
  plot->setTitle (qwtt_title);
}

void
US_Hydrodyn_Mals_Svd::save_plots ()
{
  QString use_dir = QDir::currentPath ();
  ush_win->select_from_directory_history (use_dir, this);

  QString fn = QFileDialog::getSaveFileName (
      this, us_tr ("Select a prefix name to save the plot data"), use_dir,
      "*.csv");
  ...
  fn = QFileInfo (fn).path () + QDir::separator ()
...
void
US_Hydrodyn_Mals_Svd::mode_select ()
{
  // first hide all
...
void
US_Hydrodyn_Mals_Svd::mode_select (modes mode)
{
  current_mode = mode;
  mode_select ();
  update_enables ();
...
void
US_Hydrodyn_Mals_Svd::set_mode_iqit ()
{
  mode_select (MODE_IQIT);
}

void
US_Hydrodyn_Mals_Svd::set_mode_svd ()
{
  mode_select (MODE_SVD);
}

void
US_Hydrodyn_Mals_Svd::set_mode_efa ()
{
  mode_select (MODE_EFA);
}

void
US_Hydrodyn_Mals_Svd::set_mode_efa_decomp ()
{
  mode_select (MODE_EFA_DECOMP);
}

void
US_Hydrodyn_Mals_Svd::update_efa_range_start (double)
{
  // qDebug() << "US_Hydrodyn_Mals_Svd::update_efa_range_start";
  // {
  //    QTextStream ts( stdout );
  //    for ( int i = 0; i < UHSHS_MAX_SV_FOR_EFA; ++i ) {
  //       ts << "US_Hydrodyn_Mals_Svd::update_efa_range_start SV " << i << "
  //       val " << efa_range_start[ i ]->value() << Qt::endl;
  //    }
  ...
  if ((int)qwtc_efas->value () != (int)qwtpm_lsv_range_marker.size ())
    {
      editor_msg ("red",
                  us_tr ("Internal error: inconsistency in start range marker "
                         "size & efas values. please inform the developers"));
      return;
    }
      ...
      if ((int)efa_range_start.size () <= i)
      ...
      else
        {
          ...
          if ((int)plotted_efa_lsv[i].size () <= pos)
            ...
            {
              qwtpm_lsv_range_marker[i]->setValue (
                  QPointF (plotted_efa_x[pos], plotted_efa_lsv[i][pos]));
            }
        }
    }
...
void
US_Hydrodyn_Mals_Svd::update_efa_range_end (double)
{
  // qDebug() << "US_Hydrodyn_Mals_Svd::update_efa_range_end()";
  if (efa_range_processing)
    {
      return;
    }
  ...
  if ((int)qwtc_efas->value () != (int)qwtpm_rsv_range_marker.size ())
    {
      editor_msg ("red",
                  us_tr ("Internal error: inconsistency in end range marker "
                         "size & efas values. please inform the developers"));
      return;
    }
          ...
          if ((int)plotted_efa_rsv[i].size () <= pos)
            {
              editor_msg (
                  "red",
                  us_tr ("Internal error: end range point value exceeds known "
                         "values. please inform the developers"));
              return;
            }
          else
            {
              qwtpm_rsv_range_marker[i]->setValue (
                  QPointF (plotted_efa_x[pos], plotted_efa_rsv[i][pos]));
            }
        }
    }
...
void
US_Hydrodyn_Mals_Svd::update_efas_ranges ()
{
  // qDebug() << "US_Hydrodyn_Mals_Svd::update_efas_ranges()";
...
void
US_Hydrodyn_Mals_Svd::efa_info (QString tag)
{
  QTextStream ts (stdout);

  ts << " efa_info: " << tag << " : efa_lsv.size() : " << efa_lsv.size ()
     << Qt::endl;

  for (int i = 0; i < (int)efa_lsv.size (); ++i)
    {
      ts << " efa_info: " << tag << ": efa_lsv[ " << i
         << " ].size() : " << efa_lsv[i].size () << Qt::endl;
    }

  ts << " efa_info: " << tag << " : efa_rsv.size() : " << efa_rsv.size ()
     << Qt::endl;

  for (int i = 0; i < (int)efa_rsv.size (); ++i)
    {
      ts << " efa_info: " << tag << ": efa_rsv[ " << i
         << " ].size() : " << efa_rsv[i].size () << Qt::endl;
    }
}

vector<double>
US_Hydrodyn_Mals_Svd::gradient (vector<double> &y)
{
  int len = (int)y.size ();
  int lenm1 = len - 1;
  ...
  if (len < 2)
    {
      editor_msg ("red",
                  us_tr ("Internal error: gradient called with emptyish "
                         "vector. Please inform the developers."));
      return g;
    }
  g.resize (len);
...
void
US_Hydrodyn_Mals_Svd::init_efa_values ()
{
  if (plotted_efa_lsv.size () != plotted_efa_rsv.size ())
    {
      editor_msg ("red", us_tr ("Internal error: init efa values size "
                                "mismatch. Please inform the developers."));
      return;
    }
  if (!plotted_efa_lsv.size ())
    {
      editor_msg ("red", us_tr ("Internal error: init efa values efa empty. "
                                "Please inform the developers."));
      return;
    }
    ...
    {
      {
        ...
        {
          QTextStream ts (stdout);
          US_Vector::printvector (QString ("plotted_efa_lsv[%1]").arg (i),
                                  use_v);
          US_Vector::printvector (
              QString ("gradient of plotted_efa_lsv[%1]").arg (i), g);
          ts << "gmax : " << gmax;
          ts << "gmaxrecip : " << gmaxrecip;
          US_Vector::printvector (
              QString ("init efa forward sv %1").arg (i + 1), results);
        }
#endif
...
#if defined(DEBUG_INIT_EFA_RANGES)
        QTextStream ts (stdout);
        ts << QString ("init_efa forward sv %1 use pos %2")
                  .arg (i)
                  .arg (use_value);
#endif
      }
    ...
    {
      {
        ...
        {
          QTextStream ts (stdout);
          US_Vector::printvector (QString ("plotted_efa_rsv[%1]").arg (i),
                                  use_v);
          US_Vector::printvector (
              QString ("gradient of plotted_efa_rsv[%1]").arg (i), g);
          ts << "gmin : " << gmin;
...
void
US_Hydrodyn_Mals_Svd::set_number_of_svs_for_efa ()
{
  int max_range
      = (int)(svd_D.size () > UHSHS_MAX_SV_FOR_EFA ? UHSHS_MAX_SV_FOR_EFA
                                                   : svd_D.size ());
  if (max_range < 1)
    {
      max_range = 1;
  ...
  use_sv_count
      = use_sv_U_count > use_sv_V_count ? use_sv_U_count : use_sv_V_count;
  if (use_sv_count < 1)
    {
      use_sv_count = 1;
    }

  // US_Vector::printvector2( "US_Hydrodyn_Mals_Svd::set_number_of_svs_for_efa
  // svd_autocor_U, V", svd_autocor_U, svd_autocor_V );
...
bool
US_Hydrodyn_Mals_Svd::convert_it_to_iq (QStringList files,
                                        ...
                                        QString &error_msg)
{
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
    {
      if (rx_q.indexIn (files[i]) == -1)
        {
          error_msg
              = QString (
                    us_tr ("Error: Can not find q value in file name for %1"))
                    .arg (files[i]);
          return false;
        }
      ...
      if (used_q.count (ql.back ()))
        {
          error_msg
              = QString (
                    us_tr ("Error: Duplicate q value in file name for %1"))
                    .arg (files[i]);
          return false;
        }
      used_q[ql.back ()] = true;
      ...
      if (!mals_win->f_qs.count (files[i]))
        {
          // error_msg = QString( us_tr( "Internal error: request to use %1,
          // but not found in data" ) ).arg( files[ i ] );
        }
      else
        {
          for (unsigned int j = 0;
               j < (unsigned int)mals_win->f_qs[files[i]].size (); j++)
            {
              if (!mode_testiq
                  || (mals_win->f_qs[files[i]][j] >= t_min
                      && mals_win->f_qs[files[i]][j] <= t_max))
                {
                  I_values[mals_win->f_qs[files[i]][j]][ql.back ()]
                      = mals_win->f_Is[files[i]][j];
                  if (use_errors
                      && mals_win->f_errors[files[i]].size ()
                             == mals_win->f_qs[files[i]].size ())
                    {
                      e_values[mals_win->f_qs[files[i]][j]][ql.back ()]
                          = mals_win->f_errors[files[i]][j];
                    }
                  else
                    {
                      if (use_errors)
                        {
                          ...
                          // missing errors, first noticed in %1, so no errors
                          // at all" ) )
                          //             .arg( files[ i ] ) );
                        }
                    }
                  if (!used_t.count (mals_win->f_qs[files[i]][j]))
                    {
                      tl.push_back (mals_win->f_qs[files[i]][j]);
                      used_t[mals_win->f_qs[files[i]][j]] = true;
                    }
                }
            }

          if (!mals_win->f_errors.count (files[i])
              || mals_win->f_errors[files[i]].size ()
                     != mals_win->f_Is[files[i]].size ())
            {
            ...
            {
              if (!mals_win->is_nonzero_vector (mals_win->f_errors[files[i]]))
                {
                  unsigned int zero_pts = 0;
                  for (unsigned int j = 0;
                       j < (unsigned int)mals_win->f_errors[files[i]].size ();
                       j++)
                    {
                      if (!mode_testiq
                          || (mals_win->f_qs[files[i]][j] >= t_min
                              && mals_win->f_qs[files[i]][j] <= t_max))
                        {
                          if (us_isnan (mals_win->f_errors[files[i]][j])
                              || mals_win->f_errors[files[i]][j] == 0e0)
                            {
                              zero_pts++;
                            }
                        }
                  ...
                  zero_points[files[i]]
                      = QString ("%1: %2 of %3 points")
                            .arg (files[i])
                            .arg (zero_pts)
                            .arg (mals_win->f_errors[files[i]].size ());
  ...
  vector<double> tv;
  for (list<double>::iterator it = tl.begin (); it != tl.end (); it++)
    {
      tv.push_back (*it);
    }
  ...
  if (zero_points.size () || no_errors.size ())
    {
      ...
      for (unsigned int i = 0;
           i < (unsigned int)qsl_no_errors.size () && i < 12; i++)
        {
      ...
      if (qsl_list_no_errors.size () < qsl_no_errors.size ())
        {
          qsl_list_no_errors
              << QString (us_tr ("... and %1 more not listed"))
                     .arg (qsl_no_errors.size () - qsl_list_no_errors.size ());
        }
      qs_no_errors = qsl_list_no_errors.join ("\n");

      QStringList qsl_list_zero_points;
      for (unsigned int i = 0;
           i < (unsigned int)qsl_zero_points.size () && i < 24 - used; i++)
        {
          qsl_list_zero_points << qsl_zero_points[i];
        }
      if (qsl_list_zero_points.size () < qsl_zero_points.size ())
        {
          qsl_list_zero_points
              << QString (us_tr ("... and %1 more not listed"))
                     .arg (qsl_zero_points.size ()
                           - qsl_list_zero_points.size ());
        }
  ...
  for (unsigned int t = 0; t < tv.size (); t++)
    {
      // progress->setValue( files.size() + t ); progress->setMaximum(
      // files.size() + tv.size() );
      ...
      QString name = head
                     + QString ("%1%2")
                           ...
                           .arg (mals_win->pad_zeros (tv[t], (int)tv.size ()))
                           .replace (".", "_");
      ...
      // now go through all the files to pick out the I values and errors and
      // distribute amoungst the various gaussian peaks we could also
      // reassemble the original sum of gaussians curves as a comparative
      ...
      // if ( conc_ok ) {
      //    if ( !usu->apply_natural_spline( conc_spline_x, conc_spline_y,
      //    conc_spline_y2, tv[ t ], conc_factor ) ) {
      //       // editor_msg( "red", QString( us_tr( "Error getting
      //       concentration from spline for frame %1, concentration set to
      //       zero." ) ).arg( tv[ t ] ) ); conc_factor = 0e0;
      //    }
      // }
      ...
      for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
        {
          if (!I_values.count (tv[t]))
            {
              // editor_msg( "dark red", QString( us_tr( "Notice: I values
              // missing frame/time = %1" ) ).arg( tv[ t ] ) );
            }

          if (!I_values[tv[t]].count (qv[i]))
            {
              // editor_msg( "red", QString( us_tr( "Notice: I values missing q
              // = %1" ) ).arg( qv[ i ] ) );
              continue;
            }
          ...
          if (use_errors)
            {
              if (!e_values.count (tv[t]))
                {
                  // editor_msg( "red", QString( us_tr( "Internal error: error
                  // values missing t %1" ) ).arg( tv[ t ] ) ); running =
                  // false; update_enables(); progress->reset();
                  error_msg
                      = QString (
                            us_tr (
                                "Internal error: error values missing t %1"))
                            .arg (tv[t]);
                  return false;
                }
              ...
              if (!e_values[tv[t]].count (qv[i]))
                {
                  // editor_msg( "red", QString( us_tr( "Internal error: error
                  // values missing q %1" ) ).arg( qv[ i ] ) ); running =
                  // false; update_enables(); progress->reset();
                  error_msg
                      = QString (
                            us_tr (
                                "Internal error: error values missing q %1"))
                            .arg (qv[i]);
                  return false;
                }
      ...
      /* from original
         ...
         f_Is       [ this_name ] = mals_win->f_Is       [ this_name ];
         f_errors   [ this_name ] = mals_win->f_errors   [ this_name ];
         f_is_time  [ this_name ] = mode_i_of_t;  // false;  // must all be
         I(q)
      */
...
void
US_Hydrodyn_Mals_Svd::runExplicitEFARotation (
    vector<vector<int> > &M, vector<vector<double> > & /* D */,
    bool &failed, // also a return
    vector<vector<double> > &C, vector<vector<double> > &V_bar,
    vector<vector<double> > &T, int /* niter */, double /* tol */,
    vector<int> & /* force_pos */,
    // returns
    vector<vector<double> > & /* C_ret */, bool &converged)
{
  /*
def runExplicitEFARotation(M, D, failed, C, V_bar, T, niter, tol, force_pos):
   ...
   for i in range(num_sv):
       ...
       T[i,1:num_sv] = -np.dot(V_i_0[:,0].T,
np.linalg.pinv(V_i_0[:,1:num_sv].T))
   ...
   if int(np.__version__.split('.')[0]) >= 1 and
int(np.__version__.split('.')[1])>=10: C = C/np.broadcast_to(csum, C.shape)
#normalizes by the sum of each column else: norm = np.array([csum for i in
range(C.shape[0])])
  ...
  */
  ...
  // qDebug() << "US_Hydrodyn_Mals_Svd::runExplicitEFARotation\n";
...
void
US_Hydrodyn_Mals_Svd::efa_decomp ()
{
  ...
  // qDebug() << "efa_decomp 1 efa_range_start.size() " <<
  // efa_range_start.size(); qDebug() << "efa_decomp 1 efa_range_end.size() "
  // << efa_range_end.size();
      ...
      if (start_vals[i] >= end_vals[i])
        {
          editor_msg (
              "red",
              QString (
                  us_tr ("Ranges for component %1 is empty or negative.  "
                         "Please correct.  Note that the component ranges are "
                         "in reverse order to the S.V's ranges (i.e. the "
                         "start frame for this component is S.V. %2 and the "
                         "end frame is S.V. %3.)"))
                  .arg (i + 1)
                  .arg (i + 1)
                  .arg (num_sv - i));
  ...
  */
  ...
  switch (method)
    ...
    case METHOD_HYBRID:
      {
        initHybridEFA (M, num_sv, D, C, converged, V_bar, niter, tol,
                       force_pos, failed, C_ret, T);
        if (!failed)
          {
            runIterativeEFARotation (M, D, failed, C, V_bar, T, niter, tol,
                                     force_pos, C_ret, converged, dc, k);
          }
      }
      break;
    ...
    case METHOD_EXPLICIT:
      {
        ...
        if (!failed)
          {
            runExplicitEFARotation (M, D, failed, C, V_bar, T, niter, tol,
                                    force_pos, C_ret, converged);
          }
      }
      break;
    }
  ...
  editor_msg ("blue",
              us_tr ("EFA Components: Done computing on ") + last_svd_name);

  update_enables ();
}

void
US_Hydrodyn_Mals_Svd::efa_decomp_plot ()
{
  ...
  if (plot_efa_decomp_zoomer)
    {
      // cout << QString( "plot zoomer stack size %1\n" ).arg(
      // plot_efa_decomp_zoomer->zoomRectIndex() );
  ...
  for (int i = 0; i < (int)efa_decomp_Ct.size (); ++i)
    {
      QwtPlotCurve *curve
          = new QwtPlotCurve (QString ("EFA Deconvolution SV %1").arg (i + 1));
      curve->setStyle (QwtPlotCurve::Lines);

      curve->setSamples ((double *)&(efa_decomp_x[0]),
                         (double *)&(efa_decomp_Ct[i][0]), svd_x.size ());
  ...
  //   plot_data->setAxisScale( QwtPlot::xBottom, 1, svd_x.size() );
  plot_efa_decomp->setAxisScale (QwtPlot::xBottom, efa_decomp_x[0] - 0.5,
                                 efa_decomp_x.back () + 0.5);
  plot_efa_decomp->setAxisScale (QwtPlot::yLeft,
                                 vvd_min (efa_decomp_Ct) - 0.05,
                                 vvd_max (efa_decomp_Ct) + 0.05);

  if (!plot_efa_decomp_zoomer)
    {
      plot_efa_decomp_zoomer = new ScrollZoomer (plot_efa_decomp->canvas ());
      plot_efa_decomp_zoomer->setRubberBandPen (
          QPen (Qt::yellow, 0, Qt::DotLine));
      // connect( plot_efa_decomp_zoomer, SIGNAL( zoomed( const QRectF & ) ),
      // SLOT( plot_efa_decomp_zoomed( const QRectF & ) ) );
    }

  set_title (plot_efa_decomp,
             QString ("EFA Deconvolution of %1").arg (last_svd_name));
  plot_efa_decomp->replot ();
}

void
US_Hydrodyn_Mals_Svd::initExplicitEFA (
    vector<vector<int> > & /* M */, int num_sv,
    vector<vector<double> > & /* D */, vector<vector<double> > & /* C */,
    bool & /* converged */, vector<vector<double> > & /* V_bar */,
    // returns
    bool & /* failed */, vector<vector<double> > &T_ret)
{
  /*
def initExplicitEFA(M, num_sv, D, C, converged, V_bar):
  ...
  */
  // qDebug() << "US_Hydrodyn_Mals_Svd::initExplicitEFA\n";
  T_ret.resize (num_sv);
  for (int i = 0; i < num_sv; ++i)
    {
...
void
US_Hydrodyn_Mals_Svd::initIterativeEFA (
    vector<vector<int> > &M, int /* num_sv */, vector<vector<double> > &D,
    vector<vector<double> > &C, bool & /* converged */,
    vector<vector<double> > & /* V_bar */,
    // returns
    bool & /* failed */, vector<vector<double> > &C_ret)
{
  /*
def initIterativeEFA(M, num_sv, D, C, converged, V_bar):
  ...
  */
  // qDebug() << "US_Hydrodyn_Mals_Svd::initIterativeEFA\n";
  EFAFirstRotation (M, C, D, C_ret);
}

void
US_Hydrodyn_Mals_Svd::EFAFirstRotation (vector<vector<int> > &M,
...
def EFAFirstRotation(M,C,D):
   ...
   #Have to run an initial rotation without forcing C>=0 or things typically
fail to converge (usually the SVD fails) S = np.dot(D,
np.linalg.pinv(np.transpose(M*C)))

   Cnew = np.transpose(np.dot(np.linalg.pinv(S), D))

   csum = np.sum(M*Cnew, axis = 0)
   if int(np.__version__.split('.')[0]) >= 1 and
int(np.__version__.split('.')[1])>=10: Cnew = Cnew/np.broadcast_to(csum,
Cnew.shape) #normalizes by the sum of each column else: norm = np.array([csum
for i in range(Cnew.shape[0])])
   ...
   return Cnew
  */
  // qDebug() << "US_Hydrodyn_Mals_Svd::EFAFirstRotation\n";
...
void
US_Hydrodyn_Mals_Svd::initHybridEFA (vector<vector<int> > &M, int num_sv,
                                     vector<vector<double> > &D,
                                     vector<vector<double> > &C,
                                     bool &converged,
                                     vector<vector<double> > &V_bar, int niter,
                                     double tol, vector<int> &force_pos,
                                     // returns
                                     bool &failed,
                                     vector<vector<double> > &C_ret,
...
def initHybridEFA(M, num_sv, D, C, converged, V_bar):
   ...
   if not converged:
       ...
       C, failed, temp1, temp2, temp3 = runExplicitEFARotation(M, None, None,
None, V_bar, T, None, None, None)

   return failed, C, None
  */
  // qDebug() << "US_Hydrodyn_Mals_Svd::initHybridEFA\n";

  vector<vector<double> > T;

  initExplicitEFA (M, num_sv, D, C, converged, V_bar, failed, T);
  runExplicitEFARotation (M, D, failed, C, V_bar, T, niter, tol, force_pos,
                          C_ret, converged);
}

void
US_Hydrodyn_Mals_Svd::runIterativeEFARotation (
    vector<vector<int> > &M, vector<vector<double> > &D,
    bool &failed, // also a return
    vector<vector<double> > &C, vector<vector<double> > & /* V_bar */,
    vector<vector<double> > & /* T */, int niter, double tol,
    vector<int> &force_pos,
    // returns
    vector<vector<double> > & /* C_ret */, bool &converged, vector<double> &dc,
    int &k)
{
  /*
def runIterativeEFARotation(M, D, failed, C, V_bar, T, niter, tol, force_pos):
  ...
  */
  // qDebug() << "US_Hydrodyn_Mals_Svd::runIterativeEFARotation\n";
  k = 0;
  converged = false;
  failed = false;
  ...
  if (!C.size ())
    {
      editor_msg ("red", us_tr ("Internal error: empty C in efa components "
                                "routine, please inform the developers"));
      failed = true;
      return;
    }
...
void
US_Hydrodyn_Mals_Svd::EFAUpdateRotation (vector<vector<int> > &M,
                                         vector<vector<double> > &C,
                                         vector<vector<double> > &D,
                                         vector<int> &force_pos,
                                         // returns
...
for i in range(Cnew.shape[0])])
   ...
   return Cnew
  */
  // qDebug() << "US_Hydrodyn_Mals_Svd::EFAUpdateRotation\n";
...
void
US_Hydrodyn_Mals_Svd::efa_decomp_to_mals_saxs ()
{
  // qDebug() << "efa_decomp_to_mals_saxs";
  for (int i = 0; i < (int)efa_decomp_Ct.size (); ++i)
    {
      QString this_name = QString ("EFA_of_%1_component_%2")
                              .arg (last_efa_name.replace (" ", "_"))
                              .arg (i + 1);
      mals_win->add_plot (this_name, efa_decomp_x, efa_decomp_Ct[i], true);
      editor_msg ("dark blue",
                  QString ("Added %1 to HPLC window").arg (this_name));
    }
}

void
US_Hydrodyn_Mals_Svd::set_efa_decomp_force_positive ()
{
