...
void US_Hydrodyn_Mals_Svd::setupGUI()
{
   ...
   pb_to_mals->setFont(QFont( USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1 ));
...
//    plot_efa_decomp->setMargin(USglobal->config_list.margin);
   ...
   pb_efa_decomp_to_mals_saxs->setFont(QFont( USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1 ));
   pb_efa_decomp_to_mals_saxs->setMinimumHeight(minHeight1);
   pb_efa_decomp_to_mals_saxs->setPalette( PALET_PUSHB );
   connect(pb_efa_decomp_to_mals_saxs, SIGNAL(clicked()), SLOT(efa_decomp_to_mals_saxs()));
   ...
   hide_widgets( data_widgets, 
                 !ush_win->gparams.count( "mals_svd_data_widgets" ) || ush_win->gparams[ "mals_svd_data_widgets" ] == "false" ? false : true );
   hide_widgets( editor_widgets, 
                 !ush_win->gparams.count( "mals_svd_editor_widgets" ) || ush_win->gparams[ "mals_svd_editor_widgets" ] == "false" ? false : true );
   hide_widgets( process_widgets,
                 !ush_win->gparams.count( "mals_svd_process_widgets" ) || ush_win->gparams[ "mals_svd_process_widgets" ] == "false" ? false : true );
   mode_select( MODE_IQIT );
}

void US_Hydrodyn_Mals_Svd::cancel()
{
   close();
}
...
void US_Hydrodyn_Mals_Svd::closeEvent(QCloseEvent *e)
{
   e->accept();
}
...
bool US_Hydrodyn_Mals_Svd::plot_file( QString file,
                                           double &minx,
                                           double &maxx,
                                           double &miny,
                                           double &maxy,
...
bool US_Hydrodyn_Mals_Svd::get_min_max( QString file,
                                             double &minx,
                                             double &maxx,
                                             double &miny,
                                             double &maxy,
...
void US_Hydrodyn_Mals_Svd::hide_data()
{
   ...
   ush_win->gparams[ "mals_svd_data_widgets" ] = data_widgets[ 0 ]->isVisible() ? "visible" : "hidden";
}
...
void US_Hydrodyn_Mals_Svd::hide_editor()
{
   ...
   ush_win->gparams[ "mals_svd_editor_widgets" ] = editor_widgets[ 0 ]->isVisible() ? "visible" : "hidden";
}
...
void US_Hydrodyn_Mals_Svd::hide_process()
{
   ...
   ush_win->gparams[ "mals_svd_process_widgets" ] = process_widgets[ 0 ]->isVisible() ? "visible" : "hidden";
}
...
void US_Hydrodyn_Mals_Svd::update_enables()
{
   ...
   pb_to_mals         ->setEnabled( files.size() && !sources.count( us_tr( "Original data" ) ) && ush_win->mals_widget );
...
void US_Hydrodyn_Mals_Svd::data_selection_changed()
{
   update_enables();
}

void US_Hydrodyn_Mals_Svd::sv_selection_changed()
{
   update_enables();
}
...
void US_Hydrodyn_Mals_Svd::add_i_of_q_or_t( QString source, QStringList files, bool do_update_enables )
{
...
void US_Hydrodyn_Mals_Svd::set_plot_errors_rev()
{
   update_plot_errors();
}
...
bool US_Hydrodyn_Mals_Svd::get_name_norm_mode( QString fullname, QString &name, norms &norm_mode ) {
...
bool US_Hydrodyn_Mals_Svd::get_plot_files( QStringList &use_list, QStringList &use_ref_list, norms &ref_norm_mode )
{
...
void US_Hydrodyn_Mals_Svd::setup_norm() {
   ...
   "file 0 I, errors, I_norm_pw"
      ,f_Is[ mals_selected_files[ 0 ] ]
      ,f_errors[ mals_selected_files[ 0 ] ]
      ,f_Is_norm_pw[ mals_selected_files[ 0 ] ]
      );
...
vector < vector < int > > US_Hydrodyn_Mals_Svd::transpose( vector < vector < int > > &A ) {
   ...
   //   qDebug() << "US_Hydrodyn_Mals_Svd::transpose( vvi ) rows " << rows << " cols " << cols;
...
vector < vector < double > > US_Hydrodyn_Mals_Svd::transpose( vector < vector < double > > &A ) {
   ...
   //   qDebug() << "US_Hydrodyn_Mals_Svd::transpose( vvd ) rows " << rows << " cols " << cols;
...
vector < vector < double > > US_Hydrodyn_Mals_Svd::vivd_pwmult( vector < vector < int > > &A,
...
void US_Hydrodyn_Mals_Svd::matrix_info( QString qs, vector < vector < double > > &A ) {
   ...
   qDebug() << "US_Hydrodyn_Mals_Svd::matrix_info() " << qs << " rows " << A.size() << " cols " << A[0].size();
}   
...
void US_Hydrodyn_Mals_Svd::set_mode_iqit() {
   mode_select( MODE_IQIT );
}

void US_Hydrodyn_Mals_Svd::set_mode_svd() {
   mode_select( MODE_SVD );
}

void US_Hydrodyn_Mals_Svd::set_mode_efa() {
   mode_select( MODE_EFA );
}
...
void US_Hydrodyn_Mals_Svd::update_efa_range_start( double ) {
   ...
   //    QTextStream ts( stdout );
   //    for ( int i = 0; i < UHSHS_MAX_SV_FOR_EFA; ++i ) {
   //       ts << "US_Hydrodyn_Mals_Svd::update_efa_range_start SV " << i << " val " << efa_range_start[ i ]->value() << Qt::endl;
   //    }
...
void US_Hydrodyn_Mals_Svd::set_number_of_svs_for_efa() {
   ...
   // US_Vector::printvector2( "US_Hydrodyn_Mals_Svd::set_number_of_svs_for_efa svd_autocor_U, V", svd_autocor_U, svd_autocor_V );
...
bool US_Hydrodyn_Mals_Svd::convert_it_to_iq( QStringList files, QStringList & created_files, QString & error_msg ) {
   ...
   for ( unsigned int i = 0; i < ( unsigned int ) files.size(); i++ )
   {
      ...
      if ( !mals_win->f_qs.count( files[ i ] ) )
      {
         ...
         for ( unsigned int j = 0; j < ( unsigned int ) mals_win->f_qs[ files[ i ] ].size(); j++ )
         {
            if ( !mode_testiq || ( mals_win->f_qs[ files[ i ] ][ j ] >= t_min && mals_win->f_qs[ files[ i ] ][ j ] <= t_max ) )
            {
               I_values[ mals_win->f_qs[ files[ i ] ][ j ] ][ ql.back() ] = mals_win->f_Is[ files[ i ] ][ j ];
               if ( use_errors && mals_win->f_errors[ files[ i ] ].size() == mals_win->f_qs[ files[ i ] ].size() )
               {
                  e_values[ mals_win->f_qs[ files[ i ] ][ j ] ][ ql.back() ] = mals_win->f_errors[ files[ i ] ][ j ];
               } else {
                  if ( use_errors )
                  {
                     use_errors = false;
         ...
         if ( !mals_win->f_errors.count( files[ i ] ) ||
              mals_win->f_errors[ files[ i ] ].size() != mals_win->f_Is[ files[ i ] ].size() )
         {
            ...
            if ( !mals_win->is_nonzero_vector( mals_win->f_errors[ files[ i ] ] ) )
            {
               ...
               for ( unsigned int j = 0; j < ( unsigned int ) mals_win->f_errors[ files[ i ] ].size(); j++ )
               {
                  if ( !mode_testiq || ( mals_win->f_qs[ files[ i ] ][ j ] >= t_min && mals_win->f_qs[ files[ i ] ][ j ] <= t_max ) )
                  {
                     if ( us_isnan( mals_win->f_errors[ files[ i ] ][ j ] ) || mals_win->f_errors[ files[ i ] ][ j ] == 0e0 )
                     {
                        zero_pts++;
                     }
                  }
               }
               zero_points[ files[ i ] ] = QString( "%1: %2 of %3 points" ).arg( files[ i ] ).arg( zero_pts ).arg( mals_win->f_errors[ files[ i ] ].size() );
...
void US_Hydrodyn_Mals_Svd::EFAFirstRotation(
                                                 vector < vector < int > >    & M,
                                                 vector < vector < double > > & C,
                                                 vector < vector < double > > & D,
                                                 // returns
...
void US_Hydrodyn_Mals_Svd::EFAUpdateRotation(
                                                  vector < vector < int > >    & M,
                                                  vector < vector < double > > & C,
                                                  vector < vector < double > > & D,
                                                  vector < int >               & force_pos,
