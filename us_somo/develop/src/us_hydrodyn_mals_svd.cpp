...
US_Hydrodyn_Mals_Svd::US_Hydrodyn_Mals_Svd(US_Hydrodyn_Mals *mals_win,
                                           vector<QString> mals_selected_files,
                                           QWidget *p)
    : QFrame(p) {
  ...
  if (mals_win->f_is_time[this->mals_selected_files[0]]) {
    ...
    QString error_msg;
    if (!convert_it_to_iq(qsl_mals_selected_files, original_data, error_msg)) {
  ...
  if (!norm_ok) {
    editor_msg(
        "dark red",
        us_tr(
            "Missing or zero associated errors so normalization is disabled"));
  }
...
void US_Hydrodyn_Mals_Svd::setupGUI() {
  int minHeight1 = 24;
  int minHeight3 = 25;
  ...
  for (int i = 0; i < UHSHS_MAX_SV_FOR_EFA; ++i) {
    {
      QLabel *tmp_lbl =
          new QLabel(QString(us_tr("S.V. %1 Start:")).arg(i + 1), this);
  ...
  pb_efa_decomp_to_mals_saxs = new QPushButton(us_tr("To HPLC window"), this);
  ...
  pb_iq_it =
      new QPushButton(us_tr(mode_i_of_t ? "Show I(q)" : "Show I(t)"), this);
  ...
  pb_recon->setToolTip(
      us_tr("Reconstruction is only enabled when all normalization off and SVs "
            "are selected"));
  ...
  // ----- right side
  {
    ...
    //       {
    //          QGridLayout * gl = new QGridLayout( 0 ); gl->setContentsMargins(
    //          0, 0, 0, 0 ); gl->setSpacing( 0 ); gl -> addWidget(
    //          cb_plot_errors       , 0, 0 ); gl -> addWidget(
  ...
  hide_widgets(editor_widgets,
               !ush_win->gparams.count("mals_svd_editor_widgets") ||
                       ush_win->gparams["mals_svd_editor_widgets"] == "false"
                   ? false
                   : true);
  hide_widgets(process_widgets,
               !ush_win->gparams.count("mals_svd_process_widgets") ||
                       ush_win->gparams["mals_svd_process_widgets"] == "false"
                   ? false
                   : true);
  mode_select(MODE_IQIT);
}

void US_Hydrodyn_Mals_Svd::cancel() { close(); }

void US_Hydrodyn_Mals_Svd::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/somo_mals_svd.html");
}

void US_Hydrodyn_Mals_Svd::closeEvent(QCloseEvent *e) { e->accept(); }

void US_Hydrodyn_Mals_Svd::clear_display() {
  editor->clear();
  editor->append("\n\n");
}

void US_Hydrodyn_Mals_Svd::update_font() {
  bool ok;
  QFont newFont;
...
void US_Hydrodyn_Mals_Svd::save() {
  QString fn;
...
void US_Hydrodyn_Mals_Svd::editor_msg(QString color, QString msg) {
...
void US_Hydrodyn_Mals_Svd::plot_files() {
  // puts( "plot_files" );
...
void US_Hydrodyn_Mals_Svd::plot_files(QStringList add_files, norms norm_mode) {
  // puts( "plot_files" );
  plot_data->detachItems(QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems(QwtPlotItem::Rtti_PlotMarker);
  ;
...
bool US_Hydrodyn_Mals_Svd::plot_file(QString file, double &minx, double &maxx,
  ...
  if (!f_qs_string.count(file) || !f_qs.count(file) || !(*f_Iuse).count(file) ||
      !f_pos.count(file)) {
    editor_msg(
        "red",
        QString(
            us_tr("Internal error: request to plot %1, but not found in data"))
            .arg(file));
    return false;
  }
...
bool US_Hydrodyn_Mals_Svd::get_min_max(QString file, double &minx, double &maxx,
                                       double &miny, double &maxy,
                                       norms norm_mode) {
  map<QString, vector<double> > *f_Iuse = &f_Is;
  ...
  if (!(*f_Iuse).count(file) || !f_pos.count(file)) {
    // editor_msg( "red", QString( us_tr( "Internal error: requested %1, but not
    // found in data" ) ).arg( file ) );
    return false;
  }
...
void US_Hydrodyn_Mals_Svd::disable_all() {
...
void US_Hydrodyn_Mals_Svd::q_start_text(const QString &) {}

void US_Hydrodyn_Mals_Svd::q_end_text(const QString &) {}
...
void US_Hydrodyn_Mals_Svd::replot(bool keep_mode) {
  disable_all();
...
void US_Hydrodyn_Mals_Svd::iq_it() {
  disable_all();
...
void US_Hydrodyn_Mals_Svd::to_mals() {
...
void US_Hydrodyn_Mals_Svd::axis_x() {
...
void US_Hydrodyn_Mals_Svd::axis_y() {
...
void US_Hydrodyn_Mals_Svd::hide_data() {
...
void US_Hydrodyn_Mals_Svd::hide_editor() {
...
void US_Hydrodyn_Mals_Svd::hide_process() {
...
void US_Hydrodyn_Mals_Svd::hide_widgets(vector<QWidget *> widgets, bool hide) {
  for (unsigned int i = 0; i < (unsigned int)widgets.size(); i++) {
    hide ? widgets[i]->hide() : widgets[i]->show();
  }
}

void US_Hydrodyn_Mals_Svd::update_enables() {
  // count selections
  if (running) {
    return;
  }
...
void US_Hydrodyn_Mals_Svd::data_selection_changed() { update_enables(); }

void US_Hydrodyn_Mals_Svd::sv_selection_changed() { update_enables(); }

void US_Hydrodyn_Mals_Svd::clear() {
  lv_data->clearSelection();
  update_enables();
}

QStringList US_Hydrodyn_Mals_Svd::selected_files() {
  QStringList result;
...
set<QString> US_Hydrodyn_Mals_Svd::get_current_files() {
  set<QString> result;
...
set<QString> US_Hydrodyn_Mals_Svd::get_sources() {
  set<QString> result;
...
set<QString> US_Hydrodyn_Mals_Svd::get_selected_sources() {
  set<QString> result;
  QString iq_or_it = iq_it_state ? "I(t)" : "I(q)";
...
QTreeWidgetItem *US_Hydrodyn_Mals_Svd::get_source_item(QString source) {
  QTreeWidgetItemIterator it(lv_data);
  while ((*it)) {
    QTreeWidgetItem *item = (*it);
...
int US_Hydrodyn_Mals_Svd::selected_sources() {
  QString iq_or_it = iq_it_state ? "I(t)" : "I(q)";
...
void US_Hydrodyn_Mals_Svd::clean_selected() {
...
bool US_Hydrodyn_Mals_Svd::all_children_selected(QTreeWidgetItem *lvi) {
...
bool US_Hydrodyn_Mals_Svd::is_selected(QTreeWidgetItem *lvi) {
  if (lvi->isSelected()) {
    return true;
  }
  while (lvi->parent()) {
...
void US_Hydrodyn_Mals_Svd::axis_x_title() {
...
void US_Hydrodyn_Mals_Svd::axis_y_title() {
...
bool US_Hydrodyn_Mals_Svd::plotted_matches_selected() {
  set<QString> plotted;
...
void US_Hydrodyn_Mals_Svd::add_i_of_q_or_t(QString source, QStringList files,
  ...
  if (!source_item) {
    editor_msg("red",
               QString(us_tr("Internal error: source item not defined %1"))
                   .arg(source));
    return;
  }
...
void US_Hydrodyn_Mals_Svd::rescale(bool do_update_enables) {
  // bool any_selected = false;
  double minx = 0e0;
  double maxx = 1e0;
  double miny = 0e0;
...
void US_Hydrodyn_Mals_Svd::svd_plot(bool axis_change) {
  ...
  if (svd_autocor_U.size() < svd_x.size()) {
    editor_msg("red", us_tr("Internal error: size mismatch autocorr U. Please "
  ...
  if (svd_autocor_V.size() < svd_x.size()) {
    editor_msg("red", us_tr("Internal error: size mismatch autocorr V. Please "
                            "inform the developers"));
    update_enables();
    return;
  }
...
QString US_Hydrodyn_Mals_Svd::get_name() {
  QTreeWidgetItemIterator it(lv_data);
  while ((*it)) {
    QTreeWidgetItem *item = (*it);
    if (is_selected(item)) {
...
void US_Hydrodyn_Mals_Svd::recon() {
...
QTreeWidgetItem *US_Hydrodyn_Mals_Svd::lvi_last_depth(int d) {
...
void US_Hydrodyn_Mals_Svd::color_rotate() {
...
void US_Hydrodyn_Mals_Svd::inc_rmsd_plot(bool axis_change) {
  plot_data->detachItems(QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems(QwtPlotItem::Rtti_PlotMarker);
  ;
  plotted_curves.clear();
...
void US_Hydrodyn_Mals_Svd::inc_chi_plot(bool axis_change) {
  plot_data->detachItems(QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems(QwtPlotItem::Rtti_PlotMarker);
  ;
  plotted_curves.clear();
  ...
  curve->setSamples((double *)&(chi_x[0]), (double *)&(chi_y[0]), chi_x.size());
...
void US_Hydrodyn_Mals_Svd::inc_recon() {
  ...
  last_recon_tag = QString(us_tr("\"Incremental TSVD on SVD of %1\",\"RMSD of "
                                 "fit\",\"Singular values\""))
                       .arg(last_svd_name);
...
void US_Hydrodyn_Mals_Svd::do_recon() {
  ...
  QString name =
      QString("TSVD %1reconstruction %2 SVs%3")
          .arg(recon_mode)
          .arg(sv_count)
          .arg(sv_count == 1 ? QString(" %1").arg(last_ev) : QString(""));
  ...
  QString tag =
      QString("TSVD%1_%2")
          .arg(sv_count)
          .arg(sv_count == 1 ? QString("%1_").arg(last_ev).replace(".", "_")
                             : QString(""));
  // QTextStream( stdout ) <<
  ...
  do {
    any_found = false;
    for (int i = 0; i < n; ++i) {
      QString this_name = tag + (ext ? QString("%1_").arg(ext) : QString("")) +
                          last_svd_data[i];
...
#if QT_VERSION < 0x040000
    lvinext =
        new QTreeWidgetItem(lvi, evs, QString("RMSD %1").arg(last_recon_rmsd));
#else
...
double US_Hydrodyn_Mals_Svd::vmin(vector<double> &x) {
  double min = x[0];
...
double US_Hydrodyn_Mals_Svd::vmax(vector<double> &x) {
  if (!x.size()) {
    editor_msg("red", us_tr("Internal error: vmax called with empty vector. "
...
void US_Hydrodyn_Mals_Svd::stop() {
  pb_stop->setEnabled(false);
  running = false;
...
void US_Hydrodyn_Mals_Svd::indiv_recon() {
  recon_mode = "Indiv. ";
  ...
  last_recon_tag = QString(us_tr("\"Individual TSVD on SVD of %1\",\"RMSD of "
                                 "fit\",\"Singular value\""))
                       .arg(last_svd_name);
  last_recon_evs.clear();
...
void US_Hydrodyn_Mals_Svd::set_plot_errors() {
...
void US_Hydrodyn_Mals_Svd::set_plot_errors_rev() { update_plot_errors(); }

void US_Hydrodyn_Mals_Svd::set_plot_errors_sd() {
  if (plot_errors_zoomer) {
    plot_errors_zoomer->zoom(0);
    delete plot_errors_zoomer;
    plot_errors_zoomer = (ScrollZoomer *)0;
...
void US_Hydrodyn_Mals_Svd::set_plot_errors_pct() {
  if (plot_errors_zoomer) {
    plot_errors_zoomer->zoom(0);
    delete plot_errors_zoomer;
    plot_errors_zoomer = (ScrollZoomer *)0;
...
void US_Hydrodyn_Mals_Svd::set_plot_errors_group() {
  if (plot_errors_zoomer) {
    plot_errors_zoomer->zoom(0);
    delete plot_errors_zoomer;
    plot_errors_zoomer = (ScrollZoomer *)0;
...
void US_Hydrodyn_Mals_Svd::set_plot_errors_ref() {
...
void US_Hydrodyn_Mals_Svd::update_plot_errors(bool do_update_enables) {
...
QStringList US_Hydrodyn_Mals_Svd::add_subset_data(QStringList files) {
  QStringList result;
  ...
  if (sel_sources.size() != 1) {
    editor_msg("red", QString(us_tr("Internal error: add_subset_data with more "
                                    "than one source (%1)"))
  ...
  do {
    ...
    for (int i = 0; i < (int)files.size(); ++i) {
      QString this_name = tag + (ext ? QString("%1_").arg(ext) : QString("")) +
                          last_svd_data[i].mid(
                              head.length(), files[i].length() - head.length());
      if (current_files.count(this_name)) {
        ext++;
        any_found = true;
        break;
  ...
  for (int i = 0; i < (int)files.size(); ++i) {
    QString this_name =
        tag +
        last_svd_data[i].mid(head.length(), files[i].length() - head.length());
#if QT_VERSION < 0x040000
...
QString US_Hydrodyn_Mals_Svd::get_related_source_name(QString name) {
...
QStringList US_Hydrodyn_Mals_Svd::get_files_by_name(QString name) {
...
bool US_Hydrodyn_Mals_Svd::get_name_norm_mode(QString fullname, QString &name,
                                              norms &norm_mode) {
  ...
  for (int i = NORM_NOT; i <= NORM_AVG; ++i) {
    ...
    if (fullname.contains(rx)) {
      ...
      // QTextStream( stdout ) << "get_name_norm_mode() matched, returning true,
      // name is now: '" << name << "'\n";
      return true;
    }
  }
...
bool US_Hydrodyn_Mals_Svd::get_plot_files(QStringList &use_list,
  ...
  if (sources.size() > 1) {
    // editor_msg( "red", QString( us_tr( "Internal error: get_plot_files() with
    // more than one source (%1)" ) ).arg( sources.size() ) );
    return false;
  }
  ...
  if (ref_source.isEmpty()) {
    if (source != us_tr("Original data")) {
      editor_msg("red", QString(us_tr("Internal error: get_plot_files() could "
  ...
  if (full_list.size() != full_ref_list.size()) {
    editor_msg("red", QString(us_tr("Internal error: get_plot_files() file "
                                    "list count mismatch %1 %2"))
                          .arg(full_list.size())
...
void US_Hydrodyn_Mals_Svd::do_plot_errors() {
  ...
  for (int i = 0; i < (int)use_list.size(); ++i) {
    // QTextStream( stdout )
    //    <<
    //    QString( "use %1 list name '%2' fe %4 qs %5 qss %6 ref list name '%3'
    //    fe %7 qs %8 qss %9" ) .arg( i ) .arg( use_list[ i ] ) .arg(
    ...
    //    f_errors.count( use_ref_list[ i ] ) ? "yes" : "NO" ) .arg( f_qs.count(
    //    use_ref_list[ i ] ) ? "yes" : "NO" ) .arg( f_qs_string.count(
    //    use_ref_list[ i ] ) ? "yes" : "NO" )
    //    <<
    ...
    if (use_errors &&
        (f_errors[use_ref_list[i]].size() != f_qs[use_ref_list[i]].size() ||
    ...
    if (f_qs_string[use_ref_list[i]] != f_qs_string[use_list[i]]) {
      QTextStream(stdout)
          << QString(
                 "do_plot_errors() i=%1 use_ref_list[i] = %2 use_list[i] = "
                 "%3\n")
                 .arg(i)
                 .arg(use_ref_list[i])
                 .arg(use_list[i])
    ...
    // QTextStream( stdout )
    //    <<
    //    QString( "grid_ofs for i = %1 is %2; f_pos is %3 color pos %4 colors
    //    size %5\n" ) .arg( i ) .arg( grid_ofs ) .arg( f_pos[ use_list[ i ] ] )
  ...
  for (int j = 0; j < (int)x.size(); ++j) {
    // US_Vector::printvector2( "x[j], e[j]", x[j], e[j] );
    {
      ...
      curve->setSamples((double *)&x[j][0], (double *)&y[j][0], x[j].size());
...

    {
      ...
      curve->setSamples((double *)&x[j][0], (double *)&e[j][0], x[j].size());
  ...
  if (!plot_errors_zoomer) {
    ...
    plot_errors->setAxisTitle(
        QwtPlot::yLeft,
        QString(us_tr(cb_plot_errors_pct->isChecked()
                          ? "% difference %1"
...
void US_Hydrodyn_Mals_Svd::plot_errors_jump_markers() {
...
void US_Hydrodyn_Mals_Svd::do_plot_errors_group() {
    ...
    if (cb_plot_errors_pct->isChecked()) {
      ...
      if (cb_plot_errors_sd->isChecked() && use_errors) {
        ...
        for (int i = 0; i < (int)f_qs[this_file].size(); ++i) {
          ...
          y.push_back(0e0);
          e.push_back(
              ((*ref_f_Iuse)[this_ref_file][i] - (*org_f_Iuse)[this_file][i]) /
...
bool US_Hydrodyn_Mals_Svd::setup_save(QString tag, QString &fname) {
...
void US_Hydrodyn_Mals_Svd::rmsd_save() {
  QString fname;
...
void US_Hydrodyn_Mals_Svd::svd_save() {
  QString fname;
  ...
  QString out =
      QString(us_tr("\"SVD of %1\",\"Singular value\"")).arg(last_svd_name) +
      "\n";
  for (int i = 0; i < (int)lb_ev->count(); ++i) {
...
void US_Hydrodyn_Mals_Svd::usp_config_plot_data(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_data);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Svd::usp_config_plot_svd(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_svd);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Svd::usp_config_plot_errors(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_errors);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Svd::usp_config_plot_lefa(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_lefa);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Svd::usp_config_plot_refa(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_refa);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Svd::usp_config_plot_ac(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_ac);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Svd::usp_config_plot_efa_decomp(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_efa_decomp);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Svd::set_norm_pw() {
  if (cb_norm_avg->isChecked()) {
    cb_norm_avg->setChecked(false);
  }
  clear_svd();
...
void US_Hydrodyn_Mals_Svd::set_norm_avg() {
  if (cb_norm_pw->isChecked()) {
    cb_norm_pw->setChecked(false);
  }
  clear_svd();
...
void US_Hydrodyn_Mals_Svd::clear_svd() {
  plot_svd->detachItems(QwtPlotItem::Rtti_PlotCurve);
...
void US_Hydrodyn_Mals_Svd::clear_efa() {
...
void US_Hydrodyn_Mals_Svd::clear_efa_decomp() {
...
void US_Hydrodyn_Mals_Svd::efa() {
  ...
  if (efa_lsv.size() != efa_rsv.size()) {
    editor_msg(
        "red",
        us_tr("Unexpected error: Forward EFA and Backward EFA size mismatch"));
  }
...
void US_Hydrodyn_Mals_Svd::make_plotted_efas() {
...
void US_Hydrodyn_Mals_Svd::update_efas(double val) {
  ...
  if (n != (int)efa_rsv.size()) {
    editor_msg("red", us_tr("Unexpected error: Forward EFA and Backward EFA "
                            "size mismatch in plot"));
    return;
  }
...
void US_Hydrodyn_Mals_Svd::efa_plot() {
  int n = (int)efa_lsv.size();
  if (!n) {
    update_efas_ranges();
    return;
  ...
  for (int i = 0; i < efa_plot_count; ++i) {
    {
      ...
      curve->setPen(QPen(plot_colors[i % plot_colors.size()], use_line_width,
                         Qt::SolidLine));
...

    {
      if ((int)efa_range_start.size() <= i) {
        editor_msg("red", us_tr("Internal error: start range point value not "
        ...
        if ((int)plotted_efa_lsv[i].size() <= pos) {
          ...
          m->setSymbol(new QwtSymbol(
              QwtSymbol::Cross, QBrush(plot_colors[i % plot_colors.size()]),
    ...
    }
    {
      if ((int)efa_range_end.size() <= i) {
        editor_msg("red", us_tr("Internal error: end range point value not "
                                "found. please inform the developers"));
      } else {
...
void US_Hydrodyn_Mals_Svd::setup_norm() {
  ...
  for (int i = 0; i < (int)mals_selected_files.size(); ++i) {
    ...
    if (this_q_points != q_points) {
      QMessageBox::warning(this, windowTitle(),
                           us_tr("Internal error: HPLC SVD called but curves "
...
void US_Hydrodyn_Mals_Svd::check_norm(QStringList &files) {
  ...
  for (int i = 0; i < (int)files.size(); ++i) {
    ...
    if (f_Is_norm_pw.count(this_name) || f_Is_norm_avg.count(this_name)) {
      editor_msg(
          "red",
          us_tr("Internal error: unexpected condition 1 - inconsistency, "
    ...
    if (this_q_points != q_points) {
      editor_msg(
          "red",
          us_tr("Internal error: unexpected condition 2 - length mismatch, "
                "turning off normalization, please inform the developers"));
      norm_ok = false;
      return;
    }
    ...
    for (unsigned j = 0; j < q_points; ++j) {
      if (!f_errors[this_name][j]) {
        editor_msg(
            "red",
            us_tr("Internal error: unexpected condition 3 - zero error, "
                  "turning off normalization, please inform the developers"));
...
double US_Hydrodyn_Mals_Svd::autocor1(vector<double> &x) {
  int len = (int)x.size();
...
vector<double> US_Hydrodyn_Mals_Svd::autocor(vector<vector<double> > &A) {
...
vector<vector<int> > US_Hydrodyn_Mals_Svd::transpose(vector<vector<int> > &A) {
...
vector<vector<double> > US_Hydrodyn_Mals_Svd::transpose(
...
double US_Hydrodyn_Mals_Svd::vvd_min(vector<vector<double> > &A) {
  double result = -1e99;
  int rows = (int)A.size();
  if (!rows) {
    editor_msg("red", us_tr("Internal error: vvd_min called with an empty vvd, "
                            "please inform the developers"));
    return result;
  }
  int cols = (int)A[0].size();
  if (!cols) {
    editor_msg("red", us_tr("Internal error: vvd_min called with a columnless "
...
double US_Hydrodyn_Mals_Svd::vvd_max(vector<vector<double> > &A) {
  double result = 1e99;
  int rows = (int)A.size();
  if (!rows) {
    editor_msg("red", us_tr("Internal error: vvd_max called with an empty vvd, "
                            "please inform the developers"));
    return result;
  }
  int cols = (int)A[0].size();
  if (!cols) {
    editor_msg("red", us_tr("Internal error: vvd_max called with a columnless "
                            "vvd, please inform the developers"));
    return result;
  }
...
void US_Hydrodyn_Mals_Svd::vvd_smult(vector<vector<double> > &A, double x) {
  int rows = (int)A.size();
  if (!rows) {
    editor_msg("red", us_tr("Internal error: vdd_smult called with an empty "
                            "vvd, please inform the developers"));
    return;
  }
  int cols = (int)A[0].size();
  if (!cols) {
    editor_msg("red", us_tr("Internal error: vdd_smult called with a "
                            "columnless vvd, please inform the developers"));
    return;
  }
...
vector<vector<double> > US_Hydrodyn_Mals_Svd::vivd_pwmult(
    vector<vector<int> > &A, vector<vector<double> > &B) {
  vector<vector<double> > result;
  int arows = (int)A.size();
  int brows = (int)B.size();
  if (!arows || !brows) {
    editor_msg("red", us_tr("Internal error: vivd_pwmult called with an empty "
                            "vvd, please inform the developers"));
    return result;
  }
  if (arows != brows) {
    ...
    editor_msg("red",
               us_tr("Internal error: vivd_pwmult called with incompatible vi "
                     "& vd, please inform the developers"));
    return result;
  }
  ...
  if (acols != bcols) {
    editor_msg("red",
               us_tr("Internal error: vivd_pwmult called with incompatible vi "
                     "& vd colwise, please inform the developers"));
    return result;
  }
...
void US_Hydrodyn_Mals_Svd::vvd_cnorm(vector<vector<double> > &A) {
  int rows = (int)A.size();
  if (!rows) {
    editor_msg("red", us_tr("Internal error: vvd_cnorm called with an empty "
                            "vvd, please inform the developers"));
    return;
  }
  int cols = (int)A[0].size();
  if (!cols) {
    editor_msg("red", us_tr("Internal error: vvd_cnorm called with a "
                            "columnless vvd, please inform the developers"));
    return;
  }
  ...
  for (int i = 0; i < rows; ++i) {
    ...
    if (csum[i]) {
      ...
      editor_msg("red", us_tr("Internal error: vvd_cnorm called with a zero "
...
void US_Hydrodyn_Mals_Svd::vvd_cnorm(vector<vector<double> > &A,
  ...
  if (arows != brows) {
    editor_msg("red", us_tr("Internal error: vvd_cnorm called with "
                            "incompatible vvds, please inform the developers"));
    return;
  }
...
vector<vector<double> > US_Hydrodyn_Mals_Svd::dot(vector<vector<double> > &A,
                                                  vector<vector<double> > &B) {
  vector<vector<double> > result;
  int arows = (int)A.size();
  int brows = (int)B.size();
  if (!arows || !brows) {
    editor_msg("red", us_tr("Internal error: dot called with an empty vvd's, "
                            "please inform the developers"));
    return result;
  }
...
void US_Hydrodyn_Mals_Svd::matrix_info(QString qs, vector<vector<double> > &A) {
  if (!A.size()) {
...
void US_Hydrodyn_Mals_Svd::set_title(QwtPlot *plot, QString title) {
...
void US_Hydrodyn_Mals_Svd::save_plots() {
  QString use_dir = QDir::currentPath();
  ush_win->select_from_directory_history(use_dir, this);
...
void US_Hydrodyn_Mals_Svd::mode_select() {
  // first hide all
...
void US_Hydrodyn_Mals_Svd::mode_select(modes mode) {
  current_mode = mode;
  mode_select();
  update_enables();
}

void US_Hydrodyn_Mals_Svd::set_mode_iqit() { mode_select(MODE_IQIT); }

void US_Hydrodyn_Mals_Svd::set_mode_svd() { mode_select(MODE_SVD); }

void US_Hydrodyn_Mals_Svd::set_mode_efa() { mode_select(MODE_EFA); }

void US_Hydrodyn_Mals_Svd::set_mode_efa_decomp() {
  mode_select(MODE_EFA_DECOMP);
}

void US_Hydrodyn_Mals_Svd::update_efa_range_start(double) {
  ...
  if ((int)qwtc_efas->value() != (int)qwtpm_lsv_range_marker.size()) {
    editor_msg("red",
               us_tr("Internal error: inconsistency in start range marker size "
                     "& efas values. please inform the developers"));
    return;
  }
...
void US_Hydrodyn_Mals_Svd::update_efa_range_end(double) {
  ...
  if ((int)qwtc_efas->value() != (int)qwtpm_rsv_range_marker.size()) {
    editor_msg("red",
               us_tr("Internal error: inconsistency in end range marker size & "
                     "efas values. please inform the developers"));
    return;
  }
  ...
  for (int i = 0; i < (int)qwtc_efas->value(); ++i) {
    if ((int)efa_range_end.size() <= i) {
      ...
      int pos = (int)efa_range_end[i]->value();
      if ((int)plotted_efa_rsv[i].size() <= pos) {
        editor_msg("red", us_tr("Internal error: end range point value exceeds "
                                "known values. please inform the developers"));
        return;
      } else {
...
void US_Hydrodyn_Mals_Svd::update_efas_ranges() {
...
void US_Hydrodyn_Mals_Svd::efa_info(QString tag) {
  QTextStream ts(stdout);
...
vector<double> US_Hydrodyn_Mals_Svd::gradient(vector<double> &y) {
  int len = (int)y.size();
  int lenm1 = len - 1;
  ...
  if (len < 2) {
    editor_msg("red", us_tr("Internal error: gradient called with emptyish "
...
void US_Hydrodyn_Mals_Svd::init_efa_values() {
  if (plotted_efa_lsv.size() != plotted_efa_rsv.size()) {
    editor_msg("red", us_tr("Internal error: init efa values size mismatch. "
                            "Please inform the developers."));
    return;
  }
  if (!plotted_efa_lsv.size()) {
    editor_msg("red", us_tr("Internal error: init efa values efa empty. Please "
                            "inform the developers."));
    return;
  }
  ...
  for (int i = 0; i < max_range; ++i) {
    {
...
#if defined(DEBUG_INIT_EFA_RANGES)
      {
        ...
        US_Vector::printvector(QString("init efa forward sv %1").arg(i + 1),
                               results);
      }
#endif
...
void US_Hydrodyn_Mals_Svd::set_number_of_svs_for_efa() {
  int max_range =
...
bool US_Hydrodyn_Mals_Svd::convert_it_to_iq(QStringList files,
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size(); i++) {
    if (rx_q.indexIn(files[i]) == -1) {
      error_msg =
          QString(us_tr("Error: Can not find q value in file name for %1"))
              .arg(files[i]);
      return false;
    }
    ql.push_back(rx_q.cap(1).replace("_", ".").toDouble());
    if (used_q.count(ql.back())) {
      error_msg = QString(us_tr("Error: Duplicate q value in file name for %1"))
                      .arg(files[i]);
      return false;
    }
    used_q[ql.back()] = true;
    ...
    if (!mals_win->f_qs.count(files[i])) {
      // error_msg = QString( us_tr( "Internal error: request to use %1, but not
      // found in data" ) ).arg( files[ i ] );
    } else {
      for (unsigned int j = 0;
           j < (unsigned int)mals_win->f_qs[files[i]].size(); j++) {
        if (!mode_testiq || (mals_win->f_qs[files[i]][j] >= t_min &&
                             mals_win->f_qs[files[i]][j] <= t_max)) {
          I_values[mals_win->f_qs[files[i]][j]][ql.back()] =
              mals_win->f_Is[files[i]][j];
          if (use_errors && mals_win->f_errors[files[i]].size() ==
            ...
            if (use_errors) {
              use_errors = false;
              // editor_msg( "dark red", QString( us_tr( "Notice: missing
              // errors, first noticed in %1, so no errors at all" ) )
              //             .arg( files[ i ] ) );
            }
          }
      ...
      if (!mals_win->f_errors.count(files[i]) ||
        ...
        if (!mals_win->is_nonzero_vector(mals_win->f_errors[files[i]])) {
          ...
          for (unsigned int j = 0;
               j < (unsigned int)mals_win->f_errors[files[i]].size(); j++) {
            if (!mode_testiq || (mals_win->f_qs[files[i]][j] >= t_min &&
                                 mals_win->f_qs[files[i]][j] <= t_max)) {
          ...
          zero_points[files[i]] = QString("%1: %2 of %3 points")
                                      .arg(files[i])
                                      .arg(zero_pts)
  ...
  if (zero_points.size() || no_errors.size()) {
    ...
    for (unsigned int i = 0; i < (unsigned int)qsl_no_errors.size() && i < 12;
         i++) {
      qsl_list_no_errors << qsl_no_errors[i];
      used++;
    }
    if (qsl_list_no_errors.size() < qsl_no_errors.size()) {
      qsl_list_no_errors << QString(us_tr("... and %1 more not listed"))
                                .arg(qsl_no_errors.size() -
                                     qsl_list_no_errors.size());
    }
    ...
    if (qsl_list_zero_points.size() < qsl_zero_points.size()) {
      qsl_list_zero_points << QString(us_tr("... and %1 more not listed"))
  ...
  for (unsigned int t = 0; t < tv.size(); t++) {
    ...
    // distribute amoungst the various gaussian peaks we could also reassemble
    // the original sum of gaussians curves as a comparative
    ...
    //       // editor_msg( "red", QString( us_tr( "Error getting concentration
    //       from spline for frame %1, concentration set to zero." ) ).arg( tv[
    //       t ] ) ); conc_factor = 0e0;
    //    }
    // }
    ...
    for (unsigned int i = 0; i < (unsigned int)files.size(); i++) {
      if (!I_values.count(tv[t])) {
        // editor_msg( "dark red", QString( us_tr( "Notice: I values missing
        // frame/time = %1" ) ).arg( tv[ t ] ) );
      }

      if (!I_values[tv[t]].count(qv[i])) {
        // editor_msg( "red", QString( us_tr( "Notice: I values missing q = %1"
        // ) ).arg( qv[ i ] ) );
        continue;
      }
      ...
      if (use_errors) {
        if (!e_values.count(tv[t])) {
          // editor_msg( "red", QString( us_tr( "Internal error: error values
          // missing t %1" ) ).arg( tv[ t ] ) ); running = false;
          // update_enables();
          // progress->reset();
          error_msg =
              QString(us_tr("Internal error: error values missing t %1"))
                  .arg(tv[t]);
          return false;
        }
        ...
        if (!e_values[tv[t]].count(qv[i])) {
          // editor_msg( "red", QString( us_tr( "Internal error: error values
          // missing q %1" ) ).arg( qv[ i ] ) ); running = false;
          // update_enables();
          // progress->reset();
          error_msg =
              QString(us_tr("Internal error: error values missing q %1"))
                  .arg(qv[i]);
          return false;
        }
...
void US_Hydrodyn_Mals_Svd::runExplicitEFARotation(
...
void US_Hydrodyn_Mals_Svd::efa_decomp() {
  ...
  for (int i = 0; i < num_sv; ++i) {
    if (start_vals[i] >= end_vals[i]) {
      editor_msg(
          "red",
          QString(
              us_tr("Ranges for component %1 is empty or negative.  Please "
                    "correct.  Note that the component ranges are in reverse "
                    "order to the S.V's ranges (i.e. the start frame for this "
                    "component is S.V. %2 and the end frame is S.V. %3.)"))
              .arg(i + 1)
              .arg(i + 1)
              .arg(num_sv - i));
      update_enables();
  ...
  */
  ...
  switch (method) {
    case METHOD_HYBRID: {
      initHybridEFA(M, num_sv, D, C, converged, V_bar, niter, tol, force_pos,
                    failed, C_ret, T);
      if (!failed) {
...
void US_Hydrodyn_Mals_Svd::efa_decomp_plot() {
  ...
  plot_efa_decomp->setAxisScale(QwtPlot::yLeft, vvd_min(efa_decomp_Ct) - 0.05,
...
void US_Hydrodyn_Mals_Svd::initExplicitEFA(
...
void US_Hydrodyn_Mals_Svd::initIterativeEFA(
...
void US_Hydrodyn_Mals_Svd::EFAFirstRotation(vector<vector<int> > &M,
...
void US_Hydrodyn_Mals_Svd::initHybridEFA(
    vector<vector<int> > &M, int num_sv, vector<vector<double> > &D,
    vector<vector<double> > &C, bool &converged, vector<vector<double> > &V_bar,
    int niter, double tol, vector<int> &force_pos,
    // returns
    bool &failed, vector<vector<double> > &C_ret,
...
void US_Hydrodyn_Mals_Svd::runIterativeEFARotation(
    vector<vector<int> > &M, vector<vector<double> > &D,
    bool &failed,  // also a return
    vector<vector<double> > &C, vector<vector<double> > & /* V_bar */,
    vector<vector<double> > & /* T */, int niter, double tol,
...
void US_Hydrodyn_Mals_Svd::EFAUpdateRotation(vector<vector<int> > &M,
                                             vector<vector<double> > &C,
                                             vector<vector<double> > &D,
                                             vector<int> &force_pos,
                                             // returns
...
void US_Hydrodyn_Mals_Svd::efa_decomp_to_mals_saxs() {
...
void US_Hydrodyn_Mals_Svd::set_efa_decomp_force_positive() {
