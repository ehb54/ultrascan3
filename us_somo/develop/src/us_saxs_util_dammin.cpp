...
US_Saxs_Util::run_dammin ()
{
          ...
          while (QFile::exists (destname))
            {
              ...
              if (copy_str.length () > 7)
                {
                  errormsg = QString ("Error: dammin: too many attempts to "
                                      "create unique 8 character name for %1")
  ...
  if (control_parameters.count ("dammin1pdb")
      && control_parameters.count ("damminfullname"))
    {
      ...
      if (!fi.exists ())
        {
          errormsg = QString ("Error: dammin: the expected output file %1 "
                              "does not exist")
