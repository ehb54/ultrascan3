...
bool
US_PM::compute_I (set<pm_point> &model, vector<double> &I_result)
{
  if (!model.size ())
    {
...
double
US_PM::model_fit (vector<double> &params, set<pm_point> &model,
...
bool
US_PM::compute_CYJ_I (set<pm_point> &model, vector<double> &I_result)
{
      ...
      if (!pdata.count (*it))
        {
          ...
          if (tmp_pm_data.rtp[0] == 0e0)
            ...
            {
              tmp_pm_data.rtp[1]
                  = acos (tmp_pm_data.x[2] / tmp_pm_data.rtp[0]);
 ...
 .toLatin1().data() );
  ...
  msg_log
      += us_timers.list_times (QString ("CI %1 beads : ").arg (model.size ()));
#endif
  return true;
}

bool
US_PM::compute_delta_I (set<pm_point> &model, set<pm_point> &prev_model,
                        ...
                        vector<double> &I_result)
{
 ...
 .arg( tmp_pm_data.rtp[ 2 ] )
  ...
  for (unsigned int j = 0; j < q_points; ++j)
    {
      ...
      for (unsigned int i = 0; i < (unsigned int)v_pdata_subtracts.size ();
           ++i)
        {
  ...
  msg_log
      += us_timers.list_times (QString ("DI %1 beads : ").arg (model.size ()));
#endif
  return true;
}

bool
US_PM::compute_CA_I (set<pm_point> &model, vector<double> &I_result)
{
#if defined(USE_TIMERS)
  us_timers.clear_timers ();
#endif
                      ...
                      if (it->x[1] < 0)
                        {
                          pm_datap->rtp[2]
                              = M_2PI
                                + asin (pm_datap->x[1]
                                        / sqrt (pm_datap->x[0] * pm_datap->x[0]
                                                + pm_datap->x[1]
                                                      * pm_datap->x[1]));
                        }
                      else
...
#if defined(ALT_SH)
          if (!sh::alt_conj_sh (max_harmonics, pm_datap->rtp[1],
                                pm_datap->rtp[2], Yp))
            {
              error_msg = "sh::spherical_harmonic failed";
              return false;
  ...
  msg_log += us_timers.list_times (
      QString ("FCI %1 beads : ").arg (model.size ()));
#endif
  return true;
}

double
US_PM::fitness2 (vector<double> &I_result)
{
  double chi2 = 0e0;
  double tmp;
