...
bool US_Hydrodyn::calc_grpy_hydro() {
  ...
  if (misc.parallel_grpy && !us_container_grpy->arguments().size()) {
    editor_msg("red", us_tr("parallel GRPY requested but unable to start\n"
                            "Check to make sure you have 'docker' installed "
                            "and a working network connection to the internet\n"
    ...
    if (lb_model->item(current_model)->isSelected()) {
      if (somo_processed[current_model]) {
        ...
        QTextStream(stdout)
            ...
            << "grpy file.fileName() is '" << QFileInfo(fname).baseName() << "'"
            << Qt::endl
        ...
        if (!hydro.grpy_bead_inclusion && grpy_vdw) {
          ...
          editor_msg("black",
                     QString(us_tr("Running ASA check on vdW bead model: Probe "
                                   "radius %1 [A], Threshold %2%."))
                         .arg(asa.vdw_grpy_probe_radius)
                         .arg(asa.vdw_grpy_threshold_percent));
        ...
        // qDebug() << "calc_grpy_hydro() asa rg +/- " << results.asa_rg_pos <<
        // " " << results.asa_rg_neg;
        ...
        write_bead_model(QFileInfo(fname).fileName() + "-grpy", &bead_model);
...
void US_Hydrodyn::grpy_finished(int, QProcess::ExitStatus) {
  ...
  for (int i = 0; i < (int)caps.size(); i += 3) {
    ...
    if (!found) {
      ...
      editor_msg("red", QString(us_tr("Could not find '%1' in GRPY output"))
                            .arg(caps[i].replace("\\", "")));
      grpy_success = false;
...

  {
    ...
    if (!ok) {
      editor_msg("red",
                 us_tr("Could not find rotational diffusion coefficients"));
      grpy_success = false;
    } else {
      i += 19;
      int j = 4;
  ...
  for (map<int, map<QString, double> >::iterator it = data_to_save.begin();
    ...
    this_data.results.mass =
        hydro.mass_correction
            ? hydro.mass
            : model_mw(
                  bead_models[grpy_last_model_number]);  // ( model_vector[
    ...
    this_data.rg_over_fractal_dimension_wtd_sd =
        model_vector[grpy_last_model_number].rg_over_fractal_dimension_wtd_sd;
    this_data.rg_over_fractal_dimension_wtd_wtd =
        model_vector[grpy_last_model_number].rg_over_fractal_dimension_wtd_wtd;
    ...
    if (bead_models[grpy_last_model_number].size() &&
      ...
      this_data.hydrate_probe_radius =
          bead_models[grpy_last_model_number][0].asa_hydrate_probe_radius;
    ...
    // this_data.results.asa_rg_pos << " neg " << this_data.results.asa_rg_neg;
...

    {
      ...
      if (this_data.results.mass) {
        ...
        for (unsigned int i = 0; i < bead_model.size(); ++i) {
          ...
          double bead_tot_mw =
              this_atom->bead_ref_mw + this_atom->bead_ref_ionized_mw_delta;
      ...
      {
        double factor =
            1e-2 * (Rbar / AVOGADRO) * (K0 + hydro.temperature) / pow(fconv, 2);
...

    {
      ...
      add_to_grpy +=
          ...
          us_tr(
              "Summary information\n"
              "The Translational Frictional Coefficient and Frictional Ratio "
              "are\n"
      ...
      add_to_grpy +=
          QString(
              us_tr(
                  " Rot. Diffusion coefficient            X : %1 \t1/s "
                  "(w@20C)\n"
                  " Rot. Diffusion coefficient            Y : %2 \t1/s "
                  "(w@20C)\n"
                  " Rot. Diffusion coefficient            Z : %3 \t1/s "
                  "(w@20C)\n"
                  " Rot. Frictional coefficient           X : %4 \tg*cm^2/s "
                  "(w@20C)\n"
                  " Rot. Frictional coefficient           Y : %5 \tg*cm^2/s "
                  "(w@20C)\n"
                  " Rot. Frictional coefficient           Z : %6 \tg*cm^2/s "
                  "(w@20C)\n"
                  " Rot. Stokes' radius                   X : %7 \tnm (w@20C)\n"
                  " Rot. Stokes' radius                   Y : %8 \tnm (w@20C)\n"
                  " Rot. Stokes' radius                   Z : %9 \tnm "
                  "(w@20C)\n"))
      ...
      add_to_grpy +=
          QString(us_tr(" Maximum extension                     X : %1\n"
...
void US_Hydrodyn::grpy_finalize() {
...

  {
    ...
    if (num <= 1) {
      ...
      if (num <= 1) {
        ...
        grpy_results.s20w_sd = sqrt(fabs(
            (grpy_results2.s20w - grpy_results.s20w * grpy_results.s20w * num) *
            numdecinv));
        grpy_results.D20w_sd = sqrt(fabs(
            (grpy_results2.D20w - grpy_results.D20w * grpy_results.D20w * num) *
            numdecinv));
        grpy_results.viscosity_sd =
            sqrt(fabs((grpy_results2.viscosity -
        ...
        grpy_results.tau_sd = sqrt(fabs(
            ...
            numdecinv));
        grpy_results.rs_sd = sqrt(
            fabs((grpy_results2.rs - grpy_results.rs * grpy_results.rs * num) *
                 numdecinv));
        grpy_results.rg_sd = sqrt(
            fabs((grpy_results2.rg - grpy_results.rg * grpy_results.rg * num) *
                 numdecinv));
        grpy_results.ff0_sd = sqrt(fabs(
            (grpy_results2.ff0 - grpy_results.ff0 * grpy_results.ff0 * num) *
            numdecinv));
        grpy_results.used_beads_sd = sqrt(fabs(
            (grpy_results2.used_beads -
             (double)grpy_results.used_beads * grpy_results.used_beads * num) *
            numdecinv));
        grpy_results.total_beads_sd = sqrt(fabs(
            (grpy_results2.total_beads - (double)grpy_results.total_beads *
    ...
    if (saveParams && create_hydro_res) {
      ...
      if (!f.open(QIODevice::WriteOnly)) {
        ...
        for (int i = 0; i < (int)grpy_mm_save_params.data_vector.size(); ++i) {
          t << save_util->dataString(&grpy_mm_save_params.data_vector[i])
                   .toLatin1()
                   .data();
        }
  ...
  editor_msg("black", QString("Time to process %1")
