...
double
US_Hydrodyn::model_mw (const vector<PDB_atom *> use_model)
{
  double mw = 0e0;
  for (unsigned int i = 0; i < use_model.size (); i++)
    {
      if (use_model[i]->active)
        {
          mw += use_model[i]->bead_ref_mw
                + use_model[i]->bead_ref_ionized_mw_delta;
        }
    }
  return mw;
}
...
vector<PDB_atom>
US_Hydrodyn::exposed_model (const vector<PDB_atom> &model)
{
...
int
US_Hydrodyn::grpy_used_beads_count (const vector<PDB_atom *> use_model)
{
  int used_beads = 0;
  for (unsigned int i = 0; i < use_model.size (); i++)
    {
...
int
US_Hydrodyn::total_beads_count (const vector<PDB_atom> &use_model)
{
  int total_beads = 0;
  for (unsigned int i = 0; i < use_model.size (); i++)
    {
...
US_Hydrodyn::calc_grpy_hydro ()
{
      ...
      if (!grpy_parallel_pulled & misc.parallel_grpy)
        {
          editor_msg (
              "dark red",
              us_tr ("Attempting to get the docker image for parallel GRPY\n"
                     "The first time you run this, it may take awhile\n"
                     "Please be patient\n"));
          qApp->processEvents ();
        }
      us_container_grpy
          = new US_Container_Grpy (!grpy_parallel_pulled, !misc.parallel_grpy);
    }
  ...
  if (misc.parallel_grpy && !us_container_grpy->arguments ().size ())
    {
      editor_msg ("red",
                  us_tr ("parallel GRPY requested but unable to start\n"
                         "Check to make sure you have 'docker' installed and "
      ...
      if (!qfi.exists ())
        {
          editor_msg (
              (QString) "red",
              QString ("GRPY program '%1' does not exist\n").arg (grpy_prog));
          return false;
        }
    }
          ...
          if (somo_processed[current_model])
            {
              ...
              QTextStream (stdout)
                  << "grpy file is '" << fname << "'" << Qt::endl
                  << "grpy file.fileName() is '"
                  << QFileInfo (fname).baseName () << "'" << Qt::endl
              ...
              if (!hydro.grpy_bead_inclusion && grpy_vdw)
                {
                  ...
                  editor_msg (
                      "black",
                      QString (us_tr ("Running ASA check on vdW bead model: "
                                      "Probe radius %1 [A], Threshold %2%."))
                          .arg (asa.vdw_grpy_probe_radius)
                          .arg (asa.vdw_grpy_threshold_percent));
              ...
              // qDebug() << "calc_grpy_hydro() asa rg +/- " <<
              // results.asa_rg_pos << " " << results.asa_rg_neg;

              grpy_addl_param.clear ();
              // grpy_addl_param[ "asa_rg_pos" ] = model_vector[ current_model
              // ].asa_rg_pos; grpy_addl_param[ "asa_rg_neg" ] = model_vector[
              // current_model ].asa_rg_neg;
              grpy_addl_params.push_back (grpy_addl_param);

              write_bead_model (QFileInfo (fname).fileName (), &bead_model,
...
US_Hydrodyn::grpy_process_next ()
{
  ...
  // us_qdebug( "grpy_last_processed " + grpy_last_processed );
  {
    ...
    args << "-e" << grpy_last_processed;

    connect (grpy, SIGNAL (readyReadStandardOutput ()), this,
             SLOT (grpy_readFromStdout ()));
    connect (grpy, SIGNAL (readyReadStandardError ()), this,
             SLOT (grpy_readFromStderr ()));
    connect (grpy, SIGNAL (finished (int, QProcess::ExitStatus)), this,
             SLOT (grpy_finished (int, QProcess::ExitStatus)));
          ...
          if (match.hasMatch ())
            {
              ...
              // qDebug() << "progress value " <<  101 * (
              // grpy_processed.size() - 1 ) + match.captured( 1 ).toDouble() +
              // 1;
...
void
US_Hydrodyn::grpy_readFromStderr ()
{
...
void
US_Hydrodyn::grpy_finished (int, QProcess::ExitStatus)
{
  ...
  QStringList caps;
  caps << "Rotational diffusion coefficient"
       << "1"
       << "Dr"

       << "Sedimentation coefficient \\(Mw Dlt \\(1\\. - "
          "\\(vbar\\*rho\\)\\)/\\(nA kB T\\)\\)"
       << "1"
       << "s"
      ...
      if (!found)
        {
          ...
          editor_msg ("red",
                      QString (us_tr ("Could not find '%1' in GRPY output"))
                          .arg (caps[i].replace ("\\", "")));
          grpy_success = false;
...

  {
    ...
    if (!ok)
      {
        editor_msg (
            "red", us_tr ("Could not find rotational diffusion coefficients"));
        grpy_success = false;
      }
    else
      {
        ...
        for (int k = 0; k < 3; ++k, ++i, ++j)
          {
            ...
            Dr[k] = qsld[j];
            grpy_captures["rot_diff_coef_" + toXYZ[k]].push_back (
                Dr[k].toDouble ());
  ...
  if (!batch_avg_hydro_active () && !grpy_mm)
    {
      QString grpy_out_name
          = grpy_last_processed.replace (QRegExp (".grpy$"), ".grpy_res");
      if (!overwrite_hydro)
        {
      ...
      if (!f.open (QIODevice::WriteOnly))
      ...
      else
        {
          ...
          editor_msg ("dark blue",
                      QString (us_tr ("Wrote %1")).arg (grpy_out_name));
          f.close ();
       ...
       it != data_to_save.end (); ++it)
    {
      save_data this_data
          = US_Hydrodyn_Save::save_data_initialized_from_bead_model (
              bead_models[grpy_last_model_number], !hydro.grpy_bead_inclusion);
      ...
      this_data.results.mass
          = hydro.mass_correction
                ...
                : model_mw (
                      bead_models
                          [grpy_last_model_number]); // ( model_vector[
                                                     // grpy_last_model_number
                                                     // ].mw + model_vector[
                                                     // grpy_last_model_number
                                                     // ].ionized_mw_delta );
      ...
      this_data.results.name =
          // QString( "%1-%2" ).arg( QFileInfo( grpy_last_processed
          // ).completeBaseName().replace( QRegExp( ".grpy$" ), "" ) ).arg(
          // it->first + 1 )
          QFileInfo (grpy_last_processed)
              .completeBaseName ()
              .replace (QRegExp (".grpy$"), "");
      this_data.results.used_beads = grpy_last_used_beads;
      this_data.results.total_beads
          = total_beads_count (bead_models[grpy_last_model_number]);
      this_data.num_of_unused
          = this_data.results.total_beads - this_data.results.used_beads;
      this_data.results.vbar
          = use_vbar (model_vector[grpy_last_model_number].vbar);
      this_data.proc_time
          = (double)(timers.times["compute grpy this model"]) / 1e3;
      this_data.results.asa_rg_pos
          = model_vector[grpy_last_model_number].asa_rg_pos;
      this_data.results.asa_rg_neg
          = model_vector[grpy_last_model_number].asa_rg_neg;
      ...
      this_data.fractal_dimension_parameters
          = model_vector[grpy_last_model_number].fractal_dimension_parameters;
      this_data.fractal_dimension
          = model_vector[grpy_last_model_number].fractal_dimension;
      this_data.fractal_dimension_sd
          = model_vector[grpy_last_model_number].fractal_dimension_sd;
      this_data.fractal_dimension_wtd
          = model_vector[grpy_last_model_number].fractal_dimension_wtd;
      this_data.fractal_dimension_wtd_sd
          = model_vector[grpy_last_model_number].fractal_dimension_wtd_sd;
      this_data.fractal_dimension_wtd_wtd
          = model_vector[grpy_last_model_number].fractal_dimension_wtd_wtd;
      this_data.fractal_dimension_wtd_wtd_sd
          = model_vector[grpy_last_model_number].fractal_dimension_wtd_wtd_sd;
      this_data.rg_over_fractal_dimension
          = model_vector[grpy_last_model_number].rg_over_fractal_dimension;
      this_data.rg_over_fractal_dimension_sd
          = model_vector[grpy_last_model_number].rg_over_fractal_dimension_sd;
      this_data.rg_over_fractal_dimension_wtd
          = model_vector[grpy_last_model_number].rg_over_fractal_dimension_wtd;
      this_data.rg_over_fractal_dimension_wtd_sd
          = model_vector[grpy_last_model_number]
      ...
      if (bead_models[grpy_last_model_number].size ()
          && bead_models[grpy_last_model_number][0].is_vdw == "vdw")
        {
          this_data.hydrate_probe_radius
              = bead_models[grpy_last_model_number][0]
                    .asa_hydrate_probe_radius;
          this_data.hydrate_threshold
              = bead_models[grpy_last_model_number][0].asa_hydrate_threshold;
          this_data.vdw_theo_waters
              = bead_models[grpy_last_model_number][0].vdw_theo_waters;
          this_data.vdw_exposed_residues
              = bead_models[grpy_last_model_number][0].vdw_count_exposed;
          this_data.vdw_exposed_waters
              = bead_models[grpy_last_model_number][0].vdw_theo_waters_exposed;
        }

      // qDebug() << "US_Hydrodyn::grpy_finished() asa rg pos " <<
      // this_data.results.asa_rg_pos << " neg " <<
      // this_data.results.asa_rg_neg;

      if (it->second.count ("Dr"))
        {
          this_data.rot_diff_coef = it->second["Dr"];
          // qDebug() << "this_data.rot_diff_coef : " <<
          // this_data.rot_diff_coef;
        }
...

      {
        ...
        if (this_data.results.mass)
          {
            ...
            for (unsigned int i = 0; i < bead_model.size (); ++i)
              {
                ...
                double bead_tot_mw = this_atom->bead_ref_mw
            ...
            for (unsigned int i = 0; i < bead_model.size (); ++i)
              {
                PDB_atom *this_atom = &(bead_model[i]);
                Rg2 += ((double)this_atom->bead_ref_mw
                        + this_atom->bead_ref_ionized_mw_delta)
...

        {
          if (this_data.results.D20w)
            {
              this_data.tra_fric_coef = Rbar * (K0 + hydro.temperature)
                                        / (AVOGADRO * this_data.results.D20w);

              QTextStream (stdout)
                  << "tfc " << this_data.tra_fric_coef << " = " << Qt::endl
                  << "Rbar * ( K0 + hydro.temperature )"
                  << (Rbar * (K0 + hydro.temperature)) << " / " << Qt::endl
                  << "AVOGADRO * this_data.results.D20w"
                  << (AVOGADRO * this_data.results.D20w) << Qt::endl;
            }
        }

        {
          if (this_data.rot_diff_coef)
            {
              this_data.rot_fric_coef = Rbar * (K0 + hydro.temperature)
                                        / (AVOGADRO * this_data.rot_diff_coef);

              // QTextStream( stdout )
              //    << "rfc " << this_data.rot_fric_coef << " = " << Qt::endl
              //    << "Rbar * ( K0 + hydro.temperature )" << ( Rbar * ( K0 +
              //    hydro.temperature ) ) << " / " << Qt::endl
              //    << "AVOGADRO * this_data.rot_diff_coef" << ( AVOGADRO *
              //    this_data.rot_diff_coef ) << Qt::endl
              //    ;
            }
        }
        ...
        //          Rbar * ( K0 + hydro.temperature ) / ( AVOGADRO * 6.0 * M_PI
        //          * use_solvent_visc() * this_data.results.D20w );
        //       grpy_results.rs  += this_data.results.rs;
        //       grpy_results2.rs += this_data.results.rs *
...

        {
          QTextStream (stdout)
              ...
              << "f0   = "
              << (this_data.results.rs * 6.0 * M_PI * use_solvent_visc ())
              << Qt::endl
              << "f/f0 = "
          ...
          // this_data.tra_fric_coef_sd / this_data.tra_fric_coef;
        ...
        // rot fric and stokes
        {
          double factor = 1e-2 * (Rbar / AVOGADRO) * (K0 + hydro.temperature)
                          / pow (fconv, 2);
          this_data.rot_fric_coef_x = factor / this_data.rot_diff_coef_x;
          this_data.rot_fric_coef_y = factor / this_data.rot_diff_coef_y;
          this_data.rot_fric_coef_z = factor / this_data.rot_diff_coef_z;

          this_data.rot_stokes_rad_x
              = 1e8 * fconv
                * pow (this_data.rot_fric_coef_x
                           / (8.0 * M_PI * use_solvent_visc () * 1e-2),
                       1.0 / 3.0);
          this_data.rot_stokes_rad_y
              = 1e8 * fconv
                * pow (this_data.rot_fric_coef_y
                           / (8.0 * M_PI * use_solvent_visc () * 1e-2),
                       1.0 / 3.0);
          this_data.rot_stokes_rad_z
              = 1e8 * fconv
                * pow (this_data.rot_fric_coef_z
                           / (8.0 * M_PI * use_solvent_visc () * 1e-2),
                       1.0 / 3.0);
        }
      }
      ...
      this_data.rel_times_tau_m
          = (this_data.rel_times_tau_1 + this_data.rel_times_tau_2
             + this_data.rel_times_tau_3 + this_data.rel_times_tau_4
      ...
      grpy_results.name +=
          // ( grpy_results.name.isEmpty() ? QString( this_data.results.name +
          // " " ) : QString( ",-%1" ).arg( grpy_last_model_number + 1 ) )
          this_data.results.name + " ";
...

      {
        ...
        add_to_grpy
            += "\n---------------------------------------------------------\n"
               + us_tr ("Summary information\n"
                        "The Translational Frictional Coefficient and "
        ...
        add_to_grpy
            += QString (us_tr (" Rot. Diffusion coefficient            X : %1 "
                               "\t1/s (w@20C)\n"
                               " Rot. Diffusion coefficient            Y : %2 "
                               "\t1/s (w@20C)\n"
                               " Rot. Diffusion coefficient            Z : %3 "
                               "\t1/s (w@20C)\n"
                               " Rot. Frictional coefficient           X : %4 "
                               "\tg*cm^2/s (w@20C)\n"
                               " Rot. Frictional coefficient           Y : %5 "
                               "\tg*cm^2/s (w@20C)\n"
                               " Rot. Frictional coefficient           Z : %6 "
                               "\tg*cm^2/s (w@20C)\n"
                               " Rot. Stokes' radius                   X : %7 "
                               "\tnm (w@20C)\n"
                               " Rot. Stokes' radius                   Y : %8 "
                               "\tnm (w@20C)\n"
                               " Rot. Stokes' radius                   Z : %9 "
                               "\tnm (w@20C)\n"))
                   .arg (
                       QString ().sprintf ("%5.4g", this_data.rot_diff_coef_x))
                   .arg (
                       QString ().sprintf ("%5.4g", this_data.rot_diff_coef_y))
                   .arg (
                       QString ().sprintf ("%5.4g", this_data.rot_diff_coef_z))
                   .arg (
                       QString ().sprintf ("%5.4g", this_data.rot_fric_coef_x))
                   .arg (
                       QString ().sprintf ("%5.4g", this_data.rot_fric_coef_y))
                   .arg (
                       QString ().sprintf ("%5.4g", this_data.rot_fric_coef_z))
                   .arg (QString ().sprintf ("%5.4g",
                                             this_data.rot_stokes_rad_x))
                   .arg (QString ().sprintf ("%5.4g",
                                             this_data.rot_stokes_rad_y))
                   .arg (QString ().sprintf ("%5.4g",
        ...
        add_to_grpy
            += QString (
                   us_tr (" Maximum extension                     X : %1\n"
                          ...
                          " Axial ratio                         Y:Z : %6\n"))
                   .arg (
                       QString ("%1 nm").arg (this_data.max_ext_x, 0, 'g', 4))
                   .arg (
                       QString ("%1 nm").arg (this_data.max_ext_y, 0, 'g', 4))
                   .arg (
                       QString ("%1 nm").arg (this_data.max_ext_z, 0, 'g', 4))
      ...
      if (saveParams && create_hydro_res && !grpy_mm)
        {
          QString fname
              = get_somo_dir () + "/" + this_data.results.name + ".grpy.csv";
          if (!overwrite_hydro)
            {
              fname = fileNameCheck (fname, 0, this);
            }
...
US_Hydrodyn::grpy_finalize ()
{
    ...
    else
      {
        ...
        if (num <= 1)
        ...
        else
          {
            ...
            grpy_results.s20w_sd
                = sqrt (fabs ((grpy_results2.s20w
                               - grpy_results.s20w * grpy_results.s20w * num)
                              * numdecinv));
            grpy_results.D20w_sd
                = sqrt (fabs ((grpy_results2.D20w
                               - grpy_results.D20w * grpy_results.D20w * num)
                              * numdecinv));
            grpy_results.viscosity_sd = sqrt (
                fabs ((grpy_results2.viscosity
                       - grpy_results.viscosity * grpy_results.viscosity * num)
                      * numdecinv));
            grpy_results.tau_sd = sqrt (fabs (
                (grpy_results2.tau - grpy_results.tau * grpy_results.tau * num)
                * numdecinv));
            grpy_results.rs_sd = sqrt (fabs (
                (grpy_results2.rs - grpy_results.rs * grpy_results.rs * num)
                * numdecinv));
            grpy_results.rg_sd = sqrt (fabs (
                (grpy_results2.rg - grpy_results.rg * grpy_results.rg * num)
                * numdecinv));
            grpy_results.ff0_sd = sqrt (fabs (
                (grpy_results2.ff0 - grpy_results.ff0 * grpy_results.ff0 * num)
                * numdecinv));
            grpy_results.used_beads_sd
                = sqrt (fabs ((grpy_results2.used_beads
                               - (double)grpy_results.used_beads
                                     * grpy_results.used_beads * num)
                              * numdecinv));
            grpy_results.total_beads_sd
                = sqrt (fabs ((grpy_results2.total_beads
                               - (double)grpy_results.total_beads
                                     * grpy_results.total_beads * num)
                              * numdecinv));
  ...
  if (grpy_mm)
    {
      vector<save_data> stats
          = save_util->stats (&grpy_mm_save_params.data_vector);
      ...
      if (saveParams && create_hydro_res)
        {
          ...
          if (!f.open (QIODevice::WriteOnly))
          ...
          else
            {
              ...
              for (int i = 0; i < (int)grpy_mm_save_params.data_vector.size ();
                   ++i)
                {
                  t << save_util
                           ->dataString (&grpy_mm_save_params.data_vector[i])
                           .toLatin1 ()
                           .data ();
                }
  ...
  editor_msg ("black",
              QString ("Time to process %1")
                  .arg (timers.time_min_sec ("compute grpy all models")));
