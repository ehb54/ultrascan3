...
// brownian dynamic browflex computations (this) us_hydrodyn_anaflex_core.cpp
// contains the main computational routines for brownian dynamic (anaflex)
// computations us_hydrodyn_dmd_core.cpp contains the main computational
// routines for molecular dynamic (dmd) computations us_hydrodyn_other.cpp
// contains other routines such as file i/o
...
int
US_Hydrodyn::create_anaflex_files ()
{
...
int
US_Hydrodyn::create_anaflex_files (int use_mode, int sub_mode)
{
  editor->append (
  ...
  // main file
  {
    ...
    QTextStream ts (&f);
    ts << QString ("%1l.txt            !outputfile 1\n"
                   "%2r.txt            !outputfile 2\n"
                   "%3s.txt            !outputfile 3\n"
      ...
      case 1: // steady state
        {
          ...
          int nprops
              = anaflex_options.run_mode_1_1 + anaflex_options.run_mode_1_2
                ...
                + anaflex_options.run_mode_1_12 + anaflex_options.run_mode_1_13
          ...
          ts << QString (
                    "%1,%2                 !number of codes & properties\n")
                    .arg (ncodes)
                    .arg (nprops);
          ...
          if (anaflex_options.run_mode_1_2)
            {
              ts << "2,0,0,0,0                      !square radius of "
                    "gyration\n";
            }
          if (anaflex_options.run_mode_1_3)
            {
              ts << "3,0,0,0,0                      !gzz components of "
                    "gyration tensor\n";
            }
          ...
          if (anaflex_options.run_mode_1_5)
            {
              ts << "5,0,0,0,0                      !gxy,gxz,gyz components "
                    "of gyration tensor\n";
            }
          if (anaflex_options.run_mode_1_7)
            {
              ts << "7,0,0,0,0                      !extension along z and "
                    "it's square\n";
            }
          if (anaflex_options.run_mode_1_8)
            {
              ts << "8,0,0,0,0                      !extension along x and y "
                    "and their squares\n";
            }
          ...
          if (anaflex_options.run_mode_1_13)
            {
              ts << "13,0,0,0,0                     !p2 average over "
                    "connectors \n";
            }
          if (anaflex_options.run_mode_1_14)
            {
              ts << "14,0,0,0,0                     !components of stress "
                    "tensor\n";
            }
      ...
      case 2: // time dependent
        {
          ...
          int nprops
              = anaflex_options.run_mode_2_1 + anaflex_options.run_mode_2_2
                ...
                + anaflex_options.run_mode_2_12 + anaflex_options.run_mode_2_13
                ...
                if (use_last_bead > no_of_beads)
                  {
                    if (!QMessageBox::question (
                            ...
                            QString (us_tr ("You have selected an Anaflex"
                                            " option which requires a last "
                                            "bead number.\n"
                                            "There appear to be %1 beads in "
                                            "your Browflex file\n"
                                            "And your last bead number is set "
                                            "to %2.\n"))
                                .arg (no_of_beads)
                                .arg (use_last_bead),
                            us_tr (QString ("&Use %1 as last bead")
                                       .arg (no_of_beads)),
                 ...
                 << (4.0 * M_PI
                     * anaflex_options.run_mode_3_10_refractive_index
                     / (anaflex_options.run_mode_3_10_lambda * 1e-8))
                        * sin (anaflex_options.run_mode_3_10_theta
                               * (M_PI / 180.0) / 2.0)
...
int
US_Hydrodyn::run_anaflex (int /* use_mode */, int /* sub_mode */)
{
...
void
US_Hydrodyn::anaflex_readFromStdout ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn::anaflex_readFromStderr ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn::anaflex_finished (int, QProcess::ExitStatus)
{
...
void
US_Hydrodyn::anaflex_prepare ()
{
  create_anaflex_files ();
}

void
US_Hydrodyn::anaflex_load_error (QString filename)
{
  editor_msg (
      "red",
...
bool
US_Hydrodyn::anaflex_valid_anaflex_main (QString filename)
{
...
void
US_Hydrodyn::anaflex_run ()
{
  run_anaflex ();
}

void
US_Hydrodyn::anaflex_load_results ()
{
  QString filename;
  if (anaflex_last_file != "")
    {
  ...
  if (!filename.isEmpty ())
    {
      ...
      switch (QMessageBox::question (
          ...
          QString (us_tr (fi.fileName ()
                          + " appears to be a 'main' Anaflex file, open all "
                            "ancillary files? ")),
          QMessageBox::Yes, QMessageBox::No, QMessageBox::NoButton))
        {
        case QMessageBox::No:
