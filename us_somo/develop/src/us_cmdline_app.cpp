...
US_Cmdline_App::US_Cmdline_App (QApplication *qa, QString dir,
                                QStringList args, QStringList app_text,
                                QStringList response, QString *error_msg,
                                int timer_delay_ms
                                // QStringList  * stdout,
                                // QStringList  * stderr
)
  ...
  *error_msg = "";
  ...
  if (!process.waitForStarted ())
#endif
    {
      ...
      *error_msg
          += QString ("Error: could not start process: %1\n").arg (args[0]);
      process.kill ();
      qa->quit ();
      qa->processEvents ();
      return;
              ...
              if (response[query_response_pos].left (2).contains ("__"))
                {
                  *error_msg
                      += QString (
                             "Error: undefined response <%1> to query <%2>\n")
...
void
US_Cmdline_App::finished (int, QProcess::ExitStatus)
{
