...
bool
US_Hydrodyn_Mals::create_i_of_q_ng (set<QString> &fileset, double t_min,
                                    double t_max)
{
  QStringList files;
...
bool
US_Hydrodyn_Mals::create_i_of_q_ng (QStringList files, double t_min,
                                    double t_max)
{
      ...
      if (use_conc)
        {
          ...
          US_Hydrodyn_Mals_Ciq *mals_ciq
              = new US_Hydrodyn_Mals_Ciq (this, &parameters, this);
...
bool
US_Hydrodyn_Mals::create_i_of_q (set<QString> &fileset, double t_min,
                                 double t_max)
{
  QStringList files;
  for (set<QString>::iterator it = fileset.begin (); it != fileset.end ();
...
bool
US_Hydrodyn_Mals::create_i_of_q (QStringList files, double t_min, double t_max)
{
  // us_qdebug( "ciq 0" );
  // for each selected file
  // extract q grid from file names
    ...
    double cutoff_frac
        = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_cb_makeiq_cutmax_pct"]
              ...
              ? ((US_Hydrodyn *)us_hydrodyn)
                        ->gparams["mals_makeiq_cutmax_pct"]
                        .toDouble ()
                    * 0.01
              : 0e0;
...
bool
US_Hydrodyn_Mals::create_unified_ggaussian_target (bool do_init,
...
bool
US_Hydrodyn_Mals::create_unified_ggaussian_target (QStringList &files,
                                                   bool do_init,
                                                   bool only_init_unset)
{
  ...
  if (do_init)
    {
      ...
      if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("mals_cb_gg_smooth")
          && ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_cb_gg_smooth"]
                 == "true"
          && ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("mals_gg_smooth")
          && ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_gg_smooth"].toInt ()
                 > 0)
        {
          editor_msg (
              "darkred",
...
bool
US_Hydrodyn_Mals::ggauss_recompute ()
{
...
bool
US_Hydrodyn_Mals::compute_f_gaussians (QString file, QWidget *mals_fit_widget)
{
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("mals_cb_gg_cyclic")
      && ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_cb_gg_cyclic"] == "true")
    {
      editor_msg (
          "darkRed",
          us_tr (
  ...
  // find peak of curve
  // US_Hydrodyn_Mals_Fit *fit = (US_Hydrodyn_Mals_Fit *)mals_fit_widget;
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("mals_cb_gg_oldstyle")
      && ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_cb_gg_oldstyle"]
             == "true")
    {
      bool cyclic_on
          = ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("mals_cb_gg_cyclic")
            && ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_cb_gg_cyclic"]
                   == "true";
      bool smooth_on
          = ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("mals_cb_gg_smooth")
            && ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_cb_gg_smooth"]
                   == "true"
            && ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("mals_gg_smooth")
            && ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_gg_smooth"].toInt ()
      ...
      if (cyclic_on || smooth_on)
        {
          ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_cb_gg_cyclic"] = "false";
          ...
          if (!compute_f_gaussians_trial (file, mals_fit_widget))
            {
              editor_msg ("red",
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("mals_cb_gg_smooth")
      && ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_cb_gg_smooth"] == "true"
      && ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("mals_gg_smooth")
      && ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_gg_smooth"].toInt () > 0)
    {
      if (!saved)
        {
          saved = true;
...
bool
US_Hydrodyn_Mals::initial_ggaussian_fit (QStringList &files,
                                         bool only_init_unset)
{
  wheel_file = files[0];
  ...
  US_Hydrodyn_Mals_Fit *mals_fit_window
      = new US_Hydrodyn_Mals_Fit (this, false, this);
  US_Hydrodyn::fixWinButtons (mals_fit_window);

  mals_fit_window->update_mals = false;
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
    {
      ...
      if (!only_init_unset || !f_gaussians.count (files[i]))
        {
          if (!compute_f_gaussians (files[i], (QWidget *)mals_fit_window))
            {
              return false;
            }
        }
...
void
US_Hydrodyn_Mals::add_plot_gaussian (const QString &file, const QString &tag)
{
...
bool
US_Hydrodyn_Mals::compute_f_gaussians_trial (QString file,
                                             QWidget *mals_fit_widget)
{
...
void
US_Hydrodyn_Mals::clear_smoothed ()
{
  f_qs_smoothed.clear ();
  f_Is_smoothed.clear ();
...
void
US_Hydrodyn_Mals::list_smoothed ()
{
  TSO << "list_smoothed():\n";
...
void
US_Hydrodyn_Mals::add_smoothed (const QString &name, const vector<double> &q,
...
void
US_Hydrodyn_Mals::clear_oldstyle ()
{
  f_qs_oldstyle.clear ();
  f_Is_oldstyle.clear ();
}
...
void
US_Hydrodyn_Mals::list_oldstyle ()
{
  TSO << "list_oldstyle():\n";
...
void
US_Hydrodyn_Mals::add_oldstyle (const QString &name, const vector<double> &q,
...
void
US_Hydrodyn_Mals::create_ihash_t ()
{
  disable_all ();
...
bool
US_Hydrodyn_Mals::create_ihash_t (QStringList files)
{
  double lambda_cm = mals_param_lambda * 1e-7; // nm to cm

  double K = (4 * M_PI * M_PI * mals_param_n * mals_param_n * mals_param_g_dndc
              * mals_param_g_dndc)
...
void
US_Hydrodyn_Mals::deselect_conc_file ()
{
  if (!lbl_conc_file->text ().isEmpty ())
    {
      for (int i = 0; i < lb_files->count (); i++)
...
void
US_Hydrodyn_Mals::create_istar_q ()
{
  disable_all ();
...
bool
US_Hydrodyn_Mals::create_istar_q (QStringList files, double t_min,
                                  double t_max)
{
...
bool
US_Hydrodyn_Mals::create_istar_q_ng (QStringList files, double t_min,
                                     double t_max)
{
  ...
  head = head.replace (QRegularExpression ("[\\[\\]{}]"), "");
  head += make_ihashq ? "_MALS_Ihashq_t" : "_MALS_Istarq_t";
      ...
      if (use_conc)
        {
          ...
          parameters["default_extc"]
              = QString ("%1").arg (mals_param_g_extinction_coef);
  ...
  for (unsigned int t = 0; t < tv.size (); t++)
    {
      ...
      double conc_factor = make_ihashq ? 1 : 1 / (mals_param_g_conc * 1e-3);
      ...
      if (use_conc && conc_ok)
        {
          if (!usu->apply_natural_spline (conc_spline_x, conc_spline_y,
          ...
          else
            {
              ...
              // mals_param_g_dndc ^2 / dndc ^2 used to correct for K when
              // computing I#
