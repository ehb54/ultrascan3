...
bool US_Hydrodyn_Mals::create_i_of_q_ng(set<QString> &fileset, double t_min,
                                        double t_max) {
  QStringList files;
...
bool US_Hydrodyn_Mals::create_i_of_q_ng(QStringList files, double t_min,
                                        double t_max) {
...
bool US_Hydrodyn_Mals::create_i_of_q(set<QString> &fileset, double t_min,
                                     double t_max) {
  QStringList files;
  for (set<QString>::iterator it = fileset.begin(); it != fileset.end(); ++it) {
    files << *it;
...
bool US_Hydrodyn_Mals::create_i_of_q(QStringList files, double t_min,
                                     double t_max) {
  // us_qdebug( "ciq 0" );
  // for each selected file
  // extract q grid from file names
...
bool US_Hydrodyn_Mals::create_unified_ggaussian_target(bool do_init,
...
bool US_Hydrodyn_Mals::create_unified_ggaussian_target(QStringList &files,
                                                       bool do_init,
                                                       bool only_init_unset) {
...
bool US_Hydrodyn_Mals::ggauss_recompute() {
...
bool US_Hydrodyn_Mals::compute_f_gaussians(QString file,
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count("mals_cb_gg_oldstyle") &&
      ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_cb_gg_oldstyle"] == "true") {
    bool cyclic_on =
...
bool US_Hydrodyn_Mals::initial_ggaussian_fit(QStringList &files,
                                             bool only_init_unset) {
  wheel_file = files[0];
...
void US_Hydrodyn_Mals::add_plot_gaussian(const QString &file,
                                         const QString &tag) {
  add_plot(QString("%1-%2").arg(file).arg(tag), f_qs[file],
           compute_gaussian_sum(f_qs[file], f_gaussians[file]), true, false);
}

bool US_Hydrodyn_Mals::compute_f_gaussians_trial(QString file,
                                                 QWidget *mals_fit_widget) {
...
void US_Hydrodyn_Mals::clear_smoothed() {
  f_qs_smoothed.clear();
  f_Is_smoothed.clear();
  f_best_smoothed_smoothing.clear();
}

void US_Hydrodyn_Mals::list_smoothed() {
  TSO << "list_smoothed():\n";
...
void US_Hydrodyn_Mals::add_smoothed(const QString &name,
...
void US_Hydrodyn_Mals::clear_oldstyle() {
  f_qs_oldstyle.clear();
  f_Is_oldstyle.clear();
}

void US_Hydrodyn_Mals::list_oldstyle() {
  TSO << "list_oldstyle():\n";
...
void US_Hydrodyn_Mals::add_oldstyle(const QString &name,
                                    const vector<double> &q,
                                    const vector<double> &I) {
...
bool US_Hydrodyn_Mals::create_ihash_t(QStringList files) {
  ...
  double K = (4 * M_PI * M_PI * mals_param_n * mals_param_n *
...
bool US_Hydrodyn_Mals::create_istar_q(QStringList files, double t_min,
                                      double t_max) {
...
bool US_Hydrodyn_Mals::create_istar_q_ng(QStringList files, double t_min,
                                         double t_max) {
  // if ( lbl_conc_file->text().isEmpty() ) {
  QMessageBox::information(this, windowTitle() + us_tr(": Make I*(q)"),
                           us_tr("I*(q) will be made without Gaussians"));
  ...
  if (!use_conc && !mals_param_g_conc) {
    if (1 ==
        QMessageBox::question(
            ...
            us_tr("Global concentration in Options->MALS parameters is zero\n"
                  "Do you wish to make I#(q)?"),
            us_tr("&Yes, make I#(q)"), us_tr("&No"), QString(), 0, 1)) {
      QMessageBox::critical(
          this, windowTitle() + us_tr(": Make I*(q)"),
          us_tr("To make I*(q) without a concentration curve,\n"
                "the global concentration must be defined in Options->MALS "
                "parameters"));
      return false;
    }
    make_ihashq = true;
  ...
  for (unsigned int t = 0; t < tv.size(); t++) {
    ...
    if (use_conc && conc_ok) {
      if (!usu->apply_natural_spline(conc_spline_x, conc_spline_y,
        ...
        // mals_param_g_dndc ^2 / dndc ^2 used to correct for K when computing
        // I#
