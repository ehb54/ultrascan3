...
int US_Hydrodyn::calc_somo_o() { return calc_somo(true); }
...
int US_Hydrodyn::calc_somo(bool no_ovlp_removal) {
  ...
  editor_msg("dark blue", QString(us_tr("Peptide Bond Rule is %1 for this PDB"))
      ...
      fflush(stdout);
      {
        ...
        if (retval) {
          ...
          if (error_string.length()) {
                ...
                if (this_atom->active) {
                  ...
                  if (grid.hydrate && this_atom->atom_hydration) {
                    double additional_radius =
                        pow(3 * this_atom->atom_hydration * misc.hydrovol /
                                (M_PI * 4),
                            1.0 / 3.0);
                    this_atom->bead_computed_radius += additional_radius;
#if defined(GRID_HYDRATE_DEBUG)
                    printf(
                        "hydrating atom %s %s %s hydration %f radius %f + %f "
                        "-> %f\n",
                        this_atom->name.toLatin1().data(),
                        this_atom->resName.toLatin1().data(),
                        this_atom->resSeq.toLatin1().data(),
                        this_atom->atom_hydration, this_atom->radius,
                        additional_radius, this_atom->bead_computed_radius);
#endif
                  }
            ...
            {
              editor_msg("red",
                         "Center of scattering requested, but zero scattering "
            ...
            if (grid.enable_asa) {
              ...
              for (unsigned int i = 0; i < bead_model.size(); i++) {
                printf(
                    "after asa bead %d exposed %d color %d chain %d active %d "
                    "mw %f vol %f cr %f [%f,%f,%f]\n",
                    i, bead_model[i].exposed_code, bead_model[i].bead_color,
                    bead_model[i].chain, bead_model[i].active,
                    bead_model[i].bead_ref_mw, bead_model[i].bead_ref_volume,
            ...
            if (grid.create_nmr_bead_pdb &&
              ...
              for (unsigned int k = 0; k < sf_bead_factors.size(); k++) {
                extra_text +=
                    "BSAXS:: " + sf_bead_factors[k].saxs_name.toUpper();
                ...
                for (unsigned int i = 0; i < sf_bead_factors[k].vcoeff.size();
                     i++) {
                  extra_text +=
                      QString(" %1").arg(sf_bead_factors[k].vcoeff[i]);
                }
              ...
              if (extra_saxs_coefficients.count(sf_factors.saxs_name)) {
                editor_msg(
                    "dark red",
                    QString("Notice: extra saxs coefficients %1 replaced\n")
              ...
              editor_msg("blue",
                         QString("Saxs name for dummy atom models set to %1")
              ...
              if (saxs_plot_widget) {
                if (saxs_plot_window->saxs_map.count(sf_factors.saxs_name)) {
                  editor_msg(
                      "dark red",
                      QString(us_tr("Notice: saxs window: saxs coefficients "
                                    "for %1 replaced by newly loaded values\n"))
                          .arg(sf_factors.saxs_name));
                } else {
                  saxs_plot_window->saxs_list.push_back(sf_factors);
                  editor_msg(
                      "dark blue",
                      QString(us_tr("Notice: saxs window: added coefficients "
                                    "for %1 from newly loaded values\n"))
                          .arg(sf_factors.saxs_name));
                }
            ...
            if (saxs_options.compute_saxs_coeff_for_bead_models &&
              ...
              if (!saxs_options.iq_global_avg_for_bead_models &&
                ...
                editor_msg(
                    "red",
                    QString(us_tr("Overriding setting to use global structure "
                                  "factors since bead model doesn't contain "
                                  "the correct number of structure factors "
                                  "(%1) for the beads (%2)"))
              ...
              if (saxs_options.iq_global_avg_for_bead_models ||
                ...
                if (!saxs_util->saxs_map.count(saxs_options.dummy_saxs_name)) {
                  editor_msg(
                      "red",
                      QString(us_tr("Warning: No '%1' SAXS atom found.\n"))
                          .arg(saxs_options.dummy_saxs_name));
                  for (unsigned int i = 0; i < bead_model.size(); i++) {
                    bead_model[i].saxs_data.saxs_name = "";
                  }
                } else {
                  editor_msg("blue", QString(us_tr("Notice: Loading beads with "
                                                   "saxs coefficients '%1'"))
                                         .arg(saxs_options.dummy_saxs_name));
                  for (unsigned int i = 0; i < bead_model.size(); i++) {
                    bead_model[i].saxs_name = saxs_options.dummy_saxs_name;
                    bead_model[i].saxs_data =
                        saxs_util->saxs_map[saxs_options.dummy_saxs_name];
    ...
    if (lb_model->item(current_model)->isSelected()) {
      ...
      if (somo_processed.size() > current_model &&
        ...
        if (grid.enable_asa) {
          ...
          for (unsigned int i = 0; i < bead_model.size(); i++) {
            printf(
                "after asa bead %d exposed %d color %d chain %d active %d mw "
                "%f vol %f cr %f [%f,%f,%f]\n",
                i, bead_model[i].exposed_code, bead_model[i].bead_color,
                bead_model[i].chain, bead_model[i].active,
                bead_model[i].bead_ref_mw, bead_model[i].bead_ref_volume,
        ...
        //        + 1)) + QString(bead_model_suffix.length() ? ("-" +
        //        bead_model_suffix) : "") +
...
int US_Hydrodyn::calc_vdw_beads() {
  ...
  for (current_model = 0; current_model < (unsigned int)lb_model->count();
       current_model++) {
    if (lb_model->item(current_model)->isSelected()) {
      // us_qdebug( QString( "in calc_vdw_beads: is selected current model %1"
      // ).arg( current_model ) );
      {
        QString error_string;
        // create bead model from atoms
...
int US_Hydrodyn::create_vdw_beads(QString &error_string, bool quiet) {
  ...
  if (vdw_ot_mult) {
    ...
    if (total_mw) {
      ...
      editor_msg("red",
                 QString(us_tr("Error computing CoM, OT turned off.\n")));
      vdw_ot_mult = 0;
    }
  }
  ...
  int WAT_Tf_used = 0;
  bool use_WAT_Tf =
      gparams.count("use_WAT_Tf_pdb") && gparams["use_WAT_Tf_pdb"] == "true";

  bool use_vdw_inflate =
      gparams.count("vdw_inflate") && gparams.count("vdw_inflate_multiplier") &&
      gparams["vdw_inflate_mult"] != "1" && gparams["vdw_inflate"] == "true";
    ...
    for (unsigned int k = 0;
      ...
      if (!this_atom->p_residue) {
        editor_msg(
            "red",
            QString(us_tr(
                "Internal error: p_residue not set!, contact the developers")));
        return -1;
      }
      if (!this_atom->p_atom) {
        editor_msg(
            "red",
            QString(us_tr(
                "Internal error: p_atom not set!, contact the developers")));
        return -1;
      }
      ...
      QString res_idx = QString("%1|%2").arg(use_res_name).arg(use_atom_name);
      ...
      if (vdwf.count(res_idx)) {
        // QTextStream( stdout ) << "found ionized_mw_delta " << vdwf[ res_idx
        // ].ionized_mw_delta << endl;
        PDB_atom tmp_atom;
        _vdwf this_vdwf = vdwf[res_idx];
        ...
        if (hydrate && this_vdwf.w && this_atom->asa >= asa.hydrate_threshold &&
            this_vdwf.w) {
          double tmp_vol =
              M_PI * (4e0 / 3e0) * this_vdwf.r * this_vdwf.r * this_vdwf.r +
          ...
          //    }
          // }
          // us_qdebug( QString( "original ot mult %1, waters %2, decreased
          // multiplier %3" ).arg( vdw_ot_mult ).arg( this_vdwf.w ).arg(
          // use_vdw_ot_mult ) );
          if (use_vdw_ot_mult) {
            tmp_atom.bead_coordinate = saxs_util->plus(
                tmp_atom.bead_coordinate,
                saxs_util->scale(saxs_util->normal(saxs_util->minus(
        ...
        // us_qdebug( QString( "bead model radius for %1 = %2" ).arg( res_idx
        // ).arg(  tmp_atom.bead_computed_radius ) ); us_qdebug( QString( "bead
        // model mw for %1 = %2" ).arg( res_idx ).arg( this_vdwf.mw ) );
        ...
        // QTextStream(stdout) << QString( "atom name %1 resname %2 asa %3
        // bead_asa %4 ref_asa %5 bead_recheck_asa %6\n" )
        //    .arg( tmp_atom.name )
...
#warning commented out the next three lines, likely side effects ... were they added for some purpose?
        ...
        editor_msg(
            "red",
            QString("Residue atom pair %1 unknown in vdwf.json").arg(res_idx));
        return -1;
      }
    }
  }

  {
    ...
    if (bead_model.size()) {
      ...
      bead_model[0].vdw_theo_waters =
          QString("%1").arg(summary_totals.theo_waters, 0, 'f', 0).toDouble();
  ...
  if (WAT_Tf_used) {
    editor_msg(
        "dark blue",
        QString(us_tr("Notice: %1 WATs using PDB's Tf radius recognized\n"))
            .arg(WAT_Tf_used));
  }
...

  {
    ...
    if (num <= 1) {
      ...
      if (num <= 1) {
        ...
        hullrad_results.s20w_sd =
            sqrt(fabs((hullrad_results2.s20w -
                       hullrad_results.s20w * hullrad_results.s20w * num) *
                      numdecinv));
        hullrad_results.D20w_sd =
            sqrt(fabs((hullrad_results2.D20w -
                       hullrad_results.D20w * hullrad_results.D20w * num) *
                      numdecinv));
        hullrad_results.viscosity_sd = sqrt(
            fabs((hullrad_results2.viscosity -
        ...
        hullrad_results.rg_sd =
            sqrt(fabs((hullrad_results2.rg -
                      ...
                      numdecinv));
        hullrad_results.ff0_sd =
            sqrt(fabs((hullrad_results2.ff0 -
                       hullrad_results.ff0 * hullrad_results.ff0 * num) *
                      numdecinv));
        hullrad_results.used_beads_sd =
            sqrt(fabs(((double)hullrad_results2.used_beads -
                       (double)hullrad_results.used_beads *
                           (double)hullrad_results.used_beads * num) *
                      numdecinv));
        hullrad_results.total_beads_sd =
            sqrt(fabs(((double)hullrad_results2.total_beads -
...
bool US_Hydrodyn::calc_hullrad_hydro(QString filename) {
...

  {
    ...
    if (!qd.mkdir(hullrad_tmp_path)) {
      editor_msg(
          "red",
          QString(us_tr("Could not create directory %1, check permissions"))
...
bool US_Hydrodyn::calc_fasta_vbar(QStringList &seq_chars, double &result,
                                  QString &msgs) {
  result = 0e0;
  msgs = "";
  if (!seq_chars.size()) {
  ...
  if (replaced.size()) {
    ...
    for (map<QString, int>::iterator it = replaced.begin();
         it != replaced.end(); ++it) {
      msgs +=
          QString(us_tr("Unrecognized code '%1' replaced by 'A' %2 time(s)\n"))
              .arg(it->first)
              .arg(it->second);
    }
  }
