...
// such as disulfide code us_hydrodyn_load.cpp contains code to load files
...
int
US_Hydrodyn::calc_somo_o ()
{
  return calc_somo (true);
}

int
US_Hydrodyn::calc_somo (bool no_ovlp_removal)
{
  progress->set_cli_prefix ("bm");
  citation_build_bead_model (no_ovlp_removal ? "somo-overlaps"
                                             : "somo-no-overlaps");
  {
  ...
  if (selected_models_contain ("WAT"))
    {
      QMessageBox::warning (
          this, us_tr ("Selected model contains WAT residue"),
          us_tr ("Can not process models that contain the WAT residue.\n"
                 "These are currently generated only for SAXS/SANS "
                 "computations\n"));
      return -1;
    }
  ...
  if (selected_models_contain ("XHY"))
    {
      QMessageBox::warning (
          this, us_tr ("Selected model contains XHY residue"),
          us_tr ("Can not process models that contain the XHY residue.\n"
                 "These are currently generated only for SAXS/SANS "
                 "computations\n"));
      return -1;
    }
          ...
          if (!compute_asa (false, no_ovlp_removal))
            {
              ...
              // qDebug() << "us_hydrodyn::calc_somo() current_model " <<
              // current_model << " asa_rg +/- " << results.asa_rg_pos << " "
              // << results.asa_rg_neg;
  ...
  // info_model_vector_mw( QString( "after calc_somo() : model_vector" ),
  // model_vector, true ); info_bead_models_mw( QString( "after calc_somo() :
  // model_vector" ), bead_models );
  progress->reset ();
  return 0;
}

int
US_Hydrodyn::calc_grid_pdb (bool no_ovlp_removal)
{
  citation_build_bead_model (no_ovlp_removal ? "atob-overlaps"
                                             : "atob-no-overlaps");
  {
    int models_selected = 0;
    for (int i = 0; i < lb_model->count (); i++)
      ...
      if ((grid_exposed_overlap.remove_overlap || grid_overlap.remove_overlap
              ...
              || grid_buried_overlap.translate_out))
        {
          ...
          QMessageBox::information (
              this, "US-SOMO: Build AtoB models with structure factors",
              us_tr (msg));
  ...
  editor_msg ("dark blue",
              QString (us_tr ("Peptide Bond Rule is %1 for this PDB"))
                  .arg (misc.pb_rule_on ? "on" : "off"));
  options_log = "";
  no_ovlp_removal ? append_options_log_atob_ovlp ()
      ...
      if (lb_model->item (current_model)->isSelected ())
        {
          ...
          fflush (stdout);
          {
            ...
            int mpos = 2                            // create beads
                       + 4                          // grid_atob
                       + (grid.enable_asa ? (1 + 10 // radial reduction
                ...
                if (error_string.length ())
                ...
                else
                  {
                    ...
                    if (grid.center == 2 && any_zero_si) // ssi
                      {
                        editor_msg ("red",
                                    "Center of scattering requested, but zero "
                    ...
                    bead_models[current_model] = us_hydrodyn_grid_atob (
                        &bead_model, &grid, progress, editor, this);
                    ...
                    if (grid.enable_asa)
                      {
                        ...
                        for (unsigned int i = 0; i < bead_model.size (); i++)
                          {
                            printf (
                                "after asa bead %d exposed %d color %d chain "
                                "%d active %d mw %f vol %f cr %f [%f,%f,%f]\n",
                                i, bead_model[i].exposed_code,
                                bead_model[i].bead_color, bead_model[i].chain,
                                bead_model[i].active,
...
#endif
                        // now apply radial reduction with outward translation
                        // using

                        if (!no_ovlp_removal)
                          {
                            // grid_exposed/buried_overlap
                            overlap_reduction save_sidechain_overlap
                                = sidechain_overlap;
                            overlap_reduction save_mainchain_overlap
                                = mainchain_overlap;
                            overlap_reduction save_buried_overlap
                                = buried_overlap;
                    ...
                    if (grid.create_nmr_bead_pdb
                        ...
                        && !sf_factors.saxs_name.isEmpty ())
                      {
                        ...
                        for (unsigned int k = 0; k < sf_bead_factors.size ();
                             k++)
                          {
                            extra_text
                                += "BSAXS:: "
                                   + sf_bead_factors[k].saxs_name.toUpper ();
                            for (unsigned int i = 0; i < 4; i++)
                              {
                                extra_text += QString (" %1").arg (
                                    sf_bead_factors[k].a[i]);
                                extra_text += QString (" %1").arg (
                            ...
                            extra_text
                                += "BSAXS:: " + sf_bead_factors[k].saxs_name;
                            for (unsigned int i = 0; i < 5; i++)
                              {
                                extra_text += QString (" %1").arg (
                                    sf_bead_factors[k].a5[i]);
                                extra_text += QString (" %1").arg (
                                    sf_bead_factors[k].b5[i]);
                              }
                            extra_text
                                += QString (" %1 %2\n")
                                       .arg (sf_bead_factors[k].c5)
                                       .arg (bead_model[k].saxs_excl_vol);
                            tot_excl_vol += bead_model[k].saxs_excl_vol;

                            extra_text
                                += "BSAXSV:: " + sf_bead_factors[k].saxs_name;
                            for (unsigned int i = 0;
                                 i < sf_bead_factors[k].vcoeff.size (); i++)
                              {
                                extra_text += QString (" %1").arg (
                                    sf_bead_factors[k].vcoeff[i]);
                              }
                        ...
                        for (unsigned int i = 0; i < 4; i++)
                          {
                            extra_text
                                += QString (" %1").arg (sf_factors.a[i]);
                            extra_text
                                += QString (" %1").arg (sf_factors.b[i]);
                          }
                        ...
                        for (unsigned int i = 0; i < 5; i++)
                          {
                            extra_text
                                += QString (" %1").arg (sf_factors.a5[i]);
                            extra_text
                                += QString (" %1").arg (sf_factors.b5[i]);
                          }
                        extra_text += QString (" %1 %2\n")
                                          .arg (sf_factors.c5)
                                          .arg (tot_excl_vol);

                        if (extra_saxs_coefficients.count (
                                sf_factors.saxs_name))
                          {
                            editor_msg ("dark red",
                                        QString ("Notice: extra saxs "
                                                 "coefficients %1 replaced\n")
                                            .arg (sf_factors.saxs_name));
                          }
                        else
                          {
                            saxs_options.dummy_saxs_names.push_back (
                                sf_factors.saxs_name);
                          }
                        ...
                        editor_msg (
                            "blue",
                            QString (
                                "Saxs name for dummy atom models set to %1")
                        ...
                        if (saxs_util->saxs_map.count (sf_factors.saxs_name))
                          {
                            editor_msg (
                                ...
                                QString (
                                    us_tr (
                                        "Notice: saxs coefficients for %1 "
                        ...
                        else
                          {
                            ...
                            editor_msg (
                                "dark blue",
                                QString (
                                    us_tr ("Notice: added coefficients for %1 "
                            ...
                            if (saxs_plot_window->saxs_map.count (
                                    sf_factors.saxs_name))
                              {
                                editor_msg (
                                    "dark red",
                                    QString (
                                        us_tr ("Notice: saxs window: saxs "
                                               "coefficients for %1 replaced "
                                               "by newly loaded values\n"))
                                        .arg (sf_factors.saxs_name));
                              }
                            else
                              {
                                saxs_plot_window->saxs_list.push_back (
                                    sf_factors);
                                editor_msg (
                                    "dark blue",
                                    QString (
                                        us_tr ("Notice: saxs window: added "
                                               "coefficients for %1 from "
                                               "newly loaded values\n"))
                                        .arg (sf_factors.saxs_name));
                              }
                    ...
                    if (saxs_options.compute_saxs_coeff_for_bead_models
                        && grid.create_nmr_bead_pdb)
                      {
                        if (!saxs_options.iq_global_avg_for_bead_models
                            && sf_bead_factors.size () != bead_model.size ())
                          {
                            editor_msg (
                                "red",
                                QString (
                                    us_tr ("Overriding setting to use global "
                                           "structure factors since bead "
                                           "model doesn't contain the correct "
                                           "number of structure factors (%1) "
                                           "for the beads (%2)"))
                        ...
                        if (saxs_options.iq_global_avg_for_bead_models
                            || sf_bead_factors.size () != bead_model.size ())
                          {
                            if (!saxs_util->saxs_map.count (
                                    saxs_options.dummy_saxs_name))
                              {
                                editor_msg (
                                    "red",
                                    QString (us_tr ("Warning: No '%1' SAXS "
                                                    "atom found.\n"))
                                        .arg (saxs_options.dummy_saxs_name));
                                for (unsigned int i = 0;
                                     i < bead_model.size (); i++)
                            ...
                            else
                              {
                                editor_msg (
                                    "blue",
                                    QString (
                                        us_tr ("Notice: Loading beads with "
                                               "saxs coefficients '%1'"))
                                        .arg (saxs_options.dummy_saxs_name));
                                for (unsigned int i = 0;
                                     i < bead_model.size (); i++)
                          ...
                          {
                            if (!saxs_options.iq_global_avg_for_bead_models
                                && sf_bead_factors.size ()
                                       == bead_model.size ())
                              {
                                editor_msg (
                                    "blue",
                                    us_tr ("Notice: Loading beads with bead "
                                           "computed structure factors"));
                                for (unsigned int i = 0;
                                     i < bead_model.size (); i++)
                                  {
                                    bead_model[i].saxs_name
                                        = sf_bead_factors[i].saxs_name;
                                    bead_model[i].saxs_data
                                        = sf_bead_factors[i];
                                    bead_model[i].hydrogens = 0;
                                  }
      ...
      if (lb_model->item (current_model)->isSelected ())
        {
          ...
          if (somo_processed.size () > current_model
              && somo_processed[current_model])
            {
              ...
              if (grid.center == 2) // ssi
                {
                  bool any_zero_si = false;
                  for (unsigned int i = 0;
                       i < bead_models[current_model].size (); i++)
                    {
              ...
              bead_models[current_model] = us_hydrodyn_grid_atob (
                  &bead_models[current_model], &grid, progress, editor, this);
              progress->setValue (progress->value () + 1);
              ...
              if (grid.enable_asa)
                {
                  ...
                  for (unsigned int i = 0; i < bead_model.size (); i++)
                    {
                      printf ("after asa bead %d exposed %d color %d chain %d "
                              "active %d mw %f vol %f cr %f [%f,%f,%f]\n",
                              i, bead_model[i].exposed_code,
                              bead_model[i].bead_color, bead_model[i].chain,
                              bead_model[i].active, bead_model[i].bead_ref_mw,
                              bead_model[i].bead_ref_volume,
                              bead_model[i].bead_computed_radius,
                              bead_model[i].bead_coordinate.axis[0],
                              bead_model[i].bead_coordinate.axis[1],
              ...
              write_bead_model (get_somo_dir () + SLASH + filename
                                    + QString (bead_model_suffix.length ()
                                                   ? ("-" + bead_model_suffix)
                                                   : "")
                                    + DOTSOMO,
...
US_Hydrodyn::calc_vdw_beads ()
{
  ...
  // {
  //    double vdw_ot_mult = gparams.count( "vdw_ot_mult" ) ? gparams[
  //    "vdw_ot_mult" ].toDouble() : 0; double vdw_ot_dpct = gparams.count(
  //    "vdw_ot_dpct" ) ? gparams[ "vdw_ot_dpct" ].toDouble() : 0; if (
  //    vdw_ot_mult ) {
  //       if ( vdw_ot_dpct ) {
  //          bead_model_suffix = QString( "OT%1DP%2-vdw").arg( vdw_ot_mult
  //          ).arg( vdw_ot_dpct );
  //       } else {
  ...
  for (current_model = 0; current_model < (unsigned int)lb_model->count ();
       current_model++)
    {
      if (lb_model->item (current_model)->isSelected ())
        {
          // us_qdebug( QString( "in calc_vdw_beads: is selected current model
          // %1" ).arg( current_model ) );
          {
            QString error_string;
            // create bead model from atoms
            ...
            progress->reset ();
            progress->setMaximum (
                model_vector[current_model].molecule.size ());
            progress->setValue (0);
            ...
            if (retval)
              ...
              {
                editor_msg ("black",
                            QString ("vdW model %1 contains %2 beads\n")
                                .arg (current_model + 1)
...
int
US_Hydrodyn::create_vdw_beads (QString &error_string, bool quiet)
{
  // qDebug() << "create_vdw_beads() start current model = " << current_model;
  // info_mw( "create_vdw_beads() start : model_vector[current_model]",
  // model_vector[current_model], true );
  error_string = "";
  double vdw_ot_mult
      = gparams.count ("vdw_ot_mult") ? gparams["vdw_ot_mult"].toDouble () : 0;
  // double vdw_ot_dpct = gparams.count( "vdw_ot_dpct" ) ? gparams[
  // "vdw_ot_dpct" ].toDouble() : 0; double vdw_ot_d = vdw_ot_dpct * 0.01;
  // us_qdebug( QString( "vdw ot mult %1, additional water decrease percent %2"
  // ).arg( vdw_ot_mult ).arg( vdw_ot_dpct ) );
  bool vdw_ot_alt
  ...
  if (vdw_ot_alt)
    {
      editor_msg (
          "darkred",
          us_tr ("OT alternate method selected but not yet implemented\n"));
      return -1;
    }
  ...
  if (any_wats)
    {
      vdw_ot_mult = 0;
      editor_msg ("black",
                  us_tr ("Explicit WATs found, hydration disabled\n"));
    }
  ...
  // compute ASA for hydration
  {
    ...
    int retval;
    {
      ...
      retval = us_hydrodyn_asab1_main (active_atoms, &asa, &results, false,
  ...
  if (vdw_ot_mult)
    {
      ...
      editor_msg ("black",
                  QString (us_tr ("Using OT multiplier %1 hydrodynamic radius "
                                  "%2 multiplied %3\n"
           ...
           j < model_vector[current_model].molecule.size (); j++)
        {
          for (unsigned int k = 0;
               k < model_vector[current_model].molecule[j].atom.size (); k++)
            {
              ...
              QString res_idx
                  = QString ("%1|%2")
                        .arg (this_atom->name != "OXT" ? this_atom->resName
                                                       : "OXT")
                        .arg (this_atom->name);
              if (vdwf.count (res_idx))
                {
                  ...
                  double this_mw_w_delta
                      = this_vdwf.mw + this_vdwf.ionized_mw_delta;
                  total_mw += this_mw_w_delta;
                  com.axis[0]
                      += this_atom->coordinate.axis[0] * this_mw_w_delta;
                  com.axis[1]
                      += this_atom->coordinate.axis[1] * this_mw_w_delta;
                  com.axis[2]
                      += this_atom->coordinate.axis[2] * this_mw_w_delta;
                }
              else
                {
      ...
      if (total_mw)
      ...
      else
        {
          editor_msg ("red", QString (us_tr (
                                 "Error computing CoM, OT turned off.\n")));
          vdw_ot_mult = 0;
        }
    }

  int WAT_Tf_used = 0;
  bool use_WAT_Tf = gparams.count ("use_WAT_Tf_pdb")
                    && gparams["use_WAT_Tf_pdb"] == "true";

  bool use_vdw_inflate = gparams.count ("vdw_inflate")
  ...
  if (use_vdw_inflate)
    {
      ...
      editor_msg (
          "dark red",
          us_tr (QString ("WARNING: beads are inflated by a factor of %1")
                     .arg (vdw_inflate_mult)));
    }
      ...
      for (unsigned int k = 0;
           k < model_vector[current_model].molecule[j].atom.size (); k++)
        {
          ...
          if (!this_atom->p_residue)
            {
              editor_msg ("red",
                          QString (us_tr ("Internal error: p_residue not "
                                          "set!, contact the developers")));
              return -1;
            }
          if (!this_atom->p_atom)
            {
              editor_msg ("red",
                          QString (us_tr ("Internal error: p_atom not set!, "
          ...
          QString use_res_name
              = this_atom->name != "OXT" ? this_atom->p_residue->name : "OXT";
          QString use_atom_name
              = this_atom->p_residue->name == "N1" ? "N1" : this_atom->name;
          ...
          QString res_idx
              = QString ("%1|%2").arg (use_res_name).arg (use_atom_name);
            ...
            if (!residue_atom_hybrid_map.count (mapkey))
              {
                editor_msg (
                    "red",
                    QString (us_tr ("Error: Missing hybrid name for key %1"))
                        .arg (mapkey));
                return -1;
              }
          ...
          if (vdwf.count (res_idx))
            {
              // QTextStream( stdout ) << "found ionized_mw_delta " << vdwf[
              // res_idx ].ionized_mw_delta << endl;
              PDB_atom tmp_atom;
              _vdwf this_vdwf = vdwf[res_idx];
              // overwrite from p_atom
              this_vdwf.mw = this_atom->p_atom->hybrid.mw;
              this_vdwf.ionized_mw_delta
                  = this_atom->p_atom->hybrid.ionized_mw_delta;
              ...
              if (this_atom->asa >= asa.hydrate_threshold && this_vdwf.w)
                {
                  summary_infos[this_atom->resName].theo_waters_exposed
                      += this_vdwf.w;
              ...
              if (hydrate && this_vdwf.w
                  && this_atom->asa >= asa.hydrate_threshold && this_vdwf.w)
                {
                  double tmp_vol = M_PI * (4e0 / 3e0) * this_vdwf.r
                                       * this_vdwf.r * this_vdwf.r
                                   + (this_vdwf.w * misc.hydrovol);
                  tmp_atom.bead_computed_radius
                      = pow (tmp_vol * 3e0 / (4e0 * M_PI), 1e0 / 3e0);
                  double use_vdw_ot_mult = vdw_ot_mult;
                  // if ( this_vdwf.w > 1 ) {
                  //    use_vdw_ot_mult -= vdw_ot_mult * vdw_ot_d * (
                  //    this_vdwf.w - 1 ); if ( use_vdw_ot_mult < 0e0 ) {
                  //       use_vdw_ot_mult = 0e0;
                  //    }
                  // }
                  // us_qdebug( QString( "original ot mult %1, waters %2,
                  // decreased multiplier %3" ).arg( vdw_ot_mult ).arg(
                  // this_vdwf.w ).arg( use_vdw_ot_mult ) );
                  if (use_vdw_ot_mult)
                    {
                      tmp_atom.bead_coordinate = saxs_util->plus (
                          tmp_atom.bead_coordinate,
                          saxs_util->scale (
                              saxs_util->normal (saxs_util->minus (
                                  this_atom->coordinate, com)),
                              use_vdw_ot_mult * hydro_radius));
                    }
                }
              else
                {
                  tmp_atom.bead_computed_radius
                      = this_vdwf.r * vdw_inflate_mult;
                  if (use_WAT_Tf && this_atom->resName == "WAT"
              ...
              // us_qdebug( QString( "bead model radius for %1 = %2" ).arg(
              // res_idx ).arg(  tmp_atom.bead_computed_radius ) ); us_qdebug(
              // QString( "bead model mw for %1 = %2" ).arg( res_idx ).arg(
              // this_vdwf.mw ) );
              ...
              // tmp_atom.bead_color                = this_atom->asa >=
              // asa.hydrate_threshold ? 6 : 8;
              ...
              // QTextStream(stdout) << QString( "atom name %1 resname %2 asa
              // %3 bead_asa %4 ref_asa %5 bead_recheck_asa %6\n" )
              //    .arg( tmp_atom.name )
...
#warning commented out the next three lines, likely side effects ... were they added for some purpose?
            ...
            {
              editor_msg ("red",
                          QString ("Residue atom pair %1 unknown in vdwf.json")
                              .arg (res_idx));
              return -1;
            }
        }
...

  {
    ...
    for (auto it = summary_infos.begin (); it != summary_infos.end (); ++it)
      {
    ...
    for (auto it = summary_infos.begin (); it != summary_infos.end (); ++it)
      {
        editor_msg ("black", courier,
                    QString ("%1  %2  %3   %4  %5     %6\n")
                        ...
                        .arg (QString ("%1%").arg (
                                  floor (100 * 100.0 * (double)it->second.count
                                         / (double)summary_totals.count)
                                      / 100,
                                  0, 'g', 3),
                              -6)
    ...
    editor_msg ("black", courier,
                QString ("%1  %2  %3   %4  %5     %6\n")
                    ...
                    .arg (QString ("%1%").arg (
                              floor (100 * 100.0 * (double)summary_totals.count
                                     / (double)summary_totals.count)
                                  / 100,
                              0, 'g', 3),
                          -6)
  ...
  if (WAT_Tf_used)
    {
      editor_msg (
          "dark blue",
          QString (
              us_tr ("Notice: %1 WATs using PDB's Tf radius recognized\n"))
              .arg (WAT_Tf_used));
    }
  ...
  // qDebug() << "US_Hydrodyn::create_vdw_beads() asa rg pos " <<
  // results.asa_rg_pos << " neg " << results.asa_rg_neg;

  bead_models[current_model] = bead_model;
  somo_processed[current_model] = 1;

  write_bead_model (
      get_somo_dir () + SLASH + project
          + fix_file_name (QString ("_%1").arg (model_name (current_model)))
...
bool
US_Hydrodyn::compute_asa_rgs (const vector<PDB_atom> &model)
{
...
bool
US_Hydrodyn::load_vdwf_json (QString filename)
{
  QFile f (filename);
  ...
  for (map<QString, QString>::iterator it = parameters.begin ();
    ...
    {
      // us_qdebug( QString( "vdwf read %1 %2" ).arg( it->first ).arg(
      // it->second ) );
      map<QString, QString> tmp_param
          = US_Json::split ("{" + it->second + "}");
      if (!tmp_param.count ("mw") || !tmp_param.count ("r")
          || !tmp_param.count ("w"))
        {
          us_qdebug (QString ("vdwf missing mw r or w %1 %2")
                         .arg (it->first)
                         .arg (it->second));
        }
      else
...
void
US_Hydrodyn::hullrad_finished (int, QProcess::ExitStatus)
{
      ...
      if (rx.indexIn (hullrad_stdout) == -1)
        {
          editor_msg (
              "red",
              QString (us_tr ("Could not find %1 file in HULLRAD output"))
                  .arg (caps[i].replace ("\\", "")));
      ...
      else
        {
          ...
          us_qdebug (QString ("%1 : '%2'\n")
                         .arg (caps[i])
                         .arg (hullrad_captures[caps[i]].back ()));
        }
    }
  ...
  for (map<int, map<QString, double> >::iterator it = data_to_save.begin ();
       it != data_to_save.end (); ++it)
    {
      ...
      this_data.results.name
          = QString ("%1-%2")
                .arg (QFileInfo (hullrad_filename).completeBaseName ())
                .arg (it->first + 1);
      ...
      if (saveParams && create_hydro_res)
        {
          QString fname
              = somo_dir + "/" + this_data.results.name + ".hullrad.csv";
    ...
    else
      {
        ...
        if (num <= 1)
        ...
        else
          {
            ...
            hullrad_results.s20w_sd = sqrt (
                fabs ((hullrad_results2.s20w
                       - hullrad_results.s20w * hullrad_results.s20w * num)
                      * numdecinv));
            hullrad_results.D20w_sd = sqrt (
                fabs ((hullrad_results2.D20w
                       - hullrad_results.D20w * hullrad_results.D20w * num)
                      * numdecinv));
            hullrad_results.viscosity_sd = sqrt (fabs (
                (hullrad_results2.viscosity
                 - hullrad_results.viscosity * hullrad_results.viscosity * num)
                * numdecinv));
            hullrad_results.rs_sd
                = sqrt (fabs ((hullrad_results2.rs
                               - hullrad_results.rs * hullrad_results.rs * num)
                              * numdecinv));
            hullrad_results.rg_sd
                = sqrt (fabs ((hullrad_results2.rg
                               - hullrad_results.rg * hullrad_results.rg * num)
                              * numdecinv));
            hullrad_results.ff0_sd = sqrt (
                fabs ((hullrad_results2.ff0
                       - hullrad_results.ff0 * hullrad_results.ff0 * num)
                      * numdecinv));
            hullrad_results.used_beads_sd = sqrt (
                fabs (((double)hullrad_results2.used_beads
                       - (double)hullrad_results.used_beads
                             * (double)hullrad_results.used_beads * num)
                      * numdecinv));
            hullrad_results.total_beads_sd = sqrt (
                fabs (((double)hullrad_results2.total_beads
                       - (double)hullrad_results.total_beads
                             * (double)hullrad_results.total_beads * num)
                      * numdecinv));
...
void
US_Hydrodyn::create_fasta_vbar_mw ()
{
  ...
  for (map<QString, QChar>::iterator it = residue_short_names.begin ();
       it != residue_short_names.end (); ++it)
    {

      if (it->first != "WAT" && res_vbar.count (it->first)
...
bool
US_Hydrodyn::calc_hullrad_hydro (QString filename)
{
  {
  ...
  hullrad_prog = USglobal->config_list.system_dir + SLASH +
#if defined(BIN64)
                 "bin64"
#else
                 "bin"
  ...
  if (!qfi.exists ())
    {
      editor_msg ((QString) "red",
                  QString ("Hullrad program '%1' does not exist\n")
                      .arg (hullrad_prog));
      return false;
    }
  ...
  if (!f.exists ())
    {
      editor_msg ("red",
                  QString ("PDB file %1 not found").arg (hullrad_filename));
      return false;
    }
    ...
    if (!qd.mkdir (hullrad_tmp_path))
      {
        editor_msg (
            "red",
            QString (
                us_tr ("Could not create directory %1, check permissions"))
                .arg (hullrad_tmp_path));
        return false;
      }
  }
  ...
  QString fn
      = hullrad_tmp_path + "/"
        + QFileInfo (f).fileName ().replace (QRegExp ("\\.(pdb|PDB)$"), "")
        + ext;
...
void
US_Hydrodyn::hullrad_process_next ()
{
  ...
  connect (hullrad, SIGNAL (readyReadStandardOutput ()), this,
           SLOT (hullrad_readFromStdout ()));
  connect (hullrad, SIGNAL (readyReadStandardError ()), this,
           SLOT (hullrad_readFromStderr ()));
  connect (hullrad, SIGNAL (finished (int, QProcess::ExitStatus)), this,
           SLOT (hullrad_finished (int, QProcess::ExitStatus)));
...
void
US_Hydrodyn::hullrad_readFromStdout ()
{
...
void
US_Hydrodyn::hullrad_readFromStderr ()
{
...
bool
US_Hydrodyn::calc_fasta_vbar (QStringList &seq_chars, double &result,
                              QString &msgs)
{
  result = 0e0;
  msgs = "";
  ...
  if (replaced.size ())
    {
      ...
      for (map<QString, int>::iterator it = replaced.begin ();
           it != replaced.end (); ++it)
        {
          msgs += QString (us_tr ("Unrecognized code '%1' replaced by 'A' %2 "
                                  "time(s)\n"))
                      .arg (it->first)
                      .arg (it->second);
        }
  ...
  // qDebug() << "calc_fasta_vbar result for seq of " << seq_chars.size() << "
  // elements computed as " << result;
  return true;
}

bool
US_Hydrodyn::pat_model (vector<PDB_atom> &model)
{
  ...
  // write_dati1_supc_bead_model( "us_hydrodyn_pat_model_pre_pat", ( int )
  // in_dt.size(),  &( in_dt[ 0 ] ) );
  if (!us_hydrodyn_pat_main ((int)in_dt.size (), (int)in_dt.size (),
                             &(in_dt[0]), &out_nat, &(out_dt[0])))
    {
      // cout << QString( "pat ok, out_nat %1\n" ).arg( out_nat );
      int dt_pos = 0;

      // write_dati1_pat_bead_model( "us_hydrodyn_pat_model_post_pat", out_nat,
      // &( out_dt[ 0 ] ), &( in_dt[ 0 ] ) );
