...
bool US_Saxs_Util::atom_align(vector<point> transform_from,
  ...
  if (transform_from.size() != transform_to.size()) {
    errormsg = QString(
                   "Size of transform from (%1) does not equal size of "
                   "transform to (%2)")
  ...
  for (unsigned int i = 1; i < transform_from.size(); i++) {
    for (unsigned int j = 0; j < 3; j++) {
      ...
      transform_from[i].axis[j] =
          QString("").sprintf("%.8f", transform_from[i].axis[j]).toFloat();
      transform_to[i].axis[j] =
          QString("").sprintf("%.8f", transform_to[i].axis[j]).toFloat();
#endif
...
#if defined(DEBUG_ALIGN2)
  {
    ...
    for (unsigned int i = 0; i < transform_from.size(); i++) {
      for (int j = 0; j < 3; j++) {
        out += QString("").sprintf("z2 pos i %u j %u xfrom %.8f xto %.8f\n", i,
                                   j, transform_from[i].axis[j],
                                   transform_to[i].axis[j]);
      }
    }
    cout << out << flush;
