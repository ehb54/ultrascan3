...
bool
US_Saxs_Util::atom_align (vector<point> transform_from,
                          vector<point> transform_to, vector<point> apply_from,
                          vector<point> &result)
{
  errormsg = "";
  ...
  if (transform_from.size () != transform_to.size ())
    {
      errormsg = QString ("Size of transform from (%1) does not equal size of "
                          "transform to (%2)")
      ...
      for (unsigned int j = 0; j < 3; j++)
        {
          ...
          transform_from[i].axis[j]
              = QString ("")
                    .sprintf ("%.8f", transform_from[i].axis[j])
                    .toFloat ();
          transform_to[i].axis[j]
              = QString ("")
                    .sprintf ("%.8f", transform_to[i].axis[j])
                    .toFloat ();
#endif
  ...
  for (unsigned int j = 0; j < 3; j++)
    {
      center_from.axis[j]
          = QString ("").sprintf ("%.8f", center_from.axis[j]).toFloat ();
      center_to.axis[j]
          = QString ("").sprintf ("%.8f", center_to.axis[j]).toFloat ();
    }
#endif
  ...
  if (det (rot) < 0)
    {
      ...
      S[0][0] = S[1][1] = 1.0;
      S[2][2] = det (UVT);
      S[0][1] = S[0][2] = S[1][0] = S[1][2] = S[2][0] = S[2][1] = 0.0;
    ...
    for (unsigned int i = 0; i < transform_from.size (); i++)
      {
        for (int j = 0; j < 3; j++)
          {
            out += QString ("").sprintf (
                "z2 pos i %u j %u xfrom %.8f xto %.8f\n", i, j,
                transform_from[i].axis[j], transform_to[i].axis[j]);
          }
      }
    cout << out << flush;
  }
        ...
        for (unsigned int j = 0; j < 3; j++)
          {
            if (fabs (cmp_waters[i].axis[j] - new_waters[i].axis[j]) > 1e-4)
              {
                errormsg
                    = QString ("mismatch water %1 axis %2\n").arg (i).arg (j);
                return false;
              }
          }
      }
