...
void
US_Hydrodyn::view_file (const QString &filename, QString title)
{
  QFile f (filename);
...
#else
      edit->e->setFont (
          QFont ("monospace", US3i_GuiSettings::fontSize () - 1));
#endif
      edit->e->setText (text);
      edit->show ();
    }
...
void
US_Hydrodyn::closeEvent (QCloseEvent *e)
{
  ...
  // removing the temporary directory requires that the temporary directory is
  // empty. we don't know what else is in the directory if it was created
  // previously. tmp_dir.rmdir(USglobal->config_list.root_dir + "/tmp");
  if (rasmol != NULL)
    {
...
void
US_Hydrodyn::printError (const QString &str)
{
  if (guiFlag)
    {
...
void
US_Hydrodyn::closeAttnt (QProcess *proc, QString message)
{
  switch (QMessageBox::information (
      this, us_tr ("Attention!"),
      message
          + us_tr (" is still running.\n"
...
int
US_Hydrodyn::get_color (const PDB_atom *a)
{
...
QString
US_Hydrodyn::fix_file_name (QString f)
{
...
void
US_Hydrodyn::append_options_log_somo ()
{
  QString s;
  ...
  s.sprintf ("ASA Calculation:\n"
             ...
             ,
             asa.calculation ? "On" : "Off", asa.recheck_beads ? "On" : "Off",
             asa.method ? "Rolling Sphere" : "Voronoi Tesselation",
             asa.probe_radius, asa.probe_recheck_radius, asa.threshold,
             asa.threshold_percent, asa.asab1_step);
  options_log += s;
...
void
US_Hydrodyn::append_options_log_somo_ovlp ()
{
  QString s;
  ...
  s.sprintf ("ASA Calculation:\n"
             ...
             asa.method ? "Rolling Sphere" : "Voronoi Tesselation",
             asa.probe_radius, asa.threshold, asa.threshold_percent,
             asa.asab1_step);
  options_log += s;
...
void
US_Hydrodyn::append_options_log_misc ()
{
  QString s;
...
void
US_Hydrodyn::append_options_log_atob ()
{
  QString s;
  ...
  s.sprintf ("ASA Calculation:\n"
             ...
             asa.calculation ? "On" : "Off", asa.recheck_beads ? "On" : "Off",
             asa.method ? "Rolling Sphere" : "Voronoi Tesselation",
             asa.probe_radius, asa.probe_recheck_radius, asa.grid_threshold,
             asa.grid_threshold_percent, asa.asab1_step);
  options_log += s;
  ...
  s.sprintf (
      ...
      ,
      (grid.center == 0) ? "Center of Mass"
                         : (grid.center == 2 ? "Center of Scattering Intensity"
                                             : "Center of Cubelet"),
      grid.cube_side, grid.cubic ? "On" : "Off", grid.hydrate ? "On" : "Off",
      grid.tangency ? "On" : "Off", grid.enable_asa ? "On" : "Off"
...
void
US_Hydrodyn::append_options_log_atob_ovlp ()
{
  QString s;
...
void
US_Hydrodyn::play_sounds (int type)
{
            ...
            if (QFileInfo (sf).exists ())
              {
                ...
                QString cmd
                    = QString ("mplayer %1&").arg (sf).toLatin1 ().data ();
              ...
              {
                QTextStream (stdout)
                    << "Can't locate sound file " << sf << Qt::endl;
              }
          }
          break;
        default:
...
void
US_Hydrodyn::list_model_vector (vector<PDB_model> *mv)
{
      ...
      for (unsigned int j = 0; j < (*mv)[i].molecule.size (); j++)
        {
          for (unsigned int k = 0; k < (*mv)[i].molecule[j].atom.size (); k++)
            {
              printf ("model %u chain %u atom %u atom %s seq %u resName %s "
                      "chainId %s resSeq %s\n",
                      i, j, k,
...
bool
US_Hydrodyn::install_new_version ()
{
  ...
  for (unsigned int i = 0; i < names.size (); i++)
    {
      fnew[i] = USglobal->config_list.system_dir + "/etc/somo." + names[i]
                + ".new";
  ...
  if (!any_upgrade)
    {
      ...
      if (!US_File_Util::putcontents (somorevision, contents, error))
        {
          qDebug () << "putcontents file:" << somorevision << " contents:'"
                    << contents << "' error:" << error;
        }
      return true;
    }

  // make sure we come up with a new previous version # so we don't overwrite
  // anything
      ...
      for (unsigned int i = 0; i < names.size (); i++)
        {
          if (backup[i])
            {
              while (
                  QFile::exists (QString ("%1/etc/somo-prev-%2.%3")
  ...
  QString msg
      = us_tr ("New versions will be installed for the following files:\n");
  // #ifdef WIN32
  //    msg += us_tr("Note: This step may require you to run as
  //    Administrator.\n");
  // #endif
  for (unsigned int i = 0; i < names.size (); i++)
    {
      ...
      for (unsigned int i = 0; i < names.size (); i++)
        {
          if (backup[i])
            {
              fprev[i] = QString ("%1/etc/somo-prev-%2.%3")
                             // .arg(USglobal->config_list.system_dir)
                             .arg (US_Config::get_home_dir ())
                             .arg (version)
                             .arg (names[i]);
          ...
          if (backup[i])
            {
              printf ("backing up %u (<%s> to <%s>\n", i,
                      fcur[i].toLatin1 ().data (),
                      fprev[i].toLatin1 ().data ());
              ...
              if (names[i] == "config" && backup[i])
                {
                  ...
                  map<QString, QDateTime> save_directory_last_access
                      = directory_last_access;
                  map<QString, QString> save_directory_last_filetype
  ...
  switch (QMessageBox::warning (0, us_tr ("New version detected"), msg,
                                us_tr ("&OK"), us_tr ("&Cancel"), 0, 0, 1))
    {
    case 0:
      {
        QDir qd;
                ...
                if (!qd.rename (fcur[i], fprev[i]))
                  {
                    QMessageBox::critical (
                        0, us_tr ("Could not rename file"),
                        QString (
                            "An error occured when trying to rename file\n"
                            "%1 to %2\n"
                ...
                if (!ufu.copy (fnew[i], fcur[i]))
                  {
                    QMessageBox::critical (
                        0, us_tr ("Could not rename file"),
                        QString (us_tr ("The following error occured when "
                                        "trying to copy:\n"
                                        "%1\n"))
                            .arg (ufu.errormsg));
                    exit (-1);
        ...
        if (!US_File_Util::putcontents (somorevision, contents, error))
          {
            qDebug () << "putcontents file:" << somorevision << " contents:'"
                      << contents << "' error:" << error;
            QMessageBox::critical (
                0, us_tr ("Could not create file"),
                QString (
                    us_tr ("An error occured when trying to create the file\n"
                           "\"%1\".\n"
                           "\"%2\"\n"
                           "Please check your permissions and try again.\n"))
                    .arg (somorevision)
          ...
          {
            QMessageBox::information (
                0, us_tr ("New configuration files successfully installed"),
...
void
US_Hydrodyn::editor_msg (QString color, QString msg)
{
...
void
US_Hydrodyn::editor_msg (QString color, const QFont &font, QString msg)
{
  QFont save_font
      = QFont (USglobal->config_list.fontFamily,
               USglobal->config_list.fontSize); // editor->currentFont();
  QColor save_color = Qt::black;                // editor->textColor();
...
bool
US_Hydrodyn::check_bead_model_for_nan ()
{
  bool issues = false;
...
bool
US_Hydrodyn::is_dammin_dammif (QString filename)
{
  if (!filename.isEmpty ())
    {
      ...
      do
        {
          tmp = ts.readLine ();
          if (tmp.contains (QRegExp ("^ATOM ")) && !tmp.contains ("CA  ASP "))
            {
              f.close ();
              return false;
            }
      ...
      if (ts.atEnd () & !tmp.contains ("Dummy atoms in output phase")
          && !tmp.contains ("Number of particle atoms")
          && !tmp.contains ("Number of atoms written")
          && !tmp.contains ("Filtered number of atoms")
          && !tmp.contains ("Dummy atom radius")
...
QString
US_Hydrodyn::list_chainIDs (vector<PDB_model> &mv)
{
  QString qs;
...
US_Hydrodyn::save_state ()
{
  ...
  state_molecules_residue_missing_atoms_skip
...
void
US_Hydrodyn::rescale_bead_model ()
{
          ...
          if (misc.target_volume != 0e0)
            {
              ...
              editor_msg (
                  "black",
                  QString (
                      us_tr ("Current volume %1 A^3, target volume %2 A^3\n"))
                      .arg (current_volume)
                      .arg (misc.target_volume));
              if (QString ("%1").arg (current_volume)
                ...
                {
                  double multiplier
                      = pow (misc.target_volume / current_volume, 1e0 / 3e0);
                  ...
                  editor_msg (
                      "black",
                      QString (us_tr ("After rescaling: current volume %1 "
                                      "A^3, target volume %2 A^3\n"))
                          .arg (current_volume)
                          .arg (misc.target_volume));
          ...
          if (misc.equalize_radii)
            {
              ...
              if (radii_all_equal (bead_model))
                {
                  editor_msg ("blue",
                              us_tr ("Skipped, radii already equalized"));
                }
              else
                {
                  double current_volume
                      = total_volume_of_bead_model (bead_model);
                  unsigned int beads = number_of_active_beads (bead_model);
                  double pi43 = M_PI * 4e0 / 3e0;

                  // current volume = beads * pi43 * constant-r^3
                  float radius = (float)pow (current_volume / (beads * pi43),
                                             1e0 / 3e0);

                  editor_msg ("black",
                              QString (us_tr ("Volume %1 A^3, Number of beads "
                                              "%2, Radius %3 A\n"))
                                  .arg (current_volume)
                                  .arg (beads)
                                  .arg (radius));
                  ...
                  editor_msg (
                      "black",
                      QString (
                          us_tr ("After equalizing: current volume %1 A^3\n"))
                          .arg (current_volume));
                  bead_models[current_model] = bead_model;
                }
            }
...
csv
US_Hydrodyn::pdb_to_csv (vector<PDB_model> &model_vector)
{
  csv csv1;
      ...
      for (unsigned int j = 0; j < model_vector[i].molecule.size (); j++)
        {
          for (unsigned int k = 0;
               k < model_vector[i].molecule[j].atom.size (); k++)
            {
              ...
              data.push_back (
                  QString ("%1").arg (this_atom->coordinate.axis[0]));
              data.push_back (
                  QString ("%1").arg (this_atom->coordinate.axis[1]));
              data.push_back (
                  QString ("%1").arg (this_atom->coordinate.axis[2]));
...
US_Hydrodyn::make_test_set ()
{
  ...
  if (!dir1.exists ())
    {
      editor_msg ("black",
                  QString (us_tr ("Created directory %1")).arg (test_dir));
      dir1.mkdir (test_dir);
    }
          ...
          if (residue_short_names.count (residue_name)
              && residue_short_names[residue_name] != '~')
            {
              ...
              for (unsigned int j = 0;
                   j < (unsigned int)residue_list[i].r_atom.size (); j++)
                {
                  int hydrogens = 0;
                  QString atom_name = residue_list[i].r_atom[j].name;
                  if (!atom_name.contains ("'") && !atom_name.contains ("*"))
                    {
                      ...
                      QString atom_dir
                          = test_dir + QDir::separator () + atom_name_1;
                      {
                        QDir dir1 (atom_dir);
                        if (!dir1.exists ())
                          {
                            editor_msg (
                                "black",
                                QString (us_tr ("Created directory %1"))
                                    .arg (atom_dir));
                            dir1.mkdir (atom_dir);
                          }
                      ...
                      QString hydrogen_dir = atom_dir + QDir::separator ()
                                             + QString ("%1").arg (hydrogens);
                      {
                      ...
                      if (!d)
                        {
                          pair_summary[QString ("%1H%2")
                                           .arg (atom_name_1)
                                           .arg (hydrogens)]
                              .push_back (QString ("%1 %2")
                                              .arg (residue_name)
                                              .arg (atom_name));
                        }
                    }
...
void
US_Hydrodyn::calc_vol_for_saxs ()
{
  // QTextStream( stdout ) << QString( "calc_vol_for_saxs() model_vector.size()
...
bool
US_Hydrodyn::select_from_directory_history (QString &dir, QWidget *parent,
                                            bool select_dir)
{
  if (!directory_history.size ()
      || (directory_history.size () == 1 && directory_history.contains (dir)))
    {
      return false;
    }
  ...
  US_Dirhist *usd = new US_Dirhist (directory_history, directory_last_access,
                                    directory_last_filetype, selected, is_ok,
                                    parent ? parent : this);
  ...
  //                                     QString( us_tr("Select the directory
  //                                     or Cancel for the default directory
  //                                     of\n%1") ) .arg( dir )
  //                                     ,
  //                                     use_history,
  //                                     current,
...
void
US_Hydrodyn::add_to_directory_history (QString filename, bool accessed)
{
  QFileInfo fi (filename);

  QString dir = QDir::cleanPath (fi.isDir () ? fi.filePath () : fi.path ());

  // us_qdebug( QString( "add to dir history %1 %2 %3 %4" ).arg( filename
  // ).arg( dir ).arg( fi.suffix() ).arg( accessed ? "true" : "false" ) );
  if (dir.isEmpty () || dir.contains (QRegExp ("^\\.")))
    {
      return;
    }
...
void
US_Hydrodyn::model_viewer (QString file, QString prefix, bool nodisplay)
{
  QStringList args;
        ...
        if (!process->waitForStarted ())
          {
            ...
            US_Static::us_message (
                us_tr ("Please note:"),
                QString (us_tr ("nodisplay RASMOL: '%1' could not start.  "
                                "Does input file %2 exist?\n"))
                    .arg (prog)
                    .arg (file));
            delete process;
            return;
    ...
    if (!process->startDetached (prog, args, QFileInfo (file).path ()))
      {
        US_Static::us_message (
            us_tr ("Please note:"),
            us_tr (
                "There was a problem starting RASMOL\n"
...
void
US_Hydrodyn::set_bead_colors (vector<PDB_atom *> use_model)
{
