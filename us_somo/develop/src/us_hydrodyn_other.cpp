...
void US_Hydrodyn::closeEvent(QCloseEvent *e) {
  QMessageBox mb(
      us_tr("UltraScan"), us_tr("Attention:\nAre you sure you want to exit?"),
...
void US_Hydrodyn::closeAttnt(QProcess *proc, QString message) {
  switch (QMessageBox::information(this, us_tr("Attention!"),
                                   message + us_tr(" is still running.\n"
                                                   "Do you want to close it?"),
                                   us_tr("&Kill"), us_tr("&Close gracefully"),
                                   us_tr("Leave running"),
                                   0,   // Enter == button 0
                                   2))  // Escape == button 2
  {
...
void US_Hydrodyn::append_options_log_somo() {
  ...
  s.sprintf(
      ...
      asa.method ? "Rolling Sphere" : "Voronoi Tesselation", asa.probe_radius,
      asa.probe_recheck_radius, asa.threshold, asa.threshold_percent,
      asa.asab1_step);
  options_log += s;
...
void US_Hydrodyn::append_options_log_somo_ovlp() {
  ...
  s.sprintf(
      ...
      asa.method ? "Rolling Sphere" : "Voronoi Tesselation", asa.probe_radius,
      asa.threshold, asa.threshold_percent, asa.asab1_step);
  options_log += s;
...
void US_Hydrodyn::append_options_log_atob() {
  ...
  s.sprintf(
      ...
      asa.calculation ? "On" : "Off", asa.recheck_beads ? "On" : "Off",
      asa.method ? "Rolling Sphere" : "Voronoi Tesselation", asa.probe_radius,
      asa.probe_recheck_radius, asa.grid_threshold, asa.grid_threshold_percent,
      asa.asab1_step);
  options_log += s;
  ...
  for (unsigned int i = 0; i < mv->size(); i++) {
    for (unsigned int j = 0; j < (*mv)[i].molecule.size(); j++) {
      for (unsigned int k = 0; k < (*mv)[i].molecule[j].atom.size(); k++) {
        printf(
            "model %u chain %u atom %u atom %s seq %u resName %s chainId %s "
            "resSeq %s\n",
            i, j, k, (*mv)[i].molecule[j].atom[k].name.toLatin1().data(),
...
bool US_Hydrodyn::install_new_version() {
  ...
  for (unsigned int i = 0; i < names.size(); i++) {
    fnew[i] =
        USglobal->config_list.system_dir + "/etc/somo." + names[i] + ".new";
        ...
        if (install[i]) {
          ...
          if (!ufu.copy(fnew[i], fcur[i])) {
            QMessageBox::critical(
                0, us_tr("Could not rename file"),
                QString(
                    us_tr("The following error occured when trying to copy:\n"
                          "%1\n"))
                    .arg(ufu.errormsg));
            exit(-1);
          }
  ...
  for (unsigned int i = 0; i < bead_model.size() - 1; i++) {
    for (int j = 0; j < 3; j++) {
      if (us_isnan(bead_model[i].bead_coordinate.axis[j])) {
        issues = true;
        editor_msg("red", QString("WARNING: bead %1 coordinate %2 is NAN!\n")
                              .arg(i + 1)
                              .arg(j + 1));
      }
    }
    ...
    if (lb_model->item(current_model)->isSelected() &&
      ...
      if (misc.target_volume != 0e0) {
        ...
        if (QString("%1").arg(current_volume) ==
          ...
          editor_msg("black", QString(us_tr("After rescaling: current volume "
                                            "%1 A^3, target volume %2 A^3\n"))
                                  .arg(current_volume)
                                  .arg(misc.target_volume));
...
void US_Hydrodyn::pdb_tool() {
  ...
  // save_pdb_csv( csv1 );
  US_Hydrodyn_Pdb_Tool *pdb_tool_window = new US_Hydrodyn_Pdb_Tool(csv1, this);
          ...
          if (!atom_name.contains("'") && !atom_name.contains("*")) {
            ...
            QString atom_dir = test_dir + QDir::separator() + atom_name_1;
            {
              ...
              if (!dir1.exists()) {
                editor_msg(
                    "black",
                    QString(us_tr("Created directory %1")).arg(atom_dir));
                dir1.mkdir(atom_dir);
              }
              QDir::setCurrent(atom_dir);
            }

            QString hydrogen_dir =
                atom_dir + QDir::separator() + QString("%1").arg(hydrogens);
            {
              QDir dir1(hydrogen_dir);
              if (!dir1.exists()) {
                editor_msg(
                    "black",
                    QString(us_tr("Created directory %1")).arg(hydrogen_dir));
            ...
            if (f.open(QIODevice::WriteOnly)) {
              QTextStream ts(&f);
              ts << QString("").sprintf(
                  "ATOM      1  %-3s %3s     1       0.000   0.000   0.000  "
                  "1.00 14.00           %1s\n",
                  atom_name.toLatin1().data(), residue_name.toLatin1().data(),
                  atom_name_1.toLatin1().data());

              ts << QString("").sprintf(
                  "ATOM      2  %-3s %3s     2      %2s.000   0.000   0.000  "
                  "1.00 14.00           %1s\n",
                  atom_name.toLatin1().data(), residue_name.toLatin1().data(),
                  ds[d].toLatin1().data(), atom_name_1.toLatin1().data());
              f.close();
            }
            if (!d) {
...
bool US_Hydrodyn::select_from_directory_history(QString &dir, QWidget *parent,
  ...
  //                                     QString( us_tr("Select the directory or
  //                                     Cancel for the default directory
  //                                     of\n%1") ) .arg( dir )
  //                                     ,
  //                                     use_history,
  //                                     current,
...
void US_Hydrodyn::add_to_directory_history(QString filename, bool accessed) {
  ...
  // us_qdebug( QString( "add to dir history %1 %2 %3 %4" ).arg( filename ).arg(
  // dir ).arg( fi.suffix() ).arg( accessed ? "true" : "false" ) );
...
void US_Hydrodyn::model_viewer(QString file, QString prefix, bool nodisplay) {
  QStringList args;
...

  {
    ...
    if (nodisplay) {
      ...
      if (!process->waitForStarted()) {
        ...
        US_Static::us_message(
            us_tr("Please note:"),
            QString(us_tr("nodisplay RASMOL: '%1' could not start.  Does input "
                          "file %2 exist?\n"))
                .arg(prog)
                .arg(file));
        delete process;
