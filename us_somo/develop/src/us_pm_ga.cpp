...
unsigned int
US_PM::ga_pop_selection (unsigned int size)
{
...
bool
US_PM::ga_fitness (pm_ga_individual &individual)
{
...
US_PM::ga_compute_fitness ()
{
  ...
  for (set<int>::iterator it = pm_workers_registered.begin ();
    ...
    {
      //       cout << QString(
      //                       "ga_compute_fitness initial requests for()
      ...
      // if ( us_log )
      // {
      //    us_log->log( US_Vector::qs_vector( "ga_compute_fitness: (for)
      //    params to send", ga_params ) );
      // }
      fit_calls++;

      if (MPI_SUCCESS
          != MPI_Send (&msg, sizeof (msg), MPI_CHAR, *it, PM_MSG,
                       MPI_COMM_WORLD))
        {
          if (us_log)
            {
              us_log->log (
                  QString ("%1: MPI PM_MSG Send failed ga_compute_fitness()\n")
                      .arg (myrank));
            }
      ...
      if (MPI_SUCCESS
          != MPI_Send (&(ga_params[0]), msg.vsize * sizeof (double), MPI_CHAR,
                       *it, PM_CALC_FITNESS, MPI_COMM_WORLD))
        {
          if (us_log)
            {
              us_log->log (QString ("%1: MPI PM_CALC_FITNESS Send failed "
                                    "ga_compute_fitness()\n")
                               .arg (myrank));
            }
          MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
      ...
      if (it_pop != ga_pop.end ())
        {
          if (us_log)
            {
              us_log->log (QString ("%1: error: ga_compute_fitness() no "
                                    "worker busy but we have more ga_pop?\n")
                               .arg (myrank));
            }
          MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
          exit (errorno - myrank);
  ...
  while (pm_workers_busy.size ())
    {
      ...
      if (MPI_SUCCESS
          != MPI_Recv (&msg_receive, sizeof (msg_receive), MPI_CHAR,
                       MPI_ANY_SOURCE, PM_FITNESS_RESULT, MPI_COMM_WORLD,
                       &mpi_status))
        {
          if (us_log)
            {
              us_log->log (QString ("%1: MPI PM_FITNESS_RESULT Receive failed "
                                    "in ga_compute_fitness()\n")
                               .arg (myrank));
            }
          MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
          exit (errorno - myrank);
      ...
      if (MPI_SUCCESS
          != MPI_Recv (&(vmodel[0]), 3 * msg_receive.vsize * sizeof (int16_t),
                       ...
                       PM_FITNESS_RESULT_MODEL, MPI_COMM_WORLD, &mpi_status2))
        {
          if (us_log)
            {
              us_log->log (QString ("%1: MPI PM_FITNESS_RESULT Receive failed "
      ...
      // if ( us_log )
      // {
      //    us_log->log( US_Vector::qs_vector( "ga_compute_fitness: (while)
      //    params to send", ga_params ) );
      // }
      ...
      if (MPI_SUCCESS
          != MPI_Send (&msg, sizeof (msg), MPI_CHAR, mpi_status.MPI_SOURCE,
                       PM_MSG, MPI_COMM_WORLD))
        {
          if (us_log)
            {
              us_log->log (
      ...
      if (MPI_SUCCESS
          != MPI_Send (&(ga_params[0]), msg.vsize * sizeof (double), MPI_CHAR,
                       mpi_status.MPI_SOURCE, PM_CALC_FITNESS, MPI_COMM_WORLD))
        {
          if (us_log)
            {
              us_log->log (QString ("%1: MPI PM_CALC_FITNESS Send failed "
...
bool
US_PM::ga (pm_ga_individual &best_individual)
{
  if (us_log)
    {
      us_log->log (ga_info ());
      ...
      for (unsigned int g = 0; g < ga_p.generations; ++g)
        {
              ...
              for (int pos = 0; pos < (int)ga_points.size (); ++pos)
                {
                  random_normal_sd[pos]
                      = ((double)ga_points[pos])
                        / (6e0 * log (2e0 + 2e0 * g) / log (2e0));
              ...
              if (us_log)
                {
                  ...
                  us_log->log (US_Vector::qs_vector ("random_normal_sd",
                                                     random_normal_sd));
                }
            }
          ...
          if (us_log)
            {
              us_log->log (
                  QString ("ga_: gen %1 best individual fitness %2 beads %3\n")
                      .arg (g)
...

          {
            ...
            if (us_log)
              {
                us_log->log (
                    US_Vector::qs_vector ("ga_fitness: v", ga_pop.front ().v));
                us_log->log (
                    US_Vector::qs_vector ("ga_fitness: params", ga_params));
              }
          }
              ...
              if (drand48 () < ga_p.mutate)
                {
                  ...
                  unsigned int pos
                      = (unsigned int)(drand48 () * ga_fparams_size);
                  individual.v[pos]
                      = int (((double)ga_points[pos] * drand48 ()));
              ...
              if (has_been_duplicated.count (pos))
                {
                  for (unsigned int j = 0; j < ga_fparams_size; j++)
                    {
                      individual.v[j]
                          = int (((double)ga_points[j] * drand48 ()));
                    }
                  // ga_fitness( individual );
                  individual.fitness_computed = false;
                  ga_pop.push_back (individual);
          ...
          if (us_log)
            {
              us_log->log (QString ("summary counts:\n"
                               ...
                               .arg (duplicate_count)
                               .arg (random_count)
                               .arg (elitism_count + mutate_count
                                     + sa_mutate_count + crossover_count
  ...
  if (us_log)
    {
      us_log->log (
          QString ("ga_: gen final best individual fitness %1 beads %2\n")
              .arg (ga_pop.front ().fitness)
              .arg (ga_pop.front ().model.size ()));
    }
  ...
  for (int i = 0; i < (int)ga_low_fparams.size (); i++)
    {
      if (ga_low_fparams[i] > ga_high_fparams[i])
        {
          // US_Vector::printvector2( QString( "low fparams > high fparams @
          // %1" ).arg( i ), ga_low_fparams, ga_high_fparams );
...
bool
US_PM::ga_compute_delta (unsigned int points_max)
{
  if (!ga_state_ok ())
    {
      return false;
  ...
  if (us_log)
    {
      us_log->log (
          QString ("ga_compute_delta points max %1").arg (points_max));
    }
  ...
  for (int i = 0; i < (int)ga_low_fparams.size (); i++)
    {
      if (ga_low_fparams[i] == ga_high_fparams[i])
        ...
        {
          ga_delta[i] = (ga_high_fparams[i] - ga_low_fparams[i])
                        / (double)(points_max - 1);
          if (us_log)
            {
          ...
          if (ga_delta[i] < best_delta_min)
            {
              if (us_log)
                {
                  us_log->log (QString ("ga_compute_delta: ga_delta[ %1 ] < "
                                        "best_delta_min %2")
                                   .arg (i)
                                   .arg (best_delta_min));
                }
              ...
              ga_points[i]
                  = (unsigned int)(0.5
                                   + (ga_high_fparams[i] - ga_low_fparams[i])
                                         / best_delta_min);
              if (ga_points[i] == 0)
                {
                  ga_delta[i] = 0e0;
  ...
  if (us_log)
    {
      ...
      us_log->log (
          US_Vector::qs_vector ("ga_compute_delta: deltas", ga_delta));
      us_log->log (
          US_Vector::qs_vector ("ga_compute_delta: points", ga_points));
    }

  if (total_pts <= ga_p.population
                       * (ga_p.generations > 2 ? ga_p.generations / 2
                                               : ga_p.generations))
    {
      if (us_log)
        {
          us_log->log ("ga_compute_delta: points <= population * generations "
                       "/ 2, switching to full grid search\n");
        }
...
bool
US_PM::ga_refine_limits (unsigned int top_count, bool needs_sort_unique,
                         unsigned int extend_deltas)
{
  ...
  if (us_log)
    {
      us_log->log (
          US_Vector::qs_vector2 ("ga_refine_limits: limits after clipping",
                                 ga_low_fparams, ga_high_fparams));
    }
...
bool
US_PM::ga_delta_to_fparams (vector<int> &delta, vector<double> &fparams)
{
  for (unsigned int i = 0; i < ga_fparams_size; ++i)
    {
...
bool
US_PM::ga_run (vector<int> &types, pm_ga_individual &best_individual,
               ...
               unsigned int points)
{
  if (us_log)
    {
      us_log->log ("Warning: ga_run() without limits can create "
                   "unnecessarrily huge models (i.e. slow!)\n");
    }
...
void
US_PM::ga_set_params (unsigned int ga_population, unsigned int ga_generations,
                      double ga_mutate, double ga_sa_mutate,
                      double ga_crossover, unsigned int ga_elitism,
...
void
US_PM::ga_set_params (map<QString, QString> control_parameters)
{
  ga_set_params ();
...
void
US_PM::random_normal (double mean1, double sd1, double mean2, double sd2,
                      double &x, double &y)
{
  double s;
  do
...
bool
US_PM::ga_params_to_individual (vector<double> &params,
                                pm_ga_individual &individual)
{
  vector<int> types;
...
bool
US_PM::ga_fparams_to_individual (vector<double> &fparams,
                                 pm_ga_individual &individual)
{
  if (!ga_delta_ok)
    {
  ...
  if (ga_fparams_size != fparams.size ())
    {
      error_msg = QString ("Error: ga_fparams_to_individual: preset "
                           "ga_fparams_size %1 is not equal fparams.size() %2")
                      .arg (ga_fparams_size)
                      .arg (fparams.size ());
      return false;
    }
