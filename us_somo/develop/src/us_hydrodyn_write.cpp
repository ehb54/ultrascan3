...
void US_Hydrodyn::write_bead_tsv(QString fname, vector<PDB_atom> *model) {
  ...
  fprintf(f,
          ...
          "position_coordinate~cog position~use position~sizeof(parents)~beads "
          "referenced\n");
  ...
  for (unsigned int i = 0; i < model->size(); i++) {
    if ((*model)[i].active) {
      ...
      for (unsigned int j = 0; j < (*model)[i].all_beads.size(); j++) {
        beads_referenced += QString("%1-%2-%3-%4;")
...
void US_Hydrodyn::write_bead_asa(QString fname, vector<PDB_atom> *model) {
  ...
  fprintf(f,
          "\n\n\n"
          "\tTOTAL ASA OF THE MOLECULE    = %.0f\t[A^2] (Threshold used: %.1f "
          "A^2]\n"
...
void US_Hydrodyn::write_bead_model(
    QString fname, vector<PDB_atom> *model,
    int bead_model_output = US_HYDRODYN_OUTPUT_SOMO, QString extra_text) {
...
void US_Hydrodyn::write_bead_model(QString fname, vector<PDB_atom> *model,
  ...
  if (fbeams) {
    fprintf(fbeams, "%d\t-2.000000\t%s.rmc\t%.3f\n", beads,
            QFileInfo(fname).fileName().toLatin1().data(), results.vbar);
  }
  ...
  if (fgrpy) {
    ...
    fprintf(fgrpy,
            ...
            // ,hydro.pH
            ,
            use_solvent_visc() * 0.01, tot_mw, results.vbar, use_solvent_dens(),
            hydro.unit + 2, grpy_used_beads_count(use_model)
  ...
  for (unsigned int i = 0; i < use_model.size(); i++) {
    if (use_model[i]->active) {
      ...
      if (fsomo) {
        fprintf(
            fsomo, fstring_somo.toLatin1().data(),
            ...
            use_model[i]->saxs_excl_vol, use_model[i]->bead_hydration);
      }
      if (fbeams) {
        fprintf(fbeams, fstring_beams.toLatin1().data(),
                ...
                use_model[i]->bead_coordinate.axis[2]);
        fprintf(
            frmc, fstring_rmc.toLatin1().data(),
            use_model[i]->bead_computed_radius,
            use_model[i]->bead_ref_mw + use_model[i]->bead_ref_ionized_mw_delta,
            get_color(use_model[i]));
        fprintf(
            frmc1, fstring_rmc1.toLatin1().data(),
            use_model[i]->bead_computed_radius,
            use_model[i]->bead_ref_mw + use_model[i]->bead_ref_ionized_mw_delta,
            get_color(use_model[i]), residues.toLatin1().data());
      }
  ...
  if (fsomo) {
    ...
    fputs((last_abb_msgs.isNull() ? "" : last_abb_msgs.toLatin1().data()),
          fsomo);
    ...
    if (use_model[i]->active) {
      ...
      if (fcorr) {
        fprintf(
            ...
            use_model[i]->bead_ref_mw + use_model[i]->bead_ref_ionized_mw_delta,
            use_model[i]->bead_ref_volume, use_model[i]->bead_recheck_asa,
            use_model[i]->chain, use_model[i]->serial,
            use_model[i]->exposed_code, use_model[i]->bead_color);
      }
    }
  }
  fclose(fcorr);
...
void US_Hydrodyn::save_pdb_csv(csv &csv1) {
  ...
  QString filename = QFileDialog::getSaveFileName(
      this, us_tr("Choose a filename to save the pdc"), use_dir, "*.pdc *.pdc");
...
void US_Hydrodyn::write_dati1_supc_bead_model(QString filename, int size,
                                              void *dt) {
...
void US_Hydrodyn::write_dati1_pat_bead_model(QString filename, int size,
                                             void *dtp, void *dts) {
  filename = "/tmp/" + filename;
...
bool US_Hydrodyn::write_pdb_from_model(const PDB_model &model, QString &errors,
                                       QString &writtenname,
  ...
  for (unsigned int j = 0; j < (unsigned int)model.molecule.size(); j++) {
    for (unsigned int k = 0; k < (unsigned int)model.molecule[j].atom.size();
         k++) {
