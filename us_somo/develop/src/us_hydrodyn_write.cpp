...
void
US_Hydrodyn::write_bead_spt (QString fname, vector<PDB_atom> *model,
                             bool movie_frame, float scale,
...
#define COLORMAP_SIZE 16
  ...
  FILE *fspt
      = us_fopen (QString ("%1.spt").arg (fname).toLatin1 ().data (), "w");
  FILE *fbms
      = us_fopen (QString ("%1.bms").arg (fname).toLatin1 ().data (), "w");
...
#define MAX_RADIUS 10.0
          ...
          if ((*model)[i].bead_color >= COLORMAP_SIZE)
            {
              printf ("ERROR: bead color for bead %u is to large %u\n",
                      (*model)[i].serial, get_color (&(*model)[i]));
              fflush (stdout);
            }
          fprintf (fbms, "Pb %.2f %.2f %.2f\n",
  ...
  if (movie_frame)
    {
      last_spt_text += QString ("").sprintf ("save ppm %s.ppm\n"
                                             "exit\n",
                                             fname.toLatin1 ().data ());
      movie_text.push_back (fname);
    }
  // fprintf(fspt, ( last_spt_text.isNull() ? "" :
  // last_spt_text.toLatin1().data() ) );
  fputs ((last_spt_text.isNull () ? "" : last_spt_text.toLatin1 ().data ()),
         fspt);
  fclose (fspt);
  fclose (fbms);
}
...
void
US_Hydrodyn::write_bead_tsv (QString fname, vector<PDB_atom> *model)
{
  ...
  fprintf (f, "name~residue~chainID~"
              ...
              "position_coordinate~cog position~use "
              "position~sizeof(parents)~beads referenced\n");
      ...
      if ((*model)[i].active)
        {
          ...
          fprintf (
              ...
              (*model)[i].coordinate.axis[0], (*model)[i].coordinate.axis[1],
              (*model)[i].coordinate.axis[2], (*model)[i].active ? "Y" : "N",
              (*model)[i].radius, (*model)[i].asa,
              (*model)[i].mw + (*model)[i].ionized_mw_delta,

              (*model)[i].bead_number, (*model)[i].chain, (*model)[i].serial,
              (*model)[i].is_bead ? "Y" : "N", (*model)[i].bead_asa,
              ...
              (*model)[i].active ? ((*model)[i].bead_positioner ? "Y" : "N")
                                 : "Inactive",
              (*model)[i].placing_method, (*model)[i].bead_hydration,
              (*model)[i].bead_color,
...
void
US_Hydrodyn::write_bead_asa (QString fname, vector<PDB_atom> *model)
{
  FILE *f = us_fopen (fname.toLatin1 ().data (), "w");
      ...
      if ((*model)[i].active)
        {
          ...
          total_mass += (*model)[i].bead_ref_mw
                        + (*model)[i].bead_ref_ionized_mw_delta;
  ...
  if (last_residue != "")
    {
      fprintf (f, " [ %-6d %s ]\t%.0f\t%.0f\t%.2f\n", seqno,
               last_residue.toLatin1 ().data (), residue_asa, residue_ref_asa,
               100.0 * residue_asa / residue_ref_asa);
    }

  fprintf (f,
           "\n\n\n"
           "\tTOTAL ASA OF THE MOLECULE    = %.0f\t[A^2] (Threshold used: "
           "%.1f A^2]\n"
           ...
           total_asa, asa.threshold,
           // total_vol,
           results.asa_rg_pos, results.asa_rg_neg, total_mass,
           last_molecular_cog.axis[0], last_molecular_cog.axis[1],
           last_molecular_cog.axis[2]);

  fclose (f);
  editor_msg ("dark blue",
              QString ("").sprintf ("Anhydrous volume %.2f A^3", total_vol));
}

void
US_Hydrodyn::write_bead_model (QString fname, vector<PDB_atom> *model,
                               ...
                               QString extra_text)
{
  ...
  if (bead_output.output & US_HYDRODYN_OUTPUT_GRPY)
    {
      fgrpy = us_fopen (QString ("%1.grpy").arg (fname).toLatin1 ().data (),
                        "w");
      if (last_bead_model.isEmpty ())
        {
  ...
  if (fgrpy)
    {
      ...
      fprintf (fgrpy,
               ...
               // ,hydro.pH
               ,
               use_solvent_visc () * 0.01, tot_mw, results.vbar,
               use_solvent_dens (), hydro.unit + 2,
      ...
      if (use_model[i]->active)
        {
          ...
          summary_mw += use_model[i]->bead_ref_mw
                        + use_model[i]->bead_ref_ionized_mw_delta;
          // QTextStream( stdout ) << "bead ref " << i << " mw " <<
          // use_model[i]->bead_ref_mw +
          // use_model[i]->bead_ref_ionized_mw_delta << Qt::endl;
          ...
          if (fsomo)
            {
              fprintf (fsomo, fstring_somo.toLatin1 ().data (),
                       ...
                       use_model[i]->bead_ref_mw
                           + use_model[i]->bead_ref_ionized_mw_delta,
                       get_color (use_model[i]), residues.toLatin1 ().data (),
                       use_model[i]->bead_recheck_asa, use_model[i]->num_elect,
          ...
          if (fgrpy)
            {
              ...
              if (hydro.grpy_bead_inclusion || color != 6)
                {
                  fprintf (fgrpy, fstring_hydro.toLatin1 ().data (),
                           use_model[i]->bead_coordinate.axis[0],
                           use_model[i]->bead_coordinate.axis[1],
                           use_model[i]->bead_coordinate.axis[2],
                           use_model[i]->bead_computed_radius);
  ...
  if (fsomo)
    {
      fprintf (
          ...
          "scattering_factor, hydration waters\n"
          ...
          -hydro.unit);
      // fprintf(fsomo, ( options_log.isNull() ? "" :
      // options_log.toLatin1().data() ) ); fprintf(fsomo, (
      // last_abb_msgs.isNull() ? "" : last_abb_msgs.toLatin1().data() ) );
      fputs ((options_log.isNull () ? "" : options_log.toLatin1 ().data ()),
             fsomo);
      fputs (
          (last_abb_msgs.isNull () ? "" : last_abb_msgs.toLatin1 ().data ()),
          fsomo);
...
void
US_Hydrodyn::write_corr (QString fname, vector<PDB_atom> *model)
{
  FILE *fcorr = (FILE *)0;
          ...
          if (fcorr)
            {
              fprintf (fcorr,
                       ...
                       use_model[i]->bead_recheck_asa, use_model[i]->chain,
                       use_model[i]->serial, use_model[i]->exposed_code,
...
bool
US_Hydrodyn::read_corr (QString fname, vector<PDB_atom> *model)
{
  QFile f (fname);
  ...
  if (f.open (QIODevice::ReadOnly))
    {
      editor->append (
          QString ("Reading correspondence file %1\n").arg (fname));
      QTextStream ts (&f);
      ...
      if (ts.atEnd () && i == model->size ())
          ...
          for (i = 0; i < model->size (); i++)
            {
              ...
              (*model)[i].bead_ref_ionized_mw_delta
                  = new_model[i].bead_ref_ionized_mw_delta;
        ...
        {
          editor->append (
              QString ("Correspondence file didn't match %1\n").arg (fname));
...
void
US_Hydrodyn::save_pdb_csv (csv &csv1)
{

  QString use_dir
      = USglobal->config_list.root_dir + SLASH + "somo" + SLASH + "structures";
  QString filename = QFileDialog::getSaveFileName (
      this, us_tr ("Choose a filename to save the pdc"), use_dir,
      "*.pdc *.pdc");
...
void
US_Hydrodyn::write_dati1_supc_bead_model (QString filename, int size, void *dt)
{
...
void
US_Hydrodyn::write_dati1_pat_bead_model (QString filename, int size, void *dtp,
                                         void *dts)
{
  filename = "/tmp/" + filename;
...
bool
US_Hydrodyn::write_pdb_from_model (const PDB_model &model, QString &errors,
                                   QString &writtenname,
                                   ...
                                   const QString &filename)
{
  ...
  QString pdb_header = QString ("HEADER  US-SOMO %1 File %2\n")
                           .arg (headernote)
                           .arg (QFileInfo (fname).fileName ());
  ...
  for (unsigned int j = 0; j < (unsigned int)model.molecule.size (); j++)
    {
      for (unsigned int k = 0;
           k < (unsigned int)model.molecule[j].atom.size (); k++)
        {
