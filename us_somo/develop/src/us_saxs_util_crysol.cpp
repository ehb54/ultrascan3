...
US_Saxs_Util::run_crysol ()
{
          ...
          while (QFile::exists (destname))
            {
              ...
              if (copy_str.length () > 5)
                {
                  errormsg = QString ("Error: crysol: too many attempts to "
                                      "create unique 6 character name for %1")
  ...
  if (control_parameters.count ("crysollog")
      && control_parameters.count ("crysolbasename"))
    {
      ...
      if (!fi.exists ())
        {
          errormsg = QString ("Error: crysol: the expected output file %1 "
                              "does not exist")
