...
nsa_sga_individual
nsa_sga_individual::random (unsigned int size /* in char's */)
{
...
vector<sgp_sphere>
nsa_sga_individual::sgp_spheres ()
{
          ...
          if ((v[i] >> j) & 1)
            {
              ...
              // cout << QString( "sgp_spheres: bit on i %1 j %2 pos %3 dim
...
QString
nsa_sga_individual::qs_bead_model ()
{
...
double
US_Saxs_Util::nsa_sga_fitness (nsa_sga_individual individual)
{
...
bool
US_Saxs_Util::nsa_sga (double &nrmsd)
{
           ...
           i < control_parameters["nsapopulation"].toUInt (); i++)
        {
          ...
          if (control_parameters.count ("nsamutate")
              && drand48 () < control_parameters["nsamutate"].toDouble ())
            {
              ...
              unsigned int pos
                  = (unsigned int)(individual.v.size () * drand48 ());
