...
#include "../include/us_hydrodyn_dad_conc_csv_frames.h"
...
DAD_Lambdas::summary ()
{
  return lambdas.size () ? QString (us_tr ("%1 UV-Vis " + UNICODE_LAMBDA_QS
                                           + "s loaded, range [%2:%3]"))
                               .arg (lambdas.size ())
                               .arg (lambdas.front ())
                               .arg (lambdas.back ())
                         : QString (us_tr ("No UV-Vis " + UNICODE_LAMBDA_QS
                                           + "s currently loaded"));
}
...
DAD_Lambdas::summary_rich ()
{
  ...
  qsl << QString (us_tr ("<b>UV-Vis " + UNICODE_LAMBDA_QS
                         + "s</b> loaded from %1<hr>"))
             .arg (loaded_filename)
      << QString (
             us_tr ("%1 " + UNICODE_LAMBDA_QS + "s present, range [%2:%3]"))
             .arg (lambdas.size ())
             .arg (lambdas.front ())
             .arg (lambdas.back ());
...
bool
DAD_Lambdas::load (const QString &filename, const QStringList &filelines,
                   QString &errormsg)
{
  if (!filelines.size ())
    {
    ...
    for (auto const &line : use_filelines)
      {
        if (line.split (QRegularExpression ("[\t ,]")).size () > 1)
          {
            errormsg = QString (us_tr ("multiple tokens found %1 in %2"))
                           .arg (line)
                           .arg (filename);
            return false;
          }
        ...
        double lambda = line.toDouble (&isNumeric);
        if (!isNumeric || lambda <= 0)
          {
            errormsg = QString (us_tr ("invalid lambda found %1 (%2) in %3"))
                           .arg (lambda)
                           .arg (line)
                           .arg (filename);
            return false;
        ...
        if (used.count (lambda))
          {
            errormsg = QString (us_tr ("duplicate value found %1 (%2) in %3"))
                           .arg (lambda)
                           .arg (line)
                           .arg (filename);
            return false;
...
bool
DAD_Lambdas::load (const QString &filename, QString &errormsg)
{
  QString contents;
  ...
  QStringList filelines = contents.split ("\n");
  return load (filename, filelines, errormsg);
}
...
QString
DAD_Angle::list_rich (double lambda, double n)
{
  if (!lambda || !n)
    {
...
bool
DAD_Angle::populate (const QStringList &qsl)
{
...
DAD_Angles::list ()
{
  ...
  for (auto it = dad_angle.begin (); it != dad_angle.end (); ++it)
    {
...
void
DAD_Angles::build_q_to_ri (double lambda, double n)
{
  ...
  for (auto &ma : dad_angle)
    {
      if (ma.second.has_angle_ri_corr)
        {
          q_to_ri[QString ("%1")
                      .arg (n * 4 * M_PI
                            * sin (ma.second.angle_ri_corr * M_PI / 360)
                            / (lambda * 10))
                      .toDouble ()]
              = ma.second.angle_ri_corr;
        }
    }
  // qDebug() << "q_to_ri after setup";
  // for ( auto const & it : q_to_ri ) {
  //    qDebug() << QString( "q_to_ri q %1 -> ri %2" ).arg( it.first ).arg(
  //    it.second );
  // }
}

QString
DAD_Angles::list_rich (double lambda, double n)
{
  QStringList qsl;
  if (!dad_angle.size ())
    {
  ...
  qsl << QString (us_tr ("<b>UV-Vis Angles</b> loaded from %1<hr>"))
             .arg (loaded_filename);
  if (!lambda || !n)
    {
...
bool
DAD_Angles::populate (const QString &filename, const QStringList &qsl)
{
      ...
      if (qsl[i].contains (QRegularExpression ("^[0-9]+\\s")))
        {
          ...
          QStringList qsl_dad_angle = qsu.split (QRegularExpression ("\\s+"));
          if (qsl_dad_angle.size () >= 5)
            {
              int index = qsl_dad_angle[0].toInt ();
              qsl_dad_angle.pop_front ();
              if (!dad_angle[index].populate (qsl_dad_angle))
                {
                  dad_angle.erase (index);
                }
            }
        }
    }
...
bool
DAD_Angles::load (const QString &filename, const QStringList &csvlines,
                  QString &errormsg)
{
  if (!csvlines.size ())
    {
  ...
  if (!csvlines[0].contains (
          QRegularExpression ("^\\s*\"\\s*US-SOMO UV-Vis Angles")))
    {
  ...
  while (data.size ()
         && data.front ().contains (QRegularExpression ("^\\s*\\d+,")))
    {
      ...
      dad_angle[index].angle = qsl[1].toDouble ();

      dad_angle[index].angle_ri_corr
          = qsl[2] == "\"n/a\"" ? 0 : qsl[2].toDouble ();
      dad_angle[index].has_angle_ri_corr = qsl[2] == "\"n/a\"" ? false : true;

      dad_angle[index].gain = qsl[3] == "\"n/a\"" ? 0 : qsl[3].toDouble ();
      dad_angle[index].has_gain = qsl[3] == "\"n/a\"" ? false : true;

      dad_angle[index].norm_coef
          = qsl[4] == "\"n/a\"" ? 0 : qsl[4].toDouble ();
      dad_angle[index].has_norm_coef = qsl[4] == "\"n/a\"" ? false : true;
    }
...
bool
DAD_Angles::load (const QString &filename, QString &errormsg)
{
  QString contents;
  if (!US_File_Util::getcontents (filename, contents, errormsg))
    {
...
bool
DAD_Angles::save (const QString &filename, QString &errormsg)
{
  QStringList qsl;

  qsl << "\"US-SOMO UV-Vis Angles\"\n"
...
US_Hydrodyn_Dad::US_Hydrodyn_Dad (csv csv1, void *us_hydrodyn, QWidget *p)
    : QFrame (p)
{
  this->csv1 = csv1;
  ...
  gaussian_type = GAUSS;
  ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_gaussian_type"]
      = QString ("%1").arg (gaussian_type);
#endif

  // fix up possible unconfigured or bad configuration

  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_line_width"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_line_width"] = "1";
    }
  use_line_width
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_line_width"].toUInt ();

  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_bl_linear"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_linear"] = "false";
    }
  ...
  //   }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_bl_save"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_save"] = "false";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_bl_smooth")
      || ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_smooth"] == "0"
      || ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_smooth"] == "")
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_smooth"] = "3";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_bl_reps"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_reps"] = "5";
    }
  if (((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_linear"] == "true"
      && ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_integral"] == "true")
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_linear"] = "false";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_zi_window"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_zi_window"] = "25";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_discard_it_sd_mult"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_discard_it_sd_mult"] = "2";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)
           ->gparams.count ("dad_cb_discard_it_sd_mult"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_discard_it_sd_mult"]
          = "true";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_cb_guinier_qrgmax"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_guinier_qrgmax"] = "true";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_guinier_qrgmax"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_guinier_qrgmax"] = "1.3";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_dist_max"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_dist_max"] = "50.0";
    }
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_lock_min_retry_mult")
      || !started_in_expert_mode)
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_lock_min_retry_mult"] = "2";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_maxfpk_restart")
      || !started_in_expert_mode)
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_maxfpk_restart"] = "false";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_maxfpk_restart_tries")
      || !started_in_expert_mode)
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_maxfpk_restart_tries"] = "3";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_maxfpk_restart_pct")
      || !started_in_expert_mode)
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_maxfpk_restart_pct"] = "10";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_cormap_maxq"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cormap_maxq"] = "0.05";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_cormap_alpha"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cormap_alpha"] = "0.01";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_bl_i_power")
      || !started_in_expert_mode)
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_i_power"] = "1";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)
           ->gparams.count ("dad_cb_makeiq_cutmax_pct"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_makeiq_cutmax_pct"]
          = "true";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_makeiq_cutmax_pct"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_makeiq_cutmax_pct"] = "1";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_cb_makeiq_avg_peaks"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_makeiq_avg_peaks"]
          = "false";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_makeiq_avg_peaks"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_makeiq_avg_peaks"] = "5";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_cb_gg_smooth"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_gg_smooth"] = "false";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_gg_smooth"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_gg_smooth"] = "3";
    }

  baseline_ready_to_apply = false;

  gaussian_type
      = ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_gaussian_type")
            ? (gaussian_types)((US_Hydrodyn *)us_hydrodyn)
                  ->gparams["dad_gaussian_type"]
                  .toInt ()
            : GAUSS;
  ...
  ((US_Hydrodyn *)us_hydrodyn)->dad_widget = true;
  ...
  //    pb_set_dad->setMaximumWidth ( pb_select_all->width() + 10 );
  ...
  QString dctr_file = USglobal->config_list.root_dir + QDir::separator ()
                      + "etc" + QDir::separator ()
                      + "somo_uv_vis_default_dctr.dat";
  if (QFile (dctr_file).exists ())
    {
      load_file (dctr_file);
    }

  QString dad_params_file = USglobal->config_list.root_dir + QDir::separator ()
                            + "etc" + QDir::separator ()
                            + "somo_uv_vis_default_uv_vis_param.dat";
  // set defaults always
...
{
  ((US_Hydrodyn *)us_hydrodyn)->dad_widget = false;
  delete usu;
}

void
US_Hydrodyn_Dad::push_back_color_if_ok (QColor bg, QColor set)
{
...
void
US_Hydrodyn_Dad::cancel ()
{
  close ();
}

void
US_Hydrodyn_Dad::help ()
{
  US_Help *online_help;
  online_help = new US_Help (this);
  online_help->show_help ("manual/somo/somo_uv_vis.html");
}

void
US_Hydrodyn_Dad::closeEvent (QCloseEvent *e)
{
  QMessageBox mb (
      this->windowTitle (),
  ...
  ((US_Hydrodyn *)us_hydrodyn)->dad_widget = false;
  ((US_Hydrodyn *)us_hydrodyn)->last_dad_csv = current_csv ();
  if (conc_widget)
    {
      conc_window->close ();
    }
...
void
US_Hydrodyn_Dad::editor_msg (QString color, QString msg)
{
...
void
US_Hydrodyn_Dad::editor_msg_qc (QColor qcolor, QString msg)
{
  QColor save_color = editor->textColor ();
...
bool
US_Hydrodyn_Dad::activate_saxs_window ()
{
  if (!*saxs_widget)
    {
...
void
US_Hydrodyn_Dad::remove_files (set<QString> &fileset, bool replot)
{
  QStringList files;
...
void
US_Hydrodyn_Dad::clear_files (QStringList files, bool quiet)
{
      ...
      if (selected_map.count (lb_files->item (i)->text ()))
        {
          ...
          //          if ( lbl_dad->text() == lb_files->item( i )->text() )
          //          {
          //             lbl_dad->setText( "" );
          //          }
...
class dad_sortable_qstring
{
public:
  double x;
  QString name;
  bool
  operator< (const dad_sortable_qstring &objIn) const
  {
    return x < objIn.x;
  }
};
...
void
US_Hydrodyn_Dad::add_dir ()
{
  add_files (false, true);
}

void
US_Hydrodyn_Dad::add_files (bool load_conc, bool from_dir)
{
  disable_all ();
      ...
      if (reorder)
        {
          ...
          filenames.clear ();
          for (list<dad_sortable_qstring>::iterator it = svals.begin ();
               it != svals.end (); ++it)
            {
              filenames << it->name;
...

  {
    ...
    for (auto const &name : add_filenames)
      {
        if (f_ref_index.count (name) && f_ref_index[name] != dad_param_n)
          {
    ...
    if (ref_indices.size () == 1)
      {
        QMessageBox::warning (
            this, windowTitle () + us_tr (" : Add files"),
            QString (us_tr ("Loaded files contain a different solvent "
                            "refractive index (%1)\n"
                            "than set in the Options->UV-Vis "
                            "parameters->Solvent refractive index (%2)\n"
                            "You should remove all files, adjust the "
                            "parameter value and reload the files"))
    ...
    if (ref_indices.size () > 1)
      {
        ...
        QMessageBox::warning (
            this, windowTitle () + us_tr (" : Add files"),
            QString (us_tr ("Loaded files contain multiple different solvent "
                            "refractive index (%1)\n"
                            "which do not match the Options->UV-Vis "
                            "parameters->Solvent refractive index (%2)\n"
...
void
US_Hydrodyn_Dad::add_files (QStringList filenames)
{
  map<QString, bool> existing_items;
  for (int i = 0; i < lb_files->count (); i++)
    {
...
void
US_Hydrodyn_Dad::select_all_created ()
{
  bool all_selected = true;
...
bool
US_Hydrodyn_Dad::load_file (QString filename, bool load_conc)
{
  errormsg = "";
  QFile f (filename);
  if (!f.exists ())
  ...
  if (dad_angles.populate (filename, qsl_nb))
    {
      editor_msg ("black",
                  QString ("%1 UV-Vis angles loaded\n").arg (filename));
      editor_msg ("black", dad_angles.list ());
      lbl_dad_lambdas_data->setText (
          dad_angles.list_rich (dad_param_lambda, dad_param_n));
      return false;
    }
  ...
  if (ext == "dat" && qv[0].contains (" UV-Vis parameter file"))
    {
      ...
      QRegExp rx_dad_param_g_extinction_coef (
          "^# __uv_vis_param_g_extinction_coef: (\\S+)\\s*$");
      QRegExp rx_dad_param_g_conc ("^# __uv_vis_param_g_conc: (\\S+)\\s*$");
      QRegExp rx_dad_param_DLS_detector (
          "^# __uv_vis_param_DLS_detector: (\\S+)\\s*$");
      ...
      for (int i = 1; i < (int)qv.size (); i++)
        {
          ...
          if (rx_dad_param_g_extinction_coef.indexIn (qv[i]) != -1)
            {
              dad_param_g_extinction_coef
                  = rx_dad_param_g_extinction_coef.cap (1).toDouble ();
              continue;
            }
          ...
          if (rx_dad_param_DLS_detector.indexIn (qv[i]) != -1)
            {
              dad_param_DLS_detector
                  = rx_dad_param_DLS_detector.cap (1).toInt ();
              continue;
            }
  ...
  if (ext == "csv" && load_conc)
    {
      ...
      US_Hydrodyn_Dad_Conc_Csv_Frames *conc_csv_frames_instance
          = new US_Hydrodyn_Dad_Conc_Csv_Frames (us_hydrodyn, &conc_csv_params,
                                                 this);
  ...
  if (ext == "txt")
    {
      ...
      if (dad_lambdas.load (filename, qsl, errormsg))
        {
          editor_msg ("black",
                      QString ("%1 UV-Vis lambdas loaded\n").arg (filename));
      ...
      // try absorbtion
      {
        if (dad_load (filename, qsl, errormsg))
          {
            editor_msg ("black",
                        QString (us_tr ("%1 - UV-Vis absorption data\n"))
                            .arg (filename));
            return false;
          }
        if (errormsg != "not a valid dad file")
          {
            QMessageBox::warning (
                this, "US-SOMO UV-Vis load Error",
  ...
  if (ext == "csv")
    {
      // disabled MALS angles code
      // {
      //    QString errormsg;
      //    if ( dad_angles.load( filename, qsl, errormsg ) ) {
      //       editor_msg( "black", QString( "%1 UV-Vis angles loaded\n" ).arg(
      //       filename ) ); editor_msg( "black", dad_angles.list() );
      //       lbl_dad_angles_data->setText( dad_angles.list_rich(
      //       dad_param_lambda, dad_param_n ) ); return false;
      //    }
  ...
  // ad-hoc for soleil dad time/uv/conc data
...
void
US_Hydrodyn_Dad::set_conc_file (QString file)
{
  if (file.isEmpty ())
    {
      for (int i = 0; i < lb_files->count (); i++)
...
void
US_Hydrodyn_Dad::set_dad ()
{
  for (int i = 0; i < lb_files->count (); i++)
    {
      if (lb_files->item (i)->isSelected ())
        {
          lbl_dad->setText (lb_files->item (i)->text ());
        }
    }
  update_csv_conc ();
  if (conc_widget)
...
void
US_Hydrodyn_Dad::update_created_files ()
{
  if (!disable_updates)
    {
      update_enables ();
...
US_Hydrodyn_Dad::avg ()
{
  ...
  //       if ( lb_files->item( i )->isSelected()
  //            //            &&
  //            //            lb_files->item( i )->text() != lbl_dad->text() &&
...
QString
US_Hydrodyn_Dad::qstring_common_head (QStringList qsl, bool strip_digits)
{
  if (!qsl.size ())
    {
      return "";
...
QString
US_Hydrodyn_Dad::qstring_common_tail (QStringList qsl, bool strip_digits)
{
  if (!qsl.size ())
    {
      return "";
...
QString
US_Hydrodyn_Dad::qstring_common_head (QString s1, QString s2)
{
...
QString
US_Hydrodyn_Dad::qstring_common_tail (QString s1, QString s2)
{
  int min_len = (int)s1.length ();
  if (min_len > (int)s2.length ())
    {
...
void
US_Hydrodyn_Dad::save_created_csv ()
{
  QStringList created_not_saved_list;
...
bool
US_Hydrodyn_Dad::save_files_csv (QStringList files)
{
  if (!files.size ())
    {
      editor_msg ("red",
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_csv_transposed")
...
bool
US_Hydrodyn_Dad::save_files (QStringList files)
{
...
bool
US_Hydrodyn_Dad::save_file (QString file, bool &cancel, bool &overwrite_all)
{
  if (!f_qs.count (file))
    {
      editor_msg ("red",
...

  {
    ...
    // ts << QString( windowTitle() + us_tr( " %1data:
    ...
    //    .arg( dad_angles.loaded_filename.isEmpty() ? QString("") : QString( "
    //    Angles loaded from:%1" ).arg( dad_angles.loaded_filename ) ) .arg(
    //    f_ri_corr.count( file ) ? QString( " RI-Corr scatt. angle:%1"  ).arg(
    //    f_ri_corr[ file ] ) : QString( "" ) ) .arg( f_ri_corrs.count( file )
    //    ? QString( " RI-Corr scatt. angles:%1"  ).arg( f_ri_corrs[ file ] ) :
    //    QString( "" ) ) .arg( f_ref_index.count( file ) ? QString( " Solvent
    //    refractive index:%1"  ).arg( f_ref_index[ file ] ) : QString( "" ) )
    ...
    ts << QString (windowTitle ()
              ...
              .arg (f_solvent_e_dens.count (file)
                        ...
                        : QString (""))
              .arg (f_header.count (file) ? f_header[file] : QString (""))
              .arg (dad_angles.loaded_filename.isEmpty ()
                        ? QString ("")
                        : QString (" Angles loaded from:%1")
                              .arg (dad_angles.loaded_filename))
...
void
US_Hydrodyn_Dad::update_csv_conc ()
{
  map<QString, bool> skip;
  //    if ( !lbl_dad->text().isEmpty() )
  //    {
  //       skip[ lbl_dad->text() ] = true;
  //    }
...
US_Hydrodyn_Dad::conc ()
{
  ...
  if (conc_widget)
    ...
    {
      conc_window = new US_Hydrodyn_Dad_Conc (csv_conc, this);
      conc_window->show ();
    }
}

map<QString, double>
US_Hydrodyn_Dad::current_concs (bool quiet)
{
...
vector<double>
US_Hydrodyn_Dad::union_q (QStringList files)
{
...
bool
US_Hydrodyn_Dad::get_peak (QString file, double &peak, bool full)
{
  double pos;
...
bool
US_Hydrodyn_Dad::get_peak (QString file, double &peak, double &pos, bool full)
{
  if (!f_Is.count (file))
    {
      editor_msg ("red", QString (us_tr ("Internal error: get_peak requested "
...
void
US_Hydrodyn_Dad::smooth (QStringList files)
{
  bool ok;
...
void
US_Hydrodyn_Dad::create_i_of_t (QStringList files)
{
  // find common q
  QString head = qstring_common_head (files, true);
...
bool
US_Hydrodyn_Dad::check_zi_window (QStringList &files,
                                  const QString &extra_text)
{
  int window = ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_zi_window"].toInt ();
...
void
US_Hydrodyn_Dad::check_discard_it_sd_mult (QStringList &files,
                                           bool optionally_discard)
{
  if (((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_discard_it_sd_mult"]
          != "true"
      || !files.size ())
    {
      return;
  ...
  double mult = ((US_Hydrodyn *)us_hydrodyn)
                    ->gparams["dad_discard_it_sd_mult"]
                    .toDouble ();
...
bool
US_Hydrodyn_Dad::all_selected_have_nonzero_conc ()
{
...
void
US_Hydrodyn_Dad::delete_zoomer_if_ranges_changed ()
{
}

void
US_Hydrodyn_Dad::show_created ()
{
...
void
US_Hydrodyn_Dad::show_only_created ()
{
  disable_updates = true;
  lb_files->clearSelection ();
  show_created ();
...
QString
US_Hydrodyn_Dad::vector_double_to_csv (vector<double> vd)
{
  QString result;
...
void
US_Hydrodyn_Dad::plot_zoomed (const QRectF & /* rect */)
{
...
void
US_Hydrodyn_Dad::plot_errors_zoomed (const QRectF & /* rect */)
{
}

void
US_Hydrodyn_Dad::zoom_info ()
{
  if (plot_dist_zoomer)
    {
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Dad::plot_mouse (const QMouseEvent & /* me */)
{
  // TSO << "mouse event\n";
  // zoom_info();
  if (plot_dist_zoomer)
      ...
      if (lb_files->item (i)->isSelected ())
        {
...

          {
...

            {
              ...
              //    "" ) ) .arg( f_extc.count( file ) && f_dndc.count( file ) ?
              //    QString( " dndc:%1 [ml/g]" ).arg( f_dndc[ file ] ) :
              //    QString( "" ) ) .arg( dad_angles.loaded_filename.isEmpty()
              //    ? QString("") : QString( " Angles loaded from:%1" ).arg(
              //    dad_angles.loaded_filename ) ) .arg( f_ri_corr.count( file
              //    ) ? QString( " RI-Corr scatt. angle:%1"  ).arg( f_ri_corr[
              //    file ] ) : QString( "" ) ) .arg( f_ri_corrs.count( file ) ?
              //    QString( " RI-Corr scatt. angles:%1"  ).arg( f_ri_corrs[
              //    file ] ) : QString( "" ) ) .arg( f_ref_index.count( file )
              //    ? QString( " Solvent refractive index:%1"  ).arg(
              //    f_ref_index[ file ] ) : QString( "" ) ) .arg(
              //    f_header.count( file ) ? f_header[ file ] : QString( "" ) )
              //    ;
...
bool
US_Hydrodyn_Dad::is_nonzero_vector (vector<double> &v)
{
...
bool
US_Hydrodyn_Dad::is_zero_vector (vector<double> &v)
{
  bool is_zero = true;
  for (int i = 0; i < (int)v.size (); i++)
    {
...
void
US_Hydrodyn_Dad::to_created (QString file)
{
...
void
US_Hydrodyn_Dad::rename_from_context (const QPoint &pos)
{
#if QT_VERSION >= 0x040000
...
void
US_Hydrodyn_Dad::rename_created (QListWidgetItem *lbi, const QPoint &)
{
  map<QString, bool> existing_items;
  for (int i = 0; i < (int)lb_files->count (); i++)
    {
...
void
US_Hydrodyn_Dad::add_plot (QString name, vector<double> q, vector<double> I,
...
void
US_Hydrodyn_Dad::add_plot (QString name, vector<double> q, vector<double> I,
                           vector<double> errors, bool is_time, bool replot)
{
...
bool
US_Hydrodyn_Dad::compatible_files (QStringList files)
{
  if (!files.size ())
    {
      return true;
...
bool
US_Hydrodyn_Dad::type_files (QStringList files)
{
  if (!files.size ())
    {
      return false;
...
void
US_Hydrodyn_Dad::gaussian_enables ()
{
...
void
US_Hydrodyn_Dad::update_gauss_pos ()
{
  ...
  if (errors_were_on)
    {
      ...
      qDebug () << "dad::gauss_start() emit do resize_plots()";
...
void
US_Hydrodyn_Dad::ggauss_msg_common (
    vector<map<int, set<double> > > &check_common)
{
...
void
US_Hydrodyn_Dad::gauss_pos_text (const QString &text)
{
  if (gaussians.size ())
    {
      if (current_mode == MODE_GAUSSIAN)
...
void
US_Hydrodyn_Dad::gauss_pos_width_text (const QString &text)
{
  if (gaussians.size ())
    {
      if (current_mode == MODE_GAUSSIAN)
...
void
US_Hydrodyn_Dad::gauss_pos_height_text (const QString &text)
{
  if (gaussians.size ())
    {
...
void
US_Hydrodyn_Dad::gauss_pos_dist1_text (const QString &text)
{
  if (gaussians.size ())
    {
      if (current_mode == MODE_GAUSSIAN)
...
void
US_Hydrodyn_Dad::gauss_pos_dist2_text (const QString &text)
{
  if (gaussians.size ())
    {
      if (current_mode == MODE_GAUSSIAN)
...
void
US_Hydrodyn_Dad::gauss_fit_start_text (const QString &text)
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Dad::gauss_fit_end_text (const QString &text)
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Dad::gauss_add_marker (double pos, QColor color, QString text,
#if QT_VERSION < 0x040000
                                   int
#else
                                   Qt::Alignment
...
void
US_Hydrodyn_Dad::gauss_add_hline (double center, double width)
{
  double fwhm = 2.354820045e0 * width;
...
void
US_Hydrodyn_Dad::gauss_init_markers ()
{
...
void
US_Hydrodyn_Dad::gauss_delete_markers ()
{
#if QT_VERSION < 0x040000
...
vector<double>
US_Hydrodyn_Dad::gaussian (double *g) // height, double center, double width )
{
...
void
US_Hydrodyn_Dad::gauss_replot_gaussian ()
{
...
void
US_Hydrodyn_Dad::gauss_add_gaussian (double *g, QColor color)
...
void
US_Hydrodyn_Dad::gauss_init_gaussians ()
{
...
void
US_Hydrodyn_Dad::gauss_delete_gaussians ()
{
...
void
US_Hydrodyn_Dad::gauss_pos_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Dad::gauss_pos_width_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Dad::gauss_pos_height_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Dad::gauss_pos_dist1_focus (bool hasFocus)
{
  ...
  if (hasFocus && !cb_ggauss_scroll->isChecked ())
    {
      ...
      update_gauss_pos ();
      double dist
          = ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_dist_max"].toDouble ();
...
void
US_Hydrodyn_Dad::gauss_pos_dist2_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Dad::gauss_fit_start_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Dad::gauss_fit_end_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Dad::plot_gaussian_sum ()
{
...
void
US_Hydrodyn_Dad::replot_gaussian_sum ()
{
  // add up the curve data
...
US_Hydrodyn_Dad::gauss_fit ()
{
  ...
  if (current_mode == MODE_GGAUSSIAN)
    {
      ...
      wheel_enables (false);
      disconnect (qwtw_wheel, SIGNAL (valueChanged (double)), 0, 0);
      US_Hydrodyn_Dad_Fit_Global *shfg
          = new US_Hydrodyn_Dad_Fit_Global (this, this);
...

  {
    ...
    // qDebug() << "dad::gauss_fit gauss_max_height " << gauss_max_height;
  }

  wheel_enables (false);
  disconnect (qwtw_wheel, SIGNAL (valueChanged (double)), 0, 0);
  US_Hydrodyn_Dad_Fit *shf = new US_Hydrodyn_Dad_Fit (this, true, this);
...
vector<double>
US_Hydrodyn_Dad::compute_gaussian_sum (vector<double> t, vector<double> g)
{
...
vector<double>
US_Hydrodyn_Dad::compute_gaussian (vector<double> t, vector<double> g)
{
  vector<double> result (t.size ());
...
double
US_Hydrodyn_Dad::compute_gaussian_peak (QString file, vector<double> g)
{
...
QString
US_Hydrodyn_Dad::pad_zeros (int val, int max)
{
...
QStringList
US_Hydrodyn_Dad::all_selected_files ()
{
  QStringList files;
  for (int i = 0; i < lb_files->count (); i++)
    {
...
set<QString>
US_Hydrodyn_Dad::all_selected_files_set ()
{
  set<QString> result;
  for (int i = 0; i < lb_files->count (); i++)
    {
...
int
US_Hydrodyn_Dad::ggaussian_sel_no_gaussian_count ()
{
...
bool
US_Hydrodyn_Dad::ggaussian_compatible (bool check_against_global)
{
  QStringList files = all_selected_files ();
...
bool
US_Hydrodyn_Dad::ggaussian_compatible (set<QString> &selected,
...
bool
US_Hydrodyn_Dad::ggaussian_compatible (QStringList &files,
                                       bool check_against_global)
{
...
void
US_Hydrodyn_Dad::add_ggaussian_curve (QString name, vector<double> y)
{
...
void
US_Hydrodyn_Dad::ggauss_results ()
{
  if (unified_ggaussian_ok)
    {
...
void
US_Hydrodyn_Dad::created_dir_pressed ()
{
  QString use_dir = le_dir->text ();
  ((US_Hydrodyn *)us_hydrodyn)->select_from_directory_history (use_dir, this);
  QString s = QFileDialog::getExistingDirectory (
...
void
US_Hydrodyn_Dad::gauss_as_curves ()
{
  if (current_mode == MODE_GAUSSIAN)
    {
      for (unsigned int i = 0; i < (unsigned int)gaussians.size ();
...
void
US_Hydrodyn_Dad::set_fix_width ()
{
}

void
US_Hydrodyn_Dad::set_fix_dist1 ()
{
}

void
US_Hydrodyn_Dad::set_fix_dist2 ()
{
}

bool
US_Hydrodyn_Dad::check_fit_range ()
{
  if (!gaussians.size ())
    {
      return true;
...
void
US_Hydrodyn_Dad::hide_widgets (vector<QWidget *> widgets, bool hide)
{
...
void
US_Hydrodyn_Dad::update_gauss_mode ()
{
