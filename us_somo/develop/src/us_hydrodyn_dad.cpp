...
void DAD_Lambdas::clear() { lambdas.clear(); }

QString DAD_Lambdas::list() {
  QStringList qsl;
...
bool DAD_Lambdas::load(const QString &filename, const QStringList &filelines,
                       QString &errormsg) {
  if (!filelines.size()) {
...
bool DAD_Lambdas::load(const QString &filename, QString &errormsg) {
  QString contents;
...
QString DAD_Angle::list_rich(double lambda, double n) {
  if (!lambda || !n) {
...
bool DAD_Angle::populate(const QStringList &qsl) {
...
void DAD_Angles::clear() {
  dad_angle.clear();
  q_to_ri.clear();
  loaded_filename = "";
}

void DAD_Angles::build_q_to_ri(double lambda, double n) {
    ...
    if (ma.second.has_angle_ri_corr) {
      q_to_ri[QString("%1")
                  .arg(n * 4 * M_PI *
                       ...
                       (lambda * 10))
                  .toDouble()] = ma.second.angle_ri_corr;
    }
  }
...
QStringList DAD_Angles::list_active() {
  QStringList qsl;
  for (auto it = dad_angle.begin(); it != dad_angle.end(); ++it) {
...
bool DAD_Angles::populate(const QString &filename, const QStringList &qsl) {
...
bool DAD_Angles::load(const QString &filename, const QStringList &csvlines,
                      QString &errormsg) {
  if (!csvlines.size()) {
    errormsg = us_tr("Empty file");
    return false;
  ...
  while (data.size() &&
    ...
    if (qsl.size() < 5) {
      errormsg +=
          QString(us_tr("Error in UV-Vis Angles csv line %1\n")).arg(pos);
      continue;
    }
    ...
    dad_angle[index].norm_coef = qsl[4] == "\"n/a\"" ? 0 : qsl[4].toDouble();
...
bool DAD_Angles::load(const QString &filename, QString &errormsg) {
  QString contents;
  if (!US_File_Util::getcontents(filename, contents, errormsg)) {
    return false;
  }
...
bool DAD_Angles::save(const QString &filename, QString &errormsg) {
  QStringList qsl;
...
US_Hydrodyn_Dad::US_Hydrodyn_Dad(csv csv1, void *us_hydrodyn, QWidget *p)
  ...
  //   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "dad_bl_integral"
  //   ) )
  //   {
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)
    ...
    ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_discard_it_sd_mult"] = "true";
  }
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)
           ->gparams.count("dad_maxfpk_restart_tries") ||
      !started_in_expert_mode) {
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)
    ...
    ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_makeiq_cutmax_pct"] = "true";
  }
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count("dad_cb_makeiq_avg_peaks")) {
    ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_makeiq_avg_peaks"] = "false";
  }
...
void US_Hydrodyn_Dad::push_back_color_if_ok(QColor bg, QColor set) {
...
void US_Hydrodyn_Dad::cancel() { close(); }

void US_Hydrodyn_Dad::help() {
  US_Help *online_help;
...
void US_Hydrodyn_Dad::clear_display() {
  editor->clear();
  editor->append("\n\n");
}

void US_Hydrodyn_Dad::update_font() {
  bool ok;
  QFont newFont;
...
void US_Hydrodyn_Dad::editor_msg(QString color, QString msg) {
...
void US_Hydrodyn_Dad::editor_msg_qc(QColor qcolor, QString msg) {
  QColor save_color = editor->textColor();
...
bool US_Hydrodyn_Dad::activate_saxs_window() {
  if (!*saxs_widget) {
...
csv US_Hydrodyn_Dad::current_csv() {
  csv tmp_csv = csv1;

  return tmp_csv;
}

void US_Hydrodyn_Dad::clear_files() {
  disable_all();
...
void US_Hydrodyn_Dad::remove_files(set<QString> &fileset, bool replot) {
  QStringList files;
...
void US_Hydrodyn_Dad::clear_files(QStringList files, bool quiet) {
...
class dad_sortable_qstring {
 public:
  double x;
  QString name;
  bool operator<(const dad_sortable_qstring &objIn) const {
    return x < objIn.x;
  }
};

void US_Hydrodyn_Dad::load_conc() {
  add_files(true);
  rb_pbmode_conc->setChecked(true);
  pbmode_select(PBMODE_CONC);
}

void US_Hydrodyn_Dad::add_dir() { add_files(false, true); }

void US_Hydrodyn_Dad::add_files(bool load_conc, bool from_dir) {
  disable_all();
...

  {
    ...
    if (ref_indices.size() == 1) {
      QMessageBox::warning(
          this, windowTitle() + us_tr(" : Add files"),
          QString(us_tr("Loaded files contain a different solvent refractive "
                        "index (%1)\n"
                        "than set in the Options->UV-Vis parameters->Solvent "
                        "refractive index (%2)\n"
                        "You should remove all files, adjust the parameter "
                        "value and reload the files"))
...
void US_Hydrodyn_Dad::add_files(QStringList filenames) {
  map<QString, bool> existing_items;
  for (int i = 0; i < lb_files->count(); i++) {
    existing_items[lb_files->item(i)->text()] = true;
  }
...
void US_Hydrodyn_Dad::update_files() {
  if (!disable_updates) {
    plot_files();
    update_enables();
  }
...
void US_Hydrodyn_Dad::select_all() {
...
void US_Hydrodyn_Dad::select_all_created() {
  bool all_selected = true;
...
bool US_Hydrodyn_Dad::load_file(QString filename, bool load_conc) {
  errormsg = "";
  QFile f(filename);
  if (!f.exists()) {
  ...
  if (ext == "txt") {
    ...
    // try absorbtion
    {
      ...
      if (errormsg != "not a valid dad file") {
        QMessageBox::warning(this, "US-SOMO UV-Vis load Error",
  ...
  if (ext == "csv") {
    ...
    //                            ,QString( us_tr( "Loading file %1:\nError: %2"
    //                            ) ).arg( filename ).arg( us_tr( errormsg ) )
    //                            );
    //       return false;
    //    }
    // }
...
void US_Hydrodyn_Dad::set_conc_file(QString file) {
  if (file.isEmpty()) {
    for (int i = 0; i < lb_files->count(); i++) {
      if (lb_files->item(i)->isSelected()) {
...
void US_Hydrodyn_Dad::set_signal() {
  for (int i = 0; i < lb_files->count(); i++) {
    if (lb_files->item(i)->isSelected()) {
...
void US_Hydrodyn_Dad::set_empty() {
  for (int i = 0; i < lb_files->count(); i++) {
    if (lb_files->item(i)->isSelected()) {
...
void US_Hydrodyn_Dad::update_created_files() {
  if (!disable_updates) {
    update_enables();
  }
}

void US_Hydrodyn_Dad::avg() {
  // QStringList files;
  //    for ( int i = 0; i < lb_files->count(); i++ )
  //    {
  //       if ( lb_files->item( i )->isSelected()
...
QString US_Hydrodyn_Dad::qstring_common_head(QStringList qsl,
                                             bool strip_digits) {
  if (!qsl.size()) {
    return "";
  }
...
QString US_Hydrodyn_Dad::qstring_common_tail(QStringList qsl,
                                             bool strip_digits) {
  if (!qsl.size()) {
    return "";
  }
...
QString US_Hydrodyn_Dad::qstring_common_head(QString s1, QString s2) {
...
QString US_Hydrodyn_Dad::qstring_common_tail(QString s1, QString s2) {
  int min_len = (int)s1.length();
  if (min_len > (int)s2.length()) {
    min_len = (int)s2.length();
  }
...
void US_Hydrodyn_Dad::save_created() {
  QStringList created_not_saved_list;
...
void US_Hydrodyn_Dad::save_created_csv() {
  QStringList created_not_saved_list;
...
bool US_Hydrodyn_Dad::save_files_csv(QStringList files) {
  if (!files.size()) {
...
bool US_Hydrodyn_Dad::save_files(QStringList files) {
  bool errors = false;
...
bool US_Hydrodyn_Dad::save_file(QString file, bool &cancel,
                                bool &overwrite_all) {
  if (!f_qs.count(file)) {
...
void US_Hydrodyn_Dad::update_csv_conc() {
...
map<QString, double> US_Hydrodyn_Dad::current_concs(bool quiet) {
...
map<QString, double> US_Hydrodyn_Dad::window_concs() {
  map<QString, double> concs;
  if (conc_widget) {
    csv tmp_csv = conc_window->current_csv();
    for (unsigned int i = 0; i < tmp_csv.data.size(); i++) {
...
vector<double> US_Hydrodyn_Dad::union_q(QStringList files) {
...
void US_Hydrodyn_Dad::smooth() {
  QStringList files = all_selected_files();
  smooth(files);
}

bool US_Hydrodyn_Dad::get_peak(QString file, double &peak, bool full) {
  double pos;
...
bool US_Hydrodyn_Dad::get_peak(QString file, double &peak, double &pos,
                               bool full) {
  if (!f_Is.count(file)) {
...
void US_Hydrodyn_Dad::smooth(QStringList files) {
  bool ok;
  int smoothing =
...
void US_Hydrodyn_Dad::create_i_of_t() {
  disable_all();
...
void US_Hydrodyn_Dad::create_i_of_t(QStringList files) {
  // find common q
  QString head = qstring_common_head(files, true);
...
void US_Hydrodyn_Dad::test_i_of_t() {
  disable_all();
  QStringList files = all_selected_files();
...
bool US_Hydrodyn_Dad::check_zi_window(QStringList &files,
...
void US_Hydrodyn_Dad::check_discard_it_sd_mult(QStringList &files,
...
bool US_Hydrodyn_Dad::all_selected_have_nonzero_conc() {
...
void US_Hydrodyn_Dad::delete_zoomer_if_ranges_changed() {}

void US_Hydrodyn_Dad::show_created() {
...
void US_Hydrodyn_Dad::show_only_created() {
  disable_updates = true;
  lb_files->clearSelection();
  show_created();
}

QString US_Hydrodyn_Dad::vector_double_to_csv(vector<double> vd) {
  QString result;
...
void US_Hydrodyn_Dad::plot_zoomed(const QRectF & /* rect */) {
...
void US_Hydrodyn_Dad::plot_errors_zoomed(const QRectF & /* rect */) {}

void US_Hydrodyn_Dad::zoom_info() {
  if (plot_dist_zoomer) {
#if QT_VERSION < 0x040000
...
void US_Hydrodyn_Dad::plot_mouse(const QMouseEvent & /* me */) {
  // TSO << "mouse event\n";
  // zoom_info();
  if (plot_dist_zoomer) {
...
void US_Hydrodyn_Dad::select_vis() {
#if QT_VERSION < 0x040000
...
void US_Hydrodyn_Dad::remove_vis() {
#if QT_VERSION < 0x040000
  double zrx1 = plot_dist_zoomer->zoomRect().x1();
  double zrx2 = plot_dist_zoomer->zoomRect().x2();
  double zry1 = plot_dist_zoomer->zoomRect().y1();
...
void US_Hydrodyn_Dad::crop_left() {
...
void US_Hydrodyn_Dad::crop_right() {
...
void US_Hydrodyn_Dad::crop_undo() {
  if (!crop_undos.size()) {
    return;
  }
    ...
    if (lb_files->item(i)->isSelected()) {
...

      {
...

        {
          ...
          //    ) ) .arg( f_extc.count( file ) && f_dndc.count( file ) ?
          //    QString( " dndc:%1 [ml/g]" ).arg( f_dndc[ file ] ) : QString( ""
          //    ) ) .arg( dad_angles.loaded_filename.isEmpty() ? QString("") :
          //    QString( " Angles loaded from:%1" ).arg(
          //    dad_angles.loaded_filename ) ) .arg( f_ri_corr.count( file ) ?
          //    QString( " RI-Corr scatt. angle:%1"  ).arg( f_ri_corr[ file ] )
          //    : QString( "" ) ) .arg( f_ri_corrs.count( file ) ? QString( "
          //    RI-Corr scatt. angles:%1"  ).arg( f_ri_corrs[ file ] ) :
          //    QString( "" ) ) .arg( f_ref_index.count( file ) ? QString( "
          //    Solvent refractive index:%1"  ).arg( f_ref_index[ file ] ) :
          //    QString( "" ) ) .arg( f_header.count( file ) ? f_header[ file ]
          //    : QString( "" ) )
          //    ;
...
bool US_Hydrodyn_Dad::is_nonzero_vector(vector<double> &v) {
...
bool US_Hydrodyn_Dad::is_zero_vector(vector<double> &v) {
  bool is_zero = true;
  for (int i = 0; i < (int)v.size(); i++) {
    if (v[i] != 0e0) {
      is_zero = false;
...
void US_Hydrodyn_Dad::to_created(QString file) {
...
void US_Hydrodyn_Dad::crop_to_vis() {
  // find curves within zoomRect & select only them
  double minx = plot_dist_zoomer->zoomRect().left();
  double maxx = plot_dist_zoomer->zoomRect().right();
  double miny = plot_dist_zoomer->zoomRect().top();
...
void US_Hydrodyn_Dad::legend_set() {
#if QT_VERSION >= 0x040000
...
void US_Hydrodyn_Dad::similar_files() {
  vector<QString> selected;
  for (int i = 0; i < lb_files->count(); i++) {
    if (lb_files->item(i)->isSelected()) {
      selected.push_back(lb_files->item(i)->text());
...
void US_Hydrodyn_Dad::rename_from_context(const QPoint &pos) {
#if QT_VERSION >= 0x040000
...
void US_Hydrodyn_Dad::rename_created(QListWidgetItem *lbi, const QPoint &) {
  map<QString, bool> existing_items;
  for (int i = 0; i < (int)lb_files->count(); i++) {
    existing_items[lb_files->item(i)->text()] = true;
  }
...
void US_Hydrodyn_Dad::normalize() {
  QStringList files = all_selected_files();
  //    for ( int i = 0; i < lb_files->count(); i++ )
  //    {
  //       if ( lb_files->item( i )->isSelected() &&
...
void US_Hydrodyn_Dad::add_plot(QString name, vector<double> q, vector<double> I,
...
void US_Hydrodyn_Dad::add_plot(QString name, vector<double> q, vector<double> I,
                               vector<double> errors, bool is_time,
                               bool replot) {
...
void US_Hydrodyn_Dad::crop_zero() {
...
bool US_Hydrodyn_Dad::compatible_files(QStringList files) {
  if (!files.size()) {
    return true;
  }
...
bool US_Hydrodyn_Dad::type_files(QStringList files) {
  if (!files.size()) {
    return false;
  }
...
void US_Hydrodyn_Dad::gaussian_enables() {
...
void US_Hydrodyn_Dad::update_gauss_pos() {
...
void US_Hydrodyn_Dad::gauss_start() {
...
void US_Hydrodyn_Dad::gauss_clear() {
  gaussian_pos = 0;
  gaussians.clear();
  gauss_init_markers();
  gauss_init_gaussians();
...
void US_Hydrodyn_Dad::gauss_new() {
...
void US_Hydrodyn_Dad::gauss_prev() {
  if (gaussian_pos > 0) {
    gaussian_pos--;
  }
  update_gauss_pos();
...
void US_Hydrodyn_Dad::gauss_next() {
...
void US_Hydrodyn_Dad::gauss_save() {
  {
    QDir dir1(le_created_dir->text());
    if (!dir1.exists()) {
      if (dir1.mkdir(le_created_dir->text())) {
...
void US_Hydrodyn_Dad::ggauss_msg_common(
    vector<map<int, set<double> > > &check_common) {
...
void US_Hydrodyn_Dad::gauss_pos_text(const QString &text) {
  if (gaussians.size()) {
    if (current_mode == MODE_GAUSSIAN) {
...
void US_Hydrodyn_Dad::gauss_pos_width_text(const QString &text) {
  if (gaussians.size()) {
    if (current_mode == MODE_GAUSSIAN) {
...
void US_Hydrodyn_Dad::gauss_pos_height_text(const QString &text) {
  if (gaussians.size()) {
...
void US_Hydrodyn_Dad::gauss_pos_dist1_text(const QString &text) {
  if (gaussians.size()) {
    if (current_mode == MODE_GAUSSIAN) {
...
void US_Hydrodyn_Dad::gauss_pos_dist2_text(const QString &text) {
  if (gaussians.size()) {
    if (current_mode == MODE_GAUSSIAN) {
...
void US_Hydrodyn_Dad::gauss_fit_start_text(const QString &text) {
#if QT_VERSION < 0x040000
...
void US_Hydrodyn_Dad::gauss_fit_end_text(const QString &text) {
#if QT_VERSION < 0x040000
...
void US_Hydrodyn_Dad::gauss_add_marker(double pos, QColor color, QString text,
#if QT_VERSION < 0x040000
                                       int
#else
                                       Qt::Alignment
...
void US_Hydrodyn_Dad::gauss_add_hline(double center, double width) {
  double fwhm = 2.354820045e0 * width;
...
void US_Hydrodyn_Dad::gauss_init_markers() {
...
void US_Hydrodyn_Dad::gauss_delete_markers() {
#if QT_VERSION < 0x040000
...
vector<double> US_Hydrodyn_Dad::gaussian(
...
void US_Hydrodyn_Dad::gauss_delete() {
...
void US_Hydrodyn_Dad::gauss_replot_gaussian() {
...
void US_Hydrodyn_Dad::gauss_add_gaussian(double *g, QColor color)
...
void US_Hydrodyn_Dad::gauss_init_gaussians() {
...
void US_Hydrodyn_Dad::gauss_delete_gaussians() {
...
void US_Hydrodyn_Dad::gauss_pos_focus(bool hasFocus) {
...
void US_Hydrodyn_Dad::gauss_pos_width_focus(bool hasFocus) {
...
void US_Hydrodyn_Dad::gauss_pos_height_focus(bool hasFocus) {
...
void US_Hydrodyn_Dad::gauss_pos_dist1_focus(bool hasFocus) {
...
void US_Hydrodyn_Dad::gauss_pos_dist2_focus(bool hasFocus) {
...
void US_Hydrodyn_Dad::gauss_fit_start_focus(bool hasFocus) {
...
void US_Hydrodyn_Dad::gauss_fit_end_focus(bool hasFocus) {
...
void US_Hydrodyn_Dad::plot_gaussian_sum() {
...
void US_Hydrodyn_Dad::replot_gaussian_sum() {
  // add up the curve data
...
void US_Hydrodyn_Dad::gauss_fit() {
  check_fit_range();
...
vector<double> US_Hydrodyn_Dad::compute_gaussian_sum(vector<double> t,
                                                     vector<double> g) {
...
vector<double> US_Hydrodyn_Dad::compute_gaussian(vector<double> t,
                                                 vector<double> g) {
  vector<double> result(t.size());
...
double US_Hydrodyn_Dad::compute_gaussian_peak(QString file, vector<double> g) {
...
QString US_Hydrodyn_Dad::pad_zeros(int val, int max) {
...
QStringList US_Hydrodyn_Dad::all_selected_files() {
  QStringList files;
  for (int i = 0; i < lb_files->count(); i++) {
    if (lb_files->item(i)->isSelected()) {
      files << lb_files->item(i)->text();
...
set<QString> US_Hydrodyn_Dad::all_selected_files_set() {
  set<QString> result;
  for (int i = 0; i < lb_files->count(); i++) {
    if (lb_files->item(i)->isSelected()) {
...
QStringList US_Hydrodyn_Dad::all_files() {
  QStringList files;
  for (int i = 0; i < lb_files->count(); i++) {
    files << lb_files->item(i)->text();
  }
...
map<QString, bool> US_Hydrodyn_Dad::all_files_map() {
...
int US_Hydrodyn_Dad::ggaussian_sel_no_gaussian_count() {
...
bool US_Hydrodyn_Dad::ggaussian_compatible(bool check_against_global) {
  QStringList files = all_selected_files();
  return ggaussian_compatible(files, check_against_global);
}

bool US_Hydrodyn_Dad::ggaussian_compatible(set<QString> &selected,
...
bool US_Hydrodyn_Dad::ggaussian_compatible(QStringList &files,
                                           bool check_against_global) {
...
void US_Hydrodyn_Dad::ggauss_rmsd() {
  if (ggauss_recompute()) {
...
void US_Hydrodyn_Dad::add_ggaussian_curve(QString name, vector<double> y) {
...
void US_Hydrodyn_Dad::ggauss_results() {
  if (unified_ggaussian_ok) {
...
void US_Hydrodyn_Dad::dir_pressed() {
...
void US_Hydrodyn_Dad::created_dir_pressed() {
  QString use_dir = le_dir->text();
  ((US_Hydrodyn *)us_hydrodyn)->select_from_directory_history(use_dir, this);
  QString s = QFileDialog::getExistingDirectory(
...
void US_Hydrodyn_Dad::gauss_as_curves() {
  if (current_mode == MODE_GAUSSIAN) {
    for (unsigned int i = 0; i < (unsigned int)gaussians.size();
         i += gaussian_type_size) {
...
void US_Hydrodyn_Dad::set_sd_weight() {
  if (current_mode == MODE_GAUSSIAN) {
    replot_gaussian_sum();
  } else {
    plot_errors->detachItems(QwtPlotItem::Rtti_PlotCurve);
...
void US_Hydrodyn_Dad::set_fix_width() {}

void US_Hydrodyn_Dad::set_fix_dist1() {}

void US_Hydrodyn_Dad::set_fix_dist2() {}

bool US_Hydrodyn_Dad::check_fit_range() {
  if (!gaussians.size()) {
    return true;
  }
...
void US_Hydrodyn_Dad::hide_widgets(vector<QWidget *> widgets, bool hide) {
...
void US_Hydrodyn_Dad::save_state() {
  QString use_dir = QDir::currentPath();
...
void US_Hydrodyn_Dad::update_gauss_mode() {
