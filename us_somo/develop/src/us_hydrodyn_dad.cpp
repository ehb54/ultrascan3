...

   {
      ...
      for ( auto const & line : use_filelines ) {
         ...
         if ( !isNumeric || lambda <= 0 ) {
            errormsg = QString( us_tr( "invalid lambda found %1 (%2) in %3" ) ).arg( lambda ).arg( line ).arg( filename );
            return false;
         }
            
         if ( used.count( lambda ) ) {
            errormsg = QString( us_tr( "duplicate value found %1 (%2) in %3" ) ).arg( lambda ).arg( line ).arg( filename );
            return false;
         }
         used.insert( lambda );
...
bool DAD_Angles::load( const QString & filename, const QStringList & csvlines, QString & errormsg ) {
   if ( !csvlines.size() ) {
      errormsg = us_tr( "Empty file" );
      return false;
   }
   ...
   while( data.size()
      ...
      dad_angle[index].angle_ri_corr      = qsl[2] == "\"n/a\"" ? 0 : qsl[2].toDouble();
                                 ...
                                 ) : QFrame( p )
{
   ...
   use_line_width = ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "dad_line_width" ].toUInt();
   ...
   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "dad_cb_discard_it_sd_mult" ) )
   {
   ...
   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "dad_cb_makeiq_cutmax_pct" ) )
   {
   ...
   gaussian_type = 
      ...
      (gaussian_types)( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "dad_gaussian_type" ].toInt() : GAUSS;
...
bool US_Hydrodyn_Dad::load_file( QString filename, bool load_conc )
{
   ...
   // load csv columns as time curves with rescaling for concentration time
      ...
      US_Hydrodyn_Dad_Conc_Csv_Frames * conc_csv_frames_instance = 
         new US_Hydrodyn_Dad_Conc_Csv_Frames( us_hydrodyn, & conc_csv_params, this );
...
QString US_Hydrodyn_Dad::qstring_common_head( QStringList qsl, bool strip_digits )
{
   if ( !qsl.size() )
   {
      return "";
...
QString US_Hydrodyn_Dad::qstring_common_tail( QStringList qsl, bool strip_digits )
{
   if ( !qsl.size() )
   {
      return "";
...
QString US_Hydrodyn_Dad::qstring_common_head( QString s1, 
                                                      QString s2 )
{
...
QString US_Hydrodyn_Dad::qstring_common_tail( QString s1, 
                                                      QString s2 )
{
   int min_len = (int)s1.length();
   if ( min_len > (int)s2.length() )
...
bool US_Hydrodyn_Dad::save_file( QString file, bool &cancel, bool &overwrite_all )
{
...

   {
      ...
      //    .arg( dad_angles.loaded_filename.isEmpty() ? QString("") : QString( " Angles loaded from:%1" ).arg( dad_angles.loaded_filename ) )
...
void US_Hydrodyn_Dad::conc()
{
   ...
   if ( conc_widget )
   {
      ...
      conc_window = new US_Hydrodyn_Dad_Conc( csv_conc,
                                                      this );
      conc_window->show();
   }
}
...
bool US_Hydrodyn_Dad::get_peak( QString file, double &peak, double &pos, bool full )
{
   if ( !f_Is.count( file ) )
   {
      editor_msg( "red", QString( us_tr( "Internal error: get_peak requested on %1 but no data available" ) ).arg( file ) );
...
bool US_Hydrodyn_Dad::check_zi_window( QStringList & files, const QString & extra_text )
{
...
void US_Hydrodyn_Dad::check_discard_it_sd_mult( QStringList & files, bool optionally_discard )
{
   if ( ((US_Hydrodyn *)us_hydrodyn)->gparams[ "dad_cb_discard_it_sd_mult" ] != "true" ||
        !files.size() )
   {
      return;
   }

   double mult = ((US_Hydrodyn *)us_hydrodyn)->gparams[ "dad_discard_it_sd_mult" ].toDouble();
...
void US_Hydrodyn_Dad::add_plot( QString           name,
                                      vector < double > q,
                                      vector < double > I,
                                      bool              is_time,
                                      bool              replot )
...
void US_Hydrodyn_Dad::ggauss_msg_common( vector < map < int, set < double > > > & check_common )
{
...
void US_Hydrodyn_Dad::gauss_add_marker( double pos, 
                                              QColor color, 
                                              QString text, 
#if QT_VERSION < 0x040000
                                              int 
...
vector < double > US_Hydrodyn_Dad::gaussian( double * g ) // height, double center, double width )
{
...
void US_Hydrodyn_Dad::gauss_pos_dist1_focus( bool hasFocus )
{
   ...
   if ( hasFocus && !cb_ggauss_scroll->isChecked() )
   {
      ...
      disconnect( qwtw_wheel, SIGNAL( valueChanged( double ) ), 0, 0 );
      update_gauss_pos();
      double dist = ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "dad_dist_max" ].toDouble();
...
void US_Hydrodyn_Dad::gauss_fit()
{
   ...
   if ( current_mode == MODE_GGAUSSIAN )
   {
      ...
      US_Hydrodyn_Dad_Fit_Global *shfg = 
         new US_Hydrodyn_Dad_Fit_Global(
                                              this,
                                              this );
   ...
   wheel_enables( false );
   disconnect( qwtw_wheel, SIGNAL( valueChanged( double ) ), 0, 0 );
   US_Hydrodyn_Dad_Fit *shf = 
      new US_Hydrodyn_Dad_Fit(
                                    this,
                                    true,
                                    this );
...
vector < double > US_Hydrodyn_Dad::compute_gaussian_sum( vector < double > t, vector < double > g )
{
...
vector < double > US_Hydrodyn_Dad::compute_gaussian( vector < double > t, vector < double > g )
{
   vector < double > result( t.size() );
...
bool US_Hydrodyn_Dad::ggaussian_compatible( set < QString > & selected, bool check_against_global )
{
   QStringList files;
...
bool US_Hydrodyn_Dad::ggaussian_compatible( QStringList & files, bool check_against_global )
{
