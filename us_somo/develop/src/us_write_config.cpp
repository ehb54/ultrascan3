...
bool
US_Write_Config::write_config (struct Config config_list)
{
...

  {
    ...
    if (!temp_dir.exists ())
      {
        message = us_tr (
            "The UltraScan System Directory could not be found!\n\n"
    ...
    if (!temp_dir.exists ())
      {
        if (!temp_dir.mkdir (rcdir))
          {
            message
                = us_tr ("Could not create the Configuration Directory!\n\n"
                         + rcdir + "\nPlease check your write permissions!");
            errorMessage (warning, message);
            cerr << warning << ":\n" << message << endl;
            return false;
  ...
  if (f.open (QIODevice::WriteOnly | QIODevice::Text))
    ...
    {
      message = us_tr (
          QString ("Could not open Configuration File for update.\n\n"
                   "$HOME/ultrascan/%1\nPlease check your write permissions!")
              .arg (USRC));
      errorMessage (warning, message);
      cerr << warning << ":\n" << message << endl;
...
void
US_Write_Config::errorMessage (QString &str1, QString &str2)
{
