...
bool
US_Saxs_Util::run_pat (
#if defined(CMDLINE) // to silence compiler warning

    map<QString, QString> &parameters, map<QString, QString> &results
#else
    map<QString, QString> &, map<QString, QString> &
#endif
)
{
#if defined(CMDLINE)
  ...
  if (!model_vector.size ())
    {
      results["errors"]
          += QString (" file %1: empty model vector?").arg (file);
      return false;
    }
...
bool
US_Saxs_Util::pat_model (PDB_model &model, bool bead_coordinates)
{
            ...
            for (int k = 0; k < (int)model.molecule[j].atom.size (); ++k)
              {
                ...
                // us_qdebug( QString( "atom %1 coords %2 %3 %4 radius %5 mw
                // %6" )
                //         .arg( dt_pos )
                //         .arg( dt.x )
                //         .arg( dt.y )
...
bool
US_Saxs_Util::write_model (PDB_model &model, QString filename)
{
  QFile f (filename);
...
void
US_Saxs_Util::compute_tau (double drrev1, double drrev2, double drrev3,
                           double fconv, vector<double> &tau)
{
  ...
  if (a == 1.0)
    {
      ...
      tau[6]
          = ((1.0 / tau[0]) + (2 * (1.0 / tau[1])) + (2 * (1.0 / tau[3]))) / 5;
    }
  ...
  if (a == 2.0)
    {
      ...
      tau[6]
          = ((2 * (1.0 / tau[1])) + (2 * (1.0 / tau[3])) + (1.0 / tau[4])) / 5;
    }
  ...
  // for ( int i = 0; i < (int) tau.size(); ++i )
  // {
  //    us_qdebug( QString( "compute_tau: tau results %1 %2" ).arg( i+1 ).arg(
  //    tau[ i ] ) );
  // }
}
