...
US_Saxs_Util::nsa_validate ()
{
  ...
  if (!sgp_exp_q.size () || sgp_exp_q.size () != sgp_exp_I.size ())
    {
      errormsg = "Error: empty or inconsistant grid found experimental data, "
                 "note:csv experiment grids not currently supported for sgp\n";
      return false;
    }
...
bool
US_Saxs_Util::check_overlap (vector<PDB_atom> &bm, bool quiet)
{
      ...
      for (unsigned int j = i + 1; j < bm.size () - 1; j++)
        {
          if (bm[i].bead_computed_radius + bm[j].bead_computed_radius
              > dist (bm[i].bead_coordinate, bm[j].bead_coordinate))
            {
              ...
              overlaps_found = true;
              cout << QString ("overlap found between beads %1 %2 with "
                               "distance of %3 and overlap of %4\n")
                          .arg (i + 1)
                          .arg (j + 1)
                          .arg (dist (bm[i].bead_coordinate,
                                      bm[j].bead_coordinate))
                          .arg (bm[i].bead_computed_radius
                                + bm[j].bead_computed_radius
                                - dist (bm[i].bead_coordinate,
                                        bm[j].bead_coordinate));
...
US_Saxs_Util::nsa_fitness ()
{
                  ...
                  if (d < r1 + r2)
                    {
                      ...
                      if (pn.axis[0] == 0.0 && pn.axis[1] == 0.0
                          && pn.axis[2] == 0.0)
                        {
  ...
  if (bead_models[0].size () > 1)
  ...
  else
    {
      ...
      if (!iqq_sphere ("tmp", bead_models[0][0].bead_computed_radius,
                       delta_rho, our_saxs_options.start_q,
  ...
  if (nsa_use_scaling_fit)
    {
      if (sgp_use_e)
        {
          fit_ok = scaling_fit (sgp_last_I, sgp_exp_I, sgp_exp_e,
                                nsa_last_scaling, chi2);
        }
      else
        {
...
bool
US_Saxs_Util::nsa_fitness_setup (unsigned int size)
{
  if (!size)
    {
...
double
US_Saxs_Util::nsa_gsm_f (our_vector *v)
{
...
void
US_Saxs_Util::nsa_gsm_df (our_vector *vd, our_vector *v)
{
  double y0;
  double y2;
  double sav_ve;
...
bool
US_Saxs_Util::nsa_gsm (double &nrmsd, our_vector *vi, QString method)
{
  ...
  switch (gsm_method)
    ...
    default:
      {
        errormsg = QString ("unknown gsm method %1 ")
...
QString
US_Saxs_Util::nsa_qs_bead_model ()
{
  QString qs;
...
QString
US_Saxs_Util::nsa_physical_stats ()
{
          ...
          for (unsigned int j = 0; j < 3; j++)
            {
              ...
              if (prmin.axis[j]
                ...
                {
                  prmin.axis[j] = bm[i].bead_coordinate.axis[j]
                                  - bm[i].bead_computed_radius;
                }
              if (prmax.axis[j]
                  < bm[i].bead_coordinate.axis[j] + bm[i].bead_computed_radius)
                {
                  prmax.axis[j] = bm[i].bead_coordinate.axis[j]
                                  + bm[i].bead_computed_radius;
                }
            }
        }
  ...
  nsa_physical_stats_map["result centers axial ratios x:z"]
      = QString ("%1").arg ((pmax.axis[0] - pmin.axis[0])
                            / (pmax.axis[2] - pmin.axis[2]));
  nsa_physical_stats_map["result centers axial ratios x:y"]
      = QString ("%1").arg ((pmax.axis[0] - pmin.axis[0])
                            / (pmax.axis[1] - pmin.axis[1]));
  nsa_physical_stats_map["result centers axial ratios y:z"]
      = QString ("%1").arg ((pmax.axis[1] - pmin.axis[1])
                            / (pmax.axis[2] - pmin.axis[2]));
