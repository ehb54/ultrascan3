...
US_Container_Grpy::US_Container_Grpy(bool pull, bool do_not_use_container) {
  ...
  if (usc.installed() && !do_not_use_container) {
    ...
    if (pull) {
      ...
      qDebug() << "US_Container_Grpy::US_Container_Grpy() : pull : starting "
      ...
      if (!started) {
        qDebug() << "US_Container_Grpy::US_Container_Grpy() : pull : failed to "
                    "start";
        image_name = "";  // no retry?
      }
      ...
      if (!image_name.isEmpty()) {
        ...
        if (!program.waitForFinished(PULL_TIMEOUT_SECS * 1000)) {
          qDebug() << "US_Container_Grpy::US_Container_Grpy() : pull : failed "
                      "to finish in "
      ...
      if (!image_name.isEmpty()) {
        qDebug()
            << "US_Container::US_Container_Grpy() : pull : finished, exit code "
            << program.exitCode();
...
QStringList US_Container_Grpy::arguments(QString directory) {
  ...
  if (!image_name.isEmpty()) {
    ...
    result << "run"
           ...
           << QString("%1:/work")
                  .arg(directory.isEmpty() ? QDir::currentPath() : directory)
           << image_name;
  }
