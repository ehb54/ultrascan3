...
US_Container_Grpy::US_Container_Grpy (bool pull, bool do_not_use_container)
{
  ...
  if (usc.installed () && !do_not_use_container)
    {
      image_name
          = QString ("ehb1/grpy:") + (US_Arch::is_arm () ? "arm64" : "amd64");
      if (pull)
        {
          QProcess program;
          ...
          qDebug ()
              << "US_Container_Grpy::US_Container_Grpy() : pull : starting "
              << usc.type () << arguments.join (" ");
          ...
          if (!started)
            {
              qDebug () << "US_Container_Grpy::US_Container_Grpy() : pull : "
                           "failed to start";
              image_name = ""; // no retry?
            }
          ...
          if (!image_name.isEmpty ())
            {
              qDebug ()
                  << "US_Container_Grpy::US_Container_Grpy() : pull : started";

              if (!program.waitForFinished (PULL_TIMEOUT_SECS * 1000))
                {
                  qDebug () << "US_Container_Grpy::US_Container_Grpy() : pull "
                               ": failed to finish in "
                            << PULL_TIMEOUT_SECS << " seconds";
                  image_name = ""; // no retry?
                }
            }
          ...
          if (!image_name.isEmpty ())
            {
              qDebug () << "US_Container::US_Container_Grpy() : pull : "
                           "finished, exit code "
                        << program.exitCode ();
...
US_Container_Grpy::executable ()
{
  ...
  if (image_name.isEmpty ())
    {
      result = USglobal->config_list.system_dir + QDir::separator () +
#if defined(BIN64)
               "bin64"
#else
               "bin"
...
QStringList
US_Container_Grpy::arguments (QString directory)
{
  QStringList result;
  if (!image_name.isEmpty ())
    {
      ...
      result << "run"
             ...
             << "--name" << container_names.back () << "-v"
             << QString ("%1:/work")
                    .arg (directory.isEmpty () ? QDir::currentPath ()
                                               : directory)
             << image_name;
    }
