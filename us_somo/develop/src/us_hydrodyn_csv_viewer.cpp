...
void
US_Hydrodyn_Csv_Viewer::setupGUI ()
{
  int minHeight1 = 30;
...
void
US_Hydrodyn_Csv_Viewer::cancel ()
{
  close ();
}

void
US_Hydrodyn_Csv_Viewer::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Csv_Viewer::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept ();
...
void
US_Hydrodyn_Csv_Viewer::sort_column (int section)
{
  if (section == 0)
    {
...
public:
  ...
  unsigned int row;
  bool
  operator< (const uhcv_sortable_double &objIn) const
  {
    return x < objIn.x;
  }
};
...
void
US_Hydrodyn_Csv_Viewer::numeric_sort (int section)
{
  ...
  for (list<uhcv_sortable_double>::iterator it = luhcvsd.begin ();
       it != luhcvsd.end (); it++)
    {
      for (unsigned int j = 0; j < csv1.num_data[it->row].size (); j++)
        {
          t_csv->setItem (pos, j,
                          new QTableWidgetItem (csv1.data[it->row][j]));
        }
  ...
  for (unsigned int i = 0; i < avgstdrows.size (); i++)
    {
      ...
      for (unsigned int j = 0; j < csv1.num_data[avgstdrows[i]].size (); j++)
        {
          t_csv->setItem (row, j,
                          new QTableWidgetItem (csv1.data[avgstdrows[i]][j]));
        }
    }
}
