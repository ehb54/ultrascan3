...
void US_Hydrodyn_Dad_Nth::setupGUI() {
  ...
  for (int i = 0; i < ((US_Hydrodyn_Dad *)us_hydrodyn_dad)->lb_files->count();
    ...
    if (((US_Hydrodyn_Dad *)us_hydrodyn_dad)->lb_files->item(i)->isSelected()) {
  ...
  lbl_start_name = new QLabel(((US_Hydrodyn_Dad *)us_hydrodyn_dad)
  ...
  lbl_end_name = new QLabel(((US_Hydrodyn_Dad *)us_hydrodyn_dad)
...
void US_Hydrodyn_Dad_Nth::quit() { close(); }

void US_Hydrodyn_Dad_Nth::go() {
  (*parameters)["go"] = "true";
...
void US_Hydrodyn_Dad_Nth::update_enables() {
  ...
  if (ofs >= 1 && ofs <= fcnt) {
    lbl_start_name->setText(
        ((US_Hydrodyn_Dad *)us_hydrodyn_dad)->lb_files->item(ofs - 1)->text());
  } else {
    lbl_start_name->setText("");
  }
  if (end >= 1 && end <= fcnt) {
    lbl_end_name->setText(
        ((US_Hydrodyn_Dad *)us_hydrodyn_dad)->lb_files->item(end - 1)->text());
  } else {
    lbl_end_name->setText("");
  }
...
void US_Hydrodyn_Dad_Nth::nth_only() {
...
void US_Hydrodyn_Dad_Nth::nth_add() {
  disconnect(lb_files, SIGNAL(itemSelectionChanged()), 0, 0);
  int n = le_n->text().toInt();
  int ofs = le_start->text().toInt();
  int end = le_end->text().toInt();
...
void US_Hydrodyn_Dad_Nth::contains_only() {
  disconnect(lb_files, SIGNAL(itemSelectionChanged()), 0, 0);
  lb_files->clearSelection();
  for (int i = 0; i < lb_files->count(); ++i) {
    if (((US_Hydrodyn_Dad *)us_hydrodyn_dad)
...
void US_Hydrodyn_Dad_Nth::contains_add() {
  disconnect(lb_files, SIGNAL(itemSelectionChanged()), 0, 0);
  for (int i = 0; i < lb_files->count(); ++i) {
    if (((US_Hydrodyn_Dad *)us_hydrodyn_dad)
            ->lb_files->item(i)
...
void US_Hydrodyn_Dad_Nth::pvalues_only() {
  disconnect(lb_files, SIGNAL(itemSelectionChanged()), 0, 0);
  lb_files->clearSelection();
  for (int i = 0; i < lb_files->count(); ++i) {
    QString qs =
...
void US_Hydrodyn_Dad_Nth::pvalues_add() {
  disconnect(lb_files, SIGNAL(itemSelectionChanged()), 0, 0);
  for (int i = 0; i < lb_files->count(); ++i) {
    QString qs =
        ((US_Hydrodyn_Dad *)us_hydrodyn_dad)->lb_files->item(i)->text();
...
void US_Hydrodyn_Dad_Nth::update_files_selected() {
  lb_files_sel->clear();
  for (int i = 0; i < lb_files->count(); ++i) {
    if (lb_files->item(i)->isSelected()) {
