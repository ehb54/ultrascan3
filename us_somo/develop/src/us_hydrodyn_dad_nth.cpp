...
US_Hydrodyn_Dad_Nth::US_Hydrodyn_Dad_Nth (void *us_hydrodyn_dad,
    ...
    : QDialog (p)
{
  ...
  this->ggaussian_last_pfit_map
      = &((US_Hydrodyn_Dad *)us_hydrodyn_dad)->ggaussian_last_pfit_map;

  USglobal = new US_Config ();
  setPalette (PALET_FRAME);
  setWindowTitle (us_tr ("US-SOMO: UV-Vis : Select curves"));
...
US_Hydrodyn_Dad_Nth::~US_Hydrodyn_Dad_Nth () {}

void
US_Hydrodyn_Dad_Nth::setupGUI ()
{
  bool started_in_expert_mode
      = ((US_Hydrodyn_Dad *)us_hydrodyn_dad)->started_in_expert_mode;
  ...
  for (int i = 0; i < ((US_Hydrodyn_Dad *)us_hydrodyn_dad)->lb_files->count ();
       ++i)
    {
      lb_files->addItem (QString ("%1 : %2").arg (i + 1).arg (
          ((US_Hydrodyn_Dad *)us_hydrodyn_dad)->lb_files->item (i)->text ()));
      if (((US_Hydrodyn_Dad *)us_hydrodyn_dad)
              ->lb_files->item (i)
              ->isSelected ())
        {
                        ...
                        USglobal->config_list.fontSize));
  {
    QIntValidator *qdv = new QIntValidator (
        1, ((US_Hydrodyn_Dad *)us_hydrodyn_dad)->lb_files->count (), le_n);
  ...
  {
    QIntValidator *qdv = new QIntValidator (
        1, ((US_Hydrodyn_Dad *)us_hydrodyn_dad)->lb_files->count (), le_start);
  ...
  lbl_start_name
      = new QLabel (((US_Hydrodyn_Dad *)us_hydrodyn_dad)
  ...
  le_end->setText (QString ("%1").arg (
      ((US_Hydrodyn_Dad *)us_hydrodyn_dad)->lb_files->count ()));
  ...
  {
    QIntValidator *qdv = new QIntValidator (
        2, ((US_Hydrodyn_Dad *)us_hydrodyn_dad)->lb_files->count (), le_end);
  ...
  le_contains->setText (parameters->count ("dad_nth_contains")
...
void
US_Hydrodyn_Dad_Nth::quit ()
{
  close ();
}

void
US_Hydrodyn_Dad_Nth::go ()
{
  (*parameters)["go"] = "true";
  ...
  (*parameters)["dad_nth_contains"] = le_contains->text ();

  close ();
}

void
US_Hydrodyn_Dad_Nth::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Dad_Nth::closeEvent (QCloseEvent *e)
{
...
void
US_Hydrodyn_Dad_Nth::update_enables ()
{
  int fcnt = ((US_Hydrodyn_Dad *)us_hydrodyn_dad)->lb_files->count ();
  int ofs = le_start->text ().toInt ();
  int end = le_end->text ().toInt ();
  if (ofs >= 1 && ofs <= fcnt)
    {
      lbl_start_name->setText (((US_Hydrodyn_Dad *)us_hydrodyn_dad)
                                   ->lb_files->item (ofs - 1)
                                   ->text ());
    }
  else
  ...
  if (end >= 1 && end <= fcnt)
    {
      lbl_end_name->setText (((US_Hydrodyn_Dad *)us_hydrodyn_dad)
                                 ->lb_files->item (end - 1)
                                 ->text ());
    }
  else
  ...
  for (int i = 0; i < lb_files->count (); ++i)
    {
      ...
      {
        QString qs = ((US_Hydrodyn_Dad *)us_hydrodyn_dad)
                         ->lb_files->item (i)
                         ->text ();
...
void
US_Hydrodyn_Dad_Nth::contains_only ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  lb_files->clearSelection ();
  for (int i = 0; i < lb_files->count (); ++i)
...
void
US_Hydrodyn_Dad_Nth::contains_add ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  for (int i = 0; i < lb_files->count (); ++i)
    {
...
void
US_Hydrodyn_Dad_Nth::pvalues_only ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  lb_files->clearSelection ();
  for (int i = 0; i < lb_files->count (); ++i)
...
void
US_Hydrodyn_Dad_Nth::pvalues_add ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  for (int i = 0; i < lb_files->count (); ++i)
    {
...
void
US_Hydrodyn_Dad_Nth::update_files_selected ()
{
  lb_files_sel->clear ();
  for (int i = 0; i < lb_files->count (); ++i)
    {
