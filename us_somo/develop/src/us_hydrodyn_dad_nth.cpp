                                                     ...
                                                     ) : QDialog( p )
{
   ...
   this->ggaussian_last_pfit_map              = &((US_Hydrodyn_Dad*)us_hydrodyn_dad)->ggaussian_last_pfit_map;
...
{
   bool started_in_expert_mode = ((US_Hydrodyn_Dad*)us_hydrodyn_dad)->started_in_expert_mode;
   ...
   for ( int i = 0; i < ((US_Hydrodyn_Dad*)us_hydrodyn_dad)->lb_files->count(); ++i )
   {
      lb_files->addItem( QString( "%1 : %2" ).arg( i + 1 ).arg( ((US_Hydrodyn_Dad*)us_hydrodyn_dad)->lb_files->item( i )->text() ) );
   ...
   {
      QIntValidator *qdv = new QIntValidator( 1, ((US_Hydrodyn_Dad*)us_hydrodyn_dad)->lb_files->count(), le_n );
   ...
   {
      QIntValidator *qdv = new QIntValidator( 1, ((US_Hydrodyn_Dad*)us_hydrodyn_dad)->lb_files->count(), le_start );
   ...
   lbl_start_name =  new QLabel      ( ( (US_Hydrodyn_Dad*)us_hydrodyn_dad)->lb_files->item( le_start->text().toInt() - 1 )->text(), this );
   ...
   le_end->setText( QString( "%1" ).arg( ((US_Hydrodyn_Dad*)us_hydrodyn_dad)->lb_files->count() ) );
   ...
   {
      QIntValidator *qdv = new QIntValidator( 2, ((US_Hydrodyn_Dad*)us_hydrodyn_dad)->lb_files->count(), le_end );
      le_end->setValidator( qdv );
   }
   connect( le_end, SIGNAL( textChanged( const QString & ) ), SLOT( update_enables() ) );
   le_end->setMinimumWidth( 80 );

   lbl_end_name =  new QLabel      ( ( (US_Hydrodyn_Dad*)us_hydrodyn_dad)->lb_files->item( le_end->text().toInt() - 1 )->text(), this );
   ...
   le_contains->setText( parameters->count( "dad_nth_contains" ) ? (*parameters)[ "dad_nth_contains" ] : "" );
...
void US_Hydrodyn_Dad_Nth::update_enables()
{
   ...
   if ( ofs >= 1 && ofs <= fcnt ) {
      lbl_start_name->setText( ( (US_Hydrodyn_Dad*)us_hydrodyn_dad)->lb_files->item( ofs- 1 )->text() );
   } else {
      lbl_start_name->setText( "" );
   }
   if ( end >= 1 && end <= fcnt ) {
      lbl_end_name->setText( ( (US_Hydrodyn_Dad*)us_hydrodyn_dad)->lb_files->item( end- 1 )->text() );
   } else {
   ...
   for ( int i = 0; i < lb_files->count(); ++i )
   {
      ...
      if ( ((US_Hydrodyn_Dad*)us_hydrodyn_dad)->lb_files->item( i )->text().contains( le_contains->text() ) )
      {
         contains.insert( i );
      }
      {
         QString qs = ((US_Hydrodyn_Dad*)us_hydrodyn_dad)->lb_files->item( i )->text();
