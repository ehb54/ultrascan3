...
static QRegExp rx_eb
    = QRegExp (QString ("%1%2").arg ("^").arg (UPU_EB_PREFIX));

void
US_Plot_Util::eb_handling (vector<QString> titles, vector<vector<double> > x,
                           ...
                           map<QString, vector<double> > &e)
{
  ...
  for (int i = 0; i < curves; i++)
    {
      if (titles[i].contains (rx_eb) && x[i].size () == 2 && y[i].size () == 2)
        {
  ...
  for (auto it = possible_x.begin (); it != possible_x.end (); ++it)
    {
      ...
      // ] );
      if (count_by_title.count (it->first) && count_by_title[it->first] == 1
...
bool
US_Plot_Util::printtofile (QString basename, map<QString, QwtPlot *> plots,
                           QString &errors, QString &messages)
{
  errors = "";
  messages = "";
      ...
      if (curves && max_rows)
        {
          ...
          if (QFileInfo (filename).exists ())
            {
              ...
              do
                {
                  filename = basename + plotname + QString ("-%1").arg (++ext)
                             + ".csv";
                }
          ...
          if (!f.open (QIODevice::WriteOnly))
            {
              errors = QString ("File %1 can not open for writing")
                           .arg (f.fileName ());
              return false;
            }
...
void
US_Plot_Util::plotinfo (QString name, QwtPlot *plot)
{
...
void
US_Plot_Util::align_plot_extents (const vector<QwtPlot *> &plots,
                                  bool scale_x_to_first)
{
  ...
  double max_extent = extents[0]
      = plots[0]
            ->axisWidget (QwtPlot::yLeft)
            ->scaleDraw ()
            ->extent (plots[0]->axisWidget (QwtPlot::yLeft)->font ());
      ...
      for (int i = 1; i < size; ++i)
        {
          extents[i]
              = plots[i]
                    ->axisWidget (QwtPlot::yLeft)
                    ->scaleDraw ()
                    ->extent (plots[i]->axisWidget (QwtPlot::yLeft)->font ());
...
void
US_Plot_Util::rescale (map<QString, QwtPlot *> plots,
                       map<QwtPlot *, ScrollZoomer *> plot_to_zoomer,
                       bool only_scale_y, bool rescale_rounding)
{
  map<QwtPlot *, double> plot_limit_x_range_min;
  map<QwtPlot *, double> plot_limit_x_range_max;
  rescale (plots, plot_to_zoomer, plot_limit_x_range_min,
           plot_limit_x_range_max, only_scale_y, rescale_rounding);
}
                       ...
                       bool only_scale_y, bool rescale_rounding)
{
  ...
  for (map<QString, QwtPlot *>::iterator it = plots.begin ();
       it != plots.end (); ++it)
    {
      if (!plot_to_zoomer.count (it->second) || !plot_to_zoomer[it->second])
        {
      ...
      else
        {
          ...
          if (plot_limit_x_range_min.count (it->second)
            ...
            {
              rescale (it->second, plot_to_zoomer[it->second],
                       plot_limit_x_range_min[it->second],
                       plot_limit_x_range_max[it->second], only_scale_y,
                       rescale_rounding);
            }
          else
            {
...
double
US_Plot_Util::round_digits (double value, int digits)
{
  if (value
      == 0.0) // otherwise it will return 'nan' due to the log10() of zero
    return 0.0;
...
void
US_Plot_Util::rescale (const vector<QwtPlot *> &plots,
                       map<QwtPlot *, ScrollZoomer *> plot_to_zoomer,
                       bool only_scale_y, bool rescale_rounding)
{
  map<QwtPlot *, double> plot_limit_x_range_min;
      ...
      if (!plot_to_zoomer.count (plots[i]))
      ...
      else
        {
          if (plot_limit_x_range_min.count (plots[i])
            ...
            {
              rescale (plots[i], plot_to_zoomer[plots[i]],
                       plot_limit_x_range_min[plots[i]],
                       plot_limit_x_range_max[plots[i]], only_scale_y,
                       rescale_rounding);
            }
          else
            {
...
void
US_Plot_Util::rescale (QwtPlot *plot, ScrollZoomer *zoomer, bool only_scale_y,
                       bool rescale_rounding)
{
  rescale (plot, zoomer, numeric_limits<double>::min (),
           numeric_limits<double>::max (), only_scale_y, rescale_rounding);
}

void
US_Plot_Util::rescale (QwtPlot *plot, ScrollZoomer *zoomer,
                       double limit_x_range_min, double limit_x_range_max,
                       bool only_scale_y, bool rescale_rounding)
{
#if defined(DEBUG_RESCALE)
...
#if defined(DEBUG_RESCALE_INTERNAL)
      qDebug ()
          << "Internal error: US_Plot_Util::rescale( *, false ) not supported";
#endif
      return;
    }
  if (!zoomer)
...
#if defined(DEBUG_RESCALE_INTERNAL)
      qDebug ()
          << "internal error: missing plot_to_zoomer for plot in rescale\n";
#endif
      return;
    }
  ...
  qDebug () << "rescale: x limits: [ " << limit_x_range_min << " , "
            << limit_x_range_max << " ]";
#endif
  ...
  tso << QString ().sprintf (
      ...
      ,
      min_x, max_x, min_x_alt, max_x_alt);
#endif
          ...
          for (int i = 0; i < (int)curve->dataSize (); ++i)
            {
              ...
              if (x >= min_x && x <= max_x)
                {
#if defined(DEBUG_RESCALE_CURVES)
                  tso << "point " << i << " x,y: " << curve->sample (i).x ()
                      << " , " << curve->sample (i).y () << "\n";
#endif
                  y = curve->sample (i).y ();
                  if (min_y > y)
  ...
  tso << QString ().sprintf (
      ...
      zoomer->zoomRect ().topLeft ().x (), zoomer->zoomRect ().topLeft ().y (),
...
#endif
      double smax_y
          = fabs (max_y) > fabs (min_y) ? fabs (max_y) : fabs (min_y);
      ...
      if (rescale_rounding)
      ...
      else
        {
          zoomer->zoom (QRectF (
              ...
              QPointF (zoomer->zoomRect ().bottomRight ().x (),
                       -smax_y * 1.2e0)));
        }
    }
  else
