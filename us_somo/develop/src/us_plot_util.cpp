...
void US_Plot_Util::eb_handling(vector<QString> titles,
...
bool US_Plot_Util::printtofile(QString basename, map<QString, QwtPlot *> plots,
  ...
  for (map<QString, QwtPlot *>::iterator it = plots.begin(); it != plots.end();
       ++it) {
      ...
      if (skipx && (int)skipx_vals.size() != curves - 1) {
        ...
        for (int i = 0; i < curves; ++i) {
          ...
          for (int j = 0; j < (int)new_x[i].size(); ++j) {
            this_y.push_back(
                y_vals[i].count(new_x[i][j]) ? y_vals[i][new_x[i][j]] :
#if defined(NAN)
                                             NAN
#else
                                             sqrt(-1e0)
...
void US_Plot_Util::plotinfo(QString name, QwtPlot *plot) {
...
void US_Plot_Util::align_plot_extents(const vector<QwtPlot *> &plots,
...
void US_Plot_Util::rescale(map<QString, QwtPlot *> plots,
  ...
  rescale(plots, plot_to_zoomer, plot_limit_x_range_min, plot_limit_x_range_max,
          only_scale_y, rescale_rounding);
}
...
void US_Plot_Util::rescale(QwtPlot *plot, ScrollZoomer *zoomer,
                           bool only_scale_y, bool rescale_rounding) {
...
void US_Plot_Util::rescale(QwtPlot *plot, ScrollZoomer *zoomer,
  ...
  tso << QString().sprintf(
      ...
      ,
      zoomer->zoomRect().bottomLeft().x(), zoomer->zoomRect().bottomLeft().y(),
      zoomer->zoomRect().bottomRight().x(),
      zoomer->zoomRect().bottomRight().y(), zoomer->zoomRect().topLeft().x(),
      zoomer->zoomRect().topLeft().y(), zoomer->zoomRect().topRight().x(),
  ...
  if (zoomer->symmetric_rescale) {
    ...
    if (rescale_rounding) {
      ...
      smax_y = new_smax_y;
      zoomer->zoom(
          QRectF(QPointF(zoomer->zoomRect().topLeft().x(), smax_y),
    ...
    if (rescale_rounding) {
      ...
      min_y = new_min_y;
      zoomer->zoom(
          QRectF(QPointF(zoomer->zoomRect().topLeft().x(), max_y),
                 QPointF(zoomer->zoomRect().bottomRight().x(), min_y)));
    } else {
      zoomer->zoom(
          QRectF(QPointF(zoomer->zoomRect().topLeft().x(), max_y * 1.1e0),
