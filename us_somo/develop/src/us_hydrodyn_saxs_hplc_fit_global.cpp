...
US_Hydrodyn_Saxs_Hplc_Fit_Global::US_Hydrodyn_Saxs_Hplc_Fit_Global (
    US_Hydrodyn_Saxs_Hplc *hplc_win, QWidget *p, const char *)
    : QDialog (p)
{
  ...
  hplc_ampl_width_min = ((US_Hydrodyn *)(hplc_win->us_hydrodyn))
                            ->gparams["hplc_ampl_width_min"]
                            .toDouble ();
  hplc_lock_min_retry = ((US_Hydrodyn *)(hplc_win->us_hydrodyn))
                                    ->gparams["hplc_lock_min_retry"]
                                == "true"
                            ? true
                            : false;
  hplc_lock_min_retry_mult = ((US_Hydrodyn *)(hplc_win->us_hydrodyn))
                                 ->gparams["hplc_lock_min_retry_mult"]
                                 .toDouble ();
  hplc_maxfpk_restart = ((US_Hydrodyn *)(hplc_win->us_hydrodyn))
                                    ->gparams["hplc_maxfpk_restart"]
                                == "true"
                            ? true
                            : false;
  hplc_maxfpk_restart_tries = ((US_Hydrodyn *)(hplc_win->us_hydrodyn))
                                  ->gparams["hplc_maxfpk_restart_tries"]
                                  .toUInt ();
  hplc_maxfpk_restart_pct = ((US_Hydrodyn *)(hplc_win->us_hydrodyn))
                                ->gparams["hplc_maxfpk_restart_pct"]
                                .toDouble ();
  ...
  if (use_errors
      && (hplc_win->unified_ggaussian_e.size ()
              != hplc_win->unified_ggaussian_q.size ()
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::restore ()
{
  ...
  if (update_hplc)
    {
      ...
      hplc_win->ggqfit_plot->detachItems (QwtPlotItem::Rtti_PlotCurve);
      hplc_win->ggqfit_plot->detachItems (QwtPlotItem::Rtti_PlotMarker);
      ;
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::undo ()
{
      ...
      for (int i = 0; i < (int)HFIT_GLOBAL::init_params.size (); ++i)
        {
          ((QLineEdit *)test_widgets[i])
              ->setText (QString ("%1").arg (HFIT_GLOBAL::init_params[i]));
        }
      update_test_info ();
    }
}
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::update_test_info ()
{
  QString qs_uggs = "";

  {
    ...
    for (unsigned int i = 0; i < hplc_win->unified_ggaussian_gaussians_size;
         ++i)
      {
        ...
        for (unsigned int j = 0; j < hplc_win->common_size; ++j)
          {
            qs_uggs += QString ("%1 ").arg (
                hplc_win->unified_ggaussian_params[p++], 0, 'g', 6);
          }
        qs_uggs += "\n";
      }
    ...
    for (unsigned int i = 0; i < hplc_win->unified_ggaussian_curves; ++i)
      {
        ...
        for (unsigned int k = 0;
             k < hplc_win->unified_ggaussian_gaussians_size; ++k)
          {
  ...
  lbl_test_info->setText (
      QString ("Per gaussian: common_size: %1 %2 per_file_size: %3 %4\n"
          ...
          .arg (US_Vector::qs_vector (
              "unified ggaussians", hplc_win->unified_ggaussian_params, 6, 20))
          .arg (qs_uggs)
          .arg (HFIT_GLOBAL::qs_params ()));
}

void
US_Hydrodyn_Saxs_Hplc_Fit_Global::setupGUI ()
{
  int minHeight1 = 30;

  lbl_title
      = new QLabel (us_tr ("US-SOMO: SAXS Hplc: Global Gaussian Fit"), this);
  ...
  /* old way
     ...
     hplc_win->gaussians.size() / 3 - 1 ); cout << "qs_rx:" << qs_rx << endl;
  ...
  */
  ...
  for (unsigned int i = 0;
       i < (unsigned int)hplc_win->gaussians.size () / gaussian_type_size; i++)
    {
      QCheckBox *cb_tmp;
  ...
  double peak = hplc_win->compute_gaussian_peak (hplc_win->wheel_file,
                                                 hplc_win->gaussians);
  le_epsilon->setText (
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::cancel ()
{
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept ();
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::update_common ()
{
  if (!setup_run ())
    {
      update_enables ();
  ...
  for (unsigned int i = 0; i < HFIT_GLOBAL::param_fixed.size (); i++)
    {
      if (!HFIT_GLOBAL::param_fixed[i] || HFIT_GLOBAL::comm_backref.count (i))
        {
          hplc_win->unified_ggaussian_params[i]
              = HFIT_GLOBAL::init_params[HFIT_GLOBAL::comm_backref.count (i)
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::update_enables ()
{
...
bool
US_Hydrodyn_Saxs_Hplc_Fit_Global::setup_run ()
{
  ...
  HFIT_GLOBAL::unified_ggaussian_param_index
      = hplc_win->unified_ggaussian_param_index;
  HFIT_GLOBAL::unified_ggaussian_gaussians_size
      = hplc_win->unified_ggaussian_gaussians_size;
  ...
  switch (gaussian_type)
    ...
    case US_Hydrodyn_Saxs_Hplc::EMGGMG:
      HFIT_GLOBAL::compute_gaussian_f
          = &HFIT_GLOBAL::compute_gaussian_f_EMGGMG;
      // gsm_f = &gsm_f_EMGGMG;
      break;
    case US_Hydrodyn_Saxs_Hplc::EMG:
  ...
  double dist = ((US_Hydrodyn *)(hplc_win->us_hydrodyn))
                    ->gparams["hplc_dist_max"]
                    .toDouble ();
  ...
  // the hplc_win unified_ggaussians
  ...
  for (unsigned int i = 0; i < hplc_win->unified_ggaussian_gaussians_size; i++)
    {
      ...
      if (cb_fix_center->isChecked () || fixed_curves.count (i))
        {
          ...
          HFIT_GLOBAL::fixed_params.push_back (
              hplc_win->unified_ggaussian_params[ofs]);
      ...
      else
        {
          ...
          HFIT_GLOBAL::base_params.push_back (
              hplc_win->unified_ggaussian_params[ofs]);
      ...
      if (hplc_win->cb_fix_width->isChecked ()) // if widths are common
        {
          if (cb_fix_width->isChecked () || fixed_curves.count (i))
            {
              HFIT_GLOBAL::param_pos.push_back (
                  HFIT_GLOBAL::fixed_params.size ());
              HFIT_GLOBAL::fixed_params.push_back (
                  hplc_win->unified_ggaussian_params[ofs]);
              HFIT_GLOBAL::param_fixed.push_back (true);
            }
      ...
      if (dist1_active
          && hplc_win->cb_fix_dist1->isChecked ()) // if dist1s are common
        {
          if (cb_fix_dist1->isChecked () || fixed_curves.count (i)
              || (cb_comm_dist1->isChecked () && i))
            {
              ...
              if (i && cb_comm_dist1->isChecked () && !fixed_curves.count (i)
                ...
                {
                  HFIT_GLOBAL::fixed_params.push_back (
                      hplc_win->unified_ggaussian_params[this_ofs]);
                  HFIT_GLOBAL::comm_backref[HFIT_GLOBAL::param_fixed.size ()]
                      = comm_dist_1_var_pos;
                }
              else
                {
          ...
          if (dist2_active
              && hplc_win->cb_fix_dist2->isChecked ()) // if dist2s are common
            {
                  ...
                  || (cb_comm_dist2->isChecked () && i))
                {
                  ...
                  if (i && cb_comm_dist2->isChecked ()
                      && !fixed_curves.count (i) && !fixed_curves.count (0))
                    {
                      HFIT_GLOBAL::fixed_params.push_back (
                          hplc_win->unified_ggaussian_params[this_ofs]);
                      HFIT_GLOBAL::comm_backref[HFIT_GLOBAL::param_fixed
                                                    .size ()]
                          = comm_dist_2_var_pos;
                    }
                  else
  ...
  unsigned int base_ofs
      = hplc_win->unified_ggaussian_gaussians_size * common_size;
  // assert( (unsigned int)HFIT_GLOBAL::param_pos.size() == base_ofs &&
  ...
  for (unsigned int f = 0; f < hplc_win->unified_ggaussian_curves; f++)
    {
      unsigned int ofs
          = base_ofs
            + f * per_file_size * hplc_win->unified_ggaussian_gaussians_size;
  ...
  i++ )
     {
        ...
        {
           HFIT_GLOBAL::param_pos   .push_back(
  HFIT_GLOBAL::fixed_params.size() ); HFIT_GLOBAL::fixed_params.push_back(
  hplc_win->unified_ggaussian_params[ 2 * i + 0 ] ); HFIT_GLOBAL::param_fixed
  .push_back( true ); } else { HFIT_GLOBAL::param_pos   .push_back(
  HFIT_GLOBAL::init_params.size() );
  ...
  .push_back( false );
             ...
             fixed_curves.count( i ) )
        {
           HFIT_GLOBAL::param_pos   .push_back(
  HFIT_GLOBAL::fixed_params.size() ); HFIT_GLOBAL::fixed_params.push_back(
  hplc_win->unified_ggaussian_params[ 2 * i + 1 ] ); HFIT_GLOBAL::param_fixed
  .push_back( true ); } else { HFIT_GLOBAL::param_pos   .push_back(
  HFIT_GLOBAL::init_params.size() );
  ...
  .push_back( false );
     ...
     for ( unsigned int i = 0; i < hplc_win->unified_ggaussian_gaussians_size *
  hplc_win->unified_ggaussian_curves; i++ )
     {
        unsigned int pos = i / hplc_win->unified_ggaussian_curves;

        unsigned int height_pos = 2 *
  hplc_win->unified_ggaussian_gaussians_size + i + 0;
             ...
             fixed_curves.count( pos ) )
        {
           HFIT_GLOBAL::param_pos   .push_back(
  HFIT_GLOBAL::fixed_params.size() ); HFIT_GLOBAL::fixed_params.push_back(
  hplc_win->unified_ggaussian_params[ height_pos ] ); HFIT_GLOBAL::param_fixed
  .push_back( true ); } else { HFIT_GLOBAL::param_pos   .push_back(
  HFIT_GLOBAL::init_params.size() );
  ...
  i++ )
     {
             ...
             fixed_curves.count( i ) )
        {
           HFIT_GLOBAL::param_pos   .push_back(
  HFIT_GLOBAL::fixed_params.size() ); HFIT_GLOBAL::fixed_params.push_back(
  hplc_win->unified_ggaussian_params[ i ] ); HFIT_GLOBAL::param_fixed
  .push_back( true ); } else { HFIT_GLOBAL::param_pos   .push_back(
  HFIT_GLOBAL::init_params.size() );
  ...
  .push_back( false );
     ...
     for ( unsigned int i = 0; i < hplc_win->unified_ggaussian_gaussians_size *
  2 * hplc_win->unified_ggaussian_curves; i+= 2 )
     {
        unsigned int pos = i / ( 2 * hplc_win->unified_ggaussian_curves );

        unsigned int height_pos = hplc_win->unified_ggaussian_gaussians_size +
  i + 0; unsigned int width_pos  = hplc_win->unified_ggaussian_gaussians_size +
  i + 1;
  ...
  .push_back( false );
             ...
             fixed_curves.count( pos + 1 ) )
        {
           HFIT_GLOBAL::param_pos   .push_back(
  HFIT_GLOBAL::fixed_params.size() ); HFIT_GLOBAL::fixed_params.push_back(
  hplc_win->unified_ggaussian_params[ width_pos ] ); HFIT_GLOBAL::param_fixed
  .push_back( true ); } else { HFIT_GLOBAL::param_pos   .push_back(
  HFIT_GLOBAL::init_params.size() );
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::test_mode ()
{
  if (!cb_test_mode->isChecked ())
    {
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::test ()
{
      ...
      if (!HFIT_GLOBAL::param_fixed[i] || HFIT_GLOBAL::comm_backref.count (i))
        {
          ...
          us_qdebug (QString ("param %1 taken from init params pos %2 %3")
                         .arg (i)
                         .arg (HFIT_GLOBAL::comm_backref.count (i)
                                   ? HFIT_GLOBAL::comm_backref[i]
                                   : HFIT_GLOBAL::param_pos[i])
                         .arg (HFIT_GLOBAL::comm_backref.count (i)
                                   ? "backref used"
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::lm ()
{
  update_common ();
  running = true;
  update_enables ();
  ...
  hplc_win->unified_ggaussian_params;
  ...
  for ( unsigned int i = 0; i < HFIT_GLOBAL::param_fixed.size(); i++ )
  {
          ...
          HFIT_GLOBAL::comm_backref.count( i ) )
     {
        hplc_win->unified_ggaussian_params[ i ] = HFIT_GLOBAL::init_params[
                                                                           HFIT_GLOBAL::comm_backref.count(
  i ) ? HFIT_GLOBAL::comm_backref[ i ] : HFIT_GLOBAL::param_pos[ i ]
                                                                            ];
     }
  }
  ...
  */
  ...
  for (unsigned int i = 0; i < (unsigned int)t.size (); i++)
    {
      gsumf[i] = (*HFIT_GLOBAL::compute_gaussian_f) (
          t[i], &HFIT_GLOBAL::init_params[0]);
    }
  ...
  if (org_rmsd > status.fnorm)
    {
      for (unsigned int i = 0; i < HFIT_GLOBAL::param_fixed.size (); i++)
        {
          if (!HFIT_GLOBAL::param_fixed[i]
            ...
            {
              hplc_win->unified_ggaussian_params[i]
                  = par[HFIT_GLOBAL::comm_backref.count (i)
                            ? HFIT_GLOBAL::comm_backref[i]
                            : HFIT_GLOBAL::param_pos[i]];
            }
        }
      ...
      // hplc_win->add_ggaussian_curve( "lm_after_pushback",
      // hplc_win->compute_ggaussian_gaussian_sum() );
      gaussians_undo.push_back (hplc_win->unified_ggaussian_params);
      if (update_hplc)
        {
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::gsm_sd ()
{
  ...
  {
     gsm_yp[ j ]  = HFIT_GLOBAL::compute_gaussian_f( gsm_t[ j ], (double
  *)(&HFIT_GLOBAL::base_params[ 0 ]) ); org_rmsd += ( gsm_y[ j ] - gsm_yp[ j ]
  ) * ( gsm_y[ j ] - gsm_yp[ j ] );
  }
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::gsm_ih ()
{
  return; // have to rewrite this
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::gsm_cg ()
{
  return; // have to rewrite this
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::ga ()
{
  /*
  gsm_setup();
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::stop ()
{
  running = false;
}

void
US_Hydrodyn_Saxs_Hplc_Fit_Global::grid ()
{
  return; // have to rewrite this
  setup_run ();
  ...
  unsigned int total_count = (unsigned int)pow (
      (double)pop_size, (int)HFIT_GLOBAL::init_params.size ());
  ...
  for (unsigned int j = 0; j < (unsigned int)HFIT_GLOBAL::init_params.size ();
       j++)
    {
      ofs_per[j] = (HFIT_GLOBAL::param_max[j] - HFIT_GLOBAL::param_min[j])
                   / (pop_size - 1);
    }
  ...
  for (unsigned int j = 0; j < t.size (); j++)
    {
      yp[j] = HFIT_GLOBAL::compute_gaussian_f (
          t[j], (double *)(&HFIT_GLOBAL::base_params[0]));
  ...
  for (unsigned int i = 0; i < total_count; i++)
    {
      ...
      for (unsigned int j = 0;
           j < (unsigned int)HFIT_GLOBAL::init_params.size (); j++)
        {
      ...
      for (unsigned int j = 0; j < t.size (); j++)
        {
          yp[j] = HFIT_GLOBAL::compute_gaussian_f (t[j],
                                                   (double *)(&use_par[0]));
...
void
US_Hydrodyn_Saxs_Hplc_Fit_Global::usp_config_plot_test (const QPoint &)
{
