...
US_Hydrodyn_Saxs_Hplc_Fit_Global::US_Hydrodyn_Saxs_Hplc_Fit_Global(
  ...
  hplc_lock_min_retry = ((US_Hydrodyn *)(hplc_win->us_hydrodyn))
                                    ->gparams["hplc_lock_min_retry"] == "true"
                            ? true
                            : false;
  ...
  hplc_maxfpk_restart = ((US_Hydrodyn *)(hplc_win->us_hydrodyn))
                                    ->gparams["hplc_maxfpk_restart"] == "true"
                            ? true
                            : false;
...

  {
    ...
    for (unsigned int i = 0; i < hplc_win->unified_ggaussian_gaussians_size;
      ...
      for (unsigned int j = 0; j < hplc_win->common_size; ++j) {
        qs_uggs += QString("%1 ").arg(hplc_win->unified_ggaussian_params[p++],
                                      0, 'g', 6);
      }
      qs_uggs += "\n";
    }
    ...
    for (unsigned int i = 0; i < hplc_win->unified_ggaussian_curves; ++i) {
      ...
      for (unsigned int k = 0; k < hplc_win->unified_ggaussian_gaussians_size;
           ++k) {
  ...
  lbl_test_info->setText(
      QString("Per gaussian: common_size: %1 %2 per_file_size: %3 %4\n"
          ...
          .arg(US_Vector::qs_vector("unified ggaussians",
                                    hplc_win->unified_ggaussian_params, 6, 20))
          .arg(qs_uggs)
...
bool US_Hydrodyn_Saxs_Hplc_Fit_Global::setup_run() {
  ...
  // these are a copy of the init_params without replacement from the hplc_win
  // unified_ggaussians
  ...
  for (unsigned int f = 0; f < hplc_win->unified_ggaussian_curves; f++) {
    ...
    for (unsigned int i = 0; i < hplc_win->unified_ggaussian_gaussians_size;
      ...
      if (dist1_active &&
          !hplc_win->cb_fix_dist1->isChecked())  // if dist1s are not common
      {
        if (cb_fix_dist1->isChecked() || fixed_curves.count(i) ||
            (cb_comm_dist1->isChecked() && i)) {
          HFIT_GLOBAL::param_pos.push_back(HFIT_GLOBAL::fixed_params.size());
  ...
  i++ )
     {
        ...
        {
           HFIT_GLOBAL::param_pos   .push_back( HFIT_GLOBAL::fixed_params.size()
  ); HFIT_GLOBAL::fixed_params.push_back( hplc_win->unified_ggaussian_params[ 2
  * i + 0 ] ); HFIT_GLOBAL::param_fixed .push_back( true ); } else {
           HFIT_GLOBAL::param_pos   .push_back( HFIT_GLOBAL::init_params.size()
  );
  ...
  * i + 1 ] ); HFIT_GLOBAL::param_fixed .push_back( true ); } else {
           HFIT_GLOBAL::param_pos   .push_back( HFIT_GLOBAL::init_params.size()
  );
  ...
  hplc_win->unified_ggaussian_curves; i++ )
     {
        ...
        unsigned int height_pos = 2 * hplc_win->unified_ggaussian_gaussians_size
  + i + 0;
        ...
        {
           HFIT_GLOBAL::param_pos   .push_back( HFIT_GLOBAL::fixed_params.size()
  ); HFIT_GLOBAL::fixed_params.push_back( hplc_win->unified_ggaussian_params[
  height_pos ] ); HFIT_GLOBAL::param_fixed .push_back( true ); } else {
           HFIT_GLOBAL::param_pos   .push_back( HFIT_GLOBAL::init_params.size()
  );
  ...
  ] ); HFIT_GLOBAL::param_fixed .push_back( true ); } else {
           HFIT_GLOBAL::param_pos   .push_back( HFIT_GLOBAL::init_params.size()
  );
  ...
  2 * hplc_win->unified_ggaussian_curves; i+= 2 )
     {
        ...
        unsigned int height_pos = hplc_win->unified_ggaussian_gaussians_size + i
  + 0; unsigned int width_pos  = hplc_win->unified_ggaussian_gaussians_size + i
  + 1;
  ...
  width_pos ] ); HFIT_GLOBAL::param_fixed .push_back( true ); } else {
           HFIT_GLOBAL::param_pos   .push_back( HFIT_GLOBAL::init_params.size()
  );
    ...
    if (!HFIT_GLOBAL::param_fixed[i] || HFIT_GLOBAL::comm_backref.count(i)) {
      ...
      us_qdebug(QString("param %1 taken from init params pos %2 %3")
                    ...
                    .arg(HFIT_GLOBAL::comm_backref.count(i)
                             ? HFIT_GLOBAL::comm_backref[i]
                             : HFIT_GLOBAL::param_pos[i])
                    .arg(HFIT_GLOBAL::comm_backref.count(i) ? "backref used"
                                                            : "direct pos"));
    } else {
...
void US_Hydrodyn_Saxs_Hplc_Fit_Global::lm() {
  ...
  */
  ...
  for (unsigned int i = 0; i < (unsigned int)t.size(); i++) {
    gsumf[i] =
        (*HFIT_GLOBAL::compute_gaussian_f)(t[i], &HFIT_GLOBAL::init_params[0]);
  }
...
void US_Hydrodyn_Saxs_Hplc_Fit_Global::stop() { running = false; }
...
void US_Hydrodyn_Saxs_Hplc_Fit_Global::grid() {
  ...
  unsigned int total_count =
      (unsigned int)pow((double)pop_size, (int)HFIT_GLOBAL::init_params.size());
