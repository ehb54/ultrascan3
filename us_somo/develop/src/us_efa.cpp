...
bool
US_Efa::Efa (vector<vector<double> > &A, vector<vector<double> > &result,
             bool rightEfa)
{
  qDebug () << "US_Efa::Efa";
  errors = "";
  ...
  for (int n_use = 0; n_use < n; ++n_use)
    {
...
#if defined(DO_PRINTOUT)
      {
        ...
        ts << (rightEfa ? n - n_use - 1 : n_use) << " " << rpt[0] << " "
...
bool
US_Efa::Efa_t (unsigned int numThreads, vector<vector<double> > &A,
...
void
efa_thr_t::efa_thr_setup (vector<double **> &a_t, int m, vector<int> &n_t,
      ...
      for (unsigned int i = thread; i < jobs; i += threads)
        {
          ...
          // ); SVD::dvalidate( (*a_t)[ i ], m, (*n_t)[ i ], (*w_t)[ i ],
          // (*v_t)[ i ] );
