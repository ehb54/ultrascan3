...
US_Hydrodyn_Saxs_Hplc::pm ()
{
  ...
  if (!any_selected)
    {
      editor_msg ("red",
                  us_tr ("Internal error: no files selected in pm mode"));
      return;
    }
  ...
  if (f_qs[wheel_file].size () < 2)
    {
      editor_msg ("red",
                  QString (us_tr ("Internal error: %1 almost empty data"))
                      .arg (wheel_file));
      return;
    }
  ...
  if (le_pm_q_start->text ().isEmpty () || le_pm_q_start->text () == "0"
  ...
  if (le_pm_q_end->text ().isEmpty () || le_pm_q_end->text () == "0"
  ...
  gauss_add_marker (le_pm_q_start->text ().toDouble (), Qt::red,
                    us_tr ("Start"));
  gauss_add_marker (le_pm_q_end->text ().toDouble (), Qt::red, us_tr ("End"),
                    Qt::AlignLeft | Qt::AlignTop);
  plot_dist->replot ();
}

void
US_Hydrodyn_Saxs_Hplc::pm_enables ()
{
  bool shapes_selected = false;
#if QT_VERSION >= 0x040000
  for (QList<QAbstractButton *>::iterator it
       = bg_pm_shape->buttons ().begin ();
       it != bg_pm_shape->buttons ().end (); ++it)
    {
...
US_Hydrodyn_Saxs_Hplc::pm_run ()
{
  ...
  if (cb_pm_sd->isChecked ())
    {
      for (int i = 0; i < (int)f_qs[wheel_file].size (); ++i)
        {
          if (f_qs[wheel_file][i] >= q_min && f_qs[wheel_file][i] <= q_max)
            {
              if (!(pts_cnt++ % pts))
                {
  ...
  us_qdebug ("pm_run 1");
  editor_msg ("blue",
              QString (us_tr ("PM: actual q points used %1 q range [%2:%3]"))
                  .arg (use_q.size ())
                  .arg (use_q[0])
                  .arg (use_q.back ()));
  ...
  if (!usu.run_pm (produced_q, produced_I, produced_model, run_params,
                   run_vectors))
    {
...
void
US_Hydrodyn_Saxs_Hplc::pm_q_reset ()
{
  le_pm_q_start->setText (QString ("%1").arg (f_qs[wheel_file][0]));
  le_pm_q_end->setText (QString ("%1").arg (f_qs[wheel_file].back ()));
}
...
void
US_Hydrodyn_Saxs_Hplc::pm_grid_size_text (const QString &)
{
}

void
US_Hydrodyn_Saxs_Hplc::pm_buff_e_dens_text (const QString &)
{
}

void
US_Hydrodyn_Saxs_Hplc::pm_samp_e_dens_text (const QString &)
{
}

void
US_Hydrodyn_Saxs_Hplc::pm_q_pts_text (const QString &)
{
}

void
US_Hydrodyn_Saxs_Hplc::pm_q_start_text (const QString &text)
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Saxs_Hplc::pm_q_end_text (const QString &text)
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Saxs_Hplc::pm_q_start_focus (bool hasFocus)
{
  if (hasFocus)
    {
...
void
US_Hydrodyn_Saxs_Hplc::pm_q_end_focus (bool hasFocus)
{
  if (hasFocus)
    {
...
void
US_Hydrodyn_Saxs_Hplc::rgc_enables ()
{
...
void
US_Hydrodyn_Saxs_Hplc::rgc_shape ()
{
  if (rb_rgc_shape_ellipsoid->isChecked ())
    {
...
void
US_Hydrodyn_Saxs_Hplc::rgc_calc_rg ()
{
  ...
  le_rgc_extents->setText (QString ("%1 : %2 : %3")
                               .arg (extent_a * 1e8)
                               .arg (extent_b * 1e8)
                               .arg (extent_c * 1e8));
...
void
US_Hydrodyn_Saxs_Hplc::rgc_mw_text (const QString &)
{
  rgc_calc_rg ();
}

void
US_Hydrodyn_Saxs_Hplc::rgc_vol_text (const QString &)
{
...
void
US_Hydrodyn_Saxs_Hplc::rgc_rho_text (const QString &)
{
  rgc_calc_rg ();
}

void
US_Hydrodyn_Saxs_Hplc::rgc_axis_b_text (const QString &, bool do_recompute)
{
...
void
US_Hydrodyn_Saxs_Hplc::rgc_axis_c_text (const QString &, bool do_recompute)
{
  if (le_rgc_axis_b->text ().toDouble () < le_rgc_axis_c->text ().toDouble ())
    {
...
void
US_Hydrodyn_Saxs_Hplc::rgc_rg_text (const QString &)
{
}
...
public:
  ...
  QString index;
  bool
  operator< (const testiq_sortable_double &objIn) const
  {
    return x < objIn.x;
  }
};
...
US_Hydrodyn_Saxs_Hplc::testiq ()
{
      ...
      if (lb_files->item (i)->isSelected ())
        {
          ...
          if (!plotted_curves.count (lb_files->item (i)->text ()))
            {
              editor_msg ("red",
                          QString (us_tr ("Internal error: testiq selected "
                                          "%1, but no plotted curve found"))
  ...
  if (!any_selected)
    {
      editor_msg ("red",
                  us_tr ("Internal error: no files selected in testiq mode"));
      return;
    }
  ...
  if (le_testiq_q_start->text ().isEmpty ()
    ...
    {
      disconnect (le_testiq_q_start, SIGNAL (textChanged (const QString &)), 0,
                  0);
  ...
  if (le_testiq_q_end->text ().isEmpty () || le_testiq_q_end->text () == "0"
      || le_testiq_q_end->text ().toDouble () > f_qs[wheel_file].back ())
    {
      disconnect (le_testiq_q_end, SIGNAL (textChanged (const QString &)), 0,
                  0);
  ...
  disable_all ();
  // plotted_markers.clear( );
  if (current_mode == MODE_GUINIER || current_mode == MODE_SCALE)
    {
  ...
  else
    {
      ...
      // figure out which testiq_it to select
      {
        ...
        // us_qdebug( QString( "testiq thresh %1 max_i %2 avg_i %3" ).arg(
        // thresh ).arg( max_i ).arg( avg_i ) );
        ...
        for (list<testiq_sortable_double>::iterator it = tsdl.begin ();
             it != tsdl.end (); ++it)
          {
            // us_qdebug( QString( "it loop x %1 index %2 Imin %3 Imax %4"
            // ).arg( it->x ).arg( it->index ).arg( it->Imin ).arg( it->Imax )
            // );
            if (it->x < thresh)
              {
  ...
  // us_qdebug( QString( "testitsel %1 min %2 max %3" ).arg( testiq_it_selected
  // ).arg( testiq_it_selected_Imin ).arg( testiq_it_selected_Imax ) );

  mode_select (MODE_TESTIQ);

  gauss_add_marker (le_testiq_q_start->text ().toDouble (), Qt::red,
                    us_tr ("Start"));
  gauss_add_marker (le_testiq_q_end->text ().toDouble (), Qt::red,
                    us_tr ("End"), Qt::AlignLeft | Qt::AlignTop);

  if (!unified_ggaussian_ok || !f_gaussians.count (wheel_file)
  ...
  else
    {
      if (testiq_active)
      ...
      else
        {
          ...
          for (int i = 0; i < (int)f_gaussians[wheel_file].size ();
               i += gaussian_type_size)
            {
              QRadioButton *rb = new QRadioButton (this);
              rb->setPalette (PALET_NORMAL);
              AUTFBACK (rb);
              rb->setFont (QFont (USglobal->config_list.fontFamily,
...
void
US_Hydrodyn_Saxs_Hplc::testiq_gauss_line ()
{
  int line_width = use_line_width < 3 ? (use_line_width + 1) : use_line_width;

  for (int i = 0; i < (int)rb_testiq_gaussians.size (); ++i)
    {
      QPen use_pen = QPen (rb_testiq_gaussians[i]->isChecked () ? Qt::magenta
                                                                : Qt::blue,
                           line_width, Qt::DashDotDotLine);
...
void
US_Hydrodyn_Saxs_Hplc::testiq_enables ()
{
  pb_timeshift->setEnabled (false);
  pb_wheel_cancel->setEnabled (true);
  ...
  pb_testiq_testset->setEnabled (le_testiq_q_start->text ().toDouble ()
                                     + .9999e0
                                 <= le_testiq_q_end->text ().toDouble ());
  pb_guinier->setEnabled (le_testiq_q_start->text ().toDouble () + .9999e0
                          <= le_testiq_q_end->text ().toDouble ());
  pb_scale->setEnabled (le_testiq_q_start->text ().toDouble () + .9999e0
                        <= le_testiq_q_end->text ().toDouble ());
...
bool
US_Hydrodyn_Saxs_Hplc::testiq_make ()
{
  ...
  if (cb_testiq_from_gaussian->isVisible ()
      && !rb_testiq_from_i_t->isChecked ())
    {
      return create_i_of_q (testiq_selected,
                            le_testiq_q_start->text ().toDouble (),
                            le_testiq_q_end->text ().toDouble ());
    }
  else
    {
      return create_i_of_q_ng (testiq_selected,
                               le_testiq_q_start->text ().toDouble (),
                               le_testiq_q_end->text ().toDouble ());
    }
}
...
void
US_Hydrodyn_Saxs_Hplc::testiq_visrange ()
{
  ...
  if (plot_dist_zoomer)
    {
      ...
      le_testiq_q_start->setText (
          QString ("%1").arg (plot_dist_zoomer->zoomRect ().left ()));
      le_testiq_q_end->setText (
          QString ("%1").arg (plot_dist_zoomer->zoomRect ().right ()));
#else
      le_testiq_q_start->setText (
          QString ("%1").arg (plot_dist_zoomer->zoomRect ().x1 ()));
      le_testiq_q_end->setText (
          QString ("%1").arg (plot_dist_zoomer->zoomRect ().x2 ()));
#endif
    }
  else
    {
  ...
  connect (le_testiq_q_end, SIGNAL (textChanged (const QString &)),
           SLOT (testiq_q_end_text (const QString &)));
#if QT_VERSION < 0x040000
  plot_dist->setMarkerPos (plotted_markers[0],
                           le_testiq_q_start->text ().toDouble (), 0e0);
  plot_dist->setMarkerPos (plotted_markers[1],
                           le_testiq_q_end->text ().toDouble (), 0e0);
#else
...
void
US_Hydrodyn_Saxs_Hplc::testiq_testset ()
{
  if (!testiq_make ())
    {
      testiq_enables ();
  ...
  for (int i = 0; i < (int)testiq_created_names.size (); ++i)
    {
      ...
      add_plot (this_file, testiq_created_q[this_file],
                testiq_created_I[this_file], testiq_created_e[this_file],
                false, false);
    }
  testiq_enables ();
}
...
void
US_Hydrodyn_Saxs_Hplc::testiq_q_start_text (const QString &text)
{
...
void
US_Hydrodyn_Saxs_Hplc::testiq_q_end_text (const QString &text)
{
  if (current_mode != MODE_TESTIQ)
    {
      return;
...
void
US_Hydrodyn_Saxs_Hplc::testiq_q_start_focus (bool hasFocus)
{
  if (current_mode != MODE_TESTIQ)
    {
      return;
...
void
US_Hydrodyn_Saxs_Hplc::testiq_q_end_focus (bool hasFocus)
{
  if (current_mode != MODE_TESTIQ)
    {
      return;
...
US_Hydrodyn_Saxs_Hplc::guinier ()
{
  ...
  guinier_plot->enableAxis (QwtPlot::xBottom,
                            !guinier_plot_errors->isVisible ());
  ...
  if (current_mode == MODE_TESTIQ)
    {
      ...
      if (!testiq_it_selected.isEmpty () && f_qs.count (testiq_it_selected))
        {
          ...
          if (!rb_testiq_from_i_t->isChecked ()
              ...
              && (int)f_gaussians[testiq_it_selected].size ()
                     == gaussian_type_size
                            * (int)((int)f_gaussians[testiq_it_selected]
                                        .size ()
                                    / gaussian_type_size))
            {
              guinier_it_I = compute_gaussian_sum (
                  guinier_it_t, f_gaussians[testiq_it_selected]);
              for (int i = 0; i < (int)rb_testiq_gaussians.size (); ++i)
                {
                  if (rb_testiq_gaussians[i]->isChecked ())
                    {
                      // us_qdebug( QString( "found rb_testiq_gaussian %1
                      // checked" ).arg( i ) ); build up guinier_it_pg_t, I as
                      // 2nd gaussian to display
                      ...
                      // us_qdebug( US_Vector::qs_vector2( "gaussian peak",
                      // guinier_it_pg_t, guinier_it_pg_I ) );
                    }
                }
            }
          ...
          guinier_it_Irange
              = testiq_it_selected_Imax - testiq_it_selected_Imin;
      ...
      for (int i = 0; i < (int)testiq_created_names.size (); ++i)
        {
          ...
          for (int j = 0; j < (int)guinier_q[this_file].size (); ++j)
            {
              guinier_q2[this_file].push_back (guinier_q[this_file][j]
                                               * guinier_q[this_file][j]);
            }
      ...
      if (!any_selected)
        {
          editor_msg (
              "red",
              us_tr ("Internal error: no files selected in Guinier mode"));
          return;
        }
    }
  else
    {
          ...
          if (lb_files->item (i)->isSelected ())
            {
              ...
              for (int j = 0; j < (int)f_qs[this_file].size (); ++j)
                {
                  guinier_q2[this_file].push_back (f_qs[this_file][j]
                                                   * f_qs[this_file][j]);
                }
              ...
              if (!plotted_curves.count (this_file))
                {
                  editor_msg (
                      "red",
                      QString (us_tr ("Internal error: guinier selected %1, "
                                      "but no plotted curve found"))
                          .arg (this_file));
                  return;
                }
              ...
              if (!guinier_q[this_file].size ())
                {
                  editor_msg (
                      "red",
                      QString (us_tr ("Internal error: guinier selected %1, "
                                      "but no data for curve found"))
                          .arg (this_file));
                  return;
                }

              guinier_colors[this_file]
                  = plot_colors[f_pos[this_file] % plot_colors.size ()];
      ...
      if (!f_Is.count (wheel_file))
        {
          editor_msg (
              "red", QString (us_tr ("Internal error: %1 not found in y data"))
                         .arg (wheel_file));
          return;
        }
      ...
      if (!f_Is[wheel_file].size ())
        {
          editor_msg ("red",
                      QString (us_tr ("Internal error: %1 empty y data"))
                          .arg (wheel_file));
          return;
        }
    }
  ...
  if (le_guinier_q_start->text ().isEmpty ()
    ...
    {
      disconnect (le_guinier_q_start, SIGNAL (textChanged (const QString &)),
                  0, 0);
  ...
  if (le_guinier_q_end->text ().isEmpty () || le_guinier_q_end->text () == "0"
      || le_guinier_q_end->text ().toDouble () > guinier_maxq)
    {
      disconnect (le_guinier_q_end, SIGNAL (textChanged (const QString &)), 0,
                  0);
      le_guinier_q_end->setText (
          QString ("%1").arg (guinier_maxq > 0.05 ? 0.05 : guinier_maxq));
      connect (le_guinier_q_end, SIGNAL (textChanged (const QString &)),
               SLOT (guinier_q_end_text (const QString &)));
    }

  disconnect (le_guinier_q2_start, SIGNAL (textChanged (const QString &)), 0,
              0);
  le_guinier_q2_start->setText (
      QString ("%1").arg (le_guinier_q_start->text ().toDouble ()
                          * le_guinier_q_start->text ().toDouble ()));
  ...
  le_guinier_q2_end->setText (
      QString ("%1").arg (le_guinier_q_end->text ().toDouble ()
                          * le_guinier_q_end->text ().toDouble ()));
  connect (le_guinier_q2_end, SIGNAL (textChanged (const QString &)),
           SLOT (guinier_q2_end_text (const QString &)));

  disconnect (le_guinier_delta_start, SIGNAL (textChanged (const QString &)),
              0, 0);
  ...
  disconnect (le_guinier_delta_end, SIGNAL (textChanged (const QString &)), 0,
              0);
...
void
US_Hydrodyn_Saxs_Hplc::guinier_scroll_highlight (int pos)
{
  ...
  //   guinier_plot->setCurveStyle( guinier_curves[ hidename ],
  //   QwtCurve::NoCurve ); guinier_plot->setCurveStyle( guinier_curves[
  //   showname ], QwtCurve::Lines );
  ...
  if (guinier_errorbar_curves.count (hidename))
    {
      for (int j = 0; j < (int)guinier_errorbar_curves[hidename].size (); ++j)
        {
          guinier_plot->setCurveStyle (guinier_errorbar_curves[hidename][j],
                                       QwtCurve::NoCurve);
        }
    }
  ...
  if (guinier_errorbar_curves.count (showname))
    {
      for (int j = 0; j < (int)guinier_errorbar_curves[showname].size (); ++j)
        {
          guinier_plot->setCurveStyle (guinier_errorbar_curves[showname][j],
                                       QwtCurve::Lines);
        }
    }
  ...
  if (guinier_fit_lines.count (hidename))
    {
      guinier_plot->setCurveStyle (guinier_fit_lines[hidename],
                                   QwtCurve::NoCurve);
    }

  if (guinier_fit_lines.count (showname))
    {
      guinier_plot->setCurveStyle (guinier_fit_lines[showname],
                                   QwtCurve::Lines);
    }

  if (guinier_error_curves.count (hidename))
    {
      guinier_plot_errors->setCurveStyle (guinier_error_curves[hidename],
                                          QwtCurve::NoCurve);
    }

  if (guinier_error_curves.count (showname))
    {
      guinier_plot_errors->setCurveStyle (guinier_error_curves[showname],
                                          QwtCurve::Sticks);
    }
  ...
  if (guinier_errorbar_curves.count (hidename))
    {
      for (int j = 0; j < (int)guinier_errorbar_curves[hidename].size (); ++j)
        {
          guinier_errorbar_curves[hidename][j]->setStyle (
  ...
  if (guinier_rg_curves.count (hidename))
    {
      ...
      sym.setStyle (QwtSymbol::Diamond);
      sym.setSize (stdsize);
      guinier_rg_curves[hidename]->setSymbol (
          new QwtSymbol (sym.style (), sym.brush (), sym.pen (), sym.size ()));
    }
  if (guinier_rg_curves.count (showname))
    {
      ...
      sym.setStyle (QwtSymbol::Rect);
      sym.setSize (bigsize + 2);
      guinier_rg_curves[showname]->setSymbol (
          new QwtSymbol (sym.style (), sym.brush (), sym.pen (), sym.size ()));
    }
  ...
  if (guinier_mwt_markers.count (hidename))
    {
      ...
      sym.setStyle (QwtSymbol::Diamond);
      sym.setSize (stdsize);
      guinier_mwt_markers[hidename]->setSymbol (
          new QwtSymbol (sym.style (), sym.brush (), sym.pen (), sym.size ()));
    }
  if (guinier_mwt_markers.count (showname))
    {
      ...
      sym.setStyle (QwtSymbol::Rect);
      sym.setSize (bigsize);
      guinier_mwt_markers[showname]->setSymbol (
          new QwtSymbol (sym.style (), sym.brush (), sym.pen (), sym.size ()));
    }
  ...
  if (guinier_mwc_markers.count (hidename))
    {
      ...
      sym.setStyle (QwtSymbol::Diamond);
      sym.setSize (stdsize);
      guinier_mwc_markers[hidename]->setSymbol (
          new QwtSymbol (sym.style (), sym.brush (), sym.pen (), sym.size ()));
    }
  if (guinier_mwc_markers.count (showname))
    {
      ...
      sym.setStyle (QwtSymbol::Rect);
      sym.setSize (bigsize);
      guinier_mwc_markers[showname]->setSymbol (
          new QwtSymbol (sym.style (), sym.brush (), sym.pen (), sym.size ()));
    }
#else
  const QwtSymbol *sym;
  ...
  sym = guinier_curves[hidename]->symbol ();
  guinier_curves[hidename]->setSymbol (new QwtSymbol (
      QwtSymbol::NoSymbol, sym->brush (), sym->pen (), sym->size ()));

  sym = guinier_curves[showname]->symbol ();
  guinier_curves[showname]->setSymbol (
      new QwtSymbol (QwtSymbol::Diamond, sym->brush (), sym->pen (),
                     QSize (stdsize, stdsize)));
  ...
  if (guinier_rg_curves.count (showname))
    {
      sym = guinier_rg_curves[showname]->symbol ();
      guinier_rg_curves[showname]->setSymbol (
          new QwtSymbol (QwtSymbol::Rect, sym->brush (), sym->pen (),
                         QSize (bigsize * 2, bigsize * 2)));
    }
  ...
  if (guinier_report.count (showname))
  ...
  else
    {
      editor_msg (
          "dark red",
          QString (us_tr ("No Guinier information for %1")).arg (showname));
    }
}

void
US_Hydrodyn_Saxs_Hplc::guinier_scroll ()
{
  ...
  if (cb_guinier_scroll->isChecked ())
    {
      ...
      for (int i = 0; i < (int)guinier_names.size (); ++i)
        {
...
#if QT_VERSION < 0x040000
          // guinier_plot->setCurveStyle( guinier_curves[ name ],
          // QwtCurve::NoCurve );
          ...
          if (guinier_errorbar_curves.count (name))
            {
              for (int j = 0; j < (int)guinier_errorbar_curves[name].size ();
                   ++j)
                {
                  guinier_plot->setCurveStyle (
                      guinier_errorbar_curves[name][j], QwtCurve::NoCurve);
                }
            }

          if (guinier_fit_lines.count (name))
            {
              guinier_plot->setCurveStyle (guinier_fit_lines[name],
                                           QwtCurve::NoCurve);
            }

          guinier_plot_errors->setCurveStyle (guinier_error_curves[name],
                                              QwtCurve::NoCurve);
          ...
          sym.setStyle (QwtSymbol::NoSymbol);
          guinier_curves[name]->setSymbol (new QwtSymbol (
              sym.style (), sym.brush (), sym.pen (), sym.size ()));
#else
          // guinier_curves[ name ]->setStyle( QwtPlotCurve::NoCurve );
          ...
          if (guinier_errorbar_curves.count (name))
            {
              for (int j = 0; j < (int)guinier_errorbar_curves[name].size ();
                   ++j)
                {
                  guinier_errorbar_curves[name][j]->setStyle (
                      QwtPlotCurve::NoCurve);
                }
            }
      ...
      qwtw_wheel->setRange (0, guinier_names.size () - 1);
      qwtw_wheel->setSingleStep (1);
      qwtw_wheel->setValue (guinier_scroll_pos);
      lbl_wheel_pos->setText (
          QString ("%1").arg (guinier_names[guinier_scroll_pos]));
      connect (qwtw_wheel, SIGNAL (valueChanged (double)),
               SLOT (adjust_wheel (double)));
      wheel_enables ();
      ...
      for (int i = 0; i < (int)guinier_names.size (); ++i)
        {
          ...
          if (guinier_errorbar_curves.count (name))
            {
              for (int j = 0; j < (int)guinier_errorbar_curves[name].size ();
                ...
                {
                  guinier_plot->setCurveStyle (
                      guinier_errorbar_curves[name][j], QwtCurve::Lines);
                }
            }
          ...
          if (guinier_rg_curves.count (name))
            {
              ...
              sym.setStyle (QwtSymbol::Diamond);
              sym.setSize (stdsize);
              guinier_rg_curves[name]->setSymbol (new QwtSymbol (
                  sym.style (), sym.brush (), sym.pen (), sym.size ()));
            }
#else
          // guinier_curves[ name ]->setStyle(  QwtPlotCurve::Lines );
          ...
          if (guinier_rg_curves.count (name))
            {
              sym = guinier_rg_curves[name]->symbol ();
              guinier_rg_curves[name]->setSymbol (
                  new QwtSymbol (QwtSymbol::Diamond, sym->brush (),
                                 sym->pen (), QSize (stdsize, stdsize)));
            }
#endif
#endif
        }
...
void
US_Hydrodyn_Saxs_Hplc::guinier_add_marker (QwtPlot *plot, double pos,
...
void
US_Hydrodyn_Saxs_Hplc::guinier_sd ()
{
  disable_all ();
  guinier_replot ();
  guinier_plot->replot ();
...
void
US_Hydrodyn_Saxs_Hplc::guinier_plot_rg_toggle ()
{
  if (guinier_rg_widgets[0]->isVisible ())
  ...
  else
    {
      ...
      connect (((QObject *)guinier_plot_rg->axisWidget (QwtPlot::xBottom)),
               SIGNAL (scaleDivChanged ()), usp_guinier_plot_mw,
               SLOT (scaleDivChangedXSlot ()), Qt::UniqueConnection);
      connect (((QObject *)guinier_plot_mw->axisWidget (QwtPlot::xBottom)),
               SIGNAL (scaleDivChanged ()), usp_guinier_plot_rg,
               SLOT (scaleDivChangedXSlot ()), Qt::UniqueConnection);

      ShowHide::hide_widgets (guinier_rg_widgets, false);
      guinier_plot_rg->enableAxis (QwtPlot::xBottom,
                                   !guinier_plot_mw->isVisible ());
    }
}

void
US_Hydrodyn_Saxs_Hplc::guinier_plot_mw_toggle ()
{
...
void
US_Hydrodyn_Saxs_Hplc::guinier_residuals (bool reset)
{
  if (reset)
    {
      ...
      if (guinier_markers.size () == 4)
        {
          ...
          guinier_add_marker (guinier_plot_errors,
                              le_guinier_q2_start->text ().toDouble (),
                              Qt::red, us_tr ("Start"));
          guinier_add_marker (guinier_plot_errors,
                              le_guinier_q2_end->text ().toDouble (), Qt::red,
                              us_tr ("End"), Qt::AlignLeft | Qt::AlignTop);
        }
  ...
  double minq2 = le_guinier_q2_start->text ().toDouble ()
                 - le_guinier_delta_start->text ().toDouble ();
  double maxq2 = le_guinier_q2_end->text ().toDouble ()
                 + le_guinier_delta_end->text ().toDouble ();

  for (map<QString, vector<double> >::iterator it = guinier_q2.begin ();
       it != guinier_q2.end (); ++it)
    {
      ...
      QPen use_pen
          = QPen (guinier_colors[it->first], use_line_width, Qt::SolidLine);
          ...
          for (int i = 0; i < (int)pts; ++i)
            {
              tmp = (*I)[i];
              ltmp = log (tmp > 0e0 ? tmp : 1e-99);
              e[i] = 100e0 * (ltmp - (a + b * it->second[i])) / fabs (ltmp);
              ;
            }
        }
      ...
      for (int i = 0; i < (int)pts; ++i)
        {
          if (it->second[i] >= minq2 && it->second[i] <= maxq2)
            {
              if (emin > e[i])
                {
                  emin = e[i];
...
#if QT_VERSION >= 0x040000
      curve->setSamples ((double *)&it->second[0], (double *)&e[0], pts);
#else
      guinier_plot_errors->setCurveData (curve, (double *)&it->second[0],
                                         (double *)&e[0], pts);
#endif
    }
...
void
US_Hydrodyn_Saxs_Hplc::guinier_residuals_update ()
{
  if (rb_guinier_resid_diff->isChecked ())
    {
      guinier_plot_errors->setAxisTitle (QwtPlot::yLeft,
                                         us_tr ("ln(I(q)) - Guinier line"));
    }
  if (rb_guinier_resid_sd->isChecked ())
    {
      guinier_plot_errors->setAxisTitle (
          QwtPlot::yLeft, us_tr ("(ln(I(q)) - Guinier line) / S.D."));
    }
  if (rb_guinier_resid_pct->isChecked ())
    {
      guinier_plot_errors->setAxisTitle (
          QwtPlot::yLeft,
          us_tr ("% difference [100*(ln(I(q)) - Guinier)/ln(I(q))]"));
    }
  guinier_residuals ();
  guinier_plot_errors->replot ();
}
...
void
US_Hydrodyn_Saxs_Hplc::guinier_analysis ()
{
  ...
  //   double q2endvis    = le_guinier_q2_end ->text().toDouble() +
  //   le_guinier_delta_end->text().toDouble();
  ...
  guinier_plot_rg->detachItems (QwtPlotItem::Rtti_PlotCurve);
  guinier_plot_rg->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
  guinier_rg_curves.clear ();
  ...
  guinier_plot_mw->detachItems (QwtPlotItem::Rtti_PlotCurve);
  guinier_plot_mw->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
  guinier_mw_curves.clear ();
  guinier_mwt_markers.clear ();
  guinier_mwc_markers.clear ();
  ...
  double mwt_k = ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("guinier_mwt_k")
                     ? ((US_Hydrodyn *)us_hydrodyn)
                           ->gparams["guinier_mwt_k"]
                           .toDouble ()
                     : 0e0;
  double mwt_c = ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("guinier_mwt_c")
                     ? ((US_Hydrodyn *)us_hydrodyn)
                           ->gparams["guinier_mwt_c"]
                           .toDouble ()
                     : 0e0;
  ...
  guinier_plot_summary->detachItems (QwtPlotItem::Rtti_PlotCurve);
  guinier_plot_summary->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
  vector<double> ps_x;
  vector<double> ps_rg;
  ...
  for (int i = 0; i < (int)guinier_names.size (); ++i)
    {
      progress->setValue (i);
      progress->setMaximum (guinier_names.size ());
      ...
      do
        {
          ...
          if (guinier_e[this_name].size () != guinier_q[this_name].size ())
            {
              // any_sd_off = true;
              // editor_msg( "dark red", QString( us_tr( "Notice: SD weighting
              // of Guinier fit is off for %1 since SDs are not fully present"
              // ) )
              //             .arg( this_name ) );
                  ...
                  if (guinier_q[this_name][j] >= qstart
                      && guinier_q[this_name][j] <= qend)
                    {
                      if (guinier_e[this_name][j] <= 0e0)
                        {
                          // any_sd_off = true;
                          // editor_msg( "dark red", QString( us_tr( "Notice:
                          // SD weighting of Guinier fit is off for %1 since at
                          // least one SD is zero or negative in the selected q
                          // range" ) )
                          //             .arg( this_name) );
                          use_SD_weighting = false;
                          break;
          ...
          if (pts_decrease)
            {
              if ((int)usu->wave["hplc"].q.size () - (int)pts_decrease < 5)
                {
                  if (!cb_guinier_scroll->isChecked ()
                      || i == guinier_scroll_pos)
                    {
                      editor_msg (
                          "dark red",
                          QString (us_tr ("%1 too few points left %2 after %3 "
                                          "points removed for qRgmax limit"))
                              .arg (this_name)
                              .arg ((int)usu->wave["hplc"].q.size ()
                                    - (int)pts_decrease)
                              .arg (pts_decrease));
                    }
                  break;
              ...
              usu->wave["hplc"].q.resize (usu->wave["hplc"].q.size ()
                                          - pts_decrease);
          ...
          unsigned int pend = usu->wave["hplc"].q.size ()
                                  ? usu->wave["hplc"].q.size () - 1
                                  : 0;

          computed_rg
              = usu->guinier_plot ("hplcrg", "hplc")
                && usu->guinier_fit (this_log, "hplcrg", pstart, pend, a, b,
                                     siga, sigb, chi2, Rg, I0,
              ...
              if (!cb_guinier_scroll->isChecked () || i == guinier_scroll_pos)
                {
                  editor_msg (
                      "dark red",
                      QString ("%1 could not compute Rg").arg (this_name));
                }
              break;
            }

          // us_qdebug( QString( "%1 decrease check sRgmax %2 sRgmaxlimit %3
          // %4" ).arg( this_name ).arg( sRgmax ).arg( sRgmaxlimit )
          //         .arg( do_decrease ? "do_decrease on before check" :
          //         "do_decrease off before check" ) );
          do_decrease = do_decrease && (sRgmax > sRgmaxlimit);
          if (do_decrease)
            {
              pts_decrease++;
              //    us_qdebug( QString( "%1 decreasing %2" ).arg( this_name
              //    ).arg( pts_decrease ) );
              // } else {
      ...
      if (computed_rg)
        {
          ...
          if (!us_isnan (Rg))
            {
              ...
              if (!count)
              ...
              else
                {
                  ...
                  if (us_isnan (siga)
                      || (unsigned int)usu->wave["hplc"].q.size () < 3)
                    {
                      if (rg_sd_min > Rg)
                        {
                          rg_sd_min = Rg;
          ...
          QString report = QString ("%1:%2 ").arg (pos).arg (this_name)
                           + QString ("").sprintf (
                               "Rg %.1f (%.1f) (A) I(0) %.2e (%.2e) qRg "
                               "[%.3f,%.3f] pts %u chi^2 %.2e r-chi %.2e\n",
                               Rg, sigb, I0, siga, sRgmin, sRgmax,
                               (unsigned int)usu->wave["hplc"].q.size (), chi2,
                               sqrt (chi2 / usu->wave["hplc"].q.size ()))
                           + us_tr (use_SD_weighting ? "SD  on" : "SD OFF");

          // tainer mw method
          if (US_Saxs_Util::mwt (guinier_q[this_name], guinier_I[this_name],
                                 Rg, sigb, I0, siga, mwt_k, mwt_c, mwt_qmax,
                                 Vct, Qrt, mwt, mwt_sd, messagest, notest,
                                 warningt)
              && !us_isnan (mwt))
            {
              report
                  += QString ("").sprintf (
                         " Vc[T] %.1e Qr[T] %.2e MW[RT] %.2e ", Vct, Qrt, mwt)
                     + notest;
              if (mw_min > mwt)
                {
                  mw_min = mwt;
            ...
            {
              report += us_tr (" MW[RT] could not compute " + messagest + " "
                               + notest);
            }
          ...
          if (started_in_expert_mode)
            {
              if (usu->mwc (guinier_q[this_name], guinier_I[this_name], Rg,
                            sigb, I0, siga, mwc_mw_per_N, qm, Vcc, Qrc, mwc,
                            mwc_sd, messagesc, notesc)
                  && !us_isnan (mwc))
                {
                  report += QString ("").sprintf (
                                " Vc[%.3f,C] %.1e Qr[C] %.2e MW[C] %.2e ", qm,
                                Vcc, Qrc, mwc)
                            + notesc;
                  mwc_x.push_back (pos);
                  mwc_y.push_back (mwc);
                ...
                {
                  report += us_tr (" MW[C] could not compute " + messagesc
                                   + " " + notesc);
                }
            }
          ...
          guinier_y[this_name].push_back (
              exp (a + b * guinier_q2[this_name][0]));
          ...
          if (!us_isnan (Rg))
            {
              if (!guinier_fit_lines.count (this_name))
                {
                  ...
                  curve->setStyle (QwtPlotCurve::Lines);
                  curve->setPen (QPen (guinier_colors[this_name],
                                       use_line_width, Qt::SolidLine));
                  curve->attach (guinier_plot);
#else
                  long curve = guinier_plot->insertCurve ("fl:" + this_name);
                  guinier_plot->setCurveStyle (curve, QwtCurve::Lines);
                  guinier_plot->setCurvePen (curve,
                                             QPen (guinier_colors[this_name],
                                                   use_line_width, SolidLine));
#endif
              ...
              guinier_fit_lines[this_name]->setSamples (
                  ...
                  (double *)&(guinier_y[this_name][0]), 2);
#else
...
#endif
              {
                ...
                curve->setSymbol (
                    new QwtSymbol (rg_sym.style (), rg_sym.brush (),
                                   rg_sym.pen (), rg_sym.size ()));
...

              {
                ...
                QPen use_pen = QPen (guinier_colors[this_name], use_line_width,
                                     Qt::SolidLine);
#if QT_VERSION >= 0x040000
          ...
          // US_Vector::printvector2( "guinier x,y:", guinier_x[ this_name ],
          // guinier_y[ this_name ] );
        }
    }
      ...
      {
        {
          ...
          curve->setSamples ((double *)&(ps_x[0]), (double *)&(ps_rg[0]),
                             ps_x.size ());
          curve->attach (guinier_plot_summary);
        }
        {
          QwtPlotCurve *curve = new QwtPlotCurve ("Rg sd");
          curve->setSamples ((double *)&(ps_x[0]), (double *)&(ps_rg_sd[0]),
                             ps_x.size ());
          curve->attach (guinier_plot_summary);
        }
        {
          QwtPlotCurve *curve = new QwtPlotCurve ("I0");
          curve->setSamples ((double *)&(ps_x[0]), (double *)&(ps_I0[0]),
                             ps_x.size ());
          curve->attach (guinier_plot_summary);
        }
        {
          QwtPlotCurve *curve = new QwtPlotCurve ("I0 sd");
          curve->setSamples ((double *)&(ps_x[0]), (double *)&(ps_I0_sd[0]),
                             ps_x.size ());
          curve->attach (guinier_plot_summary);
        }
      }
      ...
      {
        {
          ...
          curve->setSamples ((double *)&(mwt_x[0]), (double *)&(mwt_y[0]),
                             mwt_x.size ());
          curve->attach (guinier_plot_summary);
        }
        {
          QwtPlotCurve *curve = new QwtPlotCurve ("MW[RT] sd");
          curve->setSamples ((double *)&(mwt_x[0]), (double *)&(mwt_sds[0]),
                             mwt_x.size ());
          curve->attach (guinier_plot_summary);
        }
      }
      ...
      {
        {
          ...
          curve->setSamples ((double *)&(mwc_x[0]), (double *)&(mwc_y[0]),
                             mwc_x.size ());
          curve->attach (guinier_plot_summary);
        }
        {
          QwtPlotCurve *curve = new QwtPlotCurve ("MW[C] sd");
          curve->setSamples ((double *)&(mwc_x[0]), (double *)&(mwc_sds[0]),
                             mwc_x.size ());
          curve->attach (guinier_plot_summary);
        }
      }
        ...
        if (ps_rg.size ())
          {
            ...
            guinier_plot_summary->setCurveData (curve, (double *)&(ps_x[0]),
                                                (double *)&(ps_rg[0]),
                                                ps_x.size ());
          }
        if (ps_rg_sd.size ())
      ...
      {
        {
          ...
          guinier_plot_summary->setCurveData (curve, (double *)&(mwt_x[0]),
                                              (double *)&(mwt_y[0]),
                                              mwt_x.size ());
        }
        {
      ...
      {
        {
          ...
          guinier_plot_summary->setCurveData (curve, (double *)&(mwc_x[0]),
                                              (double *)&(mwc_y[0]),
                                              mwc_x.size ());
        }
        {
  ...
  switch (count)
    ...
    case 0:
      ...
      break;
    case 1:
      msg = QString ("qmax*Rg %1   Rg %2   I0 %3\n")
                .arg (qrg_avg)
                .arg (rg_avg)
                .arg (i0_avg);
      break;
    default:
      {
        ...
        double rg_sd
            = sqrt (countm1inv * (rg_sum2 - countinv * rg_avg * rg_avg));
        double i0_sd
            = sqrt (countm1inv * (i0_sum2 - countinv * i0_avg * i0_avg));
        ...
        msg += QString ("").sprintf (
            ...
            "%.2e (%.2e) [%.2e:%.2e]",
            qrg_avg, qrg_min, qrg_max, rg_avg, rg_sd, rg_min, rg_max, i0_avg,
            i0_sd, i0_min, i0_max);
      }
      break;
    }
          ...
          if (count > 2)
            {
              ...
              double mwt_sd
                  = sqrt (countm1inv * (mwt_sum2 - count * mwt_avg * mwt_avg));

              msg += QString ("").sprintf (" %.4g (%.4g) [%.4g:%.4g]", mwt_avg,
                                           mwt_sd, mwt_min, mwt_max);
            }
          else
            {
              msg += QString ("").sprintf (" %.4g ", mwt_avg);
            }
          if (mwc_x.size ())
            {
              msg += "   ";
          ...
          if (count > 2)
            {
              ...
              double countm1inv = 1e0 / (count - 1e0);
              double mwc_sd
                  = sqrt (countm1inv * (mwc_sum2 - count * mwc_avg * mwc_avg));

              msg += QString ("").sprintf (" %.4g (%.4g) [%.4g:%.4g]", mwc_avg,
                                           mwc_sd, mwc_min, mwc_max);
            }
          else
            {
              msg += QString ("").sprintf (" %.4g ", mwc_avg);
            }
          if (mwc_x.size ())
            {
              msg += "   ";
  ...
  if (rg_x.size () > 1)
    {
      ...
      curve->setStyle (QwtPlotCurve::Lines);
      curve->setSamples ((double *)&(rg_x[0]), (double *)&(rg_y[0]),
                         rg_x.size ());
      curve->setPen (use_pen);
      curve->attach (guinier_plot_rg);
#else
      ...
      guinier_plot_rg->setCurveStyle (curve, QwtCurve::Lines);
      guinier_plot_rg->setCurveData (curve, (double *)&(rg_x[0]),
                                     (double *)&(rg_y[0]), rg_x.size ());
      guinier_plot_rg->setCurvePen (curve, use_pen);
#endif
    }
  ...
  if (mwt_x.size () > 1)
    {
      ...
      guinier_plot_mw->setCurveData (curve, (double *)&(mwt_x[0]),
                                     (double *)&(mwt_y[0]), mwt_x.size ());
        ...
        for (int i = 0; i < (int)mwt_x.size (); ++i)
          {
            ...
            QwtPlotMarker *marker = new QwtPlotMarker;
#if QT_VERSION < 0x050000
            marker->setSymbol (new QwtSymbol (sym.style (), sym.brush (),
                                              sym.pen (), sym.size ()));
#else
            marker->setSymbol (new QwtSymbol (sym.style (), sym.brush (),
                                              sym.pen (), sym.size ()));
  ...
  if (mwc_x.size () > 1)
    {
      ...
      guinier_plot_mw->setCurveStyle (curve, QwtCurve::Lines);
      guinier_plot_mw->setCurveData (curve, (double *)&(mwc_x[0]),
                                     (double *)&(mwc_y[0]), mwc_x.size ());
      guinier_plot_mw->setCurvePen (curve, use_pen);
  ...
  // us_qdebug( QString( "space %1 rg_max %2 rg_sd_max %3 rg_min %4 rg_sd_min
  // %5 count %6" )
  //         .arg( space )
  //         .arg( rg_max )
  //         .arg( rg_sd_max )
  ...
  if (rg_x.size () > 1 && guinier_it_t.size () && space > 0e0)
    {
      // us_qdebug( QString( "testiq in rg plot ok use minmax %1 %2" ).arg(
      // use_min ).arg( use_max ) );
      vector<double> x;
      vector<double> y;
      ...
      for (int i = 0; i < (int)guinier_it_t.size (); ++i)
        {
          if (guinier_it_t[i] >= use_min && guinier_it_t[i] <= use_max)
            {
      ...
      if (ymax > ymin)
        {
          ...
          // us_qdebug( QString( "testiq in rg plot scale %1 rg_min %2 rg_max
          // %3 ymin %4 ymax %5 " ).arg( scale ).arg( rg_min ).arg( rg_max
          // ).arg( ymin ).arg( ymax ) );
          ...
          // QPen use_pen = QPen( plot_colors[ f_pos[ testiq_it_selected ] %
          // plot_colors.size() ], use_line_width, Qt::SolidLine );
          QPen use_pen = QPen (Qt::green, use_line_width,
                               y3.size () ? Qt::DotLine : Qt::SolidLine);
#if QT_VERSION >= 0x040000
          QwtPlotCurve *curve = new QwtPlotCurve ("refitline");
          curve->setStyle (QwtPlotCurve::Lines);
          curve->setSamples ((double *)&(guinier_it_t[0]), (double *)&(y2[0]),
                             guinier_it_t.size ());
          curve->setPen (use_pen);
          curve->attach (guinier_plot_rg);
#else
          ...
          guinier_plot_rg->setCurveStyle (curve, QwtCurve::Lines);
          guinier_plot_rg->setCurveData (curve, (double *)&(guinier_it_t[0]),
                                         (double *)&(y2[0]),
                                         guinier_it_t.size ());
          guinier_plot_rg->setCurvePen (curve, use_pen);
#endif
          if (y3.size ())
            {
              ...
              QwtPlotCurve *curve = new QwtPlotCurve ("refitline");
              curve->setStyle (QwtPlotCurve::Lines);
              curve->setSamples ((double *)&(guinier_it_pg_t[0]),
                                 (double *)&(y3[0]), guinier_it_pg_t.size ());
              curve->setPen (use_pen);
              curve->attach (guinier_plot_rg);
#else
              long curve = guinier_plot_rg->insertCurve ("refitlinepeak");
              guinier_plot_rg->setCurveStyle (curve, QwtCurve::Lines);
              guinier_plot_rg->setCurveData (
                  curve, (double *)&(guinier_it_pg_t[0]), (double *)&(y3[0]),
...

  {
    if (cb_guinier_lock_rg_range->isChecked ())
      {
        guinier_plot_rg->setAxisScale (
            QwtPlot::xBottom, le_guinier_rg_t_start->text ().toDouble (),
            le_guinier_rg_t_end->text ().toDouble ());
        guinier_plot_rg->setAxisScale (
            QwtPlot::yLeft, le_guinier_rg_rg_start->text ().toDouble (),
            le_guinier_rg_rg_end->text ().toDouble ());
      }
    else
      {
        guinier_plot_rg->setAxisScale (QwtPlot::xBottom, use_min, use_max);
        guinier_plot_rg->setAxisScale (QwtPlot::yLeft, rg_sd_min - space,
                                       rg_sd_max + space);

        disconnect (le_guinier_rg_t_start,
                    SIGNAL (textChanged (const QString &)), 0, 0);
        disconnect (le_guinier_rg_t_end,
                    SIGNAL (textChanged (const QString &)), 0, 0);
        disconnect (le_guinier_rg_rg_start,
                    SIGNAL (textChanged (const QString &)), 0, 0);
        disconnect (le_guinier_rg_rg_end,
                    SIGNAL (textChanged (const QString &)), 0, 0);
        ...
        connect (le_guinier_rg_rg_start,
    ...
    guinier_plot_rg_zoomer->setRubberBandPen (
        QPen (Qt::yellow, 0, Qt::DotLine));
  ...
  if ((mwt_x.size () > 1 || mwc_x.size () > 1) && guinier_it_t.size ()
      && mw_space > 0e0)
    {
      vector<double> x;
      vector<double> y;
      ...
      for (int i = 0; i < (int)guinier_it_t.size (); ++i)
        {
          if (guinier_it_t[i] >= mw_use_min && guinier_it_t[i] <= mw_use_max)
            {
              x.push_back (guinier_it_t[i]);
              y.push_back (guinier_it_I[i]);
              if (ymin > guinier_it_I[i])
      ...
      if (ymax > ymin)
        {
          ...
          // us_qdebug( QString( "testiq in mw plot scale %1 mw_min %2 mw_max
          // %3 ymin %4 ymax %5 " ).amw( scale ).amw( mw_min ).amw( mw_max
          // ).amw( ymin ).amw( ymax ) );
          ...
          guinier_plot_mw->setCurveStyle (curve, QwtCurve::Lines);
          guinier_plot_mw->setCurveData (curve, (double *)&(guinier_it_t[0]),
                                         (double *)&(y2[0]),
                                         guinier_it_t.size ());
          guinier_plot_mw->setCurvePen (curve, use_pen);
#endif
  ...
  if (mwt_x.size () || mwt_y.size ())
    {
      if (cb_guinier_lock_mw_range->isChecked ())
        {
          guinier_plot_mw->setAxisScale (
              QwtPlot::xBottom, le_guinier_mw_t_start->text ().toDouble (),
              le_guinier_mw_t_end->text ().toDouble ());
          guinier_plot_mw->setAxisScale (
              QwtPlot::yLeft, le_guinier_mw_mw_start->text ().toDouble (),
              le_guinier_mw_mw_end->text ().toDouble ());
        }
      else
        {
          guinier_plot_mw->setAxisScale (QwtPlot::xBottom, mw_use_min,
                                         mw_use_max);
          guinier_plot_mw->setAxisScale (QwtPlot::yLeft, mw_min - space,
                                         mw_max + space);

          disconnect (le_guinier_mw_t_start,
                      SIGNAL (textChanged (const QString &)), 0, 0);
          disconnect (le_guinier_mw_t_end,
                      SIGNAL (textChanged (const QString &)), 0, 0);
          disconnect (le_guinier_mw_mw_start,
                      SIGNAL (textChanged (const QString &)), 0, 0);
          disconnect (le_guinier_mw_mw_end,
                      SIGNAL (textChanged (const QString &)), 0, 0);
          ...
          le_guinier_mw_mw_start->setText (
              QString ("%1").arg (mw_min - space));
          le_guinier_mw_mw_end->setText (QString ("%1").arg (mw_max + space));
          connect (le_guinier_mw_t_start,
                   SIGNAL (textChanged (const QString &)),
          ...
          connect (le_guinier_mw_mw_start,
                   SIGNAL (textChanged (const QString &)),
                   SLOT (guinier_mw_mw_start_text (const QString &)));
          connect (le_guinier_mw_mw_end,
                   SIGNAL (textChanged (const QString &)),
      ...
      guinier_plot_mw_zoomer->setRubberBandPen (
          QPen (Qt::yellow, 0, Qt::DotLine));
  ...
  if (testiq_active && rb_testiq_gaussians.size ())
    {
      for (int i = 0; i < (int)unified_ggaussian_gaussians_size; i++)
        {
          int line_width
              = use_line_width < 3 ? (use_line_width + 1) : use_line_width;
          double pos
              = unified_ggaussian_params[(vector<double>::size_type)common_size
                                         * i];
          QString text = QString ("%1").arg (i + 1);
          QColor color
              = rb_testiq_gaussians[i]->isChecked () ? Qt::magenta : Qt::blue;
#if QT_VERSION >= 0x040000
          QwtPlotMarker *marker = new QwtPlotMarker;
          marker->setLineStyle (QwtPlotMarker::VLine);
          ...
          guinier_plot_rg->setMarkerLabelAlign (marker,
                                                Qt::AlignRight | Qt::AlignTop);
          guinier_plot_rg->setMarkerPen (
              marker, QPen (color, line_width, DashDotDotLine));
          guinier_plot_rg->setMarkerFont (
              marker, QFont ("Helvetica", 11, QFont::Bold));
...
void
US_Hydrodyn_Saxs_Hplc::guinier_delete_markers ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Saxs_Hplc::guinier_range (double minq2, double maxq2, double minI,
                                      double maxI)
{
  ...
  if (!guinier_plot_zoomer)
    {
      ...
      guinier_plot_zoomer->setRubberBandPen (
          QPen (Qt::yellow, 0, Qt::DotLine));
      guinier_plot_zoomer->setTrackerPen (QPen (Qt::red));
      // connect( guinier_plot_zoomer, SIGNAL( zoomed( const QRectF & ) ),
  ...
  if (!guinier_plot_errors_zoomer)
    {
      // puts( "redoing zoomer" );
      guinier_plot_errors_zoomer
          = new ScrollZoomer (guinier_plot_errors->canvas ());
      guinier_plot_errors_zoomer->setRubberBandPen (
          QPen (Qt::yellow, 0, Qt::DotLine));
...
void
US_Hydrodyn_Saxs_Hplc::guinier_range ()
{
  double minI = 1e99;
  double maxI = 0e0;
  double minq2 = le_guinier_q2_start->text ().toDouble ()
      ...
      for (unsigned int i = 0; i < q_points; i++)
        {
          ...
          if (I > 0e0 && guinier_q2[it->first][i] >= minq2
...
void
US_Hydrodyn_Saxs_Hplc::guinier_replot ()
{
  guinier_curves.clear ();
  guinier_markers.clear ();
  guinier_fit_lines.clear ();
  guinier_errorbar_curves.clear ();
  guinier_plot->detachItems (QwtPlotItem::Rtti_PlotCurve);
  guinier_plot->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
  guinier_add_marker (guinier_plot, le_guinier_q2_start->text ().toDouble (),
                      Qt::red, us_tr ("Start"));
  guinier_add_marker (guinier_plot, le_guinier_q2_end->text ().toDouble (),
                      Qt::red, us_tr ("End"), Qt::AlignLeft | Qt::AlignTop);
  guinier_add_marker (guinier_plot_errors,
                      le_guinier_q2_start->text ().toDouble (), Qt::red,
                      us_tr ("Start"));
  guinier_add_marker (guinier_plot_errors,
                      le_guinier_q2_end->text ().toDouble (), Qt::red,
                      us_tr ("End"), Qt::AlignLeft | Qt::AlignTop);
...

  {
    ...
    for (map<QString, vector<double> >::iterator it = guinier_q2.begin ();
      ...
      {
        bool use_error
            = (guinier_q[it->first].size () == guinier_e[it->first].size ());
        if (use_error)
          {
  ...
  for (map<QString, vector<double> >::iterator it = guinier_q2.begin ();
       it != guinier_q2.end (); ++it)
    {
      ...
      curve->setSymbol (
          new QwtSymbol (sym.style (), sym.brush (), sym.pen (), sym.size ()));
#else
      curve->setSymbol (new QwtSymbol (sym.style (), sym.brush (),
                                       QPen (guinier_colors[it->first]),
                                       sym.size ()));
#endif
#endif
...
#if QT_VERSION < 0x040000
      guinier_plot->setCurveData (curve, (double *)&(q[0]), (double *)&(I[0]),
                                  q_points);
      guinier_plot->setCurvePen (curve,
                                 QPen (use_qc, use_line_width, SolidLine));
#else
      curve->setSamples ((double *)&(q[0]), (double *)&(I[0]), q_points);
          ...
          for (int i = 0; i < (int)q_points; ++i)
            {
              ...
              guinier_plot->setCurveStyle (curve, QwtCurve::Lines);
              guinier_plot->setCurveData (curve, (double *)&(x[0]),
                                          (double *)&(y[0]), 2);
...
void
US_Hydrodyn_Saxs_Hplc::guinier_enables ()
{
  ...
  ShowHide::hide_widgets (wheel_below_widgets,
                          !cb_guinier_scroll->isChecked ());
}

bool
US_Hydrodyn_Saxs_Hplc::guinier_check_qmax (bool show_message)
{
  QString report;
...
void
US_Hydrodyn_Saxs_Hplc::check_mwt_constants (bool force)
{
...
bool
US_Hydrodyn_Saxs_Hplc::guinier_check_qmax (QString &report, bool show_message)
{
  report = "";
  ...
  for (int i = 0; i < (int)guinier_names.size (); ++i)
    {
      ...
      if (guinier_q.count (this_file) && guinier_q[this_file].size ())
        {
  ...
  report
      = QString (us_tr ("MW[RT] calculations are calibrated for a qmax of %1\n"
...
void
US_Hydrodyn_Saxs_Hplc::guinier_qrgmax ()
{
  disable_all ();
  qApp->processEvents ();
  guinier_analysis ();
...
void
US_Hydrodyn_Saxs_Hplc::guinier_qrgmax_text (const QString &)
{
...
void
US_Hydrodyn_Saxs_Hplc::guinier_q_start_text (const QString &text)
{
  ...
  if (guinier_markers.size () != 4)
    {
      editor_msg (
          "red",
          QString (
              "internal error: guinier_q_start_text markers issue size %1")
              .arg (guinier_markers.size ()));
      return;
    }
  ...
  if (!wheel_is_pressed && qwtw_wheel->value () != text.toDouble ())
    {
      qwtw_wheel->setValue (text.toDouble ());
      disconnect (le_guinier_q2_start, SIGNAL (textChanged (const QString &)),
                  0, 0);
      le_guinier_q2_start->setText (
          QString ("%1").arg (text.toDouble () * text.toDouble ()));
      connect (le_guinier_q2_start, SIGNAL (textChanged (const QString &)),
               SLOT (guinier_q2_start_text (const QString &)));
    }
...
void
US_Hydrodyn_Saxs_Hplc::guinier_q_end_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
  ...
  if (guinier_markers.size () != 4)
    {
      editor_msg (
          "red",
          QString ("internal error: guinier_q_end_text markers issue size %1")
              .arg (guinier_markers.size ()));
      return;
    }
  ...
  if (!wheel_is_pressed && qwtw_wheel->value () != text.toDouble ())
    {
      qwtw_wheel->setValue (text.toDouble ());
      disconnect (le_guinier_q2_end, SIGNAL (textChanged (const QString &)), 0,
                  0);
      le_guinier_q2_end->setText (
          QString ("%1").arg (text.toDouble () * text.toDouble ()));
      connect (le_guinier_q2_end, SIGNAL (textChanged (const QString &)),
...
void
US_Hydrodyn_Saxs_Hplc::guinier_q_start_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
  ...
  if (hasFocus)
    {
      ...
      qwtw_wheel->setSingleStep ((guinier_maxq - guinier_minq)
                                 / UHSH_G_WHEEL_RES);
      connect (qwtw_wheel, SIGNAL (valueChanged (double)),
               SLOT (adjust_wheel (double)));
...
void
US_Hydrodyn_Saxs_Hplc::guinier_q_end_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Saxs_Hplc::guinier_q2_start_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
  ...
  if (guinier_markers.size () != 4)
    {
      editor_msg (
          "red",
          QString (
              "internal error: guinier_q2_start_text markers issue size %1")
              .arg (guinier_markers.size ()));
      return;
    }
  ...
  if (!wheel_is_pressed && qwtw_wheel->value () != text.toDouble ())
    {
      ...
      le_guinier_q_start->setText (
          QString ("%1").arg (sqrt (text.toDouble ())));
      connect (le_guinier_q_start, SIGNAL (textChanged (const QString &)),
               SLOT (guinier_q_start_text (const QString &)));
    }
...
void
US_Hydrodyn_Saxs_Hplc::guinier_q2_end_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
  ...
  if (guinier_markers.size () != 4)
    {
      editor_msg (
          "red",
          QString ("internal error: guinier_q2_end_text markers issue size %1")
              .arg (guinier_markers.size ()));
      return;
    }
...
void
US_Hydrodyn_Saxs_Hplc::guinier_q2_start_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
  ...
  if (hasFocus)
    {
      ...
      qwtw_wheel->setSingleStep ((guinier_maxq2 - guinier_minq2)
                                 / UHSH_G_WHEEL_RES);
      connect (qwtw_wheel, SIGNAL (valueChanged (double)),
               SLOT (adjust_wheel (double)));
...
void
US_Hydrodyn_Saxs_Hplc::guinier_q2_end_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Saxs_Hplc::guinier_delta_start_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Saxs_Hplc::guinier_delta_end_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Saxs_Hplc::guinier_delta_start_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
  ...
  if (hasFocus)
    {
      ...
      disconnect (qwtw_wheel, SIGNAL (valueChanged (double)), 0, 0);
      qwtw_wheel->setRange (0e0, guinier_maxq2);
      qwtw_wheel->setSingleStep (guinier_maxq2 / UHSH_G_WHEEL_RES);
      connect (qwtw_wheel, SIGNAL (valueChanged (double)),
               SLOT (adjust_wheel (double)));
...
void
US_Hydrodyn_Saxs_Hplc::guinier_delta_end_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Saxs_Hplc::guinier_rg_t_start_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
    }
  guinier_plot_rg->setAxisScale (QwtPlot::xBottom, text.toDouble (),
                                 le_guinier_rg_t_end->text ().toDouble ());
  guinier_plot_rg->replot ();
  if (guinier_plot_rg_zoomer)
    {
...
void
US_Hydrodyn_Saxs_Hplc::guinier_rg_t_end_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
    }
  guinier_plot_rg->setAxisScale (QwtPlot::xBottom,
                                 le_guinier_rg_t_start->text ().toDouble (),
                                 text.toDouble ());
  guinier_plot_rg->replot ();
  if (guinier_plot_rg_zoomer)
    {
...
void
US_Hydrodyn_Saxs_Hplc::guinier_rg_t_start_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
  ...
  if (hasFocus)
    {
      ...
      qwtw_wheel->setSingleStep ((guinier_maxt - guinier_mint)
                                 / UHSH_G_WHEEL_RES);
      connect (qwtw_wheel, SIGNAL (valueChanged (double)),
               SLOT (adjust_wheel (double)));
...
void
US_Hydrodyn_Saxs_Hplc::guinier_rg_t_end_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Saxs_Hplc::guinier_rg_rg_start_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
    }
  guinier_plot_rg->setAxisScale (QwtPlot::yLeft, text.toDouble (),
                                 le_guinier_rg_rg_end->text ().toDouble ());
  guinier_plot_rg->replot ();
  if (guinier_plot_rg_zoomer)
    {
...
void
US_Hydrodyn_Saxs_Hplc::guinier_rg_rg_end_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
    }
  guinier_plot_rg->setAxisScale (QwtPlot::yLeft,
                                 le_guinier_rg_rg_start->text ().toDouble (),
                                 text.toDouble ());
  guinier_plot_rg->replot ();
  if (guinier_plot_rg_zoomer)
    {
...
void
US_Hydrodyn_Saxs_Hplc::guinier_rg_rg_start_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
  ...
  if (hasFocus)
    {
      ...
      qwtw_wheel->setSingleStep (UHSH_MAX_RG / UHSH_G_WHEEL_RES);
      connect (qwtw_wheel, SIGNAL (valueChanged (double)),
               SLOT (adjust_wheel (double)));
...
void
US_Hydrodyn_Saxs_Hplc::guinier_rg_rg_end_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Saxs_Hplc::guinier_mw_t_start_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
    }
  guinier_plot_mw->setAxisScale (QwtPlot::xBottom, text.toDouble (),
                                 le_guinier_mw_t_end->text ().toDouble ());
  guinier_plot_mw->replot ();
  if (guinier_plot_mw_zoomer)
    {
...
void
US_Hydrodyn_Saxs_Hplc::guinier_mw_t_end_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
    }
  guinier_plot_mw->setAxisScale (QwtPlot::xBottom,
                                 le_guinier_mw_t_start->text ().toDouble (),
                                 text.toDouble ());
  guinier_plot_mw->replot ();
  if (guinier_plot_mw_zoomer)
    {
...
void
US_Hydrodyn_Saxs_Hplc::guinier_mw_t_start_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Saxs_Hplc::guinier_mw_t_end_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Saxs_Hplc::guinier_mw_mw_start_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
    }
  guinier_plot_mw->setAxisScale (QwtPlot::yLeft, text.toDouble (),
                                 le_guinier_mw_mw_end->text ().toDouble ());
  guinier_plot_mw->replot ();
  if (guinier_plot_mw_zoomer)
    {
...
void
US_Hydrodyn_Saxs_Hplc::guinier_mw_mw_end_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
    }
  guinier_plot_mw->setAxisScale (QwtPlot::yLeft,
                                 le_guinier_mw_mw_start->text ().toDouble (),
                                 text.toDouble ());
  guinier_plot_mw->replot ();
  if (guinier_plot_mw_zoomer)
    {
...
void
US_Hydrodyn_Saxs_Hplc::guinier_mw_mw_start_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
  ...
  if (hasFocus)
    {
      ...
      qwtw_wheel->setSingleStep (UHSH_MAX_MW / UHSH_G_WHEEL_RES);
      connect (qwtw_wheel, SIGNAL (valueChanged (double)),
               SLOT (adjust_wheel (double)));
...
void
US_Hydrodyn_Saxs_Hplc::guinier_mw_mw_end_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
// --- wyatt ---
void
US_Hydrodyn_Saxs_Hplc::wyatt_start ()
{
  if (axis_y_log)
    {
      axis_y ();
...
#if QT_VERSION < 0x050000
    plotted_curves[wheel_file]->setSymbol (new QwtSymbol (
  ...
  if (le_wyatt_start->text ().isEmpty ()
    ...
    {
      disconnect (le_wyatt_start, SIGNAL (textChanged (const QString &)), 0,
                  0);
  ...
  if (le_wyatt_end->text ().isEmpty () || le_wyatt_end->text () == "0"
      || le_wyatt_end->text ().toDouble () > f_qs[wheel_file].back ())
    {
      ...
      le_wyatt_end->setText (
          QString ("%1").arg (f_qs[wheel_file][0] + q_len_delta));
      connect (le_wyatt_end, SIGNAL (textChanged (const QString &)),
               SLOT (wyatt_end_text (const QString &)));
    }

  if (le_wyatt_start2->text ().isEmpty () || le_wyatt_start2->text () == "0"
      || le_wyatt_start2->text ().toDouble () < f_qs[wheel_file][0])
    {
      disconnect (le_wyatt_start2, SIGNAL (textChanged (const QString &)), 0,
                  0);
      le_wyatt_start2->setText (
          QString ("%1").arg (f_qs[wheel_file].back () - q_len_delta));
      connect (le_wyatt_start2, SIGNAL (textChanged (const QString &)),
               SLOT (wyatt_start2_text (const QString &)));
    }

  if (le_wyatt_end2->text ().isEmpty () || le_wyatt_end2->text () == "0"
...
void
US_Hydrodyn_Saxs_Hplc::wyatt_enables ()
{
  ...
  wheel_enables (
#if QT_VERSION > 0x050000
      le_last_focus == le_wyatt_start || le_last_focus == le_wyatt_end
      || le_last_focus == le_wyatt_start2 || le_last_focus == le_wyatt_end2
#else
      le_wyatt_start->hasFocus () || le_wyatt_end->hasFocus ()
      || le_wyatt_start2->hasFocus () || le_wyatt_end2->hasFocus ()
#endif
  );
  pb_rescale->setEnabled (true);
  pb_rescale_y->setEnabled (true);
...
void
US_Hydrodyn_Saxs_Hplc::wyatt_init_markers ()
{
  gauss_delete_markers ();

  plotted_wyatt.clear ();

  gauss_add_marker (le_wyatt_start->text ().toDouble (), Qt::red,
                    us_tr ("Start"));
  gauss_add_marker (le_wyatt_end->text ().toDouble (), Qt::red, us_tr ("End"),
                    Qt::AlignLeft | Qt::AlignTop);

  if (cb_wyatt_2->isChecked ())
    {
      gauss_add_marker (le_wyatt_start2->text ().toDouble (), Qt::magenta,
                        us_tr ("Start"));
      gauss_add_marker (le_wyatt_end2->text ().toDouble (), Qt::magenta,
                        us_tr ("End"), Qt::AlignLeft | Qt::AlignTop);
    }
...
void
US_Hydrodyn_Saxs_Hplc::wyatt_apply ()
{
  wyatt_apply (all_selected_files ());
}

void
US_Hydrodyn_Saxs_Hplc::wyatt_apply (const QStringList &files)
{
      ...
      if (cb_wyatt_2->isChecked ())
        {
          ...
          double wyatt_error = wyatt_errors (
              files[i], le_wyatt_start->text ().toDouble (),
              le_wyatt_end->text ().toDouble (), wyatt_q, wyatt_I, wyatt_y);

          double wyatt_error2 = wyatt_errors (
              files[i], le_wyatt_start2->text ().toDouble (),
              le_wyatt_end2->text ().toDouble (), wyatt_q2, wyatt_I, wyatt_y);

          if (wyatt_error < 0e0 && wyatt_error2 < 0e0)
            {
              editor_msg (
                  "red",
                  QString (us_tr ("SD errors failed for %1")).arg (files[i]));
            }
          else
            {
              double use_e;
              if (wyatt_error < 0e0 || wyatt_error2 < 0e0)
                {
                  editor_msg (
                      "dark red",
                      QString (us_tr ("SD errors failed range %1:%2 for %3"))
                          .arg (wyatt_error < 0e0
              ...
              else
                {
                  double frac1
                      = (double)wyatt_q.size ()
                        / ((double)wyatt_q.size () + (double)wyatt_q2.size ());
                  double frac2 = 1e0 - frac1;

                  use_e = sqrt (frac1 * wyatt_error * wyatt_error
                                + frac2 * wyatt_error2 * wyatt_error2);
                }
              if (!constant_e (files[i], use_e))
                {
                  editor_msg (
                      "red",
                      QString (us_tr ("setting SD errors failed for %1"))
                          .arg (files[i]));
                }
            }
        }
        ...
        {
          double wyatt_error
              = wyatt_errors (files[i], le_wyatt_start->text ().toDouble (),
                              le_wyatt_end->text ().toDouble ());

          if (wyatt_error < 0e0 || !constant_e (files[i], wyatt_error))
            {
              editor_msg (
                  "red",
                  QString (us_tr ("SD errors failed for %1")).arg (files[i]));
...
void
US_Hydrodyn_Saxs_Hplc::wyatt_start_text (const QString &text)
{
  int pos = 0;
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Saxs_Hplc::wyatt_end_text (const QString &text)
{
  int pos = 1;
#if QT_VERSION < 0x040000
  plot_dist->setMarkerPos (plotted_markers[pos], text.toDouble (), 0e0);
...
void
US_Hydrodyn_Saxs_Hplc::wyatt_start_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Saxs_Hplc::wyatt_end_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Saxs_Hplc::wyatt_start2_text (const QString &text)
{
  int pos = 2;
#if QT_VERSION < 0x040000
  plot_dist->setMarkerPos (plotted_markers[pos], text.toDouble (), 0e0);
...
void
US_Hydrodyn_Saxs_Hplc::wyatt_end2_text (const QString &text)
{
  int pos = 3;
#if QT_VERSION < 0x040000
  plot_dist->setMarkerPos (plotted_markers[pos], text.toDouble (), 0e0);
...
void
US_Hydrodyn_Saxs_Hplc::wyatt_start2_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Saxs_Hplc::wyatt_end2_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Saxs_Hplc::replot_wyatt ()
{
  ...
  double wyatt_error = wyatt_errors (
      wheel_file, le_wyatt_start->text ().toDouble (),
      le_wyatt_end->text ().toDouble (), wyatt_q, wyatt_I, wyatt_y);
  ...
  if (wyatt_error >= 0e0)
    {
      ...
      plot_dist->setCurveData (curve, (double *)&wyatt_q[0],
                               (double *)&wyatt_y[0], wyatt_q.size ());
#else
      curve->setPen (QPen (Qt::green, use_line_width, Qt::SolidLine));
      curve->setSamples ((double *)&wyatt_q[0], (double *)&wyatt_y[0],
                         wyatt_q.size ());
      curve->attach (plot_dist);
#endif
    }

  QString msg = QString (us_tr ("SD t range %1:%2 SD %3"))
  ...
  if (cb_wyatt_2->isChecked ())
    {
      ...
      wyatt_error2 = wyatt_errors (
          wheel_file, le_wyatt_start2->text ().toDouble (),
          le_wyatt_end2->text ().toDouble (), wyatt_q2, wyatt_I2, wyatt_y2);
...
#if QT_VERSION < 0x040000
          plot_dist->setCurvePen (
              curve, QPen (Qt::yellow, use_line_width, Qt::SolidLine));
          plot_dist->setCurveData (curve, (double *)&wyatt_q2[0],
                                   (double *)&wyatt_y2[0], wyatt_q2.size ());
#else
          curve->setPen (QPen (Qt::yellow, use_line_width, Qt::SolidLine));
          curve->setSamples ((double *)&wyatt_q2[0], (double *)&wyatt_y2[0],
                             wyatt_q2.size ());
          curve->attach (plot_dist);
#endif
        }
      ...
      if (wyatt_error < 0e0 && wyatt_error2 < 0e0)
      ...
      else
        {
          ...
          msg += QString (us_tr (" t range2 %1:%2 SD %3  overall SD %4"))
      ...
      editor_msg (
          wyatt_error < 0e0 || wyatt_error2 < 0e0 ? "red" : "dark blue", msg);
    }
  else
    {
...
US_Hydrodyn_Saxs_Hplc::scale ()
{
  ...
  if (current_mode == MODE_TESTIQ)
    {
      if (!testiq_make () || !testiq_created_names.size ())
        {
          return;
        }
      for (int i = 0; i < (int)rb_testiq_gaussians.size (); ++i)
      ...
      if (lb_files->item (i)->isSelected ())
        {
          ...
          any_selected = true;
          scale_q[lb_files->item (i)->text ()]
              = f_qs[lb_files->item (i)->text ()];
          scale_I[lb_files->item (i)->text ()]
              = f_Is[lb_files->item (i)->text ()];
          scale_e[lb_files->item (i)->text ()]
              = f_errors[lb_files->item (i)->text ()];
          if (!plotted_curves.count (lb_files->item (i)->text ()))
            {
              editor_msg ("red",
                          QString (us_tr ("Internal error: scale selected %1, "
                                          "but no plotted curve found"))
                              .arg (lb_files->item (i)->text ()));
              return;
            }
  ...
  if (!any_selected)
    {
      editor_msg ("red",
                  us_tr ("Internal error: no files selected in scale mode"));
      return;
    }
  ...
  if (le_scale_q_start->text ().isEmpty () || le_scale_q_start->text () == "0"
      || le_scale_q_start->text ().toDouble () < scale_q[wheel_file][0]
      || le_scale_q_start->text ().toDouble () > scale_q[wheel_file].back ())
    {
      disconnect (le_scale_q_start, SIGNAL (textChanged (const QString &)), 0,
                  0);
  ...
  if (le_scale_q_end->text ().isEmpty () || le_scale_q_end->text () == "0"
      || le_scale_q_end->text ().toDouble () < scale_q[wheel_file][0]
      || le_scale_q_end->text ().toDouble () > scale_q[wheel_file].back ())
    {
      disconnect (le_scale_q_end, SIGNAL (textChanged (const QString &)), 0,
                  0);
      le_scale_q_end->setText (
          QString ("%1").arg (scale_q[wheel_file].back ()));
  ...
  plotted_markers.clear ();
  gauss_add_marker (le_scale_q_start->text ().toDouble (), Qt::red,
                    us_tr ("Start"));
  gauss_add_marker (le_scale_q_end->text ().toDouble (), Qt::red,
                    us_tr ("End"), Qt::AlignLeft | Qt::AlignTop);
  plot_dist->replot ();
...
void
US_Hydrodyn_Saxs_Hplc::scale_scroll_highlight (int pos)
{
...
#if QT_VERSION < 0x040000
  plot_dist->setCurveStyle (
      plotted_curves[scale_scroll_selected[scale_scroll_pos]],
      QwtCurve::NoCurve);
  plot_dist->setCurveStyle (plotted_curves[scale_scroll_selected[pos]],
                            QwtCurve::Lines);
  if (scale_plotted_errors.count (scale_scroll_selected[scale_scroll_pos]))
    {
      plot_errors->setCurveStyle (
          scale_plotted_errors[scale_scroll_selected[scale_scroll_pos]],
          QwtCurve::NoCurve);
    }
  if (scale_plotted_errors.count (scale_scroll_selected[pos]))
    {
      plot_errors->setCurveStyle (
          scale_plotted_errors[scale_scroll_selected[pos]], QwtCurve::Lines);
    }
#else
  plotted_curves[scale_scroll_selected[scale_scroll_pos]]->setStyle (
      QwtPlotCurve::NoCurve);
  plotted_curves[scale_scroll_selected[pos]]->setStyle (QwtPlotCurve::Lines);
  if (scale_plotted_errors.count (scale_scroll_selected[scale_scroll_pos]))
    {
      scale_plotted_errors[scale_scroll_selected[scale_scroll_pos]]->setStyle (
          QwtPlotCurve::NoCurve);
    }
  if (scale_plotted_errors.count (scale_scroll_selected[pos]))
    {
      scale_plotted_errors[scale_scroll_selected[pos]]->setStyle (
          QwtPlotCurve::Lines);
    }
#endif
  plot_dist->replot ();
...
void
US_Hydrodyn_Saxs_Hplc::scale_scroll ()
{
  ...
  if (cb_scale_scroll->isChecked ())
    {
      ...
      for (set<QString>::iterator it = scale_selected.begin ();
           it != scale_selected.end (); ++it)
        {
          ...
          if (scale_plotted_errors.count (*it))
            {
              plot_errors->setCurveStyle (scale_plotted_errors[*it],
                                          QwtCurve::NoCurve);
            }
#else
      ...
      wheel_enables (false);
      qwtw_wheel->setRange (0, scale_scroll_selected.size () - 1);
      qwtw_wheel->setSingleStep (1);
      qwtw_wheel->setValue (scale_scroll_pos);
      lbl_wheel_pos->setText (
          QString ("%1").arg (scale_scroll_selected[scale_scroll_pos]));
      connect (qwtw_wheel, SIGNAL (valueChanged (double)),
               SLOT (adjust_wheel (double)));
      wheel_enables ();
...
void
US_Hydrodyn_Saxs_Hplc::scale_update_plot_errors ()
{
  ...
  // us_qdebug( US_Vector::qs_vector2( QString( "spline avg target %1" ).arg(
  // scale_applied_target ), scale_spline_x, scale_spline_y ) );
  ...
  for (set<QString>::iterator it = scale_selected.begin ();
       it != scale_selected.end (); ++it)
    {
      ...
      for (int i = 0; i < (int)scale_q[*it].size (); ++i)
        {
          if (scale_q[*it][i] >= q_min && scale_q[*it][i] <= q_max)
            {
              if (usu->apply_natural_spline (scale_spline_x, scale_spline_y,
                                             scale_spline_y2, scale_q[*it][i],
                                             y))
                {
      ...
      curve->setPen (QPen (plot_colors[f_pos[*it] % plot_colors.size ()],
                           use_line_width, Qt::SolidLine));
      curve->setSamples ((double *)&this_q[0], (double *)&this_diff[0],
                         this_q.size ());
      curve->attach (plot_errors);
#else
      long curve;
      curve = plot_errors->insertCurve (*it);
      plot_errors->setCurveStyle (curve, hide ? QwtPlotCurve::NoCurve
                                              : QwtPlotCurve::Lines);
      plot_errors->setCurvePen (
          curve, QPen (plot_colors[f_pos[*it] % plot_colors.size ()],
                       use_line_width, Qt::SolidLine));
      plot_errors->setCurveData (curve, (double *)&this_q[0],
                                 (double *)&this_diff[0], this_q.size ());
#endif
      scale_plotted_errors[*it] = curve;
      // us_qdebug( US_Vector::qs_vector2( QString( "diff plot %1" ).arg( *it
      // ), this_q, this_diff ) );
    }

  plot_errors->setAxisScale (QwtPlot::yLeft, diff_ymin * 1.1e0,
                             diff_ymax * 1.1e0);
  ...
  // connect( plot_errors_zoomer, SIGNAL( zoomed( const QRectF & ) ), SLOT(
  // plot_zoomed( const QRectF & ) ) );
...
void
US_Hydrodyn_Saxs_Hplc::scale_enables ()
{
  pb_errors->setEnabled (true);
...
QString
US_Hydrodyn_Saxs_Hplc::scale_get_target (bool do_msg)
{
  double min_i = 0e0;
  double max_i = 0e0;
  double tot_i = 0e0;
  ...
  if (q_min >= q_max)
    {
      editor_msg ("red", us_tr ("Error: Scale mode: minimum q value is "
                                "greator or equal to maximum q value"));
      return "";
    }
  ...
  msg += QString ("\nScale %1 files from q: %2 %3\n")
             .arg (scale_selected.size ())
             .arg (q_min)
             .arg (q_max);

  msg += QString ("minimum total I : %1 %2\n"
                  "maximum total I : %3 %4\n"
                  "average total I : %5\n")
             .arg (min_file)
             .arg (min_i)
...
void
US_Hydrodyn_Saxs_Hplc::scale_apply ()
{
  ...
  saxs_options *our_saxs_options
      = &(((US_Hydrodyn *)us_hydrodyn)->saxs_options);

  vector<double> target_q;
  vector<double> target_I;
  vector<double> target_e;

  bool target_has_e = ((scale_q[target].size () == scale_e[target].size ())
  ...
  for (int i = 0; i < (int)scale_q[target].size (); ++i)
    {
      if (scale_q[target][i] >= q_min && scale_q[target][i] <= q_max)
        {
  ...
  if (!target_q.size ())
    {
      editor_msg (
          "red",
          QString (us_tr ("Error: Scale 'apply': the target curve for scaling "
                          "has no points in the selected q range")));
      scale_enables ();
      return;
    }
  ...
  if (do_chi2_fitting && !target_has_e)
    {
      notices += us_tr (
          "\nChi^2 fitting requested, but target data has no or some zero "
          "standard deviation data. Chi^2 fitting not used");
      do_chi2_fitting = false;
    }

  if (our_saxs_options->iqq_scale_nnls && do_chi2_fitting
      && our_saxs_options->iqq_scale_chi2_fitting)
    {
      notices += us_tr ("\nChi^2 fitting is currently not compatable with "
                        "NNLS scaling, Chi^2 fitting not used");
      do_chi2_fitting = false;
    }
      ...
      if (*it != target)
        {
          ...
          if (!usu->interpolate (source_I, target_q, scale_q[*it],
                                 scale_I[*it]))
            {
              editor_msg (
                  "red",
                  QString (
                      us_tr ("Error: Scale 'apply': %1 could not interpolate "
                             "to target on specified q range: %2"))
                      .arg (*it)
                      .arg (usu->errormsg));
              scale_enables ();
              return;
          ...
          if (our_saxs_options->iqq_scale_nnls)
            {
              usu->nnls_fit (source_I, target_I, k[*it], chi2[*it]);
            }
          else
            {
              if (do_chi2_fitting)
                {
                  usu->scaling_fit (source_I, target_I, target_e, k[*it],
                                    chi2[*it]);
                }
              else
                {
                  usu->scaling_fit (source_I, target_I, k[*it], chi2[*it]);
                }
            }

          {
            int max = (int)average_scaled_I.size ();
            max = (int)scale_I[*it].size () < max ? (int)scale_I[*it].size ()
                                                  : max;
            for (int i = 0; i < max; ++i)
              {
  ...
  // us_qdebug( US_Vector::qs_vector2( "scale_q[ target ], average_scaled_I",
  // scale_q[ target ], average_scaled_I ) );
      ...
      if (*it != target)
        {
          ...
          // us_qdebug( US_Vector::qs_vector2( QString( "%1 scale_q, scale_I"
          // ).arg( *it ), scale_q[ *it ], scale_I[ *it ] ) );
          fit_msg += QString ("%1 : scale %2 ").arg (*it).arg (k[*it]);
          if (do_chi2_fitting)
            {
              // usu.calc_chisq_prob( 0.5 * target_I.size() - (
              // do_scale_linear_offset ? 2 : 1 ), 0.5 * chi2[ *it ], chi2_prob
              // );
              fit_msg
                  += QString ("chi^2=%1 df=%2 nchi=%3")
                         .arg (chi2[*it], 6)
                         .arg (target_I.size ()
                               - (do_scale_linear_offset ? 2 : 1))
                         .arg (sqrt (chi2[*it]
                                     / (target_I.size ()
                                        - (do_scale_linear_offset ? 2 : 1))),
                               5);
          ...
          fit_msg += "\n";
          // } else {
          //    us_qdebug( US_Vector::qs_vector2( QString( "%1 scale_q,
          //    scale_I" ).arg( *it ), scale_q[ *it ], scale_I[ *it ] ) );
        }
    }
  ...
  // setup for scale analysis
  {
    ...
    map<QString, vector<vector<double> > >
        plots; // name, x, y and possibly error
    ...
    for (set<QString>::iterator it = scale_selected.begin ();
         it != scale_selected.end (); ++it)
      {
        QString name = QString ("%1_scaled_q%2_%3")
                           .arg (*it)
...

        {
          ...
          parameters[QString ("_color:%1").arg (name)] = QString ("%1").arg (
              (double)(tmp.red () * 256 + tmp.green ()) * 256 + tmp.blue ());
        }
        ...
        if (cb_scale_save_intp->isChecked () && *it != scale_applied_target)
          {
            ...
            for (int i = 0; i < (int)scale_q[scale_applied_target].size ();
                 ++i)
              {
                if (scale_q[scale_applied_target][i] >= q_min
                    && scale_q[scale_applied_target][i] <= q_max)
            ...
            if (!usu->interpolate (source_I, target_q, scale_q[*it],
                                   scale_I[*it]))
              {
                editor_msg (
                    ...
                    QString (
                        us_tr (
                            "Error: Scale 'apply': %1 could not interpolate "
                            "to target on specified q range: %2"))
                        .arg (*it)
                        .arg (usu->errormsg));
                scale_enables ();
            ...
            if (scale_e[*it].size () == scale_q[*it].size ())
              {
                if (!usu->interpolate (source_e, target_q, scale_q[*it],
                                       scale_e[*it]))
                  {
                    editor_msg (
                        "red",
                        QString (us_tr ("Error: Scale 'apply': %1 could not "
                                        "interpolate to target on specified q "
                                        "range: %2"))
                            .arg (*it)
                            .arg (usu->errormsg));
                    scale_enables ();
    ...
    // US_Hydrodyn_Saxs_Hplc_Scale_Trend * uhshst = new
    // US_Hydrodyn_Saxs_Hplc_Scale_Trend( us_hydrodyn,
    //                                                                                     parameters,
    //                                                                                     plots );
    // uhshst->show();
  }
...
void
US_Hydrodyn_Saxs_Hplc::scale_q_reset ()
{
  le_scale_q_start->setText (QString ("%1").arg (scale_q[wheel_file][0]));
...
void
US_Hydrodyn_Saxs_Hplc::scale_reset ()
{
  for (set<QString>::iterator it = scale_selected.begin ();
       it != scale_selected.end (); ++it)
    {
...
void
US_Hydrodyn_Saxs_Hplc::scale_replot ()
{
  for (set<QString>::iterator it = scale_selected.begin ();
       it != scale_selected.end (); ++it)
    {
#if QT_VERSION < 0x040000
      plot_dist->setCurveData (
          plotted_curves[*it], (double *)&(scale_q[*it][0]),
          (double *)&(scale_I[*it][0]), scale_q[*it].size ());
#else
      plotted_curves[*it]->setSamples ((double *)&(scale_q[*it][0]),
                                       (double *)&(scale_I[*it][0]),
                                       scale_q[*it].size ());
#endif
    }
...
void
US_Hydrodyn_Saxs_Hplc::scale_create ()
{
  scale_last_created.clear ();
      ...
      if (cb_scale_save_intp->isChecked () && *it != scale_applied_target)
        {
          ...
          add_plot (name, target_q, source_I, source_e, f_is_time[*it], false);
        }
      else
        {
          add_plot (name, scale_q[*it], scale_I[*it], scale_e[*it],
                    f_is_time[*it], false);
        }
...
void
US_Hydrodyn_Saxs_Hplc::scale_q_start_text (const QString &text)
{
  if (current_mode != MODE_SCALE)
    {
      return;
...
void
US_Hydrodyn_Saxs_Hplc::scale_q_end_text (const QString &text)
{
  if (current_mode != MODE_SCALE)
    {
      return;
...
void
US_Hydrodyn_Saxs_Hplc::scale_q_start_focus (bool hasFocus)
{
  if (current_mode != MODE_SCALE)
    {
      return;
  ...
  if (hasFocus && scale_q.count (wheel_file))
    {
      ...
      disconnect (qwtw_wheel, SIGNAL (valueChanged (double)), 0, 0);
      qwtw_wheel->setRange (scale_q[wheel_file][0],
                            scale_q[wheel_file].back ());
      qwtw_wheel->setSingleStep (
          (scale_q[wheel_file].back () - scale_q[wheel_file][0])
          / UHSH_WHEEL_RES);
      connect (qwtw_wheel, SIGNAL (valueChanged (double)),
               SLOT (adjust_wheel (double)));
...
void
US_Hydrodyn_Saxs_Hplc::scale_q_end_focus (bool hasFocus)
{
  if (current_mode != MODE_SCALE)
    {
      return;
...
void
US_Hydrodyn_Saxs_Hplc::ggauss_start ()
{
  plot_errors->detachItems (QwtPlotItem::Rtti_PlotCurve);
  plot_errors->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
  ...
  QTextStream (stdout) << "ggaussian_sel_no_gaussian_count() "
                       << no_gaussian_count << Qt::endl;
  QTextStream (stdout) << "total selected "
                       << ggaussian_selected_file_index.size () << Qt::endl;
  ...
  if (not_compatible)
    {
      ...
      QString msg
          = QString (
                us_tr ("NOTICE: Some files selected have Gaussians with "
                       "varying centers%1 or a different number of Gaussians "
                       "or centers that do not match the last Gaussians, "
                       "these will be reset to the last Gaussian settings"))
                .arg (msg_addendum);
  ...
  if (!cb_sd_weight->isChecked ())
    {
      switch (QMessageBox::question (
          this, windowTitle () + us_tr (": Global Gaussians"),
          QString (us_tr ("Note: SD weighting is currently turned off.\n")),
          us_tr ("&Turn on"), us_tr ("&Leave off"), QString (),
          0, // Stop == button 0
          0  // Escape == button 0
          ))
        {
    ...
    if (not_compatible)
      {
        QMessageBox::information (
            ...
            QString (
                us_tr ("NOTICE: Some files selected have Gaussians with "
                       "varying centers%1 or\na different number of Gaussians "
                       "or centers that do not match the last Gaussians.\n"))
                    .arg (msg_addendum)
    ...
    else
      {
        if (no_gaussian_count)
          {
            if (no_gaussian_count
                == (int)ggaussian_selected_file_index.size ())
              {
                QMessageBox::information (
                    this, windowTitle () + us_tr (": Global Gaussians"),
                    msg_addendum + "\n"
            ...
            else
              {
                switch (QMessageBox::question (
                    this, windowTitle () + us_tr (": Global Gaussians"),
                    QString (us_tr ("%1 selected curves do not have defined "
                                    "Gaussians and must be initialized"))
                        .arg (no_gaussian_count),
                    us_tr ("&Reinitialize only curves with missing Gaussians"),
                    us_tr (
                        "&Reinitialize all curves with last used Gaussians"),
                    QString (),
                    0, // Stop == button 0
                    0  // Escape == button 0
                    ))
        ...
        else
          {
            ...
            switch (QMessageBox::question (
                ...
                QString (us_tr (
                    ...
                    "Answer \"Yes\" if you want to re-fit the amplitudes "
                    "using the last single curve Gaussian known"
                    " [the most recent of a Gaussian file load or single "
                    "Gaussian \"Keep\"].\n\n"
                    "Answer \"No\" if you want to maintain the current "
                    "values.")),
  ...
  // add_ggaussian_curve( "unified_ggaussian_sum",
  ...
  // us_qdebug( QString( "setting wheel pos to %1 gaussian pos %2" ).arg(
  // unified_ggaussian_params[ gaussian_pos ] ).arg( gaussian_pos ) );
  update_gauss_pos ();
  qwtw_wheel->setValue (le_gauss_pos->text ().toDouble ());
  us_qdebug (QString ("setting wheel pos to %1 gaussian pos %2")
...
void
US_Hydrodyn_Saxs_Hplc::ggauss_scroll ()
{
  if (cb_ggauss_scroll->isChecked ())
  ...
  else
    {
      ...
      for (int i = 0; i < (int)ggaussian_selected_file_index.size (); ++i)
        {
          lb_files->item (ggaussian_selected_file_index[i])
              ->setSelected (true);
        }
      disable_updates = false;
...
void
US_Hydrodyn_Saxs_Hplc::ggauss_scroll_p_green ()
{
...
void
US_Hydrodyn_Saxs_Hplc::ggauss_scroll_p_red ()
{
...
void
US_Hydrodyn_Saxs_Hplc::ggauss_scroll_p_yellow ()
{
  us_qdebug ("ggauss_scroll_p_yellow()");
  ggauss_scroll_set_selected ();
}
...
void
US_Hydrodyn_Saxs_Hplc::ggauss_scroll_smoothed ()
{
...
void
US_Hydrodyn_Saxs_Hplc::ggauss_scroll_oldstyle ()
{
...
void
US_Hydrodyn_Saxs_Hplc::ggauss_scroll_set_selected ()
{
  ...
  if (!unified_ggaussian_ok)
    {
      editor_msg (
          "red",
          us_tr ("Internal error (ggauss_scroll_set_selected): Global "
                 "Gaussian mode, but unified Global Gaussians are not ok."));
      ggaussian_enables ();
      return;
    }

  if ((int)unified_ggaussian_files.size ()
          != (int)ggaussian_last_pfit_P.size ()
      || (int)unified_ggaussian_files.size ()
             != (int)ggaussian_last_pfit_N.size ()
      || (int)unified_ggaussian_files.size ()
             != (int)ggaussian_last_pfit_C.size ()
      || (int)unified_ggaussian_files.size ()
             != (int)ggaussian_last_pfit_S.size ()
      || (int)unified_ggaussian_files.size ()
             != (int)ggaussian_last_chi2.size ()
      || lbl_gauss_fit->text () == "?" || pb_ggauss_rmsd->isEnabled ())
    {
      if (ggauss_recompute ())
        {
        ...
        {
          editor_msg ("red",
                      us_tr ("Internal error (ggauss_scroll_set_selected): "
                             "building global Gaussians"));
          ggaussian_enables ();
          return;
        }
    }
  ...
  if ((int)unified_ggaussian_files.size ()
      != (int)ggaussian_last_pfit_P.size ())
    {
      editor_msg ("red", us_tr ("Internal error (ggauss_scroll_set_selected): "
                                "P set does not match Gaussian size"));
      ggaussian_enables ();
      return;
    }
  ...
  if ((int)unified_ggaussian_files.size () != (int)ggaussian_last_chi2.size ())
    {
      editor_msg ("red", us_tr ("Internal error (ggauss_scroll_set_selected): "
                                "chi2 set does not match Gaussian size"));
      ggaussian_enables ();
      return;
    }
  ...
  //    us_qdebug( QString( "active %1 file %2" ).arg( ggauss_scroll_set[ i ]
  //    ).arg( unified_ggaussian_files[ ggauss_scroll_set [ i ] ] ) );
  // }

  if (ggauss_scroll_set.size ())
    {
      disconnect (qwtw_wheel, SIGNAL (valueChanged (double)), 0, 0);
      qwtw_wheel->setRange (0, ggauss_scroll_set.size () - 1);
      qwtw_wheel->setSingleStep (1);
      connect (qwtw_wheel, SIGNAL (valueChanged (double)),
               SLOT (adjust_wheel (double)));
  ...
  else
    {
      ...
      lbl_wheel_pos_below->setText (
          us_tr ("-- no curves match current selection --"));
      disable_updates = true;
      gauss_delete_markers ();
      lb_files->clearSelection ();
      disable_updates = false;
      ...
      // clear highlighted ggqfit_plot for chi2 and pfit
      {
...
#if QT_VERSION >= 0x040000
#if QT_VERSION < 0x050000
        for (set<QwtPlotMarker *>::iterator it
             = ggaussian_pts_chi2_marked.begin ();
             it != ggaussian_pts_chi2_marked.end (); it++)
          {
            sym = (*it)->symbol ();
            sym.setSize (use_line_width * 3 + 1);
            (*it)->setSymbol (new QwtSymbol (sym.style (), sym.brush (),
                                             sym.pen (), sym.size ()));
          }
        for (set<QwtPlotMarker *>::iterator it
             = ggaussian_pts_pfit_marked.begin ();
             it != ggaussian_pts_pfit_marked.end (); it++)
          {
            sym = (*it)->symbol ();
        ...
        for (set<QwtPlotMarker *>::iterator it
          ...
          {
            sym = (*it)->symbol ();
            (*it)->setSymbol (new QwtSymbol (
                sym->style (), sym->brush (), sym->pen (),
                QSize (use_line_width * 3 + 1, use_line_width * 3 + 1)));
          }
        for (set<QwtPlotMarker *>::iterator it
             = ggaussian_pts_pfit_marked.begin ();
             it != ggaussian_pts_pfit_marked.end (); it++)
          {
            ...
            (*it)->setSymbol (new QwtSymbol (
                sym->style (), sym->brush (), sym->pen (),
                QSize (use_line_width * 2 + 1, use_line_width * 2 + 1)));
          }
#endif
#else
...
void
US_Hydrodyn_Saxs_Hplc::ggauss_scroll_highlight (int pos)
{
  ...
  if (pos >= (int)ggauss_scroll_set.size () || pos < 0)
    {
      editor_msg (
          "red",
          us_tr (
              "Internal error ggauss_scroll_highlight: pos >= size or < 0"));
      lbl_wheel_pos_below->setText ("");
      {
        QPixmap pm;
        lbl_wheel_Pcolor->setPixmap (pm);
  ...
  if (ggaussian_last_pfit_P[ggauss_scroll_set[pos]] >= 0.05)
    {
      pm.convertFromImage (qi_green->smoothScale (
          lbl_wheel_Pcolor->width () / 2, lbl_wheel_Pcolor->height () / 2,
          Qt::KeepAspectRatio));
    }
  else
    {
      if (ggaussian_last_pfit_P[ggauss_scroll_set[pos]] >= 0.01)
        {
          pm.convertFromImage (qi_yellow->smoothScale (
              lbl_wheel_Pcolor->width () / 2, lbl_wheel_Pcolor->height () / 2,
              Qt::KeepAspectRatio));
        }
      else
        {
          pm.convertFromImage (qi_red->smoothScale (
              lbl_wheel_Pcolor->width () / 2, lbl_wheel_Pcolor->height () / 2,
              Qt::KeepAspectRatio));
        }
    }
  ...
  if (ggaussian_last_pfit_P[ggauss_scroll_set[pos]] >= 0.05)
    {
      pm.convertFromImage (qi_green->scaled (
          lbl_wheel_Pcolor->width () / 2, lbl_wheel_Pcolor->height () / 2,
  ...
  else
    {
      if (ggaussian_last_pfit_P[ggauss_scroll_set[pos]] >= 0.01)
        {
          pm.convertFromImage (qi_yellow->scaled (
              lbl_wheel_Pcolor->width () / 2, lbl_wheel_Pcolor->height () / 2,
              Qt::KeepAspectRatio, Qt::SmoothTransformation));
        }
      else
        {
          pm.convertFromImage (qi_red->scaled (
              lbl_wheel_Pcolor->width () / 2, lbl_wheel_Pcolor->height () / 2,
              Qt::KeepAspectRatio, Qt::SmoothTransformation));
        }
    }
  ...
  lbl_wheel_pos_below->setText (
      QString ("%1 %2 %3 P %4%5")
          ...
          .arg (unified_ggaussian_use_errors && cb_sd_weight->isChecked ()
                    ? "nChi^2"
                    : "RMSD")
          .arg (ggaussian_last_chi2[ggauss_scroll_set[pos]], 0, 'g', 4)
          .arg (ggaussian_last_pfit_P[ggauss_scroll_set[pos]], 0, 'f', 4)
          .arg (f_best_smoothed_smoothing.count (
                    unified_ggaussian_files[ggauss_scroll_set[pos]])
                    ? QString ("\nSmoothing points %1")
                          .arg (f_best_smoothed_smoothing
                                    [unified_ggaussian_files
                                         [ggauss_scroll_set[pos]]])
                    : QString ("")));
  ...
  lb_files->clearSelection ();
  gauss_delete_markers ();
  lb_files->item (ggaussian_selected_file_index[ggauss_scroll_set[pos]])
      ->setSelected (true);
  lb_files->setCurrentItem (
      lb_files->item (ggaussian_selected_file_index[ggauss_scroll_set[pos]]));
  ...
  if (f_best_smoothed_smoothing.count (
          unified_ggaussian_files[ggauss_scroll_set[pos]]))
    {
      QTextStream (stdout)
          << QString ("found smoothed curved for %1\n")
                 .arg (unified_ggaussian_files[ggauss_scroll_set[pos]]);

      QPen use_pen
          = QPen (QColor ("#ffb29b"), use_line_width + 1, Qt::DashDotDotLine);
      QwtPlotCurve *curve = new QwtPlotCurve ("gg_scroll_gaussian_smoothed");
      curve->setStyle (QwtPlotCurve::Lines);
      curve->setSamples (
          (double *)&(
              f_qs_smoothed[unified_ggaussian_files[ggauss_scroll_set[pos]]]
                           [0]),
          (double *)&(
              f_Is_smoothed[unified_ggaussian_files[ggauss_scroll_set[pos]]]
                           [0]),
          f_qs_smoothed[unified_ggaussian_files[ggauss_scroll_set[pos]]]
              .size ());
      curve->setPen (use_pen);
  ...
  if (cb_ggauss_scroll_oldstyle->isChecked ()
    ...
    {
      QTextStream (stdout)
          << QString ("found oldstyle curved for %1\n")
                 .arg (unified_ggaussian_files[ggauss_scroll_set[pos]]);

      QPen use_pen
          = QPen (QColor ("#ffffff"), use_line_width + 1, Qt::DashLine);
      QwtPlotCurve *curve = new QwtPlotCurve ("gg_scroll_gaussian_oldstyle");
      curve->setStyle (QwtPlotCurve::Lines);
      curve->setSamples (
          (double *)&(
              f_qs_oldstyle[unified_ggaussian_files[ggauss_scroll_set[pos]]]
                           [0]),
          (double *)&(
              f_Is_oldstyle[unified_ggaussian_files[ggauss_scroll_set[pos]]]
                           [0]),
          f_qs_oldstyle[unified_ggaussian_files[ggauss_scroll_set[pos]]]
              .size ());
      curve->setPen (use_pen);
...

  {
    ...
    plot_dist->setCurveStyle (curve, QwtCurve::Lines);
    plot_dist->setCurveData (
        curve, (double *)&(ggaussian_last_gg_t[ggauss_scroll_set[pos]][0]),
        (double *)&(ggaussian_last_gg[ggauss_scroll_set[pos]][0]),
        ggaussian_last_gg[ggauss_scroll_set[pos]].size ());
...

  {
    ...
    for (int i = 0;
         i < (int)ggaussian_last_ggig[ggauss_scroll_set[pos]].size (); ++i)
      {

#if QT_VERSION >= 0x040000
        QwtPlotCurve *curve
            = new QwtPlotCurve ("gg_scroll_gaussian_individual");
        curve->setStyle (QwtPlotCurve::Lines);
        curve->setSamples (
            (double *)&(ggaussian_last_gg_t[ggauss_scroll_set[pos]][0]),
  ...
  // update plot errors
  {
    ...
    bool use_errors
        = is_nonzero_vector (ggaussian_last_e[ggauss_scroll_set[pos]]);
    if (plot_errors->isVisible ())
      {
        hide_widgets (plot_errors_widgets, false);
    ...
    update_plot_errors (
        ...
        plot_colors[ggaussian_selected_file_index[ggauss_scroll_set[pos]]
                    % plot_colors.size ()]);
    // redo_plot_errors();
  }
    ...
    if (ggaussian_pts_chi2.size ())
      {
        ...
        marker->setSymbol (new QwtSymbol (
            sym->style (), sym->brush (), sym->pen (),
            QSize (use_line_width * 7 + 1, use_line_width * 7 + 1)));
        ggaussian_pts_chi2_marked.insert (marker);
      }
    ...
    if (ggaussian_pts_pfit.size ())
      {
        ...
        marker->setSymbol (new QwtSymbol (
            sym->style (), sym->brush (), sym->pen (),
            QSize (use_line_width * 6 + 1, use_line_width * 6 + 1)));
        ggaussian_pts_pfit_marked.insert (marker);
      }
#endif
#else
...
void
US_Hydrodyn_Saxs_Hplc::ggaussian_enables ()
{
  ...
  if (cb_ggauss_scroll->isChecked ())
  ...
  else
    {
      ...
      pb_gauss_prev->setEnabled (unified_ggaussian_gaussians_size > 1
                                 && gaussian_pos > 0);
      pb_gauss_next->setEnabled (
          unified_ggaussian_gaussians_size > 1
          && gaussian_pos < unified_ggaussian_gaussians_size - 1);
      cb_sd_weight->setEnabled (
          unified_ggaussian_gaussians_size
          && le_gauss_fit_start->text ().toDouble ()
                 < le_gauss_fit_end->text ().toDouble ());
      pb_gauss_fit->setEnabled (
          unified_ggaussian_gaussians_size
          && le_gauss_fit_start->text ().toDouble ()
                 < le_gauss_fit_end->text ().toDouble ());
      le_gauss_pos->setEnabled (unified_ggaussian_gaussians_size
                                && gaussian_pos
                                       < unified_ggaussian_gaussians_size);
      le_gauss_pos_width->setEnabled (
          cb_fix_width->isChecked () && unified_ggaussian_gaussians_size
          && gaussian_pos < unified_ggaussian_gaussians_size);
      le_gauss_pos_dist1->setEnabled (
          cb_fix_dist1->isChecked () && unified_ggaussian_gaussians_size
          && gaussian_pos < unified_ggaussian_gaussians_size);
      le_gauss_pos_dist2->setEnabled (
          cb_fix_dist2->isChecked () && unified_ggaussian_gaussians_size
          && gaussian_pos < unified_ggaussian_gaussians_size);
      le_gauss_fit_start->setEnabled (
          unified_ggaussian_gaussians_size
          && gaussian_pos < unified_ggaussian_gaussians_size);
      le_gauss_fit_end->setEnabled (unified_ggaussian_gaussians_size
                                    && gaussian_pos
                                           < unified_ggaussian_gaussians_size);
      wheel_enables (unified_ggaussian_gaussians_size
                     && gaussian_pos < unified_ggaussian_gaussians_size);
...
void
US_Hydrodyn_Saxs_Hplc::timeshift ()
{
  ...
  //                                    us_tr( "SOMO: HPLC timeshift: select
  //                                    file" ), us_tr("Select the curve to
  //                                    timeshift:\n" ), selected_files, 0,
  //                                    false,
  //                                    &ok,
  //                                    this );
  ...
  if (!plotted_curves.count (wheel_file))
    {
      editor_msg ("red",
                  QString (us_tr ("Internal error: request to timeshift %1, "
  ...
  // compute a good range
  {
    ...
    // qwtw_wheel->setRange( minq_w, maxq_w); qwtw_wheel->setSingleStep(
    // (maxq_w - minq_w ) / UHSH_WHEEL_RES ); qwtw_wheel            ->setValue
    // ( ( minq_w + maxq_w ) * 5e-1 );
    ...
    qwtw_wheel->setRange (minq_w, maxq_w);
    qwtw_wheel->setSingleStep ((maxq_w - minq_w) / UHSH_WHEEL_RES);
    qwtw_wheel->setValue (center);
  }
}

void
US_Hydrodyn_Saxs_Hplc::timescale ()
{
  timescale (all_selected_files ());
}

void
US_Hydrodyn_Saxs_Hplc::timescale (const QStringList &files)
{
  bool ok;
  double res
      = US_Static::getDouble (us_tr ("US-SOMO: Saxs Hplc: Time scale"),
                              us_tr ("Enter a multiplier to scale the time"),
                              1e0, 1e-5, 1e7, 6, &ok, this);
  if (!ok)
    {
      // user pressed Cancel
      update_enables ();
