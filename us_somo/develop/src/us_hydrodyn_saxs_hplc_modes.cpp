...
void US_Hydrodyn_Saxs_Hplc::pm() {
  ...
  if (f_qs[wheel_file].size() < 2) {
    editor_msg(
        "red",
        QString(us_tr("Internal error: %1 almost empty data")).arg(wheel_file));
    return;
  }
...
void US_Hydrodyn_Saxs_Hplc::rgc_enables() {
  ...
  if (rb_rgc_shape_ellipsoid->isChecked()) {
    ...
    if (rb_rgc_shape_oblate->isChecked() || rb_rgc_shape_prolate->isChecked()) {
      le_rgc_axis_b->show();
      le_rgc_axis_c->hide();
    } else {
      le_rgc_axis_b->hide();
...
void US_Hydrodyn_Saxs_Hplc::rgc_calc_rg() {
  ...
  if (rb_rgc_shape_ellipsoid->isChecked()) {
    abc4o3pi *=
        le_rgc_axis_b->text().toDouble() * le_rgc_axis_c->text().toDouble();
  } else {
    if (rb_rgc_shape_oblate->isChecked()) {
      ...
      if (rb_rgc_shape_prolate->isChecked()) {
        abc4o3pi *=
            le_rgc_axis_b->text().toDouble() * le_rgc_axis_b->text().toDouble();
      }
    }
  }
...
void US_Hydrodyn_Saxs_Hplc::rgc_mw_text(const QString &) { rgc_calc_rg(); }
...
void US_Hydrodyn_Saxs_Hplc::rgc_rho_text(const QString &) { rgc_calc_rg(); }

void US_Hydrodyn_Saxs_Hplc::rgc_axis_b_text(const QString &,
                                            bool do_recompute) {
...
void US_Hydrodyn_Saxs_Hplc::rgc_axis_c_text(const QString &,
                                            bool do_recompute) {
  if (le_rgc_axis_b->text().toDouble() < le_rgc_axis_c->text().toDouble()) {
...
void US_Hydrodyn_Saxs_Hplc::testiq() {
  ...
  for (int i = 0; i < lb_files->count(); i++) {
    if (lb_files->item(i)->isSelected()) {
      ...
      if (!plotted_curves.count(lb_files->item(i)->text())) {
        editor_msg("red", QString(us_tr("Internal error: testiq selected %1, "
  ...
  if (le_testiq_q_start->text().isEmpty() || le_testiq_q_start->text() == "0" ||
  ...
  if (current_mode == MODE_GUINIER || current_mode == MODE_SCALE) {
    ...
    // figure out which testiq_it to select
    {
      ...
      // us_qdebug( QString( "testiq thresh %1 max_i %2 avg_i %3" ).arg( thresh
      // ).arg( max_i ).arg( avg_i ) );
      ...
      for (list<testiq_sortable_double>::iterator it = tsdl.begin();
           it != tsdl.end(); ++it) {
        // us_qdebug( QString( "it loop x %1 index %2 Imin %3 Imax %4" ).arg(
        // it->x ).arg( it->index ).arg( it->Imin ).arg( it->Imax ) );
        if (it->x < thresh) {
  ...
  gauss_add_marker(le_testiq_q_start->text().toDouble(), Qt::red,
                   us_tr("Start"));
  gauss_add_marker(le_testiq_q_end->text().toDouble(), Qt::red, us_tr("End"),
                   Qt::AlignLeft | Qt::AlignTop);
  ...
  if (!unified_ggaussian_ok || !f_gaussians.count(wheel_file) ||
    ...
    for (int i = 0; i < (int)unified_ggaussian_gaussians_size; i++) {
      gauss_add_marker(
          unified_ggaussian_params[(vector<double>::size_type)common_size * i],
...
void US_Hydrodyn_Saxs_Hplc::testiq_gauss_line() {
  ...
  for (int i = 0; i < (int)rb_testiq_gaussians.size(); ++i) {
    QPen use_pen =
        QPen(rb_testiq_gaussians[i]->isChecked() ? Qt::magenta : Qt::blue,
...
bool US_Hydrodyn_Saxs_Hplc::testiq_make() {
  ...
  if (cb_testiq_from_gaussian->isVisible() &&
    ...
    return create_i_of_q(testiq_selected, le_testiq_q_start->text().toDouble(),
                         le_testiq_q_end->text().toDouble());
  } else {
...
void US_Hydrodyn_Saxs_Hplc::testiq_testset() {
  ...
  for (int i = 0; i < (int)testiq_created_names.size(); ++i) {
    ...
    add_plot(this_file, testiq_created_q[this_file],
             testiq_created_I[this_file], testiq_created_e[this_file], false,
             false);
  }
  testiq_enables();
}
...
void US_Hydrodyn_Saxs_Hplc::guinier() {
  ...
  if (current_mode == MODE_TESTIQ) {
    ...
    if (!testiq_it_selected.isEmpty() && f_qs.count(testiq_it_selected)) {
      ...
      if (!rb_testiq_from_i_t->isChecked() &&
          ...
          (int)f_gaussians[testiq_it_selected].size() ==
              gaussian_type_size *
                  (int)((int)f_gaussians[testiq_it_selected].size() /
                        gaussian_type_size)) {
        guinier_it_I =
            compute_gaussian_sum(guinier_it_t, f_gaussians[testiq_it_selected]);
        for (int i = 0; i < (int)rb_testiq_gaussians.size(); ++i) {
          if (rb_testiq_gaussians[i]->isChecked()) {
            // us_qdebug( QString( "found rb_testiq_gaussian %1 checked" ).arg(
            // i ) ); build up guinier_it_pg_t, I as 2nd gaussian to display
    ...
    for (int i = 0; i < lb_files->count(); i++) {
      if (lb_files->item(i)->isSelected()) {
        ...
        if (!plotted_curves.count(this_file)) {
          editor_msg("red", QString(us_tr("Internal error: guinier selected "
    ...
    if (!f_qs.count(wheel_file)) {
      editor_msg("red", QString(us_tr("Internal error: %1 not found in data"))
                            .arg(wheel_file));
      return;
    }

    if (f_qs[wheel_file].size() < 2) {
      editor_msg("red", QString(us_tr("Internal error: %1 almost empty data"))
                            .arg(wheel_file));
      return;
    }
  ...
  disconnect(le_guinier_delta_start, SIGNAL(textChanged(const QString &)), 0,
             0);
...
void US_Hydrodyn_Saxs_Hplc::guinier_scroll_highlight(int pos) {
  ...
  sym = guinier_curves[showname]->symbol();
  guinier_curves[showname]->setSymbol(new QwtSymbol(
      QwtSymbol::Diamond, sym->brush(), sym->pen(), QSize(stdsize, stdsize)));
  ...
  if (guinier_rg_curves.count(hidename)) {
    sym = guinier_rg_curves[hidename]->symbol();
    guinier_rg_curves[hidename]->setSymbol(new QwtSymbol(
        QwtSymbol::Diamond, sym->brush(), sym->pen(), QSize(stdsize, stdsize)));
  }
  if (guinier_rg_curves.count(showname)) {
    sym = guinier_rg_curves[showname]->symbol();
  ...
  if (guinier_mwt_markers.count(hidename)) {
    sym = guinier_mwt_markers[hidename]->symbol();
    guinier_mwt_markers[hidename]->setSymbol(new QwtSymbol(
        QwtSymbol::Diamond, sym->brush(), sym->pen(), QSize(stdsize, stdsize)));
  }
  if (guinier_mwt_markers.count(showname)) {
    sym = guinier_mwt_markers[showname]->symbol();
    guinier_mwt_markers[showname]->setSymbol(new QwtSymbol(
        QwtSymbol::Rect, sym->brush(), sym->pen(), QSize(bigsize, bigsize)));
  }

  if (guinier_mwc_markers.count(hidename)) {
    sym = guinier_mwc_markers[hidename]->symbol();
    guinier_mwc_markers[hidename]->setSymbol(new QwtSymbol(
        QwtSymbol::Diamond, sym->brush(), sym->pen(), QSize(stdsize, stdsize)));
  }
  if (guinier_mwc_markers.count(showname)) {
    sym = guinier_mwc_markers[showname]->symbol();
    guinier_mwc_markers[showname]->setSymbol(new QwtSymbol(
        QwtSymbol::Rect, sym->brush(), sym->pen(), QSize(bigsize, bigsize)));
  }
#endif
#endif
...
void US_Hydrodyn_Saxs_Hplc::guinier_scroll() {
  ...
  if (cb_guinier_scroll->isChecked()) {
    ...
    for (int i = 0; i < (int)guinier_names.size(); ++i) {
      ...
      if (guinier_errorbar_curves.count(name)) {
        for (int j = 0; j < (int)guinier_errorbar_curves[name].size(); ++j) {
          guinier_plot->setCurveStyle(guinier_errorbar_curves[name][j],
                                      QwtCurve::NoCurve);
        }
      }
    ...
    for (int i = 0; i < (int)guinier_names.size(); ++i) {
      ...
      sym = guinier_curves[name]->symbol();
      guinier_curves[name]->setSymbol(new QwtSymbol(QwtSymbol::Diamond,
                                                    sym->brush(), sym->pen(),
...
void US_Hydrodyn_Saxs_Hplc::guinier_add_marker(QwtPlot *plot, double pos,
...
void US_Hydrodyn_Saxs_Hplc::guinier_analysis() {
  ...
  double mwt_k =
      ((US_Hydrodyn *)us_hydrodyn)->gparams.count("guinier_mwt_k")
          ? ((US_Hydrodyn *)us_hydrodyn)->gparams["guinier_mwt_k"].toDouble()
          : 0e0;
  double mwt_c =
  ...
  for (int i = 0; i < (int)guinier_names.size(); ++i) {
    ...
    do {
      ...
      if (guinier_e[this_name].size() != guinier_q[this_name].size()) {
        // any_sd_off = true;
        // editor_msg( "dark red", QString( us_tr( "Notice: SD weighting of
        // Guinier fit is off for %1 since SDs are not fully present" ) )
        //             .arg( this_name ) );
        ...
        for (int j = 0; j < (int)guinier_q[this_name].size(); j++) {
          if (guinier_q[this_name][j] >= qstart &&
            ...
            if (guinier_e[this_name][j] <= 0e0) {
              ...
              // of Guinier fit is off for %1 since at least one SD is zero or
              // negative in the selected q range" ) )
              //             .arg( this_name) );
              use_SD_weighting = false;
              break;
            }
      ...
      if (pts_decrease) {
        if ((int)usu->wave["hplc"].q.size() - (int)pts_decrease < 5) {
          if (!cb_guinier_scroll->isChecked() || i == guinier_scroll_pos) {
            editor_msg(
                "dark red",
                QString(us_tr("%1 too few points left %2 after %3 points "
                              "removed for qRgmax limit"))
                    .arg(this_name)
      ...
      unsigned int pend =
          usu->wave["hplc"].q.size() ? usu->wave["hplc"].q.size() - 1 : 0;

      computed_rg = usu->guinier_plot("hplcrg", "hplc") &&
      ...
      // us_qdebug( QString( "%1 decrease check sRgmax %2 sRgmaxlimit %3 %4"
      // ).arg( this_name ).arg( sRgmax ).arg( sRgmaxlimit )
      //         .arg( do_decrease ? "do_decrease on before check" :
      //         "do_decrease off before check" ) );
      do_decrease = do_decrease && (sRgmax > sRgmaxlimit);
      if (do_decrease) {
        pts_decrease++;
        //    us_qdebug( QString( "%1 decreasing %2" ).arg( this_name ).arg(
        //    pts_decrease ) );
        // } else {
    ...
    if (computed_rg) {
      ...
      // tainer mw method
      if (US_Saxs_Util::mwt(guinier_q[this_name], guinier_I[this_name], Rg,
                            sigb, I0, siga, mwt_k, mwt_c, mwt_qmax, Vct, Qrt,
                            mwt, mwt_sd, messagest, notest, warningt) &&
          !us_isnan(mwt)) {
        report += QString("").sprintf(" Vc[T] %.1e Qr[T] %.2e MW[RT] %.2e ",
                                      Vct, Qrt, mwt) +
                  notest;
        if (mw_min > mwt) {
          mw_min = mwt;
        ...
        report +=
            us_tr(" MW[RT] could not compute " + messagest + " " + notest);
      }

      // curtis mw method
      // activate in regular mode
      if (started_in_expert_mode) {
        if (usu->mwc(guinier_q[this_name], guinier_I[this_name], Rg, sigb, I0,
                     siga, mwc_mw_per_N, qm, Vcc, Qrc, mwc, mwc_sd, messagesc,
                     notesc) &&
            !us_isnan(mwc)) {
          report +=
              QString("").sprintf(" Vc[%.3f,C] %.1e Qr[C] %.2e MW[C] %.2e ", qm,
                                  Vcc, Qrc, mwc) +
              notesc;
          mwc_x.push_back(pos);
          mwc_y.push_back(mwc);
          ...
          report +=
              us_tr(" MW[C] could not compute " + messagesc + " " + notesc);
        }
      }
      ...
      if (!us_isnan(Rg)) {
...
#endif
        {
          ...
          curve->setSymbol(new QwtSymbol(rg_sym.style(), rg_sym.brush(),
  ...
  if (mwt_x.size() || mwc_x.size()) {
    ...
    if (mwt_x.size()) {
      ...
      if (count > 2) {
        ...
        msg += QString("").sprintf(" %.4g (%.4g) [%.4g:%.4g]", mwt_avg, mwt_sd,
                                   mwt_min, mwt_max);
      } else {
    ...
    if (mwc_x.size()) {
      ...
      if (count > 2) {
        ...
        msg += QString("").sprintf(" %.4g (%.4g) [%.4g:%.4g]", mwc_avg, mwc_sd,
                                   mwc_min, mwc_max);
      } else {
  ...
  if (rg_x.size() > 1) {
    ...
    curve->setStyle(QwtPlotCurve::Lines);
    curve->setSamples((double *)&(rg_x[0]), (double *)&(rg_y[0]), rg_x.size());
    curve->setPen(use_pen);
    curve->attach(guinier_plot_rg);
#else
  ...
  // us_qdebug( QString( "space %1 rg_max %2 rg_sd_max %3 rg_min %4 rg_sd_min %5
  // count %6" )
  //         .arg( space )
  //         .arg( rg_max )
  //         .arg( rg_sd_max )
  ...
  if (rg_x.size() > 1 && guinier_it_t.size() && space > 0e0) {
    ...
    if (ymax > ymin) {
      ...
      // us_qdebug( QString( "testiq in rg plot scale %1 rg_min %2 rg_max %3
      // ymin %4 ymax %5 " ).arg( scale ).arg( rg_min ).arg( rg_max ).arg( ymin
      // ).arg( ymax ) );
      ...
      if (y3.size()) {
        ...
        QwtPlotCurve *curve = new QwtPlotCurve("refitline");
        curve->setStyle(QwtPlotCurve::Lines);
        curve->setSamples((double *)&(guinier_it_pg_t[0]), (double *)&(y3[0]),
        ...
        guinier_plot_rg->setCurveStyle(curve, QwtCurve::Lines);
        guinier_plot_rg->setCurveData(curve, (double *)&(guinier_it_pg_t[0]),
                                      (double *)&(y3[0]),
                                      guinier_it_pg_t.size());
        guinier_plot_rg->setCurvePen(curve, use_pen);
#endif
...

  {
    if (cb_guinier_lock_rg_range->isChecked()) {
      ...
      disconnect(le_guinier_rg_t_start, SIGNAL(textChanged(const QString &)), 0,
                 0);
      disconnect(le_guinier_rg_t_end, SIGNAL(textChanged(const QString &)), 0,
                 0);
      disconnect(le_guinier_rg_rg_start, SIGNAL(textChanged(const QString &)),
                 0, 0);
      disconnect(le_guinier_rg_rg_end, SIGNAL(textChanged(const QString &)), 0,
                 0);
  ...
  if ((mwt_x.size() > 1 || mwc_x.size() > 1) && guinier_it_t.size() &&
    ...
    if (ymax > ymin) {
      ...
      // us_qdebug( QString( "testiq in mw plot scale %1 mw_min %2 mw_max %3
      // ymin %4 ymax %5 " ).amw( scale ).amw( mw_min ).amw( mw_max ).amw( ymin
      // ).amw( ymax ) );
      ...
      if (y3.size()) {
        ...
        guinier_plot_mw->setCurveStyle(curve, QwtCurve::Lines);
        guinier_plot_mw->setCurveData(curve, (double *)&(guinier_it_pg_t[0]),
                                      (double *)&(y3[0]),
                                      guinier_it_pg_t.size());
        guinier_plot_mw->setCurvePen(curve, use_pen);
#endif
  ...
  if (mwt_x.size() || mwt_y.size()) {
    if (cb_guinier_lock_mw_range->isChecked()) {
      ...
      disconnect(le_guinier_mw_t_start, SIGNAL(textChanged(const QString &)), 0,
                 0);
      disconnect(le_guinier_mw_t_end, SIGNAL(textChanged(const QString &)), 0,
                 0);
      disconnect(le_guinier_mw_mw_start, SIGNAL(textChanged(const QString &)),
                 0, 0);
      disconnect(le_guinier_mw_mw_end, SIGNAL(textChanged(const QString &)), 0,
                 0);
  ...
  if (testiq_active && rb_testiq_gaussians.size()) {
    for (int i = 0; i < (int)unified_ggaussian_gaussians_size; i++) {
      ...
      guinier_plot_rg->setMarkerPen(marker,
                                    QPen(color, line_width, DashDotDotLine));
      guinier_plot_rg->setMarkerFont(marker,
                                     QFont("Helvetica", 11, QFont::Bold));
...
void US_Hydrodyn_Saxs_Hplc::guinier_range(double minq2, double maxq2,
  ...
  if (!guinier_plot_zoomer) {
    ...
    // connect( guinier_plot_zoomer, SIGNAL( zoomed( const QRectF & ) ), SLOT(
...
void US_Hydrodyn_Saxs_Hplc::guinier_replot() {
  ...
  guinier_add_marker(guinier_plot_errors,
                     le_guinier_q2_start->text().toDouble(), Qt::red,
                     us_tr("Start"));
  guinier_add_marker(guinier_plot_errors, le_guinier_q2_end->text().toDouble(),
                     Qt::red, us_tr("End"), Qt::AlignLeft | Qt::AlignTop);
  ...
  for (map<QString, vector<double> >::iterator it = guinier_q2.begin();
    ...
    curve->setSymbol(new QwtSymbol(
        sym.style(), sym.brush(), QPen(guinier_colors[it->first]), sym.size()));
#endif
#endif
    ...
    if (do_error_bars && use_error) {
      ...
      for (int i = 0; i < (int)q_points; ++i) {
        ...
        guinier_plot->setCurveStyle(curve, QwtCurve::Lines);
        guinier_plot->setCurveData(curve, (double *)&(x[0]), (double *)&(y[0]),
                                   2);
...
bool US_Hydrodyn_Saxs_Hplc::guinier_check_qmax(QString &report,
  ...
  report = QString(us_tr("MW[RT] calculations are calibrated for a qmax of %1\n"
...
void US_Hydrodyn_Saxs_Hplc::guinier_q_start_text(const QString &text) {
  ...
  if (guinier_markers.size() != 4) {
    editor_msg(
        "red",
        QString("internal error: guinier_q_start_text markers issue size %1")
...
void US_Hydrodyn_Saxs_Hplc::guinier_q_start_focus(bool hasFocus) {
  ...
  if (hasFocus) {
    ...
    qwtw_wheel->setSingleStep((guinier_maxq - guinier_minq) / UHSH_G_WHEEL_RES);
    connect(qwtw_wheel, SIGNAL(valueChanged(double)),
            SLOT(adjust_wheel(double)));
...
void US_Hydrodyn_Saxs_Hplc::guinier_q2_start_text(const QString &text) {
  ...
  if (guinier_markers.size() != 4) {
    editor_msg(
        "red",
        QString("internal error: guinier_q2_start_text markers issue size %1")
            .arg(guinier_markers.size()));
    return;
  }
...
void US_Hydrodyn_Saxs_Hplc::guinier_rg_t_start_focus(bool hasFocus) {
  ...
  if (hasFocus) {
    ...
    qwtw_wheel->setSingleStep((guinier_maxt - guinier_mint) / UHSH_G_WHEEL_RES);
    connect(qwtw_wheel, SIGNAL(valueChanged(double)),
            SLOT(adjust_wheel(double)));
...
void US_Hydrodyn_Saxs_Hplc::wyatt_apply() { wyatt_apply(all_selected_files()); }
    ...
    if (cb_wyatt_2->isChecked()) {
      ...
      if (wyatt_error < 0e0 && wyatt_error2 < 0e0) {
        ...
        if (wyatt_error < 0e0 || wyatt_error2 < 0e0) {
          editor_msg(
              ...
              QString(us_tr("SD errors failed range %1:%2 for %3"))
                  .arg(wyatt_error < 0e0 ? le_wyatt_start->text().toDouble()
                                         : le_wyatt_start2->text().toDouble())
                  .arg(wyatt_error < 0e0 ? le_wyatt_end->text().toDouble()
                                         : le_wyatt_end2->text().toDouble())
                  .arg(files[i]));
          use_e = wyatt_error < 0e0 ? wyatt_error2 : wyatt_error;
        } else {
          double frac1 = (double)wyatt_q.size() /
        ...
        if (!constant_e(files[i], use_e)) {
          editor_msg(
              "red",
              QString(us_tr("setting SD errors failed for %1")).arg(files[i]));
        }
      }
    } else {
      double wyatt_error =
...
void US_Hydrodyn_Saxs_Hplc::replot_wyatt() {
  ...
  double wyatt_error =
      wyatt_errors(wheel_file, le_wyatt_start->text().toDouble(),
                   le_wyatt_end->text().toDouble(), wyatt_q, wyatt_I, wyatt_y);
  ...
  if (wyatt_error >= 0e0) {
    ...
    plot_dist->setCurveData(curve, (double *)&wyatt_q[0], (double *)&wyatt_y[0],
                            wyatt_q.size());
#else
  ...
  if (cb_wyatt_2->isChecked()) {
    ...
    wyatt_error2 = wyatt_errors(wheel_file, le_wyatt_start2->text().toDouble(),
                                le_wyatt_end2->text().toDouble(), wyatt_q2,
                                wyatt_I2, wyatt_y2);
...
#if QT_VERSION < 0x040000
      plot_dist->setCurvePen(curve,
                             QPen(Qt::yellow, use_line_width, Qt::SolidLine));
    ...
    editor_msg(wyatt_error < 0e0 || wyatt_error2 < 0e0 ? "red" : "dark blue",
               msg);
  } else {
...
void US_Hydrodyn_Saxs_Hplc::scale() {
  ...
  for (int i = 0; i < lb_files->count(); i++) {
    if (lb_files->item(i)->isSelected()) {
      ...
      if (!plotted_curves.count(lb_files->item(i)->text())) {
        editor_msg("red", QString(us_tr("Internal error: scale selected %1, "
                                        "but no plotted curve found"))
                              .arg(lb_files->item(i)->text()));
        return;
      }
  ...
  gauss_add_marker(le_scale_q_start->text().toDouble(), Qt::red,
                   us_tr("Start"));
  gauss_add_marker(le_scale_q_end->text().toDouble(), Qt::red, us_tr("End"),
                   Qt::AlignLeft | Qt::AlignTop);
  plot_dist->replot();
...
void US_Hydrodyn_Saxs_Hplc::scale_scroll_highlight(int pos) {
  ...
  if (scale_plotted_errors.count(scale_scroll_selected[pos])) {
    plot_errors->setCurveStyle(scale_plotted_errors[scale_scroll_selected[pos]],
                               QwtCurve::Lines);
  }
#else
...
void US_Hydrodyn_Saxs_Hplc::scale_update_plot_errors() {
  ...
  for (set<QString>::iterator it = scale_selected.begin();
    ...
    for (int i = 0; i < (int)scale_q[*it].size(); ++i) {
      if (scale_q[*it][i] >= q_min && scale_q[*it][i] <= q_max) {
        if (usu->apply_natural_spline(scale_spline_x, scale_spline_y,
                                      scale_spline_y2, scale_q[*it][i], y)) {
    ...
    plot_errors->setCurveStyle(
        curve, hide ? QwtPlotCurve::NoCurve : QwtPlotCurve::Lines);
    plot_errors->setCurvePen(curve,
                             QPen(plot_colors[f_pos[*it] % plot_colors.size()],
                                  use_line_width, Qt::SolidLine));
...
QString US_Hydrodyn_Saxs_Hplc::scale_get_target(bool do_msg) {
  ...
  if (q_min >= q_max) {
    editor_msg("red", us_tr("Error: Scale mode: minimum q value is greator or "
                            "equal to maximum q value"));
    return "";
  }
...
void US_Hydrodyn_Saxs_Hplc::scale_apply() {
  ...
  for (int i = 0; i < (int)scale_q[target].size(); ++i) {
    if (scale_q[target][i] >= q_min && scale_q[target][i] <= q_max) {
      ...
      if (target_has_e) {
        ...
        if (scale_I[target][i]) {
          avg_std_dev_frac += (scale_e[target][i] * scale_e[target][i]) /
  ...
  if (!target_q.size()) {
    editor_msg("red",
               QString(us_tr("Error: Scale 'apply': the target curve for "
                             "scaling has no points in the selected q range")));
    scale_enables();
    return;
  }
  ...
  if (our_saxs_options->iqq_scale_nnls && do_chi2_fitting &&
    ...
    notices += us_tr(
        "\nChi^2 fitting is currently not compatable with NNLS scaling, Chi^2 "
        "fitting not used");
    do_chi2_fitting = false;
  }
  ...
  for (set<QString>::iterator it = scale_selected.begin();
    ...
    if (*it != target) {
      ...
      if (!usu->interpolate(source_I, target_q, scale_q[*it], scale_I[*it])) {
        editor_msg(
            "red",
            QString(us_tr("Error: Scale 'apply': %1 could not interpolate to "
                          "target on specified q range: %2"))
                .arg(*it)
                .arg(usu->errormsg));
        scale_enables();
        return;
      ...
      if (our_saxs_options->iqq_scale_nnls) {
        ...
        if (do_chi2_fitting) {
          usu->scaling_fit(source_I, target_I, target_e, k[*it], chi2[*it]);
        } else {
  ...
  for (set<QString>::iterator it = scale_selected.begin();
    ...
    if (*it != target) {
      ...
      // us_qdebug( US_Vector::qs_vector2( QString( "%1 scale_q, scale_I" ).arg(
      // *it ), scale_q[ *it ], scale_I[ *it ] ) );
      fit_msg += QString("%1 : scale %2 ").arg(*it).arg(k[*it]);
      if (do_chi2_fitting) {
        // usu.calc_chisq_prob( 0.5 * target_I.size() - ( do_scale_linear_offset
        // ? 2 : 1 ), 0.5 * chi2[ *it ], chi2_prob );
        fit_msg +=
            QString("chi^2=%1 df=%2 nchi=%3")
                .arg(chi2[*it], 6)
                .arg(target_I.size() - (do_scale_linear_offset ? 2 : 1))
                .arg(sqrt(chi2[*it] /
                          (target_I.size() - (do_scale_linear_offset ? 2 : 1))),
                     5);
        if (avg_std_dev_frac) {
          fit_msg +=
              QString(" r_sigma=%1 nchi*r_sigma=%2 ")
                  .arg(avg_std_dev_frac)
                  .arg(avg_std_dev_frac *
                           sqrt(chi2[*it] / (target_I.size() -
      ...
      if (cb_scale_save_intp->isChecked() && *it != scale_applied_target) {
        ...
        if (scale_e[*it].size() == scale_q[*it].size()) {
          if (!usu->interpolate(source_e, target_q, scale_q[*it],
            ...
            editor_msg(
                "red",
                QString(us_tr("Error: Scale 'apply': %1 could not interpolate "
...
void US_Hydrodyn_Saxs_Hplc::scale_replot() {
  for (set<QString>::iterator it = scale_selected.begin();
       it != scale_selected.end(); ++it) {
#if QT_VERSION < 0x040000
    plot_dist->setCurveData(plotted_curves[*it], (double *)&(scale_q[*it][0]),
...
void US_Hydrodyn_Saxs_Hplc::scale_create() {
  ...
  for (set<QString>::iterator it = scale_selected.begin();
    ...
    if (cb_scale_save_intp->isChecked() && *it != scale_applied_target) {
      ...
      add_plot(name, scale_q[*it], scale_I[*it], scale_e[*it], f_is_time[*it],
               false);
    }
...
void US_Hydrodyn_Saxs_Hplc::ggauss_start() {
  ...
  if (not_compatible) {
    ...
    QString msg =
        QString(us_tr("NOTICE: Some files selected have Gaussians with varying "
                      "centers%1 or a different number of Gaussians or centers "
                      "that do not match the last Gaussians, these will be "
                      "reset to the last Gaussian settings"))
            .arg(msg_addendum);
...

  {
    if (not_compatible) {
      QMessageBox::information(
          this, windowTitle() + us_tr(": Global Gaussians"),
          QString(us_tr("NOTICE: Some files selected have Gaussians with "
      ...
      if (no_gaussian_count) {
        if (no_gaussian_count == (int)ggaussian_selected_file_index.size()) {
          ...
          switch (QMessageBox::question(
              this, windowTitle() + us_tr(": Global Gaussians"),
              QString(us_tr("%1 selected curves do not have defined Gaussians "
        ...
        switch (QMessageBox::question(
            ...
            QString(us_tr(
                ...
                "Answer \"Yes\" if you want to re-fit the amplitudes using the "
                "last single curve Gaussian known"
                " [the most recent of a Gaussian file load or single Gaussian "
                "\"Keep\"].\n\n"
...
void US_Hydrodyn_Saxs_Hplc::ggauss_scroll_set_selected() {
  ...
  if (!unified_ggaussian_ok) {
    editor_msg(
        "red",
        us_tr("Internal error (ggauss_scroll_set_selected): Global Gaussian "
              "mode, but unified Global Gaussians are not ok."));
    ggaussian_enables();
    return;
  }
  ...
  if ((int)unified_ggaussian_files.size() !=
      (int)ggaussian_last_pfit_P.size()) {
    editor_msg("red", us_tr("Internal error (ggauss_scroll_set_selected): P "
                            "set does not match Gaussian size"));
    ggaussian_enables();
    return;
  }

  if ((int)unified_ggaussian_files.size() != (int)ggaussian_last_chi2.size()) {
    editor_msg("red", us_tr("Internal error (ggauss_scroll_set_selected): chi2 "
                            "set does not match Gaussian size"));
    ggaussian_enables();
    return;
  }
...
void US_Hydrodyn_Saxs_Hplc::ggauss_scroll_highlight(int pos) {
  ...
  if (pos >= (int)ggauss_scroll_set.size() || pos < 0) {
    editor_msg(
        "red",
        us_tr("Internal error ggauss_scroll_highlight: pos >= size or < 0"));
    lbl_wheel_pos_below->setText("");
    {
      QPixmap pm;
      lbl_wheel_Pcolor->setPixmap(pm);
  ...
  if (ggaussian_last_pfit_P[ggauss_scroll_set[pos]] >= 0.05) {
    pm.convertFromImage(qi_green->smoothScale(lbl_wheel_Pcolor->width() / 2,
                                              lbl_wheel_Pcolor->height() / 2,
                                              Qt::KeepAspectRatio));
  } else {
    if (ggaussian_last_pfit_P[ggauss_scroll_set[pos]] >= 0.01) {
      pm.convertFromImage(qi_yellow->smoothScale(lbl_wheel_Pcolor->width() / 2,
                                                 lbl_wheel_Pcolor->height() / 2,
                                                 Qt::KeepAspectRatio));
    } else {
      pm.convertFromImage(qi_red->smoothScale(lbl_wheel_Pcolor->width() / 2,
                                              lbl_wheel_Pcolor->height() / 2,
                                              Qt::KeepAspectRatio));
    }
  }
  ...
  if (f_best_smoothed_smoothing.count(
    ...
    curve->setSamples(
        (double *)&(
            f_qs_smoothed[unified_ggaussian_files[ggauss_scroll_set[pos]]][0]),
        (double *)&(
            f_Is_smoothed[unified_ggaussian_files[ggauss_scroll_set[pos]]][0]),
  ...
  if (cb_ggauss_scroll_oldstyle->isChecked() &&
    ...
    curve->setSamples(
        (double *)&(
            f_qs_oldstyle[unified_ggaussian_files[ggauss_scroll_set[pos]]][0]),
        (double *)&(
            f_Is_oldstyle[unified_ggaussian_files[ggauss_scroll_set[pos]]][0]),
...

  {
    ...
    for (int i = 0; i < (int)ggaussian_last_ggig[ggauss_scroll_set[pos]].size();
         ++i) {
#if QT_VERSION >= 0x040000
  ...
  // clear highlighted ggqfit_plot for chi2 and pfit
  {
    ...
    for (set<QwtPlotMarker *>::iterator it = ggaussian_pts_chi2_marked.begin();
      ...
      sym = (*it)->symbol();
      (*it)->setSymbol(
          new QwtSymbol(sym->style(), sym->brush(), sym->pen(),
                        QSize(use_line_width * 3 + 1, use_line_width * 3 + 1)));
    }
    for (set<QwtPlotMarker *>::iterator it = ggaussian_pts_pfit_marked.begin();
         it != ggaussian_pts_pfit_marked.end(); it++) {
...
void US_Hydrodyn_Saxs_Hplc::ggaussian_enables() {
  ...
  if (cb_ggauss_scroll->isChecked()) {
    ...
    pb_gauss_prev->setEnabled(unified_ggaussian_gaussians_size > 1 &&
                              gaussian_pos > 0);
    pb_gauss_next->setEnabled(unified_ggaussian_gaussians_size > 1 &&
                              gaussian_pos <
                                  unified_ggaussian_gaussians_size - 1);
    cb_sd_weight->setEnabled(unified_ggaussian_gaussians_size &&
                             le_gauss_fit_start->text().toDouble() <
                                 le_gauss_fit_end->text().toDouble());
    pb_gauss_fit->setEnabled(unified_ggaussian_gaussians_size &&
                             le_gauss_fit_start->text().toDouble() <
                                 le_gauss_fit_end->text().toDouble());
    ...
    le_gauss_pos_dist2->setEnabled(
        ...
        gaussian_pos < unified_ggaussian_gaussians_size);
    le_gauss_fit_start->setEnabled(unified_ggaussian_gaussians_size &&
                                   gaussian_pos <
...
void US_Hydrodyn_Saxs_Hplc::timeshift() {
  ...
  if (!plotted_curves.count(wheel_file)) {
    editor_msg("red", QString(us_tr("Internal error: request to timeshift %1, "
  ...
  // compute a good range
  {
    ...
    // qwtw_wheel->setRange( minq_w, maxq_w); qwtw_wheel->setSingleStep( (maxq_w
    // - minq_w ) / UHSH_WHEEL_RES ); qwtw_wheel            ->setValue  ( (
    // minq_w + maxq_w ) * 5e-1 );
