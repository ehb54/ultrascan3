...
unsigned int
US_Hydrodyn_Saxs_Hplc_Fit::pop_selection (unsigned int size)
{
...
bool
US_Hydrodyn_Saxs_Hplc_Fit::ga_fitness (ga_individual &individual)
{
  ...
  our_vector *vi = new_our_vector (HFIT::init_params.size ());
  for (unsigned int i = 0; i < HFIT::init_params.size (); i++)
    {
...
bool
US_Hydrodyn_Saxs_Hplc_Fit::ga_run (double &nrmsd)
{
  ...
  individual.v.resize (HFIT::init_params.size ());

  // init population
  for (unsigned int i = 0; i < control_parameters["population"].toUInt (); i++)
    {
      for (unsigned int j = 0; j < HFIT::init_params.size (); j++)
        {
          individual.v[j]
              = HFIT::param_min[j]
                + (HFIT::param_max[j] - HFIT::param_min[j]) * drand48 ();
        }
  ...
  for (unsigned int g = 0; g < control_parameters["generations"].toUInt ();
       g++)
    {
      pop.sort ();
      pop.unique ();
          ...
          if (last_best_fitness > pop.front ().fitness)
          ...
          else
            {
              ...
              if (control_parameters.count ("earlytermination")
                  && gens_with_no_improvement
                         >= control_parameters["earlytermination"].toUInt ())
                {
      ...
      for (list<ga_individual>::iterator it = pop.begin (); it != pop.end ();
           it++)
        {
      ...
      for (unsigned int i = 0; i < control_parameters["population"].toUInt ();
           i++)
        {
          ...
          if (control_parameters.count ("mutate")
              && drand48 () < control_parameters["mutate"].toDouble ())
            {
              ...
              unsigned int pos
                  = (unsigned int)(drand48 () * HFIT::init_params.size ());
              individual.v[pos]
                  = HFIT::param_min[pos]
                    + (HFIT::param_max[pos] - HFIT::param_min[pos])
                          * drand48 ();
              ga_fitness (individual);
              pop.push_back (individual);
              mutate_count++;
          ...
          if (control_parameters.count ("crossover")
              && drand48 () < control_parameters["crossover"].toDouble ())
            {
              // cout << "crossover\n";
              individual = last_pop[pop_selection (last_pop.size ())];
              ga_individual individual2
                  = last_pop[pop_selection (last_pop.size ())];
              unsigned int pos
                  = (unsigned int)(drand48 () * HFIT::init_params.size ());
              for (unsigned int j = pos; j < HFIT::init_params.size (); j++)
                {
          ...
          if (has_been_duplicated.count (pos))
            {
              for (unsigned int j = 0; j < HFIT::init_params.size (); j++)
                {
                  individual.v[j] = HFIT::param_min[j]
                                    + (HFIT::param_max[j] - HFIT::param_min[j])
                                          * drand48 ();
                }
              ga_fitness (individual);
              pop.push_back (individual);
  ...
  for (unsigned int i = 0; i < HFIT::init_params.size (); i++)
    {
      HFIT::init_params[i] = pop.front ().v[i];
    }
