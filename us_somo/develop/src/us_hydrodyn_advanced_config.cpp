...
void US_Hydrodyn_AdvancedConfig::setupGUI() {
  ...
  cb_expert_mode->setText(
      us_tr(" Expert mode (skip some warning messages, add some developmental "
            "features)"));
  cb_expert_mode->setEnabled(true);
  cb_expert_mode->setChecked((*advanced_config).expert_mode);
  cb_expert_mode->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  lbl_temp_dir_threshold_mb =
      new QLabel(us_tr("Temporary directory clear size threshold [MB]:"), this);
  lbl_temp_dir_threshold_mb->setAlignment(Qt::AlignLeft | Qt::AlignVCenter);
  lbl_temp_dir_threshold_mb->setPalette(PALET_LABEL);
  lbl_temp_dir_threshold_mb->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  AUTFBACK(lbl_temp_dir_threshold_mb);

  le_temp_dir_threshold_mb = new QLineEdit(this);
  le_temp_dir_threshold_mb->setObjectName("le_temp_dir_threshold_mb Line Edit");
  ...
  le_temp_dir_threshold_mb->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  {
  ...
  cb_experimental_threads->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
...
void US_Hydrodyn_AdvancedConfig::cancel() { close(); }
