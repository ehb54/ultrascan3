...
void
US_Hydrodyn_Cluster_Config_Server::setupGUI ()
{
  int minHeight1 = 30;

  lbl_title = new QLabel (
      QString (us_tr ("Cluster System: %1")).arg (server_name), this);
...
void
US_Hydrodyn_Cluster_Config_Server::cancel ()
{
  close ();
}

void
US_Hydrodyn_Cluster_Config_Server::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Cluster_Config_Server::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept ();
...
void
US_Hydrodyn_Cluster_Config_Server::save_config ()
{
  for (unsigned int i = 0; i < lbls.size (); i++)
    {
