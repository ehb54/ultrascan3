...
US_Hydrodyn_Mals_Saxs_Fit::US_Hydrodyn_Mals_Saxs_Fit(
  ...
  if (mals_saxs_win->conc_files.count(mals_saxs_win->wheel_file)) {
    ...
    if (gaussian_type != US_Hydrodyn_Mals_Saxs::EMGGMG) {
      ...
      for (int i = gaussian_type_size; i < (int)mals_saxs_win->gaussians.size();
...
void US_Hydrodyn_Mals_Saxs_Fit::cancel() { close(); }
  ...
  for (unsigned int i = 0; i < (unsigned int)mals_saxs_win->gaussians.size();
    ...
    if (cb_fix_amplitude->isChecked() || fixed_curves.count(pos + 1)) {
      ...
      if (MSFIT::init_params.back() < min) {
        ...
        // MSFIT::init_params.size() ).arg( MSFIT::init_params.back() ) );
...
void US_Hydrodyn_Mals_Saxs_Fit::lm(bool max_free_peak_delta_run,
                                   double prev_rmsd) {
  update_common();
  setup_run();
  // puts( "lm" );
  ...
  // US_Vector::printvector( QString( "lm %1 q" ).arg( mals_saxs_win->wheel_file
  // ), x ); US_Vector::printvector( QString( "lm %1 I" ).arg(
  // mals_saxs_win->wheel_file ), mals_saxs_win->f_Is[ mals_saxs_win->wheel_file
  // ] ); US_Vector::printvector( QString( "lm %1 e" ).arg(
  // mals_saxs_win->wheel_file ), mals_saxs_win->f_errors[
  ...
  if (use_errors) {
    ...
    for (unsigned int j = 0; j < t.size(); j++) {
      yp[j] = (*MSFIT::compute_gaussian_f)(t[j],
                                           (double *)(&MSFIT::base_params[0]));
...
void US_Hydrodyn_Mals_Saxs_Fit::stop() { running = false; }
...
void US_Hydrodyn_Mals_Saxs_Fit::grid() {
  ...
  for (unsigned int j = 0; j < t.size(); j++) {
    yp[j] =
        (*MSFIT::compute_gaussian_f)(t[j], (double *)(&MSFIT::base_params[0]));
    org_rmsd += (y[j] - yp[j]) * (y[j] - yp[j]);
  }
