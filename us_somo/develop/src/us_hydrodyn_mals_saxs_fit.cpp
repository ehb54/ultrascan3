...
US_Hydrodyn_Mals_Saxs_Fit::US_Hydrodyn_Mals_Saxs_Fit (
    US_Hydrodyn_Mals_Saxs *mals_saxs_win, bool set_comm_dist, QWidget *p,
    const char *)
    : QDialog (p)
{
  // us_qdebug( "hf0" );
      ...
      if (gaussian_type != US_Hydrodyn_Mals_Saxs::EMGGMG)
      ...
      else
        {
          for (int i = gaussian_type_size;
               i < (int)mals_saxs_win->gaussians.size ();
               i += gaussian_type_size)
            {
              conc_ratios.push_back (mals_saxs_win->gaussians[2 + i]
                                     / mals_saxs_win->gaussians[2]);
            }
          // us_qdebug( US_Vector::qs_vector( "gaussians",
          // mals_saxs_win->gaussians ) ); us_qdebug( US_Vector::qs_vector(
          // "width ratios", conc_ratios ) );
        }
    }
  else
    {
...
void
US_Hydrodyn_Mals_Saxs_Fit::redo_settings ()
{
  use_errors = mals_saxs_win->cb_sd_weight->isChecked ();
  if (use_errors
      && (!mals_saxs_win->f_errors.count (mals_saxs_win->wheel_file)
          || mals_saxs_win->f_errors[mals_saxs_win->wheel_file].size ()
                 != mals_saxs_win->f_qs[mals_saxs_win->wheel_file].size ()
          || !mals_saxs_win->is_nonzero_vector (
              mals_saxs_win->f_errors[mals_saxs_win->wheel_file])))
    {
      use_errors = false;
...
void
US_Hydrodyn_Mals_Saxs_Fit::restore ()
{
...
void
US_Hydrodyn_Mals_Saxs_Fit::undo ()
{
...
void
US_Hydrodyn_Mals_Saxs_Fit::setupGUI ()
{
  int minHeight1 = 30;
  ...
  mals_saxs_win->gaussians.size() / 3 - 1 ); cout << "qs_rx:" << qs_rx << endl;
...
void
US_Hydrodyn_Mals_Saxs_Fit::cancel ()
{
  close ();
}

void
US_Hydrodyn_Mals_Saxs_Fit::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Mals_Saxs_Fit::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 3;
  global_Ypos -= 30;
  e->accept ();
...
void
US_Hydrodyn_Mals_Saxs_Fit::update_common ()
{
  if (!setup_run ())
    {
      update_enables ();
  ...
  for (unsigned int i = 0; i < MSFIT::param_fixed.size (); i++)
    {
      if (!MSFIT::param_fixed[i] || MSFIT::comm_backref.count (i))
        {
          mals_saxs_win->gaussians[i]
              = MSFIT::init_params[MSFIT::comm_backref.count (i)
...
void
US_Hydrodyn_Mals_Saxs_Fit::update_enables ()
{
...
bool
US_Hydrodyn_Mals_Saxs_Fit::setup_run ()
{
  // us_qdebug( "setup_run()" );
  ...
  for (unsigned int i = 0; i < (unsigned int)mals_saxs_win->gaussians.size ();
       i += gaussian_type_size)
    {
          ...
          if (MSFIT::init_params.back () < min)
            {
              ...
              // ).arg( MSFIT::init_params.size() ).arg(
              // MSFIT::init_params.back() ) );
      ...
      )
        {
          ...
          if (i && cb_conc_test->isChecked () && !fixed_curves.count (pos + 1)
              && !fixed_curves.count (1))
            {
              ...
              MSFIT::comm_backref[MSFIT::param_fixed.size ()]
                  = conc_width_var_pos;
              MSFIT::conc_ratios_map[MSFIT::param_fixed.size ()]
                  = conc_ratios[pos - 1];
            }
          else
            {
          ...
          if (cb_fix_dist1->isChecked () || fixed_curves.count (pos + 1)
              || (cb_comm_dist1->isChecked () && i))
            {
              ...
              if (i && cb_comm_dist1->isChecked ()
                ...
                {
                  MSFIT::fixed_params.push_back (
                      mals_saxs_win->gaussians[3 + 0]);
                  MSFIT::comm_backref[MSFIT::param_fixed.size ()]
                      = comm_dist_1_var_pos;
                }
              else
...
bool
US_Hydrodyn_Mals_Saxs_Fit::lock_zeros (vector<double> &par)
{
  ...
  for (unsigned int i = 0; i < MSFIT::param_fixed.size (); i++)
    {
      if (!MSFIT::param_fixed[i] || MSFIT::comm_backref.count (i))
        {
          tmp_gaussians[i]
              = par[MSFIT::comm_backref.count (i) ? MSFIT::comm_backref[i]
                                                  : MSFIT::param_pos[i]];
          if (MSFIT::conc_ratios_map.count (i))
            {
          ...
          if (!cb_fix_amplitude->isChecked ())
            {
              if (tmp_gaussians[0 + i]
                  <= mals_saxs_ampl_width_min * mals_saxs_lock_min_retry_mult)
                {
...
bool
US_Hydrodyn_Mals_Saxs_Fit::max_free_peak_delta (vector<double> &par)
{
...
void
US_Hydrodyn_Mals_Saxs_Fit::lm (bool max_free_peak_delta_run, double prev_rmsd)
{
  update_common ();
  setup_run ();
  // puts( "lm" );
  ...
  // US_Vector::printvector( QString( "lm %1 q" ).arg(
  // mals_saxs_win->wheel_file ), x ); US_Vector::printvector( QString( "lm %1
  // I" ).arg( mals_saxs_win->wheel_file ), mals_saxs_win->f_Is[
  // mals_saxs_win->wheel_file ] ); US_Vector::printvector( QString( "lm %1 e"
  // ).arg( mals_saxs_win->wheel_file ), mals_saxs_win->f_errors[
  // mals_saxs_win->wheel_file ] );
  ...
  if (use_errors)
    {
      ...
      for (unsigned int j = 0; j < t.size (); j++)
        {
          yp[j] = (*MSFIT::compute_gaussian_f) (
              t[j], (double *)(&MSFIT::base_params[0]));
...
#endif
      double nu
          = (double)(t.size () - (int)mals_saxs_win->gaussians.size () - 1);
      if (nu <= 0e0)
        {
          nu = 1e0;
        }
  ...
  if (max_free_peak_delta_run)
          ...
          for (unsigned int i = 0; i < MSFIT::param_fixed.size (); i++)
            {
              if (!MSFIT::param_fixed[i] || MSFIT::comm_backref.count (i))
                {
                  mals_saxs_win->gaussians[i]
                      = par[MSFIT::comm_backref.count (i)
                                ? MSFIT::comm_backref[i]
                                : MSFIT::param_pos[i]];
                  if (MSFIT::conc_ratios_map.count (i))
                    {
  ...
  else
    {
      ...
      if (mals_saxs_maxfpk_restart && max_free_peak_delta (par))
        {
...
void
US_Hydrodyn_Mals_Saxs_Fit::gsm_sd ()
{
  puts ("gsm_sd");
  update_common ();
  ...
  for (unsigned int j = 0; j < gsm_t.size (); j++)
    {
      gsm_yp[j] = (*MSFIT::compute_gaussian_f) (
          gsm_t[j], (double *)(&MSFIT::base_params[0]));
      ...
      for (unsigned int i = 0; i < MSFIT::param_fixed.size (); i++)
        {
          if (!MSFIT::param_fixed[i] || MSFIT::comm_backref.count (i))
            {
              mals_saxs_win->gaussians[i]
                  = v->d[MSFIT::comm_backref.count (i) ? MSFIT::comm_backref[i]
                                                       : MSFIT::param_pos[i]];
              if (MSFIT::conc_ratios_map.count (i))
                {
                  mals_saxs_win->gaussians[i] *= MSFIT::conc_ratios_map[i];
...
void
US_Hydrodyn_Mals_Saxs_Fit::gsm_ih ()
{
  puts ("gsm_ih");
  update_common ();
...
void
US_Hydrodyn_Mals_Saxs_Fit::gsm_cg ()
{
  puts ("gsm_cg");
  update_common ();
...
void
US_Hydrodyn_Mals_Saxs_Fit::stop ()
{
  running = false;
}

void
US_Hydrodyn_Mals_Saxs_Fit::grid ()
{
  setup_run ();
  update_common ();
  puts ("grid");
  ...
  unsigned int total_count
      = (unsigned int)pow ((double)pop_size, (int)MSFIT::init_params.size ());
  ...
  for (unsigned int j = 0; j < (unsigned int)MSFIT::init_params.size (); j++)
    {
      ofs_per[j]
          = (MSFIT::param_max[j] - MSFIT::param_min[j]) / (pop_size - 1);
    }
  ...
  for (unsigned int i = 0; i < total_count; i++)
    {
      ...
      for (unsigned int j = 0; j < (unsigned int)MSFIT::init_params.size ();
           j++)
        {
