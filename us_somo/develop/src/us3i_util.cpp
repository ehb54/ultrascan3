...
QString US3i_Util::getToken(QString& s, const QString& separator) {
  QString token;
...
QString US3i_Util::new_guid(void) {
  return QUuid::createUuid().toString().mid(1, 36);
}

// Calculate the md5hash and size of a named file (full path)
QString US3i_Util::md5sum_file(QString filename) {
  QFile f(filename);
...
QString US3i_Util::toUTCDatetimeText(QString dttext, bool knownUTC) {
  ...
  if (knownUTC) {  // If known to be UTC, just insure no single 'T' and " UTC"
                   // suffix

    if (haveT) {  // If single 'T' between date and time, replace with ' '
      utctext.replace(ixT, 1, " ");
    }

    if (!haveUTC) {  // If no " UTC" suffix, append it
      utctext.append(" UTC");
    }
  }

  else {  // If we don't know that it's UTC, convert when " UTC" is missing

    if (!haveUTC) {  // If no " UTC", convert to UTC and append " UTC"
      QDateTime dt;

      if (dttext.indexOf("-") == 4) {  // If already in ISO form, get UTC
                                       // DateTime from string in ISO form
        dt = QDateTime::fromString(dttext, Qt::ISODate).toUTC();
      }

      else {  // If in unknown form, get UTC DateTime assuming default form
...
QString US3i_Util::toISODatetimeText(QString dttext) {
  QString isotext = dttext;  // default assumes already in ISO form

  if (dttext.contains(" UTC")) {  // If UTC form on input, convert by removing "
                                  // UTC" and adding 'T'
    isotext.replace(" UTC", "").replace(dttext.indexOf(" "), 1, "T");
  }

  else if (dttext.indexOf("-") != 4) {  // If not ISO (unknown), convert by
                                        // back-and-forth from DateTime
...
QString US3i_Util::compressed_triple(const QString& ccw) {
...
// Convert a triple string from expanded to compressed form
QString US3i_Util::expanded_triple(const QString& ccw, bool spaces) {
...
unsigned char US3i_Util::hex2int(unsigned char c) {
  if (c <= '9') return (unsigned char)(c - '0');
...
void US3i_Util::uuid_parse(const QString& in, unsigned char* uu) {
...
int US3i_Util::listlistBuild(QList<QStringList>& lsl, QString& llstring) {
  int nlelem = lsl.count();
  // qDebug() << "llB: nlelem" << nlelem;

  if (nlelem == 0) return nlelem;
  ...
  for (int ii = 0; ii < delims.count(); ii++) {
    ...
    if (kfound == 0) {  // This delimeter not found:  use it for one of the two
      ndelimf++;
  ...
  // qDebug() << "llB: ndelimf" << ndelimf << "delim1" << delim1 << "delim2" <<
  // delim2;
  if (ndelimf < 2) {  // Ouch!!! Couldn't find two delimeters to use
    llstring = QString("");
    return 0;
  }
...
//   each stringlist.
int US3i_Util::listlistParse(QList<QStringList>& lsl, QString& llstring) {
  int nlelem = 0;
  ...
  if (delim1.isEmpty() ||
      delim2.isEmpty()) {  // Ouch!!  Input string doesn't even have delimeters
    lsl.clear();
    return nlelem;
  }
