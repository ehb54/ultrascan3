...
void
mQTextEdit::set_cli_progress (bool &cli_progress)
{
  this->cli_progress = &cli_progress;
}

void
mQTextEdit::set_cli_prefix (QString cli_prefix)
{
...
void
mQProgressBar::setValue (int val)
{
      ...
      if (lastprogress != newprogress)
        {
          ...
          qStdOut ()
              << QString ("~pgrs %1 : %2\n").arg (cli_prefix).arg (newprogress)
              << Qt::flush;
        }
    }
...
void
mQProgressBar::set_cli_progress (bool &cli_progress)
{
  this->cli_progress = &cli_progress;
}

void
mQProgressBar::set_cli_prefix (QString cli_prefix)
{
  this->cli_prefix = cli_prefix;
}
...
void
mQwtPlot::resizeEvent (QResizeEvent *e)
{
...
void
mQLineEdit::focusInEvent (QFocusEvent *e)
{
...
void
mQLineEdit::focusOutEvent (QFocusEvent *e)
{
...
void
mQLineEdit::mousePressEvent (QMouseEvent *e)
{
...
void
mQLabel::mousePressEvent (QMouseEvent *e)
{
...
void
mQLabel::resizeEvent (QResizeEvent *e)
{
...
mQPushButton::mQPushButton (const QString &text, QWidget *parent)
...
void
mQPushButton::mouseDoubleClickEvent (QMouseEvent *e)
{
...
void
mQThread::sleep (unsigned long secs)
{
  QThread::sleep (secs);
}

void
mQThread::msleep (unsigned long msecs)
{
  QThread::msleep (msecs);
}

void
mQThread::usleep (unsigned long usecs)
{
  QThread::usleep (usecs);
}

void
ShowHide::hide_widgets (const std::set<QWidget *> &widgets, bool hide,
                        QWidget *do_resize)
{
...
void
ShowHide::hide_widgets (const std::set<QWidget *> &widgets,
                        const std::set<QWidget *> &always_hide_widgets,
                        bool hide, QWidget *do_resize)
{
  for (auto const &widget : widgets)
    {
      hide || always_hide_widgets.count (widget) ? widget->hide ()
                                                 : widget->show ();
    }
  if (do_resize)
    {
...
void
ShowHide::hide_widgets (const std::vector<QWidget *> &widgets, bool hide,
                        QWidget *do_resize)
{
  std::set<QWidget *> always_hide_widgets;
  return hide_widgets (widgets, always_hide_widgets, hide, do_resize);
...
void
ShowHide::hide_widgets (const std::vector<QWidget *> &widgets,
                        const std::set<QWidget *> &always_hide_widgets,
                        bool hide, QWidget *do_resize)
{
...
void
ShowHide::hide_widgets (const std::vector<std::vector<QWidget *> > &widgets,
                        int row, bool hide, QWidget *do_resize)
{
  std::set<QWidget *> always_hide_widgets;
                        ...
                        bool hide, QWidget *do_resize)
{
  ...
  for (unsigned int i = 0; i < (unsigned int)widgets[row].size (); ++i)
    {
      hide || always_hide_widgets.count (widgets[row][i])
          ? widgets[row][i]->hide ()
          : widgets[row][i]->show ();
    }
  if (do_resize)
...
void
ShowHide::only_widgets (const std::vector<std::vector<QWidget *> > &widgets,
                        int row, bool hide, QWidget *do_resize)
{
  std::set<QWidget *> always_hide_widgets;
...
QStringList
MQT::get_lb_qsl (QListWidget *lb, bool only_selected)
{
  QStringList qsl;
...
void
PC::push_back_color_if_ok (QColor bg, QColor set)
{
