...
US_Hydrodyn_Cluster_Dmd::US_Hydrodyn_Cluster_Dmd(csv &csv1, void *us_hydrodyn,
  ...
  // check pdbs
  {
    ...
    for (int i = 0;
         i <
         (int)((US_Hydrodyn_Cluster *)cluster_window)->selected_files.size();
         i++) {
...
void US_Hydrodyn_Cluster_Dmd::cancel() { close(); }
...
void US_Hydrodyn_Cluster_Dmd::recompute_interval_from_points(
  ...
  for (unsigned int i = 0; i < (unsigned int)t_csv->rowCount(); i++) {
    QString toset = t_csv->item(i, basecol + 1)->text().toDouble() == 0e0
                        ? ""
                        : QString("%1").arg(
...
void US_Hydrodyn_Cluster_Dmd::recompute_points_from_interval(
    unsigned int basecol) {
  ...
  for (unsigned int i = 0; i < (unsigned int)t_csv->rowCount(); i++) {
    QString toset =
        t_csv->item(i, basecol + 2)->text().toDouble() == 0e0
            ? ""
            : QString("%1").arg(
                  (unsigned int)(t_csv->item(i, basecol)->text().toDouble() /
  ...
  for (int i = 0; i < t_csv->rowCount(); i++) {
    ...
    if (t_csv->item(i, 0)->isSelected()) {
      ...
      for (unsigned int j = 1;
           j < (unsigned int)csv_copy.data[pos % csv_copy.data.size()].size();
           j++) {
        // editor_msg( "black", QString( "setting data to row %1 from pos %2 %3
        // val %4 j %5" ) .arg( i ) .arg( pos ) .arg( pos % csv_copy.data.size()
        // ) .arg( csv_copy.data    [ pos % csv_copy.data.size() ][ j ] ) .arg(
        // j )
        // );
...
void US_Hydrodyn_Cluster_Dmd::load() {
  ...
  if (!QFile::exists(fname)) {
    QMessageBox::warning(this, us_tr("US-SOMO : Cluster DMD Setup : Load"),
...
void US_Hydrodyn_Cluster_Dmd::save_csv() {
  ...
  QString fname = QFileDialog::getSaveFileName(
      this, us_tr("Choose a filename to save the dmd run parameters"), use_dir,
      "*.dmd *.dmd");
    ...
    for (unsigned int j = 0; j < (unsigned int)from_csv.data[i].size(); j++) {
      qs += QString("%1%2")
                ...
                .arg(from_csv.data[i][j] ==
                             QString("%1").arg(from_csv.data[i][j].toDouble())
                         ? from_csv.data[i][j]
...
bool US_Hydrodyn_Cluster_Dmd::setup_residues(QString filename) {
  ...
  if (!f.exists()) {
    editor_msg(
        "red",
        QString(us_tr("Error: file %1 does not exist")).arg(full_filename));
    return false;
  }
  ...
  if (!f.open(QIODevice::ReadOnly)) {
    editor_msg(
        "red",
        QString(us_tr("Error: file %1 can not be opened.\nCheck permissions"))
            .arg(full_filename));
    return false;
  }
    ...
    if (rx_atom.indexIn(line) != -1) {
      ...
      if (last_key.isEmpty() || this_key != last_key) {
        if (dup_chain.count(this_key)) {
          editor_msg(
              "red",
              QString(us_tr("Error: repeated residue or chain %1 in file %2"))
                  .arg(this_key)
                  .arg(full_filename));
          f.close();
          return false;
  ...
  if (!residues_chain.count(filename)) {
    editor_msg("red",
               QString(us_tr("Error: no chains or residues found file %1"))
                   .arg(full_filename));
    return false;
  }
...
void US_Hydrodyn_Cluster_Dmd::residue_summary(QString filename) {
  ...
  if (!residues_chain.count(filename)) {
    editor_msg(
        "red",
        QString(us_tr("Internal Error: filename %1 not prepared for summary"))
            .arg(filename));
    return;
  }
  ...
  for (unsigned int i = 0;
    ...
    if (!residues_range_start.count(key)) {
      editor_msg("red", QString(us_tr("Internal Error: filename %1 range %2 "
    ...
    if (!residues_range_start[key].size()) {
      editor_msg(
          "red",
          QString(us_tr("Internal Error: filename %1 range %2 empty start key"))
              .arg(filename)
              .arg(key));
      return;
    }
    if (!residues_range_end[key].size()) {
      editor_msg(
          "red",
          QString(us_tr("Internal Error: filename %1 range %2 empty end key"))
              .arg(filename)
              .arg(key));
      return;
    }
    if (residues_range_end[key].size() != residues_range_end[key].size()) {
      editor_msg(
          "red",
          QString(
              us_tr("Internal Error: filename %1 range %2 start end mismatch"))
              .arg(filename)
              .arg(key));
      return;
    }
    ...
    int resseq_start =
        (int)residues_range_start[key][residues_range_chain_pos[filename][i]];
    int resseq_end =
        (int)residues_range_end[key][residues_range_chain_pos[filename][i]];
          ...
          QString(us_tr("%1: Chain %2 [%3] residue range: %4 - %5\n"))
              ...
              .arg(dmd_chain_start == dmd_chain_end
                       ...
                       : QString("%1-%2")
                             .arg(dmd_chain_start)
                             .arg(dmd_chain_end))
              .arg(residues_range_start[key]
                                       [residues_range_chain_pos[filename][i]])
              .arg(residues_range_end[key]
                                     [residues_range_chain_pos[filename][i]]));
    } else {
      editor_msg(
          "dark blue",
...
bool US_Hydrodyn_Cluster_Dmd::convert_static_range(int row) {
  if ((unsigned int)t_csv->columnCount() <= 11) {
    qDebug() << "US_Hydrodyn_Cluster_Dmd::convert_static_range() insufficient "
                "columns";
    return true;
  }
  ...
  for (unsigned int i = 0; i < (unsigned int)qsl.size(); i++) {
    ...
    if (start_residue > end_residue) {
      editor_msg(
          "red",
          QString(us_tr("Error: Static range \"%1\" has an invalid format.\n"
                        "The end residue number is less that the start residue "
                        "number\n"))
              .arg(qsl[i]));
      has_errors = true;
      continue;
    ...
    if (!residues_chain_map.count(filename) ||
      ...
      editor_msg("red", QString(us_tr("Error: No matching chain found for "
    ...
    for (unsigned int j = 0;
      ...
      if (this_chain_start <= this_chain_end) {
        {
          QString dmd_static;
          if (!dmd_native_range(filename, chain_id, (int)this_chain_start,
    ...
    for (unsigned int j = start_residue; j <= end_residue; j++) {
      ...
      if (used_bits[j] == 0) {
        editor_msg("dark red", QString(us_tr("Warning: Unused static residue "
      ...
      editor_msg("dark red",
                 QString(us_tr("Warning: Multiply used static residue %1:%2 "
                               "from static entry %3"))
                     .arg(chain_id)
                     .arg(j)
                     .arg(qsl[i]));
...
bool US_Hydrodyn_Cluster_Dmd::dmd_native_range(const QString &filename,
  ...
  if (!dmd_chain.count(filename) || !dmd_res_link.count(filename)) {
    editor_msg("dark red", QString(us_tr("Error: Internal. dmd_native_range() "
                                         "Could not find %1 in maps"))
                               .arg(filename));
    return false;
  }
  ...
  if (!dmd_chain[filename].count(chainid) ||
    ...
    editor_msg("dark red", QString(us_tr("Error: Internal. dmd_native_range() "
                                         "Could not find %1 chain %2 in maps"))
                               .arg(filename)
                               .arg(chainid));
    return false;
  }
  ...
  if (!dmd_chain[filename][chainid].count(resseq_start) ||
    ...
    editor_msg("dark red",
               QString(us_tr("Error: Internal. dmd_native_range() Could not "
                             "find %1 chain %2 residue %3 (start) in maps"))
                   .arg(filename)
                   .arg(chainid)
                   .arg(resseq_start));
    return false;
  ...
  if (!dmd_chain[filename][chainid].count(resseq_end) ||
    ...
    editor_msg("dark red",
               QString(us_tr("Error: Internal. dmd_native_range() Could not "
                             "find %1 chain %2 residue %3 (end) in maps"))
                   .arg(filename)
                   .arg(chainid)
                   .arg(resseq_end));
    return false;
