...
US_Hydrodyn_Cluster_Dmd::US_Hydrodyn_Cluster_Dmd (csv &csv1, void *us_hydrodyn,
    ...
    : QDialog (p)
{
  ...
  // check pdbs
  {
    ...
    for (int i = 0; i < (int)((US_Hydrodyn_Cluster *)cluster_window)
                            ->selected_files.size ();
         i++)
      {
        QString full_filename
            = ((US_Hydrodyn_Cluster *)cluster_window)->selected_files[i];
        ...
        if (!us->dmd_pdb_prepare (full_filename))
          {
            ...
            qDebug () << "dmd_pdb_prepare() : errors : " << us->errormsg
                      << Qt::endl;
            ((US_Hydrodyn *)us_hydrodyn)->dmd_all_pdb_prepare_reports[filename]
                = us->dmd_pdb_prepare_reports;
            ((US_Hydrodyn *)us_hydrodyn)
                    ->dmd_all_pdb_prepare_reports[filename]["errors"]
                << us->errormsg;
            if (!us->noticemsg.isEmpty ())
              {
                ((US_Hydrodyn *)us_hydrodyn)
                        ->dmd_all_pdb_prepare_reports[filename]["notice"]
                    << us->noticemsg;
              }
          }
        else
  ...
  dmd_dir = ((US_Hydrodyn *)us_hydrodyn)->somo_dir + SLASH + "cluster" + SLASH
            + "dmd";
  QDir dir1 (dmd_dir);
  if (!dir1.exists ())
    {
      editor_msg ("black",
                  QString (us_tr ("Created directory %1")).arg (dmd_dir));
      dir1.mkdir (dmd_dir);
    }
...
void
US_Hydrodyn_Cluster_Dmd::setupGUI ()
{
  int minHeight1 = 30;
  ...
  t_csv->setSelectionMode (QAbstractItemView::SingleSelection);
  ...
  column_widths << 120 << 60 << 95 << 105 << 102 << 108 << 88 << 100 << 108
                << 100 << 200 << 200;
  ...
  };
  {
    for (int i = 0; i < t_csv->rowCount (); ++i)
      {
        t_csv->item (i, 11)->setFlags (t_csv->item (i, 11)->flags ()
                                       ^ Qt::ItemIsEditable);
      }
  };
  ...
  connect (t_csv->verticalHeader (), SIGNAL (sectionPressed (int)),
           SLOT (row_header_released (int)));
  ...
  pb_copy->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize + 1));
  ...
  pb_paste->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize + 1));
  ...
  pb_paste_all->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize + 1));
  ...
  pb_dup->setFont (QFont (USglobal->config_list.fontFamily,
                          USglobal->config_list.fontSize + 1));
  ...
  QHBoxLayout *hbl_ctls = new QHBoxLayout ();
  hbl_ctls->setContentsMargins (0, 0, 0, 0);
  hbl_ctls->setSpacing (0);
...
void
US_Hydrodyn_Cluster_Dmd::cancel ()
{
  close ();
}

void
US_Hydrodyn_Cluster_Dmd::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Cluster_Dmd::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept ();
...
void
US_Hydrodyn_Cluster_Dmd::table_value (int row, int col)
{
  if (col == 3 || col == 4)
    {
      recompute_interval_from_points (3);
...
void
US_Hydrodyn_Cluster_Dmd::clear_display ()
{
  editor->clear ();
  editor->append ("\n\n");
}
...
void
US_Hydrodyn_Cluster_Dmd::update_font ()
{
  bool ok;
  QFont newFont;
...
csv
US_Hydrodyn_Cluster_Dmd::current_csv ()
{
  csv tmp_csv = csv1;
...
void
US_Hydrodyn_Cluster_Dmd::recompute_interval_from_points (unsigned int basecol)
{
  if ((unsigned int)t_csv->columnCount () <= basecol + 1)
    {
      qDebug () << "US_Hydrodyn_Cluster_Dmd::recompute_interval_from_points() "
  ...
  for (unsigned int i = 0; i < (unsigned int)t_csv->rowCount (); i++)
    {
      QString toset
          = t_csv->item (i, basecol + 1)->text ().toDouble () == 0e0
                ? ""
                : QString ("%1").arg (
                      t_csv->item (i, basecol)->text ().toDouble ()
...
void
US_Hydrodyn_Cluster_Dmd::recompute_points_from_interval (unsigned int basecol)
{
  if ((unsigned int)t_csv->columnCount () <= basecol + 2)
    {
      qDebug () << "US_Hydrodyn_Cluster_Dmd::recompute_points_from_interval() "
                   "insufficient columns";
      return;
    }
  for (unsigned int i = 0; i < (unsigned int)t_csv->rowCount (); i++)
    {
      QString toset = t_csv->item (i, basecol + 2)->text ().toDouble () == 0e0
                          ? ""
                          : QString ("%1").arg (
                                (unsigned int)(t_csv->item (i, basecol)
                                                   ->text ()
                                                   .toDouble ()
                                               / t_csv->item (i, basecol + 2)
                                                     ->text ()
                                                     .toDouble ()));
      disconnect (t_csv, SIGNAL (cellChanged (int, int)), 0, 0);
...
void
US_Hydrodyn_Cluster_Dmd::editor_msg (QString color, QString msg)
{
...
void
US_Hydrodyn_Cluster_Dmd::init_csv ()
{
...
void
US_Hydrodyn_Cluster_Dmd::reset_csv ()
{
  init_csv ();
  ...
  for (unsigned int i = 0;
       i < (unsigned int)((US_Hydrodyn_Cluster *)cluster_window)
               ->selected_files.size ();
       i++)
    {
          ...
          if (full_filenames[filename] != full_filename)
            {
              editor_msg (
                  "dark red",
                  QString (us_tr ("Warning: The same file name has been "
                                  "referenced in multiple file locations.\n"
                                  "%1 vs %2\n"
                                  "This will cause problems.  Duplicate "
                                  "reference skipped.\n"))
...
US_Hydrodyn_Cluster_Dmd::copy ()
{
  ...
  // editor_msg( "black", QString( "csv copy has %1 rows" ).arg(
  // csv_copy.data.size() ) );

  // TSO << "csv1 after copy():\n" << csv_to_qstring( csv1 ) << Qt::endl;
  // TSO << "csv_copy after copy():\n" << csv_to_qstring( csv_copy ) <<
  // Qt::endl;
      ...
      if (t_csv->item (i, 0)->isSelected ())
        {
          ...
          for (unsigned int j = 1;
               j < (unsigned int)csv_copy.data[pos % csv_copy.data.size ()]
                       .size ();
               j++)
            {
              // editor_msg( "black", QString( "setting data to row %1 from pos
              // %2 %3 val %4 j %5" ) .arg( i ) .arg( pos ) .arg( pos %
              // csv_copy.data.size() ) .arg( csv_copy.data    [ pos %
              // csv_copy.data.size() ][ j ] ) .arg( j )
              // );
              csv1.data[i][j] = csv_copy.data[pos % csv_copy.data.size ()][j];
              csv1.num_data[i][j]
                  = csv_copy.num_data[pos % csv_copy.data.size ()][j];
            }
          pos++;
        }
    }
...
void
US_Hydrodyn_Cluster_Dmd::paste_all ()
{
  csv1 = current_csv ();
  unsigned int pos = 0;
  for (int i = 0; i < t_csv->rowCount (); i++)
    {
      for (unsigned int j = 1;
           j < csv_copy.data[pos % csv_copy.data.size ()].size (); j++)
        {
          csv1.data[i][j] = csv_copy.data[pos % csv_copy.data.size ()][j];
          csv1.num_data[i][j]
              = csv_copy.num_data[pos % csv_copy.data.size ()][j];
...
void
US_Hydrodyn_Cluster_Dmd::load ()
{
  QString use_dir = dmd_dir;

  QString fname = QFileDialog::getOpenFileName (
      this, windowTitle (), use_dir, "DMD parameter files (*.dmd *.dmd)");
  ...
  if (!QFile::exists (fname))
    {
      QMessageBox::warning (
          this, us_tr ("US-SOMO : Cluster DMD Setup : Load"),
  ...
  for (map<QString, bool>::iterator it = names_not_present.begin ();
       it != names_not_present.end (); it++)
    {
      bool ok;
      QString res = US_Static::getItem (
          us_tr ("US-SOMO : Cluster DMD Setup : Load : Missing name"),
          QString (us_tr ("%1 is found in the DMD file but is not a currently "
                          "selected PDB.\n"
                          "Select a remapping or cancel to ignore:"))
              .arg (it->first),
          qsl_our_files, 0, false, &ok, this);
      if (ok)
        {
              ...
              for (unsigned int j = 1; j < (unsigned int)qsl.size (); j++)
                {
                  ...
                  csv_new.num_data[our_files[qsl[0]].front ()][j]
                      = qsl[j].toDouble ();
                }
...
void
US_Hydrodyn_Cluster_Dmd::save_csv ()
{
  QString use_dir = dmd_dir;

  QString fname = QFileDialog::getSaveFileName (
      this, us_tr ("Choose a filename to save the dmd run parameters"),
      use_dir, "*.dmd *.dmd");
  ...
  if (!f.open (QIODevice::WriteOnly))
    {
      QMessageBox::warning (
          this, us_tr ("US-SOMO : Cluster DMD Setup : Save"),
  ...
  f.close ();
  QMessageBox::information (this, us_tr ("US-SOMO : Cluster DMD Setup : Save"),
                            QString (us_tr ("File %1 saved")).arg (fname));
}

void
US_Hydrodyn_Cluster_Dmd::update_enables ()
{
  if (!disable_updates)
    {
      // editor_msg( "black", "-------------" );
...
void
US_Hydrodyn_Cluster_Dmd::reload_csv ()
{
...
void
US_Hydrodyn_Cluster_Dmd::delete_rows ()
{
...
void
US_Hydrodyn_Cluster_Dmd::select_all ()
{
  disable_updates = true;
  int selected = 0;
  vector<int> selected_rows;
...
QString
US_Hydrodyn_Cluster_Dmd::csv_to_qstring (csv &from_csv)
{
  QString qs;
      ...
      for (unsigned int j = 0; j < (unsigned int)from_csv.data[i].size (); j++)
        {
          qs += QString ("%1%2")
                    .arg (j ? "," : "")
                    .arg (from_csv.data[i][j]
                                  == QString ("%1").arg (
                                      from_csv.data[i][j].toDouble ())
                              ? from_csv.data[i][j]
...
void
US_Hydrodyn_Cluster_Dmd::row_header_released (int row)
{
...
void
US_Hydrodyn_Cluster_Dmd::sync_csv_with_selected ()
{
  csv1 = current_csv ();
  csv csv_new = csv1;
  csv_new.data.clear ();
  ...
  for (unsigned int i = 0;
       ...
       i++)
    {
      selected_map[QFileInfo (((US_Hydrodyn_Cluster *)cluster_window)
                                  ->selected_files[i])
                       .fileName ()]
          = true;
...
QStringList
US_Hydrodyn_Cluster_Dmd::csv_parse_line (QString qs)
{
...
bool
US_Hydrodyn_Cluster_Dmd::setup_residues (QString filename)
{
  if (!full_filenames.count (filename))
    {
      editor_msg (
          "red",
          QString (us_tr ("Internal Error: filename %1 has not been cached"))
              .arg (filename));
      return false;
    }
  ...
  if (!f.exists ())
    {
      editor_msg ("red", QString (us_tr ("Error: file %1 does not exist"))
                             .arg (full_filename));
      return false;
    }
  ...
  if (!f.open (QIODevice::ReadOnly))
    {
      editor_msg (
          "red",
          QString (
              us_tr ("Error: file %1 can not be opened.\nCheck permissions"))
              .arg (full_filename));
      return false;
    }
  ...
  for (map<QString, vector<unsigned int> >::iterator it
       = residues_range_start.begin ();
       it != residues_range_start.end (); it++)
    {
      ...
      if (rx_atom.indexIn (line) != -1)
        {
          ...
          // line.left(30) ).arg( chain_id ).arg( this_key );

          if (last_key.isEmpty () || this_key != last_key)
            {
              if (dup_chain.count (this_key))
                {
                  editor_msg ("red", QString (us_tr ("Error: repeated residue "
                                                     "or chain %1 in file %2"))
                                         .arg (this_key)
                                         .arg (full_filename));
                  f.close ();
                  return false;
  ...
  if (!residues_chain.count (filename))
    {
      editor_msg (
          "red", QString (us_tr ("Error: no chains or residues found file %1"))
                     .arg (full_filename));
      return false;
    }
  ...
  if (residues_chain[filename].size () == 1)
    {
      editor_msg (
          "red",
          QString (us_tr ("Error: only one residue %1:%2 found in file %2"))
  ...
  for (unsigned int i = 0; i < (unsigned int)residues_chain[filename].size ();
       i++)
    {
      ...
      if (last_chain != residues_chain[filename][i])
        {
          ...
          residues_range_chain_pos[filename].push_back (
              residues_range_start[key].size () - 1);
          // TSO << "in residues_chain - pushback: " << key << " chain " <<
          // residues_chain[ filename ][ i ] << Qt::endl;
...
void
US_Hydrodyn_Cluster_Dmd::residue_summary (QString filename)
{
  ...
  if (!residues_chain.count (filename))
    {
      editor_msg (
          "red",
          QString (
              us_tr ("Internal Error: filename %1 not prepared for summary"))
              .arg (filename));
      return;
    }
  ...
  unsigned int pos = 0;
  for (unsigned int i = 0;
       i < (unsigned int)residues_range_chain[filename].size (); i++)
    {
      ...
      if (!residues_range_start.count (key))
        {
          editor_msg ("red",
                      QString (us_tr ("Internal Error: filename %1 range %2 "
                                      "not prepared for summary"))
                          .arg (filename)
                          .arg (key));
          return;
        }
      ...
      if (!residues_range_start[key].size ())
        {
          editor_msg (
              ...
              QString (
                  us_tr (
                      "Internal Error: filename %1 range %2 empty start key"))
                  .arg (filename)
                  .arg (key));
          return;
        }
      if (!residues_range_end[key].size ())
        {
          editor_msg (
              "red",
              QString (
                  us_tr ("Internal Error: filename %1 range %2 empty end key"))
                  .arg (filename)
                  .arg (key));
          return;
        }
      if (residues_range_end[key].size () != residues_range_end[key].size ())
        {
          editor_msg ("red", QString (us_tr ("Internal Error: filename %1 "
                                             "range %2 start end mismatch"))
                                 .arg (filename)
                                 .arg (key));
          return;
        }
      ...
      int resseq_start = (int)
          residues_range_start[key][residues_range_chain_pos[filename][i]];
      int resseq_end = (int)
          residues_range_end[key][residues_range_chain_pos[filename][i]];

      bool is_hetatm
          = dmd_chain_is_hetatm[filename].count (chain_id)
            && dmd_chain_is_hetatm[filename][chain_id].count (resseq_start)
                   > 0;
      if (is_hetatm)
        {
          bool ok = true;
          if (!dmd_chain.count (filename) || !dmd_res_link.count (filename))
            {
              editor_msg (
                  "dark red",
                  QString (us_tr ("Error: Internal. dmd_native_range() Could "
                                  "not find %1 in maps"))
                      .arg (filename));
              ok = false;
            }
          ...
          int dmd_chain_start
              = ok ? dmd_chain[filename][chain_id][resseq_start] : -1;
          int dmd_chain_end
              = ok ? dmd_chain[filename][chain_id][resseq_end] : -1;

          editor_msg (
              "dark blue",
              QString (us_tr ("%1: Chain %2 [%3] residue range: %4 - %5\n"))
                  .arg (filename)
                  .arg (chain_id)
                  .arg (dmd_chain_start == dmd_chain_end
                            ...
                            : QString ("%1-%2")
                                  .arg (dmd_chain_start)
                                  .arg (dmd_chain_end))
                  .arg (residues_range_start
                            [key][residues_range_chain_pos[filename][i]])
                  .arg (residues_range_end
                            [key][residues_range_chain_pos[filename][i]]));
        }
      else
        {
...
bool
US_Hydrodyn_Cluster_Dmd::convert_static_range (int row)
{
  if ((unsigned int)t_csv->columnCount () <= 11)
    {
      qDebug () << "US_Hydrodyn_Cluster_Dmd::convert_static_range() "
                   "insufficient columns";
      return true;
    }
  ...
  for (unsigned int i = 0; i < (unsigned int)qsl.size (); i++)
    {
      ...
      if (rx.indexIn (qsl[i]) == -1)
        {
          editor_msg (
              "red",
              QString (
                  us_tr ("Error: Static range \"%1\" has an invalid format.\n"
                         "The format must be \"Chain:residue_number\" or "
                         "\"Chain:start_residue_number-end_residue_residue_"
                         "number\"\n"))
                  .arg (qsl[i]));
          has_errors = true;
          continue;
      ...
      if (start_residue > end_residue)
        {
          editor_msg (
              ...
              QString (
                  us_tr ("Error: Static range \"%1\" has an invalid format.\n"
                         "The end residue number is less that the start "
                         "residue number\n"))
                  .arg (qsl[i]));
          has_errors = true;
          continue;
      ...
      if (!residues_chain_map.count (filename)
        ...
        {
          editor_msg ("red",
                      QString (us_tr ("Error: No matching chain found for "
                                      "static range \"%1\" chain %2."))
                          .arg (qsl[i])
                          .arg (chain_id));
          has_errors = true;
          continue;
      ...
      for (unsigned int j = 0;
           j < (unsigned int)residues_range_chain[filename].size (); j++)
        {
          ...
          if (this_chain_start <= this_chain_end)
            {
              {
                QString dmd_static;
                if (!dmd_native_range (filename, chain_id,
                                       (int)this_chain_start,
                                       (int)this_chain_end, dmd_static))
                  {
                    continue;
                  }
                native_range
                    += (native_range.isEmpty () ? "" : ",") + dmd_static;
              }
              ...
              //          QString( "%1" ).arg( this_chain_start -
              //          this_chain_base + 1 ) )
              //    ;
      ...
      for (unsigned int j = start_residue; j <= end_residue; j++)
        {
          ...
          if (used_bits[j] == 0)
            {
              editor_msg ("dark red",
                          QString (us_tr ("Warning: Unused static residue "
                                          "%1:%2 from static entry %3"))
                              .arg (chain_id)
                              .arg (j)
                              .arg (qsl[i]));
              continue;
          ...
          editor_msg ("dark red",
                      QString (us_tr ("Warning: Multiply used static residue "
                                      "%1:%2 from static entry %3"))
                          .arg (chain_id)
                          .arg (j)
                          .arg (qsl[i]));
...
bool
US_Hydrodyn_Cluster_Dmd::dmd_native_range (const QString &filename,
                                           ...
                                           QString &dmd_static)
{
  ...
  if (!dmd_chain[filename][chainid].count (resseq_start)
      || !dmd_res_link[filename][chainid].count (resseq_start))
    {
      editor_msg (
          "dark red",
          QString (us_tr ("Error: Internal. dmd_native_range() Could not find "
                          "%1 chain %2 residue %3 (start) in maps"))
              .arg (filename)
              .arg (chainid)
              .arg (resseq_start));
      return false;
  ...
  if (!dmd_chain[filename][chainid].count (resseq_end)
      || !dmd_res_link[filename][chainid].count (resseq_end))
    {
      editor_msg (
          "dark red",
          QString (us_tr ("Error: Internal. dmd_native_range() Could not find "
                          "%1 chain %2 residue %3 (end) in maps"))
              .arg (filename)
              .arg (chainid)
              .arg (resseq_end));
      return false;
  ...
  if (is_hetatm)
  ...
  else
    {
      if (dmd_res_start == dmd_res_end)
        ...
        {
          dmd_static
              += QString ("%1-%2").arg (dmd_res_start).arg (dmd_res_end);
        }
    }
  dmd_static += ".*";
  return true;
