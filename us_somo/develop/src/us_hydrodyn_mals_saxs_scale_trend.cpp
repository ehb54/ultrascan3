...
US_Hydrodyn_Mals_Saxs_Scale_Trend::US_Hydrodyn_Mals_Saxs_Scale_Trend (
...
void
US_Hydrodyn_Mals_Saxs_Scale_Trend::setupGUI ()
{
  ...
  lbl_results->setFont (QFont (USglobal->config_list.fontFamily,
...
void
US_Hydrodyn_Mals_Saxs_Scale_Trend::cancel ()
{
  close ();
}

void
US_Hydrodyn_Mals_Saxs_Scale_Trend::help ()
{
  US_Help *online_help;
  online_help = new US_Help (this);
  online_help->show_help ("manual/somo/mals_saxs_scale_trend.html");
}

void
US_Hydrodyn_Mals_Saxs_Scale_Trend::closeEvent (QCloseEvent *e)
{
...
void
US_Hydrodyn_Mals_Saxs_Scale_Trend::update_plot ()
{
...
void
US_Hydrodyn_Mals_Saxs_Scale_Trend::analysis ()
{
  us_qdebug ("analysis");
  ...
  for (map<QString, vector<vector<double> > >::iterator it = plots.begin ();
       it != plots.end (); ++it)
    {
      ...
      //    us_qdebug( US_Vector::qs_vector3( QString( "plot '%1'" ).arg(
      //    it->first ),
      //                                   it->second[ 0 ],
      //                                   it->second[ 1 ],
      //                                   it->second[ 2 ] ) );
  ...
  for (int i = 0; i < (int)names.size (); ++i)
    {
      ...
      if (use_errors
          && (!e.count (names[i]) || q_size != (int)e[names[i]].size ()
  ...
  US_Saxs_Util::linear_fit (slopes_x, slopes, slopes_e, fit_a, fit_b, fit_siga,
                            fit_sigb, fit_chi2);

  {
    ...
    QString msg = QString ("Slope = %1   r = %2")
                      .arg (fit_b)
                      .arg (r_ok ? QString ("%1").arg (r) : error_msg);
...
void
US_Hydrodyn_Mals_Saxs_Scale_Trend::usp_config_plot (const QPoint &)
{
