...
void US_Hydrodyn_Mals_Saxs_Scale_Trend::setupGUI() {
...
void US_Hydrodyn_Mals_Saxs_Scale_Trend::cancel() { close(); }

void US_Hydrodyn_Mals_Saxs_Scale_Trend::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/mals_saxs_scale_trend.html");
}

void US_Hydrodyn_Mals_Saxs_Scale_Trend::closeEvent(QCloseEvent *e) {
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept();
}

void US_Hydrodyn_Mals_Saxs_Scale_Trend::update_plot() {
...
void US_Hydrodyn_Mals_Saxs_Scale_Trend::analysis() {
  us_qdebug("analysis");
  ...
  for (int i = 0; i < (int)names.size(); ++i) {
    if (q_ref != q[names[i]]) {
      us_qdebug(
          US_Vector::qs_vector2(QString("scale trend: incompatible grids %1 %2")
                                    .arg(names[0])
                                    .arg(names[i]),
                                q_ref, q[names[i]]));
      return;
...
void US_Hydrodyn_Mals_Saxs_Scale_Trend::usp_config_plot(const QPoint &) {
