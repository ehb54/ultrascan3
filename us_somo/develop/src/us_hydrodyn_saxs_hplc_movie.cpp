    ...
    : QDialog (p)
{
  ...
  timer_msec = ush_win->gparams.count ("hplc_movie_timer_ms")
                   ? ush_win->gparams["hplc_movie_timer_ms"].toInt ()
                   : 1000;

  last_show_gauss
      = ush_win->gparams.count ("hplc_movie_show_gauss")
                && ush_win->gparams["hplc_movie_show_gauss"] == "true"
            ? true
            : false;

  last_show_ref = ush_win->gparams.count ("hplc_movie_show_ref")
                          && ush_win->gparams["hplc_movie_show_ref"] == "true"
                      ? true
                      : false;

  last_mono = ush_win->gparams.count ("hplc_movie_mono")
                      && ush_win->gparams["hplc_movie_mono"] == "true"
                  ? true
                  : false;
...
void
US_Hydrodyn_Saxs_Hplc_Movie::setupGUI ()
{
  int minHeight1 = 24;
  int minHeight3 = 25;
  ...
  lbl_current = new QLabel (
      hplc_win->lb_files->item (hplc_selected_files[pos])->text (), this);
...
void
US_Hydrodyn_Saxs_Hplc_Movie::cancel ()
{
  close ();
}

void
US_Hydrodyn_Saxs_Hplc_Movie::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Saxs_Hplc_Movie::closeEvent (QCloseEvent *e)
{
  ...
  ush_win->gparams["hplc_movie_show_gauss"]
      = cb_show_gauss->isChecked () ? "true" : "false";
  ush_win->gparams["hplc_movie_show_ref"]
      = cb_show_ref->isChecked () ? "true" : "false";
  e->accept ();
}

void
US_Hydrodyn_Saxs_Hplc_Movie::front ()
{
  pos = 0;
  update_plot ();
}
...
void
US_Hydrodyn_Saxs_Hplc_Movie::prev ()
{
  if (pos > 0)
    {
      pos--;
...
void
US_Hydrodyn_Saxs_Hplc_Movie::slower ()
{
  timer_msec *= 2;
  if (timer_msec > 5000)
    {
...
void
US_Hydrodyn_Saxs_Hplc_Movie::start ()
{
  if (timer->isActive ())
    {
      timer->stop ();
...
void
US_Hydrodyn_Saxs_Hplc_Movie::faster ()
{
  timer_msec /= 2;
  if (timer_msec < 50)
    {
...
void
US_Hydrodyn_Saxs_Hplc_Movie::next ()
{
...
void
US_Hydrodyn_Saxs_Hplc_Movie::end ()
{
...
void
US_Hydrodyn_Saxs_Hplc_Movie::set_save ()
{
  update_enables ();
}

void
US_Hydrodyn_Saxs_Hplc_Movie::set_show_gauss ()
{
  update_plot ();
}

void
US_Hydrodyn_Saxs_Hplc_Movie::set_show_ref ()
{
  update_plot ();
}

void
US_Hydrodyn_Saxs_Hplc_Movie::set_mono ()
{
...
void
US_Hydrodyn_Saxs_Hplc_Movie::update_enables ()
{
  le_save->setEnabled (cb_save->isChecked ());
}

void
US_Hydrodyn_Saxs_Hplc_Movie::update_plot ()
{
  if (last_pos != pos || cb_show_gauss->isChecked () != last_show_gauss
      || cb_show_ref->isChecked () != last_show_ref)
    {
      ...
      if (cb_show_gauss->isChecked ()
          && hplc_win->f_is_time.count (
              hplc_win->lb_files->item (hplc_selected_files[pos])->text ())
          && hplc_win->f_is_time[hplc_win->lb_files
                                     ->item (hplc_selected_files[pos])
                                     ->text ()])
        {
...
void
US_Hydrodyn_Saxs_Hplc_Movie::save_plot ()
{
  if (hplc_win->plot_errors->isVisible ())
    {
      if (hplc_win->plot_ref->isVisible ())
        {
          save_plot (hplc_win->plot_dist, hplc_win->plot_ref,
                     hplc_win->plot_errors, le_save->text ());
        }
      else
        {
          save_plot (hplc_win->plot_dist, hplc_win->plot_errors,
                     le_save->text ());
        }
    }
  else
...
void
US_Hydrodyn_Saxs_Hplc_Movie::save_plot (QWidget *plot, QString tag)
{
  int mypos = pos;
...
void
US_Hydrodyn_Saxs_Hplc_Movie::join_maps (QPixmap &m1, QPixmap &m2)
{
  int m1h = m1.height ();
  int m2h = m2.height ();
...
void
US_Hydrodyn_Saxs_Hplc_Movie::save_plot (QWidget *plot, QWidget *plot2,
...
void
US_Hydrodyn_Saxs_Hplc_Movie::save_plot (QPixmap &qPix, QString tag, int mypos)
{
  QPainter paint (&qPix);
  paint.setPen (Qt::blue);
  paint.setFont (QFont (USglobal->config_list.fontFamily,
                        USglobal->config_list.fontSize - 1, QFont::Bold));
  // paint.drawText( 5, 5, hplc_win->lb_files->item( hplc_selected_files[ mypos
  // ] )->text() );
  paint.drawText (
      qPix.rect (), Qt::AlignBottom | Qt::AlignLeft,
      hplc_win->lb_files->item (hplc_selected_files[mypos])->text ());
