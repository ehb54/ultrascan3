...
US_Hydrodyn_Saxs_Hplc_Movie::US_Hydrodyn_Saxs_Hplc_Movie(
  ...
  last_show_gauss = ush_win->gparams.count("hplc_movie_show_gauss") &&
...
void US_Hydrodyn_Saxs_Hplc_Movie::cancel() { close(); }
...
void US_Hydrodyn_Saxs_Hplc_Movie::set_save() { update_enables(); }

void US_Hydrodyn_Saxs_Hplc_Movie::set_show_gauss() { update_plot(); }

void US_Hydrodyn_Saxs_Hplc_Movie::set_show_ref() { update_plot(); }
...
void US_Hydrodyn_Saxs_Hplc_Movie::update_plot() {
  if (last_pos != pos || cb_show_gauss->isChecked() != last_show_gauss ||
    ...
    if (cb_show_gauss->isChecked() &&
        ...
        hplc_win->f_is_time[hplc_win->lb_files->item(hplc_selected_files[pos])
                                ->text()]) {
...
void US_Hydrodyn_Saxs_Hplc_Movie::save_plot() {
  if (hplc_win->plot_errors->isVisible()) {
    if (hplc_win->plot_ref->isVisible()) {
      save_plot(hplc_win->plot_dist, hplc_win->plot_ref, hplc_win->plot_errors,
                le_save->text());
    } else {
...
void US_Hydrodyn_Saxs_Hplc_Movie::save_plot(QWidget *plot, QWidget *plot2,
...
void US_Hydrodyn_Saxs_Hplc_Movie::save_plot(QPixmap &qPix, QString tag,
                                            int mypos) {
  QPainter paint(&qPix);
