...
void US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames::cancel() {
  parameters->clear();
  close();
}

void US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/mals_saxs_conc_csv_frames.html");
}

void US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames::closeEvent(QCloseEvent *e) {
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept();
}

void US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames::showhide(vector<QWidget *> &widgets,
                                                     bool isChecked) {
...
void US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames::setupGUI() {
  ...
  int colmax = 0;
  lbl_label_0 =
      new QLabel(us_tr("Concentration CSV in Time to Frames conversion"), this);
  ...
  cb_startframe->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  cb_startframe->setPalette(PALET_NORMAL);
#if QT_VERSION > 0x040000
  AUTFBACK(cb_startframe);
  cb_startframe->setToolTip(
      us_tr("Optionally specify the starting frame number to extract, default "
            "is frame number 1 if not checked"));
#endif
  ...
  cb_endframe->setToolTip(
      us_tr("Optionally specify the ending frame number to extract, default is "
            "last frame number available"));
#endif
...
void US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames::update_starttime(
    const QString &str) {
  (*parameters)["starttime"] = str;
}

void US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames::update_endtime(const QString &str) {
  (*parameters)["endtime"] = str;
}

void US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames::update_pointcount(
    const QString &str) {
  (*parameters)["pointcount"] = str;
}

void US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames::update_frame1t(const QString &str) {
  (*parameters)["frame1t"] = str;
}

void US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames::update_time2frame(
    const QString &str) {
  (*parameters)["time2frame"] = str;
}

void US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames::set_startframe() {
  (*parameters)["startframe"] = cb_startframe->isChecked() ? "true" : "false";
  showhide(repeats[cb_startframe], cb_startframe->isChecked());
}

void US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames::update_startframenumber(
    const QString &str) {
  (*parameters)["startframenumber"] = str;
}

void US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames::set_endframe() {
  (*parameters)["endframe"] = cb_endframe->isChecked() ? "true" : "false";
  showhide(repeats[cb_endframe], cb_endframe->isChecked());
}

void US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames::update_endframenumber(
    const QString &str) {
  (*parameters)["endframenumber"] = str;
}

void US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames::go() {
