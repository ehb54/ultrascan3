...
US_Hydrodyn_Save::US_Hydrodyn_Save(save_info *save, void *us_hydrodyn,
                                   bool *save_widget, QWidget *p, const char *)
    : QFrame(p) {
  this->save = save;
...
void US_Hydrodyn_Save::cancel() { close(); }
...
void US_Hydrodyn_Save::rebuild() {
  ...
  for (int i = 0; i < lb_selected->count(); i++) {
    save->field_flag
        [field[descriptive_name_to_pos[lb_selected->item(i)->text()]]] = true;
...
void US_Hydrodyn_Save::tab_changed(int) { update_enables_possible(); }
...
QString US_Hydrodyn_Save::header() {
  ...
  for (unsigned int i = 0; i < save->field.size(); i++) {
    this_field =
        descriptive_name[field_to_pos[save->field[i]]].replace("\"", "''");
...
#define FARG(s) \
  arg(s, 0, field_to_format[save->field[i]], field_to_precision[save->field[i]])
        ...
        *((double *)(field_to_save_data[field[i]])) /= tmp_count_double;
...

        {
          ...
          if (tmp_vmin == tmp_vmax) {
            ...
            *((double *)(field_to_save_data[field[i]])) =
                sqrt(fabs((*((double *)(field_to_save_data[field[i]])) -
        ...
        *((double *)(field_to_save_data[field[i]]) + 2) /= tmp_count_double;
...

        {
          ...
          if (tmp_vmin1 == tmp_vmax1) {
            ...
            *((double *)(field_to_save_data[field[i]]) + 1) =
                sqrt(fabs((*((double *)(field_to_save_data[field[i]]) + 1) -
          ...
          if (tmp_vmin2 == tmp_vmax2) {
            ...
            *((double *)(field_to_save_data[field[i]]) + 2) =
                sqrt(fabs((*((double *)(field_to_save_data[field[i]]) + 2) -
...
QString US_Hydrodyn_Save::hydroFormatStats(vector<save_data> stats,
  ...
  result = "\n\t AVERAGE PARAMETERS \n\n\t\t\t\t Mean value\tSt. Dev.\n\n";
  result +=
      QString("").sprintf("- TRANS. FRICT. COEFF.        \t%.3e\t%.3e\t[g/s]\n",
  ...
  //   {
  result +=
      QString("").sprintf("- SED. COEFF. (psv %s) \t%.2f\t\t%.2f\t\t[S]\n",
                          bead_model_source.toLatin1().data(),
                          stats[0].results.s20w, stats[1].results.s20w);
  //   }

  result +=
      QString("").sprintf("- FRICTIONAL RATIO            \t%.2f\t%.3e\n\n",
                          stats[0].results.ff0, stats[1].results.ff0);
...
save_data US_Hydrodyn_Save::save_data_initialized_from_bead_model(
