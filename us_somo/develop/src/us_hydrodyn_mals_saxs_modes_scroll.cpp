...
void
US_Hydrodyn_Mals_Saxs::scroll_pair ()
{
  TSO << "scroll_pairs()\n";
  ...
  // somewhat duplicated code in ::common_time() && ::join_by_time
  {
    ...
    if (qgrid_to_names.size () > 2 || !qgrid_to_names.size ())
      {
        ...
        QMessageBox::critical (
            this, windowTitle () + us_tr (": Scroll"),
            QString (us_tr ("The %1 curves selected have more than two unique "
                            "q-grids\n\n"
                            "%2 q-grid(s) found\n"))
                    .arg (names.size ())
    ...
    if (qgrid_to_names.size () == 1)
      {
        scroll_pair_names = { qgrid_to_names.begin ()->second };
      }
    else
      {
        scroll_pair_names = { qgrid_to_names.begin ()->second,
        ...
        if (time_grids[0] != time_grids[1])
          {
            QMessageBox::critical (
                this, windowTitle () + us_tr (": Scroll"),
                QString (us_tr ("The %1 curves selected do not have the same "
                                "times\n\n"))
                    .arg (names.size ()));
            return update_enables ();
          }
  ...
  if (scroll_pair_names.size () == 2)
    {
      ...
      int mals_set = f_qs[scroll_pair_names[0][0]].front ()
                             < f_qs[scroll_pair_names[1][0]].front ()
                         ? 0
                         : 1;
      ...
      for (int i = 0; i < (int)scroll_pair_times.size (); ++i)
        {
          scroll_pair_time_to_names[scroll_pair_times[i]].push_back (
              scroll_pair_names[mals_set][i]);
          scroll_pair_time_to_names[scroll_pair_times[i]].push_back (
              scroll_pair_names[saxs_set][i]);
        }
    }
  else
    {
...
void
US_Hydrodyn_Mals_Saxs::scroll_pair_enables ()
{
...
void
US_Hydrodyn_Mals_Saxs::scroll_pair_scroll_highlight (int pos)
{
  // show the plot
  // TSO << "scroll_pair_scroll_highlight( " <<  pos << " )\n";
  if (pos >= (int)scroll_pair_times.size ())
    {
      qDebug () << "error: scroll_pair_scroll_highlight( " << pos
  ...
  if (scroll_pair_names.size () == 2)
    {
      toplot = { scroll_pair_time_to_names[time][0],
