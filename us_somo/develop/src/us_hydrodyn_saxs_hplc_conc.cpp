...
void US_Hydrodyn_Saxs_Hplc_Conc::refresh(csv &ref_csv) {
  csv new_csv = ref_csv;
  ...
  setGeometry(geometry().left(), geometry().top(), csv_width, 100 + csv_height);
}

US_Hydrodyn_Saxs_Hplc_Conc::~US_Hydrodyn_Saxs_Hplc_Conc() {}

void US_Hydrodyn_Saxs_Hplc_Conc::setupGUI() {
  int minHeight1 = 30;
...
void US_Hydrodyn_Saxs_Hplc_Conc::cancel() { close(); }

void US_Hydrodyn_Saxs_Hplc_Conc::set_ok() {
  *org_csv = current_csv();
  ((US_Hydrodyn_Saxs_Hplc *)saxs_hplc_window)->update_enables();
  close();
}

void US_Hydrodyn_Saxs_Hplc_Conc::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/somo_saxs_hplc_conc.html");
}

void US_Hydrodyn_Saxs_Hplc_Conc::closeEvent(QCloseEvent *e) {
...
void US_Hydrodyn_Saxs_Hplc_Conc::sort_column(int section) {
  t_csv->sortByColumn(
      section, order_ascending ? Qt::AscendingOrder : Qt::DescendingOrder);
  order_ascending = !order_ascending;
}

csv US_Hydrodyn_Saxs_Hplc_Conc::current_csv() {
  csv tmp_csv = csv1;
...
void US_Hydrodyn_Saxs_Hplc_Conc::copy() {
  csv1 = current_csv();
...
void US_Hydrodyn_Saxs_Hplc_Conc::paste() {
  csv1 = current_csv();
  unsigned int pos = 0;
  for (int i = 0; i < t_csv->rowCount(); i++) {
...
void US_Hydrodyn_Saxs_Hplc_Conc::paste_all() {
  csv1 = current_csv();
  unsigned int pos = 0;
  for (int i = 0; i < t_csv->rowCount(); i++) {
    for (unsigned int j = 1;
...
void US_Hydrodyn_Saxs_Hplc_Conc::update_enables() {
  if (!disable_updates) {
...
void US_Hydrodyn_Saxs_Hplc_Conc::row_header_released(int row) {
...
void US_Hydrodyn_Saxs_Hplc_Conc::reload_csv() {
  t_csv->setRowCount(csv1.data.size());
  for (unsigned int i = 0; i < csv1.data.size(); i++) {
    for (unsigned int j = 0; j < csv1.data[i].size(); j++) {
      t_csv->setItem(i, j, new QTableWidgetItem(csv1.data[i][j]));
...
void US_Hydrodyn_Saxs_Hplc_Conc::load() {
  ...
  if (QFileInfo(fname).suffix().contains(QRegExp("^(sbc|SBC)$"))) {
    ...
    if (hbscl->disp_csv.data.size()) {
      ...
      QMessageBox::warning(this,
                           us_tr("US-SOMO: SAXS Hplc: File Concentrations"),
                           us_tr("The file does not seem to contain data the "
...
void US_Hydrodyn_Saxs_Hplc_Conc::save() {
  QString use_dir = QDir::currentPath();
...
QString US_Hydrodyn_Saxs_Hplc_Conc::csv_to_qstring(csv from_csv) {
  QString qs;
...
QStringList US_Hydrodyn_Saxs_Hplc_Conc::csv_parse_line(QString qs) {
