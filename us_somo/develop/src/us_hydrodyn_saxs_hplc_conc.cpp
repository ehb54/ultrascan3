...
#include "../include/us_hydrodyn_saxs_hplc_conc_load.h"
...
US_Hydrodyn_Saxs_Hplc_Conc::US_Hydrodyn_Saxs_Hplc_Conc (csv &csv1,
    ...
    : QFrame (p)
{
  org_csv = &csv1;
  this->csv1 = csv1;
  this->saxs_hplc_window = saxs_hplc_window;
  ((US_Hydrodyn_Saxs_Hplc *)saxs_hplc_window)->conc_widget = true;
  us_hydrodyn = ((US_Hydrodyn_Saxs_Hplc *)saxs_hplc_window)->us_hydrodyn;
  USglobal = new US_Config ();
  setPalette (PALET_FRAME);
  setWindowTitle (us_tr ("US-SOMO: SAXS Hplc: File Concentrations"));
...
void
US_Hydrodyn_Saxs_Hplc_Conc::refresh (csv &ref_csv)
{
  csv new_csv = ref_csv;
  ...
  setGeometry (geometry ().left (), geometry ().top (), csv_width,
               100 + csv_height);
}

US_Hydrodyn_Saxs_Hplc_Conc::~US_Hydrodyn_Saxs_Hplc_Conc () {}

void
US_Hydrodyn_Saxs_Hplc_Conc::setupGUI ()
{
  int minHeight1 = 30;
  ...
  };
  {
    for (int i = 0; i < t_csv->rowCount (); ++i)
      {
        t_csv->item (i, 1)->setFlags (t_csv->item (i, 1)->flags ()
                                      | Qt::ItemIsEditable);
      }
  };
  ...
  connect (t_csv->verticalHeader (), SIGNAL (released (int)),
  ...
  pb_set_ok->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize + 1));
...
void
US_Hydrodyn_Saxs_Hplc_Conc::cancel ()
{
  close ();
}

void
US_Hydrodyn_Saxs_Hplc_Conc::set_ok ()
{
  *org_csv = current_csv ();
  ((US_Hydrodyn_Saxs_Hplc *)saxs_hplc_window)->update_enables ();
  close ();
}

void
US_Hydrodyn_Saxs_Hplc_Conc::help ()
{
  US_Help *online_help;
  online_help = new US_Help (this);
  online_help->show_help ("manual/somo/somo_saxs_hplc_conc.html");
}

void
US_Hydrodyn_Saxs_Hplc_Conc::closeEvent (QCloseEvent *e)
{
  ((US_Hydrodyn_Saxs_Hplc *)saxs_hplc_window)->conc_widget = false;
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept ();
}
...
void
US_Hydrodyn_Saxs_Hplc_Conc::sort_column (int section)
{
  t_csv->sortByColumn (section, order_ascending ? Qt::AscendingOrder
                                                : Qt::DescendingOrder);
  order_ascending = !order_ascending;
}

csv
US_Hydrodyn_Saxs_Hplc_Conc::current_csv ()
{
  csv tmp_csv = csv1;
...
void
US_Hydrodyn_Saxs_Hplc_Conc::copy ()
{
  csv1 = current_csv ();
...
void
US_Hydrodyn_Saxs_Hplc_Conc::paste ()
{
  csv1 = current_csv ();
  unsigned int pos = 0;
  for (int i = 0; i < t_csv->rowCount (); i++)
...
void
US_Hydrodyn_Saxs_Hplc_Conc::paste_all ()
{
  csv1 = current_csv ();
  unsigned int pos = 0;
  for (int i = 0; i < t_csv->rowCount (); i++)
...
void
US_Hydrodyn_Saxs_Hplc_Conc::update_enables ()
{
  if (!disable_updates)
    {
...
void
US_Hydrodyn_Saxs_Hplc_Conc::row_header_released (int row)
{
...
void
US_Hydrodyn_Saxs_Hplc_Conc::reload_csv ()
{
  t_csv->setRowCount (csv1.data.size ());
  for (unsigned int i = 0; i < csv1.data.size (); i++)
    {
...
void
US_Hydrodyn_Saxs_Hplc_Conc::load ()
{
  ...
  QString fname
      = QFileDialog::getOpenFileName (this, windowTitle (), use_dir,
  ...
  if (!QFile::exists (fname))
    {
      QMessageBox::warning (
          this,
          us_tr ("US-SOMO: SAXS Hplc Subtraction Utility: Concentrations"),
  ...
  if (QFileInfo (fname).suffix ().contains (QRegExp ("^(sbc|SBC)$")))
    ...
    {
      US_Hydrodyn_Saxs_Hplc_Conc_Load *hbscl
          = new US_Hydrodyn_Saxs_Hplc_Conc_Load (qsl_lines, csv1, this);
      ...
      if (hbscl->disp_csv.data.size ())
        ...
        {
          QMessageBox::warning (
              this, us_tr ("US-SOMO: SAXS Hplc: File Concentrations"),
              us_tr ("The file does not seem to contain data the this program "
                     "understands"));
          delete hbscl;
          return;
        }
...
void
US_Hydrodyn_Saxs_Hplc_Conc::save ()
{
  QString use_dir = QDir::currentPath ();
  ...
  QString fname = QFileDialog::getSaveFileName (
      this, us_tr ("Choose a filename to save the concentrations"), use_dir,
      "*.sbc *.SBC");
  ...
  QMessageBox::information (
      this, us_tr ("US-SOMO: SAXS Hplc Subtraction Utility: Concentrations"),
      QString (us_tr ("File %1 saved")).arg (fname));
}

QString
US_Hydrodyn_Saxs_Hplc_Conc::csv_to_qstring (csv from_csv)
{
  QString qs;
...
QStringList
US_Hydrodyn_Saxs_Hplc_Conc::csv_parse_line (QString qs)
{
