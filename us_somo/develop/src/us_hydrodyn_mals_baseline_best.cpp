...
void US_Hydrodyn_Mals_Baseline_Best::setupGUI() {
  int minHeight1 = 30;
  ...
  hb_plot->setTitleFont(QFont(USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize + 3, QFont::Bold));
  hb_plot->setAxisTitleFont(QwtPlot::yLeft,
                            QFont(USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize, QFont::Bold));
#endif
  hb_plot->setAxisFont(QwtPlot::yLeft,
                       QFont(USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize - 1));
#if QT_VERSION < 0x040000
  hb_plot->setAxisTitleFont(QwtPlot::xBottom,
                            QFont(USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize, QFont::Bold));
#endif
  hb_plot->setAxisFont(QwtPlot::xBottom,
                       QFont(USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize - 1));
#if QT_VERSION < 0x040000
  hb_plot->setAxisTitleFont(QwtPlot::yRight,
                            QFont(USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize, QFont::Bold));
#endif
...
void US_Hydrodyn_Mals_Baseline_Best::cancel() { close(); }

void US_Hydrodyn_Mals_Baseline_Best::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/baseline_best.html");
}

void US_Hydrodyn_Mals_Baseline_Best::closeEvent(QCloseEvent *e) {
  if (((US_Hydrodyn *)us_hydrodyn)->mals_widget) {
    US_Hydrodyn_Mals *mals_win = ((US_Hydrodyn *)us_hydrodyn)->mals_window;
...
void US_Hydrodyn_Mals_Baseline_Best::editor_hb_msg(QString color, QString msg) {
  return editor_msg(color, msg, hb_editor);
}

void US_Hydrodyn_Mals_Baseline_Best::editor_hb_ec_msg(QString msg) {
  return editor_ec_msg(msg, hb_editor);
}

void US_Hydrodyn_Mals_Baseline_Best::editor_msg(QString color, QString msg,
                                                QTextEdit *e) {
  if (!e) {
    e = editor;
  }
...
void US_Hydrodyn_Mals_Baseline_Best::editor_ec_msg(QString msg, QTextEdit *e) {
  if (!e) {
    e = editor;
  }
  QColor save_color = e->textColor();
...
void US_Hydrodyn_Mals_Baseline_Best::clear_display() {
  editor->clear();
  editor->append("\n\n");
  hb_editor->clear();
  hb_editor->append("\n\n");
...
void US_Hydrodyn_Mals_Baseline_Best::update_font() {
  bool ok;
  QFont newFont;
...
void US_Hydrodyn_Mals_Baseline_Best::save() {
  QString fn;
...
void US_Hydrodyn_Mals_Baseline_Best::displayData() {
  editor->clear();
  ...
  plot->setAxisTitle(QwtPlot::yLeft, us_tr(parameters.count("hb")
                                               ? "Red pair %"
                                               : "Average Red Cluster Size"));

  hb_plot->setAxisTitle(QwtPlot::yLeft, us_tr("Red pair %"));

  lbl_title->setText(parameters.count("title")
                         ? parameters["title"]
                         : QString(us_tr("Baseline avg. I(q) and average red "
                                         "cluster size by start frame")));
  lbl_hb_title->setText(
      parameters.count("hb_title")
          ? parameters["hb_title"]
          : QString(us_tr("Baseline avg. I(q) and red pair % by start frame")));
  ...
  if (vdparameters.count("x") && vdparameters.count("y") &&
    ...
    if (vdparameters.count("sumqx")) {
...
#if QT_VERSION < 0x040000
          long curve;
          curve = plot->insertCurve("baseline_best_sumq", QwtPlot::xBottom,
                                    QwtPlot::yRight);
      ...
      if (vdparameters.count("sumqmaxqy")) {
        ...
        QString curvename =
            QString(
                us_tr("Baseline avg. I(q) for q less than %1\n(on right axis)"))
...
#if QT_VERSION < 0x040000
        plot->setCurvePen(curve, QPen(QColor(255, 165, 0) /* orange */,
                                      2 * use_line_width, Qt::SolidLine));
        plot->setCurveData(curve, (double *)&vdparameters["sumqx"][0],
                           (double *)&vdparameters["sumqmaxqy"][0],
                           vdparameters["sumqx"].size());
#else
        curve->setPen(QPen(QColor(255, 165, 0) /* orange */, 2 * use_line_width,
                           Qt::SolidLine));
        curve->setSamples((double *)&vdparameters["sumqx"][0],
                          (double *)&vdparameters["sumqmaxqy"][0],
                          vdparameters["sumqx"].size());
        ...
        if (vdparameters.count("sumqmaxqysdm") &&
...

          {
            ...
            QString curvename = QString(us_tr("Baseline avg. I(q) ±1 SD for q "
                                              "less than %1\n(on right axis)"))
                                    .arg(dparameters["cormap_maxq"]);
#else
            QString curvename =
                QString(us_tr("Baseline avg. I(q) \u00b11 SD for q less than "
                              "%1\n(on right axis)"))
                    .arg(dparameters["cormap_maxq"]);
#endif
#if QT_VERSION < 0x040000
            long curve;
            curve =
                plot->insertCurve(curvename, QwtPlot::xBottom, QwtPlot::yRight);
            plot->setCurveStyle(curve, QwtCurve::Lines);
#else
            QwtPlotCurve *curve = new QwtPlotCurve(curvename);
            curve->setStyle(QwtPlotCurve::Lines);
...
#if QT_VERSION < 0x050000
          QString curvename =
              "Average ±1 SD of total blanks intensity\n(on right axis)";
#else
          QString curvename =
              "Average \u00b11 SD of total blanks intensity\n(on right axis)";
#endif
...
#if QT_VERSION < 0x040000
      plot->setCurvePen(curve,
                        QPen(Qt::yellow, 2 * use_line_width, Qt::SolidLine));
...
#if QT_VERSION < 0x040000
      plot->setCurvePen(curve,
                        QPen(Qt::cyan, 2 * use_line_width, Qt::SolidLine));
...
#if QT_VERSION < 0x040000
      plot->setCurvePen(curve,
                        QPen(Qt::green, 2 * use_line_width, Qt::SolidLine));
      plot->setCurveData(curve, x, y, 2);
#else
    ...
    if (!plot_zoomer) {
      ...
      if (plotted_yright) {
        // plot->setAxisScale( QwtPlot::yRight, minyr - ( fabs( minyr * .1 ) ),
        // maxyr * 1.1 );
  ...
  if (!parameters.count("hb")) {
    if (vdparameters.count("hb_x") && vdparameters.count("hb_y") &&
      ...
      if (vdparameters.count("sumqx")) {
        ...
        if (vdparameters.count("sumqy")) {
...

          {
#if QT_VERSION < 0x040000
            long curve;
            curve = hb_plot->insertCurve("baseline_best_sumq", QwtPlot::xBottom,
                                         QwtPlot::yRight);
            ...
            hb_plot->setCurveData(curve, (double *)&vdparameters["sumqx"][0],
                                  (double *)&vdparameters["sumqy"][0],
                                  vdparameters["sumqx"].size());
#else
            curve->setPen(QPen(Qt::white, 2 * use_line_width, Qt::SolidLine));
        ...
        if (vdparameters.count("sumqmaxqy")) {
          ...
          QString curvename =
              QString(
                  us_tr(
                      "Baseline avg. I(q) for q less than %1\n(on right axis)"))
                  .arg(dparameters["cormap_maxq"]);
#if QT_VERSION < 0x040000
          long curve;
...
#if QT_VERSION < 0x040000
          hb_plot->setCurvePen(curve, QPen(QColor(255, 165, 0) /* orange */,
                                           2 * use_line_width, Qt::SolidLine));
          hb_plot->setCurveData(curve, (double *)&vdparameters["sumqx"][0],
                                (double *)&vdparameters["sumqmaxqy"][0],
                                vdparameters["sumqx"].size());
      ...
      if (vdparameters.count("hb_wx")) {
        ...
        hb_plot->setCurvePen(
            curve, QPen(Qt::white, 2 * use_line_width, Qt::SolidLine));
        hb_plot->setCurveData(curve, (double *)&vdparameters["hb_wx"][0],
      ...
      if (!hb_plot_zoomer) {
        ...
        if (plotted_yright) {
          // hb_plot->setAxisScale( QwtPlot::yRight, minyr - ( fabs( minyr * .1
          // ) ), maxyr * 1.1 );
          hb_plot->setAxisScale(QwtPlot::yRight, -3e0 * (maxyr - minyr),
                                maxyr * 1.1);
        ...
        // connect( hb_plot_zoomer, SIGNAL( zoomed( const QRectF & ) ), SLOT(
        // hb_plot_zoomed( const QRectF & ) ) );
      }
    }
  }
  ...
  if (parameters.count("global_width")) {
    ...
    // ); plot->setMinimumWidth( ( parameters[ "global_width" ].toInt() - 20 ) /
    // 4 );
  }
...
void US_Hydrodyn_Mals_Baseline_Best::set_best() {
  if (((US_Hydrodyn *)us_hydrodyn)->mals_widget) {
    US_Hydrodyn_Mals *mals_win = ((US_Hydrodyn *)us_hydrodyn)->mals_window;
      ...
      if (vdparameters["bestpos"].size() > 1) {
        ...
        }
        {
          ...
          QString choice = US_Static::getItem(
              ...
              us_tr("There are multiple equivalent starting positions for the "
                    "integral baseline window\n"
...
void US_Hydrodyn_Mals_Baseline_Best::set_hb_best() {
  if (((US_Hydrodyn *)us_hydrodyn)->mals_widget) {
    US_Hydrodyn_Mals *mals_win = ((US_Hydrodyn *)us_hydrodyn)->mals_window;
...
void US_Hydrodyn_Mals_Baseline_Best::update_enables() {
  if (!parameters.count("hb")) {
...
void US_Hydrodyn_Mals_Baseline_Best::usp_config_plot(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Baseline_Best::usp_config_hb_plot(const QPoint &) {
