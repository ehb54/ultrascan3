...
US_Hydrodyn_Mals_Baseline_Best::US_Hydrodyn_Mals_Baseline_Best (
    void *us_hydrodyn, map<QString, QString> parameters,
    map<QString, double> dparameters,
    map<QString, vector<double> > vdparameters, QWidget *p, const char *)
    : QFrame (p)
{
...
void
US_Hydrodyn_Mals_Baseline_Best::setupGUI ()
{
  int minHeight1 = 30;
  ...
  lbl_hb_title->setFont (QFont (USglobal->config_list.fontFamily,
...
#endif
  plot->setAxisTitle (QwtPlot::xBottom, parameters.count ("xlegend")
                                            ? parameters["xlegend"]
                                            : us_tr ("Start Frame"));
  ...
  plot->setLegendFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize - 1));
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->mals_widget)
    {
      US_Hydrodyn_Mals *mals_win = ((US_Hydrodyn *)us_hydrodyn)->mals_window;
      mals_win->plot_info[plot_name] = plot;
    }

  //   hb_plot = new QwtPlot( qs_right );
  usp_hb_plot = new US_Plot (hb_plot, "", "", "", qs_right);
  connect ((QWidget *)hb_plot->titleLabel (),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_hb_plot (const QPoint &)));
  ((QWidget *)hb_plot->titleLabel ())
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)hb_plot->axisWidget (QwtPlot::yLeft),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_hb_plot (const QPoint &)));
  ((QWidget *)hb_plot->axisWidget (QwtPlot::yLeft))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)hb_plot->axisWidget (QwtPlot::xBottom),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_hb_plot (const QPoint &)));
  ((QWidget *)hb_plot->axisWidget (QwtPlot::xBottom))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
#if QT_VERSION < 0x040000
  ...
  hb_plot->setAxisTitleFont (
      QwtPlot::xBottom, QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize, QFont::Bold));
#endif
  hb_plot->setAxisFont (QwtPlot::xBottom,
                        QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
#if QT_VERSION < 0x040000
  hb_plot->setAxisTitleFont (
      QwtPlot::yRight, QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize, QFont::Bold));
#endif
  hb_plot->setAxisFont (QwtPlot::yRight,
                        QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  ...
  hb_plot->setLegendFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  ...
  hb_plot_name
      = windowTitle ().replace ("US-SOMO:", "").replace (" ", "_") + "_hb";
  if (((US_Hydrodyn *)us_hydrodyn)->mals_widget)
    {
      US_Hydrodyn_Mals *mals_win = ((US_Hydrodyn *)us_hydrodyn)->mals_window;
      mals_win->plot_info[hb_plot_name] = hb_plot;
    }
  ...
  pb_set_best->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize + 1));
  ...
  pb_set_hb_best->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize + 1));
  ...
  cb_show_hb->setFont (QFont (USglobal->config_list.fontFamily,
...

  {
    ...
    hb->setSpacing (0);
    {
      ...
      QVBoxLayout *vbl_left = new QVBoxLayout (0);
      vbl_left->setContentsMargins (0, 0, 0, 0);
      vbl_left->setSpacing (0);
    ...
    if (0)
      {
        QVBoxLayout *vbl_right = new QVBoxLayout (0);
        vbl_right->setContentsMargins (0, 0, 0, 0);
        vbl_right->setSpacing (0);
...
void
US_Hydrodyn_Mals_Baseline_Best::cancel ()
{
  close ();
}

void
US_Hydrodyn_Mals_Baseline_Best::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Mals_Baseline_Best::closeEvent (QCloseEvent *e)
{
  if (((US_Hydrodyn *)us_hydrodyn)->mals_widget)
    {
      US_Hydrodyn_Mals *mals_win = ((US_Hydrodyn *)us_hydrodyn)->mals_window;
      if (mals_win->plot_info.count (plot_name)
          && mals_win->plot_info[plot_name] == plot)
        {
          mals_win->plot_info.erase (plot_name);
        }
      if (mals_win->plot_info.count (hb_plot_name)
          && mals_win->plot_info[hb_plot_name] == hb_plot)
        {
          mals_win->plot_info.erase (hb_plot_name);
        }
    }
...
void
US_Hydrodyn_Mals_Baseline_Best::editor_hb_msg (QString color, QString msg)
{
  return editor_msg (color, msg, hb_editor);
}

void
US_Hydrodyn_Mals_Baseline_Best::editor_hb_ec_msg (QString msg)
{
  return editor_ec_msg (msg, hb_editor);
}

void
US_Hydrodyn_Mals_Baseline_Best::editor_msg (QString color, QString msg,
                                            QTextEdit *e)
{
  if (!e)
    {
...
void
US_Hydrodyn_Mals_Baseline_Best::editor_ec_msg (QString msg, QTextEdit *e)
{
  if (!e)
    {
      e = editor;
...
void
US_Hydrodyn_Mals_Baseline_Best::clear_display ()
{
  editor->clear ();
  editor->append ("\n\n");
  hb_editor->clear ();
...
void
US_Hydrodyn_Mals_Baseline_Best::update_font ()
{
  bool ok;
  QFont newFont;
...
void
US_Hydrodyn_Mals_Baseline_Best::save ()
{
  QString fn;
...
void
US_Hydrodyn_Mals_Baseline_Best::displayData ()
{
  ...
  ;
  hb_plot->detachItems (QwtPlotItem::Rtti_PlotCurve);
  hb_plot->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
  hb_editor->clear ();

  plot->setAxisTitle (QwtPlot::yLeft,
                      us_tr (parameters.count ("hb")
                                 ? "Red pair %"
  ...
  lbl_title->setText (
      parameters.count ("title")
          ? parameters["title"]
          : QString (us_tr ("Baseline avg. I(q) and average red cluster size "
                            "by start frame")));
  lbl_hb_title->setText (
      parameters.count ("hb_title")
          ? parameters["hb_title"]
          : QString (
                us_tr ("Baseline avg. I(q) and red pair % by start frame")));
  ...
  if (vdparameters.count ("x") && vdparameters.count ("y")
      && vdparameters["x"].size ()
      && vdparameters["x"].size () == vdparameters["y"].size ())
    {
      ...
      // US_Vector::printvector2( "baseline results", vdparameters[ "x" ],
      // vdparameters[ "y" ] );
...
#if QT_VERSION < 0x040000
                long curve;
                curve = plot->insertCurve ("baseline_best_sumq",
...
#if QT_VERSION < 0x040000
                plot->setCurvePen (curve, QPen (Qt::white, 2 * use_line_width,
                                                Qt::SolidLine));
                plot->setCurveData (curve, (double *)&vdparameters["sumqx"][0],
                                    (double *)&vdparameters["sumqy"][0],
                                    vdparameters["sumqx"].size ());
#else
                curve->setPen (
                    QPen (Qt::white, 2 * use_line_width, Qt::SolidLine));
                curve->setSamples ((double *)&vdparameters["sumqx"][0],
                                   (double *)&vdparameters["sumqy"][0],
                                   vdparameters["sumqx"].size ());
          ...
          if (vdparameters.count ("sumqmaxqy"))
            {
              ...
              QString curvename
                  = QString (us_tr ("Baseline avg. I(q) for q less than "
                                    "%1\n(on right axis)"))
                        .arg (dparameters["cormap_maxq"]);
#if QT_VERSION < 0x040000
              long curve;
              curve = plot->insertCurve (curvename, QwtPlot::xBottom,
                                         QwtPlot::yRight);
              plot->setCurveStyle (curve, QwtCurve::Lines);
#else
              ...
              plot->setCurvePen (curve,
                                 QPen (QColor (255, 165, 0) /* orange */,
                                       2 * use_line_width, Qt::SolidLine));
              plot->setCurveData (curve, (double *)&vdparameters["sumqx"][0],
                                  (double *)&vdparameters["sumqmaxqy"][0],
                                  vdparameters["sumqx"].size ());
#else
              curve->setPen (QPen (QColor (255, 165, 0) /* orange */,
                                   2 * use_line_width, Qt::SolidLine));
              curve->setSamples ((double *)&vdparameters["sumqx"][0],
                                 (double *)&vdparameters["sumqmaxqy"][0],
                                 vdparameters["sumqx"].size ());
              ...
              if (vdparameters.count ("sumqmaxqysdm")
                  && vdparameters.count ("sumqmaxqysdp"))
                {
...

                  {
                    ...
                    QString curvename
                        = QString (us_tr ("Baseline avg. I(q) ±1 SD for q "
                                          "less than %1\n(on right axis)"))
                              .arg (dparameters["cormap_maxq"]);
#else
                    QString curvename
                        = QString (us_tr ("Baseline avg. I(q) \u00b11 SD for "
                                          "q less than %1\n(on right axis)"))
                              .arg (dparameters["cormap_maxq"]);
#endif
#if QT_VERSION < 0x040000
                    ...
                    plot->setCurvePen (curve,
                                       QPen (QColor (255, 165, 0) /* orange */,
                                             2 * use_line_width, Qt::DotLine));
                    plot->setCurveData (
                        curve, (double *)&vdparameters["sumqx"][0],
...
#if QT_VERSION < 0x040000
            plot->setCurvePen (
                curve, QPen (Qt::magenta, 2 * use_line_width, Qt::SolidLine));
            plot->setCurveData (curve, x, y, 2);
#else
            curve->setPen (
                QPen (Qt::magenta, 2 * use_line_width, Qt::SolidLine));
          ...
          if (false && dparameters.count ("blanks_avg_maxq_sd"))
            {
              ...
              {
#if QT_VERSION < 0x050000
                QString curvename = "Average ±1 SD of total blanks "
                                    "intensity\n(on right axis)";
#else
                QString curvename = "Average \u00b11 SD of total blanks "
                                    "intensity\n(on right axis)";
                ...
                plot->setCurvePen (
                    curve,
                    QPen (Qt::magenta, 2 * use_line_width, Qt::DotLine));
                plot->setCurveData (curve, x, y, 2);
#else
                curve->setPen (
                    QPen (Qt::magenta, 2 * use_line_width, Qt::DotLine));
...
#if QT_VERSION < 0x040000
          plot->setCurvePen (
              curve, QPen (Qt::yellow, 2 * use_line_width, Qt::SolidLine));
          plot->setCurveData (curve, (double *)&vdparameters["yx"][0],
                              (double *)&vdparameters["yy"][0],
                              vdparameters["yx"].size ());
          plot->setCurveBaseline (curve, -0.5);
#else
          curve->setPen (QPen (Qt::yellow, 2 * use_line_width, Qt::SolidLine));
          curve->setSamples ((double *)&vdparameters["yx"][0],
                             (double *)&vdparameters["yy"][0],
                             vdparameters["yx"].size ()
...
#if QT_VERSION < 0x040000
          plot->setCurvePen (
              curve, QPen (Qt::cyan, 2 * use_line_width, Qt::SolidLine));
          plot->setCurveData (curve, (double *)&vdparameters["gx"][0],
                              (double *)&vdparameters["gy"][0],
                              vdparameters["gx"].size ());
          plot->setCurveBaseline (curve, -0.5);
#else
          curve->setPen (QPen (Qt::cyan, 2 * use_line_width, Qt::SolidLine));
          curve->setSamples ((double *)&vdparameters["gx"][0],
                             (double *)&vdparameters["gy"][0],
                             vdparameters["gx"].size ()
...
#if QT_VERSION < 0x040000
          plot->setCurvePen (
              curve, QPen (Qt::white, 2 * use_line_width, Qt::SolidLine));
          plot->setCurveData (curve, (double *)&vdparameters["wx"][0],
                              (double *)&vdparameters["wy"][0],
                              vdparameters["wx"].size ());
          plot->setCurveBaseline (curve, -0.5);
#else
          curve->setPen (QPen (Qt::white, 2 * use_line_width, Qt::SolidLine));
          curve->setSamples ((double *)&vdparameters["wx"][0],
                             (double *)&vdparameters["wy"][0],
                             vdparameters["wx"].size ()
...
#if QT_VERSION < 0x040000
          plot->setCurvePen (
              curve, QPen (Qt::green, 2 * use_line_width, Qt::SolidLine));
          plot->setCurveData (curve, x, y, 2);
#else
          ...
          if (dparameters.count ("blanksaveragesd"))
            {
              ...
              QString curvename
                  = us_tr ("Blanks' avg. red cluster size +1 SD");
#if QT_VERSION < 0x040000
              long curve;
              curve = plot->insertCurve (curvename);
              plot->setCurveStyle (curve, QwtCurve::Lines);
...
#if QT_VERSION < 0x040000
              plot->setCurvePen (
                  curve, QPen (Qt::green, 2 * use_line_width, Qt::DotLine));
              plot->setCurveData (curve, x, y, 2);
#else
              curve->setPen (
                  QPen (Qt::green, 2 * use_line_width, Qt::DotLine));
              curve->setSamples (x, y, 2);
              curve->attach (plot);
#endif
            }
      ...
      if (!plot_zoomer)
        {
          plot->setAxisScale (QwtPlot::xBottom, vdparameters["x"].front () - 1,
                              vdparameters["x"].back () + 1);
          plot->setAxisScale (QwtPlot::yLeft, -0.25, maxy * 1.1);
          if (plotted_yright)
            {
              // plot->setAxisScale( QwtPlot::yRight, minyr - ( fabs( minyr *
              // .1 ) ), maxyr * 1.1 );
              plot->setAxisScale (QwtPlot::yRight, -3e0 * (maxyr - minyr),
                                  maxyr * 1.1);
  ...
  if (!parameters.count ("hb"))
    {
      if (vdparameters.count ("hb_x") && vdparameters.count ("hb_y")
          ...
          && vdparameters["hb_x"].size () == vdparameters["hb_y"].size ())
        {
          ...
          if (vdparameters.count ("sumqx"))
            {
              ...
              hb_plot->setAxisTitle (QwtPlot::yRight,
                                     us_tr ("\nAverage I(q) [a.u.]"));
...
#if QT_VERSION < 0x040000
                    long curve;
                    curve = hb_plot->insertCurve ("baseline_best_sumq",
                                                  QwtPlot::xBottom,
                                                  QwtPlot::yRight);
                    hb_plot->setCurveStyle (curve, QwtCurve::Lines);
#else
                    QwtPlotCurve *curve
                        = new QwtPlotCurve ("baseline_best_sumq");
                    curve->setStyle (QwtPlotCurve::Lines);
#endif
              ...
              if (vdparameters.count ("sumqmaxqy"))
                {
                  ...
                  for (int i = 0; i < (int)vdparameters["sumqmaxqy"].size ();
                       ++i)
                    {
                      if (maxyr < vdparameters["sumqmaxqy"][i])
                        {
...
#if QT_VERSION < 0x040000
                  long curve;
                  curve = hb_plot->insertCurve (curvename, QwtPlot::xBottom,
                                                QwtPlot::yRight);
                  hb_plot->setCurveStyle (curve, QwtCurve::Lines);
#else
                  QwtPlotCurve *curve = new QwtPlotCurve (curvename);
...
#if QT_VERSION < 0x040000
                  hb_plot->setCurvePen (
                      curve, QPen (QColor (255, 165, 0) /* orange */,
                                   2 * use_line_width, Qt::SolidLine));
                  hb_plot->setCurveData (
                      curve, (double *)&vdparameters["sumqx"][0],
                      (double *)&vdparameters["sumqmaxqy"][0],
                      vdparameters["sumqx"].size ());
#else
                  ...
                  if (vdparameters.count ("sumqmaxqysdm")
                      && vdparameters.count ("sumqmaxqysdp"))
                    {
                      for (int i = 0; i < (int)vdparameters["sumqx"].size ();
                           ++i)
                        {
                          if (maxyr < vdparameters["sumqmaxqysdp"][i])
                            {
...

                      {
                        ...
                        QString curvename
                            = QString (
                                  us_tr ("Baseline avg. I(q) \u00b11 SD for q "
                                         "less than %1\n(on right axis)"))
                                  .arg (dparameters["cormap_maxq"]);
#endif
...
#if QT_VERSION < 0x040000
                        long curve;
                        curve = hb_plot->insertCurve (
                            curvename, QwtPlot::xBottom, QwtPlot::yRight);
                        hb_plot->setCurveStyle (curve, QwtCurve::Lines);
#else
                        QwtPlotCurve *curve = new QwtPlotCurve (curvename);
                        curve->setStyle (QwtPlotCurve::Lines);
...

          {
            ...
            hb_plot->setCurveData (curve, (double *)&vdparameters["hb_x"][0],
                                   (double *)&vdparameters["hb_y"][0],
                                   vdparameters["hb_x"].size ());
            hb_plot->setCurveBaseline (curve, -0.5);
#else
            curve->setPen (QPen (Qt::red, 2 * use_line_width, Qt::SolidLine));
            curve->setSamples ((double *)&vdparameters["hb_x"][0],
                               (double *)&vdparameters["hb_y"][0],
                               vdparameters["hb_x"].size ()
          ...
          if (vdparameters.count ("hb_wx"))
            {
              ...
              curve->setPen (
                  QPen (Qt::white, 2 * use_line_width, Qt::SolidLine));
              curve->setSamples ((double *)&vdparameters["hb_wx"][0],
                                 (double *)&vdparameters["hb_wy"][0],
                                 vdparameters["hb_wx"].size ()
          ...
          if (!hb_plot_zoomer)
            {
              hb_plot->setAxisScale (QwtPlot::xBottom,
                                     vdparameters["hb_x"].front () - 1,
                                     vdparameters["hb_x"].back () + 1);
              hb_plot->setAxisScale (QwtPlot::yLeft, -0.25, maxy * 1.1);
              if (plotted_yright)
                {
                  // hb_plot->setAxisScale( QwtPlot::yRight, minyr - ( fabs(
                  // minyr * .1 ) ), maxyr * 1.1 );
                  hb_plot->setAxisScale (QwtPlot::yRight,
                                         -3e0 * (maxyr - minyr), maxyr * 1.1);
                  hb_plot->setAxisScale (QwtPlot::yLeft, -0.25, 1.7 * maxy);
                }

              hb_plot_zoomer = new ScrollZoomer (hb_plot->canvas ());
              hb_plot_zoomer->setRubberBandPen (
                  QPen (Qt::yellow, 0, Qt::DotLine));
              hb_plot_zoomer->setTrackerPen (QPen (Qt::red));
              // connect( hb_plot_zoomer, SIGNAL( zoomed( const QRectF & ) ),
              // SLOT( hb_plot_zoomed( const QRectF & ) ) );
            }
        }
    }
...
#if QT_VERSION >= 0x040000
  {
    ...
    // this doesn't seem to work: mp.setColor( QPalette::Normal,
  ...
  if (!vdparameters.count ("bestpos") || !dparameters.count ("width"))
    {
  ...
  if (parameters.count ("global_width"))
    {
      ...
      // 4 ); plot->setMinimumWidth( ( parameters[ "global_width" ].toInt() -
      // 20 ) / 4 );
    }
...
void
US_Hydrodyn_Mals_Baseline_Best::set_best ()
{
  if (((US_Hydrodyn *)us_hydrodyn)->mals_widget)
    {
      US_Hydrodyn_Mals *mals_win = ((US_Hydrodyn *)us_hydrodyn)->mals_window;

      if (mals_win->current_mode == US_Hydrodyn_Mals::MODE_BASELINE
          && vdparameters.count ("bestpos") && dparameters.count ("width"))
        {
          double bestpos = vdparameters["bestpos"].front ();
          if (vdparameters["bestpos"].size () > 1)
            {
              QStringList qsl;
              for (int i = (int)vdparameters["bestpos"].size () - 1; i >= 0;
                   --i)
                {
                ...
                }
              {
                ...
                QString choice = US_Static::getItem (
                    windowTitle ()
                        + us_tr (" : Choose starting position for baseline"),
                    us_tr ("There are multiple equivalent starting positions "
                           "for the integral baseline window\n"
          ...
          mals_win->le_baseline_end_s->setText (QString ("%1").arg (bestpos));
          mals_win->le_baseline_end_e->setText (
              QString ("%1").arg (bestpos + dparameters["width"] - 1));
          mals_win->le_baseline_width->setText (
              QString ("%1").arg (dparameters["width"]));
          mals_win->cb_baseline_fix_width->setChecked (true);
        }
    }
}

void
US_Hydrodyn_Mals_Baseline_Best::set_hb_best ()
{
  if (((US_Hydrodyn *)us_hydrodyn)->mals_widget)
    {
      US_Hydrodyn_Mals *mals_win = ((US_Hydrodyn *)us_hydrodyn)->mals_window;
      ...
      if (mals_win->current_mode == US_Hydrodyn_Mals::MODE_BASELINE
          && vdparameters.count ("hb_bestpos") && dparameters.count ("width"))
        {
          double bestpos = vdparameters["hb_bestpos"].front ();
          if (vdparameters["hb_bestpos"].size () > 1)
            {
              QStringList qsl;
              for (int i = (int)vdparameters["hb_bestpos"].size () - 1; i >= 0;
                   --i)
                {
...
void
US_Hydrodyn_Mals_Baseline_Best::update_enables ()
{
  if (!parameters.count ("hb"))
    {
...
void
US_Hydrodyn_Mals_Baseline_Best::usp_config_plot (const QPoint &)
{
...
void
US_Hydrodyn_Mals_Baseline_Best::usp_config_hb_plot (const QPoint &)
{
