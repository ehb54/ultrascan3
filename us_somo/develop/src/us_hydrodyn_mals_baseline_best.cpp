...
void US_Hydrodyn_Mals_Baseline_Best::cancel()
{
   close();
}
...
void US_Hydrodyn_Mals_Baseline_Best::editor_msg( QString color, QString msg, QTextEdit *e )
{
   if ( !e ) {
      e = editor;
   }
...
void US_Hydrodyn_Mals_Baseline_Best::set_best() {
   if ( ((US_Hydrodyn *)us_hydrodyn)->mals_widget ) {
      ...
      if ( mals_win->current_mode == US_Hydrodyn_Mals::MODE_BASELINE &&
         ...
         mals_win->le_baseline_end_e->setText( QString( "%1" ).arg( bestpos + dparameters[ "width" ] - 1 ) );
         mals_win->le_baseline_width->setText( QString( "%1" ).arg( dparameters[ "width" ] ) );
