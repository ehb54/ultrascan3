...
bool
US_Hydrodyn_Mals::unified_ggaussian_to_f_gaussians ()
{
  ...
  /* now handled in create_unified_ggaussian_target
     ...
     vector < bool >         is_common; // is common maps the offsets to layout
     of the regular file specific gaussians vector < unsigned int > offset;
  ...
  */
  ...
  // US_Vector::printvector( "unified_ggaussian_to_f_gaussians is_common",
  // is_common ); US_Vector::printvector( "unified_ggaussian_to_f_gaussians
  // offset"   , offset    );
  ...
  if (common_size + per_file_size != (unsigned int)gaussian_type_size)
    {
      errormsg
          = QString (
                us_tr ("Internal error: unified_ggaussians_to_f_gaussians(): "
                       "common_size %1 + per_file_size %2 != "
          ...
          for (int k = 0; k < gaussian_type_size; ++k)
            {
              if (is_common[k])
                {
                  g.push_back (
                      unified_ggaussian_params[offset[k] + common_size * j]);
                }
              else
                {
                  g.push_back (
                      unified_ggaussian_params[offset[k] + per_file_size * j
                                               + index]);
                }
            }
        }
...
bool
US_Hydrodyn_Mals::gg_fit_vector (vector<double> &fit, vector<double> &pfit)
{
  if (!unified_ggaussian_ok)
    {
      editor_msg ("red", us_tr ("Internal error: gg_chi_fit_vector() called "
                                "but unified gaussians not ok"));
      return false;
    }
  ...
  for (int i = 0; i < (int)unified_ggaussian_curves; ++i)
    {
      ...
      // compute cormap p value
      {
        ...
        // us_qdebug( QString( "p fit for %1 = %2" ).arg(
        // unified_ggaussian_files[ i ] ).arg( pfit.back() ) );
      }

      double rmsd = 0e0;
      double tmp;

      // us_qdebug( US_Vector::qs_vector3( "this_t, G, I", this_t, G, f_Is[
      // unified_ggaussian_files[ i ] ] ) );
      ...
      if (use_errors)
        {
          ...
          // us_qdebug( QString( "curve %1 nu %2 rmsd %3" ).arg(
          // unified_ggaussian_files[ i ] ).arg( nu ).arg( rmsd / nu ) );
          rmsd /= nu;
        }
      else
        {
...

  {
    ...
    if (cormap_tot)
      {
        double tot_c_pct
            = 100e0 / (double)(cormap_green + cormap_yellow + cormap_red);
        QString msg = QString ("").sprintf (
            "P value analysis summary:\n"
            " %5.1f%% P >= %.2f (%.1f%% P >= %.2f) + (%.1f%% %.2f > P >= "
            "%.2f) pairs\n"
            " %5.1f%% %.2f < P pairs\n",
            tot_c_pct * (double)(cormap_green + cormap_yellow), alpha_over_5,
            tot_c_pct * (double)cormap_green, alpha,
            tot_c_pct * (double)cormap_yellow, alpha, alpha_over_5,
            tot_c_pct * (double)cormap_red, alpha_over_5);
  ...
  if (cb_ggq_plot_chi2->isChecked ())
    {
      ...
      ggqfit_plot->setAxisTitle (
          QwtPlot::yLeft, us_tr (use_errors ? "normalized Chi^2" : "RMSD"));

      {
        ...
        curve->setSamples ((double *)&(unified_ggaussian_qvals[0]),
                           (double *)&(fit[0]),
        ...
        ggqfit_plot->setCurveData (
            curve, (double *)&(unified_ggaussian_qvals[0]),
            (double *)&(fit[0]), unified_ggaussian_qvals.size ());
      ...
      // as markers
      {
        ...
        for (int i = 0; i < (int)unified_ggaussian_files.size (); ++i)
          {
            QColor qc = plot_colors[f_pos[unified_ggaussian_files[i]]
                                    % plot_colors.size ()];
            sym.setPen (qc);
            sym.setBrush (qc);
            ...
            ggqfit_plot->setMarkerPos (marker, unified_ggaussian_qvals[i],
                                       fit[i]);
#endif
  ...
  if (cb_ggq_plot_P->isChecked ())
    {
      ...
      ggqfit_plot->setAxisTitle (QwtPlot::yRight,
                                 us_tr ("P value (log scale)"));
      ...
      //       curve->attach( ggqfit_plot  );
      // #else
      //       long curve = ggqfit_plot->insertCurve( "ggqfit_p_values",
      //       QwtPlot::xBottom, QwtPlot::yRight ); ggqfit_plot->setCurveStyle(
      //       curve, QwtCurve::Lines ); ggqfit_plot->setCurveData( curve,
      //                                  (double
      //                                  *)&(unified_ggaussian_qvals[0]),
      //                                  (double *)&(pfit[0]),
      //                                  unified_ggaussian_qvals.size() );
      //       ggqfit_plot->setCurvePen( curve, use_pen );
      // #endif
...

      {
...

        {
          ...
          curve->attach (ggqfit_plot);
#else
          long curve = ggqfit_plot->insertCurve (
              "ggqfit_p_alpha_green", QwtPlot::xBottom, QwtPlot::yRight);
          ggqfit_plot->setCurveStyle (curve, QwtCurve::Lines);
          ggqfit_plot->setCurveData (curve, (double *)&(x[0]),
                                     (double *)&(y[0]), 2);
          ggqfit_plot->setCurvePen (curve, use_pen);
#endif
        }
        ...
        for (int i = 0; i < (int)unified_ggaussian_files.size (); ++i)
          {
            ...
            marker->attach (ggqfit_plot);
#else
            long marker = ggqfit_plot->insertMarker ("", QwtPlot::xBottom,
                                                     QwtPlot::yRight);
            ggqfit_plot->setMarkerSymbol (marker, sym);
      ...
      ggqfit_plot->setAxisScale (QwtPlot::yRight, pfit_min * 0.95,
                                 pfit_max * 1.15);
  ...
  ggqfit_plot->setAxisScale (QwtPlot::xBottom,
                             0.95 * unified_ggaussian_qvals[0],
                             1.05 * unified_ggaussian_qvals.back ());
...
vector<double>
US_Hydrodyn_Mals::compute_ggaussian_gaussian_sum ()
{
  vector<double> result;
  if (!unified_ggaussian_ok)
    {
      editor_msg ("red", us_tr ("Internal error: gaussian rmsd called but "
                                "unified gaussians not ok"));
      return result;
    }
  ...
  for (unsigned int i = 0; i < unified_ggaussian_curves; ++i)
    {
      ...
      */
      ...
      // US_Vector::printvector( QString( "cggs: file %1, this_t" ).arg( i ),
      // this_t ); US_Vector::printvector( QString( "cggs: file %1, g" ).arg( i
      // ), g );
...
double
US_Hydrodyn_Mals::ggaussian_rmsd (bool norm_chi)
{
  if (!unified_ggaussian_ok)
    {
      editor_msg ("red", us_tr ("Internal error: gaussian rmsd called but "
  ...
  if (unified_ggaussian_use_errors && cb_sd_weight->isChecked ())
    {
      for (unsigned int i = 0; i < (unsigned int)result.size (); i++)
        {
          double tmp
              = (result[i] - unified_ggaussian_I[i]) / unified_ggaussian_e[i];
          rmsd += tmp * tmp;
        }
      if (norm_chi)
        {
          double nu = (double)((int)result.size ()
                               - (int)unified_ggaussian_params.size () - 1);
          if (nu <= 0e0)
            {
              nu = 1e0;
            }
...
bool
US_Hydrodyn_Mals::gg_fit_replot ()
{
  if (!unified_ggaussian_ok)
    {
      editor_msg ("red",
                  us_tr ("Internal error (gg_fit_replot): Global Gaussian "
...
bool
US_Hydrodyn_Mals::data_point_window (vector<double> &q, vector<double> &I)
{
  // takes current data and returns vectors of point window size centered about
  // current center used for gauss_local_caruanas() & gauss_local_gauss()
  ...
  if (pts < 3)
    {
      editor_msg (
          "red",
          us_tr ("Internal error: data_point_window() local pts less than 3"));
      return false;
    }
  ...
  if (!f_qs.count (wheel_file) || !f_Is.count (wheel_file))
    {
      editor_msg ("red", us_tr ("Internal error: data_point_window() wheel "
                                "file has not points"));
      return false;
    }
  ...
  tso << QString ().sprintf ("center       %g\n"
                             "this_pos     %d\n"
                             "q[this_pos]  %g\n"
                             "start_pos    %d\n"
                             "q[start_pos] %g\n",
                             center, this_pos, (*t_q)[this_pos], start_pos,
                             (*t_q)[start_pos]);
...
void
US_Hydrodyn_Mals::gauss_local_caruanas ()
{
...
void
US_Hydrodyn_Mals::gauss_local_guos ()
{
...
bool
US_Hydrodyn_Mals::pvalue (const vector<double> &q, vector<double> &I,
...
bool
US_Hydrodyn_Mals::pvalue (const QString &file, double &P, QString &errormsg)
{
  errormsg = "";

  if (!f_Is.count (file) || !f_qs.count (file))
    {
      errormsg = QString ("Internal error: pvalue() %1 is missing from data")
                     .arg (file);
      return false;
    }
  ...
  if (!f_gaussians.count (file))
    {
      errormsg = QString ("Internal error: pvalue() %1 is missing gaussians")
                     .arg (file);
      return false;
    }
  ...
  if (qsize != Isize)
    {
      errormsg = QString ("Internal error: pvalue() %1 differing I & q sizes")
                     .arg (file);
      return false;
    }
  ...
  for (int i = 0; i < qsize; ++i)
    {
      if (f_qs[file][i] >= q_start && f_qs[file][i] <= q_end)
        {
  ...
  if (!q.size ())
    {
      errormsg = QString ("Error: pvalue() %1 empty q vector after range trim")
                     .arg (file);
      return false;
    }
