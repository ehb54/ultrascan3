...
bool US_Saxs_Util::run_hydro(map<QString, QString> &parameters,
  ...
  if (!set_default(results, parameters))  // setup configuration defaults before
  ...
  if (!fi.exists()) {
    ...
    if (!fi.isReadable()) {
      results["errors"] +=
          QString(" file %1 exists but is not readable.").arg(files_try);
    } else {
  ...
  if (method != "None") {
    ...
    if (fabs(results_hydro.D20w_sd) <= 1e-100) {
      ...
      results["D20w"] = (QString("").sprintf(
          "%4.2e cm^2/sec (%4.2e)", results_hydro.D20w, results_hydro.D20w_sd));
    }

    if (fabs(results_hydro.rs_sd) <= 1e-100) {
      results["rs"] = QString::number(results_hydro.rs, 'e', 2) + " nm";
    } else {
      results["rs"] = (QString("").sprintf("%4.2e nm (%4.2e)", results_hydro.rs,
                                           results_hydro.rs_sd));
    }
    ...
    if (fabs(results_hydro.rg_sd) <= 1e-100) {
      ...
      results["rg"] = (QString("").sprintf("%4.2e nm (%4.2e)", results_hydro.rg,
                                           results_hydro.rg_sd));
    }
    ...
    if (method == "Zeno") {
      ...
      if (fabs(results_hydro.tau_sd) <= 1e-100) {
        ...
        results["tau"] = (QString("").sprintf(
            "%4.2e ns (%4.2e)", results_hydro.tau, results_hydro.tau_sd));
      }
    }
    ...
    if (fabs(results_hydro.viscosity_sd) <= 1e-100) {
      ...
      results["viscosity"] =
          (QString("").sprintf("%4.2e cm^3/g (%4.2e)", results_hydro.viscosity,
...
void US_Saxs_Util::read_residue_file() {
  ...
  if (f.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    while (!ts.atEnd()) {
      ...
      for (j = 0; j < numatoms; j++) {
        ...
        if (!new_atom.name.isEmpty() && new_atom.hybrid.radius > 0.0 &&
          ...
          residue_atom_hybrid_map
              [QString("%1|%2").arg(new_residue.name).arg(new_atom.name)] =
        ...
        //                            + new_atom.name + " cannot be read and
        //                            will be deleted from List."),
      ...
      if (!new_residue.name.isEmpty() && new_residue.molvol > 0.0 &&
        ...
        // if ( advanced_config.debug_1 )
        // {
        //    printf("residue name %s unique name %s atom size %u alt size %u
        //    pos %u\n"
        //           ,new_residue.name.toLatin1().data()
  ...
  if (multi_residue_map["OXT"].size() == 1 &&
    ...
    error_count++;
    QString msg =
        "Warning: PBR rule OXT not replaced with OXT-P because there is not "
        "exactly 1 OXT and 1 PBR-OXT\n";
    error_text += msg;
    error_msg += msg;
  }
        ...
        parameters_set_first_model)  //, bool display_pdb, bool skipclearissue )
{
  ...
  for (unsigned int i = 0; i < model_vector.size(); i++) {
    ...
    if (check_for_missing_atoms_hydro(&error_string, &model_vector[i],
      ...
      errors_found++;
      us_log->log(
          QString("Encountered errors with your PDB structure for model %1:\n")
              .arg(i) +
          error_string);
...
int US_Saxs_Util::check_for_missing_atoms_hydro(
    QString *error_string, PDB_model *model, bool parameters_set_first_model) {
  ...
  for (unsigned int j = 0; j < model->molecule.size(); j++) {
    ...
    for (unsigned int k = 0; k < model->molecule[j].atom.size(); k++) {
      ...
      for (unsigned int m = 0; m < residue_list.size(); m++) {
        ...
        if ((residue_list[m].name == this_atom->resName &&
            ...
            (!k && this_atom->name == "N" && restype == 0 && misc.pb_rule_on &&
          ...
          //    residue_list[m].name.toLatin1().data()); printf("resname %s
          //    respos %d mappos %d mapsize %u\n"
          //           ,this_atom->resName.toLatin1().data()
          //           ,m
          ...
          if (lastResSeq != this_atom->resSeq) {
            ...
            if (lastResPos != -1) {
              ...
              for (unsigned int l = 0;
                   l < residue_list[lastResPos].r_atom.size(); l++) {
                ...
                if (!residue_list[lastResPos].r_atom[l].tmp_flag) {
                    ...
                    switch (molecules_residue_errors[idx].size()) {
                      ...
                      default: {
                        ...
                        for (unsigned int t = 0;
                             t < molecules_residue_errors[idx].size(); t++) {
                          // if ( advanced_config.debug_1 )
                          // {
                          //    this_error += " d1";
                          // }
                          this_error +=
                              QString("    Residue file entry %1: %2\n")
                                  .arg(t + 1)
                                  .arg(molecules_residue_errors[idx][t]);
                        }
                      } break;
                    }
                  ...
                  // error_string->
                  // append(QString("").
                  //     sprintf("Missing atom: chain %s molecule %d atom %s
                  //     residue %s %s\n",
      ...
      if (respos == -1) {
        if ((this_atom->name != "H" && this_atom->name != "D" &&
             this_atom->resName != "DOD" && this_atom->resName != "HOH" &&
             (this_atom->altLoc == "A" || this_atom->altLoc == " "))) {
          QString msg_tag;
          bool do_error_msg = true;
          if (pdb_parse.missing_residues == 0 && pdb_parse.missing_atoms == 0) {
            failure_errors++;
            ...
            //    2.1 there must be a missing atom since count doesn't match so
            //    atom controls
            ...
            if (multi_residue_map[this_atom->resName].size()) {
              ...
              if (pdb_parse.missing_atoms == 2) {
                if (molecules_residue_min_missing[QString("%1|%2").arg(j).arg(
                        this_atom->resSeq)] == -1) {
          ...
          if (do_error_msg) {
            ...
            // error_string->append(QString("").sprintf("%s: chain %s molecule
            // %d atom %s residue %s %s\n",
        ...
        if (atompos == -1) {
              ...
              if (bead_exceptions[count_idx] == 1) {
                switch (pdb_parse.missing_residues) {
                  ...
                  default:
                    failure_errors++;
                    // US_Static::us_message(us_tr("Internal error:"),
                    //                      us_tr("Unexpected pdb_parse.missing
                    //                      residues type ") +
                    //                      QString("%1\n").arg(pdb_parse.missing_residues));
                    exit(-1);
                    break;
          ...
          // error_string->append(QString("").sprintf("unknown atom chain %s
    ...
    if (lastResPos != -1) {
      // check for false entries in last residue and warn about them
      // printf("check for false entries in last residue %d, residues_found %d,
      ...
      for (unsigned int l = 0; l < residue_list[lastResPos].r_atom.size();
        ...
        if (!residue_list[lastResPos].r_atom[l].tmp_flag &&
          ...
          // if ( advanced_config.debug_1 )
          // {
          //    printf("dbg x pdb_parse.missing_atoms %d bead_exceptions[%s] %d
          ...
          // error_string->
          //  append(QString("").
          // sprintf("missing atom chain %s molecule %d atom %s residue %s
          // %s\n",
          //    lastChainID.toLatin1().data(),
          //    j + 1,
          //    residue_list[lastResPos].r_atom[l].name.toLatin1().data(),
      ...
      *error_string = "";
          ...
          switch (bead_exceptions[count_idx]) {
            ...
            case 5:  // extra or non-coded atom
            {
              ...
              if (!abb_msg_done[count_idx]) {
                abb_msgs +=
                    QString("ABB: %1Molecule %2 Residue %3 %4 Skipped\n")
                        .arg(this_atom->chainID == " "
                                 ? ""
                                 : ("Chain " + this_atom->chainID + " "))
                        .arg(j + 1)
            ...
            case 3: {
              ...
              QString new_residue_idx =
                  this_atom->resName;  // we could add atom_count to the idx for
                                       // counting by unique atom counts...
              // if ( advanced_config.debug_2  )
              // {
              ...
              if (this_atom->resSeq != lastResSeq) {
                ...
                // each instance of the residue gets a unique name, so we don't
                // have to worry about duplicates and alternate duplicates
                new_residues[new_residue_idx]++;
                new_residue_name = QString("%1_NC%2")
                                       .arg(this_atom->resName)
                                       .arg(new_residues[new_residue_idx]);
                unknown_residues[new_residue_name] = true;
                abb_msgs +=
                    QString(
                        "ABB: %1Molecule %2 Residue %3 %4 Non-coded residue %5 "
                        "created.\n")
                        .arg(this_atom->chainID == " "
                                 ? ""
                                 : ("Chain " + this_atom->chainID + " "))
                ...
                // create new_residue
                new_residue.comment =
                    QString("Temporary residue %1").arg(new_residue_name);
                ...
                new_residue.r_bead.clear();
                new_bead.hydration =
                    (unsigned int)(misc.avg_hydration * atom_counts[count_idx] +
                                   .5);
                ...
                // }
                model->residue.push_back(
                    residue_list[multi_residue_map[new_residue.name][0]]);
                lastOrgResSeq = org_model.molecule[j].atom[k].orgResSeq;
                lastOrgChainID = org_model.molecule[j].atom[k].orgChainID;
                lastOrgResName = org_model.molecule[j].atom[k].orgResName;
              } else {
                ...
                if (new_residue_name != new_residue.name) {
                  // US_Static::us_message(us_tr("Internal error:"),
                  //                      us_tr("Unexpected new residue name
                  //                      inconsistancy.\n") + QString("\"%1\"
                  //                      !=
              ...
              // the new residue should always be the last one, since we just
              // added it
              int respos = residue_list.size() - 1;
              if (new_residue.name != residue_list[respos].name) {
                // US_Static::us_message(us_tr("Internal error:"),
                //                      us_tr("Unexpected new residue name
                //                      residue_list inconsistancy."));
                exit(-1);
              }
                ...
                if (residue_list[respos].r_atom.size() < 4) {
                  ...
                  if (residue_list[respos].r_atom.size() == 4) {
                    if (aa_main_chain["N"] == 1 && aa_main_chain["CA"] == 1 &&
                      ...
                      abb_msgs +=
                          QString(
                              "ABB: %1Molecule %2 Residue %3 %4 Non-coded "
                              "residue %5 PB found, 2 beads created.\n")
                              .arg(this_atom->chainID == " "
                                       ? ""
                                       : ("Chain " + this_atom->chainID + " "))
                              .arg(j + 1)
              ...
              new_atom.name =
                  (this_atom->name == "OXT" ? "OXT'" : this_atom->name);
                  ...
                  residue_list[residue_list.size() - 1];
              {
                ...
                if (!residue_atom_hybrid_map.count(mapkey) &&
                  ...
                  // us_qdebug( QString( "hybrid map created for %1 created as
                  // %2" ).arg( mapkey ).arg(
                  // residue_atom_abb_hybrid_map[atom_to_add.name ] ) );
                }
              }
            ...
            case 4:  // missing atoms
            {
              ...
              if (pos < 0) {
                // cout << QString("Internal error: idx %1 Missing atom error %2
                // ! < 0!\n")
                //   .arg(idx)
                //   .arg(pos);
                exit(-1);
              ...
              if (this_atom->resSeq != lastResSeq) {
                // new residue to create with existing atoms, but increased
                // values... possibly for a multi-bead residue
                one_bead = false;
                current_bead_assignment = 0;
                lastResSeq = org_model.molecule[j].atom[k].resSeq;
                lastResName = org_model.molecule[j].atom[k].resName;
                ...
                for (unsigned int i = 0; i < beads_missing_atom_count.size();
                     i++) {
                  beads_missing_atom_count[i] = 0;
                }
                // accumulate missing atom count for each bead
                for (unsigned int i = 0;
                     i < molecules_residue_missing_atoms[idx][pos].size();
                     i++) {
                  beads_missing_atom_count
                      [molecules_residue_missing_atoms_beads[idx][pos][i]]++;
                }
                ...
                for (unsigned int i = 0;
                     i < residue_list[orgrespos].r_atom.size(); i++) {
                  atoms_per_bead
                      [residue_list[orgrespos].r_atom[i].bead_assignment]++;
                }
                ...
                for (unsigned int i = 0; i < beads_missing_atom_count.size();
                  ...
                  if (atoms_per_bead[i] - beads_missing_atom_count[i] < 1) {
                    ...
                    one_bead = true;
                    // printf("notice: fallback to one bead for missing
                    // atom\n");
                  }
                }

                vector<double>
                    atoms_scale_weight;  // multiplier for atoms weight
                vector<double>
                    atoms_scale_radius;  // multiplier for atoms radii
                atoms_scale_weight.resize(
                    residue_list[orgrespos].r_bead.size());
                atoms_scale_radius.resize(
                    residue_list[orgrespos].r_bead.size());
                if (one_bead) {
                  // sum molecular weight & radii cubed for total bead & bead
                  // sans missing
                  double tot_mw = 0;
                  ...
                  for (unsigned int i = 0;
                    ...
                    if (!molecules_residue_missing_atoms_skip
                            [QString("%1|%2|%3").arg(idx).arg(pos).arg(i)]) {
                      tot_mw_missing +=
                          residue_list[orgrespos].r_atom[i].hybrid.mw;
                      tot_radii3_missing += pow(
                          residue_list[orgrespos].r_atom[i].hybrid.radius, 3);
                    }
                  }
                  atoms_scale_weight[0] = tot_mw / tot_mw_missing;
                  ...
                  for (unsigned int i = 0;
                    ...
                    if (!molecules_residue_missing_atoms_skip
                      ...
                      tot_radii3_missing
                          [residue_list[orgrespos].r_atom[i].bead_assignment] +=
                          pow(residue_list[orgrespos].r_atom[i].hybrid.radius,
                              3);
                    }
                  }
                  for (unsigned int i = 0; i < beads_missing_atom_count.size();
                       i++) {
                    atoms_scale_weight[i] = tot_mw[i] / tot_mw_missing[i];
                ...
                new_residues[new_residue_idx]++;
                new_residue_name = QString("%1_MA%2")
                                       .arg(this_atom->resName)
                                       .arg(new_residues[new_residue_idx]);
                unknown_residues[new_residue_name] = true;
                ...
                abb_msgs +=
                    QString(
                        "ABB: %1Molecule %2 Residue %3 %4 Missing atom residue "
                        "copy %5 created%6.\n")
                        .arg(this_atom->chainID == " "
                                 ? ""
                                 : ("Chain " + this_atom->chainID + " "))
                ...
                if (one_bead) {
                  ...
                  for (unsigned int i = 0;
                       i < residue_list[orgrespos].r_bead.size(); i++) {
                    new_bead.volume += residue_list[orgrespos].r_bead[i].volume;
                    new_bead.mw += residue_list[orgrespos].r_bead[i].mw;
                    new_bead.hydration +=
                        residue_list[orgrespos].r_bead[i].hydration;
                  }
                  // new_bead.hydration = (unsigned int)(misc.avg_hydration *
                  ...
                  if (auto_bb_aa &&
                      !molecules_residue_missing_atoms_skip
                          [QString("%1|%2|%3").arg(idx).arg(pos).arg(0)] &&
                      !molecules_residue_missing_atoms_skip
                          [QString("%1|%2|%3").arg(idx).arg(pos).arg(1)] &&
                      !molecules_residue_missing_atoms_skip
                          [QString("%1|%2|%3").arg(idx).arg(pos).arg(2)] &&
                      !molecules_residue_missing_atoms_skip
                          [QString("%1|%2|%3").arg(idx).arg(pos).arg(3)]) {
                  ...
                  for (unsigned int i = 0;
                    ...
                    if (!molecules_residue_missing_atoms_skip
                      ...
                      new_atom.hybrid.mw *=
                          atoms_scale_weight[0];  // misc.avg_mass;
                      new_atom.hybrid.mw =
                          (int)(new_atom.hybrid.mw * 100 + .5) / 100.0;
                      new_atom.hybrid.radius *=
                          atoms_scale_radius[0];  // misc.avg_radius;
                      new_atom.hybrid.radius =
                          (int)(new_atom.hybrid.radius * 100 + .5) / 100.0;
                  ...
                  for (unsigned int i = 0;
                    ...
                    if (!molecules_residue_missing_atoms_skip
                            [QString("%1|%2|%3").arg(idx).arg(pos).arg(i)]) {
                      if (residue_list[orgrespos].r_atom[i].positioner) {
                        any_positioner[residue_list[orgrespos]
                                           .r_atom[i]
                                           .bead_assignment] = true;
                      }
                    }
                  }
                  ...
                  for (unsigned int i = 0;
                    ...
                    if (!molecules_residue_missing_atoms_skip
                      ...
                      if (beads_missing_atom_count[new_atom.bead_assignment]) {
                        new_atom.hybrid.mw *= atoms_scale_weight
                            [new_atom.bead_assignment];  // misc.avg_mass;
                        new_atom.hybrid.mw =
                            (int)(new_atom.hybrid.mw * 100 + .5) / 100.0;
                        new_atom.hybrid.radius *= atoms_scale_radius
                            [new_atom.bead_assignment];  // misc.avg_radius;
                        new_atom.hybrid.radius =
                            (int)(new_atom.hybrid.radius * 100 + .5) / 100.0;
      ...
      for (unsigned int i = 0; i < unknown_aa_residues_to_check.size(); i++) {
        ...
        if (residue_list[respos].r_bead.size() == 2 &&
          ...
          aa_main_chain["C"] = 0;
          aa_main_chain["O"] = 0;
          for (unsigned int j = 0; j < residue_list[respos].r_atom.size();
               j++) {
...
void US_Saxs_Util::update_vbar() {
  ...
  {  // If we computed vbar, we assign this to result.vbar, which should be used
     // in the calculation.
...
bool US_Saxs_Util::hard_coded_defaults(map<QString, QString> &results,
...
bool US_Saxs_Util::set_default(map<QString, QString> &results,
                               map<QString, QString> &parameters) {
  // QFile f;
  // QString str;
  // int j;
  ...
  if (!hard_coded_defaults(results, parameters)) return false;
...
bool US_Saxs_Util::calc_mw_hydro() {
  ...
  for (unsigned int i = 0; i < model_vector.size(); i++) {
    ...
    // if( !error_string.length() )
    {
      for (unsigned int j = 0; j < model_vector[i].molecule.size(); j++) {
          ...
          if (this_atom->active) {
            ...
            if (do_excl_vol) {
              ...
              if (!set_excluded_volume(
                      *this_atom, excl_vol, scaled_excl_vol, our_saxs_options,
                      residue_atom_hybrid_map, this_e, this_e_noh)) {
                return false;
              } else {
        ...
        if (model_vector[i].molecule[j].mw != 0.0) {
          QString qs =
              QString(
                  "\nModel: %1 Chain: %2 Molecular weight %3 Daltons, Volume "
                  "(from vbar) %4 A^3%5")
                  .arg(model_vector[i].model_id)
    ...
    }
    {
      QString qs =
          QString(
              "\nModel: %1 Molecular weight %2 Daltons, Volume (from vbar) %3 "
              "A^3%4")
              .arg(model_vector[i].model_id)
              .arg(model_vector[i].mw)
              .arg(mw_to_volume(model_vector[i].mw, model_vector[i].vbar))
              .arg(do_excl_vol
                       ? QString(", atomic volume %1 A^3%2 average electron "
                                 "density %3 A^-3")
                             .arg(tot_excl_vol)
      ...
      if (us_udp_msg) {
        ...
        msging["_textarea"] =
            QString(
                "\\nModel: %1 Molecular weight %2 Daltons, Volume (from vbar) "
                "%3 A^3%4")
                .arg(model_vector[i].model_id)
                .arg(model_vector[i].mw)
                .arg(mw_to_volume(model_vector[i].mw, model_vector[i].vbar))
...
bool US_Saxs_Util::calc_grid_pdb(bool /* parameters_set_first_model */) {
  ...
  //                             residue.\n" "These are currently generated only
  //                             for SAXS/SANS computations\n"
  //                             )
  //                          );
  //    return -1;
  // }
  ...
  if (grid_hydro.create_nmr_bead_pdb) {
    if ((grid_exposed_overlap.remove_overlap || grid_overlap.remove_overlap ||
      ...
      //                           "US-SOMO: Build AtoB models with structure
      //                           factors", us_tr( msg ) );
  ...
  for (unsigned int curr_m = 0; curr_m < model_vector.size(); curr_m++) {
    ...
    if (us_udp_msg) {
      ...
      msging["progress_output"] =
          QString("AtoB model calculation: %1\%")
              .arg(QString::number((int(double(ppos) / double(mppos)) * 100.0)))
    ...
    if (retval) {
      ...
      if (error_string.length()) {
          ...
          for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size();
            ...
            if (this_atom->active) {
              ...
              if (grid_hydro.hydrate && this_atom->atom_hydration) {
                double additional_radius = pow(
                    3 * this_atom->atom_hydration * misc.hydrovol / (M_PI * 4),
                    1.0 / 3.0);
                this_atom->bead_computed_radius += additional_radius;
                // #if defined(GRID_HYDRATE_DEBUG)
                //                               printf("hydrating atom %s %s %s
                //                               hydration %f radius %f + %f ->
                //                               %f\n"
                //                                      ,
                //                                      this_atom->name.toLatin1().data()
                //                                      ,
        ...
        {
          // editor_msg( "red", "Center of scattering requested, but zero
          // scattering intensities are present" );
          // pb_grid_pdb->setEnabled(true);
          // pb_somo->setEnabled(true);
          // pb_somo_o->setEnabled(true);
          // progress->reset();
        ...
        /* OMG! ***************************** */
        bead_models[i] =
            us_hydrodyn_grid_atob_hydro(&bead_model, &grid_hydro,
                                        // progress,
                                        // editor,
                                        // this);
        ...
        if (grid_hydro.enable_asa) {
          ...
          //                      i++) {
          //                         printf("after asa bead %d exposed %d color
          //                         %d chain %d active %d mw %f vol %f cr %f
          //                         [%f,%f,%f]\n",
          //                                i,
        ...
        //       ].saxs_name.toUpper(); for ( unsigned int i = 0; i < 4; i++ )
        //       {
        //          extra_text += QString( " %1" ).arg( sf_bead_factors[ k ].a[
        //          i ] ); extra_text += QString( " %1" ).arg( sf_bead_factors[
        //          k ].b[ i ] );
        //       }
        //       extra_text += QString( " %1 %2\n" ).arg( sf_bead_factors[ k ].c
        //       ).arg( bead_model[ k ].saxs_excl_vol );

        //       extra_text += "BSAXS:: " + sf_bead_factors[ k ].saxs_name;
        //       for ( unsigned int i = 0; i < 5; i++ )
        //       {
        //          extra_text += QString( " %1" ).arg( sf_bead_factors[ k ].a5[
        //          i ] ); extra_text += QString( " %1" ).arg( sf_bead_factors[
        //          k ].b5[ i ] );
        //       }
        //       extra_text += QString( " %1 %2\n" ).arg( sf_bead_factors[ k
        //       ].c5 ).arg( bead_model[ k ].saxs_excl_vol ); tot_excl_vol +=
        //       bead_model[ k ].saxs_excl_vol;

        //       extra_text += "BSAXSV:: " + sf_bead_factors[ k ].saxs_name;
        //       for ( unsigned int i = 0; i < sf_bead_factors[ k
        //       ].vcoeff.size(); i++ )
        //       {
        //          extra_text += QString( " %1" ).arg( sf_bead_factors[ k
        //          ].vcoeff[ i ] );
        //       }
        ...
        //    //editor_msg( "blue", QString( "Saxs name for dummy atom models
        //    set to %1" ).arg( saxs_options_hydro.dummy_saxs_name ) );

        //    // if ( saxs_map.count( sf_factors.saxs_name ) )
        //    // {
        //    //    editor_msg( "dark red",
        //    //                QString( us_tr( "Notice: saxs coefficients for
        //    %1 replaced by newly loaded values\n" ) )
        //    //                .arg( sf_factors.saxs_name ) );
        //    // } else {
        //    //    saxs_list.push_back( sf_factors );
        //    //    editor_msg( "dark blue",
        //    //                QString( us_tr( "Notice: added coefficients for
        //    %1 from newly loaded values\n" ) )
        //    //                .arg( sf_factors.saxs_name ) );
        //    // }
        ...
        //    {
        //       // editor_msg( "red",
        //       //             QString( us_tr( "Overriding setting to use
        //       global structure factors since bead model doesn't contain the
        //       correct number of structure factors (%1) for the beads (%2)" )
        //       )
        ...
        //       {
        //          // editor_msg( "red", QString( us_tr("Warning: No '%1' SAXS
        //          atom found.\n" ) )
        ...
        //          }
        //       } else {
        //          // editor_msg( "blue", QString( us_tr("Notice: Loading beads
        //          with saxs coefficients '%1'" ) )
        //          //             .arg( saxs_options_hydro.dummy_saxs_name ) );
        //          for( unsigned int i = 0; i < bead_model.size(); i++ )
        //          {
        ...
        //       sf_bead_factors.size() == bead_model.size() )
        //       {
        // 	  //editor_msg( "blue", us_tr("Notice: Loading beads with bead
...
void US_Saxs_Util::bead_check(bool use_threshold, bool message_type) {
  ...
  if (retval) {
    switch (retval) {
      case US_HYDRODYN_ASAB1_ERR_MEMORY_ALLOC: {
        // printError("US_HYDRODYN_ASAB1 on bead recheck encountered a memory
        // allocation error");
...
#if defined(DEBUG)
    printf(
        "bead %d %.2f %.2f %.2f %s %s bead mw %.2f bead ref mw %.2f\n", i,
        bead_model[i].bead_computed_radius, surface_area,
        bead_model[i].bead_recheck_asa,
        (use_threshold ? (bead_model[i].bead_recheck_asa > asa_hydro.threshold)
                       : (bead_model[i].bead_recheck_asa >
                          (asa_hydro.threshold_percent / 100.0) * surface_area))
            ? "exposed"
            : "buried",
  ...
  if (message_type) {
    ...
    us_log->log(
        QString(us_tr("%1 previously buried beads are exposed by rechecking\n"))
            .arg(b2e));
    if (us_udp_msg) {
      map<QString, QString> msging;
      msging["_textarea"] =
...
static void outward_translate_2_spheres(
  ...
  printf(
      ...
      "[%.3f,%.3f,%.3f]\n",
      *r1, x1[0], x1[1], x1[2], v1[0], v1[1], v1[2], *r2, x2[0], x2[1], x2[2],
      v2[0], v2[1], v2[2]);
#endif
  ...
  float sfact = sqrt(
      ...
      ((double)(*r1 * *r1 *
       ...
       pow(*r1 * *r1 +
               ...
               *r2 * (*r2 - v2[0] * x1[0] - v2[1] * x1[1] - v2[2] * x1[2] +
...
int US_Saxs_Util::radial_reduction(bool from_grid, int use_ppos, int mppos) {
  ...
  float rr_overlap[] = {(float)((sidechain_overlap.remove_hierarch
                                     ? sidechain_overlap.remove_hierarch_percent
                                     : sidechain_overlap.remove_sync_percent) /
                                100.0),
  ...
  stage_loop:
    ...
    printf(
        "popping stage %d %s%s%s%s%s%s%s%s%s%s%s%s%s%s%soverlap_reduction %f "
        "rroverlap %f\n",
    ...
    do {
      ...
      // QString("bead_model_bp-%1-%2").arg(k).arg(iter) + DOTSOMO + ".tsv",
      ...
      //   {
      //     map < QString, QString > msging;
      //     msging[ "progress_output" ] =  QString("Stage %1 popping iteration
      //     %2").arg(k+1).arg(iter); msging[ "progress1" ] =
      ...
      if (methods[k] &
          ...
          for (unsigned int j = i + 1; j < bead_model.size(); j++) {
            ...
            if (bead_model[i].active && bead_model[j].active &&
...
#if defined(DEBUG)
              printf(
                  "this overlap bead %u %u vol %f rv1 %f rv2 %f r1 %f r2 %f p1 "
                  "[%f,%f,%f] p2 [%f,%f,%f]\n",
                  bead_model[i].serial, bead_model[j].serial,
                  intersection_volume, bead_model[i].bead_ref_volume,
              ...
              if (intersection_volume >
                  ...
                  intersection_volume >
                      bead_model[j].bead_ref_volume * overlap[k]) {
                overlaps_exist = true;
        ...
        if (overlaps_exist) {
          ...
          // 	 {
          // 	   map < QString, QString > msging;
          // 	   msging[ "progress_output" ] = QString("Gridding %1 of
          // %2").arg(i+1).arg(npoints_x); 	   msging[ "progress1" ] =
          // QString::number(double(i+1)/double(npoints_x));
          // 	   //msging[ "_progress" ] =
          // QString::number(double(i+1)/double(npoints_x));
          ...
          // 	   //sleep(1);
          // 	 }
          // lbl_core_progress->setText(QString("Stage %1 popping iteration %2
          // beads popped %3").arg(k+1).arg(iter).arg(beads_popped));
          // qApp->processEvents();
          // //#define DEBUG_FUSED
#if defined(DEBUG1) || defined(DEBUG) || defined(DEBUG_FUSED)
          printf(
              "popping beads %u %u int vol %f mw1 %f mw2 %f v1 %f v2 %f c1 "
              "[%f,%f,%f] c2 [%f,%f,%f]\n",
          ...
          for (unsigned int n = 0; n < bead_model[max_bead2].all_beads.size();
               n++) {
          ...
          bead_model[max_bead1].bead_actual_radius =
              bead_model[max_bead1].bead_computed_radius =
                  pow(3 * bead_model[max_bead1].bead_ref_volume / (4.0 * M_PI),
                      1.0 / 3);
...
#if defined(DEBUG)
          printf(
              "after popping beads %d %d int volume %f radius %f mw %f vol %f "
              "coordinate [%f,%f,%f]\n",
              bead_model[max_bead1].serial, bead_model[max_bead2].serial,
              intersection_volume, bead_model[max_bead1].bead_computed_radius,
              bead_model[max_bead1].bead_ref_mw,
              bead_model[max_bead2].bead_ref_volume,
              bead_model[max_bead1].bead_coordinate.axis[0],
              bead_model[max_bead1].bead_coordinate.axis[1],
      ...
      // write_bead_tsv(somo_tmp_dir + SLASH +
      // QString("bead_model_ap-%1-%2").arg(k).arg(iter) + DOTSOMO + ".tsv",
      // &bead_model); write_bead_spt(somo_tmp_dir + SLASH +
    ...
    if (methods[k] & RADIAL_REDUCTION) {
      ...
      overlap_check(methods[k] & RR_SC ? true : false,
                    ...
                    methods[k] & RR_BURIED ? true : false, overlap_tolerance);
#endif
      if (methods[k] & RR_HIERC) {
        ...
        //   {
        // 	map < QString, QString > msging;
        // 	msging[ "progress_output" ] = QString("Stage %1 hierarchical
        // radial reduction").arg(k+1); 	msging[ "progress1" ] =
        ...
        // lbl_core_progress->setText(QString("Stage %1 hierarchical radial
...
#if defined(DEBUG)
            printf(
                "checking radial stage %d beads %d %d on chains %d %d exposed "
                "code %d %d active %s %s max il %f %s %s %s %s\n",
                k, i, j, bead_model[i].chain, bead_model[j].chain,
                bead_model[i].exposed_code, bead_model[j].exposed_code,
                bead_model[i].active ? "Y" : "N",
                bead_model[j].active ? "Y" : "N", max_intersection_length,
                ...
                (bead_model[i].active && bead_model[j].active &&
                 (methods[k] & RR_MCSC ||
                  ((methods[k] & RR_SC) && bead_model[i].chain == 1 &&
                   bead_model[j].chain == 1)) &&
                 ((methods[k] & RR_BURIED) ||
                  (bead_model[i].exposed_code == 1 &&
                   bead_model[j].exposed_code == 1)) &&
...
#if defined(DEBUG)
              printf(
                  "beads %d %d with radii %f %f with coordinates [%f,%f,%f] "
                  "[%f,%f,%f] have a sep of %f\n",
        ...
        do {
          ...
          // lbl_core_progress->setText(QString("Stage %1 hierarch. red. it. %2
          // with %3 couples").arg(k+1).arg(iter).arg(pairs.size()));
          // qApp->processEvents();
          ...
          for (unsigned int i = 0; i < pairs.size(); i++) {
            ...
            printf("pair %d %d sep %f %s %s\n", pairs[i].i, pairs[i].j,
                   ...
                   pairs[i].i == max_bead1 || pairs[i].j == max_bead1 ||
                           pairs[i].i == max_bead2 || pairs[i].j == max_bead2
                       ? "needs recompute of separation"
                       : "separation valid");
#endif
            if (pairs[i].active) {
              if (pairs[i].i == max_bead1 || pairs[i].j == max_bead1 ||
                  pairs[i].i == max_bead2 || pairs[i].j == max_bead2) {
                pairs[i].separation =
                    bead_model[pairs[i].i].bead_computed_radius +
                    bead_model[pairs[i].j].bead_computed_radius -
                    sqrt(pow(bead_model[pairs[i].i].bead_coordinate.axis[0] -
                                 bead_model[pairs[i].j].bead_coordinate.axis[0],
                             2) +
                         pow(bead_model[pairs[i].i].bead_coordinate.axis[1] -
                                 bead_model[pairs[i].j].bead_coordinate.axis[1],
                             2) +
                         pow(bead_model[pairs[i].i].bead_coordinate.axis[2] -
                                 bead_model[pairs[i].j].bead_coordinate.axis[2],
                             2));
                pairs[i].active = true;
              }
          ...
          if (max_intersection_length > TOLERANCE) {
...
#if defined(DEBUG1) || defined(DEBUG)
            printf(
                "processing radial reduction hierc iteration %d pair %d "
                "processed %d\n",
                iter, max_pair, count);
            ...
            do {
              ...
              if (methods[k] & OUTWARD_TRANSLATION ||
                ...
                if ((methods[k] & RR_MCSC &&
...
#if defined(DEBUG2)
                  printf(
                      "use bead %d no outward translation is required, one "
                      "bead to shrink, radius delta %f  cr %f %f\n",
                  ...
                  if (bead_model[use_bead].bead_computed_radius <= 0) {
                    // this is to ignore this bead for further radial reduction
                    // regardless
                    bead_model[use_bead].bead_computed_radius =
                  ...
                  if (!bead_model[use_bead].normalized_ot_is_valid) {
                    ...
                    for (unsigned int l = 0; l < 3; l++) {
                      bead_model[use_bead].normalized_ot.axis[l] =
                          ...
                          molecular_cog[l];
                      norm += bead_model[use_bead].normalized_ot.axis[l] *
                              bead_model[use_bead].normalized_ot.axis[l];
                    }
                    norm = sqrt(norm);
                    if (norm) {
                ...
                // are we shrinking just 1 bead ... if we are dealing with
                // buried beads, then only buried beads should be shrunk, not
                // exposed beads
#if defined(DEBUG2)
                ...
                if (methods[k] & RR_MCSC && !(methods[k] & RR_BURIED) &&
                    bead_model[max_bead1].chain == 1 &&
                    bead_model[max_bead2].chain == 1) {
                  printf(
                      "what are we doing here?  dealing with 2 SC's on the "
                      "MCSC run??\n");
                  // exit(-1);
                }
                if ((methods[k] & RR_BURIED &&
                     (bead_model[max_bead1].exposed_code == 1 ||
                      bead_model[max_bead2].exposed_code == 1)) ||
                    (methods[k] & RR_MCSC && !(methods[k] & RR_BURIED) &&
                     (bead_model[max_bead1].chain == 1 ||
                      bead_model[max_bead2].chain == 1))) {
...
#if defined(DEBUG2)
                  printf(
                      "intersection len %f recomputed %f radius delta %f r1 %f "
                      "r2 %f\n",
                      max_intersection_length,
                      bead_model[max_bead1].bead_computed_radius +
                          bead_model[max_bead2].bead_computed_radius -
                          sqrt(pow(bead_model[max_bead1]
                               ...
                               pow(bead_model[max_bead1]
                                       ...
                                       bead_model[max_bead2]
                                           .bead_coordinate.axis[2],
                                   2)),
                      radius_delta, bead_model[max_bead1].bead_computed_radius,
                      bead_model[max_bead2].bead_computed_radius);
#endif
                  if (radius_delta >
                      bead_model[use_bead].bead_computed_radius) {
              ...
              printf(
                  "flags %s %s %s %s\n",
                  bead_model[max_bead1].bead_computed_radius > TOLERANCE ? "Y"
                                                                         : "N",
                  bead_model[max_bead2].bead_computed_radius > TOLERANCE ? "Y"
                                                                         : "N",
                  (bead_model[max_bead1].bead_computed_radius +
                       bead_model[max_bead2].bead_computed_radius -
                       sqrt(pow(bead_model[max_bead1].bead_coordinate.axis[0] -
                                    bead_model[max_bead2]
                                        .bead_coordinate.axis[0],
                                2) +
                            pow(bead_model[max_bead1].bead_coordinate.axis[1] -
        ...
        //             radial_reduction_thr_threads(threads); for ( unsigned int
        //             j = 0; j < threads; j++ )
        //             {
        ...
        do {
...
#endif
          // write_bead_tsv(somo_tmp_dir + SLASH +
          // QString("bead_model_br-%1-%2").arg(k).arg(iter) + DOTSOMO + ".tsv",
          // &bead_model); write_bead_spt(somo_tmp_dir + SLASH +
          // QString("bead_model-br-%1-%2").arg(k).arg(iter) + DOTSOMO,
          // &bead_model);
          iter++;
#if defined(DEBUG1) || defined(DEBUG)
...
#endif
          //   lbl_core_progress->setText(QString("Stage %1 synchronous radial
          //   reduction iteration %2").arg(k+1).arg(iter));
          //               qApp->processEvents();
          if (iter > 10000) {
          ...
          //                         max_intersection_length) {
          //                            max_intersection_length =
          //                            my_pairs[j][i].separation; max_bead1 =
          //                            my_pairs[j][i].i; max_bead2 =
          //                            my_pairs[j][i].j;
          //                         }
          //                         count++;
          //                      }
...
#if defined(DEBUGX)
                printf(
                    "checking radial stage %d beads %d %d on chains %d %d "
                    "exposed code %d %d active %s %s max il %f\n",
                    k, i, j, bead_model[i].chain, bead_model[j].chain,
                    bead_model[i].exposed_code, bead_model[j].exposed_code,
                    bead_model[i].active ? "Y" : "N",
                    bead_model[j].active ? "Y" : "N", max_intersection_length);
#endif
                if ((1 || last_reduced[j]) && bead_model[i].active &&
                    bead_model[j].active &&
                    (methods[k] & RR_MCSC ||
                     ((methods[k] & RR_SC) && bead_model[i].chain == 1 &&
                      bead_model[j].chain == 1)) &&
...
#if defined(DEBUG1) || defined(DEBUG)
          printf(
              "processing radial reduction sync iteration %d pairs to process "
              "%d max int len %f\n",
              iter, count, max_intersection_length);
#endif

          // lbl_core_progress->setText(QString("Stage %1 synchron. red. it. %2
          // with %3 couples").arg(k+1).arg(iter).arg(pairs.size()));
          // qApp->processEvents();
...
#if defined(DEBUG1) || defined(DEBUG)
            printf(
                "processing radial reduction sync iteration %d pairs to "
                "process %d\n",
                iter, count);
#endif
            for (unsigned int i = 0; i < pairs.size(); i++) {
              if (!reduced[pairs[i].i] &&
                  (k == 3 ||
                   ((bead_model[pairs[i].i].exposed_code != 1 ||
                     methods[k] & RR_EXPOSED || methods[k] & RR_ALL) &&
                    (!(methods[k] & RR_MCSC) ||
                ...
                if (1) {
                  ...
                  reduced_any[use_bead] = true;
                  float radius_delta =
                      bead_model[use_bead].bead_computed_radius * rr_overlap[k];
          ...
          // write_bead_tsv(somo_tmp_dir + SLASH +
          // QString("bead_model_ar-%1-%2").arg(k).arg(iter) + DOTSOMO + ".tsv",
          // &bead_model); write_bead_spt(somo_tmp_dir + SLASH +
          // QString("bead_model-ar-%1-%2").arg(k).arg(iter) + DOTSOMO,
          // &bead_model);
          //               qApp->processEvents();
...
int US_Saxs_Util::overlap_check(bool sc, bool mc, bool buried, double tolerance,
                                int limit) {
  int retval = 0;
#if defined(DEBUG_OVERLAP)
  printf(
  ...
  for (unsigned int i = 0; i < bead_model.size() - 1; i++) {
    for (unsigned int j = i + 1; j < bead_model.size(); j++) {
      ...
      if (i == 2 && (j == 34 || j == 60))
        printf(
            "x1 overlap check  beads %d %d on chains %d %d exposed code %d %d "
            "active %s %s : radii %f %f with coordinates [%f,%f,%f] [%f,%f,%f] "
            "sep of %f - %s\n",
            i, j, bead_model[i].chain, bead_model[j].chain,
            bead_model[i].exposed_code, bead_model[j].exposed_code,
            bead_model[i].active ? "Y" : "N", bead_model[j].active ? "Y" : "N",
            bead_model[i].bead_computed_radius,
            bead_model[j].bead_computed_radius,
            bead_model[i].bead_coordinate.axis[0],
            bead_model[i].bead_coordinate.axis[1],
      ...
      if (bead_model[i].active && bead_model[j].active &&
          (mc ||
           (sc && bead_model[i].chain == 1 && bead_model[j].chain == 1)) &&
          (buried || (bead_model[i].exposed_code == 1 &&
                      bead_model[j].exposed_code == 1)) &&
          bead_model[i].bead_computed_radius > tolerance &&
          bead_model[j].bead_computed_radius > tolerance) {
        float separation = bead_model[i].bead_computed_radius +
                           bead_model[j].bead_computed_radius -
                           sqrt(pow(bead_model[i].bead_coordinate.axis[0] -
                                        bead_model[j].bead_coordinate.axis[0],
                                    2) +
        ...
        if (i == 2 && (j == 34 || j == 60))
          printf(
              "x2 overlap check  beads %d %d on chains %d %d exposed code %d "
              "%d active %s %s : radii %f %f with coordinates [%f,%f,%f] "
              "[%f,%f,%f] sep of %f - %s\n",
              i, j, bead_model[i].chain, bead_model[j].chain,
              bead_model[i].exposed_code, bead_model[j].exposed_code,
              bead_model[i].active ? "Y" : "N",
        ...
        if (bead_model[i].bead_computed_radius > tolerance * 1.001 &&
          ...
          if (limit && retval > limit) {
            // editor_msg( "red", us_tr( "There are more than %1 overlaps
            // greater than tolerance of %2\nWe suggest you use ZENO to
            // calculate hydrodynamics for this model" ).arg( limit ).arg(
            // tolerance ) );
            return retval;
          }
          // QColor save_color = editor->textColor();
          // editor->setTextColor("red");
          us_log->log(QString(us_tr("WARNING: Bead model has an overlap "
          ...
          if (us_udp_msg) {
            map<QString, QString> msging;
            msging["_textarea"] =
                QString(us_tr("WARNING: Bead model has an overlap violation on "
                              "beads %1 %2 overlap %3 A\\n"))
                    .arg(i + 1)
                    .arg(j + 1)
                    .arg(separation);
...
#if defined(DEBUG_OVERLAP)
          printf(
              "overlap check  beads %d %d on chains %d %d exposed code %d %d "
              "active %s %s : radii %f(%s) %f(%s) with coordinates [%f,%f,%f] "
              "[%f,%f,%f] sep of %f - needs reduction\n",
...
#if defined(DEBUG_OVERLAP)
  printf(
      "end of overlap checking--overlap tolerance %f--%s-%s-%s--violations "
      "%d------------------\n",
      tolerance, sc ? "side chain" : "", mc ? "main & side chain" : "",
...
int US_Saxs_Util::calc_hydro() { return do_calc_hydro(); }
...
int US_Saxs_Util::do_calc_hydro() {
  ...
  for (unsigned int curr_m = 0; curr_m < model_vector.size(); curr_m++) {
    ...
    if (somo_processed[curr_m]) {
      ...
      //          current_model ) ) ) + QString(bead_model_suffix.length() ?
      //          ("-" + bead_model_suffix) : "") + DOTSOMO, &bead_model,
      //          bead_model_from_file);
      ...
      QString("%1")
          ...
          .arg(
              (fabs((hydro.overlap_cutoff ? hydro.overlap : overlap_tolerance) -
                  ...
                  ? QString(
                        "\nNotice: Overlap reduction bead overlap tolerance %1 "
                        "does not equal the manually selected hydrodynamic "
  ...
  if (retval) {
    ...
    switch (retval) {
      case US_HYDRODYN_SUPC_FILE_NOT_FOUND: {
        // printError("Calculate RB Hydrodynamics SMI encountered a file not
        // found error");
        return retval;
        break;
      }
      case US_HYDRODYN_SUPC_OVERLAPS_EXIST: {
        // printError("Calculate RB Hydrodynamics SMI encountered overlaps in
        // the bead model\nPerhaps you should Calculate RB Hydrodynamics ZENO
        // which supports bead models with overlaps");
        return retval;
        break;
      }
      ...
      default: {
        // printError("Calculate RB Hydrodynamics SMI encountered an unknown
        // error");
        //  unknown error
        return retval;
        break;
...
QString US_Saxs_Util::getExtendedSuffix(bool prerun, bool somo) {
  ...
  if (somo) {
    if (sidechain_overlap.remove_overlap && mainchain_overlap.remove_overlap &&
        ...
        (mainchain_overlap.remove_hierarch == buried_overlap.remove_hierarch)) {
  ...
  if (!somo) {
    if (grid_exposed_overlap.remove_overlap &&
        ...
        (grid_exposed_overlap.remove_sync == grid_buried_overlap.remove_sync) &&
        ...
        (grid_buried_overlap.remove_hierarch == grid_overlap.remove_hierarch)) {
      result +=
          QString("%1").arg(grid_exposed_overlap.remove_sync ? "sy" : "hi");
    }
  ...
  if (!prerun) {
    result +=
        QString(result.length() ? "-" : "") + QString(somo ? "so" : "a2b");
  }
  us_log->log(result);
  if (us_udp_msg) {
    map<QString, QString> msging;
...
QString US_Saxs_Util::getExtendedSuffix_somo(bool prerun, bool somo,
  ...
  if (somo) {
    if (!no_ovlp_removal) {
      if (sidechain_overlap.remove_overlap &&
          mainchain_overlap.remove_overlap && buried_overlap.remove_overlap &&
          (sidechain_overlap.remove_sync == mainchain_overlap.remove_sync) &&
          (mainchain_overlap.remove_sync == buried_overlap.remove_sync) &&
          (sidechain_overlap.remove_hierarch ==
           mainchain_overlap.remove_hierarch) &&
        ...
        result +=
            QString("%1").arg(sidechain_overlap.remove_sync ? "sy" : "hi");
      }
...
void US_Saxs_Util::write_bead_model(QString fname, vector<PDB_atom> *model,
                                    QString extra_text) {
  ...
  QString fstring_beams =
      QString("%.%1f\t%.%2f\t%.%3f\n").arg(decpts).arg(decpts).arg(decpts);
  ...
  if (bead_output.output & US_HYDRODYN_OUTPUT_SOMO) {
    fsomo =
        us_fopen(QString("%1.bead_model").arg(fname).toLatin1().data(), "w");
  }
  ...
  if (fbeams) {
    fprintf(fbeams, "%d\t-2.000000\t%s.rmc\t%.3f\n", beads,
            QFileInfo(fname).fileName().toLatin1().data(), results_hydro.vbar);
  }
    ...
    if (use_model[i]->active) {
      ...
      if (!bead_model_from_file) {
        residues =
            ...
            (use_model[i]->org_chain
                 ? ".SC."
                 : ((misc.pb_rule_on && !use_model[i]->type) ? ".PB."
                                                             : ".MC.")) +
      ...
      if (fsomo) {
        fprintf(fsomo, fstring_somo.toLatin1().data(),
                ...
                use_model[i]->bead_computed_radius, use_model[i]->bead_ref_mw,
                get_color_util_saxs(use_model[i]), residues.toLatin1().data(),
...
void US_Saxs_Util::append_options_log_atob() {
  ...
  s.sprintf(
      ...
      grid_overlap.remove_sync ? "On" : "Off", grid_overlap.remove_sync_percent,
...
void US_Saxs_Util::append_options_log_somo() {
  ...
  s.sprintf(
      ...
      asa_hydro.method ? "Rolling Sphere" : "Voronoi Tesselation",
      asa_hydro.probe_radius, asa_hydro.probe_recheck_radius,
      asa_hydro.threshold, asa_hydro.threshold_percent, asa_hydro.asab1_step);
  options_log += s;
...
void US_Saxs_Util::append_options_log_somo_ovlp() {
  ...
  s.sprintf(
      ...
      asa_hydro.calculation ? "On" : "Off",
      asa_hydro.method ? "Rolling Sphere" : "Voronoi Tesselation",
      asa_hydro.probe_radius, asa_hydro.threshold, asa_hydro.threshold_percent,
      asa_hydro.asab1_step);
  options_log += s;
...
int US_Saxs_Util::calc_somo(bool no_ovlp_removal,
                            bool parameters_set_first_model) {
  // if ( selected_models_contain( "WAT" ) )
  // {
  //    QMessageBox::warning( this,
          ...
          if (us_udp_msg) {
            ...
            msging["progress_output"] =
                QString("SOMO model calculation: %1\%")
                    .arg(QString::number(int((double(ppos) / double(mppos)) *
...
int US_Saxs_Util::compute_asa(bool bd_mode, bool no_ovlp_removal,
                              bool parameters_set_first_model) {
  // cout << "CURRENT_MODEL: " << current_model << endl;
  ...
  if (check_for_missing_atoms_hydro(&error_string, &model_vector[current_model],
                                    use_parameters_set_first_model)) {
  ...
  if (asa_hydro.method == 1) {
    ...
    if (retval) {
      ...
      switch (retval) {
        case US_HYDRODYN_ASAB1_ERR_MEMORY_ALLOC: {
          // printError("US_HYDRODYN_ASAB1 encountered a memory allocation
          // error");
  ...
  // for (unsigned int i = 0; i < model_vector.size (); i++)
  {
    ...
    for (unsigned int j = 0; j < model_vector[i].molecule.size(); j++) {
      for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size();
        ...
        // int)this_atom->p_atom, this_atom->active ? "active" : "not active");
        // fflush(stdout);
    ...
    for (unsigned int j = 0; j < model_vector[i].molecule.size(); j++) {
      ...
      for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size();
        ...
        if (this_atom->active) {
          ...
          // we want to put the N on a previous bead unless it is the first one
          // of the molecule ONLY FOR residue type = 0! (amino acid) AND ONLY
          // for residues not part of the 'auto bead builder'
          ...
          if (!create_beads_normally ||
            ...
            if (create_beads_normally && misc.pb_rule_on && !this_atom->type) {
          ...
          if (!create_beads_normally || this_atom->bead_positioner) {
            ...
            // {
            //    printf("adding cog from %d to %d mw %f totmw %f (this pos
            //    [%f,%f,%f], org pos [%f,%f,%f])\n",
            //           this_atom->serial,
            //           use_atom->serial,
            //           this_atom->mw,
          ...
          //    if ( advanced_config.debug_3 )
          //    {
          //       printf("notice: atom %s %s %d excluded from cog calculation
          //       in bead %s %s %d\n",
          //              this_atom->name.toLatin1().data(),
          //              this_atom->resName.toLatin1().data(),
          //              this_atom->serial,
          ...
          if (!create_beads_normally || this_atom->bead_positioner) {
            if (use_atom->bead_positioner && this_atom->placing_method == 1) {
              fprintf(stderr,
                      "warning: 2 or more positioners in bead %s %s %d\n",
                      use_atom->name.toLatin1().data(),
                      use_atom->resName.toLatin1().data(), use_atom->serial);
            }
  ...
  // for (unsigned int i = 0; i < model_vector.size (); i++)   {
  {
    ...
    for (unsigned int j = 0; j < model_vector[i].molecule.size(); j++) {
      ...
      for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size();
        ...
        // if ( advanced_config.debug_1 )
        // {
        //    printf("pass 2b active %d is_bead %d chain %d atom %s %s %d pm
        //    %d\n",
        //           this_atom->active,
        //           this_atom->is_bead,
        //           this_atom->chain,
        ...
        // }
        if (this_atom->active && this_atom->is_bead && this_atom->chain == 0) {
          main_chain_beads++;
        }
        ...
        if (placed_N1 && broken_chain_end.count(QString("%1|%2")
                                                    .arg(this_atom->resSeq)
                                                    .arg(this_atom->resName))) {
          placed_N1 = false;
          // if ( advanced_config.debug_1 )
        ...
        if (this_atom->name == "OXT" && last_main_chain_bead) {
          // if ( advanced_config.debug_1 )
          // {
          //    printf("pass 2b active OXT %s %s %d last %s %s %d mw org %f mw
          //    new %f\n",
          //           this_atom->name.toLatin1().data(),
          //           this_atom->resName.toLatin1().data(),
          //           this_atom->serial,
        ...
        if (misc.pb_rule_on && this_atom->p_residue->type == 0 &&
            ...
            !broken_chain_head.count(QString("%1|%2")
                                         .arg(this_atom->resSeq)
                                         .arg(this_atom->resName)) &&
            this_atom->resName != "PRO" && (k || this_atom->name != "N")) {
          // if ( advanced_config.debug_1 )
          // {
          //    printf("pass 2b broken end adjustment %s %s %s org mw %.4f vol
          //    %.4f\n",
          //           this_atom->name.toLatin1().data(),
          //           this_atom->resName.toLatin1().data(),
          //           this_atom->resSeq.toLatin1().data(),
          ...
          if (multi_residue_map["PBR-NO-OXT"].size() == 1) {
            ...
            this_atom->bead_computed_radius =
                pow(3 * last_main_chain_bead->bead_ref_volume / (4.0 * M_PI),
                    1.0 / 3);
            if (this_atom->resName == "GLY") {
            ...
            // if ( advanced_config.debug_1 )
            // {
            //    printf("pass 2b broken end adjustment %s %s %s new mw %.4f vol
            //    %.4f\n",
            //           this_atom->name.toLatin1().data(),
            //           this_atom->resName.toLatin1().data(),
            //           this_atom->resSeq.toLatin1().data(),
            ...
            //    editor->setTextColor("red");
            us_log->log(
                "Chain has broken end and PBR-NO-OXT isn't uniquely defined in "
                "the residue file.");
            if (us_udp_msg) {
              map<QString, QString> msging;
              msging["_textarea"] =
                  "Chain has broken end and PBR-NO-OXT isn't uniquely defined "
                  "in the residue file.";
      ...
      if (misc.pb_rule_on && last_main_chain_bead &&
        ...
        if (multi_residue_map["PBR-NO-OXT"].size() == 1) {
          ...
          //    editor->setTextColor("red");
          us_log->log(
              "Chain has no terminating OXT and PBR-NO-OXT isn't uniquely "
              "defined in the residue file.");
          if (us_udp_msg) {
            map<QString, QString> msging;
            msging["_textarea"] =
                "Chain has no terminating OXT and PBR-NO-OXT isn't uniquely "
      ...
      for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size();
        ...
        if (this_atom->active && this_atom->is_bead) {
          ...
          printf("pass 2c hydration %s %s %d pm %d\n",
                 this_atom->name.toLatin1().data(),
                 this_atom->resName.toLatin1().data(), this_atom->serial,
                 this_atom->placing_method);
          fflush(stdout);
#endif
          ...
          this_atom->bead_computed_radius =
              pow(3 * this_atom->bead_ref_volume / (4.0 * M_PI), 1.0 / 3);
        }
      }
    }
  }
...
#warning broken by resSeq->QString
          ...
          if (this_atom->p_residue && this_atom->p_atom) {
#if defined(DEBUG)
            printf(
                "pass 3 active is bead %s %s %d bead assignment %d placing "
                "method %d\n",
                this_atom->name.toLatin1().data(),
                this_atom->resName.toLatin1().data(), this_atom->serial,
                this_atom->bead_assignment, this_atom->placing_method);
            fflush(stdout);
#endif
            switch (this_atom->placing_method) {
              case 0:  // cog
                ...
                // if (this_atom->bead_positioner) {
                // fprintf(stderr, "warning: this bead had a atom claiming
                // position & a bead placing method of cog! %s %s %d\n",
                //   this_atom->name.toLatin1().data(),
                //   this_atom->resName.toLatin1().data(),
                //   this_atom->serial);
                // }
              ...
              default:
                this_atom->bead_coordinate = this_atom->bead_cog_coordinate;
                fprintf(stderr,
                        "warning: unknown bead placing method %d %s %s %d "
                        "<using cog!>\n",
                        this_atom->placing_method,
                        this_atom->name.toLatin1().data(),
                        this_atom->resName.toLatin1().data(),
...
#if defined(DEBUG)
  printf(
      "model~molecule~atom~name~residue~chainID~"
  ...
  // for (unsigned int i = 0; i < model_vector.size (); i++)
  {
    ...
    for (unsigned int j = 0; j < model_vector[i].molecule.size(); j++) {
      for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size();
        ...
        // printf("model %d mol %d atm %d nam %s res %s xyz [%f,%f,%f] act %s
        // rads %f asa %f bead # %d chain %d serl %d is_bead %s bead_asa %f vis
        // %s code %d pos? %s pos_co [%f,%f,%f] cog [%f,%f,%f] use [%f, %f,
        // %f]\n",
        printf(
            "%d~%d~%d~%s~%s~%s~"
            "[%f,%f,%f]~%s~%f~%f~%f~"
            ...
            this_atom->bead_assignment, this_atom->chain, this_atom->serial,
            this_atom->is_bead ? "Y" : "N", this_atom->bead_asa,
        ...
        if (this_atom->active && this_atom->is_bead) {
          ...
          if (bead_count) {
            printf("res %s %s bead %d bead_mw %f sum atom mw %f diff %f\n",
                   last_res.toLatin1().data(), last_resName.toLatin1().data(),
                   last_asgn, prev_bead_mw, prev_atom_mw,
  ...
  printf("res %d %s bead %d bead_mw %f sum atom mw %f diff %f\n", last_res,
         last_resName.toLatin1().data(), last_asgn, prev_bead_mw, prev_atom_mw,
         prev_bead_mw - prev_atom_mw);
  printf("~~tot atom %d mw %f tot bead %d mw %f beads_size %u\n", atom_count,
         tot_atom_mw, bead_count, tot_bead_mw, (unsigned int)bead_model.size());
#endif
    ...
    do {
      ...
      if (methods[k] &
        ...
        if (overlaps_exist) {
          ...
          // if ( !no_ovlp_removal ) {
          //    lbl_core_progress->setText(QString("Stage %1 popping iteration
          //    %2 beads popped %3").arg(k+1).arg(iter).arg(beads_popped));
          // }
          // qApp->processEvents();
          // //#define DEBUG_FUSED
#if defined(DEBUG1) || defined(DEBUG) || defined(DEBUG_FUSED)
        ...
        do {
...
#endif
              radial_reduction_thr_threads[j]->radial_reduction_thr_setup(
                  methods[k], &bead_model, &last_reduced, &reduced,
                  &my_pairs[j], threads, overlap_tolerance);
            }
          ...
          if (max_intersection_length > TOLERANCE) {
            ...
            printf(
                ...
                iter, count);
#endif
            // lbl_core_progress->setText(QString("Stage %1 synchron. red. it.
            // %2 with %3 couples").arg(k+1).arg(iter).arg(pairs.size()));
            // qApp->processEvents();
            for (unsigned int i = 0; i < pairs.size(); i++) {
              if (!reduced[pairs[i].i] &&
                  (bead_model[pairs[i].i].exposed_code != 1 ||
...
bool US_Saxs_Util::read_pdb_hydro(QString filename,
                                  bool parameters_set_first_model) {
  errormsg = "";
  noticemsg = "";
  ...
  if (f.open(QIODevice::ReadOnly)) {
    ...
    while (!ts.atEnd()) {
      ...
      if (str1.left(5) == "MODEL" ||
          (str1.left(4) == "ATOM" &&
           last_was_ENDMDL))  // we have a new model in a multi-model file
      {
        last_was_ENDMDL = false;
        model_flag =
            true;  // we are using model descriptions (possibly multiple models)
        // str2 = str1.mid(6, 15);
          ...
          "ENDMDL")  // we need to save the previously recorded molecule
      {
        ...
        clear_temp_chain(
            &temp_chain);  // we are done with this molecule and can delete it
      }
        ...
        if (str1.mid(12, 1) != "H" && str1.mid(13, 1) != "H" &&
          ...
          if (str1.mid(16, 1) == " " || str1.mid(16, 1) == "A") {
            ...
            {  // we don't have a chain yet, so let's start a new one
              ...
              chain_flag = true;
            } else  // we have a chain, let's make sure the chain is still the
                    // same
            {
              ...
              if (!break_chain && currently_aa_chain && known_residue &&
                  !this_is_aa) {
                break_chain = true;
              }
              if (break_chain) {
                ...
                clear_temp_chain(&temp_chain);
                temp_chain.chainID =
                    str1.mid(21, 1);  // we have to start a new chain
                str2 = str1.mid(72, 4);
                temp_chain.segID = str2.trimmed();
              }
              currently_aa_chain = (!known_residue || this_is_aa);
            }
            if (assign_atom(str1, &temp_chain,
                            &last_resSeq))  // parse the current line and add it
                                            // to temp_chain
            {  // if true, we have new residue and need to add it to the residue
               // vector
              temp_model.residue.push_back(
                  current_residue);  // add the next residue of this model
            }
...
class sortable_uint {
 public:
  unsigned int x;
  bool operator<(const sortable_uint &objIn) const { return x < objIn.x; }
};
...
int US_Saxs_Util::write_pdb_hydro(QString fname, vector<PDB_atom> *model) {
  ...
  for (list<sortable_uint>::const_iterator it = connect_parents.begin();
    ...
    QString tmp = "";
    unsigned int j = 0;
    for (list<sortable_uint>::const_iterator itj = connect_list[it->x].begin();
      ...
      if (respos == -1) {
        if ((this_atom->name != "H" && this_atom->name != "D" &&
          ...
          error_string->append(QString("").sprintf(
              "unknown residue molecule %d atom %d name %s resname %s coord "
              "[%f,%f,%f]\n",
      ...
      if (respos != -1) {
        ...
        for (unsigned int m = 0; m < residue_list[respos].r_atom.size(); m++) {
          ...
          if (this_atom->name == "N" && !k && misc.pb_rule_on) {
            printf(
                "this_atom->name == N/N1 this residue_list[%d].r_atom[%d].name "
                "== %s\n",
                respos, m,
                residue_list[respos].r_atom[m].name.toLatin1().data());
          }
#endif
          if (!residue_list[respos].r_atom[m].tmp_used &&
        ...
        if (atompos == -1) {
          error_string->append(QString("").sprintf(
              "unknown atom molecule %d atom %d name %s resname %s coord "
              "[%f,%f,%f]\n",
              j + 1, k, this_atom->name.toLatin1().data(),
              this_atom->resName.toLatin1().data(),
              this_atom->coordinate.axis[0], this_atom->coordinate.axis[1],
          ...
          this_atom->placing_method =
              this_atom->p_residue->r_bead[this_atom->p_atom->bead_assignment]
                  .placing_method;
          ...
          this_atom->chain =
              (int)this_atom->p_residue->r_bead[this_atom->bead_assignment]
                  .chain;
          this_atom->atom_hydration =
          ...
          //       // QColor save_color = editor->textColor();
          //       // editor->setTextColor("red");
          //        us_log->log(QString("%1Molecule %2 Residue %3|%4 %5 Hybrid
          //        name missing. Atom skipped for SAS.\n")
          //                       .arg(this_atom->chainID == " " ? "" : ("Chain
          //                       " + this_atom->chainID + " ")) .arg(j+1)
          //                       .arg(this_atom->resName)
          //                       .arg(this_atom->name)
          //                       .arg(this_atom->resSeq));
          //        //editor->setTextColor(save_color);
          ...
          //              Hybrid %5 name missing from Hybrid file. Atom skipped
          //              for SAS.\n")
          //                             .arg(this_atom->chainID == " " ? "" :
          //                             ("Chain " + this_atom->chainID + " "))
          //                             .arg(j+1)
          ...
          //              //editor->setTextColor(save_color);
          //           } else {
          //              if ( !saxs_util->atom_map.count(this_atom->name + "~"
          //              + hybrid_name) )
          //              {
          ...
          //                 // QColor save_color = editor->textColor();
          //                 // editor->setTextColor("red");
          //                 us_log->log(QString("%1Molecule %2 Atom %3 Residue
          //                 %4 %5 Hybrid %6 name missing from Atom file. Atom
          //                 skipped for SAS.\n")
          //                                .arg(this_atom->chainID == " " ? ""
          //                                : ("Chain " + this_atom->chainID + "
          //                                ")) .arg(j+1) .arg(this_atom->name)
          //                                .arg(this_atom->resName)
          //                                .arg(this_atom->resSeq)
          //                                .arg(hybrid_name)
          //                                );
          ...
          //                    // editor->setTextColor("red");
          //                    us_log->log(QString("%1Molecule %2 Residue %3 %4
          //                    Hybrid %5 Saxs name %6 name missing from SAXS
          //                    atom file. Atom skipped.\n")
          //                                   .arg(this_atom->chainID == " " ?
          //                                   "" : ("Chain " +
          //                                   this_atom->chainID + " "))
          //                                   .arg(j+1)
          ...
          //      advanced_config.debug_2 )
          // {
          //    printf("found atom %s %s in residue %d pos %d bead asgn %d %d
          //    placing info %d mw %f bead_ref_mw %f hybrid name %s %s ba %d
          //    chain %d\n",
          //           this_atom->name.toLatin1().data(),
          //           this_atom->resName.toLatin1().data(), respos, atompos,
      ...
      if (this_atom->active) {
        ...
        if (this_atom->name != "H" && this_atom->name != "D" &&
          ...
          printf("skipped bound waters & H %s %s rad %f resseq %s\n",
                 this_atom->name.toLatin1().data(),
                 this_atom->resName.toLatin1().data(), this_atom->radius,
                 this_atom->resSeq.toLatin1().data());
          fflush(stdout);
#endif
        }
...
encountered\n"
         //                                   "It can be skipped or you can stop
processing.\n"
...
Therefore, you could manually enter a global\n"
         //                                   "value for the molecular weight in
the SOMO hydrodynamic options, and a\n"
...
atoms, the calculated molecular\n"
         //                                   "weight and vbar may be incorrect,
and you should manually enter\n"
