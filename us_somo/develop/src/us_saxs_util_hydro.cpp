...
operator<< (std::basic_ostream<char> &os, const QString &str)
{
...
bool
US_Saxs_Util::run_hydro (map<QString, QString> &parameters,
                         map<QString, QString> &results)
{
  ...
  if (!set_default (results, parameters)) // setup configuration defaults
                                          // before reading initial config
    return false;
  ...
  if (!fi_res_filename.exists ())
    {
      results["errors"]
          += QString (" file %1 does not exist.").arg (residue_filename);
      return false;
    }
  ...
  if (saxs_options_hydro.wavelength == 0)
    {
      saxs_options_hydro.start_q = saxs_options_hydro.end_q
          = saxs_options_hydro.delta_q = 0;
    }
  else
    {
      saxs_options_hydro.start_q
          = 4.0 * M_PI * sin (saxs_options_hydro.start_angle * M_PI / 360.0)
            / saxs_options_hydro.wavelength;
      saxs_options_hydro.start_q
          = floor (saxs_options_hydro.start_q * SAXS_Q_ROUNDING + 0.5)
            / SAXS_Q_ROUNDING;
      saxs_options_hydro.end_q
          = 4.0 * M_PI * sin (saxs_options_hydro.end_angle * M_PI / 360.0)
            / saxs_options_hydro.wavelength;
      saxs_options_hydro.end_q
          = floor (saxs_options_hydro.end_q * SAXS_Q_ROUNDING + 0.5)
            / SAXS_Q_ROUNDING;
      saxs_options_hydro.delta_q
          = 4.0 * M_PI * sin (saxs_options_hydro.delta_angle * M_PI / 360.0)
            / saxs_options_hydro.wavelength;
      saxs_options_hydro.delta_q
          = floor (saxs_options_hydro.delta_q * SAXS_Q_ROUNDING + 0.5)
            / SAXS_Q_ROUNDING;
    }
  ...
  QString files_try
      = parameters["pdbfile"].replace ('"', "").replace ("\\/", "/");
  parameters["_base_directory"]
      = parameters["_base_directory"].replace ("\\/", "/");
  ...
  if (!fi.exists ())
    {
      results["errors"]
          += QString (" file %1 does not exist.").arg (files_try);
    }
  else
    {
      if (!fi.isReadable ())
        {
          results["errors"] += QString (" file %1 exists but is not readable.")
                                   .arg (files_try);
        }
      else
        {
  ...
  QString model_name_file = parameters["_base_directory"] + QDir::separator ()
                            + list_of_models[0]; // 1st model only
  ...
  if (method == "SMI")
    hydro_name_file
        = hydro_name_file_try.replace (".bead_model", ".hydro_res");
  ...
  QString log_name_file
      = parameters["_base_directory"] + QDir::separator () + "runlog.txt";

  QString bead_pdb_name_file = model_name_file;
  bead_pdb_name_file.replace (".bead_model", ".pdb");
  us_qdebug (bead_pdb_name_file);

  QString orig_pdb_name_file
      = parameters["_base_directory"] + QDir::separator () + project + ".pdb";
  ...
  results["bead_pdb"] = "{\"file\":\"" + bead_pdb_name_file
                        + "\",\"script\":\"" + pdb_jsmol_script (&bead_model)
                        + "\"}";
  ...
  if (method != "None")
    {
      if (fabs (results_hydro.total_beads_sd) <= 1e-100)
        ...
        {
          results["total_beads"] = (QString ("").sprintf (
              "%u (%4.2e)", (int)(results_hydro.total_beads + .5),
      ...
      if (fabs (results_hydro.used_beads_sd) <= 1e-100)
        ...
        {
          results["used_beads"] = (QString ("").sprintf (
              "%u (%4.2e)", (int)(results_hydro.used_beads + .5),
      ...
      if (fabs (results_hydro.s20w_sd) <= 1e-100)
        {
          results["s20w"]
              = QString::number (results_hydro.s20w, 'e', 2) + " S";
        }
      else
        {
          results["s20w"] = (QString ("").sprintf (
              "%4.2e S (%4.2e)", results_hydro.s20w, results_hydro.s20w_sd));
        }

      if (fabs (results_hydro.D20w_sd) <= 1e-100)
        {
          results["D20w"]
              = QString::number (results_hydro.D20w, 'e', 2) + " cm^2/sec";
        }
      else
        {
          results["D20w"] = (QString ("").sprintf ("%4.2e cm^2/sec (%4.2e)",
                                                   results_hydro.D20w,
                                                   results_hydro.D20w_sd));
        }
      ...
      if (fabs (results_hydro.rs_sd) <= 1e-100)
        ...
        {
          results["rs"] = (QString ("").sprintf (
              "%4.2e nm (%4.2e)", results_hydro.rs, results_hydro.rs_sd));
        }
      ...
      if (fabs (results_hydro.ff0_sd) <= 1e-100)
        ...
        {
          results["ff0"] = (QString ("").sprintf (
              "%3.2f nm (%3.2e)", results_hydro.ff0, results_hydro.ff0_sd));
        }
      ...
      if (fabs (results_hydro.rg_sd) <= 1e-100)
        ...
        {
          results["rg"] = (QString ("").sprintf (
              "%4.2e nm (%4.2e)", results_hydro.rg, results_hydro.rg_sd));
        }
      ...
      if (method == "Zeno")
      ...
      else
        {
          if (fabs (results_hydro.tau_sd) <= 1e-100)
            {
              results["tau"]
                  = QString::number (results_hydro.tau, 'e', 2) + " ns";
            }
          else
            {
              results["tau"] = (QString ("").sprintf ("%4.2e ns (%4.2e)",
                                                      results_hydro.tau,
                                                      results_hydro.tau_sd));
            }
        }
      ...
      if (fabs (results_hydro.viscosity_sd) <= 1e-100)
        {
          results["viscosity"]
              = QString::number (results_hydro.viscosity, 'e', 2) + " cm^3/g";
        }
      else
        {
          results["viscosity"] = (QString ("").sprintf (
              "%4.2e cm^3/g (%4.2e)", results_hydro.viscosity,
              results_hydro.viscosity_sd));
        }
  ...
  // QString method = QString("%1").arg(((parameters.count( "atob" )) ? "AtoB
  // model" : ( (parameters.count( "zeno" )) ? "Zeno model" : "AtoB model")) );

  // QString output = "Hydrodynamic Calculations";

  // results[ "hydro_output" ] += output + " (" + method + "):" + "<br>"
  //   "<br><tr><td> total_beads  </tr></td>"  +
  //   QString::number(results_hydro.total_beads) +
  //   "<br><tr><td> used_beads  </tr></td>"  +
  //   QString::number(results_hydro.used_beads)  +
  //   "<br><tr><td> mass  </tr></td>"  + QString::number(results_hydro.mass) +
  //   "<br><tr><td> Sedimentaiton   </tr></td>"  +
  //   QString::number(results_hydro.s20w) +
  //   "<br><tr><td> Diffusion  </tr></td>"  +
  //   QString::number(results_hydro.D20w) +
  //   "<br><tr><td> viscosity  </tr></td>"  +
  //   QString::number(results_hydro.viscosity) +
  //   "<br><tr><td> ff0  </tr></td>"  + QString::number(results_hydro.ff0) +
  //   "<br>";
...
void
US_Saxs_Util::read_residue_file ()
{
  QString str1, str2;
      ...
      while (!ts.atEnd ())
        {
          ...
          for (j = 0; j < numatoms; j++)
            {
              ...
              if (new_atom.bead_assignment >= numbeads)
                {
                  error_count++;
                  QString tmp_msg
                      = us_tr (QString ("\nThe atom's bead assignment has "
                                        "exceeded the number of beads.\n"
              ...
              line_count++;
              if (!new_atom.name.isEmpty () && new_atom.hybrid.radius > 0.0
                  && new_atom.hybrid.mw > 0.0)
                {
                  residue_atom_hybrid_map[QString ("%1|%2")
                                              .arg (new_residue.name)
                                              .arg (new_atom.name)]
                      = new_atom.hybrid.name;
                  residue_atom_abb_hybrid_map[new_atom.name]
                      = new_atom.hybrid.name;
                  ...
                  if (new_residue.name.contains (QRegExp ("^PBR-")))
                    {
                      pbr_override_map[QString ("%1|%2|%3|%4")
                                           .arg (new_residue.name == "PBR-P"
                                                     ? "P"
                                                     : "NP")
                                           .arg (new_atom.name)
                                           .arg (new_atom.hybrid.name)
                                           .arg (new_atom.bead_assignment)]
                          = new_atom.positioner;
                    }
                }
              // else
              ...
              //                         File!\n\nAtom "
              //                            + new_atom.name + " cannot be read
              //                            and will be deleted from List."),
              //                         QMessageBox::Ok,
          ...
          for (j = 0; j < numbeads; j++)
            {
              ...
              // if ( advanced_config.debug_1 )
              // {
              //    printf("residue name %s loading bead %d placing method
              //    %d\n",
          ...
          if (!new_residue.name.isEmpty () && new_residue.molvol > 0.0
              && new_residue.asa > 0.0)
            {
              new_residue.unique_name
                  = QString ("%1%2")
                        .arg (new_residue.name)
              ...
              // if ( advanced_config.debug_1 )
              // {
              //    printf("residue name %s unique name %s atom size %u alt
              //    size %u pos %u\n"
              //           ,new_residue.name.toLatin1().data()
              ...
              //           ); fflush(stdout);
              // }
              multi_residue_map[new_residue.name].push_back (
                  residue_list.size ());
              ...
              for (unsigned int k = 0; k < new_residue.r_atom.size (); k++)
                {
                  QString idx
                      = QString ("%1|%2|%3")
                            ...
                            .arg (multi_residue_map[new_residue.name].size ()
                                  - 1);
  ...
  if (multi_residue_map["OXT"].size () == 1
    ...
    {
      error_count++;
      QString msg = "Warning: PBR rule OXT not replaced with OXT-P because "
                    "there is not exactly 1 OXT and 1 PBR-OXT\n";
      error_text += msg;
      error_msg += msg;
    }
  ...
  //    {
  //       editor_msg( "red", QString( us_tr( "Error: can not create MSROLL
  //       radii file: %1" ) ).arg( f_radii.fileName() ) );
  //    } else {
  //       QTextStream ts( &f_radii );
  //       for ( unsigned int i = 0; i < (unsigned int) msroll_radii.size();
  //       i++ )
  //       {
  //          ts << msroll_radii[ i ];
  //       }
  //       f_radii.close();
  //       editor_msg( "blue", QString( us_tr( "Notice: created MSROLL radii
  //       file: %1" ) ).arg( f_radii.fileName() ) );
  //    }
  ...
  //    {
  //       editor_msg( "red", QString( us_tr( "Error: can not create MSROLL
  //       names file: %1" ) ).arg( f_names.fileName() ) );
  //    } else {
  //       QTextStream ts( &f_names );
  //       for ( unsigned int i = 0; i < (unsigned int) msroll_names.size();
  //       i++ )
  //       {
  //          ts << msroll_names[ i ];
  //       }
  //       f_names.close();
  //       editor_msg( "blue", QString( us_tr( "Notice: created MSROLL names
  //       file: %1" ) ).arg( f_names.fileName() ) );
  //    }
  // }
}

bool
US_Saxs_Util::screen_pdb (
    QString filename,
    bool
        parameters_set_first_model) //, bool display_pdb, bool skipclearissue )
{
  // if ( !skipclearissue ) {
  issue_info.clear ();
  // }
  ...
  // {
  //    printError(us_tr("Filenames containing spaces are not currently
  //    supported.\n"
  ...
  // #endif
  // #if defined(BIN64)
  //       argument.append(USglobal->config_list.system_dir + SLASH + "bin64" +
  //       SLASH + "rasmol");
  // #else
  //       argument.append(USglobal->config_list.system_dir + SLASH + "bin" +
  //       SLASH + "rasmol");
  // #endif
  ...
  //           !rasmol->start())
  //       {
  //          US_Static::us_message(us_tr("Please note:"), us_tr("There was a
  //          problem starting RASMOL\n"
  ...
  for (unsigned int i = 0; i < model_vector.size (); i++)
    {
      ...
      us_log->log (
          QString ("Checking the pdb structure for model %1\n").arg (i + 1));
      ...
      // qDebug() << "screen_pdb:: 4";
      if (check_for_missing_atoms_hydro (&error_string, &model_vector[i],
                                         parameters_set_first_model))
        {
          ...
          us_log->log (
              QString (
                  "Encountered errors with your PDB structure for model %1:\n")
                  .arg (i)
              + error_string);
          ...
          if (us_udp_msg)
            {
              map<QString, QString> msging;
              msging["_textarea"]
                  = QString (QString ("Encountered errors with your PDB "
                                      "structure for model %1:\\n")
                                 .arg (i)
                             + error_string_udp);
              us_udp_msg->send_json (msging);
              // sleep(2);
          ...
          // printError(QString("Encountered errors with your PDB structure for
          // model %1:\n").
          //            arg( model_name( i ) ) + "please check the text
          //            window");
        }
  ...
  if (!model_vector.size () || !model_vector[0].molecule.size ())
    {
...
int
US_Saxs_Util::check_for_missing_atoms_hydro (QString *error_string,
                                             PDB_model *model,
                                             bool parameters_set_first_model)
{
      ...
      for (unsigned int k = 0; k < model->molecule[j].atom.size (); k++)
        {
          ...
          QString count_idx = QString ("%1|%2|%3")
                                  .arg (j)
                                  .arg (this_atom->resName)
                                  .arg (this_atom->resSeq);

          QString res_idx = QString ("%1|%2").arg (j).arg (this_atom->resSeq);
          ...
          if (multi_residue_map.count (this_atom->resName)
            ...
            {
              restype = residue_list[multi_residue_map[this_atom->resName][0]]
                            .type;
            }
          ...
          for (unsigned int m = 0; m < residue_list.size (); m++)
            {
              if (residue_list[m].name == this_atom->resName
                  && skip_residue.count (
                      QString ("%1|%2").arg (res_idx).arg (m))
                  && skip_residue[QString ("%1|%2").arg (res_idx).arg (m)]
                         == true)
                {
                  // if ( advanced_config.debug_1 )
                  // {
                  // cout <<
                  //  QString("(check for missing atoms) skipping %1
              ...
              if ((residue_list[m].name == this_atom->resName &&
                   ...
                   this_atom->name != "OXT"
                   && (k || this_atom->name != "N" || restype != 0
                       || broken_chain_head.count (
                           QString ("%1|%2")
                               .arg (this_atom->resSeq)
                               .arg (this_atom->resName))
                       || this_atom->resName == "PRO" || !misc.pb_rule_on))
                  ||

                  // if pb_rule is off, final OXT before P needs to use OXT-P

                  (residue_list[m].name == "OXT" && this_atom->name == "OXT"
                   && (misc.pb_rule_on || this_atom->resName != "PRO"))
                  || (residue_list[m].name == "OXT-P"
                      && (this_atom->name == "OXT" && !misc.pb_rule_on
                          && this_atom->resName == "PRO"))
                  ||

                  (!k && this_atom->name == "N" && restype == 0
                   && misc.pb_rule_on && residue_list[m].name == "N1"
                   ...
                   && this_atom->resName != "PRO"))
                {
                  ...
                  cout << k << " " << this_atom->p_residue->unique_name
                       << endl;
                  // cout << (int)residue_list[m].r_atom.size() << " " <<
                  // atom_counts[count_idx] - has_OXT[count_idx]  << endl; if (
                  // advanced_config.debug_1 )
                  // {

                  //    printf("residue match %d resName %s \n", m,
                  //    residue_list[m].name.toLatin1().data());
                  //    printf("resname %s respos %d mappos %d mapsize %u\n"
                  //           ,this_atom->resName.toLatin1().data()
                  //           ,m
                  //           ,multi_residue_map[this_atom->resName][0]
                  //           ,(unsigned
                  //           int)multi_residue_map[this_atom->resName].size()
                  //           );
                  // }
                  if (lastResSeq != this_atom->resSeq)
                    {
                      ...
                      if (lastResPos != -1)
                        {
                          ...
                          //  check for false entries in last residue and warn
                          //  about them
                          for (unsigned int l = 0;
                               l < residue_list[lastResPos].r_atom.size ();
                               l++)
                            {
                              // cout << "Inside missing atom 1aa" << endl;
                              if (spec_N1 && misc.pb_rule_on
                                  && residue_list[lastResPos].r_atom[l].name
                                         == "N")
                                {
                                  ...
                                  if (!error_shown[last_count_idx])
                                    {
                                          ...
                                          = QString ("%1Molecule %2 Residue "
                                                ...
                                                .arg (lastChainID == " "
                                                          ...
                                                          : ("Chain "
                                                             ...
                                                             + " "))
                                                .arg (j + 1)
                                                .arg (residue_list[lastResPos]
                                                          .name)
                                                .arg (lastResSeq);
                                      QString idx
                                          = QString ("%1|%2").arg (j).arg (
                                              lastResSeq);
                                      ...
                                      switch (molecules_residue_errors[idx]
                                                  .size ())
                                        {
                                        case 0:
                                          this_error
                                              += QString ("Missing atom %1.\n")
                                                     .arg (residue_list
                                                               [lastResPos]
                                                                   .r_atom[l]
                                                                   .name);
                                      ...
                                      // cout << QString("dbg 1: idx <%1> msg
                                      // <%2>\n").arg(last_count_idx).arg(this_error);
                                      error_shown[last_count_idx] = true;
                                    }
                                  // error_string->
                                  // append(QString("").
                                  //     sprintf("Missing atom: chain %s
                                  //     molecule %d atom %s residue %s %s\n",
                      ...
                      // m);
                      for (unsigned int l = 0;
                           l < residue_list[m].r_atom.size (); l++)
                        {
          ...
          if (respos == -1)
            {
              if ((this_atom->name != "H" && this_atom->name != "D"
                   ...
                   && this_atom->resName != "HOH"
                   && (this_atom->altLoc == "A" || this_atom->altLoc == " ")))
                {
                  QString msg_tag;
                  ...
                  if (pdb_parse.missing_residues == 0
                  ...
                  else
                    {
                      ...
                      //    2.1 there must be a missing atom since count
                      //    doesn't match so atom controls
                      // 3. residue does not exist
                      //    3.1 skip missing residue control
                      // ---------------------
                      //
                      // note: we're just checking the 1st of multiple
                      // possibilities
                      puts ("cases---");
                      if (multi_residue_map[this_atom->resName].size ())
                        {
                          ...
                          if (valid_atom_map[idx].size ())
                            ...
                            {
                              // atom does not exist, skip missing atoms
                              // controls
                              puts ("case 1.1");
                          ...
                          //    if (
                          //    molecules_residue_min_missing[QString("%1|%2").arg(j).arg(this_atom->resSeq)]
                          //    == -1 ) {
                          ...
                          if (pdb_parse.missing_atoms == 2)
                            {
                              if (molecules_residue_min_missing
                                      [QString ("%1|%2").arg (j).arg (
                                          this_atom->resSeq)]
                                  == -1)
                                {
                                  ...
                                  switch (pdb_parse.missing_residues)
                                    ...
                                    default:
                                      failure_errors++;
                                      // US_Static::us_message(us_tr("Internal
                                      // error:"),
                                      //                      us_tr("Unexpected
                                      //                      pdb_parse.missing
                                      //                      residues type ")
                        ...
                        {
                          // residue does not exist, skip missing residue
                          // controls
                  ...
                  if (do_error_msg)
                    {
                      ...
                      if (!error_shown[count_idx])
                        {
                          QString this_error
                              = QString ("%1Molecule %2 Residue %3 %4: ")
                                    .arg (this_atom->chainID == " "
                                              ? ""
                                              : ("Chain " + this_atom->chainID
                                                 + " "))
                                    .arg (j + 1)
                                    .arg (this_atom->resName)
                                    .arg (this_atom->resSeq);
                          QString idx = QString ("%1|%2").arg (j).arg (
                              this_atom->resSeq);
                          switch (molecules_residue_errors[idx].size ())
                            ...
                            case 1:
                              this_error
                                  += molecules_residue_errors[idx][0] + "\n";
                              break;
                            default:
                              {
                                this_error += "\n";
                                for (unsigned int t = 0;
                                     t < molecules_residue_errors[idx].size ();
                                     t++)
                                  {
                                    // if ( advanced_config.debug_1 )
                                    // {
                                    ...
                                    // }
                                    this_error
                                        += QString ("    Residue file entry "
                                                    "%1: %2\n")
                                               .arg (t + 1)
                                               .arg (molecules_residue_errors
                                                         [idx][t]);
                                  }
                              }
                              break;
                          ...
                          error_string->append (this_error);
                          // cout << QString("dbg 2: idx <%1> msg
                          // <%2>\n").arg(count_idx).arg(this_error);
                          error_shown[count_idx] = true;
                        }

                      // error_string->append(QString("").sprintf("%s: chain %s
                      // molecule %d atom %s residue %s %s\n",
          ...
          else
            {
              ...
              // }
              for (unsigned int m = 0; m < residue_list[respos].r_atom.size ();
                   m++)
                {
                  if (residue_list[respos].r_atom[m].name == this_atom->name
                      || (this_atom->name == "N" && misc.pb_rule_on && !k
              ...
              if (atompos == -1)
                {
                  ...
                  switch (pdb_parse.missing_atoms)
                    ...
                    default:
                      ...
                      //                      us_tr("Unexpected
                      //                      pdb_parse.missing residues type
                      //                      ") +
                      //                      QString("%1\n").arg(pdb_parse.missing_residues));
                      exit (-1);
                      break;
                  ...
                  if (!error_shown[count_idx])
                    {
                      QString this_error
                          = QString ("%1Molecule %2 Residue %3 %4: ")
                                ...
                                .arg (this_atom->resSeq);
                      QString idx
                          = QString ("%1|%2").arg (j).arg (this_atom->resSeq);
                      switch (molecules_residue_errors[idx].size ())
                        {
                        case 0:
                        ...
                        default:
                          {
                            ...
                            for (unsigned int t = 0;
                                 ...
                                 t++)
                              {
                                ...
                                this_error
                                    += QString (
                                           "    Residue file entry %1: %2\n")
                                           .arg (t + 1)
                                           .arg (molecules_residue_errors[idx]
                                                                         [t]);
                              }
                          }
                          break;
                  ...
                  // error_string->append(QString("").sprintf("unknown atom
                  // chain %s molecule %d atom %s residue %s %s\n",
                  //            this_atom->chainID.toLatin1().data(),
                  //            j + 1,
                  //            this_atom->name.toLatin1().data(),
                  //            this_atom->resSeq.toLatin1().data(),
        ...
        } // end for ( unsigned int k = 0; k < model->molecule[j].atom.size();
          // k++ )
      if (lastResPos != -1)
        {
          // check for false entries in last residue and warn about them
          // printf("check for false entries in last residue %d, residues_found
          // %d, N1s_placed %d\n", lastResPos, residues_found, N1s_placed);
          for (unsigned int l = 0; l < residue_list[lastResPos].r_atom.size ();
               l++)
            {
              if (!residue_list[lastResPos].r_atom[l].tmp_flag
                  && !(misc.pb_rule_on && residues_found == 1
                       && N1s_placed == 1))
                {

                  residue_errors[lastOrgChainID + "~" + lastOrgResName + " "
                                 + lastOrgResSeq]
                      = true;
                  errors_found++;
                  QString count_idx
                  ...
                  // if ( advanced_config.debug_1 )
                  // {
                  //    printf("dbg x pdb_parse.missing_atoms %d
                  //    bead_exceptions[%s] %d
                  //    residue_list[%d].r_atom[%d].tmp_flag %d\n"
                  ...
                  if (!error_shown[count_idx])
                    {
                      ...
                      error_string->append (this_error);
                      // cout << QString("dbg 4: idx <%1> msg
                      // <%2>\n").arg(count_idx).arg(this_error);
                      error_shown[count_idx] = true;
                    }
                  // error_string->
                  //  append(QString("").
                  // sprintf("missing atom chain %s molecule %d atom %s residue
                  // %s %s\n",
                  //    lastChainID.toLatin1().data(),
                  //    j + 1,
                  //    residue_list[lastResPos].r_atom[l].name.toLatin1().data(),
  ...
  if (error_string->length ())
    {
      ...
      // to remove from unknown those that have 2 beads and a correct main
      // chain
      ...
      if (failure_errors > 0)
      ...
      else
        {
          ...
          // cout << "Encountered the following warnings with your PDB
          // structure:\n" + *error_string << endl;
          us_log->log (
          ...
          if (us_udp_msg)
            {
              map<QString, QString> msging;
              msging["_textarea"] = "Encountered the following warnings with "
                                    "your PDB structure:\\n"
                                    + error_string_udp;
              us_udp_msg->send_json (msging);
              // sleep(1);
            }
          accumulated_msgs += "Encountered the following warnings with your "
                              "PDB structure:\\n"
                              + error_string_udp;
          ...
          *error_string = "";
          ...
          for (unsigned int j = 0; j < org_model.molecule.size (); j++)
            {
              ...
              bool auto_bb_aa
                  = false; // are we doing special amino acid handling?
              map<QString, int>
                  aa_main_chain; // to make sure we have a good main chain
              int current_bead_assignment = 0;
              bool one_bead = false; // fall back to one bead for missing atoms
              if (last_residue_type[j] == 0 && residue_types[j].size () == 1)
                {
              ...
              for (unsigned int k = 0; k < org_model.molecule[j].atom.size ();
                   k++)
                {
                  QString count_idx
                      = QString ("%1|%2|%3")
                  ...
                  cout << count_idx << " " << bead_exceptions[count_idx]
                       << endl;
                  ...
                  switch (bead_exceptions[count_idx])
                      ...
                      if (org_model.molecule[j].atom[k].resSeq != lastResSeq)
                        {
                          ...
                          model->residue.push_back (
                              residue_list[multi_residue_map[lastResName][0]]);

                          lastOrgResSeq
                              = org_model.molecule[j].atom[k].orgResSeq;
                          lastOrgChainID
                              = org_model.molecule[j].atom[k].orgChainID;
                          lastOrgResName
                              = org_model.molecule[j].atom[k].orgResName;
                        }
                      break;
                    case 2:
                    case 5: // extra or non-coded atom
                      {
                        ...
                        if (!abb_msg_done[count_idx])
                          {
                            abb_msgs += QString ("ABB: %1Molecule %2 Residue "
                                                 "%3 %4 Skipped\n")
                                            .arg (this_atom->chainID == " "
                                                      ? ""
                                                      : ("Chain "
                            ...
                            abb_msgs_udp
                                += QString ("ABB: %1Molecule %2 Residue %3 %4 "
                                            "Skipped\\n")
                                       .arg (this_atom->chainID == " "
                                                 ? ""
                                                 : ("Chain "
                    ...
                    case 3:
                      {
                        ...
                        PDB_atom *this_atom = &org_model.molecule[j].atom[k];
                        QString new_residue_idx
                            = this_atom->resName; // we could add atom_count to
                                                  // the idx for counting by
                                                  // unique atom counts...
                        // if ( advanced_config.debug_2  )
                        // {
                        //    printf("1.0 <%s> residue types %u, last_type
                        //    %u\n",
                        ...
                        if (this_atom->resSeq != lastResSeq)
                          {
                            ...
                            lastResSeq = org_model.molecule[j].atom[k].resSeq;
                            lastResName
                                = org_model.molecule[j].atom[k].resName;
                            ...
                            // each instance of the residue gets a unique name,
                            // so we don't have to worry about duplicates and
                            // alternate duplicates
                            new_residues[new_residue_idx]++;
                            new_residue_name
                                = QString ("%1_NC%2")
                                      .arg (this_atom->resName)
                                      .arg (new_residues[new_residue_idx]);
                            unknown_residues[new_residue_name] = true;
                            abb_msgs
                                += QString ("ABB: %1Molecule %2 Residue %3 %4 "
                                            "Non-coded residue %5 created.\n")
                                       .arg (this_atom->chainID == " "
                                                 ? ""
                                                 : ("Chain "
                                                    + this_atom->chainID
                            ...
                            new_residue.r_bead.clear ();
                            new_bead.hydration
                                = (unsigned int)(misc.avg_hydration
                                                     * atom_counts[count_idx]
                                                 + .5);
                            ...
                            new_bead.volume
                                = misc.avg_volume * atom_counts[count_idx];
                            new_bead.mw
                                = misc.avg_mass * atom_counts[count_idx];
                            new_residue.r_bead.push_back (new_bead);
                            multi_residue_map[new_residue.name].push_back (
                                residue_list.size ());
                            residue_list.push_back (new_residue);
                            ...
                            model->residue.push_back (
                                residue_list
                                    [multi_residue_map[new_residue.name][0]]);
                            lastOrgResSeq
                                = org_model.molecule[j].atom[k].orgResSeq;
                            lastOrgChainID
                                = org_model.molecule[j].atom[k].orgChainID;
                            ...
                            if (new_residue_name != new_residue.name)
                              {
                                // US_Static::us_message(us_tr("Internal
                                // error:"),
                                //                      us_tr("Unexpected new
                                //                      residue name
                                //                      inconsistancy.\n") +
                                //                      QString("\"%1\" !=
                                //                      \"%2\"").arg(new_residue_name).arg(new_residue.name));
                                exit (-1);
                              }
                          }
                        // the new residue should always be the last one, since
                        // we just added it
                                ...
                                if (residue_list[respos].r_atom.size () == 3)
                                  {
                                    ...
                                    if (aa_main_chain["N"] == 1
                                        ...
                                        && aa_main_chain["O"] == 1)
                                      {
                                        ...
                                        residue_list[respos].r_bead[0].chain
                                            = 0; // main chain
                                      }
                                  }
                              }
                                    ...
                                    if (aa_main_chain["N"] == 1
                                        ...
                                        && aa_main_chain["O"] == 1)
                                      {
                                        ...
                                        abb_msgs
                                            += QString (
                                                   "ABB: %1Molecule %2 "
                                                   "Residue %3 %4 Non-coded "
                                                   "residue %5 PB found, 2 "
                                                   "beads created.\n")
                                                   .arg (this_atom->chainID
                                                                 == " "
                                                             ? ""
                                        ...
                                        residue_list[respos].r_bead[0].chain
                                            = 0; // main chain
                                        residue_list[respos].r_bead[0].volume
                                            = misc.avg_volume * 4;
                                        residue_list[respos].r_bead[0].mw
                                            = misc.avg_mass * 4;

                                        // create a 2nd bead
                                        residue_list[respos].r_bead.push_back (
                                            residue_list[respos].r_bead[0]);
                                        residue_list[respos]
                                            .r_bead[1]
                                            .hydration
                                            = (unsigned int)(misc.avg_hydration
                                                                 * (atom_counts
                                                                    ...
                                                                    - 4)
                                                             + .5);
                                        residue_list[respos].r_bead[1].chain
                                            = 1; // side chain
                                        residue_list[respos].r_bead[1].volume
                                            = misc.avg_volume
                                              * (atom_counts[count_idx] - 4);
                        ...
                        new_atom.serial_number
                            = residue_list[respos].r_atom.size ();
                        ...
                        //    new_residue.name.toLatin1().data(),
                        //    atom_to_add.name.toLatin1().data(), respos);
                        // }
                        tmp_chain.atom.push_back (atom_to_add);
                        model->residue[model->residue.size () - 1]
                            = residue_list[residue_list.size () - 1];
                        {
                          QString mapkey = QString ("%1|%2")
                                               .arg (atom_to_add.resName)
                                               .arg (atom_to_add.name);
                          if (atom_to_add.name == "OXT")
                            {
                              mapkey = "OXT|OXT";
                            }
                          if (!residue_atom_hybrid_map.count (mapkey)
                              && residue_atom_abb_hybrid_map.count (
                                  atom_to_add.name))
                            {
                              residue_atom_hybrid_map[mapkey]
                                  = residue_atom_abb_hybrid_map[atom_to_add
                                                                    .name];
                              // us_qdebug( QString( "hybrid map created for %1
                              // created as %2" ).arg( mapkey ).arg(
                              // residue_atom_abb_hybrid_map[atom_to_add.name ]
                              // ) );
                            }
                        }
                      }
                    ...
                    case 4: // missing atoms
                      {
                        PDB_atom *this_atom = &org_model.molecule[j].atom[k];
                        QString idx = QString ("%1|%2").arg (j).arg (
                            org_model.molecule[j].atom[k].resSeq);
                        QString new_residue_idx = this_atom->resName;
                        int pos = molecules_residue_min_missing[idx];
                        if (pos < 0)
                          {
                            // cout << QString("Internal error: idx %1 Missing
                            // atom error %2 ! < 0!\n")
                            //   .arg(idx)
                            //   .arg(pos);
                            exit (-1);
                          }
                        // cout << QString("ta->resseq %1, pos
                        // %2\n").arg(this_atom->resName).arg(pos);

                        int orgrespos
                            = multi_residue_map[this_atom->resName][pos];
                        if (this_atom->resSeq != lastResSeq)
                          {
                            // new residue to create with existing atoms, but
                            // increased values... possibly for a multi-bead
                            // residue
                            one_bead = false;
                            current_bead_assignment = 0;
                            lastResSeq = org_model.molecule[j].atom[k].resSeq;
                            ...
                            // printf("orgrespos %d\n", orgrespos);
                            beads_missing_atom_count.resize (
                                residue_list[orgrespos].r_bead.size ());

                            for (unsigned int i = 0;
                                 i < beads_missing_atom_count.size (); i++)
                              {
                                beads_missing_atom_count[i] = 0;
                              }
                            // accumulate missing atom count for each bead
                            for (unsigned int i = 0;
                                 i < molecules_residue_missing_atoms[idx][pos]
                                         .size ();
                                 i++)
                              {
                                beads_missing_atom_count
                                    [molecules_residue_missing_atoms_beads
                                         [idx][pos][i]]++;
                              }
                            // do any of these leave an empty bead?
                            // first, we need to know how many atoms are in
                            // each bead...
                            vector<int> atoms_per_bead;
                            atoms_per_bead.resize (
                                residue_list[orgrespos].r_bead.size ());
                            for (unsigned int i = 0;
                                 i < atoms_per_bead.size (); i++)
                              {
                                atoms_per_bead[i] = 0;
                              }
                            for (unsigned int i = 0;
                                 i < residue_list[orgrespos].r_atom.size ();
                                 i++)
                              {
                                atoms_per_bead[residue_list[orgrespos]
                                                   .r_atom[i]
                                                   .bead_assignment]++;
                              }
                            ...
                            for (unsigned int i = 0;
                              ...
                              {
                                if (atoms_per_bead[i]
                                        - beads_missing_atom_count[i]
                                    < 1)
                                  {
                                    // ugh, fall back to one bead!
                                    one_bead = true;
                                    // printf("notice: fallback to one bead for
                                    // missing atom\n");
                                  }
                              }

                            vector<double>
                                atoms_scale_weight; // multiplier for atoms
                                                    // weight
                            vector<double>
                                atoms_scale_radius; // multiplier for atoms
                                                    // radii
                            atoms_scale_weight.resize (
                                residue_list[orgrespos].r_bead.size ());
                            atoms_scale_radius.resize (
                                residue_list[orgrespos].r_bead.size ());
                            if (one_bead)
                              {
                                // sum molecular weight & radii cubed for total
                                // bead & bead sans missing
                                double tot_mw = 0;
                                ...
                                for (unsigned int i = 0;
                                     i
                                     < residue_list[orgrespos].r_atom.size ();
                                     i++)
                                  {
                                    tot_mw += residue_list[orgrespos]
                                                  .r_atom[i]
                                                  .hybrid.mw;
                                    tot_radii3 += pow (residue_list[orgrespos]
                                                           .r_atom[i]
                                                           .hybrid.radius,
                                                       3);
                                    if (!molecules_residue_missing_atoms_skip
                                            [QString ("%1|%2|%3")
                                                 .arg (idx)
                                                 .arg (pos)
                                                 .arg (i)])
                                      {
                                        tot_mw_missing
                                            += residue_list[orgrespos]
                                                   .r_atom[i]
                                                   .hybrid.mw;
                                        tot_radii3_missing
                                            += pow (residue_list[orgrespos]
                                                        .r_atom[i]
                                                        .hybrid.radius,
                                                    3);
                                      }
                                ...
                                atoms_scale_radius[0]
                                    = pow (tot_radii3 / tot_radii3_missing,
                                           1.0 / 3.0);
                              }
                            else
                              {
                                     ...
                                     i++)
                                  {
                                    tot_mw[residue_list[orgrespos]
                                        ...
                                        += residue_list[orgrespos]
                                               .r_atom[i]
                                               .hybrid.mw;
                                    tot_radii3[residue_list[orgrespos]
                                                   .r_atom[i]
                                                   .bead_assignment]
                                        += pow (residue_list[orgrespos]
                                                    .r_atom[i]
                                    ...
                                    if (!molecules_residue_missing_atoms_skip
                                            [QString ("%1|%2|%3")
                                                 ...
                                                 .arg (i)])
                                      {
                                        tot_mw_missing[residue_list[orgrespos]
                                                           .r_atom[i]
                                                           .bead_assignment]
                                            += residue_list[orgrespos]
                                                   .r_atom[i]
                                ...
                                for (unsigned int i = 0;
                                     i < beads_missing_atom_count.size (); i++)
                                  {
                                    ...
                                    atoms_scale_radius[i] = pow (
                                        tot_radii3[i] / tot_radii3_missing[i],
                                        1.0 / 3.0);
                                  }
                              }
                            ...
                            if (residue_list[orgrespos].type == 0) // aa
                              {
                                unknown_aa_residues_to_check.push_back (
                                    new_residue_name);
                              }

                            abb_msgs
                                += QString ("ABB: %1Molecule %2 Residue %3 %4 "
                                            "Missing atom residue copy %5 "
                                            "created%6.\n")
                                       .arg (this_atom->chainID == " "
                                                 ? ""
                                                 : ("Chain "
                            ...
                            if (one_bead)
                              {
                                ...
                                for (unsigned int i = 0;
                                     ...
                                     i++)
                                  {
                                    new_bead.volume += residue_list[orgrespos]
                                                           .r_bead[i]
                                                           .volume;
                                    new_bead.mw += residue_list[orgrespos]
                                                       .r_bead[i]
                                                       .mw;
                                    new_bead.hydration
                                        += residue_list[orgrespos]
                                ...
                                // new_bead.hydration = (unsigned
                                // int)(misc.avg_hydration *
                                // atom_counts[count_idx] + .5);
                                new_bead.color = 10;         // light green
                                new_bead.placing_method = 0; // cog
                                new_bead.chain = 1;          // side chain
                                // do we have an AA with a complete PB?
                                if (auto_bb_aa
                                    && !molecules_residue_missing_atoms_skip
                                           [QString ("%1|%2|%3")
                                                .arg (idx)
                                                .arg (pos)
                                                .arg (0)]
                                     ...
                                     i++)
                                  {
                                    if (!molecules_residue_missing_atoms_skip
                                            [QString ("%1|%2|%3")
                                                 ...
                                                 .arg (i)])
                                      {
                                        new_atom = residue_list[orgrespos]
                                                       .r_atom[i];
                                        if (new_atom.name == "OXT")
                                          {
                                            new_atom.name = "OXT'";
                                        ...
                                        new_atom.hybrid.mw
                                            *= atoms_scale_weight
                                                [0]; // misc.avg_mass;
                                        new_atom.hybrid.mw
                                            = (int)(new_atom.hybrid.mw * 100
                                                    + .5)
                                              / 100.0;
                                        new_atom.hybrid.radius
                                            *= atoms_scale_radius
                                                [0]; // misc.avg_radius;
                                        new_atom.hybrid.radius
                                            = (int)(new_atom.hybrid.radius
                                                        * 100
                                                    + .5)
                                              / 100.0;
                            ...
                            else
                              {
                                for (unsigned int i = 0;
                                  ...
                                  {
                                    new_bead
                                        = residue_list[orgrespos].r_bead[i];
                                    if (beads_missing_atom_count[i])
                                      {
                                        // only set the beads with missing
                                        // atoms
                                        new_bead.color = 10;
                                ...
                                any_positioner.resize (
                                    residue_list[orgrespos].r_bead.size ());
                                for (unsigned int i = 0;
                                     i < any_positioner.size (); i++)
                                  {
                                     ...
                                     i++)
                                  {
                                    if (!molecules_residue_missing_atoms_skip
                                                 ...
                                                 .arg (i)])
                                      {
                                        new_atom = residue_list[orgrespos]
                                                       .r_atom[i];
                                        if (beads_missing_atom_count
                                                [new_atom.bead_assignment])
                                          {
                                            new_atom.hybrid.mw
                                                *= atoms_scale_weight
                                                    [new_atom
                                                         .bead_assignment]; // misc.avg_mass;
                                            new_atom.hybrid.mw
                                                = (int)(new_atom.hybrid.mw
                                                            * 100
                                                        + .5)
                                                  / 100.0;
                                            new_atom.hybrid.radius
                                                *= atoms_scale_radius
                                                    [new_atom
                                                         .bead_assignment]; // misc.avg_radius;
                                            new_atom.hybrid.radius
                                                = (int)(new_atom.hybrid.radius
                                                            * 100
                                                        + .5)
                    ...
                    default:
                      // US_Static::us_message(us_tr("Internal error:"),
                      //                      us_tr("Unhandled bead_exception
                      //                      code."));
                      exit (-1);
                      break;
                    } // switch
          ...
          // remove from unknown any aa residues that have 2 beads and a side
          // chain with a N,C,CA,O MC
          for (unsigned int i = 0; i < unknown_aa_residues_to_check.size ();
               i++)
            {
              // if ( advanced_config.debug_2  )
              // {
              //    printf("unknown residue %s checking for removal from
              //    broken\n",
              //           unknown_aa_residues_to_check[i].toLatin1().data());
              // }
              unsigned int respos
                  = multi_residue_map[unknown_aa_residues_to_check[i]][0];
              if (residue_list[respos].r_bead.size () == 2
                  && residue_list[respos].r_atom.size () > 4)
                {
                  ...
                  for (unsigned int j = 0;
                       j < residue_list[respos].r_atom.size (); j++)
                    {
                      aa_main_chain[residue_list[respos].r_atom[j].name]++;
                    }
                  if (aa_main_chain["N"] == 1 && aa_main_chain["CA"] == 1
                      && aa_main_chain["C"] == 1 && aa_main_chain["O"] == 1)
                    {
                      // if ( advanced_config.debug_2  )
                      // {
                      //    printf("unknown residue %s removed from broken
                      //    override\n",
                      //           unknown_aa_residues_to_check[i].toLatin1().data());
                      // }
  ...
  //                << "\t" << (unsigned int)
  //                residue_list[i].r_atom[j].positioner
...
void
US_Saxs_Util::update_vbar ()
{
  if (misc.compute_vbar
      && model_vector.size ()
             > 0) // after reading the pdb file, the vbar is calculated.
    { // If we computed vbar, we assign this to result.vbar, which should be
    ...
    {
      results_hydro.vbar
          = misc.vbar - (4.25e-4 * (K0 + misc.vbar_temperature - K20));
    }
...
bool
US_Saxs_Util::hard_coded_defaults (map<QString, QString> &results,
                                   map<QString, QString> &parameters)
{
  ...
  saxs_options_hydro.water_e_density
      = 0.334f; // water electron density in e/A^3
  ...
  saxs_options_hydro.hydrate_pdb
      = false;                      // Hydrate the PDB model? (true/false)
  ...
  saxs_options_hydro.dummy_saxs_names.push_back (
      saxs_options_hydro.dummy_saxs_name);
  ...
  saxs_options_hydro.cs_qstart
      = saxs_options_hydro.qstart * saxs_options_hydro.qstart;
  saxs_options_hydro.cs_qend
      = saxs_options_hydro.qend * saxs_options_hydro.qend;
  ...
  saxs_options_hydro.cryson_hydration_shell_contrast
      = saxs_options_hydro.d2o_conc * saxs_options_hydro.d2o_scat_len_dens
        + (1e0 - saxs_options_hydro.d2o_conc)
              * (saxs_options_hydro.h2o_scat_len_dens);
  ...
  if (parameters.count ("hydro_options"))
    {
      ...
      hydro.temperature
          = parameters["solvent_temperature"].toDouble (); // K20 - K0;
      hydro.solvent_viscosity
          = parameters["solvent_viscosity"].toDouble (); // VISC_20W * 100;
      hydro.solvent_density
          = parameters["solvent_density"].toDouble (); // DENS_20W;
    }
  // else
  //   {
  //     hydro.solvent_name = "Water";
...
bool
US_Saxs_Util::set_default (map<QString, QString> &results,
                           map<QString, QString> &parameters)
{
  // QFile f;
  // QString str;
  ...
  //                            us_tr("The somo.default configuration file was
  //                            found to be corrupt.\n"
  ...
  //  {
  //     pdb_vis->filename = string(getenv("ULTRASCAN")) + "/etc/rasmol.spt";
  //     //default color file
  //  }
  ...
  if (saxs_options_hydro.default_hybrid_filename.isEmpty ())
    {
      ...
      if (!fi_hybrid_filename.exists ())
        {
          results["errors"]
              += QString (" file %1 does not exist.").arg (hybrid_fi);
        }
      // saxs_options_hydro.default_hybrid_filename =
      // "/src/ultrascan/etc/somo.hybrid";
    }
  if (saxs_options_hydro.default_saxs_filename.isEmpty ())
    {
      saxs_options_hydro.default_saxs_filename
          = paths + SLASH + "etc" + SLASH + "somo.saxs_atoms";
      QString saxs_fi = saxs_options_hydro.default_saxs_filename;
      QFileInfo fi_saxs_filename (saxs_fi);
      if (!fi_saxs_filename.exists ())
        {
          results["errors"]
              += QString (" file %1 does not exist.").arg (saxs_fi);
        }

      // saxs_options_hydro.default_saxs_filename =
      // "/src/ultrascan/etc/somo.saxs_atoms";
    }
  if (saxs_options_hydro.default_rotamer_filename.isEmpty ())
    {
      saxs_options_hydro.default_rotamer_filename
          = paths + SLASH + "etc" + SLASH + "somo.hydrated_rotamer";
      ...
      if (!fi_rotamer_filename.exists ())
        {
          results["errors"]
              += QString (" file %1 does not exist.").arg (rotamer_fi);
        }
      // saxs_options_hydro.default_rotamer_filename =
      // "/src/ultrascan/etc/somo.hydrated_rotamer";
    }
  if (saxs_options_hydro.default_ff_filename.isEmpty ())
    {
      saxs_options_hydro.default_ff_filename
          = paths + SLASH + "etc" + SLASH + "somo.ff";
      QString ff_fi = saxs_options_hydro.default_ff_filename;
      QFileInfo fi_ff_filename (ff_fi);
      if (!fi_ff_filename.exists ())
        {
          results["errors"]
              += QString (" file %1 does not exist.").arg (ff_fi);
        }
...
void
US_Saxs_Util::calc_vol_for_saxs ()
{
      ...
      for (unsigned int j = 0; j < model_vector[i].molecule.size (); j++)
        {
          for (unsigned int k = 0;
               k < model_vector[i].molecule[j].atom.size (); k++)
            {
...
US_Saxs_Util::calc_mw_hydro ()
{
  ...
  // {
  //    errormsg = "calc_mw(): requires saxsfile, hybridfile & atomfile
  //    defined"; do_excl_vol = false; return false;
  // }
  ...
  for (unsigned int i = 0; i < model_vector.size (); i++)
    {
        ...
        for (unsigned int j = 0; j < model_vector[i].molecule.size (); j++)
          {
                    ...
                    if (this_atom->resName != "WAT")
                      {
                        ...
                        cm.axis[0]
                            += this_atom->mw * this_atom->coordinate.axis[0];
                        cm.axis[1]
                            += this_atom->mw * this_atom->coordinate.axis[1];
                        cm.axis[2]
                            += this_atom->mw * this_atom->coordinate.axis[2];
                    ...
                    if (do_excl_vol)
                      {
                        ...
                        unsigned int this_e;
                        unsigned int this_e_noh;
                        if (!set_excluded_volume (
                                *this_atom, excl_vol, scaled_excl_vol,
                                our_saxs_options, residue_atom_hybrid_map,
                                this_e, this_e_noh))
                          {
                            return false;
                          }
            ...
            if (model_vector[i].molecule[j].mw != 0.0)
              {
                QString qs
                    = QString ("\nModel: %1 Chain: %2 Molecular weight %3 "
                               "Daltons, Volume (from vbar) %4 A^3%5")
                          .arg (model_vector[i].model_id)
                          .arg (model_vector[i].molecule[j].chainID)
                          .arg (model_vector[i].molecule[j].mw)
                          .arg (mw_to_volume (model_vector[i].molecule[j].mw,
                                              model_vector[i].vbar))
                          .arg (
                              do_excl_vol
                                  ? QString (
                                        ", atomic volume %1 A^3%2 average "
                                        "electron density %3 A^-3")
                                        .arg (chain_excl_vol)
                                        .arg (
                                            chain_excl_vol
                                                    != chain_scaled_excl_vol
                                                ? QString (", scaled atomic "
                                                           "volume %1 A^2")
                                                      .arg (
                                                          chain_scaled_excl_vol)
                                                : "")
                    ...
                    if (this_atom->resName != "WAT")
                      {
                        Rg2 += (double)this_atom->mw
                               * ((double)(this_atom->coordinate.axis[0]
                                           - cm.axis[0])
                                      * (this_atom->coordinate.axis[0]
                                         - cm.axis[0])
                                  + (double)(this_atom->coordinate.axis[1]
                                             - cm.axis[1])
                                        * (this_atom->coordinate.axis[1]
                                           - cm.axis[1])
                                  + (double)(this_atom->coordinate.axis[2]
                                             - cm.axis[2])
      ...
      {
        QString qs
            = QString ("\nModel: %1 Molecular weight %2 Daltons, Volume (from "
                       "vbar) %3 A^3%4")
                  .arg (model_vector[i].model_id)
                  .arg (model_vector[i].mw)
                  .arg (
                      mw_to_volume (model_vector[i].mw, model_vector[i].vbar))
                  .arg (
                      do_excl_vol
                          ? QString (", atomic volume %1 A^3%2 average "
                                     "electron density %3 A^-3")
                                .arg (tot_excl_vol)
                                .arg (
                                    tot_excl_vol != tot_scaled_excl_vol
                                        ? QString (
                                              ", scaled atomic volume %1 A^2")
        ...
        if (us_udp_msg)
          {
            map<QString, QString> msging;
            msging["_textarea"]
                = QString ("\\nModel: %1 Molecular weight %2 Daltons, Volume "
                           "(from vbar) %3 A^3%4")
                      .arg (model_vector[i].model_id)
                      .arg (model_vector[i].mw)
                      .arg (mw_to_volume (model_vector[i].mw,
                                          model_vector[i].vbar))
                      .arg (
                          do_excl_vol
                              ? QString (", atomic volume %1 A^3%2 average "
...
bool
US_Saxs_Util::calc_grid_pdb (bool /* parameters_set_first_model */)
{
  ...
  //                             ), us_tr(
  //                                "Can not process models that contain the
  //                                WAT residue.\n" "These are currently
  //                                generated only for SAXS/SANS computations"
  //                                )
  //                             );
  //       return -1;
  //    }
  ...
  //                          us_tr(
  //                             "Can not process models that contain the XHY
  //                             residue.\n" "These are currently generated
  //                             only for SAXS/SANS computations\n"
  //                             )
  //                          );
  //    return -1;
  // }
  ...
  if (grid_hydro.create_nmr_bead_pdb)
    {
      if ((grid_exposed_overlap.remove_overlap || grid_overlap.remove_overlap
           || grid_buried_overlap.remove_overlap)
          && (grid_exposed_overlap.translate_out || grid_overlap.translate_out
              || grid_buried_overlap.translate_out))
        {
          ...
          //                           "US-SOMO: Build AtoB models with
          //                           structure factors", us_tr( msg ) );
  ...
  // editor_msg( "dark blue", QString( us_tr( "Peptide Bond Rule is %1 for this
  // PDB" ) ).arg( misc.pb_rule_on ? "on" : "off" ) );
  ...
  if (us_udp_msg)
    {
      map<QString, QString> msging;
      msging["_textarea"]
          = QString (us_tr ("Peptide Bond Rule is %1 for this PDB\\n\\n"))
                .arg (misc.pb_rule_on ? "on" : "off");
      // msging[ "_textarea" ] = QString::fromLatin1("<font
      // color=red>%1</font>").arg(us_tr( "Peptide Bond Rule is "));
      us_udp_msg->send_json (msging);
      // sleep(1);
    }
  accumulated_msgs
  ...
  for (unsigned int curr_m = 0; curr_m < model_vector.size (); curr_m++)
    {
      ...
      int mppos
          = 2                                  // create beads
            + 4                                // grid_atob
            + (grid_hydro.enable_asa ? (1 + 10 // radial reduction
      ...
      if (us_udp_msg)
        {
          ...
          msging["progress_output"]
              = QString ("AtoB model calculation: %1\%")
                    .arg (QString::number (
                        (int (double (ppos) / double (mppos)) * 100.0)))
                    .arg (100); // arg(ppos).arg(mppos);
          msging["progress1"]
              = QString::number (double (ppos) / double (mppos));
      ...
      // us_qdebug(QString::number(  int( ( double(ppos)/double(mppos) ) *
      // 100.0) ) );
          ...
          if (us_udp_msg)
            {
              map<QString, QString> msging;
              msging["_textarea"]
                  = "Errors found during the initial creation of beads\\n";
      ...
      else
        {
          if (error_string.length ())
          ...
          else
            {
              ...
              for (unsigned int j = 0; j < model_vector[i].molecule.size ();
                   j++)
                {
                  for (unsigned int k = 0;
                       k < model_vector[i].molecule[j].atom.size (); k++)
                    {
                      PDB_atom *this_atom
                          = &(model_vector[i].molecule[j].atom[k]);
                      if (this_atom->active)
                        {
                          for (unsigned int m = 0; m < 3; m++)
                            {
                              this_atom->bead_coordinate.axis[m]
                                  = this_atom->coordinate.axis[m];
                            }
                          ...
                          if (grid_hydro.hydrate && this_atom->atom_hydration)
                            {
                              double additional_radius
                                  = pow (3 * this_atom->atom_hydration
                                             * misc.hydrovol / (M_PI * 4),
                                         1.0 / 3.0);
              ...
              if (us_udp_msg)
                {
                  ...
                  msging["progress_output"]
                      = QString ("AtoB model calculation: %1\%")
                            .arg (QString::number (int (
                                (double (ppos) / double (mppos)) * 100.0)))
                            .arg (100); // arg(ppos).arg(mppos);
                  msging["progress1"]
                      = QString::number (double (ppos) / double (mppos));
              ...
              if (grid_hydro.center == 2 && any_zero_si) // ssi
                {
                  // editor_msg( "red", "Center of scattering requested, but
                  // zero scattering intensities are present" );
                  // pb_grid_pdb->setEnabled(true);
                  // pb_somo->setEnabled(true);
                  // pb_somo_o->setEnabled(true);
                  // progress->reset();
              ...
              /* OMG! ***************************** */
              bead_models[i] = us_hydrodyn_grid_atob_hydro (
                  &bead_model, &grid_hydro,
                  // progress,
                  // editor,
                  // this);
                  us_log, us_udp_msg, &accumulated_msgs);
              /* ********************************** */
              ...
              if (grid_hydro.enable_asa)
                {
                  ...
                  //                      bead_model.size(); i++) {
                  //                         printf("after asa bead %d exposed
                  //                         %d color %d chain %d active %d mw
                  //                         %f vol %f cr %f [%f,%f,%f]\n",
                  //                                i,
                      ...
                      if (us_udp_msg)
                        {
                          ...
                          msging["progress_output"]
                              = QString ("AtoB model calculation: %1\%")
                                    .arg (QString::number (
                                        int ((double (ppos) / double (mppos))
                                             * 100.0)))
                                    .arg (100); // arg(ppos).arg(mppos);
                          msging["progress1"] = QString::number (
                              double (ppos) / double (mppos));
              ...
              //    {
              //       extra_text += "BSAXS:: " + sf_bead_factors[ k
              //       ].saxs_name.toUpper(); for ( unsigned int i = 0; i < 4;
              //       i++ )
              //       {
              //          extra_text += QString( " %1" ).arg( sf_bead_factors[
              //          k ].a[ i ] ); extra_text += QString( " %1" ).arg(
              //          sf_bead_factors[ k ].b[ i ] );
              //       }
              //       extra_text += QString( " %1 %2\n" ).arg(
              //       sf_bead_factors[ k ].c ).arg( bead_model[ k
              //       ].saxs_excl_vol );

              //       extra_text += "BSAXS:: " + sf_bead_factors[ k
              //       ].saxs_name; for ( unsigned int i = 0; i < 5; i++ )
              //       {
              //          extra_text += QString( " %1" ).arg( sf_bead_factors[
              //          k ].a5[ i ] ); extra_text += QString( " %1" ).arg(
              //          sf_bead_factors[ k ].b5[ i ] );
              //       }
              //       extra_text += QString( " %1 %2\n" ).arg(
              //       sf_bead_factors[ k ].c5 ).arg( bead_model[ k
              //       ].saxs_excl_vol ); tot_excl_vol += bead_model[ k
              //       ].saxs_excl_vol;

              //       extra_text += "BSAXSV:: " + sf_bead_factors[ k
              //       ].saxs_name; for ( unsigned int i = 0; i <
              //       sf_bead_factors[ k ].vcoeff.size(); i++ )
              //       {
              //          extra_text += QString( " %1" ).arg( sf_bead_factors[
              //          k ].vcoeff[ i ] );
              //       }
              ...
              //    )
              //    {
              //      //editor_msg( "dark red", QString( "Notice: extra saxs
              //      coefficients %1 replaced\n" ).arg( sf_factors.saxs_name )
              //      );
              //    } else {
              //       saxs_options_hydro.dummy_saxs_names.push_back(
              //       sf_factors.saxs_name );
              //    }
              //    extra_saxs_coefficients[ sf_factors.saxs_name ] =
              //    sf_factors; saxs_options_hydro.dummy_saxs_name =
              //    sf_factors.saxs_name;
              //    //editor_msg( "blue", QString( "Saxs name for dummy atom
              //    models set to %1" ).arg( saxs_options_hydro.dummy_saxs_name
              //    ) );

              //    // if ( saxs_map.count( sf_factors.saxs_name ) )
              //    // {
              //    //    editor_msg( "dark red",
              //    //                QString( us_tr( "Notice: saxs
              //    coefficients for %1 replaced by newly loaded values\n" ) )
              //    //                .arg( sf_factors.saxs_name ) );
              //    // } else {
              //    //    saxs_list.push_back( sf_factors );
              //    //    editor_msg( "dark blue",
              //    //                QString( us_tr( "Notice: added
              //    coefficients for %1 from newly loaded values\n" ) )
              //    //                .arg( sf_factors.saxs_name ) );
              //    // }
              ...
              //    // if ( saxs_plot_widget )
              //    // {
              //    //    if ( saxs_plot_window->saxs_map.count(
              //    sf_factors.saxs_name ) )
              //    //    {
              //    //       editor_msg( "dark red",
              //    //                   QString( us_tr( "Notice: saxs window:
              //    saxs coefficients for %1 replaced by newly loaded values\n"
              //    ) )
              //    //                   .arg( sf_factors.saxs_name ) );
              //    //    } else {
              ...
              //    //       editor_msg( "dark blue",
              //    //                   QString( us_tr( "Notice: saxs window:
              //    added coefficients for %1 from newly loaded values\n" ) )
              //    //                   .arg( sf_factors.saxs_name ) );
              //    //    }
              //    //    saxs_plot_window->saxs_map[ sf_factors.saxs_name ] =
              //    sf_factors;
              //    // }
              // }

              // if ( saxs_options_hydro.compute_saxs_coeff_for_bead_models &&
              // grid_hydro.create_nmr_bead_pdb )
              // {
              //    if ( !saxs_options_hydro.iq_global_avg_for_bead_models &&
              //    sf_bead_factors.size() != bead_model.size() )
              //    {
              //       // editor_msg( "red",
              //       //             QString( us_tr( "Overriding setting to
              //       use global structure factors since bead model doesn't
              //       contain the correct number of structure factors (%1) for
              //       the beads (%2)" ) )
              ...
              //       ) )
              //       {
              //          // editor_msg( "red", QString( us_tr("Warning: No
              //          '%1' SAXS atom found.\n" ) )
              //          //             .arg(
              //          saxs_options_hydro.dummy_saxs_name ) ); for(unsigned
              //          int i = 0; i < bead_model.size(); i++) {
              //             bead_model[i].saxs_data.saxs_name = "";
              //          }
              //       } else {
              //          // editor_msg( "blue", QString( us_tr("Notice:
              //          Loading beads with saxs coefficients '%1'" ) )
              //          //             .arg(
              //          saxs_options_hydro.dummy_saxs_name ) ); for( unsigned
              //          int i = 0; i < bead_model.size(); i++ )
              //          {
              //             bead_model[i].saxs_name =
              //             saxs_options_hydro.dummy_saxs_name;
              //             bead_model[i].saxs_data = saxs_map[
              //             saxs_options_hydro.dummy_saxs_name ];
              ...
              //       && sf_bead_factors.size() == bead_model.size() )
              //       {
              // 	  //editor_msg( "blue", us_tr("Notice: Loading beads
              // with bead computed structure factors" ) );
              //          for( unsigned int i = 0; i < bead_model.size(); i++ )
              //          {
              //             bead_model[i].saxs_name = sf_bead_factors[ i
              //             ].saxs_name; bead_model[i].saxs_data =
              //             sf_bead_factors[ i ]; bead_model[i].hydrogens = 0;
              //          }
              //       }
              //    }
              //    bead_models[current_model] = bead_model;
              ...
              if (us_udp_msg)
                {
                  map<QString, QString> msging;
                  msging["_textarea"]
                      = QString ("Volume of bead model %1\\n")
                            .arg (total_volume_of_bead_model (bead_model));
              ...
              QString filename = project + QString ("_%1").arg (curr_m + 1);
  ...
  if (us_udp_msg)
    {
      map<QString, QString> msging;
      msging["progress_output"] = QString ("AtoB model calculation: %1\%")
...
void
US_Saxs_Util::bead_check (bool use_threshold, bool message_type)
{
  // recheck beads here
  ...
  QString error_string = "";
  int retval
      = us_hydrodyn_asab1_main_hydro (active_atoms, &asa_hydro, &results_hydro,
                                      true); // ,
                                             // progress,
                                             // editor,
                                             // this
  ...
  if (retval)
    {
      switch (retval)
        ...
        case US_HYDRODYN_ASAB1_ERR_MEMORY_ALLOC:
          {
            // printError("US_HYDRODYN_ASAB1 on bead recheck encountered a
  ...
  //  write_bead_spt(somo_dir + SLASH + project + QString("_%1").arg(
  //  model_name( current_model ) ) + "_pre_recheck" + DOTSOMO, &bead_model);
  //  write_bead_model(somo_dir + SLASH + project + QString("_%1").arg(
  //  model_name( current_model ) ) + "_pre_recheck" + DOTSOMO, &bead_model );
  ...
  for (unsigned int i = 0; i < bead_model.size (); i++)
    {
      float surface_area
          = (asa_hydro.probe_radius + bead_model[i].bead_computed_radius)
            * (asa_hydro.probe_radius + bead_model[i].bead_computed_radius) * 4
            * M_PI;
      QString msg = "";
      if (use_threshold
              ? (bead_model[i].bead_recheck_asa > asa_hydro.threshold)
              : (bead_model[i].bead_recheck_asa
                 > (asa_hydro.threshold_percent / 100.0) * surface_area))
        {
          // now exposed
      ...
      printf ("bead %d %.2f %.2f %.2f %s %s bead mw %.2f bead ref mw %.2f\n",
              i, bead_model[i].bead_computed_radius, surface_area,
              bead_model[i].bead_recheck_asa,
              (use_threshold
                   ? (bead_model[i].bead_recheck_asa > asa_hydro.threshold)
                   : (bead_model[i].bead_recheck_asa
                  ...
                  ? "exposed"
                  : "buried",
              msg.toLatin1 ().data (), bead_model[i].bead_mw,
              bead_model[i].bead_ref_mw);
#endif
    }
    //  write_bead_spt(somo_dir + SLASH + project + QString("_%1").arg(
    ...
    // +
    //                QString(bead_model_suffix.length() ? ("-" +
    //                bead_model_suffix) : "") + DOTSOMO, &bead_model);
  ...
  if (us_udp_msg)
    {
      map<QString, QString> msging;
      msging["_textarea"]
          = QString ("%1 exposed beads became buried\\n").arg (e2b);
  ...
  if (message_type)
      ...
      if (us_udp_msg)
        {
          map<QString, QString> msging;
          msging["_textarea"]
              = QString (us_tr ("%1 beads are exposed\\n")).arg (b2e);
  ...
  else
    {
      us_log->log (
          QString (
              us_tr ("%1 previously buried beads are exposed by rechecking\n"))
              .arg (b2e));
      if (us_udp_msg)
        {
          map<QString, QString> msging;
          msging["_textarea"]
              = QString (us_tr ("%1 previously buried beads are exposed by "
                                "rechecking\\n"))
                    .arg (b2e);
      ...
      accumulated_msgs += QString (us_tr ("%1 previously buried beads are "
                                          "exposed by rechecking\\n"))
                              .arg (b2e);
    }
}
...
)
{
  ...
  printf ("outward_translate_2_spheres\n"
          "sphere 1 radius %.3f center [%.3f,%.3f,%.3f] cog vector "
          "[%.3f,%.3f,%.3f]\n"
          "sphere 2 radius %.3f center [%.3f,%.3f,%.3f] cog vector "
          "[%.3f,%.3f,%.3f]\n",
          *r1, x1[0], x1[1], x1[2], v1[0], v1[1], v1[2], *r2, x2[0], x2[1],
          x2[2], v2[0], v2[1], v2[2]);
#endif
  ...
  float sfact = sqrt (
      (double)(*r1 + *r2) * (*r1 + *r2)
      * ((double)(*r1 * *r1
                      * (-1 + v1[0] * v1[0] + v1[1] * v1[1] + v1[2] * v1[2])
                  -

                  2 * *r1 * *r2
                      * (1 + v1[0] * v2[0] + v1[1] * v2[1] + v1[2] * v2[2])
                  +

                  *r2 * *r2
                      * (-1 + v2[0] * v2[0] + v2[1] * v2[1] + v2[2] * v2[2]))
             *
         ...
         pow (*r1 * *r1
                  + *r1
                        * (2 * *r2 + v1[0] * x1[0] + v1[1] * x1[1]
                           + v1[2] * x1[2] - v1[0] * x2[0] - v1[1] * x2[1]
                           - v1[2] * x2[2])
                  + *r2
  ...
  *r2 -= k2;
  ...
  printf ("new coordinates, radius\n"
          ...
          *r1, x1[0], x1[1], x1[2], *r2, x2[0], x2[1], x2[2],
          *r1 + *r2
              - sqrt ((x1[0] - x2[0]) * (x1[0] - x2[0])
                      + (x1[1] - x2[1]) * (x1[1] - x2[1])
                      + (x1[2] - x2[2]) * (x1[2] - x2[2])));
#endif
}
...
int
US_Saxs_Util::radial_reduction (bool from_grid, int use_ppos, int mppos)
{
      ...
      if (this_atom->active)
        {
          ...
          for (unsigned int m = 0; m < 3; m++)
            {
              molecular_cog[m]
                  += this_atom->bead_coordinate.axis[m] * this_atom->bead_mw;
            }
        }
    }
  if (molecular_mw)
...
#if defined(DEBUG)
  printf (
      "or sc fb %s rh %s rs %s to %s st %s ro %s %f %f %f\n"
      ...
      buried_overlap.fuse_beads_percent, buried_overlap.remove_sync_percent,
      ...
      grid_overlap.remove_overlap ? "Y" : "N", grid_overlap.fuse_beads_percent,
      grid_overlap.remove_sync_percent, grid_overlap.remove_hierarch_percent);
#endif
  ...
  float overlap[] = { (float)(sidechain_overlap.fuse_beads_percent / 100.0),
  ...
  //    write_bead_tsv(somo_tmp_dir + SLASH + "bead_model_start" + DOTSOMO +
  //    ".tsv", &bead_model); write_bead_spt(somo_tmp_dir + SLASH +
  //    "bead_model_start" + DOTSOMO, &bead_model);
  // #endif
  for (unsigned int k = (grid_hydro.enable_asa ? 0 : 3);
       k < (unsigned int)(grid_hydro.enable_asa ? 3 : 4); k++)
    {
      // only grid method
      if (!methods[k] || ((from_grid || grid_hydro.enable_asa) && k == 1))
        {
    ...
    stage_loop:
      ...
      printf (
          "popping stage %d %s%s%s%s%s%s%s%s%s%s%s%s%s%s%soverlap_reduction "
          "%f rroverlap %f\n",
          k, (methods[k] & POP_MC) ? "main chain " : "",
      ...
      do
        {
          ...
          //    return;
          // }
          // write_bead_tsv(somo_tmp_dir + SLASH +
          // QString("bead_model_bp-%1-%2").arg(k).arg(iter) + DOTSOMO +
          // ".tsv", &bead_model); write_bead_spt(somo_tmp_dir + SLASH +
          // QString("bead_model-bp-%1-%2").arg(k).arg(iter) + DOTSOMO,
          // &bead_model);
#if defined(DEBUG1) || defined(DEBUG)
          ...
          //   {
          //     map < QString, QString > msging;
          //     msging[ "progress_output" ] =  QString("Stage %1 popping
          //     iteration %2").arg(k+1).arg(iter); msging[ "progress1" ] =
          //     QString::number(double(k+1)/double(k_tot));
          //     //msging[ "_progress" ] =
          //     QString::number(double(i+1)/double(npoints_x));

          //     us_udp_msg->send_json( msging );
          //     //sleep(1);
          //   }
          // lbl_core_progress->setText(QString("Stage %1 popping iteration
          // %2").arg(k+1).arg(iter)); qApp->processEvents();

          max_intersection_volume = -1;
          overlaps_exist = false;
          if (methods[k]
              & (POP_MC | POP_SC | POP_MCSC | POP_EXPOSED | POP_BURIED
                 | POP_ALL))
            {
...
#if defined(DEBUG)
                      printf (
                          "checking popping stage %d beads %d %d on chains %d "
                          "%d exposed %d %d active %s %s max iv %f\n",
                          k, i, j, bead_model[i].chain, bead_model[j].chain,
                          bead_model[i].exposed_code,
...
#endif
                      if (bead_model[i].active && bead_model[j].active
                          && (((methods[k] & POP_SC)
                          ...
                          && (((methods[k] & POP_EXPOSED)
                              ...
                              || (methods[k] & POP_ALL)))
                        {
                          intersection_volume = int_vol_2sphere (
                              bead_model[i].bead_computed_radius,
                              bead_model[j].bead_computed_radius,
                              sqrt (
                                  pow (bead_model[i].bead_coordinate.axis[0]
                                           - bead_model[j]
                                                 .bead_coordinate.axis[0],
                                       2)
                                  + pow (bead_model[i].bead_coordinate.axis[1]
                                             - bead_model[j]
                                                   .bead_coordinate.axis[1],
                                         2)
                                  + pow (bead_model[i].bead_coordinate.axis[2]
                                             - bead_model[j]
                                                   .bead_coordinate.axis[2],
                                         2)));
#if defined(DEBUG)
                          printf (
                              "this overlap bead %u %u vol %f rv1 %f rv2 %f "
                              "r1 %f r2 %f p1 [%f,%f,%f] p2 [%f,%f,%f]\n",
                              bead_model[i].serial, bead_model[j].serial,
                              intersection_volume,
                              bead_model[i].bead_ref_volume,
                          ...
                          if (intersection_volume
                                  > bead_model[i].bead_ref_volume * overlap[k]
                              || intersection_volume
                                     > bead_model[j].bead_ref_volume
                                           * overlap[k])
                            {
                              ...
                              if (intersection_volume
                                ...
                                {
#if defined(DEBUG)
                                  printf ("best overlap so far bead %u %u vol "
                                          "%f\n",
                                          bead_model[i].serial,
                                          bead_model[j].serial,
                                          intersection_volume);
              ...
              if (overlaps_exist)
                {
                  ...
                  // 	 {
                  // 	   map < QString, QString > msging;
                  // 	   msging[ "progress_output" ] = QString("Gridding %1
                  // of %2").arg(i+1).arg(npoints_x); 	   msging[ "progress1" ] =
                  // QString::number(double(i+1)/double(npoints_x));
                  // 	   //msging[ "_progress" ] =
                  // QString::number(double(i+1)/double(npoints_x));
                  ...
                  // 	   //sleep(1);
                  // 	 }
                  // lbl_core_progress->setText(QString("Stage %1 popping
                  // iteration %2 beads popped
                  // %3").arg(k+1).arg(iter).arg(beads_popped));
                  // qApp->processEvents();
                  // //#define DEBUG_FUSED
#if defined(DEBUG1) || defined(DEBUG) || defined(DEBUG_FUSED)
                  printf ("popping beads %u %u int vol %f mw1 %f mw2 %f v1 %f "
                          "v2 %f c1 [%f,%f,%f] c2 [%f,%f,%f]\n",
                          max_bead1, max_bead2, max_intersection_volume,
                  ...
                  if (bead_model[max_bead1].chain
                          != bead_model[max_bead2].chain
                      && k == 1)
                    {
                      back_to_zero = true;
                    }
                  ...
                  // ??
                  bead_model[max_bead1].all_beads.push_back (
                      &bead_model[max_bead2]);
                  for (unsigned int n = 0;
                       n < bead_model[max_bead2].all_beads.size (); n++)
                    {
                      bead_model[max_bead1].all_beads.push_back (
                          bead_model[max_bead2].all_beads[n]);
                    }
                  ...
                  for (unsigned int m = 0; m < 3; m++)
                    {
                      bead_model[max_bead1].bead_coordinate.axis[m]
                          *= bead_model[max_bead1].bead_ref_mw;
                      bead_model[max_bead1].bead_coordinate.axis[m]
                          += bead_model[max_bead2].bead_coordinate.axis[m]
                             * bead_model[max_bead2].bead_ref_mw;
                      bead_model[max_bead1].bead_coordinate.axis[m]
                          /= bead_model[max_bead1].bead_ref_mw
                             + bead_model[max_bead2].bead_ref_mw;
                  ...
                  bead_model[max_bead1].bead_actual_radius
                      ...
                      = pow (3 * bead_model[max_bead1].bead_ref_volume
                                 / (4.0 * M_PI),
                             1.0 / 3);
                  // if fusing with a side chain bead, make sure the fused is
                  // side-chain if (bead_model[max_bead2].chain) {
...
#if defined(DEBUG)
                  printf ("after popping beads %d %d int volume %f radius %f "
                          "mw %f vol %f coordinate [%f,%f,%f]\n",
                          bead_model[max_bead1].serial,
                          bead_model[max_bead2].serial, intersection_volume,
                          bead_model[max_bead1].bead_computed_radius,
                          bead_model[max_bead1].bead_ref_mw,
                          bead_model[max_bead2].bead_ref_volume,
                          bead_model[max_bead1].bead_coordinate.axis[0],
                  ...
                  if (back_to_zero)
                    {
                      us_log->log (
                          QString ("Beads popped %1, Go back to stage %2\n")
                              .arg (beads_popped)
                              .arg (k));
                      if (us_udp_msg)
                        {
                          ...
                          msging["_textarea"]
                              = QString (
                                    "Beads popped %1, Go back to stage %2\\n")
                                    .arg (beads_popped)
                                    .arg (k);
                      ...
                      // printf("fused sc/mc bead in stage SC/MC, back to stage
                      // SC\n");
                      k = 0;
          ...
          // write_bead_tsv(somo_tmp_dir + SLASH +
          // QString("bead_model_ap-%1-%2").arg(k).arg(iter) + DOTSOMO +
          // ".tsv", &bead_model); write_bead_spt(somo_tmp_dir + SLASH +
          // QString("bead_model-ap-%1-%2").arg(k).arg(iter) + DOTSOMO,
          // &bead_model);
      ...
      // #if defined(WRITE_EXTRA_FILES)
      //       write_bead_tsv(somo_tmp_dir + SLASH +
      //       QString("bead_model_pop-%1").arg(k) + DOTSOMO + ".tsv",
      //       &bead_model); write_bead_spt(somo_tmp_dir + SLASH +
      //       QString("bead_model_pop-%1").arg(k) + DOTSOMO, &bead_model);
      // #endif
      ...
      if (methods[k] & RADIAL_REDUCTION)
        {
          ...
          vector<bool> reduced;
          vector<bool>
              reduced_any; // this is for a final recomputation of the volumes
          ...
          if (methods[k] & RR_HIERC)
            {
#if defined(DEBUG1) || defined(DEBUG)
              printf (
                  "preprocessing processing hierarchical radial reduction\n");
#endif

              // if ( us_udp_msg )
              //   {
              // 	map < QString, QString > msging;
              // 	msging[ "progress_output" ] = QString("Stage %1
              // hierarchical radial reduction").arg(k+1); 	msging[ "progress1"
              // ] = QString::number(double(k + 1)/double(npoints_x));
              // 	//msging[ "_progress" ] =
              // QString::number(double(i+1)/double(npoints_x));
              ...
              // lbl_core_progress->setText(QString("Stage %1 hierarchical
              // radial reduction").arg(k+1)); qApp->processEvents();
...
#if defined(DEBUG)
                      printf (
                          "checking radial stage %d beads %d %d on chains %d "
                          "%d exposed code %d %d active %s %s max il %f %s %s "
                          "%s %s\n",
                          k, i, j, bead_model[i].chain, bead_model[j].chain,
                          bead_model[i].exposed_code,
                          bead_model[j].exposed_code,
                          ...
                          (methods[k] & RR_BURIED) ? "RR_BURIED"
                                                   : "!RR_BURIED",
                          (methods[k] & RR_SC) ? "RR_SC" : "!RR_SC",
                          (methods[k] & RR_MCSC) ? "RR_MCSC" : "!RR_MCSC",
                          (bead_model[i].active && bead_model[j].active
                           && (methods[k] & RR_MCSC
                               || ((methods[k] & RR_SC)
                                   && bead_model[i].chain == 1
                                   && bead_model[j].chain == 1))
                           ...
                           && bead_model[j].bead_computed_radius > TOLERANCE)
                              ...
                              : "logic false");
#endif
                      //                  bool active = bead_model[i].active &&
                      //                  bead_model[j].active;
...
#if defined(DEBUG)
                          printf (
                              "beads %d %d with radii %f %f with coordinates "
                              "[%f,%f,%f] [%f,%f,%f] have a sep of %f\n",
                              i, j, bead_model[i].bead_computed_radius,
                              bead_model[j].bead_computed_radius,
                              bead_model[i].bead_coordinate.axis[0],
                              bead_model[i].bead_coordinate.axis[1],
                              bead_model[i].bead_coordinate.axis[2],
              ...
              max_bead1 = max_bead2 = -1;
              float radius_delta;
              do
                {
                  iter++;
#if defined(DEBUG1) || defined(DEBUG)
                  printf ("processing hierarchical radial reduction iteration "
                          "%d\n",
                          iter);
#endif

                  // lbl_core_progress->setText(QString("Stage %1 hierarch.
                  // red. it. %2 with %3
                  // couples").arg(k+1).arg(iter).arg(pairs.size()));
                  // qApp->processEvents();
                  ...
                  for (unsigned int i = 0; i < pairs.size (); i++)
                    {
                      ...
                      printf ("pair %d %d sep %f %s %s\n", pairs[i].i,
                              pairs[i].j, pairs[i].separation,
                              pairs[i].active ? "active" : "not active",
                              pairs[i].i == max_bead1
                                      || pairs[i].j == max_bead1
                                      || pairs[i].i == max_bead2
                                      || pairs[i].j == max_bead2
                                  ? "needs recompute of separation"
                                  : "separation valid");
#endif
                      if (pairs[i].active)
                        {
                          if (pairs[i].i == max_bead1
                              || pairs[i].j == max_bead1
                              || pairs[i].i == max_bead2
                              || pairs[i].j == max_bead2)
                            {
                              pairs[i].separation
                                  = bead_model[pairs[i].i].bead_computed_radius
                                    + bead_model[pairs[i].j]
                                          .bead_computed_radius
                                    - sqrt (
                                        pow (
                                            bead_model[pairs[i].i]
                                                    .bead_coordinate.axis[0]
                                                - bead_model[pairs[i].j]
                                                      .bead_coordinate.axis[0],
                                            2)
...
#if defined(DEBUG1) || defined(DEBUG)
                      printf ("processing radial reduction hierc iteration %d "
                              "pair %d processed %d\n",
                              iter, max_pair, count);
                      ...
                      do
                        {
                          ...
                          if (methods[k] & OUTWARD_TRANSLATION
                                  ...
                                  && methods[0] & OUTWARD_TRANSLATION))
                            {
                              ...
                              if ((methods[k] & RR_MCSC
                                       ...
                                       || bead_model[max_bead2].chain == 1)))
                                {
                                  ...
                                  if (radius_delta > bead_model[use_bead]
                                                         .bead_computed_radius)
                                    {
                                      radius_delta = bead_model[use_bead]
                                                         .bead_computed_radius;
                                    }
#if defined(DEBUG2)
                                  printf (
                                      "use bead %d no outward translation is "
                                      "required, one bead to shrink, radius "
                                      "delta %f  cr %f %f\n",
                                      use_bead, radius_delta,
                                      bead_model[use_bead]
                                          .bead_computed_radius,
                                  ...
                                  bead_model[use_bead].bead_computed_radius
                                      -= radius_delta;
                                  if (bead_model[use_bead].bead_computed_radius
                                      <= 0)
                                    {
                                      // this is to ignore this bead for
                                      // further radial reduction regardless
                                      bead_model[use_bead].bead_computed_radius
                              ...
                              else
                                {
                                  ...
                                  printf ("rr4 notv %d\n",
                                          bead_model[use_bead]
                                              .normalized_ot_is_valid);
#endif
                                  if (!bead_model[use_bead]
                                           .normalized_ot_is_valid)
                                    {
                                      float norm = 0.0;
                                      for (unsigned int l = 0; l < 3; l++)
                                        {
                                          bead_model[use_bead]
                                              .normalized_ot.axis[l]
                                              = bead_model[use_bead]
                                                    .bead_coordinate.axis[l]
                                                - molecular_cog[l];
                                          norm += bead_model[use_bead]
                                                      .normalized_ot.axis[l]
                                                  * bead_model[use_bead]
                                                        .normalized_ot.axis[l];
                                        }
                                  ...
                                  // the slight potential of one being at the
                                  // molecular cog
                                  reduced_any[max_bead1] = true;
                                  reduced_any[max_bead2] = true;
                                  outward_translate_2_spheres (
                                      &bead_model[max_bead1]
                                           .bead_computed_radius,
                                      &bead_model[max_bead2]
                                           .bead_computed_radius,
                                      bead_model[max_bead1]
                                          .bead_coordinate.axis,
                                      bead_model[max_bead2]
                                          .bead_coordinate.axis,
                          ...
                          else
                            {
                              ...
                              puts ("rr5");
#endif
                              // no outward translation is required for either
                              // bead are we shrinking just 1 bead ... if we
                              // are dealing with buried beads, then only
                              // buried beads should be shrunk, not exposed
                              // beads
#if defined(DEBUG2)
                              ...
                              if (methods[k] & RR_BURIED
                                ...
                                {
                                  printf ("what are we doing here?  buried "
                                          "and two exposed??\n");
                                  // exit(-1);
                                }
                              if (methods[k] & RR_MCSC
                                  ...
                                  && bead_model[max_bead2].chain == 1)
                                {
                                  printf ("what are we doing here?  dealing "
                                          "with 2 SC's on the MCSC run??\n");
                                  // exit(-1);
                                }
                              if ((methods[k] & RR_BURIED
                                   && (bead_model[max_bead1].exposed_code == 1
                                       || bead_model[max_bead2].exposed_code
                                              == 1))
                                  || (methods[k] & RR_MCSC
                                      && !(methods[k] & RR_BURIED)
                                      && (bead_model[max_bead1].chain == 1
                                                 ...
                                                 == 1)))
                                {
                                  ...
                                  // we are either buried with one of the beads
                                  // exposed or on the MCSC and one of the
                                  // beads is SC
                                  int use_bead;
                                  if (methods[k] & RR_BURIED)
                                    {
                                      if (bead_model[max_bead1].exposed_code
                                          == 1)
                                        {
                                          use_bead = max_bead2;
                                        }
                              ...
                              else
                                {
                                  ...
                                  int use_bead = max_bead1;
#if defined(DEBUG2)
                                  printf ("use bead %d no outward translation "
                                          "is required\n",
                                          use_bead);
#endif
                                  reduced[use_bead] = true;
                                  reduced_any[use_bead] = true;
                                  radius_delta =
                                      // bead_model[use_bead].bead_actual_radius
                                      // * rr_overlap[k];
                                      max_intersection_length
                                      * bead_model[max_bead1]
                                            .bead_computed_radius
...
#if defined(DEBUG2)
                                  printf (
                                      "intersection len %f recomputed %f "
                                      "radius delta %f r1 %f r2 %f\n",
                                      max_intersection_length,
                                      bead_model[max_bead1]
                                              .bead_computed_radius
                                          + bead_model[max_bead2]
...
#if defined(DEBUG2)
                          printf (
                              "b1r %f b2r %f current separation %f\n",
                              bead_model[max_bead1].bead_computed_radius,
          ...
          else
            {
              ...
              // lbl_core_progress->setText(QString("Stage %1 sychronous radial
              // reduction").arg(k+1)); qApp->processEvents();

              // #if defined(USE_THREADS)
              //             unsigned int threads = numThreads;
              //             vector < radial_reduction_thr_t* >
              //             radial_reduction_thr_threads(threads); for (
              //             unsigned int j = 0; j < threads; j++ )
              //             {
              //                radial_reduction_thr_threads[j] = new
              //                radial_reduction_thr_t(j);
              ...
              do
                {
...
#endif
                  // write_bead_tsv(somo_tmp_dir + SLASH +
                  // QString("bead_model_br-%1-%2").arg(k).arg(iter) + DOTSOMO
                  // + ".tsv", &bead_model); write_bead_spt(somo_tmp_dir +
                  // SLASH + QString("bead_model-br-%1-%2").arg(k).arg(iter) +
                  // DOTSOMO, &bead_model);
                  iter++;
#if defined(DEBUG1) || defined(DEBUG)
                  // printf("processing synchronous radial reduction iteration
                  // %d\n", iter);
...
#endif
                  //   lbl_core_progress->setText(QString("Stage %1 synchronous
                  //   radial reduction iteration %2").arg(k+1).arg(iter));
                  //               qApp->processEvents();
                  if (iter > 10000)
                    {
                  ...
                  //                   for ( j = 0; j < threads; j++ )
                  //                   {
                  //                      for ( i = 0; i < my_pairs[j].size();
                  //                      i++ )
                  //                      {
                  //                         pairs.push_back(my_pairs[j][i]);

                  //                         if (my_pairs[j][i].separation >
                  //                         max_intersection_length) {
                  //                            max_intersection_length =
                  //                            my_pairs[j][i].separation;
                  ...
                  for (unsigned int i = 0; i < bead_model.size () - 1; i++)
                    {
                      ...
                      if (1 || last_reduced[i])
                        {
                          for (unsigned int j = i + 1; j < bead_model.size ();
                               j++)
                            {
#if defined(DEBUGX)
                              printf ("checking radial stage %d beads %d %d "
                                      "on chains %d %d exposed code %d %d "
                                      "active %s %s max il %f\n",
                                      k, i, j, bead_model[i].chain,
                                      bead_model[j].chain,
                                      bead_model[i].exposed_code,
                                      bead_model[j].exposed_code,
                                      bead_model[i].active ? "Y" : "N",
...
#if defined(DEBUG)
                                  printf (
                                      "beads %d %d with radii %f %f with "
                                      "coordinates [%f,%f,%f] [%f,%f,%f] have "
                                      "a sep of %f\n",
                                      i, j, bead_model[i].bead_computed_radius,
                                      bead_model[j].bead_computed_radius,
                                      bead_model[i].bead_coordinate.axis[0],
...
#if defined(DEBUG1) || defined(DEBUG)
                  printf ("processing radial reduction sync iteration %d "
                          "pairs to process %d max int len %f\n",
                          iter, count, max_intersection_length);
#endif

                  // lbl_core_progress->setText(QString("Stage %1 synchron.
                  // red. it. %2 with %3
                  // couples").arg(k+1).arg(iter).arg(pairs.size()));
                  // qApp->processEvents();
                      ...
                      for (unsigned int i = 0; i < pairs.size (); i++)
                        {
                              ...
                              if (1)
                                {
                                  ...
                                  if (methods[k] & OUTWARD_TRANSLATION
                                    ...
                                    {
#if defined(DEBUG)
                                      printf ("outward translation from "
                                              "[%f,%f,%f] to ",
                                              bead_model[use_bead]
                                                  .bead_coordinate.axis[0],
                                              bead_model[use_bead]
                          ...
                          if (!reduced[pairs[i].j]
                              ...
                              && (!(methods[k] & RR_MCSC)
                                  ...
                                  || (methods[k] & RR_BURIED
                                      && bead_model[pairs[i].j].exposed_code
                                             != 1)))
                            {
                                 ...
                                 bead_model[pairs[i].j].chain == 1) ) */
                              {
                                ...
                                if (bead_model[use_bead].bead_computed_radius
                                    <= TOLERANCE)
                                  {
                                    // this is to ignore this bead for further
                                    // radial reduction regardless
                                    bead_model[use_bead].bead_computed_radius
                                        = (float)TOLERANCE * 0.9999;
                                    reduced[use_bead] = false;
                                  }
                  ...
                  // write_bead_tsv(somo_tmp_dir + SLASH +
                  // QString("bead_model_ar-%1-%2").arg(k).arg(iter) + DOTSOMO
                  // + ".tsv", &bead_model); write_bead_spt(somo_tmp_dir +
                  // SLASH + QString("bead_model-ar-%1-%2").arg(k).arg(iter) +
                  // DOTSOMO, &bead_model);
                  //               qApp->processEvents();
                  ...
                  //                      {
                  //                         delete
                  //                         radial_reduction_thr_threads[j];
                  //                      }
                  //                   }
                  // #endif
                  //                   return;
      ...
      // #if defined(WRITE_EXTRA_FILES)
      //       write_bead_tsv(somo_tmp_dir + SLASH +
      //       QString("bead_model_rr-%1").arg(k) + DOTSOMO + ".tsv",
      //       &bead_model); write_bead_spt(somo_tmp_dir + SLASH +
      //       QString("bead_model_rr-%1").arg(k) + DOTSOMO, &bead_model);
      // #endif

    } // methods

  // #if defined(WRITE_EXTRA_FILES)
  //    write_bead_tsv(somo_tmp_dir + SLASH + "bead_model_end" + DOTSOMO +
  //    ".tsv", &bead_model); write_bead_spt(somo_tmp_dir + SLASH +
  //    "bead_model_end" + DOTSOMO, &bead_model);
  // #endif
  //    //  write_bead_spt(somo_dir + SLASH + project +
  //    QString("_%1").arg(current_model + 1) +
  //    //       QString(bead_model_suffix.length() ? ("-" + bead_model_suffix)
  ...
  if (us_udp_msg)
    {
      ...
      msging["progress_output"]
          = QString ("AtoB model calculation: %1\%")
                .arg (QString::number (
                    int ((double (end_progress) / double (mppos)) * 100.0)))
                .arg (100); // arg(ppos).arg(mppos);
      msging["progress1"]
          = QString::number (double (end_progress) / double (mppos));
...
int
US_Saxs_Util::overlap_check (bool sc, bool mc, bool buried, double tolerance,
                             int limit)
{
  int retval = 0;
#if defined(DEBUG_OVERLAP)
  printf (
      "overlap checking--overlap tolerance %f--%s-%s-%s--------------------\n",
      tolerance, sc ? "side chain" : "", mc ? "main & side chain" : "",
      buried ? "buried" : "");
#endif
  for (unsigned int i = 0; i < bead_model.size () - 1; i++)
    {
      for (unsigned int j = i + 1; j < bead_model.size (); j++)
        {
          ...
          if (i == 2 && (j == 34 || j == 60))
            printf (
                "x1 overlap check  beads %d %d on chains %d %d exposed code "
                "%d %d active %s %s : radii %f %f with coordinates [%f,%f,%f] "
                "[%f,%f,%f] sep of %f - %s\n",
                i, j, bead_model[i].chain, bead_model[j].chain,
                bead_model[i].exposed_code, bead_model[j].exposed_code,
                bead_model[i].active ? "Y" : "N",
                bead_model[j].active ? "Y" : "N",
                bead_model[i].bead_computed_radius,
                bead_model[j].bead_computed_radius,
                ...
                bead_model[i].bead_computed_radius
                    + bead_model[j].bead_computed_radius
                    - sqrt (pow (bead_model[i].bead_coordinate.axis[0]
                                     - bead_model[j].bead_coordinate.axis[0],
                                 2)
                            + pow (bead_model[i].bead_coordinate.axis[1]
                                       - bead_model[j].bead_coordinate.axis[1],
...
#if defined(DEBUG_OVERLAP_2)
              if (i == 2 && (j == 34 || j == 60))
                printf ("x2 overlap check  beads %d %d on chains %d %d "
                        "exposed code %d %d active %s %s : radii %f %f with "
                        "coordinates [%f,%f,%f] [%f,%f,%f] sep of %f - %s\n",
                        i, j, bead_model[i].chain, bead_model[j].chain,
                        bead_model[i].exposed_code, bead_model[j].exposed_code,
                        bead_model[i].active ? "Y" : "N",
                        bead_model[j].active ? "Y" : "N",
                        ...
                        bead_model[j].bead_coordinate.axis[0],
                        bead_model[j].bead_coordinate.axis[1],
                        bead_model[j].bead_coordinate.axis[2], separation,
              ...
              if (bead_model[i].bead_computed_radius > tolerance * 1.001
                  && bead_model[j].bead_computed_radius > tolerance * 1.001)
                {
                  ...
                  if (limit && retval > limit)
                    {
                      // editor_msg( "red", us_tr( "There are more than %1
                      // overlaps greater than tolerance of %2\nWe suggest you
                      // use ZENO to calculate hydrodynamics for this model"
                      // ).arg( limit ).arg( tolerance ) );
                      return retval;
                    }
                  // QColor save_color = editor->textColor();
                  // editor->setTextColor("red");
                  us_log->log (
                      QString (
                          us_tr ("WARNING: Bead model has an overlap "
                                 "violation on beads %1 %2 overlap %3 A\n"))
                          .arg (i + 1)
                          .arg (j + 1)
                          .arg (separation));
                  if (us_udp_msg)
                    {
                      map<QString, QString> msging;
                      msging["_textarea"]
                          = QString (us_tr ("WARNING: Bead model has an "
                                            "overlap violation on beads %1 %2 "
                                            "overlap %3 A\\n"))
                                .arg (i + 1)
                                .arg (j + 1)
                                .arg (separation);
                  ...
                  accumulated_msgs
                      += QString (
                             us_tr (
                                 "WARNING: Bead model has an overlap "
                                 "violation on beads %1 %2 overlap %3 A\\n"))
                             .arg (i + 1)
                             .arg (j + 1)
                             .arg (separation);
...
#if defined(DEBUG_OVERLAP)
                  printf (
                      "overlap check  beads %d %d on chains %d %d exposed "
                      "code %d %d active %s %s : radii %f(%s) %f(%s) with "
                      "coordinates [%f,%f,%f] [%f,%f,%f] sep of %f - needs "
                      "reduction\n",
                      i + 1, j + 1, bead_model[i].chain, bead_model[j].chain,
                      bead_model[i].exposed_code, bead_model[j].exposed_code,
                      bead_model[i].active ? "Y" : "N",
                      bead_model[j].active ? "Y" : "N",
                      bead_model[i].bead_computed_radius,
                      bead_model[i].bead_computed_radius > tolerance * 1.001
                          ? "gt tol"
                          : "not gt tol",
                      bead_model[j].bead_computed_radius,
                      bead_model[j].bead_computed_radius > tolerance * 1.001
...
#if defined(DEBUG_OVERLAP)
  printf ("end of overlap checking--overlap tolerance "
          "%f--%s-%s-%s--violations %d------------------\n",
          tolerance, sc ? "side chain" : "", mc ? "main & side chain" : "",
          buried ? "buried" : "", retval);
#endif
  return retval;
}
...
int
US_Saxs_Util::calc_hydro ()
{
  return do_calc_hydro ();
}

int
US_Saxs_Util::do_calc_hydro ()
{
  // if ( !overwrite )
  // {
  //    setHydroFile();
      ...
      if (somo_processed[curr_m])
        {
          ...
          models_to_proc++;
          us_log->log (
              QString ("Model %1 will be included\n").arg (curr_m + 1));
          if (us_udp_msg)
            {
              map<QString, QString> msging;
              msging["_textarea"]
          ...
          // write_bead_spt(somo_dir + SLASH + project +
          //          (bead_model_from_file ? "" : QString("_%1").arg(
          //          model_name( current_model ) ) ) +
          //          QString(bead_model_suffix.length() ? ("-" +
          //          bead_model_suffix) : "") + DOTSOMO, &bead_model,
          //          bead_model_from_file);
  ...
  us_log->log (
      QString ("%1")
          //       .arg(hydro.overlap_cutoff ? hydro.overlap :
          //       overlap_tolerance)
          .arg (
              (fabs ((hydro.overlap_cutoff ? hydro.overlap : overlap_tolerance)
                     - overlap_tolerance)
               > 1e-5)
                  ? QString (
                        "\nNotice: Overlap reduction bead overlap tolerance "
                        "%1 does not equal the manually selected hydrodynamic "
                        "calculations bead overlap cut-off %2\n")
  ...
  if (us_udp_msg)
    {
      ...
      msging["_textarea"]
          = QString ("%1")
                //       .arg(hydro.overlap_cutoff ? hydro.overlap :
                //       overlap_tolerance)
                .arg ((fabs ((hydro.overlap_cutoff ? hydro.overlap
                                                   : overlap_tolerance)
                             - overlap_tolerance)
                       > 1e-5)
                          ? QString ("\nNotice: Overlap reduction bead "
                                     "overlap tolerance %1 does not equal the "
                                     "manually selected hydrodynamic "
                                     "calculations bead overlap cut-off %2\\n")
                                .arg (overlap_tolerance)
                                .arg (hydro.overlap)
                          : "");
  ...
  accumulated_msgs
      += QString ("%1")
             ...
             .arg ((fabs ((hydro.overlap_cutoff ? hydro.overlap
                          ...
                          - overlap_tolerance)
                    > 1e-5)
                       ? QString ("\nNotice: Overlap reduction bead overlap "
                                  "tolerance %1 does not equal the manually "
                                  "selected hydrodynamic calculations bead "
                                  "overlap cut-off %2\\n")
                             .arg (overlap_tolerance)
                             .arg (hydro.overlap)
                       : "");
  ...
  //    pb_bead_saxs->setEnabled(true);
  //    pb_rescale_bead_model->setEnabled( misc.target_volume != 0e0 ||
  //    misc.equalize_radii ); pb_show_hydro_results->setEnabled(false);
  //    progress->reset();
  //    return -1;
  // }
  ...
  int retval = us_hydrodyn_supc_main_hydro (
      bead_model_from_file, misc, &results_hydro, &hydro,
      hydro.overlap_cutoff ? hydro.overlap : overlap_tolerance, &bead_models,
      &somo_processed, &model_vector,
      // lb_model,
      QString (project +
               //(bead_model_from_file ? "" : QString("_%1").arg(current_model
               //+ 1)) + (bead_model_from_file ? "" : (models_to_proc == 1 ?
               //"_1" : "_%1")) + (bead_model_from_file ? "" : "_%1") +
               "_%1"
               + QString (bead_model_suffix.length ()
                              ? ("-" + bead_model_suffix)
                              : "")
          ...
          .data (),
      QString (project +
               //(bead_model_from_file ? "" : QString("_%1").arg( model_name(
               //first_model_no - 1 ) ) ) +
  ...
  if (retval)
    {
      ...
      switch (retval)
        ...
        case US_HYDRODYN_SUPC_FILE_NOT_FOUND:
          {
            // printError("Calculate RB Hydrodynamics SMI encountered a file
            // not found error");
            return retval;
            break;
          }
        case US_HYDRODYN_SUPC_OVERLAPS_EXIST:
          {
            // printError("Calculate RB Hydrodynamics SMI encountered overlaps
            // in the bead model\nPerhaps you should Calculate RB Hydrodynamics
            // ZENO which supports bead models with overlaps");
            return retval;
            break;
          }
        case US_HYDRODYN_SUPC_ERR_MEMORY_ALLOC:
          {
            // printError("Calculate RB Hydrodynamics SMI encountered a memory
            // allocation error");
            return retval;
            break;
          }
        case US_HYDRODYN_SUPC_NO_SEL_MODELS:
          {
            // printError("Calculate RB Hydrodynamics SMI was called with no
            // processed models selected");
            return retval;
            break;
          }
        case US_HYDRODYN_PAT_ERR_MEMORY_ALLOC:
          {
            // printError("US_HYDRODYN_PAT encountered a memory allocation
            // error");
            return retval;
            break;
          }
        default:
          {
            // printError("Calculate RB Hydrodynamics SMI encountered an
            // unknown error");
            //  unknown error
            return retval;
            break;
...
QString
US_Saxs_Util::getExtendedSuffix (bool prerun, bool somo)
{
  ...
  if (asa_hydro.calculation)
    {
      result += QString ("A%1").arg (somo ? asa_hydro.threshold
                                          : asa_hydro.grid_threshold);
    }

  if (asa_hydro.recheck_beads)
    {
      result += QString ("R%1").arg (somo ? asa_hydro.threshold_percent
                                          : asa_hydro.grid_threshold_percent);
    }

  if (somo)
    {
      if (sidechain_overlap.remove_overlap && mainchain_overlap.remove_overlap
          && buried_overlap.remove_overlap
          && (sidechain_overlap.remove_sync == mainchain_overlap.remove_sync)
          && (mainchain_overlap.remove_sync == buried_overlap.remove_sync)
          && (sidechain_overlap.remove_hierarch
              == mainchain_overlap.remove_hierarch)
          && (mainchain_overlap.remove_hierarch
              == buried_overlap.remove_hierarch))
        {
          result += QString ("%1").arg (sidechain_overlap.remove_sync ? "sy"
                                                                      : "hi");
        }

      if (sidechain_overlap.remove_overlap && sidechain_overlap.translate_out)
        {
          result += "OT";
        }
    }
  if (!somo)
    {
      if (grid_exposed_overlap.remove_overlap
          && grid_buried_overlap.remove_overlap && grid_overlap.remove_overlap
          && (grid_exposed_overlap.remove_sync
              == grid_buried_overlap.remove_sync)
          && (grid_buried_overlap.remove_sync == grid_overlap.remove_sync)
              ...
              == grid_overlap.remove_hierarch))
        {
          result += QString ("%1").arg (
              grid_exposed_overlap.remove_sync ? "sy" : "hi");
        }
...
QString
US_Saxs_Util::getExtendedSuffix_somo (bool prerun, bool somo,
                                      bool no_ovlp_removal)
{
  // produce a suffix based upon settings
  // e.g.
  // A20 for ASA screen 20
  ...
  if ((asa_hydro.calculation && somo) || (grid_hydro.enable_asa && !somo))
    {
      result += QString ("A%1").arg (somo ? asa_hydro.threshold
                                          : asa_hydro.grid_threshold);
    }
      ...
      if (!no_ovlp_removal)
        {
          if (sidechain_overlap.remove_overlap
            ...
            {
              result += QString ("%1").arg (
                  sidechain_overlap.remove_sync ? "sy" : "hi");
            }
...
void
US_Saxs_Util::write_bead_model (QString fname, vector<PDB_atom> *model,
                                QString extra_text)
{
  ...
  QString fstring_beams = QString ("%.%1f\t%.%2f\t%.%3f\n")
                              .arg (decpts)
                              .arg (decpts)
                              .arg (decpts);

  QString fstring_rmc = QString ("%.%1f\t%.0f\t%d\n")
                            . // zero digit MW
                        arg (decpts);

  QString fstring_rmc1 = QString ("%.%1f\t%.0f\t%d\t%s\n")
                             . // zero digit MW
                         arg (decpts);

  QString fstring_hydro = QString ("%.%1f\t%.%2f\t%.%3f\t%.%4f\n")
                              .arg (decpts)
                              .arg (decpts)
                              .arg (decpts)
                              .arg (decpts);
    ...
    case 1: // exposed sidechain -> exposed main chain -> buried
      {
        ...
        for (unsigned int i = 0; i < model->size (); i++)
          {
            if ((*model)[i].visibility == 1 && (*model)[i].chain == 1)
              {
                use_model.push_back (&(*model)[i]);
                used[i] = true;
              }
        ...
        for (unsigned int i = 0; i < model->size (); i++)
          {
            if ((*model)[i].visibility == 1 && (*model)[i].chain == 0)
              {
                use_model.push_back (&(*model)[i]);
                used[i] = true;
              }
        ...
        for (unsigned int i = 0; i < model->size (); i++)
          {
            if ((*model)[i].visibility == 0 && (*model)[i].chain == 1)
              {
                use_model.push_back (&(*model)[i]);
                used[i] = true;
              }
        ...
        for (unsigned int i = 0; i < model->size (); i++)
          {
            if ((*model)[i].visibility == 0 && (*model)[i].chain == 0)
              {
                use_model.push_back (&(*model)[i]);
                used[i] = true;
              }
  ...
  if (bead_output.output & US_HYDRODYN_OUTPUT_SOMO)
    {
      fsomo = us_fopen (
          QString ("%1.bead_model").arg (fname).toLatin1 ().data (), "w");
    }
  if (bead_output.output & US_HYDRODYN_OUTPUT_BEAMS)
    {
      fbeams = us_fopen (QString ("%1.beams").arg (fname).toLatin1 ().data (),
                         "w");
      frmc
          = us_fopen (QString ("%1.rmc").arg (fname).toLatin1 ().data (), "w");
      frmc1 = us_fopen (QString ("%1.rmc1").arg (fname).toLatin1 ().data (),
                        "w");
    }
  if (bead_output.output & US_HYDRODYN_OUTPUT_HYDRO)
    {
      fhydro
          = us_fopen (QString ("%1.dat").arg (fname).toLatin1 ().data (), "w");
    }
  ...
  if (fsomo)
    {
      fprintf (fsomo, "%d\t%.3f\n", beads, results_hydro.vbar);
    }

  if (fbeams)
    {
      fprintf (fbeams, "%d\t-2.000000\t%s.rmc\t%.3f\n", beads,
      ...
      if (use_model[i]->active)
        {
          ...
          if (!bead_model_from_file)
            {
              residues = use_model[i]->resName
                         + (use_model[i]->org_chain
                                ? ".SC."
                                : ((misc.pb_rule_on && !use_model[i]->type)
                                       ? ".PB."
                                       : ".MC."))
                         + (use_model[i]->chainID == " "
                                ? ""
              ...
              for (unsigned int j = 0; j < use_model[i]->all_beads.size ();
                   j++)
                {
                  ...
                  residues
                      += ","
                         + (use_model[i]->all_beads[j]->resName
                            + (use_model[i]->all_beads[j]->org_chain
                                   ? ".SC."
                                   : ((misc.pb_rule_on && !use_model[i]->type)
                                          ? ".PB."
                                          : ".MC."))
                            + (use_model[i]->all_beads[j]->chainID == " "
                                   ? ""
                                   : (use_model[i]->all_beads[j]->chainID
                                      + ".")));
                  // a compiler error forced this kludge using tmp_serial
          ...
          if (fsomo)
            {
              fprintf (fsomo, fstring_somo.toLatin1 ().data (),
          ...
          if (fbeams)
            {
              fprintf (fbeams, fstring_beams.toLatin1 ().data (),
                       use_model[i]->bead_coordinate.axis[0],
                       use_model[i]->bead_coordinate.axis[1],
                       use_model[i]->bead_coordinate.axis[2]);
              fprintf (frmc, fstring_rmc.toLatin1 ().data (),
                       use_model[i]->bead_computed_radius,
                       use_model[i]->bead_ref_mw,
                       get_color_util_saxs (use_model[i]));
              fprintf (frmc1, fstring_rmc1.toLatin1 ().data (),
                       use_model[i]->bead_computed_radius,
                       use_model[i]->bead_ref_mw,
                       get_color_util_saxs (use_model[i]),
                       residues.toLatin1 ().data ());
          ...
          if (fhydro)
            {
              fprintf (fhydro, fstring_hydro.toLatin1 ().data (),
                       use_model[i]->bead_coordinate.axis[0],
                       use_model[i]->bead_coordinate.axis[1],
                       use_model[i]->bead_coordinate.axis[2],
                       use_model[i]->bead_computed_radius);
  ...
  if (fsomo)
    {
      fprintf (
          ...
          -hydro.unit);
      fprintf (fsomo,
               (options_log.isNull () ? "" : options_log.toLatin1 ().data ()));
      fprintf (
          fsomo,
          (last_abb_msgs.isNull () ? "" : last_abb_msgs.toLatin1 ().data ()));
...
void
US_Saxs_Util::append_options_log_atob ()
{
  QString s;
  ...
  s.sprintf ("ASA Calculation:\n"
             ...
             asa_hydro.probe_radius, asa_hydro.probe_recheck_radius,
             asa_hydro.grid_threshold, asa_hydro.grid_threshold_percent,
  ...
  s.sprintf (
      ...
      grid_hydro.center
          ? "Center of Mass"
          : (grid_hydro.center == 2 ? "Center of Scattering Intensity"
                                    : "Center of Cubelet"),
      grid_hydro.cube_side, grid_hydro.cubic ? "On" : "Off",
      grid_hydro.hydrate ? "On" : "Off", grid_hydro.tangency ? "On" : "Off",
      ...
      ,
      grid_overlap.fuse_beads ? "On" : "Off", grid_overlap.fuse_beads_percent,
  ...
  s.sprintf ("Miscellaneous options:\n"
             "  Calculate vbar                 %s\n",
  ...
  if (!misc.compute_vbar)
    {
      s.sprintf ("  Entered vbar value             %.3f\n"
                 "  Vbar measured/computed at T=   %.2f\n",
                 misc.vbar, misc.vbar_temperature);
      options_log += s;
    }
}

void
US_Saxs_Util::append_options_log_somo ()
{
  QString s;
  ...
  s.sprintf ("ASA Calculation:\n"
             ...
             asa_hydro.method ? "Rolling Sphere" : "Voronoi Tesselation",
             asa_hydro.probe_radius, asa_hydro.probe_recheck_radius,
             asa_hydro.threshold, asa_hydro.threshold_percent,
             asa_hydro.asab1_step);
  options_log += s;

  s.sprintf ("Overlap Reduction:\n"
             "  Bead Overlap Tolerance:     %f\n"
  ...
  s.sprintf ("  Enable Peptide Bond Rule       %s\n",
             misc.pb_rule_on ? "On" : "Off");
  options_log += s;
}

void
US_Saxs_Util::append_options_log_somo_ovlp ()
{
  QString s;
  ...
  s.sprintf ("ASA Calculation:\n"
             ...
             asa_hydro.calculation ? "On" : "Off",
             asa_hydro.method ? "Rolling Sphere" : "Voronoi Tesselation",
             asa_hydro.probe_radius, asa_hydro.threshold,
             asa_hydro.threshold_percent, asa_hydro.asab1_step);
  options_log += s;
...
int
US_Saxs_Util::get_color_util_saxs (PDB_atom *a)
{
...
void
US_Saxs_Util::reload_pdb (bool parameters_set_first_model)
{
  // if ( advanced_config.debug_1 )
  // {
  ...
  //    !pb_somo->isEnabled());
  // }
  // if ( pdb_file.isEmpty() ) // why did we ever do this ? ||
  // !pb_somo->isEnabled() )
  // {
  //    return;
  // }
      ...
      if (check_for_missing_atoms_hydro (&error_string, &model_vector[i],
                                         parameters_set_first_model))
        {
          ...
          if (us_udp_msg)
            {
              map<QString, QString> msging;
              msging["_textarea"] = QString ("Encountered errors with your "
                                             "PDB structure for model %1:\\n")
                                        .arg (i)
                                    + error_string_udp;
          ...
          accumulated_msgs += QString ("Encountered errors with your PDB "
                                       "structure for model %1:\\n")
                                  .arg (i)
                              + error_string_udp;
          // printError(QString("Encountered errors with your PDB structure for
...
bool
US_Saxs_Util::calc_somo_o (bool parameters_set_first_model)
{
  bool use_parameters_set_first_model = parameters_set_first_model;
...
int
US_Saxs_Util::calc_somo (bool no_ovlp_removal, bool parameters_set_first_model)
{
  // if ( selected_models_contain( "WAT" ) )
  // {
  //    QMessageBox::warning( this,
  //                          us_tr( "Selected model contains WAT residue" ),
  //                          us_tr(
  //                             "Can not process models that contain the WAT
  //                             residue.\n" "These are currently generated
  //                             only for SAXS/SANS computations\n"
  //                             )
  //                          );
  ...
  options_log = "";
  no_ovlp_removal ? append_options_log_somo_ovlp ()
  ...
  // {
  //    us_log->log("Stopped by user\n\n");
  //    bd_anaflex_enables( ( ( browflex && browflex->state() ==
  //    QProcess::Running ) ||
  //                          ( anaflex && anaflex->state() ==
  //                          QProcess::Running ) ) ? false : true );
  //    pb_somo->setEnabled(true);
  //    pb_somo_o->setEnabled(true);
  //    pb_grid_pdb->setEnabled(true);
  //    progress->reset();
  ...
  //     !model_vector.size())
  // {
  //    fprintf(stderr, "calculations can not be run until residue & pdb files
  //    are read!\n"); pb_stop_calc->setEnabled(false); return -1;
  // }
  ...
  QString extra_text = "";
  bool use_parameters_set_first_model = parameters_set_first_model;
  for (current_model = 0; current_model < model_vector.size ();
       current_model++)
    {
      if (!any_errors) //&& lb_model->item(current_model)->isSelected())
        {
          ...
          if (!compute_asa (false, no_ovlp_removal,
                            use_parameters_set_first_model))
            {
              somo_processed[current_model] = 1;
                  ...
                  if (us_udp_msg)
                    {
                      map<QString, QString> msging;
                      msging["progress_output"]
                          = QString ("SOMO model calculation: %1\%")
                                .arg (QString::number (
                                    int ((double (ppos) / double (mppos))
                                         * 100.0))); // arg(ppos).arg(mppos);
                      msging["progress1"]
                          = QString::number (double (ppos) / double (mppos));
      ...
      QString filename = project + QString ("_%1").arg (current_model + 1);
      ...
      write_bead_model (
          ...
          filename
              + QString (
                  bead_model_suffix.length () ? ("-" + bead_model_suffix) : "")
              + DOTSOMO,
          &bead_model, extra_text);
...
int
US_Saxs_Util::compute_asa (bool bd_mode, bool no_ovlp_removal,
                           bool parameters_set_first_model)
{
  // cout << "CURRENT_MODEL: " << current_model << endl;
  ...
  if (check_for_missing_atoms_hydro (&error_string,
                                     &model_vector[current_model],
                                     use_parameters_set_first_model))
    {
      us_log->log (
          "Encountered the following errors with your PDB structure:\n"
          + error_string);
  ...
  if (us_udp_msg)
    {
      ...
      msging["progress_output"]
          = QString ("SOMO model calculation: %1\%")
                .arg (
                    QString::number (int ((double (ppos) / double (mppos))
                                          * 100.0))); // arg(ppos).arg(mppos);
    ...
    if (us_udp_msg)
      {
        map<QString, QString> msging;
        msging["_textarea"]
            = QString ("There are %1 atoms in %2 chain(s) in this model\\n")
                  .arg (no_of_atoms)
                  .arg (no_of_molecules);
  ...
  //    //    case US_SURFRACER_ERR_MEMORY_ALLOC:
  //    //       {
  //    //          printError("US_SURFRACER encountered a memory allocation
  //    error");
  //    //          return US_SURFRACER_ERR_MEMORY_ALLOC;
  //    //          break;
  //    //       }
  ...
  if (asa_hydro.method == 1)
    {
      ...
      int retval = us_hydrodyn_asab1_main_hydro (active_atoms, &asa_hydro,
                                                 &results_hydro,
                                                 false); // ,
                                                         // progress,
                                                         // editor,
      ...
      if (retval)
        {
          ...
          switch (retval)
            ...
            case US_HYDRODYN_ASAB1_ERR_MEMORY_ALLOC:
              {
                // printError("US_HYDRODYN_ASAB1 encountered a memory
                // allocation error");
            ...
            default:
              {
                // printError("US_HYDRODYN_ASAB1 encountered an unknown
                // error");
                //  unknown error
                return -1;
                break;
  ...
  // for (unsigned int i = 0; i < model_vector.size (); i++)
  {
    ...
    for (unsigned int j = 0; j < model_vector[i].molecule.size (); j++)
      {
        for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size ();
             k++)
          {
            PDB_atom *this_atom = &(model_vector[i].molecule[j].atom[k]);
            // printf("p1 i j k %d %d %d %lx %s\n", i, j, k, (long unsigned
            // int)this_atom->p_atom, this_atom->active ? "active" : "not
            // active"); fflush(stdout);

            //   this_atom->bead_assignment =
            //     (this_atom->p_atom ? (int)
            //     this_atom->p_atom->bead_assignment : -1);
            //   printf("this_atom->bead_assignment %d\n",
            //   this_atom->bead_assignment); fflush(stdout); this_atom->chain
            //   =
            //     ((this_atom->p_residue && this_atom->p_atom) ?
            //      (int)
            //      this_atom->p_residue->r_bead[this_atom->p_atom->bead_assignment].chain
            //      : -1);
            ...
            fprintf (asaf, "%s\t%s\t%s\t%.2f\n",
                     this_atom->name.toLatin1 ().data (),
                     this_atom->resName.toLatin1 ().data (),
                     this_atom->resSeq.toLatin1 ().data (), this_atom->asa);
  ...
  // #define DEBUG
  // pass 2 determine beads, cog_position, fixed_position, molecular cog
  // phase 1.
    ...
    for (unsigned int j = 0; j < model_vector[i].molecule.size (); j++)
      {
        ...
        for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size ();
          ...
          {
            PDB_atom *this_atom = &(model_vector[i].molecule[j].atom[k]);
            // printf("p2 i j k %d %d %d %lx\n", i, j, k, this_atom->p_atom);
            // fflush(stdout); this_atom->bead_positioner = false;
            if (this_atom->active)
              {
                // if ( advanced_config.debug_1 )
                // {
                ...
                // we want to put the N on a previous bead unless it is the
                // first one of the molecule ONLY FOR residue type = 0! (amino
                // acid) AND ONLY for residues not part of the 'auto bead
                // builder'
                ...
                if (!create_beads_normally
                    || ((this_atom->bead_assignment != last_bead_assignment
                        ...
                        && !(misc.pb_rule_on && this_atom->chain == 0
                             && this_atom->name == "N" && count_actives
                             && !broken_chain_head.count (
                                 QString ("%1|%2")
                                     .arg (this_atom->resSeq)
                                     .arg (this_atom->resName)))))
                  {
                    ...
                    if (create_beads_normally && misc.pb_rule_on
                        && !this_atom->type)
                      {
                        if (sidechain_N && this_atom->chain == 1)
                          {
                            if (this_atom->name == "N")
                              {
                                printf ("ERROR double N on sequential "
                                        "sidechains! PRO PRO?\n");
                              }
                            // if ( advanced_config.debug_1 ||
                            //      advanced_config.debug_2 )
                            // {
                            //    printf("adding sidechain N %f %f to
                            //    this_atom\n",
                            //           sidechain_N->asa,
                            //           sidechain_N->mw
                            //           );
                        ...
                        if (this_atom->name == "N" && this_atom->chain == 1)
                          {
                    ...
                    if (this_atom->bead_positioner)
                      {
                        ...
                        if (last_main_bead->bead_positioner
                            && this_atom->placing_method == 1)
                          {
                            fprintf (
                                stderr,
                                "warning: 2 positioners in bead %s %s %d\n",
                        ...
                        last_main_bead->bead_position_coordinate
                            = this_atom->coordinate;
                      }
                ...
                PDB_atom *use_atom;
                if (misc.pb_rule_on && create_beads_normally
                    && this_atom->chain == 0 && this_atom->name == "N"
                    && !broken_chain_head.count (QString ("%1|%2")
                                                     .arg (this_atom->resSeq)
                                                     .arg (this_atom->resName))
                ...
                // accum
                if (!create_beads_normally || this_atom->bead_positioner)
                  {
                    // if ( advanced_config.debug_3 )
                    // {
                    //    printf("adding cog from %d to %d mw %f totmw %f (this
                    //    pos [%f,%f,%f], org pos [%f,%f,%f])\n",
                    //           this_atom->serial,
                    //           use_atom->serial,
                    //           this_atom->mw,
                ...
                //    if ( advanced_config.debug_3 )
                //    {
                //       printf("notice: atom %s %s %d excluded from cog
                //       calculation in bead %s %s %d\n",
                //              this_atom->name.toLatin1().data(),
                //              this_atom->resName.toLatin1().data(),
                //              this_atom->serial,
                ...
                if (!create_beads_normally || this_atom->bead_positioner)
                  {
                    if (use_atom->bead_positioner
                      ...
                      {
                        fprintf (stderr,
                                 "warning: 2 or more positioners in bead %s "
                                 "%s %d\n",
                                 use_atom->name.toLatin1 ().data (),
                                 use_atom->resName.toLatin1 ().data (),
                                 use_atom->serial);
                ...
                if (this_atom->chain == 0 && misc.pb_rule_on
                    && this_atom->name == "N"
                    && !broken_chain_head.count (QString ("%1|%2")
                                                     .arg (this_atom->resSeq)
                                                     .arg (this_atom->resName))
                ...
                if (this_atom->chain == 0 && this_atom->name == "CA")
                  {
    ...
    for (unsigned int j = 0; j < model_vector[i].molecule.size (); j++)
      {
        ...
        for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size ();
             k++)
          {
            ...
            // if ( advanced_config.debug_1 )
            // {
            //    printf("pass 2b active %d is_bead %d chain %d atom %s %s %d
            //    pm %d\n",
            //           this_atom->active,
            //           this_atom->is_bead,
            //           this_atom->chain,
            ...
            // }
            if (this_atom->active && this_atom->is_bead
                && this_atom->chain == 0)
              {
                main_chain_beads++;
              }
            if (misc.pb_rule_on && !k && this_atom->resName == "PRO")
              {
                // if ( advanced_config.debug_1 )
                // {
            ...
            if (placed_N1
                                               ...
                                               .arg (this_atom->resName)))
              {
                ...
                // if ( advanced_config.debug_1 )
                // {
                //    printf("placed N1, found break, turned off placed N1 %s
                //    %s\n",
                //           this_atom->resName.toLatin1().data(),
                //           this_atom->resSeq.toLatin1().data()
                //           );
            ...
            if (first_is_pro && this_atom->active && this_atom->is_bead
                && this_atom->chain == 1 && this_atom->resName == "PRO")
              {
                // if ( advanced_config.debug_1 )
                // {
            ...
            if (this_atom->name == "OXT" && last_main_chain_bead)
              {
                // if ( advanced_config.debug_1 )
                // {
                //    printf("pass 2b active OXT %s %s %d last %s %s %d mw org
                //    %f mw new %f\n",
                //           this_atom->name.toLatin1().data(),
                //           this_atom->resName.toLatin1().data(),
                //           this_atom->serial,
                ...
                if (misc.pb_rule_on && this_atom->resName != "PRO"
                    && broken_chain_head.count (QString ("%1|%2")
                                                    .arg (this_atom->resSeq)
                                                    .arg (this_atom->resName))
                    && multi_residue_map.count ("NPBR-OXT"))
                  {
                    ...
                    this_atom->bead_ref_volume
                        = residue_list[posNPBR_OXT].r_bead[0].volume;
                    this_atom->bead_ref_mw
                        = residue_list[posNPBR_OXT].r_bead[0].mw;
                  }

                last_main_chain_bead->bead_ref_volume
                    = this_atom->bead_ref_volume;
                ...
                if (!misc.pb_rule_on && main_chain_beads == 1
                    && this_atom->resName != "PRO")
                  {
                    // if ( advanced_config.debug_1 )
                    // {
                ...
                last_main_chain_bead->bead_computed_radius
                    = this_atom->bead_computed_radius;
              } // OXT
            ...
            if (this_atom->active && this_atom->is_bead
                && this_atom->chain == 0)
              {
                // if ( advanced_config.debug_1 )
                // {
                //    printf("pass 2b active, bead, chain == 0 %s %s %d pm
                //    %d\n",
                //           this_atom->name.toLatin1().data(),
                //           this_atom->resName.toLatin1().data(),
                //           this_atom->serial,
                ...
                if (misc.pb_rule_on && last_main_chain_bead
                    && !broken_chain_head.count (QString ("%1|%2")
                                                     .arg (this_atom->resSeq)
                                                     .arg (this_atom->resName))
                    && (this_atom->resName == "PRO"
            ...
            if (misc.pb_rule_on && this_atom->p_residue->type == 0
                && this_atom->is_bead && this_atom->chain == 0
                && broken_chain_end.count (QString ("%1|%2")
                                               .arg (this_atom->resSeq)
                                               .arg (this_atom->resName))
                && !broken_chain_head.count (QString ("%1|%2")
                ...
                && (k || this_atom->name != "N"))
              {
                // if ( advanced_config.debug_1 )
                // {
                //    printf("pass 2b broken end adjustment %s %s %s org mw
                //    %.4f vol %.4f\n",
                //           this_atom->name.toLatin1().data(),
                //           this_atom->resName.toLatin1().data(),
                //           this_atom->resSeq.toLatin1().data(),
                ...
                if (multi_residue_map["PBR-NO-OXT"].size () == 1)
                  {
                    ...
                    this_atom->bead_ref_volume
                        = residue_list[pos].r_bead[0].volume;
                    this_atom->bead_ref_mw = residue_list[pos].r_bead[0].mw;
                    this_atom->bead_computed_radius
                        = pow (3 * last_main_chain_bead->bead_ref_volume
                                   / (4.0 * M_PI),
                               1.0 / 3);
                    if (this_atom->resName == "GLY")
                      {
                    ...
                    // if ( advanced_config.debug_1 )
                    // {
                    //    printf("pass 2b broken end adjustment %s %s %s new mw
                    //    %.4f vol %.4f\n",
                    //           this_atom->name.toLatin1().data(),
                    //           this_atom->resName.toLatin1().data(),
                    //           this_atom->resSeq.toLatin1().data(),
                ...
                else
                  {
                    //    QColor save_color = editor->textColor();
                    //    editor->setTextColor("red");
                    us_log->log ("Chain has broken end and PBR-NO-OXT isn't "
                                 "uniquely defined in the residue file.");
                    if (us_udp_msg)
                      {
                        map<QString, QString> msging;
                        msging["_textarea"]
                            = "Chain has broken end and PBR-NO-OXT isn't "
                              "uniquely defined in the residue file.";
            ...
            if (multi_residue_map["PBR-NO-OXT"].size () == 1)
              {
                ...
                last_main_chain_bead->bead_ref_volume
                    = residue_list[pos].r_bead[0].volume;
                last_main_chain_bead->bead_ref_mw
                    = residue_list[pos].r_bead[0].mw;
                last_main_chain_bead->bead_computed_radius = pow (
                    3 * last_main_chain_bead->bead_ref_volume / (4.0 * M_PI),
                    1.0 / 3);
                if (last_main_chain_bead->resName == "GLY")
                  {
                    last_main_chain_bead->bead_ref_mw += 1.01f;
            ...
            else
              {
                //    QColor save_color = editor->textColor();
                //    editor->setTextColor("red");
                us_log->log ("Chain has no terminating OXT and PBR-NO-OXT "
                             "isn't uniquely defined in the residue file.");
                if (us_udp_msg)
                  {
                    map<QString, QString> msging;
                    msging["_textarea"]
                        = "Chain has no terminating OXT and PBR-NO-OXT isn't "
                          "uniquely defined in the residue file.";
            ...
            if (this_atom->active && this_atom->is_bead)
              {
                ...
                printf ("pass 2c hydration %s %s %d pm %d\n",
                        this_atom->name.toLatin1 ().data (),
                        this_atom->resName.toLatin1 ().data (),
                        this_atom->serial, this_atom->placing_method);
                fflush (stdout);
#endif
                this_atom->bead_ref_volume_unhydrated
                    = this_atom->bead_ref_volume;
                this_atom->bead_ref_volume
                    += misc.hydrovol * this_atom->bead_hydration;
                this_atom->bead_computed_radius = pow (
                    3 * this_atom->bead_ref_volume / (4.0 * M_PI), 1.0 / 3);
              }
          }
      }
  }

  // pass 3 determine visibility, exposed code, normalize cog position, final
  // position determination compute com of entire molecule
    ...
    for (unsigned int j = 0; j < model_vector[i].molecule.size (); j++)
      {
        for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size ();
             k++)
          {
            ...
            if (this_atom->active && this_atom->chain == 0)
              {
#if defined(DEBUG)
...
#warning broken by resSeq->QString
            ...
            if (this_atom->active && this_atom->is_bead)
              {
                ...
                for (unsigned int m = 0; m < 3; m++)
                  {
                    if (this_atom->bead_cog_mw)
                      {
                        this_atom->bead_cog_coordinate.axis[m]
                            /= this_atom->bead_cog_mw;
                      }
                    else
                      {
                        this_atom->bead_cog_coordinate.axis[m] = 0;
                ...
                if (this_atom->p_residue && this_atom->p_atom)
...
#if defined(DEBUG)
                    printf ("pass 3 active is bead %s %s %d bead assignment "
                            "%d placing method %d\n",
                            this_atom->name.toLatin1 ().data (),
                            this_atom->resName.toLatin1 ().data (),
                            this_atom->serial, this_atom->bead_assignment,
                            this_atom->placing_method);
                    fflush (stdout);
#endif
                    switch (this_atom->placing_method)
                      ...
                      case 0: // cog
                        this_atom->bead_coordinate
                            = this_atom->bead_cog_coordinate;
                        // if (this_atom->bead_positioner) {
                        // fprintf(stderr, "warning: this bead had a atom
                        // claiming position & a bead placing method of cog! %s
                        // %s %d\n",
                        //   this_atom->name.toLatin1().data(),
                        //   this_atom->resName.toLatin1().data(),
                        //   this_atom->serial);
                      ...
                      case 1: // positioner
                        this_atom->bead_coordinate
                            = this_atom->bead_position_coordinate;
                        break;
                      ...
                      default:
                        this_atom->bead_coordinate
                            = this_atom->bead_cog_coordinate;
                        fprintf (stderr,
                                 "warning: unknown bead placing method %d %s "
                                 "%s %d <using cog!>\n",
                                 this_atom->placing_method,
                                 this_atom->name.toLatin1 ().data (),
                                 this_atom->resName.toLatin1 ().data (),
                ...
                else
                  {
                    fprintf (
                        stderr,
                        "serious internal error 1 on %s %s %d, quitting\n",
                        this_atom->name.toLatin1 ().data (),
                        this_atom->resName.toLatin1 ().data (),
                        this_atom->serial);
                    exit (-1);
                ...
                this_atom->visibility
                    = (this_atom->bead_asa >= asa_hydro.threshold);
#if defined(OLD_ASAB1_SC_COMPUTE)
                if (this_atom->chain == 1)
                  {
                    printf ("visibility was %d is ", this_atom->visibility);
                    this_atom->visibility
                        = (this_atom->bead_asa + bead_mc_asa[this_atom->resSeq]
                           >= asa_hydro.threshold);
                    printf ("%d\n", this_atom->visibility);
                  }
#endif
                ...
                fflush (stdout);
#endif
                if (!create_beads_normally || this_atom->visibility
  ...
  printf ("model~molecule~atom~name~residue~chainID~"
          ...
          "bead mw~position controlled?~bead positioner~baric method~bead "
          "hydration~bead color~"
    ...
    for (unsigned int j = 0; j < model_vector[i].molecule.size (); j++)
      {
        for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size ();
             k++)
          {
            ...
            // printf("model %d mol %d atm %d nam %s res %s xyz [%f,%f,%f] act
            // %s rads %f asa %f bead # %d chain %d serl %d is_bead %s bead_asa
            // %f vis %s code %d pos? %s pos_co [%f,%f,%f] cog [%f,%f,%f] use
            // [%f, %f, %f]\n",
            printf (
                "%d~%d~%d~%s~%s~%s~"
                "[%f,%f,%f]~%s~%f~%f~%f~"
                ...
                i, j, k, this_atom->name.toLatin1 ().data (),
                this_atom->resName.toLatin1 ().data (),
                this_atom->chainID.toLatin1 ().data (),

                this_atom->coordinate.axis[0], this_atom->coordinate.axis[1],
                this_atom->coordinate.axis[2], this_atom->active ? "Y" : "N",
                this_atom->radius, this_atom->asa, this_atom->mw,

                this_atom->bead_assignment, this_atom->chain,
                this_atom->serial, this_atom->is_bead ? "Y" : "N",
                this_atom->bead_asa, this_atom->visibility ? "Y" : "N",

                this_atom->exposed_code, this_atom->bead_mw,
                this_atom->bead_positioner ? "Y" : "N",
                this_atom->active ? (this_atom->bead_positioner ? "Y" : "N")
                                  : "Inactive",
                this_atom->placing_method, this_atom->bead_hydration,
                this_atom->bead_color,

                this_atom->bead_ref_volume, this_atom->bead_ref_mw,
                this_atom->bead_computed_radius,
                fabs (this_atom->bead_ref_mw - this_atom->bead_mw) < .03 ? "Y"
                                                                         : "N",
            ...
            if (this_atom->active && this_atom->is_bead)
              {
                ...
                if (bead_count)
                  {
                    printf ("res %s %s bead %d bead_mw %f sum atom mw %f diff "
                            "%f\n",
                            last_res.toLatin1 ().data (),
                            last_resName.toLatin1 ().data (), last_asgn,
                            prev_bead_mw, prev_atom_mw,
                            prev_bead_mw - prev_atom_mw);
                  }
                prev_bead_mw = 0;
                prev_atom_mw = 0;
  ...
  printf ("res %d %s bead %d bead_mw %f sum atom mw %f diff %f\n", last_res,
          last_resName.toLatin1 ().data (), last_asgn, prev_bead_mw,
          prev_atom_mw, prev_bead_mw - prev_atom_mw);
  printf ("~~tot atom %d mw %f tot bead %d mw %f beads_size %u\n", atom_count,
          tot_atom_mw, bead_count, tot_bead_mw,
  ...
  // #if defined(DEBUG_MOD)
  //    write_bead_tsv(somo_tmp_dir + SLASH + "bead_model_debug" + DOTSOMO +
  //    ".tsv", &dbg_model);
  // #endif
...
#define MIN_OVERLAP 0.0
  ...
  if (!bd_mode && sidechain_overlap.fuse_beads)
    {
      methods[0] |= POP_SC | POP_EXPOSED;
    }

  if (!bd_mode && mainchain_overlap.fuse_beads)
    {
      methods[1] |= POP_MCSC | POP_EXPOSED;
    }

  if (!bd_mode && buried_overlap.fuse_beads)
    {
      methods[2] |= POP_ALL | POP_BURIED;
    }
                  ...
                  for (unsigned int j = i + 1; j < bead_model.size (); j++)
                    {
                      ...
                      printf (
                          "checking radial stage %d beads %d %d on chains %d "
                          "%d exposed code %d %d active %s %s max il %f\n",
                          k, i, j, bead_model[i].chain, bead_model[j].chain,
                          bead_model[i].exposed_code,
                          bead_model[j].exposed_code,
                          bead_model[i].active ? "Y" : "N",
...
#if defined(USE_THREADS)
              unsigned int threads = numThreads;
              vector<radial_reduction_thr_t *> radial_reduction_thr_threads (
                  threads);
              for (unsigned int j = 0; j < threads; j++)
                {
                  radial_reduction_thr_threads[j]
                      = new radial_reduction_thr_t (j);
                  radial_reduction_thr_threads[j]->start ();
                }
#endif
              do
...
#if defined(DEBUG1) || defined(DEBUG)
                  printf (
                      "processing synchronous radial reduction iteration %d\n",
                      iter);
#endif
                  // lbl_core_progress->setText(QString("Stage %1 synchronous
                  // radial reduction iteration %2").arg(k+1).arg(iter));
...
#endif
                        radial_reduction_thr_threads[j]
                            ->radial_reduction_thr_setup (
                                methods[k], &bead_model, &last_reduced,
                                &reduced, &my_pairs[j], threads,
                                overlap_tolerance);
                      }
                      ...
                      for (unsigned int i = 0; i < pairs.size (); i++)
                        {
                          if (!reduced[pairs[i].i]
                              ...
                              && (!(methods[k] & RR_MCSC)
                                  || bead_model[pairs[i].i].chain == 0
                                  || (methods[k] & RR_BURIED
                                      && bead_model[pairs[i].i].exposed_code
                                             != 1)))
                            {
                              int use_bead = pairs[i].i;
                              /*      if ( !(methods[k] & RR_MCSC) ||
      ...
      if (us_udp_msg)
        {
          map<QString, QString> msging;
          msging["_textarea"]
              = "Finished with popping and radial reduction\\n";
  ...
  if (us_udp_msg)
    {
      ...
      msging["progress_output"]
          = QString ("SOMO model calculation: %1\%")
                .arg (QString::number (
                    int ((double (mppos - (asa_hydro.recheck_beads ? 1 : 0))
                          / double (mppos))
                         * 100.0))); // arg(ppos).arg(mppos);
      msging["progress1"] = QString::number (
          double (mppos - (asa_hydro.recheck_beads ? 1 : 0)) / double (mppos));
      ...
      if (us_udp_msg)
        {
          ...
          msging["progress_output"]
              = QString ("SOMO model calculation: %1\%")
                    .arg (QString::number (100)); // arg(ppos).arg(mppos);
...
bool
US_Saxs_Util::read_pdb_hydro (QString filename,
                              bool parameters_set_first_model)
{
  errormsg = "";
  noticemsg = "";
  ...
  model_vector.clear ();
  bead_model.clear ();
  QString last_resSeq = ""; // keeps track of residue sequence number,
                            // initialize to zero, first real one will be "1"
      ...
      while (!ts.atEnd ())
        {
              ...
              if (chain_flag)
                {
                  if (temp_chain.atom.size ())
                    {
                      sulfur_pdb_chain_idx[temp_chain.atom[0].chainID]
                          .push_back (
                              (unsigned int)temp_model.molecule.size ());
                    }
          ...
          if (str1.left (6) == "HEADER" || str1.left (5) == "TITLE")
            {
              ...
              us_log->log (
                  QString ("PDB %1: %2").arg (str1.left (6)).arg (tmp_str));
              if (us_udp_msg)
                {
                  map<QString, QString> msging;
          ...
          if (str1.left (5) == "MODEL"
              || (str1.left (4) == "ATOM"
                  && last_was_ENDMDL)) // we have a new model in a multi-model
                                       // file
            {
              last_was_ENDMDL = false;
              model_flag = true; // we are using model descriptions (possibly
                                 // multiple models)
              // str2 = str1.mid(6, 15);
          ...
          if (str1.left (6)
              == "ENDMDL") // we need to save the previously recorded molecule
            {
              last_was_ENDMDL = true;
              if (temp_chain.atom.size ())
                {
                  sulfur_pdb_chain_idx[temp_chain.atom[0].chainID].push_back (
                      (unsigned int)temp_model.molecule.size ());
                }
              temp_model.molecule.push_back (
                  temp_chain); // add the last chain of this model
              ...
              for (unsigned int i = 0; i < temp_model.molecule.size (); i++)
                {
                  QString lastResSeq = "";
                  for (unsigned int j = 0;
                       j < temp_model.molecule[i].atom.size (); j++)
                    {
                      if (temp_model.molecule[i].atom[j].resSeq != lastResSeq)
                        {
                          str += temp_model.molecule[i].atom[j].resName + " ";
                          sstr
                              += residue_short_names.count (
                                     temp_model.molecule[i].atom[j].resName)
                                     ? QString (
                                           residue_short_names[temp_model
                                                                   .molecule[i]
                                                                   .atom[j]
                                                                   .resName])
                                     : "?";
              ...
              // calc_vbar is wrong if there unknown residues, fixed later in
              // check_for_missing_atoms()
              calc_vbar (
                  &temp_model); // update the calculated vbar for this model
              model_vector.push_back (
                  temp_model); // save the model in the model vector.
              clear_temp_chain (&temp_chain); // we are done with this molecule
                                              // and can delete it
            }

          if (str1.left (4) == "ATOM"
              || str1.left (6) == "HETATM") // need to add TER
            {
                  ...
                  if (str1.mid (16, 1) == " " || str1.mid (16, 1) == "A")
                    {
                      if (str1.mid (16, 1) == "A")
                        {
                          // noticemsg += QString("Atom %1 conformation A
                          // selected\n").arg(str1.mid(6,5));
                        }
                      if (!chain_flag) // at the first time we encounter the
                                       // word ATOM
                        { // we don't have a chain yet, so let's start a new
                          // one
                      ...
                      else // we have a chain, let's make sure the chain is
                           // still the same
                        {
                          bool break_chain
                              = (temp_chain.chainID != str1.mid (21, 1));
                          ...
                          bool known_residue
                              = (multi_residue_map.count (thisResName));
                          bool this_is_aa
                              = (known_residue
                                 && residue_list[multi_residue_map[thisResName]
                                                                  [0]]
                                            .type
                                        == 0);
                          if (!break_chain && currently_aa_chain
                              && known_residue && !this_is_aa)
                            {
                              break_chain = true;
                            }
                          if (break_chain)
                              ...
                              if (temp_chain.atom.size ())
                                {
                                  if (temp_chain.atom.size ())
                                    {
                                      sulfur_pdb_chain_idx[temp_chain.atom[0]
                                                               .chainID]
                                          .push_back ((unsigned int)temp_model
                                                          .molecule.size ());
                                    }
                                  temp_model.molecule.push_back (temp_chain);
                                }
                              clear_temp_chain (&temp_chain);
                              temp_chain.chainID = str1.mid (
                                  21, 1); // we have to start a new chain
                              str2 = str1.mid (72, 4);
                              temp_chain.segID = str2.trimmed ();
                            }
                      ...
                      if (assign_atom (
                              str1, &temp_chain,
                              &last_resSeq)) // parse the current line and add
                                             // it to temp_chain
                        { // if true, we have new residue and need to add it to
                          // the residue vector
                          temp_model.residue.push_back (
                              current_residue); // add the next residue of this
                                                // model
                        }
                      temp_chain.atom.back ().model_residue_pos
                          = temp_model.residue.size () - 1;
                    }
                }
            }
        }
  ...
  if (!model_flag) // there were no model definitions, just a single molecule,
    {              // we still need to save the results
      ...
      for (unsigned int i = 0; i < temp_model.molecule.size (); i++)
        {
          QString lastResSeq = "";
          for (unsigned int j = 0; j < temp_model.molecule[i].atom.size ();
               j++)
            {
              if (temp_model.molecule[i].atom[j].resSeq != lastResSeq)
                {
                  str += temp_model.molecule[i].atom[j].resName + " ";
                  sstr += residue_short_names.count (
                              temp_model.molecule[i].atom[j].resName)
                              ? QString (
                                    residue_short_names[temp_model.molecule[i]
                                                            .atom[j]
                                                            .resName])
                              : "?";
                  sstr_pos++;
...
public:
  unsigned int x;
  bool
  operator< (const sortable_uint &objIn) const
  {
    return x < objIn.x;
  }
};
...
QString
US_Saxs_Util::pdb_jsmol_script (vector<PDB_atom> *model)
{
  QString qs;
  ...
  for (unsigned int i = 0; i < model->size (); i++)
    {
      qs += QString ("{atomno=%1}.radius=%2;{atomno=%3}.color={%4};")
                ...
                .arg (i + 1)
                .arg (colormap[get_color_util_saxs (&(*model)[i])
                               % (int)colormap.size ()]);
    }
  return qs;
}
...
int
US_Saxs_Util::write_pdb_hydro (QString fname, vector<PDB_atom> *model)
{
  QFile fpdb;
  ...
  QTextStream ts (&fpdb);
  ts << QString ("HEADER  US-SOMO BD Connection file %1\n").arg (fname);
  ...
  for (unsigned int i = 0; i < model->size (); i++)
    {
      if ((*model)[i].active
        ...
        {
          ts << QString ("").sprintf (
              "ATOM   %4d  PB  UNK A%4d    %8.3f%8.3f%8.3f  1.00 10.00        "
              "   PB  \n",
              i + 1, i + 1, (*model)[i].bead_coordinate.axis[0],
              ...
              if (us_udp_msg)
                {
                  map<QString, QString> msging;
                  msging["_textarea"]
                      = "unexpected regexp extract failure (write_pdb)!\\n";
  ...
  for (list<sortable_uint>::const_iterator it = connect_parents.begin ();
       it != connect_parents.end (); ++it)
    {
  ...
  for (list<sortable_uint>::const_iterator it = connect_parents.begin ();
       it != connect_parents.end (); ++it)
    {
      ...
      QString tmp = "";
      unsigned int j = 0;
      for (list<sortable_uint>::const_iterator itj
           = connect_list[it->x].begin ();
           itj != connect_list[it->x].end (); ++itj)
        {
...
public:
  ...
  bool pb_rule_on;
  bool
  operator< (const sortable_PDB_atom &objIn) const
  {
    if (
        //   (PRO_N_override ? 0 : pdb_atom.bead_assignment)
        (!pb_rule_on
         && (pdb_atom.bead_assignment < objIn.pdb_atom.bead_assignment))
            ? 1
...
int
US_Saxs_Util::create_beads_hydro (QString *error_string, bool quiet)
{
...
#if defined(DEBUG_MM)
  {
    ...
    us_log->log (
        QString ("There are %1 atoms in %2 molecule(s) in this model\n")
            .arg (no_of_atoms)
            .arg (no_of_molecules));
    if (us_udp_msg)
      {
          ...
          for (unsigned int m = 0; m < residue_list.size (); m++)
            {
              if (residue_list[m].name == this_atom->resName
                         ...
                         == true)
                {
                  ...
                  // {
                  //    cout <<
                  //       QString("(create beads) skipping %1
                  //       %2\n").arg(res_idx).arg(m) << endl;
                  // }
                  continue;
                }
          ...
          if (respos == -1)
            {
              if ((this_atom->name != "H" && this_atom->name != "D"
                   ...
                   && (this_atom->altLoc == "A" || this_atom->altLoc == " ")))
                {
                  error_string->append (QString ("").sprintf (
                      "unknown residue molecule %d atom %d name %s resname %s "
                      "coord [%f,%f,%f]\n",
          ...
          if (respos != -1)
            {
              ...
              if (this_atom->resSeq != last_resSeq
                  ...
                  || residue_list[respos].name == "N1")
                {
                  ...
                  for (unsigned int m = 0;
                       m < residue_list[respos].r_atom.size (); m++)
                    {
              ...
              for (unsigned int m = 0; m < residue_list[respos].r_atom.size ();
                   m++)
                {
                  ...
                  if (this_atom->name == "N" && !k && misc.pb_rule_on)
                    {
                      printf ("this_atom->name == N/N1 this "
                              "residue_list[%d].r_atom[%d].name == %s\n",
                              respos, m,
                              residue_list[respos]
                                  .r_atom[m]
                                  .name.toLatin1 ()
...
#endif
                  if (!residue_list[respos].r_atom[m].tmp_used
                      && (residue_list[respos].r_atom[m].name
                              == this_atom->name
                          || (this_atom->name == "N" && misc.pb_rule_on && !k
                              && residue_list[respos].r_atom[m].name == "N1"
                              && !broken_chain_head.count (
              ...
              if (atompos == -1)
                {
                  error_string->append (QString ("").sprintf (
                      "unknown atom molecule %d atom %d name %s resname %s "
                      "coord [%f,%f,%f]\n",
                      j + 1, k, this_atom->name.toLatin1 ().data (),
                      this_atom->resName.toLatin1 ().data (),
                      this_atom->coordinate.axis[0],
              ...
              else
                {
                  ...
                  this_atom->mw
                      = residue_list[respos].r_atom[atompos].hybrid.mw;

                  this_atom->placing_method
                      = this_atom->p_residue
                            ->r_bead[this_atom->p_atom->bead_assignment]
                            .placing_method;
                  ...
                  this_atom->chain = (int)this_atom->p_residue
                                         ->r_bead[this_atom->bead_assignment]
                                         .chain;
                  this_atom->atom_hydration
                      = residue_list[respos].r_atom[atompos].hydration;
                  this_atom->type = this_atom->p_residue->type;

                  // if ( saxs_options.compute_saxs_coeff_for_bead_models )
                  //  {
                  //     QString mapkey =
                  //     QString("%1|%2").arg(this_atom->resName).arg(this_atom->name);
                  ...
                  //     if ( !residue_atom_hybrid_map.count(mapkey) )
                  //     {
                  // 	    //cout << "error: hybrid name missing for " <<
                  // this_atom->resName << "|" << this_atom->name << endl;
                  //       // QColor save_color = editor->textColor();
                  //       // editor->setTextColor("red");
                  //        us_log->log(QString("%1Molecule %2 Residue %3|%4 %5
                  //        Hybrid name missing. Atom skipped for SAS.\n")
                  //                       .arg(this_atom->chainID == " " ? ""
                  //                       : ("Chain " + this_atom->chainID + "
                  //                       ")) .arg(j+1)
                  //                       .arg(this_atom->resName)
                  ...
                  //        )
                  //        {
                  // 	       //cout << "error: hybrid name empty for " <<
                  // this_atom->resName << "|" << this_atom->name << endl;
                  //          // QColor save_color = editor->textColor();
                  //          // editor->setTextColor("red");
                  //           us_log->log(QString("%1Molecule %2 Residue %3|%4
                  //           %5 Hybrid name missing. Atom skipped for
                  //           SAS.\n")
                  //                          .arg(this_atom->chainID == " " ?
                  //                          "" : ("Chain " +
                  //                          this_atom->chainID + " "))
                  ...
                  //           {
                  // 		  //cout << "error: hybrid_map name missing for
                  // hybrid_name " << hybrid_name << endl;
                  // 		  //QColor save_color = editor->textColor();
                  // 		  //editor->setTextColor("red");
                  //              us_log->log(QString("%1Molecule %2 Residue %3
                  //              %4 Hybrid %5 name missing from Hybrid file.
                  //              Atom skipped for SAS.\n")
                  //                             .arg(this_atom->chainID == " "
                  //                             ? "" : ("Chain " +
                  //                             this_atom->chainID + " "))
                  //                             .arg(j+1)
                  ...
                  //           } else {
                  //              if (
                  //              !saxs_util->atom_map.count(this_atom->name +
                  //              "~" + hybrid_name) )
                  //              {
                  //                 // cout << "error: atom_map missing for
                  //                 hybrid_name "
                  //                 //      << hybrid_name
                  //                 //      << " atom name "
                  //                 //      << this_atom->name
                  ...
                  //                 editor->textColor();
                  //                 // editor->setTextColor("red");
                  //                 us_log->log(QString("%1Molecule %2 Atom %3
                  //                 Residue %4 %5 Hybrid %6 name missing from
                  ...
                  //                 if ( count_hydrogens.indexIn(hybrid_name)
                  //                 != -1 )
                  //                 {
                  //                    this_atom->hydrogens =
                  //                    count_hydrogens.cap(1).toInt();
                  //                 }
                  //                 this_atom->saxs_excl_vol =
                  //                 saxs_util->atom_map[this_atom->name + "~"
                  //                 + hybrid_name].saxs_excl_vol; if (
                  //                 !saxs_util->saxs_map.count(saxs_util->hybrid_map[hybrid_name].saxs_name)
                  //                 )
                  //                 {
                  //                    // cout << "error: saxs_map missing for
                  //                    hybrid_name "
                  //                    //      << hybrid_name
                  //                    //      << " saxs name "
                  //                    //      <<
                  ...
                  //                    editor->textColor();
                  //                    // editor->setTextColor("red");
                  //                    us_log->log(QString("%1Molecule %2
                  //                    Residue %3 %4 Hybrid %5 Saxs name %6
                  //                    name missing from SAXS atom file. Atom
                  //                    skipped.\n")
                  //                                   .arg(this_atom->chainID
                  //                                   == " " ? "" : ("Chain "
                  //                                   + this_atom->chainID + "
                  ...
                  //                 } else {
                  //                    this_atom->saxs_data =
                  //                    saxs_util->saxs_map[saxs_util->hybrid_map[hybrid_name].saxs_name];
                  //                 }
                  //              }
                  //           }
                  //        }
                  ...
                  if (!misc.pb_rule_on && this_atom->name == "N" && !k)
                    {
                      // if ( advanced_config.debug_1 )
                      // {
                  ...
                  //      advanced_config.debug_2 )
                  // {
                  //    printf("found atom %s %s in residue %d pos %d bead asgn
                  //    %d %d placing info %d mw %f bead_ref_mw %f hybrid name
                  //    %s %s ba %d chain %d\n",
                  //           this_atom->name.toLatin1().data(),
                  //           this_atom->resName.toLatin1().data(), respos,
                  //           atompos, this_atom->p_atom->bead_assignment,
                  //           this_atom->bead_assignment,
          ...
          if (this_atom->active)
            {
              ...
              if (this_atom->name != "H" && this_atom->name != "D"
                ...
                {
#if defined(DEBUG)
                  printf ("skipped bound waters & H %s %s rad %f resseq %s\n",
                          this_atom->name.toLatin1 ().data (),
                          this_atom->resName.toLatin1 ().data (),
                          this_atom->radius,
                          this_atom->resSeq.toLatin1 ().data ());
         ...
         j++)
      {
        for (unsigned int k = 0;
             k < model_vector[current_model].molecule[j].atom.size (); k++)
          {
            ...
            // // this_atom->bead_assignment =
            // // (this_atom->p_atom ? (int) this_atom->p_atom->bead_assignment
            // : -1);
            // // this_atom->chain =
            // // ((this_atom->p_residue && this_atom->p_atom) ?
            // //  (int)
            ...
            this_atom->bead_positioner
                = this_atom->p_atom ? this_atom->p_atom->positioner : false;
          }
      }
  }
#define DO_SORT
  ...
  // reorder residue
  {
    for (unsigned int j = 0; j < model_vector[current_model].molecule.size ();
         j++)
      {
        ...
        unsigned int k;
        for (k = 0; k < model_vector[current_model].molecule[j].atom.size ();
             k++)
          {
            PDB_atom *this_atom
                = &model_vector[current_model].molecule[j].atom[k];
                    ...
                    for (unsigned int m = k - base_ofs; m < k; m++)
                      {
                        ...
                        // last_residue_atoms.size());
                        model_vector[current_model].molecule[j].atom[m]
                            = (last_residue_atoms.front ()).pdb_atom;
  ...
  //       model_vector[current_model].molecule.size (); j++) {
  //          for (unsigned int k = 0; k <
  //          model_vector[current_model].molecule[j].atom.size (); k++) {
  //             PDB_atom *this_atom =
  //             &model_vector[current_model].molecule[j].atom[k];
  ...
  //                   last_res != this_atom->resSeq) {
  //                   if(bead_count) {
  //                      printf("res %s %s bead %d bead_mw %f sum atom mw %f
  //                      diff %f\n",
  //                             last_res.toLatin1().data(),
...
double
US_Saxs_Util::total_volume_of_bead_model (vector<PDB_atom> &bead_model)
{
  double tot_vol = 0e0;
  ...
  for (unsigned int i = 0; i < bead_model.size (); i++)
    {
      if (bead_model[i].active)
        {
          tot_vol += (double)bead_model[i].bead_computed_radius
                     * bead_model[i].bead_computed_radius
                     * bead_model[i].bead_computed_radius;
        }
    }
   ...
   case 1 :
      {
         ...
         //                               ,us_tr("A non-coded residue was
encountered\n"
         //                                   "It can be skipped or you can
stop processing.\n"
         //                                   "This choice will be maintained
for all remaining non coded residues encountered in this PDB file.\n"
         //                                   "For specific residue details,
please review the main panel text area after making your selection.\n"
         //                                   "\n"
         //                                   "If your model contains non-coded
residues, the calculated molecular weight\n"
         //                                   "and vbar may be incorrect.
Therefore, you could manually enter a global\n"
         //                                   "value for the molecular weight
in the SOMO hydrodynamic options, and a\n"
         //                                   "global value for the vbar in the
SOMO Miscellaneous options.\n"
         //                                   "\n"
   ...
   case 2 :
      {
         ...
         //                               ,us_tr("A non-coded residue was
encountered\n"
         //                                   "It can be replaced with an
average residue, skipped or you can stop processing.\n"
         //                                   "This choice will be maintained
for all remaining non coded residues encountered in this PDB file.\n"
         //                                   "For specific residue details,
please review the main panel text area after making your selection.\n"
...
in the SOMO hydrodynamic options, and a\n"
         //                                   "global value for the vbar in the
SOMO Miscellaneous options. You can also\n"
         //                                   "review the average residue
settings in the SOMO Miscellaneous options.\n"
         //                                   "\n"
         //                                   "Please select your option below"
         //                                   )
         //                               ,us_tr( "Replace non-coded with
average residues" )
         //                               ,us_tr( "Skip non-coded residues" )
         //                               ,us_tr( "Stop processing" )
         //                               ,2
   ...
   case 1 :
      {
         ...
         // switch( QMessageBox::question(
         //                               this
         //                               ,us_tr( "US-SOMO: Missing or extra
atoms" )
         //                               ,us_tr("A missing or extra atom was
encountered when comparing with the residue table\n"
         //                                   "It can be skipped or you can
stop processing.\n"
         //                                   "This choice will be maintained
for all remaining missing atoms encountered in this PDB file.\n"
         //                                   "For specific residue details,
please review the main panel text area after making your selection.\n"
         //                                   "\n"
         //                                   "If your model contains missing
atoms, the calculated molecular\n"
         //                                   "weight and vbar may be
incorrect, and you should manually enter\n"
         //                                   "a global value for the molecular
weight in the SOMO hydrodynamic\n"
         //                                   "options, and a global value for
the vbar in the SOMO Miscellaneous\n"
         //                                   "options.\n"
         //                                   "\n"
         //                                   "Please select your option below"
         //                                   )
         //                               ,us_tr( "Skip residues with missing
atoms" )
         //                               ,us_tr( "Stop processing" )
         //                               ,QString()
         //                               ,1
...
atoms" )
         //                               ,us_tr("A missing or extra atom was
encountered when comparing with the residue table\n"
         //                                   "It can be modeled with an
approximate method, skipped or you can stop processing.\n"
         //                                   "This choice will be maintained
for all remaining missing atoms encountered in this PDB file.\n"
         //                                   "For specific residue details,
please review the main panel text area after making your selection.\n"
         //                                   "\n"
         //                                   "If you model with an approximate
method, the shape of this residue will\n"
         //                                   "be different from an exact
residue which can effect the hydrodynamic computations\n"
         //                                   "\n"
         //                                   "If you skip missing atoms, the
...
the vbar in the SOMO Miscellaneous\n"
         ...
         //                                   "Please select your option below"
         //                                   )
         //                               ,us_tr( "Model with the approximate
method" )
         //                               ,us_tr( "Skip residues with missing
atoms" )
         //                               ,us_tr( "Stop processing" )
...
void
reset_hydro_res (hydro_results results_hydro)
{
