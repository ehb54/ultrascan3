...
void
US_Hydrodyn_Mals::update_plot_errors_group ()
{
  if (!unified_ggaussian_ok)
    {
      editor_msg ("red", "Internal error: update_plot_errors_group(): "
                         "unified_ggaussian not ok\n");
      return;
    }
  ...
  for (unsigned int j = 0; j < unified_ggaussian_curves; j++)
    {
      ...
      // well, I good compiler should unwrap the if's if I put them inside one
      // loop, but I am uncertain printvector( "target", target ); printvector(
      // "fit", fit );

      if (cb_plot_errors_pct->isChecked ())
        {
          if (cb_plot_errors_sd->isChecked ()
              && errors.size () == target.size ()
              && is_nonzero_vector (errors))
            {
              // does % and errors make sense?, I am excluding this for now
              // cout << "pct mode with errors, not acceptable\n";
              for (unsigned int i = unified_ggaussian_q_start[j];
                   i < unified_ggaussian_q_end[j]; i++)
                {
                  if (target[i] != 0e0)
                    {
...

      {
        ...
        curve->setPen (QPen (plot_colors[f_pos[unified_ggaussian_files[j]]
                                         % plot_colors.size ()],
  ...
  if (current_mode == MODE_GGAUSSIAN || current_mode == MODE_GAUSSIAN)
    {
      plot_errors->setAxisTitle (
          QwtPlot::yLeft,
          us_tr (cb_plot_errors_pct->isChecked ()
                     ? "% difference"
                     : (cb_plot_errors_sd->isChecked () ? "delta I(t)/sd"
                                                        : "delta I(t)")));
    }
  else
    {
      plot_errors->setAxisTitle (
          ...
          us_tr (cb_plot_errors_pct->isChecked ()
                     ? "% difference"
                     : (cb_plot_errors_sd->isChecked () ? "delta I(q)/sd"
                                                        : "delta I(q)")));
    }
  ...
  qDebug () << QString ("").sprintf ("mals_plots::update_plot_errors_group\n"
                                     "\tminx %e maxx %e\n"
                                     "\tminy %e maxy %e\n",
                                     minx, maxx, -maxy, maxy);
#endif
  ...
  //       plot_errors_zoomer->setRubberBandPen(QPen(Qt::yellow, 0,
  //       Qt::DotLine));
...
void
US_Hydrodyn_Mals::update_plot_errors (vector<double> &grid,
                                      vector<double> &target,
                                      vector<double> &fit,
                                      vector<double> &errors, QColor use_color)
{
  ...
  if (!target.size ())
    {
      // editor_msg( "red", "Internal error: update_plot_errors(): empty
      // data\n" );
      return;
    }
  ...
  if (target.size () != fit.size ())
    {
      editor_msg ("red", "Internal error: update_plot_errors(): target.size() "
                         "!= fit.size()\n");
      return;
    }
...

  {
    ...
    plot_errors->setCurveData (curve, (double *)&x[0], (double *)&y[0],
                               x.size ());
#else
    curve->setPen (QPen (Qt::green, use_line_width, Qt::SolidLine));
    curve->setSamples ((double *)&x[0], (double *)&y[0], x.size ());
...

  {
    ...
    plot_errors->setCurvePen (
        curve, QPen (plot_errors_color, use_line_width, Qt::SolidLine));
    plot_errors->setCurveData (curve, (double *)&x[0], (double *)&e[0],
                               x.size ());
  ...
  // if ( !plot_errors_zoomer )
  {
...
#if defined(DEBUG_RESCALE)
    {
      ...
      tso << QString ("").sprintf ("mals_plots::update_plot_errors\n"
                                   "\tminx %e maxx %e\n"
                                   "\tminy %e maxy %e\n",
                                   x[0], x.back (), -maxy, maxy);
...
void
US_Hydrodyn_Mals::redo_plot_errors ()
{
  if (current_mode == MODE_GGAUSSIAN || current_mode == MODE_GAUSSIAN)
    {
      plot_errors->setAxisTitle (
...
void
US_Hydrodyn_Mals::set_plot_errors_rev ()
{
...
void
US_Hydrodyn_Mals::set_plot_errors_sd ()
{
...
void
US_Hydrodyn_Mals::set_plot_errors_pct ()
{

  if (cb_plot_errors_pct->isChecked () && cb_plot_errors_sd->isChecked ())
    {
...
void
US_Hydrodyn_Mals::set_plot_errors_group ()
{
  redo_plot_errors ();
}

void
US_Hydrodyn_Mals::plot_errors_jump_markers ()
{
  ...
  for (unsigned int i = 0; i < unified_ggaussian_curves; i++)
    {
      ...
      plot_errors->setMarkerFont (marker,
                                  QFont ("Helvetica", 11, QFont::Bold));
      plot_errors->setMarkerLabelText (
          marker,
          QString ("%1").arg (i + 1)); // unified_ggaussian_files[ i ] );
#else
...
void
US_Hydrodyn_Mals::resize_plots ()
{
  // qDebug() << "mals::resize_plots()";
...
void
US_Hydrodyn_Mals::resize_guinier_plots ()
{
  ...
  if (guinier_plot_errors->isVisible ())
    {
      ...
      for (QList<int>::iterator it = qs_sizes.begin (); it != qs_sizes.end ();
           ++it)
        {
          if (!*it)
            {
  ...
  if (current_mode == MODE_GUINIER)
    {
      ...
      connect (((QObject *)guinier_plot->axisWidget (QwtPlot::xBottom)),
               SIGNAL (scaleDivChanged ()), usp_guinier_plot_errors,
               SLOT (scaleDivChangedXSlot ()), Qt::UniqueConnection);
      connect (((QObject *)guinier_plot_errors->axisWidget (QwtPlot::xBottom)),
               SIGNAL (scaleDivChanged ()), usp_guinier_plot,
               SLOT (scaleDivChangedXSlot ()), Qt::UniqueConnection);
  ...
  if (plot_errors->isVisible ())
    {
      ...
      // qDebug() << "::errors() plot_errors isVisible(), no further mode
      // checking";
    }
  else
    {
        ...
        case MODE_SCALE:
          {
            ...
            //         f_errors[ files[ i ] ].size() != f_Is[ files[ i ]
            //         ].size() )
            //    {
            //       use_errors = false;
            //       break;
            ...
            //    disconnect( cb_plot_errors_sd, SIGNAL( clicked() ), 0, 0 );
            //    cb_plot_errors_sd->setChecked( false );
            //    connect( cb_plot_errors_sd, SIGNAL( clicked() ), SLOT(
            //    set_plot_errors_sd() ) ); cb_plot_errors_sd->hide();
            // }

            // "running" so normal update enables doesn't handle this
            plot_dist->enableAxis (QwtPlot::xBottom,
        ...
        default:
          {
            ...
            if (files.size () != 2)
              {
                editor_msg ("red", us_tr ("Internal error: plot residuals, "
                                          "not 2 selected files"));
                return;
              }
            if (f_qs[files[0]] != f_qs[files[1]])
              {
                editor_msg ("red",
                            us_tr ("Error: Residuals incompatible grids, you "
                                   "could try 'crop common'"));
                return;
              }
            vector<double> errors;
            bool use_errors_0 = f_errors.count (files[0])
                                && is_nonzero_vector (f_errors[files[0]]);
            bool use_errors_1 = f_errors.count (files[1])
                                && is_nonzero_vector (f_errors[files[1]]);
            update_plot_errors (
                f_qs[files[0]], f_Is[files[0]], f_Is[files[1]],
            ...
            if (use_errors_0)
              {
                ...
                double nchi;
                QString errors;
                if (US_Saxs_Util::calc_chisq (f_Is[files[0]], f_Is[files[1]],
                                              f_errors[files[0]], 0, chi2,
                                              nchi, errors))
                  {
...
bool
US_Hydrodyn_Mals::compatible_grids (QStringList files)
{
...
void
US_Hydrodyn_Mals::errors_multi_file (QStringList files)
{
  // check
  if (!compatible_grids (files))
    {
      editor_msg ("red", us_tr ("Residuals: curves must be on the same grid, "
...
void
US_Hydrodyn_Mals::set_dots ()
{
  set_eb ();
}

void
US_Hydrodyn_Mals::set_eb ()
{
...
US_Hydrodyn_Mals::plot_files ()
{
  ...
  if ((asfs > 20 || cb_eb->isChecked ()) && legend_vis)
    {
      legend ();
    }
  ...
  for (int i = 0; i < lb_files->count (); i++)
    {
      if (lb_files->item (i)->isSelected ())
      ...
      else
        {
          if (0)
            {
              if (get_min_max (lb_files->item (i)->text (), file_minx,
                               file_maxx, file_miny, file_maxy))
                {
                  if (first)
                    {
                      minx = file_minx;
...
#if defined(DEBUG_RESCALE)
  {
    ...
    tso << QString ("").sprintf ("mals_plots::plot_files\n"
                                 "\tminx %e maxx %e\n"
                                 "\tminy %e maxy %e\n",
                                 minx, maxx, miny, maxy);
...
bool
US_Hydrodyn_Mals::get_min_max (QString file, double &minx, double &maxx,
                               double &miny, double &maxy)
{
  if (current_mode == MODE_SCALE)
    {
      //    us_qdebug( QString("get min max mode scale %1" ).arg( file ) );
      if (!scale_q.count (file) || !scale_I.count (file)
          || !f_pos.count (file))
        {
          // editor_msg( "red", QString( us_tr( "Internal error: requested %1,
          // but not found in data" ) ).arg( file ) );
          return false;
        }
      ...
      // us_qdebug( QString("get min max mode scale %1 x %2:%3 x %4:%5" ).arg(
      // file ).arg( minx ).arg( maxx ).arg( miny ).arg( maxy ) );
    }
  else
    {
...
bool
US_Hydrodyn_Mals::plot_file (QString file, double &minx, double &maxx,
                             double &miny, double &maxy)
{
  if (!f_qs_string.count (file) || !f_qs.count (file) || !f_Is.count (file)
      || !f_pos.count (file))
  ...
  curve->setStyle (cb_dots->isChecked () ? QwtPlotCurve::Dots
                                         : QwtPlotCurve::Lines);
      ...
      if (!axis_y_log)
        {
          ...
          plot_dist->setCurveData (
              ...
              (double *)&(f_qs[file][0]), (double *)&(f_Is[file][0]),
              q_points);
          plot_dist->setCurvePen (
              Iq, QPen (plot_colors[f_pos[file] % plot_colors.size ()],
                        use_line_width, SolidLine));
          plot_dist->setCurveStyle (Iq, QwtCurve::NoCurve);
          symbol.setPen (QPen (plot_colors[f_pos[file] % plot_colors.size ()],
                               use_line_width, Qt::SolidLine));
          ...
          symbol.setPen (QPen (plot_colors[f_pos[file] % plot_colors.size ()],
                               use_line_width, Qt::SolidLine));
          curve->setSymbol (new QwtSymbol (symbol.style (), symbol.brush (),
...
#if QT_VERSION < 0x040000
              plot_dist->setCurveData (Iqeb, (double *)&(x[0]),
                                       (double *)&(y[0]), 2);
              plot_dist->setCurvePen (
                  Iqeb, QPen (plot_colors[f_pos[file] % plot_colors.size ()],
                              use_line_width, SolidLine));
#else
              curveeb->setSamples ((double *)&(x[0]), (double *)&(y[0]), 2);

              curveeb->setPen (
                  QPen (plot_colors[f_pos[file] % plot_colors.size ()],
                        use_line_width, Qt::SolidLine));
...
void
US_Hydrodyn_Mals::rescale ()
{
  // qDebug() << "rescale XY event\n";
  //    hide_widgets( plot_errors_widgets, !plot_errors_widgets[ 0
  //    ]->isVisible() ); hide_widgets( files_widgets, !files_widgets[ 0
  //    ]->isVisible() ); hide_widgets( files_expert_widgets,
  //    !files_expert_widgets[ 0 ]->isVisible() ); hide_widgets(
  //    created_files_widgets, !created_files_widgets[ 0 ]->isVisible() );
  //    hide_widgets( created_files_expert_widgets,
  //    !created_files_expert_widgets[ 0 ]->isVisible() ); hide_widgets(
  //    editor_widgets, !editor_widgets[ 0 ]->isVisible() );
...
#if defined(DEBUG_RESCALE)
  {
    ...
    tso << QString ("").sprintf ("mals_plots::rescale\n"
                                 "\tminx %e maxx %e\n"
                                 "\tminy %e maxy %e\n",
                                 minx, maxx, miny, maxy);
    tso << "zoomrect " << plot_dist_zoomer->zoomRect ().left () << " , "
  ...
  // connect( plot_dist_zoomer, SIGNAL( zoomed( const QRectF & ) ), SLOT(
  // plot_zoomed( const QRectF & ) ) );
...
US_Hydrodyn_Mals::rescale_y ()
{
  ...
  if (current_mode == MODE_GAUSSIAN || current_mode == MODE_GGAUSSIAN)
    {
  ...
  US_Plot_Util::rescale (plot_info, plot_to_zoomer, plot_limit_x_range_min,
                         plot_limit_x_range_max);
}

void
US_Hydrodyn_Mals::rescale_y_plot_errors ()
{
  // qDebug() << "rescale_y_plot_errors";
  if (current_mode == MODE_GAUSSIAN || current_mode == MODE_GGAUSSIAN)
    {
      US_Plot_Util::rescale (plot_errors, plot_errors_zoomer,
                             le_gauss_fit_start->text ().toDouble (),
                             le_gauss_fit_end->text ().toDouble ());
    }
}
