...
void US_Hydrodyn_Mals::update_plot_errors_group() {
  if (!unified_ggaussian_ok) {
    editor_msg("red",
               "Internal error: update_plot_errors_group(): unified_ggaussian "
               "not ok\n");
    return;
  }
  ...
  for (unsigned int j = 0; j < unified_ggaussian_curves; j++) {
...

    {
      ...
      curve->setPen(QPen(
          plot_colors[f_pos[unified_ggaussian_files[j]] % plot_colors.size()],
  ...
  qDebug() << QString("").sprintf(
      "mals_plots::update_plot_errors_group\n"
      "\tminx %e maxx %e\n"
      "\tminy %e maxy %e\n",
      minx, maxx, -maxy, maxy);
#endif
...
void US_Hydrodyn_Mals::update_plot_errors(vector<double> &grid,
  ...
  if (target.size() != fit.size()) {
    editor_msg(
        "red",
        "Internal error: update_plot_errors(): target.size() != fit.size()\n");
    return;
  }
  if (target.size() != grid.size()) {
    editor_msg(
        "red",
        "Internal error: update_plot_errors(): target.size() != grid.size()\n");
    return;
  }
  ...
  // well, I good compiler should unwrap the if's if I put them inside one loop,
  // but I am uncertain printvector( "target", target ); printvector( "fit", fit
  // );
...
void US_Hydrodyn_Mals::redo_plot_errors() {
  if (current_mode == MODE_GGAUSSIAN || current_mode == MODE_GAUSSIAN) {
    plot_errors->setAxisTitle(
        QwtPlot::yLeft,
        us_tr(cb_plot_errors_pct->isChecked()
...
void US_Hydrodyn_Mals::set_plot_errors_rev() {
...
void US_Hydrodyn_Mals::set_plot_errors_sd() {
...
void US_Hydrodyn_Mals::set_plot_errors_pct() {
...
void US_Hydrodyn_Mals::set_plot_errors_group() { redo_plot_errors(); }

void US_Hydrodyn_Mals::plot_errors_jump_markers() {
  ...
  for (unsigned int i = 0; i < unified_ggaussian_curves; i++) {
    ...
    plot_errors->setMarkerLabelText(
        marker, QString("%1").arg(i + 1));  // unified_ggaussian_files[ i ] );
#else
...
void US_Hydrodyn_Mals::resize_plots() {
...
void US_Hydrodyn_Mals::resize_guinier_plots() {
    ...
    switch (current_mode) {
      ...
      default: {
        ...
        if (files.size() != 2) {
          editor_msg(
              "red",
              us_tr("Internal error: plot residuals, not 2 selected files"));
          return;
        }
        if (f_qs[files[0]] != f_qs[files[1]]) {
          editor_msg("red", us_tr("Error: Residuals incompatible grids, you "
        ...
        update_plot_errors(f_qs[files[0]], f_Is[files[0]], f_Is[files[1]],
                           use_errors_0
                               ? f_errors[files[0]]
        ...
        if (use_errors_0) {
          ...
          if (US_Saxs_Util::calc_chisq(f_Is[files[0]], f_Is[files[1]],
                                       f_errors[files[0]], 0, chi2, nchi,
                                       errors)) {
            editor_msg("black",
                       QString("fit to %1 nchi %2\n").arg(files[0]).arg(nchi));
          } else {
            editor_msg("red",
...
bool US_Hydrodyn_Mals::compatible_grids(QStringList files) {
...
void US_Hydrodyn_Mals::errors_multi_file(QStringList files) {
  // check
  if (!compatible_grids(files)) {
    editor_msg("red", us_tr("Residuals: curves must be on the same grid, try "
                            "'Crop Common' first."));
    return;
  }
}
...
void US_Hydrodyn_Mals::set_dots() { set_eb(); }
...
bool US_Hydrodyn_Mals::get_min_max(QString file, double &minx, double &maxx,
                                   double &miny, double &maxy) {
  if (current_mode == MODE_SCALE) {
    //    us_qdebug( QString("get min max mode scale %1" ).arg( file ) );
    if (!scale_q.count(file) || !scale_I.count(file) || !f_pos.count(file)) {
...
bool US_Hydrodyn_Mals::plot_file(QString file, double &minx, double &maxx,
                                 double &miny, double &maxy) {
  if (!f_qs_string.count(file) || !f_qs.count(file) || !f_Is.count(file) ||
      !f_pos.count(file)) {
    editor_msg(
  ...
  if (use_error) {
    ...
    if (!axis_y_log) {
      ...
      for (unsigned int i = 0; i < q_points; i++) {
...
#if QT_VERSION < 0x040000
        plot_dist->setCurveData(Iqeb, (double *)&(x[0]), (double *)&(y[0]), 2);
        ...
        curveeb->setPen(QPen(plot_colors[f_pos[file] % plot_colors.size()],
                             use_line_width, Qt::SolidLine));
...
void US_Hydrodyn_Mals::rescale_y() {
...
void US_Hydrodyn_Mals::rescale_y_plot_errors() {
