...
void
US_Hydrodyn_Saxs::calc_iqq_nnls_fit (QString /* title */, QString csv_filename)
{
  // setup nnls run:
  ...
  if (!tagged_csv_filename.isEmpty ()
      && !tagged_csv_filename.contains (
          QRegExp ("_nnls", Qt::CaseInsensitive)))
    {
  ...
  for (map<QString, vector<double> >::iterator it = nnls_A.begin ();
       it != nnls_A.end (); it++)
    {
      ...
      if (it->second.size () != max_iqq_len)
        {
          editor_msg ("red",
                      QString (us_tr ("NNLS failed, length mismatch %1 %2\n"))
                          .arg (it->second.size ())
                          .arg (max_iqq_len));
          nnls_csv_footer
              << QString (us_tr ("\"NNLS failed, length mismatch %1 %2\""))
  ...
  if (org_size != max_iqq_len)
    {
      editor_msg (
          "red",
          QString (us_tr ("NNLS failed, target length mismatch %1 %2\n"))
              .arg (org_size)
              .arg (max_iqq_len));
      nnls_csv_footer
          << QString (us_tr ("\"NNLS failed, target length mismatch %1 %2\""))
  ...
  if (our_saxs_options->iqq_kratky_fit)
    {
      ...
      editor_msg ("blue", "Notice: Kratky fit (q^2*I)\n");
      nnls_csv_footer << "\"Notice: Kratky fit (q^2*I)\"";
  ...
  //   editor->append(QString("running nnls %1
      ...
      if (!log_ok)
        {
          editor_msg (
              "red",
              "Warning: Log fitting requested but some of the values are less "
              "than or equal to zero, so log fitting disabled\n");
          nnls_csv_footer
              << "\"Warning: Log fitting requested but some of the values are "
                 "less than or equal to zero, so log fitting disabled\"";
        }
      else
        {
          // compute log10 on A & B
          editor_msg ("blue", "Notice: Log fitting\n");
          nnls_csv_footer << "\"Notice: Log fitting\"";
          for (unsigned int i = 0; i < use_B.size (); i++)
            {
  ...
  int result = nnls ((double *)&use_A[0], (int)max_iqq_len, (int)max_iqq_len,
                     (int)nnls_A.size (), (double *)&use_B[0],
                     (double *)&use_x[0], &nnls_rmsd, (double *)&nnls_wp[0],
                     (double *)&nnls_zzp[0], (int *)&nnls_indexp[0]);

  if (result != 0)
    {
      editor->append ("NNLS error!\n");
      nnls_csv_footer << "\"NNLS error!\"";
    }

  editor->append (
      QString ("Residual Euclidian norm of NNLS fit %1\n").arg (nnls_rmsd));

  nnls_csv_footer << QString ("\"Residual Euclidian norm of NNLS fit\",%1")
                         .arg (nnls_rmsd);

  vector<double> rescaled_x
      = our_saxs_options->disable_nnls_scaling ? use_x : rescale (use_x);

  if (our_saxs_options->disable_nnls_scaling)
    {
      editor_msg ("darkred", "NNLS scaling disabled\n");
      nnls_csv_footer << "\"NNLS scaling disabled\"";
    }
  ...
  public:
    ...
    bool
    operator< (const sortable_model &objIn) const
    {
  ...
  for (auto it = sort_models.begin (); it != sort_models.end (); ++it)
    {
        ...
        if (match.hasMatch ())
        ...
        else
          {
            ...
            if (match.hasMatch ())
              ...
              {
                nnls_csv_data << QString ("\"%1\",,%2")
                                     .arg (model_name.replace ("\"", ""))
                                     .arg (rescaled_x[i]);
              }
          }
      }
  ...
  for (int i = 0; i < (int)contrib_to_plot.size (); ++i)
    {
      ...
      plot_one_iqq (nnls_r, rescaled_A, no_errors,
                    model_names[contrib_to_plot[i]]);
    }
  ...
  if (!csv_filename.isEmpty ())
    {
      ...
      QString fname = ((US_Hydrodyn *)us_hydrodyn)->somo_dir + SLASH + "saxs"
                      + SLASH + tagged_csv_filename + "_iqq.csv";
      ...
      if (of)
        {
          ...
          for (unsigned int i = 0; i < use_x.size (); i++)
            {
              fprintf (of, "\"%s\",\"%s\",%s\n",
                       QString ("%1 %2")
                           ...
                           .data (),
                       "I(q)",
                       vector_double_to_csv (nnls_A[model_names[i]])
                           .toLatin1 ()
                           .data ());
            }
          // target
          fprintf (of, "\"%s\",\"%s\",%s\n",
                   ...
                   "I(q)", vector_double_to_csv (nnls_B).toLatin1 ().data ());
...
void
US_Hydrodyn_Saxs::calc_iqq_best_fit (QString /* title */, QString csv_filename)
{
      ...
      if (it->second.size () != max_iqq_len)
        {
          editor_msg (
              "red",
              QString (us_tr ("Best fit failed, length mismatch %1 %2\n"))
                  .arg (it->second.size ())
                  .arg (max_iqq_len));
          return;
        }
  ...
  if (org_size != max_iqq_len)
    {
      editor_msg (
          "red",
          QString (us_tr ("Best fit failed, target length mismatch %1 %2\n"))
              .arg (org_size)
              .arg (max_iqq_len));
      return;
    }
  ...
  //   editor->append(QString("running best fit %1
  ...
  editor_msg ("dark blue", QString ("Best fit model: %1 %2 %3\n")
  ...
  if (is_nonzero_vector (nnls_errors))
    {
      plot_one_iqq (nnls_r, best_fit_target, nnls_errors,
                    csv_filename + " Target");
    }
  else
    {
  ...
  rescale_iqq_curve (qsl_plotted_iq_names[qsl_plotted_iq_names.size () - 1],
                     nnls_r, model);

  plot_one_iqq (nnls_r, model,
                csv_filename + " Best Fit Model "
                    + model_names[lowest_chi2_pos]);
      ...
      if (of)
        {
          ...
          for (unsigned int i = 0; i < best_fit_models.size (); i++)
            {
              fprintf (of, "\"%s\",\"%s\",%s\n",
                       QString ("%1 %2")
                           ...
                           .data (),
                       "I(q)",
                       vector_double_to_csv (best_fit_models[model_names[i]])
                           .toLatin1 ()
                           .data ());
            }
          // target
          ...
          // best fit model
          fprintf (of, "\"%s\",\"%s\",%s\n",
                   QString ("Best Fit Model " + model_names[lowest_chi2_pos])
                       .toLatin1 ()
                       .data (),
                   "I(q)", vector_double_to_csv (model).toLatin1 ().data ());
          fclose (of);
          if (plotted)
            {
              set_eb ();
...
// -----------------------------------------
void
US_Hydrodyn_Saxs::calc_nnls_fit (QString title, QString csv_filename)
{
  // setup nnls run:
  // editor->append("setting up nnls run\n");
  // unify dimension of nnls_A vectors
      ...
      if (nnls_B.size () > cropd_B.size ())
        {
          editor_msg (
              "darkred",
              QString (us_tr ("Notice: cropping models to target Dmax %1\n"))
                  .arg (cropd_r.back ()));
          qDebug () << QString (
                           us_tr ("Notice: cropping models to target Dmax %1"))
                           .arg (cropd_r.back ());
          nnls_csv_footer
              << QString (us_tr ("\"Cropping models to target Dmax\",%1"))
                     .arg (cropd_r.back ());
          ...
          for (auto it = nnls_A.begin (); it != nnls_A.end (); ++it)
            {
  ...
  if (use_errors && org_errors_size < max_pr_len && !dmax_croplen)
    {
      editor_msg ("darkred",
                  QString (us_tr ("NNLS propagating last error to extended "
                                  "zeros to match maximum model length. "
                                  "Errors size:%1 Maximum model length:%2\n"))
                      .arg (org_errors_size)
                      .arg (max_pr_len));
      nnls_csv_footer
          << QString (us_tr ("\"NNLS propagating last error to extended zeros "
                             "to match maximum model length. Errors size:%1 "
                             "Maximum model length:%2\""))
                 .arg (org_errors_size)
                 .arg (max_pr_len);
    }
  ...
  int result = nnls ((double *)&use_A[0], (int)max_pr_len, (int)max_pr_len,
                     (int)nnls_A.size (), (double *)&use_B[0],
                     (double *)&use_x[0], &nnls_rmsd, (double *)&nnls_wp[0],
                     (double *)&nnls_zzp[0], (int *)&nnls_indexp[0]);
  ...
  for (auto it = sort_models.begin (); it != sort_models.end (); ++it)
    {
        ...
        }
      {
          ...
          {
            {
              double Rg;
              QString errormsg;
              if (US_Saxs_Util::compute_rg_from_pr (nnls_r, nnls_A[model_name],
                                                    Rg, errormsg))
                {
        ...
        if (match.hasMatch ())
          {
            nnls_csv_data << QString ("\"%1\",%2,%3,%4,%5")
                                 .arg (match.captured (1))
                                 .arg (match.captured (2))
                                 .arg (rescaled_x[i])
                                 .arg (rg_msg)
  ...
  for (unsigned int j = 0; j < rescaled_x.size (); j++)
    {
      ...
      // cout << QString("model source %1 contrib %2 mw
  ...
  (*remember_mw_source)[csv_filename + " Model"]
      = "weighted average from NNLS csv models";
...

  {
    ...
    bool use_errors_for_chi2
        = use_errors || use_I_error.size () == use_I.size ();
    ...
    //       .arg( avg_std_dev_frac * sqrt( chi2 / ( use_I.size() - (
    //       do_scale_linear_offset ? 2 : 1 ) ) ), 5 );

    //    nnls_csv_footer
    //       << QString( "\"r_sigma\",%1" )     .arg( avg_std_dev_frac )
    //       << QString( "\"nchi*r_sigma\",%1" ).arg( avg_std_dev_frac * sqrt(
    //       chi2 / ( use_I.size() - ( do_scale_linear_offset ? 2 : 1 ) ) ), 5
    //       )
    //       ;
    // }
  }
  ...
  for (int i = 0; i < (int)contrib_to_plot.size (); ++i)
    {
      vector<double> no_errors;
      plot_one_pr (nnls_r, nnls_A[model_names[contrib_to_plot[i]]], no_errors,
                   model_names[contrib_to_plot[i]]);
      compute_rg_to_progress (nnls_r, nnls_A[model_names[contrib_to_plot[i]]],
                              model_names[contrib_to_plot[i]]);
    }
...

  {
    ...
    max_len = (int)use_nnls_r.size () > max_len ? (int)use_nnls_r.size ()
                                                : max_len;
    use_nnls_r = nnls_r;
    ...
    saxs_residuals_window = new US_Hydrodyn_Saxs_Residuals (
        &saxs_residuals_widget, plot_pr->width (),
        us_tr ("NNLS residuals & difference targeting:\n") + title, use_nnls_r,
        use_difference,
        // use_residual,
        use_nnls_B, use_error,
        // true,
        // true,
        // true,
        // use_error.size(),
  ...
  if (!csv_filename.isEmpty ())
    {
      // cout << "save_to_csv\n";
      QString fname = ((US_Hydrodyn *)us_hydrodyn)->somo_dir + SLASH + "saxs"
                      + SLASH + tagged_csv_filename + "_sprr_"
      ...
      if (of)
        {
          ...
          fprintf (of, "\"%s\",%.2f,%.2f,\"%s\",%s\n",
                   ...
                   compute_pr_area (nnls_B, nnls_r), "P(r)",
                   vector_double_to_csv (nnls_B).toLatin1 ().data ());

          // best fit model
          fprintf (of, "\"%s\",%.2f,%.2f,\"%s\",%s\n", "Model",
                   get_mw (model_names[0], false),
                   compute_pr_area (model, nnls_r), "P(r)",
                   vector_double_to_csv (model).toLatin1 ().data ());

          fprintf (of, "\"%s\",%.2f,%.2f,\"%s\",%s\n", "Residual",
                   get_mw (model_names[0], false),
                   compute_pr_area (residual, nnls_r), "P(r)",
                   vector_double_to_csv (residual).toLatin1 ().data ());
...
void
US_Hydrodyn_Saxs::calc_best_fit (QString title, QString csv_filename)
{
  // setup best_fit run, idea:
  //    use US_Saxs_Util::liner_fit
  //    give each model a chi^2
  ...
  if (use_errors && org_errors_size < max_pr_len)
    {
      editor_msg ("darkred",
                  QString (us_tr ("Best propagating last error to extended "
                                  "zeros to match maximum model length. "
                                  "Errors size:%1 Maximum model length:%2\n"))
                      .arg (org_errors_size)
                      .arg (max_pr_len));
      nnls_csv_footer
          << QString (us_tr ("\"Best fit propagating last error to extended "
                             "zeros to match maximum model length. Errors "
                             "size:%1 Maximum model length:%2\""))
                 .arg (org_errors_size)
                 .arg (max_pr_len);
    }
  if (use_errors)
  ...
  for (unsigned int i = 0; i < best_fit_models.size (); i++)
    {
      ...
      // US_Vector::printvector2( QString( "best fit model %1" ).arg(
      // model_names[i] ), nnls_r, use_model );
      ...
      usu.linear_fit (use_model, use_target, a[i], b[i], siga[i], sigb[i],
                      chi2[i]);
      ...
      // US_Vector::printvector3( QString( "best fit model %1, nnls_r,
      // use_model, tmp_model(scaled)\n" ).arg( model_names[i] ), nnls_r,
      // use_model, tmp_model );
      rmsds[i] = US_Saxs_Util::calc_nrmsd (tmp_model, best_fit_target);
      if (!US_Saxs_Util::calc_chisq2 (best_fit_models[model_names[i]],
                                      best_fit_target, df[i], chi2[i],
                                      prob[i]))
        {
      ...
      usu.scaling_fit (use_model, use_target, a[i], chi2[i]);
      // rescale model to target, compute rmsd
      vector<double> tmp_model (best_fit_target.size ());
      for (unsigned int j = 0; j < best_fit_target.size (); j++)
        {
  ...
  // plot
  (*remember_mw)[csv_filename + " Best Fit Model "
                 + model_names[lowest_rmsd_pos]]
      = model_mw;
  (*remember_mw_source)[csv_filename + " Model "
                        + model_names[lowest_rmsd_pos]]
      = "weight of best fit model";

  plot_one_pr (nnls_r, model,
               csv_filename + " Best Fit Model "
                   + model_names[lowest_rmsd_pos]);
  compute_rg_to_progress (nnls_r, model,
                          csv_filename + " Best Fit Model "
                              + model_names[lowest_rmsd_pos]);

  plot_one_pr (nnls_r, best_fit_target, best_fit_target_errors,
               csv_filename + " Target");
...

  {
    ...
    saxs_residuals_window = new US_Hydrodyn_Saxs_Residuals (
        ...
        use_nnls_r, use_difference,
        // use_residual,
        use_best_fit_target, use_error,
        // true,
        // true,
        // true,
        // use_error.size(),
      ...
      if (of)
        {
          ...
          fprintf (of, "\"%s\",%.2f,%.2f,\"%s\",%s\n",
                   QString ("Target %1").arg (nnls_B_name).toLatin1 ().data (),
                   model_mw, compute_pr_area (best_fit_target, nnls_r), "P(r)",
                   vector_double_to_csv (best_fit_target).toLatin1 ().data ());

          // best fit model
          fprintf (of, "\"%s\",%.2f,%.2f,\"%s\",%s\n",
                   QString ("Best Fit Model " + model_names[lowest_rmsd_pos])
                       .toLatin1 ()
                       .data (),
                   model_mw, compute_pr_area (model, nnls_r), "P(r)",
                   vector_double_to_csv (model).toLatin1 ().data ());
...
bool
US_Hydrodyn_Saxs::pvalue (const vector<double> &q, vector<double> &I,
                          vector<double> &G, double &P, QString &errormsg)
{
  errormsg = "";
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)
           ->saxs_util->cormap (q, Icm, rkl, N, S, C, P))
    {
...
bool
US_Hydrodyn_Saxs::compute_rg_to_progress (const vector<double> &r,
                                          const vector<double> &pr,
                                          const QString &filename)
{
  // QTextStream(stdout)
  //    << QString( "compute_rg_to_progress() r length %1 pr length %2 for file
  ...
  if (pr.size () < r.size ())
    {
      ...
      qDebug () << QString ("resized vectors! compute_rg_to_progress() r "
                            "length %1 pr length %2 for file %3\n")
                       .arg (r.size ())
                       .arg (pr.size ())
                       .arg (filename);
    }
  ...
  if (US_Saxs_Util::compute_rg_from_pr (use_r, pr, Rg, errormsg))
    ...
    {
      editor_msg (
          "red", QString (us_tr ("Error computing Rg from p(r) for %1 = %2\n"))
                     .arg (filename)
                     .arg (errormsg));
...
bool
US_Hydrodyn_Saxs::log_rebin (int intervals, const vector<double> &q,
                             const vector<double> &I, vector<double> &rebin_q,
                             vector<double> &rebin_I, QString &errors)
{
  vector<double> e;
...
bool
US_Hydrodyn_Saxs::log_rebin (int intervals, const vector<double> &q,
                             const vector<double> &I, const vector<double> &e,
                             vector<double> &rebin_q, vector<double> &rebin_I,
                             vector<double> &rebin_e, QString &errors)
{
  errors = "";
  ...
  // setup bins
  {
    ...
    double scale
        = (max_log - min_log) / (end_interval_val - start_interval_val);
