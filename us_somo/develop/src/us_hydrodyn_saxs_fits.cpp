...
void US_Hydrodyn_Saxs::calc_iqq_nnls_fit(QString /* title */,
  ...
  for (map<QString, vector<double> >::iterator it = nnls_A.begin();
    ...
    if (it->second.size() != max_iqq_len) {
      ...
      nnls_csv_footer << QString(
                             us_tr("\"NNLS failed, length mismatch %1 %2\""))
                             .arg(it->second.size())
                             .arg(max_iqq_len);
      return;
    }
  ...
  if (!csv_filename.isEmpty()) {
    ...
    if (of) {
      ...
      // target
      fprintf(of, "\"%s\",\"%s\",%s\n",
              QString("Target %1").arg(nnls_B_name).toLatin1().data(), "I(q)",
      ...
      if (is_nonzero_vector(nnls_errors)) {
        fprintf(of, "\"%s\",\"%s\",%s\n",
                ...
                "I(q) sd", vector_double_to_csv(nnls_errors).toLatin1().data());
      }
...
void US_Hydrodyn_Saxs::calc_iqq_best_fit(QString /* title */,
                                         QString csv_filename) {
  ...
  plot_one_iqq(
      nnls_r, model,
      csv_filename + " Best Fit Model " + model_names[lowest_chi2_pos]);
...
void US_Hydrodyn_Saxs::calc_nnls_fit(QString title, QString csv_filename) {
    ...
    if (nnls_B.size() > cropd_B.size()) {
      editor_msg("darkred",
                 QString(us_tr("Notice: cropping models to target Dmax %1\n"))
                     .arg(cropd_r.back()));
      qDebug() << QString(us_tr("Notice: cropping models to target Dmax %1"))
                      .arg(cropd_r.back());
      nnls_csv_footer << QString(us_tr("\"Cropping models to target Dmax\",%1"))
                             .arg(cropd_r.back());
  ...
  if (use_errors && org_errors_size < max_pr_len && !dmax_croplen) {
    editor_msg("darkred",
               QString(us_tr("NNLS propagating last error to extended zeros to "
                             "match maximum model length. Errors size:%1 "
                             "Maximum model length:%2\n"))
                   .arg(org_errors_size)
                   .arg(max_pr_len));
    nnls_csv_footer << QString(
                           us_tr("\"NNLS propagating last error to extended "
      ...
      if (nnls_A.count(model_name)) {
        {
          double Rg;
          QString errormsg;
          if (US_Saxs_Util::compute_rg_from_pr(nnls_r, nnls_A[model_name], Rg,
                                               errormsg)) {
...

  {
    ...
    max_len =
        (int)use_nnls_r.size() > max_len ? (int)use_nnls_r.size() : max_len;
    use_nnls_r = nnls_r;
    ...
    if (of) {
      ...
      for (unsigned int i = 0; i < use_x.size(); i++) {
        fprintf(of, "\"%s\",%.2f,%.2f,\"%s\",%s\n",
                ...
                compute_pr_area(nnls_A[model_names[i]], nnls_r), "P(r)",
                vector_double_to_csv(nnls_A[model_names[i]]).toLatin1().data());
      }
      // target
      fprintf(of, "\"%s\",%.2f,%.2f,\"%s\",%s\n",
              QString("Target %1").arg(nnls_B_name).toLatin1().data(),
              get_mw(model_names[0], false), compute_pr_area(nnls_B, nnls_r),
              "P(r)", vector_double_to_csv(nnls_B).toLatin1().data());

      // best fit model
      fprintf(of, "\"%s\",%.2f,%.2f,\"%s\",%s\n", "Model",
              get_mw(model_names[0], false), compute_pr_area(model, nnls_r),
              "P(r)", vector_double_to_csv(model).toLatin1().data());

      fprintf(of, "\"%s\",%.2f,%.2f,\"%s\",%s\n", "Residual",
              get_mw(model_names[0], false), compute_pr_area(residual, nnls_r),
              "P(r)", vector_double_to_csv(residual).toLatin1().data());
...
void US_Hydrodyn_Saxs::calc_best_fit(QString title, QString csv_filename) {
  ...
  if (use_errors && org_errors_size < max_pr_len) {
    editor_msg("darkred",
               QString(us_tr("Best propagating last error to extended zeros to "
                             "match maximum model length. Errors size:%1 "
                             "Maximum model length:%2\n"))
                   .arg(org_errors_size)
                   .arg(max_pr_len));
    nnls_csv_footer
        << QString(us_tr("\"Best fit propagating last error to extended zeros "
  ...
  for (unsigned int i = 0; i < best_fit_models.size(); i++) {
    ...
    // US_Vector::printvector3( QString( "best fit model %1, nnls_r, use_model,
    // tmp_model(scaled)\n" ).arg( model_names[i] ), nnls_r, use_model,
    // tmp_model );
    rmsds[i] = US_Saxs_Util::calc_nrmsd(tmp_model, best_fit_target);
    if (!US_Saxs_Util::calc_chisq2(best_fit_models[model_names[i]],
                                   best_fit_target, df[i], chi2[i], prob[i])) {
  ...
  editor_msg(
      "darkBlue", QString("Best fit model: %1 NRMSD %2% Chi^2 %3 nu %4\n")
  ...
  (*remember_mw)[csv_filename + " Best Fit Model " +
                 model_names[lowest_rmsd_pos]] = model_mw;
  (*remember_mw_source)[csv_filename + " Model " +
                        model_names[lowest_rmsd_pos]] =
      "weight of best fit model";

  plot_one_pr(nnls_r, model,
              csv_filename + " Best Fit Model " + model_names[lowest_rmsd_pos]);
  ...
  if (!csv_filename.isEmpty()) {
    ...
    if (of) {
      ...
      // target
      fprintf(of, "\"%s\",%.2f,%.2f,\"%s\",%s\n",
              QString("Target %1").arg(nnls_B_name).toLatin1().data(), model_mw,
              compute_pr_area(best_fit_target, nnls_r), "P(r)",
              vector_double_to_csv(best_fit_target).toLatin1().data());
...
bool US_Hydrodyn_Saxs::pvalue(const vector<double> &q, vector<double> &I,
...
bool US_Hydrodyn_Saxs::compute_rg_to_progress(const vector<double> &r,
  ...
  if (pr.size() < r.size()) {
    ...
    qDebug() << QString(
                    "resized vectors! compute_rg_to_progress() r length %1 pr "
                    "length %2 for file %3\n")
                    .arg(r.size())
                    .arg(pr.size())
                    .arg(filename);
...
bool US_Hydrodyn_Saxs::log_rebin(int intervals, const vector<double> &q,
