...
/* gsm stuff */
our_matrix *US_Hydrodyn_Mals_Fit::new_our_matrix(int rows, int cols) {
  our_matrix *m;
...
void US_Hydrodyn_Mals_Fit::free_our_matrix(our_matrix *m) {
  free(m->d);
  free(m);
}

void US_Hydrodyn_Mals_Fit::set_our_matrix(our_matrix *m, double s) {
  int i, limit;
  limit = m->rows * m->cols;
...
void US_Hydrodyn_Mals_Fit::identity_our_matrix(our_matrix *m) {
  int i;
...
void US_Hydrodyn_Mals_Fit::print_our_matrix(our_matrix *m) {
  int i, j;
...
our_vector *US_Hydrodyn_Mals_Fit::new_our_vector(int len) {
  our_vector *v;
...
void US_Hydrodyn_Mals_Fit::free_our_vector(our_vector *v) {
  free(v->d);
  free(v);
}

void US_Hydrodyn_Mals_Fit::print_our_vector(our_vector *v) {
  int i;
...
double US_Hydrodyn_Mals_Fit::l2norm_our_vector(our_vector *v1, our_vector *v2) {
  double norm = 0e0;
  int i;
...
void US_Hydrodyn_Mals_Fit::copy_our_vector(our_vector *v1, our_vector *v2) {
#if !defined(SUPRESS_ERROR_CHECKING)
  if (v1->len != v2->len) {
...
void US_Hydrodyn_Mals_Fit::set_our_vector(our_vector *v1, double s) {
  int i;
  /* could do this with ceil(log2(len)) memcpy()s */
  for (i = 0; i < v1->len; i++) {
    v1->d[i] = s;
...
void US_Hydrodyn_Mals_Fit::add_our_vector_vv(our_vector *v1, our_vector *v2) {
  int i;
#if !defined(SUPRESS_ERROR_CHECKING)
  if (v1->len != v2->len) {
...
void US_Hydrodyn_Mals_Fit::add_our_vector_vs(our_vector *v1, double s) {
  int i;
  for (i = 0; i < v1->len; i++) {
    v1->d[i] += s;
  }
...
void US_Hydrodyn_Mals_Fit::mult_our_vector_vv(our_vector *v1, our_vector *v2) {
  int i;
#if !defined(SUPRESS_ERROR_CHECKING)
  if (v1->len != v2->len) {
...
void US_Hydrodyn_Mals_Fit::mult_our_vector_vs(our_vector *v1, double s) {
  int i;
  for (i = 0; i < v1->len; i++) {
    v1->d[i] *= s;
  }
...
void US_Hydrodyn_Mals_Fit::add_our_vector_vvv(our_vector *vd, our_vector *v1,
                                              our_vector *v2) {
  int i;
#if !defined(SUPRESS_ERROR_CHECKING)
...
void US_Hydrodyn_Mals_Fit::add_our_vector_vvs(our_vector *vd, our_vector *v1,
                                              double s) {
  int i;
  for (i = 0; i < v1->len; i++) {
    vd->d[i] = v1->d[i] + s;
...
void US_Hydrodyn_Mals_Fit::sub_our_vector_vvv(our_vector *vd, our_vector *v1,
                                              our_vector *v2) {
  int i;
#if !defined(SUPRESS_ERROR_CHECKING)
  if (v1->len != v2->len || vd->len != v1->len) {
...
void US_Hydrodyn_Mals_Fit::sub_our_vector_vvs(our_vector *vd, our_vector *v1,
                                              double s) {
  int i;
  for (i = 0; i < v1->len; i++) {
    vd->d[i] = v1->d[i] - s;
...
void US_Hydrodyn_Mals_Fit::mult_our_vector_vvv(our_vector *vd, our_vector *v1,
                                               our_vector *v2) {
  int i;
#if !defined(SUPRESS_ERROR_CHECKING)
  if (v1->len != v2->len || vd->len != v1->len) {
...
void US_Hydrodyn_Mals_Fit::mult_our_vector_vvs(our_vector *vd, our_vector *v1,
                                               double s) {
  int i;
  for (i = 0; i < v1->len; i++) {
    vd->d[i] = v1->d[i] * s;
...
double US_Hydrodyn_Mals_Fit::dot_our_vector(our_vector *v1, our_vector *v2) {
  int i;
  double ret = 0e0;
  for (i = 0; i < v1->len; i++) {
    ret += v1->d[i] * v2->d[i];
...
void US_Hydrodyn_Mals_Fit::mult_our_matrix_vmv(our_vector *vd, our_matrix *m,
                                               our_vector *vs) {
  int i, j;
...
void US_Hydrodyn_Mals_Fit::our_vector_test() {
  our_vector *v1, *v2, *v3;
...
long US_Hydrodyn_Mals_Fit::min_gsm_5_1(our_vector *i, double epsilon,
                                       long max_iter) {
  ...
  while (l2norm_our_vector(u, zero) >= epsilon && iter++ < max_iter) {
    ...
    if (!(global_iter % 5)) {
      ...
      printf(
          ...
          this_rank, (gsm_f(i) <= 0e0) ? "success" : "fail", N, gsm_f(i),
          queries, fitness_type, 0, this_iterations + prev_iterations, go, 0e0,
          0e0, 0e0, 0e0, iterations_max, 0, 0, 0, min_acceptable, 0L, 0L);
      puts("");
    }
#endif
  }
...
long US_Hydrodyn_Mals_Fit::min_fr_pr_cgd(our_vector *i, double epsilon,
                                         long max_iter) {
...
long US_Hydrodyn_Mals_Fit::min_hessian_bfgs(our_vector *ip, double epsilon,
                                            long max_iter) {
#if defined(SHOW_TIMING)
  struct timeval tv1, tv2;
#endif
    ...
    */
    ...
    if (!(global_iter % 5)) {
      ...
      printf(
          ...
          "%.12g|%.12g|%ld|%d|%d|%d|%.12g|%ld|%ld",
          this_rank, (gsm_f(ip) <= 0e0) ? "success" : "fail", N, gsm_f(ip),
          queries, fitness_type, 0e0, this_iterations + prev_iterations, go,
          0e0, 0e0, 0e0, 0e0, iterations_max, 0, 0, 0, min_acceptable, 0L, 0L);
...
void US_Hydrodyn_Mals_Fit::gsm_setup() {
...
double US_Hydrodyn_Mals_Fit::gsm_f_GAUSS(our_vector *v) {
...
double US_Hydrodyn_Mals_Fit::gsm_f_EMG(our_vector *v) {
  for (unsigned int j = 0; j < gsm_yp.size(); j++) {
    gsm_yp[j] = 0e0;
  }
...
double US_Hydrodyn_Mals_Fit::gsm_f_GMG(our_vector *v) {
  for (unsigned int j = 0; j < gsm_yp.size(); j++) {
    gsm_yp[j] = 0e0;
  }
...
double US_Hydrodyn_Mals_Fit::gsm_f_EMGGMG(our_vector *v) {
  for (unsigned int j = 0; j < gsm_yp.size(); j++) {
    gsm_yp[j] = 0e0;
  }
...
void US_Hydrodyn_Mals_Fit::gsm_df(our_vector *vd, our_vector *v) {
  double y0;
  double y2;
  double sav_ve;
