...
vector<double> US_Hydrodyn_Mals_Fit::gsm_t;
vector<double> US_Hydrodyn_Mals_Fit::gsm_y;
vector<double> US_Hydrodyn_Mals_Fit::gsm_yp;

/* gsm stuff */
our_matrix *
US_Hydrodyn_Mals_Fit::new_our_matrix (int rows, int cols)
{
  our_matrix *m;
...
void
US_Hydrodyn_Mals_Fit::free_our_matrix (our_matrix *m)
{
  free (m->d);
  free (m);
}
...
void
US_Hydrodyn_Mals_Fit::set_our_matrix (our_matrix *m, double s)
{
  int i, limit;
  limit = m->rows * m->cols;
...
void
US_Hydrodyn_Mals_Fit::identity_our_matrix (our_matrix *m)
{
  int i;
...
void
US_Hydrodyn_Mals_Fit::print_our_matrix (our_matrix *m)
{
  int i, j;
...
our_vector *
US_Hydrodyn_Mals_Fit::new_our_vector (int len)
{
  our_vector *v;
...
void
US_Hydrodyn_Mals_Fit::free_our_vector (our_vector *v)
{
  free (v->d);
  free (v);
}
...
void
US_Hydrodyn_Mals_Fit::print_our_vector (our_vector *v)
{
  int i;
...
double
US_Hydrodyn_Mals_Fit::l2norm_our_vector (our_vector *v1, our_vector *v2)
{
  double norm = 0e0;
  int i;
...
void
US_Hydrodyn_Mals_Fit::copy_our_vector (our_vector *v1, our_vector *v2)
{
#if !defined(SUPRESS_ERROR_CHECKING)
  if (v1->len != v2->len)
    {
...
void
US_Hydrodyn_Mals_Fit::set_our_vector (our_vector *v1, double s)
{
  int i;
  /* could do this with ceil(log2(len)) memcpy()s */
  for (i = 0; i < v1->len; i++)
...
void
US_Hydrodyn_Mals_Fit::add_our_vector_vv (our_vector *v1, our_vector *v2)
{
  int i;
#if !defined(SUPRESS_ERROR_CHECKING)
  if (v1->len != v2->len)
...
void
US_Hydrodyn_Mals_Fit::add_our_vector_vs (our_vector *v1, double s)
{
  int i;
  for (i = 0; i < v1->len; i++)
    {
...
void
US_Hydrodyn_Mals_Fit::mult_our_vector_vv (our_vector *v1, our_vector *v2)
{
  int i;
#if !defined(SUPRESS_ERROR_CHECKING)
  if (v1->len != v2->len)
...
void
US_Hydrodyn_Mals_Fit::mult_our_vector_vs (our_vector *v1, double s)
{
  int i;
  for (i = 0; i < v1->len; i++)
    {
...
void
US_Hydrodyn_Mals_Fit::add_our_vector_vvv (our_vector *vd, our_vector *v1,
                                          our_vector *v2)
{
  int i;
#if !defined(SUPRESS_ERROR_CHECKING)
...
void
US_Hydrodyn_Mals_Fit::add_our_vector_vvs (our_vector *vd, our_vector *v1,
                                          double s)
{
  int i;
  for (i = 0; i < v1->len; i++)
...
void
US_Hydrodyn_Mals_Fit::sub_our_vector_vvv (our_vector *vd, our_vector *v1,
                                          our_vector *v2)
{
  int i;
#if !defined(SUPRESS_ERROR_CHECKING)
...
void
US_Hydrodyn_Mals_Fit::sub_our_vector_vvs (our_vector *vd, our_vector *v1,
                                          double s)
{
  int i;
  for (i = 0; i < v1->len; i++)
...
void
US_Hydrodyn_Mals_Fit::mult_our_vector_vvv (our_vector *vd, our_vector *v1,
                                           our_vector *v2)
{
  int i;
#if !defined(SUPRESS_ERROR_CHECKING)
...
void
US_Hydrodyn_Mals_Fit::mult_our_vector_vvs (our_vector *vd, our_vector *v1,
                                           double s)
{
  int i;
  for (i = 0; i < v1->len; i++)
...
double
US_Hydrodyn_Mals_Fit::dot_our_vector (our_vector *v1, our_vector *v2)
{
  int i;
  double ret = 0e0;
  for (i = 0; i < v1->len; i++)
...
void
US_Hydrodyn_Mals_Fit::mult_our_matrix_vmv (our_vector *vd, our_matrix *m,
                                           our_vector *vs)
{
  int i, j;
...
void
US_Hydrodyn_Mals_Fit::our_vector_test ()
{
  our_vector *v1, *v2, *v3;
...
long
US_Hydrodyn_Mals_Fit::min_gsm_5_1 (our_vector *i, double epsilon,
                                   long max_iter)
{
  ...
  while (l2norm_our_vector (u, zero) >= epsilon && iter++ < max_iter)
    {
      ...
      printf (
          ...
          this_rank, global_iter++, iter, fitness = gsm_f (i),
      ...
      if (!(global_iter % 5))
        {
          ...
          printf ("%d: "
                  ...
                  this_rank, (gsm_f (i) <= 0e0) ? "success" : "fail", N,
                  gsm_f (i), queries, fitness_type, 0,
...
long
US_Hydrodyn_Mals_Fit::min_fr_pr_cgd (our_vector *i, double epsilon,
                                     long max_iter)
{
...
long
US_Hydrodyn_Mals_Fit::min_hessian_bfgs (our_vector *ip, double epsilon,
                                        long max_iter)
{
  ...
  while (l2norm_our_vector (u, zero) >= epsilon && iter++ < max_iter)
    {
      ...
      printf (
          "%d: global %ld, iter %ld fitness %.12g |grad|=%.12g last reps %d\n",
          this_rank, global_iter++, iter, fitness = gsm_f (ip),
          l2norm_our_vector (u, zero), last_reps);
      fflush (stdout);
      printf ("%d: ", this_rank);
      print_our_vector (ip);
      ...
      if (!(global_iter % 5))
        {
          ...
          printf ("%d: "
                  ...
                  this_rank, (gsm_f (ip) <= 0e0) ? "success" : "fail", N,
                  gsm_f (ip), queries, fitness_type, 0e0,
                  this_iterations + prev_iterations, go, 0e0, 0e0, 0e0, 0e0,
                  iterations_max, 0, 0, 0, min_acceptable, 0L, 0L);
...
void
US_Hydrodyn_Mals_Fit::gsm_setup ()
{
  ...
  for (unsigned int j = 0; j < x.size (); j++)
    {
      if (x[j] >= start && x[j] <= end)
        {
          gsm_t.push_back (x[j]);
          gsm_y.push_back (mals_win->f_Is[mals_win->wheel_file][j]);
        }
    }
...
double
US_Hydrodyn_Mals_Fit::gsm_f_GAUSS (our_vector *v)
{
  ...
  for (unsigned int i = 0; i < (unsigned int)MFIT::param_fixed.size ();)
    {
      if (MFIT::param_fixed[i])
        {
          height = MFIT::fixed_params[MFIT::param_pos[i]];
        }
      else
        {
          height = v->d[MFIT::param_pos[i]];
          if (height < MFIT::param_min[MFIT::param_pos[i]]
              || height > MFIT::param_max[MFIT::param_pos[i]])
            {
              return 1e99;
            }
        }
      ...
      if (MFIT::param_fixed[i])
        {
          center = MFIT::fixed_params[MFIT::param_pos[i]];
        }
      else
        {
          center = v->d[MFIT::param_pos[i]];
          if (center < MFIT::param_min[MFIT::param_pos[i]]
              || center > MFIT::param_max[MFIT::param_pos[i]])
            {
              return 1e99;
            }
        }
      ...
      if (MFIT::param_fixed[i])
        {
          width = MFIT::fixed_params[MFIT::param_pos[i]];
        }
      else
        {
          width = v->d[MFIT::param_pos[i]];
          if (width < MFIT::param_min[MFIT::param_pos[i]]
              || width > MFIT::param_max[MFIT::param_pos[i]])
            {
              return 1e99;
            }
        }
...
double
US_Hydrodyn_Mals_Fit::gsm_f_EMG (our_vector *v)
{
  for (unsigned int j = 0; j < gsm_yp.size (); j++)
    {
      gsm_yp[j] = 0e0;
  ...
  for (unsigned int i = 0; i < (unsigned int)MFIT::param_fixed.size ();)
    {
      ...
      if (MFIT::param_fixed[i])
        {
          if (MFIT::comm_backref.count (i))
            {
              dist1 = v->d[MFIT::comm_backref[i]];
            }
          else
            {
              dist1 = MFIT::fixed_params[MFIT::param_pos[i]];
            }
        }
      else
        {
          dist1 = v->d[MFIT::param_pos[i]];
          if (dist1 < MFIT::param_min[MFIT::param_pos[i]]
              || dist1 > MFIT::param_max[MFIT::param_pos[i]])
            {
              return 1e99;
            }
        }
          ...
          if (fabs (dist1) < dist1_thresh)
          ...
          else
            {
              ...
              for (unsigned int j = 0; j < gsm_yp.size (); j++)
                {
                  double tmp = gsm_t[j] - center;
                  gsm_yp[j] += emg_coeff * exp (emg_exp_1 - one_over_a3 * tmp)
                               * (use_erf (tmp * one_over_sqrt2_a2 - emg_erf_2)
                                  + sign_a3);
                }
            }
...
double
US_Hydrodyn_Mals_Fit::gsm_f_GMG (our_vector *v)
{
  for (unsigned int j = 0; j < gsm_yp.size (); j++)
    {
      gsm_yp[j] = 0e0;
      ...
      if (dist1)
        {
          ...
          for (unsigned int j = 0; j < gsm_yp.size (); j++)
            {
              double tmp = gsm_t[j] - center;
              gsm_yp[j] += gmg_coeff * exp (gmg_exp_m1 * tmp * tmp)
...
double
US_Hydrodyn_Mals_Fit::gsm_f_EMGGMG (our_vector *v)
{
  for (unsigned int j = 0; j < gsm_yp.size (); j++)
    {
      gsm_yp[j] = 0e0;
      ...
      if (MFIT::param_fixed[i])
        {
          if (MFIT::comm_backref.count (i))
            {
              width = v->d[MFIT::comm_backref[i]] * MFIT::conc_ratios_map[i];
            }
          else
            {
              width = MFIT::fixed_params[MFIT::param_pos[i]];
      ...
      if (MFIT::param_fixed[i])
        {
          if (MFIT::comm_backref.count (i))
            {
              dist2 = v->d[MFIT::comm_backref[i]];
            }
          else
            {
              dist2 = MFIT::fixed_params[MFIT::param_pos[i]];
            }
        }
      else
        {
          dist2 = v->d[MFIT::param_pos[i]];
          if (dist2 < MFIT::param_min[MFIT::param_pos[i]]
              || dist2 > MFIT::param_max[MFIT::param_pos[i]])
            {
              return 1e99;
            }
        }
...
void
US_Hydrodyn_Mals_Fit::gsm_df (our_vector *vd, our_vector *v)
{
  double y0;
  double y2;
  double sav_ve;
