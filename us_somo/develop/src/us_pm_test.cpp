...
QString US_PM::test(QString name, QString oname) {
  ...
  if (BEST) {
    ...
    if (BEST_SPHERE) {
      ...
      QString outname =
          QString("%1_sh%2_best_sphere").arg(oname).arg(max_harmonics);
    ...
    if (BEST_CYLINDER) {
      ...
      QString outname =
          QString("%1_sh%2_best_cylinder").arg(oname).arg(max_harmonics);
      sphere_pm.write_model(outname, model);
      sphere_pm.write_I(outname, model);
    ...
    if (BEST_SPHEROID) {
      ...
      QString outname =
          QString("%1_sh%2_best_spheroid").arg(oname).arg(max_harmonics);
      sphere_pm.write_model(outname, model);
      sphere_pm.write_I(outname, model);
  ...
  if (MAKE_SPHERE_AND_SPHEROID) {
    ...
    QString outname =
        QString("%1_sh%2_sphere_%3").arg(oname).arg(max_harmonics).arg(radius);
    ...
    sphere_pm.create_model(params, model);
    outname =
        QString("%1_sh%2_torus_%3").arg(oname).arg(max_harmonics).arg(radius);
    sphere_pm.write_model(outname, model, params);
    sphere_pm.write_I(outname, model);
    ...
    sphere_pm.create_model(params, model);
    outname =
        QString("%1_sh%2_torus_2_%3").arg(oname).arg(max_harmonics).arg(radius);
    sphere_pm.write_model(outname, model, params);
    sphere_pm.write_I(outname, model);
  ...
  if (BEST_MD0_GA) {
    ...
    if (BEST_MD0_GA_SPHERE) {
      ...
      sphere_pm.best_md0_ga(params, model, steps, pts_max,
                            grid_conversion_factor, coarse, ref_pct, conv_div);
      us_timers.stop_all();

      QString outname =
          QString("%1_sh%2_best_MD0_GA_sphere").arg(oname).arg(max_harmonics);
      sphere_pm.write_model(outname, model, params);
      sphere_pm.write_I(outname, model);
    ...
    if (BEST_MD0_GA_CYLINDER) {
      ...
      QString outname =
          QString("%1_sh%2_best_MD0_GA_cylinder").arg(oname).arg(max_harmonics);
      sphere_pm.write_model(outname, model, params);
      sphere_pm.write_I(outname, model);
    ...
    if (BEST_MD0_GA_SPHEROID) {
      ...
      QString outname =
          QString("%1_sh%2_best_MD0_GA_spheroid").arg(oname).arg(max_harmonics);
      sphere_pm.write_model(outname, model, params);
      sphere_pm.write_I(outname, model);
    ...
    if (BEST_MD0_GA_TORUS) {
      ...
      QString outname =
          QString("%1_sh%2_best_MD0_GA_torus").arg(oname).arg(max_harmonics);
      sphere_pm.write_model(outname, model, params);
      sphere_pm.write_I(outname, model);
  ...
  if (PERF_TEST) {
    ...
    if (1) {  // ascending sphere non-delta test
    ...
    if (0) {  // descending sphere delta test
      ...
      for (params[1] = spheretest_max; params[1] >= spheretest_min;
        ...
        cout
            << QString(
                   "---------- delta descending Sphere radius %1 -----------\n")
                   .arg(params[1])
                   .toLatin1()
                   .data();
        model.clear();
