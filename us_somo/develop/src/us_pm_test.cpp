...
#define BEST_MD0                                                              \
  (BEST_MD0_SPHERE || BEST_MD0_CYLINDER || BEST_MD0_SPHEROID || BEST_MD0_TORUS)
...
#define BEST_MD0_GA                                                           \
  (BEST_MD0_GA_SPHERE || BEST_MD0_GA_CYLINDER || BEST_MD0_GA_SPHEROID         \
   || BEST_MD0_GA_TORUS)
...
QString
US_PM::test (QString name, QString oname)
{
  vector<double> q;
  vector<double> I;
  vector<double> e;
                  ...
                  if (!qs.isEmpty ())
                    {
                      ...
                      if (qsl.size () < 2)
                        {
                          cout << QString (
                                      "error: data has only %1 parameters\n")
                                      .arg (qsl.size ());
                          return "error";
                        }
...

  {
    ...
    if (!usu.compute_rayleigh_structure_factors (
            pow (pow (grid_conversion_factor, 3e0) / M_PI, 1e0 / 3e0), drho, q,
            F))
      {
        cout << "could not compute rayleigh structure factors " << usu.errormsg
             << endl;
        return "error";
      }
  }
  ...
  if (NEW_TEST)
    {
      US_PM sphere_pm (grid_conversion_factor, max_dimension,
                       //                        drho,
                       //                        buffer_e_density,
                       //                        ev,
                       max_harmonics,
      ...
      if (params == jparams)
        ...
        {
          cout << "join ERROR\n";
          US_Vector::printvector2 ("params original after join", params,
                                   jparams);
        }
      ...
      for (int i = 0; i < RANDOM_TEST; i++)
        {
          ...
          cout << QString ("Random test %1 of %2:\n")
                      .arg (i + 1)
                      .arg (RANDOM_TEST)
               << sphere_pm.list_params (params) << endl;
      ...
      if (BEST_SPHERE)
        {
          ...
          QString outname = QString ("%1_sh%2_best_sphere")
                                .arg (oname)
                                .arg (max_harmonics);
      ...
      if (BEST_CYLINDER)
        {
          ...
          QString outname = QString ("%1_sh%2_best_cylinder")
                                .arg (oname)
                                .arg (max_harmonics);
          sphere_pm.write_model (outname, model);
          sphere_pm.write_I (outname, model);
      ...
      if (BEST_SPHEROID)
        {
          ...
          QString outname = QString ("%1_sh%2_best_spheroid")
                                .arg (oname)
                                .arg (max_harmonics);
          sphere_pm.write_model (outname, model);
          sphere_pm.write_I (outname, model);
      ...
      if (BEST_TORUS)
        {
          ...
          QString outname
              = QString ("%1_sh%2_best_torus").arg (oname).arg (max_harmonics);
          sphere_pm.write_model (outname, model);
          sphere_pm.write_I (outname, model);
  ...
  if (MAKE_SPHERE_AND_SPHEROID)
    {
      ...
      QString outname = QString ("%1_sh%2_sphere_%3")
                            .arg (oname)
                            .arg (max_harmonics)
                            .arg (radius);
      ...
      sphere_pm.create_model (params, model);
      outname = QString ("%1_sh%2_spheroid_%3")
                    .arg (oname)
                    .arg (max_harmonics)
                    .arg (radius);
      sphere_pm.write_model (outname, model, params);
      ...
      sphere_pm.create_model (params, model);
      outname = QString ("%1_sh%2_ellipsoid_%3")
                    .arg (oname)
                    .arg (max_harmonics)
                    .arg (radius);
      sphere_pm.write_model (outname, model, params);
      ...
      sphere_pm.create_model (params, model);
      outname = QString ("%1_sh%2_torus_%3")
                    .arg (oname)
                    .arg (max_harmonics)
                    .arg (radius);
      sphere_pm.write_model (outname, model, params);
      ...
      sphere_pm.create_model (params, model);
      outname = QString ("%1_sh%2_torus_2_%3")
                    .arg (oname)
                    .arg (max_harmonics)
                    .arg (radius);
      sphere_pm.write_model (outname, model, params);
  ...
  if (GRID_SEARCH_SPHEROID)
    {
      ...
      QString outname = QString ("%1_sh%2_best_grid_search_spheroid")
                            .arg (oname)
                            .arg (max_harmonics);
      sphere_pm.write_model (outname, model, params);
      sphere_pm.write_I (outname, model);
  ...
  if (BEST_MD0_GA)
    {
      ...
      sphere_pm.set_best_delta (best_delta_start, best_delta_divisor,
                                best_delta_min);
      if (BEST_MD0_GA_SPHERE)
        {
          cout << "starting best sphere\n";
          params[0] = 0e0;
          sphere_pm.best_md0_ga (params, model, steps, pts_max,
                                 grid_conversion_factor, coarse, ref_pct,
                                 conv_div);
          us_timers.stop_all ();

          QString outname = QString ("%1_sh%2_best_MD0_GA_sphere")
                                .arg (oname)
                                .arg (max_harmonics);
          sphere_pm.write_model (outname, model, params);
          sphere_pm.write_I (outname, model);
      ...
      if (BEST_MD0_GA_CYLINDER)
        {
          ...
          QString outname = QString ("%1_sh%2_best_MD0_GA_cylinder")
                                .arg (oname)
                                .arg (max_harmonics);
          sphere_pm.write_model (outname, model, params);
          sphere_pm.write_I (outname, model);
      ...
      if (BEST_MD0_GA_SPHEROID)
        {
          ...
          QString outname = QString ("%1_sh%2_best_MD0_GA_spheroid")
                                .arg (oname)
                                .arg (max_harmonics);
          sphere_pm.write_model (outname, model, params);
          sphere_pm.write_I (outname, model);
      ...
      if (BEST_MD0_GA_TORUS)
        {
          ...
          QString outname = QString ("%1_sh%2_best_MD0_GA_torus")
                                .arg (oname)
                                .arg (max_harmonics);
          sphere_pm.write_model (outname, model, params);
          sphere_pm.write_I (outname, model);
          ...
          for (i = 0; i < reps; i++)
            {
              ...
              for (params[1] = spheretest_min; params[1] <= spheretest_max;
                   ++params[1])
                {
                  sphere_pm.msg_log = "";
                  model.clear ();
  ...
  if (PERF_TEST)
    {
              ...
              for (int d = 0; d < dups; d++)
                {
                  ...
                  */
                  ...
                  QString outname = QString ("%1_sh%2_FC_sphere_asc_x%2")
                                        .arg (oname)
                                        .arg (max_harmonics)
                                        .arg (params[1]);
                  sphere_pm.write_model (outname, model);
        ...
        { // ascending sphere delta test
          ...
          for (params[1] = spheretest_min; params[1] <= spheretest_max;
               ++params[1])
            {
              ...
              QString outname = QString ("%1_sh%2_AscDelta_sphere_x%2")
                                    .arg (oname)
                                    .arg (max_harmonics)
                                    .arg (params[1]);
              sphere_pm.write_model (outname, model);
              ...
              for (int d = 0; d < dups; d++)
                {
                  ...
                  QString outname = QString ("%1_sh%2_STD_sphere_x%2")
                                        .arg (oname)
                                        .arg (max_harmonics)
                                        .arg (params[1]);
                  sphere_pm.write_model (outname, model);
      ...
      if (0)
        { // descending sphere delta test
          ...
          for (params[1] = spheretest_max; params[1] >= spheretest_min;
               --params[1])
            {
              ...
              QString outname = QString ("%1_sh%2_DscDelta_sphere_x%2")
                                    .arg (oname)
                                    .arg (max_harmonics)
                                    .arg (params[1]);
              sphere_pm.write_model (outname, model);
  ...
  if (STD_MODEL) // standard model test
    {
      ...
      US_PM test_pm (grid_conversion_factor, max_dimension,
                     //                      drho,
                     //                      buffer_e_density,
                     //                      ev,
                     max_harmonics,
      ...
      if (q.size ())
        {
          ...
          QString outfile
              = QString ("%1_sh%2").arg (oname).arg (max_harmonics);
          ...
          QTextStream ts (&of);
          ts << "# US-SOMO PM .dat file containing I(q) computed on bead "
                "model\n";
