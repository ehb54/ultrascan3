...
US_AddHybridization::US_AddHybridization (bool *widget_flag, QWidget *p,
                                          const char *)
    : QWidget (p)
{
...
US_AddHybridization::setupGUI ()
{
  ...
  pb_select_file
      = new QPushButton (us_tr ("Load Hybridization Definition File"), this);
  ...
  cmb_hybrid->setObjectName ("Hybridization Listing");
  ...
  lbl_table_saxs->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize));

  cmb_saxs = new QComboBox (this);
  cmb_saxs->setObjectName ("Hybridization Listing");
  ...
  le_mw->setObjectName ("Molecular Weight Line Edit");
  ...
  lbl_number_of_saxs
      = new QLabel (us_tr (" Number of SAXS Atoms in File: 0"), this);
  ...
  AUTFBACK (lbl_radius);
  lbl_radius->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize - 1,
                              QFont::Bold));

  le_radius = new QLineEdit (this);
  le_radius->setObjectName ("Radius Line Edit");
  le_radius->setPalette (PALET_NORMAL);
  AUTFBACK (le_radius);
  le_radius->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize));
  le_radius->setMinimumHeight (minHeight1);
  connect (le_radius, SIGNAL (textChanged (const QString &)),
           SLOT (update_radius (const QString &)));

  lbl_scat_len = new QLabel (
      us_tr (" Neutron scattering length in H2O (*10^-12 cm):"), this);
  ...
  lbl_scat_len->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize - 1,
                                QFont::Bold));

  le_scat_len = new QLineEdit (this);
  le_scat_len->setObjectName ("Scat_Len Line Edit");
  le_scat_len->setPalette (PALET_NORMAL);
  AUTFBACK (le_scat_len);
  le_scat_len->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  le_scat_len->setMinimumHeight (minHeight1);
  connect (le_scat_len, SIGNAL (textChanged (const QString &)),
           SLOT (update_scat_len (const QString &)));
  ...
  lbl_exch_prot->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1,
                                 QFont::Bold));

  le_exch_prot = new QLineEdit (this);
  le_exch_prot->setObjectName ("Exch_Prot Line Edit");
  le_exch_prot->setPalette (PALET_NORMAL);
  AUTFBACK (le_exch_prot);
  le_exch_prot->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  le_exch_prot->setMinimumHeight (minHeight1);
  connect (le_exch_prot, SIGNAL (textChanged (const QString &)),
           SLOT (update_exch_prot (const QString &)));
  ...
  lbl_num_elect->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1,
                                 QFont::Bold));

  le_num_elect = new QLineEdit (this);
  le_num_elect->setObjectName ("Num_Elect Line Edit");
  le_num_elect->setPalette (PALET_NORMAL);
  AUTFBACK (le_num_elect);
  le_num_elect->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  le_num_elect->setMinimumHeight (minHeight1);
  connect (le_num_elect, SIGNAL (textChanged (const QString &)),
           SLOT (update_num_elect (const QString &)));
  ...
  le_name->setObjectName ("Hybridization name Line Edit");
  ...
  if (f.open (QIODevice::WriteOnly | QIODevice::Text))
      ...
      for (unsigned int i = 0; i < hybrid_list.size (); i++)
        {
          ts << hybrid_list[i].saxs_name << "\t"
             << hybrid_list[i].name.toUpper () << "\t" << hybrid_list[i].mw
             << "\t" << hybrid_list[i].radius << "\t"
             << hybrid_list[i].scat_len << "\t" << hybrid_list[i].exch_prot
             << "\t" << hybrid_list[i].num_elect << Qt::endl;
          //         cout << "item: " << item << ", " <<
          //         hybrid_list[i].name.toUpper() << "\t" << hybrid_list[i].mw
          //         << "\t" << hybrid_list[i].radius << ", " <<
          //         hybrid_filename << endl;
          str1.sprintf ("%d: ", i + 1);
  ...
  else
    {
      US_Static::us_message ("Attention:",
                             "Could not open the hybridization file:\n\n"
                                 + hybrid_filename);
    }
}

void
US_AddHybridization::select_file ()
{
...
void
US_AddHybridization::select_saxs_file ()
{
...
void
US_AddHybridization::update_mw (const QString &str)
{
  current_hybrid.mw = str.toFloat ();
}

void
US_AddHybridization::update_radius (const QString &str)
{
  current_hybrid.radius = str.toFloat ();
}

void
US_AddHybridization::update_scat_len (const QString &str)
{
  current_hybrid.scat_len = str.toFloat ();
}

void
US_AddHybridization::update_exch_prot (const QString &str)
{
  current_hybrid.exch_prot = str.toInt ();
}

void
US_AddHybridization::update_num_elect (const QString &str)
{
  current_hybrid.num_elect = str.toInt ();
}

void
US_AddHybridization::update_name (const QString &str)
{
  current_hybrid.name = str;
}

void
US_AddHybridization::select_hybrid (int val)
{
  QString str;
...
void
US_AddHybridization::select_saxs (int val)
{
...
void
US_AddHybridization::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 30;
  global_Ypos -= 30;
