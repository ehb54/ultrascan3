...
void US_AddHybridization::add() {
  ...
  if (f.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    for (unsigned int i = 0; i < hybrid_list.size(); i++) {
      ts << hybrid_list[i].saxs_name << "\t" << hybrid_list[i].name.toUpper()
         << "\t" << hybrid_list[i].mw << "\t" << hybrid_list[i].radius << "\t"
         << hybrid_list[i].scat_len << "\t" << hybrid_list[i].exch_prot << "\t"
         << hybrid_list[i].num_elect << Qt::endl;
      //         cout << "item: " << item << ", " <<
      //         hybrid_list[i].name.toUpper() << "\t" << hybrid_list[i].mw <<
      //         "\t" << hybrid_list[i].radius << ", " <<  hybrid_filename <<
      //         endl;
      str1.sprintf("%d: ", i + 1);
