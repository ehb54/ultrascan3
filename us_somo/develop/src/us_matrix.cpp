...
void
m3vm (double ***matrix, double **vector, unsigned int points)
/*
...
void
m3vm (float ***matrix, double **vector, unsigned int points)
/*
  This function multiplies a tri-diagonal matrix of size [points x points]
  stored in a [points x 3] matrix by a vector of size [points] and returns
  the result in vector:
...
#ifdef ADOLC
void
m3vm (double ***matrix, adouble **vector, unsigned int points)
/*
  This function multiplies a tri-diagonal matrix of size [points x points]
  stored in a [points x 3] matrix by a vector of size [points] and returns
  the result in vector:
...
void
m3vm (adouble ***matrix, adouble **vector, unsigned int points)
/*
  This function multiplies a tri-diagonal matrix of size [points x points]
  stored in a [points x 3] matrix by a vector of size [points] and returns
  the result in vector:
...
void
m3vm (float ***matrix, adouble **vector, unsigned int points)
/*
  This function multiplies a tri-diagonal matrix of size [points x points]
  stored in a [points x 3] matrix by a vector of size [points] and returns
  the result in vector:
...
#endif
void
m3vm (float ***matrix, float **vector, unsigned int points)
/*
  This function multiplies a tri-diagonal matrix of size [points x points]
  stored in a [points x 3] matrix by a vector of size [points] and returns
  the result in vector:
...
#ifdef ADOLC
void
m3vm_a (adouble ***matrix, adouble **vector, unsigned int points,
        float constant)
/*
  This function multiplies a tri-diagonal matrix of size [points x points]
  stored in a [points x 3] matrix by a vector of size [points] and returns
...
*/
{
  ...
  result[0] = constant * (*matrix)[0][1] * (*vector)[0]
              + constant * (*matrix)[0][2] * (*vector)[1];
  for (i = 1; i < points - 1; i++)
    {
      result[i] = 0.0;
      for (j = 0; j < 3; j++)
        {
          result[i]
              = result[i] + constant * (*matrix)[i][j] * (*vector)[i + j - 1];
        }
    }
  result[points - 1]
      = constant * (*matrix)[points - 1][0] * (*vector)[points - 2]
...
void
m3vm_b (float ***b, adouble ***matrix, adouble **vector, unsigned int points,
        float constant)
/*
  This function multiplies a tri-diagonal matrix of size [points x points]
  stored in a [points x 3] matrix by a vector of size [points] and returns
...
*/
{
  ...
  result[0] = ((*b)[0][1] + constant * (*matrix)[0][1]) * (*vector)[0]
              + ((*b)[0][2] + constant * (*matrix)[0][2]) * (*vector)[1];
  for (i = 1; i < points - 1; i++)
    {
      result[i] = 0.0;
      for (j = 0; j < 3; j++)
        {
          result[i] = result[i]
                      + ((*b)[i][j] + constant * (*matrix)[i][j])
                            * (*vector)[i + j - 1];
        }
    }
  result[points - 1]
      = ((*b)[points - 1][0] + constant * (*matrix)[points - 1][0])
            * (*vector)[points - 2]
        + ((*b)[points - 1][1] + constant * (*matrix)[points - 1][1])
              * (*vector)[points - 1];
  for (i = 0; i < points; i++)
    {
      (*vector)[i] = result[i];
...
#endif
void
m3vm_b (float ***b, float ***matrix, float **vector, unsigned int points,
        float constant)
/*
  This function multiplies a tri-diagonal matrix of size [points x points]
  stored in a [points x 3] matrix by a vector of size [points] and returns
...
void
m3vm_b (double ***b, double ***matrix, double **vector, unsigned int points,
        double constant)
/*
  This function multiplies a tri-diagonal matrix of size [points x points]
  stored in a [points x 3] matrix by a vector of size [points] and returns
...
void
m2vm (double ***matrix, double **vector, int points)
{
  int i, j;
  double *result;
  result = new double[points];
...
void
m2vm (float ***matrix, float **vector, int points)
{
  int i, j;
  float *result;
  result = new float[points];
...
#ifdef ADOLC
void
m2vm (double ***matrix, adouble **vector, int points)
{
  int i, j;
  adouble *result;
  result = new adouble[points];
...
void
m2vm (float ***matrix, adouble **vector, int points)
{
  int i, j;
  adouble *result;
  result = new adouble[points];
...
void
mmv (float **result, double **vector, float ***matrix, int row, int column)
{
  int i, j;
  for (i = 0; i < row; i++)
    {
...
void
mmv (float **result, double **vector, double ***matrix, int row, int column)
{
  int i, j;
  for (i = 0; i < row; i++)
    {
...
void
mmv (float **result, float **vector, double ***matrix, int row, int column)
{
  int i, j;
  for (i = 0; i < row; i++)
    {
...
void
mmv (float **result, float **vector, float ***matrix, int row, int column)
{
  int i, j;
  for (i = 0; i < row; i++)
    {
...
float
dotproduct (float **v1, float **v2, int size)
{
  float result = 0;
...
void
vvt (float ***result, float **v1, float **v2, int size)
{
  for (int i = 0; i < size; i++)
    {
...
void
ldu (double ***matrix, double **vector, unsigned int rows,
     unsigned int columns, bool flag)
{
  unsigned int i, j, k, jbgn, jend, jrow = 0, jcol = 0, k1, k2, irow, ibd2,
                                    idiag;
  ...
  for (i = 1; i < rows; i++)
    {
      ...
      (*vector)[irow - 1]
          = (*vector)[irow - 1] / (*matrix)[irow - 1][idiag - 1];
    }
}

void
ldu (float ***matrix, float **vector, unsigned int rows, unsigned int columns,
     bool flag)
{
  unsigned int i, j, k, k1, k2, irow, ibd2, idiag, jbgn, jend, jrow = 0,
                                                               jcol = 0;
  float fac;
...
#ifdef ADOLC
void
ldu (adouble ***matrix, adouble **vector, unsigned int rows,
     unsigned int columns, bool flag)
{
  unsigned int i, j, k, jbgn, jend, jrow = 0, jcol = 0, k1, k2, irow, ibd2,
                                    idiag;
...
void
ldu (float ***matrix, adouble **vector, unsigned int rows,
     unsigned int columns, bool flag)
{
  unsigned int i, j, k, jbgn, jend, jrow = 0, jcol = 0, k1, k2, irow, ibd2,
                                    idiag;
...
bool
Cholesky_Decomposition (double **a, int n)
{
  double sum, diff;
  int i, j, k;
  for (i = 0; i < n; i++)
...
//
// A*A(-1)=I, A=LL', L(L'*A(-1)) = I, L'*A(-1) = y, L * y = I (solve for y),
// now L' * A(-1) = y (solve for A(-1))
//

bool
Cholesky_Invert (double **a, double **ainv, int n)
{
  int i, j;
  double *vector;
  vector = new double[n];
 ...
 */
bool
Cholesky_SolveSystem (double **l, double *b, int n)
{
  int i, j;
...
void
LU_Decomposition (double **matrix, int *index, bool parity, int n)
{
...
void
LU_BackSubstitute (double **a, double **b, int *index, int n)
{
  // counters
  int i, ip, j, ii = -1;
  double sum = 0;
...
void
LU_SolveSystem (double **a, double **b, int n)
{
  bool parity = true;
  int *index;
  index = new int[n];
...
void
LU_Invert (double **matrix, double **result, int n)
{
  int i, *index;
...
/* This routine is supplied with M, a matrix containing the Jacobian (the value
   of each linear functional term at point x_i:
   ...
   The variable "coeff" is a vector containing the linear coefficients for each
   functional term. y_raw contains the experimental y-values on entry
*/
void
generalLeastSquares (float **M, unsigned int points, unsigned int order,
                     float *y_raw, float **coeff)
{
  double **A, *B;
  unsigned int i, j, k;
  A = new double *[order];
  ...
  // generate matrix for decomposition (only lower triangular is needed for
  // Cholesky)
  for (i = 0; i < order; i++)
    {
      for (j = 0; j <= i; j++)
      ...
      for (k = 0; k < points; k++)
        {
          B[j] += (double)y_raw[k]
                  * M[k]
                     [j]; // y_raw contains the experimental y-values on entry
        }
    }
...
US_lsfit::US_lsfit (double *coeff, double *x, double *y, int ord, int n,
                    bool mesg)
{
  order = ord;
  numpoints = n;
...
void
US_lsfit::calc_coeff_polynomial ()
{
  ...
  A = M'M, and A is positive - definite, and can be decomposed by the Cholesky
  method. evaluate A by filling in the lower triangular of the A matrix. NOTE:
  instead of polynomials evaluated at each point xi, other linearly independent
  basis function sets can be used for a fit.
  ...
  */
  ...
  for (i = 1; i < order; i++)
    ...
    } // only the lower triangular matrix is filled now, which is sufficient
      // for solution by Cholesky decomposition.
...
void
print_vector (double **v, unsigned int rows, const char *c)
{
...
void
conc_dep_s (float sigma, double **right, double **c_current, double ***a2,
            float temp1, unsigned int points)
{
  double *ca;
  unsigned int i;
...
void
conc_dep_s (float sigma, float **right, float **c_current, float ***a2,
            float temp1, unsigned int points)
{
  float *ca;
  unsigned int i;
...
#ifdef ADOLC
void
conc_dep_s (adouble sigma, adouble **right, adouble **c_current, float ***a2,
            adouble temp1, unsigned int points)
{
  adouble *ca;
  unsigned int i;
...
#if defined SECOND_ORDER
      ca[i]
          = temp1 * sigma * pow (c_current[i], 2.0)
...
void
conc_dep_s (adouble sigma, adouble **right, adouble **c_current, double ***a2,
            adouble temp1, unsigned int points)
{
  adouble *ca;
  unsigned int i;
...
void
conc_dep_d (float delta, double **right, double **c_current, double ***u,
            double ***v, double ***w, float temp2, unsigned int points,
...
void
conc_dep_d (float delta, float **right, float **c_current, float ***u,
            float ***v, float ***w, float temp2, unsigned int points,
...
#ifdef ADOLC
void
conc_dep_d (adouble delta, adouble **right, adouble **c_current, float ***u,
            float ***v, float ***w, adouble temp2, unsigned int points,
            adouble **cu, adouble **cv, adouble **cw)
{
  unsigned int i;
  for (i = 0; i < points - 1; i++)
    {
...
void
conc_dep_d (adouble delta, adouble **right, adouble **c_current, double ***u,
            double ***v, double ***w, adouble temp2, unsigned int points,
            adouble **cu, adouble **cv, adouble **cw)
{
  unsigned int i;
  for (i = 0; i < points - 1; i++)
...
/*
  ...
  void calc_A_transpose_A(double ***A, double ***product, unsigned int rows,
  unsigned int columns, bool flag)
  {
  CATA_D_Thread *dt;
...
void
ata_d_thr_t::ata_d_thr_setup (unsigned int a_columns, unsigned int a_c_start,
                              unsigned int a_c_end, double ***a_product,
...
void
ata_d_thr_t::ata_d_thr_shutdown ()
{
...
void
calc_A_transpose_A (double ***A, double ***product, unsigned int rows,
                    unsigned int columns, unsigned int threads)
{
      ...
      for (j = 0; j < threads; j++)
        {
          ...
          //       cout << "thread " << j << " c range " << c_start << " - " <<
          //       c_end << endl;
          ata_d_thr_threads[j]->ata_d_thr_setup (columns, c_start, c_end,
                                                 product, &dataarray);
          c_start = c_end;
        }
  ...
  /*
  // This version completes only the lower triangular matrix - all that's
  needed for the Cholesky decomposition: for (unsigned int i=0; i<columns; i++)
  {
...
/*
  ...
  void calc_A_transpose_A(float ***A, float ***product, unsigned int rows,
  unsigned int columns, bool flag)
  {
  CATA_F_Thread *ft;
  ...
  {
  // This version completes only the lower triangular matrix - all that's
  needed for the Cholesky decomposition: ft[i].end_j = i+1;
  }
  ft[i].start();
  }
  unsigned int count = 0;
...
*/
void
calc_A_transpose_A (float ***A, float ***product, unsigned int rows,
...
double
calc_vector_transpose_times_vector (unsigned int rows, double **vector1,
                                    double **vector2)
{
  double result = 0.0;
...
float
calc_vector_transpose_times_vector (unsigned int rows, float **vector1,
                                    float **vector2)
{
  float result = 0.0;
  for (unsigned int i = 0; i < rows; i++)
...
void
calc_matrix_times_matrix (double ***A, double ***B, double ***product,
...
void
calc_matrix_times_matrix (float ***A, float ***B, float ***product,
                          unsigned int rows, unsigned int columns)
{
  for (unsigned int i = 0; i < rows; i++)
    {
...
void
calc_matrix_times_vector (double ***A, double **B, double **product,
                          unsigned int rows, unsigned int columns)
{
  for (unsigned int i = 0; i < rows; i++)
    {
...
void
calc_matrix_times_vector (float ***A, float **B, float **product,
                          unsigned int rows, unsigned int columns)
{
  for (unsigned int i = 0; i < rows; i++)
    {
