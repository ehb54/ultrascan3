...
#ifdef ADOLC
void m3vm_a(adouble ***matrix, adouble **vector, unsigned int points,
            float constant)
/*
  This function multiplies a tri-diagonal matrix of size [points x points]
  stored in a [points x 3] matrix by a vector of size [points] and returns
...
void m3vm_b(float ***b, adouble ***matrix, adouble **vector,
            unsigned int points, float constant)
/*
  This function multiplies a tri-diagonal matrix of size [points x points]
  stored in a [points x 3] matrix by a vector of size [points] and returns
  the result in vector:
*/
{
  ...
  for (i = 1; i < points - 1; i++) {
    result[i] = 0.0;
    for (j = 0; j < 3; j++) {
      result[i] = result[i] + ((*b)[i][j] + constant * (*matrix)[i][j]) *
                                  (*vector)[i + j - 1];
    }
  }
  result[points - 1] =
...
#endif
void m3vm_b(float ***b, float ***matrix, float **vector, unsigned int points,
            float constant)
/*
  This function multiplies a tri-diagonal matrix of size [points x points]
  stored in a [points x 3] matrix by a vector of size [points] and returns
...
void m3vm_b(double ***b, double ***matrix, double **vector, unsigned int points,
            double constant)
/*
  This function multiplies a tri-diagonal matrix of size [points x points]
  stored in a [points x 3] matrix by a vector of size [points] and returns
...
void mmv(float **result, double **vector, float ***matrix, int row,
         int column) {
  int i, j;
  for (i = 0; i < row; i++) {
    (*result)[i] = 0;
...
void mmv(float **result, double **vector, double ***matrix, int row,
         int column) {
  int i, j;
  for (i = 0; i < row; i++) {
    (*result)[i] = 0;
...
void mmv(float **result, float **vector, double ***matrix, int row,
         int column) {
  int i, j;
  for (i = 0; i < row; i++) {
    (*result)[i] = 0;
...
void ldu(double ***matrix, double **vector, unsigned int rows,
    ...
    for (j = 1; j == jend; j++) {
      ...
      if (flag) {
        ...
        for (k = 1; k == jend; k++) {
          ...
          k2 = jcol + k;
          (*matrix)[jrow - 1][k2 - 1] =
              (*matrix)[jrow - 1][k2 - 1] - fac * (*matrix)[i - 1][k1 - 1];
        }
      }
    }
    (*vector)[jrow - 1] =
        (*vector)[jrow - 1] - (*matrix)[jrow - 1][jcol - 1] * (*vector)[i - 1];
  }
  ...
  for (i = 1; i < rows; i++) {
    ...
    for (j = jbgn - 1; j < jend; j++) {
      jrow = jrow + 1;
      (*vector)[irow - 1] =
          (*vector)[irow - 1] - (*matrix)[irow - 1][j] * (*vector)[jrow - 1];
    }
    (*vector)[irow - 1] = (*vector)[irow - 1] / (*matrix)[irow - 1][idiag - 1];
  }
}

void ldu(float ***matrix, float **vector, unsigned int rows,
         unsigned int columns, bool flag) {
...
#ifdef ADOLC
void ldu(adouble ***matrix, adouble **vector, unsigned int rows,
         unsigned int columns, bool flag) {
  unsigned int i, j, k, jbgn, jend, jrow = 0, jcol = 0, k1, k2, irow, ibd2,
                                    idiag;
...
void ldu(float ***matrix, adouble **vector, unsigned int rows,
         unsigned int columns, bool flag) {
  unsigned int i, j, k, jbgn, jend, jrow = 0, jcol = 0, k1, k2, irow, ibd2,
                                    idiag;
...
//
// A*A(-1)=I, A=LL', L(L'*A(-1)) = I, L'*A(-1) = y, L * y = I (solve for y), now
// L' * A(-1) = y (solve for A(-1))
//
...
*/
void generalLeastSquares(float **M, unsigned int points, unsigned int order,
  ...
  for (j = 0; j < order; j++) {
    ...
    for (k = 0; k < points; k++) {
      B[j] += (double)y_raw[k] *
              M[k][j];  // y_raw contains the experimental y-values on entry
    }
  }
...
void conc_dep_s(float sigma, double **right, double **c_current, double ***a2,
...
void conc_dep_s(float sigma, float **right, float **c_current, float ***a2,
                float temp1, unsigned int points) {
  float *ca;
  unsigned int i;
  ca = new float[points];
...
void conc_dep_d(adouble delta, adouble **right, adouble **c_current,
                double ***u, double ***v, double ***w, adouble temp2,
                unsigned int points, adouble **cu, adouble **cv, adouble **cw) {
  unsigned int i;
  for (i = 0; i < points - 1; i++) {
    (*cu)[i] = delta * (*c_current)[i] * (*c_current)[i + 1];
    (*cv)[i] = delta * pow((*c_current)[i], 2.0);
...
void ata_d_thr_t::ata_d_thr_setup(unsigned int a_columns,
                                  unsigned int a_c_start, unsigned int a_c_end,
                                  double ***a_product,
...
void calc_A_transpose_A(double ***A, double ***product, unsigned int rows,
  ...
  if (threads > 1) {
    ...
    for (j = 0; j < threads; j++) {
      ...
      //       c_end << endl;
      ata_d_thr_threads[j]->ata_d_thr_setup(columns, c_start, c_end, product,
                                            &dataarray);
      c_start = c_end;
    }
  ...
  /*
  // This version completes only the lower triangular matrix - all that's needed
  for the Cholesky decomposition: for (unsigned int i=0; i<columns; i++)
  {
...
/*
  ...
  {
  // This version completes only the lower triangular matrix - all that's needed
  for the Cholesky decomposition: ft[i].end_j = i+1;
  }
  ft[i].start();
  }
  unsigned int count = 0;
...
double calc_vector_transpose_times_vector(unsigned int rows, double **vector1,
                                          double **vector2) {
  double result = 0.0;
...
float calc_vector_transpose_times_vector(unsigned int rows, float **vector1,
                                         float **vector2) {
  float result = 0.0;
  for (unsigned int i = 0; i < rows; i++) {
    result += (*vector1)[i] * (*vector2)[i];
...
void calc_matrix_times_matrix(double ***A, double ***B, double ***product,
...
void calc_matrix_times_matrix(float ***A, float ***B, float ***product,
                              unsigned int rows, unsigned int columns) {
  for (unsigned int i = 0; i < rows; i++) {
    for (unsigned int j = 0; j < columns; j++) {
      (*product)[i][j] = 0.0;
...
void calc_matrix_times_vector(double ***A, double **B, double **product,
                              unsigned int rows, unsigned int columns) {
  for (unsigned int i = 0; i < rows; i++) {
    (*product)[i] = 0.0;
    for (unsigned int j = 0; j < columns; j++) {
...
void calc_matrix_times_vector(float ***A, float **B, float **product,
                              unsigned int rows, unsigned int columns) {
  for (unsigned int i = 0; i < rows; i++) {
    (*product)[i] = 0.0;
    for (unsigned int j = 0; j < columns; j++) {
