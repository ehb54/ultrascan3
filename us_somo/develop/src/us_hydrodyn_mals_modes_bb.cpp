...
void US_Hydrodyn_Mals::baseline_start( bool from_blanks_mode_save )
{
...

   {
      bool use_i_power = 
         ...
         ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_bl_i_power" ].toDouble() != 1e0;

      double i_power = 1e0;

      if ( use_i_power ) {
         i_power = ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_bl_i_power" ].toDouble();
...
{
   int smoothing = 0;
   bool integral = ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_bl_integral" ] == "true";
   if ( integral )
   {
      smoothing = ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_bl_smooth" ].toInt();
   }
   // bool save_bl = ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_bl_save" ] == "true";
   bool save_bl = false;
   unsigned int reps = 0;
   if ( integral )
   {
...
void US_Hydrodyn_Mals::baseline_apply( QStringList files, 
                                            bool integral, 
                                            int smoothing, 
                                            bool save_bl, 
                                            unsigned int reps,
                                            ...
                                            bool quiet)
{
   ...
   bool test_alt_integral_bl = ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_bl_save" ] == "true";
      ...
      if ( integral ) {
         {
            // recompute start_y
            if ( use_start_region ) {
               int start_region_frames = ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_bl_start_region" ].toInt();
         ...
         double alpha_epsilon = 
            ...
            ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_bl_epsilon" ].toDouble() : 5e-3;
      ...
      if ( ca.sliding( 
         ...
         // plot of results
         {
            ...
            if ( best_vdparameters[ "x" ].size() ) {
               US_Hydrodyn_Mals_Baseline_Best * uhsbb = new US_Hydrodyn_Mals_Baseline_Best( us_hydrodyn,
                                                                                                      best_parameters,
                                                                                                      best_dparameters,
                                                                                                      best_vdparameters
                                                                                                      );
...
void US_Hydrodyn_Mals::baseline_test_pos( int pos ) {
   ...
   // us_qdebug( QString( "smooth %1" ).arg( ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_bl_smooth" ].toInt() ) );

   baseline_apply( qsl
                   ,true
                   ,( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_bl_smooth" ].toInt()
                   ,true
                   ,( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_bl_reps" ].toInt()
                   ,false
                   ,true );
