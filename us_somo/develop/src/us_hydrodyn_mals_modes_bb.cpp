...
void US_Hydrodyn_Mals::blanks_start() {
    ...
    if (blanks_save_selected.size() > 1) {
      ...
      if (!files_are_time && files_compatible) {
        switch (QMessageBox::question(
            ...
            us_tr("There are available blanks data and selected files that "
                  "could be blanks\n"
                  "Which would you like to analyze?"),
            us_tr(us_tr("&Available blanks data")),
            us_tr(us_tr("Currently &selected files")), us_tr("&Cancel"), 0  //
            ,
            -1  //
            )) {
          case -1:  // escape
  ...
  if (use_default_blanks) {
    ...
    if (blanks_in_baseline_mode) {
      ...
      // ),
      //                       us_tr( "You would now be asked to load files, but
      //                       this is not yet implemented\n"
      //                           "You *can* exit and load blanks, run blanks
      //                           analysis & 'keep' and then try again\n" )
...

  {
    ...
    if (any_differences) {
      editor_msg("red", us_tr("Blanks analysis: curves must be on the same "
...

  {
...

    {
      ...
      for (unsigned int i = 0; i < (unsigned int)files.size(); ++i) {
        ...
        if (!f_qs.count(*qs)) {
          editor_msg("red", QString(us_tr("Internal error: request to use %1, "
...
void US_Hydrodyn_Mals::blanks_enables() {
...
// --- baseline ---
void US_Hydrodyn_Mals::baseline_start(bool from_blanks_mode_save) {
  if (axis_y_log) {
    axis_y();
  }
    ...
    if (use_i_power) {
      ...
      switch (QMessageBox::warning(
          ...
          QString(
              us_tr(
                  "Please note:\n\n"
                  "You are using a non-unity intensity exponent of %1 for "
                  "integral baseline correction.\n"
                  "The model of intensity proportional deposits is based on a "
                  "unity intensity exponent.\n"
                  "It is possible that a specific capillary fouling mechanism "
                  "may be correctly approximated with a non-unity exponent.\n"
                  "We urge you to use a non-unity intensity exponent with "
                  "care.\n"
    ...
    if (reps < 5) {
      switch (QMessageBox::warning(
          ...
          QString(us_tr("Please note:\n\n"
                        "You have selected the integral baseline method with "
                        "maximum iterations of %1\n"
                        "It is recommended that a minimum of 5 iterations be "
                        "used to approach convergence\n"
                        "What would you like to do?\n"))
              .arg(reps),
          us_tr("&Continue"), us_tr("&Quit"))) {
        case 0:  // continue
  ...
  if (f_qs[wheel_file].size() < IB_MIN_RANGE * 2) {
    QMessageBox::warning(
        this, windowTitle() + us_tr(" : Baseline"),
        QString(us_tr("You must have a minimum of %1 frames in your data to "
  ...
  if (baseline_integral) {
    ...
    if (q_size < IB_MIN_FRAMES) {
      QMessageBox::warning(
          this, windowTitle() + us_tr(" : Baseline"),
          us_tr(QString(
                    "Insufficient frames (%1) for integral baseline analysis.\n"
    ...
    if (!from_blanks_mode_save || !blanks_brookesmap_sliding_results.size()) {
      switch (QMessageBox::question(
          this, windowTitle() + us_tr(" : Utilize blanks in baseline analysis"),
          us_tr(
              "Do you wish to utilize reference blanks for baseline analysis?\n"
              "Without reference blanks, only the adjusted Holm-Bonferroni "
              "method can be used."),
          us_tr("&Yes, utilize blanks"), us_tr("&No, proceed without blanks"),
          QString())) {
        case -1:  // escape
        ...
        case 0: {
          switch (QMessageBox::question(
              this, windowTitle() + us_tr(" : Baseline mode blanks selection"),
              QString(us_tr(
                  "You must have blanks processed to proceed\n"
                  "\n"
                  "The blanks should be non-averaged buffer frames (whose "
                  "average value was then subtracted from the elution frames)\n"
                  "If you do not have the buffers corresponding to their "
                  "present elution, then the preloaded blanks should have been "
                  "chosen in a similar way:\n"
                  "The blanks do not have to be *the* blanks used for the "
                  "present elution, even if it is strongly recommended,\n"
                  "but at least blanks collected on the same beamline with a "
                  "similar set-up (distance, pixel size, energy).\n"
                  "\n"
            ...
            case 0:  // load blanks
            {
              ...
              new_blanks_files = QFileDialog::getOpenFileNames(
                  this, windowTitle() + us_tr(" : Select blanks files"),
                  use_dir,
              ...
              for (int i = 0; i < (int)new_blanks_files.size(); ++i) {
                QString basename =
                    QFileInfo(new_blanks_files[i]).completeBaseName();
                new_blanks << basename;
            ...
            case 1:  // load previous analysis
            {
              ...
              if (!load_parameters.count("isblanks")) {
                QMessageBox::warning(
                    this, windowTitle() + us_tr(" : Baseline"),
                    QString(us_tr("The loaded analysis does not appear to be a "
                                  "blanks analysis")),
                    QMessageBox::Ok | QMessageBox::Default,
                    QMessageBox::NoButton);
                return current_mode != MODE_NORMAL ? wheel_cancel()
              ...
              if (blanks_cormap_parameters.size() &&
                ...
                if (ca.sliding(
                        blanks_cormap_pvaluepairs, blanks_cormap_parameters,
                        blanks_brookesmap_sliding_results, this, progress)) {
    ...
    if (show_determination_info) {
      if (baseline_max_window_size < IB_MIN_FRAMES) {
        ...
        QMessageBox::information(
            this, windowTitle() + us_tr(" : Baseline"),
            us_tr("The first step of the integral baseline procedure is to "
                  "determine a constant final baseline."),
            QMessageBox::Ok | QMessageBox::Default, QMessageBox::NoButton);
      }
    }
  ...
  if (baseline_integral) {
    ...
    if (le_baseline_start_s->text().isEmpty() ||
      ...
      disconnect(le_baseline_start_s, SIGNAL(textChanged(const QString &)), 0,
                 0);
    ...
    if (le_baseline_start_e->text().isEmpty() ||
      ...
      disconnect(le_baseline_start_e, SIGNAL(textChanged(const QString &)), 0,
                 0);
      // le_baseline_start_e->setText( QString( "%1" ).arg( f_qs[ wheel_file ][
      // 0 ] + 2e0 * q_len_delta) );
      le_baseline_start_e->setText(
          QString("%1").arg(f_qs[wheel_file][0 + 2 * pos_len_delta < q_size - 1
                                                 ? 0 + 2 * pos_len_delta
                                                 : 0]));
    ...
    if (le_baseline_end->text().isEmpty() || le_baseline_end->text() == "0" ||
        le_baseline_end->text().toDouble() > f_qs[wheel_file].back()) {
      disconnect(le_baseline_end, SIGNAL(textChanged(const QString &)), 0, 0);
      // le_baseline_end->setText( QString( "%1" ).arg( f_qs[ wheel_file
      // ].back() - q_len_delta ) );
      le_baseline_end->setText(
          QString("%1").arg(f_qs[wheel_file][q_size - 1 - pos_len_delta > 0
                                                 ? q_size - 1 - pos_len_delta
                                                 : 0]));
      connect(le_baseline_end, SIGNAL(textChanged(const QString &)),
              SLOT(baseline_end_text(const QString &)));
    }
...
void US_Hydrodyn_Mals::baseline_enables() {
  ...
  if (baseline_integral) {
    ...
    pb_baseline_apply->setEnabled(baseline_best_ok &&
  ...
  wheel_enables(
      ...
      le_last_focus == le_baseline_end_e || le_last_focus == le_baseline_width
#else
...
void US_Hydrodyn_Mals::set_baseline_start_zero() {
  if (!cb_baseline_start_zero->isChecked()) {
    // QMessageBox::warning(
    //                      this,
    //                      this->windowTitle() + us_tr(": Baseline setup:
    //                      nonzero starting offset" ), us_tr(
    //                         "Having a non zero starting offset likely means
    //                         problems with buffer subtraction which should be
    //                         corrected before *any* analysis is done.\n"
    //                         ),
    //                      QMessageBox::Ok | QMessageBox::Default,
    //                      QMessageBox::NoButton
    //                      );
...
void US_Hydrodyn_Mals::baseline_init_markers() {
  ...
  if (current_mode == MODE_BLANKS) {
    ...
    if (baseline_integral) {
      ...
      if (!cb_baseline_start_zero->isChecked()) {
        gauss_add_marker(le_baseline_start_s->text().toDouble(), Qt::magenta,
                         us_tr("\nLFS  \nStart"));
        gauss_add_marker(le_baseline_start->text().toDouble(), Qt::red,
                         us_tr("Start"));
        gauss_add_marker(le_baseline_start_e->text().toDouble(), Qt::magenta,
                         us_tr("\n\n\nLFS\nEnd"));
        gauss_add_marker(le_baseline_end_s->text().toDouble(), Qt::magenta,
...
void US_Hydrodyn_Mals::replot_baseline_integral() {
...
void US_Hydrodyn_Mals::replot_baseline(QString /* qs */) {
  ...
  if (set_start && set_end) {
    // linear fit on each
    usu->linear_fit(start_q, start_I, start_intercept, start_slope, siga, sigb,
                    chi2);
    ...
    if (set_start) {
      ...
      if (f_qs[wheel_file][after_end] != f_qs[wheel_file][before_end]) {
        end_t = (f_qs[wheel_file][after_end] - end) /
                (f_qs[wheel_file][after_end] - f_qs[wheel_file][before_end]);
      } else {
        end_t = 0.5e0;
      }
      ...
      if (set_end) {
        ...
        if (f_qs[wheel_file][after_start] != f_qs[wheel_file][before_start]) {
          start_t =
              ...
              (f_qs[wheel_file][after_start] - f_qs[wheel_file][before_start]);
        } else {
          start_t = 0.5e0;
        }
  ...
  // the baseline
  {
    ...
    // ).arg( baseline_intercept ); printvector( "baseline x", x ); printvector(
    // "baseline y", y );
...
void US_Hydrodyn_Mals::baseline_apply() {
  int smoothing = 0;
  bool integral =
      ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_bl_integral"] == "true";
  if (integral) {
    smoothing = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_bl_smooth"].toInt();
  }
...
void US_Hydrodyn_Mals::baseline_apply(QStringList files, bool integral,
  ...
  if (integral) {
    check_zi_window(
        files,
        us_tr(
            "You may have already noticed this message when making I(t).\n\n"));
  }
  ...
  //                                               integral baseline method with
  //                                               maximum iterations of %1\n"
  //                                               "It is recommended that a
  //                                               minimum of 5 iterations be
  //                                               used to approach
  //                                               convergence\n" "What would
  //                                               you like to do?\n" ) )
  //                                  .arg( reps ),
  ...
  bool use_start_region =
      ...
      ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_bl_start_region"].toInt() > 0;
  ...
  if (use_i_power) {
    i_power =
        ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_bl_i_power"].toDouble();
    editor_msg("darkred", QString(us_tr("Using power of intensity (%1) for "
    ...
    //                                  note:\n\n"
    //                                               "You are using a non-unity
    //                                               intensity exponent of %1
    //                                               for integral baseline
    //                                               correction.\n" "The model
    //                                               of intesity proportional
    //                                               deposits is based on a
    //                                               unity intensity
    //                                               exponent.\n" "It is
    //                                               possible that a specific
    //                                               capillary fouling mechanism
    //                                               may be approximated
    //                                               corrected with a non-unity
    //                                               exponent.\n" "We urge you
    //                                               to use a non-unity
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size(); i++) {
    ...
    if (use_errors) {
      for (unsigned int j = 1; j < f_qs[files[i]].size(); j++) {
        if (!cb_baseline_start_zero->isChecked() &&
            f_qs[files[i]][j] >= start_s && f_qs[files[i]][j] <= start_e) {
          start_q.push_back(f_qs[files[i]][j]);
          start_I.push_back(f_Is[files[i]][j]);
        }
        if (f_qs[files[i]][j] >= end_s && f_qs[files[i]][j] <= end_e) {
      ...
      if (set_start) {
        ...
        if (f_qs[files[i]][after_end] != f_qs[files[i]][before_end]) {
          end_t = (f_qs[files[i]][after_end] - end) /
                  (f_qs[files[i]][after_end] - f_qs[files[i]][before_end]);
        } else {
          end_t = 0.5e0;
        }
        ...
        if (set_end) {
          ...
          if (f_qs[files[i]][after_start] != f_qs[files[i]][before_start]) {
            start_t =
                ...
                (f_qs[files[i]][after_start] - f_qs[files[i]][before_start]);
          } else {
            start_t = 0.5e0;
          }
    ...
    if (integral) {
      {
        ...
        if (use_start_region) {
          ...
          if (use_errors) {
            for (int j = 0;
                 j < (int)f_qs[files[i]].size() && j < start_region_frames;
                 ++j) {
              start_I.push_back(f_Is[files[i]][j]);
          ...
          editor_msg(
              "darkgreen",
              QString(
                  "Using start region size %1 for baseline with avg value %2")
      ...
      if (smoothing) {
        ...
        if (!usu.smooth(f_Is[files[i]], bl_I, smoothing)) {
          bl_I = f_Is[files[i]];
          messages
              << QString(us_tr("Error: smoothing error on %1")).arg(files[i]);
      ...
      if (delta_bl < 0e0) {
        // messages << QString( us_tr( "Notice: the overall change in baseline
        // of %1 is negative (%2), so a constant baseline correction will be
        // applied" ) ).arg( files[ i ] ).arg( delta_bl );
        messages << QString(us_tr("Notice: the overall change in baseline of "
                                  "%1 is negative (%2), so no baseline "
        ...
        do {
          ...
          if (test_alt_integral_bl) {
            for (unsigned int j = 0; j < (unsigned int)bl_I.size(); j++) {
              if (f_qs[files[i]][j] >= start && f_qs[files[i]][j] <= end) {
                if (bl_I[j] > bl[j]) {
            ...
            if (I_tot > 0e0 && delta_bl > 0e0) {
              ...
              editor_msg(
                  "dark blue",
                  QString(
                      us_tr("iteration %1 delta_Bl %2 Itot %3 gamma %4 %5%6"))
                      .arg(this_reps)
                      .arg(delta_bl)
                      .arg(I_tot)
                      .arg(alpha)
                      .arg(smoothing ? QString(" smoothing %1").arg(smoothing)
                                     : QString(""))
                      .arg(i_power != 1e0
                               ? QString(" I exponent %1").arg(i_power)
                               : QString("")));
              ...
              if (I_tot < 0e0 && delta_bl < 0e0) {
                messages << QString(
                                us_tr("Warning: the overall change in baseline "
                                      "of %1 is negative (%2) and the integral "
                                      "is less than zero, so no baseline "
                                      "correction will be applied"))
                                .arg(files[i])
                                .arg(delta_bl);
                editor_msg("dark red", messages.back());
                ...
                if (I_tot <= 0e0 && delta_bl > 0e0) {
                  messages << QString(
                                  us_tr("Warning: the integral of %1 was less "
                                        "than or equal to zero, so no baseline "
                                        "correction will be applied"))
                                  .arg(files[i]);
                  editor_msg("dark red", messages.back());
                  new_I = f_Is[files[i]];
          ...
          if (save_bl) {
            QString this_file =
                QString("BI_%1-%2").arg(files[i]).arg(this_reps);
            baseline_apply_created.push_back(this_file);
      ...
      bl_name += QString("-bl%1-%2s")
                     ...
                     .replace(".", "_");
      while (current_files.count(bl_name)) {
        bl_name = files[i] + QString("-bl%1-%2s-%3")
                                 .arg(baseline_slope, 0, 'g', 8)
                                 .arg(baseline_intercept, 0, 'g', 8)
                                 .arg(++ext)
                                 .replace(".", "_");
  ...
  if (messages.size() && !quiet) {
    ...
    QMessageBox::warning(this, this->windowTitle() + us_tr(": Baseline apply"),
                         QString(us_tr("Please note:\n\n"
                                       "These warnings were produced when "
...
void US_Hydrodyn_Mals::set_baseline_fix_width() {
  // us_qdebug( "set baseline fix width" );
  baseline_enables();
}

int US_Hydrodyn_Mals::input_double_to_pos(double d) {
...
void US_Hydrodyn_Mals::baseline_start_s_text(const QString &text) {
#if QT_VERSION < 0x040000
...
void US_Hydrodyn_Mals::baseline_start_text(const QString &text) {
#if QT_VERSION < 0x040000
...
void US_Hydrodyn_Mals::baseline_start_e_text(const QString &text) {
#if QT_VERSION < 0x040000
...
void US_Hydrodyn_Mals::baseline_end_s_text(const QString &text) {
  ...
  if (current_mode != MODE_BLANKS && baseline_integral) {
    ...
    if (cb_baseline_fix_width->isChecked()) {
      ...
      connect(le_baseline_end_e, SIGNAL(textChanged(const QString &)),
              SLOT(baseline_end_e_text(const QString &)));
#if QT_VERSION < 0x040000
      plot_dist->setMarkerPos(plotted_markers[3], f_qs[wheel_file][new_pos_e],
                              0e0);
#else
      plotted_markers[3]->setXValue(f_qs[wheel_file][new_pos_e]);
#endif
      if (new_pos_s != pos_s) {
        // disconnect( le_baseline_end_s, SIGNAL( textChanged( const QString & )
        // ), 0, 0 ); le_baseline_end_s->setText( QString( "%1" ).arg( f_qs[
        // wheel_file ][ new_pos_s ] ) ); connect( le_baseline_end_s, SIGNAL(
        // textChanged( const QString & ) ), SLOT( baseline_end_s_text( const
        // QString & ) ) );
      ...
      if (pos_e - IB_MIN_RANGE + 1 < pos_s) {
        ...
        connect(le_baseline_end_e, SIGNAL(textChanged(const QString &)),
                SLOT(baseline_end_e_text(const QString &)));
#if QT_VERSION < 0x040000
        plot_dist->setMarkerPos(plotted_markers[3], f_qs[wheel_file][pos_e],
                                0e0);
#else
...
void US_Hydrodyn_Mals::baseline_end_text(const QString &text) {
...
void US_Hydrodyn_Mals::baseline_end_e_text(const QString &text) {
  ...
  if (current_mode != MODE_BLANKS && baseline_integral) {
    ...
    if (cb_baseline_fix_width->isChecked()) {
      ...
      connect(le_baseline_end_s, SIGNAL(textChanged(const QString &)),
              SLOT(baseline_end_s_text(const QString &)));
#if QT_VERSION < 0x040000
      plot_dist->setMarkerPos(plotted_markers[0], f_qs[wheel_file][new_pos_s],
                              0e0);
      plot_dist->setMarkerPos(plotted_markers[1], f_qs[wheel_file][new_pos_s],
                              0e0);
#else
...
#endif
      if (new_pos_e != pos_e) {
        // disconnect( le_baseline_end_e, SIGNAL( textChanged( const QString & )
        // ), 0, 0 ); le_baseline_end_e->setText( QString( "%1" ).arg( f_qs[
        // wheel_file ][ new_pos_e ] ) ); connect( le_baseline_end_e, SIGNAL(
        // textChanged( const QString & ) ), SLOT( baseline_end_s_text( const
        // QString & ) ) );
      ...
      if (pos_s > pos_e - IB_MIN_RANGE + 1) {
        ...
        connect(le_baseline_end_s, SIGNAL(textChanged(const QString &)),
                SLOT(baseline_end_s_text(const QString &)));
#if QT_VERSION < 0x040000
        plot_dist->setMarkerPos(plotted_markers[0], f_qs[wheel_file][pos_s],
                                0e0);
        plot_dist->setMarkerPos(plotted_markers[1], f_qs[wheel_file][pos_s],
                                0e0);
#else
...
void US_Hydrodyn_Mals::baseline_start_s_focus(bool hasFocus) {
  ...
  if (hasFocus) {
    ...
    qwtw_wheel->setValue(
        (double)input_double_to_pos(le_baseline_start_s->text().toDouble()));
    wheel_enables();
  }
}

void US_Hydrodyn_Mals::baseline_start_focus(bool hasFocus) {
  ...
  if (hasFocus) {
    ...
    qwtw_wheel->setValue(
        (double)input_double_to_pos(le_baseline_start->text().toDouble()));
    wheel_enables();
  }
}

void US_Hydrodyn_Mals::baseline_start_e_focus(bool hasFocus) {
  ...
  if (hasFocus) {
    ...
    qwtw_wheel->setValue(
        (double)input_double_to_pos(le_baseline_start_e->text().toDouble()));
    wheel_enables();
  }
}

void US_Hydrodyn_Mals::baseline_end_s_focus(bool hasFocus) {
  ...
  if (hasFocus) {
    ...
    qwtw_wheel->setValue(
        (double)input_double_to_pos(le_baseline_end_s->text().toDouble()));
    wheel_enables();
  }
}

void US_Hydrodyn_Mals::baseline_end_focus(bool hasFocus) {
...
void US_Hydrodyn_Mals::baseline_end_e_focus(bool hasFocus) {
  ...
  if (hasFocus) {
    ...
    qwtw_wheel->setValue(
        (double)input_double_to_pos(le_baseline_end_e->text().toDouble()));
    wheel_enables();
  }
}

void US_Hydrodyn_Mals::baseline_width_text(const QString &text) {
  int blw = text.toInt();
...
void US_Hydrodyn_Mals::baseline_width_focus(bool hasFocus) {
  if (hasFocus) {
...
void US_Hydrodyn_Mals::baseline_best() {
  disable_all();
  ...
  if (baseline_cormap_parameters.size() && baseline_cormap_pvaluepairs.size()) {
    US_Hydrodyn_Saxs_Cormap_Cluster_Analysis ca;
    ...
    baseline_cormap_parameters["sliding_baseline_pos_base"] =
        QString("%1").arg(input_double_to_pos(baseline_last_cormap_run_end_s));
      ...
      // plot of results
      {
        ...
        if (!hb_mode) {
          ...
          if (blanks_brookesmap_sliding_results.count(tag)) {
            ...
            QString tagsd =
                QString("%1 sd:Red cluster size average").arg(width);
        ...
        best_parameters["title"] =
            QString(
                us_tr(hb_mode
                          ? "Baseline avg. I(q) and red pair % by start frame "
                            "(window width %1 frames)."
                          : "Baseline avg. I(q) and average red cluster size "
                            "by start frame\n(window width %1 frames)."))
                .arg(baseline_best_last_window_size);

        if (!hb_mode) {
          best_parameters["hb_title"] =
              QString(us_tr("Baseline avg. I(q) and red pair % by start "
                            "frame\n(window width %1 frames)."))
                  .arg(baseline_best_last_window_size);
        }
        ...
        if (baseline_cormap_parameters.count("decimate")) {
          ...
          if (decimate > 1) {
            ...
            best_parameters["title"] +=
                QString(us_tr(" Only every %1%2 q value selected."))
                    .arg(decimate)
                    .arg(nth_tag);
            best_parameters["hb_title"] +=
                QString(us_tr(" Only every %1%2 q value selected."))
...

        {
          ...
          if (!hb_mode) {
            ...
            // us_qdebug( QString( "ybest %1 pos %2" ).arg( hb_ybest ).arg(
            // hb_ybestposx ) ); if ( hb_ybestposx != -1 ) {
        ...
        if (true) {  // new way
          ...
          if (use_errors) {
            for (int i = 0; i < (int)baseline_selected.size(); ++i) {
              if (rx_capqv.indexIn(baseline_selected[i]) == -1) {
                editor_msg("red",
                           QString(us_tr("Baseline Find best region: Could not "
            ...
            // now msumq[ j ] and msumqmaxq[ j ] contains sd weighted xbar for
            // each frame
          } else {
            for (int i = 0; i < (int)baseline_selected.size(); ++i) {
              if (rx_capqv.indexIn(baseline_selected[i]) == -1) {
              ...
              for (int j = 0; j < (int)f_qs[baseline_selected[i]].size(); ++j) {
                ...
                if (qv <= cormap_maxq) {
                  ...
                  msum2qmaxq[j] += f_Is[baseline_selected[i]][j] *
                                   f_Is[baseline_selected[i]][j];
                  ptsmaxq[j]++;
                }
              }
          ...
          for (map<QString, double>::iterator it =
            ...
            if (rx_cap.indexIn(it->first) != -1) {
              ...
              if (baseline_best_last_window_size > 1) {
                double countm1inv =
                    1e0 / ((double)baseline_best_last_window_size - 1);
                msumqysd[pos] =
                    sqrt(countm1inv *
                         (msum2qy[pos] - countinv * msumqy[pos] * msumqy[pos]));
                msumqmaxqysd[pos] =
                    sqrt(countm1inv *
                         (msum2qmaxqy[pos] -
                          countinv * msumqmaxqy[pos] * msumqmaxqy[pos]));
              }
              msumqy[pos] *= countinv;
              ...
              if (minsumqmaxqy > msumqmaxqy[pos]) {
                ...
                minsumqmaxqysd =
                    msumqmaxqysd.count(pos) ? msumqmaxqysd[pos] : 0e0;
              }
            }
          }
          ...
          for (map<int, double>::iterator it = msumqx.begin();
            ...
            sdmaxq =
                msumqmaxqysd.count(it->first) ? msumqmaxqysd[it->first] : 0e0;
          ...
          if (blanks_cormap_parameters.count("blanks_avg_maxq_sd")) {
            best_dparameters["blanks_avg_maxq_sd"] =
                blanks_cormap_parameters["blanks_avg_maxq_sd"].toDouble();
          }
          ...
          //    it->first ] ).arg( it->second ); if ( msumqmaxq.count( it->first
          //    ) ) {
          ...
          //    cout << endl;
          // }
          // US_Vector::printvector3( "sumqx, sumqy, sumqmaxqy", sumqx, sumqy,
          // sumqmaxqy );
        }
              ...
              if (best_dparameters["minsumqmaxqy"] >
                ...
                if (best_dparameters["minsumqmaxqy"] >
                  ...
                  if (best_dparameters["minsumqmaxqy"] >
                    ...
                    tmp_msg += us_tr(
                        "Integral baseline correction is of borderline "
                        "need\n\n");
                  } else {
                    tmp_msg += us_tr(
                        "Integral baseline correction is of questionable "
                        "use\n\n");
                  }
                }
              } else {
                if (best_dparameters["minsumqmaxqy"] <
                    -5 * best_dparameters["minsumqmaxqysd"]) {
                  tmp_msg += us_tr(
                                 "It appears there is an issue with buffer "
                                 "subtraction resulting in negative data\n") +
                             us_tr(
                                 "Integral baseline correction is not "
                                 "appropriate for this situation\n") +
                             us_tr(
                                 "Perhaps a linear baseline correction would "
                                 "help, but we recommend you first verify the "
                                 "buffer subtraction\n\n");
                } else {
                  tmp_msg += us_tr(
                      "Integral baseline correction is not recommended\n\n");
                }
              }
            } else {
              ...
              if (best_dparameters["minsumqmaxqy"] > 0) {
                tmp_msg +=
                    best_vdparameters.count("yx")
                        ? us_tr(
                              "Integral baseline correction would be possible, "
                              "but no steady state end region has been "
                              "identified\n\n")
        ...
        best_parameters["hb_msg"] += best_parameters["msg"] +
        ...
        if (best_vdparameters.count("bestpos")) {
          if (best_vdparameters["bestpos"].size() > 1) {
            best_parameters["msg"] +=
                QString(hb_mode ? "  Best start positions based on red pair %  "
                                  "           %1"
                                : "  Best start positions based on avg. red "
                                  "cluster size  %1")
                    .arg(best_vdparameters["bestpos"].front());
            for (int i = 1; i < (int)best_vdparameters["bestpos"].size(); ++i) {
              best_parameters["msg"] +=
                  QString(",%1").arg(best_vdparameters["bestpos"][i]);
            }
            best_parameters["msg"] += "\n";
          } else {
            best_parameters["msg"] +=
                QString(best_vdparameters.count("yx")
                            ? "~~_darkyellow2_~~  Best start positions based "
                              "on avg. red cluster size  %1~~_darkblue_~~"
                            : (hb_mode ? "  Best start position based on red "
                                         "pair %              %1\n"
                                       : "  Best start position based on avg. "
                                         "red cluster size   %1\n"))
                    .arg(best_vdparameters["bestpos"].front());
          }
        }
        ...
        if (best_vdparameters.count("hb_bestpos")) {
          if (best_vdparameters["hb_bestpos"].size() > 1) {
            ...
            for (int i = 1; i < (int)best_vdparameters["hb_bestpos"].size();
                 ++i) {
              best_parameters["hb_msg"] +=
                  QString(",%1").arg(best_vdparameters["hb_bestpos"][i]);
            }
            best_parameters["hb_msg"] += "\n";
          } else {
            best_parameters["hb_msg"] +=
        ...
        {  // common for both msg & hb_msg
          ...
          if (best_vdparameters.count("bestsumqmaxqypos")) {
            if (best_vdparameters["bestqmaxqypos"].size() > 1) {
              tmp_msg +=
                  QString(
                      "~~_darkorange3_~~  Lowest average intensity positions   "
                      "                %1~~_darkblue_~~")
                      .arg(best_vdparameters["bestsumqmaxqypos"].front());
              for (int i = 1;
                   i < (int)best_vdparameters["bestsumqmaxqypos"].size(); ++i) {
              ...
              tmp_msg +=
                  QString(
                      "~~_darkorange3_~~  Lowest average intensity position    "
                      "                %1~~_darkblue_~~")
                      .arg(best_vdparameters["bestsumqmaxqypos"].front());
            }
          }
          ...
          best_parameters["msg"] +=
              QString(
                  ...
                  .arg(blanks_cormap_parameters.count("frames")
                           ...
                           : QString(us_tr("unknown")))
                  .arg(baseline_best_last_window_size)
                  .arg(blanks_brookesmap_sliding_results.count(
                           QString("%1 count")
                               .arg(baseline_best_last_window_size))
                           ? QString("%1").arg(
        ...
        if (best_dparameters.count("blanksaverage")) {
          best_parameters["msg"] +=
              QString(
                  "  Used window red cluster average size                 %1\n")
                  .arg(best_dparameters["blanksaverage"]);
          if (best_dparameters.count("blanksaveragesd")) {
            best_parameters["msg"] +=
                QString(
                    "  Used window red cluster average size sd              "
                    "%1\n")
        ...
        if (best_vdparameters["x"].size()) {
          US_Hydrodyn_Mals_Baseline_Best *uhsbb =
              new US_Hydrodyn_Mals_Baseline_Best(us_hydrodyn, best_parameters,
                                                 best_dparameters,
                                                 best_vdparameters);
          uhsbb->show();
...
void US_Hydrodyn_Mals::baseline_test() {
...
void US_Hydrodyn_Mals::baseline_test_pos(int pos) {
