...
{
double
compute_f (double x, const double *par)
{
  return par[0] + par[1] * exp (x * par[2]);
}
}

US_Hydrodyn_Best::US_Hydrodyn_Best (void *us_hydrodyn, bool *best_widget,
    ...
    : QFrame (p)
{
  ...
  *best_widget = true;
  ...
  tau_inputs << "EIGENVALUES OF Drr TENSOR (1/s) [1]"
...
US_Hydrodyn_Best::setupGUI ()
{
  ...
  lbl_credits_1
      = new QLabel ("Cite: Aragon, S.R. \"A precise boundary element method "
                    "for macromolecular transport properties\", J. Comp. "
                    "Chem, (2004) 25, 1191-1205",
                    this);
  ...
  lbl_credits_2 = new QLabel ("Cite: Connolly, M.L. \"The molecular surface "
                              "package\", J. Mol. Graph. (1993) 11: 139-141",
                              this);
  ...
  AUTFBACK (lbl_input);
  lbl_input->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  AUTFBACK (lb_data);
  lb_data->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize - 1, QFont::Bold));
  ...
  pb_join_results->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize + 1));
  ...
  pb_save_results->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize + 1));
  ...
  le_last_file->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize - 1,
                                QFont::Bold));
  connect (le_last_file, SIGNAL (textChanged (const QString &)),
           SLOT (set_last_file (const QString &)));
  ...
  AUTFBACK (cb_plus_lm);
  cb_plus_lm->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize - 1,
                              QFont::Bold));
  ...
  cb_errorlines->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1,
                                 QFont::Bold));
  cb_errorlines->setText (
      us_tr ("Display error lines (+/- 1 sigma of linear fit)"));
  ...
  cb_manual_rejection->setFont (QFont (USglobal->config_list.fontFamily,
                                       USglobal->config_list.fontSize - 1,
                                       QFont::Bold));
  ...
  connect (cb_manual_rejection, SIGNAL (clicked ()),
           SLOT (set_manual_rejection ()));
  input_widgets.push_back (cb_manual_rejection);

  pb_apply_qtest = new QPushButton (us_tr ("Apply Q test criterion"), this);
  pb_apply_qtest->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize + 1));
  ...
  pb_reset_qtest->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize + 1));
  ...
  rb_90_qtest->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1,
                               QFont::Bold));
  ...
  rb_80_qtest->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1,
                               QFont::Bold));
  ...
  rb_70_qtest->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1,
                               QFont::Bold));
...
#if QT_VERSION < 0x040000
  plot_data->setTitleFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize + 3,
                                  QFont::Bold));
  plot_data->setAxisTitleFont (
  ...
  AUTFBACK (lbl_points);
  lbl_points->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize - 1,
                              QFont::Bold));
  lbl_points->show ();
  ...
  lbl_points_ln->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1,
                                 QFont::Bold));
  // lbl_points_ln->show();
  lbl_points_ln->hide ();
  ...
  lbl_points_exp->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize - 1,
                                  QFont::Bold));
  // lbl_points_exp->show();
  lbl_points_exp->hide ();
...
void
US_Hydrodyn_Best::cancel ()
{
  close ();
}

void
US_Hydrodyn_Best::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Best::closeEvent (QCloseEvent *e)
{
  *best_widget = false;
  global_Xpos -= 30;
  global_Ypos -= 30;
...
void
US_Hydrodyn_Best::hide_input ()
{
  hide_widgets (input_widgets, input_widgets[0]->isVisible ());
}

void
US_Hydrodyn_Best::hide_editor ()
{
...
void
US_Hydrodyn_Best::editor_msg (QString color, QString msg)
{
...
void
US_Hydrodyn_Best::hide_widgets (vector<QWidget *> widgets, bool hide)
{
...
void
US_Hydrodyn_Best::clear_display ()
{
  editor->clear ();
  editor->append ("\n\n");
}
...
void
US_Hydrodyn_Best::load ()
{
  // open file, clear lb_data, clear plot, reload lb_data
  save_last_file = "";
  le_last_file->setText (save_last_file);
  pb_load->setEnabled (false);
  QString use_dir = USglobal->config_list.root_dir + QDir::separator ()
                    + "somo" + QDir::separator () + "cluster"
                    + QDir::separator () + "results";

  ((US_Hydrodyn *)us_hydrodyn)->select_from_directory_history (use_dir, this);

  QString filename = QFileDialog::getOpenFileName (
      this, windowTitle () + us_tr (" Load CSV from BEST results"), use_dir,
      "*.csv *.CSV");
  ...
  if (!f.open (QIODevice::ReadOnly))
    {
      editor_msg ("red",
                  QString (us_tr ("Error opening file %1 (check permissions)"))
                      .arg (f.fileName ()));
      pb_load->setEnabled (true);
      return;
    }
  ...
  if (ts.atEnd ())
    {
      ...
      editor_msg (
          "red",
          QString (us_tr ("Error on file %1 no data")).arg (f.fileName ()));
      pb_load->setEnabled (true);
      return;
    }

  {
    ...
    QStringList qsl = US_Csv::parse_line (loaded_csv_trimmed.back ())
                          .replaceInStrings ("\"", "");
    ...
    if (qsl.size () < 8)
      {
        ...
        editor_msg (
            "red",
            QString (
                us_tr ("Error on file %1 insufficient data or no triangles?"))
                .arg (f.fileName ()));
        pb_load->setEnabled (true);
        return;
      }
    ...
    // us_qdebug( QString( "points read %1 qsl size %2" ).arg( points ).arg(
    // qsl.size() ) ); us_qdebug( QString( "qsl points + 0 %1" ).arg( qsl[
    // points + 0 ] ) ); us_qdebug( QString( "qsl points + 1 %1" ).arg( qsl[
    // points + 1 ] ) ); us_qdebug( QString( "qsl points + 2 %1" ).arg( qsl[
    // points + 2 ] ) ); us_qdebug( QString( "qsl points + 3 %1" ).arg( qsl[
    // points + 3 ] ) );
    ...
    if (qsl[points + 1] != "Extrapolation to zero triangles (a)")
      {
        ...
        editor_msg (
            "red",
            QString (us_tr ("Error on file %1 improper format on line 1"))
                .arg (f.fileName ()));
        pb_load->setEnabled (true);
        return;
      }
  ...
  loaded_csv_trimmed << ts.readLine (); // triangles used
  {
    ...
    // us_qdebug( "1/triangle line:" + qsl.join( ":" ) + QString( "size %1"
    // ).arg( qsl.size() ) );
    if ((int)qsl.size () < 1 + points || qsl[0] != "1/Triangles used")
      {
        ...
        editor_msg (
            "red",
            QString (us_tr ("Error on file %1 improper format on line 3"))
                .arg (f.fileName ()));
        pb_load->setEnabled (true);
        return;
      }
        ...
        if (qsl[i] == "=-1")
          {
            ...
            editor_msg (
                "red",
                QString (
                    us_tr ("Error on file %1 line 3: 1/triangles incorrect"))
                    .arg (f.fileName ()));
            pb_load->setEnabled (true);
            return;
          }
        one_over_triangles.push_back (
            QString (qsl[i]).replace (QRegExp ("^="), "").toDouble ());
      }
  }
      ...
      if ((int)qsl.size () < 1 + points)
        {
          ...
          editor_msg (
              "red",
              QString (us_tr ("Error on file %1 improper format on line %2"))
                  .arg (f.fileName ())
                  .arg (line));
          pb_load->setEnabled (true);
          connect (lb_data, SIGNAL (itemSelectionChanged ()),
                   SLOT (data_selected ()));
          return;
        }
      // us_qdebug( QString( "qsl 0: %1 : size %2" ).arg( qsl[ 0 ] ).arg(
      // qsl.size() ) );
          ...
          for (int i = 1; i <= points; ++i)
            {
              ...
              parameter_data[qsl[0]].push_back (
                  QString (qsl[i]).replace (QRegExp ("^="), "").toDouble ());
              if (qsl[i] == "?")
                {
                  f.close ();
                  editor_msg ("red",
                              QString (us_tr ("Error on file %1 line %2 - "
                                              "missing data for column %3"))
                                  .arg (f.fileName ())
                                  .arg (line)
                                  .arg (i + 1));
                  pb_load->setEnabled (true);
...
void
US_Hydrodyn_Best::cb_changed (bool do_data)
{
...
void
US_Hydrodyn_Best::cb_changed_ln (bool do_data)
{
...
void
US_Hydrodyn_Best::cb_changed_exp (bool do_data)
{
...
void
US_Hydrodyn_Best::data_selected (bool do_recompute_tau)
{
  ...
  QString text = lb_data->selectedItems ().first ()->text ();
  // us_qdebug( QString( "selected %1 map %2" ).arg( text ).arg(
  // parameter_data[ text ].size() ) );
  plot_data->detachItems (QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
  ...
  for (int i = 0; i < (int)one_over_triangles.size (); ++i)
    {
      ...
      if (selected_points.count (i)
          && (selected_points_ln.count (i) || selected_points_exp.count (i)))
        {
      ...
      if (!selected_points.count (i) && !selected_points_ln.count (i)
...
#if QT_VERSION < 0x040000
      plot_data->setCurveData (curve, (double *)&(one_over_triangles[i]),
                               (double *)&(parameter_data[text][i]), 1);
      plot_data->setCurveStyle (curve, QwtCurve::Lines);
      plot_data->setCurveSymbol (curve, sym);
#else
      curve->setSamples ((double *)&(one_over_triangles[i]),
                         (double *)&(parameter_data[text][i]), 1);
    ...
    if (use_one_over_triangles.size () > 1)
      {
        ...
        US_Saxs_Util::linear_fit (use_one_over_triangles, use_parameter_data,
                                  a, b, siga, sigb, chi2);
        ...
        editor_msg (
            "blue",
            QString (us_tr ("%1: 0 triangle extrapolation=%2 sigma=%3 sigma "
                            "%=%4 slope=%5 sigma=%6 sigma %=%7 chi^2=%8"))
                .arg (text)
                .arg (a, 0, 'g', 8)
                .arg (siga, 0, 'g', 8)
...
#if QT_VERSION < 0x040000
          plot_data->setCurveData (curve, (double *)&(x[0]), (double *)&(y[0]),
                                   2);
...
#if QT_VERSION < 0x040000
              plot_data->setCurveData (curve, (double *)&(x[0]),
                                       (double *)&(yp[0]), 2);
              plot_data->setCurvePen (
                  curve, QPen (Qt::darkGreen, 2, Qt::DashDotLine));

#else
              curve->setSamples ((double *)&(x[0]), (double *)&(yp[0]), 2);
...
#else
              curve->setSamples ((double *)&(x[0]), (double *)&(ym[0]), 2);
    ...
    if (use_one_over_triangles_ln.size () > 1)
      {
        ...
        for (int i = 0; i < (int)use_one_over_triangles_ln.size (); ++i)
          {
            fit_x.push_back (use_one_over_triangles_ln[i]
                             * log (use_one_over_triangles_ln[i]));
            fit_y.push_back (log (use_parameter_data_ln[i]));
          }

        US_Saxs_Util::linear_fit (fit_x, fit_y, a, b, siga, sigb, chi2);
...
#define UHB_MINX 1e-20
        ...
        editor_msg (
            "blue",
            QString (
                us_tr ("%1: 0 triangle LN extrapolation=%2 sigma=%3 sigma "
                       "%=%4 slope=%5 sigma=%6 sigma %=%7 chi^2=%8"))
                .arg (text)
                .arg (exp (a), 0, 'g', 8)
                .arg (exp (a) * siga, 0, 'g', 8)
                .arg (exp (a) != 0 ? fabs (100.0 * exp (a) * siga / exp (a))
                                   : (double)0,
                      0, 'g', 8)
                .arg (b, 0, 'g', 8)
                .arg (sigb, 0, 'g', 8)
        ...
        double deltax
            = (one_over_triangles[0] * 1.1 - UHB_MINX) / (UHB_PTS - 1);
...
#else
        curve->setSamples ((double *)&(x[0]), (double *)&(y[0]), UHB_PTS);
    ...
    if (use_one_over_triangles_exp.size () > 2)
      {
        ...
        fit_y.push_back (.966);
        fit_x.push_back (.222);
        fit_y.push_back (1.123);
        fit_x.push_back (.401);
        fit_y.push_back (1.414);
        fit_x.push_back (.465);
        fit_y.push_back (1.683);
        fit_x.push_back (.633);
        fit_y.push_back (2.101);
        fit_x.push_back (.637);
        fit_y.push_back (1.94);
        fit_x.push_back (.735);
        fit_y.push_back (2.473);
        fit_x.push_back (.762);
        fit_y.push_back (2.276);
        fit_x.push_back (.791);
        fit_y.push_back (2.352);
        fit_x.push_back (.981);
        fit_y.push_back (3.544);
#endif
        ...
        for (int i = 1; i < (int)fit_x.size (); ++i)
          {
            S[i]
                = S[i - 1]
                  + .5 * (fit_y[i] + fit_y[i - 1]) * (fit_x[i] - fit_x[i - 1]);
          }
        ...
        if (det == 0e0)
          {
            editor_msg ("red",
                        us_tr ("exponential fit: zero determinant in fit"));
          }
        else
          {
            ...
            if (B1 == 0e0)
            ...
            else
              {
                ...
                if (det == 0e0)
                  {
                    editor_msg ("red", us_tr ("exponential fit: zero second "
                                              "determinant in fit"));
                  }
                else
                  {
                    ...
                    b = b2;
                    c = c1;
                    // us_qdebug( QString( "ok: a = %1 b = %2 c = %3" ).arg( a
                    // ).arg( b ).arg( c ) );
                    exp_plot_ok = true;
                  }
              }
        ...
        if (exp_plot_ok)
          {
            ...
            if (cb_plus_lm->isChecked ())
              {
                ...
                control.printflags
                    = 0; // 3; // monitor status (+1) and parameters (+2)
                ...
                LM::lmcurve_fit_rmsd (
                    (int)par.size (), (double *)&(par[0]), (int)fit_x.size (),
                    (double *)&(fit_x[0]), (double *)&(fit_y[0]),
                    BFIT::compute_f, (const LM::lm_control_struct *)&control,
                    &status);
                    ...
                    for (int i = 0; i < (int)fit_x.size (); ++i)
                      {
                        if (fit_y[i] != 0e0)
                          {
                            double t = fit_y[i] - par[0]
                                       + par[1] * exp (par[2] * fit_x[i]);
                            chi2 += t * t / fit_y[i];
                          }
                      }
                    if (chi2 < org_chi2)
                      ...
                      {
                        editor_msg ("dark red",
                                    us_tr ("Notice: LM did not improve the "
                                           "fit, discarded"));
                      }
                  }
              }
            ...
            editor_msg ("blue",
                        QString (us_tr ("%1: 0 triangle EXP extrapolation=%2 "
                                        "sigma=%2 b=%3 c=%4 chi^2=%5"))
                            .arg (text)
                            .arg (a + b, 0, 'g', 8)
                            .arg (siga, 0, 'g', 8)
                            .arg (b, 0, 'g', 8)
  ...
  {
    plot_data->setAxisScale (QwtPlot::xBottom, 0,
                             one_over_triangles[0] * 1.1e0);

    miny < 0
        ? plot_data->setAxisScale (QwtPlot::yLeft, miny * 1.03e0, maxy * .97e0)
        : plot_data->setAxisScale (QwtPlot::yLeft, miny * 0.97e0,
                                   maxy * 1.03e0);
  }
...
#endif
  if (do_recompute_tau && tau_input_set.count (text))
    {
...
US_Hydrodyn_Best::save_results ()
{
  ...
  ((US_Hydrodyn *)us_hydrodyn)->select_from_directory_history (use_dir, this);
  use_dir += QDir::separator () + QFileInfo (loaded_csv_filename).baseName ()
             + "_results.csv";
  // us_qdebug( use_dir );

  QString filename = QFileDialog::getSaveFileName (
      this, us_tr ("Select a name to save the state"), use_dir, "*.csv *.CSV");
  ...
  if (!f.open (QIODevice::WriteOnly))
    {
      editor_msg ("red", QString (us_tr ("Could not open file %1 for writing"))
                             .arg (f.fileName ()));
      return;
    }
  ...
  for (int i = 0; i < (int)lb_data->count (); ++i)
    {
      ...
      additions[lb_data->item (i)->text ()]
          = QString ("=%1,%2%3,%4%5")
                ...
                .arg (last_siga, 0, 'g', 8)
                .arg (us_isnan (last_siga) ? "=" : "")
                .arg (last_a != 0 ? fabs (100.0 * last_siga / last_a)
                                  : (double)0,
                      0, 'g', 8)
            + QString (",=%1,%2%3,%4%5")
                  .arg (last_b, 0, 'g', 8)
                  ...
                  .arg (last_sigb, 0, 'g', 8)
                  .arg (us_isnan (last_sigb) ? "=" : "")
                  .arg (last_b != 0 ? fabs (100.0 * last_sigb / last_b)
                                    : (double)0,
                        0, 'g', 8)
            + QString (",=%1,%2")
                  .arg (last_chi2, 0, 'g', 8)
          ...
          additions[lb_data->item (i)->text ()]
              += QString (",=%1,%2%3,%4%5")
                     ...
                     .arg ((us_isnan (last_siga_ln) || us_isnan (last_a_ln))
                               ...
                               : "")
                     .arg (exp (last_a_ln) != 0
                               ? fabs (100.0 * exp (last_a_ln) * last_siga_ln
                                       / exp (last_a_ln))
                               : (double)0,
                           0, 'g', 8)
                 + QString (",=%1,%2%3,%4%5")
                       ...
                       .arg (us_isnan (last_sigb_ln) ? "=" : "")
                       .arg (last_b_ln != 0
                                 ? fabs (100.0 * last_sigb_ln / last_b_ln)
                                 : (double)0,
                             0, 'g', 8)
                 + QString (",=%1,%2")
      ...
      if (exp_plot_ok)
        {
          ...
          additions[lb_data->item (i)->text ()]
              += QString (",=%1,%2%3,%4%5")
                     ...
                     .arg (us_isnan (last_siga_exp) ? "=" : "")
                     .arg (last_a_exp != 0
                               ? fabs (100.0 * last_siga_exp / last_a_exp)
                               : (double)0,
                           0, 'g', 8)
                 + QString (",=%1,%2%3,%4%5")
                       ...
                       .arg (us_isnan (last_sigb_exp) ? "=" : "")
                       .arg (last_b_exp != 0
                                 ? fabs (100.0 * last_sigb_exp / last_b_exp)
                                 : (double)0,
                             0, 'g', 8)
                 + QString (",=%1,%2")
  ...
  for (int i = 0; i < (int)loaded_csv_trimmed.size (); ++i)
    {
      QStringList qsl = US_Csv::parse_line (loaded_csv_trimmed[i])
                            .replaceInStrings ("\"", "");
  ...
  if (any_ln_plot)
    {
      out[0] += ",\"LN: Extrapolation to zero triangles (a)\",\"Sigma "
                "a\",\"Sigma a %\",\"Slope (b)\",\"Sigma b\",\"Sigma b "
                "%\",\"chi^2\"";
      out[0] += ",\"Points removed (largest number of triangles is point 1)\"";
    }
  if (any_exp_plot)
    {
      out[0] += ",\"EXP: Extrapolation to zero triangles (a)\",\"Sigma "
                "a\",\"Sigma a %\",\"Slope (b)\",\"Sigma b\",\"Sigma b "
                "%\",\"chi^2\"";
      out[0] += ",\"Points removed (largest number of triangles is point 1)\"";
    }
...
void
US_Hydrodyn_Best::recompute_tau ()
{
  ...
  if (lin_ok)
    {
      ...
      US_Saxs_Util::compute_tau (
          ...
          last_lin_extrapolation[tau_inputs[2]][0] * 1e-3, .1,
          this_tau_results);

      for (int i = 0; i < (int)this_tau_results.size (); ++i)
        {
          tau_csv_addendum_tag
              << QString (us_tr ("Linear extrapolation of Drr EV (1/s) %1"))
                     .arg (tau_msg[i]);
          tau_csv_addendum_val
              << QString ("%1").arg (this_tau_results[i], 0, 'g', 8);
          msg += tau_csv_addendum_tag.back () + " "
                 + tau_csv_addendum_val.back () + "\n";
        }
  ...
  if (log_ok)
    {
      ...
      US_Saxs_Util::compute_tau (
          ...
          exp (last_log_extrapolation[tau_inputs[2]][0]) * 1e-3, .1,
          this_tau_results);
      ...
      for (int i = 0; i < (int)this_tau_results.size (); ++i)
        {
          tau_csv_addendum_tag
              << QString (us_tr ("LOG extrapolation of Drr EV (1/s) %1"))
                     .arg (tau_msg[i]);
          tau_csv_addendum_val
              << QString ("%1").arg (this_tau_results[i], 0, 'g', 8);
          msg += tau_csv_addendum_tag.back () + " "
  ...
  if (exp_ok)
    {
      ...
      US_Saxs_Util::compute_tau (
          ...
          last_exp_extrapolation[tau_inputs[2]][0] * 1e-3, .1,
          this_tau_results);
      ...
      for (int i = 0; i < (int)this_tau_results.size (); ++i)
        {
          tau_csv_addendum_tag
              << QString (us_tr ("EXP extrapolation of Drr EV (1/s) %1"))
                     .arg (tau_msg[i]);
          tau_csv_addendum_val
              << QString ("%1").arg (this_tau_results[i], 0, 'g', 8);
          msg += tau_csv_addendum_tag.back () + " "
...
void
US_Hydrodyn_Best::toggle_points ()
{
...
void
US_Hydrodyn_Best::toggle_points_ln ()
{
  bool any_checked = false;
  for (int i = 0; i < (int)cb_points_ln.size (); ++i)
    {
      ...
      if (cb_points_ln[i]->isEnabled ())
        {
          ...
          connect (cb_points_ln[i], SIGNAL (clicked ()),
                   SLOT (cb_changed_ln ()));
        }
    }
  cb_changed_ln ();
...
void
US_Hydrodyn_Best::toggle_points_exp ()
{
  bool any_checked = false;
  for (int i = 0; i < (int)cb_points_exp.size (); ++i)
    {
      ...
      if (cb_points_exp[i]->isEnabled ())
        {
          ...
          connect (cb_points_exp[i], SIGNAL (clicked ()),
                   SLOT (cb_changed_exp ()));
        }
    }
  cb_changed_exp ();
...
void
US_Hydrodyn_Best::set_last_file (const QString &str)
{
...
US_Hydrodyn_Best::join_results ()
{
  ...
  do
    {
      files = QFileDialog::getOpenFileNames (
          this, us_tr ("Select CSV results to join"), use_dir,
          "CSV files (*.csv *.CSV)");
  ...
  QString save_file;
  {
    ...
    save_file = QFileDialog::getSaveFileName (
        this, us_tr ("Choose a name to save the joined CSV results"), use_dir,
        "CSV files (*.csv *.CSV)");
  ...
  for (int i = 0; i < (int)join_files.size (); ++i)
    {
      ...
      QStringList qsl
          = US_Csv::parse_line (ts_in.readLine ()).replaceInStrings ("\"", "");
      if (!qsl.size ())
        {
          editor_msg (
              "red", QString (us_tr ("Error: file %1 error on line 1 (empty)"))
                         .arg (join_files[i]));
          f_in.close ();
          f_out.close ();
          f_out.remove ();
      ...
      int tmp_points;
      for (tmp_points = 1;
           tmp_points < (int)qsl.size ()
           && qsl[tmp_points] != "Extrapolation to zero triangles (a)";
           ++tmp_points)
        {
        };
      tmp_points--;

      // us_qdebug( QString( "tmp_points %1 qsl size %2" ).arg( tmp_points
      // ).arg( qsl.size() ) );
      if ((int)qsl.size () <= tmp_points + 1)
        {
          editor_msg (
              "red",
              QString (us_tr ("Error: file %1 error on line 1 (points)"))
                  .arg (join_files[i]));
          f_in.close ();
          f_out.close ();
          f_out.remove ();
      ...
      if (qsl[tmp_points + 1] != "Extrapolation to zero triangles (a)")
        {
          editor_msg ("red",
                      QString (us_tr ("Error: file %1 error on line 1 (tag)"))
                          .arg (join_files[i]));
          f_in.close ();
          f_out.close ();
          f_out.remove ();
      ...
      while (!ts_in.atEnd ())
        {
          QStringList qsl = US_Csv::parse_line (
              ts_in.readLine ()); //.replaceInStrings( "\"", "" );
  ...
  ts_out << ",,\"Extrapolation to zero triangles (a)\",\"Sigma a\",\"Sigma a "
            "%\",\"Slope (b)\",\"Sigma b\",\"Sigma b %\",\"chi^2\",\"Points "
            "removed (largest number of triangles is point 1)\""
         << Qt::endl;
...
public:
  ...
  int index;
  bool
  operator< (const uhb_sortable_double &objIn) const
  {
    return x < objIn.x;
  }
};
...
US_Hydrodyn_Best::apply_qtest ()
{
  ...
  vector<double> Qc = rb_90_qtest->isChecked ()
                          ? Qc95
                          : (rb_80_qtest->isChecked () ? Qc80 : Qc70);
  ...
  for (int i = 0; i < (int)use_one_over_triangles.size (); ++i)
    {
      delta.x = use_parameter_data[i]
                - (last_a + use_one_over_triangles[i] * last_b);
      ...
      //         );
      // us_qdebug( QString( "compute delta %1 index %2" ).arg( delta.x ).arg(
      // delta.index ) );
  ...
  if (vdeltas.back ().x == vdeltas[0].x)
    {
      editor_msg (
          "blue",
          us_tr ("Q test: all deltas identical, no points can be rejected"));
      return;
    }

  int vdelta_end = (int)vdeltas.size () - 1;

  double Q = (vdeltas[vdelta_end].x - vdeltas[vdelta_end - 1].x)
             / (vdeltas[vdelta_end].x - vdeltas[0].x);

  QString msg = QString (us_tr ("Q test: Q = %1 ")).arg (Q);

  double qc_mult = rb_90_qtest->isChecked ()
                       ? 1e0
                       : (rb_80_qtest->isChecked () ? .8e0 : .7e0);
  ...
  if ((use_Qc && Qc[vdeltas.size ()] < Q)
    ...
    {
      msg += QString (us_tr ("Removing point %1"))
                 .arg (vdeltas.back ().index + 1);
      if (rb_80_qtest->isChecked ())
        {
          editor_msg ("dark red",
                      us_tr ("WARNING: 80% Q test criterion parameters used"));
        }
      if (rb_70_qtest->isChecked ())
        {
          editor_msg ("dark red",
                      us_tr ("WARNING: 70% Q test criterion parameters used"));
        }
      editor_msg ("blue", msg);
...
void
US_Hydrodyn_Best::set_loose_qtest ()
{
  if (!rb_90_qtest->isChecked ())
    {
      switch (
          QMessageBox::warning (this, windowTitle () + us_tr (" : Warning"),
...
void
US_Hydrodyn_Best::set_manual_rejection ()
{
  if (cb_manual_rejection->isChecked ())
    {
...
void
US_Hydrodyn_Best::usp_config_plot_data (const QPoint &)
{
