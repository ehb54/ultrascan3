...
void US_Hydrodyn_Best::cancel() { close(); }
...
void US_Hydrodyn_Best::load() {
  ...
  QString use_dir = USglobal->config_list.root_dir + QDir::separator() +
                    "somo" + QDir::separator() + "cluster" + QDir::separator() +
                    "results";
...

  {
    ...
    if (qsl.size() < 8) {
      ...
      editor_msg(
          "red",
          QString(us_tr("Error on file %1 insufficient data or no triangles?"))
              .arg(f.fileName()));
      pb_load->setEnabled(true);
      return;
    }
  ...
  loaded_csv_trimmed << ts.readLine();  // triangles used
  {
    ...
    for (int i = 1; i <= points; ++i) {
      if (qsl[i] == "=-1") {
        ...
        editor_msg(
            "red",
            QString(us_tr("Error on file %1 line 3: 1/triangles incorrect"))
                .arg(f.fileName()));
        pb_load->setEnabled(true);
        return;
      }
      ...
      for (int i = 1; i <= points; ++i) {
        ...
        if (qsl[i] == "?") {
          ...
          editor_msg(
              ...
              QString(
                  us_tr(
                      "Error on file %1 line %2 - missing data for column %3"))
                  .arg(f.fileName())
                  .arg(line)
                  .arg(i + 1));
          pb_load->setEnabled(true);
...
void US_Hydrodyn_Best::data_selected(bool do_recompute_tau) {
  ...
  QString text = lb_data->selectedItems().first()->text();
  // us_qdebug( QString( "selected %1 map %2" ).arg( text ).arg( parameter_data[
  // text ].size() ) );
  plot_data->detachItems(QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems(QwtPlotItem::Rtti_PlotMarker);
  ;
...

  {
    ...
    if (use_one_over_triangles.size() > 1) {
      ...
      US_Saxs_Util::linear_fit(use_one_over_triangles, use_parameter_data, a, b,
                               siga, sigb, chi2);
      ...
      editor_msg(
          "blue",
          QString(us_tr("%1: 0 triangle extrapolation=%2 sigma=%3 sigma %=%4 "
                        "slope=%5 sigma=%6 sigma %=%7 chi^2=%8"))
              .arg(text)
              .arg(a, 0, 'g', 8)
              .arg(siga, 0, 'g', 8)
...
#if QT_VERSION < 0x040000
        plot_data->setCurveData(curve, (double *)&(x[0]), (double *)&(y[0]), 2);
...
#if QT_VERSION < 0x040000
          plot_data->setCurveData(curve, (double *)&(x[0]), (double *)&(yp[0]),
                                  2);
          plot_data->setCurvePen(curve,
                                 QPen(Qt::darkGreen, 2, Qt::DashDotLine));
...
#if QT_VERSION < 0x040000
          plot_data->setCurveData(curve, (double *)&(x[0]), (double *)&(ym[0]),
                                  2);
          plot_data->setCurvePen(curve,
                                 QPen(Qt::darkGreen, 2, Qt::DashDotLine));
...
#define UHB_MINX 1e-20
      ...
      editor_msg(
          "blue",
          QString(us_tr("%1: 0 triangle LN extrapolation=%2 sigma=%3 sigma "
      ...
      if (det == 0e0) {
        ...
        if (B1 == 0e0) {
          ...
          if (det == 0e0) {
            editor_msg(
                "red",
                us_tr("exponential fit: zero second determinant in fit"));
          } else {
            double oneoverdet = 1e0 / det;
            ...
            b = b2;
            c = c1;
            // us_qdebug( QString( "ok: a = %1 b = %2 c = %3" ).arg( a ).arg( b
            // ).arg( c ) );
            exp_plot_ok = true;
          }
        }
      ...
      if (exp_plot_ok) {
        ...
        if (cb_plus_lm->isChecked()) {
          ...
          LM::lmcurve_fit_rmsd(
              ...
              (double *)&(fit_x[0]), (double *)&(fit_y[0]), BFIT::compute_f,
          ...
          if (status.fnorm < 0e0) {
            ...
            if (chi2 < org_chi2) {
              ...
              editor_msg(
                  "dark red",
                  us_tr("Notice: LM did not improve the fit, discarded"));
            }
          }
        }
        ...
        editor_msg("blue", QString(us_tr("%1: 0 triangle EXP extrapolation=%2 "
...
void US_Hydrodyn_Best::save_results() {
  ...
  if (!f.open(QIODevice::WriteOnly)) {
    editor_msg(
        "red",
        QString(us_tr("Could not open file %1 for writing")).arg(f.fileName()));
    return;
  }
  ...
  for (int i = 0; i < (int)lb_data->count(); ++i) {
    ...
    additions[lb_data->item(i)->text()] =
        QString("=%1,%2%3,%4%5")
            ...
            .arg(last_siga, 0, 'g', 8)
            .arg(us_isnan(last_siga) ? "=" : "")
            .arg(last_a != 0 ? fabs(100.0 * last_siga / last_a) : (double)0, 0,
                 'g', 8) +
        QString(",=%1,%2%3,%4%5")
            .arg(last_b, 0, 'g', 8)
            ...
            .arg(last_sigb, 0, 'g', 8)
            .arg(us_isnan(last_sigb) ? "=" : "")
            .arg(last_b != 0 ? fabs(100.0 * last_sigb / last_b) : (double)0, 0,
                 'g', 8) +
        QString(",=%1,%2").arg(last_chi2, 0, 'g', 8).arg(last_pts_removed);
    ...
    if (ln_plot_ok) {
      ...
      additions[lb_data->item(i)->text()] +=
          QString(",=%1,%2%3,%4%5")
              ...
              .arg((us_isnan(last_siga_ln) || us_isnan(last_a_ln)) ? "=" : "")
              .arg(exp(last_a_ln) != 0 ? fabs(100.0 * exp(last_a_ln) *
                                              last_siga_ln / exp(last_a_ln))
                                       : (double)0,
                   0, 'g', 8) +
          QString(",=%1,%2%3,%4%5")
              .arg(last_b_ln, 0, 'g', 8)
              ...
              .arg(us_isnan(last_sigb_ln) ? "=" : "")
              .arg(last_b_ln != 0 ? fabs(100.0 * last_sigb_ln / last_b_ln)
                                  : (double)0,
                   0, 'g', 8) +
          QString(",=%1,%2")
    ...
    if (exp_plot_ok) {
      ...
      additions[lb_data->item(i)->text()] +=
          QString(",=%1,%2%3,%4%5")
              ...
              .arg(us_isnan(last_siga_exp) ? "=" : "")
              .arg(last_a_exp != 0 ? fabs(100.0 * last_siga_exp / last_a_exp)
                                   : (double)0,
                   0, 'g', 8) +
          QString(",=%1,%2%3,%4%5")
              ...
              .arg(us_isnan(last_sigb_exp) ? "=" : "")
              .arg(last_b_exp != 0 ? fabs(100.0 * last_sigb_exp / last_b_exp)
                                   : (double)0,
                   0, 'g', 8) +
          QString(",=%1,%2")
  ...
  for (int i = 0; i < (int)loaded_csv_trimmed.size(); ++i) {
    QStringList qsl =
        US_Csv::parse_line(loaded_csv_trimmed[i]).replaceInStrings("\"", "");
  ...
  if (any_ln_plot) {
    out[0] +=
        ",\"LN: Extrapolation to zero triangles (a)\",\"Sigma a\",\"Sigma a "
        "%\",\"Slope (b)\",\"Sigma b\",\"Sigma b %\",\"chi^2\"";
    out[0] += ",\"Points removed (largest number of triangles is point 1)\"";
  }
  if (any_exp_plot) {
    out[0] +=
        ",\"EXP: Extrapolation to zero triangles (a)\",\"Sigma a\",\"Sigma a "
        "%\",\"Slope (b)\",\"Sigma b\",\"Sigma b %\",\"chi^2\"";
    out[0] += ",\"Points removed (largest number of triangles is point 1)\"";
  }
...
void US_Hydrodyn_Best::recompute_tau() {
  ...
  if (lin_ok) {
    ...
    US_Saxs_Util::compute_tau(last_lin_extrapolation[tau_inputs[0]][0] * 1e-3,
  ...
  if (log_ok) {
    ...
    QString msg;
    for (int i = 0; i < (int)this_tau_results.size(); ++i) {
      tau_csv_addendum_tag << QString(
                                  us_tr("LOG extrapolation of Drr EV (1/s) %1"))
                                  .arg(tau_msg[i]);
      tau_csv_addendum_val << QString("%1").arg(this_tau_results[i], 0, 'g', 8);
      msg += tau_csv_addendum_tag.back() + " " + tau_csv_addendum_val.back() +
             "\n";
  ...
  if (exp_ok) {
    ...
    US_Saxs_Util::compute_tau(last_exp_extrapolation[tau_inputs[0]][0] * 1e-3,
    ...
    for (int i = 0; i < (int)this_tau_results.size(); ++i) {
      tau_csv_addendum_tag << QString(
                                  us_tr("EXP extrapolation of Drr EV (1/s) %1"))
                                  .arg(tau_msg[i]);
      tau_csv_addendum_val << QString("%1").arg(this_tau_results[i], 0, 'g', 8);
      msg += tau_csv_addendum_tag.back() + " " + tau_csv_addendum_val.back() +
             "\n";
...
void US_Hydrodyn_Best::join_results() {
  ...
  do {
    files =
        QFileDialog::getOpenFileNames(this, us_tr("Select CSV results to join"),
                                      use_dir, "CSV files (*.csv *.CSV)");
  ...
  if (!f_out.open(QIODevice::WriteOnly)) {
    editor_msg("red", QString(us_tr("Error: Can not open file %1 for writing"))
                          .arg(save_file));
    return;
  }
  ...
  for (int i = 0; i < (int)join_files.size(); ++i) {
    ...
    if (!qsl.size()) {
      editor_msg("red", QString(us_tr("Error: file %1 error on line 1 (empty)"))
                            .arg(join_files[i]));
      f_in.close();
      f_out.close();
      f_out.remove();
    ...
    // us_qdebug( QString( "tmp_points %1 qsl size %2" ).arg( tmp_points ).arg(
    // qsl.size() ) );
    ...
    if (qsl[tmp_points + 1] != "Extrapolation to zero triangles (a)") {
      editor_msg("red", QString(us_tr("Error: file %1 error on line 1 (tag)"))
                            .arg(join_files[i]));
      f_in.close();
      f_out.close();
      f_out.remove();
...
class uhb_sortable_double {
 public:
  double x;
  int index;
  bool operator<(const uhb_sortable_double &objIn) const { return x < objIn.x; }
};
...
void US_Hydrodyn_Best::apply_qtest() {
  ...
  for (int i = 0; i < (int)use_one_over_triangles.size(); ++i) {
    delta.x =
        use_parameter_data[i] - (last_a + use_one_over_triangles[i] * last_b);
  ...
  double qc_mult =
      rb_90_qtest->isChecked() ? 1e0 : (rb_80_qtest->isChecked() ? .8e0 : .7e0);
...
void US_Hydrodyn_Best::set_loose_qtest() {
  if (!rb_90_qtest->isChecked()) {
    switch (QMessageBox::warning(this, windowTitle() + us_tr(" : Warning"),
