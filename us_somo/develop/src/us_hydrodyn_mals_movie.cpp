...
US_Hydrodyn_Mals_Movie::US_Hydrodyn_Mals_Movie(US_Hydrodyn_Mals *mals_win,
  ...
  last_show_gauss = ush_win->gparams.count("mals_movie_show_gauss") &&
...
void US_Hydrodyn_Mals_Movie::setupGUI() {
  int minHeight1 = 24;
  int minHeight3 = 25;
...
void US_Hydrodyn_Mals_Movie::cancel() { close(); }

void US_Hydrodyn_Mals_Movie::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/somo_mals_movie.html");
}

void US_Hydrodyn_Mals_Movie::closeEvent(QCloseEvent *e) {
...
void US_Hydrodyn_Mals_Movie::front() {
  pos = 0;
  update_plot();
}

void US_Hydrodyn_Mals_Movie::prev() {
  if (pos > 0) {
    pos--;
    update_plot();
  }
...
void US_Hydrodyn_Mals_Movie::slower() {
  timer_msec *= 2;
  if (timer_msec > 5000) {
    timer_msec = 5000;
  }
...
void US_Hydrodyn_Mals_Movie::start() {
  if (timer->isActive()) {
    timer->stop();
    pb_start->setText("[]");
    lbl_state->setText(
...
void US_Hydrodyn_Mals_Movie::faster() {
  timer_msec /= 2;
  if (timer_msec < 50) {
    timer_msec = 50;
  }
...
void US_Hydrodyn_Mals_Movie::next() {
...
void US_Hydrodyn_Mals_Movie::end() {
  pos = mals_selected_files.size() - 1;
  update_plot();
}

void US_Hydrodyn_Mals_Movie::set_save() { update_enables(); }

void US_Hydrodyn_Mals_Movie::set_show_gauss() { update_plot(); }

void US_Hydrodyn_Mals_Movie::set_show_ref() { update_plot(); }

void US_Hydrodyn_Mals_Movie::set_mono() {
...
void US_Hydrodyn_Mals_Movie::update_enables() {
  le_save->setEnabled(cb_save->isChecked());
}

void US_Hydrodyn_Mals_Movie::update_plot() {
  if (last_pos != pos || cb_show_gauss->isChecked() != last_show_gauss ||
    ...
    if (cb_show_gauss->isChecked() &&
        ...
        mals_win->f_is_time[mals_win->lb_files->item(mals_selected_files[pos])
                                ->text()]) {
...
void US_Hydrodyn_Mals_Movie::save_plot() {
  if (mals_win->plot_errors->isVisible()) {
    if (mals_win->plot_ref->isVisible()) {
      save_plot(mals_win->plot_dist, mals_win->plot_ref, mals_win->plot_errors,
                le_save->text());
    } else {
    ...
    if (mals_win->plot_ref->isVisible()) {
      save_plot(mals_win->plot_dist, mals_win->plot_ref, le_save->text());
    } else {
...
void US_Hydrodyn_Mals_Movie::save_plot(QWidget *plot, QString tag) {
  int mypos = pos;
...
void US_Hydrodyn_Mals_Movie::save_plot(QWidget *plot, QWidget *plot2,
...
void US_Hydrodyn_Mals_Movie::save_plot(QPixmap &qPix, QString tag, int mypos) {
  QPainter paint(&qPix);
