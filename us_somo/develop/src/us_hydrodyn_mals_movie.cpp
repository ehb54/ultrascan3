...
US_Hydrodyn_Mals_Movie::US_Hydrodyn_Mals_Movie (US_Hydrodyn_Mals *mals_win,
    ...
    : QDialog (p)
{
  ...
  mals_win->lb_files->clearSelection ();

  pos = 0;
  last_pos = -1;

  timer_msec = ush_win->gparams.count ("mals_movie_timer_ms")
                   ? ush_win->gparams["mals_movie_timer_ms"].toInt ()
                   : 1000;

  last_show_gauss
      = ush_win->gparams.count ("mals_movie_show_gauss")
                && ush_win->gparams["mals_movie_show_gauss"] == "true"
            ? true
            : false;

  last_show_ref = ush_win->gparams.count ("mals_movie_show_ref")
                          && ush_win->gparams["mals_movie_show_ref"] == "true"
                      ? true
                      : false;

  last_mono = ush_win->gparams.count ("mals_movie_mono")
                      && ush_win->gparams["mals_movie_mono"] == "true"
                  ? true
                  : false;
  ...
  mals_win->suppress_replot = true;
  plot_colors = mals_win->plot_colors;
  set_mono ();
  update_plot ();
  show ();
  //    cout << "doin it\n";
...
US_Hydrodyn_Mals_Movie::~US_Hydrodyn_Mals_Movie () {}

void
US_Hydrodyn_Mals_Movie::setupGUI ()
{
  int minHeight1 = 24;
  int minHeight3 = 25;
  ...
  lbl_current = new QLabel (
      mals_win->lb_files->item (mals_selected_files[pos])->text (), this);
...
void
US_Hydrodyn_Mals_Movie::cancel ()
{
  close ();
}

void
US_Hydrodyn_Mals_Movie::help ()
{
  US_Help *online_help;
  online_help = new US_Help (this);
  online_help->show_help ("manual/somo/somo_mals_movie.html");
}

void
US_Hydrodyn_Mals_Movie::closeEvent (QCloseEvent *e)
{
  mals_win->plot_colors = plot_colors;
  mals_win->disable_updates = true;
  if (mals_win->current_mode == mals_win->MODE_GAUSSIAN)
    {
      mals_win->wheel_cancel ();
    }
  mals_win->lb_files->clearSelection ();
  for (int i = 0; i < (int)mals_selected_files.size (); ++i)
    {
      mals_win->lb_files->item (mals_selected_files[i])->setSelected (true);
    }
  mals_win->suppress_replot = false;
  mals_win->plot_files ();
  mals_win->disable_updates = false;

  ush_win->gparams["mals_movie_timer_ms"] = QString ("%1").arg (timer_msec);
  ush_win->gparams["mals_movie_show_gauss"]
      = cb_show_gauss->isChecked () ? "true" : "false";
  ush_win->gparams["mals_movie_show_ref"]
      = cb_show_ref->isChecked () ? "true" : "false";
  e->accept ();
}

void
US_Hydrodyn_Mals_Movie::front ()
{
  pos = 0;
  update_plot ();
}
...
void
US_Hydrodyn_Mals_Movie::next ()
{
  if (pos < (int)mals_selected_files.size () - 1)
    {
      pos++;
      update_plot ();
    }
...
void
US_Hydrodyn_Mals_Movie::end ()
{
  pos = mals_selected_files.size () - 1;
  update_plot ();
}

void
US_Hydrodyn_Mals_Movie::set_save ()
{
  update_enables ();
}

void
US_Hydrodyn_Mals_Movie::set_show_gauss ()
{
  update_plot ();
}

void
US_Hydrodyn_Mals_Movie::set_show_ref ()
{
  update_plot ();
}

void
US_Hydrodyn_Mals_Movie::set_mono ()
{
  if (cb_mono->isChecked ())
    {
      mals_win->plot_colors.clear ();
      mals_win->plot_colors.push_back (plot_colors[0]);
    }
  else
    {
      mals_win->plot_colors = plot_colors;
...
void
US_Hydrodyn_Mals_Movie::update_enables ()
{
  le_save->setEnabled (cb_save->isChecked ());
}

void
US_Hydrodyn_Mals_Movie::update_plot ()
{
  if (last_pos != pos || cb_show_gauss->isChecked () != last_show_gauss
      || cb_show_ref->isChecked () != last_show_ref)
    {
      ...
      if (cb_show_ref->isChecked ())
        {
          mals_win->plot_ref->show ();
        }
      else
        {
          mals_win->plot_ref->hide ();
        }
      mals_win->lb_files->item (mals_selected_files[pos])->setSelected (true);
      ...
      if (cb_show_gauss->isChecked ()
          && mals_win->f_is_time.count (
              mals_win->lb_files->item (mals_selected_files[pos])->text ())
          && mals_win->f_is_time[mals_win->lb_files
                                     ->item (mals_selected_files[pos])
                                     ->text ()])
        {
          mals_win->gauss_start ();
        }
      // else {
      // mals_win->plot_files();
      // }

      mals_win->plot_dist->replot ();
      mals_win->plot_ref->replot ();
      mals_win->plot_errors->replot ();
      lbl_state->setText (QString (
          us_tr ("%1: %2 of %3")
              .arg (us_tr (timer->isActive () ? "Running" : "Stopped"))
              .arg (pos + 1)
...
void
US_Hydrodyn_Mals_Movie::save_plot ()
{
  if (mals_win->plot_errors->isVisible ())
    {
      if (mals_win->plot_ref->isVisible ())
        {
          save_plot (mals_win->plot_dist, mals_win->plot_ref,
                     mals_win->plot_errors, le_save->text ());
        }
      else
        {
          save_plot (mals_win->plot_dist, mals_win->plot_errors,
                     le_save->text ());
        }
    }
  else
    {
      if (mals_win->plot_ref->isVisible ())
        ...
        {
          save_plot (mals_win->plot_dist, le_save->text ());
        }
    }
}

void
US_Hydrodyn_Mals_Movie::save_plot (QWidget *plot, QString tag)
{
  int mypos = pos;
...
void
US_Hydrodyn_Mals_Movie::join_maps (QPixmap &m1, QPixmap &m2)
{
  int m1h = m1.height ();
  int m2h = m2.height ();
...
void
US_Hydrodyn_Mals_Movie::save_plot (QWidget *plot, QWidget *plot2,
...
void
US_Hydrodyn_Mals_Movie::save_plot (QWidget *plot, QWidget *plot2, QString tag)
{
  int mypos = pos;
  QPixmap qPix = plot->grab ();
  QPixmap qPix2 = plot2->grab ();
...
void
US_Hydrodyn_Mals_Movie::save_plot (QPixmap &qPix, QString tag, int mypos)
{
  QPainter paint (&qPix);
  paint.setPen (Qt::blue);
  paint.setFont (QFont (USglobal->config_list.fontFamily,
                        USglobal->config_list.fontSize - 1, QFont::Bold));
  // paint.drawText( 5, 5, mals_win->lb_files->item( mals_selected_files[ mypos
  // ] )->text() );
  paint.drawText (
      qPix.rect (), Qt::AlignBottom | Qt::AlignLeft,
      mals_win->lb_files->item (mals_selected_files[mypos])->text ());
