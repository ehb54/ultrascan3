...
bool
US_Saxs_Util::read_control (QString controlfile)
{
  output_files.clear ();
  ...
  if (!f.open (QIODevice::ReadOnly))
    {
      errormsg = QString ("Error: %1 can not be opened.  Check permissions")
                     .arg (controlfile);
      return false;
    }
  ...
  if (controlfile.contains (QRegExp ("\\.(tgz|TGZ)$")))
    {
      ...
      cout << "last_written_name from gunzip: " << usg.last_written_name
           << endl;
      if (GZIP_OK != result)
        {
          errormsg = QString ("Error: %1 problem ungzipping (%2)")
                         .arg (controlfile)
      ...
      if (!f.open (QIODevice::ReadOnly))
        {
          errormsg
              = QString ("Error: %1 can not be opened.  Check permissions")
                    .arg (controlfile);
          return false;
        }
    }
  ...
  if (controlfile.contains (QRegExp ("\\.(tar|TAR)$")))
    {
      ...
      if (!qslt.size ())
        {
          errormsg = QString ("Error: tar archive %1 appears empty")
                         .arg (controlfile);
          return false;
        }
  ...
  while (!ts.atEnd ())
    {
      ...
      if (rx_arg_1.indexIn (option) != -1 && qsl.size () < 1)
        {
      ...
      if (rx_arg_2.indexIn (option) != -1 && qsl.size () < 2)
        {
          errormsg = QString ("Error reading %1 line %2 : Missing argument ")
                         .arg (controlfile)
                         .arg (line);
          ...
          if (!qfc.exists ())
            {
              errormsg
                  = QString (
                        "Error reading %1 line %2 : File %3 does not exist ")
                        .arg (controlfile)
                        .arg (line)
                        .arg (qsl[0]);
              return false;
          ...
          while (!ok && trial < retries)
            {
              ...
              if (!ok && trial < retries)
                {
                  cout << QString ("DAMMIN: failed, retrying %1 of %2")
                              .arg (trial)
                              .arg (retries);
#ifndef WIN32
          ...
          if (output_dmd_pdbs.size ())
            {
              ...
              for (unsigned int i = 0;
                   i < (unsigned int)output_dmd_pdbs.size (); i++)
                {
                  control_parameters["inputfile"] = output_dmd_pdbs[i];
                  control_parameters["outputfile"]
                      = QFileInfo (output_dmd_pdbs[i]).baseName ();
      ...
      if (option == "inputfile")
        {
          ...
          if (ext != "pdb")
            {
              errormsg = QString ("Error %1 line %2 : only PDB files "
          ...
          if (model_vector.size () > 1
            ...
            {
              cout << "Notice: an NMR style model was loaded, but "
                      "\"PDBAllModels\" was not selected, so only the first "
      ...
      if (option == "nsagainit")
        {
          // each of these will be a list of numbers, which we will use for
          // initialization if the size matches
          if (!qsl.size ())
            {
          ...
          if (!nsa_mpi || !myrank)
            {
              ...
              if (!output_files.size ())
                {
                  errormsg = QString ("Error %1 line %2 : no output files "
                                      "available to collect in tar file %3")
                                 .arg (controlfile)
                                 .arg (line)
                                 .arg (qsl[0]);
                  return false;
          ...
          if (us_udp_msg)
            {
              us_udp_msg->send_json (
                  { { "_progressmsg", "Assembling output" } });
            }
          if (!nsa_mpi || !myrank)
            {
#endif
              ...
              if (!output_files.size ())
                {
                  errormsg
                      = QString ("Error %1 line %2 : no output files "
                                 "available to collect in gzipped tar file %3")
                            .arg (controlfile)
                            .arg (line)
                            .arg (qsl[0]);
                  return false;
      ...
      if (option == "1drun" || option == "onedrun")
        {
          setup_saxs_options ();
          if (!read_pdb (control_parameters["inputfile"]))
            {
      ...
      if (option == "process")
        {
          if (experimental_grids.size () < 2)
          ...
          else
            {
              ...
              for (unsigned int i = 0;
                   i < (unsigned int)experimental_grids.size (); i++)
                {
                  unsigned ext = 0;
                  QString grid_tag_base
                      = "_g" + QFileInfo (experimental_grids[i]).baseName ();
                  QString grid_tag = grid_tag_base;
                  while (tag_names.count (grid_tag))
                    {
                      grid_tag
                          = QString ("%1-%2").arg (grid_tag_base).arg (++ext);
                    }
                  control_parameters["grid_tag"] = grid_tag;
                  if (!set_control_parameters_from_experiment_file (
                          experimental_grids[i]))
                    {
                      errormsg = QString ("Error %1 line %2 : %3")
                                     .arg (controlfile)
                                     .arg (line)
      ...
      if (option == "gridsearch")
        {
          if (experimental_grids.size () != 1)
            {
              errormsg = QString ("Error %1 line %2 : exactly one experiment "
                                  "grid needed")
                             .arg (controlfile)
                             .arg (line);
              return false;
          ...
          if (model_vector.size () != 1)
            {
              errormsg
                  = QString (
                        "Error %1 line %2 : single model inputfile required")
                        .arg (controlfile)
                        .arg (line);
              return false;
            }
          ...
          if (!atom_num)
            {
              errormsg = QString ("Error %1 line %2 : selected atom numbers "
                                  "are relative and start with 1")
                             .arg (controlfile)
                             .arg (line);
              return false;
            }
          ...
          if ((unsigned int)model_vector[0].molecule[0].atom.size ()
              < atom_num)
            {
              errormsg = QString ("Error %1 line %2 : model's 1st molecule "
                                  "has insufficient atoms!")
                             .arg (controlfile)
                             .arg (line);
              return false;
            }
                  ...
                  for (z = startz; z <= endz; z += delta)
                    {
                      ...
                      // ).arg( y ).arg( z ) << flush;
                      model_vector[0].model_id
                          = org_model
                            + QString ("p%1_%2_%3").arg (x).arg (y).arg (z);
                      if (!run_iqq ())
                        {
                      ...
                      for (unsigned int i = 0; i < sgp_last_q.size (); i++)
                        {
                          msd += (sgp_last_I[i] - sgp_exp_I[i])
                                 * (sgp_last_I[i] - sgp_exp_I[i]);
                        }
...
US_Saxs_Util::process_one_iqq ()
{
  ...
  if (output_dmd_pdbs.size ())
    {
      QString save_input_file = control_parameters["inputfile"];
      QString save_output_file = (control_parameters.count ("outputfile")
                                      ? control_parameters["outputfile"]
                                      : "");
...
bool
US_Saxs_Util::set_control_parameters_from_experiment_file (
    QString filename, bool load_without_interp)
{
  errormsg = "";
  ...
  if (ext == "csv")
    {
      ...
      if (qsl_headers.size () == 0)
        {
          errormsg = QString ("Error: the file %1 has no valid q grid lines")
                         .arg (filename);
          return false;
        }
      ...
      if (qsl_q.size () < 3)
        {
          errormsg
              = QString (
                    "Error: the file %1 q grid line has insufficient columns")
                    .arg (filename);
          return false;
        }
  ...
  else
    {
      ...
      if (q.size () < 2)
        {
          errormsg = QString ("Error: file %1 error trying to get q grid")
                         .arg (filename);
          return false;
        }
    }
  ...
  control_parameters["deltaq"]
      = QString ("%1").arg ((q[(q.size () - 1)] - q[0]) / (q.size () - 1));

  if (control_parameters["deltaq"].toDouble () <= 0e0)
    {
      errormsg = QString ("Error: the file %1 q grid apparent deltaq is zero "
                          "or negative %2")
                     .arg (filename)
                     .arg (control_parameters["deltaq"]);
      return false;
...
#endif
          cout << QString (
                      "Warning: the file %1 q grid appears to have a somewhat "
                      "irregular grid with an average spacing of %2\n"
                      "Warning: the experimental grid has a minimum spacing "
                      "of %3, max %4 and average %5\n"
                      "Warning: computations will occur on a regular grid "
                      "with an equal range and number of points\n")
                      .arg (filename)
                      .arg (control_parameters["deltaq"])
                      .arg (min_delta_q)
                      .arg (max_delta_q)
...

  {
    ...
    while (rq.size () && rq.back () > sgp_exp_q.back () + 1e-6)
      {
        cout << QString ("cropping point with value %1 which is larger than "
                         "%2 by %3\n")
                    .arg (rq.back ())
                    .arg (sgp_exp_q.back ())
                    .arg (rq.back () - sgp_exp_q.back ())
             << flush;
    ...
    if (cropped_pts)
      {
        cout << QString (
                    "Notice: cropped %1 points off originally computed grid\n")
                    .arg (cropped_pts);
      }
  }
  ...
  if (!interpolate_iqq_by_case (sgp_exp_q, org_I, org_e, rq, sgp_exp_I,
                                sgp_exp_e))
    {
      return false;
    }
...
void
US_Saxs_Util::validate_control_parameters_set_one (QStringList &checks,
                                                   QStringList &vals)
{
...
bool
US_Saxs_Util::validate_control_parameters (bool for_sgp)
{
  errormsg = "";
  noticemsg = "";
...
bool
US_Saxs_Util::create_tar_output (QString filename)
{
  errormsg = "";
  US_Tar ust;
  int result;
  ...
  if (result != TAR_OK)
    {
      errormsg
          = QString ("Error: Problem creating tar archive %1").arg (filename);
      return false;
    }
...
bool
US_Saxs_Util::create_tgz_output (QString filename)
{
  errormsg = "";
  US_Tar ust;
  int result;
  ...
  if (result != TAR_OK)
    {
      errormsg = QString ("Error: Problem creating tar archive %1. %2")
                     .arg (filename)
  ...
  if (result != GZIP_OK)
    {
      errormsg = QString ("Error: Problem gzipping tar archive %1. %2")
                     .arg (filename)
                     .arg (usg.explain (result));
      return false;
    }
...
QString
US_Saxs_Util::vector_double_to_csv (vector<double> &vd)
{
  QString result;
...
bool
US_Saxs_Util::write_output (QString model, vector<double> &q,
                            vector<double> &I)
{
  // cout << "write output\n";
  ...
  if (control_parameters.count ("output")
          ...
          || control_parameters["output"] == "dat"))
    {
      ...
      QString fsaxs_part_1_name
          = control_parameters["outputfile"] + QString ("_%1").arg (model);

      QString fsaxs_part_2_name
          = (control_parameters.count ("grid_tag")
                 ? control_parameters["grid_tag"]
                 : "")
            + (control_parameters.count ("tag") ? control_parameters["tag"]
                                                : "")
            + (control_parameters.count ("cuda_tag")
                   ? control_parameters["cuda_tag"]
                   : "")
            + iqq_suffix () + "." + control_parameters["output"];
          ...
          if (file_write_count.count (fsaxs_part_1_name + fsaxs_part_2_name))
            {
              ...
              fsaxs_name = QString ("%1_mp%2%3")
                               ...
                               .arg (file_write_count[fsaxs_part_1_name
                                                      + fsaxs_part_2_name])
      ...
      if (fsaxs)
        {
          ...
          fprintf (fsaxs, "%s", last_saxs_header.toLatin1 ().data ());
...
bool
US_Saxs_Util::flush_output_one ()
{
  if (us_log)
    {
      us_log->log ("flush output\n");
    }
  if (saxs_inputfile_for_csv.size () && control_parameters.count ("output")
      && control_parameters["output"] == "csv")
    {
      ...
      if (!multi_models_per_method && !external_programs_included
          && methods_used.size () > 1)
        {
          methods_used.clear ();
          ...
          for (unsigned int i = 0; i < saxs_inputfile_for_csv.size (); i++)
            {
              ...
              if (it->first == "mm" || it->first == saxs_method_for_csv[i])
                {
                  if (method_q_pieces.count (it->first))
                      ...
                      if (method_q_pieces[it->first].count (saxs_q_for_csv[i]))
                        {
                          if (us_log)
                            {
                              us_log->log (
                                  QString ("multiple pieces for %1 q(%2:%3)\n")
                                      .arg (it->first)
                                      .arg (saxs_q_for_csv[i][0])
                                      .arg (saxs_q_for_csv[i][saxs_q_for_csv[i]
                                                                  .size ()
                                                              - 1]));
                            }
                  ...
                  else
                    {
                      if (us_log)
                        {
                          us_log->log (
                              QString ("first piece for %1 q(%2:%3)\n")
                                  .arg (it->first)
                                  .arg (saxs_q_for_csv[i][0])
                                  .arg (
                                      saxs_q_for_csv[i]
                                                    [saxs_q_for_csv[i].size ()
                                                     - 1]));
                        }
      ...
      for (map<QString, unsigned int>::iterator it = methods_used.begin ();
           it != methods_used.end (); it++)
        {
          ...
          for (map<vector<double>, unsigned int>::iterator it2
               = method_q_pieces[it->first].begin ();
               it2 != method_q_pieces[it->first].end (); it2++)
            {

              QString fname_part_1 = control_parameters["outputfile"];

              QString fname_part_2
                  = (method_q_pieces[it->first].size () > 1
                         ? QString ("_mg%1").arg (
                               ++method_piece_pos[it->first])
                         : "")
                  ...
                  if (file_write_count.count (fname_part_1 + fname_part_2))
                    {
                      ...
                      fname = QString ("%1_mp%2%3")
                                  .arg (fname_part_1)
                                  .arg (file_write_count[fname_part_1
                                                         + fname_part_2])
                                  .arg (fname_part_2);
              ...
              if (any_for_this)
                {
                  ...
                  if (of)
                           ...
                           i < saxs_inputfile_for_csv.size (); i++)
                        {
                          ...
                          if ((it->first == "mm"
                              ...
                              && it2->first == saxs_q_for_csv[i])
                            {
                              if (!q_data_added)
                                {
                                  fprintf (
                                      of, "\"Name\",\"Type; q:\",%s,\"%s\"\n",
                                      vector_double_to_csv (saxs_q_for_csv[i])
                                          .toLatin1 ()
                                          .data (),
                              ...
                              fprintf (of, "\"%s\",\"%s\",%s\n",
                                       name.toLatin1 ().data (), "I(q)",
                                       vector_double_to_csv (saxs_I_for_csv[i])
                                           .toLatin1 ()
                                           .data ());
                            }
                        }
                    ...
                    {
                      errormsg
                          = QString ("Error: could not open %1 for writing")
                                .arg (fname);
                      return false;
                    }
                }
...
bool
US_Saxs_Util::write_timings (QString file, QString msg)
{
  if (timings.isEmpty ())
    {
      return true;
  ...
  timings = "";
  errormsg
      = QString ("Error: could not open timing output file %1").arg (out_file);
  return false;
}

bool
US_Saxs_Util::mwc (const vector<double> &q_org, const vector<double> &I,
                   double Rg, double /* sigRg */, double I0, double sigI0,
                   double mw_per_N, double &qm, double &Vc, double &Qr,
                   double &mwc, double &mwc_sd, QString &messages,
                   QString &notes)
{
  // use vcm.json
  ...
  if (q_org.size () != I.size ())
    {
      messages = QString ("I, q different length %1 %2")
                     .arg (q_org.size ())
                     .arg (I.size ());
      return false;
    }

  if (!vcm.count ("q") || !vcm.count ("a") || !vcm.count ("b")
      || !vcm.count ("c") || !vcm.count ("a:y2") || !vcm.count ("b:y2")
      || !vcm.count ("c:y2"))
    {
  ...
  if (!static_apply_natural_spline (vcm["q"], vcm["a"], vcm["a:y2"], qm, a,
                                    messagesa)
      ||

      !static_apply_natural_spline (vcm["q"], vcm["b"], vcm["b:y2"], qm, b,
                                    messagesb)
      ||

      !static_apply_natural_spline (vcm["q"], vcm["c"], vcm["c:y2"], qm, c,
                                    messagesc))
    {
  ...
  {
    double b2m4acp4clogi0sqiqdq
        = b * b - 4e0 * a * c + 4e0 * c * log (I0 / sqiqdq);

    mwc_sd = (sqrt (exp (sqrt (b2m4acp4clogi0sqiqdq) / c - b / c)
    ...
    // us_qdebug( QString( "mwc %1 mwc_sd %1 mwc_sd_org %1 rg %1 sigRg %1 I0 %1
    // sigI0 %1" )
    //         .arg( mwc ).arg( mwc_sd ).arg( mwc_sd_org ).arg( Rg ).arg( sigRg
    //         ).arg( I0 ).arg( sigI0 ) );
  }

  // us_qdebug( QString( "splined a %1 b %2 c %3 Vc %4 lVc %5 lN %6 N %7 mwc
  // %8" )
  //         .arg( a )
  //         .arg( b )
  //         .arg( c )
...
double
US_Saxs_Util::prob_of_streak (int n, int c)
{
  if (prob_of_streak_cache.count (n) && prob_of_streak_cache[n].count (c))
    {
...
float
US_Saxs_Util::prob_of_streak_f (int n, int c)
{
  if (prob_of_streak_cache_f.count (n) && prob_of_streak_cache_f[n].count (c))
    {
...
bool
US_Saxs_Util::cormap (const vector<double> &q,
                      ...
                      double &P)
{
  ...
  if (m < 2)
    {
      errormsg
          = QString (us_tr ("Error in cormap: a minimum of 2 intensity "
                            "vectors are required and only %1 were given"))
                .arg (m);
      return false;
    }
      ...
      if ((int)I[i].size () != qp)
        {
          errormsg
              = QString (
                    us_tr ("Error in cormap: the intensity vectors are of "
                           "different size (%1)  than the q vector size (%2)"))
                    .arg (I[i].size ())
                    .arg (q.size ());
          return false;
        }
      ...
      for (int l = 0; l < qp; ++l)
        {
          ...
          if (sigma[k][k] == 0e0)
            {
              // us_qdebug( QString( us_tr( "Notice in cormap: found zero sigma
              // at %1 %2" ) ).arg( k ).arg( k ) );
              altsigma = true;
            }
          if (sigma[l][l] == 0e0)
          ...
          if (altsigma)
            ...
            {
              rkl[k][l]
                  = sigma[k][l] / (sqrt (sigma[k][k]) * sqrt (sigma[l][l]));
            }
        }
    }
...

  {
    ...
    // us_qdebug( QString( "N %1 Loc %2 Size %3" ).arg( qp ).arg(
    // longest_start_q + 1 ).arg( contiguous_pts ) ); us_qdebug( QString( "N %1
    // Loc %2 Size %3 PV %4" ).arg( qp ).arg( longest_start_q + 1 ).arg(
    // contiguous_pts ).arg( prob_of_streak( qp, contiguous_pts ) ) );
  }
...
bool
US_Saxs_Util::average (vector<double> &q, vector<double> &I, vector<double> &e,
                       double &avg_q, double &avg_I, double &avg_e)
{
  ...
  errormsg = "";
  if (!q.size () || q.size () != I.size () || q.size () != e.size ())
    {
      errormsg = "US_Saxs_Util::average Error: incompatible input vector "
                 "sizes. they must be equal and at least length 1";
      return false;
    }
...
bool
US_Saxs_Util::bin (vector<double> &q, vector<double> &I, vector<double> &e,
                   ...
                   vector<double> &new_e, unsigned int points)
{
  ...
  if (!q.size () || q.size () != I.size () || q.size () != e.size ())
    {
      errormsg = "US_Saxs_Util::bin Error: incompatible input vector sizes. "
                 "they must be equal and at least length 1";
      return false;
    }
  ...
  for (int i = 0; i < (int)q.size (); i += points)
    {
      ...
      if (!average (this_q, this_I, this_e, this_new_q, this_new_I,
                    this_new_e))
        {
          return false;
        }
  ...
  // cout << US_Vector::qs_vector3( "final new vectors", new_q, new_I, new_e )
  // << endl;
