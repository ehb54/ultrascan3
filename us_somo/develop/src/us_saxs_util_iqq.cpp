...
bool US_Saxs_Util::read_control(QString controlfile) {
  ...
  if (controlfile.contains(QRegExp("\\.(tgz|TGZ)$"))) {
    ...
    if (!qd.rename(controlfile, dest)) {
      errormsg =
          QString("Error: renaming %1 to %2 ").arg(controlfile).arg(dest);
      return false;
    }
  ...
  if (controlfile.contains(QRegExp("\\.(tar|TAR)$"))) {
    ...
    if (!qslt.size()) {
      errormsg =
          QString("Error: tar archive %1 appears empty").arg(controlfile);
      return false;
    }
  ...
  while (!ts.atEnd()) {
    ...
    if (option == "hydrate") {
      if (output_dmd_pdbs.size()) {
        ...
        for (unsigned int i = 0; i < (unsigned int)output_dmd_pdbs.size();
             i++) {
    ...
    if (option == "inputfile") {
      ...
      if (ext != "pdb") {
        errormsg =
            QString(
                "Error %1 line %2 : only PDB files currently supported <%3>")
                .arg(controlfile)
                .arg(line)
                .arg(qsl[0]);
        return false;
    ...
    if (option == "taroutput") {
      ...
      if (!nsa_mpi || !myrank) {
        ...
        if (!output_files.size()) {
          errormsg = QString(
                         "Error %1 line %2 : no output files available to "
                         "collect in tar file %3")
                         .arg(controlfile)
                         .arg(line)
                         .arg(qsl[0]);
          return false;
    ...
    if (option == "process") {
      if (experimental_grids.size() < 2) {
        ...
        for (unsigned int i = 0; i < (unsigned int)experimental_grids.size();
             i++) {
          unsigned ext = 0;
    ...
    if (option == "gridsearch") {
      if (experimental_grids.size() != 1) {
        errormsg =
            QString("Error %1 line %2 : exactly one experiment grid needed")
                .arg(controlfile)
                .arg(line);
        return false;
      }
      ...
      if (!atom_num) {
        errormsg = QString(
                       "Error %1 line %2 : selected atom numbers are relative "
                       "and start with 1")
                       .arg(controlfile)
                       .arg(line);
        return false;
      ...
      if ((unsigned int)model_vector[0].molecule[0].atom.size() < atom_num) {
        errormsg = QString(
                       "Error %1 line %2 : model's 1st molecule has "
                       "insufficient atoms!")
                       .arg(controlfile)
                       .arg(line);
        return false;
        ...
        for (y = starty; y <= endy; y += delta) {
          for (z = startz; z <= endz; z += delta) {
            ...
            model_vector[0].model_id =
                org_model + QString("p%1_%2_%3").arg(x).arg(y).arg(z);
            if (!run_iqq()) {
...
bool US_Saxs_Util::set_control_parameters_from_experiment_file(
  ...
  if (ext == "csv") {
    ...
    if (qsl_headers.size() == 0) {
      errormsg =
          QString("Error: the file %1 has no valid q grid lines").arg(filename);
      return false;
    }
    QStringList qsl_q = (qsl_headers[0]).split(",");
    if (qsl_q.size() < 3) {
      errormsg =
          QString("Error: the file %1 q grid line has insufficient columns")
              .arg(filename);
      return false;
    }
    ...
    if (q.size() < 2) {
      errormsg =
          QString("Error: file %1 error trying to get q grid").arg(filename);
      return false;
    }
  }
  ...
  if (control_parameters["deltaq"].toDouble() <= 0e0) {
    errormsg =
        QString(
            "Error: the file %1 q grid apparent deltaq is zero or negative %2")
            .arg(filename)
            .arg(control_parameters["deltaq"]);
    return false;
  }
  ...
  if (max_delta_q - min_delta_q > 2e-7) {
    ...
    if (!nsa_mpi || !myrank) {
      ...
      cout << QString(
                  ...
                  "Warning: the experimental grid has a minimum spacing of %3, "
                  "max %4 and average %5\n"
                  "Warning: computations will occur on a regular grid with an "
                  "equal range and number of points\n")
                  .arg(filename)
                  .arg(control_parameters["deltaq"])
                  .arg(min_delta_q)
                  .arg(max_delta_q)
...

  {
    ...
    while (rq.size() && rq.back() > sgp_exp_q.back() + 1e-6) {
      cout
          << QString(
                 "cropping point with value %1 which is larger than %2 by %3\n")
                 .arg(rq.back())
                 .arg(sgp_exp_q.back())
...
bool US_Saxs_Util::write_output(QString model, vector<double> &q,
                                vector<double> &I) {
  // cout << "write output\n";
  ...
  if (control_parameters.count("output") &&
    ...
    if (fsaxs) {
      ...
      QString last_saxs_header = QString("").sprintf(
          ...
          US_Version.toLatin1().data(), REVISION, our_saxs_options.start_q,
      ...
      errormsg =
          "Error: Could not create SAXS curve file: " + fsaxs_name + "\n";
      return false;
    }
  }
  return true;
        ...
        if (it->first == "mm" || it->first == saxs_method_for_csv[i]) {
          if (method_q_pieces.count(it->first)) {
            if (method_q_pieces[it->first].count(saxs_q_for_csv[i])) {
              ...
              if (us_log) {
                us_log->log(
                    QString("additional pieces for %1 q(%2:%3)\n")
                        .arg(it->first)
                        .arg(saxs_q_for_csv[i][0])
                        .arg(saxs_q_for_csv[i][saxs_q_for_csv[i].size() - 1]));
              }
      ...
      for (map<vector<double>, unsigned int>::iterator it2 =
        ...
        if (any_for_this) {
          ...
          if (of) {
            ...
            errormsg =
                QString("Error: could not open %1 for writing").arg(fname);
            return false;
          }
        }
      }
...
bool US_Saxs_Util::mwc(const vector<double> &q_org, const vector<double> &I,
  ...
  if (q_org.size() != I.size()) {
    messages =
        QString("I, q different length %1 %2").arg(q_org.size()).arg(I.size());
    return false;
  }
...
bool US_Saxs_Util::cormap(const vector<double> &q,
  ...
  if (m < 2) {
    errormsg = QString(us_tr("Error in cormap: a minimum of 2 intensity "
  ...
  for (int i = 0; i < m; ++i) {
    if ((int)I[i].size() != qp) {
      errormsg =
          QString(us_tr("Error in cormap: the intensity vectors are of "
  ...
  for (int k = 0; k < qp; ++k) {
    ...
    for (int l = 0; l < qp; ++l) {
      bool altsigma = false;
      if (sigma[k][k] == 0e0) {
        // us_qdebug( QString( us_tr( "Notice in cormap: found zero sigma at %1
        // %2" ) ).arg( k ).arg( k ) );
        altsigma = true;
      }
      if (sigma[l][l] == 0e0) {
...
bool US_Saxs_Util::average(vector<double> &q, vector<double> &I,
                           vector<double> &e, double &avg_q, double &avg_I,
                           double &avg_e) {
  ...
  if (!q.size() || q.size() != I.size() || q.size() != e.size()) {
    errormsg =
        "US_Saxs_Util::average Error: incompatible input vector sizes. they "
        "must be equal and at least length 1";
    return false;
  }
...
bool US_Saxs_Util::bin(vector<double> &q, vector<double> &I, vector<double> &e,
  ...
  if (!q.size() || q.size() != I.size() || q.size() != e.size()) {
    errormsg =
        "US_Saxs_Util::bin Error: incompatible input vector sizes. they must "
        "be equal and at least length 1";
    return false;
  }
