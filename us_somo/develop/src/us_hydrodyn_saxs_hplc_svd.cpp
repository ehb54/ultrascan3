...
void US_Hydrodyn_Saxs_Hplc_Svd::setupGUI() {
  ...
  hide_widgets(editor_widgets,
               !ush_win->gparams.count("hplc_svd_editor_widgets") ||
                       ush_win->gparams["hplc_svd_editor_widgets"] == "false"
                   ? false
                   : true);
  hide_widgets(process_widgets,
               !ush_win->gparams.count("hplc_svd_process_widgets") ||
                       ush_win->gparams["hplc_svd_process_widgets"] == "false"
                   ? false
                   : true);
  mode_select(MODE_IQIT);
}

void US_Hydrodyn_Saxs_Hplc_Svd::cancel() { close(); }
...
void US_Hydrodyn_Saxs_Hplc_Svd::closeEvent(QCloseEvent *e) { e->accept(); }
...
bool US_Hydrodyn_Saxs_Hplc_Svd::plot_file(QString file, double &minx,
...
bool US_Hydrodyn_Saxs_Hplc_Svd::get_min_max(QString file, double &minx,
                                            double &maxx, double &miny,
                                            double &maxy, norms norm_mode) {
  map<QString, vector<double> > *f_Iuse = &f_Is;
...
void US_Hydrodyn_Saxs_Hplc_Svd::hide_widgets(vector<QWidget *> widgets,
                                             bool hide) {
...
void US_Hydrodyn_Saxs_Hplc_Svd::data_selection_changed() { update_enables(); }

void US_Hydrodyn_Saxs_Hplc_Svd::sv_selection_changed() { update_enables(); }
...
void US_Hydrodyn_Saxs_Hplc_Svd::add_i_of_q_or_t(QString source,
                                                QStringList files,
...
void US_Hydrodyn_Saxs_Hplc_Svd::set_plot_errors_rev() { update_plot_errors(); }
...
bool US_Hydrodyn_Saxs_Hplc_Svd::get_name_norm_mode(QString fullname,
                                                   QString &name,
                                                   norms &norm_mode) {
...
bool US_Hydrodyn_Saxs_Hplc_Svd::get_plot_files(QStringList &use_list,
...
vector<vector<int> > US_Hydrodyn_Saxs_Hplc_Svd::transpose(
    vector<vector<int> > &A) {
...
vector<vector<double> > US_Hydrodyn_Saxs_Hplc_Svd::transpose(
...
void US_Hydrodyn_Saxs_Hplc_Svd::vvd_smult(vector<vector<double> > &A,
                                          double x) {
  int rows = (int)A.size();
  if (!rows) {
...
vector<vector<double> > US_Hydrodyn_Saxs_Hplc_Svd::vivd_pwmult(
...
vector<vector<double> > US_Hydrodyn_Saxs_Hplc_Svd::dot(
...
void US_Hydrodyn_Saxs_Hplc_Svd::matrix_info(QString qs,
                                            vector<vector<double> > &A) {
  if (!A.size()) {
...
void US_Hydrodyn_Saxs_Hplc_Svd::set_mode_iqit() { mode_select(MODE_IQIT); }

void US_Hydrodyn_Saxs_Hplc_Svd::set_mode_svd() { mode_select(MODE_SVD); }

void US_Hydrodyn_Saxs_Hplc_Svd::set_mode_efa() { mode_select(MODE_EFA); }
...
bool US_Hydrodyn_Saxs_Hplc_Svd::convert_it_to_iq(QStringList files,
    ...
    if (!hplc_win->f_qs.count(files[i])) {
      ...
      if (!hplc_win->f_errors.count(files[i]) ||
        ...
        if (!hplc_win->is_nonzero_vector(hplc_win->f_errors[files[i]])) {
          ...
          for (unsigned int j = 0;
               j < (unsigned int)hplc_win->f_errors[files[i]].size(); j++) {
            if (!mode_testiq || (hplc_win->f_qs[files[i]][j] >= t_min &&
                                 hplc_win->f_qs[files[i]][j] <= t_max)) {
