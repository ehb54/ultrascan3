...
bool
US_Saxs_Util::c2check (QString a, QString b)
{
  errormsg = "";
  noticemsg = "";
  ...
  for (unsigned int i = 0; i < model_vector.size (); i++)
    {
      ...
      if (atomsa.size () != atomsb.size ())
        {
          errormsg = QString ("Error: chain %1 has %2 atoms and chain %3 has "
                              "%4 atoms inputfile %5")
                         .arg (a)
                         .arg (atomsa.size ())
                         .arg (b)
      ...
      if (!atomsa.size ())
        {
          errormsg
              = QString ("Error: chain %1 and %2 have no atoms inputfile %3")
                    .arg (a)
                    .arg (atomsa.size ())
                    .arg (b)
                    .arg (atomsb.size ())
      ...
      for (int k = 0; k < (int)atomsa.size (); ++k)
        {
          if (atomsa[k].atom_name != atomsb[k].atom_name
            ...
            {
              errormsg = QString ("Error: name mismatch %1:%2:%3 vs %4:%5:%6 "
                                  "inputfile %7")
                             .arg (a)
