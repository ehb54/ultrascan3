  ...
  if (ftype == "bead_model") {
    ...
    if (f.open(QIODevice::ReadOnly)) {
      ...
      editor->setCurrentFont(new_font);
      {
        ...
        if (ssaxs.size() && !saxs_options.iq_global_avg_for_bead_models &&
          ...
          editor_msg(
              "red",
              QString(
                  us_tr("Overriding setting to use global structure factors "
                        "since bead model doesn't contain the correct number "
                        "of structure factors (%1) for the beads (%2)"))
                  .arg(bsaxs.size())
                  .arg(bead_model.size()));
        }
        ...
        if (ssaxs.size() &&
          ...
          editor_msg(
              "dark blue",
              QString(
                  us_tr("Found %1 saxs coefficient lines in bead model file\n"))
                  .arg(ssaxs.size()));
          if (ssaxs.size() > 2) {
            editor_msg(
                "red",
                QString(us_tr("Error: saxs coefficients found in file, but "
                              "incorrect # of lines %1 vs 2 expected"))
                    .arg(ssaxs.size()));
          } else {
            saxs tmp_saxs;
            QStringList qsl =
                (ssaxs[0]).split(QRegExp("\\s+"), Qt::SkipEmptyParts);
            if (qsl.size() != 12) {
              editor_msg(
                  "red",
                  QString(us_tr("Error: saxs coefficients found in file, but "
                                "incorrect # of tokens %1 vs 12 required"))
                      .arg(qsl.size()));
            } else {
              // editor_msg( "dark blue",  us_tr( "Four term saxs coefficients
              // found\n" ) );
              ...
              if (ssaxs.size() == 2) {
                ...
                if (qsl.size() != 14) {
                  editor_msg(
                      "red",
                      QString(
                          us_tr("Error: saxs coefficients found in file, but "
                                "incorrect # of tokens %1 vs 14 required"))
                          .arg(qsl.size()));
                } else {
                  // editor_msg( "dark blue",  us_tr( "Five term saxs
                  // coefficients found\n" ) );
        ...
        if (bsaxs.size() && !saxs_options.iq_global_avg_for_bead_models) {
          editor_msg(
              "blue",
              us_tr("Notice: using individual bead structure factors\n"));
          sf_bead_factors.resize(bsaxs.size());
          bool do_bsaxsv = false;
          if (bsaxsv.size()) {
            if (bsaxsv.size() * 2 == bsaxs.size()) {
              editor_msg("blue", us_tr("Notice: found correct # of variable "
                                       "length structure factors\n"));
              do_bsaxsv = true;
            } else {
              editor_msg(
                  "red",
                  us_tr("Notice: found incorrect # of variable length "
                        "structure factors, variable length not loaded\n"));
            }
          }
          ...
          for (unsigned int j = 0; j < (unsigned int)bsaxs.size(); j += 2) {
            saxs tmp_saxs;
            QStringList qsl =
                (bsaxs[j]).split(QRegExp("\\s+"), Qt::SkipEmptyParts);
            if (qsl.size() != 12) {
              editor_msg(
                  "red",
                  QString(us_tr("Error: saxs coefficients found in file, but "
              ...
              if ((unsigned int)bsaxs.size() > j + 1) {
                ...
                if (tmp_saxs.saxs_name != qsl[1].toUpper()) {
                  editor_msg("red", us_tr("Error: bead saxs coefficients bead "
                                          "number inconsistancy"));
                } else {
                  if (qsl.size() != 14) {
                    editor_msg(
                        "red",
              ...
              if (do_bsaxsv) {
                qsl =
                    (bsaxsv[j / 2]).split(QRegExp("\\s+"), Qt::SkipEmptyParts);
                QTextStream(stdout)
          ...
          if (bead_model.size() > sf_bead_factors.size()) {
            for (unsigned int i = sf_bead_factors.size(); i < bead_model.size();
                 i++) {
              bead_model[i].saxs_data =
          ...
          editor_msg(
              "dark blue",
              us_tr("Bead model structure factors saved for reapplication"));
        }
      ...
      if (bead_count != (int)bead_model.size()) {
        editor->append(QString("Error: bead count %1 does not match # of beads "
                               "read from file (%2) \n")
                           .arg(bead_count)
                           .arg(bead_model.size()));
        return -1;
  ...
  if (ftype == "pdb")  // DAMMIN;DAMMIF;DAMAVER
  {
    if (saxs_options.compute_saxs_coeff_for_bead_models) {
      if (!saxs_util->saxs_map.count(saxs_options.dummy_saxs_name)) {
        ...
        editor_msg(
            ...
            QString(
                us_tr(
                    "Notice: Loading dummy atoms with saxs coefficients '%1'"))
                .arg(saxs_options.dummy_saxs_name));
      }
    }
    ...
    if (f.open(QIODevice::ReadOnly)) {
      ...
      QRegExp rx_unit("^REMARK\\s+Units exponent(\\s*:|)\\s+(\\S+)",
                      Qt::CaseInsensitive);
      QRegExp rx_mult("^REMARK\\s+Units conversion factor(\\s*:|)\\s+(\\S+)",
                      Qt::CaseInsensitive);
      ...
      QStringList qsl;
      {
        ...
        do {
          ...
          if (rx_psv.indexIn(qs) != -1) {
            ...
            editor_msg("blue",
                       QString(us_tr("Found PSV %1 in PDB")).arg(loaded_psv));
          }
          if (rx_mw.indexIn(qs) != -1) {
            loaded_mw = rx_mw.cap(2).toDouble();
            editor_msg("blue",
                       QString(us_tr("Found MW %1 in PDB")).arg(loaded_mw));
          }
          if (rx_unit.indexIn(qs) != -1) {
            loaded_unit = rx_unit.cap(2).toUInt();
            editor_msg("blue", QString(us_tr("Found Units exponent %1 in PDB"))
                                   .arg(loaded_unit));
            if (loaded_unit == 10) {
      ...
      if (loaded_mult && !loaded_unit && saxs_options.dummy_atom_pdbs_in_nm) {
        editor_msg(
            "blue",
            us_tr(
                "Notice: Unit conversion factor in PDB forces Angstrom units"));
        saxs_options.dummy_atom_pdbs_in_nm = false;
      }
      ...
      if (!damaver && !dammin && !dammif && !damfilt && !damfilt5 && !em2dam) {
        editor->append(
            "Error in DAMMIN/DAMMIF file: couldn't find 'Dummy atoms in output "
            "phase' or 'Number of particle atoms' of 'Number of atoms "
            "written'\n");
        f.close();
        return 1;
      }
      ...
      if (rx.indexIn(last_atom)) {
        editor->append(
            "Couldn't find a last atom number in the DAMMIN/DAMMIF/DAMAVER "
            "file\n");
        f.close();
        return 1;
      }
      ...
      if (dammin || dammif) {
        do {
          tmp = ts.readLine();
          ++linepos;
        } while (!ts.atEnd() && !tmp.contains("Dummy atoms in output phase") &&
      ...
      if (dammin) {
        ...
        if (rx.indexIn(tmp) == -1) {
          editor->append(
              "Error in DAMMIN file: couldn't find number of atoms in 'Number "
              "of particle atoms' line\n");
          f.close();
          return 1;
        }
        ...
        if (ts.atEnd()) {
          editor->append(
              "Error in DAMMIN file: couldn't find 'DAM packing radius'\n");
          f.close();
          return 1;
        }
        ...
        if (rx.indexIn(tmp) == -1) {
          editor->append(
              "Error in DAMMIN file: couldn't find radius in 'Dummy atom "
              "radius' line\n");
          f.close();
          return 1;
        }
      ...
      if (dammif) {
        ...
        if (rx.indexIn(tmp) == -1) {
          editor->append(
              "Error in DAMMIF file: couldn't find number of atoms in 'Dummy "
              "atoms in output phase' line\n");
          f.close();
          return 1;
        }
        bead_count = rx.cap(1).toInt();
        ...
        if (ts.atEnd()) {
          editor->append(
              "Error in DAMMIF file: couldn't find 'Dummy atom radius'\n");
          f.close();
          return 1;
        }
        ...
        if (rx.indexIn(tmp) == -1) {
          editor->append(
              "Error in DAMMIF file: couldn't find radius in 'Dummy atom "
              "radius' line\n");
          f.close();
          return 1;
        }
      ...
      if (damaver) {
        ...
        if (ts.atEnd()) {
          editor->append(
              "Error in DAMAVER file: couldn't find 'DAM packing radius'\n");
          return 1;
        }
        ...
        if (rx.indexIn(tmp) == -1) {
          editor->append(
              "Error in DAMAVER file: couldn't find radius in 'DAM packing "
              "radius' line\n");
          return 1;
        }
        radius = rx.cap(1).toFloat();
      ...
      if (damfilt || damfilt5) {
        ...
        do {
          tmp = ts.readLine();
          ++linepos;
        } while (!ts.atEnd() && !tmp.toLower().contains("atomic radius"));

        if (ts.atEnd()) {
          editor->append(
              "Error in DAMFILT file: couldn't find 'Atomic radius'\n");
          return 1;
        }
        ...
        if (rx.indexIn(tmp.toLower()) == -1) {
          editor->append(
              "Error in DAMFILT file: couldn't find radius in 'Atomic radius' "
              "line\n");
          return 1;
        }
        radius = rx.cap(1).toFloat();
      ...
      if (em2dam) {
        ...
        do {
          tmp = ts.readLine();
          ++linepos;
        } while (!ts.atEnd() && !tmp.toLower().contains("dummy atom radius"));

        if (ts.atEnd()) {
          editor->append(
              "Error in EM2DAM file: couldn't find 'Dummy atom radius'\n");
          return 1;
        }
        ...
        if (rx.indexIn(tmp.toLower()) == -1) {
          editor->append(
              "Error EM2DAM DAMFILT file: couldn't find radius in 'Dummy atom "
              "radius' line\n");
          return 1;
        }
        radius = rx.cap(1).toFloat();
      ...
      editor->append(QString("DAMMIN/DAMMIF/DAMAVER/DAMFILT/EM2DAM model atom "
                             "radius %1 Angstrom\n")
                         .arg(radius));
      ...
      QString msg =
          QString(us_tr(" Enter values for vbar and total molecular weight: "));
      ...
      if (dammix_remember_psv.count(filename)) {
        ...
        found = true;
        do_write_bead_model = false;  // we should have already created it if we
        ...
        if (!dammix_match_remember_psv.empty()) {
          ...
          for (map<QString, float>::iterator it =
            ...
            if (filename.contains(it->first)) {
              ...
              do_write_bead_model =
                  dammix_match_remember_write_bead_model[it->first];
              found = true;
              break;
            }
          }
      ...
      if (found) {
        editor->append(
            QString("Recalled: psv %1, mw %2 Daltons\n").arg(psv).arg(mw));
      } else {
        US_Hydrodyn_Dammin_Opts *hdo =
            new US_Hydrodyn_Dammin_Opts(msg, &psv, &mw, &do_write_bead_model,
                                        &remember, &use_partial, &partial);
      ...
      if (do_bead_saxs_assign && !sf_bead_factors.size()) {
        editor_msg("red", us_tr("Warning: application of preloaded structure "
                                "factors requested but no factors are loaded"));
        do_bead_saxs_assign = false;
      }
      if (do_bead_saxs_assign && bead_count % sf_bead_factors.size()) {
        editor_msg(
            "red",
            QString(us_tr("Warning: application of preloaded structure factors "
                          "requested but the number of factors loaded %1 does "
                          "not divide the number of beads %2"))
      ...
      for (unsigned int i = 0; i < model_count; i++) {
        editor_msg("gray",
                   QString(us_tr("Loading from model %1")).arg(model_names[i]));
        int beads_loaded = 0;
        bead_model.clear();
        while (!ts.atEnd() && beads_loaded < bead_count) {
          ++linepos;
          ++beads_loaded;
          // ATOM     20  CA  ASP A   1      -8.226   5.986 215.196   1.0  20.0
          // 0 2 201
          ...
          if (do_bead_saxs_assign) {
            tmp_atom.saxs_data =
                sf_bead_factors[bead_model.size() % sf_bead_factors.size()];
      ...
      for (unsigned int i = 0; i < model_count; i++) {
        editor_msg("gray", QString(us_tr("checking overlap for Model %1"))
                               .arg(model_names[i]));
