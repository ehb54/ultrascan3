...
// (this) us_hydrodyn_beads_load.cpp contains code for reading the bead model
// file
...
int
US_Hydrodyn::read_bead_model (QString filename, bool &only_overlap)
{
  ...
  if (ftype == "bead_model")
    {
      // check for vdw data
      {
        ...
        if (ufu.read (filename, qsl)
            && qsl.filter ("vdW model parameters:").size ())
          {
            QStringList qsl_check = qsl.filter (QRegularExpression (
                "^  (Hydrate probe radius|Hydrate threshold|Theoretical "
                "waters|Exposed|Theoretical waters exposed).*:"));
            if (qsl_check.size () == 5)
              {
                ...
                QTextStream (stdout)
                    ...
                    << "\n"
                    << "read_vdw_theo_waters_exposed : "
                    << read_vdw_theo_waters_exposed << "\n"
                    << "read_asa_hydrate_probe_radius : "
                    << read_asa_hydrate_probe_radius << "\n"
                    << "read_asa_hydrate_threshold : "
                    << read_asa_hydrate_threshold << "\n"
                    << "-----------\n";
              }
            else
      ...
      if (f.open (QIODevice::ReadOnly))
        {
          bool so_ovlp
              = QFileInfo (f).completeBaseName ().contains ("so_ovlp");
          ...
          if (read_vdw && bead_model.size ())
            {
              ...
              bead_model[0].vdw_theo_waters_exposed
                  = read_vdw_theo_waters_exposed;
              bead_model[0].asa_hydrate_probe_radius
                  = read_asa_hydrate_probe_radius;
          ...
          editor->setCurrentFont (new_font);
          {
            ...
            if (ssaxs.size () && !saxs_options.iq_global_avg_for_bead_models
                && bsaxs.size () && (bead_model.size () % (bsaxs.size () / 2)))
              {
                editor_msg (
                    "red",
                    QString (
                        us_tr ("Overriding setting to use global structure "
                               "factors since bead model doesn't contain the "
                               "correct number of structure factors (%1) for "
                               "the beads (%2)"))
                        .arg (bsaxs.size ())
                        .arg (bead_model.size ()));
              }
            ...
            if (ssaxs.size ()
                && (saxs_options.iq_global_avg_for_bead_models
                    || (bsaxs.size ()
                        && (bead_model.size () % (bsaxs.size () / 2)))))
              {
                editor_msg ("dark blue",
                            QString (us_tr ("Found %1 saxs coefficient lines "
                                            "in bead model file\n"))
                                .arg (ssaxs.size ()));
                if (ssaxs.size () > 2)
                  {
                    editor_msg (
                        ...
                        QString (
                            us_tr (
                                "Error: saxs coefficients found in file, but "
                                "incorrect # of lines %1 vs 2 expected"))
                            .arg (ssaxs.size ()));
                  }
                else
                  {
                    saxs tmp_saxs;
                    QStringList qsl = (ssaxs[0]).split (QRegExp ("\\s+"),
                                                        Qt::SkipEmptyParts);
                    if (qsl.size () != 12)
                      {
                        editor_msg (
                            "red",
                            QString (us_tr ("Error: saxs coefficients found "
                                            "in file, but incorrect # of "
                                            "tokens %1 vs 12 required"))
                                .arg (qsl.size ()));
                      }
                    else
                      {
                        // editor_msg( "dark blue",  us_tr( "Four term saxs
                        // coefficients found\n" ) );
                            ...
                            if (qsl.size () != 14)
                              {
                                editor_msg (
                                    ...
                                    QString (
                                        us_tr (
                                            "Error: saxs coefficients found "
                                            "in file, but incorrect # of "
                                            "tokens %1 vs 14 required"))
                                        .arg (qsl.size ()));
                              }
                            else
                              {
                                // editor_msg( "dark blue",  us_tr( "Five term
                                // saxs coefficients found\n" ) );
                        ...
                        if (saxs_plot_widget)
                          {
                            ...
                            saxs_plot_window->saxs_map[tmp_saxs.saxs_name]
                                = tmp_saxs;
                          }

                        if (saxs_options.compute_saxs_coeff_for_bead_models
                            && saxs_util->saxs_map.count (
                            ...
                            if (!saxs_util->saxs_map.count (
                            ...
                            else
                              {
                                editor_msg (
                                    "blue",
                                    QString (
                                        us_tr ("Notice: Loading dummy atoms "
                                               "with saxs coefficients '%1'"))
                                        .arg (saxs_options.dummy_saxs_name));
                              }
                            for (unsigned int i = 0;
                                 i < (unsigned int)bead_model.size (); i++)
                              {
            ...
            if (bsaxs.size () && !saxs_options.iq_global_avg_for_bead_models)
              {
                editor_msg (
                    "blue",
                    us_tr (
                        "Notice: using individual bead structure factors\n"));
                    ...
                    if (bsaxsv.size () * 2 == bsaxs.size ())
                      {
                        editor_msg (
                            "blue",
                            us_tr ("Notice: found correct # of variable "
                      ...
                      {
                        editor_msg (
                            "red", us_tr ("Notice: found incorrect # of "
                                          "variable length structure factors, "
                                          "variable length not loaded\n"));
                      }
                  }

                for (unsigned int j = 0; j < (unsigned int)bsaxs.size ();
                     j += 2)
                  {
                    saxs tmp_saxs;
                    QStringList qsl = (bsaxs[j]).split (QRegExp ("\\s+"),
                                                        Qt::SkipEmptyParts);
                    if (qsl.size () != 12)
                      {
                        editor_msg (
                            ...
                            if (tmp_saxs.saxs_name != qsl[1].toUpper ())
                              {
                                editor_msg (
                                    "red",
                                    us_tr ("Error: bead saxs coefficients "
                                           "bead number inconsistancy"));
                              }
                            else
                              {
                                if (qsl.size () != 14)
                                  {
                                    editor_msg (
                                        "red",
                                        QString (
                                            us_tr ("Error: saxs coefficients "
                                                   "found in file, but "
                                                   "incorrect # of tokens %1 "
                                                   "vs 14 required"))
                                            .arg (qsl.size ()));
                                  }
                                else
                                  {
                                    // editor_msg( "dark blue",  us_tr( "Five
                                    // term saxs coefficients found\n" ) );
                                    for (unsigned int i = 0; i < 5; i++)
                                      {
                                        tmp_saxs.a5[i]
                                            = qsl[2 + i * 2].toFloat ();
                        ...
                        if (do_bsaxsv)
                          {
                            qsl = (bsaxsv[j / 2])
                                      .split (QRegExp ("\\s+"),
                                              Qt::SkipEmptyParts);
                            QTextStream (stdout)
                                << QString ("loading: bvsaxs qsl size %1\n")
                                       .arg (qsl.size ());
                ...
                if (bead_model.size () > sf_bead_factors.size ())
                  {
                    for (unsigned int i = sf_bead_factors.size ();
                         i < bead_model.size (); i++)
                      {
                        bead_model[i].saxs_data
                            = sf_bead_factors[i % sf_bead_factors.size ()];
                        bead_model[i].saxs_name
                            = bead_model[i].saxs_data.saxs_name;
                        bead_model[i].saxs_excl_vol
                            = bead_model[i].saxs_data.volume;
                      }
                  }

                editor_msg ("dark blue", us_tr ("Bead model structure factors "
                                                "saved for reapplication"));
              }
            ...
            if (!units_loaded)
              {
                editor_msg (
                    "red",
                    us_tr ("\nNote: The model scale (nm, angstrom, etc.) was "
                           "not found in this bead model file.\n"
                           "Please make sure the scale is correct before "
                           "computing hydrodynamic parameters.\n"));
                editor_msg (
                    "dark red",
                    QString ("Current model scale (10^-x m) (10 = Angstrom, 9 "
          ...
          if (bead_count != (int)bead_model.size ())
            {
              editor->append (QString ("Error: bead count %1 does not match # "
                                       "of beads read from file (%2) \n")
                                  .arg (bead_count)
                                  .arg (bead_model.size ()));
              return -1;
            }
          ...
          //          bead_model_suffix) : "") + DOTSOMO, &bead_model, true);
          lb_model->clear ();
          ...
          if (so_ovlp)
            ...
            {
              int overlap_check_results = overlap_check (
                  true, true, true,
      ...
      if (saxs_options.compute_saxs_coeff_for_bead_models)
        {
          if (!saxs_util->saxs_map.count (saxs_options.dummy_saxs_name))
            ...
            {
              editor_msg ("blue",
                          QString (us_tr ("Notice: Loading dummy atoms with "
                                          "saxs coefficients '%1'"))
                              .arg (saxs_options.dummy_saxs_name));
            }
        }
      ...
      if (f.open (QIODevice::ReadOnly))
        {
          ...
          QRegExp rx_psv ("^REMARK\\s+PSV(\\s*:|)\\s+(\\S+)",
                          Qt::CaseInsensitive);
          QRegExp rx_mw ("^REMARK\\s+MW(\\s*:|)\\s+(\\S+)",
                         Qt::CaseInsensitive);
          QRegExp rx_unit ("^REMARK\\s+Units exponent(\\s*:|)\\s+(\\S+)",
                           Qt::CaseInsensitive);
          QRegExp rx_mult (
              "^REMARK\\s+Units conversion factor(\\s*:|)\\s+(\\S+)",
              Qt::CaseInsensitive);
                ...
                if (rx_psv.indexIn (qs) != -1)
                  {
                    ...
                    editor_msg ("blue", QString (us_tr ("Found PSV %1 in PDB"))
                                            .arg (loaded_psv));
                  }
                if (rx_mw.indexIn (qs) != -1)
                  {
                    loaded_mw = rx_mw.cap (2).toDouble ();
                    editor_msg ("blue", QString (us_tr ("Found MW %1 in PDB"))
                                            .arg (loaded_mw));
                  }
                if (rx_unit.indexIn (qs) != -1)
                  {
                    ...
                    editor_msg (
                        "blue",
                        QString (us_tr ("Found Units exponent %1 in PDB"))
                            .arg (loaded_unit));
                    if (loaded_unit == 10)
                      {
                ...
                if (rx_mult.indexIn (qs) != -1)
                  {
                    ...
                    editor_msg (
                        "blue",
                        QString (us_tr ("Found Units conversion %1 in PDB"))
                            .arg (unit_mult));
                  }
              }
            while (!ts.atEnd ());
          ...
          if (loaded_mult && !loaded_unit
              && saxs_options.dummy_atom_pdbs_in_nm)
            {
              editor_msg ("blue", us_tr ("Notice: Unit conversion factor in "
                                         "PDB forces Angstrom units"));
              saxs_options.dummy_atom_pdbs_in_nm = false;
            }
          ...
          if (saxs_options.dummy_atom_pdbs_in_nm)
            {
              editor_msg (
                  "blue",
                  us_tr ("Notice: This PDB is being loaded in NM units"));
            }
          ...
          bool dammif
              = (qsl.filter ("Dummy atoms in output phase").count () > 0);
          ...
          if (!damaver && !dammin && !dammif && !damfilt && !damfilt5
              && !em2dam)
            {
              editor->append (
                  "Error in DAMMIN/DAMMIF file: couldn't find 'Dummy atoms in "
                  "output phase' or 'Number of particle atoms' of 'Number of "
                  "atoms written'\n");
              f.close ();
              return 1;
            }
          ...
          if (!qsl_atom.empty ())
            ...
            {
              editor->append (
                  "No ATOM lines found in DAMMIN/DAMMIF/DAMAVER file\n");
              f.close ();
              return 1;
            }
          ...
          if (rx.indexIn (last_atom))
            {
              editor->append ("Couldn't find a last atom number in the "
                              "DAMMIN/DAMMIF/DAMAVER file\n");
              f.close ();
              return 1;
            }
          ...
          if (dammin)
            {
              puts ("dammin");
              rx.setPattern (
                  "Number of particle atoms \\.*\\s*:\\s*(\\d+)\\s*");
              if (rx.indexIn (tmp) == -1)
                {
                  editor->append (
                      "Error in DAMMIN file: couldn't find number of atoms in "
                      "'Number of particle atoms' line\n");
                  f.close ();
                  return 1;
                }
              bead_count = rx.cap (1).toInt ();
              // editor->append(QString("DAMMIN model has %1
              // beads\n").arg(bead_count));
              ...
              while (!ts.atEnd () && !tmp.contains ("DAM packing radius"));

              if (ts.atEnd ())
                {
                  editor->append ("Error in DAMMIN file: couldn't find 'DAM "
                                  "packing radius'\n");
                  f.close ();
                  return 1;
                }
              ...
              rx.setPattern (
                  "DAM packing radius \\.*\\s*:\\s*(\\d+\\.\\d+)\\s*");

              if (rx.indexIn (tmp) == -1)
                {
                  editor->append ("Error in DAMMIN file: couldn't find radius "
                                  "in 'Dummy atom radius' line\n");
                  f.close ();
                  return 1;
                }
          ...
          if (dammif)
            {
              ...
              if (rx.indexIn (tmp) == -1)
                {
                  editor->append (
                      "Error in DAMMIF file: couldn't find number of atoms in "
                      "'Dummy atoms in output phase' line\n");
                  f.close ();
                  return 1;
                }
              bead_count = rx.cap (1).toInt ();
              // editor->append(QString("DAMMIF model has %1
              // beads\n").arg(bead_count));
              ...
              while (!ts.atEnd () && !tmp.contains ("Dummy atom radius"));

              if (ts.atEnd ())
                {
                  editor->append ("Error in DAMMIF file: couldn't find 'Dummy "
                                  "atom radius'\n");
                  f.close ();
                  return 1;
                }
              ...
              if (rx.indexIn (tmp) == -1)
                {
                  editor->append ("Error in DAMMIF file: couldn't find radius "
                                  "in 'Dummy atom radius' line\n");
                  f.close ();
                  return 1;
                }
          ...
          if (damaver)
            {
              ...
              if (ts.atEnd ())
                {
                  editor->append ("Error in DAMAVER file: couldn't find 'DAM "
                                  "packing radius'\n");
                  return 1;
                }
              ...
              if (rx.indexIn (tmp) == -1)
                {
                  editor->append ("Error in DAMAVER file: couldn't find "
                                  "radius in 'DAM packing radius' line\n");
                  return 1;
                }
              radius = rx.cap (1).toFloat ();
            }
          ...
          if (damfilt || damfilt5)
            {
              ...
              if (ts.atEnd ())
                {
                  editor->append ("Error in DAMFILT file: couldn't find "
                                  "'Atomic radius'\n");
                  return 1;
                }
          ...
          if (em2dam)
            {
              ...
              if (ts.atEnd ())
                {
                  editor->append ("Error in EM2DAM file: couldn't find 'Dummy "
                                  "atom radius'\n");
                  return 1;
                }

              rx.setPattern (
                  "dummy atom radius \\.*\\s*:\\s*(\\d+\\.\\d+)\\s*");

              if (rx.indexIn (tmp.toLower ()) == -1)
                {
                  editor->append ("Error EM2DAM DAMFILT file: couldn't find "
                                  "radius in 'Dummy atom radius' line\n");
                  return 1;
                }
              radius = rx.cap (1).toFloat ();
            }
          ...
          editor->append (QString ("DAMMIN/DAMMIF/DAMAVER/DAMFILT/EM2DAM "
                                   "model atom radius %1 Angstrom\n")
                              .arg (radius));
          ...
          QString msg = QString (
              us_tr (" Enter values for vbar and total molecular weight: "));
          ...
          if (dammix_remember_psv.count (filename))
            {
              ...
              found = true;
              do_write_bead_model = false; // we should have already created it
                                           // if we wanted the first time
            }
          else
            {
              if (!dammix_match_remember_psv.empty ())
                {
                  ...
                  for (map<QString, float>::iterator it
                       = dammix_match_remember_psv.begin ();
                       it != dammix_match_remember_psv.end (); it++)
                    {
                      ...
                      if (filename.contains (it->first))
                        {
                          ...
                          do_write_bead_model
                              = dammix_match_remember_write_bead_model
                                  [it->first];
                          found = true;
                          break;
                        }
          ...
          if (found)
          ...
          else
            {
              US_Hydrodyn_Dammin_Opts *hdo = new US_Hydrodyn_Dammin_Opts (
                  msg, &psv, &mw, &do_write_bead_model, &remember,
                  &use_partial, &partial);
              ...
              if (use_partial)
                {
                  ...
                  dammix_match_remember_write_bead_model[partial]
                      = do_write_bead_model;
                }
            }
          ...
          editor->append (
              "Setting overlap tolerance to .002 for DAMMIN/DAMMIF models\n");
          ...
          if (do_bead_saxs_assign && !sf_bead_factors.size ())
            {
              editor_msg (
                  "red",
                  us_tr ("Warning: application of preloaded structure factors "
                         "requested but no factors are loaded"));
          ...
          if (do_bead_saxs_assign && bead_count % sf_bead_factors.size ())
            {
              editor_msg (
                  ...
                  QString (
                      us_tr (
                          "Warning: application of preloaded structure "
                          "factors requested but the number of factors loaded "
                          "%1 does not divide the number of beads %2"))
          ...
          if (do_bead_saxs_assign)
            {
              editor_msg ("blue",
                          us_tr ("applying preloaded structure factors"));
            }

          while ((unsigned int)model_names.size () < model_count)
            {
              model_names.push_back (
                  QString ("%1").arg (model_names.size () + 1));
            }
          for (unsigned int i = 0; i < model_count; i++)
            {
              editor_msg ("gray", QString (us_tr ("Loading from model %1"))
                                      .arg (model_names[i]));
              int beads_loaded = 0;
              bead_model.clear ();
              while (!ts.atEnd () && beads_loaded < bead_count)
                {
                  ++linepos;
                  ++beads_loaded;
                  // ATOM     20  CA  ASP A   1      -8.226   5.986
                  // 215.196   1.0  20.0 0 2 201
                      ...
                      if (str.mid (0, 4) != "ATOM")
                        {
                          ...
                          while (!ts.atEnd () && str.mid (0, 4) != "ATOM");
                        }
                      if (ts.atEnd () && str.mid (0, 4) != "ATOM")
                        {
                          editor->append (
                              QString ("\nError in line %1. Expected 'ATOM' "
                                       "before end of file\n")
                                  .arg (linepos));
                          return linepos;
                        }
                  ...
                  for (unsigned int i = 0; i < 3; i++)
                    {
                      tmp_atom.bead_coordinate.axis[i]
                          = str.mid (30 + i * 8, 8).toFloat ();
                      if (saxs_options.dummy_atom_pdbs_in_nm)
                        {
                  ...
                  if (do_bead_saxs_assign)
                    {
                      tmp_atom.saxs_data
                          = sf_bead_factors[bead_model.size ()
                  ...
                  bead_model.push_back (tmp_atom);
                  // QTextStream( stdout ) << QString("bead loaded serial
                  // %1\n").arg(tmp_atom.serial);
                }
          ...
          for (unsigned int i = 0; i < model_count; i++)
            {
              lb_model->addItem (
                  QString ("Model %1 (from bead model)").arg (model_names[i]));
          ...
          for (unsigned int i = 0; i < model_count; i++)
            {
              editor_msg ("gray",
                          QString (us_tr ("checking overlap for Model %1"))
                              .arg (model_names[i]));
              bead_model = bead_models[i];
              overlap_check_results += overlap_check (
                  true, true, true,
                  hydro.overlap_cutoff ? hydro.overlap : overlap_tolerance,
                  20);
            }
...
void
US_Hydrodyn::load_bead_model ()
{
  QString use_dir
      = path_load_bead_model.isEmpty () ? somo_dir : path_load_bead_model;
  ...
  if (!filename.isEmpty ())
    {
      ...
      if (is_dammin_dammif (filename) && advanced_config.auto_view_pdb)
        {
      ...
      if (!read_bead_model (filename, only_overlap))
        {
          ...
          us_qdebug (QString ("load bead model so_ovlp %1")
