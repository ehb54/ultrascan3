...
OneLiner::OneLiner (const QString &message, QWidget *parent, const char *)
    : QDialog (parent)
{
  ...
  parameter1 = new QLineEdit (this);
  parameter1->setObjectName ("lineEdit");
...
void
OneLiner::update (const QString &Textstr)
{
  string = Textstr;
}

US_FitParameter::US_FitParameter (QWidget *parent, const char *)
    : QWidget (parent)
{
  ...
  le_value->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  le_range->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize - 1));
  ...
  cb_float->setGeometry (xpos + (unsigned int)(column5 / 2) - 7, ypos + 5, 14,
                         14);
  cb_float->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  cb_constrained->setGeometry (xpos + (unsigned int)(column5 / 2) - 7,
                               ypos + 5, 14, 14);
  cb_constrained->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  lbl_bounds->setGeometry (xpos + (unsigned int)(column6 / 2) - 7, ypos + 5,
                           14, 14);
  ...
  QGridLayout *legendLayout = new QGridLayout (this);
...
void
US_FitParameter::setEnabled (bool choice)
{
  QString str;
  if (!choice)
    {
...
void
US_FitParameter::updateValue (float value)
{
  QString str;
...
void
US_FitParameter::updateRange (float range)
{
  QString str;
...
void
US_FitParameter::setUnit (const QString &str)
{
  lbl_unit->setText (str);
}

void
US_FitParameter::setFloatRed ()
{
  lbl_bounds->setPalette (USglobal->global_colors.cg_red);
}

void
US_FitParameter::setFloatGreen ()
{
  lbl_bounds->setPalette (USglobal->global_colors.cg_green);
}

void
US_FitParameter::setFloatDisabled ()
{
  lbl_bounds->setPalette (USglobal->global_colors.cg_dk_green);
}

US_FitParameterLegend::US_FitParameterLegend (QWidget *parent, const char *)
    : QWidget (parent)
{
  USglobal = new US_Config ();
  ...
  lbl_legend1->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1,
                               QFont::Bold));
  ...
  lbl_legend2->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1,
                               QFont::Bold));
  ...
  lbl_legend3->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1,
                               QFont::Bold));
  ...
  lbl_legend4->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1,
                               QFont::Bold));
  ...
  lbl_legend5->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1,
                               QFont::Bold));
  ...
  lbl_legend6->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1,
                               QFont::Bold));
...
void
getHeader (QString *header, const QString title)
{
  QDate date;
  ...
  *header = "<html>\n<head>\n   <title>" + title
            + "</title>\n"
              ...
              "charset=iso-8859-1\">\n"
              "   <!-- Author: UltraScan II for Unix Data Analysis Software "
            + date.toString ()
            + " -->\n"
...
getFooter (QString *footer)
{
  ...
  *footer
      = "<hr><font face=helvetica size=-1>\nUltraScan Software Contact: <a "
        "href=mailto:borries.demeler@umontana.edu>Borries Demeler</a>\n";
  *footer += "<br>\nThis document has been generated with the <i><b>UltraScan "
             "II</i></b> Data Analysis Software distribution.\n<br>\n";
  *footer += "All rights reserved, Copyright " + year
             + " <a href=http://www.uthscsa.edu>The University of";
  *footer += "Texas Health Science Center at San Antonio</a>.\n<br>UltraScan "
             "Home Page: ";
  *footer += "<a "
             "href=http://www.ultrascan.uthscsa.edu>http://"
             "www.ultrascan.uthscsa.edu</a>\n<p>\n";
  *footer += "This report was generated on " + date.toString ()
             + "\n</font></body>\n</html>";
}

QString
getToken (QString *str, const QString &separator)
{
...
bool
readCenterpieceInfo (vector<struct centerpieceInfo> *cp_info_vector)
{
      ...
      while (!ts.atEnd ())
        {
          ...
          if (ok) // if the first token is not an integer, we got a comment and
...
bool
readRotorInfo (vector<struct rotorInfo> *rotor_info_vector)
{
...
int
Move (const QString &sourcefile, const QString &destfile)
{
  QFile f1, f2;
  char *buf;
  buf = new char[1024];
...
int
copy (const QString &sourcefile, const QString &destfile)
{
...
)
{
  ...
  us_imgviewer = new US_ImageViewer (
      0, "new window", Qt::WDestructiveClose | Qt::WResizeNoErase);
...
bool
US_Static::lv_any_selected (QTreeWidget *lv)
{
...
bool
US_Static::lv_any_unselected (QTreeWidget *lv)
{
...
void
US_Static::lv_select_all_or_none (QTreeWidget *lv)
{
...
int
US_Static::lvi_depth (QTreeWidgetItem *lvi)
{
#if QT_VERSION < 0x040000
  return lvi->depth ();
#else
...
QTreeWidgetItem *
US_Static::lv_lastItem (QTreeWidget *lv)
{
#if QT_VERSION < 0x040000
  return lv->lastItem ();
#else
...
double
US_Static::getDouble (const QString &title, const QString &label, double value,
                      double min, double max, int decimals, bool *ok,
                      QWidget *parent, const char *)
{
  return QInputDialog::getDouble (parent, title, label, value, min, max,
                                  decimals, ok);
}

int
US_Static::getInteger (const QString &title, const QString &label, int value,
                       int min, int max, int step, bool *ok, QWidget *parent,
                       const char *)
{
#if QT_VERSION >= 0x050000
  return QInputDialog::getInt (parent, title, label, value, min, max, step,
                               ok);
#else
  return QInputDialog::getInteger (parent, title, label, value, min, max, step,
                                   ok);
#endif
}

QString
US_Static::getItem (const QString &title, const QString &label,
                    const QStringList &list, int current, bool editable,
                    bool *ok, QWidget *parent, const char *)
{
  return QInputDialog::getItem (parent, title, label, list, current, editable,
                                ok);
}

QString
US_Static::getText (const QString &title, const QString &label,
...
int
US_Static::us_message (const QString &title, const QString &text,
                       const QString &buttonText, QWidget *parent,
                       const char *)
{
  if (!buttonText.isEmpty ())
    {
      qDebug () << "us_message title: '" << title << "' text '" << text
                << "' has buttonText '" << buttonText << "'";
    }
