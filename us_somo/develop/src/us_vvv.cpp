...
testLimits (gridpt grid[])
{
  ...
  for (i = 0; (unsigned int)i < NUMBINS && !grid[i]; i++)
    {
    }
...
/*********************************************/
int
copyGridFromTo (gridpt oldgrid[], gridpt newgrid[])
{
...
/*********************************************/
int
copyGrid (gridpt oldgrid[], gridpt newgrid[])
{
...
/*********************************************/
int
fill_AccessGrid_fromFile (int numatoms, const float probe, const char file[],
                          gridpt grid[])
{
...
/*********************************************/
int
get_ExcludeGrid_fromFile (int numatoms, const float probe, const char file[],
                          gridpt EXCgrid[])
{
...
/*********************************************/
void
trun_ExcludeGrid (const float probe, gridpt ACCgrid[], gridpt EXCgrid[])
{ // contract
  // limit grid search
...
/*********************************************/
void
grow_ExcludeGrid (const float probe, gridpt ACCgrid[], gridpt EXCgrid[])
{
  // expands
  // limit grid search
  // XMIN=(minmax[3] - MAXVDW - PROBE - 2*GRID);
  ...
  cerr << endl
       << "Growing Excluded Grid from Accessible "
       << "Grid by Probe " << probe << "..." << endl;
  printBar ();
  ...
  for (gp = 0; gp < DXYZ; gp++)
    {
      if (grid[gp] == 1)
        {
          // cerr << endl << "grid point: " << gp << " value: " << grid[gp] <<
          // endl;
          return gp;
        }
    }
...
/*********************************************/
int
get_Connected (gridpt grid[], gridpt connect[], const float x, const float y,
               const float z)
{
...
/*********************************************/
int
get_ConnectedRange (gridpt grid[], gridpt connect[], const float x,
...
/*********************************************/
int
get_Connected_Point (gridpt grid[], gridpt connect[], const int gp)
{
  if (connect == NULL)
    {
      cerr << "Allocating Grid..." << endl;
...
/*********************************************/
int
subt_Grids (gridpt biggrid[], gridpt smgrid[])
{
  /*
...
/*********************************************/
int
intersect_Grids (gridpt grid1[], gridpt grid2[])
{
  /*
  ...
  if (DEBUG > 0)
    {
      ...
      cerr << int (1000.0 * voxels / (voxels + changed)) / 10.0 << "% ]"
           << flush;
      cerr << endl << endl;
    }
  return voxels;
...
/*********************************************/
int
fill_AccessGrid (const float x, const float y, const float z, const float R,
                 gridpt grid[])
{
      ...
      for (int dj = jmin; dj <= jmax; dj++)
        {
          for (int dk = kmin; dk <= kmax; dk++)
            {
              distsq = (xk - di) * (xk - di) + (yk - dj) * (yk - dj)
                       + (zk - dk) * (zk - dk);
              if (distsq < cutoff)
                {
...
/*********************************************/
void
empty_ExcludeGrid (const int i, const int j, const int k, const float probe,
                   gridpt grid[])
{
...
/*********************************************/
void
fill_ExcludeGrid (const int i, const int j, const int k, const float probe,
                  gridpt grid[])
{
  // provides indexes (i,j,k) of grid where ijk2pt(i,j,k) = gridpt
  const float R = probe / GRID; // Aug 19: correction for oversize
...
/*********************************************/
void
pt2ijk (int pt, int &i, int &j, int &k)
{
  i = pt % DX;
  j = (pt % DXY) / DX;
  k = pt / DXY;
...
/*********************************************/
void
pt2xyz (int pt, float &x, float &y, float &z)
{
  int i, j, k;
  i = pt % DX;
  j = (pt % DXY) / DX;
...
/*********************************************/
int
xyz2pt (float x, float y, float z)
{
  int ip, jp, kp;
  ip = int ((x - XMIN) / GRID + 0.5);
  jp = int ((y - YMIN) / GRID + 0.5);
...
/*********************************************/
bool
isEdgePoint (const int i, const int j, const int k, gridpt grid[])
{
  // look at neighbors
  short int count = 0;
  ...
  if (count != 6)
    {
      cerr << "EdgePoint count " << count << " != 6" << endl;
    }
  return 0;
};
...
/*********************************************/
bool
isEdgePoint_Fill (const int pt, gridpt grid[])
{ // look at neighbors
  short int count = 0;
  ...
  if (count != 27)
    {
      cerr << "EdgePoint count " << count << " != 27" << endl;
    }
  return 0;
};
...
/*********************************************/
bool
isEdgePoint_Star (const int pt, gridpt grid[])
{
  // look at neighbors
  short int count = 0;
...
/*********************************************/
void
ijk2pdb (char line[], int i, int j, int k, int n)
{
  // char line[128];
...
/*********************************************/
void
limitToTunnelArea (const float radius, gridpt grid[])
{
...
/*********************************************/
bool
isCloseToVector (const float radius, const int pt)
{
...
/*********************************************/
float
distFromPt (const float x, const float y, const float z)
{
  // INIT POINT
...
/*********************************************/
float
crossSection (const real /* p */, const vector /* v */, const gridpt grid[])
{
...
/*********************************************/
float
crossSection (const gridpt grid[])
{
  // INIT POINT
  struct real p;
  p.x = 77.0;
  ...
  for (float k = -5; k < 100; k += 0.5)
    {
      ...
      // cerr << "crossSection:  " << k << " " << count << " of " << total <<
      // endl;
  ...
  if ((long double)float (vox) * GRIDVOL > tenp)
    {
      ...
      if (cut >= 100 || vol < 100000)
        ...
        {
          result += QString ("0%1").arg (cut);
        }
      else if (cut >= 1)
        {
          result += QString ("00%1").arg (cut);
        }
      else
        {
          result += QString ("000%1").arg (cut);
        }
      vox = vox - int (cut * tenp / GRIDVOL);
    }
  double cut = (double)float (vox) * GRIDVOL;
...
/*********************************************/
float
surface_area (gridpt grid[])
{
  // Initialize Variables
  float surf = 0.0;
  const float wt[]
      = { 0.0f,    0.894f, 1.3409f, 1.5879f, 4.0f, 2.6667f,
  ...
  */
  ...
  for (int i = 1; i <= 9; i++)
    {
      cerr << "s" << i << ":"
           << int (float (100000 * edges[i]) / totedge) / 1000.0 << " ";
      surf += edges[i] * wt[i];
    }
  cerr << endl << endl;
...
/*********************************************/
int
classifyEdgePoint (const int pt, gridpt grid[])
{
  // look at neighbors
  short int count = 0;
...
/*********************************************/
void
determine_MinMax (gridpt grid[], int minmax[])
{
...
/*********************************************/
int
bounding_box (gridpt grid[], gridpt bbox[])
{
...
#define FMAX 256
  ...
  while (argc > 1 && argv[1][0] == '-')
    {
      if (argv[1][1] == 'i')
        ...
        {
          cerr << "./Volume.exe -i <file> -g <gridspacing> -p <probe_rad> "
               << endl
               << "\t-e <EZD outfile> -o <PDB outfile> -m <MRC outfile>"
               << endl;
          cerr << "Volume.exe -- Calculate the volume and surface area for "
                  "any probe radius"
               << endl;
          cerr << endl;
          return 1;
  ...
  cerr << "Resolution:      " << int (1000.0 / float (GRIDVOL)) / 1000.0
       << " voxels per A^3" << endl;
  cerr << "Resolution:      " << int (11494.0 / float (GRIDVOL)) / 1000.0
       << " voxels per water molecule" << endl;
...
bool
compute_vol_surf (const char *xyzr_file, float use_probe, float use_grid,
                  double &volume, double &surf)
{
  double PROBE = use_probe;
  GRID = use_grid;
  finalGridDims (PROBE);

  int numatoms = read_NumAtoms (xyzr_file);
  ...
  if (PROBE > 0.0)
    {
      voxels = get_ExcludeGrid_fromFile (numatoms, PROBE, xyzr_file, EXCgrid);
    }
  else
    {
      voxels = fill_AccessGrid_fromFile (numatoms, 0.0, xyzr_file, EXCgrid);
    }
  surf = (double)surface_area (EXCgrid);
  volume = GetVol (voxels);
  free (EXCgrid);
  return true;
}

gridpt *save_EXCgrid = (gridpt *)0;
bool
setup_vol_surf (const char *xyzr_file, float use_probe, float use_grid,
                double &volume, double &surf)
{
  double PROBE = use_probe;
  GRID = use_grid;
