...
int countGrid(gridpt grid[]) {
  ...
  if (DEBUG > 0) cerr << "Counting up Voxels in Grid for Volume...  " << flush;
  ...
  if (DEBUG > 0) cerr << "done [ " << voxels << " voxels ]" << endl << endl;
  return voxels;
};
...
void zeroGrid(gridpt grid[]) {
  ...
  if (DEBUG > 0) cerr << "Zero-ing All Voxels in the Grid...  " << flush;

  // #pragma omp parallel for
  for (unsigned int pt = 0; pt < NUMBINS; pt++) {
    grid[pt] = 0;
  }
  if (DEBUG > 0) cerr << "done " << endl << endl;
  return;
};
...
void inverseGrid(gridpt grid[]) {
  ...
  if (DEBUG > 0) cerr << "Inversing All Voxels in the Grid...  " << flush;
...
/*********************************************/
int read_NumAtoms(const char file[]) {
  ifstream infile;
  int count = 0;
  char line[256];
  float minmax[6];
...
/*********************************************/
int fill_AccessGrid_fromFile(int numatoms, const float probe, const char file[],
                             gridpt grid[]) {
  if (grid == NULL) {
    cerr << "Allocating Grid..." << endl;
    grid = (gridpt *)malloc(NUMBINS);
...
/*********************************************/
int get_ExcludeGrid_fromFile(int numatoms, const float probe, const char file[],
                             gridpt EXCgrid[]) {
...
/*********************************************/
int get_Connected(gridpt grid[], gridpt connect[], const float x, const float y,
                  const float z) {
  cerr << endl << "x:" << x << " y:" << y << " z:" << z << endl;
  const int gp = xyz2pt(x, y, z);
  if (DEBUG > 0) cerr << "gp: " << gp << " grid value: " << grid[gp] << endl;
      ...
      for (int jd = -delta; !stop && jd <= delta; jd++) {
        for (int kd = -delta; !stop && kd <= delta; kd++) {
          ...
          if (grid[pt]) {
            float xn, yn, zn;
            pt2xyz(pt, xn, yn, zn);
            cerr << "nearest filled pt: " << xn << " " << yn << " " << zn
                 << endl;
            stop = 1;
          }
        }
  ...
  if (gp >= 0 && gp <= max && grid[gp]) {
    connect[gp] = 1;
    if (DEBUG > 0) cerr << "GetConnected..." << flush;
    ...
    // cerr << endl;
    if (steps > 1) {
      if (DEBUG > 0) cerr << " performed " << steps << " steps" << endl;
    } else {
      if (DEBUG > 0) cerr << " done" << endl;
    }
  } else if (gp > 0 && gp < max) {
    if (DEBUG > 0) cerr << "GetConnected: Point is NOT FILLED" << endl;
  } else {
    if (DEBUG > 0) cerr << "GetConnected: Point OUT OF RANGE" << endl;
  }
  return connected;
};

/*********************************************/
int get_ConnectedRange(gridpt grid[], gridpt connect[], const float x,
...
int subt_Grids(gridpt biggrid[], gridpt smgrid[]) {
  ...
  */
  ...
  // float cut = cat;
  if (DEBUG > 0) cerr << "Subtracting Grids (Modifies biggrid)...  " << flush;
  // printBar();
...
int intersect_Grids(gridpt grid1[], gridpt grid2[]) {
  ...
  */
  ...
  // float cut = cat;
  if (DEBUG > 0) cerr << "Intersecting Grids...  " << flush;
  // printBar();
...
/*********************************************/
int fill_AccessGrid(const float x, const float y, const float z, const float R,
                    gridpt grid[]) {
...
/*********************************************/
void empty_ExcludeGrid(const int i, const int j, const int k, const float probe,
                       gridpt grid[]) {
...
/*********************************************/
void fill_ExcludeGrid(const int i, const int j, const int k, const float probe,
                      gridpt grid[]) {
  // provides indexes (i,j,k) of grid where ijk2pt(i,j,k) = gridpt
  const float R = probe / GRID;  // Aug 19: correction for oversize
  const int r = int(R + 1);
...
/*********************************************/
int ijk2pt(int i, int j, int k) { return int(i + j * DX + k * DXY); };
...
/*********************************************/
float crossSection(const real /* p */, const vector /* v */,
                   const gridpt grid[]) {
...
void determine_MinMax(gridpt grid[], int minmax[]) {
  ...
  if (DEBUG > 0) cerr << "Determining Minima and Maxima..." << flush;
...
#define FMAX 256
  ...
  while (argc > 1 && argv[1][0] == '-') {
    if (argv[1][1] == 'i') {
      ...
      cerr << "Volume.exe -- Calculate the volume and surface area for any "
              "probe radius"
           << endl;
      cerr << endl;
      return 1;
...
bool compute_vol_surf(const char *xyzr_file, float use_probe, float use_grid,
...
bool setup_vol_surf(const char *xyzr_file, float use_probe, float use_grid,
                    double &volume, double &surf) {
  double PROBE = use_probe;
  GRID = use_grid;
  finalGridDims(PROBE);
...
bool is_pt(float x, float y, float z) {
  int pt = xyz2pt(x, y, z);
  if (pt >= 0 && pt < DXYZ) {
    if (save_EXCgrid[pt]) {
      return true;
