  ...
  for (current_model = 0; current_model < (unsigned int)lb_model->count();
    ...
    if (!any_errors && lb_model->item(current_model)->isSelected()) {
      ...
      write_contact_plot(
          ...
          &model_vector_as_loaded[current_model], bd_options.threshold_pb_pb);
    }
    if (stopFlag) {
...
void US_Hydrodyn::bd_run() { run_browflex(); }
    ...
    for (map<QString, bool>::iterator it = connection_active.begin();
      ...
      if (connection_active[it->first]) {
        if (rx.indexIn(it->first) == -1) {
          editor->append(
              "unexpected regexp extract failure (write_browflex_files)!\n");
          return -1;
        }
        ...
        switch ((connection_forced[it->first] == 0 ? 0 : 1) * 10 +
          ...
          case 0:  // pb-pb not chemical
          {
            if (bd_options.compute_pb_pb_force_constant) {
              editor->append(
                  "compute hookean force constant for pb_pb not "
                  "implemented!\n");
              return -1;
            } else {
            ...
            switch (bd_options.pb_pb_bond_type) {
              ...
              default:
                editor->append(
                    "unexpected case type pb-pb (write_browflex_files)!\n");
                return -1;
                break;
            }
          } break;
          case 1:  // pb-sc not chemical
          {
            if (bd_options.compute_pb_sc_force_constant) {
              editor->append(
                  "compute hookean force constant for pb_sc not "
                  "implemented!\n");
              return -1;
            } else {
            ...
            switch (bd_options.pb_sc_bond_type) {
              ...
              default:
                editor->append(
                    "unexpected case type pb_sc (write_browflex_files)!\n");
                return -1;
                break;
            }
          } break;
          case 2:  // sc-sc not chemical
          {
            if (bd_options.compute_sc_sc_force_constant) {
              editor->append(
                  "compute hookean force constant for sc_sc not "
                  "implemented!\n");
              return -1;
            } else {
            ...
            switch (bd_options.sc_sc_bond_type) {
              ...
              default:
                editor->append(
                    "unexpected case type sc_sc (write_browflex_files)!\n");
                return -1;
                break;
            }
          } break;
          case 10:  // pb_pb chemical
          {
            if (bd_options.compute_chem_pb_pb_force_constant) {
              editor->append(
                  "compute hookean force constant for chem_pb_pb not "
                  "implemented!\n");
              return -1;
            } else {
            ...
            switch (bd_options.chem_pb_pb_bond_type) {
              ...
              default:
                editor->append(
                    "unexpected case type chem_pb_pb "
          ...
          case 11:  // pb-sc chemical
          {
            if (bd_options.compute_chem_pb_sc_force_constant) {
              editor->append(
                  "compute hookean force constant for chem_pb_sc not "
                  "implemented!\n");
              return -1;
            } else {
            ...
            switch (bd_options.chem_pb_sc_bond_type) {
              ...
              default:
                editor->append(
                    "unexpected case type chem_pb_sc "
                    "(write_browflex_files)!\n");
                return -1;
                break;
            }
          ...
          case 12:  // sc-sc chemical
          {
            if (bd_options.compute_chem_sc_sc_force_constant) {
              editor->append(
                  "compute hookean force constant for chem_sc_sc not "
                  "implemented!\n");
              return -1;
            } else {
            ...
            switch (bd_options.chem_sc_sc_bond_type) {
              ...
              default:
                editor->append(
                    "unexpected case type chem sc-sc "
                    "(write_browflex_files)!\n");
                return -1;
                break;
            }
...
int US_Hydrodyn::write_pdb(QString fname, vector<PDB_atom> *model) {
  ...
  if (bd_options.show_pdb) {  // display pdb
...
int US_Hydrodyn::compute_bd_connections() {
        ...
        for (unsigned int j = i + 1; j < bead_models[current_model].size();
          ...
          if (bead_models[current_model][j].active) {
            if (bead_models[current_model][j].chain != 0) {
              ...
              connection_dists[QString("%1~%2").arg(last_i).arg(j)].push_back(
                  d);
              last_i = j;
            } else {
  ...
  for (unsigned int k = 1; k < models_to_proc.size(); k++) {
    ...
    for (map<QString, bool>::iterator it = connection_active.begin();
      ...
      if (rx.indexIn(it->first) == -1) {
        editor->append(
            "unexpected regexp extract failure (compute_connections)!\n");
        return -1;
      }
      int i = rx.cap(1).toInt();
      int j = rx.cap(2).toInt();
  ...
  for (map<QString, bool>::iterator it = connection_active.begin();
       it != connection_active.end(); it++) {
    if (connection_active[it->first]) {
      cout << QString("Connection %1 distance min %2 max %3 avg %4 dist:\n")
                  .arg(it->first)
...
int US_Hydrodyn::write_contact_plot(QString fname, PDB_model *model,
                                    float thresh) {
      ...
      if (this_atom->name == "CA") {
        for (unsigned int l = j; l < model->molecule.size(); l++) {
          for (unsigned int m = (l == j ? k : 0);
            ...
            if (alt_atom->name == "CA") {
              if (dist(this_atom->coordinate, alt_atom->coordinate) <= thresh) {
                ts << QString("%1 %2\n")
...
int US_Hydrodyn::write_contact_plot(QString fname, vector<PDB_atom> *model,
                                    float thresh) {
  ...
  for (unsigned int i = 0; i < model->size(); i++) {
    if ((*model)[i].chain == 0) {
      for (unsigned int j = i; j < model->size(); j++) {
        if ((*model)[j].chain == 0) {
          if (dist((*model)[i].bead_coordinate, (*model)[j].bead_coordinate) <=
              thresh) {
            ts << QString("%1 %2\n")
                      .arg((*model)[i].resSeq)
                      .arg((*model)[j].resSeq);
...
int US_Hydrodyn::build_pb_structures(PDB_model *model) {
  ...
  for (unsigned int j = 0; j < model->molecule.size(); j++) {
    ...
    if (pb_atoms.count("CA") && pb_atoms.count("C") && pb_atoms.count("O")) {
      pb_base_list[j].push_back(pb_list[j].size());
      pb_list[j].push_back(pb_atoms["CA"]);
      pb_list[j].push_back(pb_atoms["C"]);
      pb_list[j].push_back(pb_atoms["O"]);
...
void US_Hydrodyn::bd_edit() {
  ...
  if (!filename.isEmpty()) {
    ...
    switch (QMessageBox::question(
        this, us_tr("Load Browflex Files"),
        QString(us_tr(fi.fileName() + " appears to be a 'main' Browflex file, "
...
void US_Hydrodyn::bd_load_results() {
  ...
  if (!fi_traj.isReadable()) {
    editor_msg("red", QString(us_tr("\nCould not open trajectory file %1 for "
  ...
  {
    QString errmsg =
        QString(us_tr("\nMolecular file ") + "%1" + us_tr("error at line"))
            .arg(bd_last_molec_file) +
        "%1\n";
...
void US_Hydrodyn::bd_load_results_after_anaflex() {
  ...
  // molec file has molecular weight, have to input psv (like dammin/dammif
  // files) create bead model file
  ...
  QString errmsg =
      QString(us_tr("\nAnaflex log file ") + "%1" + us_tr(" error at line "))
          .arg(anaflex_last_log_file) +
      "%1\n";
    ...
    for (unsigned int j = 0; j < nconf; j++) {
      ...
      for (unsigned int k = 0; k < beads; k++) {
        ...
        for (unsigned int l = 0; l < 3; l++) {
          ...
          //  tmp_atom.bead_coordinate.axis[l] *= unit_of_length * 1e6; // to
          //  micron
        }
      ...
      editor->append(
          QString(us_tr("Created bead model %1\n"))
