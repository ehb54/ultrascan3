...
// (this) us_hydrodyn_bd_core.cpp contains the main computational routines for
...
US_Hydrodyn::bd_prepare ()
{
  ...
  bead_model_suffix = QString ("bd-%1-%2-%3")
      ...
      if (!any_errors && lb_model->item (current_model)->isSelected ())
        {
          ...
          if (!compute_asa (true))
            {
              ...
              QString corr_name = somo_dir + SLASH + "bd" + SLASH + project
          ...
          write_contact_plot (somo_dir + SLASH + "bd" + SLASH + project
                                  + QString ("_%1").arg (current_model + 1)
                                  + QString ("-pdb_contact-%1.txt")
                                        .arg (bd_options.threshold_pb_pb),
      ...
      for (current_model = 0; current_model < (unsigned int)lb_model->count ();
           current_model++)
        {
          if (lb_model->item (current_model)->isSelected ())
            {
              QString fname = somo_dir + SLASH + "bd" + SLASH + project
                              + QString ("_%1").arg (current_model + 1)
                              + QString (bead_model_suffix.length ()
                                             ? ("-" + bead_model_suffix)
                                             : "");
...
int
US_Hydrodyn::create_browflex_files ()
{
  double conv = 1e-8;
  ...
  QString filename
      = project + QString ("_%1").arg (current_model + 1)
        + QString (bead_model_suffix.length () ? ("-" + bead_model_suffix)
                                               : "")
        + "-bf";
  QString basename = somo_dir + SLASH + "bd" + SLASH + filename;
  ...
  // main file
  {
    ...
    bd_last_traj_file
        = somo_dir + SLASH + "bd" + SLASH + filename + "-tra.txt";
    bd_last_molec_file
        = somo_dir + SLASH + "bd" + SLASH + filename + "-molec.txt";
#if defined(DEBUG_CONN)
    cout << "last tra file " << bd_last_traj_file << endl;
    cout << "last molec file " << bd_last_molec_file << endl;
#endif
    QTextStream ts (&f);
    ts << QString ("%1-log.txt            !logfile\n"
  ...
  // molecular file
  {
    ...
    for (unsigned int i = 0; i < bead_models[current_model].size (); i++)
      {
        ts << QString ("  %1\n").arg (
            bead_models[current_model][i].bead_computed_radius * conv);
      }
        ...
        if (connection_active[it->first])
          {
            if (rx.indexIn (it->first) == -1)
              {
                editor->append ("unexpected regexp extract failure "
                                "(write_browflex_files)!\n");
                return -1;
              }
            ...
            switch ((connection_forced[it->first] == 0 ? 0 : 1) * 10
              ...
              case 0: // pb-pb not chemical
                {
                  if (bd_options.compute_pb_pb_force_constant)
                    {
                      editor->append ("compute hookean force constant for "
                                      "pb_pb not implemented!\n");
                      return -1;
                    }
                  else
                    {
                  ...
                  if (bd_options.compute_pb_pb_equilibrium_dist)
                    {
                      equilibrium_dist
                          = connection_dist_stats[it->first][2] * conv;
                    }
                  else
                    {
                  ...
                  switch (bd_options.pb_pb_bond_type)
                    ...
                    case 3: // hard-fene
                      ...
                      break;
                    default:
                      editor->append ("unexpected case type pb-pb "
                                      "(write_browflex_files)!\n");
                      return -1;
                      break;
                    }
              ...
              case 1: // pb-sc not chemical
                {
                  ...
                  switch (bd_options.pb_sc_bond_type)
                    ...
                    case 3: // hard-fene
                      ...
                      break;
                    default:
                      editor->append ("unexpected case type pb_sc "
                                      "(write_browflex_files)!\n");
                      return -1;
                      break;
                    }
              ...
              case 2: // sc-sc not chemical
                {
                  ...
                  switch (bd_options.sc_sc_bond_type)
                    ...
                    case 3: // hard-fene
                      ...
                      break;
                    default:
                      editor->append ("unexpected case type sc_sc "
                                      "(write_browflex_files)!\n");
                      return -1;
                      break;
                    }
              ...
              case 10: // pb_pb chemical
                {
                  ...
                  switch (bd_options.chem_pb_pb_bond_type)
                    ...
                    case 3: // hard-fene
                      ...
                      break;
                    default:
                      editor->append ("unexpected case type chem_pb_pb "
                                      "(write_browflex_files)!\n");
                      return -1;
                      break;
                    }
              ...
              case 11: // pb-sc chemical
                {
                  ...
                  switch (bd_options.chem_pb_sc_bond_type)
                    ...
                    case 3: // hard-fene
                      ...
                      break;
                    default:
                      editor->append ("unexpected case type chem_pb_sc "
                                      "(write_browflex_files)!\n");
                      return -1;
                      break;
                    }
              ...
              case 12: // sc-sc chemical
                {
                  ...
                  switch (bd_options.chem_sc_sc_bond_type)
                    ...
                    case 3: // hard-fene
                      ...
                      break;
                    default:
                      editor->append ("unexpected case type chem sc-sc "
                                      "(write_browflex_files)!\n");
                      return -1;
                      break;
                    }
              ...
              default:
                editor->append (
                    "unexpected case type (write_browflex_files)!\n");
                return -1;
                break;
              }
          }
      }
    ts << "0   Number of angles\n"
...
int
US_Hydrodyn::write_pdb (QString fname, vector<PDB_atom> *model)
{
  QFile fpdb;
...
int
US_Hydrodyn::compute_bd_connections ()
{
          ...
          if (bead_models[current_model][i].active
              && bead_models[current_model][i].chain == 0)
            {
              for (unsigned int j = i + 1;
                   j < bead_models[current_model].size (); j++)
                {
                  if (bead_models[current_model][j].active
                      && bead_models[current_model][j].chain == 0)
                    {
                      ...
                      editor->append (
                          QString ("adding forced connection %1 %2\n")
                              .arg (i)
                              .arg (j));
                      connection_forced[QString ("%1~%2").arg (i).arg (j)]
                          = true;
                      connection_active[QString ("%1~%2").arg (i).arg (j)]
                          = true;
                      connection_dists[QString ("%1~%2").arg (i).arg (j)]
                          .push_back (d);
                      break;
                    }
                }
                  ...
                  if (bead_models[current_model][j].active)
                    {
                      if (bead_models[current_model][j].chain != 0)
                        {
                          d = dist (
                              bead_models[current_model][last_i]
                                  .bead_coordinate,
                              bead_models[current_model][j].bead_coordinate);
                          editor->append (
                              QString ("adding forced connection %1 %2\n")
                                  ...
                                  .arg (j));
                          connection_forced
                              [QString ("%1~%2").arg (last_i).arg (j)]
                              = true;
                          connection_active
                              [QString ("%1~%2").arg (last_i).arg (j)]
                              = true;
                          connection_dists[QString ("%1~%2").arg (last_i).arg (
                                               j)]
                              .push_back (d);
                          last_i = j;
                        }
  ...
  write_contact_plot (
      somo_dir + SLASH + "bd" + SLASH + project
          + QString ("_%1").arg (current_model + 1)
          + QString ("-bead_contact-%1.txt").arg (bd_options.threshold_pb_pb),
      &bead_models[current_model], bd_options.threshold_pb_pb);

  for (unsigned int i = 0; i < bead_models[current_model].size () - 1; i++)
    {
      if (bead_models[current_model][i].active)
        {
          for (unsigned int j = i + 1; j < bead_models[current_model].size ();
               j++)
            {
              if (bead_models[current_model][j].active)
                {
                  ...
                  if (bead_models[current_model][i].chain == 0
                      && bead_models[current_model][j].chain == 0)
                    {
                      connection_pair_type[QString ("%1~%2").arg (i).arg (j)]
                          = 0;
                    }
                  if ((bead_models[current_model][i].chain == 0
                       && bead_models[current_model][j].chain != 0)
                  ...
                  if (( // mc - mc
                          connection_pair_type[QString ("%1~%2").arg (i).arg (
                              j)]
                              == 0
  ...
  for (unsigned int k = 1; k < models_to_proc.size (); k++)
    {
      ...
      for (map<QString, bool>::iterator it = connection_active.begin ();
           it != connection_active.end (); it++)
        {
          ...
          if (!connection_forced.count (QString ("%1~%2").arg (i).arg (j))
            ...
            {
              connection_dists[QString ("%1~%2").arg (i).arg (j)].push_back (
                  d);
            }
        }
    }
      ...
      if (connection_active[it->first])
        {
          ...
          if (connection_dists[it->first].size ())
            {
              for (unsigned int i = 0; i < connection_dists[it->first].size ();
                   i++)
                {
                  if (connection_dists[it->first][i]
                      < connection_dist_stats[it->first][0])
                    {
                      connection_dist_stats[it->first][0]
                          = connection_dists[it->first][i];
                    }
                  if (connection_dists[it->first][i]
                      > connection_dist_stats[it->first][1])
                    {
                      connection_dist_stats[it->first][1]
                          = connection_dists[it->first][i];
                    }
                  connection_dist_stats[it->first][2]
                      += connection_dists[it->first][i];
                }
              connection_dist_stats[it->first][2]
                  /= connection_dists[it->first].size ();
            }
          else
            {
              editor->append (
                  QString ("Error: BD connection %1 has no values!\n")
                      .arg (it->first));
              return -1;
            }
        }
  ...
  for (map<QString, bool>::iterator it = connection_active.begin ();
       it != connection_active.end (); it++)
    {
      if (connection_active[it->first])
        {
          cout << QString (
                      "Connection %1 distance min %2 max %3 avg %4 dist:\n")
                      .arg (it->first)
...
int
US_Hydrodyn::write_contact_plot (QString fname, PDB_model *model, float thresh)
{
          ...
          if (this_atom->name == "CA")
            {
              for (unsigned int l = j; l < model->molecule.size (); l++)
                {
                  for (unsigned int m = (l == j ? k : 0);
                       m < model->molecule[l].atom.size (); m++)
                    {
                      PDB_atom *alt_atom = &(model->molecule[l].atom[m]);
                      if (alt_atom->name == "CA")
                        {
                          if (dist (this_atom->coordinate,
                                    alt_atom->coordinate)
                              <= thresh)
                            {
                              ts << QString ("%1 %2\n")
...
int
US_Hydrodyn::write_contact_plot (QString fname, vector<PDB_atom> *model,
                                 float thresh)
{
          ...
          for (unsigned int j = i; j < model->size (); j++)
            {
              if ((*model)[j].chain == 0)
                {
                  if (dist ((*model)[i].bead_coordinate,
                            (*model)[j].bead_coordinate)
                      <= thresh)
                    {
                      ts << QString ("%1 %2\n")
                                .arg ((*model)[i].resSeq)
...
static vector<vector<unsigned int> >
    pb_list; // filters relevant atoms to molecule#, atom#
static vector<vector<unsigned int> >
    pb_base_list; // gives offsets into pb_list for pbs
...
point
US_Hydrodyn::minus (point p1, point p2) // p1 - p2
{
  point result;
...
point
US_Hydrodyn::cross (point p1, point p2) // p1 cross p2
{
  point result;
...
float
US_Hydrodyn::dot (point p1, point p2) // p1 dot p2
{
...
float
US_Hydrodyn::dist (point p1, point p2) // sqrt( (p1 - p2) dot (p1 - p2) )
{
  point p = minus (p1, p2);
  return sqrt (dot (p, p));
}
...
point
US_Hydrodyn::plane (PDB_atom *a1, PDB_atom *a2, PDB_atom *a3)
{
...
point
US_Hydrodyn::plane (point p1, point p2, point p3)
{
...
point
US_Hydrodyn::average (vector<point> *v)
{
  point result = (*v)[0];
...
int
US_Hydrodyn::build_pb_structures (PDB_model *model)
{
  ...
  for (unsigned int j = 0; j < model->molecule.size (); j++)
    {
      ...
      for (unsigned int k = 0; k < model->molecule[j].atom.size (); k++)
        {
          PDB_atom *this_atom = &(model->molecule[j].atom[k]);
          if (!first_N_found && this_atom->name == "N")
            {
              first_N_found = true;
          ...
          if (lastResSeq != this_atom->resSeq)
            {
              // new residue
              if (!pb_atoms.count ("CA") || !pb_atoms.count ("C")
                  || !pb_atoms.count ("O") || !pb_atoms.count ("N"))
                {
      ...
      for (unsigned int k = 0; k < pb_list[j].size (); k++)
        {
          ...
          cout << QString ("pb_atom %1 %2 %3 %4 %5\n")
      ...
      for (unsigned int k = 0; k < pb_base_list[j].size (); k++)
        {
          ...
          cout << QString ("base pb_atom %1 %2 %3 %4 %5\n")
                      .arg (this_atom->serial)
                      .arg (this_atom->name)
                      .arg (this_atom->resName)
                      .arg (this_atom->chainID)
          ...
          vector<point> v;
          point cross1
              = plane (&(model->molecule[j].atom[pb_list[j][p + 2]]),  // O
          ...
          if (p + 3 < pb_list[j].size ())
            {
              ...
              point cross2
                  = plane (&(model->molecule[j].atom[pb_list[j][p + 3]]),  // N
                           &(model->molecule[j].atom[pb_list[j][p + 1]]),  // C
                           &(model->molecule[j].atom[pb_list[j][p + 2]])); // O

              point cross3
                  = plane (&(model->molecule[j].atom[pb_list[j][p + 0]]), // CA
                           &(model->molecule[j].atom[pb_list[j][p + 1]]), // C
          ...
          if (k)
            {
              float angle
                  = (180.0 / 3.14159265)
                    * acos (dot (normal (minus (model->molecule[j]
                                                    .atom[pb_list[j][p - 3]]
                                                    .coordinate, // C (prior)
                                                model->molecule[j]
                                                    .atom[pb_list[j][p - 1]]
...
int
US_Hydrodyn::compute_pb_normals ()
{
...
US_Hydrodyn::run_browflex ()
{
    ...
    if (!qfi.exists ())
      {
        editor_msg (
            "red",
            QString ("Browflex program '%1' does not exist\n").arg (prog));
        return -1;
      }
    if (!qfi.isExecutable ())
      {
        editor_msg (
            "red",
            QString ("Browflex program '%1' is not executable\n").arg (prog));
        return -1;
      }
  }
  ...
  connect (browflex, SIGNAL (readyReadStandardOutput ()), this,
           SLOT (browflex_readFromStdout ()));
  connect (browflex, SIGNAL (readyReadStandardError ()), this,
           SLOT (browflex_readFromStderr ()));
  connect (browflex, SIGNAL (finished (int, QProcess::ExitStatus)), this,
           SLOT (browflex_finished (int, QProcess::ExitStatus)));
...
void
US_Hydrodyn::browflex_readFromStdout ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn::browflex_readFromStderr ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn::browflex_finished (int, QProcess::ExitStatus)
{
...
void
US_Hydrodyn::bd_anaflex_enables (bool flag)
{
...
void
US_Hydrodyn::bd_load_error (QString filename)
{
  editor_msg (
      "red",
...
bool
US_Hydrodyn::bd_valid_browflex_main (QString filename)
{
  ...
  if (bd_last_file != "")
    {
      ...
      switch (QMessageBox::question (
          this, us_tr ("Load Browflex Files"),
...
void
US_Hydrodyn::bd_edit_util (QString dir, QString filename)
{
  ...
  if (bd_last_file != "")
    {
      ...
      QFileInfo fi (bd_last_file);
      switch (QMessageBox::question (
          this, us_tr ("View/Edit Browflex Files"),
  ...
  if (!filename.isEmpty ())
    {
      QFileInfo fi (filename);
      QString dir = fi.path ();
      // open file and view this and all associated files if first line ends
      // with .txt or .TXT
      QFile f (filename);
      if (!f.open (QIODevice::ReadOnly))
        {
      ...
      switch (QMessageBox::question (
          this, us_tr ("Load Browflex Files"),
          QString (us_tr (fi.fileName ()
                          + " appears to be a 'main' Browflex file, open all "
                            "ancillary files? ")),
          QMessageBox::Yes, QMessageBox::No, QMessageBox::NoButton))
        {
        case QMessageBox::No:
...
US_Hydrodyn::bd_load_results ()
{
  ...
  if (!fi_traj.isReadable ())
    {
      editor_msg ("red", QString (us_tr ("\nCould not open trajectory file %1 "
  ...
  if (!fi_molec.isReadable ())
    {
      editor_msg ("red", QString (us_tr ("\nCould not open molecular file %1 "
                                         "for reading. Check permissions.\n"))
                             .arg (bd_last_molec_file));
      return;
    }
  ...
  // read bead sizes
  {
    ...
    if (!f.open (QIODevice::ReadOnly))
      {
        editor_msg ("red",
                    QString (us_tr ("\nCould not open molecular file %1 for "
                                    "reading. Check permissions.\n"))
                        .arg (bd_last_molec_file));
        return;
      }
  ...
  for (unsigned int i = 0; i < bd_load_results_beads; i++)
    {
      cout << QString ("bead %1 radius %2\n")
                  .arg (i)
                  .arg (bd_load_results_bead_radius[i]);
    }
#endif
...
void
US_Hydrodyn::bd_load_results_after_anaflex ()
{
  ...
  if (!fi_out1.isReadable ())
    {
      editor_msg ("red",
                  QString (us_tr ("\nCould not open Anaflex log file %1 for "
                                  "reading. Check permissions.\n"))
                      .arg (anaflex_last_out1_file));
      return;
    }
  ...
  if (!f_out1.open (QIODevice::ReadOnly))
    {
      editor_msg ("red", QString (us_tr ("\nCould not open output file %1 for "
                                         "reading. Check permissions.\n"))
                             .arg (anaflex_last_out1_file));
      return;
    }
  ...
  // read traj file, molec file, build bead models & load them up! (into
  // batch?) molec file has molecular weight, have to input psv (like
  // dammin/dammif files) create bead model file
  ...
  if (anaflex_options.nfrec > 1)
    {
      nconf = (nconf / anaflex_options.nfrec)
  ...
  if (beads != bd_load_results_bead_radius.size ())
    {
      editor_msg (
          "red",
          QString (
              us_tr ("Number of beads in Anaflex log file (%1) does not match"
      ...
      for (unsigned int j = 0; j < nconf; j++)
        {
              ...
              for (unsigned int l = 0; l < 3; l++)
                {
                  ...
                  //  tmp_atom.bead_coordinate.axis[l] *= unit_of_length * 1e6;
                  //  // to micron
                }
          ...
          if (!load_results_win_done)
            {
              ...
              US_Hydrodyn_BD_Load_Results_Opts *hblro
                  = new US_Hydrodyn_BD_Load_Results_Opts (
                      msg, &hydro.temperature, &hydro.solvent_viscosity,
                      &hydro.solvent_density, &hydro.solvent_name,
                      &hydro.solvent_acronym, &psv, bd_load_results_temp,
                      bd_load_results_solvent_visc, &check_fix_overlaps);
                      ...
                      if (grid.enable_asa)
                        {
                          ...
                          // now apply radial reduction with outward
                          // translation using
          ...
          editor->append (QString (us_tr ("Created bead model %1\n"))
                              .arg (name + QString ("-m%1-c%2").arg (i).arg (j)
                                    + ".bead_model"));
        }
    }
  progress->setValue (1);
  ...
  editor->append (
      us_tr ("Loading into batch window. (this may take some time)\n"));
  qApp->processEvents ();
  ...
  if (batch_window->count_files ())
    {
      ...
      switch (QMessageBox::question (
          this, us_tr ("Load Browflex Files"),
          QString (
              us_tr ("The batch operation window currently has files loaded.\n"
        ...
        case QMessageBox::Cancel:
          editor_msg (
              "red",
              "Loading of models into batch operation window canceled\n");
          return;
          break;
        case QMessageBox::No:
        default:
...
int
US_Hydrodyn::browflex_get_no_of_beads (QString filename)
{
#if defined(DEBUG_CONN)
