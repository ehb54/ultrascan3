...

  {
    ...
    for (unsigned int i = 0; i < (unsigned int)completed_files.count(); i++) {
      completed_jobs[completed_files[i].replace(
          QRegExp("_(out|OUT)\\.(tar|tgz|TAR|TGZ)$"), "")] = true;
    }
  }
...
void US_Hydrodyn_Cluster::cancel() { close(); }
...
void US_Hydrodyn_Cluster::add_target() {
  ...
  QStringList filenames =
      QFileDialog::getOpenFileNames(this, "Set files for grid target", use_dir,
                                    "All files (*);;"
                                    "ssaxs files (*.ssaxs);;"
                                    "csv files (*.csv);;"
...
void US_Hydrodyn_Cluster::create_pkg() {
  ...
  if (cb_for_mpi->isChecked()) {
    ...
    if (QFile::exists(tar_filename)) {
      tar_filename =
          ((US_Hydrodyn *)us_hydrodyn)->fileNameCheck(tar_filename, 0, this);
  ...
  if (!QDir::setCurrent(base_dir)) {
    editor_msg(
        "red",
        QString(us_tr("Can not find output directory %1\n")).arg(base_dir));
    return;
  }

  {
    ...
    if (qfi.exists() && !qfi.isFile()) {
      editor_msg("red", QString(us_tr("Output file \"%1\" already exists and "
  ...
  base += QString("ResidueFile     %1\n")
              .arg(common_prefix +
                   QFileInfo(((US_Hydrodyn *)us_hydrodyn)->lbl_table->text())
                       .fileName());
  ...
  base +=
      QString("AtomFile        %1\n")
          .arg(common_prefix +
               QFileInfo(our_saxs_options->default_atom_filename).fileName());
  base_source_files << our_saxs_options->default_atom_filename;
  base +=
      QString("HybridFile      %1\n")
          .arg(common_prefix +
               QFileInfo(our_saxs_options->default_hybrid_filename).fileName());
  base_source_files << our_saxs_options->default_hybrid_filename;
  base +=
      QString("SaxsFile        %1\n")
          .arg(common_prefix +
               QFileInfo(our_saxs_options->default_saxs_filename).fileName());
  ...
  if (our_saxs_options->use_somo_ff) {
    base +=
        QString("FFFile          %1\n")
            .arg(common_prefix +
                 QFileInfo(our_saxs_options->default_ff_filename).fileName());
    ...
    if (our_saxs_options->saxs_iq_foxs) {
      if (batch_window->cb_mm_all->isChecked()) {
        US_Static::us_message(
            us_tr("Please note:"),
            us_tr("You have selected to process all models in multi-model PDBs "
                  "with the FoXS external I(q) method.\n"
                  "If you are including multi-model PDBs, a single composite "
                  "curve will be produced.\n"
                  "If you wish curves for individual models, either split the "
                  "multi model PDB\n"
                  "or use the \"fast\" Iq method.\n"
                  "You can split the multi-model PDB into individual files "
                  "using the PDB Editor / Split button."));
      }
      iqmethod = "foxs";
    }
    if (our_saxs_options->saxs_iq_crysol) {
      if (batch_window->cb_mm_all->isChecked()) {
        US_Static::us_message(
            us_tr("Please note:"),
            us_tr("You have selected to process all models in multi-model PDBs "
                  "with the CRYSOL external I(q) method.\n"
                  "If you are including multi-model PDBs, only the first model "
                  "will actually be computed by CRYSOL.\n"
                  "If you wish curves for individual models, split the multi "
                  "model PDB into individual PDBs.\n"
                  "You can split the multi-model PDB into individual files "
                  "using the PDB Editor / Split button."));
      }
      iqmethod = "crysol";
  ...
  if (lb_target_files->count()) {
    if (lb_target_files->count() == 1 || !cb_split_grid->isChecked()) {
      base += QString("ExperimentGrid  %1\n")
                  .arg(common_prefix +
                       QFileInfo(lb_target_files->item(0)->text()).fileName());
  ...
  for (unsigned int i = 0; i < (unsigned int)selected_files.size(); i++) {
    if (batch_window->cb_dmd->isChecked() &&
      ...
      for (unsigned int j = 0;
           j < dmd_entry_count(QFileInfo(selected_files[i]).fileName()); j++) {
        use_selected_files
  ...
  for (unsigned int this_i = 0;
    ...
    if (multi_grid_multiplier > 1) {
      unsigned int grid = (this_i / (unsigned int)use_selected_files.size()) %
                          multi_grid_multiplier;
      out +=
          QString("ExperimentGrid  %1\n")
              .arg(common_prefix +
                   QFileInfo(lb_target_files->item(grid)->text()).fileName());
    ...
    if (advanced_multiplier > 1) {
      advanced_pos = this_i / ((unsigned int)use_selected_files.size() *
                               multi_grid_multiplier);
    }
  ...
  for (unsigned int i = 0; i < (unsigned int)base_source_files.size(); i++) {
    base_remove_file_list << pkg_dir + SLASH +
                                 QFileInfo(base_source_files[i]).fileName();
  }
  ...
  if (submitted_jobs.count(le_output_name->text())) {
    US_Static::us_message(
        ...
        us_tr("There is a previously submitted job with the same name\n"
              "Please change the output base name (job identifier) to a unique "
              "value\n"
...
void US_Hydrodyn_Cluster::split_grid() {
  ...
  if (cb_for_mpi->isChecked() && cb_split_grid->isChecked() &&
      lb_target_files->count()) {
    le_no_of_jobs->setText(
        QString("%1").arg(selected_files.size() * lb_target_files->count()));
  }
}
...
void US_Hydrodyn_Cluster::dmd() {
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->dmd_failed_validation) {
    QStringList errors;
    for (auto it =
             ((US_Hydrodyn *)us_hydrodyn)->dmd_all_pdb_prepare_reports.begin();
...
QStringList US_Hydrodyn_Cluster::advanced_addition() {
  ...
  for (unsigned int i = 0; i < (unsigned int)csv_advanced.data.size(); i++) {
    for (unsigned int i = 0; i < (unsigned int)csv_advanced.data.size(); i++) {
      if (csv_advanced.data[i].size() > 4 && csv_advanced.data[i][1] == "Y" &&
...
QString US_Hydrodyn_Cluster::dmd_base_addition(QStringList &base_source_files,
  ...
  QStringList files;
  QString dmd_base_dir =
      USglobal->config_list.system_dir + SLASH + "etc" + SLASH + "dmd" + SLASH;
...
unsigned int US_Hydrodyn_Cluster::dmd_entry_count(QString inputfile) {
  ...
  for (unsigned int i = 0; i < (unsigned int)csv_dmd.prepended_names.size();
       i++) {
    if (inputfile == csv_dmd.prepended_names[i] && csv_dmd.data[i].size() > 5 &&
        csv_dmd.data[i][1] == "Y") {
...
QString US_Hydrodyn_Cluster::dmd_file_addition(QString inputfile,
  ...
  if (use_entry != -1) {
    if (use_entry >= (int)active_csv_rows.size()) {
      editor_msg(
          "red",
          QString(us_tr("Internal Error: issue creating dmd entry for %1 "
  ...
  if (use_entry == -1 && active_csv_rows.size() > 1) {
    ...
    for (unsigned int j = 0; j < (unsigned int)active_csv_rows.size(); j++) {
      ...
      if (this_static_range != static_range) {
        editor_msg(
            "red",
            QString(us_tr("Error: multiple (%1) same named DMD (%2) entries "
  ...
  if (active_csv_rows.size() < 1) {
    editor_msg(
        "red",
        QString(us_tr("Internal Error: no active DMD rows %1 position %2.\n"))
            .arg(inputfile)
            .arg(use_entry));
    return "";
  }
  ...
  for (unsigned int j = 0; j < (unsigned int)active_csv_rows.size(); j++) {
    ...
    bool relax_ok = true;
    bool relax_is_on =
        (csv_dmd.data[i][3].toFloat() > 0 || csv_dmd.data[i][5].toFloat() > 0);
...
bool US_Hydrodyn_Cluster::validate_csv_dmd(unsigned int &number_active) {
  ...
  for (unsigned int i = 0; i < (unsigned int)csv_dmd.prepended_names.size();
    ...
    if (!selected_map.count(csv_dmd.prepended_names[i])) {
      ...
      if (csv_dmd.data[i].size() > 1 && csv_dmd.data[i][1] == "Y") {
        errormsg +=
            QString(us_tr("Error: %1 is marked as Active in the DMD settings, "
                          "but the file is not present in the batch selected "
                          "files\n"))
                .arg(csv_dmd.prepended_names[i]);
      } else {
        noticemsg +=
            QString(
                us_tr("Notice: %1 is in the DMD settings and is not Active, "
                      "but the file is not present in the batch selected "
                      "files\n"))
                .arg(csv_dmd.prepended_names[i]);
      }
      ...
      if (csv_dmd.data[i].size() > 1 && csv_dmd.data[i][1] == "Y") {
        ...
        if (csv_dmd.data[i].size() < 6) {
          errormsg += QString(us_tr("Error: %1 is marked as Active but has "
                                    "insufficient DMD settings, "
                                    "at least a Relax temp, time & output "
          ...
          if (relax_is_on && csv_dmd.data[i][2].toFloat() <= 0) {
            errormsg += QString(us_tr("Error: %1 is Active and does not have a "
                                      "positive Relax temp\n"))
                            .arg(csv_dmd.prepended_names[i]);
            relax_ok = false;
          }
          ...
          if (csv_dmd.data[i][5].toFloat() <= 0 && csv_dmd.data[i].size() > 9 &&
              csv_dmd.data[i][9].toFloat() <= 0) {
            errormsg +=
                QString(us_tr("Error: %1 is Active and the Relax output count "
                              "and the Run output count are not positive\n"))
                    .arg(csv_dmd.prepended_names[i]);
            relax_ok = false;
          }
          if (csv_dmd.data[i].size() > 9) {
            ...
            if (run_is_on && csv_dmd.data[i][6].toFloat() <= 0) {
              errormsg += QString(us_tr("Error: %1 is Active and does not have "
                                        "a positive Run temp\n"))
                              .arg(csv_dmd.prepended_names[i]);
              run_ok = false;
            }
            ...
            if (run_is_on && csv_dmd.data[i][9].toFloat() <= 0) {
              errormsg += QString(us_tr("Error: %1 is Active and the Run "
          ...
          if (!run_ok && !relax_ok) {
            errormsg +=
                QString(
                    us_tr(
                        "Error: %1 is Active neither the Relax or Run is ok\n"))
                    .arg(csv_dmd.prepended_names[i]);
          }
        }
      }
  ...
  for (map<QString, bool>::iterator it = selected_map.begin();
    ...
    if (!present_map.count(it->first)) {
      noticemsg +=
          QString(us_tr("Notice: %1 is a batch selected file but is not "
...
QString US_Hydrodyn_Cluster::options_summary() {
  ...
  if (active_additional_methods().size()) {
    ...
    qs += us_tr(
        "Note: current active methods take precedence.\n"
  ...
  switch (QMessageBox::question(
    ...
    case QMessageBox::Yes: {
      ...
      if (!ufu.move(configfile, configcorruptfile, true)) {
        editor_msg(
            "red",
            QString(us_tr("Error: Could not rename corrupt cluster "
                          "configuration file:" +
                          ufu.errormsg +
        ...
        editor_msg("blue", QString(us_tr("Notice: Renamed corrupt cluster "
  ...
  while (!ts.atEnd()) {
    ...
    if (option == "system") {
      ...
      if (cluster_systems.count(last_system)) {
        errormsg =
            QString("Error reading %1 line %2 : system %3 multiply defined.")
                .arg(configfile)
                .arg(line)
                .arg(qsl[0]);
    ...
    if (rx_systems.indexIn(option) != -1) {
      if (last_system.isEmpty()) {
        errormsg = QString(
                       "Error reading %1 line %2 : system must be specified "
                       "before %3 ")
                       .arg(configfile)
                       .arg(line)
                       .arg(option);
  ...
  for (int i = 0; i < (int)methods.size(); i++) {
    if (!i) {
      ...
      first_one_pdb_exactly =
          cluster_additional_methods_one_pdb_exactly.count(methods[i]) ? true
                                                                       : false;
    } else {
...
void US_Hydrodyn_Cluster::create_additional_methods_pkg(QString base_dir,
  ...
  if (active_additional_prepend_issue()) {
    editor_msg("red", us_tr("Multiple other methods selected that are "
                            "currently incompatible for a single job package\n"
  ...
  if (selected_files.size() != 1) {
    for (int i = 0; i < (int)methods.size(); i++) {
      if (cluster_additional_methods_one_pdb_exactly.count(methods[i])) {
        editor_msg("red", us_tr(QString("The addtional method %1 requires "
  ...
  if (cluster_additional_methods_parallel_mpi.count(methods[0])) {
    return create_additional_methods_parallel_pkg(base_dir, filename,
                                                  common_prefix, use_extension);
  }
  ...
  for (int m = 0; m < (int)methods.size(); m++) {
    if (cluster_additional_methods_modes.count(
        ...
        cluster_additional_methods_modes["additional_processing_global"].count(
            methods[m])) {
      if (!additional_processing(base, base_source_files,
                                 "additional_processing_global", methods[m])) {
        editor_msg(
            "red",
            us_tr("Internal error:additional processing per file error"));
      }
    }
  }
      ...
      if (cluster_additional_methods_options_selected->count(methods[m])) {
        for (map<QString, QString>::iterator it =
          ...
          if (!it->first.contains("file")) {
            ...
            if (cluster_additional_methods_modes["acceptcommon"].count(
                    methods[m])) {
              out += QString("%1\t%2\n")
                         .arg(it->first)
                         .arg(common_prefix + QFileInfo(it->second).fileName());
        ...
        if (cluster_additional_methods_modes.count("one_run_per_file") &&
            cluster_additional_methods_modes["one_run_per_file"].count(
                methods[m])) {
          for (unsigned int j = 0; j < (unsigned int)selected_files.size();
               ++j) {
            if ((int)j % jobs == i) {
              QString use_filename = selected_files[j];
              if (cluster_additional_methods_modes["inputfile_pat_addendum"]
                      .count(methods[m])) {
                use_filename = QFileInfo(selected_files[j]).baseName() +
                               "_pat" + "." +
                               QFileInfo(selected_files[j]).completeSuffix();
              }

              out += QString("%1 %2\n")
                         .arg(cluster_additional_methods_modes.count(
                                  "inputfilenoread") &&
              ...
              if (!already_added.count(selected_files[j])) {
                ...
                if (cluster_additional_methods_modes.count(
                    ...
                    cluster_additional_methods_modes
                        ["additional_processing_per_file"]
                            .count(methods[m])) {
                  if (!additional_processing(out, source_files_to_clear,
                                             "additional_processing_per_file",
                                             methods[m], selected_files[j])) {
                    editor_msg("red", us_tr("Internal error:additional "
...
void US_Hydrodyn_Cluster::create_additional_methods_parallel_pkg(
  ...
  // naming, otherwise it will require submission to inspect the package package
  // here is one tgz
...
static void combo_with_replacement(vector<int> &elems, unsigned int req_len,
                                   vector<int> &pos,
...
void US_Hydrodyn_Cluster::create_additional_methods_parallel_pkg_bfnb(
    QString filename) {
  QStringList methods = active_additional_methods();
  ...
  if (!lb_target_files->count()) {
    if (cluster_additional_methods_require_experimental_data.count(
            methods[0])) {
      errors += QString(us_tr("Selected other method %1 requires experimental "
                              "data and none are listed\n"))
  ...
  if (!(*cluster_additional_methods_options_selected)[methods[0]].count(
    ...
    editor_msg(
        "dark red",
        us_tr("Notice: setting sample e density to protein average of .425"));
  }

  if (!(*cluster_additional_methods_options_selected)[methods[0]].count(
          "pmbufferedensity")) {
    (*cluster_additional_methods_options_selected)
        [methods[0]]["pmbufferedensity"] =
            QString("%1").arg(our_saxs_options->water_e_density);
    editor_msg(
        "dark red",
        QString(
            us_tr(
                "Notice: setting buffer e density to SAS Options value of %1"))
            .arg(our_saxs_options->water_e_density));
  }
  ...
  if (req.size()) {
    for (map<QString, bool>::iterator it = req.begin(); it != req.end(); it++) {
    ...
    if ((*cluster_additional_methods_options_selected)[methods[0]].count(
        ...
        !(*cluster_additional_methods_options_selected)
             [methods[0]]["pmbestcoarseconversion"]
                 .isEmpty()) {
      pmapproxmaxdimension = "pmapproxmaxdimension\t" +
                             (*cluster_additional_methods_options_selected)
                                 [methods[0]]["pmbestcoarseconversion"] +
                             "\n";
    } else {
  ...
  if ((*cluster_additional_methods_options_selected)[methods[0]].count(
    ...
    if ((*cluster_additional_methods_options_selected)[methods[0]].count(
      ...
      for (int i = 1; i <= (int)qsl_pmtypes.size(); ++i) {
        ...
        for (int j = 0; j < (int)results.size(); ++j) {
          out_per_experimental_dataset +=
              QString("pmoutname %1___EXPERIMENT_NAME___-").arg(pmoutprefix);
          ...
          for (int k = 0; k < (int)results[j].size(); ++k) {
            out_per_experimental_dataset +=
                QString("%1%2").arg(k ? "," : "").arg(results[j][k]);
          }
          out_per_experimental_dataset += "\n";
      ...
      for (int i = 0; i < (int)qsl_pmtypes.size(); ++i) {
        ...
        for (int j = 0; j <= i; ++j) {
          out_per_experimental_dataset +=
              QString("%1%2").arg(j ? "," : "").arg(qsl_pmtypes[j]);
        }
        out_per_experimental_dataset += "\n";
        out_per_experimental_dataset += "pmbestga\n";
        tot_runs++;
  ...
  for (int i = 0; i < lb_target_files->count(); i++) {
    ...
    // 'g', 8 later add pmf via "pmusedummyatomff" option and possible q editing
    // (log binning), qmax, max pts ?
    vector<double> q;
    vector<double> I;
    vector<double> e;
    QString error_msg;
    ...
    if (!US_Saxs_Util::read_sas_data(target_file, q, I, e, error_msg)) {
      ...
      if (q.size() < 3) {
        errors += QString(
                      "Error: After cropping and binning, there are too few q "
                      "points left (%1)")
                      .arg(q.size());
      }
...

  {
    ...
    if (!f.open(QIODevice::WriteOnly)) {
      editor_msg(
          "red",
          QString(us_tr("Error: could not create file %1")).arg(use_file));
      return;
    }
...
bool US_Hydrodyn_Cluster::additional_processing(QString &out,
                                                QStringList &source_files,
                                                const QString &type,
  ...
  if (method == "best") {
    if (type == "additional_processing_per_file") {
      ...
      if ((*cluster_additional_methods_options_selected).count(method) &&
          (*cluster_additional_methods_options_selected)[method].count(
              "bestbestmw")) {
        mw =
            (*cluster_additional_methods_options_selected)[method]["bestbestmw"]
                .toDouble();
      }
      if ((*cluster_additional_methods_options_selected).count(method) &&
          !(*cluster_additional_methods_options_selected)[method].count(
        ...
        if (file.contains(QRegExp(".(pdb|PDB)$")) &&
          ...
          if (batch_window->screen_pdb(file, false)) {
            ...
            if (((US_Hydrodyn *)us_hydrodyn)->saxs_util->pdb_mw(file, mw)) {
              editor_msg(
                  "dark red",
                  QString(us_tr("Notice: direct atom method for computation of "
                                "molecular weight of %1 used"))
                      .arg(file));
            } else {
              editor_msg(
                  "red",
                  QString(us_tr("Error: direct atom method for computation of "
                                "molecular weight of %1 failed"))
                      .arg(file));
              ok = false;
            }
          }
          ...
          if (ok) {
            editor_msg("blue",
                       QString(us_tr("Molecular weight of %1: %2 Daltons\n"))
            ...
            if (!US_Saxs_Util::pat_model(
              ...
              editor_msg("red",
                         QString(us_tr("Error: perform PAT on  %1")).arg(file));
            } else {
              ...
              QString patfile = dir + QFileInfo(file).baseName() + "_pat" +
                                "." + QFileInfo(file).completeSuffix();
              if (!US_Saxs_Util::write_model(
                      ((US_Hydrodyn *)us_hydrodyn)->model_vector[0], patfile)) {
                editor_msg(
                    "red",
                    QString(us_tr("Error: writing PAT'd pdb %1")).arg(patfile));
              } else {
    ...
    if (type == "additional_processing_global") {
      ...
      if ((*cluster_additional_methods_options_selected).count(method) &&
          (*cluster_additional_methods_options_selected)[method].count(
              "bestexpand") &&
          (*cluster_additional_methods_options_selected)[method]["bestexpand"]
                  .toDouble() > 0e0) {
        double mult =
            (*cluster_additional_methods_options_selected)[method]["bestexpand"]
                .toDouble();
        editor_msg("dark blue",
                   QString(us_tr("Radii will be multiplied by %1")).arg(mult));
      ...
      QString dir = ((US_Hydrodyn *)us_hydrodyn)->somo_dir + QDir::separator() +
                    "tmp" + QDir::separator();
      ...
      if (!(*cluster_additional_methods_options_selected)[method].count(
        ...
        (*cluster_additional_methods_options_selected)
            [method]["bestmsrradiifile"] =
                QString(USglobal->config_list.system_dir + QDir::separator() +
                        "etc" + QDir::separator() + "best.radii");
        editor_msg("blue",
                   QString(us_tr("Notice: using default BEST radii  file: %1"))
                       .arg((*cluster_additional_methods_options_selected)
                                [method]["bestmsrradiifile"]));
      }
      ...
      if ((*cluster_additional_methods_options_selected)[method].count(
        ...
        if ((*cluster_additional_methods_options_selected)[method].count(
                "bestmsrradiifile")) {
          (*cluster_additional_methods_options_selected)[method].erase(
              "bestmsrradiifile");
        }
        if ((*cluster_additional_methods_options_selected)[method].count(
                "bestmsrpatternfile")) {
      ...
      if (!(*cluster_additional_methods_options_selected)[method].count(
        ...
        if (!f_names.open(QIODevice::WriteOnly)) {
          ...
          for (unsigned int i = 0;
               i <
               (unsigned int)((US_Hydrodyn *)us_hydrodyn)->msroll_names.size();
               i++) {
      ...
      if ((*cluster_additional_methods_options_selected)[method].count(
        ...
        if ((*cluster_additional_methods_options_selected).count(method)) {
          if ((*cluster_additional_methods_options_selected)[method].count(
              ...
              (*cluster_additional_methods_options_selected)[method].count(
                  "bestexpand")) {
            editor_msg("red", us_tr("Manual radii file does not support water "
                                    "radius or expansion"));
            return false;
          }
        }
