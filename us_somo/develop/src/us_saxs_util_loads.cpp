    ...
    while (!ts.atEnd()) {
      ...
      for (j = 0; j < numatoms; j++) {
        ...
        if (new_atom.bead_assignment >= numbeads) {
          noticemsg += QString(
                           "Warning: The atom's bead assignment has exceeded "
                           "the number of beads."
        ...
        if (!new_atom.name.isEmpty() && new_atom.hybrid.radius > 0.0 &&
          ...
          noticemsg +=
              "Notice: There was an error reading the selected Residue File. "
              "Atom" +
              new_atom.name +
...
bool US_Saxs_Util::setup_saxs_maps(QString atom_file, QString hybrid_file,
                                   QString saxs_file) {
    ...
    for (unsigned int j = 0; j < model_vector[i].molecule.size(); j++) {
      ...
      if (convert_this) {
        if (!already_messaged.count(QString("%1|%2").arg(i).arg(chainID))) {
          ...
          noticemsg += QString(
                           "Converting Chain %1 Molecule %2 to standard DNA "
                           "residue names\n")
                           .arg(chainID)
                           .arg(i + 1);
        }
...
bool US_Saxs_Util::assign_atom(const QString &str1,
  ...
  *last_resSeq = temp_atom.resSeq;  // update last residue sequence number
  ...
  if (!found) {
    //   printError(us_tr("The residue " + temp_atom.resName + " listed in this
    //   PDB file is not found in the residue table!"));
  }
...
void US_Saxs_Util::clear_temp_chain(
    PDB_chain
