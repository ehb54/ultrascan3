...
bool
US_Saxs_Util::select_residue_file (QString filename)
{
  errormsg = "";
  noticemsg = "";
          ...
          for (j = 0; j < numatoms; j++)
            {
              ...
              if (new_atom.bead_assignment >= numbeads)
                {
                  noticemsg += QString ("Warning: The atom's bead assignment "
                                        "has exceeded the number of beads."
              ...
              if (!new_atom.name.isEmpty () && new_atom.hybrid.radius > 0.0
                ...
                {
                  noticemsg
                      += "Notice: There was an error reading the selected "
                         "Residue File. Atom"
                         + new_atom.name
  ...
  if (multi_residue_map["OXT"].size () == 1
    ...
    {
      noticemsg += "Warning: PBR rule OXT not replaced with OXT-P because "
...
void
US_Saxs_Util::calc_bead_mw (residue *res)
{
  double rmw = 0.0;
...
void
US_Saxs_Util::calc_vbar (PDB_model *model)
{
...
bool
US_Saxs_Util::setup_saxs_maps (QString atom_file, QString hybrid_file,
                               QString saxs_file)
{
  return select_atom_file (atom_file) && select_hybrid_file (hybrid_file)
         && select_saxs_file (saxs_file);
}

bool
US_Saxs_Util::select_atom_file (QString filename)
{
  errormsg = "";
  QString str1;
...
bool
US_Saxs_Util::select_hybrid_file (QString filename)
{
  errormsg = "";
  QString str1;
  QFileInfo fi (filename);
...
bool
US_Saxs_Util::select_saxs_file (QString filename)
{
  errormsg = "";
  QString str1;
  QFileInfo fi (filename);
  ...
  if (f.open (QIODevice::ReadOnly | QIODevice::Text))
      ...
      while (!ts.atEnd ())
        {
          ...
          cout << QString ("Warning: %1 on line %2, invalid number of tokens, "
                           "ignoring")
                      .arg (filename)
                      .arg (line);
        }
    ...
    {
      errormsg
          = QString ("Error can not read saxs atom file %1").arg (filename);
      return false;
    }
}

bool
US_Saxs_Util::read_pdb (QStringList &qsl)
{
  errormsg = "";
  noticemsg = "";
              ...
              if (str1.mid (16, 1) == " " || str1.mid (16, 1) == "A")
                {
                  ...
                  if (!chain_flag) // at the first time we encounter the word
                                   // ATOM
                  ...
                  else // we have a chain, let's make sure the chain is still
                       // the same
                    {
                      bool break_chain
                          = (temp_chain.chainID != str1.mid (21, 1));
                      ...
                      bool this_is_aa
                          = (known_residue
                             && residue_list[multi_residue_map[thisResName][0]]
                                        .type
                                    == 0);
                  ...
                  if (assign_atom (str1, &temp_chain,
                                   &last_resSeq)) // parse the current line and
                                                  // add it to temp_chain
                    { // if true, we have new residue and need to add it to the
                      // residue vector
...
bool
US_Saxs_Util::read_pdb (QString filename)
{
  errormsg = "";
  noticemsg = "";
          ...
          if (ask_convert)
            {
              errormsg
                  = QString (
                        "Error: Chain %1 Molecule %2 only contains A, G & C "
                        "residues (so far), can not determine DNA or RNA")
                        .arg (chainID)
                        .arg (i + 1);
              return false;
            }
          if (convert_this)
            {
              if (!already_messaged.count (
                      QString ("%1|%2").arg (i).arg (chainID)))
                {
                  ...
                  noticemsg += QString ("Converting Chain %1 Molecule %2 to "
...
bool
US_Saxs_Util::assign_atom (const QString &str1, struct PDB_chain *temp_chain,
                           QString *last_resSeq)
{
  /*
  ...
  *last_resSeq = temp_atom.resSeq; // update last residue sequence number
  ...
  if (!found)
    {
      //   printError(us_tr("The residue " + temp_atom.resName + " listed in
      //   this PDB file is not found in the residue table!"));
    }
...
float
US_Saxs_Util::mw_to_volume (float mw, float vbar)
{
...
bool
US_Saxs_Util::load_mw_json (QString filename)
{
  QFile f (filename);
  if (!f.open (QIODevice::ReadOnly))
    {
      errormsg = QString ("US_Saxs_Util::load_mw_json could not open file %1")
                     .arg (filename);
      return false;
    }
  QString qs;
  ...
  for (map<QString, QString>::iterator it = parameters.begin ();
       it != parameters.end (); it++)
    {
      ...
      // us_qdebug( QString( "atom '%1' mw '%2'" ).arg( it->first ).arg(
      // it->second.toDouble() ) );
    }
  return true;
}
...
bool
US_Saxs_Util::load_vdw_json (QString filename)
{
  QFile f (filename);
  if (!f.open (QIODevice::ReadOnly))
    {
      errormsg = QString ("US_Saxs_Util::load_vdw_json could not open file %1")
                     .arg (filename);
      return false;
    }
  QString qs;
...
bool
US_Saxs_Util::load_vcm_json (QString filename)
{
  QFile f (filename);
  if (!f.open (QIODevice::ReadOnly))
    {
      errormsg = QString ("US_Saxs_Util::load_vcm_json could not open file %1")
                     .arg (filename);
      return false;
    }
  QString qs;
  ...
  for (map<QString, QString>::iterator it = parameters.begin ();
       it != parameters.end (); it++)
    {
      ...
      // us_qdebug( US_Vector::qs_vector( "vcm[" + it->first + "]", vcm[
      // it->first ] ) );
    }
  ...
  for (set<QString>::iterator it = to_spline.begin (); it != to_spline.end ();
       it++)
    {
      natural_spline (vcm["q"], vcm[*it], vcm[*it + ":y2"]);

      // us_qdebug( US_Vector::qs_vector( "vcm[" + *it + ":y2]", vcm[ *it +
      // ":y2" ] ) );
    }
