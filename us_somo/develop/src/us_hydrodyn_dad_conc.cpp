#include "../include/us_hydrodyn_dad_conc.h"

#include "../include/us3_defines.h"
#include "../include/us_hydrodyn.h"
#include "../include/us_hydrodyn_dad.h"
#include "../include/us_hydrodyn_dad_conc_load.h"
...
US_Hydrodyn_Dad_Conc::US_Hydrodyn_Dad_Conc(csv &csv1, void *dad_window,
                                           QWidget *p, const char *)
    : QFrame(p) {
  org_csv = &csv1;
  this->csv1 = csv1;
  this->dad_window = dad_window;
  ((US_Hydrodyn_Dad *)dad_window)->conc_widget = true;
...
void US_Hydrodyn_Dad_Conc::refresh(csv &ref_csv) {
  csv new_csv = ref_csv;
...
US_Hydrodyn_Dad_Conc::~US_Hydrodyn_Dad_Conc() {}

void US_Hydrodyn_Dad_Conc::setupGUI() {
  int minHeight1 = 30;
...
void US_Hydrodyn_Dad_Conc::cancel() { close(); }

void US_Hydrodyn_Dad_Conc::set_ok() {
  *org_csv = current_csv();
  ((US_Hydrodyn_Dad *)dad_window)->update_enables();
  close();
}

void US_Hydrodyn_Dad_Conc::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/somo_dad_conc.html");
}

void US_Hydrodyn_Dad_Conc::closeEvent(QCloseEvent *e) {
  ((US_Hydrodyn_Dad *)dad_window)->conc_widget = false;
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept();
}

void US_Hydrodyn_Dad_Conc::sort_column(int section) {
  t_csv->sortByColumn(
      section, order_ascending ? Qt::AscendingOrder : Qt::DescendingOrder);
  order_ascending = !order_ascending;
}

csv US_Hydrodyn_Dad_Conc::current_csv() {
  csv tmp_csv = csv1;
...
void US_Hydrodyn_Dad_Conc::copy() {
  csv1 = current_csv();
...
void US_Hydrodyn_Dad_Conc::paste() {
  csv1 = current_csv();
  unsigned int pos = 0;
  for (int i = 0; i < t_csv->rowCount(); i++) {
...
void US_Hydrodyn_Dad_Conc::paste_all() {
  csv1 = current_csv();
  unsigned int pos = 0;
  for (int i = 0; i < t_csv->rowCount(); i++) {
    for (unsigned int j = 1;
...
void US_Hydrodyn_Dad_Conc::update_enables() {
  if (!disable_updates) {
...
void US_Hydrodyn_Dad_Conc::row_header_released(int row) {
...
void US_Hydrodyn_Dad_Conc::reload_csv() {
  t_csv->setRowCount(csv1.data.size());
  for (unsigned int i = 0; i < csv1.data.size(); i++) {
    for (unsigned int j = 0; j < csv1.data[i].size(); j++) {
      t_csv->setItem(i, j, new QTableWidgetItem(csv1.data[i][j]));
...
void US_Hydrodyn_Dad_Conc::load() {
  ...
  if (!QFile::exists(fname)) {
    QMessageBox::warning(
        this, us_tr("US-SOMO: UV-Vis Subtraction Utility: Concentrations"),
  ...
  if (QFileInfo(fname).suffix().contains(QRegExp("^(sbc|SBC)$"))) {
    ...
    US_Hydrodyn_Dad_Conc_Load *hbscl =
        new US_Hydrodyn_Dad_Conc_Load(qsl_lines, csv1, this);
    US_Hydrodyn::fixWinButtons(hbscl);
    if (hbscl->disp_csv.data.size()) {
      hbscl->exec();
    } else {
      QMessageBox::warning(this, us_tr("US-SOMO: UV-Vis: File Concentrations"),
...
void US_Hydrodyn_Dad_Conc::save() {
  QString use_dir = QDir::currentPath();
...
QString US_Hydrodyn_Dad_Conc::csv_to_qstring(csv from_csv) {
  QString qs;
...
QStringList US_Hydrodyn_Dad_Conc::csv_parse_line(QString qs) {
