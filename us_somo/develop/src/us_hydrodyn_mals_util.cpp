...
QStringList US_Hydrodyn_Mals::get_frames(QStringList files, QString head,
                                         QString tail) {
  QStringList result;
...
double US_Hydrodyn_Mals::tot_intensity(QString &file, double q_min,
                                       double q_max) {
...
void US_Hydrodyn_Mals::set_created_selected(set<QString> &to_select,
                                            bool do_replot) {
  disable_updates = true;
...
int US_Hydrodyn_Mals::slice(const vector<double> &q, const vector<double> &I,
...
double US_Hydrodyn_Mals::wyatt_errors(QString file, double start_q,
                                      double end_q) {
  if (!f_qs.count(file)) {
    return -1e0;
  }
...
void US_Hydrodyn_Mals::cormap(map<QString, QString> &parameters) {
  ...
  double cormap_maxq =
      ((US_Hydrodyn *)us_hydrodyn)->gparams.count("mals_cormap_maxq")
          ? ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_cormap_maxq"].toDouble()
          : 0.05;
...
QStringList US_Hydrodyn_Mals::vector_qstring_to_qstringlist(
    vector<QString> vqs) {
  QStringList qsl;
...
bool US_Hydrodyn_Mals::mals_load(const QString &filename,
                                 const QStringList &qsl, QString &errormsg) {
  errormsg = "";
  ...
  if (!mals_angles.mals_angle.size()) {
    errormsg =
        "No MALS Angles have been defined.\nLoad them prior to loading MALS "
        "data";
    return false;
  }
  ...
  if (!mals_param_lambda || !mals_param_n) {
    errormsg =
        "Options->MALS parameters must be defined prior to loading MALS data";
    return false;
  }

  switch (QMessageBox::question(
      this, windowTitle() + us_tr(" : Load MALS Data"),
      mals_angles.list_rich() + QString("<hr>"
...
#define TIME_DELTA_TOLERANCE 0.0000001
  ...
  if (timedelta_inconsistencies) {
    QMessageBox::warning(
        ...
        QString(us_tr("%1 inconsistent time interval(s) found in MALS data\n"
  ...
  for (auto it = t.begin(); it != t.end(); ++it) {
    if (mals_angles.mals_angle.count(it->first) &&
      ...
      double q =
          RI * 4 * M_PI *
          sin(mals_angles.mals_angle[it->first].angle_ri_corr * M_PI / 360) /
          (wavelength * 10);
  ...
  if (data.size() &&
    ...
    switch (
        QMessageBox::question(this, windowTitle() + us_tr(" : Load MALS Data"),
                              us_tr("The MALS data file contains UV absorbance "
...
bool US_Hydrodyn_Mals::mals_angles_save() {
  ...
  if (mals_angles.mals_angle.size()) {
    ...
    QString use_filename = QFileDialog::getSaveFileName(
        this, us_tr("Select a file name for saving the MALS Angles"), use_dir,
        "*.csv");
