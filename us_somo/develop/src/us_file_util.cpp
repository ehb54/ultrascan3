...
bool
US_File_Util::copy (QString from, QString to, bool overwrite, QString qsheader)
{
  ...
  if (!QFile (from).exists ())
    {
      errormsg = QString ("Copy from file %1 does not exist (current dir %2, "
                          "copying to %3)")
                     .arg (from)
  ...
  if (!overwrite && QFile (to).exists () && !QFileInfo (to).isDir ())
    {
    ...
    // I didn't try QFile::readBlock, ::writeBlock, so that may make the
    // difference
      ...
      while (bytes_left
             && (bytes_written = fwrite (buf + pos, 1, bytes_left, fout)))
        {
          if (bytes_left >= bytes_written)
            ...
            {
              bytes_left = 0;
              errormsg = QString ("Copy: Error reading file %1 read overrun")
                             .arg (from);
              fclose (fin);
              fclose (fout);
              QFile::remove (to);
...
bool
US_File_Util::copy (QStringList from, QString toDir, bool overwrite)
{
  errormsg = "";
  if (!QFileInfo (toDir).isDir ())
    {
      errormsg = QString ("Copy: destination %1 is not a directory for a "
                          "multi file copy")
                     .arg (toDir);
      return false;
    }
...
bool
US_File_Util::move (QString from, QString to, bool overwrite)
{
  errormsg = "";
...
bool
US_File_Util::move (QStringList from, QString toDir, bool overwrite)
{
  errormsg = "";
...
bool
US_File_Util::read (QString from, QStringList &qsl)
{
  errormsg = "";
  qsl.clear ();
  QFile f (from);
  if (!f.open (QIODevice::ReadOnly | QIODevice::Text))
    {
      errormsg = QString ("US_File_Util:read() can not open %1 for reading")
                     .arg (from);
      return false;
    }
...
bool
US_File_Util::writeuniq (QString &outname, QString &msg, QString basename,
                         QString ext, QString &data)
{
...
void
US_Log::log (const QStringList &qsl)
{
...
bool
US_File_Util::getcontents (QString file, QString &contents, QString &error)
{
  error = "";
  contents = "";
  QFile f (file);
...
bool
US_File_Util::putcontents (QString file, QString &contents, QString &error)
{
  error = "";
  QFile f (file);
...
bool
US_File_Util::diff (QString file1, QString file2)
{
  QString error;
  QString contents1;
  QString contents2;
