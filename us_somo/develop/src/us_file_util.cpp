...
bool US_File_Util::copy(QString from, QString to, bool overwrite,
  ...
  if (!QFile(from).exists()) {
    errormsg =
        QString(
            "Copy from file %1 does not exist (current dir %2, copying to %3)")
            .arg(from)
  ...
  if (strlen(header)) {
    ...
    while (bytes_left &&
      ...
      if (bytes_left >= bytes_written) {
        ...
        bytes_left = 0;
        errormsg =
            QString("Copy: Error reading file %1 read overrun").arg(from);
        fclose(fin);
        fclose(fout);
        QFile::remove(to);
        return false;
...
bool US_File_Util::copy(QStringList from, QString toDir, bool overwrite) {
  ...
  if (!QFileInfo(toDir).isDir()) {
    errormsg =
        QString("Copy: destination %1 is not a directory for a multi file copy")
            .arg(toDir);
    return false;
  }
...
bool US_File_Util::read(QString from, QStringList &qsl) {
  ...
  if (!f.open(QIODevice::ReadOnly | QIODevice::Text)) {
    errormsg =
        QString("US_File_Util:read() can not open %1 for reading").arg(from);
    return false;
  }
...
bool US_File_Util::writeuniq(QString &outname, QString &msg, QString basename,
...
void US_Log::log(const char *c) { log(QString(c)); }

void US_Log::flushon() { autoflush = true; }

void US_Log::flushoff() { autoflush = false; }

bool US_File_Util::getcontents(QString file, QString &contents,
                               QString &error) {
  error = "";
  contents = "";
  QFile f(file);
...
bool US_File_Util::putcontents(QString file, QString &contents,
                               QString &error) {
  error = "";
  QFile f(file);
