...
void
US_Hydrodyn_Saxs_Guinier_Frames::setupGUI ()
{
  ...
  cb_yright1->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  cb_yright2->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize));
  ...
  le_reffile->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize));
  ...
  le_xbottom_min->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize));
  ...
  le_xbottom_max->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize));
  ...
  le_yleft_min->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  le_yleft_max->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  le_yright_min->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize));
  ...
  le_yright_max->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize));
...
void
US_Hydrodyn_Saxs_Guinier_Frames::cancel ()
{
  close ();
}

void
US_Hydrodyn_Saxs_Guinier_Frames::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Saxs_Guinier_Frames::closeEvent (QCloseEvent *e)
{
...
void
US_Hydrodyn_Saxs_Guinier_Frames::update_plot ()
{
  ...
  if (plots.count ("yleft"))
    {
      ...
      for (int i = 0; i < (int)(*this_plot)[0].size (); ++i)
        {
...

          {
            ...
            plot->setCurveData (curve, &(*this_plot)[1][i],
                                &(*this_plot)[2][i], 1);
#endif
          }
      ...
      {
        QString this_name
            = QString ("%1 summary line").arg (parameters["yleft"]);
        ...
        for (int i = 0; i < (int)(*this_plot)[0].size (); ++i)
          {
            ...
            // us_qdebug( QString( "%1 rgb %2 %3 %4" ).arg( rgb ).arg( red
            // ).arg( green ).arg( blue ) );

            {
              ...
              curve->attach (plot);
#else
              long curve = plot->insertCurve (this_name, QwtPlot::xBottom,
                                              QwtPlot::yRight);
              plot->setCurveStyle (curve, QwtCurve::NoCurve);
              plot->setCurveSymbol (curve, sym);
              plot->setCurveData (curve, &(*this_plot)[1][i],
            ...
            }
            {
              ...
              curve->attach (plot);
#else
              long curve = plot->insertCurve (
                  this_name + "_errorbar", QwtPlot::xBottom, QwtPlot::yRight);
              plot->setCurveStyle (curve, QwtCurve::Lines);
              plot->setCurvePen (curve, use_pen);
              plot->setCurveData (curve, x, y, 2);
#endif
  ...
  if (ref_I.size ())
    {
      ...
      // us_qdebug( US_Vector::qs_vector3( "ref domain scaled", ref_t, ref_I,
      // y) );
    }
...
void
US_Hydrodyn_Saxs_Guinier_Frames::set_reffile ()
{
  QString tmp;
  update_reffile (tmp);
}
...
void
US_Hydrodyn_Saxs_Guinier_Frames::set_yright1 ()
{
...
void
US_Hydrodyn_Saxs_Guinier_Frames::set_yright2 ()
{
  if (cb_yright1->isChecked () && cb_yright2->isChecked ())
    {
      cb_yright1->setChecked (false);
...
void
US_Hydrodyn_Saxs_Guinier_Frames::update_xbottom_min (const QString &)
{
  update_plot ();
}

void
US_Hydrodyn_Saxs_Guinier_Frames::update_xbottom_max (const QString &)
{
  update_plot ();
}

void
US_Hydrodyn_Saxs_Guinier_Frames::update_yleft_min (const QString &)
{
  update_plot ();
}

void
US_Hydrodyn_Saxs_Guinier_Frames::update_yleft_max (const QString &)
{
  update_plot ();
}

void
US_Hydrodyn_Saxs_Guinier_Frames::update_yright_min (const QString &)
{
  update_plot ();
}

void
US_Hydrodyn_Saxs_Guinier_Frames::update_yright_max (const QString &)
{
  update_plot ();
}

void
US_Hydrodyn_Saxs_Guinier_Frames::update_reffile (const QString &)
{
  QString use_dir;
  ((US_Hydrodyn *)us_hydrodyn)->select_from_directory_history (use_dir, this);
  QString filename = QFileDialog::getOpenFileName (
      this,
      us_tr ("Select a file for Reference concentration intensity curve"),
      use_dir, QString ());
...
void
US_Hydrodyn_Saxs_Guinier_Frames::usp_config_plot (const QPoint &)
{
