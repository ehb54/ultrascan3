...
US_Hydrodyn_Dad_Fit::US_Hydrodyn_Dad_Fit (US_Hydrodyn_Dad *dad_win,
  ...
  if (dad_win->conc_files.count (dad_win->wheel_file))
    {
      ...
      if (gaussian_type != US_Hydrodyn_Dad::EMGGMG)
        ...
        {
          for (int i = gaussian_type_size; i < (int)dad_win->gaussians.size ();
               i += gaussian_type_size)
            {
              conc_ratios.push_back (dad_win->gaussians[2 + i]
                                     / dad_win->gaussians[2]);
            }
...
void
US_Hydrodyn_Dad_Fit::redo_settings ()
{
  use_errors = dad_win->cb_sd_weight->isChecked ();
  if (use_errors
      && (!dad_win->f_errors.count (dad_win->wheel_file)
          || dad_win->f_errors[dad_win->wheel_file].size ()
                 != dad_win->f_qs[dad_win->wheel_file].size ()
          || !dad_win->is_nonzero_vector (
              dad_win->f_errors[dad_win->wheel_file])))
    {
      use_errors = false;
...
void
US_Hydrodyn_Dad_Fit::cancel ()
{
  close ();
}

void
US_Hydrodyn_Dad_Fit::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Dad_Fit::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 3;
  global_Ypos -= 30;
  e->accept ();
...
void
US_Hydrodyn_Dad_Fit::update_common ()
{
  if (!setup_run ())
    {
      update_enables ();
  ...
  for (unsigned int i = 0; i < DFIT::param_fixed.size (); i++)
    {
      if (!DFIT::param_fixed[i] || DFIT::comm_backref.count (i))
        {
          dad_win->gaussians[i]
              = DFIT::init_params[DFIT::comm_backref.count (i)
                                      ? DFIT::comm_backref[i]
                                      : DFIT::param_pos[i]];
...
void
US_Hydrodyn_Dad_Fit::update_enables ()
{
...
US_Hydrodyn_Dad_Fit::setup_run ()
{
  ...
  for (unsigned int i = 0; i < (unsigned int)dad_win->gaussians.size ();
       i += gaussian_type_size)
    {
          ...
          if (DFIT::init_params.back () < min)
            {
              ...
              // ).arg( DFIT::init_params.size() ).arg(
              // DFIT::init_params.back() ) );
      ...
      )
        {
          ...
          if (i && cb_conc_test->isChecked () && !fixed_curves.count (pos + 1)
              && !fixed_curves.count (1))
            {
              ...
              DFIT::comm_backref[DFIT::param_fixed.size ()]
                  = conc_width_var_pos;
              DFIT::conc_ratios_map[DFIT::param_fixed.size ()]
                  = conc_ratios[pos - 1];
            }
          else
            {
...
bool
US_Hydrodyn_Dad_Fit::lock_zeros (vector<double> &par)
{
  ...
  for (unsigned int i = 0; i < DFIT::param_fixed.size (); i++)
    {
      if (!DFIT::param_fixed[i] || DFIT::comm_backref.count (i))
        {
          tmp_gaussians[i]
              = par[DFIT::comm_backref.count (i) ? DFIT::comm_backref[i]
                                                 : DFIT::param_pos[i]];
          if (DFIT::conc_ratios_map.count (i))
            {
          ...
          if (!cb_fix_amplitude->isChecked ())
            {
              if (tmp_gaussians[0 + i]
                  <= dad_ampl_width_min * dad_lock_min_retry_mult)
                {
...
bool
US_Hydrodyn_Dad_Fit::max_free_peak_delta (vector<double> &par)
{
...
void
US_Hydrodyn_Dad_Fit::lm (bool max_free_peak_delta_run, double prev_rmsd)
{
  update_common ();
  setup_run ();
  // puts( "lm" );
  ...
  // ); US_Vector::printvector( QString( "lm %1 I" ).arg( dad_win->wheel_file
  // ), dad_win->f_Is[ dad_win->wheel_file ] ); US_Vector::printvector(
  // QString( "lm %1 e" ).arg( dad_win->wheel_file ), dad_win->f_errors[
  // dad_win->wheel_file ] );
  ...
  if (use_errors)
    {
      ...
      for (unsigned int j = 0; j < t.size (); j++)
        {
          yp[j] = (*DFIT::compute_gaussian_f) (
              t[j], (double *)(&DFIT::base_params[0]));
  ...
  if (max_free_peak_delta_run)
          ...
          for (unsigned int i = 0; i < DFIT::param_fixed.size (); i++)
            {
              if (!DFIT::param_fixed[i] || DFIT::comm_backref.count (i))
                {
                  dad_win->gaussians[i] = par[DFIT::comm_backref.count (i)
                                                  ? DFIT::comm_backref[i]
                                                  : DFIT::param_pos[i]];
                  if (DFIT::conc_ratios_map.count (i))
                    {
  ...
  else
    {
      ...
      if (dad_maxfpk_restart && max_free_peak_delta (par))
        {
...
US_Hydrodyn_Dad_Fit::gsm_sd ()
{
  ...
  for (unsigned int j = 0; j < gsm_t.size (); j++)
    {
      gsm_yp[j] = (*DFIT::compute_gaussian_f) (
          gsm_t[j], (double *)(&DFIT::base_params[0]));
      ...
      for (unsigned int i = 0; i < DFIT::param_fixed.size (); i++)
        {
          if (!DFIT::param_fixed[i] || DFIT::comm_backref.count (i))
            {
              dad_win->gaussians[i]
                  = v->d[DFIT::comm_backref.count (i) ? DFIT::comm_backref[i]
                                                      : DFIT::param_pos[i]];
              if (DFIT::conc_ratios_map.count (i))
                {
                  dad_win->gaussians[i] *= DFIT::conc_ratios_map[i];
...
void
US_Hydrodyn_Dad_Fit::stop ()
{
  running = false;
}

void
US_Hydrodyn_Dad_Fit::grid ()
{
  setup_run ();
  update_common ();
  puts ("grid");
  ...
  unsigned int total_count
      = (unsigned int)pow ((double)pop_size, (int)DFIT::init_params.size ());
  ...
  for (unsigned int j = 0; j < t.size (); j++)
    {
      yp[j] = (*DFIT::compute_gaussian_f) (t[j],
                                           (double *)(&DFIT::base_params[0]));
      org_rmsd += (y[j] - yp[j]) * (y[j] - yp[j]);
    }
  ...
  for (unsigned int i = 0; i < total_count; i++)
    {
      ...
      for (unsigned int j = 0; j < (unsigned int)DFIT::init_params.size ();
           j++)
        {
