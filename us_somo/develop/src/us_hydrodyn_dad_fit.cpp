...
void US_Hydrodyn_Dad_Fit::cancel() { close(); }
...
void US_Hydrodyn_Dad_Fit::update_common() {
  ...
  for (unsigned int i = 0; i < DFIT::param_fixed.size(); i++) {
    if (!DFIT::param_fixed[i] || DFIT::comm_backref.count(i)) {
      dad_win->gaussians[i] =
          DFIT::init_params[DFIT::comm_backref.count(i) ? DFIT::comm_backref[i]
                                                        : DFIT::param_pos[i]];
  ...
  for (unsigned int i = 0; i < (unsigned int)dad_win->gaussians.size();
    ...
    if (cb_fix_amplitude->isChecked() || fixed_curves.count(pos + 1)) {
      ...
      if (DFIT::init_params.back() < min) {
        ...
        // DFIT::init_params.size() ).arg( DFIT::init_params.back() ) );
...
bool US_Hydrodyn_Dad_Fit::lock_zeros(vector<double> &par) {
  ...
  for (unsigned int i = 0; i < DFIT::param_fixed.size(); i++) {
    if (!DFIT::param_fixed[i] || DFIT::comm_backref.count(i)) {
      tmp_gaussians[i] = par[DFIT::comm_backref.count(i) ? DFIT::comm_backref[i]
                                                         : DFIT::param_pos[i]];
      if (DFIT::conc_ratios_map.count(i)) {
...
void US_Hydrodyn_Dad_Fit::lm(bool max_free_peak_delta_run, double prev_rmsd) {
  ...
  // dad_win->f_Is[ dad_win->wheel_file ] ); US_Vector::printvector( QString(
  // "lm %1 e" ).arg( dad_win->wheel_file ), dad_win->f_errors[
  // dad_win->wheel_file ] );
  ...
  if (use_errors) {
    ...
    for (unsigned int j = 0; j < t.size(); j++) {
      yp[j] =
          (*DFIT::compute_gaussian_f)(t[j], (double *)(&DFIT::base_params[0]));
...
void US_Hydrodyn_Dad_Fit::gsm_sd() {
  ...
  for (unsigned int j = 0; j < gsm_t.size(); j++) {
    gsm_yp[j] = (*DFIT::compute_gaussian_f)(gsm_t[j],
...
void US_Hydrodyn_Dad_Fit::stop() { running = false; }
