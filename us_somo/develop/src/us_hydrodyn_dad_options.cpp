...
void US_Hydrodyn_Dad_Options::setupGUI() {
  int minHeight1 = 28;
  ...
  rb_gmg =
      new QRadioButton(us_tr("GMG (Half-Gaussian modified Gaussian)"), this);
  ...
  rb_emg =
      new QRadioButton(us_tr("EMG (Exponentially modified Gaussian)"), this);
  ...
  connect(pb_dad_parameters, SIGNAL(clicked()), SLOT(set_dad_parameters()));
  ...
  cb_csv_transposed->setChecked((*parameters)["dad_csv_transposed"] == "true");
  cb_csv_transposed->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_discard_it_sd_mult->setChecked(
      (*parameters)["dad_cb_discard_it_sd_mult"] == "true");
  cb_discard_it_sd_mult->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_gg_smooth->setText(
      us_tr("Experimental: Global Gaussian initialization smoothing. Maximum "
            "smoothing points: "));
  cb_gg_smooth->setEnabled(true);
  cb_gg_smooth->setChecked((*parameters)["dad_cb_gg_smooth"] == "true");
  cb_gg_smooth->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  lbl_ampl_width_min =
      new QLabel(us_tr("Global minimum value for amplitude and width:"), this);
  ...
  cb_lock_min_retry->setChecked((*parameters)["dad_lock_min_retry"] == "true");
  cb_lock_min_retry->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  le_lock_min_retry_mult->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  {
    QDoubleValidator *qdv =
  ...
  cb_maxfpk_restart->setChecked((*parameters)["dad_maxfpk_restart"] == "true");
  cb_maxfpk_restart->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  cb_maxfpk_restart->setPalette(PALET_NORMAL);
  AUTFBACK(cb_maxfpk_restart);

  // le_maxfpk_restart_tries = new QLineEdit( this );
  // le_maxfpk_restart_tries->setObjectName( "le_maxfpk_restart_tries Line Edit"
  // ); le_maxfpk_restart_tries->setText( (*parameters)[
  ...
  // SLOT( update_enables() ) ); le_maxfpk_restart_tries->setMinimumWidth( 60 );
...
void US_Hydrodyn_Dad_Options::quit() { close(); }

bool US_Hydrodyn_Dad_Options::any_changes() {
...
void US_Hydrodyn_Dad_Options::ok() {
  (*parameters)["ok"] = "true";
  ...
  if (rb_gauss->isChecked()) {
    (*parameters)["gaussian_type"] = QString("%1").arg(US_Hydrodyn_Dad::GAUSS);
  }
  if (rb_gmg->isChecked()) {
    (*parameters)["gaussian_type"] = QString("%1").arg(US_Hydrodyn_Dad::GMG);
  }
  if (rb_emg->isChecked()) {
    (*parameters)["gaussian_type"] = QString("%1").arg(US_Hydrodyn_Dad::EMG);
  }
  if (rb_emggmg->isChecked()) {
    (*parameters)["gaussian_type"] = QString("%1").arg(US_Hydrodyn_Dad::EMGGMG);
  }
...
void US_Hydrodyn_Dad_Options::help() {
  US_Help *online_help;
...
void US_Hydrodyn_Dad_Options::update_enables() {
  ...
  if (cb_makeiq_cutmax_pct->isEnabled() && cb_makeiq_cutmax_pct->isChecked() &&
    ...
    le_makeiq_avg_peaks->setText(
        QString("%1").arg(100.0 - le_makeiq_cutmax_pct->text().toDouble()));
  }
}

void US_Hydrodyn_Dad_Options::clear_gauss() {
  if (QMessageBox::Yes ==
...
void US_Hydrodyn_Dad_Options::set_detector() {
  {
...
void US_Hydrodyn_Dad_Options::set_dad_parameters() {
  {
    map<QString, QString> parameters;
    ...
    parameters["dad_param_n"] =
        QString("%1").arg(((US_Hydrodyn_Dad *)dad_win)->dad_param_n, 0, 'g', 8);
    ...
    parameters["dad_param_DLS_detector"] =
        QString("%1").arg(((US_Hydrodyn_Dad *)dad_win)->dad_param_DLS_detector);
...
void US_Hydrodyn_Dad_Options::fasta_file() {
  // load file
...
void US_Hydrodyn_Dad_Options::update_fasta_pH(const QString &str) {
