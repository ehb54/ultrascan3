...
void US_Hydrodyn_Mals_Options::setupGUI() {
  int minHeight1 = 28;
  ...
  le_reps->setText(
      parameters->count("mals_bl_reps") ? (*parameters)["mals_bl_reps"] : "1");
  ...
  cb_csv_transposed->setChecked((*parameters)["mals_csv_transposed"] == "true");
  ...
  cb_lock_min_retry->setChecked((*parameters)["mals_lock_min_retry"] == "true");
  ...
  cb_maxfpk_restart->setChecked((*parameters)["mals_maxfpk_restart"] == "true");
...
void US_Hydrodyn_Mals_Options::quit() { close(); }

bool US_Hydrodyn_Mals_Options::any_changes() {
...
void US_Hydrodyn_Mals_Options::ok() {
  (*parameters)["ok"] = "true";
  ...
  if (rb_gauss->isChecked()) {
    (*parameters)["gaussian_type"] = QString("%1").arg(US_Hydrodyn_Mals::GAUSS);
  }
  if (rb_gmg->isChecked()) {
    (*parameters)["gaussian_type"] = QString("%1").arg(US_Hydrodyn_Mals::GMG);
  }
  if (rb_emg->isChecked()) {
    (*parameters)["gaussian_type"] = QString("%1").arg(US_Hydrodyn_Mals::EMG);
  }
...
void US_Hydrodyn_Mals_Options::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/mals_options.html");
}

void US_Hydrodyn_Mals_Options::closeEvent(QCloseEvent *e) {
  if (any_changes()) {
    QMessageBox mb(
        this->windowTitle(),
...
void US_Hydrodyn_Mals_Options::update_enables() {
  ...
  pb_clear_gauss->setEnabled(((US_Hydrodyn_Mals *)mals_win)->any_gaussians());
  le_epsilon->setEnabled(rb_integral->isChecked());
...
void US_Hydrodyn_Mals_Options::clear_gauss() {
  if (QMessageBox::Yes ==
...
void US_Hydrodyn_Mals_Options::set_detector() {
  {
...
void US_Hydrodyn_Mals_Options::fasta_file() {
  // load file
...
void US_Hydrodyn_Mals_Options::update_fasta_pH(const QString &str) {
