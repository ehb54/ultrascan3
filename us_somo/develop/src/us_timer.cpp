...
void
US_Timer::clear_timers ()
{
}

void
US_Timer::clear_timer (QString qs)
{
  timers.erase (qs);
  counts.erase (qs);
  times.erase (qs);
...
void
US_Timer::init_timer (QString qs)
{
...
bool
US_Timer::start_timer (QString qs)
{
  if (!timers.count (qs))
    {
      return false;
...
bool
US_Timer::stop_timer (QString qs)
{
  if (!timers.count (qs))
    {
      return false;
...
QString
US_Timer::time_min_sec (QString qs)
{
  if (!timers.count (qs))
    {
...
US_Timer::list_time (QString qs)
{
  ...
  return counts[qs] == 1
             ? QString ("%1 %2 calls total time %3 ms\n")
                   ...
                   .arg (times[qs])
             : (counts[qs] == 2
                    ? QString ("%1 %2 calls total time %3 ms avg time %4 ms "
                               "sd %5 ms\n")
                          .arg (qs)
                          .arg (counts[qs])
                          .arg (times[qs])
                    ...
                    : QString ("%1 %2 calls total time %3 ms avg time %4 ms "
                          ...
                          .arg (avg)
                          .arg (sqrt (
                              (double)(1e0 * times2[qs] / (1e0 * counts[qs]))
                              - avg * avg)));
}

QString
US_Timer::list_times (QString prepend)
{
