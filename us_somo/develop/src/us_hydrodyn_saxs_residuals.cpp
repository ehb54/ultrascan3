...
US_Hydrodyn_Saxs_Residuals::US_Hydrodyn_Saxs_Residuals(
  ...
  use_errors =
      error.size() == target.size() && US_Saxs_Util::is_nonzero_vector(error);
...
void US_Hydrodyn_Saxs_Residuals::cancel() { close(); }
...
void US_Hydrodyn_Saxs_Residuals::update_plot() {
  ...
  if (plot_residuals) {
    ...
    curve->attach(plot);
    double this_miny = plot_as_percent ? residuals_pct[0] : residuals_div_sd[0];
