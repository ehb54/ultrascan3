...
US_Hydrodyn_Saxs_Residuals::US_Hydrodyn_Saxs_Residuals (
    bool *saxs_residuals_widget, unsigned int width, QString title,
    vector<double> r,
    // vector < double > difference, // no longer using difference
    vector<double> residuals, vector<double> target, vector<double> error,
    // bool plot_residuals,
    // bool plot_difference,
    // bool plot_as_percent,
    // bool use_errors,
    unsigned pen_width, QWidget *p, const char *)
    : QFrame (p)
{
  // update 2023-03
  ...
  use_errors = error.size () == target.size ()
  ...
  *saxs_residuals_widget = true;
  ...
  // US_Vector::printvector5( "pr residuals r, resid, target, error,
...
void
US_Hydrodyn_Saxs_Residuals::setupGUI ()
{
  ...
  cb_plot_residuals->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  connect (cb_plot_residuals, SIGNAL (clicked ()),
           SLOT (set_plot_residuals ()));
...
void
US_Hydrodyn_Saxs_Residuals::cancel ()
{
  close ();
}

void
US_Hydrodyn_Saxs_Residuals::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Saxs_Residuals::closeEvent (QCloseEvent *e)
{
  *saxs_residuals_widget = false;
  global_Xpos -= 30;
  global_Ypos -= 30;
...
void
US_Hydrodyn_Saxs_Residuals::set_plot_residuals ()
{
...
void
US_Hydrodyn_Saxs_Residuals::set_plot_difference ()
{
...
void
US_Hydrodyn_Saxs_Residuals::set_plot_as_percent ()
{
...
void
US_Hydrodyn_Saxs_Residuals::update_plot ()
{
  ...
  QString left_axis_title
      = plot_as_percent ? "% difference"
                        : (plot_residuals ? "Residuals/SD" : "Residuals");
  ...
  if (plot_residuals)
    {
      ...
      curve->setSamples ((double *)&(r[0]), (double *)&(residuals_div_sd[0]),
                         (int)r.size ());
      curve->setPen (QPen (Qt::yellow, pen_width, Qt::SolidLine));
      curve->attach (plot);
      double this_miny
          = plot_as_percent ? residuals_pct[0] : residuals_div_sd[0];
  ...
  if (plot_difference)
    {
      QwtPlotCurve *curve = new QwtPlotCurve ("P(r) vs r");
      curve->setStyle (QwtPlotCurve::Lines);
      curve->setSamples ((double *)&(r[0]), (double *)&(residuals[0]),
                         (int)r.size ());
      curve->setPen (QPen (Qt::yellow, pen_width, Qt::SolidLine));
      curve->attach (plot);
  ...
  if (plot_as_percent)
    {
      QwtPlotCurve *curve = new QwtPlotCurve ("P(r) vs r");
      curve->setStyle (QwtPlotCurve::Lines);
      curve->setSamples ((double *)&(r[0]), (double *)&(residuals_pct[0]),
                         (int)r.size ());
      curve->setPen (QPen (Qt::yellow, pen_width, Qt::SolidLine));
      curve->attach (plot);
...
void
US_Hydrodyn_Saxs_Residuals::usp_config_plot (const QPoint &)
{
