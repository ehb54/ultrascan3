...
void US_Hydrodyn_Mals_Saxs::scale_pair(bool no_store_original) {
  ...
  // somewhat duplicated code in ::common_time() && ::join_by_time
  {
    ...
    if (qgrid_to_names.size() != 2) {
      ...
      QMessageBox::critical(this, windowTitle() + us_tr(": Scale"),
                            QString(us_tr("The %1 curves selected do not have "
                                          "exactly two unique q-grids\n\n"
                                          "%2 q-grid(s) found\n"))
                                    .arg(names.size())
  ...
  scale_pair_use_errors = true;
  {
    ...
    if (!scale_pair_use_errors) {
      QMessageBox::warning(
          ...
          QString(
              us_tr(
                  "File(s) without SDs defined or having zero value SDs found\n"
                  "First found in:\n%1\n"
  ...
  scale_pair_add_marker(plot_dist, le_scale_pair_q1_start->text().toDouble(),
                        scale_pair_color_q1, us_tr("Start"),
                        Qt::AlignRight | Qt::AlignBottom);
  scale_pair_add_marker(plot_dist, le_scale_pair_q1_end->text().toDouble(),
                        scale_pair_color_q1, us_tr("End"),
                        Qt::AlignLeft | Qt::AlignBottom);
  scale_pair_add_marker(plot_dist, le_scale_pair_q2_start->text().toDouble(),
                        scale_pair_color_q2, us_tr("Start"),
                        Qt::AlignRight | Qt::AlignBottom);
...
void US_Hydrodyn_Mals_Saxs::scale_pair_add_marker(QwtPlot *plot, double pos,
...
void US_Hydrodyn_Mals_Saxs::scale_pair_q1_start_text(const QString &text) {
  ...
  disconnect(le_scale_pair_q1_start, SIGNAL(textChanged(const QString &)), 0,
             0);
...
void US_Hydrodyn_Mals_Saxs::scale_pair_q1_end_text(const QString &text) {
  ...
  if (scale_pair_markers.size() != 4) {
    editor_msg(
        "red",
        QString("internal error: scale_pair_q1_end_text markers issue size %1")
            .arg(scale_pair_markers.size()));
    return;
  }
...
void US_Hydrodyn_Mals_Saxs::scale_pair_q2_start_text(const QString &text) {
  ...
  disconnect(le_scale_pair_q2_start, SIGNAL(textChanged(const QString &)), 0,
             0);
...
void US_Hydrodyn_Mals_Saxs::scale_pair_q2_end_text(const QString &text) {
  ...
  if (scale_pair_markers.size() != 4) {
    editor_msg(
        "red",
        QString("internal error: scale_pair_q2_end_text markers issue size %1")
            .arg(scale_pair_markers.size()));
    return;
  }
  ...
  if (scale_pair_use_errors) {
    ...
    for (auto const &scaled_time_to_name : scale_pair_created_time_to_name) {
      ...
      for (int i = 0; i < qgrid_size; ++i) {
        ...
        f_errors[scaled_name][i] =
            f_errors[source_name][i] * scale_pair_scale * scale_pair_sd_scale;
      }
    }
  } else {
    for (auto const &scaled_time_to_name : scale_pair_created_time_to_name) {
...
#warning qwtw_wheel probably should be in ::disable_all() but for some reason we commented it out there, probably should check all wheel usage and set as appropriate
...

  {
    ...
    switch (
        (scale_pair_fit_curves)cb_scale_pair_fit_curve->currentData().toInt()) {
      ...
      case SCALE_PAIR_FIT_CURVE_P8:
        ...
        break;
      default:
        QMessageBox::critical(this, windowTitle() + us_tr(": Scale Fit"),
      ...
      QMessageBox::critical(
          ...
          QString(
              us_tr(
                  ...
                  "The current fitting method requires a minimum of %2 "
                  "points."))
              .arg(qs.size())
              .arg(minimum_pts_req));
      return scale_pair_enables();
    ...
    if (q1.size() + q2.size() != qs.size()) {
      ...
      QMessageBox::warning(this, windowTitle() + us_tr(": Scale Fit"),
  ...
  double global_chi2 = 0;
  {
    ...
    for (auto const &t : scale_pair_times) {
      if (!scale_pair_time_to_names.count(t)) {
        QMessageBox::critical(
            this, windowTitle() + us_tr(": Scale Fit"),
            QString(us_tr("Internal error : files for time %1 not found"))
                .arg(t));
        return scale_pair_enables();
      }
  ...
  switch ((US_Eigen::weight_methods)cb_scale_pair_fit_alg_weight->currentData()
              .toInt()) {
...
bool US_Hydrodyn_Mals_Saxs::scale_pair_fit_at_time(double time, int degree,
                                                   const set<double> &q1,
                                                   const set<double> &q2,
                                                   double &chi2) {
  if (!minimize_running) {
...
void US_Hydrodyn_Mals_Saxs::scale_pair_minimize() {
...

  {
    ...
    do {
      ...
      vector<QString> labels = {us_tr("Global scale start :"),
  ...
  for (double s = scale_start; s <= scale_end; s += scale_step) {
    ...
    if (best_gchi2 > global_chi2) {
      ...
      qDebug() << QString("scale %1 gChi2 %2 new best\n")
                      .arg(s)
                      .arg(global_chi2 / (qs.size() * scale_pair_times.size()));
    } else {
...
#warning save headers
  ...
  if (scale_pair_scale != last_minimize_scale) {
    ...
    switch (QMessageBox::question(
        ...
        us_tr("The scale has changed since the last 'minimize' or you did not "
              "'minimize' at all.\n"
  ...
  if (!create_saxs || scale_pair_sd_scale != 1) {
    ...
    if (scale_pair_use_errors) {
      ...
      for (auto const &scaled_time_to_name : scale_pair_time_to_names) {
        ...
        scaled_name +=
            QString(suffix).arg(use_scale_pair_scale).arg(scale_pair_sd_scale);
        ...
        add_plot(scaled_name, scale_pair_qgrids[scale_pair_mals_set], I, e,
                 false, false);
        ...
        if (!last_minimize_method.isEmpty()) {
          f_fit_method[last_created_file] = QString("%1:%2")
      ...
      for (auto const &scaled_time_to_name : scale_pair_time_to_names) {
        ...
        add_plot(scaled_name, scale_pair_qgrids[scale_pair_mals_set], I, false,
                 false);
        scale_pair_save_names.insert(last_created_file);
        scale_pair_org_selected.erase(source_name);
        scale_pair_org_selected.insert(last_created_file);
  ...
  if (create_saxs) {
    ...
    if (scale_pair_use_errors) {
      for (auto const &scaled_time_to_name : scale_pair_time_to_names) {
        ...
        add_plot(scaled_name, scale_pair_qgrids[scale_pair_saxs_set], I, e,
                 false, false);
        scale_pair_save_names.insert(last_created_file);
        scale_pair_org_selected.erase(source_name);
        scale_pair_org_selected.insert(last_created_file);
      ...
      for (auto const &scaled_time_to_name : scale_pair_time_to_names) {
        ...
        add_plot(scaled_name, scale_pair_qgrids[scale_pair_saxs_set], I, false,
                 false);
        scale_pair_save_names.insert(last_created_file);
        scale_pair_org_selected.erase(source_name);
        scale_pair_org_selected.insert(last_created_file);
