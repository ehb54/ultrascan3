...
void
US_Hydrodyn_Mals_Saxs::scale_pair (bool no_store_original)
{
  TSO << "scale_pairs()\n";
  ...
  if (!no_store_original)
    {
      ...
      scale_pair_original_sd_scale
          = le_scale_pair_sd_scale->text ().toDouble ();
    }
  ...
  // somewhat duplicated code in ::common_time() && ::join_by_time
  {
    ...
    if (qgrid_to_names.size () != 2)
      {
        ...
        QMessageBox::critical (
            this, windowTitle () + us_tr (": Scale"),
            QString (us_tr ("The %1 curves selected do not have exactly two "
                            "unique q-grids\n\n"
                            "%2 q-grid(s) found\n"))
                    .arg (names.size ())
    ...
    scale_pair_names = { qgrid_to_names.begin ()->second,
  ...
  scale_pair_mals_set = f_qs[scale_pair_names[0][0]].front ()
                                < f_qs[scale_pair_names[1][0]].front ()
                            ? 0
                            : 1;
  scale_pair_saxs_set = 1 - scale_pair_mals_set;
  variable_set = cb_scale_pair_scale_saxs->isChecked () ? scale_pair_saxs_set
                                                        : scale_pair_mals_set;
  fixed_set = cb_scale_pair_scale_saxs->isChecked () ? scale_pair_mals_set
                                                     : scale_pair_saxs_set;

  for (int i = 0; i < (int)scale_pair_times.size (); ++i)
    {
      scale_pair_time_to_names[scale_pair_times[i]].push_back (
          scale_pair_names[scale_pair_mals_set][i]);
      scale_pair_time_to_names[scale_pair_times[i]].push_back (
          scale_pair_names[scale_pair_saxs_set][i]);
    }
  ...
  scale_pair_use_errors = true;
  {
    ...
    for (auto const &name : scale_pair_names[scale_pair_mals_set])
      {
        if (!f_errors.count (name)
            || f_errors[name].size ()
                   != scale_pair_qgrids[scale_pair_mals_set].size ()
        ...
        for (auto const &name : scale_pair_names[scale_pair_saxs_set])
          {
            if (!f_errors.count (name)
                || f_errors[name].size ()
                       != scale_pair_qgrids[scale_pair_saxs_set].size ()
                || is_zero_vector (f_errors[name]))
              {
                scale_pair_use_errors = false;
                missing_errors_name = name;
    ...
    if (!scale_pair_use_errors)
      {
        QMessageBox::warning (
            this, windowTitle () + us_tr (": Scale Fit"),
            QString (us_tr ("File(s) without SDs defined or having zero value "
                            "SDs found\n"
                            "First found in:\n%1\n"
  ...
  lbl_scale_pair_q1_range->setText (cb_scale_pair_scale_saxs->isChecked ()
                                        ? us_tr (" Fit range 1 (fixed): ")
                                        : us_tr (" Fit range 1 (scales): "));
  lbl_scale_pair_q2_range->setText (cb_scale_pair_scale_saxs->isChecked ()
                                        ? us_tr (" Fit range 2 (scales): ")
                                        : us_tr (" Fit range 2 (fixed): "));

  lbl_scale_pair_sd_scale->setText (cb_scale_pair_scale_saxs->isChecked ()
                                        ? us_tr (" SAXS SD Mult.: ")
                                        : us_tr (" MALS SD Mult.: "));
  ...
  scale_pair_add_marker (
      plot_dist, le_scale_pair_q1_start->text ().toDouble (),
      scale_pair_color_q1, us_tr ("Start"), Qt::AlignRight | Qt::AlignBottom);
  scale_pair_add_marker (plot_dist, le_scale_pair_q1_end->text ().toDouble (),
                         scale_pair_color_q1, us_tr ("End"),
                         Qt::AlignLeft | Qt::AlignBottom);
  scale_pair_add_marker (
      plot_dist, le_scale_pair_q2_start->text ().toDouble (),
      scale_pair_color_q2, us_tr ("Start"), Qt::AlignRight | Qt::AlignBottom);
  scale_pair_add_marker (plot_dist, le_scale_pair_q2_end->text ().toDouble (),
                         scale_pair_color_q2, us_tr ("End"),
                         Qt::AlignLeft | Qt::AlignBottom);
  ...
  // disconnect( le_scale_pair_q1_start, SIGNAL( textChanged( const QString & )
  // ), 0, 0 ); disconnect( le_scale_pair_q1_end, SIGNAL( textChanged( const
  // QString & ) ), 0, 0 );
  if (le_scale_pair_q1_start->text ().isEmpty ()
  ...
  if (le_scale_pair_q1_end->text ().isEmpty ()
      || le_scale_pair_q1_end->text ().toDouble () > scale_pair_q1_max
      || le_scale_pair_q1_end->text ().toDouble () < scale_pair_q1_min
      || le_scale_pair_q1_end->text ().toDouble ()
             == scale_pair_q1_min // caused by the slot
  )
    {
  ...
  // SLOT( scale_pair_q1_start_text( const QString & ) ) ); connect(
  // le_scale_pair_q1_end, SIGNAL( textChanged( const QString & ) ), SLOT(
  // scale_pair_q1_end_text( const QString & ) ) );

  // disconnect( le_scale_pair_q2_start, SIGNAL( textChanged( const QString & )
  // ), 0, 0 ); disconnect( le_scale_pair_q2_end, SIGNAL( textChanged( const
  // QString & ) ), 0, 0 );
  if (le_scale_pair_q2_start->text ().isEmpty ()
  ...
  if (le_scale_pair_q2_end->text ().isEmpty ()
      || le_scale_pair_q2_end->text ().toDouble () > scale_pair_q2_max
      || le_scale_pair_q2_end->text ().toDouble () < scale_pair_q2_min
      || le_scale_pair_q2_end->text ().toDouble ()
             == scale_pair_q2_min // caused by the slot
  )
    {
  ...
  // SLOT( scale_pair_q2_start_text( const QString & ) ) ); connect(
  // le_scale_pair_q2_end, SIGNAL( textChanged( const QString & ) ), SLOT(
  // scale_pair_q2_end_text( const QString & ) ) );
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_enables ()
{
  // qDebug() << "scale_pair_enables() last_minimize_scale " <<
  // last_minimize_scale << " scale_pair_scale: " <<
  // le_scale_pair_scale->text().toDouble(); mostly disable except cancel &
  // wheel
  if (minimize_running)
    {
  ...
  pb_wheel_save->setEnabled (le_scale_pair_scale->text ().toDouble ()
                                 != scale_pair_original_scale
                             || le_scale_pair_sd_scale->text ().toDouble ()
                                    != scale_pair_original_sd_scale);
  ...
  pb_scale_pair_create_scaled_curves->setEnabled (
      (le_scale_pair_scale->text ().toDouble () != 1
       || (scale_pair_use_errors
           && le_scale_pair_sd_scale->text ().toDouble () != 1))
      && le_scale_pair_scale->text ().toDouble () > 0
      && le_scale_pair_sd_scale->text ().toDouble () > 0);

  pb_scale_pair_reset->setEnabled (
      le_scale_pair_scale->text ().toDouble () != 1
      || le_scale_pair_sd_scale->text ().toDouble () != 1);
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_scroll_highlight (int pos)
{
  // show the plot
  if (!minimize_running)
    {
  ...
  if (pos >= (int)scale_pair_times.size ())
    {
      qDebug () << "error: scale_pair_scroll_highlight( " << pos
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_add_marker (QwtPlot *plot, double pos,
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_delete_markers ()
{
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_q1_start_text (const QString &text)
{
  ...
  if (scale_pair_markers.size () != 4)
    {
      editor_msg (
          "red",
          QString (
              "internal error: scale_pair_q1_start_text markers issue size %1")
              .arg (scale_pair_markers.size ()));
      return;
    }
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_q1_end_text (const QString &text)
{
  ...
  if (scale_pair_markers.size () != 4)
    {
      editor_msg (
          "red",
          QString (
              "internal error: scale_pair_q1_end_text markers issue size %1")
              .arg (scale_pair_markers.size ()));
      return;
    }
  ...
  disconnect (le_scale_pair_q1_end, SIGNAL (textChanged (const QString &)), 0,
              0);
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_q1_start_focus (bool hasFocus)
{
  ...
  if (hasFocus)
    {
      ...
      qwtw_wheel->setSingleStep ((scale_pair_q1_max - scale_pair_q1_min)
                                 / UHMS_SCALE_WHEEL_RES);
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_q1_end_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_q2_start_text (const QString &text)
{
  ...
  if (scale_pair_markers.size () != 4)
    {
      editor_msg (
          "red",
          QString (
              "internal error: scale_pair_q2_start_text markers issue size %1")
              .arg (scale_pair_markers.size ()));
      return;
    }
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_q2_end_text (const QString &text)
{
  ...
  if (scale_pair_markers.size () != 4)
    {
      editor_msg (
          "red",
          QString (
              "internal error: scale_pair_q2_end_text markers issue size %1")
              .arg (scale_pair_markers.size ()));
      return;
    }
  ...
  disconnect (le_scale_pair_q2_end, SIGNAL (textChanged (const QString &)), 0,
              0);
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_q2_start_focus (bool hasFocus)
{
  ...
  if (hasFocus)
    {
      ...
      qwtw_wheel->setSingleStep ((scale_pair_q2_max - scale_pair_q2_min)
                                 / UHMS_SCALE_WHEEL_RES);
      // qwtw_wheel->setSingleStep( UHMS_SCALE_WHEEL_RES );
      connect (qwtw_wheel, SIGNAL (valueChanged (double)),
               SLOT (adjust_wheel (double)));
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_q2_end_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_time_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_scale (const QString &text)
{
  qDebug () << "scale_pair_scale( " << text << " )";
  // qDebug() << "scale_pair_scale() last_minimize_scale " <<
  // last_minimize_scale << " scale_pair_scale: " <<
  // le_scale_pair_scale->text().toDouble();
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_scale_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_sd_scale (const QString &text)
{
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_created_init ()
{
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_created_update ()
{
          ...
          for (int i = 0; i < qgrid_size; ++i)
            {
              ...
              f_errors[scaled_name][i] = f_errors[source_name][i]
                                         * scale_pair_scale
                                         * scale_pair_sd_scale;
            }
        }
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_created_remove ()
{
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_reset ()
{
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_fit ()
{
...
#warning qwtw_wheel probably should be in ::disable_all() but for some reason we commented it out there, probably should check all wheel usage and set as appropriate
  ...
  if (!qs.size ())
    {
      QMessageBox::critical (
          this, windowTitle () + us_tr (": Scale Fit"),
          us_tr ("There are no non-excluded points selected for fitting!"));
...

  {
    ...
    switch ((scale_pair_fit_curves)cb_scale_pair_fit_curve->currentData ()
                .toInt ())
      {
      ...
      default:
        QMessageBox::critical (
            this, windowTitle () + us_tr (": Scale Fit"),
            us_tr ("Internal error : unexpected fit curve"));
        return scale_pair_enables ();
        break;
      }
    ...
    if ((int)qs.size () < minimum_pts_req)
      {
        QMessageBox::critical (
            this, windowTitle () + us_tr (": Scale Fit"),
            QString (us_tr ("There are insufficient (%1) non-excluded points "
                            "selected.\n"
                            "The current fitting method requires a minimum of "
                            "%2 points."))
                .arg (qs.size ())
                .arg (minimum_pts_req));
        return scale_pair_enables ();
        ...
        if (!scale_pair_time_to_names.count (t))
          {
            QMessageBox::critical (
                this, windowTitle () + us_tr (": Scale Fit"),
                QString (
                    us_tr ("Internal error : files for time %1 not found"))
                    .arg (t));
            return scale_pair_enables ();
          }
  ...
  switch (
      (US_Eigen::weight_methods)cb_scale_pair_fit_alg_weight->currentData ()
          .toInt ())
    {
  ...
  if (scale_pair_use_errors && use_errors)
    {
      double scale_pair_sd_scale = le_scale_pair_sd_scale->text ().toDouble ();
      for (int i = 0; i < (int)scale_pair_qgrids[scale_pair_mals_set].size ();
           ++i)
        {
          if (q1.count (scale_pair_qgrids[scale_pair_mals_set][i]))
            {
              ...
              e.push_back (f_errors[name1][i] * scale_pair_scale
                           * scale_pair_sd_scale);
            }
        }

      for (int i = 0; i < (int)scale_pair_qgrids[scale_pair_saxs_set].size ();
           ++i)
        {
      ...
      eigen.polyfit (
          q, I, e, degree, coeff, chi2,
          (US_Eigen::fit_methods)cb_scale_pair_fit_alg->currentData ()
              .toInt (),
          (US_Eigen::weight_methods)cb_scale_pair_fit_alg_weight
              ->currentData ()
              .toInt ());
  ...
  lbl_scale_pair_msg->setText (
      QString ("nChi^2 %1 average %2 (%3 curves)")
          .arg (chi2 / q.size ())
...

  {
    ...
    scale_pair_fit_curve->setSamples ((double *)&(x[0]), (double *)&(y[0]),
                                      x.size ());
    scale_pair_fit_curve->setPen (
        QPen (QColor (Qt::red), use_line_width, Qt::SolidLine));
...
bool
US_Hydrodyn_Mals_Saxs::scale_pair_fit_at_time (double time, int degree,
                                               const set<double> &q1,
                                               const set<double> &q2,
                                               double &chi2)
{
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_minimize ()
{
    ...
    do
      {
        ...
        form.addRow (
            new QLabel (us_tr ("Minimize parameters\n"
        ...
        vector<QString> labels
            = { us_tr ("Global scale start :"), us_tr ("Global scale end :"),
                us_tr ("Step size:") };

        vector<double> defaults = { 0.5, 1.5, 0.01 };
        ...
        if (dialog.exec () == QDialog::Accepted)
          {
            ...
            if (scale_step <= 0 || scale_end <= scale_start)
              {
                try_again = true;
              }
          }
  ...
  last_minimize_method = us_tr (US_Eigen::qs_fit_method (
      (US_Eigen::fit_methods)cb_scale_pair_fit_alg->currentData ().toInt ()));
  last_minimize_weight = us_tr (US_Eigen::qs_weight_method (
      (US_Eigen::weight_methods)cb_scale_pair_fit_alg_weight->currentData ()
          .toInt ()));
  last_minimize_q_ranges = QString ("[%1-%2],[%3,%4]")
                               .arg (q1_min)
                               .arg (q1_max)
                               .arg (q2_min)
                               .arg (q2_max);
...
// qt doc says int argument for the signal, but actually QString, just dropping
// it

void
US_Hydrodyn_Mals_Saxs::scale_pair_fit_alg_index ()
{
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_fit_alg_weight_index ()
{
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_fit_curve_index ()
{
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_scale_saxs ()
{
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_create_scaled_curves ()
{
#warning save headers

  qDebug () << "scale_pair_create_scaled_curves()";
  // qDebug() << "scale_pair_create_scaled_curves() last_minimize_scale " <<
  // last_minimize_scale << " scale_pair_scale: " <<
  // le_scale_pair_scale->text().toDouble();
  ...
  // remove rounding check since we now set last_minimize scale from
  // lb_scale_pair_scale->text().toDouble() instead of the loop value if (
  // (int)(200*scale_pair_scale) != (int)(200*last_minimize_scale) ) {
  if (scale_pair_scale != last_minimize_scale)
    {
      scale_pair_minimize_clear ();
      switch (QMessageBox::question (
          this, windowTitle () + us_tr (" : Scale Make Scaled"),
          us_tr (
              "The scale has changed since the last 'minimize' or you did not "
              "'minimize' at all.\n"
              "You must 'minimize' again to keep fitting header information"),
          us_tr ("&Continue anyway"), us_tr ("&Quit")))
        {
        case 0:
          break;
        case 1:
  ...
  if (scale_pair_scale != 1)
    {
      switch (QMessageBox::question (
          ...
          us_tr ("Scaled &MALS data"), us_tr ("Inverse scaled &SAXS data")))
        {
        case 0:
          break;
        case 1:
  ...
  // create_saxs == true ... create MALS without scaling only if sd_mult != 1,
  // create SAXS files with inverse scaling
      ...
      if (scale_pair_use_errors)
        {
          ...
          for (auto const &scaled_time_to_name : scale_pair_time_to_names)
            {
              QString source_name
                  = scaled_time_to_name.second[scale_pair_mals_set];
              ...
              add_plot (scaled_name, scale_pair_qgrids[scale_pair_mals_set], I,
                        e, false, false);
              ...
              if (!last_minimize_method.isEmpty ())
                {
                  f_fit_method[last_created_file]
                      = QString ("%1:%2")
                            .arg (last_minimize_method)
                            .arg (last_minimize_weight);
                }
              else
                {
              ...
              if (last_minimize_chi2s.count (scaled_time_to_name.first))
                {
                  f_fit_chi2[last_created_file]
                      = last_minimize_chi2s[scaled_time_to_name.first];
                }
              else
                {
          ...
          for (auto const &scaled_time_to_name : scale_pair_time_to_names)
            {
              ...
              QString scaled_name = source_name;
              scaled_name = scaled_name.replace ("_common", "");
              scaled_name += QString (suffix).arg (use_scale_pair_scale);
  ...
  if (create_saxs)
    {
      ...
      int qgrid_size_saxs
          = (int)scale_pair_qgrids[scale_pair_saxs_set].size ();
      ...
      if (scale_pair_use_errors)
        {
          for (auto const &scaled_time_to_name : scale_pair_time_to_names)
            {
              QString source_name
                  = scaled_time_to_name.second[scale_pair_saxs_set];
              QString scaled_name = source_name;
              scaled_name = scaled_name.replace ("_common", "");
              scaled_name += QString (suffix).arg (use_scale_pair_scale);
              ...
              add_plot (scaled_name, scale_pair_qgrids[scale_pair_saxs_set], I,
                        e, false, false);
              scale_pair_save_names.insert (last_created_file);
              scale_pair_org_selected.erase (source_name);
              scale_pair_org_selected.insert (last_created_file);
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_minimize_clear ()
{
...
void
US_Hydrodyn_Mals_Saxs::scale_pair_fit_clear (bool replot)
{
  if (!minimize_running)
    {
