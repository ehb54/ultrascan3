    ...
    case USFD_ENRIGHT_FULL: {
      ...
      for (double enright_ca_pct = 0; enright_ca_pct <= 90;
        ...
        if (!compute_enright(points, angstrom_start, angstrom_end,
                             ...
                             enright_ca_pct + 10, fd, x, y, x_title, y_title,
                             errormsg)) {
          return false;
        }
...
bool US_Fractal_Dimension::compute_box_counting(
    const vector<pointmass>& points, double angstrom_start, double angstrom_end,
    double angstrom_steps, double& fd  // computed fractal dimension
    ,
    vector<vector<vector<double> > >& x  // x coordinate of plots
    ,
    vector<vector<vector<double> > >& y  // y coordinate of plots
  ...
  for (double edge_size = angstrom_start; edge_size <= angstrom_end;
    ...
    QTextStream(stdout)
        << QString(
               "box edge size %1 [%2], number of occupied boxes %3, volume of "
               "one box %4 [%5^3] ln(vol) %6 ln(number of occupied boxes) %7\n")
               .arg(edge_size)
...
bool US_Fractal_Dimension::compute_box_alternate(
  ...
  errormsg =
      "US_Fractal_Dimension::compute_box_alternate() - not fully implemented";
...
bool US_Fractal_Dimension::compute_enright(
    const vector<pointmass>& points, double angstrom_start, double angstrom_end,
    double angstrom_steps, double enright_ca_pct_start,
  ...
  if (points.size() < USFD_MIN_POINTS) {
    errormsg = QString("Invalid number of points for Enright %1 (minimum %2)")
                   .arg(points.size())
                   .arg(USFD_MIN_POINTS);
    return false;
  }
  ...
  size_t ca_index_end =
      (size_t)((double)ca_ordered_by_dist2.size() * enright_ca_pct_end / 100.0);
  ...
  for (size_t i = ca_index_start; i < ca_index_end; ++i) {
    ...
    for (double radius = angstrom_start; radius <= angstrom_end;
      ...
      for (auto const& p : points) {
        if (pow(p.axis[0] - ca_com[0], 2) + pow(p.axis[1] - ca_com[1], 2) +
...
bool US_Fractal_Dimension::compute_box_mass(
  ...
  if (points.size() < USFD_MIN_POINTS) {
    errormsg = QString("Invalid number of points for box mass %1 (minimum %2)")
                   .arg(points.size())
                   .arg(USFD_MIN_POINTS);
    return false;
  }
...
set<QString> US_Fractal_Dimension::mass_atoms_set() { return mass_atoms; }
