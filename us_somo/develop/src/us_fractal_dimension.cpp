...
bool
US_Fractal_Dimension::compute (
    enum US_Fractal_Dimension::methods method, const vector<pointmass> &points,
    double angstrom_start, double angstrom_end, double angstrom_steps,
    double enright_ca_pct_start, double enright_ca_pct_end,
...
)
{
  ...
  switch (method)
    {
    case USFD_BOX_MODEL:
      return compute_box_counting (points, angstrom_start, angstrom_end,
                                   angstrom_steps, fd, x, y, x_title, y_title,
                                   errormsg);
      break;

    case USFD_BOX_ALT:
      return compute_box_alternate (points, angstrom_start, angstrom_end,
                                    angstrom_steps, fd, x, y, x_title, y_title,
                                    errormsg);
      break;

    case USFD_BOX_MASS:
      return compute_box_mass (points, angstrom_start, angstrom_end,
                               angstrom_steps, fd, x, y, x_title, y_title,
                               errormsg);
      break;

    case USFD_ENRIGHT:
      return compute_enright (points, angstrom_start, angstrom_end,
    ...
    case USFD_ENRIGHT_FULL:
      {
        ...
        for (double enright_ca_pct = 0; enright_ca_pct <= 90;
             enright_ca_pct += 10)
          {
            x.clear ();
            y.clear ();

            if (!compute_enright (points, angstrom_start, angstrom_end,
                                  angstrom_steps, enright_ca_pct,
                                  enright_ca_pct + 10, fd, x, y, x_title,
                                  y_title, errormsg))
              {
                return false;
              }
            ...
            if (!x.size ())
              {
                errormsg = QString ("Enright failed to provide a result for "
                                    "slice %1 - %2 %")
                               .arg (enright_ca_pct)
    ...
    default:
      errormsg
          = "US_Fractal_Dimension::compute() invalid or unsupported method";
      return false;
    }
...
bool
US_Fractal_Dimension::compute_box_counting (
    const vector<pointmass> &points, double angstrom_start,
    double angstrom_end, double angstrom_steps,
    double &fd // computed fractal dimension
    ,
    vector<vector<vector<double> > > &x // x coordinate of plots
    ,
...
)
{
  ...
  if (points.size () < USFD_MIN_POINTS)
    {
      errormsg
          = QString (
                "Invalid number of points for box counting %1 (minimum %2)")
                .arg (points.size ())
                .arg (USFD_MIN_POINTS);
      return false;
    }
  ...
  for (double edge_size = angstrom_start; edge_size <= angstrom_end;
       edge_size += stepsize)
    {
      // qDebug() << QString( "US_Fractal_Dimension::compute_box_counting() :
      // processing edge_size %1" ).arg( edge_size );
      ...
      for (auto const &p : points)
        {
          occupied.insert ({ (int)(p.axis[0] * edge_inverse),
      ...
      QTextStream (stdout)
          << QString ("box edge size %1 [%2], number of occupied boxes %3, "
                      "volume of one box %4 [%5^3] ln(vol) %6 ln(number of "
                      "occupied boxes) %7\n")
                 .arg (edge_size)
  ...
  errormsg
      = "US_Fractal_Dimension::compute_box_counting() - not fully implemented";
...
bool
US_Fractal_Dimension::compute_box_alternate (
    const vector<pointmass> &points, double angstrom_start,
    double angstrom_end, double angstrom_steps,
    double &fd // computed fractal dimension
    ,
...
)
{
  ...
  if (points.size () < USFD_MIN_POINTS)
    {
      errormsg
          = QString (
                "Invalid number of points for box alternate %1 (minimum %2)")
                .arg (points.size ())
                .arg (USFD_MIN_POINTS);
      return false;
    }
  ...
  for (double edge_size = angstrom_start; edge_size <= angstrom_end;
       edge_size += stepsize)
    {
      // qDebug() << QString( "US_Fractal_Dimension::compute_box_alternate() :
      // processing edge_size %1" ).arg( edge_size );
      set<vector<int> > occupied;
  ...
  errormsg = "US_Fractal_Dimension::compute_box_alternate() - not fully "
             "implemented";
...
QStringList
US_Fractal_Dimension::list_points (const vector<pointmass> &points)
{
  QStringList result;
...
bool
US_Fractal_Dimension::compute_enright (
    const vector<pointmass> &points, double angstrom_start,
    double angstrom_end, double angstrom_steps, double enright_ca_pct_start,
    double enright_ca_pct_end, double &fd // computed fractal dimension
    ,
    vector<vector<vector<double> > > &x // x coordinate of plots
    ,
    vector<vector<vector<double> > > &y // y coordinate of plots
...
)
{
  ...
  qDebug ()
      << QString ("com [%1 %2 %3]").arg (com[0]).arg (com[1]).arg (com[2]);
    ...
    for (size_t i = 0; i < points.size (); ++i)
      {
        if (mass_atoms.count (points[i].name))
          {
            ca_dist2_to_center[pow (points[i].axis[0] - com[0], 2)
  ...
  // global limits
  size_t ca_index_start = (size_t)((double)ca_ordered_by_dist2.size ()
                                   * enright_ca_pct_start / 100.0);
  size_t ca_index_end = (size_t)((double)ca_ordered_by_dist2.size ()
                                 * enright_ca_pct_end / 100.0);
  ...
  for (size_t i = ca_index_start; i < ca_index_end; ++i)
    {
      ...
      vector<double> ca_com = { points[ca_ordered_by_dist2[i]].axis[0],
                                points[ca_ordered_by_dist2[i]].axis[1],
                                points[ca_ordered_by_dist2[i]].axis[2] };

      // qDebug() << QString( "ca_com %1 [%2 %3 %4]" ).arg( i ).arg( ca_com[ 0
      // ] ).arg( ca_com[1] ).arg( ca_com[2] );

      vector<double> tmp_x;
      vector<double> tmp_y;

      for (double radius = angstrom_start; radius <= angstrom_end;
           radius += stepsize)
        {
          // qDebug() << QString( "US_Fractal_Dimension::compute_enright() :
          // processing size %1" ).arg( radius );
          double radius2 = radius * radius;
          double mass = 0;

          for (auto const &p : points)
            {
              if (pow (p.axis[0] - ca_com[0], 2)
      ...
      // US_Vector::printvector2( QString( "enright %1, x,y" ).arg( i ), x[ i
      // ], y[ i ] );
    }
...
bool
US_Fractal_Dimension::compute_box_mass (
    const vector<pointmass> &points, double angstrom_start,
    double angstrom_end, double angstrom_steps,
    double &fd // computed fractal dimension
    ,
...
)
{
  ...
  for (double edge_size = angstrom_start; edge_size <= angstrom_end;
       edge_size += stepsize)
    {
      // qDebug() << QString( "US_Fractal_Dimension::compute_box_mass() :
      // processing edge_size %1" ).arg( edge_size );
      set<vector<int> > occupied;
  ...
  errormsg
      = "US_Fractal_Dimension::compute_box_mass() - not fully implemented";
...
set<QString>
US_Fractal_Dimension::mass_atoms_set ()
{
  return mass_atoms;
}

QStringList
US_Fractal_Dimension::mass_atoms_qstringlist ()
{
  QStringList result;
