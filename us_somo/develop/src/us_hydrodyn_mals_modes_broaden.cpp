...
static double bblm_fit_mode_half_gaussian( double t, const double *par ) {
   ...
   if ( !uhm->broaden_compute_one_no_ui(
                                         { sigma, tau, lambda_1 }
                                         ,bblm_kernel_size
                                         ,bblm_kernel_delta_t
                                         ,bblm_org_conc_I
   ...
   if ( !usu.linear_interpolate( bblm_conc_t, bblm_conc_I, bblm_ref_t, new_conc_I ) ) {
      uhm->editor_msg( "red", QString( "Error while fitting: interpolation error: %1\n" ).arg( usu.errormsg ) );
...
static double bblm_fit_mode_emg_gmg( double t, const double *par ) {
   ...
   // double loss = uhm->broaden_compute_loss_no_ui(
   //                                                bblm_conc_t
   //                                                ,bblm_conc_I
   //                                                ,bblm_ref_t
   //                                                ,bblm_ref_I
...
void US_Hydrodyn_Mals::broaden() {
   TSO << "US_Hydrodyn_Mals::broaden() start\n";
   ...
   broaden_parameter_value_le_widgets[ US_Band_Broaden::BAND_BROADEN_KERNEL_MODE_DEFAULT ] =
      {
         le_broaden_sigma
         ,le_broaden_tau
      };
   ...
   broaden_parameter_value_cb_widgets[ US_Band_Broaden::BAND_BROADEN_KERNEL_MODE_DEFAULT ] =
      {
         cb_broaden_sigma
         ,cb_broaden_tau
      };
   ...
   broaden_parameter_value_minimum[ US_Band_Broaden::BAND_BROADEN_KERNEL_MODE_EMG_GMG ] =
      {
         0
         ,-DBL_MAX
         ,-DBL_MAX
...
vector < double > US_Hydrodyn_Mals::broaden_parameter_current_values() {
   TSO << "US_Hydrodyn_Mals::broaden_parameter_current_values() start\n";
...
void US_Hydrodyn_Mals::broaden_done( bool save ) {
   TSO << "US_Hydrodyn_Mals::broaden_done() start\n";
   ...
   if ( save ) {
      TSO << "US_Hydrodyn_Mals::broaden_done() save\n";
      ...
      if ( !broadened.size() ) {
         ...
         case US_Band_Broaden::BAND_BROADEN_KERNEL_MODE_HALF_GAUSSIAN :
            ...
            fname = 
               ...
               .arg( le_broaden_sigma->text().toDouble(), 0, 'g', 3 )
               .arg( le_broaden_tau->text().toDouble(), 0, 'g', 3 )
               .arg( le_broaden_lambda_1->text().toDouble(), 0, 'g', 3 )
               ;
            break;
         ...
         case US_Band_Broaden::BAND_BROADEN_KERNEL_MODE_EMG_GMG :
            ...
            fname = 
               ...
               .arg( le_broaden_sigma->text().toDouble(), 0, 'g', 3 )
               .arg( le_broaden_lambda_1->text().toDouble(), 0, 'g', 3 )
               .arg( le_broaden_lambda_2->text().toDouble(), 0, 'g', 3 )
               ;
            break;
      ...
      TSO << "US_Hydrodyn_Mals::broaden_done() cancel\n";
   }
...
void US_Hydrodyn_Mals::broaden_enables() {
   if ( shutting_down ) {
      return;
   }

   TSO << "US_Hydrodyn_Mals::broaden_enables() start\n";
   ...
   cb_broaden_lambda_1          -> setEnabled( true );
   le_broaden_lambda_1          -> setEnabled( true );
   cb_broaden_lambda_2          -> setEnabled( true );
   le_broaden_lambda_2          -> setEnabled( true );
...
void US_Hydrodyn_Mals::broaden_plot( bool /* replot */ ) {
   TSO << "US_Hydrodyn_Mals::broaden_plot() start\n";
}

void US_Hydrodyn_Mals::broaden_clear_plot() {
   TSO << "US_Hydrodyn_Mals::broaden_clear_plot() start\n";
}

void US_Hydrodyn_Mals::broaden_minimize() {
   TSO << "US_Hydrodyn_Mals::broaden_minimize() start\n";
}

void US_Hydrodyn_Mals::broaden_reset() {
   TSO << "US_Hydrodyn_Mals::broaden_reset() start\n";
   ...
   cb_broaden_lambda_1 ->setChecked( true );
   le_broaden_lambda_1 ->setText( "0" );
   
   cb_broaden_lambda_2 ->setChecked( true );
...
void US_Hydrodyn_Mals::set_broaden_sigma() {
   TSO << "US_Hydrodyn_Mals::set_broaden_sigma() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::broaden_sigma_start_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_sigma_start_text() start\n";
}

void US_Hydrodyn_Mals::broaden_sigma_start_focus( bool ) {
   TSO << "US_Hydrodyn_Mals::broaden_sigma_start_focus() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::broaden_sigma_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_sigma_text() start\n";
}

void US_Hydrodyn_Mals::broaden_sigma_focus( bool hasFocus ) {
   TSO << "US_Hydrodyn_Mals::broaden_sigma_focus() start\n";
   if ( shutting_down ) {
      return;
   }
   broaden_enables();
...
void US_Hydrodyn_Mals::broaden_sigma_end_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_sigma_end_text() start\n";
}

void US_Hydrodyn_Mals::broaden_sigma_end_focus( bool ) {
   TSO << "US_Hydrodyn_Mals::broaden_sigma_end_focus() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::broaden_sigma_delta_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_sigma_delta_text() start\n";
}

void US_Hydrodyn_Mals::broaden_sigma_delta_focus( bool ) {
   TSO << "US_Hydrodyn_Mals::broaden_sigma_delta_focus() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::set_broaden_tau() {
   TSO << "US_Hydrodyn_Mals::set_broaden_tau() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::broaden_tau_start_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_tau_start_text() start\n";
}

void US_Hydrodyn_Mals::broaden_tau_start_focus( bool ) {
   TSO << "US_Hydrodyn_Mals::broaden_tau_start_focus() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::broaden_tau_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_tau_text() start\n";
}

void US_Hydrodyn_Mals::broaden_tau_focus( bool hasFocus ) {
   TSO << "US_Hydrodyn_Mals::broaden_tau_focus() start\n";
   if ( shutting_down ) {
      return;
   }
...
void US_Hydrodyn_Mals::broaden_tau_end_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_tau_end_text() start\n";
}

void US_Hydrodyn_Mals::broaden_tau_end_focus( bool ) {
   TSO << "US_Hydrodyn_Mals::broaden_tau_end_focus() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::broaden_tau_delta_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_tau_delta_text() start\n";
}

void US_Hydrodyn_Mals::broaden_tau_delta_focus( bool ) {
   TSO << "US_Hydrodyn_Mals::broaden_tau_delta_focus() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::set_broaden_lambda_1() {
   TSO << "US_Hydrodyn_Mals::set_broaden_lambda_1() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::broaden_lambda_1_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_lambda_1_text() start\n";
}

void US_Hydrodyn_Mals::broaden_lambda_1_focus( bool hasFocus ) {
   TSO << "US_Hydrodyn_Mals::broaden_lambda_1_focus() start\n";
   if ( shutting_down ) {
      return;
   }
   broaden_enables();
...
void US_Hydrodyn_Mals::set_broaden_lambda_2() {
   TSO << "US_Hydrodyn_Mals::set_broaden_lambda_2() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::broaden_lambda_2_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_lambda_2_text() start\n";
}

void US_Hydrodyn_Mals::broaden_lambda_2_focus( bool hasFocus ) {
   TSO << "US_Hydrodyn_Mals::broaden_lambda_2_focus() start\n";
   if ( shutting_down ) {
      return;
   }
   broaden_enables();
...
void US_Hydrodyn_Mals::set_broaden_deltat() {
   TSO << "US_Hydrodyn_Mals::set_broaden_deltat() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::broaden_deltat_start_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_deltat_start_text() start\n";
}

void US_Hydrodyn_Mals::broaden_deltat_start_focus( bool ) {
   TSO << "US_Hydrodyn_Mals::broaden_deltat_start_focus() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::broaden_deltat_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_deltat_text() start\n";
}

void US_Hydrodyn_Mals::broaden_deltat_focus( bool hasFocus ) {
   TSO << "US_Hydrodyn_Mals::broaden_deltat_focus() start\n";
   if ( shutting_down ) {
      return;
   }
   broaden_enables();
...
void US_Hydrodyn_Mals::broaden_deltat_end_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_deltat_end_text() start\n";
}

void US_Hydrodyn_Mals::broaden_deltat_end_focus( bool ) {
   TSO << "US_Hydrodyn_Mals::broaden_deltat_end_focus() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::broaden_deltat_delta_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_deltat_delta_text() start\n";
}

void US_Hydrodyn_Mals::broaden_deltat_delta_focus( bool ) {
   TSO << "US_Hydrodyn_Mals::broaden_deltat_delta_focus() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::set_broaden_baseline() {
   TSO << "US_Hydrodyn_Mals::set_broaden_baseline() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::broaden_baseline_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_baseline_text() start\n";
}

void US_Hydrodyn_Mals::broaden_baseline_focus( bool hasFocus ) {
   TSO << "US_Hydrodyn_Mals::broaden_baseline_focus() start\n";
   if ( shutting_down ) {
      return;
   }
   broaden_enables();
...
void US_Hydrodyn_Mals::set_broaden_scale() {
   TSO << "US_Hydrodyn_Mals::set_broaden_scale() start\n";
   broaden_enables();
}

void US_Hydrodyn_Mals::broaden_scale_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_scale_text() start\n";
}

void US_Hydrodyn_Mals::broaden_scale_focus( bool hasFocus ) {
   TSO << "US_Hydrodyn_Mals::broaden_scale_focus() start\n";
   if ( shutting_down ) {
      return;
   }
   broaden_enables();
...
void US_Hydrodyn_Mals::broaden_kernel_end_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_kernel_end_text() start\n";
}

void US_Hydrodyn_Mals::broaden_kernel_end_focus( bool hasFocus ) {
   TSO << "US_Hydrodyn_Mals::broaden_kernel_end_focus() start\n";
   if ( shutting_down ) {
      return;
   }
   broaden_enables();
...
void US_Hydrodyn_Mals::broaden_kernel_deltat_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_kernel_deltat_text() start\n";
}

void US_Hydrodyn_Mals::broaden_kernel_deltat_focus( bool hasFocus ) {
   TSO << "US_Hydrodyn_Mals::broaden_kernel_deltat_focus() start\n";
   if ( shutting_down ) {
      return;
   }
   broaden_enables();
...
void US_Hydrodyn_Mals::broaden_fit_range_start_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_fit_range_start_text() start\n";
}

void US_Hydrodyn_Mals::broaden_fit_range_start_focus( bool hasFocus ) {
   TSO << "US_Hydrodyn_Mals::broaden_fit_range_start_focus() start\n";
   if ( shutting_down ) {
      return;
   }
   broaden_enables();
   if ( !hasFocus ) {
      if ( plotted_markers.size() > 1 ) {
         ...
         TSO << "US_Hydrodyn_Mals::broaden_fit_range_focus() no marker\n";
      }
      broaden_compute_one();
   }
}

void US_Hydrodyn_Mals::broaden_fit_range_end_text( const QString & ) {
   TSO << "US_Hydrodyn_Mals::broaden_fit_range_end_text() start\n";
}

void US_Hydrodyn_Mals::broaden_fit_range_end_focus( bool hasFocus ) {
   TSO << "US_Hydrodyn_Mals::broaden_fit_range_end_focus() start\n";
   if ( shutting_down ) {
      return;
   }
   broaden_enables();
...
void US_Hydrodyn_Mals::broaden_repeak_set() {
   TSO << "US_Hydrodyn_Mals::broaden_repeak_set() start\n";
   broaden_compute_one();
}

void US_Hydrodyn_Mals::broaden_kernel_type_index() {
   TSO << "US_Hydrodyn_Mals::broaden_kernel_type_index() start\n";
   broaden_compute_one();
}

void US_Hydrodyn_Mals::broaden_kernel_mode_index() {
   TSO << "US_Hydrodyn_Mals::broaden_kernel_mode_index() start\n";
   broaden_enables();
   broaden_compute_one();
}

bool US_Hydrodyn_Mals::broaden_compute_one_no_ui(
...
void US_Hydrodyn_Mals::broaden_compute_one( bool details ) {
   broaden_last_fit = -1;
   ...
   TSO << "US_Hydrodyn_Mals::broaden_compute_one() start\n";
...
void US_Hydrodyn_Mals::broaden_lm_fit( bool final_refinement_only ) {
   TSO << "US_Hydrodyn_Mals::broaden_lm_fit() start\n";

   uhm = (US_Hydrodyn_Mals *)this;
   ...
   if ( !broaden_parameter_value_cb_widgets.count( cb_broaden_kernel_mode->currentIndex() )
        || !broaden_parameter_value_le_widgets.count( cb_broaden_kernel_mode->currentIndex() )
        || !broaden_parameter_value_minimum.count( cb_broaden_kernel_mode->currentIndex() )
        ) {
      QString msg = "Fit error: internal error: kernel mode missing from tables!";
      editor_msg( "red", msg + "\n" );
      qDebug() << msg;
      return;
   }

   // setup kernel mode specific variables
...
#warning BROADEN_LM_ENORM  is defined, alternate norm for LM fitting
...

   {
      ...
      for ( size_t i = 0; i < cb_widgets.size(); ++i ) {
         if ( cb_widgets[ i ]->isChecked() ) {
            le_widgets[ i ]->setText( QString( "%1" ).arg( best_params[ pos++ ], 0, 'g', 8 ) );
         }
      }
   ...
   if ( final_loss > original_loss ) {
      ...
      for ( size_t i = 0; i < cb_widgets.size(); ++i ) {
         if ( cb_widgets[ i ]->isChecked() ) {
            le_widgets[ i ]->setText( QString( "%1" ).arg( bblm_init_params[ pos++ ], 0, 'g', 8 ) );
         }
      }
   ...
   if ( !final_refinement_only ) {
      ...
      for ( size_t i = 0; i < cb_widgets.size(); ++i ) {
         cb_all.insert( cb_widgets[ i ] );
         if ( cb_widgets[ i ]->isChecked() ) {
            cb_active.insert( cb_widgets[ i ] );
         }
      }
...
void US_Hydrodyn_Mals::broaden_fit() {
   TSO << "US_Hydrodyn_Mals::broaden_fit() start\n";
...
double US_Hydrodyn_Mals::broaden_compute_loss() {
   TSO << "US_Hydrodyn_Mals::broaden_compute_loss() start\n";
...
void US_Hydrodyn_Mals::broaden_scale_compute() {
   TSO << "US_Hydrodyn_Mals::broaden_scale_compute() start\n";
...
vector < double > US_Hydrodyn_Mals::broaden_params() {
   vector < double > result;

   for ( auto const & le : broaden_parameter_value_le_widgets[ cb_broaden_kernel_mode->currentIndex() ] ) {
      result.push_back( le->text().toDouble() );
   }
   for ( auto const & le : {
         le_broaden_deltat
