...
bool
US_Saxs_Util::run_ssbond (map<QString, QString> &parameters,
                          map<QString, QString> &results)
{
  ...
  if (parameters.count ("output"))
    {
      QString error;
      if (!US_File_Util::putcontents (parameters["output"], ssbond_data,
                                      error))
        {
...
void
US_Saxs_Util::SS_apply (struct PDB_model &model, QString &ssbond_data)
{
...
void
US_Saxs_Util::SS_change_residue (struct PDB_model &model, const QString &line,
                                 const QString target_residue)
{
  ...
  if (!multi_residue_map.count (target_residue)
    ...
    {
      errormsg += QString (us_tr ("Residue file is missing residue name %1, "
