...
bool US_Saxs_Util::run_ssbond(map<QString, QString>& parameters,
  ...
  if (!fi.exists()) {
    ...
    if (!fi.isReadable()) {
      results["errors"] +=
          QString(" file %1 exists but is not readable.").arg(file);
      return false;
    } else {
      file = fi.filePath();
    }
...
void US_Saxs_Util::SS_change_residue(struct PDB_model& model,
                                     const QString& line,
  ...
  if (!multi_residue_map.count(target_residue) ||
    ...
    errormsg += QString(us_tr("Residue file is missing residue name %1, can "
                              "not covert residue %2 %3 chain %4"))
                    .arg(target_residue)
