...
US_Hydrodyn_Cluster_Advanced::US_Hydrodyn_Cluster_Advanced (csv &csv1,
                                                            void *us_hydrodyn,
                                                            QWidget *p,
                                                            const char *)
    : QDialog (p)
{
  ...
  QDir::setCurrent (USglobal->config_list.root_dir + SLASH + "somo" + SLASH
                    + "cluster");
...
void
US_Hydrodyn_Cluster_Advanced::setupGUI ()
{
  int minHeight1 = 30;
  ...
  for (unsigned int i = 0; i < csv1.data.size (); i++)
    {
      ...
      if (csv1.data[i].size () < 3
          || (csv1.data[i][1].isEmpty () && csv1.data[i][2].isEmpty ()))
        {
          {
            for (int i = 0; i < t_csv->columnCount (); ++i)
              {
                t_csv->item (i, i)->setFlags (t_csv->item (i, i)->flags ()
                                              ^ Qt::ItemIsEditable);
              }
          };
        }
...
void
US_Hydrodyn_Cluster_Advanced::ok ()
{
...
void
US_Hydrodyn_Cluster_Advanced::cancel ()
{
  close ();
}

void
US_Hydrodyn_Cluster_Advanced::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Cluster_Advanced::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept ();
...
void
US_Hydrodyn_Cluster_Advanced::table_value (int row, int col)
{
...
void
US_Hydrodyn_Cluster_Advanced::clear_display ()
{
  editor->clear ();
  editor->append ("\n\n");
}
...
void
US_Hydrodyn_Cluster_Advanced::update_font ()
{
  bool ok;
  QFont newFont;
...
void
US_Hydrodyn_Cluster_Advanced::save ()
{
  QString fn;
...
csv
US_Hydrodyn_Cluster_Advanced::current_csv ()
{
  csv tmp_csv = csv1;
...
void
US_Hydrodyn_Cluster_Advanced::recompute_interval_from_points ()
{
...
void
US_Hydrodyn_Cluster_Advanced::recompute_points_from_interval ()
{
  for (unsigned int i = 0; i < (unsigned int)t_csv->rowCount (); i++)
    {
      QString toset
...
void
US_Hydrodyn_Cluster_Advanced::editor_msg (QString color, QString msg)
{
...
void
US_Hydrodyn_Cluster_Advanced::reset_csv ()
{
