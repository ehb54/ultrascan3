...
US_Saxs_Util::run_best ()
{
  ...
  QString prog_base = env_ultrascan + QDir::separator () +
#if defined(BIN64)
                      "bin64"
#else
                      "bin"
  ...
  if (!strip_pdb (pdb_stripped, control_parameters["inputfilenoread"],
                  exclude_atoms_list, exclude_residues_list, true))
    {
      return false;
    }
  ...
  int p = 0;
  double start_fineness
      = (control_parameters.count ("bestmsrfinenessangle")
         && control_parameters["bestmsrfinenessangle"].toDouble () >= .5)
            ? control_parameters["bestmsrfinenessangle"].toDouble ()
            : (double)0.8;
  ...
  for (double use_fineness = start_fineness;
       found_triangles <= max_triangles && use_fineness >= 0.3;
       use_fineness -= 0.02)
    {
      QString msr_inputbase = QString ("%1_f%2").arg (inputbase).arg (
          QString ("%1").arg (use_fineness).replace (".", "_"));
      ...
      if (control_parameters.count ("bestmsrpatternfile"))
        {
          cmd = QString ("ulimit -s 16535; %1 -m %2 -r %3 -y %4 -t %5.c3p -v "
                         "%6.c3v")
                    .arg (progs[p])
                    .arg (pdb_stripped)
        ...
        {
          cmd = QString ("ulimit -s 16535; %1 -m %2 -r %3 -t %4.c3p -v %5.c3v")
                    .arg (progs[p])
                    .arg (pdb_stripped)
                    .arg (control_parameters["bestmsrradiifile"])
                    .arg (msr_inputbase)
      ...
      //      {
      //         cmd += QString( " -f %1" ).arg( control_parameters[
      ...
      if (control_parameters.count ("bestmsrcoalescer"))
        {
          cmd += QString (" -l %1").arg (
              control_parameters["bestmsrcoalescer"]);
        }

      cmd += QString (" 2> msr_%1.stderr > msr_%2.stdout")
                 .arg (msr_inputbase)
                 .arg (msr_inputbase);
      ...
      expected << msr_inputbase + ".c3p" << msr_inputbase + ".c3v"
          ...
          if (!QFile (expected[i]).exists ())
            {
              errormsg
                  += QString (
                         "BEST: %1 did not produce expected output file %2\n")
                         .arg (progs[p])
                         .arg (expected[i]);
            }
          else
      ...
      // count triangles produced
      {
        ...
        while (!ts.atEnd ())
          {
            ...
            if (qs.contains ("cannot open fineness file"))
              {
                errormsg
                    = QString ("BEST: msroll error with names/radii files %1")
            ...
            if (qs.contains ("triangles written to disk"))
              {
                ...
                // QStringList qsl = (qs.trimmed().split( " " ,
                // Qt::SkipEmptyParts ) );
                QStringList qsl;
                {
                ...
                if (qsl.size ())
                  {
                    ...
                    // us_qdebug( QString( "found triangles %1 with fineness %2
                    // on %3" )
                    //         .arg( found_triangles )
                    //         .arg( use_fineness )
                    //         .arg( control_parameters[ "inputfilenoread" ] )
                    ...
                    if (use_fineness == start_fineness
                        && found_triangles > max_triangles)
                      {
                        control_parameters["bestmsrfinenessangle"]
                            = QString ("%1").arg (
                                control_parameters["bestmsrfinenessangle"]
                                    .toDouble ()
                                + 0.1e0);
                        // us_qdebug( QString( "too many triangles with largest
                        // fineness, incrementing max fineness to %1 on %2" )
                        //         .arg( control_parameters[
                        //         "bestmsrfinenessangle" ] ) .arg(
                        ...
                        for (int i = 0; i < (int)rm_output_files.size (); ++i)
                          {
                            // us_qdebug( QString( "removing %1" ).arg(
                            // rm_output_files[ i ] ) );
                            QFile::remove (rm_output_files[i]);
                          }
                        if (control_parameters["bestmsrfinenessangle"]
                                .toDouble ()
                            > 2e0)
                          {
                            errormsg
                                = QString (
                                      "BEST: starting fineness too large %1")
                                      .arg (control_parameters
                                                ["inputfilenoread"]);
                            return false;
                          }
                    ...
                    if (found_triangles > best_triangles
                        && found_triangles <= max_triangles)
                      {
        ...
        f.close ();
        if (!did_find_triangles && !found_triangles)
          {
            control_parameters["bestmsrfinenessangle"] = QString ("%1").arg (
                control_parameters["bestmsrfinenessangle"].toDouble ()
                + 0.1e0);
            // us_qdebug( QString( "msroll failed and failed for previous
            // values of fineness, incrementing max fineness to %1 on %2" )
            ...
            if (control_parameters["bestmsrfinenessangle"].toDouble () > 2e0)
              {
                errormsg = QString ("BEST: starting fineness too large %1")
                               .arg (control_parameters["inputfilenoread"]);
                return false;
              }
  ...
  {
    QStringList expected;
    expected << inputbase + ".c3p" << inputbase + ".c3v"
  ...
  QString inputbase23
      = QString ("%1_f%2")
            .arg (inputbase.left (17))
            .arg (QString ("%1").arg (best_fineness).replace (".", "_"));
  p++;
  {
    US_File_Util ufu;
    ufu.copy (QString ("%1.c3p").arg (inputbase),
              QString ("%1.c3p").arg (inputbase23), true);
    ufu.copy (QString ("%1.c3v").arg (inputbase),
              QString ("%1.c3v").arg (inputbase23), true);

    QString cmd = QString ("%1 -f %2.c3p -nmax %3 -nmin %4 -n %5")
                      .arg (progs[p])
                      .arg (inputbase23)
    ...
    cmd += QString (" 2> rcoal_%1.stderr > rcoal_%2.stdout")
               .arg (inputbase)
               .arg (inputbase);
    ...
    // run & check output
    QStringList expected;
    expected << "rcoal_" + inputbase + ".stdout"
            ...
            if (!qs.isEmpty ())
              {
                ...
                output_files << qs;
                rm_output_files << qs;
                // us_qdebug( QString( "outfiles.back() is %1" ).arg(
                // outfiles.back() ) );
              }
            else
              {
                errormsg += "could not correctly find output files in rcoal "
                            "stdout\n";
                // lower max triangles
              }
          }
  ...
  if (!outfiles.size ())
    {
      ...
      errormsg += QString ("too few triangles %1 after rcoal fail")
                      .arg (max_triangles);
      return false;
    }
        ...
        int beads = 0;
        {
          QTextStream ts (&f);
          {
            ...
            if (rx_vert.indexIn (qs) == -1)
              {
                errormsg += QString ("Rcoal output file %1 improper format on "
                                     "line 1 %1 <%2>\n")
                                .arg (f.fileName ())
                                .arg (qs);
                f.close ();
            ...
            for (int i = 0; i < vertices; ++i)
              {
                ...
                if (rx_data.indexIn (qs) == -1)
                  {
                    errormsg += QString ("Rcoal output file %1 improper "
                                         "format on line %1 %2 <%3>\n")
                                    .arg (i + 2)
                                    .arg (f.fileName ())
                                    .arg (qs);
                    f.close ();
  ...
  for (int i = 0; i < (int)outfiles.size (); ++i)
    {
      ...
      QString cmd = QString ("%1 -f %2 -mw %3")
                        .arg (progs[p])
                        .arg (outfiles[i])
      ...
      cmd += QString (" 2> best_%1.stderr > best_%2.stdout")
                 .arg (outfiles[i])
                 .arg (outfiles[i]);
      ...
      QStringList expected;
      expected << "best_" + outfiles[i] + ".stdout"
                ...
                if (qs.contains ("NaN"))
                  {
                    is_nan = true;
                    noticemsg
                        += QString ("BEST run for %1 produced NaN results\n")
                               .arg (f.fileName ());
                  }
              }
            f.close ();
      ...
      expected << outfiles[i] + expected_base + ".log"
      ...
      if (!is_nan)
        {
          ...
          // us_qdebug( QString( "outfiles[ i ] '%1' inputbase '%2' inputbase23
          // '%3'" )
          //         .arg( outfiles[ i ] ).arg( inputbase ).arg( inputbase23 )
          //         );
          triangles << QString (outfiles[i])
                           .replace (
                               QRegExp (QString ("^%1_").arg (inputbase23)),
                               "")
                           .replace (QRegExp ("^0*"), "");
          one_over_triangles.push_back (
              triangles.back ().toDouble () != 0e0
                  ? 1e0 / triangles.back ().toDouble ()
                  : -1e0);
      ...
      else
        {
          ...
          //         .arg( control_parameters[ "inputfilenoread" ] )
          //         );
          errormsg += QString ("BEST: %1 did produced NaN results in %2 "
                               "(suggest lowering MSROLL max triangles)\n")
                          .arg (progs[p])
          ...
          if (max_triangles > MIN_TRIANGLES)
            {
              ...
              for (int i = 0; i < (int)rm_output_files.size (); ++i)
                {
                  // us_qdebug( QString( "removing %1" ).arg( rm_output_files[
                  // i ] ) );
                  QFile::remove (rm_output_files[i]);
                }
              return run_best ();
    ...
    for (int i = 0; i < (int)csvfiles.size (); ++i)
      {
        ...
        if (this_tau_input.size () != 3)
          {
            errormsg = QString ("incorrect # of tau inputs %2")
                           .arg (this_tau_input.size ());
            return false;
          }
        compute_tau (this_tau_input[0], this_tau_input[1], this_tau_input[2],
                     .1, this_tau_results);
    ...
    if (1 || do_linear_fit)
      {
        ...
        ts << "\"Extrapolation to zero triangles (a)\",\"Sigma a\",\"Sigma a "
    ...
    for (int i = 0; i < (int)csv_header.size (); ++i)
      {
        ...
        for (int j = 0; j < (int)csvresults.size (); ++j)
          {
            ...
            // us_qdebug( QString( "i %1 missing_data %2 do_linear_fit %3
            // extrapolate.count( i ) %4" )
            //         .arg( i )
        ...
        if (!missing_data && do_linear_fit && extrapolate.count (i))
          {
            linear_fit (one_over_triangles, y, a, b, siga, sigb, chi2);
            ts << QString ("=%1,=%2,=%3,=%4,=%5,=%6,=%7")
                      .arg (a, 0, 'g', 8)
                      .arg (siga, 0, 'g', 8)
                      .arg (a != 0 ? fabs (100.0 * siga / a) : (double)0, 0,
                            'g', 8)
                      .arg (b, 0, 'g', 8)
                      .arg (sigb, 0, 'g', 8)
                      .arg (b != 0 ? fabs (100.0 * sigb / b) : (double)0, 0,
                            'g', 8)
                      .arg (chi2, 0, 'g', 8);
          }
        ts << Qt::endl;
...
QStringList
US_Saxs_Util::best_output_column (QString fname)
{
  QFile f (fname);
  QStringList qsl;
  if (!f.open (QIODevice::ReadOnly))
  ...
  QRegExp rx_3s ("Dx =\\s+([0-9.+-E]+)\\s+Dy =\\s+([0-9.+-E]+)\\s+Dz "
                 "=\\s+([0-9.+-E]+)\\s*$");
  ...
  for (int i = 0; i < 8; ++i)
    {
      ...
      if (rx_1.indexIn (qs) == -1)
        {
          qsl << QString ("error in %1 could not read data pos %2")
                     .arg (f.fileName ())
                     .arg (i + 5);
          f.close ();
          return qsl;
    ...
    if (rx_3s.indexIn (qs) == -1)
      {
        qsl << QString ("error in %1 could not read data pos %2")
                   .arg (f.fileName ())
                   .arg ("Vector between Center of Viscosity/Centroid");
        f.close ();
        return qsl;
      }
    qsl << rx_3s.cap (1);
  ...
  for (int i = 0; i < 2; ++i)
    {
      ...
      if (rx_1.indexIn (qs) == -1)
        {
          qsl << QString ("error in %1 could not read data pos %2 %3")
                     .arg (f.fileName ())
                     .arg (i)
                     .arg ("CHI");
          f.close ();
...
bool
US_Saxs_Util::strip_pdb (QString &pdb_stripped, const QString &pdb,
