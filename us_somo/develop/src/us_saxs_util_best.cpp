...
bool US_Saxs_Util::run_best() {
  ...
  for (int i = 0; i < (int)required.size(); ++i) {
    if (!control_parameters.count(required[i])) {
      errormsg +=
          QString("BEST: %1 parameter must be defined\n").arg(required[i]);
    }
  }
  ...
  for (double use_fineness = start_fineness;
    ...
    if (control_parameters.count("bestmsrpatternfile")) {
      cmd = QString("ulimit -s 16535; %1 -m %2 -r %3 -y %4 -t %5.c3p -v %6.c3v")
                .arg(progs[p])
                .arg(pdb_stripped)
    ...
    for (int i = 0; i < (int)expected.size(); ++i) {
      if (!QFile(expected[i]).exists()) {
        errormsg +=
            QString("BEST: %1 did not produce expected output file %2\n")
                .arg(progs[p])
                .arg(expected[i]);
      } else {
    ...
    // count triangles produced
    {
      ...
      while (!ts.atEnd()) {
        ...
        if (qs.contains("cannot open fineness file")) {
          errormsg = QString("BEST: msroll error with names/radii files %1")
          ...
          if (qsl.size()) {
            ...
            if (use_fineness == start_fineness &&
              ...
              control_parameters["bestmsrfinenessangle"] = QString("%1").arg(
                  control_parameters["bestmsrfinenessangle"].toDouble() +
                  0.1e0);
              // us_qdebug( QString( "too many triangles with largest fineness,
              // incrementing max fineness to %1 on %2" )
      ...
      if (!did_find_triangles && !found_triangles) {
        control_parameters["bestmsrfinenessangle"] = QString("%1").arg(
            control_parameters["bestmsrfinenessangle"].toDouble() + 0.1e0);
        // us_qdebug( QString( "msroll failed and failed for previous values of
  ...
  if (!outfiles.size()) {
    ...
    errormsg +=
        QString("too few triangles %1 after rcoal fail").arg(max_triangles);
    return false;
  }
  ...
  // make bead models
  {
    ...
    for (int i = 0; i < (int)outfiles.size(); ++i) {
      ...
      if (!f.open(QIODevice::ReadOnly)) {
        errormsg +=
            QString("Could not open rcoal output file %1\n").arg(f.fileName());
        return false;
      }
      ...
      int beads = 0;
      {
        QTextStream ts(&f);
        {
          ...
          if (rx_vert.indexIn(qs) == -1) {
            errormsg +=
                QString(
                    "Rcoal output file %1 improper format on line 1 %1 <%2>\n")
                    .arg(f.fileName())
                    .arg(qs);
            f.close();
            return false;
...

        {
          ...
          for (int i = 0; i < vertices; ++i) {
            ...
            if (rx_data.indexIn(qs) == -1) {
              errormsg += QString(
                              "Rcoal output file %1 improper format on line %1 "
                              "%2 <%3>\n")
                              .arg(i + 2)
                              .arg(f.fileName())
                              .arg(qs);
  ...
  for (int i = 0; i < (int)outfiles.size(); ++i) {
    ...
    if (!is_nan) {
      ...
      triangles << QString(outfiles[i])
                       .replace(QRegExp(QString("^%1_").arg(inputbase23)), "")
                       .replace(QRegExp("^0*"), "");
      ...
      //         );
      errormsg += QString(
                      "BEST: %1 did produced NaN results in %2 (suggest "
                      "lowering MSROLL max triangles)\n")
                      .arg(progs[p])
...

  {
    ...
    for (int i = 0; i < (int)csvfiles.size(); ++i) {
      ...
      if (this_tau_input.size() != 3) {
        errormsg =
            QString("incorrect # of tau inputs %2").arg(this_tau_input.size());
        return false;
      }
      compute_tau(this_tau_input[0], this_tau_input[1], this_tau_input[2], .1,
                  this_tau_results);
...
bool US_Saxs_Util::strip_pdb(QString& pdb_stripped, const QString& pdb,
  ...
  while (!tsi.atEnd()) {
    ...
    if (rx_check_line.indexIn(qs) != -1) {
      ...
      if (exclude_hydrogens && (qs.mid(12, 1) == "H" || qs.mid(13, 1) == "H")) {
        keep = false;
      } else {
