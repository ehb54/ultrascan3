...
bool
US_Hydrodyn_Mals_Saxs::create_i_of_q_ng (set<QString> &fileset, double t_min,
                                         double t_max)
{
  QStringList files;
...
bool
US_Hydrodyn_Mals_Saxs::create_i_of_q_ng (QStringList files, double t_min,
                                         double t_max)
{
      ...
      if (use_conc)
        {
          ...
          US_Hydrodyn_Mals_Saxs_Ciq *mals_saxs_ciq
              = new US_Hydrodyn_Mals_Saxs_Ciq (this, &parameters, this);
...
bool
US_Hydrodyn_Mals_Saxs::create_i_of_q (set<QString> &fileset, double t_min,
                                      double t_max)
{
  QStringList files;
  for (set<QString>::iterator it = fileset.begin (); it != fileset.end ();
...
bool
US_Hydrodyn_Mals_Saxs::create_i_of_q (QStringList files, double t_min,
                                      double t_max)
{
  // us_qdebug( "ciq 0" );
  // for each selected file
  ...
  else
    {
      ...
      parameters["mals_saxs_cb_makeiq_avg_peaks"]
          = ((US_Hydrodyn *)us_hydrodyn)
                ->gparams["mals_saxs_cb_makeiq_avg_peaks"];
      parameters["mals_saxs_makeiq_avg_peaks"]
          = ((US_Hydrodyn *)us_hydrodyn)
                ->gparams["mals_saxs_makeiq_avg_peaks"];
...

  {
    double cutoff_frac
        = ((US_Hydrodyn *)us_hydrodyn)
                  ...
                  == "true"
              ? ((US_Hydrodyn *)us_hydrodyn)
                        ->gparams["mals_saxs_makeiq_cutmax_pct"]
                        .toDouble ()
                    * 0.01
              : 0e0;
  ...
  {
    avg_peaks = ((US_Hydrodyn *)us_hydrodyn)
                    ->gparams["mals_saxs_cb_makeiq_avg_peaks"]
                == "true";
...
bool
US_Hydrodyn_Mals_Saxs::create_unified_ggaussian_target (bool do_init,
...
bool
US_Hydrodyn_Mals_Saxs::create_unified_ggaussian_target (QStringList &files,
                                                        bool do_init,
                                                        bool only_init_unset)
{
  ...
  if (do_init)
    {
      ...
      if (((US_Hydrodyn *)us_hydrodyn)
              ->gparams.count ("mals_saxs_cb_gg_smooth")
          && ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_cb_gg_smooth"]
                 == "true"
          && ((US_Hydrodyn *)us_hydrodyn)
                 ->gparams.count ("mals_saxs_gg_smooth")
          && ((US_Hydrodyn *)us_hydrodyn)
                     ->gparams["mals_saxs_gg_smooth"]
                     .toInt ()
                 > 0)
        {
          editor_msg (
...
bool
US_Hydrodyn_Mals_Saxs::ggauss_recompute ()
{
...
bool
US_Hydrodyn_Mals_Saxs::compute_f_gaussians (QString file,
                                            QWidget *mals_saxs_fit_widget)
{
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("mals_saxs_cb_gg_cyclic")
  ...
  // find peak of curve
  // US_Hydrodyn_Mals_Saxs_Fit *fit = (US_Hydrodyn_Mals_Saxs_Fit
  // *)mals_saxs_fit_widget;
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("mals_saxs_cb_gg_oldstyle")
             ...
             == "true")
    {
      bool cyclic_on
          = ((US_Hydrodyn *)us_hydrodyn)
                ->gparams.count ("mals_saxs_cb_gg_cyclic")
            && ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_cb_gg_cyclic"]
                   == "true";
      bool smooth_on
          = ((US_Hydrodyn *)us_hydrodyn)
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("mals_saxs_cb_gg_smooth")
      ...
      && ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_gg_smooth"].toInt ()
             > 0)
    {
      if (!saved)
        {
      ...
      int smoothing = ((US_Hydrodyn *)us_hydrodyn)
                          ->gparams["mals_saxs_gg_smooth"]
                          .toInt ();
...
bool
US_Hydrodyn_Mals_Saxs::initial_ggaussian_fit (QStringList &files,
                                              bool only_init_unset)
{
  wheel_file = files[0];
  ...
  US_Hydrodyn_Mals_Saxs_Fit *mals_saxs_fit_window
      = new US_Hydrodyn_Mals_Saxs_Fit (this, false, this);
...
void
US_Hydrodyn_Mals_Saxs::add_plot_gaussian (const QString &file,
                                          const QString &tag)
{
...
bool
US_Hydrodyn_Mals_Saxs::compute_f_gaussians_trial (
    QString file, QWidget *mals_saxs_fit_widget)
{
  ...
  US_Hydrodyn_Mals_Saxs_Fit *fit
      = (US_Hydrodyn_Mals_Saxs_Fit *)mals_saxs_fit_widget;
...
void
US_Hydrodyn_Mals_Saxs::clear_smoothed ()
{
  f_qs_smoothed.clear ();
  f_Is_smoothed.clear ();
...
void
US_Hydrodyn_Mals_Saxs::list_smoothed ()
{
  TSO << "list_smoothed():\n";
...
void
US_Hydrodyn_Mals_Saxs::add_smoothed (const QString &name,
...
void
US_Hydrodyn_Mals_Saxs::clear_oldstyle ()
{
  f_qs_oldstyle.clear ();
  f_Is_oldstyle.clear ();
}
...
void
US_Hydrodyn_Mals_Saxs::list_oldstyle ()
{
  TSO << "list_oldstyle():\n";
...
void
US_Hydrodyn_Mals_Saxs::add_oldstyle (const QString &name,
                                     const vector<double> &q,
                                     const vector<double> &I)
{
...
void
US_Hydrodyn_Mals_Saxs::create_ihash_t ()
{
  disable_all ();
...
bool
US_Hydrodyn_Mals_Saxs::create_ihash_t (QStringList files)
{
  double lambda_cm = mals_saxs_param_lambda * 1e-7; // nm to cm

  double K = (4 * M_PI * M_PI * mals_saxs_param_n * mals_saxs_param_n
...
void
US_Hydrodyn_Mals_Saxs::deselect_conc_file ()
{
  if (!lbl_conc_file->text ().isEmpty ())
    {
      for (int i = 0; i < lb_files->count (); i++)
...
void
US_Hydrodyn_Mals_Saxs::create_istar_q ()
{
  disable_all ();
...
bool
US_Hydrodyn_Mals_Saxs::create_istar_q (QStringList files, double t_min,
                                       double t_max)
{
...
bool
US_Hydrodyn_Mals_Saxs::create_istar_q_ng (QStringList files, double t_min,
                                          double t_max)
{
  ...
  if (!use_conc && !mals_saxs_param_g_conc)
    {
      if (1
          == QMessageBox::question (
              ...
              us_tr ("Global concentration in Options->MALS_SAXS parameters "
                     "is zero\n"
  ...
  if (!use_conc && !make_ihashq)
    {
      QMessageBox::information (
          this, windowTitle () + us_tr (": Make I*(q)"),
          QString (
              us_tr ("I*(q) will be created using Global concentration of %1 "
  ...
  head = head.replace (QRegularExpression ("[\\[\\]{}]"), "");
  head += make_ihashq ? "_MALS_SAXS_Ihashq_t" : "_MALS_SAXS_Istarq_t";
      ...
      if (use_conc)
        {
          ...
          parameters["default_extc"]
              = QString ("%1").arg (mals_saxs_param_g_extinction_coef);
          parameters["default_dndc"]
              = QString ("%1").arg (mals_saxs_param_g_dndc);
          US_Hydrodyn_Mals_Saxs_Ciq *mals_saxs_ciq
              = new US_Hydrodyn_Mals_Saxs_Ciq (this, &parameters, this);
          US_Hydrodyn::fixWinButtons (mals_saxs_ciq);
          mals_saxs_ciq->exec ();
  ...
  for (unsigned int t = 0; t < tv.size (); t++)
    {
      ...
      double conc_factor
          = make_ihashq ? 1 : 1 / (mals_saxs_param_g_conc * 1e-3);
      ...
      if (use_conc && conc_ok)
        {
          if (!usu->apply_natural_spline (conc_spline_x, conc_spline_y,
          ...
          else
            {
              ...
              // mals_saxs_param_g_dndc ^2 / dndc ^2 used to correct for K when
              // computing I#
