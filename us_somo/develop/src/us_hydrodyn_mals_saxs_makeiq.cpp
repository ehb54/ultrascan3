...
bool US_Hydrodyn_Mals_Saxs::create_i_of_q_ng(set<QString> &fileset,
...
bool US_Hydrodyn_Mals_Saxs::create_i_of_q_ng(QStringList files, double t_min,
                                             double t_max) {
...
bool US_Hydrodyn_Mals_Saxs::create_i_of_q(set<QString> &fileset, double t_min,
                                          double t_max) {
  QStringList files;
  for (set<QString>::iterator it = fileset.begin(); it != fileset.end(); ++it) {
    files << *it;
...
bool US_Hydrodyn_Mals_Saxs::create_i_of_q(QStringList files, double t_min,
                                          double t_max) {
  // us_qdebug( "ciq 0" );
  // for each selected file
  // extract q grid from file names
...

  {
    double cutoff_frac =
        ((US_Hydrodyn *)us_hydrodyn)
                    ->gparams["mals_saxs_cb_makeiq_cutmax_pct"] == "true"
            ? ((US_Hydrodyn *)us_hydrodyn)
...

  {
    avg_peaks = ((US_Hydrodyn *)us_hydrodyn)
                    ->gparams["mals_saxs_cb_makeiq_avg_peaks"] == "true";
...
bool US_Hydrodyn_Mals_Saxs::create_unified_ggaussian_target(
...
bool US_Hydrodyn_Mals_Saxs::compute_f_gaussians(QString file,
...
bool US_Hydrodyn_Mals_Saxs::initial_ggaussian_fit(QStringList &files,
...
void US_Hydrodyn_Mals_Saxs::add_plot_gaussian(const QString &file,
                                              const QString &tag) {
  add_plot(QString("%1-%2").arg(file).arg(tag), f_qs[file],
           compute_gaussian_sum(f_qs[file], f_gaussians[file]), true, false);
}

bool US_Hydrodyn_Mals_Saxs::compute_f_gaussians_trial(
...
void US_Hydrodyn_Mals_Saxs::add_smoothed(const QString &name,
...
void US_Hydrodyn_Mals_Saxs::add_oldstyle(const QString &name,
                                         const vector<double> &q,
                                         const vector<double> &I) {
...
bool US_Hydrodyn_Mals_Saxs::create_istar_q(QStringList files, double t_min,
                                           double t_max) {
...
bool US_Hydrodyn_Mals_Saxs::create_istar_q_ng(QStringList files, double t_min,
                                              double t_max) {
  // if ( lbl_conc_file->text().isEmpty() ) {
  QMessageBox::information(this, windowTitle() + us_tr(": Make I*(q)"),
                           us_tr("I*(q) will be made without Gaussians"));
  ...
  for (unsigned int t = 0; t < tv.size(); t++) {
    ...
    double conc_factor = make_ihashq ? 1 : 1 / (mals_saxs_param_g_conc * 1e-3);
