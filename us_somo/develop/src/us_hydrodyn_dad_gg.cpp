...
bool US_Hydrodyn_Dad::unified_ggaussian_to_f_gaussians() {
  ...
  */
  ...
  if (common_size + per_file_size != (unsigned int)gaussian_type_size) {
    errormsg =
        QString(
            us_tr(
                "Internal error: unified_ggaussians_to_f_gaussians(): "
                "common_size %1 + per_file_size %2 != gaussian_type_size %3\n"))
            .arg(common_size)
            .arg(per_file_size)
    ...
    for (unsigned int j = 0; j < unified_ggaussian_gaussians_size; j++) {
      for (int k = 0; k < gaussian_type_size; ++k) {
        if (is_common[k]) {
          ...
          g.push_back(
              unified_ggaussian_params[offset[k] + per_file_size * j + index]);
        }
      }
    }
...
bool US_Hydrodyn_Dad::gg_fit_vector(vector<double> &fit, vector<double> &pfit) {
  if (!unified_ggaussian_ok) {
    editor_msg("red", us_tr("Internal error: gg_chi_fit_vector() called but "
  ...
  double alpha = ((US_Hydrodyn *)us_hydrodyn)->gparams.count("dad_cormap_alpha")
  ...
  for (int i = 0; i < (int)unified_ggaussian_curves; ++i) {
    ...
    // compute cormap p value
    {
      ...
      // us_qdebug( QString( "p fit for %1 = %2" ).arg( unified_ggaussian_files[
      // i ] ).arg( pfit.back() ) );
    }
  ...
  if (cb_ggq_plot_chi2->isChecked()) {
    ...
    ggqfit_plot->setAxisTitle(QwtPlot::yLeft,
                              us_tr(use_errors ? "normalized Chi^2" : "RMSD"));

    {
      ...
      ggqfit_plot->setCurveData(curve, (double *)&(unified_ggaussian_qvals[0]),
                                (double *)&(fit[0]),
    ...
    // as markers
    {
      ...
      for (int i = 0; i < (int)unified_ggaussian_files.size(); ++i) {
        QColor qc =
            plot_colors[f_pos[unified_ggaussian_files[i]] % plot_colors.size()];
        sym.setPen(qc);
        sym.setBrush(qc);
  ...
  if (cb_ggq_plot_P->isChecked()) {
...

    {
...

      {
        ...
        ggqfit_plot->setCurveStyle(curve, QwtCurve::Lines);
        ggqfit_plot->setCurveData(curve, (double *)&(x[0]), (double *)&(y[0]),
                                  2);
        ggqfit_plot->setCurvePen(curve, use_pen);
#endif
      }
...
#else
        long marker =
            ggqfit_plot->insertMarker("", QwtPlot::xBottom, QwtPlot::yRight);
        ggqfit_plot->setMarkerSymbol(marker, sym);
  ...
  ggqfit_plot->setAxisScale(QwtPlot::xBottom, 0.95 * unified_ggaussian_qvals[0],
...
vector<double> US_Hydrodyn_Dad::compute_ggaussian_gaussian_sum() {
  vector<double> result;
  if (!unified_ggaussian_ok) {
    editor_msg("red", us_tr("Internal error: gaussian rmsd called but unified "
                            "gaussians not ok"));
    return result;
  }
...
double US_Hydrodyn_Dad::ggaussian_rmsd(bool norm_chi) {
  if (!unified_ggaussian_ok) {
    editor_msg("red", us_tr("Internal error: gaussian rmsd called but unified "
                            "gaussians not ok"));
    return 1e99;
...
bool US_Hydrodyn_Dad::gg_fit_replot() {
  if (!unified_ggaussian_ok) {
    editor_msg("red", us_tr("Internal error (gg_fit_replot): Global Gaussian "
...
bool US_Hydrodyn_Dad::data_point_window(vector<double> &q, vector<double> &I) {
  ...
  if (!f_qs.count(wheel_file) || !f_Is.count(wheel_file)) {
    editor_msg(
        "red",
        us_tr("Internal error: data_point_window() wheel file has not points"));
    return false;
  }
  ...
  if (t_q_pts <= pts) {
    editor_msg("red", us_tr("Internal error: data_point_window() wheel file "
                            "has too few points"));
    return false;
  }
  ...
  tso << QString().sprintf(
      ...
      "start_pos    %d\n"
      "q[start_pos] %g\n",
      center, this_pos, (*t_q)[this_pos], start_pos, (*t_q)[start_pos]);
...
void US_Hydrodyn_Dad::gauss_local_caruanas() {
...
void US_Hydrodyn_Dad::gauss_local_guos() {
...
bool US_Hydrodyn_Dad::pvalue(const vector<double> &q, vector<double> &I,
...
bool US_Hydrodyn_Dad::pvalue(const QString &file, double &P,
                             QString &errormsg) {
  errormsg = "";

  if (!f_Is.count(file) || !f_qs.count(file)) {
    errormsg =
        QString("Internal error: pvalue() %1 is missing from data").arg(file);
    return false;
  }

  if (!f_gaussians.count(file)) {
    errormsg =
        QString("Internal error: pvalue() %1 is missing gaussians").arg(file);
    return false;
  }
  ...
  if (qsize != Isize) {
    errormsg =
        QString("Internal error: pvalue() %1 differing I & q sizes").arg(file);
    return false;
  }
  ...
  if (!q.size()) {
    errormsg =
        QString("Error: pvalue() %1 empty q vector after range trim").arg(file);
    return false;
  }
