...
void US_Hydrodyn_Mals_Saxs::setupGUI() {
  ...
  */
  ...
  lbl_scale_pair_fit_curve->setFont(QFont(USglobal->config_list.fontFamily,
                                          USglobal->config_list.fontSize - 1));
  ...
  cb_scale_pair_scale_saxs->setFont(QFont(USglobal->config_list.fontFamily,
                                          USglobal->config_list.fontSize - 2));
  ...
  cb_scale_pair_fit_curve->setFont(QFont(USglobal->config_list.fontFamily,
                                         USglobal->config_list.fontSize - 1));
  ...
  connect(cb_scale_pair_fit_alg_weight, SIGNAL(currentIndexChanged(QString)),
  ...
  lbl_scale_pair_sd_scale->setFont(QFont(USglobal->config_list.fontFamily,
                                         USglobal->config_list.fontSize - 1));
  ...
  lbl_scale_pair_q1_range->setFont(QFont(USglobal->config_list.fontFamily,
                                         USglobal->config_list.fontSize - 1));
  ...
  lbl_scale_pair_q2_range->setFont(QFont(USglobal->config_list.fontFamily,
                                         USglobal->config_list.fontSize - 1));
  ...
  pb_save_created_csv->setText(
      ((US_Hydrodyn *)us_hydrodyn)->gparams.count("mals_saxs_csv_transposed") &&
              ((US_Hydrodyn *)us_hydrodyn)
                      ->gparams["mals_saxs_csv_transposed"] == "true"
          ? us_tr("Save CSV Tr")
          : us_tr(" Save CSV "));
  ...
  // mals_saxs
  pb_mals_saxs_angles_save = new QPushButton(us_tr("Save Angles as CSV"), this);
  pb_mals_saxs_angles_save->setFont(QFont(USglobal->config_list.fontFamily,
                                          USglobal->config_list.fontSize - 1));
  ...
  lbl_mals_saxs_angles_data->setFont(QFont(USglobal->config_list.fontFamily,
                                           USglobal->config_list.fontSize - 1));
  lbl_mals_saxs_angles_data->setTextInteractionFlags(Qt::TextSelectableByMouse);
...
void US_Hydrodyn_Mals_Saxs::mode_setup_widgets() {
  // pb_row_widgets;
  {
...
void US_Hydrodyn_Mals_Saxs::mode_select() {
  ...
  switch (current_mode) {
    ...
    case MODE_GGAUSSIAN: {
      ...
      // only_widgets note: row 0 currently is blanks analysis, row 1 gauss mode
      // buttons+scale+testiq could add more rows...
...
void US_Hydrodyn_Mals_Saxs::mode_select(modes mode) {
  current_mode = mode;
  mode_select();
}

void US_Hydrodyn_Mals_Saxs::mode_title(QString title) {
...
void US_Hydrodyn_Mals_Saxs::update_enables() {
  ...
  bool all_t_ =
      files_selected_count &&
      files_selected_count == (unsigned int)selected_files.filter("_t").size();
  bool half_common =
      files_selected_count &&
      files_selected_count ==
  ...
  baseline_integral =
      ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_bl_integral"] == "true";
  ...
  pb_mals_saxs_angles_save->setEnabled(mals_saxs_angles.mals_saxs_angle.size());
...
void US_Hydrodyn_Mals_Saxs::model_enables() {
...
void US_Hydrodyn_Mals_Saxs::disable_all() {
  // cout << "disable all\n";
  // qwtw_wheel            ->setEnabled( false );
  pb_wheel_inc->setEnabled(false);
  pb_wheel_dec->setEnabled(false);
...
void US_Hydrodyn_Mals_Saxs::model_save() {
  disable_all();
  model_save(MQT::get_lb_qsl(lb_model_files, true));
  update_enables();
}

void US_Hydrodyn_Mals_Saxs::model_text() {
  disable_all();
  model_text(MQT::get_lb_qsl(lb_model_files, true));
  update_enables();
}

void US_Hydrodyn_Mals_Saxs::model_view() {
  disable_all();
  model_view(MQT::get_lb_qsl(lb_model_files, true));
  update_enables();
}

void US_Hydrodyn_Mals_Saxs::model_remove() {
  disable_all();
  model_remove(MQT::get_lb_qsl(lb_model_files, true));
  update_enables();
}

void US_Hydrodyn_Mals_Saxs::model_remove(QStringList files) {
  disable_updates = true;
...
void US_Hydrodyn_Mals_Saxs::model_view(QStringList files) {
...
void US_Hydrodyn_Mals_Saxs::model_text(QStringList files) {
  QString tmpd = USglobal->config_list.root_dir + "/somo/saxs/tmp";
  QDir dir(tmpd);
  if (!dir.exists()) {
    if (dir.mkdir(tmpd)) {
...
bool US_Hydrodyn_Mals_Saxs::model_save(QStringList files) {
  bool errors = false;
...
bool US_Hydrodyn_Mals_Saxs::model_save(QString file, bool &cancel,
                                       bool &overwrite_all) {
...
void US_Hydrodyn_Mals_Saxs::usp_config_plot_dist(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_dist);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Saxs::usp_config_plot_ref(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_ref);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Saxs::usp_config_plot_errors(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_errors);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Saxs::usp_config_ggqfit_plot(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_ggqfit_plot);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Saxs::usp_config_guinier_plot(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_guinier_plot);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Saxs::usp_config_guinier_plot_rg(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_guinier_plot_rg);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Saxs::usp_config_guinier_plot_mw(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_guinier_plot_mw);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Saxs::usp_config_guinier_plot_summary(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_guinier_plot_summary);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Saxs::usp_config_guinier_plot_errors(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_guinier_plot_errors);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Mals_Saxs::set_pbmode_main() { pbmode_select(PBMODE_MAIN); }

void US_Hydrodyn_Mals_Saxs::set_pbmode_sel() { pbmode_select(PBMODE_SEL); }

void US_Hydrodyn_Mals_Saxs::set_pbmode_crop() { pbmode_select(PBMODE_CROP); }

void US_Hydrodyn_Mals_Saxs::set_pbmode_conc() { pbmode_select(PBMODE_CONC); }

void US_Hydrodyn_Mals_Saxs::set_pbmode_sd() { pbmode_select(PBMODE_SD); }

void US_Hydrodyn_Mals_Saxs::set_pbmode_fasta() { pbmode_select(PBMODE_FASTA); }

void US_Hydrodyn_Mals_Saxs::set_pbmode_mals_saxs() {
...
void US_Hydrodyn_Mals_Saxs::set_pbmode_none() { pbmode_select(PBMODE_NONE); }

void US_Hydrodyn_Mals_Saxs::pbmode_select(pbmodes mode) {
...
void US_Hydrodyn_Mals_Saxs::fasta_file() {
  // load file
