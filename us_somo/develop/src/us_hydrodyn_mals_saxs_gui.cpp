...
void US_Hydrodyn_Mals_Saxs::setupGUI()
{
   ...
   */
   ...
   pb_common_time->setFont(QFont( USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1 ));
   ...
   //    pb_set_mals_saxs->setFont(QFont( USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1));
   ...
   //    lbl_mals_saxs->setFont(QFont( USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1));
   ...
   pb_save_created_csv->setText( (( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_csv_transposed" ) &&
...
//    ggqfit_plot->setMargin(USglobal->config_list.margin);
   ...
   cb_guinier_qrgmax->setChecked( (( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_cb_guinier_qrgmax" ) &&
   ...
   le_guinier_qrgmax->setText( (( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_guinier_qrgmax" ) ?
...
//   guinier_plot_summary = new QwtPlot( 0 );
   ...
   pb_mals_saxs_angles_save->setFont(QFont( USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1 ));
   pb_mals_saxs_angles_save->setMinimumHeight(minHeight1);
   pb_mals_saxs_angles_save->setPalette( PALET_PUSHB );
   connect(pb_mals_saxs_angles_save, SIGNAL(clicked()), SLOT(mals_saxs_angles_save()));
   ...
   lbl_mals_saxs_angles_data->setFont(QFont( USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1));
   ...
   rb_pbmode_mals_saxs->setFont(QFont( USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1 ));
   ...
   //    QBoxLayout * hbl_mals_saxs = new QHBoxLayout(); hbl_mals_saxs->setContentsMargins( 0, 0, 0, 0 ); hbl_mals_saxs->setSpacing( 0 );
   //    hbl_mals_saxs->addWidget ( pb_set_mals_saxs );
   //    hbl_mals_saxs->addWidget ( lbl_mals_saxs );
   ...
   QGridLayout * gl_files = new QGridLayout( 0 ); gl_files->setContentsMargins( 0, 0, 0, 0 ); gl_files->setSpacing( 0 );
   {
      ...
      gl_files->addLayout( hbl_file_buttons_3a , j, 0 ); j++;
...
//    hbl_files_plot->addLayout( vbl_plot_group );
   ...
   //                 !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_files_widgets" ) || ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_files_widgets" ] == "false" ? false : true );
   //   hide_widgets( editor_widgets, 
   //                 !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_editor_widgets" ) || ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_editor_widgets" ] == "false" ? false : true );
   //   hide_widgets( created_files_widgets,
   //                 !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_created_files_widgets" ) || ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_creaded_files_widgets" ] == "false" ? false : true );
...
void US_Hydrodyn_Mals_Saxs::mode_select()
{
   ...
   case MODE_SCROLL_PAIR  : mode_title( us_tr( "Scroll Pairs" ) );   ShowHide::hide_widgets( scroll_pair_widgets, always_hide_widgets, false ); ShowHide::only_widgets( pb_row_widgets, 1, always_hide_widgets );break;
   case MODE_SCALE_PAIR   : mode_title( pb_scale_pair->text() );     ShowHide::hide_widgets( scale_pair_widgets, always_hide_widgets , false ); ShowHide::only_widgets( pb_row_widgets, 9, always_hide_widgets );break;
...
void US_Hydrodyn_Mals_Saxs::update_enables()
{
   ...
   pb_create_ihash_t     ->setEnabled( files_selected_count > 1 && files_compatible && files_are_time && mals_saxs_param_n && mals_saxs_param_lambda && mals_saxs_param_g_dndc && selected_files.count() == selected_files.filter( "_Rt_" ).count() );
   ...
   //                                        lb_files->item( last_selected_pos )->text() != lbl_mals_saxs->text() &&
...
bool US_Hydrodyn_Mals_Saxs::model_save( QString file, bool & cancel, bool & overwrite_all )
{
...
void US_Hydrodyn_Mals_Saxs::set_pbmode_main() {
   pbmode_select( PBMODE_MAIN );
}

void US_Hydrodyn_Mals_Saxs::set_pbmode_sel() {
   pbmode_select( PBMODE_SEL );
}

void US_Hydrodyn_Mals_Saxs::set_pbmode_crop() {
   pbmode_select( PBMODE_CROP );
}

void US_Hydrodyn_Mals_Saxs::set_pbmode_conc() {
   pbmode_select( PBMODE_CONC );
}

void US_Hydrodyn_Mals_Saxs::set_pbmode_sd() {
   pbmode_select( PBMODE_SD );
}

void US_Hydrodyn_Mals_Saxs::set_pbmode_fasta() {
...
void US_Hydrodyn_Mals_Saxs::pbmode_select( pbmodes mode ) {
   ...
   case PBMODE_MALS_SAXS :
      ShowHide::hide_widgets( pbmode_mals_saxs_widgets, always_hide_widgets, false );
      break;
