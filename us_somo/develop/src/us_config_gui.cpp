...
US_Config_GUI::US_Config_GUI (QWidget *parent, const char *) : QFrame (parent)
{
  ...
  lbl_directions = new QLabel (
      us_tr ("UltraScan " + US_Version + " Configuration:"), this);
  ...
  lbl_directions->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  AUTFBACK (lbl_paths);
  lbl_paths->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  pb_browser->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize - 1));
  US_Config::connect (pb_browser, SIGNAL (clicked ()), this,
                      SLOT (open_browser_dir ()));
  ...
  US_Config::connect (le_browser, SIGNAL (textChanged (const QString &)), this,
                      SLOT (update_browser (const QString &)));

  pb_tmp_dir = new QPushButton (us_tr (" Temporary Directory:"), this);
  pb_tmp_dir->setPalette (PALET_PUSHB);
  pb_tmp_dir->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize - 1));
  US_Config::connect (pb_tmp_dir, SIGNAL (clicked ()), this,
                      SLOT (open_tmp_dir ()));

  le_tmp_dir = new QLineEdit (this);
  le_tmp_dir->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize - 1));
  le_tmp_dir->setPalette (PALET_EDIT);
  AUTFBACK (le_tmp_dir);
  US_Config::connect (le_tmp_dir, SIGNAL (textChanged (const QString &)), this,
                      SLOT (update_tmp_dir (const QString &)));

  pb_root_dir = new QPushButton (us_tr (" User's UltraScan Directory:"), this);
  pb_root_dir->setPalette (PALET_PUSHB);
  pb_root_dir->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  US_Config::connect (pb_root_dir, SIGNAL (clicked ()), this,
                      SLOT (open_root_dir ()));

  le_root_dir = new QLineEdit (this);
  le_root_dir->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  le_root_dir->setPalette (PALET_EDIT);
  AUTFBACK (le_root_dir);
  US_Config::connect (le_root_dir, SIGNAL (textChanged (const QString &)),
                      this, SLOT (update_root_dir (const QString &)));

  pb_data_dir = new QPushButton (us_tr (" Data Directory:"), this);
  pb_data_dir->setPalette (PALET_PUSHB);
  pb_data_dir->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  US_Config::connect (pb_data_dir, SIGNAL (clicked ()), this,
                      SLOT (open_data_dir ()));

  le_data_dir = new QLineEdit (this);
  le_data_dir->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  le_data_dir->setPalette (PALET_EDIT);
  AUTFBACK (le_data_dir);
  US_Config::connect (le_data_dir, SIGNAL (textChanged (const QString &)),
                      this, SLOT (update_data_dir (const QString &)));

  pb_result_dir = new QPushButton (us_tr (" Result Directory:"), this);
  pb_result_dir->setPalette (PALET_PUSHB);
  pb_result_dir->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  US_Config::connect (pb_result_dir, SIGNAL (clicked ()), this,
                      SLOT (open_result_dir ()));

  le_result_dir = new QLineEdit (this);
  le_result_dir->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  le_result_dir->setPalette (PALET_EDIT);
  AUTFBACK (le_result_dir);
  US_Config::connect (le_result_dir, SIGNAL (textChanged (const QString &)),
                      this, SLOT (update_result_dir (const QString &)));

  pb_html_dir = new QPushButton (us_tr (" HTML Reports:"), this);
  pb_html_dir->setPalette (PALET_PUSHB);
  pb_html_dir->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  US_Config::connect (pb_html_dir, SIGNAL (clicked ()), this,
                      SLOT (open_html_dir ()));

  le_html_dir = new QLineEdit (this);
  le_html_dir->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  le_html_dir->setPalette (PALET_EDIT);
  AUTFBACK (le_html_dir);
  US_Config::connect (le_html_dir, SIGNAL (textChanged (const QString &)),
                      this, SLOT (update_html_dir (const QString &)));

  pb_archive_dir = new QPushButton (us_tr (" Archive Directory:"), this);
  pb_archive_dir->setPalette (PALET_PUSHB);
  pb_archive_dir->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize - 1));
  US_Config::connect (pb_archive_dir, SIGNAL (clicked ()), this,
                      SLOT (open_archive_dir ()));

  le_archive_dir = new QLineEdit (this);
  le_archive_dir->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize - 1));
  le_archive_dir->setPalette (PALET_EDIT);
  AUTFBACK (le_archive_dir);
  US_Config::connect (le_archive_dir, SIGNAL (textChanged (const QString &)),
                      this, SLOT (update_archive_dir (const QString &)));

  pb_system_dir = new QPushButton (us_tr (" UltraScan System:"), this);
  pb_system_dir->setPalette (PALET_PUSHB);
  pb_system_dir->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  US_Config::connect (pb_system_dir, SIGNAL (clicked ()), this,
                      SLOT (open_system_dir ()));

  le_system_dir = new QLineEdit (this);
  le_system_dir->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  le_system_dir->setPalette (PALET_EDIT);
  AUTFBACK (le_system_dir);
  US_Config::connect (le_system_dir, SIGNAL (textChanged (const QString &)),
                      this, SLOT (update_system_dir (const QString &)));

  pb_help_dir = new QPushButton (us_tr (" Help Directory:"), this);
  pb_help_dir->setPalette (PALET_PUSHB);
  pb_help_dir->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  US_Config::connect (pb_help_dir, SIGNAL (clicked ()), this,
                      SLOT (open_help_dir ()));

  le_help_dir = new QLineEdit (this);
  le_help_dir->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  le_help_dir->setPalette (PALET_EDIT);
  AUTFBACK (le_help_dir);
  US_Config::connect (le_help_dir, SIGNAL (textChanged (const QString &)),
                      this, SLOT (update_help_dir (const QString &)));
  ...
  AUTFBACK (lbl_misc);
  lbl_misc->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize - 1, QFont::Bold));

  lbl_temperature_tol
      = new QLabel (us_tr (" Temperature Tolerance (ºC):"), this);
  ...
  lbl_temperature_tol->setFont (QFont (USglobal->config_list.fontFamily,
                                       USglobal->config_list.fontSize - 1,
                                       QFont::Bold));

  le_temperature_tol = new QLineEdit (this);
  le_temperature_tol->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize - 1));
  le_temperature_tol->setPalette (PALET_EDIT);
  AUTFBACK (le_temperature_tol);
  US_Config::connect (le_temperature_tol,
                      SIGNAL (textChanged (const QString &)), this,
                      SLOT (update_temperature_tol (const QString &)));
  ...
  lbl_beckman_bug->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize - 1,
                                   QFont::Bold));

  bt_on = new QRadioButton (this);
  bt_on->setText (us_tr ("on"));
  bt_on->setFont (QFont (USglobal->config_list.fontFamily,
                         USglobal->config_list.fontSize - 1, QFont::Bold));
  bt_on->setPalette (PALET_NORMAL);
  AUTFBACK (bt_on);
  US_Config::connect (bt_on, SIGNAL (clicked ()), this,
                      SLOT (update_on_button ()));

  bt_off = new QRadioButton (this);
  bt_off->setText (us_tr ("off"));
  bt_off->setFont (QFont (USglobal->config_list.fontFamily,
                          USglobal->config_list.fontSize - 1, QFont::Bold));
  bt_off->setPalette (PALET_NORMAL);
  AUTFBACK (bt_off);
  US_Config::connect (bt_off, SIGNAL (clicked ()), this,
                      SLOT (update_off_button ()));
  ...
  AUTFBACK (lbl_color);
  lbl_color->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize - 1, QFont::Bold));

  pb_color = new QPushButton (us_tr ("Change"), this);
  Q_CHECK_PTR (pb_color);
  pb_color->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize));
  pb_color->setPalette (PALET_PUSHB);
  US_Config::connect (pb_color, SIGNAL (clicked ()), this,
                      SLOT (update_color ()));
  ...
  AUTFBACK (lbl_font);
  lbl_font->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize - 1, QFont::Bold));

  pb_font = new QPushButton (us_tr ("Change"), this);
  Q_CHECK_PTR (pb_font);
  pb_font->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize));
  pb_font->setPalette (PALET_PUSHB);
  US_Config::connect (pb_font, SIGNAL (clicked ()), this,
                      SLOT (update_font ()));
  ...
  lbl_database->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize - 1,
                                QFont::Bold));

  pb_database = new QPushButton (us_tr ("Change"), this);
  pb_database->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  pb_database->setPalette (PALET_PUSHB);
  US_Config::connect (pb_database, SIGNAL (clicked ()), this,
  ...
  lbl_numThreads->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize - 1,
                                  QFont::Bold));

  cnt_numThreads = new QwtCounter (this);
  US_Hydrodyn::sizeArrows (cnt_numThreads);
  cnt_numThreads->setRange (1, MAX_THREADS);
  cnt_numThreads->setSingleStep (1);
  ...
  connect (cnt_numThreads, SIGNAL (valueChanged (double)),
           SLOT (update_numThreads (double)));
...
US_Config_GUI::setup_GUI ()
{
  ...
  lineGrid->setContentsMargins (0, 0, 0, 0);
  lineGrid->setSpacing (0);
  topbox->addLayout (lineGrid);
  lineGrid->setSpacing (2);
  lineGrid->setContentsMargins (2, 2, 2, 2);
  ...
  QGridLayout *lineGrid2 = new QGridLayout;
  lineGrid2->setContentsMargins (0, 0, 0, 0);
  lineGrid2->setSpacing (0);
  topbox->addLayout (lineGrid2);
  ;
  j = 0;
  ...
  QBoxLayout *pushbutton = new QHBoxLayout;
  pushbutton->setContentsMargins (0, 0, 0, 0);
  pushbutton->setSpacing (0);
  topbox->addLayout (pushbutton);
  pushbutton->setSpacing (2);
...
void
US_Config_GUI::closeEvent (QCloseEvent *e)
{
  e->accept ();
  global_Xpos -= 30;
  global_Ypos -= 30;
...
void
US_Config_GUI::update_on_button ()
{
  bt_off->setChecked (false);
...
void
US_Config_GUI::update_color ()
{
  US3i_Color *uscol;
  uscol = new US3i_Color ();
  US_Config::connect (uscol, SIGNAL (marginChanged (int)), this,
                      SLOT (update_margin (int)));
  uscol->show ();
}

void
US_Config_GUI::update_margin (int val)
{
  USglobal->config_list.margin = val;
}

void
US_Config_GUI::update_font ()
{
  US_Font *usfont;
  QString temp_font = USglobal->config_list.fontFamily;
  int temp_size = USglobal->config_list.fontSize;

  usfont = new US_Font (&USglobal->config_list.fontFamily,
                        &USglobal->config_list.fontSize);
...
void
US_Config_GUI::update_off_button ()
{
  bt_on->setChecked (false);
...
void
US_Config_GUI::open_browser_dir ()
{
  QString browser = QFileDialog::getOpenFileName (
      0, windowTitle (), USglobal->config_list.browser, QString ());
...
void
US_Config_GUI::update_browser (const QString &newText)
{
  if (newText != "")
...
void
US_Config_GUI::open_system_dir ()
{
  QString system = QFileDialog::getExistingDirectory (
      0, QString (), USglobal->config_list.system_dir,
...
void
US_Config_GUI::update_system_dir (const QString &newText)
{
  if (newText != "")
...
void
US_Config_GUI::open_help_dir ()
{
  QString help = QFileDialog::getExistingDirectory (
      0, QString (), USglobal->config_list.help_dir,
      QFileDialog::ShowDirsOnly);
...
void
US_Config_GUI::update_help_dir (const QString &newText)
{
  if (newText != "")
...
void
US_Config_GUI::update_temperature_tol (const QString &newText)
{
  if (newText != "")
...
void
US_Config_GUI::open_result_dir ()
{
  QString result = QFileDialog::getExistingDirectory (
      0, QString (), USglobal->config_list.result_dir,
      QFileDialog::ShowDirsOnly);
...
void
US_Config_GUI::update_result_dir (const QString &newText)
{
  USglobal->config_list.result_dir = newText;
}

void
US_Config_GUI::open_html_dir ()
{
  QString html = QFileDialog::getExistingDirectory (
      0, QString (), USglobal->config_list.html_dir,
      QFileDialog::ShowDirsOnly);
...
void
US_Config_GUI::update_html_dir (const QString &newText)
{
  if (newText != "")
...
void
US_Config_GUI::open_tmp_dir ()
{
  QString tmp = QFileDialog::getExistingDirectory (
      0, QString (), USglobal->config_list.tmp_dir, QFileDialog::ShowDirsOnly);
...
void
US_Config_GUI::update_tmp_dir (const QString &newText)
{
  if (newText != "")
...
void
US_Config_GUI::open_data_dir ()
{
  QString data = QFileDialog::getExistingDirectory (
      0, QString (), USglobal->config_list.data_dir,
      QFileDialog::ShowDirsOnly);
...
void
US_Config_GUI::update_data_dir (const QString &newText)
{
  if (newText != "")
...
void
US_Config_GUI::open_archive_dir ()
{
  QString archive = QFileDialog::getExistingDirectory (
      0, QString (), USglobal->config_list.archive_dir,
      QFileDialog::ShowDirsOnly);
...
void
US_Config_GUI::update_archive_dir (const QString &newText)
{
  if (newText != "")
...
void
US_Config_GUI::open_root_dir ()
{
  QString root = QFileDialog::getExistingDirectory (
      0, QString (), USglobal->config_list.root_dir,
      QFileDialog::ShowDirsOnly);
...
void
US_Config_GUI::update_root_dir (const QString &newText)
{
  if (newText != "")
...
void
US_Config_GUI::update_numThreads (double val)
{
