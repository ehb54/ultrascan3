...
US_Hydrodyn_BD_Load_Results_Opts::US_Hydrodyn_BD_Load_Results_Opts (
    ...
    double browflex_solvent_viscosity, bool *check_fix_overlaps, QWidget *p,
    const char *)
    : QDialog (p)
{
  this->msg = msg;
...
void
US_Hydrodyn_BD_Load_Results_Opts::setupGUI ()
{
  int minHeight1 = 30;
  int minHeight2 = 30;
  ...
  AUTFBACK (lbl_param);
  lbl_param->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize + 1, QFont::Bold));
  ...
  AUTFBACK (lbl_to_use);
  lbl_to_use->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize + 1,
                              QFont::Bold));
  ...
  le_solvent_name = new QLineEdit (this);
  le_solvent_name->setObjectName ("Solvent_Name Line Edit");
  ...
  cb_set_to_browflex->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize));
  cb_set_to_browflex->setPalette (PALET_NORMAL);
  AUTFBACK (cb_set_to_browflex);
  connect (cb_set_to_browflex, SIGNAL (clicked ()), this,
           SLOT (set_to_browflex ()));
  ...
  cb_check_fix_overlaps->setFont (QFont (USglobal->config_list.fontFamily,
                                         USglobal->config_list.fontSize));
  cb_check_fix_overlaps->setPalette (PALET_NORMAL);
  AUTFBACK (cb_check_fix_overlaps);
  connect (cb_check_fix_overlaps, SIGNAL (clicked ()), this,
           SLOT (set_check_fix_overlaps ()));
  ...
  le_solvent_acronym = new QLineEdit (this);
  le_solvent_acronym->setObjectName ("Solvent_Acronym Line Edit");
  ...
  lbl_temperature = new QLabel (
      us_tr (QString (" Temperature (%1C): ").arg (DEGREE_SYMBOL)), this);
  ...
  lbl_browflex_temperature = new QLabel (
      QString ("").sprintf ("%4.2f", browflex_temperature), this);
  ...
  lbl_browflex_temperature->setFont (QFont (USglobal->config_list.fontFamily,
                                            USglobal->config_list.fontSize));
  ...
  lbl_solvent_viscosity
      = new QLabel (us_tr (" Solvent viscosity (cP):"), this);
  ...
  lbl_browflex_solvent_viscosity = new QLabel (
      QString ("").sprintf ("%f", browflex_solvent_viscosity), this);
  lbl_browflex_solvent_viscosity->setAlignment (Qt::AlignCenter
                                                | Qt::AlignVCenter);
  lbl_browflex_solvent_viscosity->setPalette (PALET_NORMAL);
  AUTFBACK (lbl_browflex_solvent_viscosity);
  lbl_browflex_solvent_viscosity->setFont (QFont (
      USglobal->config_list.fontFamily, USglobal->config_list.fontSize));

  le_solvent_viscosity = new QLineEdit (this);
  le_solvent_viscosity->setObjectName ("Solvent_Viscosity Line Edit");
  ...
  le_solvent_density = new QLineEdit (this);
  le_solvent_density->setObjectName ("Solvent_Density Line Edit");
...
void
US_Hydrodyn_BD_Load_Results_Opts::cancel ()
{
  close ();
}

void
US_Hydrodyn_BD_Load_Results_Opts::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_BD_Load_Results_Opts::update_solvent_name (const QString &str)
{
  *solvent_name = str;
  check_solvent_browflex_defaults ();
}
...
void
US_Hydrodyn_BD_Load_Results_Opts::update_solvent_acronym (const QString &str)
{
...
void
US_Hydrodyn_BD_Load_Results_Opts::update_temperature (const QString &str)
{
...
void
US_Hydrodyn_BD_Load_Results_Opts::update_solvent_viscosity (const QString &str)
{
...
void
US_Hydrodyn_BD_Load_Results_Opts::update_solvent_density (const QString &str)
{
...
void
US_Hydrodyn_BD_Load_Results_Opts::update_psv (const QString &str)
{
  *psv = str.toDouble ();
}

void
US_Hydrodyn_BD_Load_Results_Opts::set_solvent_defaults ()
{
  if (cb_solvent_defaults->isChecked ())
    {
      ...
      *solvent_density = DENS_20W;
      ...
      le_solvent_density->setText (
          QString ("").sprintf ("%f", *solvent_density));
    }
}

void
US_Hydrodyn_BD_Load_Results_Opts::set_check_fix_overlaps ()
{
  *check_fix_overlaps = cb_check_fix_overlaps->isChecked ();
}

void
US_Hydrodyn_BD_Load_Results_Opts::set_to_browflex ()
{
...
void
US_Hydrodyn_BD_Load_Results_Opts::check_solvent_browflex_defaults ()
{
  cb_solvent_defaults->setChecked (
      *solvent_name == "Water" && *solvent_acronym == "w"
      && *temperature == K20 - K0 && *solvent_viscosity == VISC_20W * 100
      && *solvent_density == DENS_20W);
...
int
US_Hydrodyn_BD_Load_Results_Opts::write_conditions_to_file (
    QString /* filename */)
{
  return -1;
}
...
void
US_Hydrodyn_BD_Load_Results_Opts::read_conditions_from_file (
    QString /* filename */)
{
}
