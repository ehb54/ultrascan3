...
static QStringList pr_csv_pad_holes(const QStringList &qsl) {
  ...
  if (qsl_r.size() < 6) {
    TSO << "pr_csv_pad_holes() does not appear to contain any r values in the "
           "header rows.\n";
    return qsl;
  }
    ...
    for (int i = 0; i < (int)qsl_data_new.size(); ++i) {
      if (qsl_data_new[i].size() > 4 &&
          ...
          if (qsl_data_new[i][j].toDouble() > 0 &&
              ((qsl_data_new[i][3] == "\"P(r) sd\"" &&
               ...
               (qsl_data_new[i][3] == "\"P(r) normed sd\"" &&
                ...
                qsl_data_new[name2pr_normed[qsl_data_new[i][0]]][j].toDouble() >
                    0))) {
        ...
        if (!sd_gz.size()) {
          // no nonneg sds! punt
          qDebug()
              << QString("pr_csv_pad_holes() : curve %1 %2 no nonneg sds! punt")
...
void US_Hydrodyn_Saxs::load_iqq_csv(QString filename,
                                    bool just_plotted_curves) {
  vector<QString>
  ...
  if (qsl_headers.size() > 1) {
    ...
    for (unsigned int i = 1; i < (unsigned int)qsl_headers.size(); i++) {
      if (ref != qsl_headers[i]) {
        // QMessageBox mb(us_tr("UltraScan Warning"),
        //                us_tr("The csv file ") + filename + us_tr(" contains
        //                multiple different headers\nPlease manually correct
        //                the csv file."), QMessageBox::Critical,
        //                QMessageBox::NoButton, QMessageBox::NoButton,
        //                QMessageBox::NoButton, 0, 0, 1);
        // mb.exec();
        QMessageBox::critical(
  ...
  if (!just_plotted_curves) {
    if (qsl_plotted_iq_names.size()) {
      bool ok;
      grid_target =
          US_Static::getItem(us_tr("Set I(q) Grid"),
                             us_tr("Select the target plotted data set for the "
                                   "data being loaded grid:\n"
                                   "or Cancel if you wish to interpolate the "
                                   "plotted to the data being loaded"),
    ...
    if (qsl_q.size() < 3) {
      // QMessageBox mb(us_tr("UltraScan Warning"),
      //                us_tr("The csv file ") + filename + us_tr(" does not
      //                appear to contain any q values in the header rows.\n"),
      //                QMessageBox::Critical,
      //                QMessageBox::NoButton, QMessageBox::NoButton,
      //                QMessageBox::NoButton, 0, 0, 1);
      // mb.exec();
      QMessageBox::critical(this, us_tr("UltraScan Warning"),
                            us_tr("The csv file ") + filename +
                                us_tr(" does not appear to contain any q "
    ...
    if (!grid_target.isEmpty()) {
      ...
      if (!plotted_iq_names_to_pos.count(grid_target)) {
        editor_msg(
            "red",
            QString(us_tr("Internal error: could not find %1 in plotted data"))
                .arg(grid_target));
        grid_target = "";
      } else {
        original_q = q;
        ...
        if (q[0] < original_q[0] || q.back() > original_q.back()) {
          editor_msg("dark red", QString("Notice: the target curve is cropped "
                                         "to q(%1:%2) to prevent extrapolation")
                                     .arg(original_q[0])
                                     .arg(original_q.back()));

          nnls_csv_footer << QString(
        ...
        msg.replace("\"", "");
        qsl[0] = QString(
                     "\"Name\",\"Type; q:\",%1,\"%2 reinterpolated to q(%3:%4) "
                     "step %5\"")
        ...
        for (unsigned int i = 0; i < (unsigned int)qsl_data.size(); i++) {
          ...
          // interpolate original_q, original_i, original_i_error", original_q,
          // original_i, original_i_error ) << "\n";
          if (!usu.interpolate_iqq_by_case(original_q, original_i,
                                           original_i_error, q, ni, ni_error)) {
            ...
            QMessageBox::warning(
                ...
                QString(us_tr("There was an error attempting to interpolate\n"
                    ...
                    .arg(original_q.size() ? original_q[original_q.size() - 1]
                                           : 0)
                    .arg(q.size() ? q[0] : 0)
                    .arg(q.size() ? q[q.size() - 1] : 0)
                    .arg(usu.errormsg));
  ...
  if (filenames.size() > 1) {
    ...
    for (unsigned int i = 1; i < (unsigned int)filenames.size(); i++) {
      ...
      if (qsl2_headers.size() == 0) {
        // QMessageBox mb(us_tr("UltraScan Warning"),
        //                us_tr("The csv file ") + f2.fileName() + us_tr(" does
        //                not appear to contain a correct header.\nPlease
        //                manually correct the csv file."),
        //                QMessageBox::Critical,
        //                QMessageBox::NoButton, QMessageBox::NoButton,
        //                QMessageBox::NoButton, 0, 0, 1);
      ...
      // QTextStream(stdout) << US_Vector::qs_mapqsqs( "name_to_errors_map",
      // name_to_errors_map ) << "\n";
      ...
      if (q.size() > 1 && q[q.size() - 1] < max_value) {
        ...
        if (q_delta > 0e0) {
          for (double value = q[q.size() - 1] + q_delta; value <= max_value;
               value += q_delta) {
            q.push_back(value);
          }
        }
      ...
      for (QStringList::iterator it = qsl_data.begin(); it != qsl_data.end();
        ...
        if (qsl_iq.size() < 3) {
          QString msg = us_tr("The csv file ") + f2.fileName() +
                        us_tr(
                            " does not appear to contain sufficient I(q) "
                            "values in data row " +
                            qsl_iq[0] + ", skipping\n");
          editor_msg("red", msg);
        } else {
          ...
          if (all_match) {
            ...
            if (qs_tmp.contains(QRegExp("(Average|Standard deviation)"))) {
              qs_tmp.replace(QRegExp("^\""),
                             QString("\"%1: ").arg(QFileInfo(f2).baseName()));
            }
            qsl << qs_tmp;
            // TSO << "yes, all match\nadding:" << qs_tmp << endl;
          } else {
  ...
  if (found_cropping) {
    ...
    if (crop_min > q[0] || crop_max < q[q.size() - 1]) {
      editor_msg(
          "dark red",
          QString(
              "cropping loaded curves to plotted range intersection q(%1:%2)")
              .arg(crop_min)
              .arg(crop_max));
      ...
      for (unsigned int i = 0; i < (unsigned int)qsl_data.size(); i++) {
        ...
        vector<double> ni;
        vector<double> ni_error;
        if (!usu.crop(original_q, original_i, original_i_error, crop_min,
                      crop_max, ni, ni_error)) {
          editor_msg("red", usu.errormsg);
          return;
        }
  ...
  for (unsigned int i = 0; i < (unsigned int)qsl_plotted_iq_names.size(); i++) {
    ...
    if (!usu.interpolate_iqq_by_case(plotted_q[i], plotted_I[i],
                                     plotted_I_error[i], q, nic, nic_errors)) {
      editor_msg("red", usu.errormsg);
      QMessageBox::warning(
          this, "US-SOMO",
          QString(us_tr("There was an error attempting to interpolate\n"
              ...
              .arg(plotted_q[i].size() ? plotted_q[i][plotted_q[i].size() - 1]
                                       : 0)
              .arg(q.size() ? q[0] : 0)
              .arg(q.size() ? q.back() : 0)
              .arg(usu.errormsg));
    ...
    if (!added_interpolate_msg && q.size() > 1) {
      bin_msg = QString(us_tr("Plotted I(q) interpolated to delta q of %1"))
                    .arg(q[1] - q[0]);
  ...
  if (filenames.size() > 1) {
    ...
    if (new_qsl.size() != qsl.size()) {
      switch (QMessageBox::information(
          ...
          us_tr(
              "There are multiple average and/or standard deviation lines\n") +
  ...
  if ((run_nnls || run_best_fit) && !qsl_sel_names.filter(nnls_target).size()) {
  ...
  if (run_nnls || run_best_fit) {
    if (run_nnls) {
      ...
      if (nnls_csv) {
        ...
        // ((US_Hydrodyn *)us_hydrodyn)->select_from_directory_history( use_dir,
        // this );

        nnls_csv_filename = QFileDialog::getSaveFileName(
            this, us_tr("Choose a filename to save the NNLS fit"),
            use_dir + "/" +
                QFileInfo(QString("%1").arg(nnls_target).replace("\"", ""))
                    .baseName() +
                "_NNLS_results.csv",
            "*.csv");
    ...
    if (map_sel_names.count(qsl_tmp[0])) {
      ...
      if (qsl_iq.size() < 3) {
        QString msg = us_tr("The csv file ") + filename +
                      us_tr(
                          " does not appear to contain sufficient I(q) values "
                          "in data row " +
                          qsl_tmp[0] + "\n");
        editor_msg("red", msg);
        // QMessageBox mb(us_tr("UltraScan Warning"),
      ...
      // occasionally one may have a zero in the last p(r) position (?) not for
      // I(q)?
      if (I.size() > q.size()) {
        I.resize(q.size());
      }
      ...
      if (!(create_avg && only_plot_stats) && !run_nnls && !run_best_fit) {
            ...
            if (avg_rescaling_multiplier != 0e0) {
              ...
              if (!rescaling_warning_shown) {
                QMessageBox::warning(
                    this, "UltraScan",
                    QString(us_tr(
                        "Standard deviation curves require an Average curve to "
                        "be plotted first for correct rescaling\n"
                        "Since an Average curve was not plotted first, they "
                        "will simply be scaled as a best fit to the selected "
                        "target\n")));
        ...
        if (I_errors.size()) {
          plot_one_iqq(this_q, I, I_errors,
                       QFileInfo(filename).fileName() + " " + qsl_tmp[0]);
        } else {
    ...
    if (create_std_dev && sum_count > 2) {
      ...
      for (unsigned int i = 0; i < sum_iq.size(); i++) {
        ...
        std_dev[i] = tmp_std_dev > 0e0
                         ? sqrt((1e0 / ((double)sum_count - 1e0)) * tmp_std_dev)
                         : 0e0;
        if (us_isnan(sum_iq[i])) {
        ...
        if (us_isnan(std_dev[i])) {
          TSO << QString(
                     "WARNING when calc'ing std dev: isnan std_dev[%1]:\n"
                     " sum_iq[%2]  == %3\n"
                     " sum_iq2[%4] == %5\n"
                     " sum_count   == %6\n"
      ...
      if (found_nnls_model && found_nnls_target) {
        ...
        if (run_nnls) {
          if (our_saxs_options->iqq_scale_minq ||
            ...
            editor_msg("dark red",
                       QString(us_tr("Note: the NNLS fit will be performed "
                           ...
                           .arg(our_saxs_options->iqq_scale_maxq
                                    ...
                                    : ""));
            nnls_csv_footer
                << QString(us_tr("\"Note: the NNLS fit will be performed over "
                                 "a cropped range q\",%1,%2"))
                       .arg(our_saxs_options->iqq_scale_minq
                                ? QString("%1").arg(
                                      our_saxs_options->iqq_scale_minq)
                                : "")
          ...
          if (nnls_csv) {
            if (QFile::exists(nnls_csv_filename)) {
              nnls_csv_filename =
                  ((US_Hydrodyn *)us_hydrodyn)
                      ->fileNameCheck(nnls_csv_filename, 0, this);
            }
            ...
            if (f.open(QIODevice::WriteOnly)) {
              ...
              editor_msg("red",
                         QString(us_tr("Error attempting to create file: %1\n"))
                             .arg(nnls_csv_filename));
            }
          }
        }
        if (run_best_fit) {
          if (our_saxs_options->iqq_scale_minq ||
              our_saxs_options->iqq_scale_maxq) {
            editor_msg("dark red",
                       QString(us_tr("Note: the fit will be performed over a "
                                     "cropped range q(%1:%2)"))
                           .arg(our_saxs_options->iqq_scale_minq
                                    ? QString("%1").arg(
                                          our_saxs_options->iqq_scale_minq)
        ...
        editor->append(
            "NNLS error: could not find target and models in loaded data\n");
      }
    }
    if (plotted) {
      editor_msg("black", "I(q) plot done\n");
...
bool US_Hydrodyn_Saxs::load_check_csvs_compatible(QStringList filenames) {
  ...
  for (int i = 0; i < filenames.size(); ++i) {
    ...
    if (!grid.size()) {
      qDebug() << "load_check_csvs_compatible failed first line empty grid "
               << f;
      return false;
    }
    grid.removeLast();
    ...
    if (!refgrid.size()) {
      ...
      for (int j = 0; j < mingrid; ++j) {
        if (grid[j] != refgrid[j]) {
          qDebug() << "load_check_csvs_compatible failed incompatible grids "
                   << f;
          return false;
        }
      }
...
QString US_Hydrodyn_Saxs::unify_csv_files(QStringList filenames) {
  ...
  QString header = refgrid.join(",") +
                   ",,\"SOMO SAXS P(r) vs r data generated by unification of "
                   "multiple csv files";
    ...
    for (int j = 0; j < lines.size(); ++j) {
      ...
      if (line.contains("P(r) normed")) {
        ...
        if (names_used.count(fields[0])) {
          editor_msg("red", QString("unified csv load - duplicate name exists, "
                                    "%1, dropping this data\n")
                                .arg(fields[0]));
          continue;
        }
...
void US_Hydrodyn_Saxs::load_saxs(QString filename, bool just_plotted_curves,
  ...
  if (f.open(QIODevice::ReadOnly)) {
    ...
    if (ext == "int") {
      ...
      if (!our_saxs_options->crysol_default_load_difference_intensity) {
        res = US_Static::getItem(
            ...
            "Select the set you wish to plot::", lst, 0, false, &ok, this);
        if (ok) {
          // user selected an item and pressed OK
          Icolumn = 0;
    ...
    if (ext == "dat" || ext == "txt" || ext == "sprr") {
      ...
      if (qsl.filter("exp_intensity").size()) {
        ...
        switch (QMessageBox::question(
            ...
            us_tr("&Load only experimental"), us_tr("&Load only the model"),
            us_tr("&Load both"),
            2,  // Default
            0   // Escape == button 0
            )) {
    ...
    if (ext == "ssaxs") {
      ...
      if (number_of_fields >= 4) {
        ...
        res = US_Static::getItem("There are three available datasets",
                                 "Select the set you wish to plot::", lst, 0,
                                 false, &ok, this);
        if (ok) {
          // user selected an item and pressed OK
          Icolumn = 0;
    ...
    if (our_saxs_options->iq_scale_ask) {
      switch (QMessageBox::information(
          this, us_tr("UltraScan"),
          us_tr("Is this file in 1/Angstrom or 1/nm units?"), "1/&Angstrom",
          "1/&nm", 0,
          0,     // Enter == button 0
          1)) {  // Escape == button 2
    ...
    if (q.size()) {
      ...
      ((US_Hydrodyn *)us_hydrodyn)->last_saxs_header = QString("").sprintf(
          "Saxs curves from %s", filename.toLatin1().data());
      for (unsigned int i = 0; i < q.size(); i++) {
...
void US_Hydrodyn_Saxs::load_pr(bool just_plotted_curves, QString load_this,
                               bool skip_mw) {
  ...
  if (!just_plotted_curves) {
    ...
    if (filename.contains(QRegExp("_t(|-\\d+).csv$", Qt::CaseInsensitive))) {
      QMessageBox::information(
          this, "UltraScan", us_tr("Can not load transposed format csv files"));
      return;
    }
  }
  ...
  if (just_plotted_curves || f.open(QIODevice::ReadOnly)) {
    ...
    if (ext == "csv" || just_plotted_curves) {
      ...
      if (!just_plotted_curves) {
        ...
        if (qsl_r.size() < 6) {
          // QMessageBox mb(us_tr("UltraScan Warning"),
          //                us_tr("The csv file ") + filename + us_tr(" does not
          //                appear to contain any r values in the header
          //                rows.\n"), QMessageBox::Critical,
          //             QMessageBox::NoButton, QMessageBox::NoButton,
          //             QMessageBox::NoButton, 0, 0, 1);
          // mb.exec();
      ...
      if (filenames.size() > 1) {
        ...
        for (unsigned int i = 1; i < (unsigned int)filenames.size(); i++) {
          ...
          if (qsl2_headers.size() != 0) {
            ...
            // upgrade the csv format
            QRegExp rx(
                "^\"(Type; r:|P\\(r\\)|P\\(r\\) normed|P\\(r\\) sd|P\\(r\\) "
                "normed sd)\"$");
            QStringList new_qsl;
            double delta_r = 0e0;
            for (unsigned int i = 0; i < (unsigned int)qsl2.size(); i++) {
              ...
              if (tmp_qsl.size() > 1 && rx.indexIn(tmp_qsl[1]) != -1) {
                ...
                if (tmp_qsl[0] == "\"Name\"") {
                  ...
                  double tmp_area = 0e0;
                  for (unsigned int j = 1; j < (unsigned int)tmp_qsl.size();
                       j++) {
                    tmp_area += tmp_qsl[j].toDouble();
                  }
                  tmp2_qsl.push_back(QString("%1").arg(tmp_area * delta_r));
          ...
          qsl2_headers =
              qsl2.filter("\"Name\",\"MW (Daltons)\",\"Area\",\"Type; r:\"");
          if (qsl2_headers.size() == 0) {
            // QMessageBox mb(us_tr("UltraScan Warning"),
            //                us_tr("The csv file ") + f2.fileName() + us_tr("
          ...
          if (qsl_data.size() == 0) {
            ...
            //                not appear to contain any data rows.\n"),
            //                QMessageBox::Critical,
            //                QMessageBox::NoButton, QMessageBox::NoButton,
            //                QMessageBox::NoButton, 0, 0, 1);
            // mb.exec();
          ...
          if (qsl_r.size() < 6) {
            // QMessageBox mb(us_tr("UltraScan Warning"),
            //                us_tr("The csv file ") + f2.fileName() + us_tr("
            //                does not appear to contain any r values in the
            //                header rows.\n"), QMessageBox::Critical,
            //                QMessageBox::NoButton, QMessageBox::NoButton,
            //                QMessageBox::NoButton, 0, 0, 1);
            // mb.exec();
          ...
          if (r.size() > 1 && r[r.size() - 1] < max_value) {
            ...
            if (r_delta > 1e0) {
              for (double value = r[r.size() - 1] + r_delta; value <= max_value;
                   value += r_delta) {
                r.push_back(value);
              }
            }
          ...
          for (QStringList::iterator it = qsl_data.begin();
            ...
            if (qsl_pr.size() < 6) {
              QString msg = us_tr("The csv file ") + f2.fileName() +
                            us_tr(
                                " does not appear to contain sufficient p(r) "
                                "values in data row " +
                                qsl_pr[0] + ", skipping\n");
              editor_msg("red", msg);
            } else {
              ...
              if (all_match) {
                ...
                // optionally?
                if (qs_tmp.contains(QRegExp("(Average|Standard deviation)"))) {
                  qs_tmp.replace(QRegExp("^\""), QString("\"%1: ").arg(
                ...
                if (!interpolate(r, r2, this_pr, npr)) {
                  editor_msg(
                      "red",
                      QString(us_tr("Interpolation error on curve in %1 "))
                          .arg(f2.fileName()));
                  ok = false;
                }
                ...
                if (ok) {
                  ...
                  if (!qs_assoc_sd.isEmpty()) {
                    // TSO << "--> Found SD for " << new_pr_fields[0] << " in "
                    // << QFileInfo(f2).baseName() << "\n";
                    ...
                    if (qsl_pr_error.size() > 4) {
                      ...
                      if (!interpolate(r, r2, this_pr_error, new_pr_error)) {
                        editor_msg(
                            "red",
                            QString(
                                us_tr("Interpolation error on curve in %1 "))
                                .arg(f2.fileName()));
                      } else {
                        QString line =
                            QString("%1,%2\n")
                        ...
                        // TSO <<
                        // QString("r:\n%1\n").arg(vector_double_to_csv(r)); TSO
                        // <<
                        // QString("r2:\n%1\n").arg(vector_double_to_csv(r2));
                        // TSO << QString("org line:\n%1\n").arg(qs_assoc_sd);
                        // TSO << QString("new interpolated
                        // line:\n%1\n").arg(line);
                        if (line.contains(
                  ...
                  if (line.contains(QRegExp("(Average|Standard deviation)"))) {
                    line.replace(QRegExp("^\""), QString("\"%1: ").arg(
                                                     QFileInfo(f2).baseName()));
                  }
                  qsl << line;
                  {
      ...
      bool added_interpolate_msg = false;
      QString bin_msg = "";
      for (unsigned int i = 0; i < (unsigned int)qsl_plotted_pr_names.size();
           i++) {
        vector<double> npr =
            interpolate(r, plotted_r[i], plotted_pr_not_normalized[i]);
        ...
        if (plotted_pr_not_normalized_error[i].size()) {
          ...
          npr_error =
              interpolate(r, plotted_r[i], plotted_pr_not_normalized_error[i]);
        }
        if (!added_interpolate_msg && r.size() > 1) {
          bin_msg =
              QString(us_tr("Plotted P(r) interpolated to bin size of %1"))
                  .arg(r[1] - r[0]);
          if (plotted_r[i].size() > 1 &&
            ...
            bin_msg +=
                QString(
                    us_tr(
                        " which is DIFFERENT from the plotted bin size of %1"))
      ...
      for (QStringList::iterator it = qsl_data.begin(); it != qsl_data.end();
        ...
        if (map_sel_names.count(qsl_tmp[0])) {
          ...
          // 1st check the line to see if we have a mw
          {
            if (qsl_tmp[1].toFloat() > 0e0) {
              (*remember_mw)[QFileInfo(qsl_tmp[0]).fileName()] =
                  qsl_tmp[1].toFloat();
              (*remember_mw_source)[QFileInfo(qsl_tmp[0]).fileName()] =
          ...
          QStringList qsl_pr =
              QString(*it).replace(QRegularExpression(",\\s*$"), "").split(",");
          if (qsl_pr.size() < 6) {
            QString msg = us_tr("The csv file ") + filename +
                          us_tr(
                              " does not appear to contain sufficient p(r) "
          ...
          if (!qs_sd.isEmpty()) {
            ...
            if (pr_error.size() != pr.size()) {
              editor_msg("red",
                         QString(us_tr("S.D.s found in %1, but they are not of "
                                       "the same length (%2) as the P(r) data "
                                       "points (%3), dropping S.D.s"))
                             .arg(qsl_pr[0])
                             .arg(pr_error.size())
                             .arg(pr.size()));
              // US_Vector::printvector3( "pr sd mismatch, r, pr, pr_error:", r,
              // pr, pr_error );
              pr_error.clear();
            }
          }
          ...
          // r has the ordinates for the longest data, some will likely be
          // shorter
          if (r.size() > pr.size()) {
          ...
          if (!(create_avg && only_plot_stats) && !run_nnls && !run_best_fit) {
            ...
            (*remember_mw_source)[use_filename + " " + qsl_tmp[0]] =
                "loaded from csv";
            plot_one_pr(this_r, pr, pr_error, use_filename + " " + qsl_tmp[0],
                        skip_mw);
      ...
      if (create_avg && sum_count && !run_nnls && !run_best_fit) {
        ...
        plot_one_pr(this_r, pr, no_error,
                    QFileInfo(filename).fileName() + " Average", skip_mw);
        ...
        if (create_std_dev && sum_count > 2) {
          ...
          for (unsigned int i = 0; i < sum_pr.size(); i++) {
            ...
            if (us_isnan(sum_pr[i])) {
              TSO << QString(
                         "WARNING when calc'ing std dev: isnan sum_pr[%1]\n")
                         .arg(i);
            }
            if (us_isnan(sum_pr2[i])) {
              TSO << QString(
                         "WARNING when calc'ing std dev: isnan sum_pr2[%1]\n")
                         .arg(i);
            }
            if (us_isnan(std_dev[i])) {
              TSO << QString(
          ...
          double area_sd_avg = (pr_avg_area > 0e0 && pr_std_dev_area > 0e0)
          ...
          (*remember_mw)[QFileInfo(filename).fileName() +
                         " Average plus 1 std dev"] = pr_mw_avg + pr_mw_std_dev;
          ...
          plot_one_pr(
              this_r, pr, no_error,
              QFileInfo(filename).fileName() + " Average minus 1 std dev",
              skip_mw);
          ...
          plot_one_pr(
              this_r, pr, no_error,
              QFileInfo(filename).fileName() + " Average plus 1 std dev",
              skip_mw);
        }
        if (plotted) {
        ...
        if (save_to_csv) {
          ...
          QString fname = ((US_Hydrodyn *)us_hydrodyn)->somo_dir + SLASH +
                          "saxs" + SLASH + csv_filename + "_sprr_" +
          ...
          if (of) {
            ...
            fprintf(of, "\"%s\",%.2f,%.2f,\"%s\",%s\n", "Average", pr_mw_avg,
                    compute_pr_area(pr_avg, r), "P(r)",
                    vector_double_to_csv(pr_avg).toLatin1().data());
            if (pr_std_dev.size()) {
              fprintf(of, "\"%s\",%.2f,%.2f,\"%s\",%s\n", "Standard deviation",
                      pr_mw_std_dev, compute_pr_area(pr_std_dev, r), "P(r)",
                      vector_double_to_csv(pr_std_dev).toLatin1().data());
              fprintf(
                  of, "\"%s\",%.2f,%.2f,\"%s\",%s\n",
                  "Average minus 1 standard deviation",
                  pr_mw_avg - pr_mw_std_dev,
                  compute_pr_area(pr_avg_minus_std_dev, r), "P(r)",
                  vector_double_to_csv(pr_avg_minus_std_dev).toLatin1().data());
              fprintf(
                  of, "\"%s\",%.2f,%.2f,\"%s\",%s\n",
                  "Average plus 1 standard deviation",
                  pr_mw_avg + pr_mw_std_dev,
                  compute_pr_area(pr_avg_plus_std_dev, r), "P(r)",
        ...
        if (run_nnls || run_best_fit) {
          ...
          if (found_nnls_model && found_nnls_target) {
            ...
            if (run_nnls) {
              {
                QString rg_msg;
                {
                  double Rg;
                  QString errormsg;
                  if (US_Saxs_Util::compute_rg_from_pr(nnls_r, nnls_B, Rg,
                                                       errormsg)) {
              ...
              if (nnls_csv) {
                ...
                if (f.open(QIODevice::WriteOnly)) {
                  QTextStream tso(&f);
                  tso << "\"File\",\"Model\",\"Contribution weight\",\"Rg "
                         "[A]\",\"Dmax [A]\"\n";
                  tso << nnls_csv_data.join("\n") << "\n";
                  tso << "\n\"Messages:\"\n";
                  tso << nnls_csv_footer.join("\n") << "\n";
            ...
            editor->append(
                "NNLS error: could not find target and models in loaded "
                "data\n");
          }
        }
        if (plotted) {
    ...
    while (!ts.atEnd()) {
      ...
      if (qsl.size() < 2) {
        editor_msg("black",
                   QString(us_tr("Notice: File %1 : improper format line found "
    ...
    if (pr_error.size() && pr.size() != pr_error.size()) {
      ...
      editor_msg(
          "darkRed",
          QString(us_tr("File %1 : some, but not all, data had apparent "
...
void US_Hydrodyn_Saxs::rescale_iqq_curve(QString scaling_target,
                                         vector<double> &q, vector<double> &I,
...
void US_Hydrodyn_Saxs::rescale_iqq_curve(QString scaling_target,
  ...
  TSO << QString(
             "After cropping q to overlap region:\n"
             "use_q.size == %1\n")
             .arg(use_q.size());
  ...
  bool do_chi2_fitting =
      use_SDs_for_fitting_iqq ||
      (is_nonzero_vector(use_I_error) && use_I_error.size() == use_I.size());
  ...
  if (do_chi2_fitting && is_zero_vector(use_I_error)) {
    if (use_SDs_for_fitting_iqq) {
      editor_msg("red", us_tr("Chi^2 fitting requested, but target data has no "
  ...
  if (our_saxs_options->iqq_scale_nnls) {
    ...
    if (!use_SDs_for_fitting_iqq) {
      if (do_plot_residuals) {
        editor_msg("red", us_tr("Chi^2 fitting is currently not compatable "
                                "with NNLS scaling\n"));
      }
      do_chi2_fitting = false;
    }
    if (our_saxs_options->iqq_scale_linear_offset) {
      if (do_plot_residuals) {
        editor_msg("red", us_tr("Scale with linear offset is not compatable "
                                "with NNLS scaling\n"));
      }
    ...
    if (our_saxs_options->iqq_scale_linear_offset) {
      if (do_plot_residuals) {
        editor_msg(
            "red",
            us_tr("Scale with linear offset is not currently implemented\n"));
      }
      do_scale_linear_offset = false;
    }
    if (do_chi2_fitting && !is_nonzero_vector(use_I_error)) {
      if (do_plot_residuals) {
        editor_msg("red", us_tr("Chi^2 fitting disabled, zeros in target "
                                "standard deviation\n"));
      }
      do_chi2_fitting = false;
    }
  ...
  if (do_plot_residuals) {
    if (do_chi2_fitting) {
      // usu.calc_chisq_prob( 0.5 * use_I.size() - ( do_scale_linear_offset ? 2
      // : 1 ), 0.5 * chi2, chi2_prob );
      fit_msg =
...
void US_Hydrodyn_Saxs::rescale_iqq_curve_using_last_rescaling(vector<double> &I,
                                                              bool use_offset) {
  for (unsigned int i = 0; i < I.size(); i++) {
...
vector<double> US_Hydrodyn_Saxs::range_crop(vector<double> &q,
                                            vector<double> &I) {
  ...
  if (filename.isEmpty()) {
    QString use_dir =
        our_saxs_options->path_load_saxs_curve.isEmpty()
            ...
            : our_saxs_options->path_load_saxs_curve;
    select_from_directory_history(use_dir, this);
    filename = QFileDialog::getOpenFileName(this, "Open", use_dir,
                                            "All files (*);;"
                                            "ssans files (*.ssans);;"
                                            "csv files (*.csv);;"
  ...
  if (f.open(QIODevice::ReadOnly)) {
        ...
        if (mwline.size() > 1) {
          if (!((US_Hydrodyn *)us_hydrodyn)->gui_script) {
            US_Static::us_message(
                us_tr("Please note:"),
                QString(us_tr("There are multiple molecular weight lines in "
                              "the gnom file\n"
        ...
        while (!ts.atEnd()) {
          ...
          if (rx5.indexIn(tmp) != -1) {
            ...
            if (rx2.indexIn(tmp) == -1) {
              ...
              // if ( plotted )
              // {
              //   editor->setParagraphBackgroundColor ( editor->paragraphs() -
              //   1, QColor("white") ); editor->append("I(q) vs q plot
              //   done\n"); plotted = false;
              // }
              break;
            } else {
      ...
      if (prrh.indexIn(tmp) != -1) {
        ...
        while (!ts.atEnd()) {
          ...
          if (rx3.indexIn(tmp) != -1) {
            ...
            //       failed!\n").arg(mwline[0]); gnom_mw = 0e0;
            //    } else {
            //       TSO << QString("mwline cap 0 <%1> cap 1
            //       <%2>\n").arg(qx_mw.cap(0)).arg(qx_mw.cap(1)); gnom_mw =
            //       qx_mw.cap(1).toDouble();
            //    }
            //    if ( mwline.size() > 1 )
            //    {
            //       if ( !((US_Hydrodyn *)us_hydrodyn)->gui_script ) {
            //          US_Static::us_message(us_tr("Please note:"),
            //                                QString(us_tr("There are multiple
            //                                molecular weight lines in the gnom
            //                                file\n"
            //                                              "Using the first one
            //                                              found (%1
            //                                              Daltons)")).arg(gnom_mw));
            ...
            if (!sprr_saved) {
              sprr_saved = true;
              QString rxstr = "\\..*$";
              QString dest = USglobal->config_list.root_dir + "/somo/saxs/" +
                             QString("%1")
                                 .arg(QFileInfo(filename).fileName())
                                 .replace(QRegExp(rxstr), "_gnom.sprr");
              if (!((US_Hydrodyn *)us_hydrodyn)->overwrite) {
                dest =
                    ((US_Hydrodyn *)us_hydrodyn)->fileNameCheck(dest, 0, this);
              }

              double mw = get_mw(filename, false);
              QStringList sprrcontents;

              sprrcontents << QString("# GNOM P(r) from " + filename +
                                      "%1\nR\tP(r)\tSD\n")
                                  .arg(mw == -1e0
                                           ? QString("")
              ...
              // save reinterpolated to current bin size sprr
              {
                ...
                if (!interpolate(to_grid, r, pr, ipr)) {
                  editor_msg("red", QString("error interpolating gnom p(r) "
                ...
                if (pr.size() != pre.size()) {
                  qDebug()
                      << "gnom load sprr create, data error length mismatch";
                  ok = false;
                }
                ...
                if (ok) {
                  ...
                  //    }
                  //    QTextStream(stdout) << US_Vector::qs_vector4( "to_grid,
                  //    ipre, iprppre-ipr, ipr-iprmpre", to_grid, ipre,
                  ...
                  sprrcontents
                      << QString("%1 0 0\n")
                             .arg(to_grid.back() + our_saxs_options->bin_size,
                                  0, 'g', 9);
                  QString error;
                  QString putcontents = sprrcontents.join("");
                  if (!US_File_Util::putcontents(dest, putcontents, error)) {
          ...
          if (!sprr_saved) {
            ...
            // save reinterpolated to current bin size sprr
            {
              ...
              if (!natural_spline_interpolate(to_grid, r, pr, ipr)) {
                editor_msg("red", QString("error interpolating gnom p(r) grid "
              ...
              if (ok) {
                ...
                sprrcontents
                    << QString("%1 0 0\n")
                           .arg(to_grid.back() + our_saxs_options->bin_size, 0,
                                'g', 9);
                QString error;
                QString putcontents = sprrcontents.join("");
                if (!US_File_Util::putcontents(dest, putcontents, error)) {
    ...
    if (ask_save_mw_to_gnom && gnom_mw) {
      switch (QMessageBox::information(
          ...
          QString(us_tr("Do you want to save the molecular weight entered (%1 "
                        "Daltons) into the gnom.out file?"))
              .arg(gnom_mw),
          "&Ok", "&Cancel", 0,
          0,     // Enter == button 0
          1)) {  // Escape == button 2
    ...
    if (datafiles.size()) {
      for (unsigned int i = 0; i < datafiles.size(); i++) {
        QString datafile =
            our_saxs_options->path_load_gnom + QDir::separator() + datafiles[i];
