...
static QStringList
pr_csv_pad_holes (const QStringList &qsl)
{
  TSO << "pr_csv_pad_holes()\n";

  // find r vector

  QStringList qsl_headers
      = qsl.filter ("\"Name\",\"MW (Daltons)\",\"Area\",\"Type; r:\"");
  if (!qsl_headers.size ())
    {
  ...
  if (qsl_r.size () < 6)
    {
      TSO << "pr_csv_pad_holes() does not appear to contain any r values in "
             "the header rows.\n";
      return qsl;
    }
  ...
  for (auto it = pad_here.begin (); it != pad_here.end (); ++it)
    {
      ...
      for (int i = 0; i < it->second; ++i)
        {
          // fix header
          qsl_header_new.insert (
              it->first + i,
              QString ("%1").arg (qsl_header_new[it->first - 1].toDouble ()
                                  + (delta * (i + 1))));
          // fix datalines
  ...
  // map names for pr lookup on pr sd line
  {
    ...
    for (int i = 0; i < (int)qsl_data_new.size (); ++i)
      {
        if (qsl_data_new[i].size () > 4
            && qsl_data_new[i][3].contains (
                QRegularExpression ("^\"P\\(r\\)(| normed) sd\"$")))
          {
            ...
            for (int j = 4; j < (int)qsl_data_new[i].size (); ++j)
              {
                if (qsl_data_new[i][j].toDouble () > 0
                    && ((qsl_data_new[i][3] == "\"P(r) sd\""
                         ...
                         && qsl_data_new[name2pr[qsl_data_new[i][0]]].size ()
                                > j
                         && qsl_data_new[name2pr[qsl_data_new[i][0]]][j]
                                    .toDouble ()
                                > 0)
                        || (qsl_data_new[i][3] == "\"P(r) normed sd\""
                            && name2pr_normed.count (qsl_data_new[i][0])
                            && qsl_data_new[name2pr_normed[qsl_data_new[i][0]]]
                                       .size ()
                                   > j
                            && qsl_data_new[name2pr_normed[qsl_data_new[i][0]]]
                                           [j]
            ...
            if (!sd_gz.size ())
              {
                // no nonneg sds! punt
                qDebug () << QString ("pr_csv_pad_holes() : curve %1 %2 no "
                                      "nonneg sds! punt")
                                 .arg (qsl_data_new[i][0])
                                 .arg (qsl_data_new[i][3]);
                break;
              }
            sd_gz.sort ();
            ...
            use_min_sd = 0;
            // qDebug() << QString( "use min sd for curve %1 is %2" ).arg(
            // qsl_data_new[i][0] ).arg( use_min_sd ); replace zeros with
            for (int j = 4; j < (int)qsl_data_new[i].size (); ++j)
              {
                if (qsl_data_new[i][j].toDouble () <= 0)
                  {
...
static QStringList
csv_transpose (const QStringList &qsl)
{
...
static QStringList
csv_pr2iq (const QStringList &qsl)
{
  qDebug () << "csv_pr2iq()";
  // QTextStream( stdout ) << "source:" << qsl.join( "\n" ) << "\n";
...
void
US_Hydrodyn_Saxs::load_iqq_csv (QString filename, bool just_plotted_curves)
{

  vector<QString>
      filenames; // for later use to support multiple csv file loads
      ...
      if (!f.open (QIODevice::ReadOnly))
        {
          QMessageBox::warning (
              this, "UltraScan",
              QString (us_tr ("Can not open file ") + filename));
          return;
        }
      QTextStream ts (&f);
      while (!ts.atEnd ())
  ...
  if (qsl_headers.size () == 0 && !just_plotted_curves)
    {
      ...
      //                us_tr("The csv file ") + filename + us_tr(" does not
      //                appear to contain a correct header.\nPlease manually
      //                correct the csv file."), QMessageBox::Critical,
      //                QMessageBox::NoButton, QMessageBox::NoButton,
      //                QMessageBox::NoButton, 0, 0, 1);
      // mb.exec();
      QMessageBox::critical (
          this, us_tr ("UltraScan Warning"),
          us_tr ("The csv file ") + filename
              + us_tr (" does not appear to contain a correct header.\nPlease "
                       "manually correct the csv file."),
      ...
      for (unsigned int i = 1; i < (unsigned int)qsl_headers.size (); i++)
        {
          if (ref != qsl_headers[i])
            {
              // QMessageBox mb(us_tr("UltraScan Warning"),
              //                us_tr("The csv file ") + filename + us_tr("
              //                contains multiple different headers\nPlease
              //                manually correct the csv file."),
              //                QMessageBox::Critical,
              //                QMessageBox::NoButton, QMessageBox::NoButton,
              //                QMessageBox::NoButton, 0, 0, 1);
              ...
              QMessageBox::critical (
                  this, us_tr ("UltraScan Warning"),
                  us_tr ("The csv file ") + filename
                      + us_tr (" contains multiple different headers\nPlease "
                               "manually correct the csv file."),
                  QMessageBox::NoButton, QMessageBox::NoButton,
  ...
  if (just_plotted_curves && !qsl_plotted_iq_names.size ())
    {
  ...
  if (!just_plotted_curves)
    {
      if (qsl_plotted_iq_names.size ())
        {
          ...
          grid_target = US_Static::getItem (
              us_tr ("Set I(q) Grid"),
              us_tr ("Select the target plotted data set for the data being "
                     "loaded grid:\n"
                     "or Cancel if you wish to interpolate the plotted to the "
                     "data being loaded"),
      ...
      if (qsl_q.size () < 3)
        {
          // QMessageBox mb(us_tr("UltraScan Warning"),
          //                us_tr("The csv file ") + filename + us_tr(" does
          //                not appear to contain any q values in the header
          //                rows.\n"), QMessageBox::Critical,
          //                QMessageBox::NoButton, QMessageBox::NoButton,
          //                QMessageBox::NoButton, 0, 0, 1);
          // mb.exec();
          QMessageBox::critical (
              this, us_tr ("UltraScan Warning"),
              us_tr ("The csv file ") + filename
                  + us_tr (" does not appear to contain any q values in the "
                           "header rows.\n"),
              QMessageBox::NoButton, QMessageBox::NoButton,
              QMessageBox::NoButton);
          return;
          ...
          if (!plotted_iq_names_to_pos.count (grid_target))
            {
              editor_msg (
                  ...
                  QString (
                      us_tr (
                          "Internal error: could not find %1 in plotted data"))
                      .arg (grid_target));
              grid_target = "";
            }
          else
            {
              ...
              if (q[0] < original_q[0] || q.back () > original_q.back ())
                {
                  editor_msg ("dark red",
                              QString ("Notice: the target curve is cropped "
                                       "to q(%1:%2) to prevent extrapolation")
                                  .arg (original_q[0])
                                  .arg (original_q.back ()));

                  nnls_csv_footer
                      << QString ("\nNotice: the target curve is cropped to "
                                  "prevent extrapolation q range\",%1,%2\n")
                             .arg (original_q[0])
                             .arg (original_q.back ());
              ...
              msg.replace ("\"", "");
              qsl[0] = QString ("\"Name\",\"Type; q:\",%1,\"%2 reinterpolated "
                                "to q(%3:%4) step %5\"")
              ...
              for (unsigned int i = 0; i < (unsigned int)qsl_data.size (); i++)
                {
                  ...
                  // QTextStream(stdout) << US_Vector::qs_vector3( "before usu
                  // interpolate original_q, original_i, original_i_error",
                  // original_q, original_i, original_i_error ) << "\n";
                  if (!usu.interpolate_iqq_by_case (original_q, original_i,
                                                    original_i_error, q, ni,
                                                    ni_error))
                    {
                      editor_msg ("red", usu.errormsg);
                      QMessageBox::warning (
                          this, "US-SOMO",
                          QString (us_tr ("There was an error attempting to "
                                          "interpolate\n"
                                          "%1 q(%2:%3) to the common grid of "
                                          "q(%4:%5)\n"
                                          "Error : \"%6\""))
                              .arg (qsl_d[0])
                              .arg (original_q.size () ? original_q[0] : 0)
                              .arg (original_q.size ()
                                        ? original_q[original_q.size () - 1]
                                        : 0)
                              .arg (q.size () ? q[0] : 0)
                              .arg (q.size () ? q[q.size () - 1] : 0)
                              .arg (usu.errormsg));
                  ...
                  // QTextStream(stdout) << US_Vector::qs_vector3( "after usu
                  // interpolate q, ni, ni_error", q, ni, ni_error ) << "\n";

                  new_qsl << QString ("%1,\"I(q)\",%2")
                                 .arg (qsl_d[0])
                                 .arg (vector_double_to_csv (ni));
                  if (sd_map.count (qsl_d[0]))
                    {
                      new_qsl_errors
                          << QString ("%1,\"I(q) sd\",%2")
                                 .arg (qsl_d[0])
                                 .arg (vector_double_to_csv (ni_error));
                    }
                }
              for (unsigned int i = 0;
                   i < (unsigned int)new_qsl_errors.size (); i++)
                {
  ...
  else
    {
      ...
      QString header = QString ("\"Name\",\"Type; q:\",%1,%2\n")
                           .arg (vector_double_to_csv (q))
                           .arg (header_tag);
      qsl << header;
      qsl_headers << header;
  ...
  // this needs to be gone over again in detail, since it is a copy of the prr
  // version
      ...
      for (unsigned int i = 1; i < (unsigned int)filenames.size (); i++)
        {
          ...
          if (qsl2_headers.size () == 0)
            {
              // QMessageBox mb(us_tr("UltraScan Warning"),
              //                us_tr("The csv file ") + f2.fileName() +
              //                us_tr(" does not appear to contain a correct
              //                header.\nPlease manually correct the csv
              //                file."), QMessageBox::Critical,
              //                QMessageBox::NoButton, QMessageBox::NoButton,
              //                QMessageBox::NoButton, 0, 0, 1);
              // mb.exec();
              QMessageBox::critical (
                  ...
                  us_tr ("The csv file ") + f2.fileName ()
                      + us_tr (
                          " does not appear to contain a correct "
          ...
          if (qsl2_headers.size () > 1)
            {
              ...
              for (unsigned int i = 1; i < (unsigned int)qsl2_headers.size ();
                   i++)
                {
                  if (ref != qsl2_headers[i])
                    {
                      // QMessageBox mb(us_tr("UltraScan Warning"),
                      //                us_tr("The csv file ") + filename +
                      //                us_tr(" contains multiple different
                      //                headers\nPlease manually correct the
                      //                csv file."), QMessageBox::Critical,
                      //                QMessageBox::NoButton,
                      //                QMessageBox::NoButton,
                      //                QMessageBox::NoButton, 0, 0, 1);
                      // mb.exec();
          ...
          if (qsl_data.size () == 0)
            {
              // QMessageBox mb(us_tr("UltraScan Warning"),
              //                us_tr("The csv file ") + filename + us_tr("
              //                does not appear to contain any data rows.\n"),
              //                QMessageBox::Critical,
              //                QMessageBox::NoButton, QMessageBox::NoButton,
              //                QMessageBox::NoButton, 0, 0, 1);
              // mb.exec();
              QMessageBox::critical (
                  this, us_tr ("UltraScan Warning"),
                  us_tr ("The csv file ") + filename
                      + us_tr (" does not appear to contain any data rows.\n"),
                  QMessageBox::NoButton, QMessageBox::NoButton,
                  QMessageBox::NoButton);
              return;
            }
          ...
          for (QStringList::iterator it = qsl_errors.begin ();
               it != qsl_errors.end (); it++)
            {
              QStringList qsl_iq_errors = (*it).split (",");
              name_to_errors_map[qsl_iq_errors[0]] = *it;
            }

          // QTextStream(stdout) << US_Vector::qs_mapqsqs(
          // "name_to_errors_map", name_to_errors_map ) << "\n";
          ...
          if (qsl_q.size () < 3)
            {
              // QMessageBox mb(us_tr("UltraScan Warning"),
              //                us_tr("The csv file ") + f2.fileName() +
              //                us_tr(" does not appear to contain any q values
              //                in the header rows.\n"), QMessageBox::Critical,
              //                QMessageBox::NoButton, QMessageBox::NoButton,
              //                QMessageBox::NoButton, 0, 0, 1);
              // mb.exec();
              QMessageBox::critical (
                  this, us_tr ("UltraScan Warning"),
                  us_tr ("The csv file ") + f2.fileName ()
                      + us_tr (" does not appear to contain any q values in "
                               "the header rows.\n"),
                  QMessageBox::NoButton, QMessageBox::NoButton,
                  QMessageBox::NoButton);
              return;
          ...
          for (QStringList::iterator it = qsl_data.begin ();
               it != qsl_data.end (); it++)
            {
              ...
              if (qsl_iq.size () < 3)
                {
                  QString msg = us_tr ("The csv file ") + f2.fileName ()
                                + us_tr (" does not appear to contain "
                                         "sufficient I(q) values in data row "
                                         + qsl_iq[0] + ", skipping\n");
                  editor_msg ("red", msg);
                }
              else
                  ...
                  if (all_match)
                    {
                      ...
                      // optionally?
                      if (qs_tmp.contains (
                              QRegExp ("(Average|Standard deviation)")))
                        {
                          qs_tmp.replace (QRegExp ("^\""),
                                          QString ("\"%1: ").arg (
                                              QFileInfo (f2).baseName ()));
                        }
                      qsl << qs_tmp;
                      // TSO << "yes, all match\nadding:" << qs_tmp << endl;
                    }
                  else
                    {
                      ...
                      if (!interp_msg_done.count (f2.fileName ()))
                        {
                          ...
                          QString msg = us_tr ("The csv file ")
                                        + f2.fileName ()
                                        + us_tr (" will be interpolated\n");
                      ...
                      if (name_to_errors_map.count (qsl_iq[0]) != 0)
                        {
                          // the new iq:
                          QStringList qsl_iq_errors
                              = (name_to_errors_map[qsl_iq[0]]).split (",");
                          QStringList new_iq_errors_fields;
                          // pull the data values
                          vector<double> this_iq_errors;
                          for (unsigned int i = 0; i < 2; i++)
                            {
                              new_iq_errors_fields.push_back (
                                  qsl_iq_errors[i]);
                            }
                          {
                          ...
                          // interpolate q2, iq to r, reappend to new_iq_fields
                          vector<double> niq_errors
                              = interpolate (q, q2, this_iq_errors);
                          QString line
                              = QString ("%1,%2\n")
  ...
  if (found_cropping)
    {
      ...
      if (crop_min > q[0] || crop_max < q[q.size () - 1])
        {
          editor_msg ("dark red", QString ("cropping loaded curves to plotted "
                                           "range intersection q(%1:%2)")
                                      .arg (crop_min)
                                      .arg (crop_max));
          ...
          msg.replace ("\"", "");
          qsl[0]
              = QString ("\"Name\",\"Type; q:\",%1,\"%2 cropped to q(%3:%4)\"")
                    .arg (vector_double_to_csv (q))
                    .arg (msg)
                    .arg (q.size () ? q[0] : 0)
                    .arg (q.size () ? q[q.size () - 1] : 0);
          ...
          for (unsigned int i = 0; i < (unsigned int)qsl_data.size (); i++)
            {
              ...
              vector<double> ni;
              vector<double> ni_error;
              if (!usu.crop (original_q, original_i, original_i_error,
                             crop_min, crop_max, ni, ni_error))
                {
                  editor_msg ("red", usu.errormsg);
                  return;
                }
              ...
              if (sd_map.count (qsl_d[0]))
                {
                  new_qsl_errors << QString ("%1,\"I(q) sd\",%2")
                                        .arg (qsl_d[0])
                                        .arg (vector_double_to_csv (ni_error));
                }
            }
          for (unsigned int i = 0; i < (unsigned int)new_qsl_errors.size ();
               i++)
            {
              new_qsl << new_qsl_errors[i];
            }
  ...
  for (unsigned int i = 0; i < (unsigned int)qsl_plotted_iq_names.size (); i++)
    {
      ...
      // vector < double > nic_errors = interpolate(q, plotted_q[i],
      // plotted_I_error[i]);
      US_Saxs_Util usu;
      vector<double> nic;
      ...
      // QTextStream(stdout)
      //    << "append all currently plotted I(q)s to qsl\n"
      //    << US_Vector::qs_vector3( "plotted q, I, I_error", plotted_q[i],
      //    plotted_I[i], plotted_I_error[i] )
      //    ;
      if (!usu.interpolate_iqq_by_case (plotted_q[i], plotted_I[i],
                                        plotted_I_error[i], q, nic,
                                        nic_errors))
        {
          editor_msg ("red", usu.errormsg);
          QMessageBox::warning (
      ...
      if (!added_interpolate_msg && q.size () > 1)
        {
          bin_msg
              = QString (us_tr ("Plotted I(q) interpolated to delta q of %1"))
                    .arg (q[1] - q[0]);
          if (plotted_q[i].size () > 1
              && q[1] - q[0] != plotted_q[i][1] - plotted_q[i][0])
            {
              bin_msg += QString (us_tr (" which is DIFFERENT from the "
                                         "plotted delta q of %1"))
      ...
      QString line = QString ("\"%1\",\"I(q)\",%2\n")
                         .arg (qsl_plotted_iq_names[i].replace (
                             QRegExp ("(^[^\\\"]*\\\"|\\\"[^\\\"]*$)"), ""))
      ...
      if (new_qsl.size () != qsl.size ())
        {
          switch (QMessageBox::information (
              ...
              us_tr ("There are multiple average and/or standard deviation "
                     "lines\n")
                  + us_tr ("What do you want to do?"),
              us_tr ("&Skip"), us_tr ("&Just averages"), us_tr ("&Include"),
              0, // Enter == button 0
              1))
            {       // Escape == button 2
            case 0: // skip them
  ...
  if (qsl_data.size () == 0)
    {
      // QMessageBox mb(us_tr("UltraScan Warning"),
      //                us_tr("The csv file ") + filename + us_tr(" does not
      //                appear to contain any data rows.\n"),
      //                QMessageBox::Critical,
      //                QMessageBox::NoButton, QMessageBox::NoButton,
      //                QMessageBox::NoButton, 0, 0, 1);
      // mb.exec();
  ...
  for (QStringList::iterator it = qsl_errors.begin (); it != qsl_errors.end ();
       it++)
    {
      QStringList qsl_iq_errors = (*it).split (",");
      name_to_errors_map[qsl_iq_errors[0]] = *it;
  ...
  for (QStringList::iterator it = qsl_data.begin (); it != qsl_data.end ();
       it++)
    {
  ...
  US_Hydrodyn_Saxs_Iqq_Load_Csv *hslc = new US_Hydrodyn_Saxs_Iqq_Load_Csv (
      "Select models to load\n" + header_tag
          + (bin_msg.isEmpty () ? "" : "\n" + bin_msg),
      &qsl_names, &qsl_sel_names, &qsl, filename, &create_avg, &create_std_dev,
      &only_plot_stats, &save_to_csv, &csv_filename, &save_original_data,
      &run_nnls, &nnls_plot_contrib, &nnls_csv, &run_best_fit, &run_ift,
      &use_SDs_for_fitting_iqq, &nnls_target, &clear_plot_first, 1 || U_EXPT,
      us_hydrodyn);
  ...
  if (run_ift)
    {
      ...
      ift_to_process.replaceInStrings (QRegExp ("^\""), "")
      ...
      if (run_nnls)
        {
          ...
          if (nnls_csv)
            {
              QString use_dir = USglobal->config_list.root_dir + "/" + "somo"
                                + "/" + "saxs";

              // ((US_Hydrodyn *)us_hydrodyn)->select_from_directory_history(
              // use_dir, this );

              nnls_csv_filename = QFileDialog::getSaveFileName (
                  this, us_tr ("Choose a filename to save the NNLS fit"),
                  use_dir + "/"
                      + QFileInfo (QString ("%1")
                                       .arg (nnls_target)
                                       .replace ("\"", ""))
              ...
              if (!nnls_csv_filename.contains (
                      QRegExp (".csv", Qt::CaseInsensitive)))
                {
      ...
      if (map_sel_names.count (qsl_tmp[0]))
        {
          ...
          if (qsl_iq.size () < 3)
            {
              QString msg = us_tr ("The csv file ") + filename
                            + us_tr (" does not appear to contain sufficient "
                                     "I(q) values in data row "
                                     + qsl_tmp[0] + "\n");
              editor_msg ("red", msg);
              // QMessageBox mb(us_tr("UltraScan Warning"),
              //                msg,
              ...
              //                QMessageBox::NoButton, 0, 0, 1);
              // mb.exec();
              QMessageBox::critical (this, us_tr ("UltraScan Warning"), msg,
                                     QMessageBox::NoButton,
                                     QMessageBox::NoButton,
                                     QMessageBox::NoButton);
              break;
          ...
          if (run_nnls || run_best_fit)
            {
              // TSO << QString("US_Hydrodyn_Saxs::load_iqq_csv %1 size
          ...
          // q has the ordinates for the longest data, some will likely be
          // shorter
          if (q.size () > I.size ())
            {
              this_q.resize (I.size ());
            }
          // occasionally one may have a zero in the last p(r) position (?) not
          // for I(q)?
          if (I.size () > q.size ())
            {
              I.resize (q.size ());
          ...
          if (!(create_avg && only_plot_stats) && !run_nnls && !run_best_fit)
            {
                          ...
                          if (!rescaling_warning_shown)
                            {
                              QMessageBox::warning (
                                  this, "UltraScan",
                                  QString (us_tr (
                                      "Standard deviation curves require an "
                                      "Average curve to be plotted first for "
                                      "correct rescaling\n"
                                      "Since an Average curve was not plotted "
                                      "first, they will simply be scaled as a "
                                      "best fit to the selected target\n")));
              ...
              if (I_errors.size ())
                {
                  plot_one_iqq (this_q, I, I_errors,
                                QFileInfo (filename).fileName () + " "
                                    + qsl_tmp[0]);
                }
              else
                {
                  plot_one_iqq (this_q, I,
                                QFileInfo (filename).fileName () + " "
                                    + qsl_tmp[0]);
                }
            }
  ...
  if (create_avg && sum_count && !run_nnls && !run_best_fit)
    {
      ...
      if (create_std_dev && sum_count > 2)
        {
          ...
          for (unsigned int i = 0; i < sum_iq.size (); i++)
            {
              ...
              std_dev[i] = tmp_std_dev > 0e0
                               ? sqrt ((1e0 / ((double)sum_count - 1e0))
                                       * tmp_std_dev)
                               : 0e0;
              if (us_isnan (sum_iq[i]))
                {
                  TSO << QString ("WARNING when calc'ing std dev: isnan "
                                  "sum_iq[%1]\n")
                             .arg (i);
                }
              if (us_isnan (sum_iq2[i]))
              ...
              if (us_isnan (std_dev[i]))
                {
                  TSO << QString ("WARNING when calc'ing std dev: isnan "
                                  ...
                                  " sum_iq2[%4] == %5\n"
                                  " sum_count   == %6\n"
                                  " sum_iq2 - ((sum_iq * sum_iq) / sum_count) "
                                  "  == %7\n")
                             .arg (i)
                             .arg (i)
                             .arg (sum_iq[i])
          ...
          plot_one_iqq (this_q, iq_avg, iq_std_dev,
                        QFileInfo (filename).fileName () + " Average");
      ...
      if (save_to_csv)
        {
          // TSO << "save_to_csv\n";
          QString fname = ((US_Hydrodyn *)us_hydrodyn)->somo_dir + SLASH
                          + "saxs" + SLASH + csv_filename + "_iqq.csv";
          ...
          if (of)
            ...
            {
              editor_msg ("red",
                          us_tr ("ERROR creating file: " + fname + "\n"));
            }
        }
    }
  else
      ...
      if (run_nnls || run_best_fit)
        {
          ...
          if (found_nnls_model && found_nnls_target)
                  ...
                  if (our_saxs_options->iqq_scale_minq
                      || our_saxs_options->iqq_scale_maxq)
                    {
                      editor_msg (
                          "dark red",
                          QString (
                              us_tr ("Note: the NNLS fit will be performed "
                                     "over a cropped range q(%1:%2)"))
                              .arg (our_saxs_options->iqq_scale_minq
                                        ? QString ("%1").arg (
                                              our_saxs_options->iqq_scale_minq)
                                        : "")
                              .arg (our_saxs_options->iqq_scale_maxq
                                        ? QString ("%1").arg (
                                              our_saxs_options->iqq_scale_maxq)
                                        : ""));
                      nnls_csv_footer
                          << QString (
                                 us_tr (
                                     "\"Note: the NNLS fit will be performed "
                                     "over a cropped range q\",%1,%2"))
                                 .arg (our_saxs_options->iqq_scale_minq
                                           ? QString ("%1").arg (
                                                 our_saxs_options
                      ...
                      if (QFile::exists (nnls_csv_filename))
                        {
                          nnls_csv_filename
                              = ((US_Hydrodyn *)us_hydrodyn)
                                    ->fileNameCheck (nnls_csv_filename, 0,
                                                     this);
                        }
                      ...
                      if (f.open (QIODevice::WriteOnly))
                      ...
                      else
                        {
                          editor_msg (
                              ...
                              QString (
                                  us_tr (
                                      "Error attempting to create file: %1\n"))
                                  .arg (nnls_csv_filename));
                        }
                    }
                }
              if (run_best_fit)
                {
                  if (our_saxs_options->iqq_scale_minq
                      || our_saxs_options->iqq_scale_maxq)
                    {
                      editor_msg (
                          "dark red",
                          QString (us_tr ("Note: the fit will be performed "
                                          "over a cropped range q(%1:%2)"))
                              .arg (our_saxs_options->iqq_scale_minq
                                        ? QString ("%1").arg (
                                              our_saxs_options->iqq_scale_minq)
            ...
            {
              editor->append ("NNLS error: could not find target and models "
                              "in loaded data\n");
            }
        }
      if (plotted)
...
bool
US_Hydrodyn_Saxs::load_check_csvs_compatible (QStringList filenames)
{
  ...
  for (int i = 0; i < filenames.size (); ++i)
    {
      ...
      if (!US_File_Util::getcontents (f, contents, error))
        {
          qDebug () << "load_check_csvs_compatible failed to get contents "
                    << error;
          return false;
        }
      ...
      if (!grid.size ())
        {
          qDebug ()
              << "load_check_csvs_compatible failed first line empty grid "
              << f;
          return false;
        }
      grid.removeLast ();
      grid.removeLast ();
      // qDebug() << QString( "file: %1 gridsize: %2" ).arg( f ).arg(
      // grid.size() );
      if (!refgrid.size ())
        {
          refgrid = grid;
        ...
        {
          int mingrid = refgrid.size () > grid.size () ? grid.size ()
                                                       : refgrid.size ();
          for (int j = 0; j < mingrid; ++j)
            {
              if (grid[j] != refgrid[j])
                {
                  qDebug () << "load_check_csvs_compatible failed "
                               "incompatible grids "
                            << f;
                  return false;
                }
...
QString
US_Hydrodyn_Saxs::unify_csv_files (QStringList filenames)
{
          ...
          for (int j = 0; j < mingrid; ++j)
            {
              if (grid[j] != refgrid[j])
                {
                  qDebug ()
                      << "unify_csv_files failed incompatible grids " << f;
                  return "error";
                }
            }
          if (refgrid.size () < grid.size ())
  ...
  // now we have an appropriate refgrid, we can create a temp file & pad as in
  // checkjscv.pl

  QString header = refgrid.join (",")
                   + ",,\"SOMO SAXS P(r) vs r data generated by unification "
      ...
      for (int j = 0; j < lines.size (); ++j)
        {
          ...
          if (line.contains ("P(r) normed"))
            {
              ...
              if (names_used.count (fields[0]))
                {
                  editor_msg ("red",
                              QString ("unified csv load - duplicate name "
                                       "exists, %1, dropping this data\n")
                                  .arg (fields[0]));
                  continue;
                }
          ...
          if (fields.size () > refgrid.size ())
            {
              qDebug () << "unify_csv_files failed line " << j
                        << " has more fields than header " << f;
              QTextStream (stdout) << "header: " << refgrid.size () << "\n"
                                   << refgrid.join (",") << "\n"
                                   << "line  : " << fields.size () << "\n"
                                   << fields.join (",") << "\n";
...

  {
    ...
    if (!US_File_Util::writeuniq (outname, msg,
                                  ((US_Hydrodyn *)us_hydrodyn)->somo_tmp_dir
...
void
US_Hydrodyn_Saxs::load_saxs (QString filename, bool just_plotted_curves,
                             QString scaleto, bool no_scaling)
{
  if (just_plotted_curves)
    {
      load_iqq_csv ("", true);
  ...
  if (filename.isEmpty ())
    {
      QString use_dir = our_saxs_options->path_load_saxs_curve.isEmpty ()
                            ? USglobal->config_list.root_dir + SLASH + "somo"
                                  + SLASH + "saxs"
                            : our_saxs_options->path_load_saxs_curve;
      select_from_directory_history (use_dir, this);

      QStringList filenames
          = QFileDialog::getOpenFileNames (this, "Open", use_dir,
                                           "All files (*);;"
                                           "ssaxs files (*.ssaxs);;"
                                           "csv files (*.csv);;"
      ...
      if (filenames.size () == 1)
      ...
      else
        {
          ...
          if (filenames.size ()
              == filenames.filter (QRegExp (".csv$", Qt::CaseInsensitive))
                     .size ())
            {
              qDebug () << "all csv\n";
  ...
  if (ext == "pdb" || ext == "PDB")
    {
      QMessageBox::warning (
          this, "UltraScan",
          QString (us_tr ("Can not load a PDB file as a curve: ") + filename));
      return;
    }
  ...
  if (f.open (QIODevice::ReadOnly))
    {
      ...
      if (!qv.size ())
        {
          QMessageBox::warning (
              this, "UltraScan",
              QString (us_tr ("The file ") + filename + us_tr (" is empty.")));
          return;
        }
      ...
      if (qv.size () > 3)
        {
          ...
          QStringList test_list
              = (test_line).split (QRegExp ("\\s+"), Qt::SkipEmptyParts);
              ...
              if (ok)
                {
                  ...
                  if (!Icolumn)
                    {
                      cerr
                          << "US_Hydrodyn_Saxs::load_saxs : unknown type error"
                          << endl;
                      return;
                    }
          ...
          if (qsl.filter ("exp_intensity").size ())
            {
              ...
              switch (QMessageBox::question (
                  ...
                  QString (us_tr ("Please note:\n\n"
                                  "The file appears to have both experiment "
                                  "and model data\n"
                                  "What would you like to do?\n")),
                  us_tr ("&Load only experimental"),
                  us_tr ("&Load only the model"), us_tr ("&Load both"),
                  2, // Default
                  0  // Escape == button 0
                  ))
                {
          ...
          if (number_of_fields >= 4)
            {
              ...
              lst << "I(q)   Difference intensity"
                  ...
                  << "Ic(q)  Shape scattering";
              bool ok;
              res = US_Static::getItem ("There are three available datasets",
                                        "Select the set you wish to plot::",
                                        lst, 0, false, &ok, this);
              if (ok)
                {
                  // user selected an item and pressed OK
      ...
      editor->append (
          QString ("Loading SAXS data from %1 %2\n").arg (filename).arg (res));

      int startline;
      if (QRegExp ("^\\s*(\\d|.|e|E|+|-)+\\s*(\\d|.|e|E|+|-)+")
              .indexIn (qv[0]))
        {
          startline = 0;
        }
          ...
          if ((unsigned int)tokens.size () > Icolumn)
            {
              ...
              if (I_errorcolumn
                  && (unsigned int)tokens.size () > I_errorcolumn)
                {
      ...
      TSO << "q_range after load: " << q[0] << " , " << q[q.size () - 1]
          << endl;
      ...
      // TSO << "q_range after crop: " << q[0] << " , " << q[q.size() - 1] <<
      // endl;

      if (q.size () && !scaling_target.isEmpty ()
          && plotted_iq_names_to_pos.count (scaling_target))
        {
          if (Icolumn2)
            {
      ...
      if (q.size ())
        {
          if (I_error.size ())
            {
              editor_msg (our_saxs_options->iqq_expt_data_contains_variances
                              ? "red"
                              : "dark blue",
                          our_saxs_options->iqq_expt_data_contains_variances
                              ? "Loaded variance data\n"
                              : "Loaded standard deviation data\n");
              plot_one_iqq (q, I, I_error,
                            QFileInfo (filename).fileName () + tag1);
            }
          else
            {
          ...
          ((US_Hydrodyn *)us_hydrodyn)->last_saxs_header
              = QString ("").sprintf ("Saxs curves from %s",
...
static QString
assoc_sd (QString &qs, const QStringList &qsl_sd)
{
  ...
  QStringList qsl = qsl_sd.filter (QRegularExpression (
      QString ("^%1,").arg (QRegularExpression::escape (name))));
  if (!qsl.size ())
    {
...
void
US_Hydrodyn_Saxs::load_pr (bool just_plotted_curves, QString load_this,
                           bool skip_mw)
{
  if (just_plotted_curves && !qsl_plotted_pr_names.size ())
    {
      QMessageBox::warning (this, "UltraScan",
                            QString (us_tr ("There is nothing plotted!")));
      return;
  ...
  if (load_this.isEmpty ())
    {
      ...
      if (!just_plotted_curves)
        {
          filenames = QFileDialog::getOpenFileNames (this, "Open", use_dir,
                                                     "All files (*);;"
                                                     "sprr files (*.sprr_?);;"
                                                     "csv files (*.csv)",
  ...
  if (filenames.size () > 1
      && filenames.filter (QRegExp (".csv$", Qt::CaseInsensitive)).size ()
             != filenames.size ())
    {
      QMessageBox::information (
          this, "UltraScan",
          us_tr (
              "Multiple file load is currently only supported for csv files"));
      return;
    }

  if (filenames.size () > 1
      && filenames.filter (QRegExp ("_t(|-\\d+).csv$", Qt::CaseInsensitive))
             .size ())
    {
      QMessageBox::information (
          this, "UltraScan",
          us_tr ("Can not load transposed format csv files:\n")
              + filenames
                    .filter (QRegExp ("_t(|-\\d+).csv$", Qt::CaseInsensitive))
                    .join ("\n"));
      return;
    }
  ...
  if (just_plotted_curves || f.open (QIODevice::ReadOnly))
    {
      ...
      if (ext == "csv" || just_plotted_curves)
        {
          ...
          QStringList qsl;
          QStringList
              qsl_data_lines_plotted; // for potential later average save
          if (!just_plotted_curves)
            {
              QTextStream ts (&f);
              while (!ts.atEnd ())
              ...
              for (unsigned int i = 0; i < (unsigned int)qsl.size (); i++)
                {
                  ...
                  // if ( tmp_qsl.size() > 1 )
                  // {
                  //      TSO << QString("line %1 field 1 is
                  //      <%2>\n").arg(i).arg(tmp_qsl[1]);
                  // } else {
                  // TSO << QString("line %1 size not greater than 1 value
                  // <%2>\n").arg(i).arg(qsl[i]);
                  // }
                  if (tmp_qsl.size () > 1 && rx.indexIn (tmp_qsl[1]) != -1)
                    {
                      // TSO << "trying to fix\n";
                      QStringList tmp2_qsl;
                      ...
                      if (tmp_qsl[0] == "\"Name\"")
                        ...
                        {
                          tmp2_qsl.push_back (
                              QString ("%1").arg (get_mw (tmp_qsl[0], false)));
                          double tmp_area = 0e0;
                          for (unsigned int j = 1;
                               j < (unsigned int)tmp_qsl.size (); j++)
                            {
                              tmp_area += tmp_qsl[j].toDouble ();
                            }
                          tmp2_qsl.push_back (
                              QString ("%1").arg (tmp_area * delta_r));
                        }
                      for (unsigned int j = 1;
                           j < (unsigned int)tmp_qsl.size (); j++)
                        {
                  ...
                  else
                    {
                      ...
                      // simply push back blank lines or lines with only one
                      // entry
          ...
          if (qsl_headers.size () > 1)
            {
              QString ref = qsl_headers[0];
              for (unsigned int i = 1; i < (unsigned int)qsl_headers.size ();
                   i++)
                {
                  if (ref != qsl_headers[i])
                    {
          ...
          if (!just_plotted_curves)
              ...
              if (qsl_r.size () < 6)
                {
                  // QMessageBox mb(us_tr("UltraScan Warning"),
                  //                us_tr("The csv file ") + filename + us_tr("
                  //                does not appear to contain any r values in
                  //                the header rows.\n"),
                  //                QMessageBox::Critical,
                  //             QMessageBox::NoButton, QMessageBox::NoButton,
                  //             QMessageBox::NoButton, 0, 0, 1);
                  ...
                  QMessageBox::critical (
                      this, us_tr ("UltraScan Warning"),
                      us_tr ("The csv file ") + filename
                          + us_tr (" does not appear to contain any r values "
                                   "in the header rows.\n"),
                      QMessageBox::NoButton, QMessageBox::NoButton,
                      QMessageBox::NoButton);
                  return;
          ...
          else
            {
              ...
              QString header
                  = QString ("\"Name\",\"MW (Daltons)\",\"Area\",\"Type; "
                             "r:\",%1,%2\n")
          ...
          if (filenames.size () > 1)
            {
              ...
              for (unsigned int i = 1; i < (unsigned int)filenames.size ();
                   i++)
                {
                  TSO << QString ("trying file %1 %2\n")
                             .arg (i)
                  ...
                  f2.close ();
                  QStringList qsl2_headers
                      = qsl2.filter ("\"Name\",\"Type; r:\"");
                  ...
                  if (qsl2_headers.size () != 0)
                    {
                      TSO << "found old csv format, upgrading\n";
                      // upgrade the csv format
                      QRegExp rx ("^\"(Type; r:|P\\(r\\)|P\\(r\\) "
                                  "normed|P\\(r\\) sd|P\\(r\\) normed sd)\"$");
                      QStringList new_qsl;
                      double delta_r = 0e0;
                      for (unsigned int i = 0; i < (unsigned int)qsl2.size ();
                           i++)
                        {
                          QStringList tmp2_qsl;
                          ...
                          if (tmp_qsl.size () > 1
                              && rx.indexIn (tmp_qsl[1]) != -1)
                            {
                              ...
                              if (tmp_qsl[0] == "\"Name\"")
                                ...
                                {
                                  tmp2_qsl.push_back (QString ("%1").arg (
                                      get_mw (tmp_qsl[0], false)));
                                  double tmp_area = 0e0;
                                  for (unsigned int j = 1;
                                       j < (unsigned int)tmp_qsl.size (); j++)
                                    {
                            ...
                            {
                              // TSO << "skipped this line, regexp or
                              // length\n"; simply push back blank lines or
                              // lines with only one entry
                              new_qsl.push_back (qsl[i]);
                            }
                        }
                      // TSO << "orginal csv:\n" << qsl.join("\n") << endl;
                  ...
                  qsl2_headers = qsl2.filter (
                      "\"Name\",\"MW (Daltons)\",\"Area\",\"Type; r:\"");
                  if (qsl2_headers.size () == 0)
                    {
                      // QMessageBox mb(us_tr("UltraScan Warning"),
                      //                us_tr("The csv file ") + f2.fileName()
                      //                + us_tr(" does not appear to contain a
                      //                correct header.\nPlease manually
                  ...
                  if (qsl2_headers.size () > 1)
                    {
                      ...
                      for (unsigned int i = 1;
                           i < (unsigned int)qsl2_headers.size (); i++)
                        {
                          if (ref != qsl2_headers[i])
                            {
                              // QMessageBox mb(us_tr("UltraScan Warning"),
                              //                us_tr("The csv file ") +
                              //                filename + us_tr(" contains
                              //                multiple different
                              //                headers\nPlease manually
                              //                correct the csv file."),
                              //                QMessageBox::Critical,
                  ...
                  if (qsl_data.size () == 0)
                    {
                      // QMessageBox mb(us_tr("UltraScan Warning"),
                      //                us_tr("The csv file ") + filename +
                      //                us_tr(" does not appear to contain any
                      //                data rows.\n"), QMessageBox::Critical,
                      //                QMessageBox::NoButton,
                      //                QMessageBox::NoButton,
                      //                QMessageBox::NoButton, 0, 0, 1);
                      ...
                      QMessageBox::critical (
                          this, us_tr ("UltraScan Warning"),
                          us_tr ("The csv file ") + filename
                              + us_tr (" does not appear to contain any data "
                                       "rows.\n"),
                          QMessageBox::NoButton, QMessageBox::NoButton,
                          QMessageBox::NoButton);
                      return;
                  ...
                  // qDebug() << QString( "qsl_sd.size() %1" ).arg(
                  // qsl_sd.size() );
                  ...
                  if (qsl_r.size () < 6)
                    {
                      ...
                      //                us_tr("The csv file ") + f2.fileName()
                      //                + us_tr(" does not appear to contain
                      //                any r values in the header rows.\n"),
                      //                QMessageBox::Critical,
                      //                QMessageBox::NoButton,
                      //                QMessageBox::NoButton,
                      //                QMessageBox::NoButton, 0, 0, 1);
                      ...
                      QMessageBox::critical (
                          this, us_tr ("UltraScan Warning"),
                          us_tr ("The csv file ") + f2.fileName ()
                              + us_tr (" does not appear to contain any r "
                                       "values in the header rows.\n"),
                          QMessageBox::NoButton, QMessageBox::NoButton,
                          QMessageBox::NoButton);
                      return;
                    }
                      ...
                      if (qsl_pr.size () < 6)
                        {
                          QString msg
                              = us_tr ("The csv file ") + f2.fileName ()
                                + us_tr (" does not appear to contain "
                                         "sufficient p(r) values in data row "
                                         + qsl_pr[0] + ", skipping\n");
                          editor_msg ("red", msg);
                        }
                      else
                          ...
                          if (all_match)
                            {
                              ...
                              // endl;
                              {
                                ...
                                for (int i = 0;
                                     i < 8 && i < (int)new_qsl.size (); ++i)
                                  {
                                    qsl_pr_8 << new_qsl[i];
                                  }
                                source_filenames[qsl_pr_8.join (",")]
                          ...
                          else
                            {
                              ...
                              // interpolate r2, pr to r, reappend to
                              // new_pr_fields
                              vector<double> npr;
                              bool ok = true;
                              if (!interpolate (r, r2, this_pr, npr))
                                {
                                  editor_msg (
                                      "red",
                                      QString (us_tr ("Interpolation error on "
                                                      "curve in %1 "))
                                          .arg (f2.fileName ()));
                                  ok = false;
                                }
                              ...
                              if (ok)
                                {
                                  QString qs_assoc_sd
                                      = assoc_sd (new_pr_fields[0], qsl_sd);
                                  // qDebug() << "spec qs_assoc_sd " <<
                                  // qs_assoc_sd;
                                      ...
                                      if (qsl_pr_error.size () > 4)
                                        {
                                            ...
                                            }
                                          {
                                            QStringList::iterator it2
                                                = qsl_pr_error.begin ();
                                            it2 += 4;
                                            for (; it2 != qsl_pr_error.end ();
                                                 it2++)
                                              {
                                                this_pr_error.push_back (
                                                    (*it2).toDouble ());
                                              }
                                          }
          ...
          QString bin_msg = "";
          for (unsigned int i = 0;
               i < (unsigned int)qsl_plotted_pr_names.size (); i++)
            {
              vector<double> npr = interpolate (r, plotted_r[i],
                                                plotted_pr_not_normalized[i]);
              vector<double> npr_error;
              if (plotted_pr_not_normalized_error[i].size ())
                {
                  // US_Vector::printvector3( "errors exist r, plotted_r[i],
                  // plotted_pr_not_nomalized_error[i]", r, plotted_pr[i],
                  // plotted_pr_not_normalized_error[i] );
                  npr_error = interpolate (r, plotted_r[i],
                                           plotted_pr_not_normalized_error[i]);
                }
              if (!added_interpolate_msg && r.size () > 1)
                {
                  bin_msg
                      = QString (
                            us_tr (
                                "Plotted P(r) interpolated to bin size of %1"))
                            .arg (r[1] - r[0]);
                  if (plotted_r[i].size () > 1
                      && r[1] - r[0] != plotted_r[i][1] - plotted_r[i][0])
                    {
                      bin_msg += QString (us_tr (" which is DIFFERENT from "
                                                 "the plotted bin size of %1"))
              ...
              QString line
                  = QString ("\"%1\",%2,%3,\"P(r)\",%4\n")
                        .arg (qsl_plotted_pr_names[i].replace (
                            QRegExp ("(^[^\\\"]*\\\"|\\\"[^\\\"]*$)"), ""))
                        .arg (plotted_pr_mw[i])
              ...
              if (new_qsl.size () != qsl.size ())
                {
                  switch (QMessageBox::information (
                      this, us_tr ("UltraScan"),
                      us_tr ("There are multiple average and/or standard "
                             "deviation lines\n")
                          + us_tr ("What do you want to do?"),
                      us_tr ("&Skip"), us_tr ("&Just averages"),
                      us_tr ("&Include"),
                      0, // Enter == button 0
                      1))
                    {       // Escape == button 2
          ...
          US_Hydrodyn_Saxs_Load_Csv *hslc = new US_Hydrodyn_Saxs_Load_Csv (
              "Select models to load\n" + header_tag
                  + (bin_msg.isEmpty () ? "" : "\n" + bin_msg),
              &qsl_names, &qsl_sel_names, &qsl, filename, &create_avg,
              &create_std_dev, &only_plot_stats, &save_to_csv, &csv_filename,
              &save_original_data, &run_nnls, &nnls_plot_contrib, &nnls_csv,
              &run_best_fit, &use_SDs_for_fitting_prr, &nnls_target,
              &clear_plot_first, 1 || U_EXPT, us_hydrodyn);
          US_Hydrodyn::fixWinButtons (hslc);
          hslc->exec ();
          ...
          if (run_nnls || run_best_fit)
            {
              ...
              if (nnls_csv)
                {
                  QString use_dir = USglobal->config_list.root_dir + "/"
                                    + "somo" + "/" + "saxs";

                  // ((US_Hydrodyn
                  // *)us_hydrodyn)->select_from_directory_history( use_dir,
                  // this );
          ...
          // now go through qsl_data and load up any that map_sel_names
          // contains
          plotted = false;
          for (QStringList::iterator it = qsl_data.begin ();
               it != qsl_data.end (); it++)
              ...
              if (map_sel_names.count (qsl_tmp[0]))
                {
                  ...
                  // 1st check the line to see if we have a mw
                  {
                    if (qsl_tmp[1].toFloat () > 0e0)
                      {
                        (*remember_mw)[QFileInfo (qsl_tmp[0]).fileName ()]
                            = qsl_tmp[1].toFloat ();
                        (*remember_mw_source)[QFileInfo (qsl_tmp[0])
                                                  .fileName ()]
                      ...
                      {
                        if ((*remember_mw)
                                .count (QFileInfo (qsl_tmp[0]).fileName ()))
                          {
                            (*remember_mw)
                                .erase (QFileInfo (qsl_tmp[0]).fileName ());
                            (*remember_mw_source)
                                .erase (QFileInfo (qsl_tmp[0]).fileName ());
                          }
                      }
                      ...
                      if (pr_error.size () != pr.size ())
                        {
                          editor_msg (
                              "red",
                              QString (
                                  us_tr ("S.D.s found in %1, but they are not "
                                         "of the same length (%2) as the P(r) "
                                         "data points (%3), dropping S.D.s"))
                                  .arg (qsl_pr[0])
                                  .arg (pr_error.size ())
                                  .arg (pr.size ()));
                          // US_Vector::printvector3( "pr sd mismatch, r, pr,
                          // pr_error:", r, pr, pr_error );
                          pr_error.clear ();
                        }
                    }
                  ...
                  // r has the ordinates for the longest data, some will likely
                  // be shorter
                  if (r.size () > pr.size ())
                    {
                  ...
                  if (pr_error.size () && pr_error.size () != pr.size ())
                    {
                      editor_msg (
                          "red",
                          QString (
                              us_tr ("S.D.s found in %1, but they are not of "
                                     "the same length (%2) as the P(r) data "
                                     "points (%3), dropping S.D.s"))
                              .arg (qsl_pr[0])
                              .arg (pr_error.size ())
                              .arg (pr.size ()));
                      // US_Vector::printvector3( "pr sd mismatch, r, pr,
                  ...
                  if (!(create_avg && only_plot_stats) && !run_nnls
                      && !run_best_fit)
                    {
                      ...
                      (*remember_mw_source)[use_filename + " " + qsl_tmp[0]]
                          = "loaded from csv";
                      plot_one_pr (this_r, pr, pr_error,
                                   use_filename + " " + qsl_tmp[0], skip_mw);
                      compute_rg_to_progress (this_r, pr,
                                              use_filename + " " + qsl_tmp[0]);
                    }
                }
            }
          if (pr_mws.size ())
          ...
          (*remember_mw)[QFileInfo (filename).fileName () + " Average"]
              = pr_mw_avg;
          (*remember_mw_source)[QFileInfo (filename).fileName () + " Average"]
              = "computed average from selected csv models";
          (*remember_mw)["Standard deviation"] = pr_mw_avg;
          (*remember_mw_source)["Standard deviation"]
              = "computed average from selected csv models";
          (*remember_mw)[QFileInfo (filename).fileName ()
                         + " Average minus 1 std dev"]
              = pr_mw_avg;
          (*remember_mw_source)[QFileInfo (filename).fileName ()
                                + " Average minus 1 std dev"]
              = "computed average from selected csv models";
          (*remember_mw)[QFileInfo (filename).fileName ()
          ...
          (*remember_mw_source)["Average"]
              = "computed average from selected csv models";
          (*remember_mw)["Standard deviation"] = pr_mw_avg;
          (*remember_mw_source)["Standard deviation"]
              = "computed average from selected csv models";
          (*remember_mw)["Average minus 1 standard deviation"] = pr_mw_avg;
          (*remember_mw_source)["Average minus 1 standard deviation"]
              = "computed average from selected csv models";
          (*remember_mw)["Average plus 1 standard deviation"] = pr_mw_avg;
          (*remember_mw_source)["Average plus 1 standard deviation"]
              = "computed average from selected csv models";

          (*remember_mw)[csv_filename + " Model"] = pr_mw_avg;
          (*remember_mw_source)[csv_filename + " Model"]
              = "computed average from selected csv models";
          (*remember_mw)[csv_filename + " Residual"] = pr_mw_avg;
          (*remember_mw_source)[csv_filename + " Residual"]
              = "computed average from selected csv models";
          (*remember_mw)[csv_filename + " Target"] = pr_mw_avg;
          (*remember_mw_source)[csv_filename + " Target"]
              = "computed average from selected csv models";
          ...
          if (create_avg && sum_count && !run_nnls && !run_best_fit)
            {
              ...
              plot_one_pr (this_r, pr, no_error,
                           QFileInfo (filename).fileName () + " Average",
                           skip_mw);
              compute_rg_to_progress (
                  this_r, pr, QFileInfo (filename).fileName () + " Average");
                  ...
                  for (unsigned int i = 0; i < sum_pr.size (); i++)
                    {
                      ...
                      if (us_isnan (sum_pr[i]))
                        {
                          TSO << QString ("WARNING when calc'ing std dev: "
                                          "isnan sum_pr[%1]\n")
                                     .arg (i);
                        }
                      if (us_isnan (sum_pr2[i]))
              ...
              if (save_to_csv)
                {
                  // TSO << "save_to_csv\n";
                  QString fname
                      = ((US_Hydrodyn *)us_hydrodyn)->somo_dir + SLASH + "saxs"
                        + SLASH + csv_filename + "_sprr_"
                        + ((US_Hydrodyn *)us_hydrodyn)->saxs_sans_ext ()
                        + ".csv";
                  if (QFile::exists (fname))
                    // && !((US_Hydrodyn *)us_hydrodyn)->overwrite )
                    {
                  ...
                  if (of)
                    {
                      ...
                      fprintf (
                          of, "\"%s\",%.2f,%.2f,\"%s\",%s\n", "Average",
                          pr_mw_avg, compute_pr_area (pr_avg, r), "P(r)",
                          vector_double_to_csv (pr_avg).toLatin1 ().data ());
                      if (pr_std_dev.size ())
                        {
                          fprintf (of, "\"%s\",%.2f,%.2f,\"%s\",%s\n",
                                   "Standard deviation", pr_mw_std_dev,
                                   compute_pr_area (pr_std_dev, r), "P(r)",
                                   vector_double_to_csv (pr_std_dev)
                                       .toLatin1 ()
                                       .data ());
                          fprintf (of, "\"%s\",%.2f,%.2f,\"%s\",%s\n",
                                   "Average minus 1 standard deviation",
              ...
              if (run_nnls || run_best_fit)
                {
                  ...
                  if (found_nnls_model && found_nnls_target)
                    {
                      nnls_header_tag = header_tag;
                      QString use_csv_filename
                          = save_to_csv ? csv_filename : "";
                      (*remember_mw)[use_csv_filename + " Target"]
                          = (*remember_mw)[nnls_target];
                      (*remember_mw_source)[use_csv_filename + " Target"]
                          = "copied from target curve";
                      (*remember_mw)[use_csv_filename + " Model"]
                          = (*remember_mw)[nnls_target];
                      (*remember_mw_source)[use_csv_filename + " Model"]
                          = "copied from target curve";
                      if (run_nnls)
                        {

                          {
                            QString rg_msg;
                            {
                              double Rg;
                              QString errormsg;
                              if (US_Saxs_Util::compute_rg_from_pr (
                                      nnls_r, nnls_B, Rg, errormsg))
                                {
                          ...
                          if (nnls_csv)
                            {
                              if (QFile::exists (nnls_csv_filename))
                                {
                                  nnls_csv_filename
                                      = ((US_Hydrodyn *)us_hydrodyn)
                                            ->fileNameCheck (nnls_csv_filename,
                                                             0, this);
                                }
                              ...
                              if (f.open (QIODevice::WriteOnly))
                              ...
                              else
                                {
                                  editor_msg (
                                      "red",
                                      QString (us_tr ("Error attempting to "
                                                      "create file: %1\n"))
                                          .arg (nnls_csv_filename));
                                }
                            }
                    ...
                    {
                      editor->append ("NNLS error: could not find target and "
                                      "models in loaded data\n");
                    }
                }
              if (plotted)
                {
      ...
      if (!ext.contains (QRegExp ("^sprr(|_(x|n|r))")))
        {
          ...
          QString tmp;
          unsigned int pos = 0;
          QRegExp qx_mw ("molecular weight (\\d+(|\\.\\d+))",
                         Qt::CaseInsensitive);
              ...
              if (qx_mw.indexIn (tmp) != -1)
                {
                  ...
                  TSO << QString ("load pr found gnom mw %1\n").arg (mw);
                  (*remember_mw)[QFileInfo (filename).fileName ()] = mw;
                  (*remember_mw_source)[QFileInfo (filename).fileName ()]
                      = "Found in gnom.out file";
                }
              pos++;
      ...
      while (!ts.atEnd ())
        {
          // QString     qs  = ts.readLine();
          QStringList qsl
              = ts.readLine ().trimmed ().split (QRegularExpression ("\\s+"));
          ...
          if (qsl.size () < 2)
            {
              editor_msg (
                  "black",
                  QString (
                      us_tr ("Notice: File %1 : improper format line found "
                             "after r = %2, further data trimmed\n"))
                      .arg (QFileInfo (filename).fileName ())
                      .arg (r.back ()));
              break;
            }

          // qDebug() << QString( "load_pr() qsl size %1 %2" ).arg( qsl.size()
          // ).arg( qsl.join( "," ) );
      ...
      f.close ();
      // US_Vector::printvector3( "pr load 0 r, pr, pr_error", r, pr, pr_error,
      // 6 );
      ...
      if (pr_error.size () && pr.size () != pr_error.size ())
        {
          ...
          editor_msg (
              "darkRed",
              QString (
                  us_tr ("File %1 : some, but not all, data had apparent "
                         "associated error values, all error values removed"))
                  .arg (QFileInfo (filename).fileName ()));
        }
      ...
      if (mw)
        {
          ...
          (*remember_mw_source)[use_filename]
              = (*remember_mw_source)[QFileInfo (filename).fileName ()];
        }
...
void
US_Hydrodyn_Saxs::rescale_iqq_curve (QString scaling_target, vector<double> &q,
                                     vector<double> &I, bool do_plot_residuals)
{
  vector<double> I2;
...
void
US_Hydrodyn_Saxs::rescale_iqq_curve (QString scaling_target, vector<double> &q,
                                     vector<double> &I, QColor plot_color,
...
void
US_Hydrodyn_Saxs::rescale_iqq_curve (QString scaling_target, vector<double> &q,
                                     vector<double> &I, vector<double> &I2,
                                     bool do_plot_residuals)
{
  QColor plot_color = plot_colors[plotted_q.size () % plot_colors.size ()];
  rescale_iqq_curve (scaling_target, q, I, I2, plot_color, do_plot_residuals);
...
US_Hydrodyn_Saxs::rescale_iqq_curve (QString scaling_target, vector<double> &q,
                                     vector<double> &I,
                                     vector<double> & /* I2 */,
                                     QColor plot_color, bool do_plot_residuals)
{
  if (!q.size () || scaling_target.isEmpty ()
      || !plotted_iq_names_to_pos.count (scaling_target))
    {
  ...
  TSO << QString ("target q_min %1 max %2\n"
  ...
  for (unsigned int i = 0; i < plotted_q[iq_pos].size (); i++)
    {
      if (plotted_q[iq_pos][i] >= q_min && plotted_q[iq_pos][i] <= q_max)
        {
  ...
  if (do_chi2_fitting && is_zero_vector (use_I_error))
    {
      if (use_SDs_for_fitting_iqq)
        {
          editor_msg ("red", us_tr ("Chi^2 fitting requested, but target data "
                                    "has no standard deviation data\n"
  ...
  if (our_saxs_options->iqq_scale_nnls)
    {
      // iqq_scale_nnls is an *experimental" option, not enabled for normal
      // usage
      if (!use_SDs_for_fitting_iqq)
        {
          if (do_plot_residuals)
            {
              editor_msg ("red", us_tr ("Chi^2 fitting is currently not "
                                        "compatable with NNLS scaling\n"));
            }
          do_chi2_fitting = false;
        }
      if (our_saxs_options->iqq_scale_linear_offset)
        {
          if (do_plot_residuals)
            {
              editor_msg ("red", us_tr ("Scale with linear offset is not "
                                        "compatable with NNLS scaling\n"));
            }
          do_scale_linear_offset = false;
        }

      usu.nnls_fit (use_source_I, use_I, k, chi2);
    }
  else
    {
      if (do_plot_residuals)
      ...
      if (do_chi2_fitting && !is_nonzero_vector (use_I_error))
        {
          if (do_plot_residuals)
            {
              editor_msg ("red", us_tr ("Chi^2 fitting disabled, zeros in "
      ...
      if (do_plot_residuals)
        {
          ...
          nnls_csv_footer
              << QString (
                     us_tr ("\"fitting range start, end, points:\",%1,%2,%3"))
                     .arg (use_q[0])
                     .arg (use_q.back ())
                     .arg (use_q.size ());
        }
      ...
      if (do_chi2_fitting)
        {
          usu.scaling_fit (use_source_I, use_I, use_I_error, k, chi2);
        }
      else
        {
          usu.scaling_fit (use_source_I, use_I, k, chi2);
        }
    }
  ...
  if (our_saxs_options->iqq_scale_play)
    {
      ...
      double ournchi
          = do_chi2_fitting
                ? sqrt (chi2
                        / (use_I.size () - (do_scale_linear_offset ? 2 : 1)))
                : chi2;
          ...
          if (ok)
            {
              ...
              if (do_chi2_fitting)
                {
                  US_Saxs_Util::calc_mychi2 (mult_source_I, use_I, use_I_error,
                                             chi2);
                  ournchi = sqrt (
                      chi2
                      / (use_I.size () - (do_scale_linear_offset ? 2 : 1)));
  ...
  if (do_plot_residuals)
    {
      if (do_chi2_fitting)
        {
          // usu.calc_chisq_prob( 0.5 * use_I.size() - ( do_scale_linear_offset
          // ? 2 : 1 ), 0.5 * chi2, chi2_prob );
          fit_msg = QString ("chi^2=%1 df=%2 nchi=%3 nchi^2=%4")
                        .arg (chi2, 6)
      ...
      else
        {
          ...
          nnls_csv_footer << QString ("\"RMSD\",%1").arg (chi2, 5);
        }
      ...
      results += QString ("factor: %1 %2\n").arg (k).arg (fit_msg);

      nnls_csv_footer << QString ("\"Scaling factor\",%1").arg (k);
  ...
  if (do_plot_residuals)
    {
      display_iqq_residuals (scaling_target, use_q, use_I, use_source_I,
                             plot_color, use_I_error);
    }
...
void
US_Hydrodyn_Saxs::rescale_iqq_curve_using_last_rescaling (vector<double> &I,
                                                          bool use_offset)
{
  for (unsigned int i = 0; i < I.size (); i++)
    {
...
bool
US_Hydrodyn_Saxs::select_from_directory_history (QString &dir, QWidget *parent)
{
  return ((US_Hydrodyn *)us_hydrodyn)
      ->select_from_directory_history (dir, parent);
}

void
US_Hydrodyn_Saxs::add_to_directory_history (QString filename, bool accessed)
{
  ((US_Hydrodyn *)us_hydrodyn)->add_to_directory_history (filename, accessed);
}

vector<double>
US_Hydrodyn_Saxs::range_crop (vector<double> &q, vector<double> &I)
{
  if (!our_saxs_options->iqq_scale_minq && !our_saxs_options->iqq_scale_maxq)
    {
      return I;
    }
  ...
  for (unsigned int i = 0; i < q.size (); i++)
    {
      if ((!our_saxs_options->iqq_scale_minq
           || q[i] >= our_saxs_options->iqq_scale_minq)
          && (!our_saxs_options->iqq_scale_maxq
              || q[i] <= our_saxs_options->iqq_scale_maxq))
        {
...
void
US_Hydrodyn_Saxs::load_sans (QString filename, bool just_plotted_curves)
{
  if (just_plotted_curves)
    {
      load_iqq_csv ("", true);
  ...
  if (filename.isEmpty ())
    {
      ...
      select_from_directory_history (use_dir, this);
      filename
          = QFileDialog::getOpenFileName (this, "Open", use_dir,
                                          "All files (*);;"
                                          "ssans files (*.ssans);;"
                                          "csv files (*.csv);;"
...
void
US_Hydrodyn_Saxs::load_gnom ()
{
  cb_guinier->setChecked (false);
  set_guinier ();
  // map < QString, QString > params;
  // params[ "wild" ] = "10.7";
  // US_SAS_Dammin * usd = new US_SAS_Dammin( (US_Hydrodyn *)us_hydrodyn, 41e0,
  // params ); usd->show(); return;
  ...
  QString filename
      = QFileDialog::getOpenFileName (this, windowTitle (), use_dir, "*.out");
  if (filename.isEmpty ())
    {
      return;
    }
  ...
  if (f.open (QIODevice::ReadOnly))
    {
        ...
        if (mwline.size ())
          {
            ...
            if (qx_mw.indexIn (mwline[0]) == -1)
              {
                TSO << QString ("qx_mw.search of <%1> for molecular weight "
                                "failed!\n")
                           .arg (mwline[0]);
                gnom_mw = 0e0;
              }
                ...
                if (!((US_Hydrodyn *)us_hydrodyn)->gui_script)
                  {
                    US_Static::us_message (
                        us_tr ("Please note:"),
                        QString (
                            us_tr ("There are multiple molecular weight lines "
                                   "in the gnom file\n"
                                   "Using the first one found (%1 Daltons)"))
                            .arg (gnom_mw));
                  }
              }
            if (gnom_mw > 0e0)
                  ...
                  if (rx5.indexIn (tmp) != -1)
                    {
                      ...
                      // cout << "iqq point: " << rx5.cap(1).toDouble() << " "
                      // << rx5.cap(5).toDouble() << endl;
                    }
                  else
                    {
                      // end of iqq?
                      if (rx2.indexIn (tmp) == -1)
                        {
                          ...
                          gnom_q = q;
                          // plot_one_iqq(q, I,
                          // QFileInfo(filename).fileName()); if ( plotted )
                          // {
                          //   editor->setParagraphBackgroundColor (
                          //   editor->paragraphs() - 1, QColor("white") );
                  ...
                  if (rx3.indexIn (tmp) != -1)
                    {
                      ...
                      // cout << "prr point: " << rx3.cap(1).toDouble() << " "
                      // << rx3.cap(2).toDouble() << endl;
                    }
                  else
                    {
                      // end of prr
                      TSO << "end of prr\n";
                      // QRegExp qx_mw("molecular weight (\\d+(|\\.\\d+))",
                      // Qt::CaseInsensitive ); QStringList mwline =
                      // qsl_gnom.filter(qx_mw); if ( mwline.size() )
                      // {
                      //    if ( qx_mw.indexIn(mwline[0]) == -1 )
                      //    {
                      //       TSO << QString("qx_mw.search of <%1> for
                      //       molecular weight failed!\n").arg(mwline[0]);
                      //       gnom_mw = 0e0;
                      //    } else {
                      //       TSO << QString("mwline cap 0 <%1> cap 1
                      ...
                      if (cb_normalize->isChecked ())
                        {
                          normalize_pr (r, &pr, &pre,
                                        get_mw (filename, false));
                        }

                      // TSO << "load_gnom() plot_one_pr 1\n";
                      plot_one_pr (r, pr, pre,
                                   QFileInfo (filename).fileName ());
                      compute_rg_to_progress (
                          r, pr, QFileInfo (filename).fileName ());
                      ...
                      if (!sprr_saved)
                        {
                          ...
                          QString dest
                              = USglobal->config_list.root_dir + "/somo/saxs/"
                                + QString ("%1")
                                      .arg (QFileInfo (filename).fileName ())
                                      .replace (QRegExp (rxstr), "_gnom.sprr");
                          if (!((US_Hydrodyn *)us_hydrodyn)->overwrite)
                            {
                              dest = ((US_Hydrodyn *)us_hydrodyn)
                                         ->fileNameCheck (dest, 0, this);
                            }
                          ...
                          sprrcontents
                              << QString ("# GNOM P(r) from " + filename
                                          + "%1\nR\tP(r)\tSD\n")
                                     .arg (mw == -1e0
                                               ? QString ("")
                                               : QString (" mw %1 Daltons")
                                                     .arg (mw));
                          ...
                          if (!US_File_Util::putcontents (dest, putcontents,
                                                          error))
                            {
                          ...
                          // save reinterpolated to current bin size sprr
                          {
                            ...
                            for (double x = 0; x < r.back ();
                                 x += our_saxs_options->bin_size)
                              {
                                to_grid.push_back (x);
                              }
                            ...
                            if (!interpolate (to_grid, r, pr, ipr))
                              {
                                editor_msg (
                                    "red",
                                    QString ("error interpolating gnom p(r) "
                                             "grid to current binsize %1\n")
                            ...
                            if (pr.size () != pre.size ())
                              {
                                qDebug () << "gnom load sprr create, data "
                            ...
                            if (ok)
                              {
                                ...
                                // // is this different from iprppre - ipr or
                                // ipr - iprmpre ? answer is no, we could get
                                // away with one less interpolation
                                // {
                                ...
                                //    }
                                //    QTextStream(stdout) <<
                                //    US_Vector::qs_vector4( "to_grid, ipre,
                                //    iprppre-ipr, ipr-iprmpre", to_grid, ipre,
                                //    iprppremipr, iprmiprmpre );
                                // }
                                ...
                                QString dest
                                    = USglobal->config_list.root_dir
                                      + "/somo/saxs/"
                                      + QString ("%1")
                                            .arg (QFileInfo (filename)
                                                      .fileName ())
                                            .replace (
                                                QRegExp (rxstr),
                                                QString ("_gnom_bin%1.sprr")
                                ...
                                QString error;
                                QString putcontents = sprrcontents.join ("");
                                if (!US_File_Util::putcontents (
                                        dest, putcontents, error))
                                  {
                                    editor_msg ("red", error);
                                  }
                                else
                  ...
                  if (!sprr_saved)
                    {
                      ...
                      // save reinterpolated to current bin size sprr
                      {
                        ...
                        if (!natural_spline_interpolate (to_grid, r, pr, ipr))
                          {
                            editor_msg (
                                "red",
                                QString ("error interpolating gnom p(r) grid "
                                         "to current binsize %1\n")
                                    .arg (our_saxs_options->bin_size));
                            ok = false;
                          }
                        ...
                        if (pr.size () != pre.size ())
                          {
                            qDebug () << "gnom load sprr create, data error "
                                         "length mismatch";
                            ok = false;
                          }
                        ...
                        if (ok)
                          {
                            ...
                            if (!natural_spline_interpolate (to_grid, r,
                                                             prppre, iprppre))
                              {
                                editor_msg (
                                    "red",
                        ...
                        if (ok)
                          {
                            ...
                            // // is this different from iprppre - ipr or ipr -
                            // iprmpre ? answer is no, we could get away with
                            // one less interpolation
                            // {
                            //    vector < double > iprppremipr = iprppre;
                            //    vector < double > iprmiprmpre = ipr;
                            ...
                            //       iprmiprmpre[ i ] -= iprmpre[i];
                            //    }
                            //    QTextStream(stdout) << US_Vector::qs_vector4(
                            //    "to_grid, ipre, iprppre-ipr, ipr-iprmpre",
                            //    to_grid, ipre, iprppremipr, iprmiprmpre );
                            // }
                            // now create reinterpolated version
      ...
      TSO << QString ("ask_save_mw_to_gnom %1 gnom_mw %2\n")
                 .arg (ask_save_mw_to_gnom)
                 .arg (gnom_mw);
      if (ask_save_mw_to_gnom && gnom_mw)
        {
          switch (QMessageBox::information (
              this, us_tr ("Save GNOM with Molecular Weight"),
              QString (us_tr ("Do you want to save the molecular weight "
                              "entered (%1 Daltons) into the gnom.out file?"))
                  .arg (gnom_mw),
              "&Ok", "&Cancel", 0,
              0, // Enter == button 0
              1))
      ...
      if (datafiles.size ())
        {
          for (unsigned int i = 0; i < datafiles.size (); i++)
            {
              QString datafile = our_saxs_options->path_load_gnom
                                 + QDir::separator () + datafiles[i];
      ...
      if (plot_gnom)
        {
          ...
          plot_one_iqq (gnom_q, gnom_Iq_exp,
                        QFileInfo (filename).fileName () + " Experimental");
          crop_iq_data (gnom_q_reg, gnom_Iq_reg);
          plot_one_iqq (gnom_q_reg, gnom_Iq_reg,
                        QFileInfo (filename).fileName () + " From P(r)");
          if (plotted)
            {
