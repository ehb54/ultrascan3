...
void
US_Hydrodyn_Pdb_Tool_Sort::setupGUI ()
{
  int minHeight1 = 30;
  ...
  lbl_residuesa = new QLabel (
      us_tr ("Residue list A (chain:residue{-residue}{,..}"), this);
  ...
  le_residuesa->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  lbl_residuesb = new QLabel (
      us_tr ("Residue list B (chain:residue{-residue}{,..}"), this);
  ...
  le_residuesb->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  le_reportcount->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize));
  ...
  cb_order = new QCheckBox (us_tr ("Sort by maximum aggregate distance first"),
                            this);
  ...
  cb_order->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize - 1, QFont::Bold));
  cb_order->setMinimumWidth (QFontMetrics (cb_order->font ()).maxWidth ()
                             * 15);
  ...
  cb_caonly->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize - 1, QFont::Bold));
  cb_caonly->setMinimumWidth (QFontMetrics (cb_caonly->font ()).maxWidth ()
                              * 15);

  cb_caonly->setChecked (parameters->count ("caonly")
                                 && (*parameters)["caonly"] == "true"
                             ? true
                             : false);
...
void
US_Hydrodyn_Pdb_Tool_Sort::cancel ()
{
  close ();
}

void
US_Hydrodyn_Pdb_Tool_Sort::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Pdb_Tool_Sort::closeEvent (QCloseEvent *e)
{
...
void
US_Hydrodyn_Pdb_Tool_Sort::update_residuesa (const QString &)
{
  (*parameters)["residuesa"] = le_residuesa->text ();
}

void
US_Hydrodyn_Pdb_Tool_Sort::update_residuesb (const QString &)
{
  (*parameters)["residuesb"] = le_residuesb->text ();
}

void
US_Hydrodyn_Pdb_Tool_Sort::update_reportcount (const QString &)
{
  (*parameters)["reportcount"] = le_reportcount->text ();
}

void
US_Hydrodyn_Pdb_Tool_Sort::set_order ()
{
  (*parameters)["order"] = cb_order->isChecked () ? "true" : "false";
}

void
US_Hydrodyn_Pdb_Tool_Sort::set_caonly ()
{
