...
void
US_Help::show_help (QString helpFile)
{
  QString help = QDir::toNativeSeparators (USglobal->config_list.help_dir + "/"
                                           + helpFile);

  if (!QFileInfo (help).exists ())
    {
      help
          = QDir::toNativeSeparators (USglobal->config_list.help_dir
...
void
US_Help::show_URL (QString location)
{
  URL = location;
  openBrowser ();
}
...
void
US_Help::show_html_file (QString helpFile)
{
  URL = "file://" + helpFile;
  openBrowser ();
}
...
US_Help::openBrowser ()
{
  // removed since windows (10?) doesn't seem to honor the default browser
  // setting by removing the user can configure their favorite browser in
  // us3_config and it will be initialized in us_config.cpp to the QSettings()
  // value
  // #ifdef WIN32
...
void
US_Help::captureStdout ()
{
}
void
US_Help::captureStderr ()
{
}
