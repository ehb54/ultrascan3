...
void US_Hydrodyn_Dad_Cistarq::setupGUI() {
  ...
  lbl_title =
      new QLabel(parameters->count("ngmode")
  ...
  if (parameters->count("integralbaseline")) {
    lbl_save_sum_bi = new QLabel(
        us_tr("     Warning: If you have changed the starting time/frame since "
              "integral baseline correction, creation of the\n"
              "      sum of peaks curves with integral baseline will differ "
              "due to missing time/frame scattering intensity."),
        this);
  ...
  cb_sd_source->setText(us_tr(
      ...
      "Add SD computed %-wise from the difference between the sum of Gaussians "
      "and the original I(q)"));
  cb_sd_source->setEnabled(true);
  cb_sd_source->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  QLabel *lbl_sd_zeros_found =
      new QLabel(us_tr("If zeros are produced when computing SDs:  "), this);
  ...
  cb_sd_zero_avg_local_sd->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_sd_zero_keep_as_zeros->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_sd_zero_set_to_pt1pct->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  lbl_conc_to_saxs_info1 =
      new QLabel(us_tr("       This implies that all the species that were "
                       "defined as Gaussians contributing to the SAXS signal "
                       "also contribute to the concentration signal."),
                 this);
  ...
  lbl_conc_to_saxs_info1->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));

  lbl_conc_to_saxs_info2 = new QLabel(
      us_tr(
          "       Be aware that this option will result in an apparent mass "
          "artificially approximately constant along each of the deconvoluted "
          "Gaussian peaks,\n"
  ...
  lbl_conc_to_saxs_info2->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));

  lbl_conc_to_saxs_info3 = new QLabel(
      us_tr("       However, the apparent average mass for each peak should be "
            "a closer approximation to the real value when significant band "
            "broadening occurs\n"
  ...
  lbl_conc_to_saxs_info3->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  if (parameters->count("ngmode")) {
  ...
  lbl_conc =
      new QLabel(us_tr("Concentrations will be computed and will be written "
                       "along with PSVs to the output I(q) curves"),
                 this);
  ...
  lbl_gaussian->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  if (parameters->count("ngmode")) {
    lbl_gaussian->hide();
  }

  lbl_conv = new QLabel(us_tr((*parameters).count("uv")
  ...
  if (parameters->count("error")) {
    ...
    cb_makeiq_avg_peaks->setText(
        us_tr("Average resulting I(q) curves by Gaussian, using top % of max. "
              "intensity"));
...
void US_Hydrodyn_Dad_Cistarq::quit() { close(); }

void US_Hydrodyn_Dad_Cistarq::create_ng() {
  (*parameters)["go"] = "true";
  (*parameters)["make_ng"] = "true";
  close();
}

void US_Hydrodyn_Dad_Cistarq::go() {
  (*parameters)["go"] = "true";
...
void US_Hydrodyn_Dad_Cistarq::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/dad_ciq.html");
}

void US_Hydrodyn_Dad_Cistarq::closeEvent(QCloseEvent *e) {
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept();
}

void US_Hydrodyn_Dad_Cistarq::set_add_bl() {
  (*parameters)["add_baseline"] = cb_add_bl->isChecked() ? "true" : "false";
}

void US_Hydrodyn_Dad_Cistarq::set_sd_source() { update_enables(); }

void US_Hydrodyn_Dad_Cistarq::set_normalize() { update_enables(); }

void US_Hydrodyn_Dad_Cistarq::set_I0se() { update_enables(); }

void US_Hydrodyn_Dad_Cistarq::set_save_as_pct_iq() {
...
void US_Hydrodyn_Dad_Cistarq::global() {
...
void US_Hydrodyn_Dad_Cistarq::set_sd_zero_avg_local_sd() {
  (*parameters)["sd_zero_avg_local_sd"] =
      cb_sd_zero_avg_local_sd->isChecked() ? "true" : "false";
...
void US_Hydrodyn_Dad_Cistarq::set_sd_zero_keep_as_zeros() {
  (*parameters)["sd_zero_keep_as_zeros"] =
      cb_sd_zero_keep_as_zeros->isChecked() ? "true" : "false";
...
void US_Hydrodyn_Dad_Cistarq::set_sd_zero_set_to_pt1pct() {
  (*parameters)["sd_zero_set_to_pt1pct"] =
      cb_sd_zero_set_to_pt1pct->isChecked() ? "true" : "false";
...
void US_Hydrodyn_Dad_Cistarq::set_zero_drop_points() {
  (*parameters)["zero_drop_points"] =
      cb_zero_drop_points->isChecked() ? "true" : "false";
...
void US_Hydrodyn_Dad_Cistarq::set_zero_avg_local_sd() {
  (*parameters)["zero_avg_local_sd"] =
      cb_zero_avg_local_sd->isChecked() ? "true" : "false";
...
void US_Hydrodyn_Dad_Cistarq::set_zero_keep_as_zeros() {
  (*parameters)["zero_keep_as_zeros"] =
      cb_zero_keep_as_zeros->isChecked() ? "true" : "false";
...
void US_Hydrodyn_Dad_Cistarq::update_enables() {
  bool no_go = false;
...
void US_Hydrodyn_Dad_Cistarq::ws_hide(vector<QWidget *> ws, bool hide) {
  for (unsigned int i = 0; i < (unsigned int)ws.size(); i++) {
    hide ? ws[i]->hide() : ws[i]->show();
  }
}

void US_Hydrodyn_Dad_Cistarq::zeros_found() {
      ...
      QString(us_tr("Please note:\n\n"
          ...
          .arg(
              (*parameters)["no_errors"].isEmpty()
                  ? ""
                  : QString(
                        us_tr("These files have no associated errors:\n%1\n\n"))
