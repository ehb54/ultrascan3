...
QString
MALS_Angle::list_rich (double lambda, double n)
{
  if (!lambda || !n)
    {
...
bool
MALS_Angle::populate (const QStringList &qsl)
{
...
MALS_Angles::list ()
{
  ...
  for (auto it = mals_angle.begin (); it != mals_angle.end (); ++it)
    {
...
void
MALS_Angles::build_q_to_ri (double lambda, double n)
{
...
QString
MALS_Angles::list_rich (double lambda, double n)
{
  QStringList qsl;
  if (!mals_angle.size ())
    {
  ...
  qsl << QString (us_tr ("<b>MALS Angles</b> loaded from %1<hr>"))
             .arg (loaded_filename);
  if (!lambda || !n)
    {
...
bool
MALS_Angles::populate (const QString &filename, const QStringList &qsl)
{
...
bool
MALS_Angles::load (const QString &filename, const QStringList &csvlines,
                   QString &errormsg)
{
  if (!csvlines.size ())
    {
  ...
  if (!csvlines[0].contains (
          QRegularExpression ("^\\s*\"\\s*US-SOMO MALS Angles")))
    {
  ...
  while (data.size ()
         && data.front ().contains (QRegularExpression ("^\\s*\\d+,")))
    {
      ...
      mals_angle[index].angle_ri_corr
...
bool
MALS_Angles::load (const QString &filename, QString &errormsg)
{
  QString contents;
...
bool
MALS_Angles::save (const QString &filename, QString &errormsg)
{
  QStringList qsl;

  qsl << "\"US-SOMO MALS Angles\"\n"
...
US_Hydrodyn_Mals::US_Hydrodyn_Mals (csv csv1, void *us_hydrodyn, QWidget *p)
    : QFrame (p)
{
  this->csv1 = csv1;
  ...
  gaussian_type = GAUSS;
  ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_gaussian_type"]
  ...
  use_line_width
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_line_width"].toUInt ();
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)
           ->gparams.count ("mals_cb_discard_it_sd_mult"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_cb_discard_it_sd_mult"]
          = "true";
    }
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)
           ->gparams.count ("mals_maxfpk_restart_tries")
      || !started_in_expert_mode)
    {
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)
           ->gparams.count ("mals_cb_makeiq_cutmax_pct"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_cb_makeiq_cutmax_pct"]
          = "true";
    }
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)
           ->gparams.count ("mals_cb_makeiq_avg_peaks"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_cb_makeiq_avg_peaks"]
          = "false";
    }
  ...
  QString dctr_file = USglobal->config_list.root_dir + QDir::separator ()
                      + "etc" + QDir::separator ()
                      + "somo_mals_default_dctr.dat";
  if (QFile (dctr_file).exists ())
    {
      load_file (dctr_file);
    }

  QString mals_params_file = USglobal->config_list.root_dir
                             + QDir::separator () + "etc" + QDir::separator ()
                             + "somo_mals_default_mals_param.dat";
  // set defaults always
  mals_param_lambda = 0;
  mals_param_n = 0;
  mals_param_g_dndc = 0;
...
void
US_Hydrodyn_Mals::push_back_color_if_ok (QColor bg, QColor set)
{
...
void
US_Hydrodyn_Mals::cancel ()
{
  close ();
}

void
US_Hydrodyn_Mals::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Mals::closeEvent (QCloseEvent *e)
{
  QMessageBox mb (
      this->windowTitle (),
...
void
US_Hydrodyn_Mals::clear_display ()
{
  editor->clear ();
  editor->append ("\n\n");
}
...
void
US_Hydrodyn_Mals::editor_msg (QString color, QString msg)
{
...
void
US_Hydrodyn_Mals::editor_msg_qc (QColor qcolor, QString msg)
{
  QColor save_color = editor->textColor ();
...
bool
US_Hydrodyn_Mals::activate_saxs_window ()
{
  if (!*saxs_widget)
    {
...
void
US_Hydrodyn_Mals::remove_files (set<QString> &fileset, bool replot)
{
  QStringList files;
...
void
US_Hydrodyn_Mals::clear_files (QStringList files, bool quiet)
{
...
public:
  ...
  QString name;
  bool
  operator< (const mals_sortable_qstring &objIn) const
  {
    return x < objIn.x;
  }
};
...
void
US_Hydrodyn_Mals::add_dir ()
{
  add_files (false, true);
}

void
US_Hydrodyn_Mals::add_files (bool load_conc, bool from_dir)
{
  disable_all ();
...

  {
    ...
    for (auto const &name : add_filenames)
      {
        if (f_ref_index.count (name) && f_ref_index[name] != mals_param_n)
          {
    ...
    if (ref_indices.size () > 1)
      {
        ...
        QMessageBox::warning (
            ...
            QString (us_tr ("Loaded files contain multiple different solvent "
                            "refractive index (%1)\n"
                            "which do not match the Options->MALS "
                            "parameters->Solvent refractive index (%2)\n"
...
void
US_Hydrodyn_Mals::add_files (QStringList filenames)
{
  map<QString, bool> existing_items;
  for (int i = 0; i < lb_files->count (); i++)
    {
...
void
US_Hydrodyn_Mals::select_all_created ()
{
  bool all_selected = true;
...
bool
US_Hydrodyn_Mals::load_file (QString filename, bool load_conc)
{
  errormsg = "";
  QFile f (filename);
  if (!f.exists ())
  ...
  if (mals_angles.populate (filename, qsl_nb))
    {
      ...
      lbl_mals_angles_data->setText (
          mals_angles.list_rich (mals_param_lambda, mals_param_n));
      return false;
    }
  ...
  if (ext == "dat" && qv[0].contains (" MALS parameter file"))
    {
      ...
      QRegExp rx_mals_param_g_extinction_coef (
          "^# __mals_param_g_extinction_coef: (\\S+)\\s*$");
      QRegExp rx_mals_param_g_conc ("^# __mals_param_g_conc: (\\S+)\\s*$");
      QRegExp rx_mals_param_DLS_detector (
          "^# __mals_param_DLS_detector: (\\S+)\\s*$");
      ...
      for (int i = 1; i < (int)qv.size (); i++)
        {
          ...
          if (rx_mals_param_g_extinction_coef.indexIn (qv[i]) != -1)
            {
              mals_param_g_extinction_coef
                  = rx_mals_param_g_extinction_coef.cap (1).toDouble ();
              continue;
            }
          ...
          if (rx_mals_param_DLS_detector.indexIn (qv[i]) != -1)
            {
              mals_param_DLS_detector
                  = rx_mals_param_DLS_detector.cap (1).toInt ();
              continue;
            }
  ...
  if (ext == "csv" && load_conc)
    {
      ...
      US_Hydrodyn_Mals_Conc_Csv_Frames *conc_csv_frames_instance
          = new US_Hydrodyn_Mals_Conc_Csv_Frames (us_hydrodyn,
    ...
    {
      {
        ...
        if (errormsg != us_tr ("Not a MALS Angles csv file"))
          {
            QMessageBox::warning (
                this, "US-SOMO MALS Angles load Error",
...

      {
        ...
        if (mals_load (filename, qsl, errormsg))
          {
            editor_msg ("black",
                        QString (us_tr ("%1 - MALS data\n")).arg (filename));
            return false;
          }
        if (errormsg != "not a mals file")
          {
            QMessageBox::warning (
                this, "US-SOMO MALS load Error",
                QString (us_tr ("Loading file %1:\nError: %2"))
                    .arg (filename)
                    .arg (us_tr (errormsg)));
            return false;
...
void
US_Hydrodyn_Mals::set_conc_file (QString file)
{
  if (file.isEmpty ())
    {
      for (int i = 0; i < lb_files->count (); i++)
...
void
US_Hydrodyn_Mals::update_created_files ()
{
  if (!disable_updates)
    {
      update_enables ();
...
QString
US_Hydrodyn_Mals::qstring_common_head (QStringList qsl, bool strip_digits)
{
  if (!qsl.size ())
    {
      return "";
...
QString
US_Hydrodyn_Mals::qstring_common_tail (QStringList qsl, bool strip_digits)
{
  if (!qsl.size ())
    {
      return "";
...
QString
US_Hydrodyn_Mals::qstring_common_head (QString s1, QString s2)
{
...
QString
US_Hydrodyn_Mals::qstring_common_tail (QString s1, QString s2)
{
  int min_len = (int)s1.length ();
  if (min_len > (int)s2.length ())
    {
...
void
US_Hydrodyn_Mals::save_created_csv ()
{
  QStringList created_not_saved_list;
...
bool
US_Hydrodyn_Mals::save_files_csv (QStringList files)
{
  if (!files.size ())
    {
      editor_msg ("red",
...
bool
US_Hydrodyn_Mals::save_files (QStringList files)
{
...
bool
US_Hydrodyn_Mals::save_file (QString file, bool &cancel, bool &overwrite_all)
{
  if (!f_qs.count (file))
    {
      editor_msg ("red",
...

  {
    ...
    //    .arg( mals_angles.loaded_filename.isEmpty() ? QString("") : QString(
    //    " Angles loaded from:%1" ).arg( mals_angles.loaded_filename ) ) .arg(
...
void
US_Hydrodyn_Mals::update_csv_conc ()
{
...
US_Hydrodyn_Mals::conc ()
{
  ...
  if (conc_widget)
    ...
    {
      conc_window = new US_Hydrodyn_Mals_Conc (csv_conc, this);
      conc_window->show ();
    }
}

map<QString, double>
US_Hydrodyn_Mals::current_concs (bool quiet)
{
...
vector<double>
US_Hydrodyn_Mals::union_q (QStringList files)
{
...
bool
US_Hydrodyn_Mals::get_peak (QString file, double &peak, bool full)
{
  double pos;
...
bool
US_Hydrodyn_Mals::get_peak (QString file, double &peak, double &pos, bool full)
{
  if (!f_Is.count (file))
    {
      editor_msg ("red", QString (us_tr ("Internal error: get_peak requested "
...
void
US_Hydrodyn_Mals::smooth (QStringList files)
{
  bool ok;
...
void
US_Hydrodyn_Mals::create_i_of_t ()
{
  disable_all ();
...
void
US_Hydrodyn_Mals::create_i_of_t (QStringList files)
{
  // find common q
  QString head = qstring_common_head (files, true);
...
bool
US_Hydrodyn_Mals::check_zi_window (QStringList &files,
                                   const QString &extra_text)
{
  int window
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_zi_window"].toInt ();
...
void
US_Hydrodyn_Mals::check_discard_it_sd_mult (QStringList &files,
                                            bool optionally_discard)
{
  if (((US_Hydrodyn *)us_hydrodyn)->gparams["mals_cb_discard_it_sd_mult"]
          != "true"
      || !files.size ())
    {
      return;
  ...
  double mult = ((US_Hydrodyn *)us_hydrodyn)
                    ->gparams["mals_discard_it_sd_mult"]
                    .toDouble ();
...
bool
US_Hydrodyn_Mals::all_selected_have_nonzero_conc ()
{
...
void
US_Hydrodyn_Mals::delete_zoomer_if_ranges_changed ()
{
}

void
US_Hydrodyn_Mals::show_created ()
{
...
void
US_Hydrodyn_Mals::show_only_created ()
{
  disable_updates = true;
  lb_files->clearSelection ();
  show_created ();
...
QString
US_Hydrodyn_Mals::vector_double_to_csv (vector<double> vd)
{
  QString result;
...
void
US_Hydrodyn_Mals::plot_zoomed (const QRectF & /* rect */)
{
...
void
US_Hydrodyn_Mals::plot_errors_zoomed (const QRectF & /* rect */)
{
}

void
US_Hydrodyn_Mals::zoom_info ()
{
  if (plot_dist_zoomer)
    {
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Mals::plot_mouse (const QMouseEvent & /* me */)
{
  // TSO << "mouse event\n";
  // zoom_info();
  if (plot_dist_zoomer)
      ...
      if (lb_files->item (i)->isSelected ())
        {
...

          {
...

            {
              ...
              //    "" ) ) .arg( f_extc.count( file ) && f_dndc.count( file ) ?
              //    QString( " dndc:%1 [ml/g]" ).arg( f_dndc[ file ] ) :
              //    QString( "" ) ) .arg( mals_angles.loaded_filename.isEmpty()
              //    ? QString("") : QString( " Angles loaded from:%1" ).arg(
              //    mals_angles.loaded_filename ) ) .arg( f_ri_corr.count( file
...
bool
US_Hydrodyn_Mals::is_nonzero_vector (vector<double> &v)
{
...
bool
US_Hydrodyn_Mals::is_zero_vector (vector<double> &v)
{
  bool is_zero = true;
  for (int i = 0; i < (int)v.size (); i++)
    {
...
void
US_Hydrodyn_Mals::to_created (QString file)
{
...
void
US_Hydrodyn_Mals::similar_files ()
{
  vector<QString> selected;
  for (int i = 0; i < lb_files->count (); i++)
    {
...
void
US_Hydrodyn_Mals::rename_from_context (const QPoint &pos)
{
#if QT_VERSION >= 0x040000
...
void
US_Hydrodyn_Mals::rename_created (QListWidgetItem *lbi, const QPoint &)
{
  map<QString, bool> existing_items;
  for (int i = 0; i < (int)lb_files->count (); i++)
    {
...
void
US_Hydrodyn_Mals::add_plot (QString name, vector<double> q, vector<double> I,
...
bool
US_Hydrodyn_Mals::compatible_files (QStringList files)
{
  if (!files.size ())
    {
      return true;
...
bool
US_Hydrodyn_Mals::type_files (QStringList files)
{
  if (!files.size ())
    {
      return false;
...
void
US_Hydrodyn_Mals::gaussian_enables ()
{
...
void
US_Hydrodyn_Mals::update_gauss_pos ()
{
...
void
US_Hydrodyn_Mals::ggauss_msg_common (
    vector<map<int, set<double> > > &check_common)
{
...
void
US_Hydrodyn_Mals::gauss_pos_text (const QString &text)
{
  if (gaussians.size ())
    {
      if (current_mode == MODE_GAUSSIAN)
...
void
US_Hydrodyn_Mals::gauss_pos_width_text (const QString &text)
{
  if (gaussians.size ())
    {
      if (current_mode == MODE_GAUSSIAN)
...
void
US_Hydrodyn_Mals::gauss_pos_height_text (const QString &text)
{
  if (gaussians.size ())
    {
...
void
US_Hydrodyn_Mals::gauss_pos_dist1_text (const QString &text)
{
  if (gaussians.size ())
    {
      if (current_mode == MODE_GAUSSIAN)
...
void
US_Hydrodyn_Mals::gauss_pos_dist2_text (const QString &text)
{
  if (gaussians.size ())
    {
      if (current_mode == MODE_GAUSSIAN)
...
void
US_Hydrodyn_Mals::gauss_fit_start_text (const QString &text)
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Mals::gauss_fit_end_text (const QString &text)
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Mals::gauss_add_marker (double pos, QColor color, QString text,
#if QT_VERSION < 0x040000
                                    int
#else
                                    Qt::Alignment
...
void
US_Hydrodyn_Mals::gauss_add_hline (double center, double width)
{
  double fwhm = 2.354820045e0 * width;
...
void
US_Hydrodyn_Mals::gauss_init_markers ()
{
...
void
US_Hydrodyn_Mals::gauss_delete_markers ()
{
#if QT_VERSION < 0x040000
...
vector<double>
US_Hydrodyn_Mals::gaussian (double *g) // height, double center, double width )
{
...
void
US_Hydrodyn_Mals::gauss_replot_gaussian ()
{
...
void
US_Hydrodyn_Mals::gauss_add_gaussian (double *g, QColor color)
...
void
US_Hydrodyn_Mals::gauss_init_gaussians ()
{
...
void
US_Hydrodyn_Mals::gauss_delete_gaussians ()
{
...
void
US_Hydrodyn_Mals::gauss_pos_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Mals::gauss_pos_width_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Mals::gauss_pos_height_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Mals::gauss_pos_dist1_focus (bool hasFocus)
{
  ...
  if (hasFocus && !cb_ggauss_scroll->isChecked ())
    {
      ...
      update_gauss_pos ();
      double dist
          = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_dist_max"].toDouble ();
...
void
US_Hydrodyn_Mals::gauss_pos_dist2_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Mals::gauss_fit_start_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Mals::gauss_fit_end_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Mals::plot_gaussian_sum ()
{
...
void
US_Hydrodyn_Mals::replot_gaussian_sum ()
{
  // add up the curve data
...
US_Hydrodyn_Mals::gauss_fit ()
{
  ...
  if (current_mode == MODE_GGAUSSIAN)
    {
      ...
      US_Hydrodyn_Mals_Fit_Global *shfg
          = new US_Hydrodyn_Mals_Fit_Global (this, this);
  ...
  wheel_enables (false);
  disconnect (qwtw_wheel, SIGNAL (valueChanged (double)), 0, 0);
  US_Hydrodyn_Mals_Fit *shf = new US_Hydrodyn_Mals_Fit (this, true, this);
...
void
US_Hydrodyn_Mals::create_i_of_q ()
{
  disable_all ();
...
vector<double>
US_Hydrodyn_Mals::compute_gaussian_sum (vector<double> t, vector<double> g)
{
...
vector<double>
US_Hydrodyn_Mals::compute_gaussian (vector<double> t, vector<double> g)
{
  vector<double> result (t.size ());
...
double
US_Hydrodyn_Mals::compute_gaussian_peak (QString file, vector<double> g)
{
...
QString
US_Hydrodyn_Mals::pad_zeros (int val, int max)
{
...
QStringList
US_Hydrodyn_Mals::all_selected_files ()
{
  QStringList files;
  for (int i = 0; i < lb_files->count (); i++)
    {
...
set<QString>
US_Hydrodyn_Mals::all_selected_files_set ()
{
  set<QString> result;
  for (int i = 0; i < lb_files->count (); i++)
    {
...
map<QString, bool>
US_Hydrodyn_Mals::all_files_map ()
{
...
int
US_Hydrodyn_Mals::ggaussian_sel_no_gaussian_count ()
{
...
bool
US_Hydrodyn_Mals::ggaussian_compatible (bool check_against_global)
{
  QStringList files = all_selected_files ();
...
bool
US_Hydrodyn_Mals::ggaussian_compatible (set<QString> &selected,
...
bool
US_Hydrodyn_Mals::ggaussian_compatible (QStringList &files,
                                        bool check_against_global)
{
...
void
US_Hydrodyn_Mals::add_ggaussian_curve (QString name, vector<double> y)
{
...
void
US_Hydrodyn_Mals::ggauss_results ()
{
  if (unified_ggaussian_ok)
    {
...
void
US_Hydrodyn_Mals::created_dir_pressed ()
{
  QString use_dir = le_dir->text ();
  ((US_Hydrodyn *)us_hydrodyn)->select_from_directory_history (use_dir, this);
  QString s = QFileDialog::getExistingDirectory (
...
void
US_Hydrodyn_Mals::gauss_as_curves ()
{
  if (current_mode == MODE_GAUSSIAN)
    {
      for (unsigned int i = 0; i < (unsigned int)gaussians.size ();
...
void
US_Hydrodyn_Mals::set_sd_weight ()
{
  if (current_mode == MODE_GAUSSIAN)
    {
      replot_gaussian_sum ();
...
void
US_Hydrodyn_Mals::set_fix_width ()
{
}

void
US_Hydrodyn_Mals::set_fix_dist1 ()
{
}

void
US_Hydrodyn_Mals::set_fix_dist2 ()
{
}

bool
US_Hydrodyn_Mals::check_fit_range ()
{
  if (!gaussians.size ())
    {
      return true;
...
void
US_Hydrodyn_Mals::hide_widgets (vector<QWidget *> widgets, bool hide)
{
...
void
US_Hydrodyn_Mals::update_gauss_mode ()
{
