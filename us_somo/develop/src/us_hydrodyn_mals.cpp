...
bool MALS_Angles::load(const QString &filename, const QStringList &csvlines,
                       QString &errormsg) {
  if (!csvlines.size()) {
...
US_Hydrodyn_Mals::US_Hydrodyn_Mals(csv csv1, void *us_hydrodyn, QWidget *p)
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)
           ->gparams.count("mals_lock_min_retry_mult") ||
      !started_in_expert_mode) {
  ...
  QString dctr_file = USglobal->config_list.root_dir + QDir::separator() +
                      "etc" + QDir::separator() + "somo_mals_default_dctr.dat";
...
void US_Hydrodyn_Mals::cancel() { close(); }
...
void US_Hydrodyn_Mals::add_dir() { add_files(false, true); }
    ...
    if (ref_indices.size() == 1) {
      QMessageBox::warning(
          ...
          QString(us_tr("Loaded files contain a different solvent refractive "
                        "index (%1)\n"
                        "than set in the Options->MALS parameters->Solvent "
                        "refractive index (%2)\n"
...
bool US_Hydrodyn_Mals::load_file(QString filename, bool load_conc) {
  ...
  if (ext == "csv" && load_conc) {
    ...
    US_Hydrodyn_Mals_Conc_Csv_Frames *conc_csv_frames_instance =
        new US_Hydrodyn_Mals_Conc_Csv_Frames(us_hydrodyn, &conc_csv_params,
                                             this);
  ...
  if (ext == "csv") {
    {
      ...
      if (errormsg != us_tr("Not a MALS Angles csv file")) {
        QMessageBox::warning(this, "US-SOMO MALS Angles load Error",
...

    {
      ...
      if (errormsg != "not a mals file") {
        QMessageBox::warning(this, "US-SOMO MALS load Error",
                             QString(us_tr("Loading file %1:\nError: %2"))
                                 .arg(filename)
                                 .arg(us_tr(errormsg)));
        return false;
...
QString US_Hydrodyn_Mals::qstring_common_head(QStringList qsl,
                                              bool strip_digits) {
  if (!qsl.size()) {
    return "";
  }
...
QString US_Hydrodyn_Mals::qstring_common_tail(QStringList qsl,
                                              bool strip_digits) {
  if (!qsl.size()) {
    return "";
  }
...
QString US_Hydrodyn_Mals::qstring_common_head(QString s1, QString s2) {
...
QString US_Hydrodyn_Mals::qstring_common_tail(QString s1, QString s2) {
  int min_len = (int)s1.length();
  if (min_len > (int)s2.length()) {
    min_len = (int)s2.length();
  }
...
bool US_Hydrodyn_Mals::save_file(QString file, bool &cancel,
                                 bool &overwrite_all) {
  if (!f_qs.count(file)) {
...
bool US_Hydrodyn_Mals::get_peak(QString file, double &peak, double &pos,
                                bool full) {
  if (!f_Is.count(file)) {
...
bool US_Hydrodyn_Mals::check_zi_window(QStringList &files,
...
void US_Hydrodyn_Mals::check_discard_it_sd_mult(QStringList &files,
...
void US_Hydrodyn_Mals::add_plot(QString name, vector<double> q,
...
void US_Hydrodyn_Mals::ggauss_msg_common(
    vector<map<int, set<double> > > &check_common) {
...
void US_Hydrodyn_Mals::gauss_add_marker(double pos, QColor color, QString text,
#if QT_VERSION < 0x040000
                                        int
#else
                                        Qt::Alignment
...
vector<double> US_Hydrodyn_Mals::gaussian(
...
vector<double> US_Hydrodyn_Mals::compute_gaussian_sum(vector<double> t,
                                                      vector<double> g) {
...
vector<double> US_Hydrodyn_Mals::compute_gaussian(vector<double> t,
                                                  vector<double> g) {
  vector<double> result(t.size());
...
bool US_Hydrodyn_Mals::ggaussian_compatible(set<QString> &selected,
...
bool US_Hydrodyn_Mals::ggaussian_compatible(QStringList &files,
                                            bool check_against_global) {
