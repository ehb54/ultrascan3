...
bool US_Hydrodyn_Mals::load_file( QString filename, bool load_conc )
{
   ...
   {
      QRegularExpression rx_mals_param_lambda            ( "^# __mals_param_lambda: (\\S+)\\s*$" );
      QRegularExpression rx_mals_param_n                 ( "^# __mals_param_n: (\\S+)\\s*$" );
      QRegularExpression rx_mals_param_g_dndc            ( "^# __mals_param_g_dndc: (\\S+)\\s*$" );
      QRegularExpression rx_mals_param_g_extinction_coef ( "^# __mals_param_g_extinction_coef: (\\S+)\\s*$" );
      QRegularExpression rx_mals_param_g_conc            ( "^# __mals_param_g_conc: (\\S+)\\s*$" );
      QRegularExpression rx_mals_param_DLS_detector      ( "^# __mals_param_DLS_detector: (\\S+)\\s*$" );

      for ( int i = 1; i < (int) qv.size(); i++ ) {

         QRegularExpressionMatch rx_mals_param_lambda_m = rx_mals_param_lambda.match( qv[ i ] );
         if ( rx_mals_param_lambda_m.hasMatch() ) {
            mals_param_lambda = rx_mals_param_lambda_m.captured(1).toDouble();
            continue;
         }
         QRegularExpressionMatch rx_mals_param_n_m = rx_mals_param_n.match( qv[ i ] );
         if ( rx_mals_param_n_m.hasMatch() ) {
            mals_param_n = rx_mals_param_n_m.captured(1).toDouble();
            continue;
         }
         QRegularExpressionMatch rx_mals_param_g_dndc_m = rx_mals_param_g_dndc.match( qv[ i ] );
         if ( rx_mals_param_g_dndc_m.hasMatch() ) {
            mals_param_g_dndc = rx_mals_param_g_dndc_m.captured(1).toDouble();
            continue;
         }
         QRegularExpressionMatch rx_mals_param_g_extinction_coef_m = rx_mals_param_g_extinction_coef.match( qv[ i ] );
         if ( rx_mals_param_g_extinction_coef_m.hasMatch() ) {
            mals_param_g_extinction_coef = rx_mals_param_g_extinction_coef_m.captured(1).toDouble();
            continue;
         }
         QRegularExpressionMatch rx_mals_param_g_conc_m = rx_mals_param_g_conc.match( qv[ i ] );
         if ( rx_mals_param_g_conc_m.hasMatch() ) {
            mals_param_g_conc = rx_mals_param_g_conc_m.captured(1).toDouble();
            continue;
         }
         QRegularExpressionMatch rx_mals_param_DLS_detector_m = rx_mals_param_DLS_detector.match( qv[ i ] );
         if ( rx_mals_param_DLS_detector_m.hasMatch() ) {
            mals_param_DLS_detector = rx_mals_param_DLS_detector_m.captured(1).toInt();
            continue;
         }
...
void US_Hydrodyn_Mals::smooth( QStringList files )
{
   ...
   int smoothing = US_Static::getInteger(
                                            us_tr( "SOMO: MALS enter smoothing" ),
