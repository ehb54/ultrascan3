...
US_Hydrodyn_Saxs_Load_Csv::US_Hydrodyn_Saxs_Load_Csv (
    ...
    bool *nnls_plot_contrib, bool *nnls_csv, bool *run_best_fit,
...
void
US_Hydrodyn_Saxs_Load_Csv::setupGUI ()
{
  int minWidth1 = 600;
  int minHeight1 = 30;
  int minHeight1dl = 40;
...
void
US_Hydrodyn_Saxs_Load_Csv::select_all ()
{
...
void
US_Hydrodyn_Saxs_Load_Csv::ok ()
{
  close ();
}

void
US_Hydrodyn_Saxs_Load_Csv::transpose ()
{
  // pick
          ...
          if (qsl_tmp.size () > 4)
            {
              ...
              if (this_mw > 0 && this_area > 0 && qsl_tmp[3] == "\"P(r)\"")
                {
                  // QTextStream(stdout) << "ok to make P(r) normed\n";
                  QStringList this_pr_normed;
                  this_pr_normed << qsl_tmp[0] << qsl_tmp[1] << qsl_tmp[1]
                                 << "\"P(r) normed\"";
                  for (int i = 4; i < (int)qsl_tmp.size (); ++i)
                    {
                      this_pr_normed << QString ("%1").arg (
                          qsl_tmp[i].toDouble () * this_mw / this_area);
                    }
                  qsl_pr_normed << this_pr_normed.join (",");
                  continue;
                }
              if (this_mw > 0 && this_area > 0 && qsl_tmp[3] == "\"P(r) sd\"")
                {
                  QStringList this_pr_error;
                  this_pr_error << qsl_tmp[0] << qsl_tmp[1] << qsl_tmp[1]
                                << "\"P(r) normed sd\"";
                  for (int i = 4; i < (int)qsl_tmp.size (); ++i)
                    {
                      this_pr_error << QString ("%1").arg (
                          qsl_tmp[i].toDouble () * this_mw / this_area);
                    }
          ...
          if (qsl_tmp.size ())
            {
              ...
              if ((map_sel_names.count (qsl_tmp[0])
                   || it == qsl_pr_normed.begin ())
                  // && ( qsl_tmp.count() <= 3 || qsl_tmp[3] != "\"P(r)
                  // normed\"" )
              )
                {
                  ...
                  // QTextStream(stdout) << QString("line %1 qsl_tmp.count() %2
                  // size
...
void
US_Hydrodyn_Saxs_Load_Csv::save_selected ()
{
  // pick
  QString save_file = loaded_filename;
...
void
US_Hydrodyn_Saxs_Load_Csv::cancel ()
{
  qsl_sel_names->clear ();
  close ();
}
...
void
US_Hydrodyn_Saxs_Load_Csv::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Saxs_Load_Csv::update_selected ()
{
  qsl_sel_names->clear ();
  for (int i = 0; i < lb_names->count (); i++)
    {
...
void
US_Hydrodyn_Saxs_Load_Csv::update_csv_filename (const QString &str)
{
  *csv_filename = str;
}

void
US_Hydrodyn_Saxs_Load_Csv::set_create_avg ()
{
...
void
US_Hydrodyn_Saxs_Load_Csv::set_create_std_dev ()
{
...
void
US_Hydrodyn_Saxs_Load_Csv::set_only_plot_stats ()
{
...
void
US_Hydrodyn_Saxs_Load_Csv::set_save_to_csv ()
{
...
void
US_Hydrodyn_Saxs_Load_Csv::set_save_original_data ()
{
...
void
US_Hydrodyn_Saxs_Load_Csv::set_run_nnls ()
{
...
void
US_Hydrodyn_Saxs_Load_Csv::set_nnls_csv ()
{
...
void
US_Hydrodyn_Saxs_Load_Csv::set_nnls_plot_contrib ()
{
...
void
US_Hydrodyn_Saxs_Load_Csv::set_run_best_fit ()
{
...
void
US_Hydrodyn_Saxs_Load_Csv::set_use_SDs_for_fitting ()
{
...
void
US_Hydrodyn_Saxs_Load_Csv::set_clear_plot_first ()
{
...
void
US_Hydrodyn_Saxs_Load_Csv::select_target ()
{
  if (*run_nnls || *run_best_fit)
    {
...
void
US_Hydrodyn_Saxs_Load_Csv::update_enables ()
{
