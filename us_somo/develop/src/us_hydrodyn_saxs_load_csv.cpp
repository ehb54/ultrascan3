...
void US_Hydrodyn_Saxs_Load_Csv::ok() { close(); }
  ...
  if (of) {
      ...
      if (qsl_tmp.size() > 4) {
        ...
        if (this_mw > 0 && this_area > 0 && qsl_tmp[3] == "\"P(r)\"") {
          ...
          for (int i = 4; i < (int)qsl_tmp.size(); ++i) {
            this_pr_normed << QString("%1").arg(qsl_tmp[i].toDouble() *
                                                this_mw / this_area);
          }
        ...
        if (this_mw > 0 && this_area > 0 && qsl_tmp[3] == "\"P(r) sd\"") {
          ...
          this_pr_error << qsl_tmp[0] << qsl_tmp[1] << qsl_tmp[1]
                        << "\"P(r) normed sd\"";
          for (int i = 4; i < (int)qsl_tmp.size(); ++i) {
            this_pr_error << QString("%1").arg(qsl_tmp[i].toDouble() * this_mw /
                                               this_area);
          }
    ...
    for (QStringList::iterator it = qsl_pr_normed.begin();
      ...
      if (qsl_tmp.size()) {
        ...
        if ((map_sel_names.count(qsl_tmp[0]) || it == qsl_pr_normed.begin())
          ...
          // QTextStream(stdout) << QString("line %1 qsl_tmp.count() %2 size
    ...
    for (unsigned int i = 0; i < max_len; i++) {
      ...
      for (unsigned int j = 0; j < array2d_to_save.size(); j++) {
        if (array2d_to_save[j].size() <= i || array2d_to_save[j][i].isNull()) {
...
void US_Hydrodyn_Saxs_Load_Csv::set_nnls_csv() {
...
void US_Hydrodyn_Saxs_Load_Csv::set_clear_plot_first() {
