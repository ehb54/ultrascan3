...
ostream &
operator<< (ostream &out, const range_entry &c)
{
  ...
  if (c.gaps.size ())
    {
      ...
      for (map<unsigned int, bool>::iterator it = lg.begin (); it != lg.end ();
           it++)
        {
          out << " " << it->first;
        }
...
public:
  unsigned int x;
  bool
  operator< (const uhptm_sortable_uint &objIn) const
  {
    return x < objIn.x;
  }
};

US_Hydrodyn_Pdb_Tool_Merge::US_Hydrodyn_Pdb_Tool_Merge (void *us_hydrodyn,
...
void
US_Hydrodyn_Pdb_Tool_Merge::reset_csv_commands ()
{
...
void
US_Hydrodyn_Pdb_Tool_Merge::setupGUI ()
{
  int minHeight1 = 22;
  ...
  t_csv = new QTableWidget (csv_commands.data.size (),
                            csv_commands.header.size (), this);
  ...
  for (unsigned int i = 0; i < (unsigned int)csv_commands.header.size (); i++)
    {
      t_csv->setHorizontalHeaderItem (
          i, new QTableWidgetItem (csv_commands.header[i]));
    }
  ...
  //{ for ( int i = 0; i < t_csv->rowCount(); ++i ) { for ( int j = 0; j <
  //t_csv->columnCount(); ++j ) { t_csv->item( i, j )->setFlags( t_csv->item(
  //i, j )->flags() ^ Qt::ItemIsEditable ); } } };

  //   t_csv->setColumnWidth(0, ?? );
  // { for ( int i = 0; i < t_csv->rowCount(); ++i ) { t_csv->item( i,
  // t_csv->columnCount() - 1 )->setFlags( t_csv->item( i, t_csv->columnCount()
  // - 1 )->flags() ^ Qt::ItemIsEditable ); } };
  ...
  t_csv->setSelectionMode (QAbstractItemView::MultiSelection);
  t_csv->setSelectionBehavior (QAbstractItemView::SelectRows);

  connect (t_csv, SIGNAL (cellChanged (int, int)),
  ...
  pb_sel_auto->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  pb_sel_from_to_merge->setFont (QFont (USglobal->config_list.fontFamily,
                                        USglobal->config_list.fontSize - 1));
  pb_sel_from_to_merge->setMinimumHeight (minHeight1);
  pb_sel_from_to_merge->setPalette (PALET_PUSHB);
  connect (pb_sel_from_to_merge, SIGNAL (clicked ()),
           SLOT (sel_from_to_merge ()));

  pb_sel_from_to_fit = new QPushButton (us_tr ("From -> Fit"), this);
  pb_sel_from_to_fit->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize - 1));
  ...
  pb_sel_to_to_fit->setFont (QFont (USglobal->config_list.fontFamily,
                                    USglobal->config_list.fontSize - 1));
  ...
  pb_sel_to_to_cut->setFont (QFont (USglobal->config_list.fontFamily,
                                    USglobal->config_list.fontSize - 1));
  ...
  pb_extra_chains->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize - 1));
  ...
  pb_only_closest->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize - 1));
  ...
  pb_delete_row->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  ...
  pb_validate->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  ...
  pb_chains_from->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize - 1));
  ...
  le_chains_from = new QLineEdit (this);
  le_chains_from->setObjectName ("chains_from Line Edit");
  ...
  le_chains_from->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize - 1));
  le_chains_from->setReadOnly (true);

  pb_chains_to = new QPushButton (us_tr ("Chains To"), this);
  pb_chains_to->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize - 1));
  ...
  le_chains_to = new QLineEdit (this);
  le_chains_to->setObjectName ("chains_to Line Edit");
  ...
  pb_trial->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize - 1));
  ...
  QHBoxLayout *hbl_sel_cmds = new QHBoxLayout ();
  hbl_sel_cmds->setContentsMargins (0, 0, 0, 0);
  hbl_sel_cmds->setSpacing (0);
  ...
  QHBoxLayout *hbl_extra_chains = new QHBoxLayout ();
  hbl_extra_chains->setContentsMargins (0, 0, 0, 0);
  hbl_extra_chains->setSpacing (0);
  ...
  QHBoxLayout *hbl_load_save = new QHBoxLayout ();
  hbl_load_save->setContentsMargins (0, 0, 0, 0);
  hbl_load_save->setSpacing (0);
...
void
US_Hydrodyn_Pdb_Tool_Merge::cancel ()
{
  close ();
}

void
US_Hydrodyn_Pdb_Tool_Merge::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Pdb_Tool_Merge::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept ();
...
void
US_Hydrodyn_Pdb_Tool_Merge::clear_display ()
{
  editor->clear ();
  editor->append ("\n\n");
}
...
void
US_Hydrodyn_Pdb_Tool_Merge::update_font ()
{
  bool ok;
  QFont newFont;
...
void
US_Hydrodyn_Pdb_Tool_Merge::save ()
{
  QString fn;
...
void
US_Hydrodyn_Pdb_Tool_Merge::start ()
{
  running = true;
  update_enables ();
  ...
  ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)->load (lv_csv_to, filename, true);
        ...
        if (qs.contains (rx_model))
          {
            model_count++;
            QStringList qsl
                = (qs.left (20).split (QRegExp ("\\s+"), Qt::SkipEmptyParts));
            QString model_name;
            if (qsl.size () == 1)
              {
                ...
                if (!dup_model_name_msg_done)
                  {
                    ...
                    // editor_msg( "red", us_tr( "Duplicate or missing model
                    // names found, -# extensions added" ) );
                  }
              }
  ...
  progress->setValue (1);
  progress->setMaximum (model_count + 1);
  qApp->processEvents ();

  if (QFile::exists (le_target->text ()))
    {
      le_target->setText (((US_Hydrodyn *)us_hydrodyn)
                              ->fileNameCheck (le_target->text (), 0, this));
    }
  ...
  QFile ff (
      le_target->text ().replace (QRegExp (".pdb$", Qt::CaseInsensitive), "")
      + "_filtered.pdb");
  ...
  if (cb_filter->isChecked ())
    {
      tsof << QString ("HEADER    Processed by US-SOMO Cut/Splice and "
                       "*filtered* due to steric clashes\n");
      tsof << model_header;
    }
              ...
              if (model_lines.size ())
                {
                  ...
                  if (filtered)
                      ...
                      if (cb_filter->isChecked ())
                        {
                          if (!no_model)
                            {
                              tsof << QString ("").sprintf (
                                  "MODEL  %7s\n",
                                  model_name_vector[pos].toLatin1 ().data ());
                              tsof << model_remarks[model_name_vector[pos]];
                            }
                          tsof << ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
                                      ->csv_to_pdb (csv_to, true);
                          if (!no_model)
                            {
                              tsof << "ENDMDL\n";
                            }
                  ...
                  else
                    {
                      ...
                      tso << ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
                                 ->csv_to_pdb (csv_to, true);
                      if (!no_model)
                        {
                          tso << "ENDMDL\n";
                  ...
                  if (!no_model)
                    {
                      editor_msg ("dark blue",
                                  QString ("Processing model %1 done\n")
                                      .arg (model_name_vector[pos]));
                    }
                  pos++;
                }
...
void
US_Hydrodyn_Pdb_Tool_Merge::trial ()
{
  running = true;
  update_enables ();
  ...
  csv csv_from
      = ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
            ->to_csv (lv_csv_from,
                      ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)->csv1, false);
  ...
  csv csv_to
      = ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
            ->to_csv (lv_csv_to,
                      ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
                          ->csv2[((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
                                     ->csv2_pos],
                      false);
  ...
  for (unsigned int i = 0; i < (unsigned int)csv_commands.data.size (); i++)
    {
      if (csv_commands.data[i].size () < 7
          || csv_commands.num_data[i].size () < 7)
        {
          editor_msg (
              "red",
              us_tr ("Internal error: Invalid command directive format"));
          running = false;
          update_enables ();
          return;
        }
          ...
          if (!get_chains (merge_range.chain, fit_chain, cross_chain))
            {
              editor_msg (
                  "red",
                  QString (us_tr ("Internal error: Extra chain not in proper "
                                  "format <%1>, please label each chain"))
                      .arg (merge_range.chain));
              continue;
            }
  ...
  for (map<QString, range_entry>::iterator it = fit_map.begin ();
       it != fit_map.end (); it++)
    {
      ...
      if (get_chains (it->first, fit_chain, cross_chain))
        {
          update_cache_range ();
          if (!cache_from_range_pos.count (fit_chain)
              || !cache_to_range_pos.count (fit_chain))
            {
              editor_msg (
                  "red",
                  QString (us_tr ("Internal error: chain dereferencing <%1>"))
                      .arg (fit_chain));
              continue;
            }
          if (!cache_use_start.count (fit_chain))
            {
              editor_msg ("red", QString (us_tr ("Internal error: chain start "
                                                 "map entry missing  <%1>"))
                                     .arg (fit_chain));
              continue;
            }

          range_entry from_range
              = cache_from_ranges[cache_from_range_pos[fit_chain]];
          range_entry to_range
              = cache_to_ranges[cache_to_range_pos[fit_chain]];
          ...
          if ((use_start && it->second.start < to_range.start)
              || (!use_start && it->second.end > to_range.end))
            {
              if (!fit_map.count (fit_chain))
                {
                  editor_msg ("red",
                              QString (us_tr ("Internal error: chain fit map "
                                              "entry missing  <%1>"))
                                  .arg (fit_chain));
                  continue;
                }
              editor_msg (
                  "blue",
                  QString (us_tr ("Notice: setting fitting for extra chain %1 "
                                  "to fit region for chain %2"))
                      .arg (cross_chain)
                      .arg (fit_chain));
  ...
  // extract the merge & fit bits
  {
    ...
    for (unsigned int i = 0; i < (unsigned int)csv_from.data.size (); i++)
      {
        ...
        if ((merge_map.count (chain) && merge_map[chain].start <= pos
        ...
        if ((fit_map.count (chain) && fit_map[chain].start <= pos
        ...
        if (fit_map.count (chain) && fit_map[chain].start <= pos
            && fit_map[chain].end >= pos && atom == " CA ")
          {
            ...
            // editor_msg("blue", QString("adding fit point for %1 %2
            // %3").arg(chain).arg(pos).arg(atom));
        ...
        if (cross_fit_chains.count (chain))
          {
            for (unsigned int j = 0;
                 j < (unsigned int)cross_fit_chains[chain].size (); j++)
              {
                QString fit_chain = chain;
                QString cross_chain = cross_fit_chains[fit_chain][j];
                QString chain
                    = QString ("%1+%2").arg (fit_chain).arg (cross_chain);
                if (!extra_chain_reported.count (chain))
                  {
                    editor_msg ("blue",
                                QString ("Adding extra fit chains for %1")
                                    .arg (chain));
...
#if defined(USPTM_DEBUG2)
                    cout << QString (
                                "adding xchain from_fit point for %1 %2 %3\n")
                                .arg (chain)
                                .arg (pos)
                                .arg (atom);
#endif
        ...
        if (cross_cross_chains.count (chain))
          {
            for (unsigned int j = 0;
                 j < (unsigned int)cross_cross_chains[chain].size (); j++)
              {
                ...
                if (!extra_cross_chain_reported.count (chain))
                  {
                    editor_msg ("blue",
                                QString ("Adding extra merge chains for %1")
                                    .arg (chain));
    ...
    //      ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)->csv_setup_keys(
    //      csv_merge );
    //      ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)->csv_clipboard =
    //      csv_merge;
    //      ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)->csv_clipboard.name =
    //      "merge bits of " + csv_from.name;
      ...
      for (unsigned int i = 0; i < (unsigned int)csv_to.data.size (); i++)
        {
          ...
          if (!cut_map.count (chain) || cut_map[chain].start > pos
  ...
  for (map<QString, range_entry>::iterator it = to_range_map.begin ();
       it != to_range_map.end (); it++)
    {
      if (merge_map.count (it->first))
        {
          chain_to_front[it->first]
              = merge_map[it->first].end < it->second.start;
          // editor_msg( "blue", QString("Chain %1, merge chain to be added
          // %2").arg( it->first ).arg( chain_to_front[ it->first ] ? "front" :
          // "back" ) );
        }
    }
          ...
          for (unsigned int j = 0;
               j < (unsigned int)cross_fit_chains[chain].size (); j++)
            {
              ...
              if (!extra_chain_reported.count (chain))
                {
                  editor_msg ("blue",
                              QString ("Adding to extra fit chains for %1")
                                  .arg (chain));
                  extra_chain_reported[chain] = true;
                }
       ...
       it != from_fit_points.end (); it++)
    {
      if (!to_fit_points.count (it->first)
          || to_fit_points[it->first].size () != it->second.size ())
        {
          editor_msg (
              "red",
              QString ("Error (trial): Fit point mismatch for %1 (%2 vs %3)")
                  .arg (it->first)
                  .arg (to_fit_points.count (it->first)
                            ? to_fit_points[it->first].size ()
                            : -1)
                  .arg (it->second.size ()));
          running = false;
          update_enables ();
        ...
        {
          cout << QString ("Ok (trial): Fit points for %1 (%2 vs %3)\n")
                      .arg (it->first)
                      .arg (to_fit_points.count (it->first)
                                ? to_fit_points[it->first].size ()
                                : -1)
  ...
  // go through csv_to, for each chain with a transformation, add merge_bits
  // transformed
...

  {
    ...
    for (unsigned int i = 0; i < (unsigned int)csv_to.data.size (); i++)
      {
            ...
            for (unsigned int j = 0;
                 j < (unsigned int)merge_data[last_chain].size (); j++)
              {
                ...
                if (!((US_Hydrodyn *)us_hydrodyn)
                         ->atom_align (from_fit_points[last_chain],
                                       to_fit_points[last_chain], ps, result,
                                       error_msg))
                  {
                ...
                new_csv.data[datapos][8]
                    = QString ("%1").arg (result[0].axis[0]);
                new_csv.data[datapos][9]
                    = QString ("%1").arg (result[0].axis[1]);
                new_csv.data[datapos][10]
                    = QString ("%1").arg (result[0].axis[2]);
              }
          }

        if ((merge_data.count (chain) && !chain_added.count (chain)
             && chain_to_front.count (chain) && chain_to_front[chain] == true))
          {
            // cout << QString("adding merge for chain %1 before pos
            // %2\n").arg(chain).arg(pos);
            chain_added[chain] = true;
            // xform & add chain
            for (unsigned int j = 0;
                 j < (unsigned int)merge_data[chain].size (); j++)
              {
                vector<point> ps;
                point p;
                ...
                QString error_msg;
                if (!((US_Hydrodyn *)us_hydrodyn)
                         ->atom_align (from_fit_points[chain],
                                       to_fit_points[chain], ps, result,
                                       error_msg))
                  {
                    editor_msg ("red", error_msg);
                    running = false;
        ...
        // cout << QString("pushing back chain %1 pos
        // %2\n").arg(chain).arg(pos);
        new_csv.data.push_back (csv_to.data[i]);
        last_chain = chain;
      }

    if (merge_data.count (last_chain) && !chain_added.count (last_chain)
        && chain_to_front.count (last_chain)
        && chain_to_front[last_chain] == false)
      {
        // cout << QString("adding merge at end previous chain
        // %1\n").arg(last_chain);
        chain_added[last_chain] = true;
        for (unsigned int j = 0;
             j < (unsigned int)merge_data[last_chain].size (); j++)
    ...
    //  for ( map < QString, vector < vector < QString > > >
    //  merge_data.begin(); etc
    ...
    for (unsigned int i = 0; i < (unsigned int)csv_from.data.size (); i++)
      {
        ...
        if (cross_cross_chains[chain].size () != 1)
          {
            editor_msg (
                "red", QString (us_tr ("Error: only one fitting chain "
                                       "currently allowed per extra chain %1"))
                           .arg (chain));
            running = false;
            update_enables ();
            return;
...

        {
          ...
          if (!from_fit_points.count (chain) || !to_fit_points.count (chain))
            {
              editor_msg ("red",
                          QString (us_tr ("Error: missing fit points for %1"))
                              .arg (chain));
              running = false;
              update_enables ();
              return;
          ...
          if (!merge_data.count (chain))
            {
              editor_msg (
                  "red",
                  QString (us_tr ("Error: merge data for %1")).arg (chain));
              running = false;
              update_enables ();
              return;
            }
    ...
    for (unsigned int i = 0; i < (unsigned int)new_csv.data.size (); i++)
      {
        ...
        if (chain != last_chain)
          {
            // editor_msg("red", QString("chain <%1> last chain <%2>").arg(
            // chain ).arg(last_chain));
            atompos = 0;
            last_chain = chain;
          }
...
void
US_Hydrodyn_Pdb_Tool_Merge::stop ()
{
  running = false;
...
void
US_Hydrodyn_Pdb_Tool_Merge::update_enables ()
{
  if (!running)
    {
      progress->reset ();
  ...
  bool any_from_selected
      = ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)->any_selected (lv_csv_from);
  bool any_to_selected
      = ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)->any_selected (lv_csv_to);

  // pdb_sel_count counts    = ((US_Hydrodyn_Pdb_Tool
  // *)pdb_tool_window)->count_selected( lv_csv_to );

  cb_filter->setEnabled (!running);
  pb_clear->setEnabled (!running && t_csv->rowCount ());
  pb_load->setEnabled (!running);
  pb_validate->setEnabled (!running && from_exists && to_exists
                           && t_csv->rowCount ());
...
#endif
  pb_start->setEnabled (!running && from_exists && to_exists
                        && t_csv->rowCount () && target_set);
  pb_trial->setEnabled (!running && from_exists && to_exists
                        && t_csv->rowCount ());
...
void
US_Hydrodyn_Pdb_Tool_Merge::editor_msg (QString color, QString msg)
{
...
void
US_Hydrodyn_Pdb_Tool_Merge::chains_from ()
{
  QString filename;
  ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
      ->load (lv_csv_from, filename, true);
...
void
US_Hydrodyn_Pdb_Tool_Merge::chains_to ()
{
  QString filename;
  ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)->load (lv_csv_to, filename, true);
...
void
US_Hydrodyn_Pdb_Tool_Merge::target ()
{
  QString filename = QFileDialog::getSaveFileName (
      this, "Choose a name to save the result", "", "PDB (*.pdb *.PDB)");
...
void
US_Hydrodyn_Pdb_Tool_Merge::sel_auto ()
{
  clear ();
  ...
  for (unsigned int i = 0; i < (unsigned int)to_ranges.size (); i++)
    {
      ...
      if (from_range_pos.count (to_ranges[i].chain))
        {
#if defined(USPTM_DEBUG)
          cout << "common chain: found matching from range: "
               << from_ranges[from_range_pos[to_ranges[i].chain]] << endl;
#endif
          // common chain
          // only handling one end for now
          range_entry from_range
              = from_ranges[from_range_pos[to_ranges[i].chain]];
          range_entry new_merge_range;
          range_entry new_fit_range;

          bool start_good = (from_range.start < to_ranges[i].start
                             && from_range.end >= to_ranges[i].start + 2
                             && to_ranges[i].end >= to_ranges[i].start + 2);
          bool start_ok = (from_range.start <= to_ranges[i].start
                           && from_range.end >= to_ranges[i].start + 2
                           && to_ranges[i].end >= to_ranges[i].start + 2);
          bool end_good = (from_range.end > to_ranges[i].end
                           && from_range.start <= to_ranges[i].end - 2
                           && to_ranges[i].start <= to_ranges[i].end - 2);
          bool end_ok = (from_range.end >= to_ranges[i].end
                         && from_range.start <= to_ranges[i].end - 2
                         && to_ranges[i].start <= to_ranges[i].end - 2);
  ...
  for (int row = 0; row < t_csv->rowCount (); row++)
    {
      ...
      if (!cache_from_range_pos.count (chain)
          || !cache_to_range_pos.count (chain))
        {
          editor_msg (
              "red",
              QString (us_tr ("Internal error: chain dereferencing <%1>"))
      ...
      else
        {
          ...
          unsigned current_merge_start
              = t_csv->item (row, 1)->text ().toUInt ();
          ...
          if (current_merge_start > current_merge_end)
            {
              ...
              t_csv->setItem (row, 2,
                              new QTableWidgetItem (
                                  QString ("%1").arg (current_merge_end)));
              table_value (row, 2);
            }
          unsigned current_fit_start = t_csv->item (row, 3)->text ().toUInt ();
          unsigned current_fit_end = t_csv->item (row, 4)->text ().toUInt ();
          unsigned int current_fit_length_from = residue_length (
              true, chain, current_fit_start, current_fit_end);
          unsigned int current_fit_length_to = residue_length (
              false, chain, current_fit_start, current_fit_end);
          unsigned int current_fit_length
              = current_fit_length_from < current_fit_length_to
                    ? current_fit_length_from
                    : current_fit_length_to;
          if (current_fit_length < 3)
            {
              current_fit_end = residue_offset_position (true, chain,
                                                         current_fit_start, 3);
              unsigned int alt_fit_end = residue_offset_position (
                  false, chain, current_fit_start, 3);
              ...
              t_csv->setItem (row, 3,
                              new QTableWidgetItem (
                                  QString ("%1").arg (current_fit_start)));
              t_csv->setItem (
                  row, 4,
                  new QTableWidgetItem (QString ("%1").arg (current_fit_end)));
            }
        }
    }
...
void
US_Hydrodyn_Pdb_Tool_Merge::sel_to_range (QTreeWidget *lv,
                                          vector<range_entry> &ranges,
                                          bool just_selected)
{
  csv csv1 = ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
                 ->to_csv (
                     lv,
                     lv == lv_csv_from
...
void
US_Hydrodyn_Pdb_Tool_Merge::make_csv_chain_map ()
{
  csv_chain_map.clear ();
  for (int i = 0; i < t_csv->rowCount (); i++)
    {
...
void
US_Hydrodyn_Pdb_Tool_Merge::update_t_csv_range (vector<range_entry> &ranges,
  ...
  for (unsigned int i = 0; i < (unsigned int)ranges.size (); i++)
    {
      ...
      t_csv->setItem (
          pos, col_start,
          new QTableWidgetItem (QString ("%1").arg (ranges[i].start)));
      t_csv->setItem (
          pos, col_end,
          new QTableWidgetItem (QString ("%1").arg (ranges[i].end)));
      connect (t_csv, SIGNAL (cellChanged (int, int)),
               SLOT (table_value (int, int)));
    }
}
...
void
US_Hydrodyn_Pdb_Tool_Merge::sel_from_to_merge ()
{
  vector<range_entry> ranges;
...
void
US_Hydrodyn_Pdb_Tool_Merge::sel_from_to_fit ()
{
  vector<range_entry> ranges;
  sel_to_range (lv_csv_from, ranges);
...
void
US_Hydrodyn_Pdb_Tool_Merge::sel_to_to_fit ()
{
  vector<range_entry> ranges;
...
void
US_Hydrodyn_Pdb_Tool_Merge::sel_to_to_cut ()
{
  vector<range_entry> ranges;
  sel_to_range (lv_csv_to, ranges);
...
void
US_Hydrodyn_Pdb_Tool_Merge::clear ()
{
...
void
US_Hydrodyn_Pdb_Tool_Merge::validate ()
{
  running = true;
  update_enables ();
...
bool
US_Hydrodyn_Pdb_Tool_Merge::validate_commands ()
{
      ...
      else
        {
          ...
          if (!dup_extra_chains.count (cross_chain))
          ...
          else
            {
              editor_msg (
                  "red",
                  QString (us_tr ("Error: Extra chain %1 has repeated usage: "
                                  "%2 and %3, only one usage allowed\n"))
                      .arg (cross_chain)
  ...
  for (unsigned int i = 0; i < (unsigned int)fit_ranges.size (); i++)
    {
      ...
      if (!from_range_pos.count (fit_chain))
        {
          editor_msg (
              "red", QString ("Error: Chain %1 missing in \"Chains From\" PDB")
                         .arg (fit_chain));
          errors++;
        }
      else
        {
          ...
          if (from_range.start > fit_ranges[i].start
            ...
            {
              editor_msg ("red",
                          QString ("Error: Chain %1 \"Chains From\" does not "
                                   "have residues in fit range")
                              .arg (fit_ranges[i].chain));
              errors++;
            }
        }
      ...
      if (!to_range_pos.count (fit_chain))
        {
          editor_msg ("red",
                      QString ("Error: Chain %1 missing in \"Chains To\" PDB")
                          .arg (fit_chain));
          errors++;
        }
    }
      ...
      if (cut_range.start || cut_range.end)
        {
          if ((!cut_range.start && cut_range.end)
              || (cut_range.start && !cut_range.end))
            {
              editor_msg (
                  "red",
                  QString ("Error: Chain %1 cut must have both cut range "
                           "values set or none (zero is not a set value)")
                      .arg (cut_range.chain));
              errors++;
            }
          if (cut_range.start > cut_range.end)
            {
              editor_msg (
                  "red",
                  QString ("Error: Chain %1 cut range must be ascending")
                      .arg (cut_range.chain));
              errors++;
            }
          ...
          if (!to_range_pos.count (cut_range.chain))
          ...
          else
            {
              ...
              if (to_range.start > cut_range.start
                ...
                {
                  editor_msg ("red",
                              QString ("Error: Chain %1 \"Chains To\" do not "
                                       "have residues in entire cut range")
                                  .arg (cut_range.chain));
                  errors++;
                }
              else
                  ...
                  if (to_range.start != cut_range.start
                      && to_range.end != cut_range.end)
                    {
                      editor_msg (
                          "red",
                          QString (
                              "Error: Chain %1 \"Chains To\" cut start or cut "
                              "end must equal one end of the chain")
                              .arg (cut_range.chain));
                      errors++;
                    }
                  else
                    {
                      ...
                      if (to_range.start == cut_range.start
                        ...
                        {
                          editor_msg ("red",
                                      QString ("Error: Chain %1 \"Chains To\" "
                                               "can not cut entire chain")
                                          .arg (cut_range.chain));
                          errors++;
                        }
                      else
                        {
                          ...
                          if (to_range.start > to_range.end)
                            ...
                            {
                              to_ranges[to_range_pos[cut_range.chain]]
                                  = to_range;
                            }
                        }
                    }
      ...
      if (merge_range.start || merge_range.end)
        {
          if ((!merge_range.start && merge_range.end)
              || (merge_range.start && !merge_range.end))
            {
              editor_msg (
                  "red",
                  QString (
                      "Error: Chain %1 merge must have both merge range "
                      "values set or none of them (zero is not a set value)")
                      .arg (merge_range.chain));
              errors++;
            }
          if (merge_range.start > merge_range.end)
            {
              editor_msg (
                  "red",
                  QString ("Error: Chain %1 merge range must be ascending")
                      .arg (merge_range.chain));
              errors++;
            }
          if (!from_range_pos.count (merge_range.chain))
            {
              editor_msg (
                  "red",
                  QString ("Error: Chain %1 missing in \"Chains From\" PDB")
                      .arg (merge_range.chain));
              errors++;
            }
          else
            {
              range_entry from_range
                  = from_ranges[from_range_pos[merge_range.chain]];
              ...
              if (from_range.start > merge_range.start
                  || from_range.end < merge_range.end)
                {
                  editor_msg (
                      "red",
                      QString ("Error: Chain %1 \"Chains From\" does not have "
                               "residues in entire merge range")
                          .arg (merge_range.chain));
                  errors++;
                }
            }
          ...
          if (!to_range_pos.count (merge_range.chain))
            ...
            {
              range_entry to_range
                  = to_ranges[to_range_pos[merge_range.chain]];
              // make sure merge area NOT present in "to" , could happen at
              // either end
              if (to_range.start <= merge_range.end
                  && to_range.end >= merge_range.start)
                {
                  editor_msg (
                      "red",
                      QString ("Error: Chain %1 \"Chains To\" has values in "
                               "merge range and therefore needs to be cut")
                          .arg (merge_range.chain));
                  errors++;
                }
            }
      ...
      if (fit_range.start || fit_range.end)
        {
          if ((!fit_range.start && fit_range.end)
            ...
            {
              editor_msg ("red",
                          QString ("Error: Chain %1 fit range must have both "
                                   "value set or none (zero not a set value)")
                              .arg (fit_range.chain));
              errors++;
            }
          if (fit_range.start > fit_range.end)
            {
              editor_msg (
                  "red",
                  QString ("Error: Chain %1 fit range must be ascending")
                      .arg (fit_range.chain));
              errors++;
            }
          if (!from_range_pos.count (fit_range.chain))
            ...
            {
              range_entry from_range
                  = from_ranges[from_range_pos[fit_range.chain]];
              // make sure fit area present in "from"
              if (from_range.start > fit_range.start
                  || from_range.end < fit_range.end)
                {
                  editor_msg ("red",
                              QString ("Error: Chain %1 \"Chains From\" does "
                                       "not have residues in entire fit range")
                                  .arg (fit_range.chain));
                  errors++;
                }
            }
          ...
          if (!to_range_pos.count (fit_range.chain))
          ...
          else
            {
              ...
              if (to_range.start > fit_range.start
                ...
                {
                  editor_msg ("red",
                              QString ("Error: Chain %1 \"Chains To\" does "
                                       "not have residues in entire fit range")
                                  .arg (fit_range.chain));
                  errors++;
                }
...
void
US_Hydrodyn_Pdb_Tool_Merge::load ()
{
...
void
US_Hydrodyn_Pdb_Tool_Merge::csv_save ()
{
  QString use_dir = ((US_Hydrodyn *)us_hydrodyn)->somo_dir;
  QString filename = QFileDialog::getSaveFileName (
      this, us_tr ("Choose a filename to save the data"), use_dir,
      "*.csc *.CSC");
  ...
  for (unsigned int i = 0; i < (unsigned int)csv_commands.data.size (); i++)
    {
      qs = "";
      for (unsigned int j = 0; j < (unsigned int)csv_commands.data[i].size ();
           j++)
        {
          qs += QString ("%1%2")
                    .arg (j ? "," : "")
                    .arg (csv_commands.data[i][j]);
        }
      t << qs << Qt::endl;
    }
  f.close ();
...
void
US_Hydrodyn_Pdb_Tool_Merge::pdb_tool ()
{
  ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)->raise ();
}

void
US_Hydrodyn_Pdb_Tool_Merge::update_csv_commands_from_table ()
{
  csv_commands.data.clear ();
  csv_commands.num_data.clear ();
...
QStringList
US_Hydrodyn_Pdb_Tool_Merge::csv_parse_line (QString qs)
{
...
void
US_Hydrodyn_Pdb_Tool_Merge::update_t_csv_data ()
{
  ...
  for (unsigned int i = 0; i < (unsigned int)csv_commands.data.size (); i++)
    {
      for (unsigned int j = 0; j < (unsigned int)csv_commands.data[i].size ();
        ...
        {
          t_csv->setItem (i, j,
                          new QTableWidgetItem (csv_commands.data[i][j]));
        }
    }
}

void
US_Hydrodyn_Pdb_Tool_Merge::run_one ()
{
  ...
  csv_to = ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
               ->to_csv (lv_csv_to,
                         ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
                             ->csv2[((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
                                        ->csv2_pos],
       ...
       it != from_fit_points.end (); it++)
    {
      if (!to_fit_points.count (it->first)
          || to_fit_points[it->first].size () != it->second.size ())
        {
          editor_msg (
              "red",
              QString ("Error (run): Fit point mismatch for %1 (%2 vs %3)")
                  .arg (it->first)
                  .arg (to_fit_points[it->first].size ())
                  .arg (it->second.size ()));
          running = false;
          update_enables ();
          return;
        ...
        {
          cout << QString ("Ok (run): Fit points for %1 (%2 vs %3)\n")
                      .arg (it->first)
                      .arg (to_fit_points.count (it->first)
                                ? to_fit_points[it->first].size ()
                                : -1)
...
#if defined(USPTM_DEBUG_RUN_ONE)
                us_qdebug (
                    QString ("type 0 checking for vdw of %1 at datapos %2")
                        .arg (new_csv.data[datapos][13])
                        .arg (datapos));
#endif
                pos_vdw[datapos]
                    = usu->atom_vdw.count (new_csv.data[datapos][13])
                          ? usu->atom_vdw[new_csv.data[datapos][13]]
                          : (usu->atom_vdw.count ("default")
                                 ? usu->atom_vdw["default"]
                                 : 1.8);
              }
          }
...
#if defined(USPTM_DEBUG_RUN_ONE)
                us_qdebug (
                    QString ("type 1 checking for vdw of %1 at datapos %2")
                        .arg (new_csv.data[datapos][13])
                        .arg (datapos));
#endif
                pos_vdw[datapos]
        ...
        && chain_to_front[last_chain] == false)
      {
        ...
        for (unsigned int j = 0;
             j < (unsigned int)merge_data[last_chain].size (); j++)
          {
            ...
            pos_coord[datapos] = result[0];
#if defined(USPTM_DEBUG_RUN_ONE)
            us_qdebug (QString ("type 2 checking for vdw of %1 at datapos %2")
                           .arg (new_csv.data[datapos][13])
                           .arg (datapos));
#endif
    ...
    for (unsigned int i = 0; i < (unsigned int)csv_from.data.size (); i++)
      {
...

        {
...

          {
            ...
            pos_coord[datapos] = result[0];
#if defined(USPTM_DEBUG_RUN_ONE)
            us_qdebug (QString ("type 3 checking for vdw of %1 at datapos %2")
                           .arg (new_csv.data[datapos][13])
                           .arg (datapos));
#endif
            pos_vdw[datapos] = usu->atom_vdw.count (new_csv.data[datapos][13])
    ...
    if (cb_filter->isChecked ())
      {
            ...
            if (!pos_type.count (i))
              {
                ...
                pos_type[i] = 4;
                pos_coord[i] = p;
                pos_vdw[i] = usu->atom_vdw.count (new_csv.data[i][13])
        ...
        for (map<unsigned int, int>::iterator it = pos_type.begin ();
             it != pos_type.end (); ++it)
          {
            us_qdebug (QString ("pos %1 type %2 vdw %3 coord %4 %5 %6")
                           .arg (it->first)
                           .arg (it->second)
...

        {
          ...
          for (map<int, vector<unsigned int> >::iterator it
               = types_to_pos.begin ();
               it != types_to_pos.end () && !filtered; ++it)
            {
              if (it->first == 4)
                {
                  continue;
              ...
              for (int j = 0; j < (int)types_to_pos[4].size () && !filtered;
                   ++j)
                {
                  for (int k = 0; k < (int)it->second.size (); ++k)
                    {
                      x2 = pos_coord[types_to_pos[4][j]].axis[0]
                           - pos_coord[it->second[k]].axis[0];

                      y2 = pos_coord[types_to_pos[4][j]].axis[1]
                           - pos_coord[it->second[k]].axis[1];

                      z2 = pos_coord[types_to_pos[4][j]].axis[2]
                      ...
                      if (x2 + y2 + z2 <= vdw * vdw)
                        {
                          ...
                          unsigned int res4no
                              = new_csv.data[types_to_pos[4][j]][3].toUInt ();
                          unsigned int resano
                              = new_csv.data[it->second[k]][3].toUInt ();

#if defined(USPTM_DEBUG_RUN_ONE)
                          us_qdebug (
                              QString (
                                  "chain4 %1 resno %2 chainalt %3 resno %4")
                                  .arg (chain4)
                                  .arg (res4no)
                                  .arg (chaina)
                                  .arg (resano));
                          ...
                          if (!excluded)
                            {
                              QString msg
                                  = QString (
                                        "first steric clash between types 4 "
                                        "%1 at pstns %2 %3 vdws %4 %5")
                                        .arg (it->first)
                              ...
                              }
                              {
                                ...
                                us_qdebug (QString ("").sprintf (
                                    ...
                                    "ATOM  %5d%5s%4s %1s%4d    "
                                    "%8.3f%8.3f%8.3f%6.2f%6.2f          %2s\n",
                                    it->first, new_csv.data[i][5].toUInt (),
                                    new_csv.data[i][4].toLatin1 ().data (),
                                    new_csv.data[i][2].toLatin1 ().data (),
                                    new_csv.data[i][1].toLatin1 ().data (),
                                    new_csv.data[i][3].toUInt (),
...
void
US_Hydrodyn_Pdb_Tool_Merge::extra_chains ()
{
  ...
  // for each existing chain and each extra chain, add cross chain if it
  // doesn't already exist
  make_csv_chain_map ();

  for (map<QString, unsigned int>::iterator it = csv_chain_map.begin ();
       it != csv_chain_map.end (); it++)
    {
      if (it->first.length () <= 1)
        {
          for (unsigned int i = 0; i < (unsigned int)extra_chains_list.size ();
               i++)
            {
              QString cross_chain = QString ("%1+%2").arg (it->first).arg (
                  extra_chains_list[i]);
              if (!csv_chain_map.count (cross_chain))
                {
                  ...
                  t_csv->setItem (pos, 3,
                                  new QTableWidgetItem (
                                      t_csv->item (it->second, 3)->text ()));
                  t_csv->setItem (pos, 4,
                                  new QTableWidgetItem (
                                      t_csv->item (it->second, 4)->text ()));
                  fill_csv_empty_items ();
                }
            }
        }
...
void
US_Hydrodyn_Pdb_Tool_Merge::only_closest ()
{
  if (!extra_chains_list.size ())
    {
      return;
  ...
  for (map<QString, QStringList>::iterator it = chains_to_check.begin ();
       it != chains_to_check.end (); it++)
    {
      ...
      for (unsigned int i = 0; i < (unsigned int)it->second.size (); i++)
        {
          ...
          editor_msg (
              "dark gray",
              QString (us_tr ("Finding closest pair of chain %1 and %2"))
                  .arg (it->first)
                  .arg (it->second[i]));
          ...
          if (0)
            {
              ...
              chains << it->second[i];
#if defined(USPTM_TIMERS)
              us_timers.start_timer (
                  "only_closest() select chains in pdb editor");
#endif
              ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
                  ->select_chain (lv_csv_from, chains);
#if defined(USPTM_TIMERS)
              us_timers.end_timer (
                  "only_closest() select chains in pdb editor");
#endif
...
#endif
          double distance = ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
                                ->minimum_pair_distance (
                                    lv_csv_from, it->first, it->second[i],
                                    key_chain_a, key_chain_b);
#if defined(USPTM_TIMERS)
      ...
      editor_msg (
          "blue",
          QString (us_tr ("Closest chain to %1 [%2] is [%3] distance %4"))
              .arg (it->first)
              .arg (minimum_key_a)
              .arg (minimum_key_b)
      ...
      {
        QStringList chain_a_list
            = (minimum_key_a).split ("~", Qt::SkipEmptyParts);
        QStringList chain_b_list
            = (minimum_key_b).split ("~", Qt::SkipEmptyParts);
        ...
        // "?"; residue_a.replace( QRegExp( "^\\D*" ), "" ); unsigned int
        // residue_a_pos = residue_a.toUInt();
        ...
        t_csv->setItem (
            pos, 3,
            new QTableWidgetItem (QString ("%1").arg (residue_b_pos - 1)));
        t_csv->setItem (
            pos, 4,
            new QTableWidgetItem (QString ("%1").arg (residue_b_pos + 1)));
        fill_csv_empty_items ();
...
void
US_Hydrodyn_Pdb_Tool_Merge::delete_row ()
{
  for (int i = t_csv->rowCount () - 1; i >= 0; i--)
    {
...
void
US_Hydrodyn_Pdb_Tool_Merge::update_cache_range ()
{
  if (!cache_range_ok)
    {
      vector<range_entry> from_ranges;
...
void
US_Hydrodyn_Pdb_Tool_Merge::table_value (int row, int col)
{
  update_cache_range ();
          ...
          > t_csv->item (row, col)->text ().toUInt ())
        {
          ...
          t_csv->setItem (
              ...
              new QTableWidgetItem (QString ("%1").arg (
                  cache_from_ranges[cache_from_range_pos[actual_chain]]
                      .start)));
          connect (t_csv, SIGNAL (cellChanged (int, int)),
                   SLOT (table_value (int, int)));
        }
      if (cache_from_ranges[cache_from_range_pos[actual_chain]].end
          < t_csv->item (row, col)->text ().toUInt ())
        {
          ...
          t_csv->setItem (
              row, col,
              new QTableWidgetItem (QString ("%1").arg (
                  cache_from_ranges[cache_from_range_pos[actual_chain]].end)));
          connect (t_csv, SIGNAL (cellChanged (int, int)),
                   SLOT (table_value (int, int)));
        }
      if (col == 3)
        {
          if (t_csv->item (row, col)->text ().toUInt ()
              > t_csv->item (row, col + 1)->text ().toUInt ())
            {
              ...
              t_csv->setItem (
                  row, col + 1,
                  new QTableWidgetItem (t_csv->item (row, col)->text ()));
              connect (t_csv, SIGNAL (cellChanged (int, int)),
                       SLOT (table_value (int, int)));
            }
          return;
  ...
  if (!cache_use_start.count (chain))
    {
      editor_msg (
          "red",
          QString (
              us_tr ("Internal error: chain start map entry missing  <%1>"))
              .arg (chain));
      return;
    }
...
unsigned int
US_Hydrodyn_Pdb_Tool_Merge::residue_length (bool use_from, QString chain,
                                            unsigned int start_residue,
                                            unsigned int end_residue)
{
  update_cache_range ();
  if (use_from ? !cache_from_range_pos.count (chain)
               : !cache_to_range_pos.count (chain))
    {
      editor_msg (
          "red",
  ...
  range_entry *re = use_from ? &cache_from_ranges[cache_from_range_pos[chain]]
...
unsigned int
US_Hydrodyn_Pdb_Tool_Merge::residue_offset_position (
    bool use_from, QString chain, unsigned int start_residue, int offset)
{
  if (offset == 0)
    {
...
void
US_Hydrodyn_Pdb_Tool_Merge::recalc_from_merge (int row, int col)
{
  if (row >= t_csv->rowCount ())
    {
      editor_msg ("red",
                  QString (us_tr ("Internal error: row out of range  <%1>"))
                      .arg (row + 1));
      return;
    }
  ...
  if (extra_chain)
    {
      // editor_msg( "red", QString( us_tr( "Internal error: update merge, but
      // merge not valid for row <%1>" ) ).arg( row + 1 ) );
      return;
    }
  ...
  unsigned int current_fit_length_from
      = residue_length (true, chain, current_fit_start, current_fit_end);
  unsigned int current_fit_length_to
      = residue_length (false, chain, current_fit_start, current_fit_end);
  unsigned int current_fit_length
      = current_fit_length_from < current_fit_length_to
            ? current_fit_length_from
            : current_fit_length_to;

  unsigned int current_merge_length
      = residue_length (true, chain, current_merge_start, current_merge_end);
  unsigned int current_cut_length
      = residue_length (false, chain, current_cut_start, current_cut_end);
  ...
  unsigned int min_merge_start
      = use_start ? from_range.start
                  : residue_offset_position (true, chain, from_range.start, 2);
  unsigned int max_merge_end
      = use_start ? residue_offset_position (true, chain, from_range.end, -2)
                  : from_range.end;
  ...
  if (use_start)
    {
      if (to_range.start <= current_merge_end
          && current_cut_end <= current_merge_end)
        {
          current_cut_start = to_range.start;
          current_cut_end = current_merge_end;
          while (to_range.gaps.count (current_cut_end)
    ...
    {
      if (to_range.end >= current_merge_start
          && (!current_cut_start || current_cut_start >= current_merge_start))
        {
          current_cut_start = current_merge_start;
          while (to_range.gaps.count (current_cut_start)
                 && current_cut_start > to_range.start)
            {
              current_cut_start--;
            }
      ...
      if (use_start)
        {
          if (current_fit_start <= current_cut_end)
            {
              current_fit_start
                  = residue_offset_position (true, chain, current_cut_end, 1);
              unsigned int alt_fit_start
                  = residue_offset_position (false, chain, current_cut_end, 1);
              if (current_fit_start < alt_fit_start)
                {
                  current_fit_start = alt_fit_start;
                }
              current_fit_end = residue_offset_position (
                  true, chain, current_fit_start, current_fit_length - 1);
              unsigned int alt_fit_end = residue_offset_position (
                  false, chain, current_fit_start, current_fit_length - 1);
              if (current_fit_end < alt_fit_end)
                {
                  current_fit_end = alt_fit_end;
                }
      ...
      else
        {
          if (current_fit_end >= current_cut_start)
            {
              current_fit_end = residue_offset_position (
                  true, chain, current_cut_start, -1);
              unsigned int alt_fit_end = residue_offset_position (
                  false, chain, current_cut_start, -1);
              if (current_fit_end < alt_fit_end)
                {
                  current_fit_end = alt_fit_end;
                }
              current_fit_start
                  = residue_offset_position (true, chain, current_fit_end,
                                             -(int)(current_fit_length - 1));
              unsigned int alt_fit_start
                  = residue_offset_position (false, chain, current_fit_end,
                                             -(int)(current_fit_length - 1));
              if (current_fit_start < alt_fit_start)
                {
                  current_fit_start = alt_fit_start;
  ...
  if (any_changed)
    {
      ...
      if (current_merge_start_changed)
        {
          QString toset = current_merge_start
                              ? QString ("%1").arg (current_merge_start)
                              : "";
          t_csv->setItem (row, 1, new QTableWidgetItem (toset));
        }
      if (current_merge_end_changed)
        {
          QString toset = current_merge_end
                              ? QString ("%1").arg (current_merge_end)
                              : "";
      ...
      if (current_fit_start_changed)
        {
          QString toset = current_fit_start
                              ? QString ("%1").arg (current_fit_start)
                              : "";
      ...
      if (current_fit_end_changed)
        {
          QString toset
              = current_fit_end ? QString ("%1").arg (current_fit_end) : "";
          t_csv->setItem (row, 4, new QTableWidgetItem (toset));
        }
      if (current_cut_start_changed)
        {
          QString toset = current_cut_start
                              ? QString ("%1").arg (current_cut_start)
                              : "";
      ...
      if (current_cut_end_changed)
        {
          QString toset
              = current_cut_end ? QString ("%1").arg (current_cut_end) : "";
...
void
US_Hydrodyn_Pdb_Tool_Merge::recalc_from_fit (int row, int col)
{
  if (row >= t_csv->rowCount ())
    {
      editor_msg ("red",
  ...
  if (extra_chain)
    {
      // editor_msg( "red", QString( us_tr( "Internal error: update fit, but
      // fit not valid for row <%1>" ) ).arg( row + 1 ) );
      return;
    }
  ...
  unsigned int min_fit_start
      = from_range.start; // residue_offset_position( true , chain,
                          // from_range.start, 1 );
  unsigned int alt_min_fit_start
      = to_range.start; // residue_offset_position( false, chain,
                        // to_range.start  , 1 );
  ...
  unsigned int max_fit_end = from_range.end; // residue_offset_position( true ,
                                             // chain, from_range.end , -1 );
  unsigned int alt_max_fit_end
      = to_range.end; // residue_offset_position( false, chain, to_range.end ,
                      // -1 );
  ...
  if (current_fit_length < 3)
    {
      if (col == 3)
        {
          current_fit_end
              = residue_offset_position (false, chain, current_fit_start, 3);
          unsigned int alt_fit_end
              = residue_offset_position (true, chain, current_fit_start, 3);
        ...
        {
          current_fit_start
              = residue_offset_position (false, chain, current_fit_end, -3);
          unsigned int alt_fit_start
              = residue_offset_position (true, chain, current_fit_end, -3);
  ...
  current_fit_length_to
      = residue_length (false, chain, current_fit_start, current_fit_end);
  current_fit_length = current_fit_length_from < current_fit_length_to
                           ? current_fit_length_from
                           : current_fit_length_to;
  if (current_fit_length < 3)
    {
      ...
      editor_msg (
          "red",
          us_tr ("Error: could not find 3 residues to fit in this chain\n"));
      current_fit_start = 0;
      current_fit_end = 0;
      current_fit_start_changed = true;
      current_fit_end_changed = true;
      ...
      if (current_cut_end >= current_fit_start)
        {
          ...
          current_cut_start = to_range.start;
          current_cut_end
              = residue_offset_position (true, chain, current_fit_start, -1);
          while (to_range.gaps.count (current_cut_end)
                 && current_cut_end >= to_range.start)
            {
              current_cut_end--;
            }
          current_cut_start_changed = true;
      ...
      if (current_cut_start && current_cut_start <= current_fit_end)
        {
          cout << "m5\n";
          current_cut_start
              = residue_offset_position (true, chain, current_fit_end, 1);
          while (to_range.gaps.count (current_cut_start)
                 && current_cut_start < to_range.end)
            {
              current_cut_start++;
            }
          current_cut_end = to_range.end;
      ...
      if (current_merge_end >= current_fit_start)
        {
          current_merge_end
              = residue_offset_position (true, chain, current_fit_start, -1);
          while (from_range.gaps.count (current_merge_end)
                 && current_merge_end >= from_range.start)
            {
              current_merge_end--;
            }
          current_merge_end_changed = true;
      ...
      if (current_merge_start <= current_fit_end)
        {
          current_merge_start
              = residue_offset_position (true, chain, current_fit_end, 1);
          while (from_range.gaps.count (current_merge_start)
                 && current_merge_start < from_range.end)
            {
              current_merge_start++;
            }
          current_merge_start_changed = true;
...
void
US_Hydrodyn_Pdb_Tool_Merge::recalc_from_cut (int row, int col)
{
  if (row >= t_csv->rowCount ())
    {
      editor_msg ("red",
  ...
  if (extra_chain)
    {
      // editor_msg( "red", QString( us_tr( "Internal error: update cut, but
      // cut not valid for row <%1>" ) ).arg( row + 1 ) );
      return;
    }
      ...
      if (current_merge_start < current_cut_start)
        {
          ...
          while (from_range.gaps.count (current_merge_start)
                 && current_merge_start <= from_range.start)
            {
              current_merge_start++;
            }
          current_merge_start_changed = true;
  ...
  if (use_start)
  ...
  else
    {
      if (current_fit_end >= current_cut_start)
        {
          current_fit_end
              = residue_offset_position (true, chain, current_cut_start, -1);
          unsigned int alt_fit_end
              = residue_offset_position (false, chain, current_cut_start, -1);
          if (current_fit_end < alt_fit_end)
            {
              current_fit_end = alt_fit_end;
            }
          current_fit_start = residue_offset_position (
              true, chain, current_fit_end, -(int)(current_fit_length - 1));
          unsigned int alt_fit_start = residue_offset_position (
              false, chain, current_fit_end, -(int)(current_fit_length - 1));
          if (current_fit_start < alt_fit_start)
            {
              current_fit_start = alt_fit_start;
            }
...
bool
US_Hydrodyn_Pdb_Tool_Merge::get_chains (QString chain)
{
  if (chain.length () <= 1)
    {
      return false;
  ...
  if (qsl.size () != 2)
    {
      editor_msg ("red",
                  QString (us_tr ("Error: Extra chain not in proper format "
                                  "<%1>, please label each chain"))
                      .arg (chain));
      return false;
    }
  return true;
...
bool
US_Hydrodyn_Pdb_Tool_Merge::get_chains (QString chain, QString &fit_chain,
                                        QString &cross_chain)
{
  fit_chain = chain;
...
void
US_Hydrodyn_Pdb_Tool_Merge::fill_csv_empty_items ()
{
...
void
US_Hydrodyn_Pdb_Tool_Merge::info_csv (const QString &msg)
{
  int cols = (int)t_csv->columnCount ();
  int rows = (int)t_csv->rowCount ();
  QTextStream (stdout)
