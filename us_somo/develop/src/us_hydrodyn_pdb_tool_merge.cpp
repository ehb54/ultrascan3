...
class uhptm_sortable_uint {
 public:
  unsigned int x;
  bool operator<(const uhptm_sortable_uint &objIn) const { return x < objIn.x; }
};
...
void US_Hydrodyn_Pdb_Tool_Merge::setupGUI() {
  ...
  t_csv = new QTableWidget(csv_commands.data.size(), csv_commands.header.size(),
                           this);
  ...
  //t_csv->columnCount(); ++j ) { t_csv->item( i, j )->setFlags( t_csv->item( i,
  //j )->flags() ^ Qt::ItemIsEditable ); } } };
...
void US_Hydrodyn_Pdb_Tool_Merge::cancel() { close(); }
...
void US_Hydrodyn_Pdb_Tool_Merge::start() {
      ...
      if (qs.contains(rx_model)) {
        ...
        if (model_names.count(model_name)) {
          ...
          if (!dup_model_name_msg_done) {
            ...
            // editor_msg( "red", us_tr( "Duplicate or missing model names
  ...
  if (!fn.open(QIODevice::WriteOnly)) {
    QMessageBox::warning(
        this, "US-SOMO: PDB Editor : Cut/Splice",
  ...
  if (cb_filter->isChecked()) {
    tsof << QString(
        "HEADER    Processed by US-SOMO Cut/Splice and *filtered* due to "
        "steric clashes\n");
    tsof << model_header;
  }
...
void US_Hydrodyn_Pdb_Tool_Merge::trial() {
  ...
  csv csv_from =
      ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
          ->to_csv(lv_csv_from, ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)->csv1,
                   false);
  ...
  csv csv_to =
      ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
          ->to_csv(
              lv_csv_to,
              ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
                  ->csv2[((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)->csv2_pos],
              false);
      ...
      if ((use_start && it->second.start < to_range.start) ||
        ...
        if (!fit_map.count(fit_chain)) {
          editor_msg(
              "red",
              QString(
                  us_tr("Internal error: chain fit map entry missing  <%1>"))
                  .arg(fit_chain));
          continue;
        }
        editor_msg("blue", QString(us_tr("Notice: setting fitting for extra "
                                         "chain %1 to fit region for chain %2"))
                               .arg(cross_chain)
                               .arg(fit_chain));
  ...
  // extract the merge & fit bits
  {
      ...
      if (cross_fit_chains.count(chain)) {
        for (unsigned int j = 0;
          ...
          if (!extra_chain_reported.count(chain)) {
            editor_msg("blue",
                       QString("Adding extra fit chains for %1").arg(chain));
      ...
      if (cross_cross_chains.count(chain)) {
        for (unsigned int j = 0;
          ...
          if (!extra_cross_chain_reported.count(chain)) {
            editor_msg("blue",
                       QString("Adding extra merge chains for %1").arg(chain));
    ...
    //      ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)->csv_clipboard.name = "fit
    //      bits of " + csv_from.name;
    //      ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)->csv2_paste_new();
  }
  ...
  for (unsigned int i = 0; i < (unsigned int)csv_to.data.size(); i++) {
    ...
    if (cross_fit_chains.count(chain)) {
      for (unsigned int j = 0; j < (unsigned int)cross_fit_chains[chain].size();
           j++) {
        QString fit_chain = chain;
        QString cross_chain = cross_fit_chains[fit_chain][j];
        QString chain = QString("%1+%2").arg(fit_chain).arg(cross_chain);
        if (!extra_chain_reported.count(chain)) {
          editor_msg("blue",
                     QString("Adding to extra fit chains for %1").arg(chain));
          extra_chain_reported[chain] = true;
        }
...

  {
    ...
    for (unsigned int i = 0; i < (unsigned int)csv_to.data.size(); i++) {
      ...
      if (chain != last_chain && merge_data.count(last_chain) &&
          !chain_added.count(last_chain) && chain_to_front.count(last_chain) &&
      ...
      if ((merge_data.count(chain) && !chain_added.count(chain) &&
        ...
        // xform & add chain
        for (unsigned int j = 0; j < (unsigned int)merge_data[chain].size();
             j++) {
          vector<point> ps;
          point p;
          ...
          QString error_msg;
          if (!((US_Hydrodyn *)us_hydrodyn)
                   ->atom_align(from_fit_points[chain], to_fit_points[chain],
                                ps, result, error_msg)) {
            editor_msg("red", error_msg);
            running = false;
            update_enables();
    ...
    if (merge_data.count(last_chain) && !chain_added.count(last_chain) &&
      ...
      // %1\n").arg(last_chain);
      chain_added[last_chain] = true;
      for (unsigned int j = 0; j < (unsigned int)merge_data[last_chain].size();
           j++) {
        // not yet xformed
        vector<point> ps;
        point p;
    ...
    //  for ( map < QString, vector < vector < QString > > > merge_data.begin();
    //  etc
    ...
    for (unsigned int i = 0; i < (unsigned int)csv_from.data.size(); i++) {
      ...
      if (cross_cross_chains[chain].size() != 1) {
        editor_msg("red", QString(us_tr("Error: only one fitting chain "
...

      {
        ...
        if (!from_fit_points.count(chain) || !to_fit_points.count(chain)) {
          editor_msg(
              "red",
              QString(us_tr("Error: missing fit points for %1")).arg(chain));
          running = false;
          update_enables();
          return;
        }
    ...
    for (unsigned int i = 0; i < (unsigned int)new_csv.data.size(); i++) {
      ...
      if (chain != last_chain) {
        // editor_msg("red", QString("chain <%1> last chain <%2>").arg( chain
        // ).arg(last_chain));
        atompos = 0;
        last_chain = chain;
      }
...
void US_Hydrodyn_Pdb_Tool_Merge::chains_from() {
  QString filename;
  ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)->load(lv_csv_from, filename, true);
...
void US_Hydrodyn_Pdb_Tool_Merge::sel_auto() {
  ...
  for (int row = 0; row < t_csv->rowCount(); row++) {
    ...
    if (!cache_from_range_pos.count(chain) ||
      ...
      if (current_merge_start > current_merge_end) {
        ...
        t_csv->setItem(
            row, 2, new QTableWidgetItem(QString("%1").arg(current_merge_end)));
        table_value(row, 2);
      }
      ...
      if (current_fit_length < 3) {
        ...
        t_csv->setItem(
            row, 3, new QTableWidgetItem(QString("%1").arg(current_fit_start)));
        t_csv->setItem(
            row, 4, new QTableWidgetItem(QString("%1").arg(current_fit_end)));
      }
    }
  }
  ...
  for (map<QString, bool>::iterator it = cache_use_start.begin();
    ...
    cout << it->first << " " << QString("%1").arg(it->second ? "true" : "false")
         << endl;
  }
#endif
...
void US_Hydrodyn_Pdb_Tool_Merge::sel_to_range(QTreeWidget *lv,
...
void US_Hydrodyn_Pdb_Tool_Merge::update_t_csv_range(vector<range_entry> &ranges,
  ...
  for (unsigned int i = 0; i < (unsigned int)ranges.size(); i++) {
    ...
    t_csv->setItem(pos, col_start,
                   new QTableWidgetItem(QString("%1").arg(ranges[i].start)));
    t_csv->setItem(pos, col_end,
...
bool US_Hydrodyn_Pdb_Tool_Merge::validate_commands() {
  ...
  for (unsigned int i = 0; i < (unsigned int)csv_commands.data.size(); i++) {
    ...
    if (!get_chains(csv_commands.data[i][0], fit_chain, cross_chain)) {
      ...
      if (!dup_extra_chains.count(cross_chain)) {
        ...
        editor_msg("red",
                   QString(us_tr("Error: Extra chain %1 has repeated usage: %2 "
                                 "and %3, only one usage allowed\n"))
                       .arg(cross_chain)
  ...
  for (unsigned int i = 0; i < (unsigned int)fit_ranges.size(); i++) {
    ...
    if (!from_range_pos.count(fit_chain)) {
      ...
      if (from_range.start > fit_ranges[i].start ||
        ...
        editor_msg("red", QString("Error: Chain %1 \"Chains From\" does not "
    ...
    if (!to_range_pos.count(fit_chain)) {
      editor_msg("red", QString("Error: Chain %1 missing in \"Chains To\" PDB")
                            .arg(fit_chain));
      errors++;
    }
  }
  ...
  for (unsigned int i = 0; i < (unsigned int)merge_ranges.size(); i++) {
    ...
    if (cut_range.start || cut_range.end) {
      ...
      if (cut_range.start > cut_range.end) {
        editor_msg("red", QString("Error: Chain %1 cut range must be ascending")
                              .arg(cut_range.chain));
        errors++;
      }
      ...
      if (!to_range_pos.count(cut_range.chain)) {
        ...
        if (to_range.start > cut_range.start || to_range.end < cut_range.end) {
          editor_msg("red", QString("Error: Chain %1 \"Chains To\" do not have "
                                    "residues in entire cut range")
                                .arg(cut_range.chain));
          errors++;
        } else {
          ...
          if (to_range.start != cut_range.start &&
            ...
            editor_msg("red",
                       QString("Error: Chain %1 \"Chains To\" cut start or cut "
            ...
            if (to_range.start == cut_range.start &&
              ...
              editor_msg(
                  "red",
                  QString(
                      "Error: Chain %1 \"Chains To\" can not cut entire chain")
                      .arg(cut_range.chain));
              errors++;
            } else {
              // apply cut
              ...
              if (to_range.start > to_range.end) {
                editor_msg(
                    "red",
                    QString("Error: Chain %1 \"Chains To\" chain cut error")
                        .arg(cut_range.chain));
                errors++;
              } else {
    ...
    if (merge_range.start || merge_range.end) {
      if ((!merge_range.start && merge_range.end) ||
        ...
        editor_msg(
            "red",
            QString("Error: Chain %1 merge must have both merge range values "
                    "set or none of them (zero is not a set value)")
                .arg(merge_range.chain));
        errors++;
      }
      if (merge_range.start > merge_range.end) {
      ...
      if (!from_range_pos.count(merge_range.chain)) {
        ...
        if (from_range.start > merge_range.start ||
          ...
          editor_msg("red", QString("Error: Chain %1 \"Chains From\" does not "
                                    "have residues in entire merge range")
                                .arg(merge_range.chain));
          errors++;
        }
      }
    ...
    if (fit_range.start || fit_range.end) {
      if ((!fit_range.start && fit_range.end) ||
        ...
        editor_msg("red", QString("Error: Chain %1 fit range must have both "
                                  "value set or none (zero not a set value)")
                              .arg(fit_range.chain));
        errors++;
      }
      if (fit_range.start > fit_range.end) {
        editor_msg("red", QString("Error: Chain %1 fit range must be ascending")
                              .arg(fit_range.chain));
        errors++;
      }
      if (!from_range_pos.count(fit_range.chain)) {
        ...
        if (from_range.start > fit_range.start ||
          ...
          editor_msg("red", QString("Error: Chain %1 \"Chains From\" does not "
                                    "have residues in entire fit range")
                                .arg(fit_range.chain));
          errors++;
        }
      }
      ...
      if (!to_range_pos.count(fit_range.chain)) {
        ...
        if (to_range.start > fit_range.start || to_range.end < fit_range.end) {
          editor_msg("red", QString("Error: Chain %1 \"Chains To\" does not "
                                    "have residues in entire fit range")
                                .arg(fit_range.chain));
          errors++;
        }
...
void US_Hydrodyn_Pdb_Tool_Merge::load() {
  ...
  QString filename =
      QFileDialog::getOpenFileName(this, windowTitle(), use_dir, "*.csc *.CSC");
    ...
    for (unsigned int i = 0; i < (unsigned int)csv_to.data.size(); i++) {
        ...
        for (unsigned int j = 0;
...
#if defined(USPTM_DEBUG_RUN_ONE)
          us_qdebug(QString("type 0 checking for vdw of %1 at datapos %2")
          ...
          pos_vdw[datapos] =
              usu->atom_vdw.count(new_csv.data[datapos][13])
                  ...
                  : (usu->atom_vdw.count("default") ? usu->atom_vdw["default"]
                                                    : 1.8);
        }
      }
      ...
      if ((merge_data.count(chain) && !chain_added.count(chain) &&
        ...
        for (unsigned int j = 0; j < (unsigned int)merge_data[chain].size();
          ...
          pos_coord[datapos] = result[0];
#if defined(USPTM_DEBUG_RUN_ONE)
          us_qdebug(QString("type 1 checking for vdw of %1 at datapos %2")
                        .arg(new_csv.data[datapos][13])
                        .arg(datapos));
#endif
          pos_vdw[datapos] =
        ...
        for (map<int, vector<unsigned int> >::iterator it =
          ...
          for (int j = 0; j < (int)types_to_pos[4].size() && !filtered; ++j) {
            for (int k = 0; k < (int)it->second.size(); ++k) {
              ...
              vdw = pos_vdw[types_to_pos[4][j]] + pos_vdw[it->second[k]];
              ...
              if (x2 + y2 + z2 <= vdw * vdw) {
                ...
                if ((fit_map.count(chain4) && fit_map[chain4].start <= res4no &&
                     fit_map[chain4].end >= res4no) ||
                    (fit_map.count(chaina) && fit_map[chaina].start <= resano &&
                ...
                if (!excluded) {
                  QString msg = QString(
                                    "first steric clash between types 4 %1 at "
                                    "pstns %2 %3 vdws %4 %5")
                                    .arg(it->first)
...
void US_Hydrodyn_Pdb_Tool_Merge::extra_chains() {
  ...
  // for each existing chain and each extra chain, add cross chain if it doesn't
  // already exist
  make_csv_chain_map();
  ...
  for (map<QString, unsigned int>::iterator it = csv_chain_map.begin();
    ...
    if (it->first.length() <= 1) {
      for (unsigned int i = 0; i < (unsigned int)extra_chains_list.size();
           i++) {
        QString cross_chain =
            QString("%1+%2").arg(it->first).arg(extra_chains_list[i]);
        if (!csv_chain_map.count(cross_chain)) {
          ...
          t_csv->setItem(
              pos, 3, new QTableWidgetItem(t_csv->item(it->second, 3)->text()));
          t_csv->setItem(
              pos, 4, new QTableWidgetItem(t_csv->item(it->second, 4)->text()));
          fill_csv_empty_items();
        }
      }
    }
...
void US_Hydrodyn_Pdb_Tool_Merge::only_closest() {
  ...
  for (map<QString, QStringList>::iterator it = chains_to_check.begin();
    ...
    for (unsigned int i = 0; i < (unsigned int)it->second.size(); i++) {
      ...
      double distance =
          ((US_Hydrodyn_Pdb_Tool *)pdb_tool_window)
              ->minimum_pair_distance(lv_csv_from, it->first, it->second[i],
...

    {
      ...
      t_csv->setItem(pos, 0, new QTableWidgetItem(cross_chain));
      t_csv->setItem(
          pos, 3, new QTableWidgetItem(QString("%1").arg(residue_b_pos - 1)));
      t_csv->setItem(
          pos, 4, new QTableWidgetItem(QString("%1").arg(residue_b_pos + 1)));
      fill_csv_empty_items();
...
void US_Hydrodyn_Pdb_Tool_Merge::table_value(int row, int col) {
    ...
    if (cache_from_ranges[cache_from_range_pos[actual_chain]].start >
      ...
      t_csv->setItem(
          ...
          new QTableWidgetItem(QString("%1").arg(
              cache_from_ranges[cache_from_range_pos[actual_chain]].start)));
      connect(t_csv, SIGNAL(cellChanged(int, int)),
              SLOT(table_value(int, int)));
    }
  ...
  if (!cache_from_range_pos.count(chain) || !cache_to_range_pos.count(chain)) {
    editor_msg(
        "red",
        QString(us_tr("Internal error: chain dereferencing <%1>")).arg(chain));
    return;
  }
  ...
  if (!cache_use_start.count(chain)) {
    editor_msg(
        "red",
        QString(us_tr("Internal error: chain start map entry missing  <%1>"))
            .arg(chain));
    return;
  }
...
unsigned int US_Hydrodyn_Pdb_Tool_Merge::residue_length(
    bool use_from, QString chain, unsigned int start_residue,
...
unsigned int US_Hydrodyn_Pdb_Tool_Merge::residue_offset_position(
...
void US_Hydrodyn_Pdb_Tool_Merge::recalc_from_merge(int row, int col) {
  if (row >= t_csv->rowCount()) {
    editor_msg(
        "red",
        QString(us_tr("Internal error: row out of range  <%1>")).arg(row + 1));
    return;
  }
  ...
  unsigned int current_fit_length_to =
      residue_length(false, chain, current_fit_start, current_fit_end);
  unsigned int current_fit_length =
      current_fit_length_from < current_fit_length_to ? current_fit_length_from
                                                      : current_fit_length_to;
  ...
  if (any_changed) {
    ...
    if (current_merge_start_changed) {
      QString toset =
          current_merge_start ? QString("%1").arg(current_merge_start) : "";
      t_csv->setItem(row, 1, new QTableWidgetItem(toset));
    }
    if (current_merge_end_changed) {
      QString toset =
          current_merge_end ? QString("%1").arg(current_merge_end) : "";
      t_csv->setItem(row, 2, new QTableWidgetItem(toset));
    }
    if (current_fit_start_changed) {
      QString toset =
          current_fit_start ? QString("%1").arg(current_fit_start) : "";
    ...
    if (current_cut_start_changed) {
      QString toset =
          current_cut_start ? QString("%1").arg(current_cut_start) : "";
...
void US_Hydrodyn_Pdb_Tool_Merge::recalc_from_fit(int row, int col) {
  ...
  if (extra_chain) {
    // editor_msg( "red", QString( us_tr( "Internal error: update fit, but fit
    // not valid for row <%1>" ) ).arg( row + 1 ) );
    return;
  }
  ...
  unsigned int alt_min_fit_start =
      to_range.start;  // residue_offset_position( false, chain, to_range.start
                       // , 1 );
...
void US_Hydrodyn_Pdb_Tool_Merge::recalc_from_cut(int row, int col) {
  ...
  if (extra_chain) {
    // editor_msg( "red", QString( us_tr( "Internal error: update cut, but cut
    // not valid for row <%1>" ) ).arg( row + 1 ) );
    return;
  }
  ...
  if (use_start) {
    if (current_fit_start <= current_cut_end) {
      ...
      current_fit_end = residue_offset_position(true, chain, current_fit_start,
                                                current_fit_length - 1);
      unsigned int alt_fit_end = residue_offset_position(
          false, chain, current_fit_start, current_fit_length - 1);
      if (current_fit_end < alt_fit_end) {
        current_fit_end = alt_fit_end;
      }
      if (current_fit_end > to_range.end || current_fit_end > from_range.end) {
        current_fit_start = 0;
        current_fit_end = 0;
      }
      current_fit_start_changed = true;
...
bool US_Hydrodyn_Pdb_Tool_Merge::get_chains(QString chain) {
  ...
  if (qsl.size() != 2) {
    editor_msg("red", QString(us_tr("Error: Extra chain not in proper format "
...
bool US_Hydrodyn_Pdb_Tool_Merge::get_chains(QString chain, QString &fit_chain,
                                            QString &cross_chain) {
  fit_chain = chain;
