...
public:
  ...
  QString name;
  bool
  operator< (const pdb_sortable_qstring &objIn) const
  {
    return x < objIn.x;
  }
};
...
QStringList
US_Pdb_Util::sort_pdbs (const QStringList &filenames)
{
  if (filenames.size () <= 1)
    {
      return filenames;
...
QString
US_Pdb_Util::qstring_common_head (const QStringList &qsl, bool strip_digits)
{
  if (!qsl.size ())
    {
      return "";
...
QString
US_Pdb_Util::qstring_common_tail (const QStringList &qsl, bool strip_digits)
{
  if (!qsl.size ())
    {
      return "";
...
QString
US_Pdb_Util::qstring_common_head (const QString &s1, const QString &s2)
{
...
QString
US_Pdb_Util::qstring_common_tail (const QString &s1, const QString &s2)
{
  int min_len = (int)s1.length ();
  if (min_len > (int)s2.length ())
    {
...
bool
US_Pdb_Util::range_to_set (set<QString> &result, const QString &s)
{
