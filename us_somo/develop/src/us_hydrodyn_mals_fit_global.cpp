                                                                   ...
                                                                   ) : QDialog( p )
{
   ...
   mals_maxfpk_restart_tries = ( ( US_Hydrodyn * ) ( mals_win->us_hydrodyn ) )->gparams[ "mals_maxfpk_restart_tries"  ].toUInt();
   mals_maxfpk_restart_pct   = ( ( US_Hydrodyn * ) ( mals_win->us_hydrodyn ) )->gparams[ "mals_maxfpk_restart_pct"    ].toDouble();
   ...
   if ( use_errors &&
        ( mals_win->unified_ggaussian_e.size() != mals_win->unified_ggaussian_q.size() ||
...
void US_Hydrodyn_Mals_Fit_Global::restore()
{
   ...
   if ( update_mals )
   {
      ...
      mals_win->ggqfit_plot->detachItems( QwtPlotItem::Rtti_PlotCurve ); mals_win->ggqfit_plot->detachItems( QwtPlotItem::Rtti_PlotMarker );;
...
void US_Hydrodyn_Mals_Fit_Global::undo()
{
        ...
        MFIT_GLOBAL::init_params.size() == test_widgets.size() )
   {
      ...
      {
         ((QLineEdit *)test_widgets[ i ])->setText( QString( "%1" ).arg( MFIT_GLOBAL::init_params[ i ] ) );
      }
      update_test_info();
   }
}
...
void US_Hydrodyn_Mals_Fit_Global::update_test_info()
{
...

   {
      ...
      for ( unsigned int i = 0; i < mals_win->unified_ggaussian_gaussians_size; ++i ) {
         ...
         for ( unsigned int j = 0; j < mals_win->common_size; ++j ) {
            qs_uggs += QString( "%1 " ).arg(  mals_win->unified_ggaussian_params[ p++ ], 0, 'g', 6 );
         } 
         qs_uggs += "\n";
      }
      ...
      for ( unsigned int i = 0; i < mals_win->unified_ggaussian_curves; ++i ) {
         ...
         for ( unsigned int k = 0; k < mals_win->unified_ggaussian_gaussians_size; ++k ) {
   ...
   lbl_test_info->setText( 
                          ...
                          .arg( US_Vector::qs_vector( "unified ggaussians",mals_win->unified_ggaussian_params, 6, 20 ) )
                          .arg( qs_uggs )
...
void US_Hydrodyn_Mals_Fit_Global::setupGUI()
{
   ...
   /* old way
      QString qs_rx = QString( "^(|(\\d+)|(\\d+(,\\d+){0,%1}))$" ).arg( mals_win->gaussians.size() / 3 - 1 );
   ...
   */
   ...
   for ( unsigned int i = 0; i < ( unsigned int ) mals_win->gaussians.size() / gaussian_type_size; i++ )
   {
      QCheckBox *cb_tmp;
   ...
   double peak = mals_win->compute_gaussian_peak( mals_win->wheel_file, mals_win->gaussians );
...
bool US_Hydrodyn_Mals_Fit_Global::setup_run()
{
   ...
   MFIT_GLOBAL::unified_ggaussian_param_index     = mals_win->unified_ggaussian_param_index;
   MFIT_GLOBAL::unified_ggaussian_gaussians_size  = mals_win->unified_ggaussian_gaussians_size;
   ...
   double dist = ( ( US_Hydrodyn * )(mals_win->us_hydrodyn) )->gparams[ "mals_dist_max" ].toDouble();
   ...
   // base_params : these are a copy of the init_params without replacement from the mals_win unified_ggaussians
   ...
   for ( unsigned int i = 0; i < mals_win->unified_ggaussian_gaussians_size; i++ )
   {
           ...
           fixed_curves.count( i ) )
      {
         ...
         MFIT_GLOBAL::fixed_params.push_back( mals_win->unified_ggaussian_params[ ofs ] );
         ...
         MFIT_GLOBAL::base_params .push_back( mals_win->unified_ggaussian_params[ ofs ] );
      ...
      if ( dist1_active && mals_win->cb_fix_dist1->isChecked() ) // if dist1s are common
      {
              ...
              ( cb_comm_dist1->isChecked() && i ) )
         {
            ...
            {
               MFIT_GLOBAL::fixed_params.push_back( mals_win->unified_ggaussian_params[ this_ofs ] );
               MFIT_GLOBAL::comm_backref[ MFIT_GLOBAL::param_fixed.size() ] = comm_dist_1_var_pos;
            } else {
               MFIT_GLOBAL::fixed_params.push_back( mals_win->unified_ggaussian_params[ ofs ] );
            }
            MFIT_GLOBAL::param_fixed .push_back( true );
         ...
         if ( dist2_active && mals_win->cb_fix_dist2->isChecked() ) // if dist2s are common
         {
                 ...
                 ( cb_comm_dist2->isChecked() && i ) )
            {
               ...
               {
                  MFIT_GLOBAL::fixed_params.push_back( mals_win->unified_ggaussian_params[ this_ofs ] );
                  MFIT_GLOBAL::comm_backref[ MFIT_GLOBAL::param_fixed.size() ] = comm_dist_2_var_pos;
               } else {
                  MFIT_GLOBAL::fixed_params.push_back( mals_win->unified_ggaussian_params[ ofs ] );
               }
               MFIT_GLOBAL::param_fixed .push_back( true );
   ...
   unsigned int base_ofs = mals_win->unified_ggaussian_gaussians_size * common_size;
   // assert( (unsigned int)MFIT_GLOBAL::param_pos.size() == base_ofs && "common_size*number of gaussians should equal param_pos.size()" );

   for ( unsigned int f = 0; f < mals_win->unified_ggaussian_curves; f++ )
   {
      unsigned int ofs = base_ofs + f * per_file_size * mals_win->unified_ggaussian_gaussians_size;
      ...
      for ( unsigned int i = 0; i < mals_win->unified_ggaussian_gaussians_size; i++ )
      {
         ...
         if ( dist1_active && !mals_win->cb_fix_dist1->isChecked() ) // if dist1s are not common
         {
            if ( cb_fix_dist1->isChecked() ||
                 fixed_curves.count( i ) ||
                 ( cb_comm_dist1->isChecked() && i ) )
   ...
   if ( mals_win->cb_fix_width->isChecked() )
   {
      ...
      for ( unsigned int i = 0; i < mals_win->unified_ggaussian_gaussians_size; i++ )
      {
         ...
         {
            MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::fixed_params.size() );
            MFIT_GLOBAL::fixed_params.push_back( mals_win->unified_ggaussian_params[ 2 * i + 0 ] );
            MFIT_GLOBAL::param_fixed .push_back( true );
         } else {
            MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::init_params.size() );
            ...
            MFIT_GLOBAL::init_params .push_back( base_val );
            MFIT_GLOBAL::base_params .push_back( mals_win->unified_ggaussian_params[ 2 * i + 0 ] );
            MFIT_GLOBAL::param_fixed .push_back( false );
         ...
         {
            MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::fixed_params.size() );
            MFIT_GLOBAL::fixed_params.push_back( mals_win->unified_ggaussian_params[ 2 * i + 1 ] );
            MFIT_GLOBAL::param_fixed .push_back( true );
         } else {
            MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::init_params.size() );
            ...
            MFIT_GLOBAL::init_params .push_back( base_val );
            MFIT_GLOBAL::base_params .push_back( mals_win->unified_ggaussian_params[ 2 * i + 1] );
            MFIT_GLOBAL::param_fixed .push_back( false );
      ...
      for ( unsigned int i = 0; i < mals_win->unified_ggaussian_gaussians_size * mals_win->unified_ggaussian_curves; i++ )
      {
         unsigned int pos = i / mals_win->unified_ggaussian_curves;

         unsigned int height_pos = 2 * mals_win->unified_ggaussian_gaussians_size + i + 0;

         if ( cb_fix_amplitude->isChecked() ||
              fixed_curves.count( pos ) )
         {
            MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::fixed_params.size() );
            MFIT_GLOBAL::fixed_params.push_back( mals_win->unified_ggaussian_params[ height_pos ] );
            MFIT_GLOBAL::param_fixed .push_back( true );
         } else {
            MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::init_params.size() );
            ...
            MFIT_GLOBAL::init_params .push_back( base_val );
            MFIT_GLOBAL::base_params .push_back( mals_win->unified_ggaussian_params[ height_pos ] );
            MFIT_GLOBAL::param_fixed .push_back( false );
      ...
      for ( unsigned int i = 0; i < mals_win->unified_ggaussian_gaussians_size; i++ )
      {
              ...
              fixed_curves.count( i ) )
         {
            ...
            MFIT_GLOBAL::init_params .push_back( base_val );
            MFIT_GLOBAL::base_params .push_back( mals_win->unified_ggaussian_params[ i ] );
            MFIT_GLOBAL::param_fixed .push_back( false );
      ...
      for ( unsigned int i = 0; i < mals_win->unified_ggaussian_gaussians_size * 2 * mals_win->unified_ggaussian_curves; i+= 2 )
      {
         unsigned int pos = i / ( 2 * mals_win->unified_ggaussian_curves );

         unsigned int height_pos = mals_win->unified_ggaussian_gaussians_size + i + 0;
         unsigned int width_pos  = mals_win->unified_ggaussian_gaussians_size + i + 1;
         ...
         {
            MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::fixed_params.size() );
            MFIT_GLOBAL::fixed_params.push_back( mals_win->unified_ggaussian_params[ width_pos ] );
            MFIT_GLOBAL::param_fixed .push_back( true );
         } else {
            MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::init_params.size() );
            ...
            MFIT_GLOBAL::init_params .push_back( base_val );
            MFIT_GLOBAL::base_params .push_back( mals_win->unified_ggaussian_params[ width_pos ] );
            MFIT_GLOBAL::param_fixed .push_back( false );
...
void US_Hydrodyn_Mals_Fit_Global::test()
{
   ...
   for ( int i = 0; i < (int) MFIT_GLOBAL::init_params.size(); ++i )
   {
      MFIT_GLOBAL::init_params[ i ] = ((QLineEdit *)test_widgets[ i ])->text().toDouble();
   }
   ...
   for ( unsigned int i = 0; i < MFIT_GLOBAL::param_fixed.size(); i++ )
   {
           ...
           MFIT_GLOBAL::comm_backref.count( i ) )
      {
         ...
         us_qdebug( 
                ...
                .arg( MFIT_GLOBAL::comm_backref.count( i ) ? "backref used" : "direct pos" ) 
                 );
      } else {
...
void US_Hydrodyn_Mals_Fit_Global::lm()
{
   ...
   */
   ...
   {
      gsumf[ i ] = (*MFIT_GLOBAL::compute_gaussian_f)( t[ i ], &MFIT_GLOBAL::init_params[ 0 ] );
   }
...
#undef TOL
   ...
   if ( org_rmsd > status.fnorm )
   {
      ...
      // mals_win->add_ggaussian_curve( "lm_after_pushback", mals_win->compute_ggaussian_gaussian_sum() );
      gaussians_undo.push_back( mals_win->unified_ggaussian_params );
      if ( update_mals )
      {
...
void US_Hydrodyn_Mals_Fit_Global::gsm_sd()
{
   ...
   {
      gsm_yp[ j ]  = MFIT_GLOBAL::compute_gaussian_f( gsm_t[ j ], (double *)(&MFIT_GLOBAL::base_params[ 0 ]) );
...
void US_Hydrodyn_Mals_Fit_Global::stop()
{
   running = false;
}
...
void US_Hydrodyn_Mals_Fit_Global::grid()
{
   ...
   unsigned int total_count = ( unsigned int ) pow( (double) pop_size, (int) MFIT_GLOBAL::init_params.size() );
   ...
   for ( unsigned int j = 0; j < ( unsigned int ) MFIT_GLOBAL::init_params.size(); j++ )
   {
      ofs_per[ j ] = ( MFIT_GLOBAL::param_max[ j ] - MFIT_GLOBAL::param_min[ j ] ) / ( pop_size - 1 );
   }
   ...
   {
      yp[ j ]  = MFIT_GLOBAL::compute_gaussian_f( t[ j ], (double *)(&MFIT_GLOBAL::base_params[ 0 ]) );
