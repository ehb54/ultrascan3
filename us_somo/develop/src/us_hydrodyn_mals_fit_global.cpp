...
US_Hydrodyn_Mals_Fit_Global::US_Hydrodyn_Mals_Fit_Global(
  ...
  mals_lock_min_retry = ((US_Hydrodyn *)(mals_win->us_hydrodyn))
                                    ->gparams["mals_lock_min_retry"] == "true"
                            ? true
                            : false;
  ...
  mals_maxfpk_restart = ((US_Hydrodyn *)(mals_win->us_hydrodyn))
                                    ->gparams["mals_maxfpk_restart"] == "true"
                            ? true
                            : false;
...
void US_Hydrodyn_Mals_Fit_Global::restore() {
  ...
  if (update_mals) {
    ...
    mals_win->lbl_gauss_fit->setText(QString("%1").arg(new_rmsd, 0, 'g', 5));
    mals_win->pb_ggauss_rmsd->setEnabled(false);
  }
  update_enables();
}

void US_Hydrodyn_Mals_Fit_Global::undo() {
...
void US_Hydrodyn_Mals_Fit_Global::update_test_info() {
  QString qs_uggs = "";

  {
    ...
    for (unsigned int i = 0; i < mals_win->unified_ggaussian_gaussians_size;
      ...
      for (unsigned int j = 0; j < mals_win->common_size; ++j) {
        qs_uggs += QString("%1 ").arg(mals_win->unified_ggaussian_params[p++],
                                      0, 'g', 6);
      }
      qs_uggs += "\n";
    }
    ...
    for (unsigned int i = 0; i < mals_win->unified_ggaussian_curves; ++i) {
      ...
      for (unsigned int k = 0; k < mals_win->unified_ggaussian_gaussians_size;
           ++k) {
  ...
  lbl_test_info->setText(
      QString("Per gaussian: common_size: %1 %2 per_file_size: %3 %4\n"
          ...
          .arg(US_Vector::qs_vector("unified ggaussians",
                                    mals_win->unified_ggaussian_params, 6, 20))
          .arg(qs_uggs)
          .arg(MFIT_GLOBAL::qs_params()));
}

void US_Hydrodyn_Mals_Fit_Global::setupGUI() {
  int minHeight1 = 30;
  ...
  cb_pct_center_from_init->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_pct_width_from_init->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_pct_amplitude_from_init->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_pct_dist1_from_init->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_pct_dist2_from_init->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
...
void US_Hydrodyn_Mals_Fit_Global::cancel() {
...
void US_Hydrodyn_Mals_Fit_Global::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/somo_mals_fit_global.html");
}

void US_Hydrodyn_Mals_Fit_Global::closeEvent(QCloseEvent *e) {
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept();
}

void US_Hydrodyn_Mals_Fit_Global::update_common() {
  if (!setup_run()) {
    update_enables();
    return;
  }
...
void US_Hydrodyn_Mals_Fit_Global::update_enables() {
...
double compute_gaussian_f_GAUSS(double t, const double *par) {
  ...
  for (unsigned int i = 0; i < unified_ggaussian_gaussians_size; i++) {
    ...
    /*
    cout << QString( "for pos %1 t is %2 index %3 gaussian %4 center %5 height
    %6 width %7\n" ) .arg( t ) .arg( unified_q[ ( unsigned int ) t ] ) .arg(
    index ) .arg( i ) .arg( center ) .arg( height ) .arg( width )
       ;
    */
...
double compute_gaussian_f_EMGGMG(double t, const double *par) {
  ...
  for (unsigned int i = 0; i < unified_ggaussian_gaussians_size; i++) {
    ...
    /*
    cout << QString( "for pos %1 t is %2 index %3 gaussian %4 center %5 height
    %6 width %7 dist1 %8 dist2 %9\n" ) .arg( t ) .arg( unified_q[ ( unsigned int
    ) t ] ) .arg( index ) .arg( i ) .arg( center ) .arg( height ) .arg( width )
       .arg( dist1 )
       .arg( dist2 )
       ;
    */
...
bool US_Hydrodyn_Mals_Fit_Global::setup_run() {
  ...
  switch (gaussian_type) {
    case US_Hydrodyn_Mals::EMGGMG:
      MFIT_GLOBAL::compute_gaussian_f = &MFIT_GLOBAL::compute_gaussian_f_EMGGMG;
      // gsm_f = &gsm_f_EMGGMG;
      break;
    case US_Hydrodyn_Mals::EMG:
  ...
  // during search params" init_params : all the "variable during search params"
  // // this will be the search vector param_fixed : boolean as to whether or
  // not the specific param is fixed(true) or variable(false) base_params :
  // these are a copy of the init_params without replacement from the mals_win
  // unified_ggaussians
  ...
  for (unsigned int f = 0; f < mals_win->unified_ggaussian_curves; f++) {
    ...
    for (unsigned int i = 0; i < mals_win->unified_ggaussian_gaussians_size;
      ...
      if (!mals_win->cb_fix_width->isChecked())  // if widths are not common
      {
        if (cb_fix_width->isChecked() || fixed_curves.count(i)) {
          MFIT_GLOBAL::param_pos.push_back(MFIT_GLOBAL::fixed_params.size());
          MFIT_GLOBAL::fixed_params.push_back(
      ...
      if (dist1_active &&
          !mals_win->cb_fix_dist1->isChecked())  // if dist1s are not common
      {
        if (cb_fix_dist1->isChecked() || fixed_curves.count(i) ||
            (cb_comm_dist1->isChecked() && i)) {
          MFIT_GLOBAL::param_pos.push_back(MFIT_GLOBAL::fixed_params.size());
  ...
  i++ )
     {
        ...
        {
           MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::fixed_params.size()
  ); MFIT_GLOBAL::fixed_params.push_back( mals_win->unified_ggaussian_params[ 2
  * i + 0 ] ); MFIT_GLOBAL::param_fixed .push_back( true ); } else {
           MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::init_params.size()
  );
  ...
  * i + 1 ] ); MFIT_GLOBAL::param_fixed .push_back( true ); } else {
           MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::init_params.size()
  );
  ...
  mals_win->unified_ggaussian_curves; i++ )
     {
        ...
        unsigned int height_pos = 2 * mals_win->unified_ggaussian_gaussians_size
  + i + 0;
        ...
        {
           MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::fixed_params.size()
  ); MFIT_GLOBAL::fixed_params.push_back( mals_win->unified_ggaussian_params[
  height_pos ] ); MFIT_GLOBAL::param_fixed .push_back( true ); } else {
           MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::init_params.size()
  );
  ...
  i++ )
     {
        ...
        {
           MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::fixed_params.size()
  ); MFIT_GLOBAL::fixed_params.push_back( mals_win->unified_ggaussian_params[ i
  ] ); MFIT_GLOBAL::param_fixed .push_back( true ); } else {
           MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::init_params.size()
  );
  ...
  2 * mals_win->unified_ggaussian_curves; i+= 2 )
     {
        ...
        unsigned int height_pos = mals_win->unified_ggaussian_gaussians_size + i
  + 0; unsigned int width_pos  = mals_win->unified_ggaussian_gaussians_size + i
  + 1;
  ...
  width_pos ] ); MFIT_GLOBAL::param_fixed .push_back( true ); } else {
           MFIT_GLOBAL::param_pos   .push_back( MFIT_GLOBAL::init_params.size()
  );
...
void US_Hydrodyn_Mals_Fit_Global::test_mode() {
  if (!cb_test_mode->isChecked()) {
...
void US_Hydrodyn_Mals_Fit_Global::test() {
    ...
    if (!MFIT_GLOBAL::param_fixed[i] || MFIT_GLOBAL::comm_backref.count(i)) {
      ...
      us_qdebug(QString("param %1 taken from init params pos %2 %3")
                    ...
                    .arg(MFIT_GLOBAL::comm_backref.count(i)
                             ? MFIT_GLOBAL::comm_backref[i]
                             : MFIT_GLOBAL::param_pos[i])
                    .arg(MFIT_GLOBAL::comm_backref.count(i) ? "backref used"
                                                            : "direct pos"));
    } else {
...
void US_Hydrodyn_Mals_Fit_Global::lm() {
  update_common();
  running = true;
  update_enables();
  // setup_run();
  ...
  */
  ...
  for (unsigned int i = 0; i < (unsigned int)t.size(); i++) {
    gsumf[i] =
        (*MFIT_GLOBAL::compute_gaussian_f)(t[i], &MFIT_GLOBAL::init_params[0]);
  }
...
void US_Hydrodyn_Mals_Fit_Global::gsm_sd() {
...
void US_Hydrodyn_Mals_Fit_Global::gsm_ih() {
  return;  // have to rewrite this
...
void US_Hydrodyn_Mals_Fit_Global::gsm_cg() {
  return;  // have to rewrite this
...
void US_Hydrodyn_Mals_Fit_Global::ga() {
  /*
  gsm_setup();
...
void US_Hydrodyn_Mals_Fit_Global::stop() { running = false; }

void US_Hydrodyn_Mals_Fit_Global::grid() {
  return;  // have to rewrite this
  setup_run();
  ...
  unsigned int total_count =
      (unsigned int)pow((double)pop_size, (int)MFIT_GLOBAL::init_params.size());
  ...
  for (unsigned int i = 0; i < total_count; i++) {
    ...
    for (unsigned int j = 0; j < t.size(); j++) {
      yp[j] = MFIT_GLOBAL::compute_gaussian_f(t[j], (double *)(&use_par[0]));
...
void US_Hydrodyn_Mals_Fit_Global::usp_config_plot_test(const QPoint &) {
