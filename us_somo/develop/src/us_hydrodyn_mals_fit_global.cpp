...
US_Hydrodyn_Mals_Fit_Global::US_Hydrodyn_Mals_Fit_Global (
    US_Hydrodyn_Mals *mals_win, QWidget *p, const char *)
    : QDialog (p)
{
  ...
  setWindowTitle (us_tr ("US-SOMO: MALS: Global Gaussian Fit"));

  update_mals = true;
  running = false;

  gaussian_type = mals_win->gaussian_type;
  gaussian_type_size = mals_win->gaussian_type_size;

  mals_ampl_width_min = ((US_Hydrodyn *)(mals_win->us_hydrodyn))
                            ->gparams["mals_ampl_width_min"]
                            .toDouble ();
  mals_lock_min_retry = ((US_Hydrodyn *)(mals_win->us_hydrodyn))
                                    ->gparams["mals_lock_min_retry"]
                                == "true"
                            ? true
                            : false;
  mals_lock_min_retry_mult = ((US_Hydrodyn *)(mals_win->us_hydrodyn))
                                 ->gparams["mals_lock_min_retry_mult"]
                                 .toDouble ();
  mals_maxfpk_restart = ((US_Hydrodyn *)(mals_win->us_hydrodyn))
                                    ->gparams["mals_maxfpk_restart"]
                                == "true"
                            ? true
                            : false;
  mals_maxfpk_restart_tries = ((US_Hydrodyn *)(mals_win->us_hydrodyn))
                                  ->gparams["mals_maxfpk_restart_tries"]
                                  .toUInt ();
  mals_maxfpk_restart_pct = ((US_Hydrodyn *)(mals_win->us_hydrodyn))
                                ->gparams["mals_maxfpk_restart_pct"]
                                .toDouble ();
  ...
  switch (gaussian_type)
    {
    case US_Hydrodyn_Mals::EMGGMG:
      dist1_active = true;
      dist2_active = true;
      break;
    case US_Hydrodyn_Mals::EMG:
    case US_Hydrodyn_Mals::GMG:
      dist1_active = true;
      dist2_active = false;
      break;
    case US_Hydrodyn_Mals::GAUSS:
      dist1_active = false;
      dist2_active = false;
      break;
    }
  ...
  is_common.clear (); // is common maps the offsets to layout of the regular
                      // file specific gaussians
  offset.clear ();
  ...
  // width
  if (mals_win->cb_fix_width->isChecked ())
    {
  ...
  if (dist1_active)
    {
      if (mals_win->cb_fix_dist1->isChecked ())
        {
          is_common.push_back (true);
          offset.push_back (common_size++); // first common entry
        }
      ...
      if (dist2_active)
        {
          if (mals_win->cb_fix_dist2->isChecked ())
            {
              is_common.push_back (true);
              offset.push_back (common_size++); // first common entry
            }
  ...
  gaussians_undo.push_back (mals_win->unified_ggaussian_params);

  setGeometry (global_Xpos, global_Ypos, 0, 0);

  use_errors = mals_win->cb_sd_weight->isChecked ();
  if (use_errors
      && (mals_win->unified_ggaussian_e.size ()
              != mals_win->unified_ggaussian_q.size ()
          || !mals_win->is_nonzero_vector (mals_win->unified_ggaussian_e)))
    {
      use_errors = false;
  ...
  if (use_errors)
    {
      ...
      vector<double> x = mals_win->unified_ggaussian_t;
          ...
          if ((double)xui != x[i])
            {
              ...
              QMessageBox::information (
                  ...
                  us_tr ("SD weighting requested, but this currently\n"
                         "does not support fractional or negative frame "
                         "numbers.\n"
                         "SD weighting turned off."));
              break;
            }
...
void
US_Hydrodyn_Mals_Fit_Global::restore ()
{
  gaussians_undo.resize (1);
  mals_win->unified_ggaussian_params = gaussians_undo[0];
  if (update_mals)
    {
      mals_win->gauss_init_markers ();
      mals_win->update_gauss_pos ();
      mals_win->lbl_gauss_fit->setText ("?");
      mals_win->pb_ggauss_rmsd->setEnabled (true);
      mals_win->ggqfit_plot->detachItems (QwtPlotItem::Rtti_PlotCurve);
      mals_win->ggqfit_plot->detachItems (QwtPlotItem::Rtti_PlotMarker);
      ;
      mals_win->ggqfit_plot->replot ();
      double new_rmsd = mals_win->ggaussian_rmsd ();
      mals_win->lbl_gauss_fit->setText (
          QString ("%1").arg (new_rmsd, 0, 'g', 5));
      mals_win->pb_ggauss_rmsd->setEnabled (false);
    }
  update_enables ();
}

void
US_Hydrodyn_Mals_Fit_Global::undo ()
{
  ...
  mals_win->unified_ggaussian_params = gaussians_undo.back ();
  if (update_mals)
    {
      mals_win->gauss_init_markers ();
      mals_win->update_gauss_pos ();
  ...
  if (cb_test_mode->isChecked ()
      && MFIT_GLOBAL::init_params.size () == test_widgets.size ())
    {
      for (int i = 0; i < (int)MFIT_GLOBAL::init_params.size (); ++i)
        {
          ((QLineEdit *)test_widgets[i])
              ->setText (QString ("%1").arg (MFIT_GLOBAL::init_params[i]));
        }
      update_test_info ();
    }
}
...
void
US_Hydrodyn_Mals_Fit_Global::update_test_info ()
{
  QString qs_uggs = "";

  {
    ...
    qs_uggs += "common data: " + mals_win->describe_unified_common + "\n";

    for (unsigned int i = 0; i < mals_win->unified_ggaussian_gaussians_size;
         ++i)
      {
        qs_uggs += QString (" gaussian %1: ").arg (i + 1);
        for (unsigned int j = 0; j < mals_win->common_size; ++j)
          {
            qs_uggs += QString ("%1 ").arg (
                mals_win->unified_ggaussian_params[p++], 0, 'g', 6);
          }
        qs_uggs += "\n";
      }

    // now the file specific
    qs_uggs += "file specific: " + mals_win->describe_unified_per_file + "\n";
    for (unsigned int i = 0; i < mals_win->unified_ggaussian_curves; ++i)
      {
        qs_uggs += QString (" curve %1: ").arg (i + 1) + "\n";
        for (unsigned int k = 0;
             k < mals_win->unified_ggaussian_gaussians_size; ++k)
          {
            qs_uggs += QString ("  gaussian %1: ").arg (k + 1);
            for (unsigned int j = 0; j < mals_win->per_file_size; ++j)
              {
                qs_uggs += QString ("%1 ").arg (
                    mals_win->unified_ggaussian_params[p++], 0, 'g', 6);
              }
  ...
  lbl_test_info->setText (
      QString ("Per gaussian: common_size: %1 %2 per_file_size: %3 %4\n"
               "%5"
               "%6"
               "%7")
          .arg (mals_win->common_size)
          .arg (mals_win->describe_unified_common)
          .arg (mals_win->per_file_size)
          .arg (mals_win->describe_unified_per_file)
          .arg (US_Vector::qs_vector (
              "unified ggaussians", mals_win->unified_ggaussian_params, 6, 20))
          .arg (qs_uggs)
          .arg (MFIT_GLOBAL::qs_params ()));
}

void
US_Hydrodyn_Mals_Fit_Global::setupGUI ()
{
  int minHeight1 = 30;

  lbl_title = new QLabel (us_tr ("US-SOMO: MALS: Global Gaussian Fit"), this);
  ...
  cb_fix_center->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  cb_pct_center->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize));
  cb_pct_center->setPalette (PALET_NORMAL);
  AUTFBACK (cb_pct_center);
  connect (cb_pct_center, SIGNAL (clicked ()), SLOT (update_enables ()));

  le_pct_center = new mQLineEdit (this);
  le_pct_center->setObjectName ("le_pct_center Line Edit");
  ...
  le_pct_center->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize));
  ...
  cb_pct_center_from_init->setFont (QFont (USglobal->config_list.fontFamily,
                                           USglobal->config_list.fontSize));
  cb_pct_center_from_init->setPalette (PALET_NORMAL);
  AUTFBACK (cb_pct_center_from_init);
  connect (cb_pct_center_from_init, SIGNAL (clicked ()),
           SLOT (update_enables ()));
  ...
  cb_pct_width->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  cb_pct_width->setPalette (PALET_NORMAL);
  AUTFBACK (cb_pct_width);
  connect (cb_pct_width, SIGNAL (clicked ()), SLOT (update_enables ()));

  le_pct_width = new mQLineEdit (this);
  le_pct_width->setObjectName ("le_pct_width Line Edit");
  ...
  le_pct_width->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  cb_pct_width_from_init->setFont (QFont (USglobal->config_list.fontFamily,
                                          USglobal->config_list.fontSize));
  cb_pct_width_from_init->setPalette (PALET_NORMAL);
  AUTFBACK (cb_pct_width_from_init);
  connect (cb_pct_width_from_init, SIGNAL (clicked ()),
           SLOT (update_enables ()));
  ...
  cb_fix_amplitude->setFont (QFont (USglobal->config_list.fontFamily,
                                    USglobal->config_list.fontSize));
  ...
  cb_pct_amplitude->setFont (QFont (USglobal->config_list.fontFamily,
                                    USglobal->config_list.fontSize));
  ...
  le_pct_amplitude = new mQLineEdit (this);
  le_pct_amplitude->setObjectName ("le_pct_amplitude Line Edit");
  ...
  le_pct_amplitude->setFont (QFont (USglobal->config_list.fontFamily,
                                    USglobal->config_list.fontSize));
  ...
  cb_pct_amplitude_from_init->setFont (QFont (USglobal->config_list.fontFamily,
                                              USglobal->config_list.fontSize));
  cb_pct_amplitude_from_init->setPalette (PALET_NORMAL);
  AUTFBACK (cb_pct_amplitude_from_init);
  connect (cb_pct_amplitude_from_init, SIGNAL (clicked ()),
           SLOT (update_enables ()));
  ...
  cb_pct_dist1->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  cb_pct_dist1->setPalette (PALET_NORMAL);
  AUTFBACK (cb_pct_dist1);
  connect (cb_pct_dist1, SIGNAL (clicked ()), SLOT (update_enables ()));

  le_pct_dist1 = new mQLineEdit (this);
  le_pct_dist1->setObjectName ("le_pct_dist1 Line Edit");
  ...
  le_pct_dist1->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  cb_pct_dist1_from_init->setFont (QFont (USglobal->config_list.fontFamily,
                                          USglobal->config_list.fontSize));
  cb_pct_dist1_from_init->setPalette (PALET_NORMAL);
  AUTFBACK (cb_pct_dist1_from_init);
  connect (cb_pct_dist1_from_init, SIGNAL (clicked ()),
           SLOT (update_enables ()));
  ...
  cb_pct_dist2->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  cb_pct_dist2->setPalette (PALET_NORMAL);
  AUTFBACK (cb_pct_dist2);
  connect (cb_pct_dist2, SIGNAL (clicked ()), SLOT (update_enables ()));

  le_pct_dist2 = new mQLineEdit (this);
  le_pct_dist2->setObjectName ("le_pct_dist2 Line Edit");
  ...
  le_pct_dist2->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  cb_pct_dist2_from_init->setFont (QFont (USglobal->config_list.fontFamily,
                                          USglobal->config_list.fontSize));
  cb_pct_dist2_from_init->setPalette (PALET_NORMAL);
  AUTFBACK (cb_pct_dist2_from_init);
  connect (cb_pct_dist2_from_init, SIGNAL (clicked ()),
           SLOT (update_enables ()));
  ...
  cb_comm_dist1->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize));
  ...
  cb_comm_dist2->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize));
  ...
  lbl_fix_curves->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize - 1,
                                  QFont::Bold));

  /* old way
     QString qs_rx = QString( "^(|(\\d+)|(\\d+(,\\d+){0,%1}))$" ).arg(
     mals_win->gaussians.size() / 3 - 1 ); cout << "qs_rx:" << qs_rx << endl;
     QRegExp rx_fix_curves( qs_rx );
     QRegExpValidator *rx_val_fix_curves = new QRegExpValidator( rx_fix_curves,
     this ); le_fix_curves = new mQLineEdit( this );
     le_fix_curves->setObjectName( "le_fix_curves Line Edit" );
     ...
     le_fix_curves->setFont(QFont( USglobal->config_list.fontFamily,
     USglobal->config_list.fontSize )); le_fix_curves->setEnabled( false );
     le_fix_curves->setValidator( rx_val_fix_curves );
     connect( le_fix_curves, SIGNAL( textChanged( const QString & ) ), SLOT(
     update_enables() ) );
  */

  for (unsigned int i = 0;
       i < (unsigned int)mals_win->gaussians.size () / gaussian_type_size; i++)
    {
      QCheckBox *cb_tmp;
      ...
      cb_tmp->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize));
  ...
  lbl_epsilon->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1,
                               QFont::Bold));

  le_epsilon = new mQLineEdit (this);
  le_epsilon->setObjectName ("le_epsilon Line Edit");
  double peak = mals_win->compute_gaussian_peak (mals_win->wheel_file,
                                                 mals_win->gaussians);
  le_epsilon->setText (
      QString ("%1").arg (peak / 1e6 < 0.001 ? peak / 1e6 : 0.001));
  le_epsilon->setAlignment (Qt::AlignCenter | Qt::AlignVCenter);
  le_epsilon->setPalette (PALET_NORMAL);
  AUTFBACK (le_epsilon);
  le_epsilon->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize));
  ...
  lbl_iterations->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize - 1,
                                  QFont::Bold));

  le_iterations = new mQLineEdit (this);
  le_iterations->setObjectName ("le_iterations Line Edit");
  ...
  le_iterations->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize));
  ...
  lbl_population
      = new QLabel (us_tr (" Maximum calls/Population/Grid: "), this);
  ...
  lbl_population->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize - 1,
                                  QFont::Bold));

  le_population = new mQLineEdit (this);
  le_population->setObjectName ("le_population Line Edit");
  ...
  le_population->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize));
  ...
  pb_restore->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize));
  ...
  pb_undo->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize));
  ...
  pb_lm->setFont (QFont (USglobal->config_list.fontFamily,
                         USglobal->config_list.fontSize));
  ...
  pb_gsm_sd->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize));
  ...
  pb_gsm_ih->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize));
  ...
  pb_gsm_cg->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize));
  ...
  pb_ga->setFont (QFont (USglobal->config_list.fontFamily,
                         USglobal->config_list.fontSize));
  ...
  cb_test_mode->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  lbl_test_info->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1,
                                 QFont::Bold));
  lbl_test_info->hide ();

  pb_test = new QPushButton (us_tr ("Test"), this);
  pb_test->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize));
  ...
  connect ((QWidget *)plot_test->titleLabel (),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_test (const QPoint &)));
  ((QWidget *)plot_test->titleLabel ())
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)plot_test->axisWidget (QwtPlot::yLeft),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_test (const QPoint &)));
  ((QWidget *)plot_test->axisWidget (QwtPlot::yLeft))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  connect ((QWidget *)plot_test->axisWidget (QwtPlot::xBottom),
           SIGNAL (customContextMenuRequested (const QPoint &)),
           SLOT (usp_config_plot_test (const QPoint &)));
  ((QWidget *)plot_test->axisWidget (QwtPlot::xBottom))
      ->setContextMenuPolicy (Qt::CustomContextMenu);
  ...
  plot_test->setAxisTitleFont (
      QwtPlot::xBottom, QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize, QFont::Bold));
#endif
  plot_test->setAxisFont (QwtPlot::xBottom,
                          QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
#if QT_VERSION < 0x040000
  plot_test->setAxisTitleFont (
      QwtPlot::yRight, QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize, QFont::Bold));
#endif
  plot_test->setAxisFont (QwtPlot::yRight,
                          QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize - 1));
  ...
  // gl_main->addWidget( le_fix_curves  , row , 1 , 1 + ( row ) - ( row ) , 1 +
  // ( 3  ) - ( 1 ) );
  QHBoxLayout *hbl_fix_curves = new QHBoxLayout;
  hbl_fix_curves->setContentsMargins (0, 0, 0, 0);
  hbl_fix_curves->setSpacing (0);
  for (unsigned int i = 0; i < (unsigned int)cb_fix_curves.size (); i++)
    {
      hbl_fix_curves->addWidget (cb_fix_curves[i]);
    }
  gl_main->addLayout (hbl_fix_curves, row, 1, 1 + (row) - (row),
                      1 + (3) - (1));
  ...
  QHBoxLayout *hbl_restore_undo = new QHBoxLayout;
  hbl_restore_undo->setContentsMargins (0, 0, 0, 0);
  hbl_restore_undo->setSpacing (0);
  hbl_restore_undo->addWidget (pb_restore);
  hbl_restore_undo->addWidget (pb_undo);

  QHBoxLayout *hbl_runs = new QHBoxLayout;
  hbl_runs->setContentsMargins (0, 0, 0, 0);
  hbl_runs->setSpacing (0);
  ...
  hbl_prog->setContentsMargins (0, 0, 0, 0);
  hbl_prog->setSpacing (0);
  ...
  // void * us_hydrodyn = mals_win->us_hydrodyn;
  ...
  if (cb_test_mode->isVisible ())
    {
      ...
      for (int i = 0; i < mals_win->lb_files->count (); i++)
        {
          if (mals_win->lb_files->item (i)->isSelected ())
            {
              selected_count++;
              if (selected_count > 4)
                {
...
void
US_Hydrodyn_Mals_Fit_Global::cancel ()
{
  if (mals_win->lbl_gauss_fit->text () == "?"
      && !mals_win->pb_ggauss_rmsd->isEnabled ())
    {
      mals_win->pb_ggauss_rmsd->setEnabled (true);
    }
...
void
US_Hydrodyn_Mals_Fit_Global::help ()
{
  US_Help *online_help;
  online_help = new US_Help (this);
  online_help->show_help ("manual/somo/somo_mals_fit_global.html");
}

void
US_Hydrodyn_Mals_Fit_Global::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept ();
...
void
US_Hydrodyn_Mals_Fit_Global::update_common ()
{
  if (!setup_run ())
    {
      update_enables ();
  ...
  vector<double> save_unified_ggaussians = mals_win->unified_ggaussian_params;

  for (unsigned int i = 0; i < MFIT_GLOBAL::param_fixed.size (); i++)
    {
      if (!MFIT_GLOBAL::param_fixed[i] || MFIT_GLOBAL::comm_backref.count (i))
        {
          mals_win->unified_ggaussian_params[i]
              = MFIT_GLOBAL::init_params[MFIT_GLOBAL::comm_backref.count (i)
                                             ? MFIT_GLOBAL::comm_backref[i]
                                             : MFIT_GLOBAL::param_pos[i]];
        }
    }
  if (save_unified_ggaussians != mals_win->unified_ggaussian_params)
    {
      mals_win->lbl_gauss_fit->setText ("?");
      mals_win->ggqfit_plot->detachItems (QwtPlotItem::Rtti_PlotCurve);
      mals_win->ggqfit_plot->detachItems (QwtPlotItem::Rtti_PlotMarker);
...
void
US_Hydrodyn_Mals_Fit_Global::update_enables ()
{
  ...
  le_pct_center->setEnabled (!running && !cb_fix_center->isChecked ()
                             && cb_pct_center->isChecked ());
  cb_pct_center_from_init->setEnabled (!running
                                       && !cb_fix_center->isChecked ());
  ...
  le_pct_amplitude->setEnabled (!running && !cb_fix_amplitude->isChecked ()
                                && cb_pct_amplitude->isChecked ());
  cb_pct_amplitude_from_init->setEnabled (!running
                                          && !cb_fix_amplitude->isChecked ());

  cb_fix_dist1->setEnabled (!running);
  cb_pct_dist1->setEnabled (!running && !cb_fix_dist1->isChecked ());
  le_pct_dist1->setEnabled (!running && !cb_fix_dist1->isChecked ()
                            && cb_pct_dist1->isChecked ());
  ...
  le_pct_dist2->setEnabled (!running && !cb_fix_dist2->isChecked ()
                            && cb_pct_dist2->isChecked ());
  ...
  //    cb_pct_dist1->isChecked() ) && ( !dist2_active ||
  //    cb_fix_dist2->isChecked() || cb_pct_dist2->isChecked() )
  //    ;

  pb_restore->setEnabled (!running && gaussians_undo.size () > 1);
  pb_undo->setEnabled ((!running || cb_test_mode->isChecked ())
                       && gaussians_undo.size () > 1);
  ...
  pb_ga->setEnabled (
      false); // && !running && run_ok && variations_set && !use_errors );
  pb_grid->setEnabled (
      false); // && !running && run_ok && variations_set && !use_errors );
...
map<unsigned int, unsigned int>
    comm_backref; // back reference to variable param position of 1st usage

vector<double> unified_q;                           // copy from mals win
vector<unsigned int> unified_ggaussian_param_index; // copy from mals win
unsigned int unified_ggaussian_gaussians_size;      // copy from mals win
unsigned int unified_ggaussian_curves;              // copy from mals win
...
double
compute_gaussian_f_GAUSS (double t, const double *par)
{
  double result = 0e0;
  double height;
  double center;
  ...
  for (unsigned int i = 0; i < unified_ggaussian_gaussians_size; i++)
    {
      ...
      /*
      cout << QString( "for pos %1 t is %2 index %3 gaussian %4 center %5
      height %6 width %7\n" ) .arg( t ) .arg( unified_q[ ( unsigned int ) t ] )
         .arg( index )
         .arg( i )
         .arg( center )
         .arg( height )
...
double
compute_gaussian_f_EMG (double t, const double *par)
{
  double result = 0e0;
  double height;
  double center;
  ...
  for (unsigned int i = 0; i < unified_ggaussian_gaussians_size; i++)
    {
      ...
      if (is_common[3])
        ...
        /*
        cout << QString( "for pos %1 t is %2 index %3 gaussian %4 center %5
        height %6 width %7 dist1 %8\n" ) .arg( t ) .arg( unified_q[ ( unsigned
        int ) t ] ) .arg( index ) .arg( i ) .arg( center ) .arg( height ) .arg(
        width ) .arg( dist1 )
        */
        ;
      if (!dist1)
          ...
          if (fabs (dist1) < dist1_thresh)
            {
              ...
              double emg_coeff
                  = area * one_over_a3 * 5e-1 * (1e0 - frac_gauss);
              double emg_exp_1
              ...
              result += emg_coeff * exp (emg_exp_1 - one_over_a3 * tmp)
...
double
compute_gaussian_f_GMG (double t, const double *par)
{
  double result = 0e0;
  double height;
  double center;
      ...
      */
      ...
      if (!dist1)
      ...
      else
        {
          ...
          double tmp = unified_q[(unsigned int)t] - center;
          result += gmg_coeff * exp (gmg_exp_m1 * tmp * tmp)
...
double
compute_gaussian_f_EMGGMG (double t, const double *par)
{
  double result = 0e0;
  double height;
  double center;
  ...
  for (unsigned int i = 0; i < unified_ggaussian_gaussians_size; i++)
    {
      ...
      /*
      cout << QString( "for pos %1 t is %2 index %3 gaussian %4 center %5
      height %6 width %7 dist1 %8 dist2 %9\n" ) .arg( t ) .arg( unified_q[ (
      unsigned int ) t ] ) .arg( index ) .arg( i ) .arg( center ) .arg( height
      ) .arg( width ) .arg( dist1 ) .arg( dist2 )
         ;
      */
          ...
          if (!dist1)
            {
              ...
              double one_over_width = 1e0 / width;
              double one_over_a2sq_plus_a3sq
                  = 1e0 / (width * width + dist2 * dist2);
              double sqrt_one_over_a2sq_plus_a3sq
                  = sqrt (one_over_a2sq_plus_a3sq);
              double gmg_coeff
                  = area * M_ONE_OVER_SQRT2PI * sqrt_one_over_a2sq_plus_a3sq;
              double gmg_exp_m1 = -5e-1 * one_over_a2sq_plus_a3sq;
          ...
          else
            {
              if (!dist2)
                  ...
                  if (fabs (dist1) < dist1_thresh)
                    {
                      ...
                      double frac_gauss
                          = (dist1_thresh - fabs (dist1)) / dist1_thresh;
                      if (dist1 < 0)
                        {
                          dist1_thresh *= -1e0;
                        }
                      ...
                      double emg_exp_1
                          = width * width * one_over_a3 * one_over_a3 * 5e-1;
                      double emg_erf_2
                          = width * M_ONE_OVER_SQRT2 * one_over_a3;
                      double sign_a3 = dist1_thresh < 0e0 ? -1e0 : 1e0;
                      double one_over_sqrt2_a2 = M_ONE_OVER_SQRT2 / width;
                      double gauss_coeff = frac_gauss * height;
              ...
              else
                {
                  ...
                  double sqrt_one_over_a2sq_plus_a3sq
                      = sqrt (one_over_a2sq_plus_a3sq);
                  double gmg_coeff = 5e-1 * area * M_ONE_OVER_SQRT2PI
                                     * sqrt_one_over_a2sq_plus_a3sq;
                  double gmg_exp_m1 = -5e-1 * one_over_a2sq_plus_a3sq;
                  double gmg_erf_m1 = dist2 * sqrt_one_over_a2sq_plus_a3sq
                                      * M_ONE_OVER_SQRT2 * one_over_width;
...
/* old way
  ...
  //          cout << QString( "for pos %1 t is %2 index %3 gaussian %4 center
  %5 height %6 width %7\n" )
  //             .arg( t )
...
bool
US_Hydrodyn_Mals_Fit_Global::setup_run ()
{
  MFIT_GLOBAL::init_params.clear ();
  MFIT_GLOBAL::base_params.clear ();
  MFIT_GLOBAL::fixed_params.clear ();
  MFIT_GLOBAL::comm_backref.clear ();
  MFIT_GLOBAL::param_pos.clear ();
  MFIT_GLOBAL::param_fixed.clear ();
  MFIT_GLOBAL::param_min.clear ();
  MFIT_GLOBAL::param_max.clear ();
  MFIT_GLOBAL::unified_q = mals_win->unified_ggaussian_q;
  MFIT_GLOBAL::unified_ggaussian_param_index
      = mals_win->unified_ggaussian_param_index;
  MFIT_GLOBAL::unified_ggaussian_gaussians_size
      = mals_win->unified_ggaussian_gaussians_size;
  MFIT_GLOBAL::unified_ggaussian_curves = mals_win->unified_ggaussian_curves;

  MFIT_GLOBAL::per_file_size = per_file_size;
  MFIT_GLOBAL::common_size = common_size;
  MFIT_GLOBAL::is_common = is_common;
  ...
  switch (gaussian_type)
    {
    case US_Hydrodyn_Mals::EMGGMG:
      MFIT_GLOBAL::compute_gaussian_f
          = &MFIT_GLOBAL::compute_gaussian_f_EMGGMG;
      // gsm_f = &gsm_f_EMGGMG;
      break;
    case US_Hydrodyn_Mals::EMG:
      MFIT_GLOBAL::compute_gaussian_f = &MFIT_GLOBAL::compute_gaussian_f_EMG;
      // gsm_f = &gsm_f_EMG;
      break;
    case US_Hydrodyn_Mals::GMG:
      MFIT_GLOBAL::compute_gaussian_f = &MFIT_GLOBAL::compute_gaussian_f_GMG;
      // gsm_f = &gsm_f_GMG;
      break;
    case US_Hydrodyn_Mals::GAUSS:
      MFIT_GLOBAL::compute_gaussian_f = &MFIT_GLOBAL::compute_gaussian_f_GAUSS;
      // gsm_f = &gsm_f_GAUSS;
      break;
    }

  MFIT_GLOBAL::use_errors = use_errors;

  //    QStringList qsl = (le_fix_curves->text().split( "," ,
  //    Qt::SkipEmptyParts ) );
  ...
  double dist = ((US_Hydrodyn *)(mals_win->us_hydrodyn))
                    ->gparams["mals_dist_max"]
                    .toDouble ();
  ...
  // param_pos   : the index into either fixed_params (non-changing) or
  // init_params (variable during search) fixed_params: all the "non-changing
  // during search params" init_params : all the "variable during search
  // params" // this will be the search vector param_fixed : boolean as to
  // whether or not the specific param is fixed(true) or variable(false)
  // base_params : these are a copy of the init_params without replacement from
  // the mals_win unified_ggaussians
  ...
  // unified ggaussians:
  // f1,f2,..fg,file=1{v1,v2,..,vg},file=2{v1,v2,..,vg},...,file=k{v1,v2,..,vg}
  // variable access for element i of above is ( param_fixed[ i ] ?
  // fixed_params[ param_pos[ i ] ] : init_params[ param_pos[ i ] ] )

  // unified_ggaussians have in "fix_width" mode first
  // "unified_ggaussian_gaussians_size*common_size" elements as common centers
  // and common widths and the next unified_ggaussian_gaussians_size *
  // per_file_size * unified_ggaussian_curves elements as the curve specific
  // variables the fixed_params are those that are fixed globally thie

  // the common params first, since they are not per file

  for (unsigned int i = 0; i < mals_win->unified_ggaussian_gaussians_size; i++)
    {
      ...
      if (cb_fix_center->isChecked () || fixed_curves.count (i))
        {
          MFIT_GLOBAL::param_pos.push_back (MFIT_GLOBAL::fixed_params.size ());
          MFIT_GLOBAL::fixed_params.push_back (
              mals_win->unified_ggaussian_params[ofs]);
          MFIT_GLOBAL::param_fixed.push_back (true);
        }
      else
        {
          MFIT_GLOBAL::param_pos.push_back (MFIT_GLOBAL::init_params.size ());
          ...
          if (cb_pct_center_from_init->isChecked ())
            ...
            {
              base_val = mals_win->unified_ggaussian_params[ofs];
            }

          MFIT_GLOBAL::init_params.push_back (base_val);
          MFIT_GLOBAL::base_params.push_back (
              mals_win->unified_ggaussian_params[ofs]);
          MFIT_GLOBAL::param_fixed.push_back (false);
          ...
          if (MFIT_GLOBAL::init_params.back () < min)
            {
              MFIT_GLOBAL::init_params.back () = min;
            }
          if (MFIT_GLOBAL::init_params.back () > max)
            {
              MFIT_GLOBAL::init_params.back () = max;
            }

          MFIT_GLOBAL::param_min.push_back (min);
          MFIT_GLOBAL::param_max.push_back (max);
        }
      ofs++;
      this_ofs++;

      if (mals_win->cb_fix_width->isChecked ()) // if widths are common
        {
          if (cb_fix_width->isChecked () || fixed_curves.count (i))
            {
              MFIT_GLOBAL::param_pos.push_back (
                  MFIT_GLOBAL::fixed_params.size ());
              MFIT_GLOBAL::fixed_params.push_back (
                  mals_win->unified_ggaussian_params[ofs]);
              MFIT_GLOBAL::param_fixed.push_back (true);
            }
      ...
      if (dist1_active
          && mals_win->cb_fix_dist1->isChecked ()) // if dist1s are common
        {
          if (cb_fix_dist1->isChecked () || fixed_curves.count (i)
              || (cb_comm_dist1->isChecked () && i))
            {
              MFIT_GLOBAL::param_pos.push_back (
                  MFIT_GLOBAL::fixed_params.size ());
              if (i && cb_comm_dist1->isChecked () && !fixed_curves.count (i)
                  && !fixed_curves.count (0))
                {
                  MFIT_GLOBAL::fixed_params.push_back (
                      mals_win->unified_ggaussian_params[this_ofs]);
                  MFIT_GLOBAL::comm_backref[MFIT_GLOBAL::param_fixed.size ()]
                      = comm_dist_1_var_pos;
                }
              else
                {
          ...
          else
            {
              if (cb_comm_dist1->isChecked ())
                {
                  comm_dist_1_var_pos = MFIT_GLOBAL::init_params.size ();
                }
          ...
          if (dist2_active
              && mals_win->cb_fix_dist2->isChecked ()) // if dist2s are common
            {
              if (cb_fix_dist2->isChecked () || fixed_curves.count (i)
                  || (cb_comm_dist2->isChecked () && i))
                {
                  MFIT_GLOBAL::param_pos.push_back (
                      MFIT_GLOBAL::fixed_params.size ());
                  if (i && cb_comm_dist2->isChecked ()
                      && !fixed_curves.count (i) && !fixed_curves.count (0))
                    {
                      MFIT_GLOBAL::fixed_params.push_back (
                          mals_win->unified_ggaussian_params[this_ofs]);
                      MFIT_GLOBAL::comm_backref[MFIT_GLOBAL::param_fixed
                                                    .size ()]
                          = comm_dist_2_var_pos;
                    }
                  else
              ...
              else
                {
                  if (cb_comm_dist2->isChecked ())
                    {
                      comm_dist_2_var_pos = MFIT_GLOBAL::init_params.size ();
                    }
                  ...
                  if (cb_pct_dist2->isChecked ())
                    {
                      ofs = base_val * le_pct_dist2->text ().toDouble ()
                            / 100.0;
                      min = base_val - ofs;
                      max = base_val + ofs;
                    }
  ...
  unsigned int base_ofs
      = mals_win->unified_ggaussian_gaussians_size * common_size;
  // assert( (unsigned int)MFIT_GLOBAL::param_pos.size() == base_ofs &&
  // "common_size*number of gaussians should equal param_pos.size()" );

  for (unsigned int f = 0; f < mals_win->unified_ggaussian_curves; f++)
    {
      unsigned int ofs
          = base_ofs
            + f * per_file_size * mals_win->unified_ggaussian_gaussians_size;
      ...
      for (unsigned int i = 0; i < mals_win->unified_ggaussian_gaussians_size;
           i++)
        {
          ...
          if (cb_fix_amplitude->isChecked () || fixed_curves.count (i))
            {
              MFIT_GLOBAL::param_pos.push_back (
                  MFIT_GLOBAL::fixed_params.size ());
              MFIT_GLOBAL::fixed_params.push_back (
          ...
          else
            {
              ...
              if (cb_pct_amplitude->isChecked ())
                {
                  ofs = base_val * le_pct_amplitude->text ().toDouble ()
                        / 100.0;
                  min = base_val - ofs;
                  max = base_val + ofs;
                }
          ...
          if (!mals_win->cb_fix_width
                   ->isChecked ()) // if widths are not common
            {
              if (cb_fix_width->isChecked () || fixed_curves.count (i))
                {
                  MFIT_GLOBAL::param_pos.push_back (
              ...
              else
                {
                  ...
                  if (cb_pct_width->isChecked ())
                    {
                      ofs = base_val * le_pct_width->text ().toDouble ()
                            / 100.0;
                      min = base_val - ofs;
                      max = base_val + ofs;
                    }
          ...
          if (dist1_active
              && !mals_win->cb_fix_dist1
                      ->isChecked ()) // if dist1s are not common
            {
              if (cb_fix_dist1->isChecked () || fixed_curves.count (i)
                  || (cb_comm_dist1->isChecked () && i))
                {
              ...
              else
                {
                  ...
                  if (cb_pct_dist1->isChecked ())
                    {
                      ofs = base_val * le_pct_dist1->text ().toDouble ()
                            / 100.0;
                      min = base_val - ofs;
                      max = base_val + ofs;
                    }
              ...
              this_ofs++;
              if (dist2_active
                  && !mals_win->cb_fix_dist2
                          ->isChecked ()) // if dist2s are common
                {
                  if (cb_fix_dist2->isChecked () || fixed_curves.count (i)
                      || (cb_comm_dist2->isChecked () && i))
                    {
  ...
  {
     // unified_ggaussians have in "fix_width" mode first
  "unified_ggaussian_gaussians_size*2" elements as common centers and common
  widths
     // and the next unified_ggaussian_gaussians_size * 1 *
  unified_ggaussian_curves elements as the curve specific
     // heights
  ...
  i++ )
     {
        ...
        {
           MFIT_GLOBAL::param_pos   .push_back(
  MFIT_GLOBAL::fixed_params.size() ); MFIT_GLOBAL::fixed_params.push_back(
  mals_win->unified_ggaussian_params[ 2 * i + 0 ] ); MFIT_GLOBAL::param_fixed
  .push_back( true ); } else { MFIT_GLOBAL::param_pos   .push_back(
  MFIT_GLOBAL::init_params.size() );
           ...
           if ( cb_pct_center_from_init->isChecked() )
           {
              ...
              base_val = mals_win->unified_ggaussian_params[ 2 * i + 0 ];
           }
  ...
  .push_back( false );
             ...
             fixed_curves.count( i ) )
        {
           MFIT_GLOBAL::param_pos   .push_back(
  MFIT_GLOBAL::fixed_params.size() ); MFIT_GLOBAL::fixed_params.push_back(
  mals_win->unified_ggaussian_params[ 2 * i + 1 ] ); MFIT_GLOBAL::param_fixed
  .push_back( true ); } else { MFIT_GLOBAL::param_pos   .push_back(
  MFIT_GLOBAL::init_params.size() );
           ...
           if ( cb_pct_width_from_init->isChecked() )
           {
              ...
              base_val = mals_win->unified_ggaussian_params[ 2 * i + 1 ];
           }
  ...
  .push_back( false );
     ...
     for ( unsigned int i = 0; i < mals_win->unified_ggaussian_gaussians_size *
  mals_win->unified_ggaussian_curves; i++ )
     {
        unsigned int pos = i / mals_win->unified_ggaussian_curves;

        unsigned int height_pos = 2 *
  mals_win->unified_ggaussian_gaussians_size + i + 0;
             ...
             fixed_curves.count( pos ) )
        {
           MFIT_GLOBAL::param_pos   .push_back(
  MFIT_GLOBAL::fixed_params.size() ); MFIT_GLOBAL::fixed_params.push_back(
  mals_win->unified_ggaussian_params[ height_pos ] ); MFIT_GLOBAL::param_fixed
  .push_back( true ); } else { MFIT_GLOBAL::param_pos   .push_back(
  MFIT_GLOBAL::init_params.size() );
           ...
           if ( cb_pct_amplitude_from_init->isChecked() )
           {
              ...
              base_val = mals_win->unified_ggaussian_params[ height_pos ];
           }
  ...
  .push_back( false );
     ...
     // unified_ggaussians have first "unified_ggaussian_gaussians_size"
  elements as common centers
     // and the next unified_ggaussian_gaussians_size * 2 *
  unified_ggaussian_curves elements as the curve specific
     // height, widths
  ...
  i++ )
     {
             ...
             fixed_curves.count( i ) )
        {
           MFIT_GLOBAL::param_pos   .push_back(
  MFIT_GLOBAL::fixed_params.size() ); MFIT_GLOBAL::fixed_params.push_back(
  mals_win->unified_ggaussian_params[ i ] ); MFIT_GLOBAL::param_fixed
  .push_back( true ); } else { MFIT_GLOBAL::param_pos   .push_back(
  MFIT_GLOBAL::init_params.size() );
           ...
           if ( cb_pct_center_from_init->isChecked() )
           {
              ...
              base_val = mals_win->unified_ggaussian_params[ i ];
           }
  ...
  .push_back( false );
     ...
     for ( unsigned int i = 0; i < mals_win->unified_ggaussian_gaussians_size *
  2 * mals_win->unified_ggaussian_curves; i+= 2 )
     {
        unsigned int pos = i / ( 2 * mals_win->unified_ggaussian_curves );

        unsigned int height_pos = mals_win->unified_ggaussian_gaussians_size +
  i + 0; unsigned int width_pos  = mals_win->unified_ggaussian_gaussians_size +
  i + 1;
  ...
  .push_back( false );
             ...
             fixed_curves.count( pos + 1 ) )
        {
           MFIT_GLOBAL::param_pos   .push_back(
  MFIT_GLOBAL::fixed_params.size() ); MFIT_GLOBAL::fixed_params.push_back(
  mals_win->unified_ggaussian_params[ width_pos ] ); MFIT_GLOBAL::param_fixed
  .push_back( true ); } else { MFIT_GLOBAL::param_pos   .push_back(
  MFIT_GLOBAL::init_params.size() );
           ...
           if ( cb_pct_width_from_init->isChecked() )
           {
              ...
              base_val = mals_win->unified_ggaussian_params[ width_pos ];
           }
  ...
  */
  ...
  // MFIT_GLOBAL::list_params();

  // US_Vector::printvector( "is_common", is_common );

  if (!MFIT_GLOBAL::init_params.size ())
    {
      return false;
    }
  else
...
void
US_Hydrodyn_Mals_Fit_Global::test_mode ()
{
  if (!cb_test_mode->isChecked ())
    {
  ...
  pb_test->show ();
  lbl_test_info->show ();
  plot_test->detachItems (QwtPlotItem::Rtti_PlotCurve);
  plot_test->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
  plot_test->replot ();
  if (plot_test_zoomer)
    {
  ...
  for (int i = 0; i < (int)MFIT_GLOBAL::init_params.size (); ++i)
    {
      ...
      le->setEnabled (true);
      le->setText (QString ("%1").arg (MFIT_GLOBAL::init_params[i]));
      le->show ();
...
void
US_Hydrodyn_Mals_Fit_Global::test ()
{
  if (MFIT_GLOBAL::init_params.size () != test_widgets.size ())
    {
  ...
  for (int i = 0; i < (int)MFIT_GLOBAL::init_params.size (); ++i)
    {
      MFIT_GLOBAL::init_params[i]
          = ((QLineEdit *)test_widgets[i])->text ().toDouble ();
    }
      ...
      if (!MFIT_GLOBAL::param_fixed[i] || MFIT_GLOBAL::comm_backref.count (i))
        {
          mals_win->unified_ggaussian_params[i]
              = MFIT_GLOBAL::init_params[MFIT_GLOBAL::comm_backref.count (i)
                                             ? MFIT_GLOBAL::comm_backref[i]
                                             : MFIT_GLOBAL::param_pos[i]];
          us_qdebug (QString ("param %1 taken from init params pos %2 %3")
                         .arg (i)
                         .arg (MFIT_GLOBAL::comm_backref.count (i)
                                   ? MFIT_GLOBAL::comm_backref[i]
                                   : MFIT_GLOBAL::param_pos[i])
                         .arg (MFIT_GLOBAL::comm_backref.count (i)
                                   ? "backref used"
  ...
  vector<double> gsum = mals_win->compute_ggaussian_gaussian_sum ();
  vector<double> t = mals_win->unified_ggaussian_t;

#if QT_VERSION < 0x040000
  plot_test->setCurveData (test_curve, (double *)&(t[0]), (double *)&(gsum[0]),
                           t.size ());
  plot_test->setCurvePen (test_curve, QPen (Qt::green, 1, Qt::SolidLine));
#else
  test_curve->setSamples ((double *)&(t[0]), (double *)&(gsum[0]), t.size ());
  ...
  if (!plot_test_zoomer)
    {
      ...
      // connect( plot_test_zoomer, SIGNAL( zoomed( const QRectF & ) ), SLOT(
      // plot_test_zoomed( const QRectF & ) ) );
    }
...
void
US_Hydrodyn_Mals_Fit_Global::lm ()
{
  update_common ();
  running = true;
  update_enables ();
  ...
  vector<double> t = mals_win->unified_ggaussian_t;
  vector<double> y = mals_win->unified_ggaussian_I;

  vector<double> org_params = MFIT_GLOBAL::init_params;

  MFIT_GLOBAL::errors = mals_win->unified_ggaussian_e;
  MFIT_GLOBAL::errors_index.clear ();

  if (use_errors)
    {
      MFIT_GLOBAL::errors_index.resize ((unsigned int)t.back () + 1);
      for (unsigned int i = 0; i <= (unsigned int)t.back (); i++)
        {
          MFIT_GLOBAL::errors_index[i] = 0;
        }
      for (unsigned int i = 0; i < y.size (); i++)
        {
          y[i] /= MFIT_GLOBAL::errors[i];
          MFIT_GLOBAL::errors_index[(unsigned int)t[i]] = i;
        }
    }

  /* maybe no longer need
  vector < double > save_unified_ggaussians =
  mals_win->unified_ggaussian_params;

  for ( unsigned int i = 0; i < MFIT_GLOBAL::param_fixed.size(); i++ )
  {
     if ( !MFIT_GLOBAL::param_fixed[ i ] ||
          MFIT_GLOBAL::comm_backref.count( i ) )
     {
        mals_win->unified_ggaussian_params[ i ] = MFIT_GLOBAL::init_params[
                                                                           MFIT_GLOBAL::comm_backref.count(
  i ) ? MFIT_GLOBAL::comm_backref[ i ] : MFIT_GLOBAL::param_pos[ i ]
                                                                            ];
     }
  }
  ...
  */
  ...
  double org_rmsd = mals_win->ggaussian_rmsd (false);
  ...
  for (unsigned int i = 0; i < (unsigned int)t.size (); i++)
    {
      gsumf[i] = (*MFIT_GLOBAL::compute_gaussian_f) (
          t[i], &MFIT_GLOBAL::init_params[0]);
    }

  /* mals_win->unified_ggaussian_params = save_unified_ggaussians; */
...
#define TOL 1e-5
          ...
          if (tol_ok)
            {
              cout
                  << QString ("NOTICE: gsums within tolerance %1\n").arg (TOL);
            }
          else
            {
...
#undef TOL
  ...
  // mals_win->add_ggaussian_curve( "lm_start", gsumf );

  vector<double> par = MFIT_GLOBAL::init_params;
  // US_Vector::printvector( QString( "par start (rmsd %1)" ).arg( org_rmsd ),
  // par );
  ...
  qApp->processEvents ();
  // LM::qpb  = mals_win->progress;
  // LM::qApp = qApp;

  LM::lmcurve_fit_rmsd ((int)par.size (), (double *)&(par[0]), (int)t.size (),
                        (double *)&(t[0]), (double *)&(y[0]),
                        MFIT_GLOBAL::compute_gaussian_f,
                        (const LM::lm_control_struct *)&control, &status);
  ...
  // US_Vector::printvector( QString( "par after fit (norm %1)" ).arg(
  // status.fnorm ), par );
  ...
  if (org_rmsd > status.fnorm)
    {
      for (unsigned int i = 0; i < MFIT_GLOBAL::param_fixed.size (); i++)
        {
          if (!MFIT_GLOBAL::param_fixed[i]
            ...
            {
              mals_win->unified_ggaussian_params[i]
                  = par[MFIT_GLOBAL::comm_backref.count (i)
                            ? MFIT_GLOBAL::comm_backref[i]
                            : MFIT_GLOBAL::param_pos[i]];
            }
        }
      ...
      // mals_win->add_ggaussian_curve( "lm_after_pushback",
      // mals_win->compute_ggaussian_gaussian_sum() );
      gaussians_undo.push_back (mals_win->unified_ggaussian_params);
      if (update_mals)
        {
...
void
US_Hydrodyn_Mals_Fit_Global::gsm_sd ()
{
  ...
  {
     gsm_yp[ j ]  = MFIT_GLOBAL::compute_gaussian_f( gsm_t[ j ], (double
  *)(&MFIT_GLOBAL::base_params[ 0 ]) ); org_rmsd += ( gsm_y[ j ] - gsm_yp[ j ]
  ) * ( gsm_y[ j ] - gsm_yp[ j ] );
  }

  org_rmsd = sqrt( org_rmsd );

  our_vector *v = new_our_vector( MFIT_GLOBAL::init_params.size() );
  for ( int i = 0; i < v->len; i++ )
  {
     v->d[ i ] = MFIT_GLOBAL::init_params[ i ];
  }
  ...
  if ( org_rmsd > rmsd )
  {
     for ( unsigned int i = 0; i < MFIT_GLOBAL::param_fixed.size(); i++ )
     {
        if ( !MFIT_GLOBAL::param_fixed[ i ] ||
             MFIT_GLOBAL::comm_backref.count( i ) )
        {
           mals_win->unified_ggaussian_params[ i ] = v->d[
           MFIT_GLOBAL::comm_backref.count( i ) ?
           MFIT_GLOBAL::comm_backref[ i ] :
           MFIT_GLOBAL::param_pos[ i ]
           ];
...
void
US_Hydrodyn_Mals_Fit_Global::gsm_ih ()
{
  return; // have to rewrite this
...
void
US_Hydrodyn_Mals_Fit_Global::gsm_cg ()
{
  return; // have to rewrite this
...
void
US_Hydrodyn_Mals_Fit_Global::ga ()
{
  /*
  gsm_setup();
       ...
       rmsd < org_rmsd )
  {
     ...
     for ( unsigned int i = 0; i < MFIT_GLOBAL::param_fixed.size(); i++ )
     {
        if ( !MFIT_GLOBAL::param_fixed[ i ] ||
             MFIT_GLOBAL::comm_backref.count( i ) )
        {
           mals_win->unified_ggaussian_params[ i ] = par[
           MFIT_GLOBAL::comm_backref.count( i ) ?
           MFIT_GLOBAL::comm_backref[ i ] :
           MFIT_GLOBAL::param_pos[ i ]
           ];
...
void
US_Hydrodyn_Mals_Fit_Global::stop ()
{
  running = false;
}

void
US_Hydrodyn_Mals_Fit_Global::grid ()
{
  return; // have to rewrite this
  setup_run ();
  cout << "gauss fit start\n";

  vector<double> x = mals_win->f_qs[mals_win->wheel_file];
  vector<double> t;
  vector<double> y;
  vector<double> yp (x.size ());

  double start = mals_win->le_gauss_fit_start->text ().toDouble ();
  double end = mals_win->le_gauss_fit_end->text ().toDouble ();
  ...
  for (unsigned int j = 0; j < x.size (); j++)
    {
      if (x[j] >= start && x[j] <= end)
        {
          t.push_back (x[j]);
          y.push_back (mals_win->f_Is[mals_win->wheel_file][j]);
        }
    }
  ...
  unsigned int total_count = (unsigned int)pow (
      (double)pop_size, (int)MFIT_GLOBAL::init_params.size ());
  ...
  for (unsigned int j = 0; j < (unsigned int)MFIT_GLOBAL::init_params.size ();
       j++)
    {
      ofs_per[j] = (MFIT_GLOBAL::param_max[j] - MFIT_GLOBAL::param_min[j])
                   / (pop_size - 1);
    }
  ...
  for (unsigned int j = 0; j < t.size (); j++)
    {
      yp[j] = MFIT_GLOBAL::compute_gaussian_f (
          t[j], (double *)(&MFIT_GLOBAL::base_params[0]));
  ...
  for (unsigned int i = 0; i < total_count; i++)
    {
      ...
      for (unsigned int j = 0;
           j < (unsigned int)MFIT_GLOBAL::init_params.size (); j++)
        {
          unsigned int pos = remainder % pop_size;
          use_par[j] = MFIT_GLOBAL::param_min[j] + pos * ofs_per[j];
          remainder /= pop_size;
        }
      ...
      for (unsigned int j = 0; j < t.size (); j++)
        {
          yp[j] = MFIT_GLOBAL::compute_gaussian_f (t[j],
                                                   (double *)(&use_par[0]));
...
void
US_Hydrodyn_Mals_Fit_Global::usp_config_plot_test (const QPoint &)
{
