...
void US_Hydrodyn_Dad::pm_enables() {
...
void US_Hydrodyn_Dad::pm_q_reset() {
  le_pm_q_start->setText(QString("%1").arg(f_qs[wheel_file][0]));
  le_pm_q_end->setText(QString("%1").arg(f_qs[wheel_file].back()));
}

void US_Hydrodyn_Dad::pm_grid_size_text(const QString &) {}

void US_Hydrodyn_Dad::pm_buff_e_dens_text(const QString &) {}

void US_Hydrodyn_Dad::pm_samp_e_dens_text(const QString &) {}

void US_Hydrodyn_Dad::pm_q_pts_text(const QString &) {}

void US_Hydrodyn_Dad::pm_q_start_text(const QString &text) {
#if QT_VERSION < 0x040000
...
void US_Hydrodyn_Dad::pm_q_end_text(const QString &text) {
#if QT_VERSION < 0x040000
...
void US_Hydrodyn_Dad::pm_q_start_focus(bool hasFocus) {
  if (hasFocus) {
...
void US_Hydrodyn_Dad::pm_q_end_focus(bool hasFocus) {
  if (hasFocus) {
...
void US_Hydrodyn_Dad::rgc_enables() {
...
void US_Hydrodyn_Dad::rgc_shape() {
  if (rb_rgc_shape_ellipsoid->isChecked()) {
...
void US_Hydrodyn_Dad::rgc_calc_rg() {
  double V = 0e0;
...
void US_Hydrodyn_Dad::rgc_mw_text(const QString &) { rgc_calc_rg(); }

void US_Hydrodyn_Dad::rgc_vol_text(const QString &) {
...
void US_Hydrodyn_Dad::rgc_rho_text(const QString &) { rgc_calc_rg(); }

void US_Hydrodyn_Dad::rgc_axis_b_text(const QString &, bool do_recompute) {
...
void US_Hydrodyn_Dad::rgc_axis_c_text(const QString &, bool do_recompute) {
  if (le_rgc_axis_b->text().toDouble() < le_rgc_axis_c->text().toDouble()) {
...
void US_Hydrodyn_Dad::rgc_rg_text(const QString &) {}
...
void US_Hydrodyn_Dad::testiq_gauss_line() {
...
void US_Hydrodyn_Dad::testiq_enables() {
  pb_timeshift->setEnabled(false);
  pb_wheel_cancel->setEnabled(true);
...
bool US_Hydrodyn_Dad::testiq_make() {
...
void US_Hydrodyn_Dad::testiq_visrange() {
  disconnect(le_testiq_q_start, SIGNAL(textChanged(const QString &)), 0, 0);
  disconnect(le_testiq_q_end, SIGNAL(textChanged(const QString &)), 0, 0);
  if (plot_dist_zoomer) {
#if QT_VERSION >= 0x040000
...
void US_Hydrodyn_Dad::testiq_testset() {
  if (!testiq_make()) {
    testiq_enables();
    return;
  }
...
void US_Hydrodyn_Dad::testiq_q_start_text(const QString &text) {
...
void US_Hydrodyn_Dad::testiq_q_end_text(const QString &text) {
  if (current_mode != MODE_TESTIQ) {
    return;
  }
#if QT_VERSION < 0x040000
...
void US_Hydrodyn_Dad::testiq_q_start_focus(bool hasFocus) {
  if (current_mode != MODE_TESTIQ) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Dad::testiq_q_end_focus(bool hasFocus) {
  if (current_mode != MODE_TESTIQ) {
    return;
  }
  if (hasFocus) {
...
// --- wyatt ---
void US_Hydrodyn_Dad::wyatt_start() {
  if (axis_y_log) {
    axis_y();
  }
...
void US_Hydrodyn_Dad::wyatt_2() {
  wyatt_init_markers();
  replot_wyatt();
  wyatt_enables();
}

void US_Hydrodyn_Dad::wyatt_enables() {
...
void US_Hydrodyn_Dad::wyatt_init_markers() {
  gauss_delete_markers();
...
void US_Hydrodyn_Dad::wyatt_apply() { wyatt_apply(all_selected_files()); }

void US_Hydrodyn_Dad::wyatt_apply(const QStringList &files) {
...
void US_Hydrodyn_Dad::wyatt_start_text(const QString &text) {
  int pos = 0;
#if QT_VERSION < 0x040000
...
void US_Hydrodyn_Dad::wyatt_end_text(const QString &text) {
  int pos = 1;
#if QT_VERSION < 0x040000
  plot_dist->setMarkerPos(plotted_markers[pos], text.toDouble(), 0e0);
#else
...
void US_Hydrodyn_Dad::wyatt_start_focus(bool hasFocus) {
...
void US_Hydrodyn_Dad::wyatt_end_focus(bool hasFocus) {
...
void US_Hydrodyn_Dad::wyatt_start2_text(const QString &text) {
  int pos = 2;
#if QT_VERSION < 0x040000
  plot_dist->setMarkerPos(plotted_markers[pos], text.toDouble(), 0e0);
#else
...
void US_Hydrodyn_Dad::wyatt_end2_text(const QString &text) {
  int pos = 3;
#if QT_VERSION < 0x040000
  plot_dist->setMarkerPos(plotted_markers[pos], text.toDouble(), 0e0);
#else
...
void US_Hydrodyn_Dad::wyatt_start2_focus(bool hasFocus) {
...
void US_Hydrodyn_Dad::wyatt_end2_focus(bool hasFocus) {
...
void US_Hydrodyn_Dad::replot_wyatt() {
...
void US_Hydrodyn_Dad::scale_scroll_highlight(int pos) {
...
void US_Hydrodyn_Dad::scale_scroll() {
...
void US_Hydrodyn_Dad::scale_update_plot_errors() {
...
void US_Hydrodyn_Dad::scale_enables() {
...
QString US_Hydrodyn_Dad::scale_get_target(bool do_msg) {
  double min_i = 0e0;
  double max_i = 0e0;
  double tot_i = 0e0;
...
void US_Hydrodyn_Dad::scale_apply() {
...
void US_Hydrodyn_Dad::scale_q_reset() {
  le_scale_q_start->setText(QString("%1").arg(scale_q[wheel_file][0]));
  le_scale_q_end->setText(QString("%1").arg(scale_q[wheel_file].back()));
}

void US_Hydrodyn_Dad::scale_reset() {
  for (set<QString>::iterator it = scale_selected.begin();
       it != scale_selected.end(); ++it) {
    scale_q[*it] = f_qs[*it];
    scale_I[*it] = f_Is[*it];
...
void US_Hydrodyn_Dad::scale_replot() {
  for (set<QString>::iterator it = scale_selected.begin();
       it != scale_selected.end(); ++it) {
#if QT_VERSION < 0x040000
...
void US_Hydrodyn_Dad::scale_create() {
  scale_last_created.clear();
...
void US_Hydrodyn_Dad::scale_q_start_text(const QString &text) {
  if (current_mode != MODE_SCALE) {
    return;
  }
#if QT_VERSION < 0x040000
...
void US_Hydrodyn_Dad::scale_q_end_text(const QString &text) {
  if (current_mode != MODE_SCALE) {
    return;
  }
#if QT_VERSION < 0x040000
...
void US_Hydrodyn_Dad::scale_q_start_focus(bool hasFocus) {
  if (current_mode != MODE_SCALE) {
    return;
  }
...
void US_Hydrodyn_Dad::scale_q_end_focus(bool hasFocus) {
  if (current_mode != MODE_SCALE) {
    return;
  }
  if (hasFocus && scale_q.count(wheel_file)) {
...
void US_Hydrodyn_Dad::ggauss_start() {
  plot_errors->detachItems(QwtPlotItem::Rtti_PlotCurve);
  plot_errors->detachItems(QwtPlotItem::Rtti_PlotMarker);
  ;
  // if ( plot_errors_zoomer )
...
void US_Hydrodyn_Dad::ggauss_scroll() {
...
void US_Hydrodyn_Dad::ggauss_scroll_p_green() {
  us_qdebug("ggauss_scroll_p_green()");
  ggauss_scroll_set_selected();
}

void US_Hydrodyn_Dad::ggauss_scroll_p_red() {
  us_qdebug("ggauss_scroll_p_yellow()");
  ggauss_scroll_set_selected();
}

void US_Hydrodyn_Dad::ggauss_scroll_p_yellow() {
  us_qdebug("ggauss_scroll_p_yellow()");
  ggauss_scroll_set_selected();
}

void US_Hydrodyn_Dad::ggauss_scroll_smoothed() {
...
void US_Hydrodyn_Dad::ggauss_scroll_oldstyle() {
  us_qdebug("ggauss_scroll_oldstyle()");
  ggauss_scroll_highlight(ggauss_scroll_highlight_last_pos);
}

void US_Hydrodyn_Dad::ggauss_scroll_set_selected() {
...
void US_Hydrodyn_Dad::ggauss_scroll_highlight(int pos) {
...
void US_Hydrodyn_Dad::ggaussian_enables() {
...
void US_Hydrodyn_Dad::timeshift() {
...
void US_Hydrodyn_Dad::timescale() { timescale(all_selected_files()); }

void US_Hydrodyn_Dad::timescale(const QStringList &files) {
  bool ok;
  double res =
