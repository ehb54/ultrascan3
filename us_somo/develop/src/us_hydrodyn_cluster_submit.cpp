...
void US_Hydrodyn_Cluster_Submit::setupGUI() {
  ...
  lv_files->setHeaderLabels(QStringList() << us_tr("Name") << us_tr("Created")
  ...
  lb_systems->setMinimumHeight(
      ...
      (((US_Hydrodyn_Cluster *)cluster_window)->cluster_systems.size() > 8
           ? 8
           : ((US_Hydrodyn_Cluster *)cluster_window)->cluster_systems.size()));
  ...
  for (int i = 0; i < lb_systems->count(); i++) {
    if (lb_systems->item(i)->isSelected()) {
      ...
      if (((US_Hydrodyn_Cluster *)cluster_window)
        ...
        if (selected_system.count("stage") && selected_system.count("type") &&
          ...
          stage_url.replace(QRegExp(":.*$"), "");
          // stage_path      .replace( QRegExp( "^.*:" ), "" );
          // stage_url_path  += QString( "%1%2%3" ).arg( QDir::separator()
          // ).arg( cluster_id ).arg( QDir::separator() );
        } else {
          QMessageBox::warning(
              this, us_tr("US-SOMO: Cluster Submit"),
              QString(us_tr("The system %1 does not seem to have sufficient "
...
void US_Hydrodyn_Cluster_Submit::cancel() { close(); }
...
bool US_Hydrodyn_Cluster_Submit::submit_url_body(QString file, QString &url,
                                                 QString &body) {
  errormsg = "";
  cout << "submit_url_body\n";
  if (!QFile::exists(file)) {
    errormsg =
        QString(us_tr("submit_url_body: File %1 does not exist")).arg(file);
    return false;
  }
  ...
  if (file.contains(QRegExp("\\.(tar|TAR)$"))) {
    ...
    if (result != TAR_OK) {
      errormsg =
          QString(us_tr("submit_url_body: Listing tar archive %1 failed %2"))
              .arg(file)
  ...
  if (!selected_system["corespernode"].toUInt()) {
    errormsg = QString(us_tr("submit_url_body: The selected system %1 does not "
  ...
  if (!selected_system["maxcores"].toUInt()) {
    errormsg = QString(us_tr("submit_url_body: The selected system %1 does not "
                             "have a positive max cores defined"))
                   .arg(selected_system_name);
    return false;
  }
  ...
  if (!selected_system["runtime"].toUInt()) {
    errormsg = QString(us_tr("submit_url_body: The selected system %1 does not "
                             "have a positive run time defined"))
                   .arg(selected_system_name);
    return false;
  }
...
void US_Hydrodyn_Cluster_Submit::process_next() {
  ...
  if (!update_files()) {
    QMessageBox::information(this, us_tr("US-SOMO: Cluster Jobs"),
                             us_tr("No further unsubmitted jobs found"), 0);
    close();
  }
...
bool US_Hydrodyn_Cluster_Submit::move_file(QString file) {
  ...
  if (!QDir::setCurrent(pkg_dir)) {
    errormsg =
        QString(us_tr("move: can not change to directory %1")).arg(pkg_dir);
    return false;
  }
...
void US_Hydrodyn_Cluster_Submit::system_proc_finished(int,
                                                      QProcess::ExitStatus) {
...
bool US_Hydrodyn_Cluster_Submit::prepare_stage(QString file) {
  ...
  if (!QDir::setCurrent(pkg_dir)) {
    errormsg =
        QString(us_tr("stage: can not change to directory %1")).arg(pkg_dir);
    return false;
  }
...
bool US_Hydrodyn_Cluster_Submit::send_http_post(QString file, QString url,
                                                QString body) {
  ...
  errormsg = "";
  if (!QDir::setCurrent(pkg_dir)) {
    errormsg = QString(us_tr("send_http_post: can not change to directory %1"))
                   .arg(pkg_dir);
    return false;
  }
...

  {
    ...
    if (!f->open(QIODevice::ReadOnly)) {
      ...
      errormsg =
          QString(us_tr("send_http_post: can not open file %1 for reading"))
              .arg(file);
      return false;
    }
...
void US_Hydrodyn_Cluster_Submit::http_error(
...
void US_Hydrodyn_Cluster_Submit::http_uploadProgress(qint64 done,
                                                     qint64 total) {
  cout << "http: uploadProgress " << done << " " << total << "\n";
  progress->setValue(100.0 * (double)done / (double)total);
}

void US_Hydrodyn_Cluster_Submit::http_downloadProgress(qint64 done,
                                                       qint64 total) {
