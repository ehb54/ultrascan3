...
US_Hydrodyn_Cluster_Submit::US_Hydrodyn_Cluster_Submit (void *us_hydrodyn,
                                                        QWidget *p,
                                                        const char *)
    : QDialog (p)
{
  ...
  if (cluster_id.isEmpty () || cluster_pw.isEmpty ())
    {
      QMessageBox::information (this, us_tr ("US-SOMO: Cluster Submit"),
  ...
  if (!update_files (false))
    {
      QMessageBox::information (this, us_tr ("US-SOMO: Cluster Submit"),
                                us_tr ("No unsubmitted jobs found"), 0);
    }
  ...
  if (!dir2.exists ())
    {
      editor_msg ("black",
                  QString (us_tr ("Created directory %1")).arg (tmp_dir));
      dir2.mkdir (tmp_dir);
    }
...
unsigned int
US_Hydrodyn_Cluster_Submit::update_files (bool set_lv_files)
{
  files.clear ();
...
void
US_Hydrodyn_Cluster_Submit::setupGUI ()
{
  int minHeight1 = 30;
  ...
  lv_files->setHeaderLabels (QStringList ()
                             << us_tr ("Name") << us_tr ("Created")
                             << us_tr ("Size") << us_tr ("Status"));
...
void
US_Hydrodyn_Cluster_Submit::systems ()
{
      ...
      if (lb_systems->item (i)->isSelected ())
        {
          ...
          if (((US_Hydrodyn_Cluster *)cluster_window)
                  ->cluster_systems.count (selected_system_name))
            {
              selected_system = ((US_Hydrodyn_Cluster *)cluster_window)
                                    ->cluster_systems[selected_system_name];
              if (selected_system.count ("stage")
                  ...
                  && selected_system.count ("runtime"))
                {
                  ...
                  stage_url.replace (QRegExp (":.*$"), "");
                  // stage_path      .replace( QRegExp( "^.*:" ), "" );
                  // stage_url_path  += QString( "%1%2%3" ).arg(
                  // QDir::separator() ).arg( cluster_id ).arg(
                  // QDir::separator() );
                }
              else
                {
                  QMessageBox::warning (
                      this, us_tr ("US-SOMO: Cluster Submit"),
                      QString (
                          us_tr (
                              "The system %1 does not seem to have sufficient "
                              "configuration information defined"))
                          .arg (lb_systems->item (i)->text ()));
          ...
          else
            {
              QMessageBox::warning (
                  this, us_tr ("US-SOMO: Cluster Submit"),
                  QString (us_tr ("The system %1 does not seem to have any "
                                  "information"))
                      .arg (lb_systems->item (i)->text ()));
              lb_systems->clearSelection ();
            }
...
void
US_Hydrodyn_Cluster_Submit::cancel ()
{
  close ();
}

void
US_Hydrodyn_Cluster_Submit::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Cluster_Submit::closeEvent (QCloseEvent *e)
{
  if (comm_active)
    {
      http_reply->abort ();
...
void
US_Hydrodyn_Cluster_Submit::clear_display ()
{
  editor->clear ();
  editor->append ("\n\n");
}
...
void
US_Hydrodyn_Cluster_Submit::update_font ()
{
  bool ok;
  QFont newFont;
...
void
US_Hydrodyn_Cluster_Submit::save ()
{
  QString fn;
...
void
US_Hydrodyn_Cluster_Submit::editor_msg (QString color, QString msg)
{
...
void
US_Hydrodyn_Cluster_Submit::update_enables ()
{
...
void
US_Hydrodyn_Cluster_Submit::select_all ()
{
...
bool
US_Hydrodyn_Cluster_Submit::submit_url_body (QString file, QString &url,
                                             QString &body)
{
  errormsg = "";
  ...
  if (!QFile::exists (file))
    {
      errormsg = QString (us_tr ("submit_url_body: File %1 does not exist"))
                     .arg (file);
      return false;
    }
      ...
      if (result != TAR_OK)
        {
          errormsg
              = QString (
                    us_tr (
                        "submit_url_body: Listing tar archive %1 failed %2"))
                    .arg (file)
  ...
  if (!selected_system["corespernode"].toUInt ())
    {
      errormsg
          = QString (us_tr ("submit_url_body: The selected system %1 does not "
                            "have a positive cores per node defined"))
  ...
  if (!selected_system["maxcores"].toUInt ())
    {
      errormsg = QString (us_tr ("submit_url_body: The selected system %1 "
                                 "does not have a positive max cores defined"))
                     .arg (selected_system_name);
      return false;
    }
  ...
  if (!selected_system["runtime"].toUInt ())
    {
      errormsg = QString (us_tr ("submit_url_body: The selected system %1 "
                                 "does not have a positive run time defined"))
                     .arg (selected_system_name);
      return false;
    }

  unsigned int common_count
      = (unsigned int)tar_list.filter (QRegExp ("^common_")).size ();
  ...
  unsigned int processor_count
      = host_count * selected_system["corespernode"].toUInt ();
  ...
  //    .arg( stage_url )
  //    .arg( processor_count )
  //    .arg( selected_system[ "queue" ].isEmpty() ? "" : QString(
  //    "<queuename>%1</queuename>" ).arg( selected_system[ "queue" ] ) ) .arg(
  //    selected_system[ "runtime" ].toUInt() ) .arg( cluster_id ) .arg(
  //    selected_system[ "executable" ].isEmpty() ? "" : QString(
  //    "<executable>%1</executable>" ).arg( selected_system[ "executable" ] )
  //    ) .arg( job_type ) .arg( QString( "%1/%2" )
  //          .arg( target_dir )
  //          .arg( file ) );

  // cout << url << endl;

  url = QString ("/jobsubmit?") + QString ("menu=ussomo")
        + QString ("&module=somoapi_mpi") + QString ("&_clobber=1")
        + QString ("&user=%1").arg (cluster_id)
        + QString ("&pw=%1").arg (cluster_pw)
        + QString ("&id=%1").arg (
            QString ("%1-%2").arg (cluster_id).arg (file))
        ...
        + QString ("&queue=%1")
              .arg (selected_system["queue"].isEmpty ()
                        ? ""
                        : QString ("%1").arg (selected_system["queue"]))
        + QString ("&wall=%1").arg (selected_system["runtime"].toUInt ())
        + QString ("&param=json") + QString ("&dir=%1").arg (target_dir)
  ...
  // build json input
  {
    ...
    body_map["numproc"]
        = QString ("%1").arg (CLUSTER_OVERRIDE_NPROC); // processor_count );
    body_map["param"] = "json";
    body_map[job_type] = "1";
    body_map["file"] = file;
...
void
US_Hydrodyn_Cluster_Submit::stop ()
{
  if (submit_active)
    {
      submit_active = false;
...
void
US_Hydrodyn_Cluster_Submit::submit ()
{
  submit_active = true;
  update_enables ();
  cout << "setup jobs\n";
...
void
US_Hydrodyn_Cluster_Submit::process_next ()
{
  if (!submit_active)
    {
...
void
US_Hydrodyn_Cluster_Submit::process_prepare_stage ()
{
  editor_msg ("black",
              QString ("preparing stage %1").arg (next_to_process->text (0)));
...
void
US_Hydrodyn_Cluster_Submit::process_stage ()
{
...
void
US_Hydrodyn_Cluster_Submit::process_submit ()
{
  editor_msg ("black",
              QString ("submitting %1").arg (next_to_process->text (0)));
...
void
US_Hydrodyn_Cluster_Submit::process_move ()
{
  next_to_process->setText (3, us_tr ("Moving to submitted"));
  editor_msg (
      "black",
      QString ("move %1 to submitted/").arg (next_to_process->text (0)));
  cout << "process move\n";
  if (move_file (next_to_process->text (0)))
    ...
    {
      editor_msg ("red", errormsg);
      next_to_process->setText (3,
                                us_tr ("Error: Moving to submitted failed"));
...
bool
US_Hydrodyn_Cluster_Submit::move_file (QString file)
{
  US_File_Util ufu;
  ...
  if (!QDir::setCurrent (pkg_dir))
    {
      errormsg = QString (us_tr ("move: can not change to directory %1"))
                     .arg (pkg_dir);
      return false;
    }
...
void
US_Hydrodyn_Cluster_Submit::process_list ()
{
  for (map<QTreeWidgetItem *, QString>::iterator it = jobs.begin ();
       it != jobs.end (); it++)
    {
      editor_msg (
          "blue",
          QString ("%1 %2").arg (it->first->text (0)).arg (it->second));
    }
}

bool
US_Hydrodyn_Cluster_Submit::job_submit (QString file)
{
  QString url;
  QString body;
...
bool
US_Hydrodyn_Cluster_Submit::system_cmd (QStringList cmd)
{
  errormsg = "";
...
void
US_Hydrodyn_Cluster_Submit::system_proc_readFromStdout ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Cluster_Submit::system_proc_readFromStderr ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Cluster_Submit::system_proc_finished (int, QProcess::ExitStatus)
{
...
void
US_Hydrodyn_Cluster_Submit::system_proc_started ()
{
...
bool
US_Hydrodyn_Cluster_Submit::prepare_stage (QString file)
{
  errormsg = "";
  if (!QDir::setCurrent (pkg_dir))
    {
      errormsg = QString (us_tr ("stage: can not change to directory %1"))
                     .arg (pkg_dir);
      return false;
    }
...
bool
US_Hydrodyn_Cluster_Submit::stage (QString file)
{
  errormsg = "";
  if (!QDir::setCurrent (pkg_dir))
    {
  ...
  //    {
  //       errormsg = QString( us_tr( "stage: can not open file %1" ) ).arg(
  //       file ); delete ftp_file; ftp_file = ( QFile * ) 0; return false;
  //    }
...
//   cmd << QString( "%1%2/" )
//      .arg( stage_url_path )
//      .arg( QString("%1").arg( file ).replace( QRegExp(
//      "\\.(tgz|tar|TGZ|TAR)$" ), "" ) );
...
void
US_Hydrodyn_Cluster_Submit::remove ()
{
  bool any_selected = US_Static::lv_any_selected (lv_files);
  ...
  if (QMessageBox::question (this, us_tr ("US-SOMO: Cluster Jobs: Remove"),
  ...
  if (!QDir::setCurrent (pkg_dir))
    {
      editor_msg (
          "red",
          QString (us_tr ("can not change to directory %1")).arg (pkg_dir));
      return;
    }
          ...
          if (lvi->isSelected ())
            {
              if (!QFile::remove (lvi->text (0)))
                {
                  editor_msg ("red", QString (us_tr ("can not remove file %1"))
                                         .arg (lvi->text (0)));
                }
              else
                {
                  editor_msg ("black", QString (us_tr ("Removed file: %1"))
                                           .arg (lvi->text (0)));
                }
            }
        }
      ...
      if (!QFile::remove (lvi->text (0)))
        {
          editor_msg (
              "red",
              QString (us_tr ("can not remove file %1")).arg (lvi->text (0)));
        }
      else
        {
          editor_msg (
              "black",
              QString (us_tr ("Removed file: %1")).arg (lvi->text (0)));
        }
      ++it;
    }
#endif
...
bool
US_Hydrodyn_Cluster_Submit::send_http_post (QString file, QString url,
                                            QString body)
{
  ...
  if (!QDir::setCurrent (pkg_dir))
    {
      errormsg
          = QString (us_tr ("send_http_post: can not change to directory %1"))
                .arg (pkg_dir);
      return false;
    }
  ...
  if (!QFile::exists (file))
    {
      errormsg = QString (us_tr ("send_http_post: can not find file %1"))
                     .arg (file);
      return false;
    }
  ...
  //    errormsg = QString( us_tr( "send_http_post: can not open file %1 for
  //    reading" ) ).arg( file ); return false;
  // }
...

  {
    ...
    dataPart.setHeader (
        ...
        QVariant (QString ("form-data; name=\"%1\"").arg (file)));
    // dataPart.setHeader( QNetworkRequest::ContentDispositionHeader, QVariant(
    // QString( "form-data; name=\"datafile\"" ) ) );
    ...
    if (!f->open (QIODevice::ReadOnly))
      {
        ...
        errormsg
            = QString (
                  us_tr ("send_http_post: can not open file %1 for reading"))
                  .arg (file);
        return false;
      }

    dataPart.setBodyDevice (f);
    f->setParent (http_multiPart); // we cannot delete the file now, so delete
                                   // it with the multiPart
...
void
US_Hydrodyn_Cluster_Submit::http_error (QNetworkReply::NetworkError /* code */)
{
  qDebug () << "http: error";
...
void
US_Hydrodyn_Cluster_Submit::http_finished ()
{
  cout << "http: finished\n";
...
void
US_Hydrodyn_Cluster_Submit::http_uploadProgress (qint64 done, qint64 total)
{
...
void
US_Hydrodyn_Cluster_Submit::http_downloadProgress (qint64 done, qint64 total)
{
  cout << "http: downloadProgress " << done << " " << total << "\n";
}

void
US_Hydrodyn_Cluster_Submit::http_done (bool error)
{
