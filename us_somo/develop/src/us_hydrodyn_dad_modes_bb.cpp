...
US_Hydrodyn_Dad::blanks_start ()
{
          ...
          if (!files_are_time && files_compatible)
            {
              switch (QMessageBox::question (
                  this, windowTitle () + us_tr (" : Blanks analysis"),
                  us_tr ("There are available blanks data and selected files "
                ...
                {
                case -1: // escape
                  blanks_in_baseline_mode ? baseline_enables ()
                                          : update_enables ();
                  return;
                  break;
      ...
      for (int i = 0; i < (int)default_blanks.size (); ++i)
        {
          if (!f_qs.count (default_blanks[i]))
            {
              // us_qdebug( QString( "loading %1" ).arg( default_blanks_files[
              // i ] ) );
              if (!load_file (default_blanks_files[i]))
                {
                  editor_msg (
                      "red",
                      QString (us_tr ("Error: could not load blank file %1"))
                          .arg (default_blanks_files[i]));
      ...
      if (blanks_in_baseline_mode)
        {
          ...
          // QMessageBox::warning( this, windowTitle() + us_tr( " : Blanks
          // analysis" ),
          //                       us_tr( "You would now be asked to load
          //                       files, but this is not yet implemented\n"
          //                           "You *can* exit and load blanks, run
          //                           blanks analysis & 'keep' and then try
          //                           again\n" )
  ...
  if (blanks_selected.size () < IB_MIN_FRAMES)
    {
      QMessageBox::warning (
          this, windowTitle () + us_tr (" : Blanks analysis"),
          QString (us_tr ("You must have minimum of %1 frames for blanks"))
              .arg (IB_MIN_FRAMES),
          QMessageBox::Ok | QMessageBox::Default, QMessageBox::NoButton);
      // return current_mode != MODE_NORMAL ? wheel_cancel() :
      // update_enables();
      ...
      // return blanks_in_baseline_mode ? baseline_enables() :
      // update_enables();
    }

  {
    ...
    if (any_differences)
      {
        editor_msg ("red", us_tr ("Blanks analysis: curves must be on the "
                                  "same grid, try 'Crop Common' first."));
        update_enables ();
        return;
      }
  }
  ...
  if (!f_qs.count (wheel_file))
    {
      editor_msg ("red",
                  QString (us_tr ("Internal error (bm): %1 not found in data"))
                      .arg (wheel_file));
      return;
    }
  ...
  if (reset_range || le_baseline_end_s->text ().isEmpty ()
    ...
    {
      disconnect (le_baseline_end_s, SIGNAL (textChanged (const QString &)), 0,
                  0);
      le_baseline_end_s->setText (
          QString ("%1").arg (f_qs[wheel_file].front ()));
      connect (le_baseline_end_s, SIGNAL (textChanged (const QString &)),
               SLOT (baseline_end_s_text (const QString &)));
    }

  if (reset_range || le_baseline_end_e->text ().isEmpty ()
    ...
    {
      disconnect (le_baseline_end_e, SIGNAL (textChanged (const QString &)), 0,
                  0);
...
void
US_Hydrodyn_Dad::blanks_enables ()
{
  ...
  wheel_enables (
      ...
      le_last_focus == le_baseline_end_s || le_last_focus == le_baseline_end_e
#else
      le_baseline_end_s->hasFocus () || le_baseline_end_e->hasFocus ()
#endif
  );
...
// --- baseline ---
void
US_Hydrodyn_Dad::baseline_start (bool from_blanks_mode_save)
{
  if (axis_y_log)
    {
      axis_y ();
  ...
  {
    bool use_i_power
        = ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_bl_i_power")
          && ((US_Hydrodyn *)us_hydrodyn)
                     ->gparams["dad_bl_i_power"]
                     .toDouble ()
                 != 1e0;
    ...
    if (use_i_power)
      {
        i_power = ((US_Hydrodyn *)us_hydrodyn)
                      ->gparams["dad_bl_i_power"]
                      .toDouble ();
        switch (QMessageBox::warning (
            this, windowTitle () + us_tr (" : Baseline"),
            QString (us_tr ("Please note:\n\n"
                            "You are using a non-unity intensity exponent of "
                            "%1 for integral baseline correction.\n"
                            "The model of intensity proportional deposits is "
                            "based on a unity intensity exponent.\n"
                            "It is possible that a specific capillary fouling "
                            "mechanism may be correctly approximated with a "
                            "non-unity exponent.\n"
                            "We urge you to use a non-unity intensity "
                            "exponent with care.\n"
                            "What would you like to do?\n"))
                .arg (i_power),
            us_tr ("&Continue"), us_tr ("&Quit")))
          {
          case 0: // continue
            break;
          case 1: // quit
  ...
  {
    int reps = ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_reps"].toInt ();
    if (reps < 5)
      {
        switch (QMessageBox::warning (
            this, windowTitle () + us_tr (" : Baseline"),
            QString (us_tr ("Please note:\n\n"
                            "You have selected the integral baseline method "
                            "with maximum iterations of %1\n"
                            "It is recommended that a minimum of 5 iterations "
                            "be used to approach convergence\n"
                            "What would you like to do?\n"))
                .arg (reps),
            us_tr ("&Continue"), us_tr ("&Quit")))
          {
  ...
  // baseline_integral = ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[
  // "dad_bl_integral" ] == "true";
  ...
  if (f_qs[wheel_file].size () < IB_MIN_RANGE * 2)
    {
      QMessageBox::warning (
          this, windowTitle () + us_tr (" : Baseline"),
          QString (us_tr ("You must have a minimum of %1 frames in your data "
  ...
  if (baseline_integral)
    {
      ...
      if (q_size < IB_MIN_FRAMES)
        {
          QMessageBox::warning (
              this, windowTitle () + us_tr (" : Baseline"),
              us_tr (QString ("Insufficient frames (%1) for integral baseline "
                              "analysis.\n"
      ...
      if (!from_blanks_mode_save || !blanks_brookesmap_sliding_results.size ())
        {
          switch (QMessageBox::question (
              this,
              windowTitle ()
                  + us_tr (" : Utilize blanks in baseline analysis"),
              us_tr ("Do you wish to utilize reference blanks for baseline "
                     "analysis?\n"
                     "Without reference blanks, only the adjusted "
                     "Holm-Bonferroni method can be used."),
              us_tr ("&Yes, utilize blanks"),
              us_tr ("&No, proceed without blanks"), QString ()))
            {
            case -1: // escape
              return current_mode != MODE_NORMAL ? wheel_cancel ()
                                                 : update_enables ();
            ...
            case 0:
              {
                switch (QMessageBox::question (
                    this,
                    windowTitle ()
                        + us_tr (" : Baseline mode blanks selection"),
                    QString (us_tr (
                        "You must have blanks processed to proceed\n"
                        "\n"
                        "The blanks should be non-averaged buffer frames "
                        "(whose average value was then subtracted from the "
                        "elution frames)\n"
                        "If you do not have the buffers corresponding to "
                        "their present elution, then the preloaded blanks "
                        "should have been chosen in a similar way:\n"
                        "The blanks do not have to be *the* blanks used for "
                        "the present elution, even if it is strongly "
                        "recommended,\n"
                        "but at least blanks collected on the same beamline "
                        "with a similar set-up (distance, pixel size, "
                        "energy).\n"
                        "\n"
                        "You have the following options")),
                    us_tr ("Load blanks"), us_tr ("Load previous analysis"),
                  ...
                  case 0: // load blanks
                    {
                      ...
                      new_blanks_files = QFileDialog::getOpenFileNames (
                          this,
                          windowTitle () + us_tr (" : Select blanks files"),
                          use_dir,
                      ...
                      for (int i = 0; i < (int)new_blanks_files.size (); ++i)
                        {
                          QString basename = QFileInfo (new_blanks_files[i])
                                                 .completeBaseName ();
                          new_blanks << basename;
                  ...
                  case 1: // load previous analysis
                    {
                      ...
                      QString filename = QFileDialog::getOpenFileName (
                          this, windowTitle (), use_dir, "*.csv *.CSV");
                      ...
                      if (!US_Hydrodyn_Saxs_Cormap::load_csv (
                              filename, load_parameters, load_pvaluepairs,
                              load_adjpvaluepairs, load_selected_files, this))
                        {
                          return current_mode != MODE_NORMAL
                                     ? wheel_cancel ()
                                     : update_enables ();
                      ...
                      if (!load_parameters.count ("isblanks"))
                        {
                          QMessageBox::warning (
                              this, windowTitle () + us_tr (" : Baseline"),
                              QString (
                                  us_tr ("The loaded analysis does not appear "
                                         "to be a blanks analysis")),
                              QMessageBox::Ok | QMessageBox::Default,
                              QMessageBox::NoButton);
                          return current_mode != MODE_NORMAL
                                     ? wheel_cancel ()
                      ...
                      if (blanks_cormap_parameters.size ()
                          && blanks_cormap_pvaluepairs.size ())
                        {
                          ...
                          if (ca.sliding (blanks_cormap_pvaluepairs,
                                          blanks_cormap_parameters,
                                          blanks_brookesmap_sliding_results,
                                          this, progress))
                            {
                              // us_qdebug( "sliding_results" );
                              // for ( map < QString, double >::iterator it =
                              // blanks_brookesmap_sliding_results.begin();
                              //       it !=
                              //       blanks_brookesmap_sliding_results.end();
                              //       ++it ) {
                              //    cout << "\t" << it->first << " => " <<
                              //    it->second << endl;
                              // }
      ...
      baseline_max_window_size = baseline_hb_only_mode
          ...
          if (baseline_max_window_size < IB_MIN_FRAMES)
            {
              QMessageBox::warning (
                  this, windowTitle () + us_tr (" : Baseline"),
                  QString (
                      us_tr ("You must have minimum of %1 frames for blanks"))
                      .arg (IB_MIN_FRAMES),
                  QMessageBox::Ok | QMessageBox::Default,
                  QMessageBox::NoButton);
              return current_mode != MODE_NORMAL ? wheel_cancel ()
          ...
          else
            {
              QMessageBox::information (
                  this, windowTitle () + us_tr (" : Baseline"),
                  us_tr ("The first step of the integral baseline procedure "
                         "is to determine a constant final baseline."),
                  QMessageBox::Ok | QMessageBox::Default,
                  QMessageBox::NoButton);
            }
        }

      // int use_default_range = IB_DEFAULT_RANGE > baseline_max_window_size ?
      // baseline_max_window_size : IB_DEFAULT_RANGE;

      // us_qdebug( QString( "use default range %1 IB_DEFAULT_RANGE %2
      // baseline_max_window_size %3 q_size %4" )
  ...
  //                       ( f_qs[ wheel_file ].back() - f_qs[ wheel_file ][ 0
  //                       ] ) / UHSH_WHEEL_RES );
  ...
  qwtw_wheel->setRange (0, (double)f_qs[wheel_file].size () - 1);
  qwtw_wheel->setSingleStep (1);

  if (baseline_integral)
    {
      if (force_endpoints_default || le_baseline_end_s->text ().isEmpty ()
          || le_baseline_end_s->text () == "0"
          || le_baseline_end_s->text ().toDouble () > f_qs[wheel_file].back ())
        {
          disconnect (le_baseline_end_s,
                      SIGNAL (textChanged (const QString &)), 0, 0);
          le_baseline_end_s->setText (
              QString ("%1").arg (default_baseline_end_s));
          connect (le_baseline_end_s, SIGNAL (textChanged (const QString &)),
                   SLOT (baseline_end_s_text (const QString &)));
        }

      if (force_endpoints_default || le_baseline_end->text ().isEmpty ()
          || le_baseline_end->text () == "0"
          || le_baseline_end->text ().toDouble () > f_qs[wheel_file].back ())
        {
          disconnect (le_baseline_end, SIGNAL (textChanged (const QString &)),
                      0, 0);
      ...
      if (force_endpoints_default || le_baseline_end_e->text ().isEmpty ()
          || le_baseline_end_e->text () == "0"
          || le_baseline_end_e->text ().toDouble () > f_qs[wheel_file].back ())
        {
          disconnect (le_baseline_end_e,
                      SIGNAL (textChanged (const QString &)), 0, 0);
          le_baseline_end_e->setText (
              QString ("%1").arg (default_baseline_end_e));
          connect (le_baseline_end_e, SIGNAL (textChanged (const QString &)),
                   SLOT (baseline_end_e_text (const QString &)));
        }
      disconnect (le_baseline_width, SIGNAL (textChanged (const QString &)), 0,
                  0);
      le_baseline_width->setText (QString ("%1").arg (
          input_double_to_pos (le_baseline_end_e->text ().toDouble ())
  ...
  else
    {
      ...
      if (le_baseline_start_s->text ().isEmpty ()
        ...
        {
          disconnect (le_baseline_start_s,
                      SIGNAL (textChanged (const QString &)), 0, 0);
          le_baseline_start_s->setText (
              QString ("%1").arg (f_qs[wheel_file][0]));
      ...
      if (le_baseline_start->text ().isEmpty ()
        ...
        {
          disconnect (le_baseline_start,
                      SIGNAL (textChanged (const QString &)), 0, 0);
          // le_baseline_start->setText( QString( "%1" ).arg( f_qs[ wheel_file
          // ][ 0 ] + q_len_delta ) );
          le_baseline_start->setText (QString ("%1").arg (
              f_qs[wheel_file]
                  [0 + pos_len_delta < q_size - 1 ? 0 + pos_len_delta : 0]));
      ...
      if (le_baseline_start_e->text ().isEmpty ()
        ...
        {
          disconnect (le_baseline_start_e,
                      SIGNAL (textChanged (const QString &)), 0, 0);
          // le_baseline_start_e->setText( QString( "%1" ).arg( f_qs[
          // wheel_file ][ 0 ] + 2e0 * q_len_delta) );
          le_baseline_start_e->setText (QString ("%1").arg (
              f_qs[wheel_file]
                  [0 + 2 * pos_len_delta < q_size - 1 ? 0 + 2 * pos_len_delta
                                                      : 0]));
      ...
      if (le_baseline_end_s->text ().isEmpty ()
          ...
          || le_baseline_end_s->text ().toDouble () > f_qs[wheel_file].back ())
        {
          disconnect (le_baseline_end_s,
                      SIGNAL (textChanged (const QString &)), 0, 0);
          // le_baseline_end_s->setText( QString( "%1" ).arg( f_qs[ wheel_file
          // ].back() - 2e0 * q_len_delta ) );
          le_baseline_end_s->setText (QString ("%1").arg (
              f_qs[wheel_file][q_size - 1 - 2 * pos_len_delta > 0
                                   ? q_size - 1 - 2 * pos_len_delta
                                   : 0]));
          connect (le_baseline_end_s, SIGNAL (textChanged (const QString &)),
                   SLOT (baseline_end_s_text (const QString &)));
        }
      ...
      if (le_baseline_end->text ().isEmpty ()
          ...
          || le_baseline_end->text ().toDouble () > f_qs[wheel_file].back ())
        {
          disconnect (le_baseline_end, SIGNAL (textChanged (const QString &)),
                      0, 0);
          // le_baseline_end->setText( QString( "%1" ).arg( f_qs[ wheel_file
          // ].back() - q_len_delta ) );
          le_baseline_end->setText (QString ("%1").arg (
              f_qs[wheel_file]
                  [q_size - 1 - pos_len_delta > 0 ? q_size - 1 - pos_len_delta
                                                  : 0]));
          connect (le_baseline_end, SIGNAL (textChanged (const QString &)),
                   SLOT (baseline_end_text (const QString &)));
        }
  ...
  else
    {
      ...
      QMessageBox::warning (
          ...
          QString (us_tr (
              ...
              "This method compensates for a systematic drift and can "
              "distort/hide the effects of capillary fouling.\n"
              "The linear baseline correction does not have an explicit "
              "physical basis and should be applied with care\n"
              "and only in the case of \"small-amplitude\" corrections as "
...
void
US_Hydrodyn_Dad::baseline_enables ()
{
  ...
  if (baseline_integral)
    {
      ...
      pb_baseline_apply->setEnabled (
          baseline_best_ok
          && baseline_best_last_window_size
                 == le_baseline_width->text ().toInt ());
    }
  else
    {
      pb_wheel_save->setEnabled (
  ...
  wheel_enables (
      ...
      || le_last_focus == le_baseline_end_s || le_last_focus == le_baseline_end
      || le_last_focus == le_baseline_end_e
      || le_last_focus == le_baseline_width
#else
      le_baseline_start_s->hasFocus () || le_baseline_start->hasFocus ()
      || le_baseline_start_e->hasFocus () || le_baseline_end_s->hasFocus ()
      || le_baseline_end->hasFocus () || le_baseline_end_e->hasFocus ()
...
void
US_Hydrodyn_Dad::set_baseline_start_zero ()
{
  if (!cb_baseline_start_zero->isChecked ())
    {
      // QMessageBox::warning(
      //                      this,
      //                      this->windowTitle() + us_tr(": Baseline setup:
      //                      nonzero starting offset" ), us_tr(
      //                         "Having a non zero starting offset likely
      //                         means problems with buffer subtraction which
      //                         should be corrected before *any* analysis is
      //                         done.\n"
      //                         ),
      //                      QMessageBox::Ok | QMessageBox::Default,
      //                      QMessageBox::NoButton
...
void
US_Hydrodyn_Dad::baseline_init_markers ()
{
  ...
  if (current_mode == MODE_BLANKS)
  ...
  else
    {
      if (baseline_integral)
        {
          gauss_add_marker (
              le_baseline_end_s->text ().toDouble (), Qt::red,
              us_tr ("\n  & Analysis;\nEnd Integral\n   Bas. calc."),
              Qt::AlignLeft | Qt::AlignTop);
          gauss_add_marker (le_baseline_end_s->text ().toDouble (),
                            Qt::magenta, us_tr ("Start Bas. Window"),
                            Qt::AlignLeft | Qt::AlignTop);
          gauss_add_marker (le_baseline_end->text ().toDouble (), Qt::red,
                            us_tr ("\n\n\n\n\n\nEnd Bas.\nAnalysis"),
                            Qt::AlignLeft | Qt::AlignTop);
          gauss_add_marker (le_baseline_end_e->text ().toDouble (),
                            Qt::magenta, us_tr ("\n\n\n\nEnd Bas.\n  Window"),
                            Qt::AlignLeft | Qt::AlignTop);
        }
      else
        {
          if (!cb_baseline_start_zero->isChecked ())
            {
              gauss_add_marker (le_baseline_start_s->text ().toDouble (),
                                Qt::magenta, us_tr ("\nLFS  \nStart"));
              gauss_add_marker (le_baseline_start->text ().toDouble (),
                                Qt::red, us_tr ("Start"));
              gauss_add_marker (le_baseline_start_e->text ().toDouble (),
                                Qt::magenta, us_tr ("\n\n\nLFS\nEnd"));
              gauss_add_marker (le_baseline_end_s->text ().toDouble (),
                                Qt::magenta, us_tr ("\n  LFE\nStart"),
                                Qt::AlignLeft | Qt::AlignTop);
...
void
US_Hydrodyn_Dad::replot_baseline_integral ()
{
  ...
  {
    unsigned int i = 0;
    if (f_qs[wheel_file][i] >= end_s && f_qs[wheel_file][i] <= end_e)
      {
  ...
  if (end_pts)
    {
      ...
      plot_dist->setCurvePen (curve,
                              QPen (end_color, use_line_width, Qt::DashLine));
  ...
  // us_qdebug( QString( "replot_baseline_integral() end current
  // plotted_baseline curves size %1" ).arg( plotted_baseline.size() ) );
}

void
US_Hydrodyn_Dad::replot_baseline (QString /* qs */)
{
  // us_qdebug( QString( "replot_baseline( %1 ) current plotted_baseline curves
  // size %2 %3" )
  //         .arg( qs )
  //         .arg( plotted_baseline.size() ).arg( baseline_integral ? "integral
  //         mode" : "!integral mode" ) );
...

  {
    ...
    if (cb_baseline_start_zero->isChecked ())
      ...
      {
        if (f_qs[wheel_file][i] >= start_s && f_qs[wheel_file][i] <= start_e)
          {
            start_q.push_back (f_qs[wheel_file][i]);
  ...
  for (unsigned int i = 1; i < f_qs[wheel_file].size (); i++)
    {
      if (!cb_baseline_start_zero->isChecked ()
          && f_qs[wheel_file][i] >= start_s && f_qs[wheel_file][i] <= start_e)
        {
          start_q.push_back (f_qs[wheel_file][i]);
          start_I.push_back (f_Is[wheel_file][i]);
        }
      ...
      if (f_qs[wheel_file][i - 1] <= start && f_qs[wheel_file][i] >= start)
        {
          before_start = i - 1;
          after_start = i;
        }
      if (f_qs[wheel_file][i - 1] <= end && f_qs[wheel_file][i] >= end)
        {
          before_end = i - 1;
          after_end = i;
        }
  ...
  if (set_start && set_end)
    {
      // linear fit on each
      usu->linear_fit (start_q, start_I, start_intercept, start_slope, siga,
                       sigb, chi2);
      usu->linear_fit (end_q, end_I, end_intercept, end_slope, siga, sigb,
                       chi2);
  ...
  else
    {
      if (set_start)
        {
          usu->linear_fit (start_q, start_I, start_intercept, start_slope,
                           siga, sigb, chi2);
          ...
          end_y = (end_t)*f_Is[wheel_file][before_end]
      ...
      else
        {
          if (set_end)
            {
              usu->linear_fit (end_q, end_I, end_intercept, end_slope, siga,
                               sigb, chi2);

              end_y = end_intercept + end_slope * end;

              double start_t;
              if (f_qs[wheel_file][after_start]
                  != f_qs[wheel_file][before_start])
                {
                  start_t = (f_qs[wheel_file][after_start] - start)
              ...
              start_y = (start_t)*f_Is[wheel_file][before_start]
  ...
  baseline_intercept
      = ((start_y + end_y) - baseline_slope * (start + end)) * 5e-1;
  ...
  // the baseline
  {
    ...
    // baseline_slope ).arg( baseline_intercept ); printvector( "baseline x", x
...
#if QT_VERSION < 0x040000
    plot_dist->setCurvePen (curve,
                            QPen (Qt::green, use_line_width, Qt::DashLine));
    plot_dist->setCurveData (plotted_baseline[0], (double *)&x[0],
                             (double *)&y[0], 2);
#else
    curve->setPen (QPen (Qt::green, use_line_width, Qt::DashLine));
    plotted_baseline[0]->setSamples ((double *)&x[0], (double *)&y[0], 2);
    curve->attach (plot_dist);
#endif
  }
  if (set_start)
    {
      ...
      // ).arg( start_intercept ); printvector( "start x", x ); printvector(
      // "start y", y );
      ...
      plot_dist->setCurvePen (
          curve, QPen (start_color, use_line_width, Qt::DashLine));
      plot_dist->setCurveData (plotted_baseline.back (), (double *)&x[0],
                               (double *)&y[0], 2);
#else
      curve->setPen (QPen (start_color, use_line_width, Qt::DashLine));
      plotted_baseline.back ()->setSamples ((double *)&x[0], (double *)&y[0],
                                            2);
      curve->attach (plot_dist);
#endif
    }
  ...
  if (set_end)
    {
      ...
      // end_intercept ); printvector( "end x", x ); printvector( "end y", y );
...
void
US_Hydrodyn_Dad::baseline_apply ()
{
  int smoothing = 0;
  bool integral
  ...
  if (integral)
    {
      smoothing
          = ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_smooth"].toInt ();
    }
  // bool save_bl = ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "dad_bl_save" ]
  // == "true";
  bool save_bl = false;
  unsigned int reps = 0;
  if (integral)
    {
      reps = ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_reps"].toInt ();
    }
...
void
US_Hydrodyn_Dad::baseline_apply (QStringList files, bool integral,
                                 int smoothing, bool save_bl,
                                 unsigned int reps, bool do_replot, bool quiet)
{
  ...
  if (integral)
    {
      check_zi_window (files, us_tr ("You may have already noticed this "
                                     "message when making I(t).\n\n"));
    }
  ...
  if (integral && save_bl && files.size () > 3)
    {
      switch (QMessageBox::warning (
          ...
          QString (
              us_tr ("Please note:\n\n"
                     "You have selected to produce separate baseline curves "
                     "and have %1 files selected\n"
                     "This will generate upto %2 separate baseline curves in "
                     "addition to your %3 baseline subtracted files\n"
                     "What would you like to do?\n"))
              .arg (files.size ())
          ...
          us_tr (
              "&Temporarily turn off production of separate baseline curves"),
          us_tr ("&Quit"),
          0, // Stop == button 0
          0  // Escape == button 0
          ))
  ...
  //                                  Apply" ), QString( us_tr( "Please
  //                                  note:\n\n"
  //                                               "You have selected the
  //                                               integral baseline method
  //                                               with maximum iterations of
  //                                               %1\n" "It is recommended
  //                                               that a minimum of 5
  //                                               iterations be used to
  ...
  bool test_alt_integral_bl
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_save"] == "true";
  test_alt_integral_bl = true;
  // if ( test_alt_integral_bl ) {
  //    editor_msg( "red", us_tr( "Using alternative baseline integral
  //    strategy" ) );
  // }

  bool use_start_region
      = false
        && ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_bl_start_region")
        && ((US_Hydrodyn *)us_hydrodyn)
                   ->gparams["dad_bl_start_region"]
                   .toInt ()
               > 0;

  bool use_i_power
      = ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_bl_i_power")
        && ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_i_power"].toDouble ()
               != 1e0
  ...
  if (use_i_power)
    {
      i_power = ((US_Hydrodyn *)us_hydrodyn)
                    ...
                    .toDouble ();
      editor_msg ("darkred",
                  QString (us_tr ("Using power of intensity (%1) for "
                                  "distribution of delta baseline"))
                      .arg (i_power));
      // if ( !quiet ) {
      //    switch ( QMessageBox::warning(this,
      //                                  windowTitle() + us_tr( " : Baseline
      ...
      //                                               correction.\n" "The
      //                                               model of intesity
      //                                               proportional deposits is
      //                                               based on a unity
      //                                               intensity exponent.\n"
      //                                               "It is possible that a
      //                                               specific capillary
      //                                               fouling mechanism may be
      //                                               approximated corrected
      //                                               with a non-unity
      //                                               exponent.\n" "We urge
      //                                               you to use a non-unity
      //                                               intensity exponent with
      //                                               care.\n" "What would you
      //                                               like to do?\n" ) )
      //                                  .arg( i_power ),
      //                                  us_tr( "&Continue" ),
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
    {
      ...
      bool use_errors = f_errors[files[i]].size () == f_qs[files[i]].size ()
...

      {
        ...
        if (cb_baseline_start_zero->isChecked ())
          ...
          {
            if (f_qs[files[i]][j] >= start_s && f_qs[files[i]][j] <= start_e)
              {
                start_q.push_back (f_qs[files[i]][j]);
                start_I.push_back (f_Is[files[i]][j]);
              }
          }
        if (f_qs[files[i]][j] >= end_s && f_qs[files[i]][j] <= end_e)
          {
          ...
          for (unsigned int j = 1; j < f_qs[files[i]].size (); j++)
            {
              ...
              if (f_qs[files[i]][j - 1] <= start && f_qs[files[i]][j] >= start)
                {
                  before_start = j - 1;
                  after_start = j;
                }
              if (f_qs[files[i]][j - 1] <= end && f_qs[files[i]][j] >= end)
                {
                  before_end = j - 1;
                  after_end = j;
                }
          ...
          if (set_start)
            {
              ...
              end_y = (end_t)*f_Is[files[i]][before_end]
          ...
          else
            {
              if (set_end)
                {
                  usu->linear_fit (end_q, end_I, end_intercept, end_slope,
                                   siga, sigb, chi2);

                  end_y = end_intercept + end_slope * end;

                  double start_t;
                  if (f_qs[files[i]][after_start]
                      != f_qs[files[i]][before_start])
                    {
                      start_t = (f_qs[files[i]][after_start] - start)
                  ...
                  start_y = (start_t)*f_Is[files[i]][before_start]
      ...
      if (integral)
        {
          {
            ...
            if (use_start_region)
              {
                int start_region_frames = ((US_Hydrodyn *)us_hydrodyn)
                                              ->gparams["dad_bl_start_region"]
                                              .toInt ();
                ...
                if (use_errors)
                  {
                    for (int j = 0; j < (int)f_qs[files[i]].size ()
                                    && j < start_region_frames;
                         ++j)
                      {
                        start_I.push_back (f_Is[files[i]][j]);
                ...
                editor_msg ("darkgreen",
                            QString ("Using start region size %1 for baseline "
                                     "with avg value %2")
          ...
          // us_qdebug( QString( "integral baseline apply, start_y %1 end_y %2"
          // ).arg( start_y ).arg( end_y ) );
          ...
          if (smoothing)
            {
              ...
              if (!usu.smooth (f_Is[files[i]], bl_I, smoothing))
                {
                  bl_I = f_Is[files[i]];
                  messages << QString (us_tr ("Error: smoothing error on %1"))
                                  .arg (files[i]);
              ...
              else
                {
                  if (save_bl)
                    {
                      QString this_file
                          = QString ("SM%1_%2").arg (smoothing).arg (files[i]);
          ...
          // note: we have replaced alpha with gamma to avoid overloading with
          // CorMap analysis alpha
          double alpha = 0e0;
          double alpha_epsilon
              = ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_bl_epsilon")
                    ? ((US_Hydrodyn *)us_hydrodyn)
                          ->gparams["dad_bl_epsilon"]
                          .toDouble ()
                    : 5e-3;
          ...
          if (delta_bl < 0e0)
            {
              // messages << QString( us_tr( "Notice: the overall change in
              // baseline of %1 is negative (%2), so a constant baseline
              // correction will be applied" ) ).arg( files[ i ] ).arg(
              // delta_bl );
              messages << QString (
                              us_tr ("Notice: the overall change in baseline "
                                     "of %1 is negative (%2), so no baseline "
          ...
          else
            {
              do
                {
                  ...
                  if (test_alt_integral_bl)
                    {
                      for (unsigned int j = 0; j < (unsigned int)bl_I.size ();
                           j++)
                        {
                      ...
                      if (I_tot > 0e0 && delta_bl > 0e0)
                        {
                          ...
                          editor_msg (
                              "dark blue",
                              QString (us_tr ("iteration %1 delta_Bl %2 Itot "
                                              "%3 gamma %4 %5%6"))
                                  .arg (this_reps)
                                  .arg (delta_bl)
                                  .arg (I_tot)
                                  .arg (alpha)
                                  .arg (smoothing ? QString (" smoothing %1")
                                                        .arg (smoothing)
                                                  : QString (""))
                                  .arg (i_power != 1e0
                                            ? QString (" I exponent %1")
                                                  .arg (i_power)
                                            : QString ("")));
                          ...
                          if (I_tot < 0e0 && delta_bl < 0e0)
                            {
                              messages
                                  << QString (
                                         us_tr (
                                             "Warning: the overall change in "
                                             "baseline of %1 is negative (%2) "
                                             "and the integral is less than "
                                             "zero, so no baseline correction "
                                             "will be applied"))
                                         .arg (files[i])
                                         .arg (delta_bl);
                              editor_msg ("dark red", messages.back ());
                      ...
                      if (I_tot > 0e0 && delta_bl > 0e0)
                        {
                          ...
                          editor_msg ("dark blue",
                                      QString (us_tr ("iteration %1 delta_Bl "
                                                      "%2 Itot %3 gamma %4%5"))
                                          .arg (this_reps)
                                          .arg (delta_bl)
                                          .arg (I_tot)
                  ...
                  if (save_bl)
                    {
                      QString this_file = QString ("BI_%1-%2")
                                              .arg (files[i])
                                              .arg (this_reps);
                      baseline_apply_created.push_back (this_file);
              ...
              while (this_reps < reps && alpha > 0e0
                     && (fabs (alpha - last_alpha) / alpha) >= alpha_epsilon);
            }
          ...
          QString bl_name_add = use_i_power && test_alt_integral_bl
                                    ? QString ("-ip%1").arg (i_power)
                                    : QString ("");
      ...
      else
        {
          ...
          for (unsigned int j = 0; j < bl_I.size (); j++)
            {
              bl_I[j]
                  -= baseline_slope * f_qs[files[i]][j] + baseline_intercept;
            }
          bl_name += QString ("-bl%1-%2s")
                         .arg (baseline_slope, 0, 'g', 8)
                         .arg (baseline_intercept, 0, 'g', 8)
                         .replace (".", "_");
          while (current_files.count (bl_name))
            {
              bl_name = files[i]
  ...
  if (messages.size () && !quiet)
    {
      ...
      QMessageBox::warning (
          this, this->windowTitle () + us_tr (": Baseline apply"),
          QString (us_tr ("Please note:\n\n"
                          "These warnings were produced when performing the "
                          "baseline correction\n"
                          "%1\n"))
              .arg (qsl.join ("\n")),
          QMessageBox::Ok | QMessageBox::Default, QMessageBox::NoButton);
...
void
US_Hydrodyn_Dad::set_baseline_fix_width ()
{
...
int
US_Hydrodyn_Dad::input_double_to_pos (double d)
{
...
void
US_Hydrodyn_Dad::baseline_start_s_text (const QString &text)
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Dad::baseline_start_text (const QString &text)
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Dad::baseline_start_e_text (const QString &text)
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Dad::baseline_end_s_text (const QString &text)
{
  ...
  if (current_mode != MODE_BLANKS && baseline_integral)
    {
#if QT_VERSION < 0x040000
      plot_dist->setMarkerPos (plotted_markers[pos + 1], text.toDouble (),
                               0e0);
#else
      ...
      if (cb_baseline_fix_width->isChecked ())
        {
          ...
          if (blw > q_size - 1)
            {
              blw = q_size - 1;
              disconnect (le_baseline_width,
                          SIGNAL (textChanged (const QString &)), 0, 0);
              le_baseline_width->setText (QString ("%1").arg (blw));
              connect (le_baseline_width,
          ...
          disconnect (le_baseline_end_e,
                      SIGNAL (textChanged (const QString &)), 0, 0);
          le_baseline_end_e->setText (
              QString ("%1").arg (f_qs[wheel_file][new_pos_e]));
          connect (le_baseline_end_e, SIGNAL (textChanged (const QString &)),
                   SLOT (baseline_end_e_text (const QString &)));
#if QT_VERSION < 0x040000
          plot_dist->setMarkerPos (plotted_markers[3],
                                   f_qs[wheel_file][new_pos_e], 0e0);
#else
          plotted_markers[3]->setXValue (f_qs[wheel_file][new_pos_e]);
#endif
          if (new_pos_s != pos_s)
            {
              // disconnect( le_baseline_end_s, SIGNAL( textChanged( const
              // QString & ) ), 0, 0 ); le_baseline_end_s->setText( QString(
              // "%1" ).arg( f_qs[ wheel_file ][ new_pos_s ] ) ); connect(
              // le_baseline_end_s, SIGNAL( textChanged( const QString & ) ),
              // SLOT( baseline_end_s_text( const QString & ) ) );
...
#if QT_VERSION < 0x040000
              plot_dist->setMarkerPos (plotted_markers[pos],
                                       f_qs[wheel_file][new_pos_s], 0e0);
              plot_dist->setMarkerPos (plotted_markers[pos + 1],
                                       f_qs[wheel_file][new_pos_s], 0e0);
#else
              plotted_markers[pos]->setXValue (f_qs[wheel_file][new_pos_s]);
              plotted_markers[pos + 1]->setXValue (
                  f_qs[wheel_file][new_pos_s]);
#endif
            }
        }
      else
          ...
          if (pos_e - IB_MIN_RANGE + 1 < pos_s)
            {
              ...
              disconnect (le_baseline_end_e,
                          SIGNAL (textChanged (const QString &)), 0, 0);
              le_baseline_end_e->setText (
                  QString ("%1").arg (f_qs[wheel_file][pos_e]));
              connect (le_baseline_end_e,
                       SIGNAL (textChanged (const QString &)),
                       SLOT (baseline_end_e_text (const QString &)));
#if QT_VERSION < 0x040000
              plot_dist->setMarkerPos (plotted_markers[3],
                                       f_qs[wheel_file][pos_e], 0e0);
#else
...
void
US_Hydrodyn_Dad::baseline_end_text (const QString &text)
{
...
void
US_Hydrodyn_Dad::baseline_end_e_text (const QString &text)
{
      ...
      if (cb_baseline_fix_width->isChecked ())
        {
          ...
          disconnect (le_baseline_end_s,
                      SIGNAL (textChanged (const QString &)), 0, 0);
          le_baseline_end_s->setText (
              QString ("%1").arg (f_qs[wheel_file][new_pos_s]));
          connect (le_baseline_end_s, SIGNAL (textChanged (const QString &)),
                   SLOT (baseline_end_s_text (const QString &)));
#if QT_VERSION < 0x040000
          ...
          if (new_pos_e != pos_e)
            {
              // disconnect( le_baseline_end_e, SIGNAL( textChanged( const
              // QString & ) ), 0, 0 ); le_baseline_end_e->setText( QString(
              // "%1" ).arg( f_qs[ wheel_file ][ new_pos_e ] ) ); connect(
              // le_baseline_end_e, SIGNAL( textChanged( const QString & ) ),
              // SLOT( baseline_end_s_text( const QString & ) ) );
          ...
          if (pos_s > pos_e - IB_MIN_RANGE + 1)
            {
              ...
              disconnect (le_baseline_end_s,
                          SIGNAL (textChanged (const QString &)), 0, 0);
              le_baseline_end_s->setText (
                  QString ("%1").arg (f_qs[wheel_file][pos_s]));
              connect (le_baseline_end_s,
                       SIGNAL (textChanged (const QString &)),
                       SLOT (baseline_end_s_text (const QString &)));
#if QT_VERSION < 0x040000
              plot_dist->setMarkerPos (plotted_markers[0],
                                       f_qs[wheel_file][pos_s], 0e0);
              plot_dist->setMarkerPos (plotted_markers[1],
                                       f_qs[wheel_file][pos_s], 0e0);
#else
...
void
US_Hydrodyn_Dad::baseline_start_s_focus (bool hasFocus)
{
  ...
  if (hasFocus)
    {
      ...
      // qwtw_wheel->setRange( f_qs[ wheel_file ][ 0 ], f_qs[ wheel_file
      // ].back(),
      //                       ( f_qs[ wheel_file ].back() - f_qs[ wheel_file
      //                       ][ 0 ] ) / UHSH_WHEEL_RES );
      qwtw_wheel->setRange (0, (double)f_qs[wheel_file].size () - 1);
      qwtw_wheel->setSingleStep (1);
      ...
      qwtw_wheel->setValue ((double)input_double_to_pos (
          le_baseline_start_s->text ().toDouble ()));
      wheel_enables ();
    }
}

void
US_Hydrodyn_Dad::baseline_start_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Dad::baseline_start_e_focus (bool hasFocus)
{
  ...
  if (hasFocus)
    {
      ...
      qwtw_wheel->setValue ((double)input_double_to_pos (
          le_baseline_start_e->text ().toDouble ()));
      wheel_enables ();
    }
}

void
US_Hydrodyn_Dad::baseline_end_s_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Dad::baseline_end_focus (bool hasFocus)
{
  ...
  if (hasFocus)
    {
      ...
      qwtw_wheel->setValue (
          (double)input_double_to_pos (le_baseline_end->text ().toDouble ()));
      wheel_enables ();
    }
}

void
US_Hydrodyn_Dad::baseline_end_e_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Dad::baseline_width_text (const QString &text)
{
  int blw = text.toInt ();
  // us_qdebug( QString( "baseline_width_text %1 %2" ).arg( blw ).arg(
  // le_baseline_width->hasFocus() ? "hasFocus" : "!hasFocus" ) );
  if (blw < 1)
    {
  ...
  if (le_baseline_width->hasFocus ())
    {
      ...
      if (current_mode != MODE_BLANKS && baseline_integral)
        {
          int pos_s
              = input_double_to_pos (le_baseline_end_s->text ().toDouble ());
          ...
          // us_qdebug( QString( "baseline width new pos s %1, e %2" ).arg(
          // new_pos_s ).arg( new_pos_e ) );
          disconnect (le_baseline_end_e,
                      SIGNAL (textChanged (const QString &)), 0, 0);
          le_baseline_end_e->setText (
              QString ("%1").arg (f_qs[wheel_file][new_pos_e]));
...
void
US_Hydrodyn_Dad::baseline_width_focus (bool hasFocus)
{
  if (hasFocus)
    {
      le_last_focus = le_baseline_width;
      // us_qdebug( QString( "baseline_width focus blw currently %1" ).arg(
      // le_baseline_width->text() ) );
      disconnect (qwtw_wheel, SIGNAL (valueChanged (double)), 0, 0);
      qwtw_wheel->setRange (IB_MIN_RANGE, baseline_max_window_size - 1);
      qwtw_wheel->setSingleStep (1);
      connect (qwtw_wheel, SIGNAL (valueChanged (double)),
               SLOT (adjust_wheel (double)));
...
US_Hydrodyn_Dad::baseline_best ()
{
  ...
  if (!baseline_hb_only_mode)
    {
      if (blanks_cormap_parameters.count ("decimate")
  ...
  if (blanks_cormap_parameters.count ("hb") || baseline_hb_only_mode)
    {
      parameters["hb"] = "true";
      hb_mode = true;
    }
  ...
  if (blanks_cormap_parameters.count ("alpha"))
    {
      // us_qdebug( QString( "got alpha %1 from blanks" ).arg(
      // blanks_cormap_parameters[ "alpha" ] ) );
  ...
  // do sliding analysis of each region and from end_s to end with window and
  // plot avg red cluster size
  ...
  if (baseline_cormap_parameters.size ()
    ...
    {
      US_Hydrodyn_Saxs_Cormap_Cluster_Analysis ca;
      baseline_cormap_parameters["sliding_minimum_size"]
          = le_baseline_width->text ();
      baseline_cormap_parameters["sliding_maximum_size"]
          = le_baseline_width->text ();
      baseline_cormap_parameters["sliding_baseline_mode"] = "true";
      baseline_cormap_parameters["sliding_baseline_pos_base"]
          = QString ("%1").arg (
              input_double_to_pos (baseline_last_cormap_run_end_s));

      if (ca.sliding (baseline_cormap_pvaluepairs, baseline_cormap_parameters,
                      baseline_brookesmap_sliding_results,
                      baseline_hb_brookesmap_sliding_results, this, progress))
        {
          // us_qdebug( "sliding_results" );
          // for ( map < QString, double >::iterator it =
          // baseline_brookesmap_sliding_results.begin();
          ...
          // plot of results
          {
            ...
            if (!hb_mode)
              {
                ...
                QString tag
                    = QString ("%1:Red cluster size average").arg (width);
                if (blanks_brookesmap_sliding_results.count (tag))
                  {
                    best_dparameters["blanksaverage"]
                        = blanks_brookesmap_sliding_results[tag];
                    QString tagsd = QString ("%1 sd:Red cluster size average")
                                        .arg (width);
                    if (blanks_brookesmap_sliding_results.count (tagsd))
                      {
                        best_dparameters["blanksaveragesd"]
                            = blanks_brookesmap_sliding_results[tagsd];
                      }
                  }
              }

            double cormap_maxq = ((US_Hydrodyn *)us_hydrodyn)
                                         ->gparams.count ("dad_cormap_maxq")
                                     ? ((US_Hydrodyn *)us_hydrodyn)
...

            {
              ...
              best_parameters["name"] = QString ("%1%2").arg (head).arg (tail);

              // .arg( baseline_selected.size() )
            }

            best_dparameters["width"] = baseline_best_last_window_size;
            best_parameters["xlegend"]
                = QString (us_tr ("Start Frame (window width %1 frames)"))
                      .arg (baseline_best_last_window_size);
            best_parameters["title"]
                = QString (us_tr (hb_mode
                                      ? "Baseline avg. I(q) and red pair % by "
                                        "start frame (window width %1 frames)."
                                      : "Baseline avg. I(q) and average red "
                                        "cluster size by start frame\n(window "
                                        "width %1 frames)."))
                      .arg (baseline_best_last_window_size);

            if (!hb_mode)
              {
                best_parameters["hb_title"]
                    = QString (
                          us_tr ("Baseline avg. I(q) and red pair % by start "
                ...
                if (decimate > 1)
                  {
                    ...
                    best_parameters["title"]
                        += QString (
                               us_tr (" Only every %1%2 q value selected."))
                               .arg (decimate)
                               .arg (nth_tag);
                    best_parameters["hb_title"]
                        += QString (
            ...
            QRegExp rx_cap (hb_mode ? "^(\\d+):% red pairs$"
            ...
            for (map<QString, double>::iterator it
                 = baseline_brookesmap_sliding_results.begin ();
                 it != baseline_brookesmap_sliding_results.end (); ++it)
              {
            ...
            if (!hb_mode)
              {
                ...
                for (map<QString, double>::iterator it
                     = baseline_hb_brookesmap_sliding_results.begin ();
                     it != baseline_hb_brookesmap_sliding_results.end (); ++it)
                  {
                    if (rx_cap.indexIn (it->first) != -1)
                      {
                        int pos = rx_cap.cap (1).toInt ();
...

            {
              ...
              for (map<double, double>::iterator it = plotdata.begin ();
                   it != plotdata.end (); ++it)
                {
                  ...
                  if (it->second <= green_limit && !hb_mode)
                    {
              ...
              // us_qdebug( QString( "ybest %1 pos %2" ).arg( ybest ).arg(
              // ybestposx ) );
              ...
              if (!hb_mode)
                {
                          ...
                          if (hb_ybest == it->second)
                            {
                              ...
                              best_vdparameters["hb_bestpos"].push_back (
                                  it->first);
                            }
                        }
                  ...
                  // US_Vector::printvector2( "sliding analysis data", x, y );
                  // US_Vector::printvector2( "sliding analysis gy results",
                  // gx, gy );

                  best_vdparameters["hb_x"] = hb_x;
                  best_vdparameters["hb_y"] = hb_y;

                  // us_qdebug( QString( "ybest %1 pos %2" ).arg( hb_ybest
                  // ).arg( hb_ybestposx ) ); if ( hb_ybestposx != -1 ) {
              ...
              { // new way
                ...
                for (int i = 0; i < (int)baseline_selected.size (); ++i)
                  {
                    if (!f_errors.count (baseline_selected[i])
                        || f_errors[baseline_selected[i]].size ()
                               != f_Is[baseline_selected[i]].size ()
                ...
                if (use_errors)
                  {
                    for (int i = 0; i < (int)baseline_selected.size (); ++i)
                      {
                        if (rx_capqv.indexIn (baseline_selected[i]) == -1)
                          {
                            editor_msg (
                                ...
                                QString (
                                    us_tr (
                                        "Baseline Find best region: Could not "
                        ...
                        double qv
                            = rx_capqv.cap (1).replace ("_", ".").toDouble ();

                        for (int j = 0;
                             j < (int)f_qs[baseline_selected[i]].size (); ++j)
                          {
                            ...
                            if (qv <= cormap_maxq)
                              {
                                ...
                                msumqmaxq[j]
                                    += f_Is[baseline_selected[i]][j] * sdinv2;
                                ptsmaxq[j]++;
                              }
                          }
                      }
                    ...
                    // now msumq[ j ] and msumqmaxq[ j ] contains sd weighted
                    // xbar for each frame
                  }
                else
                  {
                    for (int i = 0; i < (int)baseline_selected.size (); ++i)
                      {
                        ...
                        for (int j = 0;
                             j < (int)f_qs[baseline_selected[i]].size (); ++j)
                          {
                            ...
                            msum2q[j] += f_Is[baseline_selected[i]][j]
                                         * f_Is[baseline_selected[i]][j];
                            pts[j]++;
                            if (qv <= cormap_maxq)
                              {
                    ...
                    // now msumq[ j ] and msumqmaxq[ j ] contain the average
                    // intensity
                  }
                    ...
                    if (rx_cap.indexIn (it->first) != -1)
                      {
                        ...
                        msumqmaxqy[pos] = 0e0;
                        msum2qmaxqy[pos] = 0e0;
                        for (int i = 0; i < baseline_best_last_window_size;
                             ++i)
                          {
                            int upos = i + pos;
                            if (upos < q_size)
                              {
                                ...
                                msum2qmaxqy[pos]
                                    += msumqmaxq[upos] * msumqmaxq[upos];
                              }
                          }
                        double countinv
                            = 1e0 / (double)baseline_best_last_window_size;
                        if (baseline_best_last_window_size > 1)
                          {
                            double countm1inv
                                = 1e0
                                  / ((double)baseline_best_last_window_size
                                     - 1);
                            msumqysd[pos] = sqrt (
                                countm1inv
                                * (msum2qy[pos]
                                   - countinv * msumqy[pos] * msumqy[pos]));
                            msumqmaxqysd[pos]
                                = sqrt (countm1inv
                                        * (msum2qmaxqy[pos]
                                           - countinv * msumqmaxqy[pos]
                                                 * msumqmaxqy[pos]));
                          }
                        msumqy[pos] *= countinv;
                        ...
                        if (minsumqmaxqy > msumqmaxqy[pos])
                          {
                            ...
                            minsumqmaxqysd = msumqmaxqysd.count (pos)
                                                 ? msumqmaxqysd[pos]
                                                 : 0e0;
                          }
                      }
                ...
                for (map<int, double>::iterator it = msumqx.begin ();
                     it != msumqx.end (); ++it)
                  {
                    ...
                    sd = msumqysd.count (it->first) ? msumqysd[it->first]
                                                    : 0e0;
                    sdmaxq = msumqmaxqysd.count (it->first)
                                 ? msumqmaxqysd[it->first]
                                 : 0e0;
                ...
                for (int i = 0; i < (int)sumqx.size (); ++i)
                  {
                    if (minsumqmaxqy == sumqmaxqy[i])
                      {
                        best_vdparameters["bestsumqmaxqypos"].push_back (
                            sumqx[i]);
                      }
                  }
                ...
                if (blanks_cormap_parameters.count ("blanks_avg_maxq_sd"))
                  {
                    best_dparameters["blanks_avg_maxq_sd"]
                        = blanks_cormap_parameters["blanks_avg_maxq_sd"]
                              .toDouble ();
                  }
                ...
                //       it != msumq.end();
                //       ++it ) {
                //    cout << QString( "msumq %1 %2" ).arg( f_qs[ wheel_file ][
                //    it->first ] ).arg( it->second ); if ( msumqmaxq.count(
                //    it->first ) ) {
                //       cout << QString( " msumqmaxq %1" ).arg( msumqmaxq[
                //       it->first ] );
                //    }
                //    cout << endl;
                // }
                // US_Vector::printvector3( "sumqx, sumqy, sumqmaxqy", sumqx,
                // sumqy, sumqmaxqy );
              }
            ...
            if (hb_mode)
              {
                best_parameters["msg"]
                    += "Holm-Bonferroni adjustment is active\n";
            ...
            if (blanks_cormap_parameters.count ("alpha"))
              {
                tmp_msg
                    += QString ("Alpha is %1\n\n")
                           .arg (blanks_cormap_parameters["alpha"].toDouble ()
                                 * 0.2);
              }
                ...
                if (best_dparameters.count ("minsumqmaxqy"))
                  {
                    if (false && best_dparameters.count ("minsumqmaxqysd"))
                      {
                        if (best_dparameters["minsumqmaxqy"]
                            > best_dparameters["minsumqmaxqysd"])
                          {
                            if (best_dparameters["minsumqmaxqy"]
                                > 5 * best_dparameters["minsumqmaxqysd"])
                              {
                                tmp_msg
                                    += us_tr ("Integral baseline correction "
                                              "is recommended\n\n");
                              }
                            else
                              {
                                if (best_dparameters["minsumqmaxqy"]
                                    > 3 * best_dparameters["minsumqmaxqysd"])
                                  {
                                    tmp_msg += us_tr (
                                        "Integral baseline correction is of "
                                        "borderline need\n\n");
                                  }
                                else
                                  {
                        ...
                        else
                          {
                            if (best_dparameters["minsumqmaxqy"]
                              ...
                              {
                                tmp_msg
                                    += us_tr ("It appears there is an issue "
                                              "with buffer subtraction "
                                              "resulting in negative data\n")
                                       + us_tr ("Integral baseline correction "
                                                "is not appropriate for this "
                                                "situation\n")
                                       + us_tr (
                                           "Perhaps a linear baseline "
                                           "correction would help, but we "
                                           "recommend you first verify the "
                                           "buffer subtraction\n\n");
                              }
                            else
                              {
                    ...
                    else
                      {
                        ...
                        if (best_dparameters["minsumqmaxqy"] > 0)
                          ...
                          {
                            tmp_msg += us_tr ("Integral baseline correction "
                                              "is not recommended\n\n");
                          }
                      }
                  }
            ...
            tmp_msg
                += QString ("Baseline Analysis:\n"
                            "  Name                                           "
                            "      %1\n"
                            "  q sampling interval                            "
                            "      %2\n"
                            "  Cormap Analysis maximum q [A^-1]               "
                            "      %3\n")
                       .arg (best_parameters["name"])
                       .arg (baseline_cormap_parameters.count ("decimate")
                                 ? QString ("%1").arg (
                                       baseline_cormap_parameters["decimate"])
                                 : QString ("none"))
                       .arg (cormap_maxq);

            best_dparameters["cormap_maxq"]
                = QString ("%1").arg (cormap_maxq).toDouble ();

            // accumulate common strings
            best_parameters["hb_msg"]
                += best_parameters["msg"]
                   + "Holm-Bonferroni adjustment values\n" + tmp_msg;
            ...
            if (best_vdparameters.count ("bestpos"))
              {
                if (best_vdparameters["bestpos"].size () > 1)
                  {
                    best_parameters["msg"]
                        += QString (hb_mode ? "  Best start positions based "
                                              "on red pair %             %1"
                                            : "  Best start positions based "
                                              "on avg. red cluster size  %1")
                               .arg (best_vdparameters["bestpos"].front ());
                    for (int i = 1;
                         i < (int)best_vdparameters["bestpos"].size (); ++i)
                      {
                        best_parameters["msg"] += QString (",%1").arg (
                            best_vdparameters["bestpos"][i]);
                      }
                    best_parameters["msg"] += "\n";
                  }
                else
                  {
                    best_parameters["msg"]
                        += QString (
                               best_vdparameters.count ("yx")
                                   ? "~~_darkyellow2_~~  Best start positions "
                                     "based on avg. red cluster size  "
                                     "%1~~_darkblue_~~"
                                   : (hb_mode
                                          ? "  Best start position based on "
                                            "red pair %              %1\n"
                                          : "  Best start position based on "
                                            "avg. red cluster size   %1\n"))
                               .arg (best_vdparameters["bestpos"].front ());
                  }
              }
            ...
            if (best_vdparameters.count ("hb_bestpos"))
              {
                if (best_vdparameters["hb_bestpos"].size () > 1)
                  {
                    best_parameters["hb_msg"]
                        += QString ("  Best start positions based on red pair "
                                    "%             %1")
                               .arg (best_vdparameters["hb_bestpos"].front ());
                    for (int i = 1;
                         i < (int)best_vdparameters["hb_bestpos"].size (); ++i)
                      {
                        best_parameters["hb_msg"] += QString (",%1").arg (
                            best_vdparameters["hb_bestpos"][i]);
                      }
                    best_parameters["hb_msg"] += "\n";
                  }
                else
                  {
                    best_parameters["hb_msg"]
                        += QString (("  Best start position based on red pair "
                                     "%              %1\n"))
                               .arg (best_vdparameters["hb_bestpos"].front ());
                  }
              }
              ...
              if (best_vdparameters.count ("bestsumqmaxqypos"))
                {
                  if (best_vdparameters["bestqmaxqypos"].size () > 1)
                    {
                      tmp_msg
                          += QString ("~~_darkorange3_~~  Lowest average "
                                      "intensity positions                   "
                                      "%1~~_darkblue_~~")
                                 .arg (best_vdparameters["bestsumqmaxqypos"]
                                           .front ());
                      for (int i = 1;
                           i < (int)best_vdparameters["bestsumqmaxqypos"]
                                   .size ();
                           ++i)
                        {
                          tmp_msg += QString (",%1").arg (
                              best_vdparameters["bestsumqmaxqypos"][i]);
                        }
                      tmp_msg += "\n";
                    }
                  else
            ...
            else
              {
                ...
                best_parameters["msg"]
                    += QString ("\n"
                                ...
                                "  Used window width                          "
                                "          %4\n"
                                "  Number of sliding windows of this width "
                                "averaged     %5\n")
                           .arg (blanks_cormap_parameters.count ("name")
                                     ? blanks_cormap_parameters["name"]
                                     : QString (us_tr ("unknown")))
                           .arg (blanks_cormap_parameters.count ("decimate")
                                     ? blanks_cormap_parameters["decimate"]
                                     : QString (us_tr ("none")))
                           .arg (blanks_cormap_parameters.count ("frames")
                                     ? blanks_cormap_parameters["frames"]
                                     : QString (us_tr ("unknown")))
                           .arg (baseline_best_last_window_size)
                           .arg (
                               blanks_brookesmap_sliding_results.count (
                                   QString ("%1 count")
                                       .arg (baseline_best_last_window_size))
                                   ? QString ("%1").arg (
                                         blanks_brookesmap_sliding_results
            ...
            if (best_dparameters.count ("blanksaverage"))
              {
                best_parameters["msg"]
                    += QString ("  Used window red cluster average size       "
                                "          %1\n")
            ...
            if (best_vdparameters["x"].size ())
              {
                US_Hydrodyn_Dad_Baseline_Best *uhsbb
                    = new US_Hydrodyn_Dad_Baseline_Best (
                        us_hydrodyn, best_parameters, best_dparameters,
                        best_vdparameters);
                uhsbb->show ();
...
US_Hydrodyn_Dad::baseline_test ()
{
  ...
  // ?) save & remove files as needed include full iteration plot and plot
  // original & created curve
  ...
  baseline_enables ();
  wheel_enables (false);
  qwtw_wheel->setRange (0, baseline_selected.size () - 1);
  qwtw_wheel->setSingleStep (1);
...
void
US_Hydrodyn_Dad::baseline_test_pos (int pos)
{
  // us_qdebug( QString( "baseline_test_pos( %1 ) %2" ).arg( pos )
  //         .arg(
  //              ( pos >= 0 && pos < (int) baseline_selected.size() ) ?
  //              baseline_selected[ pos ] : QString( "outta limits" ) ) );
  ...
  // create new files via apply baseline and add to baseline_test_added_set &
  // select_set

  QStringList qsl;
  qsl << baseline_selected[pos];
  // us_qdebug( QString( "smooth %1" ).arg( ( ( US_Hydrodyn * ) us_hydrodyn
  // )->gparams[ "dad_bl_smooth" ].toInt() ) );

  baseline_apply (
      qsl, true,
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_smooth"].toInt (), true,
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_bl_reps"].toInt (), false,
      true);
