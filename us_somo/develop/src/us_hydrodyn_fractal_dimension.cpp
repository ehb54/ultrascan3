...
static bool
linear_fit (const double &xmin, const double &xmax,
            const vector<vector<vector<double> > > x,
            const vector<vector<vector<double> > > y, mQwtPlot *plot,
            vector<vector<QwtPlotCurve *> > &fitcurves, QTextEdit *msgbox,
            double &fd, double &fd_sd, double &fd_wtd, double &fd_wtd_sd,
            double &fd_wtd_wtd, double &fd_wtd_wtd_sd)
{
  ...
  qDebug () << QString ("linear_fit(); xmin is %1, xmax is %2")
                   .arg (xmin)
                   .arg (xmax);
  ...
  for (size_t g = 0; g < x.size (); ++g)
    {
      if (x[g].size () != fitcurves[g].size ())
        {
          msgbox->setText (
              us_tr ("Internal error: fitcurves x size mismatch"));
          return false;
        }
      double a_sum = 0;
      double b_sum = 0;
      ...
      for (size_t i = 0; i < x[g].size (); ++i)
        {
          ...
          for (size_t j = 0; j < x[g][i].size (); ++j)
            {
              if (x[g][i][j] >= xmin - UHFD_PLOT_EPSILON
                  && x[g][i][j] <= xmax + UHFD_PLOT_EPSILON)
                {
          ...
          if (!use_x.size ())
            {
              ...
              if (fitcurves[g][i])
                {
                  fitcurves[g][i]->setSamples ((double *)&(fitx[0]),
                                               (double *)&(fity[0]), 0);
                  plot->replot ();
                }
              return false;
        ...
        }
      {
        ...
        if (!US_Average::compute (US_Average::NO_WEIGHTS, a_v, siga_v,
                                  a_avg_no_weights, a_sd_no_weights, errors))
          {
            errorsfull += errors;
          }
        if (!US_Average::compute (US_Average::NO_WEIGHTS, b_v, sigb_v,
                                  b_avg_no_weights, b_sd_no_weights, errors))
          {
            errorsfull += errors;
          }
        if (!US_Average::compute (US_Average::ONE_OVER_SD_SQ, b_v, sigb_v,
                                  b_avg_weighted, b_sd_weighted, errors))
          {
            errorsfull += errors;
          }
        ...
        msgbox->setText (
            msgbox->toPlainText ()
            + QString ("%1").arg (x.size () > 1
                                      ? QString ("Group %1 (size %2): ")
                                            .arg (g + 1)
                                            .arg (x[g].size ())
                                      : QString (""))
            + QString (
                  "a = %1 (%2%3)  b = %4 (%5%6), wtd. b = %7 (%8%9) %10\n")
  ...
  if (global_no_weights_a_v.size () > 1 && global_count)
    {
      double a_avg_no_weights;
      double a_sd_no_weights;
      double b_avg_no_weights;
      ...
      if (!US_Average::compute (US_Average::NO_WEIGHTS, global_no_weights_a_v,
                                global_no_weights_siga_v, a_avg_no_weights,
                                a_sd_no_weights, errors))
        {
          errorsfull += errors;
        }
      if (!US_Average::compute (US_Average::NO_WEIGHTS, global_no_weights_b_v,
                                global_no_weights_sigb_v, b_avg_no_weights,
                                b_sd_no_weights, errors))
        {
          errorsfull += errors;
        }
      if (!US_Average::compute (US_Average::NO_WEIGHTS, global_weighted_b_v,
                                global_weighted_sigb_v,
                                b_avg_no_weights_from_weighted,
                                b_sd_no_weights_from_weighted, errors))
        {
          errorsfull += errors;
        }
      if (!US_Average::compute (US_Average::ONE_OVER_SD_SQ,
                                global_weighted_b_v, global_weighted_sigb_v,
                                b_avg_weighted, b_sd_weighted, errors))
        {
          errorsfull += errors;
        }
      ...
      msgbox->setText (
          msgbox->toPlainText ()
          + QString ("%1").arg (x.size () > 1 ? "Global average : " : "")
          + QString ("a = %1 (%2%3)  b = %4 (%5%6), avg of wtd. b = %7 "
                     "(%8%9), wtd. of wtd. b = %10 (%11%12) %13\n")
                .arg (a_avg_no_weights, 0, 'f', 3)
                .arg (UNICODE_PLUSMINUS)
                .arg (a_sd_no_weights, 0, 'f', 3)
                .arg (b_avg_no_weights, 0, 'f', 3)
...
void
US_Hydrodyn::fractal_dimension (bool from_parameters, save_info *fd_save_info)
{
      ...
      if (!quiet)
        {
          QMessageBox::critical (
              this, windowTitle () + us_tr (": Fractal Dimension"),
  ...
  if (!from_parameters)
    {
          ...
          for (size_t k = 0;
               k < model_vector[current_model].molecule[j].atom.size (); ++k)
            {
              ...
              if (!this_atom.p_residue)
                {
                  editor_msg (
                      "red", QString (us_tr ("Internal error: p_residue not "
              ...
              if (!this_atom.p_atom)
                {
                  editor_msg (
                      "red", QString (us_tr ("Internal error: p_atom not "
                                             "set!, contact the developers")));
                  return;
                }

              QString use_res_name = this_atom.name != "OXT"
                                         ? this_atom.p_residue->name
                                         : "OXT";
              ...
              if (vdwf.count (res_idx))
                {
                  // QTextStream( stdout ) << "found ionized_mw_delta " <<
                  // vdwf[ res_idx ].ionized_mw_delta << endl;
                  PDB_atom tmp_atom;
                  ...
                  tmp_atom.bead_ref_ionized_mw_delta
                      = this_vdwf.ionized_mw_delta;
      ...
      if (!model.size ())
        {
          QMessageBox::critical (
              this, windowTitle () + us_tr (": Fractal Dimension - PAT"),
      ...
      if ("empty model" != usu.nsa_physical_stats ())
        {
          ...
          extents.axis[0] = usu.nsa_physical_stats_map
                                ["result radial extent bounding box size x"]
                                    .toDouble ()
                            * fconv;
          extents.axis[1] = usu.nsa_physical_stats_map
                                ["result radial extent bounding box size y"]
                                    .toDouble ()
                            * fconv;
          extents.axis[2] = usu.nsa_physical_stats_map
      ...
      else
        {
          QMessageBox::critical (
              this, windowTitle () + us_tr (": Fractal Dimension - PAT"),
              us_tr (
                  "Internal error - PAT computation failed - empty model(?)"));
  ...
  if (!from_parameters)
      ...
      do
        {
          ...
          dialog.setWindowTitle (
              windowTitle () + us_tr (": Fractal Dimension : Set parameters"));
          ...
          form.addRow (new QLabel (
              QString (
                  us_tr ("Set the box counting parameters<br>"
          ...
          vector<QString> labels = {
            ...
            QString (us_tr ("Start C%1 dist. % (Enright only)"))
                .arg (UNICODE_ALPHA),
            QString (us_tr ("End C%1 dist. % (Enright only)"))
                .arg (UNICODE_ALPHA),
            QString (us_tr ("Probe radius start [%1] (Rolling sphere only)"))
                .arg (UNICODE_ANGSTROM),
            QString (us_tr ("Probe radius end [%1] (Rolling sphere only)"))
                .arg (UNICODE_ANGSTROM),
          ...
          vector<double> defaults
              = { 0, sas_asa_probe_radius, 5, 20, 20, 0, 10, 1, 2, 5 };

          vector<bool> intvalidator = { false, false, false, false, true,
          ...
          AUTFBACK (cmb_method);
          cmb_method->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize - 1));
          cmb_method->setEnabled (true);
          cmb_method->setMaxVisibleItems (1);

          cmb_method->addItem (US_Fractal_Dimension::method_name (
                                   US_Fractal_Dimension::USFD_BOX_MODEL),
                               US_Fractal_Dimension::USFD_BOX_MODEL);
          cmb_method->addItem (US_Fractal_Dimension::method_name (
                                   US_Fractal_Dimension::USFD_BOX_ALT),
                               US_Fractal_Dimension::USFD_BOX_ALT);
          cmb_method->addItem (US_Fractal_Dimension::method_name (
                                   US_Fractal_Dimension::USFD_BOX_MASS),
                               US_Fractal_Dimension::USFD_BOX_MASS);
          cmb_method->addItem (US_Fractal_Dimension::method_name (
                                   US_Fractal_Dimension::USFD_ENRIGHT),
                               US_Fractal_Dimension::USFD_ENRIGHT);
          cmb_method->addItem (US_Fractal_Dimension::method_name (
                                   US_Fractal_Dimension::USFD_ENRIGHT_FULL),
                               US_Fractal_Dimension::USFD_ENRIGHT_FULL);
          cmb_method->addItem (US_Fractal_Dimension::method_name (
                                   US_Fractal_Dimension::USFD_ROLL_SPHERE),
                               US_Fractal_Dimension::USFD_ROLL_SPHERE);
          ...
          if (dialog.exec () == QDialog::Accepted)
            {
              if (((QLineEdit *)fields[0])->text ().toDouble () < 0
                  ...
                  || ((QLineEdit *)fields[2])->text ().toDouble ()
                         >= ((QLineEdit *)fields[3])->text ().toDouble ()
                  ...
                  || ((QLineEdit *)fields[5])->text ().toDouble ()
                         >= ((QLineEdit *)fields[6])->text ().toDouble ()

                  // roll sphere
                  || ((QLineEdit *)fields[7])->text ().toDouble () <= 0
                  || ((QLineEdit *)fields[8])->text ().toDouble () <= 0
                  || ((QLineEdit *)fields[7])->text ().toDouble ()
                         >= ((QLineEdit *)fields[8])->text ().toDouble ()
                ...
                {
                  // If the user didn't dismiss the dialog, do something with
                  // the fields
                  sas_asa_threshold
                  ...
                  angstrom_steps
                      = ((QLineEdit *)fields[4])->text ().toDouble ();
                  ...
                  roll_sphere_end
                      = ((QLineEdit *)fields[8])->text ().toDouble ();
                  roll_sphere_steps
                      = ((QLineEdit *)fields[9])->text ().toDouble ();
                  method = (US_Fractal_Dimension::methods)cmb_method
                               ->currentData ()
                               .toInt ();
                  qDebug ()
                      << QString ("selected method %1").arg ((int)method);
                }
            }
          else
            {
  ...
  else
    {
      ...
      sas_asa_threshold
          = gparam_value (
                US_Hydrodyn_Fractal_Dimension_Options::paramname (
                    US_Hydrodyn_Fractal_Dimension_Options::ASA_THRESHOLD))
                .toDouble ();
      sas_asa_probe_radius
          = gparam_value (
                US_Hydrodyn_Fractal_Dimension_Options::paramname (
                    US_Hydrodyn_Fractal_Dimension_Options::ASA_PROBE_RADIUS))
                .toDouble ();
      angstrom_start
          = gparam_value (
                US_Hydrodyn_Fractal_Dimension_Options::paramname (
                    US_Hydrodyn_Fractal_Dimension_Options::ANGSTROM_START))
                .toDouble ();
      angstrom_end
          = gparam_value (
                US_Hydrodyn_Fractal_Dimension_Options::paramname (
                    US_Hydrodyn_Fractal_Dimension_Options::ANGSTROM_END))
                .toDouble ();
      angstrom_steps
          = gparam_value (
                US_Hydrodyn_Fractal_Dimension_Options::paramname (
      ...
      enright_ca_pct_end
          = gparam_value (
                US_Hydrodyn_Fractal_Dimension_Options::paramname (
                    US_Hydrodyn_Fractal_Dimension_Options::ENRIGHT_CA_PCT_END))
                .toDouble ();
      roll_sphere_start
          = gparam_value (
                US_Hydrodyn_Fractal_Dimension_Options::paramname (
                    US_Hydrodyn_Fractal_Dimension_Options::ROLL_SPHERE_START))
                .toDouble ();
      roll_sphere_end
          = gparam_value (
                US_Hydrodyn_Fractal_Dimension_Options::paramname (
                    US_Hydrodyn_Fractal_Dimension_Options::ROLL_SPHERE_END))
                .toDouble ();
      roll_sphere_steps
          = gparam_value (
                US_Hydrodyn_Fractal_Dimension_Options::paramname (
                    US_Hydrodyn_Fractal_Dimension_Options::ROLL_SPHERE_STEPS))
                .toInt ();
      method = (US_Fractal_Dimension::methods)gparam_value (
                   US_Hydrodyn_Fractal_Dimension_Options::paramname (
                       US_Hydrodyn_Fractal_Dimension_Options::METHOD))
                   .toInt ();
      show_plots
          = gparam_value (US_Hydrodyn_Fractal_Dimension_Options::paramname (
      ...
      save_plot_data
          = gparam_value (US_Hydrodyn_Fractal_Dimension_Options::paramname (
                US_Hydrodyn_Fractal_Dimension_Options::SAVE_PLOT_DATA))
            == "true";

      // could add it if we need to
      // show_bead_models =
      //    gparam_value( US_Hydrodyn_Fractal_Dimension_Options::paramname(
      //    US_Hydrodyn_Fractal_Dimension_Options::SHOW_BEAD_MODELS ) ) ==
      //    "true";
    }
  ...
  for (auto const current_model : selected_models)
    {
      ...
      if (from_parameters)
      ...
      else
        {
          editor_msg (
              "darkblue",
              QString (us_tr ("Fractal dimension processing model %1\n"))
                  .arg (current_model + 1));
        }
      ...
      if (method == US_Fractal_Dimension::USFD_ROLL_SPHERE)
        {
          ...
          double stepsize
              = (roll_sphere_end - roll_sphere_start) / roll_sphere_steps;
          if (stepsize <= 0)
            {
              QMessageBox::critical (
                  this,
                  windowTitle ()
                      + us_tr (": Fractal Dimension - rolling sphere"),
                  QString (us_tr ("Invalid computed stepsize  (%1)"))
                      .arg (stepsize));

              return;
            }

          for (double radius = roll_sphere_start; radius <= roll_sphere_end;
               radius += stepsize)
            {
              ...
              editor_msg (
                  "darkblue",
                  QString (us_tr ("Computing ASA for probe radius %1 [%2]"))
                      .arg (radius, 0, 'f', 3)
                      .arg (UNICODE_ANGSTROM));
              progress->reset ();
              qApp->processEvents ();
              int retval;
              {
                ...
                retval
                    = us_hydrodyn_asab1_main (active_atoms, &asa, &results,
                                              false, progress, editor, this);
                    ...
                    case US_HYDRODYN_ASAB1_ERR_MEMORY_ALLOC:
                      {
                        QMessageBox::critical (
                            this,
                            windowTitle () + us_tr (": Fractal Dimension"),
                            us_tr ("US_HYDRODYN_ASAB1 encountered a memory "
                                   "allocation error"));
                        return;
                        break;
                      }
                    default:
                      {
                        QMessageBox::critical (
                            this,
                            windowTitle () + us_tr (": Fractal Dimension"),
                            us_tr ("US_HYDRODYN_ASAB1 encountered an unknown "
                                   "error"));
                        return;
                        break;
                      }
              ...
              if (asa_total <= 0)
                {
                  QMessageBox::critical (
                      ...
                      windowTitle ()
                          + us_tr (": Fractal Dimension - rolling sphere"),
                      QString (
                          us_tr ("ASA total %1 for radius %2 [%3] invalid"))
                          .arg (asa_total)
                          .arg (radius)
                          .arg (UNICODE_ANGSTROM));
                  return;
          ...
          // US_Vector::printvector2( "rolling sphere, log ASA, log probe
          // radius", x[0], y[0] ); return;
        }
      else
        {
          // compute ASA

          {
            ...
            int retval;
            {
              ...
              retval = us_hydrodyn_asab1_main (active_atoms, &asa, &results,
                                               false, progress, editor, this);
              asa.probe_radius = save_radius;
            }
                    ...
                    {
                      {
                        ...
                        double saxs_excl_vol;
                        {
                          ...
                          if (!residue_atom_hybrid_map.count (mapkey))
                            {
                              editor_msg (
                                  "red",
                                  QString (us_tr ("Error: Missing hybrid name "
                                                  "for key %1"))
                                      .arg (mapkey));
                              return;
                            }
                          ...
                          if (!atom_map.count (this_atom_name + "~"
                                               + hybrid_name))
                            {
                              editor_msg (
                                  "red",
                        ...
                        // QTextStream( stdout ) << "vdwf: res_idx is " <<
                        // res_idx << " " << endl;
                        ...
                        if (vdwf.count (res_idx))
                          {
                            ...
                            tmp_atom.bead_ref_ionized_mw_delta
                                = this_vdwf.ionized_mw_delta;
                            tmp_atom.mw = this_vdwf.mw;
                            tmp_atom.ionized_mw_delta
                                = this_vdwf.ionized_mw_delta;
                            tmp_atom.bead_mw = this_vdwf.mw;
                            tmp_atom.bead_ionized_mw_delta
                                = this_vdwf.ionized_mw_delta;
                            tmp_atom.bead_color = this_vdwf.color;
                            // #warning recolored for hydration
                            // tmp_atom.bead_color = hydrate && this_vdwf.w ? 1
                            // : 6; #warning recolored for ASA testing
                            // tmp_atom.bead_color                =
                            // this_atom->asa >= asa.hydrate_threshold ? 6 : 8;
                            tmp_atom.bead_recheck_asa = this_atom->asa;
                            tmp_atom.num_elect = this_vdwf.e;
                            tmp_atom.exposed_code = 1;
                            tmp_atom.all_beads.clear ();
                            ...
                            if (this_atom->asa >= asa.hydrate_threshold
                                && this_vdwf.w)
                              {
                                tmp_atom.bead_hydration = this_vdwf.w;
                              }
...

                            {
                              ...
                              tmpsas.mass
                                  = tmp_atom.mw + tmp_atom.ionized_mw_delta;
                          ...
                          {
                            editor_msg ("red", QString ("Residue atom pair %1 "
                                                        "unknown in vdwf.json")
                                                   .arg (res_idx));
                            return;
                          }
                      }
          ...
          if (show_bead_models)
            {
              ...
              QString spt_name = QString ("%1_%2-SAS")
                                     .arg (project)
                                     .arg (current_model + 1);
              spt_name = spt_name.left (30);
              write_bead_spt (use_dir + QDir::separator () + spt_name,
                              &bead_model);
              model_viewer (use_dir + QDir::separator () + spt_name + ".spt",
                            "-script");
            }
          ...
          if (total_atoms && sas.size () != total_atoms)
            {
              editor_msg (
                  ...
                  QString (us_tr ("SAS atoms: %1 (%2%) of total %3 atoms"))
                      .arg (sas.size ())
                      .arg (100.0 * (double)sas.size () / total_atoms, 0, 'f',
                            1)
                      .arg (total_atoms));
            }
        }
...

      {
        ...
        if (method != US_Fractal_Dimension::USFD_ROLL_SPHERE)
          {
            ...
            if (!ufd.compute (method, sas, angstrom_start, angstrom_end,
                              angstrom_steps, enright_ca_pct_start,
                              enright_ca_pct_end, fd, x, y, x_title, y_title,
                              type, errormsg))
              {
                if (!quiet)
                  {
            ...
            if (!quiet)
              {
                QMessageBox::critical (
                    ...
                    QString (
                        us_tr (
                            "Error computing for model %1 : %2 - no points"))
                        .arg (current_model + 1)
                        .arg (errormsg));
              }
            return;
...

        {
          ...
          // quicker if we just check the ends, but this was ensures against
          // out-of-data in US_Fractal_Dimension::compute*()
          for (auto const &v : x)
            {
              for (auto const &vv : v)
                {
...

          {
            ...
            mQwtPlot *plot;
            US_Plot usp_plot (plot, "", x_title, y_title, &dialog);

            US_Plot_Colors upc (plot->canvasBackground ().color ());

            form.addRow (new QLabel (
                QString (us_tr ("Fractal Dimension %1 plot")).arg (type)));
                ...
                for (size_t i = 0; i < x[g].size (); ++i)
                  {
                    QwtPlotCurve *curve = new QwtPlotCurve (
                        QString ("fd_calc-%1").arg (i + 1));
                    curve->setStyle (QwtPlotCurve::Dots);
                    curve->setSamples ((double *)&(x[g][i][0]),
                                       (double *)&(y[g][i][0]),
                                       x[g][i].size ());
                    ...
                    {
                      QwtPlotCurve *fitcurve = new QwtPlotCurve (
                          QString ("fd_fit_%1").arg (i + 1));
            ...
            vector<QString> labels = { us_tr ("Fit start x value :"),
            ...
            vector<double> defaults = { use_xmin, use_xmax, 0 };

            vector<bool> isinput = { true, true, false };
                ...
                if (widgets[i])
                  {
                    ...
                    if (isinput[i])
                    ...
                    else
                      {
                        ...
                        ((QTextEdit *)widgets[i])
                            ->setFixedHeight (11 * m.lineSpacing () + 5);
                      }
                    fields << widgets[i];
                  }
              }
            ...
            QDialogButtonBox *buttonBox
                = new QDialogButtonBox (Qt::Horizontal, &dialog);

            buttonBox->addButton (QDialogButtonBox::Cancel);
            QPushButton *pb_replot
                = new QPushButton (us_tr ("Replot"), &dialog);
            buttonBox->addButton (pb_replot, QDialogButtonBox::ActionRole);

            buttonBox->addButton (us_tr ("Export plot data to CSV"),
                                  QDialogButtonBox::AcceptRole);

            form.addRow (buttonBox);
            QObject::connect (pb_replot, &QPushButton::clicked, [&] () {
              use_xmin = ((QLineEdit *)fields[0])->text ().toDouble ();
              use_xmax = ((QLineEdit *)fields[1])->text ().toDouble ();
              linear_fit (use_xmin, use_xmax, x, y, plot, fitcurves,
                          (QTextEdit *)widgets[2], fd, fd_sd, fd_wtd,
                          fd_wtd_sd, fd_wtd_wtd, fd_wtd_wtd_sd);
            });
            ...
            linear_fit (use_xmin, use_xmax, x, y, plot, fitcurves,
                        (QTextEdit *)widgets[2], fd, fd_sd, fd_wtd, fd_wtd_sd,
                        fd_wtd_wtd, fd_wtd_wtd_sd);
                ...
                if (save_plot_data)
                  {
                    ...
                    map<QString, QwtPlot *> plots = {
                      { QString (
                            "_%1_%2_FD_%3_pr%4_thr%5_start%6_end%7_steps%8")
                            .arg (project)
                            .arg (current_model + 1)
                            .arg (type)
                            .arg (sas_asa_probe_radius)
                    ...
                    QString errors;
                    QString messages;
                    if (!upu.printtofile ("Fractal Dimension " + type, plots,
                                          errors, messages))
                      {
                        QMessageBox::warning (
                            this,
                    ...
                    else
                      {
                        QMessageBox::information (
                            ...
                            windowTitle () + us_tr (": Fractal Dimension"),
                            QString (
                                "Plots saved in directory <i>%1</i><br><br>%2")
                      ...
                      if (!upu.printtofile ("Fractal Dimension " + type, plots,
                      ...
                      else
                        {
                          QMessageBox::information (
                              this,
                              windowTitle () + us_tr (": Fractal Dimension"),
                              QString ("Plots saved in directory "
                                       "<i>%1</i><br><br>%2")
                                  .arg (somo_tmp_dir.replace ("//", "/"))
                                  .arg (messages));
                        }
        ...
        if (from_parameters)
          {
            editor_msg ("black",
                        us_tr (US_Hydrodyn_Fractal_Dimension_Options::options (
                            gparams, use_xmin, use_xmax)));
          }

        double rg_over_fd
            = fd == 0 || fd == -1 ? -1 : model_vector[current_model].Rg / fd;
        double rg_over_fd_sd = fd == 0 || fd == -1 ? -1 : fd_sd / fd;

        double rg_over_fd_wtd = fd_wtd == 0 || fd_wtd == -1
                                    ? -1
                                    : model_vector[current_model].Rg / fd_wtd;
        double rg_over_fd_wtd_sd
            = fd_wtd == 0 || fd_wtd == -1 ? -1 : fd_wtd_sd / fd_wtd;

        double rg_over_fd_wtd_wtd
            = fd_wtd_wtd == 0 || fd_wtd_wtd == -1
                  ? -1
                  : model_vector[current_model].Rg / fd_wtd_wtd;
        double rg_over_fd_wtd_wtd_sd = fd_wtd_wtd == 0 || fd_wtd_wtd == -1
                                           ? -1
                                           : fd_wtd_wtd_sd / fd_wtd_wtd;

        editor_msg (
            "black",
            QString (us_tr ("Fractal dimension for model %1 : %2 %3 %4\n")
                         .arg (current_model + 1)
                         .arg (fd, 0, 'f', 3)
                         .arg (UNICODE_PLUSMINUS)
                         .arg (fd_sd, 0, 'f', 3)));
        if (rg_over_fd != -1 && advanced_config.expert_mode)
          {
            editor_msg ("black", QString (us_tr ("Rg / Fractal dimension for "
                                                 "model %1.: %2 %3 %4 [%5]\n")
                                              .arg (current_model + 1)
        ...
        if (fd_wtd_wtd == -1)
          {
            ...
            if (rg_over_fd_wtd != -1 && advanced_config.expert_mode)
              {
                editor_msg ("black",
                            QString (us_tr ("Rg / Fractal dimension for model "
                                            "%1 wtd.: %2 %3 %4 [%5]\n")
                                         .arg (current_model + 1)
          ...
          {
            editor_msg ("black", QString (us_tr ("Fractal dimension for model "
                                                 "%1 avg. of wtd.: %2 %3 %4\n")
                                              .arg (current_model + 1)
                                              .arg (fd_wtd, 0, 'f', 3)
                                              .arg (UNICODE_PLUSMINUS)
            ...
            editor_msg ("black",
                        QString (us_tr ("Fractal dimension for model %1 wtd. "
                                        "of wtd.: %2 %3 %4\n")
                                     .arg (current_model + 1)
            ...
            if (rg_over_fd_wtd_wtd != -1 && advanced_config.expert_mode)
              {
                editor_msg (
                    "black",
                    QString (us_tr ("Rg / Fractal dimension for model %1 wtd. "
                                    "of wtd.: %2 %3 %4 [%5]\n")
                                 .arg (current_model + 1)
        ...
        if (from_parameters)
          {
            model_vector[current_model].fractal_dimension_parameters
                = US_Hydrodyn_Fractal_Dimension_Options::options (
                    gparams, use_xmin, use_xmax);
            ...
            model_vector[current_model].rg_over_fractal_dimension_wtd_wtd
                = rg_over_fd_wtd_wtd;
            model_vector[current_model].rg_over_fractal_dimension_wtd_wtd_sd
                = rg_over_fd_wtd_wtd_sd;

            model_vector_as_loaded[current_model].fractal_dimension_parameters
                = US_Hydrodyn_Fractal_Dimension_Options::options (
                    gparams, use_xmin, use_xmax);
            model_vector_as_loaded[current_model].fractal_dimension = fd;
            model_vector_as_loaded[current_model].fractal_dimension_sd = fd_sd;
            model_vector_as_loaded[current_model].fractal_dimension_wtd
                = fd_wtd;
            model_vector_as_loaded[current_model].fractal_dimension_wtd_sd
                = fd_wtd_sd;
            model_vector_as_loaded[current_model].fractal_dimension_wtd_wtd
                = fd_wtd_wtd;
            model_vector_as_loaded[current_model].fractal_dimension_wtd_wtd_sd
                = fd_wtd_wtd_sd;
            model_vector_as_loaded[current_model].rg_over_fractal_dimension
                = rg_over_fd;
            model_vector_as_loaded[current_model].rg_over_fractal_dimension_sd
                = rg_over_fd_sd;
            model_vector_as_loaded[current_model].rg_over_fractal_dimension_wtd
                = rg_over_fd_wtd;
            ...
            if (fd_save_info)
              {
                ...
                fd_save_info->data.results.name
                    = QString ("%1_%2").arg (project).arg (current_model + 1);
                fd_save_info->data.fractal_dimension_parameters
                    = US_Hydrodyn_Fractal_Dimension_Options::options (
                        gparams, use_xmin, use_xmax);
                ...
                fd_save_info->data.fractal_dimension_wtd_wtd_sd
                    = fd_wtd_wtd_sd;
                fd_save_info->data.rg_over_fractal_dimension = rg_over_fd;
                fd_save_info->data.rg_over_fractal_dimension_sd
                    = rg_over_fd_sd;
                fd_save_info->data.rg_over_fractal_dimension_wtd
                    = rg_over_fd_wtd;
                fd_save_info->data.rg_over_fractal_dimension_wtd_sd
                    = rg_over_fd_wtd_sd;
                fd_save_info->data.rg_over_fractal_dimension_wtd_wtd
                    = rg_over_fd_wtd_wtd;
                fd_save_info->data.rg_over_fractal_dimension_wtd_wtd_sd
                    = rg_over_fd_wtd_wtd_sd;
...
void
US_Hydrodyn::show_fractal_dimension_options ()
{
  ...
  else
    {
      fractal_dimension_options_window
          = new US_Hydrodyn_Fractal_Dimension_Options (
              &gparams, &fractal_dimension_options_widget, this);
