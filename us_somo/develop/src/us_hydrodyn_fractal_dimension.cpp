...
static bool linear_fit(const double &xmin, const double &xmax,
                       const vector<vector<vector<double> > > x,
                       const vector<vector<vector<double> > > y, mQwtPlot *plot,
                       vector<vector<QwtPlotCurve *> > &fitcurves,
                       QTextEdit *msgbox, double &fd, double &fd_sd,
                       double &fd_wtd, double &fd_wtd_sd, double &fd_wtd_wtd,
  ...
  qDebug()
      << QString("linear_fit(); xmin is %1, xmax is %2").arg(xmin).arg(xmax);
  ...
  for (size_t g = 0; g < x.size(); ++g) {
    ...
    for (size_t i = 0; i < x[g].size(); ++i) {
      ...
      if (fitcurves[g][i]) {
        ...
        fitcurves[g][i]->setSamples((double *)&(fitx[0]), (double *)&(fity[0]),
                                    2);
      }
    }
    {
  ...
  if (global_no_weights_a_v.size() > 1 && global_count) {
    ...
    if (!US_Average::compute(US_Average::ONE_OVER_SD_SQ, global_weighted_b_v,
                             global_weighted_sigb_v, b_avg_weighted,
                             b_sd_weighted, errors)) {
      errorsfull += errors;
    }
    ...
    msgbox->setText(msgbox->toPlainText() +
...
void US_Hydrodyn::fractal_dimension(bool from_parameters,
  ...
  if (!selected_models.size()) {
    if (!quiet) {
      QMessageBox::critical(this, windowTitle() + us_tr(": Fractal Dimension"),
  ...
  if (!from_parameters) {
    ...
    for (size_t j = 0; j < model_vector[current_model].molecule.size(); ++j) {
      for (size_t k = 0;
        ...
        if (!this_atom.p_residue) {
          editor_msg("red", QString(us_tr("Internal error: p_residue not set!, "
        ...
        if (vdwf.count(res_idx)) {
          ...
          editor_msg("red", QString("Residue atom pair %1 unknown in vdwf.json")
                                .arg(res_idx));
          return;
        }
      }
    ...
    if ("empty model" != usu.nsa_physical_stats()) {
      ...
      extents.axis[0] =
          usu.nsa_physical_stats_map["result radial extent bounding box size x"]
              .toDouble() *
          fconv;
      extents.axis[1] =
          usu.nsa_physical_stats_map["result radial extent bounding box size y"]
              .toDouble() *
          fconv;
      extents.axis[2] =
          usu.nsa_physical_stats_map["result radial extent bounding box size z"]
              .toDouble() *
          fconv;
  ...
  if (!from_parameters) {
    ...
    do {
      ...
      dialog.setWindowTitle(windowTitle() +
                            us_tr(": Fractal Dimension : Set parameters"));
      ...
      form.addRow(new QLabel(
          QString(us_tr("Set the box counting parameters<br>"
      ...
      cmb_method->addItem(US_Fractal_Dimension::method_name(
                          ...
                          US_Fractal_Dimension::USFD_BOX_MODEL);
      cmb_method->addItem(
          US_Fractal_Dimension::method_name(US_Fractal_Dimension::USFD_BOX_ALT),
          US_Fractal_Dimension::USFD_BOX_ALT);
      cmb_method->addItem(US_Fractal_Dimension::method_name(
                              US_Fractal_Dimension::USFD_BOX_MASS),
                          US_Fractal_Dimension::USFD_BOX_MASS);
      cmb_method->addItem(
          US_Fractal_Dimension::method_name(US_Fractal_Dimension::USFD_ENRIGHT),
      ...
      if (dialog.exec() == QDialog::Accepted) {
        if (((QLineEdit *)fields[0])->text().toDouble() < 0 ||
          ...
          method =
              (US_Fractal_Dimension::methods)cmb_method->currentData().toInt();
    ...
    enright_ca_pct_start =
        gparam_value(
            US_Hydrodyn_Fractal_Dimension_Options::paramname(
                US_Hydrodyn_Fractal_Dimension_Options::ENRIGHT_CA_PCT_START))
            .toDouble();
    enright_ca_pct_end =
        gparam_value(
            US_Hydrodyn_Fractal_Dimension_Options::paramname(
    ...
    method = (US_Fractal_Dimension::methods)gparam_value(
                 ...
                 .toInt();
    show_plots = gparam_value(US_Hydrodyn_Fractal_Dimension_Options::paramname(
                     US_Hydrodyn_Fractal_Dimension_Options::PLOTS)) == "true";
    save_plot_data =
        gparam_value(US_Hydrodyn_Fractal_Dimension_Options::paramname(
            US_Hydrodyn_Fractal_Dimension_Options::SAVE_PLOT_DATA)) == "true";

    // could add it if we need to
    // show_bead_models =
    //    gparam_value( US_Hydrodyn_Fractal_Dimension_Options::paramname(
    //    US_Hydrodyn_Fractal_Dimension_Options::SHOW_BEAD_MODELS ) ) == "true";
  }
        ...
        method == US_Fractal_Dimension::USFD_ROLL_SPHERE
            ...
            : QMessageBox::Yes ==
                  QMessageBox::question(
                      ...
                      us_tr("Display bead models of SAS ?"), QMessageBox::Yes,
  ...
  for (auto const current_model : selected_models) {
    ...
    if (method == US_Fractal_Dimension::USFD_ROLL_SPHERE) {
      ...
      if (stepsize <= 0) {
        QMessageBox::critical(
            this, windowTitle() + us_tr(": Fractal Dimension - rolling sphere"),
        ...
        if (retval) {
          ...
          switch (retval) {
            case US_HYDRODYN_ASAB1_ERR_MEMORY_ALLOC: {
              QMessageBox::critical(
                  this, windowTitle() + us_tr(": Fractal Dimension"),
                  us_tr("US_HYDRODYN_ASAB1 encountered a memory allocation "
                        "error"));
              return;
              break;
            }
      ...
      // US_Vector::printvector2( "rolling sphere, log ASA, log probe radius",
      // x[0], y[0] ); return;
    } else {
      // compute ASA
      ...
      for (unsigned int j = 0; j < model_vector[current_model].molecule.size();
        ...
        for (unsigned int k = 0;
          ...
          if (!this_atom->p_residue) {
            editor_msg("red", QString(us_tr("Internal error: p_residue not "
                                            "set!, contact the developers")));
            return;
          }
          if (!this_atom->p_atom) {
            editor_msg("red", QString(us_tr("Internal error: p_atom not set!, "
                                            "contact the developers")));
            return;
          }
      ...
      if (show_bead_models) {
        ...
        QString spt_name =
            QString("%1_%2-SAS").arg(project).arg(current_model + 1);
...

    {
      ...
      if (method != US_Fractal_Dimension::USFD_ROLL_SPHERE) {
        if (!ufd.compute(method, sas, angstrom_start, angstrom_end,
                         ...
                         enright_ca_pct_end, fd, x, y, x_title, y_title, type,
                         errormsg)) {
          if (!quiet) {
            QMessageBox::critical(
                this, windowTitle() + us_tr(": Fractal Dimension"),
...

      {
...

        {
          ...
          for (size_t g = 0; g < x.size(); ++g) {
            ...
            for (size_t i = 0; i < x[g].size(); ++i) {
              QwtPlotCurve *curve =
                  new QwtPlotCurve(QString("fd_calc-%1").arg(i + 1));
              curve->setStyle(QwtPlotCurve::Dots);
              curve->setSamples((double *)&(x[g][i][0]),
                                (double *)&(y[g][i][0]), x[g][i].size());
...

              {
                QwtPlotCurve *fitcurve =
                    new QwtPlotCurve(QString("fd_fit_%1").arg(i + 1));
          ...
          if (!show_plots) {
            if (save_plot_data) {
              ...
              QString errors;
              QString messages;
              if (!upu.printtofile("Fractal Dimension " + type, plots, errors,
                                   messages)) {
                QMessageBox::warning(
                    this, windowTitle() + us_tr(": Fractal Dimension"),
      ...
      double rg_over_fd_wtd_wtd_sd =
          fd_wtd_wtd == 0 || fd_wtd_wtd == -1 ? -1 : fd_wtd_wtd_sd / fd_wtd_wtd;
      ...
      if (fd_wtd_wtd == -1) {
        editor_msg(
            "black",
            QString(us_tr("Fractal dimension for model %1 wtd.: %2 %3 %4\n")
                        .arg(current_model + 1)
                        .arg(fd_wtd, 0, 'f', 3)
                        .arg(UNICODE_PLUSMINUS)
                        .arg(fd_wtd_sd, 0, 'f', 3)));
        if (rg_over_fd_wtd != -1 && advanced_config.expert_mode) {
          editor_msg("black", QString(us_tr("Rg / Fractal dimension for model "
                                            "%1 wtd.: %2 %3 %4 [%5]\n")
                                          .arg(current_model + 1)
      ...
      if (from_parameters) {
        model_vector[current_model].fractal_dimension_parameters =
            US_Hydrodyn_Fractal_Dimension_Options::options(gparams, use_xmin,
                                                           use_xmax);
        ...
        model_vector_as_loaded[current_model].rg_over_fractal_dimension_wtd =
            rg_over_fd_wtd;
        model_vector_as_loaded[current_model].rg_over_fractal_dimension_wtd_sd =
            rg_over_fd_wtd_sd;
        model_vector_as_loaded[current_model]
            .rg_over_fractal_dimension_wtd_wtd = rg_over_fd_wtd_wtd;
        model_vector_as_loaded[current_model]
            .rg_over_fractal_dimension_wtd_wtd_sd = rg_over_fd_wtd_wtd_sd;

        qDebug() << QString(
                        "US_Hydrodyn::fractal_dimension() set model_vector[ %1 "
                        "], FD %2 +/- %3 ; %4\n")
                        .arg(current_model)
