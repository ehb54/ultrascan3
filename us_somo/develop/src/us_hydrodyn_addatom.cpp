  ...
  if (f.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    for (unsigned int i = 0; i < atom_list.size(); i++) {
      ts << atom_list[i].name.toUpper() << "\t"
         << atom_list[i].hybrid.name.toUpper() << "\t" << atom_list[i].hybrid.mw
         << "\t" << atom_list[i].hybrid.radius << "\t"
...
void US_AddAtom::update_name(const QString &str) { current_atom.name = str; }
