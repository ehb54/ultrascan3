...
US_AddAtom::US_AddAtom (bool *widget_flag, QWidget *p, const char *)
    : QWidget (p)
{
...
US_AddAtom::setupGUI ()
{
  ...
  pb_select_saxs_file
      = new QPushButton (us_tr ("Load SAXS Coefficient File"), this);
  ...
  cmb_atom = new QComboBox (this);
  cmb_atom->setObjectName ("Atom Listing");
  cmb_atom->setPalette (PALET_NORMAL);
  AUTFBACK (cmb_atom);
  cmb_atom->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize));
  ...
  cmb_hybrid = new QComboBox (this);
  cmb_hybrid->setObjectName ("Hybrid Listing");
  cmb_hybrid->setPalette (PALET_NORMAL);
  AUTFBACK (cmb_hybrid);
  cmb_hybrid->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize));
  //   cmb_hybrid->setSizeLimit(5);
  cmb_hybrid->setMinimumHeight (minHeight1);
  connect (cmb_hybrid, SIGNAL (activated (int)), this,
           SLOT (select_hybrid (int)));
  ...
  AUTFBACK (lbl_mw1);
  lbl_mw1->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize - 1, QFont::Bold));
  ...
  AUTFBACK (lbl_mw2);
  lbl_mw2->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize));
  ...
  lbl_saxs_name1->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize - 1,
                                  QFont::Bold));
  ...
  lbl_saxs_name2->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize));
  ...
  lbl_radius1->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1,
                               QFont::Bold));
  ...
  lbl_radius2->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  ...
  lbl_hybrid1->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1,
                               QFont::Bold));
  ...
  lbl_hybrid2->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));

  lbl_saxs_excl_vol1
      = new QLabel (us_tr (" Excluded Vol. for SAXS Atom:"), this);
  ...
  lbl_saxs_excl_vol1->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize - 1,
                                      QFont::Bold));

  lbl_saxs_excl_vol2
      = new QLabel (us_tr (" Enter Excluded Volume (A^3):"), this);
  ...
  lbl_saxs_excl_vol2->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize - 1,
                                      QFont::Bold));

  lbl_number_of_atoms
      = new QLabel (us_tr (" Number of Atoms in File: 0"), this);
  ...
  lbl_number_of_atoms->setFont (QFont (USglobal->config_list.fontFamily,
                                       USglobal->config_list.fontSize - 1,
                                       QFont::Bold));

  lbl_number_of_hybrids
      = new QLabel (us_tr (" Number of Hybridizations in File: 0"), this);
  ...
  lbl_number_of_hybrids->setFont (QFont (USglobal->config_list.fontFamily,
                                         USglobal->config_list.fontSize - 1,
                                         QFont::Bold));
  ...
  AUTFBACK (lbl_name);
  lbl_name->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize - 1, QFont::Bold));

  le_name = new QLineEdit (this);
  le_name->setObjectName ("Atom name Line Edit");
  le_name->setPalette (PALET_NORMAL);
  AUTFBACK (le_name);
  le_name->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize));
  le_name->setMinimumHeight (minHeight1);
  connect (le_name, SIGNAL (textChanged (const QString &)),
           SLOT (update_name (const QString &)));

  le_excl_vol = new QLineEdit (this);
  le_excl_vol->setObjectName ("SAXS excluded volume Line Edit");
  le_excl_vol->setPalette (PALET_NORMAL);
  AUTFBACK (le_excl_vol);
  le_excl_vol->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  le_excl_vol->setMinimumHeight (minHeight1);
  le_excl_vol->setEnabled (false);
  connect (le_excl_vol, SIGNAL (textChanged (const QString &)),
           SLOT (update_excl_vol (const QString &)));
  ...
  cb_excl_vol = new QCheckBox (this);
  cb_excl_vol->setObjectName ("excluded volume checkbox");
  cb_excl_vol->setText (us_tr (" Use average value (from SAXS table) "));
  cb_excl_vol->setChecked (true);
  cb_excl_vol->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
...
{
  int item = -1;
  for (int i = 0; i < (int)atom_list.size ();
       i++) // check if the same atom was defined previously
    {
      if (atom_list[i].name.toUpper () == current_atom.name.toUpper ()
          && atom_list[i].hybrid.name.toUpper ()
                 == current_atom.hybrid.name.toUpper ())
        {
          item = i;
...
public:
  atom our_atom;
  bool
  operator< (const sortable_atom &objIn) const
  {
    QString atom_cmp
        = our_atom.name.toUpper () + "\t" + our_atom.hybrid.name.toUpper ();
    QString obj_cmp = objIn.our_atom.name.toUpper () + "\t"
                      + objIn.our_atom.hybrid.name.toUpper ();
    if (atom_cmp < obj_cmp)
      {
        return (true);
      }
...
US_AddAtom::write_atom_file ()
{
  ...
  if (f.open (QIODevice::WriteOnly | QIODevice::Text))
    {
      cmb_atom->clear ();
      str1 = QString (us_tr (" Number of Atoms in File: %1"))
                 .arg (atom_list.size ());
      QTextStream ts (&f);
      for (unsigned int i = 0; i < atom_list.size (); i++)
        {
          ts << atom_list[i].name.toUpper () << "\t"
             << atom_list[i].hybrid.name.toUpper () << "\t"
...
US_AddAtom::select_atom_file ()
{
          ...
          while (!ts.atEnd ())
            {
              ...
              if (!current_atom.name.isEmpty ()
                  && current_atom.hybrid.radius > 0.0
  ...
  if (!atom_filename.isEmpty () && !hybrid_filename.isEmpty ()
      && !saxs_filename.isEmpty ())
    {
      select_atom (0);
    }
}
...
US_AddAtom::select_hybrid_file ()
{
          ...
          while (!ts.atEnd ())
            {
              ...
              str2 = ts.readLine (); // read rest of line
              if (!current_hybrid.name.isEmpty ()
                  && current_hybrid.radius > 0.0 && current_hybrid.mw > 0.0)
                {
  ...
  str1 = QString (us_tr (" Number of Hybridizations in File: %1"))
             .arg (hybrid_list.size ());
          ...
          while (!ts.atEnd ())
            {
              ...
              cout << QString ("Warning: %1 on line %2, invalid number of "
                               "tokens, ignoring")
                          .arg (saxs_filename)
                          .arg (line);
            }
...
void
US_AddAtom::select_hybrid (int i)
{
  if (saxs_list.size () == 0)
    return;
  QString str;
...
void
US_AddAtom::update_name (const QString &str)
{
  current_atom.name = str;
}

void
US_AddAtom::update_excl_vol (const QString &str)
{
  current_atom.saxs_excl_vol = str.toFloat ();
}

void
US_AddAtom::update_hybridization_name (const QString &str)
{
  current_atom.hybrid.name = str;
}

void
US_AddAtom::select_atom (int val)
{
  QString str;
  unsigned int i, j;
...
void
US_AddAtom::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 30;
  global_Ypos -= 30;
...
void
US_AddAtom::delete_atom ()
{
  vector<struct atom>::iterator it;
  for (it = atom_list.begin (); it != atom_list.end (); it++)
    {
      if ((*it).name == current_atom.name
          && (*it).hybrid.name == current_atom.hybrid.name)
        {
          atom_list.erase (it);
          break;
        }
