...
void
US_Hydrodyn_Saxs_Buffer_Conc_Load::setupGUI ()
{
  int minHeight1 = 30;
...
void
US_Hydrodyn_Saxs_Buffer_Conc_Load::cancel ()
{
  close ();
}

void
US_Hydrodyn_Saxs_Buffer_Conc_Load::set_ok ()
{
  // set *csv1
  trial ();
  *csv1 = trial_csv;
...
void
US_Hydrodyn_Saxs_Buffer_Conc_Load::help ()
{
  US_Help *online_help;
  online_help = new US_Help (this);
  online_help->show_help ("manual/somo/somo_saxs_buffer_conc_load.html");
}

void
US_Hydrodyn_Saxs_Buffer_Conc_Load::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept ();
...
csv
US_Hydrodyn_Saxs_Buffer_Conc_Load::current_csv ()
{
...
void
US_Hydrodyn_Saxs_Buffer_Conc_Load::update_enables ()
{
  if (!disable_updates)
    {
...
void
US_Hydrodyn_Saxs_Buffer_Conc_Load::del_row ()
{
...
void
US_Hydrodyn_Saxs_Buffer_Conc_Load::set_name ()
{
  for (int i = 0; i < t_csv->columnCount (); i++)
    {
      if (t_csv->item (0, i)->isSelected ())
...
void
US_Hydrodyn_Saxs_Buffer_Conc_Load::set_conc ()
{
  for (int i = 0; i < t_csv->columnCount (); i++)
    {
      if (t_csv->item (0, i)->isSelected ())
...
QString
US_Hydrodyn_Saxs_Buffer_Conc_Load::csv_to_qstring (csv from_csv)
{
  QString qs;
...
void
US_Hydrodyn_Saxs_Buffer_Conc_Load::reload_csv ()
{
  // cout << csv_to_qstring( disp_csv );
...
void
US_Hydrodyn_Saxs_Buffer_Conc_Load::trial ()
{
...
void
US_Hydrodyn_Saxs_Buffer_Conc_Load::adjust ()
{
  // ask & add or subtract
  bool ok;
  int res = US_Static::getInteger (
      us_tr (
          "US-SOMO: SAXS Buffer: File Concentration Loader: Adjust timecode"),
...
void
US_Hydrodyn_Saxs_Buffer_Conc_Load::col_header_released (int col)
{
  t_csv->clearSelection ();
