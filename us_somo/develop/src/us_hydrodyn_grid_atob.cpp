...
AtoB (PDB *pdb, long npoints_x, long npoints_y, long npoints_z, float dx,
      ...
      const char resnam[], US_Hydrodyn *us_hydrodyn, bool create_nmr,
      QString &fn, bool &nmr_created)
{
#if defined(DEBUG)
  //   printf("npoints %ld\n", npoints);
  puts ("in atob 0");
  ...
  for (i = 0; i < npoints_x; i++) /* clears up r0 */
    {
      us_hydrodyn->lbl_core_progress->setText (
          QString ("Gridding %1 of %2").arg (i + 1).arg (npoints_x));
      qApp->processEvents ();
  ...
  if (create_nmr)
    {
      ...
      if (!fn_out.open (QIODevice::WriteOnly))
        {
          QMessageBox::warning (
              us_hydrodyn, "US-SOMO: A2B: Create mulitple model output file",
      ...
      else
        {
          ...
          for (n = 1, i = 0; i < npoints_x; i++)
            {
              us_hydrodyn->lbl_core_progress->setText (
                  QString ("Creating NMR style output file %1 of %2")
                      .arg (i)
                      .arg (npoints_x));
              qApp->processEvents ();
              for (j = 0; j < npoints_y; j++)
...
vector<PDB_atom>
us_hydrodyn_grid_atob (vector<PDB_atom> *bead_model,
                       ...
                       US_Hydrodyn *use_us_hydrodyn)
{
      ...
      if ((*bead_model)[i].active)
        {
          ...
          tmp_prop.mass = (*bead_model)[i].bead_mw
                          + (*bead_model)[i].bead_ionized_mw_delta;
  ...
  if (use_grid_options->create_nmr_bead_pdb)
    {
      fn = us_hydrodyn->somo_dir + QDir::separator () + "structures"
           + QDir::separator () + us_hydrodyn->project
      ...
      //                                          to save the multiple model
      //                                          output file" );

      if (QFile::exists (fn) && !((US_Hydrodyn *)us_hydrodyn)->overwrite)
        {
          fn = ((US_Hydrodyn *)us_hydrodyn)
                   ->fileNameCheck (fn, 0, us_hydrodyn);
        }
    }

  PDB *result_pdb = AtoB (
      (PDB *)&pdb[0], npoints_x, npoints_y, npoints_z,
      (float)use_grid_options->cube_side, (PHYSPROP *)&prop[0],
      (PHYSPROP *)&nprop[0], &natoms3, use_grid_options->center, "tmp_atob",
      use_grid_options->tangency ? 1 : 0, "PB", "RES", us_hydrodyn,
      ...
      if (!us_hydrodyn->compute_structure_factors (fn, error_msg))
        {
          ...
          us_hydrodyn->editor->verticalScrollBar ()->setValue (
  ...
  if (use_grid_options->equalize_radii_constant_volume)
    {
      us_hydrodyn->editor_msg (
          "red", QString ("Equalizing bead model radii requested, but it is "
                          "currently DISABLED"));
    }
  ...
  //    total volume %1\n" ).arg( tot_vol ) ); double pi43           = M_PI *
  //    4e0 / 3e0; float radius = (float)pow( tot_vol / ( ( double )
  //    result_bead_model.size() * pi43 ), 1e0 / 3e0 ); for ( unsigned int i =
  //    0; i < ( unsigned int ) result_bead_model.size(); i++ )
  //    {
...
bool
US_Hydrodyn::compute_structure_factors (QString filename, QString &error_msg)
{
  error_msg = "";
  if (!QFileInfo (filename).exists ())
    {
      error_msg
          = QString (
                "Error: compute_structure_factors(): file %1 does not exist")
                .arg (filename);
      return false;
    }
  if (!QFileInfo (filename).isReadable ())
    {
      error_msg
          = QString (
                "Error: compute_structure_factors(): file %1 is not readable")
                .arg (filename);
      return false;
    }
  ...
  if (!saxs_options.bead_models_rho0_in_scat_factors)
    {
      ...
      if (save_e_density)
      ...
      else
        {
          editor_msg (
              "dark blue",
              "Water electron denisty is already zero for Iq computation");
        }
    }
  batch_window->show ();
  ...
  if (!saxs_options.bead_models_rho0_in_scat_factors)
    {
      editor_msg ("dark blue",
                  QString ("Restored water electron denisty to %1")
                      .arg (save_e_density));
    }
  ...
  if (batch_window->stopFlag)
    {
      error_msg
          = "Error: compute_structure_factors(): batch computation terminated";
      return false;
    }

  cout << "batch finished\n";
  // convert csv average to .dat and "compute structure factors"
  QString csvfile = us_hydrodyn->somo_dir + QDir::separator () + "saxs"
                    + QDir::separator ()
                    + QFileInfo (filename).completeBaseName () + csv_addendum
                    + "_iqq" + batch_window->iqq_suffix () + ".csv";
  if (!QFileInfo (csvfile).exists ())
    {
      error_msg = QString ("Error: compute_structure_factors(): batch "
                           "computed csv output file %1 does not exist")
                      .arg (csvfile);
      return false;
    }
  if (!QFileInfo (csvfile).isReadable ())
    {
      error_msg = QString ("Error: compute_structure_factors(): batch "
                           "computed csv output file %1 is not readable")
                      .arg (csvfile);
      return false;
    }
  QFile f (csvfile);
  if (!f.open (QIODevice::ReadOnly))
    {
      error_msg = QString ("Error: compute_structure_factors(): batch "
                           "computed csv output file %1: can not open")
                      .arg (csvfile);
      return false;
    }
  ...
  if (qslq.size () < 3 || qslq[0] != "\"Name\"" || qslq[1] != "\"Type; q:\"")
    {
      error_msg
          = QString ("Error: compute_structure_factors(): batch computed csv "
                     "output file %1: format error in q line")
                .arg (csvfile);
      return false;
    }
  ...
  if (qslIs.size () < 2)
    {
      error_msg
          = QString ("Error: compute_structure_factors(): batch computed csv "
                     "output file %1: insufficinet Iq lines (only found %2)")
                .arg (csvfile)
                .arg (qslIs.size ());
      return false;
    }
  ...
  for (unsigned int i = 0; i < (unsigned int)qslIs.size (); i++)
    {
      ...
      if (qslI.size () < 3 || qslI[1] != "\"I(q)\"")
        {
          error_msg
              = QString ("Error: compute_structure_factors(): batch computed "
                         "csv output file %1: format error in Iq line %2")
                    .arg (csvfile)
                    .arg (i + 1);
          return false;
        }
      ...
      if (qslq.size () != qslI.size ())
        {
          error_msg = QString ("Error: compute_structure_factors(): batch "
                               "computed csv output file %1: format error (I "
                               "& q lines incompatible) Iq line %2")
                          .arg (csvfile)
                          .arg (i + 1);
          return false;
        }
  ...
  editor_msg ("blue",
              us_tr ("Computing 4 & 5 term exponentials\nThis can take awhile "
                     "& the program may seem unresponsive"));
  ...
  for (unsigned int j = 0; j < (unsigned int)Is.size (); j++)
    {
      editor_msg (
          "blue",
          QString (us_tr ("Computing 4 & 5 term exponentials for bead %1"))
              .arg (j == last_bead ? "Global average"
                                   : QString ("%1").arg (j + 1)));
      editor->verticalScrollBar ()->setValue (
          editor->verticalScrollBar ()->maximum ());
      qApp->processEvents ();
      ...
      if (!usu.compute_exponential (
              q, Is[j], coeff4, coeff5, coeffv, norm4, norm5, normv,
      ...
      editor_msg (
          "dark blue",
          QString ("structure factors name: %1\n").arg (sf_factors.saxs_name));
