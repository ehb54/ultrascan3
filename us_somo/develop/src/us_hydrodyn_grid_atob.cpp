...
static PDB *AtoB(PDB *pdb, long npoints_x, long npoints_y, long npoints_z,
                 ...
                 US_Hydrodyn *us_hydrodyn, bool create_nmr, QString &fn,
                 bool &nmr_created) {
#if defined(DEBUG)
  //   printf("npoints %ld\n", npoints);
  puts("in atob 0");
  ...
  *natoms3 = natoms2;
  ...
  if (create_nmr) {
    ...
    if (!fn_out.open(QIODevice::WriteOnly)) {
      QMessageBox::warning(us_hydrodyn,
                           "US-SOMO: A2B: Create mulitple model output file",
...
vector<PDB_atom> us_hydrodyn_grid_atob(vector<PDB_atom> *bead_model,
  ...
  for (unsigned int i = 0; i < bead_model->size(); i++) {
    if ((*bead_model)[i].active) {
      ...
      tmp_prop.mass =
          (*bead_model)[i].bead_mw + (*bead_model)[i].bead_ionized_mw_delta;
  ...
  if (use_grid_options->create_nmr_bead_pdb) {
    ...
    //                                          us_hydrodyn,
    //                                          "save the models",
    //                                          "US_SOMO: A2B: Choose a name to
    //                                          save the multiple model output
    //                                          file" );
  ...
  PDB *result_pdb =
      AtoB((PDB *)&pdb[0], npoints_x, npoints_y, npoints_z,
           ...
           us_hydrodyn, use_grid_options->create_nmr_bead_pdb, fn, nmr_created);
  ...
  if (use_grid_options->equalize_radii_constant_volume) {
    us_hydrodyn->editor_msg("red",
                            QString("Equalizing bead model radii requested, "
...
bool US_Hydrodyn::compute_structure_factors(QString filename,
  ...
  if (!QFileInfo(filename).exists()) {
    error_msg =
        QString("Error: compute_structure_factors(): file %1 does not exist")
            .arg(filename);
    return false;
  }
  if (!QFileInfo(filename).isReadable()) {
    error_msg =
        QString("Error: compute_structure_factors(): file %1 is not readable")
            .arg(filename);
    return false;
  }
  ...
  if (!saxs_options.bead_models_rho0_in_scat_factors) {
    ...
    if (save_e_density) {
      editor_msg("dark blue",
                 "Temporarily setting water electron denisty to zero for Iq "
                 "computation");
    } else {
      editor_msg("dark blue",
  ...
  if (!saxs_options.bead_models_rho0_in_scat_factors) {
    editor_msg(
        "dark blue",
        QString("Restored water electron denisty to %1").arg(save_e_density));
  }
  ...
  QString csvfile = us_hydrodyn->somo_dir + QDir::separator() + "saxs" +
                    QDir::separator() + QFileInfo(filename).completeBaseName() +
                    csv_addendum + "_iqq" + batch_window->iqq_suffix() + ".csv";
  if (!QFileInfo(csvfile).exists()) {
    error_msg = QString(
                    "Error: compute_structure_factors(): batch computed csv "
  ...
  for (unsigned int i = 0; i < (unsigned int)qslIs.size(); i++) {
    ...
    if (qslI.size() < 3 || qslI[1] != "\"I(q)\"") {
      error_msg = QString(
                      "Error: compute_structure_factors(): batch computed csv "
                      "output file %1: format error in Iq line %2")
                      .arg(csvfile)
                      .arg(i + 1);
      return false;
    }
    ...
    if (qslq.size() != qslI.size()) {
      error_msg =
          QString(
              "Error: compute_structure_factors(): batch computed csv output "
              "file %1: format error (I & q lines incompatible) Iq line %2")
              .arg(csvfile)
              .arg(i + 1);
      return false;
    }
  ...
  editor_msg("blue", us_tr("Computing 4 & 5 term exponentials\nThis can take "
                           "awhile & the program may seem unresponsive"));
  ...
  for (unsigned int j = 0; j < (unsigned int)Is.size(); j++) {
    editor_msg(
        ...
        QString(us_tr("Computing 4 & 5 term exponentials for bead %1"))
            .arg(j == last_bead ? "Global average" : QString("%1").arg(j + 1)));
    ...
    if (!usu.compute_exponential(q, Is[j], coeff4, coeff5, coeffv, norm4, norm5,
                                 normv,
      ...
      error_msg =
          QString("Error: compute_structure_factors(): %1").arg(usu.errormsg);
      return false;
    }
