...
bool US_Saxs_Util::dmd_run_with_log(const QString &, const QString &,
                                    const QString &) {
...
bool US_Saxs_Util::dmd_run_with_log(const QString &tag, const QString &cmd,
                                    const QString &log) {
...
bool US_Saxs_Util::dmd_findSS() {
  ...
  if (!system(cmd.toLatin1().data())) {
    ...
    // errormsg =  QString( "Error: command %1 did not return a successful exit
    // code" ).arg( cmd ); return false;
  }
  TSO << "Finished " + prog + "\n";
...
bool US_Saxs_Util::dmd_prepare() {
  ...
  // create constraints file
  {
    ...
    if (control_parameters.count("dmdstatic") &&
      ...
      QStringList qsl =
          (control_parameters["dmdstatic"]).split(",", Qt::SkipEmptyParts);
  ...
  if (!QFile::exists(param_file)) {
    errormsg =
        QString("Error: %1 did not create file %2").arg(prog).arg(param_file);
    return false;
  }

  if (!QFile::exists(state_file)) {
    errormsg =
        QString("Error: %1 did not create file %2").arg(prog).arg(state_file);
    return false;
  }

  if (!QFile::exists(const_file)) {
    errormsg =
        QString("Error: %1 did not create file %2").arg(prog).arg(const_file);
    return false;
  }
...
bool US_Saxs_Util::input_dimensions(point &range) {
  ...
  if (current_model > model_vector.size()) {
    errormsg = QString(
                   "input dimensions called with current model %1 greater than "
                   "the number of models loaded %2")
                   .arg(current_model)
  ...
  if (control_parameters.count("dmdboxspacing")) {
    if (control_parameters["dmdboxspacing"].toLower().contains(
      ...
      if (rx_cap_1.indexIn(control_parameters["dmdboxspacing"].toLower()) ==
              -1 &&
          rx_cap_3.indexIn(control_parameters["dmdboxspacing"].toLower()) ==
              -1) {
        errormsg =
            "Parameter for DMDBoxSpacing recognized, must in #,#,# or +#: "
            "was: " +
  ...
  for (int i = 0; i < qsl_pdb_size; ++i) {
    ...
    if (keep) {
      if (rx_check_line.indexIn(qs) != -1) {
        ...
        TSO << QString(
                   "chain_id [%1] last [%2] residue_no [%3] last [%4] key [%5] "
                   "last [%6]\n")
                   .arg(chain_id)
                   .arg(last_chain_id)
                   .arg(residue_no)
...
bool US_Saxs_Util::dmd_run(QString run_description) {
  ...
  if (!QFile::exists(param_file)) {
    errormsg =
        QString("Error: dmd_run: file %1 does not exist").arg(param_file);
    return false;
  }

  if (!QFile::exists(state_file)) {
    errormsg =
        QString("Error: dmd_run: file %1 does not exist").arg(state_file);
    return false;
  }

  if (!QFile::exists(const_file)) {
    errormsg =
        QString("Error: dmd_run: file %1 does not exist").arg(const_file);
    return false;
  }
  ...
  // create task file
  {
    ...
    if (!f.open(QIODevice::WriteOnly)) {
      errormsg =
          QString("Error: %1 can not create file %2").arg(prog).arg(task_file);
      return false;
    }
    ...
    task += QString(
                ...
                "character as \"#\" or \"//\".\n"
                "#\n"
                "#THERMOSTAT: ANDERSON or BERENDSON, right now BERENDSON is "
                "not implemented yet\n"
                "#\n"
    ...
    task +=
        QString(
            "#\n"
            "#RESTART_FILE specifies the name of the OUTPUT restart file, "
            "DEFAULT \"dmd_restart\"\n"
            "#\n"
            "#RESTART_DT specifies the delta-time to save each restart file, "
            "the file will be\n"
            ...
            "RESTART_DT              10\n"
            "#\n"
            "#ECHO_FILE specifies the name of the output file to write the "
            "output\n"
    ...
    task +=
        QString(
            "#\n"
            "#START_TIME specifies the initial time when the simulation "
            "starts, DEFAULT 0\n"
            "#\n"
  ...
  if (!QFile::exists(restart_file)) {
    errormsg =
        QString("Error: %1 did not create file %2").arg(prog).arg(restart_file);
    return false;
  }

  if (!QFile::exists(echo_file)) {
    errormsg =
        QString("Error: %1 did not create file %2").arg(prog).arg(echo_file);
    return false;
  }

  if (!QFile::exists(movie_file)) {
    errormsg =
        QString("Error: %1 did not create file %2").arg(prog).arg(movie_file);
    return false;
  }
  ...
  if (us_udp_msg) {
    us_udp_msg->send_json(
        {{"_progressmsg", QString("Extracting %1 result pdbs")
  ...
  if (control_parameters.count("dmdtimestep")) {
    ...
    if (dmdmmlastout) {
      ...
      for (int i = 0; i < allmodels_size; ++i) {
        ...
        if (rx_check_line.indexIn(qs) != -1 && qs.mid(76, 2).trimmed() == "H") {
          continue;
        }
...
QString US_Saxs_Util::dmd_next_res(const QString &source) {
  ...
  if (source.length() != 3) {
    errormsg =
        QString("dmd_next_res() : resname must be 3 character, given '%1'")
            .arg(source);
  }
...
static void map_dump(const QString &tag,
                     const map<QString, map<int, int> > &mapqii) {
  QTextStream tso(stdout);
  tso << tag << "\n";
...
static void map_dump(const QString &tag,
                     const map<QString, QStringList> &mqsqsl) {
  TSO << tag << "\n";
...
static void map_dump(const QString &tag,
                     const map<QString, map<int, QString> > &mapqiqs) {
  QTextStream tso(stdout);
  tso << tag << "\n";
...
static QStringList map_dump(const QString &tag, const map<QString, int> &mqsi,
...
static QStringList map_dump(
    const QString &tag, map<QString, map<QString, set<QString> > > &mqsmqssqs,
    bool also_stdout = true) {
  QStringList result;
  QString result_line;
  for (auto it = mqsmqssqs.begin(); it != mqsmqssqs.end(); ++it) {
    ...
    for (auto it2 = it->second.begin(); it2 != it->second.end(); ++it2) {
      result_line = " chain resseq : " +
                    QString("%1").arg(it2->first).leftJustified(6, QChar(' ')) +
                    " atoms : (" + QString("%1").arg(it2->second.size()) + ")";
...
static set<int> connected_vertices(int pos, map<int, set<int> > &chain_links) {
  ...
  for (auto it = chain_links[pos].begin(); it != chain_links[pos].end(); ++it) {
    if (!visited.count(*it)) {
...
static set<int> connected_vertices_run(int pos,
...
bool US_Saxs_Util::dmd_pdb_prepare(QStringList &qsl_pdb,
  ...
  for (int i = 0; i < qsl_pdb_size; ++i) {
    ...
    if (fields["recname"] == "REMARK") {
      if (fields["remarknum"] == "766") {
        ...
        dmd_pdb_add_back
            << line;  // could add this for other REMARKS, HELIX, SHEET etc, but
                      // needs separate block (!links.push_back)
        modified_pdb << line;
      } else {
        qsl_pdb_removed << line;
      }
      ...
      if (first_residue) {
        ...
        if (fields["recname"] == "ATOM") {
          ...
          dmd_res[fields["chainid"]][fields["resseq"].toInt()] =
              dmd_resseq_atom;
          dmd_org_chain[nth_letter(dmd_chain_num_hetatm)][dmd_resseq_atom] =
              fields["chainid"];
          dmd_org_res[nth_letter(dmd_chain_num_hetatm)][dmd_resseq_atom] =
              fields["resseq"].toInt();
          ...
          dmd_res[fields["chainid"]][fields["resseq"].toInt()] =
              dmd_resseq_hetatm;
          dmd_org_chain[nth_letter(dmd_chain_num_hetatm)][dmd_resseq_hetatm] =
              fields["chainid"];
          dmd_org_res[nth_letter(dmd_chain_num_hetatm)][dmd_resseq_hetatm] =
              fields["resseq"].toInt();
  ...
  // process LINKs
  {
    ...
    for (int i = 0; i < links_size; ++i) {
      ...
      // check if valid
      if (!dmd_chain.count(chainid1) || !dmd_chain[chainid1].count(resseq1)) {
        errormsg = QString(
                       "dmd_pdb_prepare() : could not find LINK chainid %1 "
                       "residue sequence %2 in dmd_chain map")
                       .arg(chainid1)
                       .arg(resseq1);
        return false;
      }
      if (!dmd_res_link.count(chainid1) ||
        ...
        errormsg = QString(
                       "dmd_pdb_prepare() : could not find LINK chainid %1 "
                       "residue sequence %2 in dmd_res map")
                       .arg(chainid1)
                       .arg(resseq1);
        return false;
      }
      if (!dmd_chain.count(chainid2) || !dmd_chain[chainid2].count(resseq2)) {
        errormsg = QString(
                       "dmd_pdb_prepare() : could not find LINK chainid %1 "
                       "residue sequence %2 in dmd_chain map")
                       .arg(chainid2)
    ...
    // link_check
    {
      ...
      if (!link_range_defined) {
        noticemsg += QString(
                         "Notice: using a default link range of %1 centered on "
                         "the LINK distance provided")
  ...
  if (production_run) {
    for (auto it = hetatm_lines.begin(); it != hetatm_lines.end(); ++it) {
      ...
      tso << it->second.join("\n") << "\n";
      fo.close();
      QString cmd = QString("babel -ipdb %1 -omol2 > %2.mol2 2> /dev/null")
                        .arg(fo.fileName())
                        .arg(it->first);
...
bool US_Saxs_Util::dmd_pdb_restore(const QStringList &qsl_pdb,
                                   QStringList &qsl_pdb_restored,
                                   bool add_back) {
  int qsl_pdb_size = (int)qsl_pdb.size();
  if (!qsl_pdb_size) {
  ...
  for (int i = startpos; i < qsl_pdb_size; ++i) {
    ...
    if (fields["recname"] == "HETATM" || fields["recname"] == "ATOM") {
      ...
      if (!dmd_org_chain.count(chainid) ||
        ...
        errormsg = QString(
                       "dmd_pdb_restore() : could not find LINK chainid %1 "
                       "residue sequence %2 in dmd_org_chain map")
                       .arg(chainid)
                       .arg(resseq);
        return false;
      }
      if (!dmd_org_res.count(chainid) || !dmd_org_res[chainid].count(resseq)) {
        errormsg = QString(
                       "dmd_pdb_restore() : could not find LINK chainid %1 "
                       "residue sequence %2 in dmd_org_res map")
                       .arg(chainid)
                       .arg(resseq);
        return false;
      }
      ...
      if (fields["recname"] == "HETATM") {
        if (!dmd_mol2_res.count(resname)) {
          errormsg = QString(
                         "dmd_pdb_restore() : could not find resname %1 in "
                         "dmd_mol2_res map")
                         .arg(resname);
          return false;
        }
      ...
      line.replace(
          22, 4,
          QString("%1").arg(dmd_org_res[chainid][resseq], 4, 10, QChar(' ')));
    }
