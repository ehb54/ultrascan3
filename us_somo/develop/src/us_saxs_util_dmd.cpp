...
bool
US_Saxs_Util::dmd_run_with_log (const QString &, const QString &,
                                const QString &)
{
...
bool
US_Saxs_Util::dmd_run_with_log (const QString &tag, const QString &cmd,
                                const QString &log)
{
      ...
      if (!system (cmd.toLatin1 ().data ()))
        {
          ...
          // errormsg =  QString( "Error: command %1 did not return a
          // successful exit code" ).arg( cmd ); return false;
        }
      return true;
    }
  ...
  if (int pid = fork ())
  ...
  else
    {
      ...
      FILE *pipe
          = popen (QString ("tail -f %1").arg (log).toLatin1 ().data (), "r");
          ...
          if ((int)this_time != last_time)
            {
              ...
              QString msg
                  = QString ("%1 : %2%").arg (tag).arg (pct, 0, 'g', 2);
...
US_Saxs_Util::dmd_findSS ()
{
  ...
  QString prog = env_ultrascan + SLASH +
#if defined(BIN64)
                 "bin64"
#else
                 "bin"
  ...
  QString cmd
      = QString ("%1 %2 > %3\n").arg (prog).arg (pdb).arg (constraints_file);
  ...
  if (!system (cmd.toLatin1 ().data ()))
    {
      ...
      // errormsg =  QString( "Error: command %1 did not return a successful
      // exit code" ).arg( cmd ); return false;
    }
  TSO << "Finished " + prog + "\n";
...
US_Saxs_Util::dmd_prepare ()
{
  ...
  // create constraints file
  {
    ...
    if (control_parameters.count ("dmdstatic")
      ...
      {
        QStringList qsl = (control_parameters["dmdstatic"])
...
bool
US_Saxs_Util::input_dimensions (point &range)
{
  point min;
  point max;
  ...
  if (current_model > model_vector.size ())
    {
      errormsg = QString ("input dimensions called with current model %1 "
                          "greater than the number of models loaded %2")
                     .arg (current_model)
            ...
            if (!any_set)
              {
                for (unsigned int m = 0; m < 3; m++)
                  {
                    min.axis[m] = max.axis[m] = this_atom->coordinate.axis[m]
                        = 0;
                  }
                any_set = true;
              }
  ...
  if (control_parameters.count ("dmdboxspacing"))
    {
      if (control_parameters["dmdboxspacing"].toLower ().contains (
              QRegExp ("^cubic")))
        {
          ...
          if (rx_cap.indexIn (control_parameters["dmdboxspacing"].toLower ())
              != -1)
            {
              if (rx_cap.cap (1) == "+")
                {
      ...
      else
        {
          ...
          if (rx_cap_1.indexIn (control_parameters["dmdboxspacing"].toLower ())
                  == -1
              && rx_cap_3.indexIn (
                     control_parameters["dmdboxspacing"].toLower ())
                     == -1)
            {
              errormsg = "Parameter for DMDBoxSpacing recognized, must in "
                         "#,#,# or +#: was: "
                         + control_parameters["dmdboxspacing"];
              return false;
            }
          if (rx_cap_1.indexIn (control_parameters["dmdboxspacing"].toLower ())
              != -1)
...
US_Saxs_Util::dmd_strip_pdb ()
{
  ...
  // dmdstrip is the entry point for dmd runs, so let's try alternative save
  // strategy before exposing via gui
  ...
  QString pdb = control_parameters["inputfile"];
  {
    ...
    // could get fancier here, perhaps REMARKs could be added or incorporate
    // condensed prior run count
  ...
  if (!fi.open (QIODevice::ReadOnly))
    {
      errormsg = QString ("Error: can not read file %1").arg (pdb);
      return false;
    }
  ...
  if (!fo.open (QIODevice::WriteOnly))
    {
      errormsg = QString ("Error: can not create file %1").arg (pdb_stripped);
      fi.close ();
      return false;
    }
  ...
  if (!fol.open (QIODevice::WriteOnly))
    {
      errormsg = QString ("Error: can not create file %1").arg (stripped_log);
      fi.close ();
      fo.close ();
      return false;
    }
  ...
  if (!fot.open (QIODevice::WriteOnly))
    {
      errormsg = QString ("Error: can not create file %1").arg (topparam);
      fi.close ();
      fo.close ();
      fol.close ();
      return false;
  ...
  // create topparam
  {
    ...
    for (auto it = dmd_mol2_res.begin (); it != dmd_mol2_res.end (); ++it)
      {
  ...
  for (unsigned int i = 0; i < (unsigned int)exclude_residues_list.size ();
       i++)
    {
      ...
      if (rx_check_line.indexIn (qs) != -1)
        {
          ...
          if (exclude_atoms.count (atom) || exclude_residues.count (residue))
            {
              keep = false;
            }
        }
          ...
          if (rx_check_line.indexIn (qs) != -1)
            {
              ...
              TSO << QString ("chain_id [%1] last [%2] residue_no [%3] last "
                              "[%4] key [%5] last [%6]\n")
                         .arg (chain_id)
                         .arg (last_chain_id)
                         .arg (residue_no)
...
bool
US_Saxs_Util::dmd_run (QString run_description)
{
      ...
      if (run_description.toLower () == "equi")
        {
          if (control_parameters.count ("dmdequiheatxc"))
            ...
            {
              TSO << "dmd_run: equi heat exchange using default value of "
                     "0.1\n";
              dmd_heat_xc = "0.1";
            }
        }
  ...
  // outputs
  // FIX THIS: should be renamed ? and renamed in output_files
  run_description += QString ("_tp%1_tm%2")
  ...
  // create task file
  {
    ...
    task += QString ("#THIS IS THE EXAMPLE INPUT FILE FOR xDMD SIMULATIONS.\n"
                     "#THE COMMENT LINE starts with the 1st non whitespace "
                     "character as \"#\" or \"//\".\n"
                     "#\n"
                     "#THERMOSTAT: ANDERSON or BERENDSON, right now BERENDSON "
                     "is not implemented yet\n"
                     "#\n"
                     "#T_NEW is the instant temperature at the moment of "
                     "simulation. If specified, the\n"
                     "# velocities of input state/restart willbe re-scaled\n"
                     "#\n"
                     "#T_LIMIT is the targeting temperature\n"
                     "#\n"
                     "#HEAT_X_C determines how often the exchange takes place "
                     "betwen IMAGINARY\n"
    ...
    task += QString ("#\n"
                     "#RESTART_FILE specifies the name of the OUTPUT restart "
                     "file, DEFAULT \"dmd_restart\"\n"
                     "#\n"
                     "#RESTART_DT specifies the delta-time to save each "
                     "restart file, the file will be\n"
                     "# overwritten, DEFAULT 1000\n"
                     "#\n"
                     "RESTART_FILE            %1\n"
                     "RESTART_DT              10\n"
                     "#\n"
                     "#ECHO_FILE specifies the name of the output file to "
                     "write the output\n"
                     "# parameters(energy,pressure,tempeature etc.)\n"
                     "#\n"
                     "#ECHO_DT specifies the delta-time to save the output "
                     "parameters\n"
                     "#\n"
                     "ECHO_FILE               %2\n"
                     "ECHO_DT                 0.1\n")
    ...
    task += QString (
                ...
                "#\n"
                "#MOVIE_SAVE_START specifies the starting atom to be saved, "
                "DEFAULT 1\n"
                "#\n"
                "#MOVIE_SAVE_END specifies the endinig atom to be saveed, "
                "DEAULT the last atom in the system\n"
                "#if you has no idea of the system, PLEASE do not specify "
                "MOVIE_SAVE_START and MOVIE_SAVE_END\n"
                "#\n"
                "MOVIE_FILE              %1\n"
                "MOVIE_DT                %2\n"
                "#MOVIE_SAVE_START       1\n"
    ...
    task += QString ("#\n"
                     "#START_TIME specifies the initial time when the "
                     "simulation starts, DEFAULT 0\n"
                     "#\n"
                     "#MAX_TIME specifies the maximum time for the simulation "
                     "to last\n"
                     "#\n"
                     "START_TIME              0\n"
                     "MAX_TIME                %1\n")
  ...
  // todo
  QString cmd = QString ("%1 -p %2 -s %3 -c %4 -i %5")
                    .arg (prog)
                    .arg (param_file)
                    .arg (state_file)
                    .arg (const_file)
  ...
  TSO << "Starting " + prog + "\n";
  TSO << cmd << Qt::endl;
  if (!dmd_run_with_log (is_relax ? "Relaxation" : "Equilibrium", cmd,
                         echo_file))
    {
      // if ( !system( cmd.toLatin1().data() ) ) {
  ...
  if (us_udp_msg)
    {
      us_udp_msg->send_json (
          { { "_progressmsg",
              QString ("Extracting %1 result pdbs")
                  .arg (is_relax ? "Relaxation" : "Equilibrium") } });
    }
  if (control_parameters.count ("dmdtimestep"))
    {
      ...
      prog = env_ultrascan + SLASH +
#if defined(BIN64)
             "bin64"
#else
             "bin"
#endif
             + SLASH + "complex_M2P.linux";

      {
        ...
        if (!qfi.exists ())
          {
            errormsg
                = QString ("DMD program '%1' does not exist\n").arg (prog);
            return false;
          }
        if (!qfi.isExecutable ())
          {
            errormsg
                = QString ("DMD program '%1' is not executable\n").arg (prog);
            return false;
          }
      }

      // $DMD/bin/complex_M2P.linux $DMD/lib/parameter $pdb $topparam dmd_movie
      // out.pdb constraint"; $DMD/bin/complex_M2P.linux $DMD/param/ xxx.pdb
      // /dev/null relax.dmd_movie relax.pdbs

      QString pdb_out_file = dmd_basename + "_" + run_description + ".pdb";
      QString pdb_out_to_fix_file
          = dmd_basename + "_" + run_description + ".pdb-to-fix";
      QString topparam_file = dmd_basename + ".topparam";

      cmd = QString ("%1 ../common %2 %3 %4 %5")
                .arg (prog)
                .arg (pdb)
                .arg (topparam_file)
                .arg (movie_file)
      ...
      // fix pdb file, count actual models produced, possibly split into
      // individual pdbs
      ...
      unsigned int models = 1;
      {
        ...
        if (control_parameters.count ("pdballmodels"))
        ...
        else
          {
            ...
            pdb_out_file = dmd_basename + "_" + run_description
                           + QString ("_m-%1").arg (omodel) + ".pdb";
          }
        ...
        for (int i = 0; i < qsl_pdb_size; ++i)
          {
            ...
            *tso << qs << Qt::endl;
            ...
            if (qs.contains (QRegExp ("^ENDMDL")) && i + 1 < qsl_pdb_size)
              {
                models++;
                if (control_parameters.count ("pdballmodels"))
                  {
                    ...
                    if (!fo->open (QIODevice::WriteOnly))
                      {
                        errormsg
                            = QString (
                                  "Error: %1 can not open file %2 for writing")
                                  .arg (prog)
                                  .arg (pdb_out_file);
                        delete fo;
                        return false;
      ...
      if (dmdmmlastout)
        {
          // also save allmodels
          QRegExp rx_check_line ("^(ATOM|HETATM)");
          QString pdb_out_file
              = dmd_basename + "_" + run_description + "_m-all.pdb";
...
QString
US_Saxs_Util::dmd_next_res (const QString &source)
{
  ...
  if (source.length () != 3)
    {
      errormsg
          = QString (
                "dmd_next_res() : resname must be 3 character, given '%1'")
                .arg (source);
    }
  ...
  if (size >= 1000)
    {
      QTextStream (stderr)
          << "ERROR: more than 999 HETATM residues used. Aborting.\n";
      exit (-1);
    }
...
static void
map_dump (const QString &tag, const map<QString, QString> &mapqsqs)
{
  QTextStream tso (stdout);
  tso << tag << "\n";
  for (auto it = mapqsqs.begin (); it != mapqsqs.end (); ++it)
    {
...
static void
map_dump (const QString &tag, const map<QString, map<int, int> > &mapqii)
{
  QTextStream tso (stdout);
  tso << tag << "\n";
  for (auto it = mapqii.begin (); it != mapqii.end (); ++it)
    {
      for (auto it2 = it->second.begin (); it2 != it->second.end (); ++it2)
        {

          tso << "'" << it->first << "' : " << it2->first << " -> "
              << it2->second << "\n";
        }
    }
}
...
static void
map_dump (const QString &tag, const map<QString, QStringList> &mqsqsl)
{
  TSO << tag << "\n";
  for (auto it = mqsqsl.begin (); it != mqsqsl.end (); ++it)
    {
      TSO << it->first << "\n" << it->second.join ("\n") << "\n";
    }
}
...
static void
map_dump (const QString &tag, const map<QString, map<int, QString> > &mapqiqs)
{
  QTextStream tso (stdout);
  tso << tag << "\n";
  for (auto it = mapqiqs.begin (); it != mapqiqs.end (); ++it)
    {
      for (auto it2 = it->second.begin (); it2 != it->second.end (); ++it2)
        {
...
static void
map_dump (const QString &tag, const map<int, set<int> > &mapisi)
{
  QTextStream tso (stdout);
  tso << tag << "\n";
  for (auto it = mapisi.begin (); it != mapisi.end (); ++it)
    {
      tso << it->first << " : ";
      for (auto it2 = it->second.begin (); it2 != it->second.end (); ++it2)
        {
...
static void
map_dump (const QString &tag, const set<int> &seti)
{
  QTextStream tso (stdout);
  tso << tag << "\n";
...
static QStringList
map_dump (const QString &tag, const map<QString, int> &mqsi,
          bool also_stdout = true)
{
  QStringList result;
  for (auto it = mqsi.begin (); it != mqsi.end (); ++it)
    {
...
static QStringList
map_dump (const QString &tag,
          map<QString, map<QString, set<QString> > > &mqsmqssqs,
          bool also_stdout = true)
{
  QStringList result;
  QString result_line;
  for (auto it = mqsmqssqs.begin (); it != mqsmqssqs.end (); ++it)
    {
      ...
      for (auto it2 = it->second.begin (); it2 != it->second.end (); ++it2)
        {
          result_line = " chain resseq : "
                        + QString ("%1")
                              .arg (it2->first)
                              .leftJustified (6, QChar (' '))
                        + " atoms : ("
                        + QString ("%1").arg (it2->second.size ()) + ")";
...
static set<int>
connected_vertices (int pos, map<int, set<int> > &chain_links)
{
  ...
  for (auto it = chain_links[pos].begin (); it != chain_links[pos].end ();
       ++it)
    {
      if (!visited.count (*it))
        {
...
static set<int>
connected_vertices_run (int pos, map<int, set<int> > &chain_links)
{
  visited.clear ();
...
bool
US_Saxs_Util::dmd_pdb_prepare (QString &pdb)
{
  QStringList qsl_pdb;
  QStringList qsl_pdb_removed;
  ...
  return dmd_pdb_prepare (qsl_pdb, qsl_pdb_removed, qsl_pdb_constraints,
                          false);
}

bool
US_Saxs_Util::dmd_pdb_prepare (QStringList &qsl_pdb,
                               QStringList &qsl_pdb_removed,
                               QStringList &qsl_link_constraints,
                               bool production_run)
{
  ...
  bool first_residue
      = true; // true when we start a model or chain, false after residue
  ...
  // hetatm_struct maps orignal resname and chainid:resseq to the set of atoms
  // contained for consistency validation
  map<QString, map<QString, set<QString> > > hetatm_struct;
  map<QString, int>
      hetatm_chain_atoms; // (original) chain id to number of atoms
  map<QString, QStringList>
      hetatm_lines; // lines of HETATM by new resname for mol2 processing
  ...
  for (int i = 0; i < qsl_pdb_size; ++i)
    {
      ...
      if (fields["recname"].contains (QRegExp ("^("
                                               "AUTHOR|"
                                               "CISPEP|"
                                               "COMPND|"
                                               "CRYST1|"
          ...
          if (fields["remarknum"] == "766")
            {
              ...
              dmd_pdb_add_back
                  << line; // could add this for other REMARKS, HELIX, SHEET
                           // etc, but needs separate block (!links.push_back)
              modified_pdb << line;
            }
          else
            {
      ...
      if (fields["recname"] == "HETATM" || fields["recname"] == "ATOM")
        {
          ...
          if (first_residue)
            {
              ...
              dmd_chain[fields["chainid"]][fields["resseq"].toInt ()]
                  = dmd_chain_num;
              if (fields["recname"] == "ATOM")
                {
                  ++dmd_resseq_atom;
                  dmd_res[fields["chainid"]][fields["resseq"].toInt ()]
                      = dmd_resseq_atom;
                  dmd_org_chain[nth_letter (dmd_chain_num_hetatm)]
                               [dmd_resseq_atom]
                      = fields["chainid"];
                  dmd_org_res[nth_letter (dmd_chain_num_hetatm)]
                             [dmd_resseq_atom]
                      = fields["resseq"].toInt ();
              ...
              else
                {
                  ...
                  dmd_org_res[nth_letter (dmd_chain_num_hetatm)]
                             [dmd_resseq_hetatm]
                      = fields["resseq"].toInt ();
                  chain_to_id[dmd_chain_num]
                      = fields["chainid"] + ":" + fields["resseq"];
                }
              dmd_resseq_atom_link = 1;
              dmd_res_link[fields["chainid"]][fields["resseq"].toInt ()]
                  = dmd_resseq_atom_link;
            }
      ...
      if (fields["recname"] == "HETATM")
        {
          ...
          dmd_chain_is_hetatm[fields["chainid"]].insert (
              fields["resseq"].toInt ());

          last_atom_was_hetatm = true;
          hetatm_struct[fields["resname"]]
                       [fields["chainid"] + ":" + fields["resseq"]]
                           .insert (fields["name"]);
          hetatm_chain_atoms[fields["chainid"]]++;

          if (!first_residue && last_resseq != this_resseq)
            {
              modified_pdb << "TER";
              dmd_chain[fields["chainid"]][fields["resseq"].toInt ()]
                  = ++dmd_chain_num;
      ...
      // cache LINKs (we won't know mappings until ATOMs and HETATMs are
      // processed )
  ...
  dmd_pdb_prepare_reports["HETATM Chain Detail"]
      = map_dump ("hetatm_struct", hetatm_struct, false);
  dmd_pdb_prepare_reports["HETATM Chain Atom Count"]
      = map_dump ("hetatm_chain_atoms", hetatm_chain_atoms, false);
  ...
  // process LINKs
  {
    ...
    for (int i = 0; i < links_size; ++i)
      {
        ...
        if (fields["recname"] == "REMARK" && fields["remarknum"] == "766")
          {
            if (fields.count ("dmdlinkstart") && fields.count ("dmdlinkend"))
              {
        ...
        if (!dmd_chain.count (chainid1)
          ...
          {
            errormsg
                = QString ("dmd_pdb_prepare() : could not find LINK chainid "
                           "%1 residue sequence %2 in dmd_chain map")
                      .arg (chainid1)
                      .arg (resseq1);
            return false;
          }
        if (!dmd_res_link.count (chainid1)
            || !dmd_res_link[chainid1].count (resseq1))
          {
            errormsg
                = QString ("dmd_pdb_prepare() : could not find LINK chainid "
                           "%1 residue sequence %2 in dmd_res map")
                      .arg (chainid1)
                      .arg (resseq1);
            return false;
          }
        ...
        QString constraint = QString ("AtomPair %1.%2.%3 %4.%5.%6 %7 %8")
        ...
        chain_links[dmd_chain[chainid1][resseq1]].insert (
            dmd_chain[chainid2][resseq2]);
        chain_links[dmd_chain[chainid2][resseq2]].insert (
            dmd_chain[chainid1][resseq1]);
    ...
    // link_check
    {
      ...
      if (!link_range_defined)
        {
          noticemsg += QString ("Notice: using a default link range of %1 "
                                "centered on the LINK distance provided")
  ...
  if (production_run)
    {
      for (auto it = hetatm_lines.begin (); it != hetatm_lines.end (); ++it)
        {
          QFile fo (QString ("_hetatm_%1.pdb").arg (it->first));
          if (!fo.open (QIODevice::WriteOnly))
            {
              errormsg = QString ("Error: dmd_pdb_prepare() can not open file "
                                  "%2 for writing")
                             .arg (fo.fileName ());
              return false;
            }
          ...
          fo.close ();
          QString cmd
              = QString ("babel -ipdb %1 -omol2 > %2.mol2 2> /dev/null")
                    .arg (fo.fileName ())
                    .arg (it->first);
          if (system (cmd.toLatin1 ().data ()) != 0)
            {
              errormsg = QString ("dmd_pdb_prepare() : command %1 failed")
                             .arg (cmd);
              return false;
            }
  ...
  // "\n"; map_dump( "dmd_res", dmd_res ); map_dump( "dmd_res_link",
  // dmd_res_link ); map_dump( "dmd_chain", dmd_chain ); map_dump(
  // "dmd_org_res", dmd_org_res ); map_dump( "dmd_org_chain", dmd_org_chain );
...
bool
US_Saxs_Util::dmd_pdb_restore (const QStringList &qsl_pdb,
                               QStringList &qsl_pdb_restored, bool add_back)
{
  int qsl_pdb_size = (int)qsl_pdb.size ();
  if (!qsl_pdb_size)
    {
  ...
  for (int i = startpos; i < qsl_pdb_size; ++i)
    {
      ...
      if (fields["recname"] == "TER" && i && i + 1 < qsl_pdb_size)
        {
      ...
      if (fields["recname"] == "HETATM")
        {
          line.replace (76, 2,
                        fields["element"].rightJustified (2, QChar (' ')));
        }
      ...
      if (fields["recname"] == "HETATM" || fields["recname"] == "ATOM")
        {
          ...
          if (!dmd_org_chain.count (chainid)
            ...
            {
              errormsg
                  = QString ("dmd_pdb_restore() : could not find LINK chainid "
                             "%1 residue sequence %2 in dmd_org_chain map")
                        .arg (chainid)
                        .arg (resseq);
              return false;
            }
          if (!dmd_org_res.count (chainid)
              || !dmd_org_res[chainid].count (resseq))
            {
              errormsg
                  = QString ("dmd_pdb_restore() : could not find LINK chainid "
                             "%1 residue sequence %2 in dmd_org_res map")
                        .arg (chainid)
                        .arg (resseq);
              return false;
            }
          ...
          if (fields["recname"] == "HETATM")
            {
              if (!dmd_mol2_res.count (resname))
                {
                  errormsg = QString ("dmd_pdb_restore() : could not find "
                                      "resname %1 in dmd_mol2_res map")
                                 .arg (resname);
                  return false;
                }
              line.replace (
                  17, 3, dmd_mol2_res[resname].leftJustified (3, QChar (' ')));
            }

          line.replace (21, 1, dmd_org_chain[chainid][resseq]);
          line.replace (22, 4,
                        QString ("%1").arg (dmd_org_res[chainid][resseq], 4,
                                            10, QChar (' ')));
        }
...
QString
US_Saxs_Util::dmd_fix_hetatm_name_pos (const QString &qs)
{
...
QStringList
US_Saxs_Util::dmd_fix_hetatm_name_pos (const QStringList &qsl)
{
  QStringList result;
