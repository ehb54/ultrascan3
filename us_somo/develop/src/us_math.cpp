...
double
linefit (double **x, double **y, double *slope, double *intercept,
         double *sigma, double *correlation, int arraysize)
{
  double sumx = 0, sumy = 0, sumxy = 0, sumx_sq = 0, sumy_sq = 0,
         sumchi_sq = 0, average;
  int i;
...
float
linefit (float **x, float **y, float *slope, float *intercept, float *sigma,
         float *correlation, int arraysize)
{
  float sumx = 0, sumy = 0, sumxy = 0, sumx_sq = 0, sumy_sq = 0, sumchi_sq = 0,
        average;
  int i;
  for (i = 0; i < arraysize; i++)
    {
...
us_randomize ()
{
  QTime t;
  t = QTime::currentTime ();
  int seed, now = t.msec () + t.second () * 1000 + t.minute () * 60000
                  + t.hour () * 3600000;
  seed = now;
#ifdef UNIX
  seed -= (int)getpid ();
...
float
random_range (float min_val, float max_val)
{
...
float
standard_distribution (float x)
{
  float result;
  result = (exp ((-0.5) * pow ((double)x, (double)2.0)))
           / sqrt ((double)(2 * M_PI));
  return result;
}

float
us_normal_distribution (float sigma, float mean, float x)
{
  float result;
  result = exp (-(pow ((double)((x - mean) / sigma), (double)2.0)))
           / sqrt ((double)(2 * M_PI * sigma));
  return result;
}
//
...
// bounds where exceeded (i.e., the searched value testx was not found in the
// array)
//
int
find_indexf (double testx, double **xarray, int maxcount)
{
  int i = 0;
  maxcount--;
...
int
find_indexf (float testx, float **xarray, int maxcount)
{
  int i = 0;
  maxcount--;
  while (((*xarray)[i] < testx) && (i <= maxcount))
...
int
find_indexf (int testx, int **xarray, int maxcount)
{
  int i = 0;
  maxcount--;
  while (((*xarray)[i] < testx) && (i <= maxcount))
...
int
find_indexr (double testx, double **xarray, int maxcount)
{
  int i = maxcount - 1;
...
int
find_indexr (float testx, float **xarray, int maxcount)
{
  int i = maxcount - 1;
  while (((*xarray)[i] > testx) && (i >= 0))
    {
...
int
find_indexr (int testx, int **xarray, int maxcount)
{
  int i = maxcount - 1;
  while (((*xarray)[i] > testx) && (i >= 0))
    {
...
//
void
get_1d_limits (double **array, double *lower, double *upper, int count,
               int start_count)
{
  *upper = (*array)[0];
  *lower = (*array)[0];
...
void
get_1d_limits (float **array, float *lower, float *upper, int count,
               int start_count)
{
  *upper = (*array)[0];
  *lower = (*array)[0];
...
void
get_1d_limits (int **array, int *lower, int *upper, int count, int start_count)
{
  *upper = (*array)[0];
  *lower = (*array)[0];
  for (int i = start_count + 1; i < count; i++)
...
//
void
get_2d_limits (double ***array, double *lower, double *upper, int count1,
               int count2, int start_count1, int start_count2)
{
...
void
get_2d_limits (float ***array, float *lower, float *upper, int count1,
               int count2, int start_count1, int start_count2)
{
  *upper = (*array)[start_count1][start_count2];
  *lower = (*array)[start_count1][start_count2];
...
void
get_2d_limits (int ***array, int *lower, int *upper, int count1, int count2,
               int start_count1, int start_count2)
{
  *upper = (*array)[start_count1][start_count2];
  *lower = (*array)[start_count1][start_count2];
  for (int i = start_count1; i < count1; i++)
...
void
gaussian_smoothing (float **array, unsigned int smooth, unsigned int points)
{
  unsigned int i, j, k;
  ...
  if (smooth > 1.0) // we'll apply a normalized Gaussian smoothing kernel that
                    // goes out to 2 standard deviations
    {
      ...
      x_weights[0] = 0.0;
      y_weights[0] = us_normal_distribution (
          1.0, 0.0, x_weights[0]); // standard deviation = 1.0, Mean = 0;
      for (i = 1; i < smooth; i++) // we only calculate half a Gaussian, since
      ...
      // first, take care of the left border, using an "appearing frame"
      // algorithm, starting with half a frame visible:
      ...
      for (j = 0; j < smooth; j++) // loop over all border point centers
        {
          ...
          for (k = j; k < j + smooth; k++)
            {
              sum += temp_array[k]
                     * y_weights[position]; // sum the weighted points right of
          ...
          for (int m = j - 1; m >= 0; m--)
            {
              position++;
              sum += temp_array[m]
                     * y_weights[position]; // sum all applicable points on the
                                            // left of center
              sum_y += y_weights[position];
            }
          (*array)[j] = (sum / sum_y); // normalize by the sum of all weights
                                       // that were used
        }

      // Now deal with all non-border points:

      for (j = smooth; j < points - smooth;
           j++) // loop over all non-border points
        {
          sum = 0.0;
          sum_y = 0.0;
          position = 0;
          for (k = j; k < j + smooth; k++)
            {
              // cout << "k: " << k << ", position: " << position << ",
              // y_weights: " <<  y_weights[position] << ", points: " << points
              // << endl;
              sum += temp_array[k]
                     * y_weights[position]; // sum the weighted points right of
                                            // center, including center
          ...
          (*array)[j]
              = sum
                / sum_y; // normalize by the sum of all weights that were used
        }
      ...
      for (j = points - smooth; j < points;
           j++) // loop over all right-border points
        {
          sum = 0.0;
          sum_y = 0.0;
          position = 0;
          for (int m = (int)j - 1; m >= (int)(j - smooth + 1); m--)
            {
              position++;
              sum += temp_array[m] * y_weights[position]; // sum all points on
                                                          // the left of center
              sum_y += y_weights[position];
            }
          position = 0;
          for (k = j; k < points; k++)
            {
              sum += temp_array[k]
                     * y_weights[position]; // sum the weighted points in the
                                            // disappearing frame
              sum_y
                  += y_weights[position]; // right of center, including center
              position++;
            }
          (*array)[j]
              = sum
...
double
inverse_error_function (double test_val, double step)
{
  double result = 0, y = 0;
  y += step;
  ...
  while (result < test_val)
    {
      result += (2 / pow (M_PI, (double)0.5))
                * exp (-1 * pow ((double)y, (double)2)) * step;
      y += step;
    }
  return (y);
}
...
void
calc_vbar (struct peptide *pep, QString *sequence, float *temperature)
{
  pep->vbar_sum = 0.0;
  pep->residues = 0;
  pep->mw = 0;
  ...
  pep->residues += pep->r + pep->l + pep->k + pep->a + pep->v + pep->g + pep->e
                   + pep->n + pep->i + pep->j + pep->q + pep->y + pep->o
                   + pep->p + pep->s + pep->h + pep->t + pep->d + pep->f
                   + pep->b + pep->c + pep->m + pep->w + pep->z + pep->x
                   + pep->dab + pep->dpr;
...
float
adjust_vbar20 (float vbar20, float temperature)
{
  return (vbar20 + 0.002125 + 4.25e-4 * (temperature - 25.0));
}

float
adjust_vbar (float vbar, float temperature)
{
  return (vbar - 0.002125 - 4.25e-4 * (temperature - 25.0));
}

float
calc_buoyancy_tb (float vbar20, float temperature, float density20)
{
  float density_wt, density_wt_20, t, vbar, buoyancy_tb, density_tb;
  vbar = adjust_vbar20 (vbar20, temperature);
  t = temperature;
  density_wt
      = 1.000028e-3 * ((999.83952 + 16.945176 * t) / (1 + 16.879850e-3 * t))
        - 1.000028e-3
              * ((7.9870401e-3 * pow ((double)t, (double)2)
                  + 46.170461e-6 * pow ((double)t, (double)3))
                 / (1 + 16.87985e-3 * t))
        + 1.000028e-3
              * ((105.56302e-9 * pow ((double)t, (double)4)
                  - 280.54253e-12 * pow ((double)t, (double)5))
                 / (1 + 16.87985e-3 * t));
  density_wt_20
      = 1.000028e-3 * ((999.83952 + 16.945176 * 20) / (1 + 16.879850e-3 * 20))
        - 1.000028e-3
              * ((7.9870401e-3 * pow ((double)20, (double)2)
                  + 46.170461e-6 * pow ((double)20, (double)3))
                 / (1 + 16.87985e-3 * 20))
        + 1.000028e-3
              * ((105.56302e-9 * pow ((double)20, (double)4)
                  - 280.54253e-12 * pow ((double)20, (double)5))
                 / (1 + 16.87985e-3 * 20));
...
float
calc_bottom (vector<struct rotorInfo> rotor_list,
             vector<struct centerpieceInfo> cp_list, int rotor,
             int centerpiece, int channel, unsigned int rpm)
{
  if (centerpiece < 0)
    {
      return (-1.0);
  ...
  if (rpm != 0)
    {
      for (unsigned int i = 0; i < 5; i++)
        {
          bottom += rotor_list[rotor].coefficient[i]
...
#if !defined(CMDLINE)
float
calc_bottom (int rotor, int centerpiece, int channel, unsigned int rpm)
{
  if (centerpiece < 0)
    {
      return (-1.0);
  ...
  if (!readCenterpieceInfo (&cp_list))
    {
      ...
      //    "centerpiece database file:\n\n" + USglobal->config_list.system_dir
      //    + "/etc/centerpiece.dat\n\n" "Please install the centerpiece
...
double
stretch (int rotor, unsigned int rpm)
{
  vector<struct rotorInfo> rotor_list;
  rotor_list.clear ();
  double stretch = 0.0;
  if (readRotorInfo (&rotor_list))
    ...
    {
      fprintf (stderr,
               "ERROR reading rotor info in us_math ... no stretch!\n");
    }
  rotor_list.clear ();
  return (stretch);
}
#else
float
calc_bottom (int, int, int, unsigned int)
{
  return 0;
}
double
stretch (int, unsigned int)
{
  return 0;
}
#endif
...
double
stretch_with_rotor_list (int rotor, unsigned int rpm,
 ...
 */
int
nnls (double *a, int a_dim1, int m, int n,
  ...
  while (iz1 <= iz2 && nsetp < m)
    {
      ...
      while (1)
        {
          ...
          _nnls_h12 (1, npp1, npp1 + 1, m, &a[j * a_dim1], 1, &up, &dummy, 1,
                     1, 0);
          unorm = 0.;
          if (nsetp != 0)
            for (l = 0; l < nsetp; l++)
          ...
          if ((d2 - unorm) > 0.)
            {
              ...
              for (l = 0; l < m; l++)
                zz[l] = b[l];
              _nnls_h12 (2, npp1, npp1 + 1, m, &a[j * a_dim1], 1, &up, zz, 1,
                         1, 1);
      ...
      while (++iter < itmax)
        {
          /* See if all new constrained coeffs are feasible; if not, compute
           * alpha */
                  ...
                  for (j = jj + 1; j < nsetp; j++)
                    {
                      ...
                      for (a[j + ii * a_dim1] = 0., l = 0; l < n; l++)
                        if (l != ii)
                          {
                            /* Apply procedure G2 (CC,SS,A(J-1,L),A(J,L)) */
                            temp = a[j - 1 + l * a_dim1];
                            a[j - 1 + l * a_dim1]
                                = cc * temp + ss * a[j + l * a_dim1];
                            a[j + l * a_dim1]
                                = -ss * temp + cc * a[j + l * a_dim1];
                          }
 ...
 */
void
_nnls_g1 (double a, double b, double *cterm, double *sterm, double *sig)
{
  double d1, xr, yr;
...
float
int_vol_2sphere (float r1, float r2, float d)
{
...
QString
us_double_decimal_places (double x, int dp)
{
