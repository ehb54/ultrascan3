...
unsigned long square(int num) { return (long)num * num; }

float square(float num) { return num * num; }

double square(double num) { return num * num; }

double linefit(double **x, double **y, double *slope, double *intercept,
               double *sigma, double *correlation, int arraysize) {
  double sumx = 0, sumy = 0, sumxy = 0, sumx_sq = 0, sumy_sq = 0, sumchi_sq = 0,
         average;
  int i;
...
float linefit(float **x, float **y, float *slope, float *intercept,
              float *sigma, float *correlation, int arraysize) {
...
float standard_distribution(float x) {
  float result;
  result =
      (exp((-0.5) * pow((double)x, (double)2.0))) / sqrt((double)(2 * M_PI));
  return result;
}
...
//
void get_1d_limits(double **array, double *lower, double *upper, int count,
                   int start_count) {
  *upper = (*array)[0];
  *lower = (*array)[0];
...
void get_1d_limits(float **array, float *lower, float *upper, int count,
                   int start_count) {
  *upper = (*array)[0];
  *lower = (*array)[0];
...
void get_1d_limits(int **array, int *lower, int *upper, int count,
                   int start_count) {
  *upper = (*array)[0];
  *lower = (*array)[0];
  for (int i = start_count + 1; i < count; i++) {
...
//
void get_2d_limits(double ***array, double *lower, double *upper, int count1,
...
void get_2d_limits(float ***array, float *lower, float *upper, int count1,
                   int count2, int start_count1, int start_count2) {
  *upper = (*array)[start_count1][start_count2];
  *lower = (*array)[start_count1][start_count2];
  for (int i = start_count1; i < count1; i++) {
...
void get_2d_limits(int ***array, int *lower, int *upper, int count1, int count2,
...
void gaussian_smoothing(float **array, unsigned int smooth,
                        unsigned int points) {
  unsigned int i, j, k;
                     ...
                     // goes out to 2 standard deviations
  {
    ...
    for (j = 0; j < smooth; j++)  // loop over all border point centers
    {
      ...
      for (k = j; k < j + smooth; k++) {
        sum += temp_array[k] *
               y_weights[position];  // sum the weighted points right of center,
                                     // including center
      ...
      for (int m = j - 1; m >= 0; m--) {
        position++;
        sum +=
            temp_array[m] * y_weights[position];  // sum all applicable points
                                                  // on the left of center
        sum_y += y_weights[position];
      }
      (*array)[j] =
          (sum / sum_y);  // normalize by the sum of all weights that were used
    }
         ...
         j++)  // loop over all non-border points
    {
      ...
      position = 0;
      for (k = j; k < j + smooth; k++) {
        // cout << "k: " << k << ", position: " << position << ", y_weights: "
        // <<  y_weights[position] << ", points: " << points << endl;
        sum += temp_array[k] *
               y_weights[position];  // sum the weighted points right of center,
                                     // including center
        sum_y += y_weights[position];
         ...
         j++)  // loop over all right-border points
    {
      ...
      for (int m = (int)j - 1; m >= (int)(j - smooth + 1); m--) {
        position++;
        sum += temp_array[m] *
               y_weights[position];  // sum all points on the left of center
        sum_y += y_weights[position];
      }
      position = 0;
      for (k = j; k < points; k++) {
        sum +=
            temp_array[k] * y_weights[position];  // sum the weighted points in
...
void calc_vbar(struct peptide *pep, QString *sequence, float *temperature) {
  ...
  pep->residues += pep->r + pep->l + pep->k + pep->a + pep->v + pep->g +
                   pep->e + pep->n + pep->i + pep->j + pep->q + pep->y +
                   pep->o + pep->p + pep->s + pep->h + pep->t + pep->d +
                   pep->f + pep->b + pep->c + pep->m + pep->w + pep->z +
                   pep->x + pep->dab + pep->dpr;
...
float calc_buoyancy_tb(float vbar20, float temperature, float density20) {
  ...
  density_wt =
      ...
      1.000028e-3 * ((7.9870401e-3 * pow((double)t, (double)2) +
                      46.170461e-6 * pow((double)t, (double)3)) /
                     (1 + 16.87985e-3 * t)) +
      1.000028e-3 * ((105.56302e-9 * pow((double)t, (double)4) -
                      280.54253e-12 * pow((double)t, (double)5)) /
                     (1 + 16.87985e-3 * t));
  density_wt_20 =
      1.000028e-3 * ((999.83952 + 16.945176 * 20) / (1 + 16.879850e-3 * 20)) -
      1.000028e-3 * ((7.9870401e-3 * pow((double)20, (double)2) +
                      46.170461e-6 * pow((double)20, (double)3)) /
                     (1 + 16.87985e-3 * 20)) +
      1.000028e-3 * ((105.56302e-9 * pow((double)20, (double)4) -
...
float calc_bottom(int rotor, int centerpiece, int channel, unsigned int rpm) {
  ...
  if (!readCenterpieceInfo(&cp_list)) {
    ...
    //    "/etc/centerpiece.dat\n\n" "Please install the centerpiece database
...
double stretch_with_rotor_list(int rotor, unsigned int rpm,
                               vector<rotorInfo> *rotor_list) {
  double stretch = 0.0;
  for (unsigned int i = 0; i < 5; i++) {
    stretch +=
        (*rotor_list)[rotor].coefficient[i] * pow((double)rpm, (double)i);
  }
  return (stretch);
}
 ...
 */
int nnls(double *a, int a_dim1, int m, int n,
         /* On entry, a[] contains the m by n matrix A. On exit, a[] contains
            the product matrix Q*A, where Q is an m by n orthogonal matrix
            generated implicitly by this function. Since matrix A is processed
            as a set of vectors, a_dim1 is needed to specify the storage
            increment between vectors in a[] */
         double *b,
  ...
  while (iz1 <= iz2 && nsetp < m) {
    ...
    for (iz = iz1; iz <= iz2; iz++) {
      j = index[iz];
      sm = 0.;
      for (l = npp1; l < m; l++) sm += a[l + j * a_dim1] * b[l];
      w[j] = sm;
    }
    ...
    if (nsetp != m)
      for (l = npp1; l < m; l++) a[l + j * a_dim1] = 0.;
    w[j] = 0.;
      ...
      do {
        ...
        if (jj != (nsetp - 1)) {
          ...
          for (j = jj + 1; j < nsetp; j++) {
            ii = index[j];
            index[j - 1] = ii;
            _nnls_g1(a[j - 1 + ii * a_dim1], a[j + ii * a_dim1], &cc, &ss,
                     &a[j - 1 + ii * a_dim1]);
...
float int_vol_2sphere(float r1, float r2, float d) {
  ...
  float vol =
      (M_PI / 3.0) * ((2.0 * r2 + d - cy) * (cy - d + r2) * (cy - d + r2) +
