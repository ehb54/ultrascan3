...
vector<double> US_Hydrodyn_Dad::conc_curve(vector<double> &t, unsigned int peak,
                                           double conv) {
...
QStringList US_Hydrodyn_Dad::get_frames(QStringList files, QString head,
                                        QString tail) {
  QStringList result;
...
double US_Hydrodyn_Dad::tot_intensity(QString &file, double q_min,
                                      double q_max) {
...
void US_Hydrodyn_Dad::set_created_selected(set<QString> &to_select,
                                           bool do_replot) {
  disable_updates = true;
...
int US_Hydrodyn_Dad::slice(const vector<double> &q, const vector<double> &I,
...
double US_Hydrodyn_Dad::wyatt_errors(QString file, double start_q,
                                     double end_q) {
  if (!f_qs.count(file)) {
    return -1e0;
  }
...
double US_Hydrodyn_Dad::wyatt_errors(QString file, double start_q, double end_q,
                                     vector<double> &new_q,
                                     vector<double> &new_I,
                                     vector<double> &new_y) {
  if (!f_qs.count(file)) {
...
void US_Hydrodyn_Dad::cormap(map<QString, QString> &parameters) {
  ...
  double cormap_maxq =
      ((US_Hydrodyn *)us_hydrodyn)->gparams.count("dad_cormap_maxq")
          ? ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cormap_maxq"].toDouble()
          : 0.05;
...
QStringList US_Hydrodyn_Dad::vector_qstring_to_qstringlist(
    vector<QString> vqs) {
  QStringList qsl;
...
bool US_Hydrodyn_Dad::dad_load(const QString &filename, const QStringList &qsl,
                               QString &errormsg) {
  errormsg = "";
  ...
  if (dad_lambdas.lambdas.size() != absorption_row_size) {
    errormsg =
        QString("UV-Vis absorption column count %1 does not match number of " +
  ...
  switch (QMessageBox::question(
      ...
      dad_lambdas.summary_rich() + QString("<hr>"
...

  {
    ...
    do {
      ...
      dialog.setWindowTitle(
          windowTitle() +
          us_tr(": UV-Vis load : subselect " + UNICODE_LAMBDA_QS + " range"));
      ...
      form.addRow(new QLabel(
          QString(us_tr("Fill out the values below and click OK to subselect\n"
      ...
      vector<QString> labels = {us_tr("Start " + UNICODE_LAMBDA_QS + " [nm]:"),
                                us_tr("End " + UNICODE_LAMBDA_QS + " [nm]:")};

      vector<QWidget *> widgets = {new QLineEdit(&dialog),
...

  {
    ...
    do {
      ...
      vector<double> defaults = {start_time_seconds,
    ...
    if (lambda_crop) {
      for (size_t i = 0; i < absorption_data_size; ++i) {
        ...
        if (!US_Saxs_Util::crop(dad_lambdas.lambdas, absorption_data[i],
                                lambda_start, lambda_end, x_cropped, y_cropped,
                                errormsg)) {
...
//       "Select a file name for saving the UV-Vis Angles" ),  use_dir , "*.csv"
//       );
...
//                                QString(
//                                        us_tr(
//                                              "Error saving UV-Vis Angles file
//                                              %1\n" "Error : %2\n"
//                                              )
//                                        )
//                                .arg( use_filename )
