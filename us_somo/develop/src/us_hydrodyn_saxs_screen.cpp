...
US_Hydrodyn_Saxs_Screen::US_Hydrodyn_Saxs_Screen (csv csv1, void *us_hydrodyn,
    ...
    : QFrame (p)
{
  ...
  for (unsigned int i = 0;
       i < (unsigned int)saxs_window->plotted_I_error.size (); i++)
    {
...
void
US_Hydrodyn_Saxs_Screen::setupGUI ()
{
  int minHeight1 = 30;
  int minHeight1b = 75;
  int minHeight2 = 45;
  ...
  pb_clear_plot_all->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  pb_clear_plot_row->setFont (QFont (USglobal->config_list.fontFamily,
                                     USglobal->config_list.fontSize + 1));
  ...
  pb_push->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize + 1));
  ...
  pb_load_plot->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize + 1));
  ...
  pb_save_plot->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize + 1));
  ...
  cb_plot_average->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  cb_plot_best->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  cb_plot_rg->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize));
  ...
  cb_plot_chi2->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  pb_run_all_targets
      = new QPushButton (us_tr ("Start for all plotted I(q)"), this);
  pb_run_all_targets->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize + 1));
  ...
  lbl_pos_range2->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize + 1,
                                  QFont::Bold));
  ...
  lbl_message->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize + 1,
                               QFont::Bold));
  ...
  lbl_message2->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize + 1,
                                QFont::Bold));
  ...
  lbl_message3->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize + 1,
                                QFont::Bold));
  ...
  lbl_message4->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize + 1,
                                QFont::Bold));
  ...
  qwtw_wheel2->setRange (1.0, 1.0);
  qwtw_wheel2->setSingleStep (1);
  qwtw_wheel2->setMinimumWidth (minHeight2);
  // qwtw_wheel->setTotalAngle( 3600.0 );
  connect (qwtw_wheel2, SIGNAL (valueChanged (double)),
           SLOT (adjust_wheel2 (double)));
  ...
  QHBoxLayout *hbl_plot_buttons = new QHBoxLayout ();
  ...
  QHBoxLayout *hbl_plot_checks = new QHBoxLayout ();
  hbl_plot_checks->setContentsMargins (0, 0, 0, 0);
  hbl_plot_checks->setSpacing (0);
  ...
  gbl_messages->setContentsMargins (0, 0, 0, 0);
  gbl_messages->setSpacing (0);
  ...
  vbl_wheel2->setContentsMargins (0, 0, 0, 0);
  vbl_wheel2->setSpacing (0);
  vbl_wheel2->addWidget (qwtw_wheel2);
  vbl_wheel2->addWidget (lbl_pos_range2);

  QHBoxLayout *hbl_plot_g1 = new QHBoxLayout ();
  hbl_plot_g1->setContentsMargins (0, 0, 0, 0);
  hbl_plot_g1->setSpacing (0);
  hbl_plot_g1->addLayout (vbl_plot_group);
  hbl_plot_g1->addLayout (vbl_wheel2);

  QVBoxLayout *vbl_plot_g2 = new QVBoxLayout (0);
  vbl_plot_g2->setContentsMargins (0, 0, 0, 0);
  vbl_plot_g2->setSpacing (0);
  vbl_plot_g2->addLayout (hbl_plot_g1);
  vbl_plot_g2->addLayout (hbl_plot_checks);
  vbl_plot_g2->addLayout (hbl_plot_buttons);

  QHBoxLayout *hbl_editor_plot = new QHBoxLayout ();
  hbl_editor_plot->setContentsMargins (0, 0, 0, 0);
  hbl_editor_plot->setSpacing (0);
...
void
US_Hydrodyn_Saxs_Screen::cancel ()
{
  close ();
}

void
US_Hydrodyn_Saxs_Screen::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Saxs_Screen::closeEvent (QCloseEvent *e)
{
  if (anything_plotted () && anything_plotted_since_save)
    {
      raise ();
      save_plot ();
    }
...
void
US_Hydrodyn_Saxs_Screen::table_value (int /* row */, int col)
{
...
void
US_Hydrodyn_Saxs_Screen::clear_display ()
{
  editor->clear ();
  editor->append ("\n\n");
}
...
void
US_Hydrodyn_Saxs_Screen::update_font ()
{
  bool ok;
  QFont newFont;
...
void
US_Hydrodyn_Saxs_Screen::set_target (QString scaling_target)
{
          ...
          if (t_csv->item (i, 0)->text ().contains ("Ending q (A^-1)"))
            {
              t_csv->setItem (i, 2,
                              new QTableWidgetItem (
                                  QString ("%1").arg (qs[target_pos][0])));
              t_csv->setItem (
                  i, 3,
                  new QTableWidgetItem (QString ("%1").arg (
                      qs[target_pos][qs[target_pos].size () - 1])));
              recompute_interval_from_points ();
              break;
            }
        }
...
void
US_Hydrodyn_Saxs_Screen::set_target ()
{
...
void
US_Hydrodyn_Saxs_Screen::run_all_targets ()
{
  ...
  for (unsigned int i = 0;
       i < (unsigned int)saxs_window->qsl_plotted_iq_names.size (); i++)
    {
      progress2->setValue (i + 1);
      progress2->setMaximum (saxs_window->qsl_plotted_iq_names.size () + 1);
      editor_msg (
          "blue",
...
void
US_Hydrodyn_Saxs_Screen::start (bool already_running)
{
  if (!validate () || !any_to_run ())
    {
      return;
          ...
          if (t_csv->item (i, 0)->text ().contains ("Radius (A)"))
            {
              ...
              editor_msg ("dark blue",
                          QString ("Radius range %1 to %2 step %3 (A)\n")
                              .arg (min_radius)
                              .arg (max_radius)
                              .arg (delta_radius));
            }
          if (t_csv->item (i, 0)->text ().contains ("Delta rho (A^-3)"))
            {
              ...
              editor_msg ("dark blue",
                          QString ("Delta rho range %1 to %2 step %3 (A^-3)\n")
                              .arg (min_delta_rho)
                              .arg (max_delta_rho)
                              .arg (delta_delta_rho));
            }
          ...
          if (t_csv->item (i, 0)->text ().contains ("Ending q (A^-1)"))
            {
              ...
              points_q = points[i];
              editor_msg ("dark blue",
                          QString ("End q range %1 to %2 step %3 (A^-3)\n")
                              .arg (min_end_q)
                              .arg (max_end_q)
                              .arg (delta_end_q));
            }
          ...
          if (t_csv->item (i, 0)->text ().contains ("Radius (A)"))
            {
              ...
              points_radius = 1;
              editor_msg ("dark red",
                          QString ("Using a fixed radius of %1 (A)\n")
                              .arg (min_radius));
            }
          if (t_csv->item (i, 0)->text ().contains ("Delta rho (A^-3)"))
            {
              ...
              points_delta_rho = 1;
              editor_msg ("dark red",
                          QString ("Using a fixed delta rho of %1 (A^-3)\n")
                              .arg (min_delta_rho));
            }
        }
    }
  ...
  editor_msg (
      "black",
      QString ("Total spheres tested %1\n").arg (total_points / points_q));
  ...
  for (double use_q = min_end_q; use_q <= max_end_q; use_q += delta_end_q)
    {
      ...
      progress->setMaximum (points_q + 1);
      qApp->processEvents ();
      ...
      bool has_error = !usu.iqq_sphere_fit (
          "saxs_screen", target_q, target_I, target_I_errors, min_radius,
          max_radius, delta_radius, min_delta_rho, max_delta_rho,
          delta_delta_rho, (double)saxs_window->our_saxs_options->start_q,
          use_q, by_radius, by_delta_rho, val_radius, val_delta_rho,
          index_radius, index_delta_rho, best_fit_radius, best_fit_delta_rho,
          average_radius, average_delta_rho, start_q, end_q, delta_q,
          use_points_q, best_tag, nnls_tag, cb_normalize->isChecked ());
      ...
      if (!has_error)
        {
          ...
          if (use_chi2)
            {
              usu.scaling_fit (usu.wave[best_tag].r, usu.wave[target].r,
                               usu.wave[target].s, k, chi2_best);
              usu.scaling_fit (usu.wave[nnls_tag].r, usu.wave[target].r,
                               usu.wave[target].s, k, chi2_nnls);
              chi2_best = sqrt (chi2_best / (usu.wave[target].r.size () - 1));
              chi2_nnls = sqrt (chi2_nnls / (usu.wave[target].r.size () - 1));
            }
          else
            {
              usu.scaling_fit (usu.wave[best_tag].r, usu.wave[target].r, k,
                               chi2_best);
              usu.scaling_fit (usu.wave[best_tag].r, usu.wave[target].r, k,
                               chi2_nnls);
          ...
          plot_one (QString (" Target %1 ").arg (lbl_current_target->text ()),
                    ...
                    QString (" q [ %1 : %2 ] / %3 ( %4 )")
                        ...
                        .arg (delta_q, 0, 'g', 4)
                        .arg (use_points_q),
                    radii, intensity, best_fit_radius, best_fit_delta_rho,
                    average_radius, average_delta_rho, Rg, use_chi2, chi2_best,
                    chi2_nnls);
        }
      if (!running)
        {
...
void
US_Hydrodyn_Saxs_Screen::plot_one (
    QString message, QString message2, QString message3, QString message4,
    vector<double> radii, vector<double> intensity, double best_fit_radius,
    double best_fit_delta_rho, double average_radius, double average_delta_rho,
    double rg, bool use_chi2, double chi2_best, double chi2_nnls)
{
...
void
US_Hydrodyn_Saxs_Screen::update_wheel_range ()
{
  ...
  if (messages_size)
    {
      messages_size--;
      {
        double rs = (messages_size * messages_size * 0.1 < 1.0)
                        ? messages_size * messages_size * 0.1
                        : 1.0;
        qwtw_wheel->setRange (-0.5, (double)messages_size + 0.5);
    ...
    {
      qwtw_wheel->setRange (0.0, 0.0);
      qwtw_wheel->setSingleStep (1.0);
    }
}

void
US_Hydrodyn_Saxs_Screen::plot_row (unsigned int i)
{
  if (messages.size () <= i)
    {
      return;
...
void
US_Hydrodyn_Saxs_Screen::plot_pos (unsigned int i)
{
  ...
  lbl_pos_range->setText (
      QString ("%1 of %2").arg (i + 1).arg (messages[current_row].size ()));
  ...
  plot_dist->setCurveData (curvekey, (double *)&(radiis[current_row][i][0]),
  ...
  if (cb_plot_best->isChecked ())
    {
      ...
      plot_dist->setMarkerLabelText (
          qpmkey,
          QString ("Best individual\nfit at %1%2")
              .arg (best_fit_radiuss[current_row][i])
              .arg (
                  cb_plot_chi2->isChecked ()
                      ? QString ("\n%1 = %2")
      ...
      marker1->setSymbol (new QwtSymbol (QwtSymbol::VLine, QBrush (Qt::white),
  ...
  if (cb_plot_average->isChecked ())
    {
      ...
      plot_dist->setMarkerLabelAlign (qpmkey2, cb_plot_best->isChecked ()
                                                   ? AlignLeft | AlignTop
                                                   : AlignRight | AlignTop);
      plot_dist->setMarkerPen (qpmkey2,
                               QPen (QColor (255, 141, 0), 2, DashLine));
      ...
      marker2->setSymbol (new QwtSymbol (
          QwtSymbol::VLine, QBrush (Qt::white),
          QPen (QColor (255, 141, 0), 2, Qt::DashLine), QSize (8, sizeym)));
  ...
  if (cb_plot_rg->isChecked () && target_rgs[current_row][i] != 0e0)
    {
#if QT_VERSION < 0x040000
      ...
      marker3->setSymbol (new QwtSymbol (QwtSymbol::VLine, QBrush (Qt::white),
                                         QPen (Qt::cyan, 2, Qt::DashLine),
                                         QSize (8, sizeym)));
...
void
US_Hydrodyn_Saxs_Screen::update_enables ()
{
...
bool
US_Hydrodyn_Saxs_Screen::any_to_run ()
{
  for (unsigned int i = 0; i < (unsigned int)t_csv->rowCount (); i++)
    {
      if (((QCheckBox *)(t_csv->cellWidget (i, 1)))->isChecked ()
  ...
  editor_msg ("dark red", "Nothing to do: At least one of the non \"Ending "
                          "q\" rows must have Active set to Y");
  return false;
}

void
US_Hydrodyn_Saxs_Screen::editor_msg (QString color, QString msg)
{
...
void
US_Hydrodyn_Saxs_Screen::editor_msg_qc (QColor qcolor, QString msg)
{
  QColor save_color = editor->textColor ();
...
bool
US_Hydrodyn_Saxs_Screen::validate ()
{
  bool errors = false;
  ...
  for (unsigned int i = 0; i < (unsigned int)t_csv->rowCount (); i++)
    {
      ...
      // if ( t_csv->item( i, 4 )->text().toDouble() == 1  &&
      // t_csv->text( i, 2 ).toDouble() > t_csv->item( i, 3
      // )->text().toDouble() )
      // {
...
bool
US_Hydrodyn_Saxs_Screen::activate_saxs_window ()
{
  if (!*saxs_widget)
    {
...
bool
US_Hydrodyn_Saxs_Screen::validate_saxs_window ()
{
...
void
US_Hydrodyn_Saxs_Screen::do_replot_saxs ()
{
...
void
US_Hydrodyn_Saxs_Screen::replot_saxs ()
{
  if (activate_saxs_window ())
    {
      do_replot_saxs ();
...
void
US_Hydrodyn_Saxs_Screen::save_saxs_plot ()
{
  if (!activate_saxs_window ())
    {
      return;
  ...
  if (!saxs_window->plotted_q.size ())
    {
      QMessageBox::information (this, us_tr ("US-SOMO: I(q) screen"),
  ...
  if (add_target)
    {
      ...
      if (!add_target)
        {
          QMessageBox::warning (this, "US_SOMO: I(q) screen",
...
csv
US_Hydrodyn_Saxs_Screen::current_csv ()
{
  csv tmp_csv = csv1;
...
void
US_Hydrodyn_Saxs_Screen::recompute_interval_from_points ()
{
  for (unsigned int i = 0; i < (unsigned int)t_csv->rowCount (); i++)
    {
      QString toset
...
void
US_Hydrodyn_Saxs_Screen::recompute_points_from_interval ()
{
  for (unsigned int i = 0; i < (unsigned int)t_csv->rowCount (); i++)
    {
      QString toset
...
void
US_Hydrodyn_Saxs_Screen::adjust_wheel (double pos)
{
...
void
US_Hydrodyn_Saxs_Screen::adjust_wheel2 (double pos)
{
...
US_Hydrodyn_Saxs_Screen::push ()
{
  ...
  qwtw_wheel->setSingleStep (1);
  {
    double rs = (current_row * current_row * 0.1 < 1.0)
                    ...
                    : 1.0;
    qwtw_wheel2->setRange ((double)current_row + 0.5, 0.5);
    qwtw_wheel2->setSingleStep (rs);
  }
  plot_row (current_row);
}

void
US_Hydrodyn_Saxs_Screen::clear_plot ()
{
  plot_dist->detachItems (QwtPlotItem::Rtti_PlotCurve);
  plot_dist->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
...
void
US_Hydrodyn_Saxs_Screen::clear_plot_row ()
{
...
void
US_Hydrodyn_Saxs_Screen::clear_plot_all ()
{
...
bool
US_Hydrodyn_Saxs_Screen::anything_plotted ()
{
...
void
US_Hydrodyn_Saxs_Screen::save_plot ()
{
  if (!anything_plotted ())
    {
  ...
  QString filename = QFileDialog::getSaveFileName (
      this, us_tr ("Choose a filename to save the plots"), use_dir,
      "*.ssc *.SSC");
...
void
US_Hydrodyn_Saxs_Screen::load_plot ()
{
  if (!validate_saxs_window ())
    {
      return;
  ...
  for (QStringList::iterator it = qsl_h.begin (); it != qsl_h.end (); it++)
    {
      ...
      if (csv1.header_map.count (qs))
        {
          QMessageBox::information (
              this, us_tr ("US-SOMO: SAXS screen load plots "),
...
csv
US_Hydrodyn_Saxs_Screen::plots_to_csv ()
{
  csv plot_csv;
...
void
US_Hydrodyn_Saxs_Screen::csv_to_plots (csv plot_csv)
{
  ...
  for (unsigned int i = 0; i < plot_csv.data.size (); i++)
    {
      if (plot_csv.data[i].size () < 12)
        {
          QMessageBox::information (
              this, us_tr ("US-SOMO: SAXS screen"),
              QString (us_tr (
                  "US_Hydrodyn_Saxs_Screen::csv_to_plots invalid csv")));
          return;
        }
      ...
      best_fit_radiuss[current_row].push_back (
          plot_csv.data[i][pos++].toDouble ());
      best_fit_delta_rhos[current_row].push_back (
          plot_csv.data[i][pos++].toDouble ());
      average_radiuss[current_row].push_back (
          plot_csv.data[i][pos++].toDouble ());
      average_delta_rhos[current_row].push_back (
          plot_csv.data[i][pos++].toDouble ());
      target_rgs[current_row].push_back (plot_csv.data[i][pos++].toDouble ());
      use_chi2s[current_row].push_back (
          (bool)plot_csv.data[i][pos++].toInt ());
...
QStringList
US_Hydrodyn_Saxs_Screen::csv_parse_line (QString qs)
{
...
bool
US_Hydrodyn_Saxs_Screen::get_guinier_rg (QString name, double &Rg)
{
  ...
  if (!too_few_points)
    {
      if (!usu.guinier_plot ("guinier", "data")
          || !usu.guinier_fit2 (log, "guinier", pointsmin, pointsmax,
                                sRgmaxlimit, pointweightpower, p_guinier_maxq,
                                a, b, siga, sigb, chi2, Rg, Io,
  ...
  if (too_few_points)
  ...
  else
    {
      if (us_isnan (Rg) || b >= 0e0)
        {
          editor_msg ("red", QString ("Guinier analysis of %1:\n"
      ...
      else
        {
          editor_msg ("blue",
                      QString ("").sprintf (
                          ...
                          "Rg %.1f (%.1f) (A) I(0) %.2e (%.2e) qRgmin %.3f "
                          "qRgmax %.3f points used %u chi^2 %.2e\n"

                          ,
                          name.toLatin1 ().data (), Rg,
                          sqrt (3e0) * 5e-1 * (1e0 / sqrt (-b)) * sigb, Io,
                          siga, sRgmin, sRgmax, bestend - beststart + 1,
                          chi2));
          guinier_rgs[name] = Rg;
          return true;
        }
...
void
US_Hydrodyn_Saxs_Screen::replot ()
{
  plot_pos (last_plotted_pos);
}

void
US_Hydrodyn_Saxs_Screen::usp_config_plot_dist (const QPoint &)
{
