...
void US_Hydrodyn_Saxs_Screen::cancel() { close(); }
  ...
  if (target_found) {
    for (unsigned int i = 0; i < (unsigned int)t_csv->rowCount(); i++) {
      if (t_csv->item(i, 0)->text().contains("Ending q (A^-1)")) {
        t_csv->setItem(
            i, 2, new QTableWidgetItem(QString("%1").arg(qs[target_pos][0])));
        t_csv->setItem(i, 3,
...
void US_Hydrodyn_Saxs_Screen::start(bool already_running) {
  ...
  for (double use_q = min_end_q; use_q <= max_end_q; use_q += delta_end_q) {
    ...
    bool has_error = !usu.iqq_sphere_fit(
        ...
        max_radius, delta_radius, min_delta_rho, max_delta_rho, delta_delta_rho,
        (double)saxs_window->our_saxs_options->start_q, use_q, by_radius,
        by_delta_rho, val_radius, val_delta_rho, index_radius, index_delta_rho,
        best_fit_radius, best_fit_delta_rho, average_radius, average_delta_rho,
        start_q, end_q, delta_q, use_points_q, best_tag, nnls_tag,
    ...
    if (!has_error) {
      ...
      if (use_chi2) {
        ...
        usu.scaling_fit(usu.wave[best_tag].r, usu.wave[target].r, k, chi2_best);
        usu.scaling_fit(usu.wave[best_tag].r, usu.wave[target].r, k, chi2_nnls);
...
void US_Hydrodyn_Saxs_Screen::plot_pos(unsigned int i) {
  ...
  if (cb_plot_average->isChecked()) {
    ...
    marker2->setSymbol(new QwtSymbol(QwtSymbol::VLine, QBrush(Qt::white),
                                     QPen(QColor(255, 141, 0), 2, Qt::DashLine),
                                     QSize(8, sizeym)));
...
bool US_Hydrodyn_Saxs_Screen::any_to_run() {
  ...
  editor_msg("dark red",
             "Nothing to do: At least one of the non \"Ending q\" rows must "
             "have Active set to Y");
  return false;
}
...
void US_Hydrodyn_Saxs_Screen::save_plot() {
  if (!anything_plotted()) {
    QMessageBox::information(this, us_tr("US-SOMO: SAXS screen save plots "),
                             QString(us_tr("Nothing plotted")));
    return;
  }
...
void US_Hydrodyn_Saxs_Screen::load_plot() {
  ...
  saxs_window->select_from_directory_history(use_dir, this);
  QString filename =
      QFileDialog::getOpenFileName(this, windowTitle(), use_dir, "*.ssc *.SSC");
  ...
  if (!csv1.data.size()) {
    QMessageBox::information(this, us_tr("US-SOMO: SAXS screen load plots "),
...
bool US_Hydrodyn_Saxs_Screen::get_guinier_rg(QString name, double &Rg) {
  ...
  if (!too_few_points) {
    if (!usu.guinier_plot("guinier", "data") ||
        !usu.guinier_fit2(log, "guinier", pointsmin, pointsmax, sRgmaxlimit,
    ...
    if (us_isnan(Rg) || b >= 0e0) {
      ...
      editor_msg("blue",
                 QString("").sprintf(
                     ...
                     "Rg %.1f (%.1f) (A) I(0) %.2e (%.2e) qRgmin %.3f qRgmax "
                     "%.3f points used %u chi^2 %.2e\n"

                     ,
                     name.toLatin1().data(), Rg,
                     sqrt(3e0) * 5e-1 * (1e0 / sqrt(-b)) * sigb, Io, siga,
                     sRgmin, sRgmax, bestend - beststart + 1, chi2));
      guinier_rgs[name] = Rg;
      return true;
    }
  }
...
void US_Hydrodyn_Saxs_Screen::replot() { plot_pos(last_plotted_pos); }
