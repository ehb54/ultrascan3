...
// still some hiccup on gauss replots w/residuals show:hide under
// suppress_replot control

#define SLASH QDir::separator()

US_Hydrodyn_Mals_Saxs_Movie::US_Hydrodyn_Mals_Saxs_Movie(
  ...
  mals_saxs_win->lb_files->clearSelection();

  pos = 0;
  last_pos = -1;

  timer_msec = ush_win->gparams.count("mals_saxs_movie_timer_ms")
                   ? ush_win->gparams["mals_saxs_movie_timer_ms"].toInt()
                   : 1000;

  last_show_gauss =
      ush_win->gparams.count("mals_saxs_movie_show_gauss") &&
              ush_win->gparams["mals_saxs_movie_show_gauss"] == "true"
          ? true
          : false;

  last_show_ref = ush_win->gparams.count("mals_saxs_movie_show_ref") &&
                          ush_win->gparams["mals_saxs_movie_show_ref"] == "true"
                      ? true
                      : false;

  last_mono = ush_win->gparams.count("mals_saxs_movie_mono") &&
                      ush_win->gparams["mals_saxs_movie_mono"] == "true"
                  ? true
                  : false;
  ...
  mals_saxs_win->suppress_replot = true;
  plot_colors = mals_saxs_win->plot_colors;
  set_mono();
  update_plot();
  show();
  //    cout << "doin it\n";
...
void US_Hydrodyn_Mals_Saxs_Movie::setupGUI() {
  int minHeight1 = 24;
  int minHeight3 = 25;

  // ------ data section

  lbl_state = new QLabel(QString(us_tr("Stopped: %1 of %2")
                                     .arg(pos + 1)
                                     .arg(mals_saxs_selected_files.size())),
                         this);
  ...
  AUTFBACK(lbl_state);
  lbl_state->setFont(QFont(USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize - 1, QFont::Bold));

  lbl_current = new QLabel(
      mals_saxs_win->lb_files->item(mals_saxs_selected_files[pos])->text(),
      this);
  ...
  lbl_current->setFont(QFont(USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize - 1, QFont::Bold));

  pb_front = new QPushButton("|<", this);
  pb_front->setFont(QFont(USglobal->config_list.fontFamily,
  ...
  pb_prev->setFont(QFont(USglobal->config_list.fontFamily,
                         USglobal->config_list.fontSize - 1));
  ...
  pb_slower->setFont(QFont(USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize - 1));
  ...
  pb_faster->setFont(QFont(USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize - 1));
  ...
  pb_next->setFont(QFont(USglobal->config_list.fontFamily,
                         USglobal->config_list.fontSize - 1));
  ...
  pb_end->setFont(QFont(USglobal->config_list.fontFamily,
                        USglobal->config_list.fontSize - 1));
  ...
  cb_show_gauss->setFont(QFont(USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize - 1));
  ...
  cb_show_ref->setFont(QFont(USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize - 1));
  ...
  cb_mono->setFont(QFont(USglobal->config_list.fontFamily,
                         USglobal->config_list.fontSize - 1));
  ...
  cb_save->setFont(QFont(USglobal->config_list.fontFamily,
                         USglobal->config_list.fontSize - 1));
  cb_save->setPalette(PALET_NORMAL);
  AUTFBACK(cb_save);
  connect(cb_save, SIGNAL(clicked()), SLOT(set_save()));

  le_save = new QLineEdit(this);
  le_save->setObjectName("le_save Line Edit");
  le_save->setText("");
  le_save->setAlignment(Qt::AlignCenter | Qt::AlignVCenter);
  le_save->setPalette(PALET_NORMAL);
  AUTFBACK(le_save);
  le_save->setFont(QFont(USglobal->config_list.fontFamily,
                         USglobal->config_list.fontSize - 1));
  ...
  cb_save_overwrite->setFont(QFont(USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize - 1));
...

  {
    ...
    bl_tl->setContentsMargins(0, 0, 0, 0);
    bl_tl->setSpacing(0);
...
void US_Hydrodyn_Mals_Saxs_Movie::cancel() { close(); }

void US_Hydrodyn_Mals_Saxs_Movie::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/somo_mals_saxs_movie.html");
}

void US_Hydrodyn_Mals_Saxs_Movie::closeEvent(QCloseEvent *e) {
  mals_saxs_win->plot_colors = plot_colors;
  mals_saxs_win->disable_updates = true;
  if (mals_saxs_win->current_mode == mals_saxs_win->MODE_GAUSSIAN) {
    mals_saxs_win->wheel_cancel();
  }
  mals_saxs_win->lb_files->clearSelection();
  for (int i = 0; i < (int)mals_saxs_selected_files.size(); ++i) {
    mals_saxs_win->lb_files->item(mals_saxs_selected_files[i])
        ->setSelected(true);
  }
  mals_saxs_win->suppress_replot = false;
  mals_saxs_win->plot_files();
  mals_saxs_win->disable_updates = false;

  ush_win->gparams["mals_saxs_movie_timer_ms"] = QString("%1").arg(timer_msec);
  ush_win->gparams["mals_saxs_movie_show_gauss"] =
      cb_show_gauss->isChecked() ? "true" : "false";
  ush_win->gparams["mals_saxs_movie_show_ref"] =
      cb_show_ref->isChecked() ? "true" : "false";
  e->accept();
}

void US_Hydrodyn_Mals_Saxs_Movie::front() {
  pos = 0;
  update_plot();
}

void US_Hydrodyn_Mals_Saxs_Movie::prev() {
  if (pos > 0) {
    pos--;
    update_plot();
  }
...
void US_Hydrodyn_Mals_Saxs_Movie::slower() {
  timer_msec *= 2;
  if (timer_msec > 5000) {
    timer_msec = 5000;
  }
...
void US_Hydrodyn_Mals_Saxs_Movie::start() {
  if (timer->isActive()) {
    timer->stop();
    pb_start->setText("[]");
    lbl_state->setText(
        QString(us_tr("%1: %2 of %3")
                    .arg(us_tr(timer->isActive() ? "Running" : "Stopped"))
                    .arg(pos + 1)
                    .arg(mals_saxs_selected_files.size())));
  } else {
...
void US_Hydrodyn_Mals_Saxs_Movie::faster() {
  timer_msec /= 2;
  if (timer_msec < 50) {
    timer_msec = 50;
  }
...
void US_Hydrodyn_Mals_Saxs_Movie::next() {
  if (pos < (int)mals_saxs_selected_files.size() - 1) {
    pos++;
    update_plot();
  } else {
    if (timer->isActive()) {
...
void US_Hydrodyn_Mals_Saxs_Movie::end() {
  pos = mals_saxs_selected_files.size() - 1;
  update_plot();
}

void US_Hydrodyn_Mals_Saxs_Movie::set_save() { update_enables(); }

void US_Hydrodyn_Mals_Saxs_Movie::set_show_gauss() { update_plot(); }

void US_Hydrodyn_Mals_Saxs_Movie::set_show_ref() { update_plot(); }

void US_Hydrodyn_Mals_Saxs_Movie::set_mono() {
  if (cb_mono->isChecked()) {
    mals_saxs_win->plot_colors.clear();
    mals_saxs_win->plot_colors.push_back(plot_colors[0]);
  } else {
    mals_saxs_win->plot_colors = plot_colors;
  }
  update_plot();
...
void US_Hydrodyn_Mals_Saxs_Movie::update_enables() {
  le_save->setEnabled(cb_save->isChecked());
}

void US_Hydrodyn_Mals_Saxs_Movie::update_plot() {
  if (last_pos != pos || cb_show_gauss->isChecked() != last_show_gauss ||
    ...
    if (cb_show_ref->isChecked()) {
      mals_saxs_win->plot_ref->show();
    } else {
      mals_saxs_win->plot_ref->hide();
    }
    mals_saxs_win->lb_files->item(mals_saxs_selected_files[pos])
        ->setSelected(true);
    ...
    if (cb_show_gauss->isChecked() &&
        mals_saxs_win->f_is_time.count(
            mals_saxs_win->lb_files->item(mals_saxs_selected_files[pos])
                ->text()) &&
        mals_saxs_win->f_is_time[mals_saxs_win->lb_files
                                     ->item(mals_saxs_selected_files[pos])
                                     ->text()]) {
      mals_saxs_win->gauss_start();
    }
    // else {
    // mals_saxs_win->plot_files();
    // }

    mals_saxs_win->plot_dist->replot();
    mals_saxs_win->plot_ref->replot();
    mals_saxs_win->plot_errors->replot();
    lbl_state->setText(
        QString(us_tr("%1: %2 of %3")
                    .arg(us_tr(timer->isActive() ? "Running" : "Stopped"))
                    .arg(pos + 1)
...
void US_Hydrodyn_Mals_Saxs_Movie::save_plot() {
  if (mals_saxs_win->plot_errors->isVisible()) {
    if (mals_saxs_win->plot_ref->isVisible()) {
      save_plot(mals_saxs_win->plot_dist, mals_saxs_win->plot_ref,
                mals_saxs_win->plot_errors, le_save->text());
    } else {
      save_plot(mals_saxs_win->plot_dist, mals_saxs_win->plot_errors,
                le_save->text());
    }
  } else {
    if (mals_saxs_win->plot_ref->isVisible()) {
      ...
      save_plot(mals_saxs_win->plot_dist, le_save->text());
    }
  }
}

void US_Hydrodyn_Mals_Saxs_Movie::save_plot(QWidget *plot, QString tag) {
  int mypos = pos;
...
void US_Hydrodyn_Mals_Saxs_Movie::join_maps(QPixmap &m1, QPixmap &m2) {
  int m1h = m1.height();
  int m2h = m2.height();
...
#else
  m1 = m1.copy(0, 0, m1.width() > m2.width() ? m1.width() : m2.width(),
               m1h + m2h);
#endif
  QPainter paint(&m1);
...
void US_Hydrodyn_Mals_Saxs_Movie::save_plot(QWidget *plot, QWidget *plot2,
                                            QWidget *plot3, QString tag) {
  int mypos = pos;
  QPixmap qPix = plot->grab();
...
void US_Hydrodyn_Mals_Saxs_Movie::save_plot(QPixmap &qPix, QString tag,
                                            int mypos) {
  QPainter paint(&qPix);
  paint.setPen(Qt::blue);
  paint.setFont(QFont(USglobal->config_list.fontFamily,
                      USglobal->config_list.fontSize - 1, QFont::Bold));
  // paint.drawText( 5, 5, mals_saxs_win->lb_files->item(
  // mals_saxs_selected_files[ mypos ] )->text() );
  paint.drawText(
      qPix.rect(), Qt::AlignBottom | Qt::AlignLeft,
      mals_saxs_win->lb_files->item(mals_saxs_selected_files[mypos])->text());
  ...
  QString f(ush_win->somo_dir + QDir::separator() + "saxs" + QDir::separator() +
            "tmp" + QDir::separator() + tag + "_" + frame);
