    ...
    : QDialog (p)
{
  ...
  last_show_ref
      = ush_win->gparams.count ("mals_saxs_movie_show_ref")
...
void
US_Hydrodyn_Mals_Saxs_Movie::setupGUI ()
{
  int minHeight1 = 24;
  int minHeight3 = 25;
  ...
  lbl_state
      = new QLabel (QString (us_tr ("Stopped: %1 of %2")
                                 .arg (pos + 1)
  ...
  rb_save_png->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  rb_save_jpeg->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  rb_save_bmp->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
...
void
US_Hydrodyn_Mals_Saxs_Movie::cancel ()
{
  close ();
}

void
US_Hydrodyn_Mals_Saxs_Movie::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Mals_Saxs_Movie::closeEvent (QCloseEvent *e)
{
  ...
  ush_win->gparams["mals_saxs_movie_timer_ms"]
      = QString ("%1").arg (timer_msec);
...
void
US_Hydrodyn_Mals_Saxs_Movie::front ()
{
  pos = 0;
  update_plot ();
}
...
void
US_Hydrodyn_Mals_Saxs_Movie::prev ()
{
  if (pos > 0)
    {
      pos--;
...
void
US_Hydrodyn_Mals_Saxs_Movie::slower ()
{
  timer_msec *= 2;
  if (timer_msec > 5000)
    {
...
void
US_Hydrodyn_Mals_Saxs_Movie::start ()
{
  if (timer->isActive ())
    {
      timer->stop ();
...
void
US_Hydrodyn_Mals_Saxs_Movie::faster ()
{
  timer_msec /= 2;
  if (timer_msec < 50)
    {
...
void
US_Hydrodyn_Mals_Saxs_Movie::next ()
{
...
void
US_Hydrodyn_Mals_Saxs_Movie::end ()
{
...
void
US_Hydrodyn_Mals_Saxs_Movie::set_save ()
{
  update_enables ();
}

void
US_Hydrodyn_Mals_Saxs_Movie::set_show_gauss ()
{
  update_plot ();
}

void
US_Hydrodyn_Mals_Saxs_Movie::set_show_ref ()
{
  update_plot ();
}

void
US_Hydrodyn_Mals_Saxs_Movie::set_mono ()
{
...
void
US_Hydrodyn_Mals_Saxs_Movie::update_enables ()
{
  le_save->setEnabled (cb_save->isChecked ());
}

void
US_Hydrodyn_Mals_Saxs_Movie::update_plot ()
{
      ...
      if (cb_show_gauss->isChecked ()
          && mals_saxs_win->f_is_time.count (
              mals_saxs_win->lb_files->item (mals_saxs_selected_files[pos])
                  ->text ())
          && mals_saxs_win
                 ->f_is_time[mals_saxs_win->lb_files
...
void
US_Hydrodyn_Mals_Saxs_Movie::save_plot ()
{
...
void
US_Hydrodyn_Mals_Saxs_Movie::save_plot (QWidget *plot, QString tag)
{
  int mypos = pos;
...
void
US_Hydrodyn_Mals_Saxs_Movie::join_maps (QPixmap &m1, QPixmap &m2)
{
  int m1h = m1.height ();
  int m2h = m2.height ();
...
void
US_Hydrodyn_Mals_Saxs_Movie::save_plot (QWidget *plot, QWidget *plot2,
...
void
US_Hydrodyn_Mals_Saxs_Movie::save_plot (QPixmap &qPix, QString tag, int mypos)
{
  QPainter paint (&qPix);
  ...
  paint.drawText (
      ...
      mals_saxs_win->lb_files->item (mals_saxs_selected_files[mypos])
          ->text ());
  ...
  QString f (ush_win->somo_dir + QDir::separator () + "saxs"
             + QDir::separator () + "tmp" + QDir::separator () + tag + "_"
             + frame);
