...
bool
US_PM::set_grid_size (double grid_conversion_factor, bool quiet)
{
  /*
#if defined( USE_MPI )
  if ( us_log )
...
grid_conversion_factor ) );
  ...
  */
  ...
  if (grid_conversion_factor < 0.5)
    {
      error_msg = QString ("set_grid_size requested size %1 is less than the "
                           "minimum allowed 0.5")
  ...
  cube_size = grid_conversion_factor * grid_conversion_factor
              * grid_conversion_factor;
...
bool
US_PM::reset_grid_size (bool quiet)
{
  return set_grid_size (org_conversion_factor, quiet);
}

US_PM::US_PM (double grid_conversion_factor, int max_dimension,
              unsigned int max_harmonics, vector<double> F, vector<double> q,
              vector<double> I, vector<double> e, unsigned int max_mem_in_MB,
              int debug_level, bool quiet, US_Log *us_log,
              US_Udp_Msg *us_udp_msg)
{
  ...
  this->max_dimension = this->max_dimension < USPM_MAX_VAL
                            ? this->max_dimension
                            : USPM_MAX_VAL;
  if (this->max_dimension != max_dimension)
    {
      if (us_log)
        {
          us_log->log (
              QString ("Warning: maximum dimension requested %1 exceedes "
                       "maximum allowed %2, reset to maximum allowed\n")
                  .arg (max_dimension)
  ...
  org_cube_size = grid_conversion_factor * grid_conversion_factor
                  * grid_conversion_factor;
  ...
  if (!quiet && us_log)
    {
      us_log->log (QString ("q_Y_points %1\n").arg (q_Y_points)
                   + QString ("A1v0 size  %1\n").arg (A1v0.size ()));
    }
  ...
  if (use_errors)
    {
      ...
      if (!quiet && us_log && !use_errors && any_non_zero)
        {
          us_log->log ("Notice: SD's provided but some were zero, so SD "
                       "fitting is turned off\n");
        }
    }
  ...
  if (!quiet && us_log)
    {
      us_log->log (QString ("bytes per pm_data %1\n")
                       .arg (bytes_per_pm_data)
                       .toLatin1 ()
                       .data ());
      us_log->log (QString ("bytes per pmc data %1\n")
                       .arg (bytes_per_pmc_data)
                       .toLatin1 ()
                       .data ());
...
QString
US_PM::qs_bead_model (set<pm_point> &model)
{
  ...
  if (last_best_rmsd_ok)
    {
      qs += QString ("%1 to target data: %2\n")
                .arg (use_errors ? "Chi^2" : "RMSD")
                .arg (sqrt (last_best_rmsd2));
    }
...
vector<PDB_atom>
US_PM::bead_model (set<pm_point> &model)
{
...
vector<point>
US_PM::point_model (set<pm_point> &model)
{
  vector<point> result;
...
bool
US_PM::create_model (vector<double> params, set<pm_point> &model,
                     bool only_last_model)
{
  debug (1, QString ("create_model%1")
                .arg (only_last_model ? ". Note: only last model saved" : ""));
  model.clear ();
...
void
US_PM::debug (int level, QString qs)
{
...
set<pm_point>
US_PM::recenter (set<pm_point> &model)
{
...
bool
US_PM::rotation_matrix (double l, double m, double n, double theta,
...
bool
US_PM::apply_rotation_matrix (vector<vector<double> > &rm, int x, int y, int z,
...
bool
US_PM::check_limits (vector<double> &fparams, vector<double> &low_fparams,
                     vector<double> &high_fparams)
{
  for (unsigned int i = 0; i < (unsigned int)fparams.size (); i++)
    {
      if (fparams[i] < low_fparams[i] || fparams[i] > high_fparams[i])
        {
          return false;
        }
    }
...
bool
US_PM::clip_limits (vector<double> &fparams, vector<double> &low_fparams,
                    vector<double> &high_fparams)
{
  bool clipped = false;
  for (unsigned int i = 0; i < (unsigned int)fparams.size (); i++)
...
bool
US_PM::qstring_model (QString &out, set<pm_point> &model)
{
  double rg = Rg (model);
  out = qs_bead_model (model);
  out += "Model scale (10^-x m) (10 = Angstrom, 9 = nanometer), where x is : "
         "10\n";
  out += QString ("Rg: %1\n").arg (rg);
  return true;
}
...
bool
US_PM::write_model (QString &filename, set<pm_point> &model, bool overwrite)
{
  if (!overwrite)
    {
      int ext = 0;
      ...
      while (QFile::exists (use_filename))
        {
          use_filename
              = QString ("%1-%2.bead_model").arg (filename).arg (++ext);
        }
  ...
  ts << "Model scale (10^-x m) (10 = Angstrom, 9 = nanometer), where x is : "
        "10\n";
  ts << QString ("Rg: %1\n").arg (rg);
  of.close ();
  filename.replace (QRegExp ("\\.bead_model$"), "");
...
bool
US_PM::write_model (QString &filename, set<pm_point> &model,
                    vector<double> &params, bool overwrite)
{
  if (!overwrite)
    {
      int ext = 0;
...
bool
US_PM::write_I (QString &filename, set<pm_point> &model, bool overwrite)
{
...
QString
US_PM::tmp_name (QString basename, vector<double> &params)
{
  QString fname = QString ("/tmp/somo/%1%2_")
                      .arg (basename)
...
QString
US_PM::physical_stats (set<pm_point> &model)
{
  if (!model.size ())
    {
      return "empty model\n";
  ...
  // just to stop silly compiler warning (may be used uninitialized in this
  // function)
  for (unsigned int j = 0; j < 3; j++)
    {
      pmin.axis[j] = pmax.axis[j] = prmin.axis[j] = prmax.axis[j] = 0.0f;
    }
  ...
  for (unsigned int i = 0; i < (unsigned int)adjusted_model.size (); ++i)
    {
      ...
      // volume += ( 4e0 / 3e0 ) * M_PI * bead_radius * bead_radius *
      // bead_radius;
    }
  ...
  //    for ( unsigned int i = 0; i < ( unsigned int ) adjusted_model.size() -
  //    1; ++i )
  //    {
  //       for ( unsigned int j = i + 1; j < ( unsigned int )
  //       adjusted_model.size(); ++j )
  //       {
  ...
  // last_physical_stats[ "result intersection volume"         ] = QString(
  // "%1" ).arg( volume_intersection ); last_physical_stats[ "result excluded
  // volume"             ] = QString( "%1" ).arg( volume - volume_intersection
  // );
  last_physical_stats["result centers bounding box size x"]
      = QString ("%1").arg (pmax.axis[0] - pmin.axis[0]);
  last_physical_stats["result centers bounding box size y"]
      = QString ("%1").arg (pmax.axis[1] - pmin.axis[1]);
  last_physical_stats["result centers bounding box size z"]
  ...
  last_physical_stats["approx. Shannon channels"]
      = QString ("%1").arg (approx_Dmax * q.back () / M_PI);

  last_physical_stats["result centers axial ratios x:z"] = QString ("%1").arg (
      (pmax.axis[0] - pmin.axis[0]) / (pmax.axis[2] - pmin.axis[2]));
  last_physical_stats["result centers axial ratios x:y"] = QString ("%1").arg (
      (pmax.axis[0] - pmin.axis[0]) / (pmax.axis[1] - pmin.axis[1]));
  last_physical_stats["result centers axial ratios y:z"] = QString ("%1").arg (
      (pmax.axis[1] - pmin.axis[1]) / (pmax.axis[2] - pmin.axis[2]));

  last_physical_stats["result radial extent bounding box size x"]
      = QString ("%1").arg (prmax.axis[0] - prmin.axis[0]);
  last_physical_stats["result radial extent bounding box size y"]
      = QString ("%1").arg (prmax.axis[1] - prmin.axis[1]);
  last_physical_stats["result radial extent bounding box size z"]
      = QString ("%1").arg (prmax.axis[2] - prmin.axis[2]);

  last_physical_stats["result radial extent axial ratios x:z"]
      = QString ("%1").arg ((prmax.axis[0] - prmin.axis[0])
                            / (prmax.axis[2] - prmin.axis[2]));
  last_physical_stats["result radial extent axial ratios x:y"]
      = QString ("%1").arg ((prmax.axis[0] - prmin.axis[0])
                            / (prmax.axis[1] - prmin.axis[1]));
  last_physical_stats["result radial extent axial ratios y:z"]
      = QString ("%1").arg ((prmax.axis[1] - prmin.axis[1])
                            / (prmax.axis[2] - prmin.axis[2]));
  ...
  qs += QString ("centers axial ratios: [x:z] = %1  [x:y] = %2  [y:z] = %3\n")
            .arg ((pmax.axis[0] - pmin.axis[0])
                  / (pmax.axis[2] - pmin.axis[2]))
            .arg ((pmax.axis[0] - pmin.axis[0])
                  / (pmax.axis[1] - pmin.axis[1]))
            .arg ((pmax.axis[1] - pmin.axis[1])
                  / (pmax.axis[2] - pmin.axis[2]));
...
bool
US_PM::zero_md0_params (vector<double> &params, double max_d)
{
  ...
  if (this_type >= (int)object_names.size () || this_type < 0)
    {
...
void
US_PM::random_md0_params (vector<double> &params, double max_d)
{
  ...
  for (int i = 0; i < object_m0_parameters[this_type]; i++)
    {
      params[i + 1]
          = drand48 () * (high_fparams[i] - low_fparams[i]) + low_fparams[i];
    }
}

double
US_PM::Rg (set<pm_point> &model)
{
  if (!model.size ())
    {
      return 0e0;
...
bool
US_PM::rescale_params (vector<int> &types, vector<double> &fparams,
...
bool
US_PM::rescale_params (vector<double> &params, vector<double> &low_fparams,
                       vector<double> &high_fparams,
                       double new_conversion_factor,
      ...
      for (int k = 0; k < (int)object_parameter_types[types[i]][pos].size ();
           ++k, ++fpos)
        {
          switch (object_parameter_types[types[i]][pos][k])
            {
            case COORD:
              low_fparams[fpos] = scale_factor
                                  * (fparams[fpos]
                                     - grid_conversion_factor
                                           * refinement_range_pct / 100e0);
              high_fparams[fpos] = scale_factor
                                   * (fparams[fpos]
                                      + grid_conversion_factor
                                            * refinement_range_pct / 100e0);
            ...
            default:
              ...
              low_fparams[fpos]
                  = fparams[fpos] * (1e0 - refinement_range_pct / 100e0);
              high_fparams[fpos]
                  = fparams[fpos] * (1e0 + refinement_range_pct / 100e0);
              break;
            }
...
bool
US_PM::zero_params (vector<double> &params, vector<int> &types)
{
  params.clear ();
  for (int i = 0; i < (int)types.size (); ++i)
    {
...
bool
US_PM::random_params (vector<double> &params, vector<int> &types, double max_d)
{
  params.clear ();
  if (!max_d)
    {
...
QString
US_PM::list_params (vector<double> &params)
{
  QString qs = QString ("model: grid conversion factor %1 ")
                   .arg (grid_conversion_factor);
  vector<int> types;
  vector<double> fparams;
  if (!split (params, types, fparams))
  ...
  for (int i = 0; i < (int)types.size (); ++i)
    {
      int pos = i < (int)object_parameter_types[types[i]].size ()
                    ...
                    : object_parameter_types[types[i]].size () - 1;
      qs += QString ("\n\t%1 (%2) : ")
                .arg (object_names[types[i]])
                .arg ((int)object_parameter_types[types[i]][pos].size ());
      for (int k = 0; k < (int)object_parameter_types[types[i]][pos].size ();
           ++k, ++fpos)
        {
  ...
  for (int i = 0; i < (int)object_parameter_types.size (); ++i)
    {
      qs += QString ("object: %1 model positions %2\n")
                .arg (object_names[i])
                .arg (object_parameter_types[i].size ());
      for (int j = 0; j < (int)object_parameter_types[i].size (); ++j)
        {
          qs += QString ("\tmodel position %1 parameters %2: ")
                    .arg (j)
                    .arg (object_parameter_types[i][j].size ());
          for (int k = 0; k < (int)object_parameter_types[i][j].size (); ++k)
            {
              qs += QString (" %1").arg (
                  object_type_name[object_parameter_types[i][j][k]]);
            }
          qs += "\n";
        }
    }
...
bool
US_PM::set_limits (vector<double> &params, vector<double> &low_fparams,
                   vector<double> &high_fparams, double max_d)
{
  vector<int> types;
  {
    vector<double> fparams;
...
bool
US_PM::split (vector<double> &params, vector<int> &types,
              vector<double> &fparams)
{
  types.clear ();
  fparams.clear ();
  ...
  for (int i = 0; i < (int)params.size ();)
    {
      ...
      if ((int)params.size () <= i)
        {
          error_msg
              = QString ("split: no params for type %2").arg (types.back ());
          return false;
        }
      if ((int)object_parameter_types.size () < types.back ())
        {
          error_msg
              = QString ("split: unknown object type %2").arg (types.back ());
          return false;
        }
      int pos = (int)types.size () - 1
                        < (int)object_parameter_types[types.back ()].size ()
                    ? types.size () - 1
                    : object_parameter_types[types.back ()].size () - 1;
      if (params.size () < object_parameter_types[types.back ()][pos].size ())
        {
          error_msg = QString ("split: error insufficient params for type %2")
                          .arg (types.back ());
          return false;
        }
      for (int k = 0;
           k < (int)object_parameter_types[types.back ()][pos].size (); ++k)
        {
...
bool
US_PM::join (vector<double> &params, vector<int> &types,
             vector<double> &fparams)
{
  params.clear ();
  ...
  for (int i = 0; i < (int)types.size (); ++i)
    {
      ...
      if ((int)fparams.size ()
        ...
        {
          error_msg = QString ("join: error insufficient params for type %2")
                          .arg (types[i]);
          return false;
        }
      for (int k = 0; k < (int)object_parameter_types[types[i]][pos].size ();
...
bool
US_PM::expand_types (vector<vector<double> > &types_vector, QString types,
                     bool incrementally, bool allcombinations)
{
  types_vector.clear ();
  vector<vector<int> > types_vector_int;
  bool result
      = expand_types (types_vector_int, types, incrementally, allcombinations);
...
bool
US_PM::expand_types (vector<vector<int> > &types_vector, QString types,
                     bool incrementally, bool allcombinations)
{
  types_vector.clear ();
      ...
      if (allcombinations)
        {
          ...
          for (int i = 1; i <= (int)qsl_types.size (); ++i)
            {
              vector<int> pos (i);
              combo_with_replacement (typesv, (int)pos.size (), pos,
                                      types_vector);
            }
        }
      else
      ...
      if (allcombinations)
        {
          ...
          combo_with_replacement (typesv, (int)pos.size (), pos, types_vector);
        }
      else
        {
          vector<int> this_types;
