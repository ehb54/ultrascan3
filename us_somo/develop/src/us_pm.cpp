...
grid_conversion_factor ) );
  ...
  */
  ...
  if (grid_conversion_factor < 0.5) {
    error_msg = QString(
                    "set_grid_size requested size %1 is less than the minimum "
                    "allowed 0.5")
...
US_PM::US_PM(double grid_conversion_factor, int max_dimension,
  ...
  if (this->max_dimension != max_dimension) {
    if (us_log) {
      us_log->log(QString("Warning: maximum dimension requested %1 exceedes "
  ...
  if (use_errors) {
    ...
    if (!quiet && us_log && !use_errors && any_non_zero) {
      us_log->log(
          "Notice: SD's provided but some were zero, so SD fitting is turned "
          "off\n");
    }
  }
...
bool US_PM::rotation_matrix(double l, double m, double n, double theta,
...
bool US_PM::apply_rotation_matrix(vector<vector<double> > &rm, int x, int y,
...
bool US_PM::check_limits(vector<double> &fparams, vector<double> &low_fparams,
...
bool US_PM::clip_limits(vector<double> &fparams, vector<double> &low_fparams,
                        vector<double> &high_fparams) {
  bool clipped = false;
  for (unsigned int i = 0; i < (unsigned int)fparams.size(); i++) {
...
QString US_PM::physical_stats(set<pm_point> &model) {
  ...
  // last_physical_stats[ "result intersection volume"         ] = QString( "%1"
  // ).arg( volume_intersection ); last_physical_stats[ "result excluded volume"
...
double US_PM::Rg(set<pm_point> &model) {
  ...
  for (int i = 0; i < (int)pmodel.size(); i++) {
    Rg2 +=
        ((pmodel[i].axis[0] - cm.axis[0]) * (pmodel[i].axis[0] - cm.axis[0]) +
         (pmodel[i].axis[1] - cm.axis[1]) * (pmodel[i].axis[1] - cm.axis[1]) +
         (pmodel[i].axis[2] - cm.axis[2]) * (pmodel[i].axis[2] - cm.axis[2]));
  }

  return sqrt(Rg2 / (double)pmodel.size());
}

bool US_PM::rescale_params(vector<int> &types, vector<double> &fparams,
...
bool US_PM::rescale_params(vector<double> &params, vector<double> &low_fparams,
                           vector<double> &high_fparams,
                           double new_conversion_factor,
  ...
  for (int i = 0; i < (int)types.size(); ++i) {
    ...
    for (int k = 0; k < (int)object_parameter_types[types[i]][pos].size();
      ...
      switch (object_parameter_types[types[i]][pos][k]) {
        case COORD:
          low_fparams[fpos] =
              scale_factor * (fparams[fpos] - grid_conversion_factor *
        ...
        case RADIUS:
          low_fparams[fpos] =
              scale_factor *
              ((fparams[fpos] - bead_radius_over_2gcf) -
               grid_conversion_factor * refinement_range_pct / 100e0);
          high_fparams[fpos] =
              scale_factor *
              ((fparams[fpos] + bead_radius_over_2gcf) +
               grid_conversion_factor * refinement_range_pct / 100e0);
...
bool US_PM::random_params(vector<double> &params, vector<int> &types,
                          double max_d) {
  params.clear();
  if (!max_d) {
...
QString US_PM::list_params(vector<double> &params) {
  QString qs =
      QString("model: grid conversion factor %1 ").arg(grid_conversion_factor);
  vector<int> types;
  vector<double> fparams;
  if (!split(params, types, fparams)) {
    return error_msg;
...
bool US_PM::set_limits(vector<double> &params, vector<double> &low_fparams,
...
bool US_PM::split(vector<double> &params, vector<int> &types,
                  vector<double> &fparams) {
  types.clear();
  fparams.clear();
...
bool US_PM::join(vector<double> &params, vector<int> &types,
                 vector<double> &fparams) {
  params.clear();
  ...
  for (int i = 0; i < (int)types.size(); ++i) {
    ...
    if ((int)fparams.size() <
      ...
      error_msg =
          QString("join: error insufficient params for type %2").arg(types[i]);
      return false;
    }
    for (int k = 0; k < (int)object_parameter_types[types[i]][pos].size();
         ++k, ++fpos) {
...
bool US_PM::expand_types(vector<vector<double> > &types_vector, QString types,
...
bool US_PM::expand_types(vector<vector<int> > &types_vector, QString types,
                         bool incrementally, bool allcombinations) {
  types_vector.clear();
  ...
  if (incrementally) {
    if (allcombinations) {
      ...
      vector<int> typesv;
      for (set<int>::iterator it = types_set.begin(); it != types_set.end();
           it++) {
        typesv.push_back(*it);
      }
