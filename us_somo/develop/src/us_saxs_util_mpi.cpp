...
US_Saxs_Util::run_iq_mpi (QString controlfile)
{
  ...
  if (myrank)
    {
      ...
      cout << QString ("%1: waiting for broadcast of skip_cores_per_core\n")
           ...
           << flush;
      if (MPI_SUCCESS
          != MPI_Bcast (&skip_cores_per_core, 1, MPI_UNSIGNED, 0,
                        MPI_COMM_WORLD))
        {
          MPI_Abort (MPI_COMM_WORLD, errorno);
          exit (errorno);
  ...
  else
    {
      ...
      if (qslt.filter (QRegExp ("\\.(tgz|TGZ)$")).size () != qslt.size ())
        {
          // not all tgz
          cout << QString ("Error: %1 control file does not contain only .tgz "
                           "files\n")
                      .arg (controlfile);
          MPI_Abort (MPI_COMM_WORLD, errorno);
          exit (errorno);
          ...
          for (unsigned int i = 0; i < common.size (); i++)
            {
              ...
              US_Tar ust;
              result = ust.extract (
                  qs_base_dir + QDir::separator () + controlfile, &qslt);
              if (TAR_OK != result)
                {
                  cout
                      << QString (
                ...
                if (f.exists ())
                  {
                    ...
                    // us_qdebug( QString( "%1: found directives, skip cores
                    // per core is %2" ).arg( myrank ).arg( skip_cores_per_core
                    // ) );
                  }
              }
  ...
  if (myrank % inc)
  ...
  else
    {
      for (unsigned int i = myrank / inc; i < qslt.size (); i += use_procs)
        {
          cout
              << QString ("%1: processing job %2\n").arg (myrank).arg (qslt[i])
              << flush;
          // mkdir
              ...
              if (!in_output.count (f.fileName ()))
                {
                  ...
                  full_output_list
                      << QString ("tmp_%1/%2").arg (i).arg (f.fileName ());
                }
            }
          ...
          for (unsigned int j = 0; j < job_output_files.size (); j++)
            {
              full_output_list
                  << QString ("tmp_%1/").arg (i) + job_output_files[j];
            }
        }
    }
  ...
  // us_qdebug( QString("%1: end of computation barrier my full_output_list
  ...
  if (MPI_SUCCESS
      != MPI_Allreduce (&sizeoflist, &max_individual_size, 1, MPI_UNSIGNED,
  ...
  if (MPI_SUCCESS
      != MPI_Gather (char_output_files, max_individual_size + 1, MPI_CHAR,
  ...
  if (!myrank)
    {
      ...
      for (unsigned int i = 0; i < (unsigned int)npes; i++)
        {
          // qslt = (QString( "%1" ).split( "\n" , Qt::SkipEmptyParts )
          //                            .arg( &gathered_output_files[ i * (
          //                            max_individual_size + 1 ) ] ) );
          {
            QString qs = QString ("%1").arg (
                &gathered_output_files[i * (max_individual_size + 1)]);
            qslt = (qs).split ("\n", Qt::SkipEmptyParts);
          }
...
bool
US_Saxs_Util::run_nsa_mpi (QString controlfile)
{
  ...
  if (npes < 2)
    {
      cerr << "ERROR: requires at least 2 processes to run nsa" << endl
           << flush;
  ...
  if (myrank)
  ...
  else
    {
      ...
      // copy here
      US_File_Util usu;
      usu.copy (controlfile, QDir::currentPath () + SLASH
                                 + QFileInfo (controlfile).fileName ());
      cout << QString ("copying %1 %2 <%3>\n")
                  .arg (controlfile)
                  .arg (QDir::currentPath () + SLASH
                        + QFileInfo (controlfile).fileName ())
                  .arg (usu.errormsg);
