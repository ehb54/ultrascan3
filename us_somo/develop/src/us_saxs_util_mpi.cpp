...
void debug_mpi(QString msg) {
  ...
  if (msg.contains(QRegExp("^\\d+: (SEND|RECV) "))) {
    ...
    if (f.open(QIODevice::WriteOnly | QIODevice::Append)) {
      QTextStream ts(&f);
      ts << QTime::currentTime().toString("hh:mm:ss.zzz") << ":" << msgs[0]
         << Qt::endl;
      f.close();
    }
  }
...
bool US_Saxs_Util::run_iq_mpi(QString controlfile) {
  ...
  if (myrank) {
    ...
    cout << QString("%1: waiting for broadcast of skip_cores_per_core\n")
         ...
         << flush;
    if (MPI_SUCCESS !=
        MPI_Bcast(&skip_cores_per_core, 1, MPI_UNSIGNED, 0, MPI_COMM_WORLD)) {
      MPI_Abort(MPI_COMM_WORLD, errorno);
      exit(errorno);
    }
    errorno--;
    ...
    if (qslt.filter(QRegExp("\\.(tgz|TGZ)$")).size() != qslt.size()) {
      // not all tgz
      cout << QString(
                  "Error: %1 control file does not contain only .tgz files\n")
                  .arg(controlfile);
      MPI_Abort(MPI_COMM_WORLD, errorno);
      exit(errorno);
    }
    ...
    if (common.size()) {
      for (unsigned int i = 0; i < common.size(); i++) {
        ...
        US_Tar ust;
        result =
            ust.extract(qs_base_dir + QDir::separator() + controlfile, &qslt);
        if (TAR_OK != result) {
          cout << QString("Error: %1 problem extracting tar archive (%2)\n")
                      .arg(controlfile)
                      .arg(ust.explain(result));
        ...
        // check for directives
        {
          ...
          if (f.exists()) {
            ...
            // us_qdebug( QString( "%1: found directives, skip cores per core is
            // %2" ).arg( myrank ).arg( skip_cores_per_core ) );
          }
        }
  ...
  if (MPI_SUCCESS != MPI_Allreduce(&sizeoflist, &max_individual_size, 1,
  ...
  if (MPI_SUCCESS != MPI_Gather(char_output_files, max_individual_size + 1,
                                MPI_CHAR, gathered_output_files,
                                max_individual_size + 1, MPI_CHAR, 0,
                                MPI_COMM_WORLD)) {
    MPI_Abort(MPI_COMM_WORLD, errorno);
    exit(errorno);
  }
...
bool US_Saxs_Util::run_nsa_mpi(QString controlfile) {
  ...
  if (myrank) {
    ...
    // copy here
    US_File_Util usu;
    usu.copy(controlfile,
             QDir::currentPath() + SLASH + QFileInfo(controlfile).fileName());
