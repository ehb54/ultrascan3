...
void
US_Hydrodyn_Pdb_Tool::sel_nearest_residues (QTreeWidget *lv)
{
  map<QString, QString> parameters;
  US_Hydrodyn_Pdb_Tool_Selres *selres
      = new US_Hydrodyn_Pdb_Tool_Selres (this, &parameters, this);
  ...
  bool only_new
      = parameters.count ("only_new") && parameters["only_new"] == "true";
  ...
  if (max_asa)
    {
      exposed_set
          = parameters.count ("naccess")
                ? get_exposed_set_naccess (
                      lv, max_asa, 1 == parameters.count ("naccess_sc_or_mc"),
                      false)
  ...
  while ((*it))
    {
      ...
      if (distmap.count (item) && (!max_asa || exposed_set.count (item)))
        {
          item->setSelected (true);
          // editor_msg("blue", QString( "size setting for dist %1 key %2"
          // ).arg( distmap[ item ] ).arg( key( item ) ) );
        }
      ++it;
    }
  ...
  if (!save_sel.isEmpty ())
    {
      if (!save_sel.contains ("/"))
        {
          save_sel
              = ((US_Hydrodyn *)us_hydrodyn)->somo_pdb_dir + SLASH + save_sel;
        }

      if (QFile::exists (save_sel))
        {
          save_sel = ((US_Hydrodyn *)us_hydrodyn)
                         ->fileNameCheck (save_sel, 0, this);
        }
      QFile f (save_sel);
      if (!f.open (QIODevice::WriteOnly | QIODevice::Text))
        {
          editor_msg (
              "red",
              QString (us_tr ("Error: Can not create file %1 for writing"))
                  .arg (f.fileName ()));
          return;
        }
      QTextStream t (&f);
      ...
      editor_msg ("dark blue",
                  QString (us_tr ("Created selected residue file %1"))
                      .arg (f.fileName ()));
    }
  {
    QString out;
        ...
        if (US_Static::lvi_depth (item1) == 2 && is_selected (item1))
          {
            out += QString ("%1%2")
                       .arg (out.isEmpty () ? "" : ",")
                       .arg (get_residue_number (item1));
          }
        it1++;
      }
    lv == lv_csv ? csv_msg ("blue", out) : csv2_msg ("blue", out);
  }
}

set<QTreeWidgetItem *>
US_Hydrodyn_Pdb_Tool::get_exposed_set_naccess (QTreeWidget * /* lv */,
                                               double /* max_asa */,
                                               bool /* sc_or_mc */,
                                               bool /* only_selected */)

{
  ...
  vector<QStringList> qsl_models
      = separate_models (lv == lv_csv ? csv1 : csv2[csv2_pos]);
  vector<QString> models = get_models (lv);

  if (lv_models.size () != qsl_models.size ()
      || models.size () != lv_models.size ())
    {
      editor_msg ("red", QString (us_tr ("Error: NACCESS splitting models "
                                         "size mismatch %1 vs %2 vs %3"))
                             .arg (lv_models.size ())
                             .arg (qsl_models.size ())
                             .arg (models.size ()));
      return result;
  ...
  for (int i = 0; i < (int)qsl_models.size (); ++i)
    {
      ...
      if (!naccess_result_data.size ())
        {
          editor_msg ("red", QString (us_tr ("Error: NACCESS did not return "
                                             "any results for model %1"))
                                 .arg (i + 1));
          return result;
        }
      set<QString> exposed;
      for (int j = 0; j < (int)naccess_result_data.size (); ++j)
        {
          ...
          if (naccess_result_data[j].left (3) == "RES")
            {
              QString residue = naccess_result_data[j].mid (4, 3) + "~"
                                + naccess_result_data[j].mid (8, 1).trimmed ()
                                + "~"
                                + naccess_result_data[j].mid (9, 4).trimmed ();
              double this_asa
                  = naccess_result_data[j].mid (23, 5).trimmed ().toDouble ();
              double this_asa_sc
                  = naccess_result_data[j].mid (36, 5).trimmed ().toDouble ();
              double this_asa_mc
                  = naccess_result_data[j].mid (49, 5).trimmed ().toDouble ();
              if (sc_or_mc)
                {
                  this_asa
                      = this_asa_sc > this_asa_mc ? this_asa_sc : this_asa_mc;
                }
              if (this_asa >= max_asa)
                {
                  // cout << QString( "'%1'\n" ).arg( residue );
      ...
      for (set<QString>::iterator it = exposed.begin (); it != exposed.end ();
           it++)
        {
          cout << *it << endl;
        }
...
set<QTreeWidgetItem *>
US_Hydrodyn_Pdb_Tool::get_exposed_set (QTreeWidget *lv, double max_asa,
                                       bool only_selected)
{
  set<QTreeWidgetItem *> result;
  errormsg = "";
  ...
  usu->control_parameters["asamethod"]
      = QString ("%1").arg (((US_Hydrodyn *)us_hydrodyn)->asa.method);
  ...
  while ((*it))
    {
      ...
      if (item->isSelected ())
        {
          cout << QString ("model id <%1> chain id <%2> residue name <%3> "
                           "residue number <%4>\n")
  ...
  if (usu->model_vector.size () != models.size ())
    {
      editor_msg (
          "red",
          QString (
              us_tr ("Error: model size mismatch after read pdb (%1 != %2)"))
              .arg (models.size ())
      ...
      for (unsigned int j = 0; j < usu->model_vector[i].molecule.size (); j++)
        {
          for (unsigned int k = 0;
               k < usu->model_vector[i].molecule[j].atom.size (); k++)
            {
              if (usu->model_vector[i].molecule[j].atom[k].active)
                {
                  aa++;
                  total_asa
                      += usu->model_vector[i].molecule[j].atom[k].bead_asa;
                  total_ref_asa
                      += usu->model_vector[i].molecule[j].atom[k].ref_asa;
                  total_mass
                      += usu->model_vector[i].molecule[j].atom[k].bead_ref_mw;
                  total_vol += usu->model_vector[i]
                                   .molecule[j]
                                   .atom[k]
                                   .bead_ref_volume_unhydrated;

                  QString residue
                      = usu->model_vector[i].molecule[j].atom[k].resName + "~"
                        + (usu->model_vector[i].molecule[j].atom[k].chainID
                                   == " "
                               ? "~"
                               : usu->model_vector[i]
                                     .molecule[j]
                                     ...
                                     .chainID)
                        + QString ("~%1").arg (
                            usu->model_vector[i].molecule[j].atom[k].resSeq);
                  if (residue != last_residue)
                    {
                      if (last_residue != "")
                        {
                          printf (" [ %-6d %s ]\t%.0f\t%.0f\t%.2f\n", seqno,
                                  last_residue.toLatin1 ().data (),
                                  residue_asa, residue_ref_asa,
                                  100.0 * residue_asa / residue_ref_asa);
          ...
          if (last_residue != "")
            {
              printf (" [ %-6d %s ]\t%.0f\t%.0f\t%.2f\n", seqno,
                      last_residue.toLatin1 ().data (), residue_asa,
                      residue_ref_asa, 100.0 * residue_asa / residue_ref_asa);
              if (100.0 * residue_asa / residue_ref_asa >= max_asa)
                {
                  exposed.insert (last_residue);
                }
...
vector<QString>
US_Hydrodyn_Pdb_Tool::get_models (QTreeWidget *lv)
{
...
bool
US_Hydrodyn_Pdb_Tool::naccess_run (QString /* pdb */)
{
#if QT_VERSION < 0x040000
  ...
  QString radii = USglobal->config_list.system_dir + SLASH
                  + "bin"
#if defined(BIN64)
                    "64"
#endif
    ...
    if (!qfi.exists ())
      {
        editor_msg (
            "red",
            QString ("Naccess program '%1' does not exist\n").arg (prog));
        return false;
      }
    if (!qfi.isExecutable ())
      {
        editor_msg (
            "red",
            QString ("Naccess program '%1' is not executable\n").arg (prog));
        return false;
      }
  }

  {
    ...
    if (!qfi.exists ())
      {
        editor_msg ("red",
                    QString ("Naccess support file '%1' does not exist\n")
                        .arg (radii));
        return false;
      }
  }
  ...
  if (!fi.exists ())
    {
      editor_msg ("red",
                  QString ("Naccess called but PDB file '%1' does not exist\n")
                      .arg (pdb));
      return false;
    }
  ...
  naccess->addArgument (
      QString ("%1").arg (((US_Hydrodyn *)us_hydrodyn)->asa.probe_radius));
  // naccess->addArgument( "-z" );
  // naccess->addArgument( QString( "%1" ).arg( ((US_Hydrodyn
  // *)us_hydrodyn)->asa.asab1_step ) );

  connect (naccess, SIGNAL (readyReadStandardOutput ()), this,
           SLOT (naccess_readFromStdout ()));
  connect (naccess, SIGNAL (readyReadStandardError ()), this,
           SLOT (naccess_readFromStderr ()));
  connect (naccess, SIGNAL (finished (int, QProcess::ExitStatus)), this,
           SLOT (naccess_finished (int, QProcess::ExitStatus)));
...
void
US_Hydrodyn_Pdb_Tool::naccess_readFromStdout ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Pdb_Tool::naccess_readFromStderr ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Pdb_Tool::naccess_finished (int, QProcess::ExitStatus)
{
  ...
  QString naccess_result_file
      = QString (naccess_last_pdb).replace (QRegExp (".pdb$"), "") + ".rsa";
  ...
  if (!f.exists ())
    {
      editor_msg ("red",
                  QString (us_tr ("Error: Naccess did not create file %1"))
                      .arg (naccess_result_file));
      naccess_running = false;
      return;
    }
  ...
  if (!f.open (QIODevice::ReadOnly))
    {
      editor_msg (
          "red", QString (us_tr ("Error: can not open Naccess result file %1"))
                     .arg (naccess_result_file));
      naccess_running = false;
      return;
    }
  ...
  //    if ( !f.remove() )
  //    {
  //       editor_msg("dark red", QString(us_tr("Warning: could not remove
  //       Naccess result file %1")).arg( naccess_result_file ));
  //    }
...
void
US_Hydrodyn_Pdb_Tool::naccess_started ()
{
  editor_msg ("brown", "Naccess launch exited\n");
  disconnect (naccess, SIGNAL (started ()), 0, 0);
}

vector<vector<QTreeWidgetItem *> >
US_Hydrodyn_Pdb_Tool::separate_models (QTreeWidget *lv)
{
...
vector<QStringList>
US_Hydrodyn_Pdb_Tool::separate_models (csv &ref_csv)
{
...
void
US_Hydrodyn_Pdb_Tool::select_residues_with_atoms_selected (QTreeWidget *lv)
{
  QTreeWidgetItemIterator it (lv);
  while ((*it))
    {
      QTreeWidgetItem *item = (*it);
      if (!item->isSelected () && US_Static::lvi_depth (item) == 2)
        {
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Pdb_Tool::sel (QTreeWidget *lv)
{
  ...
  QString cur_res_sel;
  {
    ...
    while ((*it))
      {
        QTreeWidgetItem *item = (*it);
        if (US_Static::lvi_depth (item) == 2 && is_selected (item))
          {
  ...
  QString text = US_Static::getText (
      ...
      QLineEdit::Normal, cur_res_sel, &ok, this);
  if (!ok)
    {
      return;
    }
  ...
  for (set<int>::iterator it = to_sel.begin (); it != to_sel.end (); it++)
    {
