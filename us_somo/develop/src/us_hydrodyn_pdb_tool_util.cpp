...
void US_Hydrodyn_Pdb_Tool::sel_nearest_residues(QTreeWidget *lv) {
  ...
  QString save_sel = parameters.count("save_sel") ? parameters["save_sel"] : "";

  double max_asa = parameters.count("asa") ? parameters["asa"].toDouble() : 0e0;
    ...
    exposed_set =
        parameters.count("naccess")
            ? get_exposed_set_naccess(
                  lv, max_asa, 1 == parameters.count("naccess_sc_or_mc"), false)
  ...
  while ((*it)) {
    ...
    if (distmap.count(item) && (!max_asa || exposed_set.count(item))) {
      ...
      // editor_msg("blue", QString( "size setting for dist %1 key %2" ).arg(
      // distmap[ item ] ).arg( key( item ) ) );
    }
    ++it;
  }
  ...
  if (!save_sel.isEmpty()) {
    ...
    editor_msg(
        "dark blue",
        QString(us_tr("Created selected residue file %1")).arg(f.fileName()));
  }
  {
    QString out;
    QTreeWidgetItemIterator it1(lv);
...
set<QTreeWidgetItem *> US_Hydrodyn_Pdb_Tool::get_exposed_set_naccess(
    QTreeWidget * /* lv */, double /* max_asa */, bool /* sc_or_mc */,
    bool /* only_selected */)

{
  ...
  if (lv_models.size() != qsl_models.size() ||
    ...
    editor_msg(
        ...
        QString(
            us_tr(
                "Error: NACCESS splitting models size mismatch %1 vs %2 vs %3"))
            .arg(lv_models.size())
            .arg(qsl_models.size())
            .arg(models.size()));
    return result;
  ...
  for (int i = 0; i < (int)qsl_models.size(); ++i) {
    ...
    if (!naccess_result_data.size()) {
      editor_msg(
          "red",
          QString(
              us_tr("Error: NACCESS did not return any results for model %1"))
              .arg(i + 1));
      return result;
    }
    set<QString> exposed;
...
set<QTreeWidgetItem *> US_Hydrodyn_Pdb_Tool::get_exposed_set(
    QTreeWidget *lv, double max_asa, bool only_selected) {
  set<QTreeWidgetItem *> result;
  errormsg = "";
  QString qs;
  ...
  while ((*it)) {
    ...
    if (item->isSelected()) {
      cout << QString(
                  "model id <%1> chain id <%2> residue name <%3> residue "
                  "number <%4>\n")
  ...
  for (unsigned int i = 0; i < (unsigned int)usu->model_vector.size(); i++) {
    ...
    for (unsigned int j = 0; j < usu->model_vector[i].molecule.size(); j++) {
      for (unsigned int k = 0; k < usu->model_vector[i].molecule[j].atom.size();
           k++) {
...
void US_Hydrodyn_Pdb_Tool::naccess_finished(int, QProcess::ExitStatus) {
  ...
  if (!f.exists()) {
    editor_msg("red", QString(us_tr("Error: Naccess did not create file %1"))
  ...
  if (!f.open(QIODevice::ReadOnly)) {
    editor_msg("red",
               QString(us_tr("Error: can not open Naccess result file %1"))
                   .arg(naccess_result_file));
    naccess_running = false;
    return;
  }
...
vector<vector<QTreeWidgetItem *> > US_Hydrodyn_Pdb_Tool::separate_models(
    QTreeWidget *lv) {
...
void US_Hydrodyn_Pdb_Tool::select_residues_with_atoms_selected(
    QTreeWidget *lv) {
  QTreeWidgetItemIterator it(lv);
  while ((*it)) {
    QTreeWidgetItem *item = (*it);
