...
US_Hydrodyn_Saxs_Iqq_Residuals::US_Hydrodyn_Saxs_Iqq_Residuals(
    ...
    vector<double> log_difference, vector<double> log_target, QColor plot_color,
    bool use_errors, bool plot_log, bool plot_difference, bool plot_as_percent,
    double avg_std_dev_frac, vector<double> std_dev_frac,
...
void US_Hydrodyn_Saxs_Iqq_Residuals::add(unsigned int width, vector<double> q,
...
void US_Hydrodyn_Saxs_Iqq_Residuals::setupGUI() {
  ...
  if (0 && avg_std_dev_frac) {
    ...
    cb_plot_mult_avg_sd_frac->setFont(QFont(USglobal->config_list.fontFamily,
...
void US_Hydrodyn_Saxs_Iqq_Residuals::cancel() { close(); }
    ...
    if (plot_difference) {
      ...
      double this_miny =
          plot_as_percent
              ...
              : (use_mult_sd_frac
                     ? differences_mult_sd[pos][0]
                     : (use_mult_avg_sd_frac ? differences_mult_avg_sd[pos][0]
                                             : differences[pos][0]));
        ...
        double val =
            plot_as_percent
                ...
                : (use_mult_sd_frac
                       ...
                       : (use_mult_avg_sd_frac ? differences_mult_avg_sd[pos][i]
                                               : differences[pos][i]));
