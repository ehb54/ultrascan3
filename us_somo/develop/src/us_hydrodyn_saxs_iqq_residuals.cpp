...
US_Hydrodyn_Saxs_Iqq_Residuals::US_Hydrodyn_Saxs_Iqq_Residuals (
    bool *saxs_iqq_residuals_widget, unsigned int width, QString title,
    vector<double> q, vector<double> difference,
    vector<double> difference_no_errors, vector<double> target,
    vector<double> log_difference, vector<double> log_target,
    QColor plot_color, bool use_errors, bool plot_log, bool plot_difference,
    bool plot_as_percent, double avg_std_dev_frac, vector<double> std_dev_frac,
    unsigned int pen_width, QWidget *p, const char *)
    : QFrame (p)
{
  ...
  add (width, q, difference, difference_no_errors, target, log_difference,
       log_target, plot_color);
}
...
void
US_Hydrodyn_Saxs_Iqq_Residuals::add (unsigned int width, vector<double> q,
                                     ...
                                     QColor plot_color)
{
...

  {
    if (std_dev_frac.size () > difference.size ())
      {
        cout << QString (
                    "Warning: additional plot smaller than original %1 %2\n")
                    .arg (std_dev_frac.size ())
                    .arg (difference.size ());
      }
      ...
      if (targets[pos][i] != 0e0)
        {
          differences_no_errors_pcts[pos][i]
              = 100.0 * differences_no_errors[pos][i] / targets[pos][i];
        }
      else
        {
...
void
US_Hydrodyn_Saxs_Iqq_Residuals::setupGUI ()
{
  ...
  cb_plot_difference->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize));
  cb_plot_difference->setPalette (PALET_NORMAL);
  AUTFBACK (cb_plot_difference);

  connect (cb_plot_difference, SIGNAL (clicked ()),
           SLOT (set_plot_difference ()));
  ...
  cb_plot_log->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  ...
  cb_plot_as_percent->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize));
  cb_plot_as_percent->setPalette (PALET_NORMAL);
  AUTFBACK (cb_plot_as_percent);
  connect (cb_plot_as_percent, SIGNAL (clicked ()),
           SLOT (set_plot_as_percent ()));
  ...
  if (0 && avg_std_dev_frac)
    {
      ...
      cb_plot_mult_avg_sd_frac->setFont (QFont (
      ...
      connect (cb_plot_mult_avg_sd_frac, SIGNAL (clicked ()),
               SLOT (set_plot_mult_avg_sd_frac ()));
      ...
      cb_plot_mult_sd_frac->setFont (QFont (USglobal->config_list.fontFamily,
                                            USglobal->config_list.fontSize));
      cb_plot_mult_sd_frac->setPalette (PALET_NORMAL);
      AUTFBACK (cb_plot_mult_sd_frac);
      connect (cb_plot_mult_sd_frac, SIGNAL (clicked ()),
               SLOT (set_plot_mult_sd_frac ()));
    }
  ...
  // background->addWidget( lbl_title , j , 0 , 1 + ( j ) - ( j ) , 1 + ( 1 ) -
  // ( 0 ) ); j++;
...
void
US_Hydrodyn_Saxs_Iqq_Residuals::cancel ()
{
  close ();
}

void
US_Hydrodyn_Saxs_Iqq_Residuals::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Saxs_Iqq_Residuals::closeEvent (QCloseEvent *e)
{
  if (!detached)
    {
      *saxs_iqq_residuals_widget = false;
...
void
US_Hydrodyn_Saxs_Iqq_Residuals::set_plot_log ()
{
...
void
US_Hydrodyn_Saxs_Iqq_Residuals::set_plot_difference ()
{
...
void
US_Hydrodyn_Saxs_Iqq_Residuals::set_plot_as_percent ()
{
...
void
US_Hydrodyn_Saxs_Iqq_Residuals::set_plot_mult_avg_sd_frac ()
{
...
void
US_Hydrodyn_Saxs_Iqq_Residuals::set_plot_mult_sd_frac ()
{
  if (cb_plot_mult_avg_sd_frac->isChecked ()
      && cb_plot_mult_sd_frac->isChecked ())
    {
...
void
US_Hydrodyn_Saxs_Iqq_Residuals::update_plot ()
{
  ...
  if (plot_difference)
    {
      bool use_mult_avg_sd_frac
          = false; // avg_std_dev_frac ? cb_plot_mult_avg_sd_frac->isChecked()
                   // : false;
      bool use_mult_sd_frac
          = false; // avg_std_dev_frac ? cb_plot_mult_sd_frac->isChecked() :
                   // false;
      ...
      if (use_errors)
        {
          left_axis_title
              = (plot_as_percent
                     ...
                     : (use_mult_sd_frac
                            ? "delta I(q) * s.d._frac / s.d. " // (double
                                                               // *)&(differences_mult_sd[pos][0])
                            : (use_mult_avg_sd_frac
                                   ? "delta I(q) * avg_s.d._frac / s.d." // (double
                                                                         // *)&(differences_mult_avg_sd[pos][0])
                                   : "delta I(q) / s.d." // (double
                                                         // *)&(differences[pos][0])
                               )));
        }
      else
  ...
  if (plot_log)
    {
      left_axis_title
          = (plot_as_percent
                 ? "% delta log10 I(q)" // (double
                                        // *)&(log_difference_pcts[pos][0])
  ...
  for (unsigned int pos = 0; pos < qs.size (); pos++)
    {
      ...
      if (plot_log)
        {
          ...
          plot->setCurveData (iqq, (double *)&(qs[pos][0]),
                              plot_as_percent
                                  ? (double *)&(log_difference_pcts[pos][0])
                                  : (double *)&(log_differences[pos][0]),
                              (int)qs[pos].size ());
          plot->setCurvePen (iqq,
                             QPen (plot_colors[pos], pen_width, SolidLine));
#else
          QwtPlotCurve *curve = new QwtPlotCurve ("Log10 I(q) vs q");
          curve->setStyle (QwtPlotCurve::Lines);
          curve->setSamples ((double *)&(qs[pos][0]),
                             plot_as_percent
                                 ? (double *)&(log_difference_pcts[pos][0])
                                 : (double *)&(log_differences[pos][0]),
                             (int)qs[pos].size ());
          ...
          curve->attach (plot);
#endif
          double this_miny = plot_as_percent ? log_difference_pcts[pos][0]
                                             : log_differences[pos][0];
          double this_maxy = this_miny;
          for (unsigned int i = 1; i < qs[pos].size (); i++)
            {
              double val = plot_as_percent ? log_difference_pcts[pos][i]
                                           : log_differences[pos][i];
              if (this_miny > val)
                {
                  this_miny = val;
                }
      ...
      if (plot_difference)
        {
          bool use_mult_avg_sd_frac
              = false; // avg_std_dev_frac ?
                       // cb_plot_mult_avg_sd_frac->isChecked() : false;
          bool use_mult_sd_frac
              = false; // avg_std_dev_frac ? cb_plot_mult_sd_frac->isChecked()
                       // : false;
...
#endif
          double this_miny
              = plot_as_percent ? differences_no_errors_pcts[pos][0]
                                : (use_mult_sd_frac
                                       ? differences_mult_sd[pos][0]
                                       : (use_mult_avg_sd_frac
                                              ? differences_mult_avg_sd[pos][0]
  ...
  // display 2sd bars
  if (qs.size () && plot_as_percent && !plot_log)
    {
      double linepos = 5.0;
      double x[2];
      double y[2];
      ...
      plot->setCurveStyle (iqq, QwtCurve::Lines);
      plot->setCurveData (iqq, (double *)&(x[0]), (double *)&(y[0]), 2);
...
void
US_Hydrodyn_Saxs_Iqq_Residuals::usp_config_plot (const QPoint &)
{
