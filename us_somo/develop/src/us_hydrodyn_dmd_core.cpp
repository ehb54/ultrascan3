...
// (this) us_hydrodyn_dmd_core.cpp contains the main computational routines for
...
void
US_Hydrodyn::dmd_static_pairs ()
{
  // for simplicity, 1st build a vector of all the atoms
  vector<PDB_atom *> atoms;
  vector<int> chainseq;

  for (unsigned int use_model = 0;
       use_model < (unsigned int)lb_model->count (); use_model++)
    {
      if (0 || lb_model->item (use_model)->isSelected ())
        {
          for (unsigned int j = 0;
               j < model_vector[use_model].molecule.size (); j++)
            {
              for (unsigned int k = 0;
                   k < model_vector[use_model].molecule[j].atom.size (); k++)
                {
                  atoms.push_back (
                      &(model_vector[use_model].molecule[j].atom[k]));
      ...
      for (unsigned int j = i + 1; j < atoms.size () - 1; j++)
        {
          ...
          if (d <= dmd_options.threshold_pb_pb)
            {
              out2 += QString ("AtomPair \t %1.%2.%3 \t %4.%5.%6 \t Static\n")
                          .arg (chainseq[i])
                          .arg (atoms[i]->resSeq)
                          .arg (atoms[i]->name)
                          .arg (chainseq[j])
  ...
  {
    QString filename = project + ".dmd_constr";
    QString basename = somo_dir + SLASH + "dmd" + SLASH + filename;
    QFile f (basename);
  ...
  {
    QString filename = project + ".dmd_constr2";
    QString basename = somo_dir + SLASH + "dmd" + SLASH + filename;
    QFile f (basename);
    if (!f.open (QIODevice::WriteOnly))
      {
