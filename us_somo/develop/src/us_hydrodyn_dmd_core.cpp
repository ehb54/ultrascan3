...
// computational routines for brownian dynamic (anaflex) computations (this)
...
void US_Hydrodyn::dmd_static_pairs() {
  ...
  for (unsigned int use_model = 0; use_model < (unsigned int)lb_model->count();
       use_model++) {
    if (0 || lb_model->item(use_model)->isSelected()) {
      for (unsigned int j = 0; j < model_vector[use_model].molecule.size();
           j++) {
