  ...
  if (f.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    while (!ts.atEnd()) {
      ...
      if (net_charge.indexIn(current_hybrid.saxs_name) != -1) {
        ...
        protons += delta;
        // QTextStream( stdout ) << "saxs_name " << current_hybrid.saxs_name <<
        // " delta " << delta << Qt::endl; } else { QTextStream( stdout ) <<
        // "saxs_name " << current_hybrid.saxs_name << " delta 0" << Qt::endl;
      }
      ...
      // protons " << protons << " electrons " << current_hybrid.num_elect <<
      // Qt::endl;
    }
    f.close();
  } else {
...
void US_Hydrodyn::read_residue_file() {
    ...
    while (!ts.atEnd()) {
      ...
      // new_residue.comment << Qt::endl;
      {
        ...
        if ((qsl.size() - 7) % 2) {
          ...
          QMessageBox::critical(
              this, us_tr(windowTitle()),
              us_tr("Please note:\n\nThere was an error reading the selected "
                    "Residue File!\n"
        ...
        if (new_residue.pKas.size() > 2) {
          QMessageBox::critical(this, us_tr(windowTitle()),
                                us_tr("Please note:\n\nThere was an error "
                                      "reading the selected Residue File!\n"
                                      "Line contains too many pKa/vbar paris, "
                                      "currently only 2 are supported:\n"
                                      "Please contact the developers to enable "
                                      "3 pKas/vbars for a residue.\n") +
      ...
      for (j = 0; j < numatoms; ++j) {
        ...
        if (!hybrid_to_protons.count(new_atom.hybrid.name) ||
          ...
          editor_msg(
              ...
              QString(
                  us_tr(
                      "Hybridization information missing for %1, net charge, "
                      "proton count and isoelectric point will be incorrect!"))
        ...
        if (new_atom.bead_assignment >= numbeads) {
          QMessageBox::critical(
              this, us_tr(windowTitle()),
              QString(us_tr("\nThe atom's bead assignment has exceeded the "
                            "number of beads.\n"
        ...
        if (!atom_serial_to_match.count(new_atom.serial_number)) {
          QMessageBox::critical(
              this, us_tr(windowTitle()),
              QString(us_tr("\nThe atom's serial number is out of range or "
                            "duplicated.\n"
                            "For residue: %1 and Atom: %2 "
                            "on line %3 of the residue file.\n"))
                  .arg(new_residue.comment)
...

        {
          ...
          // us_qdebug( QString( "vdwf[%1].mw %2 .r %3" ).arg( name ).arg( vdwf[
          // name ].mw ).arg( vdwf[ name ].r ) );
        }
        ...
        //    ionization information,\nbut there is atom in this residue already
        //    defining ionization information:\n" ) +
        //    //                          linein,
        //    //                          QMessageBox::Ok,
      ...
      if (!new_residue.name.isEmpty() && new_residue.molvol > 0.0 &&
        ...
        if (advanced_config.debug_1) {
          printf(
              "residue name %s unique name %s atom size %u alt size %u pos "
              "%u\n",
  ...
  if (misc.export_msroll) {
    ...
    if (!f_radii.open(QIODevice::WriteOnly)) {
      ...
      f_radii.close();
      editor_msg("blue", QString(us_tr("Notice: created MSROLL radii file: %1"))
                             .arg(f_radii.fileName()));
    }
    ...
    if (!f_names.open(QIODevice::WriteOnly)) {
      ...
      f_names.close();
      editor_msg("blue", QString(us_tr("Notice: created MSROLL names file: %1"))
                             .arg(f_names.fileName()));
    }
  }
  ...
  //    " << residue_list[ i ].unique_name << " " << residue_list[ i ].comment
  //    << Qt::endl;
  // }
          ...
          if (first_atom_map.count("N")) {
            if (hybrid_name_to_N.count(first_atom_map["N"]->hybrid.name)) {
              ...
              double this_mw = spec_mw[use_name] -
                               first_atom_map["N"]->hybrid.mw -
                               first_atom_map["N"]->hybrid.ionized_mw_delta;
              double this_mv = this_mw * spec_psv[use_name];
              delta_mw[hybrid_name_to_N[first_atom_map["N"]->hybrid.name]] +=
                  this_mw;
              delta_mv[hybrid_name_to_N[first_atom_map["N"]->hybrid.name]] +=
                  this_mv;
            } else {
              QTextStream(stdout)
                  << " WARNING: first N in chain has unexpected hybridization "
...
#if defined(DEBUG_VBAR)
      QTextStream(stdout) << "mw " << it->first
                          << "'s            : " << it->second << Qt::endl
                          << "mv " << it->first
                          << "'s            : " << delta_mv[it->first]
                          << Qt::endl;
  ...
  if (use_p_atom) {
    ...
    for (int j = 0; j < chains; ++j) {
      ...
      for (int k = 0; k < atoms; ++k) {
        ...
        mw_sum += this_mw;
        mw_vbar_sum +=
            this_mw * model->molecule[j].atom[k].p_residue->vbar_at_pH;
#if defined(DEBUG_VBAR)
...
#if defined(DEBUG_VBAR)
  qsl << "pH :" << le_pH->text() << "covolume :" << QString("%1").arg(covolume);
  usl->log(qsl.join(","));
  qsl.clear();
  delete usl;
#endif
...
bool US_Hydrodyn::residue_atomic_vs_bead_hydration_differences(
...
void US_Hydrodyn::compute_residues_with_atomic_vs_bead_hydration_differences(
...
bool US_Hydrodyn::model_vector_has_hydration_differences(
...
void US_Hydrodyn::clear_temp_chain(
    struct PDB_chain
...
bool US_Hydrodyn::assign_atom(const QString &str1, struct PDB_chain *temp_chain,
                              QString *last_resSeq) {
  /*
  ...
  */
  ...
  if (temp_atom.resName ==
          "HOH" ||  // we dont want to add waters to the sequence
  ...
  *last_resSeq = temp_atom.resSeq;  // update last residue sequence number
  ...
  // name, we get the 1st residue reference fixed up with reset_chain_residues()
  ...
  if (cystine_residues.count(temp_atom.resName)) {
    sulfur_pdb_chain_atom_idx[temp_atom.chainID][temp_atom.resSeq].push_back(
        (unsigned int)(temp_chain->atom.size() - 1));
    ...
    while (!ts.atEnd()) {
          ...
          "ENDMDL")  // we need to save the previously recorded molecule
      {
        ...
        if (temp_model.molecule.size()) {
          SS_apply(
              temp_model,
              QString("%1_model_%2").arg(project).arg(temp_model.model_id));
          ...
          {
            QFont courier =
                QFont("Courier", USglobal->config_list.fontSize - 1);
              ...
              if (mw_nonwat) {
                if (mw_wat) {
                  ...
                  editor_msg(
                      "dark blue",
                      QString("\nHydration [g/g] %1")
                          .arg(tot_theo_wat * 18.01528 / mw_nonwat, 0, 'g', 3));
                }
              }
          ...
          editor_msg("dark red",
                     QString(us_tr("Warning: Empty model in PDB ignored\n")));
        }
        clear_temp_chain(
          ...
          str1.left(6) == "HETATM")  // need to add TER
      {
        if (str1.mid(12, 1) != "H" && str1.mid(13, 1) != "H" &&
          ...
          if (str1.mid(16, 1) == " " || str1.mid(16, 1) == "A") {
            if (str1.mid(16, 1) == "A") {
              editor->append(QString("Atom %1 conformation A selected")
                                 .arg(str1.mid(6, 5)));
            }
                    ...
                    // same
            {
              ...
              if (!break_chain && currently_aa_chain && known_residue &&
                ...
                if (advanced_config.debug_2) {
                  printf(
                      "chain break forced  !break_chain && currently_aa_chain "
  ...
  if (model_vector_has_hydration_differences(model_vector)) {
    editor_msg("darkred",
               us_tr("WARNING: PDB contains residues with bead hydration "
                     "without atomic hydration,\nvdW models should not be used "
                     "a they rely on atomic hydration\n\n"));
  }
  return 0;
}
    ...
    for (unsigned int j = 0; j < model_vector[i].molecule.size(); j++) {
      ...
      if (ask_convert) {
        switch (QMessageBox::information(
            ...
            us_tr(QString("Chain %1 Molecule %2 only contains A, G & C "
                          "residues (so far)\n"
                          "Is it DNA or RNA?")
                      .arg(chainID)
                      .arg(i + 1)),
      ...
      if (convert_this) {
        if (!already_messaged.count(QString("%1|%2").arg(i).arg(chainID))) {
          ...
          editor->append(us_tr(QString("Converting Chain %1 Molecule %2 to "
...
void US_Hydrodyn::update_vbar() {
  ...
  {  // If we computed vbar, we assign this to result.vbar, which should be used
    ...
    results.vbar = misc.vbar - (4.25e-4 * (K0 + misc.vbar_temperature - K20));
  }
}
      ...
      if (!checked.count(idx)) {
        ...
        if (this_atom->p_residue && this_atom->model_residue_pos != -1 &&
          ...
          if (model->residue[this_atom->model_residue_pos].unique_name !=
            ...
            QTextStream(stdout)
                << QString("found residue difference %1 %2, fixing\n")
...
double US_Hydrodyn::use_vbar(double vbar) { return tc_vbar(vbar); }
...
QString US_Hydrodyn::vbar_msg(double vbar, bool only_used) {
  ...
  return QString(
             us_tr("Vbar calculated                  : %1 [cm^3/g] @ %2%3C\n"
...
QString US_Hydrodyn::pH_msg() { return QString("pH %1\n").arg(hydro.pH); }
...
QString US_Hydrodyn::model_summary_msg(const QString &msg,
  ...
  if (model->molar_volume > 0) {
    qs +=
        QString(
            us_tr("Molar volume                     : %1 [cm^3/mol] @ %2%3C\n"))
  ...
  if (hydro.temperature != 20) {
    qs +=
        QString(us_tr("Anh. Molecular vol. (from vbar)  : %1 [" +
  ...
  qs += QString(us_tr("Anh. Molecular vol. (from vbar)  : %1 [%12^3] @ %2%3C\n"
...
void US_Hydrodyn::calc_mw() {
  ...
  for (unsigned int i = 0; i < model_vector.size(); i++) {
    ...
    //    ).arg( model_vector[i].model_id ) ); editor_msg( "dark blue", courier,
    //    vbar_msg( model_vector[i].vbar ) ); editor_msg( "dark blue", courier,
    //    visc_dens_msg() );
    // }
    ...
    // create_beads in calc_mw() : model_vector[ %1 ]" ).arg( i ), model_vector[
    // i ], true );
    ...
    if (!error_string.length()) {
      if (do_vvv) {
        vvv_file.setFileName(somo_dir + "/" + "tmp" + "/" + project +
                             QString("_%1.xyzr").arg(model_vector[i].model_id));
        if (!vvv_file.open(QIODevice::WriteOnly)) {
          editor_msg(
              "red",
              QString(
                  us_tr("Error: VVV requested but can not open %1 for writing"))
      ...
      if (do_scol) {
        scol_file.setFileName(somo_dir + "/" + "tmp" + "/" + project +
                              QString("_%1.csv").arg(model_vector[i].model_id));
        if (!scol_file.open(QIODevice::WriteOnly)) {
          editor_msg("red",
                     QString(us_tr("Error: save as CSV on load PDB requested "
          ...
          scol_ts
              << "\"Atom number\",\"Atom name\",\"Residue number\",\"Residue "
                 "name\",\"Radius\",\"Mass [Da]\",\"x\",\"y\",\"z\"\n";
        }
      }
      ...
      for (unsigned int j = 0; j < model_vector[i].molecule.size(); j++) {
          ...
          if (this_atom->active) {
            ...
            if (do_excl_vol) {
              ...
              if (!saxs_util->set_excluded_volume(
                      *this_atom, excl_vol, scaled_excl_vol, saxs_options,
                      residue_atom_hybrid_map, this_e, this_e_noh, si)) {
        ...
        //                     electron density %3 A^-3") .arg( chain_excl_vol )
      ...
      if (do_vvv) {
        ...
        vvv::compute_vol_surf(vvv_file.fileName().toLatin1().data(),
                              asa.vvv_probe_radius, asa.vvv_grid_dR, volume,
                              surf);
        editor_msg("black", QString("VVV: probe %1 (A) grid side %2 (A) volume "
...

    {
      ...
      // model_vector[i].mw + model_vector[ i ].ionized_mw_delta ); QString qs =
      ...
      //    .arg( do_excl_vol ?
      //          QString(", atomic volume %1 A^3%2 average electron density %3
      //          A^-3") .arg( tot_excl_vol ) .arg( tot_excl_vol !=
      //          tot_scaled_excl_vol ?
      //                QString(", scaled atomic volume %1 A^2")
...
#if defined(U_EXPT)
      {
        ...
        //    .arg( 100.0 * ( ( model_vector[i].mw / 1000e0 ) - pow( Rg / 6.5e0,
        //    3e0 ) ) / ( model_vector[i].mw / 1000e0 ), 0, 'f', 1 )
        //    ;
      }
#endif
      // // qs += "\n" + vbar_msg( model_vector[i].vbar );
    ...
    //       us_qdebug( QString( "model %1 molecule %2 mw %3" ).arg( i ).arg( j
    //       ).arg( model_vector[ i ].molecule[ j ].mw ) );
    //    }
    //    us_qdebug( QString( "" ).sprintf(
    //                                  "model %d total from molecules mw %.2f
    //                                  as model mw %.2f cm mw %.2f model_mw
    //                                  %.2f", i, tmp_mw, model_vector[ i ].mw,
    //                                  total_cm_mw,
    //                                  model_mw
    //                                   ) );
    // }
    ...
    // info_model_residues( "after update_model_chain_ionization in calc_mw()",
    // model_vector[ i ] );
  }
  editor_msg("black", "");
...
void US_Hydrodyn::update_model_chain_ionization(struct PDB_model &model,
                                                bool quiet) {
  if (!quiet) {
    QTextStream(stdout) << "==================================================="
                           "====================================="
  ...
  for (int j = 0; j < chains; ++j) {
    ...
    for (int k = 0; k < atoms; ++k) {
      ...
      //    ].r_atom[ model.molecule[ j ].atom[ k ].atom_assignment ].hydration;
      ...
      QTextStream(stdout)
          ...
          // ].bead_number      = " << model.molecule[ j ].atom[ k ].bead_number
          // << Qt::endl
          << " model.molecule[ " << j << " ].atom[ " << k
          << " ].model_residue_pos = "
      ...
      QTextStream(stdout)
          ...
          << model.molecule[j]
                 .atom[k]
                 .p_residue->r_atom[model.molecule[j].atom[k].atom_assignment]
                 .hybrid.mw
          << Qt::endl
          << " model.molecule[ " << j << " ].atom[ " << k
          << " ].p_residue->r_atom[ "
      ...
      QTextStream(stdout)
          ...
          << model.molecule[j]
                 .atom[k]
                 .p_residue->r_bead[model.molecule[j].atom[k].bead_assignment]
                 .mw
          << Qt::endl
          << " model.molecule[ " << j << " ].atom[ " << k
          << " ].p_residue->r_bead[ "
...
vector<double> US_Hydrodyn::basic_fractions(float pH, struct residue *res) {
  ...
  if (pKas == 2) {
    ...
    if (fabs(a_debug - a) > 1e-10) {
      qDebug() << QString(
                      "US_Hydrodyn::basic_fractions(): Internal error - frac a "
                      "varies %1 %2 ")
                      .arg(a_debug)
                      .arg(a);
    }
    if (fabs(a + b + c - 1) > 1e-10) {
      qDebug() << QString(
                      "US_Hydrodyn::basic_fractions(): Internal error - frac "
                      "total != 1:  %1 + %2 + %3 = %4 ")
                      .arg(a)
                      .arg(b)
                      .arg(c)
    ...
    if (a < 0 || b < 0 || c < 0) {
      qDebug() << QString(
                      "US_Hydrodyn::basic_fractions(): Internal error - "
                      "negative fraction != 1:  %1  %2  %3 ")
                      .arg(a)
                      .arg(b)
                      .arg(c);
    }
  ...
  QString msg =
      "US_Hydrodyn::basic_fractions(): Internal error - too many pKas - "
      "unsupported";
  qDebug() << msg;
  editor_msg("red", msg);
  return fractions;
...
double US_Hydrodyn::ionized_residue_vbar(vector<double> &fractions,
                                         struct residue *res) {
  ...
  QString msg =
      "US_Hydrodyn::ionized_residue_vbar(): Internal error - too many "
...
double US_Hydrodyn::ionized_residue_atom_mw(vector<double> &fractions,
                                            struct residue *res,
                                            struct atom *atom) {
  int size = (int)fractions.size();
  ...
  if (!res->r_atom_0.count(atom->ionization_index) ||
    ...
    QString msg =
        "US_Hydrodyn::ionized_residue_atom_mw(): Internal error - r_atom maps "
        "missing";
    qDebug() << msg;
    editor_msg("red", msg);
    return 0e0;
  ...
  if (atom->ionization_index > size) {
    QString msg =
        "US_Hydrodyn::ionized_residue_atom_mw(): Internal error - atom's "
        "ionization index is out of bounds";
    qDebug() << msg;
    editor_msg("red", msg);
    return 0e0;
  }
  ...
  if (size == 2) {
    return fractions[1] * (res->r_atom_1[atom->ionization_index].hybrid.mw -
...
double US_Hydrodyn::ionized_residue_atom_radius(vector<double> &fractions,
                                                struct residue *res,
                                                struct atom *atom) {
  int size = (int)fractions.size();
  if (size == 1 || !atom->ionization_index) {
...
double US_Hydrodyn::ionized_residue_atom_hydration(vector<double> &fractions,
                                                   struct residue *res,
                                                   struct atom *atom) {
  int size = (int)fractions.size();
  if (size == 1 || !atom->ionization_index) {
...
double US_Hydrodyn::ionized_residue_atom_protons(vector<double> &fractions,
                                                 struct residue *res,
                                                 struct atom *atom) {
  int size = (int)fractions.size();
  if (size == 1 || !atom->ionization_index) {
...
void US_Hydrodyn::set_ionized_residue_vector(
    vector<struct residue> &residue_v) {
  ...
  for (int j = 0; j < residues; ++j) {
    ...
    for (int k = 0; k < atoms; ++k) {
      ...
      residue_v[j].r_atom[k].hybrid.protons = ionized_residue_atom_protons(
          fractions, &residue_v[j], &residue_v[j].r_atom[k]);
      ...
      // vdwf
      {
        QString name =
            QString("%1|%2")
                .arg(residue_v[j].r_atom[k].name != "OXT" ? residue_v[j].name
                                                          : "OXT")
    ...
    for (int k = 0; k < beads; ++k) {
      ...
      if (!residue_v[j].r_bead[k].hydration_flag) {
        QTextStream(stdout)
            << "residue name " << residue_v[j].name << " bead " << (k + 1)
            << " atom_hydration " << residue_v[j].r_bead[k].atom_hydration
            << Qt::endl;
      }
#endif
    }
    if (bead_ionized_mw.size()) {
      QTextStream(stdout)
          << "**** US_Hydrodyn::set_ionized_residue_vector_apply(): unassigned "
             "bead_ionized_mw! ****"
          << Qt::endl;
    }
  }
...
bool US_Hydrodyn::model_summary_csv(struct PDB_model *model,
  ...
  data << QString("%1").arg(mw_to_volume(model->mw + model->ionized_mw_delta,
                                         tc_vbar(model->vbar)))
       << QString("%1").arg(mw_to_volume(model->mw + model->ionized_mw_delta,
                                         tc_vbar(model->vbar)) +
                            model->hydration * misc.hydrovol)
