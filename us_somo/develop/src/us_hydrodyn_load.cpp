...
// routines such as file i/o us_hydrodyn_info.cpp contains code to report
// structures for debugging us_hydrodyn_util.cpp contains other various code,
// such as disulfide code (this) us_hydrodyn_load.cpp contains code to load
// files
...
void
US_Hydrodyn::read_hybrid_file (QString filename)
{
          ...
          if (net_charge.indexIn (current_hybrid.saxs_name) != -1)
            {
              ...
              protons += delta;
              // QTextStream( stdout ) << "saxs_name " <<
              // current_hybrid.saxs_name << " delta " << delta << Qt::endl; }
              // else { QTextStream( stdout ) << "saxs_name " <<
              // current_hybrid.saxs_name << " delta 0" << Qt::endl;
            }
          hybrid_to_electrons[current_hybrid.name] = current_hybrid.num_elect;
          hybrid_to_protons[current_hybrid.name] = protons;
          // QTextStream( stdout ) << "hybrid.name " << current_hybrid.name <<
          // " protons " << protons << " electrons " <<
          // current_hybrid.num_elect << Qt::endl;
        }
      f.close ();
    }
  else
...
US_Hydrodyn::read_residue_file ()
{
  ...
  if (advanced_config.debug_1)
    {
      QTextStream (stdout) << "residue file name: " << residue_filename
                           << Qt::endl;
    }
  residue_list.clear ();
      ...
      while (!ts.atEnd ())
        {
          ...
          line_count++;
          // QTextStream( stdout ) << (residue_list.size() + 1 ) << " " <<
          // new_residue.comment << Qt::endl;
          {
            QStringList qsl
                = (ts.readLine ()).split (rx_spaces, Qt::SkipEmptyParts);
            if ((qsl.size () - 7) % 2)
              {
                // intermediate way
                // if ( qsl.size() != 7 && qsl.size() != 9 ) {
                QMessageBox::critical (
                    this, us_tr (windowTitle ()),
                    us_tr ("Please note:\n\nThere was an error reading the "
                           "selected Residue File!\n"
            ...
            new_residue.name = qsl.front ();
            qsl.pop_front ();
            new_residue.type = qsl.front ().toUInt ();
            qsl.pop_front ();
            new_residue.molvol = qsl.front ().toDouble ();
            qsl.pop_front ();
            new_residue.asa = qsl.front ().toDouble ();
            qsl.pop_front ();
            numatoms = qsl.front ().toUInt ();
            qsl.pop_front ();
            numbeads = qsl.front ().toUInt ();
            qsl.pop_front ();
            new_residue.vbar = qsl.front ().toDouble ();
            qsl.pop_front ();
            ...
            while (qsl.size ())
              {
                new_residue.vbars.push_back (qsl.front ().toDouble ());
                qsl.pop_front ();
                new_residue.pKas.push_back (qsl.front ().toDouble ());
                qsl.pop_front ();
              }
            ...
            if (new_residue.pKas.size () > 2)
              {
                QMessageBox::critical (
                    ...
                    us_tr ("Please note:\n\nThere was an error reading the "
                           "selected Residue File!\n"
                           "Line contains too many pKa/vbar paris, currently "
                           "only 2 are supported:\n"
                           "Please contact the developers to enable 3 "
                           "pKas/vbars for a residue.\n")
          ...
          for (j = 0; j < numatoms; ++j)
            {
              ...
              new_atom.name = qsl.front ();
              qsl.pop_front ();
              new_atom.hybrid.name = qsl.front ();
              qsl.pop_front ();
              new_atom.hybrid.mw = qsl.front ().toFloat ();
              qsl.pop_front ();
              new_atom.hybrid.radius = qsl.front ().toFloat ();
              qsl.pop_front ();
              new_atom.bead_assignment = qsl.front ().toUInt ();
              qsl.pop_front ();
              ...
              if (!hybrid_to_protons.count (new_atom.hybrid.name)
                  || !hybrid_to_electrons.count (new_atom.hybrid.name))
                {
                  editor_msg (
                      "red",
                      QString (us_tr ("Hybridization information missing for "
                                      "%1, net charge, proton count and "
                                      "isoelectric point will be incorrect!"))
                          .arg (new_atom.hybrid.name));
                }
              else
                {
                  new_atom.hybrid.protons
                      = hybrid_to_protons[new_atom.hybrid.name];
                  new_atom.hybrid.num_elect
                      = hybrid_to_electrons[new_atom.hybrid.name];
                }
              ...
              new_atom.positioner = (bool)qsl.front ().toInt ();
              qsl.pop_front ();
              new_atom.serial_number = qsl.front ().toUInt ();
              qsl.pop_front ();
              new_atom.hydration = qsl.front ().toFloat ();
              qsl.pop_front ();
              ...
              if (!atom_serial_to_match.count (new_atom.serial_number))
                {
                  QMessageBox::critical (
                      this, us_tr (windowTitle ()),
                      QString (us_tr ("\nThe atom's serial number is out of "
                                      "range or duplicated.\n"
                                      "For residue: %1 and Atom: %2 "
                                      "on line %3 of the residue file.\n"))
                          .arg (new_residue.comment)
...

              {
                ...
                QString name
                    = QString ("%1|%2")
                          .arg (new_atom.name != "OXT" ? new_residue.name
                                                       : "OXT")
                          .arg (new_atom.name);
                if (!vdwf.count (name))
                  {
                    vdwf[name] = this_vdwf;
                    bead_assignment_to_vdwf[new_atom.bead_assignment].insert (
                        name);
                  }
                // us_qdebug( QString( "vdwf[%1].mw %2 .r %3" ).arg( name
                // ).arg( vdwf[ name ].mw ).arg( vdwf[ name ].r ) );
              }
              ...
              if (qsl.size ())
                {
                  ...
                  int index = qsl.front ().toInt ();
                  qsl.pop_front ();
                  new_atom_1.name = new_atom.name;
                  new_atom_1.hybrid.name = qsl.front ();
                  qsl.pop_front ();
                  new_atom_1.hybrid.mw = qsl.front ().toFloat ();
                  qsl.pop_front ();
                  new_atom_1.hybrid.radius = qsl.front ().toFloat ();
                  qsl.pop_front ();
                  new_atom_1.bead_assignment = qsl.front ().toUInt ();
                  qsl.pop_front ();
                  new_atom_1.hybrid.ionized_mw_delta = 0e0;
                  new_atom_1.positioner = (bool)qsl.front ().toInt ();
                  qsl.pop_front ();
                  new_atom_1.serial_number = qsl.front ().toUInt ();
                  qsl.pop_front ();
                  new_atom_1.hydration = qsl.front ().toFloat ();
                  qsl.pop_front ();
                  if (!hybrid_to_protons.count (new_atom_1.hybrid.name))
                    {
                      editor_msg (
                          "red",
                          QString (
                              us_tr ("Hybridization information missing for "
                                     "%1, net charge, proton count and "
                                     "isoelectric point will be incorrect!"))
                              .arg (new_atom_1.hybrid.name));
                    }
                  else
                    {
                      new_atom_1.hybrid.protons
                          = hybrid_to_protons[new_atom_1.hybrid.name];
                    }
                  ...
                  if (new_atom_1.bead_assignment != new_atom.bead_assignment
                      ...
                      || new_atom_1.serial_number != new_atom.serial_number)
                    {
                      QMessageBox::critical (
                          ...
                          us_tr ("Please note:\n\nThere was an error reading "
                                 "the selected Residue File!\n"
                                 "mismatch in ionized bead assignment, "
                                 "positioner and/or serial number:\n")
                              + linein,
                          QMessageBox::Ok, QMessageBox::NoButton,
                          QMessageBox::NoButton);
                      return;
              ...
              // intermediate way
              // if ( qsl.size() ) {
              //    new_atom.ionization_mass_change = qsl.front().toFloat();
              //    qsl.pop_front();
              ...
              //    contains ionization information,\nbut there is atom in this
              //    residue already defining ionization information:\n" ) +
              //    //                          linein,
              //    //                          QMessageBox::Ok,
              //    QMessageBox::NoButton, QMessageBox::NoButton);
              //    //    return;
              //    // }
              //    // res_ionization_mass_change[ new_residue.name ] =
              //    new_atom.ionization_mass_change; new_residue.acid_residue =
              //    ( new_atom.ionization_mass_change < 0 ); qDebug() << "found
              //    ionization mass change for " << new_residue.name << " value
              //    " << new_atom.ionization_mass_change;
              // } else {
              ...
              else
                {
                  QMessageBox::warning (
                      ...
                      us_tr (
                          ...
                          "\nAtom "
                          + new_atom.name
                          + " cannot be read and will be deleted from List."),
                      QMessageBox::Ok, QMessageBox::NoButton,
                      QMessageBox::NoButton);
                }
            }
          ...
          if (pKas_to_match.size ())
            {
              QMessageBox::critical (
                  ...
                  us_tr (
                      "Please note:\n\n"
                      "There was an error reading the selected Residue File!\n"
                      "The are missing atom entries for the given number of "
                      "pKa/vbars\n"
                      "Residue name : "
                      + new_residue.name),
                  QMessageBox::Ok, QMessageBox::NoButton,
          ...
          for (j = 0; j < numbeads; j++)
            {
              ...
              if (bead_assignment_to_vdwf.count (j))
                {
                  for (set<QString>::iterator it
                       = bead_assignment_to_vdwf[j].begin ();
                       it != bead_assignment_to_vdwf[j].end (); ++it)
                    {
              ...
              if (advanced_config.debug_1)
                {
                  printf (
                      "residue name %s loading bead %d placing method %d\n",
                      new_residue.name.toLatin1 ().data (), j,
                      new_bead.placing_method);
                  fflush (stdout);
                }
      ...
      if (!f_radii.open (QIODevice::WriteOnly))
        {
          editor_msg (
              "red",
              QString (us_tr ("Error: can not create MSROLL radii file: %1"))
                  .arg (f_radii.fileName ()));
        }
      else
        {
          ...
          f_radii.close ();
          editor_msg ("blue",
                      QString (us_tr ("Notice: created MSROLL radii file: %1"))
                          .arg (f_radii.fileName ()));
        }
      ...
      if (!f_names.open (QIODevice::WriteOnly))
        {
          editor_msg (
              "red",
              QString (us_tr ("Error: can not create MSROLL names file: %1"))
                  .arg (f_names.fileName ()));
        }
      else
        {
          ...
          f_names.close ();
          editor_msg ("blue",
                      QString (us_tr ("Notice: created MSROLL names file: %1"))
                          .arg (f_names.fileName ()));
        }
    }
  // qDebug() << "residues " << residue_list.size();
  // for ( int i = 0; i < (int) residue_list.size(); ++i ) {
  //    QTextStream( stdout ) << ( i + 1 ) << " " << residue_list[ i ].name <<
  //    " " << residue_list[ i ].unique_name << " " << residue_list[ i
  //    ].comment << Qt::endl;
  // }
  compute_residues_with_atomic_vs_bead_hydration_differences ();
  QTextStream (stdout)
      << list_residues_with_atomic_vs_bead_hydration_differences ();
}
...
double
US_Hydrodyn::calc_vbar_updated (struct PDB_model &model)
{
  int chains = (int)model.molecule.size ();
  int residues = (int)model.residue.size ();
  // spec residue info
  map<QString, QString> spec_residue
      = { { "N1", "N1" }, { "N1-", "N1-" }, { "OXT", "OXT" } };
  map<QString, double> spec_mw;
  map<QString, double> spec_psv;

  for (auto it = spec_residue.begin (); it != spec_residue.end (); ++it)
    {
      if (multi_residue_map.count (it->first))
        {
          for (int j = 0; j < (int)multi_residue_map[it->first].size (); ++j)
            {
              struct residue residue_entry
                  = residue_list[multi_residue_map[it->first][j]];
              map<QString, struct atom *> res_atom_map
                  = residue_atom_map (residue_entry);
              if (res_atom_map.count (it->second))
                {
                  spec_mw[it->first]
                      = res_atom_map[it->second]->hybrid.mw
                        + res_atom_map[it->second]->hybrid.ionized_mw_delta;
                  spec_psv[it->first] = residue_entry.vbar_at_pH;
                }
              else
                {
                  QTextStream (stdout)
                      << "WARNING: residue list atom lookup name missing: "
                      << it->second << Qt::endl;
                }
            }
        }
        ...
        {
          QTextStream (stdout)
              << "WARNING:  multi residue map is missing this residue: "
              << it->first << Qt::endl;
        }
    }
              ...
              if (first_atom_map.count ("N"))
                {
                  if (hybrid_name_to_N.count (
                          first_atom_map["N"]->hybrid.name))
                    {
                      ...
                      double this_mw
                          = spec_mw[use_name] - first_atom_map["N"]->hybrid.mw
                            - first_atom_map["N"]->hybrid.ionized_mw_delta;
                    ...
                    {
                      QTextStream (stdout)
                          << " WARNING: first N in chain has unexpected "
                             "hybridization "
                          << first_atom_map["N"]->hybrid.name << Qt::endl;
                    }
                }
            }
            {
              map<QString, struct atom *> last_atom_map
                  = last_residue_atom_map (model.molecule[j]);
  ...
  {
    double covolume
        = gparams.count ("covolume") ? gparams["covolume"].toDouble () : 0e0;
    double total_mw = mw;
    double total_mv = mv;
    ...
    for (auto it = delta_mw.begin (); it != delta_mw.end (); ++it)
      {
#if defined(DEBUG_VBAR)
        QTextStream (stdout)
            << "mw " << it->first << "'s            : " << it->second
            << Qt::endl
            << "mv " << it->first << "'s            : " << delta_mv[it->first]
            << Qt::endl;
#endif
        total_mw += it->second;
...
#if defined(DEBUG_VBAR)
    QTextStream (stdout) << "total mw            : " << total_mw << Qt::endl
                         << "total mv            : " << total_mv << Qt::endl
                         << "covolume            : " << covolume << Qt::endl
                         << "total mv + covolume : " << (total_mv + covolume)
                         << Qt::endl
                         << "vbar                : "
                         << ((total_mv + covolume) / total_mw) << Qt::endl;
#endif
...
void
US_Hydrodyn::calc_vbar (struct PDB_model *model, bool use_p_atom)
{
...
#if defined(DEBUG_VBAR)
  QString lname = QString ("/tmp/vbar-%1-pH%2.csv")
                      .arg (QFileInfo (le_pdb_file->text ()).fileName ())
                      .arg (le_pH->text ());
          ...
          for (int k = 0; k < atoms; ++k)
            {
              ...
              double this_mw = model->molecule[j].atom[k].p_atom->hybrid.mw
                               + model->molecule[j]
                                     .atom[k]
              ...
              mw_vbar_sum
                  += this_mw
                     * model->molecule[j].atom[k].p_residue->vbar_at_pH;
#if defined(DEBUG_VBAR)
              qsl << model->molecule[j].atom[k].resName
              ...
              for (int j = 0; j < atoms; ++j)
                {
                  mw += model->residue[i].r_atom[j].hybrid.mw
                        + model->residue[i].r_atom[j].hybrid.ionized_mw_delta;
                }
              mw_sum += mw;
  ...
  double covolume
      = gparams.count ("covolume") ? gparams["covolume"].toDouble () : 0e0;
  model->vbar
      = (double)floor (0.5 + (((mw_vbar_sum + covolume) / mw_sum) * 1000e0))
        / 1000e0; //  - 0.002125;

#if defined(DEBUG_VBAR)
  qsl << "pH :" << le_pH->text ()
      << "covolume :" << QString ("%1").arg (covolume);
  usl->log (qsl.join (","));
  qsl.clear ();
  delete usl;
#endif
...
bool
US_Hydrodyn::residue_atomic_vs_bead_hydration_differences (
    const struct residue &r)
{
  ...
  for (auto &a : r.r_atom)
    {
      if (a.ionization_index || a.hydration || a.hydration2)
        {
          return false;
        }
    }
...
void
US_Hydrodyn::compute_residues_with_atomic_vs_bead_hydration_differences (
    const vector<struct residue> &rl)
{
      ...
      if (residue_atomic_vs_bead_hydration_differences (r))
        {
          ...
          //    info_residue( r_tmp, "no atomic hydration but has bead
          //    hydration" );
          // }
...
QString
US_Hydrodyn::list_residues_with_atomic_vs_bead_hydration_differences ()
{
  QStringList qsl;
...
bool
US_Hydrodyn::model_vector_has_hydration_differences (
    const vector<struct PDB_model> &m)
{
  for (auto &model : m)
    {
      for (auto &chain : model.molecule)
        {
          for (auto &a : chain.atom)
            {
              if (residues_with_atomic_vs_bead_hydration_differences.count (
                      a.resName))
                {
                  return true;
                }
...
void
US_Hydrodyn::calc_bead_mw (struct residue *res)
{
  double rmw = 0.0;
          ...
          if ((*res).r_atom[j].bead_assignment == i)
            {
              ...
              // QTextStream( stdout ) << res->name << " bead " << i << " atom
              // " << res->r_atom[j].name << " mw " << res->r_atom[j].hybrid.mw
              // << Qt::endl;
            }
        }
      if (advanced_config.debug_1)
        {
          QTextStream (stdout) << res->name << " bead " << i << " mw "
                               << res->r_bead[i].mw << Qt::endl;
        }
    }
  if (advanced_config.debug_1)
    {
...
void
US_Hydrodyn::clear_temp_chain (
    struct PDB_chain
        *temp_chain) // clear all the memory from the vectors in temp_chain
{
...
bool
US_Hydrodyn::assign_atom (const QString &str1, struct PDB_chain *temp_chain,
                          QString *last_resSeq)
{
  /*
    ...
    resSeq        Residue sequence number. 27             AChar           iCode
    Code for insertion of residues. 31 - 38        Real(8.3)       x Orthogonal
    ...
    left-justified. 77 - 78        LString(2)      element       Element
  ...
  */
  ...
  if (temp_atom.resName == "HOH"
      || // we dont want to add waters to the sequence
  ...
  *last_resSeq = temp_atom.resSeq; // update last residue sequence number
  ...
  // //    {
  // //       // this can be wrong, as there can be multiple entries for the
  // same residue name
  ...
  // // if (!found)
  // // {
  // //    //   printError(us_tr("The residue " + temp_atom.resName + " listed
  // in this PDB file is not found in the residue table!"));
  // // }

  // this can be wrong, as there can be multiple entries for the same residue
  // name, we get the 1st residue reference fixed up with
  // reset_chain_residues()

  if (multi_residue_map.count (temp_atom.resName)
      && multi_residue_map[temp_atom.resName].size ())
    {
  ...
  if (cystine_residues.count (temp_atom.resName))
    {
      sulfur_pdb_chain_atom_idx[temp_atom.chainID][temp_atom.resSeq]
          .push_back ((unsigned int)(temp_chain->atom.size () - 1));
...
int
US_Hydrodyn::read_pdb (const QString &filename)
{
  // info_residue_vector( "read_pdb():: begin", residue_list );
  set_ionized_residue_vector (residue_list);
  // info_residue_vector( "read_pdb():: after set_ionized_residue_vector",
  // residue_list );
  ...
  QRegExp rx_water_multiplier ("^REMARK Multiply water Iq by (\\d+)",
                               Qt::CaseInsensitive);
      ...
      while (!ts.atEnd ())
        {
          ...
          if (advanced_config.experimental_renum)
            {
              if (str1.left (5) == "MODEL" || str1.left (6) == "ENDMDL")
                {
                  exp_resnum = 1;
                  exp_prev_resnum = -1;
                }

              if (str1.left (4) == "ATOM" || str1.left (6) == "HETATM")
                {
                  ...
                  QString alt_str1 = str1.left (22)
                                     + QString ("").sprintf ("%4d", exp_resnum)
                                     + str1.right (str1.length () - 22 - 4);
                  // us_qdebug( QString( "<:%1\n>:%2" ).arg( str1 ).arg(
                  // alt_str1 ) );
                  str1 = alt_str1;
                }
          ...
          if (saxs_options.multiply_iq_by_atomic_volume
            ...
            {
              multiply_iq_by_atomic_volume_last_water_multiplier
                  = rx_water_multiplier.cap (1).toUInt ();
              QTextStream (stdout)
                  << QString ("found water multiplier %1 in pdb\n")
                         .arg (
                             multiply_iq_by_atomic_volume_last_water_multiplier);
            }
          ...
          if (str1.left (6)
              == "ENDMDL") // we need to save the previously recorded molecule
            {
              ...
              if (temp_model.molecule.size ())
                {
                  SS_apply (temp_model, QString ("%1_model_%2")
                                            .arg (project)
                                            .arg (temp_model.model_id));
                  editor_msg (
                      "black",
                  ...
                  for (unsigned int i = 0; i < temp_model.molecule.size ();
                       i++)
                    {
                      QString lastResSeq = "";
                      for (unsigned int j = 0;
                        ...
                        {
                          PDB_atom *this_atom
                              = &(temp_model.molecule[i].atom[j]);
                          QString resname = this_atom->resName;

                          QString res_idx
                              = QString ("%1|%2")
                                    .arg (this_atom->name != "OXT" ? resname
                                                                   : "OXT")
                                    .arg (this_atom->name);
                          ...
                          if (temp_model.molecule[i].atom[j].resSeq
                              != lastResSeq)
                            {
                              if (waters.count (resname))
                                {
                              ...
                              str += resname + " ";
                              sstr += residue_short_names.count (resname)
                                          ? QString (
                                                residue_short_names[resname])
                                          : "?";
                              sstr_pos++;
                              if (!(sstr_pos % 42))
                                {
                              ...
                              lastResSeq
                                  = temp_model.molecule[i].atom[j].resSeq;
                            }
                        }
                    }
                  ...
                  editor->append (str);
                  {
                    ...
                    editor_msg (
                        "black", courier,
                        "Residue\t count\tpercent\t Theoretical waters\n");
                    temp_model.hydration_gg = 0;

                    if (resname_counts_nonwat_total)
                      {
                        for (map<QString, int>::iterator it
                             = resname_counts_nonwat.begin ();
                             it != resname_counts_nonwat.end (); ++it)
                          {
                            editor_msg (
                                "black", courier,
                                QString ("%1\t %2\t%3%\t %4\n")
                                    ...
                                    .arg (it->second)
                                    .arg (
                                        floor (100 * 100.0 * (double)it->second
                                               / (double)
                                                   resname_counts_nonwat_total)
                                            / 100,
                                        0, 'g', 3)
                                    .arg ((double)(resname_theo_wat.count (
                                                       it->first)
                                                       ? resname_theo_wat
                                                             [it->first]
                                                       : 0),
                        ...
                        if (resname_counts_wat_total)
                          {
                            ...
                            for (map<QString, int>::iterator it
                                 = resname_counts_wat.begin ();
                                 it != resname_counts_wat.end (); ++it)
                              {
                                editor_msg (
                                    "blue", courier,
                                    QString ("%1\t%2\t%3\n")
                            ...
                            if (resname_counts_wat_total
                                && resname_counts_wat.size () > 1)
                              {
                                editor_msg (
                                    "blue", courier,
                                    QString ("All\t%1\t%2\n")
                                        .arg (resname_counts_wat_total)
                                        .arg (
                                            (double)resname_counts_wat_total
                                                / (double)
                                                    resname_counts_nonwat_total,
                                            0, 'g', 2));
                              }
                            ...
                            if (mw_wat)
                              {
                                ...
                                editor_msg (
                                    "dark blue",
                                    QString ("\nHydration [g/g] %1 (explicit "
                                             "waters)")
                                        .arg (mw_wat / mw_nonwat, 0, 'g', 3));
                              }
                            else if (tot_theo_wat)
                              {
                                temp_model.hydration_gg
                                    = tot_theo_wat * 18.01528 / mw_nonwat;
                                editor_msg ("dark blue",
                                            QString ("\nHydration [g/g] %1")
                                                .arg (tot_theo_wat * 18.01528
                                                          / mw_nonwat,
                                                      0, 'g', 3));
                              }
                          }
                  ...
                  // calc_vbar is wrong if there unknown residues, fixed later
                  // in check_for_missing_atoms()
                  calc_vbar (&temp_model); // update the calculated vbar for
                                           // this model
              ...
              else
                {
                  editor_msg ("dark red",
                              QString (us_tr (
                                  "Warning: Empty model in PDB ignored\n")));
                }
          ...
          if (str1.left (4) == "ATOM"
              || str1.left (6) == "HETATM") // need to add TER
            {
              if (str1.mid (12, 1) != "H" && str1.mid (13, 1) != "H"
                  ...
                  && !str1.mid (12, 5).trimmed ().contains (
                      QRegularExpression ("^\\dH"))
                  ...
                  if (str1.mid (16, 1) == " " || str1.mid (16, 1) == "A")
                    {
                      if (str1.mid (16, 1) == "A")
                        {
                          editor->append (
                              QString ("Atom %1 conformation A selected")
                                  .arg (str1.mid (6, 5)));
                        }
                           ...
                           // still the same
                        {
                          ...
                          if (!break_chain && currently_aa_chain
                              && known_residue && !this_is_aa)
                            {
                              if (advanced_config.debug_2)
                                {
                                  printf ("chain break forced  !break_chain "
                                          "&& currently_aa_chain && "
                                          "known_residue && !this_is_aa\n");
                                }
                              break_chain = true;
                 ...
                 it != resname_counts_nonwat.end (); ++it)
              {
                editor_msg (
                    ...
                    QString ("%1\t %2\t%3%\t %4\n")
                        .arg (it->first)
                        .arg (it->second)
                        .arg (floor (100 * 100.0 * (double)it->second
                                     / (double)resname_counts_nonwat_total)
                                  / 100,
                              0, 'g', 3)
                        .arg ((double)(resname_theo_wat.count (it->first)
                                           ? resname_theo_wat[it->first]
                                           : 0),
                              0, 'f', 0));
              }
            if (resname_counts_nonwat_total
                ...
                if (resname_counts_wat_total && resname_counts_wat.size () > 1)
                  {
                    editor_msg (
                        ...
                        QString ("All\t%1\t%2\n")
                            .arg (resname_counts_wat_total)
                            .arg ((double)resname_counts_wat_total
                                      / (double)resname_counts_nonwat_total,
                                  0, 'g', 2));
                  }
              }
                ...
                if (mw_wat)
                  {
                    ...
                    editor_msg (
                        "dark blue",
                        QString ("\nHydration [g/g] %1 (explicit waters)")
                            .arg (mw_wat / mw_nonwat, 0, 'g', 3));
                  }
                else if (tot_theo_wat)
                  {
                    ...
                    editor_msg ("dark blue",
                                QString ("\nHydration [g/g] %1")
                                    .arg (tot_theo_wat * 18.01528 / mw_nonwat,
                                          0, 'g', 3));
                  }
              }
            temp_model.hydration = tot_theo_wat;
  ...
  // info_model_vector( QString( "source: model_vector" ), model_vector, {
  // "GLU", "HIS", "CGU", "ALA" } ); info_model_vector_mw( QString( "read_pdb()
  // : model_vector" ), model_vector, true );
  ...
  if (model_vector_has_hydration_differences (model_vector))
    {
      editor_msg ("darkred",
                  us_tr ("WARNING: PDB contains residues with bead hydration "
                         "without atomic hydration,\nvdW models should not be "
                         "used a they rely on atomic hydration\n\n"));
    }
  return 0;
}

void
US_Hydrodyn::dna_rna_resolve ()
{
      ...
      for (unsigned int j = 0; j < model_vector[i].molecule.size (); j++)
        {
          ...
          if (ask_convert
              && already_asked.count (QString ("%1|%2").arg (i).arg (chainID)))
            {
              ask_convert
                  = already_asked[QString ("%1|%2").arg (i).arg (chainID)];
            }
          ...
          if (ask_convert)
            {
              switch (QMessageBox::information (
                  ...
                  us_tr (QString ("Chain %1 Molecule %2 only contains A, G & "
                                  "C residues (so far)\n"
                                  "Is it DNA or RNA?")
                             .arg (chainID)
                             .arg (i + 1)),
          ...
          if (convert_this)
            {
              if (!already_messaged.count (
                      QString ("%1|%2").arg (i).arg (chainID)))
                {
                  already_messaged[QString ("%1|%2").arg (i).arg (chainID)]
                      = true;
                  editor->append (
                      us_tr (QString ("Converting Chain %1 Molecule %2 to "
...
void
US_Hydrodyn::reset_chain_residues (PDB_model *model)
{
          ...
          if (!checked.count (idx))
            {
              ...
              us_qdebug (QString ("rcr idx %1 p_r %2 %3 %4")
                             .arg (idx)
                             .arg ((long)this_atom->p_residue)
                             .arg (this_atom->model_residue_pos)
                             .arg (model->residue.size ()));
#endif
              if (this_atom->p_residue && this_atom->model_residue_pos != -1
                  && (int)model->residue.size ()
                         < this_atom->model_residue_pos)
                {
                  if (model->residue[this_atom->model_residue_pos].unique_name
                      != this_atom->p_residue->unique_name)
                    {
                      QTextStream (stdout)
                          << QString (
                                 "found residue difference %1 %2, fixing\n")
                                 .arg (model
                                           ->residue[this_atom
                                                         ->model_residue_pos]
                                           .unique_name)
...
double
US_Hydrodyn::use_vbar (double vbar)
{
  return tc_vbar (vbar);
}

double
US_Hydrodyn::tc_vbar (double vbar)
{
  double TE = K0 + hydro.temperature;
  double partvol
      = (int)(((vbar + (4.25e-4 * (TE - K20))) * 1000) + 0.5) / 1000.0;

  if (!misc.compute_vbar)
    {
      partvol
          = (int)(((misc.vbar - (4.25e-4 * (K0 + misc.vbar_temperature - K20))
                    + (4.25e-4 * (TE - K20)))
                   * 1000)
                  + 0.5)
            / 1000.0;
...
QString
US_Hydrodyn::vbar_msg (double vbar, bool only_used)
{
  double partvol = tc_vbar (vbar);

  if (only_used)
    {
      return QString (us_tr ("Vbar used                        : %1 [cm^3/g] "
                             "@ %2%3C\n"))
          .arg (partvol, 5, 'f', 3, '0')
...
US_Hydrodyn::tc_solvent_visc ()
{
  ...
  return hydro.solvent_viscosity - default_hydro.solvent_viscosity + a
         + b * exp (hydro.temperature * mcinv);
}

double
US_Hydrodyn::use_solvent_dens ()
{
  if (hydro.manual_solvent_conditions)
    {
...
US_Hydrodyn::tc_solvent_dens ()
{
  ...
  return hydro.solvent_density - default_hydro.solvent_density
         + exp (a + b * hydro.temperature * hydro.temperature);
}
...
QString
US_Hydrodyn::visc_dens_msg (bool only_used)
{

  if (hydro.manual_solvent_conditions)
    {
      return QString (us_tr ("Manual flag set\n"
  ...
  else
    {
      ...
      return QString (us_tr ("Stored solvent name      %1\n"
                             "Stored solvent viscosity %2 [cP]\n"
...
QString
US_Hydrodyn::model_summary_msg (const QString &msg, struct PDB_model *model)
{
  ...
  if (pdb_parse.find_sh)
    {
      qs += QString (us_tr ("Number of disulfide bonds        : %1\n"
                            "Number of free SH                : %2\n"))
                .arg (model->num_SS_bonds)
                .arg (model->num_SH_free);
    }

  qs += QString (us_tr ("Calculation done at pH           : %1\n"))
            .arg (le_pH->text ());

  qs += QString (us_tr ("Molecular weight                 : %1 [Da]\n"))
            .arg (model->mw + model->ionized_mw_delta);

  if (model->molar_volume > 0)
    {
      qs += QString (us_tr ("Molar volume                     : %1 [cm^3/mol] "
                            "@ %2%3C\n"))
  ...
  qs += QString (us_tr ("SAXS excluded volume (anhydrous) : %1 [%2^3]\n"))
            .arg (model->volume)
            .arg (UNICODE_ANGSTROM);

  if (hydro.temperature != 20)
    {
      qs += QString (us_tr ("Anh. Molecular vol. (from vbar)  : %1 ["
                            + UNICODE_ANGSTROM_QS + "^3] @ %2%3C\n"))
                .arg (mw_to_volume (model->mw + model->ionized_mw_delta,
                                    model->vbar))
                .arg (20)
                .arg (DEGREE_SYMBOL);
    }
  ...
  qs += QString (
            us_tr ("Anh. Molecular vol. (from vbar)  : %1 [%12^3] @ %2%3C\n"
  ...
  if (model->volume)
    {
      qs += QString (us_tr ("Average electron density         : %1 [%2^-3]\n"))
...
float
US_Hydrodyn::mw_to_volume (float mw, float vbar)
{
  return mw * vbar * 1e24 / AVOGADRO;
  ;
}
...
US_Hydrodyn::calc_mw ()
{
  ...
  // info_model_vector_mw( QString( "before calc_mw() : model_vector" ),
  // model_vector, true );
  ...
  //    {
  //       editor_msg( "red",
  //                   QString( us_tr("error: could not open %1, %2 or %3, no
  //                   atomic excluded volume calc") ) .arg(
  //                   saxs_options.default_atom_filename ) .arg(
  //                   saxs_options.default_hybrid_filename ) .arg(
  //                   saxs_options.default_saxs_filename ) );
  //       do_excl_vol = false;
  //    }
  ...
  for (unsigned int i = 0; i < model_vector.size (); i++)
    {
      ...
      //                 .arg( QString("").sprintf("%.3f",
      //                 model_vector[i].vbar) ) );
      // {
      //    QFont courier = QFont( "Courier", USglobal->config_list.fontSize );
      //    editor_msg( "dark blue", courier, QString( us_tr("\nModel: %1\n")
      //    ).arg( model_vector[i].model_id ) ); editor_msg( "dark blue",
      //    courier, vbar_msg( model_vector[i].vbar ) ); editor_msg( "dark
      //    blue", courier, visc_dens_msg() );
      // }
      ...
      // info_model_vector_mw( QString( "before create_beads calc_mw() :
      // model_vector" ), model_vector, true );
      create_beads (&error_string, true);
      // info_model_vector_mw( QString( "after create_beads calc_mw() :
      // model_vector" ), model_vector, true ); info_model_residues( "before
      // calc_vbar in calc_mw()", model_vector[ i ] ); info_mw( QString( "after
      // create_beads in calc_mw() : model_vector[ %1 ]" ).arg( i ),
      // model_vector[ i ], true );
      calc_vbar (&model_vector[i], true);
      // info_model_residues( "after calc_vbar in calc_mw()", model_vector[ i ]
      // ); info_mw( QString( "after calc_vbar in calc_mw() : model_vector[ %1
      // ]" ).arg( i ), model_vector[ i ], true );
      ...
      if (!error_string.length ())
        {
          ...
          if (do_vvv)
            {
              vvv_file.setFileName (
                  somo_dir + "/" + "tmp" + "/" + project
                  + QString ("_%1.xyzr").arg (model_vector[i].model_id));
              if (!vvv_file.open (QIODevice::WriteOnly))
                {
                  editor_msg ("red",
                              QString (us_tr ("Error: VVV requested but can "
                                              "not open %1 for writing"))
                                  .arg (vvv_file.fileName ()));
                  do_vvv = false;
                }
              else
                {
          ...
          if (do_scol)
            {
              scol_file.setFileName (
                  somo_dir + "/" + "tmp" + "/" + project
                  + QString ("_%1.csv").arg (model_vector[i].model_id));
              if (!scol_file.open (QIODevice::WriteOnly))
                {
                  editor_msg (
                      "red",
                      QString (
                          us_tr ("Error: save as CSV on load PDB requested "
                                 "but can not open %1 for writing"))
                          .arg (scol_file.fileName ()));
                  do_scol = false;
                }
              else
                {
                  scol_ts.setDevice (&scol_file);
                  scol_ts << "\"Atom number\",\"Atom name\",\"Residue "
                             "number\",\"Residue name\",\"Radius\",\"Mass "
                             "[Da]\",\"x\",\"y\",\"z\"\n";
                }
            }
          ...
          for (unsigned int j = 0; j < model_vector[i].molecule.size (); j++)
            {
                      ...
                      if (this_atom->resName != "WAT")
                        {
                          ...
                          qDebug ()
                              << QString (
                                     ...
                                     .arg (
                                         this_atom->p_atom->hybrid.num_elect);
#endif
                      ...
                      if (do_excl_vol)
                        {
                          ...
                          double si = 0e0;
                          if (!saxs_util->set_excluded_volume (
                                  *this_atom, excl_vol, scaled_excl_vol,
                                  saxs_options, residue_atom_hybrid_map,
                                  this_e, this_e_noh, si))
                            {
                          ...
                          else
                            {
                              ...
                              this_atom->si = si;
                              model_vector_as_loaded[i].molecule[j].atom[k].si
                                  = si;
                              if (this_atom->resName != "WAT")
                                {
              ...
              //               .arg( mw_to_volume(
              //               model_vector[i].molecule[j].mw,
              //               model_vector[i].vbar ) ) .arg( do_excl_vol ?
              //                     QString(", atomic volume %1 A^3%2 average
              //                     electron density %3 A^-3") .arg(
              //                     chain_excl_vol ) .arg( chain_excl_vol !=
              //                     chain_scaled_excl_vol ?
              //                           QString(", scaled atomic volume %1
              //                           A^2") .arg( chain_scaled_excl_vol )
              //                           :
              //                           ""
              //                           )
          ...
          if (do_vvv)
            {
              ...
              vvv::compute_vol_surf (vvv_file.fileName ().toLatin1 ().data (),
                                     asa.vvv_probe_radius, asa.vvv_grid_dR,
                                     volume, surf);
              editor_msg ("black",
                          QString ("VVV: probe %1 (A) grid side %2 (A) volume "
                                   "%3 (A^3) surface area %4 (A^2)")
          ...
          if (do_scol)
            {
              scol_file.close ();
              editor_msg ("blue", QString ("Created CSV atomic file: %1")
                                      .arg (scol_file.fileName ()));
            }
        }
      ...
      model_vector[i].isoelectric_point
          = compute_isoelectric_point (model_vector[i]);

      {
        dammix_remember_mw_source
            [QString ("%1 Model %2").arg (project).arg (i + 1)]
            = "computed from pdb";
        dammix_remember_mw[QString ("%1 Model %2").arg (project).arg (i + 1)]
            = model_vector[i].mw + model_vector[i].ionized_mw_delta;
        // QTextStream(stdout) << QString("dammix remember mw set to
        // %1\n").arg( model_vector[i].mw + model_vector[ i ].ionized_mw_delta
        // ); QString qs =
        //    QString( us_tr( "\nModel: %1 Molecular weight %2 Daltons, Volume
        //    (from vbar) %3 A^3%4" ) ) .arg(model_vector[i].model_id)
        //    .arg(model_vector[i].mw )
        //    .arg( mw_to_volume( model_vector[i].mw, model_vector[i].vbar ) )
        //    .arg( do_excl_vol ?
        //          QString(", atomic volume %1 A^3%2 average electron density
        //          %3 A^-3") .arg( tot_excl_vol ) .arg( tot_excl_vol !=
        //          tot_scaled_excl_vol ?
...
#if defined(U_EXPT)
        {
          ...
          //    .arg( 100.0 * ( ( model_vector[i].mw / 1000e0 ) - pow( Rg
          //    / 6.5e0, 3e0 ) ) / ( model_vector[i].mw / 1000e0 ), 0, 'f', 1 )
          //    ;
        }
#endif
        // // qs += "\n" + vbar_msg( model_vector[i].vbar );
        ...
        editor_msg (
            "black", QFont ("Courier", USglobal->config_list.fontSize),
            model_summary_msg (
                QString ("Summary information for model %1:").arg (i + 1),
                &model_vector[i]));
        last_pdb_load_calc_mw_msg << model_summary_msg (
            QString ("Summary information for model %1:").arg (i + 1),
            &model_vector[i]);
      }
      ...
      if (do_excl_vol && misc.set_target_on_load_pdb)
        {
          ...
          editor_msg (
              "blue",
              us_tr ("Target excluded volume and electron density set"));
        }
      ...
      //       us_qdebug( QString( "model %1 molecule %2 mw %3" ).arg( i ).arg(
      //       j ).arg( model_vector[ i ].molecule[ j ].mw ) );
      //    }
      //    us_qdebug( QString( "" ).sprintf(
      //                                  "model %d total from molecules mw
      //                                  %.2f as model mw %.2f cm mw %.2f
      //                                  model_mw  %.2f", i, tmp_mw,
      //                                  model_vector[ i ].mw,
      //                                  total_cm_mw,
      //                                  model_mw
      //                                   ) );
      ...
      if (model_vector_as_loaded.size () > i)
        {
          ...
          model_vector_as_loaded[i].ionized_mw_delta
              = model_vector[i].ionized_mw_delta;
          model_vector_as_loaded[i].volume = model_vector[i].volume;
          model_vector_as_loaded[i].num_elect = model_vector[i].num_elect;
          model_vector_as_loaded[i].num_SS_bonds
              = model_vector[i].num_SS_bonds;
          model_vector_as_loaded[i].num_SH_free = model_vector[i].num_SH_free;
          model_vector_as_loaded[i].vbar = model_vector[i].vbar;
        }
      // info_model_residues( "after update_model_chain_ionization in
      // calc_mw()", model_vector[ i ] );
    }
  editor_msg ("black", "");
  current_model = save_current_model;
  // info_model_vector_mw( QString( "after calc_mw() : model_vector" ),
  // model_vector, true ); info_model_vector( QString( "after calc_mw() :
  // model_vector" ), model_vector ); info_mw( QString( "after calc_mw() :
  // model_vector" ), model_vector, true );
  // info_residue_protons_electrons_at_pH( le_pH->text().toDouble(),
  // model_vector[ 0 ] );
...
void
US_Hydrodyn::update_model_chain_ionization (struct PDB_model &model,
                                            bool quiet)
{
  if (!quiet)
    {
      QTextStream (stdout) << "==============================================="
                              "========================================="
                           << Qt::endl
                           << "US_Hydrodyn::update_model_chain_ionization()"
                           << Qt::endl
                           << "==============================================="
                              "========================================="
                           << Qt::endl;
      ...
      for (int k = 0; k < atoms; ++k)
        {
          ...
          // model.hydration        +=
          //    model.residue[ model.molecule[ j ].atom[ k ].model_residue_pos
          //    ].r_atom[ model.molecule[ j ].atom[ k ].atom_assignment
          //    ].hydration;
          model.molecule[j].ionized_mw_delta
              += model.molecule[j].atom[k].ionized_mw_delta;
          ...
          QTextStream (stdout)
              << "------------------------------------------------------------"
                 "----------------------------"
              << Qt::endl
              << " model.molecule[ " << j << " ].atom[ " << k
              << " ].name              = " << model.molecule[j].atom[k].name
              << Qt::endl
              // not useful << " model.molecule[ " << j << " ].atom[ " << k <<
              // " ].bead_number      = " << model.molecule[ j ].atom[ k
              // ].bead_number      << Qt::endl
              << " model.molecule[ " << j << " ].atom[ " << k
              << " ].model_residue_pos = "
              << model.molecule[j].atom[k].model_residue_pos << Qt::endl
              << " model.molecule[ " << j << " ].atom[ " << k
              << " ].bead_assignment   = "
              << model.molecule[j].atom[k].bead_assignment << Qt::endl
              << " model.molecule[ " << j << " ].atom[ " << k
              << " ].atom_assignment   = "
              << model.molecule[j].atom[k].atom_assignment << Qt::endl
              << " model.molecule[ " << j << " ].atom[ " << k
              << " ].mw                = " << model.molecule[j].atom[k].mw
              << Qt::endl
              << " model.molecule[ " << j << " ].atom[ " << k
              << " ].ionized_mw_delta  = "
              << model.molecule[j].atom[k].ionized_mw_delta << Qt::endl;

          if ((int)model.molecule[j].atom[k].p_residue->r_atom.size ()
              <= model.molecule[j].atom[k].atom_assignment)
            {
              QTextStream (stdout)
                  << "******** p_residue->r_atom too small for atom_assignment"
                  << Qt::endl;
              continue;
            }
          ...
          if ((int)model.molecule[j].atom[k].p_residue->r_bead.size ()
              <= model.molecule[j].atom[k].bead_assignment)
            {
              QTextStream (stdout)
                  << "******** p_residue->r_bead too small for bead_assignment"
                  << Qt::endl;
              continue;
            }
          ...
          QTextStream (stdout)
              ...
              << model.molecule[j]
                     .atom[k]
                     .p_residue
                     ->r_bead[model.molecule[j].atom[k].bead_assignment]
                     .mw
              << Qt::endl
              << " model.molecule[ " << j << " ].atom[ " << k
              << " ].p_residue->r_bead[ "
          ...
          QTextStream (stdout)
              << " model.molecule[ " << j << " ].residue[ "
              << model.molecule[j].atom[k].model_residue_pos << " ].r_atom[ "
              << model.molecule[j].atom[k].atom_assignment
              << " ].hybrid.mw               = "
              << model.residue[model.molecule[j].atom[k].model_residue_pos]
                     .r_atom[model.molecule[j].atom[k].atom_assignment]
...
void
US_Hydrodyn::reset_ionized_residue_vectors ()
{
  set_ionized_residue_vector (residue_list);
}

vector<double>
US_Hydrodyn::basic_fractions (float pH, struct residue *res)
{
  ...
  if (pKas == 2)
    {
      ...
      if (fabs (a_debug - a) > 1e-10)
        {
          qDebug () << QString ("US_Hydrodyn::basic_fractions(): Internal "
                                "error - frac a varies %1 %2 ")
                           .arg (a_debug)
                           .arg (a);
        }
      ...
      if (a < 0 || b < 0 || c < 0)
        {
          qDebug () << QString ("US_Hydrodyn::basic_fractions(): Internal "
                                "error - negative fraction != 1:  %1  %2  %3 ")
                           .arg (a)
                           .arg (b)
                           .arg (c);
        }
  ...
  QString msg = "US_Hydrodyn::basic_fractions(): Internal error - too many "
                "pKas - unsupported";
  qDebug () << msg;
  editor_msg ("red", msg);
  return fractions;
...
double
US_Hydrodyn::ionized_residue_vbar (vector<double> &fractions,
                                   struct residue *res)
{
  ...
  if (size == 3)
    {
      return fractions[0] * res->vbar + fractions[1] * res->vbars[0]
             + fractions[2] * res->vbars[1];
    }
  QString msg = "US_Hydrodyn::ionized_residue_vbar(): Internal error - too "
                "many fractions - unsupported";
  qDebug () << msg;
  editor_msg ("red", msg);
  return 0e0;
}
...
double
US_Hydrodyn::ionized_residue_atom_mw (vector<double> &fractions,
                                      struct residue *res, struct atom *atom)
{
  int size = (int)fractions.size ();
  ...
  if (!res->r_atom_0.count (atom->ionization_index)
    ...
    {
      QString msg = "US_Hydrodyn::ionized_residue_atom_mw(): Internal error - "
                    "r_atom maps missing";
      qDebug () << msg;
      editor_msg ("red", msg);
      return 0e0;
  ...
  if (atom->ionization_index > size)
    {
      QString msg = "US_Hydrodyn::ionized_residue_atom_mw(): Internal error - "
                    "atom's ionization index is out of bounds";
      qDebug () << msg;
      editor_msg ("red", msg);
      return 0e0;
    }
  ...
  if (size == 2)
    {
      return fractions[1]
             * (res->r_atom_1[atom->ionization_index].hybrid.mw
                - res->r_atom_0[atom->ionization_index].hybrid.mw);
    }

  return (fractions[atom->ionization_index]
...
double
US_Hydrodyn::ionized_residue_atom_radius (vector<double> &fractions,
                                          struct residue *res,
                                          struct atom *atom)
{
  int size = (int)fractions.size ();
  ...
  double r0_3 = (double)res->r_atom_0[atom->ionization_index].hybrid.radius
                * res->r_atom_0[atom->ionization_index].hybrid.radius
                * res->r_atom_0[atom->ionization_index].hybrid.radius;

  double r1_3 = (double)res->r_atom_1[atom->ionization_index].hybrid.radius
  ...
  if (size == 2)
    {
      ...
      QTextStream (stdout)
          ...
          << fractions[atom->ionization_index - 1] << ","
          << fractions[atom->ionization_index] << ","
          << pow (fractions[atom->ionization_index - 1] * r0_3
                      + fractions[atom->ionization_index] * r1_3,
                  1 / 3e0)
          << "," << Qt::endl;
#endif
      return pow (fractions[atom->ionization_index - 1] * r0_3
                      + fractions[atom->ionization_index] * r1_3,
                  1 / 3e0);
    }

#if defined(DEBUG_RADIUS)
  QTextStream (stdout) << res->name << "," << atom->name << ","
                       << res->r_atom_0[atom->ionization_index].hybrid.radius
                       << ","
                       << res->r_atom_1[atom->ionization_index].hybrid.radius
                       << "," << fractions[atom->ionization_index - 1] << ","
...
double
US_Hydrodyn::ionized_residue_atom_hydration (vector<double> &fractions,
                                             struct residue *res,
                                             struct atom *atom)
{
  int size = (int)fractions.size ();
  ...
  if (size == 2)
    {
      ...
      QTextStream (stdout)
          ...
          << "," << Qt::endl;
#endif
      return fractions[atom->ionization_index - 1]
                 * res->r_atom_0[atom->ionization_index].hydration
             + fractions[atom->ionization_index]
                   * res->r_atom_1[atom->ionization_index].hydration;
    }
...
double
US_Hydrodyn::ionized_residue_atom_protons (vector<double> &fractions,
                                           struct residue *res,
                                           struct atom *atom)
{
  int size = (int)fractions.size ();
...
void
US_Hydrodyn::set_ionized_residue_vector (vector<struct residue> &residue_v)
{
  // this will have to be changed later to true, as we are changing the format
  // of residue QTextStream( stdout )
  //    <<
  //    "========================================================================================"
  //    << Qt::endl
...
#if defined(DEBUG_RADIUS)
  QTextStream (stdout) << "pH" << "," << pH << Qt::endl
                       << "resname" << ","
                       << "atomname" << ","
                       << "r_0 radius" << ","
                       << "r_1 radius" << ","
  ...
  for (int j = 0; j < residues; ++j)
    {
      ...
      residue_v[j].pH = pH;
      residue_v[j].vbar_at_pH
          = ionized_residue_vbar (fractions, &residue_v[j]);
      residue_v[j].ionized_mw_delta = 0e0;

      for (int k = 0; k < atoms; ++k)
        {
          double ionized_mw_for_atom = ionized_residue_atom_mw (
              fractions, &residue_v[j], &residue_v[j].r_atom[k]);
          double ionized_hydration = ionized_residue_atom_hydration (
              fractions, &residue_v[j], &residue_v[j].r_atom[k]);
          residue_v[j].r_atom[k].hybrid.ionized_mw_delta = ionized_mw_for_atom;
          residue_v[j].r_atom[k].hybrid.radius = ionized_residue_atom_radius (
              fractions, &residue_v[j], &residue_v[j].r_atom[k]);
          residue_v[j].r_atom[k].hydration = ionized_hydration;
          residue_v[j].r_atom[k].hybrid.protons
              = ionized_residue_atom_protons (fractions, &residue_v[j],
                                              &residue_v[j].r_atom[k]);
          residue_v[j].ionized_mw_delta += ionized_mw_for_atom;
          bead_ionized_mw[residue_v[j].r_atom[k].bead_assignment]
              += ionized_mw_for_atom;
          bead_atom_hydration[residue_v[j].r_atom[k].bead_assignment]
              += ionized_hydration;
          if (residue_v[j].r_atom[k].ionization_index)
            {
              bead_has_atom_with_ionization_index.insert (
                  residue_v[j].r_atom[k].bead_assignment);
            }

          // vdwf
          {
            QString name = QString ("%1|%2")
                               .arg (residue_v[j].r_atom[k].name != "OXT"
                                         ? residue_v[j].name
                                         : "OXT")
      ...
      for (int k = 0; k < beads; ++k)
        {
          residue_v[j].r_bead[k].hydration_flag
              = bead_has_atom_with_ionization_index.count (k) == 0;
          ...
          if (!residue_v[j].r_bead[k].hydration_flag)
            {
              QTextStream (stdout)
                  << "residue name " << residue_v[j].name << " bead "
                  << (k + 1) << " atom_hydration "
                  << residue_v[j].r_bead[k].atom_hydration << Qt::endl;
            }
#endif
        }
      if (bead_ionized_mw.size ())
        {
          QTextStream (stdout)
              << "**** US_Hydrodyn::set_ionized_residue_vector_apply(): "
                 "unassigned bead_ionized_mw! ****"
              << Qt::endl;
        }
    }
  ...
  for (int j = 0; j < residues; ++j)
    {
      ...
      res_mw[residue_v[j].name]
          = residue_v[j].mw + residue_v[j].ionized_mw_delta;
    }
  ...
  //    // QTextStream( stdout ) << "******** begin res_vbar, mw map ********"
  //    << Qt::endl;
  ...
  //          QTextStream( stdout ) << it->first << " vbar " << it->second << "
  //          mw " << res_mw[ it->first ] << Qt::endl;
  //       }
  //    }
  //    // QTextStream( stdout ) << "******** end res_vbar, mw map ********" <<
  //    Qt::endl;
  // }
  ...
  //       QTextStream( stdout ) << it->first << " mw " << it->second.mw << "
  //       ionized_mw_delta " << it->second.ionized_mw_delta << Qt::endl;
  //    }
  // }
...
bool
US_Hydrodyn::model_summary_csv (const QString &filename)
{
...
bool
US_Hydrodyn::model_summary_csv (struct PDB_model *model,
                                const QString &filename)
{
  ...
  if (pdb_parse.find_sh)
    {
      header << "Number of disulfide bonds"
             << "Number of free SH";
      data << QString ("%1").arg (model->num_SS_bonds)
           << QString ("%1").arg (model->num_SH_free);
    }

  header << "Calculation done at pH"
         << "Molecular weight [Da]";
  data << QString ("%1").arg (le_pH->text ())
  ...
  //       QString(
  //               us_tr(
  //                     "Anh. Molecular vol. (from vbar)  : %1 [A^3] @
  //                     %2%3C\n"
  //                     )
  //               )
  //       .arg( mw_to_volume( model->mw + model->ionized_mw_delta, model->vbar
  //       ) ) .arg( 20 ) .arg( DEGREE_SYMBOL )
  //       ;
  // }

  header << "Anh. Molecular vol. (from vbar) [" << UNICODE_ANGSTROM << "^3]"
  ...
  data << QString ("%1").arg (mw_to_volume (
      model->mw + model->ionized_mw_delta, tc_vbar (model->vbar)))
       << QString ("%1").arg (
              mw_to_volume (model->mw + model->ionized_mw_delta,
                            tc_vbar (model->vbar))
              + model->hydration * misc.hydrovol)
  ...
  QString error;
  QString qs = "\"" + header.join ("\",\"") + "\"\n" + data.join (",") + "\n";

  return US_File_Util::putcontents (filename, qs, error);
}

void
US_Hydrodyn::select_atom_file (const QString &filename)
{
  QString str1;
