...
bool
US_FFD::set_saxs_coeffs (vector<saxs> &saxs_coeffs)
{
  errormsg = "";
...
bool
US_FFD::set_saxs_coeffs (const QString & /* filename */)
{
  errormsg = "US_FFD::set_saxs_coeff( const QString & filename ) :: not yet "
             "implemented";
  qDebug () << errormsg;
  return false;
}
...
bool
US_FFD::make_saxs_coeff_map ()
{
  if (coeff_terms != 4 && coeff_terms != 5)
    {
      errormsg
          = QString (
                "US_FFD::make_saxs_coeff_map() : %1 terms not supported\n")
                .arg (coeff_terms);
      qDebug () << errormsg;
      return false;
    }
  ...
  switch (coeff_terms)
    ...
    case 5:
      ...
      break;
    default:
      errormsg = QString ("US_FFD::make_saxs_coeff_map() : unexpected "
                          "internal error - %1 terms not supported\n")
                     .arg (coeff_terms);
      qDebug () << errormsg;
      return false;
    }
...
QString
US_FFD::list_saxs_coeffs (set<QString> filter)
{
  QStringList result;
  for (auto it = saxs_coeff_map.begin (); it != saxs_coeff_map.end (); ++it)
    {
      if (!filter.size () || filter.count (it->first))
        {
...
QString
US_FFD::list_fifj (set<QString> filter)
{
  QStringList result;
  for (auto it = fifj_map.begin (); it != fifj_map.end (); ++it)
    {
      if (!filter.size ()
...
bool
US_FFD::set_q_grid (const vector<double> &q)
{
...
bool
US_FFD::set_q_grid (const vector<float> &q)
{
  int q_count = (int)q.size ();
...
bool
US_FFD::set_q_grid (ftype qstart, ftype qend, ftype increment)
{
  q.clear ();
...
#warning.c is wrong here, needs eV term subtracted
  ...
  for (auto it = saxs_coeff_map.begin (); it != saxs_coeff_map.end (); ++it)
    {
      for (auto it2 = saxs_coeff_map.begin (); it2 != saxs_coeff_map.end ();
           ++it2)
        {
          ...
          for (int i = 0; i < coeff_terms; ++i)
            {
              for (int j = 0; j < coeff_terms; ++j)
                {
                  fifj_map[index].a.push_back (it->second.a[i]
                                               * it2->second.a[j]);
                  ...
                  if (used_b.count (fifj_map[index].b.back ()))
                    {
                      ++terms_removed;
                      fifj_map[index].a[used_b[fifj_map[index].b.back ()]]
                          += fifj_map[index].a.back ();
                      fifj_map[index].a.pop_back ();
                      fifj_map[index].b.pop_back ();
                    }
                  else
                    {
                      used_b[fifj_map[index].b.back ()]
                          = (int)fifj_map[index].a.size () - 1;
                    }
#endif
                }
          ...
          for (ftype q = 0; q <= 2; q += .1)
            {
              ...
              TSO << QString ("%1 %2 q %3 singly : %4 * %5 = %6  fifj : %7  "
                              "diff : %8\n")
                         .arg (x)
                         .arg (y)
                         .arg (q)
...
ftype
US_FFD::calc_I (const QString &name, ftype q)
{
#warning.c is wrong here, needs eV term subtracted
  if (saxs_coeff_map.count (name))
    {
      ...
      for (int i = 0; i < coeff_terms; ++i)
        {
          result += saxs_coeff_map[name].a[i]
                    * exp (mqq * saxs_coeff_map[name].b[i]);
        }
      return result;
    }
  return 0;
...
ftype
US_FFD::calc_I (const set<QString> &index, ftype q)
{
  if (fifj_map.count (index))
    {
...
void
US_FFD::test_resize_saxs_coeff (int size, ftype val)
{
  for (auto it = saxs_coeff_map.begin (); it != saxs_coeff_map.end (); ++it)
    {
      ...
      for (auto &qi : q)
        {
          ...
          for (int i = 0; i < terms; ++i)
            {
              fq_map[it->first][qi]
                  += it->second.a[i] * exp (mqq * it->second.b[i]);
            }
        }
    }
