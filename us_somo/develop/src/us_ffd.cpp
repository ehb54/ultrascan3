...
bool US_FFD::set_saxs_coeffs(const QString & /* filename */) {
  errormsg =
      "US_FFD::set_saxs_coeff( const QString & filename ) :: not yet "
      "implemented";
  qDebug() << errormsg;
  return false;
}
...
bool US_FFD::make_saxs_coeff_map() {
  if (coeff_terms != 4 && coeff_terms != 5) {
    errormsg =
        QString("US_FFD::make_saxs_coeff_map() : %1 terms not supported\n")
            .arg(coeff_terms);
    qDebug() << errormsg;
    return false;
  }
  ...
  switch (coeff_terms) {
    ...
    default:
      errormsg = QString(
                     "US_FFD::make_saxs_coeff_map() : unexpected internal "
                     "error - %1 terms not supported\n")
                     .arg(coeff_terms);
      qDebug() << errormsg;
      return false;
  }
  ...
  for (auto &x : names) {
    for (auto &y : names) {
      for (ftype q = 0; q <= 2; q += .1) {
        ...
        TSO << QString(
                   "%1 %2 q %3 singly : %4 * %5 = %6  fifj : %7  diff : %8\n")
                   .arg(x)
                   .arg(y)
                   .arg(q)
                   .arg(valx)
...
#warning.c is wrong here, needs eV term subtracted
  if (saxs_coeff_map.count(name)) {
    ...
    for (int i = 0; i < coeff_terms; ++i) {
      result +=
          saxs_coeff_map[name].a[i] * exp(mqq * saxs_coeff_map[name].b[i]);
    }
    return result;
  }
  return 0;
