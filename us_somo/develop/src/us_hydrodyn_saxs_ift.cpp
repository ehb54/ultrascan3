...
void US_Hydrodyn_Saxs_Ift::showhide(vector<QWidget *> &widgets,
                                    bool isChecked) {
...
void US_Hydrodyn_Saxs_Ift::setupGUI() {
  ...
  lbl_smearing->setToolTip(
      us_tr("<p>Optionally enter a correction for slit smearing. Default is no "
            "smearing.</p><p>Enter value for constant c as given by the "
            "expression</p><p>I_smear(q) = integrate P(t)*I(sqrt(q**2 + t**2)) "
            "dt with the</p><p>primary beam length profile: P(t) = c/sqrt(pi) "
            "* exp(-c**2*t**2).</p><p>The fit and the deconvolution are both "
            "given in the ouput file fit.d</p>"));
#endif
  ...
  lbl_prpoints->setToolTip(
      us_tr("<p>Optionally enter the number of points in the estimated "
            "function p(r): more points increase the cpu-time.</p><p>Default: "
            "50, Maximum 500</p>"));
#endif
  ...
  lbl_noextracalc->setToolTip(
      us_tr("<p>Optionally Input number of extra error calculations (max "
            "1000).</p><p>Entering a large number will improve the error "
            "estimate,</p><p>but require more cpu time.</p><p>In some cases it "
            "may be a little tricky to obtain a decent</p><p>error estimate. "
            "Try testing a couple of values to see the effect. </p>"));
#endif
  ...
  lbl_transform->setToolTip(us_tr(
      ...
      "distribution)        (3*sin(q*r)-q*r*cos(q*r))/(q*r)**3)**2 * dr </p>"));
#endif
...

  {
    QStringList qsl =
        QString(
            "Debye (default -> returning p(r) with positivity "
            "constraint)~D~Negative (Debye transformation -> returning p(r) "
            "without positivity constraint)~N~MaxEnt using an ellipsoid of "
            "revolution as prior ( -> p(r) -positivity constraint)~M~Bessel "
            "(for cylindrical scatterers -> cross section "
            "distribution)~B~Cosine (lamellae -> thickness "
            "distribution)~C~Size (using spheres only -> size distribution)~S")
            .split("~");
...
#if QT_VERSION > 0x040000
  AUTFBACK(lbl_eta);
  lbl_eta->setToolTip(us_tr(
      "<p>The exact value entered here may influence the result when the "
      "information content of the data is low.</p><p>Start with a small number "
      "e.g. 0.01 to avoid numerical instabilities and long cpu times.</p>"));
#endif
  ...
  le_estimateratio->setText(
      parameters->count("estimateratio") ? (*parameters)["estimateratio"] : "");
  le_estimateratio->setEnabled(true);
  le_estimateratio->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
