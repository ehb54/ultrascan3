...
void
US_Hydrodyn_Dammin_Opts::setupGUI ()
{
  int minHeight1 = 30;
  int minHeight2 = 30;
  ...
  cb_write_bead_model->setFont (QFont (USglobal->config_list.fontFamily,
                                       USglobal->config_list.fontSize));
  cb_write_bead_model->setPalette (PALET_NORMAL);
  AUTFBACK (cb_write_bead_model);
  connect (cb_write_bead_model, SIGNAL (clicked ()),
           SLOT (set_write_bead_model ()));
...
void
US_Hydrodyn_Dammin_Opts::cancel ()
{
  close ();
}

void
US_Hydrodyn_Dammin_Opts::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Dammin_Opts::update_psv (const QString &str)
{
  *psv = str.toDouble ();
}

void
US_Hydrodyn_Dammin_Opts::update_mw (const QString &str)
{
  *mw = str.toDouble ();
}

void
US_Hydrodyn_Dammin_Opts::update_partial (const QString &str)
{
  *partial = str;
}

void
US_Hydrodyn_Dammin_Opts::set_write_bead_model ()
{
  *write_bead_model = cb_write_bead_model->isChecked ();
}

void
US_Hydrodyn_Dammin_Opts::set_remember ()
{
  *remember = cb_remember->isChecked ();
}

void
US_Hydrodyn_Dammin_Opts::set_use_partial ()
{
