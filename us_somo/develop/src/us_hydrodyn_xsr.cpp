...
double
linearFit (double *X, double *Y, double *sigY, int n, double *mb)
{

  /* Performs a weighted LSQ fit Y = mX + b: sigY[i] is the estimated stdDev.
   * of the data point Y[i] n is the number of data points */
   ...
   * delta temporarily stores the difference between exp. and calc. values;
   * chi2 is a measure of the total descrepency */
  ...
  mb[1] = Bi[1][0] * A[0]
          + Bi[1][1] * A[1]; /* intercept (background correction) */
...
double
lagrangeInterpolation (double *xn, double *yn, double x, int n)
{ /* performs an n - 1 order interpolation */
...
int
getResolutionFunction (exp_t *data)
{ /* Calculate the resolution funtion */
  ...
  /* Sq1 defines the half length of the top of the trapezium describing the
   * silt profile and Sq2 is the half length of the bottom of the trapezium */

  for (i = 0; i < NPMOD; i++)
    q[i] = QMIN
           + QMAX * (double)i
                 / (double)(NPMOD - 1); /* set the q values at which the model
                                           will be evaluated */

  for (k = 0; k < data->nContrastPoints; k++)
        ...
        for (i = 0; i < NPRES; i++)
          {
            ...
            if (data[k].qSlit[i] <= Sq1)
              data[k].slit[i]
                  = 2.0
                    / (Sq1 + Sq2); /* ensures unit area under the trapezoid */
          ...
          for (i = 0; i < NPRES; i++)
            {
              ...
              data[k].qResFunction[j][i] = data[k].qbar[j]
                                           + ((double)i - (NPRES - 1.0) / 2.0)
                                                 / 2.0 * data[k].sigq[j];
              data[k].resFunction[j][i]
                  = data[k].sF[j]
                    * GAUSSIAN (data[k].qResFunction[j][i], data[k].qbar[j],
                                data[k].sigq[j]);
            }
    }
...
int
getScaleBackground (calc_t *model, exp_t *data)
{ /* Front-end to caclulate the scale and background corrections */
...
int
getContacts (grid_t *grid)
{ /* initilises the pointers for the closest contacts */
...
int
initialiseGrid (grid_t *grid, int symmOp)
{ /* initialises the grid */
  ...
  /* x and y temporarily store the x and y coordinates read in from the pdb
   * file */

  char filename[30], buffer[999];

  /* filename is the name of the pdb filename to be read in; buffer is a
   * storage buffer */
  ...
  for (i = 0; i < grid->nGridPoints; i++)
    {
      ...
      if (nFiles == 0)
        grid[i].value
            = (int)((grid->nComponents + 1) * (double)rand ()
                    / (double)RAND_MAX); /* 0(solv.), 1(1st comp.), etc */
      else
        grid[i].value = 0;

      grid[i].xy[0][0]
          = (double)(i % grid->nX) + 0.5; /* the centre of the atoms is in the
                                             centre of each grid point */
      grid[i].xy[0][1] = (double)(i / grid->nX) + 0.5;
    }

  if (nFiles != 0)
    for (i = 0; i < nFiles; i++)
      { /* read in pdb files if instructed to do so */
        ...
        for (j = 0; j < nResidues; j++)
          {
            ...
            if (buffer[21] == 'A')
              { /* read in the asymmetric grid only */

                sscanf (buffer + 28, "%lf%lf", &x, &y);
                grid[(int)((y / grid->gridDistance - 0.5)
                           + (grid->nY - ySize) / 2)
                         * grid->nX
                     + (int)((x / grid->gridDistance - 0.5)
                             + (grid->nX - xSize))]
                    .value
                    = i + 1;
              }
...
int
getIqcalc (pr_t **pr, exp_t *data, calc_t *model)
{ /* calculate the smeared model scattering profiles */
  ...
  for (i = 0; i < NPMOD; i++)
    { /* perform numerical integration Ic(q) = 2.0*PI*PI*integral(p(r)j0(qr))dr
       */
      ...
      for (k = 0; k < model->nContrastPoints; k++)
        {
          ...
          Iq[k][i]
              = TRAPEZOID (f[0] * prTotal[k][0], 0.0,
                           pr[0]->r[0]); /* the spacing for the first point is
                                            different, hence use trapezoid */

          for (j = 2; j < pr[0]->nPrPoints; j += 2)
            Iq[k][i] += SIMPSON (
                f[j] * prTotal[k][j], f[j - 1] * prTotal[k][j - 1],
                f[j - 2] * prTotal[k][j - 2], pr[0]->r[j] - pr[0]->r[j - 1]);
        }
    }
          ...
          for (j = 0; j < NPRES; j++)
            {
              ...
              f[j]
                  = data[k].slit[j] * interpolatedIq (q, Iq[k], qtemp) / qtemp;
              if (j != 0 && j % 2 == 0)
                model[k].Iq[i]
                    += SIMPSON (f[j], f[j - 1], f[j - 2],
          ...
          for (j = 0; j < NPRES; j++)
            {
              ...
              if (j != 0 && j % 2 == 0)
                model[k].Iq[i]
                    += SIMPSON (f[j], f[j - 1], f[j - 2],
                                data[k].qResFunction[i][j]
                                    - data[k].qResFunction[i][j - 1]);
            }

        else
          model[k].Iq[i]
              = interpolatedIq (q, Iq[k], model[k].q[i])
...
double
interpolatedIq (double *q, double *Iq, double qint)
{
  ...
  for (i = 0; i < NPMOD - 1; i++)
    {
      ...
      if (qint > q[i] && qint < q[i + 1] && i <= 1)
        ...
        return (
            lagrangeInterpolation (&q[NPMOD - 4], &Iq[NPMOD - 4], qint, 4));
...
double
RgPenalty (calc_t *model)
{ /* returns the RgPenalty */

  int i;
  double penalty = 0.0;

  for (i = 0; i < model->nContrastPoints; i++)
    if (model[i].Rg > model[i].RgMax)
      penalty += model[i].Rg - model[i].RgMax;
...
double
partialCompactness (grid_t *grid)
{ /* calculates the compactness due a point surrounded by other points */
  ...
  if (grid->value != 0)
    {
      ...
      for (i = 0; i < grid->nContacts; i++)
        if (grid->value == grid->contact[i]->value)
          Ne += 1.0;

      return (exp (-0.5 * Ne)
              - exp (-0.5 * 8.0)); /* 8.0 is the maximum number of contacts for
                                      a square grid */
    }
...
double
adjustCompactness (int newValue, grid_t *grid)
{ /* adjusts the compactness, without having to recalculate it */

  int i, oldValue;

  /* i is a loop counter; oldValue temporarily stores the previous grid value
   * at that point */
  ...
  compactness
      -= partialCompactness (grid); /* subtract the contribution the gridpoint
                                       makes to the total compactness */
  ...
  oldValue = grid->value; /* change the value to the new value and store the
                             old one */
  grid->value = newValue;

  compactness
      += partialCompactness (grid); /* add the contribution the new gridpoint
                                       makes to the total compactness */
...
int
getArea (grid_t *grid)
{ /* calculates the area of each component, and the total area in the grid */
...
int
adjustArea (int oldValue, int newValue, grid_t *grid)
{ /* adjusts the total area of each component, without recalculating the whole
     thing */
...
double
compactness (grid_t *grid)
{ /* returns the total compactness of the grid */
...
int
partialLooseness (grid_t *grid, int value)
{
...
int
updatePrGaussian (double *pr, double mean, int bin, double nSymmOps)
{ /* This function distributes the pr normally, with a stdDev of 1/3 of the
     gridDistance */
  ...
  cdf1 = 0.5
         * (1.0
            + erf (2.12132
                   * ((double)bin - mean))); /* 2.12132 = 3.0/sqrt(2.0) */
...
int
getPartialPr (pr_t **pr, grid_t *grid, grid_t *point)
{ /* calculates pr for a "point" surround by the rest of the grid, neglecting
     contributions from it symmetry related self */
  ...
  for (i = 0; i < grid->nGridPoints; i++)
    { /*compute the p(r) for position "point"*/
        ...
        for (j = 0; j < grid->nSymmOps && status == 0; j++)
          {
            ...
            if (r == 0.0)
              status = 1; /* we want to treat p(r) due to symm related elements
                             of "point" seperately, so disregard all these */

            else
              updatePrGaussian (pr[point->value][grid[i].value].pr, r, (int)r,
                                (double)grid->nSymmOps);
            /* distribute the contribution normally, with a stdDev of 1/3 of
             * the gridDistance */
          }
    }
  ...
  for (i = 0; i < grid->nGridPoints; i++)
    {
      ...
      getPartialPr (
          pr, grid,
          &grid[i]); /* get the pr due to a grid element surrounded by all
                        others except those related to it by symmetry */

      for (k = 1; k < grid->nSymmOps; k++)
        { /* treat the component neglected in getPartialPr (start at k = 0,
             because k = 0 is the asymmetric grid element */
...
int
zeroPr (pr_t **pr)
{ /* zero's the pr arrays in 2D  pr data structures */
...
int
getMoments (pr_t **pr, calc_t *model)
{ /* Calculates zeroth (I0) and second (Rg) moments of the pr functions */
  ...
  for (k = 0; k < model->nContrastPoints; k++)
    { /* Do integration */
      ...
      model[k].I0 = 2.0 * PI
                    * TRAPEZOID (
                        prTotal[k][0], 0.0,
                        pr[0]->r[0]); /* Spacing for the first point is
                                         different, hence use trapezoid rule */
      model[k].Rg
          = TRAPEZOID (prTotal[k][0] * X2 (pr[0]->r[0]), 0.0, pr[0]->r[0]);

      for (i = 2; i < pr[0]->nPrPoints; i++)
        { /* The remainder of the integration can be performed with simpson's
             rule */
       ...
       * (2<r^2>) above, Rg is actually sqrt(<r^2>/<r^0>), hence we must
       * normalise <r^2> and take the sqrt */
...
double
totalResidual (calc_t *model, grid_t *grid)
...
int
printIqModel (calc_t *model)
{ /* prints the Iq data to file */
  ...
  for (i = 0; i < model->nContrastPoints; i++)
    { /*print best I(q) profile at each cycle*/
...
int
printPrModel (pr_t **pr)
{ /* print pr for each contrast point to file */
...
int
printPDB (grid_t *grid, const char *filename)
{ /* print the grid to file in a format readable by pdb veiwers */
    ...
    for (j = 0; j < grid->nSymmOps; j++)
      {
        ...
        if (grid[i].value == 1)
          fprintf (
              ...
              j + 65, i, grid[i].xy[j][0] * grid->gridDistance,
...
int
printInformation (calc_t *model, grid_t *grid, int cycle)
{
  ...
  fprintf (
      ...
      grid->totalArea, grid->loosenessWeight, grid->looseness, RGPENALTYWEIGHT,
      grid->RgPenalty, partialResidual / model->nContrastPoints,
      grid->compactnessWeight * grid->compactness / (double)grid->totalArea,
...
xsr_main (void)
{
  ...
  /* i, j, k and l are counters; symmOp represents the symmetry of the
   * filament; nX and nY are the number of grid points in the asymmetric unit
   * in the X and Y directions; nComponents is the number of components with
   * differing contrast; nContrastPoints is the number of contrast points
   * measured; nDataPoints is the number of qValues measured; nPrPoints is the
   * maximum number of points in the pr profile to be processed; *bestgrid is
   * an integer array for storing the current best grid values; oldValue
   * temporarily stores the value of the point being altered in the annealing
   * process; nReconfigurations is a counter representing the number of
   * reconfigurations attempted in the annealing process; nSuccess is the
   * number of reconfigurations that cause a decrease in the residual;
   * nRandomChanges are the number of accepted reconfigurations that cause an
   * increase in the residual; is a flag that indicates whether a given
   * reconfiguation is accepted (>0) or rejected (=0); randomPoint is the
   * gridpoint randomly chosen for reconfiguration; randomValue is the randomly
   * chosen value that "randomPoint" is to be changed to; cycle is an
   * incremental counter that keeps track of the number of cycles performed.
   */
  ...
  /* model contains the calculated scattering profiles and related properties
   * of the model */
  ...
  for (i = 0; i < NPR;
    ...
    for (j = 0; j < pr[0]->nComponents; j++)
      for (k = 0; k < pr[0]->nComponents; k++)
        pr[k][j].r[i]
            = (double)i * gridDistance
              + 0.5 * gridDistance; /* See Pr-ChoiceOfR.xls for justification
                                       of this scheme */

  for (i = 0; i < nContrastPoints; i++)
    { /* Read information from each data file and store it */
      ...
      sscanf (buffer, "%s%lf%lf", &data[i].resType, &data[i].A,
              &data[i].L); /* doesn't matter if these numbers are not present
                              if resType = NONE or NIST */

      if (strncmp (data[i].resType, "NIST", 4) == 0)
        {

          DOUBLE (data[i].sigq, NPEXP, "data[i].sigq", "main",
                  j); /* allocate memory for the NIST resolution function
                         parameters */
          DOUBLE (data[i].qbar, NPEXP, "data[i].qbar", "main", j);
          DOUBLE (data[i].sF, NPEXP, "data[i].sF", "main", j);

          DOUBLE2D (data[i].resFunction, NPEXP, NPRES, "data[i].resFunction",
                    "main", j, k);
      ...
      Drho[0] = 0.0; /* Drho[0] is the contrast of the solvent and Drho[1] is
                        the contrast of component 1 etc ... */

      for (j = 0; j < pr[0]->nComponents; j++)
        for (k = 0; k < pr[0]->nComponents; k++)
          pr[j][k].Drho[i] = Drho[j] * Drho[k];

      /* the notation is a little inconsistent here, but: pr[j][k].Drho[i] in
       * the product of the contrast of component j and k. The reason for this
       * is not immediately apparent but is related to the fact that the pr is
       * broken up into contributions between diffent components (i.e.
       * pr[i][j].pr is the pr for distances between component i and j, and
       * pr[i][i] is the pr for distances between DAMs in component i. Breaking
       * the pr down in such a was allows means that pr's for all contrast
       * points can be calculated from this basic set of pr's and means that a
       * pr need not be evaluated for each contrast point, speeding things up
       * considerably. This is akin to composite scattering functions */
      ...
      for (j = 0; j < data[i].nDataPoints; j++)
        {
          ...
          fgets (buffer, 999, input);
          if (strncmp (data[i].resType, "NIST", 4) == 0)
            sscanf (buffer, "%lf%lf%lf%lf%lf%lf", &data[i].q[j],
                    &data[i].Iq[j], &data[i].sigIq[j], &data[i].sigq[j],
                    &data[i].qbar[j], &data[i].sF[j]);
          else
  ...
  getResolutionFunction (data); /* initialise the resolution function (only
  ...
  getScaleBackground (model, data); /* this function also determines chi2
                                       values for each contrast point */
  ...
  do
    {
      ...
      if (currentResidual > bestResidual)
      ...
      else
        {
          ...
          /* If the reconfigured model is better than the old model, update the
           * old model */
          for (i = 0; i < grid->nGridPoints; i++)
          ...
          grid->RgPenalty = RgPenalty (model);
          getIqcalc (pr, data,
                     model); /* If the last random change is not accepted, old
                                chi2 values are stored */
          getScaleBackground (model, data);
          bestResidual = currentResidual;
        }

      printInformation (model, grid,
                        cycle); /* Print general info, pdb file, intensity data
                                   and the pr's to file */
      ...
      fprintf (stderr, ")/%d + %.2lf + %.2lf + %.2lf = %.2lf\n",
               ...
               grid->compactnessWeight * grid->compactness
                   / (double)grid->totalArea,
               grid->loosenessWeight * grid->looseness,
               RGPENALTYWEIGHT * grid->RgPenalty, totalResidual (model, grid));
      ...
      for (nReconfigurations = 0;
           ...
           && nSuccess + nRandomChanges
                  < MAXRECONFACTOR / 10 * grid->nGridPoints;
           nReconfigurations++)
        {

          randomPoint
              = (int)(grid->nGridPoints * (double)rand () / (double)RAND_MAX);
          randomValue = (int)((grid->nComponents + 1) * (double)rand ()
                              / (double)RAND_MAX);

          if (randomValue != grid[randomPoint].value)
            { /* if there has been no change, go back and try again! */
              ...
              if (randomPoint % grid->nX == grid->nX - 1)
                {
                  ...
                  /* adjusting the compactness for atoms that share a boundry
                   * with a symmetry related grid is tricky: just recalculate
                   * the whole thing in such cases */
                ...
                for (j = 0; j < pr[0]->nPrPoints; j++)
                  {
                    ...
                    /* getPartialPr calculates the pr due to a single atom
                     * surrounded by others, neglecting any distances between
                     * itself and its symmetry related self. If we change an
                     * atom from x to y, then we must subtract: pr[x][1,2,3,4]
                     * from the current pr, and add pr[y][1,2,3,4] */
              ...
              for (i = 1; i < grid->nSymmOps; i++)
                { /* treat the component neglected in getPartialPr */

                  r = sqrt (X2 (grid[randomPoint].xy[i][0]
                                - grid[randomPoint].xy[0][0])
                            + X2 (grid[randomPoint].xy[i][1]
                                  - grid[randomPoint].xy[0][1]));

                  updatePrGaussian (
                      pr[grid[randomPoint].value][grid[randomPoint].value].pr,
                      r, (int)r, -(double)grid->nSymmOps);
               ...
               * because the temperature was high enough to allow the residual
               * to go higher (accepted == 2) */

              else
                accepted = 0; /* model is rejected */

              if (accepted > 0)
                { /* update the value of the point and various residuals and
                     progress parameters */
                ...
                { /* Undo any changes we made to the model */
                  ...
                  for (i = 1; i < grid->nSymmOps; i++)
                    { /* treat the component neglected in getPartialPr */
                      ...
                      updatePrGaussian (
                          pr[grid[randomPoint].value][grid[randomPoint].value]
                              .pr,
                          r, (int)r, (double)grid->nSymmOps);
                      updatePrGaussian (pr[randomValue][randomValue].pr, r,
          ...
          if (nReconfigurations % 100
            ...
            fprintf (stderr,
                     "nSuccess = %d, nRandomChanges = %d, nTotalChanges = "
                     "%d(%d), NRecon = %d(%d), currentResidual = %.2lf\r",
      ...
      fprintf (stderr,
               ...
               currentResidual); /* write the information for the run completed
                                    at that temperature */

      temperature
          *= COOLINGFACTOR; /* decrease the temperature and start again */
    }
...
US_Hydrodyn_Xsr::setupGUI ()
{
  ...
  lbl_title
      = new QLabel (us_tr ("US-SOMO: SAXS Cross Sectional Analysis"), this);
...
void
US_Hydrodyn_Xsr::cancel ()
{
  close ();
}

void
US_Hydrodyn_Xsr::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Xsr::closeEvent (QCloseEvent *e)
{
...
void
US_Hydrodyn_Xsr::start ()
{
}

void
US_Hydrodyn_Xsr::stop ()
{
  running = false;
...
void
US_Hydrodyn_Xsr::update_enables ()
{
...
void
US_Hydrodyn_Xsr::editor_msg (QString color, QString msg)
{
