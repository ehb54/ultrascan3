...
US_Hydrodyn_Dad_Svd::US_Hydrodyn_Dad_Svd (US_Hydrodyn_Dad *dad_win,
                                          vector<QString> dad_selected_files,
                                          QWidget *p)
    : QFrame (p)
{
  this->dad_win = dad_win;
  this->dad_selected_files = dad_selected_files;
  this->ush_win = (US_Hydrodyn *)(dad_win->us_hydrodyn);
  this->plot_colors = dad_win->plot_colors;
  this->use_line_width = dad_win->use_line_width;
  ...
  if (dad_win->f_is_time[this->dad_selected_files[0]])
    {
      QStringList qsl_dad_selected_files;
      for (int i = 0; i < (int)this->dad_selected_files.size (); ++i)
        {
          qsl_dad_selected_files << this->dad_selected_files[i];
          ;
        }

      QString error_msg;
      if (!convert_it_to_iq (qsl_dad_selected_files, original_data, error_msg))
        {
          QMessageBox::warning (
              this, windowTitle (),
      ...
      this->dad_selected_files.clear ();
      for (int i = 0; i < (int)original_data.size (); ++i)
        {
          this->dad_selected_files.push_back (original_data[i]);
        }
    }
  else
    {
      for (int i = 0; i < (int)this->dad_selected_files.size (); ++i)
        {
          QString this_name = this->dad_selected_files[i];
          original_data.push_back (this_name);

          f_pos[this_name] = f_pos.size ();
          f_qs_string[this_name] = dad_win->f_qs_string[this_name];
          f_qs[this_name] = dad_win->f_qs[this_name];
          f_Is[this_name] = dad_win->f_Is[this_name];
          f_errors[this_name] = dad_win->f_errors[this_name];
...
US_Hydrodyn_Dad_Svd::~US_Hydrodyn_Dad_Svd () {}

void
US_Hydrodyn_Dad_Svd::setupGUI ()
{
  int minHeight1 = 24;
  int minHeight3 = 25;
  ...
  for (int i = 0; i < (int)dad_selected_files.size (); ++i)
    {
      lvi = new QTreeWidgetItem (iq, lvi);
      lvi->setText (0, dad_selected_files[i]);
    }
  ...
  pb_to_dad = new QPushButton (us_tr ("To HPLC window"), this);
  pb_to_dad->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize - 1));
  pb_to_dad->setMinimumHeight (minHeight3);
  pb_to_dad->setPalette (PALET_PUSHB);
  connect (pb_to_dad, SIGNAL (clicked ()), SLOT (to_dad ()));
  data_widgets.push_back (pb_to_dad);
  ...
  pb_efa_decomp_to_dad_saxs = new QPushButton (us_tr ("To HPLC window"), this);
  pb_efa_decomp_to_dad_saxs->setFont (QFont (
      USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1));
  pb_efa_decomp_to_dad_saxs->setMinimumHeight (minHeight1);
  pb_efa_decomp_to_dad_saxs->setPalette (PALET_PUSHB);
  connect (pb_efa_decomp_to_dad_saxs, SIGNAL (clicked ()),
           SLOT (efa_decomp_to_dad_saxs ()));
  efa_decomp_widgets.push_back (pb_efa_decomp_to_dad_saxs);
  ...
  // ----- left side
  {
    ...
    }
    {
      ...
      bl_buttons->setContentsMargins (0, 0, 0, 0);
      bl_buttons->setSpacing (0);
      bl_buttons->addWidget (pb_to_dad);
  ...
  // ----- right side
  {
    ...
    bl->addWidget (plot_efa_decomp);
    {
      ...
      bl_buttons->addWidget (pb_efa_decomp_to_dad_saxs);
      bl->addLayout (bl_buttons);
    }
  ...
  hide_widgets (data_widgets,
                !ush_win->gparams.count ("dad_svd_data_widgets")
                        || ush_win->gparams["dad_svd_data_widgets"] == "false"
                    ? false
                    : true);
  hide_widgets (editor_widgets,
                !ush_win->gparams.count ("dad_svd_editor_widgets")
                        || ush_win->gparams["dad_svd_editor_widgets"]
                               == "false"
                    ? false
                    : true);
  hide_widgets (process_widgets,
                !ush_win->gparams.count ("dad_svd_process_widgets")
                        || ush_win->gparams["dad_svd_process_widgets"]
                               == "false"
                    ? false
                    : true);
...
void
US_Hydrodyn_Dad_Svd::cancel ()
{
  close ();
}

void
US_Hydrodyn_Dad_Svd::help ()
{
  US_Help *online_help;
  online_help = new US_Help (this);
  online_help->show_help ("manual/somo/somo_dad_svd.html");
}

void
US_Hydrodyn_Dad_Svd::closeEvent (QCloseEvent *e)
{
  e->accept ();
}

void
US_Hydrodyn_Dad_Svd::clear_display ()
{
  editor->clear ();
  editor->append ("\n\n");
}
...
void
US_Hydrodyn_Dad_Svd::update_font ()
{
  bool ok;
  QFont newFont;
...
void
US_Hydrodyn_Dad_Svd::editor_msg (QString color, QString msg)
{
...
void
US_Hydrodyn_Dad_Svd::plot_files ()
{
  // puts( "plot_files" );
...
void
US_Hydrodyn_Dad_Svd::plot_files (QStringList add_files, norms norm_mode)
{
  // puts( "plot_files" );
  plot_data->detachItems (QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems (QwtPlotItem::Rtti_PlotMarker);
...
bool
US_Hydrodyn_Dad_Svd::plot_file (QString file, double &minx, double &maxx,
...
bool
US_Hydrodyn_Dad_Svd::get_min_max (QString file, double &minx, double &maxx,
                                  double &miny, double &maxy, norms norm_mode)
{
  map<QString, vector<double> > *f_Iuse = &f_Is;
...
// void US_Hydrodyn_Dad_Svd::plot_data_zoomed( const QRectF & /* rect */ )
// {
...
// void US_Hydrodyn_Dad_Svd::plot_errors_zoomed( const QRectF & /* rect */ )
// {
//    //   cout << QString( "zoomed: %1 %2 %3 %4\n" )
//    // .arg( rect.x1() )
//    // .arg( rect.x2() )
...
void
US_Hydrodyn_Dad_Svd::disable_all ()
{
  lv_data->setEnabled (false);
  pb_clear->setEnabled (false);
  pb_to_dad->setEnabled (false);
...
void
US_Hydrodyn_Dad_Svd::q_start_text (const QString &)
{
}

void
US_Hydrodyn_Dad_Svd::q_end_text (const QString &)
{
}

// void US_Hydrodyn_Dad_Svd::t_start_text( const QString & )
// {
// }

// void US_Hydrodyn_Dad_Svd::t_end_text( const QString & )
// {
// }

void
US_Hydrodyn_Dad_Svd::replot (bool keep_mode)
{
  disable_all ();
...
US_Hydrodyn_Dad_Svd::to_dad ()
{
  ...
  for (int i = 0; i < (int)files.size (); ++i)
    {
      dad_win->add_plot (files[i], f_qs[files[i]], f_Is[files[i]],
...
void
US_Hydrodyn_Dad_Svd::hide_process ()
{
...
void
US_Hydrodyn_Dad_Svd::hide_widgets (vector<QWidget *> widgets, bool hide)
{
...
void
US_Hydrodyn_Dad_Svd::update_enables ()
{
  // count selections
  if (running)
    {
  ...
  pb_to_dad->setEnabled (files.size ()
...
void
US_Hydrodyn_Dad_Svd::data_selection_changed ()
{
  update_enables ();
}

void
US_Hydrodyn_Dad_Svd::sv_selection_changed ()
{
  update_enables ();
}

void
US_Hydrodyn_Dad_Svd::clear ()
{
...
QStringList
US_Hydrodyn_Dad_Svd::selected_files ()
{
  QStringList result;
...
set<QString>
US_Hydrodyn_Dad_Svd::get_current_files ()
{
  set<QString> result;
...
set<QString>
US_Hydrodyn_Dad_Svd::get_sources ()
{
  set<QString> result;
...
set<QString>
US_Hydrodyn_Dad_Svd::get_selected_sources ()
{
  set<QString> result;
  QString iq_or_it = iq_it_state ? "I(t)" : "I(q)";
...
QTreeWidgetItem *
US_Hydrodyn_Dad_Svd::get_source_item (QString source)
{
  QTreeWidgetItemIterator it (lv_data);
  while ((*it))
    {
...
int
US_Hydrodyn_Dad_Svd::selected_sources ()
{
  QString iq_or_it = iq_it_state ? "I(t)" : "I(q)";
...
void
US_Hydrodyn_Dad_Svd::clean_selected ()
{
...
bool
US_Hydrodyn_Dad_Svd::all_children_selected (QTreeWidgetItem *lvi)
{
...
bool
US_Hydrodyn_Dad_Svd::is_selected (QTreeWidgetItem *lvi)
{
  if (lvi->isSelected ())
    {
      return true;
...
void
US_Hydrodyn_Dad_Svd::axis_x_title ()
{
...
void
US_Hydrodyn_Dad_Svd::axis_y_title ()
{
...
bool
US_Hydrodyn_Dad_Svd::plotted_matches_selected ()
{
  set<QString> plotted;
...
void
US_Hydrodyn_Dad_Svd::add_i_of_q_or_t (QString source, QStringList files,
                                      bool do_update_enables)
{
  ...
  // find common q
  QString head = dad_win->qstring_common_head (files, true);
  QString tail = dad_win->qstring_common_tail (files, true);
...
void
US_Hydrodyn_Dad_Svd::rescale (bool do_update_enables)
{
  // bool any_selected = false;
  double minx = 0e0;
  double maxx = 1e0;
...
void
US_Hydrodyn_Dad_Svd::svd_plot (bool axis_change)
{
...
#define DEBUG_MN
#if defined(DEBUG_MN)
  qDebug () << "US_Hydrodyn_Dad_Svd::svd rows " << m << " cols " << n;
...
QTreeWidgetItem *
US_Hydrodyn_Dad_Svd::lvi_last_depth (int d)
{
...
void
US_Hydrodyn_Dad_Svd::color_rotate ()
{
...
void
US_Hydrodyn_Dad_Svd::inc_rmsd_plot (bool axis_change)
{
  plot_data->detachItems (QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
...
void
US_Hydrodyn_Dad_Svd::inc_chi_plot (bool axis_change)
{
  plot_data->detachItems (QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
...
double
US_Hydrodyn_Dad_Svd::vmin (vector<double> &x)
{
  double min = x[0];
...
double
US_Hydrodyn_Dad_Svd::vmax (vector<double> &x)
{
  if (!x.size ())
    {
...
void
US_Hydrodyn_Dad_Svd::indiv_recon ()
{
  recon_mode = "Indiv. ";
...
void
US_Hydrodyn_Dad_Svd::set_plot_errors ()
{
...
void
US_Hydrodyn_Dad_Svd::set_plot_errors_rev ()
{
  update_plot_errors ();
}

void
US_Hydrodyn_Dad_Svd::set_plot_errors_sd ()
{
  if (plot_errors_zoomer)
    {
      plot_errors_zoomer->zoom (0);
...
void
US_Hydrodyn_Dad_Svd::set_plot_errors_pct ()
{
  if (plot_errors_zoomer)
    {
      plot_errors_zoomer->zoom (0);
...
void
US_Hydrodyn_Dad_Svd::set_plot_errors_group ()
{
  if (plot_errors_zoomer)
    {
      plot_errors_zoomer->zoom (0);
...
void
US_Hydrodyn_Dad_Svd::set_plot_errors_ref ()
{
...
void
US_Hydrodyn_Dad_Svd::update_plot_errors (bool do_update_enables)
{
...
QStringList
US_Hydrodyn_Dad_Svd::add_subset_data (QStringList files)
{
  QStringList result;
...
QString
US_Hydrodyn_Dad_Svd::get_related_source_name (QString name)
{
...
QStringList
US_Hydrodyn_Dad_Svd::get_files_by_name (QString name)
{
...
bool
US_Hydrodyn_Dad_Svd::get_name_norm_mode (QString fullname, QString &name,
                                         norms &norm_mode)
{
...
bool
US_Hydrodyn_Dad_Svd::get_plot_files (QStringList &use_list,
...
void
US_Hydrodyn_Dad_Svd::do_plot_errors ()
{
...
void
US_Hydrodyn_Dad_Svd::plot_errors_jump_markers ()
{
...
void
US_Hydrodyn_Dad_Svd::do_plot_errors_group ()
{
...
bool
US_Hydrodyn_Dad_Svd::setup_save (QString tag, QString &fname)
{
...
void
US_Hydrodyn_Dad_Svd::usp_config_plot_data (const QPoint &)
{
...
void
US_Hydrodyn_Dad_Svd::usp_config_plot_svd (const QPoint &)
{
...
void
US_Hydrodyn_Dad_Svd::usp_config_plot_errors (const QPoint &)
{
...
void
US_Hydrodyn_Dad_Svd::usp_config_plot_lefa (const QPoint &)
{
...
void
US_Hydrodyn_Dad_Svd::usp_config_plot_refa (const QPoint &)
{
...
void
US_Hydrodyn_Dad_Svd::usp_config_plot_ac (const QPoint &)
{
...
void
US_Hydrodyn_Dad_Svd::usp_config_plot_efa_decomp (const QPoint &)
{
...
void
US_Hydrodyn_Dad_Svd::set_norm_pw ()
{
  if (cb_norm_avg->isChecked ())
    {
      cb_norm_avg->setChecked (false);
...
void
US_Hydrodyn_Dad_Svd::set_norm_avg ()
{
  if (cb_norm_pw->isChecked ())
    {
      cb_norm_pw->setChecked (false);
...
void
US_Hydrodyn_Dad_Svd::clear_efa_decomp ()
{
...
void
US_Hydrodyn_Dad_Svd::make_plotted_efas ()
{
...
void
US_Hydrodyn_Dad_Svd::update_efas (double val)
{
  // qDebug() << "US_Hydrodyn_Dad_Svd::update_efas( " << val << " )";
...
void
US_Hydrodyn_Dad_Svd::setup_norm ()
{
  unsigned int q_points = (unsigned int)f_qs[dad_selected_files[0]].size ();

  vector<double> avg_errors (q_points);

  for (int i = 0; i < (int)dad_selected_files.size (); ++i)
    {
      QString this_name = dad_selected_files[i];
  ...
  double recip_fc = 1e0 / (double)dad_selected_files.size ();
  ...
  US_Vector::printvector3 (
      "file 0 I, errors, I_norm_pw", f_Is[dad_selected_files[0]],
      f_errors[dad_selected_files[0]], f_Is_norm_pw[dad_selected_files[0]]);

  US_Vector::printvector3 ("file 0 I, avg_errors, I_norm_avg",
                           f_Is[dad_selected_files[0]], avg_errors,
                           f_Is_norm_avg[dad_selected_files[0]]);
...
void
US_Hydrodyn_Dad_Svd::check_norm (QStringList &files)
{
...
double
US_Hydrodyn_Dad_Svd::autocor1 (vector<double> &x)
{
  int len = (int)x.size ();

#if defined(DEBUG_AUTOCOR_DETAIL)
  US_Vector::printvector ("US_Hydrodyn_Dad_Svd::autocor1 x", x);
#endif
...
vector<double>
US_Hydrodyn_Dad_Svd::autocor (vector<vector<double> > &A)
{
  ...
  {
    QTextStream ts (stdout);
    ts << "US_Hydrodyn_Dad_Svd::autocor A" << Qt::endl;
...
vector<vector<int> >
US_Hydrodyn_Dad_Svd::transpose (vector<vector<int> > &A)
{

  //   qDebug() << "US_Hydrodyn_Dad_Svd::transpose( vvi ) start";
  ...
  //   qDebug() << "US_Hydrodyn_Dad_Svd::transpose( vvi ) rows " << rows << "
  //   cols " << cols;
  ...
  //   qDebug() << "US_Hydrodyn_Dad_Svd::transpose( vvi ) return";
  return result;
}

vector<vector<double> >
US_Hydrodyn_Dad_Svd::transpose (vector<vector<double> > &A)
{

  //   qDebug() << "US_Hydrodyn_Dad_Svd::transpose( vvd ) start";
  ...
  //   qDebug() << "US_Hydrodyn_Dad_Svd::transpose( vvd ) rows " << rows << "
  //   cols " << cols;
  ...
  //   qDebug() << "US_Hydrodyn_Dad_Svd::transpose( vvd ) return";

  return result;
}

double
US_Hydrodyn_Dad_Svd::vvd_min (vector<vector<double> > &A)
{
  double result = -1e99;
  int rows = (int)A.size ();
  if (!rows)
...
double
US_Hydrodyn_Dad_Svd::vvd_max (vector<vector<double> > &A)
{
  double result = 1e99;
  int rows = (int)A.size ();
  if (!rows)
...
void
US_Hydrodyn_Dad_Svd::vvd_smult (vector<vector<double> > &A, double x)
{
  int rows = (int)A.size ();
  if (!rows)
    {
...
vector<vector<double> >
US_Hydrodyn_Dad_Svd::vivd_pwmult (vector<vector<int> > &A,
...
void
US_Hydrodyn_Dad_Svd::vvd_cnorm (vector<vector<double> > &A)
{
  int rows = (int)A.size ();
  if (!rows)
    {
...
vector<vector<double> >
US_Hydrodyn_Dad_Svd::dot (vector<vector<double> > &A,
                          vector<vector<double> > &B)
{
  vector<vector<double> > result;
  int arows = (int)A.size ();
...
void
US_Hydrodyn_Dad_Svd::matrix_info (QString qs, vector<vector<double> > &A)
{
  if (!A.size ())
    {
      qDebug () << "US_Hydrodyn_Dad_Svd::matrix_info() " << qs << " empty!";
      return;
    }
  qDebug () << "US_Hydrodyn_Dad_Svd::matrix_info() " << qs << " rows "
            << A.size () << " cols " << A[0].size ();
}

void
US_Hydrodyn_Dad_Svd::set_title (QwtPlot *plot, QString title)
{
...
void
US_Hydrodyn_Dad_Svd::save_plots ()
{
  QString use_dir = QDir::currentPath ();
  ush_win->select_from_directory_history (use_dir, this);
...
void
US_Hydrodyn_Dad_Svd::mode_select ()
{
  // first hide all
...
void
US_Hydrodyn_Dad_Svd::mode_select (modes mode)
{
  current_mode = mode;
  mode_select ();
  update_enables ();
...
void
US_Hydrodyn_Dad_Svd::set_mode_iqit ()
{
  mode_select (MODE_IQIT);
}

void
US_Hydrodyn_Dad_Svd::set_mode_svd ()
{
  mode_select (MODE_SVD);
}

void
US_Hydrodyn_Dad_Svd::set_mode_efa ()
{
  mode_select (MODE_EFA);
}

void
US_Hydrodyn_Dad_Svd::set_mode_efa_decomp ()
{
  mode_select (MODE_EFA_DECOMP);
}

void
US_Hydrodyn_Dad_Svd::update_efa_range_start (double)
{
  // qDebug() << "US_Hydrodyn_Dad_Svd::update_efa_range_start";
  // {
  //    QTextStream ts( stdout );
  //    for ( int i = 0; i < UHSHS_MAX_SV_FOR_EFA; ++i ) {
  //       ts << "US_Hydrodyn_Dad_Svd::update_efa_range_start SV " << i << "
...
void
US_Hydrodyn_Dad_Svd::update_efa_range_end (double)
{
  // qDebug() << "US_Hydrodyn_Dad_Svd::update_efa_range_end()";
  if (efa_range_processing)
    {
      return;
    }
...
void
US_Hydrodyn_Dad_Svd::update_efas_ranges ()
{
  // qDebug() << "US_Hydrodyn_Dad_Svd::update_efas_ranges()";
...
void
US_Hydrodyn_Dad_Svd::efa_info (QString tag)
{
  QTextStream ts (stdout);
...
vector<double>
US_Hydrodyn_Dad_Svd::gradient (vector<double> &y)
{
  int len = (int)y.size ();
  int lenm1 = len - 1;
...
void
US_Hydrodyn_Dad_Svd::init_efa_values ()
{
...
void
US_Hydrodyn_Dad_Svd::set_number_of_svs_for_efa ()
{
  int max_range
  ...
  // US_Vector::printvector2( "US_Hydrodyn_Dad_Svd::set_number_of_svs_for_efa
...
bool
US_Hydrodyn_Dad_Svd::convert_it_to_iq (QStringList files,
      ...
      if (!dad_win->f_qs.count (files[i]))
      ...
      else
        {
          for (unsigned int j = 0;
               j < (unsigned int)dad_win->f_qs[files[i]].size (); j++)
            {
              if (!mode_testiq
                  || (dad_win->f_qs[files[i]][j] >= t_min
                      && dad_win->f_qs[files[i]][j] <= t_max))
                {
                  I_values[dad_win->f_qs[files[i]][j]][ql.back ()]
                      = dad_win->f_Is[files[i]][j];
                  if (use_errors
                      && dad_win->f_errors[files[i]].size ()
                             == dad_win->f_qs[files[i]].size ())
                    {
                      e_values[dad_win->f_qs[files[i]][j]][ql.back ()]
                          = dad_win->f_errors[files[i]][j];
                    }
                  else
                    {
                      if (use_errors)
                  ...
                  if (!used_t.count (dad_win->f_qs[files[i]][j]))
                    {
                      tl.push_back (dad_win->f_qs[files[i]][j]);
                      used_t[dad_win->f_qs[files[i]][j]] = true;
                    }
                }
            }

          if (!dad_win->f_errors.count (files[i])
              || dad_win->f_errors[files[i]].size ()
                     != dad_win->f_Is[files[i]].size ())
            {
            ...
            {
              if (!dad_win->is_nonzero_vector (dad_win->f_errors[files[i]]))
                {
                  unsigned int zero_pts = 0;
                  for (unsigned int j = 0;
                       j < (unsigned int)dad_win->f_errors[files[i]].size ();
                       j++)
                    {
                      if (!mode_testiq
                          || (dad_win->f_qs[files[i]][j] >= t_min
                              && dad_win->f_qs[files[i]][j] <= t_max))
                        {
                          if (us_isnan (dad_win->f_errors[files[i]][j])
                              || dad_win->f_errors[files[i]][j] == 0e0)
                            {
                              zero_pts++;
                            }
                        }
                  ...
                  zero_points[files[i]]
                      = QString ("%1: %2 of %3 points")
                            .arg (files[i])
                            .arg (zero_pts)
                            .arg (dad_win->f_errors[files[i]].size ());
  ...
  for (unsigned int t = 0; t < tv.size (); t++)
    {
      ...
      QString name = head
                     + QString ("%1%2")
                           ...
                           .arg (dad_win->pad_zeros (tv[t], (int)tv.size ()))
                           .replace (".", "_");
...
void
US_Hydrodyn_Dad_Svd::runExplicitEFARotation (
...
range(C.shape[0])])
  ...
  */
  ...
  // qDebug() << "US_Hydrodyn_Dad_Svd::runExplicitEFARotation\n";
...
void
US_Hydrodyn_Dad_Svd::efa_decomp ()
{
...
void
US_Hydrodyn_Dad_Svd::efa_decomp_plot ()
{
...
void
US_Hydrodyn_Dad_Svd::initExplicitEFA (
...
def initExplicitEFA(M, num_sv, D, C, converged, V_bar):
  ...
  */
  // qDebug() << "US_Hydrodyn_Dad_Svd::initExplicitEFA\n";
  T_ret.resize (num_sv);
  for (int i = 0; i < num_sv; ++i)
    {
...
void
US_Hydrodyn_Dad_Svd::initIterativeEFA (
...
def initIterativeEFA(M, num_sv, D, C, converged, V_bar):
  ...
  */
  // qDebug() << "US_Hydrodyn_Dad_Svd::initIterativeEFA\n";
  EFAFirstRotation (M, C, D, C_ret);
}

void
US_Hydrodyn_Dad_Svd::EFAFirstRotation (vector<vector<int> > &M,
...
for i in range(Cnew.shape[0])])
   ...
   return Cnew
  */
  // qDebug() << "US_Hydrodyn_Dad_Svd::EFAFirstRotation\n";
...
void
US_Hydrodyn_Dad_Svd::initHybridEFA (vector<vector<int> > &M, int num_sv,
                                    vector<vector<double> > &D,
                                    vector<vector<double> > &C,
                                    bool &converged,
...
None, V_bar, T, None, None, None)
   ...
   return failed, C, None
  */
  // qDebug() << "US_Hydrodyn_Dad_Svd::initHybridEFA\n";
...
void
US_Hydrodyn_Dad_Svd::runIterativeEFARotation (
    vector<vector<int> > &M, vector<vector<double> > &D,
    bool &failed, // also a return
    vector<vector<double> > &C, vector<vector<double> > & /* V_bar */,
    vector<vector<double> > & /* T */, int niter, double tol,
...
def runIterativeEFARotation(M, D, failed, C, V_bar, T, niter, tol, force_pos):
  ...
  */
  // qDebug() << "US_Hydrodyn_Dad_Svd::runIterativeEFARotation\n";
  k = 0;
  converged = false;
  failed = false;
...
void
US_Hydrodyn_Dad_Svd::EFAUpdateRotation (vector<vector<int> > &M,
                                        vector<vector<double> > &C,
                                        vector<vector<double> > &D,
                                        vector<int> &force_pos,
                                        // returns
...
for i in range(Cnew.shape[0])])
   ...
   return Cnew
  */
  // qDebug() << "US_Hydrodyn_Dad_Svd::EFAUpdateRotation\n";
...
void
US_Hydrodyn_Dad_Svd::efa_decomp_to_dad_saxs ()
{
  // qDebug() << "efa_decomp_to_dad_saxs";
  for (int i = 0; i < (int)efa_decomp_Ct.size (); ++i)
    {
      QString this_name = QString ("EFA_of_%1_component_%2")
                              .arg (last_efa_name.replace (" ", "_"))
                              .arg (i + 1);
      dad_win->add_plot (this_name, efa_decomp_x, efa_decomp_Ct[i], true);
...
void
US_Hydrodyn_Dad_Svd::set_efa_decomp_force_positive ()
{
