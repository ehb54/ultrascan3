...
void US_Hydrodyn_Dad_Svd::setupGUI() {
  int minHeight1 = 24;
  int minHeight3 = 25;
  ...
  pb_efa_decomp_to_dad_saxs->setFont(QFont(USglobal->config_list.fontFamily,
                                           USglobal->config_list.fontSize - 1));
  ...
  hide_widgets(editor_widgets,
               !ush_win->gparams.count("dad_svd_editor_widgets") ||
                       ush_win->gparams["dad_svd_editor_widgets"] == "false"
                   ? false
                   : true);
  hide_widgets(process_widgets,
               !ush_win->gparams.count("dad_svd_process_widgets") ||
                       ush_win->gparams["dad_svd_process_widgets"] == "false"
                   ? false
                   : true);
  mode_select(MODE_IQIT);
}

void US_Hydrodyn_Dad_Svd::cancel() { close(); }

void US_Hydrodyn_Dad_Svd::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/somo_dad_svd.html");
}

void US_Hydrodyn_Dad_Svd::closeEvent(QCloseEvent *e) { e->accept(); }

void US_Hydrodyn_Dad_Svd::clear_display() {
  editor->clear();
  editor->append("\n\n");
}

void US_Hydrodyn_Dad_Svd::update_font() {
  bool ok;
  QFont newFont;
...
void US_Hydrodyn_Dad_Svd::editor_msg(QString color, QString msg) {
...
void US_Hydrodyn_Dad_Svd::plot_files() {
  // puts( "plot_files" );
...
void US_Hydrodyn_Dad_Svd::plot_files(QStringList add_files, norms norm_mode) {
  // puts( "plot_files" );
  plot_data->detachItems(QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems(QwtPlotItem::Rtti_PlotMarker);
  ;
...
bool US_Hydrodyn_Dad_Svd::plot_file(QString file, double &minx, double &maxx,
...
bool US_Hydrodyn_Dad_Svd::get_min_max(QString file, double &minx, double &maxx,
                                      double &miny, double &maxy,
                                      norms norm_mode) {
  map<QString, vector<double> > *f_Iuse = &f_Is;
...
void US_Hydrodyn_Dad_Svd::disable_all() {
...
void US_Hydrodyn_Dad_Svd::q_start_text(const QString &) {}

void US_Hydrodyn_Dad_Svd::q_end_text(const QString &) {}
...
void US_Hydrodyn_Dad_Svd::replot(bool keep_mode) {
  disable_all();
...
void US_Hydrodyn_Dad_Svd::iq_it() {
  disable_all();
...
void US_Hydrodyn_Dad_Svd::to_dad() {
...
void US_Hydrodyn_Dad_Svd::axis_x() {
...
void US_Hydrodyn_Dad_Svd::axis_y() {
...
void US_Hydrodyn_Dad_Svd::hide_data() {
...
void US_Hydrodyn_Dad_Svd::hide_editor() {
...
void US_Hydrodyn_Dad_Svd::hide_process() {
...
void US_Hydrodyn_Dad_Svd::hide_widgets(vector<QWidget *> widgets, bool hide) {
  for (unsigned int i = 0; i < (unsigned int)widgets.size(); i++) {
    hide ? widgets[i]->hide() : widgets[i]->show();
  }
}

void US_Hydrodyn_Dad_Svd::update_enables() {
  // count selections
  if (running) {
    return;
  }
...
void US_Hydrodyn_Dad_Svd::data_selection_changed() { update_enables(); }

void US_Hydrodyn_Dad_Svd::sv_selection_changed() { update_enables(); }

void US_Hydrodyn_Dad_Svd::clear() {
  lv_data->clearSelection();
  update_enables();
}

QStringList US_Hydrodyn_Dad_Svd::selected_files() {
  QStringList result;
...
set<QString> US_Hydrodyn_Dad_Svd::get_current_files() {
  set<QString> result;
...
set<QString> US_Hydrodyn_Dad_Svd::get_sources() {
  set<QString> result;
...
set<QString> US_Hydrodyn_Dad_Svd::get_selected_sources() {
  set<QString> result;
  QString iq_or_it = iq_it_state ? "I(t)" : "I(q)";
...
QTreeWidgetItem *US_Hydrodyn_Dad_Svd::get_source_item(QString source) {
  QTreeWidgetItemIterator it(lv_data);
  while ((*it)) {
    QTreeWidgetItem *item = (*it);
...
int US_Hydrodyn_Dad_Svd::selected_sources() {
  QString iq_or_it = iq_it_state ? "I(t)" : "I(q)";
...
void US_Hydrodyn_Dad_Svd::clean_selected() {
...
bool US_Hydrodyn_Dad_Svd::all_children_selected(QTreeWidgetItem *lvi) {
...
bool US_Hydrodyn_Dad_Svd::is_selected(QTreeWidgetItem *lvi) {
  if (lvi->isSelected()) {
    return true;
  }
  while (lvi->parent()) {
...
void US_Hydrodyn_Dad_Svd::axis_x_title() {
...
void US_Hydrodyn_Dad_Svd::axis_y_title() {
...
bool US_Hydrodyn_Dad_Svd::plotted_matches_selected() {
  set<QString> plotted;
...
void US_Hydrodyn_Dad_Svd::add_i_of_q_or_t(QString source, QStringList files,
...
void US_Hydrodyn_Dad_Svd::rescale(bool do_update_enables) {
  // bool any_selected = false;
  double minx = 0e0;
  double maxx = 1e0;
  double miny = 0e0;
...
void US_Hydrodyn_Dad_Svd::svd_plot(bool axis_change) {
...
QString US_Hydrodyn_Dad_Svd::get_name() {
  QTreeWidgetItemIterator it(lv_data);
  while ((*it)) {
    QTreeWidgetItem *item = (*it);
    if (is_selected(item)) {
...
void US_Hydrodyn_Dad_Svd::recon() {
...
QTreeWidgetItem *US_Hydrodyn_Dad_Svd::lvi_last_depth(int d) {
...
void US_Hydrodyn_Dad_Svd::color_rotate() {
...
void US_Hydrodyn_Dad_Svd::inc_rmsd_plot(bool axis_change) {
  plot_data->detachItems(QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems(QwtPlotItem::Rtti_PlotMarker);
  ;
  plotted_curves.clear();
...
void US_Hydrodyn_Dad_Svd::inc_chi_plot(bool axis_change) {
  plot_data->detachItems(QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems(QwtPlotItem::Rtti_PlotMarker);
  ;
  plotted_curves.clear();
...
void US_Hydrodyn_Dad_Svd::inc_recon() {
...
void US_Hydrodyn_Dad_Svd::do_recon() {
...
double US_Hydrodyn_Dad_Svd::vmin(vector<double> &x) {
  double min = x[0];
...
double US_Hydrodyn_Dad_Svd::vmax(vector<double> &x) {
  if (!x.size()) {
...
void US_Hydrodyn_Dad_Svd::indiv_recon() {
  recon_mode = "Indiv. ";
...
void US_Hydrodyn_Dad_Svd::set_plot_errors() {
...
void US_Hydrodyn_Dad_Svd::set_plot_errors_rev() { update_plot_errors(); }

void US_Hydrodyn_Dad_Svd::set_plot_errors_sd() {
  if (plot_errors_zoomer) {
    plot_errors_zoomer->zoom(0);
    delete plot_errors_zoomer;
    plot_errors_zoomer = (ScrollZoomer *)0;
...
void US_Hydrodyn_Dad_Svd::set_plot_errors_pct() {
  if (plot_errors_zoomer) {
    plot_errors_zoomer->zoom(0);
    delete plot_errors_zoomer;
    plot_errors_zoomer = (ScrollZoomer *)0;
...
void US_Hydrodyn_Dad_Svd::set_plot_errors_group() {
  if (plot_errors_zoomer) {
    plot_errors_zoomer->zoom(0);
    delete plot_errors_zoomer;
    plot_errors_zoomer = (ScrollZoomer *)0;
...
void US_Hydrodyn_Dad_Svd::set_plot_errors_ref() {
...
void US_Hydrodyn_Dad_Svd::update_plot_errors(bool do_update_enables) {
...
QStringList US_Hydrodyn_Dad_Svd::add_subset_data(QStringList files) {
  QStringList result;
...
QString US_Hydrodyn_Dad_Svd::get_related_source_name(QString name) {
...
QStringList US_Hydrodyn_Dad_Svd::get_files_by_name(QString name) {
...
bool US_Hydrodyn_Dad_Svd::get_name_norm_mode(QString fullname, QString &name,
                                             norms &norm_mode) {
...
bool US_Hydrodyn_Dad_Svd::get_plot_files(QStringList &use_list,
...
void US_Hydrodyn_Dad_Svd::do_plot_errors() {
...
void US_Hydrodyn_Dad_Svd::plot_errors_jump_markers() {
...
void US_Hydrodyn_Dad_Svd::do_plot_errors_group() {
...
bool US_Hydrodyn_Dad_Svd::setup_save(QString tag, QString &fname) {
...
void US_Hydrodyn_Dad_Svd::rmsd_save() {
  QString fname;
...
void US_Hydrodyn_Dad_Svd::svd_save() {
  QString fname;
...
void US_Hydrodyn_Dad_Svd::usp_config_plot_data(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_data);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Dad_Svd::usp_config_plot_svd(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_svd);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Dad_Svd::usp_config_plot_errors(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_errors);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Dad_Svd::usp_config_plot_lefa(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_lefa);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Dad_Svd::usp_config_plot_refa(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_refa);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Dad_Svd::usp_config_plot_ac(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_ac);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Dad_Svd::usp_config_plot_efa_decomp(const QPoint &) {
  US_PlotChoices *uspc = new US_PlotChoices(usp_plot_efa_decomp);
  uspc->exec();
  delete uspc;
}

void US_Hydrodyn_Dad_Svd::set_norm_pw() {
  if (cb_norm_avg->isChecked()) {
    cb_norm_avg->setChecked(false);
  }
  clear_svd();
...
void US_Hydrodyn_Dad_Svd::set_norm_avg() {
  if (cb_norm_pw->isChecked()) {
    cb_norm_pw->setChecked(false);
  }
  clear_svd();
...
void US_Hydrodyn_Dad_Svd::clear_svd() {
  plot_svd->detachItems(QwtPlotItem::Rtti_PlotCurve);
...
void US_Hydrodyn_Dad_Svd::clear_efa() {
...
void US_Hydrodyn_Dad_Svd::clear_efa_decomp() {
...
void US_Hydrodyn_Dad_Svd::make_plotted_efas() {
...
void US_Hydrodyn_Dad_Svd::update_efas(double val) {
...
void US_Hydrodyn_Dad_Svd::efa_plot() {
  int n = (int)efa_lsv.size();
  if (!n) {
    update_efas_ranges();
    return;
...
void US_Hydrodyn_Dad_Svd::setup_norm() {
...
void US_Hydrodyn_Dad_Svd::check_norm(QStringList &files) {
...
double US_Hydrodyn_Dad_Svd::autocor1(vector<double> &x) {
  int len = (int)x.size();
...
vector<double> US_Hydrodyn_Dad_Svd::autocor(vector<vector<double> > &A) {
...
vector<vector<int> > US_Hydrodyn_Dad_Svd::transpose(vector<vector<int> > &A) {
...
vector<vector<double> > US_Hydrodyn_Dad_Svd::transpose(
...
double US_Hydrodyn_Dad_Svd::vvd_min(vector<vector<double> > &A) {
  double result = -1e99;
  int rows = (int)A.size();
  if (!rows) {
...
double US_Hydrodyn_Dad_Svd::vvd_max(vector<vector<double> > &A) {
  double result = 1e99;
  int rows = (int)A.size();
  if (!rows) {
...
void US_Hydrodyn_Dad_Svd::vvd_smult(vector<vector<double> > &A, double x) {
  int rows = (int)A.size();
  if (!rows) {
...
vector<vector<double> > US_Hydrodyn_Dad_Svd::vivd_pwmult(
...
void US_Hydrodyn_Dad_Svd::vvd_cnorm(vector<vector<double> > &A) {
  int rows = (int)A.size();
  if (!rows) {
...
vector<vector<double> > US_Hydrodyn_Dad_Svd::dot(vector<vector<double> > &A,
                                                 vector<vector<double> > &B) {
  vector<vector<double> > result;
  int arows = (int)A.size();
  int brows = (int)B.size();
...
void US_Hydrodyn_Dad_Svd::matrix_info(QString qs, vector<vector<double> > &A) {
  if (!A.size()) {
...
void US_Hydrodyn_Dad_Svd::set_title(QwtPlot *plot, QString title) {
...
void US_Hydrodyn_Dad_Svd::save_plots() {
  QString use_dir = QDir::currentPath();
  ush_win->select_from_directory_history(use_dir, this);
...
void US_Hydrodyn_Dad_Svd::mode_select() {
  // first hide all
...
void US_Hydrodyn_Dad_Svd::mode_select(modes mode) {
  current_mode = mode;
  mode_select();
  update_enables();
}

void US_Hydrodyn_Dad_Svd::set_mode_iqit() { mode_select(MODE_IQIT); }

void US_Hydrodyn_Dad_Svd::set_mode_svd() { mode_select(MODE_SVD); }

void US_Hydrodyn_Dad_Svd::set_mode_efa() { mode_select(MODE_EFA); }

void US_Hydrodyn_Dad_Svd::set_mode_efa_decomp() {
  mode_select(MODE_EFA_DECOMP);
}

void US_Hydrodyn_Dad_Svd::update_efa_range_start(double) {
  // qDebug() << "US_Hydrodyn_Dad_Svd::update_efa_range_start";
  // {
  //    QTextStream ts( stdout );
  //    for ( int i = 0; i < UHSHS_MAX_SV_FOR_EFA; ++i ) {
  //       ts << "US_Hydrodyn_Dad_Svd::update_efa_range_start SV " << i << " val
  //       " << efa_range_start[ i ]->value() << Qt::endl;
  //    }
...
void US_Hydrodyn_Dad_Svd::update_efa_range_end(double) {
...
void US_Hydrodyn_Dad_Svd::update_efas_ranges() {
...
void US_Hydrodyn_Dad_Svd::efa_info(QString tag) {
  QTextStream ts(stdout);
...
vector<double> US_Hydrodyn_Dad_Svd::gradient(vector<double> &y) {
  int len = (int)y.size();
  int lenm1 = len - 1;
...
void US_Hydrodyn_Dad_Svd::init_efa_values() {
...
void US_Hydrodyn_Dad_Svd::set_number_of_svs_for_efa() {
  int max_range =
...
bool US_Hydrodyn_Dad_Svd::convert_it_to_iq(QStringList files,
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size(); i++) {
    ...
    if (!dad_win->f_qs.count(files[i])) {
      ...
      for (unsigned int j = 0; j < (unsigned int)dad_win->f_qs[files[i]].size();
           j++) {
        if (!mode_testiq || (dad_win->f_qs[files[i]][j] >= t_min &&
                             dad_win->f_qs[files[i]][j] <= t_max)) {
          I_values[dad_win->f_qs[files[i]][j]][ql.back()] =
              dad_win->f_Is[files[i]][j];
          if (use_errors && dad_win->f_errors[files[i]].size() ==
      ...
      if (!dad_win->f_errors.count(files[i]) ||
        ...
        if (!dad_win->is_nonzero_vector(dad_win->f_errors[files[i]])) {
          ...
          for (unsigned int j = 0;
               j < (unsigned int)dad_win->f_errors[files[i]].size(); j++) {
            if (!mode_testiq || (dad_win->f_qs[files[i]][j] >= t_min &&
                                 dad_win->f_qs[files[i]][j] <= t_max)) {
...
void US_Hydrodyn_Dad_Svd::runExplicitEFARotation(
...
void US_Hydrodyn_Dad_Svd::efa_decomp() {
...
void US_Hydrodyn_Dad_Svd::efa_decomp_plot() {
...
void US_Hydrodyn_Dad_Svd::initExplicitEFA(
...
void US_Hydrodyn_Dad_Svd::initIterativeEFA(
...
void US_Hydrodyn_Dad_Svd::EFAFirstRotation(vector<vector<int> > &M,
...
void US_Hydrodyn_Dad_Svd::initHybridEFA(
    vector<vector<int> > &M, int num_sv, vector<vector<double> > &D,
...
void US_Hydrodyn_Dad_Svd::runIterativeEFARotation(
    vector<vector<int> > &M, vector<vector<double> > &D,
    bool &failed,  // also a return
    vector<vector<double> > &C, vector<vector<double> > & /* V_bar */,
    vector<vector<double> > & /* T */, int niter, double tol,
...
void US_Hydrodyn_Dad_Svd::EFAUpdateRotation(vector<vector<int> > &M,
                                            vector<vector<double> > &C,
                                            vector<vector<double> > &D,
                                            vector<int> &force_pos,
                                            // returns
...
void US_Hydrodyn_Dad_Svd::efa_decomp_to_dad_saxs() {
...
void US_Hydrodyn_Dad_Svd::set_efa_decomp_force_positive() {
