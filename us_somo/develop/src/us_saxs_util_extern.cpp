...
bool US_Saxs_Util::run_saxs_iq_foxs() {
  ...
  QString cmd = QString("%1 -q %2 -s %3 %4\n")
                    ...
                    .arg((unsigned int)(1 + our_saxs_options.end_q /
  ...
  if (!QFile::exists(created_plt)) {
    ...
    if (!QFile::remove(created_plt)) {
      noticemsg += QString("Notice: remove of FoXS created file %1 failed")
                       .arg(created_dat);
    }
  }

  // now move the file to the saxs directory
  QString new_created_dat = QFileInfo(pdb).fileName() + iqq_suffix() + ".dat";
...
bool US_Saxs_Util::run_saxs_iq_crysol() {
  ...
  QString created_dat =
      crysol_last_pdb_base.replace(QRegExp("\\.(pdb|PDB)$"), "") + "00.int";
...
bool US_Saxs_Util::load_saxs(QString filename) {
  ...
  if (ext == "pdb" || ext == "PDB") {
    errormsg =
        QString("Error: load_saxs() can not load a PDB file as a curve: %1")
            .arg(filename);
    return false;
  }
  ...
  if (ext == "csv" || ext == "CSV") {
    errormsg =
        QString("Error: load_saxs() can not load a csv file as a curve: %1")
            .arg(filename);
    return false;
  }
  ...
  if (f.open(QIODevice::ReadOnly)) {
    ...
    if (!qv.size()) {
      errormsg =
          QString("Error: load_saxs() The file %1 is empty").arg(filename);
      return false;
    }
