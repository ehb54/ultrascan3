...
US_Saxs_Util::run_saxs_iq_foxs ()
{
  ...
  QString cmd = QString ("%1 -q %2 -s %3 %4\n")
                    .arg (prog)
...
#if defined(IQQ_TIMER)
  {
    ...
    QString file = QString ("%1-q%2-%3.timing")
  ...
  if (!QFile::exists (created_dat))
    {
      errormsg
          = QString ("Error: FoXS did not create file %1").arg (created_dat);
      return false;
    }
  ...
  if (!QFile::exists (created_plt))
    {
      errormsg
          = QString ("Notice: FoXS did not create file %1").arg (created_dat);
      return false;
    }
  else
    {
      if (!QFile::remove (created_plt))
        {
          noticemsg
              += QString ("Notice: remove of FoXS created file %1 failed")
                     .arg (created_dat);
        }
    }
  ...
  if (created_dat != new_created_dat
    ...
    {
      noticemsg += QString ("Notice: could not rename %1 to %2")
                       .arg (created_dat)
                       .arg (new_created_dat);
...
bool
US_Saxs_Util::run_saxs_iq_crysol ()
{
  QString pdb = control_parameters["inputfile"];
  ...
  QString crysol_last_pdb_base = QFileInfo (crysol_last_pdb)
                                     .fileName ()
  ...
  QString cmd = QString ("%1").arg (prog);

  cmd += QString (" %1").arg (our_saxs_options.crysol_version_26
                                  ? QFileInfo (use_pdb).fileName ()
                                  : use_pdb);

  cmd += QString (" /sm %1").arg (our_saxs_options.end_q);

  cmd += QString (" /ns %1").arg (
      (unsigned int)(1 + our_saxs_options.end_q / our_saxs_options.delta_q));

  cmd += QString (" /dns %1").arg (our_saxs_options.water_e_density);

  cmd += QString (" /dro %1")
             .arg (our_saxs_options.crysol_hydration_shell_contrast);

  cmd += QString (" /lm %1").arg (our_saxs_options.sh_max_harmonics);

  cmd += QString (" /fb %1").arg (our_saxs_options.sh_fibonacci_grid_order);
  ...
  if (!QFile::exists (created_dat))
    {
      errormsg
          = QString ("Error: Crysol did not create file %1").arg (created_dat);
      return false;
    }
...
bool
US_Saxs_Util::run_sans_iq_cryson ()
{
...
bool
US_Saxs_Util::load_saxs (QString filename)
{
  errormsg = "";
  if (filename.isEmpty ())
    {
  ...
  if (ext == "pdb" || ext == "PDB")
    {
      errormsg
          = QString (
                "Error: load_saxs() can not load a PDB file as a curve: %1")
                .arg (filename);
      return false;
    }
  ...
  if (ext == "csv" || ext == "CSV")
    {
      errormsg
          = QString (
                "Error: load_saxs() can not load a csv file as a curve: %1")
                .arg (filename);
      return false;
    }
  ...
  if (f.open (QIODevice::ReadOnly))
    {
      ...
      if (!qv.size ())
        {
          errormsg = QString ("Error: load_saxs() The file %1 is empty")
                         .arg (filename);
          return false;
        }
      ...
      cout << "q_range after load: " << q[0] << " , " << q[q.size () - 1]
           << endl;
