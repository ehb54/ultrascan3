...
void US_Hydrodyn_Cluster_Results::setupGUI() {
  ...
  lv_files->setHeaderLabels(
      QStringList() << us_tr("Name") << us_tr("Date created") << us_tr("Size"));
...
void US_Hydrodyn_Cluster_Results::cancel() { close(); }
...
void US_Hydrodyn_Cluster_Results::purge() {
  if (US_Static::lv_any_selected(lv_files)) {
    if (QMessageBox::question(this, us_tr("US-SOMO: Cluster Results"),
                              us_tr("Do you want purge these job results?"),
                              us_tr("&Yes"), us_tr("&No"), QString(), 0,
                              1 | QMessageBox::Default)) {
      return;
    }
      ...
      do {
        ...
        if (lvi->isSelected()) {
          if (!QFile::remove(lvi->text(0))) {
            editor_msg(
                "red",
                QString(us_tr("Error: can not remove %1")).arg(lvi->text(0)));
          } else {
          ...
          for (int i = 0; i < (int)qscheck.size(); ++i) {
            if (QFile::exists(qscheck[i])) {
              if (!QFile::remove(qscheck[i])) {
                editor_msg(
                    "red",
                    QString(us_tr("Error: can not remove %1")).arg(qscheck[i]));
              } else {
                editor_msg("black",
...
bool US_Hydrodyn_Cluster_Results::clean_dir(QString dir) {
  ...
  if (!QDir::setCurrent(dir)) {
    errormsg =
        QString(us_tr("Error: can not change to directory %1 (clean_dir)"))
            .arg(dir);
    return false;
  }
    ...
    if (tmp_files[i] != "." && tmp_files[i] != "..") {
      if (QFileInfo(tmp_files[i]).isDir()) {
        ...
        if (!QDir::setCurrent(dir)) {
          errormsg =
              QString(
                  us_tr("Error: can not change to directory %1 (clean_dir2)"))
                  .arg(dir);
          return false;
        }
        QDir qd;
        if (!qd.rmdir(tmp_files[i])) {
          errormsg = QString(us_tr("Error: can not remove directory %1 in %2"))
                         .arg(tmp_files[i])
                         .arg(tmp_dir);
          return false;
        }
...
bool US_Hydrodyn_Cluster_Results::load_one_result(QString file) {
  ...
  if (!QDir::setCurrent(completed_dir)) {
    errormsg =
        QString(
            us_tr("Error: can not change to directory %1 (load_one_result)"))
            .arg(completed_dir);
    return false;
  }
  ...
  if (!QDir::setCurrent(tmp_dir)) {
    errormsg =
        QString(
            us_tr("Error: can not change to directory %1 (load_one_result2)"))
            .arg(tmp_dir);
    return false;
  }
  ...
  if (file.contains(QRegExp("\\.(tgz|TGZ)$"))) {
    ...
    if (!qd.rename(file, dest)) {
      errormsg =
          QString(us_tr("Error: renaming %1 to %2 ")).arg(file).arg(dest);
      return false;
    }
  ...
  if (TAR_OK != result) {
    errormsg = QString(us_tr("Error: %1 problem extracting tar archive (%2)"))
                   .arg(file)
                   .arg(ust.explain(result));
    return false;
  }
  ...
  if (!tar_list.size()) {
    errormsg =
        QString(us_tr("Error: job results are empty for %1 ")).arg(org_file);
    return false;
  }
    ...
    for (unsigned int i = 0; i < (unsigned int)further_extraction.size(); i++) {
      ...
      if (!QDir::setCurrent(dirPath)) {
        errormsg =
            QString(
                us_tr(
                    "Error: can not change to directory %1 (load_one_result3)"))
                .arg(dirPath);
        return false;
      }
      ...
      for (unsigned int i = 0; i < (unsigned int)tar_list.size(); i++) {
        if (QFileInfo(tar_list[i]).path().length() &&
          ...
          errormsg = QString(us_tr("Error: Subdirectories now allowed in "
                                   "results (<%1> %2 %3 %4 %5)"))
  ...
  if (!QDir::setCurrent(tmp_dir)) {
    errormsg =
        QString(
            us_tr("Error: can not change to directory %1 (load_one_result4)"))
            .arg(tmp_dir);
    return false;
  }
...
bool US_Hydrodyn_Cluster_Results::merge_this_csv(QString dest,
                                                 vector<QString> csvs) {
  ...
  if (!QDir::setCurrent(tmp_dir)) {
    errormsg =
        QString(us_tr("Error: can not change to directory %1 (merge_this_csv)"))
            .arg(tmp_dir);
    return false;
  }
  ...
  if (!fo.open(QIODevice::WriteOnly)) {
    errormsg = QString(us_tr("Error: can not create file %1 for merged csvs"))
                   .arg(dest);
    return false;
  }
  ...
  for (unsigned int i = 0; i < (unsigned int)csvs.size(); i++) {
    if (!QFile::exists(csvs[i])) {
      errormsg = QString(us_tr("Error: csv file %1 does not exist for merging"))
                     .arg(csvs[i]);
      fo.close();
      return false;
    }
    ...
    if (!fi.open(QIODevice::ReadOnly)) {
      errormsg = QString(us_tr("Error: can not open file %1 for merging"))
                     .arg(csvs[i]);
      fo.close();
      return false;
    }
...
bool US_Hydrodyn_Cluster_Results::move_to_results(QString jobname,
                                                  QStringList final_results) {
  QString output_dir =
      results_dir + SLASH +
  ...
  if (!qd.exists(output_dir)) {
    if (!qd.mkdir(output_dir)) {
      errormsg =
          QString(us_tr("Error: can not make directory %1")).arg(output_dir);
      return false;
    }
  }
  ...
  for (unsigned int i = 0; i < (unsigned int)final_results.size(); i++) {
    ...
    if (!QDir::setCurrent(from_dir)) {
      errormsg =
          QString(
              us_tr("Error: can not change to directory %1 (move_to_results)"))
              .arg(from_dir);
      return false;
    }
