...
US_Hydrodyn_Cluster_Results::US_Hydrodyn_Cluster_Results (void *us_hydrodyn,
                                                          QWidget *p,
                                                          const char *)
    : QDialog (p)
{
  ...
  if (!update_files (false))
    {
      QMessageBox::information (this, us_tr ("US-SOMO: Cluster Results"),
                                us_tr ("No results found"), 0);
    }
...
void
US_Hydrodyn_Cluster_Results::setupGUI ()
{
  int minHeight1 = 30;
  ...
  pb_purge->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize + 1));
...
void
US_Hydrodyn_Cluster_Results::cancel ()
{
  close ();
}

void
US_Hydrodyn_Cluster_Results::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Cluster_Results::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept ();
...
void
US_Hydrodyn_Cluster_Results::clear_display ()
{
  editor->clear ();
  editor->append ("\n\n");
}
...
void
US_Hydrodyn_Cluster_Results::update_font ()
{
  bool ok;
  QFont newFont;
...
void
US_Hydrodyn_Cluster_Results::save ()
{
  QString fn;
...
void
US_Hydrodyn_Cluster_Results::editor_msg (QString color, QString msg)
{
...
void
US_Hydrodyn_Cluster_Results::update_enables ()
{
  if (!disable_updates)
    {
...
void
US_Hydrodyn_Cluster_Results::purge ()
{
  if (US_Static::lv_any_selected (lv_files))
    {
      if (QMessageBox::question (
              this, us_tr ("US-SOMO: Cluster Results"),
              us_tr ("Do you want purge these job results?"), us_tr ("&Yes"),
              us_tr ("&No"), QString (), 0, 1 | QMessageBox::Default))
        {
          return;
        }
              ...
              if (lvi->isSelected ())
                {
                  if (!QFile::remove (lvi->text (0)))
                    {
                      editor_msg ("red",
                                  QString (us_tr ("Error: can not remove %1"))
                                      .arg (lvi->text (0)));
                    }
                  else
                    {
                      editor_msg (
                          "black",
                          QString (us_tr ("Removed %1")).arg (lvi->text (0)));
                    }
                  QString qs = lvi->text (0);
                  qs.replace (QRegExp ("_out.t..$"), "");
                  QStringList qscheck;
                  qscheck << qs + ".tar" << qs + ".tgz" << qs + ".TAR"
                          << qs + ".TGZ";
      ...
      while (*it)
        {
          ...
          if (!QFile::remove (lvi->text (0)))
            {
              editor_msg ("red", QString (us_tr ("Error: can not remove %1"))
                                     .arg (lvi->text (0)));
            }
          else
            {
          ...
          qs.replace (QRegExp ("_out.t..$"), "");
          QStringList qscheck;
          qscheck << qs + ".tar" << qs + ".tgz" << qs + ".TAR" << qs + ".TGZ";
              ...
              if (QFile::exists (qscheck[i]))
                {
                  if (!QFile::remove (qscheck[i]))
                  ...
                  else
                    {
                      editor_msg (
                          "black",
                          QString (us_tr ("Removed %1")).arg (qscheck[i]));
                    }
                }
            }
          ++it;
      ...
      if (!update_files ())
        {
          QMessageBox::information (this, us_tr ("US-SOMO: Cluster Results"),
                                    us_tr ("No further job results found"), 0);
          close ();
          return;
        }
...
void
US_Hydrodyn_Cluster_Results::select_all ()
{
...
bool
US_Hydrodyn_Cluster_Results::clean_dir (QString dir)
{
  ...
  if (!QDir::setCurrent (dir))
    {
      errormsg
          = QString (
                us_tr ("Error: can not change to directory %1 (clean_dir)"))
                .arg (dir);
      return false;
    }
          ...
          if (QFileInfo (tmp_files[i]).isDir ())
            {
              ...
              if (!QDir::setCurrent (dir))
                {
                  errormsg = QString (us_tr ("Error: can not change to "
                                             "directory %1 (clean_dir2)"))
                                 .arg (dir);
                  return false;
                }
              QDir qd;
              if (!qd.rmdir (tmp_files[i]))
                {
                  errormsg
                      = QString (
                            us_tr ("Error: can not remove directory %1 in %2"))
                            .arg (tmp_files[i])
                            .arg (tmp_dir);
                  return false;
                }
          ...
          else
            {
              if (!QFile::remove (tmp_files[i]))
                {
                  errormsg
                      = QString (us_tr ("Error: can not remove file %1 in %2"))
                            .arg (tmp_files[i])
                            .arg (tmp_dir);
                  return false;
                }
...
void
US_Hydrodyn_Cluster_Results::load_results ()
{
  bool any_selected = US_Static::lv_any_selected (lv_files);
...
bool
US_Hydrodyn_Cluster_Results::load_one_result (QString file)
{
  ...
  if (!QDir::setCurrent (completed_dir))
    {
      errormsg
          = QString (
                us_tr (
                    "Error: can not change to directory %1 (load_one_result)"))
                .arg (completed_dir);
      return false;
    }
  ...
  if (!QFile::exists (file))
    {
      errormsg = QString (us_tr ("Error: file %1 does not exist in %2"))
                     .arg (file)
                     .arg (completed_dir);
      return false;
    }
  ...
  if (!QDir::setCurrent (tmp_dir))
    {
      errormsg = QString (us_tr ("Error: can not change to directory %1 "
                                 "(load_one_result2)"))
                     .arg (tmp_dir);
      return false;
    }
  ...
  if (file.contains (QRegExp ("\\.(tgz|TGZ)$")))
    {
      ...
      if (!qd.rename (file, dest))
        {
          errormsg = QString (us_tr ("Error: renaming %1 to %2 "))
                         .arg (file)
                         .arg (dest);
          return false;
        }
      ...
      if (GZIP_OK != result)
        {
          errormsg = QString (us_tr ("Error: %1 problem ungzipping (%2)"))
                         .arg (file)
      ...
      if (file.contains (QRegExp ("\\.(tgz|TGZ)$")))
        {
          // rename it to .tar (?) probably some error in us_gzip internal name
          // storage
          dest = file;
  ...
  if (TAR_OK != result)
    {
      errormsg = QString (us_tr ("Error: %1 problem list tar archive (%2)"))
                     .arg (file)
  ...
  if (TAR_OK != result)
    {
      errormsg
          = QString (us_tr ("Error: %1 problem extracting tar archive (%2)"))
                .arg (file)
                .arg (ust.explain (result));
      return false;
    }
  ...
  if (!QFile::remove (file))
    {
      errormsg = QString (us_tr ("Error: can not remove file %1 in %2"))
                     .arg (file)
                     .arg (tmp_dir);
      return false;
    }
  ...
  if (!tar_list.size ())
    {
      errormsg = QString (us_tr ("Error: job results are empty for %1 "))
                     .arg (org_file);
      return false;
    }
  ...
  if (further_extraction.size ())
    {
      for (unsigned int i = 0; i < (unsigned int)further_extraction.size ();
           i++)
        {
          ...
          if (!QDir::setCurrent (dirPath))
            {
              errormsg = QString (us_tr ("Error: can not change to directory "
                                         "%1 (load_one_result3)"))
                             .arg (dirPath);
              return false;
            }
          ...
          if (file.contains (QRegExp ("\\.(tgz|TGZ)$")))
            {
              ...
              if (GZIP_OK != result)
                {
                  errormsg
                      = QString (us_tr ("Error: %1 problem ungzipping (%2)"))
                            .arg (file)
                            .arg (usg.explain (result));
                  return false;
                }
          ...
          if (!file.contains (QRegExp ("\\.(tar|TAR)$")))
            {
              errormsg
                  = QString (us_tr ("Error: file %1 is not .tar")).arg (file);
              return false;
            }
          ...
          if (TAR_OK != result)
            {
              errormsg
                  = QString (us_tr ("Error: %1 problem list tar archive (%2)"))
                        .arg (file)
                        .arg (ust.explain (result));
              return false;
            }
          ...
          for (unsigned int i = 0; i < (unsigned int)tar_list.size (); i++)
            {
              if (QFileInfo (tar_list[i]).path ().length ()
                ...
                {
                  errormsg
                      = QString (us_tr ("Error: Subdirectories now allowed in "
                                        "results (<%1> %2 %3 %4 %5)"))
          ...
          if (TAR_OK != result)
            {
              errormsg
                  = QString (
                        us_tr (
                            "Error: %1 problem extracting tar archive (%2)"))
                        .arg (file)
                        .arg (ust.explain (result));
              return false;
            }
...
bool
US_Hydrodyn_Cluster_Results::merge_csvs (QStringList &final_results)
{
...
bool
US_Hydrodyn_Cluster_Results::merge_this_csv (QString dest,
                                             vector<QString> csvs)
{
  ...
  if (!QDir::setCurrent (tmp_dir))
    {
      errormsg
          = QString (
                us_tr (
                    "Error: can not change to directory %1 (merge_this_csv)"))
                .arg (tmp_dir);
      return false;
    }
  ...
  if (!fo.open (QIODevice::WriteOnly))
    {
      errormsg
          = QString (us_tr ("Error: can not create file %1 for merged csvs"))
                .arg (dest);
      return false;
    }
  ...
  for (unsigned int i = 0; i < (unsigned int)csvs.size (); i++)
    {
      if (!QFile::exists (csvs[i]))
        {
          errormsg
              = QString (
                    us_tr ("Error: csv file %1 does not exist for merging"))
                    .arg (csvs[i]);
          fo.close ();
          return false;
        }
      ...
      if (!fi.open (QIODevice::ReadOnly))
        {
          errormsg
              = QString (us_tr ("Error: can not open file %1 for merging"))
                    .arg (csvs[i]);
          fo.close ();
          return false;
        }
...
bool
US_Hydrodyn_Cluster_Results::move_to_results (QString jobname,
                                              QStringList final_results)
{
  QString output_dir
      = results_dir + SLASH
        + QString ("%1").arg (jobname).replace (QRegExp ("_(out|OUT).*$"), "");
  QDir qd (output_dir);
  ...
  if (!qd.exists (output_dir))
    {
      if (!qd.mkdir (output_dir))
        {
          errormsg = QString (us_tr ("Error: can not make directory %1"))
                         .arg (output_dir);
          return false;
        }
    }
  ...
  for (unsigned int i = 0; i < (unsigned int)final_results.size (); i++)
    {
      ...
      QString dest_file
          = output_dir + QDir::separator () + base_name + extension;

      int unique_copy = 0;
      while (QFile::exists (dest_file))
        {
          dest_file = output_dir + QDir::separator () + base_name
                      + QString ("-%1").arg (++unique_copy) + extension;
        }
  ...
  editor_msg ("dark blue", QString (us_tr ("Results for job %1 are now in %2"))
                               .arg (jobname)
                               .arg (output_dir));
...
unsigned int
US_Hydrodyn_Cluster_Results::update_files (bool set_lv_files)
{
  files.clear ();
