...
US_Hydrodyn_Mals_Saxs_P3d::US_Hydrodyn_Mals_Saxs_P3d(
  ...
  setWindowTitle(us_tr("US-SOMO: MALS_SAXS : Plot 3D"));
...
void US_Hydrodyn_Mals_Saxs_P3d::setupGUI() {
  int minHeight1 = 30;

  lbl_title = new QLabel(us_tr("US-SOMO: MALS_SAXS : Plot 3D"), this);
  ...
  lbl_plot_curves->setFont(QFont(USglobal->config_list.fontFamily,
  ...
  pb_plot_all->setFont(QFont(USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize + 1));
  ...
  pb_plot->setFont(QFont(USglobal->config_list.fontFamily,
                         USglobal->config_list.fontSize + 1));
  ...
  QHBoxLayout *hbl_plot_curves = new QHBoxLayout();
  hbl_plot_curves->setContentsMargins(0, 0, 0, 0);
  hbl_plot_curves->setSpacing(0);
...
void US_Hydrodyn_Mals_Saxs_P3d::plot_all() {
  for (unsigned int i = 0; i < (unsigned int)cb_plot_curves.size(); i++) {
...
void US_Hydrodyn_Mals_Saxs_P3d::quit() { close(); }

void US_Hydrodyn_Mals_Saxs_P3d::plot() {
...
void US_Hydrodyn_Mals_Saxs_P3d::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/mals_saxs_p3d.html");
}

void US_Hydrodyn_Mals_Saxs_P3d::closeEvent(QCloseEvent *e) {
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept();
}

void US_Hydrodyn_Mals_Saxs_P3d::update_enables() {
