...
US_License::US_License (QWidget *parent, const char *) : QDialog (parent)
{
  ...
  int half_buttonw
      = (int)((buttonw + 2 * column1 + 2 * column3 + 2 * spacing - column2)
              / 2);
  ...
  lbl_blank = new QLabel (
      us_tr (
          "Please enter all fields exactly as shown in the issued license,\n"
  ...
  connect (cbb_version, SIGNAL (highlighted (int)),
           SLOT (update_version (int)));
...
void
US_License::closeEvent (QCloseEvent *e)
{
  e->accept ();
  global_Xpos -= 30;
  global_Ypos -= 30;
  ...
  if (f.open (QIODevice::WriteOnly | QIODevice::Text))
    {
      ...
      US_Static::us_message (
          us_tr ("Thanks!"),
          us_tr (
              "Your license information was saved to the following file:\n\n"
      ...
      if (!ts.atEnd ())
        {
          version = ts.readLine ();
          int i = 0;
          while (i < cbb_version->count ()
                 && version != cbb_version->itemText (i))
            {
              i++;
            }
...
#define BROWSERS "Internet Explorer"
  ...
  while (strlen (qPrintable (browser[i].program)) != 0)
    {
      if (start_browser (browser[i].program, browser[i].remote,
                         browser[i].url))
        {
  ...
  if (!browser_started)
    {
      cout << "Error: Couldn't start " BROWSERS "\n"
              "Please make sure your have the appropriate browser "
              "installed!\n\n"
      ...
      US_Static::us_message (
          ...
          us_tr (
              "Couldn't start " BROWSERS "\n\n"
              "Please make sure you have the appropriate browser "
              "installed.\n\n"
...
bool
US_License::start_browser (const QString &browser, const QString &remote,
                           const QString &url)
{
#if QT_VERSION < 0x040000
  proc->clearArguments ();
...
void
US_License::update_firstname (const QString &str)
{
  firstname = str;
}

void
US_License::update_lastname (const QString &str)
{
  lastname = str;
}

void
US_License::update_institution (const QString &str)
{
  institution = str;
}

void
US_License::update_address (const QString &str)
{
  address = str;
}

void
US_License::update_city (const QString &str)
{
  city = str;
}

void
US_License::update_state (int val)
{
  state = cbb_state->itemText (val);
}

void
US_License::update_zip (const QString &str)
{
  zip = str;
}

void
US_License::update_phone (const QString &str)
{
  phone = str;
}

void
US_License::update_email (const QString &str)
{
  email = str;
}

void
US_License::update_version (int val)
{
  version = cbb_version->itemText (val);
}

void
US_License::update_licensetype (int item)
{
  licensetype = cbb_licensetype->itemText (item);
}

void
US_License::update_code (const QString &str)
{
  code = str;
}

void
US_License::update_expiration (const QString &str)
{
  expiration = str;
}

void
US_License::update_opteron_rb ()
{
  // platform = "opteron";
