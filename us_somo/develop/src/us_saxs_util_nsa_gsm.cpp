...
long
US_Saxs_Util::nsa_min_gsm_5_1 (our_vector *i, double epsilon, long max_iter)
{
#if defined(USUNG_DEBUG)
#if defined(USE_MPI)
  debug_mpi (
      QString ("%1: starting nsa_min_gsm_5_1: our_vector\n").arg (myrank));
#else
  ...
  while (l2norm_our_vector (u, zero) >= epsilon && iter++ < max_iter)
    {
      ...
      printf (
          ...
          this_rank, global_iter++, iter, fitness = nsa_gsm_f (i),
      ...
      if (!(global_iter % 5))
        {
          ...
          printf ("%d: "
                  ...
                  this_rank, (nsa_gsm_f (i) <= 0e0) ? "success" : "fail", N,
                  nsa_gsm_f (i), queries, fitness_type, 0,
...
long
US_Saxs_Util::nsa_min_fr_pr_cgd (our_vector *i, double epsilon, long max_iter)
{
...
long
US_Saxs_Util::nsa_min_hessian_bfgs (our_vector *ip, double epsilon,
                                    long max_iter)
{
  ...
  while (l2norm_our_vector (u, zero) >= epsilon && iter++ < max_iter)
    {
      ...
      printf (
          "%d: global %ld, iter %ld fitness %.12g |grad|=%.12g last reps %d\n",
          this_rank, global_iter++, iter, fitness = nsa_gsm_f (ip),
          l2norm_our_vector (u, zero), last_reps);
      fflush (stdout);
      printf ("%d: ", this_rank);
      print_our_vector (ip);
      ...
      if (!(global_iter % 5))
        {
          ...
          printf ("%d: "
                  ...
                  this_rank, (nsa_gsm_f (ip) <= 0e0) ? "success" : "fail", N,
                  nsa_gsm_f (ip), queries, fitness_type, 0e0,
                  this_iterations + prev_iterations, go, 0e0, 0e0, 0e0, 0e0,
                  iterations_max, 0, 0, 0, min_acceptable, 0L, 0L);
