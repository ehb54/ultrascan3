...
void US_Hydrodyn_Mals_Saxs_Ciq::setupGUI() {
  ...
  lbl_conc =
      new QLabel(us_tr("Concentrations will be computed and will be written "
                       "along with dn/dc values to the output I(q) curves"),
                 this);
  ...
  for (unsigned int i = 0; i < (*parameters)["gaussians"].toUInt(); i++) {
    ...
    le_tmp->setText(
        parameters->count("default_extc") ? (*parameters)["default_extc"] : "");
    ...
    le_tmp->setObjectName("le_tmp Line Edit");
    le_tmp->setText(
        parameters->count("default_dndc") ? (*parameters)["default_dndc"] : "");
    le_tmp->setAlignment(Qt::AlignCenter | Qt::AlignVCenter);
    le_tmp->setPalette(PALET_NORMAL);
    AUTFBACK(le_tmp);
    le_tmp->setFont(QFont(USglobal->config_list.fontFamily,
...
void US_Hydrodyn_Mals_Saxs_Ciq::quit() { close(); }

void US_Hydrodyn_Mals_Saxs_Ciq::create_ng() {
  (*parameters)["go"] = "true";
  (*parameters)["make_ng"] = "true";
  close();
}

void US_Hydrodyn_Mals_Saxs_Ciq::go() {
  (*parameters)["go"] = "true";
...
void US_Hydrodyn_Mals_Saxs_Ciq::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/mals_saxs_ciq.html");
}

void US_Hydrodyn_Mals_Saxs_Ciq::closeEvent(QCloseEvent *e) {
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept();
}

void US_Hydrodyn_Mals_Saxs_Ciq::set_add_bl() {
  (*parameters)["add_baseline"] = cb_add_bl->isChecked() ? "true" : "false";
}

void US_Hydrodyn_Mals_Saxs_Ciq::set_sd_source() { update_enables(); }

void US_Hydrodyn_Mals_Saxs_Ciq::set_normalize() { update_enables(); }

void US_Hydrodyn_Mals_Saxs_Ciq::set_I0se() { update_enables(); }

void US_Hydrodyn_Mals_Saxs_Ciq::set_save_as_pct_iq() {
...
void US_Hydrodyn_Mals_Saxs_Ciq::global() {
...
void US_Hydrodyn_Mals_Saxs_Ciq::set_sd_zero_avg_local_sd() {
  (*parameters)["sd_zero_avg_local_sd"] =
      cb_sd_zero_avg_local_sd->isChecked() ? "true" : "false";
...
void US_Hydrodyn_Mals_Saxs_Ciq::set_sd_zero_keep_as_zeros() {
  (*parameters)["sd_zero_keep_as_zeros"] =
      cb_sd_zero_keep_as_zeros->isChecked() ? "true" : "false";
...
void US_Hydrodyn_Mals_Saxs_Ciq::set_sd_zero_set_to_pt1pct() {
  (*parameters)["sd_zero_set_to_pt1pct"] =
      cb_sd_zero_set_to_pt1pct->isChecked() ? "true" : "false";
...
void US_Hydrodyn_Mals_Saxs_Ciq::set_zero_drop_points() {
  (*parameters)["zero_drop_points"] =
      cb_zero_drop_points->isChecked() ? "true" : "false";
...
void US_Hydrodyn_Mals_Saxs_Ciq::set_zero_avg_local_sd() {
  (*parameters)["zero_avg_local_sd"] =
      cb_zero_avg_local_sd->isChecked() ? "true" : "false";
...
void US_Hydrodyn_Mals_Saxs_Ciq::set_zero_keep_as_zeros() {
  (*parameters)["zero_keep_as_zeros"] =
      cb_zero_keep_as_zeros->isChecked() ? "true" : "false";
...
void US_Hydrodyn_Mals_Saxs_Ciq::update_enables() {
  bool no_go = false;
...
void US_Hydrodyn_Mals_Saxs_Ciq::ws_hide(vector<QWidget *> ws, bool hide) {
  for (unsigned int i = 0; i < (unsigned int)ws.size(); i++) {
    hide ? ws[i]->hide() : ws[i]->show();
  }
}

void US_Hydrodyn_Mals_Saxs_Ciq::zeros_found() {
