...
US_Hydrodyn_Mals_Saxs_Ciq::US_Hydrodyn_Mals_Saxs_Ciq (
    ...
    : QDialog (p)
{
  ...
  setWindowTitle (us_tr ("US-SOMO: MALS_SAXS : Make I*(q)"));
...
void
US_Hydrodyn_Mals_Saxs_Ciq::setupGUI ()
{
  ...
  lbl_title = new QLabel (
      parameters->count ("ngmode")
          ? us_tr ("US-SOMO: MALS_SAXS : Make I*(q) without Gaussians")
          : us_tr ("US-SOMO: MALS_SAXS : Make I*(q)"),
      this);
  ...
  cb_add_bl->setText (
      parameters->count ("baseline") ? (*parameters)["baseline"] : "");
  cb_add_bl->setEnabled (true);
  connect (cb_add_bl, SIGNAL (clicked ()), SLOT (set_add_bl ()));
  cb_add_bl->setChecked (false);
  cb_add_bl->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  cb_save_as_pct_iq->setText (
      us_tr ("Resulting I(q) created as a percent of the original I(q) ( if "
             "unchecked, I(q) will be created from the Gaussians )"));
  cb_save_as_pct_iq->setEnabled (true);
  cb_save_as_pct_iq->setFont (QFont (USglobal->config_list.fontFamily,
                                     USglobal->config_list.fontSize));
  ...
  cb_save_sum->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  ...
  if (parameters->count ("integralbaseline"))
    {
      lbl_save_sum_bi = new QLabel (
          us_tr ("     Warning: If you have changed the starting time/frame "
                 "since integral baseline correction, creation of the\n"
                 "      sum of peaks curves with integral baseline will "
                 "differ due to missing time/frame scattering intensity."),
          this);
      ...
      lbl_save_sum_bi->setFont (QFont (USglobal->config_list.fontFamily,
                                       USglobal->config_list.fontSize,
                                       QFont::Bold));
    }
  ...
  cb_sd_source->setText (us_tr (
      // "Compute standard deviations as a difference between the sum of
      // Gaussians and original I(q)"
      "Add SD computed %-wise from the difference between the sum of "
      "Gaussians and the original I(q)"));
  cb_sd_source->setEnabled (true);
  cb_sd_source->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  cb_makeiq_avg_peaks->setText (
      us_tr ("Average and normalize resulting I(q) curves by Gaussian, using "
             "top % of max. intensity"));
  ...
  // cb's co dependent
  connect (cb_save_as_pct_iq, SIGNAL (clicked ()),
           SLOT (set_save_as_pct_iq ()));
  connect (cb_sd_source, SIGNAL (clicked ()), SLOT (set_sd_source ()));
  cb_sd_source->setChecked (false);
  cb_save_as_pct_iq->setChecked (true);

  QLabel *lbl_sd_zeros_found = new QLabel (
      us_tr ("If zeros are produced when computing SDs:  "), this);
  ...
  lbl_sd_zeros_found->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize,
                                      QFont::Bold));
  if (parameters->count ("ngmode"))
    {
  ...
  connect (cb_sd_zero_avg_local_sd, SIGNAL (clicked ()),
           SLOT (set_sd_zero_avg_local_sd ()));
  cb_sd_zero_avg_local_sd->setChecked (true);
  cb_sd_zero_avg_local_sd->setFont (QFont (USglobal->config_list.fontFamily,
                                           USglobal->config_list.fontSize));
  ...
  connect (cb_sd_zero_keep_as_zeros, SIGNAL (clicked ()),
           SLOT (set_sd_zero_keep_as_zeros ()));
  cb_sd_zero_keep_as_zeros->setChecked (false);
  cb_sd_zero_keep_as_zeros->setFont (QFont (USglobal->config_list.fontFamily,
                                            USglobal->config_list.fontSize));
  ...
  connect (cb_sd_zero_set_to_pt1pct, SIGNAL (clicked ()),
           SLOT (set_sd_zero_set_to_pt1pct ()));
  cb_sd_zero_set_to_pt1pct->setChecked (false);
  cb_sd_zero_set_to_pt1pct->setFont (QFont (USglobal->config_list.fontFamily,
                                            USglobal->config_list.fontSize));
  ...
  QLabel *lbl_zeros_found = new mQLabel (
      us_tr ("I(t) is missing SDs at some points. When computing I(q) SDs : "),
      this);
  ...
  lbl_zeros_found->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize,
                                   QFont::Bold));
  ...
  connect (cb_zero_drop_points, SIGNAL (clicked ()),
           SLOT (set_zero_drop_points ()));
  cb_zero_drop_points->setChecked (false);
  cb_zero_drop_points->setFont (QFont (USglobal->config_list.fontFamily,
                                       USglobal->config_list.fontSize));
  ...
  connect (cb_zero_avg_local_sd, SIGNAL (clicked ()),
           SLOT (set_zero_avg_local_sd ()));
  cb_zero_avg_local_sd->setChecked (true);
  cb_zero_avg_local_sd->setFont (QFont (USglobal->config_list.fontFamily,
                                        USglobal->config_list.fontSize));
  ...
  connect (cb_zero_keep_as_zeros, SIGNAL (clicked ()),
           SLOT (set_zero_keep_as_zeros ()));
  cb_zero_keep_as_zeros->setChecked (false);
  cb_zero_keep_as_zeros->setFont (QFont (USglobal->config_list.fontFamily,
                                         USglobal->config_list.fontSize));
  ...
  cb_I0se->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize));
  cb_I0se->setPalette (PALET_NORMAL);
  AUTFBACK (cb_I0se);
  cb_I0se->setMinimumHeight (minHeight2);
  cb_I0se->setToolTip (
      us_tr ("<html><head/><body><p>Note: This value will be stored in the "
             "produced files and used in SOMO-SAS MW computations.</p><p>If "
             "your data is already properly normalized, you should leave this "
             "checked with a value of 1.</p></body></html>"));
  cb_I0se->hide ();

  le_I0se = new QLineEdit (this);
  le_I0se->setObjectName ("le_I0se Line Edit");
  le_I0se->setText ("1");
  le_I0se->setAlignment (Qt::AlignCenter | Qt::AlignVCenter);
  le_I0se->setPalette (PALET_NORMAL);
  AUTFBACK (le_I0se);
  le_I0se->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize));
  {
  ...
  connect (le_I0se, SIGNAL (textChanged (const QString &)),
           SLOT (update_enables ()));
  ...
  lbl_error = new QLabel (
      parameters->count ("error") ? (*parameters)["error"] : "", this);
  lbl_error->setAlignment (Qt::AlignCenter | Qt::AlignVCenter);
  lbl_error->setPalette (PALET_NORMAL);
  AUTFBACK (lbl_error);
  lbl_error->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize + 1, QFont::Bold));

  cb_conc_to_saxs = new QCheckBox (this);
  cb_conc_to_saxs->setText (
      us_tr ("Do you want to set the concentration file Gaussians centers, "
             "widths and skewness to the SAXS-optimized values,\nadjusting "
             "the amplitudes and keeping the areas constant?\n"));
  cb_conc_to_saxs->setEnabled (true);
  connect (cb_conc_to_saxs, SIGNAL (clicked ()), SLOT (update_enables ()));
  cb_conc_to_saxs->setChecked (true);
  cb_conc_to_saxs->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize));
  ...
  lbl_conc_to_saxs_info1 = new QLabel (
      us_tr ("       This implies that all the species that were defined as "
             "Gaussians contributing to the SAXS signal also contribute to "
  ...
  lbl_conc_to_saxs_info1->setFont (QFont (USglobal->config_list.fontFamily,
                                          USglobal->config_list.fontSize));

  lbl_conc_to_saxs_info2 = new QLabel (
      us_tr ("       Be aware that this option will result in an apparent "
             "mass artificially approximately constant along each of the "
             "deconvoluted Gaussian peaks,\n"
             "       reflecting just the oscillations in the original SAXS "
             "data."),
      this);
  lbl_conc_to_saxs_info2->setAlignment (Qt::AlignJustify | Qt::AlignVCenter);
  lbl_conc_to_saxs_info2->setPalette (qcg_normal_redtext);
  lbl_conc_to_saxs_info2->setFont (QFont (USglobal->config_list.fontFamily,
                                          USglobal->config_list.fontSize));

  lbl_conc_to_saxs_info3 = new QLabel (
      us_tr ("       However, the apparent average mass for each peak should "
             "be a closer approximation to the real value when significant "
  ...
  lbl_conc_to_saxs_info3->setFont (QFont (USglobal->config_list.fontFamily,
                                          USglobal->config_list.fontSize));
  if (parameters->count ("ngmode"))
    {
  ...
  lbl_conc = new QLabel (
      us_tr ("Concentrations will be computed and will be written along with "
             "dn/dc values to the output I(q) curves"),
      this);
  ...
  pb_global
      = new QPushButton (us_tr ("Duplicate Gaussian 1 values globally"), this);
  pb_global->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize));
  ...
  lbl_gaussian->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  if (parameters->count ("ngmode"))
    {
      lbl_gaussian->hide ();
  ...
  lbl_conv
      = new QLabel (us_tr ((*parameters).count ("uv")
                               ? "Extinction coefficient (ml mg^-1 cm^-1)"
                               : "Differential RI increment [dn/dc] (ml/g)"),
                    this);
  lbl_conv->setAlignment (Qt::AlignCenter | Qt::AlignVCenter);
  lbl_conv->setPalette (PALET_NORMAL);
  AUTFBACK (lbl_conv);
  lbl_conv->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize));
  ...
  AUTFBACK (lbl_dndc);
  lbl_dndc->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize));
  ...
  for (unsigned int i = 0; i < (*parameters)["gaussians"].toUInt (); i++)
    {
      ...
      le_tmp->setObjectName ("le_tmp Line Edit");
      le_tmp->setText (parameters->count ("default_extc")
      ...
      connect (le_tmp, SIGNAL (textChanged (const QString &)),
               SLOT (update_enables ()));
      le_tmp->setMinimumWidth (200);
      le_tmp->setMinimumHeight (minHeight1);
      le_conv.push_back (le_tmp);

      le_tmp = new QLineEdit (this);
      le_tmp->setObjectName ("le_tmp Line Edit");
      le_tmp->setText (parameters->count ("default_dndc")
  ...
  pb_create_ng
      = new QPushButton (us_tr ("Make I*(q) without Gaussians"), this);
  pb_create_ng->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize + 1));
  ...
  QHBoxLayout *hbl_sd_zeros = new QHBoxLayout ();
  hbl_sd_zeros->setContentsMargins (0, 0, 0, 0);
  hbl_sd_zeros->setSpacing (0);
  ...
  QHBoxLayout *hbl_zeros = new QHBoxLayout ();
  hbl_zeros->setContentsMargins (0, 0, 0, 0);
  hbl_zeros->setSpacing (0);
  ...
  {
    QHBoxLayout *hbl_avg_peaks = new QHBoxLayout ();
    hbl_avg_peaks->setContentsMargins (0, 0, 0, 0);
    hbl_avg_peaks->setSpacing (0);
  ...
  QHBoxLayout *hbl_I0se = new QHBoxLayout ();
  hbl_I0se->setContentsMargins (0, 0, 0, 0);
  hbl_I0se->setSpacing (0);
  ...
  if (parameters->count ("error"))
    {
      ...
      cb_makeiq_avg_peaks->setText (
          us_tr ("Average resulting I(q) curves by Gaussian, using top % of "
                 "max. intensity"));
...
void
US_Hydrodyn_Mals_Saxs_Ciq::quit ()
{
  close ();
}

void
US_Hydrodyn_Mals_Saxs_Ciq::create_ng ()
{
  (*parameters)["go"] = "true";
...
void
US_Hydrodyn_Mals_Saxs_Ciq::go ()
{
  (*parameters)["go"] = "true";
  (*parameters)["normalize"] = cb_normalize->isChecked () ? "true" : "false";
  (*parameters)["sd_source"]
      = cb_sd_source->isChecked () ? "difference" : "original";
  (*parameters)["save_sum"] = cb_save_sum->isChecked () ? "true" : "false";

  (*parameters)["sd_zero_avg_local_sd"]
      = cb_sd_zero_avg_local_sd->isChecked () ? "true" : "false";
  (*parameters)["sd_zero_keep_as_zeros"]
      = cb_sd_zero_keep_as_zeros->isChecked () ? "true" : "false";
  (*parameters)["sd_zero_set_to_pt1pct"]
      = cb_sd_zero_set_to_pt1pct->isChecked () ? "true" : "false";

  (*parameters)["zero_drop_points"]
      = cb_zero_drop_points->isChecked () ? "true" : "false";
  (*parameters)["zero_avg_local_sd"]
      = cb_zero_avg_local_sd->isChecked () ? "true" : "false";
  (*parameters)["zero_keep_as_zeros"]
      = cb_zero_keep_as_zeros->isChecked () ? "true" : "false";

  (*parameters)["conc_to_saxs"]
      = cb_conc_to_saxs->isChecked () ? "true" : "false";
...
void
US_Hydrodyn_Mals_Saxs_Ciq::help ()
{
  US_Help *online_help;
  online_help = new US_Help (this);
  online_help->show_help ("manual/somo/mals_saxs_ciq.html");
}

void
US_Hydrodyn_Mals_Saxs_Ciq::closeEvent (QCloseEvent *e)
{
...
void
US_Hydrodyn_Mals_Saxs_Ciq::set_add_bl ()
{
  (*parameters)["add_baseline"] = cb_add_bl->isChecked () ? "true" : "false";
}

void
US_Hydrodyn_Mals_Saxs_Ciq::set_sd_source ()
{
  update_enables ();
}

void
US_Hydrodyn_Mals_Saxs_Ciq::set_normalize ()
{
  update_enables ();
}

void
US_Hydrodyn_Mals_Saxs_Ciq::set_I0se ()
{
  update_enables ();
}

void
US_Hydrodyn_Mals_Saxs_Ciq::set_save_as_pct_iq ()
{
  (*parameters)["save_as_pct_iq"]
      = cb_save_as_pct_iq->isChecked () ? "true" : "false";
...
void
US_Hydrodyn_Mals_Saxs_Ciq::global ()
{
...
void
US_Hydrodyn_Mals_Saxs_Ciq::set_sd_zero_avg_local_sd ()
{
  (*parameters)["sd_zero_avg_local_sd"]
      = cb_sd_zero_avg_local_sd->isChecked () ? "true" : "false";
...
void
US_Hydrodyn_Mals_Saxs_Ciq::set_sd_zero_keep_as_zeros ()
{
  (*parameters)["sd_zero_keep_as_zeros"]
      = cb_sd_zero_keep_as_zeros->isChecked () ? "true" : "false";
...
void
US_Hydrodyn_Mals_Saxs_Ciq::set_sd_zero_set_to_pt1pct ()
{
  (*parameters)["sd_zero_set_to_pt1pct"]
      = cb_sd_zero_set_to_pt1pct->isChecked () ? "true" : "false";
...
void
US_Hydrodyn_Mals_Saxs_Ciq::set_zero_drop_points ()
{
  (*parameters)["zero_drop_points"]
      = cb_zero_drop_points->isChecked () ? "true" : "false";
...
void
US_Hydrodyn_Mals_Saxs_Ciq::set_zero_avg_local_sd ()
{
  (*parameters)["zero_avg_local_sd"]
      = cb_zero_avg_local_sd->isChecked () ? "true" : "false";
...
void
US_Hydrodyn_Mals_Saxs_Ciq::set_zero_keep_as_zeros ()
{
  (*parameters)["zero_keep_as_zeros"]
      = cb_zero_keep_as_zeros->isChecked () ? "true" : "false";
...
void
US_Hydrodyn_Mals_Saxs_Ciq::update_enables ()
{
  bool no_go = false;

  ws_hide (ws_sd_zeros, !cb_sd_source->isChecked ());
  ws_hide (ws_zeros, cb_sd_source->isChecked ()
...
void
US_Hydrodyn_Mals_Saxs_Ciq::ws_hide (vector<QWidget *> ws, bool hide)
{
...
void
US_Hydrodyn_Mals_Saxs_Ciq::zeros_found ()
{
  QMessageBox::information (
      this, windowTitle () + us_tr (": SD Zeros"),
