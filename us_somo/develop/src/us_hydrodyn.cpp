...
// routines us_hydrodyn_info.cpp contains code to report structures for
// debugging us_hydrodyn_util.cpp contains other various code, such as
// disulfide code us_hydrodyn_load.cpp contains code to load files
...
US_Hydrodyn::US_Hydrodyn (vector<QString> batch_file, QString gui_script_file,
                          bool init_configs_silently, QWidget *p, const char *)
    : QFrame (p)
{
  ...
  {
    vector<vector<double> > ys = { { 1 }, { 2 }, { 3 }, { 4 }, { 5 } };
    vector<double> y_avg;
    vector<double> y_sd;
    QString error_msg;
...
#if defined(EIGEN_TEST)
  {
    ...
    */
    ...
    // 1.37, .254, 3.26, .55
    vector<double> y
        = { 5.434, 19.318, 46.322, 89.746, 152.89, 239.054, 351.538 };

    vector<double> e = { 1, 1, 1, 1, 1, 1, 1 };
...
#define TSO QTextStream (stdout)
    ...
    US_Vector::printvector (
        QString ("coefficients EIGEN_SVD_JACOBI chi2 %1").arg (chi2), coeff);
    TSO << eigen.gnuplot_poly (coeff, "g");
    eigen.polyfit (x, y, e, degree, coeff, chi2, EIGEN_SVD_BDC);
    US_Vector::printvector (
        QString ("coefficients EIGEN_SVD_BDC chi2 %1").arg (chi2), coeff);
    TSO << eigen.gnuplot_poly (coeff, "h");
    eigen.polyfit (x, y, e, degree, coeff, chi2, EIGEN_HOUSEHOLDER_QR);
    US_Vector::printvector (
        QString ("coefficients EIGEN_HOUSEHOLDER_QR chi2 %1").arg (chi2),
        coeff);
    TSO << eigen.gnuplot_poly (coeff, "i");
    eigen.polyfit (x, y, e, degree, coeff, chi2,
                   EIGEN_HOUSEHOLDER_QR_PIVOT_COL);
    US_Vector::printvector (
        QString ("coefficients EIGEN_HOUSEHOLDER_QR_PIVOT_COL chi2 %1")
            .arg (chi2),
        coeff);
    ...
    US_Vector::printvector (
        QString ("coefficients EIGEN_HOUSEHOLDER_QR_PIVOT_FULL chi2 %1")
            .arg (chi2),
        coeff);
    TSO << eigen.gnuplot_poly (coeff, "k");
    eigen.polyfit (x, y, e, degree, coeff, chi2, EIGEN_NORMAL);
    US_Vector::printvector (
        QString ("coefficients EIGEN_NORMALC chi2 %1").arg (chi2), coeff);
...
#define TSO QTextStream (stdout)
  US_FFD ffd (5);
  US_Saxs_Util usu;
  if (!usu.select_saxs_file (US_Config::get_home_dir () + "etc" + SLASH
                             + "somo.saxs_atoms"))
    {
      TSO << usu.errormsg << "\n";
    }
...
#if defined(DP_TEST)
  {
    ...
    vector<double> vals
        = { 0, 1, 1.2, 1.23, 1.234, 1.2345, 1.55555, 1e10, -5.2736 };
  ...
  vector<vector<double> > A = { { 1, 2, 3 }, { 4, 5, 6 } } // { { 1, 4 },
                                                           //   { 2, 5 },
                                                           //   { 3, 6 } }
  ;
  ...
  vector<vector<double> > A2x3 = { { 1, 2, 3 }, { 4, 5, 6 } };

  vector<vector<double> > A3x2 = { { 1, 4 }, { 2, 5 }, { 3, 6 } };
  ...
  //    windowText/foreground US_GuiSettings::set_pushbColor( p );
  // }
  ...
  if (numThreads > 1)
    {
      editor->append (
          QString (us_tr ("Multi-threading enabled using %1 threads\n"))
              .arg (numThreads));
    }
  ...
  if (!dir1.exists ())
    {
      editor->append (us_tr ("Warning: Directory ") + somo_dir
                      + us_tr (" does not exist.\n"));
    }
  if (!dir2.exists ())
    {
      editor->append (us_tr ("Warning: Directory ") + somo_pdb_dir
                      + us_tr (" does not exist.\n"));
    }
  if (!dir3.exists ())
    {
      editor->append (us_tr ("Warning: Directory ") + somo_tmp_dir
                      + us_tr (" does not exist.\n"));
    }
  if (!dir4.exists ())
    {
      editor->append (us_tr ("Warning: Directory ") + somo_saxs_dir
                      + us_tr (" does not exist.\n"));
    }
  if (!dir5.exists ())
    {
      editor->append (us_tr ("Warning: Directory ") + somo_saxs_tmp_dir
                      + us_tr (" does not exist.\n"));
    }
  if (!dir6.exists ())
    {
      editor->append (us_tr ("Warning: Directory ") + somo_bd_dir
                      + us_tr (" does not exist.\n"));
    }
  ...
  if (!saxs_util->setup_saxs_maps (saxs_options.default_atom_filename,
                                   saxs_options.default_hybrid_filename,
                                   saxs_options.default_saxs_filename)
      && saxs_options.compute_saxs_coeff_for_bead_models)
    {
      ...
      editor->setTextColor ("red");
      editor->append (
          us_tr ("Warning: Error setting up SAXS structure factor files.\n"
                 "Bead model SAXS disabled.\n"
                 "Check to make sure the files in SOMO->SAXS/SANS "
                 "Options->Miscellaneous are correct.\n"));
      editor->setTextColor (save_color);
  ...
  if (!saxs_util->load_mw_json (USglobal->config_list.system_dir
                                + QDir::separator () + "etc"
  ...
  if (!saxs_util->load_vdw_json (USglobal->config_list.system_dir
                                 + QDir::separator () + "etc"
  ...
  if (!saxs_util->load_vcm_json (USglobal->config_list.system_dir
                                 + QDir::separator () + "etc"
  ...
  if (saxs_options.wavelength == 0)
  ...
  else
    {
      ...
      saxs_options.start_q
          = floor (saxs_options.start_q * SAXS_Q_ROUNDING + 0.5)
            / SAXS_Q_ROUNDING;
      saxs_options.end_q = 4.0 * M_PI
                           * sin (saxs_options.end_angle * M_PI / 360.0)
                           / saxs_options.wavelength;
      saxs_options.end_q = floor (saxs_options.end_q * SAXS_Q_ROUNDING + 0.5)
                           / SAXS_Q_ROUNDING;
      saxs_options.delta_q = 4.0 * M_PI
                             * sin (saxs_options.delta_angle * M_PI / 360.0)
                             / saxs_options.wavelength;
      saxs_options.delta_q
          = floor (saxs_options.delta_q * SAXS_Q_ROUNDING + 0.5)
            / SAXS_Q_ROUNDING;
    }
  ...
  {
    QString demo_dir
        = USglobal->config_list.system_dir + SLASH + "somo" + SLASH + "demo";
    if (QFile (demo_dir).exists ())
      ...
      {
        qDebug () << "Notice: demo dir " << demo_dir
                  << " not added to directory history as it doesn't exist";
      }
  }
...

  {
    ...
    us_qdebug (QString ("holm bonferroni returns %1")
                   .arg (US_Saxs_Util::holm_bonferroni (P, 0.01)));
  }
#endif
...
US_Hydrodyn::setupGUI ()
{
  ...
  lookup_tables->insertItem (us_tr ("Add/Edit &Hybridization"), this,
                             SLOT (hybrid ()));
  lookup_tables->insertItem (us_tr ("Add/Edit &Atom"), this,
                             SLOT (edit_atom ()));
  lookup_tables->insertItem (us_tr ("Add/Edit &Residue"), this,
                             SLOT (residue ()));
  lookup_tables->insertItem (us_tr ("Add/Edit &SAXS coefficients"), this,
                             SLOT (do_saxs ()));

  //   somo_options = new Q3PopupMenu;
  somo_options->insertItem (us_tr ("&ASA Calculation"), this,
                            SLOT (show_asa ()));
  somo_options->insertItem (us_tr ("&SoMo Overlap Reduction"), this,
                            SLOT (show_overlap ()));
  somo_options->insertItem (us_tr ("AtoB (Grid) &Overlap Reduction"), this,
                            SLOT (show_grid_overlap ()));
  somo_options->insertItem (us_tr ("&vdW Overlap Parameters"), this,
                            SLOT (show_vdw_overlap ()));
  somo_options->insertItem (us_tr ("&Hydrodynamic Calculations"), this,
                            SLOT (show_hydro ()));
  somo_options->insertItem (us_tr ("Hydrodynamic Calculations &Zeno"), this,
                            SLOT (show_zeno_options ()));
  somo_options->insertItem (us_tr ("&Miscellaneous Options"), this,
                            SLOT (show_misc ()));
  somo_options->insertItem (us_tr ("&Bead Model Output"), this,
                            SLOT (show_bead_output ()));
  somo_options->insertItem (us_tr ("&Grid Functions (AtoB)"), this,
                            SLOT (show_grid ()));
  somo_options->insertItem (us_tr ("SA&XS/SANS Options"), this,
                            SLOT (show_saxs_options ()));
  somo_options->insertItem (us_tr ("&Fractal Dimension Options"), this,
                            SLOT (show_fractal_dimension_options ()));

  //   md_options = new Q3PopupMenu;
  // md_options->insertItem(us_tr("&DMD Options"), this,
  // SLOT(show_dmd_options()));
  md_options->insertItem (us_tr ("&Browflex Options"), this,
                          SLOT (show_bd_options ()));
  md_options->insertItem (us_tr ("&Anaflex Options"), this,
                          SLOT (show_anaflex_options ()));

  //   pdb_options = new Q3PopupMenu;
  pdb_options->insertItem (us_tr ("&Parsing"), this, SLOT (pdb_parsing ()));
  pdb_options->insertItem (us_tr ("&Visualization"), this,
                           SLOT (pdb_visualization ()));

  //   configuration = new Q3PopupMenu;
  configuration->insertItem (us_tr ("&Load Configuration"), this,
                             SLOT (load_config ()));
  configuration->insertItem (us_tr ("&Save Current Configuration"), this,
                             SLOT (write_config ()));
  configuration->insertItem (us_tr ("&Reset to Default Configuration"), this,
                             SLOT (reset ()));
  configuration->insertItem (us_tr ("&Advanced Configuration"), this,
                             SLOT (show_advanced_config ()));
  configuration->insertItem (us_tr ("S&ystem Configuration"), this,
                             SLOT (run_us_config ()));
  // configuration->insertItem(us_tr("A&dministrator"), this,
  // SLOT(run_us_admin()));
  ...
  }
  {
    ...
    {
      QAction *qa
          = submenu->addAction (us_tr ("AtoB (Grid) &Overlap Reduction"));
    ...
    {
      QAction *qa
          = submenu->addAction (us_tr ("Hydrodynamic Calculations &Zeno"));
    ...
    }
    {
      ...
      connect (qa, SIGNAL (triggered ()), this,
               SLOT (show_fractal_dimension_options ()));
    }
    menu->addMenu (submenu);
  }
  ...
  }
  {
    ...
    {
      QAction *qa
          = submenu->addAction (us_tr ("&Reset to Default Configuration"));
  ...
  AUTFBACK (lbl_info1);
  lbl_info1->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  AUTFBACK (lbl_info2);
  lbl_info2->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize, QFont::Bold));
  ...
  AUTFBACK (lbl_info3);
  lbl_info3->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize, QFont::Bold));

  pb_select_residue_file
      = new QPushButton (us_tr ("Select Lookup Table"), this);
  Q_CHECK_PTR (pb_select_residue_file);
  pb_select_residue_file->setMinimumHeight (minHeight1);
  pb_select_residue_file->setFont (QFont (USglobal->config_list.fontFamily,
                                          USglobal->config_list.fontSize));
  pb_select_residue_file->setPalette (PALET_PUSHB);
  connect (pb_select_residue_file, SIGNAL (clicked ()),
           SLOT (select_residue_file ()));
  ...
  pb_view_bead_model->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize));
  pb_view_bead_model->setPalette (PALET_PUSHB);
  connect (pb_view_bead_model, SIGNAL (clicked ()), SLOT (view_bead_model ()));

  lbl_temperature = new QLabel (
      us_tr (QString (" Temp. (%1C): ").arg (DEGREE_SYMBOL)), this);
  ...
  lbl_temperature->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize - 1,
                                   QFont::Bold));

  le_temperature = new QLineEdit (this);
  le_temperature->setObjectName ("Temperature Line Edit");
  ...
  le_temperature->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize));
  le_temperature->setValidator (
      new QDoubleValidator (-273.15, 200, 2, le_temperature));
  ...
  cb_pH->setFont (QFont (USglobal->config_list.fontFamily,
                         USglobal->config_list.fontSize));
  cb_pH->setPalette (PALET_NORMAL);
  AUTFBACK (cb_pH);
  connect (cb_pH, SIGNAL (clicked ()), this, SLOT (set_pH ()));
  cb_pH->setToolTip (
      us_tr ("<html><body> enable pH dependent ionization and psv. You will "
             "need to reload the PDB if the pH is adjusted.</body></html>"));
  // eventually cb_pH->hide();

  le_pH = new QLineEdit (this);
  le_pH->setObjectName ("PH Line Edit");
  le_pH->setText (QString ("").sprintf ("%4.2f", hydro.pH));
  le_pH->setAlignment (Qt::AlignVCenter);
  le_pH->setPalette (PALET_NORMAL);
  AUTFBACK (le_pH);
  le_pH->setFont (QFont (USglobal->config_list.fontFamily,
                         USglobal->config_list.fontSize));
  le_pH->setValidator (new QDoubleValidator (0.01, 14, 2, le_pH));
  le_pH->setEnabled (cb_pH->isChecked ());
  connect (le_pH, SIGNAL (textChanged (const QString &)),
           SLOT (update_pH (const QString &)));
  le_pH->setToolTip (us_tr (
      "<html><body>enter the pH for ionization and psv calculations. You will "
      "need to reload the PDB if the pH is adjusted.</body></html>"));
  ...
  pb_load_pdb->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  ...
  pb_reload_pdb->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize));
  pb_reload_pdb->setPalette (PALET_PUSHB);
  connect (pb_reload_pdb, SIGNAL (clicked ()), SLOT (reload_pdb ()));
  pb_reload_pdb->setToolTip (
      us_tr ("<html><body>Use this to reload the PDB, primarily useful when "
             "the pH has been adjusted.</body></html>"));
  ...
  le_pdb_file->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  connect (le_pdb_file, SIGNAL (focussed (bool)),
           SLOT (le_pdb_file_focus (bool)));
  connect (le_pdb_file, SIGNAL (textChanged (const QString &)),
           SLOT (le_pdb_file_changed (const QString &)));
  ...
  AUTFBACK (lbl_model);
  lbl_model->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  pb_view_pdb->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  ...
  pb_pdb_tool->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  ...
  lb_model->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize));
  ...
  // connect(lb_model, SIGNAL(currentRowChanged(int)), this,
  // SLOT(select_model(int)));
  connect (lb_model, SIGNAL (itemSelectionChanged ()), this,
           SLOT (model_selection_changed ()));

  pb_load_bead_model
      = new QPushButton (us_tr ("Load Single Bead Model File"), this);
  Q_CHECK_PTR (pb_load_bead_model);
  pb_load_bead_model->setMinimumHeight (minHeight1);
  pb_load_bead_model->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize));
  pb_load_bead_model->setPalette (PALET_PUSHB);
  connect (pb_load_bead_model, SIGNAL (clicked ()), SLOT (load_bead_model ()));

  le_bead_model_file = new QLineEdit (this);
  le_bead_model_file->setObjectName ("bead_model_file Line Edit");
  ...
  le_bead_model_file->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize));
  connect (le_bead_model_file, SIGNAL (textChanged (const QString &)),
           SLOT (update_bead_model_file (const QString &)));
  ...
  lbl_bead_model_prefix->setFont (QFont (USglobal->config_list.fontFamily,
                                         USglobal->config_list.fontSize - 1,
                                         QFont::Bold));
  ...
  cb_setSuffix->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  cb_overwrite->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  cb_overwrite->setPalette (PALET_NORMAL);
  AUTFBACK (cb_overwrite);
  connect (cb_overwrite, SIGNAL (clicked ()), this, SLOT (set_overwrite ()));

  le_bead_model_prefix = new QLineEdit (this);
  le_bead_model_prefix->setObjectName ("bead_model_prefix Line Edit");
  ...
  le_bead_model_prefix->setFont (QFont (USglobal->config_list.fontFamily,
                                        USglobal->config_list.fontSize));
  connect (le_bead_model_prefix, SIGNAL (textChanged (const QString &)),
           SLOT (update_bead_model_prefix (const QString &)));

  le_bead_model_suffix = new QTextEdit (this);
  le_bead_model_suffix->setObjectName ("bead_model_suffix Line Edit");
  ...
  le_bead_model_suffix->setFont (QFont (USglobal->config_list.fontFamily,
                                        USglobal->config_list.fontSize));
  ...
  pb_somo->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize));
  ...
  pb_somo_o->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize, QFont::Bold));
  ...
  pb_pdb_hydrate_for_saxs->setFont (QFont (USglobal->config_list.fontFamily,
                                           USglobal->config_list.fontSize));
  pb_pdb_hydrate_for_saxs->setEnabled (true);
  pb_pdb_hydrate_for_saxs->setPalette (PALET_PUSHB);
  connect (pb_pdb_hydrate_for_saxs, SIGNAL (clicked ()),
           SLOT (pdb_hydrate_for_saxs ()));
#endif
  ...
  pb_pdb_saxs->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  ...
  pb_grid_pdb->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  pb_grid_pdb->setEnabled (false);
  pb_grid_pdb->setPalette (PALET_PUSHB);
  connect (pb_grid_pdb, SIGNAL (clicked ()), SLOT (calc_grid_pdb ()));

  // pb_grid_pdb_o = new QPushButton(us_tr("Build AtoB (Grid) Overlap Bead
  // Model"), this); pb_grid_pdb_o->setMinimumHeight(minHeight1);
  // pb_grid_pdb_o->setFont(QFont( USglobal->config_list.fontFamily,
  // USglobal->config_list.fontSize)); pb_grid_pdb_o->setEnabled(false);
  // pb_grid_pdb_o->setPalette( PALET_PUSHB );
  // connect(pb_grid_pdb_o, SIGNAL(clicked()), SLOT(calc_grid_pdb_o()));

  pb_vdw_beads
      = new QPushButton (us_tr ("Build vdW Overlap Bead Model"), this);
  pb_vdw_beads->setMinimumHeight (minHeight1);
  pb_vdw_beads->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  pb_grid->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize));
  ...
  cb_calcAutoHydro->setText (
      us_tr (" Automatically Calculate Hydrodynamics "));
  cb_calcAutoHydro->setChecked (calcAutoHydro);
  cb_calcAutoHydro->setFont (QFont (USglobal->config_list.fontFamily,
                                    USglobal->config_list.fontSize));
  cb_calcAutoHydro->setPalette (PALET_NORMAL);
  AUTFBACK (cb_calcAutoHydro);
  connect (cb_calcAutoHydro, SIGNAL (clicked ()), this,
           SLOT (set_calcAutoHydro ()));

  pb_view_asa = new QPushButton (us_tr ("View ASA Results"), this);
  Q_CHECK_PTR (pb_view_asa);
  pb_view_asa->setMinimumHeight (minHeight1);
  pb_view_asa->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  ...
  pb_visualize->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  pb_batch2->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize));
  ...
  AUTFBACK (lbl_rbh);
  lbl_rbh->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize - 1, QFont::Bold));
  ...
  pb_calc_hydro->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize));
  ...
  pb_calc_zeno->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  pb_calc_grpy->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  pb_calc_hullrad->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize));
  pb_calc_hullrad->setPalette (PALET_PUSHB);
  connect (pb_calc_hullrad, SIGNAL (clicked ()), SLOT (calc_hullrad_hydro ()));
  pb_calc_hullrad->hide ();

  pb_show_hydro_results
      = new QPushButton (us_tr ("Show Hydrodynamic Calculations"), this);
  Q_CHECK_PTR (pb_show_hydro_results);
  pb_show_hydro_results->setMinimumHeight (minHeight1);
  pb_show_hydro_results->setEnabled (false);
  pb_show_hydro_results->setFont (QFont (USglobal->config_list.fontFamily,
                                         USglobal->config_list.fontSize));
  pb_show_hydro_results->setPalette (PALET_PUSHB);
  connect (pb_show_hydro_results, SIGNAL (clicked ()),
           SLOT (show_hydro_results ()));

  pb_comparative = new QPushButton (us_tr ("Model classifier"), this);
  pb_comparative->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize));
  ...
  pb_best->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize));
  ...
  pb_open_hydro_results
      = new QPushButton (us_tr ("Open Hydrodynamic Calculations File"), this);
  Q_CHECK_PTR (pb_open_hydro_results);
  pb_open_hydro_results->setMinimumHeight (minHeight1);
  pb_open_hydro_results->setEnabled (true);
  pb_open_hydro_results->setFont (QFont (USglobal->config_list.fontFamily,
                                         USglobal->config_list.fontSize));
  pb_open_hydro_results->setPalette (PALET_PUSHB);
  connect (pb_open_hydro_results, SIGNAL (clicked ()),
           SLOT (open_hydro_results ()));

  pb_select_save_params
      = new QPushButton (us_tr ("Select Parameters to be Saved"), this);
  Q_CHECK_PTR (pb_select_save_params);
  pb_select_save_params->setMinimumHeight (minHeight1);
  pb_select_save_params->setFont (QFont (USglobal->config_list.fontFamily,
                                         USglobal->config_list.fontSize));
  pb_select_save_params->setEnabled (true);
  pb_select_save_params->setPalette (PALET_PUSHB);
  connect (pb_select_save_params, SIGNAL (clicked ()),
           SLOT (select_save_params ()));

  cb_saveParams = new QCheckBox (this);
  cb_saveParams->setText (us_tr (" Save parameters to file "));
  cb_saveParams->setChecked (saveParams);
  cb_saveParams->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize));
  ...
  pb_dmd_run->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize));
  ...
  pb_bd->setFont (QFont (USglobal->config_list.fontFamily,
                         USglobal->config_list.fontSize));
#ifdef NO_BD
  ...
  pb_fractal_dimension->setFont (QFont (USglobal->config_list.fontFamily,
                                        USglobal->config_list.fontSize));
  pb_fractal_dimension->setPalette (PALET_PUSHB);
  connect (pb_fractal_dimension, SIGNAL (clicked ()),
           SLOT (fractal_dimension ()));
  ...
  //   USglobal->config_list.fontSize)); pb_bd_prepare->setEnabled(false);
  ...
  //   USglobal->config_list.fontSize)); pb_bd_load->setEnabled(false);
  ...
  //   USglobal->config_list.fontSize)); pb_bd_edit->setEnabled(false);
  ...
  //   USglobal->config_list.fontSize)); pb_bd_run->setEnabled(false);
  //   pb_bd_run->setPalette( PALET_PUSHB );
  //   connect(pb_bd_run, SIGNAL(clicked()), SLOT(bd_run()));

  //   pb_bd_load_results = new QPushButton(us_tr("Load/Process Browflex
  //   results"), this); pb_bd_load_results->setMinimumHeight(minHeight1);
  //   pb_bd_load_results->setFont(QFont( USglobal->config_list.fontFamily,
  //   USglobal->config_list.fontSize)); pb_bd_load_results->setEnabled(false);
  //   pb_bd_load_results->setPalette( PALET_PUSHB );
  //   connect(pb_bd_load_results, SIGNAL(clicked()), SLOT(bd_load_results()));

  // ***** anaflex *******
  //   pb_anaflex_prepare = new QPushButton(us_tr("Create Anaflex files"),
  //   this); pb_anaflex_prepare->setMinimumHeight(minHeight1);
  //   pb_anaflex_prepare->setFont(QFont( USglobal->config_list.fontFamily,
  //   USglobal->config_list.fontSize)); pb_anaflex_prepare->setEnabled(false);
  ...
  //   USglobal->config_list.fontSize)); pb_anaflex_load->setEnabled(false);
  //   pb_anaflex_load->setPalette( PALET_PUSHB );
  //   connect(pb_anaflex_load, SIGNAL(clicked()), SLOT(anaflex_load()));

  //   pb_anaflex_edit = new QPushButton(us_tr("View/Edit Anaflex files"),
  //   this); pb_anaflex_edit->setMinimumHeight(minHeight1);
  //   pb_anaflex_edit->setFont(QFont( USglobal->config_list.fontFamily,
  //   USglobal->config_list.fontSize)); pb_anaflex_edit->setEnabled(false);
  ...
  //   USglobal->config_list.fontSize)); pb_anaflex_run->setEnabled(false);
  //   pb_anaflex_run->setPalette( PALET_PUSHB );
  //   connect(pb_anaflex_run, SIGNAL(clicked()), SLOT(anaflex_run()));

  //   pb_anaflex_load_results = new QPushButton(us_tr("Load Anaflex results"),
  //   this); pb_anaflex_load_results->setMinimumHeight(minHeight1);
  //   pb_anaflex_load_results->setFont(QFont(
  ...
  pb_config->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize));
  ...
  pb_stop_calc->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  editor->setWordWrapMode (advanced_config.scroll_editor
                               ? QTextOption::NoWrap
  ...
  j++;
  QHBoxLayout *hbl_pdb = new QHBoxLayout ();
  hbl_pdb->setContentsMargins (0, 0, 0, 0);
  hbl_pdb->setSpacing (0);
  ...
  hbl_pdb_saxs->setContentsMargins (0, 0, 0, 0);
  hbl_pdb_saxs->setSpacing (0);
  ...
  hbl_prefix_suffix->setContentsMargins (0, 0, 0, 0);
  hbl_prefix_suffix->setSpacing (0);
  ...
  // QGridLayout * gl_prefix_suffix = new QGridLayout;
  // gl_prefix_suffix->setContentsMargins( 0, 0, 0, 0 );
  // gl_prefix_suffix->setSpacing( 0 ); gl_prefix_suffix->addWidget(
  // le_bead_model_prefix , 0 , 0 , 1 + ( 0 ) - ( 0 ) , 1 + ( 0 ) - ( 0 ) );
  // gl_prefix_suffix->addWidget( le_bead_model_suffix , 0 , 1 , 1 + ( 0 ) - (
  // 0 ) , 1 + ( 2 ) - ( 1 ) ); background->addLayout(gl_prefix_suffix, j, 1);
  j++;
  ...
  //   QHBoxLayout * qhl_bd_1 = new QHBoxLayout; qhl_bd_1->setContentsMargins(
  //   0, 0, 0, 0 ); qhl_bd_1->setSpacing( 0 );
  ...
  bl_help_config->setContentsMargins (0, 0, 0, 0);
  bl_help_config->setSpacing (0);
  bl_help_config->addWidget (pb_help);
  bl_help_config->addWidget (pb_config);

  background->addLayout (bl_help_config, j, 0);
  {
    QBoxLayout *bl_progress = new QHBoxLayout;
    bl_progress->setContentsMargins (0, 0, 0, 0);
    bl_progress->setSpacing (0);
  ...
  connect (this, SIGNAL (progress_updated (int, int)),
           SLOT (update_progress (int, int)));
}

void
US_Hydrodyn::set_expert (bool expert)
{
  if (expert)
    {
      //      pb_best->show();
#if QT_VERSION < 0x040000
      lookup_tables->insertItem (us_tr ("Make test set"), this,
                                 SLOT (make_test_set ()));
#endif
    }
...
void
US_Hydrodyn::set_disabled (bool clear_bead_model_file)
{
  pb_somo->setEnabled (false);
  pb_somo_o->setEnabled (false);
  pb_grid_pdb->setEnabled (false);
...
void
US_Hydrodyn::set_enabled ()
{
  qDebug () << "set_enabled() bead model from file : "
            << (bead_model_from_file ? "true" : "false");
  ...
  if (overlap_widget)
  ...
  else
    {
      overlap_window = new US_Hydrodyn_Overlap (
          &sidechain_overlap, &mainchain_overlap, &buried_overlap,
          &replicate_o_r_method_somo, &grid_exposed_overlap,
          &grid_buried_overlap, &grid_overlap, &replicate_o_r_method_grid,
          &overlap_tolerance, &overlap_widget, this);
...
US_Hydrodyn::show_grid_overlap ()
{
  if (grid_overlap_widget)
    ...
    {
      grid_overlap_window = new US_Hydrodyn_Overlap (
          &grid_exposed_overlap, &grid_buried_overlap, &grid_overlap,
          &replicate_o_r_method_grid, &overlap_tolerance, &grid_overlap_widget,
          this);
...
US_Hydrodyn::show_misc ()
{
  if (misc_widget)
  ...
  else
    {
      ...
      connect (misc_window, SIGNAL (vbar_changed ()), this,
               SLOT (update_vbar ()));
      misc_window->show ();
    }
  update_vbar ();
...
US_Hydrodyn::show_vdw_overlap ()
{
  if (vdw_overlap_widget)
    ...
    {
      vdw_overlap_window
          = new US_Hydrodyn_Vdw_Overlap (&misc, &vdw_overlap_widget, this);
...
US_Hydrodyn::show_saxs_options ()
{
  if (saxs_options_widget)
    ...
    {
      saxs_options_window = new US_Hydrodyn_SaxsOptions (
          &saxs_options, &saxs_options_widget, &sas_options_saxs_widget,
          &sas_options_sans_widget, &sas_options_curve_widget,
          &sas_options_bead_model_widget, &sas_options_hydration_widget,
          &sas_options_guinier_widget, &sas_options_xsr_widget,
          &sas_options_misc_widget, &sas_options_experimental_widget, this);
...
US_Hydrodyn::show_dmd_options ()
{
  if (dmd_options_widget)
    ...
    {
      dmd_options_window = new US_Hydrodyn_DMD_Options (
          &dmd_options, &dmd_options_widget, this);
...
US_Hydrodyn::show_bd ()
{
  ...
  if (bd_widget)
    ...
    {
      bd_window = new US_Hydrodyn_BD (&bd_widget, &bd_options,
                                      &anaflex_options, this);
...
US_Hydrodyn::show_bd_options ()
{
  ...
  if (bd_options_widget)
    ...
    {
      bd_options_window
          = new US_Hydrodyn_BD_Options (&bd_options, &bd_options_widget, this);
...
void
US_Hydrodyn::show_anaflex_options ()
{
#ifdef NO_BD
  US_Static::us_message (us_tr ("Please note:"),
                         us_tr ("Function not available in this version."));
  ...
  if (anaflex_options_widget)
    ...
    {
      anaflex_options_window = new US_Hydrodyn_Anaflex_Options (
          &anaflex_options, &anaflex_options_widget, this);
...
US_Hydrodyn::show_bead_output ()
{
  if (bead_output_widget)
    ...
    {
      bead_output_window = new US_Hydrodyn_Bead_Output (
          &bead_output, &bead_output_widget, this);
...
US_Hydrodyn::show_grid ()
{
  if (grid_widget)
    ...
    {
      grid_window = new US_Hydrodyn_Grid (
          &grid_exposed_overlap, &grid_buried_overlap, &grid_overlap,
          &replicate_o_r_method_grid, &grid, &overlap_tolerance, &grid_widget,
          this);
...
US_Hydrodyn::pdb_parsing ()
{
  if (pdb_parsing_widget)
    ...
    {
      pdb_parsing_window = new US_Hydrodyn_PDB_Parsing (
          &pdb_parse, &pdb_parsing_widget, this);
...
US_Hydrodyn::pdb_visualization ()
{
  if (pdb_visualization_widget)
    ...
    {
      pdb_visualization_window = new US_Hydrodyn_PDB_Visualization (
          &pdb_vis, &pdb_visualization_widget);
...
void
US_Hydrodyn::load_config ()
{
  QString fname = QFileDialog::getOpenFileName (
      0, "Please select a SOMO configuration file...",
      US_Config::get_home_dir () + "/etc", "*.config", 0);
  if (fname == QString ())
    {
      QColor save_color = editor->textColor ();
      editor->setTextColor ("red");
...
void
US_Hydrodyn::write_config ()
{
  QString fname = US_Config::get_home_dir () + "etc/somo.config";
  switch (QMessageBox::question (
      this, windowTitle () + us_tr (": Save configuration "),
    ...
    case QMessageBox::No:
      fname = QFileDialog::getSaveFileName (
          0, "Please name your SOMO configuration file...",
          US_Config::get_home_dir () + "etc", "*.config", 0);
      break;
    default:
      return;
...
US_Hydrodyn::reset ()
{
  QMessageBox mb (
      ...
      QMessageBox::Cancel | QMessageBox::Escape, QMessageBox::No);
...
void
US_Hydrodyn::select_residue_file ()
{
  QString old_filename = residue_filename;
  residue_filename = QFileDialog::getOpenFileName (
...
void
US_Hydrodyn::reload_pdb ()
{
  // {
  //    qDebug() << "test write pdb";
  //    QString errors;
  //    write_pdb_from_model( model_vector[current_model], errors, "my header
  //    note", "my_suffex", "outputfile" );
  // }
  citation_load_pdb ();
  ...
  if (pdb_file
          .isEmpty ()) // why did we ever do this ? || !pb_somo->isEnabled() )
    {
      return;
    }
  editor->append ("\nReloading PDB file.\n");
...
void
US_Hydrodyn::clear_pdb_info (QString /* msg */)
{
...
void
US_Hydrodyn::set_pdb_info (QString /* msg */)
{
  if (misc.pb_rule_on)
    {
    ...
    {
      // us_qdebug( QString( "set_pdb_info() %1 setting pb_rule OFF" ). arg(
      // msg ) );
...
void
US_Hydrodyn::sync_pdb_info (QString /* msg */)
{
  // us_qdebug( QString( "sync_pdb_info() %1 called misc.pb_rule_on %2
  // pdb_info.count( 'pb_rule' ) %3" )
  //         .arg( msg )
  ...
  if ((misc.pb_rule_on && !pdb_info.count ("pb_rule"))
      || (!misc.pb_rule_on && pdb_info.count ("pb_rule")))
    {
      ...
      if (misc.pb_rule_on && !pdb_info.count ("pb_rule"))
        {
          // us_qdebug( QString( "sync_pdb_info() %1 misc.pb_rule was ON, now
          // OFF" ). arg( msg ) );
        ...
        {
          // us_qdebug( QString( "sync_pdb_info() %1 misc.pb_rule was OFF, now
          // ON" ). arg( msg ) );
          misc.pb_rule_on = true;
          misc.restore_pb_rule = false;
        }
...
int
US_Hydrodyn::issue_non_coded (bool quiet)
{
    ...
    case 1:
      {
        ...
        switch (QMessageBox::question (
            this, us_tr ("US-SOMO: Non coded residue"),
            us_tr ("A non-coded residue was encountered\n"
                   "It can be skipped or you can stop processing.\n"
                   "This choice will be maintained for all remaining non "
                   "coded residues encountered in this PDB file.\n"
                   "For specific residue details, please review the main "
                   "panel text area after making your selection.\n"
                   "\n"
                   "If your model contains non-coded residues, the calculated "
                   "molecular weight\n"
                   "and vbar may be incorrect. Therefore, you could manually "
                   "enter a global\n"
                   "value for the molecular weight in the SOMO hydrodynamic "
                   "options, and a\n"
                   "global value for the vbar in the SOMO Miscellaneous "
                   "options.\n"
                   "\n"
                   "Please select your option below"),
            us_tr ("Skip non-coded residues"), us_tr ("Stop processing"),
            QString (), 1, 1))
          {
          case 0:
    ...
    case 2:
      {
        ...
        switch (QMessageBox::question (
            this, us_tr ("US-SOMO: Non coded residue"),
            us_tr ("A non-coded residue was encountered\n"
                   "It can be replaced with an average residue, skipped or "
                   "you can stop processing.\n"
                   "This choice will be maintained for all remaining non "
                   "coded residues encountered in this PDB file.\n"
                   "For specific residue details, please review the main "
                   "panel text area after making your selection.\n"
                   ...
                   "review the average residue settings in the SOMO "
                   "Miscellaneous options.\n"
                   "\n"
                   "Please select your option below"),
            us_tr ("Replace non-coded with average residues"),
            us_tr ("Skip non-coded residues"), us_tr ("Stop processing"), 2,
            2))
          {
          case 0:
...
int
US_Hydrodyn::issue_missing_atom (bool quiet)
{
    ...
    case 1:
      {
        ...
        switch (QMessageBox::question (
            this, us_tr ("US-SOMO: Missing or extra atoms"),
            us_tr ("A missing or extra atom was encountered when comparing "
                   "with the residue table\n"
                   "It can be skipped or you can stop processing.\n"
                   "This choice will be maintained for all remaining missing "
                   "atoms encountered in this PDB file.\n"
                   "For specific residue details, please review the main "
                   "panel text area after making your selection.\n"
                   "\n"
                   "If your model contains missing atoms, the calculated "
                   "molecular\n"
                   "weight and vbar may be incorrect, and you should manually "
                   "enter\n"
                   "a global value for the molecular weight in the SOMO "
                   "hydrodynamic\n"
                   "options, and a global value for the vbar in the SOMO "
                   "Miscellaneous\n"
                   "options.\n"
                   "\n"
                   "Please select your option below"),
            us_tr ("Skip residues with missing atoms"),
            us_tr ("Stop processing"), QString (), 1, 1))
          {
          case 0:
    ...
    case 2:
      {
        ...
        switch (QMessageBox::question (
            ...
            us_tr ("A missing or extra atom was encountered when comparing "
                   "with the residue table\n"
                   "It can be modeled with an approximate method, skipped or "
                   "you can stop processing.\n"
                   "This choice will be maintained for all remaining missing "
                   "atoms encountered in this PDB file.\n"
                   "For specific residue details, please review the main "
                   "panel text area after making your selection.\n"
                   "\n"
                   "If you model with an approximate method, the shape of "
                   "this residue will\n"
                   "be different from an exact residue which can effect the "
...
US_Hydrodyn::load_pdb ()
{
  ...
  QString message = "";
  if (0 && pdb_parse.missing_residues == 1 && !advanced_config.expert_mode)
    {
      message += us_tr (
          "You have selected to skip missing residues. If your model contains "
          "missing\n"
          "residues, the calculated molecular weight and vbar may be "
          "incorrect, and\n"
          "you should manually enter a global value for the molecular weight "
          "in the\n"
          "SOMO hydrodynamic options, and a global value for the vbar in the "
          "SOMO\n"
          "Miscellaneous options.\n\nAre you sure you want to proceed?");
    }
  if (0 && pdb_parse.missing_residues == 2 && !advanced_config.expert_mode)
    {
      message += us_tr ("You have selected to replace non-coded residues with "
                        "an average residue.\n"
                        "If your model contains non-coded residues, the "
                        "calculated molecular weight\n"
                        "and vbar may be incorrect. Therefore, you could "
                        "manually enter a global\n"
                        "value for the molecular weight in the SOMO "
                        "hydrodynamic options, and a\n"
                        "global value for the vbar in the SOMO Miscellaneous "
                        "options. You can also\n"
                        "review the average residue settings in the SOMO "
                        "Miscellaneous options.\n\n"
  ...
  message = "";
  if (0 && pdb_parse.missing_atoms == 1 && !advanced_config.expert_mode)
    {
      message += us_tr (
          "You have selected to skip coded residues containing missing "
          "atoms.\n"
  ...
  if (0 && pdb_parse.missing_atoms == 2 && !advanced_config.expert_mode)
    {
      message += us_tr (
          "You have selected to model coded residues with missing atoms\n"
          "with an approximate method.  For best results, you should "
          "complete\n"
          "the structure.\n\n"
  ...
  QString use_dir = path_load_pdb.isEmpty () ? somo_pdb_dir : path_load_pdb;

  select_from_directory_history (use_dir, this);

  QString filename
      = QFileDialog::getOpenFileName (this, "Please select a PDB file...",
                                      use_dir, "Structures (*.pdb *.PDB)");
  ...
  if (QFileInfo (filename).fileName ().contains (" "))
    {
      printError (
          us_tr ("Filenames containing spaces are not currently supported.\n"
      ...
      for (unsigned int i = 0; i < model_vector.size (); i++)
        {
          ...
          if (check_for_missing_atoms (&error_string, &model_vector[i]))
            {
              errors_found++;
              editor->append (QString ("Encountered errors with your PDB "
                                       "structure for model %1:\n")
                                  .arg (model_name (i))
                              + error_string);
              printError (QString ("Encountered errors with your PDB "
                                   "structure for model %1:\n")
                              .arg (model_name (i))
                          + "please check the text window");
            }
  ...
  if (advanced_config.auto_calc_somo && !errors_found)
    {
      calc_somo ();
    }
}
...
bool
US_Hydrodyn::screen_pdb (QString filename, bool display_pdb,
                         bool skipclearissue)
{
  if (!skipclearissue)
    {
  ...
  if (batch_widget && batch_window->batch_job_running
      && model_vector_has_hydration_differences (model_vector))
    {
      batch_window->editor_msg (
          "darkred",
          QString (
              us_tr ("Screening: %1 : WARNING: PDB contains residues with "
                     "bead hydration without atomic hydration,\nvdW models "
                     "should not be used as they rely on atomic hydration\n"))
              .arg (filename));
    }
...
bool
US_Hydrodyn::screen_bead_model (QString filename)
{
  if (QFileInfo (filename).fileName ().contains (" "))
    {
      printError (
  ...
  if (!read_bead_model (filename, only_overlap))
    {
      ...
      bool so_ovlp
          = QFileInfo (filename).completeBaseName ().contains ("so_ovlp");
      us_qdebug (QString ("screen bead model so_ovlp %1")
                     .arg (so_ovlp ? "true" : "false"));
...
void
US_Hydrodyn::view_pdb ()
{

  QString use_dir = path_view_pdb.isEmpty () ? somo_pdb_dir : path_view_pdb;
...
QString
US_Hydrodyn::model_name (int val)
{
  return QString ("%1")
      .arg (lb_model->item (val)->text ())
      .replace ("Model: ", "");
}

void
US_Hydrodyn::model_selection_changed ()
{
  select_model ();
...
US_Hydrodyn::select_model (int)
{
  ...
  // check integrity of PDB file and confirm that all residues are correctly
  // defined in residue table
  if (results_widget)
    {
      results_window->close ();
      delete results_window;
...
void
US_Hydrodyn::write_bead_ebf (QString fname, vector<PDB_atom> *model)
{
  {
    ...
    for (unsigned int i = 0; i < model->size (); i++)
      {
        if ((*model)[i].active)
          {
            fwrite (&((*model)[i].bead_cog_coordinate.axis[0]), sizeof (float),
                    3, f);
            float color[3]
                = { .2f, .2f,
                    ((int)(*model)[i].all_beads.size () / 5) > 1
                        ? 1
                        : ((float)(*model)[i].all_beads.size () / 5.0f) };
  ...
  {
    FILE *f
        = us_fopen (QString ("%1-info").arg (fname).toLatin1 ().data (), "w");
    for (unsigned int i = 0; i < model->size (); i++)
      {
        if ((*model)[i].active)
          {
            float color[3]
                = { .2f, .2f,
                    ((*model)[i].all_beads.size () / 5) > 1
                        ? 1
                        : ((float)(*model)[i].all_beads.size () / 5.0f) };
            fprintf (f, "%f %f %f %f %f %f %f\n",
                     (*model)[i].bead_coordinate.axis[0],
                     (*model)[i].bead_coordinate.axis[1],
                     (*model)[i].bead_coordinate.axis[2], color[0], color[1],
                     color[2], (*model)[i].bead_computed_radius);
          }
      }
    fclose (f);
  }
...
void
US_Hydrodyn::update_bead_model_file (const QString &str)
{
  bead_model_file = str;
}

void
US_Hydrodyn::update_bead_model_prefix (const QString &str)
{
  bead_model_prefix = str;
}

void
US_Hydrodyn::update_temperature (const QString &str, bool update_hydro)
{
...
void
US_Hydrodyn::update_pH (const QString &str)
{
  ...
  //    ((US_Hydrodyn_Saxs_Hplc_Options*)saxs_hplc_window->saxs_hplc_options_widget)->le_fasta_pH->setText(
  //    QString( "" ).sprintf( "%4.2f", hydro.pH ) );
  // }
}
...
void
US_Hydrodyn::set_calcAutoHydro ()
{
  calcAutoHydro = cb_calcAutoHydro->isChecked ();
}

void
US_Hydrodyn::set_setSuffix ()
{
...
void
US_Hydrodyn::set_overwrite ()
{
  overwrite = cb_overwrite->isChecked ();
}

void
US_Hydrodyn::set_saveParams ()
{
...
void
US_Hydrodyn::view_asa ()
{
  QString use_dir
      = path_view_asa_res.isEmpty () ? somo_dir : path_view_asa_res;

  select_from_directory_history (use_dir, this);

  QString filename = QFileDialog::getOpenFileName (
      this, windowTitle (), use_dir, "*.asa_res *.ASA_RES");
  if (!filename.isEmpty ())
    {
  ...
  if (last_bead_model != "")
    {
      ...
      switch (QMessageBox::question (
          this, us_tr ("View Bead Model File"),
      ...
      if (last_read_bead_model != "")
        {
          ...
          switch (QMessageBox::question (
            ...
            case QMessageBox::No:
              {
                QString use_dir = path_view_bead_model.isEmpty ()
                                      ? somo_dir
...
void
US_Hydrodyn::visualize (bool movie_frame, QString dir, float scale,
                  ...
                  if (US_Saxs_Util::pat_model (tmp_model, true))
                    {
                      write_bead_spt (use_dir + SLASH + spt_name,
                                      &tmp_model.molecule[0].atom, movie_frame,
                                      scale, black_background);
                    }
                  else
                    {
                      editor_msg ("red",
                                  QString (us_tr ("PAT failed for model %1"))
                                      .arg (current_model));
                      write_bead_spt (use_dir + SLASH + spt_name, &bead_model,
                                      movie_frame, scale, black_background);
                    }
                }
              else
                {
                  ...
                  if (!rasmol->launch (last_spt_text))
                    {
                      US_Static::us_message (
                          us_tr ("Please note:"),
                          us_tr ("There was a problem starting RASMOL\n"
                                 "Please check to make sure RASMOL is "
                                 "properly installed..."));
                      return;
                    }
...
int
US_Hydrodyn::calc_hydro ()
{
  return do_calc_hydro ();
}

int
US_Hydrodyn::do_calc_hydro ()
{
  if (!overwrite)
    {
      setHydroFile ();
  ...
  le_bead_model_suffix->setText ("<center>" + bead_model_suffix + "</center>");
  // qDebug() << "us_hydrodyn::do_calc_hydro()_rg +/- " << results.asa_rg_pos
  // << " " << results.asa_rg_neg;

  int retval = us_hydrodyn_supc_main (
      &results, &hydro,
      hydro.overlap_cutoff ? hydro.overlap : overlap_tolerance, &bead_models,
      &somo_processed, &model_vector, lb_model,
      QString (project +
      ...
      QString (project
               + (bead_model_from_file
                      ? ""
                      : QString ("_%1").arg (model_name (first_model_no - 1)))
               + QString (bead_model_suffix.length ()
                              ? ("-" + bead_model_suffix)
                              : "")
               + DOTSOMO + ".beams")
          .toLatin1 ()
          .data (),
      model_names, progress, editor, this);
  ...
  if (retval)
    {
      ...
      switch (retval)
        ...
        case US_HYDRODYN_SUPC_OVERLAPS_EXIST:
          {
            printError (
                "Calculate RB Hydrodynamics SMI encountered overlaps in the "
                "bead model\nPerhaps you should Calculate RB Hydrodynamics "
                "ZENO or GRPY which support bead models with overlaps");
            return retval;
            break;
          }
        ...
        case US_HYDRODYN_PAT_ERR_MEMORY_ALLOC:
          {
            printError (
                "US_HYDRODYN_PAT encountered a memory allocation error");
            return retval;
            break;
          }
        default:
          {
            printError (
                "Calculate RB Hydrodynamics SMI encountered an unknown error");
            // unknown error
            return retval;
            break;
          }
...
void
US_Hydrodyn::show_hydro_results ()
{
  if (misc.hydro_supc)
    {
      if (results_widget)
      ...
      {
        results_window
            = new US_Hydrodyn_Results (&results, &results_widget, this);
...
void
US_Hydrodyn::select_comparative ()
{
  if (comparative_widget)
    ...
    {
      comparative_window = new US_Hydrodyn_Comparative (&comparative, this,
                                                        &comparative_widget);
...
US_Hydrodyn::best_analysis ()
{
  if (best_widget)
    ...
    {
      best_window = (QWidget *)new US_Hydrodyn_Best (this, &best_widget);
...
void
US_Hydrodyn::open_hydro_results ()
{
  QString filename = "";
  if (last_hydro_res != ""
      && QFile (somo_dir + SLASH + last_hydro_res).exists ())
    {
      QFileInfo fi (last_hydro_res);
      switch (QMessageBox::question (
          this, us_tr ("Open Hydrodynamic Calculations File"),
        ...
        case QMessageBox::No:
          {
            QString use_dir = path_open_hydro_res.isEmpty ()
                                  ? somo_dir
                                  : path_open_hydro_res;
            ...
            filename = QFileDialog::getOpenFileName (
                this, windowTitle (), use_dir,
                "*.hydro_res *.HYDRO_RES *.zno *.ZNO *.grpy_res *.GRPY_RES");
    ...
    {
      QString use_dir
          = path_open_hydro_res.isEmpty () ? somo_dir : path_open_hydro_res;
...
void
US_Hydrodyn::select_save_params ()
{
  if (save_widget)
    {
...
void
US_Hydrodyn::show_advanced_config ()
{
  if (advanced_config_widget)
    ...
    {
      advanced_config_window = new US_Hydrodyn_AdvancedConfig (
          &advanced_config, &advanced_config_widget, this);
...
void
US_Hydrodyn::stop_calc ()
{
  // qDebug() << "US_Hydrodyn::stop_calc()";
  stopFlag = true;
  anaflex_return_to_bd_load_results = false;
  if (batch_widget && batch_window->pb_stop->isEnabled ())
    {
...
int
US_Hydrodyn::calc_iqq (bool bead_model, bool create_native_saxs, bool do_raise)
{
  bead_model ? bead_saxs (create_native_saxs, do_raise)
             : pdb_saxs (create_native_saxs, do_raise);
  saxs_plot_window->show_plot_saxs_sans ();
  return 0;
}

int
US_Hydrodyn::calc_prr (bool bead_model, bool create_native_saxs, bool do_raise)
{
  bead_model ? bead_saxs (create_native_saxs, do_raise)
             : pdb_saxs (create_native_saxs, do_raise);
...
void
US_Hydrodyn::pdb_saxs (bool create_native_saxs, bool do_raise)
{
  ...
  if (selected_models.size () > 1)
    {
      US_Static::us_message (
          us_tr ("Please note:"),
          us_tr (
              "You must select exactly one model to perform SAXS functions."));
    }
  else
    {
          ...
          if (dammix_remember_mw.count (QFileInfo (filename).fileName ())
              && !saxs_options.multiply_iq_by_atomic_volume)
            {
              if (dammix_remember_mw[QFileInfo (filename).fileName ()]
                  ...
                  && (!batch_widget || !batch_window->batch_job_running))
                {
                  switch (QMessageBox::question (
                      this, us_tr ("UltraScan Notice"),
                      QString (us_tr ("Please note:\n\n"
                                      "You have remembered a molecular weight "
                                      "of %1 Daltons\n"
                                      "but the loaded pdb has a computed "
                                      "molecular weight of %2 Daltons\n"
                                      "What would you like to do?\n"))
                          .arg (dammix_remember_mw[QFileInfo (filename)
                                                       .fileName ()])
                    ...
                    case 0:
                      dammix_remember_mw[QFileInfo (filename).fileName ()]
                          = model_vector[selected_models[0]].mw;
                      dammix_remember_mw_source[QFileInfo (filename)
                                                    .fileName ()]
                          = "computed from pdb";
                      break;
                    case 1:
                      break;
                    case 2:
                      dammix_remember_mw.erase (
                          QFileInfo (filename).fileName ());
                      dammix_remember_mw_source.erase (
                          QFileInfo (filename).fileName ());
                      break;
                    default:
                      break;
          ...
          else
            {
              dammix_remember_mw[QFileInfo (filename).fileName ()]
                  = model_vector[selected_models[0]].mw
                    + model_vector[selected_models[0]].ionized_mw_delta;
      ...
      if (saxs_plot_widget)
        {
          saxs_plot_window->refresh (
              filename, pdb_file, residue_list, model_vector, bead_models,
              selected_models, multi_residue_map, residue_atom_hybrid_map, 0,
              create_native_saxs);
          if (do_raise)
            {
      ...
      else
        {
          saxs_plot_window = new US_Hydrodyn_Saxs (
              &saxs_plot_widget, &saxs_options, filename, pdb_file,
              residue_list, model_vector, bead_models, selected_models,
              multi_residue_map, residue_atom_hybrid_map, 0,
              create_native_saxs, this, 0);
...
void
US_Hydrodyn::bead_saxs (bool create_native_saxs, bool do_raise)
{
  vector<unsigned int> selected_models;
  for (unsigned int i = 0; i < (unsigned int)lb_model->count (); i++)
    {
  ...
  if (selected_models.size () != 1)
    {
      US_Static::us_message (us_tr ("Please note:"),
                             us_tr ("You must select exactly one model to "
                                    "perform SAXS functions.."));
    }
  else
    {
      QString filename
          = QFileInfo (bead_model_file).completeBaseName ()
      ...
      printf ("selected models size %u bead_models.size %u beads in "
              "selected_models[0] %u\n",
      ...
      // compute mw
      float tmp_mw = 0.0;
      for (unsigned int i = 0; i < bead_models[selected_models[0]].size ();
           i++)
        {
          tmp_mw += bead_models[selected_models[0]][i].bead_mw
                    + bead_models[selected_models[0]][i].bead_ionized_mw_delta;
        }

      if (tmp_mw != 0.0
          && dammix_remember_mw.count (QFileInfo (filename).fileName ()))
        {
          if (dammix_remember_mw[QFileInfo (filename).fileName ()] != tmp_mw)
            {
              switch (QMessageBox::question (
                  this, us_tr ("UltraScan Notice"),
                  QString (us_tr ("Please note:\n\n"
                                  "You have remembered a molecular weight of "
                                  "%1 Daltons\n"
                                  "but the loaded bead model a computed "
                                  "molecular weight of %2 Daltons\n"
                                  "What would you like to do?\n"))
                      .arg (
                          dammix_remember_mw[QFileInfo (filename).fileName ()])
      ...
      if (saxs_plot_widget)
        {
          saxs_plot_window->refresh (
              filename, pdb_file, residue_list, model_vector, bead_models,
              selected_models, multi_residue_map, residue_atom_hybrid_map, 1,
              create_native_saxs);
          if (do_raise)
            {
              saxs_plot_window->raise ();
      ...
      else
        {
          saxs_plot_window = new US_Hydrodyn_Saxs (
              &saxs_plot_widget, &saxs_options, filename, pdb_file,
              residue_list, model_vector, bead_models, selected_models,
              multi_residue_map, residue_atom_hybrid_map, 1,
              create_native_saxs, this, 0);
          fixWinButtons (saxs_plot_window);
          // always show
          saxs_plot_window->show ();
...
QString
US_Hydrodyn::getExtendedSuffix (bool prerun, bool somo, bool no_ovlp_removal,
                                bool vdw)
{
  ...
  if (vdw)
    {
      ...
      bead_model_suffix = "";
      {
        double vdw_ot_mult = gparams.count ("vdw_ot_mult")
                                 ? gparams["vdw_ot_mult"].toDouble ()
                                 : 0;
        double vdw_ot_dpct = gparams.count ("vdw_ot_dpct")
                                 ? gparams["vdw_ot_dpct"].toDouble ()
                                 : 0;
        bool vdw_ot_alt
            = gparams.count ("vdw_ot_alt") && gparams["vdw_ot_alt"] == "true";
        if (vdw_ot_mult)
          {
            if (vdw_ot_dpct)
              {
                bead_model_suffix += QString ("OT%1%2DP%3_")
                                         .arg (vdw_ot_alt ? "alt" : "")
                                         .arg (vdw_ot_mult)
                                         .arg (vdw_ot_dpct)
                                         .replace (".", "_");
              ...
              {
                bead_model_suffix += QString ("OT%1%2_")
                                         .arg (vdw_ot_alt ? "alt" : "")
                                         .arg (vdw_ot_mult)
                                         .replace (".", "_");
              }
          }
        bead_model_suffix += QString ("PR%1_TH%2_")
                                 .arg (asa.hydrate_probe_radius)
                                 .arg (asa.hydrate_threshold)
                                 .replace (".", "_");
        bead_model_suffix
            += QString ("pH%1").arg (hydro.pH).replace (".", "_");
  ...
  if (setSuffix)
    {
      ...
      if ((asa.calculation && somo) || (grid.enable_asa && !somo))
        {
          result += QString ("A%1").arg (somo ? asa.threshold
                                              : asa.grid_threshold);
        }

      if (asa.recheck_beads)
        {
          result += QString ("R%1").arg (somo ? asa.threshold_percent
      ...
      for (int i = 0; i < lb_model->count (); i++)
        {
          if (lb_model->item (i)->isSelected ())
            {
              path = somo_dir + QDir::separator () + project
                     + QString ("_%1").arg (i + 1)
              ...
              if (file != new_file)
                {
                  le_bead_model_suffix->setText ("<center>" + bead_model_suffix
                                                 + "</center>");
                  any_changes = true;
                }
              ...
              if (saveParams)
                {
                  ...
                  file = path + bead_model_suffix + ext;
                  new_file
                      = fileNameCheck (&path, &bead_model_suffix, &ext, 0);
                  if (file != new_file)
                    {
                      le_bead_model_suffix->setText (
                          "<center>" + bead_model_suffix + "</center>");
                      any_changes = true;
                    }
                }
            }
...
void
US_Hydrodyn::setSomoGridFile (bool somo)
{
              ...
              if (bead_output.output & US_HYDRODYN_OUTPUT_BEAMS)
                {
                  ...
                  // frmc =
                  // us_fopen(QString("%1.rmc").arg(fname).toLatin1().data(),
                  // "w"); frmc1 =
                  // us_fopen(QString("%1.rmc1").arg(fname).toLatin1().data(),
                  // "w");
                }
...
QString
US_Hydrodyn::fileNameCheck (QString filename, int mode, QWidget *p)
{
  // checks to see if file name exists, and if it does, according to 'mode'
  // mode == 0, stop and ask with the option for an new filename, mode == 1
  // auto increment,
...
QString
US_Hydrodyn::fileNameCheck (QString *path, QString *base, QString *ext,
                            int mode, QWidget *p)
{
      ...
      do
        {
          if (rx.indexIn (*base) != -1)
            {
              base->replace (rx,
                             QString ("-%1").arg (rx.cap (1).toInt () + 1));
            }
          else
            {
              *base += "-1";
...
QString
US_Hydrodyn::fileNameCheck2 (QString filename, bool &cancel,
                             bool &overwrite_all, int mode, QWidget *p)
{
  // checks to see if file name exists, and if it does, according to 'mode'
  // mode == 0, stop and ask with the option for an new filename, mode == 1
  // auto increment,
...
QString
US_Hydrodyn::fileNameCheck2 (QString *path, QString *base, QString *ext,
                             bool &cancel, bool &overwrite_all, int mode,
                             QWidget *p)
{
  if (!QFile::exists (*path + *base + *ext))
    {
  ...
  US_Hydrodyn_File2 *hf2 = new US_Hydrodyn_File2 (path, base, ext, cancel,
                                                  overwrite_all, &result, p);
  fixWinButtons (hf2);
...
US_Hydrodyn::dmd_run ()
{
  ...
  US_Hydrodyn_Cluster *hc = new US_Hydrodyn_Cluster (this, batch_window);
  fixWinButtons (hc);
  hc->exec ();
  delete hc;
  ...
  if (created_batch)
  ...
  else
    {
      ...
      for (unsigned int i = 0; i < batch.file.size (); i++)
        {
          if (batch.file[i].contains (
                  QRegExp ("(pdb|PDB|bead_model|BEAD_MODEL|beams|BEAMS)$")))
            {
              bool dup = false;
              if (i)
                {
              ...
              if (!dup)
              ...
              else
                {
                  load_errors
                      += QString (
                             us_tr ("File skipped: %1 (already in list)\n"))
                             .arg (batch.file[i]);
                }
            }
          else
            {
              load_errors
                  += QString (
                         us_tr ("File skipped: %1 (not a valid file name)\n"))
                         .arg (batch.file[i]);
            }
        }
      if (load_errors != "")
...
void
US_Hydrodyn::update_enables ()
{
}

void
US_Hydrodyn::sizeArrows (QwtCounter *
...
void
US_Hydrodyn::fixWinButtons (QWidget *
#if QT_VERSION >= 0x040000
                                widg
#endif
)
...
bool
US_Hydrodyn::equi_grid_bead_model (double dR)
{
  stopFlag = false;
  pb_stop_calc->setEnabled (true);
      ...
      do
        {
          dR = US_Static::getDouble (
              "dR for equi grid:",
              us_tr ("Enter a cube side value in Angstroms:"), 1e0, -1e0, 1e4,
              3, &ok, this);
          if (!ok)
            {
              editor->append ("Stopped by user\n\n");
  ...
  bead_model_suffix
      = getExtendedSuffix (false, false)
            .replace ("a2b", QString ("eqm%1").arg (dR).replace (".", "_"))
        + "g";
  le_bead_model_suffix->setText ("<center>" + bead_model_suffix + "</center>");
  if (!overwrite)
    {
      ...
      if (lb_model->item (current_model)->isSelected ())
        {
          printf ("in calc_grid: is selected current model %d\n",
                  current_model);
          fflush (stdout);
          if (somo_processed.size () > current_model
              && somo_processed[current_model])
            {
              ...
              if (!saxs_util->grid (bead_models[current_model], bead_model, dR,
                                    false, false))
                {
              ...
              for (unsigned int i = 0; i < (unsigned int)bead_model.size ();
                   i++)
                {
  ...
  if (calcAutoHydro)
    {
      switch (misc.auto_calc_hydro_method)
        ...
        default:
          editor_msg (
              "red", us_tr ("No known hydrodynamic method set for automatic "
                            "hydrodynamic calculations\n"
                            "Check SOMO->Miscellaneous Options->Automatically "
                            "calculate hydrodynamics method"));
          break;
        }
    }
  else
...
bool
US_Hydrodyn::calc_zeno_hydro ()
{
  return (int)(!calc_zeno ());
}

void
US_Hydrodyn::le_pdb_file_focus (bool)
{
  // us_qdebug( "focused" );
...
void
US_Hydrodyn::le_pdb_file_changed (const QString &)
{
  // us_qdebug( "changed" );
...
void
US_Hydrodyn::update_progress (int pos, int total)
{
...
void
US_Hydrodyn::do_update_progress (int pos, int total)
{
...
QString
US_Hydrodyn::get_somo_dir ()
{
  if (!batch_widget || !batch.results_dir || batch.results_dir_name.isEmpty ())
    {
      return somo_dir;
    }
  ...
  batch_window->editor_msg (
      ...
      QString (
          us_tr (
              "WARNING: results directory %1 could not be created, using %2"))
          .arg (dir.path ())
          .arg (somo_dir));
  return somo_dir;
}
