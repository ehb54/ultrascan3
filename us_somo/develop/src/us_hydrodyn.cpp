...
// debugging us_hydrodyn_util.cpp contains other various code, such as disulfide
// code us_hydrodyn_load.cpp contains code to load files us_hydrodyn_grpy.cpp
// contains code for grpy interface us_hydrodyn_beads.cpp contains top-level
// code for generating bead models us_hydrodyn_settings.cpp contains code for
// reading/writing settings us_hydrodyn_write.cpp contains code for writes (bead
// models etc)
...
US_Hydrodyn::US_Hydrodyn(vector<QString> batch_file, QString gui_script_file,
...
#if defined(EIGEN_TEST)
  {
    ...
    */
    ...
    // 1.37, .254, 3.26, .55
    vector<double> y = {5.434,  19.318,  46.322, 89.746,
                        152.89, 239.054, 351.538};
...
#define TSO QTextStream(stdout)
    ...
    US_Vector::printvector(
        QString("coefficients EIGEN_HOUSEHOLDER_QR chi2 %1").arg(chi2), coeff);
...
#define TSO QTextStream(stdout)
...
#if defined(DP_TEST)
  {
    ...
    vector<double> vals = {0,      1,       1.2,  1.23,   1.234,
                           1.2345, 1.55555, 1e10, -5.2736};
...
void US_Hydrodyn::setupGUI() {
  ...
  pb_select_residue_file->setFont(
  ...
  pb_reload_pdb->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  pb_reload_pdb->setPalette(PALET_PUSHB);
  connect(pb_reload_pdb, SIGNAL(clicked()), SLOT(reload_pdb()));
  pb_reload_pdb->setToolTip(
      us_tr("<html><body>Use this to reload the PDB, primarily useful when the "
            "pH has been adjusted.</body></html>"));
  ...
  pb_pdb_hydrate_for_saxs->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  pb_equi_grid_bead_model->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  // gl_prefix_suffix->addWidget( le_bead_model_suffix , 0 , 1 , 1 + ( 0 ) - ( 0
  // ) , 1 + ( 2 ) - ( 1 ) ); background->addLayout(gl_prefix_suffix, j, 1);
  j++;
  ...
  //   0, 0, 0, 0 ); qhl_bd_1->setSpacing( 0 ); qhl_bd_1->addWidget(pb_bd_load);
...
void US_Hydrodyn::show_overlap() {
  if (overlap_widget) {
    ...
    overlap_window = new US_Hydrodyn_Overlap(
        ...
        &replicate_o_r_method_somo, &grid_exposed_overlap, &grid_buried_overlap,
        &grid_overlap, &replicate_o_r_method_grid, &overlap_tolerance,
        &overlap_widget, this);
...
void US_Hydrodyn::show_grid_overlap() {
  if (grid_overlap_widget) {
    ...
    grid_overlap_window =
        new US_Hydrodyn_Overlap(&grid_exposed_overlap, &grid_buried_overlap,
                                &grid_overlap, &replicate_o_r_method_grid,
                                &overlap_tolerance, &grid_overlap_widget, this);
...
void US_Hydrodyn::show_dmd_options() {
  if (dmd_options_widget) {
    ...
    dmd_options_window =
        new US_Hydrodyn_DMD_Options(&dmd_options, &dmd_options_widget, this);
...
void US_Hydrodyn::show_bd() {
  ...
  if (bd_widget) {
    ...
    bd_window =
        new US_Hydrodyn_BD(&bd_widget, &bd_options, &anaflex_options, this);
...
void US_Hydrodyn::show_bead_output() {
  if (bead_output_widget) {
    ...
    bead_output_window =
        new US_Hydrodyn_Bead_Output(&bead_output, &bead_output_widget, this);
...
void US_Hydrodyn::show_grid() {
  if (grid_widget) {
    ...
    grid_window =
        new US_Hydrodyn_Grid(&grid_exposed_overlap, &grid_buried_overlap,
                             &grid_overlap, &replicate_o_r_method_grid, &grid,
...
void US_Hydrodyn::pdb_parsing() {
  if (pdb_parsing_widget) {
    ...
    pdb_parsing_window =
        new US_Hydrodyn_PDB_Parsing(&pdb_parse, &pdb_parsing_widget, this);
...
void US_Hydrodyn::pdb_visualization() {
  if (pdb_visualization_widget) {
    ...
    pdb_visualization_window =
        new US_Hydrodyn_PDB_Visualization(&pdb_vis, &pdb_visualization_widget);
...
void US_Hydrodyn::sync_pdb_info(QString /* msg */) {
  ...
  if ((misc.pb_rule_on && !pdb_info.count("pb_rule")) ||
    ...
    if (misc.pb_rule_on && !pdb_info.count("pb_rule")) {
      // us_qdebug( QString( "sync_pdb_info() %1 misc.pb_rule was ON, now OFF"
      // ). arg( msg ) );
      misc.pb_rule_on = false;
      misc.restore_pb_rule = true;
    } else {
      // us_qdebug( QString( "sync_pdb_info() %1 misc.pb_rule was OFF, now ON"
      // ). arg( msg ) );
      misc.pb_rule_on = true;
      misc.restore_pb_rule = false;
    }
  ...
  switch (pdb_parse.missing_residues) {
    ...
    case 1: {
      ...
      switch (QMessageBox::question(
          ...
          us_tr("A non-coded residue was encountered\n"
                ...
                "This choice will be maintained for all remaining non coded "
                "residues encountered in this PDB file.\n"
                "For specific residue details, please review the main panel "
                "text area after making your selection.\n"
                "\n"
          ...
          us_tr("Skip non-coded residues"), us_tr("Stop processing"), QString(),
          1, 1)) {
        case 0:
    ...
    case 2: {
      ...
      switch (QMessageBox::question(
          this, us_tr("US-SOMO: Non coded residue"),
          us_tr("A non-coded residue was encountered\n"
                "It can be replaced with an average residue, skipped or you "
                "can stop processing.\n"
                "This choice will be maintained for all remaining non coded "
                "residues encountered in this PDB file.\n"
                "For specific residue details, please review the main panel "
                ...
                "global value for the vbar in the SOMO Miscellaneous options. "
                "You can also\n"
                "review the average residue settings in the SOMO Miscellaneous "
                "options.\n"
                "\n"
                "Please select your option below"),
          us_tr("Replace non-coded with average residues"),
          us_tr("Skip non-coded residues"), us_tr("Stop processing"), 2, 2)) {
        case 0:
  ...
  switch (pdb_parse.missing_atoms) {
    ...
    case 1: {
      ...
      switch (QMessageBox::question(
          ...
          us_tr(
              "A missing or extra atom was encountered when comparing with the "
              "residue table\n"
              "It can be skipped or you can stop processing.\n"
              "This choice will be maintained for all remaining missing atoms "
              "encountered in this PDB file.\n"
              "For specific residue details, please review the main panel text "
              "area after making your selection.\n"
              "\n"
              ...
              "\n"
              "Please select your option below"),
          us_tr("Skip residues with missing atoms"), us_tr("Stop processing"),
          QString(), 1, 1)) {
        case 0:
    ...
    case 2: {
      ...
      switch (QMessageBox::question(
          this, us_tr("US-SOMO: Missing or extra atoms"),
          us_tr("A missing or extra atom was encountered when comparing with "
                "the residue table\n"
                "It can be modeled with an approximate method, skipped or you "
                "can stop processing.\n"
                ...
                "text area after making your selection.\n"
                "\n"
                "If you model with an approximate method, the shape of this "
                "residue will\n"
...
void US_Hydrodyn::load_pdb() {
  ...
  if (0 && pdb_parse.missing_residues == 1 && !advanced_config.expert_mode) {
    message += us_tr(
        ...
        "missing\n"
        "residues, the calculated molecular weight and vbar may be incorrect, "
        "and\n"
        "you should manually enter a global value for the molecular weight in "
        "the\n"
  ...
  if (0 && pdb_parse.missing_residues == 2 && !advanced_config.expert_mode) {
    message += us_tr(
        "You have selected to replace non-coded residues with an average "
        "residue.\n"
        "If your model contains non-coded residues, the calculated molecular "
        "weight\n"
        "and vbar may be incorrect. Therefore, you could manually enter a "
        "global\n"
        "value for the molecular weight in the SOMO hydrodynamic options, and "
        "a\n"
        "global value for the vbar in the SOMO Miscellaneous options. You can "
        "also\n"
        "review the average residue settings in the SOMO Miscellaneous "
        "options.\n\n"
        "Are you sure you want to proceed?");
  ...
  if (0 && pdb_parse.missing_atoms == 1 && !advanced_config.expert_mode) {
    message += us_tr(
        "You have selected to skip coded residues containing missing atoms.\n"
        "If your model contains missing atoms, the calculated molecular\n"
  ...
  QString filename = QFileDialog::getOpenFileName(
      this, "Please select a PDB file...", use_dir, "Structures (*.pdb *.PDB)");
...
bool US_Hydrodyn::screen_pdb(QString filename, bool display_pdb,
  ...
  if (batch_widget && batch_window->batch_job_running &&
    ...
    batch_window->editor_msg(
        "darkred",
        QString(us_tr("Screening: %1 : WARNING: PDB contains residues with "
...
void US_Hydrodyn::set_overwrite() { overwrite = cb_overwrite->isChecked(); }
...
void US_Hydrodyn::view_asa() {
  QString use_dir = path_view_asa_res.isEmpty() ? somo_dir : path_view_asa_res;

  select_from_directory_history(use_dir, this);

  QString filename = QFileDialog::getOpenFileName(this, windowTitle(), use_dir,
                                                  "*.asa_res *.ASA_RES");
  if (!filename.isEmpty()) {
  ...
  if (!chose_last_bead_model) {
    if (last_read_bead_model != "") {
      ...
      switch (QMessageBox::question(
        ...
        case QMessageBox::No: {
          QString use_dir =
              path_view_bead_model.isEmpty() ? somo_dir : path_view_bead_model;
        ...
        if (do_pat) {
          ...
          if (US_Saxs_Util::pat_model(tmp_model, true)) {
            write_bead_spt(use_dir + SLASH + spt_name,
                           &tmp_model.molecule[0].atom, movie_frame, scale,
                           black_background);
          } else {
            editor_msg(
                "red",
                QString(us_tr("PAT failed for model %1")).arg(current_model));
            write_bead_spt(use_dir + SLASH + spt_name, &bead_model, movie_frame,
...
int US_Hydrodyn::calc_hydro() { return do_calc_hydro(); }
...
int US_Hydrodyn::do_calc_hydro() {
  ...
  int retval = us_hydrodyn_supc_main(
      ...
      QString(
          ...
          // + (bead_model_from_file ? "" : (models_to_proc == 1 ? "_1" :
          // "_%1")) +
  ...
  if (retval) {
    ...
    switch (retval) {
      case US_HYDRODYN_SUPC_FILE_NOT_FOUND: {
        printError(
            "Calculate RB Hydrodynamics SMI encountered a file not found "
            "error");
        return retval;
        break;
      }
      case US_HYDRODYN_SUPC_OVERLAPS_EXIST: {
        printError(
            "Calculate RB Hydrodynamics SMI encountered overlaps in the bead "
            "model\nPerhaps you should Calculate RB Hydrodynamics ZENO or GRPY "
            "which support bead models with overlaps");
        return retval;
        break;
      }
      case US_HYDRODYN_SUPC_ERR_MEMORY_ALLOC: {
        printError(
            "Calculate RB Hydrodynamics SMI encountered a memory allocation "
            "error");
        return retval;
        break;
      }
      case US_HYDRODYN_SUPC_NO_SEL_MODELS: {
        printError(
            "Calculate RB Hydrodynamics SMI was called with no processed "
            "models selected");
        return retval;
        break;
      }
...
void US_Hydrodyn::pdb_saxs(bool create_native_saxs, bool do_raise) {
  ...
  if (selected_models.size() > 1) {
          ...
          switch (QMessageBox::question(
              ...
              QString(
                  us_tr("Please note:\n\n"
                        ...
                        "but the loaded pdb has a computed molecular weight of "
                        "%2 Daltons\n"
    ...
    if (saxs_plot_widget) {
      saxs_plot_window->refresh(filename, pdb_file, residue_list, model_vector,
                                bead_models, selected_models, multi_residue_map,
                                residue_atom_hybrid_map, 0, create_native_saxs);
      if (do_raise) {
        saxs_plot_window->raise();
      }
    } else {
      saxs_plot_window = new US_Hydrodyn_Saxs(
          &saxs_plot_widget, &saxs_options, filename, pdb_file, residue_list,
          model_vector, bead_models, selected_models, multi_residue_map,
          residue_atom_hybrid_map, 0, create_native_saxs, this, 0);
...
void US_Hydrodyn::bead_saxs(bool create_native_saxs, bool do_raise) {
  ...
  if (selected_models.size() != 1) {
    ...
    printf(
        "selected models size %u bead_models.size %u beads in "
        "selected_models[0] %u\n",
        (unsigned int)bead_models.size(), (unsigned int)selected_models.size(),
      ...
      if (dammix_remember_mw[QFileInfo(filename).fileName()] != tmp_mw) {
        switch (QMessageBox::question(
            ...
            QString(
                us_tr("Please note:\n\n"
                      "You have remembered a molecular weight of %1 Daltons\n"
                      "but the loaded bead model a computed molecular weight "
                      "of %2 Daltons\n"
                      "What would you like to do?\n"))
                .arg(dammix_remember_mw[QFileInfo(filename).fileName()])
                .arg(tmp_mw),
    ...
    if (saxs_plot_widget) {
      saxs_plot_window->refresh(filename, pdb_file, residue_list, model_vector,
                                bead_models, selected_models, multi_residue_map,
                                residue_atom_hybrid_map, 1, create_native_saxs);
      if (do_raise) {
        saxs_plot_window->raise();
      }
    } else {
      saxs_plot_window = new US_Hydrodyn_Saxs(
          &saxs_plot_widget, &saxs_options, filename, pdb_file, residue_list,
          model_vector, bead_models, selected_models, multi_residue_map,
          residue_atom_hybrid_map, 1, create_native_saxs, this, 0);
      fixWinButtons(saxs_plot_window);
      // always show
      saxs_plot_window->show();
    }
...
QString US_Hydrodyn::getExtendedSuffix(bool prerun, bool somo,
  ...
  if (vdw) {
    ...
    {
      double vdw_ot_mult =
          gparams.count("vdw_ot_mult") ? gparams["vdw_ot_mult"].toDouble() : 0;
      double vdw_ot_dpct =
          gparams.count("vdw_ot_dpct") ? gparams["vdw_ot_dpct"].toDouble() : 0;
      bool vdw_ot_alt =
    ...
    for (int i = 0; i < lb_model->count(); i++) {
      if (lb_model->item(i)->isSelected()) {
        ...
        if (bead_output.output & US_HYDRODYN_OUTPUT_BEAMS) {
          ...
          // frmc = us_fopen(QString("%1.rmc").arg(fname).toLatin1().data(),
          // "w"); frmc1 =
          // us_fopen(QString("%1.rmc1").arg(fname).toLatin1().data(), "w");
        }
...
QString US_Hydrodyn::fileNameCheck(QString filename, int mode, QWidget *p) {
  ...
  // mode == 0, stop and ask with the option for an new filename, mode == 1 auto
  // increment,
...
QString US_Hydrodyn::fileNameCheck(QString *path, QString *base, QString *ext,
                                   int mode, QWidget *p) {
...
QString US_Hydrodyn::fileNameCheck2(QString filename, bool &cancel,
...
QString US_Hydrodyn::fileNameCheck2(QString *path, QString *base, QString *ext,
  ...
  US_Hydrodyn_File2 *hf2 =
      new US_Hydrodyn_File2(path, base, ext, cancel, overwrite_all, &result, p);
  fixWinButtons(hf2);
...
void US_Hydrodyn::dmd_run() {
  if (pdb_file.isEmpty()) {
    QMessageBox::information(this, "US-SOMO: Run DMD",
...
void US_Hydrodyn::run_us_config() {
  ...
  if (!QFile(procapp).exists() && QFile(procbin).exists()) procapp = procbin;
  ...
  if (!process->start()) {
    QMessageBox::information(this, us_tr("Error"),
...
bool US_Hydrodyn::equi_grid_bead_model(double dR) {
  ...
  if (!dR) {
    ...
    do {
      dR = US_Static::getDouble(
          "dR for equi grid:", us_tr("Enter a cube side value in Angstroms:"),
          1e0, -1e0, 1e4, 3, &ok, this);
      if (!ok) {
        editor->append("Stopped by user\n\n");
        pb_grid_pdb->setEnabled(true);
        pb_vdw_beads->setEnabled(true);
  ...
  for (current_model = 0; current_model < (unsigned int)lb_model->count();
    ...
    if (lb_model->item(current_model)->isSelected()) {
      ...
      if (somo_processed.size() > current_model &&
        ...
        fflush(stdout);
        editor->append(
            QString("Gridding bead model %1\n").arg(current_model + 1));
        progress->reset();
        qApp->processEvents();

        if (!saxs_util->grid(bead_models[current_model], bead_model, dR, false,
                             false)) {
  ...
  if (calcAutoHydro) {
    switch (misc.auto_calc_hydro_method) {
      ...
      default:
        editor_msg("red",
                   us_tr("No known hydrodynamic method set for automatic "
...
bool US_Hydrodyn::calc_zeno_hydro() { return (int)(!calc_zeno()); }
...
QString US_Hydrodyn::get_somo_dir() {
  ...
  batch_window->editor_msg(
      "red",
      QString(
          us_tr("WARNING: results directory %1 could not be created, using %2"))
          .arg(dir.path())
          .arg(somo_dir));
  return somo_dir;
}
