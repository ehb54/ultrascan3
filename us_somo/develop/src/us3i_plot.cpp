...
US_Plot::scaleDivChangedXSlot ()
{
  ...
  if (rescale_mode == MODE_RESID)
    {
      // doesn't work... methinks because events haven't propagated, but can't
      // do this in a slot. maybe extend the qwtplot class and emit a signal?
      if (scroll_zoomer)
        {
...
{
  setFont (
      QFont (US3i_GuiSettings::fontFamily (), US3i_GuiSettings::fontSize ()));
...
void
US_PlotPushbutton::us3i_plotClicked (void)
{
...
US_PlotConfig::selectMargin (int index)
{
  ...
  plot->setStyleSheet (
      QString ("QwtPlot{ padding: %1px }").arg ((index + 1) * 2));
}
...
US_PlotConfig::updateLegendFont (void)
{
  ...
  bool ok;
  QFont newFont
      = QFontDialog::getFont (&ok, font, this, tr ("Set Legend Font"));
...
QColor
US_Plot_Colors::color (size_t i)
{
  return plot_colors[i % plot_colors.size ()];
}

void
US_Plot_Colors::rotate ()
{
...
void
US_Plot_Colors::push_back_color_if_ok (QColor bg, QColor set)
{
