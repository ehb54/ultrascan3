...
US_Plot::US_Plot(mQwtPlot*& parent_plot, const QString& title,
                 const QString& x_axis, const QString& y_axis, QWidget* parent)
    : QHBoxLayout() {
  zoomer = NULL;
  scroll_zoomer = NULL;
  ...
  QFont font(US3i_GuiSettings::fontFamily(), US3i_GuiSettings::fontSize(),
             QFont::Bold);
...
#if QT_VERSION > 0x050000
  plot->setStyleSheet(
      QString("QwtPlot{ padding: %1px }").arg(US3i_GuiSettings::plotMargin()));
#else
...
void US_Plot::scaleDivChangedXSlot() {
  ...
  qDebug() << "US_Plot::scaleDivChangedXSlot(): sender " << plt->objectName()
           << " receiver " << plot->objectName();
  ...
  if (rescale_mode == MODE_RESID) {
    // doesn't work... methinks because events haven't propagated, but can't do
    // this in a slot. maybe extend the qwtplot class and emit a signal?
    if (scroll_zoomer) {
...
void US_Plot::scaleDivChangedYSlot() {
  QwtPlot* plt = static_cast<QwtPlot*>((sender())->parent());
#if defined(DEBUG_SCALE_SLOTS)
  qDebug() << "US_Plot::scaleDivChangedYSlot(): sender " << plt->objectName()
           << " receiver " << plot->objectName();
#endif
  // QwtInterval  intv = plt->axisInterval (QwtPlot::xBottom);
  // plot->setAxisScale(QwtPlot::xBottom, intv.minValue(), intv.maxValue());
...
void US_Plot::scaleDivChangedSlot() {
  QwtPlot* plt = static_cast<QwtPlot*>((sender())->parent());
#if defined(DEBUG_SCALE_SLOTS)
  qDebug() << "US_Plot::scaleDivChangedSlot(): sender " << plt->objectName()
           << " receiver " << plot->objectName();
#endif
  QwtInterval intv = plt->axisInterval(QwtPlot::xBottom);
  plot->setAxisScale(QwtPlot::xBottom, intv.minValue(), intv.maxValue());
...
US_PlotPushbutton::US_PlotPushbutton(const QString& labelString, QWidget* w,
  ...
  setFont(QFont(US3i_GuiSettings::fontFamily(), US3i_GuiSettings::fontSize()));
...
US_PlotConfig::US_PlotConfig(QwtPlot* current_plot, QWidget* p)
    : US3i_widgetsDialog(p)  //( false, p, f )
{
...
void US_PlotConfig::updateTitleFont(void) {
  ...
  QFont newFont =
      QFontDialog::getFont(&ok, currentFont, this, tr("Set Title Font"));
...
void US_PlotConfig::selectMargin(int index) {
  ...
  plot->setStyleSheet(QString("QwtPlot{ padding: %1px }").arg((index + 1) * 2));
}
...
void US_PlotConfig::updateLegendFont(void) {
  ...
  bool ok;
  QFont newFont = QFontDialog::getFont(&ok, font, this, tr("Set Legend Font"));
...
void US_PlotCurveConfig::apply(void) {
  ...
  QwtSymbol* newSymbol =
      new QwtSymbol(symbolStyle, symbolBrush, symbolPen, symbolSize);
...
US_PlotAxisConfig::US_PlotAxisConfig(int currentAxis, QwtPlot* currentPlot,
                                     QWidget* parent)
    : US3i_widgetsDialog(parent)  //( false, parent, flags )
{
  plot = currentPlot;
  axis = currentAxis;
...
void US_PlotAxisConfig::selectTitleFont(void) {
  ...
  QFont newFont =
      QFontDialog::getFont(&ok, currentFont, this, tr("Set Axis Title Font"));
...
void US_PlotAxisConfig::selectScaleFont(void) {
  ...
  QFont newFont =
      QFontDialog::getFont(&ok, currentFont, this, tr("Set Axis Scale Font"));
...
US_Plot_Colors::US_Plot_Colors(const QColor& background_color) {
  ...
  push_back_color_if_ok(background_color,
  ...
  push_back_color_if_ok(background_color,
                        QColor(255, 235, 205)); /* Blanched Almond */
  ...
  push_back_color_if_ok(background_color,
                        QColor(184, 134, 11)); /* Dark Goldenrod */
  push_back_color_if_ok(background_color,
  ...
  push_back_color_if_ok(background_color,
                        QColor(233, 150, 122)); /* Dark Salmon */
  push_back_color_if_ok(background_color,
  ...
  push_back_color_if_ok(background_color,
                        QColor(255, 250, 250)); /* Ghost White */
  ...
  push_back_color_if_ok(background_color,
                        QColor(238, 221, 130)); /* Light Goldenrod */
  push_back_color_if_ok(background_color,
  ...
  push_back_color_if_ok(background_color,
                        QColor(132, 112, 255)); /* Light Slate Blue */
  push_back_color_if_ok(background_color,
                        QColor(119, 136, 153)); /* Light Slate Gray */
  push_back_color_if_ok(background_color,
  ...
  push_back_color_if_ok(background_color,
                        QColor(102, 205, 170)); /* Medium Aquamarine */
  ...
  push_back_color_if_ok(background_color,
                        QColor(0, 250, 154)); /* Medium Spring Green */
  push_back_color_if_ok(background_color,
  ...
  push_back_color_if_ok(background_color,
                        QColor(255, 222, 173));               /* Navajo White */
