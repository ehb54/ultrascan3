...
US_Hydrodyn_Saxs_Hplc_Conc_Load::US_Hydrodyn_Saxs_Hplc_Conc_Load (
    QStringList &qsl_text, csv &csv1, QWidget *p, const char *)
    : QDialog (p)
{
  this->text = &qsl_text;
  this->csv1 = &csv1;
  ...
  setWindowTitle (us_tr ("US-SOMO: SAXS Hplc: File Concentration Loader"));
...
void
US_Hydrodyn_Saxs_Hplc_Conc_Load::setupGUI ()
{
  int minHeight1 = 30;
  ...
  t_csv = new QTableWidget (disp_csv.data.size (), disp_csv.header.size (),
                            this);
  ...
  connect (t_csv->horizontalHeader (), SIGNAL (released (int)),
           SLOT (col_header_released (int)));

  pb_del_row = new QPushButton (us_tr ("Delete Rows"), this);
  pb_del_row->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  pb_set_name
      = new QPushButton (us_tr ("Set column for frame timecode"), this);
  pb_set_name->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize + 1));
  ...
  pb_adjust->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize + 1));
  ...
  pb_set_conc->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize + 1));
  ...
  AUTFBACK (lbl_trial);
  lbl_trial->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize + 1, QFont::Bold));
  ...
  // build layout
  QHBoxLayout *hbl_name = new QHBoxLayout;
  hbl_name->setContentsMargins (0, 0, 0, 0);
  hbl_name->setSpacing (0);
  ...
  QHBoxLayout *hbl_conc = new QHBoxLayout;
  hbl_conc->setContentsMargins (0, 0, 0, 0);
  hbl_conc->setSpacing (0);
  ...
  QHBoxLayout *hbl_trial = new QHBoxLayout;
  hbl_trial->setContentsMargins (0, 0, 0, 0);
  hbl_trial->setSpacing (0);
...
void
US_Hydrodyn_Saxs_Hplc_Conc_Load::cancel ()
{
  close ();
}

void
US_Hydrodyn_Saxs_Hplc_Conc_Load::set_ok ()
{
  // set *csv1
  trial ();
  *csv1 = trial_csv;
...
void
US_Hydrodyn_Saxs_Hplc_Conc_Load::help ()
{
  US_Help *online_help;
  online_help = new US_Help (this);
  online_help->show_help ("manual/somo/somo_saxs_hplc_conc_load.html");
}

void
US_Hydrodyn_Saxs_Hplc_Conc_Load::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept ();
...
csv
US_Hydrodyn_Saxs_Hplc_Conc_Load::current_csv ()
{
...
void
US_Hydrodyn_Saxs_Hplc_Conc_Load::update_enables ()
{
  if (!disable_updates)
    {
      ...
      pb_set_ok->setEnabled (
          !lbl_name->text ().isEmpty () && !lbl_conc->text ().isEmpty ()
...
void
US_Hydrodyn_Saxs_Hplc_Conc_Load::del_row ()
{
...
void
US_Hydrodyn_Saxs_Hplc_Conc_Load::set_name ()
{
  for (int i = 0; i < t_csv->columnCount (); i++)
    {
      if (t_csv->item (0, i)->isSelected ())
...
void
US_Hydrodyn_Saxs_Hplc_Conc_Load::set_conc ()
{
  for (int i = 0; i < t_csv->columnCount (); i++)
    {
      if (t_csv->item (0, i)->isSelected ())
...
QString
US_Hydrodyn_Saxs_Hplc_Conc_Load::csv_to_qstring (csv from_csv)
{
  QString qs;
...
void
US_Hydrodyn_Saxs_Hplc_Conc_Load::reload_csv ()
{
  // cout << csv_to_qstring( disp_csv );
...
void
US_Hydrodyn_Saxs_Hplc_Conc_Load::trial ()
{
  ...
  for (map<QString, QString>::iterator it = concs.begin (); it != concs.end ();
       it++)
    {
  ...
  lbl_trial->setText (
      (found > 0 ? us_tr ("Match ok") : us_tr ("No matches found"))
      + QString (us_tr (" %1 files matched, %2 not matched")
                     .arg (found)
                     .arg (not_found)));
  update_enables ();
}
...
void
US_Hydrodyn_Saxs_Hplc_Conc_Load::adjust ()
{
  // ask & add or subtract
  bool ok;
  int res = US_Static::getInteger (
      us_tr ("US-SOMO: SAXS Hplc: File Concentration Loader: Adjust timecode"),
      us_tr ("Enter a timecode correction:"), 0, -1000, +1000, 1, &ok, this);
  if (ok && res)
    {
      ...
      for (int i = 0; i < t_csv->rowCount (); i++)
        {
          t_csv->setItem (i, 0,
                          new QTableWidgetItem (QString ("%1").arg (
                              t_csv->item (i, 0)->text ().toInt () + res)));
        }
    }
}

void
US_Hydrodyn_Saxs_Hplc_Conc_Load::col_header_released (int col)
{
  t_csv->clearSelection ();
