...
// us_hydrodyn_bd_core.cpp contains the main computational routines for
// brownian dynamic browflex computations us_hydrodyn_anaflex_core.cpp contains
...
#if defined(TO_DO)
static void
outward_translate_1_sphere_1_fixed (
...
)
{
  ...
  float sfact = sqrt (
      ...
      * ((-2 * *r1 * *r2 - *r2 * *r2
          + *r1 * *r1 * (-1 + v1[0] * v1[0] + v1[1] * v1[1] + v1[2] * v1[2]))
             * ((*r1 + *r2) * (*r1 + *r2) - (x1[0] - x2[0]) * (x1[0] - x2[0])
                - (x1[1] - x2[1]) * (x1[1] - x2[1])
                - (x1[2] - x2[2]) * (x1[2] - x2[2]))
         + pow (*r1 * *r1 + *r2 * *r2
                    + *r1
...
void
US_Hydrodyn::get_atom_map (PDB_model *model)
{
  atom_counts.clear ();
  has_OXT.clear ();
                  ...
                  if (advanced_config.debug_2)
                    {
                      printf ("unknown_residues.count(%s) %u\n",
                              ...
                              (unsigned int)unknown_residues.count (
                                  this_atom->resName));
                    }
                  if ((lastResSeq != ""
                       && (lastResSeq.toInt () + 1
                      ...
                      //   == 0 )
                      {
                        ...
                        editor->append (
                            lastResSeq != ""
                                ? QString (
                                      us_tr (
                                          "Warning: break in residue sequence "
                                          "or unknown residue: %1Molecule %2 "
                                          "Residue %3 %4 & %5 %6."))
                                      ...
                                      .arg (this_atom->resSeq)
                                : QString (
                                      us_tr ("Warning: break in residue "
      ...
      if (aa && non_aa)
        {
          ...
          editor->append (
              QString (us_tr ("Notice: %1found %2 non or unknown Amino Acids "
  ...
  for (map<QString, int>::iterator it = atom_counts.begin ();
       it != atom_counts.end (); it++)
    {
      printf ("atom count map %s map pos %d\n", it->first.toLatin1 ().data (),
              it->second);
    }
  for (map<QString, int>::iterator it = has_OXT.begin (); it != has_OXT.end ();
       it++)
    {
...
void
US_Hydrodyn::build_molecule_maps (PDB_model *model)
{
        ...
        if (k > -1 && molecules_residue_missing_atoms[idx][k].size () == 0
            && molecules_residue_missing_atoms_beads[idx][k].size () == 0)
          {
            if (advanced_config.debug_1)
              {
                cout << QString ("Molecule idx <%1> resName <%2> pos <%3> no "
                                 "errors, should be ok\n")
                            .arg (idx)
                            .arg (resName)
                            .arg (k);
                ...
                if (j != (unsigned int)k)
                  {
                    ...
                    if (advanced_config.debug_1)
                      {
                        cout << QString ("Setup skip idx %1|%2\n")
                                    .arg (idx)
                                    .arg (multi_residue_map[resName][j]);
                      }
                  }
              }
          ...
          if (molecules_residue_errors[idx].size ())
            {
              for (unsigned int j = 0;
                   j < molecules_residue_errors[idx].size (); j++)
                {
                  if (molecules_residue_errors[idx][j].length ())
                    {
                      cout << QString ("Molecule idx <%1> resName <%2> match "
                                       "<%3> errors:\n")
                                  .arg (idx)
                                  .arg (resName)
                                  .arg (j);
              ...
              if (molecules_residue_min_missing[idx] > -1)
                {
                  int k = molecules_residue_min_missing[idx];
                  cout << QString ("^^^ Minimum missing position <%1> ma.sz "
                                   "%2 mab.sz %3\n")
                              .arg (k)
                              .arg (molecules_residue_missing_atoms[idx][k]
                                        .size ())
                              .arg (
                                  molecules_residue_missing_atoms_beads[idx][k]
                                      .size ());
                  for (unsigned int j = 0;
                       j < molecules_residue_missing_atoms[idx][k].size ();
                       j++)
                    {
                      cout << QString (".... missing atom %1 bead %2\n")
                                  .arg (molecules_residue_missing_atoms[idx][k]
                                                                       [j])
                                  .arg (molecules_residue_missing_atoms_beads
                                            [idx][k][j]);
                    }
                }
            }
...
int
US_Hydrodyn::check_for_missing_atoms (QString *error_string, PDB_model *model)
{
              ...
              if (residue_list[m].name == this_atom->resName
                         ...
                         == true)
                {
                  if (advanced_config.debug_1)
                    {
                      cout << QString (
                                  "(check for missing atoms) skipping %1 %2\n")
                                  .arg (res_idx)
                                  .arg (m)
                           << endl;
                    }
...
#if defined(DEBUG_MULTI_RESIDUE)
                  printf ("atom name %s residue name %s pos %u atom.size() %u "
                          "map %s size %u has oxt %d\n",
                          this_atom->name.toLatin1 ().data (),
                          this_atom->resName.toLatin1 ().data (), m,
                  ...
                  if (advanced_config.debug_1)
                    {
                      ...
                      printf (
                          "resname %s respos %d mappos %d mapsize %u\n",
                          this_atom->resName.toLatin1 ().data (), m,
                          multi_residue_map[this_atom->resName][0],
                          (unsigned int)multi_residue_map[this_atom->resName]
                              .size ());
                    }
                  if (lastResSeq != this_atom->resSeq)
                    {
                  ...
                  if (advanced_config.debug_1)
                    {
                      printf ("dbg x pdb_parse.missing_atoms %d "
                              ...
                              bead_exceptions[count_idx], lastResPos, l,
                              residue_list[lastResPos].r_atom[l].tmp_flag);
                    }
                    ...
                    case 3:
                      {
                        ...
                        if (advanced_config.debug_2)
                          {
                            printf (
                                "1.0 <%s> residue types %u, last_type %u\n",
...
// #define DEBUG_ABB_VBAR
                            ...
                            us_qdebug (
                                QString (
                                    "residue list push back, "
                                    "residue_list.size() %1 residue_name %2")
                                    .arg (residue_list.size ())
                        ...
                        if (advanced_config.debug_2)
                          {
                            printf ("1.3 <%s> adding <%s> respos %u\n",
                                    new_residue.name.toLatin1 ().data (),
                                    atom_to_add.name.toLatin1 ().data (),
                                    respos);
                          }
                            ...
                            = residue_list[residue_list.size () - 1];
                        {
                          ...
                          if (!residue_atom_hybrid_map.count (mapkey)
                                  ...
                                  atom_to_add.name))
                            {
                              ...
                              // ]; us_qdebug( QString( "hybrid map created for
                              // %1 created as %2" ).arg( mapkey ).arg(
                              // residue_atom_abb_hybrid_map[atom_to_add.name ]
                              // ) );
                              residue_atom_hybrid_map[mapkey] = "ABB";
                              {
                                QString mapkey_no_nc
                                    = QString ("%1|%2")
                                          .arg (this_atom->resName)
                                          .arg (atom_to_add.name);
                                ...
                                }
                              {
                                ...
                                if (!saxs_util->atom_map.count (atom_map_key))
                                  {
                                    saxs_util->atom_map[atom_map_key]
                                        = new_atom;
                                  }
                              }
                                                 ...
                                                 .arg (i)])
                                      {
                                        ...
                                        new_atom.hybrid.mw
                                            = (int)(new_atom.hybrid.mw * 100
                                                    + .5)
                                              / 100.0;
                                        new_atom.hybrid.ionized_mw_delta
                                            *= atoms_scale_weight
                                                [0]; // misc.avg_mass;
                                        new_atom.hybrid.ionized_mw_delta
                                            = (int)(new_atom.hybrid
          ...
          for (unsigned int i = 0; i < unknown_aa_residues_to_check.size ();
               i++)
            {
              if (advanced_config.debug_2)
                {
                  printf (
                      "unknown residue %s checking for removal from broken\n",
                      ...
                      if (advanced_config.debug_2)
                        {
                          printf ("unknown residue %s removed from broken "
                                  "override\n",
                                  unknown_aa_residues_to_check[i]
                                      .toLatin1 ()
                                      .data ());
                        }
      ...
      for (unsigned int i = 0; i < residue_list.size (); i++)
        {
          ...
          ts << residue_list[i].name.toUpper () << "\t" << residue_list[i].type
             << "\t" << str1.sprintf ("%7.2f", residue_list[i].molvol) << "\t"
             << residue_list[i].asa << "\t" << residue_list[i].r_atom.size ()
          ...
          for (unsigned int j = 0; j < residue_list[i].r_atom.size (); j++)
            {
              ts << residue_list[i].r_atom[j].name.toUpper () << "\t"
                 ...
                 << residue_list[i].r_atom[j].hydration << Qt::endl;
            }
...
// # define TOLERANCE 0.001       // this is used to place a limit on the
// allowed radial overlap
#define TOLERANCE overlap_tolerance

int
US_Hydrodyn::overlap_check (bool sc, bool mc, bool buried, double tolerance,
                            int limit, bool from_overlap_hydro)
{
  int retval = 0;
                      ...
                      if (!from_overlap_hydro)
                        {
                          editor_msg (
                              "red",
                              us_tr ("There are more than %1 overlaps greater "
                                     "than tolerance of %2\nWe suggest you "
                                     "use ZENO or GRPY to calculate "
                                     "hydrodynamics for this model")
                                  .arg (limit)
                                  .arg (tolerance));
                        }
                      return retval;
...
int
US_Hydrodyn::create_beads (QString *error_string, bool quiet)
{
  ...
  map<QString, QString> hybrid_name_to_N
      = { { "N3H0", "N1-" }, { "N3H1", "N1" } };
          ...
          for (unsigned int m = 0; m < residue_list.size (); m++)
            {
              if (residue_list[m].name == this_atom->resName
                         ...
                         == true)
                {
                  if (advanced_config.debug_1)
                    {
                      cout << QString ("(create beads) skipping %1 %2\n")
                                  .arg (res_idx)
                                  .arg (m)
                           << endl;
                    }
          ...
          if (respos != -1)
            {
              ...
              if (atompos == -1)
              ...
              else
                {
                  ...
                  this_atom->ionized_mw_delta = residue_list[respos]
                                                    .r_atom[atompos]
                                                    .hybrid.ionized_mw_delta;
                  ...
                  //    // QTextStream( stdout ) <<
                  //    "**************************************** p_residue set
                  //    " << Qt::endl; this_atom->ionized_mw_delta =
                  //    this_atom->p_residue->r_atom[atompos].hybrid.ionized_mw_delta;
                  // } else {
                  //    // QTextStream( stdout ) <<
                  //    "**************************************** p_residue NOT
                  //    set " << Qt::endl;
                  ...
                  this_atom->bead_ref_mw
                      = this_atom->p_residue
                            ->r_bead[this_atom->p_atom->bead_assignment]
                            .mw;
                  this_atom->bead_ref_ionized_mw_delta
                      = this_atom->p_residue
                            ->r_bead[this_atom->p_atom->bead_assignment]
                            .ionized_mw_delta;
                      ...
                      if (!residue_atom_hybrid_map.count (mapkey))
                        {
                          cout << "error: hybrid name missing for "
                               << this_atom->resName << "|" << this_atom->name
                               << endl;
                          QColor save_color = editor->textColor ();
                          editor->setTextColor ("red");
                          editor->append (
                              QString ("%1Molecule %2 Residue %3|%4 %5 Hybrid "
                          ...
                          if (hybrid_name.isEmpty () || !hybrid_name.length ())
                            {
                              ...
                              editor->append (
                                  QString (
                                      "%1Molecule %2 Residue %3|%4 %5 Hybrid "
                                      "name missing. Atom skipped for SAS.\n")
                                      .arg (this_atom->chainID == " "
                                                ? ""
                                                : ("Chain "
                              ...
                              if (!saxs_util->hybrid_map.count (hybrid_name))
                                {
                                  ...
                                  editor->append (
                                      QString (
                                          ...
                                          "%5 name missing from Hybrid file. "
                                  ...
                                  if (!saxs_util->atom_map.count (
                                          this_atom->name + "~" + hybrid_name))
                                    {
                                      cout << "error: atom_map missing for "
                                              "hybrid_name "
                                           << hybrid_name << " atom name "
                                           << this_atom->name << endl;
                                      QColor save_color = editor->textColor ();
                                      editor->setTextColor ("red");
                                      editor->append (
                                          QString (
                                              "%1Molecule %2 Atom %3 Residue "
                                              "%4 %5 Hybrid %6 name missing "
                                              "from Atom file. Atom skipped "
                                              "for SAS.\n")
                                              .arg (this_atom->chainID == " "
                                                        ? ""
                                                        : ("Chain "
                                  ...
                                  else
                                    {
                                      ...
                                      if (!saxs_util->saxs_map.count (
                                                  ...
                                                  .saxs_name))
                                        {
                                          ...
                                          editor->append (
                                              QString (
                                                  "%1Molecule %2 Residue %3 "
                                                  "%4 Hybrid %5 Saxs name %6 "
                                                  "name missing from SAXS "
                  ...
                  // This bit is going, we need to instead lookup ionized
                  // values
                  ...
                  if (!k && this_atom->name == "N"
                      && (!misc.pb_rule_on || this_atom->resName == "PRO"))
                    {
                      double delta_mw = 1.01f;
                      if (hybrid_name_to_N.count (
                              this_atom->p_atom->hybrid.name)
                          && multi_residue_map.count (
                              hybrid_name_to_N[this_atom->p_atom->hybrid.name])
                          && multi_residue_map
                                 [hybrid_name_to_N[this_atom->p_atom->hybrid
                                                       .name]]
                                     .size ())
                        {
                          int index = multi_residue_map
                              [hybrid_name_to_N[this_atom->p_atom->hybrid
                                                    .name]][0];
                          delta_mw = residue_list[index].r_atom[0].hybrid.mw
                                     + residue_list[index]
                                           .r_atom[0]
                                           .hybrid.ionized_mw_delta
                                     - this_atom->mw;
                  ...
                  if (advanced_config.debug_1 || advanced_config.debug_2)
                    {
                      printf (
                          "found atom %s %s in residue %d pos %d bead asgn %d "
                          "%d placing info %d mw %f bead_ref_mw %f hybrid "
                          "name %s %s ba %d chain %d\n",
                          this_atom->name.toLatin1 ().data (),
                          ...
                          this_atom->p_residue
                              ->r_bead[this_atom->p_atom->bead_assignment]
                              .placing_method,
                          this_atom->mw, this_atom->bead_ref_mw,
                          residue_list[respos]
                              .r_atom[atompos]
                              .hybrid.name.toLatin1 ()
                              .data (),
            ...
            if (advanced_config.debug_1 || advanced_config.debug_2)
              {
                printf ("p1 %s j k %d %d %p %s %d chain %d\n",
                        this_atom->name.toLatin1 ().data (), j, k,
                        this_atom->p_atom,
                ...
                if (this_atom->active)
                  {
                    if (last_asgn != (int)this_atom->bead_assignment
                        || last_res != this_atom->resSeq)
                      {
                        if (bead_count)
                          {
                            printf ("res %s %s bead %d bead_mw %f sum atom mw "
                                    "%f diff %f\n",
...
void
US_Hydrodyn::fix_N1_non_pbr (struct PDB_model &model)
{
  ...
  for (int j = 0; j < chains; ++j)
    {
      ...
      if (atoms)
        {
          map<QString, struct atom *> first_atom_map
              = first_residue_atom_map (model.molecule[j]);
          if (first_atom_map.count ("N")
              && hybrid_name_to_N.count (first_atom_map["N"]->hybrid.name)
              && multi_residue_map.count (
                  hybrid_name_to_N[first_atom_map["N"]->hybrid.name])
              && multi_residue_map
                     [hybrid_name_to_N[first_atom_map["N"]->hybrid.name]]
                         .size ())
            {
              map<QString, int> atompos_map
                  = first_residue_PDB_atom_map (model.molecule[j]);
              if (atompos_map.count ("N"))
                {
                  int atompos = atompos_map["N"];
                  int respos = multi_residue_map
                      [hybrid_name_to_N[first_atom_map["N"]->hybrid.name]][0];
                  model.molecule[j].atom[atompos].p_residue
                      = &(residue_list[respos]);
                  model.molecule[j].atom[atompos].p_atom
                      = &(residue_list[respos].r_atom[0]);
                  model.molecule[j].atom[atompos].mw
                      = model.molecule[j].atom[atompos].p_atom->hybrid.mw;
                  model.molecule[j].atom[atompos].ionized_mw_delta
                      = model.molecule[j]
                            .atom[atompos]
                            .p_atom->hybrid.ionized_mw_delta;
                }
              else
                {
                  QTextStream (stderr) << "US_Hydrodyn::fix_N1_non_pbr no 'N' "
                                          "atom found in PDB_chain";
                }
            }
        }
    }
...
void
radial_reduction_thr_t::radial_reduction_thr_setup (
    unsigned int methodk, vector<PDB_atom> *p_bead_model,
    vector<bool> *p_last_reduced, vector<bool> *p_reduced,
    vector<BPair> *p_my_pairs, unsigned int threads, double overlap_tolerance)
{
...
void
radial_reduction_thr_t::radial_reduction_thr_shutdown ()
{
...
void
radial_reduction_thr_t::radial_reduction_thr_wait ()
{
  ...
  while (1)
    {
      ...
      for (unsigned int i = thread; i < (*p_bead_model).size () - 1;
           i += threads)
        {
          (*p_reduced)[i] = false;
          if (1 || (*p_last_reduced)[i])
            {
              for (unsigned int j = i + 1; j < (*p_bead_model).size (); j++)
                {
                  if ((1 || (*p_last_reduced)[j]) && (*p_bead_model)[i].active
                          ...
                          = (*p_bead_model)[i].bead_computed_radius
                            ...
                            - sqrt (
                                pow ((*p_bead_model)[i].bead_coordinate.axis[0]
                                     ...
                                     2)
                                + pow (
                                    (*p_bead_model)[i].bead_coordinate.axis[1]
                                        - (*p_bead_model)[j]
                                              .bead_coordinate.axis[1],
                                    2)
                                + pow (
                                    (*p_bead_model)[i].bead_coordinate.axis[2]
                                        - (*p_bead_model)[j]
...
void
US_Hydrodyn::radial_reduction (bool from_grid)
{
  ...
  write_bead_tsv (somo_tmp_dir + SLASH + "bead_model_start" + DOTSOMO + ".tsv",
                  &bead_model);
  write_bead_spt (somo_tmp_dir + SLASH + "bead_model_start" + DOTSOMO,
                  &bead_model);
#endif
  for (unsigned int k = (grid.enable_asa ? 0 : 3);
       k < (unsigned int)(grid.enable_asa ? 3 : 4); k++)
    {
      // only grid method
      if (!methods[k] || ((from_grid || grid.enable_asa) && k == 1))
        {
              ...
              if (overlaps_exist)
                {
                  ...
                  lbl_core_progress->setText (
                      QString ("Stage %1 popping iteration %2 beads popped %3")
                          .arg (k + 1)
                          .arg (iter)
                          .arg (beads_popped));
                  qApp->processEvents ();
                  ...
                  for (unsigned int m = 0; m < 3; m++)
                    {
                      bead_model[max_bead1].bead_coordinate.axis[m]
                          *= bead_model[max_bead1].bead_ref_mw
                             + bead_model[max_bead1].bead_ref_ionized_mw_delta;
                      bead_model[max_bead1].bead_coordinate.axis[m]
                          += bead_model[max_bead2].bead_coordinate.axis[m]
                             * (bead_model[max_bead2].bead_ref_mw
                                + bead_model[max_bead2]
                                      .bead_ref_ionized_mw_delta);
                      bead_model[max_bead1].bead_coordinate.axis[m]
                          /= bead_model[max_bead1].bead_ref_mw
                             + bead_model[max_bead1].bead_ref_ionized_mw_delta
                             + bead_model[max_bead2].bead_ref_mw
                             + bead_model[max_bead2].bead_ref_ionized_mw_delta;
                    }
                  bead_model[max_bead1].bead_ref_mw
                      = bead_model[max_bead1].bead_ref_mw
                        + bead_model[max_bead2].bead_ref_mw;
...
#if defined(WRITE_EXTRA_FILES)
      write_bead_tsv (somo_tmp_dir + SLASH
                          + QString ("bead_model_pop-%1").arg (k) + DOTSOMO
                          + ".tsv",
                      &bead_model);
      write_bead_spt (somo_tmp_dir + SLASH
                          + QString ("bead_model_pop-%1").arg (k) + DOTSOMO,
                      &bead_model);
#endif
      ...
      if (methods[k] & RADIAL_REDUCTION)
        {
          ...
          if (methods[k] & RR_HIERC)
...
#endif
              lbl_core_progress->setText (
                  QString ("Stage %1 hierarchical radial reduction")
                      .arg (k + 1));
              qApp->processEvents ();
              ...
              do
                {
                  ...
                  lbl_core_progress->setText (
                      QString (
                          "Stage %1 hierarch. red. it. %2 with %3 couples")
                          .arg (k + 1)
                          .arg (iter)
                          .arg (pairs.size ()));
                  qApp->processEvents ();
          ...
          else
            {
              ...
              lbl_core_progress->setText (
                  QString ("Stage %1 sychronous radial reduction")
                      .arg (k + 1));
              qApp->processEvents ();
              ...
              do
                {
                  ...
                  lbl_core_progress->setText (
                      QString (
                          "Stage %1 synchron. red. it. %2 with %3 couples")
                          .arg (k + 1)
                          .arg (iter)
                          .arg (pairs.size ()));
                  qApp->processEvents ();
...
#if defined(WRITE_EXTRA_FILES)
      write_bead_tsv (somo_tmp_dir + SLASH
                          + QString ("bead_model_rr-%1").arg (k) + DOTSOMO
                          + ".tsv",
                      &bead_model);
      write_bead_spt (somo_tmp_dir + SLASH
                          + QString ("bead_model_rr-%1").arg (k) + DOTSOMO,
...
#if defined(WRITE_EXTRA_FILES)
  write_bead_tsv (somo_tmp_dir + SLASH + "bead_model_end" + DOTSOMO + ".tsv",
                  &bead_model);
  write_bead_spt (somo_tmp_dir + SLASH + "bead_model_end" + DOTSOMO,
                  &bead_model);
#endif
...
int
US_Hydrodyn::compute_asa (bool bd_mode, bool no_ovlp_removal)
{
  ...
  if (asa.method == 0)
    {
      ...
      int retval = surfracer_main (asa.probe_radius, active_atoms, false,
                                   progress, editor);
    ...
    for (unsigned int j = 0; j < model_vector[i].molecule.size (); j++)
      {
        for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size ();
             k++)
          {
            ...
            // this_atom->active ? "active" : "not active"); fflush(stdout);
                ...
                for (unsigned int m = 0; m < 3; m++)
                  {
                    molecular_cog[m]
                        += this_atom->coordinate.axis[m]
                           * (this_atom->mw + this_atom->ionized_mw_delta);
                  }
                        ...
                        if (sidechain_N && this_atom->chain == 1)
                          {
                            ...
                            this_atom->bead_ionized_mw_delta
                                += sidechain_N->bead_ionized_mw_delta;
  ...
  // for ( int j = 0; j < (int) model_vector[ current_model ].molecule.size ();
  // ++j) {
  //    for ( int k = 0; k < (int) model_vector[ current_model ].molecule[ j
  //    ].atom.size (); ++k) {
  //       PDB_atom *this_atom = &(model_vector[ current_model ].molecule[ j
  //       ].atom[ k ]); if ( this_atom->active &&
  //            this_atom->is_bead ) {
                ...
                && this_atom->chain == 1 && this_atom->resName == "PRO")
              {
                ...
                if (multi_residue_map.count ("N1-")
                    ...
                    && multi_residue_map["PRO"].size ())
                  {
                    ...
                    delta_mw = residue_list[index1].r_atom[0].hybrid.mw
                               + residue_list[index1]
                                     .r_atom[0]
                                     .hybrid.ionized_mw_delta
            ...
            if (this_atom->name == "OXT" && last_main_chain_bead)
              {
                ...
                if (misc.pb_rule_on && this_atom->resName != "PRO"
                    ...
                    && multi_residue_map.count ("NPBR-OXT"))
                  {
                    ...
                    this_atom->bead_ref_ionized_mw_delta
                        = residue_list[posNPBR_OXT].r_bead[0].ionized_mw_delta;
                  }

                last_main_chain_bead->bead_ref_volume
                    = this_atom->bead_ref_volume;
                last_main_chain_bead->bead_ref_mw = this_atom->bead_ref_mw;
                last_main_chain_bead->bead_ref_ionized_mw_delta
                    = this_atom->bead_ref_ionized_mw_delta;
                ...
                if (multi_residue_map["PBR-NO-OXT"].size () == 1)
                  {
                    ...
                    this_atom->bead_ref_ionized_mw_delta
                        = residue_list[pos].r_bead[0].ionized_mw_delta;
                    this_atom->bead_computed_radius
...
#warning broken by resSeq->QString
  ...
  write_bead_asa (somo_dir + SLASH + project
...
#if defined(DEBUG_MOD)
  write_bead_tsv (somo_tmp_dir + SLASH + "bead_model_debug" + DOTSOMO + ".tsv",
                  &dbg_model);
#endif
      ...
      do
        {
          ...
          if (!no_ovlp_removal)
            {
              lbl_core_progress->setText (
                  QString ("Stage %1 popping iteration %2")
                      .arg (k + 1)
                      .arg (iter));
            }
          qApp->processEvents ();
                  ...
                  if (!no_ovlp_removal)
                    {
                      lbl_core_progress->setText (
                          QString (
                              "Stage %1 popping iteration %2 beads popped %3")
                              .arg (k + 1)
                              .arg (iter)
                              .arg (beads_popped));
                    }
      ...
      if (methods[k] & RADIAL_REDUCTION)
        {
          ...
          if (methods[k] & RR_HIERC)
          ...
          else
            {
              ...
              lbl_core_progress->setText (
                  QString ("Stage %1 synchronous radial reduction")
                      .arg (k + 1));
              qApp->processEvents ();
#if defined(USE_THREADS)
              unsigned int threads = numThreads;
...
void
US_Hydrodyn::bead_check (bool use_threshold, bool message_type, bool vdw,
                         bool only_asa)
{
  // recheck beads here
  ...
  if (asa.method == 1)
    {
      // rolling sphere
      retval = us_hydrodyn_asab1_main (active_atoms, &asa, &results, true,
                                       progress, editor, this);
    }
  else
    {
      ...
      retval = surfracer_main (asa.probe_radius, active_atoms, true, progress,
                               editor);
    }
  ...
  if (only_asa)
    {
      ...
      editor_msg ("dark blue", QString ().sprintf ("Recomputed ASA of beads:\n"
                                                   "Total ASA: %f\n"
                                                   "Used ASA: %f\n",
                                                   total_asa, used_asa));
      return;
  ...
  if (vdw)
    {
      for (unsigned int i = 0; i < bead_model.size (); i++)
        {
          QString msg = "";
          float surface_area = (asa.vdw_grpy_probe_radius
                                + bead_model[i].bead_computed_radius)
                               * (asa.vdw_grpy_probe_radius
                                  + bead_model[i].bead_computed_radius)
                               * 4 * M_PI;
          if (bead_model[i].bead_recheck_asa
              < (asa.vdw_grpy_threshold_percent / 100.0) * surface_area)
            {
              // now buried
      ...
      if (b2e > 0)
        {
          editor_msg ("black",
                      QString ("%1 buried beads became exposed\n").arg (b2e));
        }
      ...
      for (unsigned int i = 0; i < bead_model.size (); i++)
        {
          float surface_area
              = (asa.probe_radius + bead_model[i].bead_computed_radius)
                * (asa.probe_radius + bead_model[i].bead_computed_radius) * 4
                * M_PI;
          QString msg = "";
          if (use_threshold
  ...
  write_bead_model (somo_dir + SLASH + project
                        + QString ("_%1").arg (model_name (current_model))
                        + QString (bead_model_suffix.length ()
                                       ? ("-" + bead_model_suffix)
                                       : "")
...
double
US_Hydrodyn::total_volume_of_bead_model (vector<PDB_atom> &bead_model)
{
  double tot_vol = 0e0;
...
unsigned int
US_Hydrodyn::number_of_active_beads (vector<PDB_atom> &bead_model)
{
...
bool
US_Hydrodyn::radii_all_equal (vector<PDB_atom> &bead_model)
{
