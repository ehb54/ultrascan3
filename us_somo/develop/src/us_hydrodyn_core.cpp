...
// dynamic browflex computations us_hydrodyn_anaflex_core.cpp contains the main
// computational routines for brownian dynamic (anaflex) computations
...
void US_Hydrodyn::get_atom_map(PDB_model *model) {
  ...
  for (unsigned int j = 0; j < model->molecule.size(); j++) {
    ...
    if (lastResSeq != "") {
      atom_counts[QString("%1|%2|%3").arg(j).arg(lastResName).arg(lastResSeq)] =
          atom_count;
    }
  }
  ...
  for (unsigned int j = 0; j < model->molecule.size(); j++) {
      ...
      for (unsigned int k = 0; k < model->molecule[j].atom.size(); k++) {
        ...
        if (lastResSeq != this_atom->resSeq ||
          ...
          if ((lastResSeq != "" &&
            ...
            if (lastResSeq != "") {
              broken_chain_end
                  [QString("%1|%2").arg(lastResSeq).arg(lastResName)] = true;
            } else {
            ...
            //   residue_list[multi_residue_map[lastResName][0]].type == 0 )
            {
              ...
              editor->append(
                  lastResSeq != ""
                      ? QString(us_tr("Warning: break in residue sequence or "
                      ...
                      : QString(
                            us_tr(
                                "Warning: break in residue sequence or unknown "
                                "residue: %1Molecule %2 Residue %3 %4."))
                            .arg(this_atom->chainID == " "
                                     ? ""
                                     : ("Chain " + this_atom->chainID + " "))
                            .arg(j + 1)
    ...
    if (aa && non_aa) {
      ...
      editor->append(
          QString(us_tr("Notice: %1found %2 non or unknown Amino Acids in a "
...
void US_Hydrodyn::build_molecule_maps(PDB_model *model) {
  ...
  for (unsigned int j = 0; j < model->molecule.size(); j++) {
    for (unsigned int k = 0; k < model->molecule[j].atom.size(); k++) {
      QString idx =
          QString("%1|%2").arg(j).arg(model->molecule[j].atom[k].resSeq);
    ...
    if (multi_residue_map.count(resName)) {
      ...
      for (unsigned int j = 0; j < multi_residue_map[resName].size(); j++) {
        ...
        for (unsigned int r = 0;
             ...
             r++) {
          residue_list[multi_residue_map[resName][j]].r_atom[r].tmp_used =
              false;
        }
        skip_residue[QString("%1|%2").arg(idx).arg(
            multi_residue_map[resName][j])] = false;

        // now set flags
        // first check for non-coded atoms
        for (unsigned int k = 0; k < molecules_residues_atoms[idx].size();
             k++) {
          bool found = false;
          bool any = false;
          for (unsigned int r = 0;
        ...
        for (unsigned int r = 0;
          ...
          if (!residue_list[multi_residue_map[resName][j]].r_atom[r].tmp_used) {
            error_msg += QString("Missing atom %1. ")
                             .arg(residue_list[multi_residue_map[resName][j]]
                                      .r_atom[r]
                                      .name);
            if (missing_count != -1) {
              missing_atoms.push_back(
                  residue_list[multi_residue_map[resName][j]].r_atom[r].name);
              missing_atoms_beads.push_back(
                  residue_list[multi_residue_map[resName][j]]
                      .r_atom[r]
                      .bead_assignment);
              molecules_residue_missing_atoms_skip
                  [QString("%1|%2|%3").arg(idx).arg(j).arg(r)] = true;
              missing_count++;
            }
          }
        }
    ...
    }
    {
      ...
      if (k > -1 && molecules_residue_missing_atoms[idx][k].size() == 0 &&
        ...
        if (advanced_config.debug_1) {
          cout << QString(
                      "Molecule idx <%1> resName <%2> pos <%3> no errors, "
                      "should be ok\n")
                      .arg(idx)
                      .arg(resName)
                      .arg(k);
        ...
        for (unsigned int j = 0; j < multi_residue_map[resName].size(); j++) {
          if (j != (unsigned int)k) {
            skip_residue[QString("%1|%2").arg(idx).arg(
                multi_residue_map[resName][j])] = true;
            if (advanced_config.debug_1) {
  ...
  if (advanced_config.debug_1) {
    ...
    for (unsigned int i = 0; i < molecules_idx_seq.size(); i++) {
      ...
      if (molecules_residue_errors[idx].size()) {
        for (unsigned int j = 0; j < molecules_residue_errors[idx].size();
             j++) {
          if (molecules_residue_errors[idx][j].length()) {
            cout << QString(
                        "Molecule idx <%1> resName <%2> match <%3> errors:\n")
                        .arg(idx)
                        .arg(resName)
                        .arg(j);
            ...
            cout
                << QString(
                       "Molecule idx <%1> resName <%2> match <%3> good match\n")
                       .arg(idx)
                       .arg(resName)
                       .arg(j);
          }
        ...
        if (molecules_residue_min_missing[idx] > -1) {
          int k = molecules_residue_min_missing[idx];
          cout << QString(
                      "^^^ Minimum missing position <%1> ma.sz %2 mab.sz %3\n")
                      .arg(k)
                      .arg(molecules_residue_missing_atoms[idx][k].size())
                      .arg(
                          molecules_residue_missing_atoms_beads[idx][k].size());
          for (unsigned int j = 0;
               j < molecules_residue_missing_atoms[idx][k].size(); j++) {
...
int US_Hydrodyn::check_for_missing_atoms(QString *error_string,
                                         PDB_model *model) {
    ...
    for (unsigned int k = 0; k < model->molecule[j].atom.size(); k++) {
      ...
      for (unsigned int m = 0; m < residue_list.size(); m++) {
        if (residue_list[m].name == this_atom->resName &&
          ...
          if (advanced_config.debug_1) {
            cout << QString("(check for missing atoms) skipping %1 %2\n")
                        .arg(res_idx)
                        .arg(m)
                 << endl;
          }
...
#if defined(DEBUG_MULTI_RESIDUE)
          printf(
              "atom name %s residue name %s pos %u atom.size() %u map %s size "
              "%u has oxt %d\n",
              this_atom->name.toLatin1().data(),
    ...
    if (lastResPos != -1) {
      ...
      for (unsigned int l = 0; l < residue_list[lastResPos].r_atom.size();
        ...
        if (!residue_list[lastResPos].r_atom[l].tmp_flag &&
          ...
          if (advanced_config.debug_1) {
            printf(
                "dbg x pdb_parse.missing_atoms %d bead_exceptions[%s] %d "
                "residue_list[%d].r_atom[%d].tmp_flag %d\n",
                pdb_parse.missing_atoms, count_idx.toLatin1().data(),
...
// #define DEBUG_ABB_VBAR
                ...
                us_qdebug(QString("residue list push back, residue_list.size() "
                                  "%1 residue_name %2")
                  ...
                  residue_list[residue_list.size() - 1];
              {
                ...
                if (!residue_atom_hybrid_map.count(mapkey) &&
                  ...
                  // residue_atom_abb_hybrid_map[ atom_to_add.name ]; us_qdebug(
                  // QString( "hybrid map created for %1 created as %2" ).arg(
                  // mapkey ).arg( residue_atom_abb_hybrid_map[atom_to_add.name
                  // ] ) );
                  residue_atom_hybrid_map[mapkey] = "ABB";
                  {
                    QString mapkey_no_nc = QString("%1|%2")
                                               .arg(this_atom->resName)
                                               .arg(atom_to_add.name);
                  ...
                  us_qdebug(QString("hybrid map created for %1 created as %2")
                                .arg(mapkey)
                                .arg("ABB"));
                }
              }
            ...
            case 4:  // missing atoms
            {
              ...
              if (pos < 0) {
                cout << QString(
                            "Internal error: idx %1 Missing atom error %2 ! < "
                            "0!\n")
                            .arg(idx)
                            .arg(pos);
                exit(-1);
              ...
              if (this_atom->resSeq != lastResSeq) {
                ...
                if (one_bead) {
                  ...
                  for (unsigned int i = 0;
                    ...
                    new_bead.ionized_mw_delta +=
                        residue_list[orgrespos].r_bead[i].ionized_mw_delta;
                    new_bead.hydration +=
                  ...
                  for (unsigned int i = 0;
                    ...
                    if (!molecules_residue_missing_atoms_skip
                      ...
                      new_atom.hybrid.ionized_mw_delta *=
                          atoms_scale_weight[0];  // misc.avg_mass;
                      new_atom.hybrid.ionized_mw_delta =
                          (int)(new_atom.hybrid.ionized_mw_delta * 100 + .5) /
                          100.0;
                ...
                if (new_residue_name != new_residue.name) {
                  US_Static::us_message(
                      us_tr("Internal error:"),
                      us_tr(
                          "Unexpected new residue name inconsistancy (a).\n") +
                          QString("\"%1\" != \"%2\"")
                              .arg(new_residue_name)
                              .arg(new_residue.name));
                  exit(-1);
  ...
  if (f.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    for (unsigned int i = 0; i < residue_list.size(); i++) {
      ...
      ts << residue_list[i].name.toUpper() << "\t" << residue_list[i].type
         ...
         << residue_list[i].r_bead.size() << "\t" << residue_list[i].vbar
         << Qt::endl;
        ...
        if (bead_model[i].bead_computed_radius > tolerance * 1.001 &&
          ...
          if (limit && retval > limit) {
            if (!from_overlap_hydro) {
              editor_msg("red",
                         us_tr("There are more than %1 overlaps greater than "
                               "tolerance of %2\nWe suggest you use ZENO or "
                               "GRPY to calculate hydrodynamics for this model")
                             .arg(limit)
                             .arg(tolerance));
            }
            return retval;
          ...
          if (!from_overlap_hydro) {
            editor_msg("red",
                       QString(us_tr("WARNING: Bead model has an overlap "
...
int US_Hydrodyn::create_beads(QString *error_string, bool quiet) {
  ...
  map<QString, QString> hybrid_name_to_N = {{"N3H0", "N1-"}, {"N3H1", "N1"}};
      ...
      if (respos != -1) {
        ...
        if (atompos == -1) {
          ...
          this_atom->ionized_mw_delta =
              residue_list[respos].r_atom[atompos].hybrid.ionized_mw_delta;
          ...
          this_atom->placing_method =
              this_atom->p_residue->r_bead[this_atom->p_atom->bead_assignment]
                  .placing_method;
          if (this_atom->p_residue->r_bead[this_atom->p_atom->bead_assignment]
                  .hydration_flag) {
          ...
          if (saxs_options.compute_saxs_coeff_for_bead_models) {
            ...
            if (!residue_atom_hybrid_map.count(mapkey)) {
              ...
              editor->setTextColor("red");
              editor->append(
                  QString("%1Molecule %2 Residue %3|%4 %5 Hybrid name missing. "
              ...
              if (hybrid_name.isEmpty() || !hybrid_name.length()) {
                cout << "error: hybrid name empty for " << this_atom->resName
                     << "|" << this_atom->name << endl;
                QColor save_color = editor->textColor();
                editor->setTextColor("red");
                editor->append(
                    QString("%1Molecule %2 Residue %3|%4 %5 Hybrid name "
                            "missing. Atom skipped for SAS.\n")
                        .arg(this_atom->chainID == " "
                                 ? ""
                                 : ("Chain " + this_atom->chainID + " "))
                        .arg(j + 1)
                ...
                if (!saxs_util->hybrid_map.count(hybrid_name)) {
                  ...
                  editor->append(
                      QString(
                          ...
                          "from Hybrid file. Atom skipped for SAS.\n")
                          .arg(this_atom->chainID == " "
                                   ? ""
                                   : ("Chain " + this_atom->chainID + " "))
                          .arg(j + 1)
                  ...
                  if (!saxs_util->atom_map.count(this_atom->name + "~" +
                    ...
                    cout << "error: atom_map missing for hybrid_name "
                         << hybrid_name << " atom name " << this_atom->name
                         << endl;
                    QColor save_color = editor->textColor();
                    editor->setTextColor("red");
                    editor->append(
                        QString("%1Molecule %2 Atom %3 Residue %4 %5 Hybrid %6 "
                                "name missing from Atom file. Atom skipped for "
                                "SAS.\n")
                            .arg(this_atom->chainID == " "
                                     ? ""
                                     : ("Chain " + this_atom->chainID + " "))
                    ...
                    this_atom->saxs_excl_vol =
                        saxs_util->atom_map[this_atom->name + "~" + hybrid_name]
                            .saxs_excl_vol;
                    if (!saxs_util->saxs_map.count(
                      ...
                      editor->setTextColor("red");
                      editor->append(
                          QString("%1Molecule %2 Residue %3 %4 Hybrid %5 Saxs "
                                  "name %6 name missing from SAXS atom file. "
                                  "Atom skipped.\n")
                              .arg(this_atom->chainID == " "
                                       ? ""
                                       : ("Chain " + this_atom->chainID + " "))
                              ...
                              .arg(this_atom->resSeq)
                              .arg(hybrid_name)
                              .arg(saxs_util->hybrid_map[hybrid_name]
                                       .saxs_name));
                      editor->setTextColor(save_color);
                    } else {
                      this_atom->saxs_data =
                          saxs_util->saxs_map[saxs_util->hybrid_map[hybrid_name]
                                                  .saxs_name];
                    }
                  }
                }
          ...
          if (!k && this_atom->name == "N" &&
            ...
            if (hybrid_name_to_N.count(this_atom->p_atom->hybrid.name) &&
              ...
              int index = multi_residue_map
                  [hybrid_name_to_N[this_atom->p_atom->hybrid.name]][0];
          ...
          if (advanced_config.debug_1 || advanced_config.debug_2) {
            printf(
                "found atom %s %s in residue %d pos %d bead asgn %d %d placing "
                "info %d mw %f bead_ref_mw %f hybrid name %s %s ba %d chain "
                "%d\n",
                this_atom->name.toLatin1().data(),
                this_atom->resName.toLatin1().data(), respos, atompos,
                this_atom->p_atom->bead_assignment, this_atom->bead_assignment,
                this_atom->p_residue->r_bead[this_atom->p_atom->bead_assignment]
                    .placing_method,
                ...
                this_atom->p_atom->bead_assignment, this_atom->chain);
          }
        }
      }
      ...
      for (unsigned int k = 0;
        ...
        if (advanced_config.debug_1 || advanced_config.debug_2) {
          printf("p1 %s j k %d %d %p %s %d chain %d\n",
                 this_atom->name.toLatin1().data(), j, k, this_atom->p_atom,
...
#define DO_SORT
      ...
      for (unsigned int j = 0; j < model_vector[current_model].molecule.size();
        ...
        for (unsigned int k = 0;
          ...
          printf("p1 %s j k %d %d %p %s %d %d\n",
                 this_atom->name.toLatin1().data(), j, k, this_atom->p_atom,
                 this_atom->active ? "active" : "not active",
                 this_atom->bead_assignment, this_atom->atom_assignment);
          fflush(stdout);
        }
      }
    }
  ...
  if (advanced_config.debug_1) {
    ...
    printf("~~tot atom %d mw %f tot bead %d mw %f\n", atom_count, tot_atom_mw,
           bead_count, tot_bead_mw);
  }
  ...
  for (int j = 0; j < chains; ++j) {
    ...
    if (atoms) {
      ...
      if (first_atom_map.count("N") &&
          ...
          multi_residue_map[hybrid_name_to_N[first_atom_map["N"]->hybrid.name]]
              .size()) {
        ...
        if (atompos_map.count("N")) {
          ...
          model.molecule[j].atom[atompos].ionized_mw_delta =
              model.molecule[j].atom[atompos].p_atom->hybrid.ionized_mw_delta;
        } else {
          QTextStream(stderr)
              << "US_Hydrodyn::fix_N1_non_pbr no 'N' atom found in PDB_chain";
        }
      }
    }
  }
      ...
      if (1 || (*p_last_reduced)[i]) {
        for (unsigned int j = i + 1; j < (*p_bead_model).size(); j++) {
          if ((1 || (*p_last_reduced)[j]) && (*p_bead_model)[i].active &&
              ...
              (methodk & RR_MCSC ||
               ((methodk & RR_SC) && (*p_bead_model)[i].chain == 1 &&
            ...
            float separation =
                ...
                sqrt(pow((*p_bead_model)[i].bead_coordinate.axis[0] -
                             (*p_bead_model)[j].bead_coordinate.axis[0],
                         2) +
                     pow((*p_bead_model)[i].bead_coordinate.axis[1] -
                             (*p_bead_model)[j].bead_coordinate.axis[1],
                         2) +
                     pow((*p_bead_model)[i].bead_coordinate.axis[2] -
                             (*p_bead_model)[j].bead_coordinate.axis[2],
                         2));
...
void US_Hydrodyn::radial_reduction(bool from_grid) {
  ...
  stage_loop:
    ...
    do {
      ...
      lbl_core_progress->setText(
          QString("Stage %1 popping iteration %2").arg(k + 1).arg(iter));
      qApp->processEvents();
...
#endif
#if defined(WRITE_EXTRA_FILES)
    write_bead_tsv(somo_tmp_dir + SLASH + QString("bead_model_pop-%1").arg(k) +
                       DOTSOMO + ".tsv",
                   &bead_model);
    write_bead_spt(
        somo_tmp_dir + SLASH + QString("bead_model_pop-%1").arg(k) + DOTSOMO,
        &bead_model);
#endif
    ...
    if (methods[k] & RADIAL_REDUCTION) {
      ...
      if (methods[k] & RR_HIERC) {
...
#endif
        lbl_core_progress->setText(
            QString("Stage %1 hierarchical radial reduction").arg(k + 1));
        qApp->processEvents();
...
#endif
          lbl_core_progress->setText(
              QString("Stage %1 hierarch. red. it. %2 with %3 couples")
                  .arg(k + 1)
                  .arg(iter)
                  .arg(pairs.size()));
          qApp->processEvents();
        ...
        lbl_core_progress->setText(
            QString("Stage %1 sychronous radial reduction").arg(k + 1));
        qApp->processEvents();
...
#endif
          lbl_core_progress->setText(
              QString("Stage %1 synchron. red. it. %2 with %3 couples")
                  .arg(k + 1)
                  .arg(iter)
                  .arg(pairs.size()));
          qApp->processEvents();
...
#endif
#if defined(WRITE_EXTRA_FILES)
    write_bead_tsv(somo_tmp_dir + SLASH + QString("bead_model_rr-%1").arg(k) +
                       DOTSOMO + ".tsv",
                   &bead_model);
    write_bead_spt(
        somo_tmp_dir + SLASH + QString("bead_model_rr-%1").arg(k) + DOTSOMO,
        &bead_model);
#endif
  }  // methods
#if defined(WRITE_EXTRA_FILES)
  ...
  QString filename = project + QString("_%1").arg(model_name(current_model));
...
int US_Hydrodyn::compute_asa(bool bd_mode, bool no_ovlp_removal) {
  ...
  if (check_for_missing_atoms(&error_string, &model_vector[current_model])) {
    editor->append(
        ...
        error_string);
    printError(
        "Encountered errors with your PDB structure:\n"
  ...
  if (asa.method == 0) {
    ...
    int retval =
        surfracer_main(asa.probe_radius, active_atoms, false, progress, editor);
        ...
        if (this_atom->active) {
          if (advanced_config.debug_1) {
            printf("pass 2 active %s %s %d pm %d %d\n",
                   ...
                   this_atom->placing_method, this_atom->bead_assignment);
            fflush(stdout);
          }
          molecular_mw += this_atom->mw + this_atom->ionized_mw_delta;
          for (unsigned int m = 0; m < 3; m++) {
            molecular_cog[m] += this_atom->coordinate.axis[m] *
          ...
          if (!create_beads_normally || this_atom->bead_positioner) {
            if (advanced_config.debug_3) {
              printf(
                  "adding cog from %d to %d mw %f totmw %f (this pos "
                  "[%f,%f,%f], org pos [%f,%f,%f])\n",
                  this_atom->serial, use_atom->serial, this_atom->mw,
                  use_atom->bead_cog_mw, this_atom->coordinate.axis[0],
            ...
            if (advanced_config.debug_3) {
              printf(
                  "notice: atom %s %s %d excluded from cog calculation in bead "
                  "%s %s %d\n",
                  this_atom->name.toLatin1().data(),
                  this_atom->resName.toLatin1().data(), this_atom->serial,
      ...
      for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size();
        ...
        if (advanced_config.debug_1) {
          printf("pass 2b active %d is_bead %d chain %d atom %s %s %d pm %d\n",
                 this_atom->active, this_atom->is_bead, this_atom->chain,
                 this_atom->name.toLatin1().data(),
                 this_atom->resName.toLatin1().data(), this_atom->serial,
                 this_atom->placing_method);
          fflush(stdout);
        ...
        if (this_atom->name == "OXT" && last_main_chain_bead) {
          if (advanced_config.debug_1) {
            printf(
                "pass 2b active OXT %s %s %d last %s %s %d mw org %f mw new "
                "%f\n",
                this_atom->name.toLatin1().data(),
                this_atom->resName.toLatin1().data(), this_atom->serial,
                last_main_chain_bead->name.toLatin1().data(),
                last_main_chain_bead->resName.toLatin1().data(),
                last_main_chain_bead->serial, last_main_chain_bead->bead_ref_mw,
                this_atom->bead_ref_mw);
            fflush(stdout);
          }
          this_atom->is_bead = false;
        ...
        if (misc.pb_rule_on && this_atom->p_residue->type == 0 &&
          ...
          if (advanced_config.debug_1) {
            printf(
                "pass 2b broken end adjustment %s %s %s org mw %.4f vol %.4f\n",
                this_atom->name.toLatin1().data(),
                this_atom->resName.toLatin1().data(),
                this_atom->resSeq.toLatin1().data(), this_atom->bead_ref_mw,
                this_atom->bead_ref_volume);
          }
          if (multi_residue_map["PBR-NO-OXT"].size() == 1) {
            ...
            if (advanced_config.debug_1) {
              printf(
                  "pass 2b broken end adjustment %s %s %s new mw %.4f vol "
                  "%.4f\n",
                  this_atom->name.toLatin1().data(),
                  this_atom->resName.toLatin1().data(),
                  this_atom->resSeq.toLatin1().data(), this_atom->bead_ref_mw,
  ...
  stage_loop:
    ...
    if (methods[k] & RADIAL_REDUCTION) {
      ...
      if (methods[k] & RR_HIERC) {
        ...
        lbl_core_progress->setText(
            QString("Stage %1 synchronous radial reduction").arg(k + 1));
        qApp->processEvents();
#if defined(USE_THREADS)
        unsigned int threads = numThreads;
        vector<radial_reduction_thr_t *> radial_reduction_thr_threads(threads);
...
void US_Hydrodyn::bead_check(bool use_threshold, bool message_type, bool vdw,
  ...
  if (asa.method == 1) {
    ...
    retval =
        surfracer_main(asa.probe_radius, active_atoms, true, progress, editor);
  }
  ...
  if (retval) {
    switch (retval) {
      case US_HYDRODYN_ASAB1_ERR_MEMORY_ALLOC: {
        printError(
            "US_HYDRODYN_ASAB1 on bead recheck encountered a memory allocation "
            "error");
  ...
  if (vdw) {
    for (unsigned int i = 0; i < bead_model.size(); i++) {
      ...
      float surface_area =
          ...
          (asa.vdw_grpy_probe_radius + bead_model[i].bead_computed_radius) * 4 *
          M_PI;
    ...
    for (unsigned int i = 0; i < bead_model.size(); i++) {
      ...
      QString msg = "";
      if (use_threshold ? (bead_model[i].bead_recheck_asa > asa.threshold)
      ...
      printf("bead %d %.2f %.2f %.2f %s %s bead mw %.2f bead ref mw %.2f\n", i,
             ...
             bead_model[i].bead_recheck_asa,
             (use_threshold ? (bead_model[i].bead_recheck_asa > asa.threshold)
                            : (bead_model[i].bead_recheck_asa >
                               (asa.threshold_percent / 100.0) * surface_area))
                 ? "exposed"
                 : "buried",
