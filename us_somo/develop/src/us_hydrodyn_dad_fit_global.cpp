                                                                   ...
                                                                   ) : QDialog( p )
{
   ...
   if ( use_errors &&
        ( dad_win->unified_ggaussian_e.size() != dad_win->unified_ggaussian_q.size() ||
...
void US_Hydrodyn_Dad_Fit_Global::restore()
{
   ...
   if ( update_dad )
   {
      ...
      dad_win->ggqfit_plot->detachItems( QwtPlotItem::Rtti_PlotCurve ); dad_win->ggqfit_plot->detachItems( QwtPlotItem::Rtti_PlotMarker );;
...
void US_Hydrodyn_Dad_Fit_Global::undo()
{
        ...
        DFIT_GLOBAL::init_params.size() == test_widgets.size() )
   {
      ...
      {
         ((QLineEdit *)test_widgets[ i ])->setText( QString( "%1" ).arg( DFIT_GLOBAL::init_params[ i ] ) );
      }
      update_test_info();
   }
}
...
void US_Hydrodyn_Dad_Fit_Global::update_test_info()
{
...

   {
      ...
      for ( unsigned int i = 0; i < dad_win->unified_ggaussian_gaussians_size; ++i ) {
         ...
         for ( unsigned int j = 0; j < dad_win->common_size; ++j ) {
            qs_uggs += QString( "%1 " ).arg(  dad_win->unified_ggaussian_params[ p++ ], 0, 'g', 6 );
         } 
         qs_uggs += "\n";
      }
      ...
      for ( unsigned int i = 0; i < dad_win->unified_ggaussian_curves; ++i ) {
         ...
         for ( unsigned int k = 0; k < dad_win->unified_ggaussian_gaussians_size; ++k ) {
   ...
   lbl_test_info->setText( 
                          ...
                          .arg( US_Vector::qs_vector( "unified ggaussians",dad_win->unified_ggaussian_params, 6, 20 ) )
                          .arg( qs_uggs )
...
bool US_Hydrodyn_Dad_Fit_Global::setup_run()
{
   ...
   DFIT_GLOBAL::unified_ggaussian_param_index     = dad_win->unified_ggaussian_param_index;
   DFIT_GLOBAL::unified_ggaussian_gaussians_size  = dad_win->unified_ggaussian_gaussians_size;
   ...
   // base_params : these are a copy of the init_params without replacement from the dad_win unified_ggaussians
   ...
   for ( unsigned int i = 0; i < dad_win->unified_ggaussian_gaussians_size; i++ )
   {
           ...
           fixed_curves.count( i ) )
      {
         ...
         DFIT_GLOBAL::fixed_params.push_back( dad_win->unified_ggaussian_params[ ofs ] );
         ...
         DFIT_GLOBAL::base_params .push_back( dad_win->unified_ggaussian_params[ ofs ] );
      ...
      if ( dist1_active && dad_win->cb_fix_dist1->isChecked() ) // if dist1s are common
      {
              ...
              ( cb_comm_dist1->isChecked() && i ) )
         {
            ...
            {
               DFIT_GLOBAL::fixed_params.push_back( dad_win->unified_ggaussian_params[ this_ofs ] );
               DFIT_GLOBAL::comm_backref[ DFIT_GLOBAL::param_fixed.size() ] = comm_dist_1_var_pos;
            } else {
               DFIT_GLOBAL::fixed_params.push_back( dad_win->unified_ggaussian_params[ ofs ] );
            }
            DFIT_GLOBAL::param_fixed .push_back( true );
         ...
         if ( dist2_active && dad_win->cb_fix_dist2->isChecked() ) // if dist2s are common
         {
                 ...
                 ( cb_comm_dist2->isChecked() && i ) )
            {
               ...
               {
                  DFIT_GLOBAL::fixed_params.push_back( dad_win->unified_ggaussian_params[ this_ofs ] );
                  DFIT_GLOBAL::comm_backref[ DFIT_GLOBAL::param_fixed.size() ] = comm_dist_2_var_pos;
               } else {
                  DFIT_GLOBAL::fixed_params.push_back( dad_win->unified_ggaussian_params[ ofs ] );
               }
               DFIT_GLOBAL::param_fixed .push_back( true );
   ...
   unsigned int base_ofs = dad_win->unified_ggaussian_gaussians_size * common_size;
   // assert( (unsigned int)DFIT_GLOBAL::param_pos.size() == base_ofs && "common_size*number of gaussians should equal param_pos.size()" );

   for ( unsigned int f = 0; f < dad_win->unified_ggaussian_curves; f++ )
   {
      unsigned int ofs = base_ofs + f * per_file_size * dad_win->unified_ggaussian_gaussians_size;
      ...
      for ( unsigned int i = 0; i < dad_win->unified_ggaussian_gaussians_size; i++ )
      {
         ...
         if ( dist1_active && !dad_win->cb_fix_dist1->isChecked() ) // if dist1s are not common
         {
            if ( cb_fix_dist1->isChecked() ||
                 fixed_curves.count( i ) ||
                 ( cb_comm_dist1->isChecked() && i ) )
   ...
   if ( dad_win->cb_fix_width->isChecked() )
   {
      ...
      for ( unsigned int i = 0; i < dad_win->unified_ggaussian_gaussians_size; i++ )
      {
         ...
         {
            DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::fixed_params.size() );
            DFIT_GLOBAL::fixed_params.push_back( dad_win->unified_ggaussian_params[ 2 * i + 0 ] );
            DFIT_GLOBAL::param_fixed .push_back( true );
         } else {
            DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::init_params.size() );
            ...
            DFIT_GLOBAL::init_params .push_back( base_val );
            DFIT_GLOBAL::base_params .push_back( dad_win->unified_ggaussian_params[ 2 * i + 0 ] );
            DFIT_GLOBAL::param_fixed .push_back( false );
         ...
         {
            DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::fixed_params.size() );
            DFIT_GLOBAL::fixed_params.push_back( dad_win->unified_ggaussian_params[ 2 * i + 1 ] );
            DFIT_GLOBAL::param_fixed .push_back( true );
         } else {
            DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::init_params.size() );
            ...
            DFIT_GLOBAL::init_params .push_back( base_val );
            DFIT_GLOBAL::base_params .push_back( dad_win->unified_ggaussian_params[ 2 * i + 1] );
            DFIT_GLOBAL::param_fixed .push_back( false );
      ...
      for ( unsigned int i = 0; i < dad_win->unified_ggaussian_gaussians_size * dad_win->unified_ggaussian_curves; i++ )
      {
         unsigned int pos = i / dad_win->unified_ggaussian_curves;

         unsigned int height_pos = 2 * dad_win->unified_ggaussian_gaussians_size + i + 0;

         if ( cb_fix_amplitude->isChecked() ||
              fixed_curves.count( pos ) )
         {
            DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::fixed_params.size() );
            DFIT_GLOBAL::fixed_params.push_back( dad_win->unified_ggaussian_params[ height_pos ] );
            DFIT_GLOBAL::param_fixed .push_back( true );
         } else {
            DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::init_params.size() );
            ...
            DFIT_GLOBAL::init_params .push_back( base_val );
            DFIT_GLOBAL::base_params .push_back( dad_win->unified_ggaussian_params[ height_pos ] );
            DFIT_GLOBAL::param_fixed .push_back( false );
      ...
      for ( unsigned int i = 0; i < dad_win->unified_ggaussian_gaussians_size; i++ )
      {
              ...
              fixed_curves.count( i ) )
         {
            ...
            DFIT_GLOBAL::init_params .push_back( base_val );
            DFIT_GLOBAL::base_params .push_back( dad_win->unified_ggaussian_params[ i ] );
            DFIT_GLOBAL::param_fixed .push_back( false );
      ...
      for ( unsigned int i = 0; i < dad_win->unified_ggaussian_gaussians_size * 2 * dad_win->unified_ggaussian_curves; i+= 2 )
      {
         unsigned int pos = i / ( 2 * dad_win->unified_ggaussian_curves );

         unsigned int height_pos = dad_win->unified_ggaussian_gaussians_size + i + 0;
         unsigned int width_pos  = dad_win->unified_ggaussian_gaussians_size + i + 1;
         ...
         {
            DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::fixed_params.size() );
            DFIT_GLOBAL::fixed_params.push_back( dad_win->unified_ggaussian_params[ width_pos ] );
            DFIT_GLOBAL::param_fixed .push_back( true );
         } else {
            DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::init_params.size() );
            ...
            DFIT_GLOBAL::init_params .push_back( base_val );
            DFIT_GLOBAL::base_params .push_back( dad_win->unified_ggaussian_params[ width_pos ] );
            DFIT_GLOBAL::param_fixed .push_back( false );
...
void US_Hydrodyn_Dad_Fit_Global::test()
{
   ...
   for ( int i = 0; i < (int) DFIT_GLOBAL::init_params.size(); ++i )
   {
      DFIT_GLOBAL::init_params[ i ] = ((QLineEdit *)test_widgets[ i ])->text().toDouble();
   }
   ...
   for ( unsigned int i = 0; i < DFIT_GLOBAL::param_fixed.size(); i++ )
   {
           ...
           DFIT_GLOBAL::comm_backref.count( i ) )
      {
         ...
         us_qdebug( 
                ...
                .arg( DFIT_GLOBAL::comm_backref.count( i ) ? "backref used" : "direct pos" ) 
                 );
      } else {
...
void US_Hydrodyn_Dad_Fit_Global::lm()
{
   ...
   */
   ...
   {
      gsumf[ i ] = (*DFIT_GLOBAL::compute_gaussian_f)( t[ i ], &DFIT_GLOBAL::init_params[ 0 ] );
   }
...
#undef TOL
   ...
   if ( org_rmsd > status.fnorm )
   {
      ...
      // dad_win->add_ggaussian_curve( "lm_after_pushback", dad_win->compute_ggaussian_gaussian_sum() );
      gaussians_undo.push_back( dad_win->unified_ggaussian_params );
      if ( update_dad )
      {
...
void US_Hydrodyn_Dad_Fit_Global::gsm_sd()
{
   ...
   {
      gsm_yp[ j ]  = DFIT_GLOBAL::compute_gaussian_f( gsm_t[ j ], (double *)(&DFIT_GLOBAL::base_params[ 0 ]) );
...
void US_Hydrodyn_Dad_Fit_Global::stop()
{
   running = false;
}
...
void US_Hydrodyn_Dad_Fit_Global::grid()
{
   ...
   unsigned int total_count = ( unsigned int ) pow( (double) pop_size, (int) DFIT_GLOBAL::init_params.size() );
   ...
   for ( unsigned int j = 0; j < ( unsigned int ) DFIT_GLOBAL::init_params.size(); j++ )
   {
      ofs_per[ j ] = ( DFIT_GLOBAL::param_max[ j ] - DFIT_GLOBAL::param_min[ j ] ) / ( pop_size - 1 );
   }
   ...
   {
      yp[ j ]  = DFIT_GLOBAL::compute_gaussian_f( t[ j ], (double *)(&DFIT_GLOBAL::base_params[ 0 ]) );
