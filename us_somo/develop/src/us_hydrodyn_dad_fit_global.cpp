...
US_Hydrodyn_Dad_Fit_Global::US_Hydrodyn_Dad_Fit_Global(US_Hydrodyn_Dad *dad_win,
...
void US_Hydrodyn_Dad_Fit_Global::restore() {
  ...
  if (update_dad) {
    ...
    dad_win->lbl_gauss_fit->setText(QString("%1").arg(new_rmsd, 0, 'g', 5));
    dad_win->pb_ggauss_rmsd->setEnabled(false);
  }
  update_enables();
}

void US_Hydrodyn_Dad_Fit_Global::undo() {
...
void US_Hydrodyn_Dad_Fit_Global::update_test_info() {
  QString qs_uggs = "";

  {
    ...
    for (unsigned int i = 0; i < dad_win->unified_ggaussian_gaussians_size;
      ...
      for (unsigned int j = 0; j < dad_win->common_size; ++j) {
        qs_uggs += QString("%1 ").arg(dad_win->unified_ggaussian_params[p++], 0,
                                      'g', 6);
      }
      qs_uggs += "\n";
    }
    ...
    for (unsigned int i = 0; i < dad_win->unified_ggaussian_curves; ++i) {
      ...
      for (unsigned int k = 0; k < dad_win->unified_ggaussian_gaussians_size;
        ...
        for (unsigned int j = 0; j < dad_win->per_file_size; ++j) {
          qs_uggs += QString("%1 ").arg(dad_win->unified_ggaussian_params[p++],
                                        0, 'g', 6);
        }
        qs_uggs += "\n";
      }
  ...
  lbl_test_info->setText(
      QString("Per gaussian: common_size: %1 %2 per_file_size: %3 %4\n"
          ...
          .arg(US_Vector::qs_vector("unified ggaussians",
                                    dad_win->unified_ggaussian_params, 6, 20))
          .arg(qs_uggs)
          .arg(DFIT_GLOBAL::qs_params()));
}

void US_Hydrodyn_Dad_Fit_Global::setupGUI() {
  int minHeight1 = 30;
  ...
  */
  ...
  double peak =
      dad_win->compute_gaussian_peak(dad_win->wheel_file, dad_win->gaussians);
  le_epsilon->setText(
...
void US_Hydrodyn_Dad_Fit_Global::cancel() {
...
void US_Hydrodyn_Dad_Fit_Global::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/somo_dad_fit_global.html");
}

void US_Hydrodyn_Dad_Fit_Global::closeEvent(QCloseEvent *e) {
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept();
}

void US_Hydrodyn_Dad_Fit_Global::update_common() {
  if (!setup_run()) {
    update_enables();
    return;
  }
...
void US_Hydrodyn_Dad_Fit_Global::update_enables() {
...
bool US_Hydrodyn_Dad_Fit_Global::setup_run() {
  ...
  switch (gaussian_type) {
    case US_Hydrodyn_Dad::EMGGMG:
      DFIT_GLOBAL::compute_gaussian_f = &DFIT_GLOBAL::compute_gaussian_f_EMGGMG;
      // gsm_f = &gsm_f_EMGGMG;
      break;
    case US_Hydrodyn_Dad::EMG:
  ...
  // these are a copy of the init_params without replacement from the dad_win
  // unified_ggaussians
  ...
  for (unsigned int f = 0; f < dad_win->unified_ggaussian_curves; f++) {
    ...
    for (unsigned int i = 0; i < dad_win->unified_ggaussian_gaussians_size;
      ...
      if (dist1_active &&
          !dad_win->cb_fix_dist1->isChecked())  // if dist1s are not common
      {
        if (cb_fix_dist1->isChecked() || fixed_curves.count(i) ||
            (cb_comm_dist1->isChecked() && i)) {
          DFIT_GLOBAL::param_pos.push_back(DFIT_GLOBAL::fixed_params.size());
  ...
  i++ )
     {
        ...
        {
           DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::fixed_params.size()
  ); DFIT_GLOBAL::fixed_params.push_back( dad_win->unified_ggaussian_params[ 2 *
  i + 0 ] ); DFIT_GLOBAL::param_fixed .push_back( true ); } else {
           DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::init_params.size()
  );
  ...
  i + 1 ] ); DFIT_GLOBAL::param_fixed .push_back( true ); } else {
           DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::init_params.size()
  );
  ...
  + i + 0;
        ...
        {
           DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::fixed_params.size()
  ); DFIT_GLOBAL::fixed_params.push_back( dad_win->unified_ggaussian_params[
  height_pos ] ); DFIT_GLOBAL::param_fixed .push_back( true ); } else {
           DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::init_params.size()
  );
  ...
  i++ )
     {
        ...
        {
           DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::fixed_params.size()
  ); DFIT_GLOBAL::fixed_params.push_back( dad_win->unified_ggaussian_params[ i ]
  ); DFIT_GLOBAL::param_fixed .push_back( true ); } else {
           DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::init_params.size()
  );
           ...
           DFIT_GLOBAL::init_params .push_back( base_val );
           DFIT_GLOBAL::base_params .push_back(
  dad_win->unified_ggaussian_params[ i ] ); DFIT_GLOBAL::param_fixed .push_back(
  false );
     ...
     for ( unsigned int i = 0; i < dad_win->unified_ggaussian_gaussians_size * 2
  * dad_win->unified_ggaussian_curves; i+= 2 )
     {
  ...
  width_pos ] ); DFIT_GLOBAL::param_fixed .push_back( true ); } else {
           DFIT_GLOBAL::param_pos   .push_back( DFIT_GLOBAL::init_params.size()
  );
...
void US_Hydrodyn_Dad_Fit_Global::test_mode() {
  if (!cb_test_mode->isChecked()) {
...
void US_Hydrodyn_Dad_Fit_Global::test() {
    ...
    if (!DFIT_GLOBAL::param_fixed[i] || DFIT_GLOBAL::comm_backref.count(i)) {
      ...
      us_qdebug(QString("param %1 taken from init params pos %2 %3")
                    ...
                    .arg(DFIT_GLOBAL::comm_backref.count(i)
                             ? DFIT_GLOBAL::comm_backref[i]
                             : DFIT_GLOBAL::param_pos[i])
                    .arg(DFIT_GLOBAL::comm_backref.count(i) ? "backref used"
                                                            : "direct pos"));
    } else {
...
void US_Hydrodyn_Dad_Fit_Global::lm() {
  update_common();
  running = true;
  update_enables();
  // setup_run();
  ...
  */
  ...
  for (unsigned int i = 0; i < (unsigned int)t.size(); i++) {
    gsumf[i] =
        (*DFIT_GLOBAL::compute_gaussian_f)(t[i], &DFIT_GLOBAL::init_params[0]);
  }
...
void US_Hydrodyn_Dad_Fit_Global::gsm_sd() {
...
void US_Hydrodyn_Dad_Fit_Global::gsm_ih() {
  return;  // have to rewrite this
...
void US_Hydrodyn_Dad_Fit_Global::gsm_cg() {
  return;  // have to rewrite this
...
void US_Hydrodyn_Dad_Fit_Global::ga() {
  /*
  gsm_setup();
...
void US_Hydrodyn_Dad_Fit_Global::stop() { running = false; }

void US_Hydrodyn_Dad_Fit_Global::grid() {
  return;  // have to rewrite this
  setup_run();
  ...
  unsigned int total_count =
      (unsigned int)pow((double)pop_size, (int)DFIT_GLOBAL::init_params.size());
  ...
  for (unsigned int i = 0; i < total_count; i++) {
    ...
    for (unsigned int j = 0; j < t.size(); j++) {
      yp[j] = DFIT_GLOBAL::compute_gaussian_f(t[j], (double *)(&use_par[0]));
...
void US_Hydrodyn_Dad_Fit_Global::usp_config_plot_test(const QPoint &) {
