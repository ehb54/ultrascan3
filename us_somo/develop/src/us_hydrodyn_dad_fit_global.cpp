...
US_Hydrodyn_Dad_Fit_Global::US_Hydrodyn_Dad_Fit_Global (
    US_Hydrodyn_Dad *dad_win, QWidget *p, const char *)
    : QDialog (p)
{
  ...
  dad_ampl_width_min = ((US_Hydrodyn *)(dad_win->us_hydrodyn))
                           ->gparams["dad_ampl_width_min"]
                           .toDouble ();
  dad_lock_min_retry
      = ((US_Hydrodyn *)(dad_win->us_hydrodyn))->gparams["dad_lock_min_retry"]
                == "true"
            ? true
            : false;
  dad_lock_min_retry_mult = ((US_Hydrodyn *)(dad_win->us_hydrodyn))
                                ->gparams["dad_lock_min_retry_mult"]
                                .toDouble ();
  dad_maxfpk_restart
      = ((US_Hydrodyn *)(dad_win->us_hydrodyn))->gparams["dad_maxfpk_restart"]
                == "true"
            ? true
            : false;
  dad_maxfpk_restart_tries = ((US_Hydrodyn *)(dad_win->us_hydrodyn))
                                 ->gparams["dad_maxfpk_restart_tries"]
                                 .toUInt ();
  dad_maxfpk_restart_pct = ((US_Hydrodyn *)(dad_win->us_hydrodyn))
                               ->gparams["dad_maxfpk_restart_pct"]
                               .toDouble ();
  ...
  // width
  if (dad_win->cb_fix_width->isChecked ())
    {
  ...
  if (dist1_active)
    {
      if (dad_win->cb_fix_dist1->isChecked ())
        {
          is_common.push_back (true);
          offset.push_back (common_size++); // first common entry
        }
      ...
      if (dist2_active)
        {
          if (dad_win->cb_fix_dist2->isChecked ())
            {
              is_common.push_back (true);
              offset.push_back (common_size++); // first common entry
            }
  ...
  gaussians_undo.push_back (dad_win->unified_ggaussian_params);

  setGeometry (global_Xpos, global_Ypos, 0, 0);

  use_errors = dad_win->cb_sd_weight->isChecked ();
  if (use_errors
      && (dad_win->unified_ggaussian_e.size ()
              != dad_win->unified_ggaussian_q.size ()
          || !dad_win->is_nonzero_vector (dad_win->unified_ggaussian_e)))
    {
      use_errors = false;
  ...
  if (use_errors)
    {
      ...
      vector<double> x = dad_win->unified_ggaussian_t;
...
void
US_Hydrodyn_Dad_Fit_Global::restore ()
{
  gaussians_undo.resize (1);
  dad_win->unified_ggaussian_params = gaussians_undo[0];
  if (update_dad)
    {
      dad_win->gauss_init_markers ();
      dad_win->update_gauss_pos ();
      dad_win->lbl_gauss_fit->setText ("?");
      dad_win->pb_ggauss_rmsd->setEnabled (true);
      dad_win->ggqfit_plot->detachItems (QwtPlotItem::Rtti_PlotCurve);
      dad_win->ggqfit_plot->detachItems (QwtPlotItem::Rtti_PlotMarker);
      ;
      dad_win->ggqfit_plot->replot ();
      double new_rmsd = dad_win->ggaussian_rmsd ();
      dad_win->lbl_gauss_fit->setText (
          QString ("%1").arg (new_rmsd, 0, 'g', 5));
      dad_win->pb_ggauss_rmsd->setEnabled (false);
    }
  update_enables ();
}

void
US_Hydrodyn_Dad_Fit_Global::undo ()
{
  ...
  dad_win->unified_ggaussian_params = gaussians_undo.back ();
  if (update_dad)
    {
      dad_win->gauss_init_markers ();
      dad_win->update_gauss_pos ();
  ...
  if (cb_test_mode->isChecked ()
      && DFIT_GLOBAL::init_params.size () == test_widgets.size ())
    {
      for (int i = 0; i < (int)DFIT_GLOBAL::init_params.size (); ++i)
        {
          ((QLineEdit *)test_widgets[i])
              ->setText (QString ("%1").arg (DFIT_GLOBAL::init_params[i]));
        }
      update_test_info ();
    }
}
...
void
US_Hydrodyn_Dad_Fit_Global::update_test_info ()
{
  QString qs_uggs = "";

  {
    ...
    qs_uggs += "common data: " + dad_win->describe_unified_common + "\n";

    for (unsigned int i = 0; i < dad_win->unified_ggaussian_gaussians_size;
         ++i)
      {
        qs_uggs += QString (" gaussian %1: ").arg (i + 1);
        for (unsigned int j = 0; j < dad_win->common_size; ++j)
          {
            qs_uggs += QString ("%1 ").arg (
                dad_win->unified_ggaussian_params[p++], 0, 'g', 6);
          }
        qs_uggs += "\n";
      }

    // now the file specific
    qs_uggs += "file specific: " + dad_win->describe_unified_per_file + "\n";
    for (unsigned int i = 0; i < dad_win->unified_ggaussian_curves; ++i)
      {
        qs_uggs += QString (" curve %1: ").arg (i + 1) + "\n";
        for (unsigned int k = 0; k < dad_win->unified_ggaussian_gaussians_size;
             ++k)
          {
            qs_uggs += QString ("  gaussian %1: ").arg (k + 1);
            for (unsigned int j = 0; j < dad_win->per_file_size; ++j)
              {
                qs_uggs += QString ("%1 ").arg (
                    dad_win->unified_ggaussian_params[p++], 0, 'g', 6);
              }
  ...
  lbl_test_info->setText (
      QString ("Per gaussian: common_size: %1 %2 per_file_size: %3 %4\n"
               ...
               "%6"
               "%7")
          .arg (dad_win->common_size)
          .arg (dad_win->describe_unified_common)
          .arg (dad_win->per_file_size)
          .arg (dad_win->describe_unified_per_file)
          .arg (US_Vector::qs_vector (
              "unified ggaussians", dad_win->unified_ggaussian_params, 6, 20))
          .arg (qs_uggs)
          .arg (DFIT_GLOBAL::qs_params ()));
}

void
US_Hydrodyn_Dad_Fit_Global::setupGUI ()
{
  int minHeight1 = 30;

  lbl_title
      = new QLabel (us_tr ("US-SOMO: UV-Vis: Global Gaussian Fit"), this);
  ...
  /* old way
     ...
     dad_win->gaussians.size() / 3 - 1 ); cout << "qs_rx:" << qs_rx << endl;
  ...
  */
  ...
  for (unsigned int i = 0;
       i < (unsigned int)dad_win->gaussians.size () / gaussian_type_size; i++)
    {
      QCheckBox *cb_tmp;
  ...
  double peak = dad_win->compute_gaussian_peak (dad_win->wheel_file,
                                                dad_win->gaussians);
  le_epsilon->setText (
  ...
  if (cb_test_mode->isVisible ())
    {
      ...
      for (int i = 0; i < dad_win->lb_files->count (); i++)
        {
          if (dad_win->lb_files->item (i)->isSelected ())
            {
              selected_count++;
              if (selected_count > 4)
                {
...
void
US_Hydrodyn_Dad_Fit_Global::cancel ()
{
  if (dad_win->lbl_gauss_fit->text () == "?"
      && !dad_win->pb_ggauss_rmsd->isEnabled ())
    {
      dad_win->pb_ggauss_rmsd->setEnabled (true);
    }
...
void
US_Hydrodyn_Dad_Fit_Global::help ()
{
  US_Help *online_help;
  online_help = new US_Help (this);
  online_help->show_help ("manual/somo/somo_dad_fit_global.html");
}

void
US_Hydrodyn_Dad_Fit_Global::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept ();
...
void
US_Hydrodyn_Dad_Fit_Global::update_common ()
{
  if (!setup_run ())
    {
      update_enables ();
  ...
  vector<double> save_unified_ggaussians = dad_win->unified_ggaussian_params;

  for (unsigned int i = 0; i < DFIT_GLOBAL::param_fixed.size (); i++)
    {
      if (!DFIT_GLOBAL::param_fixed[i] || DFIT_GLOBAL::comm_backref.count (i))
        {
          dad_win->unified_ggaussian_params[i]
              = DFIT_GLOBAL::init_params[DFIT_GLOBAL::comm_backref.count (i)
                                             ? DFIT_GLOBAL::comm_backref[i]
                                             : DFIT_GLOBAL::param_pos[i]];
        }
    }
  if (save_unified_ggaussians != dad_win->unified_ggaussian_params)
    {
      dad_win->lbl_gauss_fit->setText ("?");
      dad_win->ggqfit_plot->detachItems (QwtPlotItem::Rtti_PlotCurve);
      dad_win->ggqfit_plot->detachItems (QwtPlotItem::Rtti_PlotMarker);
...
void
US_Hydrodyn_Dad_Fit_Global::update_enables ()
{
...
vector<double> unified_q;                           // copy from dad win
vector<unsigned int> unified_ggaussian_param_index; // copy from dad win
unsigned int unified_ggaussian_gaussians_size;      // copy from dad win
unsigned int unified_ggaussian_curves;              // copy from dad win
...
bool
US_Hydrodyn_Dad_Fit_Global::setup_run ()
{
  DFIT_GLOBAL::init_params.clear ();
  DFIT_GLOBAL::base_params.clear ();
  DFIT_GLOBAL::fixed_params.clear ();
  DFIT_GLOBAL::comm_backref.clear ();
  DFIT_GLOBAL::param_pos.clear ();
  DFIT_GLOBAL::param_fixed.clear ();
  DFIT_GLOBAL::param_min.clear ();
  DFIT_GLOBAL::param_max.clear ();
  DFIT_GLOBAL::unified_q = dad_win->unified_ggaussian_q;
  DFIT_GLOBAL::unified_ggaussian_param_index
      = dad_win->unified_ggaussian_param_index;
  DFIT_GLOBAL::unified_ggaussian_gaussians_size
      = dad_win->unified_ggaussian_gaussians_size;
  DFIT_GLOBAL::unified_ggaussian_curves = dad_win->unified_ggaussian_curves;

  DFIT_GLOBAL::per_file_size = per_file_size;
  DFIT_GLOBAL::common_size = common_size;
  DFIT_GLOBAL::is_common = is_common;
  ...
  switch (gaussian_type)
    {
    case US_Hydrodyn_Dad::EMGGMG:
      DFIT_GLOBAL::compute_gaussian_f
          = &DFIT_GLOBAL::compute_gaussian_f_EMGGMG;
      // gsm_f = &gsm_f_EMGGMG;
      break;
    case US_Hydrodyn_Dad::EMG:
      DFIT_GLOBAL::compute_gaussian_f = &DFIT_GLOBAL::compute_gaussian_f_EMG;
      // gsm_f = &gsm_f_EMG;
      break;
    case US_Hydrodyn_Dad::GMG:
      DFIT_GLOBAL::compute_gaussian_f = &DFIT_GLOBAL::compute_gaussian_f_GMG;
      // gsm_f = &gsm_f_GMG;
      break;
    case US_Hydrodyn_Dad::GAUSS:
      DFIT_GLOBAL::compute_gaussian_f = &DFIT_GLOBAL::compute_gaussian_f_GAUSS;
      // gsm_f = &gsm_f_GAUSS;
      break;
    }

  DFIT_GLOBAL::use_errors = use_errors;
  ...
  double dist = ((US_Hydrodyn *)(dad_win->us_hydrodyn))
                    ->gparams["dad_dist_max"]
                    .toDouble ();
  ...
  // the dad_win unified_ggaussians
  ...
  for (unsigned int i = 0; i < dad_win->unified_ggaussian_gaussians_size; i++)
    {
      ...
      if (cb_fix_center->isChecked () || fixed_curves.count (i))
        {
          DFIT_GLOBAL::param_pos.push_back (DFIT_GLOBAL::fixed_params.size ());
          DFIT_GLOBAL::fixed_params.push_back (
              dad_win->unified_ggaussian_params[ofs]);
          DFIT_GLOBAL::param_fixed.push_back (true);
        }
      else
        {
          DFIT_GLOBAL::param_pos.push_back (DFIT_GLOBAL::init_params.size ());
          ...
          if (cb_pct_center_from_init->isChecked ())
            ...
            {
              base_val = dad_win->unified_ggaussian_params[ofs];
            }

          DFIT_GLOBAL::init_params.push_back (base_val);
          DFIT_GLOBAL::base_params.push_back (
              dad_win->unified_ggaussian_params[ofs]);
          DFIT_GLOBAL::param_fixed.push_back (false);
          ...
          if (DFIT_GLOBAL::init_params.back () < min)
            {
              DFIT_GLOBAL::init_params.back () = min;
            }
          if (DFIT_GLOBAL::init_params.back () > max)
            {
              DFIT_GLOBAL::init_params.back () = max;
            }

          DFIT_GLOBAL::param_min.push_back (min);
          DFIT_GLOBAL::param_max.push_back (max);
        }
      ofs++;
      this_ofs++;

      if (dad_win->cb_fix_width->isChecked ()) // if widths are common
        {
          if (cb_fix_width->isChecked () || fixed_curves.count (i))
            {
              DFIT_GLOBAL::param_pos.push_back (
                  DFIT_GLOBAL::fixed_params.size ());
              DFIT_GLOBAL::fixed_params.push_back (
                  dad_win->unified_ggaussian_params[ofs]);
              DFIT_GLOBAL::param_fixed.push_back (true);
            }
      ...
      if (dist1_active
          && dad_win->cb_fix_dist1->isChecked ()) // if dist1s are common
        {
          if (cb_fix_dist1->isChecked () || fixed_curves.count (i)
              || (cb_comm_dist1->isChecked () && i))
            {
              ...
              if (i && cb_comm_dist1->isChecked () && !fixed_curves.count (i)
                ...
                {
                  DFIT_GLOBAL::fixed_params.push_back (
                      dad_win->unified_ggaussian_params[this_ofs]);
                  DFIT_GLOBAL::comm_backref[DFIT_GLOBAL::param_fixed.size ()]
                      = comm_dist_1_var_pos;
                }
              else
                {
          ...
          else
            {
              if (cb_comm_dist1->isChecked ())
                {
                  comm_dist_1_var_pos = DFIT_GLOBAL::init_params.size ();
                }
          ...
          if (dist2_active
              && dad_win->cb_fix_dist2->isChecked ()) // if dist2s are common
            {
              if (cb_fix_dist2->isChecked () || fixed_curves.count (i)
                  || (cb_comm_dist2->isChecked () && i))
                {
                  ...
                  if (i && cb_comm_dist2->isChecked ()
                      && !fixed_curves.count (i) && !fixed_curves.count (0))
                    {
                      DFIT_GLOBAL::fixed_params.push_back (
                          dad_win->unified_ggaussian_params[this_ofs]);
                      DFIT_GLOBAL::comm_backref[DFIT_GLOBAL::param_fixed
                                                    .size ()]
                          = comm_dist_2_var_pos;
                    }
                  else
              ...
              else
                {
                  if (cb_comm_dist2->isChecked ())
                    {
                      comm_dist_2_var_pos = DFIT_GLOBAL::init_params.size ();
                    }
  ...
  unsigned int base_ofs
      = dad_win->unified_ggaussian_gaussians_size * common_size;
  // assert( (unsigned int)DFIT_GLOBAL::param_pos.size() == base_ofs &&
  // "common_size*number of gaussians should equal param_pos.size()" );

  for (unsigned int f = 0; f < dad_win->unified_ggaussian_curves; f++)
    {
      unsigned int ofs
          = base_ofs
            + f * per_file_size * dad_win->unified_ggaussian_gaussians_size;
      ...
      for (unsigned int i = 0; i < dad_win->unified_ggaussian_gaussians_size;
           i++)
        {
          ...
          if (!dad_win->cb_fix_width->isChecked ()) // if widths are not common
            {
              if (cb_fix_width->isChecked () || fixed_curves.count (i))
                {
                  DFIT_GLOBAL::param_pos.push_back (
  ...
  i++ )
     {
        ...
        {
           DFIT_GLOBAL::param_pos   .push_back(
  DFIT_GLOBAL::fixed_params.size() ); DFIT_GLOBAL::fixed_params.push_back(
  dad_win->unified_ggaussian_params[ 2 * i + 0 ] ); DFIT_GLOBAL::param_fixed
  .push_back( true ); } else { DFIT_GLOBAL::param_pos   .push_back(
  DFIT_GLOBAL::init_params.size() );
           ...
           if ( cb_pct_center_from_init->isChecked() )
           {
              ...
              base_val = dad_win->unified_ggaussian_params[ 2 * i + 0 ];
           }
  ...
  .push_back( false );
             ...
             fixed_curves.count( i ) )
        {
           DFIT_GLOBAL::param_pos   .push_back(
  DFIT_GLOBAL::fixed_params.size() ); DFIT_GLOBAL::fixed_params.push_back(
  dad_win->unified_ggaussian_params[ 2 * i + 1 ] ); DFIT_GLOBAL::param_fixed
  .push_back( true ); } else { DFIT_GLOBAL::param_pos   .push_back(
  DFIT_GLOBAL::init_params.size() );
           ...
           if ( cb_pct_width_from_init->isChecked() )
           {
              ...
              base_val = dad_win->unified_ggaussian_params[ 2 * i + 1 ];
           }
  ...
  .push_back( false );
     ...
     for ( unsigned int i = 0; i < dad_win->unified_ggaussian_gaussians_size *
  dad_win->unified_ggaussian_curves; i++ )
     {
        unsigned int pos = i / dad_win->unified_ggaussian_curves;

        unsigned int height_pos = 2 * dad_win->unified_ggaussian_gaussians_size
  + i + 0;
             ...
             fixed_curves.count( pos ) )
        {
           DFIT_GLOBAL::param_pos   .push_back(
  DFIT_GLOBAL::fixed_params.size() ); DFIT_GLOBAL::fixed_params.push_back(
  dad_win->unified_ggaussian_params[ height_pos ] ); DFIT_GLOBAL::param_fixed
  .push_back( true ); } else { DFIT_GLOBAL::param_pos   .push_back(
  DFIT_GLOBAL::init_params.size() );
           ...
           if ( cb_pct_amplitude_from_init->isChecked() )
           {
              ...
              base_val = dad_win->unified_ggaussian_params[ height_pos ];
           }
  ...
  i++ )
     {
             ...
             fixed_curves.count( i ) )
        {
           DFIT_GLOBAL::param_pos   .push_back(
  DFIT_GLOBAL::fixed_params.size() ); DFIT_GLOBAL::fixed_params.push_back(
  dad_win->unified_ggaussian_params[ i ] ); DFIT_GLOBAL::param_fixed
  .push_back( true ); } else { DFIT_GLOBAL::param_pos   .push_back(
  DFIT_GLOBAL::init_params.size() );
           ...
           if ( cb_pct_center_from_init->isChecked() )
           {
              ...
              base_val = dad_win->unified_ggaussian_params[ i ];
           }
  ...
  .push_back( false );
     ...
     for ( unsigned int i = 0; i < dad_win->unified_ggaussian_gaussians_size *
  2 * dad_win->unified_ggaussian_curves; i+= 2 )
     {
        unsigned int pos = i / ( 2 * dad_win->unified_ggaussian_curves );

        unsigned int height_pos = dad_win->unified_ggaussian_gaussians_size + i
  + 0; unsigned int width_pos  = dad_win->unified_ggaussian_gaussians_size + i
  + 1;
  ...
  .push_back( false );
             ...
             fixed_curves.count( pos + 1 ) )
        {
           DFIT_GLOBAL::param_pos   .push_back(
  DFIT_GLOBAL::fixed_params.size() ); DFIT_GLOBAL::fixed_params.push_back(
  dad_win->unified_ggaussian_params[ width_pos ] ); DFIT_GLOBAL::param_fixed
  .push_back( true ); } else { DFIT_GLOBAL::param_pos   .push_back(
  DFIT_GLOBAL::init_params.size() );
           ...
           if ( cb_pct_width_from_init->isChecked() )
           {
              ...
              base_val = dad_win->unified_ggaussian_params[ width_pos ];
           }
  ...
  */
  ...
  // DFIT_GLOBAL::list_params();

  // US_Vector::printvector( "is_common", is_common );

  if (!DFIT_GLOBAL::init_params.size ())
    {
      return false;
    }
  else
...
void
US_Hydrodyn_Dad_Fit_Global::test_mode ()
{
  if (!cb_test_mode->isChecked ())
    {
  ...
  for (int i = 0; i < (int)DFIT_GLOBAL::init_params.size (); ++i)
    {
      ...
      le->setEnabled (true);
      le->setText (QString ("%1").arg (DFIT_GLOBAL::init_params[i]));
      le->show ();
...
void
US_Hydrodyn_Dad_Fit_Global::test ()
{
  if (DFIT_GLOBAL::init_params.size () != test_widgets.size ())
    {
      ...
      if (!DFIT_GLOBAL::param_fixed[i] || DFIT_GLOBAL::comm_backref.count (i))
        {
          ...
          us_qdebug (QString ("param %1 taken from init params pos %2 %3")
                         .arg (i)
                         .arg (DFIT_GLOBAL::comm_backref.count (i)
                                   ? DFIT_GLOBAL::comm_backref[i]
                                   : DFIT_GLOBAL::param_pos[i])
                         .arg (DFIT_GLOBAL::comm_backref.count (i)
                                   ? "backref used"
  ...
  vector<double> gsum = dad_win->compute_ggaussian_gaussian_sum ();
  vector<double> t = dad_win->unified_ggaussian_t;
...
US_Hydrodyn_Dad_Fit_Global::lm ()
{
  ...
  vector<double> t = dad_win->unified_ggaussian_t;
  vector<double> y = dad_win->unified_ggaussian_I;

  vector<double> org_params = DFIT_GLOBAL::init_params;

  DFIT_GLOBAL::errors = dad_win->unified_ggaussian_e;
  DFIT_GLOBAL::errors_index.clear ();

  if (use_errors)
    {
      DFIT_GLOBAL::errors_index.resize ((unsigned int)t.back () + 1);
      for (unsigned int i = 0; i <= (unsigned int)t.back (); i++)
        {
          DFIT_GLOBAL::errors_index[i] = 0;
        }
      for (unsigned int i = 0; i < y.size (); i++)
        {
          y[i] /= DFIT_GLOBAL::errors[i];
          DFIT_GLOBAL::errors_index[(unsigned int)t[i]] = i;
        }
    }

  /* maybe no longer need
  vector < double > save_unified_ggaussians =
  dad_win->unified_ggaussian_params;

  for ( unsigned int i = 0; i < DFIT_GLOBAL::param_fixed.size(); i++ )
  {
     if ( !DFIT_GLOBAL::param_fixed[ i ] ||
          DFIT_GLOBAL::comm_backref.count( i ) )
     {
        dad_win->unified_ggaussian_params[ i ] = DFIT_GLOBAL::init_params[
                                                                           DFIT_GLOBAL::comm_backref.count(
  i ) ? DFIT_GLOBAL::comm_backref[ i ] : DFIT_GLOBAL::param_pos[ i ]
                                                                            ];
     }
  }
  ...
  */
  ...
  double org_rmsd = dad_win->ggaussian_rmsd (false);
  ...
  for (unsigned int i = 0; i < (unsigned int)t.size (); i++)
    {
      gsumf[i] = (*DFIT_GLOBAL::compute_gaussian_f) (
          t[i], &DFIT_GLOBAL::init_params[0]);
    }

  /* dad_win->unified_ggaussian_params = save_unified_ggaussians; */
...
#undef TOL
  ...
  // dad_win->add_ggaussian_curve( "lm_start", gsumf );

  vector<double> par = DFIT_GLOBAL::init_params;
  ...
  if (org_rmsd > status.fnorm)
    {
      for (unsigned int i = 0; i < DFIT_GLOBAL::param_fixed.size (); i++)
        {
          if (!DFIT_GLOBAL::param_fixed[i]
            ...
            {
              dad_win->unified_ggaussian_params[i]
                  = par[DFIT_GLOBAL::comm_backref.count (i)
                            ? DFIT_GLOBAL::comm_backref[i]
                            : DFIT_GLOBAL::param_pos[i]];
            }
        }
      ...
      // dad_win->add_ggaussian_curve( "lm_after_pushback",
      // dad_win->compute_ggaussian_gaussian_sum() );
      gaussians_undo.push_back (dad_win->unified_ggaussian_params);
      if (update_dad)
        {
...
void
US_Hydrodyn_Dad_Fit_Global::gsm_sd ()
{
  ...
  {
     gsm_yp[ j ]  = DFIT_GLOBAL::compute_gaussian_f( gsm_t[ j ], (double
  *)(&DFIT_GLOBAL::base_params[ 0 ]) ); org_rmsd += ( gsm_y[ j ] - gsm_yp[ j ]
  ) * ( gsm_y[ j ] - gsm_yp[ j ] );
  }

  org_rmsd = sqrt( org_rmsd );

  our_vector *v = new_our_vector( DFIT_GLOBAL::init_params.size() );
  for ( int i = 0; i < v->len; i++ )
  {
     v->d[ i ] = DFIT_GLOBAL::init_params[ i ];
  }
  ...
  if ( org_rmsd > rmsd )
  {
     for ( unsigned int i = 0; i < DFIT_GLOBAL::param_fixed.size(); i++ )
     {
        if ( !DFIT_GLOBAL::param_fixed[ i ] ||
             DFIT_GLOBAL::comm_backref.count( i ) )
        {
           dad_win->unified_ggaussian_params[ i ] = v->d[
           DFIT_GLOBAL::comm_backref.count( i ) ?
           DFIT_GLOBAL::comm_backref[ i ] :
           DFIT_GLOBAL::param_pos[ i ]
           ];
...
void
US_Hydrodyn_Dad_Fit_Global::gsm_ih ()
{
  return; // have to rewrite this
...
void
US_Hydrodyn_Dad_Fit_Global::gsm_cg ()
{
  return; // have to rewrite this
       ...
       rmsd < org_rmsd )
  {
     ...
     for ( unsigned int i = 0; i < DFIT_GLOBAL::param_fixed.size(); i++ )
     {
        if ( !DFIT_GLOBAL::param_fixed[ i ] ||
             DFIT_GLOBAL::comm_backref.count( i ) )
        {
           dad_win->unified_ggaussian_params[ i ] = par[
           DFIT_GLOBAL::comm_backref.count( i ) ?
           DFIT_GLOBAL::comm_backref[ i ] :
           DFIT_GLOBAL::param_pos[ i ]
           ];
...
void
US_Hydrodyn_Dad_Fit_Global::stop ()
{
  running = false;
}

void
US_Hydrodyn_Dad_Fit_Global::grid ()
{
  return; // have to rewrite this
  setup_run ();
  ...
  unsigned int total_count = (unsigned int)pow (
      (double)pop_size, (int)DFIT_GLOBAL::init_params.size ());
  ...
  for (unsigned int j = 0; j < (unsigned int)DFIT_GLOBAL::init_params.size ();
       j++)
    {
      ofs_per[j] = (DFIT_GLOBAL::param_max[j] - DFIT_GLOBAL::param_min[j])
                   / (pop_size - 1);
    }
  ...
  for (unsigned int j = 0; j < t.size (); j++)
    {
      yp[j] = DFIT_GLOBAL::compute_gaussian_f (
          t[j], (double *)(&DFIT_GLOBAL::base_params[0]));
  ...
  for (unsigned int i = 0; i < total_count; i++)
    {
      ...
      for (unsigned int j = 0;
           j < (unsigned int)DFIT_GLOBAL::init_params.size (); j++)
        {
          unsigned int pos = remainder % pop_size;
          use_par[j] = DFIT_GLOBAL::param_min[j] + pos * ofs_per[j];
          remainder /= pop_size;
        }
      ...
      for (unsigned int j = 0; j < t.size (); j++)
        {
          yp[j] = DFIT_GLOBAL::compute_gaussian_f (t[j],
                                                   (double *)(&use_par[0]));
...
void
US_Hydrodyn_Dad_Fit_Global::usp_config_plot_test (const QPoint &)
{
