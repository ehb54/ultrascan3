...
bool US_Saxs_Util::pdb_hydrate() {
  ...
  if (model_vector.size() > 1 && !control_parameters.count("pdballmodels")) {
    cout << "Notice: an NMR style model was loaded, but \"PDBAllModels\" was "
            "not selected, so only the first model will be loaded\n";
  ...
  for (unsigned int j = 0; j < model_vector[i].molecule.size(); j++) {
    for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size(); k++) {
      ...
      if (rx_main_chain.indexIn(this_atom->resName) == -1 &&
          exposed_sc.count(mapkey) && rotamers.count(this_atom->resName)) {
...
bool US_Saxs_Util::compute_to_hydrate_dihedrals() {
  ...
  for (map<QString, map<QString, point> >::iterator it = to_hydrate.begin();
    ...
    if (rx_expand_mapkey.indexIn(it->first) == -1) {
      errormsg =
          QString("Internal error: could not expand mapkey %1").arg(it->first);
      return false;
    }
    ...
    if (!dihedral_atoms.count(resName)) {
      errormsg =
          QString("No dihedral group found for residue %1.").arg(resName);
      return false;
    }
    ...
    for (unsigned int i = 0; i < dihedral_atoms[resName].size(); i++) {
      if (dihedral_atoms[resName][i].size() != 4) {
        errormsg = QString(
                       "Dihedral table size incorrect, should be 4 but is %1 "
                       "for %2th dihedral group of %3")
      ...
      dihedral = acos(dot(plane(p[0], p[1], p[2]), plane(p[1], p[2], p[3])));
...
QString US_Saxs_Util::list_to_hydrate(bool coords) {
  ...
  for (map<QString, float>::iterator it = residue_asa.begin();
    ...
    out += QString("%1 tot=%2 sc=%3 mc=%4\n")
               .arg(it->first)
               .arg(it->second)
               .arg(residue_asa_sc.count(it->first) ? residue_asa_sc[it->first]
                                                    : 0.0)
               .arg(residue_asa_mc.count(it->first) ? residue_asa_mc[it->first]
                                                    : 0.0);
  }
...
bool US_Saxs_Util::load_rotamer(QString filename) {
  ...
  if (!f.open(QIODevice::ReadOnly)) {
    errormsg =
        QString("Error: rotamer file %1 is not readable. Check permissions")
            .arg(filename);
    return false;
  }
  ...
  for (unsigned int i = 0; i < (unsigned int)qsl.size(); i++) {
    ...
    if (qsl_line[0] == "multiple-rotate:") {
      if (in_rotamer) {
        errormsg =
            QString(
                "Error in hydrated rotamer file line %1.  multiple-rotate "
                "token can not occur within a rotamer definition")
                .arg(i + 1);
        return false;
      }
      ...
      if ((qsl_line.size() - 2) % 3) {
        errormsg =
            QString(
                "Error in hydrated rotamer file line %1.  multiple-rotate: "
                "requires exactly 2 + 3n arguments, n >= 1")
                .arg(i + 1);
        return false;
      }
      ...
      if (!rotamers.count(residue)) {
        errormsg =
            QString(
                "Error in hydrated rotamer file line %1.  multiple-rotate: "
                "residue: %2 must be previously defined")
                .arg(i + 1)
                .arg(residue);
        return false;
      }
      ...
      if (rotate <= 1) {
        errormsg = QString(
                       "Error in hydrated rotamer file line %1.  "
                       "multiple-rotate: rotate count %2, must be 2 or greater")
                       .arg(i + 1)
                       .arg(rotate);
        return false;
      }
      ...
      // this info is not available yet, needs compute water_positioning atoms
      // to complete
      ...
      //                multiple-rotate: difference in water count (%2) with
      //                reference rotamer water count (%3)" ) .arg( i + 1 )
      //                .arg( water_count )
      //                .arg( rotamers[ residue ][ 0
      //                ].water_positioning_atoms.size() );
      //             return false;
      //          }
      ...
      for (unsigned int water_number = 0; water_number < water_count;
        ...
        if (!found_match) {
          errormsg =
              QString(
                  "Error in hydrated rotamer file line %1.  multiple-rotate: "
                  "residue: %2 found, but full name %3 missing")
                  .arg(i + 1)
                  .arg(residue)
                  .arg(name);
          return false;
...
#if defined(UHH_DEBUG_RR)
        cout << QString(
                    "multiple-rotate %1 x%2 found rotamer match %3 pos %4 for "
                    "water %5\n")
                    .arg(name)
                    .arg(rotate)
                    .arg(residue)
        ...
        if (ref_rotamer.waters.size() <= water_number) {
          errormsg =
              QString(
                  "Error in hydrated rotamer file line %1.  multiple-rotate: "
                  "residue: %2 found reference rotamer, but it has %3 waters, "
                  "less than %4 requested")
                  .arg(i + 1)
                  .arg(residue)
        ...
        if (ref_rotamer.waters[water_number].name != atoms[2]) {
          errormsg =
              QString(
                  "Error in hydrated rotamer file line %1.  multiple-rotate: "
                  "residue: %2 found, atom %3 missing from waters")
                  .arg(i + 1)
                  .arg(residue)
                  .arg(atoms[2]);
          return false;
        ...
        float a =
            acosf(dot(normal(minus(p[0], p[1])), normal(minus(p[2], p[1]))));
        ...
        float na = acosf(
            dot(normal(minus(np[0], np[1])), normal(minus(np[2], np[1]))));
        ...
        for (unsigned int j = 0; j < ref_rotamer.side_chain.size(); j++) {
          ...
          new_ref_rotamer.atom_map[new_ref_rotamer.side_chain[j].name]
              .coordinate = new_ref_rotamer.side_chain[j].coordinate;
        }
        ...
        new_ref_rotamer.waters[0].coordinate =
            ref_rotamer_np[ref_rotamer.side_chain.size() + water_number];
        // }
      ...
      for (unsigned int jj = 0; jj < rotated_rotamers[name].size(); jj++) {
        ...
        for (unsigned int j = 0; j < rotated_rotamers[name][jj].size(); j++) {
          ...
          for (unsigned int k = 0;
               k < rotated_rotamers[name][jj][j].side_chain.size(); k++) {
            out += QString("").sprintf(
    ...
    if (qsl_line[0] == "pointmap:") {
      ...
      if (pointmap_atoms.count(res)) {
        errormsg = QString(
                       "Error in hydrated rotamer file line %1.  %2 was "
                       "previously defined as a pointmap")
                       .arg(i + 1)
                       .arg(res);
        return false;
      }
      ...
      while (qsl_line.size()) {
        ...
        for (unsigned int j = 0; j < points; j++) {
          if (!qsl_line.size()) {
            errormsg = QString(
                           "Error in hydrated rotamer file line %1.  Invalid "
                           "number of tokens on pointmap line")
                           .arg(i + 1);
            return false;
          }
    ...
    if (qsl_line[0] == "dihedral:") {
      ...
      if (four_atoms.size()) {
        errormsg = QString(
                       "Error in hydrated rotamer file line %1.  Invalid "
                       "number of tokens %2 on dihedral line")
                       .arg(i + 1)
                       .arg(qsl_line.size() + 2);
        return false;
      }
    ...
    if (!in_rotamer) {
      ...
      if (qsl_line[0] != "start-rotamer:") {
        errormsg = QString(
                       "Error in hydrated rotamer file line %1.  Unrecognized "
                       "token %2")
                       .arg(i + 1)
                       .arg(qsl_line[0]);
        return false;
    ...
    if (qsl_line.size() == 1) {
      if (qsl_line[0] == "end-rotamer") {
        ...
        if (!tmp_rotamer.side_chain.size() || !tmp_rotamer.waters.size()) {
          errormsg =
    ...
    if (qsl_line[0] != "ATOM") {
      errormsg =
          QString(
              "Error in hydrated rotamer file line %1.  Unrecognized token %2")
              .arg(i + 1)
              .arg(qsl_line[0]);
      return false;
    }
  ...
  if (in_rotamer) {
    errormsg = "Error in hydrated rotamer file at end, no final end-rotamer.";
    return false;
  }
      ...
      for (unsigned int j = 0; j < dihedral_atoms[it->first].size(); j++) {
        ...
        for (unsigned int k = 0; k < dihedral_atoms[it->first][j].size(); k++) {
          ...
          if (!(it->second[i].atom_map.count(
            ...
            errormsg = QString("Rotamer for %1 is missing dihedral atom %2")
                           .arg(it->first)
        ...
        cout << "   cross:   " << cross(minus(p[2], p[1]), minus(p[0], p[1]))
             << endl;
        cout << "   plane2: " << plane(p[1], p[2], p[3]) << endl;
        cout << "   dot:    "
             << dot(plane(p[0], p[1], p[2]), plane(p[1], p[2], p[3])) << endl;
        cout << "   acos:   "
             << acos(dot(plane(p[0], p[1], p[2]), plane(p[1], p[2], p[3])))
             << endl;
#endif
  ...
  for (map<QString, vector<float> >::iterator it = to_hydrate_dihedrals.begin();
    ...
    for (unsigned int i = 0; i < rotamers[resName].size(); i++) {
      if (it->second.size() != rotamers[resName][i].dihedral_angles.size()) {
        errormsg = QString(
                       "Number of dihedrals do not match between side chain %1 "
                       "(%2) and rotamer %3 (%4)")
                       .arg(it->first)
                       .arg(it->second.size())
      ...
      for (unsigned int j = 0; j < it->second.size(); j++) {
        ...
        fitness +=
            fabs(it->second[j] - rotamers[resName][i].dihedral_angles[j]);
      }
...
bool US_Saxs_Util::setup_pointmap_rotamers() {
  ...
  for (map<QString, map<QString, point> >::iterator it =
    ...
    if (!pointmap_atoms_ref_residue.count(resName)) {
      errormsg =
          QString("Internal error: could not find rotamer for pointmap key %1")
              .arg(it->first);
      return false;
    }
    ...
    for (unsigned int i = 0; i < pointmap_atoms_ref_residue[resName].size();
      ...
      if (!rotamers.count(pointmap_atoms_ref_residue[resName][i])) {
        errormsg = QString(
                       "No rotamer group found for pointmap key %1 reference "
                       "residue %2.")
                       .arg(it->first)
...
bool US_Saxs_Util::compute_water_positioning_atoms() {
  ...
  for (map<QString, vector<rotamer> >::iterator it = rotamers.begin();
    ...
    for (unsigned int i = 0; i < it->second.size(); i++) {
      ...
      for (unsigned int j = 0; j < it->second[i].waters.size(); j++) {
        ...
        for (list<sortable_float>::iterator it2 = lsf.begin(); it2 != lsf.end();
             it2++) {
  ...
  for (map<QString, vector<vector<rotamer> > >::iterator it =
    ...
    if (!back_ref.count(it->first)) {
      errormsg =
          QString("Error: rotated_rotamer wth name %1 not found it rotamers")
              .arg(it->first);
      return false;
    }
    ...
    if (water_count != back_ref[it->first]->water_positioning_atoms.size()) {
      errormsg = QString(
                     "Error: rotated_rotamer wth name %1 found but water count "
                     "differs %2 vs %3")
                     .arg(it->first)
                     .arg(water_count)
...
QString US_Saxs_Util::list_water_positioning_atoms() {
  ...
  for (map<QString, vector<rotamer> >::iterator it = rotamers.begin();
    ...
    for (unsigned int i = 0; i < it->second.size(); i++) {
      out += QString("  %1:\n").arg(it->second[i].extension);
      for (unsigned int j = 0; j < it->second[i].water_positioning_atoms.size();
           j++) {
        out += QString("    water %1 :").arg(j);
        for (unsigned int k = 0;
             k < it->second[i].water_positioning_atoms[j].size(); k++) {
          out +=
              QString(" %1").arg(it->second[i].water_positioning_atoms[j][k]);
        }
        out += "\n";
      }
    }
...
bool US_Saxs_Util::compute_waters_to_add() {
  ...
  for (map<QString, rotamer>::iterator it = best_fit_rotamer.begin();
    ...
    if (!to_hydrate.count(it->first)) {
      errormsg =
          QString(
              "Internal error: best_fit_rotamer key %1 not found in to_hydrate")
              .arg(it->first);
      return false;
    }
    ...
    // it->first );
    if (it->second.water_positioning_atoms.size() != it->second.waters.size()) {
      errormsg = QString(
                     "Internal error: water positioning atom size %1 does not "
                     "match waters size %2")
    ...
    for (unsigned int i = 0; i < it->second.water_positioning_atoms.size();
      ...
      for (unsigned int j = 0; j < it->second.water_positioning_atoms[i].size();
           j++) {
        if (!it->second.atom_map.count(
                it->second.water_positioning_atoms[i][j])) {
          errormsg = QString(
                         "Internal error: water positioning atom %1 not found "
                         "in atom_map")
                         .arg(it->second.water_positioning_atoms[i][j]);
          return false;
        }
        p1[j] = it->second.atom_map[it->second.water_positioning_atoms[i][j]]
                    .coordinate;
  ...
  for (map<QString, vector<rotamer> >::iterator it = pointmap_rotamers.begin();
    ...
    if (!to_hydrate_pointmaps.count(it->first)) {
      errormsg = QString(
                     "Internal error: pointmap_rotamers key %1 not found in "
                     "to_hydrate_pointmaps")
                     .arg(it->first);
      return false;
    }
    ...
    QString resName = rx_expand_mapkey.cap(1);
    if (!pointmap_atoms.count(resName) || !pointmap_atoms_dest.count(resName) ||
        !pointmap_atoms_ref_residue.count(resName)) {
      errormsg = QString(
                     "Internal error: could not find pointmap entries for "
                     "residue of key %1")
                     .arg(it->first);
      return false;
    }
    ...
    if (it->second.size() != pointmap_atoms[resName].size() ||
      ...
      errormsg = QString(
                     "Internal error: could not find pointmap size "
    ...
    for (unsigned int i = 0; i < it->second.size(); i++) {
      ...
      // apply to each water in the rotamer the rotamer's atom_map(equivalently
      // side chain) & waters have the "dest" info

      if (it->second[i].residue != pointmap_atoms_ref_residue[resName][i]) {
        errormsg = QString(
                       "Internal error: inconsistancy for reference residue "
                       "name for key %1 pos %2 (%3 != %4)")
                       .arg(it->first)
                       .arg(i)
      ...
      for (unsigned int j = 0; j < it->second[i].waters.size(); j++) {
        ...
        cout << QString(
            ...
            "th      {\n"
            "th         if ( fabs( cmp_waters[ i ].axis[ j ] - new_waters[ i "
            "].axis[ j ] ) > 1e-4 )\n"
            "th         {\n"
        ...
        if (!has_steric_clash(new_waters[0])) {
               ...
               << flush;
          {
            ...
            // water_radius ).arg(  multi_residue_map.count( "WAT" ) ? "has WAT"
            // : "no WAT" );
            ...
            for (unsigned int j = 0; j < model_vector[i].molecule.size(); j++) {
              for (unsigned int k = 0;
                ...
                if (dist(this_atom->coordinate, p) <
                  ...
                  cout << QString("clash with structure %1 %2 %3 %4")
  ...
  for (map<QString, vector<point> >::iterator it = waters_to_add.begin();
    ...
    for (unsigned int i = 0; i < it->second.size(); i++) {
      last_hydrated_pdb_text += QString("").sprintf(
          ...
          it->second[i].axis[0], it->second[i].axis[1], it->second[i].axis[2]);
    }
  }
...
bool US_Saxs_Util::validate_pointmap() {
      ...
      if (!pointmap_atoms.count(it->first) ||
        ...
        for (unsigned int i = 0; i < it->second.size(); i++) {
          if (!rotamers.count(it->second[i])) {
            errormsg += QString(
                            "Pointmap %1 contains invalid target rotamer %2, "
            ...
            if (!rotamers[it->second[i]].size()) {
              errormsg += QString(
                              "Pointmap %1 contains empty target rotamer %2, "
                              "removing from loaded pointmaps\n")
                              .arg(it->first)
                              .arg(it->second[i]);
              to_erase[it->first] = true;
      ...
      for (unsigned int i = 0; i < it->second.size(); i++) {
        for (unsigned int j = 0; j < it->second[i].size(); j++) {
          if (!rotamers[pointmap_atoms_ref_residue[it->first][i]][0]
            ...
            errormsg +=
                QString(
                    "Pointmap %1 target rotamer %2 has an invalid atom %3 (not "
                    "found in rotamer), removing from loaded pointmaps\n")
                    .arg(it->first)
  ...
  for (map<QString, bool>::iterator it = to_erase.begin(); it != to_erase.end();
       it++) {
...
bool US_Saxs_Util::pdb_asa_for_saxs_hydrate() {
  ...
  if (control_parameters["asamethod"].toUInt() == 0) {
    ...
    int retval = surfracer_main(asa.hydrate_probe_radius, active_atoms, false,
                                progress, editor);
  ...
  if (control_parameters["asamethod"].toUInt() == 1) {
    ...
    int retval = us_saxs_util_asab1_main(active_atoms, &asa, &results, false);
      ...
      for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size();
        ...
        if (this_atom->active) {
          ...
          if (!create_beads_normally ||
            ...
            if (create_beads_normally && misc_pb_rule_on && !this_atom->type) {
      ...
      for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size();
        ...
        if (misc_pb_rule_on && this_atom->p_residue->type == 0 &&
          ...
          if (multi_residue_map["PBR-NO-OXT"].size() == 1) {
            ...
            cout << "Chain has broken end and PBR-NO-OXT isn't uniquely "
      ...
      for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size();
        ...
        if (this_atom->active && this_atom->is_bead) {
          ...
          cout << QString("bead_asa %1 %2 %3 %4 %5\n")
                      ...
                      .arg(control_parameters["asahydratethresh"].toDouble());
#endif
    ...
    for (unsigned int k = 0;
      ...
      for (unsigned int m = 0; m < residue_list.size(); m++) {
        ...
        if ((residue_list[m].name == this_atom->resName &&
            ...
            (!k && this_atom->name == "N" && restype == 0 && misc_pb_rule_on &&
        ...
        if (atompos == -1) {
          ...
          if (our_saxs_options.compute_saxs_coeff_for_bead_models) {
            ...
            if (!residue_atom_hybrid_map.count(mapkey)) {
              cout << QString(
                          "%1Molecule %2 Residue %3 %4 Hybrid name missing. "
                          "Atom skipped.\n")
                          .arg(this_atom->chainID == " "
                                   ? ""
              ...
              if (hybrid_name.isEmpty() || !hybrid_name.length()) {
                ...
                if (!saxs_util->hybrid_map.count(hybrid_name)) {
                  cout << QString(
                              "%1Molecule %2 Residue %3 %4 Hybrid %5 name "
                  ...
                  if (!saxs_util->atom_map.count(this_atom->name + "~" +
                                                 hybrid_name)) {
                    cout << QString(
                                "%1Molecule %2 Atom %3 Residue %4 %5 Hybrid %6 "
                    ...
                    if (!saxs_util->saxs_map.count(
                            saxs_util->hybrid_map[hybrid_name].saxs_name)) {
                      cout << QString(
                                  "%1Molecule %2 Residue %3 %4 Hybrid %5 Saxs "
...
void US_Saxs_Util::get_atom_map(PDB_model *model) {
  ...
  for (unsigned int j = 0; j < model->molecule.size(); j++) {
              ...
              us_log->log(
                  lastResSeq != ""
                      ? QString(
                            ...
                            "residue: %1Molecule %2 Residue %3 %4 & %5 %6.\n")
                            .arg(this_atom->chainID == " "
                                     ? ""
                                     : ("Chain " + this_atom->chainID + " "))
                            .arg(j + 1)
                            ...
                            .arg(this_atom->resName)
                            .arg(this_atom->resSeq)
                      : QString("Warning: break in residue sequence or unknown "
                ...
                msging["_textarea"] =
                    lastResSeq != ""
                        ? QString(
                              ...
                              "unknown residue: %1Molecule %2 Residue %3 %4 & "
                              "%5 %6.\\n")
                              .arg(this_atom->chainID == " "
                                       ? ""
                                       : ("Chain " + this_atom->chainID + " "))
                        ...
                        : QString(
                              "\\nWarning: break in residue sequence or "
                              "unknown residue: %1Molecule %2 Residue %3 "
                              "%4.\\n")
                              .arg(this_atom->chainID == " "
                                       ? ""
                                       : ("Chain " + this_atom->chainID + " "))
    ...
    if (aa && non_aa) {
      us_log->log(
          QString("Notice: %1found %2 non or unknown Amino Acids in a chain "
                  "containing %3 AA Residues.\n")
      ...
      if (us_udp_msg) {
        ...
        msging["_textarea"] =
            QString(
                "Notice: %1found %2 non or unknown Amino Acids in a chain "
                "containing %3 AA Residues. \\n")
                .arg(lastChainID == " " ? "" : ("Chain " + lastChainID + " "))
                .arg(non_aa)
                .arg(aa);
...
void US_Saxs_Util::build_molecule_maps(PDB_model *model) {
  ...
  for (unsigned int i = 0; i < molecules_idx_seq.size(); i++) {
    ...
    if (multi_residue_map.count(resName)) {
      ...
      residue_errors[chains[idx] + "~" + resnames[idx] + " " + resseqs[idx]] =
          true;
    }
    {
    ...
    for (unsigned int k = 0; k < model->molecule[j].atom.size(); k++) {
      ...
      if (respos == -1) {
        if ((this_atom->name != "H" && this_atom->name != "D" &&
          ...
          residue_errors[this_atom->orgChainID + "~" + this_atom->orgResName +
                         " " + this_atom->orgResSeq] = true;
                ...
                if (molecules_residue_min_missing[QString("%1|%2").arg(j).arg(
                  ...
                  switch (control_parameters["pdbmissingresidues"].toUInt()) {
                    ...
                    default:
                      failure_errors++;
                      errormsg =
                          "Internal error: Unexpected PdbMissingResidues "
                          "type " +
                          QString("%1\n").arg(
              ...
              if (bead_exceptions[count_idx] == 1) {
                switch (control_parameters["pdbmissingresidues"].toUInt()) {
                  ...
                  default:
                    failure_errors++;
                    errormsg =
                        "Internal error: Unexpected PdbMissingResidues type " +
                        QString("%1\n").arg(
                            control_parameters["pdbmissingresidues"].toUInt());
                    return false;
                    break;
                }
              }
          ...
          switch (bead_exceptions[count_idx]) {
            ...
            case 3: {
              ...
              if (this_atom->resSeq != lastResSeq) {
                ...
                new_bead.hydration =
                    (unsigned int)(misc_avg_hydration * atom_counts[count_idx] +
                                   .5);
                ...
                if (new_residue_name != new_residue.name) {
                  errormsg =
                      "Internal error: Unexpected new residue name "
                      "inconsistancy.\n" +
                      QString("\"%1\" != \"%2\"")
                          .arg(new_residue_name)
              ...
              if (new_residue.name != residue_list[respos].name) {
                errormsg =
                    "Internal error: Unexpected new residue name residue_list "
                    "inconsistancy.";
                return false;
              }
            ...
            case 4:  // missing atoms
            {
              ...
              if (pos < 0) {
                errormsg =
                    QString(
                        "Internal error: idx %1 Missing atom error %2 ! < 0!\n")
                        .arg(idx)
                        .arg(pos);
                return false;
              }
              ...
              if (this_atom->resSeq != lastResSeq) {
                ...
                if (one_bead) {
                  ...
                  // new_bead.hydration = (unsigned int)(misc_avg_hydration *
                  ...
                  for (unsigned int i = 0;
                    ...
                    if (!molecules_residue_missing_atoms_skip
                      ...
                      new_atom.hybrid.mw *=
                          atoms_scale_weight[0];  // misc_avg_mass;
                      new_atom.hybrid.mw =
                          (int)(new_atom.hybrid.mw * 100 + .5) / 100.0;
                      new_atom.hybrid.radius *=
                          atoms_scale_radius[0];  // misc_avg_radius;
                      new_atom.hybrid.radius =
                    ...
                    if (!molecules_residue_missing_atoms_skip
                      ...
                      if (beads_missing_atom_count[new_atom.bead_assignment]) {
                        new_atom.hybrid.mw *= atoms_scale_weight
                            [new_atom.bead_assignment];  // misc_avg_mass;
                        new_atom.hybrid.mw =
                            (int)(new_atom.hybrid.mw * 100 + .5) / 100.0;
                        new_atom.hybrid.radius *= atoms_scale_radius
                            [new_atom.bead_assignment];  // misc_avg_radius;
                        new_atom.hybrid.radius =
                            (int)(new_atom.hybrid.radius * 100 + .5) / 100.0;
    ...
    for (unsigned int k = 0; k < model->molecule[j].atom.size(); k++) {
      ...
      if (!checked.count(idx)) {
        ...
        if (this_atom->p_residue && this_atom->model_residue_pos != -1) {
          // cout << "Test3" << endl; // Problem after that with pdbs with
          // missing atoms - does not go to both Test3a and Test3b!!!

          if ((int)model->residue.size() <= this_atom->model_residue_pos) {
            // cout << "Test3a" << endl;
            // cout << QString( "model->residue does not contain model residue
          ...
          // ].unique_name).arg(this_atom->p_residue->unique_name)); qDebug
          // (QString ( "%1 ").arg(model->residue[ this_atom->model_residue_pos
          // ].unique_name));
...
class sortable_float_qs {
 public:
  float f;
  QString index;
  bool operator<(const sortable_float_qs &objIn) const { return (f < objIn.f); }
};
  ...
  for (unsigned int i = 0; i < residues_in_order.size(); i++) {
    ...
    if (best_fit_rotamer.count(this_residue)) {
      ...
      if (!to_hydrate.count(this_residue)) {
        errormsg = QString(
                       "Internal error: best_fit_rotamer key %1 not found in "
                       "to_hydrate")
                       .arg(this_residue);
        return false;
      }

      if (best_fit.water_positioning_atoms.size() != best_fit.waters.size()) {
        errormsg = QString(
                       "Internal error: water positioning atom size %1 does "
                       "not match waters size %2")
      ...
      if (rotated_rotamers.count(best_fit.name)) {
        // find best choice
        cout << QString(
                    "this best fit rotamer %1 is in with rotated rotamers %2\n")
                    .arg(this_residue)
                    .arg(best_fit.name);
        // for each water
        if (rotated_rotamers[best_fit.name].size() !=
          ...
          errormsg = QString(
                         "Internal error: water positioning atom size %1 does "
                         "not match best fit rotamer size %2")
                         .arg(best_fit.water_positioning_atoms.size())
                         .arg(best_fit.waters.size());
          return false;
        }
        ...
        // for each water:
        for (unsigned int ii = 0; ii < best_fit.water_positioning_atoms.size();
             ii++) {
          ...
          for (unsigned int m = 0;
            ...
            if (best_fit.water_positioning_atoms[ii].size() !=
              ...
              errormsg =
                  QString(
                      "Internal error: best fit waters count %1 does not match "
                      "reference %2")
            ...
            for (unsigned int jj = 0;
                 jj < this_ref_rotamer.water_positioning_atoms[0].size();
                 jj++) {
              if (!this_ref_rotamer.atom_map.count(
                      this_ref_rotamer.water_positioning_atoms[0][jj])) {
                errormsg =
                    QString(
                        "Internal error: water positioning atom %1 not found "
                        "in atom_map")
          ...
          count_waters++;
          cout << QString(
                      "Notice: best_fit rotamer %1 water %2 has %3 rotated "
                      "rotamers and %4\n")
                      .arg(best_fit.name)
                      .arg(ii)
        ...
        // std add one
        cout << QString(
                    "this best fit rotamer %1 is NOT a rotated rotamers %2\n")
                    .arg(this_residue)
                    .arg(best_fit.name);
        ...
        //             {
        //                errormsg = QString( "Internal error: water positioning
        //                atom size %1 does not match waters size %2" )
        //                   .arg( best_fit.water_positioning_atoms.size() )
        //                   .arg( best_fit.waters.size() );
        //                return false;
        //             }

        for (unsigned int i = 0; i < best_fit.water_positioning_atoms.size();
             i++) {
          // get coordinates of
          ...
          for (unsigned int j = 0;
            ...
            p1[j] = best_fit.atom_map[best_fit.water_positioning_atoms[i][j]]
                        .coordinate;
          ...
          if (!has_steric_clash(new_waters[0])) {
            cout << QString(
                        "Notice: best fit for %1 has no rotated rotamers and "
                        "no clash\n")
                        .arg(best_fit.name);
            count_waters_added++;
    ...
    if (pointmap_rotamers.count(this_residue)) {
      ...
      for (unsigned int i = 0; i < pointmap_rotamers[this_residue].size();
        ...
        cout << QString(
                    "Note: in compute_waters_to_add_alt(), current name '%1' "
                    "has alternates %2\n")
        ...
        if (rotated_rotamers.count(ref_rotamer.name)) {
          ...
          if (!pointmap_atoms.count(resName) ||
              ...
              !pointmap_atoms_ref_residue.count(resName)) {
            errormsg = QString(
                           "Internal error: could not find pointmap entries "
                           "for residue of key %1")
                           .arg(resName);
            return false;
          }
          ...
          for (unsigned int k = 0;
            ...
            for (unsigned int j = 0; j < p1.size(); j++) {
              ...
              p1[j] =
                  this_ref_rotamer.atom_map[pointmap_atoms_dest[resName][i][j]]
                      .coordinate;
              ...
              cout << QString("mapping: %1 %2 source %3\n")
                          ...
                          .arg(pointmap_atoms[resName][i][j]);
#endif
              p2[j] = to_hydrate_pointmaps[this_residue]
                                          [pointmap_atoms[resName][i][j]];
            ...
            if (rotated_rotamers[this_ref_rotamer.name].size() != 1) {
              errormsg =
                  QString(
                      "Error: multiple rotated pointmap rotamer %1 has %2 "
                      "waters and only one water is currently supported for "
                      "this scheme ")
                      .arg(this_ref_rotamer.name)
                      .arg(rotated_rotamers[this_ref_rotamer.name].size());
              return false;
            }

            for (unsigned int m = 0;
                 m < rotated_rotamers[this_ref_rotamer.name][0].size(); m++) {
              vector<point> rotamer_waters;
              // this is probably not correct:
          ...
          cout
              << QString(
                     "Notice: pointmap for %1 has %2 rotated rotamers and %3\n")
                     .arg(ref_rotamer.name)
          ...
          for (unsigned int j = 0; j < p1.size(); j++) {
            ...
            p1[j] = ref_rotamer.atom_map[pointmap_atoms_dest[resName][i][j]]
                        .coordinate;
          ...
          for (unsigned int j = 0; j < ref_rotamer.waters.size(); j++) {
            ...
            if (!has_steric_clash(new_waters[0])) {
              cout << QString(
                          "Notice: pointmap for %1 has no rotated rotamers and "
                          "no clash\n")
                          .arg(ref_rotamer.name);
              count_waters_added++;
              waters_to_add[this_residue].push_back(new_waters[0]);
...
bool US_Saxs_Util::list_steric_clash_recheck() {
  ...
  hydrate_clash_detail +=
      ...
      "radii,distance,overlap,percent overlap,water x,clash x, water y, clash "
      "y, water z, clash z, source, source2\n";
  ...
  for (map<QString, vector<point> >::iterator it = waters_to_add.begin();
    ...
    for (unsigned int pos = 0; pos < it->second.size(); pos++) {
        ...
        for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size();
          ...
          if (dist(this_atom->coordinate, p) <
            ...
            if (!waters_source.count(it->first)) {
              ...
              errormsg = QString(
                             "Internal error: list_steric_clash_recheck(): "
            ...
            if ((unsigned int)waters_source[it->first].size() <= pos) {
              cout << QString(
                          "waters_source[ %1 ] does not contain pos %2 size "
                          "only %3\n")
                          .arg(it->first)
                          .arg(pos)
              ...
              errormsg = QString(
                             "Internal error: list_steric_clash_recheck(): "
                             "waters_source[ %1 ] does not contain pos %2")
                             .arg(it->first)
                             .arg(pos);
              return false;
            }
            if (waters_source[it->first][pos].startsWith("PM")) {
              hydrate_clash_map_pm_water[QString("%1~%2").arg(it->first).arg(
                  pos)] = true;
            } else {
              hydrate_clash_map_rtmr_water[QString("%1~%2").arg(it->first).arg(
                  pos)] = true;
            }

            hydrate_clash_log << QString(
                                     "Water %1 number %2 clashes with %3~%4~%5 "
                                     "atom %6 by %7 A or %8%\n")
                                     .arg(it->first)
                                     .arg(pos + 1)
                                     .arg(this_atom->resName)
            ...
            hydrate_clash_detail += QString("Water:%1~%2,%3~%4~%5~%6,")
                                        .arg(it->first)
                                        .arg(pos + 1)
                                        .arg(this_atom->resName)
                                        .arg(this_atom->resSeq)
                                        .arg(this_atom->chainID)
                                        .arg(this_atom->name);
            hydrate_clash_detail += QString("%1,%2,%3,%4,")
                                        .arg(water_radius)
            ...
            hydrate_clash_detail += QString("%1,%2,%3,%4,%5,%6,")
                                        .arg(p.axis[0])
            ...
            hydrate_clash_detail +=
                QString("%1").arg(waters_source[it->first][pos]);
      ...
      for (map<QString, vector<point> >::iterator it2 = it;
        ...
        for (unsigned int j = (it2 == it ? pos + 1 : 0); j < it2->second.size();
             j++) {
          if (dist(it2->second[j], p) < 2e0 * water_radius * dist_threshold) {
            ...
            if (waters_source[it2->first][j].startsWith("PM")) {
              hydrate_clash_map_pm_water
                  [QString("%1~%2").arg(it2->first).arg(j)] = true;
            } else {
            ...
            hydrate_clash_log
                << QString(
                       "Water %1 number %2 clashes with water from %3 number "
                       "%4 by %5 A or %6%\n")
                       .arg(it->first)
                       .arg(pos + 1)
                       .arg(it2->first)
                       ...
                       .arg(100.0 *
                            (2e0 * water_radius - dist(it2->second[j], p)) /
                            (2e0 * water_radius));

            hydrate_clash_detail += QString("Water:%1~%2,Water:%3~%4,")
                                        .arg(it->first)
                                        .arg(i + 1)
                                        .arg(it2->first)
                                        .arg(j + 1);
            ...
            hydrate_clash_detail += QString("%1,%2")
  ...
  //                current_model ].mw ) )
  //                +
  //                QString( "Maximum exposed possible water to place %1, degree
  //                of hydration %2\n" ) .arg( count_waters_added +
  ...
  //                count_waters_added * 18e0 / model_vector[ current_model ].mw
  //                ) )
  //                );
  ...
  last_steric_clash_log << QString(
                               "REMARK Maximum possible water to place         "
                               "%1   degree of hydration %2\n")
                               .arg(hydrate_max_waters_no_asa)
                               .arg(QString("").sprintf(
                                   "%.2f", (double)hydrate_max_waters_no_asa *
                                               18e0 /
                                               model_vector[current_model].mw));

  last_steric_clash_log
      << QString(
             "REMARK Maximum exposed possible water to place %1   degree of "
             "hydration %2\n")
