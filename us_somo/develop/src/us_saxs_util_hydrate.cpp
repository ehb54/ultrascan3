...
ostream &
operator<< (ostream &out, const rotamer &c)
{
  out << "rotamer name: " << c.name << " residue:" << c.residue
      << " extension:" << c.extension << endl;

  out << QString (" dihedral angles: %1\n").arg (c.dihedral_angles.size ());

  out << QString (" side chain size: %1 elements:")
             .arg (c.dihedral_angles.size ());
  for (unsigned int i = 0; i < c.side_chain.size (); i++)
    {
      out << QString (" side chain element %1 name %2 %3 %4 %5\n")
                 .arg (i)
  ...
  for (unsigned int i = 0; i < c.waters.size (); i++)
    {
      out << QString (" waters element %1 name %2 %3 %4 %5\n")
                 .arg (i)
                 .arg (c.waters[i].name)
...
US_Saxs_Util::pdb_hydrate ()
{
  ...
  asa.hydrate_probe_radius
      = control_parameters["asahydrateproberadius"].toFloat ();
  ...
  our_saxs_options.alt_hydration
      = control_parameters.count ("hydratealt") ? true : false;
  our_saxs_options.hydration_rev_asa
      = control_parameters.count ("hydraterevasa") ? true : false;

  if (control_parameters.count ("hydrationscd"))
    {
      our_saxs_options.steric_clash_distance
          = control_parameters["hydrationscd"].toFloat ();
    }

  if (control_parameters.count ("hydrationrscd"))
    {
      our_saxs_options.steric_clash_recheck_distance
          = control_parameters["hydrationrscd"].toFloat ();
    }
  ...
  if (!residue_list.size () || !model_vector.size ())
    {
      errormsg
          = "Calculations can not be run until residue & pdb files are read!";
      return false;
    }
  ...
  for (current_model = 0; current_model < model_vector.size ();
       current_model++)
    {
      ...
      if (!compute_to_hydrate_dihedrals ())
        {
          errormsg
              = "Error computing dihedrals of exposed side chain: " + errormsg;
          return false;
        }
      ...
      cout << list_waters_to_add ();
      {
        QString out;
        out = "Waters to add:\n";
        for (map<QString, vector<point> >::iterator it
             = waters_to_add.begin ();
             it != waters_to_add.end (); it++)
          {
  ...
  if (model_vector.size () > 1 && !control_parameters.count ("pdballmodels"))
    {
      cout << "Notice: an NMR style model was loaded, but \"PDBAllModels\" "
              "was not selected, so only the first model will be loaded\n";
      ...
      for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size ();
           k++)
        {
          ...
          if (exposed_for_pointmap.count (mapkey))
            {
              to_hydrate_pointmaps[mapkey][this_atom->name]
          ...
          if (!accumulated_residue.count (mapkey))
            {
              ...
              if (hydrate_count.count (this_atom->resName))
                {
                  hydrate_max_waters_no_asa
                      += hydrate_count[this_atom->resName];
                }
            }
        }
    }
...
bool
US_Saxs_Util::compute_to_hydrate_dihedrals ()
{
  ...
  for (map<QString, map<QString, point> >::iterator it = to_hydrate.begin ();
       it != to_hydrate.end (); it++)
    {
      if (rx_expand_mapkey.indexIn (it->first) == -1)
        {
          errormsg = QString ("Internal error: could not expand mapkey %1")
                         .arg (it->first);
          return false;
        }
      ...
      if (!dihedral_atoms.count (resName))
        {
          errormsg = QString ("No dihedral group found for residue %1.")
                         .arg (resName);
          return false;
        }
#if defined(DEBUG_TO_HYDRATE_DIHEDRAL)
      cout << QString ("computation for to hydrate dihedral %1 %2\n")
                  .arg (it->first)
                  .arg (resName);
#endif
      for (unsigned int i = 0; i < dihedral_atoms[resName].size (); i++)
        {
          if (dihedral_atoms[resName][i].size () != 4)
            {
              errormsg = QString ("Dihedral table size incorrect, should be 4 "
                                  "but is %1 for %2th dihedral group of %3")
          ...
          for (unsigned int j = 0; j < dihedral_atoms[resName][i].size (); j++)
            {
              if (!(it->second.count (dihedral_atoms[resName][i][j])))
                {
                  errormsg = QString ("Side chain %1 is missing required "
                                      "dihedral atom %2")
                                 .arg (it->first)
          ...
          dihedral = acos (
              dot (plane (p[0], p[1], p[2]), plane (p[1], p[2], p[3])));
...
QString
US_Saxs_Util::list_to_hydrate_dihedrals ()
{
  QString out = "";
  for (map<QString, vector<float> >::iterator it
       = to_hydrate_dihedrals.begin ();
       it != to_hydrate_dihedrals.end (); it++)
    {
...
QString
US_Saxs_Util::list_to_hydrate (bool coords)
{
  ...
  for (map<QString, map<QString, point> >::iterator it
       = to_hydrate_pointmaps.begin ();
       it != to_hydrate_pointmaps.end (); it++)
    {
          ...
          if (this_atom->exposed_code == 1)
            {
              ...
              if (pointmap_atoms.count (this_atom->resName))
                {
                  out += QString ("%1 %2 %3 %4 %5 pointmap\n")
                             .arg (this_atom->serial)
                             .arg (this_atom->name)
                             .arg (this_atom->resName)
                             .arg (this_atom->chainID)
...
bool
US_Saxs_Util::load_rotamer (QString filename)
{
  rotamers.clear ();
  ...
  if (!f.open (QIODevice::ReadOnly))
    {
      errormsg
          = QString (
                "Error: rotamer file %1 is not readable. Check permissions")
                .arg (filename);
      return false;
    }
  ...
  for (unsigned int i = 0; i < (unsigned int)qsl.size (); i++)
    {
      ...
      QStringList qsl_line
          = (qsl[i]).split (rx_whitespace, Qt::SkipEmptyParts);
      if (qsl_line[0] == "multiple-rotate:")
        {
          if (in_rotamer)
            {
              errormsg = QString ("Error in hydrated rotamer file line %1.  "
                                  "multiple-rotate token can not occur within "
                                  "a rotamer definition")
                             .arg (i + 1);
              return false;
            }
          ...
          if (qsl_line.size () < 5)
            {
              errormsg
                  = QString ("Error in hydrated rotamer file line %1.  "
                             "multiple-rotate: requires at least 5 arguments")
                        .arg (i + 1);
              return false;
            }
          ...
          if ((qsl_line.size () - 2) % 3)
            {
              errormsg = QString ("Error in hydrated rotamer file line %1.  "
                                  "multiple-rotate: requires exactly 2 + 3n "
                                  "arguments, n >= 1")
                             .arg (i + 1);
              return false;
            }
          ...
          if (rotated_rotamers.count (name))
            {
              errormsg
                  = QString (
                        "Error in hydrated rotamer file line %1.  "
                        "multiple-rotate: %2 previously multiple-rotate'd")
                        .arg (i + 1)
                        .arg (name);
              return false;
            }
          ...
          if (!rotamers.count (residue))
            {
              errormsg = QString ("Error in hydrated rotamer file line %1.  "
                                  "multiple-rotate: residue: %2 must be "
                                  "previously defined")
                             .arg (i + 1)
                             .arg (residue);
              return false;
          ...
          if (rotate <= 1)
            {
              errormsg = QString ("Error in hydrated rotamer file line %1.  "
                                  "multiple-rotate: rotate count %2, must be "
                                  "2 or greater")
                             .arg (i + 1)
                             .arg (rotate);
              return false;
          ...
          // this info is not available yet, needs compute water_positioning
          // atoms to complete

          //          if ( water_count != rotamers[ residue ][ 0
          //          ].water_positioning_atoms.size() )
          //          {
          //             errormsg =
          //                QString( "Error in hydrated rotamer file line %1.
          //                multiple-rotate: difference in water count (%2)
          //                with reference rotamer water count (%3)" ) .arg( i
          //                + 1 ) .arg( water_count ) .arg( rotamers[ residue
          //                ][ 0 ].water_positioning_atoms.size() );
          //             return false;
          //          }

          rotated_rotamers[name].resize (water_count);

          for (unsigned int water_number = 0; water_number < water_count;
               water_number++)
            {
              ...
              if (!found_match)
                {
                  errormsg = QString ("Error in hydrated rotamer file line "
                                      "%1.  multiple-rotate: residue: %2 "
                                      "found, but full name %3 missing")
                                 .arg (i + 1)
                                 .arg (residue)
                                 .arg (name);
                  return false;
...
#if defined(UHH_DEBUG_RR)
              cout << QString ("multiple-rotate %1 x%2 found rotamer match %3 "
                               "pos %4 for water %5\n")
                          .arg (name)
                          .arg (rotate)
                          .arg (residue)
              ...
              for (unsigned int j = 0; j < 2; j++)
                {
                  ...
                  for (unsigned int k = 0; k < ref_rotamer.side_chain.size ();
                       k++)
                    {
                  ...
                  if (!found_sc_atom)
                    {
                      errormsg = QString ("Error in hydrated rotamer file "
                                          "line %1.  multiple-rotate: "
                                          "residue: %2 found, atom %3 missing")
                                     .arg (i + 1)
                                     .arg (residue)
                                     .arg (atoms[j]);
                      return false;
              ...
              if (ref_rotamer.waters.size () <= water_number)
                {
                  errormsg = QString ("Error in hydrated rotamer file line "
                                      "%1.  multiple-rotate: residue: %2 "
                                      "found reference rotamer, but it has %3 "
                                      "waters, less than %4 requested")
                                 .arg (i + 1)
                                 .arg (residue)
              ...
              if (ref_rotamer.waters[water_number].name != atoms[2])
                {
                  errormsg = QString ("Error in hydrated rotamer file line "
                                      "%1.  multiple-rotate: residue: %2 "
                                      "found, atom %3 missing from waters")
                                 .arg (i + 1)
                                 .arg (residue)
                                 .arg (atoms[2]);
                  return false;
...
#if defined(UHH_DEBUG_RR)
              cout << QString ("transform from: ") << p[0] << " " << p[1]
                   << " " << p[2] << endl;
              cout << QString ("transform to: ") << np[0] << " " << np[1]
                   << " " << np[2] << endl;
              ...
              for (unsigned int j = 0; j < ref_rotamer.side_chain.size (); j++)
                {
                  ref_rotamer_p.push_back (
                      ref_rotamer.side_chain[j].coordinate);
                }
              ...
              // not avail yet
              // new_ref_rotamer.water_positioning_atoms.push_back(
              // ref_rotamer.water_positioning_atoms[ water_number ] );
              for (unsigned int j = 0; j < ref_rotamer.side_chain.size (); j++)
                {
                  new_ref_rotamer.side_chain[j].coordinate = ref_rotamer_np[j];
                  new_ref_rotamer.atom_map[new_ref_rotamer.side_chain[j].name]
                      .coordinate
              ...
              new_ref_rotamer.waters[0].coordinate
                  = ref_rotamer_np[ref_rotamer.side_chain.size ()
                                   + water_number];
              // }
              ...
              for (unsigned int j = 0; j < rotate; j++)
                {
                  ...
                  new_ref_rotamer.waters[0].coordinate.axis[0]
                      = xdist * cos (rho);
                  new_ref_rotamer.waters[0].coordinate.axis[1]
                      = xdist * sin (rho);

                  // FIX THIS!!!    check rotamer for internal steric clash!

                  rotated_rotamers[name][water_number].push_back (
                      new_ref_rotamer);
                }
          ...
          // but each chain ref is rotated into a different coordinate frame,
          // so each one needs to be added separately, so we will just list
          // each vector as a separate model

          for (unsigned int jj = 0; jj < rotated_rotamers[name].size (); jj++)
            {
              out += QString ("REMARK  water %1\n").arg (jj + 1);
              for (unsigned int j = 0; j < rotated_rotamers[name][jj].size ();
                   j++)
                {
                  out += QString ("MODEL     %1\n").arg (j + 1);
                  ...
                  for (unsigned int k = 0;
                       k < rotated_rotamers[name][jj][j].side_chain.size ();
                       k++)
                    {
                      out += QString ("").sprintf (
                          "ATOM  %5d%5s%4s %1s%4d    "
                          ...
                          ++apos,
                          rotated_rotamers[name][jj][j]
                              .side_chain[k]
                              .name.toLatin1 ()
                              .data (),
                          name.left (3).toLatin1 ().data (), "A", 1,
                          rotated_rotamers[name][jj][j]
                              .side_chain[k]
                              .coordinate.axis[0],
                          rotated_rotamers[name][jj][j]
                  ...
                  out += "TER\n";
                  for (unsigned int k = 0;
                       k < rotated_rotamers[name][jj][j].waters.size (); k++)
                    {
                      out += QString ("").sprintf (
                          "ATOM  %5d%5s%4s %1s%4d    "
                          "%8.3f%8.3f%8.3f%6.2f%6.2f          %2s\n",
      ...
      if (qsl_line[0] == "pointmap:")
        {
          ...
          if (pointmap_atoms.count (res))
            {
              errormsg = QString ("Error in hydrated rotamer file line %1.  "
                                  "%2 was previously defined as a pointmap")
                             .arg (i + 1)
                             .arg (res);
              return false;
            }
              ...
              if (points < 3)
                {
                  errormsg
                      = QString (
                            "Error in hydrated rotamer file line %1.  Invalid "
                            "number of points for a map on pointmap line")
                            .arg (i + 1);
                  return false;
                }
                  ...
                  if (!qsl_line.size ())
                    {
                      errormsg
                          = QString (
                                "Error in hydrated rotamer file line %1.  "
                                "Invalid number of tokens on pointmap line")
                                .arg (i + 1);
                      return false;
                    }
          ...
          if (four_atoms.size ())
            {
              errormsg
                  = QString ("Error in hydrated rotamer file line %1.  "
                             "Invalid number of tokens %2 on dihedral line")
                        .arg (i + 1)
                        .arg (qsl_line.size () + 2);
              return false;
            }
      ...
      if (qsl_line.size () == 1)
        {
          ...
          if (qsl_line[0] != "TER")
            {
              errormsg = QString ("Error in hydrated rotamer file line %1.  "
                                  "Unrecognized token %2 in this context")
                             .arg (i + 1)
                             .arg (qsl_line[0]);
              return false;
            }
      ...
      if (!in_rotamer_waters && rx_main_chain.indexIn (qsl_line[2]) != -1)
        {
          // skip main chain
          continue;
        }
  ...
  // compute totals
  hydrate_count.clear ();
  for (map<QString, vector<vector<QString> > >::iterator it
       = pointmap_atoms.begin ();
       it != pointmap_atoms.end (); it++)
    {
      ...
      for (unsigned int i = 0; i < it->second.size () && i < 1; i++)
        {
          if (hydrate_count.count (it->first))
            {
              hydrate_count[it->first]
                  += (unsigned int)it->second[i].waters.size ();
            }
          else
            {
              hydrate_count[it->first]
...
bool
US_Saxs_Util::compute_rotamer_dihedrals ()
{
      ...
      for (unsigned int i = 0; i < it->second.size (); i++)
        {
          ...
          cout << QString ("computation for dihedral %1 %2:\n")
                      .arg (it->first)
          ...
          for (unsigned int j = 0; j < dihedral_atoms[it->first].size (); j++)
            {
              ...
              if (dihedral_atoms[it->first][j].size () != 4)
                {
                  errormsg
                      = QString ("Dihedral table size incorrect, should be 4 "
                                 "but is %1 for %2th dihedral group of %3")
              ...
              for (unsigned int k = 0;
                   k < dihedral_atoms[it->first][j].size (); k++)
                {
#if defined(DEBUG_DIHEDRAL)
                  cout << QString ("  %1: ").arg (
                      dihedral_atoms[it->first][j][k]);
#endif
                  if (!(it->second[i].atom_map.count (
                          dihedral_atoms[it->first][j][k])))
                    {
                      errormsg
                          = QString (
                                "Rotamer for %1 is missing dihedral atom %2")
                                .arg (it->first)
                                .arg (dihedral_atoms[it->first][j][k]);
                      return false;
                    }
                  p[k] = it->second[i]
                             .atom_map[dihedral_atoms[it->first][j][k]]
                             .coordinate;
#if defined(DEBUG_DIHEDRAL)
                  cout << p[k];
#endif
...
QString
US_Saxs_Util::list_rotamer_dihedrals ()
{
  QString out;
      ...
      for (unsigned int i = 0; i < it->second.size (); i++)
        {
          ...
          for (unsigned int j = 0; j < it->second[i].dihedral_angles.size ();
               j++)
            {
...
US_Saxs_Util::list_pointmaps ()
{
  ...
  for (map<QString, vector<vector<QString> > >::iterator it
       ...
       it != pointmap_atoms.end (); it++)
    {
      out += QString ("Pointmaps for residue: %1  number of maps %2\n")
                 .arg (it->first)
                 .arg (it->second.size ());
      for (unsigned int i = 0; i < it->second.size (); i++)
        {
          out += QString ("    mapping to %1\n")
                     .arg (
                         pointmap_atoms_ref_residue.count (it->first)
                             ? (pointmap_atoms_ref_residue[it->first].size ()
                                        > i
                  ...
                  out += QString ("    %1 -> %2\n")
                             ...
                             .arg (
                                 pointmap_atoms_dest.count (it->first)
                                     ? (pointmap_atoms_dest[it->first].size ()
                                                > i
                                            ? (pointmap_atoms_dest[it->first]
                                                                  [i]
                                                                      .size ()
                                                       > j
                                                   ? pointmap_atoms_dest
...
QString
US_Saxs_Util::list_rotamers (bool coords)
{
  QString out;

  out = "dihedral atoms:\n";
  for (map<QString, vector<vector<QString> > >::iterator it
       = dihedral_atoms.begin ();
       it != dihedral_atoms.end (); it++)
    {
      out += QString (" %1:\n").arg (it->first);
      for (unsigned int i = 0; i < it->second.size (); i++)
        {
...
bool
US_Saxs_Util::compute_best_fit_rotamer ()
{
  best_fit_rotamer.clear ();

  QRegExp rx_expand_mapkey ("^(.+)~(.+)~(.*)$");

  // go through the computed dihedrals, compute sum of abs differences, choose
  // best one
  for (map<QString, vector<float> >::iterator it
       = to_hydrate_dihedrals.begin ();
       it != to_hydrate_dihedrals.end (); it++)
    {
      ...
      if (!rotamers.count (resName))
        {
          errormsg = QString ("No rotamer group found for residue %1.")
                         .arg (resName);
          return false;
        }
      ...
      for (unsigned int i = 0; i < rotamers[resName].size (); i++)
        {
          if (it->second.size ()
              != rotamers[resName][i].dihedral_angles.size ())
            {
              errormsg
                  = QString ("Number of dihedrals do not match between side "
                             "chain %1 (%2) and rotamer %3 (%4)")
                        .arg (it->first)
                        .arg (it->second.size ())
          ...
          for (unsigned int j = 0; j < it->second.size (); j++)
            {
              // the dihedral angles are computed by acos, and range of acos is
              // [0,pi], so I think we are ok
              fitness += fabs (it->second[j]
                               - rotamers[resName][i].dihedral_angles[j]);
            }
...
QString
US_Saxs_Util::list_best_fit_rotamer ()
{
  QString out;
...
bool
US_Saxs_Util::setup_pointmap_rotamers ()
{
  ...
  for (map<QString, map<QString, point> >::iterator it
       ...
       it != to_hydrate_pointmaps.end (); it++)
    {
      ...
      if (!pointmap_atoms_ref_residue.count (resName))
        {
          errormsg = QString ("Internal error: could not find rotamer for "
                              "pointmap key %1")
                         .arg (it->first);
          return false;
        }

      for (unsigned int i = 0; i < pointmap_atoms_ref_residue[resName].size ();
           i++)
        {
          if (!rotamers.count (pointmap_atoms_ref_residue[resName][i]))
            {
              errormsg = QString ("No rotamer group found for pointmap key %1 "
                                  "reference residue %2.")
                             .arg (it->first)
          ...
          // use first one
          pointmap_rotamers[it->first].push_back (
              rotamers[pointmap_atoms_ref_residue[resName][i]][0]);
        }
    }
  return true;
}
...
QString
US_Saxs_Util::list_pointmap_rotamers ()
{
  QString out;

  out = "Pointmap rotamers:\n";

  for (map<QString, vector<rotamer> >::iterator it
       = pointmap_rotamers.begin ();
       it != pointmap_rotamers.end (); it++)
    {
      out += QString ("%1:").arg (it->first);
      for (unsigned int i = 0; i < it->second.size (); i++)
        {
...
bool
US_Saxs_Util::compute_water_positioning_atoms ()
{
          ...
          for (unsigned int j = 0; j < it->second[i].waters.size (); j++)
            {
              ...
              for (unsigned int k = 0; k < it->second[i].side_chain.size ();
                   k++)
                {
                  sf.f = dist (it->second[i].waters[j].coordinate,
                               it->second[i].side_chain[k].coordinate);
                  sf.index = k;
                  lsf.push_back (sf);
                }
  ...
  for (map<QString, vector<vector<rotamer> > >::iterator it
       = rotated_rotamers.begin ();
       it != rotated_rotamers.end (); it++)
    {
      if (!back_ref.count (it->first))
        {
          errormsg
              = QString (
                    "Error: rotated_rotamer wth name %1 not found it rotamers")
                    .arg (it->first);
          return false;
        }
      ...
      if (water_count != back_ref[it->first]->water_positioning_atoms.size ())
        {
          errormsg
              = QString ("Error: rotated_rotamer wth name %1 found but water "
                         "count differs %2 vs %3")
                    .arg (it->first)
                    .arg (water_count)
                    .arg (
                        back_ref[it->first]->water_positioning_atoms.size ());
          return false;
        }
      ...
      for (unsigned int i = 0; i < it->second.size (); i++)
        {
          for (unsigned int j = 0; j < it->second[i].size (); j++)
            {
              it->second[i][j].water_positioning_atoms.push_back (
                  back_ref[it->first]->water_positioning_atoms[i]);
            }
        }
    }
  return true;
...
QString
US_Saxs_Util::list_water_positioning_atoms ()
{
      ...
      for (unsigned int i = 0; i < it->second.size (); i++)
        {
          ...
          for (unsigned int j = 0;
               j < it->second[i].water_positioning_atoms.size (); j++)
            {
              out += QString ("    water %1 :").arg (j);
              for (unsigned int k = 0;
                   k < it->second[i].water_positioning_atoms[j].size (); k++)
                {
                  out += QString (" %1").arg (
                      it->second[i].water_positioning_atoms[j][k]);
                }
              out += "\n";
            }
        }
  ...
  for (map<QString, vector<rotamer> >::iterator it = rotamers.begin ();
       it != rotamers.end (); it++)
    {
      for (unsigned int i = 0; i < it->second.size (); i++)
        {
          out += QString ("%1-%2 ").arg (it->first).arg (
              it->second[i].extension);
          for (unsigned int j = 0;
               j < it->second[i].water_positioning_atoms.size (); j++)
            {
              out += QString (" [%1]").arg (j);
...
bool
US_Saxs_Util::compute_waters_to_add ()
{
  ...
  for (map<QString, rotamer>::iterator it = best_fit_rotamer.begin ();
       it != best_fit_rotamer.end (); it++)
    {
      if (!to_hydrate.count (it->first))
        {
          errormsg = QString ("Internal error: best_fit_rotamer key %1 not "
                              "found in to_hydrate")
                         .arg (it->first);
          return false;
        }
      ...
      if (it->second.water_positioning_atoms.size ()
        ...
        {
          errormsg = QString ("Internal error: water positioning atom size %1 "
                              "does not match waters size %2")
                         .arg (it->second.water_positioning_atoms.size ())
                         .arg (it->second.waters.size ());
          return false;
        }

      for (unsigned int i = 0; i < it->second.water_positioning_atoms.size ();
           i++)
        {
          // get coordinates of
          ...
          for (unsigned int j = 0;
               j < it->second.water_positioning_atoms[i].size (); j++)
            {
              if (!it->second.atom_map.count (
                      it->second.water_positioning_atoms[i][j]))
                {
                  errormsg
                      = QString ("Internal error: water positioning atom %1 "
                                 "not found in atom_map")
                            .arg (it->second.water_positioning_atoms[i][j]);
                  return false;
                }
              p1[j] = it->second
                          .atom_map[it->second.water_positioning_atoms[i][j]]
                          .coordinate;

              if (!to_hydrate[it->first].count (
                      it->second.water_positioning_atoms[i][j]))
                {
                  errormsg
                      = QString ("Internal error: water positioning atom %1 "
                                 "not found in to_hydrate atoms")
                            .arg (it->second.water_positioning_atoms[i][j]);
                  return false;
                }
              p2[j] = to_hydrate[it->first]
                                [it->second.water_positioning_atoms[i][j]];
          ...
          if (!has_steric_clash (new_waters[0]))
            {
              ...
              waters_source[it->first].push_back (
                  QString ("Rtmr:%1").arg (it->second.name));
#if defined(USUH_DEBUG_ALIGN)
  ...
  for (map<QString, vector<rotamer> >::iterator it
       ...
       it != pointmap_rotamers.end (); it++)
    {
      if (!to_hydrate_pointmaps.count (it->first))
        {
          errormsg = QString ("Internal error: pointmap_rotamers key %1 not "
                              "found in to_hydrate_pointmaps")
                         .arg (it->first);
          return false;
        }
      ...
      if (!pointmap_atoms.count (resName)
        ...
        {
          errormsg = QString ("Internal error: could not find pointmap "
                              "entries for residue of key %1")
                         .arg (it->first);
          return false;
        }
      ...
      if (it->second.size () != pointmap_atoms[resName].size ()
        ...
        {
          errormsg = QString ("Internal error: could not find pointmap size "
                              "inconsistancy for key %1")
                         .arg (it->first);
          return false;
        }
      ...
      for (unsigned int i = 0; i < it->second.size (); i++)
        {
          // for each water to add:
          // compute transformation matrix from the pointmap_atoms_dest in the
          // pointmap_atom_ref_residue to the pointmap_atoms in the residue and
          // apply to each water in the rotamer the rotamer's
          // atom_map(equivalently side chain) & waters have the "dest" info

          if (it->second[i].residue != pointmap_atoms_ref_residue[resName][i])
            {
              errormsg
                  = QString ("Internal error: inconsistancy for reference "
                             "residue name for key %1 pos %2 (%3 != %4)")
                        .arg (it->first)
                        .arg (i)
          ...
          if (p1.size () != p2.size ())
            {
              errormsg
                  = QString (
                        "Internal error: size inconsistancy for reference "
                        "residue name for key %1 pos %2 (%3 != %4)")
                        .arg (it->first)
                        .arg (i)
          ...
          for (unsigned int j = 0; j < p1.size (); j++)
            {
              ...
              //                );
              if (!it->second[i].atom_map.count (
                      pointmap_atoms_dest[resName][i][j]))
                {
                  errormsg = QString ("Internal error: atom %1 not found in "
                                      "to_hydrate_pointmaps atoms")
                                 .arg (pointmap_atoms_dest[resName][i][j]);
                  return false;
                }

              p1[j] = it->second[i]
                          .atom_map[pointmap_atoms_dest[resName][i][j]]
                          .coordinate;

              if (!to_hydrate_pointmaps[it->first].count (
                      pointmap_atoms[resName][i][j]))
                {
                  errormsg = QString ("Internal error: atom %1 not found in "
                                      "to_hydrate_pointmaps atoms")
              ...
              p2[j] = to_hydrate_pointmaps[it->first]
                                          [pointmap_atoms[resName][i][j]];

              //             cout << QString( "z1  %1 [%2,%3,%4] to
              //             [%5,%6,%7]\n" )
              //                .arg( it->second.water_positioning_atoms[ i ][
              //                j ] ) .arg( p1[ j ].axis[ 0 ] ) .arg( p1[ j
              //                ].axis[ 1 ] ) .arg( p1[ j ].axis[ 2 ] ) .arg(
              //                p2[ j ].axis[ 0 ] ) .arg( p2[ j ].axis[ 1 ] )
              //                .arg( p2[ j ].axis[ 2 ] );
            }
              ...
              for (unsigned int k = 0; k < rotamer_waters.size (); k++)
                {
                  for (unsigned int l = 0; l < 3; l++)
                    {
                      cout << QString ("").sprintf (
                          "th   rotamer_waters[ %u ].axis[ %u ] = %.10ff;\n",
              ...
              for (unsigned int k = 0; k < new_waters.size (); k++)
                {
                  for (unsigned int l = 0; l < 3; l++)
                    {
                      cout << QString ("").sprintf (
                          "th   new_waters[ %u ].axis[ %u ] = %.10ff;\n", k, l,
                          new_waters[k].axis[l]);
                    }
                }
              cout << QString (
                  "th   if ( !atom_align( p1, p2, rotamer_waters, cmp_waters "
                  ") )\n"
                  "th   {\n"
                  "th      return false;\n"
                  "th   }\n"
                  ...
                  "th      {\n"
                  "th         if ( fabs( cmp_waters[ i ].axis[ j ] - "
                  "new_waters[ i ].axis[ j ] ) > 1e-4 )\n"
                  "th         {\n"
                  "th            errormsg = QString( \"mismatch water %u axis "
                  "%u\\n\" ).arg( i ).arg( j );\n"
                  "th            return false;\n"
                  "th         }\n"
                  "th      }\n"
              ...
              if (!has_steric_clash (new_waters[0]))
                {
                  ...
                  waters_to_add[it->first].push_back (new_waters[0]);
                  waters_source[it->first].push_back (
                      QString ("PM:%1").arg (it->second[i].name));
              ...
              else
                {
                       ...
                       << flush;
                  {
                    ...
                    double dist_threshold
                        = 1e0
                          - (our_saxs_options.steric_clash_distance / 100e0);
                    double water_radius
                        = multi_residue_map.count ("WAT")
                              ? residue_list[multi_residue_map["WAT"][0]]
                                    .r_atom[0]
                                    .hybrid.radius
                              : 1.401;

                    // cout << QString( "using %1 for water radius %2\n" ).arg(
                    // water_radius ).arg(  multi_residue_map.count( "WAT" ) ?
                    // "has WAT" : "no WAT" );

                    // check structure:
                    for (unsigned int j = 0;
                         j < model_vector[i].molecule.size (); j++)
                      {
                        for (unsigned int k = 0;
                             k < model_vector[i].molecule[j].atom.size (); k++)
                          {
                            ...
                            if (dist (this_atom->coordinate, p)
                                < (this_atom->radius + water_radius)
                                      * dist_threshold)
                              {
                                cout << QString (
                                            "clash with structure %1 %2 %3 %4")
                                            .arg (
                                                dist (this_atom->coordinate, p)
                                                - (this_atom->radius
                                                   + water_radius)
                                                      * dist_threshold)
                                            .arg (this_atom->name)
                    ...
                    for (map<QString, vector<point> >::iterator it
                         ...
                         it != waters_to_add.end (); it++)
                      {
                        for (unsigned int i = 0; i < it->second.size (); i++)
                          {
                            if (dist (it->second[i], p)
                                <= 2e0 * water_radius * dist_threshold)
                              {
                                cout << QString ("clash with water %1\n")
                                            .arg (dist (it->second[i], p)
                                                  - 2e0 * water_radius
                                                        * dist_threshold)
                                     << flush;
...
float
US_Saxs_Util::min_dist_to_waters (point p)
{
  // check already added waters:
...
US_Saxs_Util::flush_pdb ()
{
  ...
  if (!f.open (QIODevice::WriteOnly))
    {
      errormsg
          = QString ("Error: can not open file %1 for writing").arg (fname);
      return false;
    }
...
bool
US_Saxs_Util::buffer_pdb_with_waters ()
{
  ...
  last_hydrated_pdb_text += QString ("MODEL    %1\n").arg (current_model + 1);

  last_hydrated_pdb_text
      += last_pdb_load_calc_mw_msg.replaceInStrings ("\n", "").join (
             "\nREMARK ")
         + "\n";
  ...
  for (unsigned int j = 0; j < model_vector[i].molecule.size (); j++)
    {
      for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size ();
           k++)
        {
          ...
          last_hydrated_pdb_text += QString ("").sprintf (
  ...
  for (map<QString, vector<point> >::iterator it = waters_to_add.begin ();
       it != waters_to_add.end (); it++)
    {
      for (unsigned int i = 0; i < it->second.size (); i++)
        {
          last_hydrated_pdb_text += QString ("").sprintf (
              "ATOM  %5d  OW  WAT %1s%4d    %8.3f%8.3f%8.3f  1.00  0.00       "
              "    O  \n",
              ++atom_number, chainID.toLatin1 ().data (), ++residue_number,
              it->second[i].axis[0], it->second[i].axis[1],
              it->second[i].axis[2]);
        }
    }
...
bool
US_Saxs_Util::selected_models_contain (QString residue)
{
...
bool
US_Saxs_Util::validate_pointmap ()
{
  ...
  for (map<QString, vector<QString> >::iterator it
       = pointmap_atoms_ref_residue.begin ();
       it != pointmap_atoms_ref_residue.end (); it++)
    {
      if (!to_erase.count (it->first))
        {
          if (!pointmap_atoms.count (it->first)
              ...
              || pointmap_atoms_dest[it->first].size () != it->second.size ())
            {
              errormsg
                  += QString (
                         "Internal error: inconsistancy with Pointmap %1\n")
                         .arg (it->first);
                  ...
                  if (!rotamers.count (it->second[i]))
                    {
                      errormsg
                          += QString (
                                 "Pointmap %1 contains invalid target rotamer "
                                 "%2, removing from loaded pointmaps\n")
                                 .arg (it->first)
                                 .arg (it->second[i]);
                      to_erase[it->first] = true;
                    }
                  else
                    {
                      if (!rotamers[it->second[i]].size ())
                        {
                          errormsg += QString ("Pointmap %1 contains empty "
                                               "target rotamer %2, removing "
                                               "from loaded pointmaps\n")
                                          .arg (it->first)
                                          .arg (it->second[i]);
                          to_erase[it->first] = true;
                        }
  ...
  for (map<QString, vector<vector<QString> > >::iterator it
       = pointmap_atoms_dest.begin ();
       it != pointmap_atoms_dest.end (); it++)
    {
      if (!to_erase.count (it->first))
        {
          for (unsigned int i = 0; i < it->second.size (); i++)
            {
              for (unsigned int j = 0; j < it->second[i].size (); j++)
                {
                  if (!rotamers[pointmap_atoms_ref_residue[it->first][i]][0]
                           .atom_map.count (it->second[i][j]))
                    {
                      errormsg
                          += QString (
                                 "Pointmap %1 target rotamer %2 has an "
                                 "invalid atom %3 (not found in rotamer), "
                                 "removing from loaded pointmaps\n")
                                 .arg (it->first)
                                 .arg (
                                     pointmap_atoms_ref_residue[it->first][i])
                                 .arg (it->second[i][j]);
...
bool
US_Saxs_Util::pdb_asa_for_saxs_hydrate ()
{
  if (!control_parameters.count ("inputfile"))
    {
      errormsg = "No input file found";
  ...
  if (control_parameters["asamethod"].toUInt () == 0)
    {
      ...
      int retval = surfracer_main (asa.hydrate_probe_radius, active_atoms,
                                   false, progress, editor);
      ...
      if (retval)
        {
          ...
          switch (retval)
            ...
            case US_SURFRACER_ERR_MISSING_RESIDUE:
              {
                errormsg = "US_SURFRACER encountered an unknown residue:"
                           + errormsg;
                return false;
                break;
              }
            case US_SURFRACER_ERR_MISSING_ATOM:
              {
                errormsg
                    = "US_SURFRACER encountered a unknown atom:\n" + errormsg;
                return false;
                break;
              }
            case US_SURFRACER_ERR_MEMORY_ALLOC:
              {
                errormsg
                    = "US_SURFRACER encountered a memory allocation error";
                return false;
                break;
              }
            default:
  ...
  if (control_parameters["asamethod"].toUInt () == 1)
    {
      ...
      cout << QString ("asa.hydrate_probe_radius %1\n"
                       "asa.probe_radius         %2\n"
                       "asa.asab1_step           %3\n"
      ...
      int retval
          = us_saxs_util_asab1_main (active_atoms, &asa, &results, false);
      ...
      if (retval)
        {
          ...
          switch (retval)
            ...
            case US_SAXS_UTIL_ASAB1_ERR_MEMORY_ALLOC:
              {
                errormsg = "US_HYDRODYN_ASAB1 encountered a memory allocation "
                           "error";
                return false;
                break;
              }
            ...
            if (this_atom->active)
              {
                ...
                if (!create_beads_normally
                    || ((this_atom->bead_assignment != last_bead_assignment
                        ...
                        && !(misc_pb_rule_on && this_atom->chain == 0
                                     ...
                                     .arg (this_atom->resName)))))
                  {
                    ...
                    if (create_beads_normally && misc_pb_rule_on
                        && !this_atom->type)
                      {
                ...
                PDB_atom *use_atom;
                if (misc_pb_rule_on && create_beads_normally
                ...
                if (this_atom->chain == 0 && misc_pb_rule_on
                    && this_atom->name == "N"
                    && !broken_chain_head.count (QString ("%1|%2")
                                                     .arg (this_atom->resSeq)
                                                     .arg (this_atom->resName))
    ...
    for (unsigned int j = 0; j < model_vector[i].molecule.size (); j++)
      {
        ...
        for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size ();
             k++)
          {
            ...
            if (misc_pb_rule_on && !k && this_atom->resName == "PRO")
              {
                first_is_pro = true;
              }
            if (placed_N1
            ...
            if (this_atom->name == "OXT" && last_main_chain_bead)
              {
                ...
                if (misc_pb_rule_on && this_atom->resName != "PRO"
                    && broken_chain_head.count (QString ("%1|%2")
                                                    .arg (this_atom->resSeq)
                                                    .arg (this_atom->resName))
                ...
                if (!misc_pb_rule_on && main_chain_beads == 1
            ...
            if (this_atom->active && this_atom->is_bead
                && this_atom->chain == 0)
              {
                ...
                if (misc_pb_rule_on && last_main_chain_bead
                    && !broken_chain_head.count (QString ("%1|%2")
                                                     .arg (this_atom->resSeq)
                                                     .arg (this_atom->resName))
                    && (this_atom->resName == "PRO"
            ...
            if (misc_pb_rule_on && this_atom->p_residue->type == 0
                ...
                && (k || this_atom->name != "N"))
              {
                if (multi_residue_map["PBR-NO-OXT"].size () == 1)
                  ...
                  {
                    cout << "Chain has broken end and PBR-NO-OXT isn't "
        ...
        if (misc_pb_rule_on && last_main_chain_bead
                    ...
                    .arg (last_main_chain_bead->resName)))
          {
            if (multi_residue_map["PBR-NO-OXT"].size () == 1)
              ...
              {
                noticemsg += "Chain has no terminating OXT and PBR-NO-OXT "
            ...
            if (this_atom->active && this_atom->is_bead)
              {
                this_atom->bead_ref_volume_unhydrated
                    = this_atom->bead_ref_volume;
                this_atom->bead_ref_volume
                    += misc_hydrovol * this_atom->bead_hydration;
            ...
            if (this_atom->active && this_atom->is_bead)
              {
                ...
                this_atom->visibility
                    = (this_atom->bead_asa
                       >= control_parameters["asahydratethresh"].toDouble ());
                ...
                cout << QString ("bead_asa %1 %2 %3 %4 %5\n")
                            ...
                            .arg (control_parameters["asahydratethresh"]
                                      .toDouble ());
#endif
                   ...
                   && (k || this_atom->name != "N" || restype != 0
                       || broken_chain_head.count (
                           QString ("%1|%2")
                               .arg (this_atom->resSeq)
                               .arg (this_atom->resName))
                       || this_atom->resName == "PRO" || !misc_pb_rule_on))
                  ||

                  // if pb_rule is off, final OXT before P needs to use OXT-P
                  (residue_list[m].name == "OXT" && this_atom->name == "OXT"
                   && (misc_pb_rule_on || this_atom->resName != "PRO"))
                  || (residue_list[m].name == "OXT-P"
                      && (this_atom->name == "OXT" && !misc_pb_rule_on
                          && this_atom->resName == "PRO"))
                  ||

                  (!k && this_atom->name == "N" && restype == 0
                   && misc_pb_rule_on && residue_list[m].name == "N1"
                   && !broken_chain_head.count (QString ("%1|%2")
                                                    .arg (this_atom->resSeq)
                                                    .arg (this_atom->resName))
                   && this_atom->resName != "PRO"))
          ...
          if (respos == -1)
            {
              if ((this_atom->name != "H" && this_atom->name != "D"
                ...
                {
                  errormsg.append (QString ("").sprintf (
          ...
          if (respos != -1)
            {
                   ...
                   m++)
                {
                  ...
                  if (!residue_list[respos].r_atom[m].tmp_used
                      && (residue_list[respos].r_atom[m].name
                              == this_atom->name
                          || (this_atom->name == "N" && misc_pb_rule_on && !k
              ...
              if (atompos == -1)
              ...
              else
                {
                  ...
                  if (our_saxs_options.compute_saxs_coeff_for_bead_models)
                    {
                      ...
                      if (!residue_atom_hybrid_map.count (mapkey))
                        {
                          cout
                              << QString ("%1Molecule %2 Residue %3 %4 Hybrid "
                                          "name missing. Atom skipped.\n")
                                     .arg (this_atom->chainID == " "
                                               ? ""
                      ...
                      else
                        {
                          ...
                          if (hybrid_name.isEmpty () || !hybrid_name.length ())
                            {
                              cout << QString (
                                          "%1Molecule %2 Residue %3 %4 Hybrid "
                                          "name missing. Atom skipped.\n")
                                          .arg (this_atom->chainID == " "
                                                    ? ""
                                                    : ("Chain "
                          ...
                          else
                            {
                              if (!saxs_util->hybrid_map.count (hybrid_name))
                                {
                                  cout << QString (
                                              "%1Molecule %2 Residue %3 %4 "
                                              "Hybrid %5 name missing from "
                                              "Hybrid file. Atom skipped.\n")
                                              .arg (this_atom->chainID == " "
                                                        ? ""
                                                        : ("Chain "
                                                           + this_atom->chainID
                                ...
                                {
                                  if (!saxs_util->atom_map.count (
                                          this_atom->name + "~" + hybrid_name))
                                    {
                                      cout << QString (
                                                  "%1Molecule %2 Atom %3 "
                                                  "Residue %4 %5 Hybrid %6 "
                                                  "name missing from Atom "
                                                  "file. Atom skipped.\n")
                                                  .arg (this_atom->chainID
                                                                == " "
                                                            ? ""
                                    ...
                                    {
                                      this_atom->saxs_name
                                          = saxs_util->hybrid_map[hybrid_name]
                                                .saxs_name;
                                      ...
                                      if (count_hydrogens.indexIn (hybrid_name)
                                        ...
                                        {
                                          this_atom->hydrogens
                                              = count_hydrogens.cap (1)
                                                    .toInt ();
                                        }
                                      this_atom->saxs_excl_vol
                                          = saxs_util
                                                ->atom_map[this_atom->name
                                                           + "~" + hybrid_name]
                                                .saxs_excl_vol;
                                      if (!saxs_util->saxs_map.count (
                                              saxs_util
                                                  ->hybrid_map[hybrid_name]
                                                  .saxs_name))
                                        {
                                          cout
                                              << QString (
                  ...
                  if (!misc_pb_rule_on && this_atom->name == "N" && !k)
                    {
                      this_atom->mw += 1.0;
                      this_atom->bead_ref_mw += 1.0;
                    }
...
void
US_Saxs_Util::get_atom_map (PDB_model *model)
{
  atom_counts.clear ();
  has_OXT.clear ();
  ...
  for (unsigned int j = 0; j < model->molecule.size (); j++)
    {
      ...
      if (lastResSeq != "")
        {
          atom_counts
              [QString ("%1|%2|%3").arg (j).arg (lastResName).arg (lastResSeq)]
              = atom_count;
        }
    }
  ...
  for (unsigned int j = 0; j < model->molecule.size (); j++)
    {
              ...
              || lastResName != this_atom->resName)
            {
              if (multi_residue_map.count (this_atom->resName)
                  ...
                  && multi_residue_map[this_atom->resName][0]
                         < (int)residue_list.size ()
                  && residue_list[multi_residue_map[this_atom->resName][0]]
                             .type
                         == 0)
                {
                  aa++;
                  ...
                  || lastResName != this_atom->resName)
                {
                  ...
                  if ((lastResSeq != ""
                          ...
                          && unknown_residues.count (this_atom->resName)))
                    {
                      ...
                      //               multi_residue_map.count(this_atom->resName)
                      //               &&
                      //   residue_list[multi_residue_map[this_atom->resName][0]].type
                      //   == 0 && multi_residue_map.count(lastResName) &&
                      //   residue_list[multi_residue_map[lastResName][0]].type
                      //   == 0 )
                      {
                        ...
                        us_log->log (
                            lastResSeq != ""
                                ? QString (
                                      "Warning: break in residue sequence or "
                                      "unknown residue: %1Molecule %2 Residue "
                                      "%3 %4 & %5 %6.\n")
                                      .arg (this_atom->chainID == " "
                                                ? ""
                                                : ("Chain "
                                      ...
                                      .arg (this_atom->resName)
                                      .arg (this_atom->resSeq)
                                : QString ("Warning: break in residue "
                                           "sequence or unknown residue: "
                                           "%1Molecule %2 Residue %3 %4.\n")
                                      .arg (this_atom->chainID == " "
                                                ? ""
                                                : ("Chain "
      ...
      if (aa && non_aa)
        {
          us_log->log (
              QString ("Notice: %1found %2 non or unknown Amino Acids in a "
                       "chain containing %3 AA Residues.\n")
                  .arg (lastChainID == " " ? ""
                                           : ("Chain " + lastChainID + " "))
                  .arg (non_aa)
                  .arg (aa));
          if (us_udp_msg)
            {
              map<QString, QString> msging;
              msging["_textarea"]
                  = QString ("Notice: %1found %2 non or unknown Amino Acids "
                             "in a chain containing %3 AA Residues. \\n")
                        .arg (lastChainID == " "
                                  ? ""
                                  : ("Chain " + lastChainID + " "))
                        .arg (non_aa)
...
void
US_Saxs_Util::build_molecule_maps (PDB_model *model)
{
      ...
      for (unsigned int k = 0; k < model->molecule[j].atom.size (); k++)
        {
          QString idx = QString ("%1|%2").arg (j).arg (
              model->molecule[j].atom[k].resSeq);
          ...
          molecules_residues_atoms[idx].push_back (
  ...
  for (unsigned int i = 0; i < molecules_idx_seq.size (); i++)
    {
      ...
      if (multi_residue_map.count (resName))
        {
          int minimum_missing
              = INT_MAX; // find the entry with the minimum # of missing atoms
          ...
          for (unsigned int j = 0; j < multi_residue_map[resName].size (); j++)
            {
              ...
              for (unsigned int r = 0;
                   r < residue_list[multi_residue_map[resName][j]]
                           .r_atom.size ();
                   r++)
                {
                  residue_list[multi_residue_map[resName][j]]
                      .r_atom[r]
                      .tmp_used
                      = false;
                }
              skip_residue[QString ("%1|%2").arg (idx).arg (
                  multi_residue_map[resName][j])]
                  = false;

              // now set flags
              // first check for non-coded atoms
              for (unsigned int k = 0;
                   k < molecules_residues_atoms[idx].size (); k++)
                {
                  bool found = false;
                  bool any = false;
                  for (unsigned int r = 0;
                       ...
                       r++)
                    {
                      if (residue_list[multi_residue_map[resName][j]]
                              .r_atom[r]
                              .name
                  ...
                  if (!found)
                    {
                      ...
                      residue_errors[chains[idx] + "~" + resnames[idx] + " "
                                     + resseqs[idx]]
                          = true;
                    }
                }
                           ...
                           .tmp_used)
                    {
                      errormsg
                          += QString ("Missing atom %1. ")
                                 .arg (residue_list[multi_residue_map[resName]
                                                                     [j]]
                                           .r_atom[r]
                                           .name);
                      if (missing_count != -1)
                        {
                          missing_atoms.push_back (
                              residue_list[multi_residue_map[resName][j]]
                                  .r_atom[r]
                                  .name);
                          missing_atoms_beads.push_back (
                              residue_list[multi_residue_map[resName][j]]
                                  .r_atom[r]
                                  .bead_assignment);
                          molecules_residue_missing_atoms_skip
                              [QString ("%1|%2|%3").arg (idx).arg (j).arg (r)]
                              = true;
                          missing_count++;
                        }
                    }
              ...
              molecules_residue_missing_atoms_beads[idx].push_back (
                  missing_atoms_beads);
        ...
        }
      {
        ...
        if (k > -1 && molecules_residue_missing_atoms[idx][k].size () == 0
            && molecules_residue_missing_atoms_beads[idx][k].size () == 0)
          {
            use_residue[idx] = multi_residue_map[resName][k];
            // skip the rest
            for (unsigned int j = 0; j < multi_residue_map[resName].size ();
                 j++)
              {
                if (j != (unsigned int)k)
                  {
...
bool
US_Saxs_Util::check_for_missing_atoms (PDB_model *model)
{
  QStringList qsl;
...
bool
US_Saxs_Util::check_for_missing_atoms (PDB_model *model, QStringList &qsl)
{
                              ...
                              if (!residue_list[lastResPos].r_atom[l].tmp_flag)
                                {
                                  // missing atoms
                                  errors_found++;
                                  if (control_parameters["pdbmissingatoms"]
                                          .toUInt ()
                                      == 0)
                                    {
                                      failure_errors++;
          ...
          if (respos == -1)
            {
              if ((this_atom->name != "H" && this_atom->name != "D"
                   ...
                   && (this_atom->altLoc == "A" || this_atom->altLoc == " ")))
                {
                  ...
                  residue_errors[this_atom->orgChainID + "~"
                                 + this_atom->orgResName + " "
                                 + this_atom->orgResSeq]
                      = true;
                      ...
                      if (multi_residue_map[this_atom->resName].size ())
                        {
                          ...
                          if (control_parameters["pdbmissingatoms"].toUInt ()
                              == 0)
                            {
                              failure_errors++;
                            }
                              ...
                              == 2)
                            {
                              if (molecules_residue_min_missing
                                  ...
                                  == -1)
                                {
                                  ...
                                  switch (
                                      control_parameters["pdbmissingresidues"]
                                          .toUInt ())
                                    {
                                    case 0:
                                      failure_errors++;
                                    ...
                                    default:
                                      failure_errors++;
                                      errormsg = "Internal error: Unexpected "
                                                 "PdbMissingResidues type "
                                                 + QString ("%1\n").arg (
                                                     control_parameters
                                                         ["pdbmissingresidues"]
                                                             .toUInt ());
                      ...
                      else
                        {
                          ...
                          if (control_parameters["pdbmissingresidues"]
                                  .toUInt ()
                              == 0)
                            {
                              failure_errors++;
              ...
              if (atompos == -1)
                {
                  ...
                  switch (control_parameters["pdbmissingatoms"].toUInt ())
                    ...
                    case 2:
                      ...
                      if (bead_exceptions[count_idx] == 1)
                        {
                          switch (control_parameters["pdbmissingresidues"]
                                      .toUInt ())
                            {
                            case 0:
                              failure_errors++;
      ...
      if (lastResPos != -1)
        {
          ...
          for (unsigned int l = 0; l < residue_list[lastResPos].r_atom.size ();
            ...
            {
              if (!residue_list[lastResPos].r_atom[l].tmp_flag
                  && !(misc_pb_rule_on && residues_found == 1
                       && N1s_placed == 1))
                {
              ...
              for (unsigned int k = 0; k < org_model.molecule[j].atom.size ();
                   k++)
                {
                  ...
                  switch (bead_exceptions[count_idx])
                    ...
                    case 3:
                      {
                        ...
                        if (this_atom->resSeq != lastResSeq)
                          {
                            ...
                            new_residue.molvol
                                = misc_avg_volume * atom_counts[count_idx];
                            ...
                            new_bead.hydration
                                = (unsigned int)(misc_avg_hydration
                                                     * atom_counts[count_idx]
                                                 + .5);
                            ...
                            new_bead.volume
                                = misc_avg_volume * atom_counts[count_idx];
                            new_bead.mw
                                = misc_avg_mass * atom_counts[count_idx];
                        ...
                        else
                          {
                            ...
                            if (new_residue_name != new_residue.name)
                              {
                                errormsg = "Internal error: Unexpected new "
                                           "residue name inconsistancy.\n"
                                           + QString ("\"%1\" != \"%2\"")
                                                 .arg (new_residue_name)
                        ...
                        if (new_residue.name != residue_list[respos].name)
                          {
                            errormsg
                                = "Internal error: Unexpected new residue "
                                  "name residue_list inconsistancy.";
                            return false;
                          }
                    ...
                    case 4: // missing atoms
                      {
                        ...
                        if (pos < 0)
                          {
                            errormsg
                                = QString ("Internal error: idx %1 Missing "
                                           "atom error %2 ! < 0!\n")
                                      .arg (idx)
                                      .arg (pos);
                            return false;
                                        ...
                                        if (beads_missing_atom_count
                                                [new_atom.bead_assignment])
                                          {
                                            ...
                                            new_atom.hybrid.radius
                                                *= atoms_scale_radius
                                                    [new_atom
                                                         .bead_assignment]; // misc_avg_radius;
                                            new_atom.hybrid.radius
                                                = (int)(new_atom.hybrid.radius
                                                            * 100
                                                        + .5)
                        ...
                        if (new_residue.name != residue_list[respos].name)
                          {
                            errormsg
                                = "Internal error: Unexpected new residue "
                                  "name residue_list inconsistancy (a).";
                            return false;
                          }
                        // ok, now we can push back the modified atom
                        PDB_atom atom_to_add = org_model.molecule[j].atom[k];
                    ...
                    default:
                      errormsg
                          = "Internal error: Unhandled bead_exception code.";
                      return false;
                      break;
                    } // switch
                } // atoms
...
void
US_Saxs_Util::reset_chain_residues (PDB_model *model)
{
  build_molecule_maps (model);
          ...
          if (!checked.count (idx))
            {
              ...
              if (this_atom->p_residue && this_atom->model_residue_pos != -1)
                {
                  // cout << "Test3" << endl; // Problem after that with pdbs
                  // with missing atoms - does not go to both Test3a and
                  // Test3b!!!

                  if ((int)model->residue.size ()
                      <= this_atom->model_residue_pos)
                    {

                      // cout << "Test3a" << endl;
                      // cout << QString( "model->residue does not contain
                      // model residue pos %1\n" ).arg(
                      // this_atom->model_residue_pos ) << flush;
                      continue;
                    }

                  // qDebug (QString (" rp %1 sr %2 pr %3"
                  // ).arg(this_atom->model_residue_pos).arg(model->residue.size()).arg((long)
                  // this_atom->p_residue));

                  // qDebug (QString ( "%1  %2" ).arg(model->residue[
                  // this_atom->model_residue_pos
                  // ].unique_name).arg(this_atom->p_residue->unique_name));
                  // qDebug (QString ( "%1 ").arg(model->residue[
                  // this_atom->model_residue_pos ].unique_name));

                  // cout << "Test3aa" << endl;

                  if (model->residue[this_atom->model_residue_pos].unique_name
                      != (this_atom->p_residue)->unique_name)
                    {
                      cout << "Test3b" << endl;
                      // cout << QString( "found residue difference %1 %2,
                      // fixing\n" )
                      //    .arg( model->residue[ this_atom->model_residue_pos
                      //    ].unique_name ) .arg(
                      //    this_atom->p_residue->unique_name );
                      model->residue[this_atom->model_residue_pos]
                          = *this_atom->p_residue;
                      // qDebug (QString ( "%1  %2" ).arg(model->residue[
                      // this_atom->model_residue_pos
                      // ].unique_name).arg(this_atom->p_residue->unique_name));
...
public:
  ...
  QString index;
  bool
  operator< (const sortable_float_qs &objIn) const
  {
    return (f < objIn.f);
  }
};
...
bool
US_Saxs_Util::compute_waters_to_add_alt ()
{
  puts ("Transforming waters to add to pdb coordinates");
  ...
  // phase zero: build up list in ASA order and process in that order
  // potentially doing any type of pointmap/best fit etc.
  ...
  for (list<sortable_float_qs>::iterator it = lsf.begin (); it != lsf.end ();
       it++)
    {
      ...
      if (best_fit_rotamer.count (this_residue))
        {
          ...
          if (!to_hydrate.count (this_residue))
            {
              errormsg = QString ("Internal error: best_fit_rotamer key %1 "
                                  "not found in to_hydrate")
                             .arg (this_residue);
              return false;
            }
          ...
          if (best_fit.water_positioning_atoms.size ()
            ...
            {
              errormsg = QString ("Internal error: water positioning atom "
                                  "size %1 does not match waters size %2")
          ...
          if (rotated_rotamers.count (best_fit.name))
            {
              // find best choice
              cout << QString ("this best fit rotamer %1 is in with rotated "
                               "rotamers %2\n")
                          .arg (this_residue)
                          .arg (best_fit.name);
              // for each water
              if (rotated_rotamers[best_fit.name].size ()
                  != best_fit.water_positioning_atoms.size ())
                {
                  errormsg
                      = QString ("Internal error: water positioning atom size "
                                 "%1 does not match best fit rotamer size %2")
                            .arg (best_fit.water_positioning_atoms.size ())
                            .arg (best_fit.waters.size ());
                  return false;
                }
              ...
              for (unsigned int ii = 0;
                   ii < best_fit.water_positioning_atoms.size (); ii++)
                {
                  ...
                  for (unsigned int m = 0;
                       m < rotated_rotamers[best_fit.name][ii].size (); m++)
                    {
                      rotamer this_ref_rotamer
                          = rotated_rotamers[best_fit.name][ii][m];
                      ...
                      if (best_fit.water_positioning_atoms[ii].size ()
                          != this_ref_rotamer.water_positioning_atoms[0]
                                 .size ())
                        {
                          errormsg
                              = QString (
                                    "Internal error: best fit waters count %1 "
                                    "does not match reference %2")
                                    .arg (best_fit.water_positioning_atoms[ii]
                                              .size ())
                                    .arg (this_ref_rotamer
                                              .water_positioning_atoms[0]
                                              .size ());
                          return false;
                        }

                      for (unsigned int jj = 0;
                           jj < this_ref_rotamer.water_positioning_atoms[0]
                                    .size ();
                           jj++)
                        {
                          if (!this_ref_rotamer.atom_map.count (
                                  this_ref_rotamer
                                      .water_positioning_atoms[0][jj]))
                            {
                              errormsg
                                  = QString (
                                        "Internal error: water positioning "
                                        "atom %1 not found in atom_map")
                                        .arg (
                                            this_ref_rotamer
                                                .water_positioning_atoms[0]
                                                                        [jj]);
                          ...
                          // this must change to the atom map of the reference
                          // rotamer
                          p1[jj]
                              = this_ref_rotamer
                                    .atom_map[this_ref_rotamer
                                                  .water_positioning_atoms[0]
                                                                          [jj]]
                                    .coordinate;

                          if (!to_hydrate[this_residue].count (
                                  best_fit.water_positioning_atoms[ii][jj]))
                            {
                              errormsg
                                  = QString ("Internal error: water "
                                             "positioning atom %1 not found "
                                             "in to_hydrate atoms")
                                        .arg (
                                            best_fit
                                                .water_positioning_atoms[ii]
                                                                        [jj]);
                              return false;
                            }
                          p2[jj] = to_hydrate
                      ...
                      rotamer_waters.push_back (
                          this_ref_rotamer.waters[0].coordinate);
                      vector<point> new_waters;
                      if (!atom_align (p1, p2, rotamer_waters, new_waters))
                        {
                          return false;
                      ...
                      float this_dist
                          = min_dist_to_struct_and_waters (new_waters[0]);
                      if (this_dist > 0)
                        {
                          // min_dist = this_dist;
                          min_dist_pos = m;
                  ...
                  count_waters++;
                  cout << QString ("Notice: best_fit rotamer %1 water %2 has "
                                   "%3 rotated rotamers and %4\n")
                              .arg (best_fit.name)
                              .arg (ii)
                              .arg (
                                  rotated_rotamers[best_fit.name][ii].size ())
                              .arg (
                                  found_non_clash
                                      ? QString ("the best one is number %1 ")
                                            .arg (min_dist_pos)
                                      : QString (
                                            "there was a clash at all tries"));

                  if (found_non_clash && !has_steric_clash (min_water[0]))
                    {
                      count_waters_added++;
                      waters_to_add[this_residue].push_back (min_water[0]);
                      waters_source[this_residue].push_back (
                          QString ("RtmrMC:%1").arg (best_fit.name));
            ...
            {
              // std add one
              cout << QString ("this best fit rotamer %1 is NOT a rotated "
                               "rotamers %2\n")
                          .arg (this_residue)
                          .arg (best_fit.name);

              // for each water to add:
              // cout << QString( "need to compute best transform matrix for
              // %1:\n" ).arg( it->first );

              //             if ( best_fit.water_positioning_atoms.size() !=
              //                  best_fit.waters.size() )
              //             {
              //                errormsg = QString( "Internal error: water
              //                positioning atom size %1 does not match waters
              //                size %2" )
              //                   .arg(
              //                   best_fit.water_positioning_atoms.size() )
              //                   .arg( best_fit.waters.size() );
              ...
              for (unsigned int i = 0;
                   i < best_fit.water_positioning_atoms.size (); i++)
                {
                  // get coordinates of
                  ...
                  for (unsigned int j = 0;
                       j < best_fit.water_positioning_atoms[i].size (); j++)
                    {
                      if (!best_fit.atom_map.count (
                              best_fit.water_positioning_atoms[i][j]))
                        {
                          errormsg
                              = QString ("Internal error: water positioning "
                                         "atom %1 not found in atom_map")
                                    .arg (best_fit
                                              .water_positioning_atoms[i][j]);
                          return false;
                        }
                      p1[j] = best_fit
                                  .atom_map[best_fit
                      ...
                      if (!to_hydrate[this_residue].count (
                              best_fit.water_positioning_atoms[i][j]))
                        {
                          errormsg
                              = QString (
                                    "Internal error: water positioning atom "
                                    "%1 not found in to_hydrate atoms")
                                    .arg (best_fit
                                              .water_positioning_atoms[i][j]);
                          return false;
                        }
                      p2[j]
                          = to_hydrate[this_residue]
                                      [best_fit.water_positioning_atoms[i][j]];
                  ...
                  vector<point> new_waters;
                  // cout << QString( " and apply it to the point
                  // [%1,%2,%3]\n") .arg( best_fit.waters[ i ].coordinate.axis[
                  // 0 ] ) .arg( best_fit.waters[ i ].coordinate.axis[ 1 ] )
                  ...
                  if (!has_steric_clash (new_waters[0]))
                    {
                      cout << QString ("Notice: best fit for %1 has no "
                                       "rotated rotamers and no clash\n")
                                  .arg (best_fit.name);
                      count_waters_added++;
                      waters_to_add[this_residue].push_back (new_waters[0]);
                      waters_source[this_residue].push_back (
                          QString ("Rtmr:%1").arg (best_fit.name));
      ...
      if (pointmap_rotamers.count (this_residue))
        {
          if (!to_hydrate_pointmaps.count (this_residue))
            {
              errormsg = QString ("Internal error: pointmap_rotamers key %1 "
                                  "not found in to_hydrate_pointmaps")
                             .arg (this_residue);
              return false;
            }
          ...
          if (pointmap_rotamers[this_residue].size ()
                  != pointmap_atoms[resName].size ()
              || pointmap_rotamers[this_residue].size ()
                     != pointmap_atoms_dest[resName].size ()
              || pointmap_rotamers[this_residue].size ()
                     != pointmap_atoms_ref_residue[resName].size ())
            {
              errormsg = QString ("Internal error: could not find pointmap "
                                  "size inconsistancy for key %1")
                             .arg (this_residue);
              return false;
            }

          // this is for each water
          for (unsigned int i = 0; i < pointmap_rotamers[this_residue].size ();
               i++)
            {
              // for each water to add:
              // compute transformation matrix from the pointmap_atoms_dest in
              // the pointmap_atom_ref_residue to the pointmap_atoms in the
              // residue and apply to each water in the rotamer the rotamer's
              // atom_map(equivalently side chain) & waters have the "dest"
              // info

              rotamer ref_rotamer = pointmap_rotamers[this_residue][i];
#if defined(DEBUG_POINTMAP)
              cout << QString ("Note: in compute_waters_to_add_alt(), current "
                               "name '%1' has alternates %2\n")
                          .arg (ref_rotamer.name)
                          .arg (rotated_rotamers.count (ref_rotamer.name)
                                    ? "yes"
                                    : "no");
#endif
              ...
              if (rotated_rotamers.count (ref_rotamer.name))
                {
                  ...
                  if (rx_expand_mapkey.indexIn (this_residue) == -1)
                    {
                      errormsg
                          = QString (
                                "Internal error: could not expand mapkey %1")
                                .arg (this_residue);
                      return false;
                    }
                  ...
                  if (!pointmap_atoms.count (resName)
                    ...
                    {
                      errormsg
                          = QString ("Internal error: could not find pointmap "
                                     "entries for residue of key %1")
                                .arg (resName);
                      return false;
                    }
                  ...
                  for (unsigned int k = 0;
                       k < rotated_rotamers[ref_rotamer.name][i].size (); k++)
                    {
                      rotamer this_ref_rotamer
                          = rotated_rotamers[ref_rotamer.name][i][k];

                      if (this_ref_rotamer.residue
                          != pointmap_atoms_ref_residue[resName][i])
                        {
                          errormsg
                              = QString ("Internal error: inconsistancy for "
                                         "reference residue name for key %1 "
                                         "pos %2 (%3 != %4) ")
                                    .arg (this_residue)
                                    .arg (i)
                                    .arg (this_ref_rotamer.residue)
                                    .arg (pointmap_atoms_ref_residue[resName]
                                                                    [i]);
                          return false;
                        }
                      ...
                      if (p1.size () != p2.size ())
                        {
                          errormsg
                              = QString ("Internal error: size inconsistancy "
                                         "for reference residue name for key "
                                         "%1 pos %2 (%3 != %4)")
                                    .arg (this_residue)
                                    .arg (i)
                                    .arg (pointmap_atoms[resName][i].size ())
                                    .arg (pointmap_atoms_dest[resName][i]
                                              .size ());
                          return false;
                        }
                      ...
                      for (unsigned int j = 0; j < p1.size (); j++)
                        {
                          ...
                          if (!this_ref_rotamer.atom_map.count (
                                  pointmap_atoms_dest[resName][i][j]))
                            {
                              errormsg
                                  = QString (
                                        "Internal error: atom %1 not found in "
                                        "to_hydrate_pointmaps atoms")
                                        .arg (pointmap_atoms_dest[resName][i]
                                                                 [j]);
                              return false;
                          ...
                          p1[j] = this_ref_rotamer
                                      .atom_map[pointmap_atoms_dest[resName][i]
                                                                   [j]]
                                      .coordinate;

                          if (!to_hydrate_pointmaps[this_residue].count (
                                  pointmap_atoms[resName][i][j]))
                            {
                              errormsg
                                  = QString (
                          ...
                          cout << QString ("mapping: %1 %2 source %3\n")
                                      ...
                                      .arg (pointmap_atoms[resName][i][j]);
#endif
                          p2[j] = to_hydrate_pointmaps
                              [this_residue][pointmap_atoms[resName][i][j]];
                      ...
                      if (rotated_rotamers[this_ref_rotamer.name].size () != 1)
                        {
                          errormsg
                              = QString (
                                    "Error: multiple rotated pointmap rotamer "
                                    "%1 has %2 waters and only one water is "
                                    "currently supported for this scheme ")
                                    .arg (this_ref_rotamer.name)
                                    .arg (
                                        rotated_rotamers[this_ref_rotamer.name]
                                            .size ());
                          return false;
                        }

                      for (unsigned int m = 0;
                           m < rotated_rotamers[this_ref_rotamer.name][0]
                                   .size ();
                           m++)
                        {
                          vector<point> rotamer_waters;
                          // this is probably not correct:
                          rotamer_waters.push_back (
                              rotated_rotamers[this_ref_rotamer.name][0][m]
                                  .waters[0]
                                  .coordinate);
                          vector<point> new_waters;
                          if (!atom_align (p1, p2, rotamer_waters, new_waters))
                  ...
                  cout << QString ("Notice: pointmap for %1 has %2 rotated "
                                   "rotamers and %3\n")
                              .arg (ref_rotamer.name)
                              .arg (rotated_rotamers[ref_rotamer.name][0]
                                        .size ())
                              .arg (
                                  found_non_clash
                                      ? QString ("the best one is number %1 ")
                  ...
                  if (found_non_clash && !has_steric_clash (min_water[0]))
                    {
                      ...
                      waters_to_add[this_residue].push_back (min_water[0]);
                      waters_source[this_residue].push_back (
                          QString ("PMmc:%1").arg (ref_rotamer.name));
              ...
              else
                {
                  ...
                  if (ref_rotamer.residue
                      != pointmap_atoms_ref_residue[resName][i])
                    {
                      errormsg
                          = QString (
                                "Internal error: inconsistancy for reference "
                                "residue name for key %1 pos %2 (%3 != %4)")
                                .arg (this_residue)
                                .arg (i)
                  ...
                  if (p1.size () != p2.size ())
                    {
                      errormsg
                          = QString ("Internal error: size inconsistancy for "
                                     "reference residue name for key %1 pos "
                                     "%2 (%3 != %4)")
                                .arg (this_residue)
                                .arg (i)
                                .arg (pointmap_atoms[resName][i].size ())
                  ...
                  for (unsigned int j = 0; j < p1.size (); j++)
                    {
                      ...
                      // .arg(i)
                      // .arg(pointmap_atoms_dest[ resName ][ i ][ j ] );
                      if (!ref_rotamer.atom_map.count (
                              pointmap_atoms_dest[resName][i][j]))
                        {
                          errormsg
                              = QString ("Internal error: atom %1 not found "
                                         "in to_hydrate_pointmaps atoms")
                                    .arg (pointmap_atoms_dest[resName][i][j]);
                          return false;
                        }
                  ...
                  for (unsigned int j = 0; j < ref_rotamer.waters.size (); j++)
                    {
                      vector<point> rotamer_waters;
                      rotamer_waters.push_back (
                          ref_rotamer.waters[j].coordinate);
                      vector<point> new_waters;
                      if (!atom_align (p1, p2, rotamer_waters, new_waters))
                        {
                          return false;
                      ...
                      if (!has_steric_clash (new_waters[0]))
                        {
                          cout << QString ("Notice: pointmap for %1 has no "
                                           "rotated rotamers and no clash\n")
                                      .arg (ref_rotamer.name);
                          count_waters_added++;
                          waters_to_add[this_residue].push_back (
                              new_waters[0]);
                          waters_source[this_residue].push_back (
                              QString ("PM:%1").arg (ref_rotamer.name));
                          cout
                              << QString ("p5 adding to waters source %1 %2\n")
                                     .arg (this_residue)
                                     .arg (ref_rotamer.name);
                        }
                      else
                        {
                          cout
                              << QString ("Notice: pointmap for %1 has no "
                                          "rotated rotamers and has a clash\n")
                                     .arg (ref_rotamer.name);
                          count_waters_not_added++;
                        }
                    }
...
bool
US_Saxs_Util::has_steric_clash (point p)
{
  unsigned int i = current_model;

  double dist_threshold
      = 1e0 - (our_saxs_options.steric_clash_distance / 100e0);
  double water_radius
      = multi_residue_map.count ("WAT")
            ? residue_list[multi_residue_map["WAT"][0]].r_atom[0].hybrid.radius
            : 1.401;
  ...
  for (unsigned int j = 0; j < model_vector[i].molecule.size (); j++)
    {
      for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size ();
           k++)
        {
          PDB_atom *this_atom = &(model_vector[i].molecule[j].atom[k]);
          if (dist (this_atom->coordinate, p)
              < (this_atom->radius + water_radius) * dist_threshold)
            {
              return true;
            }
        }
...
bool
US_Saxs_Util::list_steric_clash_recheck ()
{
  ...
  unsigned int i = current_model;
  double dist_threshold
      = 1e0 - (our_saxs_options.steric_clash_recheck_distance / 100e0);
  double water_radius
      = multi_residue_map.count ("WAT")
            ? residue_list[multi_residue_map["WAT"][0]].r_atom[0].hybrid.radius
            : 1.401;
  ...
  hydrate_clash_detail
      += "Water,clash,radius water,radius clash,sum "
         "radii,distance,overlap,percent overlap,water x,clash x, water y, "
         "clash y, water z, clash z, source, source2\n";
              ...
              for (unsigned int k = 0;
                   k < model_vector[i].molecule[j].atom.size (); k++)
                {
                  ...
                  if (dist (this_atom->coordinate, p)
                      < (this_atom->radius + water_radius) * dist_threshold)
                    {
                      hydrate_clash_map_structure[QString ("%1%2:%3.%4")
                                                      .arg (this_atom->resName)
                                                      .arg (this_atom->resSeq)
                                                      .arg (this_atom->chainID)
                                                      .arg (this_atom->name)]
                      ...
                      if (!waters_source.count (it->first))
                        {
                          cout << QString (
                                      "waters_source does not contain %1\n")
                                      .arg (it->first);
                          cout << QString ("current waters source map:\n");
                          for (map<QString, vector<QString> >::iterator it2
                               = waters_source.begin ();
                               it2 != waters_source.end (); it2++)
                            {
                          ...
                          errormsg
                              = QString ("Internal error: "
                                         "list_steric_clash_recheck(): "
                                         "waters_source does not contain %1")
                                    .arg (it->first);
                          return false;
                        }
                      if ((unsigned int)waters_source[it->first].size ()
                          <= pos)
                        {
                          cout << QString ("waters_source[ %1 ] does not "
                                           "contain pos %2 size only %3\n")
                                      .arg (it->first)
                                      .arg (pos)
                                      .arg (waters_source[it->first].size ());
                          errormsg = QString ("Internal error: "
                                              "list_steric_clash_recheck(): "
                                              "waters_source[ %1 ] does not "
                                              "contain pos %2")
                                         .arg (it->first)
                                         .arg (pos);
                          return false;
                      ...
                      if (waters_source[it->first][pos].startsWith ("PM"))
                        {
                          hydrate_clash_map_pm_water
                              [QString ("%1~%2").arg (it->first).arg (pos)]
                              = true;
                        }
                      else
                        {
                      ...
                      hydrate_clash_log
                          << QString ("Water %1 number %2 clashes with "
                                      "%3~%4~%5 atom %6 by %7 A or %8%\n")
                                 .arg (it->first)
                                 .arg (pos + 1)
                                 .arg (this_atom->resName)
                                 .arg (this_atom->resSeq)
                      ...
                      hydrate_clash_detail += QString ("%1").arg (
                          waters_source[it->first][pos]);

                      hydrate_clash_detail += "\n";

                      if (this_atom->chain == 1)
                        {
                          steric_clash_recheck_summary
                              [" structure side chain"]++;
                          if (!counted.count (
                                  QString ("%1~%2").arg (it->first).arg (pos)))
                            {
                              steric_clash_recheck_summary
                                  ["waters with clashes total"]++;
                              if (waters_source[it->first][pos].startsWith (
                                      "PM"))
                                {
                                  steric_clash_recheck_summary
                                      ["waters with clashes (Pointmap)"]++;
                                }
                              else
                                {
                                  steric_clash_recheck_summary
                                      ["waters with clashes (Rotamer)"]++;
                                }
                              counted[QString ("%1~%2").arg (it->first).arg (
                                  pos)]
                                  = true;
                            }
                        }
                        ...
                        {
                          steric_clash_recheck_summary
                              [" structure main chain"]++;
                        }
                      steric_clash_recheck_summary[" structure total"]++;
                      steric_clash_recheck_summary
                          ["structure or other water"]++;
                    }
                }
            }

          // check other added waters:
          // map < QString, vector < point > >::iterator it2 =
          // waters_to_add.begin(); while ( *it2 != *it && it2 !=
          // waters_to_add.end() )
          // {
          // it2++;
          // }
          for (map<QString, vector<point> >::iterator it2 = it;
               it2 != waters_to_add.end (); it2++)
            {
              for (unsigned int j = (it2 == it ? pos + 1 : 0);
                   j < it2->second.size (); j++)
                {
                  if (dist (it2->second[j], p)
                      < 2e0 * water_radius * dist_threshold)
                    {
                      if (waters_source[it->first][pos].startsWith ("PM"))
                        {
                      ...
                      if (waters_source[it2->first][j].startsWith ("PM"))
                        {
                          hydrate_clash_map_pm_water
                              [QString ("%1~%2").arg (it2->first).arg (j)]
                              = true;
                        }
                      else
                        {
                      ...
                      hydrate_clash_log
                          << QString ("Water %1 number %2 clashes with water "
                                      "from %3 number %4 by %5 A or %6%\n")
                                 .arg (it->first)
                                 .arg (pos + 1)
                                 .arg (it2->first)
                                 .arg (j + 1)
                      ...
                      if (!counted.count (
                              QString ("%1~%2").arg (it->first).arg (pos)))
                        {
                          ...
                          counted[QString ("%1~%2").arg (it->first).arg (pos)]
                              = true;
                        }
                      if (!counted.count (
                              QString ("%1~%2").arg (it2->first).arg (j)))
                        {
                          ...
                          counted[QString ("%1~%2").arg (it2->first).arg (j)]
                              = true;
                        }
                    }
                }
  ...
  for (map<QString, unsigned int>::iterator it
       = steric_clash_recheck_summary.begin ();
       it != steric_clash_recheck_summary.end (); it++)
    {
  ...
  //    editor_msg( "blue",
  //                QString( "Maximum possible water to place %1, degree of
  //                hydration %2\n" ) .arg( hydrate_max_waters_no_asa ) .arg(
  //                QString( "").sprintf( "%.2f", ( double )
  //                hydrate_max_waters_no_asa * 18e0 / model_vector[
  //                current_model ].mw ) )
  //                +
  //                QString( "Maximum exposed possible water to place %1,
  //                degree of hydration %2\n" ) .arg( count_waters_added +
  //                count_waters_not_added ) .arg( QString( "" ).sprintf(
  //                "%.2f", ( double ) ( count_waters_added +
  //                count_waters_not_added )  * 18e0 / model_vector[
  //                current_model ].mw ) )
  //                +
  //                QString( "Waters placed %1, degree of hydration %2\n" )
  //                .arg( count_waters_added )
  //                .arg( QString( "" ).sprintf( "%.2f", ( double )
  //                count_waters_added * 18e0 / model_vector[ current_model
  //                ].mw ) )
  //                );

  last_steric_clash_log.clear ();
  last_steric_clash_log << QString ("REMARK MW %1 Daltons\n")
                               .arg (model_vector[current_model].mw);

  last_steric_clash_log << QString ("REMARK Maximum possible water to place   "
                                    "      %1   degree of hydration %2\n")
                               .arg (hydrate_max_waters_no_asa)
                               .arg (QString ("").sprintf (
                                   "%.2f",
                                   (double)hydrate_max_waters_no_asa * 18e0
                                       / model_vector[current_model].mw));

  last_steric_clash_log
      << QString ("REMARK Maximum exposed possible water to place %1   degree "
                  "of hydration %2\n")
             .arg (count_waters_added + count_waters_not_added)
             .arg (QString ("").sprintf (
                 "%.2f", (double)(count_waters_added + count_waters_not_added)
                             * 18e0 / model_vector[current_model].mw));

  last_steric_clash_log << QString ("REMARK Waters placed                     "
                                    "      %1   degree of hydration %2\n")
                               .arg (count_waters_added)
                               .arg (QString ("").sprintf (
                                   "%.2f",
                                   (double)count_waters_added * 18e0
                                       / model_vector[current_model].mw));

  last_steric_clash_log << QString (
  ...
  QString fname = QString ("%1_%2_clash.txt")
  ...
  if (f.open (QIODevice::WriteOnly))
    {
      ...
      ts << qs;
      ts << "\nDetailed clash info:\n\n";
      for (unsigned int i = 0; i < (unsigned int)hydrate_clash_log.size ();
           i++)
        {
  ...
  {
    QString fname = QString ("%1_%2_clash.csv")
                        .arg (QFileInfo (pdb_file).baseName ())
                        .arg (model_vector[current_model].model_id);
...
float
US_Saxs_Util::min_dist_to_struct_and_waters (point p)
{
  unsigned int i = current_model;
  double dist_threshold
      = 1e0 - (our_saxs_options.steric_clash_distance / 100e0);
  ...
  for (unsigned int j = 0; j < model_vector[i].molecule.size (); j++)
    {
      for (unsigned int k = 0; k < model_vector[i].molecule[j].atom.size ();
           k++)
        {
          ...
          this_dist = dist (this_atom->coordinate, p)
                      - (this_atom->radius + water_radius) * dist_threshold;
          if (this_dist < 0e0)
            {
              return this_dist;
  ...
  for (map<QString, vector<point> >::iterator it = waters_to_add.begin ();
       it != waters_to_add.end (); it++)
    {
      for (unsigned int i = 0; i < it->second.size (); i++)
        {
          this_dist
              = dist (it->second[i], p) - 2e0 * water_radius * dist_threshold;
          if (this_dist < 0e0)
            {
              return -1.0;
            }
