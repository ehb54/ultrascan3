...
int us_hydrodyn_asab1_main(vector<PDB_atom *> use_active_atoms,
  ...
  if (check_asa == 1) {
    ...
    for (l = 0; l < nat; l++) {
      ...
      if (recheck) {
        ...
        sapp = 4.0f * M_PI * (rprobe + active_atoms[l]->bead_computed_radius) *
...
static void initarray() {
  ...
  printf("\n\n\t%s",
         "Output files: Enter '0' for default (provaly, provaly1, provaly2, "
         "asaris)");
