...
us_hydrodyn_asab1_main (vector<PDB_atom *> use_active_atoms,
                        asa_options *use_asa_opts, hydro_results *use_results,
                        bool use_recheck, QProgressBar *use_progress,
                        QTextEdit *use_editor, US_Hydrodyn *use_us_hydrodyn)
{
  ...
  for (i = 1; i < passi; i++) /* iteration for the number of steps */
    {
      ...
      us_hydrodyn->lbl_core_progress->setText (
          QString ("Iteration %1 of %2 with %3 %4")
              .arg (i + 1)
              .arg (passi)
              .arg (cont1)
      ...
      for (l = 0; l < nat; l++)
        {
          ...
          if (recheck)
            {
              sa = 4.0f * M_PI * active_atoms[l]->bead_computed_radius
                   * active_atoms[l]->bead_computed_radius;
              sapp = 4.0f * M_PI
                     * (rprobe + active_atoms[l]->bead_computed_radius)
                     * (rprobe + active_atoms[l]->bead_computed_radius);
            }
          else
            {
              sa = 4.0f * M_PI * active_atoms[l]->radius
                   * active_atoms[l]->radius;
              sapp = 4.0f * M_PI * (rprobe + active_atoms[l]->radius)
              ...
              if (temp >= min_asa)
                {
                  ...
                  printf ("\n #%d [bead %4d] - Surf_%6.2f - ASA_%6.2f "
                          "o/oASA_%6.2f - Threshold_%6.2f",
                          countb, l + 1, zz, asa[l], asa[l] * 100 / zz,
                          min_asa);
#endif
                }
            }
