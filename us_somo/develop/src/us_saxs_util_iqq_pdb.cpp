...
bool US_Saxs_Util::calc_saxs_iq_native_fast() {
  ...
  for (unsigned int i = 0; i < model_vector.size(); i++) {
    ...
    noticemsg +=
        QString("Preparing model %1 for SAXS plot.\n").arg(current_model + 1);
    ...
    for (unsigned int j = 0; j < model_vector[current_model].molecule.size();
      ...
      for (unsigned int k = 0;
        ...
        if (!atom_map.count(this_atom_name + "~" + hybrid_name)) {
          ...
          noticemsg += QString(
                           "%1Molecule %2 Atom %3 Residue %4 %5 Hybrid %6 name "
                           "missing from Atom file. Atom skipped.\n")
                           .arg(this_atom->chainID == " "
                                    ? ""
                                    : ("Chain " + this_atom->chainID + " "))
                           .arg(j + 1)
    ...
    if (our_saxs_options.iq_exact_q) {
      ...
      if (!exact_q.size()) {
        // editor_msg( "dark red", QString( us_tr( "Notice: exact q is empty,
    ...
    noticemsg +=
        QString("Average electron density %1\n").arg(total_e / tot_excl_vol, 4);
    if (tot_excl_vol_noh != tot_excl_vol || total_e_noh != total_e) {
      ...
      noticemsg +=
          QString("Electron density of hydration %1\n")
      ...
      if (create_native_saxs) {
        ...
        if (ok_to_write) {
          ...
          if (fpr) {
            ...
            fprintf(fpr, "%s", last_saxs_header.toLatin1().data());
            ...
            noticemsg +=
                "WARNING: Could not create PR curve file: " + fpr_name + "\n";
          }
        }
      } else {
        vector<double> r;
  ...
  for (unsigned int i = 0; i < model_vector.size(); i++) {
    ...
    for (unsigned int j = 0; j < model_vector[current_model].molecule.size();
      ...
      for (unsigned int k = 0;
        ...
        // cout << QString("atom %1 hybrid %2 excl vol %3 by hybrid radius
        // %4\n") .arg(this_atom->name) .arg(this_atom->hybrid_name)
    ...
    if (control_parameters.count("iqcuda")) {
      control_parameters["cuda_tag"] =
          QString("cuda%1").arg(control_parameters["iqcuda"].toUInt() > 0
                                    ? control_parameters["iqcuda"].toUInt()
                                    : 32);
    } else {
...
bool US_Saxs_Util::calc_saxs_iq_native_hybrid() {
  ...
  for (unsigned int i = 0; i < model_vector.size(); i++) {
    ...
    if (our_saxs_options.iq_exact_q) {
      // editor_msg( "blue", QString( us_tr( "Using exact q" ) ) );
      if (!exact_q.size()) {
        // editor_msg( "dark red", QString( "Notice: exact q is empty, computing
        // based upon current q range " ) );
        exact_q.resize(q_points);
        for (unsigned int j = 0; j < q_points; j++) {
          exact_q[j] = our_saxs_options.start_q + j * our_saxs_options.delta_q;
        }
...
double US_Saxs_Util::compute_ff(
  ...
  if (our_saxs_options.use_somo_ff) {
    ...
    if (ff_table.count(ffkey)) {
      // ok
      double ff;
      if (!apply_natural_spline(ff_q[ff_table[ffkey]], ff_ff[ff_table[ffkey]],
                                ff_y2[ff_table[ffkey]], q, ff)) {
        noticemsg +=
            QString("Warning: error applying natural spline to %1 at %2 <%3>")
                .arg(ffkey)
                .arg(q)
                .arg(errormsg);
        errormsg = "";
      ...
      if (!ff_sent_msg1.count(ffkey)) {
        noticemsg +=
            QString("Warning: key %1 not found in ff_table").arg(ffkey);
  ...
  if (hybrid_coords[na].size() <= h) {
    noticemsg += QString(
                     "ERROR: More hydrogens requested (%1) than available (%2) "
...
bool US_Saxs_Util::load_ff_table(QString filename) {
  ...
  if (!f.open(QIODevice::ReadOnly)) {
    errormsg =
        QString("Error: load ff table %1 can not be opened").arg(filename);
    return false;
  }
  ...
  while (!ts.atEnd()) {
    ...
    if (qsl[0] == "residueatom") {
      ...
      if (qsl.count()) {
        errormsg = QString("Error: load_ff line %1, residue/atoms not paired")
                       .arg(line);
        return false;
      }
      continue;
    ...
    if (qsl[0] == "excludedvolume") {
      ...
      if (qsl.count() != 1) {
        errormsg = QString(
                       "Error: load_ff line %1, excluded volume requires "
      ...
      if (ff_ev.size() != pos) {
        errormsg =
            QString("Error: load_ff line %1, excluded volume multiply defined")
                .arg(line);
        return false;
      }
    ...
    if (qsl[0] == "startdata") {
      ...
      if (ff_ev.size() != ff_ff.size() + 1) {
        errormsg = QString(
                       "Error: load_ff line %1, startdata with no excluded "
                       "volume defined")
                       .arg(line);
        return false;
      }
      ...
      while (!ts.atEnd()) {
        ...
        if (qsl[0] == "enddata") {
          if (new_q.size() < 10) {
            errormsg =
                QString("Error: load_ff line %1, empty or insufficient data")
                    .arg(line);
            return false;
          }
        ...
        if (qsl.size() != 2) {
          errormsg = QString(
                         "Error: load_ff line %1, data lines must have exactly "
                         "two fields")
                         .arg(line);
          return false;
        }
      ...
      if (!ok) {
        errormsg =
            QString(
                "Error: load_ff line %1, permature eof: no enddata token found")
                .arg(line);
        return false;
      }
      continue;
...
map<QString, unsigned int> US_Saxs_Util::get_atom_summary_counts(
  ...
  for (unsigned int j = 0; j < model->molecule.size(); j++) {
    for (unsigned int k = 0; k < model->molecule[j].atom.size(); k++) {
      ...
      if (hybrid_name.isEmpty() || !hybrid_name.length()) {
        cout << QString(
                    "%1Molecule %2 Residue %3 %4 Hybrid name missing. Atom "
                    "skipped.\n")
                    .arg(this_atom->chainID == " "
                             ? ""
                             : ("Chain " + this_atom->chainID + " "))
...
QString US_Saxs_Util::list_atom_summary_counts(
    PDB_model *model, map<QString, QString> &residue_atom_hybrid_map,
    saxs_options *use_saxs_options) {
  map<QString, unsigned int> asc =
      get_atom_summary_counts(model, residue_atom_hybrid_map, use_saxs_options);
  QString qs;
  for (map<QString, unsigned int>::iterator it = asc.begin(); it != asc.end();
       it++) {
    qs += QString("%1 %2\n").arg(it->first).arg(it->second);
  }
  return qs;
...
bool US_Saxs_Util::compute_scale_excl_vol() {
        ...
        if (model_vector[current_model].molecule[j].atom.size()) {
          ...
          if (swh_chain) {
            if (our_saxs_options.swh_excl_vol > 0e0) {
              ...
              scale_excl_vol_msgs +=
                  QString(
                      "Adding water to excluded volume using preset excl vol "
                      "%1 number of waters %2 -> excluded volume %3\n")
                      .arg(our_saxs_options.swh_excl_vol)
                      .arg(swh_count)
              ...
              if (hybrid_name.isEmpty() || !hybrid_name.length() ||
                ...
                noticemsg += QString(
                                 "Molecule %1 Residue WAT OW Hybrid/map/atom "
              ...
              scale_excl_vol_msgs +=
                  QString(
                      "Adding water to excluded volume using defined excl vol "
                      "%1 number of waters %2 -> excluded volume %3\n")
                      .arg(swh_excl_vol)
                      .arg(swh_count)
            ...
            if (swh_count) {
              errormsg =
                  "a chain with WAT/OW and other residues is currently "
                  "unsupported\n";
              return false;
            }

            our_saxs_options.iq_target_ev +=
                mw_to_volume(model_vector[current_model].molecule[j].mw,
                             model_vector[i].vbar);
            scale_excl_vol_msgs +=
                QString(
                    "Adding molecule to excluded volume from mw/vbar "
    ...
    for (unsigned int j = 0; j < model_vector[current_model].molecule.size();
      ...
      for (unsigned int k = 0;
        ...
        if (!hybrid_map.count(hybrid_name)) {
          ...
          scale_excl_vol_msgs +=
              QString(
                  "%1Molecule %2 Residue %3 %4 Hybrid %5 name missing from "
        ...
        if (!atom_map.count(this_atom_name + "~" + hybrid_name)) {
          ...
          scale_excl_vol_msgs +=
              QString(
                  "%1Molecule %2 Atom %3 Residue %4 %5 Hybrid %6 name missing "
  ...
  scale_excl_vol_msgs +=
      QString(
          "Scaling excluded volume to match target (%1), default scattering "
          "center sum without WAT %2, total %3, new scaler = %4")
          .arg(our_saxs_options.iq_target_ev)
          .arg(tot_excl_vol_noh)
          .arg(tot_excl_vol)
          .arg(our_saxs_options.scale_excl_vol);
...
bool US_Saxs_Util::pdb_mw(QString file, double &mw) {
  ...
  if (!f.open(QIODevice::ReadOnly)) {
    errormsg =
        QString("US_Saxs_Util::pdb_pw(): could not open file %1").arg(file);
    return false;
  }
  QTextStream ts(&f);
  QStringList qsl;
...
bool US_Saxs_Util::pdb_mw(QStringList &qsl, double &mw) {
  ...
  for (int i = 0; i < (int)qsl.size(); ++i) {
    if (rx_atom.indexIn(qsl[i]) != -1) {
      ...
      if (qs.isEmpty()) {
        errormsg =
            "US_Saxs_Util::pdb_pw(): atom names must be defined in columns 77, "
            "78 of pdb";
        return false;
      }
  ...
  for (map<QString, int>::iterator it = atom_counts.begin();
    ...
    if (!atom_mw.count(it->first)) {
      errormsg =
          QString(
              "US_Saxs_Util::pdb_pw(): atom_mw map does not contain atom %1")
              .arg(it->first);
      return false;
    }
