...
bool
US_Saxs_Util::calc_saxs_iq_native_fast ()
{
  errormsg = "";
  noticemsg = "";
  ...
  for (unsigned int i = 0; i < model_vector.size (); i++)
    {
          ...
          for (unsigned int k = 0;
               k < model_vector[current_model].molecule[j].atom.size (); k++)
            {
              ...
              if (!atom_map.count (this_atom_name + "~" + hybrid_name))
                {
                  cout << "error: (saxs_iqq_pdb 1) atom_map missing for "
                          "hybrid_name "
      ...
      if (our_saxs_options.iq_exact_q)
        {
          ...
          if (!exact_q.size ())
            {
              // editor_msg( "dark red", QString( us_tr( "Notice: exact q is
              // empty, computing based upon current q range " ) ) );
      ...
      noticemsg += QString ("Number of atoms %1.\n"
                            "q range %2 to %3 with a stepsize of %4 giving %5 "
                            "q-points.\n")
                       .arg (atoms.size ())
      ...
      for (unsigned int i = 0; i < atoms.size (); i++)
        {
          ...
          // the our_saxs_options.c + our_saxs_options.a[i] * exp() can be
          // precomputed possibly saving time... but this isn't our most
      ...
      if (control_parameters.count ("fdcomputepr"))
        {
                  ...
                  if (fpr)
                    {
                      ...
                      noticemsg
                          += "P(r) curve file: " + fpr_name + " created.\n";
                      vector<double> r;
                      vector<double> pr;
                      vector<double> pr_n;
                      ...
                      QString last_saxs_header = QString ("").sprintf (
                    ...
                    {
                      noticemsg += "WARNING: Could not create PR curve file: "
                                   + fpr_name + "\n";
                    }
                }
            }
          ...
          plotted_pr_mw.push_back (
              (float)get_mw (control_parameters["inputfile"], false));
...
#if defined(IQQ_TIMER)
      {
        ...
        QString file = QString ("%1_%2_q%3-%4.timing")
...
//  ------------------------------------------------------------------------------------------------------
bool
US_Saxs_Util::calc_saxs_iq_native_debye ()
{
  errormsg = "";
  noticemsg = "";
  ...
  for (unsigned int i = 0; i < model_vector.size (); i++)
    {
           ...
           j < model_vector[current_model].molecule.size (); j++)
        {
          for (unsigned int k = 0;
               k < model_vector[current_model].molecule[j].atom.size (); k++)
            {
              ...
              // cout << QString("atom %1 hybrid %2 excl vol %3 by hybrid
              // radius %4\n") .arg(this_atom->name)
              // .arg(this_atom->hybrid_name)
              // .arg(atom_map[this_atom->name + "~" +
              // hybrid_name].saxs_excl_vol) .arg(M_PI *
              // hybrid_map[hybrid_name].radius *
              // hybrid_map[hybrid_name].radius *
              // hybrid_map[hybrid_name].radius)
              // ;
      ...
      if (control_parameters.count ("iqcuda"))
        {
          if (!iqq_cuda (q, atoms, fp, I,
                         control_parameters["iqcuda"].toUInt () > 0
                             ? control_parameters["iqcuda"].toUInt ()
                             : 32))
            {
              return false;
            }
      ...
      if (control_parameters.count ("iqcuda"))
        {
          control_parameters["cuda_tag"] = QString ("cuda%1").arg (
              control_parameters["iqcuda"].toUInt () > 0
                  ? control_parameters["iqcuda"].toUInt ()
                  : 32);
        }
...
//  ------------------------------------------------------------------------------------------------------
bool
US_Saxs_Util::calc_saxs_iq_native_hybrid ()
{
  errormsg = "";
  noticemsg = "";
      ...
      if (our_saxs_options.iq_exact_q)
        {
          ...
          if (!exact_q.size ())
            {
              // editor_msg( "dark red", QString( "Notice: exact q is empty,
              // computing based upon current q range " ) );
              exact_q.resize (q_points);
              for (unsigned int j = 0; j < q_points; j++)
                {
                  exact_q[j] = our_saxs_options.start_q
      ...
      if (our_saxs_options.saxs_iq_hybrid_adaptive)
        {
          ...
          if (!create_adaptive_grid (q, I, our_saxs_options.hybrid2_q_points,
                                     r))
            {
        ...
        if (our_saxs_options.saxs_iq_native_hybrid)
        ...
        else
          {
            if (our_saxs_options.saxs_iq_native_hybrid2)
              ...
              {
                if (!cubic_spline_interpolate_iq_curve (q, use_q, fast_I, I,
                                                        I))
                  {
                    cout << errormsg << endl;
                    return false;
...
void
US_Saxs_Util::setup_saxs_options ()
{
  ...
  our_saxs_options.frac_of_exch_pep
  ...
  if (control_parameters.count ("wateredensity"))
    {
      our_saxs_options.water_e_density
          = control_parameters["wateredensity"].toFloat ();
    }

  if (control_parameters.count ("swhexclvol"))
    {
      our_saxs_options.swh_excl_vol
          = control_parameters["swhexclvol"].toFloat ();
    }

  if (control_parameters.count ("scaleexclvol"))
    {
      our_saxs_options.scale_excl_vol
          = control_parameters["scaleexclvol"].toFloat ();
    }
  ...
  if (control_parameters.count ("fdbinsize"))
    {
      our_saxs_options.fast_bin_size
          = control_parameters["fdbinsize"].toFloat ();
    }
  if (control_parameters.count ("fdmodulation"))
    {
      our_saxs_options.fast_modulation
          = control_parameters["fdmodulation"].toFloat ();
    }
  if (control_parameters.count ("hypoints"))
    {
      our_saxs_options.hybrid2_q_points
          = control_parameters["hypoints"].toUInt ();
    }
  if (control_parameters.count ("crysolharm"))
    {
      our_saxs_options.sh_max_harmonics
          = control_parameters["crysolharm"].toUInt ();
    }
  if (control_parameters.count ("crysolgrid"))
    {
      our_saxs_options.sh_fibonacci_grid_order
          = control_parameters["crysolgrid"].toUInt ();
    }
  if (control_parameters.count ("crysolchs"))
    {
      our_saxs_options.crysol_hydration_shell_contrast
          = control_parameters["crysolchs"].toFloat ();
    }
  ...
  if (control_parameters.count ("iqtargetev"))
    {
      our_saxs_options.iq_target_ev
          = control_parameters["iqtargetev"].toDouble ();
    }

  our_saxs_options.hybrid_radius_excl_vol
      = control_parameters.count ("hybridradiusexclvol") ? true : false;
  our_saxs_options.use_iq_target_ev
      = control_parameters.count ("useiqtargetev") ? true : false;
  our_saxs_options.set_iq_target_ev_from_vbar
      = control_parameters.count ("setiqtargetevfromvbar") ? true : false;
  ...
  if (control_parameters.count ("iqmethod"))
    {
      ...
      if (qs == "hy" || qs == "hya" || qs == "h2" || qs == "h2a" || qs == "h3"
          || qs == "h3a")
        {
...
double
US_Saxs_Util::compute_ff (saxs &sa, // gaussian decomposition for the main atom
                          ...
                          double q, double q_o_4pi2)
{
      ...
      if (ff_table.count (ffkey))
        {
          ...
          if (!apply_natural_spline (ff_q[ff_table[ffkey]],
                                     ...
                                     ff_y2[ff_table[ffkey]], q, ff))
            {
              noticemsg += QString ("Warning: error applying natural spline "
                                    "to %1 at %2 <%3>")
                               .arg (ffkey)
                               .arg (q)
                               .arg (errormsg);
      ...
      else
        {
          if (!ff_sent_msg1.count (ffkey))
            {
              noticemsg += QString ("Warning: key %1 not found in ff_table")
                               .arg (ffkey);
  ...
  if (!hybrid_coords.count (na))
    {
      noticemsg += QString ("ERROR: No hybrid info for atom %1, computing for "
                            "ZERO hydrogens")
                       .arg (na);
...
bool
US_Saxs_Util::load_ff_table (QString filename)
{
  ...
  if (!f.exists ())
    {
      errormsg
          = QString ("Error: load ff table %1 does not exist").arg (filename);
      return false;
    }

  if (!f.open (QIODevice::ReadOnly))
    {
      errormsg = QString ("Error: load ff table %1 can not be opened")
                     .arg (filename);
      return false;
    }
  ...
  while (!ts.atEnd ())
    {
      ...
      if (qsl[0] == "excludedvolume")
        {
          ...
          if (qsl.count () != 1)
            {
              errormsg = QString ("Error: load_ff line %1, excluded volume "
      ...
      if (qsl[0] == "startdata")
        {
          ...
          if (ff_ev.size () != ff_ff.size () + 1)
            {
              errormsg = QString ("Error: load_ff line %1, startdata with no "
                                  "excluded volume defined")
                             .arg (line);
              return false;
            }
          ...
          while (!ts.atEnd ())
            {
              ...
              if (qsl[0] == "enddata")
                {
                  if (new_q.size () < 10)
                    {
                      errormsg = QString ("Error: load_ff line %1, empty or "
                                          "insufficient data")
                                     .arg (line);
                      return false;
                    }
              ...
              if (qsl.size () != 2)
                {
                  errormsg = QString ("Error: load_ff line %1, data lines "
          ...
          if (!ok)
            {
              errormsg = QString ("Error: load_ff line %1, permature eof: no "
                                  "enddata token found")
                             .arg (line);
              return false;
            }
      ...
      errormsg = QString ("Error: load_ff line %1, unknown token %2")
                     .arg (line)
                     .arg (qsl[0]);
      return false;
    }
...
double
US_Saxs_Util::get_ff_ev (QString res, QString atm)
{
...
US_Saxs_Util::get_atom_summary_counts (
      ...
      for (unsigned int k = 0; k < model->molecule[j].atom.size (); k++)
        {
          ...
          if (hybrid_name.isEmpty () || !hybrid_name.length ())
            {
              cout << QString ("%1Molecule %2 Residue %3 %4 Hybrid name "
                               "missing. Atom skipped.\n")
                          .arg (this_atom->chainID == " "
                                    ? ""
                                    : ("Chain " + this_atom->chainID + " "))
...
QString
US_Saxs_Util::list_atom_summary_counts (
    PDB_model *model, map<QString, QString> &residue_atom_hybrid_map,
    saxs_options *use_saxs_options)
{
  map<QString, unsigned int> asc = get_atom_summary_counts (
      model, residue_atom_hybrid_map, use_saxs_options);
  QString qs;
...
bool
US_Saxs_Util::compute_scale_excl_vol ()
{
  errormsg = "";
  noticemsg = "";
              ...
              if (model_vector[current_model].molecule[j].atom.size ())
                {
                  ...
                  if (swh_chain)
                      ...
                      if (our_saxs_options.swh_excl_vol > 0e0)
                        {
                          our_saxs_options.iq_target_ev
                              += (double)our_saxs_options.swh_excl_vol
                                 * swh_count;
                          scale_excl_vol_msgs
                              += QString ("Adding water to excluded volume "
                      ...
                      else
                        {
                          ...
                          if (hybrid_name.isEmpty () || !hybrid_name.length ()
                            ...
                            {
                              noticemsg += QString ("Molecule %1 Residue WAT "
                          ...
                          scale_excl_vol_msgs
                              += QString (
                                     "Adding water to excluded volume using "
                                     "defined excl vol %1 number of waters %2 "
                                     "-> excluded volume %3\n")
                                     .arg (swh_excl_vol)
                                     .arg (swh_count)
                  ...
                  else
                    {
                      if (swh_count)
                        {
                          errormsg = "a chain with WAT/OW and other residues "
                                     "is currently unsupported\n";
                          return false;
                        }

                      our_saxs_options.iq_target_ev += mw_to_volume (
                          model_vector[current_model].molecule[j].mw,
                          model_vector[i].vbar);
                      scale_excl_vol_msgs
                          += QString ("Adding molecule to excluded volume "
                                      "from mw/vbar computation: mw %1 vbar "
                                      "%2 -> excluded volume %3\n")
           ...
           j < model_vector[current_model].molecule.size (); j++)
        {
          for (unsigned int k = 0;
               k < model_vector[current_model].molecule[j].atom.size (); k++)
            {
              ...
              if (!atom_map.count (this_atom_name + "~" + hybrid_name))
                {
                  cout << "error: (saxs_iqq_pdb 2) atom_map missing for "
                          "hybrid_name "
                       << hybrid_name << " atom name " << this_atom_name
                       << endl;
                  scale_excl_vol_msgs
  ...
  scale_excl_vol_msgs
      += QString (
             "Scaling excluded volume to match target (%1), default "
             "scattering center sum without WAT %2, total %3, new scaler = %4")
             .arg (our_saxs_options.iq_target_ev)
             .arg (tot_excl_vol_noh)
             .arg (tot_excl_vol)
             .arg (our_saxs_options.scale_excl_vol);
...
bool
US_Saxs_Util::pdb_mw (QString file, double &mw)
{
  errormsg = "";
  QFile f (file);
  if (!f.open (QIODevice::ReadOnly))
    {
      errormsg = QString ("US_Saxs_Util::pdb_pw(): could not open file %1")
                     .arg (file);
      return false;
    }
  QTextStream ts (&f);
...
bool
US_Saxs_Util::pdb_mw (QStringList &qsl, double &mw)
{
  if (!atom_mw.size ())
    {
      ...
      if (rx_atom.indexIn (qsl[i]) != -1)
        {
          ...
          if (qs.isEmpty ())
            {
              errormsg = "US_Saxs_Util::pdb_pw(): atom names must be defined "
                         "in columns 77, 78 of pdb";
              return false;
            }
  ...
  for (map<QString, int>::iterator it = atom_counts.begin ();
       it != atom_counts.end (); it++)
    {
      if (!atom_mw.count (it->first))
        {
          errormsg = QString ("US_Saxs_Util::pdb_pw(): atom_mw map does not "
                              "contain atom %1")
                         .arg (it->first);
          return false;
        }
