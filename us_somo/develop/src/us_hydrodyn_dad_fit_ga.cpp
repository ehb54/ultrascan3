...
unsigned int
US_Hydrodyn_Dad_Fit::pop_selection (unsigned int size)
{
...
bool
US_Hydrodyn_Dad_Fit::ga_fitness (ga_individual &individual)
{
...
bool
US_Hydrodyn_Dad_Fit::ga_run (double &nrmsd)
{
      ...
      for (unsigned int j = 0; j < DFIT::init_params.size (); j++)
        {
          individual.v[j]
              = DFIT::param_min[j]
                + (DFIT::param_max[j] - DFIT::param_min[j]) * drand48 ();
        }
           ...
           i++)
        {
          ...
          if (control_parameters.count ("mutate")
              && drand48 () < control_parameters["mutate"].toDouble ())
            {
              ...
              unsigned int pos
                  = (unsigned int)(drand48 () * DFIT::init_params.size ());
              individual.v[pos]
                  = DFIT::param_min[pos]
                    + (DFIT::param_max[pos] - DFIT::param_min[pos])
                          * drand48 ();
              ga_fitness (individual);
              pop.push_back (individual);
              mutate_count++;
          ...
          if (has_been_duplicated.count (pos))
            {
              for (unsigned int j = 0; j < DFIT::init_params.size (); j++)
                {
                  individual.v[j] = DFIT::param_min[j]
                                    + (DFIT::param_max[j] - DFIT::param_min[j])
                                          * drand48 ();
                }
              ga_fitness (individual);
              pop.push_back (individual);
