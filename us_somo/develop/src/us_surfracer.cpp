...
static float *curvat; /*atom coordinates, radii and molecular (smooth) areas
...
toruscenter (int i, int j, float *tij) /*torus center */
{
  ...
  tij[1] = 0.5
           * (a[3 * i + 1] + a[3 * j + 1] + (a[3 * j + 1] - a[3 * i + 1]) * c);
  tij[2] = 0.5
           * (a[3 * i + 2] + a[3 * j + 2] + (a[3 * j + 2] - a[3 * i + 2]) * c);
}
...
static void
cunnvec (int i, int j, float *pijk,
...
static float
thetasi (int i, int j) /*saddle width angle- here torusrad taken because of the
                          cancellation */
{
...
static void
saddle (int nedge, int natom,
        int atom2) /*saddle part of the molecular surface; nedge is the edge
                      number */
{
  ...
  if (prober <= rij)
  ...
  else
    {
      ...
      a1 = phi
           * (prober * prober * (2. * sin (theta1) - si)
              - prober * rij * (2. * theta1 - theti));
      molarea[natom] += a1;

      if (calcmode == 3)
        curvat[natom] += phi
                         * (prober * (-4. * sin (theta1) + 2. * si)
                            + rij * (2. * theta1 - theti));

      a2 = phi
           * (prober * prober * (2. * sin (theta1) - sj)
              - prober * rij * (2. * theta1 - thetj));
      molarea[atom2] += a2;

      if (calcmode == 3)
        curvat[atom2] += phi
                         * (prober * (-4. * sin (theta1) + 2. * sj)
                            + rij * (2. * theta1 - thetj));
    }
}
...
checkcycle (int j, int nver, float cycle[][3], float *point)
{
  ...
  for (i = 0; i <= nver;
    ...
    {       /*recording it in proj[50][3] */
      ...
      vtimk (v, 1. / sqrt (dis2 (point, cycle[i]))); /*unit vector from point
                                                        to vertex cycle[i] */
...
static int
searchcycle (int ncycles, int cycle1, int respect[][MAXCYCLES],
  ...
  for (i = 0; i <= ncycles - 1; i++)
    {
      if (i != cycle1) /*do not analyze the same cycle */
        {
          if (respect[cycle1][i] && respect[i][cycle1]) /*they have each other
                                                           in their interior */
            {
              for (j = 0; j <= ncycles - 1; j++)
                {
                  if (j != i && j != cycle1) /*checking the other cycles */
                    {
                      if (respect[cycle1][j]
                          && respect[i][j]) /*cycle j is in the interior of
                                               cycles cycle1 and i */
                        {
                          if (respect[j][cycle1]
                              && respect[j][i]) /*cycles cycle1 and i are in
                                                   the interior of j */
                            {
                              continue;
                            }
              ...
              if (j == ncycles) /*all cycles that are in the interior of cycle1
                                   and i have cycle 1 in their interior */
                {
                  hits[nhit] = i;
                  nhit++;
                }
...
potoedge (int natom, int v0, int *v1, int *verte, int nverte)
{
    ...
    if (i != v0 && verte[i] != -1) /*looking for vertices that coincide with v0
      ...
      if (fabs (ver[verte[v0] * 3] - ver[verte[i] * 3]) < 1.e-6
          ...
          && fabs (ver[verte[v0] * 3 + 2] - ver[verte[i] * 3 + 2])
                 < 1.e-6) /*version 3.0 change. Precise equality replaced with
                             the interval */
        {
  ...
  vtimk (e1, 1. / ar[natom]); /*e1 is a unit vector from the center of atom
                                 atomn to the vertex verte[v0] */
  ...
  for (i = 1; i <= ndegen - 1;
       i++) /*recording the atoms forming the degenerate vertexes */
    {
      ...
      if (verat[verte[degen[i]] * 3] != natom)
        {
          for (j = 0; j <= ndegat - 1;
               j++) /*making sure the atoms do not repeat */
  ...
  /*for the future: here is a good place to erase irrelevant degenerate
   * vertices */
    ...
    if (i != v0 && verte[i] != -1)
      {
        ...
        if (k
            == ndegen) /*the vertex is not degenerate to the current vertex */
      ...
      for (i = 0; i <= vc - 1; i++)
        {
          ...
          vtimk (check, 1. / d); /*check is a unit vector from torus center to
                                    a vertex */
          ...
          if (ang != 0. && ang != M_PI) /*0 and pi are not considered here */
                                        /*not zero or pi */
            {
              ...
              if (sp < 0) /*the angle is more than 180 counting
                             counterclockwise from e3 */
                ang = 2. * M_PI - ang;
            }
  ...
  return (
      corratom); /*returning the atom forming the correct edge with natom */
}

/*convexarea() returns the convex area of surface surrounded by cycles pointed
 * by hits*/
static float
convexarea (int natom, int cycles[][MAXCYCLES], int common[][MAXCYCLES],
            int *hits, int nhits, int *nvincyc)
{
  int i, j;
  float area = 0.;
      ...
      if (cycles[hits[i]][0] == -2) /*circle */
      ...
      else /*cycles consisting of edges */
        {
          ...
          area += phis (natom, common[hits[i]][nvincyc[hits[i]] - 1],
                        ...
                        cycles[hits[i]][0])
                  * sin (
                      thetasi (natom, common[hits[i]][nvincyc[hits[i]] - 1]));
...
static void
managedge (int natom, int cycles[][MAXCYCLES], int common[][MAXCYCLES],
           int *hits, int nhits, int *nvincyc)
{
  int i, j, l, p;
  dbg ("me 1");
          ...
          if (j == ned) /*this is a brand new one */
            {
              ...
              edge[l * 2]
                  = -2; /*records cycle papameters into the empty spot */
              edge[l * 2 + 1] = cycles[hits[i]][1];
              edgeatom[l]
                  = common[hits[i]][0]; /*not analyzed atom goes first */

              if (calcmode > 1)
                saddle (l, natom,
                        edgeatom[l]); /*calculating the saddle area */
               ...
               j++) /*checking all edges but the last one */
            {
              ...
              for (l = 0; l <= ned - 1; l++)
                if ((cycles[hits[i]][j] == edge[l * 2]
                    ...
                    && edgeatom[l] == natom) /*found it- the  vertexes (in the
                                                right order!) are the same */
                  {
                    dbg ("me 4.2");
                    if (calcmode > 1)
              ...
              if (l == ned) /*did not find it */
                {
                  ...
                  edge[p * 2]
                      = cycles[hits[i]]
                              [j + 1]; /*recording in the reverse order */
                  ...
                  if (calcmode > 1)
                    {
                      ...
                      concave (
                          edge[p * 2],
                          natom); /*calculating the concave molecular surface
                                     and writing it into molarea */
                    }
                }
              dbg ("me 4.6");
            }
          ...
          if (l == ned) /*did not find it */
            {
              ...
              edge[p * 2]
                  = cycles[hits[i]][0]; /*recording in the reverse order */
...
static int
buildcycles (int natom, int cycles[][MAXCYCLES], int common[][MAXCYCLES],
             int *nvincyc)
{
      ...
      if (cirat[i * 2] == natom
          || cirat[i * 2 + 1] == natom) /*i-th circle is on natom */
        {
          ...
          if (cirat[i * 2] != natom)
            ...
            common[icycle][0] = cirat[i * 2 + 1]; /*common has the other atom
                                                     in the 0-th element */

          icycle++; /*degenerate cycles are not chemically possible and
                       extremely unlikely */
        }
    }
  ...
  while (1) /*break inside */
    {
      ...
      /*so we are still here- that means verte[i] is the vertex that will start
       * a brand new cycle for us!!! */
      if (verte[i] == -1)
        {
          printf ("ERROR dc4!!\n");
        }
            ...
            if (cycles[icycle][p] != verte[ii] && verte[ii] != -1) // emre?
              if (fabs (ver[cycles[icycle][p] * 3] - ver[verte[ii] * 3])
                         ...
                         < 1.e-6)
                {
                  ...
                  verte[ii] = -1; /*this will eliminate possible degenerate
                                     ones as well */
                }
...
static int
find1cyc (int nedge, int cycles[][MAXCYCLES], int icycle,
      ...
      if (cycles[i][0] == -2)
      ...
      else
        {
          for (j = 0; j <= nvincyc[i] - 1; j++)
            if (cycles[i][j]
                == edge[nedge * 2]) /*cycle[i] and the edge share the same
                                       vertex!!- found it */
              break;
        }
...
static void
loadresp (int natom, int icycle, int cycles[][MAXCYCLES],
      ...
      if (cycles[i][0] == -2) /*a cycle is a circle */
        {
          for (j = 0; j <= icycle - 1; j++)
            if (j != i)
              respect[i][j]
                  = 1; /*any cycle is internal with respect to a circle */
        }
      ...
      else /*it's not a circle */
        {
          for (j = 0; j <= nvincyc[i] - 1; j++)
            {
              verte[j][0]
                  = ver[cycles[i][j] * 3]; /*loading cycle[i] into verte */
          ...
          for (k = 0; k <= icycle - 1; k++)
            {
              if (k != i) /*checking the cycles k other than i itself */
                {
                  if (j == 2)
                    respect[i][k] = 1; /*if cycle i consists of 2 vertices any
                                          other one is internal to it */
                  else
                    {
                      if (cycles[k][0] == -2) /*cycle k is a circle */
                        respect[i][k] = checkcycle (
                            natom, j - 1, verte,
                            &cir[cycles[k][1]
                                 * 3]); /*check a point on the circle */
                      else
                        respect[i][k] = checkcycle (
                            natom, j - 1, verte,
                            &ver[cycles[k][0]
                                 * 3]); /*check the first vertex */
                    }
                }
            }
        }
...
int
surfracer_main (float prober, vector<PDB_atom *> active_atoms, bool recheck,
                QProgressBar *use_progress, QTextEdit *use_editor)
{
  ...
  for (unsigned int i = 0; i < active_atoms.size (); i++)
    {
      printf ("active atom %d %s %s rad %f resseq %d\n", i,
              ...
              active_atoms[i]->radius, active_atoms[i]->resSeq);
      fflush (stdout);
    }
#endif
                          ...
                          if (active_atoms[i]->group != active_atoms[j]->group)
                            {
                              ...
                              editor->append (
                                  QString ("Joining group %1 into group %2\n")
                                      .arg (removed_group)
                                      .arg (joined_group));
                              qApp->processEvents ();
                              for (unsigned int k = 0;
                                   k < active_atoms.size (); k++)
                                {
  ...
  for (unsigned int atom_group = 1; atom_group < atom_groups.size ();
       atom_group++)
    {
      if (atom_groups[atom_group].size ()
        ...
        {
          editor->append (
              QString ("Computing ASA for group %1\n").arg (atom_group));
          qApp->processEvents ();
          ...
          a = (float *)calloc (atomnumber * 3,
                               sizeof (float)); /*   coordinates   */
          if (!a)
            {
              free_alloced ();
          ...
          aarea = (float *)calloc (atomnumber,
                                   sizeof (float)); /* accessible area  */
          if (!aarea)
            {
              free_alloced ();
              //   fprintf(stderr, "memory allocation error\n");
          ...
          molarea = (float *)calloc (
              atomnumber, sizeof (float)); /*  molecular (smooth) area  */
          if (!molarea)
            {
              free_alloced ();
              //   fprintf(stderr, "memory allocation error\n");
          ...
          curvat
              = (float *)calloc (atomnumber, sizeof (float)); /*  curvature  */
          if (!curvat)
            {
              free_alloced ();
              //   fprintf(stderr, "memory allocation error\n");
          ...
          visits = (short *)calloc (
              atomnumber, sizeof (short)); /* count # of visits E54 */
          if (!visits)
            {
              free_alloced ();
              //   fprintf(stderr, "memory allocation error\n");
          ...
          /*This part of the program loads up atomic coordinates from the file
           * and assigns radii */
                ...
                for (i = 0; i < atomnumber; i++)
                  {
                    ...
                    a[3 * i + 1]
                        = atom_groups[atom_group][i]->coordinate.axis[1];
                    a[3 * i + 2]
                        = atom_groups[atom_group][i]->coordinate.axis[2];
                ...
                for (i = 0; i < atomnumber; i++)
                  {
                    ...
                    // printf("radius %d %f\n", i, ar[i]);
                    a[3 * i]
                        = atom_groups[atom_group][i]->bead_coordinate.axis[0];
                    a[3 * i + 1]
                        = atom_groups[atom_group][i]->bead_coordinate.axis[1];
                    a[3 * i + 2]
                        = atom_groups[atom_group][i]->bead_coordinate.axis[2];
                    fprintf (fout, "\tar[apos] = %f;\n", ar[i]);
                    fprintf (fout, "\ta[3 * apos] = %f;\n", a[3 * i]);
                    fprintf (fout, "\ta[3 * apos + 1] = %f;\n", a[3 * i + 1]);
                    fprintf (fout, "\ta[3 * apos + 2] = %f;\n", a[3 * i + 2]);
          ...
          atomcon = (int *)calloc (
              atomnumber, sizeof (int)); /*allocating memory for array storing
                                            contacts for an atom */
          if (!atomcon)
            {
              free_alloced ();
              //   fprintf(stderr, "memory allocation error\n");
          ...
          for (ai = 0; ai <= atomnumber - 1;
               ai++) /*taking an atom (going through all) */
            {
              ...
              for (i = 0, k = 0; i <= atomnumber - 1;
                ...
                if (i != ai) /*all but atom ai */
                  if (a[i * 3] < xmax && a[i * 3] > xmin && a[i * 3 + 1] < ymax
                      ...
                      && a[i * 3 + 2]
                             > zmin) /*eliminating the atoms whose centers lie
                                        outside of the box */
                    {
                      rsum = ar[ai] + ar[i];

                      if ((dist2ij[k] = dist2 (ai, i))
                          < rsum * rsum) /* here filling up dist2ij array here
                                            too (limit = 500 contacting
                                            atoms!!) */
                        {
                          dist2ij[k] = sqrt (
                              dist2ij[k]); /*storing actual distances */

                          vdiff (&a[3 * i], &a[3 * ai], uconij[k]);
                          vtimk (uconij[k],
                                 1. / dist2ij[k]); /*calculation of torus axis
                                                      unit vector uij */
              ...
              /*at this point the atoms overlapping with atom ai are stored in
               * array atomcon */

              for (aj = 0; aj <= k - 1;
                   aj++) /*aj is an index in atomcon (atoms contacting ai) */
                {
                  if (atomcon[aj] > ai) /*choosing so that the atom number is
                                           larger than ai- ordered contact */
                    {

                      for (ak = aj + 1, nvk = 0; ak <= k - 1;
                           ak++) /*because the atoms in atomcon are ordered
                                    already- start with the next index here to
                                    get an ordered triplet */
                        { /*if no intersections with the circle formed by
                             atomcon[aj] and ai,atomcon[aj] pair, nvk stays 0
                           */
                          rsum = ar[atomcon[ak]] + ar[atomcon[aj]];

                          if (dist2 (atomcon[ak], atomcon[aj])
                              < rsum * rsum) /*atom atomcon[ak] overlaps with
                                                atom atomcon[aj] */
                            {                /*torus axis unit vectors */
                              ...
                              if (cba < 1
                                  && cba > -1) /*the three are not collinear */
                                {
                                  vectorprod (uconij[aj], uconij[ak], uijk);
                                  sba = sqrt (
                                      1. - cba * cba); /*sin of base angle */
                                  vtimk (uijk, 1. / sba); /*this is base plane
                                                             normal vector */
                                  vectorprod (
                                      uijk, uconij[aj],
                                  ...
                                  vtimk (utb,
                                         scalarprod (uconij[ak], bijk) / sba);
                                  vsum (tconij[aj], utb, bijk); /*base point */

                                  vdiff (bijk, &a[ai * 3],
                                         utb); /*utb is screwed up- don't use
                                                  it except below */
                                  ph2 = ar[ai] * ar[ai]
                                        - scalarprod (
                                            utb,
                                            utb); /*square of probe height */

                                  if (ph2 > 0) /*there are two probe positions
                                                  (two vertexes)! */
                                    { /*recording the two vertexes into ve1 and
                                         ve2 */
                                      nvk = 1;
                                      ...
                                      /*check whether vertexes 1 and 2 are
                                       * buried */
                                      bur1 = 0;
                                      bur2 = 0;
                                      if (k > 2)
                                      ...
                                      if (!bur1) /*first vertex is not buried
                                                  */
                                        {
                                          veat[3 * nv] = ai; /*storing the
                                                                vertex atoms */
                                          veat[3 * nv + 1] = atomcon[aj];
                                          veat[3 * nv + 2] = atomcon[ak];

                                          ve[3 * nv]
                                              = ve1[0]; /*storing the vertex
                                                           coordinates */
                                          ve[3 * nv + 1] = ve1[1];
                                          ve[3 * nv + 2] = ve1[2];
                                  ...
                                  /* loaded vertexes else  there are no such
                                   * probe positions to form them */
                                }
                      ...
                      if (nvk
                          == 0) /*checking whether atom ak forms a vertex
                                   (possibly buried) with ai and atomcon[aj] */
                        if (k > 1)
                          for (ak = 0; ak <= aj - 1; ak++)
                            {
                              rsum = ar[atomcon[ak]] + ar[atomcon[aj]];
                              if (dist2 (atomcon[ak], atomcon[aj])
                                ...
                                {                /*torus axis unit vectors */
                                  ...
                                  cba = scalarprod (
                                      uconij[aj],
                                      uconij[ak]); /*cosine of base triangle
                                                      angle for these three
                                                      atoms */
                                  if (cba < 1 && cba > -1) /*the three are not
                                                              collinear */
                                    {
                                      vectorprod (uconij[aj], uconij[ak],
                                                  uijk);
                                      sba = sqrt (
                                          1.
                                          - cba * cba); /*sin of base angle */
                                      vtimk (uijk,
                                             1. / sba); /*this is base plane
                                                           normal vector */
                                      vectorprod (uijk, uconij[aj],
                                                  utb); /*torus basepoint unit
                                                           vector */

                                      /*calculation of base point bijk */

                                      vdiff (tconij[ak], tconij[aj], bijk);
                                      vtimk (utb, scalarprod (uconij[ak], bijk)
                                                      / sba);
                                      vsum (tconij[aj], utb,
                                            bijk); /*base point */

                                      vdiff (bijk, &a[ai * 3],
                                             utb); /*utb is screwed up- don't
                                                      use it except below */
                                      ph2 = ar[ai] * ar[ai]
                                            - scalarprod (
                                                utb, utb); /*square of probe
                                                              height */
                                      ...
                                      if (ph2 > 0)
                                        {
                                          nvk = 1;
                                          break;
                                        } /*vertex can be placed but it is
                                             buried so it was not recorded */
                                    }
                                }
                            }

                      if (nvk == 0) /*no atoms messing in */
                        { /*one bad possiblility left - the whole contact
                             circle can be buried */
                          ...
                          vtimk (diff,
                                 torusrad (
                                     ai,
                                     atomcon[aj])); /*diff1 is a radial vector
                                                       of torus radius length
                                                       from the torus center */
                          vsum (tconij[aj], diff,
                                re); /*finished re calculation */

                          /*it is sufficient to check burial of only this point
                           * re */
                          ...
                          if (!bur1) /*the circle is not buried */
                            {
                              ...
                              ci[3 * ncircle] = re[0]; /*storing circle point
                                                          coordinates */
                              ci[3 * ncircle + 1] = re[1];
                              ci[3 * ncircle + 2] = re[2];
          ...
          /*finished going through all atoms- all surface and cavity vertexes
           * and contact circles are loaded */
          ...
          if (nv > 0) /*if there are vertices */
            {
              verat = (int *)calloc (
                  nv * 3, sizeof (int)); /*vertex forming atom triplets */
              if (!verat)
                {
          ...
          if (ciat)
            ...
            /*one caveat to be taken care of later is a very unlikely but
             * possible exact coincidence of two or more vertices */
          ...
          /*finding the atom with the highest north pole-it's accessible and
          ...
          dbg ("2");
          /*loading all internal-external relationships between the cycles of
           * atom0 */
          loadresp (atom0, ncycle, cycles, respect, nvincyc);

          /*the highest point belongs to the accessible surface area */
          /*we need to create another cycle - circle formed by this highest
           * point */
          dbg ("3");
          ...
          for (i = 0; i <= ncycle - 1; i++)
            {
              respect[ncycle][i]
                  = 1; /*all cycles are internal with respect to it */
            }
          ...
          highest[2]
              = a[atom0 * 3 + 2] + ar[atom0]; /*the highest north pole */
                  ...
                  if (cycles[i][0] == -2)
                  ...
                  else
                    {
                      for (j = 0; j <= MAXCYCLES && cycles[i][j] != -1; j++)
                        {
                          verte[j][0] = ver[cycles[i][j]
                                            * 3]; /*j vertices of cycle i */
                          verte[j][1] = ver[cycles[i][j] * 3 + 1];
                          verte[j][2] = ver[cycles[i][j] * 3 + 2];
                        }
                      respect[i][ncycle] = checkcycle (
                          atom0, j - 1, verte,
          ...
          if (calcmode > 1)
            molarea[atom0]
                += asar * (ar[atom0] - prober) * (ar[atom0] - prober)
          ...
          while (1)
            {
              ...
              if (i == ned) /*outer surface calculation completed */
              ...
              else /*perform calculation for the next atom */
                {
                  ...
                  dbg ("5.6");
                  cyc1 = find1cyc (i, cycles, ncycle,
                                   nvincyc); /*find first cycle */
                  dbg ("5.7");
      ...
      if (strcmp ("ATOM", str) == 0)
        {
          ...
          if (strcmp (acidname, "DOD") != 0 && strcmp (acidname, "HOH") != 0
              ...
              && (str[16] == 'A'
                  || str[16]
                         == ' ')) /*disregarding bound waters and hydrogens */
            {
              ...
              // gcvt(molarea[k],5,asa);           /*converting molecular areas
              // into a string*/
              for (j = 0; asa[j] != '\0'; j++)
                ;
              asa[j] = ' ';
              ...
              // gcvt(curvat[k],5,asa);           /*converting molecular areas
              // into a string*/
              for (j = 0; asa[j] != '\0'; j++)
                ;
              asa[j] = ' ';
              ...
              fputs (str,
                     asafile); /*writing the radii and areas to the file */
  ...
  // (in Angstrom^2):"); printf("\nTotal area = %.2f, Polar area= %.2f ,
  // Non-polar area= %.2f", molpolarea + molnparea, molpolarea, molnparea);
  ...
  while (1)
    {
      ...
      if (i == nv + ncircle) /*did not find untreated elements- there are NO
                                cavities */
        {
          // if (cavn == 1)
        ...
        { /*cavity surface calculation */ /*finding the first atom */
          ...
          loadresp (atom0, ncycle, cycles, respect,
                    nvincyc); /*load relationships */
          cyc1 = fi1cyc (i, cycles, ncycle,
                         nvincyc); /*find first cycle by essentially the same
                                      function as find1cyc */
          nhits = searchcycle (ncycle, cyc1, respect,
                               hits); /*hits includes all cycles that belong
                                         the surface of interest but cyc1 */
          hits[nhits] = cyc1;
          nhits++; /*now ALL cycles are included!! */
          ...
          if (asar >= 0.001)
            ...
            aarea[atom0] += 0.0001;
          if (calcmode > 1)
            molarea[atom0] += asar * (ar[atom0] - prober)
                              * (ar[atom0] - prober) / (ar[atom0] * ar[atom0]);
          managedge (atom0, cycles, common, hits, nhits,
                     nvincyc); /*edge array management */
          ...
          while (1)
            {
              ...
              if (i == ned) /*outer surface calculation completed */
                ...
                { /*printf("\nProbe rolled over atom %d",edgeatom[i]); */ /* line
                                                                             used
                                                                             in
                                                                             debugging
                                                                           */
              ...
              if (aarea[i] != 0.)         /*found the cavity atom */
                {
                  ...
                  // gcvt(molarea[i],5,asa);           /*converting molecular
                  // areas into a string*/
                  for (j = 0; asa[j] != '\0'; j++)
                    ;
                  asa[j] = ' ';
                  ...
                  // gcvt(curvat[i],5,asa);           /*converting molecular
                  // areas into a string*/
                  for (j = 0; asa[j] != '\0'; j++)
                    ;
                  asa[j] = ' ';
