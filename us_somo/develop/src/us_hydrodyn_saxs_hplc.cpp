...
US_Hydrodyn_Saxs_Hplc::US_Hydrodyn_Saxs_Hplc(csv csv1, void *us_hydrodyn,
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)
           ->gparams.count("hplc_lock_min_retry_mult") ||
      !started_in_expert_mode) {
  ...
  QString dctr_file = USglobal->config_list.root_dir + QDir::separator() +
                      "etc" + QDir::separator() + "somo_hplc_default_dctr.dat";
...
void US_Hydrodyn_Saxs_Hplc::cancel() { close(); }
...
void US_Hydrodyn_Saxs_Hplc::add_dir() { add_files(false, true); }
...
bool US_Hydrodyn_Saxs_Hplc::load_file(QString filename, bool load_conc) {
  ...
  //       directive %3" ) ).arg( filename ).arg( i + 1 ).arg( qv[ i ] ); return
  //       false;
  //    }
  //    errormsg = "";
  //    return false;
  ...
  if (ext == "csv" && load_conc) {
    ...
    US_Hydrodyn_Saxs_Hplc_Conc_Csv_Frames *conc_csv_frames_instance =
        new US_Hydrodyn_Saxs_Hplc_Conc_Csv_Frames(us_hydrodyn, &conc_csv_params,
                                                  this);
...
QString US_Hydrodyn_Saxs_Hplc::qstring_common_head(QStringList qsl,
                                                   bool strip_digits) {
  if (!qsl.size()) {
    return "";
  }
...
QString US_Hydrodyn_Saxs_Hplc::qstring_common_tail(QStringList qsl,
                                                   bool strip_digits) {
  if (!qsl.size()) {
    return "";
  }
...
QString US_Hydrodyn_Saxs_Hplc::qstring_common_head(QString s1, QString s2) {
...
QString US_Hydrodyn_Saxs_Hplc::qstring_common_tail(QString s1, QString s2) {
  int min_len = (int)s1.length();
  if (min_len > (int)s2.length()) {
    min_len = (int)s2.length();
  }
...
bool US_Hydrodyn_Saxs_Hplc::save_file(QString file, bool &cancel,
                                      bool &overwrite_all) {
  if (!f_qs.count(file)) {
      ...
      if (!f_qs.count(*qs)) {
        ...
        if (frac_t_not_warned && timestamp != (double)((int)timestamp)) {
          QMessageBox::warning(
              ...
              us_tr("Fractional times found. This has not been fully tested"),
              QMessageBox::Ok);
...
#ifdef DEBUG_LOAD_REORDER
        us_qdebug(
            QString("%1 tmp %2 value %3 tmp_org %4 tmp_mid %5 tmp_clear %6")
                .arg(*qs)
                .arg(tmp)
                .arg(timestamp)
                .arg(tmp_org)
...
void US_Hydrodyn_Saxs_Hplc::check_discard_it_sd_mult(QStringList &files,
...
void US_Hydrodyn_Saxs_Hplc::plot_errors_zoomed(const QRectF & /* rect */) {}
    ...
    if (lb_files->item(i)->isSelected()) {
...

      {
...

        {
          ...
          //    ) ) .arg( f_I0se.count( file ) ? QString( " I0se:%1 [a.u.]"
          //    ).arg( f_I0se[ file ] ) : QString( "" ) ) .arg( f_I0st.count(
          //    file ) ? QString( " I0st:%1 [a.u.]" ).arg( f_I0st[ file ] ) :
          //    QString( "" ) ) .arg( use_conc ) // f_conc.count( file ) ?
          //    QString( " Conc:%1" ).arg( f_conc[ file ] ) : QString( "" ) )
          //    .arg( f_extc.count( file ) ? QString( " ExtC_or_DRIinc:%1"
          //    ).arg( f_extc[ file ] ) : QString( "" ) ) .arg( f_time.count(
          //    file ) ? QString( " Time:%1" ).arg( f_time[ file ] ) : QString(
          //    "" ) ) .arg( f_diffusion_len.count( file ) ? QString( "
          //    Diffusion Length: %1 [cm]" ).arg( f_diffusion_len[ file ] ) :
          //    QString("") ) .arg( f_e_nucleon_ratio.count( file ) ? QString( "
          //    Electron/nucleon ratio Z/A: %1" ).arg( f_e_nucleon_ratio[ file ]
          //    ) : QString("") ) .arg( f_nucleon_mass.count( file ) ? QString(
          //    " Nucleon mass: %1 [g]" ).arg( f_nucleon_mass[ file ] ) :
          //    QString("") ) .arg( f_solvent_e_dens.count( file ) ? QString( "
          //    Solvent e density: %1 [e A^-3]" ).arg( f_solvent_e_dens[ file ]
...
void US_Hydrodyn_Saxs_Hplc::rename_created(QListWidgetItem *lbi,
                                           const QPoint &) {
  map<QString, bool> existing_items;
  for (int i = 0; i < (int)lb_files->count(); i++) {
    existing_items[lb_files->item(i)->text()] = true;
...
void US_Hydrodyn_Saxs_Hplc::add_plot(QString name, vector<double> q,
...
void US_Hydrodyn_Saxs_Hplc::ggauss_msg_common(
    vector<map<int, set<double> > > &check_common) {
...
void US_Hydrodyn_Saxs_Hplc::gauss_add_marker(double pos, QColor color,
                                             QString text,
                                             Qt::Alignment align) {
...
vector<double> US_Hydrodyn_Saxs_Hplc::gaussian(
...
void US_Hydrodyn_Saxs_Hplc::gauss_pos_width_focus(bool hasFocus) {
  cout
      << QString("gauss_pos_width_focus %1\n").arg(hasFocus ? "true" : "false");
  if (hasFocus && !cb_ggauss_scroll->isChecked()) {
...
void US_Hydrodyn_Saxs_Hplc::gauss_pos_dist1_focus(bool hasFocus) {
  cout
      << QString("gauss_pos_dist1_focus %1\n").arg(hasFocus ? "true" : "false");
  if (hasFocus && !cb_ggauss_scroll->isChecked()) {
...
void US_Hydrodyn_Saxs_Hplc::gauss_pos_dist2_focus(bool hasFocus) {
  cout
      << QString("gauss_pos_dist2_focus %1\n").arg(hasFocus ? "true" : "false");
  if (hasFocus && !cb_ggauss_scroll->isChecked()) {
...
void US_Hydrodyn_Saxs_Hplc::gauss_fit_start_focus(bool hasFocus) {
  cout
      << QString("gauss_fit_start_focus %1\n").arg(hasFocus ? "true" : "false");
  if (hasFocus && !cb_ggauss_scroll->isChecked()) {
...
vector<double> US_Hydrodyn_Saxs_Hplc::compute_gaussian_sum(vector<double> t,
                                                           vector<double> g) {
...
vector<double> US_Hydrodyn_Saxs_Hplc::compute_gaussian(vector<double> t,
                                                       vector<double> g) {
  vector<double> result(t.size());
...
double US_Hydrodyn_Saxs_Hplc::compute_gaussian_peak(QString file,
                                                    vector<double> g) {
...
bool US_Hydrodyn_Saxs_Hplc::ggaussian_compatible(set<QString> &selected,
...
void US_Hydrodyn_Saxs_Hplc::add_ggaussian_curve(QString name,
                                                vector<double> y) {
