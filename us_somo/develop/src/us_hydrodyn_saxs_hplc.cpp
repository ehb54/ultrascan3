...
US_Hydrodyn_Saxs_Hplc::US_Hydrodyn_Saxs_Hplc (csv csv1, void *us_hydrodyn,
                                              QWidget *p)
    : QFrame (p)
{
  this->csv1 = csv1;
  ...
  gaussian_type = GAUSS;
  ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_gaussian_type"]
  ...
  use_line_width
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_line_width"].toUInt ();
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("hplc_bl_smooth")
      ...
      || ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_bl_smooth"] == "")
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_bl_smooth"] = "3";
    }
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)
           ->gparams.count ("hplc_cb_discard_it_sd_mult"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_cb_discard_it_sd_mult"]
          = "true";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("hplc_cb_guinier_qrgmax"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_cb_guinier_qrgmax"] = "true";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("hplc_guinier_qrgmax"))
    {
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("hplc_ampl_width_min")
      || !started_in_expert_mode)
    {
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("hplc_lock_min_retry_mult")
      || !started_in_expert_mode)
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_lock_min_retry_mult"] = "2";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("hplc_maxfpk_restart")
      || !started_in_expert_mode)
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_maxfpk_restart"] = "false";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)
           ->gparams.count ("hplc_maxfpk_restart_tries")
      || !started_in_expert_mode)
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_maxfpk_restart_tries"] = "3";
    }
  if (!((US_Hydrodyn *)us_hydrodyn)->gparams.count ("hplc_maxfpk_restart_pct")
      || !started_in_expert_mode)
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_maxfpk_restart_pct"] = "10";
    }
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)
           ->gparams.count ("hplc_cb_makeiq_cutmax_pct"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_cb_makeiq_cutmax_pct"]
          = "true";
    }
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)
           ->gparams.count ("hplc_cb_makeiq_avg_peaks"))
    {
      ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_cb_makeiq_avg_peaks"]
          = "false";
    }
...
void
US_Hydrodyn_Saxs_Hplc::push_back_color_if_ok (QColor bg, QColor set)
{
...
void
US_Hydrodyn_Saxs_Hplc::cancel ()
{
  close ();
}

void
US_Hydrodyn_Saxs_Hplc::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Saxs_Hplc::closeEvent (QCloseEvent *e)
{
  QMessageBox mb (
      this->windowTitle (),
...
void
US_Hydrodyn_Saxs_Hplc::clear_display ()
{
  editor->clear ();
  editor->append ("\n\n");
}
...
void
US_Hydrodyn_Saxs_Hplc::update_font ()
{
  bool ok;
  QFont newFont;
...
void
US_Hydrodyn_Saxs_Hplc::editor_msg (QString color, QString msg)
{
...
void
US_Hydrodyn_Saxs_Hplc::editor_msg_qc (QColor qcolor, QString msg)
{
  QColor save_color = editor->textColor ();
...
bool
US_Hydrodyn_Saxs_Hplc::activate_saxs_window ()
{
  if (!*saxs_widget)
    {
...
csv
US_Hydrodyn_Saxs_Hplc::current_csv ()
{
  csv tmp_csv = csv1;
...
void
US_Hydrodyn_Saxs_Hplc::clear_files ()
{
  disable_all ();
...
void
US_Hydrodyn_Saxs_Hplc::remove_files (set<QString> &fileset, bool replot)
{
  QStringList files;
...
void
US_Hydrodyn_Saxs_Hplc::clear_files (QStringList files, bool quiet)
{
...
public:
  ...
  QString name;
  bool
  operator< (const hplc_sortable_qstring &objIn) const
  {
    return x < objIn.x;
  }
};
...
void
US_Hydrodyn_Saxs_Hplc::load_conc ()
{
  add_files (true);
...
void
US_Hydrodyn_Saxs_Hplc::add_dir ()
{
  add_files (false, true);
}

void
US_Hydrodyn_Saxs_Hplc::add_files (bool load_conc, bool from_dir)
{
  disable_all ();
...
void
US_Hydrodyn_Saxs_Hplc::add_files (QStringList filenames)
{
  map<QString, bool> existing_items;
  for (int i = 0; i < lb_files->count (); i++)
    {
...
void
US_Hydrodyn_Saxs_Hplc::update_files ()
{
  if (!disable_updates)
    {
      plot_files ();
...
void
US_Hydrodyn_Saxs_Hplc::select_all ()
{
...
void
US_Hydrodyn_Saxs_Hplc::select_all_created ()
{
  bool all_selected = true;
...
bool
US_Hydrodyn_Saxs_Hplc::load_file (QString filename, bool load_conc)
{
  errormsg = "";
  QFile f (filename);
  if (!f.exists ())
  ...
  // {
  //    QRegExp rx_saxs_hplc_param_g_conc                  ( "^#
  //    __saxs_hplc_param_g_conc: (\\S+)\\s*$" ); QRegExp
  //    rx_saxs_hplc_param_g_psv                   ( "^#
  //    __saxs_hplc_param_g_psv: (\\S+)\\s*$" ); QRegExp
  //    rx_saxs_hplc_param_I0_exp                  ( "^#
  //    __saxs_hplc_param_I0_exp: (\\S+)\\s*$" ); QRegExp
  //    rx_saxs_hplc_param_I0_theo                 ( "^#
  //    __saxs_hplc_param_I0_theo: (\\S+)\\s*$" ); QRegExp
  //    rx_saxs_hplc_param_diffusion_len           ( "^#
  //    __saxs_hplc_param_diffusion_len: (\\S+)\\s*$" ); QRegExp
  //    rx_saxs_hplc_param_electron_nucleon_ratio  ( "^#
  //    __saxs_hplc_param_electron_nucleon_ratio: (\\S+)\\s*$" ); QRegExp
  //    rx_saxs_hplc_param_nucleon_mass            ( "^#
  //    __saxs_hplc_param_nucleon_mass: (\\S+)\\s*$" ); QRegExp
  //    rx_saxs_hplc_param_solvent_electron_density( "^#
  //    __saxs_hplc_param_solvent_electron_density: (\\S+)\\s*$" );

  //    for ( int i = 1; i < (int) qv.size(); i++ ) {

  //       if ( rx_saxs_hplc_param_g_conc.indexIn( qv[ i ] ) != -1 ) {
  //          saxs_hplc_param_g_conc = rx_saxs_hplc_param_g_conc.cap( 1
  //          ).toDouble(); continue;
  //       }
  //       if ( rx_saxs_hplc_param_g_psv.indexIn( qv[ i ] ) != -1 ) {
  //          saxs_hplc_param_g_psv = rx_saxs_hplc_param_g_psv.cap( 1
  //          ).toDouble(); continue;
  //       }
  //       if ( rx_saxs_hplc_param_I0_exp.indexIn( qv[ i ] ) != -1 ) {
  //          saxs_hplc_param_I0_exp = rx_saxs_hplc_param_I0_exp.cap( 1
  //          ).toDouble(); continue;
  //       }
  //       if ( rx_saxs_hplc_param_I0_theo.indexIn( qv[ i ] ) != -1 ) {
  //          saxs_hplc_param_I0_theo = rx_saxs_hplc_param_I0_theo.cap( 1
  //          ).toDouble(); continue;
  //       }
  //       if ( rx_saxs_hplc_param_diffusion_len.indexIn( qv[ i ] ) != -1 ) {
  //          saxs_hplc_param_diffusion_len =
  //          rx_saxs_hplc_param_diffusion_len.cap( 1 ).toDouble(); continue;
  //       }
  //       if ( rx_saxs_hplc_param_electron_nucleon_ratio.indexIn( qv[ i ] ) !=
  //       -1 ) {
  //          saxs_hplc_param_electron_nucleon_ratio =
  //          rx_saxs_hplc_param_electron_nucleon_ratio.cap( 1 ).toDouble();
  //          continue;
  //       }
  //       if ( rx_saxs_hplc_param_nucleon_mass.indexIn( qv[ i ] ) != -1 ) {
  //          saxs_hplc_param_nucleon_mass =
  //          rx_saxs_hplc_param_nucleon_mass.cap( 1 ).toDouble(); continue;
  //       }
  //       if ( rx_saxs_hplc_param_solvent_electron_density.indexIn( qv[ i ] )
  //       != -1 ) {
  //          saxs_hplc_param_solvent_electron_density =
  //          rx_saxs_hplc_param_solvent_electron_density.cap( 1 ).toDouble();
  //          continue;
  //       }

  //       errormsg = QString( us_tr( "Error: loading %1 line %2 unrecognied
  //       directive %3" ) ).arg( filename ).arg( i + 1 ).arg( qv[ i ] );
  //       return false;
  //    }
  //    errormsg = "";
  //    return false;
  ...
  if (ext == "csv" && load_conc)
    {
      ...
      US_Hydrodyn_Saxs_Hplc_Conc_Csv_Frames *conc_csv_frames_instance
          = new US_Hydrodyn_Saxs_Hplc_Conc_Csv_Frames (us_hydrodyn,
...
void
US_Hydrodyn_Saxs_Hplc::set_conc_file (QString file)
{
  if (file.isEmpty ())
    {
      for (int i = 0; i < lb_files->count (); i++)
...
void
US_Hydrodyn_Saxs_Hplc::set_hplc ()
{
  for (int i = 0; i < lb_files->count (); i++)
    {
      if (lb_files->item (i)->isSelected ())
...
void
US_Hydrodyn_Saxs_Hplc::set_signal ()
{
  for (int i = 0; i < lb_files->count (); i++)
    {
      if (lb_files->item (i)->isSelected ())
...
void
US_Hydrodyn_Saxs_Hplc::set_empty ()
{
  for (int i = 0; i < lb_files->count (); i++)
    {
      if (lb_files->item (i)->isSelected ())
...
void
US_Hydrodyn_Saxs_Hplc::update_created_files ()
{
  if (!disable_updates)
    {
      update_enables ();
...
QString
US_Hydrodyn_Saxs_Hplc::qstring_common_head (QStringList qsl, bool strip_digits)
{
  if (!qsl.size ())
    {
      return "";
...
QString
US_Hydrodyn_Saxs_Hplc::qstring_common_tail (QStringList qsl, bool strip_digits)
{
  if (!qsl.size ())
    {
      return "";
...
QString
US_Hydrodyn_Saxs_Hplc::qstring_common_head (QString s1, QString s2)
{
...
QString
US_Hydrodyn_Saxs_Hplc::qstring_common_tail (QString s1, QString s2)
{
  int min_len = (int)s1.length ();
  if (min_len > (int)s2.length ())
    {
...
void
US_Hydrodyn_Saxs_Hplc::save_created ()
{
  QStringList created_not_saved_list;
...
void
US_Hydrodyn_Saxs_Hplc::save_created_csv ()
{
  QStringList created_not_saved_list;
...
bool
US_Hydrodyn_Saxs_Hplc::save_files_csv (QStringList files)
{
  if (!files.size ())
    {
      editor_msg ("red",
...
bool
US_Hydrodyn_Saxs_Hplc::save_files (QStringList files)
{
...
bool
US_Hydrodyn_Saxs_Hplc::save_file (QString file, bool &cancel,
                                  bool &overwrite_all)
{
  if (!f_qs.count (file))
    {
...

  {
    ...
    //    ) ) .arg( file ) .arg( units ) .arg( f_psv .count( file ) ? QString(
    //    " PSV:%1 [mL/g]"  ).arg( f_psv [ file ] ) : QString( "" ) ) .arg(
    //    f_I0se.count( file ) ? QString( " I0se:%1 [a.u.]" ).arg( f_I0se[ file
    //    ] ) : QString( "" ) ) .arg( f_I0st.count( file ) ? QString( " I0st:%1
    //    [a.u.]" ).arg( f_I0st[ file ] ) : QString( "" ) ) .arg( use_conc ) //
    //    f_conc.count( file ) ? QString( " Conc:%1" ).arg( f_conc[ file ] ) :
    //    QString( "" ) ) .arg( f_extc.count( file ) ? QString( "
    //    ExtC_or_DRIinc:%1" ).arg( f_extc[ file ] ) : QString( "" ) ) .arg(
    //    f_time.count( file ) ? QString( " Time:%1" ).arg( f_time[ file ] ) :
    //    QString( "" ) ) .arg( f_diffusion_len.count( file ) ? QString( "
    //    Diffusion Length: %1 [cm]" ).arg( f_diffusion_len[ file ] ) :
    //    QString("") ) .arg( f_e_nucleon_ratio.count( file ) ? QString( "
    //    Electron/nucleon ratio Z/A: %1" ).arg( f_e_nucleon_ratio[ file ] ) :
    //    QString("") ) .arg( f_nucleon_mass.count( file ) ? QString( " Nucleon
    //    mass: %1 [g]" ).arg( f_nucleon_mass[ file ] ) : QString("") ) .arg(
    //    f_solvent_e_dens.count( file ) ? QString( " Solvent e density: %1 [e
    //    A^-3]" ).arg( f_solvent_e_dens[ file ] ) : QString("") ) .arg(
...
void
US_Hydrodyn_Saxs_Hplc::update_csv_conc ()
{
...
US_Hydrodyn_Saxs_Hplc::conc ()
{
  ...
  if (conc_widget)
    ...
    {
      conc_window = new US_Hydrodyn_Saxs_Hplc_Conc (csv_conc, this);
      conc_window->show ();
    }
}

map<QString, double>
US_Hydrodyn_Saxs_Hplc::current_concs (bool quiet)
{
...
map<QString, double>
US_Hydrodyn_Saxs_Hplc::window_concs ()
{
  map<QString, double> concs;
  if (conc_widget)
    {
...
void
US_Hydrodyn_Saxs_Hplc::conc_avg ()
{
  //    QStringList files;
  //    for ( int i = 0; i < lb_files->count(); i++ )
  //    {
...
vector<double>
US_Hydrodyn_Saxs_Hplc::union_q (QStringList files)
{
...
bool
US_Hydrodyn_Saxs_Hplc::get_peak (QString file, double &peak, bool full)
{
  double pos;
...
bool
US_Hydrodyn_Saxs_Hplc::get_peak (QString file, double &peak, double &pos,
                                 bool full)
{
  if (!f_Is.count (file))
    {
...
void
US_Hydrodyn_Saxs_Hplc::smooth (QStringList files)
{
  bool ok;
...
void
US_Hydrodyn_Saxs_Hplc::create_i_of_t ()
{
  disable_all ();
...
void
US_Hydrodyn_Saxs_Hplc::create_i_of_t (QStringList files)
{
  // find common q
  QString head = qstring_common_head (files, true);
        ...
        if (!f_qs.count (*qs))
        ...
        else
          {
            ...
            if (frac_t_not_warned && timestamp != (double)((int)timestamp))
              {
                QMessageBox::warning (
                    this, this->windowTitle () + us_tr (": Make I(t)"),
                    us_tr ("Fractional times found. This has not been fully "
                           "tested"),
                    QMessageBox::Ok);
            ...
            us_qdebug (
                QString (
                    "%1 tmp %2 value %3 tmp_org %4 tmp_mid %5 tmp_clear %6")
                    .arg (*qs)
                    .arg (tmp)
                    .arg (timestamp)
                    .arg (tmp_org)
...
void
US_Hydrodyn_Saxs_Hplc::test_i_of_t ()
{
  disable_all ();
  QStringList files = all_selected_files ();
...
bool
US_Hydrodyn_Saxs_Hplc::check_zi_window (QStringList &files,
                                        const QString &extra_text)
{
  int window
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_zi_window"].toInt ();
...
void
US_Hydrodyn_Saxs_Hplc::check_discard_it_sd_mult (QStringList &files,
                                                 bool optionally_discard)
{
  if (((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_cb_discard_it_sd_mult"]
          != "true"
      || !files.size ())
    {
      return;
  ...
  double mult = ((US_Hydrodyn *)us_hydrodyn)
                    ->gparams["hplc_discard_it_sd_mult"]
                    .toDouble ();
...
bool
US_Hydrodyn_Saxs_Hplc::all_selected_have_nonzero_conc ()
{
...
void
US_Hydrodyn_Saxs_Hplc::delete_zoomer_if_ranges_changed ()
{
}

void
US_Hydrodyn_Saxs_Hplc::show_created ()
{
...
void
US_Hydrodyn_Saxs_Hplc::show_only_created ()
{
  disable_updates = true;
  lb_files->clearSelection ();
  show_created ();
...
QString
US_Hydrodyn_Saxs_Hplc::vector_double_to_csv (vector<double> vd)
{
  QString result;
...
void
US_Hydrodyn_Saxs_Hplc::plot_zoomed (const QRectF & /* rect */)
{
...
void
US_Hydrodyn_Saxs_Hplc::plot_errors_zoomed (const QRectF & /* rect */)
{
}

void
US_Hydrodyn_Saxs_Hplc::zoom_info ()
{
  if (plot_dist_zoomer)
    {
...
void
US_Hydrodyn_Saxs_Hplc::plot_mouse (const QMouseEvent & /* me */)
{
  // cout << "mouse event\n";
  // zoom_info();
  if (plot_dist_zoomer)
...
void
US_Hydrodyn_Saxs_Hplc::select_vis ()
{
...
void
US_Hydrodyn_Saxs_Hplc::remove_vis ()
{
  double zrx1 = plot_dist_zoomer->zoomRect ().left ();
  double zrx2 = plot_dist_zoomer->zoomRect ().right ();
  double zry1 = plot_dist_zoomer->zoomRect ().top ();
...
void
US_Hydrodyn_Saxs_Hplc::crop_left ()
{
  ...
  if (minx < plot_dist_zoomer->zoomRect ().left ()
...
void
US_Hydrodyn_Saxs_Hplc::crop_right ()
{
...
void
US_Hydrodyn_Saxs_Hplc::crop_undo ()
{
  if (!crop_undos.size ())
    {
      return;
      ...
      if (lb_files->item (i)->isSelected ())
        {
...

          {
...

            {
              ...
              //    "Frame " : "" ) ) .arg( file ) .arg( units ) .arg( f_psv
              //    .count( file ) ? QString( " PSV:%1 [mL/g]"  ).arg( f_psv [
              //    file ] ) : QString( "" ) ) .arg( f_I0se.count( file ) ?
              //    QString( " I0se:%1 [a.u.]" ).arg( f_I0se[ file ] ) :
              //    QString( "" ) ) .arg( f_I0st.count( file ) ? QString( "
              //    I0st:%1 [a.u.]" ).arg( f_I0st[ file ] ) : QString( "" ) )
              //    .arg( use_conc ) // f_conc.count( file ) ? QString( "
              //    Conc:%1" ).arg( f_conc[ file ] ) : QString( "" ) ) .arg(
              //    f_extc.count( file ) ? QString( " ExtC_or_DRIinc:%1" ).arg(
              //    f_extc[ file ] ) : QString( "" ) ) .arg( f_time.count( file
              //    ) ? QString( " Time:%1" ).arg( f_time[ file ] ) : QString(
              //    "" ) ) .arg( f_diffusion_len.count( file ) ? QString( "
              //    Diffusion Length: %1 [cm]" ).arg( f_diffusion_len[ file ] )
              //    : QString("") ) .arg( f_e_nucleon_ratio.count( file ) ?
              //    QString( " Electron/nucleon ratio Z/A: %1" ).arg(
              //    f_e_nucleon_ratio[ file ] ) : QString("") ) .arg(
              //    f_nucleon_mass.count( file ) ? QString( " Nucleon mass: %1
              //    [g]" ).arg( f_nucleon_mass[ file ] ) : QString("") ) .arg(
              //    f_solvent_e_dens.count( file ) ? QString( " Solvent e
              //    density: %1 [e A^-3]" ).arg( f_solvent_e_dens[ file ] ) :
...
bool
US_Hydrodyn_Saxs_Hplc::is_nonzero_vector (vector<double> &v)
{
...
bool
US_Hydrodyn_Saxs_Hplc::is_zero_vector (vector<double> &v)
{
  bool is_zero = true;
  for (int i = 0; i < (int)v.size (); i++)
    {
...
void
US_Hydrodyn_Saxs_Hplc::to_created (QString file)
{
...
void
US_Hydrodyn_Saxs_Hplc::crop_vis ()
{
  // find curves within zoomRect & select only them
...
void
US_Hydrodyn_Saxs_Hplc::crop_to_vis ()
{
  // find curves within zoomRect & select only them
  double minx = plot_dist_zoomer->zoomRect ().left ();
  double maxx = plot_dist_zoomer->zoomRect ().right ();
...
void
US_Hydrodyn_Saxs_Hplc::legend_set ()
{
...
void
US_Hydrodyn_Saxs_Hplc::similar_files ()
{
  vector<QString> selected;
  for (int i = 0; i < lb_files->count (); i++)
    {
  ...
  cout << QString (
              "select to match <%1> in directory <%2> using regexp <%3>\n")
              .arg (similar)
              .arg (dir)
              .arg (match)
              .toLatin1 ()
...
void
US_Hydrodyn_Saxs_Hplc::regex_load ()
{
...
void
US_Hydrodyn_Saxs_Hplc::rename_from_context (const QPoint &pos)
{
...
void
US_Hydrodyn_Saxs_Hplc::rename_created (QListWidgetItem *lbi, const QPoint &)
{
  map<QString, bool> existing_items;
  for (int i = 0; i < (int)lb_files->count (); i++)
    {
...
void
US_Hydrodyn_Saxs_Hplc::normalize ()
{
...
void
US_Hydrodyn_Saxs_Hplc::normalize (set<QString> &produced)
{
  QStringList files = all_selected_files ();
  //    for ( int i = 0; i < lb_files->count(); i++ )
  //    {
...
void
US_Hydrodyn_Saxs_Hplc::add_plot (QString name, vector<double> q,
...
void
US_Hydrodyn_Saxs_Hplc::crop_zero ()
{
...
bool
US_Hydrodyn_Saxs_Hplc::compatible_files (QStringList files)
{
  if (!files.size ())
    {
      return true;
...
bool
US_Hydrodyn_Saxs_Hplc::type_files (QStringList files)
{
  if (!files.size ())
    {
      return false;
...
void
US_Hydrodyn_Saxs_Hplc::gaussian_enables ()
{
...
void
US_Hydrodyn_Saxs_Hplc::update_gauss_pos ()
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_start ()
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_clear ()
{
  gaussian_pos = 0;
  gaussians.clear ();
  gauss_init_markers ();
...
void
US_Hydrodyn_Saxs_Hplc::gauss_new ()
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_prev ()
{
  if (gaussian_pos > 0)
    {
      gaussian_pos--;
...
void
US_Hydrodyn_Saxs_Hplc::gauss_next ()
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_save ()
{
  {
    QDir dir1 (le_created_dir->text ());
    if (!dir1.exists ())
...
void
US_Hydrodyn_Saxs_Hplc::ggauss_msg_common (
    vector<map<int, set<double> > > &check_common)
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_pos_text (const QString &text)
{
  if (gaussians.size ())
    {
      if (current_mode == MODE_GAUSSIAN)
...
void
US_Hydrodyn_Saxs_Hplc::gauss_pos_width_text (const QString &text)
{
  if (gaussians.size ())
    {
      if (current_mode == MODE_GAUSSIAN)
...
void
US_Hydrodyn_Saxs_Hplc::gauss_pos_height_text (const QString &text)
{
  if (gaussians.size ())
    {
...
void
US_Hydrodyn_Saxs_Hplc::gauss_pos_dist1_text (const QString &text)
{
  if (gaussians.size ())
    {
      if (current_mode == MODE_GAUSSIAN)
...
void
US_Hydrodyn_Saxs_Hplc::gauss_pos_dist2_text (const QString &text)
{
  if (gaussians.size ())
    {
      if (current_mode == MODE_GAUSSIAN)
...
void
US_Hydrodyn_Saxs_Hplc::gauss_fit_start_text (const QString &text)
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_fit_end_text (const QString &text)
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_add_marker (double pos, QColor color,
                                         QString text, Qt::Alignment align)
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_add_hline (double center, double width)
{
  double fwhm = 2.354820045e0 * width;
...
void
US_Hydrodyn_Saxs_Hplc::gauss_init_markers ()
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_delete_markers ()
{
...
vector<double>
US_Hydrodyn_Saxs_Hplc::gaussian (
...
void
US_Hydrodyn_Saxs_Hplc::gauss_delete ()
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_replot_gaussian ()
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_add_gaussian (double *g, QColor color)
...
void
US_Hydrodyn_Saxs_Hplc::gauss_init_gaussians ()
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_delete_gaussians ()
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_pos_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_pos_width_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_pos_height_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_pos_dist1_focus (bool hasFocus)
{
  ...
  if (hasFocus && !cb_ggauss_scroll->isChecked ())
    {
      ...
      update_gauss_pos ();
      double dist
          = ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_dist_max"].toDouble ();
...
void
US_Hydrodyn_Saxs_Hplc::gauss_pos_dist2_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_fit_start_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Saxs_Hplc::gauss_fit_end_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Saxs_Hplc::plot_gaussian_sum ()
{
...
void
US_Hydrodyn_Saxs_Hplc::replot_gaussian_sum ()
{
  // add up the curve data
...
void
US_Hydrodyn_Saxs_Hplc::gauss_fit ()
{
  check_fit_range ();
  ...
  if (current_mode == MODE_GGAUSSIAN)
    {
      ...
      US_Hydrodyn_Saxs_Hplc_Fit_Global *shfg
          = new US_Hydrodyn_Saxs_Hplc_Fit_Global (this, this);
  ...
  US_Hydrodyn_Saxs_Hplc_Fit *shf
      = new US_Hydrodyn_Saxs_Hplc_Fit (this, true, this);
...
void
US_Hydrodyn_Saxs_Hplc::create_i_of_q ()
{
  disable_all ();
...
vector<double>
US_Hydrodyn_Saxs_Hplc::compute_gaussian_sum (vector<double> t,
                                             vector<double> g)
{
...
vector<double>
US_Hydrodyn_Saxs_Hplc::compute_gaussian (vector<double> t, vector<double> g)
{
  vector<double> result (t.size ());
...
double
US_Hydrodyn_Saxs_Hplc::compute_gaussian_peak (QString file, vector<double> g)
{
...
QString
US_Hydrodyn_Saxs_Hplc::pad_zeros (int val, int max)
{
...
QStringList
US_Hydrodyn_Saxs_Hplc::all_selected_files ()
{
  QStringList files;
  for (int i = 0; i < lb_files->count (); i++)
    {
...
set<QString>
US_Hydrodyn_Saxs_Hplc::all_selected_files_set ()
{
  set<QString> result;
  for (int i = 0; i < lb_files->count (); i++)
    {
...
QStringList
US_Hydrodyn_Saxs_Hplc::all_files ()
{
  QStringList files;
  for (int i = 0; i < lb_files->count (); i++)
    {
...
map<QString, bool>
US_Hydrodyn_Saxs_Hplc::all_files_map ()
{
...
int
US_Hydrodyn_Saxs_Hplc::ggaussian_sel_no_gaussian_count ()
{
...
bool
US_Hydrodyn_Saxs_Hplc::ggaussian_compatible (bool check_against_global)
{
  QStringList files = all_selected_files ();
...
bool
US_Hydrodyn_Saxs_Hplc::ggaussian_compatible (set<QString> &selected,
...
bool
US_Hydrodyn_Saxs_Hplc::ggaussian_compatible (QStringList &files,
                                             bool check_against_global)
{
...
i++ )
   {
      ...
      if ( cb_fix_width->isChecked() )
      {
         for ( unsigned int j = 0; j < unified_ggaussian_gaussians_size; j++ )
         {
            ...
            //          cout << QString( "for pos %1 t is %2 index %3 gaussian
...
void
US_Hydrodyn_Saxs_Hplc::ggauss_rmsd ()
{
  if (ggauss_recompute ())
    {
...
void
US_Hydrodyn_Saxs_Hplc::add_ggaussian_curve (QString name, vector<double> y)
{
...
void
US_Hydrodyn_Saxs_Hplc::ggauss_results ()
{
  if (unified_ggaussian_ok)
    {
...
void
US_Hydrodyn_Saxs_Hplc::dir_pressed ()
{
...
void
US_Hydrodyn_Saxs_Hplc::created_dir_pressed ()
{
  QString use_dir = le_dir->text ();
  ((US_Hydrodyn *)us_hydrodyn)->select_from_directory_history (use_dir, this);
  QString s = QFileDialog::getExistingDirectory (
...
void
US_Hydrodyn_Saxs_Hplc::gauss_as_curves ()
{
  if (current_mode == MODE_GAUSSIAN)
    {
      for (unsigned int i = 0; i < (unsigned int)gaussians.size ();
...
void
US_Hydrodyn_Saxs_Hplc::set_sd_weight ()
{
  if (current_mode == MODE_GAUSSIAN)
    {
      replot_gaussian_sum ();
...
void
US_Hydrodyn_Saxs_Hplc::set_fix_width ()
{
}

void
US_Hydrodyn_Saxs_Hplc::set_fix_dist1 ()
{
}

void
US_Hydrodyn_Saxs_Hplc::set_fix_dist2 ()
{
}

bool
US_Hydrodyn_Saxs_Hplc::check_fit_range ()
{
  if (!gaussians.size ())
    {
      return true;
...
void
US_Hydrodyn_Saxs_Hplc::hide_widgets (vector<QWidget *> widgets, bool hide)
{
...
void
US_Hydrodyn_Saxs_Hplc::save_state ()
{
  QString use_dir = QDir::currentPath ();
...
void
US_Hydrodyn_Saxs_Hplc::update_gauss_mode ()
{
