...
mals_saxs_stack_data
US_Hydrodyn_Mals_Saxs::current_data (bool selected_only)
{
          ...
          if (selected.count (lb_created_files->item (i)->text ()))
            {
              ...
              tmp_stack
                  .created_selected_files[lb_created_files->item (i)->text ()]
                  = true;
            }
        }
    }
...
void
US_Hydrodyn_Mals_Saxs::stack_push_all ()
{
  disable_all ();
...
void
US_Hydrodyn_Mals_Saxs::stack_push_sel ()
{
  disable_all ();
...
void
US_Hydrodyn_Mals_Saxs::set_current_data (mals_saxs_stack_data &tmp_stack)
{
  lb_files->clear ();
...
void
US_Hydrodyn_Mals_Saxs::stack_drop ()
{
...
void
US_Hydrodyn_Mals_Saxs::stack_rot_up ()
{
...
void
US_Hydrodyn_Mals_Saxs::stack_rot_down ()
{
  vector<mals_saxs_stack_data> new_stack;
...
void
US_Hydrodyn_Mals_Saxs::stack_swap ()
{
...
void
US_Hydrodyn_Mals_Saxs::stack_join ()
{
  stack_join (stack_data.back ());
}

void
US_Hydrodyn_Mals_Saxs::stack_join (mals_saxs_stack_data &tmp_stack)
{
      ...
      if (!f_qs.count (name))
        {
          ...
          f_is_time[name] = tmp_stack.f_is_time.count (name)
                                ? tmp_stack.f_is_time[name]
                                : false;
          f_psv[name]
              = tmp_stack.f_psv.count (name) ? tmp_stack.f_psv[name] : 0e0;
          f_header[name] = tmp_stack.f_header.count (name)
          ...
          if (tmp_stack.created_files_not_saved.count (name))
            {
              created_files_not_saved[name]
                  = tmp_stack.created_files_not_saved[name];
            }
...
void
US_Hydrodyn_Mals_Saxs::stack_copy ()
{
...
void
US_Hydrodyn_Mals_Saxs::stack_pcopy ()
{
      ...
      if (!clipboard.f_qs.count (name))
        {
          ...
          clipboard.f_psv[name]
              = adds.f_psv.count (name) ? adds.f_psv[name] : 0e0;
...
void
US_Hydrodyn_Mals_Saxs::stack_paste ()
{
  disable_all ();
...
void
US_Hydrodyn_Mals_Saxs::stack_clear ()
{
  disable_all ();
