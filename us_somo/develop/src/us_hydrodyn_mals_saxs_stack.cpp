...
mals_saxs_stack_data US_Hydrodyn_Mals_Saxs::current_data(bool selected_only) {
  mals_saxs_stack_data tmp_stack;
  if (selected_only) {
    ...
    for (unsigned int i = 0; i < (unsigned int)files.size(); i++) {
      ...
      if (created_files_not_saved.count(files[i])) {
        tmp_stack.created_files_not_saved[files[i]] =
            created_files_not_saved[files[i]];
      }
    ...
    for (int i = 0; i < lb_created_files->count(); i++) {
      if (selected.count(lb_created_files->item(i)->text())) {
        ...
        tmp_stack.created_selected_files[lb_created_files->item(i)->text()] =
            true;
      }
    }
  } else {
...
void US_Hydrodyn_Mals_Saxs::stack_push_all() {
  disable_all();
...
void US_Hydrodyn_Mals_Saxs::stack_push_sel() {
  disable_all();
...
void US_Hydrodyn_Mals_Saxs::set_current_data(mals_saxs_stack_data& tmp_stack) {
  lb_files->clear();
  ...
  for (int i = 0; i < lb_created_files->count(); i++) {
    if (tmp_stack.created_selected_files.count(
...
void US_Hydrodyn_Mals_Saxs::stack_drop() {
  ...
  if (created_not_saved_list.size()) {
    ...
    switch (QMessageBox::warning(
        ...
        QString(us_tr("Please note:\n\n"
            ...
            .arg(qsl.join("\n")),
        us_tr("&Save them now"), us_tr("&Remove them anyway"), us_tr("&Quit"),
        0,  // Stop == button 0
        0   // Escape == button 0
        )) {
...
void US_Hydrodyn_Mals_Saxs::stack_rot_up() {
  vector<mals_saxs_stack_data> new_stack;
...
void US_Hydrodyn_Mals_Saxs::stack_rot_down() {
  vector<mals_saxs_stack_data> new_stack;
...
void US_Hydrodyn_Mals_Saxs::stack_swap() {
  mals_saxs_stack_data tmp_stack = current_data();
  disable_updates = true;
  set_current_data(stack_data.back());
...
void US_Hydrodyn_Mals_Saxs::stack_join() { stack_join(stack_data.back()); }

void US_Hydrodyn_Mals_Saxs::stack_join(mals_saxs_stack_data& tmp_stack) {
  // join all files in stack_data.back() with current
  mals_saxs_stack_data current = current_data();

  disable_updates = true;

  map<QString, bool> created_files;

  for (unsigned int i = 0; i < (unsigned int)tmp_stack.created_files.size();
       i++) {
  ...
  for (unsigned int i = 0; i < (unsigned int)tmp_stack.files.size(); i++) {
    ...
    if (!f_qs.count(name)) {
      ...
      f_is_time[name] =
          tmp_stack.f_is_time.count(name) ? tmp_stack.f_is_time[name] : false;
      f_psv[name] = tmp_stack.f_psv.count(name) ? tmp_stack.f_psv[name] : 0e0;
      f_header[name] =
          tmp_stack.f_header.count(name) ? tmp_stack.f_header[name] : "";
      f_I0se[name] =
          tmp_stack.f_I0se.count(name) ? tmp_stack.f_I0se[name] : 0e0;
      f_conc[name] =
          tmp_stack.f_conc.count(name) ? tmp_stack.f_conc[name] : 0e0;
...
void US_Hydrodyn_Mals_Saxs::stack_copy() {
  clipboard = current_data(true);
  update_enables();
}

void US_Hydrodyn_Mals_Saxs::stack_pcopy() {
  mals_saxs_stack_data adds = current_data(true);
  ...
  for (unsigned int i = 0; i < (unsigned int)adds.files.size(); i++) {
    ...
    if (!clipboard.f_qs.count(name)) {
      ...
      clipboard.f_is_time[name] =
          adds.f_is_time.count(name) ? adds.f_is_time[name] : false;
      clipboard.f_psv[name] = adds.f_psv.count(name) ? adds.f_psv[name] : 0e0;
      clipboard.f_header[name] =
          adds.f_header.count(name) ? adds.f_header[name] : "";
      clipboard.f_I0se[name] =
          adds.f_I0se.count(name) ? adds.f_I0se[name] : 0e0;
      clipboard.f_conc[name] =
          adds.f_conc.count(name) ? adds.f_conc[name] : 0e0;
      ...
      if (adds.created_files_not_saved.count(name)) {
        clipboard.created_files_not_saved[name] =
            adds.created_files_not_saved[name];
      }
      ...
      if (created_files.count(name)) {
        ...
        if (adds.created_selected_files.count(name)) {
          clipboard.created_selected_files[name] =
              adds.created_selected_files[name];
        }
      }
...
void US_Hydrodyn_Mals_Saxs::stack_paste() {
  disable_all();
  stack_join(clipboard);
}

void US_Hydrodyn_Mals_Saxs::stack_clear() {
  disable_all();
