...
void US_Hydrodyn_Mals_Simulate::setupGUI() {
  if (files.size() > 5) {
    QMessageBox::warning(
        this, windowTitle(),
        us_tr(
            "A maximum of 5 intensity curves may be selected for simulation"));
    close();
    return;
  }
  ...
  for (int i = 0; i < (int)files.size(); ++i) {
    ...
    this_data.le_i_mult->setFont(QFont(USglobal->config_list.fontFamily,
                                       USglobal->config_list.fontSize - 1));
    ...
    this_data.le_center->setFont(QFont(USglobal->config_list.fontFamily,
                                       USglobal->config_list.fontSize - 1));
...
void US_Hydrodyn_Mals_Simulate::cancel() { close(); }

void US_Hydrodyn_Mals_Simulate::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/simulate.html");
}

void US_Hydrodyn_Mals_Simulate::closeEvent(QCloseEvent *e) {
  QMessageBox mb(
      this->windowTitle(),
...
void US_Hydrodyn_Mals_Simulate::displayData() { update_enables(); }

void US_Hydrodyn_Mals_Simulate::update_enables() {
...
static double_pair random_normal(double mean1, double sd1, double mean2,
                                 double sd2) {
  double s;
  double_pair v;
  do {
...
void US_Hydrodyn_Mals_Simulate::simulate() {
      ...
      if (cb_i_power->isChecked()) {
        ...
        for (int j = 0; j < (int)Itt.size(); ++j) {
          ...
          for (int k = 0; k < (int)t.size(); ++k) {
            // accum += pow( accum, exponent ) + pow( Itt[ j ][ k ] * alpha,
            // i_power ); accum += ( accum + pow( Itt[ j ][ k ] * alpha, i_power
            // ) ) * exponent;
