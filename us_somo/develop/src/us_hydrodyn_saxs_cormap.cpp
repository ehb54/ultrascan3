...
US_Hydrodyn_Saxs_Cormap::US_Hydrodyn_Saxs_Cormap(
    ...
    vector<vector<double> > pvaluepairs, vector<vector<double> > adjpvaluepairs,
    vector<QString> selected_files, QWidget *p, const char *)
    : QFrame(p) {
...
void US_Hydrodyn_Saxs_Cormap::cancel() { close(); }
...
bool US_Hydrodyn_Saxs_Cormap::streak_check(vector<int> &row1, vector<int> &row2,
...
void US_Hydrodyn_Saxs_Cormap_Cluster_Analysis::cluster_expand(
    uhs_index_pair x) {
  // check neighbors
...
bool US_Hydrodyn_Saxs_Cormap::cluster_analysis() {
  ...
  if (ca.run(pvaluepairs, parameters, csv_report, this)) {
    ...
    if (ca.cluster_size_histogram.size()) {
      ...
      {
        int use_line_width =
            parameters.count("linewidth") ? parameters["linewidth"].toInt() : 1;
...
bool US_Hydrodyn_Saxs_Cormap_Cluster_Analysis::sliding(
  ...
  if (pc != (int)pvaluepairs[0].size()) {
    errormsg = us_tr(
        "Internal error: cormap of brookes selected, but pvaluepairs not "
        "square");
    if (parent) {
  ...
  for (int i = start_size; i <= largest_window; ++i) {
    ...
    for (int j = 0; j <= pc - i; ++j) {
      // us_qdebug( QString( "start sliding analysis size %1 pos %2" ).arg( i
      // ).arg( j ) );
      for (int k = 0; k < i; ++k) {
        for (int l = 0; l < i; ++l) {
      ...
      if (parameters.count("hb")) {
        ...
        parameters["alpha"] =
            QString("%1").arg(US_Saxs_Util::holm_bonferroni(P, alpha));
        parameters["alpha_over_5"] =
            QString("%1").arg(US_Saxs_Util::holm_bonferroni(P, alpha_over_5));
      }
    ...
    if (!count) {
      errormsg = us_tr(
          "Brookes map sliding cluster analysis: internal error count is "
          "zero?");
      if (parent) {
        QMessageBox::warning(parent, parent->windowTitle(), errormsg);
      }
      ...
      for (map<QString, QString>::iterator it = csv_report.begin();
        ...
        if (count > 1) {
          ...
          sliding_results[this_tag2] =
              sqrt(countm1inv * (sliding_results[this_tag2] -
    ...
    for (int j = i + 1; j < pc; ++j) {
      ...
      if (!cluster_marked.count(x) && cluster_data[x] == -1) {
        ...
        cluster_size_histogram[(int)cluster_red.size()] =
            1 + (cluster_size_histogram.count((int)cluster_red.size())
                     ? cluster_size_histogram[(int)cluster_red.size()]
                     : 0);
      }
    }
        ...
        cluster_sizes.size()
            ...
            QString(us_tr("Red cluster count %1, average size %2 \u00b1%3 %4, "
                ...
                .arg(max_cluster_size &&
                         ...
                         ? QString(us_tr(" has %1 occurrence%2 and %3begins at "
                                         "[%4,%5]"))
                               .arg(cluster_size_histogram.rbegin()->second)
                               .arg(cluster_size_histogram.rbegin()->second > 1
                                        ? "s"
                                        : "")
                               .arg(cluster_size_histogram.rbegin()->second > 1
                                        ? "first occurrence "
...
void US_Hydrodyn_Saxs_Cormap::save_csv() {
  ...
  //                      functional" ), QMessageBox::Ok | QMessageBox::Default,
...
void US_Hydrodyn_Saxs_Cormap::load_csv() {
  ...
  QString filename =
      QFileDialog::getOpenFileName(this, windowTitle(), use_dir, "*.csv *.CSV");
...
bool US_Hydrodyn_Saxs_Cormap::load_csv(QString filename,
...
void US_Hydrodyn_Saxs_Cormap::csv_corrupt_msg(QString file, int line,
                                              QWidget *parent) {
  if (parent) {
    QMessageBox::warning(
        parent, parent->windowTitle() + " : Load CSV file",
        QString(
            us_tr("The CSV file %1 line %2: ended permaturely or is corrupted"))
            .arg(file)
            .arg(line + 1));
  }
}
...
void US_Hydrodyn_Saxs_Cormap::displayData() {
  ...
  if (parameters.count("as_pairs")) {
    ...
    msg += QString("").sprintf(
        ...
        " %5.1f%% red pairs\n",
        tot_c_pct * (double)(green_c + yellow_c), tot_c_pct * (double)green_c,
        tot_c_pct * (double)yellow_c, tot_c_pct * (double)red_c);
    ...
    if (!parameters.count("hide_adjpvalues")) {
      msg += QString("").sprintf(
          ...
          tot_c_pct * (double)adj_green_c, tot_c_pct * (double)adj_yellow_c,
    ...
    if (!parameters.count("hide_hb_pvalues")) {
      if (parameters.count("hb")) {
        msg += QString("").sprintf(
            ...
            tot_c_pct * (double)hb_green_c, tot_c_pct * (double)hb_yellow_c,
  ...
  if (false && parameters.count("cormap_of_brookes")) {
    ...
    if (pc != (int)pvaluepairs[0].size()) {
      QMessageBox::warning(this, windowTitle(),
                           us_tr("Internal error: cormap of brookes selected, "
                                 "but pvaluepairs not square"));
    } else {
      ...
      for (int i = 0; i < pc; ++i) {
        for (int j = 0; j < pc; ++j) {
          if (i != j) {
            rows[i].push_back(
                i == j ? 0 : (pvaluepairs[i][j] >= alpha_over_5 ? 1 : -1));
          }
        }
      ...
      for (int i = 0; !bad_streak && i < pc; ++i) {
        if (!streak_check(row0, rows[i], N, S, C, P)) {
          ...
          QMessageBox::warning(
              ...
              QString(
                  us_tr(
                      "Internal error: cormap of brookes got cormap error %1"))
                  .arg(((US_Hydrodyn *)us_hydrodyn)->saxs_util->errormsg));
          break;
        }
      ...
      if (pc > 2) {
        ...
        double pct_red_sd = sqrt(
            countm1inv * (sum2_pctred - countinv * avg_pctred * avg_pctred));
...
#if QT_VERSION < 0x050000
        cobheader +=
            QString("Contiguous red points average %1\% ±%2 %3 maximum %4\%\n")
#else
        cobheader +=
            QString(
                "Contiguous red points average %1\% \u00b1%2 %3 maximum %4\%\n")
#endif
                .arg(QString("").sprintf("%4.2f", avg_pctred))
                .arg(QString("").sprintf("%4.2f", pct_red_sd))
                .arg(avg_pctred > 0
                         ? QString("").sprintf("(%.1f%%)",
                                               100.0 * pct_red_sd / avg_pctred)
                         : QString(""))
  ...
  if (parameters.count("linewisesummary")) {
    ...
    if (sfs != (int)pvaluepairs.size() || sfs != (int)pvaluepairs[0].size()) {
      msg += QString(
                 "Internal error, cormap utility can't compute linewise "
                 "summary %1 != %2 or !%3\n")
                 .arg(sfs)
      ...
      if (sfs > 2) {
        ...
        avgP_sd =
            sqrt(countm1inv * (sum2_avgP - countinv * avg_avgP * avg_avgP));
        pct_red_sd = sqrt(
            countm1inv * (sum2_pct_red - countinv * avg_pct_red * avg_pct_red));
        ...
        msg +=
            QString(
                "\nAverage one-to-all P value %1 \u00b1%2 %3 \% red %4\% "
                "\u00b1%5 %6\n")
#endif
...
#if QT_VERSION < 0x040000
            plot->setCurvePen(curve,
                              QPen(Qt::green, use_line_width, Qt::DotLine));
...
#endif
          // connect( plot_zoomer, SIGNAL( zoomed( const QRectF & ) ), SLOT(
  ...
  if (parameters.count("save_csv")) {
    ...
    for (int i = 0; i < (int)csv_headers.size(); ++i) {
      out +=
          (i ? "," : "") +
          (csv_report.count(csv_headers[i]) ? csv_report[csv_headers[i]] : "");
    }
    out += "\n";
    ...
    if (parameters.count("global_width")) {
      ...
      if (!parameters.count("as_pairs")) {
        if (parameters.count("clusteranalysis")) {
          ...
          plot_cluster_geom.setWidth((parameters["global_width"].toInt() - 20) /
                                     4);
          plot_cluster->setGeometry(plot_geom);

          // plot->setMaximumWidth( ( parameters[ "global_width" ].toInt() - 20
          // ) / 4 ); plot->setMinimumWidth( ( parameters[ "global_width"
          // ].toInt() - 20 ) / 4 ); plot_cluster->setMaximumWidth( (
          // parameters[ "global_width" ].toInt() - 20 ) / 4 );
          // plot_cluster->setMinimumWidth( ( parameters[ "global_width"
          // ].toInt() - 20 ) / 4 );
        } else {
          QRect plot_geom = plot->geometry();
          ...
          plot->setGeometry(plot_geom);
          // plot->setMaximumWidth( ( parameters[ "global_width" ].toInt() - 20
          // ) / 2 ); plot->setMinimumWidth( ( parameters[ "global_width"
          // ].toInt() - 20 ) / 2 );
        }
      }
    }
...
void US_Hydrodyn_Saxs_Cormap::imageResized() {
...

  {
    ...
    if (parameters.count("as_pairs")) {
      ...
      if (cb_adj->isChecked()) {
        lbl_f_title->setText(parameters.count("ppvm_title_adj")
        ...
        if (cb_hb->isChecked()) {
          lbl_f_title->setText("Holm-Bonferroni adjusted\n" +
          ...
          pm.convertFromImage(
#if QT_VERSION >= 0x040000
              qi_hb->scaled(QSize(avail_i, avail_i), Qt::KeepAspectRatio)
#else
