...
US_Config::US_Config (QObject *parent) : QObject (parent)
{
#if QT_VERSION >= 0x040000
...
US_Config::US_Config (QString /* dummy */, QObject *parent) : QObject (parent)
{
...
bool
US_Config::write_default_colors ()
{
...
US_Config::setModelString ()
{
  ...
  // model 14
  m.push_back (
      us_tr ("User-defined self/Hetero-Association: A + B <=> AB, nA <=> An"));
  // model 15
  m.push_back (
      us_tr ("User-Defined Monomer-Nmer, some monomer is incompetent"));
  // model 16
  ...
  m.push_back (
      us_tr ("User-Defined Monomer-Nmer plus contaminant")); // model 19
}

bool
US_Config::col_exists ()
{
  QString version = "0.0";
  bool flag = false;
...
US_Config::setDefault ()
{
  ...
  // config_list.browser = "C:/Program Files (x86)/Internet
  // Explorer/iexplore.exe";
  config_list.browser
      = QSettings ("HKEY_CLASSES_ROOT\\http\\shell\\open\\command",
  ...
  if (ultrascan != "")
    ...
    {
      QString base
          = qApp->applicationDirPath ().remove (QRegExp ("/bin(|64)$"));
  ...
  if (ultrascan == "")
    {
      ...
      // message = us_tr( "The required environment variable ULTRASCAN is not
      // set.\n"
...
#ifndef MAC
  QString real_ultrascan = QDir::toNativeSeparators (
      qApp->applicationDirPath ().remove (QRegExp ("/bin(|64)$")));
  QString match
      = config_list.system_dir != real_ultrascan ? "different" : "the same";
      ...
      if (!w_config->write_config (config_list))
        {
          QMessageBox::warning (
              0, "Write error",
              "Error attemption to write new configuration file!");
          delete w_config;
          exit (1);
        }
      // QMessageBox::information( 0, "updated", "updated configuration to
      // reflect new program directory" );
    }
#endif
  return (true);
}
  ...
  if (moved[4])
    {
      ...
      */
      ...
      if (moved[0])
        config_list.archive_dir = home + "archive";
      if (moved[1])
        config_list.data_dir = home + "data";
      if (moved[2])
        config_list.html_dir = home + "reports";
      if (moved[3])
        config_list.result_dir = home + "results";
      US_Write_Config *w_config;
      w_config = new US_Write_Config (this);
