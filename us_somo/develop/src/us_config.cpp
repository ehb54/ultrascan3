...
void US_Config::setDefault() {
  ...
  config_list.tmp_dir = QDir::toNativeSeparators(config_list.root_dir + "tmp");
...
bool US_Config::read() {
  ...
  if (config_list.system_dir != real_ultrascan) {
    ...
    w_config = new US_Write_Config(this);
    if (!w_config->write_config(config_list)) {
      QMessageBox::warning(0, "Write error",
                           "Error attemption to write new configuration file!");
      delete w_config;
      exit(1);
    }
    // QMessageBox::information( 0, "updated", "updated configuration to reflect
...
void US_Config::move_files() {
  ...
  for (i = 0; i < 4; i++) {
    if (QFile(oldhome + oldfile[i]).exists() &&
      ...
      cout << "Moved " << oldhome + oldfile[i] << " to " << home + newfile[i]
           << endl;
    }
  }
