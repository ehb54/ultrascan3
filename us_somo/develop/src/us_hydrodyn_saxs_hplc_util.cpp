...
QStringList US_Hydrodyn_Saxs_Hplc::get_frames(QStringList files, QString head,
                                              QString tail) {
  QStringList result;
  ...
  result = files.replaceInStrings(QRegExp("^" + QRegExp::escape(head)), "")
               ...
               .replaceInStrings(QRegExp("^(\\d*_?\\d+)([^0-9_]|_[a-zA-Z]).*$"),
                                 "\\1");
...
double US_Hydrodyn_Saxs_Hplc::tot_intensity(QString &file, double q_min,
                                            double q_max) {
...
void US_Hydrodyn_Saxs_Hplc::set_selected(set<QString> &to_select,
                                         bool do_replot) {
  disable_updates = true;
  lb_files->clearSelection();
  for (int i = 0; i < (int)lb_files->count(); i++) {
...
void US_Hydrodyn_Saxs_Hplc::set_created_selected(set<QString> &to_select,
                                                 bool do_replot) {
  disable_updates = true;
...
bool US_Hydrodyn_Saxs_Hplc::constant_e(QString file, double e) {
  if (!f_qs.count(file)) {
    return false;
  }
...
int US_Hydrodyn_Saxs_Hplc::slice(const vector<double> &q,
...
double US_Hydrodyn_Saxs_Hplc::wyatt_errors(QString file, double start_q,
                                           double end_q) {
  if (!f_qs.count(file)) {
    return -1e0;
  }
...
double US_Hydrodyn_Saxs_Hplc::wyatt_errors(const vector<double> &q,
...
void US_Hydrodyn_Saxs_Hplc::cormap(map<QString, QString> &parameters) {
  ...
  double cormap_maxq =
      ((US_Hydrodyn *)us_hydrodyn)->gparams.count("hplc_cormap_maxq")
          ? ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_cormap_maxq"].toDouble()
          : 0.05;
...
QStringList US_Hydrodyn_Saxs_Hplc::vector_qstring_to_qstringlist(
    vector<QString> vqs) {
  QStringList qsl;
...
void US_Hydrodyn_Saxs_Hplc::simulate() {
  ...
  US_Hydrodyn_Saxs_Hplc_Simulate *uhshs =
      new US_Hydrodyn_Saxs_Hplc_Simulate(us_hydrodyn, this, selected, q, I, e);
  uhshs->show();
}
...
void US_Hydrodyn_Saxs_Hplc::gauss_mode() {
  ...
  parameters["gaussian_type"] = QString("%1").arg(gaussian_type);
  US_Hydrodyn_Saxs_Hplc_Gauss_Mode *sho = new US_Hydrodyn_Saxs_Hplc_Gauss_Mode(
      &parameters, (US_Hydrodyn *)us_hydrodyn, this);
...
void US_Hydrodyn_Saxs_Hplc::reset_saxs_hplc_params() {
  ...
  saxs_hplc_param_solvent_electron_density =
      QString("%1")
          .arg(((US_Hydrodyn *)us_hydrodyn)->saxs_options.water_e_density, 0,
               'f', 4)
          .toDouble();
}
