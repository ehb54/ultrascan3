...
void US_Hydrodyn_Saxs_Buffer::setupGUI() {
  ...
  rb_np_ignore =
      new QRadioButton(us_tr("Ignore (log of non-positive not defined)"), this);
  ...
  cb_multi_sub->setText(
      us_tr(" Subtract buffer from every selected file (exepting set buffer "
            "and set blank) "));
...
void US_Hydrodyn_Saxs_Buffer::cancel() { close(); }
...
void US_Hydrodyn_Saxs_Buffer::table_value(int row, int col) {
  ...
  if (col == 2 || col == 3 || col == 5 || col == 6) {
    if (!t_csv->item(row, col)->text().isEmpty() &&
        t_csv->item(row, col)->text() !=
            QString("%1").arg(t_csv->item(row, col)->text().toDouble())) {
  ...
  if (col == 4) {
    if (!t_csv->item(row, col)->text().isEmpty() &&
        t_csv->item(row, col)->text() !=
            QString("%1").arg(t_csv->item(row, col)->text().toUInt())) {
      t_csv->setItem(row, col,
                     new QTableWidgetItem(QString("%1").arg(
...
void US_Hydrodyn_Saxs_Buffer::run_one(bool do_plot) {
  ...
  if (!current_files.count(solution)) {
    editor_msg(
        "red",
        QString(us_tr("Error: no data found for solution %1")).arg(solution));
    return;
  }
  ...
  if (f_qs[buffer].size() != f_qs[solution].size() ||
    ...
    editor_msg("red", us_tr("Error: incompatible grids, the files selected do "
  ...
  for (unsigned int j = 0; j < f_Is[buffer].size(); j++) {
    if (fabs(bsub_q[j] - f_qs[buffer][j]) > Q_VAL_TOL ||
      ...
      editor_msg("red", us_tr("Error: incompatible grids, the q values differ "
  ...
  if ((use_alpha && t_csv->item(0, 6)->text().isEmpty()) ||
    ...
    editor_msg(
        "red",
        us_tr("Internal error: method selected does not have current values"));
    return;
  }
  ...
  if (buffer_has_errors && !solution_has_errors) {
    editor_msg(
        "dark red",
        us_tr("Warning: the buffer has errors defined but not the solution"));

    bsub_error = f_errors[buffer];
  } else {
    if (!buffer_has_errors && !solution_has_errors && empty_has_errors) {
      // this is a strange case
      editor_msg("dark red", us_tr("Warning: the blank has errors defined but "
                                   "not the solution or buffer!"));
  ...
  if (any_negative) {
    ...
    if (rb_np_crop->isChecked()) {
      editor_msg(
          "dark red",
          QString(
              us_tr("Warning: non-positive values caused cropping at q = %1"))
    ...
    if (rb_np_min->isChecked()) {
      editor_msg(
          ...
          QString(
              us_tr(
                  "Warning: non-positive values caused %1 minimum values set"))
              .arg(negative_count));
      result = 1;
    }
    ...
    if (rb_np_ask->isChecked()) {
      result = QMessageBox::warning(
          ...
          QString(
              ...
              .arg(negative_count)
              .arg(bsub_q[negative_pos]),
          us_tr("&Crop the data"), us_tr("&Set to the minimum positive value"),
    ...
    switch (result) {
      case 0:  // crop
      {
        if (negative_pos < 2) {
          if (rb_np_ask->isChecked()) {
            QMessageBox::warning(
                this, title, us_tr("Insufficient data left after cropping"));
          } else {
...
void US_Hydrodyn_Saxs_Buffer::update_enables() {
  ...
  pb_run_best->setEnabled(!running &&
                          (!lbl_signal->text().isEmpty() ||
                           (cb_multi_sub->isChecked() &&
                            non_buffer_non_empty_files_selected_count > 0)) &&
                          !lbl_buffer->text().isEmpty() && !any_best_empty &&
                          any_selected);
...
void US_Hydrodyn_Saxs_Buffer::plot_files() {
  ...
  for (int i = 0; i < lb_files->count(); i++) {
    if (lb_files->item(i)->isSelected()) {
      // any_selected = true;
      if (plot_file(lb_files->item(i)->text(), file_minx, file_maxx, file_miny,
                    file_maxy)) {
        if (first) {
          minx = file_minx;
          maxx = file_maxx;
...
bool US_Hydrodyn_Saxs_Buffer::plot_file(QString file, double &minx,
  ...
  if (!axis_y_log) {
    ...
    plot_dist->setCurvePen(
        Iq, QPen(plot_colors[f_pos[file] % plot_colors.size()], 1, SolidLine));
#else
    curve->setSamples(
    ...
    curve->setPen(
        QPen(plot_colors[f_pos[file] % plot_colors.size()], 1, Qt::SolidLine));
...
bool US_Hydrodyn_Saxs_Buffer::get_min_max(QString file, double &minx,
                                          double &maxx, double &miny,
                                          double &maxy) {
  if (!f_qs_string.count(file) || !f_qs.count(file) || !f_Is.count(file) ||
      !f_pos.count(file)) {
...
void US_Hydrodyn_Saxs_Buffer::avg(QStringList files) {
  ...
  for (int i = 0; i < (int)avg_qs.size(); i++) {
    ...
    // sum_weight2[ i ] );
    avg_sd[i] = sqrt(sum / (((double)files.size() - 1e0) *
...
QString US_Hydrodyn_Saxs_Buffer::qstring_common_head(QString s1, QString s2) {
...
QString US_Hydrodyn_Saxs_Buffer::qstring_common_tail(QString s1, QString s2) {
  int min_len = (int)s1.length();
  if (min_len > (int)s2.length()) {
    min_len = (int)s2.length();
  }
...
bool US_Hydrodyn_Saxs_Buffer::save_file(QString file, bool &cancel,
                                        bool &overwrite_all) {
  if (!f_qs.count(file)) {
...
void US_Hydrodyn_Saxs_Buffer::rename_created(QListWidgetItem *lbi,
                                             const QPoint &) {
  map<QString, bool> existing_items;
  for (int i = 0; i < (int)lb_files->count(); i++) {
    existing_items[lb_files->item(i)->text()] = true;
...
void US_Hydrodyn_Saxs_Buffer::add_plot(QString name, vector<double> q,
                                       vector<double> I) {
...
void US_Hydrodyn_Saxs_Buffer::wheel_cancel() {
  ...
  if (plotted_curves.count(wheel_file) && f_pos.count(wheel_file)) {
    ...
    plot_dist->setCurvePen(
        ...
        QPen(plot_colors[f_pos[wheel_file] % plot_colors.size()], 1,
             SolidLine));
#else
    plotted_curves[wheel_file]->setPen(QPen(
        plot_colors[f_pos[wheel_file] % plot_colors.size()], 1, Qt::SolidLine));
#endif
  }
  ...
  if (plotted_curves.count(join_file) && f_pos.count(join_file)) {
    ...
    plot_dist->setCurvePen(
        ...
        QPen(plot_colors[f_pos[join_file] % plot_colors.size()], 1, SolidLine));
#else
    plotted_curves[join_file]->setPen(QPen(
        plot_colors[f_pos[join_file] % plot_colors.size()], 1, Qt::SolidLine));
#endif
  }
...
void US_Hydrodyn_Saxs_Buffer::join_enables() {
  ...
  qwtw_wheel->setEnabled(le_join_offset->hasFocus() ||
                         le_join_mult->hasFocus() ||
                         le_join_start->hasFocus() ||
                         le_join_point->hasFocus() || le_join_end->hasFocus());
}
...
void US_Hydrodyn_Saxs_Buffer::join_set_wheel_range() {
  ...
  for (unsigned int i = 0; i < (unsigned int)f_qs[join_file].size(); i++) {
    if (f_qs[join_file][i] >= join_low_q && f_qs[join_file][i] <= join_high_q) {
...
void US_Hydrodyn_Saxs_Buffer::join_add_marker(double pos, QColor color,
                                              QString text,
#if QT_VERSION < 0x040000
                                              int
#else
  ...
  if (join_adjust_lowq) {
    ...
    for (unsigned int i = 0; i < (unsigned int)f_qs[wheel_file].size(); i++) {
      if (f_qs[wheel_file][i] >= join_start &&
        ...
        if (use_errors) {
          // which one's errors to use?, what about compatible grids etc?
          // nwheel_y.push_back( ( mult * f_Is[ wheel_file ][ i ] + offset ) /
          // f_errors[ wheel_file ][ i ] );
          wheel_e.push_back(f_errors[wheel_file][i]);
        }
      }
  ...
  if (le_join_offset->text().toDouble() != 0e0) {
    QMessageBox::information(
        ...
        us_tr(
            ...
            "which is thus swept under the carpet but could lead to erroneous "
            "interpretation."));
  }
...
void US_Hydrodyn_Saxs_Buffer::asum(QStringList files) {
  ...
  for (int i = 0; i < (int)files.size(); i++) {
    ...
    bool ok;
    double res = US_Static::getDouble(title + us_tr(": Weighted sum"), txt, 1,
                                      1e-5, 1e8, 5, &ok, this);
    if (ok) {
      // user entered something and pressed OK
      mults.push_back(res);
  ...
  for (int i = 0; i < (int)files.size(); i++) {
    ...
    if (first) {
      ...
      avg_conc = concs.count(this_file) ? mults[i] * concs[this_file] : 0e0;
      avg_psv = f_psv.count(this_file) ? f_psv[this_file] : 0e0;
      avg_I0se = f_I0se.count(this_file) ? f_I0se[this_file] : 0e0;
      ...
      for (int j = 0; j < (int)t_Is[this_file].size(); j++) {
        ...
        avg_Is[j] += t_Is[this_file][j];
        if (j < (int)t_errors[this_file].size() && j < (int)avg_sd.size()) {
...
void US_Hydrodyn_Saxs_Buffer::select_these(map<QString, QString> &parameters,
                                           bool reenable) {
  disable_updates = true;
  lb_files->clearSelection();
  for (int i = 0; i < lb_files->count(); ++i) {
...
double US_Hydrodyn_Saxs_Buffer::tot_intensity(QString &file, double q_min,
                                              double q_max) {
  if (!f_qs_string.count(file) || !f_qs.count(file) || !f_Is.count(file) ||
      !f_pos.count(file)) {
    editor_msg(
