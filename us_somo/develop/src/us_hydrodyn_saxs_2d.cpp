...
vector<vector<double> > bulatov_main(int N, int Nstep) {
  ...
  // format to stdout\n"); fprintf(stderr,"example of usage: points_on_sphere 10
...
void US_Hydrodyn_Saxs_2d::setupGUI() {
  ...
  le_detector_pixels_height->setObjectName("Detector_Pixels_Height Line Edit");
  ...
  le_detector_pixels_height->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  le_beam_center_pixels_height->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  connect(le_beam_center_pixels_height, SIGNAL(textChanged(const QString &)),
  ...
  le_beam_center_pixels_width->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
...
void US_Hydrodyn_Saxs_2d::cancel() { close(); }
...
#else
  pm.convertFromImage(
      i_2d.scaled(i_2d.width() > US_SAXS_2D_PIXMAX
                      ? US_SAXS_2D_PIXMAX
                      : (i_2d.width() < US_SAXS_2D_PIXMIN ? US_SAXS_2D_PIXMIN
                                                          : i_2d.width()),
                  i_2d.height() > US_SAXS_2D_PIXMAX
...
void US_Hydrodyn_Saxs_2d::start() {
  ...
  for (unsigned int r = 0; r < rotations.size(); r++) {
    ...
    if (rotations.size() > 1) {
      ...
      if (cb_save_pdbs->isChecked()) {
        ...
        if (ok_to_write) {
          ...
          ts << QString("MODEL     %1\n").arg(r + 1);
          ts << QString(
                    "REMARK    Axis for rotation from original ( %1 , %2 , %3 "
                    ")\n")
                    .arg(rotations[r][0])
                    .arg(rotations[r][1])
                    .arg(rotations[r][2]);
          ...
          editor_msg("blue", QString(us_tr("Added rotated model %1 to %2"))
                                 .arg(r + 1)
                                 .arg(fname));
          // cout << QString( us_tr( "Added rotated model %1 to %2" ) ).arg( r +
          // 1 ).arg( fname ) << endl;
        }
      }
    }
    ...
    for (unsigned int a = 0; a < atoms.size(); a++) {
      ...
      editor_msg(
          "gray",
          QString(us_tr("Computing atom %1\n")).arg(atoms[a].hybrid_name));
      ...
      for (unsigned int i = 0; i < data.size(); i++) {
        for (unsigned int j = 0; j < data[0].size(); j++) {
          ...
          pixpos[1] =
              (double)j * detector_height_per_pixel - beam_center_height;
          ...
          Q[2] =
              2.0 * M_PI *
              (detector_distance - (double)atoms[a].pos[2]);  // ?? I had 1 here
          ...
          //                   q_2_over_4pi ) + saxs.a[ 3 ] * exp( -saxs.b[ 3 ]
          //                   * q_2_over_4pi ) + atoms[ a ].hydrogens * (
          //                   saxsH.c +
          //                     saxsH.a[ 0 ] * exp( -saxsH.b[ 0 ] *
          //                     q_2_over_4pi ) + saxsH.a[ 1 ] * exp( -saxsH.b[
          //                     1 ] * q_2_over_4pi ) + saxsH.a[ 2 ] * exp(
          //                     -saxsH.b[ 2 ] * q_2_over_4pi ) + saxsH.a[ 3 ] *
          //                     exp( -saxsH.b[ 3 ] * q_2_over_4pi ) );
...
void US_Hydrodyn_Saxs_2d::update_detector_pixels_height(
    const QString & /* str */) {
  reset_2d();
}

void US_Hydrodyn_Saxs_2d::update_detector_pixels_width(
    const QString & /* str */) {
  reset_2d();
}
...
#if QT_VERSION < 0x040000
    pm.convertFromImage(i_2d.smoothScale(
        i_2d.width() > US_SAXS_2D_PIXMAX ? US_SAXS_2D_PIXMAX : i_2d.width(),
        i_2d.height() > US_SAXS_2D_PIXMAX ? US_SAXS_2D_PIXMAX : i_2d.height(),
        Qt::KeepAspectRatio));
#else
...
void US_Hydrodyn_Saxs_2d::update_beam_center_pixels_height(
    const QString & /* str */) {}

void US_Hydrodyn_Saxs_2d::update_beam_center_pixels_width(
    const QString & /* str */) {}
...
void US_Hydrodyn_Saxs_2d::report_variables() {
  ...
  editor_msg("black", QString(us_tr("Pixel zero, zero (top, left!) is at %1 m, "
                                    "%2 m from beam center"))
  ...
  editor_msg("black", QString(us_tr("q of pixel 0, %1: %2 (1/A)\n"))
                          .arg(detector_pixels_width - 1)
                          .arg(q_of_pixel((int)0, detector_pixels_width - 1)));

  editor_msg("black", QString(us_tr("q of pixel %1, 0: %2 (1/A)\n"))
...
void US_Hydrodyn_Saxs_2d::set_pos(unsigned int i) {
  ...
  lbl_wheel_pos->setText(QString("%1 of %2").arg(i).arg(data_stack.size()));
