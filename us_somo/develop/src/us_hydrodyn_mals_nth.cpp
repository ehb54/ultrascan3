...
US_Hydrodyn_Mals_Nth::US_Hydrodyn_Mals_Nth (void *us_hydrodyn_mals,
    ...
    : QDialog (p)
{
  ...
  this->ggaussian_last_pfit_map
      = &((US_Hydrodyn_Mals *)us_hydrodyn_mals)->ggaussian_last_pfit_map;
...
void
US_Hydrodyn_Mals_Nth::setupGUI ()
{
  bool started_in_expert_mode
      = ((US_Hydrodyn_Mals *)us_hydrodyn_mals)->started_in_expert_mode;
  ...
  for (int i = 0;
       i < ((US_Hydrodyn_Mals *)us_hydrodyn_mals)->lb_files->count (); ++i)
    {
      lb_files->addItem (QString ("%1 : %2").arg (i + 1).arg (
          ((US_Hydrodyn_Mals *)us_hydrodyn_mals)
              ->lb_files->item (i)
              ->text ()));
      if (((US_Hydrodyn_Mals *)us_hydrodyn_mals)
              ->lb_files->item (i)
              ->isSelected ())
        {
                        ...
                        USglobal->config_list.fontSize));
  {
    QIntValidator *qdv = new QIntValidator (
        1, ((US_Hydrodyn_Mals *)us_hydrodyn_mals)->lb_files->count (), le_n);
  ...
  {
    QIntValidator *qdv = new QIntValidator (
        1, ((US_Hydrodyn_Mals *)us_hydrodyn_mals)->lb_files->count (),
        le_start);
  ...
  lbl_start_name
      = new QLabel (((US_Hydrodyn_Mals *)us_hydrodyn_mals)
  ...
  le_end->setText (QString ("%1").arg (
      ((US_Hydrodyn_Mals *)us_hydrodyn_mals)->lb_files->count ()));
  ...
  {
    QIntValidator *qdv = new QIntValidator (
        2, ((US_Hydrodyn_Mals *)us_hydrodyn_mals)->lb_files->count (), le_end);
  ...
  le_contains->setText (parameters->count ("mals_nth_contains")
...
void
US_Hydrodyn_Mals_Nth::quit ()
{
  close ();
}

void
US_Hydrodyn_Mals_Nth::go ()
{
  (*parameters)["go"] = "true";
...
void
US_Hydrodyn_Mals_Nth::closeEvent (QCloseEvent *e)
{
...
void
US_Hydrodyn_Mals_Nth::update_enables ()
{
  ...
  if (ofs >= 1 && ofs <= fcnt)
    {
      lbl_start_name->setText (((US_Hydrodyn_Mals *)us_hydrodyn_mals)
                                   ->lb_files->item (ofs - 1)
                                   ->text ());
    }
  else
  ...
  if (end >= 1 && end <= fcnt)
    {
      lbl_end_name->setText (((US_Hydrodyn_Mals *)us_hydrodyn_mals)
                                 ->lb_files->item (end - 1)
                                 ->text ());
    }
  else
  ...
  for (int i = 0; i < lb_files->count (); ++i)
    {
      ...
      {
        QString qs = ((US_Hydrodyn_Mals *)us_hydrodyn_mals)
                         ->lb_files->item (i)
                         ->text ();
...
void
US_Hydrodyn_Mals_Nth::contains_only ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  lb_files->clearSelection ();
  for (int i = 0; i < lb_files->count (); ++i)
...
void
US_Hydrodyn_Mals_Nth::contains_add ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  for (int i = 0; i < lb_files->count (); ++i)
    {
...
void
US_Hydrodyn_Mals_Nth::pvalues_only ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  lb_files->clearSelection ();
  for (int i = 0; i < lb_files->count (); ++i)
...
void
US_Hydrodyn_Mals_Nth::pvalues_add ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  for (int i = 0; i < lb_files->count (); ++i)
    {
...
void
US_Hydrodyn_Mals_Nth::update_files_selected ()
{
  lb_files_sel->clear ();
  for (int i = 0; i < lb_files->count (); ++i)
    {
