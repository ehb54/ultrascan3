...
void US_Hydrodyn_Mals::guinier_add_marker(QwtPlot *plot, double pos,
...
void US_Hydrodyn_Mals::guinier_analysis() {
  ...
  for (int i = 0; i < (int)guinier_names.size(); ++i) {
    ...
    do {
      ...
      unsigned int pend =
          usu->wave["mals"].q.size() ? usu->wave["mals"].q.size() - 1 : 0;

      computed_rg = usu->guinier_plot("malsrg", "mals") &&
...
void US_Hydrodyn_Mals::guinier_range(double minq2, double maxq2, double minI,
                                     double maxI) {
...
bool US_Hydrodyn_Mals::guinier_check_qmax(QString &report, bool show_message) {
  report = "";
