...
bool US_Hydrodyn_Dad::create_i_of_q_ng( set < QString > & fileset, double t_min, double t_max )
{
   QStringList files;
...
bool US_Hydrodyn_Dad::create_i_of_q_ng( QStringList files, double t_min, double t_max )
{
   ...
   for ( unsigned int i = 0; i < ( unsigned int ) files.size(); i++ )
   {
      ...
      QRegularExpressionMatch rx_blc_m = rx_blc.match( files[ i ] );
      if ( rx_blc_m.hasMatch() )
      {
         any_blc = true;
      }
...
bool US_Hydrodyn_Dad::create_i_of_q( set < QString > & fileset, double t_min, double t_max )
{
   QStringList files;
   for ( set < QString >::iterator it = fileset.begin();
         it != fileset.end();
...
bool US_Hydrodyn_Dad::create_i_of_q( QStringList files, double t_min, double t_max )
{
   // us_qdebug( "ciq 0" );
   // for each selected file
   // extract q grid from file names
   ...
   if ( !mode_testiq )
   {
      QFile f( ((US_Hydrodyn *)us_hydrodyn)->somo_dir + QDir::separator() + "saxs" + QDir::separator() + "tmp" +  QDir::separator() + "dad_frac.csv" );
...

   {
      double cutoff_frac = 
         ...
         ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "dad_makeiq_cutmax_pct" ].toDouble() * 0.01 :
         0e0;
   ...
   {
      avg_peaks = ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "dad_cb_makeiq_avg_peaks" ] == "true";

      if ( avg_peaks ) {
         double peak_frac = 
            1e0 - ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "dad_makeiq_avg_peaks" ].toDouble() * 0.01;
   ...
   if ( avg_peaks ) {
      QString peak_tag = QString( "co%1_" ).arg( ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "dad_makeiq_avg_peaks" ] );
...
bool US_Hydrodyn_Dad::create_unified_ggaussian_target( bool do_init, bool only_init_unset )
{
   QStringList files = all_selected_files();
   return create_unified_ggaussian_target( files, do_init, only_init_unset );
}

bool US_Hydrodyn_Dad::create_unified_ggaussian_target( QStringList & files, bool do_init, bool only_init_unset )
{
...
bool US_Hydrodyn_Dad::compute_f_gaussians( QString file, QWidget *dad_fit_widget )
{
   ...
   if ( ( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "dad_cb_gg_smooth" )
      ...
      int smoothing = ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "dad_gg_smooth" ].toInt();
...
bool US_Hydrodyn_Dad::initial_ggaussian_fit( QStringList & files, bool only_init_unset ) {
   wheel_file = files[ 0 ];
...
void US_Hydrodyn_Dad::add_plot_gaussian( const QString &file, const QString &tag ) {
   add_plot(
...
bool US_Hydrodyn_Dad::compute_f_gaussians_trial( QString file, QWidget *dad_fit_widget ) {
...
void US_Hydrodyn_Dad::add_smoothed(
                                         const QString            & name
...
void US_Hydrodyn_Dad::add_oldstyle(
                                         const QString            & name
                                         ,const vector < double > & q
                                         ,const vector < double > & I
                                         ) {
...
bool US_Hydrodyn_Dad::create_ihash_t( QStringList files ) {
   ...
   double K =
      (
       4
       * M_PI * M_PI
       * dad_param_n * dad_param_n
       * dad_param_g_dndc * dad_param_g_dndc
       ) / (
            AVOGADRO
...
bool US_Hydrodyn_Dad::create_istar_q( QStringList /* files */, double /* t_min */, double /* t_max */ ) {
...
bool US_Hydrodyn_Dad::create_istar_q_ng( QStringList files, double t_min, double t_max ) {
      ...
      if ( 1 == QMessageBox::question(this, 
         ...
         QMessageBox::critical( this,
                                ...
                                us_tr(
                                      ...
                                      "the global concentration must be defined in Options->DAD parameters"
                                      )
                                );
         return false;
      }
   ...
   if ( !mode_testiq ) {
      ...
      if ( use_conc ) {
         ...
         parameters[ "ngmode" ] = "true";
         parameters[ "gaussians" ] = "1";
         parameters[ "default_extc" ] = QString( "%1" ).arg( dad_param_g_extinction_coef );
