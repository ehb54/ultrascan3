...
bool US_Hydrodyn_Dad::create_i_of_q_ng(set<QString> &fileset, double t_min,
                                       double t_max) {
  QStringList files;
...
bool US_Hydrodyn_Dad::create_i_of_q_ng(QStringList files, double t_min,
                                       double t_max) {
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size(); i++) {
    ...
    if (rx_q.indexIn(files[i]) == -1) {
      editor_msg("red",
                 QString(us_tr("Error: Can not find " + UNICODE_LAMBDA_QS +
    ...
    if (used_q.count(ql.back())) {
      editor_msg("red", QString(us_tr("Error: Duplicate " + UNICODE_LAMBDA_QS +
                                      " value in file name for %1"))
                            .arg(files[i]));
      progress->reset();
      update_enables();
    ...
    if (!f_qs.count(files[i])) {
      ...
      for (unsigned int j = 0; j < (unsigned int)f_qs[files[i]].size(); j++) {
        if (!mode_testiq ||
            (f_qs[files[i]][j] >= t_min && f_qs[files[i]][j] <= t_max)) {
          ...
          if (use_errors &&
            ...
            if (use_errors) {
              use_errors = false;
              editor_msg("dark red",
                         QString(us_tr("Notice: missing errors, first noticed "
                                       "in %1, so no errors at all"))
                             .arg(files[i]));
            }
          }
      ...
      if (!f_errors.count(files[i]) ||
        ...
        if (!is_nonzero_vector(f_errors[files[i]])) {
          ...
          for (unsigned int j = 0; j < (unsigned int)f_errors[files[i]].size();
               j++) {
            if (!mode_testiq ||
                (f_qs[files[i]][j] >= t_min && f_qs[files[i]][j] <= t_max)) {
  ...
  if (!mode_testiq) {
    ...
    if (use_conc && (!f_qs.count(lbl_conc_file->text()) ||
                     !f_Is.count(lbl_conc_file->text()))) {
      editor_msg(
          "red",
          QString(
              us_tr("Internal error: Concentration file %1 set, but associated "
...

      {
        ...
        if (rx_repeak.indexIn(lbl_conc_file->text()) != -1) {
          ...
          if (conc_repeak == 0e0) {
            ...
            editor_msg(
                "dark blue",
                QString(us_tr("Notice: concentration scaling repeak value %1"))
                    .arg(conc_repeak));
          }
        }
      }
  ...
  for (unsigned int t = 0; t < tv.size(); t++) {
    ...
    if (conc_ok) {
      if (!usu->apply_natural_spline(conc_spline_x, conc_spline_y,
        ...
        editor_msg("red",
                   QString(us_tr("Error getting concentration from spline for "
                                 "frame %1, concentration set to zero."))
                       .arg(tv[t]));
        conc_factor = 0e0;
      }
    }
    ...
    for (unsigned int i = 0; i < (unsigned int)files.size(); i++) {
      ...
      if (!I_values[tv[t]].count(qv[i])) {
        editor_msg(
            "red",
            QString(us_tr("Notice: I values missing q = %1")).arg(qv[i]));
        continue;
      }
...
bool US_Hydrodyn_Dad::create_i_of_q(set<QString> &fileset, double t_min,
                                    double t_max) {
  QStringList files;
  for (set<QString>::iterator it = fileset.begin(); it != fileset.end(); ++it) {
    files << *it;
...
bool US_Hydrodyn_Dad::create_i_of_q(QStringList files, double t_min,
                                    double t_max) {
  // us_qdebug( "ciq 0" );
  // for each selected file
  // extract q grid from file names
  ...
  if (!ggaussian_compatible(false)) {
    editor_msg(
        "red",
        us_tr("NOTICE: Some files selected have Gaussians with varying centers "
              "or a different number of Gaussians or centers, Please enter "
              "\"Global Gaussians\" with these files selected and then "
              "\"Keep\" before pressing \"Make I(q)\""));
    return false;
  }
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size(); i++) {
    ...
    if (rx_bl.indexIn(files[i]) == -1) {
      ...
      // ).arg( bl_slope.back(), 0, 'g', 8 ).arg( bl_intercept.back(), 0, 'g', 8
      // ).toLatin1().data();
      bl_count++;
      any_bl = true;
    }
    ...
    if (rx_bi.indexIn(files[i]) == -1) {
      ...
      // bi_delta.back(), 0, 'g', 8 ).arg( bi_alpha.back(), 0, 'g', 8
      // ).toLatin1().data();
  ...
  if (mode_testiq) {
    ...
    if (bl_count) {
      parameters["baseline"] =
          QString(us_tr("Add back the baselines when making I(q).  Baselines "
                        "were found for %1 of the %2 curves"))
              .arg(bl_count)
              .arg(files.size());
    }
    ...
    if (lbl_conc_file->text().isEmpty()) {
      parameters["error"] = QString(
          us_tr("Concentration controls disabled: no concentration file set"));
      no_conc = true;
    } else {
      if (!f_gaussians.count(lbl_conc_file->text())) {
        parameters["error"] =
            QString(us_tr("Concentration controls disabled: no Gaussians "
                          "defined for concentration file"));
        no_conc = true;
      } else {
        if (f_gaussians[lbl_conc_file->text()].size() / gaussian_type_size !=
            f_gaussians[files[0]].size() / gaussian_type_size) {
          parameters["error"] =
              QString(us_tr("Concentration controls disabled: Concentration "
                            "file Gaussian count (%1)\n does not match global "
                            "curves Gaussian count (%2)"))
    ...
    if (!no_conc) {
      ...
      if (rx_repeak.indexIn(lbl_conc_file->text()) != -1) {
        ...
        if (conc_repeak == 0e0) {
          conc_repeak = 1e0;
          editor_msg("red", us_tr("Error: concentration repeak scaling value "
                                  "extracted is 0, turning off back scaling"));
        } else {
          editor_msg(
              "dark blue",
        ...
        editor_msg(
            "dark red",
            us_tr("Notice: no concentration repeak scaling value found"));
      }
    } else {
      TSO << "no conc\n";
    }
    ...
    if (bl_count && (!parameters.count("add_baseline") ||
    ...
    if (!no_conc) {
      for (unsigned int i = 0;
           i < (unsigned int)f_gaussians[files[0]].size() / gaussian_type_size;
           i++) {
  ...
  //         QMessageBox::question(this,
  //                               this->windowTitle(),
  //                               us_tr( "Save as Gaussians or a percent of the
  //                               original I(q)?" ), us_tr( "&Gaussians" ),
  ...
  if (conc_ok && conc_to_saxs) {
    ...
    for (unsigned int i = 0; i < (unsigned int)conv.size(); i++) {
      ...
      QString qs =
          QString(us_tr("Concentration Gaussian %1: center %2 area %3"))
              .arg(i + 1)
              .arg(tmp_gc[1])
              .arg(conc_area);
      ...
      add_plot(QString(lbl_conc_file->text() + "_sas_adjusted_pk%1").arg(i + 1),
               tv, compute_gaussian(tv, tmp_gn), true, false);
    }
    // us_qdebug( US_Vector::qs_vector3( "conc, ref, new conc", conc_gaussians,
    // ref_gaussians, new_conc_gaussians ) );
  ...
  if (normalize_by_conc || conc_ok) {
    ...
    for (unsigned int i = 0; i < (unsigned int)conv.size(); i++) {
      ...
      // conc_curve( tv, i, conv[ i ] ), true, false ); alt method
      {
        ...
        tmp_g[0] = conc_gaussians[0 + i * gaussian_type_size] * detector_conv /
                   (conc_repeak * conv[i]);
        ...
        add_plot(QString("conc_mg_per_ml_peak%1").arg(i + 1), tv, concs.back(),
                 true, false);
      }
    }
  }
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size(); i++) {
    ...
    for (unsigned int j = 0; j < (unsigned int)f_gaussians[files[i]].size();
      ...
      // US_Vector::printvector( QString( "file %1 gaussian %2 params" ).arg(
      // files[ i ] ).arg( 1 + ( j / gaussian_type_size ) ), tmp_g );
    ...
    // US_Vector::printvector( QString( "areas file %1 (sum %2)" ).arg( i ).arg(
    // tmp_area_sum, 0, 'g', 8 ), tmp_area ); add_plot( QString( "fg_%1_gsum"
    // ).arg( i ), tv, tmp_sum, true, false );
  }
  ...
  if (mode_testiq && (unsigned int)rb_testiq_gaussians.size() != num_of_gauss) {
    editor_msg(
        "red",
        QString(
            us_tr("Test I(q) internal error: Gaussian count mismatch %1 %2"))
  ...
  if (save_sum && any_bi) {
    ...
    for (int q = 0; q < (int)qv.size(); ++q) {
      ...
      if (!bi_alpha.count(qv[q])) {
        ...
        editor_msg(
            "red",
            QString(us_tr("Warning: could not find alpha value for integral "
                          "baseline reconstruction for q %1 A^-1"))
                .arg(qv[q]));
        bi_alpha[qv[q]] = 0e0;
      }
      ...
      for (int t = 0; t < (int)tv.size(); ++t) {
        ...
        if (!I_values[tv[t]].count(qv[q])) {
          // // us_qdebug( QString( "I_values[%1][%2] missing" ).arg( tv[ t ]
          // ).arg( qv[ q ] ) );
          continue;
        }
      ...
      //         .arg( bl )
      //         .arg( QString( bi_delta.count( qv[ q ] ) ? QString( "%1" ).arg(
      //         bi_delta[ qv[ q ] ] ) : QString( "na" ) ) ) .arg(
...

  {
    ...
    if (avg_peaks) {
      double peak_frac = 1e0 - ((US_Hydrodyn *)us_hydrodyn)
      ...
      if (peak_frac > 0e0) {
        for (unsigned int g = 0; g < num_of_gauss; g++) {
          ...
          editor_msg(
              "dark blue",
              QString(
                  us_tr("Cutoff for peak averaging of Gaussian %1 is [%2,%3]"))
                  .arg(g + 1)
                  .arg(tcpeak_min[g])
                  .arg(tcpeak_max[g]));
        }
  ...
  for (unsigned int t = 0; t < tv.size(); t++) {
    ...
    for (unsigned int g = 0; g < num_of_gauss; g++) {
      ...
      // us_qdebug( QString( "ciq create g loop t %1 g %2 go on" ).arg( t ).arg(
      // g + 1 ) ); build up an I(q)
      double conc_factor = 0e0;
      ...
      if ((conc_ok || normalize_by_conc) && concs[g][t] > 0e0) {
        ...
        vector<double> tmp_g(gaussian_type_size);
        tmp_g[0] = conc_gaussians[0 + g * gaussian_type_size] * detector_conv /
                   (conc_repeak * conv[g]);
        ...
        //             TSO << QString( "peak %1 center %2 fwhm %3 t %4 tv[t]
        //             %5\n" )
        //                .arg( g + 1 )
        //                .arg( center )
        //                .arg( fwhm )
      ...
      for (unsigned int i = 0; i < (unsigned int)files.size(); i++) {
        ...
        // us_qdebug( QString( "ciq: pulling tmp_G i %1 g %2 t %3" ).arg( i
        // ).arg( g + 1 ).arg( t ) );
        ...
        if (fs[i][t] == 0e0) {
          if (!reported_gs0) {
            TSO << QString(
                       "Notice: file %1 t %2 gaussian sum is zero (further "
                       "instances ignored)\n")
                       .arg(i)
                       .arg(t);
            reported_gs0 = true;
        ...
        if (any_bl) {
          double pct_area =
              1e0;  // 1e0 / ( double ) num_of_gauss; // g_area[ i ][ g ];
      ...
      if (conc_ok) {
        ...
        for (unsigned int i = 0; i < csv_conc.data.size(); i++) {
          if (csv_conc.data[i].size() > 1 && csv_conc.data[i][0] == name) {
      ...
      if (sd_from_difference) {
        ...
        if (sd_avg_local) {
          ...
          if (more_zeros) {
            editor_msg("dark red",
                       us_tr("Warning: too many adjacent S.D. zeros, could not "
                             "set all S.D.'s to non-zero values"));
          }
        }
    ...
    if (!mode_testiq && save_sum) {
      if (save_gaussians) {
        ...
        if (any_bl) {
          add_plot(
              QString("sumG_bsba_T%1").arg(pad_zeros(tv[t], (int)tv.size())),
        ...
        if (any_bi) {
          ...
          if (bi_recon.count(tv[t])) {
            for (int q = 0; q < (int)qv.size(); ++q) {
              gsG_bi_recon[q] +=
                  bi_recon[tv[t]].count(qv[q]) ? bi_recon[tv[t]][qv[q]] : 0e0;
            }
          }
          add_plot(
              QString("sumG_bsbai_T%1").arg(pad_zeros(tv[t], (int)tv.size())),
        ...
        if (any_bl) {
          add_plot(
              QString("sumI_bsba_T%1").arg(pad_zeros(tv[t], (int)tv.size())),
        ...
        if (any_bi) {
          ...
          add_plot(
              QString("sumI_bsbai_T%1").arg(pad_zeros(tv[t], (int)tv.size())),
              qv, gsI_bi_recon, gse, false, false);
        }
      }
    }

    // add_plot( QString( "sumIr_T%1" ).arg( pad_zeros( tv[ t ], (int) tv.size()
    // ) ), qv, gsI_recon, gse, false, false ); add_plot( QString( "sumGr_T%1"
    // ).arg( pad_zeros( tv[ t ], (int) tv.size() ) ), qv, gsG_recon, gse,
    // false, false );
  }  // for each q value
...
bool US_Hydrodyn_Dad::create_unified_ggaussian_target(bool do_init,
...
bool US_Hydrodyn_Dad::create_unified_ggaussian_target(QStringList &files,
                                                      bool do_init,
                                                      bool only_init_unset) {
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size(); i++) {
    if (!f_gaussians.count(files[i])) {
      editor_msg(
          "red",
          QString(us_tr("Internal error: %1 does not have a gaussian set"))
              .arg(files[i]));
      return false;
    }
    if (f_gaussians[files[i]].size() != gaussians.size()) {
      editor_msg(
          "red",
          QString(us_tr("Internal error: %1 has an incompatible gaussian set"))
              .arg(files[i]));
      return false;
    }
        ...
        QString(us_tr("Please note:\n\n"
            ...
            .arg(
                qsl_list_no_errors.size()
                    ? QString(
                          us_tr(
                              "These files have no associated errors:\n%1\n\n"))
                          .arg(qsl_list_no_errors.join("\n"))
                    : "")
            .arg(qsl_list_zero_points.size()
                     ? QString(us_tr("These files have points with missing "
                                     "SDs:\n%1\n\n"))
...
bool US_Hydrodyn_Dad::ggauss_recompute() {
  ...
  for (unsigned int i = 0; i < (unsigned int)unified_ggaussian_files.size();
    ...
    if (!f_qs.count(unified_ggaussian_files[i]) ||
      ...
      editor_msg(
          "red",
          QString(
              us_tr("Internal error: %1 has no or empty or insufficient data"))
  ...
  if (!is_nonzero_vector(unified_ggaussian_e)) {
    ...
    if (!error_msg) {
      editor_msg("dark red",
                 us_tr("WARNING: some errors are zero so global errors are off "
                       "for computing RMSD and global fitting"));
    }
  }
...
bool US_Hydrodyn_Dad::compute_f_gaussians(QString file,
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count("dad_cb_gg_cyclic") &&
    ...
    editor_msg(
        "darkRed",
        us_tr("Experimental: Global Gaussian Gaussian cyclic fit - active\n"));
  }
  ...
  if (cb_sd_weight->isChecked() && f_errors.count(file) &&
    ...
    // ] ); US_Vector::printvector( QString( "cfg as %1 I" ).arg( file ), f_Is[
    // file ] ); US_Vector::printvector( QString( "cfg as %1 e" ).arg( file ),
    // f_errors[ file ] );
  }
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count("dad_cb_gg_oldstyle") &&
    ...
    if (cyclic_on || smooth_on) {
      ...
      if (!compute_f_gaussians_trial(file, dad_fit_widget)) {
        editor_msg(
            "red",
            QString(us_tr("Error: computing Gaussians for %1")).arg(file));
        return false;
      }
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count("dad_cb_gg_smooth") &&
    ...
    // get initial fit p value
    {
      ...
      if (pvalue(file, P, errormsg)) {
        ...
        editor_msg(
            "red",
            QString("%1 pvalue compute failed %2\n").arg(file).arg(errormsg));
        f_qs[file] = save_q;
        f_Is[file] = save_I;
        f_errors[file] = save_e;
        return false;
    ...
    for (int s = 1; s <= smoothing; ++s) {
      ...
      if (usu->smooth(f_Is[file], smoothed_I, s)) {
        ...
        editor_msg(
            "red",
            QString("Error smoothing %1 : %2").arg(file).arg(usu->errormsg));
        return false;
      }
      ...
      // get p value
      {
        ...
        // qDebug() << QString( "gg smoothing %1 s %2 p %3" ).arg( file ).arg( s
        // ).arg( P );
      }
    }
    ...
    if (best_smoothing > 0) {
      ...
      add_plot(QString("%1-sm%2").arg(file).arg(best_smoothing), f_qs[file],
               bestsmoothedI, f_errors[file], true, false);
...
bool US_Hydrodyn_Dad::initial_ggaussian_fit(QStringList &files,
                                            bool only_init_unset) {
  wheel_file = files[0];
...
void US_Hydrodyn_Dad::add_plot_gaussian(const QString &file,
                                        const QString &tag) {
  add_plot(QString("%1-%2").arg(file).arg(tag), f_qs[file],
           compute_gaussian_sum(f_qs[file], f_gaussians[file]), true, false);
}

bool US_Hydrodyn_Dad::compute_f_gaussians_trial(QString file,
                                                QWidget *dad_fit_widget) {
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count("dad_cb_gg_cyclic") &&
      ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_gg_cyclic"] == "true") {
    // QTextStream(stdout) << QString( "Experimental: Global Gaussian Gaussian
    // cyclic fit - active - %1\n" ).arg( file );
...
void US_Hydrodyn_Dad::clear_smoothed() {
  f_qs_smoothed.clear();
  f_Is_smoothed.clear();
  f_best_smoothed_smoothing.clear();
}

void US_Hydrodyn_Dad::list_smoothed() {
  TSO << "list_smoothed():\n";
...
void US_Hydrodyn_Dad::add_smoothed(const QString &name, const vector<double> &q,
...
void US_Hydrodyn_Dad::clear_oldstyle() {
  f_qs_oldstyle.clear();
  f_Is_oldstyle.clear();
}

void US_Hydrodyn_Dad::list_oldstyle() {
  TSO << "list_oldstyle():\n";
...
void US_Hydrodyn_Dad::add_oldstyle(const QString &name, const vector<double> &q,
                                   const vector<double> &I) {
  f_qs_oldstyle[name] = q;
  f_Is_oldstyle[name] = I;
}

void US_Hydrodyn_Dad::create_ihash_t() {
  disable_all();
...
bool US_Hydrodyn_Dad::create_ihash_t(QStringList files) {
...
void US_Hydrodyn_Dad::deselect_conc_file() {
  if (!lbl_conc_file->text().isEmpty()) {
    for (int i = 0; i < lb_files->count(); i++) {
...
void US_Hydrodyn_Dad::create_istar_q() {
  disable_all();
...
bool US_Hydrodyn_Dad::create_istar_q(QStringList files, double t_min,
                                     double t_max) {
...
bool US_Hydrodyn_Dad::create_istar_q_ng(QStringList files, double t_min,
                                        double t_max) {
  // if ( lbl_conc_file->text().isEmpty() ) {
  QMessageBox::information(this, windowTitle() + us_tr(": Make I*(q)"),
                           us_tr("I*(q) will be made without Gaussians"));
  ...
  if (use_conc) {
    for (unsigned int i = 0; i < (unsigned int)files.size(); i++) {
      if (!f_g_dndc.count(files[i])) {
        QMessageBox::critical(
            this, windowTitle() + us_tr(": Make I*(q)"),
            QString(us_tr("I*(q) with concentration curve, global dn/dc not "
                          "defined for curve %1"))
                .arg(files[i]));
        return false;
      }
      ...
      if (!i) {
        ...
        if (use_g_dndc != f_g_dndc[files[i]]) {
          QMessageBox::critical(
              this, windowTitle() + us_tr(": Make I*(q)"),
              QString(
                  us_tr("I*(q) with concentration curve: global dn/dc "
                        "inconsistent\n"
  ...
  if (!use_conc && !dad_param_g_conc) {
    if (1 ==
        QMessageBox::question(
            ...
            us_tr("Global concentration in Options->DAD parameters is zero\n"
                  "Do you wish to make I#(q)?"),
            us_tr("&Yes, make I#(q)"), us_tr("&No"), QString(), 0, 1)) {
      QMessageBox::critical(
          this, windowTitle() + us_tr(": Make I*(q)"),
          us_tr("To make I*(q) without a concentration curve,\n"
                "the global concentration must be defined in Options->DAD "
                "parameters"));
      return false;
    }
    make_ihashq = true;
  ...
  if (!use_conc && !make_ihashq) {
    QMessageBox::information(
        this, windowTitle() + us_tr(": Make I*(q)"),
        QString(us_tr("I*(q) will be created using Global concentration of %1 "
                      "[mg/mL]\n(set in Options->DAD parameters)"))
            .arg(dad_param_g_conc));
  }

  if (use_conc && !detector_conv) {
    QMessageBox::critical(this, windowTitle() + us_tr(": Make I*(q)"),
  ...
  for (unsigned int t = 0; t < tv.size(); t++) {
    ...
    if (use_conc && conc_ok) {
      if (!usu->apply_natural_spline(conc_spline_x, conc_spline_y,
        ...
        // dad_param_g_dndc ^2 / dndc ^2 used to correct for K when computing I#
