...
#include "../include/us_hydrodyn_dad_cistarq.h"
...
bool
US_Hydrodyn_Dad::create_i_of_q_ng (set<QString> &fileset, double t_min,
                                   double t_max)
{
  QStringList files;
...
bool
US_Hydrodyn_Dad::create_i_of_q_ng (QStringList files, double t_min,
                                   double t_max)
{
  ...
  if (0 && !mode_testiq)
    {
      QMessageBox::information (this, windowTitle () + us_tr (": Make I(q)"),
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
    {
      if (!(i % 20))
        {
          progress->setValue (i);
          progress->setMaximum (files.size () * 2);
          qApp->processEvents ();
        }
      ...
      if (rx_q.indexIn (files[i]) == -1)
        {
          editor_msg (
              "red", QString (us_tr ("Error: Can not find " + UNICODE_LAMBDA_QS
                                     + " value in file name for %1"))
                         .arg (files[i]));
          progress->reset ();
          update_enables ();
          return false;
      ...
      if (used_q.count (ql.back ()))
        {
          editor_msg ("red",
                      QString (us_tr ("Error: Duplicate " + UNICODE_LAMBDA_QS
                                      + " value in file name for %1"))
                          .arg (files[i]));
          progress->reset ();
          update_enables ();
                      ...
                      if (use_errors)
                        {
                          ...
                          editor_msg (
                              "dark red",
                              QString (
                                  us_tr ("Notice: missing errors, first "
                                         "noticed in %1, so no errors at all"))
                                  .arg (files[i]));
                        }
                    }
              ...
              if (!is_nonzero_vector (f_errors[files[i]]))
                {
                  ...
                  for (unsigned int j = 0;
                       j < (unsigned int)f_errors[files[i]].size (); j++)
                    {
                      if (!mode_testiq
                          || (f_qs[files[i]][j] >= t_min
                              && f_qs[files[i]][j] <= t_max))
                        {
                          if (us_isnan (f_errors[files[i]][j])
                              || f_errors[files[i]][j] == 0e0)
                            {
                              zero_pts++;
                            }
                        }
  ...
  if (!mode_testiq)
    {
      ...
      if (use_conc
        ...
        {
          editor_msg ("red",
                      QString (us_tr ("Internal error: Concentration file %1 "
                                      "set, but associated data is "
                                      "incomplete.  Concentration disabled."))
      ...
      if (use_conc)
        {
          ...
          US_Hydrodyn_Dad_Ciq *dad_ciq
              = new US_Hydrodyn_Dad_Ciq (this, &parameters, this);
          US_Hydrodyn::fixWinButtons (dad_ciq);
          dad_ciq->exec ();
          delete dad_ciq;
          ...
          conv = parameters.count ("conv 0") ? parameters["conv 0"].toDouble ()
                                             : 0e0;
          psv = parameters.count ("psv 0") ? parameters["psv 0"].toDouble ()
                                           : 0e0;
          if (conv == 0e0 || psv == 0e0)
            {
              progress->reset ();
                ...
                if (conc_repeak == 0e0)
                  {
                    ...
                    editor_msg (
                        "red",
                        us_tr ("Error: concentration repeak scaling value "
                               "extracted is 0, turning off back scaling"));
                  }
                else
                  {
                    editor_msg ("dark blue",
                                QString (us_tr ("Notice: concentration "
                                                "scaling repeak value %1"))
                                    .arg (conc_repeak));
                  }
              }
          }
  ...
  for (unsigned int t = 0; t < tv.size (); t++)
    {
      progress->setValue (files.size () + t);
      progress->setMaximum (files.size () + tv.size ());
      ...
      if (conc_ok)
        {
          if (!usu->apply_natural_spline (conc_spline_x, conc_spline_y,
                                          conc_spline_y2, tv[t], conc_factor))
            {
              editor_msg (
                  "red",
                  QString (us_tr ("Error getting concentration from spline "
                                  "for frame %1, concentration set to zero."))
                      .arg (tv[t]));
              conc_factor = 0e0;
            }
        }
      ...
      for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
        {
          if (!I_values.count (tv[t]))
            {
              editor_msg (
                  "dark red",
                  QString (us_tr ("Notice: I values missing frame/time = %1"))
                      .arg (tv[t]));
            }
          ...
          if (!I_values[tv[t]].count (qv[i]))
            {
              editor_msg ("red",
                          QString (us_tr ("Notice: I values missing q = %1"))
                              .arg (qv[i]));
              continue;
            }
              ...
              if (!e_values.count (tv[t]))
                {
                  editor_msg (
                      "red",
                      QString (
                          us_tr ("Internal error: error values missing t %1"))
                          .arg (tv[t]));
                  running = false;
                  update_enables ();
                  progress->reset ();
              ...
              if (!e_values[tv[t]].count (qv[i]))
                {
                  editor_msg (
                      "red",
                      QString (
                          us_tr ("Internal error: error values missing q %1"))
                          .arg (qv[i]));
                  running = false;
                  update_enables ();
                  progress->reset ();
  ...
  if (!mode_testiq)
    {
      editor_msg ("dark blue",
                  us_tr ("Finished: Make A(" + UNICODE_LAMBDA_QS + ")"));
      running = false;
      if (to_select.size ())
        {
...
bool
US_Hydrodyn_Dad::create_i_of_q (set<QString> &fileset, double t_min,
                                double t_max)
{
  QStringList files;
  for (set<QString>::iterator it = fileset.begin (); it != fileset.end ();
...
bool
US_Hydrodyn_Dad::create_i_of_q (QStringList files, double t_min, double t_max)
{
  // us_qdebug( "ciq 0" );
  // for each selected file
  // extract q grid from file names
  ...
  if (!ggaussian_compatible (false))
    {
      editor_msg (
          ...
          us_tr ("NOTICE: Some files selected have Gaussians with varying "
                 ...
                 "Please enter \"Global Gaussians\" with these files selected "
                 "and then \"Keep\" before pressing \"Make I(q)\""));
      return false;
    }
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
    {
      ...
      if (rx_bl.indexIn (files[i]) == -1)
      ...
      else
        {
          ...
          bl_intercept.push_back (
              rx_bl.cap (3).replace ("_", ".").toDouble ());
          // TSO << QString( "bl for file %1 slope %2 intercept %3\n" ).arg( i
          // ).arg( bl_slope.back(), 0, 'g', 8 ).arg( bl_intercept.back(), 0,
          // 'g', 8 ).toLatin1().data();
          bl_count++;
          any_bl = true;
        }
      ...
      if (rx_bi.indexIn (files[i]) == -1)
      ...
      else
        {
          ...
          // ); bi_alpha .push_back( rx_bi.cap( 3 ).replace( "_", "."
          // ).toDouble() ); TSO << QString( "bi for file %1 delta  %2 alpha
          // %3\n" ).arg( i ).arg( bi_delta.back(), 0, 'g', 8 ).arg(
          // bi_alpha.back(), 0, 'g', 8 ).toLatin1().data();
  ...
  if (mode_testiq)
    {
      ...
      // us_qdebug( QString( "ciq mode testiq save_gaussians %1 %2" ).arg(
      // save_gaussians ? "true" : "false" ).arg( bl_count ) );
      bl_count = 0;
      // us_qdebug( QString( "now ciq mode testiq save_gaussians %1 %2" ).arg(
      // save_gaussians ? "true" : "false" ).arg( bl_count ) );
    }
  else
    {
      ...
      if (bl_count)
        {
          parameters["baseline"]
              = QString (
                    us_tr ("Add back the baselines when making I(q).  "
                           "Baselines were found for %1 of the %2 curves"))
                    .arg (bl_count)
                    .arg (files.size ());
        }
      ...
      parameters["gaussians"] = QString ("%1").arg (
          f_gaussians[files[0]].size () / gaussian_type_size);
      ...
      if (lbl_conc_file->text ().isEmpty ())
        {
          parameters["error"] = QString (us_tr (
              "Concentration controls disabled: no concentration file set"));
          no_conc = true;
        }
      else
        {
          if (!f_gaussians.count (lbl_conc_file->text ()))
            {
              parameters["error"] = QString (
                  us_tr ("Concentration controls disabled: no Gaussians "
                         "defined for concentration file"));
              no_conc = true;
            }
          else
            {
              if (f_gaussians[lbl_conc_file->text ()].size ()
                      / gaussian_type_size
                  != f_gaussians[files[0]].size () / gaussian_type_size)
                {
                  parameters["error"]
                      = QString (us_tr ("Concentration controls disabled: "
                                        "Concentration file Gaussian count "
                                        "(%1)\n does not match global curves "
                                        "Gaussian count (%2)"))
                            .arg (f_gaussians[lbl_conc_file->text ()].size ())
                            .arg (f_gaussians[files[0]].size ()
                                  / gaussian_type_size);
                  no_conc = true;
                }
            }
          ...
          if (rx_repeak.indexIn (lbl_conc_file->text ()) != -1)
              ...
              if (conc_repeak == 0e0)
              ...
              else
                {
                  editor_msg (
                      ...
                      QString (
                          us_tr (
                              "Notice: concentration scaling repeak value %1"))
                          .arg (conc_repeak));
                }
            }
          else
            {
              editor_msg (
                  "dark red",
                  us_tr (
                      "Notice: no concentration repeak scaling value found"));
            }
        }
      else
        {
      ...
      if (bl_count
          && (!parameters.count ("add_baseline")
              || parameters["add_baseline"] == "false"))
        {
          bl_count = 0;
          TSO << "ciq: bl off\n";
        }

      if (parameters.count ("save_as_pct_iq")
          && parameters["save_as_pct_iq"] == "true")
        {
      ...
      if (parameters.count ("sd_source")
          && parameters["sd_source"] == "difference")
        {
      ...
      if (parameters.count ("make_ng") && parameters["make_ng"] == "true")
        {
          progress->reset ();
      ...
      if (!no_conc)
        {
          for (unsigned int i = 0;
               i < (unsigned int)f_gaussians[files[0]].size ()
                       / gaussian_type_size;
               i++)
            {
              conv.push_back (
                  parameters.count (QString ("conv %1").arg (i))
                      ? parameters[QString ("conv %1").arg (i)].toDouble ()
                      : 0e0);
              psv.push_back (
                  parameters.count (QString ("psv %1").arg (i))
                      ? parameters[QString ("psv %1").arg (i)].toDouble ()
                      : 0e0);
            }
          conc_ok = true;
          if (parameters.count ("normalize")
  ...
  //                               this->windowTitle(),
  //                               QString( us_tr( "Baselines were found for %1
  //                               of the %2 curves\n"
  //                                            "Do you want to add back the
  //                                            baselines when making I(q)?" )
  //                                            )
  //                               .arg( bl_count )
  //                               .arg( files.size() ),
  //                               us_tr( "&Yes" ),
  ...
  //         QMessageBox::question(this,
  //                               this->windowTitle(),
  //                               us_tr( "Save as Gaussians or a percent of
  //                               the original I(q)?" ), us_tr( "&Gaussians"
  //                               ), us_tr( "Percent of &I(q)" ), QString(),
  //                               0,
  //                               1
  //                               ) == 0;
  running = true;
      ...
      for (unsigned int i = 0; i < (unsigned int)conv.size (); i++)
        {
          ...
          QString qs
              = QString (
                    us_tr ("Concentration Gaussian %1: center %2 area %3"))
                    .arg (i + 1)
                    .arg (tmp_gc[1])
                    .arg (conc_area);
          ...
          add_plot (QString (lbl_conc_file->text () + "_sas_adjusted_pk%1")
                        .arg (i + 1),
                    tv, compute_gaussian (tv, tmp_gn), true, false);
        }
      // us_qdebug( US_Vector::qs_vector3( "conc, ref, new conc",
      // conc_gaussians, ref_gaussians, new_conc_gaussians ) );
      conc_gaussians = new_conc_gaussians;
      add_plot (lbl_conc_file->text () + "_sas_adjusted", tv,
                compute_gaussian_sum (tv, conc_gaussians), true, false);
    }

  // if ( conc_ok && conc_to_saxs )
  // {
  //    editor_msg( "red", "Adjusting concentration curve to SAXS optimized
  //    values is not yet enabled" ); running = false; progress->reset();
  //    update_enables();
  //    return false;
  // }
  ...
  if (normalize_by_conc || conc_ok)
    {
      ...
      for (unsigned int i = 0; i < (unsigned int)conv.size (); i++)
        {
          // add_plot( QString( "conc_mg_per_ml_peak%1" ).arg( i + 1 ), tv,
          // conc_curve( tv, i, conv[ i ] ), true, false ); alt method
          {
            double detector_conv = 0e0;
            if (detector_uv)
              {
            ...
            tmp_g[0] = conc_gaussians[0 + i * gaussian_type_size]
                       * detector_conv / (conc_repeak * conv[i]);
            ...
            add_plot (QString ("conc_mg_per_ml_peak%1").arg (i + 1), tv,
                      concs.back (), true, false);
          }
        }
    }

  // now for each I(t) distribute the I for each frame according to the
  // gaussians
  // !!! **              ---- >>>> check for baseline, if present, optionally
  // add back
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
    {
      ...
      for (unsigned int j = 0; j < (unsigned int)f_gaussians[files[i]].size ();
           j += gaussian_type_size)
        {
          ...
          // US_Vector::printvector( QString( "file %1 gaussian %2 params"
          // ).arg( files[ i ] ).arg( 1 + ( j / gaussian_type_size ) ), tmp_g
          // );
          ...
          // add_plot( QString( "%1_fg_g%2" ).arg( files[ i ] ).arg( 1 + ( j /
          // gaussian_type_size ) ), tv, tmp, true, false );
        }
      fg.push_back (tmp_v);
      ...
      // US_Vector::printvector( QString( "areas file %1 (sum %2)" ).arg( i
      // ).arg( tmp_area_sum, 0, 'g', 8 ), tmp_area ); add_plot( QString(
      // "fg_%1_gsum" ).arg( i ), tv, tmp_sum, true, false );
    }

  // US_Vector::printvector( "area sums", g_area_sum );

  unsigned int num_of_gauss
      = (unsigned int)gaussians.size () / gaussian_type_size;
  ...
  if (mode_testiq && (unsigned int)rb_testiq_gaussians.size () != num_of_gauss)
    {
      editor_msg (
          ...
          QString (
              us_tr (
                  "Test I(q) internal error: Gaussian count mismatch %1 %2"))
  ...
  if (!mode_testiq)
    {
      QFile f (((US_Hydrodyn *)us_hydrodyn)->somo_dir + QDir::separator ()
               + "saxs" + QDir::separator () + "tmp" + QDir::separator ()
               + "dad_frac.csv");
  ...
  if (save_sum && any_bi)
    {
      ...
      //            .arg( qv[ i ] )
      //            .arg( QString( bi_delta.count( qv[ i ] ) ? QString( "%1"
      //            ).arg( bi_delta[ qv[ i ] ] ) : QString( "na" ) ) ) .arg(
      //            QString( bi_alpha.count( qv[ i ] ) ? QString( "%1" ).arg(
      //            bi_alpha[ qv[ i ] ] ) : QString( "na" ) ) )
      //            );
      // }
      ...
      for (int q = 0; q < (int)qv.size (); ++q)
        {
          ...
          if (!bi_alpha.count (qv[q]))
            {
              ...
              editor_msg (
                  "red",
                  QString (
                      us_tr ("Warning: could not find alpha value for "
                             "integral baseline reconstruction for q %1 A^-1"))
                      .arg (qv[q]));
              bi_alpha[qv[q]] = 0e0;
            }
          ...
          for (int t = 0; t < (int)tv.size (); ++t)
            {
              ...
              if (!I_values[tv[t]].count (qv[q]))
                {
                  // // us_qdebug( QString( "I_values[%1][%2] missing" ).arg(
                  // tv[ t ] ).arg( qv[ q ] ) );
                  continue;
                }
          ...
          //         .arg( bl )
          //         .arg( QString( bi_delta.count( qv[ q ] ) ? QString( "%1"
          //         ).arg( bi_delta[ qv[ q ] ] ) : QString( "na" ) ) ) .arg(
          //         bi_delta.count( qv[ q ] ) ? fabs( bi_delta[ qv[ q ] ] - bl
          //         ) : 1e99 )
          //         );
        }
    }
        ...
        for (unsigned int g = 0; g < num_of_gauss; g++)
          {
            ...
            if (dist1_active)
              {
                ...
                if (dist2_active)
                  {
                    tmp_g[4]
                        = f_gaussians[files[0]][4 + g * gaussian_type_size];
                  }
              }
            ...
            editor_msg ("dark blue",
                        QString (us_tr ("Cutoff for Gaussian %1 is [%2,%3]"))
                            .arg (g + 1)
                            .arg (tc_min[g])
                            .arg (tc_max[g]));
          }
...

  {
    ...
    if (avg_peaks)
      {
        double peak_frac = 1e0
                           - ((US_Hydrodyn *)us_hydrodyn)
                                     ->gparams["dad_makeiq_avg_peaks"]
                                     .toDouble ()
                                 * 0.01;
            ...
            for (unsigned int g = 0; g < num_of_gauss; g++)
              {
                ...
                if (dist1_active)
                  {
                    tmp_g[3]
                        = f_gaussians[files[0]][3 + g * gaussian_type_size];
                    if (dist2_active)
                      {
                        tmp_g[4] = f_gaussians[files[0]]
                                              [4 + g * gaussian_type_size];
                      }
                  }
                vector<double> gI = compute_gaussian (tv, tmp_g);
                double max = 0e0;
                ...
                editor_msg ("dark blue",
                            QString (us_tr ("Cutoff for peak averaging of "
                                            "Gaussian %1 is [%2,%3]"))
                                .arg (g + 1)
                                .arg (tcpeak_min[g])
                                .arg (tcpeak_max[g]));
      ...
      for (unsigned int g = 0; g < num_of_gauss; g++)
        {
          ...
          if (use_cutoff && (t < tc_min[g] || t > tc_max[g]))
            {
              skip_create = true;
            }
          ...
          // us_qdebug( QString( "ciq create g loop t %1 g %2 go on" ).arg( t
          // ).arg( g + 1 ) ); build up an I(q)
          double conc_factor = 0e0;
          ...
          if ((conc_ok || normalize_by_conc) && concs[g][t] > 0e0)
            {
              ...
              vector<double> tmp_g (gaussian_type_size);
              tmp_g[0] = conc_gaussians[0 + g * gaussian_type_size]
                         * detector_conv / (conc_repeak * conv[g]);
              ...
              //             TSO << QString( "peak %1 center %2 fwhm %3 t %4
              //             tv[t] %5\n" )
              //                .arg( g + 1 )
              //                .arg( center )
              //                .arg( fwhm )
          ...
          // now go through all the files to pick out the I values and errors
          // and distribute amoungst the various gaussian peaks we could also
          // reassemble the original sum of gaussians curves as a comparative
          ...
          for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
            {
              if (!I_values.count (tv[t]))
                {
                  editor_msg (
                      "dark red",
                      QString (
                          us_tr ("Notice: I values missing frame/time = %1"))
                          .arg (tv[t]));
                  //                running = false;
                  //                update_enables();
                  //                return;
              ...
              // us_qdebug( QString( "ciq: pulling tmp_G i %1 g %2 t %3" ).arg(
              // i ).arg( g + 1 ).arg( t ) );
              ...
              if (fs[i][t] == 0e0)
                {
                  if (!reported_gs0)
                    {
                      TSO << QString ("Notice: file %1 t %2 gaussian sum is "
                                      "zero (further instances ignored)\n")
                                 .arg (i)
                                 .arg (t);
                      reported_gs0 = true;
                    }
              ...
              // {
              // 141212 (change error mode ... propagate original errors) tmp_e
              // *= frac_of_gaussian_sum;
              // }
              ...
              if (any_bl)
                {
                  double pct_area = 1e0; // 1e0 / ( double ) num_of_gauss; //
                                         // g_area[ i ][ g ];
                  double ofs
                      = (bl_intercept[i] + tv[t] * bl_slope[i]) * pct_area;
                  if (bl_count)
                    {
                      tmp_I += ofs;
                      tmp_G += ofs;
              ...
              if (!skip_create)
                {
                  ...
                  lb_created_files->scrollToItem (
                      lb_created_files->item (lb_created_files->count () - 1));
                  lb_files->addItem (name);
                  lb_files->scrollToItem (
                      lb_files->item (lb_files->count () - 1));
                  created_files_not_saved[name] = true;
                  if (add_avg_peaks)
                    {
              ...
              if (sd_drop_zeros)
                {
                  ...
                  vector<double> tmp_I;
                  vector<double> tmp_e;
                  for (unsigned int i = 0; i < (unsigned int)use_e.size ();
                       i++)
                    {
                      if (use_e[i] > 0e0)
                        {
                          ...
                          if (use_e[i] <= 0e0)
                            {
                              ...
                              if (i == (unsigned int)use_e.size () - 1
                                  && i > 0)
                                {
                                  use_e[i] = use_e[i - 1];
                                  if (use_e[i] <= 0e0)
                  ...
                  if (more_zeros)
                    {
                      editor_msg (
                          "dark red",
                          us_tr (
                              "Warning: too many adjacent S.D. zeros, could "
                              "not set all S.D.'s to non-zero values"));
                    }
                }
          ...
          for (unsigned int i = 0; i < (unsigned int)used_names.size (); i++)
            {
              ...
              // US_Vector::printvector( QString( "used_pcts for %1" ).arg(
              // used_names[ i ] ), used_pcts[ i ] );

              if (use_errors)
                {
                  for (unsigned int j = 0; j < (unsigned int)use_e.size ();
                       j++)
                    {
                      use_e[j] *= used_pcts[i][j];
                      use_e[j] = sqrt (use_e[j] * use_e[j]
                                       + f_errors[used_names[i]][j]
                                             * f_errors[used_names[i]][j]);
                    }
                }
              else
      ...
      if (!mode_testiq && save_sum)
        {
          if (save_gaussians)
            {
              add_plot (QString ("sumG%1_T%2")
                            .arg ((any_bl || any_bi) ? "_bs" : "")
                            .arg (pad_zeros (tv[t], (int)tv.size ())),
                        qv, gsG, gse, false, false);
              if (any_bl)
                {
                  add_plot (QString ("sumG_bsba_T%1")
                                .arg (pad_zeros (tv[t], (int)tv.size ())),
                            qv, gsG_recon, gse, false, false);
                }
              if (any_bi)
                {
                  ...
                  if (bi_recon.count (tv[t]))
                    {
                      for (int q = 0; q < (int)qv.size (); ++q)
                        {
                          gsG_bi_recon[q] += bi_recon[tv[t]].count (qv[q])
                                                 ? bi_recon[tv[t]][qv[q]]
                                                 : 0e0;
                        }
                    }
                  add_plot (QString ("sumG_bsbai_T%1")
                                .arg (pad_zeros (tv[t], (int)tv.size ())),
                            qv, gsG_bi_recon, gse, false, false);
                }
            }
          else
            {
              add_plot (QString ("sumI%1_T%2")
                            .arg ((any_bl || any_bi) ? "_bs" : "")
                            .arg (pad_zeros (tv[t], (int)tv.size ())),
                        qv, gsI, gse, false, false);
              if (any_bl)
                {
                  add_plot (QString ("sumI_bsba_T%1")
                                .arg (pad_zeros (tv[t], (int)tv.size ())),
                            qv, gsI_recon, gse, false, false);
                }
              if (any_bi)
                {
                  ...
                  if (bi_recon.count (tv[t]))
                    {
                      for (int q = 0; q < (int)qv.size (); ++q)
                        {
                          gsI_bi_recon[q] += bi_recon[tv[t]].count (qv[q])
                                                 ? bi_recon[tv[t]][qv[q]]
                                                 : 0e0;
                        }
                    }
                  add_plot (QString ("sumI_bsbai_T%1")
                                .arg (pad_zeros (tv[t], (int)tv.size ())),
                            qv, gsI_bi_recon, gse, false, false);
                }
            }
        }

      // add_plot( QString( "sumIr_T%1" ).arg( pad_zeros( tv[ t ], (int)
      // tv.size() ) ), qv, gsI_recon, gse, false, false ); add_plot( QString(
      // "sumGr_T%1" ).arg( pad_zeros( tv[ t ], (int) tv.size() ) ), qv,
      // gsG_recon, gse, false, false );
    } // for each q value
  ...
  if (avg_peaks)
    {
      QString peak_tag = QString ("co%1_").arg (
          ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_makeiq_avg_peaks"]);
              ...
              if (conc_ok)
                {
                  ...
                  last_selected.insert (
                      lb_files->item (lb_files->count () - 1)->text ());
                  set_selected (last_selected);
                  normalize ();
                  final_files_set.insert (
                      lb_files->item (lb_files->count () - 1)->text ());
                }
              else
                {
      ...
      if (final_files_set.size () < 9 && !cb_eb->isChecked ())
        {
...
bool
US_Hydrodyn_Dad::create_unified_ggaussian_target (bool do_init,
                                                  bool only_init_unset)
{
...
bool
US_Hydrodyn_Dad::create_unified_ggaussian_target (QStringList &files,
                                                  bool do_init,
                                                  bool only_init_unset)
{
  ...
  if (do_init)
    {
      ...
      if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_cb_gg_smooth")
          ...
          && ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_gg_smooth"].toInt ()
                 > 0)
        {
          editor_msg (
              "darkred",
              QString ("Experimental smoothing points %1 will be applied\n")
                  .arg (((US_Hydrodyn *)us_hydrodyn)
                            ->gparams["dad_gg_smooth"]
                            .toInt ()));
        }
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
    {
      if (!f_gaussians.count (files[i]))
        {
          editor_msg (
              "red",
              QString (
                  us_tr ("Internal error: %1 does not have a gaussian set"))
                  .arg (files[i]));
          return false;
        }
      if (f_gaussians[files[i]].size () != gaussians.size ())
        {
          editor_msg (
              ...
              QString (
                  us_tr (
                      "Internal error: %1 has an incompatible gaussian set"))
                  .arg (files[i]));
          return false;
        }
      ...
      unified_ggaussian_qvals.push_back (
          rx_q.cap (1).replace ("_", ".").toDouble ());

      for (unsigned int j = 0; j < (unsigned int)f_gaussians[files[i]].size ();
           j += gaussian_type_size)
        {
          unified_ggaussian_params.push_back (
              f_gaussians[files[i]][0 + j]); // height
          if (!cb_fix_width->isChecked ())
            {
              unified_ggaussian_params.push_back (
                  f_gaussians[files[i]][2 + j]); // width
            }
          if (dist1_active && !cb_fix_dist1->isChecked ())
            {
              unified_ggaussian_params.push_back (
                  f_gaussians[files[i]][3 + j]); // dist1
            }
          if (dist2_active && !cb_fix_dist2->isChecked ())
            {
              unified_ggaussian_params.push_back (
                  f_gaussians[files[i]][4 + j]); // dist2
            }
        }
  ...
  progress->setValue (unified_ggaussian_curves * 1.1);
  progress->setMaximum (unified_ggaussian_curves * 1.2);
  qApp->processEvents ();
  if (!ggauss_recompute ())
    {
      progress->reset ();
  ...
  // US_Vector::printvector( "unified param index:",
  // unified_ggaussian_param_index );
...
bool
US_Hydrodyn_Dad::ggauss_recompute ()
{
  ...
  for (unsigned int i = 0; i < (unsigned int)unified_ggaussian_files.size ();
       i++)
    {
      ...
      if (!f_qs.count (unified_ggaussian_files[i])
        ...
        {
          editor_msg ("red", QString (us_tr ("Internal error: %1 has no or "
      ...
      if (unified_ggaussian_use_errors
          && (!f_errors.count (unified_ggaussian_files[i])
              || f_errors[unified_ggaussian_files[i]].size ()
                     != f_qs[unified_ggaussian_files[i]].size ()))
        {
          editor_msg (
              "dark red",
              QString (us_tr ("WARNING: %1 has no errors so global errors are "
                              "off for computing RMSD and global fitting"))
                  .arg (unified_ggaussian_files[i]));
          error_msg = true;
          unified_ggaussian_use_errors = false;
          unified_ggaussian_e.clear ();
      ...
      for (unsigned int j = 0;
           j < (unsigned int)f_qs[unified_ggaussian_files[i]].size (); j++)
        {
          if (f_qs[unified_ggaussian_files[i]][j] >= q_start
              && f_qs[unified_ggaussian_files[i]][j] <= q_end)
            {
              ...
              // unified_gguassian_param_index is the base of the variable
              // parameters for this curve

              unified_ggaussian_param_index.push_back (
                  unified_ggaussian_gaussians_size
                  * (common_size + i * per_file_size));
              /* old way
                if ( cb_fix_width->isChecked() )
                {
                unified_ggaussian_param_index .push_back(
                unified_ggaussian_gaussians_size * ( 2 + i ) ); } else {
                unified_ggaussian_param_index .push_back(
                unified_ggaussian_gaussians_size * ( 1 + i * 2 ) );
                }
              */
              unified_ggaussian_q.push_back (
                  f_qs[unified_ggaussian_files[i]][j]);
              unified_ggaussian_I.push_back (
                  f_Is[unified_ggaussian_files[i]][j]);
              if (unified_ggaussian_use_errors)
                {
                  unified_ggaussian_e.push_back (
                      f_errors[unified_ggaussian_files[i]][j]);
                }
            }
        }
  ...
  if (!is_nonzero_vector (unified_ggaussian_e))
    {
      ...
      if (!error_msg)
        {
          editor_msg ("dark red",
                      us_tr ("WARNING: some errors are zero so global errors "
                             "are off for computing RMSD and global fitting"));
        }
    }
...
bool
US_Hydrodyn_Dad::compute_f_gaussians (QString file, QWidget *dad_fit_widget)
{
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_cb_gg_cyclic")
      && ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_gg_cyclic"] == "true")
    {
      editor_msg (
          "darkRed",
          us_tr (
              "Experimental: Global Gaussian Gaussian cyclic fit - active\n"));
    }
  // us_qdebug( QString( "compute_f_gaussians %1" ).arg( file ) );
  // take current gaussians & compute for this curve
  // find peak of curve
  // US_Hydrodyn_Dad_Fit *fit = (US_Hydrodyn_Dad_Fit *)dad_fit_widget;
  ...
  bool saved = false;
  if (cb_sd_weight->isChecked () && f_errors.count (file)
      && f_errors[file].size ())
    {
      saved = true;
      ...
      // US_Vector::printvector( QString( "cfg as %1 q" ).arg( file ), f_qs[
      // file ] ); US_Vector::printvector( QString( "cfg as %1 I" ).arg( file
      // ), f_Is[ file ] ); US_Vector::printvector( QString( "cfg as %1 e"
      // ).arg( file ), f_errors[ file ] );
    }

  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_cb_gg_oldstyle")
      && ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_gg_oldstyle"] == "true")
    {
      bool cyclic_on
          = ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_cb_gg_cyclic")
            && ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_gg_cyclic"]
                   == "true";
      bool smooth_on
          = ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_cb_gg_smooth")
            && ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_gg_smooth"]
                   == "true"
            && ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_gg_smooth")
            && ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_gg_smooth"].toInt ()
      ...
      if (cyclic_on || smooth_on)
        {
          ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_gg_cyclic"] = "false";
          ...
          if (!compute_f_gaussians_trial (file, dad_fit_widget))
            {
              editor_msg ("red",
                          QString (us_tr ("Error: computing Gaussians for %1"))
                              .arg (file));
              return false;
            }
          ...
          add_oldstyle (file, f_qs[file], gsum);
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_cb_gg_smooth")
      ...
      && ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_gg_smooth"].toInt () > 0)
    {
      ...
      if (!compute_f_gaussians_trial (file, dad_fit_widget))
        {
          editor_msg ("red",
                      QString ("Error computing gaussians %1 [a]").arg (file));
          f_qs[file] = save_q;
          f_Is[file] = save_I;
          f_errors[file] = save_e;
          return false;
      ...
      // qDebug() << QString( "gg smoothing %1 s %2 p %3" ).arg( file ).arg( 0
      // ).arg( best_pvalue );
      ...
      for (int s = 1; s <= smoothing; ++s)
        {
          ...
          if (usu->smooth (f_Is[file], smoothed_I, s))
            ...
            {
              editor_msg ("red", QString ("Error smoothing %1 : %2")
                                     .arg (file)
                                     .arg (usu->errormsg));
              return false;
            }
          ...
          if (!compute_f_gaussians_trial (file, dad_fit_widget))
            {
              ...
              editor_msg (
                  "red",
                  QString ("Error computing gaussians %1 [b]").arg (file));
              return false;
            }
          ...
          // get p value
          {
            ...
            // qDebug() << QString( "gg smoothing %1 s %2 p %3" ).arg( file
            // ).arg( s ).arg( P );
          }
        }
      ...
      if (best_smoothing > 0)
        {
          // qDebug() << QString( "gg smoothing %1 best s %2 p %3" ).arg( file
          // ).arg( best_smoothing ).arg( best_pvalue );
          add_plot (QString ("%1-sm%2").arg (file).arg (best_smoothing),
                    f_qs[file], bestsmoothedI, f_errors[file], true, false);
          add_smoothed (file, f_qs[file], bestsmoothedI, best_smoothing);

          // add_plot_gaussian( file, QString( "sm%1-g" ).arg( best_smoothing )
          // ); to debug them all vector < double > save_g = f_gaussians[ file
...
bool
US_Hydrodyn_Dad::initial_ggaussian_fit (QStringList &files,
                                        bool only_init_unset)
{
  wheel_file = files[0];
  ...
  US_Hydrodyn_Dad_Fit *dad_fit_window
      = new US_Hydrodyn_Dad_Fit (this, false, this);
  US_Hydrodyn::fixWinButtons (dad_fit_window);

  dad_fit_window->update_dad = false;
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
    {
      progress->setValue (i);
      progress->setMaximum (files.size () * 1.2);
      qApp->processEvents ();
#if defined(GG_DEBUG)
      us_qdebug (QString ("------ processing initial gaussian fit %1 ------")
                     .arg (files[i]));
#endif

      if (!only_init_unset || !f_gaussians.count (files[i]))
        {
          if (!compute_f_gaussians (files[i], (QWidget *)dad_fit_window))
            {
              return false;
            }
        }
...
void
US_Hydrodyn_Dad::add_plot_gaussian (const QString &file, const QString &tag)
{
  add_plot (QString ("%1-%2").arg (file).arg (tag), f_qs[file],
            compute_gaussian_sum (f_qs[file], f_gaussians[file]), true, false);
}

bool
US_Hydrodyn_Dad::compute_f_gaussians_trial (QString file,
                                            QWidget *dad_fit_widget)
{
  ...
  // TSO << QString( "cfg: %1 org_gaussian peak %2, curve peak %3, scaling %4
  // new gaussian peak %5\n" )
  //    .arg( file )
  //    .arg( gmax )
  //    .arg( peak )
  ...
  // QTextStream(stdout) << QString( "compute_f_gaussians_trial number of
  // gaussians %1\n" ).arg( fit->cb_fix_curves.size() );
#if defined(GG_DEBUG)
  QTextStream (stdout)
      << QString ("epsilon is %1\n").arg (fit->le_epsilon->text ());
#endif

  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("dad_cb_gg_cyclic")
      && ((US_Hydrodyn *)us_hydrodyn)->gparams["dad_cb_gg_cyclic"] == "true")
    {
      // QTextStream(stdout) << QString( "Experimental: Global Gaussian
      // Gaussian cyclic fit - active - %1\n" ).arg( file );
  ...
  // US_Vector::printvector2( "compute_f_gaussians: gaussians before, after",
  // tmp_gs, gaussians ); printvector( "cfg: after fit gaussians", gaussians );
...
void
US_Hydrodyn_Dad::clear_smoothed ()
{
  f_qs_smoothed.clear ();
  f_Is_smoothed.clear ();
...
void
US_Hydrodyn_Dad::list_smoothed ()
{
  TSO << "list_smoothed():\n";
  for (auto it = f_qs_smoothed.begin (); it != f_qs_smoothed.end (); ++it)
    {
...
void
US_Hydrodyn_Dad::add_smoothed (const QString &name, const vector<double> &q,
                               const vector<double> &I, int best_smoothing)
{
...
void
US_Hydrodyn_Dad::clear_oldstyle ()
{
  f_qs_oldstyle.clear ();
  f_Is_oldstyle.clear ();
}
...
void
US_Hydrodyn_Dad::list_oldstyle ()
{
  TSO << "list_oldstyle():\n";
  for (auto it = f_qs_oldstyle.begin (); it != f_qs_oldstyle.end (); ++it)
    {
...
void
US_Hydrodyn_Dad::add_oldstyle (const QString &name, const vector<double> &q,
...
void
US_Hydrodyn_Dad::create_ihash_t ()
{
  disable_all ();
...
bool
US_Hydrodyn_Dad::create_ihash_t (QStringList files)
{
  double lambda_cm = dad_param_lambda * 1e-7; // nm to cm

  double K = (4 * M_PI * M_PI * dad_param_n * dad_param_n * dad_param_g_dndc
              * dad_param_g_dndc)
             / (AVOGADRO * lambda_cm * lambda_cm * lambda_cm * lambda_cm);
  ...
  for (const auto &name : files)
    {
      ...
      if (!f_Is.count (name) || !f_errors.count (name) || !f_qs.count (name))
        {
          editor_msg (
              "red",
              QString ("Internal error: missing data for %1\n").arg (name));
        }
      else
        {
          ...
          f_g_dndc[last_created_file] = dad_param_g_dndc;
...
void
US_Hydrodyn_Dad::deselect_conc_file ()
{
  if (!lbl_conc_file->text ().isEmpty ())
    {
      for (int i = 0; i < lb_files->count (); i++)
...
void
US_Hydrodyn_Dad::create_istar_q ()
{
  disable_all ();
  ...
  gaussians.size () ? (void)create_istar_q (files)
                    : (void)create_istar_q_ng (files);
...
bool
US_Hydrodyn_Dad::create_istar_q (QStringList files, double t_min, double t_max)
{
...
bool
US_Hydrodyn_Dad::create_istar_q_ng (QStringList files, double t_min,
                                    double t_max)
{
  ...
  //                          us_tr( "I*(q) with concentration curve not
  ...
  if (use_conc
          ...
          || !f_Is.count (lbl_conc_file->text ())))
    {
      editor_msg (
          ...
          QString (
              us_tr (
                  "Internal error: Concentration file %1 set, but associated "
                  "data is incomplete.  Concentration disabled."))
              .arg (lbl_conc_file->text ()));
      lbl_conc_file->setText ("");
      use_conc = false;
    }
          ...
          if (!f_g_dndc.count (files[i]))
            {
              QMessageBox::critical (
                  this, windowTitle () + us_tr (": Make I*(q)"),
                  QString (us_tr ("I*(q) with concentration curve, global "
                                  "dn/dc not defined for curve %1"))
                      .arg (files[i]));
              return false;
            }
              ...
              if (use_g_dndc != f_g_dndc[files[i]])
                {
                  QMessageBox::critical (
                      ...
                      QString (us_tr ("I*(q) with concentration curve: global "
                                      "dn/dc inconsistent\n"
                                      "e.g. curve %1 has value %2 and curve "
                                      "%3 has value %4"))
                          .arg (files[0])
                          .arg (use_g_dndc)
                          .arg (files[i])
  ...
  if (!use_conc && !dad_param_g_conc)
    {
      if (1
          == QMessageBox::question (
              this, this->windowTitle () + us_tr (": Make I*(q)"),
              us_tr (
                  "Global concentration in Options->DAD parameters is zero\n"
                  "Do you wish to make I#(q)?"),
              us_tr ("&Yes, make I#(q)"), us_tr ("&No"), QString (), 0, 1))
        {
          QMessageBox::critical (
              this, windowTitle () + us_tr (": Make I*(q)"),
              us_tr ("To make I*(q) without a concentration curve,\n"
                     "the global concentration must be defined in "
                     "Options->DAD parameters"));
          return false;
        }
      make_ihashq = true;
    }
  if (!use_conc && !make_ihashq)
    {
      QMessageBox::information (
          this, windowTitle () + us_tr (": Make I*(q)"),
          QString (us_tr ("I*(q) will be created using Global concentration "
                          "of %1 [mg/mL]\n(set in Options->DAD parameters)"))
              .arg (dad_param_g_conc));
    }
  ...
  if (use_conc && !detector_conv)
    {
      QMessageBox::critical (
          this, windowTitle () + us_tr (": Make I*(q)"),
          us_tr ("The detector properties must be defined "
                 "Options->Concentraton Detector Properties"));
      return false;
    }
  ...
  editor_msg ("dark blue",
              us_tr (make_ihashq ? "Starting: Make I#(q) without Gaussians"
  ...
  head = head.replace (QRegularExpression ("[\\[\\]{}]"), "");
  head += make_ihashq ? "_UV_Vis_Ihashq_t" : "_UV_Vis_Istarq_t";
  ...
  // for ( auto const & it : dad_angles.q_to_ri ) {
  //    qDebug() << QString( "checking q_to_ri q %1 -> ri %2" ).arg( it.first
  //    ).arg( it.second );
  // }
  for (auto const &q : qv)
    {
      if (dad_angles.q_to_ri.count (q))
        {
          ri_corrs << QString ("%1").arg (dad_angles.q_to_ri[q]);
      ...
      if (use_conc)
        {
          ...
          parameters["ngmode"] = "true";
          parameters["gaussians"] = "1";
          parameters["default_extc"]
              = QString ("%1").arg (dad_param_g_extinction_coef);
          parameters["default_dndc"] = QString ("%1").arg (dad_param_g_dndc);
          US_Hydrodyn_Dad_Ciq *dad_ciq
              = new US_Hydrodyn_Dad_Ciq (this, &parameters, this);
          US_Hydrodyn::fixWinButtons (dad_ciq);
          dad_ciq->exec ();
          ...
          conv = parameters.count ("conv 0") ? parameters["conv 0"].toDouble ()
                                             : 0e0;
          dndc = parameters.count ("dndc 0") ? parameters["dndc 0"].toDouble ()
                                             : 0e0;
          if (conv == 0e0 || dndc == 0e0)
            {
              // TSO << "dad_ciq conv or dndc zero\n";
              progress->reset ();
              update_enables ();
              return false;
            }
  ...
  for (unsigned int t = 0; t < tv.size (); t++)
    {
...

      {
        ...
        name = head
               + QString ("%1%2%3")
                     ...
                     .arg (QString ("%1").arg (frac).replace (
      ...
      double conc_factor = make_ihashq ? 1 : 1 / (dad_param_g_conc * 1e-3);

      double store_conc = dad_param_g_conc;
      ...
      if (use_conc && conc_ok)
        {
          if (!usu->apply_natural_spline (conc_spline_x, conc_spline_y,
          ...
          else
            {
              ...
              // dad_param_g_dndc ^2 / dndc ^2 used to correct for K when
              // computing I#
              store_conc = conc_factor * 1e-3;
              conc_factor
                  = (use_g_dndc * use_g_dndc) / (dndc * dndc * conc_factor);
            }
        }
...
#warning check if dad_param_g_extinction_coef even used?
              ...
              // f_extc      [ name ] = dad_param_g_extinction_coef;
              f_g_dndc[name] = dad_param_g_dndc;
              f_dndc[name] = dad_param_g_dndc;
              f_conc[name] = dad_param_g_conc;
            }
          if (ri_corrs.size ())
            {
              f_ri_corrs[name] = ri_corrs.join (",");
            }
          f_ref_index[name] = dad_param_n;
