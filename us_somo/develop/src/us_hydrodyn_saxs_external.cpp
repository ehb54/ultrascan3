...
void US_Hydrodyn_Saxs::editor_msg(const char *color, QColor bgcolor,
                                  QString msg) {
  editor_msg(QColor(color), bgcolor, msg);
}
...
void US_Hydrodyn_Saxs::editor_msg(QColor color, const char *bgcolor,
                                  QString msg) {
  editor_msg(color, QColor(bgcolor), msg);
}
...
void US_Hydrodyn_Saxs::editor_msg(const char *color, const char *bgcolor,
                                  QString msg) {
  editor_msg(QColor(color), QColor(bgcolor), msg);
}
...
void US_Hydrodyn_Saxs::call_ift(bool rerun) {
  ...
  if (!rerun) {
    ...
    for (int i = 0; i < (int)qsl_plotted_iq_names.size(); ++i) {
      ...
      if (plotted_I_error[ift_pos].size() != plotted_I[ift_pos].size()) {
        editor_msg(
            "dark red",
            QString(us_tr("Notice: curve %1 skipped as it has no errors"))
                .arg(qsl_plotted_iq_names[i]));
        continue;
      }
      if (!US_Saxs_Util::is_nonzero_vector(plotted_I_error[ift_pos])) {
        editor_msg("dark red", QString(us_tr("Notice: curve %1 skipped as it "
                                             "has some zero value errors"))
                                   .arg(qsl_plotted_iq_names[i]));
        continue;
      }
    ...
    if (ift_to_process.size() > 1) {
      bool ok;
      QString res =
          US_Static::getItem(us_tr("US-SOMO SAXS IFT"),
                             us_tr("Select the curve you wish to process:"),
                             ift_to_process, 0, false, &ok, this);
  ...
  if ((int)plotted_q.size() <= ift_pos || (int)plotted_I.size() <= ift_pos) {
    editor_msg("red", QString("internal error: no plotted data for curve %1")
                          .arg(ift_last_processed));
    return;
  }

  if (!plotted_q[ift_pos].size()) {
    editor_msg("red", QString("internal error: empty plotted data for curve %1")
                          .arg(ift_last_processed));
    return;
  }
...
bool US_Hydrodyn_Saxs::last_pr_rebin_save(const QString &header,
                                          const QString &rxstr,
...
void US_Hydrodyn_Saxs::ift_finished(int, QProcess::ExitStatus) {
  ...
  for (int i = 0; i < (int)caps.size(); ++i) {
    ...
    if (rx.indexIn(ift_stdout) == -1) {
      editor_msg("red", QString(us_tr("Could not find %1 file in IFT output"))
                            .arg(caps[i].replace("\\", "")));
    } else {
  ...
  if (files.count(caps[1])) {
    ...
    if (!f.open(QIODevice::ReadOnly)) {
      editor_msg("red", QString("Error: Could not open file %1 for reading.")
                            .arg(files[caps[1]]));
    } else {
      QTextStream ts(&f);
#if QT_VERSION < 0x040000
  ...
  if (files.count(caps[2])) {
    ...
    if (!US_File_Util::getcontents(files[caps[2]], prcontents, error)) {
      ...
      for (int i = 0; i < qsl.size(); ++i) {
        ...
        if (line.size() < 3) {
          ...
          // QTextStream( stdout ) << QString( "this_pr %1 min_pr %2 this_sd %3
          // min_sd %4\n" ).arg( this_pr ).arg( min_pr ).arg( this_sd ).arg(
          // min_sd );
        }
      }
      ...
      if (min_pr == 1e99 && min_sd == 1e99) {
        ...
        if (mw != -1) {
          // US_Vector::printvector3( QString( "ift %1 pr r, pr, pre" ).arg( ,
          // r, pr, pre );
    ...
    QString header =
        QString("# IFT P(r) from " + ift_last_processed + "%1\nR\tP(r)\tSD\n")
            .arg(mw == -1e0 ? QString("") : QString(" mw %1 Daltons").arg(mw));
    ...
    if (ok_to_replace) {
      ...
      if (!prcontents_normed.isEmpty()) {
        ...
        QString dest_normed = USglobal->config_list.root_dir + "/somo/saxs/" +
                              QString("%1")
                                  .arg(ift_last_processed)
                                  .replace(QRegExp(rxstr), "_ift.dat");
        if (!((US_Hydrodyn *)us_hydrodyn)->overwrite) {
          dest_normed =
              ((US_Hydrodyn *)us_hydrodyn)->fileNameCheck(dest_normed, 0, this);
        }
...
void US_Hydrodyn_Saxs::foxs_finished(int, QProcess::ExitStatus) {
  ...
  if (!QFile::exists(created_dat)) {
    editor_msg(
        "red",
        QString(us_tr("Error: FoXS did not create file %1")).arg(created_dat));
  ...
  if (!QFile::exists(created_plt)) {
    editor_msg(
        "dark red",
        QString(us_tr("Notice: FoXS did not create file %1")).arg(created_dat));
  } else {
  ...
  if (QFile::exists(new_created_dat)) {
    if (((US_Hydrodyn *)us_hydrodyn)->overwrite) {
      editor_msg("red",
                 QString(us_tr("Notice: overwriting %1")).arg(new_created_dat));
    } else {
      new_created_dat =
          ((US_Hydrodyn *)us_hydrodyn)->fileNameCheck(new_created_dat, 0, this);
    }
...
int US_Hydrodyn_Saxs::run_saxs_iq_crysol(QString pdb) {
  ...
  if (!qd_dir.isReadable()) {
    editor_msg("red", QString("Crysol called but the directory '%1' is not "
  ...
  if (!qd_dir2.mkdir(qd_dir2.path())) {
    editor_msg("red", QString("Crysol called but could not create the "
                              "temporary directory '%1'. Check permissions\n")
                          .arg(qd_dir2.path()));
    return -1;
  }
  if (!qd_dir2.exists()) {
  ...
  if (crysol_manual_mode) {
    ...
    crysol_response << QString("%1").arg(our_saxs_options->water_e_density, 0,
                                         'f', 4);
    ...
    crysol_manual_input << QString("%1").arg(
        (unsigned int)(our_saxs_options->end_q / our_saxs_options->delta_q));
    crysol_manual_input << (our_saxs_options->crysol_explicit_hydrogens ? "Y"
                                                                        : "N");
    ...
    crysol_manual_input << QString("%1").arg(our_saxs_options->water_e_density,
                                             0, 'f', 4);
    ...
    crysol->setWorkingDirectory(dir);
    {
      if (our_saxs_options->crysol_version_3) {
        ...
        args << QString("--smax=%1").arg(our_saxs_options->end_q)
             << QString("--ns=%1").arg(
        ...
        args << (our_saxs_options->crysol_version_26
             ...
             << "/dns" << QString("%1").arg(our_saxs_options->water_e_density)

             << "/dro"
             << QString("%1").arg(
                    our_saxs_options->crysol_hydration_shell_contrast)

             << "/lm" << QString("%1").arg(our_saxs_options->sh_max_harmonics)
    ...
    if ((int)crysol_app_text.size() > crysol_query_response_pos &&
      ...
      if (crysol_response[crysol_query_response_pos] != "___run___") {
        if (crysol_response[crysol_query_response_pos].left(2).contains("__")) {
          // us_qdebug(
          //        QString( us_tr( "Error: undefined response <%1> to query
          //        <%2>" ) ) .arg( crysol_response[ crysol_query_response_pos ]
...
#endif
    if (!f.open(QIODevice::ReadOnly)) {
      editor_msg("red", QString(us_tr("Error: trying to read output file %1"))
                            .arg(f.fileName()));
      pb_plot_saxs_sans->setEnabled(true);
      return;
    }
...
void US_Hydrodyn_Saxs::crysol_finishup() {
  ...
  if (our_saxs_options->crysol_version_3) {
    created_dat =
        crysol_last_pdb_base.replace(QRegExp("\\.(pdb|PDB)$"), "") + type;
  } else {
    created_dat = crysol_last_pdb_base.replace(QRegExp("\\.(pdb|PDB)$"), "") +
                  "00" + type;
  }
  ...
  if (!new_intensity_file.isEmpty()) {
    created_dat =
        QFileInfo(created_dat).path() + QDir::separator() + new_intensity_file;
  }
  if (!new_fit_file.isEmpty()) {
    created_dat =
        QFileInfo(created_dat).path() + QDir::separator() + new_fit_file;
  }

  // us_qdebug( "created_dat after rplc: " + created_dat );

  if (!QFile::exists(created_dat)) {
    editor_msg("red", QString(us_tr("Error: Crysol did not create file %1"))
                          .arg(created_dat));
    pb_plot_saxs_sans->setEnabled(true);
    external_running = false;
    return;
...
int US_Hydrodyn_Saxs::run_sans_iq_cryson(QString pdb) {
  ...
  if (!qd_dir.isReadable()) {
    editor_msg("red", QString("Cryson called but the directory '%1' is not "
                              "readable. Check permissions\n")
                          .arg(qd_dir.path()));
    return -1;
  }
  ...
  args << (our_saxs_options->crysol_version_26 ? QFileInfo(use_pdb).fileName()
                                               : use_pdb)
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count("perdeuteration")) {
    args << "/per" << ((US_Hydrodyn *)us_hydrodyn)->gparams["perdeuteration"];
  }

  args << "/lm" << QString("%1").arg(our_saxs_options->cryson_sh_max_harmonics)
...
void US_Hydrodyn_Saxs::cryson_finished(int, QProcess::ExitStatus) {
  ...
  QString created_dat =
      cryson_last_pdb_base.replace(QRegExp("\\.(pdb|PDB)$"), "") + "00.int";

  if (!QFile::exists(created_dat)) {
    editor_msg("red", QString(us_tr("Error: Cryson did not create file %1"))
                          .arg(created_dat));
    pb_plot_saxs_sans->setEnabled(true);
    external_running = false;
    return;
...
void US_Hydrodyn_Saxs::sastbx_finished(int, QProcess::ExitStatus) {
  ...
  if (!QFile::exists(created_dat)) {
    editor_msg("red", QString(us_tr("Error: Sastbx did not create file %1"))
                          .arg(created_dat));
    pb_plot_saxs_sans->setEnabled(true);
    external_running = false;
    return;
