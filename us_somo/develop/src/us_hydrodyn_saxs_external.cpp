...
void
US_Hydrodyn_Saxs::editor_msg (QColor color, QColor bgcolor, QString msg)
{
  msg.replace (QRegExp ("\n*$"), "");
  msg += "\n";
  // QColor save_color_bg = editor->paragraphBackgroundColor(
  // editor->paragraphs() - 1 );
...
void
US_Hydrodyn_Saxs::editor_msg (QColor color, QString msg)
{
...
void
US_Hydrodyn_Saxs::editor_msg (QString color, QString msg)
{
  editor_msg (QColor (color), msg);
}

void
US_Hydrodyn_Saxs::editor_msg (const char *color, QString msg)
{
  editor_msg (QString (color), msg);
}

void
US_Hydrodyn_Saxs::editor_msg (QString color, QColor bgcolor, QString msg)
{
  editor_msg (QColor (color), bgcolor, msg);
}

void
US_Hydrodyn_Saxs::editor_msg (const char *color, QColor bgcolor, QString msg)
{
  editor_msg (QColor (color), bgcolor, msg);
}

void
US_Hydrodyn_Saxs::editor_msg (QColor color, QString bgcolor, QString msg)
{
  editor_msg (color, QColor (bgcolor), msg);
}

void
US_Hydrodyn_Saxs::editor_msg (QColor color, const char *bgcolor, QString msg)
{
  editor_msg (color, QColor (bgcolor), msg);
}

void
US_Hydrodyn_Saxs::editor_msg (QString color, QString bgcolor, QString msg)
{
  editor_msg (QColor (color), QColor (bgcolor), msg);
}

void
US_Hydrodyn_Saxs::editor_msg (const char *color, const char *bgcolor,
                              QString msg)
{
  editor_msg (QColor (color), QColor (bgcolor), msg);
}
...
void
US_Hydrodyn_Saxs::call_ift (bool rerun)
{
  us_qdebug ("ift()");
  if (external_running)
    {
      ...
      for (int i = 0; i < (int)qsl_plotted_iq_names.size (); ++i)
        {
          if (!plotted_iq_names_to_pos.count (qsl_plotted_iq_names[i]))
            {
              editor_msg (
                  "red",
                  QString (
                      "internal error: no plotted position info for curve %1")
          ...
          if ((int)plotted_q.size () <= ift_pos
              ...
              || (int)plotted_I_error.size () <= ift_pos)
            {
              editor_msg (
                  "red",
                  QString ("internal error: no plotted data for curve %1")
                      .arg (qsl_plotted_iq_names[i]));
              return;
            }
          ...
          if (plotted_I_error[ift_pos].size () != plotted_I[ift_pos].size ())
            {
              editor_msg (
                  "dark red",
                  QString (
                      us_tr ("Notice: curve %1 skipped as it has no errors"))
                      .arg (qsl_plotted_iq_names[i]));
              continue;
            }
          if (!US_Saxs_Util::is_nonzero_vector (plotted_I_error[ift_pos]))
            {
              editor_msg ("dark red",
                          QString (us_tr ("Notice: curve %1 skipped as it has "
                                          "some zero value errors"))
                              .arg (qsl_plotted_iq_names[i]));
              continue;
            }
      ...
      if (!ift_to_process.size ())
        {
          US_Static::us_message (
              us_tr ("US-SOMO SAXS IFT"),
              QString (us_tr ("No I(q) curves with full errors available to "
      ...
      if (ift_to_process.size () > 1)
        {
          ...
          QString res = US_Static::getItem (
              us_tr ("US-SOMO SAXS IFT"),
              us_tr ("Select the curve you wish to process:"), ift_to_process,
              0, false, &ok, this);
  ...
  ift_prog = USglobal->config_list.system_dir + SLASH +
#if defined(BIN64)
             "bin64"
#else
             "bin"
...

  {
    ...
    if (!qfi.exists () && ift_prog.contains ("bin64"))
      {
        ift_prog = USglobal->config_list.system_dir + SLASH + "bin" + SLASH
                   + "iftci"
#if defined(Q_OS_WIN)
                   + "_win64.exe"
#else
    ...
    if (!qfi.exists ())
      {
        editor_msg (
            (QString) "red",
            QString ("IFT program '%1' does not exist\n").arg (ift_prog));
        return;
      }
    if (!qfi.isExecutable ())
      {
        editor_msg (
            "red",
            QString ("IFT program '%1' is not executable\n").arg (ift_prog));
        return;
      }
  }
  ...
  if (!plotted_iq_names_to_pos.count (ift_last_processed))
    {
      editor_msg (
          "red",
          QString ("internal error: no plotted position info for curve %1")
              .arg (ift_last_processed));
      return;
    }

  int ift_pos = plotted_iq_names_to_pos[ift_last_processed];

  if ((int)plotted_q.size () <= ift_pos || (int)plotted_I.size () <= ift_pos)
    {
      editor_msg ("red",
                  QString ("internal error: no plotted data for curve %1")
                      .arg (ift_last_processed));
  ...
  if (!plotted_q[ift_pos].size ())
    {
      editor_msg ("red",
                  QString ("internal error: empty plotted data for curve %1")
                      .arg (ift_last_processed));
      return;
    }
  ...
  ((US_Hydrodyn *)us_hydrodyn)->gparams["buffer_nth_shown"]
      = ift_parameters.count ("buffer_nth_shown")
            ? ift_parameters["buffer_nth_shown"]
            : "";

  if (!ift_parameters.count ("button") || ift_parameters["button"] != "go")
    {
      return;
    }
...
void
US_Hydrodyn_Saxs::ift_process_next ()
{

  if (!ift_to_process.size ())
    {
      if (ift_parameters.count ("overwriteoff")
          && ift_parameters["overwriteoff"] == "true")
        {
    ...
    if (!usu.write (ift_last_processed.left (IFT_SHORTEN), "data"))
      {
        editor_msg (
            "red",
            QString ("Error: Could not write file %1. Disk space issue?")
                .arg (ift_last_processed));
        return;
      }
  }
  ...
  ts
      ...
      << "\n"
      //    # l4
      << ((ift_parameters.count ("dmaxfixed")
           && ift_parameters["dmaxfixed"] == "true")
              ? "f"
              : "")
      << ift_parameters["dmax"]
      << "\n"
      ...
      << "\n"
      //    # l6
      << ((ift_parameters.count ("alphafixed")
           && ift_parameters["alphafixed"] == "true")
              ? "f"
              : "")
      << ift_parameters["alpha"]
      << "\n"
      //    # l7
      << (ift_parameters.count ("smearing") ? ift_parameters["smearing"] : "")
      << "\n"
      //    # l8
      << ((ift_parameters.count ("estimateratiofixed")
           && ift_parameters["estimateratiofixed"] == "true")
              ? "f"
              : "")
      << ift_parameters["estimateratio"]
      << "\n"
      //    # l9
      ...
      << "\n"
      //    # l13
      << ((ift_parameters.count ("fitbackground")
           && ift_parameters["fitbackground"] == "true")
              ? "y"
              : "n")
      << "\n"
      //    # l14
      << ((ift_parameters.count ("smallplot")
           && ift_parameters["smallplot"] == "true")
              ? "S"
              : "L")
      << "\n";
  ...
  //    "\n"; # l5 $inputfile .= $$ref{ 'eta' } . "\n"; # l6 $inputfile .= (
  //    $$ref{ 'alphafixed' } ? "f" : "" ) . $$ref{ 'alpha' } . "\n"; # l7
  //    $inputfile .= $$ref{ 'smearing' } . "\n";
  //    # l8
  //    $inputfile .= ( $$ref{ 'estimateratiofixed' } ? "f" : "" ) . $$ref{
  //    'estimateratio' } . "\n"; # l9 $inputfile .= $$ref{ 'fitratio' } .
  //    "\n"; # l10 $inputfile .= $$ref{ 'prpoints' } . "\n"; # l11 $inputfile
  //    .= $$ref{ 'noextracalc' } . "\n"; # l12 $inputfile .= $$ref{
  //    'transform' } . "\n"; # l13 $inputfile .= ( $$ref{ 'fitbackground' } ?
  //    "y" : "n" ) . "\n"; # l14 $inputfile .= ( $$ref{ 'smallplot' } ? "S" :
  //    "L" ) . "\n";
  // }
  ...
  connect (ift, SIGNAL (readyReadStandardOutput ()), this,
           SLOT (ift_readFromStdout ()));
  connect (ift, SIGNAL (readyReadStandardError ()), this,
           SLOT (ift_readFromStderr ()));
  connect (ift, SIGNAL (finished (int, QProcess::ExitStatus)), this,
           SLOT (ift_finished (int, QProcess::ExitStatus)));
...
void
US_Hydrodyn_Saxs::ift_readFromStdout ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Saxs::ift_readFromStderr ()
{
#if QT_VERSION < 0x040000
...
bool
US_Hydrodyn_Saxs::last_pr_rebin_save (const QString &header,
                                      const QString &rxstr,
                                      QStringList &created_files)
{
  vector<double> rebin_r;
  // new bin
  for (double x = 0; x <= plotted_r.back ().back ();
  ...
  if (!((US_Hydrodyn *)us_hydrodyn)->overwrite)
    {
      dest_rebin
          = ((US_Hydrodyn *)us_hydrodyn)->fileNameCheck (dest_rebin, 0, this);
    }
  {
    QString error;
...
void
US_Hydrodyn_Saxs::ift_finished (int, QProcess::ExitStatus)
{
  ...
  for (int i = 0; i < (int)caps.size (); ++i)
    {
      ...
      if (rx.indexIn (ift_stdout) == -1)
        {
          editor_msg ("red",
                      QString (us_tr ("Could not find %1 file in IFT output"))
                          .arg (caps[i].replace ("\\", "")));
        }
      else
        {
  ...
  if (files.count (caps[1]))
    {
      ...
      if (!f.open (QIODevice::ReadOnly))
        {
          editor_msg ("red",
                      QString ("Error: Could not open file %1 for reading.")
                          .arg (files[caps[1]]));
        }
      else
        {
  ...
  if (files.count (caps[2]))
    {
      ...
      double mw = get_mw (QString ("%1")
                              .arg (ift_last_processed)
                              .replace (QRegExp ("\\..*$"), "_ift P(r)"),
                          false, true);
      ...
      else
        {
          ...
          for (int i = 0; i < qsl.size (); ++i)
            {
              ...
              if (line.size () < 3)
              ...
              else
                {
                  ...
                  // QTextStream( stdout ) << QString( "this_pr %1 min_pr %2
                  // this_sd %3 min_sd %4\n" ).arg( this_pr ).arg( min_pr
                  // ).arg( this_sd ).arg( min_sd );
                }
            }
          ...
          if (min_pr == 1e99 && min_sd == 1e99)
          ...
          else
            {
              ...
              if (mw != -1)
                {
                  // US_Vector::printvector3( QString( "ift %1 pr r, pr, pre"
                  // ).arg( , r, pr, pre );
                  normalize_pr (r, &pr, &pre, mw);
                  for (int i = 0; i < (int)r.size (); ++i)
                    {
                      newcontents_normed << QString ("%1 %2 %3")
                                                .arg (r[i], 0, 'g', 9)
                                                .arg (pr[i], 0, 'g', 9)
                                                .arg (pre[i], 0, 'g', 9);
                    }
      ...
      QString dest = USglobal->config_list.root_dir + "/somo/saxs/"
                     + QString ("%1")
                           .arg (ift_last_processed)
                           .replace (QRegExp (rxstr), "_ift.sprr");
      if (!((US_Hydrodyn *)us_hydrodyn)->overwrite)
        {
      ...
      QString header
          = QString ("# IFT P(r) from " + ift_last_processed
                     + "%1\nR\tP(r)\tSD\n")
                .arg (mw == -1e0 ? QString ("")
          ...
          if (!prcontents_normed.isEmpty ())
            {
              ...
              QString dest_normed
                  = USglobal->config_list.root_dir + "/somo/saxs/"
                    + QString ("%1")
                          .arg (ift_last_processed)
                          .replace (QRegExp (rxstr), "_ift.dat");
              if (!((US_Hydrodyn *)us_hydrodyn)->overwrite)
                {
                  dest_normed = ((US_Hydrodyn *)us_hydrodyn)
                                    ->fileNameCheck (dest_normed, 0, this);
                }
              if (!US_File_Util::putcontents (dest_normed, prcontents_normed,
                                              error))
                {
                  editor_msg ("red", error);
                }
  ...
  if (files.count (caps[0]))
    {
      ...
      QString dest = USglobal->config_list.root_dir + "/somo/saxs/"
                     + QString ("%1")
                           .arg (ift_last_processed)
                           .replace (QRegExp (rxstr), "_ift_summary.txt");
      if (!((US_Hydrodyn *)us_hydrodyn)->overwrite)
        {
          dest = ((US_Hydrodyn *)us_hydrodyn)->fileNameCheck (dest, 0, this);
        }
  ...
  if (files.count (caps[4]))
    {
      ...
      QString dest = USglobal->config_list.root_dir + "/somo/saxs/"
                     + QString ("%1")
                           .arg (ift_last_processed)
                           .replace (QRegExp (rxstr), "_fit.ssaxs");
      if (!((US_Hydrodyn *)us_hydrodyn)->overwrite)
        {
          dest = ((US_Hydrodyn *)us_hydrodyn)->fileNameCheck (dest, 0, this);
        }
      US_File_Util ufu;
      if (!ufu.copy (files[caps[4]], dest, true,
                     "# IFT I(q) fitting from " + ift_last_processed
                         + "\nq\tI(q)\tSD\n"))
        {
          editor_msg ("red", ufu.errormsg);
        }
...
int
US_Hydrodyn_Saxs::run_saxs_iq_foxs (QString pdb)
{
    ...
    if (!qfi.exists ())
      {
        editor_msg ((QString) "red",
                    QString ("FoXS program '%1' does not exist\n").arg (prog));
        return -1;
      }
    if (!qfi.isExecutable ())
      {
        editor_msg (
            "red",
            QString ("FoXS program '%1' is not executable\n").arg (prog));
        return -1;
      }
  }
  ...
  if (!fi.exists ())
    {
      editor_msg ("red",
                  QString ("FoXS called but PDB file '%1' does not exist\n")
                      .arg (pdb));
      return -1;
    }
  ...
  foxs->addArgument ("-s");
  foxs->addArgument (QString ("%1").arg (
      (unsigned int)(our_saxs_options->end_q / our_saxs_options->delta_q)));

  foxs->addArgument (pdb);

  cout << QString ("foxs -q %1 -s %2 %3\n")
              .arg (our_saxs_options->end_q)
              .arg ((unsigned int)(our_saxs_options->end_q
                                   / our_saxs_options->delta_q))
              .arg (pdb);

  connect (foxs, SIGNAL (readyReadStandardOutput ()), this,
           SLOT (foxs_readFromStdout ()));
  connect (foxs, SIGNAL (readyReadStandardError ()), this,
           SLOT (foxs_readFromStderr ()));
  connect (foxs, SIGNAL (finished (int, QProcess::ExitStatus)), this,
           SLOT (foxs_finished (int, QProcess::ExitStatus)));
...
#else
  {
    ...
    args << "-q" << QString ("%1").arg (our_saxs_options->end_q) << "-s"
         << QString ("%1").arg ((unsigned int)(our_saxs_options->end_q
                                               / our_saxs_options->delta_q))
         << pdb;
...
void
US_Hydrodyn_Saxs::foxs_readFromStdout ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Saxs::foxs_readFromStderr ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Saxs::foxs_finished (int, QProcess::ExitStatus)
{
  //   for ( int i = 0; i < 10000; i++ )
  //   {
  foxs_readFromStderr ();
  ...
  if (!QFile::exists (created_dat))
    {
      editor_msg ("red", QString (us_tr ("Error: FoXS did not create file %1"))
                             .arg (created_dat));
  ...
  if (!QFile::exists (created_plt))
    {
      editor_msg ("dark red",
                  QString (us_tr ("Notice: FoXS did not create file %1"))
                      .arg (created_dat));
    }
  else
    {
      if (!QFile::remove (created_plt))
        {
          editor_msg (
              "red",
              QString (us_tr ("Notice: remove of FoXS created file %1 failed"))
                  .arg (created_dat));
        }
    }
  ...
  QString new_created_dat
      = ((US_Hydrodyn *)us_hydrodyn)->somo_dir + SLASH + "saxs" + SLASH
        + QFileInfo (foxs_last_pdb).fileName () + iqq_suffix () + ".dat";

  if (QFile::exists (new_created_dat))
    {
      if (((US_Hydrodyn *)us_hydrodyn)->overwrite)
        {
          editor_msg ("red", QString (us_tr ("Notice: overwriting %1"))
                                 .arg (new_created_dat));
        }
      else
        {
          new_created_dat = ((US_Hydrodyn *)us_hydrodyn)
                                ->fileNameCheck (new_created_dat, 0, this);
        }
  ...
  if (!qd.rename (created_dat, new_created_dat))
    {
      editor_msg ("red", QString (us_tr ("Notice: could not rename %1 to %2"))
                             .arg (created_dat)
                             .arg (new_created_dat));
...
int
US_Hydrodyn_Saxs::run_saxs_iq_crysol (QString pdb)
{
  QString prog = USglobal->config_list.system_dir + SLASH +
#if defined(BIN64)
                 "bin64"
    ...
    if (!qfi.exists ())
      {
        editor_msg (
            "red",
            QString ("Crysol program '%1' does not exist\n").arg (prog));
        return -1;
      }
    if (!qfi.isExecutable ())
      {
        editor_msg (
            "red",
            QString ("Crysol program '%1' is not executable\n").arg (prog));
        return -1;
      }
  }
  ...
  if (!fi.exists ())
    {
      editor_msg ("red",
                  QString ("Crysol called but PDB file '%1' does not exist\n")
                      .arg (pdb));
      return -1;
    }
  ...
  if (!qd_dir.exists ())
    {
      editor_msg (
          "red",
          QString ("Crysol called but the directory '%1' does not exist\n")
              .arg (qd_dir.path ()));
      return -1;
    }
  ...
  if (!qd_dir.isReadable ())
    {
      editor_msg ("red", QString ("Crysol called but the directory '%1' is "
  ...
  if (qd_dir2.exists ())
    {
      ...
      do
        {
          qd_dir2.setPath (dir + QDir::separator () + uniqstr
                           + QString ("-%1").arg (ext++));
        }
  ...
  if (!qd_dir2.mkdir (qd_dir2.path ()))
    {
      editor_msg ("red",
                  QString ("Crysol called but could not create the temporary "
                           "directory '%1'. Check permissions\n")
                      .arg (qd_dir2.path ()));
      return -1;
    }
  if (!qd_dir2.exists ())
  ...
  if (our_saxs_options->crysol_version_26)
    {
      crysol_last_pdb_base = dir + SLASH
                             + QFileInfo (crysol_last_pdb)
                                   .fileName ()
                                   .replace (QRegExp ("\\.(pdb|PDB)$"), "")
                                   .left (6)
                             + ".pdb";
    }
  else
  ...
  // always copy pdb
  //   if (  our_saxs_options->crysol_version_26 ||
  //        QFileInfo(crysol_last_pdb).fileName() !=
  //        QFileInfo(crysol_last_pdb_base).fileName() )
  {
    QFile f (pdb);
    if (!f.open (QIODevice::ReadOnly))
      {
        editor_msg (
            "red",
            QString ("Could not open file %1. Check permissions\n").arg (pdb));
        return -1;
      }
  ...
  editor_msg (
      "dark blue",
      QString ("Crysol: last pdb base [%1]").arg (crysol_last_pdb_base));
  ...
  {
    QString base
        = crysol_last_pdb_base.replace (QRegExp ("\\.(pdb|PDB)$"), "");
    cout << "base: <" << base << ">\n";
              ...
              .isEmpty ())
    {
      if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("sas_crysol_ra")
                 ...
                 > 0e0)
        {
          editor_msg (
              "dark red",
              QString (us_tr ("Note: Manual average atomic radius %1 (A)"))
                  .arg (
      ...
      if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("sas_crysol_vol")
        ...
        {
          editor_msg ("dark red",
                      QString (us_tr ("Note: Manual excluded volume %1 (A^3)"))
  ...
  if (crysol_manual_mode)
    {
      editor_msg (
          "dark red",
          QString (us_tr ("Note: crysol running in interactive mode")));
      ...
      crysol_response << (our_saxs_options->crysol_version_26
                              ? QFileInfo (use_pdb).fileName ()
                              : use_pdb);
      crysol_app_text << "Maximum order of  harmonics ........... <";
      crysol_response << QString ("%1").arg (
      ...
      crysol_response << (our_saxs_options->crysol_explicit_hydrogens ? "Y"
                                                                      : "N");
      ...
      crysol_response << QString ("%1").arg (our_saxs_options->water_e_density,
                                             0, 'f', 4);
      ...
      crysol_response << QString ("%1").arg (
          our_saxs_options->crysol_hydration_shell_contrast, 0, 'f',
          4); // hydration shell contrast
      crysol_app_text << "Average atomic radius .................. <";
      crysol_response << QString (
      ...
      crysol_manual_input << (our_saxs_options->crysol_version_26
                                  ? QFileInfo (use_pdb).fileName ()
                                  : use_pdb);
      crysol_manual_input << QString ("%1").arg (
          our_saxs_options->sh_max_harmonics);
      crysol_manual_input << QString ("%1").arg (
          our_saxs_options->sh_fibonacci_grid_order);
      ...
      crysol_manual_input << QString ("%1").arg (
          (unsigned int)(our_saxs_options->end_q / our_saxs_options->delta_q));
      crysol_manual_input << (our_saxs_options->crysol_explicit_hydrogens
                                  ? "Y"
                                  : "N");
      crysol_manual_input << "Y"; // fit expt curve
      crysol_manual_input << QFileInfo (((US_Hydrodyn *)us_hydrodyn)
                                            ->gparams["saxs_crysol_target"])
                                 .fileName ();
      crysol_manual_input << "N"; // subtract constant
      crysol_manual_input << "1"; // angular units
      crysol_manual_input << QString ("%1").arg (
          our_saxs_options->water_e_density, 0, 'f', 4);
      ...
      // create input log file, run system command into output
      {
          ...
          if (!f.open (QIODevice::WriteOnly))
            {
              editor_msg (
                  "red",
                  QString (us_tr ("Error: trying to create input file %1"))
                      .arg (f.fileName ()));
              pb_plot_saxs_sans->setEnabled (true);
              return -1;
            }
          ...
          if (!f.open (QIODevice::WriteOnly))
            {
              editor_msg (
                  "red",
                  QString (us_tr ("Error: trying to create batch file %1"))
                      .arg (f.fileName ()));
              pb_plot_saxs_sans->setEnabled (true);
              return -1;
            }
        ...
        //             wchar_t wtext[ 2048 ];
        //             mbstowcs( wtext, cmd.toLatin1().data(), strlen(
        //             cmd.toLatin1().data() ) + 1 ); //Plus null
        ...
        //             // wchar_t wtext[ 1024 ];
        //             // mbstowcs(wtext, cmd.toLatin1().data(),
        //             strlen(cmd.toLatin1().data())+1);//Plus null

        //             // if (CreateProcessW( NULL, wtext, NULL, NULL, false,
          ...
          if (!f.open (QIODevice::ReadOnly))
            {
              editor_msg (
                  "red",
                  QString (us_tr ("Error: trying to read output file %1"))
                      .arg (f.fileName ()));
              pb_plot_saxs_sans->setEnabled (true);
              return -1;
            }
  ...
  else
    {
      ...
      crysol->setWorkingDirectory (dir);
      {
        if (our_saxs_options->crysol_version_3)
          {
            ...
            args
                ...
                << QString ("--ns=%1").arg (
                       (unsigned int)(our_saxs_options->end_q
                                      / our_saxs_options->delta_q))
          ...
          {
            args << (our_saxs_options->crysol_version_26
                         ? QFileInfo (use_pdb).fileName ()
                         : use_pdb)

                 << "/sm" << QString ("%1").arg (our_saxs_options->end_q)
        ...
        if (U_EXPT
          ...
          {
            US_File_Util ufu;
            if (!ufu.copy (((US_Hydrodyn *)us_hydrodyn)
                               ->gparams["saxs_crysol_target"],
                           dir))
              {
                editor_msg ("red", ufu.errormsg);
                if (ufu.errormsg.contains (QRegExp ("exists$")))
                  {
                    args << QFileInfo (((US_Hydrodyn *)us_hydrodyn)
                                           ->gparams["saxs_crysol_target"])
                                .fileName ();
                  }
              }
  ...
  connect (crysol, SIGNAL (readyReadStandardOutput ()), this,
           SLOT (crysol_readFromStdout ()));
  connect (crysol, SIGNAL (readyReadStandardError ()), this,
           SLOT (crysol_readFromStderr ()));
  connect (crysol, SIGNAL (finished (int, QProcess::ExitStatus)), this,
           SLOT (crysol_finished (int, QProcess::ExitStatus)));

#if defined(UHSE_APP_RESPONSE_WAY)
  if (crysol_manual_mode)
    {
      connect (&crysol_timer, SIGNAL (timeout ()), this,
               SLOT (crysol_timeout ()));
...
void
US_Hydrodyn_Saxs::crysol_timeout ()
{
...
void
US_Hydrodyn_Saxs::crysol_readFromStdout ()
{
#if QT_VERSION < 0x040000
  ...
  if (!crysol_run_to_end && crysol_app_text.size ())
    {
      ...
      if (crysol_query_response_pos >= (int)crysol_app_text.size ())
        {
          ...
          if (crysol_timer_delay_ms)
            {
              // us_qdebug( QString( "starting timer for %1 seconds" ).arg( (
              // double )crysol_timer_delay_ms / 1000e0 ) );
      ...
      if ((int)crysol_app_text.size () > crysol_query_response_pos
          && text.contains (crysol_app_text[crysol_query_response_pos]))
        {
          // us_qdebug( QString( "received <%1> from application" ).arg(
          // crysol_app_text[ crysol_query_response_pos ] ) );
          if (crysol_response[crysol_query_response_pos] != "___run___")
            {
              if (crysol_response[crysol_query_response_pos]
                      .left (2)
                      .contains ("__"))
                {
                  // us_qdebug(
                  //        QString( us_tr( "Error: undefined response <%1> to
                  //        query <%2>" ) ) .arg( crysol_response[
                  //        crysol_query_response_pos ] ) .arg(
                  //        crysol_app_text[ crysol_query_response_pos ] ) )
                  //    ;
                  crysol->kill ();
                  return;
                }
              // us_qdebug( QString( "sent     <%1> to application"   ).arg(
              // crysol_response[ crysol_query_response_pos ] ) );
...
void
US_Hydrodyn_Saxs::crysol_readFromStderr ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Saxs::crysol_finished (int, QProcess::ExitStatus)
{
  ...
  if (crysol_manual_mode)
    {
#if QT_VERSION < 0x040000
      QFile f (crysol->workingDirectory ().dirName () + QDir::separator ()
               + "output");
#else
...
void
US_Hydrodyn_Saxs::crysol_finishup ()
{
  QStringList intensity_lines
      = crysol_stdout.filter (QRegExp ("Intensities\\s+saved to file"));
  QStringList fit_lines
      = crysol_stdout.filter (QRegExp ("Data fit\\s+saved to file"));
  ...
  if (our_saxs_options->crysol_version_3)
    ...
    {
      created_dat
          = crysol_last_pdb_base.replace (QRegExp ("\\.(pdb|PDB)$"), "") + "00"
            + type;
    }
  ...
  if (!new_intensity_file.isEmpty ())
    {
      created_dat = QFileInfo (created_dat).path () + QDir::separator ()
                    + new_intensity_file;
    }
  ...
  if (!QFile::exists (created_dat))
    {
      editor_msg ("red",
                  QString (us_tr ("Error: Crysol did not create file %1"))
                      .arg (created_dat));
      pb_plot_saxs_sans->setEnabled (true);
      external_running = false;
      return;
  ...
  QString new_created_dat
      = ((US_Hydrodyn *)us_hydrodyn)->somo_dir + SLASH + "saxs" + SLASH
        + QFileInfo (crysol_last_pdb.replace (QRegExp ("\\.(pdb|PDB)$"), ""))
              .fileName ()
        + (selected_models[0] == 0
               ? QString ("")
               : QString ("_m%1").arg (selected_models[0] + 1))
        + iqq_suffix () + type;
...
void
US_Hydrodyn_Saxs::crysol_wroteToStdin ()
{
  // us_qdebug( "Crysol wroteToStdin" );
}

void
US_Hydrodyn_Saxs::crysol_started ()
{
...
int
US_Hydrodyn_Saxs::run_sans_iq_cryson (QString pdb)
{
  QString prog = USglobal->config_list.system_dir + SLASH +
#if defined(BIN64)
                 "bin64"
    ...
    if (!qfi.exists ())
      {
        editor_msg (
            "red",
            QString ("Cryson program '%1' does not exist\n").arg (prog));
        return -1;
      }
    if (!qfi.isExecutable ())
      {
        editor_msg (
            "red",
            QString ("Cryson program '%1' is not executable\n").arg (prog));
        return -1;
      }
  }
  ...
  if (!fi.exists ())
    {
      editor_msg ("red",
                  QString ("Cryson called but PDB file '%1' does not exist\n")
                      .arg (pdb));
      return -1;
    }
  ...
  if (!qd_dir.exists ())
    {
      editor_msg (
          "red",
          QString ("Cryson called but the directory '%1' does not exist\n")
              .arg (qd_dir.path ()));
      return -1;
    }
  ...
  if (!qd_dir.isReadable ())
    {
      editor_msg ("red", QString ("Cryson called but the directory '%1' is "
                                  "not readable. Check permissions\n")
                             .arg (qd_dir.path ()));
      return -1;
    }
  ...
  cryson_last_pdb = pdb;
  cryson_last_pdb_base = dir + SLASH
                         + QFileInfo (cryson_last_pdb)
                               .fileName ()
                               .replace (QRegExp ("\\.(pdb|PDB)$"), "")
                               .left (6)
                         + ".pdb";
  ...
  if (our_saxs_options->crysol_version_26
      && QFileInfo (cryson_last_pdb).fileName ()
             != QFileInfo (cryson_last_pdb_base).fileName ())
    {
      QFile f (pdb);
      if (!f.open (QIODevice::ReadOnly))
        {
  ...
  // {
  //    QString base =
  //    cryson_last_pdb_base.replace(QRegExp("\\.(pdb|PDB)$"),""); cout <<
  //    "base: <" << base << ">\n";
  ...
  cryson->addArgument (our_saxs_options->crysol_version_26
                           ? QFileInfo (use_pdb).fileName ()
                           : use_pdb);
  ...
  cryson->addArgument (QString ("%1").arg (
      (unsigned int)(our_saxs_options->end_q / our_saxs_options->delta_q)));
  ...
  if (our_saxs_options->cryson_manual_hs)
    {
      ...
      cryson->addArgument (QString ("%1").arg (
          our_saxs_options->cryson_hydration_shell_contrast));
    }
  ...
  args << (our_saxs_options->crysol_version_26
       ...
       << "/D2O" << QString ("%1").arg (our_saxs_options->d2o_conc);
  if (our_saxs_options->cryson_manual_hs)
    {
      args << "/dro"
           << QString ("%1").arg (
  ...
  args << "/lm"
       ...
       << "/fb"
       << QString ("%1").arg (
              our_saxs_options->cryson_sh_fibonacci_grid_order);
#endif
  ...
  connect (cryson, SIGNAL (readyReadStandardOutput ()), this,
           SLOT (cryson_readFromStdout ()));
  connect (cryson, SIGNAL (readyReadStandardError ()), this,
           SLOT (cryson_readFromStderr ()));
  connect (cryson, SIGNAL (finished (int, QProcess::ExitStatus)), this,
           SLOT (cryson_finished (int, QProcess::ExitStatus)));
...
void
US_Hydrodyn_Saxs::cryson_readFromStdout ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Saxs::cryson_readFromStderr ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Saxs::cryson_finished (int, QProcess::ExitStatus)
{
  //   for ( int i = 0; i < 10000; i++ )
  //   {
  ...
  QString created_dat
      = cryson_last_pdb_base.replace (QRegExp ("\\.(pdb|PDB)$"), "")
        + "00.int";

  if (!QFile::exists (created_dat))
    {
      editor_msg ("red",
                  QString (us_tr ("Error: Cryson did not create file %1"))
                      .arg (created_dat));
      pb_plot_saxs_sans->setEnabled (true);
      external_running = false;
      return;
  ...
  QString new_created_dat
      = ((US_Hydrodyn *)us_hydrodyn)->somo_dir + SLASH + "saxs" + SLASH
        + QFileInfo (cryson_last_pdb.replace (QRegExp ("\\.(pdb|PDB)$"), ""))
              .fileName ()
        + iqq_suffix () + ".int";
...
void
US_Hydrodyn_Saxs::cryson_started ()
{
...
int
US_Hydrodyn_Saxs::run_saxs_iq_sastbx (QString pdb)
{
  QString prog = USglobal->config_list.system_dir + SLASH +
#if defined(BIN64)
                 "bin64"
    ...
    if (!qfi.exists ())
      {
        editor_msg (
            "red",
            QString ("Sastbx program '%1' does not exist\n").arg (prog));
        return -1;
      }
    if (!qfi.isExecutable ())
      {
        editor_msg (
            "red",
            QString ("Sastbx program '%1' is not executable\n").arg (prog));
        return -1;
      }
  }
  ...
  if (!fi.exists ())
    {
      editor_msg ("red",
                  QString ("Sastbx called but PDB file '%1' does not exist\n")
                      .arg (pdb));
      return -1;
    }
...
#else
  QStringList args;
  args << QString ("structure=%1").arg (pdb);
#endif
  ...
  sastbx->addArgument (QString ("n_step=%1")
                           .arg ((unsigned int)(our_saxs_options->end_q
                                                / our_saxs_options->delta_q)));
  sastbx->addArgument (
      QString ("rho=%1").arg (our_saxs_options->water_e_density));
  sastbx->addArgument (QString ("drho=%1").arg (
      our_saxs_options->crysol_hydration_shell_contrast));
  sastbx->addArgument (
...
#else
  args << QString ("method=%1").arg (method)
...
#endif
  connect (sastbx, SIGNAL (readyReadStandardOutput ()), this,
           SLOT (sastbx_readFromStdout ()));
  connect (sastbx, SIGNAL (readyReadStandardError ()), this,
           SLOT (sastbx_readFromStderr ()));
  connect (sastbx, SIGNAL (finished (int, QProcess::ExitStatus)), this,
           SLOT (sastbx_finished (int, QProcess::ExitStatus)));
...
void
US_Hydrodyn_Saxs::sastbx_readFromStdout ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Saxs::sastbx_readFromStderr ()
{
#if QT_VERSION < 0x040000
...
void
US_Hydrodyn_Saxs::sastbx_finished (int, QProcess::ExitStatus)
{
  //   for ( int i = 0; i < 10000; i++ )
  //   {
  ...
  if (!QFile::exists (created_dat))
    {
      editor_msg ("red",
                  QString (us_tr ("Error: Sastbx did not create file %1"))
                      .arg (created_dat));
      pb_plot_saxs_sans->setEnabled (true);
      external_running = false;
      return;
  ...
  QString new_created_dat
      = ((US_Hydrodyn *)us_hydrodyn)->somo_dir + SLASH + "saxs" + SLASH
        + QFileInfo (sastbx_last_pdb).fileName () + iqq_suffix () + ".int";
...
void
US_Hydrodyn_Saxs::sastbx_started ()
{
