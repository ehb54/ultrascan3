...
dad_stack_data US_Hydrodyn_Dad::current_data(bool selected_only) {
  dad_stack_data tmp_stack;
  if (selected_only) {
...
void US_Hydrodyn_Dad::stack_push_all() {
  disable_all();
...
void US_Hydrodyn_Dad::stack_push_sel() {
  disable_all();
...
void US_Hydrodyn_Dad::set_current_data(dad_stack_data& tmp_stack) {
  lb_files->clear();
...
void US_Hydrodyn_Dad::stack_drop() {
...
void US_Hydrodyn_Dad::stack_rot_up() {
  vector<dad_stack_data> new_stack;
...
void US_Hydrodyn_Dad::stack_rot_down() {
  vector<dad_stack_data> new_stack;
...
void US_Hydrodyn_Dad::stack_swap() {
  dad_stack_data tmp_stack = current_data();
  disable_updates = true;
  set_current_data(stack_data.back());
...
void US_Hydrodyn_Dad::stack_join() { stack_join(stack_data.back()); }

void US_Hydrodyn_Dad::stack_join(dad_stack_data& tmp_stack) {
  // join all files in stack_data.back() with current
  dad_stack_data current = current_data();
...
void US_Hydrodyn_Dad::stack_copy() {
  clipboard = current_data(true);
  update_enables();
}

void US_Hydrodyn_Dad::stack_pcopy() {
  dad_stack_data adds = current_data(true);
...
void US_Hydrodyn_Dad::stack_paste() {
  disable_all();
  stack_join(clipboard);
}

void US_Hydrodyn_Dad::stack_clear() {
  disable_all();
