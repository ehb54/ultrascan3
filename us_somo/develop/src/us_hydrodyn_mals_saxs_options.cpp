...
void
US_Hydrodyn_Mals_Saxs_Options::setupGUI ()
{
  int minHeight1 = 28;
  ...
  rb_linear->setChecked (parameters->count ("mals_saxs_bl_linear")
                         && (*parameters)["mals_saxs_bl_linear"] == "true");
  ...
  rb_integral->setChecked (parameters->count ("mals_saxs_bl_integral")
  ...
  cb_save_bl->setChecked (parameters->count ("mals_saxs_bl_save")
                          && (*parameters)["mals_saxs_bl_save"] == "true");
  ...
  le_smooth->setText (parameters->count ("mals_saxs_bl_smooth")
  ...
  le_start_region->setText (parameters->count ("mals_saxs_bl_start_region")
  ...
  le_i_power->setText (parameters->count ("mals_saxs_bl_i_power")
  ...
  le_reps->setText (parameters->count ("mals_saxs_bl_reps")
  ...
  le_epsilon->setText (parameters->count ("mals_saxs_bl_epsilon")
  ...
  le_cormap_maxq->setText (parameters->count ("mals_saxs_cormap_maxq")
  ...
  le_cormap_alpha->setText (parameters->count ("mals_saxs_cormap_alpha")
  ...
  lbl_interp_method->setFont (QFont (USglobal->config_list.fontFamily,
                                     USglobal->config_list.fontSize,
                                     QFont::Bold));
  ...
  cb_interp_method->setFont (QFont (USglobal->config_list.fontFamily,
                                    USglobal->config_list.fontSize - 1));
  ...
  cb_interp_method->addItem (us_tr ("Cubic Spline"),
                             INTERP_METHOD_CUBIC_SPLINE);
  ...
  pb_mals_saxs_parameters
      = new QPushButton (us_tr ("MALS+SAXS Processing Parameters"), this);
  pb_mals_saxs_parameters->setFont (QFont (
  ...
  connect (pb_mals_saxs_parameters, SIGNAL (clicked ()),
           SLOT (set_mals_saxs_parameters ()));
  ...
  cb_csv_transposed->setChecked ((*parameters)["mals_saxs_csv_transposed"]
                                 == "true");
  ...
  cb_discard_it_sd_mult->setChecked (
      (*parameters)["mals_saxs_cb_discard_it_sd_mult"] == "true");
  ...
  cb_gg_oldstyle->setChecked ((*parameters)["mals_saxs_cb_gg_oldstyle"]
                              == "true");
  ...
  cb_lock_min_retry->setChecked ((*parameters)["mals_saxs_lock_min_retry"]
                                 == "true");
  ...
  le_lock_min_retry_mult->setText (
  ...
  cb_maxfpk_restart->setChecked ((*parameters)["mals_saxs_maxfpk_restart"]
                                 == "true");
  ...
  cb_makeiq_cutmax_pct->setChecked (
      (*parameters)["mals_saxs_cb_makeiq_cutmax_pct"] == "true");
  ...
  cb_makeiq_avg_peaks->setChecked (
      (*parameters)["mals_saxs_cb_makeiq_avg_peaks"] == "true");
...
void
US_Hydrodyn_Mals_Saxs_Options::quit ()
{
  close ();
}

bool
US_Hydrodyn_Mals_Saxs_Options::any_changes ()
{
  ...
  return (*parameters)["mals_saxs_bl_linear"]
...
void
US_Hydrodyn_Mals_Saxs_Options::ok ()
{
  (*parameters)["ok"] = "true";
  ...
  (*parameters)["mals_saxs_interp_method"]
      = QString ("%1").arg (cb_interp_method->currentData ().toInt ());

  if (rb_gauss->isChecked ())
    {
      (*parameters)["gaussian_type"]
          = QString ("%1").arg (US_Hydrodyn_Mals_Saxs::GAUSS);
    }
  if (rb_gmg->isChecked ())
    {
      (*parameters)["gaussian_type"]
          = QString ("%1").arg (US_Hydrodyn_Mals_Saxs::GMG);
    }
  if (rb_emg->isChecked ())
    {
      (*parameters)["gaussian_type"]
          = QString ("%1").arg (US_Hydrodyn_Mals_Saxs::EMG);
    }
  if (rb_emggmg->isChecked ())
    {
      (*parameters)["gaussian_type"]
          = QString ("%1").arg (US_Hydrodyn_Mals_Saxs::EMGGMG);
    }
...
void
US_Hydrodyn_Mals_Saxs_Options::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Mals_Saxs_Options::closeEvent (QCloseEvent *e)
{
  if (any_changes ())
    {
      QMessageBox mb (
...
void
US_Hydrodyn_Mals_Saxs_Options::update_enables ()
{
  ...
  pb_clear_gauss->setEnabled (
      ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->any_gaussians ());
  le_epsilon->setEnabled (rb_integral->isChecked ());
...
void
US_Hydrodyn_Mals_Saxs_Options::clear_gauss ()
{
  if (QMessageBox::Yes
...
void
US_Hydrodyn_Mals_Saxs_Options::set_detector ()
{
  {
    map<QString, QString> parameters;
    parameters["uv_conv"] = QString ("%1").arg (
        ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->detector_uv_conv, 0, 'g', 8);
    parameters["ri_conv"] = QString ("%1").arg (
        ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->detector_ri_conv, 0, 'g', 8);
    ...
    ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->detector_uv
        = (parameters.count ("uv") && parameters["uv"] == "true") ? true
                                                                  : false;
    ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->detector_ri
        = (parameters.count ("ri") && parameters["ri"] == "true") ? true
                                                                  : false;
    ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->detector_ri_conv
        = parameters["ri_conv"].toDouble ();
    ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->detector_uv_conv
        = parameters["uv_conv"].toDouble ();
  }
}

void
US_Hydrodyn_Mals_Saxs_Options::set_mals_saxs_parameters ()
{
  {
    map<QString, QString> parameters;
    parameters["mals_saxs_param_lambda"] = QString ("%1").arg (
        ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->mals_saxs_param_lambda, 0,
        'g', 8);
    parameters["mals_saxs_param_n"] = QString ("%1").arg (
        ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->mals_saxs_param_n, 0, 'g',
        8);
    parameters["mals_saxs_param_g_dndc"] = QString ("%1").arg (
        ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->mals_saxs_param_g_dndc, 0,
        'g', 8);
    parameters["mals_saxs_param_g_extinction_coef"]
        = QString ("%1").arg (((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)
                                  ->mals_saxs_param_g_extinction_coef,
                              0, 'g', 8);
    parameters["mals_saxs_param_g_conc"] = QString ("%1").arg (
        ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->mals_saxs_param_g_conc, 0,
        'g', 8);
    parameters["mals_saxs_param_DLS_detector"]
        = QString ("%1").arg (((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)
                                  ->mals_saxs_param_DLS_detector);

    US_Hydrodyn_Mals_Saxs_Parameters *mals_saxs_parameters
        = new US_Hydrodyn_Mals_Saxs_Parameters (this, &parameters, this);
    ...
    ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->mals_saxs_param_lambda
        = parameters["mals_saxs_param_lambda"].toDouble ();
    ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->mals_saxs_param_n
        = parameters["mals_saxs_param_n"].toDouble ();
    ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->mals_saxs_param_g_dndc
        = parameters["mals_saxs_param_g_dndc"].toDouble ();
    ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->mals_saxs_param_g_extinction_coef
        = parameters["mals_saxs_param_g_extinction_coef"].toDouble ();
    ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->mals_saxs_param_g_conc
        = parameters["mals_saxs_param_g_conc"].toDouble ();
    ((US_Hydrodyn_Mals_Saxs *)mals_saxs_win)->mals_saxs_param_DLS_detector
        = parameters["mals_saxs_param_DLS_detector"].toInt ();
  }
}

void
US_Hydrodyn_Mals_Saxs_Options::fasta_file ()
{
  // load file
...
void
US_Hydrodyn_Mals_Saxs_Options::update_fasta_pH (const QString &str)
{
