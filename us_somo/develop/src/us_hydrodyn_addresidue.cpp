...
void US_AddResidue::setupGUI() {
  ...
  lbl_select_beadatom =
      new QLabel(us_tr(" Select Atom for Bead (multi-selection OK): "), this);
  ...
  le_bead_hydro_from_atom->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
...
void US_AddResidue::add() {
  ...
  if (cmp1 != cmp2) {
    switch (QMessageBox::warning(
        ...
        us_tr("&Correct the bead volume manually"), us_tr("&Override"), 0, 0,
        0)) {
  ...
  for (i = 0; i < residue_list.size(); i++) {
    if (residue_list[i].name.toUpper() == new_residue.name.toUpper() &&
        residue_list[i].comment.toUpper() == new_residue.comment.toUpper()) {
      item = (int)i;
...
void US_AddResidue::select_residue_file() {
  ...
  if (residue_filename.isEmpty()) {
    ...
    if (f.open(QIODevice::ReadWrite | QIODevice::Text)) {
      ...
      //                QMessageBox::No,
      //                QMessageBox::Cancel | QMessageBox::Escape, 0, 0, false,
      //                0);
      // switch (mb.exec())
      ...
      if ((*res).r_atom[j].bead_assignment == i) {
        ...
        if (res->r_atom[j].ionization_index &&
          ...
          res->r_bead[i].atom_hydration2 +=
              res->r_atom_1[res->r_atom[j].ionization_index].hydration;
          res->r_bead[i].mw2 +=
              res->r_atom_1[res->r_atom[j].ionization_index].hybrid.mw;
        } else {
...
void US_AddResidue::update_name(const QString &str) { new_residue.name = str; }
...
void US_AddResidue::select_beadatom() {
  ...
  for (i = 0; i < (unsigned int)lb_select_beadatom->count(); i++) {
    if (new_residue.r_atom[i].bead_assignment == current_bead) {
      new_residue.r_atom[i].bead_assignment =
          100000;  // reset all atoms previously assigned to this bead to a
                   // different value
    }
  }
  for (i = 0; i < (unsigned int)lb_select_beadatom->count(); i++) {
  ...
  if (new_residue.r_bead[current_bead].atom_hydration ==
    ...
    cnt_hydration->setEnabled(false);
    cnt_hydration->setToolTip(
        us_tr("Disabled because pKa values exist for at least one atom in this "
              "bead<br>"));
...
void US_AddResidue::select_r_bead(int val) {
  ...
  if (1 || existing_residue) {
    ...
    if (enable_assign_hydration) {
      ...
      cnt_hydration->setEnabled(false);
      cnt_hydration->setToolTip(
          us_tr("Disabled because hydration or pKa values exist for at least "
                "one atom in this residue<br>"));
      cb_hydration->setToolTip(
          us_tr("Disabled because hydration or pKa values exist for at least "
                "one atom in this residue<br>"));
      str2.sprintf("[pH 0]: %7.2f  [pH 14]: %7.2f",
    ...
    //    h_volume2 ); str2.sprintf("[pH 0]: %7.2f A  [pH 14]: %7.2f A", radius,
    //    radius2 );
    // }
...
void US_AddResidue::accept_residue() {
  ...
  if (new_residue.name.isEmpty() || new_residue.r_atom.size() == 0 ||
    ...
    QMessageBox::warning(
        this, us_tr("UltraScan Warning"),
        us_tr(
            "Please define all residue fields\nbefore accepting the residue!"),
        QMessageBox::Ok, QMessageBox::NoButton, QMessageBox::NoButton);
  } else {
    le_residue_name->setEnabled(false);
    le_residue_comment->setEnabled(false);
...
void US_AddResidue::accept_atom() {
  ...
  str.sprintf(qPrintable("Atom %d: " + atom_list[current_item2].name),
              current_item1 + 1);
  str +=
      " (" + new_residue.r_atom[current_item1].hybrid.name + ", Positioning: ";
...
void US_AddResidue::info_residue(const QString &msg) {
  ...
  for (unsigned int j = 0; j < new_residue.r_bead.size(); j++) {
    TSO << new_residue.r_bead[j].hydration << "\t"
        ...
        << new_residue.r_bead[j].chain << "\t" << new_residue.r_bead[j].volume
        << "\n";
  }
  ...
  for (auto it = new_residue.r_atom_1.begin(); it != new_residue.r_atom_1.end();
       ++it) {
...
bool US_AddResidue::update_pKas(int atomno) {
  ...
  if (ionization_index) {
    ...
    if (cb_enable_pKa->isChecked()) {
      ...
      pKas_need_sort = new_residue.pKas.size() > 1 &&
  ...
  for (int i = 0; i < (int)new_residue.r_atom.size(); ++i) {
    if (atom_name_to_no[new_residue.r_atom[i].name] &&
      ...
      errors += QString(
                    "Duplicate atom name %1 found in atom %2, atom names must "
                    "be unique\n")
  ...
  if (ok && pKas_need_sort) {
    ...
    for (auto it = new_residue.r_atom_1.begin();
      ...
      if ((int)new_residue.pKas.size() < it->first) {
        TSO << "ERROR new_residue.pKas don't contain expected ionization "
               "index\n";
        ok = false;
      } else {
  ...
  if (new_residue.pKas.size() > 2) {
    errors += QString(
                  "Currently only 2 atoms with pKa enabled are supported, you "
                  "have defined %1\n")
...
void US_AddResidue::set_chain(int val) { new_bead.chain = val; }
...
void US_AddResidue::write_residue_file() {
  ...
  if (f.open(QIODevice::WriteOnly | QIODevice::Text)) {
...

    {
      ...
      for (auto it = sort_residues.begin(); it != sort_residues.end(); ++it) {
    ...
    for (unsigned int i = 0; i < residue_list.size(); i++) {
      ...
      for (int j = 0; j < (int)residue_list[i].pKas.size(); ++j) {
        ts << "\t" << residue_list[i].vbar << "\t" << residue_list[i].pKas[j];
      }
