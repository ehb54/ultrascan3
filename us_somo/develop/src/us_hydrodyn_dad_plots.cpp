...
void
US_Hydrodyn_Dad::update_plot_errors_group ()
{
  ...
  qDebug () << QString ("").sprintf ("dad_plots::update_plot_errors_group\n"
                                     "\tminx %e maxx %e\n"
                                     "\tminy %e maxy %e\n",
                                     minx, maxx, -maxy, maxy);
#endif
...
void
US_Hydrodyn_Dad::update_plot_errors (vector<double> &grid,
                                     ...
                                     vector<double> &errors, QColor use_color)
{
  ...
  // if ( !plot_errors_zoomer )
  {
...
#if defined(DEBUG_RESCALE)
    {
      ...
      tso << QString ("").sprintf ("dad_plots::update_plot_errors\n"
                                   "\tminx %e maxx %e\n"
                                   "\tminy %e maxy %e\n",
    ...
    //       plot_errors_zoomer = new ScrollZoomer(plot_errors->canvas());
    //       plot_errors_zoomer->setRubberBandPen(QPen(Qt::yellow, 0,
    //       Qt::DotLine)); connect( plot_errors_zoomer, SIGNAL( zoomed( const
    //       QRectF & ) ), SLOT( plot_zoomed( const QRectF & ) ) );
  }
...
void
US_Hydrodyn_Dad::redo_plot_errors ()
{
  if (current_mode == MODE_GGAUSSIAN || current_mode == MODE_GAUSSIAN)
    {
      plot_errors->setAxisTitle (
...
void
US_Hydrodyn_Dad::set_plot_errors_rev ()
{
...
void
US_Hydrodyn_Dad::set_plot_errors_sd ()
{
...
void
US_Hydrodyn_Dad::set_plot_errors_pct ()
{
...
void
US_Hydrodyn_Dad::set_plot_errors_group ()
{
  redo_plot_errors ();
}

void
US_Hydrodyn_Dad::plot_errors_jump_markers ()
{
...
void
US_Hydrodyn_Dad::resize_plots ()
{
  // qDebug() << "dad::resize_plots()";
...
void
US_Hydrodyn_Dad::resize_guinier_plots ()
{
...
bool
US_Hydrodyn_Dad::compatible_grids (QStringList files)
{
...
void
US_Hydrodyn_Dad::errors_multi_file (QStringList files)
{
  // check
...
void
US_Hydrodyn_Dad::set_dots ()
{
  set_eb ();
}

void
US_Hydrodyn_Dad::set_eb ()
{
...
void
US_Hydrodyn_Dad::plot_files (bool save_zoom_state)
{
  qDebug () << "plot files";
  if (suppress_plot)
    {
  ...
  if (current_mode != MODE_POWERFIT && current_mode != MODE_BASELINE2)
    {
...
#if defined(DEBUG_RESCALE)
  {
    ...
    tso << QString ("").sprintf ("dad_plots::plot_files\n"
                                 "\tminx %e maxx %e\n"
                                 "\tminy %e maxy %e\n",
                                 minx, maxx, miny, maxy);
...
bool
US_Hydrodyn_Dad::get_min_max (QString file, double &minx, double &maxx,
...
#define PLOT_COLOR(file)                                                      \
  (plot_color.count (file) ? plot_color[file]                                 \
                           : plot_colors[f_pos[file] % plot_colors.size ()])
#define PLOT_STYLE(file, default)                                             \
  (plot_style.count (file) ? plot_style[file] : default)
#define PLOT_WIDTH(file)                                                      \
  (plot_width_multiplier.count (file)                                         \
       ? use_line_width * plot_width_multiplier[file]                         \
       : use_line_width)

bool
US_Hydrodyn_Dad::plot_file (QString file, double &minx, double &maxx,
                            double &miny, double &maxy)
{
  if (!f_qs_string.count (file) || !f_qs.count (file) || !f_Is.count (file)
      || !f_pos.count (file))
      ...
      if (!axis_y_log)
        {
          ...
          curve->setPen (QPen (PLOT_COLOR (file), PLOT_WIDTH (file),
                               PLOT_STYLE (file, Qt::SolidLine)));
          if (!plot_style.count (file))
            {
              curve->setStyle (QwtPlotCurve::NoCurve);
              symbol.setPen (
                  QPen (PLOT_COLOR (file), PLOT_WIDTH (file), Qt::SolidLine));
              curve->setSymbol (new QwtSymbol (symbol.style (),
                                               symbol.brush (), symbol.pen (),
                                               symbol.size ()));
            }
          ...
          for (unsigned int i = 0; i < q_points; i++)
            {
              ...
              curveeb->setPen (
                  QPen (PLOT_COLOR (file), use_line_width, Qt::SolidLine));
  ...
  // US_Vector::printvector2( QString( "plot_file( %1 ) exclude points" ).arg(
  // file ), exclude_x, exclude_y );
  ...
  if (exclude_x.size ())
    {
      ...
      curve->setSamples ((double *)&(exclude_x[0]), (double *)&(exclude_y[0]),
                         exclude_x.size ());
...
US_Hydrodyn_Dad::rescale ()
{
...
#if defined(DEBUG_RESCALE)
  {
    ...
    tso << QString ("").sprintf ("dad_plots::rescale\n"
                                 "\tminx %e maxx %e\n"
                                 "\tminy %e maxy %e\n",
                                 minx, maxx, miny, maxy);
    tso << "zoomrect " << plot_dist_zoomer->zoomRect ().left () << " , "
...
void
US_Hydrodyn_Dad::rescale_y_plot_errors ()
{
