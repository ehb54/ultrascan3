...
void US_Hydrodyn_Dad::update_plot_errors_group() {
...
void US_Hydrodyn_Dad::update_plot_errors(vector<double> &grid,
...
void US_Hydrodyn_Dad::redo_plot_errors() {
  if (current_mode == MODE_GGAUSSIAN || current_mode == MODE_GAUSSIAN) {
    plot_errors->setAxisTitle(
        QwtPlot::yLeft,
        us_tr(cb_plot_errors_pct->isChecked()
...
void US_Hydrodyn_Dad::set_plot_errors_rev() {
...
void US_Hydrodyn_Dad::set_plot_errors_sd() {
...
void US_Hydrodyn_Dad::set_plot_errors_pct() {
...
void US_Hydrodyn_Dad::set_plot_errors_group() { redo_plot_errors(); }
...
void US_Hydrodyn_Dad::resize_plots() {
...
void US_Hydrodyn_Dad::resize_guinier_plots() {
...
bool US_Hydrodyn_Dad::compatible_grids(QStringList files) {
...
void US_Hydrodyn_Dad::errors_multi_file(QStringList files) {
  // check
...
void US_Hydrodyn_Dad::set_dots() { set_eb(); }
...
bool US_Hydrodyn_Dad::get_min_max(QString file, double &minx, double &maxx,
...
bool US_Hydrodyn_Dad::plot_file(QString file, double &minx, double &maxx,
                                double &miny, double &maxy) {
  if (!f_qs_string.count(file) || !f_qs.count(file) || !f_Is.count(file) ||
      !f_pos.count(file)) {
    editor_msg(
  ...
  if (exclude_x.size()) {
    ...
    curve->setSamples((double *)&(exclude_x[0]), (double *)&(exclude_y[0]),
                      exclude_x.size());
    curve->setSymbol(new QwtSymbol(symbol.style(), symbol.brush(), symbol.pen(),
                                   symbol.size()));
    curve->attach(plot_dist);
  }
  return true;
...
void US_Hydrodyn_Dad::rescale_y() {
...
void US_Hydrodyn_Dad::rescale_y_plot_errors() {
