...
bool
US_Hydrodyn::atom_align (vector<point> transform_from,
                         vector<point> transform_to, vector<point> apply_from,
                         vector<point> &result, QString &error_msg,
                         bool force_zero_center)
{
  ...
  if (transform_from.size () != transform_to.size ())
    {
      error_msg = QString (us_tr ("Size of transform from (%1) does not equal "
                                  "size of transform to (%2)"))
