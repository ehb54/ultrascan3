...
void US_Hydrodyn_Mals_Saxs_Nth::setupGUI() {
  bool started_in_expert_mode =
      ((US_Hydrodyn_Mals_Saxs *)us_hydrodyn_mals_saxs)->started_in_expert_mode;
  ...
  if (paired_restore_valid &&
      original_selections ==
          MALS_SAXS->qsl_to_set(MALS_SAXS->saved_nth_last_paired_selections)) {
    paired_store_valid = false;
  }
      ...
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  {
    QIntValidator *qdv = new QIntValidator(
        1, ((US_Hydrodyn_Mals_Saxs *)us_hydrodyn_mals_saxs)->lb_files->count(),
        le_n);
  ...
  lbl_start_name = new QLabel(((US_Hydrodyn_Mals_Saxs *)us_hydrodyn_mals_saxs)
  ...
  {
    QIntValidator *qdv = new QIntValidator(
        2, ((US_Hydrodyn_Mals_Saxs *)us_hydrodyn_mals_saxs)->lb_files->count(),
        le_end);
  ...
  lbl_end_name = new QLabel(((US_Hydrodyn_Mals_Saxs *)us_hydrodyn_mals_saxs)
...
void US_Hydrodyn_Mals_Saxs_Nth::quit() { close(); }
...
bool US_Hydrodyn_Mals_Saxs_Nth::set_paired_ranges() {
  ...
  if (use_names.count(MALS_SAXS->saved_nth_pair_names[0][0])) {
    for (int i = 0; i < (int)MALS_SAXS->saved_nth_pair_names[0].size(); ++i) {
      // qDebug() << QString( "pos %1 : curve %2 pos %3 paired to curve %4 pos
      // %5")
      //    .arg( i )
      ...
      //    ;
      paired_map_to_pos[names_to_pos[MALS_SAXS->saved_nth_pair_names[0][i]]] =
    ...
    for (int i = 0; i < (int)MALS_SAXS->saved_nth_pair_names[0].size(); ++i) {
      ...
      //    .arg( names_to_pos[ MALS_SAXS->saved_nth_pair_names[0][i] ] )
      //    ;
      paired_map_to_pos[names_to_pos[MALS_SAXS->saved_nth_pair_names[1][i]]] =
