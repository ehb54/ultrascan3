    ...
    : QDialog (p)
{
  ...
  alpha = parameters->count ("alpha") ? (*parameters)["alpha"].toDouble ()
                                      : 0.05;
...
void
US_Hydrodyn_Mals_Saxs_Nth::setupGUI ()
{
  ...
  for (int i = 0; i < MALS_SAXS->lb_files->count (); ++i)
    {
      lb_files->addItem (QString ("%1 : %2").arg (i + 1).arg (
          MALS_SAXS->lb_files->item (i)->text ()));
  ...
  if (paired_restore_valid
      && original_selections
             == MALS_SAXS->qsl_to_set (
                 MALS_SAXS->saved_nth_last_paired_selections))
    {
      paired_store_valid = false;
    }
  ...
  pb_paired_store->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize));
  ...
  pb_paired_restore = new QPushButton (
      us_tr ("Restore previously stored paired selections"), this);
  pb_paired_restore->setFont (QFont (USglobal->config_list.fontFamily,
                                     USglobal->config_list.fontSize));
  ...
  cb_paired_limit->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize - 1));
                        ...
                        USglobal->config_list.fontSize));
  {
    QIntValidator *qdv = new QIntValidator (
        1,
        ((US_Hydrodyn_Mals_Saxs *)us_hydrodyn_mals_saxs)->lb_files->count (),
        le_n);
  ...
  lbl_start_name
      = new QLabel (((US_Hydrodyn_Mals_Saxs *)us_hydrodyn_mals_saxs)
  ...
  le_contains->setText (parameters->count ("mals_saxs_nth_contains")
  ...
  AUTFBACK (lbl_pvalue);
  lbl_pvalue->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize, QFont::Bold));

  cb_pvalues_green = new QCheckBox (this);
  cb_pvalues_green->setText (us_tr ("Green P values"));
  cb_pvalues_green->setChecked (true);
  cb_pvalues_green->setFont (QFont (USglobal->config_list.fontFamily,
                                    USglobal->config_list.fontSize - 1));
  ...
  cb_pvalues_yellow->setFont (QFont (USglobal->config_list.fontFamily,
                                     USglobal->config_list.fontSize - 1));
  ...
  cb_pvalues_red->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize - 1));
  ...
  pb_pvalues_only->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize));
  ...
  pb_pvalues_add->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize));
  ...
  if (!ggaussian_last_pfit_map->size () || !started_in_expert_mode)
    {
      lbl_pvalue->hide ();
...
void
US_Hydrodyn_Mals_Saxs_Nth::quit ()
{
  close ();
}

void
US_Hydrodyn_Mals_Saxs_Nth::go ()
{
  (*parameters)["go"] = "true";
...
void
US_Hydrodyn_Mals_Saxs_Nth::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Mals_Saxs_Nth::closeEvent (QCloseEvent *e)
{
...
void
US_Hydrodyn_Mals_Saxs_Nth::update_enables ()
{
  int fcnt
      = ((US_Hydrodyn_Mals_Saxs *)us_hydrodyn_mals_saxs)->lb_files->count ();
  ...
  if (ofs >= 1 && ofs <= fcnt)
    {
      lbl_start_name->setText (((US_Hydrodyn_Mals_Saxs *)us_hydrodyn_mals_saxs)
                                   ->lb_files->item (ofs - 1)
                                   ->text ());
    }
  else
  ...
  if (end >= 1 && end <= fcnt)
    {
      lbl_end_name->setText (((US_Hydrodyn_Mals_Saxs *)us_hydrodyn_mals_saxs)
                                 ->lb_files->item (end - 1)
                                 ->text ());
    }
  else
  ...
  for (int i = 0; i < lb_files->count (); ++i)
    {
      ...
      if (((US_Hydrodyn_Mals_Saxs *)us_hydrodyn_mals_saxs)
              ->lb_files->item (i)
              ->text ()
      ...
      {
        QString qs = ((US_Hydrodyn_Mals_Saxs *)us_hydrodyn_mals_saxs)
                         ->lb_files->item (i)
                         ->text ();
        if (ggaussian_last_pfit_map->count (qs))
          {
            ...
            if ((cb_pvalues_green->isChecked () && pv >= alpha)
                || (cb_pvalues_yellow->isChecked () && pv >= alpha_over_5
                    && pv < alpha)
  ...
  for (set<int>::iterator it = pvalues.begin (); it != pvalues.end (); it++)
    {
      if (!selected.count (*it))
        {
  ...
  pb_pvalues_only->setEnabled (any_pvalues_not_selected
                               || any_selected_not_pvalues);
  pb_pvalues_add->setEnabled (any_pvalues_not_selected
                              && any_selected_not_pvalues);
  ...
  lb_files->setSelectionMode (cb_paired_limit->isChecked ()
...
void
US_Hydrodyn_Mals_Saxs_Nth::nth_only ()
{
...
void
US_Hydrodyn_Mals_Saxs_Nth::nth_add ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  int n = le_n->text ().toInt ();
  int ofs = le_start->text ().toInt ();
...
void
US_Hydrodyn_Mals_Saxs_Nth::contains_only ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  lb_files->clearSelection ();
  lb_files->setSelectionMode (QListWidget::ExtendedSelection);
...
void
US_Hydrodyn_Mals_Saxs_Nth::contains_add ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  lb_files->setSelectionMode (QListWidget::ExtendedSelection);
  for (int i = 0; i < lb_files->count (); ++i)
...
void
US_Hydrodyn_Mals_Saxs_Nth::pvalues_only ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  lb_files->clearSelection ();
  lb_files->setSelectionMode (QListWidget::ExtendedSelection);
...
void
US_Hydrodyn_Mals_Saxs_Nth::pvalues_add ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  lb_files->setSelectionMode (QListWidget::ExtendedSelection);
  for (int i = 0; i < lb_files->count (); ++i)
...
void
US_Hydrodyn_Mals_Saxs_Nth::update_files_selected ()
{
  lb_files_sel->clear ();
  for (int i = 0; i < lb_files->count (); ++i)
    {
...
void
US_Hydrodyn_Mals_Saxs_Nth::paired_store ()
{
  // store paired selections
  qDebug () << "paired_store()";
  MALS_SAXS->saved_nth_last_paired_selections
      = MALS_SAXS->set_to_qsl (original_selections);
  paired_store_valid = false;
  update_enables ();
}

void
US_Hydrodyn_Mals_Saxs_Nth::paired_restore ()
{
  ...
  MALS_SAXS->check_files_selected_paired (
      MALS_SAXS->saved_nth_last_paired_selections);
  set_paired_ranges ();
  update_enables ();
}
...
void
US_Hydrodyn_Mals_Saxs_Nth::paired_limit ()
{
  ...
  if (!cb_paired_limit->isChecked ())
    {
      paired_limit_valid = false;
      paired_store_valid = false;
      ((QIntValidator *)le_start->validator ())
          ->setRange (1, lb_files->count ());
      ((QIntValidator *)le_end->validator ())
          ->setRange (1, lb_files->count ());
    }
  qDebug () << "paired_limit()";
  update_enables ();
...
bool
US_Hydrodyn_Mals_Saxs_Nth::set_paired_ranges ()
{
  ...
  if (use_names.count (MALS_SAXS->saved_nth_pair_names[0][0]))
    {
      for (int i = 0; i < (int)MALS_SAXS->saved_nth_pair_names[0].size (); ++i)
        {
          // qDebug() << QString( "pos %1 : curve %2 pos %3 paired to curve %4
          // pos %5")
          //    .arg( i )
          ...
          //    ;
          paired_map_to_pos
              [names_to_pos[MALS_SAXS->saved_nth_pair_names[0][i]]]
              = names_to_pos[MALS_SAXS->saved_nth_pair_names[1][i]];
        }
    }
  else
    {
  ...
  // not qintvalidators don't really validate the range, just the number of
  // digits

  ((QIntValidator *)le_start->validator ())
      ->setRange (use_pos_start, use_pos_end);
  ((QIntValidator *)le_end->validator ())
      ->setRange (use_pos_start, use_pos_end);
