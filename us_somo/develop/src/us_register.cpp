...
#define TITLE "Silicon Graphics"
      ...
      if (register_list.platform == "opteron")
        selected_platform = "Intel/AMD 64-bit";
      if (register_list.platform == "intel")
        selected_platform = "Intel/AMD 32-bit";
      if (register_list.platform == "sparc")
        selected_platform = "Sun Sparc";
      if (register_list.platform == "mac")
        selected_platform = "Macintosh";
      if (register_list.platform == "sgi")
        selected_platform = "Silicon Graphics";
      if (register_list.platform == "generic")
        selected_platform = "Generic";

      if (register_list.platform != PLATFORM
          && register_list.platform != "generic")
        {
          str = us_tr ("You are running UltraScan on the " TITLE " platform,\n"
...
#define OS_TITLE " Sun Solaris"
      ...
      if (register_list.os != OS)
        {
          str = us_tr ("You are running UltraScan with a" OS_TITLE
                       " operating system,\n"
                       "but your license is issued for the "
                       + register_list.os.toUpper ()
                       + " operating system\n\n"
                         "You will have to update your license file before\n"
                         "proceeding. Click on 'Register' to obtain an\n"
                         "UltraScan License for a" OS_TITLE
                         " operating system.");
  ...
  else // file not found
    {
      ...
      str = us_tr ("UltraScan could not find your license file:\n\n" + lcfile
                   + "\n\n"
                     "You will have to update your license file before\n"
  ...
  if (QString::compare (temp_license, register_list.license) == 0)
    {
      ...
      QDate expiration
          = QDate::fromString (register_list.expiration, Qt::TextDate);
      ...
      if (expiration >= today)
        ...
        {
          str = us_tr ("The license in your home directory is expired.\n"
                       "You will have to update your license file before\n"
                       "proceeding. Click on 'Register' to obtain a new\n"
                       "UltraScan License");
    ...
    {
      str = us_tr ("The license in your home directory is invalid.\n"
                   "You will have to update your license file before\n"
                   "proceeding. Click on 'Register' to obtain a new\n"
                   "UltraScan License");
...
*/
void
US_Register::license_info (const QString &str)
{
  switch (QMessageBox::critical (this, us_tr ("UltraScan License Error"), str,
                                 us_tr ("&Register"), us_tr ("&Cancel"), 0, 1))
