...
US_Dirhist::US_Dirhist (QStringList &history,
                        map<QString, QDateTime> &last_access,
                        map<QString, QString> &last_filetype,
                        QString &selected, bool &is_ok, QWidget *p,
                        const char *)
    : QDialog (p)
{
  ...
  t_hist->setMinimumWidth (1.1
                           * (t_hist->columnWidth (0) + t_hist->columnWidth (1)
...
US_Dirhist::setupGUI ()
{
  ...
  t_hist->setFont (QFont (USglobal->config_list.fontFamily,
                          USglobal->config_list.fontSize, QFont::Bold));
  ...
  t_hist->setHorizontalHeaderItem (0,
                                   new QTableWidgetItem (us_tr ("Directory")));
  t_hist->setHorizontalHeaderItem (
      1, new QTableWidgetItem (us_tr ("Last access")));
  t_hist->setHorizontalHeaderItem (
      3, new QTableWidgetItem (us_tr ("Last type loaded")));
    ...
    for (int i = 0; i < (int)history->size (); ++i)
      {
        ...
        {
          QString toset = (last_filetype->count ((*history)[i])
                           && !(*last_filetype)[(*history)[i]].isEmpty ())
        ...
        }
        {
          ...
          QString type = last_filetype->count ((*history)[i])
                             ? (*last_filetype)[(*history)[i]]
                             : "";
  ...
  t_hist->setSelectionMode (QAbstractItemView::MultiSelection);
  t_hist->setSelectionBehavior (QAbstractItemView::SelectRows);
  ...
  if (!any_selected && t_hist->rowCount ())
    {
      t_hist->selectRow (0);
...

  {
    for (int i = 0; i < t_hist->rowCount (); ++i)
      {
        for (int j = 0; j < t_hist->columnCount (); ++j)
          {
            t_hist->item (i, j)->setFlags (t_hist->item (i, j)->flags ()
                                           ^ Qt::ItemIsEditable);
          }
      }
  };
  ...
  connect (t_hist->horizontalHeader (), SIGNAL (clicked (int)),
           SLOT (t_sort_column (int)));
  connect (t_hist, SIGNAL (doubleClicked (int, int, int, const QPoint &)),
           SLOT (t_doubleClicked (int, int, int, const QPoint &)));
#else
  connect (t_hist->horizontalHeader (), SIGNAL (sectionClicked (int)),
           SLOT (t_sort_column (int)));
  connect (t_hist, SIGNAL (cellDoubleClicked (int, int)),
           SLOT (t_doubleClicked (int, int)));
#endif
  ...
  pb_del->setFont (QFont (USglobal->config_list.fontFamily,
...
void
US_Dirhist::closeEvent (QCloseEvent *e)
{
  // global_Xpos -= 30;
  // global_Ypos -= 30;
  e->accept ();
...
void
US_Dirhist::t_sort_column (int col)
{
  ...
  t_hist->sortByColumn (col ? col + 1 : col, order_ascending
                                                 ? Qt::AscendingOrder
                                                 : Qt::DescendingOrder);
...
void
US_Dirhist::t_doubleClicked (int row, int)
{
  *is_ok = true;
...
void
US_Dirhist::t_doubleClicked (int row, int col, int, const QPoint &)
{
