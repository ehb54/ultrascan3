...
main (int argc, const char **argv)
{
  /*
    ...
    zno	"zeno" file -- mainoutput file for results --
  ...
  */
  ...
  // WRITE(OUTPUT,FMT,F321,1,S<TRG,endend,28,0);
          ...
          if (zeno_us_udp_msg)
            {
              ...
              msging["progress_output"]
                  = QString ("Hydro (Zeno) calculation: %1\%")
                        .arg (QString::number (
          ...
          unsigned int *savehits, int /* P2 */, int /* P1 */)
{
  ...
  *zeno_done = false;
  ...
  if (!Tshush->silent)
    {
      zeno_us_log->log ("ZENO calculation start\n");
      if (zeno_us_udp_msg)
        {
          map<QString, QString> msging;
          msging["_textarea"] = "ZENO calculation start\\n";
         ...
         double *tten, int /* P1 */, int /* P2 */)
{
  ...
  *rg_done = false;
  ...
  if (!Tshush->silent)
    {
      zeno_us_log->log ("ZENO interior calculation start\n");
      if (zeno_us_udp_msg)
        {
          map<QString, QString> msging;
          msging["_textarea"] = "ZENO interior calculation start\\n";
         ...
         int /* P2 */)
{
  ...
  *kirk_done = false;
  ...
  if (!Tshush->silent)
    {
      zeno_us_log->log ("ZENO surface calculation start\n");
      if (zeno_us_udp_msg)
        {
          map<QString, QString> msging;
          msging["_textarea"] = "ZENO surface calculation start\\n";
...
US_Hydrodyn_Zeno::US_Hydrodyn_Zeno (hydro_options *options,
                                    hydro_results *results, US_Log *us_log,
...
US_Hydrodyn_Zeno::run (QString filename, vector<PDB_atom> *bead_model,
                       double &sum_mass, double &sum_volume, bool keep_files,
                       bool zeno_cxx, int threads)
{
  ...
  //    );
  //    // error_msg = QString( "Error: can not change to directory: %1" ).arg(
  ...
  if (options->unit != -9 && options->unit != -10)
    {
      // error_msg = QString( "Error: units must be A(10) or nm(9) and value is
      // %1" ).arg( -options->unit );
      return false;
    }
  ...
  if (zeno_cxx)
    {
      ...
      argv[argc++] = strdup ("-n");
      argv[argc++] = strdup (QString ("%1")
                                 .arg (options->zeno_zeno_steps * 1000)
                                 .toLatin1 ()
                                 .data ());

      // us_qdebug( QString( "zeno steps %1" ).arg( options->zeno_zeno_steps *
      // 1000 ) );
      ...
      // zeno_progress->setValue( 0, progress_steps );
      ppos = 0;
      mppos = progress_steps;
      ...
      zeno_cxx_main (argc, argv,
                     QString ("%1.zno").arg (filename).toLatin1 ().data (),
                     true, zeno_us_udp_msg);
#endif
      ...
      //{
      //    us_hydrodyn->last_hydro_res = QFileInfo( filename + ".zno"
      //    ).fileName();
      // }
    }
  else
    {
      ...
      //  zeno_progress->setValue( 0 ); zeno_progress->setMaximum(
      //  progress_steps );
      ppos = 0;
      mppos = progress_steps;
      ...
      //  zeno_progress->setValue( 1 ); zeno_progress->setMaximum( 1 );
...
US_Saxs_Util::calc_zeno_hydro ()
{
  ...
  //    SOMO->Hydrodynamic Calculations Zeno" ) ); return false;
  // }
  ...
  //                QString( us_tr( "Zeno integration selected with %1 thousand
  //                MC iterations" ) ) .arg( hydro.zeno_zeno_steps ) );
  // }
  ...
  //                thousand MC iterations" ) ) .arg( hydro.zeno_interior_steps
  //                ) );
  // }
  ...
  //                thousand MC iterations" ) ) .arg( hydro.zeno_surface_steps
  //                ) );
  // }
  ...
  int zeno_cxx = NEW_ZENO;       // pass from Genapp's GUI
  int numThreads = THREADNUMBER; // pass from Genapp's GUI
  double sum_mass = 0e0;
      ...
      if (somo_processed[current_model])
        {
          ...
          bead_model = bead_models[current_model];
          bool result
              = uhz.run (QString ( // somo_dir +
                                   //  QDir::separator() +
                             project + QString ("_%1").arg (current_model + 1)
                             + QString (bead_model_suffix.length ()
                                            ? ("-" + bead_model_suffix)
                                            : "")),
                         &bead_models[current_model], sum_mass, sum_volume,
                         true, zeno_cxx, numThreads);

          QString last_hydro_res
              = QFileInfo (project + QString ("_%1").arg (current_model + 1)
                           + QString (bead_model_suffix.length ()
                                          ? ("-" + bead_model_suffix)
                                          : "")
                           + ".zno")
          ...
          if (!result)
          ...
          else
            {
              ...
              // need to get rg
              // this_data.results.rg            = model_vector[ current_model
              // ].Rg;
              ...
              if (!f.exists () || !f.open (QIODevice::ReadOnly))
                {
                  ...
                  // editor_msg( "red", "ZENO computation failed to produce
                  // output file" ); pb_calc_hydro->setEnabled(
                  // was_hydro_enabled ); pb_calc_zeno->setEnabled(true);
                  // pb_bead_saxs->setEnabled(true);
                  // pb_rescale_bead_model->setEnabled( misc.target_volume !=
                  // 0e0 || misc.equalize_radii );
                  // pb_show_hydro_results->setEnabled(false);
                  // progress->reset();
                  return false;
                }
              ...
              // editor_msg( "dark blue",
              //             QString( "ZENO computation output file %1 opened
              //             %2 lines" ) .arg( last_hydro_res ) .arg(
              //             qsl.size() ) );
              ...
              if (zeno_cxx)
                {
                  ...
                  param_rx.push_back (
                      QRegExp ("^Capacitance:\\s+(\\S+)\\s*$"));
                  param_name.push_back ("results.rs");
                  param_cap_pos.push_back (1);

                  param_rx.push_back (
                      QRegExp ("^Intrinsic viscosity:\\s+(\\S+)\\s*$"));
              ...
              for (unsigned int i = 0; i < (unsigned int)param_rx.size (); i++)
                {
                  ...
                  switch (param_qsl[i].size ())
                    ...
                    case 1:
                      {
                        ...
                        // editor_msg( "dark blue", QString( "ZENO produced
                        // parameter: %1 value %2\n" ).arg( param_name[ i ]
                        // ).arg( qs ) );
                    ...
                    default:
                      ...
                      //  {
                      //    //editor_msg( "red", QString( "ZENO produced
                      //    multiple values for parameter: %1\n" ).arg(
                      //    param_name[ i ] ) );
                      //  }
                      break;
                    }
              ...
              if (zeno_cxx)
                {
                  ...
                  if (use_mass)
                    {
                      // double tmp = (4e0/3e0) * this_data.results.viscosity *
                      // AVOGADRO * 1e-24 * sum_volume;
                  ...
                  this_data.tra_fric_coef = 6.0 * M_PI
                  ...
                  if (this_data.tra_fric_coef)
                    {
                      ...
                      this_data.results.D20w = (K0 + hydro.temperature)
                                               * 1.38064852e-8
...

              {
                ...
                if (this_data.results.viscosity && this_data.results.mass
                    && this_data.results.vbar)
                  {
                    //                  this_data.results.ff0 =
                    //                  this_data.tra_fric_coef / ( 6e0 * M_PI
                    //                  *  this_data.results.viscosity *
                    //                  this_data.results.rs );

                    // us_qdebug(
                    //        QString( "f is %1 ETAo %2 partvol %3 fconv %4
                    //        mass %5" ) .arg( this_data.tra_fric_coef ) .arg(
                    //        this_data.hydro.solvent_viscosity * 1e-2 ) .arg(
                    ...
                    this_data.results.ff0
                        = this_data.tra_fric_coef * 1e-7 * fconv
                          / (6e0 * M_PI * this_data.hydro.solvent_viscosity
                             * 1e-2
                    ...
                    // us_qdebug( QString( "computed ff0 %1" ).arg(
                    // this_data.results.ff0 ) );
                  }
                ...
                if (this_data.tra_fric_coef)
                  {
                    // us_qdebug(
                    //        QString( "s20w mass is %1 partvol %2 DENS %3 f
                    //        %4" ) .arg( this_data.results.mass ) .arg(
                    //        this_data.results.vbar ) .arg(
                    //        this_data.hydro.solvent_density ) .arg(
                    //        this_data.tra_fric_coef )
                    //         );
                    this_data.results.s20w =
                        ...
                        //      this_data.hydro.solvent_density ) / (
                        ...
                        * (1e0
                           - (this_data.results.vbar
                              * this_data.hydro.solvent_density))
                        / (6e0 * M_PI * hydro.solvent_viscosity
                           * this_data.results.rs * AVOGADRO * fconv);
                  }
                ...
                if (this_data.results.mass)
                  {
                    ...
                    for (unsigned int i = 0; i < bead_model.size (); ++i)
                      {
                        ...
                        cm.axis[0] += this_atom->mw
                                      * this_atom->bead_coordinate.axis[0];
                        cm.axis[1] += this_atom->mw
                                      * this_atom->bead_coordinate.axis[1];
                        cm.axis[2] += this_atom->mw
                    ...
                    for (unsigned int i = 0; i < bead_model.size (); ++i)
                      {
                        ...
                        Rg2 += (double)this_atom->mw
                               * ((double)(this_atom->bead_coordinate.axis[0]
                                           - cm.axis[0])
                                      * (this_atom->bead_coordinate.axis[0]
                                         - cm.axis[0])
                                  + (double)(this_atom->bead_coordinate.axis[1]
                                             - cm.axis[1])
                                        * (this_atom->bead_coordinate.axis[1]
                                           - cm.axis[1])
                                  + (double)(this_atom->bead_coordinate.axis[2]
                                             - cm.axis[2])
                                        * (this_atom->bead_coordinate.axis[2]
                                           - cm.axis[2]));
                      }
                    ...
                    // us_qdebug( QString( "rg %1 fconv %2 rg2 %3 total_cm_mw
                    // %4" ).arg( Rg ).arg( fconv ).arg( Rg2 ).arg( total_cm_mw
                    // ) );
                  }
              }
              ...
              // append to zno file
              {
                QString add_to_zeno
                    = QString (
                          ...
                          .arg (QString ("").sprintf ("%4.2e S",
                                                      this_data.results.s20w))
                          .arg (QString ("").sprintf ("%3.2f",
                                                      this_data.results.ff0))
                          .arg (QString ("").sprintf ("%4.2e nm",
                                                      this_data.results.rg));

                if (zeno_cxx)
                  {
                    add_to_zeno
                        += QString (" Intrinsic Viscosity M      : %1\n"
                               ...
                               .arg (QString ("").sprintf (
                                   ...
                                   this_data.results.viscosity))
                               .arg (QString ("").sprintf (
                                   "%4.2e g/s", this_data.tra_fric_coef))
                               .arg (QString ("").sprintf (
                                   "%4.2e cm^2/sec", this_data.results.D20w));
                  }
              ...
              //    bead_models; saxs_util->bead_models.resize( 1 );
              //    saxs_util->bead_models[ 0 ] = bead_model;
              //    if ( "empty model" != saxs_util->nsa_physical_stats() )
              //    {
              //       this_data.tot_volume_of =
              //       saxs_util->nsa_physical_stats_map[ "result excluded
              //       volume" ].toDouble();

              //       this_data.max_ext_x = saxs_util->nsa_physical_stats_map[
              //       "result radial extent bounding box size x" ].toDouble();
              //       this_data.max_ext_y = saxs_util->nsa_physical_stats_map[
              //       "result radial extent bounding box size y" ].toDouble();
              //       this_data.max_ext_z = saxs_util->nsa_physical_stats_map[
              //       "result radial extent bounding box size z" ].toDouble();

              //       this_data.axi_ratios_xz =
              //       saxs_util->nsa_physical_stats_map[ "result radial extent
              //       axial ratios x:z" ].toDouble(); this_data.axi_ratios_xy
              //       = saxs_util->nsa_physical_stats_map[ "result radial
              //       extent axial ratios x:y" ].toDouble();
              //       this_data.axi_ratios_yz =
              //       saxs_util->nsa_physical_stats_map[ "result radial extent
              //       axial ratios y:z" ].toDouble();
              //    } else {
              // 	    //editor_msg( "red", QString( "Internal error: Bead
              // model is empty?" ) );
              //    }
              // }
              ...
              // this_data.results.mass        * this_data.results.mass;
              zeno_results2.s20w
                  += this_data.results.s20w * this_data.results.s20w;
              zeno_results2.D20w
                  += this_data.results.D20w * this_data.results.D20w;
              zeno_results2.viscosity
                  += this_data.results.viscosity * this_data.results.viscosity;
              ...
              // this_data.results.vbar        * this_data.results.vbar;
              zeno_results2.ff0
                  += this_data.results.ff0 * this_data.results.ff0;
              zeno_results2.used_beads += this_data.results.used_beads
                                          * this_data.results.used_beads;
              zeno_results2.total_beads += this_data.results.total_beads
                                           * this_data.results.total_beads;
              ...
              //       fprintf(of, "%s",
              //       save_util->dataString(&this_data).toLatin1().data());
              //       fclose(of);
              //    }
              // }
              // // print out results:
  ...
  us_log->log ("Calculate hydrodynamics (Zeno) completed\n");
  if (us_udp_msg)
    {
      map<QString, QString> msging;
      msging["_textarea"] = "Calculate hydrodynamics (Zeno) completed\\n";
...
//    else
//    {
//       hydro_zeno_window = new US_Hydrodyn_Hydro_Zeno( &hydro,
//       &hydro_zeno_widget, this ); hydro_zeno_window->show();
//    }
// }
