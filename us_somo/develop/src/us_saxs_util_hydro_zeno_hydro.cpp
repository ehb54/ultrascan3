...
bool US_Hydrodyn_Zeno::run(QString filename, vector<PDB_atom> *bead_model,
  ...
  for (unsigned int i = 0; i < bead_model->size(); i++) {
    ...
    sum_volume +=
        (4.0 / 3.0) * M_PI * pow((*bead_model)[i].bead_computed_radius, 3);
  }
  ...
  if (zeno_cxx) {
    ...
    argv[argc++] = strdup("-n");
    argv[argc++] = strdup(
        QString("%1").arg(options->zeno_zeno_steps * 1000).toLatin1().data());
    ...
    zeno_cxx_main(argc, argv, QString("%1.zno").arg(filename).toLatin1().data(),
                  true, zeno_us_udp_msg);
#endif

    if (zeno_us_udp_msg) {
      map<QString, QString> msging;
      msging["progress_output"] = QString("Hydro (Zeno) calculation: %1\%")
                                      .arg(100)
                                      .arg(100);  // arg(ppos).arg(mppos);
...
bool US_Saxs_Util::calc_zeno_hydro() {
  // cout<<"calc zeno\n";
    ...
    if (somo_processed[current_model]) {
      ...
      bool result = uhz.run(
          QString(  // somo_dir +
              ...
              QString(bead_model_suffix.length() ? ("-" + bead_model_suffix)
                                                 : "")),
          &bead_models[current_model], sum_mass, sum_volume, true, zeno_cxx,
          numThreads);
      ...
      if (!result) {
        ...
        for (unsigned int i = 0; i < (unsigned int)param_rx.size(); i++) {
          ...
          switch (param_qsl[i].size()) {
            case 1: {
              ...
              // editor_msg( "dark blue", QString( "ZENO produced parameter: %1
            ...
            case 0:
              // cout << "INSIDE0 " << endl;
              //  {
              //     // editor_msg( "red", QString( "ZENO did not produce
            ...
            default:
              ...
              //  {
              //    //editor_msg( "red", QString( "ZENO produced multiple values
              //    for parameter: %1\n" ).arg( param_name[ i ] ) );
              //  }
              break;
          }
        ...
        if (zeno_cxx) {
          ...
          if (use_mass) {
            // double tmp = (4e0/3e0) * this_data.results.viscosity * AVOGADRO *
            // 1e-24 * sum_volume; this_data.results.viscosity = tmp / sum_mass;
          ...
          this_data.tra_fric_coef = 6.0 * M_PI * hydro.solvent_viscosity *
                                    this_data.results.rs * 1e-1;
          if (this_data.tra_fric_coef) {
            // 1.3864852e-8 is  boltzman's constant with a conversion, probably
            // needs fconv
            this_data.results.D20w = (K0 + hydro.temperature) * 1.38064852e-8 /
...

        {
          ...
          if (this_data.results.viscosity && this_data.results.mass &&
            ...
            //                  this_data.results.ff0 = this_data.tra_fric_coef
            //                  / ( 6e0 * M_PI *  this_data.results.viscosity *
          ...
          if (this_data.tra_fric_coef) {
            ...
            this_data.results.s20w =
                ...
                (1e0 -
                 (this_data.results.vbar * this_data.hydro.solvent_density)) /
                (6e0 * M_PI * hydro.solvent_viscosity * this_data.results.rs *
                 AVOGADRO * fconv);
          }
          ...
          if (this_data.results.mass) {
            ...
            for (unsigned int i = 0; i < bead_model.size(); ++i) {
              ...
              cm.axis[0] += this_atom->mw * this_atom->bead_coordinate.axis[0];
              cm.axis[1] += this_atom->mw * this_atom->bead_coordinate.axis[1];
              cm.axis[2] += this_atom->mw * this_atom->bead_coordinate.axis[2];
            ...
            // us_qdebug( QString( "rg %1 fconv %2 rg2 %3 total_cm_mw %4" ).arg(
            // Rg ).arg( fconv ).arg( Rg2 ).arg( total_cm_mw ) );
          }
        }
        ...
        // append to zno file
        {
          ...
          if (zeno_cxx) {
            add_to_zeno += QString(
                               ...
                               .arg(QString("").sprintf(
                                   "%4.2e cm^3/g", this_data.results.viscosity))
                               .arg(QString("").sprintf(
        ...
        //    {
        //       this_data.tot_volume_of = saxs_util->nsa_physical_stats_map[
        //       "result excluded volume" ].toDouble();
        ...
        //       this_data.axi_ratios_xz = saxs_util->nsa_physical_stats_map[
        //       "result radial extent axial ratios x:z" ].toDouble();
        //       this_data.axi_ratios_xy = saxs_util->nsa_physical_stats_map[
        //       "result radial extent axial ratios x:y" ].toDouble();
        //       this_data.axi_ratios_yz = saxs_util->nsa_physical_stats_map[
        //       "result radial extent axial ratios y:z" ].toDouble();
        //    } else {
        // 	    //editor_msg( "red", QString( "Internal error: Bead model is
        // empty?" ) );
        //    }
        // }
        ...
        // no sd: zeno_results2.mass                 += this_data.results.mass
        ...
        // no sd: zeno_results2.vbar                 += this_data.results.vbar
        // * this_data.results.vbar;
