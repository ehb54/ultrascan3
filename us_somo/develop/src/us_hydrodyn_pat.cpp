...
int us_hydrodyn_pat_main(int use_nmax, int in_nat, struct dati1_supc *in_dt,
                         int *out_nat, struct dati1_pat *out_dt) {
  nmax = use_nmax;
  ...
  for (i = 0; i < nat; i++) {
    ...
    if (i < 3) {
      ...
      // in_dt[i].x,in_dt[i].y,in_dt[i].z,in_dt[i].r, in_dt[i].m, in_dt[i].col);
      // printf("reading in coords dt bead %d @ %f %f %f %f %f %d\n", i,
      // dt[i].x,dt[i].y,dt[i].z,dt[i].r, dt[i].m, dt[i].col);
    }
  }
#if defined(OLD_WAY_CHECK)
  inp_inter();
...
RET:
  ...
  for (i = 0; i < 3; i++) v1[i] = xx[i];

  if (cc == 0) {
    sisli(a2);

    for (i = 0; i < 3; i++) v2[i] = xx[i];

    sisli(a3);

    for (i = 0; i < 3; i++) v3[i] = xx[i];
  } else {
    dl2 = dl3 = 999999;
    for (i = 0; i < 3; i++) {
      v2[i] = 0.0;
  ...
  */
  ...
  if ((vv[0][1] != 0.0) || (vv[0][2] != 0.0) || (vv[0][3] != 0.0)) {
    autovett = 1;
    vect = 1;
    for (i = 0; i < 3; i++) v1[i] = vv[0][i + 1];

    if ((vv[1][1] != 0.0) || (vv[1][2] != 0.0) || (vv[1][3] != 0.0)) {
      vect = 2;
      for (i = 0; i < 3; i++) v2[i] = vv[1][i + 1];
    }
  }
    ...
    autovett = 2;
    vect = 1;
    for (i = 0; i < 3; i++) v1[i] = vv[1][i + 1];
  } else {
    autovett = 3;
    vect = 1;
    for (i = 0; i < 3; i++) v1[i] = vv[2][i + 1];
  }
  ...
  if (contatore > 4) {
    *out_nat = nat;
    ...
    for (i = 0; i < nat; i++) {
      ...
      fprintf(omol, "%f\n", dtn[i].z);
      if ((raggio == -1.0) || (raggio == -3.0)) fprintf(ormc, "%f\t", dt[i].rg);
    ...
    if ((fabs(a[0][0] - vv[0][0]) < 1) && (fabs(a[1][1] - vv[1][0]) < 1) &&
        (fabs(a[2][2] - vv[2][0]) < 1)) {
      *out_nat = nat;
      for (i = 0; i < nat; i++) {
        // if ( i < 3 )
...
static double approx(double a) {
  ...
  if ((ceil(a) - a <= 0.01) && (ceil(a) - a >= 0.0)) app = ceil(a);

  if ((a - floor(a) <= 0.01) && (a - floor(a) >= 0.0)) app = floor(a);
...
static void calc_inertia_tensor() {
  ...
  for (i = 0; i < 3; i++)
    for (j = 0; j < 3; j++) a[i][j] = approx(a[i][j]);
}
...
static void initarray() {
  ...
  if (raggio == 0.0) {
    fscanf(mol, "%s", ragcol);
    rmc = us_fopen(ragcol,
                   "r"); /* opening the file containing the unhydrated radii,
                            the masses and the colors of the beads */
    ...
    rmc = us_fopen(
        ragcol,
        "r"); /* opening the file containg both the hydrated and unhydrated
                 radii, the masses and the colors of the beads */
...
static void sol(double c[2][3]) {
  ...
  if ((c1 < -0.01) || (c1 > 0.01) || (c2 < -0.01) || (c2 > 0.01) ||
      (c3 < -0.01) || (c3 > 0.01)) {
    flag = 1;
...
#if defined(USE_MAIN)
int main() { us_hydrodyn_pat_main(250); }
#endif
