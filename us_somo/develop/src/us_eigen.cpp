...
bool
US_Eigen::evaluate_polynomial (const vector<double> &coeff,
                               const double x_start, const double x_end,
                               const int points, vector<double> &x,
                               vector<double> &y)
{
  error_text = "";
  if (x_start >= x_end)
  ...
  for (double pos = x_start, i = 0; pos <= x_end && i < points;
       pos += delta, ++i)
    {
      x[i] = pos;
...
void
US_Eigen::polyfit (const std::vector<double> &t, const std::vector<double> &v,
...
bool
US_Eigen::polyfit (const vector<double> &x, const vector<double> &y,
                   const int degree,
                   vector<double> &coeff // the polynomial coefficients
                   ,
                   double &chi2, fit_methods fit_method,
...
bool
US_Eigen::polyfit (const vector<double> &x_input,
                   const vector<double> &y_input,
                   const vector<double> &e_input, const int degree,
                   vector<double> &coeff // the polynomial coefficients
                   ,
                   double &chi2, fit_methods fit_method,
                   weight_methods weight_method)
{
  ...
  // probably could swap the for and switch ... does compiler optimization do
  // this?
  ...
  coeff = std::vector<double> (coefficients.data (),
                               coefficients.data () + numCoefficients);
  ...
  for (size_t i = 0; i < y_input_size; ++i)
    {
      ...
      // QTextStream(stdout) << QString( "chi2 calc x %1 y obs %2  y model
      // %3\n" )
      //    .arg( x_input[i] )
      //    .arg( y_input[i] )
      //    .arg( y_model )
      //    ;
      chi2 += (y_input[i] - y_model) * (y_input[i] - y_model)
              / (e_input[i] * e_input[i]);
    }
...
QString
US_Eigen::gnuplot_poly (const vector<double> &coeff, QString fname)
{
...
QString
US_Eigen::qs_fit_method (enum fit_methods fit_method)
{
  switch (fit_method)
    {
...
QString
US_Eigen::qs_weight_method (enum weight_methods weight_method)
{
  switch (weight_method)
    {
