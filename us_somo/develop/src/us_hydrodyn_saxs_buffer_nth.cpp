...
US_Hydrodyn_Saxs_Buffer_Nth::US_Hydrodyn_Saxs_Buffer_Nth (
    void *us_hydrodyn_saxs_buffer, map<QString, QString> *parameters,
    QWidget *p, const char *)
    : QDialog (p)
{
  this->us_hydrodyn_saxs_buffer = us_hydrodyn_saxs_buffer;
  this->us_hydrodyn
      = ((US_Hydrodyn_Saxs_Buffer *)us_hydrodyn_saxs_buffer)->us_hydrodyn;
...
void
US_Hydrodyn_Saxs_Buffer_Nth::setupGUI ()
{
  int minHeight1 = 24;
  ...
  lbl_files->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize, QFont::Normal));
  ...
  for (int i = 0; i < ((US_Hydrodyn_Saxs_Buffer *)us_hydrodyn_saxs_buffer)
                          ->lb_files->count ();
       ++i)
    {
      lb_files->addItem (QString ("%1 : %2").arg (i + 1).arg (
          ((US_Hydrodyn_Saxs_Buffer *)us_hydrodyn_saxs_buffer)
              ->lb_files->item (i)
              ->text ()));
      if (((US_Hydrodyn_Saxs_Buffer *)us_hydrodyn_saxs_buffer)
              ->lb_files->item (i)
              ->isSelected ())
        {
  ...
  lbl_files_sel->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  lb_files_sel->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize - 1));
  ...
  lbl_files_selected->setFont (QFont (USglobal->config_list.fontFamily,
                                      USglobal->config_list.fontSize,
                                      QFont::Normal));
  ...
  lbl_select_nth->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize,
                                  QFont::Bold));
  ...
  AUTFBACK (lbl_n);
  lbl_n->setFont (QFont (USglobal->config_list.fontFamily,
                         USglobal->config_list.fontSize, QFont::Bold));

  select_widgets.push_back (lbl_n);

  le_n = new QLineEdit (this);
  le_n->setObjectName ("le_n Line Edit");
  le_n->setText ("1");
  le_n->setAlignment (Qt::AlignCenter | Qt::AlignVCenter);
  le_n->setPalette (PALET_NORMAL);
  AUTFBACK (le_n);
  le_n->setFont (QFont (USglobal->config_list.fontFamily,
                        USglobal->config_list.fontSize));
  {
    QIntValidator *qdv = new QIntValidator (
        1,
        ((US_Hydrodyn_Saxs_Buffer *)us_hydrodyn_saxs_buffer)
            ->lb_files->count (),
        le_n);
  ...
  connect (le_n, SIGNAL (textChanged (const QString &)),
           SLOT (update_enables ()));
  ...
  AUTFBACK (lbl_start);
  lbl_start->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize, QFont::Bold));
  select_widgets.push_back (lbl_start);

  le_start = new QLineEdit (this);
  le_start->setObjectName ("le_start Line Edit");
  le_start->setText ("1");
  le_start->setAlignment (Qt::AlignCenter | Qt::AlignVCenter);
  le_start->setPalette (PALET_NORMAL);
  AUTFBACK (le_start);
  le_start->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize));
  {
    QIntValidator *qdv = new QIntValidator (
        1,
  ...
  connect (le_start, SIGNAL (textChanged (const QString &)),
           SLOT (update_enables ()));
  le_start->setMinimumWidth (80);
  select_widgets.push_back (le_start);

  lbl_start_name
      = new QLabel (((US_Hydrodyn_Saxs_Buffer *)us_hydrodyn_saxs_buffer)
                        ->lb_files->item (le_start->text ().toInt () - 1)
                        ->text (),
                    this);
  lbl_start_name->setAlignment (Qt::AlignLeft | Qt::AlignVCenter);
  lbl_start_name->setPalette (PALET_NORMAL);
  AUTFBACK (lbl_start_name);
  lbl_start_name->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize,
                                  QFont::Normal));
  ...
  AUTFBACK (lbl_end);
  lbl_end->setFont (QFont (USglobal->config_list.fontFamily,
                           USglobal->config_list.fontSize, QFont::Bold));

  select_widgets.push_back (lbl_end);

  le_end = new QLineEdit (this);
  le_end->setObjectName ("le_end Line Edit");
  le_end->setText (
      QString ("%1").arg (((US_Hydrodyn_Saxs_Buffer *)us_hydrodyn_saxs_buffer)
                              ->lb_files->count ()));
  le_end->setAlignment (Qt::AlignCenter | Qt::AlignVCenter);
  le_end->setPalette (PALET_NORMAL);
  AUTFBACK (le_end);
  le_end->setFont (QFont (USglobal->config_list.fontFamily,
                          USglobal->config_list.fontSize));
  {
    QIntValidator *qdv = new QIntValidator (
        2,
  ...
  connect (le_end, SIGNAL (textChanged (const QString &)),
           SLOT (update_enables ()));
  ...
  pb_nth_only->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  ...
  pb_nth_add->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize));
  ...
  lbl_contain->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize, QFont::Bold));
  ...
  lbl_contains->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize, QFont::Bold));

  contain_widgets.push_back (lbl_contains);

  le_contains = new QLineEdit (this);
  le_contains->setObjectName ("le_contains Line Edit");
  le_contains->setText (parameters->count ("buffer_nth_contains")
  ...
  le_contains->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  connect (le_contains, SIGNAL (textChanged (const QString &)),
           SLOT (update_enables ()));
  ...
  pb_contains_only->setFont (QFont (USglobal->config_list.fontFamily,
                                    USglobal->config_list.fontSize));
  ...
  pb_contains_add->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize));
  ...
  lbl_intensity->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize, QFont::Bold));
  ...
  AUTFBACK (cb_q_range);
  cb_q_range->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize));
  ...
  pb_i_avg_all
      = new QPushButton (us_tr ("Compute average intensity of all"), this);
  pb_i_avg_all->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  pb_i_avg_sel = new QPushButton (
      us_tr ("Compute average intensity of selected"), this);
  pb_i_avg_sel->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  te_q = new QTextEdit (this);
  te_q->setObjectName ("te_q Line Edit");
  te_q->setText ("");
  te_q->setPalette (PALET_NORMAL);
  AUTFBACK (te_q);
  te_q->setFont (QFont (USglobal->config_list.fontFamily,
                        USglobal->config_list.fontSize));
  ...
  AUTFBACK (rb_i_above);
  rb_i_above->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize));
  ...
  rb_i_below
      = new QRadioButton (us_tr ("Below intensity level [A.U.]:"), this);
  rb_i_below->setPalette (PALET_NORMAL);
  AUTFBACK (rb_i_below);
  rb_i_below->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize));
  ...
  pb_i_add->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize));
  ...
  pb_clear_plot->setFont (QFont (USglobal->config_list.fontFamily,
                                 USglobal->config_list.fontSize));
  ...
  pb_save_dat->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  ...
  pb_do_select->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
  ...
  QVBoxLayout *left = new QVBoxLayout (0);
  left->setContentsMargins (0, 0, 0, 0);
  left->setSpacing (0);

  left->addWidget (lbl_title);

  QHBoxLayout *hbl_files_pane = new QHBoxLayout ();
  hbl_files_pane->setContentsMargins (0, 0, 0, 0);
  hbl_files_pane->setSpacing (0);
  {
  ...
  }
  {
    ...
    vbl_files_sel->setContentsMargins (0, 0, 0, 0);
    vbl_files_sel->setSpacing (0);
  ...
  left->addWidget (lbl_select_nth);
  {
    ...
    gl_nth->setContentsMargins (0, 0, 0, 0);
    gl_nth->setSpacing (0);
    ...
    QHBoxLayout *hbl_sel = new QHBoxLayout ();
    hbl_sel->setContentsMargins (0, 0, 0, 0);
    hbl_sel->setSpacing (0);
  ...
  left->addWidget (lbl_contain);
  {
    ...
    gl_contains->setContentsMargins (0, 0, 0, 0);
    gl_contains->setSpacing (0);
  ...
  {
    QGridLayout *gl_intensity = new QGridLayout (0);
    gl_intensity->setContentsMargins (0, 0, 0, 0);
    gl_intensity->setSpacing (0);
...
void
US_Hydrodyn_Saxs_Buffer_Nth::quit ()
{
  close ();
}

void
US_Hydrodyn_Saxs_Buffer_Nth::do_select (bool update)
{
...
void
US_Hydrodyn_Saxs_Buffer_Nth::go ()
{
  (*parameters)["go"] = "true";
...
void
US_Hydrodyn_Saxs_Buffer_Nth::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Saxs_Buffer_Nth::closeEvent (QCloseEvent *e)
{
...
void
US_Hydrodyn_Saxs_Buffer_Nth::update_enables ()
{
  lbl_start_name->setText (
      ((US_Hydrodyn_Saxs_Buffer *)us_hydrodyn_saxs_buffer)
          ->lb_files->item (le_start->text ().toInt () - 1)
          ->text ());
  lbl_end_name->setText (((US_Hydrodyn_Saxs_Buffer *)us_hydrodyn_saxs_buffer)
                             ->lb_files->item (le_end->text ().toInt () - 1)
                             ->text ());
  ...
  if (n > 0)
    {
      for (int i = ofs - 1; i < end; i += n)
        {
          range.insert (i);
          if (!any_in_range_not_selected && !selected.count (i))
            {
  ...
  pb_nth_only->setEnabled (any_in_range_not_selected
                           || any_selected_not_in_range);
  pb_nth_add->setEnabled (any_in_range_not_selected
                          && any_selected_not_in_range);
  ...
  for (set<int>::iterator it = contains.begin (); it != contains.end (); it++)
    {
  ...
  pb_contains_only->setEnabled (any_contains_not_selected
                                || any_selected_not_contains);
  pb_contains_add->setEnabled (any_contains_not_selected
                               && any_selected_not_contains);

  lbl_files_selected->setText (QString ("%1 of %2 selected")
                                   .arg (files_selected)
                                   .arg (lb_files->count ()));
  ...
  for (set<int>::iterator it = intensity.begin (); it != intensity.end ();
       it++)
    {
      if (!selected.count (*it))
        {
  ...
  pb_i_only->setEnabled (
      !le_i_level->text ().isEmpty ()
      && (any_intensity_not_selected || any_selected_not_intensity));
  pb_i_add->setEnabled (!le_i_level->text ().isEmpty ()
                        && any_intensity_not_selected
                        && any_selected_not_intensity);
}

void
US_Hydrodyn_Saxs_Buffer_Nth::nth_only ()
{
...
void
US_Hydrodyn_Saxs_Buffer_Nth::nth_add ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  int n = le_n->text ().toInt ();
  int ofs = le_start->text ().toInt ();
...
void
US_Hydrodyn_Saxs_Buffer_Nth::contains_only ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  lb_files->clearSelection ();
  for (int i = 0; i < lb_files->count (); ++i)
...
void
US_Hydrodyn_Saxs_Buffer_Nth::contains_add ()
{
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  for (int i = 0; i < lb_files->count (); ++i)
    {
...
void
US_Hydrodyn_Saxs_Buffer_Nth::update_files_selected ()
{
  lb_files_sel->clear ();
  for (int i = 0; i < lb_files->count (); ++i)
    {
...
void
US_Hydrodyn_Saxs_Buffer_Nth::hide_select ()
{
  if (select_widgets.size ())
    {
      ShowHide::hide_widgets (select_widgets, select_widgets[0]->isVisible (),
                              this);
...
void
US_Hydrodyn_Saxs_Buffer_Nth::hide_files ()
{
  if (files_widgets.size ())
    {
      ShowHide::hide_widgets (files_widgets, files_widgets[0]->isVisible (),
                              this);
...
void
US_Hydrodyn_Saxs_Buffer_Nth::hide_contain ()
{
  if (contain_widgets.size ())
    {
      ShowHide::hide_widgets (contain_widgets,
                              contain_widgets[0]->isVisible (), this);
...
void
US_Hydrodyn_Saxs_Buffer_Nth::hide_intensity ()
{
  if (intensity_widgets.size ())
    {
      ShowHide::hide_widgets (intensity_widgets,
                              intensity_widgets[0]->isVisible (), this);
...
void
US_Hydrodyn_Saxs_Buffer_Nth::i_avg (QStringList files)
{
  double min_i = 0e0;
  double max_i = 0e0;
  double tot_i = 0e0;
  ...
  for (int i = 0; i < (int)files.size (); ++i)
    {
      x.push_back (
          QString (files[i]).replace (QRegExp (" :.*$"), "").toDouble ());
      double this_i = ((US_Hydrodyn_Saxs_Buffer *)us_hydrodyn_saxs_buffer)
                          ->tot_intensity (QString (files[i]).replace (
                                               QRegExp ("^\\d+ : "), ""),
                                           q_min, q_max);
      y.push_back (this_i);
      if (!i || min_i > this_i)
  ...
  qwtw_wheel->setRange (min_i, max_i);
  qwtw_wheel->setSingleStep ((max_i - min_i) / 10000000);
  update_i_level ();

  QString plotname = QString (files[0])
                         .replace (QRegExp ("^\\d+ : "), "")
                         .replace (QRegExp (".(dat|DAT)$"), "");
  plotname += cb_q_range->isChecked () ? QString ("_qs%1_qe%2")
                                             .arg (q_min)
                                             .arg (q_max)
                                             .replace (".", "_")
                                       : QString ("_q_all");
  ...
  plot_data->setCurvePen (
      curve, QPen (pc->color ((int)plotted_curves.size () - 1), 1, SolidLine));
#else
  curve->setSamples ((double *)&(x[0]), (double *)&(y[0]), x.size ());

  curve->setPen (
      QPen (pc->color ((int)plotted_curves.size () - 1), 1, Qt::SolidLine));
...
void
US_Hydrodyn_Saxs_Buffer_Nth::i_avg_all ()
{
  i_avg (MQT::get_lb_qsl (lb_files));
}

void
US_Hydrodyn_Saxs_Buffer_Nth::i_avg_sel ()
{
  i_avg (MQT::get_lb_qsl (lb_files, true));
}

set<int>
US_Hydrodyn_Saxs_Buffer_Nth::get_intensity_selected ()
{
  ...
  //   us_qdebug( QString( "get_intensity_selected files size %1 q_min %2 q_max
  //   %3 cutoff %4" ).arg( files.size() ).arg( q_min ).arg( q_max ).arg(
  //   cutoff ) );
...
void
US_Hydrodyn_Saxs_Buffer_Nth::i_only ()
{
  set<int> to_select = get_intensity_selected ();
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  lb_files->clearSelection ();
  for (set<int>::iterator it = to_select.begin (); it != to_select.end ();
       it++)
    {
...
void
US_Hydrodyn_Saxs_Buffer_Nth::i_add ()
{
  set<int> to_select = get_intensity_selected ();
  disconnect (lb_files, SIGNAL (itemSelectionChanged ()), 0, 0);
  for (set<int>::iterator it = to_select.begin (); it != to_select.end ();
...
void
US_Hydrodyn_Saxs_Buffer_Nth::clear_plot ()
{
...
void
US_Hydrodyn_Saxs_Buffer_Nth::color_rotate ()
{
  pc->color_rotate ();
  for (int i = 0; i < (int)plotted_curves.size (); ++i)
    {
#if QT_VERSION < 0x040000
      plot_data->setCurvePen (plotted_curves[i],
                              QPen (pc->color (i), 1, SolidLine));
#else
...
void
US_Hydrodyn_Saxs_Buffer_Nth::save_dat ()
{
  ...
  QString s = QFileDialog::getExistingDirectory (
      this, us_tr ("Choose a directory to save the intensity plots"), use_dir,
      ...
      if (QFile::exists (fname))
        {
          fname = ((US_Hydrodyn *)us_hydrodyn)
                      ->fileNameCheck2 (fname, cancel, overwrite_all, 0, this);
          raise ();
          if (cancel)
            {
              return;
      ...
      if (!f.open (QIODevice::WriteOnly))
        {
          QMessageBox::warning (
              this, windowTitle (),
              us_tr (QString ("could not open %1 for writing")
                         .arg (f.fileName ())));
          return;
        }
  ...
  QMessageBox::information (
      this, windowTitle () + us_tr (" : Write plotted files"),
...
void
US_Hydrodyn_Saxs_Buffer_Nth::update_i_level ()
{
  // add plot marker
  ...
  if (!plot_marker)
    {
      ...
      *plot_marker = plot_data->insertMarker ();
      ...
      plot_data->setMarkerPen (*plot_marker,
                               QPen (Qt::red, 1, DashDotDotLine));
#else
  ...
  if (!wheel_is_pressed
      && qwtw_wheel->value () != le_i_level->text ().toDouble ())
    {
...
void
US_Hydrodyn_Saxs_Buffer_Nth::adjust_wheel (double pos)
{
  le_i_level->setText (QString ("%1").arg (pos));
}

void
US_Hydrodyn_Saxs_Buffer_Nth::wheel_pressed ()
{
...
void
US_Hydrodyn_Saxs_Buffer_Nth::wheel_released ()
{
...
void
US_Hydrodyn_Saxs_Buffer_Nth::usp_config_plot_data (const QPoint &)
{
