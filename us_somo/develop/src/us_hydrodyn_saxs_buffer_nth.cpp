...
void US_Hydrodyn_Saxs_Buffer_Nth::setupGUI() {
  ...
  for (int i = 0;
       i <
       ((US_Hydrodyn_Saxs_Buffer *)us_hydrodyn_saxs_buffer)->lb_files->count();
       ++i) {
  ...
  lbl_end_name = new QLabel(((US_Hydrodyn_Saxs_Buffer *)us_hydrodyn_saxs_buffer)
  ...
  pb_i_avg_sel =
      new QPushButton(us_tr("Compute average intensity of selected"), this);
  pb_i_avg_sel->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
...
void US_Hydrodyn_Saxs_Buffer_Nth::quit() { close(); }
...
void US_Hydrodyn_Saxs_Buffer_Nth::update_enables() {
  lbl_start_name->setText(((US_Hydrodyn_Saxs_Buffer *)us_hydrodyn_saxs_buffer)
                              ->lb_files->item(le_start->text().toInt() - 1)
                              ->text());
  ...
  lbl_files_selected->setText(
      QString("%1 of %2 selected").arg(files_selected).arg(lb_files->count()));
  ...
  for (set<int>::iterator it = intensity.begin(); it != intensity.end(); it++) {
    if (!selected.count(*it)) {
...
void US_Hydrodyn_Saxs_Buffer_Nth::hide_contain() {
  if (contain_widgets.size()) {
    ShowHide::hide_widgets(contain_widgets, contain_widgets[0]->isVisible(),
                           this);
...
void US_Hydrodyn_Saxs_Buffer_Nth::hide_intensity() {
  if (intensity_widgets.size()) {
    ShowHide::hide_widgets(intensity_widgets, intensity_widgets[0]->isVisible(),
                           this);
...
void US_Hydrodyn_Saxs_Buffer_Nth::i_avg(QStringList files) {
  ...
  for (int i = 0; i < (int)files.size(); ++i) {
    ...
    double this_i =
        ((US_Hydrodyn_Saxs_Buffer *)us_hydrodyn_saxs_buffer)
            ->tot_intensity(QString(files[i]).replace(QRegExp("^\\d+ : "), ""),
                            q_min, q_max);
    y.push_back(this_i);
    if (!i || min_i > this_i) {
      min_i = this_i;
...
set<int> US_Hydrodyn_Saxs_Buffer_Nth::get_intensity_selected() {
  ...
  //   %3 cutoff %4" ).arg( files.size() ).arg( q_min ).arg( q_max ).arg( cutoff
  //   ) );
...
void US_Hydrodyn_Saxs_Buffer_Nth::i_only() {
  ...
  disconnect(lb_files, SIGNAL(itemSelectionChanged()), 0, 0);
  lb_files->clearSelection();
  for (set<int>::iterator it = to_select.begin(); it != to_select.end(); it++) {
