  ...
  switch (widget_id) {
    ...
    case ENRIGHT_CA_PCT_END:
      return QString(us_tr("End %1, dist. %"))
          .arg(ufd.mass_atoms_qstringlist().join(","));
    case ROLL_SPHERE_START:
      return QString(us_tr("Probe radius start [%1]")).arg(UNICODE_ANGSTROM);
    case ROLL_SPHERE_END:
...
QString US_Hydrodyn_Fractal_Dimension_Options::tooltip(WidgetId widget_id) {
  switch (widget_id) {
    ...
    case ASA_PROBE_RADIUS:
      return us_tr(
          "Used to exclude interior atoms\nAn ASA Threshold of 0 will include "
          "all atoms");
    ...
    case ENABLED:
      return us_tr("The FD calculation will be run on a PDB load when enabled");
    case PLOTS:
      return us_tr(
          "Show plots when running the FD calculation (disabled in batch "
          "mode)");
    case SAVE_PLOT_DATA:
...
QVariant US_Hydrodyn_Fractal_Dimension_Options::defaultvalue(
    WidgetId widget_id) {
  switch (widget_id) {
    case ASA_THRESHOLD:
      return QVariant(0);
...
QVariant US_Hydrodyn_Fractal_Dimension_Options::paramvalue(
    ...
    switch (type(w)) {
      ...
      case QCOMBOBOX:
        (*parameters)[paramname(w)] =
            QString("%1").arg(((QComboBox *)widgets[w])->currentData().toInt());
        break;
      case QCHECKBOX:
        (*parameters)[paramname(w)] =
...
QString US_Hydrodyn_Fractal_Dimension_Options::options(
    map<QString, QString> &parameters, double xmin, double xmax) {
  QString result = "";

  US_Fractal_Dimension::methods method =
      (US_Fractal_Dimension::methods)paramvalue(METHOD, parameters).toInt();
  ...
  switch (method) {
    ...
    case US_Fractal_Dimension::USFD_ENRIGHT:
      result +=
          QString(
              " ; Dm(slice) ; Start/End %1 - %2 [%3] Steps %4 ; Slice %5 - %6 "
              "%")
              .arg(xmin != DBL_MAX
                       ? pow(10, xmin)
                       : paramvalue(ANGSTROM_START, parameters).toDouble(),
    ...
    case US_Fractal_Dimension::USFD_ROLL_SPHERE:
      result +=
          QString(" ; Ds ; Start/End %1 - %2 [%3] Steps %4")
              .arg(xmin != DBL_MAX
                       ? pow(10, xmin)
                       : paramvalue(ROLL_SPHERE_START, parameters).toDouble(),
                   0, 'f', 2)
              .arg(xmax != DBL_MAX
                       ? pow(10, xmax)
                       : paramvalue(ROLL_SPHERE_END, parameters).toDouble(),
                   0, 'f', 2)
              // .arg( UNICODE_ANGSTROM ) not working on csv export
              .arg("A")
