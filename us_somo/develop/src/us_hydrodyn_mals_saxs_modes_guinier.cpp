...
void
US_Hydrodyn_Mals_Saxs::guinier_scroll_highlight (int pos)
{
...
void
US_Hydrodyn_Mals_Saxs::guinier_scroll ()
{
...
void
US_Hydrodyn_Mals_Saxs::guinier_add_marker (QwtPlot *plot, double pos,
...
void
US_Hydrodyn_Mals_Saxs::guinier_sd ()
{
  disable_all ();
  guinier_replot ();
  guinier_plot->replot ();
...
void
US_Hydrodyn_Mals_Saxs::guinier_plot_rg_toggle ()
{
...
void
US_Hydrodyn_Mals_Saxs::guinier_plot_mw_toggle ()
{
...
void
US_Hydrodyn_Mals_Saxs::guinier_residuals (bool reset)
{
  if (reset)
    {
...
void
US_Hydrodyn_Mals_Saxs::guinier_residuals_update ()
{
  if (rb_guinier_resid_diff->isChecked ())
    {
...
void
US_Hydrodyn_Mals_Saxs::guinier_analysis ()
{
  ...
  // TSO <<
  //    QString( "guinier_analysis: qstart %1 q2start^.5 %2 qend %3 q2end^.5
  //    %4\n" ) .arg( qstart ) .arg( sqrt( q2start ) ) .arg( qend ) .arg( sqrt(
  //    q2end ) )
  //    ;
      ...
      do
        {
          ...
          usu->wave["mals_saxs"].q.clear ();
          usu->wave["mals_saxs"].r.clear ();
          usu->wave["mals_saxs"].s.clear ();
          ...
          for (int j = 0; j < (int)guinier_q[this_name].size (); j++)
            {
              if (guinier_q[this_name][j] >= qstart
                  && guinier_q[this_name][j] <= qend)
                {
                  usu->wave["mals_saxs"].q.push_back (guinier_q[this_name][j]);
                  usu->wave["mals_saxs"].r.push_back (guinier_I[this_name][j]);
                  if (use_SD_weighting)
                    {
                      usu->wave["mals_saxs"].s.push_back (
                          guinier_e[this_name][j]);
                    }
                }
            }
          ...
          if (pts_decrease)
            {
              if ((int)usu->wave["mals_saxs"].q.size () - (int)pts_decrease
                  < 5)
                {
                  if (!cb_guinier_scroll->isChecked ()
                      || i == guinier_scroll_pos)
                    {
                      editor_msg (
                          ...
                          QString (us_tr ("%1 too few points left %2 after %3 "
                              ...
                              .arg (this_name)
                              .arg ((int)usu->wave["mals_saxs"].q.size ()
                                    - (int)pts_decrease)
                              .arg (pts_decrease));
                    }
                  break;
              ...
              usu->wave["mals_saxs"].q.resize (usu->wave["mals_saxs"].q.size ()
                                               - pts_decrease);
              usu->wave["mals_saxs"].r.resize (
                  usu->wave["mals_saxs"].q.size ());
              if (use_SD_weighting)
                {
                  usu->wave["mals_saxs"].s.resize (
                      usu->wave["mals_saxs"].q.size ());
                }
            }

          unsigned int pstart = 0;
          unsigned int pend = usu->wave["mals_saxs"].q.size ()
                                  ? usu->wave["mals_saxs"].q.size () - 1
                                  : 0;

          computed_rg
              = usu->guinier_plot ("mals_saxsrg", "mals_saxs")
                && usu->guinier_fit (this_log, "mals_saxsrg", pstart, pend, a,
                                     b, siga, sigb, chi2, Rg, I0,
      ...
      if (computed_rg)
        {
          ...
          if (!us_isnan (Rg))
            {
              ...
              if (!count)
              ...
              else
                {
                  ...
                  if (us_isnan (siga)
                      || (unsigned int)usu->wave["mals_saxs"].q.size () < 3)
                    {
                      if (rg_sd_min > Rg)
                        {
                          rg_sd_min = Rg;
          ...
          QString report
              = QString ("%1:%2 ").arg (pos).arg (this_name)
                + QString ("").sprintf (
                    "Rg %.1f (%.1f) (A) MW %.2e (%.2e) qRg [%.3f,%.3f] pts %u "
                    "chi^2 %.2e r-chi %.2e\n",
                    Rg, sigb, I0, siga, sRgmin, sRgmax,
                    (unsigned int)usu->wave["mals_saxs"].q.size (), chi2,
                    sqrt (chi2 / usu->wave["mals_saxs"].q.size ()))
                + us_tr (use_SD_weighting ? "SD  on" : "SD OFF");

          // tainer mw method
          if (0)
            {
              if (US_Saxs_Util::mwt (guinier_q[this_name],
                                     guinier_I[this_name], Rg, sigb, I0, siga,
                                     mwt_k, mwt_c, mwt_qmax, Vct, Qrt, mwt,
                                     mwt_sd, messagest, notest, warningt)
                  && !us_isnan (mwt))
                {
                  report += QString ("").sprintf (
                                " Vc[T] %.1e Qr[T] %.2e MW[RT] %.2e ", Vct,
                                Qrt, mwt)
                            + notest;
                  if (mw_min > mwt)
                    {
  ...
  switch (count)
    ...
    case 0:
      ...
      break;
    case 1:
      msg = QString ("qmax*Rg %1   Rg %2   MW %3")
                .arg (qrg_avg)
                .arg (rg_avg)
                .arg (i0_avg);
      break;
...
void
US_Hydrodyn_Mals_Saxs::guinier_delete_markers ()
{
...
void
US_Hydrodyn_Mals_Saxs::guinier_range (double minq2, double maxq2, double minI,
                                      double maxI)
{
...
void
US_Hydrodyn_Mals_Saxs::guinier_range ()
{
  double minI = 1e99;
  double maxI = 0e0;
  double minq2 = le_guinier_q2_start->text ().toDouble ()
...
void
US_Hydrodyn_Mals_Saxs::guinier_replot ()
{
...
void
US_Hydrodyn_Mals_Saxs::guinier_enables ()
{
...
bool
US_Hydrodyn_Mals_Saxs::guinier_check_qmax (bool show_message)
{
  QString report;
...
void
US_Hydrodyn_Mals_Saxs::check_mwt_constants (bool force)
{
...
bool
US_Hydrodyn_Mals_Saxs::guinier_check_qmax (QString &report, bool show_message)
{
  report = "";
  ...
  return true;
  // for mals_saxs i*q not doing MW[RT]
...
void
US_Hydrodyn_Mals_Saxs::guinier_qrgmax ()
{
  disable_all ();
  qApp->processEvents ();
  guinier_analysis ();
...
void
US_Hydrodyn_Mals_Saxs::guinier_qrgmax_text (const QString &)
{
...
void
US_Hydrodyn_Mals_Saxs::guinier_q_start_text (const QString &text)
{
...
void
US_Hydrodyn_Mals_Saxs::guinier_q_end_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_q_start_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_q_end_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_q2_start_text (const QString &text)
{
  ...
  disconnect (le_guinier_q_start, SIGNAL (textChanged (const QString &)), 0,
              0);
...
void
US_Hydrodyn_Mals_Saxs::guinier_q2_end_text (const QString &text)
{
...
void
US_Hydrodyn_Mals_Saxs::guinier_q2_start_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_q2_end_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_delta_start_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_delta_end_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_delta_start_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_delta_end_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_rg_t_start_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_rg_t_end_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_rg_t_start_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_rg_t_end_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_rg_rg_start_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_rg_rg_end_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_rg_rg_start_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_rg_rg_end_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_mw_t_start_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_mw_t_end_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_mw_t_start_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_mw_t_end_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_mw_mw_start_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_mw_mw_end_text (const QString &text)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_mw_mw_start_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
...
void
US_Hydrodyn_Mals_Saxs::guinier_mw_mw_end_focus (bool hasFocus)
{
  if (current_mode != MODE_GUINIER)
    {
      return;
