...
void US_Hydrodyn_Mals_Saxs::guinier_add_marker( 
                                               QwtPlot * plot,
                                               double pos, 
                                               QColor color, 
                                               QString text, 
   ...
   for ( int i = 0; i < (int) guinier_names.size(); ++i )
   {
      ...
      do {
         ...
         if ( pts_decrease )
         {
            if ( (int) usu->wave[ "mals_saxs" ].q.size() - (int) pts_decrease < 5 )
            {
               if ( !cb_guinier_scroll->isChecked() || i == guinier_scroll_pos )
               {
                  editor_msg( "dark red", QString( us_tr( "%1 too few points left %2 after %3 points removed for qRgmax limit" ) )
                              .arg( this_name )
                              .arg( (int) usu->wave[ "mals_saxs" ].q.size() - (int) pts_decrease )
                              .arg( pts_decrease ) 
                              );
               }
               break;
            ...
            usu->wave[ "mals_saxs" ].q.resize( usu->wave[ "mals_saxs" ].q.size() - pts_decrease );
         ...
         unsigned int pend   = usu->wave[ "mals_saxs" ].q.size() ? usu->wave[ "mals_saxs" ].q.size() - 1 : 0;
      ...
      if ( computed_rg )
      {
         ...
         if ( !us_isnan( Rg ) )
         {
            ...
            if ( !count )
            {
               ...
               if ( us_isnan( siga ) || (unsigned int) usu->wave[ "mals_saxs" ].q.size() < 3 )
               {
                  if ( rg_sd_min > Rg )
                  {
                     rg_sd_min = Rg;
         ...
         QString report =
            ...
            QString::asprintf( "Rg %.1f (%.1f ) (A) MW %.2e (%.2e) qRg [%.3f,%.3f] pts %u chi^2 %.2e r-chi %.2e\n"
                               , Rg
                               , sigb
                               , I0
                               , siga
                               , sRgmin
                               , sRgmax
                               , (unsigned int) usu->wave[ "mals_saxs" ].q.size()
                               , chi2
