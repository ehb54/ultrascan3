...
void US_Hydrodyn_Mals_Saxs::guinier() {
...
void US_Hydrodyn_Mals_Saxs::guinier_scroll_highlight(int pos) {
...
void US_Hydrodyn_Mals_Saxs::guinier_scroll() {
...
void US_Hydrodyn_Mals_Saxs::guinier_add_marker(QwtPlot *plot, double pos,
...
void US_Hydrodyn_Mals_Saxs::guinier_sd() {
  disable_all();
  guinier_replot();
  guinier_plot->replot();
  //   guinier_analysis();
...
void US_Hydrodyn_Mals_Saxs::guinier_plot_rg_toggle() {
...
void US_Hydrodyn_Mals_Saxs::guinier_plot_mw_toggle() {
...
void US_Hydrodyn_Mals_Saxs::guinier_residuals(bool reset) {
  if (reset) {
...
void US_Hydrodyn_Mals_Saxs::guinier_residuals_update() {
  if (rb_guinier_resid_diff->isChecked()) {
...
void US_Hydrodyn_Mals_Saxs::guinier_analysis() {
  ...
  for (int i = 0; i < (int)guinier_names.size(); ++i) {
    ...
    do {
      ...
      for (int j = 0; j < (int)guinier_q[this_name].size(); j++) {
        if (guinier_q[this_name][j] >= qstart &&
          ...
          if (use_SD_weighting) {
            usu->wave["mals_saxs"].s.push_back(guinier_e[this_name][j]);
          }
        }
      }

      if (pts_decrease) {
        if ((int)usu->wave["mals_saxs"].q.size() - (int)pts_decrease < 5) {
        ...
        usu->wave["mals_saxs"].q.resize(usu->wave["mals_saxs"].q.size() -
                                        pts_decrease);
        usu->wave["mals_saxs"].r.resize(usu->wave["mals_saxs"].q.size());
        if (use_SD_weighting) {
          usu->wave["mals_saxs"].s.resize(usu->wave["mals_saxs"].q.size());
        }
      }
      ...
      computed_rg = usu->guinier_plot("mals_saxsrg", "mals_saxs") &&
    ...
    if (computed_rg) {
      ...
      QString report = QString("%1:%2 ").arg(pos).arg(this_name) +
                       QString("").sprintf(
                           "Rg %.1f (%.1f) (A) MW %.2e (%.2e) qRg [%.3f,%.3f] "
                           "pts %u chi^2 %.2e r-chi %.2e\n",
...
void US_Hydrodyn_Mals_Saxs::guinier_delete_markers() {
  guinier_plot->detachItems(QwtPlotItem::Rtti_PlotMarker);
  guinier_plot_errors->detachItems(QwtPlotItem::Rtti_PlotMarker);
}

void US_Hydrodyn_Mals_Saxs::guinier_range(double minq2, double maxq2,
...
void US_Hydrodyn_Mals_Saxs::guinier_range() {
  double minI = 1e99;
  double maxI = 0e0;
  double minq2 = le_guinier_q2_start->text().toDouble() -
                 le_guinier_delta_start->text().toDouble();
...
void US_Hydrodyn_Mals_Saxs::guinier_replot() {
...
void US_Hydrodyn_Mals_Saxs::guinier_enables() {
...
void US_Hydrodyn_Mals_Saxs::check_mwt_constants(bool force) {
...
bool US_Hydrodyn_Mals_Saxs::guinier_check_qmax(QString &report,
                                               bool show_message) {
  report = "";
...
void US_Hydrodyn_Mals_Saxs::guinier_qrgmax() {
  disable_all();
  qApp->processEvents();
  guinier_analysis();
  guinier_enables();
...
void US_Hydrodyn_Mals_Saxs::guinier_qrgmax_text(const QString &) {
...
void US_Hydrodyn_Mals_Saxs::guinier_q_start_text(const QString &text) {
...
void US_Hydrodyn_Mals_Saxs::guinier_q_end_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (guinier_markers.size() != 4) {
...
void US_Hydrodyn_Mals_Saxs::guinier_q_start_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Mals_Saxs::guinier_q_end_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Mals_Saxs::guinier_q2_start_text(const QString &text) {
...
void US_Hydrodyn_Mals_Saxs::guinier_q2_end_text(const QString &text) {
...
void US_Hydrodyn_Mals_Saxs::guinier_q2_start_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Mals_Saxs::guinier_q2_end_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Mals_Saxs::guinier_delta_start_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (!wheel_is_pressed && qwtw_wheel->value() != text.toDouble()) {
...
void US_Hydrodyn_Mals_Saxs::guinier_delta_end_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (!wheel_is_pressed && qwtw_wheel->value() != text.toDouble()) {
...
void US_Hydrodyn_Mals_Saxs::guinier_delta_start_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Mals_Saxs::guinier_delta_end_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Mals_Saxs::guinier_rg_t_start_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
...
void US_Hydrodyn_Mals_Saxs::guinier_rg_t_end_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  guinier_plot_rg->setAxisScale(QwtPlot::xBottom,
...
void US_Hydrodyn_Mals_Saxs::guinier_rg_t_start_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Mals_Saxs::guinier_rg_t_end_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Mals_Saxs::guinier_rg_rg_start_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
...
void US_Hydrodyn_Mals_Saxs::guinier_rg_rg_end_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  guinier_plot_rg->setAxisScale(QwtPlot::yLeft,
...
void US_Hydrodyn_Mals_Saxs::guinier_rg_rg_start_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Mals_Saxs::guinier_rg_rg_end_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Mals_Saxs::guinier_mw_t_start_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
...
void US_Hydrodyn_Mals_Saxs::guinier_mw_t_end_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  guinier_plot_mw->setAxisScale(QwtPlot::xBottom,
...
void US_Hydrodyn_Mals_Saxs::guinier_mw_t_start_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Mals_Saxs::guinier_mw_t_end_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Mals_Saxs::guinier_mw_mw_start_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
...
void US_Hydrodyn_Mals_Saxs::guinier_mw_mw_end_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  guinier_plot_mw->setAxisScale(QwtPlot::yLeft,
...
void US_Hydrodyn_Mals_Saxs::guinier_mw_mw_start_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Mals_Saxs::guinier_mw_mw_end_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
