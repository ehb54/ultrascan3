...
US_Hydrodyn_Mals_Saxs_Svd::US_Hydrodyn_Mals_Saxs_Svd (
    ...
    vector<QString> mals_saxs_selected_files, QWidget *p)
    : QFrame (p)
{
  ...
  if (mals_saxs_win->f_is_time[this->mals_saxs_selected_files[0]])
    {
      ...
      QString error_msg;
      if (!convert_it_to_iq (qsl_mals_saxs_selected_files, original_data,
                             error_msg))
        {
          QMessageBox::warning (
              this, windowTitle (),
...
void
US_Hydrodyn_Mals_Saxs_Svd::setupGUI ()
{
  int minHeight1 = 24;
  int minHeight3 = 25;
  ...
  pb_to_mals_saxs->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize - 1));
  ...
  pb_efa_decomp_to_mals_saxs_saxs
      = new QPushButton (us_tr ("To HPLC window"), this);
  pb_efa_decomp_to_mals_saxs_saxs->setFont (QFont (
      USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1));
  pb_efa_decomp_to_mals_saxs_saxs->setMinimumHeight (minHeight1);
  pb_efa_decomp_to_mals_saxs_saxs->setPalette (PALET_PUSHB);
  connect (pb_efa_decomp_to_mals_saxs_saxs, SIGNAL (clicked ()),
           SLOT (efa_decomp_to_mals_saxs_saxs ()));
  ...
  hide_widgets (data_widgets,
                !ush_win->gparams.count ("mals_saxs_svd_data_widgets")
                        || ush_win->gparams["mals_saxs_svd_data_widgets"]
                               == "false"
                    ? false
                    : true);
  hide_widgets (editor_widgets,
                !ush_win->gparams.count ("mals_saxs_svd_editor_widgets")
                        || ush_win->gparams["mals_saxs_svd_editor_widgets"]
                               == "false"
                    ? false
                    : true);
  hide_widgets (process_widgets,
                !ush_win->gparams.count ("mals_saxs_svd_process_widgets")
                        || ush_win->gparams["mals_saxs_svd_process_widgets"]
                               == "false"
                    ? false
                    : true);
...
void
US_Hydrodyn_Mals_Saxs_Svd::cancel ()
{
  close ();
}

void
US_Hydrodyn_Mals_Saxs_Svd::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Mals_Saxs_Svd::closeEvent (QCloseEvent *e)
{
  e->accept ();
}

void
US_Hydrodyn_Mals_Saxs_Svd::clear_display ()
{
  editor->clear ();
  editor->append ("\n\n");
}
...
void
US_Hydrodyn_Mals_Saxs_Svd::update_font ()
{
  bool ok;
  QFont newFont;
...
void
US_Hydrodyn_Mals_Saxs_Svd::save ()
{
  QString fn;
...
void
US_Hydrodyn_Mals_Saxs_Svd::editor_msg (QString color, QString msg)
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::plot_files ()
{
  // puts( "plot_files" );
...
void
US_Hydrodyn_Mals_Saxs_Svd::plot_files (QStringList add_files, norms norm_mode)
{
  // puts( "plot_files" );
  plot_data->detachItems (QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems (QwtPlotItem::Rtti_PlotMarker);
...
bool
US_Hydrodyn_Mals_Saxs_Svd::plot_file (QString file, double &minx, double &maxx,
...
bool
US_Hydrodyn_Mals_Saxs_Svd::get_min_max (QString file, double &minx,
...
void
US_Hydrodyn_Mals_Saxs_Svd::disable_all ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::q_start_text (const QString &)
{
}

void
US_Hydrodyn_Mals_Saxs_Svd::q_end_text (const QString &)
{
}
...
void
US_Hydrodyn_Mals_Saxs_Svd::replot (bool keep_mode)
{
  disable_all ();
...
void
US_Hydrodyn_Mals_Saxs_Svd::iq_it ()
{
  disable_all ();
...
void
US_Hydrodyn_Mals_Saxs_Svd::to_mals_saxs ()
{
  ...
  for (int i = 0; i < (int)files.size (); ++i)
    {
      mals_saxs_win->add_plot (files[i], f_qs[files[i]], f_Is[files[i]],
...
void
US_Hydrodyn_Mals_Saxs_Svd::axis_x ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::axis_y ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::hide_data ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::hide_editor ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::hide_process ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::hide_widgets (vector<QWidget *> widgets, bool hide)
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::update_enables ()
{
  // count selections
  if (running)
    {
  ...
  pb_to_mals_saxs->setEnabled (files.size ()
                               && !sources.count (us_tr ("Original data"))
                               && ush_win->mals_saxs_widget);
...
void
US_Hydrodyn_Mals_Saxs_Svd::data_selection_changed ()
{
  update_enables ();
}

void
US_Hydrodyn_Mals_Saxs_Svd::sv_selection_changed ()
{
  update_enables ();
}

void
US_Hydrodyn_Mals_Saxs_Svd::clear ()
{
...
QStringList
US_Hydrodyn_Mals_Saxs_Svd::selected_files ()
{
  QStringList result;
...
set<QString>
US_Hydrodyn_Mals_Saxs_Svd::get_current_files ()
{
  set<QString> result;
...
set<QString>
US_Hydrodyn_Mals_Saxs_Svd::get_sources ()
{
  set<QString> result;
...
set<QString>
US_Hydrodyn_Mals_Saxs_Svd::get_selected_sources ()
{
  set<QString> result;
  QString iq_or_it = iq_it_state ? "I(t)" : "I(q)";
...
QTreeWidgetItem *
US_Hydrodyn_Mals_Saxs_Svd::get_source_item (QString source)
{
  QTreeWidgetItemIterator it (lv_data);
  while ((*it))
    {
...
int
US_Hydrodyn_Mals_Saxs_Svd::selected_sources ()
{
  QString iq_or_it = iq_it_state ? "I(t)" : "I(q)";
...
void
US_Hydrodyn_Mals_Saxs_Svd::clean_selected ()
{
...
bool
US_Hydrodyn_Mals_Saxs_Svd::all_children_selected (QTreeWidgetItem *lvi)
{
...
bool
US_Hydrodyn_Mals_Saxs_Svd::is_selected (QTreeWidgetItem *lvi)
{
  if (lvi->isSelected ())
    {
      return true;
...
void
US_Hydrodyn_Mals_Saxs_Svd::axis_x_title ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::axis_y_title ()
{
...
bool
US_Hydrodyn_Mals_Saxs_Svd::plotted_matches_selected ()
{
  set<QString> plotted;
...
void
US_Hydrodyn_Mals_Saxs_Svd::add_i_of_q_or_t (QString source, QStringList files,
...
void
US_Hydrodyn_Mals_Saxs_Svd::rescale (bool do_update_enables)
{
  // bool any_selected = false;
  double minx = 0e0;
  double maxx = 1e0;
...
void
US_Hydrodyn_Mals_Saxs_Svd::svd_plot (bool axis_change)
{
...
QString
US_Hydrodyn_Mals_Saxs_Svd::get_name ()
{
  QTreeWidgetItemIterator it (lv_data);
  while ((*it))
    {
...
void
US_Hydrodyn_Mals_Saxs_Svd::recon ()
{
...
QTreeWidgetItem *
US_Hydrodyn_Mals_Saxs_Svd::lvi_last_depth (int d)
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::color_rotate ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::inc_rmsd_plot (bool axis_change)
{
  plot_data->detachItems (QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
...
void
US_Hydrodyn_Mals_Saxs_Svd::inc_chi_plot (bool axis_change)
{
  plot_data->detachItems (QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
...
void
US_Hydrodyn_Mals_Saxs_Svd::inc_recon ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::do_recon ()
{
...
double
US_Hydrodyn_Mals_Saxs_Svd::vmin (vector<double> &x)
{
  double min = x[0];
...
double
US_Hydrodyn_Mals_Saxs_Svd::vmax (vector<double> &x)
{
  if (!x.size ())
    {
...
void
US_Hydrodyn_Mals_Saxs_Svd::stop ()
{
  pb_stop->setEnabled (false);
  running = false;
...
void
US_Hydrodyn_Mals_Saxs_Svd::indiv_recon ()
{
  recon_mode = "Indiv. ";
...
void
US_Hydrodyn_Mals_Saxs_Svd::set_plot_errors ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::set_plot_errors_rev ()
{
  update_plot_errors ();
}

void
US_Hydrodyn_Mals_Saxs_Svd::set_plot_errors_sd ()
{
  if (plot_errors_zoomer)
    {
      plot_errors_zoomer->zoom (0);
...
void
US_Hydrodyn_Mals_Saxs_Svd::set_plot_errors_pct ()
{
  if (plot_errors_zoomer)
    {
      plot_errors_zoomer->zoom (0);
...
void
US_Hydrodyn_Mals_Saxs_Svd::set_plot_errors_group ()
{
  if (plot_errors_zoomer)
    {
      plot_errors_zoomer->zoom (0);
...
void
US_Hydrodyn_Mals_Saxs_Svd::set_plot_errors_ref ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::update_plot_errors (bool do_update_enables)
{
...
QStringList
US_Hydrodyn_Mals_Saxs_Svd::add_subset_data (QStringList files)
{
  QStringList result;
...
QString
US_Hydrodyn_Mals_Saxs_Svd::get_related_source_name (QString name)
{
...
QStringList
US_Hydrodyn_Mals_Saxs_Svd::get_files_by_name (QString name)
{
...
bool
US_Hydrodyn_Mals_Saxs_Svd::get_name_norm_mode (QString fullname, QString &name,
                                               norms &norm_mode)
{
...
bool
US_Hydrodyn_Mals_Saxs_Svd::get_plot_files (QStringList &use_list,
...
void
US_Hydrodyn_Mals_Saxs_Svd::do_plot_errors ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::plot_errors_jump_markers ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::do_plot_errors_group ()
{
...
bool
US_Hydrodyn_Mals_Saxs_Svd::setup_save (QString tag, QString &fname)
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::rmsd_save ()
{
  QString fname;
...
void
US_Hydrodyn_Mals_Saxs_Svd::svd_save ()
{
  QString fname;
...
void
US_Hydrodyn_Mals_Saxs_Svd::usp_config_plot_data (const QPoint &)
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::usp_config_plot_svd (const QPoint &)
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::usp_config_plot_errors (const QPoint &)
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::usp_config_plot_lefa (const QPoint &)
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::usp_config_plot_refa (const QPoint &)
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::usp_config_plot_ac (const QPoint &)
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::usp_config_plot_efa_decomp (const QPoint &)
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::set_norm_pw ()
{
  if (cb_norm_avg->isChecked ())
    {
      cb_norm_avg->setChecked (false);
...
void
US_Hydrodyn_Mals_Saxs_Svd::set_norm_avg ()
{
  if (cb_norm_pw->isChecked ())
    {
      cb_norm_pw->setChecked (false);
...
void
US_Hydrodyn_Mals_Saxs_Svd::clear_svd ()
{
  plot_svd->detachItems (QwtPlotItem::Rtti_PlotCurve);
...
void
US_Hydrodyn_Mals_Saxs_Svd::clear_efa ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::clear_efa_decomp ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::make_plotted_efas ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::update_efas (double val)
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::efa_plot ()
{
  int n = (int)efa_lsv.size ();
  if (!n)
    {
...
void
US_Hydrodyn_Mals_Saxs_Svd::setup_norm ()
{
  unsigned int q_points
      = (unsigned int)f_qs[mals_saxs_selected_files[0]].size ();
  ...
  US_Vector::printvector3 ("file 0 I, errors, I_norm_pw",
  ...
  US_Vector::printvector3 ("file 0 I, avg_errors, I_norm_avg",
                           f_Is[mals_saxs_selected_files[0]], avg_errors,
...
void
US_Hydrodyn_Mals_Saxs_Svd::check_norm (QStringList &files)
{
...
double
US_Hydrodyn_Mals_Saxs_Svd::autocor1 (vector<double> &x)
{
  int len = (int)x.size ();
...
vector<double>
US_Hydrodyn_Mals_Saxs_Svd::autocor (vector<vector<double> > &A)
{
...
vector<vector<int> >
US_Hydrodyn_Mals_Saxs_Svd::transpose (vector<vector<int> > &A)
{
  ...
  //   qDebug() << "US_Hydrodyn_Mals_Saxs_Svd::transpose( vvi ) rows " << rows
  //   << " cols " << cols;
...
US_Hydrodyn_Mals_Saxs_Svd::transpose (vector<vector<double> > &A)
{
  ...
  //   qDebug() << "US_Hydrodyn_Mals_Saxs_Svd::transpose( vvd ) rows " << rows
  //   << " cols " << cols;
...
double
US_Hydrodyn_Mals_Saxs_Svd::vvd_min (vector<vector<double> > &A)
{
  double result = -1e99;
  int rows = (int)A.size ();
  if (!rows)
...
double
US_Hydrodyn_Mals_Saxs_Svd::vvd_max (vector<vector<double> > &A)
{
  double result = 1e99;
  int rows = (int)A.size ();
  if (!rows)
...
void
US_Hydrodyn_Mals_Saxs_Svd::vvd_smult (vector<vector<double> > &A, double x)
{
  int rows = (int)A.size ();
  if (!rows)
    {
...
vector<vector<double> >
US_Hydrodyn_Mals_Saxs_Svd::vivd_pwmult (vector<vector<int> > &A,
...
void
US_Hydrodyn_Mals_Saxs_Svd::vvd_cnorm (vector<vector<double> > &A)
{
  int rows = (int)A.size ();
  if (!rows)
    {
...
vector<vector<double> >
US_Hydrodyn_Mals_Saxs_Svd::dot (vector<vector<double> > &A,
                                vector<vector<double> > &B)
{
  vector<vector<double> > result;
  int arows = (int)A.size ();
...
void
US_Hydrodyn_Mals_Saxs_Svd::matrix_info (QString qs, vector<vector<double> > &A)
{
  if (!A.size ())
    {
      qDebug () << "US_Hydrodyn_Mals_Saxs_Svd::matrix_info() " << qs
                << " empty!";
      return;
    }
  qDebug () << "US_Hydrodyn_Mals_Saxs_Svd::matrix_info() " << qs << " rows "
            << A.size () << " cols " << A[0].size ();
}

void
US_Hydrodyn_Mals_Saxs_Svd::set_title (QwtPlot *plot, QString title)
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::save_plots ()
{
  QString use_dir = QDir::currentPath ();
  ush_win->select_from_directory_history (use_dir, this);
...
void
US_Hydrodyn_Mals_Saxs_Svd::mode_select ()
{
  // first hide all
...
void
US_Hydrodyn_Mals_Saxs_Svd::mode_select (modes mode)
{
  current_mode = mode;
  mode_select ();
  update_enables ();
...
void
US_Hydrodyn_Mals_Saxs_Svd::set_mode_iqit ()
{
  mode_select (MODE_IQIT);
}

void
US_Hydrodyn_Mals_Saxs_Svd::set_mode_svd ()
{
  mode_select (MODE_SVD);
}

void
US_Hydrodyn_Mals_Saxs_Svd::set_mode_efa ()
{
  mode_select (MODE_EFA);
}

void
US_Hydrodyn_Mals_Saxs_Svd::set_mode_efa_decomp ()
{
  mode_select (MODE_EFA_DECOMP);
}

void
US_Hydrodyn_Mals_Saxs_Svd::update_efa_range_start (double)
{
  // qDebug() << "US_Hydrodyn_Mals_Saxs_Svd::update_efa_range_start";
  // {
  //    QTextStream ts( stdout );
  //    for ( int i = 0; i < UHSHS_MAX_SV_FOR_EFA; ++i ) {
  //       ts << "US_Hydrodyn_Mals_Saxs_Svd::update_efa_range_start SV " << i
...
void
US_Hydrodyn_Mals_Saxs_Svd::update_efa_range_end (double)
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::update_efas_ranges ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::efa_info (QString tag)
{
  QTextStream ts (stdout);
...
vector<double>
US_Hydrodyn_Mals_Saxs_Svd::gradient (vector<double> &y)
{
  int len = (int)y.size ();
  int lenm1 = len - 1;
...
void
US_Hydrodyn_Mals_Saxs_Svd::init_efa_values ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::set_number_of_svs_for_efa ()
{
  int max_range
  ...
  // "US_Hydrodyn_Mals_Saxs_Svd::set_number_of_svs_for_efa svd_autocor_U, V",
...
bool
US_Hydrodyn_Mals_Saxs_Svd::convert_it_to_iq (QStringList files,
      ...
      if (!mals_saxs_win->f_qs.count (files[i]))
      ...
      else
        {
          for (unsigned int j = 0;
               j < (unsigned int)mals_saxs_win->f_qs[files[i]].size (); j++)
            {
              if (!mode_testiq
                  || (mals_saxs_win->f_qs[files[i]][j] >= t_min
                      && mals_saxs_win->f_qs[files[i]][j] <= t_max))
                {
                  I_values[mals_saxs_win->f_qs[files[i]][j]][ql.back ()]
                      = mals_saxs_win->f_Is[files[i]][j];
                  if (use_errors
                      && mals_saxs_win->f_errors[files[i]].size ()
                             == mals_saxs_win->f_qs[files[i]].size ())
                    {
                      e_values[mals_saxs_win->f_qs[files[i]][j]][ql.back ()]
                          = mals_saxs_win->f_errors[files[i]][j];
                    }
                  else
                    {
                      if (use_errors)
          ...
          if (!mals_saxs_win->f_errors.count (files[i])
              || mals_saxs_win->f_errors[files[i]].size ()
                     != mals_saxs_win->f_Is[files[i]].size ())
            {
            ...
            {
              if (!mals_saxs_win->is_nonzero_vector (
                      mals_saxs_win->f_errors[files[i]]))
                {
                  unsigned int zero_pts = 0;
                  for (unsigned int j = 0;
                       j < (unsigned int)mals_saxs_win->f_errors[files[i]]
                               .size ();
                       j++)
                    {
                      if (!mode_testiq
                          || (mals_saxs_win->f_qs[files[i]][j] >= t_min
                              && mals_saxs_win->f_qs[files[i]][j] <= t_max))
                        {
                          if (us_isnan (mals_saxs_win->f_errors[files[i]][j])
                              || mals_saxs_win->f_errors[files[i]][j] == 0e0)
                            {
                              zero_pts++;
                            }
                        }
                  ...
                  zero_points[files[i]]
                      = QString ("%1: %2 of %3 points")
                            .arg (files[i])
                            .arg (zero_pts)
                            .arg (mals_saxs_win->f_errors[files[i]].size ());
...
void
US_Hydrodyn_Mals_Saxs_Svd::runExplicitEFARotation (
...
void
US_Hydrodyn_Mals_Saxs_Svd::efa_decomp ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::efa_decomp_plot ()
{
...
void
US_Hydrodyn_Mals_Saxs_Svd::initExplicitEFA (
...
void
US_Hydrodyn_Mals_Saxs_Svd::initIterativeEFA (
...
void
US_Hydrodyn_Mals_Saxs_Svd::EFAFirstRotation (vector<vector<int> > &M,
...
void
US_Hydrodyn_Mals_Saxs_Svd::initHybridEFA (
    vector<vector<int> > &M, int num_sv, vector<vector<double> > &D,
    vector<vector<double> > &C, bool &converged,
...
void
US_Hydrodyn_Mals_Saxs_Svd::runIterativeEFARotation (
    vector<vector<int> > &M, vector<vector<double> > &D,
    bool &failed, // also a return
    vector<vector<double> > &C, vector<vector<double> > & /* V_bar */,
    vector<vector<double> > & /* T */, int niter, double tol,
...
void
US_Hydrodyn_Mals_Saxs_Svd::EFAUpdateRotation (vector<vector<int> > &M,
                                              vector<vector<double> > &C,
                                              vector<vector<double> > &D,
                                              vector<int> &force_pos,
                                              // returns
...
void
US_Hydrodyn_Mals_Saxs_Svd::efa_decomp_to_mals_saxs_saxs ()
{
  ...
  for (int i = 0; i < (int)efa_decomp_Ct.size (); ++i)
    {
      QString this_name = QString ("EFA_of_%1_component_%2")
                              ...
                              .arg (i + 1);
      mals_saxs_win->add_plot (this_name, efa_decomp_x, efa_decomp_Ct[i],
                               true);
...
void
US_Hydrodyn_Mals_Saxs_Svd::set_efa_decomp_force_positive ()
{
