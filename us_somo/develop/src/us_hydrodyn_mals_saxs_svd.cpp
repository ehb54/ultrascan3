...
void US_Hydrodyn_Mals_Saxs_Svd::setupGUI() {
  ...
  hide_widgets(data_widgets,
               !ush_win->gparams.count("mals_saxs_svd_data_widgets") ||
                       ush_win->gparams["mals_saxs_svd_data_widgets"] == "false"
                   ? false
                   : true);
  hide_widgets(
      editor_widgets,
      !ush_win->gparams.count("mals_saxs_svd_editor_widgets") ||
              ush_win->gparams["mals_saxs_svd_editor_widgets"] == "false"
          ? false
          : true);
  hide_widgets(
      process_widgets,
      !ush_win->gparams.count("mals_saxs_svd_process_widgets") ||
              ush_win->gparams["mals_saxs_svd_process_widgets"] == "false"
          ? false
          : true);
  mode_select(MODE_IQIT);
}

void US_Hydrodyn_Mals_Saxs_Svd::cancel() { close(); }
...
void US_Hydrodyn_Mals_Saxs_Svd::closeEvent(QCloseEvent *e) { e->accept(); }
...
void US_Hydrodyn_Mals_Saxs_Svd::plot_files(QStringList add_files,
                                           norms norm_mode) {
  // puts( "plot_files" );
  plot_data->detachItems(QwtPlotItem::Rtti_PlotCurve);
  plot_data->detachItems(QwtPlotItem::Rtti_PlotMarker);
...
bool US_Hydrodyn_Mals_Saxs_Svd::plot_file(QString file, double &minx,
...
bool US_Hydrodyn_Mals_Saxs_Svd::get_min_max(QString file, double &minx,
                                            double &maxx, double &miny,
                                            double &maxy, norms norm_mode) {
  map<QString, vector<double> > *f_Iuse = &f_Is;
...
void US_Hydrodyn_Mals_Saxs_Svd::hide_widgets(vector<QWidget *> widgets,
                                             bool hide) {
...
void US_Hydrodyn_Mals_Saxs_Svd::data_selection_changed() { update_enables(); }

void US_Hydrodyn_Mals_Saxs_Svd::sv_selection_changed() { update_enables(); }
...
void US_Hydrodyn_Mals_Saxs_Svd::add_i_of_q_or_t(QString source,
                                                QStringList files,
...
void US_Hydrodyn_Mals_Saxs_Svd::set_plot_errors_rev() { update_plot_errors(); }
...
bool US_Hydrodyn_Mals_Saxs_Svd::get_name_norm_mode(QString fullname,
                                                   QString &name,
                                                   norms &norm_mode) {
...
bool US_Hydrodyn_Mals_Saxs_Svd::get_plot_files(QStringList &use_list,
...
vector<vector<int> > US_Hydrodyn_Mals_Saxs_Svd::transpose(
    vector<vector<int> > &A) {
...
vector<vector<double> > US_Hydrodyn_Mals_Saxs_Svd::transpose(
...
void US_Hydrodyn_Mals_Saxs_Svd::vvd_smult(vector<vector<double> > &A,
                                          double x) {
  int rows = (int)A.size();
  if (!rows) {
...
vector<vector<double> > US_Hydrodyn_Mals_Saxs_Svd::vivd_pwmult(
...
vector<vector<double> > US_Hydrodyn_Mals_Saxs_Svd::dot(
...
void US_Hydrodyn_Mals_Saxs_Svd::matrix_info(QString qs,
                                            vector<vector<double> > &A) {
  if (!A.size()) {
...
void US_Hydrodyn_Mals_Saxs_Svd::set_mode_iqit() { mode_select(MODE_IQIT); }

void US_Hydrodyn_Mals_Saxs_Svd::set_mode_svd() { mode_select(MODE_SVD); }

void US_Hydrodyn_Mals_Saxs_Svd::set_mode_efa() { mode_select(MODE_EFA); }
...
bool US_Hydrodyn_Mals_Saxs_Svd::convert_it_to_iq(QStringList files,
    ...
    if (!mals_saxs_win->f_qs.count(files[i])) {
      ...
      if (!mals_saxs_win->f_errors.count(files[i]) ||
        ...
        if (!mals_saxs_win->is_nonzero_vector(
          ...
          for (unsigned int j = 0;
               j < (unsigned int)mals_saxs_win->f_errors[files[i]].size();
               j++) {
            if (!mode_testiq || (mals_saxs_win->f_qs[files[i]][j] >= t_min &&
                                 mals_saxs_win->f_qs[files[i]][j] <= t_max)) {
...
void US_Hydrodyn_Mals_Saxs_Svd::EFAFirstRotation(
    vector<vector<int> > &M, vector<vector<double> > &C,
...
void US_Hydrodyn_Mals_Saxs_Svd::EFAUpdateRotation(
    vector<vector<int> > &M, vector<vector<double> > &C,
    vector<vector<double> > &D, vector<int> &force_pos,
    // returns
    vector<vector<double> > &Cnew) {
  /*
...
void US_Hydrodyn_Mals_Saxs_Svd::efa_decomp_to_mals_saxs_saxs() {
  ...
  for (int i = 0; i < (int)efa_decomp_Ct.size(); ++i) {
    QString this_name = QString("EFA_of_%1_component_%2")
                            ...
                            .arg(i + 1);
    mals_saxs_win->add_plot(this_name, efa_decomp_x, efa_decomp_Ct[i], true);
