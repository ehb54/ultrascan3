...
void US_Hydrodyn_Mals_Saxs_Cistarq::setupGUI()
{
   ...
   lbl_title =  new QLabel      ( parameters->count( "ngmode" ) ?  us_tr( "US-SOMO: MALS_SAXS : Make I*(q) without Gaussians" ) :  us_tr( "US-SOMO: MALS_SAXS : Make I*(q)" ), this );
...
void US_Hydrodyn_Mals_Saxs_Cistarq::quit()
{
   close();
}
...
void US_Hydrodyn_Mals_Saxs_Cistarq::set_sd_source()
{
   update_enables();
}

void US_Hydrodyn_Mals_Saxs_Cistarq::set_normalize()
{
   update_enables();
}

void US_Hydrodyn_Mals_Saxs_Cistarq::set_I0se()
{
   update_enables();
}
