...
US3i_Color::US3i_Color (QWidget *w) : US3i_widgets (true, w)
{
  ...
  // Plot example
  us_plot = new US_Plot (plot, tr ("Sample Plot"), tr ("X-axis"), ("Y-axis"));
  ...
  // For some reason, this needs to be after the first call to
  // updateSchemeList();
  ...
  connect (elements, SIGNAL (currentRowChanged (int)), this,
           SLOT (selectedElement (int)));
...
void
US3i_Color::getCurrentSettings (void)
{
...
void
US3i_Color::selectedElement (int index)
{
  QPalette p;
  QColor c;
...
void
US3i_Color::resetEditBoxes (void)
{
...
US3i_Color::pick_color1 (void)
{
  ...
  switch (elements->currentRow ())
    ...
    case FRAME:
      ...
      current.frameColor.setColor (QPalette::Inactive, QPalette::WindowText,
                                   c);
      current.frameColor.setColor (QPalette::Disabled, QPalette::WindowText,
                                   c);
      resetFrames ();
      break;
    ...
    case DISABLED_PB:
      current.pushbColor.setColor (QPalette::Disabled, QPalette::ButtonText,
                                   c);
      resetButtons ();
      break;
    ...
    case LABELS:
      ...
      current.labelColor.setColor (QPalette::Inactive, QPalette::WindowText,
                                   c);
      current.labelColor.setColor (QPalette::Disabled, QPalette::WindowText,
                                   c);
      resetLabels ();
      break;
...
US3i_Color::pick_color3 (void)
{
  ...
  switch (elements->currentRow ())
    ...
    case NORMAL_PB:
      current.pushbColor.setColor (QPalette::Inactive, QPalette::ButtonText,
                                   c);
      resetButtons ();
      break;
...
US3i_Color::pick_color4 (void)
{
  ...
  switch (elements->currentRow ())
    ...
    case OTHER_WIDGETS:
      ...
      current.normalColor.setColor (QPalette::Inactive, QPalette::ButtonText,
                                    c);
      current.normalColor.setColor (QPalette::Disabled, QPalette::ButtonText,
                                    c);
      resetWidgets ();
      break;
...
void
US3i_Color::updateWidgets (double newval)
{
  lcd->display (newval);
  progress->reset ();
...
void
US3i_Color::selMargin (int index)
{
  current.plotMargin = (index + 1) * 2;
  // plot->setMargin( current.plotMargin );
  plot->setStyleSheet (
      QString ("QwtPlot{ padding: %1px }").arg (current.plotMargin));
}

void
US3i_Color::apply (void)
{
...
US3i_Color::save_as (void)
{
  ...
  if (custom.size () == 0)
    {
      QMessageBox::warning (this, tr ("Save scheme"),
...
void
US3i_Color::updateSchemeList (const QString &active)
{
  schemes->clear ();
...
void
US3i_Color::selected_scheme (void)
{
  switch (schemes->currentRow ())
    ...
    default:
      ...
      QSettings settings ("UTHSCSA", "UltraScan");
      current.frameColor
          = settings.value (custom + "frameColor").value<QPalette> ();
      current.pushbColor
          = settings.value (custom + "pushbColor").value<QPalette> ();
      current.labelColor
          = settings.value (custom + "labelColor").value<QPalette> ();
      current.editColor
          = settings.value (custom + "editColor").value<QPalette> ();
      current.normalColor
          = settings.value (custom + "normalColor").value<QPalette> ();
      current.lcdColor
          = settings.value (custom + "lcdColor").value<QPalette> ();
      current.plotColor
          = settings.value (custom + "plotColor").value<QPalette> ();

      current.plotMargin = settings.value (custom + "plotMargin").toInt ();

      current.plotCurve
          = settings.value (custom + "plotCurve").value<QColor> ();
      current.plotBg = settings.value (custom + "plotBg").value<QColor> ();
      current.plotMajorGrid
          = settings.value (custom + "plotMajGrid").value<QColor> ();
      current.plotMinorGrid
          = settings.value (custom + "plotMinGrid").value<QColor> ();
      current.plotPicker
          = settings.value (custom + "plotPicker").value<QColor> ();
...
void
US3i_Color::selectStyle (const QString &styleName)
{
