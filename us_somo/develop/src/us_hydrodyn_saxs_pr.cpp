...
double US_Hydrodyn_Saxs::get_mw(QString filename, bool display_mw_msg,
  ...
  if (found) {
    ...
    US_Hydrodyn_Saxs_Mw *smw = new US_Hydrodyn_Saxs_Mw(
        msg, &mw, &last_used_mw, &remember, &use_partial, &partial, remember_mw,
        remember_mw_source, allow_none, this);
...
void US_Hydrodyn_Saxs::check_pr_grid(vector<double> &r, vector<double> &pr,
  ...
  if (r.size() != pr.size()) {
    editor_msg("red", QString("Internal error: the r grid has %1 points, put "
                              "the P(r) values have %2 points!")
                          .arg(r.size())
                          .arg(pr.size()));
    return;
  }
  ...
  editor_msg("dark red",
             "The P(r) grid does not have uniform spacing: interpolating to "
             "current bin size");
...
bool US_Hydrodyn_Saxs::mw_from_I0(QString name, double I0_exp, double &MW,
  ...
  if (our_saxs_options->guinier_use_standards) {
    ...
    if (I0_std_exp == 0e0) {
      errormsg =
          us_tr("Error: I0 standard experimental is 0, can not compute MW");
      MW = 0e0;
      return false;
    }
  ...
  internal_contrast =
      ...
      (1e0 / (guinier_electron_nucleon_ratio * our_saxs_options->nucleon_mass) -
...
bool US_Hydrodyn_Saxs::ml_from_qI0(QString name, double I0_exp, double &ML,
                                   double &internal_contrast) {
  double conc;
  double psv;
  double I0_std_exp;
  ...
  if (our_saxs_options->guinier_use_standards) {
    if (I0_std_exp == 0e0) {
      errormsg =
          us_tr("Error: I0 standard experimental is 0, can not compute ML");
      ML = 0e0;
      return false;
    }
    I0_exp_to_theo_mult = our_saxs_options->I0_theo / I0_std_exp;
...
bool US_Hydrodyn_Saxs::ma_from_q2I0(QString name, double I0_exp, double &MA,
                                    double &internal_contrast) {
  double conc;
  double psv;
  double I0_std_exp;
  ...
  if (our_saxs_options->guinier_use_standards) {
    if (I0_std_exp == 0e0) {
      errormsg =
          us_tr("Error: I0 standard experimental is 0, can not compute MA");
      MA = 0e0;
      return false;
    }
    I0_exp_to_theo_mult = our_saxs_options->I0_theo / I0_std_exp;
...
void US_Hydrodyn_Saxs::sync_conc_csv()  // removes deleted curves, adds
                                        // non-extant curves with default
{
...
void US_Hydrodyn_Saxs::add_conc_csv(QString name, double conc, double psv,
                                    double I0_std) {
#if defined(DEBUG_CSV)
...
bool US_Hydrodyn_Saxs::get_conc_csv_values(QString name, double &conc,
                                           double &psv, double &I0_std_exp) {
  for (unsigned int i = 0; i < conc_csv.data.size(); i++) {
    if (name == conc_csv.data[i][0]) {
...
void US_Hydrodyn_Saxs::pr_info2(const QString &msg) { pr_info(msg, true); }
...
void US_Hydrodyn_Saxs::pr_info(const QString &msg, bool detail) {
  ...
  for (int i = 0; i < sizes.back(); ++i) {
    if (detail) {
      ...
      TSO << QString(
                 "entry %1 [%8]: r %2 pr %3 pr_error %4 pr_nn %5 pr_nn_e %6 "
                 "pr_mw %7\n")
                 .arg(i)
                 .arg((int)plotted_r.size() > i ? (int)plotted_r[i].size() : -1)
                 .arg((int)plotted_pr.size() > i ? (int)plotted_pr[i].size()
                                                 : -1)
                 ...
                 .arg((int)plotted_pr_not_normalized.size() > i
                          ...
                          : -1)
                 .arg((int)plotted_pr_not_normalized_error.size() > i
...
bool US_Hydrodyn_Saxs::pr_to_iq() {
  ...
  QString source = "";
  {
    ...
    source = US_Static::getItem(
        us_tr("Compute I(q)"), us_tr("Select the P(r) curve to calculate I(q)"),
  ...
  errormsg =
      QString(
          us_tr(
              "Internal error, could not find source '%1' in plotted names\n"))
          .arg(source);
...
bool US_Hydrodyn_Saxs::pr_to_iq(int pos, QString name) {
  ...
  if (our_saxs_options->iq_exact_q) {
    ...
    if (!exact_q.size()) {
      editor_msg("dark red",
                 QString(us_tr("Notice: exact q is empty, computing based upon "
                               "current q range ")));
...
bool US_Hydrodyn_Saxs::pr_to_iq(const map<double, double> &pr_exact,
                                QString name) {
  errormsg = "";
