...
double
US_Hydrodyn_Saxs::get_mw (QString filename, bool display_mw_msg,
                          bool allow_none)
{
  // enter MW and PSV
      ...
      if (!(*match_remember_mw).empty ())
        {
          ...
          for (map<QString, float>::iterator it
               = (*match_remember_mw).begin ();
               it != (*match_remember_mw).end (); it++)
            {
              ...
              if (filename.contains (it->first))
                {
                  ...
                  source = QString (us_tr ("file name matched \"%1\""))
                               .arg (it->first);
                  found = true;
                  break;
                }
  ...
  if (found)
  ...
  else
    {
      US_Hydrodyn_Saxs_Mw *smw = new US_Hydrodyn_Saxs_Mw (
          msg, &mw, &last_used_mw, &remember, &use_partial, &partial,
          remember_mw, remember_mw_source, allow_none, this);
  ...
  //   printf( "%s is %g\n",  QString( "get mw filename %1" ).arg( filename
  //   ).toLatin1().data(), mw );
  return mw;
}

void
US_Hydrodyn_Saxs::normalize_pr (vector<double> r, vector<double> *pr,
                                double mw)
{
...
void
US_Hydrodyn_Saxs::check_pr_grid (vector<double> &r, vector<double> &pr)
{
                                 ...
                                 vector<double> &pr_error)
{
  ...
  if (r.size () != pr.size ())
    {
      editor_msg ("red", QString ("Internal error: the r grid has %1 points, "
                                  "put the P(r) values have %2 points!")
                             .arg (r.size ())
                             .arg (pr.size ()));
      return;
    }
  ...
  editor_msg ("dark red", "The P(r) grid does not have uniform spacing: "
                          "interpolating to current bin size");

  vector<double> new_r;
  double start_r = r[0];
  double end_r = r[r.size () - 1];

  for (double this_r = start_r; this_r <= end_r + our_saxs_options->bin_size;
  ...
  if (pr_error.size ())
    {
      ...
      // for ( unsigned int i = 0; i < new_r.size(); i++ )
      // {
      //    if ( !usu.apply_natural_spline( r, pr_error, y2, new_r[ i ],
      //    new_pr_error[ i ] ) )
      //    {
      //       editor_msg( "red", usu.errormsg );
      //       editor_msg( "red", us_tr( "Error attempting to interpolate" ) );
...
bool
US_Hydrodyn_Saxs::mw_from_I0 (QString name, double I0_exp, double &MW,
                              double &internal_contrast)
{
  ...
  if (our_saxs_options->guinier_use_standards)
    {
      ...
      if (I0_std_exp == 0e0)
        {
          errormsg = us_tr (
              "Error: I0 standard experimental is 0, can not compute MW");
          MW = 0e0;
          return false;
        }
  ...
  internal_contrast = our_saxs_options->diffusion_len
                      * (1e0
                             / (guinier_electron_nucleon_ratio
                                * our_saxs_options->nucleon_mass)
                         - psv * (1e24 * our_saxs_options->water_e_density));

  MW = I0_prot_theo * AVOGADRO / (conc * 1e-3)
       / (internal_contrast * internal_contrast);

#if defined(DEBUG_MW)
  cout << QString ("I0_std_theo         %1\n"
                   "I0_std_exp          %2\n")
              .arg (our_saxs_options->I0_theo, 0, 'e', 8)
              .arg (I0_std_exp, 0, 'e', 8);

  cout << QString ("I0_exp_to_theo_mult %1\n"
                   "diffusion len       %2\n"
                   "psv                 %3\n"
                   "water_e_density     %4\n"
                   "I0_prot_theo        %5\n"
...
bool
US_Hydrodyn_Saxs::ml_from_qI0 (QString name, double I0_exp, double &ML,
                               double &internal_contrast)
{
  double conc;
  double psv;
  ...
  if (our_saxs_options->guinier_use_standards)
    {
      if (I0_std_exp == 0e0)
        {
          errormsg = us_tr (
              "Error: I0 standard experimental is 0, can not compute ML");
          ML = 0e0;
          return false;
        }
      I0_exp_to_theo_mult = our_saxs_options->I0_theo / I0_std_exp;
  ...
  double use_psv
      = our_saxs_options->use_cs_psv ? our_saxs_options->cs_psv : psv;
  ...
  internal_contrast
      = our_saxs_options->diffusion_len
        * (1e0
               / (guinier_electron_nucleon_ratio
                  * our_saxs_options->nucleon_mass)
           - use_psv * (1e24 * our_saxs_options->water_e_density));

  ML = I0_prot_theo * AVOGADRO / (conc * 1e-3)
       / (internal_contrast * internal_contrast) / M_PI;
...
bool
US_Hydrodyn_Saxs::ma_from_q2I0 (QString name, double I0_exp, double &MA,
                                double &internal_contrast)
{
  double conc;
  double psv;
  ...
  if (our_saxs_options->guinier_use_standards)
    {
      if (I0_std_exp == 0e0)
        {
          errormsg = us_tr (
              "Error: I0 standard experimental is 0, can not compute MA");
          MA = 0e0;
          return false;
        }
      I0_exp_to_theo_mult = our_saxs_options->I0_theo / I0_std_exp;
  ...
  MA = I0_prot_theo * AVOGADRO / (conc * 1e-3)
       / (internal_contrast * internal_contrast) / (2e0 * M_PI);
...
void
US_Hydrodyn_Saxs::guinier_window ()
{
#if defined(DEBUG_MW2)
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->sas_options_guinier_widget)
    {
      if (((US_Hydrodyn *)us_hydrodyn)
              ->sas_options_guinier_window->isVisible ())
        {
...
void
US_Hydrodyn_Saxs::sync_conc_csv () // removes deleted curves, adds non-extant
                                   // curves with default
{
  if (!conc_csv.data.size ())
    {
      // setup & add all
      conc_csv.name
          = "Set Curve Concentration, PSV and I0 standard experimental";
  ...
  for (unsigned int i = 0; i < (unsigned int)plotted_Iq.size (); i++)
    {
...
void
US_Hydrodyn_Saxs::add_conc_csv (QString name, double conc, double psv,
                                double I0_std)
{
#if defined(DEBUG_CSV)
...
void
US_Hydrodyn_Saxs::update_conc_csv (QString name, double conc, double psv,
                                   double I0_std)
{
#if defined(DEBUG_CSV)
...
void
US_Hydrodyn_Saxs::update_conc_csv (QString name, double conc)
{
  update_conc_csv (name, conc, our_saxs_options->psv,
                   our_saxs_options->I0_exp);
}

void
US_Hydrodyn_Saxs::update_conc_csv (QString name)
{
  update_conc_csv (name, our_saxs_options->conc, our_saxs_options->psv,
                   our_saxs_options->I0_exp);
}

void
US_Hydrodyn_Saxs::add_conc_csv (QString name)
{
  add_conc_csv (name, our_saxs_options->conc, our_saxs_options->psv,
                our_saxs_options->I0_exp);
}

bool
US_Hydrodyn_Saxs::get_conc_csv_values (QString name, double &conc, double &psv,
                                       double &I0_std_exp)
{
  for (unsigned int i = 0; i < conc_csv.data.size (); i++)
    {
...
void
US_Hydrodyn_Saxs::pr_info2 (const QString &msg)
{
  pr_info (msg, true);
}

void
US_Hydrodyn_Saxs::pr_info (const QString &msg, bool detail)
{

  TSO << "==== pr_info( " << msg << " ) ==== START ====\n"
      << QString ("plotted_r.size()                          %1\n"
                  "plotted_pr.size()                         %2\n"
      ...
      if (detail)
        {
          US_Vector::printvector5 (
              QString (
                  "entry %1 mw %2 plotted_r,_pr,_pr_error,_pr_nn,pr_nn_error")
                  .arg ((int)qsl_plotted_pr_names.size () > i
                            ? qsl_plotted_pr_names[i]
                            : QString ("?"))
                  .arg ((int)plotted_pr_mw.size () > i ? plotted_pr_mw[i]
                                                       : -1),
              ...
              (int)plotted_pr_not_normalized.size () > i
                  ? plotted_pr_not_normalized[i]
                  : emptyv,
              (int)plotted_pr_not_normalized_error.size () > i
                  ? plotted_pr_not_normalized_error[i]
                  : emptyv);
        }
      else
        {
          TSO << QString ("entry %1 [%8]: r %2 pr %3 pr_error %4 pr_nn %5 "
                          "pr_nn_e %6 pr_mw %7\n")
                     .arg (i)
                     .arg ((int)plotted_r.size () > i
                               ? (int)plotted_r[i].size ()
                               : -1)
                     .arg ((int)plotted_pr.size () > i
                               ? (int)plotted_pr[i].size ()
                               : -1)
                     .arg ((int)plotted_pr_error.size () > i
                               ? (int)plotted_pr_error[i].size ()
                               : -1)
                     .arg ((int)plotted_pr_not_normalized.size () > i
                               ? (int)plotted_pr_not_normalized[i].size ()
                               : -1)
                     .arg (
                         (int)plotted_pr_not_normalized_error.size () > i
                             ? (int)plotted_pr_not_normalized_error[i].size ()
                             : -1)
                     .arg ((int)plotted_pr_mw.size () > i ? plotted_pr_mw[i]
                                                          : -1)
                     .arg ((int)qsl_plotted_pr_names.size () > i
...
US_Hydrodyn_Saxs::pr_to_iq ()
{
  ...
  QString source = "";
  {
    ...
    source = US_Static::getItem (
        ...
        qsl_plotted_pr_names, 0, false, &ok, this);
    if (ok)
      {
  ...
  errormsg = QString (us_tr ("Internal error, could not find source '%1' in "
                             "plotted names\n"))
                 .arg (source);
...
bool
US_Hydrodyn_Saxs::pr_to_iq (int pos, QString name)
{
  errormsg = "";
  TSO << QString ("pr_to_iq( %1 )\n").arg (pos);

  if ((int)plotted_pr.size () <= pos || (int)plotted_r.size () <= pos)
    {
  ...
  int q_points
      = (int)floor (((our_saxs_options->end_q - our_saxs_options->start_q)
  ...
  if (our_saxs_options->iq_exact_q)
    {
      ...
      if (!exact_q.size ())
        {
          editor_msg ("dark red",
                      QString (us_tr ("Notice: exact q is empty, computing "
  ...
  for (int i = 1; i < q_points; ++i)
    {
      ...
      I[i] *= dr * exp (-our_saxs_options->fast_modulation * q[i] * q[i]);
    }
...
bool
US_Hydrodyn_Saxs::pr_to_iq (const map<double, double> &pr_exact, QString name)
{
  errormsg = "";
  ...
  for (int i = 0; i < q_points; ++i)
    {
      double qr;
      for (auto it = pr_exact.begin (); it != pr_exact.end (); ++it)
        {
          qr = q[i] * it->first;
          I[i] += it->second * ((fabs (qr) < 1e-16) ? 1.0 : sin (qr) / qr);
        }
      I[i] *= exp (-our_saxs_options->fast_modulation * q[i]);
    }
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->batch_widget
      && ((US_Hydrodyn *)us_hydrodyn)->batch_window->batch_job_running)
    {
