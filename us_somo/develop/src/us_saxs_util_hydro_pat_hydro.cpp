...
int
us_hydrodyn_pat_main_hydro (int use_nmax, int in_nat, struct dati1_supc *in_dt,
      ...
      if (i < 3)
        {
          ...
          // in_dt[i].x,in_dt[i].y,in_dt[i].z,in_dt[i].r, in_dt[i].m,
          // in_dt[i].col); printf("reading in coords dt bead %d @ %f %f %f %f
  ...
  if (raggio == 0.0)
    {
      fscanf (mol, "%s", ragcol);
      rmc = us_fopen (ragcol,
                      "r"); /* opening the file containing the unhydrated
