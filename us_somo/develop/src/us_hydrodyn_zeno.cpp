...
void parse(int *maxelts, int *nelts, int eltype[], float *bv, float *tol,
           ...
           int *maxwords, int *nwords, int P1, int P2, int P3, int P4, int P5);
...
void report(char *id, char *actions, int m1[], int *nelts, float *tol,
            ...
            unsigned int *bc, unsigned int *bbf, double temp[], char *tunit,
            double mass[], char *munit, double visc[], char *vunit,
            double buoy[], float *hscale, double *tae, double uae[],
            double *vae, double *wae, float *q2pade, double eigens[],
            double xx[], float q[], float sq[], float dsq[], float xyzlow[],
            float xyzhih[], int P1, int P2, int P3, int P6, int P4, int P5);
...
void poplar(int *mz, int *ms, int *mi, int *mp, unsigned int *bl,
            ...
            double eta[], double buoy[], char *lunit, char *tunit, char *munit,
            char *vunit, char *qunit, double *tae, double uae[], double *vae,
            double *wae, float *q2pade, double eigens[], double xx[], float q[],
            double *stfac, double lhalf[], double box1[], double box2[], int P1,
...
void parameters(unsigned int *bt, double temp[], unsigned int *bc, double eta[],
                char *tunit, char *vunit, double celsius[], int P1, int P2);
...
void diffuse(double t1[], double d[], char *lunit, char *dunit, int P1, int P2);
...
void modifiers(unsigned int *nobuffer, char *buffer, char *String,
               ...
               char *dictionary, int map[], int *maxwords, int *nwords, int P2,
               int P3, int P1, int P4);
...
void maxcylinder(float c[], float n[], float *r, float *l, float *t, float *dd);

void prosolcyl(float c[], float n[], float *r, float *l, float *t, float v[],
               float *s1, float *s2);

void procylinder(float c[], float n[], float *r, float *l, float *t, float v[],
                 float *s1, float *s2);

void minsolcyl(float c[], float n[], float *r, float *l, float *t, float p[],
               float *d);

void insidesolcyl(float c[], float n[], float *r, float *l, float *t, float p[],
                  unsigned int *inside);

void mincylinder(float c[], float n[], float *r, float *l, float *t, float p[],
                 float *d);

void maxtorus(float c[], float n[], float *r1, float *r2, float *t, float *dd);

void protorus(float c[], float n[], float *r1, float *r2, float *tr, float v[],
              float *s1, float *s2);
...
void bridge(int *maxelts, int eltype[], float *bv, int *nelts, float *rotations,
            float v[], float *d);
...
void accume(float t[], int kk[], int *khitp, int *khite, double *vp, double *ve,
            int *loop);
...
void wagaroo(int *maxelts, int eltype[], float *bv, int *nelts, float *rlaunch,
             float *rotations, int *m1, float *rg2int, float *delta_rg2int,
             float *volume, float *delta_volume, unsigned int *rg_done, int *mi,
             char *id, float q[], float sq[], float dsq[], char *round,
             unsigned int *savehits, float vaar[], double *tten, int P1,
             int P2);
...
void toss_cylinder(float c[], float n[], float *rad, float *alen, float *tumble,
                   float rt[]);
...
void getsurface(int *maxelts, int eltype[], float *bv, int *nelts, float saar[],
...
int main(int argc, const char **argv) {
  ...
  */
  ...
  static float q[82], sq[82], dsq[82], rg2surf, delta_rg2surf, kirk, delta_kirk,
      span, delta_span;
  ...
  */
  ...
  T2 = MAXWORDS;
  puts("z3");
  parse(&T1, &nelts, eltype, (float *)bv, &tol, (float *)rotations, &tol_given,
        unitcode, &bt, &bm, &bw, &bc, &bbf, temp, tunit, mass, munit, visc,
        vunit, buoy, &hscale, &rlaunch, &launch_done, dictionary, map, &T2,
        &nwords, 2, 6, 6, 6, 10);
  puts("z4");
  /*
    Begin output file:
  ...
  */
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (*(actions + i) == 'i' || *(actions + i) == 'I') {
      ...
      */
      T1 = MAXELTS;
      all_around(&T1, eltype, (float *)bv, &nelts, xyzlow, xyzhih, &rlaunch,
                 (float *)rotations, &m1do, &rg2int, &delta_rg2int, &volume,
                 &delta_volume, &rg_done, &mi, id, q, sq, dsq, round,
                 &savehits_i, (double *)tten, USZ_MAX_ID_SIZE, 80);
    } else if (*(actions + i) == 'c' || *(actions + i) == 'C') {
      /*
        Do the covered-interior integrations:
      */
      T1 = MAXELTS;
      wagaroo(&T1, eltype, (float *)bv, &nelts, &rlaunch, (float *)rotations,
              &m1do, &rg2int, &delta_rg2int, &volume, &delta_volume, &rg_done,
              &mi, id, q, sq, dsq, round, &savehits_i, vaar, (double *)tten,
              USZ_MAX_ID_SIZE, 80);
      ...
      */
      ...
      blizzard(&T1, eltype, (float *)bv, &nelts, &m1do, &tol, &rlaunch,
               ...
               &zeno_done, &delta_cap, (float *)delta_bongo, &mz, id, &tae, uae,
               (double *)vae, (double *)wae, round, bubble, &bubble_rad, nebtab,
               &nneb, &ninn, rlist, strikes, &savehits_z, USZ_MAX_ID_SIZE, 80);
      ...
      */
      ...
      captain(&T1, eltype, (float *)bv, &nelts, &m1do, (float *)rotations,
              &kirk_done, saar, &kirk, &delta_kirk, &surface, &delta_surface,
              &rg2surf, &delta_rg2surf, &ms, id, round, &savehits_s,
              USZ_MAX_ID_SIZE, 80);
      ...
      */
      T1 = MAXELTS;
      calipers(&T1, eltype, (float *)bv, &nelts, &m1do, (float *)rotations,
  ...
  report(id, actions, m1, &nelts, &tol, &rlaunch, &cap, &delta_cap,
         ...
         &shadow, &delta_shadow, &shadow_done, &zeno_done, &kirk_done, &rg_done,
         &span_done, &launch_done, &tol_given, unitcode, &mz, &mi, &ms, &mp,
         &bt, &bm, &bw, &bc, &bbf, temp, tunit, mass, munit, visc, vunit, buoy,
         &hscale, &tae, uae, (double *)vae, (double *)wae, &q2pade, eigens, xx,
         q, sq, dsq, xyzlow, xyzhih, USZ_MAX_ID_SIZE, 4, 2, 6, 6, 6);
...
void report(char * /* id */, char *actions, int m1[], int * /* nelts */,
            ...
            double *vae, double *wae, float *q2pade, double eigens[],
            double xx[], float q[], float sq[], float dsq[], float xyzlow[],
            float xyzhih[], int /* P1 */, int /* P2 */, int /* P3 */,
            int /* P6 */, int /* P4 */, int /* P5 */) {
  /*
  ...
  */
  for (i = 0; i < 4; i++) {
    ...
    if (do_something)
      WRITE(Tfilenumbers->nzno, FMT, F206, 1, STRG, b1, 1, STRG, b2, 1, INT4,
            nzip, 0);
  }
  ...
  */
  poplar(mz, ms, mi, mp, &bl, &bk, &bs, &bi, bt, bm, bw, bc, bbf, &bz, &bp,
         ...
         tten, rk, surf, rg2surfd, v, rg2intd, rp, rap, temp, mass, visc, buoy,
         lunit, tunit, munit, vunit, qunit, tae, uae, vae, wae, q2pade, eigens,
         xx, q, (double *)stfac, lhalf, box1, box2, 6, 6, 6, 6, 6);
  return;
}
void lighthalf(double qa[], double qb[], double sqa[], double sqb[],
               double lhalf[]) {
...
void poplar(int * /* mz */, int * /* ms */, int * /* mi */, int * /* mp */,
            unsigned int *bl, unsigned int *bk, unsigned int *bs,
            unsigned int *bi, unsigned int *bt, unsigned int *bm,
            unsigned int *bw, unsigned int *bc, unsigned int *bbf,
            unsigned int *bz, unsigned int *bp, unsigned int *bpp, double *rl,
            double *eps, double cap[], double a11[], double a12[], double a13[],
            double a21[], double a22[], double a23[], double a31[],
            double a32[], double a33[], double *tten, double rk[],
            double surf[], double rg2surf[], double v[], double rg2int[],
            double rp[], double rap[], double temp[], double mass[],
            ...
            char *vunit, char *qunit, double *tae, double uae[], double *vae,
            double *wae, float *q2pade, double eigens[], double xx[], float q[],
            double *stfac, double lhalf[], double box1[], double box2[],
...
void writehandy(double *tae, double uae[], double *vae, double *wae,
  ...
  for (i = 1; i <= 3; i++) {
    WRITE(Tfilenumbers->nzno, FMT, F901, 1, INT4, i, REAL8, uae[i - 1], STRG,
          nounit, 6, 0);
    WRITE(Tfilenumbers->nznr, FMT, F901, 1, INT4, i, REAL8, uae[i - 1], STRG,
          nounit, 6, 0);
  }
  for (i = 1; i <= 3; i++) {
    for (j = 1; j <= 3; j++) {
...
void bundle(char *nod1, char *nod2, char *name, int /* P1 */, int /* P2 */,
            int /* P3 */) {
  static int lk, kk, i;
  /*
   ********************************************************
...
void friction(double t1[], double fric[], char *lunit, char *funit,
              int /* P1 */, int /* P2 */) {
...
void longjohn(double tx[], char *name, char *u, char *mane, char *line,
  ...
  if (nex == 0) {
    WRITE(INTERNAL, longname, 49, FMT, F898, 1, STRG, String, 20, STRG, blanks,
          6, 0);
  } else if (nex >= 10) {
    WRITE(INTERNAL, longname, 49, FMT, F899, 1, STRG, String, 20, STRG, byte, 1,
          INT4, nex, STRG, blanks, 6, 0);
  } else {
    WRITE(INTERNAL, longname, 49, FMT, F897, 1, STRG, String, 20, STRG, byte, 1,
          INT4, nex, STRG, blanks, 6, 0);
  }
  /*
    20 + 1 +1+ 20+  1 + 6 = 49
...
void writeme(char *qunit, char *nounit, float *q, double *stfac1,
  ...
  WRITE(Tfilenumbers->nznr, FMT, F700, 1, STRG, mane, 30, DO, 2, REAL8, t, STRG,
        nounit, 6, 0);
  return;
}
...
void show_errors() {
  ...
  */
   ...
   ********************************************************
   */
  static const char *F611[] = {"('  Point found inside cube.  Only a cause')"};
  static const char *F612[] = {"('  for concern if the following number is')"};
  static const char *F613[] = {"('  not close to 1:',g20.8)"};
  static const char *F621[] = {"('  Overstretched ellipsoid.  Only a cause')"};
  static const char *F622[] = {"('  for concern if the following number is')"};
...
void do_launch(int *maxelts, int *nelts, int eltype[], float *bv,
  ...
  */
  ...
  static Cmorse *Tmorse = (Cmorse *)Xmorse;
  static float c[3], v1[3], v2[3], v3[3], n[3], n1[3], n2[3], x[3], y[3], zl[3],
      zh[3], t[3][3], xaxis[3], yaxis[3], zaxis[3];
  static int i, j;
...
S1:
  if (*nobuffer) {
    if (READ(Tfilenumbers->nbod, IOSTAT, NULL, 1, FMT, F980, 1, STRG, buffer,
             80, 0))
      goto S20;
    cleartabs(buffer, 80);
    /*
...
void webster(char *dictionary, int map[], int *nwords, int * /* maxwords */,
             int /* P1 */) {
  typedef struct {
    int sphere_code;
    int triangle_code;
  ...
  while (!atend) { /*  GRANDDADDY loop goes through all */
    ...
    */
    ...
    if (ntype == Tmorse->sphere_code || ntype == Tmorse->cube_code) {
      ...
      modifiers(&nobuffer, buffer, String, &atend, down, (double *)download,
                dictionary, map, maxwords, nwords, 80, 80, 13, 10);
      *nelts = *nelts + 1;
  ...
  for (i = 0; i < 13; i++) {
    if (*(down + i) == '@') {
      ...
      *(download + i + 12) = 0.0e0;
      ...
      if (READ(INTERNAL, String, 80, IOSTAT, NULL, 2, LISTIO, DO, 1, REAL8,
               (download + i), 0))
        goto S999;
      *(download + i + 12) = 0.0e0;
    } else if (*(down + i) == 'd') {
...
void mintriangle(float v1[], float v2[], float v3[], float p[], float *d) {
  ...
  */
  static float b2[3], b1[3], b3[3], p1[3], t3b3[3], q[3], dp1b3, db3b3, t3, d11,
      d12, d22, q1, q2, t1, t2, d1, d2, d3, dx;
  /*
   *************************************************
   */
...
void prodisk(float c[], float[] /* n[] */, float *r, float *tr, float v[],
             float *s1, float *s2) {
...
void mindisk(float c[], float n[], float *r, float * /* tr */, float p[],
             float *d) {
  /*
    Given a disk centered at c, with unit normal n and radius r.
    Also given an arbitrary point p.
...
void maxsolcyl(float c[], float /* n */[], float *r, float *l, float *t,
               float *dd) {
  static float T1;
  /*
...
void minsolcyl(float c[], float /* n */[], float *r, float *l, float *t,
               float p[], float *d)
/*
  Given a solid cylinder centered at c, with unit normal n, radius r,
  length l.
...
void mintorus(float c[], float /* n */[], float *r1, float *r2, float *t,
              float p[], float *d) {
  /*
    TORUS[(cx,cy,cz),(nx,ny,nz),r1,r2]
    Given a torus centered at c, with unit normal n, radii r1 and r2.
...
void proellipsoid(float c[], float /* n1 */[], float /* n2 */[], float *aa,
                  float *bb, float *cc, float *t, float v[], float *s1,
                  float *s2) {
...
void insideellipsoid(float c[], float /* n1 */[], float /* n2 */[], float *aa,
...
void exellipsoid(float c[], float /* n1 */[], float /* n2 */[], float *aa,
                 float *bb, float *cc, float *t, float po[], int *minmax,
                 float *d) {
  static float T1, T2;
...
void blizzard(int *maxelts, int eltype[], float *bv, int *nelts, int *m1,
              ...
              unsigned int *zeno_done, float *delta_cap, float *delta_bongo,
              int *mz, char * /* id */, double *tae, double uae[], double *vae,
              double *wae, char * /* round */, float bubble[],
              float *bubble_rad, int nebtab[], int *nneb, int *ninn,
              float rlist[], double strikes[], unsigned int *savehits,
              int /* P2 */, int /* P1 */) {
  /*
  ...
  *zeno_done = false;
  ...
  for (jax = 1; jax <= mtdo; jax++) {
    ...
    sphere(rt, &r);
    charge(rt, &r, kk);
    park(maxelts, eltype, bv, rotations, nelts, rt, &r, &r2, &hit, tol, bubble,
         bubble_rad, nebtab, nneb, ninn, rlist, &hitelt);
    if (hit) {
      accume(rt, kk, (int *)khitp, (int *)khite, (double *)vp, (double *)ve,
             &loop);
      strikes[hitelt - 1] = strikes[hitelt - 1] + 1.0e0;
      if (*savehits) {
        plus_or_minus(kk, pom, 3);
        WRITE(Tfilenumbers->nzh, FMT, F4444, 1, STRG, pom, 3, DO, 3, REAL4, rt,
              0);
      }
    }
    //          need = fifnint(79.0F*(float)jax/(float)mtdo);
  ...
  *tae = taer[0];
  for (i = 0; i < 3; i++) {
    for (j = 0; j < 3; j++) {
      *(wae + i + j * 3) =
          *(wae + i + j * 3) * (12.0e0 * 3.14159265e0) * pow((double)r, 2.0);
      *(vae + i + j * 3) =
          *(vae + i + j * 3) * (12.0e0 * 3.14159265e0) * pow((double)r, 2.0);
    }
  }
  /*
  ...
  */
  ...
  for (J1 = 0; J1 < 6; J1++) {
    for (j2 = 0; j2 < 2; j2++) {
      ...
      for (j3 = 0; j3 < 4; j3++) {
        READ(DO, 1, INT4, &var[j3][j2][J1], DO, 1, INT4, &varmag[j3][j2][J1],
             MORE);
      }
      READ(0);
    }
      ...
      *(wae + i + j * 3) = 0.0e0;
      for (k = 0; k < 20; k++) {
        *(vae + i + j * 3) = *(vp + i + (j + k * 3) * 3) +
                             *(ve + i + (j + k * 3) * 3) + *(vae + i + j * 3);
        *(wae + i + j * 3) = *(vp + i + (j + k * 3) * 3) -
                             *(ve + i + (j + k * 3) * 3) + *(wae + i + j * 3);
      }
...
void seeder() {
  ...
  */
  ...
  QString time = QString("").sprintf(
      "%d%d%d%d", QTime::currentTime().hour(), QTime::currentTime().minute(),
...
void toss_point(float rt[], int *maxelts, int eltype[], float *bv, int *nelts,
  ...
  static Cmorse *Tmorse = (Cmorse *)Xmorse;
  static float c[3], x[3], y[3], zl[3], zh[3], n[3], n1[3], n2[3], tumble[3][3],
      zip, sum;
  static int i, kdo;
  static float side, r;
  static int j;
...
void toss_pillar(float /* x */[], float /* y */[], float /* zl */[],
                 float /* zh */[], float /* rt */[]) {
  /*
   *************************************************
   */
...
void toss_ellipsoid(float /* c */[], float /* n1 */[], float /* n2 */[],
...
void toss_cylinder(float /* c */[], float /* n */[], float * /* rad */,
...
void toss_torus(float /* c */[], float /* n */[], float * /* r1 */,
...
void pillarvol(float /* x */[], float /* y */[], float /* zl */[],
...
void captain(int *maxelts, int eltype[], float *bv, int *nelts, int *m1do,
             ...
             float *kirk, float *delta_kirk, float *surface,
             float *delta_surface, float *rg2surf, float *delta_rg2surf,
             int *ms, char * /* id */, char * /* round */,
             unsigned int *savehits, int /* P1 */, int /* P2 */) {
  typedef struct {
    int sphere_code;
    int triangle_code;
    int disk_code;
  ...
  *kirk_done = false;
  ...
  for (i = 1; i <= *m1do; i++) {
    loop = fifmod(i, 20) + 1;
    getsurface(maxelts, eltype, bv, nelts, saar, &total, v1, trials, rotations,
               &loop);
    getsurface(maxelts, eltype, bv, nelts, saar, &total, v2, trials, rotations,
               &loop);
    if (*savehits) {
...
void doverellipsoid(float c[], float /* n1 */[], float /* n2 */[], float *aa,
...
bool US_Hydrodyn_Zeno::run(QString filename, vector<PDB_atom> *bead_model,
                           double &sum_mass, double &sum_volume,
                           const double &Rg, mQProgressBar *use_progress,
                           bool keep_files, bool zeno_cxx, int threads) {
  ...
  if (!QDir::setCurrent(QFileInfo(filename).filePath())) {
    ...
    // QFileInfo( filename ).path() ); return false;
  }
  ...
  if (options->unit != -9 && options->unit != -10) {
    error_msg = QString("Error: units must be A(10) or nm(9) and value is %1")
                    .arg(-options->unit);
    return false;
  }
  ...
  for (unsigned int i = 0; i < bead_model->size(); i++) {
    ...
    sum_volume +=
        (4e0 / 3e0) * M_PI * pow((*bead_model)[i].bead_computed_radius, 3);
  }
  ...
  if (options->zeno_surface_thickness_from_rg) {
    ...
    // sigmoid
    double st = options->zeno_surface_thickness_from_rg_a /
                (1 + exp(-(Rg - options->zeno_surface_thickness_from_rg_b) /
  ...
  us_hydrodyn->editor_msg("black",
                          QString("Beads used %1\n").arg(bead_model->size()));
  ...
  if (zeno_cxx) {
    ...
    if (us_hydrodyn->gparams["zeno_max_cap"] == "true") {
      argv[argc++] = strdup(QString("--max-rsd-capacitance=%1")
    ...
    // );
    argv[argc++] =
        strdup(QString("--num-threads=%1").arg(threads).toLatin1().data());

    argv[argc++] = strdup(QString("--seed=%1")
    ...
    zeno_cxx_main(
        ...
        QString("%1.zno%2")
            .arg(filename)
            .arg(us_hydrodyn->batch_avg_hydro_active() || us_hydrodyn->zeno_mm
                     ? "_Tmp_Remove"
                     : "")
            .toLatin1()
            .data(),
...
bool US_Hydrodyn::calc_zeno() {
  ...
  if (hydro.zeno_zeno) {
    editor_msg(
        "dark blue",
        QString(
            us_tr("Zeno integration selected with %1 thousand MC iterations"))
            .arg(hydro.zeno_zeno_steps));
  }

  if (hydro.zeno_interior) {
    editor_msg("dark blue", QString(us_tr("Zeno interior integration selected "
                                          "with %1 thousand MC iterations"))
                                .arg(hydro.zeno_interior_steps));
  }

  if (hydro.zeno_surface) {
    editor_msg("dark blue", QString(us_tr("Zeno surface integration selected "
                                          "with %1 thousand MC iterations"))
  ...
  if (zeno_cxx) {
    editor_msg(
        "darkRed",
        "Notice: ZENO method is not currently not available for this platform");
    qApp->processEvents();
    zeno_cxx = false;
  }
#if QT_VERSION >= 0x040000
  ...
  bool has_overlap =
      overlap_check(true, true, true,
                    hydro.overlap_cutoff ? hydro.overlap : overlap_tolerance, 1,
                    true) > 0;
      ...
      if (somo_processed[current_model]) {
        ...
        for (int this_repeat = 0; this_repeat < repeats; ++this_repeat) {
          ...
          if (hydro.zeno_surface_thickness_from_rg) {
            ...
            double st = hydro.zeno_surface_thickness_from_rg_a /
                        (1 + exp(-(model_vector[current_model].asa_rg_pos -
                                   hydro.zeno_surface_thickness_from_rg_b) /
                                 hydro.zeno_surface_thickness_from_rg_c));
            editor_msg(
                "darkblue",
                QString(us_tr("Computed skin thickness for model %1 from Rg %2 "
                              "[%3] is %4 [%5]\n"))
                    .arg(current_model + 1)
            ...
            if (st <= 0) {
              editor_msg(
                  "red",
                  us_tr("NOTICE: zero or negative computed skin thickness will "
                        "be ignored, reverting to ZENO default\n"));
            } else {
          ...
          bool result =
              uhz.run(fname, &bead_models[current_model], sum_mass, sum_volume,
          ...
          if (!result) {
            ...
            save_data this_data =
                US_Hydrodyn_Save::save_data_initialized_from_bead_model(
            ...
            if (bead_models[current_model].size() &&
              ...
              this_data.hydrate_probe_radius =
                  bead_models[current_model][0].asa_hydrate_probe_radius;
              this_data.hydrate_threshold =
                  bead_models[current_model][0].asa_hydrate_threshold;
              this_data.vdw_theo_waters =
                  bead_models[current_model][0].vdw_theo_waters;
              this_data.vdw_exposed_residues =
                  bead_models[current_model][0].vdw_count_exposed;
              this_data.vdw_exposed_waters =
                  bead_models[current_model][0].vdw_theo_waters_exposed;
            }

            this_data.fractal_dimension_parameters =
                model_vector[current_model].fractal_dimension_parameters;
            ...
            this_data.fractal_dimension_wtd_sd =
                model_vector[current_model].fractal_dimension_wtd_sd;
            this_data.fractal_dimension_wtd_wtd =
                model_vector[current_model].fractal_dimension_wtd_wtd;
            this_data.fractal_dimension_wtd_wtd_sd =
                model_vector[current_model].fractal_dimension_wtd_wtd_sd;
            this_data.rg_over_fractal_dimension =
                model_vector[current_model].rg_over_fractal_dimension;
            this_data.rg_over_fractal_dimension_sd =
                model_vector[current_model].rg_over_fractal_dimension_sd;
            this_data.rg_over_fractal_dimension_wtd =
                model_vector[current_model].rg_over_fractal_dimension_wtd;
            this_data.rg_over_fractal_dimension_wtd_sd =
                model_vector[current_model].rg_over_fractal_dimension_wtd_sd;
            this_data.rg_over_fractal_dimension_wtd_wtd =
                model_vector[current_model].rg_over_fractal_dimension_wtd_wtd;
            ...
            if (!f.exists() || !f.open(QIODevice::ReadOnly)) {
              editor_msg("red",
                         "ZENO computation failed to produce output file");
              pb_calc_hydro->setEnabled(was_hydro_enabled);
              pb_calc_zeno->setEnabled(true);
              pb_bead_saxs->setEnabled(true);
              pb_calc_grpy->setEnabled(true);
            ...
            //             QString( "ZENO computation output file %1 opened %2
            //             lines" ) .arg( last_hydro_res ) .arg( qsl.size() ) );
            ...
            if (zeno_cxx) {
              ...
              // "^Capacitance:\\s+(\\S+)\\s*$" ) ); param_name   .push_back(
              // "results.rs" ); param_cap_pos.push_back( 1 );

              // param_rx     .push_back( QRegExp( "^Intrinsic
              // viscosity:\\s+(\\S+)\\s*$" ) ); param_name   .push_back(
              // "results.viscosity" ); param_cap_pos.push_back( 1 );
              ...
              param_rx.push_back(
                  QRegExp("^Intrinsic viscosity with mass units "
                          ".cm.3/g.:\\s+(\\S+)\\s+./-\\s+(\\S+)$"));
              ...
              param_rx.push_back(
                  QRegExp("^Prefactor for computing intrinsic "
                          "viscosity:\\s+(\\S+)\\s+./-\\s+(\\S+)$"));
              ...
              param_rx.push_back(
                  QRegExp("^Mean electric polarizability "
                          ".(?:\\S+).:\\s+(\\S+)\\s+./-\\s+(\\S+)$"));
              ...
              param_rx.push_back(
                  QRegExp("^Rg .interior.  . . . . . . . .\\s+(\\S+)\\s+"));
              param_name.push_back("results.rg");
              param_cap_pos.push_back(1);

              param_rx.push_back(
                  QRegExp("^surface area . . . . . . . . .\\s+(\\S+)\\s+"));
              param_name.push_back("tot_surf_area");
              param_cap_pos.push_back(1);

              param_rx.push_back(
                  QRegExp("^volume . . . . . . . . . . . .\\s+(\\S+)\\s+"));
              param_name.push_back("used_beads_vol");
              param_cap_pos.push_back(1);
            ...
            for (unsigned int i = 0; i < (unsigned int)param_rx.size(); i++) {
              ...
              switch (param_qsl[i].size()) {
                case 1: {
                  ...
                  // us_qdebug( QString( "returned searching %1 returned %2"
                  // ).arg( param_qsl[ i ][ 0 ] ).arg( qs ) );
                  ...
                  // editor_msg( "dark blue", QString( "ZENO produced parameter:
                ...
                case 0: {
                  // editor_msg( "red", QString( "ZENO did not produce expected
                  // parameter: %1\n" ).arg( param_name[ i ] ) );
                } break;
                default: {
                  editor_msg(
                      "red",
                      QString(
                          "ZENO produced multiple values for parameter: %1\n")
                          .arg(param_name[i]));
                } break;
              }
            }
            ...
            if (zeno_cxx) {
              ...
              if (use_mass) {
                ...
                // AVOGADRO * 1e-24 * sum_volume; this_data.results.viscosity =
                // tmp / sum_mass;
                this_data.results.viscosity *=
                    this_data.use_beads_vol / (use_mass * 1.660538921);
              } else {
                this_data.results.viscosity = 0e0;
              }

              this_data.tra_fric_coef =
                  6e0 * M_PI * use_solvent_visc() * this_data.results.rs * 1e-1;

              if (this_data.tra_fric_coef) {
                // 1.3864852e-8 is  boltzman's constant with a conversion,
                // probably needs fconv
...

            {
              ...
              //        this_data.results.rs ) .arg( this_data.results.viscosity
              //        )
              //        );
              ...
              if (this_data.results.viscosity && this_data.results.mass &&
                ...
                this_data.results.ff0 =
                    ...
                    (fconv * 6e0 * M_PI * use_solvent_visc() *
                     pow(3.0 * this_data.results.mass * this_data.results.vbar /
                             (4.0 * M_PI * AVOGADRO),
                         1.0 / 3.0));

                this_data.results.ff0_sd = this_data.results.ff0 *
                                           this_data.tra_fric_coef_sd /
                                           this_data.tra_fric_coef;

                us_qdebug(
                    QString("computed ff0 %1").arg(this_data.results.ff0));
              }
              ...
              if (this_data.tra_fric_coef) {
                // us_qdebug(
                //        QString( "s20w mass is %1 partvol %2 DENS %3 f %4 d20w
                //        %5" ) .arg( this_data.results.mass ) .arg(
                //        this_data.results.vbar ) .arg( use_solvent_dens() )
                ...
                this_data.results.s20w =
                    ...
                    (1e0 - (this_data.results.vbar * use_solvent_dens())) /
                    (6e0 * M_PI * use_solvent_visc() * this_data.results.rs *
                     AVOGADRO);

                this_data.results.s20w_sd = this_data.results.s20w *
                ...
                // this_data.results.s20w ).arg( alt_s20w ) );
              }
              ...
              if (this_data.results.mass) {
                ...
                for (unsigned int i = 0; i < bead_model.size(); ++i) {
                  ...
                  double bead_tot_mw =
                      this_atom->bead_mw + this_atom->bead_ionized_mw_delta;
                  cm.axis[0] +=
                      bead_tot_mw * this_atom->bead_coordinate.axis[0];
                  cm.axis[1] +=
                      bead_tot_mw * this_atom->bead_coordinate.axis[1];
                  cm.axis[2] +=
                      bead_tot_mw * this_atom->bead_coordinate.axis[2];
                ...
                us_qdebug(QString("rg %1 fconv %2 rg2 %3 total_cm_mw %4")
                              .arg(Rg)
                              .arg(fconv)
                              .arg(Rg2)
                              .arg(total_cm_mw));
...

            {
              ...
              if (!has_overlap && sum_volume) {
                this_data.dimless_eta = this_data.zeno_eta_prefactor *
                                        this_data.zeno_mep / sum_volume;
                if (this_data.zeno_eta_prefactor_sd && this_data.zeno_mep_sd) {
                  this_data.dimless_eta_sd =
                      (this_data.zeno_eta_prefactor * this_data.zeno_mep /
                       sum_volume) *
                      sqrt(this_data.zeno_eta_prefactor_sd *
                               this_data.zeno_eta_prefactor_sd /
                               (this_data.zeno_eta_prefactor *
                                this_data.zeno_eta_prefactor) +
                           this_data.zeno_mep_sd * this_data.zeno_mep_sd /
                               (this_data.zeno_mep * this_data.zeno_mep));
                }
              }
            }
...

            {
              ...
              if ("empty model" != saxs_util->nsa_physical_stats()) {
                this_data.tot_volume_of =
                    saxs_util->nsa_physical_stats_map["result excluded volume"]
                        .toDouble();
                ...
                this_data.max_ext_x =
                    saxs_util
                        ->nsa_physical_stats_map
                            ["result radial extent bounding box size x"]
                        .toDouble() *
                    fconv;
                this_data.max_ext_y =
                    saxs_util
                        ->nsa_physical_stats_map
                            ["result radial extent bounding box size y"]
                        .toDouble() *
                    fconv;
                this_data.max_ext_z =
                    saxs_util
                        ->nsa_physical_stats_map
                            ["result radial extent bounding box size z"]
                        .toDouble() *
                    fconv;
                ...
                this_data.axi_ratios_xz =
                    saxs_util
                        ->nsa_physical_stats_map
                            ["result radial extent axial ratios x:z"]
                        .toDouble();
                this_data.axi_ratios_xy =
                    saxs_util
                        ->nsa_physical_stats_map
                            ["result radial extent axial ratios x:y"]
                        .toDouble();
                this_data.axi_ratios_yz =
                    saxs_util
                        ->nsa_physical_stats_map
                            ["result radial extent axial ratios y:z"]
                        .toDouble();
              } else {
                editor_msg("red",
                           QString("Internal error: Bead model is empty?"));
              }
            }
            ...
            // append to zno file
            {
              ...
              if (zeno_cxx) {
                add_to_zeno =
                    ...
                    "\n" +
                    us_tr(
                        "N.B. The above parameters, directly calculated by "
                        "ZENO,\n"
                        "are not linked to the model's physical dimensions,\n"
                        "in contrast to the US-SOMO Derived parameters "
                        "below.\n") +
                    "---------------------------------------------------------"
                    "\n"
#else
                    ...
                    us_tr(
                        "Summary information\n"
                        "The Sedimentation coefficient and Frictional Ratio "
                        "are\n"
              ...
              add_to_zeno +=
                  QString("\nZENO computed on %1 Model %2%3\n")
                      ...
                      .arg(bead_model_suffix.length()
                               ? (" Bead model suffix: " + bead_model_suffix)
                               : "");
              add_to_zeno +=
                  QString("Number of beads used: %1\n").arg(bead_model.size());
              ...
              if (hydro.zeno_surface_thickness_from_rg) {
                add_to_zeno += QString("Computed skin thickness: %1\n")
                                   .arg(hydro.zeno_surface_thickness_from_rg_a +
                                        hydro.zeno_surface_thickness_from_rg_b *
                                            model_vector[current_model].Rg);
              }
                    ...
                    QString(
                        ...
                        .arg(this_data.results.rs_sd
                                 ? QString("").sprintf(" [%4.2e]",
                                                       this_data.results.rs_sd)
                                 : "")
                        .arg(QString("").sprintf("%4.2e cm^3/g",
                                                 this_data.results.viscosity))
                        .arg(this_data.results.viscosity_sd
                                 ...
                                 : "")
                        .arg(QString("").sprintf("%4.2e g/s",
                                                 this_data.tra_fric_coef))
                        .arg(this_data.tra_fric_coef_sd
                                 ? QString("").sprintf(
                                       " [%4.2e]", this_data.tra_fric_coef_sd)
                                 : "")
                        .arg(QString("").sprintf("%4.2e cm^2/sec",
                                                 this_data.results.D20w))
                        .arg(this_data.results.D20w_sd
                                 ? QString("").sprintf(
                                       " [%4.2e]", this_data.results.D20w_sd)
                                 : "");
                  ...
                  if (!has_overlap) {
                    add_to_zeno +=
                        QString(
                            ...
                            .arg(QString("").sprintf("%4.2e",
                                                     this_data.dimless_eta))
                            .arg(this_data.dimless_eta_sd
                                     ? QString("").sprintf(
                                           " [%4.2e]", this_data.dimless_eta_sd)
                                     : "");
                  }
                }
              }
            ...
            zeno_results2.used_beads +=
                this_data.results.used_beads * this_data.results.used_beads;
            zeno_results2.total_beads +=
                this_data.results.total_beads * this_data.results.total_beads;
...

  {
    ...
    if (num <= 1) {
      ...
      if (num <= 1) {
        ...
        zeno_results.s20w_sd = sqrt(fabs(
            (zeno_results2.s20w - zeno_results.s20w * zeno_results.s20w * num) *
            numdecinv));
        zeno_results.D20w_sd = sqrt(fabs(
            (zeno_results2.D20w - zeno_results.D20w * zeno_results.D20w * num) *
            numdecinv));
        zeno_results.viscosity_sd =
            sqrt(fabs((zeno_results2.viscosity -
                       zeno_results.viscosity * zeno_results.viscosity * num) *
                      numdecinv));
        zeno_results.rs_sd = sqrt(
            fabs((zeno_results2.rs - zeno_results.rs * zeno_results.rs * num) *
                 numdecinv));
        zeno_results.rg_sd = sqrt(
            fabs((zeno_results2.rg - zeno_results.rg * zeno_results.rg * num) *
                 numdecinv));
        zeno_results.ff0_sd = sqrt(fabs(
            (zeno_results2.ff0 - zeno_results.ff0 * zeno_results.ff0 * num) *
            numdecinv));
        zeno_results.used_beads_sd = sqrt(fabs(
            (zeno_results2.used_beads -
             (double)zeno_results.used_beads * zeno_results.used_beads * num) *
            numdecinv));
        zeno_results.total_beads_sd = sqrt(fabs(
            (zeno_results2.total_beads - (double)zeno_results.total_beads *
  ...
  if (zeno_mm) {
...

    {
      ...
      if (!f.open(QIODevice::WriteOnly)) {
        editor_msg(
            "red",
            QString(us_tr("Error: could not open output file %1 for writing"))
                .arg(zeno_out_name));
      } else {
        QTextStream t(&f);
        t << zeno_mm_results;
    ...
    bool create_hydro_res = !(batch_widget && batch_window->save_batch_active);
    ...
    if (saveParams && create_hydro_res) {
      ...
      if (!f.open(QIODevice::WriteOnly)) {
        ...
        for (int i = 0; i < (int)zeno_mm_save_params.data_vector.size(); ++i) {
          t << save_util->dataString(&zeno_mm_save_params.data_vector[i])
                   .toLatin1()
                   .data();
        }
...
void US_Hydrodyn::show_zeno() { cout << "show zeno\n"; }
