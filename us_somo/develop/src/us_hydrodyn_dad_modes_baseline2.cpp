...
void US_Hydrodyn_Dad::baseline2_start() {
  ...
  if (names.size() != 1) {
    QMessageBox::critical(this, windowTitle() + us_tr(": Baseline"),
                          QString(us_tr("%1 curves selected, Baseline requires "
  ...
  if (le_baseline2_q_end->text().isEmpty() ||
      ...
      le_baseline2_q_end->text().toDouble() <=
          baseline2_q_min + 0.001  // == caused by the slot, +0.001 for rounding
  ) {
...
void US_Hydrodyn_Dad::baseline2_enables() {
  ...
  // le_baseline2_scale->text().toDouble(); mostly disable except cancel & wheel
  // if ( minimize_running ) {
...
void US_Hydrodyn_Dad::baseline2_add_marker(QwtPlot *plot, double pos,
...
void US_Hydrodyn_Dad::baseline2_q_start_text(const QString &text) {
  ...
  if (baseline2_markers.size() != 2) {
    editor_msg(
        "red",
        QString("internal error: baseline2_q_start_text markers issue size %1")
...
void US_Hydrodyn_Dad::baseline2_q_end_text(const QString &text) {
  ...
  if (baseline2_markers.size() != 2) {
    editor_msg(
        "red",
        QString("internal error: baseline2_q_end_text markers issue size %1")
            .arg(baseline2_markers.size()));
    return;
  }
...
void US_Hydrodyn_Dad::baseline2_create_adjusted_curve(const QString &name) {
  ...
  if (points) {
    ...
    editor_msg(
        "red",
        QString(
            us_tr("Error: no data points found in baseline range for curve %1"))
            .arg(name));
    return;
  }
}
