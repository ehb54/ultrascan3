...
void
US_Hydrodyn_Dad::baseline2_start ()
{
  TSO << "baseline2()\n";
  ...
  if (names.size () != 1)
    {
      QMessageBox::critical (
          ...
          QString (us_tr ("%1 curves selected, Baseline requires exactly one "
                          "curve selected\n"))
              .arg (names.size ()));
  ...
  baseline2_use_errors
      = f_errors.count (baseline2_name) && f_qs.count (baseline2_name)
        && f_errors[baseline2_name].size () == f_qs[baseline2_name].size ()
        && !is_zero_vector (f_errors[baseline2_name]);

  // if ( !baseline2_use_errors &&
  //      (
  //       (US_Eigen::weight_methods)
  //       cb_baseline2_fit_alg_weight->currentData().toInt() ==
  //       US_Eigen::EIGEN_1_OVER_SD
  //       || (US_Eigen::weight_methods)
  //       cb_baseline2_fit_alg_weight->currentData().toInt() ==
  //       US_Eigen::EIGEN_1_OVER_SD_SQ
  //        ) ) {
  //    cb_baseline2_fit_alg_weight->setCurrentIndex( 0 ); //
  ...
  baseline2_add_marker (plot_dist, le_baseline2_q_start->text ().toDouble (),
                        baseline2_color_q, us_tr ("Start"),
                        Qt::AlignRight | Qt::AlignBottom);
  baseline2_add_marker (plot_dist, le_baseline2_q_end->text ().toDouble (),
                        baseline2_color_q, us_tr ("End"),
  ...
  if (le_baseline2_q_start->text ().isEmpty ()
  ...
  if (le_baseline2_q_end->text ().isEmpty ()
      || le_baseline2_q_end->text ().toDouble () > baseline2_q_max
      || le_baseline2_q_end->text ().toDouble ()
             <= baseline2_q_min
    ...
    {
      qDebug () << "baseline2 q end NOT set, value "
                << le_baseline2_q_end->text ();
    }

  lbl_wheel_pos->setText ("");
  pb_baseline2_create_adjusted_curve->setEnabled (
      false); // set in curve_update & curve_clear
  baseline2_enables ();
}

void
US_Hydrodyn_Dad::baseline2_enables ()
{
  // qDebug() << "baseline2_enables() last_minimize_scale " <<
  // last_minimize_scale << " baseline2_scale: " <<
  // le_baseline2_scale->text().toDouble(); mostly disable except cancel &
  // wheel if ( minimize_running ) {
...
void
US_Hydrodyn_Dad::baseline2_scroll_highlight (int pos)
{
  qDebug () << "baseline2_scroll_highlight " << pos;
}

void
US_Hydrodyn_Dad::baseline2_add_marker (QwtPlot *plot, double pos, QColor color,
...
void
US_Hydrodyn_Dad::baseline2_delete_markers ()
{
...
void
US_Hydrodyn_Dad::baseline2_q_start_text (const QString &text)
{
  ...
  if (baseline2_markers.size () != 2)
    {
      editor_msg (
          "red",
          QString (
              "internal error: baseline2_q_start_text markers issue size %1")
              .arg (baseline2_markers.size ()));
      return;
    }
  ...
  disconnect (le_baseline2_q_start, SIGNAL (textChanged (const QString &)), 0,
              0);
...
void
US_Hydrodyn_Dad::baseline2_q_end_text (const QString &text)
{
  ...
  if (baseline2_markers.size () != 2)
    {
      editor_msg (
          "red",
          QString (
              "internal error: baseline2_q_end_text markers issue size %1")
              .arg (baseline2_markers.size ()));
      return;
    }
  ...
  disconnect (le_baseline2_q_end, SIGNAL (textChanged (const QString &)), 0,
              0);
...
void
US_Hydrodyn_Dad::baseline2_q_start_focus (bool hasFocus)
{
  ...
  if (hasFocus)
    {
      ...
      qwtw_wheel->setSingleStep ((baseline2_q_max - baseline2_q_min)
...
void
US_Hydrodyn_Dad::baseline2_q_end_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Dad::baseline2_fit_clear (bool replot)
{
...
void
US_Hydrodyn_Dad::baseline2_curve_update ()
{
  ...
  if (points)
    {
      ...
      baseline2_fit_curve->setPen (
          QPen (Qt::green, use_line_width, Qt::SolidLine));
      baseline2_fit_curve->setSamples ((double *)&x[0], (double *)&y[0], 2);
      baseline2_fit_curve->attach (plot_dist);
      lbl_baseline2_msg->setText (
          QString (us_tr ("Fit average value %1")).arg (baseline2_fit_avg));
...
void
US_Hydrodyn_Dad::baseline2_apply ()
{
...
void
US_Hydrodyn_Dad::baseline2_create_adjusted_curve ()
{
...
void
US_Hydrodyn_Dad::baseline2_create_adjusted_curve (const set<QString> &names)
{
...
void
US_Hydrodyn_Dad::baseline2_create_adjusted_curve (const QString &name)
{
  ...
  if (!f_qs.count (name) || !f_Is.count (name))
    {
      editor_msg (
          "red",
          QString (us_tr ("Error: missing data for curve %1")).arg (name));
      return;
    }
  ...
  if (points)
    {
      ...
      QString newname
          = name + QString ("_blc%1").arg (fit_avg).replace (".", "_");
    ...
    {
      editor_msg ("red", QString (us_tr ("Error: no data points found in "
                                         "baseline range for curve %1"))
                             .arg (name));
      return;
    }
}
