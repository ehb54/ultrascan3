...
void US_Hydrodyn_Dad::guinier_add_marker( 
                                               QwtPlot * plot,
                                               double pos, 
                                               QColor color, 
                                               QString text, 
...
void US_Hydrodyn_Dad::guinier_analysis()
{
   ...
   for ( int i = 0; i < (int) guinier_names.size(); ++i )
   {
      ...
      do {
         ...
         unsigned int pend   = usu->wave[ "dad" ].q.size() ? usu->wave[ "dad" ].q.size() - 1 : 0;
      ...
      if ( computed_rg )
      {
         ...
         QString report =
            ...
            QString::asprintf( "Rg %.1f (%.1f ) (A) MW %.2e (%.2e) qRg [%.3f,%.3f] pts %u chi^2 %.2e r-chi %.2e\n"
                               ...
                               , sRgmin
                               , sRgmax
                               , (unsigned int) usu->wave[ "dad" ].q.size()
                               , chi2
...
bool US_Hydrodyn_Dad::guinier_check_qmax( QString & report,
                                                bool show_message )
{
   report = "";
