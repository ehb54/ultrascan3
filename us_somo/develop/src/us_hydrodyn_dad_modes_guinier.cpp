...
void US_Hydrodyn_Dad::guinier_scroll_highlight(int pos) {
...
void US_Hydrodyn_Dad::guinier_scroll() {
...
void US_Hydrodyn_Dad::guinier_add_marker(QwtPlot *plot, double pos,
...
void US_Hydrodyn_Dad::guinier_sd() {
  disable_all();
  guinier_replot();
  guinier_plot->replot();
  //   guinier_analysis();
...
void US_Hydrodyn_Dad::guinier_plot_rg_toggle() {
...
void US_Hydrodyn_Dad::guinier_plot_mw_toggle() {
...
void US_Hydrodyn_Dad::guinier_residuals(bool reset) {
  if (reset) {
...
void US_Hydrodyn_Dad::guinier_residuals_update() {
  if (rb_guinier_resid_diff->isChecked()) {
...
void US_Hydrodyn_Dad::guinier_analysis() {
  ...
  for (int i = 0; i < (int)guinier_names.size(); ++i) {
    ...
    do {
          ...
          if (!cb_guinier_scroll->isChecked() || i == guinier_scroll_pos) {
            editor_msg(
                ...
                QString(us_tr("%1 too few points left %2 after %3 points "
                    ...
                    .arg(this_name)
                    .arg((int)usu->wave["dad"].q.size() - (int)pts_decrease)
                    .arg(pts_decrease));
          }
          break;
        }

        usu->wave["dad"].q.resize(usu->wave["dad"].q.size() - pts_decrease);
      ...
      unsigned int pend =
          usu->wave["dad"].q.size() ? usu->wave["dad"].q.size() - 1 : 0;

      computed_rg = usu->guinier_plot("dadrg", "dad") &&
    ...
    if (computed_rg) {
      ...
      if (!us_isnan(Rg)) {
        ...
        if (!count) {
          ...
          if (us_isnan(siga) || (unsigned int)usu->wave["dad"].q.size() < 3) {
            if (rg_sd_min > Rg) {
              rg_sd_min = Rg;
            }
            if (rg_sd_max < Rg) {
...
void US_Hydrodyn_Dad::guinier_delete_markers() {
  guinier_plot->detachItems(QwtPlotItem::Rtti_PlotMarker);
  guinier_plot_errors->detachItems(QwtPlotItem::Rtti_PlotMarker);
}

void US_Hydrodyn_Dad::guinier_range(double minq2, double maxq2, double minI,
                                    double maxI) {
...
void US_Hydrodyn_Dad::guinier_range() {
  double minI = 1e99;
  double maxI = 0e0;
  double minq2 = le_guinier_q2_start->text().toDouble() -
                 le_guinier_delta_start->text().toDouble();
...
void US_Hydrodyn_Dad::guinier_replot() {
...
void US_Hydrodyn_Dad::guinier_enables() {
...
bool US_Hydrodyn_Dad::guinier_check_qmax(bool show_message) {
  QString report;
  return guinier_check_qmax(report, show_message);
}

void US_Hydrodyn_Dad::check_mwt_constants(bool force) {
...
bool US_Hydrodyn_Dad::guinier_check_qmax(QString &report, bool show_message) {
  report = "";
...
void US_Hydrodyn_Dad::guinier_qrgmax() {
  disable_all();
  qApp->processEvents();
  guinier_analysis();
  guinier_enables();
...
void US_Hydrodyn_Dad::guinier_qrgmax_text(const QString &) {
...
void US_Hydrodyn_Dad::guinier_q_start_text(const QString &text) {
...
void US_Hydrodyn_Dad::guinier_q_end_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (guinier_markers.size() != 4) {
...
void US_Hydrodyn_Dad::guinier_q_start_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Dad::guinier_q_end_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Dad::guinier_q2_start_text(const QString &text) {
...
void US_Hydrodyn_Dad::guinier_q2_end_text(const QString &text) {
...
void US_Hydrodyn_Dad::guinier_q2_start_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Dad::guinier_q2_end_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Dad::guinier_delta_start_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (!wheel_is_pressed && qwtw_wheel->value() != text.toDouble()) {
...
void US_Hydrodyn_Dad::guinier_delta_end_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (!wheel_is_pressed && qwtw_wheel->value() != text.toDouble()) {
...
void US_Hydrodyn_Dad::guinier_delta_start_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Dad::guinier_delta_end_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Dad::guinier_rg_t_start_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
...
void US_Hydrodyn_Dad::guinier_rg_t_end_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  guinier_plot_rg->setAxisScale(QwtPlot::xBottom,
...
void US_Hydrodyn_Dad::guinier_rg_t_start_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Dad::guinier_rg_t_end_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Dad::guinier_rg_rg_start_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
...
void US_Hydrodyn_Dad::guinier_rg_rg_end_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  guinier_plot_rg->setAxisScale(QwtPlot::yLeft,
...
void US_Hydrodyn_Dad::guinier_rg_rg_start_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Dad::guinier_rg_rg_end_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Dad::guinier_mw_t_start_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
...
void US_Hydrodyn_Dad::guinier_mw_t_end_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  guinier_plot_mw->setAxisScale(QwtPlot::xBottom,
...
void US_Hydrodyn_Dad::guinier_mw_t_start_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Dad::guinier_mw_t_end_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Dad::guinier_mw_mw_start_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
...
void US_Hydrodyn_Dad::guinier_mw_mw_end_text(const QString &text) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  guinier_plot_mw->setAxisScale(QwtPlot::yLeft,
...
void US_Hydrodyn_Dad::guinier_mw_mw_start_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
...
void US_Hydrodyn_Dad::guinier_mw_mw_end_focus(bool hasFocus) {
  if (current_mode != MODE_GUINIER) {
    return;
  }
  if (hasFocus) {
