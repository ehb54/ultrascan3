...
US_ImageViewer::US_ImageViewer (QWidget *parent, const char *, int wFlags)
    : QWidget (parent, name, wFlags), conversion_flags (Qt::PreferDither),
      helpmsg (0)
{
  pickx = -1;
  picky = -1;
  ...
  connect (savepixmap, SIGNAL (activated (int)), this,
           SLOT (savePixmap (int)));
  ...
  edit->insertItem ("&Horizontal flip", this, SLOT (hFlip ()),
                    Qt::ALT + Qt::Key_H);
  edit->insertItem ("&Vertical flip", this, SLOT (vFlip ()),
                    Qt::ALT + Qt::Key_V);
  edit->insertItem ("&Rotate 180", this, SLOT (rot180 ()),
                    Qt::ALT + Qt::Key_R);
  edit->insertSeparator ();
...
*/
void
US_ImageViewer::doOption (int item)
{
  if (item == ss || item == cc)
    {
      // Toggle
  ...
  if (item == ac)
    {
      conversion_flags
          = (conversion_flags & ~Qt::ColorMode_Mask) | Qt::AutoColor;
    }
  else if (item == co)
    {
      conversion_flags
          = (conversion_flags & ~Qt::ColorMode_Mask) | Qt::ColorOnly;
    }
  else if (item == mo)
    {
      conversion_flags
    ...
    {
      conversion_flags = (conversion_flags & ~Qt::AlphaDither_Mask)
...
*/
void
US_ImageViewer::setMenuItemFlags ()
{
  // File
  ...
  bool may_need_dithering
      = may_need_color_dithering
  ...
  options->setItemChecked (ac, (conversion_flags & Qt::ColorMode_Mask)
                                   == Qt::AutoColor);
  options->setItemChecked (co, (conversion_flags & Qt::ColorMode_Mask)
                                   == Qt::ColorOnly);
  options->setItemChecked (mo, (conversion_flags & Qt::ColorMode_Mask)
                                   == Qt::MonoOnly);
  options->setItemChecked (fd, (conversion_flags & Qt::Dither_Mask)
                                   == Qt::DiffuseDither);
  options->setItemChecked (bd, (conversion_flags & Qt::Dither_Mask)
                                   == Qt::OrderedDither);
  options->setItemChecked (td, (conversion_flags & Qt::Dither_Mask)
                                   == Qt::ThresholdDither);
  options->setItemChecked (ta, (conversion_flags & Qt::AlphaDither_Mask)
                                   == Qt::ThresholdAlphaDither);
  options->setItemChecked (fa, (conversion_flags & Qt::AlphaDither_Mask)
                                   == Qt::DiffuseAlphaDither);
  options->setItemChecked (ba, (conversion_flags & Qt::AlphaDither_Mask)
                                   == Qt::OrderedAlphaDither);
  options->setItemChecked (ad, (conversion_flags & Qt::DitherMode_Mask)
                                   == Qt::PreferDither);
  options->setItemChecked (dd, (conversion_flags & Qt::DitherMode_Mask)
                                   == Qt::AvoidDither);
}

void
US_ImageViewer::updateStatus ()
{
...
*/
void
US_ImageViewer::saveImage (int item)
{
  const char *fmt = saveimage->text (item);
  QString savefilename = QFileDialog::getSaveFileName (this, caption (),
                                                       QString (), QString ());
...
*/
void
US_ImageViewer::savePixmap (int item)
{
...
*/
void
US_ImageViewer::openFile ()
{
  QString newfilename = QFileDialog::getOpenFileName (this, caption (),
                                                      QString (), QString ());
...
*/
void
US_ImageViewer::openFile (QString filename)
{
  if (!filename.isEmpty ())
    {
      loadImage (filename);
...
bool
US_ImageViewer::loadImage (const QString &fileName)
{
  filename = fileName;
  bool ok = false;
  if (!filename.isEmpty ())
...
bool
US_ImageViewer::smooth () const
{
  return options->isItemChecked (ss);
}

bool
US_ImageViewer::useColorContext () const
{
...
void
US_ImageViewer::resizeEvent (QResizeEvent *)
{
...
bool
US_ImageViewer::convertEvent (QMouseEvent *e, int &x, int &y)
{
  if (pm.size () != QSize (0, 0))
    {
      int h
          = height () - menubar->heightForWidth (width ()) - status->height ();
...
void
US_ImageViewer::mousePressEvent (QMouseEvent *e)
{
  may_be_other = convertEvent (e, clickx, clicky);
}

void
US_ImageViewer::mouseReleaseEvent (QMouseEvent *)
{
  if (may_be_other)
    other = this;
}
...
*/
void
US_ImageViewer::mouseMoveEvent (QMouseEvent *e)
{
...
void
US_ImageViewer::paintEvent (QPaintEvent *e)
{
  if (pm.size () != QSize (0, 0))
    { // is an image loaded?
      QPainter painter (this);
...
*/
void
US_ImageViewer::giveHelp ()
{
  if (!helpmsg)
    {
      QString helptext = "<p>Supported input formats:"
                         "<blockquote>";
...
void
US_ImageViewer::copyFrom (US_ImageViewer *s)
{
  if (clickx >= 0)
    {
      int dx = clickx;
...
void
US_ImageViewer::setImage (const QImage &newimage)
{
  image = newimage;
...
void
US_ImageViewer::to1Bit ()
{
  toBitDepth (1);
}

void
US_ImageViewer::to8Bit ()
{
  toBitDepth (8);
}

void
US_ImageViewer::to32Bit ()
{
  toBitDepth (32);
}

void
US_ImageViewer::toBitDepth (int d)
{
