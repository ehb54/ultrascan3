     ...
     helpmsg( 0 )
{
   ...
   file->insertItem( "&New window", this,  SLOT(newWindow()), Qt::CTRL+Qt::Key_N );
   ...
   edit->insertItem("&Horizontal flip", this, SLOT(hFlip()), Qt::ALT+Qt::Key_H);
...
void US_ImageViewer::doOption(int item)
{
   ...
   if ( item == ac ) {
      ...
      conversion_flags = ( conversion_flags & ~Qt::Dither_Mask ) | Qt::DiffuseDither;
   } else if ( item == bd ) {
      conversion_flags = ( conversion_flags & ~Qt::Dither_Mask ) | Qt::OrderedDither;
   } else if ( item == td ) {
      conversion_flags = ( conversion_flags & ~Qt::Dither_Mask ) | Qt::ThresholdDither;
   } else if ( item == ta ) {
      conversion_flags = ( conversion_flags & ~Qt::AlphaDither_Mask ) | Qt::ThresholdAlphaDither;
   } else if ( item == fa ) {
      conversion_flags = ( conversion_flags & ~Qt::AlphaDither_Mask ) | Qt::DiffuseAlphaDither;
   } else if ( item == ba ) {
      conversion_flags = ( conversion_flags & ~Qt::AlphaDither_Mask ) | Qt::OrderedAlphaDither;
   } else if ( item == ad ) {
      conversion_flags = ( conversion_flags & ~Qt::DitherMode_Mask ) | Qt::PreferDither;
   } else if ( item == dd ) {
      conversion_flags = ( conversion_flags & ~Qt::DitherMode_Mask ) | Qt::AvoidDither;
   }
...
void US_ImageViewer::setMenuItemFlags()
{
   ...
   options->setItemChecked( ac, (conversion_flags & Qt::ColorMode_Mask) == Qt::AutoColor );
   options->setItemChecked( co, (conversion_flags & Qt::ColorMode_Mask) == Qt::ColorOnly );
   options->setItemChecked( mo, (conversion_flags & Qt::ColorMode_Mask) == Qt::MonoOnly );
   options->setItemChecked( fd, (conversion_flags & Qt::Dither_Mask) == Qt::DiffuseDither );
   options->setItemChecked( bd, (conversion_flags & Qt::Dither_Mask) == Qt::OrderedDither );
   options->setItemChecked( td, (conversion_flags & Qt::Dither_Mask) == Qt::ThresholdDither );
   options->setItemChecked( ta, (conversion_flags & Qt::AlphaDither_Mask) == Qt::ThresholdAlphaDither );
   options->setItemChecked( fa, (conversion_flags & Qt::AlphaDither_Mask) == Qt::DiffuseAlphaDither );
   options->setItemChecked( ba, (conversion_flags & Qt::AlphaDither_Mask) == Qt::OrderedAlphaDither );
   options->setItemChecked( ad, (conversion_flags & Qt::DitherMode_Mask) == Qt::PreferDither );
   options->setItemChecked( dd, (conversion_flags & Qt::DitherMode_Mask) == Qt::AvoidDither );
}
...
{
   US_ImageViewer* that = new US_ImageViewer(0, "new window", Qt::WDestructiveClose);
...
{
   if (!helpmsg) {
      QString helptext = "<p>Supported input formats:""<blockquote>";
...
void US_ImageViewer::hFlip()
{
   setImage(image.mirror(TRUE,FALSE));
}

void US_ImageViewer::vFlip()
{
   setImage(image.mirror(FALSE,TRUE));
}

void US_ImageViewer::rot180()
{
...
void US_ImageViewer::to1Bit()
{
   toBitDepth(1);
}

void US_ImageViewer::to8Bit()
{
   toBitDepth(8);
}

void US_ImageViewer::to32Bit()
{
   toBitDepth(32);
}
