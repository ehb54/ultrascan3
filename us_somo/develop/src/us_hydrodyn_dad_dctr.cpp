...
US_Hydrodyn_Dad_Dctr::US_Hydrodyn_Dad_Dctr (void *us_hydrodyn_dad,
    ...
    : QDialog (p)
{
  ...
  setWindowTitle (us_tr ("US-SOMO: UV-Vis : Concentration Detector"));
...
US_Hydrodyn_Dad_Dctr::~US_Hydrodyn_Dad_Dctr () {}

void
US_Hydrodyn_Dad_Dctr::setupGUI ()
{
  int minHeight1 = 30;

  lbl_title
      = new QLabel (us_tr ("US-SOMO: UV-Vis : Concentration Detector"), this);
  ...
  cb_uv->setFont (QFont (USglobal->config_list.fontFamily,
                         USglobal->config_list.fontSize));
  ...
  lbl_uv_conv->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  lbl_uv_conv->setMinimumHeight (minHeight1);

  le_uv_conv = new QLineEdit (this);
  le_uv_conv->setObjectName ("le_uv_conv Line Edit");
  ...
  AUTFBACK (le_uv_conv);
  le_uv_conv->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize));
  {
  ...
  connect (le_uv_conv, SIGNAL (textChanged (const QString &)),
           SLOT (uv_conv_text (const QString &)));
  ...
  cb_ri->setFont (QFont (USglobal->config_list.fontFamily,
                         USglobal->config_list.fontSize));
  ...
  lbl_ri_conv->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
  lbl_ri_conv->setMinimumHeight (minHeight1);

  le_ri_conv = new QLineEdit (this);
  le_ri_conv->setObjectName ("le_ri_conv Line Edit");
  ...
  AUTFBACK (le_ri_conv);
  le_ri_conv->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize));
  {
  ...
  connect (le_ri_conv, SIGNAL (textChanged (const QString &)),
           SLOT (ri_conv_text (const QString &)));
  ...
  always_hide_widgets.insert ({ cb_ri, lbl_ri_conv, le_ri_conv });

  ShowHide::hide_widgets (always_hide_widgets);
}

void
US_Hydrodyn_Dad_Dctr::quit ()
{
  close ();
}

void
US_Hydrodyn_Dad_Dctr::save ()
{
  ...
  QString fn = QFileDialog::getSaveFileName (
      this, us_tr ("Select a name to save the detector settings"), use_dir,
      "*_dctr.dat");
...
void
US_Hydrodyn_Dad_Dctr::closeEvent (QCloseEvent *e)
{
...
void
US_Hydrodyn_Dad_Dctr::uv_conv_text (const QString &text)
{
  (*parameters)["uv_conv"] = text;
}

void
US_Hydrodyn_Dad_Dctr::ri_conv_text (const QString &text)
{
