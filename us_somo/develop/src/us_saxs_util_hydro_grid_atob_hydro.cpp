...
// #define DEBUG_ATOB
#if defined(OLD_WAY)
static float particle_max_size(PDB *pdb) {
  PDB *p;
  float max_size = 0.0;
  float temp = 0.0;
...
static vector<float> particle_max_sizes(PDB *pdb) {
  PDB *p;
  float max_x = 0.0;
  float max_y = 0.0;
  float max_z = 0.0;
...
static float ***f3tensor(long nrl, long nrh, long ncl, long nch, long ndl,
                         long ndh)
...
static PDB *AtoB(PDB *pdb, long npoints_x, long npoints_y, long npoints_z,
                 float dx,
                 // long *natoms,
                 PHYSPROP *prop, PHYSPROP *nprop, long *natoms3,
                 int centre_or_cog, const char input_file[],
                 int set_bead_radius, const char atnam[], const char resnam[],
  ...
  /*
    ...
    half the cube's side input_file used to generate log file atnam ? string for
    atom name used atnam ? string for residue name used
  */
  ...
  if (!(ro = (float ***)f3tensor(0, npoints_x, 0, npoints_y, 0, npoints_z)) ||
    ...
    if (ro) free_f3tensor(ro, 0, npoints_x, 0, npoints_y, 0, npoints_z);
    if (x) free_f3tensor(x, 0, npoints_x, 0, npoints_y, 0, npoints_z);
    if (y) free_f3tensor(y, 0, npoints_x, 0, npoints_y, 0, npoints_z);
    if (z) free_f3tensor(z, 0, npoints_x, 0, npoints_y, 0, npoints_z);
    if (mass) free_f3tensor(mass, 0, npoints_x, 0, npoints_y, 0, npoints_z);
    if (si) free_f3tensor(si, 0, npoints_x, 0, npoints_y, 0, npoints_z);
    if (saxs_excl_vol)
    ...
    for (j = 0; j < npoints_y; j++) {
      for (k = 0; k < npoints_z; k++) {
        if (ro[i][j][k] > 0.000001) {
          ...
          current->occ = 1.0;
          // printf("#%d: x=%f , y=%f,
          // z=%f\n",i,current->x,current->y,current->z); fprintf(stderr,"\n
          // >>>>> set bead radius = %d !!! >>> \n",set_bead_radius); assigns
          // radius, mass and colour
          if (centre_or_cog == 0 || centre_or_cog == 2 || set_bead_radius == 0)
            nthis->rVW = (float)ro[i][j][k]; /*Van der Walls radii */
          else {
            if (ro[i][j][k] <= (float)(dx / 2))  // if coord centre of cell, it
                                                 // puts a maximum radius
              nthis->rVW =
                  (float)ro[i][j][k];  // equal to half the cell's diameter
            else {
  ...
  *natoms3 = natoms2;
  ...
  if (create_nmr) {
    ...
    if (!fn_out.open(QIODevice::WriteOnly)) {
      // QMessageBox::warning( us_hydrodyn, "US-SOMO: A2B: Create mulitple model
      // output file",
      //                        QString( "Could not open %1 for writing!" ).arg(
      //                        fn ) );
    } else {
      ...
      for (n = 1, i = 0; i < npoints_x; i++) {
        ...
        //     //sleep(1);
        //   }
        // us_hydrodyn->lbl_core_progress->setText( QString( "Creating NMR style
        // output file %1 of %2" ).arg( i ).arg( npoints_x ) );
        // qApp->processEvents();
        for (j = 0; j < npoints_y; j++) {
          for (k = 0; k < npoints_z; k++) {
            if (mass[i][j][k] > 0)  // gets only if there is a bead
                  ...
                  if (((int)((p->y) / dx) + (int)(npoints_y / 2)) == j) {
                    if (((int)((p->z) / dx) + (int)(npoints_z / 2)) == k) {
                      tso << QString("").sprintf(
                          ...
                          "         %2s\n",
                          ++atomno, p->atnam, p->resnam, p->chain, p->resnum,
...

{
  ...
  for (unsigned int i = 0; i < bead_model->size(); i++) {
    if ((*bead_model)[i].active) {
      ...
      // cout << QString( "atom %1 si %2\n" ).arg( (*bead_model)[i].name ).arg(
      // (*bead_model)[i].si );; if ( us_hydrodyn->advanced_config.debug_1 )
      // {
      ...
      printf("%f %f %f %f %f %d %d %f %f %f\n", tmp_pdb.x, tmp_pdb.y, tmp_pdb.z,
             tmp_pdb.occ, tmp_pdb.bval, tmp_pdb.atnum, tmp_pdb.resnum,
             tmp_prop.f, tmp_prop.rVW, tmp_prop.mass);
#endif
    }
  }
  ...
  /*This assures that the spacing left between the box frontere and the closest
    particle atom is larger than three times the standard VW radius, that is the
    electron density will always have space left to expand at least 1.5A away
  ...
  if (!(nprop = (PHYSPROP *)malloc((int)(npoints_x * npoints_y * npoints_z) *
    ...
    us_log->log(
        "ERROR: Memory allocation failure.  Please try a larger grid cube size "
        "and/or close all other applications");
    if (us_udp_msg) {
      map<QString, QString> msging;
      msging["_textarea"] =
  ...
  //    //                                          us_hydrodyn,
  //    //                                          "save the models",
  //    //                                          "US_SOMO: A2B: Choose a name
  //    to save the multiple model output file" );
  ...
  for (; this_pdb; this_pdb = this_pdb->next, this_prop = this_prop->next) {
    ...
    printf("after a2b: %d %s %s %s %s %g %g %g %g %g\n", this_pdb->atnum,
           this_pdb->atnam, this_pdb->resnam, this_pdb->insert, this_pdb->chain,
           this_pdb->x, this_pdb->y, this_pdb->z, this_prop->rVW,
           this_prop->mass);
#endif
  }
  free(nprop);
  ...
  if (use_grid_options->equalize_radii_constant_volume) {
    ...
    for (unsigned int i = 0; i < (unsigned int)result_bead_model.size(); i++) {
      tot_vol += (4e0 / 3e0) * M_PI * result_bead_model[i].radius *
                 result_bead_model[i].radius * result_bead_model[i].radius;
    }
...
//       if ( save_e_density )
//       {
//          editor_msg( "dark blue", "Temporarily setting water electron denisty
//          to zero for Iq computation" );
//       } else {
//          editor_msg( "dark blue", "Water electron denisty is already zero for
//          Iq computation" );
//       }
//    }
//    batch_window->show();
...
//       {
//          error_msg = QString( "Error: compute_structure_factors(): batch
//          computed csv output file %1: format error (I & q lines incompatible)
//          Iq line %2" ).arg( csvfile ).arg( i + 1 ); return false;
//       }
...
//       for bead %1" ) ).arg( j == last_bead ? "Global average" : QString( "%1"
//       ).arg( j + 1 ) ) );
//       editor->verticalScrollBar()->setValue(editor->verticalScrollBar()->maximum());
//       qApp->processEvents();
...
//                                      ) )
//       {
//          error_msg = QString( "Error: compute_structure_factors(): %1" ).arg(
//          usu.errormsg ); return false;
//       }
