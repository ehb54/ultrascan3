...
#define VW_average_radius 1.5 /* Van der Walls average radius equal to 1.5 A  \
                               */
#define EXPANSION_COEF                                                        \
  0.2 /*how much bigger LMAX will be with respect to the protein max size!    \
         (0.2=20%) */
...
static void
free_f3tensor (float ***t, long nrl, long nrh, long ncl, long nch, long ndl,
               long ndh)
...
static PDB *
AtoB (PDB *pdb, long npoints_x, long npoints_y, long npoints_z, float dx,
      // long *natoms,
      PHYSPROP *prop, PHYSPROP *nprop, long *natoms3, int centre_or_cog,
      const char input_file[], int set_bead_radius, const char atnam[],
      const char resnam[],
      // US_Hydrodyn *us_hydrodyn,
      bool create_nmr, QString &fn, bool &nmr_created)
{
#if defined(DEBUG)
  //   printf("npoints %ld\n", npoints);
  puts ("in atob 0");
  ...
  /*
    ...
    dx= cube's side
    prop= PHYSPROP structure with radii and masses of beads read from
    AtomicVW.dat nprop= pointer where it puts new pdb PHYSPROP to be used if
    writing file in output format. has to be allocated in calling routine!
    natoms3= number of atoms in new AtoB model. has to be allocated in calling
    routine! centre_of_cog= if zero places beads in cubes centres, if 1 if
    calculates the cog of the beads in the cube and places bead therein
    set_bead_radius= if zero, does nothing, if not it sets the bead radius to
    half the cube's side input_file used to generate log file atnam ? string
    for atom name used atnam ? string for residue name used
  */
  ...
  if (!(ro = (float ***)f3tensor (0, npoints_x, 0, npoints_y, 0, npoints_z))
      || !(mass
           = (float ***)f3tensor (0, npoints_x, 0, npoints_y, 0, npoints_z))
      || !(saxs_excl_vol
           = (float ***)f3tensor (0, npoints_x, 0, npoints_y, 0, npoints_z))
      ...
      || !(z = (float ***)f3tensor (0, npoints_x, 0, npoints_y, 0, npoints_z)))
    {
      ...
      if (saxs_excl_vol)
        free_f3tensor (saxs_excl_vol, 0, npoints_x, 0, npoints_y, 0,
                       npoints_z);
      return (PDB *)0;
    }
#if defined(DEBUG)
  ...
  fflush (stdout);
#endif
  for (p = pdb, pp = prop, n = 0; p;
       p = p->next, pp = pp->next, n++) /* over the atoms  */
    {
#if defined(DEBUG)
      ...
      ro[x_c][y_c][z_c]
          += pow (pp->rVW, 3); /* = ctte*electron_density*gaussian */
#if defined(DEBUG)
      puts ("in atob 3c_2");
      fflush (stdout);
#endif
  ...
  for (i = 0; i < npoints_x;
       i++) /* Now the radius of the beads are in r0[][][] */
    for (j = 0; j < npoints_y; j++)
      for (k = 0; k < npoints_z; k++)
  ...
  for (i = 0; i < npoints_x; i++) /* clears up r0 */
    {
      // us_hydrodyn->lbl_core_progress->setText(QString("Gridding %1 of
      // %2").arg(i+1).arg(npoints_x)); qApp->processEvents(); if
      // (us_hydrodyn->stopFlag)
      // {
      //    free_f3tensor(ro, 0, npoints_x, 0, npoints_y, 0, npoints_z);
      //    free_f3tensor(x, 0, npoints_x, 0, npoints_y, 0, npoints_z);
      //    free_f3tensor(y, 0, npoints_x, 0, npoints_y, 0, npoints_z);
              ...
              if (ro[i][j][k] > 0.000001)
                {
                  ...
                  // fprintf(stderr,"\n >>>>> set bead radius = %d !!! >>>
                  // \n",set_bead_radius); assigns radius, mass and colour
                  if (centre_or_cog == 0 || centre_or_cog == 2
                      || set_bead_radius == 0)
                    nthis->rVW = (float)ro[i][j][k]; /*Van der Walls radii */
                  else
                    {
                      if (ro[i][j][k]
                          <= (float)(dx / 2)) // if coord centre of cell, it
                                              // puts a maximum radius
                        nthis->rVW = (float)
                            ro[i][j][k]; // equal to half the cell's diameter
                      else
                        {
                  ...
                  nthis->saxs_excl_vol
                      = (float)saxs_excl_vol[i][j][k]; /* atomic mass */
  ...
  /*    for(i=0,p=npdb,pp=nprop;p;p=p->next,pp=pp->next,i++)
        ...
        //fprintf(rmc,"%f\t%.0f\t%.0f\n",pp->rVW,pp->mass,pp->f);// writes RMC
     file
  ...
  for (n = 1, i = 0; i < npoints_x; i++)
    {
      ...
      //   }
      // us_hydrodyn->lbl_core_progress->setText(QString("Gridding %1 of
      // %2").arg(i+1).arg(npoints_x)); qApp->processEvents();
      for (j = 0; j < npoints_y; j++)
        {
          for (k = 0; k < npoints_z; k++)
            {
              if (mass[i][j][k] > 0) // gets only if there is a bead
                {
                  ...
                  for (p = pdb, nn = 0; p;
                       p = p->next, nn++) // goes over all atoms
  ...
  if (create_nmr)
    {
      ...
      if (!fn_out.open (QIODevice::WriteOnly))
        {
          // QMessageBox::warning( us_hydrodyn, "US-SOMO: A2B: Create mulitple
          // model output file",
          ...
          for (n = 1, i = 0; i < npoints_x; i++)
            {
              ...
              //     //sleep(1);
              //   }
              // us_hydrodyn->lbl_core_progress->setText( QString( "Creating
              // NMR style output file %1 of %2" ).arg( i ).arg( npoints_x ) );
              // qApp->processEvents();
              for (j = 0; j < npoints_y; j++)
                {
                  for (k = 0; k < npoints_z; k++)
                                      ...
                                      if (((int)((p->z) / dx)
                                          ...
                                          == k)
                                        {
                                          tso << QString ("").sprintf (
                                              ...
                                              "    %2s\n",
                                              ++atomno, p->atnam, p->resnam,
                                              p->chain, p->resnum, p->x, p->y,
                                              p->z, p->occ, p->bval, "  ");
                                        }
                                    }
                                }
...
vector<PDB_atom>
us_hydrodyn_grid_atob_hydro (vector<PDB_atom> *bead_model,
...

{
      ...
      if ((*bead_model)[i].active)
        {
          ...
          fflush (stdout);
#endif
          strncpy (tmp_pdb.atnam, (*bead_model)[i].name.toLatin1 ().data (),
                   7);
          tmp_pdb.atnam[7] = 0;

          strncpy (tmp_pdb.resnam,
                   (*bead_model)[i].resName.toLatin1 ().data (), 7);
          tmp_pdb.resnam[7] = 0;

          strncpy (tmp_pdb.insert, (*bead_model)[i].iCode.toLatin1 ().data (),
                   7);
          tmp_pdb.insert[7] = 0;

          strncpy (tmp_pdb.chain, (*bead_model)[i].chainID.toLatin1 ().data (),
                   7);
          tmp_pdb.chain[7] = 0;
          ...
          // cout << QString( "atom %1 si %2\n" ).arg( (*bead_model)[i].name
          // ).arg( (*bead_model)[i].si );; if (
          // us_hydrodyn->advanced_config.debug_1 )
          // {
          ...
          printf ("%f %f %f %f %f %d %d %f %f %f\n", tmp_pdb.x, tmp_pdb.y,
                  tmp_pdb.z, tmp_pdb.occ, tmp_pdb.bval, tmp_pdb.atnum,
                  tmp_pdb.resnum, tmp_prop.f, tmp_prop.rVW, tmp_prop.mass);
#endif
        }
    }
  ...
  /*This assures that the spacing left between the box frontere and the closest
    particle atom is larger than three times the standard VW radius, that is
    the electron density will always have space left to expand at least 1.5A
  ...
  if (!(nprop = (PHYSPROP *)malloc ((int)(npoints_x * npoints_y * npoints_z)
                                    * sizeof (PHYSPROP))))
    {
      //    QColor save_color = us_hydrodyn->editor->textColor();
      //    us_hydrodyn->editor->setTextColor("red");
      //    us_hydrodyn->editor->append("ERROR: Memory allocation failure.
      //    Please try a larger grid cube size and/or close all other
      //    applications");
      us_log->log ("ERROR: Memory allocation failure.  Please try a larger "
                   "grid cube size and/or close all other applications");
      if (us_udp_msg)
        {
          map<QString, QString> msging;
          msging["_textarea"]
              = "ERROR: Memory allocation failure.  Please try a larger grid "
                "cube size and/or close all other applications";

          us_udp_msg->send_json (msging);
          // sleep(1);
        }
      // accumulated_msgs += "ERROR: Memory allocation failure.  Please try a
      // larger grid cube size and/or close all other applications";
      accumulated_msgs->append (
          "ERROR: Memory allocation failure.  Please try a larger grid cube "
          "size and/or close all other applications");
  ...
  printf ("npoints_x %d\n"
          ...
          "cube side %f\n"
          "max radius %f\n",
          (int)npoints_x, (int)npoints_y, (int)npoints_z,
          (float)use_grid_options->cube_side, max_radius);
#endif
  ...
  // {
  //    fn =
  //       us_hydrodyn->somo_dir + QDir::separator() + "structures" +
  //       QDir::separator() + us_hydrodyn->project + QString("_%1").arg(
  //       us_hydrodyn->current_model + 1) + QString(
  //       us_hydrodyn->bead_model_suffix.length() ?
  ...
  //    //                                          us_hydrodyn,
  //    //                                          "save the models",
  //    //                                          "US_SOMO: A2B: Choose a
  //    name to save the multiple model output file" );
  ...
  PDB *result_pdb
      = AtoB ((PDB *)&pdb[0], npoints_x, npoints_y, npoints_z,
              (float)use_grid_options->cube_side, (PHYSPROP *)&prop[0],
              (PHYSPROP *)&nprop[0], &natoms3, use_grid_options->center,
              "tmp_atob", use_grid_options->tangency ? 1 : 0, "PB", "RES",
              // us_hydrodyn,
              use_grid_options->create_nmr_bead_pdb, fn, nmr_created);
  ...
  if (!result_pdb)
    {
      ...
      // QColor save_color = us_hydrodyn->editor->textColor();
      // us_hydrodyn->editor->setTextColor("red");
      // us_hydrodyn->editor->append("ERROR: Memory allocation failure.  Please
      // try a larger grid cube size and/or close all other applications");
  ...
  for (; this_pdb; this_pdb = this_pdb->next, this_prop = this_prop->next)
    {
      ...
      // #define DEBUG_ATOB
#if defined(DEBUG_ATOB)
      printf ("after a2b: %d %s %s %s %s %g %g %g %g %g\n", this_pdb->atnum,
              this_pdb->atnam, this_pdb->resnam, this_pdb->insert,
              this_pdb->chain, this_pdb->x, this_pdb->y, this_pdb->z,
              this_prop->rVW, this_prop->mass);
#endif
    }
  free (nprop);
#if defined(DEBUG)
  ...
  fflush (stdout);
#endif
  // editor->append(QString("Grid contains %1
  // beads\n").arg(result_bead_model.size()));

  us_log->log (
      QString ("Grid contains %1 beads\n").arg (result_bead_model.size ()));
  if (us_udp_msg)
    {
      map<QString, QString> msging;
      msging["_textarea"] = QString ("Grid contains %1 beads\\n")
                                .arg (result_bead_model.size ());
  ...
  // printf("bead model size %d\n", (int)result_bead_model.size());
  // fflush(stdout); if ( us_hydrodyn->advanced_config.debug_1 )
  // {
  //    printf("grid pre mw %.6f (%d), post mw %.6f (%d), diff %.6f\n", pre_mw,
  //    pre_mw_c, post_mw, post_mw_c, pre_mw - post_mw);
  // }

  if (use_grid_options->equalize_radii_constant_volume)
    {
      double tot_vol = 0e0;
      for (unsigned int i = 0; i < (unsigned int)result_bead_model.size ();
           i++)
        {
          tot_vol += (4e0 / 3e0) * M_PI * result_bead_model[i].radius
                     * result_bead_model[i].radius
                     * result_bead_model[i].radius;
        }
      // us_hydrodyn->editor_msg( "dark blue", QString( "Equalizing bead model,
      // total volume %1\n" ).arg( tot_vol ) );
      double pi43 = M_PI * 4e0 / 3e0;
      float radius = (float)pow (
          tot_vol / ((double)result_bead_model.size () * pi43), 1e0 / 3e0);
      for (unsigned int i = 0; i < (unsigned int)result_bead_model.size ();
           i++)
        {
...
//    {
//       error_msg = QString( "Error: compute_structure_factors(): file %1 does
//       not exist" ).arg( filename ); return false;
//    }
//    if ( !QFileInfo( filename ).isReadable() )
//    {
//       error_msg = QString( "Error: compute_structure_factors(): file %1 is
//       not readable" ).arg( filename ); return false;
//    }
...
//    + csv_addendum ); batch_window->cb_compute_iq_avg->setChecked( true );
...
//       if ( save_e_density )
//       {
//          editor_msg( "dark blue", "Temporarily setting water electron
//          denisty to zero for Iq computation" );
//       } else {
//          editor_msg( "dark blue", "Water electron denisty is already zero
//          for Iq computation" );
//       }
//    }
//    batch_window->show();
...
//    if ( !saxs_options.bead_models_rho0_in_scat_factors )
//    {
//       editor_msg( "dark blue", QString( "Restored water electron denisty to
//       %1" ).arg( save_e_density ) );
//    }

//    if ( batch_window->stopFlag )
//    {
//       error_msg = "Error: compute_structure_factors(): batch computation
...
//    {
//       error_msg = QString( "Error: compute_structure_factors(): batch
//       computed csv output file %1 does not exist" ).arg( csvfile ); return
//       false;
//    }
//    if ( !QFileInfo( csvfile ).isReadable() )
//    {
//       error_msg = QString( "Error: compute_structure_factors(): batch
//       computed csv output file %1 is not readable" ).arg( csvfile ); return
//       false;
//    }
//    QFile f( csvfile );
...
//    {
//       error_msg = QString( "Error: compute_structure_factors(): batch
//       computed csv output file %1: can not open" ).arg( csvfile ); return
//       false;
//    }
...
//    {
//       error_msg = QString( "Error: compute_structure_factors(): batch
//       computed csv output file %1: format error in q line" ).arg( csvfile );
//       return false;
//    }
...
//    {
//       error_msg = QString( "Error: compute_structure_factors(): batch
//       computed csv output file %1: insufficinet Iq lines (only found %2)"
//       ).arg( csvfile ).arg( qslIs.size() ); return false;
//    }
...
//       {
//          error_msg = QString( "Error: compute_structure_factors(): batch
//          computed csv output file %1: format error in Iq line %2" ).arg(
//          csvfile ).arg( i + 1 ); return false;
//       }
...
//       {
//          error_msg = QString( "Error: compute_structure_factors(): batch
//          computed csv output file %1: format error (I & q lines
//          incompatible) Iq line %2" ).arg( csvfile ).arg( i + 1 ); return
//          false;
//       }
...
//    take awhile & the program may seem unresponsive" ) );
...
//    {
//       editor_msg( "blue", QString( us_tr( "Computing 4 & 5 term exponentials
//       for bead %1" ) ).arg( j == last_bead ? "Global average" : QString(
//       "%1" ).arg( j + 1 ) ) );
//       editor->verticalScrollBar()->setValue(editor->verticalScrollBar()->maximum());
//       qApp->processEvents();
...
//                                      ) )
//       {
//          error_msg = QString( "Error: compute_structure_factors(): %1"
//          ).arg( usu.errormsg ); return false;
//       }
...
//       editor_msg( "dark blue", QString( "Exponentials 4 terms norm %1 nnorm
//       %2 (%3): %4" )
//                   .arg( norm4 )
//                   .arg( nnorm4 )
//                   .arg( norm4tag )
...
//       editor_msg( "dark blue", QString( "Exponentials 5 terms norm %1 nnorm
//       %2 (%3): %4" )
//                   .arg( norm5 )
//                   .arg( nnorm5 )
//                   .arg( norm5tag )
