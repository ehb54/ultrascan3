					...
					
{
  ...
  us_udp_msg = use_us_udp_msg;
   ...
   //    us_hydrodyn->editor->append("ERROR: Memory allocation failure.  Please try a larger grid cube size and/or close all other applications");
       us_log->log("ERROR: Memory allocation failure.  Please try a larger grid cube size and/or close all other applications");
       if ( us_udp_msg )
	 {
	   map < QString, QString > msging;
	   msging[ "_textarea" ] = "ERROR: Memory allocation failure.  Please try a larger grid cube size and/or close all other applications";
   ...
   us_log->log(QString("Grid contains %1 beads\n").arg(result_bead_model.size()));
   if ( us_udp_msg )
     {
       map < QString, QString > msging;
