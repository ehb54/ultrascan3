...
int
US_Tar::create (const QString &archive, const QString &directory,
                QStringList *list)
{
...
int
US_Tar::create (const QString &archive, const QStringList &files,
                QStringList *list)
{
  ...
  //     a.  If the file is a directory, continue for each file in the
  //     directory b.  Write the header to the archive c.  Copy the file to the
  //     archive
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
    {
      ...
      // Block and character devices, pipes, and symbolic links should be
      // ignored
    }
...
#define O_BINARY 0
  ...
  ofd = open (archive.toLatin1 ().data (), O_WRONLY | O_CREAT | O_BINARY,
              0644);
  // cout << "ofd=" << ofd << endl;
  if (ofd < 0)
    return TAR_CANNOTCREATE;
  ...
  try
  ...
  catch (int error)
    {
      close (ifd);
      close (ofd);
      QFile::remove (archive.toLatin1 ().data ());
      return error;
    }
...
void
US_Tar::process_dir (const QString &path, QStringList &all)
{
  QDir dir (path);
...
void
US_Tar::write_file (const QString &file)
{
  // Create and output the header
  QFileInfo f (file);

  struct stat stats;
  int ret = stat (file.toLatin1 ().data (), &stats);
  if (ret < 0)
    throw TAR_CANTSTAT;
  ...
  strcpy (tar_header.header.name, file.toLatin1 ().data ());

  int perms = TSUID | TSGID | TSVTX | TUREAD | TUWRITE | TUEXEC | TGREAD
  ...
  memcpy (&tar_header.header.chksum, "        ",
          sizeof tar_header.header.chksum);

  // char typeflag;      /* 156 */

  if (f.isDir ())
    tar_header.header.typeflag = '5';
  else if (f.isFile ())
    tar_header.header.typeflag = '0';
  else
    throw TAR_INTERNAL;
  ...
  memcpy ((void *)(buffer + blocks_written * BLOCK_SIZE), (void *)tar_header.h,
          sizeof tar_header);
  ...
  if (!f.isDir ())
    {
      ifd = open (file.toLatin1 ().data (), O_RDONLY | O_BINARY);
      if (ifd < 0)
        throw TAR_READERROR;
      ...
      if (full_blocks > 0 && space >= full_blocks)
        {
          ...
          if (input != full_blocks * BLOCK_SIZE)
            throw TAR_READERROR;
      ...
      // printf( "buf addr=0x%08x; offset=%08x\n", &(buffer[0]), blocks_written
      // * BLOCK_SIZE ); cout << "input=" << input << endl; cout << "final
      // blocks_written=" << blocks_written << endl;
      close (ifd);
    }
}

void
US_Tar::write_long_filename (const QString &filename)
{
  ...
  for (int i = 0; i < full_blocks; i++)
    {
      ...
      memcpy ((void *)tar_header.h,
              (void *)filename.mid (i * BLOCK_SIZE, BLOCK_SIZE)
                  .toLatin1 ()
                  .data (),
              BLOCK_SIZE);
      ...
      memcpy ((void *)(buffer + blocks_written * BLOCK_SIZE),
              (void *)tar_header.h, sizeof tar_header);
  ...
  if ((length) % BLOCK_SIZE) // Copy what is left
    {
      ...
      memcpy (
          (void *)tar_header.h,
          (void *)filename.mid (full_blocks * BLOCK_SIZE).toLatin1 ().data (),
          length % BLOCK_SIZE);
  ...
  memset ((void *)tar_header.h, 0, sizeof (tar_header));
  memcpy ((void *)tar_header.h, filename.toLatin1 ().data (), 100);
}

void
US_Tar::archive_end (void)
{
  // Write a null block
...
///////////////////////////
int
US_Tar::extract (const QString &archive, QStringList *list)
{
  /* 1. Open the archve
  ...
  try
      ...
      while (true)
        {
          ...
          if (list)
            list->append (filename); // Return list if requested
          ...
          if (directory)
            ...
            {
              ofd = open (filename.toLatin1 ().data (),
                          O_WRONLY | O_CREAT | O_BINARY, 0644);
              if (ofd < 0)
                throw TAR_WRITEERROR;
              ...
              if (skip == BLOCK_SIZE)
                skip = 0; // If file size is exact multple of blocks
          ...
          // are broken and trash the modes of the file.
          ...
          time.modtime = mtime;
          utime (filename.toLatin1 ().data (), &time);

#ifndef WIN32
          // Update permissions
          chmod (filename.toLatin1 ().data (), mode);
  ...
  catch (int error)
    {
      ...
      for (size_t i = files.size () - 1; i <= 0; i++)
        {
          QFile::remove (files[i].toLatin1 ().data ());
        }
  ...
  for (unsigned int i = 0; i < (unsigned int)dirs.size (); i++)
    {
      ...
      utime (dirs[i].toLatin1 ().data (), &time);
    }
...
/////////////////////////////
int
US_Tar::list (const QString &archive, QStringList &files, bool brief)
{
  ifd = open (archive.toLatin1 ().data (), O_RDONLY | O_BINARY);
  if (ifd < 0)
    return TAR_NOTFOUND;
      ...
      while (true)
        {
          ...
          if (!brief)
            {
              files << format_permissions (mode, directory) + " " + uname + "/"
                           + gname + " " + s.sprintf ("%10d", fsize) + " "
                           + format_datetime (mtime) + " " + filename;
            }
          else
            {
              files << filename;
...
QString
US_Tar::format_permissions (const unsigned int mode, const bool dir)
{
  QString s = "----------";
  if (dir)
    s = s.replace (0, 1, "d");
...
QString
US_Tar::format_datetime (const unsigned int mtime)
{
  time_t time = mtime;
  struct tm *t = localtime (&time);

  QString s;
  s = s.sprintf ("%04d-%02d-%02d %02d:%02d", t->tm_year + 1900, t->tm_mon + 1,
...
/////////////////////////////
QString
US_Tar::explain (const int error)
{
  QString explanation;
  switch (error)
    {
