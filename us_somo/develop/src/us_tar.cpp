...
#define O_BINARY 0
  ...
  ofd = open(archive.toLatin1().data(), O_WRONLY | O_CREAT | O_BINARY, 0644);
  // cout << "ofd=" << ofd << endl;
...
void US_Tar::write_file(const QString& file) {
  ...
  if (!f.isDir()) {
    ...
    if (full_blocks > 0 && space >= full_blocks) {
      ...
      input =
          read(ifd, buffer + blocks_written * BLOCK_SIZE, space * BLOCK_SIZE);
...
void US_Tar::write_long_filename(const QString& filename) {
  ...
  for (int i = 0; i < full_blocks; i++) {
    ...
    memcpy((void*)tar_header.h,
           (void*)filename.mid(i * BLOCK_SIZE, BLOCK_SIZE).toLatin1().data(),
           BLOCK_SIZE);
...
int US_Tar::extract(const QString& archive, QStringList* list) {
  ...
  try {
    while (true) {
      ...
      {
        ofd = open(filename.toLatin1().data(), O_WRONLY | O_CREAT | O_BINARY,
                   0644);
