...
#if !defined(CMDLINE)
bool US_Saxs_Util::run_hydro(map<QString, QString>&, map<QString, QString>&) {
  return false;
}

void US_Saxs_Util::read_residue_file() {};
bool US_Saxs_Util::screen_pdb(QString, bool) { return false; };
bool US_Saxs_Util::set_default(map<QString, QString>&, map<QString, QString>&) {
  return false;
};

#endif

bool US_Saxs_Util::run_pm(map<QString, QString>& parameters,
                          map<QString, QString>& results) {
...

  {
    ...
    for (int i = 0; i < (int)files_try.size(); ++i) {
      ...
      if (!fi.exists()) {
        ...
        if (!fi.isReadable()) {
          results["errors"] +=
              QString(" file %1 exists but is not readable.").arg(files_try[i]);
        } else {
          files << files_try[i];
        }
      }
  ...
  for (int i = 0; i < (int)files.size(); ++i) {
    if (us_udp_msg) {
      map<QString, QString> msging;
      msging["status"] =
          QString("processing %1").arg(QFileInfo(files[i]).fileName());
      us_udp_msg->send_json(msging);
    }

    map<QString, vector<double> > vectors;

    if (!read_sas_data(files[i], vectors["pmq"], vectors["pmi"], vectors["pme"],
                       errormsg)) {
    ...
    double pmminq =
        parameters.count("pmminq") ? parameters["pmminq"].toDouble() : 0e0;
    double pmmaxq =
        parameters.count("pmmaxq") ? parameters["pmmaxq"].toDouble() : 7e0;
    int pmqpoints =
        parameters.count("pmqpoints") ? parameters["pmqpoints"].toInt() : 5000;
    bool pmlogbin = parameters.count("pmlogbin");

    clip_data(pmminq, pmmaxq, vectors["pmq"], vectors["pmi"], vectors["pme"]);

    if (vectors["pmq"].size() < 3) {
      results["errors"] +=
          QString(
              " after q range cropping %1 had less then 3 points of data left")
              .arg(files[i]);
      continue;
    }
    ...
    if (vectors["pmq"].size() < 3) {
      results["errors"] +=
          QString(" after binning %1 had less then 3 points of data left")
              .arg(files[i]);
      continue;
    }
    ...
    if (!run_pm(produced_q, produced_I, produced_model, parameters, vectors,
                false)) {
      results["errors"] +=
          QString(" run_pm error %1: %2").arg(files[i]).arg(errormsg);
      continue;
    }
...
bool US_Saxs_Util::flush_pm_csv(vector<QString>& csv_name,
                                vector<double>& csv_q,
...
bool US_Saxs_Util::run_pm(QString controlfile) {
  ...
  if (f.open(QIODevice::WriteOnly)) {
    ...
    ;
    ts << QString(
              "ga fit calls %1 time %2 sec_per_fit %3 fit_per_sec %4 "
  ...
  if (use_tar) {
    ...
    if (!dod.exists()) {
      ...
      if (!ndod.mkdir(newdir)) {
        if (us_log) {
          us_log->log(
              QString("Warning: could not create outputData \"%1\" directory\n")
                  .arg(ndod.path()));
        }
      }
    ...
    if (dod.exists()) {
      QString dest =
          outputData + QDir::separator() + QFileInfo(results_file).fileName();
      QDir qd;
      if (us_log) {
        us_log->log(
            QString("renaming: %1 to %2\n").arg(results_file).arg(dest));
      }
...
bool US_Saxs_Util::run_pm(QStringList qsl_commands) {
  ...
  for (int i = 0; i < (int)qsl_commands.size(); i++) {
    ...
    if (rx_arg.indexIn(option) != -1 && qsl.size() < 1) {
      errormsg =
          QString("Error reading controlfile line %1 : Missing argument ")
              .arg(i + 1);
      return false;
    }
    ...
    if (option == "pmrayleighdrho") {
      ...
      if (!compute_rayleigh_structure_factors(
              pow(pow(control_parameters["pmgridsize"].toDouble(), 3e0) / M_PI,
                  1e0 / 3e0),
              control_parameters["pmrayleighdrho"].toDouble(),
              control_vectors["pmq"], control_vectors["pmf"])) {
        errormsg =
            QString("Error controlfile line %1 : %2").arg(i + 1).arg(errormsg);
        return false;
      }
    ...
    if (option == "pmapproxmaxdimension") {
      ...
      if (!pm.approx_max_dimension(
              params, control_parameters["pmbestcoarseconversion"].toDouble(),
              approx_max_d)) {
        return false;
      }
    ...
    if (option == "pmbestmd0") {
      ...
      if (control_vectors.count("pmtypes") &&
        ...
        errormsg = QString(
                       "Error controlfile line %1 : pmtypes must have exactly "
                       "one parameter for pmbestmd0")
                       .arg(i + 1);
        return false;
      }
...
#if defined(USE_MPI)
      {
        ...
        for (int i = 1; i < npes; ++i) {
          if (MPI_SUCCESS != MPI_Send(&msg, sizeof(pm_msg), MPI_CHAR, i, PM_MSG,
            ...
            if (us_log) {
              us_log->log(
                  QString("%1: MPI send failed in best_md0_ga() PM_NEW_PM\n")
                      .arg(myrank));
            }
          ...
          if (MPI_SUCCESS != MPI_Send(&(d[0]), tot_vsize * sizeof(double),
                                      MPI_CHAR, i, PM_NEW_PM, MPI_COMM_WORLD)) {
            if (us_log) {
              us_log->log(
                  QString(
                      "%1: MPI send failed in best_md0_ga() PM_NEW_PM_DATA\n")
                      .arg(myrank));
            }
            MPI_Abort(MPI_COMM_WORLD, errorno - myrank);
            exit(errorno - myrank);
...
#endif
      pm.ga_set_params(control_parameters["pmgapopulation"].toUInt(),
      ...
      pm.set_best_delta(control_parameters["pmbestdeltastart"].toDouble(),
      ...
      if (control_vectors.count("pmparams")) {
        ...
        if (control_vectors.count("pmtypes")) {
          ...
          errormsg = QString(
                         "Error controlfile line %1 : pmparams or pmtypes must "
                         "be defined")
                         .arg(i + 1);
          return false;
        }
      ...
      if (!pm.best_md0_ga(
              params, model, control_parameters["pmbestmd0stepstoga"].toUInt(),
...
#if defined(USE_MPI)
      {
        ...
        for (int i = 1; i < npes; ++i) {
          ...
          if (MPI_SUCCESS != MPI_Send(&(d[0]), tot_vsize * sizeof(double),
            ...
            if (us_log) {
              us_log->log(
                  QString("%1: MPI send failed in bestga() PM_NEW_PM_DATA\n")
                      .arg(myrank));
            }
            MPI_Abort(MPI_COMM_WORLD, errorno - myrank);
            exit(errorno - myrank);
...
bool US_Saxs_Util::run_pm_ok(QString option) {
  // required params
  {
    ...
    for (int i = 0; i < (int)params_required.size(); i++) {
      if (!control_parameters.count(params_required[i])) {
        errormsg =
            QString("missing required parameter %1").arg(params_required[i]);
        return false;
      }
    }
  }
  ...
  if (option != "pmapproxmaxdimension") {
    ...
    if (number_of_params_found > 1) {
      if (us_log) {
        us_log->log(
            "Warning: pmparams superceded pmtypes (both were defined)\n");
      }
    }
  }
...
bool US_Saxs_Util::run_pm(map<QString, vector<double> >& produced_q,
                          map<QString, vector<double> >& produced_I,
                          map<QString, QString>& produced_model,
  ...
  if (control_parameters.count("pmrayleighdrho")) {
    ...
    if (!compute_rayleigh_structure_factors(
            ...
            control_vectors["pmq"], control_vectors["pmf"])) {
      errormsg =
          QString("Error computing structure factors : %1").arg(errormsg);
      return false;
    }
    double bed = control_parameters["pmbufferedensity"].toDouble();
    if (bed) {
    ...
    if (!quiet && us_log) {
      us_log->log("Rayleigh structure factors computed\n");
      us_log->log(US_Vector::qs_vector2("pmq pmf", control_vectors["pmq"],
                                        control_vectors["pmf"]));
    }
    ...
    if (control_parameters.count("pmapproxmaxdimension")) {
      ...
      if (!run_pm_ok("pmapproxmaxdimension")) {
        errormsg = QString("Error when computing approx max dimension : %1")
                       .arg(errormsg);
        return false;
      }

      if (!quiet && us_log) {
        us_log->log(QString("uspm approxmaxdim %1")
      ...
      if (us_udp_msg) {
        map<QString, QString> msging;
        msging["status"] = QString("approximate maximum dimension of %1 is %2")
                               .arg(control_parameters.count("pmoutname")
                                        ? control_parameters["pmoutname"]
                                        : "unknown")
                               .arg(approx_max_d);
    ...
    if (control_parameters.count("pmbestmd0")) {
      ...
      if (control_vectors.count("pmtypes") &&
          control_vectors["pmtypes"].size() != 1) {
        errormsg = QString(
            "Error pmbestmd0 : pmtypes must have exactly one parameter for "
            "pmbestmd0");
        return false;
      }
      ...
      if (control_vectors.count("pmparams")) {
        ...
        if (control_vectors.count("pmtypes")) {
          ...
          errormsg =
              QString("Error bestga : pmparams or pmtypes must be defined");
          return false;
        }
      }
      if (!pm.best_md0_ga(
    ...
    if (control_parameters.count("pmbestga")) {
      ...
      if (!control_parameters.count("approx_max_d") &&
          control_parameters["approx_max_d"].toUInt() == 0) {
        errormsg = QString(
            "Error pmbestga : pmappromxmaxdimension option must be selected "
            "prior to pmbestga");
        return false;
      }
      ...
      if (!quiet && us_log) {
        us_log->log(QString("uspm bestga again %1")
                        .arg(control_parameters["pmgridsize"].toDouble()));
      }
        ...
        if (control_vectors.count("pmtypes")) {
          ...
          if (control_parameters.count("pmtypes")) {
            if (!pm.expand_types(
                ...
                !types_vector.size()) {
              errormsg =
                  QString("Error pmbestga : pmtypes string did not expand:") +
                  pm.error_msg;
              return false;
            }
          } else {
            errormsg =
                QString("Error pmbestga : pmparams or pmtypes must be defined");
            return false;
          }
        }
      }
      ...
      if (types_vector.size()) {
        ...
        for (int i = 0; i < (int)types_vector.size(); ++i) {
          ...
          if (us_udp_msg) {
            map<QString, QString> msging;
            msging["status"] = QString("start GA for %1%2")
                                   .arg(control_parameters.count("pmoutname")
                                            ? control_parameters["pmoutname"]
                                            : "unknown")
          ...
          if (us_udp_msg) {
            ...
            bool fitok =
                pm.use_errors
                    ? scaling_fit(produced_I[outname], control_vectors["pmi"],
                        ...
                        fitok
                            ? QString(" %1 %2  Params %3  Shannon channels %4")
                                  ...
                                  .arg(
                                      pm.use_errors
                                          ? chi2 /
                                                (control_vectors["pmq"].size() -
                                                 1)
                                          : chi2,
                                      0, 'g', 4)
                                  .arg(params.size())
                                  .arg(pm.last_physical_stats.count(
                                           "approx. Shannon channels")
...
bool US_Saxs_Util::run_align(map<QString, QString>& parameters,
                             map<QString, QString>& results) {
  ...
  if (frommap) {
    ...
    for (int i = 0; i < from_size; ++i) {
      ...
      if (fields["recname"].contains(rx_atomhetatm) &&
        ...
        from[i] =
            line.mid(0, 21) + frommap_chains[fields["chainid"]] + line.mid(22);
        if (!quiet) {
          TSO << "-" << line << "\n"
              << "+" << from[i] << "\n";
        }
  ...
  set<QString> atoms;  // format "ChainID:Residue:Atomname"
  {
    ...
    for (int i = 0; i < (int)level0.count(); ++i) {
      ...
      if (level1.count() != 3) {
        results["errors"] += QString(
                                 "incorrect atoms format, %1 should be "
      ...
      for (int j = 0; j < (int)level2.count(); ++j) {
        QString key =
            QString("%1:%2:%3").arg(chainid).arg(resseq).arg(level2[j]);
        atoms.insert(key);
      }
    }
  }
    ...
    for (int i = 0; i < from_size; ++i) {
      ...
      if (fields["recname"].contains(rx_atomhetatm)) {
        ...
        if (from_points.count(key)) {
          results["errors"] +=
              QString("from file %1 has duplicate chain:resseq:atom entry %2\n")
                  .arg(parameters["from"])
                  .arg(key);
          return false;
        }
    ...
    for (int i = 0; i < to_size; ++i) {
      ...
      if (fields["recname"].contains(rx_atomhetatm)) {
        ...
        if (to_points.count(key)) {
          results["errors"] +=
              QString("to file %1 has duplicate chain:resseq:atom entry %2\n")
                  .arg(parameters["to"])
                  .arg(key);
          return false;
        }
...

  {
...

    {
      int i = 0;
      for (auto it = from_points.begin(); it != from_points.end(); ++it) {
        remapped_from_points[it->first] =
            make_tuple(result[i].axis[0], result[i].axis[1], result[i].axis[2]);
        ++i;
      }
    }
  }
  ...
  if (1 || !quiet) {
    for (auto it = from_points.begin(); it != from_points.end(); ++it) {
      ...
      TSO << QString(
                 "%1 from [%2,%3,%4] remapped as [%5,%6,%7] matching to point "
                 "[%8,%9,%10] remapped dist %11\n")
                 .arg(it->first)
                 .arg(get<0>(it->second))
                 .arg(get<1>(it->second))
                 .arg(get<2>(it->second))
  ...
  if (parameters.count("dist")) {
    ...
    for (int i = 0; i < (int)level0.count(); ++i) {
      ...
      if (!atoms.count(key1)) {
        results["errors"] +=
            QString("dist atoms must be in the mapped atoms, not found %1\n")
                .arg(key1);
        keys_ok = false;
      }
      if (!atoms.count(key2)) {
      ...
      if (key1.left(1) == parameters["cut"] ||
        ...
        results["errors"] += QString(
                                 "first distance atom must be an non-cut "
                                 "chain, 2nd a cut chain. %1 %2\n")
                                 .arg(key1)
                                 .arg(key2);
        keys_ok = false;
      }
...

      {
        ...
        TSO << QString(
                   "pair %1-%2 original distance %3 transformed distance %4\n")
                   .arg(key1)
                   .arg(key2)
                   .arg(distorg, 0, 'f', 3)
                   .arg(distnew, 0, 'f', 3);
