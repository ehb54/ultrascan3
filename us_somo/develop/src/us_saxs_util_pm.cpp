...
QString
US_Saxs_Util::run_json (QString &json)
{
  // note: alternate version for mpi in in run_json_mpi() in
  // us_saxs_util_pm_mpi.cpp TSO << "run json start\n"; TSO << "parameters as
  // string\n" << json << "\n";
  ...
  if (parameters.count ("_udphost") && parameters.count ("_udpport")
      && parameters.count ("_uuid"))
    {
      map<QString, QString> msging;
      msging["_uuid"] = results["_uuid"];
      // msging[ "status" ] = results[ "_textarea" ];
      us_udp_msg = new US_Udp_Msg (parameters["_udphost"],
                                   (quint16)parameters["_udpport"].toUInt ());
...

  {
    ...
    if (count > 1)
      {
        results["errors"]
            = "only one run type currently allowed per input json";
        return US_Json::compose (results);
      }
  }
...
#if !defined(CMDLINE)
bool
US_Saxs_Util::run_hydro (map<QString, QString> &, map<QString, QString> &)
{
  return false;
}
...
bool
US_Saxs_Util::screen_pdb (QString, bool)
{
  return false;
};
bool
US_Saxs_Util::set_default (map<QString, QString> &, map<QString, QString> &)
{
  return false;
};
...
bool
US_Saxs_Util::run_pm (map<QString, QString> &parameters,
                      map<QString, QString> &results)
{
    ...
    for (int i = 0; i < (int)files_try.size (); ++i)
      {
        ...
        if (!fi.exists ())
          {
            results["errors"]
                += QString (" file %1 does not exist.").arg (files_try[i]);
          }
        else
          {
            if (!fi.isReadable ())
              {
                results["errors"]
                    += QString (" file %1 exists but is not readable.")
                           .arg (files_try[i]);
              }
            else
              {
  ...
  for (int i = 0; i < (int)files.size (); ++i)
    {
      if (us_udp_msg)
        {
          map<QString, QString> msging;
          msging["status"] = QString ("processing %1")
      ...
      if (!read_sas_data (files[i], vectors["pmq"], vectors["pmi"],
                          vectors["pme"], errormsg))
        {
          results["errors"] += " " + errormsg;
          continue;
        }

      double pmminq = parameters.count ("pmminq")
                          ? parameters["pmminq"].toDouble ()
                          : 0e0;
      double pmmaxq = parameters.count ("pmmaxq")
                          ? parameters["pmmaxq"].toDouble ()
                          : 7e0;
      int pmqpoints = parameters.count ("pmqpoints")
                          ? parameters["pmqpoints"].toInt ()
                          : 5000;
      bool pmlogbin = parameters.count ("pmlogbin");

      clip_data (pmminq, pmmaxq, vectors["pmq"], vectors["pmi"],
                 vectors["pme"]);

      if (vectors["pmq"].size () < 3)
        {
          results["errors"] += QString (" after q range cropping %1 had less "
                                        "then 3 points of data left")
                                   .arg (files[i]);
          continue;
        }
      ...
      bin_data (pmqpoints, pmlogbin, vectors["pmq"], vectors["pmi"],
                vectors["pme"], errormsg, noticemsg);
      ...
      if (vectors["pmq"].size () < 3)
        {
          results["errors"]
              += QString (
                     " after binning %1 had less then 3 points of data left")
                     .arg (files[i]);
          continue;
        }
      ...
      if (!run_pm (produced_q, produced_I, produced_model, parameters, vectors,
                   false))
        {
          results["errors"] += QString (" run_pm error %1: %2")
                                   .arg (files[i])
                                   .arg (errormsg);
          continue;
        }
      ...
      for (map<QString, vector<double> >::iterator it = produced_q.begin ();
           it != produced_q.end (); ++it)
        {
          ...
          if (!US_File_Util::writeuniq (name, msg, it->first, "bead_model",
                                        produced_model[it->first]))
            {
              results["errors"] += QString (" write error: %1").arg (msg);
            }
          else
      ...
      if (us_udp_msg)
        {
          map<QString, QString> msging;
          msging["status"]
              = QString ("finished %1").arg (QFileInfo (files[i]).fileName ());
  ...
  if (results.count ("errors") && !results["errors"].isEmpty ())
    {
      return false;
    }
  return true;
...
bool
US_Saxs_Util::flush_pm_csv (vector<QString> &csv_name, vector<double> &csv_q,
...
bool
US_Saxs_Util::run_pm (QString controlfile)
{
  // for now, everyone reads the control file & sets things up to the point of
  // nsa run
  ...
  if (qd1 != qd2)
    {
      ufu.copy (controlfile, QDir::currentPath () + QDir::separator ()
                                 + QFileInfo (controlfile).fileName ());
      if (us_log)
        {
          us_log->log (QString ("copying %1 %2 <%3>\n")
                           .arg (controlfile)
  ...
  if (controlfile.contains (QRegExp ("\\.(tar|TAR)$")))
    {
      ...
      if (TAR_OK != result)
        {
          errormsg
              = QString ("Error: %1 problem extracting tar archive (%2)\n")
                    .arg (controlfile)
  ...
  if (f.open (QIODevice::WriteOnly))
    {
      ...
      ;
      ts << QString ("ga fit calls %1 time %2 sec_per_fit %3 fit_per_sec %4 "
                     "fit_per_sec_per_worker_proc %5\n")
                .arg (pm_ga_fitness_calls)
                .arg (pm_ga_fitness_secs)
                .arg (pm_ga_fitness_calls != 0
                          ? pm_ga_fitness_secs / (double)pm_ga_fitness_calls
                          : 0e0)
                .arg (pm_ga_fitness_secs != 0e0
                          ? (double)pm_ga_fitness_calls / pm_ga_fitness_secs
                          : 0e0)
#if defined(USE_MPI)
                .arg ((pm_ga_fitness_secs != 0e0
                           ? (double)pm_ga_fitness_calls / pm_ga_fitness_secs
      ...
      ts << QString ("ga % fitness %1\n")
                .arg (100e0 * pm_ga_fitness_secs
                      / ((double)usupm_timer.times["pm_run"] / 1000e0));
      ts << "end-timings\n";
  ...
  if (use_tar)
    {
          ...
          if (!ndod.mkdir (newdir))
            {
              if (us_log)
                {
                  us_log->log (QString ("Warning: could not create outputData "
                                        "\"%1\" directory\n")
                                   .arg (ndod.path ()));
                }
            }
      ...
      if (dod.exists ())
        {
          QString dest = outputData + QDir::separator ()
                         + QFileInfo (results_file).fileName ();
          QDir qd;
          if (us_log)
            {
              ...
              if (us_log)
                {
                  us_log->log (
                      QString (
                          "Warning: could not rename outputData %1 to %2\n")
                          .arg (results_file)
                          .arg (dest));
                }
            }
      ...
      else
        {
          if (us_log)
            {
              us_log->log (
                  QString ("Error: %1 does not exist\n").arg (outputData));
            }
        }
    }
...
bool
US_Saxs_Util::run_pm (QStringList qsl_commands)
{
  output_files.clear ();
  ...
  for (int i = 0; i < (int)qsl_commands.size (); i++)
    {
      ...
      if (rx_valid.indexIn (option) == -1)
        {
          errormsg
              = QString ("Error controlfile line %1 : Unrecognized token %2")
                    .arg (i + 1)
                    .arg (option);
          return false;
        }

      if (rx_arg.indexIn (option) != -1 && qsl.size () < 1)
        {
          errormsg
              = QString (
                    "Error reading controlfile line %1 : Missing argument ")
                    .arg (i + 1);
          return false;
        }
      ...
      if (rx_vector.indexIn (option) != -1)
        {
          ...
          if (us_log)
            {
              us_log->log (
                  US_Vector::qs_vector (option, control_vectors[option]));
            }
        }

      if (option == "pmq" && make_csv && csv_name.size ())
        {
          if (!flush_pm_csv (csv_name, csv_q, csv_I))
            {
              return false;
            }
        }
      ...
      if (option == "pmrayleighdrho")
        {
          ...
          if (!control_parameters.count ("pmbufferedensity"))
            {
              errormsg
                  = "pmbufferedensity must be defined before pmrayleighdro";
              return false;
            }

          if (!compute_rayleigh_structure_factors (
                  pow (pow (control_parameters["pmgridsize"].toDouble (), 3e0)
                           / M_PI,
                       1e0 / 3e0),
                  control_parameters["pmrayleighdrho"].toDouble (),
                  control_vectors["pmq"], control_vectors["pmf"]))
            {
              errormsg = QString ("Error controlfile line %1 : %2")
                             .arg (i + 1)
                             .arg (errormsg);
              return false;
            }
          ...
          if (bed)
            {
              double vi
                  = pow (control_parameters["pmgridsize"].toDouble (), 3e0);
              double vie = vi * bed;
          ...
          // US_Vector::printvector2( "pmq pmf", control_vectors[ "pmq" ],
          // control_vectors[ "pmf" ] );
          continue;
        }
      ...
      if (option == "pmapproxmaxdimension")
        {
          ...
          control_parameters["approx_max_d"]
              = QString ("%1").arg (approx_max_d);
          if (us_log)
            {
      ...
      if (option == "pmbestmd0")
        {
          ...
          if (control_vectors.count ("pmtypes")
            ...
            {
              errormsg = QString ("Error controlfile line %1 : pmtypes must "
                                  "have exactly one parameter for pmbestmd0")
                             .arg (i + 1);
              return false;
            }
              ...
              if (!srand48_done)
                {
                  ...
                  if (us_log)
                    {
                      us_log->log (
                          QString ("to reproduce use random seed %1\n")
                              .arg (li));
                    }
                  srand48 (li);
                }
...
#if defined(USE_MPI)
          {
            ...
            msg.grid_conversion_factor
                = control_parameters["pmgridsize"].toDouble ();
            msg.max_dimension
                = (uint32_t)control_parameters["pmmaxdimension"].toUInt ();
            msg.max_harmonics
                = (uint32_t)control_parameters["pmharmonics"].toUInt ();
            msg.max_mem_in_MB
                = (uint32_t)control_parameters["pmmemory"].toUInt ();
            ...
            for (int i = 1; i < npes; ++i)
              {
                if (MPI_SUCCESS
                    != MPI_Send (&msg, sizeof (pm_msg), MPI_CHAR, i, PM_MSG,
                                 MPI_COMM_WORLD))
                  {
                    if (us_log)
                      {
                        us_log->log (QString ("%1: MPI send failed in "
                                              "best_md0_ga() PM_NEW_PM\n")
                                         .arg (myrank));
                      }
                ...
                if (MPI_SUCCESS
                    != MPI_Send (&(d[0]), tot_vsize * sizeof (double),
                                 MPI_CHAR, i, PM_NEW_PM, MPI_COMM_WORLD))
                  {
                    if (us_log)
                      {
                        us_log->log (QString ("%1: MPI send failed in "
                                              "best_md0_ga() PM_NEW_PM_DATA\n")
                                         .arg (myrank));
                      }
                    MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
                    exit (errorno - myrank);
          ...
          if (control_vectors.count ("pmparams"))
          ...
          else
            {
              if (control_vectors.count ("pmtypes"))
                {
                  vector<int> types;
                  for (int i = 0; i < (int)control_vectors["pmtypes"].size ();
                       i++)
                    {
                ...
                {
                  errormsg = QString ("Error controlfile line %1 : pmparams "
                                      "or pmtypes must be defined")
                                 .arg (i + 1);
                  return false;
                }
            }
      ...
      if (option == "pmbestga")
        {
          ...
          if (!control_parameters.count ("approx_max_d")
              && control_parameters["approx_max_d"].toUInt () == 0)
            {
              errormsg
                  = QString (
                        "Error controlfile line %1 : pmappromxmaxdimension "
                        "option must be selected prior to pmbestga")
                        .arg (i + 1);
              return false;
            }
                    ...
                    if (us_log)
                      {
                        us_log->log (
                            QString (
                                "%1: MPI send failed in bestga() PM_NEW_PM\n")
                                .arg (myrank));
                      }
                    MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
                    exit (errorno - myrank);
          ...
          if (!pm.best_ga (
                  params, model, control_parameters["pmgapointsmax"].toUInt (),
                  control_parameters["pmbestfinestconversion"].toDouble (),
                  control_parameters["pmbestcoarseconversion"].toDouble (),
                  control_parameters["pmbestrefinementrangepct"].toDouble (),
                  control_parameters["pmbestconversiondivisor"].toDouble ()))
...
bool
US_Saxs_Util::run_pm_ok (QString option)
{
  // required params
  {
    QStringList params_required;
    params_required << "pmgridsize";

    for (int i = 0; i < (int)params_required.size (); i++)
      {
        if (!control_parameters.count (params_required[i]))
          {
            errormsg = QString ("missing required parameter %1")
                           .arg (params_required[i]);
            return false;
          }
      }
  }
  ...
  // defaultable params
  {
    ...
    if (option == "pmbestmd0")
      {
        default_params << "pmbestmd0stepstoga";
      }
    ...
    for (int i = 0; i < (int)default_params.size (); i++)
      {
        if (!control_parameters.count (default_params[i]))
          {
            control_parameters[default_params[i]]
                = defaults[default_params[i]];
            if (us_log)
              {
                us_log->log (
                    QString ("Notice: setting '%1' to default value of %2\n")
    ...
    for (int i = 0; i < (int)qslv_required.size (); i++)
      {
        if (!control_vectors.count (qslv_required[i]))
          {
            errormsg = QString ("missing required vector %1")
                           .arg (qslv_required[i]);
            return false;
          }
        if (!i)
        ...
        else
          {
            if (vector_size != (int)control_vectors[qslv_required[i]].size ())
              {
                errormsg
                    = QString ("required vector %1 incompatible size %2 vs %3")
                          .arg (qslv_required[i])
    ...
    for (int i = 0; i < (int)qslv_optional.size (); i++)
      {
        if (control_vectors.count (qslv_optional[i])
          ...
          {
            errormsg
                = QString ("optional vector %1 incompatible size %2 vs %3")
                      .arg (qslv_optional[i])
  ...
  if (option != "pmapproxmaxdimension")
    {
      ...
      if (!number_of_params_found && !control_parameters.count ("pmtypes"))
        {
          errormsg = QString ("missing required vector one of: %1")
                         .arg (qslv_required.join (","));
          return false;
        }
      if (number_of_params_found > 1)
        {
          if (us_log)
            {
              us_log->log ("Warning: pmparams superceded pmtypes (both were "
                           "defined)\n");
            }
        }
    }
  ...
  if (us_log)
    {
      for (map<QString, QString>::iterator it = control_parameters.begin ();
           it != control_parameters.end (); it++)
        {
          us_log->log (QString ("\"%1\"\t%2\t%3\n")
                           .arg (it->first)
                           .arg (it->second)
                           .arg (it->second.toDouble ()));
        }
    }
...
bool
US_Saxs_Util::run_pm (map<QString, vector<double> > &produced_q,
                      map<QString, vector<double> > &produced_I,
                      map<QString, QString> &produced_model,
                      map<QString, QString> &parameters,
                      map<QString, vector<double> > &vectors, bool quiet)
{
  output_files.clear ();
  job_output_files.clear ();
  write_output_count = 0;
  ...
  if (control_parameters.count ("pmrayleighdrho"))
    {
      ...
      if (!compute_rayleigh_structure_factors (
              ...
              control_vectors["pmq"], control_vectors["pmf"]))
        {
          errormsg = QString ("Error computing structure factors : %1")
                         .arg (errormsg);
          return false;
        }
      double bed = control_parameters["pmbufferedensity"].toDouble ();
      ...
      if (!quiet && us_log)
        {
          ...
          us_log->log (US_Vector::qs_vector2 (
              "pmq pmf", control_vectors["pmq"], control_vectors["pmf"]));
        }
      ...
      if (control_parameters.count ("pmapproxmaxdimension"))
        {
          ...
          if (!run_pm_ok ("pmapproxmaxdimension"))
            {
              errormsg
                  = QString ("Error when computing approx max dimension : %1")
                        .arg (errormsg);
              return false;
            }
          ...
          if (!quiet && us_log)
            {
              us_log->log (
                  QString ("uspm approxmaxdim %1")
                      .arg (control_parameters["pmgridsize"].toDouble ()));
            }
          ...
          if (us_udp_msg)
            {
              ...
              msging["status"]
                  = QString ("computing approximate maximum dimension of %1")
                        .arg (control_parameters.count ("pmoutname")
                                  ? control_parameters["pmoutname"]
                                  : "unknown");
              us_udp_msg->send_json (msging);
            }
          US_PM pm (control_parameters["pmgridsize"].toDouble (),
      ...
      if (control_parameters.count ("pmbestmd0"))
        {
          ...
          if (control_vectors.count ("pmtypes")
              && control_vectors["pmtypes"].size () != 1)
            {
              errormsg = QString ("Error pmbestmd0 : pmtypes must have "
                                  "exactly one parameter for pmbestmd0");
              return false;
            }
          ...
          us_qdebug (QString ("uspm bestmd0 %1")
                         .arg (control_parameters["pmgridsize"].toDouble ()));
          ...
          if (control_vectors.count ("pmparams"))
          ...
          else
            {
              if (control_vectors.count ("pmtypes"))
                ...
                {
                  errormsg = QString (
                      "Error bestga : pmparams or pmtypes must be defined");
                  return false;
                }
            }
          if (!pm.best_md0_ga (
          ...
          QString outname
              = control_parameters["pmoutname"] + pm.get_name (types);

          int ext = 0;
          while (produced_I.count (outname))
            {
              outname = control_parameters["pmoutname"] + pm.get_name (types)
                        + QString ("-%1").arg (++ext);
            }
      ...
      if (control_parameters.count ("pmbestga"))
        {
          ...
          if (!control_parameters.count ("approx_max_d")
              && control_parameters["approx_max_d"].toUInt () == 0)
            {
              errormsg = QString ("Error pmbestga : pmappromxmaxdimension "
                                  "option must be selected prior to pmbestga");
              return false;
            }
              ...
              if (control_vectors.count ("pmtypes"))
              ...
              else
                {
                  if (control_parameters.count ("pmtypes"))
                    {
                      if (!pm.expand_types (
                              types_vector, control_parameters["pmtypes"],
                              (control_parameters.count ("pmincrementally")
                               && control_parameters["pmincrementally"]
                                      == "on"),
                              (control_parameters.count ("pmallcombinations")
                               && control_parameters["pmallcombinations"]
                                      == "on"))
                          || !types_vector.size ())
                        {
                          errormsg = QString ("Error pmbestga : pmtypes "
                                              "string did not expand:")
                                     + pm.error_msg;
                          return false;
                        }
                    }
                    ...
                    {
                      errormsg = QString ("Error pmbestga : pmparams or "
                                          "pmtypes must be defined");
                      return false;
                    }
                }
            }
          ...
          if (types_vector.size ())
            {
              if (!quiet && us_log)
                {
                  us_log->log (
                      QString ("total types %1").arg (types_vector.size ()));
                }
              for (int i = 0; i < (int)types_vector.size (); ++i)
                {
                  ...
                  if (!pm.best_ga (
                          params, model,
                          control_parameters["pmgapointsmax"].toUInt (),
                          control_parameters["pmbestfinestconversion"]
                              .toDouble (),
                          control_parameters["pmbestcoarseconversion"]
                              .toDouble (),
                          control_parameters["pmbestrefinementrangepct"]
                              .toDouble (),
                          control_parameters["pmbestconversiondivisor"]
                              .toDouble ()))
                    {
                      errormsg = "Error:" + pm.error_msg;
                      return false;
                  ...
                  while (produced_I.count (outname))
                    {
                      outname = control_parameters["pmoutname"]
                                + pm.get_name (types)
                                + QString ("-%1").arg (++ext);
                    }
                  ...
                  if (!pm.qstring_model (produced_model[outname], model,
                                         params))
                    {
                      errormsg
                          = QString ("Error producing model %1").arg (outname);
                      return false;
                    }
                  ...
                  if (us_udp_msg)
                    {
                      ...
                      bool fitok
                          = pm.use_errors
                                ? scaling_fit (produced_I[outname],
                                               control_vectors["pmi"],
                                               control_vectors["pme"], a, chi2)
                                : scaling_fit (produced_I[outname],
                                               control_vectors["pmi"], a,
                                               chi2);
                      ...
                      msging["status"]
                          = QString ("end GA for %1%2 model%3")
                                ...
                                .arg (
                                    fitok
                                        ? QString (" %1 %2  Params %3  "
                                                   "Shannon channels %4")
                                              .arg (pm.use_errors ? "nchi"
                                                                  : "rmsd")
                                              .arg (
                                                  pm.use_errors
                                                      ? chi2
...
static point
tuple_point (tuple<double, double, double> t)
{
  point pt;
  pt.axis[0] = get<0> (t);
  pt.axis[1] = get<1> (t);
...
bool
US_Saxs_Util::run_align (map<QString, QString> &parameters,
                         map<QString, QString> &results)
{
  ...
  bool adjcoords = parameters.count ("adjcoords") != 0
                   && parameters["adjcoords"].toInt () != 0;
      ...
      for (int i = 0; i < (int)level0.count (); ++i)
        {
          ...
          if (level1.count () != 2)
            {
              results["errors"]
                  += "frommap format must be orgchainID:newchainID{;..}\n";
              return false;
            }
      ...
      for (int i = 0; i < from_size; ++i)
        {
          ...
          if (fields["recname"].contains (rx_atomhetatm)
            ...
            {
              from[i] = line.mid (0, 21) + frommap_chains[fields["chainid"]]
                        + line.mid (22);
              if (!quiet)
                {
                  TSO << "-" << line << "\n"
    ...
    for (int i = 0; i < (int)level0.count (); ++i)
      {
        ...
        if (level1.count () != 3)
          {
            results["errors"]
                += QString ("incorrect atoms format, %1 should be "
                            "chainid:resseq:atomname{,atomname,...}\n")
                       .arg (level0[i]);
            continue;
          }
  ...
  if (save)
    {
      ...
      if (save_parts.count () != 2)
        {
          results["errors"]
              += "save format must be ChainID:startresseq-endresseq\n";
          return false;
        }
        ...
        if (fields["recname"].contains (rx_atomhetatm))
          {
            ...
            if (from_points.count (key))
              {
                results["errors"] += QString ("from file %1 has duplicate "
                                              "chain:resseq:atom entry %2\n")
                                         .arg (parameters["from"])
                                         .arg (key);
                return false;
              }
            from_points[key]
                = make_tuple (fields["x"].toDouble (), fields["y"].toDouble (),
                              fields["z"].toDouble ());
          }
      }
        ...
        if (fields["recname"].contains (rx_atomhetatm))
          {
            QString key = QString ("%1:%2:%3")
                              ...
                              .arg (fields["resseq"])
                              .arg (fields["name"]);
            if (save && fields["chainid"] == save_chainid
            ...
            if (to_points.count (key))
              {
                results["errors"] += QString ("to file %1 has duplicate "
                                              "chain:resseq:atom entry %2\n")
                                         .arg (parameters["to"])
                                         .arg (key);
                return false;
  ...
  for (auto it = atoms.begin (); it != atoms.end (); ++it)
    {
      if (!from_points.count (*it))
        {
          results["errors"]
              += QString ("from file %1 does not specifiy atom %2\n")
                     .arg (parameters["from"])
                     .arg (*it);
        }
      if (!to_points.count (*it))
        {
          results["errors"]
              += QString ("to file %1 does not specifiy atom %2\n")
                     .arg (parameters["to"])
                     .arg (*it);
        }
    }
  ...
  if (adjcoords)
    {
      TSO << "adjusting coordinates to positive values active\n";
      {
        ...
        for (auto it = from_points.begin (); it != from_points.end (); ++it)
          {
      ...
      }
      {
        ...
        for (auto it = to_points.begin (); it != to_points.end (); ++it)
          {
            if (min_x > get<0> (it->second))
              {
                min_x = get<0> (it->second);
  ...
  if (!quiet)
    {
      for (auto it = atoms.begin (); it != atoms.end (); ++it)
        {
          TSO << QString ("%1 from [%2,%3,%4] to [%5,%6,%7]\n")
                     .arg (*it)
...

  {
    ...
    if (!atom_align (transform_from, transform_to, apply_from, result))
      {
        results["errors"] += errormsg;
        return false;
      }
      ...
      for (auto it = from_points.begin (); it != from_points.end (); ++it)
        {
          remapped_from_points[it->first] = make_tuple (
              result[i].axis[0], result[i].axis[1], result[i].axis[2]);
          ++i;
        }
    }
  }
      ...
      for (auto it = from_points.begin (); it != from_points.end (); ++it)
        {
          ...
          TSO << QString ("%1 from [%2,%3,%4] remapped as [%5,%6,%7] matching "
                          "to point [%8,%9,%10] remapped dist %11\n")
                     .arg (it->first)
                     .arg (get<0> (it->second))
                     .arg (get<1> (it->second))
                     .arg (get<2> (it->second))
  ...
  if (adjcoords)
    {
      for (auto it = remapped_from_points.begin ();
           it != remapped_from_points.end (); ++it)
        {
      ...
      for (int i = 0; i < (int)level0.count (); ++i)
        {
          ...
          if (level1.count () != 2)
            {
              results["errors"]
                  += QString (
                         "incorrect dist format, %1 should be "
                         "chainid:resseq:atomname-chainid:resseq:atomname\n")
                         .arg (level0[i]);
              continue;
            }
          ...
          if (!atoms.count (key1))
            {
              results["errors"] += QString ("dist atoms must be in the mapped "
                                            "atoms, not found %1\n")
                                       .arg (key1);
              keys_ok = false;
            }
          ...
          if (key1.left (1) == parameters["cut"]
            ...
            {
              results["errors"]
                  += QString ("first distance atom must be an non-cut chain, "
                              "2nd a cut chain. %1 %2\n")
                         .arg (key1)
                         .arg (key2);
              keys_ok = false;
          ...
          {
            float distorg = dist (tuple_point (from_points[key1]),
                                  tuple_point (from_points[key2]));
            float distnew = dist (tuple_point (to_points[key1]),
                                  tuple_point (remapped_from_points[key2]));

            TSO << QString ("pair %1-%2 original distance %3 transformed "
                            "distance %4\n")
                       .arg (key1)
                       .arg (key2)
                       .arg (distorg, 0, 'f', 3)
