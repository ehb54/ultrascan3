...
void
US_PM::set_best_delta (double best_delta_start, double best_delta_divisor,
  ...
  if (this->best_delta_min < USPM_BEST_DELTA_MIN)
    {
      ...
      if (us_log)
        {
          us_log->log (QString ("Notice: best delta mininimum requested %1 is "
                                "below hard limit %2, reset to %3\n")
                           .arg (best_delta_min)
...
bool
US_PM::best_vary_one_param (unsigned int param_to_vary, vector<double> &params,
  ...
  while (delta >= best_delta_min)
    {
      ...
      if (us_log)
        {
          us_log->log (QString ("best_vary_one_param: ------------ this limit "
                                "%1 %2 delta %3\n")
                           .arg (low_limit)
                           .arg (high_limit)
                           .arg (delta));
      ...
      for (params[param_to_vary] = low_limit;
           params[param_to_vary] <= high_limit; params[param_to_vary] += delta)
        {
          bool skip = false;
          ...
          if (!skip)
            {
              ...
              //                .arg( this_model.size() )
              //                ;
              //             for ( int i = 1; i <= (int)object_m0_parameters[
              //             (int) params[ 0 ] ]; ++i )
              //             {
              ...
              if (us_log)
                {
                  us_log->log (US_Vector::qs_vector (
                      QString ("best_vary_one_param: %1beads %2 fitness %3, "
                               "params:")
            ...
            {
              //             QString qs =
              //                QString( "skipping %1 (empty or identical to
              //                previous) beads %2 prev %3 params:" ) .arg(
              //                object_names[ (int) params[ 0 ] ] ) .arg(
              ...
              if (steps_without_change > 100)
                {
                  if (us_log)
                    {
                      us_log->log (
                          "best_vary_one_param: Too many steps without model "
                          "change, terminating inner loop\n");
                    }
                  break;
                }
          ...
          if (best_size != -1e99)
            {
              if (us_log)
                {
                  us_log->log ("best_vary_one_param: found nothing to "
                               "recenter on, centering on best +/- delta\n");
                }
          ...
          else
            {
              if (us_log)
                {
                  us_log->log ("best_vary_one_param: found nothing to "
                               "recenter on, terminating outer loop\n");
                }
              break;
            }
        }
      ...
      if (best_size == high_limit)
        {
          if (us_log)
            {
              us_log->log ("best_vary_one_param: best found at end, extending "
                           "interval by 5 delta\n");
            }
...
bool
US_PM::best_vary_two_param (unsigned int param_to_vary_1,
  ...
  while (delta > best_delta_min)
    {
      ...
      if (us_log)
        {
          us_log->log (QString ("best_vary_two_param: ------------ this limit "
                                "%1 %2 delta %3\n")
                           .arg (low_limit)
                           .arg (high_limit)
                           .arg (delta));
          ...
          if (us_log)
            {
              us_log->log (US_Vector::qs_vector (
                  QString (
                      "best_vary_two_param: %1beads %2 fitness %3, params:")
                      .arg (this_fit < prev_fit ? "**" : "  ")
                      .arg (this_model.size ())
                      .arg (this_fit),
                  params));
      ...
      if (last_fitness_3_pos < 0e0 || last_fitness_1_pos < 0e0)
        {
          if (us_log)
            {
              us_log->log ("best_vary_two_param: found nothing to recenter "
                           "on, terminating outer loop\n");
            }
          break;
        }
      low_limit = last_fitness_3_pos;
...
bool
US_PM::best_md0 (vector<double> &params, set<pm_point> &model,
            ...
            if (us_log)
              {
                us_log->log (
                    QString (
                        "%1: MPI send failed in best_ga() PM_NEW_GRID_SIZE\n")
                        .arg (myrank));
              }
  ...
  while (grid_conversion_factor >= finest_conversion)
    {
      ...
      if (us_log)
        {
          ...
          us_log->log (US_Vector::qs_vector2 (
              "param limits before clipping:", next_low_fparams,
              next_high_fparams));
        }
      ...
      if (us_log)
        {
          us_log->log (US_Vector::qs_vector2 ("param limits:", low_fparams,
                                              high_fparams));
        }
      ...
      if (us_log)
        {
          ...
          us_log->log (US_Vector::qs_vector2 (
              "new limits after rescaling:", next_low_fparams,
              next_high_fparams));
        }
        ...
        for (int i = 1; i < npes; ++i)
          {
            if (MPI_SUCCESS
                             ...
                             MPI_COMM_WORLD))
              {
                if (us_log)
                  {
                    us_log->log (QString ("%1: MPI send failed in best_ga() "
                                          "PM_NEW_GRID_SIZE\n")
                                     .arg (myrank));
                  }
                MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
  ...
  if (high_limit - low_limit <= best_delta_min)
    {
      if (us_log)
        {
          us_log->log (
              QString ("best_vary_one_param: parameter range %1:%2 is smaller "
                       "than minimum delta %3, returning average model\n")
                  .arg (low_limit)
                  .arg (high_limit)
                  .arg (best_delta_min));
        }
  ...
  if (high_limit - low_limit <= best_delta_min)
    {
      if (us_log)
        {
          us_log->log (
              QString ("best_vary_two_param: parameter range %1:%2 is smaller "
                       "than minimum delta %3, returning average model\n")
                  .arg (low_limit)
                  .arg (high_limit)
                  .arg (best_delta_min));
  ...
  while (delta > best_delta_min)
    {
      ...
      for (params[param_to_vary_2] = low_limit;
           ...
           params[param_to_vary_2] += delta)
        {
          best_vary_one_param (param_to_vary_1, params, low_fparams,
                               high_fparams, this_model, this_fit);
          fitnesses[params[param_to_vary_2]] = this_fit;
...
bool
US_PM::grid_search (vector<double> &params, vector<double> &delta,
                    ...
                    set<pm_point> &model)
{
  ...
  if (us_log)
    {
      us_log->log (US_Vector::qs_vector2 (
          "grid_search: low, high: ", low_fparams, high_fparams));
...
bool
US_PM::best_md0_ga (vector<double> &params, set<pm_point> &model,
  ...
  for (; grid_conversion_factor >= finest_conversion; ++steps)
    {
      ...
      //       ); rescale_params( types, high_fparams,
...
bool
US_PM::approx_max_dimension (vector<double> &params, double coarse_conversion,
...
bool
US_PM::best_ga (vector<double> &params, set<pm_point> &model,
  ...
  for (; grid_conversion_factor >= finest_conversion; ++steps)
    {
      ...
      {
        QString oname
            = QString ("/root/ultrascan/somo/saxs/tmp/last_best_gc_%1")
                  .arg (grid_conversion_factor)
                  .replace (".", "_");
        write_model (oname, model, params);
        write_I (oname, model);
          ...
          if (ga_sequential_pegged > USPM_SEQ_PEGGED_LIMIT)
            {
              if (us_log)
                {
                  us_log->log (
                      QString ("Warning: sequential pegged limit %1 reached\n")
