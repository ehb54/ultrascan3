...
void
US_Hydrodyn::gui_script_msg (int line, QString cmd, QString msg)
{
...
void
US_Hydrodyn::gui_script_error (int line, QString cmd, QString msg, bool doexit)
{
  ...
  for (int i = 0; i < scriptlinesc; ++i)
    {
      ...
      if (l.contains (rx_comment) || l.contains (rx_empty))
        {
          continue;
        }
      ...
      if (cmd == "exit")
      ...
      else if (cmd == "config")
        {
          ...
          if (read_config (opt1))
            {
              gui_script_error (
                  i, cmd, QString ("config file '%1' failed").arg (opt1));
            }
        }
      ...
      else if (cmd == "threads")
        {
          ...
          gui_script_msg (i, cmd,
                          QString ("Threads now %1 write config result %2\n")
                              .arg (opt1)
          ...
          if (opt1 == "overwrite")
          ...
          else if (opt1 == "saveloadstats")
            {
              ...
              if (!model_summary_csv (opt2))
                {
                  gui_script_error (i, cmd + " " + opt1 + " " + opt2,
                                    "failed");
                }
