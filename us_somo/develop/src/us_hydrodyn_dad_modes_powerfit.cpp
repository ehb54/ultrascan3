...
void US_Hydrodyn_Dad::powerfit_corrected_lambda_create(
...
void US_Hydrodyn_Dad::powerfit(bool /* no_store_original */) {
  ...
  if (names.size() != 1) {
    QMessageBox::critical(this, windowTitle() + us_tr(": Powerfit"),
                          QString(us_tr("%1 curves selected, Powerfit requires "
                                        "exactly one curve selected\n"))
                              .arg(names.size()));
  ...
  if (!powerfit_use_errors &&
      ((US_Eigen::weight_methods)cb_powerfit_fit_alg_weight->currentData()
               .toInt() == US_Eigen::EIGEN_1_OVER_SD ||
       (US_Eigen::weight_methods)cb_powerfit_fit_alg_weight->currentData()
               .toInt() == US_Eigen::EIGEN_1_OVER_SD_SQ)) {
  ...
  powerfit_add_marker(
      plot_dist, le_powerfit_lambda2->text().toDouble(), Qt::magenta,
      UNICODE_LAMBDA_QS + "(scat)",
      Qt::AlignRight | Qt::AlignBaseline);  // Qt::AlignVCenter );
  ...
  if (le_powerfit_q_end->text().isEmpty() ||
      ...
      le_powerfit_q_end->text().toDouble() <=
          powerfit_q_min + 0.001  // == caused by the slot, +0.001 for rounding
  ) {
...
void US_Hydrodyn_Dad::powerfit_enables() {
  ...
  pb_powerfit_reset->setEnabled(le_powerfit_a->text() != "0" ||
...
void US_Hydrodyn_Dad::powerfit_add_marker(QwtPlot *plot, double pos,
...
void US_Hydrodyn_Dad::powerfit_q_start_text(const QString &text) {
  ...
  if (powerfit_markers.size() != 4) {
    editor_msg(
        "red",
        QString("internal error: powerfit_q_start_text markers issue size %1")
...
void US_Hydrodyn_Dad::powerfit_uncorrected_conc_text(
    const QString & /* text */) {
...
#warning qwtw_wheel probably should be in ::disable_all() but for some reason we commented it out there, probably should check all wheel usage and set as appropriate
...

  {
    ...
    LM::lmcurve_fit_rmsd((int)par.size(), (double *)&(par[0]), (int)q.size(),
                         (double *)&(q[0]), (double *)&(I[0]),
...

  {
    ...
    if (powerfit_use_errors && chi2 != 0) {
      msg +=
          QString(" Chi^2 %2 nChi^2 %3").arg(chi2).arg(chi2 / (double)q.size());
    }
...
vector<double> US_Hydrodyn_Dad::powerfit_curve(const vector<double> &x,
...
vector<double> US_Hydrodyn_Dad::curve_trim(const vector<double> x, double x_min,
                                           double x_max) {
...
void US_Hydrodyn_Dad::powerfit_compute_conc() {
...

  {
    ...
    powerfit_uncorrected_conc =
        QString("%1").arg(powerfit_uncorrected_conc, 0, 'f', 3).toDouble();
    le_powerfit_uncorrected_conc->setText(
    ...
    // compute scat corrected
    {
      ...
      if (dad_param_lambda2 < x.front() || dad_param_lambda > x.back()) {
        ...
        if (!usu->linear_interpolate(x, y, target, y2)) {
          ...
          powerfit_scat_conc =
              (abs_at_lambda - abs_at_lambda2) / dad_param_g_extinction_coef;
          powerfit_scat_conc =
              QString("%1").arg(powerfit_scat_conc, 0, 'f', 3).toDouble();
          le_powerfit_scat_conc->setText(
...
void US_Hydrodyn_Dad::powerfit_set_b_default() {
  ...
  powerfit_b_default = QString("%1").arg(
      1e0 / fabs(pow(q_start, -le_powerfit_c_min->text().toDouble()) / val));
