...
void
US_Hydrodyn_Dad::powerfit_corrected_lambda_create (const vector<double> &x)
{
  ...
  for (const auto &v : x)
    {
      double dndc2_correction
          = dad_param_dndc2_a + dad_param_dndc2_b * pow (v, dad_param_dndc2_c);
      double n2_correction
          = dad_param_n2_a + dad_param_n2_b * pow (v, dad_param_n2_c);

      powerfit_corrected_lambda[v]
          = v * pow (dndc2_correction * n2_correction, -1);

      TSO << QString ("lambda %1 --> %2\n")
                 .arg (v)
                 .arg (powerfit_corrected_lambda[v]);

      // TSO <<
      //    QString( "lambda %1 corrected %2 (dndc factor %3, n2 factor %4)\n"
      //    ) .arg( v ) .arg( v * powerfit_corrected_lambda[ v ] ) .arg(
      //    dndc2_correction ) .arg( n2_correction )
      //    ;
    }
}

void
US_Hydrodyn_Dad::powerfit (bool /* no_store_original */)
{
  TSO << "powerfit()\n";
  ...
  if (names.size () != 1)
    {
      QMessageBox::critical (
          this, windowTitle () + us_tr (": Powerfit"),
          QString (us_tr ("%1 curves selected, Powerfit requires exactly one "
                          "curve selected\n"))
              .arg (names.size ()));
  ...
  for (size_t i = 0; i < f_qs[powerfit_name].size (); ++i)
    {
      powerfit_org_I_at_lambda
          [QString ("%1").arg (f_qs[powerfit_name][i]).toDouble ()]
          = f_Is[powerfit_name][i];
    }

  // using errors?

  powerfit_use_errors
      = f_errors.count (powerfit_name) && f_qs.count (powerfit_name)
        && f_errors[powerfit_name].size () == f_qs[powerfit_name].size ()
        && !is_zero_vector (f_errors[powerfit_name]);

  if (!powerfit_use_errors
      && ((US_Eigen::weight_methods)cb_powerfit_fit_alg_weight->currentData ()
                  .toInt ()
              == US_Eigen::EIGEN_1_OVER_SD
          || (US_Eigen::weight_methods)cb_powerfit_fit_alg_weight
                     ->currentData ()
                     .toInt ()
                 == US_Eigen::EIGEN_1_OVER_SD_SQ))
    {
      cb_powerfit_fit_alg_weight->setCurrentIndex (
          0); // US_Eigen::EIGEN_NO_WEIGHTS
    }

  // move to _gui   lbl_powerfit_q_range->setText( us_tr( " Fit range 1
  // (fixed): " );
  ...
  powerfit_add_marker (plot_dist, le_powerfit_q_start->text ().toDouble (),
                       powerfit_color_q, us_tr ("Start"),
                       Qt::AlignRight | Qt::AlignBottom);
  powerfit_add_marker (plot_dist, le_powerfit_q_end->text ().toDouble (),
                       powerfit_color_q, us_tr ("End"),
                       Qt::AlignLeft | Qt::AlignBottom);
  powerfit_add_marker (
      plot_dist, dad_param_lambda, Qt::red, UNICODE_LAMBDA_QS + "(abs)",
      Qt::AlignRight | Qt::AlignBaseline); // Qt::AlignVCenter );
  powerfit_add_marker (plot_dist, le_powerfit_lambda2->text ().toDouble (),
                       Qt::magenta, UNICODE_LAMBDA_QS + "(scat)",
                       Qt::AlignRight
                           | Qt::AlignBaseline); // Qt::AlignVCenter );
  ...
  if (le_powerfit_q_start->text ().isEmpty ()
  ...
  if (le_powerfit_q_end->text ().isEmpty ()
      || le_powerfit_q_end->text ().toDouble () > powerfit_q_max
      || le_powerfit_q_end->text ().toDouble ()
             <= powerfit_q_min
                    + 0.001 // == caused by the slot, +0.001 for rounding
  )
    {
    ...
    {
      qDebug () << "powerfit q end NOT set, value "
                << le_powerfit_q_end->text ();
    }
...
void
US_Hydrodyn_Dad::powerfit_enables ()
{
  // qDebug() << "powerfit_enables() last_minimize_scale " <<
  // last_minimize_scale << " powerfit_scale: " <<
  // le_powerfit_scale->text().toDouble(); mostly disable except cancel & wheel
  if (minimize_running)
    {
  ...
  pb_powerfit_fit->setEnabled (
      le_powerfit_c_min->text ().toDouble ()
          <= le_powerfit_c_max->text ().toDouble ()
...
void
US_Hydrodyn_Dad::powerfit_scroll_highlight (int pos)
{
  qDebug () << "powerfit_scroll_highlight " << pos;
}

void
US_Hydrodyn_Dad::powerfit_add_marker (QwtPlot *plot, double pos, QColor color,
...
void
US_Hydrodyn_Dad::powerfit_delete_markers ()
{
...
void
US_Hydrodyn_Dad::powerfit_q_start_text (const QString &text)
{
  ...
  if (powerfit_markers.size () != 4)
    {
      editor_msg (
          "red",
          QString (
              "internal error: powerfit_q_start_text markers issue size %1")
              .arg (powerfit_markers.size ()));
      return;
    }
  ...
  disconnect (le_powerfit_q_start, SIGNAL (textChanged (const QString &)), 0,
              0);
...
void
US_Hydrodyn_Dad::powerfit_q_end_text (const QString &text)
{
  ...
  if (powerfit_markers.size () != 4)
    {
      editor_msg (
          "red",
          QString ("internal error: powerfit_q_end_text markers issue size %1")
              .arg (powerfit_markers.size ()));
      return;
    }
...
void
US_Hydrodyn_Dad::powerfit_q_start_focus (bool hasFocus)
{
  ...
  if (hasFocus)
    {
      ...
      qwtw_wheel->setSingleStep ((powerfit_q_max - powerfit_q_min)
...
void
US_Hydrodyn_Dad::powerfit_q_end_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Dad::powerfit_a_text (const QString & /* text */)
{
...
void
US_Hydrodyn_Dad::powerfit_a_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Dad::powerfit_b_text (const QString & /* text */)
{
...
void
US_Hydrodyn_Dad::powerfit_b_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Dad::powerfit_c_text (const QString & /* text */)
{
...
void
US_Hydrodyn_Dad::powerfit_c_focus (bool hasFocus)
{
  qDebug () << "powerfit_a_focus";
  if (!hasFocus)
    {
...
void
US_Hydrodyn_Dad::powerfit_c_min_text (const QString & /* text */)
{
...
void
US_Hydrodyn_Dad::powerfit_c_min_focus (bool /* hasFocus */)
{
  qDebug () << "powerfit_c_min_focus";
}

void
US_Hydrodyn_Dad::powerfit_c_max_text (const QString & /* text */)
{
...
void
US_Hydrodyn_Dad::powerfit_c_max_focus (bool /* hasFocus */)
{
  qDebug () << "powerfit_c_max_focus";
}

void
US_Hydrodyn_Dad::powerfit_fit_epsilon_text (const QString & /* text */)
{
  qDebug () << "powerfit_fit_epsilon_text";
}

void
US_Hydrodyn_Dad::powerfit_fit_epsilon_focus (bool /* hasFocus */)
{
  qDebug () << "powerfit_fit_epsilon_focus";
}

void
US_Hydrodyn_Dad::powerfit_fit_iterations_text (const QString & /* text */)
{
  qDebug () << "powerfit_fit_iterations_text";
}

void
US_Hydrodyn_Dad::powerfit_fit_iterations_focus (bool /* hasFocus */)
{
  qDebug () << "powerfit_fit_iterations_focus";
}

void
US_Hydrodyn_Dad::powerfit_fit_max_calls_text (const QString & /* text */)
{
  qDebug () << "powerfit_fit_max_calls_text";
}

void
US_Hydrodyn_Dad::powerfit_fit_max_calls_focus (bool /* hasFocus */)
{
  qDebug () << "powerfit_fit_max_calls_focus";
}

void
US_Hydrodyn_Dad::powerfit_computed_conc_text (const QString & /* text */)
{
  qDebug () << "powerfit_computed_conc_text";
}

void
US_Hydrodyn_Dad::powerfit_computed_conc_focus (bool /* hasFocus */)
{
  qDebug () << "powerfit_computed_conc_focus";
}

void
US_Hydrodyn_Dad::powerfit_scat_conc_text (const QString & /* text */)
{
  qDebug () << "powerfit_scat_conc_text";
}

void
US_Hydrodyn_Dad::powerfit_scat_conc_focus (bool /* hasFocus */)
{
  qDebug () << "powerfit_scat_conc_focus";
}

void
US_Hydrodyn_Dad::powerfit_uncorrected_conc_text (const QString & /* text */)
{
  qDebug () << "powerfit_uncorrected_conc_text";
}

void
US_Hydrodyn_Dad::powerfit_uncorrected_conc_focus (bool /* hasFocus */)
{
  qDebug () << "powerfit_uncorrected_conc_focus";
}

void
US_Hydrodyn_Dad::powerfit_lambda_text (const QString &text)
{
...
void
US_Hydrodyn_Dad::powerfit_lambda_focus (bool /* hasFocus */)
{
  qDebug () << "powerfit_lambda_focus";
}

void
US_Hydrodyn_Dad::powerfit_lambda_abs_text (const QString & /* text */)
{
  qDebug () << "powerfit_lambda_abs_text";
}

void
US_Hydrodyn_Dad::powerfit_lambda_abs_focus (bool /* hasFocus */)
{
  qDebug () << "powerfit_lambda_abs_focus";
}

void
US_Hydrodyn_Dad::powerfit_lambda2_text (const QString &text)
{
...
void
US_Hydrodyn_Dad::powerfit_lambda2_focus (bool /* hasFocus */)
{
  qDebug () << "powerfit_lambda_focus";
}

void
US_Hydrodyn_Dad::powerfit_lambda2_abs_text (const QString & /* text */)
{
  qDebug () << "powerfit_lambda2_abs_text";
}

void
US_Hydrodyn_Dad::powerfit_lambda2_abs_focus (bool /* hasFocus */)
{
  qDebug () << "powerfit_lambda2_abs_focus";
}

void
US_Hydrodyn_Dad::powerfit_extinction_coef_text (const QString &text)
{
...
void
US_Hydrodyn_Dad::powerfit_extinction_coef_focus (bool /* hasFocus */)
{
  qDebug () << "powerfit_extinction_coef_focus";
}

double
compute_powerfit (double t, const double *par)
{
  double c = par[2];

  if (c < powerfit_c_min || c > powerfit_c_max)
    {
      return DBL_MAX;
    }

  // if ( !fit_weights_index.count( t ) ) {
  //    qDebug() << "compute_powerfit error : fit_weights_index[ " << t << " ]
  //    does not exist";
  // }
  // if ( fit_weights.size() <= fit_weights_index[ t ] ) {
  //    qDebug() << "compute_powerfit error : fit_weights[ " <<
  //    fit_weights_index[ t ] << " ] index out of range";
  // }
...
double
compute_powerfit_dispersion_correction (double t, const double *par)
{
  double c = par[2];
  ...
  return fit_weights[fit_weights_index[t]]
         * (par[0] + par[1] * pow (powerfit_corrected_lambda[t], -c));
}

void
US_Hydrodyn_Dad::powerfit_fit ()
{
  qDebug () << "powerfit_fit()";
  disable_all ();
#warning qwtw_wheel probably should be in ::disable_all() but for some reason we commented it out there, probably should check all wheel usage and set as appropriate
  ...
  switch ((US_Eigen::weight_methods)cb_powerfit_fit_alg_weight->currentData ()
              .toInt ())
    {
...

  {
    ...
    vector<double> par = { le_powerfit_a->text ().toDouble (),
                           le_powerfit_b->text ().toDouble (),
                           le_powerfit_c->text ().toDouble () };

    LM::lmcurve_fit_rmsd ((int)par.size (), (double *)&(par[0]),
                          (int)q.size (), (double *)&(q[0]), (double *)&(I[0]),
                          cb_powerfit_dispersion_correction->isChecked ()
                              ? compute_powerfit_dispersion_correction
                              : compute_powerfit,
...

  {
    ...
    if (powerfit_use_errors && chi2 != 0)
      {
        msg += QString (" Chi^2 %2 nChi^2 %3")
                   .arg (chi2)
                   .arg (chi2 / (double)q.size ());
      }
...
void
US_Hydrodyn_Dad::powerfit_fit_alg_index ()
{
...
void
US_Hydrodyn_Dad::powerfit_fit_alg_weight_index ()
{
...
void
US_Hydrodyn_Dad::powerfit_fit_curve_index ()
{
...
void
US_Hydrodyn_Dad::powerfit_reset ()
{
...
void
US_Hydrodyn_Dad::powerfit_fit_clear (bool replot)
{
...
vector<double>
US_Hydrodyn_Dad::powerfit_curve (const vector<double> &x, double A, double B,
                                 double C)
{
  ...
  US_Vector::printvector2 (
      QString ("powerfit_curve %1, %2, %3").arg (A).arg (B).arg (C), x, y);
  return y;
}

vector<double>
US_Hydrodyn_Dad::curve_trim (const vector<double> x, double x_min,
                             double x_max)
{
...
void
US_Hydrodyn_Dad::powerfit_curve_update ()
{
  ...
  vector<double> y = powerfit_curve (x, le_powerfit_a->text ().toDouble (),
                                     le_powerfit_b->text ().toDouble (),
                                     le_powerfit_c->text ().toDouble ());
  ...
  if (powerfit_use_errors)
    {
      add_plot ("Scattering corrected", x, y, f_errors[powerfit_name], false,
                false);
    }
  else
    {
...
void
US_Hydrodyn_Dad::powerfit_create_adjusted_curve ()
{
  ...
  QString name = powerfit_name
                 + QString ("_scA_%1_scB_%2_scC_%3")
...
void
US_Hydrodyn_Dad::powerfit_compute_conc ()
{
  le_powerfit_uncorrected_conc->setText ("");
  le_powerfit_scat_conc->setText ("");
  le_powerfit_computed_conc->setText ("");
  ...
  bool using_corrected_data
      = powerfit_corrected_data_ok
        && powerfit_corrected_data.size () == f_qs[powerfit_name].size ();
...

  {
    ...
    if (dad_param_lambda < x.front () || dad_param_lambda > x.back ())
      {
        le_powerfit_uncorrected_conc->setText (
            us_tr (UNICODE_LAMBDA_QS + " out of range"));
        return;
      }
    ...
    le_powerfit_lambda_abs->setText (
        QString ("%1").arg (abs_at_lambda, 0, 'f', 3));
    powerfit_uncorrected_conc = abs_at_lambda / dad_param_g_extinction_coef;
    powerfit_uncorrected_conc = QString ("%1")
                                    .arg (powerfit_uncorrected_conc, 0, 'f', 3)
                                    .toDouble ();
    le_powerfit_uncorrected_conc->setText (
        QString ("%1").arg (powerfit_uncorrected_conc, 0, 'f', 3));
      ...
      else
        {
          ...
          if (!usu->linear_interpolate (x, y, target, y2))
          ...
          else
            {
              ...
              le_powerfit_lambda2_abs->setText (
                  QString ("%1").arg (abs_at_lambda2, 0, 'f', 3));
              powerfit_scat_conc = (abs_at_lambda - abs_at_lambda2)
                                   / dad_param_g_extinction_coef;
              powerfit_scat_conc = QString ("%1")
                                       .arg (powerfit_scat_conc, 0, 'f', 3)
                                       .toDouble ();
              le_powerfit_scat_conc->setText (
                  QString ("%1").arg (powerfit_scat_conc, 0, 'f', 3));
            }
        }
    }
  }
  ...
  if (using_corrected_data)
    {
      ...
      powerfit_conc
          = QString ("%1").arg (powerfit_conc, 0, 'f', 3).toDouble ();
      le_powerfit_computed_conc->setText (
          QString ("%1").arg (powerfit_conc, 0, 'f', 3));
    }
}

void
US_Hydrodyn_Dad::powerfit_set_b_default ()
{
  ...
  if (powerfit_org_I_at_lambda.count (q_start))
  ...
  else
    {
      ...
      if (!usu->linear_interpolate (f_qs[powerfit_name], f_Is[powerfit_name],
                                    target, y2))
        {
  ...
  powerfit_b_default = QString ("%1").arg (
      1e0
      / fabs (pow (q_start, -le_powerfit_c_min->text ().toDouble ()) / val));
  le_powerfit_b->setText (powerfit_b_default);
}

void
US_Hydrodyn_Dad::powerfit_dispersion_correction_clicked ()
{
