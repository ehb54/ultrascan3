...
unsigned int
US_Hydrodyn_Mals_Fit::pop_selection (unsigned int size)
{
...
bool
US_Hydrodyn_Mals_Fit::ga_fitness (ga_individual &individual)
{
  ...
  our_vector *vi = new_our_vector (MFIT::init_params.size ());
  for (unsigned int i = 0; i < MFIT::init_params.size (); i++)
    {
...
bool
US_Hydrodyn_Mals_Fit::ga_run (double &nrmsd)
{
  ...
  individual.v.resize (MFIT::init_params.size ());

  // init population
  for (unsigned int i = 0; i < control_parameters["population"].toUInt (); i++)
    {
      for (unsigned int j = 0; j < MFIT::init_params.size (); j++)
        {
          individual.v[j]
              = MFIT::param_min[j]
                + (MFIT::param_max[j] - MFIT::param_min[j]) * drand48 ();
        }
           ...
           i++)
        {
          ...
          if (control_parameters.count ("mutate")
              && drand48 () < control_parameters["mutate"].toDouble ())
            {
              ...
              unsigned int pos
                  = (unsigned int)(drand48 () * MFIT::init_params.size ());
              individual.v[pos]
                  = MFIT::param_min[pos]
                    + (MFIT::param_max[pos] - MFIT::param_min[pos])
                          * drand48 ();
              ga_fitness (individual);
              pop.push_back (individual);
              mutate_count++;
          ...
          if (control_parameters.count ("crossover")
              && drand48 () < control_parameters["crossover"].toDouble ())
            {
              ...
              unsigned int pos
                  = (unsigned int)(drand48 () * MFIT::init_params.size ());
              for (unsigned int j = pos; j < MFIT::init_params.size (); j++)
                {
          ...
          if (has_been_duplicated.count (pos))
            {
              for (unsigned int j = 0; j < MFIT::init_params.size (); j++)
                {
                  individual.v[j] = MFIT::param_min[j]
                                    + (MFIT::param_max[j] - MFIT::param_min[j])
                                          * drand48 ();
                }
              ga_fitness (individual);
              pop.push_back (individual);
  ...
  for (unsigned int i = 0; i < MFIT::init_params.size (); i++)
    {
      MFIT::init_params[i] = pop.front ().v[i];
    }
