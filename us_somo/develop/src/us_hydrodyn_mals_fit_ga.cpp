...
unsigned int US_Hydrodyn_Mals_Fit::pop_selection(unsigned int size) {
...
bool US_Hydrodyn_Mals_Fit::ga_fitness(ga_individual &individual) {
...
bool US_Hydrodyn_Mals_Fit::ga_run(double &nrmsd) {
  ...
  for (unsigned int g = 0; g < control_parameters["generations"].toUInt();
    ...
    for (unsigned int i = 0; i < control_parameters["population"].toUInt();
      ...
      if (control_parameters.count("mutate") &&
        ...
        unsigned int pos = (unsigned int)(drand48() * MFIT::init_params.size());
        individual.v[pos] =
            MFIT::param_min[pos] +
            (MFIT::param_max[pos] - MFIT::param_min[pos]) * drand48();
        ga_fitness(individual);
        pop.push_back(individual);
        mutate_count++;
        continue;
