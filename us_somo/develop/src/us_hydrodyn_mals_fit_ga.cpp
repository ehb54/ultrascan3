...
bool US_Hydrodyn_Mals_Fit::ga_run( double & nrmsd )
{
   ...
   for ( unsigned int i = 0; i < control_parameters[ "population" ].toUInt(); i++ )
   {
      ...
      {
         individual.v[ j ] = MFIT::param_min[ j ] + ( MFIT::param_max[ j ] - MFIT::param_min[ j ] ) * drand48();
      }
   ...
   for ( unsigned int g = 0; g < control_parameters[ "generations" ].toUInt(); g++ )
   {
      ...
      for ( unsigned int i = 0; i < control_parameters[ "population" ].toUInt(); i++ )
      {
              ...
              drand48() < control_parameters[ "mutate" ].toDouble() )
         {
            ...
            individual.v[ pos ] = MFIT::param_min[ pos ] + ( MFIT::param_max[ pos ] - MFIT::param_min[ pos ] ) * drand48();
            ga_fitness( individual );
            pop.push_back( individual );
            mutate_count++;
            continue;
