...
void
US_Hydrodyn_DMD_Options::setupGUI ()
{
  int minHeight1 = 30;
  ...
  cb_pdb_static_pairs->setFont (QFont (USglobal->config_list.fontFamily,
                                       USglobal->config_list.fontSize));
  cb_pdb_static_pairs->setPalette (PALET_NORMAL);
  AUTFBACK (cb_pdb_static_pairs);
  connect (cb_pdb_static_pairs, SIGNAL (clicked ()),
           SLOT (set_pdb_static_pairs ()));
  ...
  cnt_threshold_pb_pb->setFont (QFont (USglobal->config_list.fontFamily,
                                       USglobal->config_list.fontSize));
  ...
  cnt_threshold_pb_sc->setFont (QFont (USglobal->config_list.fontFamily,
                                       USglobal->config_list.fontSize));
  ...
  cnt_threshold_sc_sc->setFont (QFont (USglobal->config_list.fontFamily,
                                       USglobal->config_list.fontSize));
  ...
  j++;
  gl_thresh->addWidget (cb_pdb_static_pairs, j, 0, 1 + (j) - (j),
                        1 + (1) - (0));
  j++;
...
void
US_Hydrodyn_DMD_Options::cancel ()
{
  close ();
}

void
US_Hydrodyn_DMD_Options::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_DMD_Options::closeEvent (QCloseEvent *e)
{
  *dmd_widget = false;
  global_Xpos -= 30;
  global_Ypos -= 30;
...
void
US_Hydrodyn_DMD_Options::update_threshold_pb_pb (double val)
{
...
void
US_Hydrodyn_DMD_Options::update_threshold_pb_sc (double val)
{
...
void
US_Hydrodyn_DMD_Options::update_threshold_sc_sc (double val)
{
...
void
US_Hydrodyn_DMD_Options::set_pdb_static_pairs ()
{
...
void
US_Hydrodyn_DMD_Options::set_force_chem ()
{
