...
US_Hydrodyn_Results::US_Hydrodyn_Results(struct hydro_results *results,
                                         bool *result_widget, void *us_hydrodyn,
...
void US_Hydrodyn_Results::setupGUI() {
  ...
  lbl_info =
      new QLabel(us_tr(QString("SOMO Hydrodynamic Results\n(%1 at %2%3C, pH "
  ...
  if (fabs(results->used_beads_sd) <= 1e-100) {
    ...
    le_used_beads->setText(QString("").sprintf(
        "%u (%4.2e)", (int)(results->used_beads + .5), results->used_beads_sd));
  }
  ...
  if (fabs((*results).s20w_sd) <= 1e-100) {
    ...
    le_s20w->setText(
        str.sprintf("%4.2e S (%4.2e)", (*results).s20w, (*results).s20w_sd));
  }
  ...
  if (le_method->text() == "Zeno" ||
    ...
    if (fabs((*results).tau_sd) <= 1e-100) {
      ...
      le_tau->setText(
          str.sprintf("%4.2e ns (%4.2e)", (*results).tau, (*results).tau_sd));
    }
  }
  ...
  if (fabs((*results).viscosity_sd) <= 1e-100) {
    ...
    le_viscosity->setText(str.sprintf(
        "%4.2e cm^3/g (%4.2e)", (*results).viscosity, (*results).viscosity_sd));
  }
...
void US_Hydrodyn_Results::cancel() { close(); }
...
void US_Hydrodyn_Results::load_asa() {
  QString filename = QFileDialog::getOpenFileName(this, windowTitle(), somo_dir,
                                                  "*.asa_res *.ASA_RES");
  if (!filename.isEmpty()) {
    view_file(filename);
  }
