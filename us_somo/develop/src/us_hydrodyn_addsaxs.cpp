  ...
  if (f.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    for (unsigned int i = 0; i < saxs_list.size(); i++) {
      ...
      if (saxs_list[i].a5[0] && saxs_list[i].b5[0]) {
        ts << saxs_list[i].saxs_name.toUpper() << "\t" << saxs_list[i].a5[0]
           << "\t" << saxs_list[i].b5[0] << "\t" << saxs_list[i].a5[1] << "\t"
           << saxs_list[i].b5[1] << "\t" << saxs_list[i].a5[2] << "\t"
           << saxs_list[i].b5[2] << "\t" << saxs_list[i].a5[3] << "\t"
           << saxs_list[i].b5[3] << "\t" << saxs_list[i].a5[4] << "\t"
...
void US_AddSaxs::select_file() {
  ...
  if (saxs_filename.isEmpty()) {
    ...
    if (f.open(QIODevice::ReadOnly | QIODevice::Text)) {
      ...
      while (!ts.atEnd()) {
        QStringList qsl =
            (ts.readLine()).split(QRegExp("\\s+"), Qt::SkipEmptyParts);
        int pos = 0;
        if (qsl.size() == 11) {
