  ...
  for (unsigned int i = 0; i < bead_models.size(); i++) {
    ...
    for (unsigned int i = 0; i < atoms.size(); i++) {
      ...
      if (our_saxs_options.bead_model_rayleigh) {
        ...
        if (compute_rayleigh_structure_factors(atoms[i].radius * scaling_root,
                                               delta_rho, q, F)) {
          fp[0][i] = F[0];
          rayleigh_ok = true;
        } else {
      ...
      if (!rayleigh_ok) {
        ...
        for (unsigned int j = 0; j < q_points; j++) {
          ...
          f[j][i] =
              ...
              saxs.a[3] * exp(-saxs.b[3] * q_over_4pi_2[j]) +
              atoms[i].hydrogens *
                  (saxsH.c + saxsH.a[0] * exp(-saxsH.b[0] * q_over_4pi_2[j]) +
...
double compute_exponential_f(double t, const double *par) {
  ...
  for (unsigned int i = 0; i < exponential_terms; i++) {
    ...
    // par[ i * 2 ] ).arg( par[ 1 + i * 2 ] );
    result +=
        par[1 + i * 2] * exp(par[2 + i * 2] * minusoneoverfourpisq * t * t);
  }
...
bool US_Saxs_Util::compute_exponential(vector<double> &q, vector<double> &I,
                                       vector<double> &coeff4,
                                       vector<double> &coeff5, double &norm4,
                                       double &norm5) {
  vector<double> coeffv;
  double normv;
  return compute_exponential(q, I, coeff4, coeff5, coeffv, norm4, norm5, normv,
                             0);
}
...
bool US_Saxs_Util::compute_exponential(
  ...
  if (q.size() != I.size()) {
    errormsg =
        QString(
            "compute_exponential() I.size() %1 does not equal q.size() %2\n")
            .arg(I.size())
            .arg(q.size());
    return false;
  }
    ...
    if (use_quick_fitting) {
      for (unsigned int this_terms = 0; this_terms <= terms; this_terms++) {
        ...
        lmcurve_fit((int)this_n_par, (double *)&(par[0]), (int)m_dat,
                    ...
                    compute_exponential_f, (const lm_control_struct *)&control,
                    &status);
        ...
        for (unsigned int i = 0; i <= this_n_par; i++) {
          ...
          if (use_gsm_fitting) {
            ...
            do {
              ...
              lmcurve_fit((int)this_n_par, (double *)&(gsm_par[0]), (int)m_dat,
                          (double *)&(compute_exponential_t[0]),
                          (double *)&(compute_exponential_y[0]),
                          compute_exponential_f,
                          (const lm_control_struct *)&control, &status);
...
bool US_Saxs_Util::compute_gsm_exponentials(vector<double> &par,
  ...
  cout
      << QString("start gsm method %1 org norm %2\n").arg(method).arg(org_norm);
...
double US_Saxs_Util::compute_gsm_exponentials_f(our_vector *v) {
  ...
  for (unsigned int j = 0; j < compute_gsm_exponentials_mtto4pi2.size(); j++) {
    ...
    for (unsigned int i = 1; i < (unsigned int)v->len - 1; i += 2) {
      cy[j] +=
          v->d[i] * exp(v->d[i + 1] * compute_gsm_exponentials_mtto4pi2[j]);
    }
  }
...
bool US_Saxs_Util::smooth(vector<double> &x, vector<double> &result,
