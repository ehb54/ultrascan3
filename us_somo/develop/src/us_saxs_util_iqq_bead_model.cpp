...
bool
US_Saxs_Util::calc_saxs_iq_native_fast_bead_model ()
{
  errormsg = "";
  noticemsg = "";
          ...
          if (our_saxs_options.bead_model_rayleigh)
            {
              ...
              if (compute_rayleigh_structure_factors (
                ...
                {
                  noticemsg
                      += "using Rayleigh structure factors failed:" + errormsg;
                }
          ...
          if (!rayleigh_ok)
            {
              ...
              f[0][i] =
                  // atoms[i].srv *
                  (atoms[i].saxs_data.c + atoms[i].saxs_data.a[0]
                   + atoms[i].saxs_data.a[1] + atoms[i].saxs_data.a[2]
...
//  ------------------------------------------------------------------------------------------------------
bool
US_Saxs_Util::calc_saxs_iq_native_debye_bead_model ()
{
              ...
              for (unsigned int j = 0; j < q_points; j++)
                {
                  ...
                  f[j][i] =
                      // atoms[i].srv *
                      (saxs.c + saxs.a[0] * exp (-saxs.b[0] * q_over_4pi_2[j])
...
//  ------------------------------------------------------------------------------------------------------
bool
US_Saxs_Util::calc_saxs_iq_native_hybrid_bead_model ()
{
  errormsg = "";
  noticemsg = "";
                  ...
                  f[j][i]
                      = saxs.c + saxs.a[0] * exp (-saxs.b[0] * q_over_4pi_2[j])
                        ...
                        + atoms[i].hydrogens
                              * (saxsH.c
                                 + saxsH.a[0]
                                       * exp (-saxsH.b[0] * q_over_4pi_2[j])
                                 + saxsH.a[1]
                                       * exp (-saxsH.b[1] * q_over_4pi_2[j])
                                 + saxsH.a[2]
                                       * exp (-saxsH.b[2] * q_over_4pi_2[j])
                                 + saxsH.a[3]
...
bool
US_Saxs_Util::run_iqq_bead_model ()
{
  errormsg = "";
  noticemsg = "";
...
double
compute_exponential_f (double t, const double *par)
{
  double result = par[0];
  //       cout << QString( "t %1 minusoneoverfourisq %2 minusoneoverfourpisq *
  //       t * t %3\n" )
  //          .arg( t )
...
bool
US_Saxs_Util::compute_exponential (vector<double> &q, vector<double> &I,
                                   vector<double> &coeff4,
                                   vector<double> &coeff5, double &norm4,
                                   double &norm5)
{
  vector<double> coeffv;
  double normv;
  return compute_exponential (q, I, coeff4, coeff5, coeffv, norm4, norm5,
                              normv, 0);
}
...
bool
US_Saxs_Util::compute_exponential (
    vector<double> &q, vector<double> &I, vector<double> &coeff4,
    vector<double> &coeff5, vector<double> &coeffv, double &norm4,
    double &norm5, double &normv,
    // compute increased coeff size (when maxv > 5)
    unsigned int maxv, bool use_gsm_fitting, bool use_quick_fitting)
{
  if (q.size () != I.size ())
    {
      errormsg = QString ("compute_exponential() I.size() %1 does not equal "
                          "q.size() %2\n")
                     .arg (I.size ())
                     .arg (q.size ());
      return false;
        ...
        for (unsigned int this_terms = 0; this_terms <= terms; this_terms++)
          {
            ...
            lmcurve_fit ((int)this_n_par, (double *)&(par[0]), (int)m_dat,
                         (double *)&(compute_exponential_t[0]),
                         (double *)&(compute_exponential_y[0]),
                         compute_exponential_f,
                         (const lm_control_struct *)&control, &status);
            ...
            //             if ( sum < 0e0 )
            //             {
            //                cout << QString( "WARNING: negative scattering at
            //                q=0\n" );
            //             }
        ...
        for (unsigned int this_terms = 0; this_terms <= terms; this_terms++)
          {
            ...
            for (unsigned int i = 0; i <= this_n_par; i++)
              {
                cout << QString (
                            "computing %1 term exponential fit try %2 of %3\n")
                            .arg (this_terms)
                            .arg (i + 1)
                            .arg (this_n_par + 1)
                     << flush;
                ...
                if (use_gsm_fitting)
                  {
                    ...
                    lmcurve_fit ((int)this_n_par, (double *)&(par[0]),
                                 (int)m_dat,
                                 (double *)&(compute_exponential_t[0]),
                                 (double *)&(compute_exponential_y[0]),
                                 compute_exponential_f,
                    ...
                    do
                      {
                        ...
                        lmcurve_fit (
                            (int)this_n_par, (double *)&(gsm_par[0]),
                            (int)m_dat, (double *)&(compute_exponential_t[0]),
                            (double *)&(compute_exponential_y[0]),
                            compute_exponential_f,
                            (const lm_control_struct *)&control, &status);
                        r = drand48 ();
                        ...
                        if (start_rmsd > end_rmsd)
                          {
                            cout << QString ("gsm repeat %1 for improvement, "
                                             "%2 %3\n")
                                        .arg (++gsm_repeat)
                                        .arg (start_rmsd)
                                        .arg (end_rmsd);
                    ...
                    while (start_rmsd > end_rmsd && gsm_repeat < 50
                           && end_rmsd / 500e0 < par_before_gsm_norm);
...
void
US_Saxs_Util::compute_gsm_exponentials_setup (vector<double> &t,
                                              vector<double> &y)
{
...
bool
US_Saxs_Util::compute_gsm_exponentials (vector<double> &par, vector<double> &t,
                                        vector<double> &y, QString method)
{
  ...
  switch (gsm_method)
    ...
    case CONJUGATE_GRADIENT:
      nsa_min_fr_pr_cgd (v, gsmepsilon, gsmiterations);
      break;
    case STEEPEST_DESCENT:
      nsa_min_gsm_5_1 (v, gsmepsilon, gsmiterations);
      break;
    case INVERSE_HESSIAN:
      nsa_min_hessian_bfgs (v, gsmepsilon, gsmiterations);
      break;
    default:
      {
...
double
US_Saxs_Util::compute_gsm_exponentials_f (our_vector *v)
{
  ...
  for (unsigned int j = 0; j < compute_gsm_exponentials_mtto4pi2.size (); j++)
    {
      ...
      for (unsigned int i = 1; i < (unsigned int)v->len - 1; i += 2)
        {
          cy[j] += v->d[i]
                   * exp (v->d[i + 1] * compute_gsm_exponentials_mtto4pi2[j]);
        }
    }
    ...
    double &scattering_intensity)
{
  ...
  scattering_intensity
      = compute_ff (this_atom.saxs_data, saxsH, this_atom.resName,
                    this_atom.saxs_name, this_atom.name, hydrogens, 0e0, 0e0);
  scattering_intensity *= scattering_intensity;
  // cout << QString( "compute saxs for atom %1 si %2\n" ).arg( this_atom.name
  // ).arg( scattering_intensity );
  return true;
}
...
bool
US_Saxs_Util::grid (
    vector<PDB_atom> &bm, vector<PDB_atom> &result, double dR,
...
)
{
  ...
  for (pgrid.axis[0] = pmin.axis[0]; pgrid.axis[0] <= pmax.axis[0];
       pgrid.axis[0] += dR)
    {
      for (pgrid.axis[1] = pmin.axis[1]; pgrid.axis[1] <= pmax.axis[1];
           pgrid.axis[1] += dR)
        {
          for (pgrid.axis[2] = pmin.axis[2]; pgrid.axis[2] <= pmax.axis[2];
               pgrid.axis[2] += dR)
            {
              unsigned int close_count = 0;
              for (unsigned int i = 0; i < (unsigned int)bm.size (); i++)
                {
                  if (dist (pgrid, bm[i].bead_coordinate)
                      <= bm[i].bead_computed_radius)
                    {
                      close_count++;
                      if (!accum_overlaps)
                        {
...
bool
US_Saxs_Util::smooth (vector<double> &x, vector<double> &result,
                      unsigned int points, bool smooth_endpoints)
{
  errormsg = "";
  if (!x.size ())
    {
