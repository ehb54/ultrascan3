...
US_Hydrodyn_Saxs_Hplc_Fit::US_Hydrodyn_Saxs_Hplc_Fit (
    US_Hydrodyn_Saxs_Hplc *hplc_win, bool set_comm_dist, QWidget *p,
    const char *)
    : QDialog (p)
{
  // us_qdebug( "hf0" );
  ...
  setWindowTitle (us_tr ("US-SOMO: SAXS Hplc: Gaussian Fit"));
      ...
      if (gaussian_type != US_Hydrodyn_Saxs_Hplc::EMGGMG)
      ...
      else
        {
          for (int i = gaussian_type_size;
               i < (int)hplc_win->gaussians.size (); i += gaussian_type_size)
            {
              conc_ratios.push_back (hplc_win->gaussians[2 + i]
                                     / hplc_win->gaussians[2]);
            }
...
void
US_Hydrodyn_Saxs_Hplc_Fit::redo_settings ()
{
  use_errors = hplc_win->cb_sd_weight->isChecked ();
  if (use_errors
      && (!hplc_win->f_errors.count (hplc_win->wheel_file)
          || hplc_win->f_errors[hplc_win->wheel_file].size ()
                 != hplc_win->f_qs[hplc_win->wheel_file].size ()
          || !hplc_win->is_nonzero_vector (
              hplc_win->f_errors[hplc_win->wheel_file])))
    {
      use_errors = false;
...
void
US_Hydrodyn_Saxs_Hplc_Fit::restore ()
{
...
void
US_Hydrodyn_Saxs_Hplc_Fit::undo ()
{
...
void
US_Hydrodyn_Saxs_Hplc_Fit::setupGUI ()
{
  int minHeight1 = 30;

  lbl_title = new QLabel (us_tr ("US-SOMO: SAXS Hplc: Gaussian Fit"), this);
...
void
US_Hydrodyn_Saxs_Hplc_Fit::cancel ()
{
  close ();
}

void
US_Hydrodyn_Saxs_Hplc_Fit::help ()
{
  US_Help *online_help;
  online_help = new US_Help (this);
  online_help->show_help ("manual/somo/somo_saxs_hplc_fit.html");
}

void
US_Hydrodyn_Saxs_Hplc_Fit::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 3;
  global_Ypos -= 30;
  e->accept ();
...
void
US_Hydrodyn_Saxs_Hplc_Fit::update_common ()
{
  if (!setup_run ())
    {
      update_enables ();
  ...
  for (unsigned int i = 0; i < HFIT::param_fixed.size (); i++)
    {
      if (!HFIT::param_fixed[i] || HFIT::comm_backref.count (i))
        {
          hplc_win->gaussians[i]
              = HFIT::init_params[HFIT::comm_backref.count (i)
                                      ? HFIT::comm_backref[i]
                                      : HFIT::param_pos[i]];
...
void
US_Hydrodyn_Saxs_Hplc_Fit::update_enables ()
{
...
bool
US_Hydrodyn_Saxs_Hplc_Fit::setup_run ()
{
  // us_qdebug( "setup_run()" );
  ...
  for (unsigned int i = 0; i < (unsigned int)hplc_win->gaussians.size ();
       i += gaussian_type_size)
    {
      ...
      if (cb_fix_amplitude->isChecked () || fixed_curves.count (pos + 1))
      ...
      else
        {
          ...
          if (HFIT::init_params.back () < min)
            {
              // us_qdebug( QString( "min set amp init_params.size() %1 val %2"
              // ).arg( HFIT::init_params.size() ).arg(
              // HFIT::init_params.back() ) );
              HFIT::init_params.back () = min;
            }
          if (HFIT::init_params.back () > max)
            {
              HFIT::init_params.back () = max;
            }
      ...
      if (cb_fix_width->isChecked () || fixed_curves.count (pos + 1)
      ...
      )
        {
          ...
          if (i && cb_conc_test->isChecked () && !fixed_curves.count (pos + 1)
            ...
            {
              HFIT::fixed_params.push_back (hplc_win->gaussians[2 + 0]);
              HFIT::comm_backref[HFIT::param_fixed.size ()]
                  = conc_width_var_pos;
              HFIT::conc_ratios_map[HFIT::param_fixed.size ()]
                  = conc_ratios[pos - 1];
            }
          else
            {
        ...
        {
          conc_width_var_pos = HFIT::init_params.size ();
          HFIT::param_pos.push_back (HFIT::init_params.size ());
          ...
          if (cb_fix_dist1->isChecked () || fixed_curves.count (pos + 1)
          ...
          else
            {
              if (cb_comm_dist1->isChecked ())
                {
                  comm_dist_1_var_pos = HFIT::init_params.size ();
                }
              HFIT::param_pos.push_back (HFIT::init_params.size ());
              ...
              if (cb_fix_dist2->isChecked () || fixed_curves.count (pos + 1)
                  || (cb_comm_dist2->isChecked () && i))
                {
                  ...
                  if (i && cb_comm_dist2->isChecked ()
                      ...
                      && !fixed_curves.count (1))
                    {
                      HFIT::fixed_params.push_back (
                          hplc_win->gaussians[4 + 0]);
                      HFIT::comm_backref[HFIT::param_fixed.size ()]
                          = comm_dist_2_var_pos;
                    }
                  else
              ...
              else
                {
                  if (cb_comm_dist2->isChecked ())
                    {
                      comm_dist_2_var_pos = HFIT::init_params.size ();
                    }
                  HFIT::param_pos.push_back (HFIT::init_params.size ());
...
bool
US_Hydrodyn_Saxs_Hplc_Fit::lock_zeros (vector<double> &par)
{
  ...
  vector<double> tmp_gaussians = hplc_win->gaussians;
  ...
  for (unsigned int i = 0; i < HFIT::param_fixed.size (); i++)
    {
      if (!HFIT::param_fixed[i] || HFIT::comm_backref.count (i))
        {
          tmp_gaussians[i]
              = par[HFIT::comm_backref.count (i) ? HFIT::comm_backref[i]
                                                 : HFIT::param_pos[i]];
          if (HFIT::conc_ratios_map.count (i))
            {
          ...
          if (!cb_fix_amplitude->isChecked ())
            {
              if (tmp_gaussians[0 + i]
                  <= hplc_ampl_width_min * hplc_lock_min_retry_mult)
                {
...
bool
US_Hydrodyn_Saxs_Hplc_Fit::max_free_peak_delta (vector<double> &par)
{
...
void
US_Hydrodyn_Saxs_Hplc_Fit::lm (bool max_free_peak_delta_run, double prev_rmsd)
{
  update_common ();
  setup_run ();
  // puts( "lm" );
  ...
  // US_Vector::printvector( QString( "lm %1 q" ).arg( hplc_win->wheel_file ),
  // x ); US_Vector::printvector( QString( "lm %1 I" ).arg(
  // hplc_win->wheel_file ), hplc_win->f_Is[ hplc_win->wheel_file ] );
  // US_Vector::printvector( QString( "lm %1 e" ).arg( hplc_win->wheel_file ),
  // hplc_win->f_errors[ hplc_win->wheel_file ] );
  ...
  if (use_errors)
    {
      ...
      for (unsigned int j = 0; j < t.size (); j++)
        {
          yp[j] = (*HFIT::compute_gaussian_f) (
              t[j], (double *)(&HFIT::base_params[0]));
          org_rmsd += (y[j] - yp[j]) * (y[j] - yp[j]);
        }
#if defined(UHHSF_FULL_DEBUG)
      US_Vector::printvector3 ("sd y, yp, e", y, yp, HFIT::errors);
#endif
      double nu = (double)(t.size () - (int)hplc_win->gaussians.size () - 1);
      if (nu <= 0e0)
        {
          nu = 1e0;
        }
  ...
  if (use_errors)
    {
      ...
      for (unsigned int j = 0; j < t.size (); j++)
        {
          yp[j] = (*HFIT::compute_gaussian_f) (t[j], (double *)(&par[0]));
  ...
  if (max_free_peak_delta_run)
          ...
          for (unsigned int i = 0; i < HFIT::param_fixed.size (); i++)
            {
              if (!HFIT::param_fixed[i] || HFIT::comm_backref.count (i))
                {
                  hplc_win->gaussians[i] = par[HFIT::comm_backref.count (i)
                                                   ? HFIT::comm_backref[i]
                                                   : HFIT::param_pos[i]];
                  if (HFIT::conc_ratios_map.count (i))
                    {
  ...
  else
    {
      ...
      if (hplc_maxfpk_restart && max_free_peak_delta (par))
        {
...
void
US_Hydrodyn_Saxs_Hplc_Fit::gsm_sd ()
{
  puts ("gsm_sd");
  update_common ();
  ...
  for (unsigned int j = 0; j < gsm_t.size (); j++)
    {
      gsm_yp[j] = (*HFIT::compute_gaussian_f) (
          gsm_t[j], (double *)(&HFIT::base_params[0]));
      ...
      for (unsigned int i = 0; i < HFIT::param_fixed.size (); i++)
        {
          if (!HFIT::param_fixed[i] || HFIT::comm_backref.count (i))
            {
              hplc_win->gaussians[i]
                  = v->d[HFIT::comm_backref.count (i) ? HFIT::comm_backref[i]
                                                      : HFIT::param_pos[i]];
              if (HFIT::conc_ratios_map.count (i))
                {
                  hplc_win->gaussians[i] *= HFIT::conc_ratios_map[i];
...
void
US_Hydrodyn_Saxs_Hplc_Fit::gsm_ih ()
{
  puts ("gsm_ih");
  update_common ();
...
void
US_Hydrodyn_Saxs_Hplc_Fit::gsm_cg ()
{
  puts ("gsm_cg");
  update_common ();
...
void
US_Hydrodyn_Saxs_Hplc_Fit::stop ()
{
  running = false;
}

void
US_Hydrodyn_Saxs_Hplc_Fit::grid ()
{
  setup_run ();
  update_common ();
  puts ("grid");
  ...
  unsigned int total_count
      = (unsigned int)pow ((double)pop_size, (int)HFIT::init_params.size ());
  ...
  for (unsigned int j = 0; j < t.size (); j++)
    {
      yp[j] = (*HFIT::compute_gaussian_f) (t[j],
                                           (double *)(&HFIT::base_params[0]));
      org_rmsd += (y[j] - yp[j]) * (y[j] - yp[j]);
    }
  ...
  for (unsigned int i = 0; i < total_count; i++)
    {
      ...
      for (unsigned int j = 0; j < (unsigned int)HFIT::init_params.size ();
           j++)
        {
