...
US_Hydrodyn_Saxs_Hplc_Fit::US_Hydrodyn_Saxs_Hplc_Fit(
  ...
  if (hplc_win->conc_files.count(hplc_win->wheel_file)) {
    ...
    if (gaussian_type != US_Hydrodyn_Saxs_Hplc::EMGGMG) {
      ...
      for (int i = gaussian_type_size; i < (int)hplc_win->gaussians.size();
...
void US_Hydrodyn_Saxs_Hplc_Fit::redo_settings() {
  ...
  if (use_errors && (!hplc_win->f_errors.count(hplc_win->wheel_file) ||
...
void US_Hydrodyn_Saxs_Hplc_Fit::cancel() { close(); }
  ...
  for (unsigned int i = 0; i < HFIT::param_fixed.size(); i++) {
    if (!HFIT::param_fixed[i] || HFIT::comm_backref.count(i)) {
      hplc_win->gaussians[i] =
          HFIT::init_params[HFIT::comm_backref.count(i) ? HFIT::comm_backref[i]
                                                        : HFIT::param_pos[i]];
  ...
  for (unsigned int i = 0; i < (unsigned int)hplc_win->gaussians.size();
    ...
    if (cb_fix_amplitude->isChecked() || fixed_curves.count(pos + 1)) {
      ...
      if (HFIT::init_params.back() < min) {
        ...
        // HFIT::init_params.size() ).arg( HFIT::init_params.back() ) );
    ...
    if (cb_fix_width->isChecked() || fixed_curves.count(pos + 1) ||
      ...
      if (i && cb_conc_test->isChecked() && !fixed_curves.count(pos + 1) &&
        ...
        HFIT::comm_backref[HFIT::param_fixed.size()] = conc_width_var_pos;
        HFIT::conc_ratios_map[HFIT::param_fixed.size()] = conc_ratios[pos - 1];
      } else {
...
bool US_Hydrodyn_Saxs_Hplc_Fit::lock_zeros(vector<double> &par) {
  ...
  for (unsigned int i = 0; i < HFIT::param_fixed.size(); i++) {
    if (!HFIT::param_fixed[i] || HFIT::comm_backref.count(i)) {
      tmp_gaussians[i] = par[HFIT::comm_backref.count(i) ? HFIT::comm_backref[i]
                                                         : HFIT::param_pos[i]];
      if (HFIT::conc_ratios_map.count(i)) {
...
void US_Hydrodyn_Saxs_Hplc_Fit::lm(bool max_free_peak_delta_run,
                                   double prev_rmsd) {
  update_common();
  setup_run();
  // puts( "lm" );
  ...
  // ); US_Vector::printvector( QString( "lm %1 I" ).arg( hplc_win->wheel_file
  // ), hplc_win->f_Is[ hplc_win->wheel_file ] ); US_Vector::printvector(
  // QString( "lm %1 e" ).arg( hplc_win->wheel_file ), hplc_win->f_errors[
  // hplc_win->wheel_file ] );
  ...
  if (use_errors) {
    ...
    for (unsigned int j = 0; j < t.size(); j++) {
      yp[j] =
          (*HFIT::compute_gaussian_f)(t[j], (double *)(&HFIT::base_params[0]));
...
void US_Hydrodyn_Saxs_Hplc_Fit::gsm_sd() {
  ...
  for (unsigned int j = 0; j < gsm_t.size(); j++) {
    gsm_yp[j] = (*HFIT::compute_gaussian_f)(gsm_t[j],
...
void US_Hydrodyn_Saxs_Hplc_Fit::stop() { running = false; }
