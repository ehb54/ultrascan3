...
void
US_Hydrodyn_Saxs::calc_saxs_iq_native_sh ()
{
  US_Timer us_timers;
  ...
  for (unsigned int i = 0; i < selected_models.size (); i++)
    {
      ...
      progress_saxs->setValue (0);
      progress_saxs->setMaximum (our_saxs_options->sh_max_harmonics + 3);
      qApp->processEvents ();
          ...
          for (unsigned int k = 0;
               k < model_vector[current_model].molecule[j].atom.size (); k++)
            {
              ...
              if (!atom_map.count (this_atom_name + "~" + hybrid_name))
                {
                  cout << "error: (saxs_iqq_sh_1) atom_map missing for "
                          "hybrid_name "
              ...
              //             cout << QString("atom %1 hybrid %2 excl vol %3 by
              //             hybrid radius %4\n")
              //                .arg(this_atom->name)
              //                .arg(this_atom->hybrid_name)
              //                .arg(atom_map[this_atom_name + "~" +
      ...
      progress_saxs->setMaximum (atoms.size ());
      qApp->processEvents ();
          ...
          if (!types.count (key))
            {
              ...
              for (int j = 0; j < (int)q.size (); j++)
                {
                  Fa[j]
                      = compute_ff (saxs, saxsH, atoms[i].residue_name,
      ...
      unsigned int Y_points
          = id.max_harmonics + 1 + (id.max_harmonics) * (id.max_harmonics + 1);
      ...
      if (0)
        {
          ...
          for (int i = 0; i < (int)model.size (); i++)
            {
              printf ("%d: %f %f %f %d\n", world_rank, model[i].x[0],
                      model[i].x[1], model[i].x[2], model[i].ff_type);
            }
        }
      ...
      for (int m = 0; m < model_size; ++m, ++modelp)
        {
          if (!(m % 100))
            {
              progress_saxs->setValue (m + 1);
              progress_saxs->setMaximum (model_size + 1);
              qApp->processEvents ();
              if (stopFlag)
                {
                  editor->append (us_tr ("Terminated by user request.\n"));
          ...
          if (datap->rtp[0] == 0e0)
          ...
          else
            {
              ...
              if (modelp->x[0] == 0 && modelp->x[1] == 0)
                ...
                {
                  double asinc = modelp->x[1]
                                 / sqrt ((modelp->x[0] * modelp->x[0]
                                          + modelp->x[1] * modelp->x[1]));
                  if (asinc > 1e0)
                    {
                      asinc = 1e0;
          ...
          sh::alt_conj_sh (id.max_harmonics, datap->rtp[1], datap->rtp[2], Yp);
          ...
          for (unsigned int j = 0; j < q_points; ++j)
            {
              ...
              if (!shs.shs_compute_sphbes (qp_t_rtp0, Jp))
                {
                  editor_msg ("red", QString ("Error: SH-D: %1 ")
                                         .arg (shs.error_msg.c_str ()));
                  return;
                }
                  ...
                  for (unsigned int i = 0; i < q.size (); i++)
                    {
                      ...
                      ((US_Hydrodyn *)us_hydrodyn)
                          ->last_saxs_iqqa.push_back (I[i]);
                      ((US_Hydrodyn *)us_hydrodyn)
                          ->last_saxs_iqqc.push_back (0e0);
                    }
                  fclose (fsaxs);
                }
              else
...
void
US_Hydrodyn_Saxs::calc_saxs_iq_native_sh_bead_model ()
{
  // don't forget to later merge deleted waters into model_vector
  // right now we are going with first residue map entry
  stopFlag = false;
  ...
  for (unsigned int i = 0; i < selected_models.size (); i++)
    {
      ...
      for (unsigned int j = 0; j < bead_models[current_model].size (); j++)
        {
          ...
          // volume new_atom.excl_vol = ( 4.0 / 3.0 ) * M_PI *
          // pow(this_atom->bead_computed_radius, 3);
          ...
          new_atom.excl_vol *= our_saxs_options->scale_excl_vol;
          new_atom.srv
              = sqrt (new_atom.excl_vol / this_atom->saxs_data.volume);
          ...
          for (unsigned int i = 0; i < atoms.size (); i++)
            {
              fprintf (fsaxs_atoms, "%s %s %.3f %.3f %.3f %.2f %.2f %.2f\n",
                       ...
                       atoms[i].excl_vol, atoms[i].srv, atoms[i].saxs_data.c);
            }
          fclose (fsaxs_atoms);
        }
      ...
      double delta_rho = ((US_Hydrodyn *)us_hydrodyn)->misc.target_e_density
      ...
      if (our_saxs_options->bead_model_rayleigh)
        {
          editor_msg (
              "blue",
              QString ("using Rayleigh structure factors, delta_rho %1\n")
                  .arg (delta_rho));
        }
      ...
      if (our_saxs_options->scale_excl_vol != 1e0)
        {
          scaling_root
              = pow ((double)our_saxs_options->scale_excl_vol, 1e0 / 3e0);
        }

      for (unsigned int i = 0; i < atoms.size (); i++)
        {
          saxs saxs = bead_models[current_model][i]
                          .saxs_data; // = saxs_map[atoms[i].saxs_name];
          //          if ( !i )
          //          {
          //             QString qs;
          ...
          //                ).arg( saxs.saxs_name ).arg( saxs.c5 ); for (
          //                unsigned int i = 0; i < 5; i++ )
          //                {
          //                   qs += QString( "a%1=%2 b%3=%4 " ).arg( i ).arg(
          //                   saxs.a5[ i ] ).arg( i ).arg( saxs.b5[ i ] );
          //                }
          //             } else {
          //                cout << QString( "atom 0 saxs_name %1 c %2\n"
          //                ).arg( saxs.saxs_name ).arg( saxs.c ); for (
          //                unsigned int i = 0; i < 4; i++ )
          //                {
          //                   qs += QString( "a%1=%2 b%3=%4 " ).arg( i ).arg(
          //                   saxs.a[ i ] ).arg( i ).arg( saxs.b[ i ] );
          //                }
          //             }
          //             cout << qs << endl << flush;
          //          }
          ...
          if (our_saxs_options->bead_model_rayleigh)
            {
              ...
              if (usu.compute_rayleigh_structure_factors (
                      atoms[i].radius * scaling_root, delta_rho, q, F))
                {
                  for (unsigned int j = 0; j < q_points; j++)
                    {
                      fp[j][i] = F[j];
                ...
                {
                  editor_msg ("red", "using Rayleigh structure factors failed:"
                                         + usu.errormsg);
                }
            }
          ...
          if (!rayleigh_ok)
            {
              if (!told_you)
                {
                  editor_msg ("blue",
                              "NOT using Rayleigh structure factors\n");
                  told_you = true;
                }
              ...
              for (unsigned int j = 0; j < q_points; j++)
                {
                  ...
                  //                   f[j][i] =
                  //                      saxs.c5 +
                  //                      saxs.a5[0] * exp(-saxs.b5[0] *
                  //                      q_over_4pi_2[j]) + saxs.a5[1] *
                  //                      exp(-saxs.b5[1] * q_over_4pi_2[j]) +
                  //                      saxs.a5[2] * exp(-saxs.b5[2] *
                  //                      q_over_4pi_2[j]) + saxs.a5[3] *
                  //                      exp(-saxs.b5[3] * q_over_4pi_2[j]) +
                  //                      saxs.a5[4] * exp(-saxs.b5[4] *
                  //                      q_over_4pi_2[j]);
                  //                } else {
                  //                   f[j][i] =
                  //                      saxs.c +
                  //                      saxs.a[0] * exp(-saxs.b[0] *
                  //                      q_over_4pi_2[j]) + saxs.a[1] *
                  //                      exp(-saxs.b[1] * q_over_4pi_2[j]) +
                  //                      saxs.a[2] * exp(-saxs.b[2] *
                  //                      q_over_4pi_2[j]) + saxs.a[3] *
                  //                      exp(-saxs.b[3] * q_over_4pi_2[j]);
                  //                }
