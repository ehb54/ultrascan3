    ...
    for (int m = 0; m < model_size; ++m, ++modelp) {
      ...
      for (unsigned int j = 0; j < q_points; ++j) {
        ...
        if (!shs.shs_compute_sphbes(qp_t_rtp0, Jp)) {
          editor_msg("red",
                     QString("Error: SH-D: %1 ").arg(shs.error_msg.c_str()));
          return;
        }
  ...
  for (unsigned int i = 0; i < selected_models.size(); i++) {
    ...
    for (unsigned int j = 0; j < bead_models[current_model].size(); j++) {
      ...
      if (our_saxs_options->bead_models_use_bead_radius_ev) {
        new_atom.excl_vol =
            (4.0 / 3.0) * M_PI * pow(this_atom->bead_computed_radius, 3);
      }
    ...
    if (fsaxs_atoms) {
      for (unsigned int i = 0; i < atoms.size(); i++) {
        fprintf(fsaxs_atoms, "%s %s %.3f %.3f %.3f %.2f %.2f %.2f\n",
                ...
                atoms[i].hybrid_name.toLatin1().data(), atoms[i].pos[0],
                atoms[i].pos[1], atoms[i].pos[2], atoms[i].excl_vol,
                atoms[i].srv, atoms[i].saxs_data.c);
      }
      fclose(fsaxs_atoms);
    }
    ...
    for (unsigned int i = 0; i < atoms.size(); i++) {
      ...
      //                saxs.saxs_name ).arg( saxs.c5 ); for ( unsigned int i =
      //                0; i < 5; i++ )
      //                {
      ...
      //             } else {
      //                cout << QString( "atom 0 saxs_name %1 c %2\n" ).arg(
      //                saxs.saxs_name ).arg( saxs.c ); for ( unsigned int i =
      //                0; i < 4; i++ )
      //                {
      //                   qs += QString( "a%1=%2 b%3=%4 " ).arg( i ).arg(
      ...
      if (our_saxs_options->bead_model_rayleigh) {
        ...
        if (usu.compute_rayleigh_structure_factors(
          ...
          editor_msg("red",
                     "using Rayleigh structure factors failed:" + usu.errormsg);
        }
      }
      ...
      if (!rayleigh_ok) {
        ...
        for (unsigned int j = 0; j < q_points; j++) {
          ...
          //                      q_over_4pi_2[j]) + saxs.a[2] * exp(-saxs.b[2]
