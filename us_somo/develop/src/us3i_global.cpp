...
void
US3i_Global::set_global_position (const QPoint &p)
{
  read_global ();
...
QPoint
US3i_Global::global_position (void)
{
  read_global ();
...
void
US3i_Global::setPasswd (const QString &pw)
{
  read_global ();
  strncpy (global.passwd, pw.toLatin1 ().constData (),
           sizeof global.passwd - 1);
  write_global ();
}

QString
US3i_Global::passwd (void)
{
  read_global ();
  char pw[64];
