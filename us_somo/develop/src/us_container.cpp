...
US_Container::~US_Container() { qDebug() << "US_Container::~US_Container()"; }
...
bool US_Container::installed() {
  ...
  static vector<QString> types = {"docker", "podman"};
  ...
  for (const auto& t : types) {
    ...
    // QString stdOutput =
    // QString::fromLocal8Bit(program.readAllStandardOutput()); QString stdError
...
bool US_Container::test() {
  ...
  qDebug() << "US_Container::test() : starting " << TEST_PROGRAM << " "
           << arguments.join(" ");
  ...
  if (!program.waitForFinished(TEST_PROGRAM_TIMEOUT_SECS * 1000)) {
    qDebug() << "US_Container::test() : failed to finish in "
             << TEST_PROGRAM_TIMEOUT_SECS << " seconds";
    program.kill();
    return false;
  }

  qDebug() << "US_Container::test() : finished, exit code "
           << program.exitCode();

  QTextStream(stdout) << "US_Container::test() : stdout\n"
                      << "-----------------------------------------------------"
                         "---------------------------\n"
...
QString US_Container::unique_name(QString basename) {
  ...
  QString result;
  result = UNIQUE_NAME_PREFIX + basename +
...
QString US_Container::type() { return container_type; }
