  ...
  for (unsigned int i = 0; i < (unsigned int)qs.length (); i++)
    {
      ...
      if (qc == '[' && in_json && !in_quote && in_tok2 && !in_bracket
          && !in_brace && !tok2.length ())
        {
#if defined(USJ_DEBUG)
      ...
      if (qc == '}' && in_json && !in_quote && in_tok2 && in_brace
          && !in_bracket)
        {
#if defined(USJ_DEBUG)
          cout << QString ("close bracket found %1 %2\n")
...
QString
US_Json::compose (map<QString, QString> &mqq)
{
  QString result = "{";

  QRegExp rx_no_quotes ("^(\\d+|null|\\[.*\\]|\\{.*\\})$");

  for (map<QString, QString>::iterator it = mqq.begin (); it != mqq.end ();
       it++)
    {

      result
          += (rx_no_quotes.indexIn (it->first) != -1 ? it->first
                                                     : "\"" + it->first + "\"")
             + ":"
...
US_Udp_Msg::~US_Udp_Msg () { delete qsd; }

void
US_Udp_Msg::send_json (map<QString, QString> json)
{
  for (map<QString, QString>::iterator it = default_json.begin ();
       it != default_json.end (); ++it)
    {
...
void
US_Udp_Msg::send (QString &msg)
{
  qsd->writeDatagram (msg.toUtf8 (), msg.length (), QHostAddress (host), port);
}

void
US_Udp_Msg::set_default_json (map<QString, QString> &json)
{
  default_json = json;
}
