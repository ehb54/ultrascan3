...
void US_Hydrodyn_Mals_Saxs::adjust_wheel(double pos) {
...
void US_Hydrodyn_Mals_Saxs::wheel_cancel(bool from_wheel_save) {
  ...
  switch (current_mode) {
    ...
    case MODE_WYATT: {
      ...
      // #if QT_VERSION < 0x040000
      //             plot_dist->setCurvePen( plotted_curves[ wheel_file ], QPen(
      //             plot_colors[ f_pos[ wheel_file ] % plot_colors.size()],
      //             use_line_width, Qt::SolidLine)); plot_dist->setCurveSymbol(
      //             plotted_curves[ wheel_file ], symbol );
      // #else
      //             plotted_curves[ wheel_file ]->setPen( QPen( plot_colors[
      //             f_pos[ wheel_file ] % plot_colors.size() ], use_line_width,
      //             Qt::SolidLine ) ); plotted_curves[ wheel_file ]->setSymbol(
      //             new QwtSymbol( symbol.style(), symbol.brush(),
...
void US_Hydrodyn_Mals_Saxs::wheel_save() {
  clear_smoothed();  // for now always on cancel or save
  clear_oldstyle();  // for now always on cancel or save
  ...
  switch (current_mode) {
    ...
    case MODE_GAUSSIAN: {
      ...
      for (unsigned int g = 0; g < (unsigned int)gaussians.size();
        ...
        editor_msg(
            "darkblue",
            QString("Gaussian %1: center %2 height %3 width %4 area %5 % of "
                    "total %6\n")
                ...
                .arg(tot_area != 0e0
                         ? 100e0 * g_area[g / gaussian_type_size] / tot_area
                         : 0e0));
      }
    ...
    case MODE_BLANKS: {
      ...
      if (blanks_cormap_parameters.size() && blanks_cormap_pvaluepairs.size()) {
        US_Hydrodyn_Saxs_Cormap_Cluster_Analysis ca;
        if (ca.sliding(blanks_cormap_pvaluepairs, blanks_cormap_parameters,
      ...
      for (int i = 0; i < (int)blanks_selected.size(); ++i) {
        if (f_name.count(blanks_selected[i])) {
          ...
          editor_msg("red", us_tr("Internal error: mode_blanks closing, "
                                  "missing blanks selected"));
    ...
    case MODE_TIMESHIFT: {
      ...
      QString save_name =
          wheel_file +
          QString("_ts%1").arg(qwtw_wheel->value()).replace(".", "_");

      int ext = 0;
      while (current_files.count(save_name)) {
        save_name = wheel_file + QString("_ts%1-%2")
...
void US_Hydrodyn_Mals_Saxs::wheel_pressed() {
  // qDebug() << "wheel_pressed()";
  wheel_is_pressed = true;
}

void US_Hydrodyn_Mals_Saxs::wheel_released() {
