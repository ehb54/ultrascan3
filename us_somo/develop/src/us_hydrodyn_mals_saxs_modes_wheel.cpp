...
void
US_Hydrodyn_Mals_Saxs::wheel_dec ()
{
...
void
US_Hydrodyn_Mals_Saxs::wheel_inc ()
{
...
void
US_Hydrodyn_Mals_Saxs::wheel_enables (bool enable)
{
  qwtw_wheel->setEnabled (enable);
  pb_wheel_dec->setEnabled (enable
                            && qwtw_wheel->value () > qwtw_wheel->minimum ());
  pb_wheel_inc->setEnabled (enable
                            && qwtw_wheel->value () < qwtw_wheel->maximum ());
}

void
US_Hydrodyn_Mals_Saxs::adjust_wheel (double pos)
{
  ...
  // (unsigned long)le_last_focus ).arg( current_mode );
  ...
  switch (current_mode)
    ...
    case MODE_GUINIER:
      {
        if (!le_last_focus && cb_guinier_scroll->isChecked ())
          {
            // us_qdebug( QString( "guinier scroll is checked value %1" ).arg(
            // pos ) );
    ...
    case MODE_SCALE:
      {
        if (!le_last_focus && cb_scale_scroll->isChecked ())
          {
            // us_qdebug( QString( "scale scroll is checked value %1" ).arg(
            // pos ) );
            lbl_wheel_pos->setText (
                QString ("%1").arg (scale_scroll_selected[pos]));
    ...
    case MODE_BLANKS:
      {
        ...
        le_last_focus->setText (
            QString ("%1").arg (f_qs[wheel_file][(int)pos]));
        ...
        if (current_mode == MODE_GGAUSSIAN)
          {
            ...
            if (cb_fix_dist1->isChecked () && le_gauss_pos_dist1->hasFocus ())
              {
                // cout << "aw: pos dist1 focus\n";
                le_last_focus = le_gauss_pos_dist1;
              }
            if (cb_fix_dist2->isChecked () && le_gauss_pos_dist2->hasFocus ())
              {
                // cout << "aw: pos dist2 focus\n";
                le_last_focus = le_gauss_pos_dist2;
              }
    ...
    case MODE_TIMESHIFT:
      {
        ...
        plot_dist->setCurveData (
            ...
            (double *)&(offset_q[0]), (double *)&(f_Is[wheel_file][0]),
            offset_q.size ());
#else
    ...
    case MODE_SCALE_PAIR:
      ...
      if (!le_last_focus)
        {
          // lbl_wheel_pos->setText( "" );
          scale_pair_scroll_highlight (pos);
        }
      else
        {
          le_last_focus->setText (QString ("%1").arg (pos));
    ...
    default:
      us_qdebug ("adjust wheel called in invalid mode");
      break;
    }
}

void
US_Hydrodyn_Mals_Saxs::wheel_cancel (bool from_wheel_save)
{
  ...
  switch (current_mode)
        ...
        if (cb_ggauss_scroll->isChecked ())
          {
            ...
            for (int i = 0; i < (int)ggaussian_selected_file_index.size ();
                 ++i)
              {
        ...
        if (plotted_curves.count (wheel_file) && f_pos.count (wheel_file))
          {
            ...
            plot_dist->setCurvePen (
                ...
                QPen (plot_colors[f_pos[wheel_file] % plot_colors.size ()],
    ...
    case MODE_WYATT:
      {
        ...
        //             QPen( plot_colors[ f_pos[ wheel_file ] %
        //             plot_colors.size()], use_line_width, Qt::SolidLine));
        //             plot_dist->setCurveSymbol( plotted_curves[ wheel_file ],
        //             symbol );
        // #else
        //             plotted_curves[ wheel_file ]->setPen( QPen( plot_colors[
        //             f_pos[ wheel_file ] % plot_colors.size() ],
        //             use_line_width, Qt::SolidLine ) ); plotted_curves[
        //             wheel_file ]->setSymbol( new QwtSymbol( symbol.style(),
        //             symbol.brush(), symbol.pen(), symbol.size() ) );
        // #endif
        //          }
        plot_files ();
      }
    ...
    case MODE_BLANKS:
      {
        ...
        rescale ();
        if (blanks_in_baseline_mode && blanks_cormap_parameters.size ()
    ...
    case MODE_BASELINE:
      {
        ...
        le_baseline_start_e->setText (
            QString ("%1").arg (org_baseline_start_e));
        ...
        disconnect (le_baseline_end, SIGNAL (textChanged (const QString &)), 0,
                    0);
    ...
    case MODE_SCALE_PAIR:
      ...
      le_scale_pair_scale->setText (
          QString ("%1").arg (scale_pair_original_scale));
      le_scale_pair_sd_scale->setText (
          QString ("%1").arg (scale_pair_original_sd_scale));
      scale_pair_delete_markers ();
      mode_select (MODE_NORMAL);
      break;

    default:
      us_qdebug ("wheel cancel called in invalid mode");
      break;
    }
...
void
US_Hydrodyn_Mals_Saxs::wheel_save ()
{
  clear_smoothed (); // for now always on cancel or save
  clear_oldstyle (); // for now always on cancel or save
  ...
  switch (current_mode)
    ...
    case MODE_GAUSSIAN:
      {
        ...
        for (unsigned int g = 0; g < (unsigned int)gaussians.size ();
             g += gaussian_type_size)
          {
        ...
        for (unsigned int g = 0; g < (unsigned int)gaussians.size ();
          ...
          {
            editor_msg ("darkblue",
                        QString ("Gaussian %1: center %2 height %3 width %4 "
                                 "area %5 % of total %6\n")
                            ...
                            .arg (tot_area != 0e0
                                      ? 100e0 * g_area[g / gaussian_type_size]
                                            / tot_area
                                      : 0e0));
          }
        ...
        for (int i = 0; i < (int)blanks_selected.size (); ++i)
          {
            if (f_name.count (blanks_selected[i]))
              ...
              {
                editor_msg ("red", us_tr ("Internal error: mode_blanks "
                                          "closing, missing blanks selected"));
    ...
    case MODE_TIMESHIFT:
      {
        ...
        QString save_name = wheel_file
                            + QString ("_ts%1")
                                  .arg (qwtw_wheel->value ())
                                  .replace (".", "_");

        int ext = 0;
        while (current_files.count (save_name))
          {
            save_name = wheel_file
                        + QString ("_ts%1-%2")
                              .arg (qwtw_wheel->value ())
                              .arg (++ext)
                              .replace (".", "_");
        ...
        for (unsigned int i = 0; i < (unsigned int)f_qs[save_name].size ();
             i++)
          {
            f_qs[save_name][i] += qwtw_wheel->value ();
            f_qs_string[save_name][i]
                = QString ("%1").arg (f_qs[save_name][i]);
          }
        ...
        f_conc[save_name]
            = f_conc.count (wheel_file) ? f_conc[wheel_file] : 0e0;
        f_psv[save_name] = f_psv.count (wheel_file) ? f_psv[wheel_file] : 0e0;
        f_I0se[save_name]
            = f_I0se.count (wheel_file) ? f_I0se[wheel_file] : 0e0;
        {
          vector<double> tmp;
        ...
        if (QMessageBox::Yes
            == QMessageBox::question (
                this,
                windowTitle ()
                    + us_tr (": Timeshift : set concentration file"),
                us_tr ("Would you like to *set* the timeshifted concentration "
                       "file?"),
      ...
      if (scale_pair_save_names.size ())
        {
          QMessageBox::warning (
              ...
              QString (us_tr (
                  ...
                  "If you scale again when you join, you will get double "
                  "scaling!")));
        }
      scale_pair_created_remove ();
      scale_pair_delete_markers ();
    ...
    default:
      us_qdebug ("wheel save called in invalid mode");
      break;
    }
}

void
US_Hydrodyn_Mals_Saxs::wheel_pressed ()
{
...
void
US_Hydrodyn_Mals_Saxs::wheel_released ()
{
