...
//     bool US_PM::"object"( int model_pos, vector < double > & params, vector <
//     double > & params_left, set < pm_point > & model )
...
bool US_PM::best_md0(vector<double>& params, vector<double>& low_fparams,
  ...
  switch (this_type) {
    case SPHERE:
      return best_sphere(params, low_fparams, high_fparams, model);
      break;
    case CYLINDER:
      return best_cylinder(params, low_fparams, high_fparams, model);
      break;
    case SPHEROID:
      return best_spheroid(params, low_fparams, high_fparams, model);
      break;
    case ELLIPSOID:
      return best_ellipsoid(params, low_fparams, high_fparams, model);
      break;
    case TORUS:
      return best_torus(params, low_fparams, high_fparams, model);
      break;
      // case TORUS_SEGMENT : return best_torus_segment( params, low_fparams,
      // high_fparams, model ); break;
    default:
      error_msg = "best_md0: invalid type";
      return false;
      break;
  }
  return false;
...
bool US_PM::create_1_model(int model_pos, vector<double>& params,
  ...
  switch ((int)params[0]) {
   ...
   case TORUS_SEGMENT :
    ...
    default:
      error_msg = QString("US_PM::create_1_model: object type %1 not defined")
                      .arg((int)params[0]);
      return false;
      break;
  }
...
bool US_PM::sphere(int model_pos, vector<double>& params,
                   vector<double>& params_left, set<pm_point>& model) {
  debug(1, "sphere");
  int ofs = 1;
  ...
  double one_over_radius2 = 1e0 / ((radius + bead_radius_over_2gcf) *
                                   (radius + bead_radius_over_2gcf));

  debug(2, QString("sphere @ %1 %2 %3 radius %4")
               .arg(centerx)
               .arg(centery)
               .arg(centerz)
               .arg(radius));
...
bool US_PM::cylinder(int model_pos, vector<double>& params,
                     vector<double>& params_left, set<pm_point>& model) {
  debug(1, "cylinder----------------------------------------");
  int ofs = 1;
  double radius = params[ofs++];
  ...
  double vh[3] = {endx - basex, endy - basey, endz - basez};

  double h2 = vh[0] * vh[0] + vh[1] * vh[1] + vh[2] * vh[2];

  double lh2 = sqrt(h2);

  double nvh[3] = {vh[0] / lh2, vh[1] / lh2, vh[2] / lh2};
  ...
  for (int x = minx; x <= maxx; ++x) {
    for (int y = miny; y <= maxy; ++y) {
      for (int z = minz; z <= maxz; ++z) {
        ...
        double p_b[3] = {(double)x - basex, (double)y - basey,
                         (double)z - basez};

        double dot_p_b_d = p_b[0] * nvh[0] + p_b[1] * nvh[1] + p_b[2] * nvh[2];

        double d_dot_p_b_d[3] = {nvh[0] * dot_p_b_d, nvh[1] * dot_p_b_d,
                                 nvh[2] * dot_p_b_d};

        double p_b_minus_d_dot_p_b_d[3] = {p_b[0] - d_dot_p_b_d[0],
                                           p_b[1] - d_dot_p_b_d[1],
                                           p_b[2] - d_dot_p_b_d[2]};

        r2 = p_b_minus_d_dot_p_b_d[0] * p_b_minus_d_dot_p_b_d[0] +
        ...
        if (r2 LE_OR_LT radius2 + bead_radius_over_2gcf) {
          if (p_b[0] * p_b[0] + p_b[1] * p_b[1] + p_b[2] * p_b[2] LE_OR_LT h2 +
              r2 + bead_radius_over_2gcf) {
            double p_e[3] = {(double)x - endx, (double)y - endy,
                             (double)z - endz};

            if (p_e[0] * p_e[0] + p_e[1] * p_e[1] +
...
bool US_PM::ellipsoid(int model_pos, vector<double>& params,
                      vector<double>& params_left, set<pm_point>& model) {
  debug(1, "ellipsoid");
  int ofs = 1;
  ...
  double one_over_radiusa2 = 1e0 / ((radiusa + bead_radius_over_2gcf) *
                                    (radiusa + bead_radius_over_2gcf));
  double one_over_radiusb2 = 1e0 / ((radiusb + bead_radius_over_2gcf) *
                                    (radiusb + bead_radius_over_2gcf));
  double one_over_radiusc2 = 1e0 / ((radiusc + bead_radius_over_2gcf) *
                                    (radiusc + bead_radius_over_2gcf));
  ...
  if (model_pos && theta) {
    ...
    for (int x = minx; x <= maxx; ++x) {
      for (int y = miny; y <= maxy; ++y) {
        for (int z = minz; z <= maxz; ++z) {
          ...
          if (one_over_radiusa2 * (v_p_c[0] * v_p_c[0]) +
    ...
    for (int x = minx; x <= maxx; ++x) {
      for (int y = miny; y <= maxy; ++y) {
        for (int z = minz; z <= maxz; ++z) {
          double v_p_c[3] = {(double)x - centerx, (double)y - centery,
                             (double)z - centerz};
...
bool US_PM::spheroid(int model_pos, vector<double>& params,
                     vector<double>& params_left, set<pm_point>& model) {
  debug(1, "spheroid");
  int ofs = 1;
  double radiusa = params[ofs++];
    ...
    for (int x = minx; x <= maxx; ++x) {
      for (int y = miny; y <= maxy; ++y) {
        for (int z = minz; z <= maxz; ++z) {
          ...
          if (one_over_radiusa2 * (v_p_c[0] * v_p_c[0] + v_p_c[1] * v_p_c[1]) +
...
bool US_PM::torus_segment(int model_pos, vector<double>& params,
                          vector<double>& params_left, set<pm_point>& model) {
  debug(1, "torus_segment");
  int ofs = 1;
  double radiusa = params[ofs++];
  double radiusb = params[ofs++];
  /* double enda    = */ params[ofs++];  // a %'ge ? or an angle.  assume start
                                         // with 0 moduli 2PI or 100% maybe
                                         // require limits on params
...
bool US_PM::torus(int model_pos, vector<double>& params,
                  vector<double>& params_left, set<pm_point>& model) {
  debug(1, "torus");
  int ofs = 1;
  double radiusa = params[ofs++];
  ...
  double radiusa2 =
      (radiusa + bead_radius_over_2gcf) * (radiusa + bead_radius_over_2gcf);
  // double oneoverradiusa2 = 1e0 / ( ( radiusa + bead_radius_over_2gcf ) * (
  // radiusa + bead_radius_over_2gcf ) );
  double oneoverradiusb2 = 1e0 / ((radiusb + bead_radius_over_2gcf) *
                                  (radiusb + bead_radius_over_2gcf));
