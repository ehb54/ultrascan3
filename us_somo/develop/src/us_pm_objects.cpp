...
bool
US_PM::best_md0 (vector<double> &params, vector<double> &low_fparams,
...
QString
US_PM::get_name (vector<int> &types)
{
  QString result;
...
bool
US_PM::create_1_model (int model_pos, vector<double> &params,
...
bool
US_PM::sphere (int model_pos, vector<double> &params,
               vector<double> &params_left, set<pm_point> &model)
{
  debug (1, "sphere");
  int ofs = 1;
...
bool
US_PM::cylinder (int model_pos, vector<double> &params,
                 vector<double> &params_left, set<pm_point> &model)
{
  debug (1, "cylinder----------------------------------------");
  int ofs = 1;
          ...
          for (int z = minz; z <= maxz; ++z)
            {
              ...
              if (r2 LE_OR_LT radius2 + bead_radius_over_2gcf)
                {
                  if (p_b[0] * p_b[0] + p_b[1] * p_b[1]
                      + p_b[2] * p_b[2] LE_OR_LT h2 + r2
                      + bead_radius_over_2gcf)
                    {
                      double p_e[3] = { (double)x - endx, (double)y - endy,
                                        (double)z - endz };

                      if (p_e[0] * p_e[0] + p_e[1] * p_e[1]
                          + p_e[2] * p_e[2] LE_OR_LT h2 + r2
                          + bead_radius_over_2gcf)
                        {
                          pmp.x[0] = (int16_t)x;
                          pmp.x[1] = (int16_t)y;
...
bool
US_PM::ellipsoid (int model_pos, vector<double> &params,
                  vector<double> &params_left, set<pm_point> &model)
{
  debug (1, "ellipsoid");
  int ofs = 1;
  ...
  double magnorminv
      = 1e0 / sqrt (normx * normx + normy * normy + normz * normz);
  normx *= magnorminv;
  normy *= magnorminv;
  normz *= magnorminv;
  ...
  double one_over_radiusa2 = 1e0
                             / ((radiusa + bead_radius_over_2gcf)
                                * (radiusa + bead_radius_over_2gcf));
  double one_over_radiusb2 = 1e0
                             / ((radiusb + bead_radius_over_2gcf)
                                * (radiusb + bead_radius_over_2gcf));
  double one_over_radiusc2 = 1e0
...
bool
US_PM::spheroid (int model_pos, vector<double> &params,
                 vector<double> &params_left, set<pm_point> &model)
{
  debug (1, "spheroid");
  int ofs = 1;
              ...
              for (int z = minz; z <= maxz; ++z)
                {
                  ...
                  if (one_over_radiusa2
                          * (v_p_c[0] * v_p_c[0] + v_p_c[1] * v_p_c[1])
...
bool
US_PM::torus_segment (int model_pos, vector<double> &params,
                      vector<double> &params_left, set<pm_point> &model)
{
...
bool
US_PM::torus (int model_pos, vector<double> &params,
              vector<double> &params_left, set<pm_point> &model)
{
  debug (1, "torus");
  int ofs = 1;
                  ...
                  for (double z = (double)minz; z <= (double)maxz; z += ofsx)
                    {
                      ...
                      if (oneoverradiusb2
                          * (((double)x - c[0]) * ((double)x - c[0])
                             + ((double)y - c[1]) * ((double)y - c[1])
                             + ((double)z - c[2]) * ((double)z - c[2]))
                              LE_OR_LT 1e0)
                        {
                          apply_rotation_matrix (rm, x, y, z, newx, newy,
                                                 newz);
