...
void US_Hydrodyn_PDB_Parsing::setupGUI()
{
   ...
   cb_use_WAT_Tf->setText(us_tr(" Use explicit WAT radii as specified in PDB the T-factor field"));
   cb_use_WAT_Tf->setEnabled( true );
   cb_use_WAT_Tf->setChecked( ((US_Hydrodyn *)us_hydrodyn)->gparams.count( "use_WAT_Tf_pdb" ) &&
                                    ((US_Hydrodyn *)us_hydrodyn)->gparams[ "use_WAT_Tf_pdb" ] == "true" );
   cb_use_WAT_Tf->setMinimumHeight(minHeight1);
   cb_use_WAT_Tf->setFont(QFont( USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
   ...
   default : qDebug() << "pdb parsing missing residues selection error"; break;
   }
   ...
   default : qDebug() << "pdb parsing missing atoms selection error"; break;
   }
...
{
   ((US_Hydrodyn *)us_hydrodyn)->gparams[ "use_WAT_Tf_pdb" ] = cb_use_WAT_Tf->isChecked() ? "true" : "false";
   // ((US_Hydrodyn *)us_hydrodyn)->display_default_differences();
}
