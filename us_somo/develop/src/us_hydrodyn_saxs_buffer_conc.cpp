...
US_Hydrodyn_Saxs_Buffer_Conc::US_Hydrodyn_Saxs_Buffer_Conc (
    csv &csv1, void *saxs_buffer_window, QWidget *p, const char *)
    : QFrame (p)
{
  org_csv = &csv1;
  this->csv1 = csv1;
  ...
  setWindowTitle (us_tr ("US-SOMO: SAXS Buffer: File Concentrations"));
...
void
US_Hydrodyn_Saxs_Buffer_Conc::refresh (csv &ref_csv)
{
  csv new_csv = ref_csv;
...
void
US_Hydrodyn_Saxs_Buffer_Conc::setupGUI ()
{
  int minHeight1 = 30;
...
void
US_Hydrodyn_Saxs_Buffer_Conc::cancel ()
{
  close ();
}

void
US_Hydrodyn_Saxs_Buffer_Conc::set_ok ()
{
  *org_csv = current_csv ();
...
void
US_Hydrodyn_Saxs_Buffer_Conc::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Saxs_Buffer_Conc::closeEvent (QCloseEvent *e)
{
...
void
US_Hydrodyn_Saxs_Buffer_Conc::sort_column (int section)
{
...
csv
US_Hydrodyn_Saxs_Buffer_Conc::current_csv ()
{
  csv tmp_csv = csv1;
...
void
US_Hydrodyn_Saxs_Buffer_Conc::copy ()
{
  csv1 = current_csv ();
...
void
US_Hydrodyn_Saxs_Buffer_Conc::paste ()
{
  csv1 = current_csv ();
  unsigned int pos = 0;
  for (int i = 0; i < t_csv->rowCount (); i++)
...
void
US_Hydrodyn_Saxs_Buffer_Conc::paste_all ()
{
  csv1 = current_csv ();
  unsigned int pos = 0;
  for (int i = 0; i < t_csv->rowCount (); i++)
...
void
US_Hydrodyn_Saxs_Buffer_Conc::update_enables ()
{
  if (!disable_updates)
    {
...
void
US_Hydrodyn_Saxs_Buffer_Conc::row_header_released (int row)
{
...
void
US_Hydrodyn_Saxs_Buffer_Conc::reload_csv ()
{
  t_csv->setRowCount (csv1.data.size ());
  for (unsigned int i = 0; i < csv1.data.size (); i++)
    {
...
void
US_Hydrodyn_Saxs_Buffer_Conc::load ()
{
  ...
  if (!QFile::exists (fname))
    {
      QMessageBox::warning (
          this,
          us_tr ("US-SOMO: SAXS Buffer Subtraction Utility: Concentrations"),
  ...
  if (QFileInfo (fname).suffix ().contains (QRegExp ("^(sbc|SBC)$")))
  ...
  else
    {
      US_Hydrodyn_Saxs_Buffer_Conc_Load *hbscl
          = new US_Hydrodyn_Saxs_Buffer_Conc_Load (qsl_lines, csv1, this);
      ...
      if (hbscl->disp_csv.data.size ())
        ...
        {
          QMessageBox::warning (
              this, us_tr ("US-SOMO: SAXS Buffer: File Concentrations"),
...
void
US_Hydrodyn_Saxs_Buffer_Conc::save ()
{
  QString use_dir = QDir::currentPath ();
  ...
  QMessageBox::information (
      this, us_tr ("US-SOMO: SAXS Buffer Subtraction Utility: Concentrations"),
      QString (us_tr ("File %1 saved")).arg (fname));
}

QString
US_Hydrodyn_Saxs_Buffer_Conc::csv_to_qstring (csv from_csv)
{
  QString qs;
...
QStringList
US_Hydrodyn_Saxs_Buffer_Conc::csv_parse_line (QString qs)
{
