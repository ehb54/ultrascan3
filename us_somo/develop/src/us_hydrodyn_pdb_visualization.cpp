...
void
US_Hydrodyn_PDB_Visualization::setupGUI ()
{
  int minHeight1 = 30;
  ...
  cb_default->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  cb_spacefilling->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize));
  ...
  cb_custom->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize));
  ...
  rb_default->setFont (QFont (USglobal->config_list.fontFamily,
                              USglobal->config_list.fontSize));
  ...
  rb_spacefilling->setFont (QFont (USglobal->config_list.fontFamily,
                                   USglobal->config_list.fontSize));
  rb_spacefilling->setPalette (PALET_NORMAL);
  AUTFBACK (rb_spacefilling);
  connect (rb_spacefilling, SIGNAL (clicked ()), this,
           SLOT (select_option ()));
  ...
  rb_custom->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize));
  ...
  le_filename->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize));
...
void
US_Hydrodyn_PDB_Visualization::select_option ()
{
...
void
US_Hydrodyn_PDB_Visualization::select_option (int val)
{
  (*pdb).visualization = val;
}

void
US_Hydrodyn_PDB_Visualization::update_filename (const QString &filename)
{
  (*pdb).filename = filename;
}

void
US_Hydrodyn_PDB_Visualization::select_filename ()
{
...
void
US_Hydrodyn_PDB_Visualization::cancel ()
{
  close ();
}

void
US_Hydrodyn_PDB_Visualization::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_PDB_Visualization::closeEvent (QCloseEvent *e)
{
  *pdb_visualization_widget = false;
  global_Xpos -= 30;
  global_Ypos -= 30;
