...
void
US_Hydrodyn_Saxs::plot_one_pr (vector<double> r, vector<double> pr,
                               QString name, bool skip_mw)
{
...
void
US_Hydrodyn_Saxs::plot_one_pr (vector<double> r, vector<double> pr,
                               vector<double> pr_error, QString name,
                               bool /* skip_mw */
                               ,
                               bool do_replot)
{
  ...
  plotted_pr_mw.push_back (
      cb_normalize->isChecked () ? get_mw (name, false, true) : -1e0);
  ...
  {
    if (cb_pr_eb->isChecked () && pr_error.size ()
        && pr_error.size () == r.size ())
      {
        {
          QwtPlotCurve *curve = new QwtPlotCurve (plot_name);
          curve->setStyle (QwtPlotCurve::NoCurve);
          curve->setSamples ((double *)&(r[0]), (double *)&(pr[0]),
                             (int)r.size ());
          curve->setPen (QPen (plot_colors[p % plot_colors.size ()],
                               6 + pen_width * 1, Qt::SolidLine));
          QwtSymbol sym;
        ...
        for (int i = 0; i < (int)pr_error.size (); ++i)
          {
            ...
            curve->setSamples ((double *)&(eb_x[0]), (double *)&(eb_y[0]), 2);
            curve->setPen (QPen (plot_colors[p % plot_colors.size ()],
  ...
  if (do_replot)
    {
      editor_msg (plot_colors[p % plot_colors.size ()],
                  plot_saxs->canvasBackground ().color (), name);
    }
...
void
US_Hydrodyn_Saxs::set_plot_pr_range (double &minx, double &maxx, double &miny,
                                     double &maxy)
{
  minx = 0e0;
  maxx = 1e0;
  ...
  if (plotted_r.size () && plotted_r[0].size ())
    {
      minx = plotted_r[0][0];
  ...
  for (unsigned int i = 0; i < plotted_pr.size (); i++)
    {
      if (plotted_pr[i].size ())
        {
          bool has_error
              = plotted_pr_error[i].size () == plotted_pr[i].size ();
...
void
US_Hydrodyn_Saxs::plot_one_iqq (vector<double> q, vector<double> I,
                                QString name)
{
...
void
US_Hydrodyn_Saxs::plot_one_iqq (vector<double> q, vector<double> I,
                                vector<double> I_error, QString name)
{
  if (q.size () < I.size ())
    {
      q.resize (I.size ());
  ...
  if (our_saxs_options->compute_exponentials)
    {
      ...
      // simple test:
      {
        ...
        double ffv = compute_ff (saxsC, saxsH, gly, C, C, 0, 2e0,
                                 pow (2e0 / (4e0 * M_PI), 2));
      ...
      if (is_zero_vector (I_error))
          ...
          do
            {
              ...
              text = US_Static::getText (
                  ...
                  us_tr ("If you want to compute structure factors for this "
                         "curve\n"
                         "Please define a dummy saxs name or press CANCEL to "
                         "skip this computation.\n"
                         + dupmsg + "Name for these structure factors:"),
                  ...
                  if (saxs_map.count (text))
                    {
                      switch (QMessageBox::question (
                          ...
                          us_tr ("US-SOMO: Compute structure factors : "
                                 "duplicate name"),
                          QString (
                              us_tr ("The name %1 was previously defined\n"
                                     "Do you want to overwrite the values?"))
                              .arg (text),
                          us_tr ("&Overwrite"), us_tr ("&Try again"),
                          QString (), 1, 0))
                        {
                        case 0:
                          break;
          ...
          if (ok)
            {
              ...
              QMessageBox::information (
                  this,
                  us_tr ("US-SOMO: Compute structure factors : start "
                         "computation"),
                  QString (us_tr ("Start computation\n"
                                  "NOTE: This could take some time and the "
                                  "program will appear unresponsive")));

              if (!usu.compute_exponential (
                      q, I, coeff4, coeff5, coeffv, norm4, norm5, normv,
                      our_saxs_options->bead_models_use_var_len_sf
                          ? our_saxs_options->bead_models_var_len_sf_max
                          : 5))
                {
              ...
              else
                {
                  ...
                  QString qs4 = text;
                  coeff4.resize (9);
                  for (unsigned int i = 1; i < (unsigned int)coeff4.size ();
                       i++)
                    {
                  ...
                  editor_msg (
                      "dark blue",
                      QString (
                          "Exponentials 4 terms norm %1 nnorm %2 (%3): %4")
                          .arg (norm4)
                          .arg (nnorm4)
                          .arg (norm4tag)
                          .arg (qs4));
                  ...
                  QString qs5 = text;
                  for (unsigned int i = 1; i < (unsigned int)coeff5.size ();
                       i++)
                    {
                  ...
                  editor_msg (
                      "dark blue",
                      QString (
                          "Exponentials 5 terms norm %1 nnorm %2 (%3): %4")
                          .arg (norm5)
                          .arg (nnorm5)
                          .arg (norm5tag)
                          .arg (qs5));
                  switch (QMessageBox::question (
                      ...
                      QString (
                          ...
                          .arg (nnorm5)
                          .arg (norm5tag),
                      us_tr ("&Save to file"), us_tr ("&Forget them")))
                    {
                    case 0:
                      {
                        // add to saxs_map
                        ...
                        // get excluded volume
                        {
                          ...
                          double res = US_Static::getDouble (
                              us_tr ("US-SOMO: Compute structure factors : "
                                     "enter excluded volume"),
                              "Enter a default excluded volume in Angstroms^3 "
                              "(can be zero):",
                              0, 0, 1e20, 4, &ok, this);
                          if (ok)
                            {
                          ...
                          else
                            {
                              ...
                              QMessageBox::information (
                                  ...
                                  us_tr ("US-SOMO: Compute structure factors "
                                         ": excluded volume"),
                                  QString (
                                      us_tr ("Excluded volume is set to %1\n"))
                        ...
                        if (add_to_saxs_list)
                          ...
                          {
                            for (unsigned int i = 0; i < saxs_list.size ();
                                 i++)
                              {
                        ...
                        // now write out file
                        {
                          QString org_filename
                              = our_saxs_options->default_saxs_filename;
                          bool ok;
                          QFile f;
                          QString filename;
                          ...
                          do
                            {
                              ...
                              filename = QFileDialog::getSaveFileName (
                                  ...
                                  us_tr ("US-SOMO: Compute structure factors "
                                         ": save data"),
                                  org_filename, "*.saxs_atoms *.SAXS_ATOMS");
                              ...
                              if (!filename.isEmpty ())
                                {
                                  ...
                                  switch (QMessageBox::question (
                                      this,
                                      us_tr ("US-SOMO: Compute structure "
                                             "factors : save data"),
                          ...
                          if (ok && !filename.isEmpty ())
                            {
                              ...
                              f.close ();
                              if (our_saxs_options->default_saxs_filename
                                  != filename)
                                {
                                  our_saxs_options->default_saxs_filename
                                      = filename;
                                  QMessageBox::information (
                                      this,
                                      us_tr (
                                          "US-SOMO: Compute structure factors "
                                          ": new default saxs filename"),
                                      QString (us_tr ("The default saxs atoms "
                                                      "file is now %1\n"))
                                          .arg (filename));
                                  lbl_saxs_table->setText (
                              ...
                              if (our_saxs_options->dummy_saxs_name != text)
                                {
                                  ...
                                  QMessageBox::information (
                                      this,
                                      us_tr ("US-SOMO: Compute structure "
                                             "factors : new default saxs "
                                             "dummy atom name"),
                                      QString (us_tr ("The default dummy atom "
                                                      "name is now %1\n"))
                                          .arg (text));
                                }
                              select_saxs_file (filename);
                              ((US_Hydrodyn *)us_hydrodyn)
                                  ->saxs_util->select_saxs_file (filename);
                            }
                        }
                      }
        ...
        {
          editor_msg ("dark red",
                      QString (us_tr ("Skipped: Computing %1 term "
                                      "exponentials, since I(q) has errors"))
...

  {
    ...
    curve->setSamples (cb_guinier->isChecked () ? (double *)&(q2[0])
                                                : (double *)&(q[0]),
                       (double *)&(I[0]), q.size ());
    curve->setPen (
...
void
US_Hydrodyn_Saxs::push_back_zero_I_error ()
{
  if (!plotted_I.size ())
    {
      return;
...
bool
US_Hydrodyn_Saxs::is_zero_vector (vector<double> &v)
{
  bool all_zero = true;
...
bool
US_Hydrodyn_Saxs::is_nonzero_vector (vector<double> &v)
{
...
QString
US_Hydrodyn_Saxs::Iq_plotted_summary ()
{
  QString qs = "Iq_plotted_summary:\n";

  qs += QString ("plotted_q.size() %1\n"
                 "plotted_q2.size() %2\n"
                 "plotted_I.size() %3\n"
  ...
  for (unsigned int i = 0; i < plotted_q.size (); i++)
    {
      ...
      qs += QString ("pos %1 q.size() %2 q2.size() %3 I.size() %4 "
                     "I_error.size() %5 %6 %7 q[%8:%9]\n")
                .arg (i)
...
void
US_Hydrodyn_Saxs::do_plot_resid ()
{
  ...
  plot_resid->detachItems (QwtPlotItem::Rtti_PlotCurve);
  plot_resid->detachItems (QwtPlotItem::Rtti_PlotMarker);
  ;
                      ...
                      if (cb_resid_pct->isChecked ())
                        {
                          e.push_back (
                              ...
                              * (log (plotted_q[i][q] * plotted_I[i][q] > 0e0
                                          ? plotted_q[i][q] * plotted_I[i][q]
                                          : 1e-99)
                                 - (plotted_cs_guinier_a[i]
                                    + plotted_cs_guinier_b[i]
                                          * plotted_q2[i][q]))
                              / fabs (
                                  log (plotted_q[i][q] * plotted_I[i][q] > 0e0
                                           ? plotted_q[i][q] * plotted_I[i][q]
                          ...
                          if (cb_resid_sd->isChecked ())
                            {
                              if ((int)plotted_I_error.size () > i
                                ...
                                {
                                  e.push_back (
                                      (log (plotted_q[i][q] * plotted_I[i][q]
                                                    > 0e0
                                                ? plotted_q[i][q]
                                                      * plotted_I[i][q]
                                                : 1e-99)
                                  ...
                                  * (log (plotted_q[i][q] * plotted_I[i][q]
                                     ...
                                     - (plotted_cs_guinier_a[i]
                                        + plotted_cs_guinier_b[i]
                                              * plotted_q2[i][q]))
                                  / fabs (log (
                                      plotted_q[i][q] * plotted_I[i][q] > 0e0
                                          ? plotted_q[i][q] * plotted_I[i][q]
                                          : 1e-99)));
                            }
                          else
                  ...
                  if (plotted_cs_guinier_pts_removed.count (i)
                      && plotted_cs_guinier_pts_removed[i].size ())
                    {
                      do_plot_resid (x, y, e,
                                     ...
                                     plot_colors[i % plot_colors.size ()]);
                    }
                  else
                    {
                      do_plot_resid (x, y, e,
                          ...
                          if (cb_resid_pct->isChecked ())
                            {
                              e.push_back (
                                  100e0
                                  * (log (plotted_q[i][q] * plotted_q[i][q]
                                                      * plotted_I[i][q]
                                                  > 0e0
                                              ? plotted_q[i][q]
                                                    * plotted_q[i][q]
                                                    * plotted_I[i][q]
                                              : 1e-99)
                                     - (plotted_Rt_guinier_a[i]
                                        + plotted_Rt_guinier_b[i]
                                              * plotted_q2[i][q]))
                                  / fabs (log (
                                      plotted_q[i][q] * plotted_q[i][q]
                                                  * plotted_I[i][q]
                                              > 0e0
                                          ? plotted_q[i][q] * plotted_q[i][q]
                                                * plotted_I[i][q]
  ...
  if (some_pts_skipped)
    {
      editor_msg (
          "dark red",
          us_tr (
              "Warning: points with zero s.d.'s or I(q)'s are not displayed"));
    }

  if (any_plotted)
    {
#if defined(OLD_WAY)
      plot_resid->setAxisTitle (
          QwtPlot::yLeft,
          us_tr (
...

      {
        ...
        curve->setSamples ((double *)&x_d2[0], (double *)&y_d[0], 2);
        curve->attach (plot_resid);
      }

      if (cb_guinier->isChecked ())
        {
          minq2 = le_guinier_lowq2->text ().toDouble ()
                      ? le_guinier_lowq2->text ().toDouble ()
                      : minq2;

          maxq2 = le_guinier_highq2->text ().toDouble ()
      ...
      plot_resid->setAxisScale (QwtPlot::yLeft, -maxabse * 1.2e0,
                                maxabse * 1.2e0);
      ...
      // connect( plot_resid_zoomer, SIGNAL( zoomed( const QRectF & ) ), SLOT(
      // resid_zoomed( const QRectF & ) ) );

      plot_saxs->replot ();
      plot_resid->replot ();
      set_resid_show ();
      if (started_in_expert_mode && plotted_q.size () == 1)
        {
...
void
US_Hydrodyn_Saxs::resid_zoomed (const QRect &)
{
  // qDebug() << "_Saxs::resid_zoomed()";
}

void
US_Hydrodyn_Saxs::set_resid_pct ()
{
  if (cb_resid_sd->isChecked ())
    {
  ...
  ((US_Hydrodyn *)us_hydrodyn)->gparams["saxs_cb_resid_pct"]
      = cb_resid_pct->isChecked () ? "1" : "0";

  // plot_resid->plotLayout()->setAlignCanvasToScale( QwtPlot::xTop,
  // !plot_resid->plotLayout()->alignCanvasToScale( QwtPlot::xTop    ) );
  // plot_saxs ->plotLayout()->setAlignCanvasToScale( QwtPlot::xBottom,
  // !plot_saxs ->plotLayout() ->alignCanvasToScale( QwtPlot::xBottom ) );
  do_plot_resid ();
  emit pb_rescale_y->clicked ();
}

void
US_Hydrodyn_Saxs::set_resid_sd ()
{
  if (cb_resid_pct->isChecked ())
    {
  ...
  do_plot_resid ();
  ((US_Hydrodyn *)us_hydrodyn)->gparams["saxs_cb_resid_sd"]
      = cb_resid_sd->isChecked () ? "1" : "0";
  emit pb_rescale_y->clicked ();
}

void
US_Hydrodyn_Saxs::set_manual_guinier ()
{
...
void
US_Hydrodyn_Saxs::set_resid_show ()
{
  if (cb_resid_show->isChecked ())
    {
      ...
      connect (((QObject *)plot_saxs->axisWidget (QwtPlot::xBottom)),
               SIGNAL (scaleDivChanged ()), usp_plot_resid,
               SLOT (scaleDivChangedXSlot ()), Qt::UniqueConnection);
      connect (((QObject *)plot_resid->axisWidget (QwtPlot::xBottom)),
               SIGNAL (scaleDivChanged ()), usp_plot_saxs,
               SLOT (scaleDivChangedXSlot ()), Qt::UniqueConnection);
  ...
  ((US_Hydrodyn *)us_hydrodyn)->gparams["saxs_cb_resid_show"]
      = cb_resid_show->isChecked () ? "1" : "0";
}

void
US_Hydrodyn_Saxs::set_resid_show_errorbars ()
{
  ((US_Hydrodyn *)us_hydrodyn)->gparams["saxs_cb_resid_show_errorbars"]
      = cb_resid_show_errorbars->isChecked () ? "1" : "0";
  set_guinier ();
  emit pb_rescale_y->clicked ();
}

void
US_Hydrodyn_Saxs::do_plot_resid (vector<double> &x, vector<double> &y,
                                 vector<double> &e, QColor qc)
{
  {
    QwtPlotCurve *curve = new QwtPlotCurve ("base");
    curve->setStyle (QwtPlotCurve::Lines);
...
void
US_Hydrodyn_Saxs::do_plot_resid (vector<double> &x_d, vector<double> &e_d,
                                 QColor qc)
{
  {
    QwtPlotCurve *curve = new QwtPlotCurve ("errors");
    ...
    curve->setSamples ((double *)&x_d[0], (double *)&e_d[0], x_d.size ());
    curve->setStyle (QwtPlotCurve::Sticks);
    curve->attach (plot_resid);
  }
}
...
US_Hydrodyn_Saxs::do_plot_resid (vector<double> &x, vector<double> &y,
                                 vector<double> &e,
                                 map<double, double> &pts_removed, QColor qc)
{
...
US_Hydrodyn_Saxs::pp ()
{
  ...
  for (map<QString, US_Hydrodyn_Saxs_Iqq_Residuals *>::iterator it
       = saxs_iqq_residuals_windows.begin ();
       it != saxs_iqq_residuals_windows.end (); ++it)
    {
      if (it->second && saxs_iqq_residuals_widgets.count (it->first)
          && saxs_iqq_residuals_widgets[it->first] && it->second->plot)
        {
          use_plot_info["US-SOMO SAXS Residuals " + it->first]
              = it->second->plot;
        }
    }
  ...
  if (saxs_residuals_widget)
    {
      use_plot_info["US-SOMO SAXS PRR Residuals"]
          = saxs_residuals_window->plot;
    }

  // save and restore flags since SD errorbars duplicate Err. Maybe should
  // remove sd errorbars flag or mirror
...
void
US_Hydrodyn_Saxs::fix_xBottom ()
{
  plot_saxs->enableAxis (QwtPlot::xBottom, !plot_resid->isVisible ());
}

void
US_Hydrodyn_Saxs::do_rescale ()
{
  // qDebug() << "do_rescale";
}

void
US_Hydrodyn_Saxs::do_rescale_y ()
{
  qDebug () << "do_rescale_y";
  ...
  plot_resid->setMaximumHeight (
      (int)(0.2 * (plot_resid->height () + plot_saxs->height ())));
  // plot_resid->replot();
  ...
  // ); plot_resid->plotLayout()->setAlignCanvasToScales( align ); plot_saxs
  // ->plotLayout()->setAlignCanvasToScales( align ); plot_resid->replot();
  // plot_saxs->replot();
}

void
US_Hydrodyn_Saxs::resid_resized ()
{
...
US_Hydrodyn_Saxs::pr_replot ()
{
  ...
  vector<vector<double> > save_plotted_pr_not_normalized
      = plotted_pr_not_normalized;
  vector<vector<double> > save_plotted_pr_not_normalized_error
      = plotted_pr_not_normalized_error;
  ...
  for (int i = 0; i < (int)save_plotted_r.size (); ++i)
    {
      plot_one_pr (save_plotted_r[i], save_plotted_pr[i],
                   save_plotted_pr_error[i], save_qsl_plotted_pr_names[i],
                   true, false);
    }
...
void
US_Hydrodyn_Saxs::set_pr_sd (vector<double> &r, vector<double> &pr,
                             vector<double> &pr_error)
{
  if (!pr_error.size ())
    {
    ...
    if (has_non_pos_sds)
      {
        ...
        for (int i = 0; i < (int)pr_error.size (); ++i)
          {
            if (pr_error[i] <= 0)
              {
                double new_pr_sd;
                if (!usu.apply_natural_spline (r_nz, pr_error_nz, y2, r[i],
                                               new_pr_sd))
                  {
                    qDebug ()
                        << "error set_pr_sd() error applying natural spline";
                  }
                else
                  {
        ...
        if (has_non_pos_sds)
          {
            qDebug () << "error set_pr_sd() still has non positive SDs after "
                         "natural spline!";
            return;
          }
      }
...
void
US_Hydrodyn_Saxs::crop_pr_tail (vector<double> &r, vector<double> &pr,
                                vector<double> &pr_error)
{
...
void
US_Hydrodyn_Saxs::prop_pr_sd_tail (vector<double> &pr_error, unsigned int len)
{
  if (!pr_error.size ())
    {
      // no errors
...
void
US_Hydrodyn_Saxs::pad_pr_plotted ()
{
  ...
  if (needs_interp)
    {
      ...
      editor_msg ("darkred", "Loaded curves have varying r spacing, "
                             "interpolating all to current bin size\n");
      ...
      for (int i = 0; i < (int)plotted_r.size (); ++i)
        {
          if (plotted_r[i].size () > 2)
            {
              for (double r = 0; r <= plotted_r[i].back ();
                   r += (double)our_saxs_options->bin_size)
                {
                  new_r[i].push_back (r);
                }

              if (!interpolate (new_r[i], plotted_r[i], plotted_pr[i],
                                new_pr[i]))
                {
                  editor_msg ("red", QString ("Error interpolating p(r) grid "
              ...
              if (plotted_pr_error[i].size ())
                {
                  if (!interpolate (new_r[i], plotted_r[i],
                                    plotted_pr_error[i], new_pr_error[i]))
                    {
                      ok = false;
                      break;
                    }
      ...
      if (!ok)
        {
          editor_msg (
              "red",
              QString ("Error interpolating p(r) grid to current binsize %1\n")
                  .arg (our_saxs_options->bin_size));
          return;
        }
      else
      ...
      if (needs_interp)
        {
          editor_msg (
              "red",
              QString ("error: incompatible r spacing after interpolation\n"));
          return;
        }
    }
...
void
US_Hydrodyn_Saxs::clear_plot_pr (bool full_clear)
{
  if (full_clear)
    {
      plotted_pr.clear ();
      ...
      plot_pr->detachItems (QwtPlotItem::Rtti_PlotCurve);
      plot_pr->detachItems (QwtPlotItem::Rtti_PlotMarker);
      ;
      plot_pr->replot ();
      pr_legend_vis = false;
      set_pr_legend ();
  ...
  // if plotted data with errors & those without errors exist, clear those
  // without errors if only plotted data with errors exist, drop the last one
  ...
  if ((int)plotted_r.size () != total_pr
  ...
  for (int i = 0; i < (int)pr_with_error_index.size (); ++i)
    {
      plot_one_pr (save_plotted_r[pr_with_error_index[i]],
                   save_plotted_pr[pr_with_error_index[i]],
                   save_plotted_pr_error[pr_with_error_index[i]],
                   save_qsl_plotted_pr_names[pr_with_error_index[i]], true,
                   false);
    }
