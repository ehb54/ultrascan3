...
void US_Hydrodyn_Saxs::plot_one_pr(vector<double> r, vector<double> pr,
...
void US_Hydrodyn_Saxs::plot_one_pr(vector<double> r, vector<double> pr,
  ...
  if (pr_error.size()) {
    if (pr_error.size() < pr.size() || US_Saxs_Util::is_zero_vector(pr_error)) {
      pr_error.clear();
    } else {
  ...
  plotted_pr_mw.push_back(cb_normalize->isChecked() ? get_mw(name, false, true)
                                                    : -1e0);
...
void US_Hydrodyn_Saxs::set_plot_pr_range(double &minx, double &maxx,
...
void US_Hydrodyn_Saxs::plot_one_iqq(vector<double> q, vector<double> I,
                                    QString name) {
...
void US_Hydrodyn_Saxs::plot_one_iqq(vector<double> q, vector<double> I,
  ...
  if (our_saxs_options->compute_exponentials) {
    ...
    if (is_zero_vector(I_error)) {
      ...
      do {
        ...
        text = US_Static::getText(
            ...
            us_tr("If you want to compute structure factors for this curve\n"
                  "Please define a dummy saxs name or press CANCEL to skip "
                  "this computation.\n" +
          ...
          if (saxs_map.count(text)) {
            switch (QMessageBox::question(
                ...
                QString(us_tr("The name %1 was previously defined\n"
                    ...
                    .arg(text),
                us_tr("&Overwrite"), us_tr("&Try again"), QString(), 1, 0)) {
              case 0:
                break;
              case 1:
      ...
      if (ok) {
        ...
        QMessageBox::information(
            ...
            QString(us_tr("Start computation\n"
                          "NOTE: This could take some time and the program "
        ...
        if (!usu.compute_exponential(
          ...
          editor_msg("dark blue",
                     QString("Exponentials 4 terms norm %1 nnorm %2 (%3): %4")
                         .arg(norm4)
                         .arg(nnorm4)
                         .arg(norm4tag)
                         .arg(qs4));
          ...
          TSO << "Terms5: " << qs5 << endl;
          editor_msg("dark blue",
                     QString("Exponentials 5 terms norm %1 nnorm %2 (%3): %4")
                         .arg(norm5)
                         .arg(nnorm5)
                         .arg(norm5tag)
                         .arg(qs5));
          switch (QMessageBox::question(
              this, us_tr("US-SOMO: Compute structure factors : computed"),
            ...
            case 0: {
              ...
              // get excluded volume
              {
                ...
                double res = US_Static::getDouble(
                    us_tr("US-SOMO: Compute structure factors : enter excluded "
                          "volume"),
                    "Enter a default excluded volume in Angstroms^3 (can be "
                    "zero):",
                    0, 0, 1e20, 4, &ok, this);
                if (ok) {
                  ...
                  QMessageBox::information(
                      this,
                      us_tr("US-SOMO: Compute structure factors : excluded "
                            "volume"),
                  ...
                  if (!filename.isEmpty()) {
                    ...
                    if (ok && !filename.isEmpty() &&
                      ...
                      QMessageBox::warning(
                          this,
                          us_tr(
                              "US-SOMO: Compute structure factors : save data"),
                          QString(us_tr("Error trying to write the file %1 "
                ...
                if (ok && !filename.isEmpty()) {
                  ...
                  for (unsigned int i = 0; i < saxs_list.size(); i++) {
                    ts << saxs_list[i].saxs_name.toUpper() << "\t"
                       << saxs_list[i].a[0] << "\t" << saxs_list[i].b[0] << "\t"
                       << saxs_list[i].a[1] << "\t" << saxs_list[i].b[1] << "\t"
                       << saxs_list[i].a[2] << "\t" << saxs_list[i].b[2] << "\t"
                       << saxs_list[i].a[3] << "\t" << saxs_list[i].b[3] << "\t"
                       << saxs_list[i].c << "\t" << saxs_list[i].volume
                       << Qt::endl;
                    ts << saxs_list[i].saxs_name.toUpper() << "\t"
                       << saxs_list[i].a5[0] << "\t" << saxs_list[i].b5[0]
                       << "\t" << saxs_list[i].a5[1] << "\t"
                       << saxs_list[i].b5[1] << "\t" << saxs_list[i].a5[2]
                       << "\t" << saxs_list[i].b5[2] << "\t"
                       << saxs_list[i].a5[3] << "\t" << saxs_list[i].b5[3]
                       << "\t" << saxs_list[i].a5[4] << "\t"
                       << saxs_list[i].b5[4] << "\t" << saxs_list[i].c5 << "\t"
                  ...
                  if (our_saxs_options->default_saxs_filename != filename) {
                    ...
                    QMessageBox::information(
                        this,
                        us_tr("US-SOMO: Compute structure factors : new "
                              "default saxs filename"),
                        QString(
                            us_tr("The default saxs atoms file is now %1\n"))
                            .arg(filename));
                  ...
                  if (our_saxs_options->dummy_saxs_name != text) {
                    ...
                    QMessageBox::information(
                        this,
                        us_tr("US-SOMO: Compute structure factors : new "
                              "default saxs dummy atom name"),
                        QString(
                            us_tr("The default dummy atom name is now %1\n"))
                            .arg(text));
                  }
      ...
      editor_msg("dark red",
                 QString(us_tr("Skipped: Computing %1 term exponentials, since "
                               "I(q) has errors"))
...
QString US_Hydrodyn_Saxs::Iq_plotted_summary() {
  ...
  for (unsigned int i = 0; i < plotted_q.size(); i++) {
    ...
    qs += QString(
              "pos %1 q.size() %2 q2.size() %3 I.size() %4 I_error.size() %5 "
              "%6 %7 q[%8:%9]\n")
              .arg(i)
...
void US_Hydrodyn_Saxs::do_plot_resid() {
      ...
      if (plotted_cs_guinier_valid.count(i) &&
            ...
            if (cb_resid_pct->isChecked()) {
              e.push_back(100e0 *
                          (log(plotted_q[i][q] * plotted_I[i][q] > 0e0
                           ...
                           (plotted_cs_guinier_a[i] +
                            plotted_cs_guinier_b[i] * plotted_q2[i][q])) /
              ...
              if (cb_resid_sd->isChecked()) {
                if ((int)plotted_I_error.size() > i &&
                    ...
                    plotted_I_error[i][q] != 0e0 && plotted_I[i][q] != 0e0) {
        ...
        if (x.size()) {
          if (plotted_cs_guinier_pts_removed.count(i) &&
            ...
            do_plot_resid(x, y, e, plotted_cs_guinier_pts_removed[i],
        ...
        if (plotted_Rt_guinier_valid.count(i) &&
          ...
          for (int q = 0; q < (int)plotted_q2[i].size(); ++q) {
            if (plotted_q2[i][q] >= plotted_Rt_guinier_lowq2[i] &&
              ...
              if (cb_resid_pct->isChecked()) {
                e.push_back(
                    100e0 *
                    (log(plotted_q[i][q] * plotted_q[i][q] * plotted_I[i][q] >
                                 0e0
                             ? plotted_q[i][q] * plotted_q[i][q] *
                                   plotted_I[i][q]
                             : 1e-99) -
                     (plotted_Rt_guinier_a[i] +
                      plotted_Rt_guinier_b[i] * plotted_q2[i][q])) /
                    fabs(log(plotted_q[i][q] * plotted_q[i][q] *
                                         plotted_I[i][q] >
                                     0e0
                                 ? plotted_q[i][q] * plotted_q[i][q] *
                                       plotted_I[i][q]
                ...
                if (cb_resid_sd->isChecked()) {
                  if ((int)plotted_I_error.size() > i &&
                      (int)plotted_I_error[i].size() > q &&
                      plotted_I_error[i][q] != 0e0 && plotted_I[i][q] != 0e0) {
                    e.push_back((log(plotted_q[i][q] * plotted_q[i][q] *
                                                 plotted_I[i][q] >
                                             0e0
                                         ? plotted_q[i][q] * plotted_q[i][q] *
                                               plotted_I[i][q]
              ...
              if (plotted_q2[i][q] >= plot_minx &&
                ...
                if (cb_resid_pct->isChecked()) {
                  ...
                  if (cb_resid_sd->isChecked()) {
                    ...
                    e_d.push_back(log(plotted_q[i][q] * plotted_q[i][q] *
                                                  plotted_I[i][q] >
                                              0e0
                                          ? plotted_q[i][q] * plotted_q[i][q] *
                                                plotted_I[i][q]
          ...
          if (x.size()) {
            if (plotted_Rt_guinier_pts_removed.count(i) &&
              ...
              do_plot_resid(x, y, e, plotted_Rt_guinier_pts_removed[i],
                            plot_colors[i % plot_colors.size()]);
            } else {
              do_plot_resid(x, y, e, plot_colors[i % plot_colors.size()]);
            }
        ...
        if (plotted_guinier_valid.count(i) &&
          ...
          for (int q = 0; q < (int)plotted_q2[i].size(); ++q) {
            if (plotted_q2[i][q] >= plotted_guinier_lowq2[i] &&
              ...
              if (cb_resid_pct->isChecked()) {
                e.push_back(
                    100e0 *
                    (log(plotted_I[i][q] > 0e0 ? plotted_I[i][q] : 1e-99) -
                     (plotted_guinier_a[i] +
                      plotted_guinier_b[i] * plotted_q2[i][q])) /
                    fabs(log(plotted_I[i][q] > 0e0 ? plotted_I[i][q] : 1e-99)));
              } else {
                if (cb_resid_sd->isChecked()) {
                  if ((int)plotted_I_error.size() > i &&
                      (int)plotted_I_error[i].size() > q &&
                ...
                if (cb_resid_pct->isChecked()) {
                  e_d.push_back(
                      ...
                      (log(plotted_I[i][q] > 0e0 ? plotted_I[i][q] : 1e-99) -
                       (plotted_guinier_a[i] +
                        plotted_guinier_b[i] * plotted_q2[i][q])) /
                      fabs(log(plotted_I[i][q] > 0e0 ? plotted_I[i][q]
                                                     : 1e-99)));
                } else {
                  if (cb_resid_sd->isChecked()) {
                    if ((int)plotted_I_error[i].size() > q &&
                        ...
                        plotted_I[i][q] != 0e0) {
                      e_d.push_back(
                          (log(plotted_I[i][q] > 0e0 ? plotted_I[i][q]
                                                     : 1e-99) -
                           (plotted_guinier_a[i] +
                            plotted_guinier_b[i] * plotted_q2[i][q])) /
                          (plotted_I_error[i][q] / plotted_I[i][q]));
          ...
          if (x.size()) {
            if (plotted_guinier_pts_removed.count(i) &&
              ...
              do_plot_resid(x, y, e, plotted_guinier_pts_removed[i],
                            plot_colors[i % plot_colors.size()]);
            } else {
              do_plot_resid(x, y, e, plot_colors[i % plot_colors.size()]);
            }
  ...
  if (some_pts_skipped) {
    editor_msg(
        "dark red",
        us_tr("Warning: points with zero s.d.'s or I(q)'s are not displayed"));
  }
...
void US_Hydrodyn_Saxs::do_plot_resid(vector<double> &x, vector<double> &y,
...
void US_Hydrodyn_Saxs::do_plot_resid(vector<double> &x_d, vector<double> &e_d,
                                     QColor qc) {
  {
    QwtPlotCurve *curve = new QwtPlotCurve("errors");
    curve->setStyle(QwtPlotCurve::Lines);
...
void US_Hydrodyn_Saxs::pr_replot() {
  ...
  for (int i = 0; i < (int)save_plotted_r.size(); ++i) {
    plot_one_pr(save_plotted_r[i], save_plotted_pr[i], save_plotted_pr_error[i],
                save_qsl_plotted_pr_names[i], true, false);
  }

  plot_pr->replot();
}

void US_Hydrodyn_Saxs::set_pr_sd(vector<double> &r, vector<double> &pr,
                                 vector<double> &pr_error) {
  if (!pr_error.size()) {
    // no errors
    return;
...
void US_Hydrodyn_Saxs::crop_pr_tail(vector<double> &r, vector<double> &pr,
                                    vector<double> &pr_error) {
...
void US_Hydrodyn_Saxs::prop_pr_sd_tail(vector<double> &pr_error,
                                       unsigned int len) {
  if (!pr_error.size()) {
    // no errors
    return;
...
void US_Hydrodyn_Saxs::pad_pr_plotted() {
  ...
  if (needs_interp) {
    ...
    editor_msg("darkred",
               "Loaded curves have varying r spacing, interpolating all to "
               "current bin size\n");
    ...
    for (int i = 0; i < (int)plotted_r.size(); ++i) {
      if (plotted_r[i].size() > 2) {
        ...
        if (plotted_pr_error[i].size()) {
          if (!interpolate(new_r[i], plotted_r[i], plotted_pr_error[i],
                           new_pr_error[i])) {
            ok = false;
            break;
          }
        ...
        if (!interpolate(new_r[i], plotted_r[i], plotted_pr_not_normalized[i],
...
void US_Hydrodyn_Saxs::clear_plot_pr(bool full_clear) {
  ...
  for (int i = 0; i < (int)pr_with_error_index.size(); ++i) {
    plot_one_pr(save_plotted_r[pr_with_error_index[i]],
                ...
                save_qsl_plotted_pr_names[pr_with_error_index[i]], true, false);
  }
