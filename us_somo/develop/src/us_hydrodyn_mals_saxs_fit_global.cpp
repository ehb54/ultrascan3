...
US_Hydrodyn_Mals_Saxs_Fit_Global::US_Hydrodyn_Mals_Saxs_Fit_Global (
    US_Hydrodyn_Mals_Saxs *mals_saxs_win, QWidget *p, const char *)
    : QDialog (p)
{
  ...
  setWindowTitle (us_tr ("US-SOMO: MALS_SAXS: Global Gaussian Fit"));
  ...
  mals_saxs_ampl_width_min = ((US_Hydrodyn *)(mals_saxs_win->us_hydrodyn))
                                 ->gparams["mals_saxs_ampl_width_min"]
                                 .toDouble ();
  mals_saxs_lock_min_retry = ((US_Hydrodyn *)(mals_saxs_win->us_hydrodyn))
                                         ->gparams["mals_saxs_lock_min_retry"]
                                     == "true"
                                 ? true
                                 : false;
  mals_saxs_lock_min_retry_mult
      = ((US_Hydrodyn *)(mals_saxs_win->us_hydrodyn))
            ->gparams["mals_saxs_lock_min_retry_mult"]
            .toDouble ();
  mals_saxs_maxfpk_restart = ((US_Hydrodyn *)(mals_saxs_win->us_hydrodyn))
                                         ->gparams["mals_saxs_maxfpk_restart"]
                                     == "true"
                                 ? true
                                 : false;
  mals_saxs_maxfpk_restart_tries
      = ((US_Hydrodyn *)(mals_saxs_win->us_hydrodyn))
            ->gparams["mals_saxs_maxfpk_restart_tries"]
            .toUInt ();
  mals_saxs_maxfpk_restart_pct = ((US_Hydrodyn *)(mals_saxs_win->us_hydrodyn))
                                     ->gparams["mals_saxs_maxfpk_restart_pct"]
                                     .toDouble ();
  ...
  // width
  if (mals_saxs_win->cb_fix_width->isChecked ())
    {
  ...
  if (dist1_active)
    {
      if (mals_saxs_win->cb_fix_dist1->isChecked ())
        {
          is_common.push_back (true);
          offset.push_back (common_size++); // first common entry
        }
      ...
      if (dist2_active)
        {
          if (mals_saxs_win->cb_fix_dist2->isChecked ())
            {
              is_common.push_back (true);
              offset.push_back (common_size++); // first common entry
            }
  ...
  gaussians_undo.push_back (mals_saxs_win->unified_ggaussian_params);

  setGeometry (global_Xpos, global_Ypos, 0, 0);

  use_errors = mals_saxs_win->cb_sd_weight->isChecked ();
  if (use_errors
      && (mals_saxs_win->unified_ggaussian_e.size ()
              != mals_saxs_win->unified_ggaussian_q.size ()
          || !mals_saxs_win->is_nonzero_vector (
              mals_saxs_win->unified_ggaussian_e)))
    {
      use_errors = false;
  ...
  if (use_errors)
    {
      ...
      vector<double> x = mals_saxs_win->unified_ggaussian_t;
...
void
US_Hydrodyn_Mals_Saxs_Fit_Global::restore ()
{
  gaussians_undo.resize (1);
  mals_saxs_win->unified_ggaussian_params = gaussians_undo[0];
  if (update_mals_saxs)
    {
      mals_saxs_win->gauss_init_markers ();
      mals_saxs_win->update_gauss_pos ();
      mals_saxs_win->lbl_gauss_fit->setText ("?");
      mals_saxs_win->pb_ggauss_rmsd->setEnabled (true);
      mals_saxs_win->ggqfit_plot->detachItems (QwtPlotItem::Rtti_PlotCurve);
      mals_saxs_win->ggqfit_plot->detachItems (QwtPlotItem::Rtti_PlotMarker);
      ;
      mals_saxs_win->ggqfit_plot->replot ();
      double new_rmsd = mals_saxs_win->ggaussian_rmsd ();
      mals_saxs_win->lbl_gauss_fit->setText (
          QString ("%1").arg (new_rmsd, 0, 'g', 5));
      mals_saxs_win->pb_ggauss_rmsd->setEnabled (false);
    }
  update_enables ();
}

void
US_Hydrodyn_Mals_Saxs_Fit_Global::undo ()
{
  ...
  mals_saxs_win->unified_ggaussian_params = gaussians_undo.back ();
  if (update_mals_saxs)
    {
      mals_saxs_win->gauss_init_markers ();
      mals_saxs_win->update_gauss_pos ();
  ...
  if (cb_test_mode->isChecked ()
      && MSFIT_GLOBAL::init_params.size () == test_widgets.size ())
    {
      for (int i = 0; i < (int)MSFIT_GLOBAL::init_params.size (); ++i)
        {
          ((QLineEdit *)test_widgets[i])
              ->setText (QString ("%1").arg (MSFIT_GLOBAL::init_params[i]));
        }
      update_test_info ();
    }
}
...
void
US_Hydrodyn_Mals_Saxs_Fit_Global::update_test_info ()
{
  QString qs_uggs = "";

  {
    ...
    qs_uggs += "common data: " + mals_saxs_win->describe_unified_common + "\n";

    for (unsigned int i = 0;
         i < mals_saxs_win->unified_ggaussian_gaussians_size; ++i)
      {
        qs_uggs += QString (" gaussian %1: ").arg (i + 1);
        for (unsigned int j = 0; j < mals_saxs_win->common_size; ++j)
          {
            qs_uggs += QString ("%1 ").arg (
                mals_saxs_win->unified_ggaussian_params[p++], 0, 'g', 6);
          }
        qs_uggs += "\n";
      }
    ...
    qs_uggs
        += "file specific: " + mals_saxs_win->describe_unified_per_file + "\n";
    for (unsigned int i = 0; i < mals_saxs_win->unified_ggaussian_curves; ++i)
      {
        qs_uggs += QString (" curve %1: ").arg (i + 1) + "\n";
        for (unsigned int k = 0;
             k < mals_saxs_win->unified_ggaussian_gaussians_size; ++k)
          {
            qs_uggs += QString ("  gaussian %1: ").arg (k + 1);
            for (unsigned int j = 0; j < mals_saxs_win->per_file_size; ++j)
              {
                qs_uggs += QString ("%1 ").arg (
                    mals_saxs_win->unified_ggaussian_params[p++], 0, 'g', 6);
              }
  ...
  lbl_test_info->setText (
      QString ("Per gaussian: common_size: %1 %2 per_file_size: %3 %4\n"
               ...
               "%6"
               "%7")
          .arg (mals_saxs_win->common_size)
          .arg (mals_saxs_win->describe_unified_common)
          .arg (mals_saxs_win->per_file_size)
          .arg (mals_saxs_win->describe_unified_per_file)
          .arg (US_Vector::qs_vector ("unified ggaussians",
                                      mals_saxs_win->unified_ggaussian_params,
                                      6, 20))
          .arg (qs_uggs)
          .arg (MSFIT_GLOBAL::qs_params ()));
}

void
US_Hydrodyn_Mals_Saxs_Fit_Global::setupGUI ()
{
  int minHeight1 = 30;

  lbl_title
      = new QLabel (us_tr ("US-SOMO: MALS_SAXS: Global Gaussian Fit"), this);
  ...
  /* old way
     ...
     mals_saxs_win->gaussians.size() / 3 - 1 ); cout << "qs_rx:" << qs_rx <<
     endl; QRegExp rx_fix_curves( qs_rx ); QRegExpValidator *rx_val_fix_curves
     = new QRegExpValidator( rx_fix_curves, this ); le_fix_curves = new
     mQLineEdit( this );       le_fix_curves->setObjectName( "le_fix_curves
     Line Edit" ); le_fix_curves->setText( "" );
  ...
  */
  ...
  for (unsigned int i = 0;
       i < (unsigned int)mals_saxs_win->gaussians.size () / gaussian_type_size;
       i++)
    {
      QCheckBox *cb_tmp;
  ...
  double peak = mals_saxs_win->compute_gaussian_peak (
      mals_saxs_win->wheel_file, mals_saxs_win->gaussians);
  le_epsilon->setText (
  ...
  if (cb_test_mode->isVisible ())
    {
      ...
      for (int i = 0; i < mals_saxs_win->lb_files->count (); i++)
        {
          if (mals_saxs_win->lb_files->item (i)->isSelected ())
            {
              selected_count++;
              if (selected_count > 4)
                {
...
void
US_Hydrodyn_Mals_Saxs_Fit_Global::cancel ()
{
  if (mals_saxs_win->lbl_gauss_fit->text () == "?"
      && !mals_saxs_win->pb_ggauss_rmsd->isEnabled ())
    {
      mals_saxs_win->pb_ggauss_rmsd->setEnabled (true);
    }
...
void
US_Hydrodyn_Mals_Saxs_Fit_Global::help ()
{
  US_Help *online_help;
  online_help = new US_Help (this);
  online_help->show_help ("manual/somo/somo_mals_saxs_fit_global.html");
}

void
US_Hydrodyn_Mals_Saxs_Fit_Global::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept ();
...
void
US_Hydrodyn_Mals_Saxs_Fit_Global::update_common ()
{
  if (!setup_run ())
    {
      update_enables ();
  ...
  for (unsigned int i = 0; i < MSFIT_GLOBAL::param_fixed.size (); i++)
    {
      if (!MSFIT_GLOBAL::param_fixed[i]
          || MSFIT_GLOBAL::comm_backref.count (i))
        {
          mals_saxs_win->unified_ggaussian_params[i]
              = MSFIT_GLOBAL::init_params[MSFIT_GLOBAL::comm_backref.count (i)
                                              ? MSFIT_GLOBAL::comm_backref[i]
                                              : MSFIT_GLOBAL::param_pos[i]];
        }
    }
  if (save_unified_ggaussians != mals_saxs_win->unified_ggaussian_params)
    {
      mals_saxs_win->lbl_gauss_fit->setText ("?");
      mals_saxs_win->ggqfit_plot->detachItems (QwtPlotItem::Rtti_PlotCurve);
      mals_saxs_win->ggqfit_plot->detachItems (QwtPlotItem::Rtti_PlotMarker);
...
void
US_Hydrodyn_Mals_Saxs_Fit_Global::update_enables ()
{
...
vector<double> unified_q;                           // copy from mals_saxs win
vector<unsigned int> unified_ggaussian_param_index; // copy from mals_saxs win
unsigned int unified_ggaussian_gaussians_size;      // copy from mals_saxs win
unsigned int unified_ggaussian_curves;              // copy from mals_saxs win
...
bool
US_Hydrodyn_Mals_Saxs_Fit_Global::setup_run ()
{
  MSFIT_GLOBAL::init_params.clear ();
  MSFIT_GLOBAL::base_params.clear ();
  MSFIT_GLOBAL::fixed_params.clear ();
  MSFIT_GLOBAL::comm_backref.clear ();
  MSFIT_GLOBAL::param_pos.clear ();
  MSFIT_GLOBAL::param_fixed.clear ();
  MSFIT_GLOBAL::param_min.clear ();
  MSFIT_GLOBAL::param_max.clear ();
  MSFIT_GLOBAL::unified_q = mals_saxs_win->unified_ggaussian_q;
  MSFIT_GLOBAL::unified_ggaussian_param_index
      = mals_saxs_win->unified_ggaussian_param_index;
  MSFIT_GLOBAL::unified_ggaussian_gaussians_size
      = mals_saxs_win->unified_ggaussian_gaussians_size;
  MSFIT_GLOBAL::unified_ggaussian_curves
      = mals_saxs_win->unified_ggaussian_curves;

  MSFIT_GLOBAL::per_file_size = per_file_size;
  MSFIT_GLOBAL::common_size = common_size;
  MSFIT_GLOBAL::is_common = is_common;
  ...
  switch (gaussian_type)
    {
    case US_Hydrodyn_Mals_Saxs::EMGGMG:
      MSFIT_GLOBAL::compute_gaussian_f
          = &MSFIT_GLOBAL::compute_gaussian_f_EMGGMG;
      // gsm_f = &gsm_f_EMGGMG;
      break;
    case US_Hydrodyn_Mals_Saxs::EMG:
      MSFIT_GLOBAL::compute_gaussian_f = &MSFIT_GLOBAL::compute_gaussian_f_EMG;
      // gsm_f = &gsm_f_EMG;
      break;
    case US_Hydrodyn_Mals_Saxs::GMG:
      MSFIT_GLOBAL::compute_gaussian_f = &MSFIT_GLOBAL::compute_gaussian_f_GMG;
      // gsm_f = &gsm_f_GMG;
      break;
    case US_Hydrodyn_Mals_Saxs::GAUSS:
      MSFIT_GLOBAL::compute_gaussian_f
          = &MSFIT_GLOBAL::compute_gaussian_f_GAUSS;
      // gsm_f = &gsm_f_GAUSS;
      break;
    }

  MSFIT_GLOBAL::use_errors = use_errors;
  ...
  double dist = ((US_Hydrodyn *)(mals_saxs_win->us_hydrodyn))
  ...
  // the mals_saxs_win unified_ggaussians
  ...
  for (unsigned int i = 0; i < mals_saxs_win->unified_ggaussian_gaussians_size;
       i++)
    {
      ...
      if (cb_fix_center->isChecked () || fixed_curves.count (i))
        {
          MSFIT_GLOBAL::param_pos.push_back (
              MSFIT_GLOBAL::fixed_params.size ());
          MSFIT_GLOBAL::fixed_params.push_back (
              mals_saxs_win->unified_ggaussian_params[ofs]);
          MSFIT_GLOBAL::param_fixed.push_back (true);
        }
      else
        {
          MSFIT_GLOBAL::param_pos.push_back (
              MSFIT_GLOBAL::init_params.size ());
          ...
          if (cb_pct_center_from_init->isChecked ())
            ...
            {
              base_val = mals_saxs_win->unified_ggaussian_params[ofs];
            }

          MSFIT_GLOBAL::init_params.push_back (base_val);
          MSFIT_GLOBAL::base_params.push_back (
              mals_saxs_win->unified_ggaussian_params[ofs]);
          MSFIT_GLOBAL::param_fixed.push_back (false);
          ...
          if (MSFIT_GLOBAL::init_params.back () < min)
            {
              MSFIT_GLOBAL::init_params.back () = min;
            }
          if (MSFIT_GLOBAL::init_params.back () > max)
            {
              MSFIT_GLOBAL::init_params.back () = max;
            }

          MSFIT_GLOBAL::param_min.push_back (min);
          MSFIT_GLOBAL::param_max.push_back (max);
        }
      ofs++;
      this_ofs++;

      if (mals_saxs_win->cb_fix_width->isChecked ()) // if widths are common
        {
      ...
      if (dist1_active
          && mals_saxs_win->cb_fix_dist1->isChecked ()) // if dist1s are common
        {
          if (cb_fix_dist1->isChecked () || fixed_curves.count (i)
              || (cb_comm_dist1->isChecked () && i))
            {
              ...
              if (i && cb_comm_dist1->isChecked () && !fixed_curves.count (i)
                ...
                {
                  MSFIT_GLOBAL::fixed_params.push_back (
                      mals_saxs_win->unified_ggaussian_params[this_ofs]);
                  MSFIT_GLOBAL::comm_backref[MSFIT_GLOBAL::param_fixed.size ()]
                      = comm_dist_1_var_pos;
                }
              else
                {
          ...
          else
            {
              if (cb_comm_dist1->isChecked ())
                {
                  comm_dist_1_var_pos = MSFIT_GLOBAL::init_params.size ();
                }
              ...
              if (cb_fix_dist2->isChecked () || fixed_curves.count (i)
                  || (cb_comm_dist2->isChecked () && i))
                {
                  ...
                  if (i && cb_comm_dist2->isChecked ()
                      && !fixed_curves.count (i) && !fixed_curves.count (0))
                    {
                      MSFIT_GLOBAL::fixed_params.push_back (
                          mals_saxs_win->unified_ggaussian_params[this_ofs]);
                      MSFIT_GLOBAL::comm_backref[MSFIT_GLOBAL::param_fixed
                                                     .size ()]
                          = comm_dist_2_var_pos;
                    }
                  else
              ...
              else
                {
                  if (cb_comm_dist2->isChecked ())
                    {
                      comm_dist_2_var_pos = MSFIT_GLOBAL::init_params.size ();
                    }
  ...
  unsigned int base_ofs
      = mals_saxs_win->unified_ggaussian_gaussians_size * common_size;
  // assert( (unsigned int)MSFIT_GLOBAL::param_pos.size() == base_ofs &&
  // "common_size*number of gaussians should equal param_pos.size()" );

  for (unsigned int f = 0; f < mals_saxs_win->unified_ggaussian_curves; f++)
    {
      unsigned int ofs
          = base_ofs
            + f * per_file_size
                      ...
                      || (cb_comm_dist2->isChecked () && i))
                    {
                      ...
                      if (i && cb_comm_dist2->isChecked ()
                          ...
                          && !fixed_curves.count (0))
                        {
                          MSFIT_GLOBAL::fixed_params.push_back (
                              mals_saxs_win
                                  ->unified_ggaussian_params[this_ofs]);
                          MSFIT_GLOBAL::comm_backref[MSFIT_GLOBAL::param_fixed
                                                         .size ()]
                              = comm_dist_2_var_pos;
                        }
  ...
  mals_saxs_win->unified_ggaussian_gaussians_size; i++ )
     {
        if ( cb_fix_center->isChecked() ||
             fixed_curves.count( i ) )
        {
           MSFIT_GLOBAL::param_pos   .push_back(
  MSFIT_GLOBAL::fixed_params.size() ); MSFIT_GLOBAL::fixed_params.push_back(
  mals_saxs_win->unified_ggaussian_params[ 2 * i + 0 ] );
           MSFIT_GLOBAL::param_fixed .push_back( true );
        } else {
           MSFIT_GLOBAL::param_pos   .push_back(
  MSFIT_GLOBAL::init_params.size() );
           ...
           if ( cb_pct_center_from_init->isChecked() )
           {
              ...
              base_val = mals_saxs_win->unified_ggaussian_params[ 2 * i + 0 ];
           }
  ...
  mals_saxs_win->unified_ggaussian_params[ 2 * i + 0 ] );
             ...
             fixed_curves.count( i ) )
        {
           MSFIT_GLOBAL::param_pos   .push_back(
  MSFIT_GLOBAL::fixed_params.size() ); MSFIT_GLOBAL::fixed_params.push_back(
  mals_saxs_win->unified_ggaussian_params[ 2 * i + 1 ] );
           MSFIT_GLOBAL::param_fixed .push_back( true );
        } else {
           MSFIT_GLOBAL::param_pos   .push_back(
  MSFIT_GLOBAL::init_params.size() );
           ...
           if ( cb_pct_width_from_init->isChecked() )
           {
              ...
              base_val = mals_saxs_win->unified_ggaussian_params[ 2 * i + 1 ];
           }
  ...
  mals_saxs_win->unified_ggaussian_params[ 2 * i + 1] );
     ...
     for ( unsigned int i = 0; i <
  mals_saxs_win->unified_ggaussian_gaussians_size *
  mals_saxs_win->unified_ggaussian_curves; i++ )
     {
        unsigned int pos = i / mals_saxs_win->unified_ggaussian_curves;

        unsigned int height_pos = 2 *
  mals_saxs_win->unified_ggaussian_gaussians_size + i + 0;
             ...
             fixed_curves.count( pos ) )
        {
           MSFIT_GLOBAL::param_pos   .push_back(
  MSFIT_GLOBAL::fixed_params.size() ); MSFIT_GLOBAL::fixed_params.push_back(
  mals_saxs_win->unified_ggaussian_params[ height_pos ] );
           MSFIT_GLOBAL::param_fixed .push_back( true );
        } else {
           MSFIT_GLOBAL::param_pos   .push_back(
  MSFIT_GLOBAL::init_params.size() );
           ...
           if ( cb_pct_amplitude_from_init->isChecked() )
           {
              ...
              base_val = mals_saxs_win->unified_ggaussian_params[ height_pos ];
           }
  ...
  mals_saxs_win->unified_ggaussian_gaussians_size; i++ )
     {
             ...
             fixed_curves.count( i ) )
        {
           MSFIT_GLOBAL::param_pos   .push_back(
  MSFIT_GLOBAL::fixed_params.size() ); MSFIT_GLOBAL::fixed_params.push_back(
  mals_saxs_win->unified_ggaussian_params[ i ] ); MSFIT_GLOBAL::param_fixed
  .push_back( true ); } else { MSFIT_GLOBAL::param_pos   .push_back(
  MSFIT_GLOBAL::init_params.size() );
           ...
           if ( cb_pct_center_from_init->isChecked() )
           {
              ...
              base_val = mals_saxs_win->unified_ggaussian_params[ i ];
           }
  ...
  .push_back( false );
     ...
     for ( unsigned int i = 0; i <
  mals_saxs_win->unified_ggaussian_gaussians_size * 2 *
  mals_saxs_win->unified_ggaussian_curves; i+= 2 )
     {
        unsigned int pos = i / ( 2 * mals_saxs_win->unified_ggaussian_curves );

        unsigned int height_pos =
  mals_saxs_win->unified_ggaussian_gaussians_size + i + 0; unsigned int
  width_pos  = mals_saxs_win->unified_ggaussian_gaussians_size + i + 1;
  ...
  mals_saxs_win->unified_ggaussian_params[ height_pos ] );
             ...
             fixed_curves.count( pos + 1 ) )
        {
           MSFIT_GLOBAL::param_pos   .push_back(
  MSFIT_GLOBAL::fixed_params.size() ); MSFIT_GLOBAL::fixed_params.push_back(
  mals_saxs_win->unified_ggaussian_params[ width_pos ] );
           MSFIT_GLOBAL::param_fixed .push_back( true );
        } else {
           MSFIT_GLOBAL::param_pos   .push_back(
  MSFIT_GLOBAL::init_params.size() );
           ...
           if ( cb_pct_width_from_init->isChecked() )
           {
              ...
              base_val = mals_saxs_win->unified_ggaussian_params[ width_pos ];
           }
  ...
  */
  ...
  // MSFIT_GLOBAL::list_params();

  // US_Vector::printvector( "is_common", is_common );

  if (!MSFIT_GLOBAL::init_params.size ())
    {
      return false;
    }
  else
...
void
US_Hydrodyn_Mals_Saxs_Fit_Global::test_mode ()
{
  if (!cb_test_mode->isChecked ())
    {
  ...
  for (int i = 0; i < (int)MSFIT_GLOBAL::init_params.size (); ++i)
    {
      ...
      le->setEnabled (true);
      le->setText (QString ("%1").arg (MSFIT_GLOBAL::init_params[i]));
      le->show ();
...
void
US_Hydrodyn_Mals_Saxs_Fit_Global::test ()
{
  if (MSFIT_GLOBAL::init_params.size () != test_widgets.size ())
    {
      ...
      if (!MSFIT_GLOBAL::param_fixed[i]
          || MSFIT_GLOBAL::comm_backref.count (i))
        {
          ...
          us_qdebug (QString ("param %1 taken from init params pos %2 %3")
                         .arg (i)
                         .arg (MSFIT_GLOBAL::comm_backref.count (i)
                                   ? MSFIT_GLOBAL::comm_backref[i]
                                   : MSFIT_GLOBAL::param_pos[i])
                         .arg (MSFIT_GLOBAL::comm_backref.count (i)
                                   ? "backref used"
  ...
  vector<double> gsum = mals_saxs_win->compute_ggaussian_gaussian_sum ();
  vector<double> t = mals_saxs_win->unified_ggaussian_t;
...
void
US_Hydrodyn_Mals_Saxs_Fit_Global::lm ()
{
  update_common ();
  running = true;
  update_enables ();
  ...
  vector<double> t = mals_saxs_win->unified_ggaussian_t;
  vector<double> y = mals_saxs_win->unified_ggaussian_I;

  vector<double> org_params = MSFIT_GLOBAL::init_params;

  MSFIT_GLOBAL::errors = mals_saxs_win->unified_ggaussian_e;
  MSFIT_GLOBAL::errors_index.clear ();

  if (use_errors)
    {
      MSFIT_GLOBAL::errors_index.resize ((unsigned int)t.back () + 1);
      for (unsigned int i = 0; i <= (unsigned int)t.back (); i++)
        {
          MSFIT_GLOBAL::errors_index[i] = 0;
        }
      for (unsigned int i = 0; i < y.size (); i++)
        {
          y[i] /= MSFIT_GLOBAL::errors[i];
          MSFIT_GLOBAL::errors_index[(unsigned int)t[i]] = i;
        }
    }
  ...
  for ( unsigned int i = 0; i < MSFIT_GLOBAL::param_fixed.size(); i++ )
  {
          ...
          MSFIT_GLOBAL::comm_backref.count( i ) )
     {
        mals_saxs_win->unified_ggaussian_params[ i ] =
  MSFIT_GLOBAL::init_params[ MSFIT_GLOBAL::comm_backref.count( i ) ?
                                                                           MSFIT_GLOBAL::comm_backref[
  i ] : MSFIT_GLOBAL::param_pos[ i ]
                                                                            ];
     }
  }
  ...
  */
  ...
  double org_rmsd = mals_saxs_win->ggaussian_rmsd (false);
  ...
  for (unsigned int i = 0; i < (unsigned int)t.size (); i++)
    {
      gsumf[i] = (*MSFIT_GLOBAL::compute_gaussian_f) (
          t[i], &MSFIT_GLOBAL::init_params[0]);
    }

  /* mals_saxs_win->unified_ggaussian_params = save_unified_ggaussians; */
...
#undef TOL
  ...
  // mals_saxs_win->add_ggaussian_curve( "lm_start", gsumf );

  vector<double> par = MSFIT_GLOBAL::init_params;
  ...
  if (org_rmsd > status.fnorm)
    {
      for (unsigned int i = 0; i < MSFIT_GLOBAL::param_fixed.size (); i++)
        {
          if (!MSFIT_GLOBAL::param_fixed[i]
            ...
            {
              mals_saxs_win->unified_ggaussian_params[i]
                  = par[MSFIT_GLOBAL::comm_backref.count (i)
                            ? MSFIT_GLOBAL::comm_backref[i]
                            : MSFIT_GLOBAL::param_pos[i]];
            }
        }
      ...
      // mals_saxs_win->add_ggaussian_curve( "lm_after_pushback",
      // mals_saxs_win->compute_ggaussian_gaussian_sum() );
      gaussians_undo.push_back (mals_saxs_win->unified_ggaussian_params);
      if (update_mals_saxs)
        {
...
void
US_Hydrodyn_Mals_Saxs_Fit_Global::gsm_sd ()
{
  ...
  {
     gsm_yp[ j ]  = MSFIT_GLOBAL::compute_gaussian_f( gsm_t[ j ], (double
  *)(&MSFIT_GLOBAL::base_params[ 0 ]) ); org_rmsd += ( gsm_y[ j ] - gsm_yp[ j ]
  ) * ( gsm_y[ j ] - gsm_yp[ j ] );
  }

  org_rmsd = sqrt( org_rmsd );

  our_vector *v = new_our_vector( MSFIT_GLOBAL::init_params.size() );
  for ( int i = 0; i < v->len; i++ )
  {
     v->d[ i ] = MSFIT_GLOBAL::init_params[ i ];
  }
  ...
  if ( org_rmsd > rmsd )
  {
     for ( unsigned int i = 0; i < MSFIT_GLOBAL::param_fixed.size(); i++ )
     {
        if ( !MSFIT_GLOBAL::param_fixed[ i ] ||
             MSFIT_GLOBAL::comm_backref.count( i ) )
        {
           mals_saxs_win->unified_ggaussian_params[ i ] = v->d[
           MSFIT_GLOBAL::comm_backref.count( i ) ?
           MSFIT_GLOBAL::comm_backref[ i ] :
           MSFIT_GLOBAL::param_pos[ i ]
           ];
...
void
US_Hydrodyn_Mals_Saxs_Fit_Global::gsm_ih ()
{
  return; // have to rewrite this
...
void
US_Hydrodyn_Mals_Saxs_Fit_Global::gsm_cg ()
{
  return; // have to rewrite this
...
void
US_Hydrodyn_Mals_Saxs_Fit_Global::ga ()
{
  /*
  gsm_setup();
       ...
       rmsd < org_rmsd )
  {
     ...
     for ( unsigned int i = 0; i < MSFIT_GLOBAL::param_fixed.size(); i++ )
     {
        if ( !MSFIT_GLOBAL::param_fixed[ i ] ||
             MSFIT_GLOBAL::comm_backref.count( i ) )
        {
           mals_saxs_win->unified_ggaussian_params[ i ] = par[
           MSFIT_GLOBAL::comm_backref.count( i ) ?
           MSFIT_GLOBAL::comm_backref[ i ] :
           MSFIT_GLOBAL::param_pos[ i ]
           ];
...
void
US_Hydrodyn_Mals_Saxs_Fit_Global::stop ()
{
  running = false;
}

void
US_Hydrodyn_Mals_Saxs_Fit_Global::grid ()
{
  return; // have to rewrite this
  setup_run ();
  ...
  unsigned int total_count = (unsigned int)pow (
      (double)pop_size, (int)MSFIT_GLOBAL::init_params.size ());
  ...
  for (unsigned int j = 0; j < (unsigned int)MSFIT_GLOBAL::init_params.size ();
       j++)
    {
      ofs_per[j] = (MSFIT_GLOBAL::param_max[j] - MSFIT_GLOBAL::param_min[j])
                   / (pop_size - 1);
    }
  ...
  for (unsigned int j = 0; j < t.size (); j++)
    {
      yp[j] = MSFIT_GLOBAL::compute_gaussian_f (
          t[j], (double *)(&MSFIT_GLOBAL::base_params[0]));
  ...
  for (unsigned int i = 0; i < total_count; i++)
    {
      ...
      for (unsigned int j = 0;
           j < (unsigned int)MSFIT_GLOBAL::init_params.size (); j++)
        {
          unsigned int pos = remainder % pop_size;
          use_par[j] = MSFIT_GLOBAL::param_min[j] + pos * ofs_per[j];
          remainder /= pop_size;
        }
      ...
      for (unsigned int j = 0; j < t.size (); j++)
        {
          yp[j] = MSFIT_GLOBAL::compute_gaussian_f (t[j],
                                                    (double *)(&use_par[0]));
...
void
US_Hydrodyn_Mals_Saxs_Fit_Global::usp_config_plot_test (const QPoint &)
{
