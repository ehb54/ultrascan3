...
US_Hydrodyn_Mals_Saxs_Fit_Global::US_Hydrodyn_Mals_Saxs_Fit_Global(
  ...
  mals_saxs_ampl_width_min = ((US_Hydrodyn *)(mals_saxs_win->us_hydrodyn))
                                 ...
                                 .toDouble();
  mals_saxs_lock_min_retry =
      ((US_Hydrodyn *)(mals_saxs_win->us_hydrodyn))
                  ->gparams["mals_saxs_lock_min_retry"] == "true"
          ? true
          : false;
  mals_saxs_lock_min_retry_mult = ((US_Hydrodyn *)(mals_saxs_win->us_hydrodyn))
                                      ->gparams["mals_saxs_lock_min_retry_mult"]
                                      .toDouble();
  mals_saxs_maxfpk_restart =
      ((US_Hydrodyn *)(mals_saxs_win->us_hydrodyn))
                  ->gparams["mals_saxs_maxfpk_restart"] == "true"
          ? true
          : false;
  ...
  if (use_errors &&
      (mals_saxs_win->unified_ggaussian_e.size() !=
       ...
       !mals_saxs_win->is_nonzero_vector(mals_saxs_win->unified_ggaussian_e))) {
    use_errors = false;
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::restore() {
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::undo() {
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::update_test_info() {
  QString qs_uggs = "";
  ...
  lbl_test_info->setText(
      QString("Per gaussian: common_size: %1 %2 per_file_size: %3 %4\n"
          ...
          .arg(US_Vector::qs_vector("unified ggaussians",
                                    mals_saxs_win->unified_ggaussian_params, 6,
                                    20))
          .arg(qs_uggs)
          .arg(MSFIT_GLOBAL::qs_params()));
}

void US_Hydrodyn_Mals_Saxs_Fit_Global::setupGUI() {
  int minHeight1 = 30;
  ...
  /* old way
     ...
     mQLineEdit( this );       le_fix_curves->setObjectName( "le_fix_curves Line
     Edit" ); le_fix_curves->setText( "" );
  ...
  */
  ...
  double peak = mals_saxs_win->compute_gaussian_peak(mals_saxs_win->wheel_file,
                                                     mals_saxs_win->gaussians);
  le_epsilon->setText(
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::cancel() {
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/somo_mals_saxs_fit_global.html");
}

void US_Hydrodyn_Mals_Saxs_Fit_Global::closeEvent(QCloseEvent *e) {
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept();
}

void US_Hydrodyn_Mals_Saxs_Fit_Global::update_common() {
  if (!setup_run()) {
    update_enables();
    return;
  }
  ...
  for (unsigned int i = 0; i < MSFIT_GLOBAL::param_fixed.size(); i++) {
    if (!MSFIT_GLOBAL::param_fixed[i] || MSFIT_GLOBAL::comm_backref.count(i)) {
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::update_enables() {
...
bool US_Hydrodyn_Mals_Saxs_Fit_Global::setup_run() {
  ...
  // these are a copy of the init_params without replacement from the
  // mals_saxs_win unified_ggaussians
  ...
  for (unsigned int i = 0; i < mals_saxs_win->unified_ggaussian_gaussians_size;
    ...
    if (cb_fix_center->isChecked() || fixed_curves.count(i)) {
      MSFIT_GLOBAL::param_pos.push_back(MSFIT_GLOBAL::fixed_params.size());
      MSFIT_GLOBAL::fixed_params.push_back(
          mals_saxs_win->unified_ggaussian_params[ofs]);
      MSFIT_GLOBAL::param_fixed.push_back(true);
    } else {
      MSFIT_GLOBAL::param_pos.push_back(MSFIT_GLOBAL::init_params.size());
        ...
        mals_saxs_win->cb_fix_dist1->isChecked())  // if dist1s are common
    {
      ...
      if (dist2_active &&
          mals_saxs_win->cb_fix_dist2->isChecked())  // if dist2s are common
      {
  ...
  for (unsigned int f = 0; f < mals_saxs_win->unified_ggaussian_curves; f++) {
    unsigned int ofs =
        base_ofs +
        f * per_file_size * mals_saxs_win->unified_ggaussian_gaussians_size;
    ...
    for (unsigned int i = 0;
      ...
      if (dist1_active && !mals_saxs_win->cb_fix_dist1
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::test_mode() {
  if (!cb_test_mode->isChecked()) {
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::test() {
    ...
    if (!MSFIT_GLOBAL::param_fixed[i] || MSFIT_GLOBAL::comm_backref.count(i)) {
      ...
      us_qdebug(QString("param %1 taken from init params pos %2 %3")
                    ...
                    .arg(MSFIT_GLOBAL::comm_backref.count(i)
                             ? MSFIT_GLOBAL::comm_backref[i]
                             : MSFIT_GLOBAL::param_pos[i])
                    .arg(MSFIT_GLOBAL::comm_backref.count(i) ? "backref used"
                                                             : "direct pos"));
    } else {
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::lm() {
  update_common();
  running = true;
  update_enables();
  // setup_run();
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::gsm_sd() {
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::gsm_ih() {
  return;  // have to rewrite this
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::gsm_cg() {
  return;  // have to rewrite this
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::ga() {
  /*
  gsm_setup();
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::stop() { running = false; }

void US_Hydrodyn_Mals_Saxs_Fit_Global::grid() {
  return;  // have to rewrite this
  setup_run();
  ...
  for (unsigned int i = 0; i < total_count; i++) {
    ...
    for (unsigned int j = 0; j < t.size(); j++) {
      yp[j] = MSFIT_GLOBAL::compute_gaussian_f(t[j], (double *)(&use_par[0]));
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::usp_config_plot_test(const QPoint &) {
