                                                                   ...
                                                                   ) : QDialog( p )
{
   ...
   if ( use_errors &&
        ( mals_saxs_win->unified_ggaussian_e.size() != mals_saxs_win->unified_ggaussian_q.size() ||
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::restore()
{
   ...
   if ( update_mals_saxs )
   {
      ...
      mals_saxs_win->ggqfit_plot->detachItems( QwtPlotItem::Rtti_PlotCurve ); mals_saxs_win->ggqfit_plot->detachItems( QwtPlotItem::Rtti_PlotMarker );;
      mals_saxs_win->ggqfit_plot->replot();
      double new_rmsd = mals_saxs_win->ggaussian_rmsd();
      mals_saxs_win->lbl_gauss_fit->setText( QString( "%1" ).arg( new_rmsd, 0, 'g', 5 ) );
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::undo()
{
        ...
        MSFIT_GLOBAL::init_params.size() == test_widgets.size() )
   {
      ...
      {
         ((QLineEdit *)test_widgets[ i ])->setText( QString( "%1" ).arg( MSFIT_GLOBAL::init_params[ i ] ) );
      }
      update_test_info();
   }
}
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::update_test_info()
{
...

   {
      ...
      for ( unsigned int i = 0; i < mals_saxs_win->unified_ggaussian_gaussians_size; ++i ) {
         qs_uggs += QString( " gaussian %1: " ).arg( i + 1 );
         for ( unsigned int j = 0; j < mals_saxs_win->common_size; ++j ) {
            qs_uggs += QString( "%1 " ).arg(  mals_saxs_win->unified_ggaussian_params[ p++ ], 0, 'g', 6 );
         } 
         qs_uggs += "\n";
      }

      // now the file specific
      qs_uggs += "file specific: " + mals_saxs_win->describe_unified_per_file + "\n";
      for ( unsigned int i = 0; i < mals_saxs_win->unified_ggaussian_curves; ++i ) {
         qs_uggs += QString( " curve %1: " ).arg( i + 1) + "\n";
         for ( unsigned int k = 0; k < mals_saxs_win->unified_ggaussian_gaussians_size; ++k ) {
   ...
   lbl_test_info->setText( 
                          ...
                          .arg( US_Vector::qs_vector( "unified ggaussians",mals_saxs_win->unified_ggaussian_params, 6, 20 ) )
                          .arg( qs_uggs )
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::setupGUI()
{
   ...
   lbl_title = new QLabel( us_tr( "US-SOMO: MALS_SAXS: Global Gaussian Fit" ), this);
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::update_common()
{
   ...
   vector < double > save_unified_ggaussians = mals_saxs_win->unified_ggaussian_params;
   ...
   for ( unsigned int i = 0; i < MSFIT_GLOBAL::param_fixed.size(); i++ )
   {
      ...
      {
         mals_saxs_win->unified_ggaussian_params[ i ] = MSFIT_GLOBAL::init_params[ 
                                                                            MSFIT_GLOBAL::comm_backref.count( i ) ?
...
bool US_Hydrodyn_Mals_Saxs_Fit_Global::setup_run()
{
   ...
   MSFIT_GLOBAL::unified_ggaussian_param_index     = mals_saxs_win->unified_ggaussian_param_index;
   MSFIT_GLOBAL::unified_ggaussian_gaussians_size  = mals_saxs_win->unified_ggaussian_gaussians_size;
   MSFIT_GLOBAL::unified_ggaussian_curves          = mals_saxs_win->unified_ggaussian_curves;
   ...
   {
   case US_Hydrodyn_Mals_Saxs::EMGGMG :
      MSFIT_GLOBAL::compute_gaussian_f = &MSFIT_GLOBAL::compute_gaussian_f_EMGGMG;
      // gsm_f = &gsm_f_EMGGMG;
      break;
   case US_Hydrodyn_Mals_Saxs::EMG :
   ...
   case US_Hydrodyn_Mals_Saxs::GMG :
      ...
      break;
   case US_Hydrodyn_Mals_Saxs::GAUSS :
      MSFIT_GLOBAL::compute_gaussian_f = &MSFIT_GLOBAL::compute_gaussian_f_GAUSS;
      // gsm_f = &gsm_f_GAUSS;
      break;
   }
   ...
   // base_params : these are a copy of the init_params without replacement from the mals_saxs_win unified_ggaussians
   ...
   for ( unsigned int i = 0; i < mals_saxs_win->unified_ggaussian_gaussians_size; i++ )
   {
           ...
           fixed_curves.count( i ) )
      {
         ...
         MSFIT_GLOBAL::fixed_params.push_back( mals_saxs_win->unified_ggaussian_params[ ofs ] );
         ...
         MSFIT_GLOBAL::base_params .push_back( mals_saxs_win->unified_ggaussian_params[ ofs ] );
      ...
      if ( dist1_active && mals_saxs_win->cb_fix_dist1->isChecked() ) // if dist1s are common
      {
              ...
              ( cb_comm_dist1->isChecked() && i ) )
         {
            ...
            {
               MSFIT_GLOBAL::fixed_params.push_back( mals_saxs_win->unified_ggaussian_params[ this_ofs ] );
               MSFIT_GLOBAL::comm_backref[ MSFIT_GLOBAL::param_fixed.size() ] = comm_dist_1_var_pos;
            } else {
               MSFIT_GLOBAL::fixed_params.push_back( mals_saxs_win->unified_ggaussian_params[ ofs ] );
            }
            MSFIT_GLOBAL::param_fixed .push_back( true );
         ...
         if ( dist2_active && mals_saxs_win->cb_fix_dist2->isChecked() ) // if dist2s are common
         {
                 ...
                 ( cb_comm_dist2->isChecked() && i ) )
            {
               ...
               {
                  MSFIT_GLOBAL::fixed_params.push_back( mals_saxs_win->unified_ggaussian_params[ this_ofs ] );
                  MSFIT_GLOBAL::comm_backref[ MSFIT_GLOBAL::param_fixed.size() ] = comm_dist_2_var_pos;
               } else {
                  MSFIT_GLOBAL::fixed_params.push_back( mals_saxs_win->unified_ggaussian_params[ ofs ] );
               }
               MSFIT_GLOBAL::param_fixed .push_back( true );
   ...
   unsigned int base_ofs = mals_saxs_win->unified_ggaussian_gaussians_size * common_size;
   // assert( (unsigned int)MSFIT_GLOBAL::param_pos.size() == base_ofs && "common_size*number of gaussians should equal param_pos.size()" );

   for ( unsigned int f = 0; f < mals_saxs_win->unified_ggaussian_curves; f++ )
   {
      unsigned int ofs = base_ofs + f * per_file_size * mals_saxs_win->unified_ggaussian_gaussians_size;
      ...
      for ( unsigned int i = 0; i < mals_saxs_win->unified_ggaussian_gaussians_size; i++ )
      {
         ...
         if ( !mals_saxs_win->cb_fix_width->isChecked() ) // if widths are not common
         {
            if ( cb_fix_width->isChecked() ||
                 fixed_curves.count( i ) )
            {
         ...
         if ( dist1_active && !mals_saxs_win->cb_fix_dist1->isChecked() ) // if dist1s are not common
         {
            if ( cb_fix_dist1->isChecked() ||
                 fixed_curves.count( i ) ||
                 ( cb_comm_dist1->isChecked() && i ) )
   ...
   if ( mals_saxs_win->cb_fix_width->isChecked() )
   {
      ...
      for ( unsigned int i = 0; i < mals_saxs_win->unified_ggaussian_gaussians_size; i++ )
      {
         ...
         {
            MSFIT_GLOBAL::param_pos   .push_back( MSFIT_GLOBAL::fixed_params.size() );
            MSFIT_GLOBAL::fixed_params.push_back( mals_saxs_win->unified_ggaussian_params[ 2 * i + 0 ] );
            MSFIT_GLOBAL::param_fixed .push_back( true );
         } else {
            MSFIT_GLOBAL::param_pos   .push_back( MSFIT_GLOBAL::init_params.size() );
            ...
            MSFIT_GLOBAL::init_params .push_back( base_val );
            MSFIT_GLOBAL::base_params .push_back( mals_saxs_win->unified_ggaussian_params[ 2 * i + 0 ] );
            MSFIT_GLOBAL::param_fixed .push_back( false );
         ...
         {
            MSFIT_GLOBAL::param_pos   .push_back( MSFIT_GLOBAL::fixed_params.size() );
            MSFIT_GLOBAL::fixed_params.push_back( mals_saxs_win->unified_ggaussian_params[ 2 * i + 1 ] );
            MSFIT_GLOBAL::param_fixed .push_back( true );
         } else {
            MSFIT_GLOBAL::param_pos   .push_back( MSFIT_GLOBAL::init_params.size() );
            ...
            MSFIT_GLOBAL::init_params .push_back( base_val );
            MSFIT_GLOBAL::base_params .push_back( mals_saxs_win->unified_ggaussian_params[ 2 * i + 1] );
            MSFIT_GLOBAL::param_fixed .push_back( false );
      ...
      for ( unsigned int i = 0; i < mals_saxs_win->unified_ggaussian_gaussians_size * mals_saxs_win->unified_ggaussian_curves; i++ )
      {
         unsigned int pos = i / mals_saxs_win->unified_ggaussian_curves;

         unsigned int height_pos = 2 * mals_saxs_win->unified_ggaussian_gaussians_size + i + 0;

         if ( cb_fix_amplitude->isChecked() ||
              fixed_curves.count( pos ) )
         {
            MSFIT_GLOBAL::param_pos   .push_back( MSFIT_GLOBAL::fixed_params.size() );
            MSFIT_GLOBAL::fixed_params.push_back( mals_saxs_win->unified_ggaussian_params[ height_pos ] );
            MSFIT_GLOBAL::param_fixed .push_back( true );
         } else {
            MSFIT_GLOBAL::param_pos   .push_back( MSFIT_GLOBAL::init_params.size() );
            ...
            MSFIT_GLOBAL::init_params .push_back( base_val );
            MSFIT_GLOBAL::base_params .push_back( mals_saxs_win->unified_ggaussian_params[ height_pos ] );
            MSFIT_GLOBAL::param_fixed .push_back( false );
      ...
      for ( unsigned int i = 0; i < mals_saxs_win->unified_ggaussian_gaussians_size; i++ )
      {
         ...
         {
            MSFIT_GLOBAL::param_pos   .push_back( MSFIT_GLOBAL::fixed_params.size() );
            MSFIT_GLOBAL::fixed_params.push_back( mals_saxs_win->unified_ggaussian_params[ i ] );
            MSFIT_GLOBAL::param_fixed .push_back( true );
         } else {
            MSFIT_GLOBAL::param_pos   .push_back( MSFIT_GLOBAL::init_params.size() );
            ...
            MSFIT_GLOBAL::init_params .push_back( base_val );
            MSFIT_GLOBAL::base_params .push_back( mals_saxs_win->unified_ggaussian_params[ i ] );
            MSFIT_GLOBAL::param_fixed .push_back( false );
      ...
      for ( unsigned int i = 0; i < mals_saxs_win->unified_ggaussian_gaussians_size * 2 * mals_saxs_win->unified_ggaussian_curves; i+= 2 )
      {
         unsigned int pos = i / ( 2 * mals_saxs_win->unified_ggaussian_curves );

         unsigned int height_pos = mals_saxs_win->unified_ggaussian_gaussians_size + i + 0;
         unsigned int width_pos  = mals_saxs_win->unified_ggaussian_gaussians_size + i + 1;
         ...
         {
            MSFIT_GLOBAL::param_pos   .push_back( MSFIT_GLOBAL::fixed_params.size() );
            MSFIT_GLOBAL::fixed_params.push_back( mals_saxs_win->unified_ggaussian_params[ width_pos ] );
            MSFIT_GLOBAL::param_fixed .push_back( true );
         } else {
            MSFIT_GLOBAL::param_pos   .push_back( MSFIT_GLOBAL::init_params.size() );
            ...
            MSFIT_GLOBAL::init_params .push_back( base_val );
            MSFIT_GLOBAL::base_params .push_back( mals_saxs_win->unified_ggaussian_params[ width_pos ] );
            MSFIT_GLOBAL::param_fixed .push_back( false );
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::test()
{
   ...
   for ( int i = 0; i < (int) MSFIT_GLOBAL::init_params.size(); ++i )
   {
      MSFIT_GLOBAL::init_params[ i ] = ((QLineEdit *)test_widgets[ i ])->text().toDouble();
   }
   ...
   for ( unsigned int i = 0; i < MSFIT_GLOBAL::param_fixed.size(); i++ )
   {
           ...
           MSFIT_GLOBAL::comm_backref.count( i ) )
      {
         ...
         us_qdebug( 
                ...
                .arg( MSFIT_GLOBAL::comm_backref.count( i ) ? "backref used" : "direct pos" ) 
                 );
      } else {
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::lm()
{
   ...
   */
   ...
   {
      gsumf[ i ] = (*MSFIT_GLOBAL::compute_gaussian_f)( t[ i ], &MSFIT_GLOBAL::init_params[ 0 ] );
   }
...
#undef TOL
   ...
   if ( org_rmsd > status.fnorm )
   {
      ...
      // mals_saxs_win->add_ggaussian_curve( "lm_after_pushback", mals_saxs_win->compute_ggaussian_gaussian_sum() );
      gaussians_undo.push_back( mals_saxs_win->unified_ggaussian_params );
      if ( update_mals_saxs )
      {
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::gsm_sd()
{
   ...
   {
      gsm_yp[ j ]  = MSFIT_GLOBAL::compute_gaussian_f( gsm_t[ j ], (double *)(&MSFIT_GLOBAL::base_params[ 0 ]) );
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::stop()
{
   running = false;
}
...
void US_Hydrodyn_Mals_Saxs_Fit_Global::grid()
{
   ...
   unsigned int total_count = ( unsigned int ) pow( (double) pop_size, (int) MSFIT_GLOBAL::init_params.size() );
   ...
   for ( unsigned int j = 0; j < ( unsigned int ) MSFIT_GLOBAL::init_params.size(); j++ )
   {
      ofs_per[ j ] = ( MSFIT_GLOBAL::param_max[ j ] - MSFIT_GLOBAL::param_min[ j ] ) / ( pop_size - 1 );
   }
   ...
   {
      yp[ j ]  = MSFIT_GLOBAL::compute_gaussian_f( t[ j ], (double *)(&MSFIT_GLOBAL::base_params[ 0 ]) );
