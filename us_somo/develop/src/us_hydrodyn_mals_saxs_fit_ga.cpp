...
unsigned int
US_Hydrodyn_Mals_Saxs_Fit::pop_selection (unsigned int size)
{
...
bool
US_Hydrodyn_Mals_Saxs_Fit::ga_fitness (ga_individual &individual)
{
  ...
  our_vector *vi = new_our_vector (MSFIT::init_params.size ());
  for (unsigned int i = 0; i < MSFIT::init_params.size (); i++)
    {
...
bool
US_Hydrodyn_Mals_Saxs_Fit::ga_run (double &nrmsd)
{
  ...
  individual.v.resize (MSFIT::init_params.size ());

  // init population
  for (unsigned int i = 0; i < control_parameters["population"].toUInt (); i++)
    {
      for (unsigned int j = 0; j < MSFIT::init_params.size (); j++)
        {
          individual.v[j]
              = MSFIT::param_min[j]
                + (MSFIT::param_max[j] - MSFIT::param_min[j]) * drand48 ();
        }
           ...
           i++)
        {
          ...
          if (control_parameters.count ("mutate")
              && drand48 () < control_parameters["mutate"].toDouble ())
            {
              ...
              unsigned int pos
                  = (unsigned int)(drand48 () * MSFIT::init_params.size ());
              individual.v[pos]
                  = MSFIT::param_min[pos]
                    + (MSFIT::param_max[pos] - MSFIT::param_min[pos])
                          * drand48 ();
              ga_fitness (individual);
              pop.push_back (individual);
              mutate_count++;
          ...
          if (control_parameters.count ("crossover")
              && drand48 () < control_parameters["crossover"].toDouble ())
            {
              ...
              unsigned int pos
                  = (unsigned int)(drand48 () * MSFIT::init_params.size ());
              for (unsigned int j = pos; j < MSFIT::init_params.size (); j++)
                {
              ...
              for (unsigned int j = 0; j < MSFIT::init_params.size (); j++)
                {
                  individual.v[j]
                      = MSFIT::param_min[j]
                        + (MSFIT::param_max[j] - MSFIT::param_min[j])
                              * drand48 ();
                }
              ga_fitness (individual);
              pop.push_back (individual);
  ...
  for (unsigned int i = 0; i < MSFIT::init_params.size (); i++)
    {
      MSFIT::init_params[i] = pop.front ().v[i];
    }
