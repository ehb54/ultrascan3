...
bool US_Hydrodyn_Mals_Saxs_Fit::ga_run( double & nrmsd )
{
   ...
   for ( unsigned int i = 0; i < control_parameters[ "population" ].toUInt(); i++ )
   {
      ...
      {
         individual.v[ j ] = MSFIT::param_min[ j ] + ( MSFIT::param_max[ j ] - MSFIT::param_min[ j ] ) * drand48();
      }
   ...
   for ( unsigned int g = 0; g < control_parameters[ "generations" ].toUInt(); g++ )
   {
      ...
      for ( unsigned int i = 0; i < control_parameters[ "population" ].toUInt(); i++ )
      {
              ...
              drand48() < control_parameters[ "mutate" ].toDouble() )
         {
            ...
            unsigned int pos = ( unsigned int )( drand48() * MSFIT::init_params.size() );
            individual.v[ pos ] = MSFIT::param_min[ pos ] + ( MSFIT::param_max[ pos ] - MSFIT::param_min[ pos ] ) * drand48();
            ga_fitness( individual );
            pop.push_back( individual );
            mutate_count++;
            continue;
