...
unsigned int US_Hydrodyn_Mals_Saxs_Fit::pop_selection(unsigned int size) {
...
bool US_Hydrodyn_Mals_Saxs_Fit::ga_fitness(ga_individual &individual) {
...
bool US_Hydrodyn_Mals_Saxs_Fit::ga_run(double &nrmsd) {
  ...
  for (unsigned int i = 0; i < control_parameters["population"].toUInt(); i++) {
    for (unsigned int j = 0; j < MSFIT::init_params.size(); j++) {
      individual.v[j] = MSFIT::param_min[j] +
  ...
  for (unsigned int g = 0; g < control_parameters["generations"].toUInt();
    ...
    for (unsigned int i = 0; i < control_parameters["population"].toUInt();
      ...
      if (control_parameters.count("mutate") &&
        ...
        individual.v[pos] =
            MSFIT::param_min[pos] +
            (MSFIT::param_max[pos] - MSFIT::param_min[pos]) * drand48();
        ga_fitness(individual);
        pop.push_back(individual);
        mutate_count++;
        continue;
