...
void US_Hydrodyn::SS_apply(struct PDB_model& model, const QString& ssbondfile) {
  ...
  editor_msg(
      "dark blue",
      QString("Checking disulfide distance, current distance threshold %1 [A]")
          .arg(thresh_SS));
...
#if defined(DEBUG_SS)
      QTextStream(stdout) << "sulfur pdb line   " << sulfur_pdb_line[i]
                          << Qt::endl
                          << "sulfur coordinates"
...
#if defined(DEBUG_SS)
      QTextStream(stdout) << "sulfurs paired: distance [A]:"
                          << pair_distance[it->first] << Qt::endl
...
void US_Hydrodyn::SS_change_residue(struct PDB_model& model,
                                    const QString& line,
  ...
  if (!multi_residue_map.count(target_residue) ||
    ...
    editor_msg("red", QString(us_tr("Residue file is missing residue name %1, "
  ...
  if (!sulfur_pdb_chain_idx.count(chain_id) ||
    ...
    QTextStream(stdout) << "SS_change_residue(): Internal error chain index "
                           "vector missing or empty chain_id "
                        << chain_id << Qt::endl;
    return;
  }
  if (!sulfur_pdb_chain_atom_idx.count(chain_id) ||
    ...
    QTextStream(stdout) << "SS_change_residue(): Internal error chain index "
                           "vector missing or empty chain_id "
                        << chain_id << " residue_sequence " << residue_sequence
                        << Qt::endl;
    return;
  }
...
#if defined(DEBUG_SS)
  {
    ...
    for (int i = 0; i < (int)sulfur_pdb_chain_idx[chain_id].size(); ++i) {
      QTextStream(stdout) << "    sulfur_pdb_chain_idx[ " << chain_id << " ][ "
                          << i << " ] = " << sulfur_pdb_chain_idx[chain_id][i]
                          << Qt::endl;
    }
    for (int i = 0;
         i < (int)sulfur_pdb_chain_atom_idx[chain_id][residue_sequence].size();
         ++i) {
      QTextStream(stdout)
          << "    sulfur_pdb_chain_atom_idx[ " << chain_id << " ][ "
          << residue_sequence << " ][ " << i
...

  {
      ...
      if ((unsigned int)model.molecule.size() >
        ...
        if ((unsigned int)model.molecule[sulfur_pdb_chain_idx[chain_id][i]]
          ...
          for (unsigned int j = 0;
               j < (unsigned int)
                       sulfur_pdb_chain_atom_idx[chain_id][residue_sequence]
                           .size();
               ++j) {
            if (model.molecule[sulfur_pdb_chain_idx[chain_id][i]]
                    .atom[sulfur_pdb_chain_atom_idx[chain_id][residue_sequence]
                                                   [j]]
                    .resName != source_residue) {
              QTextStream(stdout)
                  << "SS_change_residue(): Internal error source residue "
                     "resname mismatch "
                  << " chain_id '" << chain_id << "'"
                  << " ref resname '"
                  << model.molecule[sulfur_pdb_chain_idx[chain_id][i]]
            ...
            model.molecule[sulfur_pdb_chain_idx[chain_id][i]]
                .atom[sulfur_pdb_chain_atom_idx[chain_id][residue_sequence][j]]
                .resName =
                model.molecule[sulfur_pdb_chain_idx[chain_id][i]]
                    .atom[sulfur_pdb_chain_atom_idx[chain_id][residue_sequence]
                                                   [j]]
                    .orgResName = target_residue;
          }
          break;
        } else {
#if defined(DEBUG_SS)
    ...
    if (found) {
      ...
      QTextStream(stdout) << "SS_change_residue(): Internal error could not "
                             "find match in model chain_id "
  ...
  // source_residue & residue sequence once for the residue replace model_vector
  // residue[ model_vector molecule(chain) model_residue_pos ] with new_residue
}
...
#define PH_END 14

  {
    ...
    do {
      ...
      //    QTextStream( stdout )
      //       << QString("").sprintf(
      //                              "iter %d: net charge (start,middle,end) %g
      //                              %g %g pos (start,middle,end) %g %g %g\n"
      //                              ,iter
      //                              ,start_p
      //                              ,middle_p
      //                              ,end_p
    ...
    if (!root_found) {
      editor_msg("red", QString(us_tr("Isoelectric point could not be found in "
...
map<QString, struct atom*> US_Hydrodyn::residue_atom_map(
...
map<QString, struct atom*> US_Hydrodyn::first_residue_atom_map(
    struct PDB_chain& chain) {
  map<QString, struct atom*> result;
  ...
  if (!multi_residue_map.count(resName) || !multi_residue_map[resName].size()) {
  ...
  for (int i = 0; i < atoms; ++i) {
    if (i &&
        (resName != chain.atom[i].resName || resSeq != chain.atom[i].resSeq)) {
      return result;
    }
    if (chain.atom[i].p_atom) {
      if (org_residue_entry.count(chain.atom[i].name)) {
        ...
        if (chain.atom[i].name != "OXT") {
          QTextStream(stderr)
              << "US_Hydrodyn::first_residue_atom_map() resName " << resName
              << " WARNING atom " << chain.atom[i].name
              << " not found in residue list, using p_atom" << Qt::endl;
        }
        result[chain.atom[i].name] = chain.atom[i].p_atom;
      }
    } else {
      QTextStream(stderr)
          << "US_Hydrodyn::first_residue_atom_map() p_atom not set for atom "
...
map<QString, int> US_Hydrodyn::first_residue_PDB_atom_map(
    struct PDB_chain& chain) {
...
map<QString, struct atom*> US_Hydrodyn::last_residue_atom_map(
    struct PDB_chain& chain) {
  map<QString, struct atom*> result;
  int atoms = (int)chain.atom.size();
  if (!atoms) {
  ...
  for (int i = atoms - 1; i >= 0; --i) {
    ...
    if (chain.atom[i].p_atom) {
      ...
      QTextStream(stderr)
          << "US_Hydrodyn::last_residue_atom_map() p_atom not set for atom "
          << chain.atom[i].name << Qt::endl;
    }
  }
  return result;
...
void US_Hydrodyn::clear_temp_dirs() {
  ...
  if (total_size >= (quint64)advanced_config.temp_dir_threshold_mb) {
    switch (QMessageBox::question(
        ...
        QString(
            us_tr(
                "Clear temporary directories using approximately %1 MB of disk "
                "space?\n"
