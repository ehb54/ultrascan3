...
QWidget *ScrollZoomer::cornerWidget() const { return d_cornerWidget; }
...
bool ScrollZoomer::needScrollBar(Qt::Orientation orientation) const {
  ...
  switch (mode) {
    ...
    case Qt::ScrollBarAlwaysOff:
      ...
      break;
    default: {
      if (baseMin < zoomMin || baseMax > zoomMax) needed = true;
      break;
    }
  }
  return needed;
...
void ScrollZoomer::updateScrollBars() {
  ...
  if (showVScrollBar) {
    ...
    if (verticalScrollBar()) {
      ...
      layout->setCanvasMargin(
          layout->canvasMargin(yScrollBarAxis) - verticalScrollBar()->extent(),
          yScrollBarAxis);
    }
  }
  ...
  if (showHScrollBar && showVScrollBar) {
    ...
    if (d_cornerWidget) d_cornerWidget->hide();
  }
...
void ScrollZoomer::scrollBarMoved(Qt::Orientation o, double min, double max) {
  Q_UNUSED(max);
