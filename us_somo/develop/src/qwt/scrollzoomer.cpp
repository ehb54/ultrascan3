...
public:
  ScrollData ()
      : scrollBar (NULL), position (ScrollZoomer::OppositeToScale),
...
ScrollZoomer::ScrollZoomer (QWidget *canvas)
    : QwtPlotZoomer (canvas), d_cornerWidget (NULL), d_hScrollData (NULL),
      d_vScrollData (NULL), d_inZoom (false)
{
...
ScrollZoomer::layoutScrollBars (const QRect &rect)
{
  ...
  if (hScrollBar && hScrollBar->isVisible ())
    {
      int x = rect.x ();
      int y
          = (hPos == QwtPlot::xTop) ? rect.top () : rect.bottom () - hdim + 1;
      int w = rect.width ();
  ...
  if (vScrollBar && vScrollBar->isVisible ())
    {
      ...
      int x
          = (vPos == QwtPlot::yLeft) ? rect.left () : rect.right () - vdim + 1;
      int y = rect.y ();
  ...
  if (hScrollBar && hScrollBar->isVisible () && vScrollBar
      && vScrollBar->isVisible ())
    {
      if (d_cornerWidget)
        {
          QRect cornerRect (vScrollBar->pos ().x (), hScrollBar->pos ().y (),
                            vdim, hdim);
