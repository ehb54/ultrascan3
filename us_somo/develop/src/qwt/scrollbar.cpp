...
ScrollBar::ScrollBar(QWidget *parent) : QScrollBar(parent) { init(); }
...
bool ScrollBar::isInverted() const { return d_inverted; }
...
void ScrollBar::moveSlider(double min, double max) {
  ...
  if (isInverted()) tick = d_baseTicks - tick;

  setSliderPosition(tick);
  blockSignals(false);
}

double ScrollBar::minBaseValue() const { return d_minBase; }

double ScrollBar::maxBaseValue() const { return d_maxBase; }

void ScrollBar::sliderRange(int value, double &min, double &max) const {
  if (isInverted()) value = d_baseTicks - value;
...
int ScrollBar::extent() const {
  ...
  if (orientation() == Qt::Horizontal) opt.state |= QStyle::State_Horizontal;
