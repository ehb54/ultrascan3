...
void US_Hydrodyn_Hydro::setupGUI() {
  ...
  lbl_temperature =
      new QLabel(us_tr(QString(" Temperature (%1C): ").arg(DEGREE_SYMBOL)),
  ...
  cb_manual_solvent_conditions->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  le_tc_solvent_viscosity->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  bg_volume_correction = new QGroupBox(
      "Total Volume of Model: (for Rotational Diff. and Intrinsic Visc. Volume "
      "Corr.) (for SMI)");

  rb_auto_volume = new QRadioButton();
  rb_auto_volume->setText(us_tr(" Auto. (Sum of Bead Vol.)"));
  rb_auto_volume->setEnabled(true);
  rb_auto_volume->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  bg_buried = new QGroupBox(
      "Include Buried Beads in Volume Correction for Calculation of (for "
      "SMI):");
...
void US_Hydrodyn_Hydro::check_solvent_defaults() {
  cb_solvent_defaults->setChecked(hydro->solvent_name == "Water" &&
                                  hydro->solvent_acronym == "w" &&
                                  hydro->temperature == K20 - K0 &&
                                  hydro->solvent_viscosity == VISC_20W * 100 &&
                                  hydro->solvent_density == DENS_20W);
...
void US_Hydrodyn_Hydro::cancel() { close(); }
