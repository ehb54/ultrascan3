  ...
  for (unsigned int g = 1; g <= control_parameters["sgpgenerations"].toUInt ();
       g++)
    {
      ...
      for (unsigned int i = 0;
           i < control_parameters["sgppopulation"].toUInt (); i++)
        {
          ...
          if (control_parameters.count ("sgpplague")
              && drand48 () < control_parameters["sgpplague"].toDouble ())
            {
              ...
              do
                {
                  sgp.random_normal (r1, r2, pop_size_avg, pop_size_sd,
                                     pop_size_avg, pop_size_sd);
                }
              while (r1 < 2.0);
...
unsigned int
US_Saxs_Util::sgp_pop_selection ()
{
...
double
US_Saxs_Util::sgp_fitness (sgp_node *node)
{
  ...
  if (bm.size () > 1)
  ...
  else
    {
      ...
      if (!iqq_sphere ("tmp", bm[0].radius, delta_rho,
...
void
US_Saxs_Util::sgp_calculate_population_fitness ()
{
  for (unsigned int i = 0; i < population.size (); i++)
    {
...
void
US_Saxs_Util::sgp_sort_population ()
{
