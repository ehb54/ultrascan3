...
static void
write_csv_data (QFile *f, QString I_tag)
{
  int maxlen = 0;
  ...
  for (int i = 0; i < (int)guinier_names.size (); ++i)
    {
      ...
      for (int j = 0; j < maxlen; ++j)
        {
          out[j + 1] << (j < (int)guinier_q2[i].size ()
                             ? QString ("%1").arg (guinier_q2[i][j], 0, 'g', 8)
                             : QString (""));
        }
      ...
      for (int j = 0; j < maxlen; ++j)
        {
          out[j + 1] << (j < (int)guinier_lnI[i].size ()
                             ? QString ("%1").arg (guinier_lnI[i][j], 0, 'g',
                                                   8)
                             : QString (""));
        }
      ...
      if (guinier_sd[i].size ())
        {
          ...
          for (int j = 0; j < maxlen; ++j)
            {
              out[j + 1] << (j < (int)guinier_sd[i].size ()
                                 ? QString ("%1").arg (guinier_sd[i][j], 0,
                                                       'g', 8)
                                 : QString (""));
            }
...
void
US_Hydrodyn_Saxs::run_guinier_Rt ()
{
  clear_csv_data ();
  ...
  if (our_saxs_options->guinier_csv)
    {
      QString filename = USglobal->config_list.root_dir + SLASH + "somo"
                         + SLASH + "saxs" + SLASH
      ...
      if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("guinier_csv_save_data")
                 ...
                 == "1")
        {
          QString filename = USglobal->config_list.root_dir + SLASH + "somo"
                             + SLASH + "saxs" + SLASH
                             + our_saxs_options->guinier_csv_filename
                             + "_tv_data.csv";
  ...
  if (plotted_Iq.size () > 1)
    {
      ...
      QStringList toplot;
      toplot << "yleft" << us_tr ("Rt [Angstrom]") << "Rt"
...
void
US_Hydrodyn_Saxs::guinier_frame_plot (const QString &title,
                                      const QStringList &toplot)
{
  map<QString, QString> parameters;
  map<QString, vector<vector<double> > >
      plots; // type (left or right) => and vectors of color, x, y and possibly
             // error
  ...
  for (unsigned int i = 0; i < plotted_Iq.size (); i++)
    {
      ...
      color_index.push_back ((double)(tmp.red () * 256 + tmp.green ()) * 256
                             + tmp.blue ());
...

  {
    ...
    QString head
        = US_Pdb_Util::qstring_common_head (qsl_plotted_iq_names, true);
    QString tail
        = US_Pdb_Util::qstring_common_tail (qsl_plotted_iq_names, true);
    ...
    for (unsigned int i = 0; i < plotted_Iq.size (); i++)
      {
        ...
        // us_qdebug( QString( "filename %1 tmp is %2" ).arg(
        // qsl_plotted_iq_names[ i ] ).arg( tmp ) );
            ...
            if (!skip_curves.count (i))
              {
                ...
                QString key_sd
                    = QString ("%1-%2-sd").arg (i).arg (toplot[k + 2]);
                ...
                if (guinier_scratch.count (key))
                  {
                    ...
                    sd.push_back (guinier_scratch.count (key_sd)
                                      ? guinier_scratch[key_sd]
                                      : 0e0);
                  }
              }
          }
  ...
  if (plots.size ())
    {
      US_Hydrodyn_Saxs_Guinier_Frames *uhsgf
          = new US_Hydrodyn_Saxs_Guinier_Frames (us_hydrodyn, parameters,
                                                 plots);
      uhsgf->show ();
    }
}
...
void
US_Hydrodyn_Saxs::run_guinier_cs ()
{
  clear_csv_data ();
  guinier_scratch.clear ();
  ...
  if (plotted_Iq.size () > 1)
    {
      ...
      QStringList toplot;
      toplot << "yleft" << us_tr ("Rc [Angstrom]") << "Rc"
...
void
US_Hydrodyn_Saxs::run_guinier_analysis ()
{
  check_mwt_constants ();
  ...
  if (plotted_Iq.size () > 1)
    {
      ...
      QStringList toplot;
      toplot << "yleft" << us_tr ("Rg [Angstrom]") << "Rg"
...
bool
US_Hydrodyn_Saxs::guinier_analysis (unsigned int i, QString &csvlog)
{
      ...
      if (plotted_I_error[i].size () != plotted_q[i].size ())
        {
          editor_msg (
              "dark red",
              QString (us_tr ("Notice: SD weighting of Guinier fit is off for "
                              "%1 since SDs are not fully present"))
                  ...
                  if (plotted_I_error[i][j] <= 0e0)
                    {
                      editor_msg (
                          ...
                          QString (
                              us_tr (
                                  "Notice: SD weighting of Guinier fit is off "
                                  "for %1 since at least one SD is zero or "
  ...
  if (use_guinier_outlier_reject && !use_SD_weighting)
    {
      editor_msg (
          "dark red",
          QString (us_tr ("Notice: Turning off outlier rejection for Guinier "
                          "fit of %1 since SD weighting is off"))
              .arg (qsl_plotted_iq_names[i]));
  ...
  // US_Vector::printvector3( "q2 I e", q2 /* usu.wave["data"].q */,
  // usu.wave["data"].r, usu.wave["data"].s );
  ...
  if (!too_few_points)
    {
      if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("guinier_auto_fit")
        ...
        {
          if (!usu.guinier_plot ("guinier", "data")
              || !usu.guinier_fit2 (this_log, "guinier", pointsmin, pointsmax,
            ...
            {
              editor->append (
                  QString ("Error performing Guinier analysis on %1\n"
                           + usu.errormsg + "\n")
                      .arg (qsl_plotted_iq_names[i]));
              return false;
            }
      ...
      else
        {
          bool do_decrease
              = ((US_Hydrodyn *)us_hydrodyn)
                    ->gparams.count ("guinier_use_qRlimit")
          ...
          do
            {
                  ...
                  if ((int)usu.wave["data"].q.size () - (int)pts_decrease < 5)
                    {
                      internal_error
                          = QString (
                                us_tr ("Too few points left (%1) after %2 "
                                       "points removed for qRgmax limit"))
                                .arg ((int)usu.wave["data"].q.size ()
                                      - (int)pts_decrease)
                                .arg (pts_decrease);
                      // editor->append( QString( "Error performing Guinier
                      // analysis on %1: %2\n" ).arg( qsl_plotted_iq_names[ i ]
                      // ).arg( internal_error ) );
                      break;
                    }

                  usu.wave["data"].q.resize (usu.wave["data"].q.size ()
                                             - pts_decrease);
              ...
              //             if ( do_decrease )
              //             {
              //                US_Vector::printvector3( "q I e",
              //                usu.wave["data"].q, usu.wave["data"].r,
              //                usu.wave["data"].s );
              //             }

              unsigned int pstart = 0;
              unsigned int pend = usu.wave["data"].q.size ()
                                      ? usu.wave["data"].q.size () - 1
                                      : 0;
              bestend = pend;
              beststart = pstart;
              ...
              if (use_guinier_outlier_reject)
                {
                  // puts( "outlier reject" );
                  // US_Vector::printvector( "'data'.q orig", usu.wave[ "data"
                  // ].q );
                  map<double, double> removed;
                  if (!usu.guinier_plot ("guinier", "data")
                      || !usu.guinier_fit_with_removal (
                          this_log, "guinier", pstart, pend, a, b, siga, sigb,
                          chi2, Rg, I0,
                          smax, // don't know why these are flipped
                          smin, sRgmin, sRgmax,
                          our_saxs_options->guinier_outlier_reject_dist,
                          removed, pts_removed, false))
                    {
                      editor->append (
                          QString ("Error performing Guinier analysis on %1\n"
                                   + usu.errormsg + "\n")
                  ...
                  if (removed.size ())
                    {
                      ...
                      // US_Vector::printvector( "'data'.q", usu.wave[ "data"
                      // ].q ); US_Vector::printvector( "'guinier'.q",
                      // usu.wave[ "guinier" ].q ); vector < double > f; vector
                      // < double > q2;
                      ...
                      for (int j = 0; j < (int)usu.wave["data"].q.size (); ++j)
                        {
                          // q2.push_back( usu.wave[ "data" ].q[ j ] *
                          // usu.wave[ "data" ].q[ j ] );
                          if (removed_match.count (QString ("%1").arg (
                                  usu.wave["data"].q[j]
                                  * usu.wave["data"].q[j])))
                            {
                              qs_removed
                                  += QString ("%1%2")
                                         .arg (qs_removed.isEmpty () ? ""
                                                                     : "_")
                                         .arg (j + 1);
                            }
                        }
                      // cout << "qs_removed:" << qs_removed << endl;
                      // US_Vector::printvector( "q2 computed from
                      // wave['data']", q2 );
                    }
                  else
                    {
              ...
              //             if ( do_decrease )
              //             {
              //                printf( "do decrease, sRgmax %g sRgmaxlimit
              //                %g\n", sRgmax, sRgmaxlimit );
              //             }
  ...
  if (too_few_points || (bestend - beststart + 1 - pts_removed < pointsmin)
      || !internal_error.isEmpty ())
    {
      if (!internal_error.isEmpty ())
        {
          report = QString ("Guinier analysis of %1:\n"
                            "**** %2 ****\n")
                       .arg (qsl_plotted_iq_names[i])
                       .arg (internal_error);
          csvlog += QString ("\"%1\","
      ...
      else
        {
          if (pts_removed)
            {
              report = QString ("Guinier analysis of %1:\n"
                                "**** Could not compute Rg, too few data "
                                "points %2 after removal ****\n")
                           .arg (qsl_plotted_iq_names[i])
  ...
  else
    {
      if (us_isnan (Rg) || b >= 0e0)
      ...
      else
        {
          ...
          //          if ( mw_from_I0( qsl_plotted_iq_names[ i ], siga, MW_sd,
          //          ICL ) )
          //          {
          //             // cout << QString( "siga %1 MW_sd %2 ICL %3\n" ).arg(
          //             siga ).arg( MW_sd ).arg( ICL );
          //          } else {
          //             // cout << errormsg << endl;
          //          }
          ...
          report = QString ("").sprintf (
              ...
              "Rg %.1f (%.1f) (A) I(0) %.2e (%.2e) MW %.2e (%.2e) q^2 [ "
              "%.5f:%.5f] qRgmin %.3f qRgmax %.3f points used %u chi^2 %.2e "
              "reduced-chi %.2e",
              qsl_plotted_iq_names[i].toLatin1 ().data (), Rg, sigb, I0, siga,
              MW, MW_sd, smin * smin, smax * smax, sRgmin, sRgmax,
              bestend - beststart + 1 - pts_removed, chi2,
              sqrt (chi2 / (bestend - beststart - 1 - pts_removed)));

          // us_qdebug( QString( "plotted q.size() %1" ).arg( plotted_q[ i
          // ].size() ) );
          ...
          if (!istarq && !ihashq)
            {
              if (US_Saxs_Util::mwt (
                      plotted_q[i], plotted_I[i], Rg, sigb, I0, siga,
                      ((US_Hydrodyn *)us_hydrodyn)
                              ->gparams.count ("guinier_mwt_k")
                          ? ((US_Hydrodyn *)us_hydrodyn)
                                ->gparams["guinier_mwt_k"]
                                .toDouble ()
                          : 0e0,
                      ((US_Hydrodyn *)us_hydrodyn)
                              ->gparams.count ("guinier_mwt_c")
                          ? ((US_Hydrodyn *)us_hydrodyn)
                                ->gparams["guinier_mwt_c"]
                                .toDouble ()
                          : 0e0,
                      ((US_Hydrodyn *)us_hydrodyn)
                              ->gparams.count ("guinier_mwt_qmax")
                          ? ((US_Hydrodyn *)us_hydrodyn)
                                ->gparams["guinier_mwt_qmax"]
                                .toDouble ()
                          : 0e0,
                      Vct, Qrt, mwt, mwt_sd, messagest, notest, warningt))
                {
                  report += QString ("").sprintf (
                ...
                {
                  report += us_tr (" MW(Vc) could not compute " + messagest
                                   + " " + notest);
                }
            }
          ...
          if (started_in_expert_mode && !istarq && !ihashq)
            {
              if (((US_Hydrodyn *)us_hydrodyn)
                      ->saxs_util->mwc (
                          plotted_q[i], plotted_I[i], Rg, sigb, I0, siga,
                          ((US_Hydrodyn *)us_hydrodyn)
                                  ->gparams.count ("guinier_mwc_mw_per_N")
                              ? ((US_Hydrodyn *)us_hydrodyn)
                                    ->gparams["guinier_mwc_mw_per_N"]
                                    .toDouble ()
                              : 0e0,
                          qm, Vcc, Qrc, mwc, mwc_sd, messagesc, notesc))
                {
                  report += QString ("").sprintf (
                ...
                {
                  report += us_tr (" MW(Vc) could not compute " + messagesc
                                   + " " + notesc);
                }
            }
          ...
          plotted_guinier_y[i].push_back (
              exp (plotted_guinier_a[i]
                   + plotted_guinier_b[i] * plotted_guinier_lowq2[i]));
          plotted_guinier_y[i].push_back (
              exp (plotted_guinier_a[i]
                   + plotted_guinier_b[i] * plotted_guinier_highq2[i]));
          ...
          */
          ...
          csvlog += QString ("%1,"
                        ...
                        .arg (chi2)
                        .arg (sqrt (
                            chi2 / (bestend - beststart - 1 - pts_removed)));

          {
            ...
            get_conc_csv_values (qsl_plotted_iq_names[i], conc, psv,
                                 I0_std_exp);
            csvlog
                += QString ("%1,"
                            "%2,"
          ...
          if (plotted_I[i][j] > 0e0)
            {
              ...
              if (!mw_from_I0 (qsl_plotted_iq_names[i], exp (lnI.back ()), mw,
                               ICL))
                {
              ...
              if (use_SD_weighting)
                {
                  sd.push_back (
                      fabs (plotted_I_error[i][j] / plotted_I[i][j]));
                  // ( lnI.back() / log( plotted_I[ i ][ j ] ) ) * fabs(
                  // plotted_I_error[ i ][ j ] / plotted_I[ i ][ j ] ) : 0e0 );
                  // fabs( lnI.back() ) * ( plotted_I_error[ i ][ j ] /
                  // plotted_I[ i ][ j ] ) : 0e0 );
                }
            }
        }
  ...
  editor_msg (plot_colors[i % plot_colors.size ()],
              plot_saxs->canvasBackground ().color (), report);
...
bool
US_Hydrodyn_Saxs::cs_guinier_analysis (unsigned int i, QString &csvlog)
{
  if (i > plotted_Iq.size ())
    {
      editor->append ("internal error: invalid plot selected\n");
  ...
  for (unsigned int j = 0; j < plotted_q[i].size (); j++)
    {
      if (plotted_q[i][j] >= our_saxs_options->qstart
          && plotted_q[i][j] <= our_saxs_options->qend)
        {
          ...
          if (use_SD_weighting)
            {
              usu.wave["data"].s.push_back (plotted_q[i][j]
                                            * plotted_I_error[i][j]);
            }
        }
    }

  // US_Vector::printvector3( "cs1 q2 I e", q2 /* usu.wave["data"].q */,
  // usu.wave["data"].r, usu.wave["data"].s );
  ...
  if (!too_few_points)
    {
      if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("guinier_auto_fit")
          ...
          && (unsigned int)usu.wave["data"].q.size () > pointsmin)
        {
          if (!usu.guinier_plot ("cs_guinier", "data")
              || !usu.guinier_fit2 (
                  this_log, "cs_guinier", pointsmin, pointsmax, sRgmaxlimit,
                  pointweightpower, p_guinier_maxq, a, b, siga, sigb, chi2, Rg,
                  I0,
                  smax, // don't know why these are flipped
                  smin, sRgmin, sRgmax, beststart, bestend, true))
            {
              editor_msg (
                  "red",
          ...
          do
            {
              ...
              for (unsigned int j = 0; j < plotted_q[i].size (); j++)
                {
                  if (plotted_q[i][j] >= our_saxs_options->qstart
                      && plotted_q[i][j] <= our_saxs_options->qend)
                    {
                      usu.wave["data"].q.push_back (plotted_q[i][j]);
                      // q2.push_back(  plotted_q2[ i ][ j ] );
                      usu.wave["data"].r.push_back (plotted_q[i][j]
                                                    * plotted_I[i][j]);
                      if (use_SD_weighting)
                        {
                          usu.wave["data"].s.push_back (
                              plotted_q[i][j] * plotted_I_error[i][j]);
                        }
                    }
                }
                      ...
                      internal_error
                          = QString (
                                ...
                                .arg ((int)usu.wave["data"].q.size ()
                                      - (int)pts_decrease)
                                .arg (pts_decrease);
                      // editor->append( QString( "Error performing CS Guinier
                      // analysis on %1: %2\n" ).arg( qsl_plotted_iq_names[ i ]
                      // ).arg( internal_error ) );
                      break;
                    }
              ...
              // US_Vector::printvector3( "cs2 q2 I e", q2 /*
              // usu.wave["data"].q */, usu.wave["data"].r, usu.wave["data"].s
              // );
              ...
              if (use_guinier_outlier_reject)
                {
                  ...
                  if (!usu.guinier_plot ("guinier", "data")
                      || !usu.guinier_fit_with_removal (
                          ...
                          smin, sRgmin, sRgmax,
                          our_saxs_options->guinier_outlier_reject_dist,
                          removed, pts_removed, true))
                    {
                      editor->append (
                          QString (
                              "Error performing CS Guinier analysis on %1\n"
                              + usu.errormsg + "\n")
                              .arg (qsl_plotted_iq_names[i]));
                      return false;
                    }
  ...
  if (too_few_points || (bestend - beststart + 1 - pts_removed < pointsmin)
      || !internal_error.isEmpty ())
    {
      if (!internal_error.isEmpty ())
        {
          report = QString ("CS Guinier analysis of %1:\n"
                            "**** %2 ****\n")
                       .arg (qsl_plotted_iq_names[i])
                       .arg (internal_error);
          csvlog += QString ("\"%1\","
      ...
      else
        {
          if (pts_removed)
            {
              report = QString ("CS Guinier analysis of %1:\n"
                                "**** Could not compute Rc, too few data "
                                "points %2 after removal ****\n")
                           .arg (qsl_plotted_iq_names[i])
                           .arg (bestend - beststart + 1 - pts_removed);
              csvlog
      ...
      if (us_isnan (Rg) || b >= 0e0)
      ...
      else
        {
          ...
          report = QString ("").sprintf (
              ...
              "Rc %.1f (%.1f) (A) I(0) %.2e (%.2e) M/L %.2e (%.2e) q^2 "
              "[%.5f:%.5f] qRcmin %.3f qRcmax %.3f points used %u chi^2 %.2e "
              "reduced-chi %.2e\n",
              qsl_plotted_iq_names[i].toLatin1 ().data (), Rg, sigb, I0, siga,
              ML, ML_sd, smin * smin, smax * smax, sRgmin, sRgmax,
              bestend - beststart + 1 - pts_removed, chi2,
              sqrt (chi2 / (bestend - beststart - 1 - pts_removed)));
          ...
          plotted_cs_guinier_y[i].push_back (
              exp (plotted_cs_guinier_a[i]
                   + plotted_cs_guinier_b[i] * plotted_cs_guinier_lowq2[i]));
          plotted_cs_guinier_y[i].push_back (
              exp (plotted_cs_guinier_a[i]
                   + plotted_cs_guinier_b[i] * plotted_cs_guinier_highq2[i]));

          /*
          cout << QString( "cs guinier line range q %1 %2 q^2 %3 %4 a %5 b
          %6\n" ) .arg( smin  ) .arg( smax ) .arg( plotted_cs_guinier_lowq2[ i
          ] ) .arg( plotted_cs_guinier_highq2[ i ] ) .arg( a ) .arg( b )
             ;
          */
          ...
          if (plotted_I[i][j] > 0e0)
            {
              ...
              if (!ml_from_qI0 (qsl_plotted_iq_names[i], exp (lnI.back ()), ml,
                                ICL))
                {
              ...
              if (use_SD_weighting)
                {
                  sd.push_back (
                      fabs (plotted_I_error[i][j] / plotted_I[i][j]));
                  // ( lnI.back() / log( plotted_q[ i ][ j ] * plotted_I[ i ][
                  // j ] ) ) * fabs( plotted_I_error[ i ][ j ] / plotted_I[ i
                  // ][ j ] ) : 0e0 ); fabs( lnI.back() ) * ( plotted_I_error[
                  // i ][ j ] / plotted_I[ i ][ j ] ) : 0e0 );
                }
            }
        }
      ...
      if (use_guinier_outlier_reject)
        {
          guinier_removed.push_back (plotted_cs_guinier_pts_removed.count (i)
                                         ? plotted_cs_guinier_pts_removed[i]
                                         : empty_removed);
        }
      else
        {
...
bool
US_Hydrodyn_Saxs::Rt_guinier_analysis (unsigned int i, QString &csvlog)
{
  if (i > plotted_Iq.size ())
    {
      editor->append ("internal error: invalid plot selected\n");
  ...
  for (unsigned int j = 0; j < plotted_q[i].size (); j++)
    {
      if (plotted_q[i][j] >= our_saxs_options->qstart
          && plotted_q[i][j] <= our_saxs_options->qend)
        {
          usu.wave["data"].q.push_back (plotted_q[i][j]);
          // q2.push_back(  plotted_q2[ i ][ j ] );
          usu.wave["data"].r.push_back (plotted_q[i][j] * plotted_q[i][j]
                                        * plotted_I[i][j]);
          if (use_SD_weighting)
            {
              usu.wave["data"].s.push_back (plotted_q[i][j] * plotted_q[i][j]
                                            * plotted_I_error[i][j]);
            }
        }
    }
  ...
  if (!too_few_points)
    {
      if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("guinier_auto_fit")
          ...
          && (unsigned int)usu.wave["data"].q.size () > pointsmin)
        {
          if (!usu.guinier_plot ("tv_guinier", "data")
              || !usu.guinier_fit2 (
                  this_log, "tv_guinier", pointsmin, pointsmax, sRgmaxlimit,
                  pointweightpower, p_guinier_maxq, a, b, siga, sigb, chi2, Rg,
                  I0,
                  smax, // don't know why these are flipped
                  smin, sRgmin, sRgmax, beststart, bestend, false, true))
            {
              editor_msg (
                  "red",
              ...
              for (unsigned int j = 0; j < plotted_q[i].size (); j++)
                {
                  if (plotted_q[i][j] >= our_saxs_options->qstart
                      && plotted_q[i][j] <= our_saxs_options->qend)
                    {
                      usu.wave["data"].q.push_back (plotted_q[i][j]);
                      // q2.push_back(  plotted_q2[ i ][ j ] );
                      usu.wave["data"].r.push_back (
                          plotted_q[i][j] * plotted_q[i][j] * plotted_I[i][j]);
                      if (use_SD_weighting)
                        {
                          usu.wave["data"].s.push_back (
                              plotted_q[i][j] * plotted_q[i][j]
                              * plotted_I_error[i][j]);
                        }
                    }
                }
                      ...
                      internal_error
                          = QString (
                                ...
                                .arg ((int)usu.wave["data"].q.size ()
                                      - (int)pts_decrease)
                                .arg (pts_decrease);
                      // editor->append( QString( "Error performing TV Guinier
                      // analysis on %1: %2\n" ).arg( qsl_plotted_iq_names[ i ]
                      // ).arg( internal_error ) );
                      break;
                    }
              ...
              if (use_guinier_outlier_reject)
                {
                  ...
                  if (!usu.guinier_plot ("guinier", "data")
                      || !usu.guinier_fit_with_removal (
                          ...
                          smin, sRgmin, sRgmax,
                          our_saxs_options->guinier_outlier_reject_dist,
                          removed, pts_removed, false, true))
                    {
                      editor->append (
                          QString (
                              "Error performing TV Guinier analysis on %1\n"
                              + usu.errormsg + "\n")
                              .arg (qsl_plotted_iq_names[i]));
                      return false;
                    }
  ...
  if (too_few_points || (bestend - beststart + 1 - pts_removed < pointsmin)
      || !internal_error.isEmpty ())
    {
      if (!internal_error.isEmpty ())
        {
          report = QString ("TV Guinier analysis of %1:\n"
                            "**** %2 ****\n")
                       .arg (qsl_plotted_iq_names[i])
                       .arg (internal_error);
          csvlog += QString ("\"%1\","
      ...
      else
        {
          if (pts_removed)
            {
              report = QString ("TV Guinier analysis of %1:\n"
                                "**** Could not compute Rt, too few data "
                                "points %2 after removal ****\n")
                           .arg (qsl_plotted_iq_names[i])
                           .arg (bestend - beststart + 1 - pts_removed);
              csvlog
      ...
      if (us_isnan (Rg) || b >= 0e0)
      ...
      else
        {
          ...
          report = QString ("").sprintf (
              ...
              "Rt %.1f (%.1f) (A) I(0) %.2e (%.2e) M/A %.2e (%.2e) q^2 "
              "[%.5f:%.5f] qRcmin %.3f qRcmax %.3f points used %u chi^2 %.2e "
              "reduced-chi %.2e\n",
              qsl_plotted_iq_names[i].toLatin1 ().data (), Rg, sigb, I0, siga,
              MA, MA_sd, smin * smin, smax * smax, sRgmin, sRgmax,
              bestend - beststart + 1 - pts_removed, chi2,
              sqrt (chi2 / (bestend - beststart - 1 - pts_removed)));
          ...
          plotted_Rt_guinier_y[i].push_back (
              exp (plotted_Rt_guinier_a[i]
                   + plotted_Rt_guinier_b[i] * plotted_Rt_guinier_lowq2[i]));
          plotted_Rt_guinier_y[i].push_back (
              exp (plotted_Rt_guinier_a[i]
                   + plotted_Rt_guinier_b[i] * plotted_Rt_guinier_highq2[i]));

          /*
          cout << QString( "cs guinier line range q %1 %2 q^2 %3 %4 a %5 b
          %6\n" ) .arg( smin  ) .arg( smax ) .arg( plotted_Rt_guinier_lowq2[ i
          ] ) .arg( plotted_Rt_guinier_highq2[ i ] ) .arg( a ) .arg( b )
             ;
          */
          ...
          if (plotted_I[i][j] > 0e0)
            {
              q2.push_back (plotted_q[i][j] * plotted_q[i][j]);
              lnI.push_back (
                  log (plotted_q[i][j] * plotted_q[i][j] * plotted_I[i][j]));
              if (!ma_from_q2I0 (qsl_plotted_iq_names[i], exp (lnI.back ()),
                                 ma, ICL))
                {
              ...
              if (use_SD_weighting)
                {
                  sd.push_back (
                      fabs (plotted_I_error[i][j] / plotted_I[i][j]));
                  // ( lnI.back() / log( plotted_q[ i ][ j ] * plotted_q[ i ][
                  // j ] * plotted_I[ i ][ j ] ) ) * fabs( plotted_I_error[ i
                  // ][ j ] / plotted_I[ i ][ j ] ) : 0e0 );
                  // fabs( lnI.back() ) * ( plotted_I_error[ i ][ j ] /
                  // plotted_I[ i ][ j ] ) : 0e0 );
                }
      ...
      if (use_guinier_outlier_reject)
        {
          guinier_removed.push_back (plotted_Rt_guinier_pts_removed.count (i)
                                         ? plotted_Rt_guinier_pts_removed[i]
                                         : empty_removed);
        }
      else
        {
...
US_Hydrodyn_Saxs::set_guinier ()
{
      ...
      plot_saxs->setTitle (
          (cb_guinier->isChecked ()
               ? (cb_cs_guinier->isChecked ()
                      ? "CS Guinier "
                      : (cb_Rt_guinier->isChecked () ? "TV Guinier "
                                                     : "Guinier "))
               : "")
          + us_tr ("SANS Curve"));
    }
  ...
  ;
  for (unsigned int p = 0; p < plotted_Iq.size (); p++)
    {
      ...
      if (cb_guinier->isChecked () && cb_Rt_guinier->isChecked ())
        {
          for (unsigned int i = 0; i < plotted_q[p].size (); i++)
            {
              pI.push_back (plotted_q[p][i] * plotted_q[p][i]
                            * plotted_I[p][i]);
            }
        }
      ...
      plotted_Iq[p]->setSamples (
          ...
          cb_kratky->isChecked () ? (double *)&(q2I[0])
              ...
              (cb_guinier->isChecked ()
                   ...
                   : (double *)&(plotted_I[p][0])),
          q_points);
      plotted_Iq[p]->setPen (QPen (plot_colors[p % plot_colors.size ()],
  ...
  bool do_errorbars = ((US_Hydrodyn *)us_hydrodyn)
                          ->gparams.count ("saxs_cb_resid_show_errorbars")
                      && ((US_Hydrodyn *)us_hydrodyn)
                                 ->gparams["saxs_cb_resid_show_errorbars"]
                             == "1";
      ...
      if (do_errorbars)
        {
          ...
          if (total_points > 20000)
            {
              editor_msg ("dark red", "Notice: error bars are turned off due "
                                      "to too many curve-points");
              do_errorbars = false;
            }
        }
  ...
  for (int i = 0; i < niqsize; i++)
    {
      if (cb_guinier->isChecked ())
          ...
          if (cb_cs_guinier->isChecked ())
            {
              ...
              if (plotted_cs_guinier_valid.count (i)
                  && plotted_cs_guinier_valid[i] == true)
                {
                  ...
                  plotted_cs_Gp_curves[i]->setSamples (
                      ...
                      (double *)&(plotted_cs_guinier_y[i][0]), 2);
                  plotted_cs_Gp_curves[i]->setPen (
                      QPen (QColor ("dark red"), pen_width, Qt::SolidLine));
                  ...
                  if (plotted_q2[i].size ())
                    {
                      ...
                      y[0] = exp (plotted_cs_guinier_a[i]
                                  + plotted_cs_guinier_b[i] * x[0]);
                      y[1] = exp (plotted_cs_guinier_a[i]
                                  + plotted_cs_guinier_b[i] * x[1]);
                      plotted_cs_Gp_curves_full[i] = new QwtPlotCurve (
                          QString (QString ("CS Gn. %1")
                                       .arg (qsl_plotted_iq_names[i])));
                      plotted_cs_Gp_curves_full[i]->setStyle (
                          QwtPlotCurve::Lines);
                      plotted_cs_Gp_curves_full[i]->setSamples (
                          (double *)&(x[0]), (double *)&(y[0]), 2);
                      plotted_cs_Gp_curves_full[i]->setPen (
                          QPen (QColor ("dark red"), pen_width, Qt::DotLine));
              ...
              plotted_Iq[i]->setSymbol (new QwtSymbol (
                  ...
                  if (plotted_Rt_guinier_valid.count (i)
                      && plotted_Rt_guinier_valid[i] == true)
                    {
                      ...
                      plotted_Rt_Gp_curves[i]->setSamples (
                          ...
                          (double *)&(plotted_Rt_guinier_y[i][0]), 2);
                      plotted_Rt_Gp_curves[i]->setPen (QPen (
                          QColor ("dark red"), pen_width, Qt::SolidLine));
                      ...
                      if (plotted_q2[i].size ())
                        {
                          ...
                          x[0] = plotted_q2[i][0];
                          x[1] = plotted_q2[i].back ();
                          y[0] = exp (plotted_Rt_guinier_a[i]
                                      + plotted_Rt_guinier_b[i] * x[0]);
                          y[1] = exp (plotted_Rt_guinier_a[i]
                                      + plotted_Rt_guinier_b[i] * x[1]);
                          plotted_Rt_Gp_curves_full[i] = new QwtPlotCurve (
                              QString (QString ("CS Gn. %1")
                                           .arg (qsl_plotted_iq_names[i])));
                          plotted_Rt_Gp_curves_full[i]->setStyle (
                              QwtPlotCurve::Lines);
                          plotted_Rt_Gp_curves_full[i]->setSamples (
                              (double *)&(x[0]), (double *)&(y[0]), 2);
                          plotted_Rt_Gp_curves_full[i]->setPen (QPen (
                              QColor ("dark red"), pen_width, Qt::DotLine));
                  ...
                  if (plotted_guinier_valid.count (i)
                      && plotted_guinier_valid[i] == true)
                    {
                      ...
                      plotted_Gp_curves[i]->setSamples (
                          ...
                          (double *)&(plotted_guinier_y[i][0]), 2);
                      plotted_Gp_curves[i]->setPen (QPen (
                          QColor ("dark red"), pen_width, Qt::SolidLine));
                      ...
                      if (plotted_q2[i].size ())
                        {
                          ...
                          x[0] = plotted_q2[i][0];
                          x[1] = plotted_q2[i].back ();
                          y[0] = exp (plotted_guinier_a[i]
                                      + plotted_guinier_b[i] * x[0]);
                          y[1] = exp (plotted_guinier_a[i]
                                      + plotted_guinier_b[i] * x[1]);
                          plotted_Gp_curves_full[i]
                              = new QwtPlotCurve (QString ("Gn. %1").arg (
                                  qsl_plotted_iq_names[i]));
                          plotted_Gp_curves_full[i]->setStyle (
                              QwtPlotCurve::Lines);
                          plotted_Gp_curves_full[i]->setSamples (
                              (double *)&(x[0]), (double *)&(y[0]), 2);
                          plotted_Gp_curves_full[i]->setPen (QPen (
                              QColor ("dark red"), pen_width, Qt::DotLine));
      ...
      else
        {
          ...
          plotted_Iq[i]->setSymbol (new QwtSymbol (sym.style (), sym.brush (),
                                                   sym.pen (), sym.size ()));
          // plotted_Iq[ i ]->setStyle( QwtPlotCurve::Lines );
          plotted_Iq[i]->setPen (QPen (plot_colors[i % plot_colors.size ()],
                                       pen_width, Qt::SolidLine));
        }
...
void
US_Hydrodyn_Saxs::set_guinier_eb ()
{
  ...
  symbol.setBrush (
      Qt::NoBrush); // plot_colors[ f_pos[ file ] % plot_colors.size() ] );

  for (int i = 0; i < niqsize; i++)
    {
      bool useable_errors = plotted_I[i].size () == plotted_I_error[i].size ()
                            && !std::all_of (plotted_I_error[i].begin (),
                                             plotted_I_error[i].end (),
                                             [] (double i) { return i == 0; });
...

      {
                ...
                for (unsigned int j = 0; j < q_points; ++j)
                  {
                    I.push_back (plotted_q2[i][j] * plotted_I[i][j]);
                    pElow.push_back (
                        I.back () - plotted_q2[i][j] * plotted_I_error[i][j]);
                    pEhigh.push_back (
                        I.back () + plotted_q2[i][j] * plotted_I_error[i][j]);
                  }
              }
                        ...
                        if (plotted_I[i][j] > 0e0)
                          {
                            ...
                            q2.push_back (plotted_q2[i][j]);
                            I.push_back (plotted_q[i][j] * plotted_I[i][j]);
                            pElow.push_back (I.back ()
                                             - plotted_q[i][j]
                                                   * plotted_I_error[i][j]);
                            pEhigh.push_back (I.back ()
                                              + plotted_q[i][j]
                                                    * plotted_I_error[i][j]);
                          }
                      }
        ...
        symbol.setPen (
            QPen (plot_colors[i % plot_colors.size ()],
                  useable_errors ? POINT_SYMBOL_WIDTH : use_line_width,
                  Qt::SolidLine));

        symbol.setStyle (useable_errors ? QwtSymbol::Diamond
                                        : QwtSymbol::NoSymbol);
        plotted_Iq[i]->setStyle (useable_errors ? QwtPlotCurve::NoCurve
                                                : QwtPlotCurve::Lines);
        plotted_Iq[i]->setSymbol (new QwtSymbol (
            ...
            for (int k = 0; k < ebs; ++k)
              {
                ...
                QwtPlotCurve *curve = new QwtPlotCurve (
                    UPU_EB_PREFIX + qsl_plotted_iq_names[i]);
                curve->setStyle (QwtPlotCurve::Lines);
                curve->setPen (QPen (plot_colors[i % plot_colors.size ()],
                                     ERRORBAR_WIDTH, Qt::SolidLine));
...
void
US_Hydrodyn_Saxs::clear_guinier ()
{
  // remove any existing Guinier curves

  for (unsigned int g = 0; g < plotted_Gp_curves.size (); g++)
    {
  ...
  for (unsigned int g = 0; g < plotted_Gp_curves_full.size (); g++)
    {
      if (plotted_guinier_plotted.count (g)
          && plotted_guinier_plotted[g] == true)
        {
...
void
US_Hydrodyn_Saxs::clear_cs_guinier ()
{
  // remove any existing Guinier curves

  for (unsigned int g = 0; g < plotted_cs_Gp_curves.size (); g++)
    {
  ...
  for (unsigned int g = 0; g < plotted_cs_Gp_curves_full.size (); g++)
    {
      if (plotted_cs_guinier_plotted.count (g)
          && plotted_cs_guinier_plotted[g] == true)
        {
...
void
US_Hydrodyn_Saxs::clear_Rt_guinier ()
{
  // remove any existing Guinier curves

  for (unsigned int g = 0; g < plotted_Rt_Gp_curves.size (); g++)
    {
  ...
  for (unsigned int g = 0; g < plotted_Rt_Gp_curves_full.size (); g++)
    {
      if (plotted_Rt_guinier_plotted.count (g)
          && plotted_Rt_guinier_plotted[g] == true)
        {
...
void
US_Hydrodyn_Saxs::adjust_wheel (double pos)
{
  cout << QString ("pos is now %1\n").arg (pos);
}

void
US_Hydrodyn_Saxs::manual_guinier_fit_start_text (const QString &text)
{
...
void
US_Hydrodyn_Saxs::manual_guinier_fit_end_text (const QString &text)
{
  if (!wheel_is_pressed && qwtw_wheel->value () != text.toDouble ())
    {
      qwtw_wheel->setValue (text.toDouble ());
...
void
US_Hydrodyn_Saxs::manual_guinier_fit_start_focus (bool hasFocus)
{
  ...
  if (hasFocus)
    {
      ...
      // qwtw_wheel->setSingleStep( join_offset_delta ); connect( qwtw_wheel,
      // SIGNAL( valueChanged( double ) ), SLOT( adjust_wheel( double ) ) );
...
void
US_Hydrodyn_Saxs::manual_guinier_fit_end_focus (bool hasFocus)
{
...
void
US_Hydrodyn_Saxs::plot_guinier_pts_removed (int i, bool cs, bool Rt)
{
  ...
  if (cs)
    {
      ...
      for (map<double, double>::iterator it
           = plotted_cs_guinier_pts_removed[i].begin ();
           it != plotted_cs_guinier_pts_removed[i].end (); it++)
        {
      ...
      if (Rt)
        {
          ...
          sym.setPen (QPen (Qt::red));
          for (map<double, double>::iterator it
               = plotted_Rt_guinier_pts_removed[i].begin ();
               it != plotted_Rt_guinier_pts_removed[i].end (); it++)
            {
      ...
      else
        {
          ...
          for (map<double, double>::iterator it
               = plotted_guinier_pts_removed[i].begin ();
               it != plotted_guinier_pts_removed[i].end (); it++)
            {

              // cout << QString( "red marker %1 %2 %3\n" ).arg( it->first
              // ).arg( it->second ).arg( exp( it->second ) );
...
void
US_Hydrodyn_Saxs::plot_guinier_error_bars (int i, bool cs, bool Rt)
{
          ...
          if (e)
            {
              ...
              // plotted_q2[ i ][ q ] ).arg( I ).arg( e );
              plotted_guinier_error_bars.push_back (
                  new QwtPlotCurve (qsl_plotted_iq_names[i]));
              plotted_guinier_error_bars.back ()->setStyle (
                  QwtPlotCurve::Lines);
              plotted_guinier_error_bars.back ()->setSamples (
                  (double *)&(x[0]), (double *)&(y[0]), 2);
              plotted_guinier_error_bars.back ()->setPen (
                  QPen (plot_colors[i % plot_colors.size ()], pen_width,
                        Qt::SolidLine));
...
void
US_Hydrodyn_Saxs::clear_guinier_error_bars ()
{
  for (unsigned int g = 0; g < plotted_guinier_error_bars.size (); g++)
    {
...
void
US_Hydrodyn_Saxs::manual_guinier_process ()
{
}
