  ...
  for (int i = 0; i < (int)guinier_names.size(); ++i) {
    ...
    for (int j = 0; j < maxlen; ++j) {
      out[j + 1] << (j < (int)guinier_lnI[i].size()
                         ? QString("%1").arg(guinier_lnI[i][j], 0, 'g', 8)
                         : QString(""));
    }
    ...
    if (guinier_sd[i].size()) {
      ...
      for (int j = 0; j < maxlen; ++j) {
        out[j + 1] << (j < (int)guinier_sd[i].size()
                           ? QString("%1").arg(guinier_sd[i][j], 0, 'g', 8)
                           : QString(""));
      }
      if (guinier_removed[i].size()) {
        ...
        for (int j = 0; j < maxlen; ++j) {
          out[j + 1] << (j < (int)guinier_q2[i].size() &&
                                 guinier_removed[i].count(guinier_q2[i][j])
                             ? "\"X\""
                             : "");
        }
      }
...
void US_Hydrodyn_Saxs::run_guinier_Rt() {
  ...
  if (our_saxs_options->guinier_csv) {
    QString filename = USglobal->config_list.root_dir + SLASH + "somo" + SLASH +
                       "saxs" + SLASH + our_saxs_options->guinier_csv_filename +
                       "_tv.csv";
    ...
    if (!f.open(QIODevice::WriteOnly)) {
      editor_msg("red",
                 QString(us_tr("Can not create file %1\n")).arg(f.fileName()));
    } else {
      QTextStream ts(&f);
      ts << csvlog;
      f.close();
    ...
    if (((US_Hydrodyn *)us_hydrodyn)->gparams.count("guinier_csv_save_data") &&
      ...
      QString filename =
          ...
          SLASH + our_saxs_options->guinier_csv_filename + "_tv_data.csv";
...
void US_Hydrodyn_Saxs::guinier_frame_plot(const QString &title,
      ...
      for (unsigned int i = 0; i < plotted_Iq.size(); i++) {
        if (!skip_curves.count(i)) {
          ...
          if (guinier_scratch.count(key)) {
            ...
            sd.push_back(guinier_scratch.count(key_sd) ? guinier_scratch[key_sd]
                                                       : 0e0);
          }
        }
      }
  ...
  if (plots.size()) {
    US_Hydrodyn_Saxs_Guinier_Frames *uhsgf =
        new US_Hydrodyn_Saxs_Guinier_Frames(us_hydrodyn, parameters, plots);
    uhsgf->show();
  }
}
...
void US_Hydrodyn_Saxs::run_guinier_cs() {
  ...
  if (our_saxs_options->guinier_csv) {
    ...
    if (((US_Hydrodyn *)us_hydrodyn)->gparams.count("guinier_csv_save_data") &&
      ...
      QString filename =
          USglobal->config_list.root_dir + SLASH + "somo" + SLASH + "saxs" +
          SLASH + our_saxs_options->guinier_csv_filename + "_cs_data.csv";
...
bool US_Hydrodyn_Saxs::guinier_analysis(unsigned int i, QString &csvlog) {
      ...
      for (unsigned int j = 0; j < plotted_q[i].size(); j++) {
        if (plotted_q[i][j] >= our_saxs_options->qstart &&
          ...
          if (plotted_I_error[i][j] <= 0e0) {
            editor_msg("dark red",
                       QString(us_tr("Notice: SD weighting of Guinier fit is "
                                     "off for %1 since at least one SD is zero "
                                     "or negative in the selected q range"))
                           .arg(qsl_plotted_iq_names[i]));
            use_SD_weighting = false;
            break;
          }
  ...
  if (use_guinier_outlier_reject && !use_SD_weighting) {
    editor_msg("dark red",
               QString(us_tr("Notice: Turning off outlier rejection for "
                             "Guinier fit of %1 since SD weighting is off"))
                   .arg(qsl_plotted_iq_names[i]));
    ...
    if (((US_Hydrodyn *)us_hydrodyn)->gparams.count("guinier_auto_fit") &&
      ...
      if (!usu.guinier_plot("guinier", "data") ||
          !usu.guinier_fit2(this_log, "guinier", pointsmin, pointsmax,
                            sRgmaxlimit, pointweightpower, p_guinier_maxq, a, b,
      ...
      do {
        ...
        if (pts_decrease) {
          if ((int)usu.wave["data"].q.size() - (int)pts_decrease < 5) {
            internal_error =
                QString(us_tr("Too few points left (%1) after %2 points "
                              "removed for qRgmax limit"))
                    .arg((int)usu.wave["data"].q.size() - (int)pts_decrease)
                    .arg(pts_decrease);
            // editor->append( QString( "Error performing Guinier analysis on
        ...
        //             if ( do_decrease )
        //             {
        //                US_Vector::printvector3( "q I e", usu.wave["data"].q,
        //                usu.wave["data"].r, usu.wave["data"].s );
        //             }

        unsigned int pstart = 0;
        unsigned int pend =
            usu.wave["data"].q.size() ? usu.wave["data"].q.size() - 1 : 0;
        bestend = pend;
        beststart = pstart;
        ...
        if (use_guinier_outlier_reject) {
          ...
          if (!usu.guinier_plot("guinier", "data") ||
              !usu.guinier_fit_with_removal(
                  this_log, "guinier", pstart, pend, a, b, siga, sigb, chi2, Rg,
                  I0,
                  smax,  // don't know why these are flipped
                  smin, sRgmin, sRgmax,
                  our_saxs_options->guinier_outlier_reject_dist, removed,
                  pts_removed, false)) {
            editor->append(QString("Error performing Guinier analysis on %1\n" +
                                   usu.errormsg + "\n")
                               .arg(qsl_plotted_iq_names[i]));
          ...
          if (removed.size()) {
            ...
            for (int j = 0; j < (int)usu.wave["data"].q.size(); ++j) {
              // q2.push_back( usu.wave[ "data" ].q[ j ] * usu.wave[ "data" ].q[
              // j ] );
              if (removed_match.count(QString("%1").arg(
                      usu.wave["data"].q[j] * usu.wave["data"].q[j]))) {
                qs_removed += QString("%1%2")
                                  .arg(qs_removed.isEmpty() ? "" : "_")
                                  .arg(j + 1);
              }
            }
          ...
          plotted_guinier_pts_removed.erase(i);
          if (!usu.guinier_plot("guinier", "data") ||
              !usu.guinier_fit(this_log, "guinier", pstart, pend, a, b, siga,
                               sigb, chi2, Rg, I0,
                               smax,  // don't know why these are flipped
                               smin, sRgmin, sRgmax)) {
            editor->append(QString("Error performing Guinier analysis on %1\n" +
  ...
  if (too_few_points || (bestend - beststart + 1 - pts_removed < pointsmin) ||
    ...
    if (!internal_error.isEmpty()) {
      ...
      if (pts_removed) {
        report = QString(
                     "Guinier analysis of %1:\n"
                     "**** Could not compute Rg, too few data points %2 after "
                     "removal ****\n")
                     .arg(qsl_plotted_iq_names[i])
    ...
    if (us_isnan(Rg) || b >= 0e0) {
      ...
      report = QString("").sprintf(
          ...
          "Rg %.1f (%.1f) (A) I(0) %.2e (%.2e) MW %.2e (%.2e) q^2 [ %.5f:%.5f] "
          "qRgmin %.3f qRgmax %.3f points used %u chi^2 %.2e reduced-chi %.2e",
          qsl_plotted_iq_names[i].toLatin1().data(), Rg, sigb, I0, siga, MW,
          MW_sd, smin * smin, smax * smax, sRgmin, sRgmax,
      ...
      if (!istarq && !ihashq) {
        if (US_Saxs_Util::mwt(
          ...
          report +=
              us_tr(" MW(Vc) could not compute " + messagest + " " + notest);
        }
      }
      ...
      if (started_in_expert_mode && !istarq && !ihashq) {
        if (((US_Hydrodyn *)us_hydrodyn)
                ->saxs_util->mwc(plotted_q[i], plotted_I[i], Rg, sigb, I0, siga,
                                 ((US_Hydrodyn *)us_hydrodyn)
                                     ...
                                     ? ((US_Hydrodyn *)us_hydrodyn)
                                           ...
                                           .toDouble()
                                     : 0e0,
                                 qm, Vcc, Qrc, mwc, mwc_sd, messagesc,
                                 notesc)) {
          report +=
          ...
          report +=
              us_tr(" MW(Vc) could not compute " + messagesc + " " + notesc);
        }
      }
    ...
    for (unsigned int j = 0; j < plotted_q[i].size(); j++) {
      if (plotted_I[i][j] > 0e0) {
        ...
        if (use_SD_weighting) {
          ...
          // plotted_I_error[ i ][ j ] / plotted_I[ i ][ j ] ) : 0e0 ); fabs(
          // lnI.back() ) * ( plotted_I_error[ i ][ j ] / plotted_I[ i ][ j ] )
          // : 0e0 );
        }
      }
    }
  ...
  if (!too_few_points) {
    if (((US_Hydrodyn *)us_hydrodyn)->gparams.count("guinier_auto_fit") &&
      ...
      if (!usu.guinier_plot("cs_guinier", "data") ||
          !usu.guinier_fit2(this_log, "cs_guinier", pointsmin, pointsmax,
                            sRgmaxlimit, pointweightpower, p_guinier_maxq, a, b,
                            siga, sigb, chi2, Rg, I0,
                            smax,  // don't know why these are flipped
      ...
      do {
          ...
          if ((int)usu.wave["data"].q.size() - (int)pts_decrease < 5) {
            internal_error =
                QString(us_tr("Too few points left (%1) after %2 points "
                    ...
                    .arg((int)usu.wave["data"].q.size() - (int)pts_decrease)
                    .arg(pts_decrease);
            // editor->append( QString( "Error performing CS Guinier analysis on
            // %1: %2\n" ).arg( qsl_plotted_iq_names[ i ] ).arg( internal_error
            // ) );
            break;
          }
        ...
        // US_Vector::printvector3( "cs2 q2 I e", q2 /* usu.wave["data"].q */,
        // usu.wave["data"].r, usu.wave["data"].s );
  ...
  if (too_few_points || (bestend - beststart + 1 - pts_removed < pointsmin) ||
    ...
    if (!internal_error.isEmpty()) {
      ...
      if (pts_removed) {
        report = QString(
                     "CS Guinier analysis of %1:\n"
                     "**** Could not compute Rc, too few data points %2 after "
                     "removal ****\n")
                     .arg(qsl_plotted_iq_names[i])
                     .arg(bestend - beststart + 1 - pts_removed);
        csvlog += QString(
    ...
    if (us_isnan(Rg) || b >= 0e0) {
      ...
      report = QString("").sprintf(
          ...
          "Rc %.1f (%.1f) (A) I(0) %.2e (%.2e) M/L %.2e (%.2e) q^2 [%.5f:%.5f] "
          "qRcmin %.3f qRcmax %.3f points used %u chi^2 %.2e reduced-chi "
          "%.2e\n",
          qsl_plotted_iq_names[i].toLatin1().data(), Rg, sigb, I0, siga, ML,
          ML_sd, smin * smin, smax * smax, sRgmin, sRgmax,
          bestend - beststart + 1 - pts_removed, chi2,
          sqrt(chi2 / (bestend - beststart - 1 - pts_removed)));
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count("guinier_csv_save_data") &&
    ...
    for (unsigned int j = 0; j < plotted_q[i].size(); j++) {
      if (plotted_I[i][j] > 0e0) {
        ...
        if (use_SD_weighting) {
          sd.push_back(fabs(plotted_I_error[i][j] / plotted_I[i][j]));
          // ( lnI.back() / log( plotted_q[ i ][ j ] * plotted_I[ i ][ j ] ) ) *
          // fabs( plotted_I_error[ i ][ j ] / plotted_I[ i ][ j ] ) : 0e0 );
          // fabs( lnI.back() ) * ( plotted_I_error[ i ][ j ] / plotted_I[ i ][
          // j ] ) : 0e0 );
        }
      }
    }
...
bool US_Hydrodyn_Saxs::Rt_guinier_analysis(unsigned int i, QString &csvlog) {
  ...
  double sRgmaxlimit =
      ((US_Hydrodyn *)us_hydrodyn)->gparams.count("guinier_qRtmax")
          ? ((US_Hydrodyn *)us_hydrodyn)->gparams["guinier_qRtmax"].toDouble()
          : 1e0;
    ...
    if (((US_Hydrodyn *)us_hydrodyn)->gparams.count("guinier_auto_fit") &&
      ...
      if (!usu.guinier_plot("tv_guinier", "data") ||
          !usu.guinier_fit2(
              ...
              pointweightpower, p_guinier_maxq, a, b, siga, sigb, chi2, Rg, I0,
              smax,  // don't know why these are flipped
          ...
          if ((int)usu.wave["data"].q.size() - (int)pts_decrease < 5) {
            internal_error =
                QString(us_tr("Too few points left (%1) after %2 points "
                    ...
                    .arg((int)usu.wave["data"].q.size() - (int)pts_decrease)
                    .arg(pts_decrease);
            // editor->append( QString( "Error performing TV Guinier analysis on
            // %1: %2\n" ).arg( qsl_plotted_iq_names[ i ] ).arg( internal_error
            // ) );
            break;
          }
  ...
  if (too_few_points || (bestend - beststart + 1 - pts_removed < pointsmin) ||
    ...
    if (!internal_error.isEmpty()) {
      ...
      if (pts_removed) {
        report = QString(
                     "TV Guinier analysis of %1:\n"
                     "**** Could not compute Rt, too few data points %2 after "
                     "removal ****\n")
                     .arg(qsl_plotted_iq_names[i])
                     .arg(bestend - beststart + 1 - pts_removed);
        csvlog += QString(
    ...
    if (us_isnan(Rg) || b >= 0e0) {
      ...
      report = QString("").sprintf(
          ...
          "Rt %.1f (%.1f) (A) I(0) %.2e (%.2e) M/A %.2e (%.2e) q^2 [%.5f:%.5f] "
          "qRcmin %.3f qRcmax %.3f points used %u chi^2 %.2e reduced-chi "
          "%.2e\n",
          qsl_plotted_iq_names[i].toLatin1().data(), Rg, sigb, I0, siga, MA,
          MA_sd, smin * smin, smax * smax, sRgmin, sRgmax,
          bestend - beststart + 1 - pts_removed, chi2,
          sqrt(chi2 / (bestend - beststart - 1 - pts_removed)));
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams.count("guinier_csv_save_data") &&
    ...
    for (unsigned int j = 0; j < plotted_q[i].size(); j++) {
      if (plotted_I[i][j] > 0e0) {
        ...
        if (use_SD_weighting) {
          sd.push_back(fabs(plotted_I_error[i][j] / plotted_I[i][j]));
          // ( lnI.back() / log( plotted_q[ i ][ j ] * plotted_q[ i ][ j ] *
          // plotted_I[ i ][ j ] ) ) * fabs( plotted_I_error[ i ][ j ] /
          // plotted_I[ i ][ j ] ) : 0e0 );
          // fabs( lnI.back() ) * ( plotted_I_error[ i ][ j ] / plotted_I[ i ][
          // j ] ) : 0e0 );
        }
...
void US_Hydrodyn_Saxs::set_guinier() {
    ...
    plot_saxs->setTitle(
        (cb_guinier->isChecked()
             ? (cb_cs_guinier->isChecked()
                    ? "CS Guinier "
                    : (cb_Rt_guinier->isChecked() ? "TV Guinier " : "Guinier "))
             : "") +
        us_tr("SANS Curve"));
  } else {
    plot_saxs->setTitle(
  ...
  // clear the plot and replot the curves in correct mode ( kratky or guniner or
  // cs-guinier )
  ...
  for (unsigned int p = 0; p < plotted_Iq.size(); p++) {
    ...
    plotted_Iq[p]->setSamples(
        ...
        cb_kratky->isChecked() ? (double *)&(q2I[0])
            ...
            (cb_guinier->isChecked() &&
                     (cb_cs_guinier->isChecked() || cb_Rt_guinier->isChecked())
                 ? (double *)&(pI[0])
                 : (double *)&(plotted_I[p][0])),
        q_points);
    plotted_Iq[p]->setPen(
  ...
  if (cb_guinier->isChecked()) {
    if (do_errorbars) {
      ...
      if (total_points > 20000) {
        editor_msg(
            "dark red",
            "Notice: error bars are turned off due to too many curve-points");
        do_errorbars = false;
      }
    }
  ...
  for (int i = 0; i < niqsize; i++) {
    if (cb_guinier->isChecked()) {
      if (cb_cs_guinier->isChecked()) {
        ...
        if (plotted_cs_guinier_valid.count(i) &&
          ...
          if (plotted_q2[i].size()) {
            ...
            y[0] =
                exp(plotted_cs_guinier_a[i] + plotted_cs_guinier_b[i] * x[0]);
            y[1] =
                exp(plotted_cs_guinier_a[i] + plotted_cs_guinier_b[i] * x[1]);
            ...
            plotted_cs_Gp_curves_full[i]->setSamples((double *)&(x[0]),
                                                     (double *)&(y[0]), 2);
        ...
        if (cb_Rt_guinier->isChecked()) {
          ...
          if (plotted_Rt_guinier_valid.count(i) &&
            ...
            if (plotted_q2[i].size()) {
              ...
              x[1] = plotted_q2[i].back();
              y[0] =
                  exp(plotted_Rt_guinier_a[i] + plotted_Rt_guinier_b[i] * x[0]);
              y[1] =
                  exp(plotted_Rt_guinier_a[i] + plotted_Rt_guinier_b[i] * x[1]);
              plotted_Rt_Gp_curves_full[i] = new QwtPlotCurve(
                  QString(QString("CS Gn. %1").arg(qsl_plotted_iq_names[i])));
              plotted_Rt_Gp_curves_full[i]->setStyle(QwtPlotCurve::Lines);
              plotted_Rt_Gp_curves_full[i]->setSamples((double *)&(x[0]),
                                                       (double *)&(y[0]), 2);
              plotted_Rt_Gp_curves_full[i]->setPen(
                  QPen(QColor("dark red"), pen_width, Qt::DotLine));
          ...
          if (plotted_guinier_valid.count(i) &&
            ...
            if (plotted_q2[i].size()) {
              ...
              plotted_Gp_curves_full[i]->setSamples((double *)&(x[0]),
                                                    (double *)&(y[0]), 2);
              plotted_Gp_curves_full[i]->setPen(
                  QPen(QColor("dark red"), pen_width, Qt::DotLine));
      ...
      // plotted_Iq[ i ]->setStyle( QwtPlotCurve::Lines );
      plotted_Iq[i]->setPen(
          QPen(plot_colors[i % plot_colors.size()], pen_width, Qt::SolidLine));
    }
...
void US_Hydrodyn_Saxs::set_guinier_eb() {
  ...
  for (int i = 0; i < niqsize; i++) {
    bool useable_errors =
        ...
        !std::all_of(plotted_I_error[i].begin(), plotted_I_error[i].end(),
                     [](double i) { return i == 0; });
...

    {
        ...
        if (cb_kratky->isChecked()) {
          ...
          for (unsigned int j = 0; j < q_points; ++j) {
            ...
            pElow.push_back(I.back() -
                            plotted_q2[i][j] * plotted_I_error[i][j]);
      ...
      symbol.setPen(QPen(plot_colors[i % plot_colors.size()],
      ...
      plotted_Iq[i]->setStyle(useable_errors ? QwtPlotCurve::NoCurve
                                             : QwtPlotCurve::Lines);
      plotted_Iq[i]->setSymbol(new QwtSymbol(symbol.style(), symbol.brush(),
                                             symbol.pen(), symbol.size()));
      plotted_Iq[i]->setSamples(
          cb_guinier->isChecked() ? (double *)&(q2[0]) : (double *)&(q[0]),
          (double *)&(I[0]), q.size());
      if (use_error) {
        ...
        for (int k = 0; k < ebs; ++k) {
          ...
          y[1] = pEhigh[k];
          QwtPlotCurve *curve =
              new QwtPlotCurve(UPU_EB_PREFIX + qsl_plotted_iq_names[i]);
          curve->setStyle(QwtPlotCurve::Lines);
  ...
  if (cs) {
    for (int q = 0; q < (int)plotted_q[i].size(); ++q) {
      ...
      if (e) {
        ...
        plotted_guinier_error_bars.back()->setSamples((double *)&(x[0]),
                                                      (double *)&(y[0]), 2);
        plotted_guinier_error_bars.back()->setPen(QPen(
            plot_colors[i % plot_colors.size()], pen_width, Qt::SolidLine));
