...
bool
US_Saxs_Util::bspline_prepare (unsigned int knots, unsigned int degree)
{
  ...
  if (knots - degree - 1 <= 0)
    {
      errormsg = "Error: prepare bspline: knots - degree - 1 must be greater "
                 "than zero";
      return false;
    }
  ...
  if (sgp_exp_q.size () < knots || sgp_exp_q.size () < degree)
    {
      errormsg = "Error: prepare bspline: q grid must be bigger than number "
                 "of knots or N";
      return false;
    }
...
bool
US_Saxs_Util::bspline_basis (unsigned int basis, double q, double &value)
{
  if (basis > bspline_basis_functions)
    {
      errormsg = QString ("Error: bspline_basis: requested basis function %1 "
                          "but only %2 defined")
                     .arg (basis)
                     .arg (bspline_basis_functions);
      return false;
      ...
      if (diff > 0e0)
        {
          ...
          value += pow (diff, (int)bspline_degree)
                   * bspline_omega[basis][i - basis];
          // check omega:
          double omega = 1e0;
          ...
          if (fabs (1e0 / omega - bspline_omega[basis][i - basis]) > 1e-5)
            {
              cout << QString (
                          "stored omega different basis %1 pos %2 %3 %4\n")
                          .arg (basis)
                          .arg (i)
                          .arg (1e0 / omega)
