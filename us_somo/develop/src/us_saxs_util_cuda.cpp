...
bool
US_Saxs_Util::iqq_cuda (vector<double> &q, vector<saxs_atom> &atoms,
                        ...
                        unsigned int threads_per_block)
{
  ...
  if (our_saxs_options.subtract_radius)
    {
      errormsg
          = "cuda iqq does not currently support radius subtraction option";
      return false;
    }
  ...
  for (unsigned int i = 0; i < fp.size (); ++i) // should be q.size()
    {
      for (unsigned int j = 0; j < fp[i].size ();
           ++j) // should be atoms.size()
        {
...
#else
bool
US_Saxs_Util::iqq_cuda (vector<double> &, vector<saxs_atom> &,
