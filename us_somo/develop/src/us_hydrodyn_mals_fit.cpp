                                                     ...
                                                     ) : QDialog( p )
{
   ...
   if ( mals_win->conc_files.count( mals_win->wheel_file ) )
   {
      ...
      if ( gaussian_type != US_Hydrodyn_Mals::EMGGMG )
      {
         ...
         for ( int i = gaussian_type_size; i < (int) mals_win->gaussians.size(); i += gaussian_type_size )
         {
            conc_ratios.push_back( mals_win->gaussians[ 2 + i ] / mals_win->gaussians[ 2 ] );
         }
...
void US_Hydrodyn_Mals_Fit::redo_settings()
{
   ...
   if ( use_errors &&
        ( !mals_win->f_errors.count( mals_win->wheel_file ) ||
          mals_win->f_errors[ mals_win->wheel_file ].size() != mals_win->f_qs[ mals_win->wheel_file ].size() ||
          !mals_win->is_nonzero_vector ( mals_win->f_errors[ mals_win->wheel_file ] ) ) )
   {
      use_errors = false;
      ...
      mals_win->editor_msg( "dark red",  QString( us_tr( "SD weighting requested, but the errors associated "
                                                      "with the selected file %1 are not all non-zero. " 
                                                      "SD weighting turned off for this file." ) ).arg( mals_win->wheel_file ) );
   }
   if ( use_errors )
   {
...
void US_Hydrodyn_Mals_Fit::cancel()
{
   close();
}
...
void US_Hydrodyn_Mals_Fit::update_common()
{
   ...
   {
      if ( !MFIT::param_fixed[ i ] ||
           MFIT::comm_backref.count( i ) )
      {
         mals_win->gaussians[ i ] = MFIT::init_params[ 
                                                      MFIT::comm_backref.count( i ) ?
                                                      MFIT::comm_backref[ i ] :
                                                      MFIT::param_pos[ i ] 
                                                       ];
...
bool US_Hydrodyn_Mals_Fit::setup_run()
{
   ...
   for ( unsigned int i = 0; i < ( unsigned int ) mals_win->gaussians.size(); i+= gaussian_type_size )
   {
           ...
           fixed_curves.count( pos + 1 ) )
      {
         ...
         if ( MFIT::init_params.back() < min ) {
            // us_qdebug( QString( "min set amp init_params.size() %1 val %2" ).arg( MFIT::init_params.size() ).arg( MFIT::init_params.back() ) );
...
bool US_Hydrodyn_Mals_Fit::lock_zeros( vector < double > & par )
{
   ...
   for ( unsigned int i = 0; i < ( unsigned int ) tmp_gaussians.size(); i+= gaussian_type_size )
   {
      ...
      if ( !cb_fix_curves[ pos ]->isChecked() ) {
         ...
         if ( !cb_fix_width->isChecked() ) {
            if ( tmp_gaussians[ 2 + i ] <= mals_ampl_width_min * mals_lock_min_retry_mult ) {
               to_lock.insert( pos + 1 );
               continue;
            }
         }
...
void US_Hydrodyn_Mals_Fit::lm( bool max_free_peak_delta_run, double prev_rmsd )
{
   ...
   // US_Vector::printvector( QString( "lm %1 I" ).arg( mals_win->wheel_file ), mals_win->f_Is[ mals_win->wheel_file ] );
   // US_Vector::printvector( QString( "lm %1 e" ).arg( mals_win->wheel_file ), mals_win->f_errors[ mals_win->wheel_file ] );
   ...
   if ( use_errors ) 
   {
      ...
      {
         yp[ j ]  = (*MFIT::compute_gaussian_f)( t[ j ], (double *)(&MFIT::base_params[ 0 ] ) );
...
void US_Hydrodyn_Mals_Fit::gsm_sd()
{
   ...
   {
      gsm_yp[ j ]  = (*MFIT::compute_gaussian_f)( gsm_t[ j ], (double *)(&MFIT::base_params[ 0 ]) );
...
void US_Hydrodyn_Mals_Fit::stop()
{
   running = false;
}
...
void US_Hydrodyn_Mals_Fit::grid()
{
   ...
   unsigned int total_count = ( unsigned int ) pow( (double) pop_size, (int) MFIT::init_params.size() );
