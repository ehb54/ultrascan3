...
US_Hydrodyn_Mals_Fit::US_Hydrodyn_Mals_Fit(US_Hydrodyn_Mals *mals_win,
  ...
  if (mals_win->conc_files.count(mals_win->wheel_file)) {
    ...
    if (gaussian_type != US_Hydrodyn_Mals::EMGGMG) {
      ...
      for (int i = gaussian_type_size; i < (int)mals_win->gaussians.size();
...
void US_Hydrodyn_Mals_Fit::redo_settings() {
  use_errors = mals_win->cb_sd_weight->isChecked();
  if (use_errors && (!mals_win->f_errors.count(mals_win->wheel_file) ||
    ...
    //                                 us_tr( "SD weighting requested, but the
    //                                 errors associated\n"
    //                                     "with the selected file are not all
    //                                     non-zero.\n" "SD weighting turned
    //                                     off" ) );
...
void US_Hydrodyn_Mals_Fit::restore() {
...
void US_Hydrodyn_Mals_Fit::undo() {
...
void US_Hydrodyn_Mals_Fit::setupGUI() {
  int minHeight1 = 30;
...
void US_Hydrodyn_Mals_Fit::cancel() { close(); }

void US_Hydrodyn_Mals_Fit::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/somo_mals_fit.html");
}

void US_Hydrodyn_Mals_Fit::closeEvent(QCloseEvent *e) {
  global_Xpos -= 3;
  global_Ypos -= 30;
  e->accept();
}

void US_Hydrodyn_Mals_Fit::update_common() {
  if (!setup_run()) {
    update_enables();
    return;
  }
  ...
  for (unsigned int i = 0; i < MFIT::param_fixed.size(); i++) {
    if (!MFIT::param_fixed[i] || MFIT::comm_backref.count(i)) {
      mals_win->gaussians[i] =
          MFIT::init_params[MFIT::comm_backref.count(i) ? MFIT::comm_backref[i]
                                                        : MFIT::param_pos[i]];
...
void US_Hydrodyn_Mals_Fit::update_enables() {
...
double compute_gaussian_f_GAUSS(double t, const double *par) {
  ...
  for (unsigned int i = 0; i < (unsigned int)param_fixed.size();) {
    ...
    if (param_fixed[i]) {
      ...
      if (width < param_min[param_pos[i]] || width > param_max[param_pos[i]]) {
        return 1e99;
      }
    }
...
double compute_gaussian_f_EMG(double t, const double *par) {
  ...
  for (unsigned int i = 0; i < (unsigned int)param_fixed.size();) {
    ...
    if (param_fixed[i]) {
      ...
      if (dist1 < param_min[param_pos[i]] || dist1 > param_max[param_pos[i]]) {
        return 1e99;
      }
    }
...
double compute_gaussian_f_EMGGMG(double t, const double *par) {
  ...
  for (unsigned int i = 0; i < (unsigned int)param_fixed.size();) {
    ...
    if (param_fixed[i]) {
      ...
      if (dist2 < param_min[param_pos[i]] || dist2 > param_max[param_pos[i]]) {
        return 1e99;
      }
    }
...
bool US_Hydrodyn_Mals_Fit::setup_run() {
  // us_qdebug( "setup_run()" );
  ...
  for (unsigned int i = 0; i < (unsigned int)mals_win->gaussians.size();
    ...
    if (cb_fix_amplitude->isChecked() || fixed_curves.count(pos + 1)) {
      ...
      // us_qdebug( QString( "in setup_run, pos %1 base_val is %2" ).arg( pos
      // ).arg( base_val ) );
      ...
      if (MFIT::init_params.back() < min) {
        // us_qdebug( QString( "min set amp init_params.size() %1 val %2" ).arg(
        // MFIT::init_params.size() ).arg( MFIT::init_params.back() ) );
    ...
    if (cb_fix_width->isChecked() || fixed_curves.count(pos + 1) ||
        (cb_conc_test->isChecked() &&
         i)  // for the conc_test, the widths of gaussians 1 will be fixed
             // ratios of the 1st param
    ) {
      MFIT::param_pos.push_back(MFIT::fixed_params.size());
      if (i && cb_conc_test->isChecked() && !fixed_curves.count(pos + 1) &&
        ...
        MFIT::comm_backref[MFIT::param_fixed.size()] = conc_width_var_pos;
        MFIT::conc_ratios_map[MFIT::param_fixed.size()] = conc_ratios[pos - 1];
      } else {
    ...
    if (dist1_active) {
      if (cb_fix_dist1->isChecked() || fixed_curves.count(pos + 1) ||
        ...
        if (i && cb_comm_dist1->isChecked() && !fixed_curves.count(pos + 1) &&
          ...
          MFIT::comm_backref[MFIT::param_fixed.size()] = comm_dist_1_var_pos;
        } else {
      ...
      if (dist2_active) {
        if (cb_fix_dist2->isChecked() || fixed_curves.count(pos + 1) ||
          ...
          if (i && cb_comm_dist2->isChecked() && !fixed_curves.count(pos + 1) &&
              !fixed_curves.count(1)) {
            MFIT::fixed_params.push_back(mals_win->gaussians[4 + 0]);
            MFIT::comm_backref[MFIT::param_fixed.size()] = comm_dist_2_var_pos;
          } else {
...
bool US_Hydrodyn_Mals_Fit::lock_zeros(vector<double> &par) {
  ...
  for (unsigned int i = 0; i < MFIT::param_fixed.size(); i++) {
    if (!MFIT::param_fixed[i] || MFIT::comm_backref.count(i)) {
      tmp_gaussians[i] = par[MFIT::comm_backref.count(i) ? MFIT::comm_backref[i]
                                                         : MFIT::param_pos[i]];
      if (MFIT::conc_ratios_map.count(i)) {
...
bool US_Hydrodyn_Mals_Fit::max_free_peak_delta(vector<double> &par) {
...
void US_Hydrodyn_Mals_Fit::lm(bool max_free_peak_delta_run, double prev_rmsd) {
  update_common();
  setup_run();
  // puts( "lm" );
  ...
  // US_Vector::printvector( QString( "lm %1 q" ).arg( mals_win->wheel_file ), x
  // ); US_Vector::printvector( QString( "lm %1 I" ).arg( mals_win->wheel_file
  // ), mals_win->f_Is[ mals_win->wheel_file ] ); US_Vector::printvector(
  // QString( "lm %1 e" ).arg( mals_win->wheel_file ), mals_win->f_errors[
  // mals_win->wheel_file ] );
  ...
  if (use_errors) {
    ...
    for (unsigned int j = 0; j < t.size(); j++) {
      yp[j] =
          (*MFIT::compute_gaussian_f)(t[j], (double *)(&MFIT::base_params[0]));
  ...
  if (use_errors) {
    ...
    new_fnorm /= nu;
    // us_qdebug( QString( "recomputing fnorm for sd's ... should match display
    // field %1" ).arg( new_fnorm ) ); org_rmsd = sqrt( org_rmsd );
...
#if defined(UHHSF_FULL_DEBUG)
  US_Vector::printvector(QString("par after fit (norm %1)").arg(status.fnorm),
                         par);
#endif
...
void US_Hydrodyn_Mals_Fit::gsm_sd() {
  puts("gsm_sd");
  update_common();
  ...
  for (unsigned int j = 0; j < gsm_t.size(); j++) {
    gsm_yp[j] = (*MFIT::compute_gaussian_f)(gsm_t[j],
  ...
  min_gsm_5_1(v, le_epsilon->text().toDouble(), le_iterations->text().toLong());
...
void US_Hydrodyn_Mals_Fit::gsm_ih() {
  puts("gsm_ih");
  update_common();
...
void US_Hydrodyn_Mals_Fit::gsm_cg() {
  puts("gsm_cg");
  update_common();
...
void US_Hydrodyn_Mals_Fit::stop() { running = false; }

void US_Hydrodyn_Mals_Fit::grid() {
  setup_run();
  update_common();
  puts("grid");
  cout << "gauss fit start\n";
