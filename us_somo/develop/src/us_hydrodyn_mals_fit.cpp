...
US_Hydrodyn_Mals_Fit::US_Hydrodyn_Mals_Fit (US_Hydrodyn_Mals *mals_win,
                                            bool set_comm_dist, QWidget *p,
                                            const char *)
    : QDialog (p)
{
  // us_qdebug( "hf0" );
  ...
  setWindowTitle (us_tr ("US-SOMO: MALS: Gaussian Fit"));
  ...
  gaussians_undo.push_back (mals_win->gaussians);
  ...
  if (mals_win->conc_files.count (mals_win->wheel_file))
    {
      cb_pct_center->setChecked (dist1_active);
      cb_pct_center_from_init->setChecked (true);
      le_pct_center->setText ("2");
      if (gaussian_type != US_Hydrodyn_Mals::EMGGMG)
        {
          cb_conc_test->hide ();
        }
      else
        {
          for (int i = gaussian_type_size;
               i < (int)mals_win->gaussians.size (); i += gaussian_type_size)
            {
              conc_ratios.push_back (mals_win->gaussians[2 + i]
                                     / mals_win->gaussians[2]);
            }
          // us_qdebug( US_Vector::qs_vector( "gaussians", mals_win->gaussians
...
US_Hydrodyn_Mals_Fit::~US_Hydrodyn_Mals_Fit () {}

void
US_Hydrodyn_Mals_Fit::redo_settings ()
{
  use_errors = mals_win->cb_sd_weight->isChecked ();
  if (use_errors
      && (!mals_win->f_errors.count (mals_win->wheel_file)
          || mals_win->f_errors[mals_win->wheel_file].size ()
                 != mals_win->f_qs[mals_win->wheel_file].size ()
          || !mals_win->is_nonzero_vector (
              mals_win->f_errors[mals_win->wheel_file])))
    {
      use_errors = false;

      //       QMessageBox::information( this, this->windowTitle(),
      //                                 us_tr( "SD weighting requested, but
      //                                 the errors associated\n"
      //                                     "with the selected file are not
      //                                     all non-zero.\n" "SD weighting
      //                                     turned off" ) );

      mals_win->editor_msg (
          "dark red",
          QString (us_tr ("SD weighting requested, but the errors associated "
                          "with the selected file %1 are not all non-zero. "
                          "SD weighting turned off for this file."))
              .arg (mals_win->wheel_file));
    }
  if (use_errors)
    {
...
void
US_Hydrodyn_Mals_Fit::restore ()
{
  gaussians_undo.resize (1);
  mals_win->gaussians = gaussians_undo[0];
  if (update_mals)
    {
      mals_win->gauss_init_markers ();
      mals_win->gauss_init_gaussians ();
...
US_Hydrodyn_Mals_Fit::undo ()
{
  ...
  mals_win->gaussians = gaussians_undo.back ();
  if (update_mals)
    {
      mals_win->gauss_init_markers ();
      mals_win->gauss_init_gaussians ();
...
void
US_Hydrodyn_Mals_Fit::setupGUI ()
{
  int minHeight1 = 30;

  lbl_title = new QLabel (us_tr ("US-SOMO: MALS: Gaussian Fit"), this);
  ...
  this ); le_fix_curves = new mQLineEdit( this ); le_fix_curves->setObjectName(
  "le_fix_curves Line Edit" ); le_fix_curves->setText( "" );
  ...
  */
  ...
  cb_conc_test->setFont (QFont (USglobal->config_list.fontFamily,
                                USglobal->config_list.fontSize));
...
void
US_Hydrodyn_Mals_Fit::cancel ()
{
  close ();
}

void
US_Hydrodyn_Mals_Fit::help ()
{
  US_Help *online_help;
  online_help = new US_Help (this);
  online_help->show_help ("manual/somo/somo_mals_fit.html");
}

void
US_Hydrodyn_Mals_Fit::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 3;
  global_Ypos -= 30;
  e->accept ();
...
void
US_Hydrodyn_Mals_Fit::update_common ()
{
  if (!setup_run ())
    {
      update_enables ();
  ...
  vector<double> save_gaussians = mals_win->gaussians;
  for (unsigned int i = 0; i < MFIT::param_fixed.size (); i++)
    {
      if (!MFIT::param_fixed[i] || MFIT::comm_backref.count (i))
        {
          mals_win->gaussians[i]
              = MFIT::init_params[MFIT::comm_backref.count (i)
                                      ? MFIT::comm_backref[i]
                                      : MFIT::param_pos[i]];
          if (MFIT::conc_ratios_map.count (i))
            {
              mals_win->gaussians[i] *= MFIT::conc_ratios_map[i];
            }
        }
    }
  if (update_mals && save_gaussians != mals_win->gaussians)
    {
      gaussians_undo.push_back (mals_win->gaussians);
      mals_win->gauss_init_markers ();
      mals_win->gauss_init_gaussians ();
...
void
US_Hydrodyn_Mals_Fit::update_enables ()
{
  ...
  cb_pct_width->setEnabled (!cb_conc_test->isChecked () && !running
                            && !cb_fix_width->isChecked ());
  le_pct_width->setEnabled (!cb_conc_test->isChecked () && !running
                            && !cb_fix_width->isChecked ()
                            && cb_pct_width->isChecked ());
  cb_pct_width_from_init->setEnabled (!cb_conc_test->isChecked () && !running
                                      && !cb_fix_width->isChecked ());
...
list_params ()
{
  ...
  for (map<unsigned int, unsigned int>::iterator it = comm_backref.begin ();
    ...
    {
      us_qdebug (
          QString ("backref pos %1 to %2\n").arg (it->first).arg (it->second));
    }
}
};

bool
US_Hydrodyn_Mals_Fit::setup_run ()
{
  // us_qdebug( "setup_run()" );
  MFIT::init_params.clear ();
  MFIT::base_params.clear ();
  MFIT::fixed_params.clear ();
  MFIT::comm_backref.clear ();
  MFIT::param_pos.clear ();
  MFIT::param_fixed.clear ();
  MFIT::param_min.clear ();
  MFIT::param_max.clear ();

  map<unsigned int, bool> fixed_curves;

  MFIT::conc_test = false;
  MFIT::conc_ratios_map.clear ();

  switch (gaussian_type)
    {
    case US_Hydrodyn_Mals::EMGGMG:
      MFIT::compute_gaussian_f = &MFIT::compute_gaussian_f_EMGGMG;
      gsm_f = &gsm_f_EMGGMG;
      MFIT::conc_test = cb_conc_test->isChecked ();
      break;
    case US_Hydrodyn_Mals::EMG:
      MFIT::compute_gaussian_f = &MFIT::compute_gaussian_f_EMG;
      gsm_f = &gsm_f_EMG;
      break;
    case US_Hydrodyn_Mals::GMG:
      MFIT::compute_gaussian_f = &MFIT::compute_gaussian_f_GMG;
      gsm_f = &gsm_f_GMG;
      break;
    case US_Hydrodyn_Mals::GAUSS:
      MFIT::compute_gaussian_f = &MFIT::compute_gaussian_f_GAUSS;
      gsm_f = &gsm_f_GAUSS;
      break;
    }

  MFIT::use_errors = use_errors;
  ...
  for (unsigned int i = 0; i < (unsigned int)mals_win->gaussians.size ();
       i += gaussian_type_size)
    {
      unsigned int pos = i / gaussian_type_size;

      if (cb_fix_amplitude->isChecked () || fixed_curves.count (pos + 1))
        {
          MFIT::param_pos.push_back (MFIT::fixed_params.size ());
          MFIT::fixed_params.push_back (mals_win->gaussians[0 + i]);
          MFIT::param_fixed.push_back (true);
        }
      else
        {
          MFIT::param_pos.push_back (MFIT::init_params.size ());
          ...
          if (cb_pct_amplitude_from_init->isChecked ())
            ...
            {
              base_val = mals_win->gaussians[0 + i];
            }

          // us_qdebug( QString( "in setup_run, pos %1 base_val is %2" ).arg(
          // pos ).arg( base_val ) );

          MFIT::init_params.push_back (base_val);
          MFIT::base_params.push_back (mals_win->gaussians[0 + i]);
          ...
          if (MFIT::init_params.back () < min)
            {
              // us_qdebug( QString( "min set amp init_params.size() %1 val %2"
              // ).arg( MFIT::init_params.size() ).arg(
              // MFIT::init_params.back() ) );
              MFIT::init_params.back () = min;
            }
          if (MFIT::init_params.back () > max)
            {
              MFIT::init_params.back () = max;
            }

          MFIT::param_min.push_back (min);
          MFIT::param_max.push_back (max);
        }

      if (cb_fix_center->isChecked () || fixed_curves.count (pos + 1))
        {
          MFIT::param_pos.push_back (MFIT::fixed_params.size ());
          MFIT::fixed_params.push_back (mals_win->gaussians[1 + i]);
          MFIT::param_fixed.push_back (true);
        }
      else
        {
          ...
          if (cb_pct_center_from_init->isChecked ())
            ...
            {
              base_val = mals_win->gaussians[1 + i];
            }

          MFIT::init_params.push_back (base_val);
          MFIT::base_params.push_back (mals_win->gaussians[1 + i]);
          MFIT::param_fixed.push_back (false);
      ...
      if (cb_fix_width->isChecked () || fixed_curves.count (pos + 1)
          || (cb_conc_test->isChecked ()
              && i) // for the conc_test, the widths of gaussians 1 will be
                    // fixed ratios of the 1st param
      )
        {
          MFIT::param_pos.push_back (MFIT::fixed_params.size ());
          if (i && cb_conc_test->isChecked () && !fixed_curves.count (pos + 1)
              && !fixed_curves.count (1))
            {
              MFIT::fixed_params.push_back (mals_win->gaussians[2 + 0]);
              MFIT::comm_backref[MFIT::param_fixed.size ()]
                  = conc_width_var_pos;
              MFIT::conc_ratios_map[MFIT::param_fixed.size ()]
                  = conc_ratios[pos - 1];
            }
          else
            {
              MFIT::fixed_params.push_back (mals_win->gaussians[2 + i]);
            }
          MFIT::param_fixed.push_back (true);
        }
      else
        {
          conc_width_var_pos = MFIT::init_params.size ();
          MFIT::param_pos.push_back (MFIT::init_params.size ());
          ...
          if (cb_pct_width_from_init->isChecked ())
            ...
            {
              base_val = mals_win->gaussians[2 + i];
            }

          MFIT::init_params.push_back (base_val);
          MFIT::base_params.push_back (mals_win->gaussians[2 + i]);
          MFIT::param_fixed.push_back (false);
      ...
      if (dist1_active)
        {
          if (cb_fix_dist1->isChecked () || fixed_curves.count (pos + 1)
              || (cb_comm_dist1->isChecked () && i))
            {
              MFIT::param_pos.push_back (MFIT::fixed_params.size ());
              if (i && cb_comm_dist1->isChecked ()
                  && !fixed_curves.count (pos + 1) && !fixed_curves.count (1))
                {
                  MFIT::fixed_params.push_back (mals_win->gaussians[3 + 0]);
                  MFIT::comm_backref[MFIT::param_fixed.size ()]
                      = comm_dist_1_var_pos;
                }
              else
                {
                  MFIT::fixed_params.push_back (mals_win->gaussians[3 + i]);
                }
              MFIT::param_fixed.push_back (true);
            }
          else
            {
              if (cb_comm_dist1->isChecked ())
                {
                  comm_dist_1_var_pos = MFIT::init_params.size ();
                }
              MFIT::param_pos.push_back (MFIT::init_params.size ());
              ...
              if (cb_pct_dist1_from_init->isChecked ())
                ...
                {
                  base_val = mals_win->gaussians[3 + i];
                }

              MFIT::init_params.push_back (base_val);
              MFIT::base_params.push_back (mals_win->gaussians[3 + i]);
              MFIT::param_fixed.push_back (false);
          ...
          if (dist2_active)
            {
              if (cb_fix_dist2->isChecked () || fixed_curves.count (pos + 1)
                  || (cb_comm_dist2->isChecked () && i))
                {
                  ...
                  if (i && cb_comm_dist2->isChecked ()
                      ...
                      && !fixed_curves.count (1))
                    {
                      MFIT::fixed_params.push_back (
                          mals_win->gaussians[4 + 0]);
                      MFIT::comm_backref[MFIT::param_fixed.size ()]
                          = comm_dist_2_var_pos;
                    }
                  else
                    {
                      MFIT::fixed_params.push_back (
                          mals_win->gaussians[4 + i]);
                    }
                  MFIT::fixed_params.push_back (mals_win->gaussians[4 + i]);
                  MFIT::param_fixed.push_back (true);
                }
              else
                {
                  if (cb_comm_dist2->isChecked ())
                    {
                      comm_dist_2_var_pos = MFIT::init_params.size ();
                    }
                  MFIT::param_pos.push_back (MFIT::init_params.size ());
                  ...
                  if (cb_pct_dist2_from_init->isChecked ())
                    ...
                    {
                      base_val = mals_win->gaussians[4 + i];
                    }

                  MFIT::init_params.push_back (base_val);
                  MFIT::base_params.push_back (mals_win->gaussians[4 + i]);
                  MFIT::param_fixed.push_back (false);
...
bool
US_Hydrodyn_Mals_Fit::lock_zeros (vector<double> &par)
{
  ...
  if (!mals_lock_min_retry)
    {
      // us_qdebug( "lock_zeros() not enabled" );
      return false;
    }

  if (cb_fix_amplitude->isChecked () && cb_fix_width->isChecked ())
    {
  ...
  vector<double> tmp_gaussians = mals_win->gaussians;
  ...
  for (unsigned int i = 0; i < MFIT::param_fixed.size (); i++)
    {
      if (!MFIT::param_fixed[i] || MFIT::comm_backref.count (i))
        {
          tmp_gaussians[i]
              = par[MFIT::comm_backref.count (i) ? MFIT::comm_backref[i]
                                                 : MFIT::param_pos[i]];
          if (MFIT::conc_ratios_map.count (i))
            {
              tmp_gaussians[i] *= MFIT::conc_ratios_map[i];
            }
        }
    }
  ...
  for (unsigned int i = 0; i < (unsigned int)tmp_gaussians.size ();
       i += gaussian_type_size)
    {
      unsigned int pos = i / gaussian_type_size;
      ...
      if (!cb_fix_curves[pos]->isChecked ())
        {
          ...
          if (!cb_fix_amplitude->isChecked ())
            {
              if (tmp_gaussians[0 + i]
                  <= mals_ampl_width_min * mals_lock_min_retry_mult)
                {
  ...
  for (set<unsigned int>::iterator it = to_lock.begin (); it != to_lock.end ();
       ++it)
    {
...
bool
US_Hydrodyn_Mals_Fit::max_free_peak_delta (vector<double> &par)
{
  // us_qdebug( "max_free_peak_delta_zeros() start" );

  if (!mals_maxfpk_restart)
    {
  ...
  // us_qdebug( QString( "max_free_peak_delta() found max peak pos %1" ).arg(
  // max_pos + 1 ) );

  tmp_gaussians[0 + max_i] *= 1e0 + mals_maxfpk_restart_pct * 0.01;
  // gaussians_undo.push_back( mals_win->gaussians );
  mals_win->gaussians = tmp_gaussians;
  if (update_mals)
    {
      mals_win->gauss_init_markers ();
      mals_win->gauss_init_gaussians ();
...
void
US_Hydrodyn_Mals_Fit::lm (bool max_free_peak_delta_run, double prev_rmsd)
{
  update_common ();
  setup_run ();
  // puts( "lm" );
  ...
  vector<double> t;
  vector<double> y;
  MFIT::errors = mals_win->f_errors[mals_win->wheel_file];
  MFIT::errors_index.clear ();

  double start = mals_win->le_gauss_fit_start->text ().toDouble ();
  double end = mals_win->le_gauss_fit_end->text ().toDouble ();

  // US_Vector::printvector( QString( "lm %1 q" ).arg( mals_win->wheel_file ),
  // x ); US_Vector::printvector( QString( "lm %1 I" ).arg(
  // mals_win->wheel_file ), mals_win->f_Is[ mals_win->wheel_file ] );
  // US_Vector::printvector( QString( "lm %1 e" ).arg( mals_win->wheel_file ),
  // mals_win->f_errors[ mals_win->wheel_file ] );
  ...
  if (use_errors)
    {
      MFIT::errors_index.resize ((unsigned int)t.back () + 1);
      for (unsigned int i = 0; i <= (unsigned int)t.back (); i++)
        {
          MFIT::errors_index[i] = 0;
        }
      for (unsigned int i = 0; i < y.size (); i++)
        {
          y[i] /= MFIT::errors[i];
          MFIT::errors_index[(unsigned int)t[i]] = i;
        }
    }

  vector<double> org_params = MFIT::init_params;
  double org_rmsd = 0e0;
  ...
  if (use_errors)
    {
      ...
      for (unsigned int j = 0; j < t.size (); j++)
        {
          yp[j] = (*MFIT::compute_gaussian_f) (
              t[j], (double *)(&MFIT::base_params[0]));
          org_rmsd += (y[j] - yp[j]) * (y[j] - yp[j]);
        }
#if defined(UHHSF_FULL_DEBUG)
      US_Vector::printvector3 ("sd y, yp, e", y, yp, MFIT::errors);
#endif
      double nu = (double)(t.size () - (int)mals_win->gaussians.size () - 1);
      if (nu <= 0e0)
        {
          nu = 1e0;
        }
  ...
  vector<double> par = MFIT::init_params;
  // US_Vector::printvector( QString( "par start (rmsd %1)" ).arg( org_rmsd ),
  // par ); us_qdebug( QString( "par start (rmsd %1)\n" ).arg( org_rmsd ) );
  ...
  if (use_errors)
    {
      ...
      for (unsigned int j = 0; j < t.size (); j++)
        {
          yp[j] = (*MFIT::compute_gaussian_f) (t[j], (double *)(&par[0]));
      ...
      new_fnorm /= nu;
      // us_qdebug( QString( "recomputing fnorm for sd's ... should match
      // display field %1" ).arg( new_fnorm ) ); org_rmsd = sqrt( org_rmsd );
  ...
  US_Vector::printvector (
      QString ("par after fit (norm %1)").arg (status.fnorm), par);
#endif
  ...
  if (max_free_peak_delta_run)
    {
      // us_qdebug( QString( "max free peak delta run results org_rmsd %1
      // prev_rmsd %2 status.fnorm %3" )
      //         .arg( org_rmsd )
      //         .arg( prev_rmsd )
      //         .arg( status.fnorm )
      //         );
      if (org_rmsd > status.fnorm && prev_rmsd > status.fnorm)
        {
          ...
          for (unsigned int i = 0; i < MFIT::param_fixed.size (); i++)
            {
              if (!MFIT::param_fixed[i] || MFIT::comm_backref.count (i))
                {
                  mals_win->gaussians[i] = par[MFIT::comm_backref.count (i)
                                                   ? MFIT::comm_backref[i]
                                                   : MFIT::param_pos[i]];
                  if (MFIT::conc_ratios_map.count (i))
                    {
  ...
  else
    {
      ...
      if (mals_maxfpk_restart && max_free_peak_delta (par))
        {
...
US_Hydrodyn_Mals_Fit::gsm_sd ()
{
  ...
  for (unsigned int j = 0; j < gsm_t.size (); j++)
    {
      gsm_yp[j] = (*MFIT::compute_gaussian_f) (
          gsm_t[j], (double *)(&MFIT::base_params[0]));
      org_rmsd += (gsm_y[j] - gsm_yp[j]) * (gsm_y[j] - gsm_yp[j]);
    }

  org_rmsd = sqrt (org_rmsd);

  our_vector *v = new_our_vector (MFIT::init_params.size ());
  for (int i = 0; i < v->len; i++)
    {
      v->d[i] = MFIT::init_params[i];
    }
  ...
  min_gsm_5_1 (v, le_epsilon->text ().toDouble (),
      ...
      for (unsigned int i = 0; i < MFIT::param_fixed.size (); i++)
        {
          if (!MFIT::param_fixed[i] || MFIT::comm_backref.count (i))
            {
              mals_win->gaussians[i]
                  = v->d[MFIT::comm_backref.count (i) ? MFIT::comm_backref[i]
                                                      : MFIT::param_pos[i]];
              if (MFIT::conc_ratios_map.count (i))
                {
                  mals_win->gaussians[i] *= MFIT::conc_ratios_map[i];
...
US_Hydrodyn_Mals_Fit::gsm_ih ()
{
  ...
  min_hessian_bfgs (v, le_epsilon->text ().toDouble (),
                    le_iterations->text ().toLong ());
...
US_Hydrodyn_Mals_Fit::gsm_cg ()
{
  ...
  min_fr_pr_cgd (v, le_epsilon->text ().toDouble (),
                 le_iterations->text ().toLong ());
...
void
US_Hydrodyn_Mals_Fit::stop ()
{
  running = false;
}

void
US_Hydrodyn_Mals_Fit::grid ()
{
  setup_run ();
  update_common ();
  puts ("grid");
  ...
  unsigned int total_count
      = (unsigned int)pow ((double)pop_size, (int)MFIT::init_params.size ());
  ...
  for (unsigned int j = 0; j < (unsigned int)MFIT::init_params.size (); j++)
    {
      ofs_per[j] = (MFIT::param_max[j] - MFIT::param_min[j]) / (pop_size - 1);
    }
  ...
  for (unsigned int j = 0; j < t.size (); j++)
    {
      yp[j] = (*MFIT::compute_gaussian_f) (t[j],
                                           (double *)(&MFIT::base_params[0]));
      org_rmsd += (y[j] - yp[j]) * (y[j] - yp[j]);
    }
  ...
  for (unsigned int i = 0; i < total_count; i++)
    {
      ...
      for (unsigned int j = 0; j < (unsigned int)MFIT::init_params.size ();
           j++)
        {
          unsigned int pos = remainder % pop_size;
          use_par[j] = MFIT::param_min[j] + pos * ofs_per[j];
          remainder /= pop_size;
        }
      ...
      for (unsigned int j = 0; j < t.size (); j++)
        {
          yp[j] = (*MFIT::compute_gaussian_f) (t[j], (double *)(&use_par[0]));
