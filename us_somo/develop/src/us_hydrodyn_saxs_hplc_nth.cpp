...
void US_Hydrodyn_Saxs_Hplc_Nth::setupGUI()
{
   ...
   for ( int i = 0; i < ((US_Hydrodyn_Saxs_Hplc*)us_hydrodyn_saxs_hplc)->lb_files->count(); ++i )
   {
      lb_files->addItem( QString( "%1 : %2" ).arg( i + 1 ).arg( ((US_Hydrodyn_Saxs_Hplc*)us_hydrodyn_saxs_hplc)->lb_files->item( i )->text() ) );
      if ( ((US_Hydrodyn_Saxs_Hplc*)us_hydrodyn_saxs_hplc)->lb_files->item( i )->isSelected() )
      {
   ...
   {
      QIntValidator *qdv = new QIntValidator( 1, ((US_Hydrodyn_Saxs_Hplc*)us_hydrodyn_saxs_hplc)->lb_files->count(), le_n );
   ...
   {
      QIntValidator *qdv = new QIntValidator( 1, ((US_Hydrodyn_Saxs_Hplc*)us_hydrodyn_saxs_hplc)->lb_files->count(), le_start );
   ...
   lbl_start_name =  new QLabel      ( ( (US_Hydrodyn_Saxs_Hplc*)us_hydrodyn_saxs_hplc)->lb_files->item( le_start->text().toInt() - 1 )->text(), this );
   ...
   {
      QIntValidator *qdv = new QIntValidator( 2, ((US_Hydrodyn_Saxs_Hplc*)us_hydrodyn_saxs_hplc)->lb_files->count(), le_end );
      le_end->setValidator( qdv );
   }
   connect( le_end, SIGNAL( textChanged( const QString & ) ), SLOT( update_enables() ) );
   le_end->setMinimumWidth( 80 );

   lbl_end_name =  new QLabel      ( ( (US_Hydrodyn_Saxs_Hplc*)us_hydrodyn_saxs_hplc)->lb_files->item( le_end->text().toInt() - 1 )->text(), this );
...
void US_Hydrodyn_Saxs_Hplc_Nth::update_enables()
{
   ...
   for ( int i = 0; i < lb_files->count(); ++i )
   {
      ...
      if ( ((US_Hydrodyn_Saxs_Hplc*)us_hydrodyn_saxs_hplc)->lb_files->item( i )->text().contains( le_contains->text() ) )
      {
         contains.insert( i );
      }
      {
