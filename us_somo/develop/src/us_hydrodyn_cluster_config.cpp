...
void US_Hydrodyn_Cluster_Config::save_config() {
  ...
  for (map<QString, map<QString, QString> >::iterator it =
    ...
    if (it->second.count("stage")) {
      ...
      if (domain_to_dir.count(domain) && domain_to_dir[domain] != dir) {
        QMessageBox::warning(
            this, lbl_title->text(),
            QString(us_tr("Staging locations for duplicate physical servers "
                          "must be identical\n"
                          "There is an inconsistancy with stage '%1' found "
                          "with server '%2'\n"
  ...
  for (map<QString, map<QString, QString> >::iterator it =
    ...
    if (it->second.count("stage")) {
      ...
      ((US_Hydrodyn_Cluster *)cluster_window)->cluster_stage_to_system[system] =
          it->first;
    }
  }
...
void US_Hydrodyn_Cluster_Config::add_new() {
  ...
  if (any_selected && cluster_systems.count(lb_systems->item(pos)->text())) {
    switch (
        QMessageBox::question(this, us_tr("US-SOMO: Cluster config: Add new"),
                              QString(us_tr("Do you want to copy system data "
                                            "from the selected system '%1?'"))
    ...
    if (lb_systems->item(i)->isSelected()) {
      if (cluster_systems.count(lb_systems->item(i)->text())) {
        ...
        QMessageBox::warning(
            ...
            QString(
                us_tr("Internal error: The system %1 does not seem to exist?"))
    ...
    if (lb_systems->item(i)->isSelected()) {
      ...
      if (cluster_systems.count(lb_systems->item(i)->text())) {
        ...
        QMessageBox::warning(
            this, us_tr("US-SOMO: Cluster config"),
            QString(
                us_tr("The system %1 does not seem to have any information"))
                .arg(lb_systems->item(i)->text()));
      }
    }
  }
...
void US_Hydrodyn_Cluster_Config::http_error(
...
void US_Hydrodyn_Cluster_Config::http_finished() {
  ...
  if (comm_mode == "check_user") {
    ...
    if (readJson.count("name") && readJson["name"] == le_cluster_id->text()) {
      check_not_ok = false;
    } else {
      check_not_ok = true;
    }
  ...
  if (comm_mode == "add_user") {
    ...
    current_response_status =
        readJson.count("info")
            ? (readJson["info"].contains(QRegExp("added$")) ? QString("Success")
                                                            : readJson["info"])
            : QString("Error");
  }
  http_done(false);
...
void US_Hydrodyn_Cluster_Config::http_uploadProgress(qint64 done,
                                                     qint64 total) {
  cout << "http: uploadProgress " << done << " " << total << "\n";
}

void US_Hydrodyn_Cluster_Config::http_downloadProgress(qint64 done,
                                                       qint64 total) {
...
void US_Hydrodyn_Cluster_Config::http_done(bool error) {
  ...
  if (comm_mode == "add_user") {
    if (current_response_status == "Success") {
      QMessageBox::information(this, us_tr("US-SOMO: Cluster Config: Add user"),
      ...
      QMessageBox::warning(
          ...
          QString(
              us_tr(
                  "There was an error trying to add User \"%1\"\nWarning: %2"))
              .arg(le_cluster_id->text())
  ...
  if (comm_mode == "check_user") {
    if (check_not_ok) {
      QMessageBox::information(
          ...
          QString(
              us_tr("User \"%1\" does not exist.\nPress \"Add user\" to add."))
              .arg(le_cluster_id->text()));
    } else {
      map<QString, QString> readJson = US_Json::split(current_http_response);
      QString errors;
      ...
      // {
      //    errors += QString( us_tr( "Incorrect password.  Possibly another
      //    user has registered the same cluster id.\n" ) );
      // } else {
      //    password_ok = true;
      // }
      ...
      if (errors.isEmpty()) {
        ...
        QMessageBox::warning(this, us_tr("US-SOMO: Cluster Config: Check user"),
...
void US_Hydrodyn_Cluster_Config::check_user() {
  if (le_cluster_pw->text() != le_cluster_pw2->text()) {
    QMessageBox::warning(this, us_tr("US-SOMO: Cluster Config: Check user"),
                         us_tr("The passwords do not match"), QMessageBox::Ok,
  ...
  if (le_cluster_id->text().length() < 4) {
    QMessageBox::warning(this, us_tr("US-SOMO: Cluster Config: Check user"),
                         us_tr("Enter a longer cluster id"), QMessageBox::Ok,
                         QMessageBox::NoButton);
    return;
  }
  ...
  if (le_cluster_email->text().isEmpty()) {
    QMessageBox::warning(this, us_tr("US-SOMO: Cluster Config: Check user"),
                         us_tr("The email address is empty"), QMessageBox::Ok,
                         QMessageBox::NoButton);
    return;
  }
  ...
  if (le_submit_url->text().isEmpty()) {
    QMessageBox::warning(this, us_tr("US-SOMO: Cluster Config: Check user"),
                         us_tr("The Job Submit URL is empty"), QMessageBox::Ok,
                         QMessageBox::NoButton);
    return;
  }
  ...
  QString manage_url_host =
      QString("%1").arg(le_manage_url->text()).replace(QRegExp(":.*$"), "");
  ...
  http_request.setUrl(
      QUrl(QString("http://%1%2").arg(le_manage_url->text()).arg(msg_request)));
