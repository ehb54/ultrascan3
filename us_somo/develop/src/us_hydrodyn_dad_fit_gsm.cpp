...
vector<double> US_Hydrodyn_Dad_Fit::gsm_t;
vector<double> US_Hydrodyn_Dad_Fit::gsm_y;
vector<double> US_Hydrodyn_Dad_Fit::gsm_yp;

/* gsm stuff */
our_matrix *
US_Hydrodyn_Dad_Fit::new_our_matrix (int rows, int cols)
{
  our_matrix *m;
...
void
US_Hydrodyn_Dad_Fit::free_our_matrix (our_matrix *m)
{
  free (m->d);
  free (m);
}
...
void
US_Hydrodyn_Dad_Fit::set_our_matrix (our_matrix *m, double s)
{
  int i, limit;
  limit = m->rows * m->cols;
...
void
US_Hydrodyn_Dad_Fit::identity_our_matrix (our_matrix *m)
{
  int i;
...
void
US_Hydrodyn_Dad_Fit::print_our_matrix (our_matrix *m)
{
  int i, j;
...
our_vector *
US_Hydrodyn_Dad_Fit::new_our_vector (int len)
{
  our_vector *v;
...
void
US_Hydrodyn_Dad_Fit::free_our_vector (our_vector *v)
{
  free (v->d);
  free (v);
}
...
void
US_Hydrodyn_Dad_Fit::print_our_vector (our_vector *v)
{
  int i;
...
double
US_Hydrodyn_Dad_Fit::l2norm_our_vector (our_vector *v1, our_vector *v2)
{
  double norm = 0e0;
  int i;
...
void
US_Hydrodyn_Dad_Fit::copy_our_vector (our_vector *v1, our_vector *v2)
{
#if !defined(SUPRESS_ERROR_CHECKING)
  if (v1->len != v2->len)
    {
...
void
US_Hydrodyn_Dad_Fit::set_our_vector (our_vector *v1, double s)
{
  int i;
  /* could do this with ceil(log2(len)) memcpy()s */
  for (i = 0; i < v1->len; i++)
...
void
US_Hydrodyn_Dad_Fit::add_our_vector_vv (our_vector *v1, our_vector *v2)
{
  int i;
#if !defined(SUPRESS_ERROR_CHECKING)
  if (v1->len != v2->len)
...
void
US_Hydrodyn_Dad_Fit::add_our_vector_vs (our_vector *v1, double s)
{
  int i;
  for (i = 0; i < v1->len; i++)
    {
...
void
US_Hydrodyn_Dad_Fit::mult_our_vector_vv (our_vector *v1, our_vector *v2)
{
  int i;
#if !defined(SUPRESS_ERROR_CHECKING)
  if (v1->len != v2->len)
...
void
US_Hydrodyn_Dad_Fit::mult_our_vector_vs (our_vector *v1, double s)
{
  int i;
  for (i = 0; i < v1->len; i++)
    {
...
void
US_Hydrodyn_Dad_Fit::add_our_vector_vvv (our_vector *vd, our_vector *v1,
                                         our_vector *v2)
{
  int i;
#if !defined(SUPRESS_ERROR_CHECKING)
...
void
US_Hydrodyn_Dad_Fit::add_our_vector_vvs (our_vector *vd, our_vector *v1,
                                         double s)
{
  int i;
  for (i = 0; i < v1->len; i++)
...
void
US_Hydrodyn_Dad_Fit::sub_our_vector_vvv (our_vector *vd, our_vector *v1,
                                         our_vector *v2)
{
  int i;
#if !defined(SUPRESS_ERROR_CHECKING)
...
void
US_Hydrodyn_Dad_Fit::sub_our_vector_vvs (our_vector *vd, our_vector *v1,
                                         double s)
{
  int i;
  for (i = 0; i < v1->len; i++)
...
void
US_Hydrodyn_Dad_Fit::mult_our_vector_vvv (our_vector *vd, our_vector *v1,
                                          our_vector *v2)
{
  int i;
#if !defined(SUPRESS_ERROR_CHECKING)
...
void
US_Hydrodyn_Dad_Fit::mult_our_vector_vvs (our_vector *vd, our_vector *v1,
                                          double s)
{
  int i;
  for (i = 0; i < v1->len; i++)
...
double
US_Hydrodyn_Dad_Fit::dot_our_vector (our_vector *v1, our_vector *v2)
{
  int i;
  double ret = 0e0;
  for (i = 0; i < v1->len; i++)
...
void
US_Hydrodyn_Dad_Fit::mult_our_matrix_vmv (our_vector *vd, our_matrix *m,
                                          our_vector *vs)
{
  int i, j;
...
void
US_Hydrodyn_Dad_Fit::our_vector_test ()
{
  our_vector *v1, *v2, *v3;
...
long
US_Hydrodyn_Dad_Fit::min_gsm_5_1 (our_vector *i, double epsilon, long max_iter)
{
...
long
US_Hydrodyn_Dad_Fit::min_fr_pr_cgd (our_vector *i, double epsilon,
                                    long max_iter)
{
...
long
US_Hydrodyn_Dad_Fit::min_hessian_bfgs (our_vector *ip, double epsilon,
                                       long max_iter)
{
  ...
  for (unsigned int j = 0; j < x.size (); j++)
    {
      if (x[j] >= start && x[j] <= end)
        {
          gsm_t.push_back (x[j]);
          gsm_y.push_back (dad_win->f_Is[dad_win->wheel_file][j]);
        }
    }
...
double
US_Hydrodyn_Dad_Fit::gsm_f_GAUSS (our_vector *v)
{
  ...
  for (unsigned int i = 0; i < (unsigned int)DFIT::param_fixed.size ();)
    {
      if (DFIT::param_fixed[i])
        {
          height = DFIT::fixed_params[DFIT::param_pos[i]];
        }
      else
        {
          height = v->d[DFIT::param_pos[i]];
          if (height < DFIT::param_min[DFIT::param_pos[i]]
              || height > DFIT::param_max[DFIT::param_pos[i]])
            {
              return 1e99;
            }
        }
      ...
      if (DFIT::param_fixed[i])
        {
          center = DFIT::fixed_params[DFIT::param_pos[i]];
        }
      else
        {
          center = v->d[DFIT::param_pos[i]];
          if (center < DFIT::param_min[DFIT::param_pos[i]]
              || center > DFIT::param_max[DFIT::param_pos[i]])
            {
              return 1e99;
            }
        }
      ...
      if (DFIT::param_fixed[i])
        {
          width = DFIT::fixed_params[DFIT::param_pos[i]];
        }
      else
        {
          width = v->d[DFIT::param_pos[i]];
          if (width < DFIT::param_min[DFIT::param_pos[i]]
              || width > DFIT::param_max[DFIT::param_pos[i]])
            {
              return 1e99;
            }
        }
...
double
US_Hydrodyn_Dad_Fit::gsm_f_EMG (our_vector *v)
{
  for (unsigned int j = 0; j < gsm_yp.size (); j++)
    {
      gsm_yp[j] = 0e0;
  ...
  for (unsigned int i = 0; i < (unsigned int)DFIT::param_fixed.size ();)
    {
      ...
      if (DFIT::param_fixed[i])
        {
          if (DFIT::comm_backref.count (i))
            {
              dist1 = v->d[DFIT::comm_backref[i]];
            }
          else
            {
              dist1 = DFIT::fixed_params[DFIT::param_pos[i]];
            }
        }
      else
        {
          dist1 = v->d[DFIT::param_pos[i]];
          if (dist1 < DFIT::param_min[DFIT::param_pos[i]]
              || dist1 > DFIT::param_max[DFIT::param_pos[i]])
            {
              return 1e99;
            }
        }
...
double
US_Hydrodyn_Dad_Fit::gsm_f_GMG (our_vector *v)
{
  for (unsigned int j = 0; j < gsm_yp.size (); j++)
    {
      gsm_yp[j] = 0e0;
...
double
US_Hydrodyn_Dad_Fit::gsm_f_EMGGMG (our_vector *v)
{
  for (unsigned int j = 0; j < gsm_yp.size (); j++)
    {
      gsm_yp[j] = 0e0;
      ...
      if (DFIT::param_fixed[i])
        {
          if (DFIT::comm_backref.count (i))
            {
              width = v->d[DFIT::comm_backref[i]] * DFIT::conc_ratios_map[i];
            }
          else
            {
              width = DFIT::fixed_params[DFIT::param_pos[i]];
      ...
      if (DFIT::param_fixed[i])
        {
          if (DFIT::comm_backref.count (i))
            {
              dist2 = v->d[DFIT::comm_backref[i]];
            }
          else
            {
              dist2 = DFIT::fixed_params[DFIT::param_pos[i]];
            }
        }
      else
        {
          dist2 = v->d[DFIT::param_pos[i]];
          if (dist2 < DFIT::param_min[DFIT::param_pos[i]]
              || dist2 > DFIT::param_max[DFIT::param_pos[i]])
            {
              return 1e99;
            }
        }
...
void
US_Hydrodyn_Dad_Fit::gsm_df (our_vector *vd, our_vector *v)
{
  double y0;
  double y2;
  double sav_ve;
