...
void
lm_printout_std (int n_par, const double *par, int m_dat,
                 const void * /* data */, const double *fvec, int printflags,
...
void
lmmin (int n_par, double *par, int m_dat, const void *data,
                         ...
                         int iflag, int iter, int nfev))
{
  ...
  if ((fvec = (double *)malloc (m * sizeof (double))) == NULL
      ...
      || (fjac = (double *)malloc ((unsigned long)n * m * sizeof (double)))
             == NULL
...
void
lm_lmdif (int m, int n, double *x, double *fvec, double ftol, double xtol,
...
void
lm_lmpar (int n, double *r, int ldr, int *ipvt, double *diag, double *qtb,
...
void
lm_qrfac (int m, int n, double *a, int pivot, int *ipvt, double *rdiag,
          double *acnorm, double *wa)
{
  /*
...
void
lm_qrsolv (int n, double *r, int ldr, int *ipvt, double *diag, double *qtb,
...
double
lm_rmsdnorm (int n, const double *x)
{
  double rmsd = 0e0;
  for (int i = 0; i < n; i++)
    {
...
void
lmcurve_evaluate (const double *par, int m_dat, const void *data, double *fvec,
                  int * /* info */)
{
  int i;
  for (i = 0; i < m_dat; i++)
    fvec[i] = ((lmcurve_data_struct *)data)->y[i]
              - ((lmcurve_data_struct *)data)
                    ->f (((lmcurve_data_struct *)data)->t[i], par);
  // *info = *info; /* to prevent a 'unused variable' warning */
}

void
lmcurve_fit (int n_par, double *par, int m_dat, const double *t,
...
void
lmcurve_fit_rmsd (int n_par, double *par, int m_dat, const double *t,
                  const double *y, double (*f) (double t, const double *par),
                  const lm_control_struct *control, lm_status_struct *status)
{
  lmcurve_data_struct data = { t, y, f };
