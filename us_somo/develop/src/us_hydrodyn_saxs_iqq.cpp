...
bool
US_Hydrodyn_Saxs::compute_scale_excl_vol ()
{
                  ...
                  if (swh_chain)
                    {
                      if (our_saxs_options->swh_excl_vol > 0e0)
                        {
                          our_saxs_options->iq_target_ev
                              += (double)our_saxs_options->swh_excl_vol
                                 * swh_count;
                          editor_msg (
                              "blue",
                              QString (
                                  us_tr ("Adding water to excluded volume "
                                         "using preset excl vol %1 number of "
                                         "waters %2 -> excluded volume %3\n"))
                                  .arg (our_saxs_options->swh_excl_vol)
                                  .arg (swh_count)
                                  .arg ((double)our_saxs_options->swh_excl_vol
                                        * swh_count));
                        }
                      else
                        {
                          ...
                          if (hybrid_name.isEmpty () || !hybrid_name.length ()
                              || !hybrid_map.count (hybrid_name)
                              || !atom_map.count ("OW~" + hybrid_name))
                            {
                              cout << "error: hybrid or map or atom name "
                                      "missing for WAT|OW"
                                   << endl;
                              editor_msg ("red",
                                          QString ("Molecule %1 Residue WAT "
                                                   "OW Hybrid/map/atom name "
                                                   "missing. Atom skipped.\n")
                                              .arg (j + 1));
                              continue;
                            }
                          double swh_excl_vol
                              = atom_map["OW~" + hybrid_name].saxs_excl_vol;
                          our_saxs_options->iq_target_ev
                              += swh_excl_vol * swh_count;
                          editor_msg (
                              "blue",
                              QString (
                                  us_tr ("Adding water to excluded volume "
                                         "using defined excl vol %1 number of "
                                         "waters %2 -> excluded volume %3\n"))
                                  .arg (swh_excl_vol)
                                  .arg (swh_count)
                      ...
                      if (swh_count)
                        {
                          editor_msg (
                              "red",
                              us_tr ("Error: a chain with WAT/OW and other "
                                     "residues is currently unsupported\n"));
                          return false;
                        }

                      our_saxs_options->iq_target_ev
                          += US_Hydrodyn::mw_to_volume (
                              model_vector[current_model].molecule[j].mw,
                              model_vector[i].vbar);
                      editor_msg (
                          "blue",
                          QString (us_tr ("Adding molecule to excluded volume "
                                          "from mw/vbar computation: mw %1 "
                                          "vbar %2 -> excluded volume %3\n"))
                              .arg (model_vector[current_model].molecule[j].mw)
                              .arg (model_vector[current_model].vbar)
                              .arg (US_Hydrodyn::mw_to_volume (
                                  model_vector[current_model].molecule[j].mw,
                                  model_vector[current_model].vbar)));
                    }
                }
            }
        }
  ...
  editor_msg ("blue", QString (us_tr ("Total computed excluded volume: %1\n"))
          ...
          for (unsigned int k = 0;
               k < model_vector[current_model].molecule[j].atom.size (); k++)
            {
              ...
              if (!atom_map.count (this_atom_name + "~" + hybrid_name))
                {
                  cout << "error: (saxs_iqq 1) atom_map missing for "
                          "hybrid_name "
              ...
              // this is probably correct but FoXS uses the saxs table excluded
              // volume
              new_atom.excl_vol
              ...
              if (our_saxs_options->use_somo_ff)
                {
                  double this_ev
                      = get_ff_ev (new_atom.residue_name, new_atom.atom_name);
                  if (this_ev)
                    {
              ...
              if (our_saxs_options->hybrid_radius_excl_vol)
                {
                  new_atom.excl_vol = (4.0 / 3.0) * M_PI
  ...
  if (tot_excl_vol_noh > 0e0)
    {
      our_saxs_options->scale_excl_vol
          = (our_saxs_options->iq_target_ev - tot_excl_vol + tot_excl_vol_noh)
            / tot_excl_vol_noh;
    }
  else
    {
  ...
  editor_msg ("blue",
              QString (us_tr ("Scaling excluded volume to match target (%1), "
                              "default scattering center sum without WAT %2, "
                              "total %3, new scaler = %4"))
                  .arg (our_saxs_options->iq_target_ev)
...
void
US_Hydrodyn_Saxs::calc_saxs_iq_native_fast ()
{
  int WAT_Tf_used = 0;
  bool use_WAT_Tf
  ...
  for (unsigned int i = 0; i < selected_models.size (); i++)
    {
           ...
           j < model_vector[current_model].molecule.size (); j++)
        {
          for (unsigned int k = 0;
               k < model_vector[current_model].molecule[j].atom.size (); k++)
            {
              ...
              if (!atom_map.count (this_atom_name + "~" + hybrid_name))
                {
                  cout << "error: (saxs_iqq 2) atom_map missing for "
                          "hybrid_name "
                       << hybrid_name << " atom name " << this_atom_name
                       << endl;
                  editor_msg (
              ...
              if (use_WAT_Tf && this_atom->resName == "WAT"
                ...
                {
                  new_atom.excl_vol
                      = this_atom->tempFactor * this_atom->tempFactor
                        * this_atom->tempFactor * M_PI * 4e0 / 3e0;
                  WAT_Tf_used++;
                }
      ...
      if (atoms.size () == 1)
        {
          editor_msg (
              "dark blue",
              us_tr ("Notice: using full Debye since only 1 atom available"));
          return calc_saxs_iq_native_debye ();
        }

      is_only_one_swh
          = (atoms.size () == 1
             && (atoms[0].atom_name == "OW" || atoms[0].atom_name == "O"));
          ...
          if (!exact_q.size ())
            {
              editor_msg (
                  "dark red",
                  QString (us_tr ("Notice: exact q is empty, computing based "
                                  "upon current q range ")));
              exact_q.resize (q_points);
              for (unsigned int j = 0; j < q_points; j++)
                {
                  exact_q[j] = our_saxs_options->start_q
                               + j * our_saxs_options->delta_q;
                }
            }
          else
            {
      ...
      for (unsigned int i = 0; i < atoms.size (); i++)
        {
          ...
          f[0][i] = compute_ff (saxs, saxsH, atoms[i].residue_name,
                                atoms[i].saxs_name, atoms[i].atom_name,
                                atoms[i].hydrogens, 0.0, 0.0);
              ...
              for (unsigned int i = 0; i < as1; i++)
                {
                  ...
                  for (unsigned int k = i + 1; k < as; k++)
                    {
                      rik2 = (atoms[i].pos[0] - atoms[k].pos[0])
      ...
      editor->append (
          QString (us_tr ("Total excluded volume %1\n")).arg (tot_excl_vol));
      editor->append (QString (us_tr ("Average electron density %1\n"))
                          .arg (total_e / tot_excl_vol, 4));
      if (tot_excl_vol_noh != tot_excl_vol || total_e_noh != total_e)
        {
          editor->append (
              QString (us_tr ("Total unhydrated excluded volume %1\n"))
                  .arg (tot_excl_vol_noh));
          editor->append (
              QString (us_tr ("Average unhydrated electron density %1\n"))
                  .arg (total_e_noh / tot_excl_vol_noh));
          editor->append (
              QString (us_tr ("Electron density of hydration %1\n"))
                  .arg ((total_e - total_e_noh)
                        / (tot_excl_vol - tot_excl_vol_noh)));
        }
      ...
      if (our_saxs_options->saxs_iq_native_fast_compute_pr)
        {
          if (create_native_saxs)
            {
              QString fpr_name = USglobal->config_list.root_dir + SLASH
                                 + "somo" + SLASH + "saxs" + SLASH
                                 + sprr_filestring ();
                  ...
                  if (fpr)
                    {
                      ...
                      ((US_Hydrodyn *)us_hydrodyn)->last_saxs_header
                          = QString ("").sprintf (
                              "SOMO p(r) vs r data generated from %s by "
                              "US_SOMO %s %s bin size %f mw %.2f Daltons area "
                              "%.2f\n",
          ...
          else
            {
              ...
              ((US_Hydrodyn *)us_hydrodyn)->last_saxs_header
                  = QString ("").sprintf (
                      ...
                      "bin size %f mw %.2f Daltons area %.2f\n",
                      model_filename.toLatin1 ().data (),
                      US_Version.toLatin1 ().data (), REVISION, delta,
                      get_mw (te_filename2->text (), false),
                      compute_pr_area (pr, r));
          ...
          plotted_pr_mw.push_back (
              (float)get_mw (te_filename2->text (), false));
      ...
      progress_saxs->setValue (1);
      progress_saxs->setMaximum (2);
          ...
          for (unsigned int j = 0; j < hist.size (); j++)
            {
              ...
              // cout << "sinc(x) = "  << (( fabs(x) < 1e-16 ) ? 1.0 : sin(x) /
              // x) << endl; cout << "r_dist[" << j << "] = " << hist[j] <<
              // endl;
      ...
      if (our_saxs_options->multiply_iq_by_atomic_volume)
        {
          if (model_vector[current_model].volume == 0.0)
            {
              editor_msg ("red", us_tr ("Volume is zero so NOT multiplying "
                                        "I(q) by atomic volume\n"));
            }
          else
            {
              editor_msg (
                  "blue",
                  QString (us_tr ("Multiplying I(q) by atomic volume of %1\n"))
              ...
              if (is_only_one_swh
                         ...
                         > 0)
                {
                  editor_msg (
                      ...
                      QString (
                          us_tr (
                              "Multiplying I(q) by water multiplier of %1\n"))
                          .arg (
                              ((US_Hydrodyn *)us_hydrodyn)
                                  ->multiply_iq_by_atomic_volume_last_water_multiplier));
                  for (unsigned int i = 0; i < I.size (); i++)
      ...
      //      for ( unsigned int i = 0; i < q.size(); i++ )
      //      {
      //         cout << QString("plotted_q[%1][%2] = %3  plotted_I[%4]I[%5] =
      //         %6\n")
      //    .arg(p)
      //            .arg(i)
      //            .arg(plotted_q[p][i])
      ...
      if (create_native_saxs)
      ...
      else
        {
          ...
          ((US_Hydrodyn *)us_hydrodyn)->last_saxs_iqqc.clear ();
          ((US_Hydrodyn *)us_hydrodyn)->last_saxs_header
              = QString ("").sprintf ("Simulated SAXS data generated from %s "
                                      "by US_SOMO %s %s q(%f:%f) step %f\n",
                                      model_filename.toLatin1 ().data (),
                                      US_Version.toLatin1 ().data (), REVISION,
...
//  ------------------------------------------------------------------------------------------------------
void
US_Hydrodyn_Saxs::calc_saxs_iq_native_debye ()
{
  int WAT_Tf_used = 0;
  bool use_WAT_Tf
      = ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("use_WAT_Tf_pdb")
  ...
  for (unsigned int i = 0; i < selected_models.size (); i++)
    {
           ...
           j < model_vector[current_model].molecule.size (); j++)
        {
          for (unsigned int k = 0;
               k < model_vector[current_model].molecule[j].atom.size (); k++)
            {
              ...
              if (!atom_map.count (this_atom_name + "~" + hybrid_name))
                {
                  cout << "error: (saxs_iqq 3) atom_map missing for "
                          "hybrid_name "
                       << hybrid_name << " atom name " << this_atom_name
                       << endl;
                  editor_msg (
              ...
              cout
                  << QString (
                         ...
                         .arg (atom_map[this_atom_name + "~" + hybrid_name]
                                   .saxs_excl_vol)
...
#if defined(SAXS_DEBUG2)
              QTextStream (stderr)
                  << "Atom: " << this_atom->name << " Residue: " << use_resname
                  << " SAXS atom: " << new_atom.saxs_name
                  << " Coordinates: " << new_atom.pos[0] << " , "
                  << new_atom.pos[1] << " , " << new_atom.pos[2]
                  << " Excl vol: " << new_atom.excl_vol << endl;
      ...
      // spec calc for now
      {
        ...
        for (unsigned int i = 0; i < atoms.size (); i++)
          {
            ...
            double this_n = compute_ff (sm, saxsH, atoms[i].residue_name,
                                        atoms[i].saxs_name, atoms[i].atom_name,
                                        atoms[i].hydrogens, 0e0, 0e0);
#if defined(DEBUG_SAXS)
        ...
        cout << QString ("exact I0 (rho0 == 0) %1 I0 %2 diff %3 rho0 V^2 %4 "
                         "ev total %5\n")
                    .arg (eI0rho0)
                    .arg (eI0)
                    .arg (eI0rho0 - eI0)
          ...
          for (unsigned int j = 0; j < q_points; j++)
            {
              ...
              f[j][i] = compute_ff (saxs, saxsH, atoms[i].residue_name,
                                    atoms[i].saxs_name, atoms[i].atom_name,
                                    atoms[i].hydrogens, q[j], q_over_4pi_2[j]);

              fc[j][i]
                  = vie
                    * exp (vi_23_4pi * q2[j] * our_saxs_options->ev_exp_mult);
              fp[j][i] = f[j][i] - fc[j][i];
      ...
      if (save_calc_to_csv)
        {
          ...
          QFile f (((US_Hydrodyn *)us_hydrodyn)->somo_dir + SLASH + "tmp"
                   + SLASH + "last_iqq.csv");
...
//  ------------------------------------------------------------------------------------------------------
void
US_Hydrodyn_Saxs::calc_saxs_iq_native_hybrid2 ()
{
  // don't forget to later merge deleted waters into model_vector
  // right now we are going with first residue map entry
  stopFlag = false;
  ...
  for (unsigned int i = 0; i < selected_models.size (); i++)
    {
           ...
           j < model_vector[current_model].molecule.size (); j++)
        {
          for (unsigned int k = 0;
               k < model_vector[current_model].molecule[j].atom.size (); k++)
            {
              ...
              if (!atom_map.count (this_atom_name + "~" + hybrid_name))
                {
                  cout << "error: (saxs_iqq 4) atom_map missing for "
                          "hybrid_name "
                       << hybrid_name << " atom name " << this_atom_name
                       << endl;
                  editor_msg (
          ...
          for (unsigned int j = 0; j < q_points; j++)
            {
              ...
              //                  q_over_4pi_2[j]) + saxsH.a[1] *
              //                  exp(-saxsH.b[1] * q_over_4pi_2[j]) +
              //                  saxsH.a[2] * exp(-saxsH.b[2] *
              //                  q_over_4pi_2[j]) + saxsH.a[3] *
              //                  exp(-saxsH.b[3] * q_over_4pi_2[j]) );
      ...
      if (our_saxs_options->saxs_iq_hybrid_adaptive)
        {
          US_Saxs_Util usu;
          if (!usu.create_adaptive_grid (
                  q, I, our_saxs_options->hybrid2_q_points, r))
            {
        ...
        if (our_saxs_options->saxs_iq_native_hybrid)
        ...
        else
          {
            if (our_saxs_options->saxs_iq_native_hybrid2)
              {
                if (!usu.quadratic_interpolate_iq_curve (q, use_q, fast_I, I,
                                                         I))
                  {
                    cout << usu.errormsg << endl;
                  }
              ...
              {
                if (!usu.cubic_spline_interpolate_iq_curve (q, use_q, fast_I,
                                                            I, I))
                  {
                    cout << usu.errormsg << endl;
                  }
...
void
US_Hydrodyn_Saxs::set_scaling_target (QString &scaling_target)
{
  if (!our_saxs_options->iqq_default_scaling_target.isEmpty ()
      && qsl_plotted_iq_names
             .filter (QRegExp (QString ("^%1$").arg (
      ...
      if (!our_saxs_options->disable_iq_scaling
          && qsl_plotted_iq_names.size ())
        {
          ...
          scaling_target = US_Static::getItem (
              ...
              us_tr ("Select the target plotted data set for scaling the "
                     "loaded data:\n"
    ...
    double q, double q_o_4pi2)
{
  ...
  cout << QString ("compute_ff: q: %1 nr:%2 na:%3 naf:%4 h:%5 use_somo_ff %6 "
                   "alt_ff %7\n")
              .arg (q)
              .arg (nr)
              .arg (na)
      ...
      if (ff_table.count (ffkey))
        {
          ...
          QString errormsg;
          double ff;
          if (!US_Saxs_Util::static_apply_natural_spline (
                  ff_q[ff_table[ffkey]], ff_ff[ff_table[ffkey]],
                  ff_y2[ff_table[ffkey]], q, ff, errormsg))
            {
              editor_msg ("red",
                          QString (us_tr ("Warning: error applying natural "
                                          "spline to %1 at %2 <%3>"))
                              .arg (ffkey)
                              .arg (q)
                              .arg (errormsg));
          ...
          if (!ff_sent_msg1.count (ffkey))
            {
              editor_msg (
                  "dark red",
                  QString (us_tr ("Warning: key %1 not found in ff_table"))
                      .arg (ffkey));
  ...
  if (!h)
    {
      if (our_saxs_options->five_term_gaussians)
        {
          return sa.c5 + sa.a5[0] * exp (-sa.b5[0] * q_o_4pi2)
      ...
      else
        {
          ...
          return sa.c + sa.a[0] * exp (-sa.b[0] * q_o_4pi2)
  ...
  if (!hybrid_coords.count (na))
    {
      editor_msg ("red", QString ("ERROR: No hybrid info for atom %1, "
                                  "computing for ZERO hydrogens")
                             .arg (na));
  ...
  if (hybrid_coords[na].size () <= h)
    {
      editor_msg (
          "red",
          QString ("ERROR: More hydrogens requested (%1) than available (%2) "
                   "for atom %2, computing for available hydrogens")
              .arg (h)
...
bool
US_Hydrodyn_Saxs::load_ff_table (QString filename)
{
  if (ff_table_loaded && last_ff_filename == filename)
    {
      editor_msg ("dark red", "Notice: this version is doing an unneeded "
  ...
  if (!f.exists ())
    {
      editor_msg ("red",
                  QString (us_tr ("Error: load ff table %1 does not exist"))
                      .arg (filename));
      return false;
    }
  ...
  if (!f.open (QIODevice::ReadOnly))
    {
      editor_msg ("red",
                  QString (us_tr ("Error: load ff table %1 can not be opened"))
                      .arg (filename));
      return false;
    }
  ...
  while (!ts.atEnd ())
    {
          ...
          if (qsl.count ())
            {
              editor_msg (
                  ...
                  QString (
                      us_tr (
                          "Error: load_ff line %1, residue/atoms not paired"))
                      .arg (line));
              return false;
            }
          continue;
      ...
      if (qsl[0] == "excludedvolume")
        {
          ...
          if (qsl.count () != 1)
            {
              editor_msg (
                  "red",
                  QString (us_tr ("Error: load_ff line %1, excluded volume "
          ...
          if (ff_ev.size () != pos)
            {
              editor_msg ("red",
                          QString (us_tr ("Error: load_ff line %1, excluded "
                                          "volume multiply defined"))
                              .arg (line));
              return false;
            }
      ...
      if (qsl[0] == "startdata")
        {
          ...
          if (ff_ev.size () != ff_ff.size () + 1)
            {
              editor_msg ("red",
                          QString (us_tr ("Error: load_ff line %1, startdata "
                                          "with no excluded volume defined"))
                              .arg (line));
              return false;
            }
              ...
              if (qsl[0] == "enddata")
                {
                  if (new_q.size () < 10)
                    {
                      editor_msg (
                          "red", QString (us_tr ("Error: load_ff line %1, "
              ...
              if (qsl.size () != 2)
                {
                  editor_msg (
                      "red",
                      QString (us_tr ("Error: load_ff line %1, data lines "
          ...
          if (!ok)
            {
              editor_msg ("red",
                          QString (us_tr ("Error: load_ff line %1, permature "
                                          "eof: no enddata token found"))
                              .arg (line));
              return false;
            }
          continue;
      ...
      editor_msg ("red",
                  QString (us_tr ("Error: load_ff line %1, unknown token %2"))
                      .arg (line)
                      .arg (qsl[0]));
      return false;
    }
  ...
  editor_msg ("blue",
              QString (us_tr ("ff table %1 loaded ok")).arg (filename));
...
double
US_Hydrodyn_Saxs::get_ff_ev (QString res, QString atm)
{
...
bool
US_Hydrodyn_Saxs::create_somo_ff ()
{
  errormsg = "";
  ...
  if (!f.open (QIODevice::WriteOnly))
    {
      errormsg
          = QString (us_tr ("Error: can not create %1")).arg (f.fileName ());
      return false;
    }
      ...
      for (unsigned int h = 0; h <= max_h; h++)
        {
          ...
          for (unsigned int j = 0; j < q_points; j++)
            {
              double I = compute_ff (saxsA, saxsH, res, ta, ta, h, q[j],
                                     q_over_4pi_2[j]);
              ts << QString ("%1 %2\n")
                        .arg (q[j], 0, 'g', 17)
                        .arg (I, 0, 'g', 17);
...
void
US_Hydrodyn_Saxs::create_shd (vector<saxs_atom> &org_atoms, vector<double> &q,
                              vector<double> &q2, vector<double> &q_over_4pi_2)
{
  puts ("create_shd");
  ...
  for (int i = 0; i < (int)atoms.size (); i++)
    {
      ...
      QString key
          = atoms[i].atom_name
            + QString ("%1").arg (
                atoms[i].hydrogens ? QString ("H%1").arg (atoms[i].hydrogens)
                                   : QString (""));
      if (!types.count (key))
        {
          {
          ...
          for (int j = 0; j < (int)q.size (); j++)
            {
              Fa[j] = compute_ff (saxs, saxsH, atoms[i].residue_name,
                                  atoms[i].saxs_name, atoms[i].atom_name,
                                  atoms[i].hydrogens, q[j], q_over_4pi_2[j])
                      - vie
                            * exp (vi_23_4pi * q2[j]
                                   * our_saxs_options->ev_exp_mult);
            }
          F.push_back (Fa);
        }
  ...
  // now write it all out
  {
    ...
    if (!ofs.is_open ())
      {
        editor_msg ("red",
                    QString (us_tr ("Error: could not open %1")).arg (fname));
        return;
      }
    ...
    if (!ofs.write ((char *)&id, sizeof (id)))
      {
        ...
        editor_msg (
            "red",
            QString (us_tr ("Error: could not write id to %1")).arg (fname));
        return;
      }
        ...
        if (!ofs.write ((char *)(&F[i][0]), sizeof (double) * id.q_size))
          {
            ofs.close ();
            editor_msg ("red",
                        QString (us_tr ("Error: could not write F to %1"))
                            .arg (fname));
            return;
          }
      }
    ...
    if (!ofs.write ((char *)(&q[0]), sizeof (double) * id.q_size))
      {
        ...
        editor_msg (
            "red",
            QString (us_tr ("Error: could not write q to %1")).arg (fname));
        return;
      }

    if (!ofs.write ((char *)(&model[0]),
                    sizeof (struct shd_point) * id.model_size))
      {
        ofs.close ();
        editor_msg ("red",
                    QString (us_tr ("Error: could not write model to %1"))
                        .arg (fname));
        return;
      }
