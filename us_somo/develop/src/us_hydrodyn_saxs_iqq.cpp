...
bool US_Hydrodyn_Saxs::compute_scale_excl_vol() {
          ...
          if (swh_chain) {
            if (our_saxs_options->swh_excl_vol > 0e0) {
              our_saxs_options->iq_target_ev +=
                  (double)our_saxs_options->swh_excl_vol * swh_count;
              editor_msg(
                  "blue",
                  QString(
                      us_tr(
                          "Adding water to excluded volume using preset excl "
                          "vol %1 number of waters %2 -> excluded volume %3\n"))
              ...
              if (hybrid_name.isEmpty() || !hybrid_name.length() ||
                ...
                cout << "error: hybrid or map or atom name missing for WAT|OW"
                     << endl;
                editor_msg("red",
                           QString("Molecule %1 Residue WAT OW Hybrid/map/atom "
              ...
              editor_msg(
                  ...
                  QString(
                      us_tr(
                          "Adding water to excluded volume using defined excl "
                          "vol %1 number of waters %2 -> excluded volume %3\n"))
                      .arg(swh_excl_vol)
                      .arg(swh_count)
            ...
            if (swh_count) {
              editor_msg("red", us_tr("Error: a chain with WAT/OW and other "
                                      "residues is currently unsupported\n"));
              return false;
            }

            our_saxs_options->iq_target_ev += US_Hydrodyn::mw_to_volume(
                model_vector[current_model].molecule[j].mw,
                model_vector[i].vbar);
            editor_msg(
                "blue",
                QString(
                    us_tr("Adding molecule to excluded volume from mw/vbar "
                          "computation: mw %1 vbar %2 -> excluded volume %3\n"))
    ...
    for (unsigned int j = 0; j < model_vector[current_model].molecule.size();
      ...
      for (unsigned int k = 0;
        ...
        if (our_saxs_options->hybrid_radius_excl_vol) {
          new_atom.excl_vol =
              (4.0 / 3.0) * M_PI * hybrid_map[hybrid_name].radius *
  ...
  editor_msg("blue", QString(us_tr("Scaling excluded volume to match target "
                                   "(%1), default scattering center sum "
                                   "without WAT %2, total %3, new scaler = %4"))
                         .arg(our_saxs_options->iq_target_ev)
...
void US_Hydrodyn_Saxs::calc_saxs_iq_native_fast() {
  ...
  for (unsigned int i = 0; i < selected_models.size(); i++) {
    ...
    for (unsigned int j = 0; j < model_vector[current_model].molecule.size();
      ...
      for (unsigned int k = 0;
        ...
        if (use_WAT_Tf && this_atom->resName == "WAT" &&
          ...
          new_atom.excl_vol = this_atom->tempFactor * this_atom->tempFactor *
        ...
        cout << "Atom: " << this_atom->name << " Residue: " << use_resname
    ...
    is_only_one_swh = (atoms.size() == 1 && (atoms[0].atom_name == "OW" ||
                                             atoms[0].atom_name == "O"));

    // save the atoms to a temporary file
    QString fsaxs_atoms_name =
        USglobal->config_list.root_dir + SLASH "somo" + SLASH "saxs" + "SLASH" +
        "tmp" + SLASH + QString("%1").arg(te_filename2->text()) +
        QString("_%1").arg(model_vector[current_model].model_id) + ".atoms";
    ...
    if (our_saxs_options->iq_exact_q) {
      ...
      if (!exact_q.size()) {
        ...
        for (unsigned int j = 0; j < q_points; j++) {
          exact_q[j] =
              our_saxs_options->start_q + j * our_saxs_options->delta_q;
        }
      } else {
    ...
    for (unsigned int i = 0; i < atoms.size(); i++) {
      ...
      f[0][i] =
          compute_ff(saxs, saxsH, atoms[i].residue_name, atoms[i].saxs_name,
                     atoms[i].atom_name, atoms[i].hydrogens, 0.0, 0.0);
    ...
    if (our_saxs_options->saxs_iq_native_fast_compute_pr) {
      if (create_native_saxs) {
        QString fpr_name = USglobal->config_list.root_dir + SLASH + "somo" +
                           SLASH + "saxs" + SLASH + sprr_filestring();
        ...
        if (ok_to_write) {
          ...
          if (fpr) {
            ...
            //                us_tr("The output file ") + fpr_name + us_tr("
    ...
    for (unsigned int i = 0; i < q.size(); i++) {
      ...
      for (unsigned int j = 0; j < hist.size(); j++) {
        ...
        // cout << "sinc(x) = "  << (( fabs(x) < 1e-16 ) ? 1.0 : sin(x) / x) <<
        // endl; cout << "r_dist[" << j << "] = " << hist[j] << endl;
    ...
    if (our_saxs_options->multiply_iq_by_atomic_volume) {
      if (model_vector[current_model].volume == 0.0) {
        editor_msg(
            "red",
            us_tr("Volume is zero so NOT multiplying I(q) by atomic volume\n"));
      } else {
        editor_msg("blue",
        ...
        if (is_only_one_swh &&
          ...
          editor_msg(
              "blue",
              QString(us_tr("Multiplying I(q) by water multiplier of %1\n"))
                  .arg(
                      ((US_Hydrodyn *)us_hydrodyn)
                          ->multiply_iq_by_atomic_volume_last_water_multiplier));
          for (unsigned int i = 0; i < I.size(); i++) {
            I[i] *= ((US_Hydrodyn *)us_hydrodyn)
                        ->multiply_iq_by_atomic_volume_last_water_multiplier;
          }
        }
      }
    ...
    if (create_native_saxs) {
      ...
      if (ok_to_write) {
        ...
        if (fsaxs) {
          ...
          ((US_Hydrodyn *)us_hydrodyn)->last_saxs_iqqc.clear();
          ((US_Hydrodyn *)us_hydrodyn)->last_saxs_header = QString("").sprintf(
              "Simulated SAXS data generated from %s by US_SOMO %s %s q(%f:%f) "
              "step %f\n",
              model_filename.toLatin1().data(), US_Version.toLatin1().data(),
...
void US_Hydrodyn_Saxs::calc_saxs_iq_native_debye() {
  ...
  for (unsigned int i = 0; i < selected_models.size(); i++) {
    ...
    QString fsaxs_atoms_name =
        ...
        SLASH + "tmp" + SLASH + QString("%1").arg(te_filename2->text()) +
        QString("_%1").arg(model_vector[current_model].model_id) + ".atoms";
    ...
    // spec calc for now
    {
      ...
      for (unsigned int i = 0; i < atoms.size(); i++) {
        ...
        double this_n =
            compute_ff(sm, saxsH, atoms[i].residue_name, atoms[i].saxs_name,
                       atoms[i].atom_name, atoms[i].hydrogens, 0e0, 0e0);
#if defined(DEBUG_SAXS)
      ...
      for (unsigned int i = 0; i < n.size(); i++) {
        for (unsigned int j = 0; j < n.size(); j++) {
          I0rho0 += n[i] * n[j];
          I0 += n[i] * n[j] -
                rho0 * (n[i] * v[j] + n[j] * v[i] - rho0 * (v[i] * v[j]));
          evx += rho0 * (v[i] * v[j]);
        }
      }
      ...
      cout << QString(
                  "exact I0 (rho0 == 0) %1 I0 %2 diff %3 rho0 V^2 %4 ev total "
                  "%5\n")
                  .arg(eI0rho0)
                  .arg(eI0)
                  .arg(eI0rho0 - eI0)
...
void US_Hydrodyn_Saxs::calc_saxs_iq_native_hybrid2() {
  ...
  for (unsigned int i = 0; i < selected_models.size(); i++) {
    ...
    if (our_saxs_options->saxs_iq_hybrid_adaptive) {
      US_Saxs_Util usu;
      if (!usu.create_adaptive_grid(q, I, our_saxs_options->hybrid2_q_points,
                                    r)) {
...
void US_Hydrodyn_Saxs::set_scaling_target(QString &scaling_target) {
  if (!our_saxs_options->iqq_default_scaling_target.isEmpty() &&
    ...
    if (!our_saxs_options->disable_iq_scaling && qsl_plotted_iq_names.size()) {
      bool ok;
      scaling_target =
          US_Static::getItem(us_tr("Scale I(q) Curve"),
                             us_tr("Select the target plotted data set for "
                                   "scaling the loaded data:\n"
...
double US_Hydrodyn_Saxs::compute_ff(
  ...
  cout << QString(
              "compute_ff: q: %1 nr:%2 na:%3 naf:%4 h:%5 use_somo_ff %6 alt_ff "
              "%7\n")
              .arg(q)
              .arg(nr)
              .arg(na)
    ...
    if (ff_table.count(ffkey)) {
      ...
      if (!US_Saxs_Util::static_apply_natural_spline(
        ...
        editor_msg(
            ...
            QString(
                us_tr(
                    "Warning: error applying natural spline to %1 at %2 <%3>"))
                .arg(ffkey)
                .arg(q)
                .arg(errormsg));
        qApp->processEvents();
      ...
      if (!ff_sent_msg1.count(ffkey)) {
        editor_msg(
            "dark red",
            QString(us_tr("Warning: key %1 not found in ff_table")).arg(ffkey));
  ...
  if (!hybrid_coords.count(na)) {
    editor_msg(
        "red",
        QString(
            "ERROR: No hybrid info for atom %1, computing for ZERO hydrogens")
            .arg(na));
  ...
  if (hybrid_coords[na].size() <= h) {
    editor_msg("red",
               QString("ERROR: More hydrogens requested (%1) than available "
                       "(%2) for atom %2, computing for available hydrogens")
                   .arg(h)
...
bool US_Hydrodyn_Saxs::load_ff_table(QString filename) {
  if (ff_table_loaded && last_ff_filename == filename) {
    editor_msg("dark red",
               "Notice: this version is doing an unneeded reload for debugging "
               "purposes");
    // return true;
  }
  ...
  if (!f.exists()) {
    editor_msg(
        "red",
        QString(us_tr("Error: load ff table %1 does not exist")).arg(filename));
    return false;
  }
  ...
  while (!ts.atEnd()) {
    ...
    if (qsl[0] == "residueatom") {
      ...
      if (qsl.count()) {
        editor_msg(
            "red",
            QString(us_tr("Error: load_ff line %1, residue/atoms not paired"))
                .arg(line));
        return false;
      }
      continue;
    ...
    if (qsl[0] == "excludedvolume") {
      ...
      if (ff_ev.size() != pos) {
        editor_msg(
            ...
            QString(
                us_tr(
                    "Error: load_ff line %1, excluded volume multiply defined"))
                .arg(line));
        return false;
      }
    ...
    if (qsl[0] == "startdata") {
      ...
      if (ff_ev.size() != ff_ff.size() + 1) {
        editor_msg("red", QString(us_tr("Error: load_ff line %1, startdata "
      ...
      while (!ts.atEnd()) {
        ...
        if (qsl[0] == "enddata") {
          if (new_q.size() < 10) {
            editor_msg(
                "red",
                QString(
                    us_tr("Error: load_ff line %1, empty or insufficient data"))
                    .arg(line));
            return false;
          }
        ...
        if (qsl.size() != 2) {
          editor_msg("red", QString(us_tr("Error: load_ff line %1, data lines "
      ...
      if (!ok) {
        editor_msg("red", QString(us_tr("Error: load_ff line %1, permature "
    ...
    editor_msg("red", QString(us_tr("Error: load_ff line %1, unknown token %2"))
                          .arg(line)
                          .arg(qsl[0]));
    return false;
  }
...
bool US_Hydrodyn_Saxs::create_somo_ff() {
  ...
  QString res = "";
  for (map<QString, saxs>::iterator it = saxs_map.begin(); it != saxs_map.end();
       it++) {
    QString ta = it->first;
    ...
    for (unsigned int h = 0; h <= max_h; h++) {
      ...
      for (unsigned int j = 0; j < q_points; j++) {
        double I =
            compute_ff(saxsA, saxsH, res, ta, ta, h, q[j], q_over_4pi_2[j]);
...
void US_Hydrodyn_Saxs::create_shd(vector<saxs_atom> &org_atoms,
                                  vector<double> &q, vector<double> &q2,
  ...
  for (int i = 0; i < (int)atoms.size(); i++) {
    ...
    QString key = atoms[i].atom_name +
                  QString("%1").arg(atoms[i].hydrogens
                                        ? QString("H%1").arg(atoms[i].hydrogens)
                                        : QString(""));
    if (!types.count(key)) {
      {
  ...
  // now write it all out
  {
    ...
    for (uint32_t i = 0; i < id.F_size; ++i) {
      if (!ofs.write((char *)(&F[i][0]), sizeof(double) * id.q_size)) {
        ofs.close();
        editor_msg("red",
                   QString(us_tr("Error: could not write F to %1")).arg(fname));
        return;
      }
    }
    ...
    if (!ofs.write((char *)(&model[0]),
      ...
      editor_msg(
          "red",
          QString(us_tr("Error: could not write model to %1")).arg(fname));
      return;
    }
