...
void US_Hydrodyn_Batch::setupGUI() {
  ...
  bg_residues =
      new QGroupBox(3, Qt::Vertical, "If non-coded residues are found:", this);
  ...
  cb_compute_iq_only_avg->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_compute_prr_std_dev->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_results_dir->setToolTip(
      "Currently only available when only Build vdW bead model or Calculate RB "
...
void US_Hydrodyn_Batch::cancel() { close(); }
...
void US_Hydrodyn_Batch::closeEvent(QCloseEvent *e) {
  if (batch_job_running) {
    QMessageBox mb(
        this->windowTitle(),
        us_tr("Attention:\nThere is a batch job running, do you really want to "
              "stop it?"),
...
void US_Hydrodyn_Batch::add_files() {
  ...
  while (it != filenames.end()) {
    ...
    if (!dup) {
      ...
      editor->setTextColor("dark red");
      editor->append(
          QString(us_tr("File skipped: %1 (already in list)")).arg(*it));
    }
    ++it;
  }
  editor->setTextColor(save_color);
...
void US_Hydrodyn_Batch::update_enables() {
  ...
  bool anything_to_do =
      cb_somo->isChecked() || cb_somo_o->isChecked() || cb_grid->isChecked() ||
      cb_vdw_beads->isChecked() || cb_iqq->isChecked() || cb_prr->isChecked() ||
      cb_hydro->isChecked() || cb_zeno->isChecked() || cb_grpy->isChecked() ||
      cb_hullrad->isChecked() || cb_dmd->isChecked() || cb_fd->isChecked() || 0;

  if ((cb_vdw_beads->isChecked() || cb_somo->isChecked() ||
       cb_somo_o->isChecked() || cb_zeno->isChecked() ||
       cb_grpy->isChecked()) &&
      !cb_grid->isChecked() && !cb_iqq->isChecked() && !cb_prr->isChecked() &&
      !cb_hydro->isChecked() && !cb_hullrad->isChecked() &&
      !cb_dmd->isChecked()) {
...
void US_Hydrodyn_Batch::residue(int val) { batch->missing_residues = val; }
...
void US_Hydrodyn_Batch::atom(int val) { batch->missing_atoms = val; }
...
void US_Hydrodyn_Batch::stop_processing() {
  ...
  editor_msg(
      "dark red",
      "Files selected may have changed, please verify before rerunning Start.");
  enable_after_stop();
  disable_updates = false;
  save_batch_active = false;
      ...
      if (((US_Hydrodyn *)us_hydrodyn)->saxs_search_widget) {
        ((US_Hydrodyn *)us_hydrodyn)
            ->saxs_search_window->editor_msg(
                "blue", us_tr("Set search parameters for batch mode and then\n"
...
void US_Hydrodyn_Batch::start(bool quiet) {
  ...
  if (batch->fd &&
      ((US_Hydrodyn *)us_hydrodyn)
              ->gparam_value(US_Hydrodyn_Fractal_Dimension_Options::paramname(
                  US_Hydrodyn_Fractal_Dimension_Options::ENABLED)) != "true") {
    switch (QMessageBox::warning(
        this, windowTitle() + us_tr(": Warning"),
        QString(
            us_tr("Fractal Dimension computations are not enabled in options\n"
                  "What would you like to do?\n")),
  ...
  if (batch->fd &&
      ((US_Hydrodyn *)us_hydrodyn)
              ->gparam_value(US_Hydrodyn_Fractal_Dimension_Options::paramname(
                  US_Hydrodyn_Fractal_Dimension_Options::PLOTS)) == "true") {
    switch (QMessageBox::warning(
        this, windowTitle() + us_tr(": Warning"),
        QString(us_tr(
  ...
  for (int i = 0; i < lb_files->count(); i++) {
    ...
    if (lb_files->item(i)->isSelected()) {
      ...
      for (int mindex = 0; mindex < (int)models.size(); ++mindex) {
        ...
        // QFileInfo(get_file_name( i, m ) ).fileName() ); TSO << QString( "-->
        // processing index %1 model %2 file %3\n" ).arg( mindex ).arg( m ).arg(
        // get_file_name( i, m ) );
        ...
        if (result || proceed_anyway) {
          if (result) {
            ...
            editor->setTextColor("dark red");
            editor->append(
                QString(us_tr("Screening: %1 not ok, but proceeding anyway.")
                            .arg(file)));
            editor->setTextColor("dark blue");
            result = 1;
          }
          ...
          if (pdb_mode) {
            ...
            if (batch->grid) {
              ...
              result =
                  ((US_Hydrodyn *)us_hydrodyn)->calc_grid_pdb() ? false : true;
            ...
            if (batch->somo_o) {
              ...
              job_timer.start_timer(
                  QString("%1 atob").arg(get_file_name(i, m)));
              result =
                  ((US_Hydrodyn *)us_hydrodyn)->calc_somo_o() ? false : true;
              job_timer.end_timer(QString("%1 atob").arg(get_file_name(i, m)));
            }
            if (batch->vdw_beads) {
              job_timer.init_timer(QString("%1 atob").arg(get_file_name(i, m)));
              job_timer.start_timer(
                  QString("%1 atob").arg(get_file_name(i, m)));
              result =
                  ((US_Hydrodyn *)us_hydrodyn)->calc_vdw_beads() ? false : true;
              job_timer.end_timer(QString("%1 atob").arg(get_file_name(i, m)));
            }
            restore_us_hydrodyn_settings();
          }
          ...
          if (result && batch->iqq) {
            ...
            if (batch->mm_all && lb_model->count() > 1) {
              ...
              for (unsigned int ii = 0; ii < lb_model_rows && !stopFlag; ii++) {
                ...
                if (batch->hydrate) {
                  if (!pdb_mode) {
                    // we should never get here since bead models normally don't
                    // contain multiple models but possible some sort of DAM
                    // model might...
                    editor_msg("dark red",
                               "Bead models can not be hydrated, continuing "
                               "without hydration\n");
                  } else {
                    job_timer.init_timer(
                        QString("%1 hydrate").arg(get_file_name(i, m)));
                    job_timer.start_timer(
                        QString("%1 hydrate").arg(get_file_name(i, m)));
                    result = ((US_Hydrodyn *)us_hydrodyn)
                                         ->pdb_hydrate_for_saxs(true) != 0
                                 ? false
                                 : true;
                    job_timer.end_timer(
                        QString("%1 hydrate").arg(get_file_name(i, m)));
                ...
                if (result) {
                  hydrated_pdb_nmr_text +=
                      ((US_Hydrodyn *)us_hydrodyn)->last_hydrated_pdb_text;
#endif
                  if (((US_Hydrodyn *)us_hydrodyn)->saxs_plot_widget) {
                    ((US_Hydrodyn *)us_hydrodyn)
                        ->saxs_plot_window->clear_plot_saxs(true);
                  }
                  if (batch->saxs_search) {
                    if (activate_saxs_search_window()) {
                      job_timer.init_timer(
                          QString("%1 saxs_search").arg(get_file_name(i, m)));
                      job_timer.start_timer(
                          QString("%1 saxs_search").arg(get_file_name(i, m)));
                    ...
                    job_timer.init_timer(
                        QString("%1 calc iqq").arg(get_file_name(i, m)));
                    job_timer.start_timer(
                        QString("%1 calc_iqq").arg(get_file_name(i, m)));
                    result = ((US_Hydrodyn *)us_hydrodyn)
                                     ->calc_iqq(!pdb_mode,
                                                !batch->csv_saxs ||
                                                    batch->create_native_saxs,
                                                false)
                                 ? false
                                 : true;
                    job_timer.end_timer(
                  ...
                  if (batch->hydrate && pdb_mode && batch->mm_all &&
                    ...
                    QDir qd;
                    cout << "remove: "
                         << ((US_Hydrodyn *)us_hydrodyn)->last_hydrated_pdb_name
                         << endl;
                    qd.remove(
                        ((US_Hydrodyn *)us_hydrodyn)->last_hydrated_pdb_name);
                  }
                  ...
                  if (batch->csv_saxs) {
                    if (batch->saxs_search) {
                      ...
                      if (saxs_q.size() <
                          ((US_Hydrodyn *)us_hydrodyn)->last_saxs_q.size()) {
                        saxs_q = ((US_Hydrodyn *)us_hydrodyn)->last_saxs_q;
                      }
                      saxs_iqq.push_back(
                          ((US_Hydrodyn *)us_hydrodyn)->last_saxs_iqq);
                      saxs_iqqa.push_back(
                          ((US_Hydrodyn *)us_hydrodyn)->last_saxs_iqqa);
                      saxs_iqqc.push_back(
                          ((US_Hydrodyn *)us_hydrodyn)->last_saxs_iqqc);
                    }
                  }
#if defined(USE_H)
                }
              ...
              if (batch->hydrate) {
                ...
                if (!f.open(QIODevice::WriteOnly)) {
                  editor_msg("red",
                             QString(us_tr("can not open file %1 for writing"))
                                 .arg(fname));
                } else {
                  QTextStream ts(&f);
              ...
              if (batch->hydrate) {
                if (!pdb_mode) {
                  // we should never get here since bead models normally don't
                  // contain multiple models but possible some sort of DAM model
                  // might...
                  editor_msg("dark red",
                             "Bead models can not be hydrated, continuing "
                             "without hydration\n");
                  ...
                  if (batch->saxs_search) {
                    if (result &&
                      ...
                      for (unsigned int i = 0;
                           i <
                           ((US_Hydrodyn *)us_hydrodyn)
                               ->saxs_search_window->csv_source_name_iqq.size();
                           i++) {
                        csv_source_name_iqq.push_back(
                            ((US_Hydrodyn *)us_hydrodyn)
                                ->saxs_search_window->csv_source_name_iqq[i]);
            ...
            if (batch->mm_all && lb_model->count() > 1) {
              ...
              for (unsigned int ii = 0; ii < lb_model_rows && !stopFlag; ii++) {
...
#endif
                editor_msg("dark gray",
                           QString(us_tr("Processing P(r): %1 from %2"))
                               .arg(lb_model->item(ii)->text())
                               .arg(QFileInfo(get_file_name(i, m)).fileName()));
                ;
                qApp->processEvents();
                ...
                if (result) {
                  ...
                  job_timer.init_timer(
                      QString("%1 calc_prr").arg(get_file_name(i, m)));
                  job_timer.start_timer(
                      QString("%1 calc_prr").arg(get_file_name(i, m)));
                  result = ((US_Hydrodyn *)us_hydrodyn)
                                   ->calc_prr(!pdb_mode,
                  ...
                  if (batch->csv_saxs) {
                    ...
                    if (saxs_r.size() <
                        ((US_Hydrodyn *)us_hydrodyn)->last_saxs_r.size()) {
                      saxs_r = ((US_Hydrodyn *)us_hydrodyn)->last_saxs_r;
                    }
                    saxs_prr.push_back(
                        ((US_Hydrodyn *)us_hydrodyn)->last_saxs_prr);
                    saxs_prr_norm.push_back(
                        ((US_Hydrodyn *)us_hydrodyn)->last_saxs_prr_norm);
          ...
          if (result && (batch->hydro || batch->zeno || batch->grpy) &&
            ...
            save_us_hydrodyn_settings();
            job_timer.init_timer(
                QString("%1 hydrodynamics").arg(get_file_name(i, m)));
            job_timer.start_timer(
                QString("%1 hydrodynamics").arg(get_file_name(i, m)));
            if (batch->mm_all) {
              lb_model->selectAll();
            ...
            if (batch->hydro) {
              result =
                  ((US_Hydrodyn *)us_hydrodyn)->calc_hydro() ? false : true;
            }
            if (batch->zeno) {
              result = ((US_Hydrodyn *)us_hydrodyn)->calc_zeno_hydro() ? false
                                                                       : true;
            }
            if (batch->grpy) {
          ...
          if (result && batch->hullrad) {
            if (file.contains(QRegExp(".(pdb|PDB)$"))) {
              ...
              // us_qdebug( QString( "hullrad call result %1 hullrad_running %2"
              // ).arg( result ? "true" : "false" ).arg(  ((US_Hydrodyn
  ...
  if (batch->fd && ((US_Hydrodyn *)us_hydrodyn)->saveParams &&
    ...
    if (of) {
      fprintf(
          of, "%s",
          ((US_Hydrodyn *)us_hydrodyn)->save_util->header().toLatin1().data());
  ...
  if (save_batch_active) {
    ...
    if (((US_Hydrodyn *)us_hydrodyn)->save_params.data_vector.size() > 1) {
      stats = ((US_Hydrodyn *)us_hydrodyn)
                  ->save_util->stats(
                      &((US_Hydrodyn *)us_hydrodyn)->save_params.data_vector);
    }
    ...
    if (!batch->hullrad) {
      ...
      if (of) {
        ...
        for (unsigned int i = 0;
             i < ((US_Hydrodyn *)us_hydrodyn)->save_params.data_vector.size();
             i++) {
          fprintf(of, "%s",
                  ((US_Hydrodyn *)us_hydrodyn)
        ...
        for (unsigned int i = 0;
          ...
          fprintf(
              ...
              ((US_Hydrodyn *)us_hydrodyn)
                  ->save_util
                  ->dataString(&(
                      ((US_Hydrodyn *)us_hydrodyn)->save_params.data_vector[i]))
                  .toLatin1()
                  .data());
        }
        if (stats.size() == 2) {
        ...
        if (model_lines.length()) {
          if (!(pos % res)) {
            ...
            if (!fn_out.open(QIODevice::WriteOnly)) {
              editor_msg("red", QString(us_tr("could not create file %1, "
...
int US_Hydrodyn_Batch::count_files() { return lb_files->count(); }
  ...
  if (!cancel_req) {
    ...
    if (!fi.exists()) {
      ...
      if (!new_dir.mkdir(proc_dir)) {
        editor->setTextColor("red");
        editor->append(
            us_tr("Error: Could not create directory " + proc_dir + "\n"));
        editor->setTextColor(save_color);
        cancel_req = true;
      } else {
        editor->setTextColor("dark blue");
  ...
  for (int i = 0; i < lb_files->count(); i++) {
    ...
    if (lb_files->item(i)->isSelected()) {
      ...
      if (file.contains(QRegExp(".(pdb|PDB)$"))) {
        ...
        editor->setTextColor("red");
        editor->append(
            QString(us_tr("PDB not yet supported for movie frames: %1"))
                .arg(file));
      } else {
        result = screen_bead_model(file);
        if (result) {
        ...
        ((US_Hydrodyn *)us_hydrodyn)
            ->visualize(true, proc_dir, scale, black_background, do_pat);
      }
    }
  }
  ...
  */
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->movie_text.size()) {
    ...
    for (unsigned int i = 0;
      ...
      cmd1.push_back(
          cd + "pnmquant 256 " + fi.fileName() + ".ppm > " + fi.fileName() +
          "-q.ppm\n" + "ppmtogif " + fi.fileName() + "-q.ppm > " +
          fi.fileName() + ".gif\n" + "echo " + fi.fileName() + ".gif >> " +
          output_file + ".list\n" +
          (use_tc ? QString("mogrify -gravity southwest -fill %1 -font "
                            "Courier-10-Pitch-Regular -pointsize %2 -draw "
                            "'text 25,25 \"%3 %4\"' ")
                  ...
                  : "") +
          (title != "" ? QString("mogrify -gravity north -fill %1 -pointsize "
                                 "%2 -draw 'text 25,0 \"%3\"' ")
                                 .arg(black_background ? "white" : "black")
                                 .arg(tc_pointsize)
                                 .arg(title) +
    ...
    editor_msg(
        "dark blue",
        QString(us_tr("Created movie file %1")).arg(output_file + ".avi"));
  } else {
...
void US_Hydrodyn_Batch::save_csv_saxs_iqq(bool quiet) {
  QString fname = ((US_Hydrodyn *)us_hydrodyn)->somo_dir + SLASH + "saxs" +
                  SLASH + batch->csv_saxs_name + "_iqq" + iqq_suffix() + ".csv";
  ...
  if (of) {
    ...
    if (batch->compute_iq_avg) {
      for (unsigned int i = 0; i < csv_source_name_iqq.size(); i++) {
        // cout << "model: " << i << " " << vector_double_to_csv(saxs_iqq[i]) <<
        // endl;
  ...
  if (of) {
    ...
    if (batch->compute_prr_avg) {
      for (unsigned int i = 0; i < csv_source_name_prr.size(); i++) {
        // cout << "model: " << i << " " << vector_double_to_csv(saxs_prr[i]) <<
        // endl;
    ...
    if (batch->compute_prr_avg && sum_count > 1) {
      ...
      if (batch->compute_prr_std_dev && sum_count > 2) {
        fprintf(of, "\"%s\",%.2f,%.2f,\"%s\",%s\n", "Standard deviation",
                pr_mw_std_dev, compute_pr_area(pr_std_dev, saxs_r), "P(r)",
                vector_double_to_csv(pr_std_dev).toLatin1().data());
        fprintf(of, "\"%s\",%.2f,%.2f,\"%s\",%s\n",
                "Average minus 1 standard deviation", pr_mw_avg - pr_mw_std_dev,
                compute_pr_area(pr_avg_minus_std_dev, saxs_r), "P(r)",
                vector_double_to_csv(pr_avg_minus_std_dev).toLatin1().data());
        fprintf(of, "\"%s\",%.2f,%.2f,\"%s\",%s\n",
                "Average plus 1 standard deviation", pr_mw_avg + pr_mw_std_dev,
