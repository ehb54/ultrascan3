...
// */ extern char ragcol[SMAX];       /* array for the file name of the radii,
// masses and colors file */
//
...
int us_hydrodyn_asab1_main_hydro(vector<PDB_atom *> use_active_atoms,
...
// )
{
  ...
  */
...

  {
    ...
    while (min_asa < 0.0) {
      // printf("\n\nASA RE-CHECK  - Insert the minimum ASA %% threshold level
      // [0-99] : ");
      scanf("%f", &min_asa);
      if ((min_asa < 0.0) | (min_asa > 99.0)) min_asa = (float)-1.0;
    }
  } else {
    // em("s5");
    init();
  ...
  */
  ...
  for (i = 1; i < passi; i++) /* iteration for the number of steps */
  {
    ...
    // us_hydrodyn->lbl_core_progress->setText(QString("Iteration %1 of %2 with
    // %3 %4")
    //                                         .arg(i+1)
    //                                         .arg(passi)
    //                                         .arg(cont1)
  ...
  for (l = 1; l < nat; l++) /* GROUPING ASA OF ATOMS INTO ASA OF RESIDUES */
  {
    ...
    if ((strcmp(dd1->elm, azoto) == 0) || (strcmp(dd1->elm, carbonio0) == 0) ||
      ...
      /*      printf("\n%s\n","Ora passo");
              printf("\n%d\t%s%d%s\t%f\t%s\t%f\n",l-1,"dt[",l-1,"].tab:
         ",dt[l-1].tab,"asamin: ",asamin); scanf("%d",&topolino); getchar(); */
  ...
  for (l = 1; l < nat; l++) {
    ...
    if (strcmp(dd1->elm, "CA") == 0) {
      ...
      /*            printf("\n%s\n","Ora passo in asapep CA");
                    printf("\n%s\t%d\t%s\t%f\t%s\t%f\n","indCA= ",indCA,"asapep:
         ",asapep,"asa[l]",asa[l]); scanf("%d",&topolino); getchar(); */
    }
                ...
                if (strcmp(dd2->elm, azoto) == 0) {
                  if (dd2->col == 10) {
                    ...
                    /*   printf("\n%d\t%s\t%s\t%s\t%d\t%s\t%s\n",i,"Pluto
                       ...
                       ",dd2->descr); scanf("%s",pluto); getchar(); */
                  } else
                    fprintf(mol1, "%d\n", 1);
                }
...
a300:
  ...
  if (check_asa == 1) {
    ...
    for (l = 0; l < nat; l++) {
      ...
      if (isnan(asa[l])) {
        // printf("ASA WARNING NAN begin replaced by zero: atom %u asa %f > sa+p
        // %f (sa %f)\n",
        //        l,
        //        asa[l],
        //        sapp,
...
static void initarray() {
  ...
  /* IN ROWS 21-26 AND 49-54 ARE THE VALUES FOR GAL, NAG, FUC, MAN, SIA AND GLUC
   * (OG1) DEDUCED FROM KALIANNAN 2001 */
  ...
  // printf("\n\n\t%s", "Output files: Enter '0' for default (provaly, provaly1,
  // provaly2, asaris)"); printf("\n\t%s", "                    '1' for new
  // filenames\n\n\t\t\t-->");
  scanf("%d", &fe2);
  getchar();
  if (fe2 == 0) {
...
a50:
  ...
  if (strcmp(arr_temp, "END") == 0) goto a200;
...
static void readline(int ind) {
  ...
  for (j = 0; j < 24; j++) fscanf(init3_brook, "%c", &(stringa[j]));
  ...
  if (flagr1 != 1) {
    ...
    dd->descr[9] = 0;
    for (j = 0; j < 9; j++) {
      if (stringa[13 + j] == ' ') stringa[13 + j] = '_';
    ...
    for (j = 0; j < 9; j++) {
      ...
      if (stringa[11 + j] == ' ') stringa[11 + j] = '_';
  ...
  switch (dd->elm[0]) {
    ...
    case 'O':
      if (isalpha(dd->elm[1]) != 0 || isdigit(dd->elm[1]) != 0) /* selects Ox */
      {
        ...
        {
          if (isalpha(dd->elm[2]) != 0 ||
              isdigit(dd->elm[2]) != 0) /* selects O >= 10 or O1A/O1B  */
          {
            if ((dd->elm[2] == 'A') ||
        ...
        else if (dd->elm[1] == '4') /* O4 of sugar  O2H1 */
        {
          flrad = (float)1.46;
          dd->m = 17;
          /*             if(strcmp(dd->amin,"OG1")==0) / * OCTYL GLUCOSIDE O4
           * O2H1 */
          /*               strcpy(dd->amin,"OG1"); */
        }
        ...
        else if (dd->elm[1] == '5') /* O5 of sugar */
        {
          /* MAN, GAL, NAG, FUC O1H0 */
          {
            flrad = (float)1.42;
            dd->m = 16;
            /*             if(strcmp(dd->amin,"OG1")==0) / * OCTYL GLUCOSIDE O5
             * O2H1 */
            /*               strcpy(dd->amin,"OG1"); */
          }
        }
...
static void assigntab(int xx) {
  ...
  if (rprobe >
      0.001) /* 25/6/02 CONTROLLO, E' STRANO! LA FORMULA È PER L'AREA DEL
                CERCHIO, NON PER LA SUPERFICE DELLA SFERA!!!!! IMPLEMENTATA
      ...
      switch (dd->amin[1]) {
        ...
        case ('R'):
          ...
          switch (dd->elm[0]) {
            case ('C'):
              dd->m = 14; /*C? */
              if (dd->elm[1] == 'Z') dd->m = 12;
              /*CZ*/ if (dd->elm[1] == 'A')
                dd->m = 13;
              /*CA*/ break;

            case ('N'):
              if (dd->elm[1] == 'E') dd->m = 15;
              /*NE*/ if (dd->elm[2] == '1')
                dd->m = 16;                      /*NH1 */
              if (dd->elm[2] == '2') dd->m = 17; /*NH2 */
              break;
          }
          break;
    ...
    case ('T'):
      switch (dd->amin[1]) {
        case ('H'):
          dd->tab = arr1[k + 16];
          dd->vol = (float)118.3;
          if (strcmp(dd->elm, "CB") == 0) dd->m = 13;
          if (strcmp(dd->elm, "OG1") == 0) dd->m = 17;
          if (strcmp(dd->elm, "CG2") == 0) dd->m = 15;
          break;
