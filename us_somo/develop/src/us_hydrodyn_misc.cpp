...
US_Hydrodyn_Misc::US_Hydrodyn_Misc(struct misc_options *misc, bool *misc_widget,
...
void US_Hydrodyn_Misc::setupGUI() {
  ...
  lbl_vbar_temperature = new QLabel(
      us_tr(QString(" Vbar measured/computed at T=[%1C]: ").arg(DEGREE_SYMBOL)),
      this);
  ...
  lbl_avg_banner =
      new QLabel(us_tr("Average Parameters for Automatic Bead Builder:"), this);
  ...
  lbl_avg_vbar = new QLabel(
      us_tr(" Average Residue vbar [cm" + UNICODE_SUPER_3_QS + "/g]: "), this);
  ...
  lbl_target_e_density =
      new QLabel(us_tr(" Target electron density [" + UNICODE_ANGSTROM_QS +
  ...
  lbl_covolume =
      new QLabel(us_tr(" Covolume [cm" + UNICODE_SUPER_3_QS + "/mol]: "), this);
  ...
  le_covolume->setText(
      QString("%1").arg(((US_Hydrodyn *)us_hydrodyn)->gparams.count("covolume")
  ...
  cb_export_msroll->setText(
      us_tr(" Create MSROLL atomic radii and name files on load residue file"));
  ...
  rightside->setContentsMargins(margin, margin, margin, margin);
  {
    ...
    // j++;
    // rightside->addWidget( lbl_threshold , j , 0 , 1 + ( j ) - ( j ) , 1 + ( 1
    // ) - ( 0 ) ); j++; rightside->addWidget(lbl_covolume, j, 0);
...
void US_Hydrodyn_Misc::cancel() { close(); }
