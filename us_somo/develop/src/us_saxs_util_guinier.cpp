...
void US_Saxs_Util::linear_fit(vector<double> x, vector<double> y, double &a,
...
void US_Saxs_Util::linear_fit(vector<double> x, vector<double> y,
                              vector<double> e, double &a, double &b,
...
bool US_Saxs_Util::guinier_fit(
    ...
    double &smax, double &smin, double &sRgmin, double &sRgmax, bool compute_Rc,
    bool compute_Rt) {
  vector<double> x;
  vector<double> y;
  errormsg = "";
  ...
  if (wave[tag].q.size() <= endpos) {
    errormsg = QString(
                   "selected end point %1 is more than the number of points "
                   "available (%2)")
                   .arg(endpos + 1)
                   .arg(wave[tag].q.size());
    return false;
  ...
  if (wave[tag].s.size() && wave[tag].s.size() == wave[tag].q.size() &&
    ...
    //          .arg( wave[ tag ].s.size() )
    //          .arg( wave[ tag ].s.size() == wave[ tag ].q.size() ? "equals q
    //          size" : "not equal q size" ) .arg( is_nonzero_vector( wave[ tag
    //          ].s ) ? "is non zero" : "is not nonzero" );
    linear_fit(x, y, a, b, siga, sigb, chi2);
  }

  /*
  cout << QString( "last linear fit: a %1 b %2 siga %3 sigb %4 ndf %5 chi2 %6\n"
  ) .arg( a ) .arg( b ) .arg( siga ) .arg( sigb ) .arg( x.size() - 2 ) .arg(
  chi2 ) .toLatin1().data()
     ;
  */
  ...
  log += QString("").sprintf(
      "|| %u || %u || %g || %g || %g || %g || %g || %g || %g || %g || %g || %g "
      "|| %g ||\n",
      startpos + 1, endpos + 1, smin, smax, sRgmin, sRgmax, Rg, I0, a, b, siga,
      sigb, chi2);
  log += QString(
...
bool US_Saxs_Util::guinier_fit2(
  ...
  for (unsigned int i = 0; i < wave[tag].q.size() - pointsmin; i++) {
    for (unsigned int j = i + pointsmin - 1;
      ...
      if (!guinier_fit(mylog, tag, i, j, a, b, siga, sigb, chi2, Rg, I0, smin,
                       smax, sRgmin, sRgmax, compute_Rc, compute_Rt)) {
        // log += mylog;
  ...
  if (!guinier_fit(log, tag, beststart, bestend, a, b, siga, sigb, chi2, Rg, I0,
                   smin, smax, sRgmin, sRgmax, compute_Rc, compute_Rt)) {
    return false;
  }
...
bool US_Saxs_Util::guinier_plot(QString outtag, QString tag) {
  ...
  for (unsigned int i = 0; i < wave[tag].q.size(); i++) {
    ...
    wave[outtag].s.push_back(use_errors && wave[tag].r[i] > 0e0
                                 ?
                                 // wave[ outtag ].r.back() * ( wave[ tag ].s[ i
                                 // ] / wave[ tag ].r[ i ] ) : 0e0 );
...
bool US_Saxs_Util::guinier_remove_points(QString outtag, QString tag,
                                         map<double, double> &removed,
                                         unsigned int &pts_removed,
                                         unsigned int &startpos,
                                         unsigned int &endpos, double a,
                                         double b, double sd_limit) {
  ...
  for (unsigned int i = startpos; i <= endpos; i++) {
    ...
    */
    ...
    if (fabs(a + b * wave[tag].q[i] - wave[tag].r[i]) <
      ...
      // wave[ tag ].q[ i ] ).arg( wave[ tag ].r[ i ] ).arg( pts_removed ).arg(
      // new_endpos );
    }
  }
...
bool US_Saxs_Util::guinier_fit_with_removal(
    ...
    unsigned int startpos, unsigned int endpos, double &a, double &b,
    double &siga, double &sigb, double &chi2, double &Rg, double &I0,
    double &smax, double &smin, double &sRgmin, double &sRgmax, double sd_limit,
    map<double, double> &removed, unsigned int &pts_removed, bool compute_Rc,
    bool compute_Rt) {
  ...
  do {
    ...
    if (!guinier_fit(log, tag, startpos, endpos, a, b, siga, sigb, chi2, Rg, I0,
                     smax, smin, sRgmin, sRgmax, compute_Rc, compute_Rt)) {
      return false;
    }

    if (!guinier_remove_points(outtag, tag, removed, this_pts_removed, startpos,
                               endpos, a, b, sd_limit)) {
      return false;
    }
