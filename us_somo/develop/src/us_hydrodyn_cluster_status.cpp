...
US_Hydrodyn_Cluster_Status::US_Hydrodyn_Cluster_Status(void *us_hydrodyn,
  ...
  if (cluster_id.isEmpty() || cluster_pw.isEmpty()) {
    QMessageBox::information(this, us_tr("US-SOMO: Cluster Jobs"),
                             us_tr("Cluster credentials must be set in Cluster "
  ...
  if (!update_files(false)) {
    QMessageBox::information(this, us_tr("US-SOMO: Cluster Status"),
                             us_tr("No unretrieved submitted jobs found"), 0);
  }
...
void US_Hydrodyn_Cluster_Status::setupGUI() {
  ...
  lv_files->setHeaderLabels(QStringList() << us_tr("Name") << us_tr("Status")
  ...
  if (QMessageBox::question(
    ...
    for (map<QTreeWidgetItem *, QString>::iterator it = jobs.begin();
      ...
      if (!QFile::remove(it->first->text(0))) {
        editor_msg("red",
                   QString(us_tr("Error removing %1")).arg(it->first->text(0)));
      }
    }
    ...
    for (map<QTreeWidgetItem *, QString>::iterator it = jobs.begin();
      ...
      editor_msg("black", QString(us_tr("Pushing back to unsubmitted %1"))
                              .arg(it->first->text(0)));
  ...
  for (map<QTreeWidgetItem *, QString>::iterator it = jobs.begin();
    ...
    if (!it->first->text(2).contains(us_tr("Error"))) {
      ...
      if (!ufu.move(it->first->text(0), completed_dir + SLASH)) {
        editor_msg("red", QString(us_tr("Error moving job to completed %1: %2"))
                              .arg(it->first->text(0))
                              .arg(ufu.errormsg));
      }
    } else {
...
void US_Hydrodyn_Cluster_Status::get_next_retrieve() {
  ...
  // editor_msg( "black", QString( "retrieving %1" ).arg( next_to_process->text(
  // 0 ) ) );
...
bool US_Hydrodyn_Cluster_Status::schedule_retrieve(QString file) {
  ...
  */
    ...
    if (previously_retrieved.size()) {
      switch (QMessageBox::question(
          ...
          QString(us_tr("Results for %1 already exist\n"
              ...
              .arg(next_to_process->text(0)),
          us_tr("&Rename previous results"), us_tr("&Delete previous results"),
          us_tr("&Stop"), 0)) {
        case 0: {
          ...
          editor_msg(
              "dark red",
              QString(
                  us_tr("Notice: previous results files renamed to %1-%2_out"))
  ...
  QString url =
      QString("/jobresults?") + QString("&user=%1").arg(cluster_id) +
...
void US_Hydrodyn_Cluster_Status::system_proc_finished(int,
                                                      QProcess::ExitStatus) {
  ...
  next_to_process->setText(2, retrieved.size()
...
void US_Hydrodyn_Cluster_Status::http_error(
...
void US_Hydrodyn_Cluster_Status::http_uploadProgress(qint64 done,
                                                     qint64 total) {
  // cout << "http: uploadProgress " << done << " " << total << "\n";
  progress->setValue(100.0 * (double)done / (double)total);
}

void US_Hydrodyn_Cluster_Status::http_downloadProgress(qint64 done,
                                                       qint64 total) {
...
void US_Hydrodyn_Cluster_Status::http_retrieve_error(
    QNetworkReply::NetworkError /* code */) {
...
void US_Hydrodyn_Cluster_Status::http_retrieve_uploadProgress(qint64 done,
                                                              qint64 total) {
  qDebug() << "http_retrieve_uploadProgress count " << done << " " << total
           << "\n";
}

void US_Hydrodyn_Cluster_Status::http_retrieve_downloadProgress(qint64 done,
                                                                qint64 total) {
