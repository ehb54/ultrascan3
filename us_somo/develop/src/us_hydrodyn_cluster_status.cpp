...
US_Hydrodyn_Cluster_Status::US_Hydrodyn_Cluster_Status (void *us_hydrodyn,
                                                        QWidget *p,
                                                        const char *)
    : QDialog (p)
{
  ...
  cluster_id
      = ((US_Hydrodyn_Cluster *)cluster_window)->cluster_config["userid"];
  cluster_pw
      = ((US_Hydrodyn_Cluster *)cluster_window)->cluster_config["userpw"];

  manage_url
      = ((US_Hydrodyn_Cluster *)cluster_window)->cluster_config["manage"];
  ...
  // submit_url      = ((US_Hydrodyn_Cluster *) cluster_window
  // )->cluster_config[ "server" ]; submit_url_host = submit_url;
  ...
  // stage_url_path += QString( "%1%2%3" ).arg( QDir::separator() ).arg(
  // cluster_id ).arg( QDir::separator() );

  if (cluster_id.isEmpty () || cluster_pw.isEmpty ())
    {
      QMessageBox::information (this, us_tr ("US-SOMO: Cluster Jobs"),
                                us_tr ("Cluster credentials must be set in "
                                       "Cluster Config before continuing"),
                                0);
      return;
    }
  ...
  if (!update_files (false))
    {
      QMessageBox::information (this, us_tr ("US-SOMO: Cluster Status"),
...
unsigned int
US_Hydrodyn_Cluster_Status::update_files (bool set_lv_files)
{
...
void
US_Hydrodyn_Cluster_Status::setupGUI ()
{
  int minHeight1 = 30;
  ...
  AUTFBACK (lv_files);
  lv_files->setFont (QFont (USglobal->config_list.fontFamily,
                            USglobal->config_list.fontSize + 1, QFont::Bold));
  lv_files->setEnabled (true);
  lv_files->setSelectionMode (QAbstractItemView::MultiSelection);

  lv_files->setColumnCount (4);
  lv_files->setHeaderLabels (
      QStringList () << us_tr ("Name") << us_tr ("Status")
                     << us_tr ("Additional Info") << us_tr ("Date created"));
  ...
  connect (lv_files, SIGNAL (itemSelectionChanged ()),
           SLOT (update_enables ()));

  pb_refresh = new QPushButton (us_tr ("Refresh status"), this);
  pb_refresh->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  pb_remove->setFont (QFont (USglobal->config_list.fontFamily,
                             USglobal->config_list.fontSize + 1));
  pb_remove->setMinimumHeight (minHeight1);
  pb_remove->setPalette (PALET_PUSHB);
  connect (pb_remove, SIGNAL (clicked ()), SLOT (remove ()));

  pb_retrieve_selected
      = new QPushButton (us_tr ("Retrieve selected results"), this);
  pb_retrieve_selected->setFont (QFont (USglobal->config_list.fontFamily,
                                        USglobal->config_list.fontSize + 1));
  pb_retrieve_selected->setMinimumHeight (minHeight1);
  pb_retrieve_selected->setPalette (PALET_PUSHB);
  connect (pb_retrieve_selected, SIGNAL (clicked ()),
           SLOT (retrieve_selected ()));

  pb_retrieve
      = new QPushButton (us_tr ("Retrieve all available results"), this);
  pb_retrieve->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize + 1));
  ...
  QHBoxLayout *hbl_buttons1 = new QHBoxLayout ();
  hbl_buttons1->setContentsMargins (0, 0, 0, 0);
  hbl_buttons1->setSpacing (0);
...
void
US_Hydrodyn_Cluster_Status::cancel ()
{
  if (comm_active)
    {
      http_reply->abort ();
...
void
US_Hydrodyn_Cluster_Status::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Cluster_Status::closeEvent (QCloseEvent *e)
{
  if (comm_active)
    {
      http_reply->abort ();
...
void
US_Hydrodyn_Cluster_Status::clear_display ()
{
  editor->clear ();
  editor->append ("\n\n");
}
...
void
US_Hydrodyn_Cluster_Status::update_font ()
{
  bool ok;
  QFont newFont;
...
void
US_Hydrodyn_Cluster_Status::save ()
{
  QString fn;
...
void
US_Hydrodyn_Cluster_Status::editor_msg (QString color, QString msg)
{
...
void
US_Hydrodyn_Cluster_Status::update_enables ()
{
  if (!disable_updates)
    {
      ...
      pb_refresh->setEnabled (!processing_active && !comm_active
                              && !system_proc_active);
      pb_remove->setEnabled (!processing_active && !comm_active
                             && !system_proc_active && any_selected);
      pb_retrieve_selected->setEnabled (!processing_active && !comm_active
                                        && !system_proc_active
                                        && any_selected_completed);
      pb_retrieve->setEnabled (!processing_active && !comm_active
                               && !system_proc_active && any_completed);
    }
...
void
US_Hydrodyn_Cluster_Status::refresh ()
{
  if (comm_active)
    {
      http_reply->abort ();
...
void
US_Hydrodyn_Cluster_Status::remove ()
{
  cancel_selected ();
...
void
US_Hydrodyn_Cluster_Status::retrieve ()
{
  //   update_files();
  comm_mode = "retrieve";
  jobs.clear ();
...
void
US_Hydrodyn_Cluster_Status::retrieve_selected ()
{
  //   update_files();
  comm_mode = "retrieve";
  jobs.clear ();
...
void
US_Hydrodyn_Cluster_Status::cancel_selected ()
{
  update_enables ();
  comm_mode = "cancel";
  jobs.clear ();
...
void
US_Hydrodyn_Cluster_Status::get_status ()
{
  // update_enables();
  comm_mode = "status";
  jobs.clear ();
...
void
US_Hydrodyn_Cluster_Status::next_status ()
{
  if (stopFlag)
    {
  ...
  for (map<QTreeWidgetItem *, QString>::iterator it = jobs.begin ();
       it != jobs.end (); it++)
    {
      ...
      if (it->second == "get status" || it->second == "cancel job")
        {
...
void
US_Hydrodyn_Cluster_Status::complete_remove ()
{
  if (QMessageBox::question (
          this, us_tr ("US-SOMO: Cluster Status"),
          us_tr ("What do you want to do with the canceled jobs?"),
          us_tr ("&Push back to unsubmitted"), us_tr ("&Remove completely"),
          QString (), 0, 1))
    {
      ...
      for (map<QTreeWidgetItem *, QString>::iterator it = jobs.begin ();
           it != jobs.end (); it++)
        {
          ...
          editor_msg (
              "black",
              QString (us_tr ("Removing %1")).arg (it->first->text (0)));
          if (!QFile::remove (it->first->text (0)))
            {
              editor_msg ("red", QString (us_tr ("Error removing %1"))
                                     .arg (it->first->text (0)));
            }
        }
  ...
  else
    {
      ...
      for (map<QTreeWidgetItem *, QString>::iterator it = jobs.begin ();
           it != jobs.end (); it++)
        {
          ...
          editor_msg ("black",
                      QString (us_tr ("Pushing back to unsubmitted %1"))
                          .arg (it->first->text (0)));
          if (!ufu.move (it->first->text (0), pkg_dir + SLASH))
            {
              editor_msg (
                  "red",
                  QString (us_tr ("Error pushing back to unsubmitted %1: %2"))
                      .arg (it->first->text (0))
                      .arg (ufu.errormsg));
            }
        }
  ...
  if (!update_files ())
    {
      QMessageBox::information (
          this, us_tr ("US-SOMO: Cluster Status"),
          us_tr ("No further unretrieved submitted jobs found"), 0);
      close ();
      return;
    }
  refresh ();
...
void
US_Hydrodyn_Cluster_Status::complete_retrieve ()
{
      ...
      if (!it->first->text (2).contains (us_tr ("Error")))
        {
          ...
          editor_msg ("black", QString (us_tr ("Moving jobs to completed %1"))
                                   .arg (it->first->text (0)));
          if (!ufu.move (it->first->text (0), completed_dir + SLASH))
            {
              editor_msg (
                  "red",
                  QString (us_tr ("Error moving job to completed %1: %2"))
                      .arg (it->first->text (0))
                      .arg (ufu.errormsg));
            }
        }
  ...
  if (!allok)
    {
      QMessageBox::information (this, us_tr ("US-SOMO: Cluster Status"),
                                us_tr ("Some job's results are missing"), 0);
    }
...
void
US_Hydrodyn_Cluster_Status::get_next_retrieve ()
{
  ...
  // editor_msg( "black", QString( "retrieving %1" ).arg(
  // next_to_process->text( 0 ) ) );
...
bool
US_Hydrodyn_Cluster_Status::schedule_retrieve (QString file)
{
  qDebug () << "schedule_retrieve()";
  errormsg = "";
  if (!QDir::setCurrent (completed_dir))
    {
      errormsg = QString (us_tr ("retrieve: can not change to directory %1"))
                     .arg (completed_dir);
      return false;
    }
  ...
  {
     errormsg = QString( us_tr( "Error: can not determine system host for job
  %1" ) ).arg( next_to_process->text( 0 ) ); return false;
  }

  if ( !((US_Hydrodyn_Cluster *) cluster_window
  )->cluster_stage_to_system.count( job_hostname[ next_to_process->text( 0 ) ]
  ) )
  {
     errormsg = QString( us_tr( "Error: no configured hosts for job %1 running
  on %2" ) ) .arg( next_to_process->text( 0 ) ) .arg( job_hostname[
  next_to_process->text( 0 ) ] ); return false;
  }

  QString selected_system_name =  ((US_Hydrodyn_Cluster *) cluster_window
  )->cluster_stage_to_system[ job_hostname[ next_to_process->text( 0 ) ] ];
  ...
  cout <<
     ...
     if ( ((US_Hydrodyn_Cluster *)cluster_window)->cluster_systems.count(
  selected_system_name ) )
     {
     map < QString, QString > selected_system = ((US_Hydrodyn_Cluster
  *)cluster_window)->cluster_systems[ selected_system_name ]; if (
  selected_system.count( "ftp" ) )
     ...
     // stage_path      .replace( QRegExp( "^.*:" ), "" );
     // stage_url_path  += QString( "%1%2%3" ).arg( QDir::separator() ).arg(
  cluster_id ).arg( QDir::separator() ); ftp_url         = selected_system[
  "ftp" ]; ftp_url_host    = ftp_url; ftp_url_port    = ftp_url; ftp_url_host
  .replace( QRegExp( ":.*$" ), "" ); ftp_url_port    .replace( QRegExp( "^.*:"
  ), "" ); } else { errormsg = QString( us_tr( "The system %1 does not seem to
  have sufficient configuration information defined" ) ).arg(
  selected_system_name ); return false;
     }
     } else {
     errormsg = QString( us_tr( "The system %1 does not seem to have any
  information" ) ).arg( selected_system_name ); return false;
     }
  */
  ...
  // move any previously retrieved results
  {
    ...
    QDir qd;
    QStringList previously_retrieved = qd.entryList (
        QStringList ()
        << QString ("%1_out.t??")
               .arg (QString ("%1")
                         .arg (next_to_process->text (0))
    ...
    if (previously_retrieved.size ())
      {
        switch (QMessageBox::question (
            this, us_tr ("US-SOMO: Cluster Status: Retrieve Results"),
            QString (us_tr ("Results for %1 already exist\n"
                            "What to you want to do?"))
                .arg (next_to_process->text (0)),
            us_tr ("&Rename previous results"),
            us_tr ("&Delete previous results"), us_tr ("&Stop"), 0))
          {
          case 0:
            {
              ...
              do
                {
                  ...
                  for (unsigned int i = 0;
                       i < (unsigned int)previously_retrieved.size (); i++)
                    {
                      ...
                      cout << QString ("test base <%1> text num %2 ext <%3>\n")
                                  ...
                                  .arg (test_ext);
                      QString test_file
                          = test_base + QString ("-%1").arg (ext) + test_ext;
              ...
              for (unsigned int i = 0;
                   i < (unsigned int)previously_retrieved.size (); i++)
                {
                  ...
                  if (!qd2.rename (previously_retrieved[i], test_file))
                    {
                      errormsg
                          = QString (us_tr ("Error: failed renaming %1 to %2"))
              ...
              editor_msg (
                  "dark red",
                  QString (us_tr ("Notice: previous results files renamed to "
                                  "%1-%2_out"))
  ...
  retrieve_file_name += QString ("_out.%1").arg (
      retrieve_file_name.contains (QRegExp ("^(bfnb|bfnbpm|oned)_")) ? "tgz"
                                                                     : "tar");
  ...
  QString url = QString ("/jobresults?")
  ...
  http_request.setUrl (
      QUrl (QString ("http://%1%2").arg (manage_url).arg (url)));
  http_reply = http_access_manager->get (http_request);

  connect (http_reply, SIGNAL (downloadProgress (qint64, qint64)), this,
           SLOT (http_retrieve_downloadProgress (qint64, qint64)));
  connect (http_reply, SIGNAL (uploadProgress (qint64, qint64)), this,
           SLOT (http_retrieve_uploadProgress (qint64, qint64)));
  connect (http_reply, SIGNAL (finished ()), this,
           SLOT (http_retrieve_finished ()));
  connect (http_reply, SIGNAL (error (QNetworkReply::NetworkError)), this,
           SLOT (http_retrieve_error (QNetworkReply::NetworkError)));
  ...
  // get_file += QString( "_out.%1" ).arg( get_file.contains( QRegExp(
  // "^(bfnb|bfnbpm|oned)_" ) ) ? "tgz" : "tar" );
  ...
  // {
  //    errormsg = QString( us_tr( "stage: can not open file %1" ) ).arg( file
  //    ); delete ftp_file; ftp_file = ( QFile * ) 0; return false;
  // }

  // QString target_dir = QString( "%1" ).arg( next_to_process->text( 0 )
  // ).replace( QRegExp( "\\.(tar|tgz)$" ), "" );
  ...
  // ), "" ) ) .arg( QString( "%1_out.t??" ) .arg( QString( "%1" ).arg( file
  // ).replace( QRegExp( "\\.(tgz|tar|TGZ|TAR)$" ), "" ) ) );
  //  cmd << QString( "." );
...
void
US_Hydrodyn_Cluster_Status::get_next_status ()
{
  if (comm_mode == "cancel")
    {
  ...
  if (comm_mode == "status")
    {
      // editor_msg( "black", QString( "refreshing status for %1" ).arg(
      // next_to_process->text( 0 ) ) );
...
bool
US_Hydrodyn_Cluster_Status::send_http_get (QString file)
{
  ...
  // connect( &submit_http, SIGNAL( stateChanged ( int ) ), this, SLOT(
  // http_stateChanged ( int ) ) ); connect( &submit_http, SIGNAL(
  // responseHeaderReceived ( const QHttpResponseHeader & ) ), this, SLOT(
  // http_responseHeaderReceived ( const QHttpResponseHeader & ) ) ); connect(
  // &submit_http, SIGNAL( readyRead ( const QHttpResponseHeader & ) ), this,
  // SLOT( http_readyRead ( const QHttpResponseHeader & ) ) ); connect(
  // &submit_http, SIGNAL( dataSendProgress ( int, int ) ), this, SLOT(
  // http_dataSendProgress ( int, int ) ) ); connect( &submit_http, SIGNAL(
  // dataReadProgress ( int, int ) ), this, SLOT( http_dataReadProgress ( int,
  // int ) ) ); connect( &submit_http, SIGNAL( requestStarted ( int ) ), this,
  // SLOT( http_requestStarted ( int ) ) ); connect( &submit_http, SIGNAL(
  // requestFinished ( int, bool ) ), this, SLOT( http_requestFinished ( int,
  // bool ) ) ); connect( &submit_http, SIGNAL( done ( bool ) ), this, SLOT(
  // http_done ( bool ) ) );
  ...
  if (comm_mode == "status")
    {
      ...
      //                  .arg( cluster_id )
      //                  .arg( file ) );
      url = QString ("/jobstatus?") + QString ("&user=%1").arg (cluster_id)
            + QString ("&pw=%1").arg (cluster_pw)
            + QString ("&_uuid=%1")
                  .arg (QString ("%1-%2").arg (cluster_id).arg (file));
      ...
      connect (http_reply, SIGNAL (downloadProgress (qint64, qint64)), this,
               SLOT (http_downloadProgress (qint64, qint64)));
      connect (http_reply, SIGNAL (uploadProgress (qint64, qint64)), this,
               SLOT (http_uploadProgress (qint64, qint64)));
      connect (http_reply, SIGNAL (finished ()), this,
               SLOT (http_finished ()));
      connect (http_reply, SIGNAL (error (QNetworkReply::NetworkError)), this,
               SLOT (http_error (QNetworkReply::NetworkError)));
    }
  ...
  if (comm_mode == "cancel")
    {
      ...
      //                  .arg( cluster_id )
      //                  .arg( file ) );
      url = QString ("/jobcancel?") + QString ("&user=%1").arg (cluster_id)
            + QString ("&pw=%1").arg (cluster_pw)
            + QString ("&_uuid=%1")
                  .arg (QString ("%1-%2").arg (cluster_id).arg (file));
...
bool
US_Hydrodyn_Cluster_Status::system_cmd (QStringList cmd)
{
  errormsg = "";
  ...
  connect (system_proc, SIGNAL (readyReadStandardOutput ()), this,
           SLOT (system_proc_readFromStdout ()));
  connect (system_proc, SIGNAL (readyReadStandardError ()), this,
           SLOT (system_proc_readFromStderr ()));
  connect (system_proc, SIGNAL (finished (int, QProcess::ExitStatus)), this,
           SLOT (system_proc_finished (int, QProcess::ExitStatus)));
  connect (system_proc, SIGNAL (started ()), this,
           SLOT (system_proc_started ()));
...
void
US_Hydrodyn_Cluster_Status::system_proc_readFromStdout ()
{
  editor_msg ("brown", QString (system_proc->readAllStandardOutput ()));
}

void
US_Hydrodyn_Cluster_Status::system_proc_readFromStderr ()
{
  editor_msg ("red", QString (system_proc->readAllStandardError ()));
}

void
US_Hydrodyn_Cluster_Status::system_proc_finished (int, QProcess::ExitStatus)
{
  ...
  // disconnect( system_proc, SIGNAL(finished( int, QProcess::ExitStatus )), 0,
  // 0);
  ...
  QDir::setCurrent (completed_dir);
  QDir qd;
  QStringList retrieved = qd.entryList (
      QStringList ()
      << QString ("%1_out.t??")
             .arg (QString ("%1")
                       .arg (next_to_process->text (0))
  ...
  next_to_process->setText (
      2, retrieved.size () ? QString (us_tr ("Retrieved %1 result file%2"))
                                 .arg (retrieved.size ())
...
void
US_Hydrodyn_Cluster_Status::system_proc_started ()
{
...
void
US_Hydrodyn_Cluster_Status::stop ()
{
  stopFlag = true;
  pb_stop->setEnabled (false);
  if (comm_active)
...
void
US_Hydrodyn_Cluster_Status::http_error (QNetworkReply::NetworkError /* code */)
{
  qDebug () << "http: error";
...
void
US_Hydrodyn_Cluster_Status::http_uploadProgress (qint64 done, qint64 total)
{
...
void
US_Hydrodyn_Cluster_Status::http_downloadProgress (qint64 done, qint64 total)
{
...
void
US_Hydrodyn_Cluster_Status::http_done (bool error)
{
...
void
US_Hydrodyn_Cluster_Status::http_finished ()
{
...
void
US_Hydrodyn_Cluster_Status::http_retrieve_error (
...
void
US_Hydrodyn_Cluster_Status::http_retrieve_uploadProgress (qint64 done,
                                                          qint64 total)
{
...
void
US_Hydrodyn_Cluster_Status::http_retrieve_downloadProgress (qint64 done,
                                                            qint64 total)
{
  ...
  if (retrieve_file->write (qba) != qba.size ())
    {
      current_http_error
          = QString ("error writing file %1").arg (retrieve_file->fileName ());
      http_reply->abort ();
      http_retrieve_done (true);
    }
}
...
void
US_Hydrodyn_Cluster_Status::http_retrieve_done (bool error)
{
...
void
US_Hydrodyn_Cluster_Status::http_retrieve_finished ()
{
