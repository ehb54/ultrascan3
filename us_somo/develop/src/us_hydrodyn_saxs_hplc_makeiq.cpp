...
bool US_Hydrodyn_Saxs_Hplc::create_i_of_q_ng(set<QString> &fileset,
...
bool US_Hydrodyn_Saxs_Hplc::create_i_of_q_ng(QStringList files, double t_min,
                                             double t_max) {
  ...
  if (!mode_testiq) {
    ...
    if (use_conc) {
      ...
      if (saxs_hplc_param_g_conc && saxs_hplc_param_g_psv) {
        ...
        parameters["istarq_ok"] = "true";
        parameters["istarq_message"] =
            QString(us_tr("Make I*(q) using the globally defined concentration "
                          "%1 [mg/mL] and psv %2 [mL/g]?"))
  ...
  for (unsigned int t = 0; t < tv.size(); t++) {
      ...
      if (!usu->apply_natural_spline(conc_spline_x, conc_spline_y,
        ...
        if (istarq_mode == ISTARQ_CONC_POINTWISE) {
          ...
          istarq_norm_factor =
              AVOGADRO /
              (conc_factor * 1e-3 *
               pow(saxs_hplc_param_diffusion_len *
                       (1e0 / (saxs_hplc_param_electron_nucleon_ratio *
    ...
    if (istarq_mode == ISTARQ_CONC_GLOBAL) {
      istarq_norm_factor =
          AVOGADRO / (saxs_hplc_param_g_conc * 1e-3 *
                      pow(saxs_hplc_param_diffusion_len *
                              (1e0 / (saxs_hplc_param_electron_nucleon_ratio *
                                      saxs_hplc_param_nucleon_mass) -
...
bool US_Hydrodyn_Saxs_Hplc::create_i_of_q(set<QString> &fileset, double t_min,
                                          double t_max) {
  QStringList files;
  for (set<QString>::iterator it = fileset.begin(); it != fileset.end(); ++it) {
    files << *it;
...
bool US_Hydrodyn_Saxs_Hplc::create_i_of_q(QStringList files, double t_min,
                                          double t_max) {
  // us_qdebug( "ciq 0" );
  // for each selected file
  // extract q grid from file names
  ...
  if (mode_testiq) {
    ...
    if (no_conc) {
      ...
      parameters["istarq_message"] = us_tr(
          "Make I*(q) not available: a concentration curve must be set when "
          "Gaussians are defined");
      ...
      //    istarq_mode                    = ISTARQ_NONE;
      //    parameters[ "istarq_ok" ]      = "false";
      //    parameters[ "istarq_message" ] = us_tr( "Make I*(q) not available:
      //    no concentration nor psv data available" );
      // }
    } else {
      istarq_mode = ISTARQ_CONC_POINTWISE;
      parameters["istarq_ok"] = "true";
  ...
  for (unsigned int t = 0; t < tv.size(); t++) {
    ...
    for (unsigned int g = 0; g < num_of_gauss; g++) {
      ...
      if ((conc_ok || normalize_by_conc) && concs[g][t] > 0e0) {
        ...
        //             cout << QString( "peak %1 center %2 fwhm %3 t %4 tv[t]
        //             %5\n" )
        //                .arg( g + 1 )
        //                .arg( center )
        //                .arg( fwhm )
    ...
    for (unsigned int i = 0; i < (unsigned int)avg_peaks_names.size(); ++i) {
      if (avg_peaks_names[i].size()) {
        ...
        if (conc_ok && istarq_mode == ISTARQ_NONE) {
          ...
          // last_selected.insert( lb_files->item( lb_files->count() - 1
          // )->text() ); set_selected( last_selected ); normalize();
...
void US_Hydrodyn_Saxs_Hplc::add_plot_gaussian(const QString &file,
                                              const QString &tag) {
  add_plot(QString("%1-%2").arg(file).arg(tag), f_qs[file],
           compute_gaussian_sum(f_qs[file], f_gaussians[file]), true, false);
}

bool US_Hydrodyn_Saxs_Hplc::compute_f_gaussians_trial(
    QString file, QWidget *hplc_fit_widget) {
...
void US_Hydrodyn_Saxs_Hplc::add_smoothed(const QString &name,
...
void US_Hydrodyn_Saxs_Hplc::add_oldstyle(const QString &name,
                                         const vector<double> &q,
                                         const vector<double> &I) {
...
bool US_Hydrodyn_Saxs_Hplc::create_ihashq(set<QString> &fileset, double t_min,
                                          double t_max) {
  QStringList files;
  for (set<QString>::iterator it = fileset.begin(); it != fileset.end(); ++it) {
    files << *it;
...
bool US_Hydrodyn_Saxs_Hplc::create_ihashq(QStringList files, double t_min,
                                          double t_max) {
  ...
  // 1st verify parameters
  {
    ...
    switch (QMessageBox::question(
        ...
        qsl.join("") + QString(us_tr("<hr>"
                                     "Accept these processing parameters?"
                                     "<br>"
                                     "<hr>"
                                     "<i>Adjust values in Options->SAXS "
                                     "Processing Parameters</i>")))) {
  ...
  // exposure times
  {
    ...
    form.addRow(
        new QLabel(us_tr("Convert frames to time\n"
    ...
    vector<QString> labels = {us_tr("Starting time [s]:"),
    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      for (auto &frame : frames) {
        frame = QString("%1").arg(
            start_time + frame.toDouble() * (exposure_time + frame_interval) +
            0.5 * exposure_time);
      }
    }
  }
  ...
  // i0 normalization
  {
    ...
    do {
      ...
      vector<double> defaults = {saxs_hplc_param_I0_exp,
      ...
      if (dialog.exec() == QDialog::Accepted) {
        ...
        if (fields[1]->text().toDouble() == 0) {
          ...
          i0_norm = fields[1]->text().toDouble() / fields[0]->text().toDouble();
        }
      }
    } while (try_again);
  }
...

  {
    ...
    do {
      ...
      // Add some text above the fields
      form.addRow(
          new QLabel(us_tr("Enter a concentration to produce I*(q)\n"
