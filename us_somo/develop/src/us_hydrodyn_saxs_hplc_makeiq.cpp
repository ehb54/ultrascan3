...
bool
US_Hydrodyn_Saxs_Hplc::create_i_of_q_ng (set<QString> &fileset, double t_min,
                                         double t_max)
{
  QStringList files;
...
bool
US_Hydrodyn_Saxs_Hplc::create_i_of_q_ng (QStringList files, double t_min,
                                         double t_max)
{
      ...
      if (use_conc)
        {
          ...
          parameters["hplc_param_I0_exp"]
              = QString ("%1").arg (saxs_hplc_param_I0_exp);
          ...
          parameters["istarq_message"]
              = QString (
                    us_tr ("Make I*(q) using the concentration curve %1 ?"))
                    .arg (lbl_conc_file->text ());

          US_Hydrodyn_Saxs_Hplc_Ciq *hplc_ciq
              = new US_Hydrodyn_Saxs_Hplc_Ciq (this, &parameters, this);
        ...
        {
          if (saxs_hplc_param_g_conc && saxs_hplc_param_g_psv)
            {
              istarq_mode = ISTARQ_CONC_GLOBAL;
              parameters["istarq_ok"] = "true";
              parameters["istarq_message"]
                  = QString (
                        us_tr ("Make I*(q) using the globally defined "
                               "concentration %1 [mg/mL] and psv %2 [mL/g]?"))
      ...
      if (istarq_mode == ISTARQ_CONC_GLOBAL)
        {
          istarq_norm_factor
              = AVOGADRO
                / (saxs_hplc_param_g_conc * 1e-3
                   * pow (
                       saxs_hplc_param_diffusion_len
                           * (1e0
                                  / (saxs_hplc_param_electron_nucleon_ratio
                                     * saxs_hplc_param_nucleon_mass)
                              - (saxs_hplc_param_g_psv * 1e24
                                 * saxs_hplc_param_solvent_electron_density)),
                       2e0));
      ...
      if (mode_testiq)
      ...
      else
        {
          ...
          if (istarq_mode == ISTARQ_CONC_GLOBAL)
            {
              // TSO <<
              //    QString( "ISTARQ_CONC_GLOBAL name %1 conc %2 psv%3 I0se
              //    %4\n" ) .arg( name ) .arg( saxs_hplc_param_g_conc ) .arg(
              //    saxs_hplc_param_g_psv ) .arg( I0se )
              //    ;
...
bool
US_Hydrodyn_Saxs_Hplc::create_i_of_q (set<QString> &fileset, double t_min,
                                      double t_max)
{
  QStringList files;
  for (set<QString>::iterator it = fileset.begin (); it != fileset.end ();
...
bool
US_Hydrodyn_Saxs_Hplc::create_i_of_q (QStringList files, double t_min,
                                      double t_max)
{
  // us_qdebug( "ciq 0" );
  // for each selected file
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
    {
      ...
      if (rx_bi.indexIn (files[i]) == -1)
      ...
      else
        {
          ...
          // ).toDouble() ); cout << QString( "bi for file %1 delta  %2 alpha
  ...
  if (mode_testiq)
  ...
  else
    {
      ...
      if (no_conc)
        {
          ...
          parameters["istarq_message"]
              = us_tr ("Make I*(q) not available: a concentration curve must "
                       "be set when Gaussians are defined");
          ...
          //    parameters[ "istarq_ok" ]      = "true";
          //    parameters[ "istarq_message" ] =
          //       QString( us_tr( "Make I*(q) using the globally defined
          //       concentration %1 [mg/mL] and psv %2 [mL/g]?" ) ) .arg(
          //       saxs_hplc_param_g_conc ) .arg( saxs_hplc_param_g_psv )
          //       ;
          // } else {
          //    istarq_mode                    = ISTARQ_NONE;
          //    parameters[ "istarq_ok" ]      = "false";
          //    parameters[ "istarq_message" ] = us_tr( "Make I*(q) not
          //    available: no concentration nor psv data available" );
          // }
        }
      else
        {
      ...
      // TSO << US_Vector::qs_mapqsqs( "hplc_ciq parameters before ciq",
      // parameters );
      ...
      hplc_ciq->exec ();
      delete hplc_ciq;
      // TSO << US_Vector::qs_mapqsqs( "hplc_ciq parameters after ciq",
      // parameters );
          ...
          if (istarq_mode != ISTARQ_NONE)
            {
              editor_msg (
                  "red",
                  "make istarq not implemented without gaussians yet\n");
              progress->reset ();
              update_enables ();
              return false;
            }
    ...
    double cutoff_frac
        = ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_cb_makeiq_cutmax_pct"]
              ...
              ? ((US_Hydrodyn *)us_hydrodyn)
                        ->gparams["hplc_makeiq_cutmax_pct"]
                        .toDouble ()
                    * 0.01
              : 0e0;
      ...
      for (unsigned int g = 0; g < num_of_gauss; g++)
        {
          ...
          if ((conc_ok || normalize_by_conc) && concs[g][t] > 0e0)
            {
              ...
              //             cout << QString( "peak %1 center %2 fwhm %3 t %4
              //             tv[t] %5\n" )
              //                .arg( g + 1 )
              //                .arg( center )
              //                .arg( fwhm )
              ...
              if (fs[i][t] == 0e0)
                {
                  if (!reported_gs0)
                    {
                      cout << QString ("Notice: file %1 t %2 gaussian sum is "
          ...
          if (normalize_by_conc && istarq_mode == ISTARQ_NONE
              && norm_factor != 1e0)
            {
              ...
              if (conc_ok && istarq_mode == ISTARQ_NONE)
                {
                  ...
                  // last_selected.insert( lb_files->item( lb_files->count() -
                  // 1 )->text() ); set_selected( last_selected ); normalize();
...
bool
US_Hydrodyn_Saxs_Hplc::create_unified_ggaussian_target (bool do_init,
...
bool
US_Hydrodyn_Saxs_Hplc::create_unified_ggaussian_target (QStringList &files,
                                                        bool do_init,
                                                        bool only_init_unset)
{
  ...
  if (do_init)
    {
      ...
      if (((US_Hydrodyn *)us_hydrodyn)->gparams.count ("hplc_cb_gg_smooth")
          ...
          && ((US_Hydrodyn *)us_hydrodyn)->gparams["hplc_gg_smooth"].toInt ()
                 > 0)
        {
          editor_msg (
              "darkred",
...
bool
US_Hydrodyn_Saxs_Hplc::ggauss_recompute ()
{
...
bool
US_Hydrodyn_Saxs_Hplc::compute_f_gaussians (QString file,
                                            QWidget *hplc_fit_widget)
{
  ...
  // find peak of curve
  // US_Hydrodyn_Saxs_Hplc_Fit *fit = (US_Hydrodyn_Saxs_Hplc_Fit
  // *)hplc_fit_widget;
...
bool
US_Hydrodyn_Saxs_Hplc::initial_ggaussian_fit (QStringList &files,
                                              bool only_init_unset)
{
  wheel_file = files[0];
  ...
  US_Hydrodyn_Saxs_Hplc_Fit *hplc_fit_window
      = new US_Hydrodyn_Saxs_Hplc_Fit (this, false, this);
...
void
US_Hydrodyn_Saxs_Hplc::add_plot_gaussian (const QString &file,
                                          const QString &tag)
{
...
bool
US_Hydrodyn_Saxs_Hplc::compute_f_gaussians_trial (QString file,
                                                  QWidget *hplc_fit_widget)
{
  ...
  // cout << QString( "cfg: %1 org_gaussian peak %2, curve peak %3, scaling %4
  // new gaussian peak %5\n" )
  //    .arg( file )
  //    .arg( gmax )
  //    .arg( peak )
  ...
  US_Hydrodyn_Saxs_Hplc_Fit *fit
      = (US_Hydrodyn_Saxs_Hplc_Fit *)hplc_fit_widget;
...
void
US_Hydrodyn_Saxs_Hplc::clear_smoothed ()
{
  f_qs_smoothed.clear ();
  f_Is_smoothed.clear ();
...
void
US_Hydrodyn_Saxs_Hplc::list_smoothed ()
{
  TSO << "list_smoothed():\n";
...
void
US_Hydrodyn_Saxs_Hplc::add_smoothed (const QString &name,
...
void
US_Hydrodyn_Saxs_Hplc::clear_oldstyle ()
{
  f_qs_oldstyle.clear ();
  f_Is_oldstyle.clear ();
}
...
void
US_Hydrodyn_Saxs_Hplc::list_oldstyle ()
{
  TSO << "list_oldstyle():\n";
...
void
US_Hydrodyn_Saxs_Hplc::add_oldstyle (const QString &name,
                                     const vector<double> &q,
                                     const vector<double> &I)
{
...
bool
US_Hydrodyn_Saxs_Hplc::create_ihashq (set<QString> &fileset, double t_min,
                                      double t_max)
{
  QStringList files;
  for (set<QString>::iterator it = fileset.begin (); it != fileset.end ();
...
void
US_Hydrodyn_Saxs_Hplc::create_ihashq ()
{
  disable_all ();
...
bool
US_Hydrodyn_Saxs_Hplc::create_ihashq (QStringList files, double t_min,
                                      double t_max)
{
  ...
  // 1st verify parameters
  {
    vector<QString> labels
        = { "Partial specific volume [mL/g]:", "Diffusion length [cm]:",
            "Electron/nucleon ratio Z/A:", "Nucleon mass [g]:",
            "Solvent electron density [e A^-3]:" };

    vector<double> values
        = { saxs_hplc_param_g_psv, saxs_hplc_param_diffusion_len,
            saxs_hplc_param_electron_nucleon_ratio,
            saxs_hplc_param_nucleon_mass,
            saxs_hplc_param_solvent_electron_density };

    QStringList qsl;
    qsl << "<b>SAXS Processing parameters</b>"
           "<hr>"
           "<table border=1 bgcolor=#FFF cellpadding=1.5>";

    for (int i = 0; i < (int)labels.size (); ++i)
      {
        qsl << QString ("<tr><td>%1</td><td>%2</td></tr>")
                   .arg (us_tr (labels[i]))
                   .arg (values[i]);
      }
    ...
    switch (QMessageBox::question (
        this, windowTitle () + us_tr (" : Make I#,I*(q)"),
        qsl.join ("")
            + QString (us_tr ("<hr>"
                              "Accept these processing parameters?"
                              "<br>"
                              "<hr>"
                              "<i>Adjust values in Options->SAXS Processing "
                              "Parameters</i>"))))
      {
  ...
  // exposure times
  {
    ...
    vector<QString> labels
        = { us_tr ("Starting time [s]:"), us_tr ("Exposure time [s]:"),
            us_tr ("Frame interval [s]:") };

    vector<double> defaults = { 0, 0, 1 };
  ...
  for (auto &frame : frames)
    {
      ...
      if (signbit (frame.toDouble ()))
      ...
      else
        {
          frame = QString ("t%1%2")
                      .arg (pad_zeros (
                          whole, (int)(frames.back ().toDouble () + .5) * 10))
                      .arg (QString ("%1").arg (frac).replace (
                          QRegularExpression ("^0"), ""));
        }
    }
    ...
    do
      {
        ...
        // Add some text above the fields
        form.addRow (new QLabel (
            us_tr ("Use I0 standards for normalization\n"
                   "Fill out the values below and click OK\n"
        ...
        vector<QString> labels = { us_tr ("I0 standard experimental [a.u.]:"),
                                   us_tr ("I0 standard theoretical [a.u.]:") };

        vector<double> defaults
            = { saxs_hplc_param_I0_exp, saxs_hplc_param_I0_theo };
        ...
        if (dialog.exec () == QDialog::Accepted)
          {
            ...
            if (fields[1]->text ().toDouble () == 0)
            ...
            else
              {
                ...
                i0_norm = fields[1]->text ().toDouble ()
    ...
    do
      {
        ...
        // Add some text above the fields
        form.addRow (new QLabel (
            us_tr ("Enter a concentration to produce I*(q)\n"
        ...
        vector<QString> labels = { us_tr ("Concentration [mg/mL]:") };
      ...
      = saxs_hplc_param_diffusion_len
        * (1e0
               / (saxs_hplc_param_electron_nucleon_ratio
                  * saxs_hplc_param_nucleon_mass)
           - saxs_hplc_param_g_psv
                 * (1e24 * saxs_hplc_param_solvent_electron_density));

  double I0mult = i0_norm * AVOGADRO / (conc_mult * 1e-3)
                  / (internal_contrast * internal_contrast);

  TSO << QString ("diffusion len     %1\n"
                  "electron nucleon  %2\n"
                  "nucleon mass      %3\n"
                  "psv               %4\n"
                  "solvent e density %5\n"
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size (); ++i)
    {
      ...
      QString hash_name
          = head + (istarq ? "_Istarq_" : "_Ihashq_") + frame + tail;
      ...
      f_e_nucleon_ratio[last_created_file]
          = saxs_hplc_param_electron_nucleon_ratio;
      f_nucleon_mass[last_created_file] = saxs_hplc_param_nucleon_mass;
      f_solvent_e_dens[last_created_file]
          = saxs_hplc_param_solvent_electron_density;
