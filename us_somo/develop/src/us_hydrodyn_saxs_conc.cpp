...
US_Hydrodyn_Saxs_Conc::US_Hydrodyn_Saxs_Conc (csv &csv1, void *saxs_window,
    ...
    : QDialog (p)
{
  ...
  setWindowTitle (us_tr ("US-SOMO: Guinier: Set Curve Concentration, PSV and "
                         "I0 standard experimental"));
...
void
US_Hydrodyn_Saxs_Conc::refresh (csv &ref_csv)
{
  csv new_csv = ref_csv;
...
void
US_Hydrodyn_Saxs_Conc::setupGUI ()
{
  int minHeight1 = 30;
...
#else
  connect (t_csv->verticalHeader (), SIGNAL (sectionClicked (int)),
           SLOT (row_header_released (int)));
#endif
  ...
  pb_reset_to_defaults
      = new QPushButton (us_tr ("Reset to defaults values"), this);
  pb_reset_to_defaults->setFont (QFont (USglobal->config_list.fontFamily,
                                        USglobal->config_list.fontSize + 1));
  pb_reset_to_defaults->setMinimumHeight (minHeight1);
  pb_reset_to_defaults->setPalette (PALET_PUSHB);
  connect (pb_reset_to_defaults, SIGNAL (clicked ()),
...
void
US_Hydrodyn_Saxs_Conc::cancel ()
{
  close ();
}

void
US_Hydrodyn_Saxs_Conc::set_ok ()
{
  *org_csv = current_csv ();
  close ();
}
...
void
US_Hydrodyn_Saxs_Conc::help ()
{
  US_Help *online_help;
  online_help = new US_Help (this);
  online_help->show_help ("manual/somo/somo_saxs_conc.html");
}

void
US_Hydrodyn_Saxs_Conc::closeEvent (QCloseEvent *e)
{
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept ();
...
void
US_Hydrodyn_Saxs_Conc::sort_column (int section)
{
...
csv
US_Hydrodyn_Saxs_Conc::current_csv ()
{
  csv tmp_csv = csv1;
...
void
US_Hydrodyn_Saxs_Conc::paste_all ()
{
  csv1 = current_csv ();
  unsigned int pos = 0;
  for (int i = 0; i < t_csv->rowCount (); i++)
...
void
US_Hydrodyn_Saxs_Conc::reset_to_defaults ()
{
  csv1 = current_csv ();
  for (int i = 0; i < t_csv->rowCount (); i++)
    {
      csv1.data[i][1] = QString ("%1").arg (
          ((US_Hydrodyn_Saxs *)saxs_window)->our_saxs_options->conc);
      csv1.data[i][2] = QString ("%1").arg (
          ((US_Hydrodyn_Saxs *)saxs_window)->our_saxs_options->psv);
      csv1.data[i][3] = QString ("%1").arg (
          ((US_Hydrodyn_Saxs *)saxs_window)->our_saxs_options->I0_exp);
...
void
US_Hydrodyn_Saxs_Conc::update_enables ()
{
  if (!disable_updates)
    {
...
void
US_Hydrodyn_Saxs_Conc::row_header_released (int row)
{
...
void
US_Hydrodyn_Saxs_Conc::reload_csv ()
{
  t_csv->setRowCount (csv1.data.size ());
  for (unsigned int i = 0; i < csv1.data.size (); i++)
    {
...
US_Hydrodyn_Saxs_Conc::load ()
{
  ...
  QString fname = QFileDialog::getOpenFileName (
      this, windowTitle (), use_dir, "Concentration files (*.sxc *.SXC)");
      ...
      for (unsigned int i = 0; i < (unsigned int)qsl_lines.size (); i++)
        {
          ...
          if (qsl.size () > 3 && our_files.count (qsl[0]))
            {
...
QString
US_Hydrodyn_Saxs_Conc::csv_to_qstring (csv from_csv)
{
  QString qs;
...
QStringList
US_Hydrodyn_Saxs_Conc::csv_parse_line (QString qs)
{
