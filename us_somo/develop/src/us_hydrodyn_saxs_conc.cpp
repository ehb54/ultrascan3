...
US_Hydrodyn_Saxs_Conc::US_Hydrodyn_Saxs_Conc(csv &csv1, void *saxs_window,
  ...
  setWindowTitle(
      us_tr("US-SOMO: Guinier: Set Curve Concentration, PSV and I0 standard "
            "experimental"));
...
void US_Hydrodyn_Saxs_Conc::cancel() { close(); }
...
QString US_Hydrodyn_Saxs_Conc::csv_to_qstring(csv from_csv) {
  ...
  for (unsigned int i = 0; i < from_csv.data.size(); i++) {
    for (unsigned int j = 0; j < from_csv.data[i].size(); j++) {
      qs +=
          QString("%1%2").arg(j ? "," : "").arg(from_csv.data[i][j].trimmed());
    }
    qs += "\n";
  }
