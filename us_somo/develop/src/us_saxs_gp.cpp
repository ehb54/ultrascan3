...
sgp_node::validate_params ()
{
  ...
  if (sgp_params.count ("branchmax") && sgp_params["branchmax"] < 1.0)
    {
      qs += QString ("Error: parameter 'branchmax' must be at least 1 "
                     "(currently set to %1)'\n")
  ...
  for (unsigned int i = 0; i < (unsigned int)params.size (); i++)
    {
      if (!sgp_params.count (params[i]))
        ...
        {
          qs += QString ("Parameter '%1' has prexisting value %2, NOT set to "
                         "default value of %3\n")
                    .arg (params[i])
...
void
sgp_node::random_normal (double &r1, double &r2, double mean1, double sd1,
...
point
sgp_node::cross (point p1, point p2) // p1 cross p2
{
  point result;
...
float
sgp_node::dot (point p1, point p2) // p1 dot p2
{
...
point
sgp_node::plus (point p1, point p2)
{
  point result;
...
point
sgp_node::scale (point p, float m)
{
...
QString
sgp_node::contents (bool include_children)
{
  QString qs;
...
sgp_node::contents_by_pos ()
{
  ...
  for (unsigned int i = 0; i < size (); i++)
    {
      qs += QString ("sgp: pos %1 %2 depth %3 distance %4 radius %5 normal "
                     "[%6,%7,%8]\n")
                .arg (i)
...
{
  sgp_node *tmp_node
      = new sgp_node (node->normal, node->distance, node->radius);
...
bool
sgp_node::insert_copy (unsigned int pos, sgp_node *node)
{
  if (pos >= size ())
    {
      return false;
      ...
      if (sgp_params["branchmax"] > 1)
        {
          ...
          while (new_sgp->ref (pos)->children.size ()
                 >= sgp_params["branchmax"]);
        }
      else
        {
          pos = i - 1;
...
bool
sgp_node::mutate (unsigned int pos)
{
  if (size () <= pos)
    {
      return false;
...
bool
sgp_node::crossover (sgp_node *&result, sgp_node *node1, sgp_node *node2)
{
...
void
sgp_node::check_normal (QString qs)
{
  for (unsigned int i = 0; i < size (); i++)
    {
...
sgp_node::test ()
{
  ...
  for (unsigned int i = 0; i < our_sgp->size (); i++)
    {
      cout << i << " : "
           << (our_sgp->ref (i) ? our_sgp->ref (i)->contents (false)
                                : "null pos\n");
    }
