...
void US_Hydrodyn_Mals_Saxs::invert_all_created() {
  disable_all();
...
void US_Hydrodyn_Mals_Saxs::remove_created() {
  disable_all();
  QStringList files;
...
void US_Hydrodyn_Mals_Saxs::add() {
  ...
  if (use_errors) {
    add_plot(name, f_qs[files[0]], sum, e,
             f_is_time.count(files[0]) ? f_is_time[files[0]] : false, false);
  } else {
...
bool US_Hydrodyn_Mals_Saxs::all_have_f_gaussians(QStringList &files) {
...
void US_Hydrodyn_Mals_Saxs::p3d() {
#if QT_VERSION < 0x040000
  disable_all();
  ...
  if (!ggaussian_compatible(files, false)) {
    editor_msg(
        ...
        cb_fix_width->isChecked()
            ? us_tr("NOTICE: Some files selected have Gaussians with varying "
                    "centers or widths or a different number of Gaussians or "
                    "centers or widths that do not match the last Gaussians")
            : us_tr("NOTICE: Some files selected have Gaussians with varying "
                    "centers or a different number of Gaussians or centers "
                    "that do not match the last Gaussians."));
    update_enables();
    return;
  }
  ...
  for (unsigned int i = 1; i < (unsigned int)files.size(); i++) {
    if (f_qs[files[0]] != f_qs[files[i]]) {
      editor_msg("red", QString(us_tr("Error: Incompatible grids (comparing %1 "
                                      "and %2). Suggest: Crop Common"))
                            .arg(files[0])
                            .arg(files[i]));
      update_enables();
      return;
  ...
  vector<double> q;
  {
    ...
    for (unsigned int i = 0; i < (unsigned int)files.size(); i++) {
      ...
      if (used_q.count(ql.back())) {
        editor_msg(
            "red",
            QString(us_tr("Error: Duplicate q value in file name for %1"))
                .arg(files[i]));
        update_enables();
        return;
      }
...

  {
    ...
    parameters["gaussians"] =
        QString("%1").arg(f_gaussians[files[0]].size() / gaussian_type_size);
  ...
  // plot 3d
  {
    ...
    if (plot3d_flag) {
      ...
      plot3d_window = new Mesh2MainWindow(&plot3d_flag, title, xtitle, ytitle,
                                          ztitle, data3d, &controlvar_3d);
    }
...
void US_Hydrodyn_Mals_Saxs::set_detector() {
  disable_all();

  {
    ...
    detector_uv =
        (parameters.count("uv") && parameters["uv"] == "true") ? true : false;
    detector_ri =
        (parameters.count("ri") && parameters["ri"] == "true") ? true : false;
...
/*
   ...
   ) ) .arg( qsl.join( "\n" ) ), us_tr( "&Save them now" ), us_tr( "&Remove them
   anyway" ), us_tr( "&Quit from removing files" ), 0, // Stop == button 0 0 //
   Escape == button 0 ) )
      {
...
bool US_Hydrodyn_Mals_Saxs::height_wheel_file(double &val, double pos) {
...
void US_Hydrodyn_Mals_Saxs::gauss_match_amplitude() {
  if (current_mode != MODE_GAUSSIAN) {
    return;
  }
...
bool US_Hydrodyn_Mals_Saxs::opt_repeak_gaussians(QString file) {
  if (!gaussians.size()) {
    return false;
  }
  ...
  if (scale < .5 || scale > 1.5) {
    switch (QMessageBox::warning(
        this, windowTitle(),
        QString(us_tr("Please note:\n\n"
                      "The current Gaussians should be scaled by %1 to be in "
                      "the range of this curve.\n"
                      "What would you like to do?\n"))
            .arg(scale),
        us_tr("&Rescale the Gaussian amplitudes"), us_tr("&Do not rescale"),
        QString(),
        0,  // Stop == button 0
        0   // Escape == button 0
        )) {
...
vector<double> US_Hydrodyn_Mals_Saxs::conc_curve(vector<double> &t,
                                                 unsigned int peak,
                                                 double conv) {
  ...
  if (conc_file.isEmpty()) {
    ...
    if (!f_gaussians.count(conc_file)) {
      editor_msg("red", us_tr("Internal error: conc_curve(): no Gaussians "
  ...
  if (!detector_uv && !detector_ri) {
    editor_msg("red",
               us_tr("Internal error: conc_curve(): No detector type set"));
    return result;
  }
...
void US_Hydrodyn_Mals_Saxs::adjacent() {
  QString match_name;
  int match_pos = 0;
  QStringList turn_on;
  ...
  if (match_name.contains("_bsub_a")) {
    ...
    rx.setPattern(
        QString("^%1")
            ...
            .replace(QRegExp("\\d+$"), "\\d+") +
        QString("%1$").arg(match_name).replace(QRegExp("^.*_bsub"), "_bsub"));
  }

  if (!found && match_name.contains(QRegExp("_cn\\d+.*$"))) {
    found = true;
    rx.setPattern(
        QString("^%1").arg(match_name).replace(QRegExp("_cn\\d+.*$"), ""));
  }

  if (!found && match_name.contains(QRegExp("\\d+$"))) {
    found = true;
    rx.setPattern(QString("^%1").arg(match_name).replace(QRegExp("\\d+$"), ""));
  }
...
void US_Hydrodyn_Mals_Saxs::adjacent_created() {
  QString match_name;
  int match_pos = 0;
  QStringList turn_on;
...
bool US_Hydrodyn_Mals_Saxs::adjacent_select(QListWidget *lb, QString match) {
  bool ok;
...
QString US_Hydrodyn_Mals_Saxs::select_conc_file(QString tag) {
...
void US_Hydrodyn_Mals_Saxs::repeak() {
  QStringList files = all_selected_files();
  repeak(files);
}

void US_Hydrodyn_Mals_Saxs::repeak(QStringList files) {
  ...
  if (peak_target_has_errors) {
    ...
    if (any_without_errors) {
      switch (QMessageBox::question(
          ...
          // QString( us_tr( "The target has SDs but %1 of %2 file%3 to repeak
          // do not have SDs at every point\n"
          //              "What would you like to do?\n" ) )
          // .arg( wo_errors_count ).arg( files.size() - 1 ).arg( files.size() >
          // 2 ? "s" : "" ),
          QString(
              us_tr(
                  "The repeak target:\n\t%1\nhas SDs, but the concentration "
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size(); i++) {
    ...
    if (any_without_errors &&
      ...
      if (match_sd) {
        ...
        for (unsigned int j = 0; j < (unsigned int)f_qs[peak_target].size();
             j++) {
          if (f_Is[peak_target][j] != 0e0) {
            fracsd[floor(f_qs[peak_target][j] + 5e-1)] =
                fabs(f_errors[peak_target][j] / f_Is[peak_target][j]);
          }
        }
        ...
        for (unsigned int j = 0; j < (unsigned int)repeak_I.size(); j++) {
          if (fracsd.count(floor(f_qs[files[i]][j] + 5e-1))) {
            repeak_e[j] =
                fabs(repeak_I[j] * fracsd[floor(f_qs[files[i]][j] + 5e-1)]);
          } else {
            if (f_qs[files[i]][j] < f_qs[peak_target][0]) {
              repeak_e[j] = fabs(repeak_I[j] *
                                 fracsd[floor(f_qs[peak_target][0] + 5e-1)]);
            } else {
              repeak_e[j] = fabs(
                  repeak_I[j] * fracsd[floor(f_qs[peak_target].back() + 5e-1)]);
            }
          }
        }
      } else {
    ...
    int ext = 0;
    QString repeak_name =
        files[i] + QString("-rp%1").arg(scale, 0, 'g', 8).replace(".", "_");
    repeak_name.replace(".", "_");
    while (current_files.count(repeak_name)) {
      repeak_name = files[i] + QString("-rp%1-%2")
                                   .arg(scale, 0, 'g', 8)
                                   .arg(++ext)
                                   .replace(".", "_");
      repeak_name.replace(".", "_");
...
void US_Hydrodyn_Mals_Saxs::crop_common() {
  ...
  editor_msg(
      ...
      QString(us_tr("Crop common:\n"
                    ...
                    "with %3 points\n"
                    "Current selected files have a common  q-range of (%4:%5) "
                    "with %6 points\n"))
...
void US_Hydrodyn_Mals_Saxs::to_saxs() {
...
QStringList US_Hydrodyn_Mals_Saxs::get_frames(QStringList files, QString head,
                                              QString tail) {
  QStringList result;
...
void US_Hydrodyn_Mals_Saxs::avg(QStringList files, QString suffix) {
  ...
  if (all_nonzero_errors) {
    for (map<QString, vector<double>>::iterator it = t_qs.begin();
         it != t_qs.end(); it++) {
      if (t_errors[it->first].size()) {
        if (all_nonzero_errors && !is_nonzero_vector(t_errors[it->first])) {
          all_nonzero_errors = false;
          break;
        }
      } else {
  ...
  for (int i = 0; i < (int)files.size(); i++) {
    ...
    if (first) {
      ...
      for (int j = 0; j < (int)t_Is[this_file].size(); j++) {
        double weight =
            all_nonzero_errors
                ? 1e0 / (t_errors[this_file][j] * t_errors[this_file][j])
                : 1e0;
        sum_weight[j] = weight;
      ...
      if (avg_qs.size() != t_qs[this_file].size()) {
        editor_msg("red", us_tr("Error: incompatible grids, the files selected "
  ...
  if (selected_count < 2) {
    editor_msg("red", us_tr("Error: not at least 2 files selected so there is "
                            "nothing to average"));
    return;
  }
  ...
  for (int i = 0; i < (int)avg_qs.size(); i++) {
    ...
    if (false) {
      ...
      for (int j = 0; j < (int)selected_count; j++) {
        ...
        double se = all_nonzero_errors
                        ? (t_errors[this_file][i] * t_errors[this_file][i])
                        : 0e0;
        // double weight  = all_nonzero_errors ? 1e0 / ( t_errors[ this_file ][
        // i ] * t_errors[ this_file ][ i ] ) : 1e0; double invweight  =
        // all_nonzero_errors ? ( t_errors[ this_file ][ i ] * t_errors[
        // this_file ][ i ] ) : 1e0; sum += weight * ( invweight * t_Is[
        // this_file ][ i ] - avg_Is[ i ] ) * ( invweight *  t_Is[ this_file ][
        // i ] - avg_Is[ i ] );
        sse += se;
      }
      // sum *= sum_weight[ i ] / ( sum_weight[ i ] * sum_weight[ i ] -
      // sum_weight2[ i ] ); avg_sd[ i ] = sqrt( sum / ( ( (double) files.size()
      // - 1e0 ) * ( sum_weight[ i ]  / (double) files.size() ) ) );
  ...
  if (all_nonzero_errors) {
    ...
    for (int i = 0; i < (int)selected_count; ++i) {
      ...
      //    }
      // }
      usu->scaling_fit(t_Is[this_file], avg_Is, t_errors[this_file], k, chi2);

      // QTextStream(stdout) << QString( "%1 scaling factor %2 chi2 %3\n" ).arg(
      // this_file ).arg( k ).arg( chi2 );
      // {
      ...
      // cout << US_Vector::qs_vector4( QString( "curve %1 scaling %2" ).arg(
      // this_file ).arg( k ), t_Is[ this_file ], avg_Is, org_error,  t_errors[
      // this_file ] );
    }
...
void US_Hydrodyn_Mals_Saxs::conc_avg(QStringList files) {
  // create average of selected
  vector<QString> avg_qs_string;
  vector<double> avg_qs;
  vector<double> avg_Is;
  ...
  for (int i = 0; i < (int)avg_qs.size(); i++) {
    ...
    for (int j = 0; j < (int)files.size(); j++) {
      ...
      double weight =
          all_nonzero_errors
              ...
              : 1e0;
      double invweight = all_nonzero_errors
                             ? (t_errors[this_file][i] * t_errors[this_file][i])
                             : 1e0;
      sum +=
          weight *
    ...
    // sum *= sum_weight[ i ] / ( sum_weight[ i ] * sum_weight[ i ] -
    // sum_weight2[ i ] );
    avg_sd[i] = avg_conc * sqrt(sum / (((double)files.size() - 1e0) *
...
void US_Hydrodyn_Mals_Saxs::select_nth() {
  map<QString, QString> parameters;
...
void US_Hydrodyn_Mals_Saxs::axis_y(bool nochange, bool no_replot) {
  if (!nochange) {
...
void US_Hydrodyn_Mals_Saxs::axis_x(bool nochange, bool no_replot) {
  if (!nochange) {
...
void US_Hydrodyn_Mals_Saxs::options() {
  map<QString, QString> parameters;
...
void US_Hydrodyn_Mals_Saxs::clear_gaussians() {
  unified_ggaussian_ok = false;
  f_gaussians.clear();
  gaussians.clear();
  org_gaussians.clear();
...
bool US_Hydrodyn_Mals_Saxs::any_gaussians() {
  return f_gaussians.size() || gaussians.size() || org_gaussians.size() ||
         org_f_gaussians.size();
}

void US_Hydrodyn_Mals_Saxs::hide_files() {
  if (U_EXPT) {
...
void US_Hydrodyn_Mals_Saxs::hide_created_files() {
  if (U_EXPT) {
...
void US_Hydrodyn_Mals_Saxs::hide_editor() {
...
void US_Hydrodyn_Mals_Saxs::svd() {
  disable_all();
...

  {
    ...
    if (any_differences) {
      editor_msg("red", us_tr("SVD: curves must be on the same grid, try 'Crop "
                              "Common' first."));
      update_enables();
      return;
    }
...
void US_Hydrodyn_Mals_Saxs::ref() {
  if (plot_ref->isVisible() || lbl_conc_file->text().isEmpty()) {
    ...
    if (!f_qs.count(lbl_conc_file->text())) {
      plot_ref->hide();
      editor_msg("dark red", us_tr("Warning: Reference plot selected but no "
                                   "concentration file found"));
      update_enables();
      return;
    }
    QStringList files = all_selected_files();
...
void US_Hydrodyn_Mals_Saxs::update_ref() {
  if (!plot_ref->isVisible()) {
    return;
  }
  if (!f_time.count(last_selected_file)) {
  ...
  plot_ref->setMarkerLabelText(ref_marker,
                               QString("%1").arg(f_time[last_selected_file]));
#else
...
double US_Hydrodyn_Mals_Saxs::tot_intensity(QString &file, double q_min,
                                            double q_max) {
  if (!f_qs_string.count(file) || !f_qs.count(file) || !f_Is.count(file) ||
    ...
    editor_msg(
        "red",
        QString(
            us_tr("Total intensity compute internal error: no curve named %1"))
            .arg(file));
    return -9e99;
  }
...
bool US_Hydrodyn_Mals_Saxs::set_selected(const QStringList &to_select_qsl,
                                         bool do_replot) {
...
bool US_Hydrodyn_Mals_Saxs::set_selected(const set<QString> &to_select,
                                         bool do_replot) {
  disable_updates = true;
  lb_files->clearSelection();
...
bool US_Hydrodyn_Mals_Saxs::set_created_selected(const set<QString> &to_select,
                                                 bool do_replot) {
  disable_updates = true;
...
void US_Hydrodyn_Mals_Saxs::artificial_gaussians() {
  ...
  bool ipe = (QMessageBox::Yes ==
              QMessageBox::question(
                  ...
                  us_tr("Add intensity proportional errors?"), QMessageBox::Yes,
                  QMessageBox::No));
...
bool US_Hydrodyn_Mals_Saxs::constant_e(QString file, double e) {
  if (!f_qs.count(file)) {
    return false;
  }
...
int US_Hydrodyn_Mals_Saxs::slice(const vector<double> &q,
...
double US_Hydrodyn_Mals_Saxs::wyatt_errors(QString file, double start_q,
                                           double end_q) {
  if (!f_qs.count(file)) {
    return -1e0;
  }
...
double US_Hydrodyn_Mals_Saxs::wyatt_errors(const vector<double> &q,
...
void US_Hydrodyn_Mals_Saxs::plot_debug() {
    ...
    if (curves) {
      ...
      tso << QString("plot for %1\n").arg(it->first)
          << QString().sprintf(
                 ...
                 ,
                 (it->second->plotLayout()->alignCanvasToScale(QwtPlot::yLeft)
                      ? "True"
                      : "False"),
                 (it->second->plotLayout()->alignCanvasToScale(QwtPlot::yRight)
                      ? "True"
                      : "False")
                 ...
                 it->second->axisWidget(QwtPlot::yLeft)
                     ->scaleDraw()
                     ->extent(it->second->axisWidget(QwtPlot::yLeft)->font()),
                 it->second->axisWidget(QwtPlot::yRight)
                     ->scaleDraw()
                     ->extent(it->second->axisWidget(QwtPlot::yRight)->font()),
                 (it->second->plotLayout()->canvasMargin(QwtPlot::yLeft) +
                  it->second->plotLayout()->canvasMargin(QwtPlot::yRight) +
                  it->second->plotLayout()->scaleRect(QwtPlot::yLeft).x() +
                  it->second->plotLayout()->scaleRect(QwtPlot::yRight).x() +
...
bool US_Hydrodyn_Mals_Saxs::ask_to_decimate(map<QString, QString> &parameters) {
  switch (QMessageBox::warning(
      this,
...
bool US_Hydrodyn_Mals_Saxs::ask_cormap_minq(map<QString, QString> &parameters) {
  bool ok;
...
void US_Hydrodyn_Mals_Saxs::cormap() {
  map<QString, QString> parameters;
  cormap(parameters);
}

void US_Hydrodyn_Mals_Saxs::cormap(map<QString, QString> &parameters) {
  disable_all();
  ...
  switch (current_mode) {
    case MODE_SCALE: {
      ...
      int decimate =
          parameters.count("decimate") ? parameters["decimate"].toInt() : 0;
      ...
      if (!f_is_time.count(scale_selected_names[0])) {
        editor_msg(
            "red",
            us_tr(
                "Internal error: PVP in scale mode files not in global files"));
        scale_enables();
        return;
      }
      ...
      if (files_are_time) {
        ...
        for (int i = 0; i < (int)use_preq_scale_selected_names.size(); ++i) {
          if (rx_cap.indexIn(use_preq_scale_selected_names[i]) == -1) {
            ...
            //                       ) , QString( us_tr( "Could not extract q
            //                       value from file name %1" ) ) .arg(
            //                       use_preq_scale_selected_names[ i ] )
            //                       ,QMessageBox::Ok | QMessageBox::Default
            //                       ,QMessageBox::NoButton
            //                       );
            editor_msg("red", QString(us_tr("PVP Analysis: Could not extract q "
          ...
          // us_qdebug( QString( "baseline cormap captured %1 from %2" ).arg( qv
          // ).arg( use_preq_scale_selected_names[ i ] ) );
      ...
      if (!files_are_time) {
        ...
        {
          parameters["title"] =
              QString(us_tr("Scale mode %1: Full q range used for analysis."))
                  .arg(scale_applied ? QString("scaled on %1 range %2 to %3")
          ...
          if (cormap_minq > 0e0) {
            parameters["title"] +=
                QString(" Minimum q limit is %1 [A^-1].").arg(cormap_minq);
            parameters["title_adj"] +=
                QString(" Minimum q limit is %1 [A^-1].").arg(cormap_minq);
          }
      ...
      // once for maxq cutoff
      {
...

        {
          ...
          for (int i = 0; i < (int)use_scale_selected_names.size(); ++i) {
            ...
            if (scale_q.count(this_file) && scale_I.count(this_file) &&
                scale_q[this_file].size() && scale_I[this_file].size()) {
          ...
          if (any_differences) {
            editor_msg("red", us_tr("PVP: curves must be on the same grid, try "
                                    "'Crop Common' first."));
            scale_enables();
            return;
          }
        ...
        {
          parameters["title"] =
              QString(us_tr("Scale mode %1: Maximum q limit is %2 [A^-1]."))
                  .arg(scale_applied ? QString("scaled on %1 range %2 to %3")
                                           .arg(files_are_time ? "frame" : "q")
                                           .arg(scale_applied_q_min)
                                           .arg(scale_applied_q_max)
    ...
    case MODE_BLANKS: {
      ...
      // US_Vector::printvector( "blanks_created_q", blanks_created_q );
      {
        // QRegExp rx_cap( "_It_q(\\d+_\\d+)" );
        int use_decimate =
            parameters.count("decimate") ? parameters["decimate"].toInt() : 1;
        for (int i = 0; i < (int)use_preq_blanks_created.size(); ++i) {
          ...
          if (blanks_created_q[(vector<double>::size_type)i * use_decimate] <=
                  cormap_maxq &&
              blanks_created_q[(vector<double>::size_type)i * use_decimate] >=
                  cormap_minq) {
...

      {
        ...
        for (int i = 0; i < fcount; ++i) {
          ...
          for (int k = 0; k < blanks_size; ++k) {
            if (!skip_k.count(k)) {
              sum_sd2 += f_errors[use_blanks_created[k]][indicies[i]] *
              ...
              //         ) .arg( f_errors[ use_blanks_created[ k ] ][ indicies[
              //         i ] ]  ) );
            }
          }
      ...
      for (int i = 0; i < fcount - 1; ++i) {
        ...
        for (int j = i + 1; j < fcount; ++j) {
          ...
          // US_Vector::printvector2( QString( "blanks for cormap i %1 j %2 I"
          // ).arg( i ).arg( j ), I[ 0 ], I[ 1 ] );
...

      {
        ...
        parameters["title_adj"] =
            QString(us_tr("Blanks mode t %1 to %2.")).arg(q_start).arg(q_end);
    ...
    case MODE_BASELINE: {
      ...
      QStringList use_baseline_selected;
      {
        ...
        for (int i = 0; i < (int)use_preq_baseline_selected.size(); ++i) {
          if (rx_cap.indexIn(use_preq_baseline_selected[i]) == -1) {
            // QMessageBox::warning( this
            //                       , windowTitle() + us_tr( " : Baseline PVP
            //                       Analysis" ) , QString( us_tr( "Could not
            ...
            //                       );
            editor_msg("red",
                       QString(us_tr("Baseline PVP Analysis: Could not extract "
                                     "q value from file name %1"))
                           .arg(use_preq_baseline_selected[i]));
            baseline_enables();
            return;
          }
      ...
      //        ) .arg( fcount ) .arg( indicies.size() ) .arg( baseline_size )
      //        .arg( m )
      //         );
      ...
      for (int i = 0; i < fcount - 1; ++i) {
        ...
        for (int j = i + 1; j < fcount; ++j) {
          ...
          // US_Vector::printvector2( QString( "for cormap i %1 j %2 I" ).arg( i
          // ).arg( j ), I[ 0 ], I[ 1 ] );
...

      {
        parameters["title"] =
            QString(
                us_tr(
                    "Baseline mode t %1 to %2. Maximum q limit is %3 [A^-1]."))
                .arg(q_start)
                .arg(q_end)
                .arg(cormap_maxq);
        ...
        parameters["title_adj"] =
            QString(us_tr("Baseline mode t %1 to %2.")).arg(q_start).arg(q_end);
    ...
    case MODE_GGAUSSIAN: {
      if (!unified_ggaussian_ok) {
        editor_msg("red", us_tr("Internal error (PVP): Global Gaussian mode, "
...

      {
        ...
        US_Hydrodyn_Saxs_Cormap *uhcm = new US_Hydrodyn_Saxs_Cormap(
            us_hydrodyn, parameters, pvaluepairs, pvaluepairs, selected_files);
        if (parameters.count("close")) {
          delete uhcm;
        } else {
          uhcm->show();
    ...
    default: {
      ...
      for (int i = 0; i < fcount - 1; ++i) {
        ...
        for (int j = i + 1; j < fcount; ++j) {
          ...
          // US_Vector::printvector2( QString( "default for cormap i %1 j %2 I"
          // ).arg( i ).arg( j ), I[ 0 ], I[ 1 ] );
      ...
      {
        parameters["title"] =
            QString(us_tr("Maximum q limit is %1 [A^-1].")).arg(cormap_maxq);
        ...
        US_Hydrodyn_Saxs_Cormap *uhcm =
            new US_Hydrodyn_Saxs_Cormap(us_hydrodyn, parameters, pvaluepairs,
                                        adjpvaluepairs, use_selected_files);
        if (parameters.count("close")) {
          delete uhcm;
        } else {
          uhcm->show();
...
void US_Hydrodyn_Mals_Saxs::bin() {
  QStringList files = all_selected_files();
  bin(files);
}

void US_Hydrodyn_Mals_Saxs::bin(QStringList files) {
  bool ok;
  int binning = US_Static::getInteger(
      us_tr("SOMO: HPLC enter binning"),
      us_tr("Enter the number of points of binning:"), 2, 2, 10, 1, &ok, this);
  if (!ok) {
    return;
  }
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size(); i++) {
    ...
    if (usu.bin(f_qs[files[i]], f_Is[files[i]], f_errors[files[i]], binned_q,
                binned_I, binned_e, binning)) {
...
void US_Hydrodyn_Mals_Saxs::bb_cm_inc() {
  // us_qdebug( "bb_cm_inc" );
  ...
  switch (QMessageBox::question(
      ...
      us_tr("&Slide current window forward"), us_tr("&Increment window size"),
      us_tr("&Cancel"), 0  //
      ,
      -1  //
      )) {
  ...
  // us_qdebug( QString( "start %1 end %2 min %3 max %4" ).arg( start ).arg( end
  // ).arg( min ).arg( max ) );
...
bool US_Hydrodyn_Mals_Saxs::blanks_params() {
  ...
  for (int pts = points_start; pts <= points && pts <= points_end;
    ...
    while (pts + pos - 1 < points) {
      ...
      // if ( !fn.isEmpty() ) {
      //    parameters[ "save_png" ] = fn + QString( "_fr%1_%2" ).arg( start
      //    ).arg( end ) + ".png"; parameters[ "save_csv" ] = fn + ".csv";
      // }
      cormap(parameters);
      save_parameters["csv_skip_report_header"] = "true";
      pos += 10;
...
QStringList US_Hydrodyn_Mals_Saxs::vector_qstring_to_qstringlist(
    vector<QString> vqs) {
  QStringList qsl;
...
void US_Hydrodyn_Mals_Saxs::simulate() {
  us_qdebug("simulate");
...

  {
    ...
    if (any_differences) {
      editor_msg("red", us_tr("Simulate: curves must be on the same grid, try "
                              "'Crop Common' first."));
      update_enables();
      return;
    }
  ...
  US_Hydrodyn_Mals_Saxs_Simulate *uhshs =
      new US_Hydrodyn_Mals_Saxs_Simulate(us_hydrodyn, this, selected, q, I, e);
  uhshs->show();
}

void US_Hydrodyn_Mals_Saxs::gauss_mode() {
  map<QString, QString> parameters;
  ...
  parameters["gaussian_type"] = QString("%1").arg(gaussian_type);
  US_Hydrodyn_Mals_Saxs_Gauss_Mode *sho = new US_Hydrodyn_Mals_Saxs_Gauss_Mode(
      &parameters, (US_Hydrodyn *)us_hydrodyn, this);
...
bool US_Hydrodyn_Mals_Saxs::mals_saxs_load(const QString &filename,
  ...
  if (!mals_saxs_angles.mals_saxs_angle.size()) {
    errormsg =
        "No MALS_SAXS Angles have been defined.\nLoad them prior to loading "
        "MALS_SAXS data";
    return false;
  }
  ...
  if (!mals_saxs_param_lambda || !mals_saxs_param_n) {
    errormsg =
        "Options->MALS_SAXS parameters must be defined prior to loading "
        "MALS_SAXS data";
    return false;
  }
...
#define TIME_DELTA_TOLERANCE 0.0000001
  ...
  if (timedelta_inconsistencies) {
    QMessageBox::warning(
        ...
        QString(
            us_tr("%1 inconsistent time interval(s) found in MALS_SAXS data\n"
  ...
  if (!avg_timedelta_count || !avg_timedelta_sum) {
    errormsg =
        "Could not determine a collection interval from the MALS_SAXS data";
    return false;
  }
  ...
  // adjust_times dialog
  {
    ...
    do {
      // US_Vector::printvector( QString( "mals_saxs detector %1 times" ).arg(
      // detector_index[0] ), t[detector_index[0]] );
      ...
      vector<QWidget *> widgets = {(QWidget *)0, (QWidget *)0,
  ...
  for (auto it = t.begin(); it != t.end(); ++it) {
    if (mals_saxs_angles.mals_saxs_angle.count(it->first) &&
      ...
      double q = RI * 4 * M_PI *
                 sin(mals_saxs_angles.mals_saxs_angle[it->first].angle_ri_corr *
                     M_PI / 360) /
                 (wavelength * 10);
  ...
  if (data.size() &&
      data.front().contains(QRegularExpression("^\\s*UV absorbance data:"))) {
    ...
    //                                          windowTitle() + us_tr( " : Load
    //                                          MALS_SAXS Data" )
    //                                          ,"Choose the detector for
    //                                          repeaking the UV data or Cancel
    //                                          to not repeak"
    ...
    //       "\\s+" ) ); repeak_detector = qsl.front().toInt(); TSO << "repeak
    //       detector " << repeak_detector << "\n";
    //    }
    // }
    ...
    if (load_uv_data) {
...

      {
        ...
        TSO << QString(
                   "avg_uvtimedelta %1 --> seconds %2 [%3:%4] frames [%5:%6]\n")
...
bool US_Hydrodyn_Mals_Saxs::mals_saxs_angles_save() {
  QString errormsg = "";
  if (mals_saxs_angles.mals_saxs_angle.size()) {
...
void US_Hydrodyn_Mals_Saxs::conc_info(const QString &msg) {
  TSO << "============================================================\n"
      << msg << "\n"
      << "------------------------------------------------------------\n";
...
void US_Hydrodyn_Mals_Saxs::dndc_info(const QString &msg) {
  TSO << "============================================================\n"
      << msg << "\n"
      << "------------------------------------------------------------\n";
...
void US_Hydrodyn_Mals_Saxs::extc_info(const QString &msg) {
  TSO << "============================================================\n"
      << msg << "\n"
      << "------------------------------------------------------------\n";
...
vector<double> US_Hydrodyn_Mals_Saxs::get_time_grid_from_namelist(
...
void US_Hydrodyn_Mals_Saxs::common_time() {
  disable_all();
...

  {
    ...
    for (auto const &name : names) {
      ...
      if (use_errors && (!f_errors.count(name) ||
    ...
    if (qgrid_to_names.size() != 2) {
      ...
      for (auto &it : qgrid_to_names) {
        detail += QString(us_tr("grid %1 first (of %2) file name: %3\n\n"))
                      .arg(++pos)
                      .arg(it.second.size())
                      .arg(it.second.front());
      }
      ...
      QMessageBox::critical(this,
                            ...
                            QString(us_tr("The %1 curves selected do not have "
                                          "exactly 2 unique q-grids\n\n"
                                          "%2 q-grid(s) found\n"))
                                    .arg(names.size())
  ...
  if (!use_errors) {
    QMessageBox::warning(this, windowTitle() + us_tr(": I#,*(q) common time"),
                         QString(us_tr("Not all files have associated errors, "
                                       "so the result will not have errors"
  ...
  int mals_set =
      f_qs[qgrid_names[0][0]].front() < f_qs[qgrid_names[1][0]].front() ? 0 : 1;
  ...
  // q value at a time should lessen memory requirements (but is it faster? does
  // it matter?)
  progress->reset();
      ...
      switch (((US_Hydrodyn *)us_hydrodyn)
        ...
        case US_Hydrodyn_Mals_Saxs_Options::INTERP_METHOD_QUADRATIC: {
          for (int i = 0; i < (int)output_qs.size(); ++i) {
            ...
            if (!usu->quadratic_interpolate(time_grids[mals_set], yerrors,
                                            output_times, yerrors2)) {
              QString msg = us_tr("Interpolation error : " + usu->errormsg);
              QMessageBox::critical(
                  this, windowTitle() + us_tr(": I#,*(q) common time"), msg);
        ...
        case US_Hydrodyn_Mals_Saxs_Options::INTERP_METHOD_CUBIC_SPLINE: {
          for (int i = 0; i < (int)output_qs.size(); ++i) {
            ...
            for (auto const &t : output_times) {
              ...
              usu->apply_natural_spline(time_grids[mals_set], yerrors, yerrors2,
                                        t, error);
...

  {
    ...
    if (use_errors) {
      for (auto const &t : output_times) {
        QString name =
            QString("%1%2%3%4_common").arg(head).arg(t).arg(tail).arg(method);
...

  {
    ...
    for (auto const &t : mals_times) {
      if (!saxs_names_by_time.count(t)) {
        qDebug() << QString(
                        "internal error: common_times() can't find matching "
                        "saxs mals time %1")
                        .arg(t);
      } else {
...
void US_Hydrodyn_Mals_Saxs::join_by_time() {
  ...
  if (!use_errors) {
    QMessageBox::warning(this, windowTitle() + us_tr(": Join I#,*(q) by time"),
                         QString(us_tr("Not all files have associated errors, "
                                       "so the result will not have errors"
                                       "\n\nFirst discovered in file:\n%1"))
                             .arg(missing_errors_name));
...

  {
    ...
    if (time_grids[0] != time_grids[1]) {
      QMessageBox::critical(
          this, windowTitle() + us_tr(": Join I#,*(q) by time"),
          QString(
              us_tr("The %1 curves selected do not have the same times\n\n"))
              .arg(names.size()));
      return update_enables();
    }
  ...
  vector<double> output_qs = US_Vector::vunion(f_qs[qgrid_names[mals_set][0]],
                                               f_qs[qgrid_names[saxs_set][0]]);
  ...
  if (common_qs.size()) {
    QMessageBox::warning(this, windowTitle() + us_tr(": Join I#,*(q) by time"),
                         QString(us_tr("The 2 q grids have %1 common point(s)"
                                       ...
                                       "\n\nThe values from the lower q-value "
                                       "set (typically MALS) will be used"))
                             .arg(common_qs.size())
                             .arg(common_qs.front()));
  }
...
bool US_Hydrodyn_Mals_Saxs::check_files_selected_paired(
...

  {
    ...
    for (auto const &name : files) {
      if (!f_qs.count(name) || !f_Is.count(name)) {
        qDebug() << QString(
                        "check_selected_paired() : internal error, data "
                        "missing for %1")
                        .arg(name);
        return false;
      }
...
bool US_Hydrodyn_Mals_Saxs::mals_params_interpolate(
...
bool US_Hydrodyn_Mals_Saxs::mals_params_interpolate(const vector<double> &times,
...

  {
    ...
    for (size_t i = 1; i < (size_t)qsl.size(); ++i) {
      ...
      if (fields.size() < 2) {
        errormsg = QString(us_tr("File %1 has insufficient data on line %2"))
                       .arg(filename)
                       .arg(i + 1);
        return false;
      }
      ...
      if (used_times.count(param_times.back())) {
        errormsg = QString(us_tr("File %1 apparently has a duplicate time %2"))
                       .arg(filename)
                       .arg(i + 1);
        return false;
      }
...

  {
    ...
    if (field_counts.size() != 1) {
      ...
      errormsg = QString(us_tr("File %1 has varying data field counts (%2)"))
                     .arg(filename)
                     .arg(counts_msg);
      return false;
    }
  ...
  double start_time_min =
      times.front() > param_times.front() ? times.front() : param_times.front();

  double end_time_max =
      times.back() < param_times.back() ? times.back() : param_times.back();
...

  {
    ...
    QString msg =
        QString(us_tr("%1 MALS parameters were identified"
                      "\n\nThe MALS parameters time grid has %2 time points "
                      "covering times [%3:%4]"
                      "\n\nThe common time grid has %5 time points covering "
                      "times [%6:%7]"
  ...
  QString separator = filename.contains(QRegularExpression(
  ...
  // save the file
  {
    ...
    QString use_filename = QFileDialog::getSaveFileName(
        this,
        us_tr("Select a file name for saving the Interpolated MALS parameters"),
        use_dir, "*." + ext);
