...
#include "../include/us_hydrodyn_mals_saxs_gauss_mode.h"
...
void
US_Hydrodyn_Mals_Saxs::invert_all_created ()
{
  disable_all ();
...
void
US_Hydrodyn_Mals_Saxs::remove_created ()
{
  disable_all ();
  QStringList files;
...
US_Hydrodyn_Mals_Saxs::add ()
{
      ...
      if (f_qs[files[0]] != f_qs[files[i]])
        {
          editor_msg (
              "red",
              QString (us_tr ("Error: Residuals incompatible grids (comparing "
                              "%1 and %2). Suggest: Crop Common"))
                  .arg (files[0])
                  .arg (files[i]));
          update_enables ();
          return;
  ...
  if (use_errors)
    {
      add_plot (name, f_qs[files[0]], sum, e,
                f_is_time.count (files[0]) ? f_is_time[files[0]] : false,
                false);
    }
  else
    {
      add_plot (name, f_qs[files[0]], sum,
                f_is_time.count (files[0]) ? f_is_time[files[0]] : false,
                false);
    }
  update_enables ();
...
bool
US_Hydrodyn_Mals_Saxs::all_have_f_gaussians (QStringList &files)
{
...
US_Hydrodyn_Mals_Saxs::p3d ()
{
  ...
  if (!all_have_f_gaussians (files))
    {
      editor_msg ("red",
                  us_tr ("Error: Not all files have Gaussians defined"));
      update_enables ();
      return;
    }
  if (!ggaussian_compatible (files, false))
    {
      editor_msg (
          ...
          cb_fix_width->isChecked ()
              ? us_tr (
                    "NOTICE: Some files selected have Gaussians with varying "
                    "centers or widths or a different number of Gaussians or "
                    "centers or widths that do not match the last Gaussians")
              : us_tr ("NOTICE: Some files selected have Gaussians with "
                       "varying centers or a different number of Gaussians or "
                       "centers that do not match the last Gaussians."));
      update_enables ();
      return;
    }
  ...
  for (unsigned int i = 1; i < (unsigned int)files.size (); i++)
    {
      if (f_qs[files[0]] != f_qs[files[i]])
        {
          editor_msg ("red",
                      QString (us_tr ("Error: Incompatible grids (comparing "
                                      "%1 and %2). Suggest: Crop Common"))
                          .arg (files[0])
                          .arg (files[i]));
          update_enables ();
...

  {
    ...
    parameters["gaussians"] = QString ("%1").arg (f_gaussians[files[0]].size ()
                                                  / gaussian_type_size);

    US_Hydrodyn_Mals_Saxs_P3d *mals_saxs_p3d
        = new US_Hydrodyn_Mals_Saxs_P3d (this, &parameters, this);
    US_Hydrodyn::fixWinButtons (mals_saxs_p3d);
    mals_saxs_p3d->exec ();
    delete mals_saxs_p3d;
  ...
  if (!g_to_plot.size ())
    {
      editor_msg ("dark red",
                  us_tr ("Plot 3D: no Gaussians selected to plot"));
      update_enables ();
      return;
    }
  ...
  // vector < vector < vector < double > > > fg; // a vector of the individual
  // gaussians
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
    {
      ...
      for (unsigned int j = 0; j < (unsigned int)f_gaussians[files[i]].size ();
  ...
  // plot 3d
  {
    ...
    controlvar_3d.yscaling
        = 10e0 / (f_qs[files[0]].back () - f_qs[files[0]][0]);
    ...
    cout << QString ("3d params t %1,%2 scaling %3\n"
                     "          q %4,%5 scaling %6\n"
                     "       maxI %7    scaling %8\n")
                .arg (controlvar_3d.minx)
                .arg (controlvar_3d.maxx)
                .arg (controlvar_3d.xscaling)
                .arg (controlvar_3d.miny)
                .arg (controlvar_3d.maxy)
                .arg (controlvar_3d.yscaling)
                .arg (maxI)
                .arg (controlvar_3d.zscaling);
    ...
    if (plot3d_flag)
      {
        plot3d_window->setParameters (title, xtitle, ytitle, ztitle, data3d,
                                      &controlvar_3d);
      }
    else
      {
        plot3d_window
            = new Mesh2MainWindow (&plot3d_flag, title, xtitle, ytitle, ztitle,
                                   data3d, &controlvar_3d);
      }
...
void
US_Hydrodyn_Mals_Saxs::set_detector ()
{
  disable_all ();

  {
    ...
    US_Hydrodyn_Mals_Saxs_Dctr *mals_saxs_dctr
        = new US_Hydrodyn_Mals_Saxs_Dctr (this, &parameters, this);
    ...
    detector_uv = (parameters.count ("uv") && parameters["uv"] == "true")
                      ? true
                      : false;
    detector_ri = (parameters.count ("ri") && parameters["ri"] == "true")
                      ? true
                      : false;
...
/*
   ...
   if ( created_not_saved_list.size() )
   {
      ...
      {
         qsl << QString( us_tr( "... and %1 more not listed" ) ).arg(
   created_not_saved_list.size() - qsl.size() );
      }

      switch ( QMessageBox::warning(this,
                                    us_tr( "US-SOMO: MALS_SAXS Remove Files" ),
                                    QString( us_tr( "Please note:\n\n"
                                                 "These files were created but
   not saved as .dat files:\n"
                                                 "%1\n\n"
                                                 "What would you like to do?\n"
   ) ) .arg( qsl.join( "\n" ) ), us_tr( "&Save them now" ), us_tr( "&Remove
   them anyway" ), us_tr( "&Quit from removing files" ), 0, // Stop == button 0
                                    0 // Escape == button 0
                                    ) )
      {
...
bool
US_Hydrodyn_Mals_Saxs::height_wheel_file (double &val, double pos)
{
  ...
  return US_Saxs_Util::static_apply_natural_spline (
      height_natural_spline_x, height_natural_spline_y,
      height_natural_spline_y2, pos, val, errormsg);
}

void
US_Hydrodyn_Mals_Saxs::gauss_match_amplitude ()
{
  if (current_mode != MODE_GAUSSIAN)
    {
      return;
...
bool
US_Hydrodyn_Mals_Saxs::opt_repeak_gaussians (QString file)
{
  if (!gaussians.size ())
    {
      return false;
  ...
  if (scale < .5 || scale > 1.5)
    {
      switch (QMessageBox::warning (
          this, windowTitle (),
          QString (us_tr ("Please note:\n\n"
                          "The current Gaussians should be scaled by %1 to be "
                          "in the range of this curve.\n"
                          "What would you like to do?\n"))
              .arg (scale),
          us_tr ("&Rescale the Gaussian amplitudes"),
          us_tr ("&Do not rescale"), QString (),
          0, // Stop == button 0
          0  // Escape == button 0
          ))
        {
...
vector<double>
US_Hydrodyn_Mals_Saxs::conc_curve (vector<double> &t, unsigned int peak,
                                   double conv)
{
  ...
  if (conc_file.isEmpty ())
    {
      editor_msg (
          "red",
          us_tr ("Internal error: conc_curve(): no concentration file set"));
      return result;
    }
  else
    {
      if (!f_gaussians.count (conc_file))
        {
          editor_msg ("red",
                      us_tr ("Internal error: conc_curve(): no Gaussians "
                             "defined for concentration file"));
          return result;
        }
    }

  if (peak
      >= (unsigned int)f_gaussians[conc_file].size () / gaussian_type_size)
    {
      editor_msg (
          "red",
          QString (us_tr ("Internal error: conc_curve(): Gaussian requested "
                          "(%1) exceedes available (%2)"))
              .arg (peak + 1)
  ...
  if (!detector_uv && !detector_ri)
    {
      editor_msg (
          "red", us_tr ("Internal error: conc_curve(): No detector type set"));
      return result;
    }
  ...
  // US_Vector::printvector( QString( "conc_curve peak %1 conv %2 detector_conv
  // %3" ).arg( peak + 1 ).arg( conv ).arg( detector_conv ), t );
  ...
  // US_Vector::printvector( QString( "conc_curve gaussian before conversion"
  // ), result );
...
bool
US_Hydrodyn_Mals_Saxs::adjacent_ok (QString name)
{
  if (name.contains ("_bsub_a") || name.contains (QRegExp ("\\d+$")))
    {
...
void
US_Hydrodyn_Mals_Saxs::adjacent ()
{
  QString match_name;
  int match_pos = 0;
  QStringList turn_on;
  ...
  if (!found && match_name.contains (QRegExp ("\\d+$")))
    {
      found = true;
      rx.setPattern (
          QString ("^%1").arg (match_name).replace (QRegExp ("\\d+$"), ""));
    }
...
void
US_Hydrodyn_Mals_Saxs::adjacent_created ()
{
  QString match_name;
  int match_pos = 0;
  QStringList turn_on;
...
bool
US_Hydrodyn_Mals_Saxs::adjacent_select (QListWidget *lb, QString match)
{
  bool ok;
  ...
  match = US_Static::getText (
      ...
      us_tr ("Regular expression search\n"
             ...
             " () group\n"
             "e.g.:\n"
             " ^([A-B]\\d){2} would match anything that started with A or B "
             "followed by a digit twice,\n"
             " i.e. A1B2 would match\n\n"
             "Enter regular expression pattern:\n"),
      QLineEdit::Normal, match, &ok, this);
...
QString
US_Hydrodyn_Mals_Saxs::select_conc_file (QString tag)
{
  ...
  editor_msg (
      "dark blue",
      QString (us_tr ("Concentration file for %1: %2")).arg (tag).arg (file));
  return file;
}

void
US_Hydrodyn_Mals_Saxs::repeak ()
{
  QStringList files = all_selected_files ();
  repeak (files);
}
...
void
US_Hydrodyn_Mals_Saxs::repeak (QStringList files)
{
  ...
  //                                             us_tr( "SOMO: HPLC repeak:
  //                                             enter peak target" ),
  //                                             us_tr("Select the peak target
  //                                             file:\n" ), files, 0, false,
  //                                             &ok,
  //                                             this );
  // if ( !ok ) {
  ...
  bool peak_target_has_errors
      = (f_errors.count (peak_target)
  ...
  if (peak_target_has_errors)
    {
      unsigned int pts = 0;
      for (unsigned int i = 0; i < (unsigned int)f_errors[peak_target].size ();
           i++)
        {
          if (f_Is[peak_target][i] != 0e0)
            {
              avg_sd_mult
                  += fabs (f_errors[peak_target][i] / f_Is[peak_target][i]);
              pts++;
            }
        }
      ...
      if (any_without_errors)
        {
          switch (QMessageBox::question (
              this, windowTitle () + us_tr (": repeak"),
              // QString( us_tr( "The target has SDs but %1 of %2 file%3 to
              // repeak do not have SDs at every point\n"
              //              "What would you like to do?\n" ) )
              // .arg( wo_errors_count ).arg( files.size() - 1 ).arg(
              // files.size() > 2 ? "s" : "" ),
              QString (us_tr ("The repeak target:\n\t%1\nhas SDs, but the "
                              "concentration file:\n\t%2\nto repeak does not "
                              "have SDs at every point.\n\n"
                              "What would you like to do?\n"))
                  .arg (peak_target)
                  .arg (use_conc),
              us_tr ("&Ignore SDs"), us_tr ("Match target SD % pointwise"),
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
    {
      ...
      if (any_without_errors
          && (repeak_e.size () != repeak_I.size ()
              || !is_nonzero_vector (repeak_e)))
        {
          if (match_sd)
            {
              map<double, double> fracsd;
              for (unsigned int j = 0;
                   j < (unsigned int)f_qs[peak_target].size (); j++)
                {
                  if (f_Is[peak_target][j] != 0e0)
                    {
                      fracsd[floor (f_qs[peak_target][j] + 5e-1)] = fabs (
                          f_errors[peak_target][j] / f_Is[peak_target][j]);
                    }
                }
                  ...
                  if (fracsd.count (floor (f_qs[files[i]][j] + 5e-1)))
                    {
                      repeak_e[j]
                          = fabs (repeak_I[j]
                                  * fracsd[floor (f_qs[files[i]][j] + 5e-1)]);
                    }
                  else
                    {
                      if (f_qs[files[i]][j] < f_qs[peak_target][0])
                        {
                          repeak_e[j] = fabs (
                              repeak_I[j]
                              * fracsd[floor (f_qs[peak_target][0] + 5e-1)]);
                        }
                      else
                        {
                          repeak_e[j]
                              = fabs (repeak_I[j]
                                      * fracsd[floor (f_qs[peak_target].back ()
                                                      + 5e-1)]);
                        }
                    }
                }
      ...
      QString repeak_name
          = files[i]
            + QString ("-rp%1").arg (scale, 0, 'g', 8).replace (".", "_");
  ...
  if (QMessageBox::Yes
      == QMessageBox::question (
          this, windowTitle () + us_tr (": repeak : set concentration file"),
...
void
US_Hydrodyn_Mals_Saxs::crop_common ()
{
      ...
      if (lb_files->item (i)->isSelected ())
        {
          ...
          if (f_qs.count (this_file) && f_Is.count (this_file)
              && f_qs[this_file].size () && f_Is[this_file].size ())
            {
  ...
  editor_msg ("black",
              QString (us_tr ("Crop common:\n"
                              "Current selected files have a maximal q-range "
                              "of (%1:%2) with %3 points\n"
                              "Current selected files have a common  q-range "
                              "of (%4:%5) with %6 points\n"))
                  .arg (v_union.size () ? QString ("%1").arg (v_union[0])
                                        : QString ("empty"))
                  .arg (v_union.size () ? QString ("%1").arg (v_union.back ())
                                        : QString ("empty"))
                  .arg (v_union.size ())
                  .arg (v_int.size () ? QString ("%1").arg (v_int[0])
                                      : QString ("empty"))
                  .arg (v_int.size () ? QString ("%1").arg (v_int.back ())
                                      : QString ("empty"))
                  .arg (v_int.size ()));
  ...
  if (!any_differences)
    {
      editor_msg (
          "black",
          us_tr ("Crop common: no differences between selected grids"));
      return;
    }
      ...
      if (lb_files->item (i)->isSelected ())
        {
          ...
          if (f_qs.count (this_file) && f_Is.count (this_file))
            {
              if (f_errors.count (this_file) && f_errors[this_file].size ())
                {
                  saxs_window->plot_one_iqq (f_qs[this_file], f_Is[this_file],
                                             f_errors[this_file], this_file);
                }
              else
                {
                  saxs_window->plot_one_iqq (f_qs[this_file], f_Is[this_file],
              ...
              if (concs.count (this_file) && concs[this_file] != 0e0)
              ...
              else
                {
                  if (f_conc.count (this_file) && f_conc[this_file] != 0e0)
                    ...
                    {
                      use_conc
                          = ((US_Hydrodyn *)us_hydrodyn)->saxs_options.conc;
                    }
                }
              saxs_window->update_conc_csv (
                  saxs_window->qsl_plotted_iq_names.back (), use_conc,
                  (f_psv.count (this_file) && f_psv[this_file] != 0e0)
                      ? f_psv[this_file]
                      : ((US_Hydrodyn *)us_hydrodyn)->saxs_options.psv,
                  (f_I0se.count (this_file) && f_I0se[this_file] != 0e0)
                      ? f_I0se[this_file]
                      : ((US_Hydrodyn *)us_hydrodyn)->saxs_options.I0_exp);
            }
          else
            {
...
QStringList
US_Hydrodyn_Mals_Saxs::get_frames (QStringList files, QString head,
                                   QString tail)
{
  QStringList result;
#if defined(DEBUG_FRAME_NAMES)
  QTextStream (stdout) << QString ("get_frames():\n"
                                   "--> files: %1\n"
                                   "--> head : %2\n"
                                   "--> tail : %3\n")
                              .arg (files.join ("\n"))
                              ...
                              .arg (tail);
#endif
  result = files.replaceInStrings (QRegExp ("^" + QRegExp::escape (head)), "")
...
void
US_Hydrodyn_Mals_Saxs::avg (QStringList files, QString suffix)
{
  ...
  if (crop)
    {
      editor_msg (
          "dark red",
          QString (us_tr ("Notice: averaging requires cropping to %1 points"))
              .arg (min_q_len));
  ...
  if (all_nonzero_errors)
    ...
    {
      editor_msg ("black",
                  us_tr ("Notice: NOT using standard deviation in mean "
                         "calculation, since some sd's were zero or missing"));
    }
      ...
      if (false && all_nonzero_errors)
        {
          for (int j = 0; j < (int)t_Is[this_file].size (); j++)
            {
              t_Is[this_file][j]
                  /= t_errors[this_file][j] * t_errors[this_file][j];
            }
        }
      ...
      if (first)
        {
          ...
          for (int j = 0; j < (int)t_Is[this_file].size (); j++)
            {
              double weight = all_nonzero_errors
                                  ? 1e0
                                        / (t_errors[this_file][j]
                                           * t_errors[this_file][j])
                                  : 1e0;
              sum_weight[j] = weight;
          ...
          avg_conc = concs.count (this_file) ? concs[this_file] : 0e0;
          ...
          if (avg_qs.size () != t_qs[this_file].size ())
            {
              editor_msg (
                  "red",
                  us_tr ("Error: incompatible grids, the files selected do "
          ...
          for (int j = 0; j < (int)t_Is[this_file].size (); j++)
            {
              if (fabs (avg_qs[j] - t_qs[this_file][j]) > Q_VAL_TOL)
                {
                  editor_msg ("red",
                              us_tr ("Error: incompatible grids, the q values "
                                     "differ between selected files"));
                  cout << QString ("val1 %1 val2 %2 fabs diff %3\n")
                              .arg (avg_qs[j])
                              .arg (t_qs[this_file][j])
                              .arg (fabs (avg_qs[j] - t_qs[this_file][j]));
                  return;
                }
  ...
  if (selected_count < 2)
    {
      editor_msg ("red", us_tr ("Error: not at least 2 files selected so "
          ...
          for (int j = 0; j < (int)selected_count; j++)
            {
              ...
              double se
                  = all_nonzero_errors
                        ? (t_errors[this_file][i] * t_errors[this_file][i])
                        : 0e0;
              // double weight  = all_nonzero_errors ? 1e0 / ( t_errors[
              // this_file ][ i ] * t_errors[ this_file ][ i ] ) : 1e0; double
              // invweight  = all_nonzero_errors ? ( t_errors[ this_file ][ i ]
              // * t_errors[ this_file ][ i ] ) : 1e0; sum += weight * (
              // invweight * t_Is[ this_file ][ i ] - avg_Is[ i ] ) * (
              // invweight *  t_Is[ this_file ][ i ] - avg_Is[ i ] );
              sse += se;
            }
          // sum *= sum_weight[ i ] / ( sum_weight[ i ] * sum_weight[ i ] -
          // sum_weight2[ i ] ); avg_sd[ i ] = sqrt( sum / ( ( (double)
          // files.size() - 1e0 ) * ( sum_weight[ i ]  / (double) files.size()
          // ) ) );
          avg_sd[i] = sqrt (sse) / (double)selected_count;
        }
    }

  // get scaling factor for each t_Is -> avg_Is, then scale the SD's by this
  // multiplier for each curve What you can do is average the intensities
  // WITHOUT scaling, but average the SD WITH scaling. The scaling factor for
  // the SD being that which bring all curves to the final average one.
      ...
      for (int i = 0; i < (int)selected_count; ++i)
        {
          ...
          //    }
          // }
          usu->scaling_fit (t_Is[this_file], avg_Is, t_errors[this_file], k,
                            chi2);
          ...
          //       tmp_plot[ j ] *= k;
          //    }
          //    add_plot(  QString( "%1_scaled" ).arg( this_file ), avg_qs,
          //    tmp_plot, true, false );
          // }
          ...
          // cout << US_Vector::qs_vector4( QString( "curve %1 scaling %2"
          // ).arg( this_file ).arg( k ), t_Is[ this_file ], avg_Is, org_error,
          // t_errors[ this_file ] );
        }
  ...
  QString framename = QString ("f%1_%2_%3_")
                          .arg (framenames.size ())
                          .arg (framenames.front ())
                          .arg (framenames.back ());

  int ext = 0;

  if (!head.isEmpty () && !head.contains (QRegExp ("_$")))
    {
      head += "_";
    }
  if (!tail.isEmpty () && !tail.contains (QRegExp ("^_")))
    {
      tail = "_" + tail;
    }
  ...
  while (current_files.count (avg_name))
    {
      avg_name
          = head + suffix + framename + QString ("avg-%1").arg (++ext) + tail;
    }
  ...
  f_header[avg_name] = QString (" Frames (%1) %2")
                           .arg (framenames.size ())
                           .arg (framenames.join (","));
  {
    vector<double> tmp;
  ...
  for (int i = 0; i < (int)csv_conc.data.size (); i++)
    {
      if (csv_conc.data[i].size () > 1 && csv_conc.data[i][0] == avg_name)
        {
...
void
US_Hydrodyn_Mals_Saxs::conc_avg (QStringList files)
{
  // create average of selected
  vector<QString> avg_qs_string;
  vector<double> avg_qs;
      ...
      if (!inv_concs.count (this_file))
        {
          editor_msg (
              "red",
              QString (us_tr ("Error: found zero or no concentration for %1"))
                  .arg (this_file));
          return;
        }
      if (first)
  ...
  for (int i = 0; i < (int)avg_qs.size (); i++)
    {
      ...
      for (int j = 0; j < (int)files.size (); j++)
        {
          ...
          double weight
              = all_nonzero_errors
                    ? 1e0 / (t_errors[this_file][i] * t_errors[this_file][i])
                    : 1e0;
          double invweight
              = all_nonzero_errors
                    ? (t_errors[this_file][i] * t_errors[this_file][i])
                    : 1e0;
          sum += weight
                 * (invweight * inv_concs[this_file] * t_Is[this_file][i]
                    - avg_Is[i])
                 * (invweight * inv_concs[this_file] * t_Is[this_file][i]
                    - avg_Is[i]);
        }
      ...
      avg_sd[i] = avg_conc
                  * sqrt (sum
                          / (((double)files.size () - 1e0)
                             * (sum_weight[i] / (double)files.size ())));
...
void
US_Hydrodyn_Mals_Saxs::select_nth ()
{
  map<QString, QString> parameters;

  parameters["mals_saxs_nth_contains"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("mals_saxs_nth_contains")
            ? ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_nth_contains"]
            : "";

  US_Hydrodyn_Mals_Saxs_Nth *mals_saxs_nth
      = new US_Hydrodyn_Mals_Saxs_Nth (this, &parameters, this);
  US_Hydrodyn::fixWinButtons (mals_saxs_nth);
  mals_saxs_nth->exec ();
  delete mals_saxs_nth;
  ...
  ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_nth_contains"]
      = parameters.count ("mals_saxs_nth_contains")
            ? parameters["mals_saxs_nth_contains"]
            : "";
...
void
US_Hydrodyn_Mals_Saxs::axis_y (bool nochange, bool no_replot)
{
  if (!nochange)
    {
  ...
  if (axis_y_log)
    {
      ...
      plot_dist->setAxisScaleEngine (QwtPlot::yLeft,
    ...
    case MODE_PM:
      {
        ...
        gauss_add_marker (le_pm_q_start->text ().toDouble (), Qt::red,
                          us_tr ("Start"));
        gauss_add_marker (le_pm_q_end->text ().toDouble (), Qt::red,
...
void
US_Hydrodyn_Mals_Saxs::axis_x (bool nochange, bool no_replot)
{
  if (!nochange)
    {
  ...
  if (axis_x_log)
    {
      ...
      plot_dist->setAxisScaleEngine (QwtPlot::xBottom,
                                     new QwtLogScaleEngine (10));
      plot_errors->setAxisTitle (QwtPlot::xBottom,
                                 title + us_tr (" (log scale)"));
      plot_errors->setAxisScaleEngine (QwtPlot::xBottom,
                                       new QwtLogScaleEngine (10));
    }
  else
    {
...
US_Hydrodyn_Mals_Saxs::options ()
{
  ...
  parameters["mals_saxs_bl_linear"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_bl_linear"];
  parameters["mals_saxs_bl_integral"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_bl_integral"];
  parameters["mals_saxs_bl_save"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_bl_save"];
  parameters["mals_saxs_bl_smooth"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_bl_smooth"];
  parameters["mals_saxs_bl_start_region"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_bl_start_region"];
  parameters["mals_saxs_bl_i_power"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_bl_i_power"];
  parameters["mals_saxs_bl_reps"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_bl_reps"];
  parameters["mals_saxs_bl_epsilon"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_bl_epsilon"];
  parameters["mals_saxs_cormap_maxq"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_cormap_maxq"];
  parameters["mals_saxs_cormap_alpha"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_cormap_alpha"];
  parameters["mals_saxs_zi_window"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_zi_window"];
  parameters["mals_saxs_discard_it_sd_mult"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_discard_it_sd_mult"];
  parameters["mals_saxs_cb_discard_it_sd_mult"]
      = ((US_Hydrodyn *)us_hydrodyn)
            ->gparams["mals_saxs_cb_discard_it_sd_mult"];
  parameters["mals_saxs_guinier_qrgmax"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_guinier_qrgmax"];
  parameters["mals_saxs_cb_guinier_qrgmax"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_cb_guinier_qrgmax"];
  parameters["mals_saxs_gg_smooth"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_gg_smooth"];
  parameters["mals_saxs_cb_gg_smooth"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_cb_gg_smooth"];
  parameters["mals_saxs_cb_gg_cyclic"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_cb_gg_cyclic"];
  parameters["mals_saxs_cb_gg_oldstyle"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_cb_gg_oldstyle"];
  parameters["mals_saxs_dist_max"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_dist_max"];
  parameters["guinier_mwt_k"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["guinier_mwt_k"];
  parameters["guinier_mwt_c"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["guinier_mwt_c"];
  parameters["guinier_mwt_qmax"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["guinier_mwt_qmax"];

  parameters["mals_saxs_csv_transposed"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_csv_transposed"];

  parameters["mals_saxs_ampl_width_min"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_ampl_width_min"];
  parameters["mals_saxs_lock_min_retry"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_lock_min_retry"];
  parameters["mals_saxs_lock_min_retry_mult"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_lock_min_retry_mult"];
  parameters["mals_saxs_maxfpk_restart"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_maxfpk_restart"];
  parameters["mals_saxs_maxfpk_restart_tries"]
      = ((US_Hydrodyn *)us_hydrodyn)
            ->gparams["mals_saxs_maxfpk_restart_tries"];
  parameters["mals_saxs_maxfpk_restart_pct"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_maxfpk_restart_pct"];

  parameters["mals_saxs_makeiq_cutmax_pct"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_makeiq_cutmax_pct"];
  parameters["mals_saxs_cb_makeiq_cutmax_pct"]
      = ((US_Hydrodyn *)us_hydrodyn)
            ->gparams["mals_saxs_cb_makeiq_cutmax_pct"];
  parameters["mals_saxs_cb_makeiq_avg_peaks"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_cb_makeiq_avg_peaks"];
  parameters["mals_saxs_makeiq_avg_peaks"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_makeiq_avg_peaks"];

  parameters["mals_saxs_interp_method"]
      = ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_interp_method"];

  parameters["mals_saxs_csv_transposed"]
      = ((US_Hydrodyn *)us_hydrodyn)
                ->gparams.count ("mals_saxs_csv_transposed")
            ? ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_csv_transposed"]
            : "false";

  parameters["gaussian_type"] = QString ("%1").arg (gaussian_type);
  mals_saxs_options_widget = new US_Hydrodyn_Mals_Saxs_Options (
      &parameters, (US_Hydrodyn *)us_hydrodyn, this);
  US_Hydrodyn::fixWinButtons (
      (US_Hydrodyn_Mals_Saxs_Options *)mals_saxs_options_widget);
  ((US_Hydrodyn_Mals_Saxs_Options *)mals_saxs_options_widget)->exec ();
  delete (US_Hydrodyn_Mals_Saxs_Options *)mals_saxs_options_widget;
  ...
  if (((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_bl_linear"]
          != parameters["mals_saxs_bl_linear"]
      || ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_bl_integral"]
             != parameters["mals_saxs_bl_integral"]
      || ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_cormap_maxq"]
  ...
  cb_guinier_qrgmax->setChecked (parameters["mals_saxs_cb_guinier_qrgmax"]
                                 == "true");
...
void
US_Hydrodyn_Mals_Saxs::clear_gaussians ()
{
  unified_ggaussian_ok = false;
  f_gaussians.clear ();
  gaussians.clear ();
...
bool
US_Hydrodyn_Mals_Saxs::any_gaussians ()
{
  return f_gaussians.size () || gaussians.size () || org_gaussians.size ()
         || org_f_gaussians.size ();
}

void
US_Hydrodyn_Mals_Saxs::hide_files ()
{
  if (U_EXPT)
    {
  ...
  ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_files_widgets"]
      = files_widgets[0]->isVisible () ? "visible" : "hidden";
}

void
US_Hydrodyn_Mals_Saxs::hide_created_files ()
{
  if (U_EXPT)
    {
      hide_widgets (created_files_expert_widgets,
                    created_files_widgets[0]->isVisible ());
    }
  hide_widgets (created_files_widgets, created_files_widgets[0]->isVisible ());

  ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_created_files_widgets"]
      = created_files_widgets[0]->isVisible () ? "visible" : "hidden";
}

void
US_Hydrodyn_Mals_Saxs::hide_editor ()
{
  ...
  ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_editor_widgets"]
      = editor_widgets[0]->isVisible () ? "visible" : "hidden";
}

void
US_Hydrodyn_Mals_Saxs::svd ()
{
  disable_all ();
...

  {
    ...
    if (any_differences)
      {
        editor_msg ("red", us_tr ("SVD: curves must be on the same grid, try "
                                  "'Crop Common' first."));
        update_enables ();
        return;
      }
  ...
  new US_Hydrodyn_Mals_Saxs_Svd (this, selected_files);
}

void
US_Hydrodyn_Mals_Saxs::line_width ()
{
  ...
  ((US_Hydrodyn *)us_hydrodyn)->gparams["mals_saxs_line_width"]
      = QString ("%1").arg (use_line_width);
...
void
US_Hydrodyn_Mals_Saxs::color_rotate ()
{
  US_Plot_Zoom upz (plot_dist, plot_dist_zoomer);
...
void
US_Hydrodyn_Mals_Saxs::movie ()
{
  disable_all ();

  US_Hydrodyn_Mals_Saxs_Movie *shm = new US_Hydrodyn_Mals_Saxs_Movie (this);
  ...
  if (plot_ref->isVisible () || lbl_conc_file->text ().isEmpty ())
  ...
  else
    {
      ...
      if (!f_qs.count (lbl_conc_file->text ()))
        {
          plot_ref->hide ();
          editor_msg ("dark red", us_tr ("Warning: Reference plot selected "
                                         "but no concentration file found"));
          update_enables ();
          return;
        }
      QStringList files = all_selected_files ();
      if (!f_time.count (last_selected_file))
        {
          plot_ref->hide ();
          editor_msg ("dark red",
                      us_tr ("Warning: no time known for this curve"));
          update_enables ();
          return;
        }
...
void
US_Hydrodyn_Mals_Saxs::update_ref ()
{
  if (!plot_ref->isVisible ())
    {
      return;
  ...
  plot_ref->setMarkerLabelText (
      ref_marker, QString ("%1").arg (f_time[last_selected_file]));
#else
...
double
US_Hydrodyn_Mals_Saxs::tot_intensity (QString &file, double q_min,
                                      double q_max)
{
  if (!f_qs_string.count (file) || !f_qs.count (file) || !f_Is.count (file)
      || !f_pos.count (file))
    {
      editor_msg (
          "red",
          QString (
              us_tr (
                  "Total intensity compute internal error: no curve named %1"))
              .arg (file));
      return -9e99;
    }
  ...
  for (int i = 0; i < (int)f_qs[file].size (); ++i)
    {
      if (f_qs[file][i] >= q_min && f_qs[file][i] <= q_max)
        {
          tot_i += f_Is[file][i];
        }
    }
...
bool
US_Hydrodyn_Mals_Saxs::set_selected (const QStringList &to_select_qsl,
                                     bool do_replot)
{
...
bool
US_Hydrodyn_Mals_Saxs::set_selected (const set<QString> &to_select,
                                     bool do_replot)
{
  disable_updates = true;
  lb_files->clearSelection ();
      ...
      if (to_select.count (lb_files->item (i)->text ()))
        {
          ...
          // qDebug() << "set_selected() : to plot " << lb_files->item( i
          // )->text();
        }
    }
...
bool
US_Hydrodyn_Mals_Saxs::set_created_selected (const set<QString> &to_select,
                                             bool do_replot)
{
  disable_updates = true;
...
void
US_Hydrodyn_Mals_Saxs::artificial_gaussians ()
{
  // based upon gaussian mode, generate a bunch of I(t) with an arbitrary
  // center, width and distortions
  ...
  bool ipe = (QMessageBox::Yes
              == QMessageBox::question (
                  this, windowTitle () + us_tr (": Artificial Gaussians"),
  ...
  for (int i = 0; i < (int)f_Is[wheel_file].size (); ++i)
    {
      ...
      add_plot (QString ("%1_ag_%2_q%3")
                    .arg (wheel_file)
                    .arg (QString ("%1")
                              .arg (gaussian_type_tag)
                              .toLower ()
                              .replace ("+", ""))
                    .arg (f_qs[wheel_file][i])
                    .replace (".", "_"),
                t, I, e, true, false);
    }
  update_enables ();
...
bool
US_Hydrodyn_Mals_Saxs::constant_e (QString file, double e)
{
  if (!f_qs.count (file))
    {
      return false;
  ...
  f_gaussians[use_sde_name]
      = f_gaussians.count (file) ? f_gaussians[file] : vector<double> (0);
...
int
US_Hydrodyn_Mals_Saxs::slice (const vector<double> &q, const vector<double> &I,
                              double start_q, double end_q,
                              vector<double> &new_q, vector<double> &new_I)
{
  new_q.clear ();
  new_I.clear ();
  if (q.size () != I.size ())
...
double
US_Hydrodyn_Mals_Saxs::wyatt_errors (QString file, double start_q,
                                     double end_q)
{
  if (!f_qs.count (file))
    {
...
double
US_Hydrodyn_Mals_Saxs::wyatt_errors (QString file, double start_q,
                                     double end_q, vector<double> &new_q,
                                     vector<double> &new_I,
                                     vector<double> &new_y)
{
  if (!f_qs.count (file))
  ...
  return wyatt_errors (f_qs[file], f_Is[file], start_q, end_q, new_q, new_I,
                       new_y);
}

double
US_Hydrodyn_Mals_Saxs::wyatt_errors (const vector<double> &q,
                                     const vector<double> &I, double start_q,
                                     double end_q)
{
  vector<double> new_q;
  vector<double> new_I;
                                     ...
                                     vector<double> &new_y)
{
  ...
  // compute SVD of M (see: us_hydrodyn_mals_saxs_svd.cpp and us_svd.cpp for
  // usage)
  ...
  // editor_msg( "blue", us_tr( "Wyatt errors: SVD: matrix M created, computing
  // SVD" ) );
...
void
US_Hydrodyn_Mals_Saxs::plot_debug ()
{
      ...
      if (curves)
        {
          ...
          it->second->axisWidget (QwtPlot::yLeft)
              ->getBorderDistHint (yLeftStartDist, yLeftEndDist);
          it->second->axisWidget (QwtPlot::yRight)
              ->getBorderDistHint (yRightStartDist, yRightEndDist);
          ...
          tso << QString ("plot for %1\n").arg (it->first)
              << QString ().sprintf (
                     ...
                     "plotLayout()->canvasRect()                         %g, "
                     "%g\n"
                     "plotLayout()->titleRect()                          %g, "
                     "%g\n"
                     "plotLayout()->footerRect()                         %g, "
                     "%g\n"
                     "plotLayout()->legendRect()                         %g, "
                     "%g\n"

                     "plotLayout()->scaleRect( yLeft )                   %g, "
                     "%g\n"
                     "plotLayout()->scaleRect( yRight )                  %g, "
                     "%g\n"

                     "plotLayout()->canvasMargin( yLeft )                %d\n"
                     "plotLayout()->canvasMargin( yRight )               %d\n"

                     "axisWidget( yLeft )->getBorderDistHint( yLeft )    %d, "
                     "%d\n"
                     "axisWidget( yRight )->getBorderDistHint( yRight )  %d, "
                     "%d\n"
                     ...
                     ,
                     (it->second->plotLayout ()->alignCanvasToScale (
                          QwtPlot::yLeft)
                          ? "True"
                          : "False"),
                     (it->second->plotLayout ()->alignCanvasToScale (
                     ...
                     it->second->plotLayout ()->legendRect ().y ()
                         ...
                         ,
                     it->second->plotLayout ()
                         ->scaleRect (QwtPlot::yLeft)
                         .x (),
                     it->second->plotLayout ()->scaleRect (QwtPlot::yLeft).y ()

                         ,
                     it->second->plotLayout ()
                         ->scaleRect (QwtPlot::yRight)
                         .x (),
                     it->second->plotLayout ()
                         ->scaleRect (QwtPlot::yRight)
                         .y ()
                     ...
                     it->second->plotLayout ()->canvasMargin (QwtPlot::yRight)
                         ...
                         ,
                     yLeftStartDist, yLeftEndDist

                     ,
                     yRightStartDist, yRightEndDist

                     ,
                     it->second->axisWidget (QwtPlot::yLeft)
                         ->scaleDraw ()
                         ->extent (
                             it->second->axisWidget (QwtPlot::yLeft)->font ()),
                     it->second->axisWidget (QwtPlot::yRight)
                         ->scaleDraw ()
                         ->extent (it->second->axisWidget (QwtPlot::yRight)
                                       ->font ()),
                     (it->second->plotLayout ()->canvasMargin (QwtPlot::yLeft)
                      + it->second->plotLayout ()->canvasMargin (
                          QwtPlot::yRight)
                      + it->second->plotLayout ()
                            ->scaleRect (QwtPlot::yLeft)
                            .x ()
...
bool
US_Hydrodyn_Mals_Saxs::ask_to_decimate (map<QString, QString> &parameters)
{
  switch (QMessageBox::warning (
      this,
      windowTitle ()
          + us_tr (" : Optional PVP Analysis Correlation Correction"),
      us_tr ("Warning: datasets having finely spaced q-values might exhibit "
             "cross-correlation issues.\n"
             "Sampling one every few q points will alleviate this problem.\n"
             "Would you like to?"),
      us_tr ("&Sample alternate q points"),
      us_tr ("&Specify a larger gap in q points"), us_tr ("&Continue"),
      2 // Default continue
      ,
      -1 //
      ))
  ...
  int result = US_Static::getInteger (
      windowTitle ()
          + us_tr (" : Optional PVP Analysis Correlation Correction"),
      us_tr ("Sample one q point out of every:"), 3, 3, 10, 1, &ok, this);
  if (!ok)
    {
      return false;
    }
...
bool
US_Hydrodyn_Mals_Saxs::ask_cormap_minq (map<QString, QString> &parameters)
{
  bool ok;
  double result = US_Static::getDouble (
      windowTitle () + us_tr (" : Optional PVP Analysis q minimum cutoff"),
      us_tr ("If you have noisy low q data, you may wish to eliminate these "
             "points\n"
             "from PVP analysis by setting a non-zero minimum q value here:"),
      0, 0,
      parameters.count ("cormap_maxq")
          ? parameters["cormap_maxq"].toDouble () - 0.01
          : 0.04,
      4, &ok, this);
  if (!ok)
    {
...
void
US_Hydrodyn_Mals_Saxs::cormap (map<QString, QString> &parameters)
{
  disable_all ();
  ...
  if (!parameters.count ("alpha"))
    {
      parameters["alpha"] = QString ("%1").arg (
          ((US_Hydrodyn *)us_hydrodyn)
                  ->gparams.count ("mals_saxs_cormap_alpha")
              ? ((US_Hydrodyn *)us_hydrodyn)
                        ->gparams["mals_saxs_cormap_alpha"]
                        .toDouble ()
                    * 5
              : 0.05);
    }
  ...
  double cormap_maxq
      = ((US_Hydrodyn *)us_hydrodyn)->gparams.count ("mals_saxs_cormap_maxq")
            ? ((US_Hydrodyn *)us_hydrodyn)
                  ->gparams["mals_saxs_cormap_maxq"]
                  .toDouble ()
            : 0.05;
  ...
  switch (current_mode)
    ...
    case MODE_SCALE:
      {
        if (!scale_selected.size () || !scale_selected_names.size ())
          {
        ...
        int decimate = parameters.count ("decimate")
                           ? parameters["decimate"].toInt ()
                           : 0;
        double cormap_minq = parameters.count ("cormap_minq")
                                 ? parameters["cormap_minq"].toDouble ()
                                 : 0e0;

        // if files are time, then grab alternates, otherwise grab alternate q
        // points
        if (!f_is_time.count (scale_selected_names[0]))
          {
            editor_msg ("red", us_tr ("Internal error: PVP in scale mode "
                                      "files not in global files"));
            scale_enables ();
            return;
          }
        ...
        if (files_are_time && decimate)
          {
            for (int i = 0; i < (int)scale_selected_names.size ();
                 i += decimate)
              {
                use_preq_scale_selected_names.push_back (
                    scale_selected_names[i]);
              }
          }
        else
          {
        ...
        if (files_are_time)
          {
            ...
            for (int i = 0; i < (int)use_preq_scale_selected_names.size ();
                 ++i)
              {
                if (rx_cap.indexIn (use_preq_scale_selected_names[i]) == -1)
                  {
                    // QMessageBox::warning( this
                    //                       , windowTitle() + us_tr( " : PVP
                    //                       Analysis" ) , QString( us_tr(
                    //                       "Could not extract q value from
                    //                       file name %1" ) ) .arg(
                    //                       use_preq_scale_selected_names[ i ]
                    //                       ) ,QMessageBox::Ok |
                    //                       QMessageBox::Default
                    ...
                    editor_msg (
                        "red",
                        QString (us_tr ("PVP Analysis: Could not extract q "
                                        "value from file name %1"))
                            .arg (use_preq_scale_selected_names[i]));
                    scale_enables ();
                    return;
                  }
                double qv = rx_cap.cap (1).replace ("_", ".").toDouble ();
                // us_qdebug( QString( "baseline cormap captured %1 from %2"
                // ).arg( qv ).arg( use_preq_scale_selected_names[ i ] ) );
                if (qv <= cormap_maxq && qv >= cormap_minq)
                  {
                    use_scale_selected_names.push_back (
                        use_preq_scale_selected_names[i]);
                  }
              }
          }
        ...
        if (use_scale_selected_names.size () < 2)
          {
            editor_msg (
                "red",
                us_tr ("Insufficient curves remaining for PVP Analysis"));
            scale_enables ();
            return;
          }
        ...
        if (!files_are_time)
          {
            ...
            for (int i = 0; i < fcount; ++i)
              {
                ...
                if (use_names_max_len
                    < (int)use_scale_selected_names[i].length ())
                  {
                    use_names_max_len
                        = (int)use_scale_selected_names[i].length ();
                  }
            ...
            parameters["msg"]
                = QString ("%1\t%2\t    N  Start point  C   P-value\n")
                ...
                for (int j = i + 1; j < fcount; ++j)
                  {
                    ...
                    parameters["msg"] += QString ("%1\t%2\t%3\t%4\t%5\t%6"
                                                  // "\t%7"
                                                  "\n")
                                             .arg (use_scale_selected_names[i],
                                                   -use_names_max_len)
                                             .arg (use_scale_selected_names[j],
                                                   -use_names_max_len)
                                             .arg (N, 6)
                                             .arg (S, 6)
                                             .arg (C, 6)
...

            {
              parameters["title"]
                  = QString (
                        us_tr (
                            "Scale mode %1: Full q range used for analysis."))
                        .arg (scale_applied
              ...
              if (cormap_minq > 0e0)
                {
                  parameters["title"]
                      += QString (" Minimum q limit is %1 [A^-1].")
                             .arg (cormap_minq);
                  parameters["title_adj"]
                      += QString (" Minimum q limit is %1 [A^-1].")
                             .arg (cormap_minq);
                  ...
                  if (decimate > 1)
                    {
                      ...
                      parameters["title"]
                          += QString (
                                 us_tr (" Only every %1%2 q value sampled."))
                                 .arg (decimate)
                                 .arg (nth_tag);
                      parameters["title_adj"]
                          += QString (
              ...
              // parameters[ "name"              ] =
              // use_scale_selected_names.front();
              {
                QStringList qsl
                    = vector_qstring_to_qstringlist (use_scale_selected_names);
                QString head = qstring_common_head (qsl, true);
                QString tail = qstring_common_tail (qsl, true);
                parameters["name"]
                    = QString ("%1_%2%3_%4%5_cqmn%6_mx%7")
                          .arg (head)
                          .arg (tail)
                          .arg (parameters.count ("decimate")
                                    ? QString ("_s%1").arg (
                                          parameters["decimate"])
                                    : QString (""))
              ...
              US_Hydrodyn_Saxs_Cormap *uhcm = new US_Hydrodyn_Saxs_Cormap (
                  us_hydrodyn, parameters, pvaluepairs, adjpvaluepairs,
        ...
        // once for maxq cutoff
        {
...

          {
            ...
            for (int i = 0; i < (int)use_scale_selected_names.size (); ++i)
              {
                ...
                if (scale_q.count (this_file) && scale_I.count (this_file)
            ...
            if (any_differences)
              {
                editor_msg ("red", us_tr ("PVP: curves must be on the same "
                                          "grid, try 'Crop Common' first."));
                scale_enables ();
                return;
              }
          }
          ...
          if (do_q_decimate)
            {
              ...
              for (int k = 0; k < q_points; k += use_decimate)
                {
                  if (q[k] <= cormap_maxq && q[k] >= cormap_minq)
                    {
                      new_q.push_back (q[k]);
                    }
                }
    ...
    case MODE_BLANKS:
      {
        ...
        {
          // QRegExp rx_cap( "_It_q(\\d+_\\d+)" );
          int use_decimate = parameters.count ("decimate")
                                 ? parameters["decimate"].toInt ()
                                 : 1;
          for (int i = 0; i < (int)use_preq_blanks_created.size (); ++i)
            {
              ...
              //    editor_msg( "red",
              //                QString( us_tr( "PVP Analysis: Could not
              //                extract q value from file name %1" ) ) .arg(
              //                use_preq_blanks_created[ i ] ) );
              //    baseline_enables();
              //    return;
              // }
              // double qv = rx_cap.cap( 1 ).replace( "_", "." ).toDouble();
              // us_qdebug( QString( "qv %1 blanks_created_q[ i ] %2" ).arg( qv
              // ).arg( blanks_created_q[ i * use_decimate ] ) );

              // if ( qv <= cormap_maxq ) {
              if (blanks_created_q[(vector<double>::size_type)i * use_decimate]
                      <= cormap_maxq
                  && blanks_created_q[(vector<double>::size_type)i
                                      * use_decimate]
                         >= cormap_minq)
                {
        ...
        for (int i = 0; i < (int)f_qs[wheel_file].size (); ++i)
          {
            if (f_qs[wheel_file][i] >= q_start && f_qs[wheel_file][i] <= q_end)
              {
        ...
        if (!t.size ())
          {
            editor_msg (
                "red",
                us_tr ("PVP in blanks mode has empty start / end range"));
            blanks_enables ();
            return;
          }
        ...
        // us_qdebug( QString( "cormap blanks blanks_size %1" ).arg(
        // blanks_size ) );
        ...
        //        " ) .arg( fcount ) .arg( indicies.size() ) .arg( blanks_size
        //        ) .arg( m )
        //         );
          ...
          for (int k = 0; k < blanks_size; ++k)
            {
              if (!f_errors.count (use_blanks_created[k])
                  || f_errors[use_blanks_created[k]].size ()
                         != f_Is[use_blanks_created[k]].size ()
                  ...
                  if (!skip_k.count (k))
                    {
                      ...
                      //         .arg( f_qs[ use_blanks_created[ k ] ][
                      //         indicies[ i ] ] ) .arg( f_errors[
                      //         use_blanks_created[ k ] ][ indicies[ i ] ]  )
                      //         );
                    }
                }
            ...
            for (int j = i + 1; j < fcount; ++j)
              {
                ...
                // US_Vector::printvector2( QString( "blanks for cormap i %1 j
                // %2 I" ).arg( i ).arg( j ), I[ 0 ], I[ 1 ] );
                ...
                parameters["msg"]
                    += QString ("%1\t%2\t%3\t%4\t%5\t%6"
                           ...
                           .arg (C, 6)
                           .arg (
                               QString ("").sprintf ("%.4g", P).leftJustified (
                                   12))
                    // .arg( adjP )
                    ;
              }
        ...
        {
          parameters["title"]
              = QString (us_tr ("Blanks mode t %1 to %2. Maximum q limit is "
                                "%3 [A^-1]."))
                    .arg (q_start)
                    .arg (q_end)
                    .arg (cormap_maxq);
          ...
          parameters["csv_id_data"]
              = QString ("%1,%2,%3").arg (q_start).arg (q_end).arg (fcount);
          parameters["global_width"] = "1000";
          parameters["global_height"] = "700";
          parameters["image_height"] = "300";

          parameters["title_adj"] = QString (us_tr ("Blanks mode t %1 to %2."))
                                        .arg (q_start)
                                        .arg (q_end);

          if (cormap_minq > 0e0)
            {
              parameters["title"] += QString (" Minimum q limit is %1 [A^-1].")
                                         .arg (cormap_minq);
              parameters["title_adj"]
                  += QString (" Minimum q limit is %1 [A^-1].")
                         .arg (cormap_minq);
              ...
              if (decimate > 1)
                {
                  ...
                  parameters["title"]
                      += QString (us_tr (" Only every %1%2 q value sampled."))
                             .arg (decimate)
                             .arg (nth_tag);
                  parameters["title_adj"]
                      += QString (us_tr (" Only every %1%2 q value sampled."))
          ...
          // parameters[ "name"              ] = blanks_created.front();
          {
            ...
            parameters["name"]
                = QString ("%1_%2%3_f%4_cqmx%5")
                      .arg (head)
                      .arg (tail)
                      .arg (parameters.count ("decimate")
                                ? QString ("_s%1").arg (parameters["decimate"])
                                : QString (""))
                      .arg (pvaluepairs.size ())
                      .arg (cormap_maxq)
                      .replace (".", "_");
          ...
          US_Hydrodyn_Saxs_Cormap *uhcm = new US_Hydrodyn_Saxs_Cormap (
              us_hydrodyn, parameters, pvaluepairs, adjpvaluepairs, use_names);
          blanks_last_cormap_parameters = parameters;
          blanks_last_cormap_pvaluepairs = pvaluepairs;
          blanks_last_cormap_run_end_s
              = le_baseline_end_s->text ().toDouble ();
          blanks_last_cormap_run_end_e
              = le_baseline_end_e->text ().toDouble ();
    ...
    case MODE_BASELINE:
      {
          ...
          for (int i = 0; i < (int)use_preq_baseline_selected.size (); ++i)
            {
              if (rx_cap.indexIn (use_preq_baseline_selected[i]) == -1)
                {
                  // QMessageBox::warning( this
                  //                       , windowTitle() + us_tr( " :
                  //                       Baseline PVP Analysis" ) , QString(
                  //                       us_tr( "Could not extract q value
                  //                       from file name %1" ) ) .arg(
                  //                       use_preq_baseline_selected[ i ] )
                  //                       ,QMessageBox::Ok |
                  //                       QMessageBox::Default
                  //                       ,QMessageBox::NoButton
                  //                       );
                  editor_msg (
                      "red",
                      QString (us_tr ("Baseline PVP Analysis: Could not "
                                      "extract q value from file name %1"))
              ...
              double qv = rx_cap.cap (1).replace ("_", ".").toDouble ();
              // us_qdebug( QString( "baseline cormap captured %1 from %2"
              // ).arg( qv ).arg( use_preq_baseline_selected[ i ] ) );
              if (qv <= cormap_maxq && qv >= cormap_minq)
                {
        ...
        double q_start = le_baseline_end_s->text ().toDouble ();
        double q_end = parameters.count ("baseline_use_end")
        ...
        if (!t.size ())
          {
            editor_msg (
                "red",
                us_tr ("PVP in baseline mode has empty start / end range"));
            baseline_enables ();
            return;
          }
        ...
        // us_qdebug(
        //        QString( "mb5 fcount %1 indicies.size %2 baseline.size %3 m
        //        %4 " ) .arg( fcount ) .arg( indicies.size() ) .arg(
        //        baseline_size ) .arg( m )
        //         );
            ...
            for (int j = i + 1; j < fcount; ++j)
              {
                ...
                // US_Vector::printvector2( QString( "for cormap i %1 j %2 I"
                // ).arg( i ).arg( j ), I[ 0 ], I[ 1 ] );
        ...
        {
          parameters["title"]
              = QString (us_tr ("Baseline mode t %1 to %2. Maximum q limit is "
                                "%3 [A^-1]."))
                    .arg (q_start)
                    .arg (q_end)
                    .arg (cormap_maxq);
          ...
          parameters["title_adj"]
              = QString (us_tr ("Baseline mode t %1 to %2."))
                    .arg (q_start)
                    .arg (q_end);
    ...
    case MODE_GGAUSSIAN:
      {
        if (!unified_ggaussian_ok)
          {
            editor_msg ("red",
                        us_tr ("Internal error (PVP): Global Gaussian mode, "
                               "but unified Global Gaussians are not ok."));
            ggaussian_enables ();
            return;
          }
            ...
            || lbl_gauss_fit->text () == "?" || pb_ggauss_rmsd->isEnabled ())
          {
            if (ggauss_recompute ())
            ...
            else
              {
                editor_msg (
                    "red",
                    us_tr ("Internal error (PVP) building global Gaussians"));
                ggaussian_enables ();
                return;
              }
          }
        ...
        if ((int)unified_ggaussian_files.size ()
                   ...
                   != (int)ggaussian_last_chi2.size ())
          {
            editor_msg (
                "red",
                QString (us_tr ("Internal error (PVP): Global Gaussian mode, "
                                "last_pfit_* size (%1 %2 %3 %4 %5) does not "
                                "match number number of Gaussians (%6)"))
        ...
        parameters["msg"]
            = QString (" Ref. : %1\t   N  Start point  C   P-value\n")
                  .arg ("File", -use_names_max_len);
        ...
        for (int i = 0; i < fcount; ++i)
          {
            ...
            parameters["msg"]
                += QString ("%1 : %2\t%3\t%4\t%5\t%6"
                       ...
                       .arg (QString ("")
                                 .sprintf ("%.4g", ggaussian_last_pfit_P[i])
                                 .leftJustified (12))
                // .arg( adjP )
                ;
          }

        {
          ...
          parameters["title"]
              = QString (us_tr ("Global Gaussian mode t range %1 to %2."))
          ...
          US_Hydrodyn_Saxs_Cormap *uhcm = new US_Hydrodyn_Saxs_Cormap (
              us_hydrodyn, parameters, pvaluepairs, pvaluepairs,
              selected_files);
          if (parameters.count ("close"))
            {
              delete uhcm;
    ...
    default:
      {
        ...
        if (!f_is_time.count (selected_files[0]))
          {
            editor_msg (
                "red",
                us_tr ("Internal error: PVP files not in global files"));
            update_enables ();
            return;
          }
            ...
            for (int i = 0; i < (int)use_preq_selected_files.size (); ++i)
              {
                ...
                double qv = rx_cap.cap (1).replace ("_", ".").toDouble ();
                // us_qdebug( QString( "baseline cormap captured %1 from %2"
                // ).arg( qv ).arg( use_preq_selected_files[ i ] ) );
                if (qv <= cormap_maxq && qv >= cormap_minq)
                  {
        ...
        if (do_q_decimate)
          {
            // us_qdebug( QString( "do_q_decimate use_decimate %1 q_points %2"
            // ).arg( use_decimate ).arg( q_points ) );
            vector<double> new_q;
            for (int k = 0; k < q_points; k += use_decimate)
              {
                if (q[k] <= cormap_maxq && q[k] >= cormap_minq)
        ...
        // US_Vector::printvector2( QString( "cormap minq %1 maxq %2 q
        // undecimated_q" ).arg( cormap_minq ).arg( cormap_maxq ), q,
        // undecimated_q );
            ...
            for (int j = i + 1; j < fcount; ++j)
              {
                ...
                // US_Vector::printvector2( QString( "default for cormap i %1 j
                // %2 I" ).arg( i ).arg( j ), I[ 0 ], I[ 1 ] );
...

        {
          ...
          {
            QStringList qsl
                = vector_qstring_to_qstringlist (use_selected_files);
            QString head = qstring_common_head (qsl, true);
            QString tail = qstring_common_tail (qsl, true);
            parameters["name"]
                = QString ("%1_%2%3_%4%5_cqmx%6")
                      .arg (head)
                      .arg (tail)
                      .arg (parameters.count ("decimate")
                                ? QString ("_s%1").arg (parameters["decimate"])
...
void
US_Hydrodyn_Mals_Saxs::bin (QStringList files)
{
  bool ok;
  int binning = US_Static::getInteger (
      us_tr ("SOMO: HPLC enter binning"),
      us_tr ("Enter the number of points of binning:"), 2, 2, 10, 1, &ok,
      this);
  if (!ok)
    {
      return;
  ...
  for (unsigned int i = 0; i < (unsigned int)files.size (); i++)
    {
      ...
      while (current_files.count (binned_name))
        {
          binned_name
              = files[i] + QString ("-bin%1-%2").arg (binning).arg (++ext);
        }

      if (usu.bin (f_qs[files[i]], f_Is[files[i]], f_errors[files[i]],
                   binned_q, binned_I, binned_e, binning))
        {
        ...
        {
          editor_msg ("red",
                      QString ("Error: binning error trying to create %1\n")
                          .arg (binned_name));
        }
    }
...
void
US_Hydrodyn_Mals_Saxs::bb_cm_inc ()
{
  // us_qdebug( "bb_cm_inc" );
  ...
  switch (QMessageBox::question (
      this, windowTitle () + us_tr (" : PVP analysis auto increment"),
      us_tr ("Choose your options for automatic running"),
      us_tr ("&Slide current window forward"),
      us_tr ("&Increment window size"), us_tr ("&Cancel"), 0 //
      ,
      -1 //
      ))
    {
  ...
  QString fn = QFileDialog::getSaveFileName (
      this, us_tr ("Choose a file name for the images"), QString (),
      "png (*.png *.PNG)");
  ...
  // us_qdebug( QString( "start %1 end %2 min %3 max %4" ).arg( start ).arg(
  // end ).arg( min ).arg( max ) );
  ...
  while (end <= max)
    {
      ...
      // us_qdebug( QString( "run test for start %1 end %2" ).arg( start ).arg(
      // end ) );
      parameters = save_parameters;
      if (!fn.isEmpty ())
        {
          parameters["save_png"]
              = fn + QString ("_fr%1_%2").arg (start).arg (end) + ".png";
...
bool
US_Hydrodyn_Mals_Saxs::blanks_params ()
{
  ...
  if (points < 10)
    {
      QMessageBox::warning (
          this, windowTitle () + us_tr (" : Analyze blanks"),
          QString (us_tr (
              "To properly analyze blanks requires a minimum of 10 blank "
              "frames.\n"
  ...
  for (int pts = points_start; pts <= points && pts <= points_end;
       pts += points_inc)
    {
      double start = f_qs[wheel_file].front ();
      double end = f_qs[wheel_file][pts - 1];
      int pos = 0;
      us_qdebug (QString ("blanks_params pts %1 start %2 end %3")
                     .arg (pts)
                     .arg (start)
                     .arg (end));
      ...
      while (pts + pos - 1 < points)
        {
          ...
          le_baseline_end_s->setText (QString ("%1").arg (start));
          le_baseline_end_e->setText (QString ("%1").arg (end));
          // us_qdebug( QString( "run test for start %1 end %2" ).arg( start
          // ).arg( end ) );
          parameters = save_parameters;
          parameters["data_csv"] = QString ("%1").arg (pts);
          // if ( !fn.isEmpty() ) {
          //    parameters[ "save_png" ] = fn + QString( "_fr%1_%2" ).arg(
          //    start ).arg( end ) + ".png"; parameters[ "save_csv" ] = fn +
          //    ".csv";
          // }
          cormap (parameters);
          save_parameters["csv_skip_report_header"] = "true";
  ...
  map<QString, map<QString, vector<QString> > > data_csv
      = ((US_Hydrodyn *)us_hydrodyn)->data_csv;
  QStringList data_csv_headers
      = ((US_Hydrodyn *)us_hydrodyn)->data_csv_headers;
      ...
      if (data_csv.count (data_csv_headers[i]))
        {
          for (map<QString, vector<QString> >::iterator it
               = data_csv[data_csv_headers[i]].begin ();
               it != data_csv[data_csv_headers[i]].end (); it++)
            {
              cout << QString ("   %1:\n").arg (it->first);
...
QStringList
US_Hydrodyn_Mals_Saxs::vector_qstring_to_qstringlist (vector<QString> vqs)
{
  QStringList qsl;
...
void
US_Hydrodyn_Mals_Saxs::simulate ()
{
  us_qdebug ("simulate");
...

  {
    ...
    if (any_differences)
      {
        editor_msg ("red", us_tr ("Simulate: curves must be on the same grid, "
                                  "try 'Crop Common' first."));
        update_enables ();
        return;
      }
  }
  ...
  US_Hydrodyn_Mals_Saxs_Simulate *uhshs = new US_Hydrodyn_Mals_Saxs_Simulate (
      us_hydrodyn, this, selected, q, I, e);
  uhshs->show ();
}

void
US_Hydrodyn_Mals_Saxs::gauss_mode ()
{
  map<QString, QString> parameters;
  ...
  parameters["gaussian_type"] = QString ("%1").arg (gaussian_type);
  US_Hydrodyn_Mals_Saxs_Gauss_Mode *sho
      = new US_Hydrodyn_Mals_Saxs_Gauss_Mode (
          &parameters, (US_Hydrodyn *)us_hydrodyn, this);
...
bool
US_Hydrodyn_Mals_Saxs::mals_saxs_load (const QString &filename,
                                       const QStringList &qsl,
                                       QString &errormsg)
{
  errormsg = "";

  // check if qsl is a mals_saxs data file, if not return false
  if (qsl.size () < 4
      || !qsl[0].contains (QRegularExpression ("^light scattering data:")))
    {
      errormsg = "not mals_saxs file";
      return false;
    }

  if (!qsl[1].contains (QRegularExpression ("^Rayleigh ratio"))
      || !qsl[2].contains (QRegularExpression ("^time \\(min")))
    {
      errormsg = "incorrect MALS_SAXS data format";
      return false;
    }
  ...
  if (!mals_saxs_angles.mals_saxs_angle.size ())
    {
      errormsg = "No MALS_SAXS Angles have been defined.\nLoad them prior to "
                 "loading MALS_SAXS data";
      return false;
    }

  if (!mals_saxs_param_lambda || !mals_saxs_param_n)
    {
      errormsg = "Options->MALS_SAXS parameters must be defined prior to "
                 "loading MALS_SAXS data";
      return false;
    }

  switch (QMessageBox::question (
      this, windowTitle () + us_tr (" : Load MALS_SAXS Data"),
      mals_saxs_angles.list_rich ()
          + QString ("<hr>"
                     "Proceed with these MALS_SAXS Angles?")))
    {
    case QMessageBox::Yes:
      break;
    default:
      errormsg = "MALS_SAXS Angles need to be loaded";
      return false;
      break;
    }

  double wavelength = mals_saxs_param_lambda;
  double RI = mals_saxs_param_n;

  // process data and make i(q)

  errormsg = "MALS_SAXS ready to process... to do";
  ...
  if (!detector_index.size ())
    {
      errormsg = "No detectors found in MALS_SAXS data";
      return false;
    }
...
#define TIME_DELTA_TOLERANCE 0.0000001
  ...
  while (data.size ()
         && data.front ().contains (QRegularExpression ("^-?[0-9.]+,")))
    {
      ...
      if ((int)detector_index.size () * 2 + 1 != data_line.size ())
        {
          TSO << QString ("detector index size %1 data line size %2\n")
                     .arg (detector_index.size ())
                     .arg (data_line.size ());
          errormsg
              = "error in data line format : " + data_line.join (",") + "\n";
          return false;
        }
  ...
  if (timedelta_inconsistencies)
    {
      QMessageBox::warning (
          this, "US-SOMO MALS_SAXS load",
          QString (
              us_tr (
                  "%1 inconsistent time interval(s) found in MALS_SAXS data\n"
  ...
  if (!avg_timedelta_count || !avg_timedelta_sum)
    {
      errormsg = "Could not determine a collection interval from the "
                 "MALS_SAXS data";
      return false;
    }
  ...
  if (!t[detector_index[0]].size ())
    {
      errormsg = "Could not determine collection times the MALS_SAXS data";
      return false;
    }

  bool adjust_times = false;
  double org_start_time_minutes = t[detector_index[0]][0];
  double org_collection_interval_minutes
      = avg_timedelta_sum / avg_timedelta_count;
  ...
  // adjust_times dialog
  {
    ...
    do
      {
        // US_Vector::printvector( QString( "mals_saxs detector %1 times"
        // ).arg( detector_index[0] ), t[detector_index[0]] );

        QDialog dialog (this);
        dialog.setWindowTitle (windowTitle ()
                               + us_tr (": MALS_SAXS load : Adjust times"));
        ...
        form.addRow (new QLabel (
            us_tr ("Optionally adjust the start time and collection interval\n"
        ...
        vector<QWidget *> widgets
            = { (QWidget *)0, (QWidget *)0, new QLineEdit (&dialog),
                new QLineEdit (&dialog) };

        vector<double> defaults = { 0, 0, 60 * org_start_time_minutes,
            ...
            if (widgets[i])
              {
                ...
                ((QLineEdit *)widgets[i])
                    ->setText (QString ("%1").arg (defaults[i]));
                fields << widgets[i];
              }
          }
        ...
        if (dialog.exec () == QDialog::Accepted)
          {
            if (((QLineEdit *)fields[1])->text ().toDouble () == 0)
              ...
              {
                adjust_times = true;
                // If the user didn't dismiss the dialog, do something with the
                // fields
                new_start_time_seconds
                    = ((QLineEdit *)fields[0])->text ().toDouble ();
                new_collection_interval_seconds
                    = ((QLineEdit *)fields[1])->text ().toDouble ();
                adjust_start_time_seconds
                    = new_start_time_seconds - org_start_time_minutes * 60;
                adjust_collection_interval_seconds
                    = new_collection_interval_seconds
                      / (60 * org_collection_interval_minutes);
                TSO << QString ("adjust start time seconds          %1\n"
  ...
  // use_filename.replace( QRegularExpression( "\\.csv$",
  ...
  for (auto it = t.begin (); it != t.end (); ++it)
    {
      if (mals_saxs_angles.mals_saxs_angle.count (it->first)
          && mals_saxs_angles.mals_saxs_angle[it->first].has_angle_ri_corr)
        {
          double q
              = RI * 4 * M_PI
                * sin (
                    mals_saxs_angles.mals_saxs_angle[it->first].angle_ri_corr
                    * M_PI / 360)
                / (wavelength * 10);
          QString name = QString ("%1_D%2_Rt_q%3")
                             .arg (use_filename)
                             .arg (pad_zeros (it->first, t.end ()->first))
                             .arg (q);
          add_plot (name, t[it->first], I[it->first], sd[it->first], true,
                    false);
        }
    }
  ...
  //       it != t.end();
  //       ++it ) {
  //    US_Vector::printvector3( QString( "channel %1" ).arg( it->first ),
  //    t[it->first], I[it->first], sd[it->first] );
  // }

  // possibly also UV absorbance data at the end

  while (data.size ()
         && data.front ().contains (QRegularExpression ("^\\s*$")))
    {
      data.pop_front ();
    }
  ...
  if (data.size ()
      && data.front ().contains (
          QRegularExpression ("^\\s*UV absorbance data:")))
    {
      bool load_uv_data = false;
      switch (QMessageBox::question (
          this, windowTitle () + us_tr (" : Load MALS_SAXS Data"),
          us_tr ("The MALS_SAXS data file contains UV absorbance data.\nDo "
                 "you wish to load this?")))
        {
        case QMessageBox::Yes:
          load_uv_data = true;
          break;
      ...
      //                                          windowTitle() + us_tr( " :
      //                                          Load MALS_SAXS Data" )
      //                                          ,"Choose the detector for
      //                                          repeaking the UV data or
      //                                          Cancel to not repeak"
      //                                          ,mals_saxs_angles.list_active()
      //                                          ,mals_saxs_angles.list_active().indexOf(
      //                                          QRegularExpression(
      //                                          "^\\s*\\d+\\s+90\\s+.*$" ) )
      //                                          ,false
      //                                          ,&ok
      //                                          );
      //    if ( ok ) {
      //       QStringList qsl = detector.trimmed().split( QRegularExpression(
      //       "\\s+" ) ); repeak_detector = qsl.front().toInt(); TSO <<
      //       "repeak detector " << repeak_detector << "\n";
      //    }
      // }
      ...
      if (load_uv_data)
        {
          ...
          editor_msg (
              "block",
              QString ("UV plot data found and loaded as %1\n").arg (name));
...

          {
            ...
            TSO << QString ("avg_uvtimedelta %1 --> seconds %2 [%3:%4] frames "
                            "[%5:%6]\n")
...
bool
US_Hydrodyn_Mals_Saxs::mals_saxs_angles_save ()
{
  QString errormsg = "";
  if (mals_saxs_angles.mals_saxs_angle.size ())
    {
      ...
      QString use_filename = QFileDialog::getSaveFileName (
          this, us_tr ("Select a file name for saving the MALS+SAXS Angles"),
          use_dir, "*.csv");
      ...
      use_filename.replace (
          QRegularExpression ("\\.csv$",
                              QRegularExpression::CaseInsensitiveOption),
          "");
      use_filename += ".csv";
      ...
      if (!mals_saxs_angles.save (use_filename, errormsg))
        {
          QMessageBox::warning (
              this, windowTitle (),
              QString (us_tr ("Error saving MALS_SAXS Angles file %1\n"
                              "Error : %2\n"))
                  .arg (use_filename)
                  .arg (errormsg));
          return false;
...
void
US_Hydrodyn_Mals_Saxs::conc_info (const QString &msg)
{
  TSO << "============================================================\n"
      << msg << "\n"
      << "------------------------------------------------------------\n";
...
void
US_Hydrodyn_Mals_Saxs::dndc_info (const QString &msg)
{
  TSO << "============================================================\n"
      << msg << "\n"
      << "------------------------------------------------------------\n";
...
void
US_Hydrodyn_Mals_Saxs::extc_info (const QString &msg)
{
  TSO << "============================================================\n"
      << msg << "\n"
      << "------------------------------------------------------------\n";
...
vector<double>
US_Hydrodyn_Mals_Saxs::get_time_grid_from_namelist (const QStringList &files)
{
  vector<double> result;
  ...
  QStringList frames = get_frames (files, head, tail)
                           .replaceInStrings ("_", ".")
                           .replaceInStrings (QRegularExpression ("^0+"), "0");

  // QTextStream(stdout) << "frames:\n" << frames.join( "\n" ) <<
  // "\n--------\n";

  for (auto &it : frames)
    {
      result.push_back (it.toDouble ());
    }

  // US_Vector::printvector( "get_time_grid result from frames above", result
  // );
  return result;
}

void
US_Hydrodyn_Mals_Saxs::common_time ()
{
  disable_all ();

  // find two unique q-grids && verify point compatibility for interpolation
...

  {
        ...
        if (!f_qs.count (name) || !f_Is.count (name))
          {
            QMessageBox::critical (
                this, windowTitle () + us_tr (": I#,*(q) common time"),
                QString (us_tr ("Internal error: %1 has no q-grid!"))
                    .arg (name));
    ...
    if (qgrid_to_names.size () != 2)
      {
        ...
        QMessageBox::critical (
            this, windowTitle () + us_tr (": I#,*(q) common time"),
            QString (us_tr ("The %1 curves selected do not have exactly 2 "
                            "unique q-grids\n\n"
                            "%2 q-grid(s) found\n"))
                    .arg (names.size ())
    ...
    qgrid_names = { qgrid_to_names.begin ()->second,
  ...
  if (!use_errors)
    {
      QMessageBox::warning (
          this, windowTitle () + us_tr (": I#,*(q) common time"),
          QString (us_tr ("Not all files have associated errors, so the "
                          "result will not have errors"
                          "\n\nFirst discovered in file:\n%1"))
              .arg (missing_errors_name));
    }

  // now qgrid_names is a vector of size 2 with vector values of the file names
  // of each qgrid with verified f_qs & f_Is
  ...
  double start_time_min = time_grids[0].front () > time_grids[1].front ()
                              ? time_grids[0].front ()
                              : time_grids[1].front ();

  double end_time_max = time_grids[0].back () < time_grids[1].back ()
                            ? time_grids[0].back ()
                            : time_grids[1].back ();
  ...
  int mals_set
      = f_qs[qgrid_names[0][0]].front () < f_qs[qgrid_names[1][0]].front ()
            ? 0
            : 1;
  ...
  {
    QString msg
        = QString (us_tr ("Set one has %1 files begining with\n%2\nand covers "
                          "times [%3:%4] and q values [%5:%6]"
                          "\n\nSet two has %7 files begining with\n%8\nand "
                          "covers times [%9:%10] and q values [%11:%12]"
                          "\n\nThe common time range is [%13:%14]"))
              .arg (qgrid_names[0].size ())
              .arg (qstring_common_head (qgrid_names[0], true))
              .arg (time_grids[0].front ())
              .arg (time_grids[0].back ())
              .arg (f_qs[qgrid_names[0][0]].front ())
              .arg (f_qs[qgrid_names[0][0]].back ())
              .arg (qgrid_names[1].size ())
              .arg (qstring_common_head (qgrid_names[1], true))
              .arg (time_grids[1].front ())
              .arg (time_grids[1].back ())
              .arg (f_qs[qgrid_names[1][0]].front ())
              .arg (f_qs[qgrid_names[1][0]].back ())
              .arg (start_time_min)
              .arg (end_time_max);
    ...
    if (end_time_max < start_time_min)
      {
        ...
        QMessageBox::critical (
            this, windowTitle () + us_tr (": I#,*(q) common time"), msg);
        return update_enables ();
      }
  ...
  if (!mals_params_interpolate (output_times) && !errormsg.isEmpty ())
    {
      QMessageBox::information (
          this, windowTitle () + us_tr (": MALS parameters file interpolate"),
          errormsg);
    }

  // setup & apply natural splines for interpolation of I and errors, keep q
  // values to produce new I{star|hash}q's for the interpolated data with new
  // times
  ...
  // q value at a time should lessen memory requirements (but is it faster?
  // does it matter?)
  progress->reset ();
...

  {
    ...
    if (use_errors)
      {
        switch (((US_Hydrodyn *)us_hydrodyn)
                    ->gparams["mals_saxs_interp_method"]
                    .toInt ())
          {
              ...
              for (int i = 0; i < (int)output_qs.size (); ++i)
                {
                  ...
                  if (!usu->linear_interpolate (time_grids[mals_set], yIs,
                                                output_times, yIs2))
                    {
                      QString msg
                          = us_tr ("Interpolation error : " + usu->errormsg);
                      QMessageBox::critical (
                          this,
                  ...
                  if (!usu->linear_interpolate (time_grids[mals_set], yerrors,
                                                output_times, yerrors2))
                    {
                      QString msg
                          = us_tr ("Interpolation error : " + usu->errormsg);
                      QMessageBox::critical (
              ...
              for (int i = 0; i < (int)output_qs.size (); ++i)
                {
                  ...
                  if (!usu->quadratic_interpolate (time_grids[mals_set], yIs,
                                                   output_times, yIs2))
                    {
                      QString msg
                          = us_tr ("Interpolation error : " + usu->errormsg);
                  ...
                  if (!usu->quadratic_interpolate (time_grids[mals_set],
                                                   yerrors, output_times,
                                                   yerrors2))
                    {
                      QString msg
                          = us_tr ("Interpolation error : " + usu->errormsg);
              ...
              for (int i = 0; i < (int)output_qs.size (); ++i)
                {
                  ...
                  usu->natural_spline (time_grids[mals_set], yerrors,
                                       yerrors2);
                  ...
                  for (auto const &t : output_times)
                    {
                      double I;
                      double error;
                      usu->apply_natural_spline (time_grids[mals_set], yIs,
                                                 yIs2, t, I);
                      usu->apply_natural_spline (time_grids[mals_set], yerrors,
                                                 yerrors2, t, error);
        ...
        for (int i = 0; i < (int)output_qs.size (); ++i)
          {
            ...
            for (auto const &t : output_times)
              {
                double I;
                usu->apply_natural_spline (time_grids[mals_set], yIs, yIs2, t,
                                           I);
                output_Is[t].push_back (I);
              }
          }
    ...
    if (use_errors)
      {
        for (auto const &t : output_times)
          {
            QString name = QString ("%1%2%3%4_common")
                               .arg (head)
                               .arg (t)
                               .arg (tail)
                               .arg (method);
            add_plot (name, output_qs, output_Is[t], output_errors[t], false,
                      false);
...

  {
    ...
    {
      vector<double> saxs_times
          = get_time_grid_from_namelist (qgrid_names[saxs_set]);
    ...
    for (auto const &t : mals_times)
      {
        if (!saxs_names_by_time.count (t))
          {
            qDebug () << QString ("internal error: common_times() can't find "
                                  "matching saxs mals time %1")
                             .arg (t);
          }
        else
          {
...
void
US_Hydrodyn_Mals_Saxs::join_by_time ()
{
  disable_all ();
  ...
  if (q_exclude.size ())
    {
      switch (QMessageBox::question (
          this, windowTitle () + us_tr (" : Join I#,*(q) by time"),
        ...
        if (!f_qs.count (name) || !f_Is.count (name))
          {
            QMessageBox::critical (
                this, windowTitle () + us_tr (": Join I#,*(q) by time"),
                QString (us_tr ("Internal error: %1 has no q-values!"))
                    .arg (name));
            return update_enables ();
          }
        if (use_errors
  ...
  vector<double> output_qs = US_Vector::vunion (
      f_qs[qgrid_names[mals_set][0]], f_qs[qgrid_names[saxs_set][0]]);
  vector<double> common_qs = US_Vector::intersection (
      f_qs[qgrid_names[mals_set][0]], f_qs[qgrid_names[saxs_set][0]]);
  ...
  if (common_qs.size ())
    {
      QMessageBox::warning (
          this, windowTitle () + us_tr (": Join I#,*(q) by time"),
          QString (us_tr ("The 2 q grids have %1 common point(s)"
                          "\n\nFirst example is q-value %2"
                          "\n\nThe values from the lower q-value set "
                          "(typically MALS) will be used"))
              .arg (common_qs.size ())
              .arg (common_qs.front ()));
    }
...

  {
    ...
    US_Vector::printvector ("qgrid_time_to_names.front()",
                            qgrid_time_to_names[qgrid_times[0]]);

    for (auto const &tnames : qgrid_time_to_names)
      {
        map<double, vector<double> > merged;

        QString name = QString ("%1%2%3%4_joined")
                           .arg (head)
                           .arg (tnames.first)
                           .arg (tail)
                           .arg (mals_tail);
        ...
        if (use_errors)
          {
            for (int i = 0; i < saxs_q_size; ++i)
              {
                merged[saxs_q[i]] = { f_Is[tnames.second[1]][i],
                                      f_errors[tnames.second[1]][i] };
              }
            for (int i = 0; i < mals_q_size; ++i)
              {
                merged[mals_q[i]] = { f_Is[tnames.second[0]][i],
                ...
                if (!merged.size ())
                  {
                    QMessageBox::critical (
                        ...
                        us_tr ("no points remain after removing excluded q "
                               "values!"));
                    return update_enables ();
                  }
              }
            ...
            if (f_fit_method.count (tnames.second[0]))
              {
                f_fit_method[last_created_file]
                    = f_fit_method[tnames.second[0]];
              }
            else if (f_fit_method.count (tnames.second[1]))
              {
                f_fit_method[last_created_file]
                    = f_fit_method[tnames.second[1]];
              }
            else
              {
            ...
            if (f_fit_q_ranges.count (tnames.second[0]))
              {
                f_fit_q_ranges[last_created_file]
                    = f_fit_q_ranges[tnames.second[0]];
              }
            else if (f_fit_q_ranges.count (tnames.second[1]))
              {
                f_fit_q_ranges[last_created_file]
                    = f_fit_q_ranges[tnames.second[1]];
              }
            else
              {
            ...
            if (f_fit_sd_scale.count (tnames.second[0]))
              {
                f_fit_sd_scale[last_created_file]
                    = f_fit_sd_scale[tnames.second[0]];
              }
            else if (f_fit_sd_scale.count (tnames.second[1]))
              {
                f_fit_sd_scale[last_created_file]
                    = f_fit_sd_scale[tnames.second[1]];
              }
            else
              {
...
bool
US_Hydrodyn_Mals_Saxs::check_files_selected_paired ()
{
  return check_files_selected_paired (all_selected_files ());
}

bool
US_Hydrodyn_Mals_Saxs::check_files_selected_paired (const QStringList &files)
{
...

  {
    ...
    for (auto const &name : files)
      {
        if (!f_qs.count (name) || !f_Is.count (name))
          {
            qDebug () << QString ("check_selected_paired() : internal error, "
                                  "data missing for %1")
                             .arg (name);
            return false;
          }
    ...
    saved_nth_pair_names = { qgrid_to_names.begin ()->second,
                             (--qgrid_to_names.end ())->second };
  }
    ...
    if (time_grids[0] != time_grids[1])
      {
        US_Vector::printvector2 (
            "check_selected_paired() : time_grids[0] != time_grids[1]",
            time_grids[0], time_grids[1]);
...
bool
US_Hydrodyn_Mals_Saxs::saved_nth_last_paired_valid ()
{
...
set<QString>
US_Hydrodyn_Mals_Saxs::qsl_to_set (const QStringList &qsl)
{
  set<QString> result;
...
QStringList
US_Hydrodyn_Mals_Saxs::set_to_qsl (const set<QString> &qsset)
{
  QStringList result;
...
bool
US_Hydrodyn_Mals_Saxs::mals_params_interpolate (const QStringList &filenames)
{
  return mals_params_interpolate (get_time_grid_from_namelist (filenames));
}

bool
US_Hydrodyn_Mals_Saxs::mals_params_interpolate (const vector<double> &times)
{
                                                ...
                                                const QString &filename)
{
  ...
  if (qsl.size () < 3)
    {
      errormsg
          = QString (us_tr ("File %1 has insufficient data")).arg (filename);
      return false;
    }
    ...
    for (size_t i = 1; i < (size_t)qsl.size (); ++i)
      {
        ...
        if (fields.size () < 2)
          {
            errormsg
                = QString (us_tr ("File %1 has insufficient data on line %2"))
                      .arg (filename)
                      .arg (i + 1);
            return false;
          }
        ...
        if (used_times.count (param_times.back ()))
          {
            errormsg
                = QString (
                      us_tr ("File %1 apparently has a duplicate time %2"))
                      .arg (filename)
                      .arg (i + 1);
            return false;
          }
    ...
    if (field_counts.size () != 1)
      {
        ...
        errormsg
            = QString (us_tr ("File %1 has varying data field counts (%2)"))
                  .arg (filename)
                  .arg (counts_msg);
        return false;
      }
  ...
  double start_time_min = times.front () > param_times.front ()
                              ? times.front ()
                              : param_times.front ();

  double end_time_max = times.back () < param_times.back ()
                            ? times.back ()
                            : param_times.back ();
...

  {
    ...
    QString msg
        = QString (us_tr ("%1 MALS parameters were identified"
                          "\n\nThe MALS parameters time grid has %2 time "
                          "points covering times [%3:%4]"
                          "\n\nThe common time grid has %5 time points "
                          "covering times [%6:%7]"
                          "\n\nThe resulting time grid has %8 points covering "
                          "times [%9:%10]"
    ...
    if (end_time_max < start_time_min)
      {
        ...
        QMessageBox::critical (
            this,
            windowTitle ()
                + us_tr (": Interpolate supplementary MALS parameters"),
            msg);
        errormsg = "";
        return false;
      }
          ...
          for (size_t field_pos = 0; field_pos < field_count; ++field_pos)
            {
              vector<double> y;
              vector<double> y2;
              for (size_t time_pos = 0; time_pos < param_times.size ();
                   ++time_pos)
                {
              ...
              if (!usu->linear_interpolate (param_times, y, output_times, y2))
                {
                  ...
                  QMessageBox::critical (
                      ...
                      windowTitle ()
                          + us_tr (
                              ": Interpolate supplementary MALS parameters"),
                      msg);
                  errormsg = "";
                  return false;
                }
          ...
          for (size_t field_pos = 0; field_pos < field_count; ++field_pos)
            {
              ...
              if (!usu->quadratic_interpolate (param_times, y, output_times,
                                               y2))
                {
                  QString msg
                      = us_tr ("Interpolation error : " + usu->errormsg);
  ...
  QString separator
      = filename.contains (QRegularExpression (
            "\\.csv$", QRegularExpression::CaseInsensitiveOption))
            ? ","
            : "\t";
  ...
  if (filename.contains (QRegularExpression (
      ...
      && !headers[0].contains ("\""))
    {
      headers.replaceInStrings (QRegularExpression ("^"), "\"")
  ...
  // save the file
  {
    QString ext = filename.contains (QRegularExpression (
                      "\\.csv$", QRegularExpression::CaseInsensitiveOption))
                      ? "csv"
                      : "txt";
    ...
    // name = name.replace( QRegularExpression( "\\.(csv|txt)$",
    // QRegularExpression::CaseInsensitiveOption ), "" ); name +=
    // "_interpolated." + ext;
    ...
    QString use_filename = QFileDialog::getSaveFileName (
        this,
        us_tr (
            "Select a file name for saving the Interpolated MALS parameters"),
        use_dir, "*." + ext);
    ...
    use_filename = use_filename.replace (
        QRegularExpression ("\\.(csv|txt)$",
                            QRegularExpression::CaseInsensitiveOption),
        "");
    use_filename += "." + ext;
    ...
    if (US_File_Util::putcontents (use_filename, contents, errormsg))
      {
        ...
        editor_msg (
            "darkblue",
            QString (us_tr ("Interpolated MALS parameters saved as %1"))
                .arg (use_filename));
        return true;
      }
  }
