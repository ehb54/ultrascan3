...
US_Hydrodyn_Mals_Conc::US_Hydrodyn_Mals_Conc(csv &csv1, void *mals_window,
                                             QWidget *p, const char *)
    : QFrame(p) {
  org_csv = &csv1;
  this->csv1 = csv1;
  this->mals_window = mals_window;
  ((US_Hydrodyn_Mals *)mals_window)->conc_widget = true;
  us_hydrodyn = ((US_Hydrodyn_Mals *)mals_window)->us_hydrodyn;
  USglobal = new US_Config();
  setPalette(PALET_FRAME);
  setWindowTitle(us_tr("US-SOMO: MALS: File Concentrations"));
...
void US_Hydrodyn_Mals_Conc::refresh(csv &ref_csv) {
  csv new_csv = ref_csv;
...
US_Hydrodyn_Mals_Conc::~US_Hydrodyn_Mals_Conc() {}

void US_Hydrodyn_Mals_Conc::setupGUI() {
  int minHeight1 = 30;
...
void US_Hydrodyn_Mals_Conc::cancel() { close(); }

void US_Hydrodyn_Mals_Conc::set_ok() {
  *org_csv = current_csv();
  ((US_Hydrodyn_Mals *)mals_window)->update_enables();
  close();
}

void US_Hydrodyn_Mals_Conc::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/somo_mals_conc.html");
}

void US_Hydrodyn_Mals_Conc::closeEvent(QCloseEvent *e) {
  ((US_Hydrodyn_Mals *)mals_window)->conc_widget = false;
  global_Xpos -= 30;
  global_Ypos -= 30;
  e->accept();
}

void US_Hydrodyn_Mals_Conc::sort_column(int section) {
  t_csv->sortByColumn(
      section, order_ascending ? Qt::AscendingOrder : Qt::DescendingOrder);
  order_ascending = !order_ascending;
}

csv US_Hydrodyn_Mals_Conc::current_csv() {
  csv tmp_csv = csv1;
...
void US_Hydrodyn_Mals_Conc::copy() {
  csv1 = current_csv();
...
void US_Hydrodyn_Mals_Conc::paste() {
  csv1 = current_csv();
  unsigned int pos = 0;
  for (int i = 0; i < t_csv->rowCount(); i++) {
...
void US_Hydrodyn_Mals_Conc::paste_all() {
  csv1 = current_csv();
  unsigned int pos = 0;
  for (int i = 0; i < t_csv->rowCount(); i++) {
    for (unsigned int j = 1;
...
void US_Hydrodyn_Mals_Conc::update_enables() {
  if (!disable_updates) {
...
void US_Hydrodyn_Mals_Conc::row_header_released(int row) {
...
void US_Hydrodyn_Mals_Conc::reload_csv() {
  t_csv->setRowCount(csv1.data.size());
  for (unsigned int i = 0; i < csv1.data.size(); i++) {
    for (unsigned int j = 0; j < csv1.data[i].size(); j++) {
      t_csv->setItem(i, j, new QTableWidgetItem(csv1.data[i][j]));
...
void US_Hydrodyn_Mals_Conc::load() {
  ...
  if (!QFile::exists(fname)) {
    QMessageBox::warning(
        this, us_tr("US-SOMO: MALS Subtraction Utility: Concentrations"),
  ...
  if (QFileInfo(fname).suffix().contains(QRegExp("^(sbc|SBC)$"))) {
    ...
    US_Hydrodyn_Mals_Conc_Load *hbscl =
        new US_Hydrodyn_Mals_Conc_Load(qsl_lines, csv1, this);
    US_Hydrodyn::fixWinButtons(hbscl);
    if (hbscl->disp_csv.data.size()) {
      hbscl->exec();
    } else {
      QMessageBox::warning(this, us_tr("US-SOMO: MALS: File Concentrations"),
...
void US_Hydrodyn_Mals_Conc::save() {
  QString use_dir = QDir::currentPath();
...
QString US_Hydrodyn_Mals_Conc::csv_to_qstring(csv from_csv) {
  QString qs;
...
QStringList US_Hydrodyn_Mals_Conc::csv_parse_line(QString qs) {
