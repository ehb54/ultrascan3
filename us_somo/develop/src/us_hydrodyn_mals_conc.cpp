...
void
US_Hydrodyn_Mals_Conc::refresh (csv &ref_csv)
{
  csv new_csv = ref_csv;
...
void
US_Hydrodyn_Mals_Conc::setupGUI ()
{
  int minHeight1 = 30;
...
void
US_Hydrodyn_Mals_Conc::cancel ()
{
  close ();
}

void
US_Hydrodyn_Mals_Conc::set_ok ()
{
  *org_csv = current_csv ();
...
void
US_Hydrodyn_Mals_Conc::closeEvent (QCloseEvent *e)
{
...
void
US_Hydrodyn_Mals_Conc::sort_column (int section)
{
...
csv
US_Hydrodyn_Mals_Conc::current_csv ()
{
  csv tmp_csv = csv1;
...
void
US_Hydrodyn_Mals_Conc::paste_all ()
{
  csv1 = current_csv ();
  unsigned int pos = 0;
  for (int i = 0; i < t_csv->rowCount (); i++)
...
void
US_Hydrodyn_Mals_Conc::update_enables ()
{
  if (!disable_updates)
    {
...
void
US_Hydrodyn_Mals_Conc::row_header_released (int row)
{
...
void
US_Hydrodyn_Mals_Conc::reload_csv ()
{
  t_csv->setRowCount (csv1.data.size ());
  for (unsigned int i = 0; i < csv1.data.size (); i++)
    {
      ...
      if (hbscl->disp_csv.data.size ())
      ...
      else
        {
          QMessageBox::warning (this,
                                us_tr ("US-SOMO: MALS: File Concentrations"),
                                us_tr ("The file does not seem to contain "
                                       "data the this program understands"));
          delete hbscl;
          return;
        }
      delete hbscl;
...
QString
US_Hydrodyn_Mals_Conc::csv_to_qstring (csv from_csv)
{
  QString qs;
...
QStringList
US_Hydrodyn_Mals_Conc::csv_parse_line (QString qs)
{
