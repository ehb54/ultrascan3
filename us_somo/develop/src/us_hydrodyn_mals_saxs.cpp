...
QString MALS_SAXS_Angles::list() {
   ...
   for ( auto it = mals_saxs_angle.begin();
         it != mals_saxs_angle.end();
         ++it ) {
...
QString MALS_SAXS_Angles::list_rich( double lambda, double n ) {
   ...
   qsl << QString( us_tr( "<b>MALS_SAXS Angles</b> loaded from %1<hr>" ) ).arg( loaded_filename );
   if ( !lambda || !n ) {
...
bool MALS_SAXS_Angles::populate( const QString & filename, const QStringList & qsl ) {
   if ( !qsl.size()
        || qsl[0] != "Detector"
...
bool MALS_SAXS_Angles::load( const QString & filename, const QStringList & csvlines, QString & errormsg ) {
   if ( !csvlines.size() ) {
      errormsg = us_tr( "Empty file" );
      return false;
   }

   if ( !csvlines[0].contains( QRegularExpression( "^\\s*\"\\s*US-SOMO MALS_SAXS Angles" ) ) ) {
   ...
   while( data.size()
      ...
      data.pop_front();
      if ( qsl.size() < 5 ) {
         errormsg += QString( us_tr( "Error in MALS_SAXS Angles csv line %1\n" ) ).arg( pos );
         continue;
      }
      ...
      mals_saxs_angle[index].angle_ri_corr      = qsl[2] == "\"n/a\"" ? 0 : qsl[2].toDouble();
      mals_saxs_angle[index].has_angle_ri_corr  = qsl[2] == "\"n/a\"" ? false : true;
      
      mals_saxs_angle[index].gain               = qsl[3] == "\"n/a\"" ? 0 : qsl[3].toDouble();
      mals_saxs_angle[index].has_gain           = qsl[3] == "\"n/a\"" ? false : true;

      mals_saxs_angle[index].norm_coef          = qsl[4] == "\"n/a\"" ? 0 : qsl[4].toDouble();
                                             ...
                                             ) : QFrame( p )
{
   ...
   gaussian_type = GAUSS;
   ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_gaussian_type" ] = QString( "%1" ).arg( gaussian_type );
#endif
   ...
   use_line_width = ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_line_width" ].toUInt();

   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_bl_linear" ) )
   {
      ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_bl_linear" ] = "false";
   }
   //   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_bl_integral" ) )
   //   {
   ...
   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_discard_it_sd_mult" ) )
   {
      ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_discard_it_sd_mult" ] = "2";
   }
   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_cb_discard_it_sd_mult" ) )
   {
      ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_cb_discard_it_sd_mult" ] = "true";
   }
   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_cb_guinier_qrgmax" ) )
   {
      ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_cb_guinier_qrgmax" ] = "true";
   }
   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_guinier_qrgmax" ) )
   {
   ...
   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_lock_min_retry_mult"   ) ||
        !started_in_expert_mode ) {
      ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_lock_min_retry_mult"   ] = "2";
   }
   ...
   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_maxfpk_restart_tries"  ) ||
        !started_in_expert_mode ) {
      ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_maxfpk_restart_tries"  ] = "3";
   }
   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_maxfpk_restart_pct"    ) ||
        !started_in_expert_mode ) {
      ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_maxfpk_restart_pct"    ] = "10";
   }
   ...
   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_cb_makeiq_cutmax_pct" ) )
   {
      ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_cb_makeiq_cutmax_pct" ] = "true";
   }
   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_makeiq_cutmax_pct" ) )
   {
      ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_makeiq_cutmax_pct" ] = "1";
   }
   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_cb_makeiq_avg_peaks" ) )
   {
      ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_cb_makeiq_avg_peaks" ] = "false";
   }
   if ( !( ( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_makeiq_avg_peaks" ) )
   {
   ...
   gaussian_type = 
      ...
      (gaussian_types)( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_gaussian_type" ].toInt() : GAUSS;
...
void US_Hydrodyn_Mals_Saxs::cancel()
{
   close();
}
...
void US_Hydrodyn_Mals_Saxs::add_dir() {
...
bool US_Hydrodyn_Mals_Saxs::load_file( QString filename, bool load_conc )
{
   ...
   if ( mals_saxs_angles.populate( filename, qsl_nb ) ) {
      ...
      lbl_mals_saxs_angles_data->setText( mals_saxs_angles.list_rich( mals_saxs_param_lambda, mals_saxs_param_n ) );
      return false;
   }
        ...
        ext == "sprr" )
   {
      ...
      QRegularExpression rx_fit_sd_scale       ( " Fit MALS SD mult:\\s*(\\S+)(\\s|$)" );
   ...
   {
      QRegularExpression rx_mals_saxs_param_lambda            ( "^# __mals_saxs_param_lambda: (\\S+)\\s*$" );
      QRegularExpression rx_mals_saxs_param_n                 ( "^# __mals_saxs_param_n: (\\S+)\\s*$" );
      QRegularExpression rx_mals_saxs_param_g_dndc            ( "^# __mals_saxs_param_g_dndc: (\\S+)\\s*$" );
      QRegularExpression rx_mals_saxs_param_g_extinction_coef ( "^# __mals_saxs_param_g_extinction_coef: (\\S+)\\s*$" );
      QRegularExpression rx_mals_saxs_param_g_conc            ( "^# __mals_saxs_param_g_conc: (\\S+)\\s*$" );
      QRegularExpression rx_mals_saxs_param_DLS_detector      ( "^# __mals_saxs_param_DLS_detector: (\\S+)\\s*$" );

      for ( int i = 1; i < (int) qv.size(); i++ ) {

         QRegularExpressionMatch rx_mals_saxs_param_lambda_m = rx_mals_saxs_param_lambda.match( qv[ i ] );
         if ( rx_mals_saxs_param_lambda_m.hasMatch() ) {
            mals_saxs_param_lambda = rx_mals_saxs_param_lambda_m.captured(1).toDouble();
            continue;
         }
         QRegularExpressionMatch rx_mals_saxs_param_n_m = rx_mals_saxs_param_n.match( qv[ i ] );
         if ( rx_mals_saxs_param_n_m.hasMatch() ) {
            mals_saxs_param_n = rx_mals_saxs_param_n_m.captured(1).toDouble();
            continue;
         }
         QRegularExpressionMatch rx_mals_saxs_param_g_dndc_m = rx_mals_saxs_param_g_dndc.match( qv[ i ] );
         if ( rx_mals_saxs_param_g_dndc_m.hasMatch() ) {
            mals_saxs_param_g_dndc = rx_mals_saxs_param_g_dndc_m.captured(1).toDouble();
            continue;
         }
         QRegularExpressionMatch rx_mals_saxs_param_g_extinction_coef_m = rx_mals_saxs_param_g_extinction_coef.match( qv[ i ] );
         if ( rx_mals_saxs_param_g_extinction_coef_m.hasMatch() ) {
            mals_saxs_param_g_extinction_coef = rx_mals_saxs_param_g_extinction_coef_m.captured(1).toDouble();
            continue;
         }
         QRegularExpressionMatch rx_mals_saxs_param_g_conc_m = rx_mals_saxs_param_g_conc.match( qv[ i ] );
         if ( rx_mals_saxs_param_g_conc_m.hasMatch() ) {
            mals_saxs_param_g_conc = rx_mals_saxs_param_g_conc_m.captured(1).toDouble();
            continue;
         }
         QRegularExpressionMatch rx_mals_saxs_param_DLS_detector_m = rx_mals_saxs_param_DLS_detector.match( qv[ i ] );
         if ( rx_mals_saxs_param_DLS_detector_m.hasMatch() ) {
            mals_saxs_param_DLS_detector = rx_mals_saxs_param_DLS_detector_m.captured(1).toInt();
            continue;
         }
   ...
   // load csv columns as time curves with rescaling for concentration time
      ...
      US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames * conc_csv_frames_instance = 
         new US_Hydrodyn_Mals_Saxs_Conc_Csv_Frames( us_hydrodyn, & conc_csv_params, this );
   ...
   {
      {
         ...
         if ( errormsg != us_tr( "Not a MALS_SAXS Angles csv file" ) ) {
            QMessageBox::warning(
                                 this
                                 ,"US-SOMO MALS_SAXS Angles load Error"
...

      {
         ...
         if ( mals_saxs_load( filename, qsl, errormsg ) ) {
            editor_msg( "black", QString( us_tr( "%1 - MALS_SAXS data\n" ) ).arg( filename ) );
            return false;
         }
         if ( errormsg != "not a mals_saxs file" ) {
            QMessageBox::warning(
                                 this
                                 ,"US-SOMO MALS_SAXS load Error"
                                 ,QString( us_tr( "Loading file %1:\nError: %2" ) ).arg( filename ).arg( us_tr( errormsg ) )
                                 );
            return false;
         }
...
QString US_Hydrodyn_Mals_Saxs::qstring_common_head( QStringList qsl, bool strip_digits )
{
   if ( !qsl.size() )
   {
      return "";
...
QString US_Hydrodyn_Mals_Saxs::qstring_common_tail( QStringList qsl, bool strip_digits )
{
   if ( !qsl.size() )
   {
      return "";
...
QString US_Hydrodyn_Mals_Saxs::qstring_common_head( QString s1, 
                                                      QString s2 )
{
...
QString US_Hydrodyn_Mals_Saxs::qstring_common_tail( QString s1, 
                                                      QString s2 )
{
   int min_len = (int)s1.length();
   if ( min_len > (int)s2.length() )
...
bool US_Hydrodyn_Mals_Saxs::save_files_csv( QStringList files )
{
   ...
   if ( (( US_Hydrodyn * ) us_hydrodyn )->gparams.count( "mals_saxs_csv_transposed" ) &&
        (( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_csv_transposed" ] == "true" )
   {
      // transpose output
...
bool US_Hydrodyn_Mals_Saxs::save_file( QString file, bool &cancel, bool &overwrite_all )
{
   if ( !f_qs.count( file ) )
   {
...
void US_Hydrodyn_Mals_Saxs::conc()
{
   ...
   if ( conc_widget )
   {
      ...
      conc_window = new US_Hydrodyn_Mals_Saxs_Conc( csv_conc,
                                                      this );
      conc_window->show();
   }
}
...
bool US_Hydrodyn_Mals_Saxs::get_peak( QString file, double &peak, double &pos, bool full )
{
   if ( !f_Is.count( file ) )
   {
      editor_msg( "red", QString( us_tr( "Internal error: get_peak requested on %1 but no data available" ) ).arg( file ) );
...
bool US_Hydrodyn_Mals_Saxs::check_zi_window( QStringList & files, const QString & extra_text )
{
   int window = ((US_Hydrodyn *)us_hydrodyn)->gparams[ "mals_saxs_zi_window" ].toInt();
...
void US_Hydrodyn_Mals_Saxs::check_discard_it_sd_mult( QStringList & files, bool optionally_discard )
{
   if ( ((US_Hydrodyn *)us_hydrodyn)->gparams[ "mals_saxs_cb_discard_it_sd_mult" ] != "true" ||
        !files.size() )
   {
      return;
   }

   double mult = ((US_Hydrodyn *)us_hydrodyn)->gparams[ "mals_saxs_discard_it_sd_mult" ].toDouble();
...
void US_Hydrodyn_Mals_Saxs::add_plot( QString           name,
                                      vector < double > q,
                                      vector < double > I,
                                      bool              is_time,
                                      bool              replot )
...
void US_Hydrodyn_Mals_Saxs::ggauss_msg_common( vector < map < int, set < double > > > & check_common )
{
...
vector < double > US_Hydrodyn_Mals_Saxs::gaussian( double * g ) // height, double center, double width )
{
...
void US_Hydrodyn_Mals_Saxs::gauss_pos_dist1_focus( bool hasFocus )
{
   ...
   if ( hasFocus && !cb_ggauss_scroll->isChecked() )
   {
      ...
      disconnect( qwtw_wheel, SIGNAL( valueChanged( double ) ), 0, 0 );
      update_gauss_pos();
      double dist = ( ( US_Hydrodyn * ) us_hydrodyn )->gparams[ "mals_saxs_dist_max" ].toDouble();
...
void US_Hydrodyn_Mals_Saxs::gauss_fit()
{
   ...
   if ( current_mode == MODE_GGAUSSIAN )
   {
      ...
      US_Hydrodyn_Mals_Saxs_Fit_Global *shfg = 
         new US_Hydrodyn_Mals_Saxs_Fit_Global(
                                              this,
                                              this );
...
vector < double > US_Hydrodyn_Mals_Saxs::compute_gaussian_sum( vector < double > t, vector < double > g )
{
...
vector < double > US_Hydrodyn_Mals_Saxs::compute_gaussian( vector < double > t, vector < double > g )
{
   vector < double > result( t.size() );
...
double US_Hydrodyn_Mals_Saxs::compute_gaussian_peak( QString file, vector < double > g )
{
...
bool US_Hydrodyn_Mals_Saxs::ggaussian_compatible( set < QString > & selected, bool check_against_global )
{
   QStringList files;
...
bool US_Hydrodyn_Mals_Saxs::ggaussian_compatible( QStringList & files, bool check_against_global )
{
...
void US_Hydrodyn_Mals_Saxs::add_ggaussian_curve( QString name, vector < double > y )
{
