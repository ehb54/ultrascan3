...
US_Editor::US_Editor (int flag, QWidget *parent, const char *)
    : QFrame (parent)
{
#if QT_VERSION < 0x040000
  if (flag == 0)
  ...
  else
    {
      ...
      m->insertItem (us_tr ("&File"), file);
      file->insertItem (us_tr ("New"), this, SLOT (newDoc ()),
                        Qt::ALT + Qt::Key_N);
      file->insertItem (us_tr ("Open"), this, SLOT (load ()),
                        Qt::ALT + Qt::Key_O);
      ...
      file->insertItem (us_tr ("Close"), this, SLOT (closeDoc ()),
                        Qt::ALT + Qt::Key_W);
      file->insertItem (us_tr ("Quit"), qApp, SLOT (quit ()),
                        Qt::ALT + Qt::Key_Q);
    }
#else
  if (flag == 0)
...
void
US_Editor::load (const QString &fName)
{
  QFile f (fName);
...
void
US_Editor::resizeEvent (QResizeEvent *)
{
  if (e && m)
...
void
US_Editor::append (const QString &str)
{
  e->append (str);
}
...
TextEdit::setupFileActions ()
{
  ...
  a = new QAction (qPixmapFromMimeSource ("fileprint.xpm"),
                   us_tr ("&Print..."), Qt::CTRL + Qt::Key_P, this,
                   "filePrint");
...
TextEdit::setupTextActions ()
{
  ...
  actionTextItalic = new QAction (qPixmapFromMimeSource ("textitalic.xpm"),
                                  us_tr ("&Italic"), Qt::CTRL + Qt::Key_I,
                                  this, "textItalic");
  ...
  actionTextUnderline = new QAction (
      qPixmapFromMimeSource ("textunder.xpm"), us_tr ("&Underline"),
  ...
  actionAlignCenter = new QAction (qPixmapFromMimeSource ("textcenter.xpm"),
                                   us_tr ("C&enter"), Qt::CTRL + Qt::Key_E,
                                   grp, "textCenter");
  ...
  actionAlignJustify = new QAction (qPixmapFromMimeSource ("textjustify.xpm"),
                                    us_tr ("&Justify"), Qt::CTRL + Qt::Key_J,
                                    grp, "textjustify");
...
void
TextEdit::load (const QString &f, QString title, bool ourfmt,
                Qt::TextFormat fmt)
{
  this->title = title;
  this->fmt = fmt;
  ...
  if (ourfmt)
  ...
  else
    {
      if (!Q3StyleSheet::mightBeRichText (txt))
        {
          txt = Q3StyleSheet::convertFromPlainText (
              txt, Q3StyleSheetItem::WhiteSpacePre);
        }
    }
#else
  edit->setTextFormat (fmt);
...
void
TextEdit::load_text (QString text, QString title)
{
  QTextEdit *edit = new QTextEdit (tabWidget);
  if (!title.isEmpty ())
    {
...
void
TextEdit::doConnections (QTextEdit *e)
{
...
void
TextEdit::textFamily (const QString &f)
{
  if (!currentEditor ())
    return;
...
void
TextEdit::textSize (const QString &p)
{
  if (!currentEditor ())
    return;
...
void
TextEdit::fontChanged (const QFont &f)
{
...
void
TextEdit::colorChanged (const QColor &c)
{
  QPixmap pix (16, 16);
...
void
TextEdit::alignmentChanged (int a)
{
...
void
TextEdit::load (const QString &, QString, bool)
{
}

void
TextEdit::load_text (QString, QString)
{
}
#endif
