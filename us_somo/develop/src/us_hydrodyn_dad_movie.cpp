...
US_Hydrodyn_Dad_Movie::US_Hydrodyn_Dad_Movie(US_Hydrodyn_Dad *dad_win,
  ...
  for (int i = 0; i < dad_win->lb_files->count(); i++) {
    if (dad_win->lb_files->item(i)->isSelected()) {
      dad_selected_files.push_back(i);
    }
  }
  ...
  dad_win->lb_files->clearSelection();

  pos = 0;
  last_pos = -1;

  timer_msec = ush_win->gparams.count("dad_movie_timer_ms")
                   ? ush_win->gparams["dad_movie_timer_ms"].toInt()
                   : 1000;

  last_show_gauss = ush_win->gparams.count("dad_movie_show_gauss") &&
                            ush_win->gparams["dad_movie_show_gauss"] == "true"
                        ? true
                        : false;

  last_show_ref = ush_win->gparams.count("dad_movie_show_ref") &&
                          ush_win->gparams["dad_movie_show_ref"] == "true"
                      ? true
                      : false;

  last_mono = ush_win->gparams.count("dad_movie_mono") &&
                      ush_win->gparams["dad_movie_mono"] == "true"
                  ? true
                  : false;
  ...
  dad_win->suppress_replot = true;
  plot_colors = dad_win->plot_colors;
  set_mono();
  update_plot();
  show();
  //    cout << "doin it\n";
...
US_Hydrodyn_Dad_Movie::~US_Hydrodyn_Dad_Movie() {}

void US_Hydrodyn_Dad_Movie::setupGUI() {
  int minHeight1 = 24;
  int minHeight3 = 25;
  ...
  lbl_current = new QLabel(
      dad_win->lb_files->item(dad_selected_files[pos])->text(), this);
...
void US_Hydrodyn_Dad_Movie::cancel() { close(); }

void US_Hydrodyn_Dad_Movie::help() {
  US_Help *online_help;
  online_help = new US_Help(this);
  online_help->show_help("manual/somo/somo_dad_movie.html");
}

void US_Hydrodyn_Dad_Movie::closeEvent(QCloseEvent *e) {
  dad_win->plot_colors = plot_colors;
  dad_win->disable_updates = true;
  if (dad_win->current_mode == dad_win->MODE_GAUSSIAN) {
    dad_win->wheel_cancel();
  }
  dad_win->lb_files->clearSelection();
  for (int i = 0; i < (int)dad_selected_files.size(); ++i) {
    dad_win->lb_files->item(dad_selected_files[i])->setSelected(true);
  }
  dad_win->suppress_replot = false;
  dad_win->plot_files();
  dad_win->disable_updates = false;

  ush_win->gparams["dad_movie_timer_ms"] = QString("%1").arg(timer_msec);
  ush_win->gparams["dad_movie_show_gauss"] =
      cb_show_gauss->isChecked() ? "true" : "false";
  ush_win->gparams["dad_movie_show_ref"] =
      cb_show_ref->isChecked() ? "true" : "false";
  e->accept();
}

void US_Hydrodyn_Dad_Movie::front() {
  pos = 0;
  update_plot();
}

void US_Hydrodyn_Dad_Movie::prev() {
  if (pos > 0) {
    pos--;
    update_plot();
  }
...
void US_Hydrodyn_Dad_Movie::slower() {
  timer_msec *= 2;
  if (timer_msec > 5000) {
    timer_msec = 5000;
  }
...
void US_Hydrodyn_Dad_Movie::start() {
  if (timer->isActive()) {
    timer->stop();
    pb_start->setText("[]");
    lbl_state->setText(
...
void US_Hydrodyn_Dad_Movie::faster() {
  timer_msec /= 2;
  if (timer_msec < 50) {
    timer_msec = 50;
  }
...
void US_Hydrodyn_Dad_Movie::next() {
  if (pos < (int)dad_selected_files.size() - 1) {
    pos++;
    update_plot();
  } else {
    if (timer->isActive()) {
...
void US_Hydrodyn_Dad_Movie::end() {
  pos = dad_selected_files.size() - 1;
  update_plot();
}

void US_Hydrodyn_Dad_Movie::set_save() { update_enables(); }

void US_Hydrodyn_Dad_Movie::set_show_gauss() { update_plot(); }

void US_Hydrodyn_Dad_Movie::set_show_ref() { update_plot(); }

void US_Hydrodyn_Dad_Movie::set_mono() {
  if (cb_mono->isChecked()) {
    dad_win->plot_colors.clear();
    dad_win->plot_colors.push_back(plot_colors[0]);
  } else {
    dad_win->plot_colors = plot_colors;
  }
  update_plot();
...
void US_Hydrodyn_Dad_Movie::update_enables() {
  le_save->setEnabled(cb_save->isChecked());
}

void US_Hydrodyn_Dad_Movie::update_plot() {
  if (last_pos != pos || cb_show_gauss->isChecked() != last_show_gauss ||
    ...
    if (cb_show_ref->isChecked()) {
      dad_win->plot_ref->show();
    } else {
      dad_win->plot_ref->hide();
    }
    dad_win->lb_files->item(dad_selected_files[pos])->setSelected(true);

    if (dad_win->current_mode == dad_win->MODE_GAUSSIAN) {
      dad_win->wheel_cancel();
    }
    if (cb_show_gauss->isChecked() &&
        dad_win->f_is_time.count(
            dad_win->lb_files->item(dad_selected_files[pos])->text()) &&
        dad_win->f_is_time[dad_win->lb_files->item(dad_selected_files[pos])
                               ->text()]) {
      dad_win->gauss_start();
    }
    // else {
    ...
    dad_win->plot_dist->replot();
    dad_win->plot_ref->replot();
    dad_win->plot_errors->replot();
    lbl_state->setText(
        QString(us_tr("%1: %2 of %3")
                    .arg(us_tr(timer->isActive() ? "Running" : "Stopped"))
                    .arg(pos + 1)
...
void US_Hydrodyn_Dad_Movie::save_plot() {
  if (dad_win->plot_errors->isVisible()) {
    if (dad_win->plot_ref->isVisible()) {
      save_plot(dad_win->plot_dist, dad_win->plot_ref, dad_win->plot_errors,
                le_save->text());
    } else {
      save_plot(dad_win->plot_dist, dad_win->plot_errors, le_save->text());
    }
  } else {
    if (dad_win->plot_ref->isVisible()) {
      save_plot(dad_win->plot_dist, dad_win->plot_ref, le_save->text());
    } else {
      save_plot(dad_win->plot_dist, le_save->text());
    }
  }
}

void US_Hydrodyn_Dad_Movie::save_plot(QWidget *plot, QString tag) {
  int mypos = pos;
...
void US_Hydrodyn_Dad_Movie::join_maps(QPixmap &m1, QPixmap &m2) {
  int m1h = m1.height();
  int m2h = m2.height();
...
void US_Hydrodyn_Dad_Movie::save_plot(QWidget *plot, QWidget *plot2,
...
void US_Hydrodyn_Dad_Movie::save_plot(QPixmap &qPix, QString tag, int mypos) {
  QPainter paint(&qPix);
  paint.setPen(Qt::blue);
  paint.setFont(QFont(USglobal->config_list.fontFamily,
                      USglobal->config_list.fontSize - 1, QFont::Bold));
  // paint.drawText( 5, 5, dad_win->lb_files->item( dad_selected_files[ mypos ]
  // )->text() );
  paint.drawText(qPix.rect(), Qt::AlignBottom | Qt::AlignLeft,
                 dad_win->lb_files->item(dad_selected_files[mypos])->text());
