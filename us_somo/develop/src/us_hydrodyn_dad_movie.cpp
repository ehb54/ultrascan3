                                                         ...
                                                         ) : QDialog ( p )
{
   ...
   timer_msec = 
      ush_win->gparams.count( "dad_movie_timer_ms" ) ?
      ush_win->gparams[ "dad_movie_timer_ms" ].toInt() : 1000;

   last_show_gauss =
      ush_win->gparams.count( "dad_movie_show_gauss" ) && ush_win->gparams[ "dad_movie_show_gauss" ] == "true" ?
      true : false;

   last_show_ref =
      ush_win->gparams.count( "dad_movie_show_ref" ) && ush_win->gparams[ "dad_movie_show_ref" ] == "true" ?
      true : false;

   last_mono =
      ush_win->gparams.count( "dad_movie_mono" ) && ush_win->gparams[ "dad_movie_mono" ] == "true" ?
      true : false;
...
void US_Hydrodyn_Dad_Movie::setupGUI()
{
   ...
   lbl_state = new QLabel( QString( us_tr( "Stopped: %1 of %2" ).arg( pos + 1 ).arg( dad_selected_files.size() ) ), this);
   ...
   lbl_current = new QLabel( dad_win->lb_files->item( dad_selected_files[ pos ] )->text(), this);
...
void US_Hydrodyn_Dad_Movie::cancel()
{
   close();
}
...
void US_Hydrodyn_Dad_Movie::closeEvent(QCloseEvent *e)
{
   ...
   ush_win->gparams[ "dad_movie_show_gauss" ] = cb_show_gauss->isChecked() ? "true" : "false";
   ush_win->gparams[ "dad_movie_show_ref"   ] = cb_show_ref->isChecked() ? "true" : "false";
   e->accept();
}
...
void US_Hydrodyn_Dad_Movie::start()
{
   if ( timer->isActive() )
   {
      timer->stop();
      pb_start->setText( "[]" );
      lbl_state  -> setText( QString( us_tr( "%1: %2 of %3" ).arg( us_tr( timer->isActive() ? "Running" : "Stopped" ) ).arg( pos + 1 ).arg( dad_selected_files.size() ) ) );
   } else {
...
void US_Hydrodyn_Dad_Movie::set_save()
{
   update_enables();
}

void US_Hydrodyn_Dad_Movie::set_show_gauss()
{
   update_plot();
}

void US_Hydrodyn_Dad_Movie::set_show_ref()
{
   update_plot();
}
...
void US_Hydrodyn_Dad_Movie::update_plot()
{
        ...
        cb_show_ref  ->isChecked() != last_show_ref )
   {
      ...
      if ( cb_show_gauss->isChecked() &&
           dad_win->f_is_time.count( dad_win->lb_files->item( dad_selected_files[ pos ] )->text() ) &&
           dad_win->f_is_time[ dad_win->lb_files->item( dad_selected_files[ pos ] )->text() ] )
      {
         dad_win->gauss_start();
      }
      // else {
      ...
      lbl_state  -> setText( QString( us_tr( "%1: %2 of %3" ).arg( us_tr( timer->isActive() ? "Running" : "Stopped" ) ).arg( pos + 1 ).arg( dad_selected_files.size() ) ) );
      lbl_current-> setText( dad_win->lb_files->item( dad_selected_files[ pos ] )->text() );
...
void US_Hydrodyn_Dad_Movie::save_plot()
{
   if ( dad_win->plot_errors->isVisible() )
   {
      ...
      {
         save_plot( dad_win->plot_dist, dad_win->plot_ref, dad_win->plot_errors, le_save->text() );
      } else {
...
void US_Hydrodyn_Dad_Movie::save_plot( QWidget *plot, QWidget *plot2, QWidget *plot3, QString tag )
{
   int mypos = pos;
   QPixmap qPix  = plot->grab();
...
void US_Hydrodyn_Dad_Movie::save_plot( QWidget *plot, QWidget *plot2, QString tag )
{
   int mypos = pos;
   QPixmap qPix  = plot->grab();
   QPixmap qPix2 = plot2->grab();
...
void US_Hydrodyn_Dad_Movie::save_plot( QPixmap & qPix, QString tag, int mypos )
{
   ...
   // paint.drawText( 5, 5, dad_win->lb_files->item( dad_selected_files[ mypos ] )->text() );
   paint.drawText( qPix.rect(), Qt::AlignBottom | Qt::AlignLeft, dad_win->lb_files->item( dad_selected_files[ mypos ] )->text() );
