...
comparative_entry US_Hydrodyn_Comparative::empty_comparative_entry(
    QString name) {
...
QString US_Hydrodyn_Comparative::serialize_comparative_entry(
    comparative_entry ce) {
  return QString(
...
comparative_entry US_Hydrodyn_Comparative::deserialize_comparative_entry(
    QString qs) {
...
QString US_Hydrodyn_Comparative::serialize_comparative_info(
    comparative_info ci) {
...
comparative_info US_Hydrodyn_Comparative::deserialize_comparative_info(
    QString qs) {
...
void US_Hydrodyn_Comparative::setupGUI() {
  ...
  cb_include_in_weight_s->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_include_in_weight_D->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_include_in_weight_sr->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_include_in_weight_fr->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_include_in_weight_rg->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_include_in_weight_tau->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_store_reference_tau->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_include_in_weight_eta->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
  ...
  cb_store_reference_eta->setFont(
      QFont(USglobal->config_list.fontFamily, USglobal->config_list.fontSize));
...
void US_Hydrodyn_Comparative::cancel() { close(); }
...
void US_Hydrodyn_Comparative::update_enables() {
  if (updates_enabled) {
    ...
    le_min_s->setEnabled(cb_by_ec->isChecked() && enable_s &&
                         cb_active_s->isChecked() && comparative->ce_s.buckets);
    ...
    le_min_D->setEnabled(cb_by_ec->isChecked() && enable_D &&
                         cb_active_D->isChecked() && comparative->ce_D.buckets);
    ...
    le_rank_sr->setEnabled(enable_sr && cb_active_sr->isChecked() &&
                           cb_rank->isChecked());
    cb_include_in_weight_sr->setEnabled(enable_sr &&
    ...
    le_rank_fr->setEnabled(enable_fr && cb_active_fr->isChecked() &&
                           cb_rank->isChecked());
    cb_include_in_weight_fr->setEnabled(enable_fr &&
    ...
    le_rank_rg->setEnabled(enable_rg && cb_active_rg->isChecked() &&
                           cb_rank->isChecked());
    cb_include_in_weight_rg->setEnabled(enable_rg &&
    ...
    le_rank_tau->setEnabled(enable_tau && cb_active_tau->isChecked() &&
                            cb_rank->isChecked());
    cb_include_in_weight_tau->setEnabled(enable_tau &&
    ...
    le_rank_eta->setEnabled(enable_eta && cb_active_eta->isChecked() &&
                            cb_rank->isChecked());
    cb_include_in_weight_eta->setEnabled(enable_eta &&
...
void US_Hydrodyn_Comparative::load_csv() {
  ...
  QStringList filenames =
      QFileDialog::getOpenFileNames(this, "Open", use_dir, "csv files (*.csv)");
    ...
    if (!csv_error.isEmpty()) {
      ...
      if (!csv_warn.isEmpty()) {
        QMessageBox::warning(
            ...
            QString(
                us_tr(
                    "Loading the csv %1 produced the following warnings:\n%2"))
                .arg(*it)
                .arg(csv_warn));
  ...
  for (int i = 0; i < lb_loaded->count(); i++) {
    if (lb_loaded->item(i)->isSelected()) {
      if (csvs.count(lb_loaded->item(i)->text())) {
        ...
        editor_msg("red",
                   QString(us_tr("internal error: could not find %1 csv data"))
...
void US_Hydrodyn_Comparative::loaded_set_ranges() {
  if (!any_loaded_selected()) {
    editor_msg("red", us_tr("internal error: loaded set exp min/max called but "
                            "no item selected!\n"));
    return;
  }
...
void US_Hydrodyn_Comparative::csv_remove(QString name, int i) {
  ...
  if (csvs.count(name)) {
    for (unsigned int j = 0; j < csvs[name].prepended_names.size(); j++) {
      // cout << QString(" name: %1|%2\n").arg(csvs[ name ].name).arg(csvs[ name
      // ].prepended_names[j]);
      map<QString, bool>::iterator it = loaded_csv_row_prepended_names.find(
    ...
    editor_msg(
        ...
        QString(
            us_tr(
                "internal error: csv_remove %1 could not find csv to remove!"))
            .arg(name));
  }
...
void US_Hydrodyn_Comparative::set_loaded_csv_row_prepended_names(csv &csv1) {
  for (unsigned int i = 0; i < csv1.prepended_names.size(); i++) {
    if (loaded_csv_row_prepended_names.count(csv1.name + "|" +
      ...
      editor_msg(
          "red",
          us_tr("internal error: duplicate loaded prepended names found!"));
    } else {
...
void US_Hydrodyn_Comparative::save_csv() {
  if (any_loaded_selected() && !one_loaded_selected()) {
    ...
    if (any_loaded_selected() && !one_loaded_selected()) {
      editor_msg("red",
                 "oops, csv_merge_loaded_selected didn't work as planned");
      return;
    }
  }
...
csv US_Hydrodyn_Comparative::csv_read(QString filename) {
  ...
  if (loaded_csv_names.count(filename)) {
    if (QMessageBox::warning(this, us_tr("Read CSV"),
                             QString(us_tr("The CSV named %1 is already "
    ...
    if (!removed_one) {
      editor_msg(
          "red",
          QString(us_tr("internal error: could not find %1 in loaded list."))
              .arg(filename));
    }
  }
  ...
  for (QStringList::iterator it = qsl_h.begin(); it != qsl_h.end(); it++) {
    ...
    if (csv1.header_map.count(qs)) {
      csv_error =
          QString(us_tr("Duplicate header name \"%1\" found in file %2"))
              .arg(qs)
              .arg(f.fileName());
      return csv1;
    }
  ...
  unsigned int row = 1;
  {
    ...
    for (; it != qsl.end(); it++) {
      ...
      vector<double> vd;
      if (qsl_d.size()) {
        for (QStringList::iterator it2 = qsl_d.begin(); it2 != qsl_d.end();
             it2++) {
          if (vd.size() >= csv1.header.size()) {
            csv_warn += QString(us_tr("%1Row %2 has more columns than the "
        ...
        if (vd.size() < csv1.header.size()) {
          csv_warn +=
              QString(us_tr("%1Row %2 has insufficient columns, skipped"))
                  .arg(csv_warn.isEmpty() ? "" : "\n")
                  .arg(row);
        } else {
  ...
  for (unsigned int i = 0; i < csv1.data.size(); i++) {
    ...
    loaded_csv_row_prepended_names[csv1.name + "|" + qs_prepend +
                                   csv1.data[i][csv1.header_map["Model name"]] +
                                   QString("%1").arg(
                                       inc ? QString("-%1").arg(inc) : "")] =
        true;
  ...
  for (unsigned int i = 0; i < (unsigned int)lb_loaded->count(); i++) {
    if (lb_loaded->item(i)->isSelected()) {
      ...
      for (unsigned int j = 0;
           j < csvs[lb_loaded->item(i)->text()].prepended_names.size(); j++) {
        if (selected_names.count(
          ...
          selected_models[csvs[lb_loaded->item(i)->text()].prepended_names[j]] =
              true;
        }
      }
    }
...
bool US_Hydrodyn_Comparative::csv_process(csv &csv1) {
  ...
  if (processed_fields_exist) {
    if (!QMessageBox::question(this, us_tr("Process"),
                               us_tr("Some of the added columns that will be "
                                     "computed already exist.\n"
                                     "What do you want remove them?"),
                               us_tr("&Yes, remove them"),
                               us_tr("&No, rename them"), QString(), 0, 1)) {
      for (unsigned int i = 0; i < ce_names.size(); i++) {
        if (do_[i]) {
    ...
    for (unsigned int j = 0; j < ce_names.size(); j++) {
      if (do_[j]) {
        ...
        if (abs_diff_[j]) {
          ...
          if (comparative->by_pct && exp_[j] != 0.0) {
            csv1.num_data[i][col_abs_diff_[j]] =
                100.0 * csv1.num_data[i][col_abs_diff_[j]] / exp_[j];
          }
          csv1.data[i][col_abs_diff_[j]] =
              QString("%1").arg(csv1.num_data[i][col_abs_diff_[j]]);
        }
        if (do_weight && ce_map[ce_names[j]]->include_in_weight) {
          if (comparative->by_pct && exp_[j] != 0.0) {
            ...
            weight += fabs(csv1.num_data[i][col_model_[j]] - exp_[j]) *
                      ce_map[ce_names[j]]->weight;
          }
        }
        if (do_ec && ec_[j]) {
...
bool US_Hydrodyn_Comparative::csv_get_min_max(double &min, double &max,
...
bool US_Hydrodyn_Comparative::csv_get_loaded_min_max(double &min, double &max,
                                                     comparative_entry ce) {
  ...
  for (unsigned int i = 0; i < (unsigned int)lb_loaded->count(); i++) {
    if (lb_loaded->item(i)->isSelected()) {
      ...
      if (!csv_get_min_max(tmp_min, tmp_max, rows_used_count, ce,
        ...
        editor_msg("red",
                   QString(us_tr("internal error: could not find %1 in csv %2"))
                       .arg(ce.name)
                       .arg(lb_loaded->item(i)->text()));
        return false;
      }
      if (!rows_used_count) {
        editor_msg(
            ...
            QString(
                us_tr(
                    "internal error: could not find any rows for %1 in csv %2"))
                .arg(ce.name)
                .arg(lb_loaded->item(i)->text()));
        return false;
      }
  ...
  if (!total_rows_used_count) {
    editor_msg("red",
               QString(us_tr("internal error: could not find any rows for %1 "
                             "in any of the loaded selected csvs"))
                   .arg(ce.name));
    return false;
  }
  return true;
...
bool US_Hydrodyn_Comparative::csv_get_selected_min_max(double &min, double &max,
                                                       comparative_entry ce) {
...
bool US_Hydrodyn_Comparative::csv_premerge_column_warning(csv &csv1,
                                                          csv &csv2) {
  bool is_ok = true;
  for (unsigned int i = 0; i < csv1.header.size(); i++) {
...
bool US_Hydrodyn_Comparative::
    csv_premerge_column_warning_all_loaded_selected() {
  csv_premerge_missing_header_map.clear();
  csv_premerge_missing_header_qsl.clear();
...
void US_Hydrodyn_Comparative::csv_make_unique_header_name(csv &csv1,
                                                          QString name) {
  if (!csv1.header_map.count(name)) {
    return;
  }
