  ...
  for (unsigned int i = startloop; i <= endloop; i++)
    {
      ...
      if (!myrank)
        {
          cout
              << QString ("%1: running nsa for size %2\n").arg (myrank).arg (i)
              << flush;
        }
      ...
      if (control_parameters.count ("nsaga"))
              ...
              if (use_warning_time)
                {
                  ...
                  if (MPI_SUCCESS
                      != MPI_Bcast ((void *)&timed_out, 1, MPI_INT, 0,
                                    MPI_COMM_WORLD))
                    {
      ...
      else
        {
          if (nsa_mpi)
            {
              errormsg
                  = "nsagsm without ga not currently supported under nsa mpi";
              return false;
            }
          if (!nsa_gsm (nrmsd))
            {
      ...
      if (!nsa_mpi || !myrank)
        {
          QString outname
              = (control_parameters.count ("experimentgrid")
                     ? (QFileInfo (control_parameters["experimentgrid"])
                            .baseName ()
                        + "_")
                     : "")
                + QString ("%1sa-%2%3%4%5")
                      ...
                      .arg (control_parameters.count ("nsaess") ? "ess-" : "")
                      .arg (control_parameters.count ("nsaexcl") ? "excl-"
                                                                 : "")
          ...
          if (f.open (QIODevice::WriteOnly))
            {
              ...
              ts << QString (" distance quantum %1\n"
                        ...
                        .arg (sgp_params["distancemin"]
                              * sgp_params["distancequantum"])
                        .arg (sgp_params["distancemax"]
                              * sgp_params["distancequantum"])
                        .arg (sgp_params["radiusmin"]
                              * sgp_params["distancequantum"])
                        .arg (sgp_params["radiusmax"]
                              * sgp_params["distancequantum"]);

              ts << QString (" target curve     %1\n"
                             " target curve sd  %2\n"
                             " fitness          %3\n")
                        .arg (control_parameters["experimentgrid"])
                        .arg (sgp_use_e ? "present"
                                        : "not present or not useable")
                        .arg (nrmsd);
              ...
              // create json info
              {
                ...
                for (map<QString, QString>::iterator it
                     = nsa_physical_stats_map.begin ();
                     it != nsa_physical_stats_map.end (); it++)
                  {
                ...
                if (nsa_use_scaling_fit)
                  {
                    out_params["result last scaling"]
                        = QString ("%1").arg (nsa_last_scaling);
                    double delta_rho
                        = control_parameters["targetedensity"].toDouble ()
                          - our_saxs_options.water_e_density;
                    if (fabs (delta_rho) < 1e-5)
                      {
                        delta_rho = 0e0;
                      }
                    double delta_rho_prime
                        = sqrt (nsa_last_scaling * delta_rho * delta_rho);
                    out_params["result computed delta rho from scaling"]
                        = QString ("%1").arg (delta_rho_prime);
                    out_params["result computed target rho from scaling"]
                        = QString ("%1").arg (delta_rho_prime - delta_rho);
                  }
                ust.end_timer ("nsa ga time");
                out_params["stats nsa ga time"]
                    = ust.list_time ("nsa ga time").replace ("\n", "");
                out_params["stats np"] = QString ("%1").arg (npes);
                out_params["stats host"]
                    = QString ("%1").arg (getenv ("HOSTNAME"));
                ts << Qt::endl
                   << "__json:" << US_Json::compose (out_params) << Qt::endl;
              }
...
#if defined(USUNGM_DEBUG)
  debug_mpi (
      QString ("%1: start nsa_worker: BARRIER enter nsa ga initial barrier\n")
          .arg (myrank));
#endif
...
#if defined(USUNGM_DEBUG)
  debug_mpi (
      QString ("%1: SEND MPI send in nsa_ga_worker() initial registry\n")
          .arg (myrank));
  debug_mpi (QString ("%1: message_received.size %2 messsage_send.size %3\n")
                 .arg (myrank)
                 .arg (message_received.size ())
                 .arg (message_sent.size ()));
#endif

  if (MPI_SUCCESS
      != MPI_Send (&(message_sent[0]), message_sent.size (), MPI_DOUBLE, 0, 0,
                   MPI_COMM_WORLD))
    {
      cout << QString (
                  "%1: MPI send failed in nsa_ga_worker() initial registry\n")
                  .arg (myrank)
           << flush;
      MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
      exit (errorno - myrank);
...
#endif
      if (MPI_SUCCESS
          != MPI_Recv (&(message_received[0]), message_received.size (),
...
#if defined(USUNGM_DEBUG)
      debug_mpi (
          QString ("%1: MPI  in nsa_ga_worker() msg received\n").arg (myrank));
#endif
      ...
      if (message_received[0] == 1.0)
        {
#if defined(USUNGM_DEBUG)
          debug_mpi (
              QString ("%1: MPI  in nsa_ga_worker() msg received is 1.0\n")
                  .arg (myrank));
#endif
          for (unsigned int i = 0; i < nsa_var_ref.size (); i++)
            {
          ...
          if (MPI_SUCCESS
              != MPI_Send (&(message_sent[0]), message_sent.size (),
                           MPI_DOUBLE, 0, 0, MPI_COMM_WORLD))
            {
              errorno -= 2000;
              cout << QString ("%1: MPI send failed in nsa_ga_worker() "
                               "returning results\n")
                          .arg (myrank)
                   << flush;
...
bool
US_Saxs_Util::nsa_ga_close_workers ()
{
#if defined(USUNGM_DEBUG)
  ...
  for (map<int, bool>::iterator it = registered_workers.begin ();
       it != registered_workers.end (); it++)
    {
      ...
      if (MPI_SUCCESS
          != MPI_Send (&(message_sent[0]), message_sent.size (), MPI_DOUBLE,
                       it->first, 0, MPI_COMM_WORLD))
        {
          cout << QString ("%1: MPI send failed in nsa_ga_process_queue() "
                           "closing waiting worker\n")
                      .arg (myrank)
               << flush;
          MPI_Abort (MPI_COMM_WORLD, errorno);
          exit (errorno);
...
bool
US_Saxs_Util::nsa_ga_process_queue ()
{
  // talk to workers
      ...
      if (queued_requests.size ())
        {
...
#if defined(USUNGM_DEBUG)
          debug_mpi (QString ("%1: SEND nsa ga begin process queue sending "
                              "work assignment to worker %1 \n")
                         .arg (myrank)
                         .arg (it->first));
#endif
          if (MPI_SUCCESS
              != MPI_Send (&(message_sent[0]), message_sent.size (),
                           MPI_DOUBLE, it->first, 0, MPI_COMM_WORLD))
            {
              cout << QString ("%1: MPI send failed in nsa_ga_process_queue() "
                               "feeding waiting workers\n")
                          .arg (myrank)
                   << flush;
              MPI_Abort (MPI_COMM_WORLD, errorno);
              exit (errorno);
          ...
          debug_mpi (
              QString (
                  "%1: nsa ga begin process queue  worker %2 busy worker \n")
                  .arg (myrank)
                  .arg (it->first));
#endif
        ...
        {
#if defined(USUNGM_DEBUG)
          cout << QString ("%1: nsa ga begin process queue  worker %2 "
                           "still_waiting \n")
                      .arg (myrank)
                      .arg (it->first)
               << flush;
...
#if defined(USUNGM_DEBUG)
  debug_mpi (QString ("%1: nsa_ga_process_queue begin process queue: all "
                      "requests queued, now wait \n"
                      " : busy_workers       %2\n"
                      " : queued_requests    %3\n"
                      " : registered_workers %4\n")
                 .arg (myrank)
  ...
  while (busy_workers.size () || queued_requests.size ()
         || registered_workers.size () != npes - 1)
    {
#if defined(USUNGM_DEBUG)
      debug_mpi (
          QString ("%1: RECV nsa_ga_process_queue: waiting for message \n"
                   " : busy_workers       %2\n"
                   " : queued_requests    %3\n"
                   " : registered_workers %4\n")
              .arg (myrank)
      ...
      if (MPI_SUCCESS
          != MPI_Recv (&(message_received[0]), message_received.size (),
                       MPI_DOUBLE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD,
                       &mpi_status))
        {
          cout << QString (
                      "%1: MPI Received failed in nsa_ga_process_queue()\n")
                      .arg (myrank)
               << flush;
          MPI_Abort (MPI_COMM_WORLD, errorno);
          exit (errorno);
...
#if defined(USUNGM_DEBUG)
      debug_mpi (
          QString ("%1: nsa_ga_process_queue: received message %2 from %3\n")
              .arg (myrank)
...
#if defined(USUNGM_DEBUG)
          debug_mpi (
              QString ("%1: nsa_ga_process_queue: more queued requests \n")
                  .arg (myrank));
#endif
          message_sent[0] = 1.0;
          for (unsigned int i = 0; i < nsa_var_ref.size (); i++)
...
#if defined(USUNGM_DEBUG)
          debug_mpi (QString ("%1: SEND MPI send in nsa_ga_process_queue() "
                              "normal work assignment %2\n")
                         .arg (myrank)
                         .arg (mpi_status.MPI_SOURCE));
#endif
          if (MPI_SUCCESS
              != MPI_Send (&(message_sent[0]), message_sent.size (),
                           MPI_DOUBLE, mpi_status.MPI_SOURCE, 0,
                           MPI_COMM_WORLD))
            {
              cout << QString ("%1: MPI send failed in nsa_ga_process_queue() "
                               "in normal work assignment\n")
                          .arg (myrank)
                   << flush;
...
bool
US_Saxs_Util::nsa_ga_master_test (double &nrmsd)
{
#if defined(USUNGM_DEBUG)
  debug_mpi (
      QString ("%1: BARRIER enter nsa ga initial barrier\n").arg (myrank));
#endif
  if (MPI_SUCCESS != MPI_Barrier (MPI_COMM_WORLD))
    {
      ...
      for (unsigned int j = 0; j < nsa_var_ref.size (); j++)
        {
          individual.v[j] = nsa_var_min[j]
                            + (nsa_var_max[j] - nsa_var_min[j]) * drand48 ();
        }
...
bool
US_Saxs_Util::nsa_ga_master (double &nrmsd)
{
#if defined(USUNGM_DEBUG)
  debug_mpi (
      QString (
          "%1: nsa_ga_master() start: BARRIER enter nsa ga initial barrier\n")
          .arg (myrank));
#endif
  if (MPI_SUCCESS != MPI_Barrier (MPI_COMM_WORLD))
    {
      ...
      if (nsa_ga_inits.count ((unsigned int)nsa_var_ref.size ())
          && nsa_ga_inits[(unsigned int)nsa_var_ref.size ()].size () > i)
        {
          ...
          if (cropped)
            {
              cout << QString (
                          "Warning: nsa ga init size %1 entry %2 cropped\n")
  ...
  for (unsigned int g = 0; g < control_parameters["nsagenerations"].toUInt ();
       g++)
    {
      if (use_warning_time && QDateTime::currentDateTime () >= warning_time)
        {
          // make timeout message
          cout << "timeout\n";
          timed_out = true;
          ...
          if (last_best_fitness > nsa_pop.front ().fitness)
          ...
          else
            {
              ...
              if (control_parameters.count ("nsaearlytermination")
                  && gens_with_no_improvement
                         >= control_parameters["nsaearlytermination"]
                                .toUInt ())
                {
      ...
      for (unsigned int i = 0;
           i < control_parameters["nsapopulation"].toUInt (); i++)
        {
          ...
          if (control_parameters.count ("nsamutate")
              && drand48 () < control_parameters["nsamutate"].toDouble ())
            {
              ...
              unsigned int pos
                  = (unsigned int)(drand48 () * nsa_var_ref.size ());
              individual.v[pos]
                  = nsa_var_min[pos]
                    + (nsa_var_max[pos] - nsa_var_min[pos]) * drand48 ();
              queued_requests.push_back (individual);
              mutate_count++;
              continue;
            }
...
#if defined(USUNGM_DEBUG)
      debug_mpi (QString ("summary counts:\n"
                          " elitism   %1\n"
                          " mutate    %2\n"
                          " crossover %3\n"
                          " duplicate %4\n"
