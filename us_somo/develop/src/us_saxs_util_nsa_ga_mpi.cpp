    ...
    if (control_parameters.count("nsaga")) {
      if (!nsa_mpi) {
        ...
        if (use_warning_time) {
          ...
          if (MPI_SUCCESS !=
              MPI_Bcast((void*)&timed_out, 1, MPI_INT, 0, MPI_COMM_WORLD)) {
    ...
    if (!nsa_mpi || !myrank) {
      QString outname =
          (control_parameters.count("experimentgrid")
               ? (QFileInfo(control_parameters["experimentgrid"]).baseName() +
                  "_")
               : "") +
          QString("%1sa-%2%3%4%5")
              .arg(i)
      ...
      if (f.open(QIODevice::WriteOnly)) {
        ...
        // create json info
        {
          ...
          for (unsigned int i = 0; i < nsa_var_ref.size(); i++) {
            out_params["results vector"] +=
                QString("%1%2").arg(i ? " " : "").arg(*(nsa_var_ref[i]));
          }
...
bool US_Saxs_Util::nsa_ga_worker() {
  ...
  if (MPI_SUCCESS != MPI_Send(&(message_sent[0]), message_sent.size(),
  ...
  do {
#if defined(USUNGM_DEBUG)
    debug_mpi(
        QString("%1: RECV MPI  in nsa_ga_worker() receive msg\n").arg(myrank));
#endif
    if (MPI_SUCCESS != MPI_Recv(&(message_received[0]), message_received.size(),
                                MPI_DOUBLE, 0, MPI_ANY_TAG, MPI_COMM_WORLD,
                                &mpi_status)) {
      cout
          << QString("%1: MPI Received failed in nsa_ga_worker()\n").arg(myrank)
          << flush;
      MPI_Abort(MPI_COMM_WORLD, errorno - myrank);
      exit(errorno - myrank);
    }
    ...
    if (message_received[0] == 1.0) {
...
#if defined(USUNGM_DEBUG)
      debug_mpi(
          QString("%1: MPI  in nsa_ga_worker() send results\n").arg(myrank));
#endif
      message_sent[0] = 1.0;
      ...
      if (MPI_SUCCESS != MPI_Send(&(message_sent[0]), message_sent.size(),
        ...
        cout
            << QString(
                   "%1: MPI send failed in nsa_ga_worker() returning results\n")
                   .arg(myrank)
            << flush;
...
bool US_Saxs_Util::nsa_ga_close_workers() {
  ...
  for (map<int, bool>::iterator it = registered_workers.begin();
    ...
    if (MPI_SUCCESS != MPI_Send(&(message_sent[0]), message_sent.size(),
      ...
      cout << QString(
                  "%1: MPI send failed in nsa_ga_process_queue() closing "
                  "waiting worker\n")
                  .arg(myrank)
           << flush;
      MPI_Abort(MPI_COMM_WORLD, errorno);
...
bool US_Saxs_Util::nsa_ga_process_queue() {
...
#if defined(USUNGM_DEBUG)
      debug_mpi(QString("%1: SEND nsa ga begin process queue sending work "
                        "assignment to worker %1 \n")
                    .arg(myrank)
                    .arg(it->first));
#endif
      if (MPI_SUCCESS != MPI_Send(&(message_sent[0]), message_sent.size(),
                                  MPI_DOUBLE, it->first, 0, MPI_COMM_WORLD)) {
        cout << QString(
                    "%1: MPI send failed in nsa_ga_process_queue() feeding "
                    "waiting workers\n")
                    .arg(myrank)
             << flush;
        MPI_Abort(MPI_COMM_WORLD, errorno);
...
#if defined(USUNGM_DEBUG)
      debug_mpi(
          QString("%1: nsa ga begin process queue  worker %2 busy worker \n")
              .arg(myrank)
              .arg(it->first));
#endif
...
#if defined(USUNGM_DEBUG)
      cout << QString(
                  "%1: nsa ga begin process queue  worker %2 still_waiting \n")
                  .arg(myrank)
                  .arg(it->first)
           << flush;
#endif
  ...
  while (busy_workers.size() || queued_requests.size() ||
...
#if defined(USUNGM_DEBUG)
    debug_mpi(QString("%1: nsa_ga_process_queue: received message %2 from %3\n")
                  .arg(myrank)
    ...
    if (queued_requests.size()) {
#if defined(USUNGM_DEBUG)
      debug_mpi(QString("%1: nsa_ga_process_queue: more queued requests \n")
                    .arg(myrank));
#endif
      message_sent[0] = 1.0;
      for (unsigned int i = 0; i < nsa_var_ref.size(); i++) {
...
#if defined(USUNGM_DEBUG)
      debug_mpi(QString("%1: SEND MPI send in nsa_ga_process_queue() normal "
                        "work assignment %2\n")
                    .arg(myrank)
                    .arg(mpi_status.MPI_SOURCE));
#endif
      if (MPI_SUCCESS != MPI_Send(&(message_sent[0]), message_sent.size(),
                                  ...
                                  MPI_COMM_WORLD)) {
        cout << QString(
                    "%1: MPI send failed in nsa_ga_process_queue() in normal "
                    "work assignment\n")
                    .arg(myrank)
             << flush;
...
bool US_Saxs_Util::nsa_ga_master_test(double& nrmsd) {
  ...
  for (unsigned int i = 0; i < 3; i++) {
    ...
    for (unsigned int j = 0; j < nsa_var_ref.size(); j++) {
      individual.v[j] =
          nsa_var_min[j] + (nsa_var_max[j] - nsa_var_min[j]) * drand48();
    }
  ...
  for (unsigned int g = 0; g < control_parameters["nsagenerations"].toUInt();
    ...
    for (unsigned int i = 0; i < control_parameters["nsapopulation"].toUInt();
      ...
      if (control_parameters.count("nsamutate") &&
        ...
        individual.v[pos] = nsa_var_min[pos] +
