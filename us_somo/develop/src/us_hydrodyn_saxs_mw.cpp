...
US_Hydrodyn_Saxs_Mw::US_Hydrodyn_Saxs_Mw (
    QString msg, float *mw, float *last_mw, bool *remember, bool *use_partial,
    QString *partial, map<QString, float> *remember_mw,
    map<QString, QString> *remember_mw_source, bool allow_none, QWidget *p,
    const char *)
    : QDialog (p)
{
  this->msg = msg;
...
US_Hydrodyn_Saxs_Mw::setupGUI ()
{
  ...
  pb_known_mw->setFont (QFont (USglobal->config_list.fontFamily,
                               USglobal->config_list.fontSize + 1));
  ...
  if (allow_none)
    {
      ...
      connect (pb_do_not_normalize, SIGNAL (clicked ()),
               SLOT (set_do_not_normalize ()));
    }

  pb_set_to_last_used_mw
      = new QPushButton (us_tr ("Set to last used weight:"), this);
  pb_set_to_last_used_mw->setFont (QFont (USglobal->config_list.fontFamily,
                                          USglobal->config_list.fontSize + 1));
  ...
  connect (pb_set_to_last_used_mw, SIGNAL (clicked ()),
           SLOT (set_to_last_used_mw ()));

  lbl_last_used_mw
      = new QLabel (QString ("").sprintf ("%5.3f", *last_mw), this);
  ...
  cb_remember->setFont (QFont (USglobal->config_list.fontFamily,
  ...
  cb_use_partial->setFont (QFont (USglobal->config_list.fontFamily,
                                  USglobal->config_list.fontSize));
...
void
US_Hydrodyn_Saxs_Mw::set_to_last_used_mw ()
{
...
void
US_Hydrodyn_Saxs_Mw::cancel ()
{
  close ();
}

void
US_Hydrodyn_Saxs_Mw::help ()
{
  US_Help *online_help;
...
void
US_Hydrodyn_Saxs_Mw::update_mw (const QString &str)
{
  *mw = str.toDouble ();
}

void
US_Hydrodyn_Saxs_Mw::update_partial (const QString &str)
{
  *partial = str;
}

void
US_Hydrodyn_Saxs_Mw::set_remember ()
{
  *remember = cb_remember->isChecked ();
}

void
US_Hydrodyn_Saxs_Mw::set_use_partial ()
{
  *use_partial = cb_use_partial->isChecked ();
}

void
US_Hydrodyn_Saxs_Mw::set_do_not_normalize ()
{
  le_mw->setText ("-1");
  close ();
}
...
void
US_Hydrodyn_Saxs_Mw::set_known_mw ()
{
  ...
  for (map<QString, float>::iterator it = remember_mw->begin ();
       it != remember_mw->end (); ++it)
    {
      if (rx.indexIn (it->first) != -1 && !rx.cap (1).isEmpty ())
        {
          if (!good_mw.count (rx.cap (1)))
            {
              ...
              if (remember_mw_source->count (it->first))
                {
                  good_mw_source[rx.cap (1)]
                      = (*remember_mw_source)[it->first];
                }
            }
          else
            {
...

  {
...

    {
      ...
      QString choice = US_Static::getItem (
          ...
          us_tr ("Select from a previously recorded MWs or Cancel"), qsl, 0,
          false, &ok, this);
      if (!ok)
        {
          return;
