...
ostream &
operator<< (ostream &out, const point &c)
{
...
point
US_Saxs_Util::a2sb_cubelet (point p1)
{
...
point
US_Saxs_Util::a2sb_cubelet (PDB_atom &this_atom)
{
...
US_Saxs_Util::a2sb_run ()
{
  ...
  for (unsigned int current_model = 0;
       ...
       current_model++)
    {
      ...
      for (unsigned int j = 0; j < model_vector[0].molecule.size (); j++)
        {
          for (unsigned int k = 0;
               k < model_vector[0].molecule[j].atom.size (); k++)
            {
              tot_atoms++;
      ...
      // write out 1sa model ( ? with difference to saxs curve of original
      // model )
      ...
      QString outname
          = QString ("%1%2-%3-%4%5%6-a2sb")
                .arg (QFileInfo (control_parameters["inputfile"])
                          .completeBaseName ())
      ...
      if (f.open (QIODevice::WriteOnly))
        ...
        {
          errormsg = QString ("Error: could not create file %1")
                         .arg (f.fileName ());
          failed = true;
          break;
        }
