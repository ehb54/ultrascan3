  ...
  */
  ...
  do {
    ...
    if (MPI_SUCCESS != MPI_Recv(&msg, sizeof(pm_msg), MPI_CHAR, 0, PM_MSG,
      ...
      if (us_log) {
        us_log->log(
            QString("%1: MPI Initial Receive failed in pm_mpi_worker()\n")
                .arg(myrank));
      }
      MPI_Abort(MPI_COMM_WORLD, errorno - myrank);
      exit(errorno - myrank);
    ...
    switch (msg.type) {
      ...
      case PM_NEW_PM: {
        ...
        if (MPI_SUCCESS != MPI_Recv(&(d[0]), tot_vsize * sizeof(double),
          ...
          if (us_log) {
            us_log->log(
                QString("%1: MPI PM_NEW_PM Receive failed in pm_mpi_worker()\n")
                    .arg(myrank));
          }
          MPI_Abort(MPI_COMM_WORLD, errorno - myrank);
          exit(errorno - myrank);
      ...
      case PM_NEW_GRID_SIZE: {
        ...
        if (!pm) {
          if (us_log) {
            us_log->log(QString("%1: MPI PM_NEW_GRID_SIZE Receive called "
        ...
        if (!pm->set_grid_size(msg.grid_conversion_factor, true)) {
          if (us_log) {
            us_log->log(QString("%1: MPI PM_NEW_GRID_SIZE failed to set grid "
      ...
      case PM_CALC_FITNESS: {
        ...
        if (!pm) {
          if (us_log) {
            us_log->log(QString("%1: MPI PM_CALC_FITNESS Receive called before "
                                "PM_NEW_PM  pm_mpi_worker()\n")
                            .arg(myrank));
          }
          MPI_Abort(MPI_COMM_WORLD, errorno - myrank);
          exit(errorno - myrank);
        ...
        if (MPI_SUCCESS != MPI_Recv(&(params[0]), msg.vsize * sizeof(double),
                                    ...
                                    MPI_COMM_WORLD, &mpi_status)) {
          if (us_log) {
            us_log->log(QString("%1: MPI PM_CALC_FITNESS Receive failed in "
                                "pm_mpi_worker()\n")
                            .arg(myrank));
          }
          MPI_Abort(MPI_COMM_WORLD, errorno - myrank);
        ...
        // US_Vector::printvector( QString( "%1: PM_CALC_FITNESS params:" ).arg(
        // myrank ), params ); cout << flush;

        if (!pm->create_model(params, model) ||
            !pm->compute_I(model, I_result)) {
          if (us_log) {
            us_log->log(
                QString("%1: MPI PM_CALC_FITNESS error %2 in pm_mpi_worker()\n")
                    .arg(myrank)
                    .arg(pm->error_msg));
          }
          MPI_Abort(MPI_COMM_WORLD, errorno - myrank);
        ...
        if (MPI_SUCCESS != MPI_Send(&msg, sizeof(msg), MPI_CHAR, 0,
          ...
          if (us_log) {
            us_log->log(QString("%1: MPI PM_CALC_FITNESS Send 1 failed in "
                                "pm_mpi_worker()\n")
                            .arg(myrank));
          }
          MPI_Abort(MPI_COMM_WORLD, errorno - myrank);
        ...
        if (MPI_SUCCESS !=
          ...
          if (us_log) {
            us_log->log(QString("%1: MPI PM_CALC_FITNESS Send 2 failed in "
                                "pm_mpi_worker()\n")
                            .arg(myrank));
          }
          MPI_Abort(MPI_COMM_WORLD, errorno - myrank);
      ...
      default: {
        if (us_log) {
          us_log->log(QString("%1: MPI Receive unknown message type %2 in "
                              "pm_mpi_worker()\n")
                          .arg(myrank)
                          .arg(msg.type));
        }
