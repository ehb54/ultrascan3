...
ostream &
operator<< (ostream &out, const pm_msg &c)
{
...
bool
US_Saxs_Util::run_json_mpi (QString &json)
{
  // note: alternate version for non-mpi in in run_json() in
  // us_saxs_util_pm.cpp
  ...
  if (!myrank && parameters.count ("_udphostip")
      && parameters.count ("_udpport") && parameters.count ("_uuid"))
    {
      map<QString, QString> msging;
      msging["_uuid"] = results["_uuid"];
      us_udp_msg = new US_Udp_Msg (parameters["_udphostip"],
        ...
        if (!myrank)
          {
            ...
            cout << MPI_JSON_SNIP_START
                 << US_Json::compose (results).toLatin1 ().data () << endl
                 << MPI_JSON_SNIP_END << flush;
          }
        return true;
      }
    if (count > 1)
    ...
    for (int i = 1; i < npes; ++i)
      {
        if (MPI_SUCCESS
          ...
          {
            us_log->log (
                QString ("%1: MPI PM_SHUTDOWN failed\n").arg (myrank));
            delete us_log;
  ...
  cout << MPI_JSON_SNIP_START << US_Json::compose (results).toLatin1 ().data ()
       << endl
       << MPI_JSON_SNIP_END << flush;
  return true;
}
...
bool
US_Saxs_Util::run_pm_mpi (QString controlfile)
{
...
US_Saxs_Util::pm_mpi_worker ()
{
  ...
  /* no registry for now, assume all workers available
    ...
    if ( us_log )
    {
    us_log->log( QString( "%1: MPI send failed in nsa_ga_worker() initial
    registry\n" ).arg( myrank ) );
    }
    MPI_Abort( MPI_COMM_WORLD, errorno - myrank );
    exit( errorno - myrank );
  ...
  do
    {
      ...
      // }
      if (MPI_SUCCESS
          != MPI_Recv (&msg, sizeof (pm_msg), MPI_CHAR, 0, PM_MSG,
          ...
          if (us_log)
            {
              us_log->log (
                  QString (
                      "%1: MPI Initial Receive failed in pm_mpi_worker()\n")
                      .arg (myrank));
            }
          MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
          exit (errorno - myrank);
        ...
        case PM_NEW_PM:
          {
            ...
            if (MPI_SUCCESS
                != MPI_Recv (&(d[0]), tot_vsize * sizeof (double), MPI_CHAR, 0,
                             PM_NEW_PM, MPI_COMM_WORLD, &mpi_status))
              {
                if (us_log)
                  {
                    us_log->log (QString ("%1: MPI PM_NEW_PM Receive failed "
                                          "in pm_mpi_worker()\n")
                                     .arg (myrank));
                  }
                MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
            ...
            pm = new US_PM (msg.grid_conversion_factor, msg.max_dimension,
                            msg.max_harmonics, F, q, I, e, msg.max_mem_in_MB,
                            0, true);
          }
          break;
            ...
            if (!pm)
              {
                if (us_log)
                  {
                    us_log->log (
                        QString ("%1: MPI PM_NEW_GRID_SIZE Receive called "
                                 "before PM_NEW_PM  pm_mpi_worker()\n")
                            .arg (myrank));
                  }
                MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
                exit (errorno - myrank);
            ...
            if (!pm->set_grid_size (msg.grid_conversion_factor, true))
              {
                if (us_log)
                  {
                    us_log->log (
                        QString ("%1: MPI PM_NEW_GRID_SIZE failed to set grid "
                                 "size %2  pm_mpi_worker()\n")
                            .arg (myrank)
                            .arg (msg.grid_conversion_factor));
                  }
                MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
                exit (errorno - myrank);
              }
        ...
        case PM_CALC_FITNESS:
          {
            ...
            if (!pm)
              {
                if (us_log)
                  {
                    us_log->log (
                        QString ("%1: MPI PM_CALC_FITNESS Receive called "
                                 "before PM_NEW_PM  pm_mpi_worker()\n")
                            .arg (myrank));
                  }
                MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
            ...
            if (MPI_SUCCESS
                != MPI_Recv (&(params[0]), msg.vsize * sizeof (double),
                             MPI_CHAR, 0, PM_CALC_FITNESS, MPI_COMM_WORLD,
                             &mpi_status))
              {
                if (us_log)
                  {
                    us_log->log (QString ("%1: MPI PM_CALC_FITNESS Receive "
                                          "failed in pm_mpi_worker()\n")
                                     .arg (myrank));
                  }
                MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
                exit (errorno - myrank);
            ...
            // US_Vector::printvector( QString( "%1: PM_CALC_FITNESS params:"
            // ).arg( myrank ), params ); cout << flush;
            ...
            if (!pm->create_model (params, model)
                || !pm->compute_I (model, I_result))
              {
                if (us_log)
                  {
                    us_log->log (QString ("%1: MPI PM_CALC_FITNESS error %2 "
                                          "in pm_mpi_worker()\n")
                                     .arg (myrank)
                                     .arg (pm->error_msg));
                  }
            ...
            if (MPI_SUCCESS
                != MPI_Send (&msg, sizeof (msg), MPI_CHAR, 0,
                             PM_FITNESS_RESULT, MPI_COMM_WORLD))
              {
                if (us_log)
                  {
                    us_log->log (QString ("%1: MPI PM_CALC_FITNESS Send 1 "
                                          "failed in pm_mpi_worker()\n")
                                     .arg (myrank));
                  }
                MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
            ...
            if (MPI_SUCCESS
                != MPI_Send (&(vmodel[0]), 3 * msg.vsize * sizeof (int16_t),
                             MPI_CHAR, 0, PM_FITNESS_RESULT_MODEL,
                             MPI_COMM_WORLD))
              {
                if (us_log)
                  {
                    us_log->log (QString ("%1: MPI PM_CALC_FITNESS Send 2 "
                                          "failed in pm_mpi_worker()\n")
                                     .arg (myrank));
                  }
                MPI_Abort (MPI_COMM_WORLD, errorno - myrank);
        ...
        default:
          {
            if (us_log)
              {
                us_log->log (QString ("%1: MPI Receive unknown message type "
                                      "%2 in pm_mpi_worker()\n")
                                 .arg (myrank)
                                 .arg (msg.type));
              }
