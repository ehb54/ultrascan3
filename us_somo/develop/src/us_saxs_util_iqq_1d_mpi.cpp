...
bool US_Saxs_Util::run_1d_mpi(QString controlfile) {
  ...
  if (!controlfile.contains(QRegExp("\\.(tgz|TGZ|tar|TAR)$"))) {
    errormsg =
        QString("controlfile must be .tgz or .tar, was %1").arg(controlfile);
  ...
  if (myrank) {
    ...
    // ) << flush;
    if (MPI_SUCCESS !=
        MPI_Bcast(char_files, sizeoflist + 1, MPI_CHAR, 0, MPI_COMM_WORLD)) {
      MPI_Abort(MPI_COMM_WORLD, errorno);
      exit(errorno);
    }
    errorno--;
    ...
    for (int i = 0; i < (int)org_output_files.size(); i++) {
      ...
      for (int r = 0; r < npes; r++) {
        ...
        if (!set_control_parameters_from_experiment_file(f.fileName(), true)) {
          errors +=
              QString(
                  "%1: Error trying to get grid, ignorming this processor%2\n")
                  .arg(myrank)
                  .arg(errormsg);
          continue;
        }
  ...
  if (!myrank) {
    ...
    if (!dod.exists()) {
      ...
      if (!ndod.mkdir(newdir)) {
        cout << QString(
                    "Warning: could not create outputData \"%1\" directory\n")
                    .arg(ndod.path());
      }
...
bool US_Saxs_Util::compute_1d_mpi() {
  ...
  if (control_parameters.count("1dintermediatesaves")) {
    ...
    for (unsigned int i = 0; i < (unsigned int)qsl.size(); i++) {
      ...
      if (!pos || pos >= sample_rotations) {
        errormsg = QString(
                       "1drun: 1dintermediatesaves bad value %1 found at pos "
                       "%2 in line <%3>\n"
                       "either not a positive natural number or value larger "
                       "than total sample rotations %4")
                       .arg(pos)
                       .arg(i + 1)
                       .arg(qsl[i])
  ...
  for (unsigned int r = 0; r < rotations.size(); r++) {
    ...
    if (rotations.size() > 1) {
      ...
      if (!atom_align(transform_from, transform_to, atom_positions, result)) {
      ...
      // and rotate excluded volume (this may have to be done piecemeal to save
      // memory

      if (!atom_align(transform_from, transform_to, excluded_volume, result)) {
    ...
    for (int t = 0; t < axis_rotations; t++) {
      ...
      for (unsigned int a = 0; a < atoms.size(); a++) {
        for (unsigned int i = 0; i < s1d_data.size(); i++) {
          ...
          vector<double> Rv(3);
          Rv[0] =
              (cospsi + (1.0 - cospsi) * rotations[r][0] * rotations[r][0]) *
          ...
          Rv[1] = ((1.0 - cospsi) * rotations[r][0] * rotations[r][1] +
                  ...
                  (double)atoms[a].pos[0];
          Rv[1] +=
              (cospsi + (1.0 - cospsi) * rotations[r][1] * rotations[r][1]) *
              (double)atoms[a].pos[1];
          ...
          Rv[2] += ((1.0 - cospsi) * rotations[r][1] * rotations[r][2] +
                   ...
                   (double)atoms[a].pos[1];
          Rv[2] +=
              (cospsi + (1.0 - cospsi) * rotations[r][2] * rotations[r][2]) *
              (double)atoms[a].pos[2];
          ...
          double F_at = compute_ff(saxs, saxsH, atoms[a].residue_name,
...
void US_Saxs_Util::hypercube_rejection_drand_rotations(
  ...
  if (!myrank) {
    ...
    for (int i = 1; i < npes; i++) {
      ...
      // send them
      if (MPI_SUCCESS != MPI_Send((void*)&send_recv_rotations[0], proc_rots3,
    ...
    if (MPI_SUCCESS != MPI_Recv((void*)&send_recv_rotations[0], proc_rots3,
  ...
  cout
      << QString("%1: my rotations size %2\n").arg(myrank).arg(rotations.size())
      << flush;
  return;
}

bool US_Saxs_Util::load_rotations_mpi(unsigned int number,
                                      vector<vector<double> >& rotations) {
  ...
  if (!f.open(QIODevice::ReadOnly)) {
    errormsg =
        QString("Notice: found cached rotations file %1 but could not open it")
            .arg(f.fileName());
    return false;
  }
  ...
  while (!ts.atEnd() && line < number) {
    ...
    if (qsl.size() != 3) {
      errormsg = QString(
                     "Notice: error in found cached rotations file %1 line %2, "
                     "does not contain 3 tokens")
                     .arg(f.fileName())
                     .arg(line);
      f.close();
      return false;
