...
bool
US_Saxs_Util::run_1d_mpi (QString controlfile)
{
  ...
  if (!controlfile.contains (QRegExp ("\\.(tgz|TGZ|tar|TAR)$")))
    {
      errormsg = QString ("controlfile must be .tgz or .tar, was %1")
                     .arg (controlfile);
  ...
  if (myrank)
    {
      ...
      // myrank ) << flush;
      if (MPI_SUCCESS
          != MPI_Bcast (&sizeoflist, 1, MPI_UNSIGNED, 0, MPI_COMM_WORLD))
        {
          MPI_Abort (MPI_COMM_WORLD, errorno);
          exit (errorno);
        }
      ...
      // myrank ) << flush;
      if (MPI_SUCCESS
          != MPI_Bcast (char_files, sizeoflist + 1, MPI_CHAR, 0,
                        MPI_COMM_WORLD))
        {
          MPI_Abort (MPI_COMM_WORLD, errorno);
          exit (errorno);
  ...
  else
    {
      ...
      // copy here
      US_File_Util usu;
      usu.copy (controlfile, QDir::currentPath () + QDir::separator ()
  ...
  if (!myrank)
    {
          ...
          for (int r = 0; r < npes; r++)
            {
              ...
              if (!f.exists ())
                {
                  cout << QString (
                              "%1: expected output file %2 does not exist\n")
                              .arg (myrank)
                              .arg (f.fileName ())
                       << flush;
              ...
              if (!set_control_parameters_from_experiment_file (f.fileName (),
                                                                true))
                {
                  errors += QString ("%1: Error trying to get grid, ignorming "
                                     "this processor%2\n")
                                .arg (myrank)
                                .arg (errormsg);
                  continue;
              ...
              if (!r)
              ...
              else
                {
                  if (I.size () != sgp_exp_I.size ())
                    {
                      cout << QString ("%1: collect results size mismatch %1 "
                                       "vs %2 \n")
                                  .arg (myrank)
                                  .arg (I.size ())
                                  .arg (sgp_exp_I.size ())
                  ...
                  for (unsigned int i = 0; i < (unsigned int)sgp_exp_I.size ();
                       i++)
                    {
                      I[i] += sgp_exp_I[i];
                    }
      ...
      cout << QString ("%1: my output files:<%2>\ncontrolfile %3\n")
                  .arg (myrank)
                  .arg (output_files.join (":"))
                  .arg (controlfile)
           << flush;
      ...
      if (!dod.exists ())
        {
          ...
          if (!ndod.mkdir (newdir))
            {
              cout << QString ("Warning: could not create outputData \"%1\" "
                               "directory\n")
                          .arg (ndod.path ());
            }
...
US_Saxs_Util::compute_1d_mpi ()
{
  ...
  if (control_parameters.count ("output")
      && (control_parameters["output"] == "csv"))
    {
      if (!myrank)
        {
          errormsg
              = "compute_1d_mpi: output type 'csv' not currently supported";
        }
      return false;
    }
      ...
      for (unsigned int i = 0; i < (unsigned int)qsl.size (); i++)
        {
          ...
          if (!pos || pos >= sample_rotations)
            {
              errormsg
                  = QString ("1drun: 1dintermediatesaves bad value %1 found "
                             "at pos %2 in line <%3>\n"
                             "either not a positive natural number or value "
                             "larger than total sample rotations %4")
                        .arg (pos)
                        .arg (i + 1)
                        .arg (qsl[i])
  ...
  QString qs_total_plots
      = QString ("%1").arg (sample_rotations * axis_rotations);
  ...
  QString name = QString ("rank%1_%2_tp%3_rot%4_ar%5_d3R%6_rho0%7_pr%8_t%9")
                     .arg (myrank)
      ...
      if (rotations.size () > 1)
        {
          ...
          transform_to[3] = normal (cross (transform_to[1], transform_to[2]));
          ...
          if (!atom_align (transform_from, transform_to, atom_positions,
                           result))
            {
          ...
          if (!atom_align (transform_from, transform_to, excluded_volume,
                           result))
            {
      ...
      for (int t = 0; t < axis_rotations; t++)
        {
              ...
              for (unsigned int i = 0; i < s1d_data.size (); i++)
                {
                  ...
                  double S_length = sqrt (detector_distance * detector_distance
                                          + pixpos * pixpos);
                       ...
                       j < (unsigned int)excluded_volume.size (); j++)
                    {
                      ...
                      Rv[0] = (cospsi
                               + (1.0 - cospsi) * rotations[r][0]
                                     * rotations[r][0])
                              * (double)excluded_volume[j].axis[0];
                      ...
                      Rv[0] += ((1.0 - cospsi) * rotations[r][0]
                                    * rotations[r][2]
                                + rotations[r][1] * sinpsi)
                               * (double)excluded_volume[j].axis[2];

                      Rv[1]
                          = ((1.0 - cospsi) * rotations[r][0] * rotations[r][1]
                             + rotations[r][2] * sinpsi)
                            * (double)excluded_volume[j].axis[0];
                      Rv[1] += (cospsi
                      ...
                      Rv[2]
                          = ((1.0 - cospsi) * rotations[r][0] * rotations[r][2]
                             - rotations[r][1] * sinpsi)
                            * (double)excluded_volume[j].axis[0];
                      ...
                      s1d_data[i]
                          -= rho0expiQdotR
                             * complex<double> (deltaR * deltaR * deltaR, 0);
                    }
                }
            }
          ...
          if (intermediate_saves.count (npes * (r + 1))
              && t == axis_rotations - 1)
            {
...
void
US_Saxs_Util::hypercube_rejection_drand_rotations (
    unsigned int number, vector<vector<double> > &rotations)
{
  ...
  if (!myrank)
      ...
      for (int i = 1; i < npes; i++)
        {
          ...
          // send them
          if (MPI_SUCCESS
              != MPI_Send ((void *)&send_recv_rotations[0], proc_rots3,
  ...
  else
    {
      ...
      if (MPI_SUCCESS
          != MPI_Recv ((void *)&send_recv_rotations[0], proc_rots3, MPI_DOUBLE,
                       0, myrank, MPI_COMM_WORLD, &mpi_status))
        {
          MPI_Abort (MPI_COMM_WORLD, -10000 - myrank);
          exit (-10000 - myrank);
        }
...
bool
US_Saxs_Util::load_rotations_mpi (unsigned int number,
                                  vector<vector<double> > &rotations)
{
  ...
  while (!ts.atEnd () && line < number)
    {
      ...
      if (qsl.size () != 3)
        {
          errormsg = QString ("Notice: error in found cached rotations file "
  ...
  if (line < number)
    {
      errormsg
          = QString (
                "Notice: error:  cached rotations file %1 line %2, does not "
                "contain sufficient rotations (%3 requested vs %4 found)")
                .arg (f.fileName ())
                .arg (line)
                .arg (number)
                .arg (rotations.size ());
