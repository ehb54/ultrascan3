...
US_Color::US_Color(QWidget *p, const char *) : QFrame(p) {
  ...
  for (QStringList::Iterator it = entries1.begin(); it != entries1.end();
    ...
    str = (*it).toLatin1().data();
    str.truncate(
        str.lastIndexOf(".", -1, Qt::CaseInsensitive));  // strip the extension
...
void US_Color::selected_scheme(int scheme) {
  ...
  if (scheme == 0) {
    ...
    colfile =
        USglobal->config_list.system_dir + "/etc/" + selected_scheme + ".col";
...
void US_Color::delete_scheme() {
  QString str = us_tr("Do you really want to delete the\n \"") +
                ...
                us_tr("\" color scheme?");
  int result = QMessageBox::warning(0, us_tr("Attention:"), str, us_tr("Yes"),
...
void US_Color::save_as() {
  ...
  for (int i = 0; i < lb_scheme->count(); i++) {
    if (save_str ==
        lb_scheme->item(i)->text())  // if the item is already listed we don't
  ...
  QString filename;
  filename =
      QDir::toNativeSeparators(USglobal->config_list.root_dir + "/" + save_str);
...
void US_Color::update_save_str(const QString &temp_str) { save_str = temp_str; }
