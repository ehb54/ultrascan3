...
main (int argc, char **argv)
{
      ...
      if (QString (a.arguments ()[argcbase]).contains (QRegExp ("^-g")))
        {
          ...
          gui_script_filename
              = QFileInfo (gui_script_filename).absoluteFilePath ();
          argcbase++;
          continue;
        }
    }
  ...
  if (residue_file)
    {
      ...
      hydrodyn->lbl_table->setText (
          QDir::toNativeSeparators (residue_filename));
    }
  if (expert)
    {
  ...
  {
    QString icon = hydrodyn->USglobal->config_list.system_dir + "/etc/"
                   + "somo3_icon_128x128.ico";
...
void
process_script (QString script_filename, US_Hydrodyn *h)
{
  h->numThreads = 1;
          ...
          if (rx2.cap (1) == "load")
            {
              ...
              if (rx2.cap (2).toLower ().contains (QRegExp ("\\.residue$")))
                {
                  ...
                  h->read_residue_file ();
                  h->lbl_table->setText (
                      QDir::toNativeSeparators (rx2.cap (2)));
                }
              ...
              if (rx2.cap (2).toLower ().contains (
                      QRegExp (".(bead_model|beams)$")))
                {
                  ok = true;
              ...
              if (ok)
                ...
                {
                  cout << QString (
                              "unknown file load type on line %1: \"%2\"\n")
                              .arg (line)
                              .arg (c);
                  ok = true;
                }
          ...
          if (rx0.cap (1) == "grid")
            {
              ok = true;
              cout << QString ("grid\t");
              bool result = (loadfiletype == "pdb" ? h->calc_grid_pdb ()
                                                   : h->calc_grid ())
                                ? false
                                : true;
              cout << QString ("%1\n").arg (result ? "ok" : "not ok");
