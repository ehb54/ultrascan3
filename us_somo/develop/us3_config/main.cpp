...
main (int argc, char **argv)
{
  ...
  if (!OK)
  ...
  else // OK to continue
    {
      ...
      {
        QString icon = configuration->USglobal->config_list.system_dir
                       + "/etc/" + "somo3_config_128x128.ico";
...
void
USconfig_check::debug (const QString &str)
{
  // QFile f( "debug.txt" );
      ...
      else
        {
          int result = QMessageBox::information (
              ...
              us_tr ("The configuration file format is out of date.\n"
                     ...
                     "configuration file, but any customized settings will be "
                     "lost."),
  ...
  if (exists (dir + "/etc"))
    {
      ...
      debug ("Writing " + dir + "/" + QString (USRC)
             + " with system dir=" + dir);
      write_default (dir);
...
bool
USconfig_check::exists (const QString &file)
{
  QFile f (file);
  return f.exists ();
}
...
void
USconfig_check::write_default (const QString &dir)
{
  set_default (dir);
...
void
USconfig_check::set_default (const QString &system_dir)
{
...
USconfig_setup::USconfig_setup ()
{
  ...
  do
    {
      ...
      QString dir = QFileDialog::getExistingDirectory (
          this, "Select the UltraScan System Directory", QString ());
      ...
      result = QMessageBox::information (
          this, us_tr ("Setup"),
          us_tr (
              "The selected directory is not the UltraScan system directory.\n"
              "Try again."),
          QMessageBox::Ok, QMessageBox::Cancel);
    }
  ...
  // Show results
  QMessageBox::information (this, us_tr ("Setup"), us_tr (msg));
