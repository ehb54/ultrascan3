...
void
our_abort (MPI_Comm comm, int errno)
{
#if !defined(RUN_SINGLE)
...
main (int argc, char **argv)
{
  ...
  if (!world_rank)
    {
      ...
      for (int32_t i = 0; i < id.F_size; ++i)
        {
          if (MPI_SUCCESS
              != MPI_Bcast (&(F[i][0]), id.q_size, MPI_SHD_DOUBLE, 0,
                            MPI_COMM_WORLD))
            {
              cerr << "Error: MPI_Bcast( id ) sender failed" << endl;
              our_abort (MPI_COMM_WORLD, errorno);
      ...
      if (MPI_SUCCESS
          != MPI_Scatter ((void *)&(model[0]),
                          id.model_size * sizeof (struct shd_point), MPI_CHAR,
                          (void *)&(my_model[0]),
                          id.model_size * sizeof (struct shd_point), MPI_CHAR,
                          0, MPI_COMM_WORLD))
        {
      ...
      for (int i = 1; i < world_size; ++i)
        {
          ...
          if (MPI_SUCCESS
              != MPI_Isend ((void *)&(model[id.model_size * (i - 1)]),
                            id.model_size * sizeof (struct shd_point),
                            MPI_CHAR, i, i, MPI_COMM_WORLD, &(mpi_req[i - 1])))
            {
  ...
  else
    {
      ...
      cout << "max harmonics:" << world_rank << " " << id.max_harmonics
           << endl;
      ...
      if (MPI_SUCCESS
          != MPI_Scatter (null, id.model_size * sizeof (struct shd_point),
                          MPI_CHAR, &(my_model[0]),
                          id.model_size * sizeof (struct shd_point), MPI_CHAR,
                          0, MPI_COMM_WORLD))
        {
      ...
      if (MPI_SUCCESS
          != MPI_Recv ((void *)&(my_model[0]),
                       id.model_size * sizeof (struct shd_point), MPI_CHAR, 0,
                       world_rank, MPI_COMM_WORLD, &mpistat))
        {
  ...
  if (MPI_SUCCESS
      != MPI_Reduce ((void *)&(Avp[0]), (void *)&(Avpsum[0]),
  ...
  if (!world_rank)
    {
      ...
      printf ("%d of %d: compute amplitudes %gms model size %d\n", world_rank,
              world_size, (time_end - time_start) * 1e3,
              (int)my_model.size ());
    }
#endif
  MPI_Finalize ();
