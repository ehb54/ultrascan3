...
US_Reporter::US_Reporter() : US_Widgets() {
   ...
   connect(cb_runids, SIGNAL(currentIndexChanged(int)), this, SLOT(new_runid(int)));
   connect(pb_view, SIGNAL(clicked()), this, SLOT(view()));
   connect(pb_save, SIGNAL(clicked()), this, SLOT(save()));
   connect(pb_loadpr, SIGNAL(clicked()), this, SLOT(load_profile()));
   connect(pb_savepr, SIGNAL(clicked()), this, SLOT(save_profile()));
   connect(pb_syncdb, SIGNAL(clicked()), this, SLOT(sync_db()));
   connect(pb_help, SIGNAL(clicked()), this, SLOT(help()));
   connect(pb_close, SIGNAL(clicked()), this, SLOT(close()));

   cb_runids->setToolTip(tr("Select the Run for which to produce a composite report"));
   pb_view->setToolTip(tr("View the composite report of selected individual reports"));
   pb_save->setToolTip(tr("Create a composite report using selected individual reports"));
   pb_loadpr->setToolTip(tr("Load a previously saved report-selection profile"));
   pb_savepr->setToolTip(tr("Save a report-selection profile for later use"));
   pb_help->setToolTip(tr("Display detailed US_Reporter documentation text and images"));
   pb_close->setToolTip(tr("Close the US_Reporter window and exit"));
   ...
   tw_recs->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 1));
   tw_recs->setObjectName(QString("tree-widget"));
   tw_recs->setAutoFillBackground(true);
   tw_recs->installEventFilter(this);

   connect(tw_recs, SIGNAL(itemPressed(QTreeWidgetItem *, int)), this, SLOT(clickedItem(QTreeWidgetItem *)));
   connect(tw_recs, SIGNAL(itemChanged(QTreeWidgetItem *, int)), this, SLOT(changedItem(QTreeWidgetItem *, int)));
...
void US_Reporter::changedItem(QTreeWidgetItem *item, int col) {
   //DbgLv(1) << "changedItem";
   if (col == 0 && change_tree) { // If column 0 changed (state), set new state in tree
...
void US_Reporter::row_context(QTreeWidgetItem *item) {
   ...
   connect(showact, SIGNAL(triggered()), this, SLOT(item_show()));
   connect(viewact, SIGNAL(triggered()), this, SLOT(item_view()));
   connect(saveact, SIGNAL(triggered()), this, SLOT(item_save()));
...
void US_Reporter::build_runids() {
   ...
   QStringList rdirs = QDir(rdir).entryList(QDir::AllDirs | QDir::NoDotAndDotDot, QDir::Name);
   QStringList aucfil("*.auc");
   rdir = rdir + "/";

   for (int ii = 0; ii < rdirs.size(); ii++) { // Examine the subdirectories of the results directory
      QString runid = rdirs.at(ii);
      QString subdir = rdir + runid;
      // Get the list of AUC files in the subdirectory
      QStringList afiles = QDir(subdir).entryList(aucfil, QDir::Files, QDir::Name);
      int naucf = afiles.size();

      if (naucf > 0) { // Add the Run to the list for the Runs combo box
         if (nruns == 0)
...
void US_Reporter::new_runid(int row) {
   ...
   if (runID == "all") { // For run "all", loop to build descriptions for all runs
...
void US_Reporter::build_descs(QString &runID, int &linex) {
   ...
   QString rdir = US_Settings::resultDir().replace("\\", "/") + "/" + runID + "/";
   QStringList aucfil("*.auc");
   QStringList afiles = QDir(rdir).entryList(aucfil, QDir::Files, QDir::Name);
   int naucf = afiles.size();
   DbgLv(1) << " BD:  naucf" << naucf << "aucfil" << aucfil[ 0 ];

   for (int ii = 0; ii <= naucf; ii++) { // Examine the AUC files in the Run's results subdirectory
      QString fname;
      QString trnam;
      QString tripl;
      ...
      if (ii < naucf) {
         ...
         tripl = fname.section(".", -4, -4) + " / " + fname.section(".", -3, -3) + " / " + fname.section(".", -2, -2);
      }
      ...
      QStringList rfiles = QDir(path).entryList(trifil, QDir::Files, QDir::Name);
      ...
      cdesc.linen = linex + 1;
      cdesc.level = 1;
      cdesc.label = tr("Cell ") + tripl.section("/", 0, 0).simplified() + tr("/Channel ")
                    + tripl.section("/", 1, 1).simplified() + "/" + tripl.section("/", 2, 2).simplified() + " nm";
      ...
      for (int jj = 0; jj < appnames.size(); jj++) { // Examine possible application (analysis) names
         ...
         rafilt << appnmLo + "." + trnam + ".*.*" << appnmUp + "." + trnam + ".*.*";
         QStringList rafiles = QDir(path).entryList(rafilt, QDir::Files, QDir::Name);
         DbgLv(1) << " BD:   nappf" << rafiles.size() << "rafilt" << rafilt[ 0 ] << rafilt[ 1 ] << rafilt[ 2 ];
         ...
         for (int kk = 0; kk < extnames.size(); kk++) { // Examine possible extensions
            QString extname = extnames.at(kk);
            QString exlabel = extmap[ extname ];
            extname = extname.section(":", 1, 1);

            for (int mm = 0; mm < rptnames.size(); mm++) { // Examine possible reports
               ...
               if (rafiles.indexOf(rpfname) < 0) { // Skip if name (even lower/upper case version) not in list
               ...
               if (rpfname.contains(".svg")) { // Skip if SVG file has PNG equivalent
                  QString rpfnpng = QString(rpfname).section(".", 0, -2) + ".png";
               ...
               cdesc.lastmodDate = US_Util::toUTCDatetimeText(
                  QFileInfo(cdesc.filepath).lastModified().toUTC().toString(Qt::ISODate), true);
...
void US_Reporter::build_map(QString ttag, QMap<QString, QString> &labmap) {
   ...
   if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
      ...
      QMessageBox::warning(this, tr("UltraScan Error"), tr("Unable to open the file:\n\n") + path);
      return;
   }
...
void US_Reporter::build_tree() {
   ...
   for (int ii = 0; ii < nitems; ii++) { // Loop to add items to the data tree
      cdesc = adescs.at(ii);
      int lev = cdesc.level;
      cvals.clear();
      ...
      if (lev > 0) { // For all beyond the first, a tree item is attached to a parent item
      ...
      else { // The first item is attached to the base tree root
   ...
   if (nitems > 15) { // If there are many items, collapse some or all of them
      if (nl2its > 10)
         tw_recs->collapseAll(); // Collapse all if many analyses

      else { // If not so many, collapse only analysis subtrees
         QList<QTreeWidgetItem *> items
            = tw_recs->findItems(QString(""), Qt::MatchFixedString | Qt::MatchWrap | Qt::MatchRecursive, 0);
...
void US_Reporter::count_children(DataDesc *idesc, int &nchild, int &nchkd) {
   ...
   for (int jj = jndx; jj < nitems; jj++) { // Examine items from next line to the end
      DataDesc *chdesc = ( DataDesc * ) &adescs.at(jj);
      int clev = chdesc->level;

      if (clev <= plev)
         break; // Break at sibling or end of siblings

      nchild++; // Bump count of descendants

      if (chdesc->checkState == 2)
         nchkd++; // Bump count of checked descendants
      DbgLv(1) << "  CnCh: jj clev plev nchild nchkd" << jj << clev << plev << nchild << nchkd;
   }
...
void US_Reporter::state_children(DataDesc *idesc, int &state) {
   ...
   if (jndx == nitems)
      return; // Nothing more to do for the last item

   for (int jj = jndx; jj < nitems; jj++) { // Examine items from next one to the end
      DataDesc *chdesc = ( DataDesc * ) &adescs.at(jj);
      int clev = chdesc->level;
...
void US_Reporter::state_parents(DataDesc *idesc, int &state) {
   ...
   for (int jj = lndx; jj >= 0; jj--) { // Examine items from next above back to the first item
      DataDesc *pdesc = ( DataDesc * ) &adescs.at(jj);
      int plev = pdesc->level;
      DbgLv(1) << "  StPa: jj plev clev state" << jj << plev << clev << state;

      if (plev >= clev)
         continue; // Ignore children or siblings

      count_children(pdesc, nchild, nchkd); // Reset parent state
      DbgLv(1) << "  StPa:   nchild nchkd" << nchild << nchkd;
   }

   if (nchkd > 0) { // If any items are checked, enable View and Save buttons
      pb_view->setEnabled(true);
      pb_save->setEnabled(true);
   }

   else { // If no items are checked, disable View and Save buttons
      pb_view->setEnabled(false);
      pb_save->setEnabled(false);
   }
}
...
void US_Reporter::mark_checked() {
   ...
   for (int jj = 0; jj < adescs.size(); jj++) { // Set the tree item state according to the data description setting
      //DbgLv(1) << "  MkCk:   jj" << jj;
      item = items.at(jj);
...
void US_Reporter::view() {
   ...
   { // Open the PDF file for viewing with system's "open-with" app
      ...
      if (!QDesktopServices::openUrl(file_url))
         QMessageBox::warning(
            this, tr("Composite Report *ERROR*"), tr("Unable to open the composite report file:\n") + ppdfpath);
   }

   else { // Report error in creating PDF
      QMessageBox::warning(
         this, tr("Composite Report *ERROR*"), tr("Unable to create a composite report file:\n") + ppdfpath);
   }
}
...
bool US_Reporter::write_report() {
   ...
   if (nsrpts == 0) {
      QMessageBox::warning(this, tr("Select Error"), tr("No reports have been selected"));
      QApplication::restoreOverrideCursor();
      qApp->processEvents();
      return false;
   }
   ...
   int logof = (hsclogo.isEmpty() ? 0 : 4) + (becklogo.isEmpty() ? 0 : 2) + (us3logo.isEmpty() ? 0 : 1);

   if (logof != 0) {
      rptpage += "    <p>\n";

      if ((logof & 4) != 0)
         rptpage += "      <img src=\"" + hsclogo + "\" alt=\"HSC logo\"/>\n";

      if ((logof & 2) != 0)
         rptpage += "      <img src=\"" + becklogo + "\" alt=\"Beckman logo\"/>";

      if ((logof & 1) != 0) {
         if (logof > 1)
            rptpage += "<br/>";

         rptpage += "\n      <img src=\"" + us3logo + "\" alt=\"Ultrascan III logo\"/>\n";
      }
   ...
   for (int ii = 0; ii < nsrpts; ii++) { // Compose an entry in the composite HTML for each component item
      ...
      if (is_plot) { // Is a plot:  new page if 2nd or after non-plot
         ...
         if (jplot != 1 && ii > 0) { // Previous was 2nd plot on the page or was a non-plot
            //DbgLv(1) << "++jplot" << jplot << "ii" << ii << "NEW PAGE";
            rptpage += ppageclass;
            jplot = 1; // mark as 1st plot on page
         }

         else { // Previous was the 1st plot on the page
            if ((chght + phght) < mxpageht) { // If 2 plots will fit on a page, mark this plot as second
               //DbgLv(1) << "++comb.height (NOPAGE)" << (chght+phght) << "mxpageht" << mxpageht;
               rptpage += pheadclass;
               jplot = 2; // mark as 2nd plot on page
            }

            else { // If second plot on page will exceed space, force a new page
      ...
      else if (ii > 0) { // Is not a plot and not the 1st item:  starts a new page
         rptpage += ppageclass;
      ...
      if (is_plot) { // The item is a plot, so "<img.." will be used
         QString fileimg = idesc->filename;

         if (fileimg.contains(".svg")) { // For SVG[Z], create a PNG equivalent
            QSvgRenderer svgrend;
            ...
            {
               QMessageBox::warning(
                  this, tr("Composite Report *ERROR*"), tr("Unable to create an svg-to-png file:\n") + pathimg);
            }
         ...
         rptpage += "    <div><img src=\"" + fileimg + "\" alt=\"" + idesc->label + "\"/></div>\n\n";
      }

      else if (is_data) { // The item is Data text, so copy it with line breaks added
      ...
      else { // The item is HTML, so copy it with header,footer removed
         QFile fi(idesc->filepath);
         if (fi.open(QIODevice::ReadOnly | QIODevice::Text)) {
            QTextStream ts(&fi);
            int stage = 0;
            ...
            while (!ts.atEnd()) {
               ...
               if (stage == 0) { // skip early part of component HTML until body
                  if (ln.contains("<body>"))
               ...
               else if (ln.contains("</body>")) { // mark end of body
                  stage = 2;
   ...
   QFile flo(pagepath);
   if (flo.open(
          QIODevice::WriteOnly | QIODevice::Truncate)) { // Output the composite page to a file in the composite folder
      QTextStream tso(&flo);
      tso << rptpage;
      flo.close();
   }
...
void US_Reporter::write_pdf() {
   ...
   if (!QFile::copy(ppdffold, ppdfpath)) {
      QMessageBox::information(this, tr("UltraScan Error"), tr("Unable to (over-)write the file:\n\n") + ppdfpath);
   }
   load_ok = true;
}
...
bool US_Reporter::count_reports() {
   ...
   for (int ii = 0; ii < adescs.size(); ii++) { // Review data description records
      DataDesc *idesc = ( DataDesc * ) &adescs.at(ii);

      DbgLv(1) << "cnt_rpt: ii lev state" << ii << idesc->level << idesc->checkState;
      if (idesc->level != 3 || idesc->checkState != 2)
         continue; // Only reports checked
      ...
      if (nsruns == 0 || !se_runids.contains(idesc->runid)) { // Save a list of unique runIDs
         nsruns++;
      ...
      DbgLv(1) << "cnt_rpt:  ns rpts,runs,htmls,plots" << nsrpts << nsruns << nshtmls << nsplots;
   }
...
void US_Reporter::item_view() {
   QString fileexts = tr("HTML files (*.html);;PLOT files (*.svgz *.svg *.png);;"
   ...
   if (cdesc.type.contains("Plot")) { // For plots, write an <img ...> line
      mtext = QString("<head>\n<title>") + cdesc.filepath + QString("</title>\n</head>\n<img src=\"") + cdesc.filepath
              + QString("\" alt=\"" + cdesc.label + "\" />\n");
   }

   else { // For non-plot, simply copy the file itself
...
void US_Reporter::item_show() {
   ...
   QString mtext = tr("Data Tree Item at Row %1 -- \n\n").arg(row + 1) + tr("  Tree Level     : ")
                   + QString::number(cdesc.level) + "\n" + tr("  Check State    : ") + QString::number(cdesc.checkState)
...
void US_Reporter::item_save() {
   ...
   QString fileexts = fileext + tr(" files (*.") + fileext + ");;" + fileanp + tr(" files (") + fileanp + "*);;"
                      + tr("All files (*)");
   QString ofilname = archdir + filename;

   // Open a dialog and get the Save-As file path name
   QString fn = QFileDialog::getSaveFileName(this, tr("Save Report File As ..."), ofilname, fileexts);
...
void US_Reporter::load_profile() {
   ...
   QString fn = QFileDialog::getOpenFileName(
      this, tr("Load Report-Select Parameters in:"), US_Settings::etcDir(),
   ...
   if (xfi.open(QIODevice::ReadOnly | QIODevice::Text)) {
      ...
      while (!xml.atEnd()) { // Get all unique analysis+report selections from the XML file
         xml.readNext();
   ...
   for (int ii = 0; ii < adescs.size(); ii++) { // Propagate selections to report tree
      DataDesc *idesc = ( DataDesc * ) &adescs.at(ii);

      if (idesc->level != 3)
         continue; // Only reports
...
void US_Reporter::save_profile() {
   ...
   QString fn = QFileDialog::getSaveFileName(
      this, tr("Save Report-Selection Parameters in:"), US_Settings::etcDir(),
      tr("ReportSelect files (rs_*.xml);;"
         "All XML files (*.xml);;"
         "All files (*)"));
   ...
   if (xfo.open(QIODevice::WriteOnly | QIODevice::Text)) { // Write unique selections to the XML file
      ...
      for (int ii = 0; ii < adescs.size(); ii++) { // Examine all the data records, looking for selections
         DataDesc *idesc = ( DataDesc * ) &adescs.at(ii);
...
void US_Reporter::copy_logos(QString cmppath) {
   ...
   if (hsclogo.startsWith("..") && becklogo.startsWith("..") && us3logo.startsWith(".."))
   ...
   QString relpath = "../etc/";
   hsclogo = hsclogo.isEmpty() ? hsclogo : hsclogo.replace(etcpath, relpath);
   becklogo = becklogo.isEmpty() ? becklogo : becklogo.replace(etcpath, relpath);
   us3logo = us3logo.isEmpty() ? us3logo : us3logo.replace(etcpath, relpath);
   ...
   if (
      (hsclogo.isEmpty() || QFile(hscnewp).exists()) && (becklogo.isEmpty() || QFile(becknewp).exists())
   ...
   if (!dircmp.mkpath(etcnewp)) {
      QMessageBox::warning(
         this, tr("Composite Report *ERROR*"), tr("Unable to create a composite report directory:\n") + etcnewp);
      return;
   }
