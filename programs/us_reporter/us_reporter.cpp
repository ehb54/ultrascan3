...
void
US_Reporter::clickedItem (QTreeWidgetItem *item)
{
...
void
US_Reporter::changedItem (QTreeWidgetItem *item, int col)
{
  // DbgLv(1) << "changedItem";
...
void
US_Reporter::row_context (QTreeWidgetItem *item)
{
  int row = item->type () - (int)QTreeWidgetItem::UserType;
...
void
US_Reporter::build_runids ()
{
  int nruns = 0;
  QString rdir = US_Settings::resultDir ().replace ("\\", "/");
  QStringList rdirs = QDir (rdir).entryList (
...
void
US_Reporter::build_descs (QString &runID, int &linex)
{
  ...
  for (int ii = 0; ii <= naucf; ii++)
    { // Examine the AUC files in the Run's results subdirectory
      ...
      QStringList rfiles
          = QDir (path).entryList (trifil, QDir::Files, QDir::Name);
      ...
      for (int jj = 0; jj < appnames.size (); jj++)
        { // Examine possible application (analysis) names
          ...
          DbgLv (1) << " BD:   nappf" << rafiles.size () << "rafilt"
                    << rafilt[0] << rafilt[1] << rafilt[2];
                ...
                { // Examine possible reports
                  ...
                  if (rpfname.contains (".svg"))
                    { // Skip if SVG file has PNG equivalent
                      QString rpfnpng
                          = QString (rpfname).section (".", 0, -2) + ".png";
                  ...
                  adescs << cdesc;
                  linex++;
                  DbgLv (1) << " BD: line lev label" << cdesc.linen
                            << cdesc.level << cdesc.label;
...
void
US_Reporter::build_map (QString ttag, QMap<QString, QString> &labmap)
{
  int kmap = 0;
...
void
US_Reporter::count_children (DataDesc *idesc, int &nchild, int &nchkd)
{
  int nitems = adescs.size ();
...
void
US_Reporter::state_children (DataDesc *idesc, int &state)
{
  int nitems = adescs.size ();
  int jndx = idesc->linen;
  int plev = idesc->level;
...
void
US_Reporter::state_parents (DataDesc *idesc, int &state)
{
  ...
  if (logof != 0)
    {
      ...
      if ((logof & 2) != 0)
        rptpage
            += "      <img src=\"" + becklogo + "\" alt=\"Beckman logo\"/>";
    ...
    { // Compose an entry in the composite HTML for each component item
      ...
      if (is_plot)
        { // Is a plot:  new page if 2nd or after non-plot
          ...
          DbgLv (1) << "  plot.height" << chght << idesc->filename << "ph"
                    << phght;
            ...
            { // Previous was the 1st plot on the page
              if ((chght + phght) < mxpageht)
                { // If 2 plots will fit on a page, mark this plot as second
                  // DbgLv(1) << "++comb.height (NOPAGE)" << (chght+phght) <<
                  // "mxpageht" << mxpageht;
                  rptpage += pheadclass;
      ...
      if (is_plot)
        { // The item is a plot, so "<img.." will be used
          ...
          rptpage += "    <div><img src=\"" + fileimg + "\" alt=\""
                     + idesc->label + "\"/></div>\n\n";
        }
...
US_Reporter::item_show ()
{
  ...
  QString mtext = tr ("Data Tree Item at Row %1 -- \n\n").arg (row + 1)
                  ...
                  + "\n" + tr ("  Check State    : ")
                  + QString::number (cdesc.checkState) + "\n"
...
US_Reporter::item_save ()
{
  ...
  QString fn = QFileDialog::getSaveFileName (
      this, tr ("Save Report File As ..."), ofilname, fileexts);
...
void
US_Reporter::save_profile ()
{
  // Open a file dialog to get a name for the profile save file
  QString fn = QFileDialog::getSaveFileName (
      this, tr ("Save Report-Selection Parameters in:"),
      US_Settings::etcDir (),
      tr ("ReportSelect files (rs_*.xml);;"
          "All XML files (*.xml);;"
          "All files (*)"));
...
void
US_Reporter::copy_logos (QString cmppath)
{
  ...
  becklogo
      = becklogo.isEmpty () ? becklogo : becklogo.replace (etcpath, relpath);
...
QString
US_Reporter::pad_line (const QString linein)
{
  QString lineout;
  const QChar cbln (' ');
  const QChar ctab ('\t');
