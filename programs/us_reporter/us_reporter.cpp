...
US_Reporter::US_Reporter() : US_Widgets() {
  ...
  if (US_Settings::debug_match("hsclogo")) hsclogo = logopath + "logo_hsc.png";
  ...
  connect(cb_runids, SIGNAL(currentIndexChanged(int)), this,
          SLOT(new_runid(int)));
  ...
  connect(tw_recs, SIGNAL(itemChanged(QTreeWidgetItem*, int)), this,
...
void US_Reporter::changedItem(QTreeWidgetItem* item, int col) {
  // DbgLv(1) << "changedItem";
  if (col == 0 &&
      change_tree) {  // If column 0 changed (state), set new state in tree
...
void US_Reporter::build_runids() {
  ...
  for (int ii = 0; ii < rdirs.size();
       ii++) {  // Examine the subdirectories of the results directory
    QString runid = rdirs.at(ii);
    QString subdir = rdir + runid;
    // Get the list of AUC files in the subdirectory
    QStringList afiles =
        QDir(subdir).entryList(aucfil, QDir::Files, QDir::Name);
    int naucf = afiles.size();

    if (naucf > 0) {  // Add the Run to the list for the Runs combo box
      if (nruns == 0) sl_runids << "all";  // Prefix the list with "all"
...
void US_Reporter::new_runid(int row) {
  ...
  if (runID ==
      "all") {  // For run "all", loop to build descriptions for all runs
...
void US_Reporter::build_descs(QString& runID, int& linex) {
  ...
  if (linex == 0) adescs.clear();  // If first line, clear list
  ...
  linex++;
  DbgLv(1) << " BD: line lev label" << cdesc.linen << cdesc.level
           << cdesc.label;

  QString rdir =
      US_Settings::resultDir().replace("\\", "/") + "/" + runID + "/";
  QStringList aucfil("*.auc");
  QStringList afiles = QDir(rdir).entryList(aucfil, QDir::Files, QDir::Name);
  int naucf = afiles.size();
  DbgLv(1) << " BD:  naucf" << naucf << "aucfil" << aucfil[0];

  for (int ii = 0; ii <= naucf;
       ii++) {  // Examine the AUC files in the Run's results subdirectory
    QString fname;
    QString trnam;
    QString tripl;
    ...
    if (ii < naucf) {
      ...
      tripl = fname.section(".", -4, -4) + " / " + fname.section(".", -3, -3) +
              " / " + fname.section(".", -2, -2);
    }
    ...
    cdesc.linen = linex + 1;
    cdesc.level = 1;
    cdesc.label = tr("Cell ") + tripl.section("/", 0, 0).simplified() +
                  tr("/Channel ") + tripl.section("/", 1, 1).simplified() +
    ...
    for (int jj = 0; jj < appnames.size();
         jj++) {  // Examine possible application (analysis) names
      ...
      rafilt << appnmLo + "." + trnam + ".*.*"
             << appnmUp + "." + trnam + ".*.*";
      QStringList rafiles =
          QDir(path).entryList(rafilt, QDir::Files, QDir::Name);
      ...
      for (int kk = 0; kk < extnames.size();
           kk++) {  // Examine possible extensions
        QString extname = extnames.at(kk);
        QString exlabel = extmap[extname];
        extname = extname.section(":", 1, 1);

        for (int mm = 0; mm < rptnames.size();
             mm++) {  // Examine possible reports
          ...
          // DbgLv(1) << " BD:     nrptf" << rafiles.size() << "rpfname" <<
          // rpfname;

          if (rafiles.indexOf(rpfname) <
              0) {  // Skip if name (even lower/upper case version) not in list
            int jj1 = rafiles.indexOf(appnmLo + basname);
            int jj2 = rafiles.indexOf(appnmUp + basname);
            if (jj1 < 0 && jj2 < 0) continue;
            rpfname = (jj1 < 0) ? rpfname : rpfnmLo;
            rpfname = (jj2 < 0) ? rpfname : rpfnmUp;
          }

          if (rpfname.contains(
                  ".svg")) {  // Skip if SVG file has PNG equivalent
            QString rpfnpng = QString(rpfname).section(".", 0, -2) + ".png";
            if (rafiles.indexOf(rpfnpng) >= 0) continue;
          }
          ...
          cdesc.analysis = aplabel;
          cdesc.lastmodDate =
              US_Util::toUTCDatetimeText(QFileInfo(cdesc.filepath)
                                             .lastModified()
                                             .toUTC()
                                             .toString(Qt::ISODate),
                                         true);
...
void US_Reporter::build_tree() {
  ...
  for (int ii = 0; ii < nitems; ii++) {  // Loop to add items to the data tree
    ...
    if (lev > 0) {  // For all beyond the first, a tree item is attached to a
                    // parent item
  ...
  if (nitems > 15) {  // If there are many items, collapse some or all of them
    if (nl2its > 10)
      ...
      QList<QTreeWidgetItem*> items = tw_recs->findItems(
          QString(""),
          Qt::MatchFixedString | Qt::MatchWrap | Qt::MatchRecursive, 0);

      for (int ii = 0; ii < nitems; ii++)
        if (adescs.at(ii).level == 2) tw_recs->collapseItem(items.at(ii));
    }
  }
...
void US_Reporter::count_children(DataDesc* idesc, int& nchild, int& nchkd) {
  ...
  for (int jj = jndx; jj < nitems;
       jj++) {  // Examine items from next line to the end
    ...
    if (chdesc->checkState == 2) nchkd++;  // Bump count of checked descendants
...
void US_Reporter::state_children(DataDesc* idesc, int& state) {
  ...
  DbgLv(1) << "  StCh: items jndx plev state" << nitems << jndx << plev
           << state;

  if (jndx == nitems) return;  // Nothing more to do for the last item

  for (int jj = jndx; jj < nitems;
       jj++) {  // Examine items from next one to the end
    DataDesc* chdesc = (DataDesc*)&adescs.at(jj);
    int clev = chdesc->level;
...
void US_Reporter::state_parents(DataDesc* idesc, int& state) {
  ...
  for (int jj = lndx; jj >= 0;
       jj--) {  // Examine items from next above back to the first item
...
void US_Reporter::mark_checked() {
  QList<QTreeWidgetItem*> items = tw_recs->findItems(
      QString(""), Qt::MatchFixedString | Qt::MatchWrap | Qt::MatchRecursive,
      0);
  QTreeWidgetItem* item;
  change_tree = false;  // Disable slot for item change
                        // DbgLv(1) << "  MkCk: items size" << items.size();

  for (int jj = 0; jj < adescs.size();
       jj++) {  // Set the tree item state according to the data description
                // setting
                // DbgLv(1) << "  MkCk:   jj" << jj;
    item = items.at(jj);
...
void US_Reporter::view() {
  ...
  {  // Open the PDF file for viewing with system's "open-with" app
    ...
    if (!QDesktopServices::openUrl(file_url))
      QMessageBox::warning(
          this, tr("Composite Report *ERROR*"),
...
bool US_Reporter::write_report() {
  ...
  int logof = (hsclogo.isEmpty() ? 0 : 4) + (becklogo.isEmpty() ? 0 : 2) +
              (us3logo.isEmpty() ? 0 : 1);
  ...
  for (int ii = 0; ii < nsrpts; ii++) {  // Compose an entry in the composite
                                         // HTML for each component item
    ...
    if (is_plot) {  // Is a plot:  new page if 2nd or after non-plot
      ...
      if (jplot != 1 &&
          ii > 0) {  // Previous was 2nd plot on the page or was a non-plot
        ...
        if ((chght + phght) < mxpageht) {  // If 2 plots will fit on a page,
                                           // mark this plot as second
          // DbgLv(1) << "++comb.height (NOPAGE)" << (chght+phght) << "mxpageht"
          // << mxpageht;
          rptpage += pheadclass;
          ...
          DbgLv(1) << "++comb.height" << (chght + phght) << "NEW PAGE"
                   << idesc->filename;
          rptpage += ppageclass;
          jplot = 1;  // mark as 1st plot on page
        }
             ...
             0) {  // Is not a plot and not the 1st item:  starts a new page
      rptpage += ppageclass;
    ...
    if (is_plot) {  // The item is a plot, so "<img.." will be used
      ...
      rptpage += "    <div><img src=\"" + fileimg + "\" alt=\"" + idesc->label +
                 "\"/></div>\n\n";
    }

    else if (is_data) {  // The item is Data text, so copy it with line breaks
                         // added
  ...
  QFile flo(pagepath);
  if (flo.open(QIODevice::WriteOnly |
               QIODevice::Truncate)) {  // Output the composite page to a file
                                        // in the composite folder
    QTextStream tso(&flo);
    tso << rptpage;
    flo.close();
  }
...
bool US_Reporter::count_reports() {
  ...
  for (int ii = 0; ii < adescs.size();
       ii++) {  // Review data description records
    DataDesc* idesc = (DataDesc*)&adescs.at(ii);

    DbgLv(1) << "cnt_rpt: ii lev state" << ii << idesc->level
             << idesc->checkState;
    if (idesc->level != 3 || idesc->checkState != 2)
      continue;  // Only reports checked

    if (nsrpts == 0) cdesc = *idesc;  // Save first found
    ...
    if (nsruns == 0 ||
        !se_runids.contains(idesc->runid)) {  // Save a list of unique runIDs
      nsruns++;
...
void US_Reporter::item_view() {
  QString fileexts =
      tr("HTML files (*.html);;PLOT files (*.svgz *.svg *.png);;"
  ...
  if (cdesc.type.contains("Plot")) {  // For plots, write an <img ...> line
    ...
    if (fi.open(QIODevice::ReadOnly | QIODevice::Text)) {
      ...
      while (!ts.atEnd()) mtext += ts.readLine() + "\n";
...
void US_Reporter::item_save() {
  ...
  QString fileexts = fileext + tr(" files (*.") + fileext + ");;" + fileanp +
                     tr(" files (") + fileanp + "*);;" + tr("All files (*)");
  QString ofilname = archdir + filename;

  // Open a dialog and get the Save-As file path name
  QString fn = QFileDialog::getSaveFileName(this, tr("Save Report File As ..."),
                                            ofilname, fileexts);
...
void US_Reporter::load_profile() {
  ...
  if (xfi.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    while (!xml.atEnd()) {  // Get all unique analysis+report selections from
                            // the XML file
      xml.readNext();
  ...
  for (int ii = 0; ii < adescs.size();
       ii++) {  // Propagate selections to report tree
    DataDesc* idesc = (DataDesc*)&adescs.at(ii);
...
void US_Reporter::save_profile() {
  ...
  if (xfo.open(QIODevice::WriteOnly |
               QIODevice::Text)) {  // Write unique selections to the XML file
    ...
    for (int ii = 0; ii < adescs.size();
         ii++) {  // Examine all the data records, looking for selections
      DataDesc* idesc = (DataDesc*)&adescs.at(ii);
...
void US_Reporter::copy_logos(QString cmppath) {
  ...
  if (hsclogo.startsWith("..") && becklogo.startsWith("..") &&
      us3logo.startsWith(".."))
...
QString US_Reporter::pad_line(const QString linein) {
  ...
  for (int ii = 0; ii < linein.size(); ii++) {
    ...
    else if (lchar == ctab)  // Replace tab with 2-5 NBSPs
    {
      ...
      for (int jj = 0; jj < nspc; jj++) lineout.append(s_nbsp);
    }
