...
US_SyncWithDB::US_SyncWithDB() : US_WidgetsDialog(0, 0) {
   ...
   QString name = (invlev > 0) ? QString::number(US_Settings::us_inv_ID()) + ": " : "";
   ...
   QString desc = tr("<b>Note:</b> Proceeding may result in local reports<br/>"
...
void US_SyncWithDB::scan_db_reports() {
   ...
   QDir dirres(resdir);
   QStringList resruns = dirres.entryList(QDir::AllDirs | QDir::NoDotAndDotDot, QDir::Name);
   ...
   desc = desc.left(nbdchr) + tr("DB reports for local runIDs is %1").arg(kreports);
   te_desc->setHtml(desc);
   qApp->processEvents();
   ...
   for (int ii = 0; ii < kreports; ii++) {
      ...
      for (int jj = 0; jj < ntriples; jj++) {
         ...
         desc = desc.left(nbdchr)
                + tr("Run %1, Triple %2: comparing %3 documents...").arg(ii + 1).arg(jj + 1).arg(ndocs);
         te_desc->setHtml(desc);
         qApp->processEvents();
   ...
   QMessageBox::information(
      ...
      tr("Run IDs: %1 updated, %2 added.\n"
         ...
         .arg(ndocrpl)
         .arg(ndocadd));
   qDebug() << "nrunrpl nrunadd ndocrpl ndocadd" << nrunrpl << nrunadd << ndocrpl << ndocadd;
}
