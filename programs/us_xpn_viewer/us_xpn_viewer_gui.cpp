...
SpeedoMeter::SpeedoMeter(QWidget *parent) : QwtDial(parent), d_label("RPM") {
   ...
   QwtDialSimpleNeedle *needle
      = new QwtDialSimpleNeedle(QwtDialSimpleNeedle::Arrow, true, Qt::red, QColor(Qt::gray).light(130));
   setNeedle(needle);
}
...
// WheelBox
WheelBox::WheelBox(Qt::Orientation orientation, double minv, double maxv, QString label, bool log, QWidget *parent) :
    QWidget(parent) {
...
US_XpnDataViewer::US_XpnDataViewer(QString auto_mode) : US_Widgets() {
   ...
   connect(cb_cellchn, SIGNAL(currentIndexChanged(int)), this, SLOT(changeCellCh()));
   connect(cb_rstart, SIGNAL(currentIndexChanged(int)), this, SLOT(changeRadius()));
   connect(cb_rend, SIGNAL(currentIndexChanged(int)), this, SLOT(changeRadius()));
   connect(cb_pltrec, SIGNAL(currentIndexChanged(int)), this, SLOT(changeRecord()));
   ...
   connect(ct_rinterv, SIGNAL(valueChanged(double)), this, SLOT(changeInterval()));
   ...
   plot = new US_Plot(data_plot, tr("Intensity Data"), tr("Radius (in cm)"), tr("Intensity"));
   ...
   connect(plot, SIGNAL(zoomedCorners(QRectF)), this, SLOT(currentRectf(QRectF)));
   ...
   //plot RPM
   plot_rpm = new US_Plot(data_plot_rpm, "", tr("Elapsed Time (minutes)"), tr("RPM"));
   QFont tfont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(), QFont::Bold);
   ...
   legend->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() - 1));
   ...
   picker_rpm->setMousePattern(QwtEventPattern::MouseSelect1, Qt::LeftButton, Qt::ControlModifier);
   ...
   connect(plot_rpm, SIGNAL(zoomedCorners(QRectF)), this, SLOT(currentRectf(QRectF)));
...
void US_XpnDataViewer::reset(void) {
   ...
   connect(xpn_data, SIGNAL(status_text(QString)), this, SLOT(status_report(QString)));
...
bool US_XpnDataViewer::check_sysdata_connection(void) {
   ...
   qDebug() << "status_sys_data & connected_itself = ? " << status_sys_data << " & " << link1->connected_itself << " = "
            << combined_check;
   ...
   if (!combined_check) {
      ...
      QString msg_sys_text = QString("Attention! UltraScan is not able to communicate with the data acquisition server "
                                     "on the %1. Please check the following: ")
                                .arg(xpndesc);
      QString msg_sys_text_info
         = QString(
              "1. %1 is turned on \n2. the data acquisition server on %1 is running \n3. your license key is stored in "
              "$HOME/ultrascan/etc/optima and is not expired \n\nUse of the Optima Data Viewer is suspended until this "
...
void US_XpnDataViewer::test_optima_connection() {
   ...
   if (xpnhost.isEmpty() || xpnport.isEmpty() || xpnname.isEmpty() || xpnuser.isEmpty() || xpnpasw.isEmpty()) {
...
void US_XpnDataViewer::enableControls(void) {
   ...
   DbgLv(1) << "ec: ncc nwl nsc npt ntpt" << ncellch << nlambda << nscan << npoint << ntpoint << "Mwl" << isMWL;
...
bool US_XpnDataViewer::load_xpn_raw_auto() {
   ...
   if (RunID_to_retrieve.toInt() == 0) {
      ...
      if (!runID_passed.isEmpty()) {
         if (runID_passed.toInt() != RunID_to_retrieve.toInt()) {
            ...
            QMessageBox::warning(
               ...
               tr("It appears the scheduled run can no longer be accessed, and was probably terminated prematurely. "
                  "The GMP record will be deleted. Please resubmit the protocol from Ultrascan GMP."));
      ...
      xpn_data->scan_runs_auto(
         runInfo, RunID_to_retrieve); // ALEXEY initial query (for us_comproject needs to be based on ExpId )
...
void US_XpnDataViewer::update_autoflow_runId_timeStarted(void) {
   ...
   QStringList qry;
   qry << "update_autoflow_runid_starttime" << ExpID_to_use << RunID_to_retrieve << OptimaName;
...
void US_XpnDataViewer::updateautoflow_record_atLiveUpdate(void) {
   ...
   QStringList qry;
   qry << "update_autoflow_at_live_update" << RunID_to_retrieve << currentDir << OptimaName;
   ...
   if (correctRadii == "NO") {
      qry.clear();
      qry << "update_autoflow_at_live_update_radiicorr" << RunID_to_retrieve << OptimaName;
   ...
   if (experimentAborted) {
      qry.clear();
      qry << "update_autoflow_at_live_update_expaborted" << RunID_to_retrieve << OptimaName;
   ...
   if (experimentAborted_remotely) {
      qry.clear();
      qry << "update_autoflow_at_live_update_expaborted_remotely" << QString::number(autoflowStatusID)
          << RunID_to_retrieve << OptimaName;
   ...
   if (opticsFailed) {
      qry.clear();
      qry << "update_autoflow_at_live_update_optics_types_failed" << opticsFailed_type << RunID_to_retrieve
          << OptimaName;
...
void US_XpnDataViewer::stop_optima(void) {
   ...
   if (msgBox.clickedButton() == Accept) {
      ...
      gmp_stopOptima_map
         = pw_at.getPasswd_auditTrail("GMP Stop Optima Form", "Please fill out GMP Stop Optima form:", user_stop);

      int gmp_stopOptima_map_size = gmp_stopOptima_map.keys().size();
      qDebug() << "stopOptima map: " << gmp_stopOptima_map.keys() << gmp_stopOptima_map.keys().size()
               << gmp_stopOptima_map_size << gmp_stopOptima_map.keys().isEmpty() << gmp_stopOptima_map[ "User:" ]
               << gmp_stopOptima_map[ "Comment:" ] << gmp_stopOptima_map[ "Master Password:" ];
...
void US_XpnDataViewer::skip_optima_stage(void) {
   ...
   if (msgBox.clickedButton() == Accept) {
      ...
      gmp_stopOptima_map = pw_at.getPasswd_auditTrail(
         "GMP Skip Stage Optima Form", "Please fill out GMP Skip Stage Optima form:", user_stop);

      int gmp_stopOptima_map_size = gmp_stopOptima_map.keys().size();
      qDebug() << "skipOptima map: " << gmp_stopOptima_map.keys() << gmp_stopOptima_map.keys().size()
               << gmp_stopOptima_map_size << gmp_stopOptima_map.keys().isEmpty() << gmp_stopOptima_map[ "User:" ]
               << gmp_stopOptima_map[ "Comment:" ] << gmp_stopOptima_map[ "Master Password:" ];
...
void US_XpnDataViewer::record_live_update_status(QString o_type, QString comment_text) {
   ...
   qDebug() << "IN LIVE_UPDATE, record autoflow status [STOP | SKIP]: ID,name,email,lev" << ID << fname << lname
            << email << level;
   ...
   if (o_type == "STOP") {
      ...
      if (!autoflowStatusID) {
         //create new record
         qry << "new_autoflowStatusStopOptima_record" << QString::number(autoflowID_passed) << stopOptima_Json;
         ...
         //update
         qry << "update_autoflowStatusStopOptima_record" << QString::number(autoflowStatusID)
   ...
   if (o_type == "SKIP") {
      ...
      if (!autoflowStatusID) {
         //create new record
         qry << "new_autoflowStatusSkipOptima_record" << QString::number(autoflowID_passed) << skipOptima_Json;
         ...
         //update
         qry << "update_autoflowStatusSkipOptima_record" << QString::number(autoflowStatusID)
   ...
   if (!autoflowStatusID) {
      QMessageBox::warning(
         ...
         tr("autoflowStatus (LIVE_UPDATE {STOP,SKIP}): There was a problem with creating a record in autoflowStatus "
            "table \n")
            + db->lastError());
...
void US_XpnDataViewer::check_for_sysdata(void) {
   ...
   QList<int> dhms_r;
   int running_time
      = exp_time; // + etimoff; //ALEXEY: OR elapsed_time + etimoff? OR just exp_time ???? //ALEXEY: etimoff < 0 ?
   ...
   running_time_text = QString::number(dhms_r[ 1 ]) + ":" + QString::number(dhms_r[ 2 ]) + ":"
   ...
   elapsed_time_text = QString::number(dhms_e[ 1 ]) + ":" + QString::number(dhms_e[ 2 ]) + ":"
   ...
   qDebug() << "Remaining Time:  TotalDuration.toInt() - ( exp_time ): -- " << TotalDuration.toInt() << " - "
            << (exp_time) << " = " << remaining_time;
   ...
   //ALEXEY: if times >~1 day, update #hours
   if (dhms_remain[ 0 ] > 0) {
      qDebug() << "Remaining Time [DD HH MM SS]: " << dhms_remain[ 0 ] << dhms_remain[ 1 ] << dhms_remain[ 2 ]
               << dhms_remain[ 3 ];

      dhms_remain[ 1 ] += dhms_remain[ 0 ] * 24;
   }
   remaining_time_text = QString::number(dhms_remain[ 1 ]) + ":" + QString::number(dhms_remain[ 2 ]) + ":"
   ...
   if (exp_status == 5 || exp_status == 0) {
      ...
      //Maybe add this?
      sys_thread
         ->quit(); // ALEXEY: I think this emits Qthread's finished() signal... (connected to stopping timer_sysdata)
...
void US_XpnDataViewer::check_for_data(QMap<QString, QString> &protocol_details) {
   ...
   if (
      CellChNumber_str.contains("IP") && CellChNumber_str.contains("RI") && TripleNumber_str.contains("IP")
      && TripleNumber_str.contains("RI")) {
      combinedOptics = true;
      ...
      qDebug() << "Multiple OS: IP: CellChNumber, TripleNumber: " << CellChNumber_map[ "IP" ]
               << TripleNumber_map[ "IP" ];
      qDebug() << "Multiple OS: RI: CellChNumber, TripleNumber: " << CellChNumber_map[ "RI" ]
               << TripleNumber_map[ "RI" ];
   }
...
void US_XpnDataViewer::reset_liveupdate_panel(void) {
   ...
   qDebug() << "BEFORE: " << in_reload_auto << ", " << in_reload_all_data << ", " << in_reload_data_init << ", "
...
void US_XpnDataViewer::end_processes(void) {
   ...
   qDebug() << "MIDDLE: " << in_reload_auto << ", " << in_reload_all_data << ", " << in_reload_data_init << ", "
            << in_reload_check_sysdata;
   ...
   if (!in_reload_auto && !in_reload_all_data && !in_reload_data_init && !in_reload_check_sysdata) {
      ...
      qDebug() << "AFTER: " << in_reload_auto << ", " << in_reload_all_data << ", " << in_reload_data_init << ", "
               << in_reload_check_sysdata;
      ...
      if (optima_connection_dropped) {
         ...
         QString msg_sys_text = QString(tr("Attention! UltraScan GMP is not able to communicate with the data "
                                           "acquisition server on the %1.\n\n "))
                                   .arg(xpndesc);
         QString msg_sys_text_info
            = QString(tr("The program will <b>Return</b> to \"Manage Optima Runs\" "
...
void US_XpnDataViewer::retrieve_xpn_raw_auto(void) {
   ...
   for (int ii = 0; ii < runInfo.count(); ii++) // In principle there should be just 1 record in runInfo
   {
      ...
      qDebug() << "IN retrieve_xpn_raw_auto: rDesc " << rDesc << ", lRunID: " << lRunID
               << ", RunID: " << RunID_to_retrieve;
   ...
   qDebug() << "[TIME] of xpn_data->import_data_auto: " << int(timer_xpn_data_import_data_auto.elapsed() / 1000)
            << " sec";
   ...
   int ntsrows = xpn_data->countOf(
      "scan_rows"); // <-- gets 'ntsrow' from xpn_data->import_data_auto( iRunId, scanmask, o_connected );
      ...
      if (CheckExpComplete_auto(RunID_to_retrieve, o_connection) == 0) //ALEXEY should be == 3 as per documentation
      {
         ...
         if (!timer_check_sysdata->isActive()) // Check if sys_data Timer is stopped
         {
            ...
            QMessageBox::warning(
               ...
               tr("No data were produced. The current run will be erased. The program will return to the list of "
                  "Optima runs..."));
      ...
      if (CheckExpComplete_auto(RunID_to_retrieve, o_connection_1) == 5) {
         ...
         if (!timer_check_sysdata->isActive()) // Check if sys_data Timer is stopped
         {
            ...
            QMessageBox::warning(
               ...
               tr("While run is finished, no data were produced. The current run will be erased. The program will "
                  "return to the list of Optima runs..."));
   ...
   else if ((scanmask & 1) != 0) //<-- case of combined IP+RI runs: scanmask = 5; (5&1 = 1)
   {
      ...
      DbgLv(1) << "RDa:   runType2 scanmask" << runType2 << scanmask << "[ifw]scn_rows"
               << xpn_data->countOf("iscn_rows") << xpn_data->countOf("fscn_rows") << xpn_data->countOf("wscn_rows");
...
DbgLv(1) << "RDa:   runType2 scanmask" << runType2 << scanmask << "[ifw]scn_rows"
 ...
 << xpn_data->countOf( "wscn_rows" );
   ...
   *******************************************************************************************************/
   ...
   connect(cb_optsys, SIGNAL(currentIndexChanged(int)), this, SLOT(changeOptics_auto()));
...
// #endif
   ...
   DbgLv(1) << "RDa: nwl wvlo wvhi" << nlambda << wvlo << wvhi << "ncellch" << ncellch << "nlambda" << nlambda
            << "ntriple" << ntriple << triples.count();

   qDebug() << "RDa: nwl wvlo wvhi" << nlambda << wvlo << wvhi << "ncellch" << ncellch << "nlambda" << nlambda
            << "ntriple" << ntriple << triples.count();
#endif
   ...
   *******************************************************************************************************/
   ...
   if (combinedOptics) {
      qDebug() << "CellChNumber, cellchans.count() for runType " << runType << ": "
               << CellChNumber_map[ runType ].toInt() << ", " << cellchans.count();
      qDebug() << "TripleNumber, ntriple for runType " << runType << ": " << TripleNumber_map[ runType ].toInt() << ", "
               << ntriple;
   }
   else {
      qDebug() << "CellChNumber, cellchans.count() for runType " << runType << ": " << CellChNumber.toInt() << ", "
               << cellchans.count();
   ...
   if (CheckExpComplete_auto(RunID_to_retrieve, o_connection) == 0) //ALEXEY should be == 3 as per documentation
   {
      ...
      qDebug() << "[ABORTION IN EARLY STAGE...] : combinedOptics,  opsys_auto.count(), opsys_auto -- " << combinedOptics
   ...
   if (!combinedOptics) {
      ...
      qDebug() << " In retrieve_xpn_raw_auto(): Combined optics! cellchans.count(), CellChNumber_map[ runType "
                  "].toInt(); ntriple, TripleNumber_map[ runType ].toInt(), runtype -- "
               << cellchans.count() << ", " << CellChNumber_map[ runType ].toInt() << "; " << ntriple << ", "
...
void US_XpnDataViewer::load_xpn_raw() {
   ...
   QString dbhost = xpnhost;
   int dbport = xpnport.toInt();
   DbgLv(1) << "RDr: call connect_data  dbname h p u w" << xpnname << dbhost << dbport << xpnuser << xpnpasw;
   if (xpn_data->connect_data(dbhost, dbport, xpnname, xpnuser, xpnpasw)) {
      if (dbg_level > 0)
         xpn_data->dump_tables();
...
DbgLv(1) << "RDr:  post-filter runs" << runInfo.count();
   ...
   if (scanmask == 1 || scanmask == 2 || scanmask == 4 || scanmask == 8) { // Single type of data present
      runType = (scanmask == 2) ? "FI" : runType;
      runType = (scanmask == 4) ? "IP" : runType;
      runType = (scanmask == 8) ? "WI" : runType;
      if (scanmask == 1)
      ...
      DbgLv(1) << "RDr:   runType2 scanmask" << runType2 << scanmask << "[ifw]scn_rows"
               << xpn_data->countOf("iscn_rows") << xpn_data->countOf("fscn_rows") << xpn_data->countOf("wscn_rows");
      if (
         (runType2 == "IP" && xpn_data->countOf("iscn_rows") == 0)
         || (runType2 == "FI" && xpn_data->countOf("fscn_rows") == 0)
         || (runType2 == "WI" && xpn_data->countOf("wscn_rows") == 0)) {
         msg += tr("\nScans are missing so only ") + drtype1 + tr(" scans are processed.");
         ...
         if (mbox.clickedButton() == pb_opt2) {
            ...
            optndx = 1;
            DbgLv(1) << "RDr:   runType2 scanmask" << runType2 << scanmask << "[ifw]scn_rows"
                     << xpn_data->countOf("iscn_rows") << xpn_data->countOf("fscn_rows")
                     << xpn_data->countOf("wscn_rows");
         }
      }
   }
   ...
   connect(cb_optsys, SIGNAL(currentIndexChanged(int)), this, SLOT(changeOptics()));
...
DbgLv(1) << "RDr: nwl wvlo wvhi" << nlambda << wvlo << wvhi
   ...
   DbgLv(1) << "RDr: nwl wvlo wvhi" << nlambda << wvlo << wvhi << "ncellch" << ncellch << "nlambda" << nlambda
            << "ntriple" << ntriple << triples.count();
#endif
...
void US_XpnDataViewer::load_auc_xpn() {
   ...
   for (int ii = 0; ii < ntriple; ii++) {
      ...
      QString celchn = QString::number(rdata.cell) + " / " + QString(rdata.channel);
      ...
      le_status->setText(tr("Data in for triple %1 of %2").arg(ii + 1).arg(ntriple));
      qApp->processEvents();
   }
   ...
   npoint = allData[ 0 ].pointCount();
   ntpoint = nscan * npoint;
   DbgLv(1) << "RDa: mwr ncellch nlambda nscan npoint" << ncellch << nlambda << nscan << npoint;
   DbgLv(1) << "RDa:   rvS rvE" << r_radii[ 0 ] << r_radii[ npoint - 1 ];
...
void US_XpnDataViewer::plot_all(void) {
   ...
   for (int scnx = 0; scnx < scan_knt; scnx++) { // Build Y vector for each scan
      ...
      if (scnx < 3 || (scnx + 4) > nscan)
         DbgLv(1) << "PltA:   vv[n]" << vv[ kpoint - 1 ] << "scnx" << scnx << "scan_nbr" << scan_nbr << "colx" << colx;
   ...
   DbgLv(1) << "PltA: xlo xhi" << last_xmin << last_xmax << "ylo yhi" << last_ymin << last_ymax;
}
...
void US_XpnDataViewer::changeCellCh(void) {
   ...
   if (last_xmin < 0.0) { // If first time plotting any data, detect actual data bounds
      have_rngs = false;
      compute_ranges();
      ...
      for (int trx = trxs; trx < trxe; trx++) { // Accumulate Y bounds, the amplitude bounds
   ...
   DbgLv(1) << "chgCC:  xmin xmax ymin ymax" << last_xmin << last_xmax << last_ymin << last_ymax;

   have_rngs = false;
   int lplrec = ntriple - 1;

   // Make sure cell/channel and plot record are in sync
   if (!isMWL) { // If plot record ranges are triples, point to first of cell/channel
      ...
      if (ccval != ccplr) {
         for (int jj = 0; jj < ntriple; jj++) {
            ...
            if (ccplr == ccval) { // Set plot record to first triple from cell/channel
               connect_ranges(false);
...
void US_XpnDataViewer::changeRecord(void) {
   ...
   if (isMWL) { // For MWL, limit record range to lambda range
      ...
      QString new_cc = QString(cb_pltrec->currentText()).section("/", 0, 1).replace("/", " / ");
...
void US_XpnDataViewer::changeOptics(void) {
   ...
   if (ostyp == "Interference")
      ...
      rtype = "WI";
   DbgLv(1) << "chgOpt: optrx" << optrx << "ostyp" << ostyp << "rtype" << rtype << "nopts"
            << cb_optsys->children().count();

   qDebug() << "chgOpt: optrx" << optrx << "ostyp" << ostyp << "rtype" << rtype << "runType" << runType << "nopts"
            << cb_optsys->children().count();
   ...
   xpn_data->build_rawData(
      allData); // ALEXEY <-- need to update data BEFORE with xpn_data->import_data() OR reimport_data()
   double tm2 = ( double ) sttime.msecsTo(QDateTime::currentDateTime()) / 1000.0;
   ...
   int wvhi = lambdas[ nlambda - 1 ];
   ntriple = xpn_data->data_triples(triples);
   DbgLv(1) << "chgOpt: nwl wvlo wvhi" << nlambda << wvlo << wvhi << "ncellch" << ncellch << "nlambda" << nlambda
            << "ntriple" << ntriple << triples.count();
   ...
   qDebug() << "IF timer restarts? auto_mode_bool, timer_all_data_avail->isActive(), inExport: " << auto_mode_bool
...
void US_XpnDataViewer::export_auc_auto(bool &tmstampOK) {
   ...
   if (noptsy > 1) { // Export data from Optical Systems other than currently selected one
...
void US_XpnDataViewer::export_auc() {
   ...
   if (runIDt != runID) { // Set runID to new entry given by user
      ...
      while ((pos = rx.indexIn(new_runID)) != -1) { // Loop thru any invalid characters given (not alphanum.,'_','-')
         new_runID.replace(pos, 1, "_"); // Replace 1 char at pos
      }
...
int US_XpnDataViewer::dvec_index(QVector<double> &dvec, const double dval) {
   ...
   if (indx < 0) { // If no exact match was found, look for a match within epsilon
      ...
      for (int jj = 0; jj < dvec.size(); jj++) { // Search doubles vector
         ...
         if (ddif < eps) { // If vector value matches within epsilon, break and return
            indx = jj;
            break;
         }
      }
...
void US_XpnDataViewer::reloadData() {
   ...
   if (!upd_ok) { // No change in data scans:  report inability to update
      ...
      if (!smsg.endsWith(tr(" scans)"))) { // If need be, add scan count to the status message
   ...
   double tm2 = ( double ) sttime.msecsTo(QDateTime::currentDateTime()) / 1000.0;
   DbgLv(1) << "RLd:      build-raw done: tm1 tm2" << tm1 << tm2 << "tm2i" << (tm2 - tm1);
   ...
   ct_from->setMaximum(nscan);
   ct_to->setMaximum(nscan);
   le_status->setText(tr("Update of AUC data complete -- now %1 scans.").arg(nscan));
   qApp->processEvents();
...
void US_XpnDataViewer::reloadData_auto() {
   ...
   if (!upd_ok) { // No change in data scans:  report inability to update
      ...
      if (!smsg.endsWith(tr(" total)"))) { // If need be, add scan count to the status message
         //smsg        = smsg + tr( "  (%1 scans)" ).arg( nscan );
...
void US_XpnDataViewer::timerEvent(QTimerEvent *event) {
   ...
   DbgLv(1) << "            timerEvent:   tim_id" << tim_id << "    "
            << QDateTime::currentDateTime().toString("hh:mm:ss") << "  rlt_id" << rlt_id;

   if (tim_id != rlt_id) { // if other than auto-reload event, pass on to normal handler
...
void US_XpnDataViewer::changeInterval() {
   ...
   if (!ck_autorld->isChecked()) { // If no auto reload checked, ignore this interval change for now
      rlt_id = 0;
      return;
   }
...
void US_XpnDataViewer::selectColorMap() {
   ...
   QString cmfpath = QFileDialog::getOpenFileName(this, tr("Load Color Map File"), US_Settings::etcDir(), filter, 0, 0);
...
void US_XpnDataViewer::correct_radii() {
   ...
   if (correction.size() > 0) {
      ...
      for (int jd = 0; jd < ntripl; jd++) {
         ...
         if (jd < 3 || (jd + 4) > ntripl)
            DbgLv(1) << "c_r:  ri0 ro0" << r_radii[ 0 ] << allData[ jd ].xvalues[ 0 ] << "rin ron"
                     << r_radii[ npoint - 1 ] << allData[ jd ].xvalues[ npoint - 1 ];
      }
   }
...
void US_XpnDataViewer::currentRectf(QRectF rectf) {
   ...
   QString msg = tr("%1, %2-to-%3 Radial adjustment scan...").arg(cellch).arg(radv1).arg(radv2);
   ...
   for (int jd = 0; jd < allData.count(); jd++) {
      ...
      QString outline = QString::number(wavelen) + "," + QString().asprintf("%.3f", radiusw) + "\n";
      datxto << outline;
   } // END: datasets loop
   dafile.close();
   ...
   if (nspeed > 1) { // If multi-speed, add search/output as "wavelen/speed/radpos"
      QVector<int> wvlns;
      ...
      for (int jd = 0; jd < allData.count(); jd++) { // Get information for each dataset
         US_DataIO::RawData *rdata = &allData[ jd ];
         ...
         for (int jq = 0; jq < nspeed; jq++) {
            ...
            ascdat.fill(0.0, kpoint);
            for (int js = jsf; js < jsl; js++) { // Average scans in the current speed step
               US_DataIO::Scan *dscan = &rdata->scanData[ js ];
            ...
            double radiusw = rdata->xvalues[ irpos + irx1 ];
            menrads << radiusw;
            DbgLv(1) << "  wavelen/speed/radpos:  " << wavelen << " / " << irpm << " / " << radiusw << "  irpos"
                     << irpos;
      ...
      for (int js = 0; js < nspeed; js++) { // Compose speed,meniscus,meniscus... rows
         // Start data line with speed value
         outline = QString::number(srpms[ js ]);
         int jv = js;

         for (int jq = 0; jq < nwvlen; jq++, jv += nspeed) { // append meniscus values for each column
