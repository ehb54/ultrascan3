...
void DialBox::setSpeed(double v) { d_dial->setValue(v); }
...
QString SpeedoMeter::label() const { return d_label; }

// Reloaded virtual function - needed to display units label (RPM)
void SpeedoMeter::drawScaleContents(QPainter* painter, const QPointF& center,
                                    double radius) const {
...
QWidget* WheelBox::createBox(Qt::Orientation orientation, double minv,
                             double maxv, bool log) {
...
US_XpnDataViewer::US_XpnDataViewer(QString auto_mode) : US_Widgets() {
  ...
  connect(ct_to, SIGNAL(valueChanged(double)), this, SLOT(exclude_to(double)));
  ...
  connect(pb_skip_stage, SIGNAL(clicked()), this, SLOT(skip_optima_stage()));
  ...
  vacuum_box =
      new WheelBox(Qt::Vertical, 0, 1500, vacuum_label, true);  // Blue/Red box
  ...
  // plot RPM
  plot_rpm =
      new US_Plot(data_plot_rpm, "", tr("Elapsed Time (minutes)"), tr("RPM"));
  ...
  // "experimentName" ] = QString("some_name"); protocol_details[ "CellChNumber"
  // ] = QString("IP:3,RI:6"); protocol_details[ "TripleNumber" ] =
  // QString("IP:3,RI:42"); protocol_details[ "OptimaName" ] = QString("Optima
  // 1"); protocol_details[ "duration" ]   = QString("43200");
...
void US_XpnDataViewer::changeOptima(int ndx) {
  ...
  for (int ii = 0; ii < instruments.size(); ii++) {
    ...
    if (name == descr) currentInstrument = instruments[ii];
  }
  ...
  if (!combined_check) {
    ...
    QString msg_sys_text =
        QString(
            ...
            "acquisition server on the %1. Please check the following: ")
            .arg(xpndesc);
    QString msg_sys_text_info =
        QString(
            ...
            "Optima Data Viewer is suspended until this condition is resolved. "
            "\n\nYou may check other Optima machines for proper conneciton.")
            .arg(xpndesc);
...
void US_XpnDataViewer::test_optima_connection() {
  ...
  bool o_connected = xpn_data1->connect_data(xpnhost, xpnport.toInt(), xpnname,
                                             xpnuser, xpnpasw);
  xpn_data1->close();
  delete xpn_data1;
...
void US_XpnDataViewer::selectOptimaByName_auto(QString OptimaName) {
  for (int ii = 0; ii < instruments.size(); ii++) {
    QString name = instruments[ii]["name"].trimmed();
    if (name == OptimaName) currentInstrument = instruments[ii];
  }
...
void US_XpnDataViewer::enableControls(void) {
  ...
  if (allData.size() == 0) {  // If no data yet, just reset
    reset();
    return;
  }
  ...
  for (int jj = 0; jj < nlambda; jj++) sllmbs << QString::number(lambdas[jj]);
...
bool US_XpnDataViewer::load_xpn_raw_auto() {
  ...
  if (xpn_data->connect_data(dbhost, dbport, xpnname, xpnuser, xpnpasw)) {
    ...
    // Implement: query ExperiementRun and based on ExpID build array of RunIDs,
    // find the bigger (the latest) and call it RunID_to_retrieve
    ...
    // 	  QString lRunID_t      = QString( rDesc_t ).mid( 1 ).section( delim_t,
    // 0, 0 );

    // 	  qDebug() << "FIRST: runInfo: delim, rDesc, lRunID: " << delim_t << ",
    // " << rDesc_t << ", " << lRunID_t;
    // 	}
  } else {
    runInfo.clear();
  ...
  if (RunID_to_retrieve.toInt() == 0) {
    ...
    // 				tr( "Status" ),
    // 				tr( "Run was submitted to the Optima, but not
    // launched yet. \n" 				    "Awaiting for data to emerge... \n" ) ); OR Message on
    // connection to Optima: BUT it should be connected here as experiment has
    // just been submitted...
    status_ok = false;
  } else {
    status_ok = true;
    ...
    // ALEXEY: make sure ExpID is coupled to the RunID which is already in the
    // autoflow DB
    ...
    if (!runID_passed.isEmpty()) {
      if (runID_passed.toInt() != RunID_to_retrieve.toInt()) {
        ...
        QMessageBox::warning(
            ...
            tr("It appears the scheduled run can no longer be accessed, and "
               "was probably terminated prematurely. The GMP record will be "
               "deleted. Please resubmit the protocol from Ultrascan GMP."));
    ...
    // ALEXEY: need to update 'autoflow' table with the unique RunID_to_retrieve
    // && Start Run Time fields !!! Conditional:  Do it ONLY once !!!
    ...
    xpn_data->scan_runs_auto(
        runInfo, RunID_to_retrieve);  // ALEXEY initial query (for us_comproject
                                      // needs to be based on ExpId )
    ...
    // ALEXEY: Start another timer - SysData (RPM, Temp.) - should be run in a
    // separate thread!!! Plus to check_for_sys_Data: elapsed time counter!!!
    temp_data.clear();
    rpm_data.clear();
    time_data.clear();
    ...
    // ); int exp_status = CheckExpComplete_auto( RunID_to_retrieve  ); ALEXEY:
    // here, if not sys_data_linked && if run NOT finished || NOT aborted
    // (exp_status != 0 || exp_status != 5),pop a dialogue suggesting to fix the
    // error, return to the main screen
    //                                     && if run IS  finished || IS aborted,
    //                                     proceed to the IMPORT stage.
    ...
    // check_for_sysdata( )  ), Qt::QueuedConnection ) ; //Qt::DirectConnection
    // );
    connect(timer_check_sysdata, SIGNAL(timeout()), this,
            SLOT(check_for_sysdata()));  //, Qt::QueuedConnection );
    // QThread's started() SIGNAL: before the run()/exec() function is called!!!
    // Is this a potential issue, timer is started from a thread???
...
void US_XpnDataViewer::updateautoflow_record_atLiveUpdate(void) {
  ...
  if (!status_liveupdate_unique) {
    QMessageBox::information(
        ...
        tr("The program advanced or is advancing to the next stage!\n\n"
           ...
           "used different program session to proceed to the IMPORT or later "
           "stage. \n\n"
  ...
  if (experimentAborted_remotely) {
    ...
    qry << "update_autoflow_at_live_update_expaborted_remotely"
        << QString::number(autoflowStatusID) << RunID_to_retrieve << OptimaName;
...
void US_XpnDataViewer::stop_optima(void) {
  ...
  msgBox.setInformativeText(
      "<font color='red'><b>NOTE:</b> if stopped, ongoing experiment will be "
      "abortded! </font>");
  ...
  if (msgBox.clickedButton() == Accept) {
    ...
    qDebug() << "stopOptima map: " << gmp_stopOptima_map.keys()
             << gmp_stopOptima_map.keys().size() << gmp_stopOptima_map_size
             << gmp_stopOptima_map.keys().isEmpty()
             << gmp_stopOptima_map["User:"] << gmp_stopOptima_map["Comment:"]
    ...
    // if ( combinedOptics )
    // 	{
    // 	  if ( opsys_auto.count() < 2 ) //combo-run && just 1 ootics collected
    // (so far)
    // 	    {
    // 	      QString optics_failed    = (runType == "RI" ) ? "Interference" :
    // "Absorbance"; 	      QString optics_processed = (runType == "RI" ) ?
    ...
    /* We can (or even should) do it here - NOT at the time of switching to 3.
     * IMPORT,    */
...
void US_XpnDataViewer::record_live_update_status(QString o_type,
  ...
  // == autoflowID_passed; that is a scenario for BOTH operations, SKIP && STOP
  ...
  if (!autoflowStatusID) {
    QMessageBox::warning(
        ...
        tr("autoflowStatus (LIVE_UPDATE {STOP,SKIP}): There was a problem with "
...
void US_XpnDataViewer::check_for_sysdata(void) {
  ...
  //   exp_time       = xpn_data->countOf_sysdata( "exp_time"  ).toInt(); //time
  //   form the start
  ...
  // Total Scans
  QString scans_msg =
      QString::number(runningscans) + " (of " + QString::number(totscans) + ")";
  ...
  if (running_time < 0) running_time = 0;
  ...
  // Elapsed Time
  qDebug() << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Elapsed Time Offset as read form "
              "autoflow table DB:"
           << ElapsedTimeOffset;
  ...
  if (exp_status == 5 || exp_status == 0) {
    ...
    if (sys_thread->isFinished()) qDebug() << "QThread STOPPED !!! ";

    if (!timer_check_sysdata->isActive())
      qDebug()
          << "QTimer timer_check_sysdata STOPPED by quitting the QThread !!! ";
    ...
    if (exp_status == 0 && o_connection) experimentAborted = true;
             ...
             ->isActive())  // Check if reload_data Timer is stopped
    {
      if (!timer_data_reload->isActive()) {
        ...
        if (!tmstampOK) {
          ...
          QMessageBox::critical(
              ...
              tr("This run did not start at zero RPM because the w2t value "
                 ...
                 "experiment "
                 "cannot be reliably analyzed. It fails GMP requirements and "
                 "will "
                 "therefore be aborted. The run will be deleted.\n\n"
                 "The data can be retrieved with the \"View Raw Optima Data\" "
                 "function "
        ...
        // QString message_done     = tr( "Experiment was completed. Optima data
        // saved..." ); QMessageBox::information( this, mtitle_complete,
        // message_done );

        updateautoflow_record_atLiveUpdate();
        // emit experiment_complete_auto( currentDir, ProtocolName,
        // invID_passed, correctRadii  );  // Updtade later: what should be
        // passed with signal ??
...
void US_XpnDataViewer::check_for_data(
    QMap<QString, QString>& protocol_details) {
  ...
  // TripleNumber = protocol_details[ "TripleNumber" ];            // otherwise:
  // if one OS, singke numbers, as currently
  ...
  msg_data_avail->setText(
      tr("Run named <b>%1</b> was submitted to: <br>"
         ...
         "<br>You may return to managing Optima runs now and reattach later by "
         "reopening and "
...
void US_XpnDataViewer::reset_liveupdate_panel(void) {
  ...
  if (!timer_all_data_avail->isActive())
    qDebug() << "QTimer timer_all_data_avail STOPPED by clickig Manage Optima "
                "runs !!! ";
...
void US_XpnDataViewer::end_processes(void) {
  ...
  if (!in_reload_auto && !in_reload_all_data && !in_reload_data_init &&
    ...
    if (optima_connection_dropped) {
      ...
      QString msg_sys_text =
          QString(tr("Attention! UltraScan GMP is not able to communicate with "
                     "the data acquisition server on the %1.\n\n "))
              .arg(xpndesc);
      QString msg_sys_text_info =
          QString(
              tr("The program will <b>Return</b> to \"Manage Optima Runs\" "
                 "where you can re-attach to this run later "
                 "by clicking \"Select Optima Run to follow\" once the network "
                 "issue is resolved. UltraScan will then resume data "
                 "acquisition.\n\n"
                 "NOTE: If the network connection cannot be re-established to "
                 "the ongoing run, "
                 "you can delete this run from the Run Manager (\"Delete "
                 "Record\" button). "
                 "The data will still be collected on the %1, "
                 "but will need to be imported and processed manually at the "
                 "end of the experiment."))
              .arg(xpndesc);

      QPushButton* Accept_sys =
          msgBox_sys_data.addButton(tr("Return"), QMessageBox::YesRole);
      msgBox_sys_data.setText(msg_sys_text);
      msgBox_sys_data.setInformativeText(msg_sys_text_info);
      msgBox_sys_data.exec();
...
void US_XpnDataViewer::retrieve_xpn_raw_auto(void) {
  ...
  // 	   timer_all_data_avail->stop();
  // 	   disconnect(timer_all_data_avail, SIGNAL(timeout()), 0, 0);
  // //Disconnect timer from anything 	   qDebug() << "in [retrieve_xpn_raw_auto()]:
  // stop timer_all_data_avail"; 	   qDebug() << "in [retrieve_xpn_raw_auto()]:
  // statusExp == 0 && NO Coneection to Optima!"; 	   in_reload_all_data  = false;
  ...
  xpn_data->import_data_auto(iRunId, scanmask,
                             o_connected);  // ALEXEY <-- actual data retreiving

  qDebug() << "[TIME] of xpn_data->import_data_auto: "
           << int(timer_xpn_data_import_data_auto.elapsed() / 1000) << " sec";
  ...
  if (!o_connected) {
    ...
    qDebug() << "in [retrieve_xpn_raw_auto()]: statusExp == 0 && NO Coneection "
                "to Optima!";
    in_reload_all_data = false;
  ...
  if (ntsrows < 1) {
    le_status->setText(tr("Run %1 has no associated data...").arg(fRunId));
        ...
        0)  // ALEXEY should be == 3 as per documentation
    {
               ...
               ->isActive())  // Check if sys_data Timer is stopped
      {
        ...
        QMessageBox::warning(
            ...
            tr("No data were produced. The current run will be erased. The "
               "program will return to the list of Optima runs..."));
    ...
    if (CheckExpComplete_auto(RunID_to_retrieve, o_connection_1) == 5) {
      ...
      {
        // message on aborted run with no data
        QMessageBox::warning(this, tr("Run Finished with No Data"),
                             tr("While run is finished, no data were produced. "
                                "The current run will be erased. The program "
                                "will return to the list of Optima runs..."));
           ...
           0)  //<-- case of combined IP+RI runs: scanmask = 5; (5&1 = 1)
  {
    ...
    QMessageBox::AcceptRole ); QPushButton* pb_opt2 = mbox.addButton( drtype2,
    QMessageBox::RejectRole ); mbox.setEscapeButton ( pb_opt2 );
  ...
  *********************************************************************
  ...
  cb_optsys"; connect( cb_optsys,    SIGNAL( currentIndexChanged( int ) ), this,
  ...
  *******************************************************************************************************/
  ...
  if (opsys_auto[optndx_auto] == "Absorbance") runType = "RI";
  if (opsys_auto[optndx_auto] == "Interference") runType = "IP";
  ...
  DbgLv(1)
      << "RDa: 3. Crashes HERE!!!! (after  xpn_data->build_rawData( allData ))";
  ...
  *******************************************************************************************************/
  ...
  if (!combinedOptics) {
    if (cellchans.count() == CellChNumber.toInt() &&
        ntriple == TripleNumber
                       .toInt())  // <--- Change to the values from the protocol
    {
      // stop timer
      timer_all_data_avail->stop();
      disconnect(timer_all_data_avail, SIGNAL(timeout()), 0,
    ...
    if (cellchans.count() == CellChNumber_map[runType].toInt() &&
      ...
      } else  // not all optics processed & Exp Completed
      {
        ...
        if (expStat == 5 ||
            expStat == 0)  // Run Completed, or Aborted (SHOULD BE '3' as per
                           // documentaiton but never implemented...)
        {
          // stop timer
          timer_all_data_avail->stop();
          disconnect(timer_all_data_avail, SIGNAL(timeout()), 0,
                   ...
                   ->isActive())  // Check if sys_data Timer is stopped
          {
            ...
            updateautoflow_record_atLiveUpdate();  // We need to include info on
                                                   // the failed optics here!!!
...
void US_XpnDataViewer::load_xpn_raw() {
  ...
  if (!check_sysdata_connection()) return;
  /////////////////////////////////////////////
...
DbgLv(1) << "RDr:  post-filter runs" << runInfo.count();
  ...
  if (scanmask == 1 || scanmask == 2 || scanmask == 4 ||
      scanmask == 8) {  // Single type of data present
    runType = (scanmask == 2) ? "FI" : runType;
    runType = (scanmask == 4) ? "IP" : runType;
    runType = (scanmask == 8) ? "WI" : runType;
    if (scanmask == 1)
  ...
  else if ((scanmask & 1) != 0) {  // Multiple data types present
    QApplication::restoreOverrideCursor();
    QApplication::restoreOverrideCursor();
    QString runType2("IP");
    runType2 = ((scanmask & 2) != 0) ? "FI" : runType2;
    ...
    if ((runType2 == "IP" && xpn_data->countOf("iscn_rows") == 0) ||
      ...
      msg += tr("\nScans are missing so only ") + drtype1 +
             tr(" scans are processed.");
      QMessageBox::warning(this, tr("Multiple Types with Missing Scans"), msg);
    }
...
void US_XpnDataViewer::load_auc_xpn() {
  ...
  if (lddiag.exec() == QDialog::Rejected) return;
  ...
  dirdir.makeAbsolute();
  if (dir.right(1) != "/") dir += "/";  // Ensure trailing /
  xpn_fnames = dirdir.entryList(QStringList("*.auc"), QDir::Files, QDir::Name);
  xpn_fnames.sort();
  ...
  for (int ii = 0; ii < ntriple; ii++) {
    ...
    if (!cellchans.contains(celchn)) cellchans << celchn;

    if (!lambdas.contains(lambda)) lambdas << lambda;

    if (!triples.contains(triple)) triples << triple;
...
void US_XpnDataViewer::plot_titles(void) {
  ...
  if (runType == "IP") {
    ...
    title = "Wavelength Intensity Data\nRun ID: " + runID +
            "\n    Cell: " + cell + "  Channel: " + chan + "  Radius: " + wvln;
...
void US_XpnDataViewer::plot_all(void) {
  ...
  for (int ptx = 0; ptx < kpoint; ptx++, rdx++) {  // One-time build of X vector
    rr[ptx] = rdata->xvalues[rdx];
  }
  DbgLv(1) << "PltA:   rr[n]" << rr[kpoint - 1];

  //   for ( int scnx = 0; scnx < nscan; scnx++ )
  for (int scnx = 0; scnx < scan_knt; scnx++) {  // Build Y vector for each scan
  ...
  if (last_xmin < 0.0) {  // If first time, use auto scale to set plot ranges
    data_plot->setAxisAutoScale(QwtPlot::yLeft);
    data_plot->setAxisAutoScale(QwtPlot::xBottom);
  }

  else {  // After first time, use the same plot ranges as set before
...
void US_XpnDataViewer::changeCellCh(void) {
  ...
  if (last_xmin <
      0.0) {  // If first time plotting any data, detect actual data bounds
    have_rngs = false;
    compute_ranges();
    ...
    for (int trx = trxs; trx < trxe;
         trx++) {  // Accumulate Y bounds, the amplitude bounds
  ...
  else {  // After first time, detect what has been already set
    QwtScaleDiv* sdx = AXISSCALEDIV(QwtPlot::xBottom);
    QwtScaleDiv* sdy = AXISSCALEDIV(QwtPlot::yLeft);
    last_xmin = sdx->lowerBound();
    last_xmax = sdx->upperBound();
  ...
  if (!isMWL) {  // If plot record ranges are triples, point to first of
                 // cell/channel
    ...
    if (ccval != ccplr) {
      for (int jj = 0; jj < ntriple; jj++) {
        ...
        if (ccplr ==
            ccval) {  // Set plot record to first triple from cell/channel
          connect_ranges(false);
  ...
  } else {  // If plot record ranges are wavelengths, point to first wavelength
...
void US_XpnDataViewer::changeRecord(void) {
  ...
  if (isMWL) {  // For MWL, limit record range to lambda range
    lplrec = nlambda - 1;
  }

  else {  // For non-MWL, change cell/channel if appropriate
    ...
    if (new_cc != celchn) {
      ...
      QString new_wl = QString(cb_pltrec->currentText()).section("/", 2, 2);
...
void US_XpnDataViewer::changeOptics(void) {
  ...
  qDebug() << "chgOpt: optrx" << optrx << "ostyp" << ostyp << "rtype" << rtype
           << "runType" << runType << "nopts" << cb_optsys->children().count();

  // If simply re-choosing the same optics, bale out now
  if (rtype == runType) return;
...
void US_XpnDataViewer::connect_ranges(bool conn) {
  if (conn) {  // Connect the range-related controls
    connect(cb_cellchn, SIGNAL(currentIndexChanged(int)), this,
            SLOT(changeCellCh()));
    connect(cb_rstart, SIGNAL(currentIndexChanged(int)), this,
            SLOT(changeRadius()));
  ...
  else {  // Disconnect the range-related controls
    cb_cellchn->disconnect();
...
void US_XpnDataViewer::export_auc_auto(bool& tmstampOK) {
  ...
  if (noptsy > 1) {  // Export data from Optical Systems other than currently
                     // selected one
    ...
    for (int osx = 0; osx < noptsy; osx++) {
      ...
      int kfiles =
          xpn_data->export_auc_auto(allData, tmstampOK) - 2;  // Export data
      nfiles += kfiles;  // Total files written
    }

    // Restore Optical System selection to what it was before
    // cb_optsys->setCurrentIndex( currsx );   //ALEXEY <-- not needed to repeat
    // data build!!!
  }
...
xpn_data->reimport_data() for other optics!!!
         ...
         cb_optsys->setCurrentIndex( osx );
         correct_radii();                  // Chromatic aberration correction if
needed int kfiles     = xpn_data->export_auc( allData ) - 2;  // Export data
         nfiles        += kfiles;          // Total files written
      }
...
void US_XpnDataViewer::export_auc() {
  ...
  if (runIDt != runID) {           // Set runID to new entry given by user
    QRegExp rx("[^A-Za-z0-9_-]");  // Possibly modify entry
    QString new_runID = runIDt;
    int pos = 0;

    while (
        (pos = rx.indexIn(new_runID)) !=
        -1) {  // Loop thru any invalid characters given (not alphanum.,'_','-')
      new_runID.replace(pos, 1, "_");  // Replace 1 char at pos
    }

    DbgLv(1) << "ExpAuc: new_runID len" << new_runID.length();
    if (new_runID.length() > 60) {
      new_runID = QString(new_runID).left(52) + QString(new_runID).right(8);
...
void US_XpnDataViewer::exclude_scans() {
  ...
  for (int scnx = 0; scnx < nscan; scnx++) {
    ...
    if (scan_knt >= scan_from && scan_knt <= scan_to) excludes << scnx;
...
int US_XpnDataViewer::dvec_index(QVector<double>& dvec, const double dval) {
  ...
  if (indx <
      0) {  // If no exact match was found, look for a match within epsilon

    for (int jj = 0; jj < dvec.size(); jj++) {  // Search doubles vector
      double ddif = qAbs(dvec[jj] - dval) / dmag;
      // if(jj<3||(jj+4)>dvec.size())
      // DbgLv(1) << "dvix:   jj" << jj << "dvj" << dvec[jj] << "ddif" << ddif;

      if (ddif <
          eps) {  // If vector value matches within epsilon, break and return
        indx = jj;
        break;
      }
    }
...
void US_XpnDataViewer::reloadData() {
  ...
  if (!upd_ok) {  // No change in data scans:  report inability to update
    nscan = allData[trpxs].scanCount();
    DbgLv(1) << "RLd:      upd_ok" << upd_ok << "rlt_id" << rlt_id << "nscan"
             << nscan;
    if (rlt_id == 0)  // Output message only if not auto-reload
    {
      QMessageBox::warning(this, tr("Reload Data Not Possible"),
                           tr("The \"Reload Data\" action had no effect.\n"
                              "No additional data has been recorded."));
    }

    if (!smsg.endsWith(tr(
            " scans)"))) {  // If need be, add scan count to the status message
...
void US_XpnDataViewer::reloadData_auto() {
  ...
  if (!o_connected) {
    ...
    qDebug()
        << "in [reloadData_auto()]: statusExp == 0 && NO Coneection to Optima!";
    in_reload_auto = false;
    link->disconnected_itself = true;  // do we need to set it explicitly?
    reset_liveupdate_panel();  // assumes link-> disconnected_itself = true!!!
    qApp->processEvents();
  ...
  if (!upd_ok) {  // No change in data scans:  report inability to update
    ...
    if (!smsg.endsWith(tr(
            " total)"))) {  // If need be, add scan count to the status message
                            // smsg        = smsg + tr( "  (%1 scans)" ).arg(
                            // nscan );
    ...
    /*** Check Experiement Status: if completed, kill the timer, export the data
     * into AUC format, return, signal to switch panels in US_comproject ***/
    bool o_connection = true;
    ...
    if (statusExp == 5 || statusExp == 0) {
      ...
      if (statusExp == 0 && o_connection) experimentAborted = true;

      // if ( statusExp == 0 ) // If there is still connection, then exp. is
      // truly aborted!!
      //   {
      //     //check connection to Optima: if at this point statusExp=0 due to
      //     lost connection
      //     //then, stop everything && return to the Run Manager

      //     /*
      //     Link *link1 = new Link( xpndesc ); // --- THIS DID NOT WORK ?? bool
      //     status_sys_data = link1->connectToServer( xpnhost,
      ...
      //     qDebug() << "DB parms: " << xpndesc11 << xpnhost11 << xpnport11 <<
      //     xpnname11 << xpnuser11 << xpnpasw11 << xpnmsgPort11;
      ...
      // 	  timer_data_reload->stop();
      // 	  disconnect(timer_data_reload, SIGNAL(timeout()), 0, 0);
      // //Disconnect timer from anything 	  qDebug() << "in [reloadData_auto()]:
      // Stop auto-reload timer: " ; 	  qDebug() << "in [reloadData_auto()]:
      // statusExp == 0 && NO Coneection to Optima!"; 	  in_reload_auto   = false;
      // 	  link-> disconnected_itself = true; // do we need to set it
      // explicitly? 	  reset_liveupdate_panel();          //assumes link->
      // disconnected_itself = true!!! 	  qApp->processEvents();
               ...
               ->isActive())  // Check if sys_data Timer is stopped
      {
        ...
        // ALEXEY Export AUC data: devise export_auc_auto() function which would
        // return directory name with saved data - to pass to emit signal
        // below...
        bool tmstampOK = true;
        export_auc_auto(tmstampOK);
...
void US_XpnDataViewer::timerEvent(QTimerEvent* event) {
  ...
  DbgLv(1) << "            timerEvent:   tim_id" << tim_id << "    "
           << QDateTime::currentDateTime().toString("hh:mm:ss") << "  rlt_id"
           << rlt_id;

  if (tim_id !=
      rlt_id) {  // if other than auto-reload event, pass on to normal handler
...
void US_XpnDataViewer::changeReload() {
  ...
  if (ck_autorld->isChecked()) {  // Changing to checked:  start the timer
    data_plot->replot();
  ...
  else {  // Just been unchecked:  stop the timer if need be
...
void US_XpnDataViewer::changeInterval() {
  ...
  if (!ck_autorld->isChecked()) {  // If no auto reload checked, ignore this
                                   // interval change for now
    rlt_id = 0;
    return;
  }
    ...
    for (int jd = 0; jd < ntripl; jd++) {
      ...
      while ((int)lambda.at(i) != (int)allData[jd].scanData[0].wavelength) {
        ...
        if (correction.at(i) != 0.0) {
          for (int jr = 0; jr < npoint; jr++) {  // Correct each radial point
...
void US_XpnDataViewer::currentRectf(QRectF rectf) {
  ...
  if (radv2 > 7.0) return;  // Skip further processing if not reasonable zoom
  int irx1 = 0;
  int irx2 = 0;
  double kpoint = 0;
  QString cellch = cb_cellchn->currentText();
  ...
  QDir dir;
  if (!dir.exists(impath)) dir.mkpath(impath);
  ...
  for (int jd = 0; jd < allData.count(); jd++) {
    ...
    QString dchann = QString::number(rdata->cell) + QString(rdata->channel);
    if (dchann != cech) continue;

    if (irx2 == 0) {  // Get radial range indexes
    ...
    for (int js = 0; js < nscan; js++) {
      ...
      if (ssrpm != prrpm) {  // Speed changes from previous: add to speed list
        srpms << ssrpm;
        ssfscs << js;
        sslscs << js;
        ssx = nspeed;
        prrpm = ssrpm;
        nspeed++;
      } else {  // Same speed as previous scan: update last index
        sslscs[ssx] = js;
      }
  ...
  if (nspeed >
      1) {  // If multi-speed, add search/output as "wavelen/speed/radpos"
    QVector<int> wvlns;
    ...
    for (int jq = 0; jq < nspeed; jq++) {  // Add speeds to header line
      outline += "," + QString::number(srpms[jq]);
    }
    outline += "\n";
    datxto << outline;

    for (int jd = 0; jd < allData.count();
         jd++) {  // Get information for each dataset
      US_DataIO::RawData* rdata = &allData[jd];
      ...
      for (int jq = 0; jq < nspeed; jq++) {
        ...
        int nscan = jsl - jsf;
        double afact = 1.0 / (double)nscan;
        DbgLv(1) << " MSPD: jq" << jq << "speed" << irpm << "jsf,jsl,nsc" << jsf
                 << jsl << nscan;
        ascdat.fill(0.0, kpoint);
        for (int js = jsf; js < jsl;
             js++) {  // Average scans in the current speed step
          US_DataIO::Scan* dscan = &rdata->scanData[js];
        ...
        double radiusw = rdata->xvalues[irpos + irx1];
        menrads << radiusw;
        DbgLv(1) << "  wavelen/speed/radpos:  " << wavelen << " / " << irpm
                 << " / " << radiusw << "  irpos" << irpos;
    ...
    for (int jw = 0; jw < nwvlen; jw++) {  // Add wavelengths to header line
      outline += "," + QString::number(wvlns[jw]);
    }
    outline += "\n";
    datxto2 << outline;

    for (int js = 0; js < nspeed;
         js++) {  // Compose speed,meniscus,meniscus... rows
      // Start data line with speed value
      outline = QString::number(srpms[js]);
      int jv = js;

      for (int jq = 0; jq < nwvlen;
           jq++, jv += nspeed) {  // append meniscus values for each column
