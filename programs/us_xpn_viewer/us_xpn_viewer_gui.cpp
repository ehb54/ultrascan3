...
{
  qDebug() << "in stop_optima(), p_mode -- " << p_mode;
  QMessageBox msgBox;
...
{
  qDebug() << "in skip_optima_stage(), p_mode -- " << p_mode;
  QMessageBox msgBox;
...
{
  //What mode are we in?
  p_mode = protocol_details["p_mode"];
  qDebug() << "2.LIVE_UPDATE mode: " << p_mode; 
