...
DialBox::DialBox (QWidget *parent) : QWidget (parent)
{
  ...
  connect (d_dial, SIGNAL (valueChanged (double)), this,
           SLOT (setNum (double)));
...
void
DialBox::setSpeed (double v)
{
  d_dial->setValue (v);
}

void
DialBox::setNum (double v)
{
  QString text;
  // text.setNum( v, 'f', 2 );
...
// Speedometer
SpeedoMeter::SpeedoMeter (QWidget *parent) : QwtDial (parent), d_label ("RPM")
{
...
void
SpeedoMeter::setLabel (const QString &label)
{
  d_label = label;
  update ();
}
...
void
SpeedoMeter::drawScaleContents (QPainter *painter, const QPointF &center,
                                double radius) const
{
...
// WheelBox
WheelBox::WheelBox (Qt::Orientation orientation, double minv, double maxv,
                    QString label, bool log, QWidget *parent)
    : QWidget (parent)
{
  ...
  // connect( d_thermo, SIGNAL( valueChanged( double ) ), this, SLOT( setNum(
  // double ) ) );
...
QWidget *
WheelBox::createBox (Qt::Orientation orientation, double minv, double maxv,
                     bool log)
{
...
void
WheelBox::setTemp (double v, QString label)
{
...
void
WheelBox::setNum (double v, QString label)
{
...
US_XpnDataViewer::US_XpnDataViewer (QString auto_mode) : US_Widgets ()
{
  ...
  // Signals and Slots
  connect (pb_loadXpn, SIGNAL (clicked ()), this, SLOT (load_xpn_raw ()));
  connect (pb_loadAUC, SIGNAL (clicked ()), this, SLOT (load_auc_xpn ()));
  connect (pb_reset, SIGNAL (clicked ()), this, SLOT (resetAll ()));
  connect (pb_details, SIGNAL (clicked ()), this, SLOT (runDetails ()));
  connect (pb_saveauc, SIGNAL (clicked ()), this, SLOT (export_auc ()));
  connect (pb_reload, SIGNAL (clicked ()), this, SLOT (reloadData ()));
  connect (ck_autorld, SIGNAL (clicked ()), this, SLOT (changeReload ()));
  connect (cb_cellchn, SIGNAL (currentIndexChanged (int)), this,
           SLOT (changeCellCh ()));
  connect (cb_rstart, SIGNAL (currentIndexChanged (int)), this,
           SLOT (changeRadius ()));
  connect (cb_rend, SIGNAL (currentIndexChanged (int)), this,
           SLOT (changeRadius ()));
  connect (cb_pltrec, SIGNAL (currentIndexChanged (int)), this,
           SLOT (changeRecord ()));
  connect (pb_prev, SIGNAL (clicked ()), this, SLOT (prevPlot ()));
  connect (pb_next, SIGNAL (clicked ()), this, SLOT (nextPlot ()));
  connect (ct_from, SIGNAL (valueChanged (double)), this,
           SLOT (exclude_from (double)));
  connect (ct_to, SIGNAL (valueChanged (double)), this,
           SLOT (exclude_to (double)));
  connect (pb_exclude, SIGNAL (clicked ()), this, SLOT (exclude_scans ()));
  connect (pb_include, SIGNAL (clicked ()), this, SLOT (include_scans ()));
  connect (pb_plot2d, SIGNAL (clicked ()), this, SLOT (changeCellCh ()));
  connect (pb_showtmst, SIGNAL (clicked ()), this, SLOT (showTimeState ()));
  connect (pb_colmap, SIGNAL (clicked ()), this, SLOT (selectColorMap ()));
  connect (ct_rinterv, SIGNAL (valueChanged (double)), this,
           SLOT (changeInterval ()));
  connect (pb_help, SIGNAL (clicked ()), this, SLOT (help ()));
  connect (pb_close, SIGNAL (clicked ()), this, SLOT (close ()));

  // STOP && Skip stage: sys_server commands
  connect (pb_stop, SIGNAL (clicked ()), this, SLOT (stop_optima ()));
  connect (pb_skip_stage, SIGNAL (clicked ()), this,
           SLOT (skip_optima_stage ()));
  ...
  plot = new US_Plot (data_plot, tr ("Intensity Data"), tr ("Radius (in cm)"),
                      tr ("Intensity"));
  ...
  connect (plot, SIGNAL (zoomedCorners (QRectF)), this,
           SLOT (currentRectf (QRectF)));
  ...
  temperature_box
      = new WheelBox (Qt::Vertical, 0, 40, temp_label, false); // Blue/Red box
  live_params->addWidget (temperature_box, row_params, 6, 1, 3);

  vacuum_label = QString ("Vacuum:,") + QChar (0x00b5) + QString ("m");
  vacuum_box = new WheelBox (Qt::Vertical, 0, 1500, vacuum_label,
                             true); // Blue/Red box
  ...
  // //Plots for Temp.
  // plot_temp             = new US_Plot( data_plot_temp, "", "",
  // QString("Degrees, ")+QChar(0x2103));
  ...
  plot_rpm = new US_Plot (data_plot_rpm, "", tr ("Elapsed Time (minutes)"),
                          tr ("RPM"));
  QFont tfont (US_GuiSettings::fontFamily (), US_GuiSettings::fontSize (),
               QFont::Bold);
  ...
  legend->setFont (
      QFont (US_GuiSettings::fontFamily (), US_GuiSettings::fontSize () - 1));
  ...
  picker_rpm->setMousePattern (QwtEventPattern::MouseSelect1, Qt::LeftButton,
                               Qt::ControlModifier);
  ...
  connect (plot_rpm, SIGNAL (zoomedCorners (QRectF)), this,
           SLOT (currentRectf (QRectF)));
  ...
  // //1. Temporary test - C. Horne's experiment: ExpID = 408
  // (ExperimentDefinition, Postgres, Optima 1); ProtocolID = 43 (Us-lims,
  // Mysql);
  ...
  // 2. Temporary test - S. Ling's experiment: ExpID = 465
  // (ExperimentDefinition, Postgres, Optima 1); ProtocolID = 150 (Us-lims,
  // Mysql);
  ...
  // 3. Temporary test - Harmen's experiment:  ExpID = 286
  // (ExperimentDefinition, Postgres, Optima 2); ProtocolID = 286 (Us-lims,
  // Mysql);
  ...
  //  QString("HarmenS_20180417_Purity_Run_6"); protocol_details[
  //  "experimentName" ] = QString("some_name"); protocol_details[
  //  "CellChNumber" ] = QString("16"); protocol_details[ "TripleNumber" ] =
  //  QString("29"); protocol_details[ "OptimaName" ] = QString("Optima 2");
  ...
  // protocol_details[ "protocolName"] =
  // QString("alexey-1h-3itf-6abs-uv-7wvl-bd5"); protocol_details[
  // "experimentName" ] = QString("some_name"); protocol_details[
  // "CellChNumber" ] = QString("IP:3,RI:6"); protocol_details[ "TripleNumber"
  // ] = QString("IP:3,RI:42"); protocol_details[ "OptimaName" ] =
  // QString("Optima 1"); protocol_details[ "duration" ]   = QString("43200");
...
US_XpnDataViewer::US_XpnDataViewer () : US_Widgets ()
{
  ...
  // 				 "You should add entries via Preferences:Optima
  // Host Preferences\n" 				 "as soon as possible" ) );
  //   }
  // else
  ...
  for (int ii = 0; ii < instruments.count (); ii++)
    sl_optimas << instruments[ii]["ID"] + ": " + instruments[ii]["name"];
...
US_XpnDataViewer::reset (void)
{
  ...
  connect (xpn_data, SIGNAL (status_text (QString)), this,
           SLOT (status_report (QString)));
...
void
US_XpnDataViewer::reset_auto (void)
{
...
void
US_XpnDataViewer::read_optima_machines (US_DB2 *db)
{
  QStringList q ("");
  q.clear ();
...
void
US_XpnDataViewer::changeOptima (int ndx)
{
...
bool
US_XpnDataViewer::check_sysdata_connection (void)
{
  Link *link1 = new Link (xpndesc);
  // Link *link1 = new Link();
  bool status_sys_data = link1->connectToServer (xpnhost, xpnmsgPort.toInt ());

  qDebug () << "in [check_sysdata_connection]:  status_sys_data: "
            << status_sys_data;
  ...
  qDebug () << "status_sys_data & connected_itself = ? " << status_sys_data
            << " & " << link1->connected_itself << " = " << combined_check;
  ...
  if (!combined_check)
    {
      ...
      QString msg_sys_text
          = QString ("Attention! UltraScan is not able to communicate with "
                     "the data acquisition server on the %1. Please check the "
                     "following: ")
                .arg (xpndesc);
      QString msg_sys_text_info
          = QString ("1. %1 is turned on \n2. the data acquisition server on "
                     "%1 is running \n3. your license key is stored in "
                     "$HOME/ultrascan/etc/optima and is not expired \n\nUse "
                     "of the Optima Data Viewer is suspended until this "
                     "condition is resolved. \n\nYou may check other Optima "
                     "machines for proper conneciton.")
                .arg (xpndesc);

      QPushButton *Accept_sys
          = msgBox_sys_data.addButton (tr ("OK"), QMessageBox::YesRole);
...
// Slot to test Optima connection when Optima selection changed <-------------
// //New
void
US_XpnDataViewer::test_optima_connection ()
{

  qDebug () << "Optima in use: name, host, port, dbname, dbuser, dbpasw: "
            << xpndesc << " " << xpnhost << " " << xpnport << " " << xpnname
            << " " << xpnuser << " " << xpnpasw;

  QPalette orig_pal = le_optima_connected->palette ();

  if (xpnhost.isEmpty () || xpnport.isEmpty () || xpnname.isEmpty ()
      || xpnuser.isEmpty () || xpnpasw.isEmpty ())
    {
  ...
  bool o_connected = xpn_data1->connect_data (xpnhost, xpnport.toInt (),
                                              xpnname, xpnuser, xpnpasw);
  xpn_data1->close ();
  delete xpn_data1;
...
// Slot to select Optima in use by name for autoflow <-----------------------
// // New
void
US_XpnDataViewer::selectOptimaByName_auto (QString OptimaName)
{
  for (int ii = 0; ii < instruments.size (); ii++)
    {
      QString name = instruments[ii]["name"].trimmed ();
...
void
US_XpnDataViewer::resetAll (void)
{
  if (allData.size () > 0)
    {
...
void
US_XpnDataViewer::enableControls (void)
{
  const QChar chlamb (955);
  ...
  if (nlambda == 1)
    ...
    le_lrange->setText (sllmbs[0] + tr (" to ") + sllmbs[nlambda - 1]);
...
bool
US_XpnDataViewer::load_xpn_raw_auto ()
{
  ...
  if (xpn_data->connect_data (dbhost, dbport, xpnname, xpnuser, xpnpasw))
    {
      ...
      // Implement: query ExperiementRun and based on ExpID build array of
      // RunIDs, find the bigger (the latest) and call it RunID_to_retrieve
      ...
      // xpn_data->scan_runs( runInfo );                          // ALEXEY
      // initial query (for us_comproject needs to be based on ExpId )
      //  xpn_data->filter_runs( runInfo );                        // ALEXEY
      //  Optima data filtering by type [Absorbance, Interference etc.]

      // for ( int ii = 0; ii < runInfo.count(); ii++ )
      // 	{
      // 	  QString delim_t       = QString( runInfo[ 0 ] ).left( 1 );
      // 	  QString rDesc_t       = runInfo[ ii ];
      // 	  QString lRunID_t      = QString( rDesc_t ).mid( 1 ).section(
      // delim_t, 0, 0 );

      // 	  qDebug() << "FIRST: runInfo: delim, rDesc, lRunID: " <<
      // delim_t << ", " << rDesc_t << ", " << lRunID_t;
      // 	}
    }
  else
    {
  ...
  if (RunID_to_retrieve.toInt () == 0)
    {
      ...
      // 				tr( "Status" ),
      // 				tr( "Run was submitted to the Optima,
      // but not launched yet. \n" 				    "Awaiting for data to emerge... \n" ) ); OR
      // Message on connection to Optima: BUT it should be connected here as
      // experiment has just been submitted...
      status_ok = false;
    }
  else
    {
      ...
      disconnect (timer_data_init, SIGNAL (timeout ()), 0,
                  0); // Disconnect timer from anything
      msg_data_avail->accept ();
      // msg_data_avail->close();
      // ok_msg_data->click();

      // ALEXEY: make sure ExpID is coupled to the RunID which is already in
      // the autoflow DB
      qDebug () << "runID_passed: " << runID_passed << "; "
                << "RunID_to_retrieve: " << RunID_to_retrieve;
          ...
          if (runID_passed.toInt () != RunID_to_retrieve.toInt ())
            {
              ...
              QMessageBox::warning (
                  this, tr ("RunID Mismatch!"),
                  tr ("It appears the scheduled run can no longer be "
                      "accessed, and was probably terminated prematurely. The "
                      "GMP record will be deleted. Please resubmit the "
                      "protocol from Ultrascan GMP."));
      ...
      // ALEXEY: need to update 'autoflow' table with the unique
      // RunID_to_retrieve && Start Run Time fields !!! Conditional:  Do it
      // ONLY once !!!
      ...
      qDebug () << "Elapsed Time Offset as read form autoflow table DB:"
                << ElapsedTimeOffset;
      ...
      runInfo.clear ();
      xpn_data->scan_runs_auto (
          runInfo,
          RunID_to_retrieve); // ALEXEY initial query (for us_comproject needs
                              // to be based on ExpId )
      ...
      for (int ii = 0; ii < runInfo.count (); ii++)
        {
          ...
          qDebug () << "SECOND: runInfo: delim, rDesc, lRunID: " << delim_t
                    << ", " << rDesc_t << ", " << lRunID_t;
        }

      // ALEXEY: Start another timer - SysData (RPM, Temp.) - should be run in
      // a separate thread!!! Plus to check_for_sys_Data: elapsed time
      // counter!!!
      temp_data.clear ();
      rpm_data.clear ();
      time_data.clear ();
      ...
      connect (timer_all_data_avail, SIGNAL (timeout ()), this,
               SLOT (retrieve_xpn_raw_auto ()));
      timer_all_data_avail->start (40000); // 40 sec

      // Somewhere here start sys_server (instead of timer_check_sysdata - BUT
      // move to sys_thread)
      qDebug () << "CONNECTING TO SERVER: " << xpnhost << xpnmsgPort.toInt ();
      link->connectToServer (xpnhost, xpnmsgPort.toInt ());

      // bool sys_data_linked = link->connectToServer( xpnhost,
      // xpnmsgPort.toInt() ); int exp_status = CheckExpComplete_auto(
      // RunID_to_retrieve  ); ALEXEY: here, if not sys_data_linked && if run
      // NOT finished || NOT aborted (exp_status != 0 || exp_status != 5),pop a
      // dialogue suggesting to fix the error, return to the main screen
      //                                     && if run IS  finished || IS
      //                                     aborted, proceed to the IMPORT
      //                                     stage.

      timer_check_sysdata->setInterval (3000);
      timer_check_sysdata->moveToThread (sys_thread);
      // connect( timer_check_sysdata, SIGNAL(timeout()), this, SLOT(
      // check_for_sysdata( )  ), Qt::QueuedConnection ) ;
      // //Qt::DirectConnection );
      connect (timer_check_sysdata, SIGNAL (timeout ()), this,
               SLOT (check_for_sysdata ())); //, Qt::QueuedConnection );
      // QThread's started() SIGNAL: before the run()/exec() function is
      // called!!! Is this a potential issue, timer is started from a thread???
      connect (sys_thread, SIGNAL (started ()), timer_check_sysdata,
               SLOT (start ()));
      connect (sys_thread, SIGNAL (finished ()), timer_check_sysdata,
               SLOT (stop ()));
      sys_thread->start ();
      ...
      // // Check if all triple info is available
      // timer_all_data_avail = new QTimer;
      // connect(timer_all_data_avail, SIGNAL(timeout()), this, SLOT(
      // retrieve_xpn_raw_auto ( ) )); timer_all_data_avail->start(5000); // 5
      // sec
    }
...
void
US_XpnDataViewer::update_autoflow_runId_timeStarted (void)
{
  // Check DB connection
  US_Passwd pw;
  ...
  QStringList qry;
  qry << "update_autoflow_runid_starttime" << ExpID_to_use << RunID_to_retrieve
      << OptimaName;

  db->query (qry);
  // ALEXEY: Updates with runID && timeStarted only once, when runID &&
  // runStarttime IS NULL
}

// Read diffference btw started tiem and NOW() as an offset for Elapsed time
int
US_XpnDataViewer::read_timeElapsed_offset (void)
{
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd ();
  ...
  QStringList qry;
  qry << "read_autoflow_times_mod" << RunID_to_retrieve << OptimaName;
...
void
US_XpnDataViewer::delete_autoflow_record (void)
{
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd ();
...
// Delete autoflow record upon Run abortion
void
US_XpnDataViewer::updateautoflow_record_atLiveUpdate (void)
{
  /***/
  ...
  if (!status_liveupdate_unique)
    {
      QMessageBox::information (
          ...
          tr ("The program advanced or is advancing to the next stage!\n\n"
              ...
              "used different program session to proceed to the IMPORT or "
              "later stage. \n\n"
  ...
  QStringList qry;
  qry << "update_autoflow_at_live_update" << RunID_to_retrieve << currentDir
      << OptimaName;
  ...
  // ALEXEY: if there is NO chromatic Abber. data, set corrRadii in autoflow
  // record to 'NO'
  if (correctRadii == "NO")
    {
      qry.clear ();
      qry << "update_autoflow_at_live_update_radiicorr" << RunID_to_retrieve
          << OptimaName;
  ...
  // ALEXEY: if run was aborted manually from the Optima panel, set expAborted
  // to 'YES'
  if (experimentAborted)
    {
      qry.clear ();
      qry << "update_autoflow_at_live_update_expaborted" << RunID_to_retrieve
          << OptimaName;
  ...
  // If aborted remotely (From GUI, not from the panel!!), update autoflow
  // record with 'statusID':
  if (experimentAborted_remotely)
    {
      qry.clear ();
      qry << "update_autoflow_at_live_update_expaborted_remotely"
          << QString::number (autoflowStatusID) << RunID_to_retrieve
          << OptimaName;
  ...
  if (opticsFailed)
    {
      ...
      qry << "update_autoflow_at_live_update_optics_types_failed"
          << opticsFailed_type << RunID_to_retrieve << OptimaName;
...
int
US_XpnDataViewer::read_autoflow_stages_record (int autoflowID)
{
  int status = 0;
  ...
  // qDebug() << "BEFORE query ";
  QStringList qry;
  qry << "autoflow_liveupdate_status" << QString::number (autoflowID);
...
void
US_XpnDataViewer::revert_autoflow_stages_record (int autoflowID)
{
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd ();
  ...
  // qDebug() << "BEFORE query ";
  QStringList qry;
  qry << "autoflow_liveupdate_status_revert" << QString::number (autoflowID);
...
// Stop machine
void
US_XpnDataViewer::stop_optima (void)
{
  QMessageBox msgBox;
  msgBox.setText (tr ("\nYou are about to STOP Optima machine! \n\n")
                  + tr ("Do you want to proceed ?\n"));
  msgBox.setInformativeText ("<font color='red'><b>NOTE:</b> if stopped, "
                             "ongoing experiment will be abortded! </font>");
  ...
  if (msgBox.clickedButton () == Accept)
    {
      ...
      //  bool ok;
      //  QString msg = QString(tr("Put a comment describing reason for a
      //  STOP:")); QString default_text = QString(tr("Reason for STOP: "));
      //  QString comment_text = QInputDialog::getText( this,
      //  						    tr( "Reason for
      //  STOP" ), 						    msg, QLineEdit::Normal, default_text, &ok );
      ...
      gmp_stopOptima_map = pw_at.getPasswd_auditTrail (
          "GMP Stop Optima Form",
          "Please fill out GMP Stop Optima form:", user_stop);

      int gmp_stopOptima_map_size = gmp_stopOptima_map.keys ().size ();
      qDebug () << "stopOptima map: " << gmp_stopOptima_map.keys ()
                << gmp_stopOptima_map.keys ().size ()
                << gmp_stopOptima_map_size
                << gmp_stopOptima_map.keys ().isEmpty ()
                << gmp_stopOptima_map["User:"]
                << gmp_stopOptima_map["Comment:"]
                << gmp_stopOptima_map["Master Password:"];

      if (gmp_stopOptima_map_size == 0
          || gmp_stopOptima_map.keys ().isEmpty ())
        return;
      ///////////////////////////////////////
      ...
      // IF status "0", it will switch to IMPORT and will ask what to do with
      // data IF status "0" && no data collected yet - what then ? Needs
      // testing...
      // **************  if ( link->tot_scans.toInt() == 0 ) ???

      // if ( combinedOptics )
      // 	{
      // 	  if ( opsys_auto.count() < 2 ) //combo-run && just 1 ootics
      // collected (so far)
      // 	    {
      // 	      QString optics_failed    = (runType == "RI" ) ?
      // "Interference" : "Absorbance"; 	      QString optics_processed = (runType ==
      // "RI" ) ? "Absorbance" : "Interference";
      ...
      // Now, create OR update (if exists due to clicking "Skip Stage")
      // autoflowStatus record:
      /* We can (or even should) do it here - NOT at the time of switching
       * to 3. IMPORT,    */
...
// skip stage
void
US_XpnDataViewer::skip_optima_stage (void)
{
  QMessageBox msgBox;
  ...
  if (msgBox.clickedButton () == Accept)
    {
      ...
      // bool ok;
      // QString msg = QString(tr("Put a comment describing reason for a SKIP
      // stage:")); QString default_text = QString(tr("Reason for SKIP: "));
      // QString comment_text = QInputDialog::getText( this,
      // 						    tr( "Reason for
      // SKIP" ), 						    msg, QLineEdit::Normal, default_text, &ok );
      ...
      QMap<QString, QString> gmp_stopOptima_map;
      gmp_stopOptima_map = pw_at.getPasswd_auditTrail (
          "GMP Skip Stage Optima Form",
          "Please fill out GMP Skip Stage Optima form:", user_stop);

      int gmp_stopOptima_map_size = gmp_stopOptima_map.keys ().size ();
      qDebug () << "skipOptima map: " << gmp_stopOptima_map.keys ()
                << gmp_stopOptima_map.keys ().size ()
                << gmp_stopOptima_map_size
                << gmp_stopOptima_map.keys ().isEmpty ()
                << gmp_stopOptima_map["User:"]
      ...
      // Now, create OR update (if exists due to clicking "Stop Optima")
      // autoflowStatus record:
      /* We can (or even should) do it here - NOT at the time of switching
       * to 3. IMPORT,    */
...
//
void
US_XpnDataViewer::record_live_update_status (QString o_type,
                                             QString comment_text)
{
  ...
  qDebug () << "IN LIVE_UPDATE, record autoflow status [STOP | SKIP]: "
               "ID,name,email,lev"
            << ID << fname << lname << email << level;

  // first, check if there is already a record in autoflowStatus with
  // autoflowID == autoflowID_passed; that is a scenario for BOTH operations,
  // SKIP && STOP if there IS record, update it; otherwise create a new one..
  qry.clear ();
      ...
      if (!autoflowStatusID)
        {
          ...
          qry << "new_autoflowStatusStopOptima_record"
              << QString::number (autoflowID_passed) << stopOptima_Json;
      ...
      if (!autoflowStatusID)
        {
          ...
          qry << "new_autoflowStatusSkipOptima_record"
              << QString::number (autoflowID_passed) << skipOptima_Json;
  ...
  if (!autoflowStatusID)
    {
      QMessageBox::warning (
          ...
          tr ("autoflowStatus (LIVE_UPDATE {STOP,SKIP}): There was a problem "
              "with creating a record in autoflowStatus table \n")
              + db->lastError ());

      return;
    }
  qDebug ()
      << "in record_live_update_status: stopOptima_Json,skipOptima_Json -- "
      << stopOptima_Json << "\n"
      << skipOptima_Json;
}
...
void
US_XpnDataViewer::check_for_sysdata (void)
{
  qDebug () << "sys_timer IS RUNNING here: ";
  qDebug () << "sys_timer IS RUNNING here: in_reload_check_sysdata "
            << in_reload_check_sysdata;
  ...
  // while ( temperature == 0 ) // what if the temperature is actually set to
  // zero degrees?
  // {
  //   xpn_data->update_isysrec( idrun );
  //   exp_time       = xpn_data->countOf_sysdata( "exp_time"  ).toInt();
  //   //time form the start
  // //stage_number   = xpn_data->countOf_sysdata( "stage_number" ).toInt();
  // //stage number
  //   temperature    = xpn_data->countOf_sysdata( "tempera" ).toDouble();
  //   //temperature rpm            = xpn_data->countOf_sysdata( "last_rpm"
  //   ).toInt();     //revolutions per minute !
  //   //etimoff        = xpn_data->countOf_sysdata( "etim_off"  ).toInt();
  //   //experimental time offset
  // }
  ...
  // Total Scans
  QString scans_msg = QString::number (runningscans) + " (of "
                      + QString::number (totscans) + ")";
  le_totscans->setText (scans_msg);

  // Cerrent Stage
  QString stage_msg = QString::number (current_stage) + " (of "
                      + QString::number (totstage) + ")";
  ...
  QList<int> dhms_r;
  int running_time
      = exp_time; // + etimoff; //ALEXEY: OR elapsed_time + etimoff? OR just
                  // exp_time ???? //ALEXEY: etimoff < 0 ?

  qDebug () << "Experiment Time (AKA Running Time: exp_time) from the "
               "link-machine ---- "
            << exp_time;
  ...
  running_time_text = QString::number (dhms_r[1]) + ":"
                      + QString::number (dhms_r[2]) + ":"
                      + QString::number (dhms_r[3]);

  qDebug () << "Experiment Time (AKA Raunning Time: exp_time) from the "
               "link-machine text ---- "
            << running_time_text;
  ...
  // Elapsed Time
  qDebug () << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Elapsed Time Offset as read "
               "form autoflow table DB:"
            << ElapsedTimeOffset;

  QList<int> dhms_e;
  int elapsed_time
      = int (elapsed_timer->elapsed () / 1000) + ElapsedTimeOffset;
  ...
  elapsed_time_text = QString::number (dhms_e[1]) + ":"
                      + QString::number (dhms_e[2]) + ":"
                      + QString::number (dhms_e[3]);
  ...
  qDebug () << "Remaining Time:  TotalDuration.toInt() - ( exp_time ): -- "
            << TotalDuration.toInt () << " - " << (exp_time) << " = "
            << remaining_time;
  ...
  if (dhms_remain[0] > 0)
    {
      qDebug () << "Remaining Time [DD HH MM SS]: " << dhms_remain[0]
                << dhms_remain[1] << dhms_remain[2] << dhms_remain[3];

      dhms_remain[1] += dhms_remain[0] * 24;
    }
  remaining_time_text = QString::number (dhms_remain[1]) + ":"
                        + QString::number (dhms_remain[2]) + ":"
                        + QString::number (dhms_remain[3]);
  ...
  time_data.push_back (
      double (elapsed_time_1 / 60.0)); // Running time in minutes
  temp_data.push_back (temperature);

  // time_data.push_back( double(elapsed_time_1/60.0) );  // Running time in
  // minutes

  // //Debugs for RPM, Temp., Time
  // qDebug() << "Sizes of arrays: RMP.size(), temperature.size() " <<
  // rpm_data.size() << ", " << temp_data.size();
  ...
  // data_plot_rpm->setAxisScale( QwtPlot::xBottom, 0.0,
  // double(elapsed_time_1/60.0) );  // <-- HERE
  data_plot_rpm->setAxisScale (QwtPlot::xBottom,
                               double (ElapsedTimeOffset / 60.0),
                               double (elapsed_time_1 / 60.0)); //
  // data_plot_rpm->setAxisScale( QwtPlot::xBottom, 0.0, double(exp_time/60.0 +
  // counter_mins ) ); // for testing only
  ...
  if (exp_status == 5 || exp_status == 0)
    {
      // timer_check_sysdata->stop();
      // ALEXEY: This timer cannot be stopped from another thread, but can be
      // dealt with signal/slot upon Qthread termination..
      //         disconnection maybe enough...
      disconnect (timer_check_sysdata, SIGNAL (timeout ()), 0,
                  0); // Disconnect timer from anything
      // Maybe add this?
      sys_thread->quit (); // ALEXEY: I think this emits Qthread's finished()
                           // signal... (connected to stopping timer_sysdata)
      qApp->processEvents (); // <-- IMPORTANT to process event loop while
                              // stopping thread!!!
      ...
      if (!timer_check_sysdata->isActive ())
        qDebug () << "QTimer timer_check_sysdata STOPPED by quitting the "
                     "QThread !!! ";
      ...
      if (finishing_live_update)
        {
          qDebug () << "Exp. completed: finishing_live_update:  "
                    << finishing_live_update;
          in_reload_check_sysdata = false;
          return;
        }
      ...
      if (!timer_all_data_avail
               ->isActive ()) // Check if reload_data Timer is stopped
        {
          if (!timer_data_reload->isActive ())
            {
              qDebug ()
                  << "TRY PROCEED INTO == 5/0 from check_for_sys_data()....";
              ...
              if (!tmstampOK)
                {
                  ...
                  QMessageBox::critical (
                      this, tr ("GMP Compliance Problem:"),
                      tr ("This run did not start at zero RPM because the w2t "
                          "value "
                          "was non-zero at the beginning of the experiment. "
                          "\n\n"
                          "Therefore, the boundary conditions are unknown and "
                          "the experiment "
                          "cannot be reliably analyzed. It fails GMP "
                          "requirements and will "
                          "therefore be aborted. The run will be deleted.\n\n"
                          "The data can be retrieved with the \"View Raw "
                          "Optima Data\" function "
                          "in the UltraScan Utilities."));

                  delete_autoflow_record (); /** Do we want to delete GMP run
                                                NOW? **/
                  reset_auto ();
                  emit aborted_back_to_initAutoflow ();
              ...
              // QString message_done     = tr( "Experiment was completed.
              // Optima data saved..." ); QMessageBox::information( this,
              // mtitle_complete, message_done );

              updateautoflow_record_atLiveUpdate ();
              // emit experiment_complete_auto( currentDir, ProtocolName,
              // invID_passed, correctRadii  );  // Updtade later: what should
              // be passed with signal ??
...
void
US_XpnDataViewer::timeToList (int &sectime, QList<int> &dhms)
{
...
void
US_XpnDataViewer::check_for_data (QMap<QString, QString> &protocol_details)
{
  ...
  xpn_data->setEtimOffZero (); // ALEXEY: intialize etimoff to zero for the
                               // first time
  ...
  // CellChNumber = protocol_details[ "CellChNumber" ];            //ALEXEY: in
  // autoflow, these should be QStringLists - #chanels && #triples for each OS
  // TripleNumber = protocol_details[ "TripleNumber" ];            //
  // otherwise: if one OS, singke numbers, as currently
  ...
  if (CellChNumber_str.contains ("IP") && CellChNumber_str.contains ("RI")
      && TripleNumber_str.contains ("IP") && TripleNumber_str.contains ("RI"))
    {
      ...
      qDebug () << "Multiple OS: IP: CellChNumber, TripleNumber: "
                << CellChNumber_map["IP"] << TripleNumber_map["IP"];
      qDebug () << "Multiple OS: RI: CellChNumber, TripleNumber: "
                << CellChNumber_map["RI"] << TripleNumber_map["RI"];
    }
  else // Regular single Optics run
    {
      ...
      qDebug () << "Single OS: CellChNumber, TripleNumber: " << CellChNumber
                << TripleNumber;
    }
  ...
  qDebug () << "RUNID_PASSED !!! AutoflowID " << runID_passed << " !!! "
  ...
  // check connection to Optima server: if no -- reset all & go back to run
  // manager
  ...
  connect (timer_data_init, SIGNAL (timeout ()), this,
           SLOT (load_xpn_raw_auto ()));
  ...
  msg_data_avail->setWindowFlags (Qt::CustomizeWindowHint
                                  | Qt::WindowTitleHint);
  // QPushButton *okButton = msg_data_avail->addButton(tr("Ok"),
  // QMessageBox::AcceptRole); okButton->hide();

  QPushButton *Close = msg_data_avail->addButton (
      tr ("Return to Managing Optima Runs?"), QMessageBox::RejectRole);
  msg_data_avail->setText (
      tr ("Run named <b>%1</b> was submitted to: <br>"
          "<br><b>%2</b> <br>"
          "<br>Please start this method scan from the instrument panel. <br>"
          "<br>You may return to managing Optima runs now and reattach later "
          "by reopening and "
...
void
US_XpnDataViewer::reset_liveupdate_panel_public (void)
{
...
// Reset LIVE UPDATE panel && stop timers && quit threads
void
US_XpnDataViewer::reset_liveupdate_panel (void)
{
  // Quit sys_thread, emit finished() to stop timer_check_sysdata from withing
  // thread
  if (!sys_thread->isFinished ())
    {
      sys_thread->quit ();
  ...
  if (!timer_check_sysdata->isActive ())
    qDebug () << "QTimer timer_check_sysdata STOPPED by clickig Manage Optima "
                 "runs !!! ";

  if (!timer_data_init->isActive ())
    qDebug ()
        << "QTimer timer_data_init STOPPED by clickig Manage Optima runs !!! ";

  if (!timer_all_data_avail->isActive ())
    qDebug () << "QTimer timer_all_data_avail STOPPED by clickig Manage "
                 "Optima runs !!! ";

  if (!timer_data_reload->isActive ())
    qDebug () << "QTimer timer_data_reload STOPPED by clickig Manage Optima "
                 "runs !!! ";
  /*************************************/

  qDebug () << "BEFORE: " << in_reload_auto << ", " << in_reload_all_data
            << ", " << in_reload_data_init << ", " << in_reload_check_sysdata;

  // ALEXEY: should wait for execution of all below variables to be false (end
  // of all timer functions) BEFORE reset_all();
  //  Introduce QTimer which checks for all abpve vartibales to be false (check
  //  all related functions to always reset them to false on completion) Put
  //  reset_auto() into the timer && and stop/disconnect timer from within
  //  connected SLOT.
  // timer_end_processes = new QTimer;
  connect (timer_end_processes, SIGNAL (timeout ()), this,
           SLOT (end_processes ()));
...
void
US_XpnDataViewer::end_processes (void)
{
  qDebug () << "In the END process: in_reload_end_processes = "
            << in_reload_end_processes;
  ...
  qDebug () << "MIDDLE: " << in_reload_auto << ", " << in_reload_all_data
            << ", " << in_reload_data_init << ", " << in_reload_check_sysdata;

  if (!in_reload_auto && !in_reload_all_data && !in_reload_data_init
      && !in_reload_check_sysdata)
    {

      timer_end_processes->stop ();
      disconnect (timer_end_processes, SIGNAL (timeout ()), 0,
                  0); // Disconnect timer from anything

      // ALEXEY: may not be needed
      qDebug () << "LIVE UPDATE panel has been reset!";
      qDebug () << "AFTER: " << in_reload_auto << ", " << in_reload_all_data
                << ", " << in_reload_data_init << ", "
                << in_reload_check_sysdata;

      bool optima_connection_dropped = link->disconnected_itself;
      qDebug () << "[in end_processes()]: optima_connection_dropped -- "
                << optima_connection_dropped;
      ...
      if (optima_connection_dropped)
        {
          ...
          QMessageBox msgBox_sys_data;
          msgBox_sys_data.setIcon (QMessageBox::Critical);
          msgBox_sys_data.setWindowFlags (Qt::CustomizeWindowHint
                                          | Qt::WindowTitleHint);
          msgBox_sys_data.setWindowTitle (
              tr ("Optima System Data Server Connection Problem!"));
          ...
          QString msg_sys_text
              = QString (
                    tr ("Attention! UltraScan GMP is not able to communicate "
                        "with the data acquisition server on the %1.\n\n "))
                    .arg (xpndesc);
          QString msg_sys_text_info
              = QString (tr ("The program will <b>Return</b> to \"Manage "
                             "Optima Runs\" "
                             "where you can re-attach to this run later "
                             "by clicking \"Select Optima Run to follow\" "
                             "once the network "
                             "issue is resolved. UltraScan will then resume "
                             "data acquisition.\n\n"
                             "NOTE: If the network connection cannot be "
                             "re-established to the ongoing run, "
                             "you can delete this run from the Run Manager "
                             "(\"Delete Record\" button). "
                             "The data will still be collected on the %1, "
                             "but will need to be imported and processed "
                             "manually at the end of the experiment."))
                    .arg (xpndesc);

          QPushButton *Accept_sys = msgBox_sys_data.addButton (
              tr ("Return"), QMessageBox::YesRole);
          msgBox_sys_data.setText (msg_sys_text);
          msgBox_sys_data.setInformativeText (msg_sys_text_info);
          msgBox_sys_data.exec ();
...
// to end all existing update processes
void
US_XpnDataViewer::end_process_all_data_avail (void)
{
  qDebug () << "In the END process: in_reload_end_processes = "
            << in_reload_end_process_all_data_avail;
  ...
  if (!in_reload_all_data && !in_reload_auto)
    {
      ...
      disconnect (timer_end_process_all_data_avail, SIGNAL (timeout ()), 0,
                  0); // Disconnect timer from anything
...
void
US_XpnDataViewer::retrieve_xpn_raw_auto (void)
{
  ...
  //   {
  //     //check connection to Optima: if at this point statusExp=0 due to lost
  //     connection
  //     //then, stop everything && return to the Run Manager

  //     /*
  //     Link *link1 = new Link( xpndesc ); // THIS DID NOT WORK... bool
  //     status_sys_data = link1->connectToServer( xpnhost, xpnmsgPort.toInt()
  //     ); qDebug() << "in [retrieve_xpn_raw_auto()]: statusExp == 0;
  //     status_sys_data: " << status_sys_data;

  //     bool combined_check = status_sys_data & link1->connected_itself;
  //     link1->disconnectFromServer();
  //     qDebug() << "in [retrieve_xpn_raw_auto()]: status_sys_data &
  //     connected_itself = ? "
  // 		<< status_sys_data << " & " << link1->connected_itself << " = "
  // << combined_check;
  ...
  //     bool o_connected           = xpn_data11->connect_data( xpnhost,
  //     xpnport.toInt(), xpnname, xpnuser,  xpnpasw ); xpn_data11->close();
  //     delete xpn_data11;
  ...
  // 	   timer_all_data_avail->stop();
  // 	   disconnect(timer_all_data_avail, SIGNAL(timeout()), 0, 0);
  // //Disconnect timer from anything 	   qDebug() << "in
  // [retrieve_xpn_raw_auto()]: stop timer_all_data_avail"; 	   qDebug() << "in
  // [retrieve_xpn_raw_auto()]: statusExp == 0 && NO Coneection to Optima!";
  // 	   in_reload_all_data  = false;
  // 	   link-> disconnected_itself = true; // do we need to set it
  // explicitly? 	   reset_liveupdate_panel();          //assumes link->
  // disconnected_itself = true!!! 	   qApp->processEvents();
  ...
  QString drDesc = "";
  QString delim = (runInfo.count () > 0) ? QString (runInfo[0]).left (1) : "";
  /************* For Automated *****/
  // Search description list and choose the item with matching runID
  for (int ii = 0; ii < runInfo.count ();
       ii++) // In principle there should be just 1 record in runInfo
    {
      QString rDesc = runInfo[ii];
      QString lRunID = QString (rDesc).mid (1).section (delim, 0, 0);

      qDebug () << "IN retrieve_xpn_raw_auto: rDesc " << rDesc
                << ", lRunID: " << lRunID << ", RunID: " << RunID_to_retrieve;
  ...
  //    if ( lddiag->exec() == QDialog::Rejected )                    //ALEXEY
  //    need drDesc but do NOT need dialog
  //    {
  ...
  DbgLv (1) << "RDa:     iRId" << iRunId << "sMsks scnmask" << sMasks
            << scanmask;

  // ALEXEY: maybe put in_reload_check_sysdata = true; and then false (after
  // xpn_data->import_data( iRunId, scanmask ); ) in_reload_check_sysdata =
  // true; //ALEXEY

  // xpn_data->import_data( iRunId, scanmask ); // ALEXEY <-- actual data
  // retreiving
  ...
  xpn_data->import_data_auto (
      iRunId, scanmask, o_connected); // ALEXEY <-- actual data retreiving

  qDebug () << "[TIME] of xpn_data->import_data_auto: "
            << int (timer_xpn_data_import_data_auto.elapsed () / 1000)
            << " sec";
  ...
  if (!o_connected)
    {
      timer_all_data_avail->stop ();
      disconnect (timer_all_data_avail, SIGNAL (timeout ()), 0,
                  0); // Disconnect timer from anything
      qDebug () << "in [retrieve_xpn_raw_auto()]: stop timer_all_data_avail";
      qDebug () << "in [retrieve_xpn_raw_auto()]: statusExp == 0 && NO "
                   "Coneection to Optima!";
      in_reload_all_data = false;
      link->disconnected_itself = true; // do we need to set it explicitly?
      reset_liveupdate_panel ();        // assumes link-> disconnected_itself =
                                 // true!!!
      qApp->processEvents ();
  ...
  int ntsrows = xpn_data->countOf (
      "scan_rows"); // <-- gets 'ntsrow' from xpn_data->import_data_auto(
                    // iRunId, scanmask, o_connected ); plus scan_xpndata(
                    // const int runId, const QChar scantype )
  ...
  if (ntsrows < 1)
    {
      le_status->setText (
          tr ("Run %1 has no associated data...").arg (fRunId));

      in_reload_all_data = false;

      // ALEXEY: rare case when no data but exp. aborted !!!!
      bool o_connection = true;
      if (CheckExpComplete_auto (RunID_to_retrieve, o_connection)
          == 0) // ALEXEY should be == 3 as per documentation
        {
          if (finishing_live_update)
            {
              timer_all_data_avail->stop ();
          ...
          if (!timer_check_sysdata
                   ->isActive ()) // Check if sys_data Timer is stopped
            {
              // message on aborted run with no data
              QMessageBox::warning (
                  this, tr ("Run Was Aborted"),
                  tr ("No data were produced. The current run will be erased. "
                      "The program will return to the list of Optima "
                      "runs..."));
      ...
      if (CheckExpComplete_auto (RunID_to_retrieve, o_connection_1) == 5)
        {
          ...
          if (!timer_check_sysdata
                   ->isActive ()) // Check if sys_data Timer is stopped
            {
              // message on aborted run with no data
              QMessageBox::warning (
                  this, tr ("Run Finished with No Data"),
                  tr ("While run is finished, no data were produced. The "
                      "current run will be erased. The program will return to "
                      "the list of Optima runs..."));
      ...
      qDebug ()
          << "ntsrows < 1, possibly as a result of no Posgresql conneciton!";
      return;
    }
  ...
  if (scanmask == 1 || scanmask == 2 || scanmask == 4 || scanmask == 8)
    {
      ...
      //    opsys << "Wavelength";
           ...
           != 0) //<-- case of combined IP+RI runs: scanmask = 5; (5&1 = 1)
    {
      ...
      QString msg = tr ("Multiple scan data types are present:\n") + "'"
                    + drtype1 + "'\n or \n" + "'" + drtype2 + "' .\n";
      DbgLv (1) << "RDa:   runType2 scanmask" << runType2 << scanmask
                << "[ifw]scn_rows" << xpn_data->countOf ("iscn_rows")
      ...
      drtype1
            ...
            else
            {
               ...
               mbox.setText( msg );
               QPushButton* pb_opt1 = mbox.addButton( drtype1,
      QMessageBox::AcceptRole ); QPushButton* pb_opt2 = mbox.addButton(
      drtype2, QMessageBox::RejectRole ); mbox.setEscapeButton ( pb_opt2 );
  ...
  !in_reload_all_data_set_gui )
    {
      ...
      cb_optsys->addItems( opsys_auto );                                  //
  ALEXEY fill out Optics listbox DbgLv(1) << "RDa: 1ab. Crashes HERE!!!! -
  BEFORE Setting index to cb_optsys: optndx_auto = " << optndx_auto;
      cb_optsys->setCurrentIndex( optndx_auto );
      DbgLv(1) << "RDa: 1ac. Crashes HERE!!!! - AFTER Setting index to
  cb_optsys"; connect( cb_optsys,    SIGNAL( currentIndexChanged( int ) ),
  ...
  *******************************************************************************************************/
  ...
  DbgLv (1) << "RDa: 1ab. Crashes HERE!!!! - BEFORE Setting index to "
               "cb_optsys: optndx_auto = "
            << optndx_auto;
  cb_optsys->setCurrentIndex (optndx_auto);
  ...
  connect (cb_optsys, SIGNAL (currentIndexChanged (int)), this,
           SLOT (changeOptics_auto ()));
  ...
  DbgLv (1) << "RDa: 2. Crashes HERE!!!! (after xpn_data->set_run_values( "
               "runID, runType ) )";
  ...
  DbgLv (1) << "RDa: 3. Crashes HERE!!!! (after  xpn_data->build_rawData( "
               "allData ))";
  ...
  !in_reload_all_data_set_gui )
    {
      ...
      cb_cellchn->disconnect();
      cb_cellchn->clear();
      cb_cellchn->addItems( cellchans );                             // ALEXEY
  fill out Cells/Channels listbox connect( cb_cellchn,   SIGNAL(
  currentIndexChanged( int ) ), this,         SLOT  ( changeCellCh( ) ) );
    }
...
// #endif
  ...
  DbgLv (1) << "RDa: nwl wvlo wvhi" << nlambda << wvlo << wvhi << "ncellch"
            << ncellch << "nlambda" << nlambda << "ntriple" << ntriple
            << triples.count ();

  qDebug () << "RDa: nwl wvlo wvhi" << nlambda << wvlo << wvhi << "ncellch"
            << ncellch << "nlambda" << nlambda << "ntriple" << ntriple
            << triples.count ();
#endif
  ...
  !in_reload_all_data_set_gui )
    {
      ...
      enableControls();                                    //ALEXEY ...and
  actual plotting data in_reload_all_data_set_gui = true;
    }
  ...
  *******************************************************************************************************/
  ...
  if (combinedOptics)
    {
      qDebug () << "CellChNumber, cellchans.count() for runType " << runType
                << ": " << CellChNumber_map[runType].toInt () << ", "
                << cellchans.count ();
      qDebug () << "TripleNumber, ntriple for runType " << runType << ": "
                << TripleNumber_map[runType].toInt () << ", " << ntriple;
    }
  else
    {
      qDebug () << "CellChNumber, cellchans.count() for runType " << runType
                << ": " << CellChNumber.toInt () << ", " << cellchans.count ();
      qDebug () << "TripleNumber, ntriple for runType " << runType << ": "
                << TripleNumber.toInt () << ", " << ntriple;
    }
  ...
  if (CheckExpComplete_auto (RunID_to_retrieve, o_connection)
      == 0) // ALEXEY should be == 3 as per documentation
    {
      ...
      qDebug () << "[ABORTION IN EARLY STAGE...] : combinedOptics,  "
                   "opsys_auto.count(), opsys_auto -- "
                << combinedOptics << opsys_auto.count () << opsys_auto;
      if (combinedOptics && opsys_auto.count () < 2)
        {
          QString optics_failed
              = (runType == "RI") ? "Interference" : "Absorbance";
          QString optics_processed
              = (runType == "RI") ? "Absorbance" : "Interference";
          // Inform user that run completed but one of the optics failed
          QMessageBox::critical (
              this, tr ("Optima Optics Failed | Not Initiated:"),
          ...
          if (!tmstampOK)
            {
              ...
              QMessageBox::critical (
                  ...
                  tr ("This run did not start at zero RPM because the w2t "
                      "value "
                      "was non-zero at the beginning of the experiment. \n\n"
                      "Therefore, the boundary conditions are unknown and the "
                      "experiment "
                      "cannot be reliably analyzed. It fails GMP requirements "
                      "and will "
                      "therefore be aborted. The run will be deleted.\n\n"
                      "The data can be retrieved with the \"View Raw Optima "
                      "Data\" function "
                      "in the UltraScan Utilities."));
  ...
  if (!combinedOptics)
    {
      if (cellchans.count () == CellChNumber.toInt ()
          && ntriple == TripleNumber.toInt ()) // <--- Change to the values
                                               // from the protocol
        {
          // stop timer
          timer_all_data_avail->stop ();
          ...
          if (!finishing_live_update)
            {
              ...
              // update hereafter
              connect (timer_data_reload, SIGNAL (timeout ()), this,
                       SLOT (reloadData_auto ()));
    ...
    {
      qDebug () << " In retrieve_xpn_raw_auto(): Combined optics! "
                   "cellchans.count(), CellChNumber_map[ runType ].toInt(); "
                   "ntriple, TripleNumber_map[ runType ].toInt(), runtype -- "
                << cellchans.count () << ", "
                << CellChNumber_map[runType].toInt () << "; " << ntriple
                << ", " << TripleNumber_map[runType].toInt () << runType;
      if (cellchans.count () == CellChNumber_map[runType].toInt ()
          && ntriple == TripleNumber_map[runType].toInt ())
        {
          qDebug () << "opsys_auto, opsys_auto.count(): " << opsys_auto
                    << opsys_auto.count ();

          // ALEXEY: here - either check that
          //  1. xpn_data->countOf( "ascn_rows" ) != 0 && xpn_data->countOf(
          //  "iscn_rows" ) != 0 OR opsys_auto.count() > 1
          ...
          if (opsys_auto.count () > 1) // All optics processed
          ...
          else // not all optics processed & Exp Completed
            {
              ...
              if (expStat == 5
                  || expStat
                         == 0) // Run Completed, or Aborted (SHOULD BE '3' as
                               // per documentaiton but never implemented...)
                {
                  // stop timer
                  timer_all_data_avail->stop ();
                  disconnect (timer_all_data_avail, SIGNAL (timeout ()), 0,
                  ...
                  if (!timer_check_sysdata
                           ->isActive ()) // Check if sys_data Timer is stopped
                    {
                      ...
                      QString optics_processed
                          = (runType == "RI") ? "Absorbance" : "Interference";
                      // Inform user that run completed but one of the optics
                      // failed
                      QMessageBox::critical (
                          this, tr ("Optima Optics Failed:"),
                          tr ("Data collection for %1 optics type failed. \n\n"
                              "The program will proceed to the 3. IMPORT "
                              "stage where "
                              "collected data (%2) can be saved into DB.")
                              .arg (optics_failed)
                              .arg (optics_processed));
                      ...
                      updateautoflow_record_atLiveUpdate (); // We need to
                                                             // include info on
                                                             // the failed
                                                             // optics here!!!
...
US_XpnDataViewer::load_xpn_raw ()
{
  ...
  QString dbhost = xpnhost;
  int dbport = xpnport.toInt ();
  DbgLv (1) << "RDr: call connect_data  dbname h p u w" << xpnname << dbhost
            << dbport << xpnuser << xpnpasw;
  if (xpn_data->connect_data (dbhost, dbport, xpnname, xpnuser, xpnpasw))
    {
      if (dbg_level > 0)
        xpn_data->dump_tables ();
...
DbgLv(1) << "RDr:  post-filter runs" << runInfo.count();
  ...
  scanmask += QString (sMasks).mid (4, 1) == "1" ? 4 : 0;
  scanmask += QString (sMasks).mid (6, 1) == "1" ? 8 : 0;
  DbgLv (1) << "RDr:     iRId" << iRunId << "sMsks scnmask" << sMasks
            << scanmask;
  ...
  if (scanmask == 1 || scanmask == 2 || scanmask == 4 || scanmask == 8)
    ...
    { // Multiple data types present
      ...
      QString msg = tr ("Multiple scan data types are present:\n") + "'"
                    + drtype1 + "'\n or \n" + "'" + drtype2 + "' .\n";
      DbgLv (1) << "RDr:   runType2 scanmask" << runType2 << scanmask
                << "[ifw]scn_rows" << xpn_data->countOf ("iscn_rows")
                << xpn_data->countOf ("fscn_rows")
                << xpn_data->countOf ("wscn_rows");
      if ((runType2 == "IP" && xpn_data->countOf ("iscn_rows") == 0)
          ...
          || (runType2 == "WI" && xpn_data->countOf ("wscn_rows") == 0))
        {
          msg += tr ("\nScans are missing so only ") + drtype1
                 + tr (" scans are processed.");
          QMessageBox::warning (this, tr ("Multiple Types with Missing Scans"),
                                msg);
        }
      ...
      else
        {
          ...
          mbox.setText (msg);
          QPushButton *pb_opt1
              = mbox.addButton (drtype1, QMessageBox::AcceptRole);
          QPushButton *pb_opt2
              = mbox.addButton (drtype2, QMessageBox::RejectRole);
...
DbgLv(1) << "RDr: nwl wvlo wvhi" << nlambda << wvlo << wvhi
  ...
  DbgLv (1) << "RDr: nwl wvlo wvhi" << nlambda << wvlo << wvhi << "ncellch"
            << ncellch << "nlambda" << nlambda << "ntriple" << ntriple
            << triples.count ();
#endif
...
US_XpnDataViewer::load_auc_xpn ()
{
  ...
  if (dir.right (1) != "/")
    dir += "/"; // Ensure trailing /
  xpn_fnames
      = dirdir.entryList (QStringList ("*.auc"), QDir::Files, QDir::Name);
  xpn_fnames.sort ();
  ...
  for (int ii = 0; ii < ntriple; ii++)
    {
      ...
      le_status->setText (
          tr ("Data in for triple %1 of %2").arg (ii + 1).arg (ntriple));
      qApp->processEvents ();
    }
  ...
  if (missm > 0)
    {
      QMessageBox::warning (
          this, tr ("Data/File Cell/Channel Mismatches"),
  ...
  npoint = allData[0].pointCount ();
  ntpoint = nscan * npoint;
  DbgLv (1) << "RDa: mwr ncellch nlambda nscan npoint" << ncellch << nlambda
            << nscan << npoint;
  DbgLv (1) << "RDa:   rvS rvE" << r_radii[0] << r_radii[npoint - 1];
  le_status->setText (tr ("All %1 raw AUCs have been loaded.").arg (ntriple));
  QApplication::restoreOverrideCursor ();
  QApplication::restoreOverrideCursor ();
  qApp->processEvents ();
...
void
US_XpnDataViewer::runDetails (void)
{
  ...
  editd->e->setFont (
      QFont (US_Widgets::fixedFont ().family (), US_GuiSettings::fontSize ()));
  editd->e->setText (msg);
  editd->show ();
}
...
void
US_XpnDataViewer::plot_current (void)
{
  if (allData.size () == 0)
    return;
...
void
US_XpnDataViewer::plot_titles (void)
{
  ...
  QString title = "Radial Intensity Data\nRun ID: " + runID + "\n    Cell: "
                  + cell + "  Channel: " + chan + "  Wavelength: " + wvln;
...
void
US_XpnDataViewer::plot_all (void)
{
  dPlotClearAll (data_plot);
  grid = us_grid (data_plot);
  ...
  for (int scnx = 0; scnx < scan_knt; scnx++)
    { // Build Y vector for each scan
      ...
      QString title = tr ("Raw Data at scan ") + QString::number (scan_nbr);
  ...
  DbgLv (1) << "PltA: xlo xhi" << last_xmin << last_xmax << "ylo yhi"
            << last_ymin << last_ymax;
}

// Slot to handle a change in start or end radius
void
US_XpnDataViewer::changeRadius ()
{
  DbgLv (1) << "chgRadius";
  have_rngs = false;
...
void
US_XpnDataViewer::changeCellCh (void)
{
  ...
  DbgLv (1) << "chgCC:  xmin xmax ymin ymax" << last_xmin << last_xmax
            << last_ymin << last_ymax;
      ...
      if (ccval != ccplr)
        {
          for (int jj = 0; jj < ntriple; jj++)
            {
              ccplr = triples[jj].section ("/", 0, 1).simplified ().replace (
                  " / ", "/");
...
void
US_XpnDataViewer::changeRecord (void)
{
      ...
      if (new_cc != celchn)
        {
          ...
          QString new_wl
              = QString (cb_pltrec->currentText ()).section ("/", 2, 2);
...
void
US_XpnDataViewer::changeOptics_auto (void)
{
  ...
  if (!inExport)
    {
      connect (timer_end_process_all_data_avail, SIGNAL (timeout ()), this,
               SLOT (end_process_all_data_avail ()));
...
// Slot to handle a change in the optical system
void
US_XpnDataViewer::changeOptics (void)
{
  ...
  if (ostyp == "Interference")
    ...
    rtype = "WI";
  DbgLv (1) << "chgOpt: optrx" << optrx << "ostyp" << ostyp << "rtype" << rtype
            << "nopts" << cb_optsys->children ().count ();

  qDebug () << "chgOpt: optrx" << optrx << "ostyp" << ostyp << "rtype" << rtype
            << "runType" << runType << "nopts"
            << cb_optsys->children ().count ();
  ...
  if (inExport)
    {
      ...
      int iRunId = fRunId.toInt ();
      DbgLv (1) << "IN ::changeOptics() when Exporting: RLd:  runID" << runID
                << "runix" << runix << "iRunId" << iRunId;
      int scanmask = 1;
      scanmask = (runType == "FI") ? 2 : scanmask;
      scanmask = (runType == "IP") ? 4 : scanmask;
      scanmask = (runType == "WI") ? 8 : scanmask;
      DbgLv (1) << "RLd:     iRunId" << iRunId << "runType scanmask" << runType
                << scanmask;
  ...
  xpn_data->build_rawData (
      allData); // ALEXEY <-- need to update data BEFORE with
                // xpn_data->import_data() OR reimport_data()
  double tm2 = (double)sttime.msecsTo (QDateTime::currentDateTime ()) / 1000.0;
  ...
  int wvhi = lambdas[nlambda - 1];
  ntriple = xpn_data->data_triples (triples);
  DbgLv (1) << "chgOpt: nwl wvlo wvhi" << nlambda << wvlo << wvhi << "ncellch"
            << ncellch << "nlambda" << nlambda << "ntriple" << ntriple
            << triples.count ();
  ...
  qDebug () << "IF timer restarts? auto_mode_bool, "
               "timer_all_data_avail->isActive(), inExport: "
            << auto_mode_bool << timer_all_data_avail->isActive () << inExport;
...
void
US_XpnDataViewer::prevPlot (void)
{
...
void
US_XpnDataViewer::nextPlot (void)
{
  int pltrx = cb_pltrec->currentIndex () + 1;
...
void
US_XpnDataViewer::compute_ranges ()
{
  ...
  DbgLv (1) << "cmpR:  rS rE rxS rxE" << rad_start << rad_end << radxs
            << radxe;
  ...
  DbgLv (1) << "cmpR:  isMWL" << isMWL << "kpoint" << kpoint << "trpxs"
            << trpxs;
}

// Connect or Disconnect plot-range related controls
void
US_XpnDataViewer::connect_ranges (bool conn)
{
  if (conn)
...
void
US_XpnDataViewer::export_auc_auto (bool &tmstampOK)
{
  inExport = true;

  int nfiles = 0;
  int noptsy = cb_optsys->count ();
  qDebug () << "ExpAucA: noptsy koptsy" << noptsy
            << cb_optsys->children ().count ();
      ...
      for (int osx = 0; osx < noptsy; osx++)
        {
          ...
          // ALEXEY: somewhere here OR in ::changeOptics_auto() make sure to
          // xpn_data->reimport_data() for other optics!!!
          //  Look at the reloadData_auto()

          cb_optsys->setCurrentIndex (osx);
          correct_radii (); // Chromatic aberration correction if needed
          int kfiles = xpn_data->export_auc_auto (allData, tmstampOK)
                       - 2; // Export data
          nfiles += kfiles; // Total files written
        }

      // Restore Optical System selection to what it was before
      // cb_optsys->setCurrentIndex( currsx );   //ALEXEY <-- not needed to
      // repeat data build!!!
    }
...
//   int noptsy     = cb_optsys->children().count();
   int noptsy     = cb_optsys->count();
DbgLv(1) << "ExpAucA: noptsy koptsy" << noptsy <<
cb_optsys->children().count(); qDebug() << "ExpAucA: noptsy koptsy" << noptsy
<< cb_optsys->children().count();
...
xpn_data->reimport_data() for other optics!!!
         ...
         cb_optsys->setCurrentIndex( osx );
         correct_radii();                  // Chromatic aberration correction
if needed int kfiles     = xpn_data->export_auc( allData ) - 2;  // Export data
         nfiles        += kfiles;          // Total files written
      }
...
void
US_XpnDataViewer::export_auc ()
{
  if (!isRaw)
    {
      int status = QMessageBox::information (
          this, tr ("Data Overwrite Warning..."),
          tr ("This operation will overwrite all data currently in"
              "the same directory where these data were loaded from. "
              "Proceed? "),
          tr ("&OK"), tr ("&Cancel"), 0, 0, 1);
      if (status != 0)
        return;
  ...
  if (runIDt != runID)
    {                                // Set runID to new entry given by user
      ...
      if (new_runID.length () > 60)
        {
          new_runID
              = QString (new_runID).left (52) + QString (new_runID).right (8);
      ...
      QMessageBox::warning (this, tr ("RunId Name Changed"),
                            tr ("The runId name has been changed."
                                "\nNew runId:\n  ")
                                + new_runID);
  ...
  qDebug () << "Optical Systems Count: (noptsy) = "
            << cb_optsys->children ().count ();
  ...
  if (noptsy > 1)
    { // Export data from Optical Systems other than currently selected one
      ...
      for (int osx = 0; osx < noptsy; osx++)
        {
          if (osx == currsx)
            continue; // Skip already-handled opt sys
...
void
US_XpnDataViewer::exclude_from (double sfr)
{
...
void
US_XpnDataViewer::exclude_to (double sto)
{
  int scan_to = (int)sto;
  int scan_from = (int)ct_from->value ();
...
void
US_XpnDataViewer::exclude_scans ()
{
  int scan_from = (int)ct_from->value ();
  int scan_to = (int)ct_to->value ();
  int scan_knt = 1;
...
void
US_XpnDataViewer::include_scans ()
{
  excludes.clear ();

  kscan = nscan;
  ktpoint = kscan * kpoint;
  DbgLv (1) << "Incl: nscan" << nscan << "kscn ecnt" << kscan
            << excludes.count ();
  ct_to->setValue (0);
...
int
US_XpnDataViewer::dvec_index (QVector<double> &dvec, const double dval)
{
    ...
    { // If no exact match was found, look for a match within epsilon
      ...
      for (int jj = 0; jj < dvec.size (); jj++)
        { // Search doubles vector
          ...
          // DbgLv(1) << "dvix:   jj" << jj << "dvj" << dvec[jj] << "ddif" <<
          // ddif;
...
void
US_XpnDataViewer::showTimeState ()
{
  QString tspath = currentDir + "/" + runID + ".time_state.tmst";
...
void
US_XpnDataViewer::status_report (QString stat_text)
{
  qApp->processEvents ();
...
US_XpnDataViewer::reloadData ()
{
  ...
  QString fRunId = runID.mid (runix);
  int iRunId = fRunId.toInt ();
  DbgLv (1) << "RLd:  runID" << runID << "runix" << runix << "iRunId"
            << iRunId;
  int scanmask = 1;
  scanmask = (runType == "FI") ? 2 : scanmask;
  scanmask = (runType == "IP") ? 4 : scanmask;
  ...
  if (!upd_ok)
    { // No change in data scans:  report inability to update
      ...
      DbgLv (1) << "RLd:      upd_ok" << upd_ok << "rlt_id" << rlt_id
                << "nscan" << nscan;
      if (rlt_id == 0) // Output message only if not auto-reload
        {
          QMessageBox::warning (
              this, tr ("Reload Data Not Possible"),
  ...
  double tm2 = (double)sttime.msecsTo (QDateTime::currentDateTime ()) / 1000.0;
  DbgLv (1) << "RLd:      build-raw done: tm1 tm2" << tm1 << tm2 << "tm2i"
            << (tm2 - tm1);
  ...
  ct_to->setMaximum (nscan);
  le_status->setText (
      tr ("Update of AUC data complete -- now %1 scans.").arg (nscan));
  qApp->processEvents ();
...
int
US_XpnDataViewer::CheckExpComplete_auto (QString &runid, bool &o_conn)
{
  // Implement Optima's ExperimentRun query for RunStatus field [enum: 0 -
  // NoRunInfo; 2- InProgress; 5- CompleteOK], look in db_defines.h of Dennis's
  // util in utils/us_xpn_data.cpp
...
// Slot to reload data
void
US_XpnDataViewer::reloadData_auto ()
{
  ...
  if (!o_connected)
    {
      timer_data_reload->stop ();
      disconnect (timer_data_reload, SIGNAL (timeout ()), 0,
                  0); // Disconnect timer from anything
      qDebug () << "in [reloadData_auto()]: Stop auto-reload timer: ";
      qDebug () << "in [reloadData_auto()]: statusExp == 0 && NO Coneection "
                   "to Optima!";
      in_reload_auto = false;
      link->disconnected_itself = true; // do we need to set it explicitly?
      reset_liveupdate_panel ();        // assumes link-> disconnected_itself =
  ...
  if (!upd_ok)
    { // No change in data scans:  report inability to update
      ...
      qDebug () << "in reloadData_auto(): udp_ok -> false: (1) no changes in "
                   "# scans, OR no Optima connection!!!";
      ...
      /*** Check Experiement Status: if completed, kill the timer, export the
       * data into AUC format, return, signal to switch panels in US_comproject
       * ***/
      bool o_connection = true;
      ...
      if (statusExp == 5 || statusExp == 0)
        {
          ...
          // if ( statusExp == 0 ) // If there is still connection, then exp.
          // is truly aborted!!
          //   {
          //     //check connection to Optima: if at this point statusExp=0 due
          //     to lost connection
          //     //then, stop everything && return to the Run Manager

          //     /*
          //     Link *link1 = new Link( xpndesc ); // --- THIS DID NOT WORK ??
          //     bool status_sys_data = link1->connectToServer( xpnhost,
          //     xpnmsgPort.toInt() ); qDebug() << "in [reloadData_auto()]:
          //     statusExp == 0; status_sys_data: " << status_sys_data;

          //     bool combined_check = status_sys_data &
          //     link1->connected_itself; link1->disconnectFromServer();
          //     qDebug() << "in [reloadData_auto()]: status_sys_data &
          //     connected_itself = ? "
          // 	       << status_sys_data << " & " << link1->connected_itself
          // << " = " << combined_check;
          //     delete link1; //  --- THIS DID NOT WORK ??
          ...
          //     qDebug() << "DB parms: " << xpndesc11 << xpnhost11 <<
          //     xpnport11 << xpnname11 << xpnuser11 << xpnpasw11 <<
          //     xpnmsgPort11;

          //     US_XpnData* xpn_data11 = new US_XpnData();
          //     bool o_connected       = xpn_data11->connect_data( xpnhost11,
          //     xpnport11.toInt(), xpnname11, xpnuser11,  xpnpasw11 );
          //     xpn_data11->close();
          //     delete xpn_data11;

          //     qDebug() << "DB connection, o_connected ?  -- " <<
          //     o_connected;
          //     //end of checking connection to Optima sys_data server
          ...
          // 	  disconnect(timer_data_reload, SIGNAL(timeout()), 0, 0);
          // //Disconnect timer from anything 	  qDebug() << "in
          // [reloadData_auto()]: Stop auto-reload timer: " ; 	  qDebug() << "in
          // [reloadData_auto()]: statusExp == 0 && NO Coneection to Optima!";
          // 	  in_reload_auto   = false;
          // 	  link-> disconnected_itself = true; // do we need to set it
          // explicitly? 	  reset_liveupdate_panel();          //assumes link->
          // disconnected_itself = true!!! 	  qApp->processEvents();
          ...
          if (!timer_check_sysdata
                   ->isActive ()) // Check if sys_data Timer is stopped
            {
              ...
              // ALEXEY Export AUC data: devise export_auc_auto() function
              // which would return directory name with saved data - to pass to
              // emit signal below...
              bool tmstampOK = true;
              export_auc_auto (tmstampOK);
...
void
US_XpnDataViewer::timerEvent (QTimerEvent *event)
{
...
void
US_XpnDataViewer::changeInterval ()
{
...
void
US_XpnDataViewer::selectColorMap ()
{
  ...
  QString cmfpath = QFileDialog::getOpenFileName (
      this, tr ("Load Color Map File"), US_Settings::etcDir (), filter, 0, 0);
...
// Apply a chromatic aberration correction to auc data radius values <--- New
// function: uses lambdas/corr. from DB
void
US_XpnDataViewer::correct_radii ()
{
      ...
      foreach (QString str, strl)
        {
          ...
          correction.push_back (
              double (str.split (",")[1].trimmed ().toFloat ()));

          qDebug () << str.split (",")[0].trimmed () << " "
                    << str.split (",")[1].trimmed ();
        }
    }
  ...
  if (correction.size () > 0)
    {
      if (!auto_mode_bool)
        {
          int response = QMessageBox::question (
              this, tr ("Chromatic Aberration Correction:"),
              tr ("Wavelength correction data for currently used Optima "
                  "machine\n"
      ...
      // For each triple, get the wavelength; then compute and apply a
      // correction
      for (int jd = 0; jd < ntripl; jd++)
        {
              ...
              if (correction.at (i) != 0.0)
                {
                  for (int jr = 0; jr < npoint; jr++)
                    { // Correct each radial point
                      radval = r_radii[jr]
                               - correction.at (i); // Corrected radius
          ...
          if (jd < 3 || (jd + 4) > ntripl)
            DbgLv (1) << "c_r:  ri0 ro0" << r_radii[0]
                      << allData[jd].xvalues[0] << "rin ron"
                      << r_radii[npoint - 1]
                      << allData[jd].xvalues[npoint - 1];
        }
    }
...
OLD function void US_XpnDataViewer::correct_radii()
{
   const double rad_inc = 1e-5;     // Radius precision
   int ntripl = allData.count();
   int npoint = allData[ 0 ].pointCount();
   ...
   if ( cofile.open( QIODevice::ReadOnly | QIODevice::Text ) )
   {
      ...
      while ( ! cotxti.atEnd() )
      {
         ...
         {  // Get values from first non-empty, non-comment line
            ...
            if (i == fline.section( " ", 0, 0 ).simplified().toInt())
            {
               ...
               correction.push_back(fline.section( " ", 1, 1
).simplified().toDouble());
            }
            i++;
         }
      ...
      else
      {
         QMessageBox::warning( this,
            ...
            tr( "The wavelength correction file:\n") +
                US_Settings::etcDir() + "/chromo-aberration-array.dat\n" +
            tr( "is found and will be used to correct your data for
chromatic\n" "aberration between 190 nm and 800 nm. Exported data will be
modified.\n") );
      }
   }
   else
   {
      // only use chromo-aberration-coeffs.dat if chromo-aberration-array.dat
isn't available cofname = US_Settings::etcDir() +
"/chromo-aberration-coeffs.dat"; cofile.setFileName( cofname );

      if ( cofile.open( QIODevice::ReadOnly | QIODevice::Text ) )
      {
         double a_coef  = 0.0;         // Default 5th order polynomial
coefficents double b1_coef = 0.0; double b2_coef = 0.0; double b3_coef = 0.0;
         double b4_coef = 0.0;
         double b5_coef = 0.0;
         QTextStream cotxti( &cofile );
         while ( ! cotxti.atEnd() )
...
aberration\n" "between 190 nm and 800 nm. Exported data will be modified.\n")
   ...
   {
      // For each triple, get the wavelength; then compute and apply a
correction for ( int jd = 0; jd < ntripl; jd++ )
      {
         int i=0;
         while ((int) lambda.at(i) != (int) allData[ jd ].scanData[ 0
].wavelength)
         {
            i++; // Wavelength index
            if (i > 610) break;
            if (correction.at(i)  != 0.0 )
            {
               for ( int jr = 0; jr < npoint; jr++ )
               {  // Correct each radial point
                  radval = r_radii[ jr ] - correction.at(i);     // Corrected
radius radval = qRound( radval / rad_inc ) * rad_inc; // Rounded allData[ jd
].xvalues[ jr ] = radval;          // Replace radius
               }
            }
         }
if (jd<3 || (jd+4)>ntripl )
...
void
US_XpnDataViewer::currentRectf (QRectF rectf)
{
          ...
          for (int jq = 0; jq < nspeed; jq++)
            {
              ...
              int nscan = jsl - jsf;
              double afact = 1.0 / (double)nscan;
              DbgLv (1) << " MSPD: jq" << jq << "speed" << irpm
                        << "jsf,jsl,nsc" << jsf << jsl << nscan;
              ascdat.fill (0.0, kpoint);
  ...
  le_status->setText (tr ("%1  Radial adjustment scan complete.").arg (cech));
  QApplication::restoreOverrideCursor ();
  qApp->processEvents ();
}
