  ...
  for (int ii = 0; ii < rdirs.count (); ii++)
    {
      ...
      qDebug () << "LdDk:   ii" << ii << "run" << rdirs[ii] << "count"
                << nfiles;
      ...
      rr.date = date;
      rr.ntriple = nfiles;
      qDebug () << "LdDk:   ii" << ii << "     runID date count" << rr.runID
                << rr.date << rr.ntriple;
...
void
US_XpnRunAuc::limit_data (const QString &sfilt)
{
  ...
  for (int ii = 0; ii < runInfo.size (); ii++)
    {
      ...
      item->setFlags (item->flags () ^ Qt::ItemIsEditable);
      tw->setItem (ii, 2, item);
      qDebug () << "setItems ii" << ii << "date runID dbId" << rr.date
                << rr.runID << rr.dbId;
