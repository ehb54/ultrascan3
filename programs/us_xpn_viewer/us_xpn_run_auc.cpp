...
void US_XpnRunAuc::load_runs(void) {
  ...
  if (impdir.right(1) != "/") impdir = impdir + "/";  // Insure trailing slash
  ...
  QStringList runids;
  QStringList rdirs =
      QDir(impdir).entryList(QDir::AllDirs | QDir::NoDotAndDotDot, QDir::Name);
...
void US_XpnRunAuc::limit_data(const QString& sfilt) {
  ...
  for (int ii = 0; ii < runInfo.size(); ii++) {
    ...
    item->setFlags(item->flags() ^ Qt::ItemIsEditable);
    tw->setItem(ii, 2, item);
    qDebug() << "setItems ii" << ii << "date runID dbId" << rr.date << rr.runID
             << rr.dbId;
