...
void US_XpnRunRaw::load_runs(void) {
   ...
   for (int ii = 0; ii < nruns; ii++) {
      ...
      rirec.runID = QString().sprintf("%s-run%d", rirec.expname.toLatin1().data(), rirec.dbId);
...
void US_XpnRunRaw::populate_list() {
   ...
   QStringList headers;
   headers << tr("Run") << tr("Date") << tr("RunId") << tr("ExpId") << tr("DataCount") << tr("Experiment")
           << tr("Researcher") << tr("A?") << tr("F?") << tr("I?") << tr("W?");
