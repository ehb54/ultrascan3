...
US_vHW_Enhanced::US_vHW_Enhanced() : US_AnalysisBase2() {
   ...
   connect(pb_dstrpl, SIGNAL(clicked()), this, SLOT(distr_plot()));
   connect(pb_selegr, SIGNAL(clicked()), this, SLOT(sel_groups()));
   connect(pb_replot, SIGNAL(clicked()), this, SLOT(plot_refresh()));
   connect(ck_modelpl, SIGNAL(toggled(bool)), this, SLOT(data_plot()));
   connect(ck_vhw_enh, SIGNAL(toggled(bool)), this, SLOT(data_plot()));
   connect(ck_use_fed, SIGNAL(toggled(bool)), this, SLOT(data_plot()));
   ...
   connect(ct_tolerance, SIGNAL(valueChanged(double)), this, SLOT(update_bdtoler(double)));
   ...
   connect(ct_division, SIGNAL(valueChanged(double)), this, SLOT(update_divis(double)));
...
void US_vHW_Enhanced::load(void) {
   ...
   connect(gpick, SIGNAL(mouseDown(const QwtDoublePoint &)), this, SLOT(groupClick(const QwtDoublePoint &)));
   groupstep = NONE;
   ...
   for (int ii = 0; ii < triples.size(); ii++) {
      ...
      DbgLv(1) << "vhw:   init:   init'd simraw (s x p)" << simraw.scanCount() << simraw.pointCount();
      DbgLv(1) << "vhw:   init:   copy'd simdat (s x p)" << simdat.scanCount() << simdat.pointCount();
...
void US_vHW_Enhanced::data_plot(void) {
   ...
   if (mo_plats) { // Calculate plateaus from a model
      model_plateaus();
   }

   else { // Calculate plateaus from fitting to specified values
      fitted_plateaus();
   }
   ...
   if (vhw_enh) { // Calculate all plot points using enhanced method
      vhw_calcs_enhanced(ptx, pty);
   }

   else { // Calculate all plot points using standard method
      vhw_calcs_standard(ptx, pty);
   }

   // Remove points that are singles in a division
   for (int jj = 0; jj < divsCount; jj++) { // Examine each division
      int count = 0;
      for (int ii = 0; ii < lscnCount; ii++) { // Count unexcluded scan points in a division
      ...
      if (count == 1) { // Mark a division with a single point as having none
         int kk = jj;
   ...
   QString ttrip = edata->cell + "/" + edata->channel + "/" + edata->wavelength;
   ...
   for (int ii = 0; ii < lscnCount; ii++) {
      ...
      for (int jj = 0; jj < divsCount; jj++) {
         ...
         if (xv >= 0.0 && yv >= 0.0) { // Points in a scan
            x[ count ] = xv;
            y[ count ] = yv;
      ...
      if (count > 0) { // Plot the points in a scan
         curve = us_curve(data_plot1, tr("Sed Coeff Points, scan %1").arg(ii + 1));
   ...
   for (int jj = 0; jj < divsCount; jj++) { // Walk thru divisions, fitting line to points from all scans
      count = 0;

      for (int ii = 0; ii < lscnCount; ii++) {
         kk = ii * divsCount + jj; // Sed. coeff. index

         if (ptx[ ii ] > 0.0 && pty[ kk ] > 0.0) { // Points for scans in a division
            x[ count ] = ptx[ ii ];
            y[ count ] = pty[ kk ];
            count++;
            int js = liveScans[ ii ];
      ...
      if (count > 1) { // Fit a line to the scan points in a division
   ...
   for (int ii = 0; ii < lscnCount; ii++) { // Accumulate the points of the back-diffusion cutoff line
   ...
   for (int mm = 0; mm < 20; mm++)
      DbgLv(1) << "TIMINGS: mm" << mm << "kcalls kmsecs" << kcalls[ mm ] << kmsecs[ mm ] << cd[ mm ];
   //*TIMING
}
...
void US_vHW_Enhanced::write_report(QTextStream &ts) {
   ...
   ts << html_header("US_vHW_Enhanced", tr("van Holde - Weischet Analysis"), edata);
   ts << analysis("");
   ...
   if (ngrp == 0)
      ...
      ts << table_row(tr("Group:"), tr("Average S:"), tr("Relative Amount:"));

      double tsed = 0.0;

      for (int jj = 0; jj < ngrp; jj++) {
         ts << table_row(
            QString().sprintf("%3d:", jj + 1), QString().sprintf("%6.2f", groupdat.at(jj).sed),
   ...
   ts << table_row(tr("Average S:"), QString::number(Swavg * 1.0e13));
   ts << table_row(tr("Initial Concentration from plateau fit:"), QString::number(C0) + tr(" OD/fringes"));
   ts << indent(4) + "</table>\n";
   ...
   for (int ii = 0; ii < lscnCount; ii++) { // accumulate time,plateau pairs for line fit
      int js = liveScans[ ii ];
   ...
   ts << "\n" + indent(4) + "<br/><table>\n";
   ts << table_row(tr("Initial Concentration:"), QString::number(ci) + " OD");
   ts << table_row(tr("Correlation Coefficient:"), QString::number(cor));
   ts << table_row(tr("Standard Deviation:"), QString::number(sig));
   ts << table_row(tr("Initial Concentration from exponential fit:"), QString::number(C0) + " OD");
   ts << indent(4) + "</table>\n";
...
void US_vHW_Enhanced::save_data(void) {
   ...
   QString basernam = US_Settings::resultDir() + "/" + edata->runID + "/vHW." + tripl + ".";
   QStringList files;
   ...
   QString basename = US_Settings::reportDir() + "/" + edata->runID + "/vHW." + tripl + ".";
   ...
   QString dsinfFile = QString(basename).replace("/vHW.", "/dsinfo.") + "dataset_info.html";
   ...
   if (last_plot != row) { // If plot dialog last opened not for this triple, get files now
      US_DistribPlot *dialog = new US_DistribPlot(bfracs, dseds, total_conc);
   ...
   else { // Otherwise, copy the temporary files created earlier
   ...
   if (disk_controls->db()) { // Also save report files to the database
...
void US_vHW_Enhanced::view_report(void) {
   ...
   edit->resize(700, 600);
   edit->e->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize()));
...
void US_vHW_Enhanced::sel_groups(void) {
   if (groupSel) { // had been Select, now doing Clear
      groupstep = NONE;
   ...
   else { // had been Clear, now doing new Select
      groupstep = START;
      pb_selegr->setText(tr("Clear Groups"));
      groupxy.clear();

      QMessageBox::information(
         this, tr("Group Selection"),
...
int US_vHW_Enhanced::first_gteq(double concenv, QVector<double> &rvalues, int valueCount, int defndx) {
   int index = defndx; // Set return index to default

   for (int jj = 0; jj < valueCount; jj++) { // Find index where readings value equals or exceeds given concentration
...
int US_vHW_Enhanced::first_gteq(double concenv, QVector<double> &rvalues, int valueCount) {
...
double US_vHW_Enhanced::sed_coeff(double cconc, double oterm, double *radP, int *ndxP) {
   //*TIMING
   kcalls[ 16 ] += 1;
   QDateTime sttime = QDateTime::currentDateTime();
   //*TIMING
   ...
   if (j2 >= 0 && oterm >= 0.0) { // Likely need to interpolate radius from two input values
      int j1 = j2 - 1;

      if (j2 > 0) { // Interpolate radius value
   ...
   if (rv0 > 0.0) { // Use radius and other terms to get corrected sedimentation coeff. value
...
void US_vHW_Enhanced::div_seds() {
   ...
   //*TIMING
   kcalls[ 11 ] += 1;
   QDateTime sttim1 = QDateTime::currentDateTime();
   //*TIMING
   for (int ii = 0; ii < lscnCount; ii++) {
      ...
      bdleft = bdtoler * bdifcsqr / (bdiff_sedc * omegasq * (bottom + edata->meniscus) * timesqr);
      xbdleft = US_Math2::find_root(bdleft);
      DbgLv(1) << "  bottom meniscus bdleft" << bottom << edata->meniscus << bdleft << "tcor tsqr tm_corr" << timecor
               << timesqr << time_correction;
      ...
      DbgLv(1) << "  ii nscnu" << ii << nscnu + 1 << " bdsed radD yr" << bdiff_sedc << radD << yr[ nscnu ];
   ...
   //*TIMING
   kcalls[ 12 ] += 1;
   QDateTime sttim2 = QDateTime::currentDateTime();
   //*TIMING

   for (int jj = 0; jj < divsCount; jj++) { // Loop to fit points across scans in a division
      //*TIMING
      kcalls[ 13 ] += 1;
      QDateTime sttim3 = QDateTime::currentDateTime();
      //*TIMING
      double dsed;
      double slope;
      double sigma;
      ...
      for (int kk = 0; kk < nscnu; kk++) { // Accumulate concentration, sed.coeff. for all scans, this div
         ...
         if (radC > radD) { // Gone beyond back-diffusion cutoff: exit loop with truncated list
            kscnu = kk;
            yy[ kk ] = SEDC_NOVAL;
      ...
      //*TIMING
      kcalls[ 14 ] += 1;
      QDateTime sttim4 = QDateTime::currentDateTime();
      //*TIMING
      ...
      for (int kk = 0; kk < kscnu; kk++) { // Remove any leading points below meniscus
         if (yy[ kk ] != SEDC_NOVAL) { // Sed coeff value not from below meniscus
            xtvec << xx[ kk ];
            ytvec << yy[ kk ];
            ii++;
         }
      ...
      DbgLv(1) << " KS" << kscnu << kscsv << "size xtv ytv" << xtvec.size() << ytvec.size();
      //*TIMING
      kmsecs[ 14 ] += sttim4.msecsTo(QDateTime::currentDateTime());
      //*TIMING
      //*TIMING
      kcalls[ 15 ] += 1;
      QDateTime sttim5 = QDateTime::currentDateTime();
      //*TIMING
      ...
      if (kscnu > 1) {
         ...
         DbgLv(1) << " KS" << kscnu << kscsv << "jj" << jj << "xx0 xxn yy0 yyn" << xt[ 0 ] << xt[ kscnu - 1 ] << yt[ 0 ]
                  << yt[ kscnu - 1 ] << "slo sed" << slope << dsed;
      }
   ...
   for (int kk = 0; kk < nscnu; kk++) { // Save the Radius,Concentration points for back-diffusion cutoff curve
      bdrads << xr[ kk ];
      bdcons << yr[ kk ];
   }
...
void US_vHW_Enhanced::groupClick(const QwtDoublePoint &p) {
   ...
   QString gbanner;
   int ngroup;
   DbgLv(1) << "groupClick: step" << groupstep << "x y" << p.x() << p.y();
   ...
   switch (groupstep) {
      ...
      case END:
         ...
         gbanner = tr("Group %1: %2 (%3%)").arg(ngroup).arg(cgrdata.sed).arg(cgrdata.percent);
         label.setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() + 2, QFont::Bold));
...
void US_vHW_Enhanced::add_group_info() {
   ...
   for (int jj = 0; jj < divsUsed; jj++) { // walk thru all division lines to see if within clicked range
      ...
      if (yh <= grdat.y1 && yl >= grdat.y2) { // line is in group:  add division to group
...
void US_vHW_Enhanced::write_vhw() {
   ...
   QString filename = dirname + "/vHW." + QString(triples.at(row)).replace(" / ", "") + ".extrap.csv";
   ...
   for (int ii = 0; ii < lscnCount; ii++) {
      ...
      int js = liveScans[ ii ];
      control = fsep;
      QString dat = QString().sprintf("\"%11.8f\"", (1.0 / sqrt(edata->scanData[ js ].seconds - time_correction)));
      dat.replace(" ", "");
      ts << dat << control;
...
void US_vHW_Enhanced::write_dis() {
   QString filename = US_Settings::resultDir() + "/" + edata->runID + "/vHW."
                      + QString(triples.at(row)).replace(" / ", "") + ".s-c-distrib.csv";
   QFile res_f(filename);
   double bterm = 100.0 * boundPct / ( double ) divsCount;
   double pterm = 100.0 * positPct + bterm;
   double bfrac;
   ...
   for (int jj = 0; jj < dseds.size(); jj++) {
      ...
      dline.sprintf(
         "\"%9.2f\",\"%7d\",\"%12.6f\",\"%12.6f\",\"%12.6f\",\"%12.6f\"\n", bfrac, dpnts[ jj ], dslos[ jj ],
         dseds[ jj ], dsigs[ jj ], dcors[ jj ]);
      dline.replace(" ", "");
      ts << dline;
   }
...
void US_vHW_Enhanced::write_model() {
   QString filename = US_Settings::resultDir() + "/" + edata->runID + "/vHW."
                      + QString(triples.at(row)).replace(" / ", "") + ".fef_model.rpt";
   QFile res_f(filename);
   ...
   QString line04 = "1            \t" + tr("# Sedimentation Coefficient fitting control\n");
   QString line05 = "7.00e-07     \t" + tr("# Diffusion Coefficient in D\n");
   QString line06 = "7.00e-08     \t" + tr("# Diffusion Coefficient range in D\n");
   QString line07 = "1            \t" + tr("# Diffusion Coefficient fitting control\n");
   QString line10 = "0            \t" + tr("# Partial Concentration fitting control\n");
   ...
   QString line17 = "1            \t" + tr("# Molecular Weight fitting control\n");
   QString line18 = "0            \t" + tr("# Part. Spec. Volume fitting control\n");
   ...
   for (int ii = 0; ii < groups; ii++) {
      ...
      ts << gsed << "  " << tr("\t# Sedimentation Coefficient in s\n");
      ts << gsed / 10.0 << "  " << tr("\t# Sedimentation Coefficient range in s\n");
      ts << line04 << line05 << line06 << line07;
      ts << gcon << "    " << tr("\t# Partial Concentration in OD\n");
      ts << gcon / 10.0 << "   " << tr("\t# Partial Concentration range in OD\n");
...
QString US_vHW_Enhanced::text_time(double seconds) { // default mins,secs text
...
double US_vHW_Enhanced::sedcoeff_intercept() {
   ...
   for (int ii = 0; ii < lscnCount; ii++) { // Accumulate x,y values:  1/sqrt(time), sed_coeff
      int js = liveScans[ ii ];
      dscan = &edata->scanData[ js ];
      ...
      DbgLv(2) << " s-i: range baseline basecut platcut mconc" << range << baseline << basecut << platcut << mconc;
   }
   ...
   if (sedc <= 0.0) { // If it dropped below zero, back up line until it crosses to positive
      DbgLv(1) << "sedco-intcp **SEDC**" << sedc << "x0 y0 xn yn n" << xr[ 0 ] << yr[ 0 ] << xr[ nscnu - 1 ]
               << yr[ nscnu - 1 ] << nscnu;
...
void US_vHW_Enhanced::copy_data_files(QString plot1File, QString plot2File, QString data2File) {
...
bool US_vHW_Enhanced::fitted_plateaus() {
   ...
   DbgLv(1) << "3rd-ord poly fit coefs:" << coefs[ 0 ] << coefs[ 1 ] << coefs[ 2 ] << coefs[ 3 ];

   for (int jj = 0; jj < kf; jj++) {
      double xx = csx[ jj ];
      double yy = coefs[ 0 ] + coefs[ 1 ] * xx + coefs[ 2 ] * xx * xx + coefs[ 3 ] * xx * xx * xx;
      fsy[ jj ] = yy;
   ...
   for (int jj = 2; jj < kf; jj++) {
      ...
      if (currd == 0.0 || (currd > 0.0 && prevd < 0.0) || (currd < 0.0 && prevd > 0.0)) { // Zero point or zero crossing
         jrelp = jj - 1;
      ...
      if (prevy < 0.0 && curry < prevy) { // Count consecutive decreases in negative slopes
         kdecr++;
         if (kdecr > 2) { // 3rd consecutive decrease in negative slope, break at this point
            jrelp = jj;
      ...
      else { // Reset count of consecutive decreasing negative slopes
         kdecr = 0;
      }
   ...
   DbgLv(1) << " KRELP" << krelp << "   slope intcp sigma" << slope << intcp << sigma;
   ...
   for (int ii = 0; ii < lscnCount; ii++) { // each extrapolated plateau is exp of Y for X of corrected time
      int js = liveScans[ ii ];
      dscan = &edata->scanData[ js ];
...
void US_vHW_Enhanced::vhw_calcs_standard(double *ptx, double *pty) {
   ...
   C0 = 0.0;
   Swavg = 0.0;
   edata = (ck_use_fed->isChecked() && have_model()) ? &dsimList[ row ] : &dataList[ row ];
   ...
   for (int ii = 0; ii < lscnCount; ii++) {
      ...
      DbgLv(1) << "scn liv" << ii + 1 << kl << " radius concen time" << bdrad << bdcon << timev;
      ...
      for (int jj = 0; jj < divsCount; jj++) { // walk through division points; get sed. coeff. by place in readings
         ...
         if (divrad > bdrad) { // Mark a point to be excluded by back-diffusion
            sedc = SEDC_NOVAL;
            DbgLv(1) << "CPm: *excl* div" << jj + 1 << " drad dcon " << divrad << mconc;
         }
         DbgLv(1) << "CPm: ii jj" << ii << jj << "kk iSedc" << kk << sedc << "mconc divrad bdrad" << mconc << divrad
                  << bdrad;
...
void US_vHW_Enhanced::vhw_calcs_enhanced(double *ptx, double *pty) {
   ...
   while (iter <= mxiter) {
      ...
      for (int ii = 0; ii < lscnCount; ii++) {
         ...
         for (int jj = 0; jj < divsCount; jj++) { // Recalculate partial concentrations based on sedcoeff intercepts
            sedc = (jj < divsUsed) ? dseds[ jj ] : SEDC_NOVAL;
            cpij = (sedc != SEDC_NOVAL) ? (c0term * exp(sedc * eterm)) : CPijs[ ii ][ jj ];
            CPijs[ ii ][ jj ] = cpij;
            sumcpij += cpij;
         ...
         for (int jj = 0; jj < divsCount; jj++) { // Spread the difference to each partial plateau concentration
            CPijs[ ii ][ jj ] += sdiff;
         }

         avgdif += qAbs(sdiff); // Sum of difference magnitudes
         count++;
         DbgLv(1) << "   iter scn " << iter << ii + 1 << " sumcpij span " << sumcpij << span << "  sdiff sumabsdif"
                  << sdiff << avgdif;
      }
...
void US_vHW_Enhanced::vert_exclude_lines() {
   ...
   for (int ii = 0; ii < lscnCount; ii++) {
      ...
      xx[ 1 ] = xx[ 0 ];
      curve = us_curve(data_plot1, tr("Scan %1 Exclude Marker").arg(ixsc));
...
void US_vHW_Enhanced::exclude_from(double from) {
   ...
   if (to < from) { // Adjust exclude-to if need be
      ct_to->disconnect();
      ct_to->setValue(from);
...
void US_vHW_Enhanced::exclude_to(double to) {
   ...
   if (from > to || from == 0) { // Adjust exclude-from if need be
      DbgLv(1) << "(2)TO=" << to;
      ct_from->disconnect();
      if (from > to)
         ct_from->setValue(to);
...
void US_vHW_Enhanced::live_scans() {
   ...
   for (int ii = 0; ii < scanCount; ii++) { // For non-excluded scans, save scan index and plateau concentration
...
void US_vHW_Enhanced::init_partials() {
   ...
   for (int ii = 0; ii < lscnCount; ii++) { // Populate partial concentration lists
...
void US_vHW_Enhanced::update_mid_concs() {
   ...
   for (int ii = 0; ii < lscnCount; ii++) { // Populate mid-division concentration vector of each scan
      range = scPlats[ ii ] - baseline;
      bconc = baseline + range * positPct; // Division 1 base conc.

      for (int jj = 0; jj < divsCount; jj++) { // Determine new concentrations in divisions of the scan
...
void US_vHW_Enhanced::create_simulation() {
   ...
   if (cnst_vbar) { // Compute data corrections for constant vbar
      solution.vbar20 = le_vbar->text().toDouble();
      solution.vbar = US_Math2::calcCommonVbar(solution_rec, avgTemp);
      US_Math2::data_correction(avgTemp, solution);
   }
   ...
   for (int ii = 0; ii < amodel.components.size(); ii++) {
      if (vari_vbar) { // Compute data corrections for varying vbar
   ...
   int mm = nn / 2;
   DbgLv(1) << " cr.sim.:  sd 00 0m 0n l0 lm ln" << simraw.value(0, 0) << simraw.value(0, mm) << simraw.value(0, nn)
            << simraw.value(ll, 0) << simraw.value(ll, mm) << simraw.value(ll, nn);
...
void US_vHW_Enhanced::plot_data2() {
   if (!ck_use_fed->isChecked() || !have_model()) { // Normal experimental data plot
   ...
   if (simda->dataType == "RI")
      dataType = tr("Intensity");
   if (simda->dataType == "WI")
      dataType = tr("Intensity");
   if (simda->dataType == "IP")
      dataType = tr("Interference");
   if (simda->dataType == "FI")
      dataType = tr("Fluorescence");

   QString header = tr("Simulation Velocity Data for\n ") + simda->runID + "   (" + simda->cell + "/" + simda->channel
                    + "/" + simda->wavelength + ")";
...
void US_vHW_Enhanced::get_model() {
   ...
   if (
      modelGUID.isEmpty() || modelGUID.length() != 36
      || (!mmodlGUID.isEmpty() && mmodlGUID == modelGUID)) { // No new model load if none exists or already loaded matches noise
      DbgLv(1) << "gMo:  MODEL empty/matches  IDln" << modelGUID.length() << "modID" << modelGUID << "mmoID"
               << mmodlGUID;
      return;
   }
