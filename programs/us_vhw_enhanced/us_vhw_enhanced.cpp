...
US_vHW_Enhanced::US_vHW_Enhanced() : US_AnalysisBase2() {
  ...
  connect(ct_tolerance, SIGNAL(valueChanged(double)), this,
          SLOT(update_bdtoler(double)));
...
void US_vHW_Enhanced::load(void) {
  ...
  connect(gpick, SIGNAL(mouseDown(const QwtDoublePoint&)), this,
          SLOT(groupClick(const QwtDoublePoint&)));
  groupstep = NONE;
  ...
  for (int ii = 0; ii < triples.size(); ii++) {
    ...
    DbgLv(1) << "vhw:   init:   init'd simraw (s x p)" << simraw.scanCount()
             << simraw.pointCount();
    DbgLv(1) << "vhw:   init:   copy'd simdat (s x p)" << simdat.scanCount()
             << simdat.pointCount();
...
void US_vHW_Enhanced::data_plot(void) {
  ...
  if (!dataLoaded || vbar <= 0.0 || skipPlot) return;

  if (!forcePlot && ck_manrepl->isChecked()) return;
  ...
  for (int jj = 0; jj < divsCount; jj++) {  // Examine each division
    int count = 0;
    for (int ii = 0; ii < lscnCount;
         ii++) {  // Count unexcluded scan points in a division
  ...
  for (int jj = 0; jj < divsCount;
       jj++) {  // Walk thru divisions, fitting line to points from all scans
    count = 0;
    ...
    if (count > 1) {  // Fit a line to the scan points in a division
      if (jj < 3 || jj > (divsCount - 9))
        DbgLv(1) << "plot2 jj count" << jj << count << "  y0 yn" << y[0]
                 << y[count - 1];
      double sigma = 0.0;
      double correl;
      ...
      if (jj < 3 || jj > (divsCount - 9))
        DbgLv(1) << "plot2 jj" << jj << "x0 y0 x1 y1" << x[0] << y[0] << x[1]
                 << y[1];
  ...
  for (int ii = 0; ii < lscnCount;
       ii++) {  // Accumulate the points of the back-diffusion cutoff line
  ...
  aseds.clear();
  for (int ii = 0; ii < totalCount; ii++) aseds << pty[ii];
  ...
  for (int mm = 0; mm < 20; mm++)
    DbgLv(1) << "TIMINGS: mm" << mm << "kcalls kmsecs" << kcalls[mm]
             << kmsecs[mm] << cd[mm];
  //*TIMING
}
...
void US_vHW_Enhanced::write_report(QTextStream& ts) {
  ...
  ts << html_header("US_vHW_Enhanced", tr("van Holde - Weischet Analysis"),
                    edata);
  ts << analysis("");
  ...
  if (ngrp == 0)
    ...
    for (int jj = 0; jj < ngrp; jj++) {
      ts << table_row(QString().sprintf("%3d:", jj + 1),
  ...
  for (int ii = 0; ii < lscnCount;
       ii++) {  // accumulate time,plateau pairs for line fit
    int js = liveScans[ii];
...
void US_vHW_Enhanced::save_data(void) {
  ...
  QString basernam =
      US_Settings::resultDir() + "/" + edata->runID + "/vHW." + tripl + ".";
  QStringList files;
  ...
  QString basename =
      US_Settings::reportDir() + "/" + edata->runID + "/vHW." + tripl + ".";
  ...
  QString dsinfFile =
      QString(basename).replace("/vHW.", "/dsinfo.") + "dataset_info.html";
  ...
  if (!rpt_f.open(QIODevice::WriteOnly | QIODevice::Text)) return;
  ...
  if (last_plot !=
      row) {  // If plot dialog last opened not for this triple, get files now
    US_DistribPlot* dialog = new US_DistribPlot(bfracs, dseds, total_conc);
...
// update density
void US_vHW_Enhanced::update_density(double dval) { density = dval; }

// update viscosity
void US_vHW_Enhanced::update_viscosity(double dval) { viscosity = dval; }

// update vbar
void US_vHW_Enhanced::update_vbar(double dval) { vbar = dval; }
...
int US_vHW_Enhanced::first_gteq(double concenv, QVector<double>& rvalues,
                                int valueCount, int defndx) {
  int index = defndx;  // Set return index to default

  for (int jj = 0; jj < valueCount;
       jj++) {  // Find index where readings value equals or exceeds given
                // concentration
...
double US_vHW_Enhanced::sed_coeff(double cconc, double oterm, double* radP,
                                  int* ndxP) {
  //*TIMING
  kcalls[16] += 1;
  QDateTime sttime = QDateTime::currentDateTime();
  ...
  if (j2 >= 0 &&
      oterm >=
          0.0) {  // Likely need to interpolate radius from two input values
    int j1 = j2 - 1;
  ...
  if (rv0 > 0.0) {  // Use radius and other terms to get corrected sedimentation
                    // coeff. value
    sedc = correc * log(rv0 / edata->meniscus) / oterm;
    // DbgLv(2) << "sed_coeff:   rv0" << rv0 << " sedc" << sedc << " oterm" <<
    // oterm;
  }
...
void US_vHW_Enhanced::div_seds() {
  ...
  for (int ii = 0; ii < lscnCount; ii++) {
    ...
    int js = liveScans[ii];
    DbgLv(1) << "  ii nscnu" << ii << nscnu << " js lscnCount" << js
             << lscnCount;
    dscan = &edata->scanData[js];
    ...
    DbgLv(1) << "  bottom meniscus bdleft" << bottom << edata->meniscus
             << bdleft << "tcor tsqr tm_corr" << timecor << timesqr
             << time_correction;
    ...
    DbgLv(1) << "  bdifsedc find_root toler" << bdiff_sedc << xbdleft
             << bdtoler;
  ...
  for (int jj = 0; jj < divsCount;
       jj++) {  // Loop to fit points across scans in a division
    //*TIMING
    kcalls[13] += 1;
    ...
    for (int kk = 0; kk < nscnu; kk++) {  // Accumulate concentration,
                                          // sed.coeff. for all scans, this div
      ...
      // DbgLv(1) << "DVS    kk jj" << kk << jj << "mconc cpij" << mconc <<
      // cpij;
      ...
      if (radC > radD) {  // Gone beyond back-diffusion cutoff: exit loop with
                          // truncated list
        kscnu = kk;
        yy[kk] = SEDC_NOVAL;
        // DbgLv(1) << " div kscnu" << jj+1 << kscnu
        //    << " radC radD" << radC << radD << " mconc sedcc" << mconc <<
        //    yy[kk];
        break;
      }
    }
    ...
    for (int kk = 0; kk < kscnu;
         kk++) {                   // Remove any leading points below meniscus
    ...
    if (kscnu > 1) {
      ...
      DbgLv(1) << " KS" << kscnu << kscsv << "jj" << jj << "xx0 xxn yy0 yyn"
               << xt[0] << xt[kscnu - 1] << yt[0] << yt[kscnu - 1] << "slo sed"
               << slope << dsed;
    }
  ...
  for (int kk = 0; kk < nscnu; kk++) {  // Save the Radius,Concentration points
                                        // for back-diffusion cutoff curve
    bdrads << xr[kk];
    bdcons << yr[kk];
  }
...
void US_vHW_Enhanced::add_group_info() {
  ...
  for (int jj = 0; jj < divsUsed;
       jj++) {  // walk thru all division lines to see if within clicked range
    ...
    if (yh <= grdat.y1 &&
        yl >= grdat.y2) {      // line is in group:  add division to group
...
void US_vHW_Enhanced::write_vhw() {
  ...
  if (!dirr.exists(dirname)) dirr.mkpath(dirname);

  if (!dirp.exists(dirrept)) dirp.mkpath(dirrept);
...
void US_vHW_Enhanced::write_dis() {
  ...
  for (int jj = 0; jj < dseds.size(); jj++) {
    ...
    dline.sprintf(
        ...
        bfrac, dpnts[jj], dslos[jj], dseds[jj], dsigs[jj], dcors[jj]);
    dline.replace(" ", "");
    ts << dline;
  }
...
void US_vHW_Enhanced::write_model() {
  ...
  QString line04 =
      "1            \t" + tr("# Sedimentation Coefficient fitting control\n");
  QString line05 = "7.00e-07     \t" + tr("# Diffusion Coefficient in D\n");
  QString line06 =
      "7.00e-08     \t" + tr("# Diffusion Coefficient range in D\n");
  QString line07 =
      "1            \t" + tr("# Diffusion Coefficient fitting control\n");
  QString line10 =
      "0            \t" + tr("# Partial Concentration fitting control\n");
  ...
  QString line17 =
      "1            \t" + tr("# Molecular Weight fitting control\n");
  QString line18 =
      "0            \t" + tr("# Part. Spec. Volume fitting control\n");
...
double US_vHW_Enhanced::sedcoeff_intercept() {
  ...
  for (int ii = 0; ii < lscnCount;
       ii++) {  // Accumulate x,y values:  1/sqrt(time), sed_coeff
    int js = liveScans[ii];
    dscan = &edata->scanData[js];
    ...
    DbgLv(2) << " s-i: range baseline basecut platcut mconc" << range
             << baseline << basecut << platcut << mconc;
  }
  ...
  if (sedc <= 0.0) {  // If it dropped below zero, back up line until it crosses
                      // to positive
    DbgLv(1) << "sedco-intcp **SEDC**" << sedc << "x0 y0 xn yn n" << xr[0]
             << yr[0] << xr[nscnu - 1] << yr[nscnu - 1] << nscnu;
...
void US_vHW_Enhanced::copy_data_files(QString plot1File, QString plot2File,
                                      QString data2File) {
  ...
  if (tfp1.exists()) {
    if (QFile(plot1File).remove()) DbgLv(1) << "CDF: removed:" << plot1File;
    if (tfp1.copy(plot1File)) DbgLv(1) << "CDF: copied:" << tplot1File;
  }

  if (tfp2.exists()) {
    if (QFile(plot2File).remove()) DbgLv(1) << "CDF: removed:" << plot2File;
    if (tfp2.copy(plot2File)) DbgLv(1) << "CDF: copied:" << tplot2File;
  }

  if (tfd2.exists()) {
    if (QFile(data2File).remove()) DbgLv(1) << "CDF: removed:" << data2File;
    if (tfd2.copy(data2File)) DbgLv(1) << "CDF: copied:" << tdata2File;
  }

  if (tfp3.exists()) {
    if (QFile(plot3File).remove()) DbgLv(1) << "CDF: removed:" << plot3File;
    if (tfp3.copy(plot3File)) DbgLv(1) << "CDF: copied:" << tplot3File;
  }

  if (tfp4.exists()) {
    if (QFile(plot4File).remove()) DbgLv(1) << "CDF: removed:" << plot4File;
    if (tfp4.copy(plot4File)) DbgLv(1) << "CDF: copied:" << tplot4File;
  }
}
...
bool US_vHW_Enhanced::fitted_plateaus() {
  ...
  if (!dataLoaded || vbar <= 0.0) return false;
  ...
  for (int jj = 0; jj < kf; jj++) {
    double xx = csx[jj];
    double yy =
        coefs[0] + coefs[1] * xx + coefs[2] * xx * xx + coefs[3] * xx * xx * xx;
    fsy[jj] = yy;
    DbgLv(1) << " k,fit-slope point: " << jj << xx << yy << " raw slope"
             << csy[jj];
  }
  ...
  for (int jj = 2; jj < kf; jj++) {
    ...
    if (currd == 0.0 || (currd > 0.0 && prevd < 0.0) ||
        (currd < 0.0 && prevd > 0.0)) {  // Zero point or zero crossing
      jrelp = jj - 1;
      ...
      if (dbg_level >= 1) {
        for (int mm = jj + 1; mm < kf; mm++) {
          ...
          DbgLv(1) << "  k cx cy cd pd" << mm << currx << curry << currd
                   << prevd;
        }
      }
      break;
    ...
    if (prevy < 0.0 &&
        curry < prevy) {  // Count consecutive decreases in negative slopes
      kdecr++;
      if (kdecr > 2) {  // 3rd consecutive decrease in negative slope, break at
                        // this point
        jrelp = jj;
  ...
  DbgLv(1) << " KRELP" << krelp << "   slope intcp sigma" << slope << intcp
           << sigma;
  ...
  for (int ii = 0; ii < lscnCount;
       ii++) {  // each extrapolated plateau is exp of Y for X of corrected time
    int js = liveScans[ii];
    dscan = &edata->scanData[js];
...
bool US_vHW_Enhanced::model_plateaus() {
  ...
  DbgLv(1) << "Mpl:ncmp" << ncomp << "scorr" << scorr
...
void US_vHW_Enhanced::vhw_calcs_standard(double* ptx, double* pty) {
  ...
  C0 = 0.0;
  Swavg = 0.0;
  edata = (ck_use_fed->isChecked() && have_model()) ? &dsimList[row]
                                                    : &dataList[row];

  div_seds();

  for (int ii = 0; ii < totalCount; ii++) pty[ii] = SEDC_NOVAL;
  ...
  for (int ii = 0; ii < lscnCount; ii++) {
    ...
    DbgLv(1) << "scn liv" << ii + 1 << kl << " radius concen time" << bdrad
             << bdcon << timev;
    ...
    for (int jj = 0; jj < divsCount;
         jj++) {  // walk through division points; get sed. coeff. by place in
                  // readings
      ...
      if (divrad > bdrad) {  // Mark a point to be excluded by back-diffusion
        sedc = SEDC_NOVAL;
        DbgLv(1) << "CPm: *excl* div" << jj + 1 << " drad dcon " << divrad
                 << mconc;
      }
...
void US_vHW_Enhanced::vhw_calcs_enhanced(double* ptx, double* pty) {
  ...
  while (iter <= mxiter) {
    ...
    for (int ii = 0; ii < lscnCount; ii++) {
      ...
      for (int jj = 0; jj < divsCount;
           jj++) {  // Recalculate partial concentrations based on sedcoeff
                    // intercepts
        sedc = (jj < divsUsed) ? dseds[jj] : SEDC_NOVAL;
        cpij =
            (sedc != SEDC_NOVAL) ? (c0term * exp(sedc * eterm)) : CPijs[ii][jj];
        CPijs[ii][jj] = cpij;
        sumcpij += cpij;
        // DbgLv(1) << "    div " << jj+1 << "  tcdps cpij " << tcpds.at(jj) <<
        // cpij;
      }
      ...
      for (int jj = 0; jj < divsCount; jj++) {  // Spread the difference to each
                                                // partial plateau concentration
        CPijs[ii][jj] += sdiff;
      }
...
void US_vHW_Enhanced::live_scans() {
  ...
  for (int ii = 0; ii < scanCount; ii++) {  // For non-excluded scans, save scan
                                            // index and plateau concentration
...
void US_vHW_Enhanced::init_partials() {
  ...
  for (int ii = 0; ii < lscnCount;
       ii++) {  // Populate partial concentration lists
...
void US_vHW_Enhanced::update_mid_concs() {
  ...
  for (int ii = 0; ii < lscnCount;
       ii++) {  // Populate mid-division concentration vector of each scan
    range = scPlats[ii] - baseline;
    bconc = baseline + range * positPct;  // Division 1 base conc.

    for (int jj = 0; jj < divsCount;
         jj++) {  // Determine new concentrations in divisions of the scan
...
void US_vHW_Enhanced::create_simulation() {
  ...
  int mm = nn / 2;
  DbgLv(1) << " cr.sim.:  sd 00 0m 0n l0 lm ln" << simraw.value(0, 0)
           << simraw.value(0, mm) << simraw.value(0, nn) << simraw.value(ll, 0)
           << simraw.value(ll, mm) << simraw.value(ll, nn);
...
void US_vHW_Enhanced::plot_data2() {
  if (!ck_use_fed->isChecked() ||
      !have_model()) {  // Normal experimental data plot
...
void US_vHW_Enhanced::get_model() {
  ...
  if (modelGUID.isEmpty() || modelGUID.length() != 36 ||
      (!mmodlGUID.isEmpty() &&
       mmodlGUID == modelGUID)) {  // No new model load if none exists or
                                   // already loaded matches noise
