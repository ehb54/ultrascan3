...
// distribution plot
void
US_vHW_Enhanced::distr_plot (void)
{
...
// data plot
void
US_vHW_Enhanced::data_plot (void)
{
  double xmax = 0.0;
  double ymax = 0.0;
  int count = 0;
    ...
    { // Walk thru divisions, fitting line to points from all scans
      ...
      if (count > 1)
        { // Fit a line to the scan points in a division
          ...
          if (jj < 3 || jj > (divsCount - 9))
            DbgLv (1) << "plot2 jj" << jj << "x0 y0 x1 y1" << x[0] << y[0]
                      << x[1] << y[1];
...
void
US_vHW_Enhanced::write_report (QTextStream &ts)
{
...
void
US_vHW_Enhanced::save_data (void)
{
  QApplication::setOverrideCursor (QCursor (Qt::WaitCursor));
...
void
US_vHW_Enhanced::view_report (void)
{
...
void
US_vHW_Enhanced::sel_groups (void)
{
  if (groupSel)
...
// update density
void
US_vHW_Enhanced::update_density (double dval)
{
  density = dval;
}
...
// update viscosity
void
US_vHW_Enhanced::update_viscosity (double dval)
{
  viscosity = dval;
}
...
// update vbar
void
US_vHW_Enhanced::update_vbar (double dval)
{
  vbar = dval;
}

void
US_vHW_Enhanced::update_bdtoler (double dval)
{
  bdtoler = dval;
...
void
US_vHW_Enhanced::update_divis (double dval)
{
...
int
US_vHW_Enhanced::first_gteq (double concenv, QVector<double> &rvalues,
...
double
US_vHW_Enhanced::sed_coeff (double cconc, double oterm, double *radP,
                            int *ndxP)
{
  //*TIMING
  kcalls[16] += 1;
    ...
    { // Loop to fit points across scans in a division
      ...
      for (int kk = 0; kk < nscnu; kk++)
        { // Accumulate concentration, sed.coeff. for all scans, this div
          ...
          if (radC > radD)
            { // Gone beyond back-diffusion cutoff: exit loop with truncated
              // list
              kscnu = kk;
              yy[kk] = SEDC_NOVAL;
      ...
      for (int kk = 0; kk < kscnu; kk++)
        { // Remove any leading points below meniscus
          if (yy[kk] != SEDC_NOVAL)
            ...
            DbgLv (1) << "KS +++ SED<=0.0 divjj" << jj << "scnkk ii" << kk
                      << ii;
        }
      kscnu = ii;
      ...
      if (kscnu > 1)
        {
          ...
          DbgLv (1) << " KS" << kscnu << kscsv << "jj" << jj
                    << "xx0 xxn yy0 yyn" << xt[0] << xt[kscnu - 1] << yt[0]
      ...
      else
        {
          ...
          slope = 0.0;
          ii = (kscnu < 1) ? 1 : kscnu;
          DbgLv (1) << "    kscnu" << kscnu << "yy0 yyn " << yy[0]
                    << yy[ii - 1];
          continue;
        }
      ...
      dcors << corre;
      dpnts << kscnu;
      DbgLv (1) << "JJ" << jj << "DSED" << dsed << "dseds size"
                << dseds.size ();
      //*TIMING
...
double
US_vHW_Enhanced::back_diff_coeff (double sedc)
{
...
void
US_vHW_Enhanced::groupClick (const QwtDoublePoint &p)
{
...
void
US_vHW_Enhanced::add_group_info ()
{
...
QString
US_vHW_Enhanced::text_time (double seconds, int type)
{
...
QString
US_vHW_Enhanced::text_time (double seconds)
...
QStringList
US_vHW_Enhanced::last_edit_files (QStringList files)
{
  QStringList ofiles;
  QStringList part;
  QString file;
...
void
US_vHW_Enhanced::new_triple (int row)
{
...
void
US_vHW_Enhanced::update (int row)
{
  haveZone = false;
  edata = &dataList[row];
  //*TIMING
...
double
US_vHW_Enhanced::sedcoeff_intercept ()
{
  //*TIMING
  kcalls[2] += 1;
  QDateTime sttime = QDateTime::currentDateTime ();
...
void
US_vHW_Enhanced::copy_data_files (QString plot1File, QString plot2File,
                                  QString data2File)
{
...
bool
US_vHW_Enhanced::fitted_plateaus ()
{
  //*TIMING
  kcalls[3] += 1;
  QDateTime sttime = QDateTime::currentDateTime ();
  ...
  for (int jj = 2; jj < kf; jj++)
    {
      ...
      // DbgLv(1) << "  k cx cy cd pd" << jj << currx << curry << currd <<
      // prevd;
      DbgLv (1) << "  k" << jj << " x fslo" << currx << curry << " deriv"
                << currd;
              ...
              for (int mm = jj + 1; mm < kf; mm++)
                {
                  ...
                  DbgLv (1) << "  k cx cy cd pd" << mm << currx << curry
                            << currd << prevd;
                }
            }
          break;
      ...
      if (prevy < 0.0 && curry < prevy)
        { // Count consecutive decreases in negative slopes
          kdecr++;
          if (kdecr > 2)
            { // 3rd consecutive decrease in negative slope, break at this
              // point
              jrelp = jj;
              DbgLv (1) << "    KDECR>2  jrelp prevy curry" << jrelp << prevy
                        << curry;
              break;
            }
        }
...
bool
US_vHW_Enhanced::model_plateaus ()
{
  //*TIMING
  kcalls[4] += 1;
  QDateTime sttime = QDateTime::currentDateTime ();
...
void
US_vHW_Enhanced::vhw_calcs_standard (double *ptx, double *pty)
{
  //*TIMING
  kcalls[5] += 1;
  QDateTime sttime = QDateTime::currentDateTime ();
        ...
        { // walk through division points; get sed. coeff. by place in readings
          ...
          if (divrad > bdrad)
            { // Mark a point to be excluded by back-diffusion
              sedc = SEDC_NOVAL;
              DbgLv (1) << "CPm: *excl* div" << jj + 1 << " drad dcon "
                        << divrad << mconc;
            }
...
void
US_vHW_Enhanced::vhw_calcs_enhanced (double *ptx, double *pty)
{
  //*TIMING
  kcalls[6] += 1;
  QDateTime sttime = QDateTime::currentDateTime ();
      ...
      for (int ii = 0; ii < lscnCount; ii++)
        {
          ...
          for (int jj = 0; jj < divsCount; jj++)
            { // Recalculate partial concentrations based on sedcoeff
              // intercepts
              sedc = (jj < divsUsed) ? dseds[jj] : SEDC_NOVAL;
              cpij = (sedc != SEDC_NOVAL) ? (c0term * exp (sedc * eterm))
                                          : CPijs[ii][jj];
              CPijs[ii][jj] = cpij;
              sumcpij += cpij;
              // DbgLv(1) << "    div " << jj+1 << "  tcdps cpij " <<
              // tcpds.at(jj) << cpij;
            }
          ...
          DbgLv (1) << "   iter scn " << iter << ii + 1 << " sumcpij span "
                    << sumcpij << span << "  sdiff sumabsdif" << sdiff
                    << avgdif;
        }
      ...
      for (int jj = 0; jj < divsCount; jj++)
        { // walk through division points; get sed. coeff. by place in readings
          ...
          if (divrad > bdrad)
            { // Mark a point to be excluded by back-diffusion
              sedc = SEDC_NOVAL;
              DbgLv (1) << " *excl* div" << jj + 1 << " drad dcon " << divrad
                        << mconc;
            }
...
// Plot any upper plot vertical excluded-scan lines
void
US_vHW_Enhanced::vert_exclude_lines ()
{
  double ymax;
  double xx[2];
  double yy[2];
...
void
US_vHW_Enhanced::exclude_from (double from)
{
...
void
US_vHW_Enhanced::exclude_to (double to)
{
  DbgLv (1) << "(1)TO=" << to;
...
void
US_vHW_Enhanced::update_mid_concs ()
{
  //*TIMING
  kcalls[10] += 1;
  QDateTime sttime = QDateTime::currentDateTime ();
    ...
    { // Populate mid-division concentration vector of each scan
      ...
      for (int jj = 0; jj < divsCount; jj++)
        { // Determine new concentrations in divisions of the scan
          ...
          // DbgLv(1) << "UPD: mconc cpij" << mconc << cpij << "ii jj" << ii <<
          // jj;
        }
    }
  //*TIMING
...
void
US_vHW_Enhanced::create_simulation ()
{
  row = lw_triples->currentRow ();
  expda = &dataList[row];
  simda = &dsimList[row];
  ...
  DbgLv (1) << " cr.sim.:  sd 00 0m 0n l0 lm ln" << simraw.value (0, 0)
            << simraw.value (0, mm) << simraw.value (0, nn)
            << simraw.value (ll, 0) << simraw.value (ll, mm)
            << simraw.value (ll, nn);
