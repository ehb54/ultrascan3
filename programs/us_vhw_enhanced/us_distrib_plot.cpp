...
US_DistribPlot::US_DistribPlot(QVector<double> &divfracs, QVector<double> &divsedcs, const double tconc) :
   ...
   QBoxLayout *plot = new US_Plot(
      data_plot, tr("Sedimentation Coefficient Distribution"), tr("Sedimentation Coefficient"), tr("Boundary Fraction"));
   ...
   connect(pb_plot_type, SIGNAL(clicked()), this, SLOT(type_plot()));
   connect(pb_help, SIGNAL(clicked()), this, SLOT(help()));
   connect(pb_close, SIGNAL(clicked()), this, SLOT(save_and_close()));
   connect(pb_histogram, SIGNAL(clicked()), this, SLOT(hide_histo()));
   connect(pb_envelope, SIGNAL(clicked()), this, SLOT(hide_envel()));
   connect(ct_sensitivity, SIGNAL(valueChanged(double)), this, SLOT(change_sensit(double)));
   connect(ct_smoothing, SIGNAL(valueChanged(double)), this, SLOT(change_smooth(double)));
}
...
void US_DistribPlot::save_plots(QString &plot1File, QString &plot2File) {
   ...
   QString dat2File = US_Settings::resultDir() + "/" + runID + "/" + plot1File.section("/", -1, -1).section(".", 0, 1)
                      + ".s-c-envelope.csv";
...
void US_DistribPlot::type_plot(void) {
   if (plotType == DISTR) { // change to histogram plot
      plotType = plotTypeH;
   ...
   else { // change to distribution plot
      plotType = DISTR;
...
void US_DistribPlot::plot_distrib(void) {
   // Set up the axes
   data_plot->setAxisTitle(QwtPlot::yLeft, tr("Boundary Fraction"));
   data_plot->setAxisTitle(QwtPlot::xBottom, tr("Sedimentation Coefficient"));
   data_plot->setAxisScale(QwtPlot::yLeft, 0.0, 100.0, 20.0);
...
void US_DistribPlot::plot_histogram(void) {
   ...
   // Set up the axes
   data_plot->setAxisTitle(QwtPlot::yLeft, tr("Relative Concentration"));
   data_plot->setAxisTitle(QwtPlot::xBottom, tr("Sedimentation Coefficient"));
...
void US_DistribPlot::plot_envelope(void) {
   ...
   if (plotType == ENVEL) { // if envelope only, must set up axes (otherwise handled by histo)
      data_plot->setAxisTitle(QwtPlot::yLeft, tr("Relative Concentration"));
      data_plot->setAxisTitle(QwtPlot::xBottom, tr("Sedimentation Coefficient"));
...
int US_DistribPlot::histo_data(QVector<double> &xvec, QVector<double> &yvec) {
   int steps;
   int stoff = 0;
   ...
   for (int jj = 0; jj < divsCount; jj++) { // get min,max intercept sedimentation coefficients
   ...
   for (int jj = 0; jj < steps; jj++) { // accumulate histogram values
      int kbin = 0;
      ...
      for (int kk = 0; kk < divsCount; kk++) { // accumulate count of sedcoeff values in current bin
         sedc = dsedcs.at(kk);
...
int US_DistribPlot::envel_data(QVector<double> &xvec, QVector<double> &yvec) {
   int steps;
   int array = 300;
   double max_cept = 1.0e-6;
   double min_cept = 1.0e+6;
   ...
   if (array <= steps) { // insure envelope array size bigger than histogram array size
      array = steps + 1;
   }
   ...
   for (int jj = 0; jj < array; jj++) { // initialize envelope values
   ...
   for (int jj = 0; jj < steps; jj++) { // calculate histogram values and envelope values based on them
      ...
      for (int kk = 0; kk < divsCount; kk++) { // count sedcoeffs within current step range
         sedc = dsedcs.at(kk);
   ...
   for (int kk = 0; kk < array; kk++) { // accumulate the envelope values sum
      env_sum += yval[ kk ];
   }
   ...
   for (int kk = 0; kk < array; kk++) { // normalize Y values
      yval[ kk ] *= scale;
   }
