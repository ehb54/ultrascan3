...
US_DistribPlot::US_DistribPlot(QVector<double>& divfracs,
  ...
  // Plot Rows
  QBoxLayout* plot =
      new US_Plot(data_plot, tr("Sedimentation Coefficient Distribution"),
                  tr("Sedimentation Coefficient"), tr("Boundary Fraction"));
  ...
  connect(ct_sensitivity, SIGNAL(valueChanged(double)), this,
          SLOT(change_sensit(double)));
  connect(ct_smoothing, SIGNAL(valueChanged(double)), this,
          SLOT(change_smooth(double)));
}
...
void US_DistribPlot::plot_histogram(void) {
  ...
  for (int jj = 0; jj < npoints; jj++) esum += yy[jj];
  ...
  for (int jj = 0; jj < npoints; jj++) yy[jj] *= yscl;
...
void US_DistribPlot::plot_envelope(void) {
  ...
  if (plotType == ENVEL) {  // if envelope only, must set up axes (otherwise
                            // handled by histo)
    data_plot->setAxisTitle(QwtPlot::yLeft, tr("Relative Concentration"));
    data_plot->setAxisTitle(QwtPlot::xBottom, tr("Sedimentation Coefficient"));
...
int US_DistribPlot::histo_data(QVector<double>& xvec, QVector<double>& yvec) {
  ...
  for (int jj = 0; jj < steps; jj++) {  // accumulate histogram values
    ...
    for (int kk = 0; kk < divsCount;
         kk++) {  // accumulate count of sedcoeff values in current bin
      sedc = dsedcs.at(kk);
  ...
  for (int jj = 0; jj < steps;
    ...
    if (kbin > 0) {  // if non-empty bin, update envelope Y values
      for (int kk = 0; kk < array; kk++) {
        ...
        DbgLv(2) << "ED:  kk" << kk << "xdif bink yval" << xdif << bink
                 << yval[kk];
      }
    }
...
DbgLv(1) << "SaveDat: tot_conc tconc" << tot_conc << tconc
 << "tcpwr" << tcpwr;
  ...
  for (int ii = 0; ii < nepts; ii++) {
    ...
    if (ii < nhpts)
      line =
          QString().sprintf("\"%.6f\",\"%.6f\",\"%.6f\",\"%9.2f\",\"%9.4e\"\n",
                            eseds[ii], efrqs[ii], hseds[ii], hfrqs[ii], tconc);
    else
