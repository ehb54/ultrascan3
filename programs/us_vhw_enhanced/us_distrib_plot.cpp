    ...
    : US_WidgetsDialog (0, 0), bfracs (divfracs), dsedcs (divsedcs)
{
  ...
  QBoxLayout *plot = new US_Plot (
      data_plot, tr ("Sedimentation Coefficient Distribution"),
...
void
US_DistribPlot::save_plots (QString &plot1File, QString &plot2File)
{
...
void
US_DistribPlot::hide_histo (void)
{
...
void
US_DistribPlot::hide_envel (void)
{
  bool isHis = (plotType == HISTO || plotType == COMBO);
  bool isEnv = (plotType == ENVEL || plotType == COMBO);
...
void
US_DistribPlot::change_sensit (double value)
{
  nSensit = (int)value;
...
void
US_DistribPlot::change_smooth (double value)
{
  nSmooth = (int)value;
...
void
US_DistribPlot::plot_distrib (void)
{
  // Set up the axes
...
// plot histogram
void
US_DistribPlot::plot_histogram (void)
{
  QVector<double> xvec;
  QVector<double> yvec;
...
// plot envelope
void
US_DistribPlot::plot_envelope (void)
{
  QVector<double> xvec;
  QVector<double> yvec;
  double *xx;
...
void
US_DistribPlot::plot_combined (void)
{
  plot_histogram ();
  plot_envelope ();
}
...
int
US_DistribPlot::histo_data (QVector<double> &xvec, QVector<double> &yvec)
{
  int steps;
  int stoff = 0;
...
int
US_DistribPlot::envel_data (QVector<double> &xvec, QVector<double> &yvec)
{
  int steps;
  int array = 300;
  double max_cept = 1.0e-6;
...
void
US_DistribPlot::save_data_file (QString data2File)
{
  ...
  for (int ii = 0; ii < nepts; ii++)
    {
      ...
      if (ii < nhpts)
        line = QString ().sprintf (
            "\"%.6f\",\"%.6f\",\"%.6f\",\"%9.2f\",\"%9.4e\"\n", eseds[ii],
            efrqs[ii], hseds[ii], hfrqs[ii], tconc);
      else
