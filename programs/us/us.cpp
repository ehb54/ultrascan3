...
int main(int argc, char* argv[]) {
  ...
  if (result != US_License_t::OK) {
    ...
    QPushButton* Register = mBox.addButton(
        qApp->translate("UltraScan III", "Register"), QMessageBox::ActionRole);
  ...
  if (DB_list.size() > 0 && defaultDB.size() > 0) {
    ...
    if (db.lastErrno() != US_DB2::OK) {
      ...
      msgBox.setText(
          "Error making the DB connection! User-level cannot be synchronized.");
...
void US_Action::onTriggered(bool) { emit indexTriggered(index); }
/////////////US_Win
US_Win::US_Win(QWidget* parent, Qt::WindowFlags flags)
  ...
  if (!g.isValid()) {
    ...
    QMessageBox::critical(
        this, windowTitle(),
        QString(tr("There is a error connecting to global memory:\n"
                   "\"%1\"\n"
  ...
  if (!US_Settings::status().isEmpty()) {
    ...
    QMessageBox::critical(
        this, windowTitle(),
        QString(
            tr("There is a error with your settings file:\n"
               "\"%1\"\n"
  ...
  addMenu(P_DMGAINIT, tr("Initialize Discrete Model &Genetic Algorithm (DMGA)"),
          velocity);
...
//  fit        ->setFont( mfont );
  ...
  if (US_Settings::default_data_location() != 2) {
    ...
    connect(&notices_get_url, SIGNAL(downloaded()), this, SLOT(notices_ready()),
            Qt::UniqueConnection);
...
void US_Win::terminated(int code, QProcess::ExitStatus status) {
  ...
  for (pr = procs.begin(); pr != procs.end(); pr++) {
    ...
    if (process->state() == QProcess::NotRunning) {
      ...
      if (status == QProcess::CrashExit) {  // Process crashed:  output a
                                            // message and possibly do a pop-up
        QString msg_sb;
        QString msg_mb;
...
void US_Win::launch(int index) {
  ...
  if (!QFile(procapp).exists()) procapp = procbin;
...
void US_Win::logo(int width) {
  ...
  QString version = "Version " + US_Version +
                    " ( " REVISION
...
void US_Win::apply_prefs() {
  ...
  for (int ii = 0; ii < mens.count();
       ii++) {  // loop to get and reset actions font for each menu
    QList<QAction*> acts = mens.at(ii)->findChildren<QAction*>();

    for (int jj = 0; jj < acts.count();
         jj++) {  // reset the font for each submenu action
...
//    information QDateTime pn_time = ln_time;                       // Previous
//    check time ln_time           = QDateTime::currentDateTime();  // Reset
//    last notice time
...
void US_Win::notices_ready() {
  ...
  if (parseError.error != QJsonParseError::NoError) {
    qWarning() << "notices_ready(): JSON parse error at " << parseError.offset
  ...
  for (auto it = notices_jarray.constBegin(); it != notices_jarray.constEnd();
       ++it) {
    const QJsonObject obj = it->toObject();
    if (!obj.contains("message") || !obj.contains("revision") ||
        !obj.contains("type")) {
      qWarning() << "notices_ready(): JSON array element does not contain "
                    "message, type & revision keys, skipping";
      continue;
    }
  ...
  if (do_abort) {  // Append an additional note if an abort is happening
