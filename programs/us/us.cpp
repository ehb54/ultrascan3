...
int main(int argc, char *argv[]) {
   ...
   if (result != US_License_t::OK) {
      ...
      QPushButton *Register = mBox.addButton(qApp->translate("UltraScan III", "Register"), QMessageBox::ActionRole);
...
//////////////US_Action
US_Action::US_Action(int i, const QString &text, QObject *parent) : QAction(text, parent), index(i) {
...
/////////////US_Win
US_Win::US_Win(QWidget *parent, Qt::WindowFlags flags) : QMainWindow(parent, flags) {
  ...
  addMenu(  P_CEXPERI  , tr( "Create E&xperiment"                ), utilities );
   ...
   addMenu(P_MODEL1, tr("&Model s, D and f from MW for 4 basic shapes"), simulation);
   addMenu(P_MODEL2, tr("&Predict f and axial ratios for 4 basic shapes"), simulation);
...
#ifndef Q_OS_MAC
   QFont bfont = QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() - 1, QFont::Bold);
...
#ifndef Q_OS_MAC
   QFont mfont = QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() - 1, QFont::Normal);
   file->setFont(mfont);
   edit->setFont(mfont);
...
#ifndef Q_OS_MAC
   QFont font = QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() - 1, QFont::Normal);
   action->setFont(font);
#endif

   connect(action, SIGNAL(indexTriggered(int)), this, SLOT(onIndexTriggered(int)));
...
void US_Win::terminated(int code, QProcess::ExitStatus status) {
   ...
   for (pr = procs.begin(); pr != procs.end(); pr++) {
      ...
      if (process->state() == QProcess::NotRunning) {
         ...
         if (status == QProcess::CrashExit) { // Process crashed:  output a message and possibly do a pop-up
            QString msg_sb;
            QString msg_mb;
            QString msg_ru = p[ index ].runningMsg;
            QString estderr = QString(process->readAllStandardError()).right(2000); // End lines of STDERR
...
void US_Win::launch(int index) {
   ...
   if (p[ index ].maxRunCount <= p[ index ].currentRunCount && p[ index ].maxRunCount > 0) {
      if (p[ index ].index == P_CONFIG) {
         ...
         QMessageBox::warning(this, tr("Error"), pname + tr(" is already running."));
      }
   ...
   connect(process, SIGNAL(finished(int, QProcess::ExitStatus)), this, SLOT(terminated(int, QProcess::ExitStatus)));
   ...
   {
      QMessageBox::information(this, tr("Error"), tr("There was a problem creating a subprocess\nfor ") + pname);
   }
   else {
      p[ index ].currentRunCount++;
...
void US_Win::logo(int width) {
   ...
   QString version = "Version " + US_Version + " ( " REVISION " ) for " OS_TITLE; // REVISION is #define "Revision: xxx"
...
void US_Win::apply_prefs() {
   ...
   for (int ii = 0; ii < mens.count(); ii++) { // loop to get and reset actions font for each menu
      QList<QAction *> acts = mens.at(ii)->findChildren<QAction *>();

      for (int jj = 0; jj < acts.count(); jj++) { // reset the font for each submenu action
...
void US_Win::notices_ready() {
   ...
   for (auto it = notices_jarray.constBegin(); it != notices_jarray.constEnd(); ++it) {
   ...
   for (int ii = 0; ii < ( int ) msgs.size(); ++ii) {
      double msg_version = QString("%1").arg(revs[ ii ]).replace(QRegularExpression("\\.\\d+$"), "").toDouble();
      double msg_revision
         = QString("%1").arg(revs[ ii ]).replace(QRegularExpression("^[^\\.]*\\.\\d+\\."), "").toDouble();

      // Skip messages where message version.revision is less than our version.revision
      if (sys_version > msg_version || (sys_version == msg_version && sys_revision > msg_revision)) {
         continue;
      }

      // Add current message to full text
      msg_note += typeMap[ types[ ii ] ] + " for release " + revs[ ii ] + ":<br>" + msgs[ ii ] + "<br>";
