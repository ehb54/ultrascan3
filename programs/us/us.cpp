...
void US_Win::splash( void )
{
    ...
    bigframe->setGeometry          ( 0, y, width, height );
    bigframe->setAutoFillBackground( true );
    bigframe->lower(); // Ensure it's below menubar/statusbar
...
void US_Win::logo( int width )
{
    ...
    // BUILD_DATE and BUILD_TIME may already contain "UTC", so use a simpler format
    QString buildDate = QString(BUILD_DATE).trimmed();
    QString buildTime = QString(BUILD_TIME).trimmed();

    // Remove redundant "UTC" from date if present, we'll add it once at the end
    buildDate.replace(" UTC", "");
    buildTime.replace(" UTC", "");

    const QString builtOn = QString("Built on %1 at %2 UTC")
            .arg(buildDate).arg(buildTime);
    ...
    smallframe->setGeometry(0, 0, availW, availH);
    smallframe->show();
}
...
void US_Win::resizeEvent(QResizeEvent* event)
{
    QMainWindow::resizeEvent(event);

    // Update splash screen if it's currently visible
    if (smallframe && smallframe->isVisible()) {
    ...
    // Update bigframe if it exists
    if (bigframe) {
        const int y = menuBar()->size().rheight();
        const int height = this->height() - y - statusBar()->size().rheight();
        bigframe->setGeometry(0, y, width(), height);
    }
}
