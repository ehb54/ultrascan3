  ...
  if (result != US_License_t::OK)
    {
      ...
      QPushButton *Register
          = mBox.addButton (qApp->translate ("UltraScan III", "Register"),
                            QMessageBox::ActionRole);

      mBox.setDefaultButton (Register);
      mBox.setWindowTitle (
          qApp->translate ("UltraScan III", "UltraScan License Problem"));
      ...
      if (db.lastErrno () != US_DB2::OK)
        {
          ...
          msgBox.setText ("Error making the DB connection! User-level cannot "
                          "be synchronized.");
      ...
      else
        {
          ...
          qDebug () << "USCFG: UpdInv: ID,name,lev" << ID << fname << lname
                    << level;
          // if(ID<1) return;
    ...
    : QMainWindow (parent, flags)
{
  ...
  if (!g.isValid ())
    {
      ...
      QMessageBox::critical (
          this, windowTitle (),
          QString (
              tr ("There is a error connecting to global memory:\n"
                  "\"%1\"\n"
  ...
  if (!US_Settings::status ().isEmpty ())
    {
      ...
      QMessageBox::critical (
          this, windowTitle (),
          QString (tr ("There is a error with your settings file:\n"
                       "\"%1\"\n"
                       "This can cause various problems and should be "
                       "corrected.\n"))
  ...
  addMenu (P_DMGAINIT,
           tr ("Initialize Discrete Model &Genetic Algorithm (DMGA)"),
           velocity);
  addMenu (P_PCSA, tr ("&Parametrically Constrained Spectrum Analysis (PCSA)"),
           velocity);
  ...
  addMenu (P_LEGDATA, tr ("&Convert Optima Data (Beckman tar.gz) "),
           utilities);
  ...
  // addMenu(  P_GMPRPT   , tr( "&GMP Report Generator and Viewer"  ),
  // utilities );
  ...
  addMenu (P_EQUILTIMESIM, tr ("Estimate Equilibrium &Times"), simulation);
...
//  menuBar()->addMenu( fit         );
  ...
  QFont mfont = QFont (US_GuiSettings::fontFamily (),
                       US_GuiSettings::fontSize () - 1, QFont::Normal);
  file->setFont (mfont);
  edit->setFont (mfont);
  ...
  if (US_Settings::default_data_location () != 2)
    {
      ...
      connect (&notices_get_url, SIGNAL (downloaded ()), this,
               SLOT (notices_ready ()), Qt::UniqueConnection);
...
void
US_Win::addMenu (int index, const QString &label, QMenu *menu)
{
  ...
  connect (action, SIGNAL (indexTriggered (int)), this,
           SLOT (onIndexTriggered (int)));

  menu->addAction (action);
}

void
US_Win::onIndexTriggered (int index)
{
  if (index == 4)
    close ();

  // qDebug() << index << P_CONFIG << P_END;
  if (index >= P_CONFIG && index < P_END)
    launch (index);
  if (index >= HELP && index < HELP_END)
    help (index);
}

void
US_Win::terminated (int code, QProcess::ExitStatus status)
{
  ...
  for (pr = procs.begin (); pr != procs.end (); pr++)
    {
      ...
      if (process->state () == QProcess::NotRunning)
        {
          qDebug () << "PROCESS NotRunning index" << index << "Proc name"
                    << d->name;
          ...
          if (status == QProcess::CrashExit)
            { // Process crashed:  output a message and possibly do a pop-up
              ...
              qDebug () << "PROCESS   status" << status << "e-stderr len"
                        << estderr.length ();
                ...
                { // It was other than bad_alloc
                  ...
                  if (kwhat > 0)
                    { // We have a "what():" hint in stderr
                      QString msg_wh
                          = estderr.mid (kwhat + 7, 20).simplified ();
                      msg_sb = "[ " + msg_wh + " ] crash: " + msg_ru;
                      msg_mb
                          = tr ("Process Crash [ ") + msg_wh + " ]\n" + msg_ru;
                    }
...
US_Win::launch (int index)
{
  ...
  statusBar ()->showMessage (tr ("Loading ") + p[index].runningMsg + "...");
  ...
  statusBar ()->showMessage (tr ("Loaded ") + p[index].runningMsg + "...");
}

void
US_Win::closeProcs (void)
{
  QString names;
  ...
  box.setText (
      QString (tr ("The following process%1 still active:\n\n%2"
...
void
US_Win::closeEvent (QCloseEvent *e)
{
  if (!procs.isEmpty ())
    closeProcs ();
  e->accept ();
    ...
    case HELP_ABOUT:
      QMessageBox::information (
          ...
          tr ("UltraScan III version %1\n"
              ...
              "Phone:  (406) 285-1935\n"
              "E-mail: borries.demeler@umontana.edu")
              .arg (US_Version)
              .arg (REVISION));
...
//    int  level        = 0;                             // Max level:
//    information QDateTime pn_time = ln_time;                       //
//    Previous check time ln_time           = QDateTime::currentDateTime();  //
//    Reset last notice time

//    if ( US_Settings::default_data_location() == 2 )
//       return do_abort;      // If default data location is Disk, do not
//       bother
...
//    {
//       double msg_version  = QString( "%1" ).arg( revs[ii] ).replace(
//       QRegularExpression( "\\.\\d+$" ), "" ).toDouble(); double msg_revision
//       = QString( "%1" ).arg( revs[ii] ).replace( QRegularExpression(
//       "^[^\\.]*\\.\\d+\\." ), "" ).toDouble();

//       // // Skip messages for warn/crit same revision or any earlier than
//       current
//       // if ( ( irevs[ ii ] == s_rev  &&  types[ ii ] != "info" )  ||
//       //      irevs[ ii ] < s_rev )     continue;

//       // Skip messages where message version.revision is less than our
//       version.revision if ( sys_version > msg_version ||
...
US_Win::notices_ready ()
{
  ...
  if (parseError.error != QJsonParseError::NoError)
    {
      qWarning () << "notices_ready(): JSON parse error at "
  ...
  for (auto it = notices_jarray.constBegin ();
       it != notices_jarray.constEnd (); ++it)
    {
      const QJsonObject obj = it->toObject ();
      if (!obj.contains ("message") || !obj.contains ("revision")
          || !obj.contains ("type"))
        {
          qWarning () << "notices_ready(): JSON array element does not "
                         "contain message, type & revision keys, skipping";
          continue;
        }
      ...
      if (types.back () == "info")
        nn_info++;
      else if (types.back () == "warn")
        nn_warn++;
      else if (types.back () == "crit")
        nn_crit++;
    }
  ...
  for (int ii = 0; ii < (int)msgs.size (); ++ii)
    {
      double msg_version = QString ("%1")
                               .arg (revs[ii])
                               .replace (QRegularExpression ("\\.\\d+$"), "")
                               .toDouble ();
      double msg_revision
          = QString ("%1")
                .arg (revs[ii])
      ...
      if (sys_version > msg_version
          || (sys_version == msg_version && sys_revision > msg_revision))
        {
          continue;
        }

      // Add current message to full text
      msg_note += typeMap[types[ii]] + " for release " + revs[ii] + ":<br>"
                  + msgs[ii] + "<br>";
