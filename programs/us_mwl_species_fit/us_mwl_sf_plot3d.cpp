...
US_MWL_SF_PLOT3D::US_MWL_SF_PLOT3D (QWidget *w, const SFData &spFitData)
    : US_WidgetsDialog (w)
{
  ...
  scale = spFitData.wavelenghts.at (nWavelengths - 1)
          - spFitData.wavelenghts.at (0);
  lambdaScaled.clear ();
  lambda4ct.clear ();
  ...
  QGridLayout *plotAbs_gl
      = us_checkbox ("Plot Magnitude of Residuals", ckb_plotAbs, false);
  ...
  camera_list << "LeftBelow" << "RightBelow" << "BehindBelow"
              << "DirectlyBelow";
  camera_list << "DirectlyAbove" << "DirectlyAboveCW45"
              << "DirectlyAboveCCW45";
  ...
  US_Plot *us_dataPlot = new US_Plot (
      dataPlot, tr (""), tr ("Wavelength (nm)"), tr ("Absorbance"));
  ...
  connect (cb_scan, SIGNAL (currentIndexChanged (int)), this,
           SLOT (newScan (int)));
  connect (pb_next, SIGNAL (clicked ()), this, SLOT (nextScan ()));
  connect (pb_prev, SIGNAL (clicked ()), this, SLOT (prevScan ()));
  connect (cb_camera, SIGNAL (currentIndexChanged (int)), this,
           SLOT (resetCamera (int)));
  connect (cb_theme, SIGNAL (currentTextChanged (QString)), this,
           SLOT (setTheme (QString)));
  ...
  connect (ct_min_rp, SIGNAL (valueChanged (double)), this,
           SLOT (adjustRpMin (double)));
  connect (ct_max_rp, SIGNAL (valueChanged (double)), this,
           SLOT (adjustRpMax (double)));
  connect (ct_min_wl, SIGNAL (valueChanged (double)), this,
           SLOT (adjustWlMin (double)));
  connect (ct_max_wl, SIGNAL (valueChanged (double)), this,
           SLOT (adjustWlMax (double)));
  ...
  connect (rb_radial, SIGNAL (toggled (bool)), this,
           SLOT (toggleRadial (bool)));
  connect (rb_lambda, SIGNAL (toggled (bool)), this,
           SLOT (toggleLambda (bool)));
  connect (sli_xAngle, SIGNAL (valueChanged (int)), this,
           SLOT (new_xAngle (int)));
  connect (sli_yAngle, SIGNAL (valueChanged (int)), this,
           SLOT (new_yAngle (int)));
  connect (sli_zAngle, SIGNAL (valueChanged (int)), this,
           SLOT (new_zAngle (int)));
  ...
  connect (ckb_rendall, SIGNAL (stateChanged (int)), this,
           SLOT (render_option (int)));
  connect (camera, SIGNAL (xRotationChanged (float)), this,
           SLOT (cameraChanged (float)));
  connect (camera, SIGNAL (yRotationChanged (float)), this,
           SLOT (cameraChanged (float)));
...
US_MWL_SF_PLOT3D::~US_MWL_SF_PLOT3D () { delete graph; }

void
US_MWL_SF_PLOT3D::setTheme (const QString theme)
{
  if (theme == "Qt")
  ...
  if (ckb_rendall->isChecked ())
    {
      QString dir = QFileDialog::getExistingDirectory (
          this, tr ("Choose a Directory"), US_Settings::resultDir (),
      ...
      if (state == QDialog::Rejected)
      ...
      else if (state == QDialog::Accepted)
        {
          ...
          QColor color = US_GuiSettings::pushbColor ().color (
  ...
  else
    {
      QString fileName = QFileDialog::getSaveFileName (
          this, tr ("Save Image (*.png *.xpm *.jpg)"),
          US_Settings::resultDir (),
          tr ("Images (*.bmp *.jpg *.jpeg *.png *.ppm *.xbm *.xpm)"));
      if (fileName.size () == 0)
        return;
      QImage image
          = graph->renderToImage (0, graph->size () * ct_scale->value ());
      int quality = qRound (ct_quality->value ());
      ...
      for (int i = idRP_l; i <= idRP_h; i++)
        {
          ...
          (*newRow)[index++].setPosition (
              QVector3D ((float)rp, (float)data, (float)wl));
        }
      *dataArray << newRow;
    }
...
void
US_MWL_SF_PLOT3D::newScan (int id)
{
  scanId = id;
  plot2d ();
  plot3d ();
...
void
US_MWL_SF_PLOT3D::nextScan ()
{
  cb_scan->setCurrentIndex (scanId + 1);
}

void
US_MWL_SF_PLOT3D::prevScan ()
{
  cb_scan->setCurrentIndex (scanId - 1);
}

void
US_MWL_SF_PLOT3D::resetCamera (int)
{
  camera->disconnect ();
...
void
US_MWL_SF_PLOT3D::set_ct_rp (bool ascent)
{
...
void
US_MWL_SF_PLOT3D::adjustRpMin (double)
{
  set_ct_rp (true);
}

void
US_MWL_SF_PLOT3D::adjustRpMax (double)
{
  set_ct_rp (false);
}

void
US_MWL_SF_PLOT3D::set_ct_wl (bool ascent)
{
...
void
US_MWL_SF_PLOT3D::adjustWlMin (double)
{
  set_ct_wl (true);
}

void
US_MWL_SF_PLOT3D::adjustWlMax (double)
{
  set_ct_wl (false);
}

void
US_MWL_SF_PLOT3D::toggleNone (bool)
{
  graph->setSelectionMode (QAbstract3DGraph::SelectionNone);
}
void
US_MWL_SF_PLOT3D::togglePoint (bool)
{
  graph->setSelectionMode (QAbstract3DGraph::SelectionItem);
}
void
US_MWL_SF_PLOT3D::toggleLambda (bool)
{
  graph->setSelectionMode (QAbstract3DGraph::SelectionItemAndColumn
                           | QAbstract3DGraph::SelectionSlice);
}
void
US_MWL_SF_PLOT3D::toggleRadial (bool)
{
...
void
US_MWL_SF_PLOT3D::new_xAngle (int value)
{
  graph->axisX ()->setLabelAutoRotation (value);
}

void
US_MWL_SF_PLOT3D::new_yAngle (int value)
{
  graph->axisY ()->setLabelAutoRotation (value);
}

void
US_MWL_SF_PLOT3D::new_zAngle (int value)
{
  graph->axisZ ()->setLabelAutoRotation (value);
}

void
US_MWL_SF_PLOT3D::reset_xAngle ()
{
  sli_xAngle->setValue (xAngle);
}

void
US_MWL_SF_PLOT3D::reset_yAngle ()
{
  sli_yAngle->setValue (yAngle);
}

void
US_MWL_SF_PLOT3D::reset_zAngle ()
{
  sli_zAngle->setValue (zAngle);
}

void
US_MWL_SF_PLOT3D::new_rpoint (int)
{
  plot2d ();
}

void
US_MWL_SF_PLOT3D::new_rpid ()
{
  bool state;
...
US_MWL_SF_PLOT3D::plot2d ()
{
  ...
  lb_runs_test->setText (
      tr ("Runs Test Statistics for the Radius %1 (cm)").arg (rpval));
  for (int i = 0; i < nwl; ++i)
    {
      int id_wl = i + idWL_l;
...
void
US_MWL_SF_PLOT3D::get_minMaxMean (void)
{
...
void
US_MWL_SF_PLOT3D::render_option (int)
{
...
void
US_MWL_SF_PLOT3D::enable_wgt (bool state)
{
...
void
US_MWL_SF_PLOT3D::closeEvent (QCloseEvent *event)
{
  renderLoopState = false;
  event->accept ();
}
...
void
US_MWL_SF_PLOT3D::cameraChanged (float)
{
  cb_camera->setCurrentIndex (0);
}

void
US_MWL_SF_PLOT3D::set_radial_slider (void)
{
  ...
  connect (sli_radial, SIGNAL (valueChanged (int)), this,
           SLOT (new_rpoint (int)));
  connect (le_rpid, SIGNAL (returnPressed ()), this, SLOT (new_rpid ()));
}

bool
US_MWL_SF_PLOT3D::get_runs_test (QVector<double> sample, bool display)
{
      ...
      if (randstate)
        {
          ...
          lb_rt_rstate->setStyleSheet (
              tr ("QLabel { background-color : %1; color : black; }")
                  .arg (color.name ()));
        }
      else
        {
...
void
US_MWL_SF_PLOT3D::fill_table ()
{
  lookupTable.clear ();
  QStringList tableRows;
  tableRows << "20,15,27,19,14,26,18,14,26,17,14,25,16,13,24,15,13,24,14,12,"
               "23,13,11,22,12,11,21,11,10,20,10,10,19,9,9,19,8,8,16,7,7,-1,6,"
               "7,-1,5,6,-1,4,5,-1,3,4,-1,2,3,-1";
  tableRows << "19,14,26,18,14,25,17,13,25,16,13,24,15,12,23,14,12,22,13,11,"
               "21,12,11,21,11,10,20,10,9,19,9,9,19,8,8,16,7,7,-1,6,7,-1,5,6,-"
               "1,4,5,-1,3,4,-1,2,3,-1";
  tableRows << "18,13,25,17,13,24,16,12,24,15,12,23,14,11,22,13,11,21,12,10,"
               "21,11,10,19,10,9,18,9,9,19,8,8,16,7,7,-1,6,6,-1,5,6,-1,4,5,-1,"
               "3,4,-1,2,3,-1";
  tableRows
      << "17,12,24,16,12,23,15,12,22,14,11,22,13,11,21,12,10,20,11,10,19,10,9,"
         "18,9,8,19,8,8,16,7,7,-1,6,6,-1,5,5,-1,4,5,-1,3,4,-1,2,3,-1";
  tableRows << "16,12,22,15,11,22,14,11,21,13,10,21,12,10,21,11,9,19,10,9,18,"
               "9,8,19,8,7,16,7,7,-1,6,6,-1,5,5,-1,4,5,-1,3,4,-1,2,3,-1";
  tableRows << "15,11,21,14,10,21,13,10,20,12,9,19,11,9,18,10,8,17,9,8,19,8,7,"
               "15,7,7,14,6,6,-1,5,5,-1,4,4,-1,3,4,-1,2,3,-1";
  tableRows << "14,10,20,13,10,19,12,9,19,11,9,18,10,8,17,9,8,16,8,7,15,7,6,"
               "14,6,6,-1,5,5,-1,4,4,-1,3,3,-1,2,3,-1";
  tableRows << "13,9,19,12,8,19,11,8,18,10,8,17,9,7,16,8,7,15,7,6,14,6,6,-1,5,"
               "5,-1,4,4,-1,3,3,-1,2,3,-1";
  tableRows << "12,8,18,11,8,17,10,8,16,9,7,15,8,7,15,7,6,13,6,5,12,5,5,-1,4,"
               "4,-1,3,3,-1,2,3,-1";
  tableRows << "11,8,16,10,7,16,9,7,15,8,6,14,7,6,13,6,5,12,5,5,-1,4,4,-1,3,3,"
               "-1,2,-1,-1";
  tableRows
      << "10,7,15,9,6,15,8,6,14,7,6,13,6,5,12,5,4,-1,4,4,-1,3,3,-1,2,-1,-1";
...
US_MWL_SF_PLOT3D::new_table_row (QString rowStr)
{
...
QValue3DAxisFormatter *
CustomFormatter::createNewInstance () const
{
  return new CustomFormatter ();
}

void
CustomFormatter::populateCopy (QValue3DAxisFormatter &copy) const
{
      ...
      if (subGridPositions ().size ())
        {
          for (int j = 0; j < subGridCount; j++)
            subGridPositions ()[i * subGridCount + j]
                = gridValue + subSegmentStep * (j + 1);
        }
    }
...
QString
CustomFormatter::stringForValue (qreal value, const QString &format) const
{
  //    Q_UNUSED(format)
...
public:
  ...
  virtual QwtText
  trackerTextF (const QPointF &pos) const
  {
    QColor bg (Qt::white);
    bg.setAlpha (200);
...
SpectrogramData::SpectrogramData (QVector<double> points,
                                  QVector<double> scans,
    ...
    : QDialog (parent)
{
  ...
  connect (pb_print, SIGNAL (clicked ()), this, SLOT (printPlot ()));
  ...
  colorList << "white" << "black" << "red" << "darkRed" << "green"
            ...
            << "magenta" << "darkMagenta" << "yellow" << "darkYellow" << "gray"
            << "darkGray" << "lightGray";
  ...
  connect (cb_color_r, SIGNAL (currentIndexChanged (int)), this,
           SLOT (setColorMap ()));
  ...
  connect (cb_color_nr, SIGNAL (currentIndexChanged (int)), this,
           SLOT (setColorMap ()));
  ...
  //    const QwtInterval zInterval = d_spectrogram->data()->interval(
  //    Qt::ZAxis );
  ...
  //    zInterval.maxValue() ); d_plot->enableAxis( QwtPlot::yRight );
...
QColor
RunsTestWidget::getColor (QString cname)
{
  QColor color;
  if (cname == "white")
...
RunsTestWidget::setColorMap ()
{
  ...
  //    const QwtInterval zInterval = d_spectrogram->data()->interval(
  //    Qt::ZAxis );
  d_spectrogram->setColorMap (
      new QwtLinearColorMap (color_nr, color_r, QwtColorMap::RGB));
  //    axis->setColorMap( zInterval, new QwtLinearColorMap( color_1, color_2,
  //    QwtColorMap::RGB ) );
  int alpha = 250;
...
void
RunsTestWidget::showContour (bool on)
{
