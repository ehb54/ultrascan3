...
US_MWL_SF_PLOT3D::US_MWL_SF_PLOT3D(QWidget *w, const SFData &spFitData)
  ...
  scale =
      spFitData.wavelenghts.at(nWavelengths - 1) - spFitData.wavelenghts.at(0);
  lambdaScaled.clear();
  lambda4ct.clear();
  ...
  US_Plot *us_dataPlot =
      new US_Plot(dataPlot, tr(""), tr("Wavelength (nm)"), tr("Absorbance"));
  ...
  US_Plot *us_devPlot =
      new US_Plot(errorPlot, tr(""), tr("Wavelength (nm)"), tr("Residual"));
  ...
  if (ckb_rendall->isChecked()) {
    ...
    if (state == QDialog::Rejected) {
      ...
      if (le_bname->text().isEmpty()) {
        QMessageBox::warning(this, tr("Error!"), tr("Empty Basename Error!"));
        return;
      }
      enable_wgt(false);
    ...
    QString fileName = QFileDialog::getSaveFileName(
        this, tr("Save Image (*.png *.xpm *.jpg)"), US_Settings::resultDir(),
...
void US_MWL_SF_PLOT3D::newScan(int id) {
  ...
  if (!renderRunState) render_option(0);
}

void US_MWL_SF_PLOT3D::nextScan() { cb_scan->setCurrentIndex(scanId + 1); }

void US_MWL_SF_PLOT3D::prevScan() { cb_scan->setCurrentIndex(scanId - 1); }
...
void US_MWL_SF_PLOT3D::adjustRpMin(double) { set_ct_rp(true); }

void US_MWL_SF_PLOT3D::adjustRpMax(double) { set_ct_rp(false); }
...
void US_MWL_SF_PLOT3D::adjustWlMin(double) { set_ct_wl(true); }

void US_MWL_SF_PLOT3D::adjustWlMax(double) { set_ct_wl(false); }
...
void US_MWL_SF_PLOT3D::reset_xAngle() { sli_xAngle->setValue(xAngle); }

void US_MWL_SF_PLOT3D::reset_yAngle() { sli_yAngle->setValue(yAngle); }

void US_MWL_SF_PLOT3D::reset_zAngle() { sli_zAngle->setValue(zAngle); }

void US_MWL_SF_PLOT3D::new_rpoint(int) { plot2d(); }
...
void US_MWL_SF_PLOT3D::cameraChanged(float) { cb_camera->setCurrentIndex(0); }
...
bool US_MWL_SF_PLOT3D::get_runs_test(QVector<double> sample, bool display) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (i > 0) {
      double valp = sp[i - 1];
      if ((val >= median && valp < median) || (val < median && valp >= median))
        nruns++;
    }
    if (val >= median)
      npos++;
...
void US_MWL_SF_PLOT3D::fill_table() {
  ...
  tableRows << "20,15,27,19,14,26,18,14,26,17,14,25,16,13,24,15,13,24,14,12,23,"
               "13,11,22,12,11,21,11,10,20,10,10,19,9,9,19,8,8,16,7,7,-1,6,7,-"
               "1,5,6,-1,4,5,-1,3,4,-1,2,3,-1";
  tableRows << "19,14,26,18,14,25,17,13,25,16,13,24,15,12,23,14,12,22,13,11,21,"
               "12,11,21,11,10,20,10,9,19,9,9,19,8,8,16,7,7,-1,6,7,-1,5,6,-1,4,"
               "5,-1,3,4,-1,2,3,-1";
  tableRows
      << "18,13,25,17,13,24,16,12,24,15,12,23,14,11,22,13,11,21,12,10,21,11,10,"
         "19,10,9,18,9,9,19,8,8,16,7,7,-1,6,6,-1,5,6,-1,4,5,-1,3,4,-1,2,3,-1";
  tableRows
      << "17,12,24,16,12,23,15,12,22,14,11,22,13,11,21,12,10,20,11,10,19,10,9,"
         "18,9,8,19,8,8,16,7,7,-1,6,6,-1,5,5,-1,4,5,-1,3,4,-1,2,3,-1";
  tableRows << "16,12,22,15,11,22,14,11,21,13,10,21,12,10,21,11,9,19,10,9,18,9,"
               "8,19,8,7,16,7,7,-1,6,6,-1,5,5,-1,4,5,-1,3,4,-1,2,3,-1";
  tableRows << "15,11,21,14,10,21,13,10,20,12,9,19,11,9,18,10,8,17,9,8,19,8,7,"
               "15,7,7,14,6,6,-1,5,5,-1,4,4,-1,3,4,-1,2,3,-1";
  tableRows << "14,10,20,13,10,19,12,9,19,11,9,18,10,8,17,9,8,16,8,7,15,7,6,14,"
               "6,6,-1,5,5,-1,4,4,-1,3,3,-1,2,3,-1";
  tableRows << "13,9,19,12,8,19,11,8,18,10,8,17,9,7,16,8,7,15,7,6,14,6,6,-1,5,"
               "5,-1,4,4,-1,3,3,-1,2,3,-1";
  tableRows << "12,8,18,11,8,17,10,8,16,9,7,15,8,7,15,7,6,13,6,5,12,5,5,-1,4,4,"
               "-1,3,3,-1,2,3,-1";
...
void US_MWL_SF_PLOT3D::plotPixMap() {
  ...
  for (int i = 0; i < nScans; i++) scans << i + 1;
...
void CustomFormatter::recalculate() {
  ...
  if (subGridCount > 0) subSegmentStep = segmentStep / qreal(subGridCount + 1);
...
QString CustomFormatter::stringForValue(qreal value,
...
SpectrogramData::SpectrogramData(QVector<double> points, QVector<double> scans,
  ...
  for (int i = 0; i < points.size() - 1; i++) {
    ...
    if (i == 0) xrange << points.first() - (mid - points.first());
    xrange << mid;
  }
  ...
  for (int i = 0; i < scans.size() - 1; i++) {
    ...
    if (i == 0) yrange << scans.first() - (mid - scans.first());
    yrange << mid;
  }
