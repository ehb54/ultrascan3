...
US_MWL_SF_PLOT3D::US_MWL_SF_PLOT3D(QWidget *w, const SFData &spFitData) : US_WidgetsDialog(w) {
   ...
   US_Plot *us_dataPlot = new US_Plot(dataPlot, tr(""), tr("Wavelength (nm)"), tr("Absorbance"));
   ...
   US_Plot *us_devPlot = new US_Plot(errorPlot, tr(""), tr("Wavelength (nm)"), tr("Residual"));
...
void US_MWL_SF_PLOT3D::renderImage() {
   ...
   if (ckb_rendall->isChecked()) {
      ...
      if (bname != baseName)
         QMessageBox::warning(this, tr("Warning!"), tr("The new basename is\n%1").arg(baseName));
...
void US_MWL_SF_PLOT3D::toggleLambda(bool) {
   graph->setSelectionMode(QAbstract3DGraph::SelectionItemAndColumn | QAbstract3DGraph::SelectionSlice);
}
...
void US_MWL_SF_PLOT3D::plot2d() {
   ...
   QwtSymbol *symbol = new QwtSymbol(QwtSymbol::Ellipse, QBrush(Qt::red), QPen(Qt::red, 3), QSize(5, 5));
...
void US_MWL_SF_PLOT3D::fill_table() {
   ...
   tableRows << "20,15,27,19,14,26,18,14,26,17,14,25,16,13,24,15,13,24,14,12,23,13,11,22,12,11,21,11,10,20,10,10,19,9,"
                "9,19,8,8,16,7,7,-1,6,7,-1,5,6,-1,4,5,-1,3,4,-1,2,3,-1";
   tableRows << "19,14,26,18,14,25,17,13,25,16,13,24,15,12,23,14,12,22,13,11,21,12,11,21,11,10,20,10,9,19,9,9,19,8,8,"
                "16,7,7,-1,6,7,-1,5,6,-1,4,5,-1,3,4,-1,2,3,-1";
   tableRows << "18,13,25,17,13,24,16,12,24,15,12,23,14,11,22,13,11,21,12,10,21,11,10,19,10,9,18,9,9,19,8,8,16,7,7,-1,"
                "6,6,-1,5,6,-1,4,5,-1,3,4,-1,2,3,-1";
   tableRows << "17,12,24,16,12,23,15,12,22,14,11,22,13,11,21,12,10,20,11,10,19,10,9,18,9,8,19,8,8,16,7,7,-1,6,6,-1,5,"
                "5,-1,4,5,-1,3,4,-1,2,3,-1";
   tableRows << "16,12,22,15,11,22,14,11,21,13,10,21,12,10,21,11,9,19,10,9,18,9,8,19,8,7,16,7,7,-1,6,6,-1,5,5,-1,4,5,-"
                "1,3,4,-1,2,3,-1";
   tableRows
      << "15,11,21,14,10,21,13,10,20,12,9,19,11,9,18,10,8,17,9,8,19,8,7,15,7,7,14,6,6,-1,5,5,-1,4,4,-1,3,4,-1,2,3,-1";
...
CustomFormatter::CustomFormatter(qreal i_minval, qreal i_maxval) : QValue3DAxisFormatter() {
...
SpectrogramData::SpectrogramData(QVector<double> points, QVector<double> scans, QVector<QVector<double>> states) :
    QwtRasterData() {
   xrange.clear();
...
RunsTestWidget::RunsTestWidget(
   QVector<double> points, QVector<double> scans, QVector<QVector<double>> states, QWidget *parent) : QDialog(parent) {
   this->setWindowTitle("Runs Test PixelMap");
   Qt::WindowFlags flags;
   flags = flags | Qt::CustomizeWindowHint | Qt::WindowSystemMenuHint | Qt::WindowMaximizeButtonHint
   ...
   colorList << "white" << "black" << "red" << "darkRed" << "green" << "darkGreen" << "blue" << "darkBlue" << "cyan"
   ...
   connect(cb_color_r, SIGNAL(currentIndexChanged(int)), this, SLOT(setColorMap()));
   ...
   connect(cb_color_nr, SIGNAL(currentIndexChanged(int)), this, SLOT(setColorMap()));
