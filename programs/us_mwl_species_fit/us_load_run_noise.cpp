...
int US_LoadRunNoise::count_noise(
   const bool dbload, const QString runID, QStringList *edIDsP, QStringList *noiInfP, int *neditsP) {
...
int US_LoadRunNoise::get_noises(const QString edGUID, US_Noise &ti_noise, US_Noise &ri_noise) {
   int noisflag = 0;
   ...
   if (edx < 0) { // Edit not found, so no noise
      ...
      DbgLv(1) << "LRNoi:gn: nf" << noisflag << "noiIti,Iri" << noiIti << noiIri << "ninfo" << ninfo;
...
int US_LoadRunNoise::scan_db(const QString runID) {
   ...
   while (dbP->next()) { // Accumulate latest edits from raw records of the run
      ...
      DbgLv(1) << "LRNoi:scdb:     aucID" << aucID << "aucx" << aucx << "edtGI" << edtGI;
      ...
      else if (etime > vEetimes[ aucx ]) { // Duplicate edit in a raw:  save information from the latest one
         vEaucIDs[ aucx ] = aucID;
         vEedtIDs[ aucx ] = edtID;
         vEedtGIs[ aucx ] = edtGI;
         vEetimes[ aucx ] = etime;
   ...
   for (int ii = 0; ii < vEedtGIs.count(); ii++) { // Build the list of latest-edit GUIDs for the run
      edGUIDs << vEedtGIs[ ii ];
   }
   ...
   DbgLv(1) << "LRNoi:scdb:   vNedtGIs count" << nnoie << "nG0 nGn" << ng0 << ngn << "edGUIDS k" << edGUIDs.count();
   const QString dlm("^");
   ...
   for (int ii = 0; ii < edGUIDs.count(); ii++) {
      ...
      QString ninfo("");
      QString nflag("");
      DbgLv(1) << "LRNoi:scdb: ii" << ii << "fx lx" << fendx << lendx << "edtGI" << edtGI;
      ...
      else if (fendx < lendx) { // Pare down to single entry with possibly multiple noises
         noiflag = 0;
...
int US_LoadRunNoise::scan_disk(const QString runID) {
   ...
   for (int ii = 0; ii < f_names.count(); ii++) {
      ...
      while (!xml.atEnd()) { // Search XML elements until we find "editGUID"
         ...
         if (xml.isStartElement() && xml.name() == "editGUID") { // test for desired editGUID
            attr = xml.attributes();
