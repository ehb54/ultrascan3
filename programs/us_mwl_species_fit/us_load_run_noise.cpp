...
int
US_LoadRunNoise::count_noise (const bool dbload, const QString runID,
...
int
US_LoadRunNoise::get_noises (const QString edGUID, US_Noise &ti_noise,
                             US_Noise &ri_noise)
{
  int noisflag = 0;
  ...
  if (edx < 0)
    ...
    { // Get noise(s), based on noise information for given edit
      ...
      DbgLv (1) << "LRNoi:gn: nf" << noisflag << "noiIti,Iri" << noiIti
                << noiIri << "ninfo" << ninfo;
        ...
        { // Has TI noise, so get it
          if (dbP != NULL)
            { // Load from DB
              ...
              DbgLv (1) << "LRNoi:gn:   noiIti" << noiIti << "count"
                        << ti_noise.count;
            }
          else
        ...
        { // Has RI noise, so get it
          if (dbP != NULL)
            { // Load from DB
              ...
              DbgLv (1) << "LRNoi:gn:   noiIri" << noiIri << "count"
                        << ri_noise.count;
            }
          else
            { // Load from local file
...
int
US_LoadRunNoise::scan_db (const QString runID)
{
  int krnois = 0;
  QStringList query;
  ...
  DbgLv (1) << "LRNoi:scdb:  dbP" << dbP << "invID" << invID << "runID"
            << runID;
  ...
  DbgLv (1) << "LRNoi:scdb:   edGUIDS count" << eeknt << "eG0 eGn" << eg0
            << egn;
  ...
  DbgLv (1) << "LRNoi:scdb:   vNedtGIs count" << nnoie << "nG0 nGn" << ng0
            << ngn << "edGUIDS k" << edGUIDs.count ();
  const QString dlm ("^");
  ...
  for (int ii = 0; ii < edGUIDs.count (); ii++)
    {
      ...
      if (fendx < 0)
        ...
        { // Pare down to single entry with possibly multiple noises
          ...
          for (int jj = fendx; jj <= lendx; jj++)
            {
              // DbgLv(1) << "LRNoi:scdb:     jj" << jj << "jmodID" <<
              // vNmodIDs[jj];
              if (jj != jlate && vNmodIDs[jj] == modID)
                {
                  ...
                  noiflag = 3;
                  jlate2 = jj;
                  // DbgLv(1) << "LRNoi:scdb:    jlate2" << jlate2 << "noif" <<
                  // noiflag
      ...
      ninfo = dlm + edtGI + dlm + modID + dlm + nflag + dlm + noiIt + dlm
              + noiIr;
...
int
US_LoadRunNoise::scan_disk (const QString runID)
{
  QVector<QString> vNedtIDs; // Noise info vectors
  QVector<QString> vNedtGIs;
  QVector<QString> vNnoiIDs;
          ...
          if (xml.isStartElement () && xml.name () == "model")
            { // test for desired editGUID
              ...
              if (!edGUIDs.contains (edtGI))
                break;
              // DbgLv(1) << "LRNoi:scdk:      modGI" << modGI << "edtGI" <<
              // edtGI;
  ...
  for (int ii = 0; ii < edGUIDs.count (); ii++)
    {
      ...
      if (fendx < 0)
        ...
        { // Pare down to single entry with possibly multiple noises
          ...
          for (int jj = fendx; jj <= lendx; jj++)
            {
              // DbgLv(1) << "LRNoi:scdk:     jj" << jj << "jmodID" <<
              // vNmodIDs[jj];
              if (jj != jlate && vNmodIDs[jj] == modID)
                {
                  noTyp = vNnoTyps[jj];
                  noiflag = 3;
                  jlate2 = jj;
                  // DbgLv(1) << "LRNoi:scdk:    jlate2" << jlate2 << "noif" <<
                  // noiflag
                  //  << "type descr" << noTyp << vNdescrs[jlate2];
                  break;
                }
