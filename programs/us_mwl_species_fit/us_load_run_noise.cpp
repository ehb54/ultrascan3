...
// Alt. Constructor
US_LoadRunNoise::US_LoadRunNoise(QMap<QString, QString>& protocol_details,
                                 QObject* parent)
    : QObject(parent) {
  dbg_level = US_Settings::us_debug();
  dbP = NULL;
...
int US_LoadRunNoise::count_noise(const bool dbload, const QString runID,
                                 QStringList* edIDsP, QStringList* noiInfP,
                                 int* neditsP) {
  ...
  if (nrnois < 1) return nrnois;

  if (edIDsP != NULL) *edIDsP = edGUIDs;

  if (noiInfP != NULL) *noiInfP = noiInfos;

  if (neditsP != NULL) *neditsP = edGUIDs.count();

  return nrnois;
}

// Return noises for a given edit GUID
int US_LoadRunNoise::get_noises(const QString edGUID, US_Noise& ti_noise,
                                US_Noise& ri_noise) {
  int noisflag = 0;
  int edx = edGUIDs.indexOf(edGUID);

  if (edx < 0) {  // Edit not found, so no noise
    ti_noise = US_Noise();
    ri_noise = US_Noise();
  }

  else {  // Get noise(s), based on noise information for given edit
    ...
    if ((noisflag & 1) != 0) {  // Has TI noise, so get it
      if (dbP != NULL) {        // Load from DB
        ti_noise.load(noiIti, dbP);
        DbgLv(1) << "LRNoi:gn:   noiIti" << noiIti << "count" << ti_noise.count;
      } else {  // Load from local file
        ti_noise.load(noiIti);
      }
    } else {  // No TI noise, so empty it
      ti_noise = US_Noise();
    }

    if ((noisflag & 2) != 0) {  // Has RI noise, so get it
      if (dbP != NULL) {        // Load from DB
...
int US_LoadRunNoise::scan_db(const QString runID) {
  ...
  if (dbP->lastErrno() != US_DB2::OK) return 0;

  query.clear();
  query << "get_edit_desc_by_runID" << invID << runID;
  dbP->query(query);

  while (dbP->next()) {  // Accumulate latest edits from raw records of the run
    ...
    DbgLv(1) << "LRNoi:scdb:     aucID" << aucID << "aucx" << aucx << "edtGI"
             << edtGI;

    if (aucx < 0) {  // First encountered raw for an edit:  append information
      vEaucIDs << aucID;
      vEedtIDs << edtID;
      vEedtGIs << edtGI;
      vEetimes << etime;
    ...
    else if (etime > vEetimes[aucx]) {  // Duplicate edit in a raw:  save
                                        // information from the latest one
      vEaucIDs[aucx] = aucID;
      vEedtIDs[aucx] = edtID;
      vEedtGIs[aucx] = edtGI;
      vEetimes[aucx] = etime;
  ...
  for (int ii = 0; ii < vEedtGIs.count();
       ii++) {  // Build the list of latest-edit GUIDs for the run
    edGUIDs << vEedtGIs[ii];
  }
  int eeknt = edGUIDs.count();
  QString eg0 = (eeknt > 0) ? edGUIDs[0] : "";
  QString egn = (eeknt > 0) ? edGUIDs[eeknt - 1] : "";
  DbgLv(1) << "LRNoi:scdb:   edGUIDS count" << eeknt << "eG0 eGn" << eg0 << egn;

  query.clear();
  query << "get_noise_desc" << invID;
  dbP->query(query);

  while (dbP->next()) {  // Save noise information where edit is in list
    ...
    vNedtIDs << edtID;
    // DbgLv(1) << "LRNoi:scdb:   edtx" << edtx << "edtID edtGI" << edtID <<
    // edtGI;
    vNedtGIs << edtGI;
    vNnoiIDs << noiID;
    vNnoiGIs << noiGI;
  ...
  DbgLv(1) << "LRNoi:scdb:   vNedtGIs count" << nnoie << "nG0 nGn" << ng0 << ngn
           << "edGUIDS k" << edGUIDs.count();
  const QString dlm("^");
  QDateTime ntime = (nnoie > 0) ? vNntimes[0] : QDateTime::currentDateTime();
  ...
  for (int ii = 0; ii < edGUIDs.count(); ii++) {
    ...
    QString ninfo("");
    QString nflag("");
    DbgLv(1) << "LRNoi:scdb: ii" << ii << "fx lx" << fendx << lendx << "edtGI"
             << edtGI;

    if (fendx < 0) {  // Create a single entry for edit with no-noise type
      nflag = QString("0");
      noiIt = QString("-1");
      noiIr = QString("-1");
    }

    else if (fendx < lendx) {  // Pare down to single entry with possibly
                               // multiple noises
      noiflag = 0;
    ...
    else if (fendx == lendx) {  // Create a single entry with a single noise
    ...
    ninfo = dlm + edtGI + dlm + modID + dlm + nflag + dlm + noiIt + dlm + noiIr;
...
int US_LoadRunNoise::scan_disk(const QString runID) {
  ...
  for (int ii = 0; ii < f_names.count(); ii++) {
    ...
    if (!e_file.open(QIODevice::ReadOnly | QIODevice::Text)) continue;

    QXmlStreamReader xml(&e_file);

    while (!xml.atEnd()) {  // Search XML elements until we find "editGUID"
      xml.readNext();

      if (xml.isStartElement() &&
          xml.name() == "editGUID") {  // test for desired editGUID
        attr = xml.attributes();
  ...
  for (int ii = 0; ii < f_names.count(); ii++) {
    ...
    while (!xml.atEnd()) {  // Search XML elements for model GUID values
      xml.readNext();

      if (xml.isStartElement() &&
          xml.name() == "model") {  // test for desired editGUID
        attr = xml.attributes();
        QString edtGI = attr.value("editGUID").toString();
        QString modGI = attr.value("modelGUID").toString();

        if (!edGUIDs.contains(edtGI)) break;
  ...
  for (int ii = 0; ii < f_names.count(); ii++) {
    ...
    while (!xml.atEnd()) {  // Search XML elements until we find "noise"
      ...
      if (xml.isStartElement() &&
          xml.name() == "noise") {  // test for desired editGUID
        attr = xml.attributes();
        QString modGI = attr.value("modelGUID").toString();

        if (!mmGIs.contains(modGI)) break;
