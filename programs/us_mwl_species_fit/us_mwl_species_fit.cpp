    ...
    : US_AnalysisBase2 ()
{
  ...
  connect (pb_loadsfit, SIGNAL (clicked ()), this, SLOT (loadSpecs ()));
  connect (pb_sfitdata, SIGNAL (clicked ()), this, SLOT (specFitData ()));
  ...
  // pass edata BC
  protocol_details_p["baseline_corrections"]
      = this->protocol_details["baseline_corrections"];
  protocol_details_p["meniscus_info"]
      = this->protocol_details["meniscus_info"];
  ...
  if (!read_protocol (msg_to_user))
  ...
  else // continue
    {
      ...
      for ( int i=0; i< ext_keys.size(); ++ i )
        {
          qDebug() << "For channel: " << ext_keys[i] << ", there are profiles
      for: "; QMap< QString, QMap< double, double > > analytes_profs =
      extinction_profiles_per_channel[ ext_keys[i] ]; QStringList
      analytes_profs_keys = analytes_profs.keys(); for ( int j=0; j<
      analytes_profs_keys.size(); ++ j )
            {
              QMap< double, double > ext_prof_s = analytes_profs[
      analytes_profs_keys[j] ]; qDebug() << analytes_profs_keys[j] << "; it has
      " << ext_prof_s.keys().size() << "wvls..." ;
            }
        }
      //end debug
      */
          ...
          if (this->protocol_details["abde_etype"] == "MWL")
            {
              ...
              QMap<QString, QMap<double, double> > analytes_profs
                  = extinction_profiles_per_channel[triple_text];

              loadSpecs_auto (analytes_profs);
              specFitData ();

              // save ssf-dir name for future DB save
              protocol_details_p["ssf_dir_name"]
                  = this->protocol_details["ssf_dir_name"];
            }

          // pass ranges from reportItems
          protocol_details_p["channels_to_radial_ranges"]
              = this->protocol_details["channels_to_radial_ranges"];
          protocol_details_p["directory_for_gmp"]
              = this->protocol_details["directory_for_gmp"];
        }
      rmsd_for_gmp.chop (1);
...
void
US_MwlSpeciesFit::data_plot (void)
{
  plot_data2 ();
...
void
US_MwlSpeciesFit::plot_data2 (void)
{
      ...
      while (jj < points && escan->rvalues[jj] < lower_limit)
        {
          ...
          if (us_gmp_auto_mode)
            vv[count] -= (rr[count] * edata->bl_corr_slope
                          + edata->bl_corr_yintercept);
...
void
US_MwlSpeciesFit::plot_data1 (void)
{
  if (jspec >= 0)
  ...
  QString dataType = tr ("Absorbance");
  if (strcmp (rdata->type, "RI") == 0)
    dataType = tr ("Intensity");
  if (strcmp (rdata->type, "WI") == 0)
    dataType = tr ("Intensity");
  if (strcmp (rdata->type, "IP") == 0)
    dataType = tr ("Interference");
  if (strcmp (rdata->type, "FI") == 0)
    dataType = tr ("Fluorescence");

  QString header = tr ("Output Velocity Data for\n") + runID + "  (" + str_ce
                   + "/" + rdata->channel + "/" + str_wl + ")";
...
void
US_MwlSpeciesFit::write_report (QTextStream &ts)
{
  int index = lw_triples->currentRow ();
  US_DataIO::EditedData *edata = &dataList[index];

  QString sm_results
      = table_row (tr ("Average Second Moment S: "), +" 1.0 s * 10e-13");

  ts << html_header (QString ("US_Second_Moment"),
                     QString ("Second Moment Analysis"), edata);
  ts << analysis (sm_results);
  ts << indent (2) + "</body>\n</html>\n";
}

void
US_MwlSpeciesFit::view (void)
{
...
US_MwlSpeciesFit::load (void)
{
  ...
  if (us_gmp_auto_mode)
    {
      // dialog = new US_DataLoader( edlast, dbdisk, rawList, dataList,
      ...
      protocol_details["directory_for_gmp"]
          = US_Settings::resultDir () + "/" + dir_gmp.split (";")[1];
      ...
      for (int i = 0; i < dataList.size (); ++i)
        {
          QString triple_name = dataList[i].cell + dataList[i].channel
                                + dataList[i].wavelength;
          qDebug () << "channel " << triple_name << ", the baseline slope  "
                    << dataList[i].bl_corr_slope << ", intercept "
                    << dataList[i].bl_corr_yintercept << ", left "
                    << dataList[i].xvalues[0] << ", right "
                    << dataList[i].xvalues.last () << ", editID "
                    << dataList[i].editID << ", editGUID "
                    << dataList[i].editGUID;
          triples_bc
              += triple_name + ":" + QString::number (dataList[i].xvalues[0])
      ...
      for (int i = 0; i < ch_men_list.size (); ++i)
        {
          channel_men_info
              += ch_men_list[i] + ":" + chans_meniscus[ch_men_list[i]] + ",";
        }
  ...
  for (int ii = 0; ii < ntrips; ii++)
    {
      ...
      DbgLv (1) << "  trip" << ii << "noise subtraction  noisf" << noisf
                << "tiC riC" << ti_noise.count << ri_noise.count << "tiK riK"
                << ti_noise.values.count () << ri_noise.values.count ();
  ...
  for (int ii = 0; ii < celchns.count (); ii++)
    {
      ...
      QString triple = celchns[ii].left (1) + " / " + celchns[ii].mid (1)
                       + " / " + fwvln + "-" + lwvln;
...
bool
US_MwlSpeciesFit::read_protocol (QStringList &msg_to_user)
{
  ...
  QString xmlstr ("");
  US_ProtocolUtil::read_record_auto (ProtocolName_auto, invID_p, &xmlstr, NULL,
                                     &db);
      ...
      for (int kk = 0; kk < report_items_number; ++kk)
        {
          ...
          if (curr_item.type == "Radius")
            {
              range_l_h += QString::number (curr_item.range_low) + "-"
                           + QString::number (curr_item.range_high) + ",";
            }
        }
      range_l_h.chop (1);
      channels_to_radial_ranges += channame_short + ":" + range_l_h + ";";
      qDebug () << "For channel " << channame_short
                << ", ranges are: " << range_l_h;
      // channels_to_radial_ranges[ channame_short ] = range_l_h;
    }
      ...
      if (nwavl > 1 && mwl_deconv)
        {
          ...
          if (status == US_DB2::NO_BUFFER)
          ...
          else if (status == US_DB2::NO_ANALYTE)
            {
              QMessageBox::information (
                  this, tr ("Attention"),
                  tr ("One of the analytes this solution refers to was not "
                      "found.\n"
                      "Please restore and try again.\n"));
              return false;
            }
          ...
          for (int i = 0; i < num_analytes; ++i)
            {
              ...
              extinction_profiles_per_channel[channel_s][a_name]
                  = analyte.extinction;
            }
          ...
          if (buff_req) // only if buffer spectrum required
            {
              ...
              // fill in the QMap of ext. profiles
              extinction_profiles_per_channel[channel_s][b_name]
                  = buffer.extinction;
            }
...
bool
US_MwlSpeciesFit::validExtinctionProfile (QString desc, QList<double> all_wvls,
...
void
US_MwlSpeciesFit::loadSpecs_auto (
    QMap<QString, QMap<double, double> > analytes_profs)
{
  ...
  for (int j = 0; j < nspecies_tmp; ++j)
    {
      ...
      qDebug () << analytes_profs_keys[j] << "; it has "
                << ext_prof_s.keys ().size () << "wvls...";
  ...
  //                           arg(curr_celchnwvl.at(0)).arg(curr_celchnwvl.at(nwls
  //                           - 1)) + tr("Please provide the proper spectral
  //                           profiles."));
  //     return;
  // }
...
US_MwlSpeciesFit::loadSpecs ()
{
  ...
  for (int ii = 0; ii < flist.size (); ii++)
    {
      ...
      US_CSV_Loader *csv_loader
          = new US_CSV_Loader (filepath, note, true, this);
      int check = csv_loader->exec ();
      if (check == QDialog::Rejected)
        {
          QMessageBox::critical (this, "Warning!",
                                 "Loading species is canceled!");
          return;
        }
      else if (check == -2)
        {
          QMessageBox::critical (
              this, "Error!",
              tr ("The loaded file is not in text format!\n\n%1")
                  .arg (filepath));
          return;
        }
      ...
      if (csv_data.columnCount () < 2)
        {
          QMessageBox::critical (this, "Warning!",
                                 "At least two data columns are needed:\n"
                                     + filepath);
          return;
        }
      else
          ...
          if (spwvls.contains (iwavl))
            {
              QMessageBox::warning (
                  ...
                  tr ("The file:\n") + finfo.fileName ()
                      ...
                      + tr ("Please provide the wavelength values without "
                            "decimals."));
              return;
            }
          spwvls << iwavl;
  ...
  //    << spwavls_tmp[js] << spwavls_tmp[je] << spconcs_tmp[js] <<
  //    spconcs_tmp[je];
  // }
  for (int ii = 0; ii < nspecies_tmp; ii++)
    {
      ...
      for (int jj = 0; jj < nwavl; jj++)
        {
          DbgLv (0) << " jj" << jj << "nwavl" << nwavl << "wn cn"
                    << spwavls_tmp.at (ii).at (jj)
                    << spconcs_tmp.at (ii).at (jj);
        }
    }
  ...
  if (bad_files.size () > 0)
    {
      ...
      QMessageBox::warning (
          ...
          tr ("The following file(s):\n") + fnames
              + tr ("\ndo(es) not have the valid data over"
                    " the range of %1 to %2 nm. ")
                    .arg (curr_celchnwvl.at (0))
                    .arg (curr_celchnwvl.at (nwls - 1))
...
US_MwlSpeciesFit::specFitData ()
{
  ...
  for (int ii = trpxs; ii < trpxe; ii++)
    {
      ...
      DbgLv (1) << "sfd:  klambda ii lmb" << klambda << ii
                << lambdas[klambda - 1];
    }
  DbgLv (1) << "sfd: klambda ccx trpxs trpxe" << klambda << ccx << trpxs
            << trpxe;
    ...
    { // Loop through each species, storing reading value for each wavelength
      ...
      for (int jj = 0; jj < nwavl; jj++, kw++)
        { // Get wavelength,concentration vectors for this species
          cswavls << spwavls[kw];
          csconcs << spconcs[kw];
          DbgLv (1) << "sfd: Get wavelength,concentration vectors for species "
                    << ii << ":" << spwavls[kw] << spconcs[kw];
        }
      ...
      for (int jj = 0; jj < klambda; jj++)
        { // Loop through each wavelength of the current range
          ...
          //         {  // Data wavelength beyond species range:  use zero
          //         concentration
          //            cval          = 0.0;
          ...
          //         else
          //         {  // Must search species list and interpolate between
          //         wavelengths
          ...
          //               if ( iwavl > kwavl )
          //               {  // Gone beyond species wavelength:  interpolate
          //               using previous
          //                  int kkp      = qMax( 0, kk - 1 );
          //                  double dcval = cval - csconcs[ kkp ];
          //                  double wval1 = (double)kwavl;
          //                  double wval0 = (double)cswavls[ kkp ];
          //                  double dwavl = ( kkp == kk ) ? 1.0 : ( wval1 -
          //                  wval0 ); cval        += ( ( wval - wval0 ) *
          //                  dcval / dwavl );
          ...
          if ((jj < 4) || ((jj + 5) > klambda) || (jj == (klambda / 2)))
            DbgLv (1) << "sfd:   wv" << jj << iwavl << "ka" << ka << "cval"
                      << cval;
          nnls_a[ka++] = cval;
        } // END:  loop through selected lambda range
      DbgLv (1) << "sfd:    nnls_a" << nnls_a[ka - klambda]
                << nnls_a[ka - klambda + 1] << nnls_a[ka - klambda + 2]
                << nnls_a[ka - klambda / 2] << nnls_a[ka - 3] << nnls_a[ka - 2]
                << nnls_a[ka - 1];
  ...
  DbgLv (1) << "sfd:    sv_nnls_a" << sv_nnls_a[ka - klambda]
            << sv_nnls_a[ka - klambda + 1] << sv_nnls_a[ka - klambda + 2]
            << sv_nnls_a[ka - klambda / 2] << sv_nnls_a[ka - 3]
            << sv_nnls_a[ka - 2] << sv_nnls_a[ka - 1];

  qDebug () << "Fitting...4";
  for (int ii = 0; ii < nspecies; ii++)
    { // Loop to create synthetic data

      qDebug () << "spfiles, spfiles.size() == " << spfiles << spfiles.size ();
      ...
      DbgLv (0) << "sfd:  xv 0 1 p n" << synData[kd].xvalues[0]
                << synData[kd].xvalues[1] << synData[kd].xvalues[krpad]
                << synData[kd].xvalues[kradp - 1];
  ...
  int narows = klambda;
  DbgLv (1) << "sfd: narows kscan inclsize" << narows << kscan
            << inclscns.size ();
    ...
    { // Loop through non-excluded scans
      ...
      for (int jj = krpad; jj < kradp; jj++, jr++)
        { // Loop through radius values
          ...
          orgSp_rpwl << orgSp_wl;
          DbgLv (1) << "sfd:  NNLS rnorm cmp_rnorm" << rnorm
                    << qSqrt (rnorm_c);
        }
  ...
  for (int ii = 0; ii < nspecies; ii++, kd++)
    {
      ...
      if (us_gmp_auto_mode)
        {
          QString solvent_name
              = spfiles[ii].section ("/", -1, -1).section (".", 0, -2);
          str_wl
              = "." + solvent_name + QString ().sprintf ("_%03i.auc", ii + 1);
        }
      else
      ...
      DbgLv (1) << "sfd:  msx mrx" << msx << mrx << "nsscan nspoint" << nsscan
                << nspoint;
      if (msx >= nsscan)
        {
          msx = nsscan - 1;
  ...
  msg += tr ("\n time1-0: %1").arg (time0.msecsTo (time1))
         + " ms  (Read Fit Files)";
  msg += tr ("\n time2-1: %1").arg (time1.msecsTo (time2))
         + " ms  (Initialize AUC)";
  msg += tr ("\n time3-2: %1").arg (time2.msecsTo (time3)) + " ms  (NNLS)";
  msg += tr ("\n time9-3: %1").arg (time3.msecsTo (time9))
         + " ms  (Write Synth.AUC)";
  msg += tr ("\n time9-0: %1").arg (time0.msecsTo (time9))
         + " ms  (All Fit Steps)";
...
void
US_MwlSpeciesFit::new_triple (const int ccx)
{
...
int
US_MwlSpeciesFit::triple_index (const int ccx)
{
  return (ftndxs[ccx] + ltndxs[ccx]) / 2;
}

void
US_MwlSpeciesFit::apply_noise (const int tripx, US_Noise &ti_noise,
                               US_Noise &ri_noise)
{
  US_DataIO::EditedData *edata = &dataList[tripx];
  ...
  noiflags[tripx] = (ntinois > 0 ? 1 : 0) + (nrinois > 0 ? 2 : 0);

  DbgLv (1) << "   ApNoi:    noiflag" << noiflags[tripx];
  te_desc->setHtml (tr ("<b>Subtracting noise from triple ") + triples[tripx]
                    + " ...</b><br/><b> ( "
...
US_MwlSpeciesFit::prev_plot ()
{
  ...
  DbgLv (1) << "PrevPlot: nspecies ltspec jspec" << nspecies << ltspec
            << jspec;
...
US_MwlSpeciesFit::next_plot ()
{
  ...
  DbgLv (1) << "NextPlot: nspecies ltspec jspec" << nspecies << ltspec
            << jspec;
...
void
US_MwlSpeciesFit::get_fit_error ()
{
  le_fit_error->clear ();
  int ccx = lw_triples->currentRow ();
  ...
  if (us_gmp_auto_mode)
    {
      ...
      qDebug () << "RMSD_: triple_m, rmsd_for_gmp -- " << triple_m << ", "
                << rmsd_for_gmp;
    }
}

void
US_MwlSpeciesFit::rmsd_3dplot ()
{
  int ccx = lw_triples->currentRow ();
