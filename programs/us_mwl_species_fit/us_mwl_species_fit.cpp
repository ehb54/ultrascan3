...
US_MwlSpeciesFit::US_MwlSpeciesFit(QMap<QString, QString>& protocol_details_p)
  ...
  if (!read_protocol(msg_to_user)) {
    ...
    QMessageBox::critical(this,
                          ...
                          msg_to_user.join("\n") +
                              tr("\n\nPlease upload valid extinction profiles "
                                 "for above specified analytes "
...
void US_MwlSpeciesFit::plot_data2(void) {
  ...
  for (int ii = 0; ii < scanCount; ii++) {
    ...
    while (jj < points && escan->rvalues[jj] < lower_limit) {
      ...
      if (us_gmp_auto_mode)
        vv[count] -=
            (rr[count] * edata->bl_corr_slope + edata->bl_corr_yintercept);
...
void US_MwlSpeciesFit::plot_data1(void) {
  ...
  if (jspec < 0 || !have_p1[jspec]) return;
...
void US_MwlSpeciesFit::save(void) {
  ...
  QString dsinfFile =
      QString(filebase).replace("/secmo.", "/dsinfo.") + "dataset_info.html";
...
void US_MwlSpeciesFit::load(void) {
  ...
  if (us_gmp_auto_mode) {
    ...
    for (int i = 0; i < dataList.size(); ++i) {
      QString triple_name =
          dataList[i].cell + dataList[i].channel + dataList[i].wavelength;
      qDebug() << "channel " << triple_name << ", the baseline slope  "
               ...
               << dataList[i].editID << ", editGUID " << dataList[i].editGUID;
  ...
  for (int ii = 0; ii < ntrips; ii++) {
    ...
    if (ccx < 0) {  // New cell/channel:  add to list and save first index
      celchns << celchn;
      ftndxs << ii;
      ltndxs << ii;
    } else {  // Existing cell/channel:  update last index
      ltndxs[ccx] = ii;
    }
  ...
  for (int ii = 0; ii < currProto.rpRange.nranges; ii++) {
    ...
    if (nwavl > 1 && mwl_deconv) {
      ...
      if (buff_req)  // only if buffer spectrum required
      {
        ...
        if (!validExtinctionProfile(b_desc, all_wvls, buffer.extinction.keys(),
                                    msg_to_user))
          all_profiles_exist = false;
...
bool US_MwlSpeciesFit::validExtinctionProfile(QString desc,
...
void US_MwlSpeciesFit::specFitData() {
  ...
  for (int ii = 0; ii < nspecies; ii++) {  // Loop through each species, storing
                                           // reading value for each wavelength
    int nwavl = nwavls[ii];
    QVector<int> cswavls;
    QVector<double> csconcs;

    for (int jj = 0; jj < nwavl;
         jj++, kw++) {  // Get wavelength,concentration vectors for this species
      cswavls << spwavls[kw];
      csconcs << spconcs[kw];
      DbgLv(1) << "sfd: Get wavelength,concentration vectors for species " << ii
               << ":" << spwavls[kw] << spconcs[kw];
    }
    DbgLv(1) << "sfd:  sp" << ii << "nwavl" << nwavl;

    for (int jj = 0; jj < klambda;
         jj++) {  // Loop through each wavelength of the current range
      int iwavl = lambdas[jj];
      double cval = 0.0;
      //         double wval  = (double)iwavl;
      int kwx = cswavls.indexOf(iwavl);

      if (kwx >= 0) {  // Matching wavelength:  get corresponding concentration
        cval = csconcs[kwx];
      }
      ...
      //               if ( iwavl > kwavl )
      //               {  // Gone beyond species wavelength:  interpolate using
      //               previous
      //                  int kkp      = qMax( 0, kk - 1 );
  ...
  qDebug() << "Fitting...4";
  for (int ii = 0; ii < nspecies; ii++) {  // Loop to create synthetic data
    ...
    if (edata->dataType == "RI") memcpy(synData[kd].type, "RA", 2);
    ...
    DbgLv(0) << "sfd:    jr" << jr << "synDsiz" << synData.size() << "scnSiz"
             << synData[0].scanData.size() << "kscan" << kscan << "rScnSiz"
             << rdata->scanCount() << rdata->scanData.count();
  ...
  for (int ii = 0; ii < kscan; ii++) {  // Loop through non-excluded scans
    int js = inclscns[ii];
    int jr = radxs;
    ...
    for (int jj = krpad; jj < kradp;
         jj++, jr++) {  // Loop through radius values
      int trx = trpxs + lmbxs;
      nnls_a = sv_nnls_a;

      for (int kk = 0; kk < klambda;
           kk++, trx++) {  // Store scan,radius reading for each wavelength in
                           // channel
        ...
        if (us_gmp_auto_mode) {
          ...
          nnls_b[kk] -=
              (dataList[trx].xvalues[jr] * dataList[trx].bl_corr_slope +
               dataList[trx].bl_corr_yintercept);
        }
      }
      ...
      US_Math2::nnls(nnls_a.data(), narows, narows, nspecies, nnls_b.data(),
                     nnls_x.data(), &rnorm);
  ...
  if (!QDir().exists(dirSyn)) QDir().mkpath(dirSyn);
  ...
  // remember dir name
  if (us_gmp_auto_mode) protocol_details["ssf_dir_name"] = dirSyn;
...
void US_MwlSpeciesFit::apply_noise(const int tripx, US_Noise& ti_noise,
                                   US_Noise& ri_noise) {
  US_DataIO::EditedData* edata = &dataList[tripx];
  ...
  if (ntinois > 0 || nrinois > 0) {  // Apply noise(s) to data
