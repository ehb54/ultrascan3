...
void US_MwlSpeciesFit::write_report(QTextStream &ts) {
   ...
   QString sm_results = table_row(tr("Average Second Moment S: "), +" 1.0 s * 10e-13");
...
void US_MwlSpeciesFit::load(void) {
   ...
   if (us_gmp_auto_mode) {
      ...
      dialog = new US_DataLoader(true, dbdisk, rawList, dataList, triples, description, protocol_details, "none");
      ...
      for (int i = 0; i < dataList.size(); ++i) {
         ...
         qDebug() << "channel " << triple_name << ", the baseline slope  " << dataList[ i ].bl_corr_slope
                  << ", intercept " << dataList[ i ].bl_corr_yintercept << ", left " << dataList[ i ].xvalues[ 0 ]
                  << ", right " << dataList[ i ].xvalues.last() << ", editID " << dataList[ i ].editID << ", editGUID "
                  << dataList[ i ].editGUID;
         triples_bc += triple_name + ":" + QString::number(dataList[ i ].xvalues[ 0 ]) + ","
      ...
      for (int i = 0; i < ch_men_list.size(); ++i) {
         channel_men_info += ch_men_list[ i ] + ":" + chans_meniscus[ ch_men_list[ i ] ] + ",";
      }
   ...
   for (int ii = 0; ii < ntrips; ii++) {
      ...
      QString celchn = triple.section("/", 0, 0).simplified() + triple.section("/", 1, 1).simplified();
      QString waveln = triple.section("/", 2, 2).simplified();
      int ccx = celchns.indexOf(celchn);
      DbgLv(0) << "trip" << ii << "triple" << triple << "celchn" << celchn << "waveln" << waveln << "ccx" << ccx;
      ...
      int noisf = ldnois->get_noises(edGUID, ti_noise, ri_noise);
      DbgLv(1) << "  trip" << ii << "noise subtraction  noisf" << noisf << "tiC riC" << ti_noise.count << ri_noise.count
   ...
   for (int ii = 0; ii < celchns.count(); ii++) {
      ...
      QString triple = celchns[ ii ].left(1) + " / " + celchns[ ii ].mid(1) + " / " + fwvln + "-" + lwvln;
...
bool US_MwlSpeciesFit::validExtinctionProfile(
...
void US_MwlSpeciesFit::loadSpecs_auto(QMap<QString, QMap<double, double>> analytes_profs) {
   ...
   DbgLv(1) << "Species ktspec sD,hvp sizes" << ktspec << synData.size() << have_p1.size();
...
void US_MwlSpeciesFit::loadSpecs() {
   ...
   QStringList flist = QFileDialog::getOpenFileNames(
      this, "Load Species Files", US_Settings::resultDir(), "Text Files (*.dat *.csv *.dsp );; All Files (*)");
   if (flist.size() < 2) {
      QMessageBox::critical(this, tr("Multiple Species Needed"), tr("At lease 2 species must be chosen!\n"));
      return;
   }
   ...
   for (int ii = 0; ii < nspecies_tmp; ii++) {
      ...
      for (int jj = 0; jj < nwavl; jj++) {
         DbgLv(0) << " jj" << jj << "nwavl" << nwavl << "wn cn" << spwavls_tmp.at(ii).at(jj)
...
void US_MwlSpeciesFit::specFitData() {
   ...
   int ka = 0;
   int kw = 0;
   DbgLv(1) << "sfd: size a,b,x" << nnls_a.count() << nnls_b.count() << nnls_x.count();

   for (int ii = 0; ii < nspecies; ii++) { // Loop through each species, storing reading value for each wavelength
      int nwavl = nwavls[ ii ];
      QVector<int> cswavls;
      QVector<double> csconcs;

      for (int jj = 0; jj < nwavl; jj++, kw++) { // Get wavelength,concentration vectors for this species
         cswavls << spwavls[ kw ];
         csconcs << spconcs[ kw ];
         DbgLv(1) << "sfd: Get wavelength,concentration vectors for species " << ii << ":" << spwavls[ kw ]
                  << spconcs[ kw ];
      }
      DbgLv(1) << "sfd:  sp" << ii << "nwavl" << nwavl;

      for (int jj = 0; jj < klambda; jj++) { // Loop through each wavelength of the current range
      ...
      DbgLv(1) << "sfd:    nnls_a" << nnls_a[ ka - klambda ] << nnls_a[ ka - klambda + 1 ] << nnls_a[ ka - klambda + 2 ]
               << nnls_a[ ka - klambda / 2 ] << nnls_a[ ka - 3 ] << nnls_a[ ka - 2 ] << nnls_a[ ka - 1 ];
   ...
   DbgLv(1) << "sfd:    sv_nnls_a" << sv_nnls_a[ ka - klambda ] << sv_nnls_a[ ka - klambda + 1 ]
            << sv_nnls_a[ ka - klambda + 2 ] << sv_nnls_a[ ka - klambda / 2 ] << sv_nnls_a[ ka - 3 ]
   ...
   for (int ii = 0; ii < nspecies; ii++) { // Loop to create synthetic data
      ...
      DbgLv(0) << "sfd:    jr" << jr << "synDsiz" << synData.size() << "scnSiz" << synData[ 0 ].scanData.size()
               << "kscan" << kscan << "rScnSiz" << rdata->scanCount() << rdata->scanData.count();
      int kexcl = excludedScans.count();
      DbgLv(0) << "sfd:    nscan kscan exclknt" << nscan << kscan << kexcl;
      if (kexcl > 4)
         DbgLv(0) << "sfd:    excls 0 1 m n" << excludedScans[ 0 ] << excludedScans[ 1 ] << excludedScans[ kexcl / 2 ]
      ...
      DbgLv(0) << "sfd:  xv 0 1 p n" << synData[ kd ].xvalues[ 0 ] << synData[ kd ].xvalues[ 1 ]
   ...
   for (int ii = 0; ii < kscan; ii++) { // Loop through non-excluded scans
      ...
      for (int jj = krpad; jj < kradp; jj++, jr++) { // Loop through radius values
         int trx = trpxs + lmbxs;
         nnls_a = sv_nnls_a;

         for (int kk = 0; kk < klambda; kk++, trx++) { // Store scan,radius reading for each wavelength in channel
            ...
            if (us_gmp_auto_mode) {
               ...
               nnls_b[ kk ]
                  -= (dataList[ trx ].xvalues[ jr ] * dataList[ trx ].bl_corr_slope + dataList[ trx ].bl_corr_yintercept);
            }
         }
         ...
         US_Math2::nnls(nnls_a.data(), narows, narows, nspecies, nnls_b.data(), nnls_x.data(), &rnorm);
         DbgLv(1) << "sfd:  NNLS  ii jj kl" << ii << jj << klambda << " x:" << nnls_x[ 0 ] << nnls_x[ nspecies - 1 ];
   ...
   int ma = ja / 2;
   DbgLv(1) << "sfd:     NNLS a:" << nnls_a[ 0 ] << nnls_a[ 1 ] << nnls_a[ ma - 1 ] << nnls_a[ ma ] << nnls_a[ ma + 1 ]
            << nnls_a[ ja - 10 ] << nnls_a[ ja ];
   int ms = kscan / 2;
   int mr = kradii / 2;
   //ms=100;
   //mr=100;
   ...
   for (int ii = 0; ii < nspecies; ii++, kd++) {
      ...
      DbgLv(1) << "sfd:  msx mrx" << msx << mrx << "nsscan nspoint" << nsscan << nspoint;
      if (msx >= nsscan) {
         msx = nsscan - 1;
         nits = msx - 1;
      }
...
void US_MwlSpeciesFit::apply_noise(const int tripx, US_Noise &ti_noise, US_Noise &ri_noise) {
   US_DataIO::EditedData *edata = &dataList[ tripx ];
   ...
   te_desc->setHtml(
      tr("<b>Subtracting noise from triple ") + triples[ tripx ] + " ...</b><br/><b> ( "
...
void US_MwlSpeciesFit::get_fit_error() {
   ...
   if (us_gmp_auto_mode) {
      ...
      qDebug() << "RMSD_: triple_m, rmsd_for_gmp -- " << triple_m << ", " << rmsd_for_gmp;
   }
}
