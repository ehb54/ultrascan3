 ...
 * @brief Structure to hold average values for multiple rotor calibration
 * scans.
 */
struct Average_multi
{
 ...
 * @brief The US_RotorCalibration class provides a user interface for rotor
 * calibration.
 */
...
private:
  ...
  QVector<double> stretch_factors,
      std_dev;            //!< Stretch factors and standard deviations
  QVector<Limit> limit;   //!< Limits
  QVector<double> bounds; //!< X-limits for multi-channel calibration mask
  QVector<QwtDoubleRect>
      bounds_rect; //!< Limits for multi-channel calibration mask

  QwtDoubleRect
      zoom_mask; //!< Zoomed rectangle for multi-channel calibration mask
...
private slots:
  ...
  void currentRect (
      QwtDoubleRect rect); //!< Slot to handle current rectangle selection
  void currentRectf (
      QRectF rect); //!< Slot to handle current rectangle selection (QRectF)
  ...
  double findAverage (QwtDoubleRect rect, US_DataIO::RawData data,
                      int channel); //!< Find the average
