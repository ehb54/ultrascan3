...
void US_RotorCalibration::loadDB(void) {
  ...
  for (int i = 0; i < rawDataIDs.size(); i++) {
    ...
    int readStatus = db.readBlobFromDB(filename, QString("download_aucData"),
                                       rawDataIDs[i].toInt());
    ...
    if (dataType != "RI") {
      if (dataType != "FI") {
        QMessageBox::information(this, tr("Error"),
                                 runID +
                                     tr(" is not absorbance or fluorescence "
                                        "intensity data,\nit is ") +
                                     dataType + tr(" data - aborting."));
        return;
      }
...
void US_RotorCalibration::divide(QwtDoubleRect rect) {
  ...
  if (zoomed) {
    le_instructions->setText(
        tr("Please control-left click between each vertical region, then click "
           "calculate..."));
  }
  pick->disconnect();
...
void US_RotorCalibration::next() {
  if (!top_of_cell) {
    ...
    if (current_triple == allData.size()) {
      ...
      le_instructions->setText(
          "All vertical regions have been reviewed, calculating rotor "
          "calibration...");
      pb_accept->setEnabled(false);
...
void US_RotorCalibration::calculate() {
  ...
  for (int i = 0; i < reading.size(); i++) {
    ...
    for (int j = 0; j < reading[i].size(); j++) {
      if ((int)reading[i][j] != 0) ts << reading[i][j] << "\t";
    }
  }
  ...
  for (int i = 0; i < reading.size(); i++) {
    ...
    for (int j = 1; j < reading[i].size(); j++) {
      if ((int)reading[i][j] != 0) ts << reading[i][j] - reading[i][0] << "\t";
    }
  }
  ...
  for (int i = 1; i < speeds.size(); i++) {
    ...
    stretch_factors << sum1 /
                           k;  // save the average of all values for this speed.
  ...
  fileText +=
      "\nBased on these stretching factors, the bottom of each cell and "
      "channel at ";
    ...
    for (int k = 0; k < maxchannel; k++) {
      ...
      for (int i = 0; i < speeds.size(); i++) {
        for (int L = 0; L < avg2.size(); L++) {
          if (avg2[L].rpm == speeds[i] && avg2[L].cell == j &&
            ...
            if ((int)avg2[L].top != 0) {
              sum1 +=
                  avg2[L].top - (coef[1] * speeds[i] + coef[2] * sq(speeds[i]));
              m++;
            }

            if ((int)avg2[L].bottom != 0) {
              sum2 += avg2[L].bottom -
                      (coef[1] * speeds[i] + coef[2] * sq(speeds[i]));
              n++;
            }
          }
        }
      ...
      if (n > 0 && m > 0) {
        fileText +=
            ...
            QString("%1").arg((sum1 / m) + ((sum2 / n) - (sum1 / m)) / 2.0, 0,
                              'e', 5) +
            "\n";
...
void US_RotorCalibration::calc_6channel(void) {
  ...
  for (k = 0; k < bounds.size(); k++) {
    ...
    for (j = 0; j < speeds.size(); j++) {
      ...
      // if (speeds[j] == 7000) qDebug() << "k:" << SE_tmp.channel << "Speed:"
      // << speeds[j] << val1 << val2;
    }
      ...
      for (i = 0; i < avg_multi2.size(); i++) {
        if (avg_multi2[i].rpm == speeds[j] && avg_multi2[i].index == k) {
          ...
          average += avg_multi2[i].avg -
                     (coef[0] + coef[1] * speeds[j] + coef[2] * sq(speeds[j]));
        }
      }
      radavg.push_back(
          average /
          count);  // store all averages for later std. dev. calculation.
      sum += average / count;  // accumulate all stretch-corrected edge radii
                               // for a single egde
    }
  ...
  fileText +=
      "Below is a listing of the edge positions at rest for the 7 calibration "
      "channels (in cm):\n\n";
  ...
  for (int i = 0; i < edge_measured.size(); i++) {
    fileText +=
        ...
        "\n";  // first known edge should be ignored because it is mostly shaded
               // by screwring
  }

  fileText +=
      "\n\nBelow is a listing of the stretching values as a function of "
      "speed:\n\n";
  fileText += "Speed: Stretch (cm): Standard Dev.:\n";
...
void US_RotorCalibration::changeminrpm(QString str) { minrpm = str.toInt(); }
