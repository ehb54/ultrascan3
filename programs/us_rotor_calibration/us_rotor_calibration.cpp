...
US_RotorCalibration::US_RotorCalibration() : US_Widgets() {
   ...
   connect(cb_minrpm, SIGNAL(highlighted(QString)), this, SLOT(changeminrpm(QString)));
   ...
   plot = new US_Plot(
      data_plot, tr("Intensity Data\n(Channel A in red, Channel B in green)"), tr("Radius (in cm)"), tr("Intensity"));
...
#if QT_VERSION < 0x050000
   connect(plot, SIGNAL(zoomed(QwtDoubleRect)), this, SLOT(currentRect(QwtDoubleRect)));
#else
...
void US_RotorCalibration::loadDisk(void) {
   ...
   workingDir = QFileDialog::getExistingDirectory(
      this, tr("US3 Raw Data Directory"), US_Settings::resultDir(), QFileDialog::DontResolveSymlinks);
   ...
   if (allData.isEmpty()) {
      QMessageBox::warning(this, tr("UltraScan Error"), tr("Could not read any data file."));
      return;
   }
   ...
   if (dataType != "RI") {
      if (dataType != "FI") {
         le_instructions->setText(
            tr("Attention - ") + runID + tr(" is not absorbance or fluorescence intensity data,\nit is ") + dataType
            + tr(" data - aborting."));
         return;
      }
   }
...
void US_RotorCalibration::loadDB(void) {
   ...
   if (rawDataIDs.size() < 1) {
      QMessageBox::information(this, tr("Error"), tr("There were no auc files found in the database."));
      return;
   }
   ...
   for (int i = 0; i < rawDataIDs.size(); i++) {
      ...
      if (readStatus != US_DB2::OK) {
         QMessageBox::warning(this, tr("Error"), tr("Error downloading the data.\n") + db.lastError() + "\n" + filename);
         return;
      }
      ...
      if (result != US_DataIO::OK) {
         QMessageBox::warning(
            this, tr("Error"), tr("Could not read data file.\n") + US_DataIO::errorString(result) + "\n" + filename);
         return;
      }
   ...
   pb_reset->setEnabled(true);
   pb_accept->setEnabled(true);
   connect(cb_wavelengths, SIGNAL(currentIndexChanged(int)), this, SLOT(changeLambda(int)));
   next();
}
...
void US_RotorCalibration::plotAll(void) {
   ...
   for (int j = 0; j < allData[ current_triple ].scanData.size(); j++) //all scans in each triple
   {
      ...
      ts << tr("Cell ") << allData[ current_triple ].cell << tr(", Channel ") << allData[ current_triple ].channel
         << tr(", Speed ") << allData[ current_triple ].scanData[ j ].rpm << tr(", Scan ") << j << tr(", GUID ") << guid
         << tr(", Type ") << type;
      //qDebug() << title;
...
void US_RotorCalibration::calculate() {
   ...
   fileText += "Stretch = " + QString("%1").arg(coef[ 0 ], 0, 'e', 5) + " + " + QString("%1").arg(coef[ 1 ], 0, 'e', 5)
               + " rpm + " + QString("%1").arg(coef[ 2 ], 0, 'e', 5) + " rpm^2\n\n";

   fileText += "Below is a listing of the stretching values as a function of speed:\n\n";
   fileText += "Speed: Stretch (cm): Standard Dev.:\n";

   fileText += QString("%1").arg(0, 5, 10) + "   " + QString("%1").arg(0.0, 0, 'e', 5) + "   "
               + QString("%1").arg(0.0, 0, 'e', 5) + "\n";

   for (int i = 0; i < size; i++) {
      fileText += QString("%1").arg(speeds[ i + 1 ], 5, 10) + "   " + QString("%1").arg(y[ i ], 0, 'e', 5) + "   "
   ...
   for (int j = 0; j < cells.size(); j++) {
      for (int k = 0; k < maxchannel; k++) {
         ...
         for (int i = 0; i < speeds.size(); i++) {
            for (int L = 0; L < avg2.size(); L++) {
               if (avg2[ L ].rpm == speeds[ i ] && avg2[ L ].cell == j && avg2[ L ].channel == k) {
                  if (( int ) avg2[ L ].top != 0) {
                     sum1 += avg2[ L ].top - (coef[ 1 ] * speeds[ i ] + coef[ 2 ] * sq(speeds[ i ]));
                     m++;
                  }

                  if (( int ) avg2[ L ].bottom != 0) {
                     sum2 += avg2[ L ].bottom - (coef[ 1 ] * speeds[ i ] + coef[ 2 ] * sq(speeds[ i ]));
                     n++;
                  }
               }
            }
         ...
         fileText += QString("%1").arg(j + 1, 2, 10) + "      " + QString("%1").arg(k + 1, 2, 10) + "      ";
...
void US_RotorCalibration::calc_6channel(void) {
   ...
   for (int i = 0; i < edge_measured.size(); i++) {
      fileText += QString("%1").arg(i + 1, 3) + "   " + QString("%1").arg(edge_measured[ i ], 0, 'f', 5) + "   "
   ...
   for (int i = 0; i < numspeeds; i++) {
      fileText += QString("%1").arg(speeds[ i ], 5, 10) + "   " + QString("%1").arg(y[ i ], 0, 'e', 5) + "   "
                  + QString("%1").arg((sd1[ i ] - y[ i ]) / 2.0, 0, 'e', 5) + "\n";
   }
   pb_calculate->setEnabled(false);
}

double US_RotorCalibration::findAverage(QwtDoubleRect rect, US_DataIO::RawData data, int i) {
   double average = 0.0;
   int j = 0, k = 0;
   while (data.xvalues[ j ] < rect.right() && j < data.xvalues.size() - 1) {
      ...
      if (
         data.xvalues[ j ] > rect.left() && data.scanData[ i ].rvalues[ j ] < rect.bottom()
...
void US_RotorCalibration::save(void) {
   ...
   if (db.lastErrno() == US_DB2::NOROWS) {
      QMessageBox::information(this, tr("Error"), tr("The current runID cannot be found in the database"));
...
void US_RotorCalibration::findTriple() {
   ...
   while (current_triple < allData.size()) {
      if (current_cell == allData[ current_triple ].cell && current_channel == ( QString ) allData[ current_triple ].channel) {
         break;
      }
      current_triple++;
   }
