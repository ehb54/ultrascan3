...
US_RotorCalibration::US_RotorCalibration () : US_Widgets ()
{
  ...
  connect (cb_minrpm, SIGNAL (highlighted (QString)), this,
           SLOT (changeminrpm (QString)));

  QGridLayout *lo_6channel
      = us_checkbox (tr ("Use 7-Slot Cal. Mask"), cb_6channel, false);
  ...
  QGridLayout *lo_bottom
      = us_radiobutton (tr ("Bottom of channel"), rb_bottom);
  connect (rb_bottom, SIGNAL (clicked ()), SLOT (update_position ()));
  top->addLayout (lo_bottom, row++, 0);

  QGridLayout *lo_assigned
      = us_checkbox (tr ("Limits are assigned"), cb_assigned, false);
...
#if QT_VERSION < 0x050000
  connect (plot, SIGNAL (zoomed (QwtDoubleRect)), this,
           SLOT (currentRect (QwtDoubleRect)));
#else
...
void
US_RotorCalibration::use_6channel ()
{
...
US_RotorCalibration::reset ()
{
  ...
  //   connect (ct_channel, SIGNAL(valueChanged (double)), this,
  //   SLOT(update_channel(double))); connect (ct_cell,    SIGNAL(valueChanged
  //   (double)), this, SLOT(update_cell(double)));
...
void
US_RotorCalibration::source_changed (bool db)
{
...
void
US_RotorCalibration::update_disk_db (bool db)
{
...
void
US_RotorCalibration::loadDisk (void)
{
  ...
  files = d.entryList (nameFilters, QDir::Files | QDir::Readable, QDir::Name);
  ...
  for (int i = 0; i < files.size (); i++)
    {
      ...
      if ((part[3] == "A") && (maxchannel < 1))
        maxchannel = 1;
      if ((part[3] == "B") && (maxchannel < 2))
        maxchannel = 2;
      if ((part[3] == "C") && (maxchannel < 3))
        maxchannel = 3;
      if ((part[3] == "D") && (maxchannel < 4))
        maxchannel = 4;
      if ((part[3] == "E") && (maxchannel < 5))
        maxchannel = 5;
      if ((part[3] == "F") && (maxchannel < 6))
        maxchannel = 6;
      if ((part[3] == "G") && (maxchannel < 7))
        maxchannel = 7;
      if ((part[3] == "H") && (maxchannel < 8))
        maxchannel = 8;
...
void
US_RotorCalibration::loadDB (void)
{
  US_Passwd pw;
  ...
  for (int i = 0; i < rawDataIDs.size (); i++)
    {
      ...
      int readStatus = db.readBlobFromDB (
          filename, QString ("download_aucData"), rawDataIDs[i].toInt ());
      ...
      dataType
          = QString (QChar (data.type[0])) + QString (QChar (data.type[1]));
          ...
          if (dataType != "FI")
            {
              QMessageBox::information (
                  this, tr ("Error"),
                  runID
                      + tr (" is not absorbance or fluorescence intensity "
                            "data,\nit is ")
                      + dataType + tr (" data - aborting."));
              return;
            }
  ...
  pb_reset->setEnabled (true);
  pb_accept->setEnabled (true);
  connect (cb_wavelengths, SIGNAL (currentIndexChanged (int)), this,
           SLOT (changeLambda (int)));
  next ();
}
...
void
US_RotorCalibration::plotAll (void)
{
  dataPlotClear (data_plot);
  ...
  for (int j = 0; j < allData[current_triple].scanData.size ();
       j++) // all scans in each triple
    {
      QString title = "";
      QString guid
          = US_Util::uuid_unparse ((uchar *)allData[current_triple].rawGUID);
      char type[3];
      type[2] = '\0';
      memcpy (type, allData[current_triple].type, 2);
      QTextStream ts (&title);
      ts << tr ("Cell ") << allData[current_triple].cell << tr (", Channel ")
         << allData[current_triple].channel << tr (", Speed ")
         << allData[current_triple].scanData[j].rpm << tr (", Scan ") << j
         << tr (", GUID ") << guid << tr (", Type ") << type;
      // qDebug() << title;
      ...
      if ((QString)allData[current_triple].channel == "B")
        ct_channel->setValue (2);
      if ((QString)allData[current_triple].channel == "C")
        ct_channel->setValue (3);
      if ((QString)allData[current_triple].channel == "D")
        ct_channel->setValue (4);
      if ((QString)allData[current_triple].channel == "E")
        ct_channel->setValue (5);
      if ((QString)allData[current_triple].channel == "F")
        ct_channel->setValue (6);
      if ((QString)allData[current_triple].channel == "G")
        ct_channel->setValue (7);
      if ((QString)allData[current_triple].channel == "H")
        ct_channel->setValue (8);
      connect (ct_channel, SIGNAL (valueChanged (double)), this,
               SLOT (update_channel (double)));
...
// set the flag for this limit set to true. (disabled) Only assign rectangles
// that result from zoom actions when newlimit is true. Set newlimit to false
// immediately afterwards to prevent automatic zoom actions to override a zoom
// limit
void
US_RotorCalibration::currentRect (QwtDoubleRect rect)
{
...
void
US_RotorCalibration::currentRectf (QRectF rectf)
{
  QwtDoubleRect rect;
...
void
US_RotorCalibration::divide (QwtDoubleRect rect)
{
  zoom_mask = rect;
  ...
  if (zoomed)
    {
      le_instructions->setText (
          tr ("Please control-left click between each vertical region, then "
              "click calculate..."));
    }
  pick->disconnect ();
...
void
US_RotorCalibration::mouse (const QwtDoublePoint &p)
{
...
US_RotorCalibration::next ()
{
  if (!top_of_cell)
    {
      ...
      if (current_triple == allData.size ())
        {
          ...
          le_instructions->setText ("All vertical regions have been reviewed, "
                                    "calculating rotor calibration...");
          pb_accept->setEnabled (false);
      ...
      if ((QString)allData[current_triple].channel == "C")
        ct_channel->setValue (3.0);
      if ((QString)allData[current_triple].channel == "D")
        ct_channel->setValue (4.0);
      if ((QString)allData[current_triple].channel == "E")
        ct_channel->setValue (5.0);
      if ((QString)allData[current_triple].channel == "F")
        ct_channel->setValue (6.0);
      if ((QString)allData[current_triple].channel == "G")
        ct_channel->setValue (7.0);
      if ((QString)allData[current_triple].channel == "H")
        ct_channel->setValue (8.0);
      connect (ct_cell, SIGNAL (valueChanged (double)), this,
               SLOT (update_cell (double)));
      connect (ct_channel, SIGNAL (valueChanged (double)), this,
               SLOT (update_channel (double)));
    }
  ...
  pb_accept->setEnabled (true);
  le_instructions->setText (
      tr ("Please zoom the left vertical region and click "
...
US_RotorCalibration::calculate ()
{
  ...
  for (int i = 0; i < allData.size (); i++) // all the triples
    {
      for (int j = 0; j < allData[i].scanData.size ();
           j++) // all scans in each triple
        {
          if ((QString)allData[i].channel == "A")
            tmp_avg.channel = 0;
          if ((QString)allData[i].channel == "B")
            tmp_avg.channel = 1;
          if ((QString)allData[i].channel == "C")
            tmp_avg.channel = 2;
          if ((QString)allData[i].channel == "D")
            tmp_avg.channel = 3;
          if ((QString)allData[i].channel == "E")
            tmp_avg.channel = 4;
          if ((QString)allData[i].channel == "F")
            tmp_avg.channel = 5;
          if ((QString)allData[i].channel == "G")
            tmp_avg.channel = 6;
          if ((QString)allData[i].channel == "H")
            tmp_avg.channel = 7;
          ...
          for (int k = 0; k < maxchannel; k++)
            {
              ...
              for (int L = 0; L < avg.size (); L++)
                {
                  if (avg[L].rpm == speeds[i] && avg[L].cell == cells[j]
                      && avg[L].channel == k)
                    {
                      if (avg[L].top != 0)
                        {
          ...
          for (int k = 0; k < speeds.size (); k++)
            {
              for (int L = 0; L < avg2.size (); L++)
                {
                  if (avg2[L].rpm == speeds[k] && avg2[L].cell == i
                      && avg2[L].channel == j)
                    {
  ...
  for (int i = 1; i < speeds.size (); i++)
    {
      ...
      stretch_factors
          << sum1 / k; // save the average of all values for this speed.
  ...
  // since the calculations were done with the lowest speed (which isn't zero)
  // as a reference, the intercept at rpm=zero should now be negative, which
  // reflects the stretching difference between zero rpm and the lowest speed
  // used here as a reference. To correct for this error, the zeroth-order term
  // needs to be added as an offset to all stretch values and the readings need
  // to be refit, to hopefully give a vanishing zeroth order term.
  ...
  fileText += "Below is a listing of the stretching values as a function of "
              "speed:\n\n";
  ...
  fileText += "\nBased on these stretching factors, the bottom of each cell "
              "and channel at ";
          ...
          for (int i = 0; i < speeds.size (); i++)
            {
              for (int L = 0; L < avg2.size (); L++)
                {
                  if (avg2[L].rpm == speeds[i] && avg2[L].cell == j
                      && avg2[L].channel == k)
                    {
                      if ((int)avg2[L].top != 0)
                        {
  ...
  fileText
      += QString ("%1").arg (center_avg / center_count, 0, 'e', 5) + "\n\n";
}

void
US_RotorCalibration::calc_6channel (void)
{
  QwtDoubleRect rect;
  int i, j, k;
  ...
  for (k = 0; k < bounds.size (); k++)
    {
      ...
      for (i = 0; i < allData.size (); i++) // all the triples
        {
          for (j = 0; j < allData[i].scanData.size ();
               j++) // all scans in each triple
            {
              if ((QString)allData[i].channel == "A")
                tmp_avg_multi.channel = 0;
              if ((QString)allData[i].channel == "B")
                tmp_avg_multi.channel = 1;
              if ((QString)allData[i].channel == "C")
                tmp_avg_multi.channel = 2;
              if ((QString)allData[i].channel == "D")
                tmp_avg_multi.channel = 3;
              if ((QString)allData[i].channel == "E")
                tmp_avg_multi.channel = 4;
              if ((QString)allData[i].channel == "F")
                tmp_avg_multi.channel = 5;
              if ((QString)allData[i].channel == "G")
                tmp_avg_multi.channel = 6;
              if ((QString)allData[i].channel == "H")
                tmp_avg_multi.channel = 7;
  ...
  for (i = 0; i < allData.size (); i++) // all the triples
    {
      for (j = 0; j < allData[i].scanData.size ();
        ...
        {
          if (!speeds.contains ((int)allData[i].scanData[j].rpm)
              && allData[i].scanData[j].rpm >= minrpm)
            {
              speeds << (int)allData[i].scanData[j].rpm;
            }
        }
      ...
      for (j = 0; j < speeds.size (); j++)
        {
          ...
          // if (speeds[j] == 7000) qDebug() << "k:" << SE_tmp.channel <<
          // "Speed:" << speeds[j] << val1 << val2;
        }
      SE_v.push_back (SE_tmp);
    }

  // now calculate the standard deviation from each speed step over all
  // available channels numspeeds contains the number of different speeds
  // available for each channel channels = SE_v.size() and contains the number
  ...
  for (k = 0; k < bounds.size (); k++) // bounds equals the number of edges
    {
          ...
          for (i = 0; i < avg_multi2.size (); i++)
            {
              if (avg_multi2[i].rpm == speeds[j] && avg_multi2[i].index == k)
                {
                  count++;
                  average += avg_multi2[i].avg
                             - (coef[0] + coef[1] * speeds[j]
                                + coef[2] * sq (speeds[j]));
                }
            }
          radavg.push_back (
              average
              / count); // store all averages for later std. dev. calculation.
          sum += average / count; // accumulate all stretch-corrected edge
                                  // radii for a single egde
        }
      ...
      // qDebug() << "Edge" << k << edge_measured[k] << "+/-" <<
      // pow(sigma_sum/speeds.size(), 0.5) << "predicted:" << edge_known[k] <<
      // "difference:" << edge_known[k]-edge_measured[k];
    }
  ...
  fileText = "CALIBRATION REPORT FOR ROTOR: " + rotor
             + "\nPERFORMED ON: " + now.toString ();
  fileText += "\n\nCalibration is based on data from run: " + runID + " at "
              + cb_wavelengths->currentText () + " nm";
  fileText += "\n\nThe following equation was fitted to the measured "
              "stretch values for this rotor:\n\n";
  ...
  fileText += "Below is a listing of the edge positions at rest for the 7 "
              "calibration channels (in cm):\n\n";
  ...
  for (int i = 0; i < edge_measured.size (); i++)
    {
      fileText += QString ("%1").arg (i + 1, 3) + "   "
                  ...
                  + QString ("%1").arg (edge_known[i + 1] - edge_measured[i],
                                        0, 'e', 5)
                  + "\n"; // first known edge should be ignored because it is
                          // mostly shaded by screwring
    }

  fileText += "\n\nBelow is a listing of the stretching values as a function "
              "of speed:\n\n";
  fileText += "Speed: Stretch (cm): Standard Dev.:\n";
...
double
US_RotorCalibration::findAverage (QwtDoubleRect rect, US_DataIO::RawData data,
                                  int i)
{
  double average = 0.0;
  int j = 0, k = 0;
  while (data.xvalues[j] < rect.right () && j < data.xvalues.size () - 1)
    {
      // Note: rect.bottom() is actually the upper limit of the bounding
      // rectangle, and rect.top() is the lower limit of the bounding rectangle
      // - weird screen coordinates?
...
US_RotorCalibration::save (void)
{
  ...
  q << runID << QString::number (US_Settings::us_inv_ID ());
  db.query (q);
  ...
  q.clear ();
  q << "get_rotor_info" << QString::number (current.rotorID);
  db.query (q);
  db.next ();
  current.rotorGUID = db.value (0).toString ();

  US_RotorGui *rotorDialog
      = new US_RotorGui (current,                  // calibration data
...
void
US_RotorCalibration::findTriple ()
{
  pb_accept->setEnabled (true);
  current_triple = 0;
...
void
US_RotorCalibration::update_used ()
{
  cb_assigned->setChecked (false);
  if (top_of_cell)
    {
...
void
US_RotorCalibration::update_plot ()
{
  newlimit = false;
  plot->btnZoom->setChecked (false);
  plotAll ();
...
void
US_RotorCalibration::update_cell (double val)
{
...
void
US_RotorCalibration::update_channel (double val)
{
  if ((int)val == 1)
    {
      current_channel = "A";
...
void
US_RotorCalibration::update_position ()
{
...
void
US_RotorCalibration::changeLambda (int l)
{
  current_triple = l;
  zoomed = false;
  x.clear ();
...
void
US_RotorCalibration::changeminrpm (QString str)
{
  minrpm = str.toInt ();
}
