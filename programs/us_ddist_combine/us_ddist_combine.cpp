...
US_DDistr_Combine::US_DDistr_Combine(const QString auto_mode) : US_Widgets() {
  ...
  QLayout* lo_mdltype = us_checkbox(
      tr("Use model descriptions for list and legend"), ck_mdltype, false);
  ...
  connect(ck_2dsamcmw, SIGNAL(stateChanged(int)), this,
          SLOT(methodChanged(int)));
  connect(ck_2dsagl, SIGNAL(stateChanged(int)), this, SLOT(methodChanged(int)));
  connect(ck_2dsaglmc, SIGNAL(stateChanged(int)), this,
          SLOT(methodChanged(int)));
  connect(ck_2dsacg, SIGNAL(stateChanged(int)), this, SLOT(methodChanged(int)));
  connect(ck_2dsacgit, SIGNAL(stateChanged(int)), this,
          SLOT(methodChanged(int)));
  connect(ck_2dsacgfm, SIGNAL(stateChanged(int)), this,
          SLOT(methodChanged(int)));
  connect(ck_2dsacgmc, SIGNAL(stateChanged(int)), this,
          SLOT(methodChanged(int)));
  ...
  connect(ck_pcsaismc, SIGNAL(stateChanged(int)), this,
          SLOT(methodChanged(int)));
  connect(ck_pcsaslmc, SIGNAL(stateChanged(int)), this,
          SLOT(methodChanged(int)));
  connect(ck_pcsadsmc, SIGNAL(stateChanged(int)), this,
          SLOT(methodChanged(int)));
  connect(ck_pcsahlmc, SIGNAL(stateChanged(int)), this,
          SLOT(methodChanged(int)));
  connect(ck_pcsaistr, SIGNAL(stateChanged(int)), this,
          SLOT(methodChanged(int)));
  connect(ck_pcsasltr, SIGNAL(stateChanged(int)), this,
          SLOT(methodChanged(int)));
  connect(ck_pcsadstr, SIGNAL(stateChanged(int)), this,
          SLOT(methodChanged(int)));
  connect(ck_pcsahltr, SIGNAL(stateChanged(int)), this,
          SLOT(methodChanged(int)));
  connect(ck_pcsa2o, SIGNAL(stateChanged(int)), this, SLOT(methodChanged(int)));
  connect(ck_pcsa2omc, SIGNAL(stateChanged(int)), this,
          SLOT(methodChanged(int)));
  connect(ck_pcsa2otr, SIGNAL(stateChanged(int)), this,
          SLOT(methodChanged(int)));
  connect(ck_dtall, SIGNAL(stateChanged(int)), this,
          SLOT(allMethodChanged(int)));
  ...
  connect(lw_runids, SIGNAL(currentRowChanged(int)), this,
          SLOT(runid_select(int)));
  connect(lw_models, SIGNAL(currentRowChanged(int)), this,
          SLOT(model_select(int)));

  QBoxLayout* plot =
      new US_Plot(data_plot1, tr("Discrete s20,W Distributions"),
  ...
  if (a_mode.isEmpty()) data_plot1->insertLegend(legend, QwtPlot::BottomLegend);
...
QList<QStringList> US_DDistr_Combine::load_auto(QStringList runids_passed,
                                                QStringList aDescrs_passed) {
  QStringList runids;
  ...
  for (int ii = 0; ii < distros.size(); ii++) {
    ...
    if (distros[ii].runID == runID ||
        distros[ii].runID.startsWith(
            grunID)) {  // Only (possibly) add item with matching run ID
...
void US_DDistr_Combine::load(void) {
  ...
  if (dbg_level > 0) {
    ...
    for (int ii = 0; ii < aDescrs.count(); ii++) {
      ...
      DbgLv(1) << "  ii" << ii << "mrun" << mrun << "mgid" << mgid << "mdes"
               << mdes << "iter" << iter << "ddes" << ddes;
    }
  }
  //*DEBUG*
  ...
  for (int ii = 0; ii < nrunids;
       ii++) {  // Add run IDs to list and to project combo box
  ...
  for (int ii = 0; ii < distros.count();
       ii++) {  // Build a list of unique methods of distros
    QString method =
        distros[ii].mdescr.section(".", -1, -1).section("_", -3, -3);
    DbgLv(1) << "ii method" << ii << method << "mdes" << distros[ii].mdescr;
    if (!methods.contains(method)) methods << method;
  }
  ...
  bool hv_2dsagl =
      methods.contains("2DSA-GL") || methods.contains("2DSA-GL-SG") ||
      methods.contains("2DSA-GL-SC") || methods.contains("2DSA-GL-VR") ||
      methods.contains("2DSA-GL-IT") || methods.contains("2DSA-GL-SG-IT") ||
      methods.contains("2DSA-GL-SC-IT") || methods.contains("2DSA-GL-VR-IT");
  bool hv_2dsaglmc =
      methods.contains("2DSA-GL-MC") || methods.contains("2DSA-GL-SG-MC") ||
  ...
  bool hv_gagl = methods.contains("GA-GL") || methods.contains("GA-GL-SG") ||
                 methods.contains("GA-GL-SC") || methods.contains("GA-GL-VR");
  bool hv_gaglmc =
      methods.contains("GA-GL-MC") || methods.contains("GA-GL-SG-MC") ||
      methods.contains("GA-GL-SC-MC") || methods.contains("GA-GL-VR-MC");
  bool hv_pcsais =
      methods.contains("PCSA-IS") || methods.contains("PCSA-GL-SG-IS") ||
      methods.contains("PCSA-GL-SC-IS") || methods.contains("PCSA-GL-VR-IS");
  bool hv_pcsasl =
      methods.contains("PCSA-SL") || methods.contains("PCSA-GL-SG-SL") ||
      methods.contains("PCSA-GL-SC-SL") || methods.contains("PCSA-GL-VR-SL");
  bool hv_pcsads =
      methods.contains("PCSA-DS") || methods.contains("PCSA-GL-SG-DS") ||
      methods.contains("PCSA-GL-SC-DS") || methods.contains("PCSA-GL-VR-DS");
  bool hv_pcsahl =
      methods.contains("PCSA-HL") || methods.contains("PCSA-GL-SG-HL") ||
  ...
  bool hv_dmgagl =
      methods.contains("DMGA-GL") || methods.contains("DMGA-GL-SG") ||
      methods.contains("DMGA-GL-SC") || methods.contains("DMGA-GL-VR");
  bool hv_dmgaglmc =
      methods.contains("DMGA-GL-MC") || methods.contains("DMGA-GL-SG-MC") ||
  ...
  for (int ii = fckrow; ii < lckrow;
       ii++) {  // Examine the leftLayout rows with checkboxes for row we can
                // hide
    int nenab = 0;           // Number of enabled checkboxes in row
    QList<QWidget*> rwidgs;  // List of row widgets (checkboxes)

    for (int jj = 0; jj < nllcol;
         jj++) {  // Examine columns in the row for widgets
      ...
      if (litem != NULL) {  // Check a layout for a widget child
        ...
        if (layo != NULL && widg0 == NULL &&
            layo->count() >
                1) {  // Item is a layout:  check that its child is a widget
    ...
    if (nenab == 0) {  // If this row had no enabled checkboxes, hide the row
      ...
      for (int jj = 0; jj < rwidgs.count();
           jj++) {  // Hide each widget in the row
      ...
      for (int jj = 0; jj < rwidgs.count();
           jj++) {  // Show each widget in the row
...
void US_DDistr_Combine::plot_distr_auto(DistrDesc ddesc, QString distrID,
  ...
  if (sigma_p > 0.0) {
    ...
    if (ind_distro)
      ndispt = envel_data_auto(ddesc.xvals, ddesc.yvals, xenv, yenv, sigma_p,
                               xmin_p, xmax_p);
  ...
  // add ranges ///////////////////////////////////////////////////////////
  // Also, include info on vertical lines (integration limits) -- passed also
  // from c_parms[ "integration_limits" ]
  ...
  if (!ranges_p.isEmpty()) {
    ...
    for (int i = 0; i < c_ranges.size(); ++i) {
...
#else
      QwtScaleDiv* y_axis =
          (QwtScaleDiv*)&(data_plot1->axisScaleDiv(QwtPlot::yLeft));
#endif
      ...
      while (color_index >= ncolors) color_index -= ncolors;
...
void US_DDistr_Combine::save(void) {
  ...
  for (int ii = 0; ii < pdistrs.size(); ii++) {
    ...
    if (oproj == "All") {  // If save-plot project is "All", save lists of
                           // runids and indexes
    ...
    else if (prun == runID) {  // If save-plot project matches current run, save
                               // it and its index
      prunids << prun;
      prndxs << ii;
      break;
  ...
  while (iruns < nruns) {
    ...
    if (dkdb_cntrls->db()) {
      ...
      while (db.next()) {
        ...
        DbgLv(1) << "SV:   rawID" << rawID << "efname" << efname << "trip1"
                 << trip1;
        // Save rawID when we have found a triple match
        if (efname.contains(trip1)) break;
      }
      ...
      while (db.next()) {
        ...
        if (efname.contains(
                trip1)) {  // Keep saving editID from last triple match
          editID = aeditID;
      ...
      DbgLv(1) << "SV: editID idEdit" << editID << idEdit << "  eeditID"
               << eeditID;
      ...
      DbgLv(1) << "SV:runID" << runID << "idEdit" << idEdit << "fnamlst"
               << fnamlst << "trdesc" << trdesc;
...
void US_DDistr_Combine::runid_select(int row) {
  ...
  for (int ii = 0; ii < distros.size(); ii++) {
    ...
    if (distros[ii].runID == runID ||
      ...
      if (mfilter) {  // If method-filtering, skip any item whose method is not
                      // checked
        QString meth = mdesc.section(".", -1, -1).section("_", -3, -3);
        DbgLv(1) << "RunIDSel:    meth" << meth;
        if (!methods.contains(meth)) continue;
...
QMap<QStringList, QList<QColor> > US_DDistr_Combine::model_select_auto(
  ...
  if (!pdisIDs.contains(
          distrID)) {  // If this distro not yet filled out, do so now
  ...
  te_status->setText(
      tr("Count of plotted distributions: %1.").arg(pdistrs.count()));
  ...
  return s_type_map;
  // return pdisIDs;
  // ALEXEY: also need to return color corresponsing to each plotted curve:
  // (QColor) pdistrs[ ii ].color Possibly, return a structure {QstringList
  // pdisIDS, QList<QColor>  pdistrs[ ii ].color }
}
...
QString US_DDistr_Combine::distribID(QString mdescr, QString ddescr) {
  ...
  iterID = (method != "2DSA-FM") ? iterID.section("_", -2, -2)
  ...
  for (int ii = 0; ii < distros.size();
       ii++) {  // Find index of full model description match as distinguishing
                // suffix
...
void US_DDistr_Combine::fill_in_desc(DistrDesc& ddesc, int distx) {
  if (ddesc.xvals.size() > 0 && ddesc.model.components.size() > 0 &&
      ddesc.xtype == xtype)
    return;
  ...
  for (int jj = 1; jj < ncomps; jj++) {
    ...
    if (equivalent(xval, xvpr,
                   epsilon)) {  // Effectively equal x values:  sum y values
      yval += yvpr;
  ...
  for (int jj = 0; jj < nn; jj++) {
    if (ddesc.yvals[jj] !=
        0.0) {  // Move point to next non-zero output location
...
void US_DDistr_Combine::write_data(QString& dataFile, QString& listFile,
  ...
  for (int jj = 0; jj < maxnvl;
       jj++) {  // Build and write xvalue+concentration data line
    line = "";
  ...
  for (int ii = 0; ii < nplots;
       ii++) {  // Build and write each long-description line
    line = pdlong[ii] + "\n";
    tsl << line;
  }
...
int US_DDistr_Combine::reportDocsFromFiles(QString& runID, QString& fdir,
  ...
  if (dbg_level > 0) {
    ...
    for (int ii = 0; ii < freport.triples.size(); ii++) {
      ...
      DbgLv(1) << "DFF:  triple" << ii << "docssize" << ndoc << "ID"
               << freport.triples[ii].tripleID << "triple"
               << freport.triples[ii].triple;
      int jj = ndoc - 1;
      if (ndoc > 0) {
        DbgLv(1) << "DFF:    doc" << 0 << "ID"
                 << freport.triples[ii].docs[0].documentID << "label"
                 << freport.triples[ii].docs[0].label;
        DbgLv(1) << "DFF:    doc" << jj << "ID"
                 << freport.triples[ii].docs[jj].documentID << "label"
    ...
    if (ndx >= 0) {
      ...
      DbgLv(1) << "DFF:  triple" << ndx << "docs size" << ndoc << "ID"
               << freport.triples[ndx].tripleID << "triple"
               << freport.triples[ndx].triple;
      if (ndoc > 0) {
        DbgLv(1) << "DFF:    doc" << 0 << "ID"
                 << freport.triples[ndx].docs[0].documentID << "label"
                 << freport.triples[ndx].docs[0].label;
        int jj = ndoc - 1;
        DbgLv(1) << "DFF:    doc" << jj << "ID"
                 << freport.triples[ndx].docs[jj].documentID << "label"
...
void US_DDistr_Combine::methodChanged(int state) {
  if (state == Qt::Unchecked) ck_dtall->setChecked(false);
...
void US_DDistr_Combine::changedPlotX(bool on_state) {
  ...
  if (npdis > 0) {  // Re-do plot distros to account for X-type change
    ...
    for (int ii = 0; ii < npdis;
         ii++) {  // Build rudimentary plot distros without value arrays
      pddist = &pdistrs[ii];
...
// React to a change in the X type of plots
QMap<QStringList, QList<QColor> > US_DDistr_Combine::changedPlotX_auto(
    int type, QMap<QString, QString> c_parms) {
  xtype = type;
...
int US_DDistr_Combine::envel_data(QVector<double>& xvals,
                                  QVector<double>& yvals,
                                  QVector<double>& xenvs,
                                  QVector<double>& yenvs) {
  int arrsize = 300;
  ...
  for (int jj = 0; jj < vCount;
       jj++) {  // Get min,max of x values (e.g., sedimentation coefficients)
  ...
  for (int kk = 0; kk < arrsize;
       kk++) {                 // Loop to compute envelope grid Y values
    double xval_env = xe[kk];  // Envelope X value
    double yval_env = 0.0;     // Initial envelope Y value

    for (int jj = 0; jj < vCount;
         jj++) {                 // Accumulate Gaussian Y's from each solute
  ...
  double yemx = -1e+30;
  double xemx = -1e+30;
  for (int kk = 0; kk < arrsize; kk++) {  // Normalize Y values so integral is
                                          // equal to total concentration
    ye[kk] *= scale;
    if (ye[kk] > yemx) {
      yemx = ye[kk];
      xemx = xe[kk];
  ...
  DbgLv(1) << "ED: Final esum" << env_sum << "csum" << con_sum << "xemx yemx"
           << xemx << yemx << "sigma  vcount" << sigma << vCount;

  return arrsize;  // Return size of arrays
}

// Generate envelope data: copy for GMP Report
int US_DDistr_Combine::envel_data_auto(QVector<double>& xvals,
                                       QVector<double>& yvals,
                                       QVector<double>& xenvs,
                                       QVector<double>& yenvs,
                                       double sigma_passed, double xmin_passed,
                                       double xmax_passed) {
  int arrsize = 300;
  int vCount = xvals.size();
  double min_xval = 1.0e+50;
  ...
  // Use passed xmin/xmax values:           //ALEXEY <----------- here, use
  // pre-defined parms!!!
  ...
  // double sigma     = ct_sigma->value();
  double sigma =
      sigma_passed;  // ALEXEY <----------- here, use pre-defined parms!!!
