...
US_DDistr_Combine::US_DDistr_Combine (const QString auto_mode) : US_Widgets ()
{
  ...
  connect (ck_2dsa, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_2dsait, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_2dsamc, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_2dsamw, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_2dsamcmw, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_2dsagl, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_2dsaglmc, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_2dsacg, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  ...
  connect (ck_2dsacgmc, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_2dsafm, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_ga, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_gamc, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_gamw, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_gamcmw, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_gagl, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_gaglmc, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_pcsais, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_pcsasl, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_pcsads, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_pcsahl, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  ...
  connect (ck_pcsahltr, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  connect (ck_pcsa2o, SIGNAL (stateChanged (int)), this,
           SLOT (methodChanged (int)));
  ...
  connect (rb_pltsw, SIGNAL (toggled (bool)), this,
           SLOT (changedPlotX (bool)));
  connect (rb_pltMW, SIGNAL (toggled (bool)), this,
           SLOT (changedPlotX (bool)));
  connect (rb_pltDw, SIGNAL (toggled (bool)), this,
           SLOT (changedPlotX (bool)));
  connect (rb_pltff0, SIGNAL (toggled (bool)), this,
           SLOT (changedPlotX (bool)));
  connect (rb_pltvb, SIGNAL (toggled (bool)), this,
           SLOT (changedPlotX (bool)));
  connect (rb_pltMWl, SIGNAL (toggled (bool)), this,
           SLOT (changedPlotX (bool)));

  connect (ct_sigma, SIGNAL (valueChanged (double)), this,
           SLOT (envvalChange ()));
  connect (le_plxmin, SIGNAL (editingFinished ()), this,
           SLOT (envvalChange ()));
  connect (le_plxmax, SIGNAL (editingFinished ()), this,
           SLOT (envvalChange ()));

  connect (ck_mdltype, SIGNAL (stateChanged (int)), this,
           SLOT (ltypeChanged ()));
...
QList<QStringList>
US_DDistr_Combine::load_auto (QStringList runids_passed,
  ...
  for (int ii = 0; ii < distros.size (); ii++)
    {
      ...
      if (distros[ii].runID == runID || distros[ii].runID.startsWith (grunID))
        { // Only (possibly) add item with matching run ID
          ...
          // if ( mfilter )
          //   {  // If method-filtering, skip any item whose method is not
          //   checked
          //     QString meth = mdesc.section( ".", -1, -1 ).section( "_", -3,
          //     -3 ); DbgLv(1) << "RunIDSel:    meth" << meth; if ( !
          //     methods.contains( meth ) )  continue;
          //   }
          ...
          // qDebug() << "load_auto: distribID() -- " << distribID( mdesc,
          // ddesc );
        }
    }
...
QwtPlot *
US_DDistr_Combine::rp_data_plot1 ()
{
...
// reset data_plot1
void
US_DDistr_Combine::reset_data_plot1 ()
{
  xtype = 0;
  pdistrs.clear ();
  pdisIDs.clear ();
...
US_DDistr_Combine::load (void)
{
  ...
  connect (&srdiag, SIGNAL (changed (bool)), this,
           SLOT (update_disk_db (bool)));
  srdiag.exec ();
                ...
                { // Item is a layout:  check that its child is a widget
                  ...
                  if (widg1 != NULL)
                    {                  // Save widget and test if it is enabled
                      ...
                      rwidgs << widg1;
                      bool enab
                          = widg1->isEnabled (); // Test if widget enabled
...
void
US_DDistr_Combine::reset_data (void)
{
  distros.clear ();
  distIDs.clear ();
  aDescrs.clear ();
...
void
US_DDistr_Combine::reset_plot (void)
{
  data_plot1->setCanvasBackground (QBrush (Qt::white));
...
void
US_DDistr_Combine::plot_data_auto (QMap<QString, QString> c_parms)
{
...
// Plot all data
void
US_DDistr_Combine::plot_data (void)
{
  DbgLv (1) << "pDa:  xtype" << xtype;
  dataPlotClear (data_plot1);
  data_plot1->setCanvasBackground (QBrush (Qt::white));
...
void
US_DDistr_Combine::plot_distr (DistrDesc ddesc, QString distrID)
{
...
void
US_DDistr_Combine::plot_distr_auto (DistrDesc ddesc, QString distrID,
  ...
  if (sigma_p > 0.0)
    {
      ...
      if (ind_distro)
        ndispt = envel_data_auto (ddesc.xvals, ddesc.yvals, xenv, yenv,
                                  sigma_p, xmin_p, xmax_p);
  ...
  if (!ranges_p.isEmpty ())
    {
      ...
      qDebug () << "c_ranges, c_ranges.size() --" << c_ranges
                << c_ranges.size ();
      ...
      if (dkdb_cntrls->db ())
        {
          ...
          QString trdesc
              = "Combined Analyses (" + trfirst + "..." + trlast + ")";
          ...
          while (db.next ())
            {
              ...
              DbgLv (1) << "SV:   rawID" << rawID << "efname" << efname
                        << "trip1" << trip1;
          ...
          int st
              = reportDocsFromFiles (runID, fdir, rfiles, &db, idEdit, trdesc);
...
// RunID selected
void
US_DDistr_Combine::runid_select (int row)
{
...
QMap<QStringList, QList<QColor> >
US_DDistr_Combine::model_select_auto (QString modelNameMod,
                                      QMap<QString, QString> c_parms)
{
...
void
US_DDistr_Combine::model_select (int row)
{
...
void
US_DDistr_Combine::setColor (DistrDesc &ddesc, int distx)
{
  possibleColors (); // Make sure possible colors exist
...
void
US_DDistr_Combine::possibleColors ()
{
  if (colors.size () > 0)
    return;
...
QString
US_DDistr_Combine::distribID (QString mdescr, QString ddescr)
{
  const int mxrch = 30;
  int mdx = 9999;
  QString distrID;
  ...
  if (ck_mdltype->isChecked ())
    { // Model-description list/legend type
      distrID
          = runID + "." + triple + "." + andate + "_" + method + "_" + iterID;
    }
  else
...
void
US_DDistr_Combine::update_disk_db (bool isDB)
{
...
void
US_DDistr_Combine::fill_in_desc (DistrDesc &ddesc, int distx)
{
...
void
US_DDistr_Combine::write_data (QString &dataFile, QString &listFile, int &irun)
{
  int arrsize = 300;
...
int
US_DDistr_Combine::reportDocsFromFiles (QString &runID, QString &fdir,
...
int
US_DDistr_Combine::distro_by_descr (QString &mdesc)
{
  int index = -1;
...
int
US_DDistr_Combine::distro_by_mguid (QString &mguid)
{
  int index = -1;
...
int
US_DDistr_Combine::distro_by_runid (QString &runid, int first)
{
  int index = -1;
...
void
US_DDistr_Combine::update_distros ()
{
  distros.clear ();
...
void
US_DDistr_Combine::methodChanged (int state)
{
...
void
US_DDistr_Combine::allMethodChanged (int state)
{
  if (state == Qt::Checked)
    {
...
void
US_DDistr_Combine::list_distributions ()
{
...
void
US_DDistr_Combine::changedPlotX (bool on_state)
{
  if (!on_state)
    return;
...
// React to a change in the X type of plots
QMap<QStringList, QList<QColor> >
US_DDistr_Combine::changedPlotX_auto (int type, QMap<QString, QString> c_parms)
{
  xtype = type;
...
void
US_DDistr_Combine::ltypeChanged ()
{
...
bool
US_DDistr_Combine::equivalent (double aa, double bb, double eps)
{
...
int
US_DDistr_Combine::envel_data (QVector<double> &xvals, QVector<double> &yvals,
...
int
US_DDistr_Combine::envel_data_auto (QVector<double> &xvals,
                                    QVector<double> &yvals,
                                    QVector<double> &xenvs,
                                    QVector<double> &yenvs,
...
void
US_DDistr_Combine::envvalChange ()
{
