   ...
   private:
      ...
      class DistrDesc
      {
         public:
            ...
            QVector< double >  xvals;    // Distribution X values (e.g., sed.coeffs)
            QVector< double >  yvals;    // Distribution Y values (concentrations)
            QColor             color;    // Curve color
            int                iters;    // Iterations flag (>0 means MC)
            int                xtype;    // X type flag (0-4 for x,MW,D,ff0,vbar)
      ...
      QStringList    aDescrs;          // All model descr. strings
