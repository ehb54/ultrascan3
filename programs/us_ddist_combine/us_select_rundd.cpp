...
void
US_SelectRunDD::search (const QString &search_string)
{
...
void
US_SelectRunDD::scan_dbase_runs ()
{
  US_Passwd pw;
...
void
US_SelectRunDD::scan_local_runs (void)
{
  QTime timer;
  timer.start ();
  mRDates.clear ();
                  ...
                  if (mdesc.contains ("-MC"))
                    {
                      ...
                      if (kk > 0)
                        {
                          int niters = attr.value ("MCIteration")
                                           .toString ()
                                           .toInt ();
  ...
  for (int ii = 0; ii < aucdirs.count (); ii++)
    { // Examine all the AUC files that exist locally
      ...
      QString fdate
          = QFileInfo (subdir).lastModified ().toString ("yyMMddhhmm");
...
void
US_SelectRunDD::update_person (int ID)
{
...
void
US_SelectRunDD::update_disk_db (bool isDB)
{
  emit changed (isDB);
...
void
US_SelectRunDD::selectionChanged ()
{
  QList<QListWidgetItem *> selitems = lw_data->selectedItems ();
...
void
US_SelectRunDD::scan_dbase_models ()
{
  US_Passwd pw;
  US_DB2 db (pw.getPasswd ());
...
void
US_SelectRunDD::scan_local_models ()
{
  ...
  for (int rr = 0; rr < runIDs.count (); rr++)
    {
      ...
      nrmods += db.functionQuery (query);
      // DbgLv(1) << "KntM:   rr" << rr << "nrmods" << nrmods << "runid" <<
      // runid;
...
void
US_SelectRunDD::sort_rlabels (const QStringList rlabs)
{
  slabels.clear ();
