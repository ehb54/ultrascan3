...
void US_SelectRunDD::accepted() {
  ...
  if (namodel == 0) {
    QMessageBox::warning(this, tr("No Implied Models"),
...
void US_SelectRunDD::scan_dbase_runs() {
  ...
  mcounted = false;
  DbgLv(1) << "ScDB:counts: runIDs" << runIDs.count() << "rlabels"
           << rlabels.count();
...
void US_SelectRunDD::scan_local_runs(void) {
  ...
  for (int ii = 0; ii < f_names.size(); ii++) {  // Examine each model file
    ...
    while (!xml.atEnd()) {
      ...
      if (xml.isStartElement()) {
        if (xml.name() ==
            "model") {  // Get the description and GUID of model and test it
          ...
          if (ddesc.isEmpty()) {
            if (ddmap.contains(eGUID)) ddesc = ddmap[eGUID];
          } else {
            if (!ddmap.contains(eGUID)) ddmap[eGUID] = ddesc;
          }
          ...
          wDescrs << odesc;
          if ((dbg_level > 0) &&
              (!mdesc.contains("-MC_") || mdesc.contains("_mc")))
            DbgLv(1) << "ScLo: odesc" << odesc;
        }

        if (xml.name() == "analyte") break;
      }
    }
  }
  ...
  for (int mm = 0; mm < wDescrs.count(); mm++) {
    ...
    if (ddesc.isEmpty()) {
      ...
      if (ddmap.contains(eGUID)) {
        ddesc = ddmap[eGUID];
        DbgLv(1) << "ScLo: mm" << mm << "eGUID" << eGUID << "new ddesc"
                 << ddesc;
  ...
  for (int ii = 0; ii < aucdirs.count();
       ii++) {  // Examine all the AUC files that exist locally
    ...
    if (mRunIDs.contains(runID)) {  // If this runID is represented for models,
                                    // it is selectable
    ...
    DbgLv(1) << "ScLo:    count_allr" << count_allr << "count_list"
             << count_list << "   runID" << runID;
  }
...
void US_SelectRunDD::selectionChanged() {
  ...
  if (!counted) {
    ...
    kseld = selitems.size();
    if (kseld > 0) lw_data->setCurrentItem(selitems[0]);
    connect(lw_data, SIGNAL(itemSelectionChanged()), this,
            SLOT(selectionChanged()));
  }
...
void US_SelectRunDD::scan_local_models() {
  ...
  for (int mm = 0; mm < aDescrs.count(); mm++) {
    ...
    if (runIDs.contains(runid) || runIDs.startsWith(grunid)) wDescrs << mdesc;
  }
  DbgLv(1) << "ScMl:counts: aDescrs" << aDescrs.count() << "wDescrs"
           << wDescrs.count();
}
...
void US_SelectRunDD::count_models() {
  ...
  DbgLv(1) << "KntM:  counts: runIDs rmodKnts" << runIDs.count()
           << rmodKnts.count();
