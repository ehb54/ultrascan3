...
US_SelectRunDD::US_SelectRunDD(bool dbase, QStringList &runIDs, QStringList &mdescrs) :
    US_WidgetsDialog(0, 0), runIDs(runIDs), mDescrs(mdescrs) {
   sel_db = dbase;
   dbg_level = US_Settings::us_debug();
   nimodel = mDescrs.count();
   mcounted = false;

   setWindowTitle(tr("Select Run ID(s) for Discrete Distributions (%1)").arg(sel_db ? "DB" : "Local"));
   ...
   dkdb_cntrls = new US_Disk_DB_Controls(sel_db ? US_Disk_DB_Controls::DB : US_Disk_DB_Controls::Disk);
   ...
   connect(lw_data, SIGNAL(itemSelectionChanged()), this, SLOT(selectionChanged()));
...
void US_SelectRunDD::list_data() {
   ...
   if (rlabels.size() == 0) { // Report and return now if no items found
   ...
   for (int ii = 0; ii < slabels.size(); ii++) { // Propagate list widget with labels
   ...
   te_status->setText(tr("%1 scanned run IDs were used to derive the list. Of these,\n"
...
void US_SelectRunDD::accepted() {
   ...
   QString slines = te_status->toPlainText() + tr("\nScanning models for selected run(s)...");
...
void US_SelectRunDD::scan_dbase_runs() {
   ...
   while (db.next()) { // Get a list of all runs in the database
...
void US_SelectRunDD::scan_local_runs(void) {
   ...
   QStringList f_names = QDir(mdir).entryList(mfilt, QDir::Files, QDir::Name);

   for (int ii = 0; ii < f_names.size(); ii++) { // Examine each model file
      ...
      while (!xml.atEnd()) {
         ...
         if (xml.isStartElement()) {
            if (xml.name() == "model") { // Get the description and GUID of model and test it
               ...
               if (runid.isEmpty() || runid.length() < 2)
                  continue;
               ...
               if (mdesc.contains("-MC")) {
                  ...
                  if (kk > 0) {
                     int niters = attr.value("MCIteration").toString().toInt();
                     mdesc = QString(mdesc).left(kk) + QString().sprintf("_mcN%03d", niters)
   ...
   for (int ii = 0; ii < aucdirs.count(); ii++) { // Examine all the AUC files that exist locally
      ...
      if (mRunIDs.contains(runID)) { // If this runID is represented for models, it is selectable
      ...
      DbgLv(1) << "ScLo:    count_allr" << count_allr << "count_list" << count_list << "   runID" << runID;
   }
...
void US_SelectRunDD::count_models() {
   ...
   DbgLv(1) << "KntM:  counts: runIDs rmodKnts" << runIDs.count() << rmodKnts.count();
...
void US_SelectRunDD::sort_rlabels(const QStringList rlabs) {
   ...
   for (int ii = 0; ii < keys.count(); ii++) { // Create a list of concatenated date+label strings
   ...
   for (int ii = svals.count() - 1; ii >= 0; ii--) { // Create the sorted labels list (reverse date order)
