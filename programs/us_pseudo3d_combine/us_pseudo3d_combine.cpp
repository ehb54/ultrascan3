...
US_Pseudo3D_Combine::US_Pseudo3D_Combine () : US_Widgets ()
{
  ...
  QBoxLayout *plot = new US_Plot (
      data_plot, tr ("Pseudo-3D Distribution Data"), xa_title, ya_title);
...
void
US_Pseudo3D_Combine::reset_auto (void)
{
  reset ();
}

void
US_Pseudo3D_Combine::reset (void)
{
...
// plot the data
void
US_Pseudo3D_Combine::plot_data (void)
{
  ...
  pick->setTrackerPen (
      QPen (csum > 600 ? QColor (Qt::black) : QColor (Qt::white)));
  ...
  US_SpectrogramData &spec_dat
      = (US_SpectrogramData &)*(d_spectrogram->data ());
#endif
  QwtDoubleRect drect;
      ...
      for (int ii = 0; ii < system.size (); ii++)
        {
          ...
          QList<S_Solute> *sol_z
              = zpcent ? &tsys->sk_distro_zp : &tsys->sk_distro;
  ...
  if (sv_plot)
    { // Automatically save plot image in a PNG file
      ...
      QPixmap plotmap = QPixmap::grabWidget (data_plot, 0, 0, pmsize.width (),
                                             pmsize.height ());
#endif
...
void
US_Pseudo3D_Combine::plot_data (int)
{
  plot_data ();
}

void
US_Pseudo3D_Combine::update_resolu (double dval)
{
  resolu = dval;
}

void
US_Pseudo3D_Combine::update_xreso (double dval)
{
  xreso = dval;
}

void
US_Pseudo3D_Combine::update_yreso (double dval)
{
  yreso = dval;
}

void
US_Pseudo3D_Combine::update_zfloor (double dval)
{
  zfloor = dval;
}

void
US_Pseudo3D_Combine::update_curr_distr (double dval)
{
...
void
US_Pseudo3D_Combine::update_plot_smin (double dval)
{
  plt_smin = dval;
...
void
US_Pseudo3D_Combine::update_plot_smax (double dval)
{
  plt_smax = dval;
...
void
US_Pseudo3D_Combine::update_plot_kmin (double dval)
{
  plt_kmin = dval;
}

void
US_Pseudo3D_Combine::update_plot_kmax (double dval)
{
  plt_kmax = dval;
}

void
US_Pseudo3D_Combine::select_autosxy ()
{
...
void
US_Pseudo3D_Combine::select_autoscz ()
{
...
void
US_Pseudo3D_Combine::select_conloop ()
{
...
void
US_Pseudo3D_Combine::load_distro ()
{
...
void
US_Pseudo3D_Combine::load_distro_auto (QString invID_passed,
...
QwtPlot *
US_Pseudo3D_Combine::rp_data_plot ()
{
...
void
US_Pseudo3D_Combine::load_distro (US_Model model, QString mdescr)
{
  DisSys tsys;
  S_Solute sol_sk;
  S_Solute sol_xy;
  ...
  tsys.analys_name
      = asys.section ("_", 0, -4) + "_" + asys.section ("_", -2, -1);
...
void
US_Pseudo3D_Combine::load_color ()
{
...
void
US_Pseudo3D_Combine::set_limits ()
{
  double smin = 1.0e30;
  double smax = -1.0e30;
  double kmin = 1.0e30;
...
void
US_Pseudo3D_Combine::sort_distro (QList<S_Solute> &listsols, bool reduce)
{
...
void
US_Pseudo3D_Combine::timerEvent (QTimerEvent *event)
{
...
void
US_Pseudo3D_Combine::update_disk_db (bool isDB)
{
...
void
US_Pseudo3D_Combine::select_prefilt (void)
{
  QString pfmsg;
  int nruns = 0;
  pfilts.clear ();
  ...
  if (nruns == 0)
    ...
    pfmsg = tr ("RunID prefilter - 1 run: ") + QString (pfilts[0]).left (20)
            + " ...";
...
void
US_Pseudo3D_Combine::remove_distro (void)
{
...
void
US_Pseudo3D_Combine::select_x_axis_auto (int ival)
{
  select_x_axis (ival);
}
...
void
US_Pseudo3D_Combine::select_y_axis_auto (int ival)
{
  select_y_axis (ival);
}
...
void
US_Pseudo3D_Combine::select_x_axis (int ival)
{
...
void
US_Pseudo3D_Combine::select_y_axis (int ival)
{
...
void
US_Pseudo3D_Combine::build_xy_distro ()
{
  if (system.size () < 1)
    return;
  DisSys *tsys = (DisSys *)&system.at (curr_distr);
...
QString
US_Pseudo3D_Combine::anno_title (int pltndx)
{
  QString a_title;
...
QwtLinearColorMap *
US_Pseudo3D_Combine::ColorMapCopy (QwtLinearColorMap *colormap)
{
