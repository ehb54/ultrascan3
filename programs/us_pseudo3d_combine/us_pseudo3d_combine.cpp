...
US_Pseudo3D_Combine::US_Pseudo3D_Combine() : US_Widgets() {
  ...
  connect(ck_autosxy, SIGNAL(clicked()), this, SLOT(select_autosxy()));

  us_checkbox(tr("Autoscale Z"), ck_autoscz, true);
  connect(ck_autoscz, SIGNAL(clicked()), this, SLOT(select_autoscz()));

  us_checkbox(tr("Continuous Loop"), ck_conloop, true);
  connect(ck_conloop, SIGNAL(clicked()), this, SLOT(select_conloop()));
  ...
  connect(ct_plt_kmin, SIGNAL(valueChanged(double)), this,
          SLOT(update_plot_kmin(double)));
  ...
  connect(ct_plt_kmax, SIGNAL(valueChanged(double)), this,
          SLOT(update_plot_kmax(double)));
  ...
  connect(ct_plt_smin, SIGNAL(valueChanged(double)), this,
          SLOT(update_plot_smin(double)));
  ...
  connect(ct_plt_smax, SIGNAL(valueChanged(double)), this,
          SLOT(update_plot_smax(double)));
  ...
  connect(ct_curr_distr, SIGNAL(valueChanged(double)), this,
          SLOT(update_curr_distr(double)));
  ...
  le_cmap_name =
      us_lineedit(tr("Default Color Map: w-cyan-magenta-red-black"), -1, true);
  ...
  connect(pb_pltall, SIGNAL(clicked()), this, SLOT(plotall()));

  pb_stopplt = us_pushbutton(tr("Stop Plotting Loop"));
  pb_stopplt->setEnabled(false);
  connect(pb_stopplt, SIGNAL(clicked()), this, SLOT(stop()));
  ...
  QBoxLayout* plot = new US_Plot(data_plot, tr("Pseudo-3D Distribution Data"),
                                 xa_title, ya_title);
  ...
  zTitle.setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(),
                       QFont::Bold));
...
void US_Pseudo3D_Combine::reset_auto(void) { reset(); }
...
void US_Pseudo3D_Combine::plot_data(void) {
  ...
  if (curr_distr < 0 ||
      curr_distr >=
          syssiz) {  // current distro index somehow out of valid range
    qDebug() << "curr_distr=" << curr_distr << "  ( sys.size()=" << syssiz
             << " )";
    syssiz--;
  ...
  if (auto_sxy)
    ...
    drect = QwtDoubleRect(plt_smin, plt_kmin, (plt_smax - plt_smin),
                          (plt_kmax - plt_kmin));
  }
  ...
  if (auto_sxy) {  // Auto scale x and y
    data_plot->setAxisAutoScale(QwtPlot::yLeft);
    data_plot->setAxisAutoScale(QwtPlot::xBottom);
  } else {  // Manual limits on x and y
  ...
  rightAxis->setColorMap(QwtInterval(0.0, plt_zmax), ColorMapCopy(colormap));
#endif
  ...
  QString dtext = tr("Run:  ") + tsys->run_name + " (" + tsys->method +
                  ")\n    " + tsys->analys_name;
  ...
  if (sv_plot) {  // Automatically save plot image in a PNG file
    ...
    QPixmap plotmap =
        QPixmap::grabWidget(data_plot, 0, 0, pmsize.width(), pmsize.height());
#endif

    QString runid = tsys->run_name.section(".", 0, -2);
    QString triple = tsys->run_name.section(".", -1, -1);
    QString report =
        QString("pseudo3d_") + s_attrs[plot_x] + "_" + s_attrs[plot_y];

    QString ofdir = US_Settings::reportDir() + "/" + runid;
    QDir dirof(ofdir);
    if (!dirof.exists()) QDir(US_Settings::reportDir()).mkdir(runid);
    QString ofname = tsys->method + "." + triple + "." + report + ".png";
    QString ofpath = ofdir + "/" + ofname;

    plotmap.save(ofpath);
    dtext = dtext + tr("\nPLOT %1 SAVED to local").arg(curr_distr + 1);

    if (dkdb_cntrls->db() &&
        !ck_locsave->isChecked()) {  // Save a copy to the database
...
void US_Pseudo3D_Combine::plot_data(int) { plot_data(); }

void US_Pseudo3D_Combine::update_resolu(double dval) { resolu = dval; }

void US_Pseudo3D_Combine::update_xreso(double dval) { xreso = dval; }

void US_Pseudo3D_Combine::update_yreso(double dval) { yreso = dval; }

void US_Pseudo3D_Combine::update_zfloor(double dval) { zfloor = dval; }
...
void US_Pseudo3D_Combine::update_plot_kmin(double dval) { plt_kmin = dval; }

void US_Pseudo3D_Combine::update_plot_kmax(double dval) { plt_kmax = dval; }
...
void US_Pseudo3D_Combine::load_distro_auto(QString invID_passed,
...
void US_Pseudo3D_Combine::load_distro(US_Model model, QString mdescr) {
  ...
  if (model.monteCarlo) {  // Revisit setting if Monte Carlo
    QString miter = mdescr.section(mdescr.left(1), 6);
    int kiter = miter.isEmpty() ? 0 : miter.toInt();

    if (kiter <
        2) {  // Turn off flag if not composite MC model (is individual MC)
      tsys.monte_carlo = false;
    }
  }

  te_distr_info->setText(tr("Run:  ") + tsys.run_name + " (" + tsys.method +
                         ")\n    " + tsys.analys_name);
  plt_zmin_co = 1e+8;
  plt_zmax_co = -1e+8;
  plt_zmin_zp = 100.0;
  ...
  if (tsys.distro_type != (int)US_Model::COFS) {
    ...
    DbgLv(1) << "LD: zmin zmax totconc" << plt_zmin_co << plt_zmax_co
             << tot_conc;
...
void US_Pseudo3D_Combine::plotall() {
  ...
  if (curr_distr == system.size()) curr_distr--;
...
void US_Pseudo3D_Combine::set_limits() {
  ...
  kmin -= kinc;
  kmax += kinc;
  DbgLv(1) << "SL: adjusted smin smax kmin kmax" << smin << smax << kmin
           << kmax;

  if (auto_sxy) {  // Set auto limits on X and Y
    sinc = pow(10.0, qFloor(log10(smax)) - 3.0);
    kinc = pow(10.0, qFloor(log10(kmax)) - 3.0);
    if (qAbs((smax - smin) / smax) <
        0.001) {  // Put padding around virtually constant value
      smin -= sinc;
      smax += sinc;
    }
    if (qAbs((kmax - kmin) / kmax) <
        0.001) {  // Put padding around virtually constant value
      kmin -= kinc;
      kmax += kinc;
    }
...
void US_Pseudo3D_Combine::sort_distro(QList<S_Solute>& listsols, bool reduce) {
...
void US_Pseudo3D_Combine::timerEvent(QTimerEvent* event) {
  ...
  if (tm_id != patm_id) {  // if other than plot loop timer event, pass on to
                           // normal handler
  ...
  if (syssiz > 0 && looping) {  // If still looping, plot the next distribution
    if (jdistr > maxsiz) {      // If we have passed the end in looping, reset
      jdistr = 0;

      if (!cont_loop) {  // If not in continuous loop, turn off looping flag
        jdistr = curr_distr;
        looping = false;
      }

      else {  // If in continuous loop, turn off save-plot flag
        need_save = false;
      }
    }
    curr_distr = jdistr;
  ...
  if (curr_distr > maxsiz ||
      !looping) {  // If past last distro or Stop clicked, stop the loop
    killTimer(tm_id);
    pb_stopplt->setEnabled(false);
...
void US_Pseudo3D_Combine::select_x_axis_auto(int ival) { select_x_axis(ival); }

// Select coordinate for vertical axis: copy for GMP
void US_Pseudo3D_Combine::select_y_axis_auto(int ival) { select_y_axis(ival); }
...
void US_Pseudo3D_Combine::select_x_axis(int ival) {
  ...
  qDebug() << "Pseudo3D: x_axis changed: ival,  xlabs[ plot_x ] -- " << ival
           << xlabs[plot_x];

  lb_plt_smin->setText(tr("Plot Limit ") + xlabs[plot_x] + tr(" Minimum:"));
  lb_plt_smax->setText(tr("Plot Limit ") + xlabs[plot_x] + tr(" Maximum:"));
...
void US_Pseudo3D_Combine::select_y_axis(int ival) {
  ...
  qDebug() << "Pseudo3D: y_axis changed: ival,  ylabs[ plot_y ] -- " << ival
           << ylabs[plot_y];

  lb_plt_kmin->setText(tr("Plot Limit ") + ylabs[plot_y] + tr(" Minimum:"));
  lb_plt_kmax->setText(tr("Plot Limit ") + ylabs[plot_y] + tr(" Maximum:"));
...
void US_Pseudo3D_Combine::build_xy_distro() {
  if (system.size() < 1) return;
  DisSys* tsys = (DisSys*)&system.at(curr_distr);
  if (tsys->plot_x == plot_x && tsys->plot_y == plot_y) return;
...
QwtLinearColorMap* US_Pseudo3D_Combine::ColorMapCopy(
