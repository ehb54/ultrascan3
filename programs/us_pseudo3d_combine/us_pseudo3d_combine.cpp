...
US_Pseudo3D_Combine::US_Pseudo3D_Combine() : US_Widgets() {
   ...
   connect(ct_plt_kmin, SIGNAL(valueChanged(double)), this, SLOT(update_plot_kmin(double)));
   ...
   connect(ct_plt_kmax, SIGNAL(valueChanged(double)), this, SLOT(update_plot_kmax(double)));
   ...
   connect(ct_plt_smin, SIGNAL(valueChanged(double)), this, SLOT(update_plot_smin(double)));
   ...
   connect(ct_plt_smax, SIGNAL(valueChanged(double)), this, SLOT(update_plot_smax(double)));
   ...
   connect(ct_curr_distr, SIGNAL(valueChanged(double)), this, SLOT(update_curr_distr(double)));

   te_distr_info = us_textedit();
   te_distr_info->setText(tr("Run:  runID.triple (method)\n") + tr("    analysisID"));
   us_setReadOnly(te_distr_info, true);

   le_cmap_name = us_lineedit(tr("Default Color Map: w-cyan-magenta-red-black"), -1, true);
   ...
   QBoxLayout *plot = new US_Plot(data_plot, tr("Pseudo-3D Distribution Data"), xa_title, ya_title);
   ...
   zTitle.setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(), QFont::Bold));
...
void US_Pseudo3D_Combine::plot_data(void) {
   ...
   if (curr_distr < 0 || curr_distr >= syssiz) { // current distro index somehow out of valid range
   ...
   if (zpcent) {
      data_plot->setAxisTitle(QwtPlot::yRight, tr("Percent of Total Concentration"));
   ...
   if (auto_sxy)
      ...
      drect = QwtDoubleRect(plt_smin, plt_kmin, (plt_smax - plt_smin), (plt_kmax - plt_kmin));
   }
...
#if QT_VERSION < 0x050000
   rightAxis->setColorMap(QwtDoubleInterval(0.0, plt_zmax), d_spectrogram->colorMap());
#else
   ...
   bool sv_plot = ck_savepl->isChecked() && ((looping && !ck_conloop->isChecked()) || !looping);
   ...
   if (sv_plot) { // Automatically save plot image in a PNG file
      ...
      QPixmap plotmap = QPixmap::grabWidget(data_plot, 0, 0, pmsize.width(), pmsize.height());
#endif

      QString runid = tsys->run_name.section(".", 0, -2);
      QString triple = tsys->run_name.section(".", -1, -1);
      QString report = QString("pseudo3d_") + s_attrs[ plot_x ] + "_" + s_attrs[ plot_y ];
      ...
      if (dkdb_cntrls->db() && !ck_locsave->isChecked()) { // Save a copy to the database
         ...
         qDebug() << "DB-save: currdist" << curr_distr << "svtime:" << time0.msecsTo(time1);
...
void US_Pseudo3D_Combine::load_distro(US_Model model, QString mdescr) {
   ...
   tsys.method = (mdesc.contains("-CG")) ? tsys.method.replace("2DSA", "2DSA-CG") : tsys.method;
   ...
   if (model.monteCarlo) { // Revisit setting if Monte Carlo
      ...
      if (kiter < 2) { // Turn off flag if not composite MC model (is individual MC)
...
void US_Pseudo3D_Combine::set_limits() {
   ...
   if (auto_sxy) { // Set auto limits on X and Y
      ...
      if (qAbs((smax - smin) / smax) < 0.001) { // Put padding around virtually constant value
         smin -= sinc;
         smax += sinc;
      }
      if (qAbs((kmax - kmin) / kmax) < 0.001) { // Put padding around virtually constant value
         kmin -= kinc;
         kmax += kinc;
      }
   ...
   DbgLv(1) << "SL: plt_smin _smax _kmin _kmax" << plt_smin << plt_smax << plt_kmin << plt_kmax;
}
...
void US_Pseudo3D_Combine::sort_distro(QList<S_Solute> &listsols, bool reduce) {
   ...
   if (reduce) { // skip any duplicates in sorted list
      ...
      while ((++jj) != listsols.end()) { // loop to compare each entry to previous
      ...
      if (kdup > 0) { // if some reduction happened, replace list with reduced version
         double sc = 1.0 / ( double ) (kdup + 1);
         DbgLv(1) << "KDUP" << kdup;
         //sc = 1.0;

         for (int ii = 0; ii < reduced.size(); ii++) { // first scale c values by reciprocal of maximum replicate count
            reduced[ ii ].c *= sc;
         }
...
void US_Pseudo3D_Combine::timerEvent(QTimerEvent *event) {
   ...
   if (tm_id != patm_id) { // if other than plot loop timer event, pass on to normal handler
   ...
   if (curr_distr > maxsiz || !looping) { // If past last distro or Stop clicked, stop the loop
      killTimer(tm_id);
      pb_stopplt->setEnabled(false);
...
void US_Pseudo3D_Combine::select_prefilt(void) {
   ...
   if (nruns == 0)
      ...
      pfmsg = tr("RunID prefilter - %1 runs: ").arg(nruns) + QString(pfilts[ 0 ]).left(20) + "*, ...";
