//! \file us_remove_distros.cpp

#include "us_remove_distros.h"
#include "us_gui_settings.h"

// Constructor:  remove-distributions dialog widget
US_RemoveDistros::US_RemoveDistros(QList<DisSys> &adistros, QWidget *p) : US_WidgetsDialog(p, 0), distros(adistros) {
   ...
   connect(pb_remove, SIGNAL(clicked()), this, SLOT(remove()));
   connect(pb_restore, SIGNAL(clicked()), this, SLOT(restore()));
   connect(pb_help, SIGNAL(clicked()), this, SLOT(help()));
   connect(pb_cancel, SIGNAL(clicked()), this, SLOT(reject()));
   connect(pb_accept, SIGNAL(clicked()), this, SLOT(accepted()));

   connect(lw_distrs, SIGNAL(itemSelectionChanged()), this, SLOT(selectionsChanged()));
   ...
   te_status->setText(tr("The list shows all the original %1 distributions.\n"
...
void US_RemoveDistros::selectionsChanged() {
   // Get the list of selected items
   QList<QListWidgetItem *> selitems = lw_distrs->selectedItems();
   nd_selected = selitems.count();

   if (nd_removed == 0) { // No list entries have thus-far been removed
      if (nd_selected == 0) { // No items removed or selected
         te_status->setText(tr("The list shows all the original %1 distributions.\n"
                               "No models are currently selected for removal.")
                               .arg(nd_orig));
      }

      else if (nd_selected == 1) { // No items removed, but 1 selected
         te_status->setText(tr("The list shows all the original %1 distributions.\n"
                               "One model is currently selected for removal.")
                               .arg(nd_orig));
      }

      else { // No items removed, but several selected
         te_status->setText(tr("The list shows all the original %1 distributions.\n"
   ...
   else { // Some items were previously removed
      if (nd_selected == 0) { // Removals were done, but currently no items selected
         te_status->setText(tr("The list shows distributions with %1 removed.\n"
                               "No more models are currently selected for removal.")
                               .arg(nd_removed));
      }

      else if (nd_selected == 1) { // Removals were done, and currently one more item is selected
         te_status->setText(tr("The list shows distributions with %1 removed.\n"
                               "One more model is currently selected for removal.")
                               .arg(nd_removed));
      }

      else { // Removals were done, and currently some more items are selected
         te_status->setText(tr("The list shows distributions with %1 removed.\n"
...
void US_RemoveDistros::remove() {
   // Get the list-widget items currently selected
   QList<QListWidgetItem *> selitems = lw_distrs->selectedItems();
   nd_selected = selitems.count();
   nd_removed += nd_selected;

   for (int jj = 0; jj < nd_selected; jj++) { // Get a selected item and remove its description from the current list
...
void US_RemoveDistros::accepted() {
   if (nd_selected > 0) { // Accept attempt with selections and no Remove clicked
      if (
         QMessageBox::Yes
         == QMessageBox::warning(
            this, tr("Outstanding Selections"),
            ...
            QMessageBox::Yes)) { // If "Yes" to above, add current selections to the remove list
         remove();
      }
   }

   for (int jj = nd_orig - 1; jj >= 0; jj--) { // Remove all entries in the original that are not in the current list
      QString mdesc = mdesc_orig[ jj ];

      if (!mdesc_list.contains(mdesc)) { // This original list item is to be removed from the passed list
         distros.removeAt(jj);
      }
   }
