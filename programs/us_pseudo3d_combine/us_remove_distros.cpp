...
US_RemoveDistros::US_RemoveDistros(QList<DisSys>& adistros, QWidget* p)
  ...
  connect(lw_distrs, SIGNAL(itemSelectionChanged()), this,
          SLOT(selectionsChanged()));
...
void US_RemoveDistros::selectionsChanged() {
  ...
  if (nd_removed == 0) {     // No list entries have thus-far been removed
    ...
    if (nd_selected ==
        0) {  // Removals were done, but currently no items selected
      te_status->setText(
    ...
    else if (nd_selected == 1) {  // Removals were done, and currently one more
                                  // item is selected
...
void US_RemoveDistros::remove() {
  ...
  for (int jj = 0; jj < nd_selected;
       jj++) {  // Get a selected item and remove its description from the
                // current list
...
void US_RemoveDistros::accepted() {
  if (nd_selected >
      0) {  // Accept attempt with selections and no Remove clicked
    if (QMessageBox::Yes ==
        QMessageBox::warning(
            ...
            QMessageBox::Yes)) {  // If "Yes" to above, add current selections
                                  // to the remove list
      remove();
    }
  }

  for (int jj = nd_orig - 1; jj >= 0;
       jj--) {  // Remove all entries in the original that are not in the
                // current list
    QString mdesc = mdesc_orig[jj];

    if (!mdesc_list.contains(mdesc)) {  // This original list item is to be
                                        // removed from the passed list
      distros.removeAt(jj);
    }
  }
