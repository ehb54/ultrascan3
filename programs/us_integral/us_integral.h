...
class US_Integral : public US_Widgets {
 ...
 private:
  ...
  QVector<QVector<double> >
      v_frats;  //!< Vector of friction ratios per fraction
  QVector<QVector<double> > v_sedcs;  //!< Vector of sedimentation coefficient
                                      //!< vectors per distribution
  QVector<QVector<double> >
      v_difcs;  //!< Vector of diffusion coefficient vectors per distribution
 ...
 private slots:
  ...
  void write_csv(const QString filename, const QString headers,
                 QVector<double>& values, const QString xheaders,
                 QVector<double>& xvalues);

  //! \brief Slot to display help information
  void help() { showHelp.show_help("integral.html"); }
};
#endif
