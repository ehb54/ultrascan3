...
bool distro_lessthan_s(const S_Solute &solu1, const S_Solute &solu2) { // TRUE iff  (s1<s2) || (s1==s2 && d1<d2)
...
US_Integral::US_Integral() : US_Widgets() {
   ...
   QBoxLayout *plot = new US_Plot(data_plot, tr("Integral Data"), xa_title, ya_title);
...
void US_Integral::save(void) {
   ...
   for (int ii = 0; ii < ndists; ii++) {
      ...
      fpath = reppath + fname;
      write_csv(fpath, "mmass", v_mmass[ ii ], "boundary_fraction", v_bfracs);
      fnames << fname;

      fname = fpfix + "frat_" + QString::number((ii + 1)) + ".csv";
      fpath = reppath + fname;
      write_csv(fpath, "frats", v_mmass[ ii ], "boundary_fraction", v_bfracs);
      fnames << fname;
   ...
   const int plxs[] = { ATTR_S, ATTR_D, ATTR_W, ATTR_K };
   const char *pltp[] = { "sedc", "difc", "mass", "frat" };
...
void US_Integral::load_distro(US_Model model, QString mdescr) {
   ...
   tsys.label = edata.description + " (" + efname.section(".", 0, 0) + " " + efname.section(".", -5, -2) + " "
                + tsys.method + ")" + "[" + QString::number(alldis.size()) + "]";
...
void US_Integral::sort_distro(QList<S_Solute> &listsols, bool reduce) {
   ...
   switch (this->plot_x) {
      case ATTR_D:
         qSort(listsols.begin(), listsols.end(), distro_lessthan_d);
         break;
      case ATTR_W:
         qSort(listsols.begin(), listsols.end(), distro_lessthan_w);
         break;
      case ATTR_K:
         qSort(listsols.begin(), listsols.end(), distro_lessthan_k);
         break;
      case ATTR_S:
         qSort(listsols.begin(), listsols.end(), distro_lessthan_s);
         break;
   }
   ...
   if (reduce) { // skip any duplicates in sorted list
      ...
      while ((++jj) != listsols.end()) { // loop to compare each entry to previous
         ...
         if (sol1.s != sol2.s || sol1.d != sol2.d || sol1.w != sol2.w || sol1.k != sol2.k) { // not a duplicate, so output to temporary list
            reduced.append(sol2);
            jdup = 0;
         }
...
void US_Integral::build_bf_vects() {
   ...
   for (int ii = 0; ii < ndists; ii++) {
      ...
      for (int jj = 0; jj < npoints; jj++) { // Append values for each bf-point for this model
      ...
      if (nsmoo > 1) { // Apply gaussian smoothing to vectors
