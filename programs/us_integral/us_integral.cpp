...
bool
distro_lessthan_s (const S_Solute &solu1, const S_Solute &solu2)
...
bool
distro_lessthan_w (const S_Solute &solu1, const S_Solute &solu2)
{
...
bool
distro_lessthan_k (const S_Solute &solu1, const S_Solute &solu2)
{
...
bool
distro_lessthan_d (const S_Solute &solu1, const S_Solute &solu2)
{
...
US_Integral::US_Integral () : US_Widgets ()
{
  ...
  QBoxLayout *plot
      = new US_Plot (data_plot, tr ("Integral Data"), xa_title, ya_title);
  ...
  for (int ii = 0; ii < ndists; ii++)
    {
      ...
      fpath = reppath + fname;
      write_csv (fpath, "sedi_coeff", v_sedcs[ii], "boundary_fraction",
                 v_bfracs);
      fnames << fname;
      ...
      fpath = reppath + fname;
      write_csv (fpath, "diff_coeff", v_difcs[ii], "boundary_fraction",
                 v_bfracs);
      fnames << fname;
    }
  ...
  if (ncurvs == 1)
    {
      ...
      DbgLv (1) << "DaPl:   npoint" << npoint << "xx" << xx[0]
                << xx[npoint - 1] << "yy" << yy[0] << yy[npoint - 1];
...
void
US_Integral::plot_data (int cplx)
{
  plot_x = cplx;
  plot_data ();
...
void
US_Integral::load_distro (US_Model model, QString mdescr)
{
  DisSys tsys;
  S_Solute sol_in;
  S_Solute sol_nm;
  ...
  tsys.label = edata.description + " (" + efname.section (".", 0, 0) + " "
               + efname.section (".", -5, -2) + " " + tsys.method + ")" + "["
               + QString::number (alldis.size ()) + "]";
  ...
  for (int jj = 0; jj < nsolmc; jj++)
    {
      ...
      tot_conc += sol_in.c;
      if (jj < 3 || (jj + 4) > nsolmc)
        DbgLv (1) << "LD:    jj" << jj << "soli s,d,c,t" << sol_in.s
                  << sol_in.d << sol_in.c << tot_conc;
    }
  ...
  DbgLv (1) << "LD: totconc" << tot_conc << "nsolin nsolnm" << nsolin
            << nsolnm;
  ...
  DbgLv (1) << "LD: nsolin nsolnm" << nsolin << nsolnm
            << tsys.nm_distro.size ();
  ...
  for (int jj = 0; jj < nsolnm; jj++)
    {
      ...
      if (jj < 3 || (jj + 4) > nsolnm)
        DbgLv (1) << "LD:    jj" << jj << "solb s,d,c,f" << sol_bf.s
                  << sol_bf.d << sol_bf.c << sol_bf.f;
    }
...
void
US_Integral::resort_sol (QVector<DisSys> &list_dist)
{
...
void
US_Integral::sort_distro (QList<S_Solute> &listsols, bool reduce)
{
    ...
    { // skip any duplicates in sorted list
      ...
      while ((++jj) != listsols.end ())
        {             // loop to compare each entry to previous
          ...
          if (sol1.s != sol2.s || sol1.d != sol2.d || sol1.w != sol2.w
              || sol1.k != sol2.k)
...
void
US_Integral::update_disk_db (bool isDB)
{
...
void
US_Integral::select_prefilt (void)
{
  QString pfmsg;
  QString dinfo;
  int nruns = 0;
...
void
US_Integral::remove_distro (void)
{
...
void
US_Integral::select_x_axis (int ival)
{
...
void
US_Integral::build_bf_distro (int modx)
{
  if (alldis.size () <= modx)
    return;
...
QString
US_Integral::anno_title (int pltndx)
{
  QString a_title;
...
QString
US_Integral::ptype_text (int pltndx)
{
  QString t_text;
...
void
US_Integral::update_divis (double dval)
{
  DbgLv (1) << "UpdDiv:" << dval;
  resort_sol (alldis);
  build_bf_dists (); // (Re-)build boundary fraction distributions
...
void
US_Integral::write_csv (const QString fpath, const QString hdr1,
...
void
US_Integral::models_summary (void)
{
  ...
  for (int jj = 0; jj < alldis.size (); jj++)
    { // Compose and display a distribution line for distro info box
      ...
      mdesc = (kk > 0) ? "..." + QString (mdesc).mid ((kk + 1), 20) + "..."
                       : mdesc;
