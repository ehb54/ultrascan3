...
US_ComProjectMain::US_ComProjectMain (QString us_mode) : US_Widgets ()
{
  ...
  welcome->setText (
      " <br> Welcome to UltraScan GMP<br> for setting up, monitoring, editing "
      "and analyzing <br> AUC experiments and produced data... <br>");
  ...
  // tabWidget->tabBar()->setStyleSheet( "QTabBar::tab {min-width: 70;}
  // QTabBar::tab:selected {background: qlineargradient(x1: 0, y1: 0, x2: 0,
  // y2: 1, stop: 0 #fafafa, stop: 0.4 #f4f4f4, stop: 0.5 #e7e7e7, stop: 1.0
  // #fafafa); } QTabBar::tab:hover {background: lightgray;} QTabBar::tab:first
  // {background: blue; color: lightgray; min-width: 50;}
  // QTabBar::tab:first:hover {background: #4169E1; color: white}
  // QTabBar::tab:disabled { color: rgba(0, 0, 0, 70%) } ");

  // no hoover
  tabWidget->tabBar ()->setStyleSheet (
      "QTabBar::tab {min-width: 70;} QTabBar::tab:selected {background: "
      "qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 #fafafa, stop: 0.4 "
      "#f4f4f4, stop: 0.5 #e7e7e7, stop: 1.0 #fafafa); } QTabBar::tab:first "
      "{background: blue; color: lightgray; min-width: 50;}  "
      "QTabBar::tab:first:hover {background: #4169E1; color: white}   "
      "QTabBar::tab:disabled { color: rgba(0, 0, 0, 70%) } ");
  // tabWidget->tabBar()->setStyleSheet( "QTabBar::tab {min-width: 70;}
  // QTabBar::tab:selected {background: qlineargradient(x1: 0, y1: 0, x2: 0,
  // y2: 1, stop: 0 #fafafa, stop: 0.4 #f4f4f4, stop: 0.5 #e7e7e7, stop: 1.0
  // #fafafa); } QTabBar::tab:first {background: blue; color: lightgray;
  // min-width: 50;}  QTabBar::tab:first:hover {background: #4169E1; color:
  // white}  QTabBar::tab:last {background: blue; color: lightgray; min-width:
  // 50;}  QTabBar::tab:last:hover {background: #4169E1; color: white}
  // QTabBar::tab:disabled { color: rgba(0, 0, 0, 70%) } ");
  ...
  qDebug () << "TabWidget size    : " << tabWidget->width ()
            << tabWidget->height ();
  qDebug () << "TabWidget->tabBar position: " << tabWidget->tabBar ()->x ()
            << tabWidget->tabBar ()->y ();
  qDebug () << "TabWidget->tabBar size    : " << tabWidget->tabBar ()->width ()
  ...
  for (int i = 0; i < tabWidget->count (); ++i)
    {
      // ALEXEY: OR enable all tabs ? (e.g. for demonstration, in a read-only
      // mode or the like ?)
      if (i == 0)
  ...
  // pb_close_program->setStyleSheet( "QPushButton { background: red; color:
  // lightgray; min-height: 20 } QPushButton::hover {  background-color:
  // #9E0606; color: white } "); connect( pb_close_program, SIGNAL( clicked()
  // ), this, SLOT( close()  ) ); main->addWidget( pb_close_program );
  ...
  connect (epanInit, SIGNAL (define_new_experiment_init (QStringList &)), this,
           SLOT (define_new_experiment (QStringList &)));
  connect (epanInit,
           SIGNAL (switch_to_live_update_init (QMap<QString, QString> &)),
           this, SLOT (switch_to_live_update (QMap<QString, QString> &)));
  connect (epanInit,
           SIGNAL (switch_to_post_processing_init (QMap<QString, QString> &)),
           this, SLOT (switch_to_post_processing (QMap<QString, QString> &)));
  connect (epanInit, SIGNAL (to_initAutoflow ()), this, SLOT (close_all ()));

  connect (this, SIGNAL (pass_used_instruments (QStringList &)), epanExp,
           SLOT (pass_used_instruments (QStringList &)));

  connect (epanInit, SIGNAL (pass_allow_dataDisk_only ()), epanExp,
           SLOT (allow_dataDisk_only ()));

  connect (epanExp, SIGNAL (switch_to_live_update (QMap<QString, QString> &)),
           this, SLOT (switch_to_live_update (QMap<QString, QString> &)));
  connect (epanExp, SIGNAL (switch_to_import (QMap<QString, QString> &)), this,
           SLOT (switch_to_post_processing (QMap<QString, QString> &)));
  connect (this, SIGNAL (pass_to_live_update (QMap<QString, QString> &)),
           epanObserv,
           SLOT (process_protocol_details (QMap<QString, QString> &)));
  connect (epanExp, SIGNAL (to_autoflow_records ()), this,
           SLOT (to_autoflow_records ()));
  connect (epanExp, SIGNAL (switch_to_initAutoflow ()), this,
           SLOT (close_all ()));

  connect (epanObserv,
           SIGNAL (switch_to_post_processing (QMap<QString, QString> &)), this,
           SLOT (switch_to_post_processing (QMap<QString, QString> &)));
  connect (this, SIGNAL (pass_to_post_processing (QMap<QString, QString> &)),
           epanPostProd,
           SLOT (import_data_us_convert (QMap<QString, QString> &)));

  // connect( this, SIGNAL( clear_experiment( QString & ) ),  epanExp, SLOT(
  // clear_experiment( QString & )  ) );
  connect (epanObserv, SIGNAL (close_everything ()), this,
           SLOT (close_all ()));
  connect (this, SIGNAL (reset_live_update ()), epanObserv,
           SLOT (reset_live_update ()));
  connect (epanObserv, SIGNAL (processes_stopped ()), this,
           SLOT (liveupdate_stopped ()));
  connect (epanObserv, SIGNAL (stop_nodata ()), this, SLOT (close_all ()));

  connect (epanPostProd, SIGNAL (switch_to_initAutoflow ()), this,
           SLOT (close_all ()));
  connect (this, SIGNAL (reset_lims_import ()), epanPostProd,
           SLOT (reset_lims_import ()));
...
US_ComProjectMain::US_ComProjectMain () : US_Widgets ()
{
  ...
  tabWidget->tabBar ()->setStyleSheet (
      ...
      "QTabBar::tab:first:hover {background: #4169E1; color: white}  "
      "QTabBar::tab:disabled { color: rgba(0, 0, 0, 70%)  } "
      "QTabBar::tab:last:disabled {background: #90EE90;  min-width: 50;}  "
      "QTabBar::tab:last:selected {background: #556B2F; color: lightgray;  "
      "min-width: 50;}");
  ...
  connect (epanInit,
           ...
           this, SLOT (switch_to_post_processing (QMap<QString, QString> &)));
  connect (epanInit,
           SIGNAL (switch_to_editing_init (QMap<QString, QString> &)), this,
           SLOT (switch_to_editing (QMap<QString, QString> &)));
  connect (epanInit,
           SIGNAL (switch_to_analysis_init (QMap<QString, QString> &)), this,
           SLOT (switch_to_analysis (QMap<QString, QString> &)));
  connect (epanInit, SIGNAL (switch_to_report_init (QMap<QString, QString> &)),
           this, SLOT (switch_to_report (QMap<QString, QString> &)));
  connect (epanInit, SIGNAL (switch_to_esign_init (QMap<QString, QString> &)),
           this, SLOT (switch_to_esign (QMap<QString, QString> &)));
  connect (epanInit, SIGNAL (to_initAutoflow ()), this, SLOT (close_all ()));
  ...
  connect (epanPostProd, SIGNAL (switch_to_editing (QMap<QString, QString> &)),
           this, SLOT (switch_to_editing (QMap<QString, QString> &)));
  connect (this, SIGNAL (pass_to_editing (QMap<QString, QString> &)),
           epanEditing, SLOT (do_editing (QMap<QString, QString> &)));
  connect (this, SIGNAL (reset_lims_import ()), epanPostProd,
           SLOT (reset_lims_import ()));
  connect (epanPostProd, SIGNAL (switch_to_initAutoflow ()), this,
           SLOT (close_all ()));
  connect (this, SIGNAL (reset_data_editing ()), epanEditing,
           SLOT (reset_data_editing ()));

  connect (epanEditing, SIGNAL (switch_to_analysis (QMap<QString, QString> &)),
           this, SLOT (switch_to_analysis (QMap<QString, QString> &)));
  connect (epanEditing, SIGNAL (switch_to_report (QMap<QString, QString> &)),
           this, SLOT (switch_to_report (QMap<QString, QString> &)));
  connect (this, SIGNAL (pass_to_analysis (QMap<QString, QString> &)),
           epanAnalysis, SLOT (do_analysis (QMap<QString, QString> &)));
  connect (epanEditing, SIGNAL (switch_to_initAutoflow ()), this,
           SLOT (close_all ()));

  connect (epanAnalysis, SIGNAL (processes_stopped ()), this,
           SLOT (analysis_update_stopped ()));
  connect (epanAnalysis, SIGNAL (switch_to_initAutoflow ()), this,
           SLOT (close_all ()));

  connect (epanAnalysis, SIGNAL (switch_to_report (QMap<QString, QString> &)),
           this, SLOT (switch_to_report (QMap<QString, QString> &)));
  connect (this, SIGNAL (pass_to_report (QMap<QString, QString> &)),
           epanReport, SLOT (do_report (QMap<QString, QString> &)));

  connect (this, SIGNAL (reset_reporting ()), epanReport,
           SLOT (reset_reporting ()));

  // E-Signs
  connect (epanReport, SIGNAL (switch_to_esign (QMap<QString, QString> &)),
           this, SLOT (switch_to_esign (QMap<QString, QString> &)));
  connect (this, SIGNAL (pass_to_esign (QMap<QString, QString> &)), epanSign,
           SLOT (do_esign (QMap<QString, QString> &)));
  connect (this, SIGNAL (reset_esigning ()), epanSign,
           SLOT (reset_esigning ()));
...
void
US_ComProjectMain::checkDataLocation (void)
{
  qDebug () << "Data Location ( DB==1; Disk==2 ) -- "
  ...
  if (US_Settings::default_data_location () == 2) // Disk
    {
      ...
      QMessageBox::warning (this, tr ("Data Location set to Disk"),
                            tr ("Please change Data Location to \"Database\" "
                                "in the configuraiton of your database! \n"));
...
void
US_ComProjectMain::initPanels (int panx)
{
  ...
  if (panx == 0) // Autoflow Records
    {
      // Depending on from we jump to InitDialogue, we may need to stop/reset
      // current stages: E.g., LIVE_UPDATE: stop all timers, reset GUI etc..
      // Maybe a warning message "You are going to leave this panel.." needs to
      // be shown
      ...
      if (curr_panx == 2)
        {
          // 2 cases:
          /*
                1. Avaiting for run to be launched ("Back to Managing Optima
             Runs") -- SET
                2. More complex: Back to Managing runs from active LIVE_UPDATE
             stage -- stop all timers and other processes...
          */

          qDebug () << "Jumping from LIVE UPDATE.";

          // 2. Stop all timers/threads, reset GUI - when stopping fully
          // working LIVE UPDATE
...
void
US_ComProjectMain::show_liveupdate_finishing_msg (void)
{

  msg_liveupdate_finishing = new QMessageBox (this);
  msg_liveupdate_finishing->setIcon (QMessageBox::Information);

  msg_liveupdate_finishing->setWindowFlags (Qt::CustomizeWindowHint
                                            | Qt::WindowTitleHint
                                            | Qt::WindowStaysOnTopHint);
  msg_liveupdate_finishing->setStandardButtons (0);
  msg_liveupdate_finishing->setWindowTitle (tr ("Updating..."));
  msg_liveupdate_finishing->setText (
      tr ("Finishing LIVE UPDATE processes... Please wait..."));
  msg_liveupdate_finishing->setStyleSheet (
      "background-color: #36454f; color : #D3D9DF;");

  int tab_width = this->tabWidget->tabBar ()->width ();
  int upper_height = this->gen_banner->height () + // this->welcome->height()
                     +this->logWidget->height ()
                     + this->test_footer->height ();
...
//
void
US_ComProjectMain::show_analysis_update_finishing_msg (void)
{

  msg_analysis_update_finishing = new QMessageBox (this);
  msg_analysis_update_finishing->setIcon (QMessageBox::Information);

  msg_analysis_update_finishing->setWindowFlags (Qt::CustomizeWindowHint
                                                 | Qt::WindowTitleHint
                                                 | Qt::WindowStaysOnTopHint);
  msg_analysis_update_finishing->setStandardButtons (0);
  msg_analysis_update_finishing->setWindowTitle (tr ("Updating..."));
  msg_analysis_update_finishing->setText (
      tr ("Finishing ANALYSIS UPDATE processes... Please wait..."));
  msg_analysis_update_finishing->setStyleSheet (
      "background-color: #36454f; color : #D3D9DF;");
...
void
US_ComProjectMain::liveupdate_stopped (void)
{
...
void
US_ComProjectMain::analysis_update_stopped (void)
{
...
void
US_ComProjectMain::closeEvent (QCloseEvent *event)
{
  window_closed = true;
  ...
  if (!data_location_disk)
    {
      qDebug () << "initDialogue: true/false 1 : "
                << epanInit->initDialogueOpen;
      emit us_comproject_closed ();
      close_initDialogue ();
      qDebug () << "initDialogue: true/false 3 : "
                << epanInit->initDialogueOpen;
...
void
US_ComProjectMain::to_autoflow_records (void)
{
  // check_current_stage();
}

void
US_ComProjectMain::close_initDialogue (void)
{
...
void
US_ComProjectMain::delete_psql_record (int ExpId)
{
  ...
  if (dbxpn.open ())
    {
      ...
      QString ScanId = "5996";
      if(! query_abs_scan.prepare(QString("DELETE FROM %1 WHERE \"ScanId\" =
      %2").arg(qrytab_abs).arg(ScanId) ) ) qDebug() <<
      query_abs_scan.lastError().text();

      if (query_abs_scan.exec())
        {
          qDebug() << "AbsorbanceScanParameters record # :" << ScanId  <<
      "deleted !";
        }
      else
        {
          QString errmsg   = "Delete record error: " +
      query_abs_scan.lastError().text(); QMessageBox::critical( this, tr(
      "*ERROR* in Deleting Absorbance Record" ), tr( "An error occurred in the
      attempt to delete" " AbsorbanceScanParameters from AUC DB\n  %1 table\n
      %2 ." ).arg( qrytab_abs ).arg( errmsg ) ); return;
        }
      ...
      // FugeProfile
      int FugeId = 308;
      if(! query_fuge.prepare(QString("DELETE FROM %1 WHERE
      \"FugeRunProfileId\" = %2").arg(qrytab_fuge).arg(FugeId) ) ) qDebug() <<
      query_fuge.lastError().text();
      ...
      if (query_fuge.exec())
        ...
        {
          QString errmsg   = "Delete record error: " +
      query_fuge.lastError().text();; QMessageBox::critical( this, tr( "*ERROR*
      in Deleting Fuge Profile" ), tr( "An error occurred in the attempt to
      delete" " FugeProfile from AUC DB\n  %1 table\n  %2 ." ).arg( qrytab_fuge
      ).arg( errmsg ) ); return;
        }
      ...
      */
      ...
      if (!query_expdef.prepare (
              QString ("DELETE FROM %1 WHERE \"ExperimentId\" = %2")
                  .arg (qrytab_expdef)
                  .arg (ExpId)))
      ...
      if (query_expdef.exec ())
        {
          qDebug () << "ExperimentDefinition record # :" << ExpId
                    << "deleted !";
        }
      else
        {
          QString errmsg
              = "Delete record error: " + query_expdef.lastError ().text ();
          ;
          QMessageBox::critical (
              this, tr ("*ERROR* in Deleting Experimental Method"),
...
// Slot to define new exp. (from initial dialog)
void
US_ComProjectMain::define_new_experiment (QStringList &occupied_instruments)
{
  ...
  tabWidget->setCurrentIndex (1); // Maybe lock this panel from now on? i.e.
  ...
  qDebug ()
      << "In define_new_experiment( QStringList & occupied_instruments )";
...
void
US_ComProjectMain::close_all (void)
{
  xpn_viewer_closed_soft = true;
  tabWidget->setCurrentIndex (0);
  // epanInit  ->initAutoflowPanel();  //DO NOT Duplicate - was called in
  // previous line while changing Tab!!!
...
// Slot to pass submitted to Optima run info to the Live Update tab
void
US_ComProjectMain::switch_to_live_update (
    QMap<QString, QString> &protocol_details)
{
  tabWidget->setCurrentIndex (2); // Maybe lock this panel from now on? i.e.
                                  // tabWidget->tabBar()->setEnabled(false) ??
  curr_panx = 2;
...
void
US_ComProjectMain::switch_to_post_processing (
    QMap<QString, QString> &protocol_details)
{
  tabWidget->setCurrentIndex (3); // Maybe lock this panel from now on? i.e.
                                  // tabWidget->tabBar()-setEnabled(false) ??
  curr_panx = 3;
...
void
US_ComProjectMain::switch_to_editing (QMap<QString, QString> &protocol_details)
{
  tabWidget->setCurrentIndex (4); // Maybe lock this panel from now on? i.e.
                                  // tabWidget->tabBar()-setEnabled(false) ??
  curr_panx = 4;
...
// Slot to switch from the Import to Editing tab
void
US_ComProjectMain::switch_to_analysis (
    QMap<QString, QString> &protocol_details)
{
  // tabWidget->setCurrentIndex( 5 );   // Maybe lock this panel from now on?
  // i.e. tabWidget->tabBar()-setEnabled(false) ?? curr_panx = 5;
  ...
  msg_analysissetup->setWindowFlags (Qt::CustomizeWindowHint
                                     | Qt::WindowTitleHint
                                     | Qt::WindowStaysOnTopHint);
  msg_analysissetup->setStandardButtons (0);
  msg_analysissetup->setWindowTitle (tr ("Updating..."));
  msg_analysissetup->setText (
      tr ("Generating triple list for ANALYSIS... Please wait..."));
  msg_analysissetup->setStyleSheet (
      "background-color: #36454f; color : #D3D9DF;");
  ...
  tabWidget->setCurrentIndex (5); // Maybe lock this panel from now on? i.e.
                                  // tabWidget->tabBar()-setEnabled(false) ??
  curr_panx = 5;
...
void
US_ComProjectMain::switch_to_report (QMap<QString, QString> &protocol_details)
{
  tabWidget->setCurrentIndex (6); // Maybe lock this panel from now on? i.e.
                                  // tabWidget->tabBar()-setEnabled(false) ??
  curr_panx = 6;
...
void
US_ComProjectMain::switch_to_esign (QMap<QString, QString> &protocol_details)
{
  /**  BEFORE going to e-Signatures: Check if user is among
   * operators|reviewers|approvers**/

  /*
     Covered in list_all_autoflow_records():
      -- isOperRev( user_id, autolfowId ): if false, run NOT shown in the GMP
     run list Do we want to rather show all runs BUT not allow to switch to
     E-SIGN if ( !isOperRev( user_id, autolfowId )) ??
  */

  /****************************************************************************************/

  tabWidget->setCurrentIndex (7); // Maybe lock this panel from now on? i.e.
                                  // tabWidget->tabBar()-setEnabled(false) ??
  curr_panx = 7;
...
void
US_ComProjectMain::call_AutoflowDialogue (void) //<--- Entry point from main
{
    ...
    : US_WidgetsDialog (topw, 0)
{
  ...
  // panel_desc->setPlainText(" Tab to Show Init Dialogue...  ---UNDER
  // CONSTRUCTION--- "); panel_desc->setReadOnly(true);
  ...
  if (initDialogueOpen)
    {
      ...
      if (new_main_w > pdiag_autoflow->width ()
          || new_main_h > pdiag_autoflow->height ())
        {
...
// Init Autoflow Panel
void
US_InitDialogueGui::initAutoflowPanel (void)
{
  initRecords ();
  initRecordsDialogue ();
}
...
void
US_InitDialogueGui::checkCertificates (void)
{
  ...
  for (int i = 0; i < this->instruments.size (); ++i)
    {
      ...
      bool status_sys_data
          = link->connectToServer_init_check (dbhost, optima_msgPort.toInt ());
      ...
      qDebug () << "Qt's way of showing expiraiton date: "
                << cert.expiryDate () << expdate_str;
      ...
      // ////////////////////////////////////////////////////////////
      qDebug () << "combined_check, daysToExpiration, certfile_exists -- "
                << combined_check << daysToExpiration << certfile_exists;
      ...
      if (!combined_check || daysToExpiration <= 0 || !certfile_exists)
        {
              ...
              if (daysToExpiration > 0)
                {
                  ...
                  msg_sys_text_info
                      += QString (tr ("1. %1 is turned on\n"
                                      "2. the data acquisition server on %1 "
                                      "is running\n"
                                      "3. your license key is stored in "
                                      "$HOME/ultrascan/etc/optima and is "
                                      "valid and not expired.\n"))
                             .arg (alias);
                  //"Submission of the experimental protocol is suspended until
                  //this condition is resolved." )).arg(alias);
                }
              else
                {
                  msg_sys_text_info
                      += QString (
                             tr ("\n%1 Your license key for %2 is expired!\n"
                                 "Please renew the key and try to submit "
                                 "again.\n"))
                             .arg (QChar (0x2022), alias);
                  // "Submission of the experimental protocol is suspended.\n"
                  //"Please renew the key and try to submit again.")).arg(
                  //alias );
                }
            }
          else
            {
              msg_sys_text_info += QString (tr ("\n%1 It appears the license "
                                                "key for %2 is missing:"))
                                       .arg (QChar (0x2022), alias);
          ...
          // //msg_sys_text_info += QString( tr("\n\nThe program will be
          // closed.") ); msgBox_sys_data.setText( msg_sys_text );
          // msgBox_sys_data.setInformativeText( msg_sys_text_info );

          // QPushButton *Cancel_sys    = msgBox_sys_data.addButton(tr("OK"),
          ...
          if (daysToExpiration <= 30)
            {
              ...
              QMessageBox::warning (
                  this,
                  QString (tr ("License Key for %1 Nearing Expiraiton"))
                      .arg (alias),
                  QString (
                      tr ("Your license key will expire within %1 days. \n\n "
  ...
  if (count_instruments_disconnected == this->instruments.size ())
    {
      ...
      QString msg_sys_text
          = QString ("Attention! UltraScan is not able to communicate with "
                     "the data acquisition server(s) on the %1.")
                .arg (inst_names);
      ...
      if (Optima_names.size () > 1)
        msg_sys_text_info_final
            += QString (tr ("\n\nSubmission of the experimental protocol to "
                            "the Optima instruments is suspended until at "
                            "least one of these conditions is resolved."));
      else
        msg_sys_text_info_final += QString (
            tr ("\n\nSubmission of the experimental protocol to the Optima "
                "instrument is suspended until this condition is resolved."));

      if (!mainw->us_mode_bool)
        msg_sys_text_info_final
            += QString (tr ("\n\nUser has an option to upload data from disk "
                            "(tab 2. Lab/Rotor -> Select Data Source) and "
                            "proceed with that workflow."));

      msgBox_sys_data.setInformativeText (msg_sys_text_info_final);
      QPushButton *Cancel_sys
          = msgBox_sys_data.addButton (tr ("OK"), QMessageBox::RejectRole);
      msgBox_sys_data.exec ();
...
void
US_InitDialogueGui::initRecords (void) // <-- 1st entry point !!
{
  ...
  qDebug () << "Init Autoflow Records: DONE, occupied instruments: "
            << occupied_instruments;
}

// Init AutoflowRecords Dialogue: call from _main.cpp
void
US_InitDialogueGui::initRecordsDialogue (void)
{

  // ALEXEY: ensure that NO pdiag_dialog(s) open before initialization...
  // IMPORTANT! Will Not be needed in the production version where all but
  // currently active Tab is enabled... But may be a good idea to retain...
  /* ---------------------------------------------------------------------------------------*/
  if (initDialogueOpen)
    {
  ...
  if (initMsgNorecOpen)
    {
      msg_norec->reject (); // Not just close() - as it triggers setting up exp
                            // panel...
      initMsgNorecOpen = false;
  ...
  if (initMsgNorecDelOpen)
    {
      msg_norec_del->reject (); // Not just close() - as it triggers setting up
                                // exp panel...
      initMsgNorecDelOpen = false;
  ...
  // if ( autoflow_records < 1 )
  if (autoflow_records < 1
      || autoflowdata.size () < 1) // can be that for UL<3, autoflow runs from
                                   // others are not shown
    {
      ...
      msg_norec->setText (tr ("There are no Optima runs to follow.<br><br>"
                              "You will be switched to <b>Experiment</b> "
                              "stage to design and submit new protocol."));
  ...
  connect (pdiag_autoflow, SIGNAL (accept_autoflow_deletion ()), this,
           SLOT (update_autoflow_data ()));
  connect (pdiag_autoflow, SIGNAL (accept_refresh_states ()), this,
           SLOT (refresh_optima_states ()));
  pdiag_autoflow->setParent (this, Qt::Widget);

  offset = 20;
  pdiag_autoflow->move (offset, 2 * offset);

  // mainw->cornerWidget->setFixedWidth( mainw->tabWidget->tabBar()->width() -
  // 20 );
  ...
  qDebug () << "occupied_instruments.size(), instruments.size()"
            << occupied_instruments.size () << ", " << instruments.size ();
  ...
  else
    {
      ...
      if (occupied_instruments.size () == 0)
      ...
      else
        {
          ...
          msg_instr_use->setText (
              tr ("The following Optima instrument(s)<br>"
  ...
  qDebug () << "failedID: str, INT --  " << protocol_details["failedID"]
  ...
  if (failedID.toInt () != 0)
    {
      ...
      QMap<QString, QString> failed_details
          = read_autoflow_failed_record (failedID);

      QMessageBox msgBox_f;
      msgBox_f.setText (
          tr ("The selected GMP run is marked as FAILED:<br><br>")
          + tr ("<b>Run Name:&emsp;</b>") + ProtName + tr ("<br>")
          + tr ("<b>Failing stage:&emsp;</b> ") + failed_details["failedStage"]
          + tr ("<br>") + tr ("<b>Reason:&emsp;</b> ")
          + failed_details["failedMsg"]);
      ...
      if (msgBox_f.clickedButton () == Cancel)
      ...
      else if (msgBox_f.clickedButton () == Confirm)
        {
          ...
          /*
            1. revert 'liveUpdate, import, editing' fields in autolfowStages to
            DEFAULT
            ...
            5. delete autoflowHistory record (if was already created - ONLY
            case if REPORT was proceeded ?)
            6. Reset all autoflow record fields updated starting from
            LIVE_UPDATE
                         - dataPath
                         - filename
                         - intensityID
          ...
          */
          ...
          // Create fresh autoflowStatus record with the submitter person info
          // & timestamp:
              ...
              if (instName.contains ("Optima") && dataSource == "INSTRUMENT")
              ...
              else // disk Data
                {
                  QMessageBox::warning (this, tr ("Data Absent on Disk!"),
                                        tr ("No data associated with this run "
...
void
US_InitDialogueGui::do_run_tables_cleanup (QMap<QString, QString> run_details)
{
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd ();
  ...
  qry << "autoflow_stages_revert" << run_details["autoflowID"];
  db->query (qry);

  // deleting autoflowIntensity Record
  qry.clear ();
  qry << "delete_autoflow_intensity_record" << run_details["autoflowID"]
      << run_details["intensityID"];
  db->query (qry);

  // deleting autoflowStatus Record
  qry.clear ();
  qry << "delete_autoflow_status_record" << run_details["autoflowID"]
      << run_details["statusID"];
  db->query (qry);

  // deleting autoflowFailed Record
  qry.clear ();
  qry << "delete_autoflow_failed_record" << run_details["autoflowID"]
  ...
  for (int i = 0; i < analysisIDs_list.size (); ++i)
    {
      ...
      qry << "update_autoflow_analysis_record_at_deletion"
          << QString ("Canceled for Failed run, top-level") << requestID;
  ...
  qry.clear ();
  qry << "delete_autoflow_history_record" << run_details["autoflowID"];
  db->query (qry);

  // Clean certain field in autoflow record for re-initializing from 2.
  // LIVE_UPDATE
  qry.clear ();
  qry << "update_autoflow_at_reset_live_update" << run_details["autoflowID"];
  db->query (qry);

  // Restore autoflowReports' records 'tripleDropped' to DEFAULT ('none')
  //  1. Create reportIDs list from protocol's AProfile;
  //  2. Go over reports in the autoflowReport table & SET 'tripleDropped' to
  //  'none'
...
void
US_InitDialogueGui::do_create_autoflowStatus_for_failedRun (
    QMap<QString, QString> run_details)
{
  ...
  qry.clear ();
  qry << "new_autoflowStatusGMPCreate_record" << run_details["autoflowID"]
      << createGMPRun_Json;

  qDebug () << "[FAILED run init]: new_autoflowStatusGMPCreate_record qry -- "
            << qry;
  ...
  if (!autoflowStatusID)
    {
      QMessageBox::warning (this, tr ("AutoflowStatus Record Problem"),
                            tr ("autoflowStatus (FAILED GMP run re-INIT): "
                                "There was a problem with creating a record "
                                "in autoflowStatus table \n")
                                + db->lastError ());

      return;
    }
  qDebug ()
      << "in do_create_autoflowStatus_for_failedRun: createGMPRun_Json -- "
      << createGMPRun_Json;
   ...
   * ****************/
  qry.clear ();
  qry << "update_autoflow_with_statusID" << run_details["autoflowID"]
      << QString::number (autoflowStatusID);

  qDebug () << "[FAILED run init]: update_autoflow_with_statusID qry -- "
            << qry;
  db->query (qry);
}
...
bool
US_InitDialogueGui::readAProfileBasicParms_auto (QXmlStreamReader &xmli)
{
  while (!xmli.atEnd ())
    {
...
void
US_InitDialogueGui::do_run_data_cleanup (QMap<QString, QString> run_details)
{
  ...
  for (int i = 0; i < fileNameList.size (); ++i)
    {
      ...
      // US_Experiment::deleteRunPcsaMrecs( db, run_details[ "invID_passed" ],
      ...
      status = db->statusQuery (qry);
      qDebug () << "Cleaning Data (del data) for FAILED run: del_exp stat"
                << status;
...
void
US_InitDialogueGui::refresh_optima_states (void)
{
  initAutoflowPanel ();
}

// Re-evaluate autoflow records & occupied instruments & if Define Another Exp.
// should be enabled....
void
US_InitDialogueGui::update_autoflow_data (void)
{
...
void
US_InitDialogueGui::read_optima_machines (US_DB2 *db)
{
...
int
US_InitDialogueGui::list_all_autoflow_records (
    QList<QStringList> &autoflowdata, US_DB2 *dbP)
{
  int nrecs = 0;
  autoflowdata.clear ();
  ...
  while (dbP->next ())
    {
          ...
          if (status == "LIVE_UPDATE")
            {
              ...
              QMap<QString, QString> currentInstrument
                  = selectCurrentOptima (optimaname);
              US_XpnData *xpn_data = new US_XpnData ();
              if (xpn_data->connect_data (
                      currentInstrument["optimaHost"],
                      ...
                      currentInstrument["optimaDBpassw"]))
                {
                  ...
                  qDebug () << "Exp status for runID " << runID << ": "
                            << exp_status;
                }
              else
      ...
      // Treat eSign separately: only show runs at this stage to reviewers &&
      // operator:
      if (status == "E-SIGNATURES")
        {
          ...
          // reviewers: check against new 'autoflowGMPReportEsign.
          // statusSignJson["to_process"]' table's field:
      ...
      else
        {
          ...
          if (US_Settings::us_inv_level () < 3)
          ...
          else
            {
              ...
              qDebug ()
                  << "2. IN list_all_autoflow_records(), autoflowentry -- "
                  << autoflowentry;
            }
        }
    }
...
US_InitDialogueGui::selectCurrentOptima (QString optimaname)
{
  QMap<QString, QString> currentInstrument;
...
bool
US_InitDialogueGui::isOperRev (int uID, QString autoflow_id)
{
  bool yesRev = false;
  ...
  while (db->next ())
    {
      ...
      operator_list = db->value (3).toString (); // json array [1 value for
                                                 // now]
  ...
  QJsonDocument jsonDocRevList
      = QJsonDocument::fromJson (reviewers_list.toUtf8 ());
  QJsonDocument jsonDocOperList
      = QJsonDocument::fromJson (operator_list.toUtf8 ());
  QJsonDocument jsonDocApprList
      = QJsonDocument::fromJson (approvers_list.toUtf8 ());

  if (!jsonDocRevList.isArray () || !jsonDocOperList.isArray ()
      || !jsonDocApprList.isArray ())
    {
      qDebug () << "jsonDocRevList OR jsonDocOperList OR jsonDocApprList not "
                   "a JSON Array!";
      return yesRev;
    }
...
// Query autoflow for # records
int
US_InitDialogueGui::get_autoflow_records (void)
{
...
// Query autoflow for # records
QMap<QString, QString>
US_InitDialogueGui::read_autoflow_record (int autoflowID)
{
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd ();
...
// Query autoflow for # records
QMap<QString, QString>
US_InitDialogueGui::read_autoflow_failed_record (QString failedID)
{
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd ();
  ...
  QStringList qry;
  qry << "read_autoflow_failed_record" << failedID;
  ...
  if (db->lastErrno () == US_DB2::OK) // Autoflow record exists
    ...
    {
      QMessageBox::warning (
          this, tr ("No autoflowFailed Record Exists"),
...
//////////////////////////////////////////////////////////////////////////////////
// US_ExperGUI
US_ExperGui::US_ExperGui (QWidget *topw) : US_WidgetsDialog (topw, 0)
{
  mainw = (US_ComProjectMain *)topw;
  ...
  // pb_openexp->setStyleSheet("QPushButton{background-color: #318CE7; border:
  // 1px solid white; background-image: url(temp_path); color: black;
  // border-radius: 15px; font-size: 18px; font-weight: bold; }
  // QPushButton:hover{background-color: #6699CC;}
  // QPushButton:disabled{background-color:#6699CC ; color: white}");
  ...
  pb_openexp->setStyleSheet (
      "QPushButton{border: none} QPushButton:hover{border: 1px solid gray}");
  // pb_openexp->setStyleSheet("QPushButton{background-color: #318CE7; border:
  // 1px solid white; background-image: url(temp_path); color: black;
  // border-radius: 15px; font-size: 18px; font-weight: bold; }
  // QPushButton:hover{background-color: #6699CC;}
  ...
  connect (sdiag, SIGNAL (us_exp_is_closed ()), this,
           SLOT (us_exp_is_closed_set_button ()));
  // connect( this,  SIGNAL( set_auto_mode() ),   sdiag, SLOT(
  // auto_mode_passed() ) );

  connect (this, SIGNAL (define_used_instruments (QStringList &)), sdiag,
           SLOT (exclude_used_instruments (QStringList &)));

  connect (this, SIGNAL (data_disk_only ()), sdiag,
           SLOT (enable_data_disk_only ()));

  connect (sdiag, SIGNAL (close_expsetup_msg ()), this,
           SLOT (expsetup_msg_closed ()));

  connect (sdiag, SIGNAL (to_live_update (QMap<QString, QString> &)), this,
           SLOT (to_live_update (QMap<QString, QString> &)));

  connect (sdiag, SIGNAL (to_import (QMap<QString, QString> &)), this,
           SLOT (to_import (QMap<QString, QString> &)));

  connect (this, SIGNAL (reset_experiment (QString &)), sdiag,
           SLOT (us_exp_clear (QString &)));

  connect (sdiag, SIGNAL (exp_cleared ()), this, SLOT (exp_cleared ()));

  // return automatically to Run Manager:
  connect (sdiag, SIGNAL (back_to_initAutoflow ()), this,
           SLOT (to_initAutoflow ()));
...
void
US_ExperGui::resizeEvent (QResizeEvent *event)
{
  int tab_width = mainw->tabWidget->tabBar ()->width ();
  int upper_height = mainw->gen_banner->height () + // mainw->welcome->height()
                     +mainw->logWidget->height ()
  ...
  // if (mainw->width() - offset > sdiag->width() || mainw->height() - 2*offset
  // > sdiag->height()) {
...
void
US_ExperGui::pass_used_instruments (QStringList &occupied_instruments)
{
  qDebug () << "In US_ExperGui::pass_used_instruments( QStringList & "
               "occupied_instruments )";
  emit define_used_instruments (occupied_instruments);
}

void
US_ExperGui::allow_dataDisk_only (void)
{
...
void
US_ExperGui::to_initAutoflow (void)
{
...
void
US_ExperGui::to_live_update (QMap<QString, QString> &protocol_details)
{
...
void
US_ExperGui::to_import (QMap<QString, QString> &protocol_details)
{
...
void
US_ExperGui::us_exp_is_closed_set_button ()
{
...
void
US_ExperGui::expsetup_msg_closed (void)
{
  mainw->msg_expsetup->accept ();
  // mainw->diag_expsetup->close();
}

// On click to open US_Experiment  <-- NOT USED, us_experimnet is loaded
// immediately
void
US_ExperGui::manageExperiment ()
{
  qDebug () << "test00";
  ...
  new_height = mainw->gen_banner->height () + mainw->welcome->height ()
               + height_cum + sdiag->height () + mainw->logWidget->height ()
               + mainw->test_footer->height () + offset;

  qDebug () << "Tab width: " << tab_width
            << ", US_Exp width: " << sdiag->width ();
...
// US_Observe
// /////////////////////////////////////////////////////////////////////////////////
US_ObservGui::US_ObservGui (QWidget *topw) : US_WidgetsDialog (topw, 0)
{
  mainw = (US_ComProjectMain *)topw;
  ...
  connect (this, SIGNAL (to_xpn_viewer (QMap<QString, QString> &)), sdiag,
           SLOT (check_for_data (QMap<QString, QString> &)));

  // ALEXEY: devise SLOT saying what to do upon completion of experiment and
  // exporting AUC data to hard drive - Import Experimental Data  !!!
  connect (sdiag, SIGNAL (experiment_complete_auto (QMap<QString, QString> &)),
           this, SLOT (to_post_processing (QMap<QString, QString> &)));

  // ALEXEY: to reset timers/threads and GUI when returning back to InitDialog
  connect (this, SIGNAL (reset_live_update_passed ()), sdiag,
           SLOT (reset_liveupdate_panel ()));

  // ALEXEY: close program, emitted from sdiag
  connect (sdiag, SIGNAL (close_program ()), this, SLOT (to_close_program ()));

  connect (sdiag, SIGNAL (liveupdate_processes_stopped ()), this,
           SLOT (processes_stopped_passed ()));

  // ALEXEY: premature abortion with no data
  connect (sdiag, SIGNAL (aborted_back_to_initAutoflow ()), this,
           SLOT (to_initAutoflow_xpnviewer ()));
...
void
US_ObservGui::resizeEvent (QResizeEvent *event)
{
  int tab_width = mainw->tabWidget->tabBar ()->width ();
  int upper_height = mainw->gen_banner->height () + // mainw->welcome->height()
                     +mainw->logWidget->height ()
...
void
US_ObservGui::to_initAutoflow_xpnviewer (void)
{
  emit stop_nodata ();
}

void
US_ObservGui::processes_stopped_passed (void)
{
  emit processes_stopped ();
}

void
US_ObservGui::reset_live_update (void)
{
...
void
US_ObservGui::process_protocol_details (
    QMap<QString, QString> &protocol_details)
{
  // Use protocol details to retrieve data from Optima's DB
  // Query ExperimentRun table for runname/ExpDefId
  // If null (i.e. run is not launched yet), Information box - "Run was
  // submitted, but not launched yet.. Awaiting for data to emerge."

  // QString mtitle    = tr( "Reading Protocol" );
  // QString message   = tr( "Protocol details passed. <br> Name: %1 <br> ID:
  // %2"
  // ).arg(protocol_details["experimentName"]).arg(protocol_details["experimentId"]);
  // QMessageBox::information( this, mtitle, message );

  emit to_xpn_viewer (protocol_details);
}

// void US_ObservGui::to_post_processing( QString & currDir, QString &
...
void
US_ObservGui::to_post_processing (QMap<QString, QString> &protocol_details)
{
  emit switch_to_post_processing (protocol_details);
}

void
US_ObservGui::to_close_program (void)
{
  // sdiag->close();
...
// US_PostProd
US_PostProdGui::US_PostProdGui (QWidget *topw) : US_WidgetsDialog (topw, 0)
{
  mainw = (US_ComProjectMain *)topw;
  ...
  // panel_desc->viewport()->setAutoFillBackground(false);
  // panel_desc->setFrameStyle(QFrame::NoFrame);
  // panel_desc->setPlainText(" Tab to Retrieve and Process Experimental
  // Data..."); panel_desc->setReadOnly(true);
  // //panel_desc->setMaximumHeight(30);
  // QFontMetrics m (panel_desc -> font()) ;
  // int RowHeight = m.lineSpacing() ;
  // panel_desc -> setFixedHeight  (2* RowHeight) ;
  ...
  connect (this, SIGNAL (to_post_prod (QMap<QString, QString> &)), sdiag,
           SLOT (import_data_auto (QMap<QString, QString> &)));
  connect (this, SIGNAL (reset_lims_import_passed ()), sdiag,
           SLOT (reset_limsimport_panel ()));

  // emit signal after data loaded
  connect (sdiag, SIGNAL (data_loaded ()), this, SLOT (resize_main ()));

  // ALEXEY: switch to Editing
  connect (sdiag, SIGNAL (saving_complete_auto (QMap<QString, QString> &)),
           this, SLOT (to_editing (QMap<QString, QString> &)));
  // ALEXEY: for academic ver. switch back to experiment
  connect (sdiag, SIGNAL (saving_complete_back_to_initAutoflow ()), this,
           SLOT (to_initAutoflow ()));
...
void
US_PostProdGui::resize_main (void)
{
  // Trigger resize to update size of the Edit_Data
...
void
US_PostProdGui::reset_lims_import (void)
{
  emit reset_lims_import_passed ();
}

void
US_PostProdGui::import_data_us_convert (
    QMap<QString, QString> &protocol_details)
{
...
void
US_PostProdGui::to_editing (QMap<QString, QString> &protocol_details)
{
...
void
US_PostProdGui::to_initAutoflow (void)
{
  emit switch_to_initAutoflow ();
}

void
US_PostProdGui::resizeEvent (QResizeEvent *event)
{
  int tab_width = mainw->tabWidget->tabBar ()->width ();
  int upper_height = mainw->gen_banner->height () + // mainw->welcome->height()
                     +mainw->logWidget->height ()
...
// US_Editing
US_EditingGui::US_EditingGui (QWidget *topw) : US_WidgetsDialog (topw, 0)
{
  mainw = (US_ComProjectMain *)topw;
  ...
  // panel_desc->viewport()->setAutoFillBackground(false);
  // panel_desc->setFrameStyle(QFrame::NoFrame);
  // panel_desc->setPlainText(" Tab to Edit Experimental Data... ---UNDER
  // CONSTRUCTION--- "); panel_desc->setReadOnly(true);
  // //panel_desc->setMaximumHeight(30);
  // QFontMetrics m (panel_desc -> font()) ;
  // int RowHeight = m.lineSpacing() ;
  ...
  // //Later - do actual editing form sdiag (load_auto() ) - whatever it will
  // be: (us_edit.cpp)
  connect (this, SIGNAL (start_editing (QMap<QString, QString> &)), sdiag,
           SLOT (load_auto (QMap<QString, QString> &)));
  // emit signal after data loaded
  connect (sdiag, SIGNAL (data_loaded ()), this, SLOT (resize_main ()));

  connect (this, SIGNAL (reset_data_editing_passed ()), sdiag,
           SLOT (reset_editdata_panel ()));

  // ALEXEY: switch to Analysis || REPORT (abde)
  connect (sdiag, SIGNAL (edit_complete_auto (QMap<QString, QString> &)), this,
           SLOT (to_analysis (QMap<QString, QString> &)));
  connect (sdiag, SIGNAL (edit_complete_auto_abde (QMap<QString, QString> &)),
           this, SLOT (to_report (QMap<QString, QString> &)));
...
void
US_EditingGui::resize_main (void)
{
  // Trigger resize to update size of the Edit_Data
  int curr_h = mainw->height () + 1;
  int curr_w = mainw->width () + 1;
...
void
US_EditingGui::resizeEvent (QResizeEvent *event)
{
  int tab_width = mainw->tabWidget->tabBar ()->width ();
  int upper_height = mainw->gen_banner->height () + // mainw->welcome->height()
                     +mainw->logWidget->height ()
...
void
US_EditingGui::to_analysis (QMap<QString, QString> &protocol_details)
{
  emit switch_to_analysis (protocol_details);
}

void
US_EditingGui::to_report (QMap<QString, QString> &protocol_details)
{
  emit switch_to_report (protocol_details);
}

void
US_EditingGui::to_initAutoflow (void)
{
  emit switch_to_initAutoflow ();
}

void
US_EditingGui::reset_data_editing (void)
{
  emit reset_data_editing_passed ();
}

void
US_EditingGui::do_editing (QMap<QString, QString> &protocol_details)
{
  emit start_editing (protocol_details);
}

// US_Analysis
US_AnalysisGui::US_AnalysisGui (QWidget *topw) : US_WidgetsDialog (topw, 0)
{
  mainw = (US_ComProjectMain *)topw;
  ...
  // panel_desc->viewport()->setAutoFillBackground(false);
  // panel_desc->setFrameStyle(QFrame::NoFrame);
  // panel_desc->setPlainText(" Tab to Analyse Experimental Data... ---UNDER
  // CONSTRUCTION--- "); panel_desc->setReadOnly(true);
  // //panel_desc->setMaximumHeight(30);
  // QFontMetrics m (panel_desc -> font()) ;
  // int RowHeight = m.lineSpacing() ;
  ...
  connect (this, SIGNAL (start_analysis (QMap<QString, QString> &)), sdiag,
           SLOT (initPanel (QMap<QString, QString> &)));
  // In initPanel() - re-generate GUI based on # of rows corresponding to #
  // stages in AProfile...

  // When coming back to Manage Optima Runs
  connect (sdiag, SIGNAL (analysis_update_process_stopped ()), this,
           SLOT (processes_stopped_passed ()));

  connect (sdiag, SIGNAL (close_analysissetup_msg ()), this,
           SLOT (analysissetup_msg_closed ()));

  // ALEXEY: back to initAutoflow when user stuck at FITMEN (already processed)
  // and program proceeded to REPORT
  connect (sdiag, SIGNAL (analysis_back_to_initAutoflow ()), this,
           SLOT (to_initAutoflow ()));

  // ALEXEY: switch to Report
  connect (sdiag, SIGNAL (analysis_complete_auto (QMap<QString, QString> &)),
           this, SLOT (to_report (QMap<QString, QString> &)));
...
void
US_AnalysisGui::resizeEvent (QResizeEvent *event)
{
  int tab_width = mainw->tabWidget->tabBar ()->width ();
  int upper_height = mainw->gen_banner->height () + // mainw->welcome->height()
                     +mainw->logWidget->height ()
...
void
US_AnalysisGui::do_analysis (QMap<QString, QString> &protocol_details)
{
  emit start_analysis (protocol_details);
}

void
US_AnalysisGui::processes_stopped_passed (void)
{
  emit processes_stopped ();
}
...
// Close msg on setting up new EXP
void
US_AnalysisGui::analysissetup_msg_closed (void)
{
...
void
US_AnalysisGui::to_initAutoflow (void)
{
  emit switch_to_initAutoflow ();
}
...
void
US_AnalysisGui::to_report (QMap<QString, QString> &protocol_details)
{
  emit switch_to_report (protocol_details);
}
    ...
    : US_WidgetsDialog (topw, 0)
{
  ...
  // panel_desc->viewport()->setAutoFillBackground(false);
  // panel_desc->setFrameStyle(QFrame::NoFrame);
  // panel_desc->setPlainText(" Tab to Generate Report...  ---UNDER
  // CONSTRUCTION--- "); panel_desc->setReadOnly(true);
  // //panel_desc->setMaximumHeight(30);
  // QFontMetrics m (panel_desc -> font()) ;
  // int RowHeight = m.lineSpacing() ;
  ...
  // connect( this, SIGNAL( start_report( QMap < QString, QString > & ) ),
  // sdiag, SLOT( initPanel ( QMap < QString, QString > & )  ) );
  connect (this, SIGNAL (start_report (QMap<QString, QString> &)), sdiag,
           SLOT (loadRun_auto (QMap<QString, QString> &)));

  connect (this, SIGNAL (reset_reporting_passed ()), sdiag,
           SLOT (reset_report_panel ()));
...
void
US_ReportStageGui::resizeEvent (QResizeEvent *event)
{
  int tab_width = mainw->tabWidget->tabBar ()->width ();
  int upper_height = mainw->gen_banner->height () + // mainw->welcome->height()
                     +mainw->logWidget->height ()
...
void
US_ReportStageGui::do_report (QMap<QString, QString> &protocol_details)
{
  emit start_report (protocol_details);
}

void
US_ReportStageGui::reset_reporting (void)
{
    ...
    : US_WidgetsDialog (topw, 0)
{
  ...
  connect (this, SIGNAL (start_esign (QMap<QString, QString> &)), sdiag,
           SLOT (initPanel_auto (QMap<QString, QString> &)));
  connect (this, SIGNAL (reset_esigning_passed ()), sdiag,
           SLOT (reset_esign_panel ()));
...
void
US_eSignaturesGui::resizeEvent (QResizeEvent *event)
{
  int tab_width = mainw->tabWidget->tabBar ()->width ();
  int upper_height = mainw->gen_banner->height () + // mainw->welcome->height()
                     +mainw->logWidget->height ()
...
void
US_eSignaturesGui::do_esign (QMap<QString, QString> &protocol_details)
{
  emit start_esign (protocol_details);
}

void
US_eSignaturesGui::reset_esigning (void)
{
