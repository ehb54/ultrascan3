...
US_ComProjectMain::US_ComProjectMain(QString us_mode) : US_Widgets() {
  ...
  // QTabBar::tab:selected {background: qlineargradient(x1: 0, y1: 0, x2: 0, y2:
  // 1, stop: 0 #fafafa, stop: 0.4 #f4f4f4, stop: 0.5 #e7e7e7, stop: 1.0
  ...
  for (int i = 0; i < tabWidget->count(); ++i) {
    // ALEXEY: OR enable all tabs ? (e.g. for demonstration, in a read-only mode
    // or the like ?)
    if (i == 0)
  ...
  connect(epanInit, SIGNAL(switch_to_live_update_init(QMap<QString, QString>&)),
          this, SLOT(switch_to_live_update(QMap<QString, QString>&)));
  connect(epanInit,
          SIGNAL(switch_to_post_processing_init(QMap<QString, QString>&)), this,
  ...
  connect(epanExp, SIGNAL(switch_to_live_update(QMap<QString, QString>&)), this,
          SLOT(switch_to_live_update(QMap<QString, QString>&)));
  ...
  connect(this, SIGNAL(pass_to_live_update(QMap<QString, QString>&)),
          epanObserv, SLOT(process_protocol_details(QMap<QString, QString>&)));
  ...
  connect(this, SIGNAL(pass_to_post_processing(QMap<QString, QString>&)),
          epanPostProd, SLOT(import_data_us_convert(QMap<QString, QString>&)));
...
US_ComProjectMain::US_ComProjectMain() : US_Widgets() {
  ...
  connect(epanInit,
          SIGNAL(switch_to_post_processing_init(QMap<QString, QString>&)), this,
          SLOT(switch_to_post_processing(QMap<QString, QString>&)));
  connect(epanInit, SIGNAL(switch_to_editing_init(QMap<QString, QString>&)),
          this, SLOT(switch_to_editing(QMap<QString, QString>&)));
  connect(epanInit, SIGNAL(switch_to_analysis_init(QMap<QString, QString>&)),
  ...
  connect(epanInit, SIGNAL(switch_to_esign_init(QMap<QString, QString>&)), this,
          SLOT(switch_to_esign(QMap<QString, QString>&)));
  connect(epanInit, SIGNAL(to_initAutoflow()), this, SLOT(close_all()));
  ...
  connect(epanPostProd, SIGNAL(switch_to_editing(QMap<QString, QString>&)),
          this, SLOT(switch_to_editing(QMap<QString, QString>&)));
  connect(this, SIGNAL(pass_to_editing(QMap<QString, QString>&)), epanEditing,
          SLOT(do_editing(QMap<QString, QString>&)));
  connect(this, SIGNAL(reset_lims_import()), epanPostProd,
          SLOT(reset_lims_import()));
  connect(epanPostProd, SIGNAL(switch_to_initAutoflow()), this,
          SLOT(close_all()));
  ...
  connect(epanEditing, SIGNAL(switch_to_analysis(QMap<QString, QString>&)),
          this, SLOT(switch_to_analysis(QMap<QString, QString>&)));
  connect(epanEditing, SIGNAL(switch_to_report(QMap<QString, QString>&)), this,
          SLOT(switch_to_report(QMap<QString, QString>&)));
  connect(this, SIGNAL(pass_to_analysis(QMap<QString, QString>&)), epanAnalysis,
          SLOT(do_analysis(QMap<QString, QString>&)));
  ...
  connect(epanAnalysis, SIGNAL(switch_to_report(QMap<QString, QString>&)), this,
          SLOT(switch_to_report(QMap<QString, QString>&)));
  connect(this, SIGNAL(pass_to_report(QMap<QString, QString>&)), epanReport,
          SLOT(do_report(QMap<QString, QString>&)));

  connect(this, SIGNAL(reset_reporting()), epanReport, SLOT(reset_reporting()));

  // E-Signs
  connect(epanReport, SIGNAL(switch_to_esign(QMap<QString, QString>&)), this,
          SLOT(switch_to_esign(QMap<QString, QString>&)));
...
void US_ComProjectMain::checkDataLocation(void) {
  ...
  if (US_Settings::default_data_location() == 2)  // Disk
  {
    ...
    QMessageBox::warning(this, tr("Data Location set to Disk"),
                         tr("Please change Data Location to \"Database\" in "
                            "the configuraiton of your database! \n"));
...
void US_ComProjectMain::initPanels(int panx) {
  ...
  if (panx == 0)  // Autoflow Records
  {
    ...
    // current stages: E.g., LIVE_UPDATE: stop all timers, reset GUI etc.. Maybe
    // a warning message "You are going to leave this panel.." needs to be shown
    ...
    if (curr_panx == 2) {
      // 2 cases:
      /*
            1. Avaiting for run to be launched ("Back to Managing Optima Runs")
         -- SET
            2. More complex: Back to Managing runs from active LIVE_UPDATE stage
         -- stop all timers and other processes...
      */

      qDebug() << "Jumping from LIVE UPDATE.";

      // 2. Stop all timers/threads, reset GUI - when stopping fully working
      // LIVE UPDATE
...
void US_ComProjectMain::show_liveupdate_finishing_msg(void) {
  ...
  msg_liveupdate_finishing->setWindowFlags(
...
void US_ComProjectMain::show_analysis_update_finishing_msg(void) {
  ...
  msg_analysis_update_finishing->setWindowFlags(
      Qt::CustomizeWindowHint | Qt::WindowTitleHint | Qt::WindowStaysOnTopHint);
...
void US_ComProjectMain::close_initDialogue(void) {
  ...
  if (epanInit->initDialogueOpen) epanInit->pdiag_autoflow->close();
  if (epanInit->initMsgNorecOpen) epanInit->msg_norec->reject();
  // msg_norec->close();
}
...
void US_ComProjectMain::delete_psql_record(int ExpId) {
  ...
  if (dbxpn.open()) {
    ...
    query_abs_scan.lastError().text(); QMessageBox::critical( this, tr( "*ERROR*
    in Deleting Absorbance Record" ), tr( "An error occurred in the attempt to
    delete" " AbsorbanceScanParameters from AUC DB\n  %1 table\n  %2 ." ).arg(
    qrytab_abs ).arg( errmsg ) ); return;
      }
    ...
    // FugeProfile
    int FugeId = 308;
    if(! query_fuge.prepare(QString("DELETE FROM %1 WHERE \"FugeRunProfileId\" =
    %2").arg(qrytab_fuge).arg(FugeId) ) ) qDebug() <<
    ...
    if (query_fuge.exec())
      ...
      {
        QString errmsg   = "Delete record error: " +
    query_fuge.lastError().text();; QMessageBox::critical( this, tr( "*ERROR* in
    Deleting Fuge Profile" ), tr( "An error occurred in the attempt to delete"
                                   " FugeProfile from AUC DB\n  %1 table\n  %2
    ." ).arg( qrytab_fuge ).arg( errmsg ) ); return;
      }
    ...
    */
    ...
    if (query_expdef.exec()) {
      ...
      ;
      QMessageBox::critical(this, tr("*ERROR* in Deleting Experimental Method"),
...
// Slot to define new exp. (from initial dialog)
void US_ComProjectMain::define_new_experiment(
...
// Slot to pass submitted to Optima run info to the Live Update tab
void US_ComProjectMain::switch_to_live_update(
...
void US_ComProjectMain::switch_to_post_processing(
    QMap<QString, QString>& protocol_details) {
...
void US_ComProjectMain::switch_to_editing(
    QMap<QString, QString>& protocol_details) {
...
// Slot to switch from the Import to Editing tab
void US_ComProjectMain::switch_to_analysis(
    QMap<QString, QString>& protocol_details) {
  ...
  msg_analysissetup->setWindowFlags(
      Qt::CustomizeWindowHint | Qt::WindowTitleHint | Qt::WindowStaysOnTopHint);
...
void US_ComProjectMain::switch_to_report(
    QMap<QString, QString>& protocol_details) {
...
void US_ComProjectMain::switch_to_esign(
    QMap<QString, QString>& protocol_details) {
...
void US_ComProjectMain::call_AutoflowDialogue(
    void)  //<--- Entry point from main
{
  ...
  for (int i = 0; i < this->instruments.size(); ++i) {
    ...
    if (!combined_check || daysToExpiration <= 0 || !certfile_exists) {
      ...
      if (certfile_exists) {
        if (daysToExpiration > 0) {
          ...
          msg_sys_text_info +=
              QString(tr("1. %1 is turned on\n"
                         ...
                         "$HOME/ultrascan/etc/optima and is valid and not "
                         "expired.\n"))
                  .arg(alias);
          //"Submission of the experimental protocol is suspended until this
          //condition is resolved." )).arg(alias);
        } else {
          msg_sys_text_info +=
        ...
        msg_sys_text_info +=
            QString(tr("\n%1 It appears the license key for %2 is missing:"))
                .arg(QChar(0x2022), alias);
        msg_sys_text_info +=
  ...
  if (count_instruments_disconnected == this->instruments.size()) {
    ...
    if (Optima_names.size() > 1)
      msg_sys_text_info_final +=
          QString(tr("\n\nSubmission of the experimental protocol to the "
                     "Optima instruments is suspended until at least one of "
                     "these conditions is resolved."));
    else
    ...
    if (!mainw->us_mode_bool)
      msg_sys_text_info_final += QString(tr(
          "\n\nUser has an option to upload data from disk (tab 2. Lab/Rotor "
          "-> Select Data Source) and proceed with that workflow."));
...
void US_InitDialogueGui::initRecordsDialogue(void) {
  ...
  if (autoflow_records < 1 ||
      autoflowdata.size() <
          1)  // can be that for UL<3, autoflow runs from others are not shown
  {
    ...
    msg_norec->setText(
        tr("There are no Optima runs to follow.<br><br>"
           "You will be switched to <b>Experiment</b> stage to design and "
           "submit new protocol."));
    ...
    if (msg_norec->exec() == QMessageBox::Ok) {
      ...
      qApp->processEvents();
      if (isDataDiskOnly) emit pass_allow_dataDisk_only();
    }
  ...
  pdiag_autoflow =
      new US_SelectItem(autoflowdata, hdrs, pdtitle, &prx, autoflow_btn, -3);
  ...
  if (failedID.toInt() != 0) {
    ...
    msgBox_f.setText(tr("The selected GMP run is marked as FAILED:<br><br>") +
                     ...
                     tr("<b>Failing stage:&emsp;</b> ") +
                     failed_details["failedStage"] + tr("<br>") +
    ...
    if (msgBox_f.clickedButton() == Cancel) {
      ...
      /*
        ...
        5. delete autoflowHistory record (if was already created - ONLY case if
        REPORT was proceeded ?)
...
void US_InitDialogueGui::do_run_tables_cleanup(
...
void US_InitDialogueGui::do_create_autoflowStatus_for_failedRun(
    QMap<QString, QString> run_details) {
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd();
  ...
  if (!autoflowStatusID) {
    QMessageBox::warning(this, tr("AutoflowStatus Record Problem"),
                         tr("autoflowStatus (FAILED GMP run re-INIT): "
                            "There was a problem with creating a record in "
                            "autoflowStatus table \n") +
                             db->lastError());

    return;
  }
  qDebug() << "in do_create_autoflowStatus_for_failedRun: createGMPRun_Json -- "
           << createGMPRun_Json;
...
void US_InitDialogueGui::do_run_data_cleanup(
    QMap<QString, QString> run_details) {
  // Get proper filename
...
void US_InitDialogueGui::refresh_optima_states(void) { initAutoflowPanel(); }
...
int US_InitDialogueGui::list_all_autoflow_records(
  ...
  if (dbP->next()) user_id = dbP->value(0).toInt();
  ...
  if (dbP->lastErrno() != US_DB2::OK) return nrecs;
  ...
  while (dbP->next()) {
    ...
    // autoflowentry << id << runname << optimaname  << time_created.toString();
    // // << time_started.toString(); // << local.toString( Qt::ISODate );
    ...
    if (status == "E-SIGNATURES") {
      ...
      if (US_Settings::us_inv_level() < 3) {
        ...
        qDebug() << "2. IN list_all_autoflow_records(), autoflowentry -- "
                 << autoflowentry;
      }
    }
  }
...
QMap<QString, QString> US_InitDialogueGui::selectCurrentOptima(
    QString optimaname) {
  QMap<QString, QString> currentInstrument;
...
bool US_InitDialogueGui::isOperRev(int uID, QString autoflow_id) {
  ...
  if (db->lastErrno() != US_DB2::OK) return yesRev;
  ...
  if (!jsonDocRevList.isArray() || !jsonDocOperList.isArray() ||
    ...
    qDebug() << "jsonDocRevList OR jsonDocOperList OR jsonDocApprList not a "
                "JSON Array!";
    return yesRev;
  }
...
// Query autoflow for # records
QMap<QString, QString> US_InitDialogueGui::read_autoflow_record(
    int autoflowID) {
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd();
...
// Query autoflow for # records
QMap<QString, QString> US_InitDialogueGui::read_autoflow_failed_record(
    QString failedID) {
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd();
...
US_ExperGui::US_ExperGui(QWidget* topw) : US_WidgetsDialog(topw, 0) {
  ...
  if (mainw->us_mode_bool) sdiag->us_mode_passed();
...
void US_ExperGui::to_initAutoflow(void) { emit switch_to_initAutoflow(); }
...
void US_ExperGui::exp_cleared(void) { emit to_autoflow_records(); }
...
void US_ObservGui::to_initAutoflow_xpnviewer(void) { emit stop_nodata(); }

void US_ObservGui::processes_stopped_passed(void) { emit processes_stopped(); }

void US_ObservGui::reset_live_update(void) { emit reset_live_update_passed(); }

// Live Update's get and use submitted run's protocol details
void US_ObservGui::process_protocol_details(
    QMap<QString, QString>& protocol_details) {
...
void US_ObservGui::to_post_processing(
    QMap<QString, QString>& protocol_details) {
...
US_PostProdGui::US_PostProdGui(QWidget* topw) : US_WidgetsDialog(topw, 0) {
  ...
  connect(sdiag, SIGNAL(saving_complete_auto(QMap<QString, QString>&)), this,
          SLOT(to_editing(QMap<QString, QString>&)));
...
void US_PostProdGui::import_data_us_convert(
    QMap<QString, QString>& protocol_details) {
...
void US_PostProdGui::to_initAutoflow(void) { emit switch_to_initAutoflow(); }
...
US_EditingGui::US_EditingGui(QWidget* topw) : US_WidgetsDialog(topw, 0) {
  ...
  connect(sdiag, SIGNAL(edit_complete_auto_abde(QMap<QString, QString>&)), this,
          SLOT(to_report(QMap<QString, QString>&)));
...
void US_EditingGui::to_initAutoflow(void) { emit switch_to_initAutoflow(); }
...
US_AnalysisGui::US_AnalysisGui(QWidget* topw) : US_WidgetsDialog(topw, 0) {
  ...
  connect(sdiag, SIGNAL(analysis_complete_auto(QMap<QString, QString>&)), this,
          SLOT(to_report(QMap<QString, QString>&)));
...
void US_AnalysisGui::to_initAutoflow(void) { emit switch_to_initAutoflow(); }
...
void US_ReportStageGui::reset_reporting(void) { emit reset_reporting_passed(); }
...
void US_eSignaturesGui::reset_esigning(void) { emit reset_esigning_passed(); }
