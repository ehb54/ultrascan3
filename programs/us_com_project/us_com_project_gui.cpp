...
US_ComProjectMain::US_ComProjectMain(QString us_mode) : US_Widgets() {
   ...
   welcome->setText(" <br> Welcome to UltraScan GMP<br> for setting up, monitoring, editing and analyzing <br> AUC "
                    "experiments and produced data... <br>");
   ...
   connect(
      ...
      SLOT(switch_to_live_update(QMap<QString, QString> &)));
   connect(
      epanExp, SIGNAL(switch_to_import(QMap<QString, QString> &)), this,
      SLOT(switch_to_post_processing(QMap<QString, QString> &)));
   connect(
...
US_ComProjectMain::US_ComProjectMain() : US_Widgets() {
   ...
   tabWidget->tabBar()->setStyleSheet(
      ...
      "stop: 0 #fafafa, stop: 0.4 #f4f4f4, stop: 0.5 #e7e7e7, stop: 1.0 #fafafa); } QTabBar::tab:first {background: "
      "blue; color: lightgray; min-width: 50;}  QTabBar::tab:first:hover {background: #4169E1; color: white}  "
      "QTabBar::tab:disabled { color: rgba(0, 0, 0, 70%)  } QTabBar::tab:last:disabled {background: #90EE90;  "
      "min-width: 50;}  QTabBar::tab:last:selected {background: #556B2F; color: lightgray;  min-width: 50;}");
   ...
   connect(
      epanInit, SIGNAL(switch_to_esign_init(QMap<QString, QString> &)), this,
   ...
   connect(
      ...
      SLOT(switch_to_analysis(QMap<QString, QString> &)));
   connect(
      epanEditing, SIGNAL(switch_to_report(QMap<QString, QString> &)), this,
      SLOT(switch_to_report(QMap<QString, QString> &)));
   connect(
   ...
   //E-Signs
   connect(
      epanReport, SIGNAL(switch_to_esign(QMap<QString, QString> &)), this,
      SLOT(switch_to_esign(QMap<QString, QString> &)));
...
void US_InitDialogueGui::checkCertificates(void) {
   ...
   for (int i = 0; i < this->instruments.size(); ++i) {
      ...
      qDebug() << "In CHECK Connection: alias, dbhost, optima_msgPort -- " << alias << dbhost << optima_msgPort;
      ...
      qDebug() << "combined_check, daysToExpiration, certfile_exists -- " << combined_check << daysToExpiration
               << certfile_exists;
      ...
      if (!combined_check || daysToExpiration <= 0 || !certfile_exists) {
         ...
         if (certfile_exists) {
            if (daysToExpiration > 0) {
               ...
               msg_sys_text_info += QString(tr("1. %1 is turned on\n"
                                               ...
                                               "3. your license key is stored in $HOME/ultrascan/etc/optima and is "
         ...
         if (daysToExpiration <= 30) {
            ...
            QMessageBox::warning(
               this, QString(tr("License Key for %1 Nearing Expiraiton")).arg(alias),
               QString(
   ...
   if (count_instruments_disconnected == this->instruments.size()) {
      ...
      QString msg_sys_text
         = QString("Attention! UltraScan is not able to communicate with the data acquisition server(s) on the %1.")
              .arg(inst_names);
      msgBox_sys_data.setText(msg_sys_text);
      ...
      if (Optima_names.size() > 1)
         msg_sys_text_info_final += QString(tr("\n\nSubmission of the experimental protocol to the Optima instruments "
                                               "is suspended until at least one of these conditions is resolved."));
      else
         msg_sys_text_info_final += QString(tr("\n\nSubmission of the experimental protocol to the Optima instrument "
                                               "is suspended until this condition is resolved."));
   ...
   // Normal re-attachement
      ...
      if (stage == "EDITING") {
         ...
         if (currDir.isEmpty() || !directory.exists()) {
            ...
            {
               QMessageBox::warning(
                  this, tr("Data Absent on Disk!"), tr("No data associated with this run is found on the disk! \n"));
               initAutoflowPanel();
               //emit to_initAutoflow();
               return;
            }
   ...
   while (dbP->next()) {
      ...
      else //Experiment was started on Optima (at least we know that...)
      {
         if (status == "LIVE_UPDATE") {
            ...
            if (xpn_data->connect_data(
                   currentInstrument[ "optimaHost" ], currentInstrument[ "optimaPort" ].toInt(),
                   currentInstrument[ "optimaDBname" ], currentInstrument[ "optimaDBusername" ],
      ...
      if (status == "E-SIGNATURES") {
         ...
         if (US_Settings::us_inv_level() < 3) {
            ...
            qDebug() << "user_id, operatorID.toInt(), invID.toInt() -- " << user_id << operatorID.toInt()
                     << invID.toInt();
...
US_ExperGui::US_ExperGui(QWidget *topw) : US_WidgetsDialog(topw, 0) {
   ...
   connect(sdiag, SIGNAL(to_import(QMap<QString, QString> &)), this, SLOT(to_import(QMap<QString, QString> &)));
...
US_EditingGui::US_EditingGui(QWidget *topw) : US_WidgetsDialog(topw, 0) {
   ...
   connect(
      sdiag, SIGNAL(edit_complete_auto_abde(QMap<QString, QString> &)), this, SLOT(to_report(QMap<QString, QString> &)));
...
//eSignatures
US_eSignaturesGui::US_eSignaturesGui(QWidget *topw) : US_WidgetsDialog(topw, 0) {
   mainw = ( US_ComProjectMain * ) topw;
