...
US_Norm_Profile::US_Norm_Profile(QString auto_mode) : US_Widgets() {
   ...
   connect(cb_chann, SIGNAL(currentIndexChanged(int)), SLOT(new_chann_auto(int)));
   ...
   usplot = new US_Plot(plot, tr(""), tr("Radius (in cm)"), tr("Absorbance"), true, "", "");
   ...
   connect(lw_inpData, SIGNAL(itemDoubleClicked(QListWidgetItem *)), this, SLOT(slt_addRmItem(QListWidgetItem *)));
   ...
   connect(picker, SIGNAL(cMouseUp(const QwtDoublePoint &)), this, SLOT(slt_mouse(const QwtDoublePoint &)));
...
void US_Norm_Profile::load_data_auto_report(QMap<QString, QString> &protocol_details) {
   ...
   QMap<QString, QString> abde_analysis_parms = read_autoflowAnalysisABDE_record(protocol_details[ "autoflowID" ]);

   qDebug() << "ABDE analysis PARMS: -- " << abde_analysis_parms[ "ID" ] << abde_analysis_parms[ "etype" ]
            << abde_analysis_parms[ "xnorms_percents" ] << abde_analysis_parms[ "filename_blc" ];

   //parse respective JSONs
   parse_abde_analysis_jsons(
      abde_analysis_parms[ "xnorms_percents" ], protocol_details, data_per_channel_xnorm, data_per_channel_norm_cb,
      data_per_channel_ranges_percents, data_per_channel_rmsd, data_per_channel_meniscus);

   parse_abde_analysis_jsons(
      abde_analysis_parms[ "filename_blc" ], protocol_details, data_per_channel_xnorm, data_per_channel_norm_cb,
      data_per_channel_ranges_percents, data_per_channel_rmsd, data_per_channel_meniscus);
   ...
   qDebug() << "[in load_for_REPORT]: us_auto_mode, abde_etype -- " << us_auto_mode << abde_etype;
...
void US_Norm_Profile::slt_loadAUC_auto_report(QMap<QString, QString> &protocol_details) {
   ...
   for (int i = 0; i < triples.size(); i++) {
      ...
      QString fn
         = tr("%1.%2.%3.%4.%5").arg(runid, dataType, ccw.at(0).trimmed(), ccw.at(1).trimmed(), ccw.at(2).trimmed());
   ...
   for (int i = 0; i < channList.size(); ++i) {
      ...
      qDebug() << "channel, " << channList[ i ]
               ...
               << data_per_channel_xnorm[ channList[ i ] ] << data_per_channel_norm_cb[ channList[ i ] ]
               << data_per_channel_rmsd[ channList[ i ] ] << data_per_channel_ranges_percents[ channList[ i ] ]
...
void US_Norm_Profile::slt_loadAUC_auto(QMap<QString, QString> &protocol_details) {
   ...
   if (badFiles.size() != 0) {
      QMessageBox::warning(this, "Error!", "These files could not be loaded!\n" + badFiles.join("\n"));
   }
   ...
   qDebug() << "in ABDE_norm: reading eProfiles BC-- " << protocol_details[ "baseline_corrections" ];
   qDebug() << "in ABDE_norm: reading meniscus info -- " << protocol_details[ "meniscus_info" ];
...
void US_Norm_Profile::new_chann_auto(int index) {
   ...
   for (int i = 0; i < filenames.size(); ++i) {
      ...
      if (channame.contains(file_n_list[ 2 ]) && channame.contains(file_n_list[ 3 ])) {
...
void US_Norm_Profile::slt_loadAUC() {
   ...
   if (!isLocal) {
      ...
      for (int i = 0; i < triples.size(); i++) {
         QStringList ccw = triples.at(i).split(u'/');
         US_DataIO::RawData rawData = allData.at(i);
         QString fn = tr("%1.%2.%3.%4").arg(runid, ccw.at(0).trimmed(), ccw.at(1).trimmed(), ccw.at(2).trimmed());
         QString fp = tr("%1.%2.auc").arg(dirname, fn);
         filenames << fn;
         filePaths << fp;
         lw_inpData->addItem(fn);
      ...
      QStringList fPath
         = QFileDialog::getOpenFileNames(this, tr("Open AUC File"), US_Settings::importDir(), tr(".auc (*.auc)"));
      if (fPath.size() == 0)
         return;
...
QMap<QString, QVector<double>> US_Norm_Profile::trapz(QVector<double> xval, QVector<double> yval) {
...
QPair<int, int> US_Norm_Profile::getXlimit(QVector<double> xval_in, double xmin, double xmax) {
...
void US_Norm_Profile::selectData_auto(void) {
   ...
   for (int i = 0; i < lw_selData->count(); i++) {
      inpIds << filenames.indexOf(lw_selData->item(i)->text());
      //get channame
      qDebug() << "in selectData_auto(): filename -- " << lw_selData->item(i)->text();
...
void US_Norm_Profile::plotData(void) {
   ...
   if (ckb_rawData->isChecked()) {
      ...
      for (int i = 0; i < nd; i++) {
         ...
         if (us_auto_mode) {
            if (abde_etype == "MWL") {
               if (
                  selFilenames.at(i).contains("_002") || selFilenames.at(i).contains(".002")
                  || selFilenames.at(i).endsWith(".2") || selFilenames.at(i).contains("protein", Qt::CaseInsensitive)
                  ...
                  selFilenames.at(i).contains("_001") || selFilenames.at(i).contains(".001")
                  || selFilenames.at(i).endsWith(".1") || selFilenames.at(i).contains("dna", Qt::CaseInsensitive)) {
      ...
      qDebug() << "[plotting rawData: minY_global, maxY_global ] -- " << minY_global << maxY_global;
   }
   ...
   if (ckb_integral->isChecked()) {
      ...
      qDebug() << "[plotting Integral: minY_global, maxY_global ] -- " << minY_global << maxY_global;
   ...
   if (us_auto_mode && ckb_ranges->isChecked()) {
      ...
      for (int i = 0; i < chann_ranges_l.size(); ++i) {
         ...
         if (chann_ranges_c_l[ 0 ] == channame) {
            qDebug() << "channel " << channame << ", has range(s): " << chann_ranges_c_l[ 1 ];
      ...
      for (int i = 0; i < c_ranges.size(); ++i) {
         ...
         qDebug() << "In Plotting ranges: maxY_global, minY_global ---  " << maxY_global << minY_global;
...
void US_Norm_Profile::find_percent_from_range(
   QString channame, QString point1_s, QString point2_s, QVector<double> xp_intN_protein,
   ...
   qDebug() << "channel " << channame << ", range " << range
            << ", percent: " << double(yp_intN_protein[ p2_ind ] - yp_intN_protein[ p1_ind ]);
}
...
void US_Norm_Profile::slt_save() {
   ...
   QString fname = QFileDialog::getSaveFileName(this, tr("Save File"), US_Settings::reportDir(), tr("(*.csv)"));
   if (fname.size() == 0)
      return;
...
void US_Norm_Profile::save_auto(void) {
   ...
   if (!areAllNormalized_auto(msg_u) && abde_etype == "MWL") {
      ...
      int status
         = QMessageBox::warning(this, tr("Channel Normalization"), msg_u, tr("&Proceed"), tr("&Cancel"), 0, 0, 1);
  ...
  ***/
   ...
   qry1.clear();
   qry1 << "update_autoflowAnalysisABDE_record" << json_p << filename_blc_p << prot_details[ "autoflowID" ];

   int status = dbP->statusQuery(qry1);

   if (status == US_DB2::NO_AUTOFLOW_RECORD) {
      QMessageBox::warning(
         this, tr("AutoflowAnalysisABDE Record Not Updated"),
...
void US_Norm_Profile::record_AnalysisABDE_status(QMap<QString, QString> gmp_form) {
   ...
   if (autoflowStatusID) {
      ...
      qry << "update_autoflowStatusAnalysisABDE_record" // to implement: add 'analysisABDE' && 'analysisABDEts' to autoflowStatus!!!!!
          << QString::number(autoflowStatusID) << prot_details[ "autoflowID" ] << AnalysisABDE_Json;
...
void US_Norm_Profile::revert_autoflowAnalysisABDEstages_record(QString autoflowID) {
   ...
   qDebug() << "[in revert_autoflowAnalysisABDEstages_record(), query] -- " << qry;
...
void US_Norm_Profile::parse_abde_analysis_jsons(
   QString abde_analysis_parms_string, QMap<QString, QString> &protocol_details,
   QMap<QString, double> &data_chann_x_norm, QMap<QString, int> &data_chann_x_norm_cb,
   QMap<QString, QMap<QString, double>> &data_chann_range_percent, QMap<QString, double> &data_chann_rmsd,
