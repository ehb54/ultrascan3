...
US_Norm_Profile::US_Norm_Profile (QString auto_mode) : US_Widgets ()
{
  ...
  connect (pb_next_chann, SIGNAL (clicked ()), SLOT (next_chann_auto ()));
  connect (pb_prev_chann, SIGNAL (clicked ()), SLOT (prev_chann_auto ()));
  ...
  QGridLayout *us_xrange = us_checkbox ("Limit Radius", ckb_xrange);
  pb_pick_rp = us_pushbutton ("Pick Two Points", false);

  ckb_norm_max = new QCheckBox ();
  QGridLayout *us_norm_max
      = us_checkbox (" Normalize\n by Maximum", ckb_norm_max);
  ...
  QSpacerItem *spacer2
      = new QSpacerItem (10, 25 * ihgt, QSizePolicy::Expanding);
  bottom_lyt->addItem (spacer2, 3, 0, 1, 2);

  QPushButton *pb_help = us_pushbutton (tr ("Help"));
  pb_save_auto = us_pushbutton (tr ("Save Profiles"));
  connect (pb_save_auto, SIGNAL (clicked ()), this, SLOT (save_auto ()));
  ...
  usplot = new US_Plot (plot, tr (""), tr ("Radius (in cm)"),
                        tr ("Absorbance"), true, "", "");
  ...
  connect (lw_inpData, SIGNAL (itemDoubleClicked (QListWidgetItem *)), this,
           SLOT (slt_addRmItem (QListWidgetItem *)));
  connect (lw_inpData, SIGNAL (currentRowChanged (int)), this,
           SLOT (slt_inItemSel (int)));
  connect (lw_selData, SIGNAL (currentRowChanged (int)), this,
           SLOT (slt_outItemSel (int)));
  ...
  // connect(pb_pick_norm, SIGNAL(clicked()), this,
  // SLOT(slt_pickPoint_auto()));
  connect (ckb_xrange, SIGNAL (stateChanged (int)), this,
           SLOT (slt_xrange (int)));
  connect (ckb_legend, SIGNAL (stateChanged (int)), this,
           SLOT (slt_legend (int)));
  connect (ckb_integral, SIGNAL (stateChanged (int)), this,
           SLOT (slt_integral (int)));
  connect (ckb_norm, SIGNAL (stateChanged (int)), this, SLOT (slt_norm (int)));
  connect (ckb_rawData, SIGNAL (stateChanged (int)), this,
           SLOT (slt_rawData (int)));
  connect (ckb_norm_max, SIGNAL (stateChanged (int)), this,
           SLOT (slt_norm_by_max (int)));
  connect (ckb_ranges, SIGNAL (stateChanged (int)), this,
           SLOT (slt_ranges (int)));
  ...
  // QString("37a4c516-cf7f-4470-9102-8a98805c2cae"); protocol_details[
  // "filename" ]     = QString("AAV_GMP_test_030325-run2366-dataDiskRun-71");
  ...
  // //protocol_details["ssf_dir_name"]   =
  // QString("/home/alexey/ultrascan/imports/SSF-AAV_GMP_test_030325-run2366-dataDiskRun-71");
  // /** When for REPORT: These next fields will be read from
  // autoflowAnalysisABDE record: 	protocol_details["abde_etype"]     =
  // QString("MWL"); 	protocol_details["directory_for_gmp"] = QString("");
  // 	protocol_details[ "baseline_corrections" ] = QString("");
  // 	protocol_details["filename_abde"]  =
  // QString("SSF-AAV_GMP_test_030325-run2366-dataDiskRun-71");
  // 	protocol_details[ "channels_to_radial_ranges" ] =
  // QString("2A:6.2-6.5,6.6-6.9;2B:5.8-7;4A:6.1-6.5,6.6-6.94;4B:6.25-6.55,6.65-7");
  ...
  // QString("GMP-test-ABDE-Disk-SingleWVL"); protocol_details[ "aprofileguid"
  // ] = QString("0a033cbb-8cc6-4efd-be99-81772f0c7d02"); protocol_details[
  // "filename" ]     =
  // QString("McCue_AAVCsCl_25JAN25-run2255-dataDiskRun-72"); protocol_details[
  ...
  // QString("47"); protocol_details[ "autoflowID" ]   = QString("72");
  // //protocol_details["ssf_dir_name"]   = QString("");
  // /** When for REPORT: These next fields will be read from
  // autoflowAnalysisABDE record: 	protocol_details["abde_etype"]     =
  // QString("SWL"); 	protocol_details["directory_for_gmp"] =
  // QString("/home/alexey/ultrascan/results/McCue_AAVCsCl_25JAN25-run2255-dataDiskRun-72");
  // 	protocol_details[ "baseline_corrections" ] =
  // QString("2A230:6.037,7.08,0.0175125,-0.034958");
  // 	protocol_details["filename_abde"]  =
  // QString("McCue_AAVCsCl_25JAN25-run2255-dataDiskRun-72"); 	protocol_details[
  // "channels_to_radial_ranges" ] = QString("2A:6.2-6.5,6.6-6.9");
  // 	protocol_details[ "x_normalizations" ] = QString("");
  // /** plus info on x_norm form DB's JSON -- for MWL only**/
...
US_Norm_Profile::US_Norm_Profile () : US_Widgets ()
{
  ...
  ckb_norm_max = new QCheckBox ();
  QGridLayout *us_norm_max
      = us_checkbox ("Normalize by Maximum", ckb_norm_max);
  pb_pick_norm = us_pushbutton ("Pick a Point");
...
void
US_Norm_Profile::slt_xrange (int state)
{
...
void
US_Norm_Profile::slt_addRmItem (QListWidgetItem *item)
{
...
void
US_Norm_Profile::slt_rmItem (void)
{
...
void
US_Norm_Profile::slt_cleanList (void)
{
  picker_state = XNONE;
...
void
US_Norm_Profile::load_data_auto (QMap<QString, QString> &protocol_details)
{
...
void
US_Norm_Profile::load_data_auto_report (
    QMap<QString, QString> &protocol_details)
{
  ...
  qDebug () << "ABDE analysis PARMS: -- " << abde_analysis_parms["ID"]
  ...
  parse_abde_analysis_jsons (abde_analysis_parms["xnorms_percents"],
                             protocol_details, data_per_channel_xnorm,
                             data_per_channel_norm_cb,
                             data_per_channel_ranges_percents,
                             data_per_channel_rmsd, data_per_channel_meniscus);
...
void
US_Norm_Profile::slt_loadAUC_auto_report (
    QMap<QString, QString> &protocol_details)
{
  channList.clear ();
  ...
  QString workingDir;
  US_LoadAUC *dialog
      = new US_LoadAUC (false, protocol_details, allData, triples, workingDir);
  ...
  for (int i = 0; i < triples.size (); i++)
    {
      ...
      QString fn = tr ("%1.%2.%3.%4.%5")
                       .arg (runid, dataType, ccw.at (0).trimmed (),
...
void
US_Norm_Profile::slt_loadAUC_auto (QMap<QString, QString> &protocol_details)
{
  channList.clear ();
  QString dirName;
  if (abde_etype == "MWL")
  ...
  QStringList fileList
      = runDir.entryList (QStringList () << "*.auc", QDir::Files, QDir::Name);
  QStringList fPath;
          ...
          for (int j = 0; j < channels_rmsds_l.size (); ++j)
            {
              ...
              if (c_chan_rmsd_channel == channList[i])
                data_per_channel_rmsd[channList[i]]
                    = c_chan_rmsd_rmsd.toDouble ();
            }
        }

      // and also, fill meniscus info per channel
      QStringList channels_menisc_l
          = protocol_details["meniscus_info"].split (",");
...
QString
US_Norm_Profile::select_channel_public (int index)
{
  qDebug () << "[in select_channel_public()]: channame, index, abde_etype, "
               "us_auto_mode -- "
            << cb_chann->itemText (index) << index << abde_etype
            << us_auto_mode;
...
void
US_Norm_Profile::new_chann_auto (int index)
{
  slt_cleanList ();
...
bool
US_Norm_Profile::areAllProcessed_auto (void)
{
...
bool
US_Norm_Profile::areAllNormalized_auto (QString &msg)
{
  msg.clear ();
  bool all_processed = true;
  for (int i = 0; i < channList.size (); ++i)
    {
      if (data_per_channel_xnorm[channList[i]] == -1)
        {
          msg += "WARNING: Distributions for some channel(s) are not "
                 "normalized!";
          msg += "\n\nThis will result in the non-normalized distributions in "
                 "the GMP Report...";
          all_processed = false;
          break;
        }
...
void
US_Norm_Profile::next_chann_auto (void)
{
...
void
US_Norm_Profile::prev_chann_auto (void)
{
  int row = cb_chann->currentIndex () - 1;
  if (row >= 0)
    cb_chann->setCurrentIndex (row);
...
US_Norm_Profile::slt_loadAUC ()
{
  ...
  if (!isLocal)
      ...
      for (int i = 0; i < triples.size (); i++)
        {
          QStringList ccw = triples.at (i).split (u '/');
          US_DataIO::RawData rawData = allData.at (i);
          QString fn = tr ("%1.%2.%3.%4")
                           .arg (runid, ccw.at (0).trimmed (),
                                 ccw.at (1).trimmed (), ccw.at (2).trimmed ());
          QString fp = tr ("%1.%2.auc").arg (dirname, fn);
          filenames << fn;
          filePaths << fp;
    ...
    {
      QStringList fPath = QFileDialog::getOpenFileNames (
          this, tr ("Open AUC File"), US_Settings::importDir (),
          tr (".auc (*.auc)"));
      if (fPath.size () == 0)
        return;
...
US_Norm_Profile::trapz (QVector<double> xval, QVector<double> yval)
{
  ...
  if (us_auto_mode)
    {
      ...
      else if (flag_pnorm && data_per_channel_xnorm[channame] > x[0]
               && data_per_channel_xnorm[channame] < x[np - 1])
        {
...
QPair<int, int>
US_Norm_Profile::getXlimit (QVector<double> xval_in, double xmin, double xmax)
{
...
void
US_Norm_Profile::selectData (void)
{
  xvalues_sel.clear ();
  yvalues_sel.clear ();
  yvaluesN_sel.clear ();
  ...
  for (int i = 0; i < inpIds.size (); i++)
    {
      ...
      if (flag_limit)
        {
          QPair<int, int> pair
              = getXlimit (xvalues.at (id), x_min_picked, x_max_picked);
...
void
US_Norm_Profile::selectData_auto (void)
{
  xvalues_sel.clear ();
  yvalues_sel.clear ();
  yvaluesN_sel.clear ();
      ...
      if (abde_etype == "MWL")
      ...
      else // SWL
        {
          ...
          for (int bl = 0; bl < chann_blc_list.size (); ++bl)
            {
              ...
              if (triple_name.contains (channame))
                {
                  QStringList edit_parms
                      = chanblcinfo.split (":")[1].split (",");
          ...
          for (int yv = 0; yv < yvalues.at (id).size (); ++yv)
            yval_corrected.push_back (
                yvalues.at (id)[yv]
                - (xvalues.at (id)[yv] * bl_slope + bl_interp));
...
US_Norm_Profile::plotData (void)
{
  ...
  penstyles_list << Qt::DashLine << Qt::DotLine << Qt::DashDotLine
                 << Qt::DashDotDotLine;

  // symbols
  QVector<QwtSymbol *> line_symbols;
  QwtSymbol *s_cross = new QwtSymbol (QwtSymbol::Cross, QBrush (Qt::black),
                                      QPen (Qt::black), QSize (4, 4));
  QwtSymbol *s_xcross = new QwtSymbol (QwtSymbol::XCross, QBrush (Qt::black),
                                       QPen (Qt::black), QSize (4, 4));
  QwtSymbol *s_hline = new QwtSymbol (QwtSymbol::HLine, QBrush (Qt::black),
                                      QPen (Qt::black), QSize (4, 4));
  QwtSymbol *s_utriangle
      = new QwtSymbol (QwtSymbol::UTriangle, QBrush (Qt::black),
                       QPen (Qt::black), QSize (4, 4));
  QwtSymbol *s_dtriangle
      = new QwtSymbol (QwtSymbol::DTriangle, QBrush (Qt::black),
                       QPen (Qt::black), QSize (4, 4));
              ...
              if (abde_etype == "MWL")
                {
                  if (selFilenames.at (i).contains ("_002")
                      ...
                      || selFilenames.at (i).contains ("protein",
                                                       Qt::CaseInsensitive)
                      || selFilenames.at (i).contains ("prot",
                                                       Qt::CaseInsensitive))
                    {
                      ...
                      pen.setColor ("magenta");
                           ...
                           || selFilenames.at (i).contains (
                               "dna", Qt::CaseInsensitive))
                    {
              ...
              if (abde_etype == "MWL")
                {
                  if (selFilenames.at (i).contains ("_002")
                                                       ...
                                                       Qt::CaseInsensitive))
                    {
                      ...
                      xp_intN_protein
                          = data_per_channel[channame]["midxval"][i];
                      yp_intN_protein
                          = data_per_channel[channame]["integralN"][i];
                      // qDebug() << "yp_intN_protein, size() " <<
                      // yp_intN_protein << ", size: " <<
                      // yp_intN_protein.size();
                    }
                  else if (selFilenames.at (i).contains ("_001")
                           || selFilenames.at (i).contains (".001")
      ...
      for (int i = 0; i < chann_ranges_l.size (); ++i)
        {
          ...
          if (chann_ranges_c_l[0] == channame)
            {
              qDebug () << "channel " << channame
                        << ", has range(s): " << chann_ranges_c_l[1];
      ...
      for (int i = 0; i < c_ranges.size (); ++i)
        {
          ...
          // yp_intN_protein = data_per_channel[ channame
          // ]["integralN"].at(i).data();
          if (!us_auto_mode_report)
            find_percent_from_range (channame, point1_s, point2_s,
                                     xp_intN_protein, yp_intN_protein);
...
#else
          QwtScaleDiv *y_axis
              = (QwtScaleDiv *)&(plot->axisScaleDiv (QwtPlot::yLeft));
#endif

          double padding
              = (y_axis->upperBound () - y_axis->lowerBound ()) / 30.0;

          qDebug () << "In Plotting ranges: y_axis->upperBound(), "
                       "y_axis->lowerBound(), padding --- "
                    << y_axis->upperBound () << y_axis->lowerBound ()
                    << padding;
          qDebug () << "In Plotting ranges: maxY, minY ---  " << maxY << minY;
          ...
          // QPen pen1 = QPen( QBrush( color_list[ color_index ] ), 2.0,
          // Qt::DotLine ); pen1.setColor("black"); int style_num = 0; if ( i <
          // penstyles_list.size() )
          //   style_num = i;
...
void
US_Norm_Profile::find_percent_from_range (QString channame, QString point1_s,
                                          QString point2_s,
                                          QVector<double> xp_intN_protein,
                                          QVector<double> yp_intN_protein)
{
  ...
  qDebug () << "p1, p2 -- " << p1 << p2;
  // qDebug() << "[in find_percent_from_range() ] xp_intN_protein, size() " <<
  // xp_intN_protein << ", size: " << xp_intN_protein.size();
  ...
  data_per_channel_ranges_percents[channame][range]
...
void
US_Norm_Profile::slt_legend (int state)
{
...
void
US_Norm_Profile::slt_ranges (int)
{
  plotData ();
}

void
US_Norm_Profile::slt_rawData (int)
{
  plotData ();
}

void
US_Norm_Profile::slt_integral (int)
{
  plotData ();
}

void
US_Norm_Profile::slt_norm (int)
{
...
void
US_Norm_Profile::slt_pickRange ()
{
  x_min_picked = -1;
  x_max_picked = -1;
  if (selFilenames.size () == 0)
    return;
  if (picker_state == XNORM)
    return;
...
void
US_Norm_Profile::slt_mouse (const QwtDoublePoint &point)
{
  if (selFilenames.size () == 0)
    return;
  if (picker_state == XNONE)
    return;
  QColor color = US_GuiSettings::pushbColor ().color (QPalette::Active,
                                                      QPalette::Button);
  QString bkgc
      = tr ("QPushButton { background-color: %1 }").arg (color.name ());
...
void
US_Norm_Profile::enableWidgets (bool state)
{
...
US_Norm_Profile::slt_save ()
{
  ...
  QString fname = QFileDialog::getSaveFileName (
      this, tr ("Save File"), US_Settings::reportDir (), tr ("(*.csv)"));
  if (fname.size () == 0)
    return;
          ...
          for (int i = 0; i < nd; i++)
            {
              ...
              if (line < np)
                { // X_scan
                  outStream
                      << QString::number (xvalues_sel.at (i).at (line), 'f', 4)
                      << ",";
                  newLine = true;
                }
              else
              ...
              if (line < np)
                { // Scan
                  outStream
                      << QString::number (yvalues_sel.at (i).at (line), 'f', 6)
                      << ",";
                  newLine = true;
                }
              else
              ...
              if (line < np)
                { // Scan_norm
                  outStream << QString::number (yvaluesN_sel.at (i).at (line),
                                                'f', 6)
                            << ",";
                  newLine = true;
                }
              ...
              if (line < np - 1)
                { // X_integral
                  outStream
                      << QString::number (midxval_sel.at (i).at (line), 'f', 4)
                      << ",";
                  newLine = true;
                }
              else
              ...
              if (line < np - 1)
                { // Integral
                  outStream << QString::number (integral_sel.at (i).at (line),
                                                'f', 6)
                            << ",";
                  newLine = true;
                }
              ...
              if (line < np - 1)
                { // Integral_norm
                  outStream << QString::number (integralN_sel.at (i).at (line),
                                                'f', 6);
                  newLine = true;
                }
...
void
US_Norm_Profile::closeEvent (QCloseEvent *event)
{
  emit widgetClosed ();
  event->accept ();
}
...
void
US_Norm_Profile::slt_inItemSel (int row)
{
  if (row < 0)
    {
      return;
...
void
US_Norm_Profile::slt_outItemSel (int row)
{
  if (row < 0)
    {
      return;
...
void
US_Norm_Profile::slt_norm_by_max (int state)
{
  if (us_auto_mode)
    {
      QString channame = cb_chann->currentText ();
...
void
US_Norm_Profile::save_auto (void)
{
  QString msg_u;
  if (!areAllNormalized_auto (msg_u) && abde_etype == "MWL")
    {
      ...
      int status
          = QMessageBox::warning (this, tr ("Channel Normalization"), msg_u,
                                  tr ("&Proceed"), tr ("&Cancel"), 0, 0, 1);
  ...
  ***/
  ...
  for (int i = 0; i < channList.size (); ++i)
    {
      ...
      qDebug () << "x_norm point for channel " << channame << ", " << x_normal;
      // x_normalization
      json_p += "\"" + channame + "\":{\"x_norm\":\"" + x_normal + "\",";

      // meniscus
      json_p += "\"meniscus\":\""
                + QString::number (data_per_channel_meniscus[channame])
                + "\",";

      // add rmsds
      if (abde_etype == "MWL")
        json_p += "\"rmsd\":\""
                  + QString::number (data_per_channel_rmsd[channame]) + "\",";

      // now over ranges:percents
      QMap<QString, double> ranges_percents
          = data_per_channel_ranges_percents[channame];
      json_p += "\"percents\":{";
  ...
  US_Passwd pw_at;
  QMap<QString, QString> gmp_submitter_map = pw_at.getPasswd_auditTrail (
      "GMP Run ABDE Form",
      "Please fill out GMP run ABDE-Analysis form:", user_submitter);
  ...
  if (gmp_submitter_map_size == 0 || gmp_submitter_map.keys ().isEmpty ())
  ...
  else
    {
      ...
      status_analysis_abde_unique
          = read_autoflowAnalyisABDEstages_record (prot_details["autoflowID"]);

      qDebug () << "status_analysis_abde_unique -- "
                << status_analysis_abde_unique;
      ...
      if (!status_analysis_abde_unique)
        {
          ...
          QMessageBox::information (
              ...
              tr ("The program advanced or is advancing to the next stage!\n\n"
                  ...
                  "has already saved ABDE analysis profiles into DB using "
                  "different program "
  ...
  qry1.clear ();
  qry1 << "update_autoflowAnalysisABDE_record" << json_p << filename_blc_p
  ...
  // Also, update autoflowStatus's 'analysisABDE' && 'analysisABDEts' (new
  // fields) with info from gmp_submitter_map:
  ...
  // Finally, switch to 6. REPORT -- need to communicate with parent
  // autoflow_Analysis....
...
void
US_Norm_Profile::record_AnalysisABDE_status (QMap<QString, QString> gmp_form)
{
  // Check DB connection
  US_Passwd pw;
  ...
  if (autoflowStatusID)
    {
      // update
      qry << "update_autoflowStatusAnalysisABDE_record" // to implement: add
                                                        // 'analysisABDE' &&
                                                        // 'analysisABDEts' to
                                                        // autoflowStatus!!!!!
          << QString::number (autoflowStatusID) << prot_details["autoflowID"]
          << AnalysisABDE_Json;
  ...
  else
    {
      QMessageBox::warning (
          ...
          tr ("autoflowStatus (analysisABDE): There was a problem with "
              "identifying "
...
int
US_Norm_Profile::read_autoflowAnalyisABDEstages_record (QString autoflowID)
{
  int status = 0;
  ...
  // qDebug() << "BEFORE query ";
  QStringList qry;
  qry << "autoflow_abde_analysis_status" << autoflowID;
...
void
US_Norm_Profile::revert_autoflowAnalysisABDEstages_record (QString autoflowID)
{
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd ();
  ...
  QStringList qry;
  qry << "autoflow_abde_analysis_status_revert" << autoflowID;
...
void
US_Norm_Profile::update_autoflow_record_atAnalysisABDE (void)
{
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd ();
  ...
  QStringList qry;
  qry << "update_autoflow_at_analysis" << prot_details["autoflowID"];
...
US_Norm_Profile::read_autoflowAnalysisABDE_record (QString aID)
{
  QMap<QString, QString> abde_analysis_parms;
  ...
  QStringList qry;
  qry << "read_autoflowAnalysisABDE_record" << aID;
  db->query (qry);
...
void
US_Norm_Profile::parse_abde_analysis_jsons (
    QString abde_analysis_parms_string,
    QMap<QString, QString> &protocol_details,
    ...
    QMap<QString, double> &data_chann_menisc)
{
  ...
  if (!abde_analysis_parms_string.isEmpty ())
    {
      QJsonDocument jsonDoc
          = QJsonDocument::fromJson (abde_analysis_parms_string.toUtf8 ());
              ...
              foreach (const QString &key_1, json_obj_1.keys ())
                {
                  ...
                  qDebug () << "XNORMS-1, PERCENTS-1 key-1, value-1: " << key_1
                            << value_1;
                  if (key_1 == "x_norm")
                    {
                      double x_norm_val = value_1.toString ().toDouble ();
                      data_chann_x_norm[key] = x_norm_val;
                      data_chann_x_norm_cb[key]
                          = (x_norm_val != -1) ? Qt::Unchecked : Qt::Checked;
                    }
  ...
  if (!channels_to_radial_ranges.isEmpty ())
    {
      qDebug () << "[PARSE], channels_to_radial_ranges -- "
                << channels_to_radial_ranges;
      protocol_details["channels_to_radial_ranges"]
          = channels_to_radial_ranges;
    }
