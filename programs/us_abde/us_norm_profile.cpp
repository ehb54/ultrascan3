...
US_Norm_Profile::US_Norm_Profile(QString auto_mode) : US_Widgets() {
  ...
  usplot = new US_Plot(plot, tr(""), tr("Radius (in cm)"), tr("Absorbance"),
                       true, "", "");
  ...
  // "filename" ]     = QString("McCue_AAVCsCl_25JAN25-run2255-dataDiskRun-72");
  // protocol_details[ "analysisIDs"  ] = QString("");
  // protocol_details[ "expType" ]      = QString("ABDE");
  // protocol_details[ "dataSource" ]   = QString("dataDiskAUC");
...
US_Norm_Profile::US_Norm_Profile() : US_Widgets() {
  ...
  ckb_norm_max = new QCheckBox();
  QGridLayout* us_norm_max = us_checkbox("Normalize by Maximum", ckb_norm_max);
  pb_pick_norm = us_pushbutton("Pick a Point");
...
void US_Norm_Profile::load_data_auto_report(
    QMap<QString, QString>& protocol_details) {
...
QwtPlot* US_Norm_Profile::rp_data_plot() { return plot; }

// For use in GMP REPORTing
void US_Norm_Profile::slt_loadAUC_auto_report(
    QMap<QString, QString>& protocol_details) {
  channList.clear();
...
void US_Norm_Profile::slt_loadAUC_auto(
    QMap<QString, QString>& protocol_details) {
  channList.clear();
  QString dirName;
  if (abde_etype == "MWL")
  ...
  for (int i = 0; i < fPath.size(); i++) {
    if (filePaths.contains(fPath.at(i))) continue;
...
QString US_Norm_Profile::select_channel_public(int index) {
  qDebug() << "[in select_channel_public()]: channame, index, abde_etype, "
              "us_auto_mode -- "
           << cb_chann->itemText(index) << index << abde_etype << us_auto_mode;
...
bool US_Norm_Profile::areAllNormalized_auto(QString& msg) {
  ...
  for (int i = 0; i < channList.size(); ++i) {
    if (data_per_channel_xnorm[channList[i]] == -1) {
      ...
      msg +=
          "\n\nThis will result in the non-normalized distributions in the GMP "
          "Report...";
      all_processed = false;
      break;
    }
...
void US_Norm_Profile::next_chann_auto(void) {
  ...
  if ((row + 1) <= cb_chann->count()) cb_chann->setCurrentIndex(row);
}

//[AUTO] prev channel button
void US_Norm_Profile::prev_chann_auto(void) {
  int row = cb_chann->currentIndex() - 1;
  if (row >= 0) cb_chann->setCurrentIndex(row);
}
  ...
  if (!isLocal) {
    ...
    for (int i = 0; i < triples.size(); i++) {
      ...
      QString fn = tr("%1.%2.%3.%4")
                       .arg(runid, ccw.at(0).trimmed(), ccw.at(1).trimmed(),
                            ccw.at(2).trimmed());
      QString fp = tr("%1.%2.auc").arg(dirname, fn);
      filenames << fn;
      filePaths << fp;
...
QMap<QString, QVector<double>> US_Norm_Profile::trapz(QVector<double> xval,
                                                      QVector<double> yval) {
...
QPair<int, int> US_Norm_Profile::getXlimit(QVector<double> xval_in, double xmin,
                                           double xmax) {
  ...
  for (int i = 0; i < inpIds.size(); i++) {
    ...
    } else  // SWL
    {
      ...
      for (int yv = 0; yv < yvalues.at(id).size(); ++yv)
        yval_corrected.push_back(yvalues.at(id)[yv] -
...
void US_Norm_Profile::plotData(void) {
  ...
  QwtSymbol* s_hline = new QwtSymbol(QwtSymbol::HLine, QBrush(Qt::black),
                                     QPen(Qt::black), QSize(4, 4));
  QwtSymbol* s_utriangle = new QwtSymbol(
      QwtSymbol::UTriangle, QBrush(Qt::black), QPen(Qt::black), QSize(4, 4));
  QwtSymbol* s_dtriangle = new QwtSymbol(
      QwtSymbol::DTriangle, QBrush(Qt::black), QPen(Qt::black), QSize(4, 4));
  ...
  if (ckb_integral->isChecked()) {
    ...
    for (int i = 0; i < nd; i++) {
      ...
      if (us_auto_mode) {
        if (abde_etype == "MWL") {
          if (selFilenames.at(i).contains("_002") ||
            ...
            // qDebug() << "yp_intN_protein, size() " << yp_intN_protein << ",
            // size: " << yp_intN_protein.size();
          } else if (selFilenames.at(i).contains("_001") ||
                     selFilenames.at(i).contains(".001") ||
                     selFilenames.at(i).endsWith(".1") ||
                     selFilenames.at(i).contains("dna", Qt::CaseInsensitive)) {
      ...
      if (us_auto_mode)
        ...
        if (ckb_rawData->isChecked()) curve->setYAxis(QwtPlot::yRight);
      }
  ...
  if (us_auto_mode && ckb_ranges->isChecked()) {
    ...
    for (int i = 0; i < c_ranges.size(); ++i) {
      ...
      if (!us_auto_mode_report)
        find_percent_from_range(channame, point1_s, point2_s, xp_intN_protein,
                                yp_intN_protein);
      ...
      // ); pen1.setColor("black"); int style_num = 0; if ( i <
      // penstyles_list.size() )
      //   style_num = i;
      ...
      int symbol_number = 0;
      if (i < line_symbols.size()) symbol_number = i;
...
void US_Norm_Profile::find_percent_from_range(QString channame,
                                              QString point1_s,
                                              QString point2_s,
  ...
  for (int i = 0; i < xp_intN_protein.size(); ++i) {
    if (xp_intN_protein[i] <= p1) p1_ind = i;
  }

  for (int i = 0; i < xp_intN_protein.size(); ++i) {
    if (xp_intN_protein[i] <= p2) p2_ind = i;
  }
...
void US_Norm_Profile::slt_ranges(int) { plotData(); }

void US_Norm_Profile::slt_rawData(int) { plotData(); }

void US_Norm_Profile::slt_integral(int) { plotData(); }
...
void US_Norm_Profile::save_auto(void) {
  ...
  if (!areAllNormalized_auto(msg_u) && abde_etype == "MWL") {
    ...
    int status = QMessageBox::warning(this, tr("Channel Normalization"), msg_u,
  ...
  ***/
  ...
  if (!filename_p_list.isEmpty()) filename_p = filename_p_list.takeLast();
    ...
    if (!status_analysis_abde_unique) {
      QMessageBox::information(
          ...
          tr("The program advanced or is advancing to the next stage!\n\n"
             ...
             "has already saved ABDE analysis profiles into DB using different "
             "program "
  ...
  if (status == US_DB2::NO_AUTOFLOW_RECORD) {
    QMessageBox::warning(this, tr("AutoflowAnalysisABDE Record Not Updated"),
...
void US_Norm_Profile::record_AnalysisABDE_status(
    QMap<QString, QString> gmp_form) {
  // Check DB connection
  US_Passwd pw;
...
void US_Norm_Profile::revert_autoflowAnalysisABDEstages_record(
    QString autoflowID) {
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd();
...
QMap<QString, QString> US_Norm_Profile::read_autoflowAnalysisABDE_record(
    QString aID) {
  QMap<QString, QString> abde_analysis_parms;
...
void US_Norm_Profile::parse_abde_analysis_jsons(
