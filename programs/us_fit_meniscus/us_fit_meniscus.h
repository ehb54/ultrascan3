...
//! \brief Contains the declaration of the US_FitMeniscus class and its
//! members.
...
#define DbgLv(a)                                                              \
  if (dbg_level >= a)                                                         \
  qDebug () //!< dbg_level-conditioned qDebug()
...
private slots:
  ...
  read_autoflowAnalysis_record (const QString &record_id);
  ...
  void noises_in_edit (QString editID, QStringList &modelIDs,
                       QStringList &noiseIDs, QStringList &noiseDescr);
