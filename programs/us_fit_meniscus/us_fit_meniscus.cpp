    ...
    : US_Widgets ()
{
  ...
  connect (ct_zfloor, SIGNAL (valueChanged (double)), this,
           SLOT (plot_data ()));
  ...
  connect (sb_order, SIGNAL (valueChanged (int)), this,
           SLOT (plot_data (int)));
...
void
US_FitMeniscus::mouse (const QwtDoublePoint &p)
{
  double radius_min[2];
  double rmsd_min[2];
...
US_FitMeniscus::plot_3d (void)
{
  ...
  my_bott[0] = b_bott;
  my_bott[1] = b_bott;
  QwtPlotCurve *mmeni_curve
      = us_curve (meniscus_plot, tr ("Low-RMSD Meniscus"));
...
int
US_FitMeniscus::read_autoflowAnalysisStages (const QString &requestID)
{
  int status = 0;
...
US_FitMeniscus::read_autoflowAnalysis_record (const QString &requestID)
{
...
void
US_FitMeniscus::edit_update (void)
{
  /***/
  ...
  autoflowAnalysisHistory
        ...
        QMessageBox::information( this,
                                  tr( "FITMEN | Triple Analysis already
  processed" ), tr( "It appears that FITMEN stage has already been processed by
  " "a different user from different session and " "the entire analysis for the
  current triple is completed. \n\n" "The program will return to the autoflow
  runs dialog where " "you can re-attach to the actual current stage of the
  run. "));
  ...
  ANALYSIS tab where " "you can continue to monitor the overall analysis
  progress." ) );
        ...
        else
          {
            ...
            QMessageBox::information( this,
                                      ...
                                      tr( "It appears that FITMEN stage has
  already been processed by " "a different user from different session.\n\n"
                                          "The program will return to ANALYSIS
  tab where " "you can continue to monitor the overall analysis progress." ) );
...
#define BOTT_LOWVAL 7.0
  ...
  if (bad_vals)
    {
      if (!auto_mode)
        {
          int response
              = QMessageBox::critical ((QWidget *)this, mhdr, mmsg,
                                       QMessageBox::Save, QMessageBox::Cancel);

          if (response == QMessageBox::Cancel)
            {
              QMessageBox::information (
                  (QWidget *)this, tr ("Canceled"),
  ...
  if (!all_wvl || nedtfs == 1)
      ...
      if (mennew >= lefval)
        {
          if (!auto_mode)
            ...
            {
              bad_men_vals = true;
              // QString reason_for_failure = QString( "The selected Meniscus
              // value, %1 , extends into the data range whose left-side value
              // is %2") 	.arg( mennew ) 	.arg( lefval ); triple_information[
              // "failed" ] = reason_for_failure; emit bad_meniscus_values(
              // triple_information ); close();
    ...
    { // Apply to all wavelengths in a cell/channel
      ...
      if (auto_mode)
        {
          progress_msg = new QProgressDialog ("Updating edit profiles...",
              ...
              if (!auto_mode)
                {
                  QMessageBox::warning (this,
                                        tr ("Meniscus within Data Range"),
                                        tr ("The selected Meniscus value, %1 "
                                            ", extends into the data"
                                            " range whose left-side value is "
                                            "%2 . This Edit update"
                                            " cannot be performed!")
                                            .arg (mennew)
                                            .arg (lefval));
                ...
                {
                  bad_men_vals = true;
                  // QString reason_for_failure = QString( "The selected
                  // Meniscus value, %1 , extends into the data range whose
                  // left-side value is %2")
                  //   .arg( mennew )
                  //   .arg( lefval );
                  // triple_information[ "failed" ] = reason_for_failure;
                  // emit bad_meniscus_values( triple_information );
  ...
  if (auto_mode)
    {
      ...
      // Identify if Meniscus || Bottom || (Meniscus && Bottom) have been
      // changed
      if (have3val)
        { // Fit is meniscus + bottom
        }
...
void
US_FitMeniscus::revert_autoflow_analysis_stages_record (
...
void
US_FitMeniscus::file_loaded (QString fn)
{
  DbgLv (1) << "FL: IN:  fn" << fn;
...
void
US_FitMeniscus::get_editProfile_copy (
...
// Slot for handling a loaded file:  set the name of loaded,edit files
bool
US_FitMeniscus::file_loaded_auto (QMap<QString, QString> &triple_information)
{
...
// Scan the database for models to use to write local fit table files
void
US_FitMeniscus::scan_dbase_auto (QMap<QString, QString> &triple_information)
{
  QVector<ModelDesc> mDescrs;  // Vector of model description objects
  US_Passwd pw;                // DB password
  US_DB2 db (pw.getPasswd ()); // DB control
...
void
US_FitMeniscus::update_disk_db (bool isDB)
{
...
void
US_FitMeniscus::update_db_edit (QString edtext, QString efilepath,
                                QString &msg)
{
      ...
      if (ck_confirm->isChecked ())
        {
          ...
          int response = QMessageBox::question (
              this, tr ("Remove Models and Noises?"), msg, QMessageBox::Yes,
              QMessageBox::Cancel);
          rmv_mdls = (response == QMessageBox::Yes);
        }
...
void
US_FitMeniscus::noises_in_edit (QString modDesc, QStringList &nieDescs,
...
QwtLinearColorMap *
US_FitMeniscus::ColorMapCopy (QwtLinearColorMap *colormap)
{
...
void
US_FitMeniscus::change_plot_type ()
{
  dataPlotClear (meniscus_plot);
  ...
  if (have3val)
    ...
    { // Set up 2-D plot
      ...
      meniscus_plot->setAxisTitle (QwtPlot::xBottom, tr ("Meniscus Radius"));
      meniscus_plot->enableAxis (QwtPlot::yRight, false);
      meniscus_plot->setCanvasBackground (
          QBrush (US_GuiSettings::plotCanvasBG ()));
    }
...
void
US_FitMeniscus::index_model_setfit ()
{
  if (have3val)
...
void
US_FitMeniscus::sel_investigator (void)
{
  if (!dkdb_cntrls->db ())
...
void
US_FitMeniscus::assign_investigator (int invID)
{
