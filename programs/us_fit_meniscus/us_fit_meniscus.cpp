...
US_FitMeniscus::US_FitMeniscus(QMap<QString, QString> triple_info_map)
    : US_Widgets() {
  this->triple_information = triple_info_map;
  qDebug() << "triple_information: filename, triple_name: "
  ...
  te_data->setMinimumWidth(
      fm.width("123 :  6.34567, 7.34567, 0.00567890  0.012345"));
  ...
  QBoxLayout* plot = new US_Plot(meniscus_plot, tr("Meniscus,Bottom Fit"),
  ...
  le_status->setToolTip(tr("Results of the last action performed"));
  ...
  connect(ct_zfloor, SIGNAL(valueChanged(double)), this, SLOT(plot_data()));
  ...
  le_men_lor->setToolTip(tr("Selected-minimum meniscus radius value"));
  le_bot_lor = us_lineedit("", -1, true);
  le_bot_lor->setToolTip(tr("Selected-minimum bottom radius value"));
  le_men_fit = us_lineedit("", -1, false);
  le_men_fit->setToolTip(tr("Fit/Editable meniscus radius value"));
  le_bot_fit = us_lineedit("", -1, false);
  le_bot_fit->setToolTip(tr("Fit/Editable bottom radius value"));
  ...
  if (auto_mode) {
    ...
    le_men_sel->setToolTip(tr("Selected meniscus radius value"));
  } else {
    le_men_sel = us_lineedit("", -1, false);
    le_men_sel->setToolTip(tr("Selected/Editable meniscus radius value"));
  }

  le_rms_error = us_lineedit("", -1, true);
  le_rms_error->setToolTip(tr("RMS error of curve to meniscus,rmsd points"));

  connect(sb_order, SIGNAL(valueChanged(int)), this, SLOT(plot_data(int)));
  ...
  connect(pb_update, SIGNAL(clicked()), this, SLOT(edit_update()));
  ...
  connect(pb_scandb, SIGNAL(clicked()), this, SLOT(scan_dbase()));
  ...
  connect(pb_invest, SIGNAL(clicked()), this, SLOT(sel_investigator()));
  QString inv_name = QString::number(US_Settings::us_inv_ID()) + ": " +
  ...
  ck_confirm->setToolTip(tr("Pop up confirmation dialogs at each update step"));
  ...
  connect(pb_plot, SIGNAL(clicked()), this, SLOT(plot_data()));
  pb_plot->setToolTip(tr("Plot,analyze meniscus,rmsd from current text"));

  pb_reset = us_pushbutton(tr("Reset"));
  connect(pb_reset, SIGNAL(clicked()), this, SLOT(reset()));
  pb_reset->setToolTip(tr("Clear text,plot and various other controls"));

  QPushButton* pb_help = us_pushbutton(tr("Help"));
  connect(pb_help, SIGNAL(clicked()), this, SLOT(help()));
  pb_help->setToolTip(tr("Open a dialog with detailed documentation"));

  QPushButton* pb_accept = us_pushbutton(tr("Close"));
  connect(pb_accept, SIGNAL(clicked()), this, SLOT(close()));
  pb_accept->setToolTip(tr("Close this dialog and exit the program"));
  ...
  for (int ii = 0; ii < 16; ii++) cntrlsLayout->setColumnStretch(ii, 1);
...
// Plot the data
void US_FitMeniscus::plot_data(int) { plot_data(); }

// Plot the data
void US_FitMeniscus::plot_data(void) {
  if (v_meni.count() < 1 && te_data->e->toPlainText().length() > 20)
    load_data();
...
void US_FitMeniscus::load_data() {
  ...
  if ((have3val && !was3val) || (!have3val && was3val)) change_plot_type();
}
...
void US_FitMeniscus::plot_3d(void) {
  ...
  meniscus_plot->axisTitle(QwtPlot::yRight)
      .setFont(meniscus_plot->axisTitle(QwtPlot::yLeft).font());
  meniscus_plot->setAxisScale(QwtPlot::yRight, spec_dat->range().minValue(),
                              spec_dat->range().maxValue());
...
void US_FitMeniscus::plot_2d(void) {
  ...
  meniscus_plot->setAxisScale(QwtPlot::xBottom, minx - overscan,
                              maxx + overscan);
  ...
  if (order == 2) {
    ...
    // c1 + 2 * c2 * x = 0
    fit_xvl = -c[1] / (2.0 * c[2]);
    qDebug() << "2nd order...: c[1], c[2], fit_xvl -- " << c[1] << c[2]
             << fit_xvl;
  } else {
  ...
  QwtSymbol* sym = new QwtSymbol(QwtSymbol::Cross, QBrush(Qt::white), markerPen,
                                 QSize(9, 9));
...
QMap<QString, QString> US_FitMeniscus::read_autoflowAnalysis_record(
...
), tr( "It appears that FITMEN stage has already been processed by " "a
different user from different session and " "the entire analysis for the current
triple is completed. \n\n" "The program will return to the autoflow runs dialog
where " "you can re-attach to the actual current stage of the run. "));
          ...
          if ( status_gen != "WAIT" )
            {
              QMessageBox::information( this,
                                        ...
                                        tr( "It appears that FITMEN stage has
already been processed by " "a different user from different session.\n\n" "The
program will return to ANALYSIS tab where " "you can continue to monitor the
...
*/
  ...
  //-- However, it's not complete: upon updating edit profile below, the row
  //needs to be locked while updating --//
...
#define BOTT_LOWVAL 7.0
  ...
  if (bad_vals) {
    if (!auto_mode) {
      int response = QMessageBox::critical(
          (QWidget*)this, mhdr, mmsg, QMessageBox::Save, QMessageBox::Cancel);

      if (response == QMessageBox::Cancel) {
        QMessageBox::information((QWidget*)this, tr("Canceled"),
                                 tr("\"Update Edit\" has been canceled!"));

        return;
      } else {
        QMessageBox::information((QWidget*)this, tr("Saving"),
  ...
  if (!all_wvl || nedtfs == 1) {  // Apply to a single triple
    ...
    if (mennew >= lefval) {
      if (!auto_mode) {
        QMessageBox::warning(
            this, tr("Meniscus within Data Range"),
      ...
      {
        bad_men_vals = true;
        // QString reason_for_failure = QString( "The selected Meniscus value,
        // %1 , extends into the data range whose left-side value is %2") 	.arg(
        // mennew ) 	.arg( lefval ); triple_information[ "failed" ] =
        // reason_for_failure; emit bad_meniscus_values( triple_information );
        // close();
    ...
    if (auto_mode) le_status->setText("Updating edit profile: " + fname_edit);
    ...
    if (confirm) {  // Confirm at each update step
      mmsg +=
      ...
      int response =
          QMessageBox::question(this, tr("Edit File Updated"), mmsg,
    ...
    // progress_msg = NULL;
    if (auto_mode) {
      progress_msg = new QProgressDialog("Updating edit profiles...", QString(),
                                         0, nedtfs, this);
    ...
    for (int jj = 0; jj < nedtfs;
      ...
      if (mennew >= lefval) {
        ...
        } else  // GMP
        {
          bad_men_vals = true;
          // QString reason_for_failure = QString( "The selected Meniscus value,
          // %1 , extends into the data range whose left-side value is %2")
          //   .arg( mennew )
          //   .arg( lefval );
          // triple_information[ "failed" ] = reason_for_failure;
          // emit bad_meniscus_values( triple_information );
...
void US_FitMeniscus::revert_autoflow_analysis_stages_record(
    const QString& requestID) {
  // Check DB connection
  US_Passwd pw;
...
void US_FitMeniscus::get_editProfile_copy(
...
// Slot for handling a loaded file:  set the name of loaded,edit files
bool US_FitMeniscus::file_loaded_auto(
    QMap<QString, QString>& triple_information) {
...
void US_FitMeniscus::scan_dbase() {
  ...
  le_status->setText(tr("Scanning DB fit-meniscus models ..."));
...
// Scan the database for models to use to write local fit table files
void US_FitMeniscus::scan_dbase_auto(
    QMap<QString, QString>& triple_information) {
  QVector<ModelDesc> mDescrs;  // Vector of model description objects
  US_Passwd pw;                // DB password
  US_DB2 db(pw.getPasswd());   // DB control
...
void US_FitMeniscus::update_disk_db(bool isDB) { pb_scandb->setEnabled(isDB); }
...
void US_FitMeniscus::update_db_edit(QString edtext, QString efilepath,
  ...
  DbgLv(1) << "updDbEd: nedtfs" << nedtfs << "applymwl"
           << ck_applymwl->isChecked();
...
void US_FitMeniscus::remove_models() {
  ...
  if (ndmods > 0 || nlmods > 0) {  // There are models to scan, so build a list
    ...
    if (ck_confirm->isChecked()) {
      ...
      int response =
          QMessageBox::question(this, tr("Remove Models and Noises?"), msg,
                                QMessageBox::Yes, QMessageBox::Cancel);
      rmv_mdls = (response == QMessageBox::Yes);
    }
...
void US_FitMeniscus::change_plot_type() {
  ...
  if (have3val) {  // Set up 3-D plot
    ...
    meniscus_plot->setAxisTitle(QwtPlot::yLeft,
                                tr("2DSA Meniscus,Bottom RMSD"));
    meniscus_plot->setAxisTitle(QwtPlot::xBottom, tr("Meniscus Radius"));
    meniscus_plot->enableAxis(QwtPlot::yRight, true);
  }

  else {  // Set up 2-D plot
...
void US_FitMeniscus::sel_investigator(void) {
  ...
  connect(inv_dialog, SIGNAL(investigator_accepted(int)), this,
