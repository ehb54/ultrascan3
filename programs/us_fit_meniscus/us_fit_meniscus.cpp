...
US_FitMeniscus::US_FitMeniscus(QMap<QString, QString> triple_info_map) : US_Widgets() {
   ...
   qDebug() << "triple_information: filename, triple_name: " << triple_information[ "filename" ]
   ...
   te_data = new US_Editor(US_Editor::LOAD, false, "results/*.dat;;*.fitmen.dat;;*.fitbot.dat;;*.*");
   connect(te_data, SIGNAL(US_EditorLoadComplete(QString)), SLOT(file_loaded(QString)));
   ...
   QBoxLayout *plot = new US_Plot(
      meniscus_plot, tr("Meniscus,Bottom Fit"), tr("Distance from Base Radii"), tr("2DSA Meniscus,Bottom RMSD"));
   ...
   le_mprads->setToolTip(tr("Meniscus,Bottom current radii (midpoint of ranges)"));
   ...
   pb_update->setToolTip(tr("Update edit record with meniscus; remove non-chosen models"));
   ...
   pb_scandb->setToolTip(tr("Scan fit-meniscus models in DB; create local table files"));

   QPushButton *pb_invest = us_pushbutton(tr("Select Investigator"));
   connect(pb_invest, SIGNAL(clicked()), this, SLOT(sel_investigator()));
   QString inv_name = QString::number(US_Settings::us_inv_ID()) + ": " + US_Settings::us_inv_name();
   ...
   ck_applymwl->setToolTip(tr("Apply the meniscus update to all wavelengths of"
...
void US_FitMeniscus::load_data() {
   ...
   if ((have3val && !was3val) || (!have3val && was3val))
      change_plot_type();
}
...
void US_FitMeniscus::plot_3d(void) {
   ...
   le_bot_lor->setText(QString::number(b_bott, 'f', 5) + "  ( " + QString::number(min_ix) + " )");
   ...
   meniscus_plot->axisTitle(QwtPlot::yRight).setFont(meniscus_plot->axisTitle(QwtPlot::yLeft).font());
   meniscus_plot->setAxisScale(QwtPlot::yRight, spec_dat->range().minValue(), spec_dat->range().maxValue());
   ...
   le_mprads->setText(QString::number(mid_meni) + ", " + QString::number(mid_bott));
...
void US_FitMeniscus::plot_2d(void) {
   ...
   meniscus_plot->setAxisScale(QwtPlot::xBottom, minx - overscan, maxx + overscan);
   ...
   if (order == 2) {
      ...
      // c1 + 2 * c2 * x = 0
      fit_xvl = -c[ 1 ] / (2.0 * c[ 2 ]);
      qDebug() << "2nd order...: c[1], c[2], fit_xvl -- " << c[ 1 ] << c[ 2 ] << fit_xvl;
   }
   else {
   ...
   QwtSymbol *sym = new QwtSymbol(QwtSymbol::Cross, QBrush(Qt::white), markerPen, QSize(9, 9));
...
#define BOTT_LOWVAL 7.0
   ...
   if (bad_vals) {
      if (!auto_mode) {
         int response = QMessageBox::critical(( QWidget * ) this, mhdr, mmsg, QMessageBox::Save, QMessageBox::Cancel);

         if (response == QMessageBox::Cancel) {
            QMessageBox::information(( QWidget * ) this, tr("Canceled"), tr("\"Update Edit\" has been canceled!"));

            return;
         }
         else {
            QMessageBox::information(( QWidget * ) this, tr("Saving"), tr("\"Update Edit\" will proceed!"));
         }
      }
   ...
   if (!all_wvl || nedtfs == 1) { // Apply to a single triple
      ...
      if (confirm) { // Confirm at each update step
         ...
         int response
            = QMessageBox::question(this, tr("Edit File Updated"), mmsg, QMessageBox::Yes, QMessageBox::Cancel);
...
void US_FitMeniscus::scan_dbase() {
   ...
   le_status->setText(tr("Comparing to existing local meniscus,rmsd table files ..."));
   mfnams.clear();
   ufnams.clear();
   uantms.clear();
...
void US_FitMeniscus::update_db_edit(QString edtext, QString efilepath, QString &msg) {
...
void US_FitMeniscus::remove_models() {
   ...
   if (ndmods > 0 || nlmods > 0) { // There are models to scan, so build a list of models,noises to remove
      ...
      if (ck_confirm->isChecked()) {
         ...
         int response
            = QMessageBox::question(this, tr("Remove Models and Noises?"), msg, QMessageBox::Yes, QMessageBox::Cancel);
         rmv_mdls = (response == QMessageBox::Yes);
      }
...
void US_FitMeniscus::noises_in_edit(QString modDesc, QStringList &nieDescs, QStringList &nieIDs, QStringList &nieFnams) {
...
void US_FitMeniscus::sel_investigator(void) {
   if (!dkdb_cntrls->db())
      return; // Ignore if Disk selected

   int personID = US_Settings::us_inv_ID();
   US_Investigator *inv_dialog = new US_Investigator(true, personID);

   connect(inv_dialog, SIGNAL(investigator_accepted(int)), this, SLOT(assign_investigator(int)));
