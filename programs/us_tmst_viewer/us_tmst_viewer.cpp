...
void US_TmstViewer::showTimeState( )
{
   ...
   {  // For database, first download record to local disk
      ...
      QString tsdesc = tmstdata[ selx ][ 0 ];
      tspath         = US_Settings::tmpDir() + "/"
                       + tsdesc + ".time_state.tmst";
      US_TimeState::dbExamine( &db, &idTmst, 0, &fname,
                               &xdefs, &cksumd, 0 );
DbgLv(1) << "sTS: db : fname" << fname << "cksumd" << cksumd
 << "tspath" << tspath << "idTmst" << idTmst;
      if ( QFile( tspath ).exists() )
      {  // File exists, so check if it matches DB
         QString cksumf   = US_Util::md5sum_file( tspath );
DbgLv(1) << "sTS: db :   cksumf" << cksumf;
         if ( cksumf != cksumd )
         {  // Non-matching:  need to download new version
...
DbgLv(1) << "sTS: db :    DB-COPY_FILE_EXISTS";
      ...
      {  // File non-existent, so need to simply download
...
void US_TmstViewer::tmst_items( const QString source )
{
   ...
   if ( source == "database" )
   {  // Read all database "timestate" table rows
      US_Passwd pw;
      US_DB2 db( pw.getPasswd() );
...
DbgLv(1) << "items:  DB query" << qry << "DB numrows" << numrows
 << "DB err" << db.lastErrno() << db.lastError();
      ...
      for ( int ii = 0; ii < expIds.count(); ii++ )
      {  // Issue a query for each experiment ID of timestates
         qry.clear();
         ...
         while ( db.next() )
         {
            ...
            QString cksum    = db.value( 3 ).toString() + " " +
                               db.value( 4 ).toString();
            QString descr    = QString( fname ).section( ".", 0, -3 );
            QString udate    = US_Util::toUTCDatetimeText(
                               db.value( 5 ).toDateTime().toString( Qt::ISODate )
                              , true ).section( " ", 0, 0 ).simplified();
...
DbgLv(1) << "items:  DB tmste" << tmste;
   ...
   {  // Read all "*.tmst" local files from the specified source
      QStringList fdirs;
      QString     bdir;

      if ( source == "tmp" )
      {  // From the single */ultrascan/tmp directory
...
DbgLv(1) << "items:  source" << source << "bdir" << bdir;
      ...
      {  // Get the list of .tmst files in the directory
         ...
         QStringList tfiles = QDir( sdir ).entryList(
               tsfilt, QDir::Files, QDir::Name );
         if ( sdir.endsWith( "/" ) )
            sdir             = sdir.left( sdir.length() - 1 );

         for ( int jj = 0; jj < tfiles.size(); jj++ )
         {  // Build an entry from a file in the directory
            ...
            QString fdate    = US_Util::toUTCDatetimeText( QFileInfo( tmstfp )
