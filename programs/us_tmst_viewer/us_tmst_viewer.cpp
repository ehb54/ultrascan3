...
void US_TmstViewer::loadTimeState() {
   ...
   if (ck_src_db->isChecked()) { // Entries from the database (copied to */ultrascan/tmp)
      tmst_items("database");
   }
...
void US_TmstViewer::showTimeState() {
   ...
   if (tssrc == "database") { // For database, first download record to local disk
      ...
      DbgLv(1) << "sTS: db : fname" << fname << "cksumd" << cksumd << "tspath" << tspath << "idTmst" << idTmst;
...
void US_TmstViewer::tmst_items(const QString source) {
   ...
   if (source == "database") { // Read all database "timestate" table rows
      ...
      DbgLv(1) << "items:  DB query" << qry << "DB numrows" << numrows << "DB err" << db.lastErrno() << db.lastError();
      QStringList expIds;

      while (db.next()) { // Accumulate a list of experiment IDs of user's timestates
         expIds << db.value(0).toString();
      }

      for (int ii = 0; ii < expIds.count(); ii++) { // Issue a query for each experiment ID of timestates
         qry.clear();
         ...
         while (db.next()) {
            ...
            QString udate = US_Util::toUTCDatetimeText(db.value(5).toDateTime().toString(Qt::ISODate), true)
                               .section(" ", 0, 0)
                               .simplified();
      ...
      else if (source == "imports") { // From all */ultrascan/imports/* directories
         bdir = US_Settings::importDir();
         fdirs = QDir(bdir).entryList(QDir::AllDirs | QDir::NoDotAndDotDot, QDir::Name);
      }
      else if (source == "results") { // From all */ultrascan/results/* directories
      ...
      for (int ii = 0; ii < fdirs.size(); ii++) { // Get the list of .tmst files in the directory
         QString sdir = bdir + "/" + fdirs.at(ii);
         QDir dirdir(sdir);
         QStringList tfiles = QDir(sdir).entryList(tsfilt, QDir::Files, QDir::Name);
         if (sdir.endsWith("/"))
            sdir = sdir.left(sdir.length() - 1);

         for (int jj = 0; jj < tfiles.size(); jj++) { // Build an entry from a file in the directory
            ...
            QString fdate
               = US_Util::toUTCDatetimeText(QFileInfo(tmstfp).lastModified().toUTC().toString(Qt::ISODate), true)
                    .section(" ", 0, 0)
                    .simplified();
