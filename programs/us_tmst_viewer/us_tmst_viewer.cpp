...
void
US_TmstViewer::enableControls (void)
{
...
void
US_TmstViewer::tmst_items (const QString source)
{
  QStringList tmste;
          ...
          while (db.next ())
            {
              ...
              QString cksum
                  = db.value (3).toString () + " " + db.value (4).toString ();
        ...
        { // Get the list of .tmst files in the directory
          ...
          for (int jj = 0; jj < tfiles.size (); jj++)
            { // Build an entry from a file in the directory
              ...
              QString fdate
                  = US_Util::toUTCDatetimeText (
                        QFileInfo (tmstfp).lastModified ().toUTC ().toString (
                            Qt::ISODate),
                        true)
                        .section (" ", 0, 0)
                        .simplified ();
