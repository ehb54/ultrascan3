...
int
US_SyncExperiment::synchronize (US_DataModel::DataDesc &cdesc)
{
  int status = 0;
  QString runID;
        ...
        { // get IDs for raw(AUC), experiment, and run
          ...
          if (havedexp)
            { // pick up GUID for possible compare to local
              ...
              DbgLv (1) << "db get_exp_inf: projID "
                        << db->value (1).toString ();
              DbgLv (1) << "db get_exp_inf: runID  "
                        << db->value (2).toString ();
              DbgLv (1) << "db get_exp_inf: rotorID"
                        << db->value (6).toString ();
              DbgLv (1) << "db get_exp_inf: type   "
                        << db->value (7).toString ();
              DbgLv (1) << "db get_exp_inf: comment"
                        << db->value (10).toString ();
              DbgLv (1) << "db get_exp_inf: dateUpd"
                        << db->value (12).toString ();
              if (dexpGUID.isEmpty () || dexpGUID.startsWith ("0000"))
                { // try to set DB experiment GUID
                  ...
                  dexpGUID
                      = lexpGUID.isEmpty () ? US_Util::new_guid () : lexpGUID;
                  query.clear ();
                  query << "update_experiment" << expID << dexpGUID << projID
                        << runID << labID << instrID << operID << rotorID
                        << type << runTemp << label << comment << centrf;
                  ...
                  if (stat == US_DB2::OK)
                    {
                      ...
                      cdesc.recordID = rawID.toInt ();
                      DbgLv (1) << "   EXP update: ID exp run raw" << expID
                                << runID << rawID;
                    }
                  else
                    {
  ...
  if (no_go)
    { // appears we can not do the upload/download:  inform user
      ...
      QMessageBox::information (parentw, tr ("Experiment Record Problem"),
                                msg);
      status = 3003;
    }
...
QString
US_SyncExperiment::expGUIDauc (QString aucfile)
{
  // default dummy GUID
