...
int US_SyncExperiment::synchronize(US_DataModel::DataDesc &cdesc) {
   ...
   if ((cdesc.recState & US_DataModel::REC_LO) != 0) { // there is a local raw record:  check its parent
      havelraw = true;
      lexpGUID = expGUIDauc(cdesc.filename);
      havelexp = !lexpGUID.startsWith("0000");
      runID = cdesc.filename.section("/", -1, -1).section(".", 0, 0);

      if ((cdesc.recState & US_DataModel::REC_DB) == 0) { // local only, so this must be an upload operation
         oper = "upload";
   ...
   if ((cdesc.recState & US_DataModel::REC_DB) != 0) { // there is a database raw record:  check its parent
      query.clear();
      query << "get_rawDataID_from_GUID" << cdesc.dataGUID;

      db->query(query);

      if (db->lastErrno() == US_DB2::OK) { // get IDs for raw(AUC), experiment, and run
         db->next();
         ...
         if (havedexp) { // pick up GUID for possible compare to local
            ...
            if (dexpGUID.isEmpty() || dexpGUID.startsWith("0000")) { // try to set DB experiment GUID
               ...
               query.clear();
               query << "update_experiment" << expID << dexpGUID << projID << runID << labID << instrID << operID
                     << rotorID << type << runTemp << label << comment << centrf;
      ...
      if ((cdesc.recState & US_DataModel::REC_LO) == 0) { // database only, so this must be an download operation
         oper = "download";
   ...
   if (havedexp && havelexp) { // if we have experiment on db and local, check for consistency
...
QString US_SyncExperiment::expGUIDauc(QString aucfile) {
   ...
   if (file.open(QIODevice::ReadOnly | QIODevice::Text)) { // navigate to experiment guid attribute
      QXmlStreamReader xml(&file);

      while (!xml.atEnd()) {
         xml.readNext();

         if (xml.isStartElement() && xml.name() == "experiment") { // found it!  return its value
