...
US_ManageData::US_ManageData () : US_Widgets ()
{
  ...
  connect (cb_source, SIGNAL (currentIndexChanged (QString)), this,
           SLOT (reset ()));
...
void
US_ManageData::reset_hsbuttons (bool show, bool edts, bool mods, bool nois)
{
  QString blabl;
  QString btool;
...
void
US_ManageData::sel_investigator ()
{
...
void
US_ManageData::assign_investigator (int invID)
{
  personID = invID;
...
void
US_ManageData::clickedItem (QTreeWidgetItem *item)
{
...
void
US_ManageData::dtree_help ()
{
}

// format an item action text for a message box
QString
US_ManageData::action_text (QString exstext, QString acttext)
{
...
void
US_ManageData::action_result (int stat, QString item_act)
{
  if (stat != 999)
    { // proceed was selected:  test status of action
      ...
      if (stat == 0)
        ...
        { // action got an error
          QMessageBox::warning (this, item_act + tr (" *ERROR*!"),
...
void
US_ManageData::reportDataStatus ()
{
  ...
  // Reformat and display report on record counts
  te_status->setText (QString ().sprintf ("%5d", ncrecs)
...
void
US_ManageData::selected_runID (QString selrunID)
{
  ...
  if (selrunID.startsWith (tr ("Select individual")))
    { // Combo box select is "Select individual run" so present list dialog
      ...
      if (srdiag->exec () == QDialog::Accepted && selx >= 0)
  ...
  lb_status->setText (
      (ntriple == 1)
          ? tr ("The run has a single triple")
          : tr ("%1 triples are available to select").arg (ntriple));
}

// Slot to handle a newly select runID filter
void
US_ManageData::selected_runID (int index)
{
