...
US_ManageData::US_ManageData() : US_Widgets() {
   ...
   pb_invtor->setToolTip(tr("Use an Investigator dialog to set the database person ID"));
   pb_scanda->setToolTip(tr("Scan Database and Local data, with content analysis"));
   pb_helpdt->setToolTip(tr("Show a short Help/Legend dialog for notes on the data tree"));
   pb_reset->setToolTip(tr("Reset the data display to its default state"));
   pb_help->setToolTip(tr("Display detailed US_ManageData documentation text and images"));
   pb_close->setToolTip(tr("Close the US_ManageData window and exit"));
   cb_runid->setToolTip(tr("Select a single run ID with which to populate the data tree "));
   cb_triple->setToolTip(tr("Select a single triple of the run to populate the data tree "));
   cb_source->setToolTip(tr("Select a filter to limit records based on source (DB/Local) "));
   ...
   te_status->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 2));
   te_status->setText(tr(" 0 Combined Total data sets;\n"
...
void US_ManageData::toggle_models() {
   ...
   if (show) { // for show models, must expand levels above; reset edit,model buttons
...
void US_ManageData::toggle_noises() {
   ...
   if (show) { // for show noise, must expand above; reset edit,model,noise buttons
...
void US_ManageData::reset_hsbuttons(bool show, bool edts, bool mods, bool nois) {
   QString blabl;
   QString btool;
   ...
   if (show) { // show (expand) is the new state
      if (edts) { // Hide Edits
         ...
         pb_hsedit->setToolTip(tr("Collapse All, so Edits and all descendants are hidden"));
      }

      if (mods) { // Hide Models
         pb_hsmodl->setText(tr("Hide All Models"));
         pb_hsmodl->setToolTip(tr("Collapse Edits, so Models and their children are hidden"));
      }
      ...
      if (edts) { // Show Edits
         ...
         pb_hsedit->setToolTip(tr("Expand Raws, so Edits and parent Raws are shown"));
      }

      if (mods) { // Show Models
         pb_hsmodl->setText(tr("Show All Models"));
         pb_hsmodl->setToolTip(tr("Expand Edits, so Models and their parents are shown"));
      }

      if (nois) { // Show Noises
         pb_hsnois->setText(tr("Expand All"));
         pb_hsnois->setToolTip(tr("Expand All, so Noises and all ancestors are shown"));
      }
   }
}
...
void US_ManageData::assign_investigator(int invID) {
   personID = invID;
   le_invtor->setText(QString::number(personID) + ": " + US_Settings::us_inv_name());
...
void US_ManageData::clickedItem(QTreeWidgetItem *item) {
   ...
   if (QApplication::mouseButtons() == Qt::RightButton) { // only bring up context menu if right-mouse-button was clicked
...
void US_ManageData::action_result(int stat, QString item_act) {
   if (stat != 999) { // proceed was selected:  test status of action
      ...
      if (stat == 0) { // action was successful
         ...
         QMessageBox::warning(
            this, item_act + tr(" *ERROR*!"), tr("The \"%1\" action had an error: %2").arg(item_act).arg(stat));
...
void US_ManageData::reportDataStatus() {
   ...
   te_status->setText(
      ...
      + QString().sprintf("%5d", ndraws) + tr(" Database RawData    records;\n  ") + QString().sprintf("%5d", ndedts)
      ...
      + QString().sprintf("%5d", nlraws) + tr(" Local    RawData    records;\n  ") + QString().sprintf("%5d", nledts)
...
void US_ManageData::selected_runID(QString selrunID) {
   ...
   if (selrunID.startsWith(
          tr("Select individual"))) { // Combo box select is "Select individual run" so present list dialog
      int selx = -1;
      QStringList hdrs;
      ...
      for (int ii = 0; ii < runIDs.count(); ii++) { // Build data consisting of a single Run ID column
         runid_data << QStringList(runIDs[ ii ]);
      }

      US_SelectItem *srdiag = new US_SelectItem(runid_data, hdrs, dtitl, &selx, 1);
      if (srdiag->exec() == QDialog::Accepted && selx >= 0) { // Add selected run to comboBox and set Run ID variable
   ...
   lb_status->setText(
      (ntriple == 1) ? tr("The run has a single triple") : tr("%1 triples are available to select").arg(ntriple));
}
