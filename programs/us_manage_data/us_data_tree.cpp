...
US_DataTree::US_DataTree(US_DataModel *dmodel, QTreeWidget *treewidg, QWidget *parent /*=0*/) {
   da_model = dmodel;
   tw_recs = treewidg;
...
void US_DataTree::toggle_expand(QString c1str, bool show) {
   QList<QTreeWidgetItem *> listi = tw_recs->findItems(c1str, Qt::MatchExactly | Qt::MatchWrap | Qt::MatchRecursive, 0);
...
void US_DataTree::build_dtree() {
   ...
   for (int ii = 0; ii < ncrecs; ii++) {
      ...
      if (cdesc.recordID < 0 && isLoc) { // local only
      ...
      else if (isDba && !isLoc) { // database only
      ...
      if (cdesc.recState == US_DataModel::NOSTAT) { // mark artificial record with color and source text
         fbru = QBrush(colorGray);
         rsrc = "dummy";
      }

      for (int jj = (ii + 1); jj < ncrecs; jj++) { // count children and descendants from next-row until back-to-level
      ...
      anch = (ityp < 4) ? QString("%1, %2").arg(nchdb).arg(nchlo) : ""; // children
      ande = (ityp < 3) ? QString("%1, %2").arg(ndedb).arg(ndelo) : ""; // descendants
      ...
      if (ityp == 1) { // Raws are children of the root
         item = new QTreeWidgetItem(tw_recs, cvals, wiutype);
      }

      else { // others are children of the next level up
...
void US_DataTree::row_context_menu(QTreeWidgetItem *item) {
   ...
   if (nsel > 1) { // If multiple selections, set first row
   ...
   if (nsel > 1) { // Multiple selections:  "record/branch" to "branches"
   ...
   if (cdesc.recType == 1) { // Raw:  "record/branches" to "descendants"
   ...
   connect(aupload, SIGNAL(triggered()), this, SLOT(item_upload()));
   connect(adnload, SIGNAL(triggered()), this, SLOT(item_download()));
   connect(adbasrm, SIGNAL(triggered()), this, SLOT(item_remove_db()));
   connect(aloclrm, SIGNAL(triggered()), this, SLOT(item_remove_loc()));
   connect(abothrm, SIGNAL(triggered()), this, SLOT(item_remove_all()));
   connect(ashdeta, SIGNAL(triggered()), this, SLOT(item_details()));

   // disable menu items that are not appropriate to the record

   if ((cdesc.recState & US_DataModel::REC_LO) == 0) { // if record not local, no upload and no remove from local or both
      aupload->setEnabled(false);
      aloclrm->setEnabled(false);
      abothrm->setEnabled(false);
   }

   if ((cdesc.recState & US_DataModel::REC_DB) == 0) { // if record not db, no download and no remove from db or both
...
void US_DataTree::dtree_help() {
   ...
   QString mtext = tr("Data Tree Columns --\n\n") + tr(" \"Type\"       :  Type of data set record -\n")
...
void US_DataTree::item_details() {
   ...
   QString mtext = tr("Data Tree Item at Row %1 -- \n\n").arg(irow)
                   ...
                   + tr("  DB record ID   : %1\n").arg(cdesc.recordID) + tr("  Global ID      : ") + cdesc.dataGUID
                   + "\n" + tr("  Parent GUID    : ") + cdesc.parentGUID + "\n"
                   + tr("  DB parent ID   : %1\n").arg(cdesc.parentID) + tr("  File Directory : ")
                   + cdesc.filename.section("/", 0, -2) + "\n" + tr("  File Name      : ")
                   + cdesc.filename.section("/", -1, -1) + "\n" + tr("  File Last Mod  : ") + cdesc.filemodDate + "\n"
                   + tr("  DB   Last Mod  : ") + cdesc.lastmodDate + "\n" + tr("  Record State   : ")
                   + record_state(cdesc.recState) + "\n";

   if (cdesc.contents.length() < 60) {
      mtext = mtext + tr("  Content Checks : ") + cdesc.contents;
   }
   else {
      QString cont1 = cdesc.contents.section(" ", 0, 1).simplified();
      QString cont2 = cdesc.contents.section(" ", 2, 4).simplified();
      mtext = mtext + tr("  Content Checks : ") + cont1 + "\n" + "                   " + cont2 + "\n";
#if 0
...
void US_DataTree::items_details() {
   ...
   QString mtext = tr("Data Tree Items from Rows %1 to %2 -- \n\n").arg(irow).arg(krow)
                   + tr("  Type   (first) : %1\n").arg(rtyps[ cdesc.recType - 1 ]) + tr("    Description    : ")
                   + cdesc.description + "\n" + tr("    DB record ID   : %1\n").arg(cdesc.recordID)
                   + tr("    File Directory : ") + cdesc.filename.section("/", 0, -2) + "\n"
                   + tr("    File Name      : ") + cdesc.filename.section("/", -1, -1) + "\n"
                   + tr("  Type    (last) : %1\n").arg(rtyps[ kdesc.recType - 1 ]) + tr("    Description    : ")
                   + kdesc.description + "\n" + tr("    DB record ID   : %1\n").arg(kdesc.recordID)
                   + tr("    File Directory : ") + kdesc.filename.section("/", 0, -2) + "\n"
                   + tr("    File Name      : ") + kdesc.filename.section("/", -1, -1) + "\n"
...
QString US_DataTree::action_text(QString exstext, QString acttext) {
   QString lines = (selitems.size() == 1) ? tr("This item exists on %1.<br>"
...
bool US_DataTree::is_modelIDs_from_autoflow(QString mID) {
   ...
   if (db.lastErrno() != US_DB2::OK) {
      QMessageBox::warning(parentw, tr("Connection Problem"), tr("Could not connect to database \n") + db.lastError());
      return false;
   }
...
bool US_DataTree::check_filename_for_autoflow(QString rFilename) {
   ...
   qDebug() << "in check_filename_for_autoflow(): autoflowNumber, isRequired -- " << autoflowNumber << isRequired;
...
int US_DataTree::do_actions(QString item_exs, QString item_act) {
   ...
   int naerrs = 0;
   int istat = 0;
   bool frDB = item_exs.contains("Datab") || item_exs.contains("DB ");
   ...
   bool upLoad = item_act.contains("DB create") || item_act.contains("DB replace");
   bool remove = item_act.contains("remove");
   QProgressBar *progress = da_model->progrBar();
   QLabel *stlabel = da_model->statlab();

   if (remove && frDB) { // REMOVE from DB
         ...
         if (ddesc.recType == 1 || ddesc.recType == 2 || ddesc.recType == 3 || ddesc.recType == 4) // all types
         {
            ...
            if (isRequired) {
               QMessageBox::information(
                  parentw, tr("Data Cannot be Deleted"),
	  ...
	  ****/
         ...
         if (ddesc.recType == 3 || ddesc.recType == 4) //model OR noise
         {
            ...
            if (is_modelIDs_from_autoflow(QString::number(model_idDB))) {
               ...
               QMessageBox::information(
                  parentw, tr("Selected Model GMP produced!"),
         ...
         /** TEMP   ***/
         if (ityp > lrtyp) { // Just mark as deleted if descendant of last removed
            ddesc.recordID = -1;
   ...
   if (remove && frLoc) { // REMOVE from LOCAL
      for (int ii = 4; ii > 1; ii--) { // Remove records from bottom up
         for (int jj = 0; jj < narows; jj++) {
            int irow = actrows[ jj ];
            US_DataModel::DataDesc ddesc = da_model->row_datadesc(irow);
            int ityp = ddesc.recType;
            ...
            if (ityp != ii)
               continue; // skip if not at current level
   ...
   if (upLoad) { // UPLOAD to DB
      for (int ii = 2; ii < 5; ii++) { // Upload records from top down
         for (int jj = 0; jj < narows; jj++) {
            int irow = actrows[ jj ];
            US_DataModel::DataDesc ddesc = da_model->row_datadesc(irow);
            int ityp = ddesc.recType;
   ...
   if (dnLoad) { // DOWNLOAD to LOCAL
      ...
      progress->setValue(0);
      int ndown = 0;
      for (int ii = 2; ii < 5; ii++) { // Download records from top down
         for (int jj = 0; jj < narows; jj++) {
            int irow = actrows[ jj ];
            US_DataModel::DataDesc ddesc = da_model->row_datadesc(irow);
            int ityp = ddesc.recType;
   ...
   if (naerrs > 0 && naerrs < narows) { // Append to error message, explaining partial success
...
int US_DataTree::action_rows() {
   ...
   for (int ii = 0; ii < nsrows; ii++) {
      ...
      for (int jrow = (irow + 1); jrow < ncrecs; jrow++) { // add descendants from next-row until back-to-level
...
bool US_DataTree::significant_diffs(US_DataModel::DataDesc ddesc, US_DB2 *dbP) {
   ...
   if (ddesc.recType == 3) {
      ...
      if (dmodel == fmodel)
         ...
         (dmodel.monteCarlo == fmodel.monteCarlo) && (dmodel.description == fmodel.description)
         && (dmodel.modelGUID == fmodel.modelGUID) && (dmodel.editGUID == fmodel.editGUID)
         && (dmodel.wavelength == fmodel.wavelength) && (dmodel.variance == fmodel.variance)
         && (dmodel.meniscus == fmodel.meniscus) && (dmodel.optics == fmodel.optics)
         && (dmodel.analysis == fmodel.analysis) && (dmodel.global == fmodel.global)
         && (dmodel.coSedSolute == fmodel.coSedSolute)
         && (dmodel.components.size() == fmodel.components.size())) { // Difference can only be in component values
         for (int ii = 0; ii < dmodel.components.size(); ii++) {
            if (dmodel.components[ ii ] != fmodel.components[ ii ]) { // There is a components difference, so measure it
   ...
   // if noise, check for differences beyond tolerance in noise values
      ...
      if (dnoise == fnoise)
         ...
         (dnoise.type == fnoise.type) && (dnoise.description == fnoise.description)
         && (dnoise.noiseGUID == fnoise.noiseGUID) && (dnoise.modelGUID == fnoise.modelGUID)
         && (dnoise.minradius == fnoise.minradius) && (dnoise.maxradius == fnoise.maxradius)
         && (dnoise.values.size() == fnoise.values.size())) { // Difference can only be in noise values
