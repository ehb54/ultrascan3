...
void US_DataTree::toggle_expand(QString c1str, bool show) {
  ...
  for (int ii = 0; ii < listi.size(); ii++) listi.at(ii)->setExpanded(show);
}
...
void US_DataTree::build_dtree() {
  ...
  for (int ii = 0; ii < ncrecs; ii++) {
    ...
    if (cdesc.recState == US_DataModel::NOSTAT) {  // mark artificial record
                                                   // with color and source text
      fbru = QBrush(colorGray);
      rsrc = "dummy";
    }

    for (int jj = (ii + 1); jj < ncrecs;
         jj++) {  // count children and descendants from next-row until
                  // back-to-level
    ...
    anch =
        (ityp < 4) ? QString("%1, %2").arg(nchdb).arg(nchlo) : "";  // children
    ande = (ityp < 3) ? QString("%1, %2").arg(ndedb).arg(ndelo)
                      : "";  // descendants
...
void US_DataTree::row_context_menu(QTreeWidgetItem* item) {
  ...
  if ((cdesc.recState & US_DataModel::REC_LO) ==
      0) {  // if record not local, no upload and no remove from local or both
  ...
  if ((cdesc.recState & US_DataModel::REC_DB) ==
      0) {  // if record not db, no download and no remove from db or both
  ...
  while (--jrow > 0 && cdesc.recType > 1) {
    ...
    if (jdesc.recType < cdesc.recType) break;
  }

  if ((jdesc.recState & US_DataModel::REC_DB) == 0) aupload->setEnabled(false);
  ...
  QString lines = (selitems.size() == 1)
                      ...
                      : tr("These items exists on %1.<br>"
                           "Are you sure you want to proceed with %2s?<ul>")
                            .arg(exstext)
                            .arg(acttext);
  return lines + tr("<li><b>No </b> to cancel the action;</li>"
...
QString US_DataTree::record_state(int istate) {
  ...
  if (flags != "NOSTAT") flags = flags.mid(7, 999);  // remove any "NOSTAT|"
...
bool US_DataTree::is_modelIDs_from_autoflow(QString mID) {
  ...
  while (db.next()) modelInfos_autoflow << db.value(0).toString();

  return parse_models_desc_json(modelInfos_autoflow, mID);
}

bool US_DataTree::parse_models_desc_json(QStringList modelInfos_autoflow,
                                         QString mID) {
  ...
  for (int i = 0; i < modelInfos_autoflow.size(); ++i) {
    ...
    if (modelDescJson.isEmpty()) return false;
        ...
        for (int i = 0; i < json_array.size(); ++i) {
          foreach (const QString& array_key, json_array[i].toObject().keys()) {
            ...
            if (array_key == "modelID") {
              ...
              if (c_modelID == mID) {
                qDebug() << "Model to affect [delete:update], ID=" << mID
                         << " IS autoflow produced!!!";
                return true;
              }
            }
          }
...
int US_DataTree::do_actions(QString item_exs, QString item_act) {
  ...
  bool upLoad =
      item_act.contains("DB create") || item_act.contains("DB replace");
  ...
  if (remove && frDB) {  // REMOVE from DB
    ...
    for (int jj = 0; jj < narows; jj++) {
      ...
      qDebug() << "DataModel: recType, recordID, parentID, description: "
               << ddesc.recType   // recType={1,2,3,4} => {Raw, EditedData,
                                  // Model, Noise}
               << ddesc.recordID  // to delete EditedData, Model, OR Noise (
                                  // recType==2,3,4)
               << ddesc.parentID  // to delete Raw Data                    (
                                  // recType==1)
               << ddesc.description;

      // Now, by the recType, determine the ID of the rawData (top-level
      // parentID) Obvious for types {1,2}=> {Raw, EditedData}
      QString rFilename = ddesc.description.split(".")[0].simplified();
      qDebug() << "Filename of the run: " << rFilename;

      if (ddesc.recType == 1 || ddesc.recType == 2 || ddesc.recType == 3 ||
          ddesc.recType == 4)  // all types
      {
        ...
        if (isRequired) {
          QMessageBox::information(parentw, tr("Data Cannot be Deleted"),
                                   tr("The Data for the Run:\n\n"
                                      "\"%1\"\n\n"
                                      "can NOT be deleted since it is required "
                                      "by the GMP framework!")
                                       .arg(rFilename));
      ...
      /*****
            NOTE: if we do not want to delete *any* model/noise associated but
       NOT actually required by GMP run, we can stop here.... Below, is a finer
       model/noise treatment based on GMP run to require ONLY {2DSA-IT, 2DSA-MC
       & PSCA} models...
       ****/
  ...
  if (naerrs > 0 &&
      naerrs < narows) {  // Append to error message, explaining partial success
...
int US_DataTree::action_rows() {
  ...
  for (int ii = 0; ii < nsrows; ii++) {
    ...
    for (int jrow = (irow + 1); jrow < ncrecs;
         jrow++) {  // add descendants from next-row until back-to-level
...
bool US_DataTree::significant_diffs(US_DataModel::DataDesc ddesc, US_DB2* dbP) {
  ...
  if (ddesc.recType == 3) {
    ...
    if (dst != US_DB2::OK) dmodel.description = "DBAD:" + dmodel.description;

    if (fst != US_DB2::OK) fmodel.description = "FBAD:" + fmodel.description;
    ...
    if (dmodel == fmodel)
      differs = false;  // Differences only in formatting
             ...
             (dmodel.components.size() ==
              fmodel.components
                  .size())) {  // Difference can only be in component values
      for (int ii = 0; ii < dmodel.components.size(); ii++) {
        if (dmodel.components[ii] !=
            fmodel.components[ii]) {  // There is a components difference, so
                                      // measure it
  ...
  // if noise, check for differences beyond tolerance in noise values
    ...
    if (dst != US_DB2::OK) dnoise.description = "DBAD:" + dnoise.description;

    if (fst != US_DB2::OK) fnoise.description = "FBAD:" + fnoise.description;
    ...
    if (dnoise == fnoise)
      differs = false;  // Differences only in formatting
             ...
             (dnoise.values.size() ==
              fnoise.values
                  .size())) {  // Difference can only be in noise values
