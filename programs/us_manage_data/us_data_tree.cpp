...
void
US_DataTree::toggle_expand (QString c1str, bool show)
{
  ...
  for (int ii = 0; ii < ncrecs; ii++)
    {
      ...
      anch = (ityp < 4) ? QString ("%1, %2").arg (nchdb).arg (nchlo)
                        : ""; // children
...
void
US_DataTree::row_context_menu (QTreeWidgetItem *item)
{
...
void
US_DataTree::record_type (int recType, QString &item_act)
{
  const char *rtyps[] = { "RawData", "EditedData", "Model", "Noise" };
...
QString
US_DataTree::action_text (QString exstext, QString acttext)
{
...
void
US_DataTree::action_result (int stat, QString item_act)
{
  ...
  if (stat != 999)
    { // proceed was selected:  test status of action
      ...
      if (stat == 0)
        ...
        { // action got an error
          QMessageBox::warning (parentw, item_act + tr (" *ERROR*!"),
...
QString
US_DataTree::record_state (int istate)
{
...
// Check for model
bool
US_DataTree::is_modelIDs_from_autoflow (QString mID)
{
...
bool
US_DataTree::parse_models_desc_json (QStringList modelInfos_autoflow,
                                     QString mID)
{
                  ...
                  foreach (const QString &array_key,
                           json_array[i].toObject ().keys ())
                    {
                      ...
                      if (array_key == "modelID")
                        {
                          QString c_modelID = json_array[i]
                                                  .toObject ()
                                                  .value (array_key)
                                                  .toString ();
                          ...
                          if (c_modelID == mID)
                            {
                              qDebug ()
                                  << "Model to affect [delete:update], ID="
                                  << mID << " IS autoflow produced!!!";
                              return true;
                            }
                        }
                    }
...
bool
US_DataTree::check_filename_for_autoflow (QString rFilename)
{
  ...
  qDebug ()
      << "in check_filename_for_autoflow(): autoflowNumber, isRequired -- "
...
int
US_DataTree::do_actions (QString item_exs, QString item_act)
{
  int narows = action_rows ();
      ...
      for (int jj = 0; jj < narows; jj++)
        {
              ...
              if (isRequired)
                {
                  QMessageBox::information (
                      ...
                      tr ("The Data for the Run:\n\n"
                          "\"%1\"\n\n"
                          "can NOT be deleted since it is required by the GMP "
                          "framework!")
                          .arg (rFilename));
          ...
          /*****
                NOTE: if we do not want to delete *any* model/noise associated
           but NOT actually required by GMP run, we can stop here.... Below, is
           a finer model/noise treatment based on GMP run to require ONLY
           {2DSA-IT, 2DSA-MC & PSCA} models...
           ****/
...
bool
US_DataTree::significant_diffs (US_DataModel::DataDesc ddesc, US_DB2 *dbP)
{
          ...
          for (int ii = 0; ii < dmodel.components.size (); ii++)
            {
              if (dmodel.components[ii] != fmodel.components[ii])
                { // There is a components difference, so measure it
                  US_Model::SimulationComponent *dcomp
                      = &dmodel.components[ii];
                  US_Model::SimulationComponent *fcomp
                      = &fmodel.components[ii];
                  ...
                  difrat = qAbs (dcvalu - fcvalu) / valmin;
                  difmax = qMax (difrat, difmax);
                  DbgLv (1) << "   CONFLICT? D dcv fcv dr" << dcvalu << fcvalu
                            << difrat;
