...
int
US_DataProcess::record_upload (int row)
{
  ...
  if ((cdesc.recState & US_DataModel::REC_DB) != 0 && ftime < dtime)
      ...
      if (stat == 0)
        {
          ...
          stat = db->writeBlobToDB (filepath, QString ("upload_aucData"),
                                    idData);
    ...
    { // upload an EditedData record
      ...
      if ((stat = db->lastErrno ()) != US_DB2::OK)
      ...
      else
        {
          ...
          stat = db->writeBlobToDB (filepath, QString ("upload_editData"),
                                    idData);
...
int
US_DataProcess::record_download (int row)
{
  int stat = 0;
  QStringList query;
  ...
  if (cdesc.recType == 1 && cdesc.recType != 1) // never download raw
    ...
    { // download an EditedData record
      ...
      if (stat != 0)
        errMsg
            = tr ("Edited download status %1").arg (stat) + "\n " + filepath;
    }
...
int
US_DataProcess::record_remove_db (int row)
{
  int stat = 0;
  QStringList query;
      ...
      if ((stat = db->statusQuery (query)) != 0)
        {
          ...
          DbgLv (1) << "MOD_RMV:    lastErr" << db->lastError ()
                    << db->lastErrno ();
          stat = 2013;
        }
      else
      ...
      if ((stat = db->statusQuery (query)) != 0)
        {
          ...
          DbgLv (1) << "NOI_RMV:    lastErr" << db->lastError ()
                    << db->lastErrno ();
          stat = 2014;
        }
      else
...
int
US_DataProcess::record_remove_local (int row)
{
  int stat = 0;
  cdesc = da_model->row_datadesc (row);
...
QString
US_DataProcess::get_model_filename (US_DataModel::DataDesc *ddesc)
{
...
// Get next model file name
QString
US_DataProcess::get_model_filename (QString guid)
{
  QString path;
...
QString
US_DataProcess::get_noise_filename (US_DataModel::DataDesc *ddesc)
{
  QString path = ddesc->filename;
...
// Get next noise file name
QString
US_DataProcess::get_noise_filename (QString guid)
{
...
void
US_DataProcess::partialError (int nacterrs, int nactions)
{
...
void
US_DataProcess::appendError (QString aperrMsg)
{
  errMsg += aperrMsg;
}
...
bool
US_DataProcess::raw_ancestor_ok (int row)
{
  bool isOK = false;
  int jrow = row;
