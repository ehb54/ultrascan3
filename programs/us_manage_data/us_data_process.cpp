...
int US_DataProcess::record_upload(int row) {
   ...
   if (!raw_ancestor_ok(row)) { // Cannot upload if ancestor Raw is not in both DB and Local
   ...
   if ((cdesc.recState & US_DataModel::REC_DB) != 0 && ftime < dtime) { // Cannot replace DB record with older Local record
   ...
   { // upload a Raw record
      ...
      if ((stat = db->lastErrno()) != US_DB2::OK) {
         ...
         if (idData < 0) {
            query.clear();
            query << "new_editedData" << rawDataID << editGUID << runID << filename << comment;
            db->query(query);
         ...
         query.clear();
         query << "update_editedData" << editID << rawDataID << editGUID << label << filename << comment;
         db->query(query);
...
int US_DataProcess::record_download(int row) {
   ...
   if (!raw_ancestor_ok(row)) { // Cannot download if ancestor Raw is not in both DB and Local
   ...
   filepath = (filepath == filename) ? US_Settings::resultDir() + "/" + runID + "/" + filename : filepath;
...
int US_DataProcess::record_remove_db(int row) {
   ...
   if (cdesc.recType == 1) { // remove a Raw record
      ...
      db->query(query);
      db->next();
      DbgLv(1) << "REC_RMV: rawGUID rawID expID" << cdesc.dataGUID << db->value(0).toString()
               << db->value(1).toString();
      query.clear();
...
int US_DataProcess::record_remove_local(int row) {
   ...
   if (file.exists()) { // the file to remove does exist
      ...
      if (file.remove()) { // the remove was successful
         ...
         if ((cdesc.recState & US_DataModel::REC_DB) == 0) { // it was local-only, so now it's a dummy
...
bool US_DataProcess::raw_ancestor_ok(int row) {
   ...
   while (--jrow >= 0) { // walk up tree until a Raw record is encountered.
      ...
      if (ddesc.recType == 1) { // This is the Raw ancestor of the given record
         if ((ddesc.recState & bSrc) == bSrc) { // If both DB and Local, mark as OK for upload/download
            isOK = true;
         }
         break;
      }
