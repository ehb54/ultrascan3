...
US_DB2* US_DataModel::dbase() { return db; }

// Get progress bar pointer
QProgressBar* US_DataModel::progrBar() { return progress; }

// Get status label pointer
QLabel* US_DataModel::statlab() { return lb_status; }

// get us_data_process object pointer
QObject* US_DataModel::procobj() { return ob_process; }

// get us_data_tree object pointer
QObject* US_DataModel::treeobj() { return ob_tree; }
...
void US_DataModel::getRunIDs(QStringList& runIDs, int& source) {
  ...
  if (source != 1) {
    ...
    for (int ii = 0; ii < aucdirs.size();
         ii++) {  // Loop thru potential data directories; add any new with AUC
                  // content
      ...
      if (naucf > 0) {  // Possibly add if run has AUC content
        if (!runIDs.contains(runID) &&
            source !=
                4) {  // This run not in local and exclude-DB-only not specified
          runIDs << runID;
        }
      }
    }
...
void US_DataModel::getTriples(QStringList& triples, QString runID) {
  ...
  aucfilt << runID + "*.auc";
  QStringList aucfiles =
      QDir(aucdir).entryList(aucfilt, QDir::Files, QDir::Name);
  int naucf = aucfiles.size();
  DbgLv(1) << "gTr:  naucf" << naucf;

  for (int ii = 0; ii < naucf;
       ii++) {  // Loop thru files to add any new triples found
...
US_DataModel::DataDesc US_DataModel::current_datadesc() { return cdesc; }
...
void US_DataModel::setCurrent(int irow) { cdesc = adescs.at(irow); }

// get count of total data records
int US_DataModel::recCount() { return adescs.size(); }

// get count of DB data records
int US_DataModel::recCountDB() { return ddescs.size(); }

// get count of local data records
int US_DataModel::recCountLoc() { return ldescs.size(); }
...
void US_DataModel::scan_dbase() {
  ...
  if (!filt_source.isEmpty() &&
      filt_source ==
          "Local Only") {  // If source filter is "Local-only", skip DB scan
    ddescs.clear();
    return;
  }

  if (chgrows.size() >
      0) {  // If changes since last scan, just modify existing db descriptions
    review_dbase();
    return;
  }
  ...
  if (dbg_level > 1) {
    ...
    for (int ii = 0; ii < expIDs.size(); ii++) {
      ...
      QString date = US_Util::toUTCDatetimeText(
          db->value(12).toDateTime().toString(Qt::ISODate), true);
      DbgLv(2) << "  " << expID << expGUID << runID << label << comment << date;
    }
  }
    ...
    while (db->next()) {  // Read Raw records
      ...
      if (rfilt_q) {
        ...
        date = US_Util::toUTCDatetimeText(
            db->value(7).toDateTime().toString(Qt::ISODate), true);
      ...
      if (comment.isEmpty()) comment = filename.section(".", 0, -2);

      if (!label.contains(".")) label = filename.section(".", 0, -2);

      if (!rfilt) rawIDs << recID;

      DbgLv(2) << "BrDb:     raw expGid" << expGUID;
      DbgLv(2) << "BrDb:      label filename comment" << label << filename
               << comment;
      ...
      cdesc.parentGUID =
          cdesc.parentGUID.length() == 36 ? cdesc.parentGUID : dmyGUID;
  ...
  for (int jq = 0; jq < nqry; jq++) {
    ...
    while (db->next()) {  // Read Edit records
      ...
      if (!rfilt) edtIDs << recID;

      QString subType = filebase.section(".", 2, 2);
      contents = cksum + " " + recsize;
      DbgLv(2) << "BrDb:     edt  id eGID rGID label date" << irecID << editGUID
               << rawGUID << label << date;
      // DbgLv(2) << "BrDb:       (E)contents" << contents;

      if (!filename.contains("/"))
        filename = US_Settings::resultDir() + "/" +
                   filename.section(".", 0, 0) + "/" + filename;
      ...
      cdesc.contents = contents;
      cdesc.description =
          (comment.isEmpty()) ? filebase.section(".", 0, 2) : comment;
  ...
  for (int jq = 0; jq < nqry; jq++) {
    ...
    while (db->next()) {  // get model information from DB
      ...
      if ((++kmdl) == 1) DbgLv(2) << "BrDb:  First Model " << nowTime();
      if (rfilt) {
      ...
      if (descript.length() ==
          80) {  // Truncated description?  save for later testing/replacement
        tmodels << recID;
        tmodnxs << ddescs.size();
      }

      QString editGUID = db->value(5).toString();
      DbgLv(2) << "BrDb: MOD id" << recID << " edID" << editID << " edGID"
               << editGUID;
      ...
      if (label.length() > 40) label = label.left(13) + "..." + label.right(24);
  ...
  for (int jq = 0; jq < nqry; jq++) {
    ...
    while (db->next()) {  // Get noise information from DB
      ...
      QString date = US_Util::toUTCDatetimeText(
          db->value(6).toDateTime().toString(Qt::ISODate), true);
      QString cksum = db->value(7).toString();
      QString recsize = db->value(8).toString();
      QString descript = db->value(9).toString();
      DbgLv(2) << "BrDb: NOI id" << recID << " edID" << editID << " moID"
               << modelID << " descript" << descript;
      ...
      if (descript.isEmpty() || descript.length() == 80) {
        ...
        if (jmod >= 0) {
          ...
          if (descript.length() ==
              80) {  // Truncated description?  Save for later review/replace
            tnoises << recID;
            tnoinxs << ddescs.size();
          }
      ...
      // DbgLv(3) << "BrDb:
      // contents================================================"; DbgLv(3) <<
      // contents.left( 200 ); DbgLv(3) << "BrDb:
      // contents================================================";
      ...
      cdesc.filemodDate = "";
      cdesc.lastmodDate = date;
      DbgLv(2) << "BrDb:       noi id nGID dsc typ noityp" << irecID
               << noiseGUID << descript << cdesc.subType << noiseType;
  ...
  for (int ii = 0; ii < tmodels.size();
       ii++) {  // Change truncated model descriptions
    recID = tmodels[ii];
    int jdsc = tmodnxs[ii];
  ...
  for (int ii = 0; ii < tnoises.size();
       ii++) {  // Change truncated noise descriptions
    recID = tnoises[ii];
    int jdsc = tnoinxs[ii];
    cdesc = ddescs.at(jdsc);
    US_Noise noise1;
  ...
  progress->setMaximum(nstep);
  qApp->processEvents();
  DbgLv(1) << "BrDb: kr ke km kn" << rawIDs.size() << edtIDs.size()
           << modIDs.size() << noiIDs.size();
...
void US_DataModel::scan_local() {
  ...
  if (!filt_source.isEmpty() &&
      filt_source ==
          "DB Only") {  // If source filter is "DB-only", skip Local scan
    return;
  }
  ...
  QString dmyGUID = "00000000-0000-0000-0000-000000000000";
  QStringList aucdirs =
      rfilt ? QStringList(filt_run)
            : QDir(rdir).entryList(QDir::AllDirs | QDir::NoDotAndDotDot,
                                   QDir::Name);
  ...
  if (rfilt) {  // If run filtering, count actual files that match
    ...
    for (int ii = 0; ii < naucd; ii++) {
      ...
      edtpatt =
          tfilt ? filt_run + ".*" + filt_triple + ".xml" : filt_run + ".*.xml";
      edtfilt.clear();
      edtfilt << edtpatt;
      QStringList edtfiles =
          QDir(subdir).entryList(edtfilt, QDir::Files, QDir::Name);
      nedtf += edtfiles.size();
      aucpatt =
          tfilt ? filt_run + ".*" + filt_triple + ".auc" : filt_run + ".*.auc";
    }
  ...
  DbgLv(1) << "BrLoc:  naucd nedtf nmodf nnoif nstep" << naucd << nedtf << nmodf
           << nnoif << nstep;
  aucfilt.clear();
  edtfilt.clear();
  aucfilt << aucpatt;
  ...
  for (int ii = 0; ii < naucd; ii++) {  // loop thru potential data directories
    ...
    for (int jj = 0; jj < naucf;
         jj++) {  // loop thru .auc files found in a directory
      ...
      cdesc.filemodDate = US_Util::toUTCDatetimeText(
          QFileInfo(aucfile).lastModified().toUTC().toString(Qt::ISODate),
          true);
      cdesc.lastmodDate = "";
      ...
      for (int kk = 0; kk < edtfiles.size(); kk++) {
        ...
        cdesc.filemodDate = US_Util::toUTCDatetimeText(
            QFileInfo(edtfile).lastModified().toUTC().toString(Qt::ISODate),
            true);
        cdesc.lastmodDate = "";
  ...
  for (int ii = 0; ii < nmodf; ii++) {  // loop thru potential model files
    ...
    DbgLv(2) << "BrLoc:   mdl ii" << ii << "modfil" << modfil << "pGUID"
             << cdesc.parentGUID;
    ...
    cdesc.filemodDate = US_Util::toUTCDatetimeText(
        QFileInfo(modfil).lastModified().toUTC().toString(Qt::ISODate), true);
    cdesc.lastmodDate = "";
    if (cdesc.dataGUID.length() != 36 || cdesc.dataGUID == dmyGUID)
      cdesc.dataGUID = US_Util::new_guid();
  ...
  for (int ii = 0; ii < nnoif; ii++) {  // loop thru potential noise files
    ...
    cdesc.filemodDate = US_Util::toUTCDatetimeText(
        QFileInfo(noifil).lastModified().toUTC().toString(Qt::ISODate), true);
    cdesc.lastmodDate = "";
...
void US_DataModel::merge_dblocal() {
  ...
  DbgLv(1) << "MERGE: nd nl dlab llab" << nddes << nldes << descd.label
           << descl.label;
  ...
  while (jdr < nddes &&
         jlr < nldes) {  // main loop to merge records until one is exhausted
    ...
    while (descd.dataGUID ==
           descl.dataGUID) {  // records match in GUID:  merge them into one
      ...
      DbgLv(2) << "MERGE:  kar jdr jlr (1)GID" << kar << jdr << jlr
               << descd.dataGUID;
      kar++;
      ...
      if (++jdr < nddes)         // bump db count and test if done
        ...
        if (++jlr < nldes) descl = ldescs.at(jlr);  // get next local record
        break;
      }
    ...
    if (jdr >= nddes || jlr >= nldes) break;

    while (descd.recType >
           descl.recType) {  // output db records that are left-over children
      adescs << descd;
      DbgLv(2) << "MERGE:  kar jdr jlr (2)GID" << kar << jdr << jlr
               << descd.dataGUID;
      kar++;
    ...
    while (descl.recType >
           descd.recType) {  // output local records that are left-over children
      adescs << descl;
      DbgLv(2) << "MERGE:  kar jdr jlr (3)GID" << kar << jdr << jlr
               << descl.dataGUID;
      kar++;
    ...
    DbgLv(2) << "MERGE: rtype dlabel llabel" << descd.recType << dlabel
             << llabel;

    if (dlabel <
        llabel) {  // output db record first based on alphabetic label sort
      adescs << descd;
      DbgLv(2) << "MERGE:  kar jdr jlr (4)GID" << kar << jdr << jlr
               << descd.dataGUID;
      kar++;
      ...
      adescs << descl;
      DbgLv(2) << "MERGE:  kar jdr jlr (5)GID" << kar << jdr << jlr
               << descl.dataGUID;
      kar++;
  ...
  while (jdr < nddes) {
    ...
    DbgLv(2) << "MERGE:  kar jdr jlr (8)GID" << kar << jdr << jlr
             << descd.dataGUID;
  ...
  while (jlr < nldes) {
    ...
    DbgLv(2) << "MERGE:  kar jdr jlr (9)GID" << kar << jdr << jlr
             << descl.dataGUID;
    progress->setValue(++kar);
    qApp->processEvents();
  }
...
void US_DataModel::sort_descs(QVector<DataDesc>& descs) {
  ...
  for (int ii = 0; ii < nrecs;
       ii++) {  // build sort strings for Raw,Edit,Model,Noise; copy unsorted
                // vector
    desct = descs[ii];
  ...
  if (review_descs(sortr, tdess)) return;
  DbgLv(2) << "SrtD:  RD(r)" << nowTime();
  if (review_descs(sorte, tdess)) return;
  DbgLv(2) << "SrtD:  RD(e)" << nowTime();
  if (review_descs(sortm, tdess)) return;
  DbgLv(2) << "SrtD:  RD(m)" << nowTime();
  if (review_descs(sortn, tdess)) return;
  ...
  for (int ii = 0; ii < orphn.size();
       ii++) {  // for each orphan noise, create a dummy model
    dsorts = orphn.at(ii);
    ...
    if (dpGUID.length() < 2 ||
        dpGUID == dmyGUID) {  // handle case where there is no valid parentGUID
    ...
    if (guidsm.indexOf(dpGUID) >= 0) continue;

    if (dpGUID == dmyGUID) cdesc.label = "Dummy-Model-for-Orphans";
    ...
    cdesc.filemodDate = US_Util::toUTCDatetimeText(
        QDateTime::currentDateTime().toUTC().toString(Qt::ISODate), true);
  ...
  for (int ii = 0; ii < orphm.size();
       ii++) {  // for each orphan model, create a dummy edit
    dsorts = orphm.at(ii);
    dlabel = dsorts.section(":", 0, 0);
    dindex = dsorts.section(":", 1, 1);
    ddGUID = dsorts.section(":", 2, 2).simplified();
    ...
    // If this record is no longer an orphan, skip creating new parent
    if (guidse.indexOf(dpGUID) >= 0) continue;

    if (dpGUID == dmyGUID) cdesc.label = "Dummy-Edit-for-Orphans";
  ...
  for (int ii = 0; ii < orphe.size();
       ii++) {  // for each orphan edit, create a dummy raw
    dsorts = orphe.at(ii);
    dlabel = dsorts.section(":", 0, 0);
    dindex = dsorts.section(":", 1, 1);
    ddGUID = dsorts.section(":", 2, 2).simplified();
    ...
    if (dpGUID.length() <
        2) {          // handle case where there is no valid parentGUID
      if (ndmy == 0)  // first time:  create one
        dpGUID = dmyGUID;
      else
        dpGUID = ppGUID;  // afterwards:  re-use same parent
    ...
    // If this record is no longer an orphan, skip creating new parent
    if (guidsr.indexOf(dpGUID) >= 0) continue;

    if (dpGUID == dmyGUID) cdesc.label = "Dummy-Raw-for-Orphans";
  ...
  for (int ii = 0; ii < countR; ii++) {  // loop to output sorted Raw records
    ...
    for (int jj = 0; jj < countE;
         jj++) {  // loop to output sorted Edit records for the above Raw
      ...
      for (int mm = 0; mm < countM;
           mm++) {  // loop to output sorted Model records for above Edit
        ...
        for (int nn = 0; nn < countN;
             nn++) {  // loop to output sorted Noise records for above Model
  ...
  if (noutR != countR || noutE != countE || noutM != countM ||
      noutN != countN) {  // not all accounted for, so we will need some dummy
                          // parents
...
bool US_DataModel::review_descs(QStringList& sorts, QVector<DataDesc>& descv) {
  ...
  for (int ii = 1; ii < nrecs;
       ii++) {  // do a pass to determine if there are duplicate GUIDs
    ...
    if (jj >= 0) {
      kmult++;
      if (!multis.contains(
              jj)) {   // not yet marked, so mark previous as multiple
    ...
    if (kmult >
        0) {  // this pass found a duplicate:  save the index and bump count
      multis << ii;
      nmult++;
      DbgLv(1) << "RvwD: nmult" << nmult << "ii,jj" << ii << jj << "cGUID,pGUID"
               << cGUID << tGUIDs[jj];
    }
  ...
  if (nmult > 0) {  // there were multiple instances of the same GUID
    ...
    if (msgBox.exec() ==
        QMessageBox::Ok) {  // user wants details, so display them
      QString fileexts =
          tr("Text,Log files (*.txt *.log);;") + tr("All files (*)");
      QString pGUID = "";
      QString cGUID;
      QString label;
...
QStringList US_DataModel::list_orphans(QStringList& rlist, QStringList& plist) {
  ...
  for (int ii = 0; ii < rlist.size();
       ii++) {  // examine parentGUID for each record in the list
...
QString US_DataModel::sort_string(
    DataDesc ddesc, int indx) {  // create string for ascending sort on label
  QString label = (ddesc.recType < 3) ? ddesc.label : ddesc.description;
  int lablen = label.length();
  if (lablen < maxdlen) label = label.leftJustified(maxdlen, ' ');
...
void US_DataModel::review_dbase() {
  ...
  for (int row = 0; row < adescs.size();
       row++) {  // Review all records looking for changed DB records
    cdesc = adescs[row];

    if (chgrows.contains(
            row)) {  // Test whether to add to list if a changed row
      DbgLv(1) << "RvDB:   row state ID" << row << cdesc.recState
               << cdesc.recordID;
