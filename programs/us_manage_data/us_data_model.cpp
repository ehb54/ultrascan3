...
void
US_DataModel::setDatabase (US_DB2 *a_db)
{
...
void
US_DataModel::setProgress (QProgressBar *a_progr, QLabel *a_lbstat)
{
...
void
US_DataModel::setSiblings (QObject *a_proc, QObject *a_tree)
{
...
void
US_DataModel::getRunIDs (QStringList &runIDs, int &source)
{
  runIDs.clear ();
...
void
US_DataModel::getTriples (QStringList &triples, QString runID)
{
  triples.clear ();
...
void
US_DataModel::setFilters (QString a_runf, QString a_tripf, QString a_srcf)
{
...
US_DataModel::row_datadesc (int irow)
{
...
void
US_DataModel::change_datadesc (DataDesc ddesc, int row)
{
  adescs[row] = ddesc;
  cdesc = ddesc;
  chgrows << row;
...
void
US_DataModel::setCurrent (int irow)
{
...
US_DataModel::scan_dbase ()
{
      ...
      for (int ii = 0; ii < expIDs.size (); ii++)
        {
          ...
          DbgLv (2) << "  " << expID << expGUID << runID << label << comment
                    << date;
        }
    }
  ...
  for (int jq = 0; jq < nqry; jq++)
    {
      ...
      while (db->next ())
        { // Read Edit records
          ...
          irecID = recID.toInt ();
          DbgLv (2) << "BrDb: EDT id" << recID << " raID"
                    << db->value (3).toString () << " expID"
                    << db->value (4).toString ();
          ...
          contents = cksum + " " + recsize;
          DbgLv (2) << "BrDb:     edt  id eGID rGID label date" << irecID
                    << editGUID << rawGUID << label << date;
  ...
  for (int jq = 0; jq < nqry; jq++)
    {
              ...
              if (jmod >= 0)
                {
                  ...
                  DbgLv (2)
                      << "BrDb:     jmod" << jmod << " descript" << descript;
                }
            }
          // DbgLv(3) << "BrDb:
          // contents================================================";
      ...
      DbgLv (2) << "BrDb:  Noise IDs" << nowTime () << "size"
                << noiIDs.size ();
    }
  ...
  for (int ii = 0; ii < tmodels.size (); ii++)
    { // Change truncated model descriptions
      ...
      DbgLv (2) << "BrDb:   ii jdsc" << ii << jdsc << "dsc1"
                << cdesc.description << "dsc2" << descript;
      cdesc.description = descript;
      cdesc.label = label;
...
US_DataModel::scan_local ()
{
  ...
  QStringList modfils
      = QDir (dirm).entryList (modfilt, QDir::Files, QDir::Name);
  QStringList noifils
      = QDir (dirn).entryList (noifilt, QDir::Files, QDir::Name);
  int ktask = 0;
      ...
      for (int ii = 0; ii < naucd; ii++)
        {
          ...
          edtpatt = tfilt ? filt_run + ".*" + filt_triple + ".xml"
                          : filt_run + ".*.xml";
          edtfilt.clear ();
          edtfilt << edtpatt;
          ...
          nedtf += edtfiles.size ();
          aucpatt = tfilt ? filt_run + ".*" + filt_triple + ".auc"
                          : filt_run + ".*.auc";
        }

      DbgLv (1) << "BrLoc:  modf size" << modfils.size () << "filt_run"
                << filt_run;
  ...
  DbgLv (1) << "BrLoc:  naucd nedtf nmodf nnoif nstep" << naucd << nedtf
            << nmodf << nnoif << nstep;
  aucfilt.clear ();
  edtfilt.clear ();
  aucfilt << aucpatt;
      ...
      for (int jj = 0; jj < naucf; jj++)
        { // loop thru .auc files found in a directory
          ...
          cdesc.filemodDate = US_Util::toUTCDatetimeText (
              QFileInfo (aucfile).lastModified ().toUTC ().toString (
                  Qt::ISODate),
              true);
          cdesc.lastmodDate = "";
          ...
          for (int kk = 0; kk < edtfiles.size (); kk++)
            {
              ...
              // DbgLv(2) << "BrLoc:      (E)contents edtfile" << contents <<
              // edtfile;
              ...
              cdesc.filemodDate = US_Util::toUTCDatetimeText (
                  QFileInfo (edtfile).lastModified ().toUTC ().toString (
                      Qt::ISODate),
                  true);
              cdesc.lastmodDate = "";
  ...
  for (int ii = 0; ii < nmodf; ii++)
    { // loop thru potential model files
      ...
      cdesc.filemodDate = US_Util::toUTCDatetimeText (
          QFileInfo (modfil).lastModified ().toUTC ().toString (Qt::ISODate),
          true);
      cdesc.lastmodDate = "";
      if (cdesc.dataGUID.length () != 36 || cdesc.dataGUID == dmyGUID)
        cdesc.dataGUID = US_Util::new_guid ();
  ...
  for (int ii = 0; ii < nnoif; ii++)
    { // loop thru potential noise files
      ...
      cdesc.filemodDate = US_Util::toUTCDatetimeText (
          QFileInfo (noifil).lastModified ().toUTC ().toString (Qt::ISODate),
          true);
      cdesc.lastmodDate = "";
...
US_DataModel::merge_dblocal ()
{
  ...
  DbgLv (2) << " a/d/l sizes" << adescs.size () << ddescs.size ()
            << ldescs.size ();
...
void
US_DataModel::sort_descs (QVector<DataDesc> &descs)
{
  ...
  if (noutR != countR || noutE != countE || noutM != countM || noutN != countN)
    { // not all accounted for, so we will need some dummy parents
      DbgLv (1) << "sort_desc: count REMN" << countR << countE << countM
                << countN;
      DbgLv (1) << "sort_desc:   nout REMN" << noutR << noutE << noutM
                << noutN;
    }
}
...
bool
US_DataModel::review_descs (QStringList &sorts, QVector<DataDesc> &descv)
{
  bool abort = false;
...
int
US_DataModel::index_substring (QString ss, int ixs, QStringList &sl)
{
  QString sexp = "XXX";
  QRegExp rexp;
...
QStringList
US_DataModel::filter_substring (QString ss, int ixs, QStringList &sl)
{
  QStringList subl;
...
QStringList
US_DataModel::list_orphans (QStringList &rlist, QStringList &plist)
{
  QStringList olist;
  QStringList tlist;
...
int
US_DataModel::record_state_flag (DataDesc descr, int pstate)
{
...
QString
US_DataModel::sort_string (DataDesc ddesc, int indx)
...
QString
US_DataModel::model_type (int imtype, int nassoc, int gtype, bool isMC)
{
  QString mtype;
...
// compose string describing model type
QString
US_DataModel::model_type (US_Model model)
{
...
// compose string describing model type
QString
US_DataModel::model_type (QString modxml)
{
  QChar quo ('"');
  int jj;
  int imtype;
...
QString
US_DataModel::expGUIDauc (QString aucfile)
{
    ...
    { // Review all records looking for changed DB records
      ...
      if (chgrows.contains (row))
        { // Test whether to add to list if a changed row
          ...
          DbgLv (1) << "RvDB:     ++ upd cdesc, row state" << row
                    << cdesc.recState;
        }
