...
US_ProtocolDevMain::US_ProtocolDevMain() : US_Widgets() {
   ...
   tabWidget->tabBar()->setStyleSheet(
      "QTabBar::tab {min-width: 70;} QTabBar::tab:selected {background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, "
      "stop: 0 #fafafa, stop: 0.4 #f4f4f4, stop: 0.5 #e7e7e7, stop: 1.0 #fafafa); } QTabBar::tab:first {background: "
      "blue; color: lightgray; min-width: 50;}  QTabBar::tab:first:hover {background: #4169E1; color: white}  "
   ...
   connect(
      epanInit, SIGNAL(define_new_experiment_init(QMap<QString, QString> &)), this,
      SLOT(define_new_experiment(QMap<QString, QString> &)));
   connect(
      epanInit, SIGNAL(switch_to_live_update_init(QMap<QString, QString> &)), this,
      SLOT(switch_to_live_update(QMap<QString, QString> &)));
   connect(
      epanInit, SIGNAL(switch_to_post_processing_init(QMap<QString, QString> &)), this,
      SLOT(switch_to_post_processing(QMap<QString, QString> &)));
   connect(
      epanInit, SIGNAL(switch_to_editing_init(QMap<QString, QString> &)), this,
      SLOT(switch_to_editing(QMap<QString, QString> &)));
   connect(
      epanInit, SIGNAL(switch_to_analysis_init(QMap<QString, QString> &)), this,
      SLOT(switch_to_analysis(QMap<QString, QString> &)));
   connect(
      epanInit, SIGNAL(switch_to_report_init(QMap<QString, QString> &)), this,
      SLOT(switch_to_report(QMap<QString, QString> &)));
   connect(epanInit, SIGNAL(to_initAutoflow()), this, SLOT(close_all()));

   connect(
      this, SIGNAL(pass_used_instruments(QMap<QString, QString> &)), epanExp,
   ...
   connect(
      epanExp, SIGNAL(switch_to_live_update(QMap<QString, QString> &)), this,
      SLOT(switch_to_live_update(QMap<QString, QString> &)));
   connect(
      this, SIGNAL(pass_to_live_update(QMap<QString, QString> &)), epanObserv,
   ...
   connect(
      epanObserv, SIGNAL(switch_to_post_processing(QMap<QString, QString> &)), this,
      SLOT(switch_to_post_processing(QMap<QString, QString> &)));
   connect(
      this, SIGNAL(pass_to_post_processing(QMap<QString, QString> &)), epanPostProd,
   ...
   connect(
      epanPostProd, SIGNAL(switch_to_editing(QMap<QString, QString> &)), this,
      SLOT(switch_to_editing(QMap<QString, QString> &)));
   connect(
      this, SIGNAL(pass_to_editing(QMap<QString, QString> &)), epanEditing, SLOT(do_editing(QMap<QString, QString> &)));
   ...
   connect(
      epanEditing, SIGNAL(switch_to_analysis(QMap<QString, QString> &)), this,
      SLOT(switch_to_analysis(QMap<QString, QString> &)));
   connect(
      this, SIGNAL(pass_to_analysis(QMap<QString, QString> &)), epanAnalysis,
   ...
   connect(
      epanAnalysis, SIGNAL(switch_to_report(QMap<QString, QString> &)), this,
      SLOT(switch_to_report(QMap<QString, QString> &)));
...
void US_ProtocolDevMain::checkDataLocation(void) {
   ...
   if (US_Settings::default_data_location() == 2) //Disk
   {
      ...
      //window_closed = true;
      QMessageBox::warning(
         this, tr("Data Location set to Disk"),
...
// Slot to define new exp. (from initial dialog)
void US_ProtocolDevMain::define_new_experiment(QMap<QString, QString> &protocol_details) {
...
void US_ProtocolDevMain::switch_to_analysis(QMap<QString, QString> &protocol_details) {
...
void US_ProtocolDevMain::call_AutoflowDialogue(void) //<--- Entry point from main
{
...
US_InitDialogueGui::US_InitDialogueGui(QWidget *topw) : US_WidgetsDialog(topw, 0) {
...
//Dialog to display all autoflowHistory runs && select one, passing protocol name && invID to US_ExpGUI
void US_InitDialogueGui::load_autoflowHistory_dialog(void) {
   ...
   //else if ( pdiag_autoflowHistory->exec() == QDialog::Rejected )
      ...
      qDebug() << "Closing main pdiag_autoflow dialog && rebuilding: initAutoflowPanel() -> initRecordsDialog() -> "
...
void US_InitDialogueGui::initRecordsDialogue(void) {
   ...
   if (autoflow_records < 1) {
      ...
      msg_norec->setText(tr("There are no PROTOCOL DEVELOPMENT runs to follow.<br><br>"
                            "You will be directed to the dialog where you can select one of the completed runs "
   ...
   qDebug() << "occupied_instruments.size(), instruments.size()" << occupied_instruments.size() << ", "
            << instruments.size();
   ...
   qDebug() << "failedID: str, INT --  " << protocol_details[ "failedID" ] << protocol_details[ "failedID" ].toInt();
   ...
   if (failedID.toInt() != 0) {
      ...
      msgBox_f.setText(
         tr("The selected GMP run is marked as FAILED:<br><br>") + tr("<b>Run Name:&emsp;</b>") + ProtName + tr("<br>")
         + tr("<b>Failing stage:&emsp;</b> ") + failed_details[ "failedStage" ] + tr("<br>")
         + tr("<b>Reason:&emsp;</b> ") + failed_details[ "failedMsg" ]);

      msgBox_f.setInformativeText(
         tr("<font color='red'><b>ATTENTION:</b></font> If you choose to Procceed, all existing data for this run will "
            "be deleted from DB, ")
...
void US_InitDialogueGui::do_run_tables_cleanup(QMap<QString, QString> run_details) {
   ...
   qry.clear();
   qry << "delete_autoflow_intensity_record" << run_details[ "autoflowID" ] << run_details[ "intensityID" ];
   db->query(qry);

   //deleting autoflowStatus Record
   qry.clear();
   qry << "delete_autoflow_status_record" << run_details[ "autoflowID" ] << run_details[ "statusID" ];
   db->query(qry);

   //deleting autoflowFailed Record
   qry.clear();
   qry << "delete_autoflow_failed_record" << run_details[ "autoflowID" ] << run_details[ "failedID" ];
   db->query(qry);
   ...
   for (int i = 0; i < analysisIDs_list.size(); ++i) {
      ...
      qry.clear();
      qry << "update_autoflow_analysis_record_at_deletion" << QString("Canceled for Failed run, top-level")
          << requestID;
   ...
   qry.clear();
   qry << "delete_autoflow_analysis_history_records_by_autoflowID" << run_details[ "autoflowID" ];
   db->query(qry);

   //delete autoflowModelsLink records for given autoflowID :
   qry.clear();
   qry << "delete_autoflow_model_links_records_by_autoflowID" << run_details[ "autoflowID" ];
   db->query(qry);
...
void US_InitDialogueGui::do_create_autoflowStatus_for_failedRun(QMap<QString, QString> run_details) {
   ...
   qry.clear();
   qry << "new_autoflowStatusGMPCreate_record" << run_details[ "autoflowID" ] << createGMPRun_Json;
   ...
   qry.clear();
   qry << "update_autoflow_with_statusID" << run_details[ "autoflowID" ] << QString::number(autoflowStatusID);
...
void US_InitDialogueGui::do_run_data_cleanup(QMap<QString, QString> run_details) {
   ...
   for (int i = 0; i < fileNameList.size(); ++i) {
      ...
      qry << "get_experiment_info_by_runID" << fileNameList[ i ] << run_details[ "invID_passed" ];
...
void US_InitDialogueGui::update_autoflow_data(void) {
   ...
   if (autoflowdata.size() < 1) {
      ...
      qDebug() << "DELETION of LAST autoflow record: CLOSING pdiag_autoflow dialog && trigering ::initAutolfowPanel() "
      ...
      msg_norec_del->setText(tr("There are no PROTOCOL DEVELOPMENT runs to follow.<br><br>"
                                "You will be directed to the dialog where you can select one of the completed runs "
                                "and define an updated protocol for that run."));
...
int US_InitDialogueGui::list_all_autoflow_records(QList<QStringList> &autoflowdata, US_DB2 *dbP, QString type) {
   int nrecs = 0;
   autoflowdata.clear();
...
// Query autoflow for # records
QMap<QString, QString> US_InitDialogueGui::read_autoflow_record(int autoflowID, QString type) {
   // Check DB connection
   US_Passwd pw;
   QString masterpw = pw.getPasswd();
   US_DB2 *db = new US_DB2(masterpw);
...
US_ExperGui::US_ExperGui(QWidget *topw) : US_WidgetsDialog(topw, 0) {
   ...
   connect(
      this, SIGNAL(define_used_instruments(QMap<QString, QString> &)), sdiag,
   ...
   connect(sdiag, SIGNAL(to_live_update(QMap<QString, QString> &)), this, SLOT(to_live_update(QMap<QString, QString> &)));
...
void US_ExperGui::manageExperiment() {
   ...
   new_height = mainw->gen_banner->height() + mainw->welcome->height() + height_cum + sdiag->height()
                + mainw->logWidget->height() + mainw->test_footer->height() + offset;
...
US_ObservGui::US_ObservGui(QWidget *topw) : US_WidgetsDialog(topw, 0) {
   ...
   connect(
      sdiag, SIGNAL(experiment_complete_auto(QMap<QString, QString> &)), this,
      SLOT(to_post_processing(QMap<QString, QString> &)));
...
US_PostProdGui::US_PostProdGui(QWidget *topw) : US_WidgetsDialog(topw, 0) {
   ...
   connect(
      sdiag, SIGNAL(saving_complete_auto(QMap<QString, QString> &)), this, SLOT(to_editing(QMap<QString, QString> &)));
...
US_AnalysisGui::US_AnalysisGui(QWidget *topw) : US_WidgetsDialog(topw, 0) {
   ...
   connect(
      sdiag, SIGNAL(analysis_complete_auto(QMap<QString, QString> &)), this, SLOT(to_report(QMap<QString, QString> &)));
...
// US_Report
US_ReportStageGui::US_ReportStageGui(QWidget *topw) : US_WidgetsDialog(topw, 0) {
   mainw = ( US_ProtocolDevMain * ) topw;
