...
// Main Constructor
US_ProtocolDevMain::US_ProtocolDevMain () : US_Widgets ()
{
  ...
  setWindowTitle (tr ("UltraScan Protocol Development Module"));
  ...
  gen_banner
      = us_banner (tr ("UltraScan Protocol Development Module, v. 0.1"));
  ...
  // welcome->setText(" <br> Welcome to UltraScan GMP<br> for setting up,
  // monitoring, editing and analyzing <br> AUC experiments and produced
  // data... <br>");
  ...
  connect (epanInit,
           SIGNAL (define_new_experiment_init (QMap<QString, QString> &)),
           this, SLOT (define_new_experiment (QMap<QString, QString> &)));
  connect (epanInit,
  ...
  connect (this, SIGNAL (pass_used_instruments (QMap<QString, QString> &)),
           epanExp, SLOT (pass_used_instruments (QMap<QString, QString> &)));
  // connect( epanExp, SIGNAL( switch_to_editing( QMap < QString, QString > & )
...
void
US_ProtocolDevMain::checkDataLocation (void)
{
...
void
US_ProtocolDevMain::initPanels (int panx)
{
...
void
US_ProtocolDevMain::show_liveupdate_finishing_msg (void)
{
...
//
void
US_ProtocolDevMain::show_analysis_update_finishing_msg (void)
{
...
void
US_ProtocolDevMain::liveupdate_stopped (void)
{
...
void
US_ProtocolDevMain::analysis_update_stopped (void)
{
...
void
US_ProtocolDevMain::closeEvent (QCloseEvent *event)
{
  window_closed = true;
...
void
US_ProtocolDevMain::to_autoflow_records (void)
{
  // check_current_stage();
}

void
US_ProtocolDevMain::close_initDialogue (void)
{
...
// Slot to define new exp. (from initial dialog)
void
US_ProtocolDevMain::define_new_experiment (
...
// Slot to pass submitted to Optima run info to the Live Update tab
void
US_ProtocolDevMain::switch_to_live_update (
    QMap<QString, QString> &protocol_details)
{
...
void
US_ProtocolDevMain::switch_to_post_processing (
    QMap<QString, QString> &protocol_details)
{
...
void
US_ProtocolDevMain::switch_to_editing (
    QMap<QString, QString> &protocol_details)
{
...
// Slot to switch to Analysis tab
void
US_ProtocolDevMain::switch_to_analysis (
    QMap<QString, QString> &protocol_details)
{
...
void
US_ProtocolDevMain::switch_to_report (QMap<QString, QString> &protocol_details)
{
...
void
US_ProtocolDevMain::call_AutoflowDialogue (void) //<--- Entry point from main
{
...
US_InitDialogueGui::initRecords (void) // <-- 1st entry point !!
{
  ...
  list_all_autoflow_records (autoflowdata, dbP, "DEV");
...
// Dialog to display all autoflowHistory runs && select one, passing protocol
// name && invID to US_ExpGUI
void
US_InitDialogueGui::load_autoflowHistory_dialog (void)
{
  US_Passwd pw;
  US_DB2 *dbP = new US_DB2 (pw.getPasswd ());

  list_all_autoflow_records (autoflowdataHistory, dbP, "HISTORY");

  QString pdtitle (tr ("Select Completed Autoflow Run"));
  QStringList hdrs;
  int prx;
  hdrs << "ID"
       << "Run Name"
  ...
  pdiag_autoflowHistory = new US_SelectItem (autoflowdataHistory, hdrs,
                                             pdtitle, &prx, autoflow_btn, -3);
  ...
  // else if ( pdiag_autoflowHistory->exec() == QDialog::Rejected )
    ...
    {
      qDebug () << "Closing main pdiag_autoflow dialog && rebuilding: "
                   "initAutoflowPanel() -> initRecordsDialog() -> "
  ...
  protocol_details = read_autoflow_record (autoflowID, "HISTORY");
  ...
  qDebug () << "Protocol Name: " << protocol_details["protocolName"];
  qDebug () << "InvID: " << protocol_details["invID_passed"];
  ...
  qDebug () << "aprofileguid: " << protocol_details["aprofileguid"];

  emit define_new_experiment_init (protocol_details);
}
...
US_InitDialogueGui::initRecordsDialogue (void)
{
  ...
  if (autoflow_records < 1)
    {
      ...
      msg_norec->setText (
          tr ("There are no PROTOCOL DEVELOPMENT runs to follow.<br><br>"
              "You will be directed to the dialog where you can select one of "
              "the completed runs "
              "and define an updated protocol for that run."));
  ...
  if (pdiag_autoflow->exec () == QDialog::Accepted)
  ...
  else
    {
      ...
      if (occupied_instruments.size () == 0)
        {
          ...
          // HERE; check if there are still autoflow_records & it's not a
          // spurious reinitialization of autoflowHistoryDalog!!
          int actual_autoflow_records = get_autoflow_records ();
          qDebug () << "Actual Autoflow_Reconds #: "
                    << actual_autoflow_records;
      ...
      // else
      // 	{
      // 	  QString list_instruments_in_use =
      // occupied_instruments.join(", ");

      // 	  QMessageBox * msg_instr_use = new QMessageBox(this);
      // 	  msg_instr_use->setIcon(QMessageBox::Information);
      // 	  msg_instr_use->setText(tr( "The following Optima
      // instrument(s)<br>" 				       "are currently in use: <br><br>"
      // 				       "<b>%1</b> <br><br>"
      // 				       "You will not be able to submit
      // another run<br>" 				       "to these instruments at the moment." ) 				   .arg(
      // list_instruments_in_use ) );
  ...
  int autoflowID = autoflow_id_selected.toInt ();
  protocol_details = read_autoflow_record (autoflowID, "DEV");
  ...
  if (failedID.toInt () != 0)
    {
      ...
      msgBox_f.setInformativeText (
          tr ("<font color='red'><b>ATTENTION:</b></font> If you choose to "
              "Procceed, all existing data for this run will be deleted from "
              "DB, ")
  ...
  if (autoflowdata.size () < 1)
    {
      ...
      pdiag_autoflow->close (); // HERE - it triggers ::initAutolfowPanel() ->
                                // ::load_autoflowHistrory_dialog()!!!
      qDebug () << "DELETION of LAST autoflow record: CLOSING pdiag_autoflow "
                   "dialog && trigering ::initAutolfowPanel() -> "
                   "::load_autoflowHistrory_dialog()!!!";
      qApp->processEvents ();
      ...
      if (msg_norec_del->exec () == QMessageBox::Ok)
        {
          ...
          qDebug () << "AFTER DELETION of LAST autoflow record: Calling "
                       "load_autoflowHistory_dialog() -- ";
          load_autoflowHistory_dialog ();
...
US_InitDialogueGui::list_all_autoflow_records (
    QList<QStringList> &autoflowdata, US_DB2 *dbP, QString type)
{
  int nrecs = 0;
  autoflowdata.clear ();
      ...
      if (type == "HISTORY")
        {
          ...
          // autoflowentry << id << history_runname << optimaname  <<
      ...
      if (US_Settings::us_inv_level () < 3)
        {
          ...
          if (user_id
              && (user_id == operatorID.toInt () || user_id == invID.toInt ()))
            {
...
US_InitDialogueGui::get_autoflow_records (void)
{
  ...
  QStringList qry;
  qry << "count_autoflow_dev_records";
...
US_InitDialogueGui::read_autoflow_record (int autoflowID, QString type)
{
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd ();
...
US_ExperGui::US_ExperGui (QWidget *topw) : US_WidgetsDialog (topw, 0)
{
  ...
  connect (this, SIGNAL (define_used_instruments (QMap<QString, QString> &)),
           sdiag,
           SLOT (accept_passed_protocol_details (QMap<QString, QString> &)));
...
void
US_ExperGui::pass_used_instruments (QMap<QString, QString> &protocol_details)
{
  qDebug () << "In US_ExperGui::pass_used_instruments( QMap < QString, "
               "QString > & protocol_details )";
