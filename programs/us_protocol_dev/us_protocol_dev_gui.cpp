...
US_ProtocolDevMain::US_ProtocolDevMain() : US_Widgets() {
  ...
  // monitoring, editing and analyzing <br> AUC experiments and produced data...
  // <br>");
  ...
  connect(epanInit, SIGNAL(define_new_experiment_init(QMap<QString, QString>&)),
  ...
  connect(this, SIGNAL(pass_used_instruments(QMap<QString, QString>&)), epanExp,
          SLOT(pass_used_instruments(QMap<QString, QString>&)));
...
void US_ProtocolDevMain::checkDataLocation(void) {
...
void US_ProtocolDevMain::initPanels(int panx) {
...
void US_ProtocolDevMain::closeEvent(QCloseEvent* event) {
  window_closed = true;
...
void US_ProtocolDevMain::to_autoflow_records(void) {
  // check_current_stage();
}

void US_ProtocolDevMain::close_initDialogue(void) {
...
// Slot to define new exp. (from initial dialog)
void US_ProtocolDevMain::define_new_experiment(
...
void US_ProtocolDevMain::close_all(void) {
...
// Slot to pass submitted to Optima run info to the Live Update tab
void US_ProtocolDevMain::switch_to_live_update(
    QMap<QString, QString>& protocol_details) {
...
void US_ProtocolDevMain::switch_to_post_processing(
    QMap<QString, QString>& protocol_details) {
...
void US_ProtocolDevMain::switch_to_editing(
    QMap<QString, QString>& protocol_details) {
...
void US_ProtocolDevMain::switch_to_analysis(
    QMap<QString, QString>& protocol_details) {
...
void US_ProtocolDevMain::switch_to_report(
    QMap<QString, QString>& protocol_details) {
...
void US_ProtocolDevMain::call_AutoflowDialogue(
...
void US_InitDialogueGui::load_autoflowHistory_dialog(void) {
  US_Passwd pw;
  US_DB2* dbP = new US_DB2(pw.getPasswd());
  ...
  pdiag_autoflowHistory = new US_SelectItem(autoflowdataHistory, hdrs, pdtitle,
                                            &prx, autoflow_btn, -3);
...
void US_InitDialogueGui::initRecordsDialogue(void) {
  ...
  if (autoflow_records < 1) {
    ...
    msg_norec->setText(
        tr("There are no PROTOCOL DEVELOPMENT runs to follow.<br><br>"
           "You will be directed to the dialog where you can select one of the "
           "completed runs "
  ...
  if (pdiag_autoflow->exec() == QDialog::Accepted) {
    ...
    if (occupied_instruments.size() == 0) {
      ...
      // HERE; check if there are still autoflow_records & it's not a spurious
      // reinitialization of autoflowHistoryDalog!!
      ...
      if (actual_autoflow_records > 0) load_autoflowHistory_dialog();
    ...
    // run<br>" 				       "to these instruments at the moment." ) 				   .arg(
...
int US_InitDialogueGui::list_all_autoflow_records(
  ...
  if (type == "DEV") qry << "get_autoflow_dev_desc";
  if (type == "HISTORY") qry << "get_autoflow_history_desc";
  ...
  while (dbP->next()) {
    ...
    if (type == "HISTORY") {
      ...
      autoflowentry << id << history_runname << optimaname << ptime_created;
    }
    if (type == "DEV") {
...
QMap<QString, QString> US_InitDialogueGui::read_autoflow_record(int autoflowID,
  ...
  if (type == "DEV") qry << "read_autoflow_record";
  if (type == "HISTORY") qry << "read_autoflow_history_record";
...
US_ExperGui::US_ExperGui(QWidget* topw) : US_WidgetsDialog(topw, 0) {
  ...
  connect(this, SIGNAL(define_used_instruments(QMap<QString, QString>&)), sdiag,
...
void US_ExperGui::pass_used_instruments(
    QMap<QString, QString>& protocol_details) {
  qDebug() << "In US_ExperGui::pass_used_instruments( QMap < QString, QString "
              "> & protocol_details )";
