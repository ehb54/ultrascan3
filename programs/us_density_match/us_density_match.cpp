...
bool distro_lessthan(const S_Solute &solu1, const S_Solute &solu2) { // TRUE iff  (s1<s2) || (s1==s2 && d1<d2)
...
US_Density_Match::US_Density_Match() : US_Widgets() {
   ...
   connect(bg_di_avg, SIGNAL(buttonReleased(int)), this, SLOT(select_di_avg(int)));
   ...
   connect(ct_boundaryPct, SIGNAL(valueChanged(double)), this, SLOT(update_divis(double)));
   connect(ct_boundaryPos, SIGNAL(valueChanged(double)), this, SLOT(update_divis(double)));
   connect(ct_smoothing, SIGNAL(valueChanged(double)), this, SLOT(update_divis(double)));
   ...
   QBoxLayout *plot = new US_Plot(data_plot, tr("Density Matching Data"), xa_title, ya_title);
...
void US_Density_Match::plot_data(void) {
   ...
   if (ncurvs == 1) {
      ...
      DbgLv(1) << "DaPl:   npoint" << npoint << "xx" << xx[ 0 ] << xx[ npoint - 1 ] << "yy" << yy[ 0 ]
               << yy[ npoint - 1 ];
   ...
   QColor lncolr1[] = { QColor(Qt::blue), QColor(Qt::red),    QColor(Qt::green), QColor(Qt::magenta),
                        QColor(Qt::cyan), QColor(Qt::yellow), QColor(Qt::gray),  QColor(Qt::black) };
...
void US_Density_Match::load_distro(US_Model model, QString mdescr) {
   ...
   if (dkdb_cntrls->db()) { // Set up to read from database
      ...
      QString rpath = edir + "/" + efname.section(".", 0, -7) + "." + efname.section(".", -5, -2) + ".auc";
   ...
   DbgLv(1) << "LD:  edata: desc run cell chan" << edata.description << edata.runID << edata.cell << edata.channel;
   ...
   US_SolutionVals::values(dbP, &edata, soluID, cvbar20, bdens, bvisc, cmprss, bmanu, errmsg);
   tsys.bdensity = bdens.toDouble();
   DbgLv(1) << "LD:  solval: bdens soluID bdensity" << bdens << soluID << tsys.bdensity;
   ...
   for (int jj = 0; jj < nsolmc; jj++) {
      ...
      tot_conc += sol_in.c;
      if (jj < 3 || (jj + 4) > nsolmc)
         DbgLv(1) << "LD:    jj" << jj << "soli s,d,c,t" << sol_in.s << sol_in.d << sol_in.c << tot_conc;
   }
   ...
   for (int jj = 0; jj < nsolnm; jj++) {
      ...
      if (jj < 3 || (jj + 4) > nsolnm)
         DbgLv(1) << "LD:    jj" << jj << "soln s,d,c" << sol_nm.s << sol_nm.d << sol_nm.c;
   }
   ...
   for (int jj = 0; jj < nsolnm; jj++) {
      ...
      if (jj < 3 || (jj + 4) > nsolnm)
         DbgLv(1) << "LD:    jj" << jj << "solb s,d,c,f" << sol_bf.s << sol_bf.d << sol_bf.c << sol_bf.f;
   }
...
void US_Density_Match::sort_distro(QList<S_Solute> &listsols, bool reduce) {
   ...
   if (reduce) { // skip any duplicates in sorted list
      ...
      while ((++jj) != listsols.end()) { // loop to compare each entry to previous
         ...
         if (sol1.s != sol2.s || sol1.d != sol2.d) { // not a duplicate, so output to temporary list
            reduced.append(sol2);
            jdup = 0;
         }
...
void US_Density_Match::set_mparms(void) {
   ...
   if (mpdiag.exec() == QDialog::Accepted) { // Redo text box summarizing models; calculate vectors
...
void US_Density_Match::build_bf_distro(int modx) {
   ...
   DbgLv(1) << "BldBf: bfrac bfextn bfincr" << bfrac << bfextn << bfincr << "nsolbo nsolbf" << nsolbo << nsolbf
            << (bfextn / bfincr) << ksolbf;
   int j2 = 0;

   // Create solute points with specified boundary fraction extent
   for (int kk = 0; kk < nsolbf; kk++) {
      while (j2 < nsolbo) { // Position boundary fraction within input distribution
         if (bfrac <= tsys->bo_distro[ j2 ].f || j2 == (nsolbo - 1))
            break;
         j2++;
      }
      ...
      if (kk < 3 || (kk + 4) > nsolbf || ((kk2 > (nsolbf - 4)) && (kk2 < nsolbf + 4)))
         DbgLv(1) << "BldBf:  kk bfrac" << kk << bfrac << "j2" << j2 << "f s d" << sol_bf.f << sol_bf.s << sol_bf.d;
...
void US_Density_Match::build_bf_dists() {
   ...
   if (diff_avg > 0) { // Replace diffusion coefficients with simple or weighted averages
      int npoints = alldis[ 0 ].bf_distro.size();
      for (int jj = 0; jj < npoints; jj++) {
         double dsum = 0.0;
         double wsum = 0.0;
         for (int ii = 0; ii < ndists; ii++) { // Accumulate (weighted) sum and sum of weights
...
void US_Density_Match::build_bf_vects() {
   ...
   for (int jj = 0; jj < npoints; jj++) { // Grab each fraction from 1st model's distro
      v_bfracs << alldis[ 0 ].bf_distro[ jj ].f;
   }
   DbgLv(1) << "BldVc: bf 0 1 k n" << v_bfracs[ 0 ] << v_bfracs[ 1 ] << v_bfracs[ npoints - 2 ]
            << v_bfracs[ npoints - 1 ];
   ...
   for (int ii = 0; ii < ndists; ii++) {
      ...
      for (int jj = 0; jj < npoints; jj++) { // Append s,D values for each bf-point for this model
      ...
      DbgLv(1) << "BldVc:     di 0 1 k n" << v_difcs[ ii ][ 0 ] << v_difcs[ ii ][ 1 ] << v_difcs[ ii ][ npoints - 2 ]
               << v_difcs[ ii ][ npoints - 1 ];
   ...
   for (int jj = 0; jj < npoints; jj++) {
      v_seds.clear();
      for (int ii = 0; ii < ndists; ii++) { // Build sed coeffs vector across models
   ...
   DbgLv(1) << "BldVc: vb 0 1 k n" << v_vbars[ 0 ] << v_vbars[ 1 ] << v_vbars[ npoints - 2 ] << v_vbars[ npoints - 1 ];
   ...
   DbgLv(1) << "BldVc: mm 0 1 k n" << v_mmass[ 0 ] << v_mmass[ 1 ] << v_mmass[ npoints - 2 ] << v_mmass[ npoints - 1 ];
   ...
   DbgLv(1) << "BldVc: hr 0 1 k n" << v_hrads[ 0 ] << v_hrads[ 1 ] << v_hrads[ npoints - 2 ] << v_hrads[ npoints - 1 ];
...
//      double fzero     = 6.0 * M_PI * VISC_20W * rzero;
   ...
   DbgLv(1) << "BldVc: fr 0 1 k n" << v_frats[ 0 ] << v_frats[ 1 ] << v_frats[ npoints - 2 ] << v_frats[ npoints - 1 ];
...
void US_Density_Match::write_csv(
   const QString fpath, const QString hdr1, QVector<double> &vals1, const QString hdr2, QVector<double> &vals2) {
   ...
   for (int jj = 0; jj < vals1.size(); jj++) {
      QString line = QString().sprintf("\"%9.6e\",\"%6.4f\"\n", vals1[ jj ], vals2[ jj ]);
      line.replace(" ", "");
      ts << line;
   }
...
void US_Density_Match::models_summary(void) {
   ...
   for (int jj = 0; jj < alldis.size(); jj++) { // Compose and display a distribution line for distro info box
      ...
      dinfo += QString().sprintf("%.1f  %f  ", d2opct, bdens) + mlab + "  " + mdesc + "\n";
   }
