...
US_Density_Match::US_Density_Match () : US_Widgets ()
{
  ...
  rb_da_s->setToolTip (tr (
      "Do a SIMPLE average of Diffusion Coefficients across distributions"));
  ...
  QBoxLayout *plot = new US_Plot (data_plot, tr ("Density Matching Data"),
                                  xa_title, ya_title);
...
// Plot the data
void
US_Density_Match::plot_data (void)
{
...
void
US_Density_Match::plot_data (int cplx)
{
  plot_x = cplx;
  plot_data ();
...
void
US_Density_Match::load_distro (US_Model model, QString mdescr)
{
  DisSys tsys;
  S_Solute sol_in;
  S_Solute sol_nm;
...
int
US_Density_Match::plot_x_select ()
{
  int plotx = ATTR_S;
...
void
US_Density_Match::sort_distro (QList<S_Solute> &listsols, bool reduce)
{
...
void
US_Density_Match::update_disk_db (bool isDB)
{
...
void
US_Density_Match::select_prefilt (void)
{
  QString pfmsg;
  QString dinfo;
  int nruns = 0;
...
void
US_Density_Match::remove_distro (void)
{
...
void
US_Density_Match::set_mparms (void)
{
...
void
US_Density_Match::select_x_axis (int ival)
{
...
void
US_Density_Match::select_di_avg (int ival)
{
...
void
US_Density_Match::build_bf_distro (int modx)
{
  if (alldis.size () <= modx)
    return;
...
void
US_Density_Match::build_bf_dists ()
{
...
void
US_Density_Match::build_bf_vects ()
{
...
QString
US_Density_Match::anno_title (int pltndx)
{
  QString a_title;
...
QString
US_Density_Match::ptype_text (int pltndx)
{
  QString t_text;
...
void
US_Density_Match::update_divis (double dval)
{
  DbgLv (1) << "UpdDiv:" << dval;
  build_bf_dists (); // (Re-)build boundary fraction distributions
  build_bf_vects (); // (Re-)build boundary fraction vectors
...
void
US_Density_Match::write_csv (const QString fpath, const QString hdr1,
...
void
US_Density_Match::models_summary (void)
{
  ...
  for (int jj = 0; jj < alldis.size (); jj++)
    { // Compose and display a distribution line for distro info box
      ...
      DbgLv (1) << "mosmry:  jj" << jj << "d2opct bdens mlab" << d2opct
                << bdens << mlab;
