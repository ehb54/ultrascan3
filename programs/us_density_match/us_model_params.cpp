...
US_ModelParams::US_ModelParams(QVector<DisSys> &adistros, QWidget *p) : US_WidgetsDialog(p, 0), distros(adistros) {
   ...
   for (int jj = 0; jj < nd_orig; jj++) {
      ...
      connect(le_d2op, SIGNAL(textChanged(const QString &)), this, SLOT(lnedChanged(const QString &)));
      connect(le_dens, SIGNAL(textChanged(const QString &)), this, SLOT(lnedChanged(const QString &)));
      connect(le_mlab, SIGNAL(textChanged(const QString &)), this, SLOT(lnedChanged(const QString &)));
   }
   ...
   for (int ii = 0; ii < 4; ii++) { // Adjust each column
      int kk = ii;
      int ll = kk;
      for (int jj = 1; jj < nd_orig; jj++) { // Each box connected to next row's box
...
void US_ModelParams::accepted() {
   ...
   for (int jj = 0; jj < ochilds.size(); jj++) {
      ...
      if (pname == "D2OP") {
         ...
         DbgLv(1) << " acc:  jj rowx" << jj << rowx << "pname value" << pname << lned->text().toDouble();
      }
      else if (pname == "DENS") {
         ...
         DbgLv(1) << " acc:  jj rowx" << jj << rowx << "pname value" << pname << lned->text();
      }
   }
...
void US_ModelParams::compute_densities() {
   ...
   for (int jj = 0; jj < ochilds.size(); jj++) {
      ...
      if (pname == "D2OP") { // Handle D2O percent text
         ...
         else if (nzd2pc > 0) { // Compute density where percent!=0; update text box
   ...
   if (misspc) { // Fill in computed densities before first pct=0
      for (int ii = 0; ii < nd_orig; ii++) {
         density = m_dens[ ii ];
         if (density == 0.0) { // Compute density when unable to do so during 1st pass
            d2opct = m_d2op[ ii ];
            density = denszpc + d2opct * dnslope;
...
bool US_ModelParams::values_filled() {
   ...
   DbgLv(1) << " vlf: nzd2pc nad2pc nadens nalabs" << nzd2pc << nad2pc << nadens << nalabs << "allden size"
            << allden.size() << "nd_orig" << nd_orig;
   ...
   bool filled
      = ((nzd2pc > 0) && (nad2pc == nd_orig) && (nadens == nd_orig) && (nalabs == nd_orig) && (allden.size() > 1));
...
bool US_ModelParams::all_percents() {
   ...
   for (int jj = 0; jj < ochilds.size(); jj++) { // Examine any D2O percent texts
      QObject *ochild = ochilds[ jj ];
      QString cname = ochild->objectName();
      if (cname.isEmpty())
         continue;
