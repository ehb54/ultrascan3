...
void
US_ModelParams::compute_densities ()
{
...
US_ModelParams::values_filled ()
{
              ...
              if (pctval == 0.0)
                {
                  ...
                  DbgLv (1) << " vlf:    pctval" << pctval << "nzd2pc nad2pc"
                            << nzd2pc << nad2pc;
                }
            }
        }
              ...
              if (density > 0.0)
                { // Bump if non-empty, numeric; save unique values
                  ...
                  if (!allden.contains (density))
                    allden << density;
                  DbgLv (1)
                      << " vlf:    density" << density << "nadens" << nadens;
                }
            }
        }
      else if (pname == "MLAB")
  ...
  DbgLv (1) << " vlf: nzd2pc nad2pc nadens nalabs" << nzd2pc << nad2pc
            << nadens << nalabs << "allden size" << allden.size () << "nd_orig"
            << nd_orig;
...
void
US_ModelParams::lnedChanged (const QString & /*text*/)
{
