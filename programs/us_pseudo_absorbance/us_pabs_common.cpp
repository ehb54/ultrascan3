...
void
HSVcolormap::set_n_colors (int num)
{
  nc = num;
}

int
HSVcolormap::set_hue_start (int num)
{
  if (num < 0 || num > 359)
    return 1;
  hstart = num;
...
int
HSVcolormap::set_hue_stop (int num)
{
  if (num < 0 || num > 359)
    return 1;
  hstop = num;
...
int
HSVcolormap::set_saturation (int num)
{
  if (num < 0 || num > 255)
    return 1;
  saturation = num;
...
int
HSVcolormap::set_transparency (int num)
{
  if (num < 0 || num > 255)
    return 1;
  alpha = num;
...
int
HSVcolormap::get_colorlist (QVector<QColor> &colorList)
{
  bool state = true;
