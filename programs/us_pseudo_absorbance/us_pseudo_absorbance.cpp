...
void
US_PseudoAbsorbance::select_cnvtSc ()
{
  new_program (CONVERT);
}

void
US_PseudoAbsorbance::select_addRef ()
{
  new_program (ADDREFSCAN);
}

void
US_PseudoAbsorbance::select_rmRi ()
{
  new_program (REMOVERI);
}

void
US_PseudoAbsorbance::turn_on (QPushButton *button, bool state)
{
  ...
  QString off = tr ("QPushButton { background-color: %1;"
                    "font-size: 10pt; font-weight: normal; color: #2b2b2b;}")
                    .arg (color.name ());
  if (state)
...
template <class T>
bool
US_PseudoAbsorbance::cls_program (T *p)
{
  bool state;
  if (p->hasData)
    {
      int mb = QMessageBox::question (
          this, "Warning!",
...
void
US_PseudoAbsorbance::new_program (int state)
{
  ...
  if (state == ADDREFSCAN)
    {
      ...
      connect (addRefScan->pb_close, SIGNAL (clicked ()), this,
               SLOT (close ()));
  ...
  else if (state == CONVERT)
    {
      convertScan = new US_ConvertScan ();
      main_lyt->addWidget (convertScan);
      connect (convertScan->pb_close, SIGNAL (clicked ()), this,
               SLOT (close ()));
      program_state = CONVERT;
      turn_on (pb_cnvtSc, true);
    }
