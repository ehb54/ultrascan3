...
void US_PseudoAbsorbance::select_cnvtSc() { new_program(CONVERT); }

void US_PseudoAbsorbance::select_addRef() { new_program(ADDREFSCAN); }

void US_PseudoAbsorbance::select_rmRi() { new_program(REMOVERI); }
...
void US_PseudoAbsorbance::turn_on(QPushButton *button, bool state) {
  ...
  QString on =
      tr("QPushButton { background-color: #00ff1e;"
...
template <class T>
bool US_PseudoAbsorbance::cls_program(T *p) {
  bool state;
  if (p->hasData) {
