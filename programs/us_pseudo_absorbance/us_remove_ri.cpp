...
US_RemoveRI::US_RemoveRI() : US_Widgets() {
   ...
   usplot_data = new US_Plot(qwtplot_data, tr(""), tr("Radius (in cm)"), tr("Intensity"), true, "", "");
   ...
   usplot_fit = new US_Plot(qwtplot_fit, tr(""), tr("Scan Number"), tr("Integral"), true, "", "");
...
void US_RemoveRI::slt_import(void) {
   ...
   dir = QFileDialog::getExistingDirectory(
      this, tr("Open Raw Data Directory"), US_Settings::importDir(), QFileDialog::DontResolveSymlinks);
   ...
   for (int i = 0; i < fileList.size(); ++i) {
      ...
      if (runId.isNull())
         ...
         QMessageBox::warning(this, tr("Error"), tr("Multiple runIDs found in the directory"));
         return;
      }
      ...
      if (runType.isNull())
         ...
         QMessageBox::warning(this, tr("Error"), tr("Multiple run types found in the directory"));
         return;
      }
...
void US_RemoveRI::set_cb_triples() {
   ...
   for (int i = 0; i < n_ccw; ++i) {
      ...
      QString item_i = item.arg(cell).arg(channel).arg(min_wl).arg(max_wl).arg(nwl);
   ...
   connect(cb_triples, SIGNAL(currentIndexChanged(int)), this, SLOT(slt_new_ccw(int)));
   cb_triples->setCurrentIndex(ccw_id);
}
...
void US_RemoveRI::plot_fit(bool state) {
   ...
   // corrected
   if (fit_state) {
      QwtSymbol *symbol = new QwtSymbol(QwtSymbol::Ellipse, QBrush(Qt::red), QPen(Qt::red, 1), QSize(3, 3));
...
double US_RemoveRI::trapz(const double *x, const double *y, int lxid, int hxid) {
   double dx;
   double sum = 0;
...
void US_RemoveRI::slt_polyfit() {
   ...
   for (int i = 0; i < nwl; i++) {
      ...
      if (cb_autofit->isChecked()) {
         ...
         if (order == -1) {
            QMessageBox::warning(this, tr("Error!"), tr("Autofit couldn't found the proper polynomial order"));
            clean_states(FIT_S);
            return;
         }
         fitOrder[ id ] = order;
         ...
         if (st) {
            ...
            QMessageBox::warning(
               this, tr("Error!"), tr("Couldn't fit the polynomial function with order of: ") + QString::number(order));
            clean_states(FIT_S);
            return;
         }
      }
