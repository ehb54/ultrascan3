...
void US_RemoveRI::slt_import(void) {
  ...
  dir = QFileDialog::getExistingDirectory(this, tr("Open Raw Data Directory"),
...
void US_RemoveRI::slt_new_ccw(int id) {
  ...
  QStringList items;
  for (int i = 0; i < n_wavls; ++i) items << QString::number(wavelength.at(i));
...
void US_RemoveRI::slt_mouse(const QwtDoublePoint& point) {
  ...
  if (x > min_x && x < max_x) {
    ...
    QString mess(
        "Pick a point between the minimum and maximum"
...
void US_RemoveRI::slt_save(void) {
  ...
  for (int i = 0; i < n_ccw; ++i) {
    for (int j = 0; j < ccwItemList.wavelength.at(i).size(); ++j) {
      ...
      QString fn =
          fileName.arg(runIdOut).arg(cell).arg(channel).arg(wavelength);
...
void US_RemoveRI::set_cb_triples() {
  ...
  for (int i = 0; i < ccwList.size(); ++i) {
    ...
    if (!cell_list.contains(cell)) cell_list << cell;
  }
...
void US_RemoveRI::plot_data(bool state) {
  ...
  for (int i = 0; i < ns; ++i) {
    ...
    if (error == 0) pen_plot.setColor(colorList[i % nc]);
...
void US_RemoveRI::plot_fit(bool state) {
  ...
  if (fit_state) {
    ...
    for (int i = 0; i < ns; i++) {
      double yf = 0;
      for (int j = 0; j < fitOrder.at(index); j++) yf += cp[j] * qPow(xp[i], j);
      fit << yf;
    }
...
double US_RemoveRI::trapz(const double* x, const double* y, int lxid,
                          int hxid) {
  double dx;
  double sum = 0;
...
void US_RemoveRI::slt_polyfit() {
  ...
  for (int i = 0; i < nwl; i++) {
    ...
    QVector<double> x(ns);
    double* xp = x.data();
    for (int j = 0; j < ns; j++) xp[j] = (double)j;
    if (cb_autofit->isChecked()) {
      ...
      for (int j = 0; j < coeff.size(); j++) cp[j] = coeff.at(j);
    } else {
...
double US_RemoveRI::get_rsqrd(double* x, double* y, int np,
                              QVector<double> coeff) {
  double res = 0;
  double ave = 0;
  for (int i = 0; i < np; i++) {
    double yf = 0;
    for (int j = 0; j < coeff.size(); j++) yf += coeff.at(j) * qPow(x[i], j);
    res += qPow(y[i] - yf, 2);
    ave += y[i];
  }
  ave /= np;
  double var = 0;
  for (int i = 0; i < np; i++) var += qPow(y[i] - ave, 2);
  double rs = 1 - (res / var);
  return rs;
}
...
void US_RemoveRI::correct_data() {
  ...
  for (int i = 0; i < nwl; i++) {
    ...
    for (int j = 0; j < ns; j++) {
      double yf = 0;
      for (int k = 0; k < order; k++) yf += coeff.at(k) * qPow(j, k);
      ...
      for (int k = 0; k < np; k++) rcp[k] = rp[k] + shift;
    }
  }
  integrate(CDATA_S);
}
...
void US_RemoveRI::pb_save_avail() {
  bool state = true;
  for (int i = 0; i < fitState.size(); i++) state = state && fitState.at(i);
  if (state)
