...
US_AddRefScan::US_AddRefScan () : US_Widgets ()
{
  ...
  // tab 0
  US_Plot *tab0_usplotL
      = new US_Plot (tab0_plotLU, tr (""), tr ("Radius (in cm)"),
  ...
  US_Plot *tab0_usplotR
      = new US_Plot (tab0_plotRU, tr (""), tr ("Radius (in cm)"),
                     tr ("Intensity"), true, "", "rainbow");
  ...
  US_Plot *tab0_devplotL = new US_Plot (
      tab0_plotLD, tr (""), tr ("Radius (in cm)"), tr ("Deviation"));
  ...
  US_Plot *tab0_devplotR = new US_Plot (
      tab0_plotRD, tr (""), tr ("Radius (in cm)"), tr ("Deviation"));
  ...
  US_Plot *tab1_usplotLU = new US_Plot (
      tab1_plotLU, tr (""), tr ("RMSD of Scans"), tr ("Distribution"));
  ...
  US_Plot *tab1_usplotRU = new US_Plot (
      tab1_plotRU, tr (""), tr ("RMSD of Scans"), tr ("Intensity of Scans"));
  ...
  US_Plot *tab1_usplotLD = new US_Plot (
      tab1_plotLD, tr (""), tr ("RMSD of Scans"), tr ("Wavelength (in nm)"));
  ...
  US_Plot *tab1_usplotRD
      = new US_Plot (tab1_plotRD, tr (""), tr ("Intensity of Scans"),
                     tr ("Wavelength (in nm)"));
  ...
  connect (this, SIGNAL (sig_plot_dist (bool)), this,
           SLOT (slt_plot_dist (bool)));
  connect (this, SIGNAL (sig_plot_inty (bool)), this,
           SLOT (slt_plot_inty (bool)));
  connect (this, SIGNAL (sig_plot_ovlp (bool)), this,
           SLOT (slt_plot_ovlp (bool)));
}

//***SLOTS***//

void
US_AddRefScan::slt_import ()
{
  QFileDialog *fdialog;
  ...
  if (runID_new.size () > 0)
    {
      ...
      QMessageBox::warning (this, tr ("Warning: runID(s) Name Changed !"),
                            mess);
    }
  ...
  connect (ckb_align, SIGNAL (stateChanged (int)), this,
           SLOT (slt_aln_state (int)));
...
void
US_AddRefScan::slt_prev_id (void)
{
  --wavl_id;
...
void
US_AddRefScan::slt_next_id (void)
{
  ++wavl_id;
  cb_plot_id->setCurrentIndex (wavl_id);
  return;
...
void
US_AddRefScan::slt_set_id (int id)
{
  wavl_id = id;
...
void
US_AddRefScan::slt_new_bws (double val)
{
...
void
US_AddRefScan::slt_new_wlen (double val)
{
  winlen = val;
  emit sig_cluster (cls_state::ON);
  emit sig_plot_ovlp (true);
...
void
US_AddRefScan::slt_cls_state (int state)
{
  if (state == Qt::Checked)
    emit sig_cluster (cls_state::ON);
  else
...
void
US_AddRefScan::slt_cluster (int state)
{
...
void
US_AddRefScan::slt_cls_ctrl (void)
{
...
void
US_AddRefScan::slt_aln_state (int)
{
  emit sig_plot_r (true);
  return;
}
...
void
US_AddRefScan::slt_reset_bws (void)
{
  if (ckb_bws_all->isChecked ())
...
void
US_AddRefScan::slt_find_merge (void)
{
  this->setCursor (QCursor (Qt::BusyCursor));
  scanWavl_W.clear ();
  scanWavl_W << scanWavl;
...
void
US_AddRefScan::slt_plot_l (bool state)
{
  ...
  for (int i = 0; i < ns; ++i)
    {
      ...
      r = dev.data ();
      QwtSymbol *symbol1
          = new QwtSymbol (QwtSymbol::Ellipse, QBrush (Qt::green),
  ...
  tab0_plotLD->setAxisScale (QwtPlot::xBottom, min_x - dx / 10,
                             max_x + dx / 10);
...
void
US_AddRefScan::slt_plot_r (bool state)
{
  ...
  tab0_plotRD->setAxisScale (QwtPlot::xBottom, min_x - dx / 10,
                             max_x + dx / 10);
  if (ckb_align->isChecked ())
...
void
US_AddRefScan::slt_plot_dist (bool state)
{
...
void
US_AddRefScan::slt_plot_inty (bool state)
{
  tab1_plotRU->detachItems (QwtPlotItem::Rtti_PlotItem, false);
  if (!state)
    {
...
void
US_AddRefScan::slt_plot_ovlp (bool state)
{
...
void
US_AddRefScan::slt_CA_state (int state)
{
  CAValues.clear ();
...
QVector<double>
US_AddRefScan::get_std (QVector<QVector<double> > dev)
{
...
void
US_AddRefScan::write2txt (const QString &file, US_RefScanDataIO::RefData &data)
{
  QFile out_file (file);
  QString sval;
  double dval;
...
void
US_AddRefScan::set_wavl_ctrl (void)
{
  cb_plot_id->disconnect ();
  cb_plot_id->clear ();
  wavl_id = 0;
...
bool
US_AddRefScan::parse_files (QStringList files_path)
{
          ...
          if (min_x != x1 || max_x != x2)
            {
              ...
              QMessageBox msgBox;
              QString text
                  = tr ("Radial point arrays do not match. "
      ...
      if (np != n_points)
        {
          QMessageBox::information (
              this, tr ("Error"),
              tr ("The current file is not compatible with the first parsed "
                  "file."
                  "This might be due to chromatic aberration correction!\n\n"
                  "First file:\nName:%1\n"
  ...
  for (i = 0; i < n_wavls; ++i)
    {
      percent = QString::number (100.0 * (i + 1) / n_wavls, 'f', 1);
      le_status->setText (
          status.arg (QChar (38)).arg (percent).arg (QChar (37)));
      qApp->processEvents ();
...
QVector<int>
US_AddRefScan::arange (int start, int stop, int step)
{
  QVector<int> vector;
  while (start < stop)
    {
...
QVector<int>
US_AddRefScan::arange (int start, int stop)
{
  QVector<int> vector;
  int step = 1;
  while (start < stop)
...
QVector<double>
US_AddRefScan::linspace (double start, double stop, int num)
{
...
double
US_AddRefScan::pdf (double x, double mu, double sigma)
{
...
double
US_AddRefScan::quantile (QVector<double> x, double q)
{
  double qnt;
  int size_x = x.size ();
...
double
US_AddRefScan::skewness (QVector<double> x)
{
...
double
US_AddRefScan::variance (QVector<double> x)
{
  double var = 0;
  double mean = 0;
  int size_x = x.size ();
...
double
US_AddRefScan::bwe_norm (QVector<double> x)
{
  int size_x = x.size ();
...
double
US_AddRefScan::sj_phi6 (double x)
{
  double y;
...
double
US_AddRefScan::sj_phi4 (double x)
{
  double y;
...
double
US_AddRefScan::sj_param (QVector<QVector<double> > *W, int size_x, double h,
                         double tdb, double sda)
{
  int i, j;
  double alpha2, w2, sdalpha2, v;
  alpha2 = 1.357 * std::pow (std::abs (sda / tdb), 1 / 7.0)
           * std::pow (h, 5 / 7.0);
  sdalpha2 = 0;
  for (j = 0; j < size_x; ++j)
    {
...
double
US_AddRefScan::bwe_sj (QVector<double> x)
{
  int i, j;
  int size_x = x.size ();
...
US_AddRefScan::kde (QVector<double> x, double bw)
{
...
US_AddRefScan::loc_minmax (QVector<double> sdiff)
{
  int i;
  double s1, s2;
...
void
US_AddRefScan::estimate_bw (void)
{
...
void
US_AddRefScan::find_clusters (void)
{
  ...
  for (int i = 0; i < n_wavls; ++i)
    {
      ...
      get_clusters_i (current.rmsd, current.scid, wavelength.at (i),
                      rmsd_i_kde, minmax_loc);
      percent = QString::number (100.0 * (i + 1) / n_wavls, 'f', 1);
      le_status->setText (status.arg (percent).arg (QChar (37)));
      qApp->processEvents ();
    }
...
void
US_AddRefScan::get_clusters_i (QVector<double> rmsd_i, QVector<int> ids_i,
...
void
US_AddRefScan::merge_clusters (void)
{
...
QVector<bool>
US_AddRefScan::trimming (QVector<double> rmsd, double threshold)
{
  int ns = rmsd.size ();
...
QVector<bool>
US_AddRefScan::cropping (QVector<double> rmsd, QVector<bool> tsk,
                         double margin)
{
  int ns = rmsd.size ();
...
void
US_AddRefScan::get_current (int id)
{
  current.clear ();
...
void
US_AddRefScan::get_plot_params (void)
{
  double max_x = -1e20;
  double min_x = 1e20;
  double max_r = -1e20;
...
QVector<double>
US_AddRefScan::get_ref_rval (QVector<int> ids)
{
  int i, j, ns;
  ns = ids.size ();
  int *idp = ids.data ();
...
US_AddRefScan::get_ref_dev (QVector<int> ids, QVector<double> ref,
...
bool
US_AddRefScan::check_runIDs (US_DB2 *db, QVector<int> &experimentIDs,
          ...
          if (instrumentID == -1)
          ...
          else
            {
              if (instrumentID != insId)
                {
                  QString mess (
                      "The instrument ID of all RunIDs must be same!");
                  QMessageBox::information (this, tr ("Error"), mess);
                  return false;
                }
            }
...
void
US_AddRefScan::save_local (US_RefScanDataIO::RefData &refScans)
{
  // QString outFileName;
  ...
  QString fpath = QFileDialog::getSaveFileName (
      this, "Save Reference Scan File", US_Settings::dataDir (),
      tr ("dat (*.dat)"));
  if (fpath.isEmpty ())
    {
      return;
      ...
      for (int ii = 0; ii < refScans.nPoints; ii++)
        {
          ...
          for (int jj = 0; jj < refScans.nWavelength; jj++)
            {
              textout << ";"
                      << QString::number (refScans.rValues.at (jj).at (ii));
            }
          textout << "\n";
        }
    }
...
void
US_AddRefScan::save_db (US_RefScanDataIO::RefData &refScans)
{
  QVector<int> experimentIDs;
  QVector<QDate> runIDs_date;
  int instrumentID;
  bool checked
      = check_runIDs (dbCon, experimentIDs, runIDs_date, instrumentID);
  if (!checked)
    {
...
void
US_AddRefScan::check_connection ()
{
...
void
US_AddRefScan::get_refScanDBinfo (US_DB2 *db,
...
QDate
US_AddRefScan::str2date (QString date)
{
  QRegExp re ("\\d+:\\d+:\\d+");
    ...
    : US_WidgetsDialog (nullptr, Qt::Dialog)
{
  ...
  connect (le_base, SIGNAL (textEdited (QString)), this,
           SLOT (slt_edit (QString)));
}

void
FileNameWidget::slt_ok (void)
{
...
void
FileNameWidget::slt_cancel (void)
{
  *fileName = "";
  this->reject ();
}
...
void
FileNameWidget::slt_edit (QString text)
{
...
QFileInfo
FileNameWidget::check_fname (void)
{
  QString fn;
  fn += le_prep->text ();
...
void
setRefTime::slt_set_days (int month)
{
  month += 1;
