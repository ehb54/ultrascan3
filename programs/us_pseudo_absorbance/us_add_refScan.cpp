...
US_AddRefScan::US_AddRefScan() : US_Widgets() {
   ...
   // tab 0
   US_Plot *tab0_usplotL = new US_Plot(tab0_plotLU, tr(""), tr("Radius (in cm)"), tr("Intensity"), true, "", "rainbow");
   ...
   US_Plot *tab0_usplotR = new US_Plot(tab0_plotRU, tr(""), tr("Radius (in cm)"), tr("Intensity"), true, "", "rainbow");
   ...
   US_Plot *tab0_devplotL = new US_Plot(tab0_plotLD, tr(""), tr("Radius (in cm)"), tr("Deviation"));
   ...
   US_Plot *tab0_devplotR = new US_Plot(tab0_plotRD, tr(""), tr("Radius (in cm)"), tr("Deviation"));
   ...
   US_Plot *tab1_usplotLU = new US_Plot(tab1_plotLU, tr(""), tr("RMSD of Scans"), tr("Distribution"));
   ...
   US_Plot *tab1_usplotRU = new US_Plot(tab1_plotRU, tr(""), tr("RMSD of Scans"), tr("Intensity of Scans"));
   ...
   US_Plot *tab1_usplotLD = new US_Plot(tab1_plotLD, tr(""), tr("RMSD of Scans"), tr("Wavelength (in nm)"));
   ...
   US_Plot *tab1_usplotRD = new US_Plot(tab1_plotRD, tr(""), tr("Intensity of Scans"), tr("Wavelength (in nm)"));
...
void US_AddRefScan::slt_import() {
   ...
   if (runID_new.size() > 0) {
      ...
      for (int i = 0; i < runID_new.size(); ++i)
         mess = mess.append(runID_old.at(i)).append("->").append(runID_new.at(i)).append("\n");
...
void US_AddRefScan::slt_plot_l(bool state) {
   ...
   for (int i = 0; i < ns; ++i) {
      ...
      r = dev.data();
      QwtSymbol *symbol1 = new QwtSymbol(QwtSymbol::Ellipse, QBrush(Qt::green), QPen(Qt::green, 0), QSize(1, 1));
...
void US_AddRefScan::slt_plot_r(bool state) {
   ...
   for (int i = 0; i < ns; ++i) {
      ...
      r = dev_s.data();
      QwtSymbol *symbol2 = new QwtSymbol(QwtSymbol::Ellipse, QBrush(Qt::green), QPen(Qt::green, 0), QSize(1, 1));
...
void US_AddRefScan::slt_plot_dist(bool state) {
   ...
   QPen nopen = QPen(Qt::NoBrush, 0, Qt::NoPen);
   // curve 2: rmsd vline
   QwtSymbol *symbol = new QwtSymbol(QwtSymbol::VLine, QBrush(Qt::yellow), QPen(Qt::darkGray, 2), QSize(1, 10));
   ...
   QVector<int> loc;
   // curve 3: min loc
   QwtSymbol *symbol_min = new QwtSymbol(QwtSymbol::VLine, QBrush(Qt::yellow), QPen(Qt::red, 3), QSize(1, 15));
   ...
   // curve 4: max loc
   QwtSymbol *symbol_max = new QwtSymbol(QwtSymbol::Ellipse, QBrush(Qt::yellow), QPen(Qt::red, 2), QSize(8, 8));
...
void US_AddRefScan::slt_plot_inty(bool state) {
   ...
   //  curve
   QwtSymbol *symbol = new QwtSymbol(QwtSymbol::Ellipse, QBrush(Qt::black), QPen(Qt::black, 2), QSize(4, 4));
...
void US_AddRefScan::slt_load_CA() {
   ...
   if (ckb_CA_local->isChecked()) {
      QString filename
         = QFileDialog::getOpenFileName(this, tr("Open File"), US_Settings::workBaseDir(), tr("Text File (*.dat *.txt)"));
      if (filename.isEmpty())
         return;
      ...
      if (!file.open(QIODevice::ReadOnly)) {
         QMessageBox::warning(this, "Error!", tr("Cannot open file for reading.\n%1").arg(filename));
         return;
      }
...
double US_AddRefScan::sj_param(QVector<QVector<double>> *W, int size_x, double h, double tdb, double sda) {
   int i, j;
...
void US_AddRefScan::get_clusters_i(
   QVector<double> rmsd_i, QVector<int> ids_i, double wl, QVector<QVector<double>> kde_out,
   QMap<QString, QVector<int>> minmax_loc) {
...
QVector<bool> US_AddRefScan::trimming(QVector<double> rmsd, double threshold) {
   ...
   std::stable_sort(sorted_ids.begin(), sorted_ids.end(), [ &rmsd ](int i1, int i2) {
...
QVector<QVector<QVector<double>>> US_AddRefScan::get_ref_dev(
   QVector<int> ids, QVector<double> ref, QVector<double> mean, double aveMean) {
   QVector<double> rval;
...
bool US_AddRefScan::check_runIDs(
   US_DB2 *db, QVector<int> &experimentIDs, QVector<QDate> &runIDs_date, int &instrumentID) {
...
void US_AddRefScan::save_local(US_RefScanDataIO::RefData &refScans) {
   ...
   QString fpath
      = QFileDialog::getSaveFileName(this, "Save Reference Scan File", US_Settings::dataDir(), tr("dat (*.dat)"));
   if (fpath.isEmpty()) {
      return;
   }
...
void US_AddRefScan::save_db(US_RefScanDataIO::RefData &refScans) {
   ...
   if (state_newRef == US_DB2::OK) {
      ...
      if (referr == US_RefScanDataIO::OK) {
         int db_write = dbCon->writeBlobToDB(fpath, QString("upload_referenceScanData"), refDataID);
...
setRefTime::setRefTime(QWidget *w, QDate &date) : US_WidgetsDialog(w, Qt::Dialog) {
