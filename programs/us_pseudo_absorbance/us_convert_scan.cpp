...
US_ConvertScan::US_ConvertScan () : US_Widgets ()
{
  ...
  font = QFont (US_Widgets::fixedFont ().family (),
  ...
  tb_triple->setHorizontalHeaderLabels (
      QStringList{ "Cell/Channel", "Lambda", "Reference", "Save" });
  ...
  QGridLayout *chkb_lyt
      = us_checkbox ("Plot Both Intensity and Reference scans", chkb_abs_int);
  chkb_lyt->setAlignment (Qt::AlignCenter);

  QwtText xLabel, yLabel;
  usplot_insty = new US_Plot (qwtplot_insty, tr (""), tr ("Radius (in cm)"),
  ...
  usplot_abs = new US_Plot (qwtplot_abs, tr (""), tr ("Radius (in cm)"),
  ...
  picker_abs->setMousePattern (QwtEventPattern::MouseSelect1, Qt::LeftButton,
                               Qt::ControlModifier);
  ...
  connect (pb_import, &QPushButton::clicked, this,
           &US_ConvertScan::import_run);
  connect (pb_load_ref, &QPushButton::clicked, this,
           &US_ConvertScan::load_ref_scan);
  ...
  connect (pb_pick_rp, &QPushButton::clicked, this,
           &US_ConvertScan::pick_region);
  connect (pb_apply, &QPushButton::clicked, this,
           &US_ConvertScan::apply_nscans);
  connect (pb_default, &QPushButton::clicked, this,
           &US_ConvertScan::default_region);
  // connect(ct_smooth, &QwtCounter::valueChanged, this,
  // &US_ConvertScan::update_scan_smooth);
  connect (ct_maxod, &QwtCounter::valueChanged, this,
           &US_ConvertScan::plot_absorbance);
  connect (chkb_abs_int, &QCheckBox::stateChanged, this,
           &US_ConvertScan::plot_ref_state);
}
...
void
US_ConvertScan::align_center (QwtCounter *ct)
{
...
US_ConvertScan::apply_nscans ()
{
  ...
  connect (ct_scan_from, &QwtCounter::valueChanged, this,
           &US_ConvertScan::lower_scan_range);
  connect (ct_scan_to, &QwtCounter::valueChanged, this,
           &US_ConvertScan::upper_scan_range);
...
US_ConvertScan::load_ref_scan ()
{
  ...
  QString fpath = QFileDialog::getOpenFileName (
      this, "Load Reference Scans", US_Settings::dataDir (), filter);
  if (fpath.isEmpty ())
    {
      return;
    }
      ...
      if (file_loaded.compare (fpath) == 0)
        {
          QMessageBox::warning (
              this, "Error!",
              "The following file has already been uploaded!\n" + fpath);
          return;
        }
    }
  ...
  double max_r = 8;
  QString mesg
      = tr ("Reference scan files should have at least two columns with the "
            "following pattern. "
            "The preferred data separator is semicolon. "
            "The first column is the radial points where header name is (cm). "
            "The radial point should be between %1 to %2 cm and equally "
            "distanced with 0.001 cm. "
            "The rest of the columns are reference profiles at different "
            "wavelengths. "
            "They should be positive values where the header specifies what "
            "wavelength they are for."
              ...
              if (val < 180 || val > 800)
                {
                  QMessageBox::warning (
                      this, "Error!",
                      "Error in the header! Wavelengths are out of range. "
          ...
          if (d <= 0)
            {
              QMessageBox::warning (
                  this, "Error!",
                  mesg_r + " Negative and zero values are not allowed!");
              return;
            }
        }
      yvalues << col;
...
int
US_ConvertScan::read_auc (QVector<US_DataIO::RawData> &rawdata,
                          ...
                          QString &working_dir)
{
  ...
  QDir inDir
      = QDir (working_dir, "*.auc", QDir::Name, QDir::Files | QDir::Readable);
  if (inDir.count () == 0)
    {
  ...
  for (int ii = 0; ii < fileList.size (); ++ii)
    {
      ...
      int state = US_DataIO::readRawData (fileList.at (ii).absoluteFilePath (),
                                          rdata);
  ...
  if (disk_controls->db ())
    {
      ...
      QStringList triples;
      US_LoadAUC *dialog
          = new US_LoadAUC (false, allData, triples, workingDir);
  ...
  else
    {
      workingDir = QFileDialog::getExistingDirectory (
          this, tr ("Open Run ID"), US_Settings::importDir (),
...
void
US_ConvertScan::set_ct_scans (int maxval)
{
  ct_scans->disconnect ();
  ...
  connect (ct_scans, &QwtCounter::valueChanged, this,
           &US_ConvertScan::update_nscans);
}

void
US_ConvertScan::lower_scan_range (double value)
{
...
void
US_ConvertScan::upper_scan_range (double value)
{
...
void
US_ConvertScan::offon_prev_next ()
{
  int n_wavls, wavl_id;
  int row = tb_triple->currentRow ();
  int role = tb_triple->item (row, 0)->data (Qt::UserRole).toInt ();
...
void
US_ConvertScan::set_wavl_ctrl ()
{
  if (tb_triple->currentColumn () >= 2)
    return;
...
void
US_ConvertScan::select_id (int id)
{
  disconnect_picker ();
  int row = tb_triple->currentRow ();
  int role = tb_triple->item (row, 0)->data (Qt::UserRole).toInt ();
...
void
US_ConvertScan::select_refscan (int ref_row)
{
...
void
US_ConvertScan::disconnect_picker ()
{
  picker_abs->disconnect ();
...
US_ConvertScan::pick_region ()
{
  ...
  connect (picker_abs, &US_PlotPicker::cMouseUp, this,
           &US_ConvertScan::mouse_click);
  plot_absorbance ();
}

void
US_ConvertScan::mouse_click (const QwtDoublePoint &point)
{
  double x = point.x ();
  int row = tb_triple->currentRow ();
  double min_x = intensity_data.at (ccw_items.at (row).rawdata_ids.at (0))
                     .xvalues.first ();
  double max_x = intensity_data.at (ccw_items.at (row).rawdata_ids.at (0))
                     .xvalues.last ();
  if (x < min_x || x > max_x)
    {
      QString mess (
          "The picked point is beyond the data values. Please try again!\n"
  ...
  if (first <= 0)
  ...
  else
    {
      if (x <= first)
        {
          QString mess ("The second point has to be greater than the first "
...
bool
US_ConvertScan::set_abs_runid (QString &runid)
{
...
US_ConvertScan::save_run ()
{
  ...
  if (run_list.size () == 0)
  ...
  else if (run_list.size () > 1)
    {
      QMessageBox::warning (this, "Warning!",
                            "Triples from multiple RunIDs are selected!\n"
      ...
      for (int jj = 0; jj < ccw_items.at (ii).rawdata_ids.size (); jj++)
        {
          ...
          if (absorbance_state.at (rid) == -1)
          ...
          else
            {
              ...
              QVector<QVector<double> > absorbance
                  = absorbance_data.at (rid).mid (pos, len);
              QVector<double> shifts
                  = absorbance_shifts.at (rid).mid (pos, len);
  ...
  if (out_rawdata.isEmpty ())
    {
      QMessageBox::warning (this, "Error!",
                            "No absorbance data found to save!");
      return;
    }
  if (!miss_ccw.isEmpty ())
    {
...
US_ConvertScan::set_table ()
{
  ...
  for (int ii = 0; ii < refscan_files.size (); ii++)
    {
      ...
      QString item_wvl = tr ("%1-%2 (%3)")
                             .arg (refscan_files.at (ii).min_wvl)
                             .arg (refscan_files.at (ii).max_wvl)
                             .arg (refscan_files.at (ii).nwvl);
      QTableWidgetItem *twi_wvl;
      twi_wvl = new QTableWidgetItem (item_wvl);
      twi_wvl->setFlags (twi_wvl->flags () & ~Qt::ItemIsEditable);
      twi_wvl->setFont (font);
  ...
  connect (tb_triple, &QTableWidget::currentCellChanged, this,
           &US_ConvertScan::set_wavl_ctrl);
...
void
US_ConvertScan::list_ccw_items (QString &runid)
{
...
US_ConvertScan::plot_intensity ()
{
  ...
  flag = flag
         && (absorbance_state.at (raw_id) >= 0
             && absorbance_state.at (raw_id) < 100);
  flag = !flag;
  ...
  for (int ii = ii_0; ii < ii_1; ++ii)
    {
      ...
      QwtPlotCurve *curve
          = us_curve (qwtplot_insty, tr ("Intensity %1").arg (cnt + 1));
...
US_ConvertScan::plot_refscan ()
{
  ...
  if (role >= 100)
  ...
  else
    {
      ...
      int raw_id
          = ccw_items.at (tb_triple->currentRow ()).rawdata_ids.at (wavl_id);
  ...
  for (int ii = 0; ii < yvalues.size (); ii++)
    {
      ...
      QwtPlotCurve *curve
          = us_curve (qwtplot_insty, tr ("Reference %1").arg (ii + 1));
      curve->setPen (pen);
  ...
  for (int ii = ii_0; ii < ii_1; ++ii)
    {
      ...
      QwtPlotCurve *curve
          = us_curve (qwtplot_abs, tr ("Absorbance %1").arg (cnt + 1));
      if (error == 0)
        pen_plot.setColor (colorList[cnt % nc]);
      curve->setPen (pen_plot);
      curve->setSamples (xp, rp, np);
...
bool
US_ConvertScan::get_refval_file (int ref_id, int raw_id)
{

  ref_id -= 100;
  US_DataIO::RawData rawdata = intensity_data.at (raw_id);
  int iwvl_tgt
      = static_cast<int> (qRound (rawdata.scanData.at (0).wavelength * 10));
  QVector<double> xvals_tgt = rawdata.xvalues;

  bool ok = false;
  for (int ii = 0; ii < refscan_files.at (ref_id).wavelength.size (); ii++)
    {
      int iwvl = static_cast<int> (
          qRound (refscan_files.at (ref_id).wavelength.at (ii) * 10));
      if (iwvl == iwvl_tgt)
        {
          QVector<double> ref_xvals = refscan_files.at (ref_id).xvalues;
          QVector<double> ref_yvals
              = refscan_files.at (ref_id).yvalues.at (ii);
              ...
              if (smooth > 0)
                {
                  ...
                  QVector<double> yvals
                      = smooth_refscan (ref_yvals, smooth, true, true, max_OD);
                  ref_yvals.clear ();
                  ref_yvals << yvals;
                }
...
bool
US_ConvertScan::get_refval_buffer (int ref_row, int raw_id)
{

  int iwvl_tgt = static_cast<int> (
      qRound (intensity_data.at (raw_id).scanData.at (0).wavelength * 10));
  ...
  foreach (int rid, ccw_items.at (ref_row).rawdata_ids)
    {
      US_DataIO::RawData rawdata = intensity_data.at (rid);
      int iwvl = static_cast<int> (
          qRound (rawdata.scanData.at (0).wavelength * 10));
      if (iwvl_tgt == iwvl)
        {
              ...
              if (linear_interpolation (xvals_tgt, ref_xvals, ref_yvals))
                {
                  if (smooth > 0)
                    {
                      double max_OD = ct_maxod->value ();
                      QVector<double> yvals = smooth_refscan (
                          ref_yvals, smooth, true, true, max_OD);
                      ref_yvals.clear ();
                      ref_yvals << yvals;
                    }
...
void
US_ConvertScan::trim_absorbance (QVector<QVector<double> > &absorbance,
...
void
US_ConvertScan::calc_absorbance (int item_row)
{
  if (ccw_items.isEmpty ())
    return;
...
void
US_ConvertScan::update_shifts (int item_row)
{
  if (ccw_items.isEmpty ())
    return;
...
QVector<double>
US_ConvertScan::smooth_refscan (QVector<double> array, int winlen, bool ave,
                                bool intsy, double maxVal)
{
  if (ave)
    {
      int np = array.size ();
...
bool
US_ConvertScan::linear_interpolation (const QVector<double> &xt_vals,
