...
void US_ConvertScan::load_ref_scan() {
  ...
  QString mesg =
      tr("Reference scan files should have at least two columns with the "
         ...
         "The radial point should be between %1 to %2 cm and equally distanced "
         "with 0.001 cm. "
...
int US_ConvertScan::read_auc(QVector<US_DataIO::RawData>& rawdata,
  ...
  for (int ii = 0; ii < fileList.size(); ++ii) {
    ...
    int state =
        US_DataIO::readRawData(fileList.at(ii).absoluteFilePath(), rdata);
    ...
    if (run_id != rid) {
      QMessageBox::warning(this, tr("Warning!"), tr("Multiple RunIDs Found!"));
      return -1;
    }
...
void US_ConvertScan::mouse_click(const QwtDoublePoint& point) {
  ...
  int row = tb_triple->currentRow();
  double min_x =
      intensity_data.at(ccw_items.at(row).rawdata_ids.at(0)).xvalues.first();
  double max_x =
      intensity_data.at(ccw_items.at(row).rawdata_ids.at(0)).xvalues.last();
  if (x < min_x || x > max_x) {
    QString mess(
  ...
  if (first <= 0) {
    ...
    if (x <= first) {
      QString mess(
          "The second point has to be greater than the first one. Please try "
          "again!\n"
          "The picked point = %1 cm\n"
...
void US_ConvertScan::plot_intensity() {
  ...
  flag = flag && (absorbance_state.at(raw_id) >= 0 &&
...
void US_ConvertScan::plot_absorbance() {
  ...
  for (int ii = ii_0; ii < ii_1; ++ii) {
    ...
    if (error == 0) pen_plot.setColor(colorList[cnt % nc]);
    curve->setPen(pen_plot);
    curve->setSamples(xp, rp, np);
    cnt++;
  }
...
QVector<double> US_ConvertScan::smooth_refscan(QVector<double> array,
                                               int winlen, bool ave, bool intsy,
                                               double maxVal) {
  if (ave) {
    int np = array.size();
