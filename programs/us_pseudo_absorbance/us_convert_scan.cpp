...
US_ConvertScan::US_ConvertScan() : US_Widgets() {
   ...
   usplot_insty = new US_Plot(qwtplot_insty, tr(""), tr("Radius (in cm)"), tr("Intensity"), true, "", "rainbow");
   ...
   usplot_abs = new US_Plot(qwtplot_abs, tr(""), tr("Radius (in cm)"), tr("Absorbance"), true, "", "rainbow");
   ...
   picker_abs->setMousePattern(QwtEventPattern::MouseSelect1, Qt::LeftButton, Qt::ControlModifier);
...
void US_ConvertScan::load_ref_scan() {
   ...
   QString fpath = QFileDialog::getOpenFileName(this, "Load Reference Scans", US_Settings::dataDir(), filter);
   if (fpath.isEmpty()) {
      return;
   }
   ...
   std::sort(wavl_idx.begin(), wavl_idx.end(), [](auto a, auto b) { return a.first < b.first; });
   RefscanFile reffile;
...
int US_ConvertScan::read_auc(
   QVector<US_DataIO::RawData> &rawdata, QVector<QVector<QVector<double>>> &absorbance,
   QVector<QVector<QVector<double>>> &refscans, QVector<QVector<double>> &shifts, QString &working_dir) {
   ...
   if (loaded_runids.contains(run_id)) {
      QMessageBox::warning(this, tr("Error"), tr("The RunID (%1) is already loaded!").arg(run_id));
      return -1;
   }
   ...
   for (int ii = 0; ii < fileList.size(); ++ii) {
      ...
      if (!run_type_avail.contains(rtp)) {
         QMessageBox::warning(this, tr("Error"), tr("The Run type (%1) is not supported!").arg(rtp));
         return -1;
      }
      if (run_type.isEmpty()) {
         run_type = rtp;
         ...
         if (run_type != rtp) {
            QMessageBox::warning(this, tr("Error"), tr("Multiple run types are found in the directory!"));
            return -1;
         }
      }
   ...
   if (!bad_files.isEmpty()) {
      QMessageBox::warning(this, tr("Warning!"), tr("Unable to load %1 of the files").arg(bad_files.size()));
   }
...
void US_ConvertScan::import_run() {
   ...
   if (disk_controls->db()) {
      ...
      workingDir = QFileDialog::getExistingDirectory(
         this, tr("Open Run ID"), US_Settings::importDir(), QFileDialog::DontResolveSymlinks);
...
void US_ConvertScan::trim_absorbance(QVector<QVector<double>> &absorbance, QVector<double> &shift, bool state) {
   double max_OD = ct_maxod->value();
   int ns = absorbance.size();
...
QVector<double> US_ConvertScan::smooth_refscan(QVector<double> array, int winlen, bool ave, bool intsy, double maxVal) {
   if (ave) {
      int np = array.size();
...
bool US_ConvertScan::linear_interpolation(
   const QVector<double> &xt_vals, QVector<double> &x_vals, QVector<double> &y_vals) {
