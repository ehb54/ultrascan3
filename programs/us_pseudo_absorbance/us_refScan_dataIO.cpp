...
int US_RefScanDataIO::readRefData(const QString& file, RefData& data) {
  ...
  try {
    ...
    for (int j = 0; j < data.nPoints; ++j) data.xValues << min_x + dx * j;
...
void US_RefScanDataIO::write(QDataStream& ds, const char* c, int len,
                             quint32& crc) {
...
QVector<QVector<double> > US_RefScanDataIO::RefData::get_CA_corrected(
    bool& state) {
