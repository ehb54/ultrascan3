...
void MyButton::pushed(void) { emit click(which); };
...
US_ColorGradient::US_ColorGradient(QWidget* parent, Qt::WindowFlags flags)
  ...
  QString btext =
      tr("Please select the number of steps\n"
...
void US_ColorGradient::safe_close(void) {
  if (new_mods) {  // there have been mods since last file save
    ...
    msgBox.setText(
        tr("You have modified the gradient since the "
  ...
  if (!load_file.isEmpty()) {
    ...
    while (!xmli.atEnd()) {
      ...
      if (xmli.isStartElement() &&
          xmli.name() == "color") {  // update color step entries
...
void US_ColorGradient::update_banner(void) {
  ...
  if (have_save) {
    btext +=
        tr("  Save file: ") + QFileInfo(out_filename).completeBaseName() + " )";
  } else {
