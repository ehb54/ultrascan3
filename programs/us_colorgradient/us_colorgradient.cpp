    ...
    : US_Widgets (true, parent, flags)
{
  ...
  connect (ct_nsteps, SIGNAL (valueChanged (double)), this,
           SLOT (update_steps (double)));
...
void
US_ColorGradient::c_click (int which)
{
...
void
US_ColorGradient::c_cnt_change (double /*count*/)
{
  new_mods = true;
...
void
US_ColorGradient::save_gradient (void)
{
  ...
  if (!save_file.isEmpty ())
    {
      ...
      QString png_fname
          = out_filename.replace (QRegExp (".xml$"), QString (".png"));
...
// and then to close.
void
US_ColorGradient::safe_close (void)
{
  if (new_mods)
...
void
US_ColorGradient::load_gradient (void)
{
...
void
US_ColorGradient::update_steps (double newval)
{
...
void
US_ColorGradient::show_gradient (void)
{
...
void
US_ColorGradient::update_banner (void)
{
  int ncolors = 1;
  int nsteps = qRound (ct_nsteps->value ());
  ...
  QString steps
      = (nsteps == 1) ? "1 step" : QString::number (nsteps) + " steps";
