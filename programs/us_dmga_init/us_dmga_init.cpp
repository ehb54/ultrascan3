...
// Define a base model
void
US_DMGA_Init::define_model (void)
{
...
void
US_DMGA_Init::load_constraints (void)
{
...
void
US_DMGA_Init::define_constraints (void)
{
...
void
US_DMGA_Init::save_constraints (void)
{
  DbgLv (1) << "dGA:save_constraints";
  QString now
      = QDateTime::currentDateTime ().toUTC ().toString ("yyyyMMdd-hhmm");
  ...
  QString msg1
      = tr ("A Discrete Model Genetic Algorithm Constraints"
  ...
  if (mbox.clickedButton () == pb_edit)
    { // Open another dialog to get a custom name prefix
      ...
      suffix = QInputDialog::getText (this, tr ("Model Description Suffix"),
                                      msg2, QLineEdit::Normal, suffix, &ok);
...
void
US_DMGA_Init::update_disk_db (bool isDB)
{
  if (isDB)
    dkdb_cntrls->set_db ();
  else
...
void
US_DMGA_Init::new_base_model (US_Model new_model)
{
  bmodel = new_model;
}
...
void
US_DMGA_Init::timerEvent (QTimerEvent *event)
{
          ...
          while (db.next ())
            {
              ...
              DbgLv (1) << " mdlID" << mdlID << "mdesc" << mdesc << "editID"
                        << edtID << "editGUID" << edtGID;
            }
          ...
          for (int ii = 0; ii < nlmodel; ii++)
            {
              ...
              DbgLv (1) << "   mdlID" << mdlID << "atype" << atype << "jj"
                        << jj;
