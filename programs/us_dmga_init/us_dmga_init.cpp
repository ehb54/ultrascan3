...
void US_DMGA_Init::define_model(void) {
   ...
   connect(mddiag, SIGNAL(valueChanged(US_Model)), SLOT(new_base_model(US_Model)));
...
void US_DMGA_Init::define_constraints(void) {
   ...
   if (cmodel.components.size() == 0) { // If no constraints model, point to the base model
      cmodel = bmodel;
...
void US_DMGA_Init::save_model(void) {
   ...
   if (code == US_DB2::OK) {
      ...
      QMessageBox::information(
         this, mtitle, tr("The file \"") + cmodel.description + tr("\"\n was successfully saved to ") + destination);
   }
   else {
      QMessageBox::warning(
         this, mtitle,
         tr("Writing the file \"") + cmodel.description + tr("\"\n resulted in error code %1 .").arg(code));
   }
}
...
void US_DMGA_Init::save_constraints(void) {
   ...
   if (mbox.clickedButton() == pb_canc)
      return;
   if (mbox.clickedButton() == pb_edit) { // Open another dialog to get a custom name prefix
      bool ok;
      ...
      suffix = QInputDialog::getText(this, tr("Model Description Suffix"), msg2, QLineEdit::Normal, suffix, &ok);
...
void US_DMGA_Init::timerEvent(QTimerEvent *event) {
   ...
   if (tm_id != sctm_id) { // If other than scan delay, pass it on to the normal handler
   ...
   if (dkdb_cntrls->db()) { // Scan models in the database
      ...
      if (numodel > 0) { // If there are models unassigned to edits, scan them
         ...
         while (db.next()) {
            ...
            DbgLv(1) << " mdlID" << mdlID << "mdesc" << mdesc << "editID" << edtID << "editGUID" << edtGID;
         }
   ...
   DbgLv(1) << " numodel" << numodel << "ndmodel ncmodel nmmodel" << ndmodel << ncmodel << nmmodel;
