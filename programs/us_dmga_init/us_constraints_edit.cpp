    ...
    : US_WidgetsDialog (0, 0), cmodel (current_model)
{
  ...
  QGridLayout *lo_mw
      = us_checkbox (tr ("Molecular Wt. (mw)"), ck_sel_mw, true);
  QGridLayout *lo_ff0
      = us_checkbox (tr ("Frictional Ratio (f/f0)"), ck_sel_ff0, true);
  ...
  QGridLayout *lo_conc
      = us_checkbox (tr ("Partial Concentration"), ck_sel_conc, true);
      ...
      for (int ii = 0; ii < cmodel.associations.size (); ii += 2)
        {
          ...
          if (rcomps.size () == 2)
            {
              name = QString::number (st1) + QString (clets).mid (rc1, 1)
                     + QString::number (ol1) + " => "
                     + QString::number (qAbs (st2))
                     + QString (clets).mid (rc2, 1) + QString::number (ol2);
            }
          else if (rcomps.size () == 3)
            {
              name = QString::number (st1) + QString (clets).mid (rc1, 1)
                     + QString::number (ol1) + " + " + QString::number (st2)
                     + QString (clets).mid (rc2, 1) + QString::number (ol2)
                     + " => " + QString::number (qAbs (st3))
                     + QString (clets).mid (rc3, 1) + QString::number (ol3);
            }
          else
            {
  ...
  DbgLv (1) << "cnG:main:  m size" << size () << "fwide fhigh" << fwide
            << fhigh;
...
void
US_ConstraintsEdit::load_c0 (void)
{
...
void
US_ConstraintsEdit::set_molar (void)
{
  int row = lw_comps->currentRow ();
  if (row < 0)
    return;
...
void
US_ConstraintsEdit::acceptProp (void)
{
...
void
US_ConstraintsEdit::co_sed (int new_state)
{
  if (inUpdate)
    return;
...
void
US_ConstraintsEdit::source_changed (bool db)
{
...
void
US_ConstraintsEdit::check_mw (bool checked)
{
...
void
US_ConstraintsEdit::check_ff0 (bool checked)
{
...
void
US_ConstraintsEdit::check_s (bool checked)
{
...
void
US_ConstraintsEdit::check_D (bool checked)
{
...
void
US_ConstraintsEdit::check_f (bool checked)
{
...
void
US_ConstraintsEdit::float_vbar (bool floats)
{
...
void
US_ConstraintsEdit::float_mw (bool floats)
{
...
void
US_ConstraintsEdit::float_ff0 (bool floats)
{
...
void
US_ConstraintsEdit::float_s (bool floats)
{
...
void
US_ConstraintsEdit::float_D (bool floats)
{
...
void
US_ConstraintsEdit::float_f (bool floats)
{
...
void
US_ConstraintsEdit::float_conc (bool floats)
{
...
void
US_ConstraintsEdit::float_kd (bool floats)
{
...
void
US_ConstraintsEdit::float_koff (bool floats)
{
...
void
US_ConstraintsEdit::logsc_mw (bool logscl)
{
  DbgLv (1) << "cnG:logsc_mw   logscl" << logscl;
}

void
US_ConstraintsEdit::logsc_kd (bool logscl)
{
  DbgLv (1) << "cnG:logsc_kd   logscl" << logscl;
}
void
US_ConstraintsEdit::logsc_koff (bool logscl)
{
...
void
US_ConstraintsEdit::comps_connect (bool c_on)
{
  if (c_on)
    { // Turn connections on
...
// Utility to connect/disconnect component slots
void
US_ConstraintsEdit::assocs_connect (bool c_on)
{
  if (c_on)
    { // Turn connections on
...
int
US_ConstraintsEdit::count_checks ()
{
  int nchecks
      = (ck_sel_mw->isChecked () ? 1 : 0) + (ck_sel_ff0->isChecked () ? 1 : 0)
        + (ck_sel_s->isChecked () ? 1 : 0) + (ck_sel_D->isChecked () ? 1 : 0)
...
void
US_ConstraintsEdit::component_select (int srow)
{
  ...
  if (is_prod)
    { // Impose various restrictions for product component
      ...
      for (int ii = 0; ii < cmodel.associations.size (); ii++)
        {
          ...
          DbgLv (1) << "cnG:    ii" << ii << "rc0" << rcomps->at (0) << "crow"
                    << crow;
          if (rcomps->at (0) < 0)
            continue;
                  ...
                  if (rc1 >= 0 && st1 > 0)
                    {
                      ...
                      esum += extinc;
                      DbgLv (1)
                          << "cnG:      mcx" << mcx << "cval,vsum,wsum,esum"
                          << cval << vsum << wsum << esum;
                    }
                }
      ...
      DbgLv (1) << "cnG: miss_vb,mw,ff,co" << miss_vb << miss_mw << miss_ff
                << miss_co;
  ...
  for (int ii = 0; ii < cnsv.size (); ii++)
    {
      ...
      DbgLv (1) << "cnG:cmp_sel: ii" << ii << "atype" << cnsv[ii].atype << "fl"
                << floats;
  ...
  le_oligomer->setText (
      QString::number (cmodel.components[lcompx[crow]].oligomer));
...
void
US_ConstraintsEdit::association_select (int srow)
{
...
void
US_ConstraintsEdit::save_comp_settings (int crow, QVector<C_CONSTRAINT> &cnsv)
{
  ...
  if (kselect != 4)
    {
      qDebug () << "*ERROR* dmGA_Init: component" << cnse.mcompx + 1 << "has"
                << kselect << "attributes selected (SB 4)";
    }
...
void
US_ConstraintsEdit::save_assoc_settings (int arow, QVector<C_CONSTRAINT> &cnsv)
{
  C_CONSTRAINT cnse;
...
void
US_ConstraintsEdit::float_par (bool floats, QLineEdit *le_val,
                               QLineEdit *le_min, QLineEdit *le_max)
{
  bool fixed = !floats;
...
void
US_ConstraintsEdit::check_value (const C_CONSTRAINT cnse, QLineEdit *le_val,
                                 QLineEdit *le_min, QLineEdit *le_max)
{
...
void
US_ConstraintsEdit::check_selects ()
{
  ...
  if (kcheck == 2)
    { // Right number of attributes are selected
      ...
      DbgLv (1) << "cnG:ck_sels: ff0 BEF" << scva.f_f0 << scll.f_f0
                << schh.f_f0 << sclh.f_f0 << schl.f_f0 << "ksel" << ksel;
      ...
      DbgLv (1) << "cnG:ck_sels: ff0 AFT" << scva.f_f0 << scll.f_f0
                << schh.f_f0 << sclh.f_f0 << schl.f_f0;
...
double
US_ConstraintsEdit::constr_value (const C_ATYPE atype,
                                  QVector<C_CONSTRAINT> &cnsv)
{
  double cval = 0.0;
  ...
  for (int ii = 0; ii < cnsv.size (); ii++)
    {
      if (cnsv[ii].atype == atype)
        { // Found the type:  return its value or range average
          ...
          if (cnsv[ii].floats)
            {
              cval = cnsv[ii].logscl ? exp ((log (cmin) + log (cmax)) * 0.5)
                                     : ((cmin + cmax) * 0.5);
            }
