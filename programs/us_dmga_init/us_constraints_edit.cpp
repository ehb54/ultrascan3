...
US_ConstraintsEdit::US_ConstraintsEdit(US_Model& current_model)
  ...
  QGridLayout* lo_vbar =
      us_checkbox(tr("Vbar at 20 ") + DEGC + " (ml/g):", ck_sel_vbar, true);
  ...
  // Advanced parameters
  QLabel* lb_sigma =
      us_label(tr("Concentration Dependency of s (<span>&sigma;</span>):"));
  le_sigma = us_lineedit("");
  QLabel* lb_delta =
      us_label(tr("Concentration Dependency of D (<span>&delta;</span>):"));
  ...
  QPushButton* pb_recompute =
      us_pushbutton(tr("Re-compute unselected component attribute values"));
  ...
  if (cmodel.analysis == US_Model::DMGA_CONSTR) {  // Constraints model
    ...
    if (lw_assocs->count() == 0) lw_assocs->addItem("(none)");
      ...
      if (rcomps->contains(crow)) {
        ...
        for (int jj = 0; jj < rcomps->size(); jj++) {
          ...
          if (rc1 >= 0 && st1 > 0) {
            ...
            wsum += mw;
            esum += extinc;
            DbgLv(1) << "cnG:      mcx" << mcx << "cval,vsum,wsum,esum" << cval
                     << vsum << wsum << esum;
          }
        }
...
void US_ConstraintsEdit::association_select(int srow) {
  ...
  if (srow != arow &&
      arow >= 0) {  // Save settings from previous component screen
    save_assoc_settings(arow, cnsv);
...
void US_ConstraintsEdit::float_par(bool floats, QLineEdit* le_val,
                                   QLineEdit* le_min, QLineEdit* le_max) {
  bool fixed = !floats;

  DbgLv(1) << "cnG:float_par floats" << floats;
  if (floats) {  // Changed to float:  at least 1 float, so capable of saving
                 // constraints
...
void US_ConstraintsEdit::check_value(const C_CONSTRAINT cnse, QLineEdit* le_val,
                                     QLineEdit* le_min, QLineEdit* le_max) {
...
double US_ConstraintsEdit::constr_value(const C_ATYPE atype,
  ...
  for (int ii = 0; ii < cnsv.size(); ii++) {
    if (cnsv[ii].atype ==
        atype) {  // Found the type:  return its value or range average
