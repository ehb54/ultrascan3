...
US_MLinesPlot::US_MLinesPlot (double &ylo, double &yhi, double &xlo,
                              double &xhi, int &typ, int &nkp, int &nlp,
                              int &bmx)
    : US_WidgetsDialog (0, 0), ymin (ylo), ymax (yhi), xmin (xlo), xmax (xhi),
      ctype (typ), nkpts (nkp), nlpts (nlp), bmndx (bmx)
{
  // lay out the GUI
...
US_MLinesPlot::plot_data ()
{
  ...
  QString title;
  QwtPlotCurve *curv;
  DbgLv (1) << "RP:PD xmin xmax" << xmin << xmax << "ymin ymax" << ymin
            << ymax;
  ...
  xtitl = (attr_x == US_ZSolute::ATTR_S)
              ? tr ("Sedimentation Coefficient (x 1e13)")
              : xtitl;
  xtitl = (attr_x == US_ZSolute::ATTR_K) ? tr ("Frictional Ratio (f/f0)")
                                         : xtitl;
  xtitl = (attr_x == US_ZSolute::ATTR_W) ? tr ("Molecular Weight (Dalton)")
                                         : xtitl;
  xtitl = (attr_x == US_ZSolute::ATTR_V) ? tr ("Specific Density (vbar_20W)")
                                         : xtitl;
  xtitl
      = (attr_x == US_ZSolute::ATTR_D) ? tr ("Diffusion Coefficient") : xtitl;
  ytitl = (attr_y == US_ZSolute::ATTR_S)
              ? tr ("Sedimentation Coefficient (x 1e13)")
              : ytitl;
  ytitl = (attr_y == US_ZSolute::ATTR_K) ? tr ("Frictional Ratio (f/f0)")
                                         : ytitl;
  ytitl = (attr_y == US_ZSolute::ATTR_W) ? tr ("Molecular Weight (Dalton)")
                                         : ytitl;
  ytitl = (attr_y == US_ZSolute::ATTR_V) ? tr ("Specific Density (vbar_20W)")
                                         : ytitl;
  ytitl
      = (attr_y == US_ZSolute::ATTR_D) ? tr ("Diffusion Coefficient") : ytitl;
...
void
US_MLinesPlot::setModel (US_Model *a_model, QVector<US_ModelRecord> &mrs)
{
  model = a_model;
  mrecs = mrs;
...
void
US_MLinesPlot::updateElite (double value)
{
  neline = (int)value;
...
void
US_MLinesPlot::updateSolutes (double value)
{
  nsline = (int)value;
...
void
US_MLinesPlot::updateVisible (double value)
{
  nvline = (int)value;
...
QColor
US_MLinesPlot::positionColor (double pos)
{
...
void
US_MLinesPlot::showColorItems (bool visible)
{
...
void
US_MLinesPlot::selectColorMap (void)
{
