...
US_MLinesPlot::US_MLinesPlot(double& ylo, double& yhi, double& xlo, double& xhi,
                             int& typ, int& nkp, int& nlp, int& bmx)
  ...
  plotLayout1 = new US_Plot(data_plot1, tr("Model Lines"),
  ...
  connect(ct_neline, SIGNAL(valueChanged(double)), this,
          SLOT(updateElite(double)));
  connect(ct_nsline, SIGNAL(valueChanged(double)), this,
          SLOT(updateSolutes(double)));
  connect(ct_nvline, SIGNAL(valueChanged(double)), this,
          SLOT(updateVisible(double)));
...
void US_MLinesPlot::close_all() { close(); }
...
void US_MLinesPlot::plot_data() {
  ...
  DbgLv(1) << "RP:PD stype" << stype << "attr_x attr_y attr_z" << attr_x
           << attr_y << attr_z;
  ...
  xtitl = (attr_x == US_ZSolute::ATTR_S)
              ...
              : xtitl;
  xtitl =
      (attr_x == US_ZSolute::ATTR_K) ? tr("Frictional Ratio (f/f0)") : xtitl;
  xtitl =
      (attr_x == US_ZSolute::ATTR_W) ? tr("Molecular Weight (Dalton)") : xtitl;
  xtitl = (attr_x == US_ZSolute::ATTR_V) ? tr("Specific Density (vbar_20W)")
                                         : xtitl;
  ...
  ytitl = (attr_y == US_ZSolute::ATTR_S)
              ...
              : ytitl;
  ytitl =
      (attr_y == US_ZSolute::ATTR_K) ? tr("Frictional Ratio (f/f0)") : ytitl;
  ytitl =
      (attr_y == US_ZSolute::ATTR_W) ? tr("Molecular Weight (Dalton)") : ytitl;
  ytitl = (attr_y == US_ZSolute::ATTR_V) ? tr("Specific Density (vbar_20W)")
                                         : ytitl;
  ...
  if (got_best) {  // Plot lines after best and sorted model records have been
                   // produced
    ...
    int blnx = 0;
    if (mrecs[1].taskx == mrecs[0].taskx) blnx = 1;
    if (mrecs[2].taskx == mrecs[0].taskx) blnx = 2;
    ...
    for (int ii = (nmodel - 1); ii >= 0;
         ii--) {  // Loop over model records from worst (highest rmsd) to best

      // Skip line processing if beyond visible count
      if (ii >= nvline) continue;
      ...
      else if (ii < neline && ii > blnx) {  // Elite:  color according to
                                            // position in elite range
      ...
      if (ctype == CTYPE_SL ||
          ctype ==
              CTYPE_HL) {  // For straight line, just draw from start to end
        klpts = 2;
        xx[0] = xmin;
        xx[1] = xmax;
        yy[0] = mrecs[ii].str_y;
      ...
      if (do_curv) curv->setSamples(xx, yy, klpts);

      if (ii <
          nsline) {  // If within solutes-lines count, plot the solute points
        ...
        for (int kk = 0; kk < ncomp; kk++) {
          ...
          title = tr("Solute Curve ") + QString::number(ii) + " Point " +
                  QString::number(kk);
          curv = us_curve(data_plot1, title);
        ...
        DbgLv(1) << "RP:PD       ncomp" << ncomp << "x0 y0 xn yn" << xx[0]
                 << yy[0] << xx[ncomp - 1] << yy[ncomp - 1];
      }
    }  // END: models loop
...
QwtLinearColorMap* US_MLinesPlot::reverseColorMap() {
  QwtLinearColorMap* rcolmap =
      new QwtLinearColorMap(colormap->color2(), colormap->color1());
  ...
  DbgLv(1) << "rCM:   color1" << rcolmap->color1() << "color2"
           << rcolmap->color2();
