...
US_MLinesPlot::US_MLinesPlot(double &ylo, double &yhi, double &xlo, double &xhi, int &typ, int &nkp, int &nlp, int &bmx) :
    US_WidgetsDialog(0, 0), ymin(ylo), ymax(yhi), xmin(xlo), xmax(xhi), ctype(typ), nkpts(nkp), nlpts(nlp), bmndx(bmx) {
   // lay out the GUI
   ...
   if (ctype == CTYPE_SL)
      s_type = tr("Straight Line");
   else if (ctype == CTYPE_IS)
      s_type = tr("Increasing Sigmoid");
   else if (ctype == CTYPE_DS)
      s_type = tr("Decreasing Sigmoid");
   else if (ctype == CTYPE_HL)
      s_type = tr("Horizontal Line [C(s)]");
   else if (ctype == CTYPE_ALL)
      s_type = tr("All (SL + IS + DS)");
   else if (ctype == CTYPE_2O)
      s_type = tr("Second-Order Power Law");
   ...
   plotLayout1 = new US_Plot(
      data_plot1, tr("Model Lines"), tr("Sedimentation Coefficient (x 1e13)"), tr("Frictional Ratio (f/f0)"));
   ...
   connect(ct_neline, SIGNAL(valueChanged(double)), this, SLOT(updateElite(double)));
   connect(ct_nsline, SIGNAL(valueChanged(double)), this, SLOT(updateSolutes(double)));
   connect(ct_nvline, SIGNAL(valueChanged(double)), this, SLOT(updateVisible(double)));
...
void US_MLinesPlot::plot_data() {
   ...
   DbgLv(1) << "RP:PD stype" << stype << "attr_x attr_y attr_z" << attr_x << attr_y << attr_z;
   QString xtitl = tr("Sedimentation Coefficient (x 1e13)");
   QString ytitl = tr("Frictional Ratio (f/f0)");
   xtitl = (attr_x == US_ZSolute::ATTR_S) ? tr("Sedimentation Coefficient (x 1e13)") : xtitl;
   xtitl = (attr_x == US_ZSolute::ATTR_K) ? tr("Frictional Ratio (f/f0)") : xtitl;
   xtitl = (attr_x == US_ZSolute::ATTR_W) ? tr("Molecular Weight (Dalton)") : xtitl;
   xtitl = (attr_x == US_ZSolute::ATTR_V) ? tr("Specific Density (vbar_20W)") : xtitl;
   xtitl = (attr_x == US_ZSolute::ATTR_D) ? tr("Diffusion Coefficient") : xtitl;
   ytitl = (attr_y == US_ZSolute::ATTR_S) ? tr("Sedimentation Coefficient (x 1e13)") : ytitl;
   ytitl = (attr_y == US_ZSolute::ATTR_K) ? tr("Frictional Ratio (f/f0)") : ytitl;
   ytitl = (attr_y == US_ZSolute::ATTR_W) ? tr("Molecular Weight (Dalton)") : ytitl;
   ytitl = (attr_y == US_ZSolute::ATTR_V) ? tr("Specific Density (vbar_20W)") : ytitl;
   ytitl = (attr_y == US_ZSolute::ATTR_D) ? tr("Diffusion Coefficient") : ytitl;


   //if ( attr_x == US_ZSolute::ATTR_S )
   if (attr_x == US_ZSolute::ATTR_S || attr_x == US_ZSolute::ATTR_K) {
   ...
   if (attr_y == US_ZSolute::ATTR_S || attr_y == US_ZSolute::ATTR_K) {
   ...
   if (got_best) { // Plot lines after best and sorted model records have been produced
      ...
      for (int ii = (nmodel - 1); ii >= 0; ii--) { // Loop over model records from worst (highest rmsd) to best
         ...
         if (ii == blnx) { // Best:  color with top color and use wider line
            title = tr("Best Curve ") + QString::number(ii);
            curv = us_curve(data_plot1, title);
            curv->setPen(pen_best); // Red
         }

         else if (ii < neline && ii > blnx) { // Elite:  color according to position in elite range
         ...
         else if (ii > blnx) { // Non-elite:  color gray
         ...
         if (ctype == CTYPE_SL || ctype == CTYPE_HL) { // For straight line, just draw from start to end
            klpts = 2;
            xx[ 0 ] = xmin;
            xx[ 1 ] = xmax;
            yy[ 0 ] = mrecs[ ii ].str_y;
         ...
         else { // Otherwise, set each point on the curve
         ...
         if (ii < nsline) { // If within solutes-lines count, plot the solute points
            ...
            for (int kk = 0; kk < ncomp; kk++) {
               ...
               title = tr("Solute Curve ") + QString::number(ii) + " Point " + QString::number(kk);
               curv = us_curve(data_plot1, title);
            ...
            DbgLv(1) << "RP:PD       ncomp" << ncomp << "x0 y0 xn yn" << xx[ 0 ] << yy[ 0 ] << xx[ ncomp - 1 ]
                     << yy[ ncomp - 1 ];
         }
      } // END: models loop
   ...
   else { // Plot lines before any best-fit computations
      for (int ii = 0; ii < nmodl; ii++) {
         for (int kk = 0; kk < nlpts; kk++) { // Accumulate the curve points
            xx[ kk ] = mrecs[ ii ].isolutes[ kk ].x * xpscl;
            yy[ kk ] = mrecs[ ii ].isolutes[ kk ].y * ypscl;
         }
...
void US_MLinesPlot::setModel(US_Model *a_model, QVector<US_ModelRecord> &mrs) {
   model = a_model;
   mrecs = mrs;
...
QwtLinearColorMap *US_MLinesPlot::reverseColorMap() {
   ...
   for (double pos = 0.0; pos <= 1.0; pos += 0.2) {
      DbgLv(1) << "  CM position" << pos << "InColor" << positionColor(pos) << "RevColor"
               << QColor(rcolmap->rgb(cinterv, pos));
   }
   return rcolmap;
}
