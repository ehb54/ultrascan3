      ...
      alpha (alf)
{
  ...
  if (mrec.str_y == mrec.par1)
    {
      ctype = mrec.str_y != mrec.end_y ? tr ("Straight Line")
                                       : tr ("Horizontal Line");
    }
  ...
  plotLayout1 = new US_Plot (data_plot1, tr ("Alpha Scan Points"),
                             tr ("Variance (x 1e-5)"),
...
US_RpScan::scan ()
{
  ...
  if (nthr == 1)
        ...
        { // Loop to evaluate each alpha in the specified range
          ...
          if (ja == 0)
            { // For the first one do a full compute and save the A,B matrices
              ...
              solvesim->calc_residuals (0, 1, sim_vals, true, &sv_nnls_a,
                                        &sv_nnls_b);
            ...
            { // After 1st, regularize by modifying A matrix diagonal, then
              // NNLS
              apply_alpha (calpha, &sv_nnls_a, &sv_nnls_b, nscans, npoints,
                           nisols, v_vari, v_xnsq);
            }
    ...
    { // Multi-threaded alpha scan
      le_stattext->setText (
          tr ("Beginning %1-Thread Alpha Scan ...").arg (nthr));
      qApp->processEvents ();
      wthreads.clear ();
      sv_nnls_a.clear ();
      sv_nnls_b.clear ();
  ...
  if (nalpha > 6)
    {
      ...
      while (nlp < nalpha)
        {
          double avg = US_Math2::linefit (&xx, &yy, &slope, &intcp, &sigma,
                                          &corre, nlp);
          DbgLv (1) << "TRP:H1:  avg" << avg << "nlp" << nlp;
          DbgLv (1) << "TRP:H1:   sl" << slope << "in" << intcp << "sg"
                    << sigma << "co" << corre;
      ...
      DbgLv (1) << "TRP:H1:   l1: x1,y1,x2,y2" << xl1p1 << yl1p1 << xl1p2
                << yl1p2;
      ...
      for (int jj = 0; jj < 5; jj++, je--)
        {
          xe[jj] = xx[je];
          ye[jj] = yy[je];
          DbgLv (1) << "TRP:H2: jj x y" << jj << xe[jj] << ye[jj] << "je"
                    << je;
        }
      ...
      DbgLv (1) << "TRP:H2:   l2: x1,y1,x2,y2" << xl2p1 << yl2p1 << xl2p2
                << yl2p2;
      ...
      double xcvp2 = xx[je];
      double ycvp2 = yy[je];
      bool do_plot = (xl3p1 >= xcvp1 && yl3p1 <= ycvp1 && xl3p1 <= xcvp2
                      && yl3p1 >= ycvp2);
      ...
      if (do_plot)
        {
          ...
          xh[1] = xl3p2;
          yh[1] = yl3p2;
          DbgLv (1) << "TRP:H3: x1,y1,x2,y2" << xl3p1 << yl3p1 << yl3p2
                    << yl3p2;
...
void
US_RpScan::mouse (const QwtDoublePoint &p)
{
  double xloc = p.x ();
  double yloc = p.y ();
  double xcurv;
  ...
  QwtSymbol *sym
      = new QwtSymbol (QwtSymbol::Cross, sbrush, spen, QSize (8, 8));
...
void
US_RpScan::process_job (WorkerThreadPc *wthrd)
{
  DbgLv (1) << "SCPJ: IN";
...
void
US_RpScan::apply_alpha (const double alpha, QVector<double> *psv_nnls_a,
                        QVector<double> *psv_nnls_b, const int nscans,
                        const int npoints, const int nisols, double &variance,
                        double &xnormsq)
{
