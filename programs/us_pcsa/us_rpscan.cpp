...
US_RpScan::US_RpScan(QList<US_SolveSim::DataSet*>& dsets, US_ModelRecord& mr,
                     int& thr, double& alf, QWidget* p)
  ...
  if (mrec.str_y == mrec.par1) {
    ctype =
        mrec.str_y != mrec.end_y ? tr("Straight Line") : tr("Horizontal Line");
  ...
  plotLayout1 =
      new US_Plot(data_plot1, tr("Alpha Scan Points"), tr("Variance (x 1e-5)"),
...
void US_RpScan::scan() {
  ...
  if (nthr == 1) {  // Non-multi-threaded scan
    ...
    QString astat = tr("Of %1 models, %2 done (Alpha %3, thread %4)")
                        .arg(nalpha)
                        .arg(nacomp)
                        .arg(alphas[0])
                        .arg(0);
    ...
    for (int jt = 0; jt < nthr;
         jt++) {  // Submit the first tasks using available threads
      WorkPacketPc wtask;
      calpha = alphas[jt + 1];
      US_SolveSim::Simulation sim_vals;
      sim_vals.alpha = calpha;
    ...
    for (int jt = 0; jt < nthr; jt++) delete wthreads[jt];
...
void US_RpScan::plot_data() {
  ...
  data_plot1->setAxisTitle(QwtPlot::xBottom, tr("Variance (x 1e%1)").arg(lgv)),
      data_plot1->setAxisTitle(
          QwtPlot::yLeft,
          (lgx == 0) ? tr("Norm of X (solute concentrations)")
  ...
  if (nalpha > 6) {
    ...
    US_Math2::nearest_curve_point(xx, yy, nalpha, true, xl3p1, yl3p1, &xl3p2,
                                  &yl3p2, alphas.data(), &alpha);
    ...
    double ycvp2 = yy[je];
    bool do_plot =
        (xl3p1 >= xcvp1 && yl3p1 <= ycvp1 && xl3p1 <= xcvp2 && yl3p1 >= ycvp2);
    DbgLv(1) << "TRP:T4:   cv: x1,y1" << xcvp1 << ycvp1 << "x2,y2" << xcvp2
             << ycvp2 << " do_plot" << do_plot;
  ...
  connect(pick, SIGNAL(cMouseUp(const QwtDoublePoint&)), this,
...
void US_RpScan::mouse(const QwtDoublePoint& p) {
  ...
  US_Math2::nearest_curve_point(varias.data(), xnorms.data(), nalpha, true,
                                xloc, yloc, &xcurv, &ycurv, alphas.data(),
                                &alpha);
...
void US_RpScan::process_job(WorkerThreadPc* wthrd) {
  ...
  if (nasubm < nalpha) {  // We need to set up to submit another task
    ...
    wtask.csolutes.clear();
    DbgLv(1) << "SCPJ:     new subm taskx alpha" << nasubm
             << wtask.sim_vals.alpha;
    ...
    wthr->define_work(wtask);
    DbgLv(1) << "SCPJ:     new subm   work defined  sv_nnls_a"
             << wtask.psv_nnls_b;
    //      wthr->disconnect();
    connect(wthr, SIGNAL(work_complete(WorkerThreadPc*)), this,
            SLOT(process_job(WorkerThreadPc*)));
