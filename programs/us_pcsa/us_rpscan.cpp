...
US_RpScan::US_RpScan(QList<US_SolveSim::DataSet *> &dsets, US_ModelRecord &mr, int &thr, double &alf, QWidget *p) :
    US_WidgetsDialog(p, 0), dsets(dsets), mrec(mr), nthr(thr), alpha(alf) {
   alpha = 0.0;
   ...
   if (mrec.str_y == mrec.par1) {
      ctype = mrec.str_y != mrec.end_y ? tr("Straight Line") : tr("Horizontal Line");
   }
   ...
   plotLayout1 = new US_Plot(
      data_plot1, tr("Alpha Scan Points"), tr("Variance (x 1e-5)"), tr("Norm of X (solute concentrations)"));
   ...
   connect(pb_cancel, SIGNAL(clicked()), this, SLOT(reject_it()));
   connect(pb_accept, SIGNAL(clicked()), this, SLOT(accept_it()));
...
void US_RpScan::scan() {
   ...
   if (nthr == 1) { // Non-multi-threaded scan
      ...
      for (int ja = 0; ja < nalpha; ja++) { // Loop to evaluate each alpha in the specified range
         ...
         QString astat = tr("Of %1 models, computed %2 (Alpha %3)").arg(nalpha).arg(kalf).arg(calpha);
   ...
   else { // Multi-threaded alpha scan
      le_stattext->setText(tr("Beginning %1-Thread Alpha Scan ...").arg(nthr));
      qApp->processEvents();
      wthreads.clear();
      sv_nnls_a.clear();
      sv_nnls_b.clear();
      ...
      v_vari = sim_vals.variance;
      v_xnsq = sim_vals.xnormsq;
      qDebug() << "A0: alpha" << calpha << "vari xnsq" << v_vari << v_xnsq << "ncsols" << sim_vals.zsolutes.size();
      ;
      varias[ 0 ] = v_vari;
      xnorms[ 0 ] = v_xnsq;
      nasubm = 1;
      ...
      QString astat = tr("Of %1 models, %2 done (Alpha %3, thread %4)").arg(nalpha).arg(nacomp).arg(alphas[ 0 ]).arg(0);
      le_stattext->setText(astat);
      b_progress->setValue(1);

      for (int jt = 0; jt < nthr; jt++) { // Submit the first tasks using available threads
         WorkPacketPc wtask;
         calpha = alphas[ jt + 1 ];
         US_SolveSim::Simulation sim_vals;
         sim_vals.alpha = calpha;
   ...
   if (ktimeh > 0)
      astat += tr(" %1 hr., %2 min., %3 sec.").arg(ktimeh).arg(ktimem).arg(ktimes);
...
void US_RpScan::plot_data() {
   ...
   if (nalpha > 1) {
      ...
      data_plot1->setTitle(
         tr("Alpha Scan Points\nAlphas from %1 to %2 by %3").arg(alphas[ 0 ]).arg(alphas[ je ]).arg(ainc));
   }
   else {
      data_plot1->setTitle(tr("Alpha Scan Points"));
   }

   data_plot1->setAxisTitle(QwtPlot::xBottom, tr("Variance (x 1e%1)").arg(lgv)),
      data_plot1->setAxisTitle(
         QwtPlot::yLeft, (lgx == 0) ? tr("Norm of X (solute concentrations)")
   ...
   if (nalpha > 6) {
      ...
      while (nlp < nalpha) {
         ...
         DbgLv(1) << "TRP:H1:   sl" << slope << "in" << intcp << "sg" << sigma << "co" << corre;
      ...
      US_Math2::nearest_curve_point(xx, yy, nalpha, true, xl3p1, yl3p1, &xl3p2, &yl3p2, alphas.data(), &alpha);
      ...
      double xcvp2 = xx[ je ];
      double ycvp2 = yy[ je ];
      bool do_plot = (xl3p1 >= xcvp1 && yl3p1 <= ycvp1 && xl3p1 <= xcvp2 && yl3p1 >= ycvp2);
      DbgLv(1) << "TRP:T4:   cv: x1,y1" << xcvp1 << ycvp1 << "x2,y2" << xcvp2 << ycvp2 << " do_plot" << do_plot;
   ...
   connect(pick, SIGNAL(cMouseUp(const QwtDoublePoint &)), this, SLOT(mouse(const QwtDoublePoint &)));
...
void US_RpScan::mouse(const QwtDoublePoint &p) {
   ...
   US_Math2::nearest_curve_point(
      varias.data(), xnorms.data(), nalpha, true, xloc, yloc, &xcurv, &ycurv, alphas.data(), &alpha);
...
void US_RpScan::process_job(WorkerThreadPc *wthrd) {
   ...
   //   delete wthrd;
   QString astat
      = tr("Of %1 models, %2 done (Alpha %3, thread %4)").arg(nalpha).arg(nacomp).arg(alphas[ taskx ]).arg(thrn);
   le_stattext->setText(astat);
   DbgLv(1) << "SCPJ:    astat" << astat;

   if (nacomp == nalpha) { // We are done with all tasks
      qApp->processEvents();
      DbgLv(1) << "SCPJ:    ALL-DONE";
      return;
   }

   if (nasubm < nalpha) { // We need to set up to submit another task
      ...
      wtask.psv_nnls_a = &sv_nnls_a;
      wtask.psv_nnls_b = &sv_nnls_b;
      DbgLv(1) << "SCPJ:     new subm nnls_a nnls_b" << wtask.psv_nnls_a << wtask.psv_nnls_b << "b size"
               << sv_nnls_b.size();
      wtask.csolutes.clear();
...
void US_RpScan::apply_alpha(
   ...
   qDebug() << "AA: alpha" << alpha << "vari xnsq" << variance << xnormsq << "ncsols" << ncsols;
   int mm = npoints / 2;
   qDebug() << "AA: mm=" << mm << "a[m] b[m] s[m]" << (*psv_nnls_a)[ mm ] << (*psv_nnls_b)[ mm ] << simdat[ mm ];
}
