...
void
US_pcsa::analysis_done (int updflag)
{
  if (updflag == (-1))
  ...
  DbgLv (1) << "  edat0 sdat0 rdat0" << edata->value (0, 0)
      ...
      if (dbP->lastErrno () == US_DB2::OK)
        {
          ...
          DbgLv (1) << "SS: ss count" << speed_steps.count () << "idExp"
                    << idExp;
...
US_pcsa::save (void)
{
  ...
  QString editID = edata->editID.startsWith ("20") ? edata->editID.mid (2)
                                                   : edata->editID;
    ...
    { // Output the individual models of a MonteCarlo set
      ...
      if (dbP != NULL)
        ...
        { // Local: rename created file to models directory
          tfile.rename (mname);
          DbgLv (1) << "SV: MC models     write Local by rename tname"
                    << tname;
        }
    }
...
QPointer<QTextEdit>
US_pcsa::mw_status_text ()
{
  return te_status;
}
QStringList *
US_pcsa::mw_model_stats ()
{
  return &model_stats;
}
...
// Write HTML report file
void
US_pcsa::write_report (QTextStream &ts)
{
  QString curvtype = model_stats[1];
...
void
US_pcsa::write_bmap (const QString plotFile)
{
...
void
US_pcsa::child_closed (QObject *o)
{
