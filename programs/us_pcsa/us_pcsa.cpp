...
void US_pcsa::analysis_done(int updflag) {
   ...
   DbgLv(1) << "  edat0 sdat0 rdat0" << edata->value(0, 0) << sdata.value(0, 0) << rdata.value(0, 0);
...
void US_pcsa::load(void) {
   ...
   if (dbP != NULL) { // Fetch the speed steps for the experiment from the database
      QStringList query;
      QString expID;
      int idExp = 0;
      ...
      if (dbP->lastErrno() == US_DB2::OK) {
         ...
         if (speed_steps.count() > 0)
            DbgLv(1) << "SS:  ss0 w2tfirst w2tlast timefirst timelast" << speed_steps[ 0 ].w2t_first
...
void US_pcsa::data_plot(void) {
   ...
   for (int ii = 0; ii < nscans; ii++) {
      ...
      for (int jj = 0; jj < npoints; jj++) {
         ...
         double evalu = edata->value(ii, jj) - sdata.value(ii, jj) - tinoi - rinoi;
         va[ jj ] = evalu;
         vari += sq(evalu);
      }
...
void US_pcsa::save(void) {
   ...
   QString editID = edata->editID.startsWith("20") ? edata->editID.mid(2) : edata->editID;
   ...
   while (indx > 0) { // build a list of available model file names
      ...
      if (!mdnams.contains(mname)) { // Add to the list of new-name models
         mnames << mname;
   ...
   if (mciters < 2) { // Output the single non-MC model
   ...
   else { // Output the individual models of a MonteCarlo set
      ...
      QFile tfile(tname);
      if (dbP != NULL) { // DB:  load, write to DB, delete
         model.load(tname);
         model.description
            = QString(tname).section("/", -1, -1).replace(".model.xml", ".model").replace(".mdl.tmp", ".model");
         model.write(dbP);
         tfile.remove();
         DbgLv(1) << "SV: MC models     write DB from tname" << tname;
      }

      else { // Local: rename created file to models directory
         tfile.rename(mname);
   ...
   if (have_ti) { // output a TI noise
      ...
      DbgLv(1) << "SV: Pre-sum tno tni" << ti_noise.values[ 0 ] << ti_noise_in.values[ 0 ] << "counts sizes"
               << ti_noise.count << nicount << ti_noise.values.size() << ti_noise_in.values.size();
   ...
   QString ptmp4File = tmppath + "PCSA" + dext + ".mlines." + QString::number(getpid()) + ".png";
   ...
   if (resplotd == 0) {
      ...
      connect(resplotd, SIGNAL(destroyed(QObject *)), this, SLOT(child_closed(QObject *)));
   }
...
void US_pcsa::open_3dplot() {
   ...
   eplotcd->show();
   connect(eplotcd, SIGNAL(destroyed(QObject *)), this, SLOT(child_closed(QObject *)));
}
...
void US_pcsa::open_fitcntl() {
   ...
   if (buoy <= 0.0) {
      QMessageBox::critical(
         ...
         tr("The current vbar20 value (%1) implies a buoyancy\n"
            ...
            "range to represent floating data.")
            .arg(vbar20)
            .arg(buoy));
      return;
   }
   ...
   analcd->move(acd_pos);
   analcd->show();
   connect(analcd, SIGNAL(destroyed(QObject *)), this, SLOT(child_closed(QObject *)));
   qApp->processEvents();
}
...
QString US_pcsa::distrib_info() {
   ...
   if (maDesc.startsWith(runID)) { // Saved model:  get analysis description from model description
      ...
      maDesc = "a" + QDateTime::currentDateTime().toUTC().toString("yyMMddhhmm") + "_PCSA_local_01";
   }
...
QString US_pcsa::model_statistics() {
   ...
   QString mstr = "\n" + indent(4) + tr("<h3>Model Statistics:</h3>\n") + indent(4) + "<table>\n";
...
void US_pcsa::write_report(QTextStream &ts) {
   QString curvtype = model_stats[ 1 ];
   QString hdr = tr("Parametrically Constrained Spectrum Analysis") + "<br/>( " + curvtype + " )";
...
void US_pcsa::close(void) {
   QString tripleID = (edata != 0) ? edata->cell + edata->channel + edata->wavelength : "";
   QString ptmp4File = US_Settings::tmpDir() + "/PCSA." + tripleID + ".mlines." + QString::number(getpid()) + ".png";
...
void US_pcsa::child_closed(QObject *o) {
   ...
   if (oname.contains("AnalysisControl"))
      ...
      eplotcd = 0;
   DbgLv(1) << "pcsa:CC: return res epl ana" << resplotd << eplotcd << analcd << "oname" << oname;
}
