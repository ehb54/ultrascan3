...
void US_pcsa::analysis_done(int updflag) {
  ...
  DbgLv(1) << "  edat0 sdat0 rdat0" << edata->value(0, 0) << sdata.value(0, 0)
           << rdata.value(0, 0);
...
void US_pcsa::load(void) {
  ...
  if (dbP !=
      NULL) {  // Fetch the speed steps for the experiment from the database
    QStringList query;
    QString expID;
    int idExp = 0;
  ...
  if (nssp > 0) {
    ...
    for (int ds = 0; ds < ntriples;
         ds++) {  // Scan data time ranges and compare to experiment speed steps
      edata = &dataList[ds];
      int lesc = edata->scanCount() - 1;
      int etm1 = edata->scanData[0].seconds;
      int etm2 = edata->scanData[lesc].seconds;

      if (etm1 < stm1 || etm2 > stm2) {  // Data times beyond speed step ranges,
                                         // so flag use of data ranges
        nssp = 0;
        speed_steps.clear();
        break;
      }
...
void US_pcsa::data_plot(void) {
  ...
  if (!dataLoaded || sdata.scanData.size() != edata->scanData.size()) return;
...
void US_pcsa::save(void) {
  ...
  QString editID =
      edata->editID.startsWith("20") ? edata->editID.mid(2) : edata->editID;
  ...
  if (model.alphaRP != 0.0) analysisType = analysisType + "-TR";

  if (mciters > 1) analysisType = analysisType + "-MC";
  ...
  while (indx > 0) {  // build a list of available model file names
    ...
    if (!mdnams.contains(mname)) {  // Add to the list of new-name models
      mnames << mname;
      DbgLv(1) << "SV: kmodels mciters" << kmodels << mciters << "mname"
               << mname;

      if (++kmodels >= mciters) break;
    }
  }
  ...
  DbgLv(1) << "SV: Pre-sum tno tni" << tino << tini << "rno rni" << rino
           << rini;
  ...
  if (mciters < 2) {  // Output the single non-MC model
    ...
    if (dbP != NULL) {  // DB:  load, write to DB, delete
      model.load(tname);
      model.description = QString(tname)
                              .section("/", -1, -1)
  ...
  if (have_ti) {  // output a TI noise
    ...
    DbgLv(1) << "SV: Pre-sum tno tni" << ti_noise.values[0]
             << ti_noise_in.values[0] << "counts sizes" << ti_noise.count
             << nicount << ti_noise.values.size() << ti_noise_in.values.size();
    ...
    if (err != US_DB2::OK) qDebug() << "*ERROR* writing noise" << nname;
  ...
  QString ptmp4File =
      tmppath + "PCSA" + dext + ".mlines." + QString::number(getpid()) + ".png";
  ...
  if (resplotd == 0) {
    ...
    connect(resplotd, SIGNAL(destroyed(QObject*)), this,
            SLOT(child_closed(QObject*)));
  }
  ...
  if (QFile::copy(ptmp4File, plot4File)) QFile::remove(ptmp4File);
...
void US_pcsa::open_3dplot() {
  ...
  eplotcd->show();
  connect(eplotcd, SIGNAL(destroyed(QObject*)), this,
          SLOT(child_closed(QObject*)));
}
...
void US_pcsa::open_fitcntl() {
  ...
  if (exp_steps) dset.simparams.speed_step = speed_steps;
  ...
  analcd->move(acd_pos);
  analcd->show();
  connect(analcd, SIGNAL(destroyed(QObject*)), this,
          SLOT(child_closed(QObject*)));
  qApp->processEvents();
}
...
QString US_pcsa::distrib_info() {
  ...
  if (maDesc.startsWith(runID)) {  // Saved model:  get analysis description
    ...
    maDesc = "a" + QDateTime::currentDateTime().toUTC().toString("yyMMddhhmm") +
             "_PCSA_local_01";
  }
...
void US_pcsa::write_report(QTextStream& ts) {
  QString curvtype = model_stats[1];
  QString hdr = tr("Parametrically Constrained Spectrum Analysis") + "<br/>( " +
                curvtype + " )";

  if (model.alphaRP > 0.0) hdr += "<br/>- Tikhonov Regularization";
...
void US_pcsa::close(void) {
  QString tripleID =
      (edata != 0) ? edata->cell + edata->channel + edata->wavelength : "";
  QString ptmp4File = US_Settings::tmpDir() + "/PCSA." + tripleID + ".mlines." +
  ...
  if (resplotd != 0) resplotd->close();
  if (eplotcd != 0) eplotcd->close();
  if (analcd != 0) analcd->close();
}
