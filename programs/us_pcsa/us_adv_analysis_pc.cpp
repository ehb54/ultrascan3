...
US_AdvAnalysisPc::US_AdvAnalysisPc (QVector<US_ModelRecord> *p_mrs,
                                    const int nth, US_SolveSim::DataSet *ds0,
                                    QWidget *p)
    : US_WidgetsDialog (p, 0)
{
  ...
  if (ncsols > 0)
    { // We are starting with models already computed
      ...
      DbgLv (1) << "AA: mr p1 p2  m0 p1 p2" << mrec.par1 << mrec.par2
                << mrecs[0].par1 << mrecs[0].par2 << "  typ ni nc" << ctype
                << nisols << ncsols;
  ...
  connect (cb_curvtype, SIGNAL (currentIndexChanged (int)), this,
           SLOT (curvtypeChanged (int)));
...
#endif
  connect (le_mciters, SIGNAL (textChanged (const QString &)), this,
           SLOT (mciterChanged (double)));
...
int
US_AdvAnalysisPc::advanced_results (QVector<US_ModelRecord> *p_mrecsmc)
{
...
void
US_AdvAnalysisPc::curvtypeChanged (int ivalue)
{
...
void
US_AdvAnalysisPc::mciterChanged (double value)
{
...
US_AdvAnalysisPc::load_mrecs ()
{
  ...
  DbgLv (1) << "mrldDiag post-accept xmin xmax ymin ymax" << xmin << xmax
            << ymin << ymax << "nmrecs" << nmrecs;
...
US_AdvAnalysisPc::store_mrecs ()
{
  ...
  US_ModelRecord::write_modelrecs (xmlo, mrecs, mrdesc, ctype, xmin, xmax,
                                   ymin, ymax, stype);
  fileo.close ();
  ...
  stat_mrecs (tr ("Model Records have been stored in file\n"
                  "  \"%1\", of directory\n  \"%2\".")
                  .arg (fname)
                  .arg (fdir),
              true);
          ...
          if (xmlname == "modelrecord")
            {
              ...
              mrec.csolutes.clear ();
              ncsols = 0;
              // DbgLv(1) << "LM:xml:    nmrecs" << nmrecs << "kisols" <<
              // kisols;
            }
...
US_AdvAnalysisPc::store_bfm ()
{
  ...
  stat_bfm (tr ("The Best Final Model has been stored in file\n"
                "  \"%1\", of directory\n  \"%2\".")
                .arg (fname)
                .arg (fdir),
            true);
...
US_AdvAnalysisPc::build_bfm ()
{
  ...
  stat_bfm (tr ("A new Best Final Model has been built\n"
  ...
  stat_mrecs (tr ("A new best final %1-solute model ( RMSD = %2 )\n"
...
void
US_AdvAnalysisPc::start_montecarlo ()
{
  // Test and return immediately if valid mrecs still required
  ...
  stat_bfm (
      tr ("%1 Monte Carlo iterations are being computed...").arg (mciters));
  ksiters = 0;
  kciters = 0;
  ...
  if (nsol_p != nsol_m || difp1 > 1.0e-4 || difp2 > 1.0e-4 || dymin > 1.0e-4
      || dymin > 1.0e-4)
    {
      ...
      mrec.ymin = ymin_p;
      mrec.ymax = ymax_p;
      DbgLv (1) << " MC reset curves: p1_m p2_m p1_p p2_p ns_m ns_p" << par1_m
                << par2_m << par1_p << par2_p << nsol_m << nsol_p;
      DbgLv (1) << " MC reset curves:   dp1 dp2 sdp1 sdp2" << difp1 << difp2
                << (par1_m - par1_p) << (par2_m - par2_p);
      DbgLv (1) << " MC reset curves:   dyl dyh sdyl sdyh" << dymin << dymax
                << (ymin_m - ymin_p) << (ymax_m - ymax_p);
  ...
  if (nthr == 1)
      ...
      for (int iter = 0; iter < mciters; iter++)
        {
          ...
          stat_bfm (tr ("%1 are completed ( last:  %2-solute,  RMSD=%3 )")
                        .arg (kciters)
                        .arg (ncsols)
                        .arg (mrec_mc.rmsd),
                    true, 1);
    ...
    { // Monte Carlo in threads
      ...
      mrec_mc.csolutes = sim_vals.zsolutes;
      ncsols = mrec_mc.csolutes.size ();
      DbgLv (1) << "  kciters" << kciters << "rmsd" << mrec_mc.rmsd << "ncsols"
                << ncsols << "res tskx,thrn" << 1 << 0;
      stat_bfm (tr ("%1 are completed ( last:  %2-solute,  RMSD=%3 )")
                    .arg (kciters)
                    .arg (ncsols)
                    .arg (mrec_mc.rmsd),
      ...
      for (int jt = 0; jt < nthr; jt++)
        {
          ...
          connect (wthrd, SIGNAL (work_complete (WorkerThreadPc *)), this,
          ...
          ksiters++;
          DbgLv (1) << "    ksiters" << ksiters << "dsets[0]"
                    << wtask.dsets[0];
        }
    }
}
...
void
US_AdvAnalysisPc::set_gaussians (US_SolveSim::Simulation &sim_vals)
{
  ...
  if (gausmoo)
    { // Scale the sigmas so they are at the same level as original residuals
      ...
      DbgLv (1) << "AA: gausmoo: rmsd-i rmsc-s sigscl" << rmsdi << rmsds
                << sigscl;
    }
...
void
US_AdvAnalysisPc::apply_gaussians ()
{
  int nscans = edata->scanCount ();
  int npoints = edata->pointCount ();
  int kk = 0;
...
void
US_AdvAnalysisPc::process_job (WorkerThreadPc *wthr)
{
  kciters++;
  ...
  ncsols = mrec_mc.csolutes.size ();
  DbgLv (1) << "  kciters" << kciters << "rmsd" << mrec_mc.rmsd << "ncsols"
            << ncsols << "res tskx,thrn" << wresult.taskx << wresult.thrn;
...
void
US_AdvAnalysisPc::montecarlo_done (void)
{
...
DbgLv(1) << "MCD: cc ccin ncsols" << cc << ccin << ncsols;
      ...
      for (int rr = 0; rr < npoints; rr++)
        {
          ...
          varia += sq (
              (edata->value (ss, rr) - sdata->value (ss, rr) - rinoi - tinoi));
        }
    }
  ...
  stat_bfm (tr ("MC models and final %1-solute composite model are built.")
                .arg (ncsols),
            true);
...
void
US_AdvAnalysisPc::under_construct (QString proc)
{
  QMessageBox::information (
      this, tr ("UNDER CONSTRUCTION"),
      tr ("Implementation of <b>%1</b> is not yet complete.").arg (proc));
}

// Re-generate the input solute curve points for a model record
void
US_AdvAnalysisPc::curve_isolutes (US_ModelRecord &mrec)
{
  ...
  if (ctype == CTYPE_IS) // Increasing Sigmoid
  ...
  else if (ctype == CTYPE_2O) // 2nd-Order Power Law
    {
      ...
      //                       - log10( xmin );
      double bval
          = ((mrec.str_y - cval) / qMax (0.00001, aval)) - log10 (xmin);
...
void
US_AdvAnalysisPc::bfm_model (void)
{
  US_Model modela;
  US_Model::SimulationComponent zcomponent;
  zcomponent.vbar20 = dset0->vbar20;
...
void
US_AdvAnalysisPc::stat_mrecs (const QString msg, bool append, int line)
{
...
void
US_AdvAnalysisPc::stat_bfm (const QString msg, bool append, int line)
{
...
void
US_AdvAnalysisPc::show_stat (QTextEdit *tedit, const QString msg, bool append,
                             int aft_line)
{
  if (append)
...
void
US_AdvAnalysisPc::set_fittings (QVector<US_ModelRecord> &s_mrecs)
{
  ...
  ctype = s_mrec.ctype;
  DbgLv (1) << "AA:SF: ctype x,y min,max" << ctype << xmin << xmax << ymin
            << ymax;
...
DbgLv(1) << "AA:SF:   ii jj ni" << ii << jj << nisols << "x y" << xval << yval;
  ...
  DbgLv (1) << "AA:SF: (2) x,y min,max" << ctype << xmin << xmax << ymin
            << ymax;
}

// Return a flag and possibly warn if operation requires valid mrecs
bool
US_AdvAnalysisPc::mrecs_required (QString oper)
{
...
bool
US_AdvAnalysisPc::bfm_incompat (QString fname)
{
  ...
  if (inCompat)
    {
      const char *ctps[] = { "Straight Line", "Increasing Sigmoid",
                             "Decreasing Sigmoid", "Horizontal Line" };
      int ftx = ftype;
...
void
US_AdvAnalysisPc::test_db_mrecs ()
{
  US_Passwd pw;
  ...
  if (dbP->lastErrno () == US_DB2::OK)
    {
      ...
      qDebug () << "   moID moGUID" << moID << moGUID;
      qDebug () << "   desc" << descr;
      qDebug () << "   cksm size" << cksm << xsize << "xml len"
                << xmlstr.length ();
