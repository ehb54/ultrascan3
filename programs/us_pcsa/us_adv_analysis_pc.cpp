...
void US_AdvAnalysisPc::select() {
  ...
  if (mrs_new &&
      p_mrecs != 0) {  // If model records are new, return them to the caller
    *p_mrecs = mrecs;
  ...
  if (!mc_done) {  // Insure there is no montecarlo left over if there shouldn't
                   // be
    mrecs_mc.clear();
  }
...
void US_AdvAnalysisPc::curvtypeChanged(int ivalue) {
  ...
  if (ctype == CTYPE_HL) lb_y_endpt->setText(tr("Line Y End Points:"));
}
...
void US_AdvAnalysisPc::load_mrecs() {
  ...
  DbgLv(1) << "mrldDiag post-accept xmin xmax ymin ymax" << xmin << xmax << ymin
           << ymax << "nmrecs" << nmrecs;
...
void US_AdvAnalysisPc::store_mrecs() {
  ...
  if (mrecs_required("Store Model Records")) return;
  ...
  if (fname.startsWith(
    ...
    store_file = fdir + fname.mid(1);
               ...
               "pcsa-mrs-")) {  // If no "pcsa-mrs-" prefix, add one
  ...
  US_ModelRecord::write_modelrecs(xmlo, mrecs, mrdesc, ctype, xmin, xmax, ymin,
                                  ymax, stype);
  fileo.close();
...
void US_AdvAnalysisPc::load_bfm() {
  ...
  //  Test and return immediately if valid mrecs still required
  if (mrecs_required("Load Final Model")) return;
    ...
    if (xmli.isComment()) {  // Verify DOCTYPE PcsaBestFinalModel
      ...
      if (comm.contains("PcsaBestFinalModel"))
        ...
        QMessageBox::critical(
            ...
            tr("File \"%1\" is not a PcsaBestFinalModel XML file.").arg(fname));
        filei.close();
        return;
      }
    }
...
void US_AdvAnalysisPc::store_bfm() {
  ...
  if (fname.startsWith(
    ...
    store_file = fdir + fname.mid(1);
               ...
               "pcsa-bfm-")) {  // If no "pcsa-bfm-" prefix, add one
...
void US_AdvAnalysisPc::build_bfm() {
  ...
  //  Test and return immediately if valid mrecs still required
  if (mrecs_required("Build Final Model")) return;
  ...
  if (ctype == CTYPE_SL) {
    ...
    mrec.par1 = le_sigmpar1->text().toDouble();
    mrec.par2 = le_sigmpar2->text().toDouble();
    if (mrecs.size() > 0) mrec.par3 = mrecs[0].par3;
  }
...
void US_AdvAnalysisPc::start_montecarlo() {
  // Test and return immediately if valid mrecs still required
  if (mrecs_required("Start Monte Carlo")) return;
  ...
  if (nthr == 1) {  // Monte Carlo with single thread
    ...
    for (int jt = 0; jt < nthr;
         jt++) {  // Build up a list of datasets for each thread
      wkdsets << wkdset;
    }
...
void US_AdvAnalysisPc::set_gaussians(US_SolveSim::Simulation& sim_vals) {
  ...
  for (int ss = 0; ss < nscans;
       ss++) {  // Loop to accumulate the residuals from iteration 1
    QVector<double> vv(npoints);

    for (int rr = 0; rr < npoints;
         rr++) {  // Get all residuals points from a scan
  ...
  if (gausmoo) {  // Scale the sigmas so they are at the same level as original
                  // residuals
...
void US_AdvAnalysisPc::process_job(WorkerThreadPc* wthr) {
  ...
  if (kciters == mciters) {  // Complete the MonteCarlo process
    ...
    wtask.dsets[0]->run_data = wdata;
    wtask.taskx = ksiters;
    DbgLv(1) << "    ksiters" << ksiters << "     wt tskx,thrn" << wtask.taskx
             << wtask.thrn;
...
#if 1
      model.components[cc] = zcomponent;
      US_ZSolute::set_mcomp_values(model.components[cc], csolutes[cc], stype,
                                   true);
#endif
...
DbgLv(1) << "MCD: cc ccin ncsols" << cc << ccin << ncsols;
  ...
  for (int ss = 0; ss < nscans; ss++) {
    ...
    for (int rr = 0; rr < npoints; rr++) {
      ...
      varia +=
          sq((edata->value(ss, rr) - sdata->value(ss, rr) - rinoi - tinoi));
    }
  }
...
void US_AdvAnalysisPc::bfm_model(void) {
  ...
  if (rmsd > rmsdsv && rmsd > mrecs[3].rmsd) {  // Computed rmsd too high:  must
                                                // have skipped noise, so ignore
    mrecs[0].sim_data = mrec.sim_data;
    mrecs[0].residuals = mrec.residuals;
    mrec = mrecs[0];
    model = mrec.model;

    if (!mrec.modelGUID.isEmpty()) {  // If possible, load the model referred to
                                      // in best model record
      US_Passwd pw;
      bool loadDB = dset0->requestID.contains("DB");
...
void US_AdvAnalysisPc::show_stat(QTextEdit* tedit, const QString msg,
  ...
  if (append) {  // Message gets appended
    ...
    if (aft_line > 0) {  // Append after the specified line
      ...
      for (int ii = 0; ii < qMin(aft_line, stlines.size());
           ii++) {  // Rebuild lines before the specified one
        sttext += stlines[ii] + "\n";
      }
    }
...
bool US_AdvAnalysisPc::bfm_incompat(QString fname) {
  ...
  if (inCompat) {
    ...
    QString fpars = QString(ctps[ftx]) + tr(" ; s %1 to %2 ; f/f0 %3 to %4")
                                             .arg(fxmin)
                                             .arg(fxmax)
                                             .arg(fymin)
                                             .arg(fymax);
    QString rpars = QString(ctps[rtx]) + tr(" ; s %1 to %2 ; f/f0 %3 to %4")
                                             .arg(rxmin)
                                             .arg(rxmax)
                                             .arg(rymin)
                                             .arg(rymax);
