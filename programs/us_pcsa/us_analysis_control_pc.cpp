    ...
    : US_WidgetsDialog (p, 0), dsets (dsets)
{
  ...
  connect (ct_tralpha, SIGNAL (valueChanged (double)), this,
           SLOT (set_alpha ()));
...
void
US_AnalysisControlPc::optimize_options ()
{
...
void
US_AnalysisControlPc::uncheck_optimize (int /*ckflag*/)
{
}

// start fit button clicked
void
US_AnalysisControlPc::start ()
{
  US_pcsa *mainw = (US_pcsa *)parentw;
  ...
  if (processor == 0)
    {
      ...
      dsets[0]->zcoeffs[0]
          = le_z_func->text ().section (" ", 0, 0).toDouble ();
  ...
  if (need_fit)
    processor->start_fit (slo, sup, klo, kup, nvar, res, typ, nthr, noif,
                          lmmxc, gfits, gfthr, alpha);
...
US_AnalysisControlPc::advanced ()
{
  ...
  US_AdvAnalysisPc *aadiag
      = new US_AdvAnalysisPc (&mrecs, nthr, dsets[0], this);
  ...
  if (aadiag->exec () == QDialog::Accepted)
    {
      ...
      if (mmcupd)
        ...
        { // Report new BFM from other advanced controls action
          QString fmsg = tr ("\nA newer best model has been created from "
                             "Advanced Controls\n"
      ...
      if (state != 0)
        { // Where advanced controls have made changes, copy them
          ...
          int nmtsks
              = (mrecs[0].taskx == mrecs[1].taskx) ? (nmrecs - 1) : nmrecs;
...
// Close all windows
void
US_AnalysisControlPc::close_all ()
{
...
void
US_AnalysisControlPc::xlim_change ()
{
...
void
US_AnalysisControlPc::ylim_change ()
{
  compute ();
}
...
void
US_AnalysisControlPc::reso_change ()
{
...
void
US_AnalysisControlPc::type_change ()
{
...
void
US_AnalysisControlPc::set_alpha ()
{
...
void
US_AnalysisControlPc::update_progress (double variance)
{
  ncsteps++;
...
void
US_AnalysisControlPc::progress_message (QString pmsg, bool append)
{
  QString amsg;
...
void
US_AnalysisControlPc::reset_steps (int kcs, int nct)
{
...
void
US_AnalysisControlPc::completed_process (int stage)
{
  US_pcsa *mainw = (US_pcsa *)parentw;
  ...
  processor->get_results (sdata, rdata, model, ti_noise, ri_noise, bmndx,
                          modelstats, mrecs);
  DbgLv (1) << "AC:cp: RES: ti,ri counts" << ti_noise->count
            << ri_noise->count;
...
void
US_AnalysisControlPc::plot_lines ()
{
  ctypex = cb_curvtype->currentIndex ();
  ...
  connect (mlnplotd, SIGNAL (destroyed (QObject *)), this,
           SLOT (closed (QObject *)));
...
void
US_AnalysisControlPc::closed (QObject *o)
{
...
void
US_AnalysisControlPc::fit_final (void)
{
  need_fit = true;
  need_final = true;
  fitpars = QString ();
...
void
US_AnalysisControlPc::scan_alpha (void)
{
  US_ModelRecord mrec;
  need_fit = false;
  need_final = false;
...
void
US_AnalysisControlPc::final_only (void)
{
  need_fit = false;
  need_final = true;
  int klpts = (int)ct_cresolu->value ();
...
QString
US_AnalysisControlPc::fitpars_string ()
{
  ...
  return QString ().sprintf ("%d %.5e %.5e %.5e %.5e %d %d", typ, xlo, xup,
                             ylo, yup, nvar, noif);
}

// Re-Connect or disconnect fit parameters controls
void
US_AnalysisControlPc::fitpars_connect (bool reconn)
{
  if (reconn)
    { // Reconnect controls
      ...
      connect (ct_uplimity, SIGNAL (valueChanged (double)), this,
               SLOT (ylim_change ()));
      connect (cb_z_type, SIGNAL (activated (int)), this,
               SLOT (ztype_change (int)));
      connect (ct_varcount, SIGNAL (valueChanged (double)), this,
               SLOT (compute ()));
...
void
US_AnalysisControlPc::recompute_mrec ()
{
...
int
US_AnalysisControlPc::memory_check ()
{
...
void
US_AnalysisControlPc::select_x_axis (int ival)
{
...
void
US_AnalysisControlPc::select_y_axis (int ival)
{
...
// Set Z type
void
US_AnalysisControlPc::ztype_change (int newx)
{
  attr_z = ATTR_V;
...
void
US_AnalysisControlPc::adjust_xyz (const int chg_ndx)
{
...
void
US_AnalysisControlPc::set_solute_type ()
{
