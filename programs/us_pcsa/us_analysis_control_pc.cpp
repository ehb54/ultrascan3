...
US_AnalysisControlPc::US_AnalysisControlPc(QList<US_SolveSim::DataSet *> &dsets, QWidget *p) :
   ...
   mlnplotd = 0;
   fitpars = QString();
   ctypes << CTYPE_SL << CTYPE_IS << CTYPE_DS << CTYPE_HL << CTYPE_2O << CTYPE_ALL;
   attr_x = 0;
   attr_y = 1;
   attr_z = 3;
   ...
   else { // Aarrggg! No pointer back to parent!!!
      DbgLv(0) << "*ERROR* AnalysisControl has no pointer back to Main PCSA";
      QMessageBox::critical(
         this, tr("Parent Pointer NULL!"), tr("*ERROR* AnalysisControl has no pointer back to Main PCSA"));
      close();
      return;
   }
   ...
   QLayout *lo_lmalpha = us_checkbox(tr("Regularize in L-M Fits"), ck_lmalpha);
   QLayout *lo_fxalpha = us_checkbox(tr("Regularize in Grid Fits"), ck_fxalpha);
   ...
   connect(ct_tralpha, SIGNAL(valueChanged(double)), this, SLOT(set_alpha()));

   connect(pb_pltlines, SIGNAL(clicked()), this, SLOT(plot_lines()));
   connect(pb_startfit, SIGNAL(clicked()), this, SLOT(fit_final()));
   connect(pb_scanregp, SIGNAL(clicked()), this, SLOT(scan_alpha()));
   connect(pb_finalmdl, SIGNAL(clicked()), this, SLOT(final_only()));
   ...
   connect(pb_advanaly, SIGNAL(clicked()), this, SLOT(advanced()));
...
void US_AnalysisControlPc::start() {
   ...
   if ((ct_uplimitx->value() - ct_lolimitx->value()) < 0.0 || (ct_uplimity->value() - ct_lolimity->value()) < 0.0) {
   ...
   if (processor == 0) {
      ...
      dsets[ 0 ]->zcoeffs[ 0 ] = le_z_func->text().section(" ", 0, 0).toDouble();
   ...
   if (attr_x == US_ZSolute::ATTR_S && US_SolveSim::checkGridSize(dsets, s_max, smsg)) {
   ...
   if (need_fit) { // Not resuming after an Alpha scan
      if (!need_final)
   ...
   DbgLv(1) << "AnaC: need_fit need_fnl" << need_fit << need_final << "alpha" << alpha;
   ...
   connect(processor, SIGNAL(progress_update(double)), this, SLOT(update_progress(double)));
   ...
   if (need_fit)
      processor->start_fit(slo, sup, klo, kup, nvar, res, typ, nthr, noif, lmmxc, gfits, gfthr, alpha);
...
void US_AnalysisControlPc::advanced() {
   ...
   if (mrecs.size() > 0)
      DbgLv(1) << "AC:advanced mrecs0 p1 p2" << mrecs[ 0 ].par1 << mrecs[ 0 ].par2 << "ctype" << mrecs[ 0 ].ctype;
   int nthr = ( int ) ct_thrdcnt->value();

   US_AdvAnalysisPc *aadiag = new US_AdvAnalysisPc(&mrecs, nthr, dsets[ 0 ], this);
   ...
   if (aadiag->exec() == QDialog::Accepted) {
      ...
      if (ck_tinoise->isChecked() || ck_rinoise->isChecked()) {
         QMessageBox::warning(
            this, tr("Problematic combination!"),
      ...
      DbgLv(1) << "AC:advanced: mrec0 sols" << mrecs[ 0 ].csolutes.size() << "mrecs size" << mrecs.size()
               << "mrecs_mc size" << mrecs_mc.size() << "model compsize" << model->components.size();

      if (mmcupd) { // Report new BFM from MonteCarlo
      ...
      else if (mrsupd) { // Report new BFM from other advanced controls action
      ...
      if (state != 0) { // Where advanced controls have made changes, copy them
         pb_startfit->setEnabled(true);
         pb_scanregp->setEnabled(true);
         pb_finalmdl->setEnabled(true);
         pb_stopfit->setEnabled(false);
         ...
         processor->get_results(sdata, rdata, model, ti_noise, ri_noise, bmndx, *mw_modstats, mrecs);

         ctype = mrecs[ 0 ].ctype;
         int v_ctype = mrecs[ 0 ].v_ctype;
         int nmrecs = mrecs.size();
         int nmtsks = (mrecs[ 0 ].taskx == mrecs[ 1 ].taskx) ? (nmrecs - 1) : nmrecs;
         nmtsks = (mrecs[ 1 ].taskx == mrecs[ 2 ].taskx) ? (nmrecs - 1) : nmrecs;
         int strec = nmrecs - nmtsks;
         nypts = (v_ctype != CTYPE_ALL) ? nmtsks : (nmtsks / 3);
         nypts = (ctype != CTYPE_HL && ctype != CTYPE_2O) ? qRound(sqrt(( double ) nypts)) : nypts;
         nypts = (ctype != CTYPE_2O) ? nypts : qRound(pow(( double ) nypts, 0.33333));
...
void US_AnalysisControlPc::reso_change() {
   DbgLv(1) << "RESO_CHANGE: need_fit" << need_fit << "reso" << ct_cresolu->value();
   if (need_fit)
      compute();
}
...
void US_AnalysisControlPc::completed_process(int stage) {
   ...
   if (stage == 7) { // If an alpha scan can now be done, report L-M info and mark scan-ready
      US_ModelRecord mrec;
   ...
   if (stage == 8) { // If starting L-M, turn off Stop Fit
      pb_stopfit->setEnabled(false);
      return;
   }

   if (stage == 6) { // If stopped because of memory usage, execute stop_fit
      stop_fit();
      return;
   }
   ...
   processor->get_results(sdata, rdata, model, ti_noise, ri_noise, bmndx, modelstats, mrecs);
...
void US_AnalysisControlPc::compute() {
   ...
   DbgLv(1) << "AC:CM: ctype" << ctype << "stype" << sol_type << US_ModelRecord::stype_text(sol_type);

   if (ctype == CTYPE_SL) {
      US_ModelRecord::compute_slines(xmin, xmax, ymin, ymax, nypts, nlpts, parlims, mrecs);
   }
   else if (ctype == CTYPE_IS || ctype == CTYPE_DS) {
      US_ModelRecord::compute_sigmoids(ctype, xmin, xmax, ymin, ymax, nypts, nlpts, parlims, mrecs);
   }
   else if (ctype == CTYPE_HL) {
      nlmodl = nypts;

      US_ModelRecord::compute_hlines(xmin, xmax, ymin, ymax, nypts, nlpts, parlims, mrecs);
   }
   else if (ctype == CTYPE_2O) {
      nlmodl *= nypts;

      US_ModelRecord::compute_2ndorder(xmin, xmax, ymin, ymax, nypts, nlpts, parlims, mrecs);
   }
   DbgLv(1) << "AC:CM: mrecs0.sol0 x y z c" << mrecs[ 0 ].isolutes[ 0 ].x << mrecs[ 0 ].isolutes[ 0 ].y
            << mrecs[ 0 ].isolutes[ 0 ].z << mrecs[ 0 ].isolutes[ 0 ].c;

   QString amsg = tr("The number of test models is %1,\n").arg(nlmodl);
   if (ctype == CTYPE_2O) {
   ...
   amsg += tr(" with each curve model consisting of %1 points.").arg(nlpts);
   te_status->setText(amsg);
...
void US_AnalysisControlPc::plot_lines() {
   ctypex = cb_curvtype->currentIndex();
   ctype = (mrecs[ 0 ].v_ctype == CTYPE_ALL) ? CTYPE_ALL : ctypes[ ctypex ];
   xmin = ct_lolimitx->value();
   xmax = ct_uplimitx->value();
   ymin = ct_lolimity->value();
   ymax = ct_uplimity->value();
   ...
   mlnplotd = new US_MLinesPlot(ymin, ymax, xmin, xmax, ctype, nypts, nlpts, bmndx);

   connect(mlnplotd, SIGNAL(destroyed(QObject *)), this, SLOT(closed(QObject *)));
   ...
   QString filepath = US_Settings::tmpDir() + "/PCSA." + edata->cell + edata->channel + edata->wavelength + ".mlines."
                      + QString::number(getpid()) + ".png";
#if QT_VERSION > 0x050000
   QPixmap pixmap = (( QWidget * ) mlnplotd)->grab();
#else
   QPixmap pixmap = QPixmap::grabWidget(mlnplotd, 0, 0, mlnplotd->width(), mlnplotd->height());
#endif
...
void US_AnalysisControlPc::scan_alpha(void) {
   ...
   if (klpts != nlpts) { // Must recompute final LM model since resol. points value has changed
      recompute_mrec();
      mrec = mrecs[ 0 ];
      nlpts = klpts;
   }
...
void US_AnalysisControlPc::fitpars_connect(bool reconn) {
   if (reconn) { // Reconnect controls
      ...
      connect(cb_curvtype, SIGNAL(activated(int)), this, SLOT(type_change()));
      connect(ct_lolimitx, SIGNAL(valueChanged(double)), this, SLOT(xlim_change()));
      connect(ct_uplimitx, SIGNAL(valueChanged(double)), this, SLOT(xlim_change()));
      connect(ct_lolimity, SIGNAL(valueChanged(double)), this, SLOT(ylim_change()));
      connect(ct_uplimity, SIGNAL(valueChanged(double)), this, SLOT(ylim_change()));
      connect(cb_z_type, SIGNAL(activated(int)), this, SLOT(ztype_change(int)));
      connect(ct_varcount, SIGNAL(valueChanged(double)), this, SLOT(compute()));
      connect(ct_cresolu, SIGNAL(valueChanged(double)), this, SLOT(reso_change()));
   }

   else { // Disconnect controls
...
void US_AnalysisControlPc::recompute_mrec() {
   ...
   DbgLv(1) << "AC:RM: mrec0 solsize" << mrec.isolutes.size() << "s0 x,y" << mrec.isolutes[ 0 ].x
            << mrec.isolutes[ 0 ].y << "sm x,y" << mrec.isolutes[ mm ].x << mrec.isolutes[ mm ].y << "sn x,y"
            << mrec.isolutes[ nn ].x << mrec.isolutes[ nn ].y;
   mrec.isolutes.clear();
   US_ZSolute isol;
   xmin = ct_lolimitx->value();
   xmax = ct_uplimitx->value();
   ...
   if (ctype == CTYPE_SL) {
      ...
      for (int kk = 0; kk < nlpts; kk++) { // Loop over points on a line
         isol.x = xval * xscl;
         isol.y = yval * yscl;
         mrec.isolutes << isol;
         xval += xinc;
      ...
      for (int kk = 0; kk < nlpts; kk++) { // Loop over points on a sigmoid curve
      ...
      for (int ll = 0; ll < nlpts; ll++) { // Loop over points on a power law curve
         isol.x = xval * xscl;
   ...
   DbgLv(1) << "AC:RM: NEW mrec0 solsize" << mrec.isolutes.size() << "s0 x,y" << mrec.isolutes[ 0 ].x
            << mrec.isolutes[ 0 ].y << "sm x,y" << mrec.isolutes[ mm ].x << mrec.isolutes[ mm ].y << "sn x,y"
            << mrec.isolutes[ nn ].x << mrec.isolutes[ nn ].y;
   if (processor != 0)
...
int US_AnalysisControlPc::memory_check() {
   ...
   DbgLv(1) << "MEMck: memtot,ava,use,safe,need" << memtot << memava << memuse << memsafe << memneed << "dsz msz ns nt"
            << idsz << imsz << nsols << nthrds;
...
void US_AnalysisControlPc::select_x_axis(int ival) {
   ...
   if (attr_x == attr_y) { // If new X same as current Y, change Y
...
void US_AnalysisControlPc::adjust_xyz(const int chg_ndx) {
   ...
   if (chg_ndx == 1) { // X has just changed
      ct_lolimitx->setValue(xlow);
      ct_uplimitx->setValue(xhigh);
   }
   else if (chg_ndx == 2) { // Y has just changed
      ct_lolimity->setValue(ylow);
      ct_uplimity->setValue(yhigh);
   }
   else { // Z has just changed
      double zdef = (attr_z == US_ZSolute::ATTR_V) ? dsets[ 0 ]->vbar20 : vdefs[ attr_z ];
...
void US_AnalysisControlPc::set_solute_type() {
   ...
   sol_type = (attr_x << 6) + (attr_y << 3) + attr_z;
   DbgLv(1) << "AC:SST: ztx x,y,z" << zt_ndx << attr_x << attr_y << attr_z << "sol_type" << sol_type
