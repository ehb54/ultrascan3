...
void US_AnalysisControlPc::start() {
  ...
  DbgLv(1) << "AnaC: need_fit need_fnl" << need_fit << need_final << "alpha"
           << alpha;
  ...
  connect(processor, SIGNAL(progress_update(double)), this,
          SLOT(update_progress(double)));
  ...
  if (need_fit)
    processor->start_fit(slo, sup, klo, kup, nvar, res, typ, nthr, noif, lmmxc,
                         gfits, gfthr, alpha);
...
void US_AnalysisControlPc::advanced() {
  ...
  if (aadiag->exec() == QDialog::Accepted) {
    ...
    DbgLv(1) << "AC:advanced: mrec0 sols" << mrecs[0].csolutes.size()
             << "mrecs size" << mrecs.size() << "mrecs_mc size"
             << mrecs_mc.size() << "model compsize" << model->components.size();
    ...
    if (state != 0) {  // Where advanced controls have made changes, copy them
      ...
      DbgLv(1) << "AC: (B)stype" << sol_type << "zcoeff0"
               << dsets[0]->zcoeffs[0];
...
void US_AnalysisControlPc::close_all() {
  ...
  if ((QObject*)mlnplotd != (QObject*)0) mlnplotd->close();

  if (processor != 0) delete processor;

  accept();
}

// Public close slot
void US_AnalysisControlPc::close() { close_all(); }
...
void US_AnalysisControlPc::ylim_change() { compute(); }

// Handle change in resolution count
void US_AnalysisControlPc::reso_change() {
  DbgLv(1) << "RESO_CHANGE: need_fit" << need_fit << "reso"
           << ct_cresolu->value();
  if (need_fit) compute();
}
...
void US_AnalysisControlPc::completed_process(int stage) {
  ...
  if (stage == 7) {  // If an alpha scan can now be done, report L-M info and
                     // mark scan-ready
    US_ModelRecord mrec;
...
void US_AnalysisControlPc::compute() {
  ...
  if (ctype == CTYPE_SL) {
    ...
    US_ModelRecord::compute_sigmoids(ctype, xmin, xmax, ymin, ymax, nypts,
                                     nlpts, parlims, mrecs);
    ...
    US_ModelRecord::compute_2ndorder(xmin, xmax, ymin, ymax, nypts, nlpts,
                                     parlims, mrecs);
  }
  DbgLv(1) << "AC:CM: mrecs0.sol0 x y z c" << mrecs[0].isolutes[0].x
           << mrecs[0].isolutes[0].y << mrecs[0].isolutes[0].z
...
void US_AnalysisControlPc::plot_lines() {
  ...
  if (mlnplotd != 0) mlnplotd->close();
  DbgLv(1) << "PL:  mlnplotd closed";

  mlnplotd =
      new US_MLinesPlot(ymin, ymax, xmin, xmax, ctype, nypts, nlpts, bmndx);
  ...
  QString filepath = US_Settings::tmpDir() + "/PCSA." + edata->cell +
                     edata->channel + edata->wavelength + ".mlines." +
                     QString::number(getpid()) + ".png";
#if QT_VERSION > 0x050000
  QPixmap pixmap = ((QWidget*)mlnplotd)->grab();
#else
  QPixmap pixmap = QPixmap::grabWidget(mlnplotd, 0, 0, mlnplotd->width(),
                                       mlnplotd->height());
#endif
...
void US_AnalysisControlPc::closed(QObject* o) {
  ...
  if (oname.contains("MLinesPlot")) mlnplotd = 0;
}
...
void US_AnalysisControlPc::scan_alpha(void) {
  ...
  if (klpts != nlpts) {  // Must recompute final LM model since resol. points
                         // value has changed
    recompute_mrec();
    mrec = mrecs[0];
    nlpts = klpts;
...
QString US_AnalysisControlPc::fitpars_string() {
  ...
  return QString().sprintf("%d %.5e %.5e %.5e %.5e %d %d", typ, xlo, xup, ylo,
                           yup, nvar, noif);
}
...
void US_AnalysisControlPc::fitpars_connect(bool reconn) {
  if (reconn) {  // Reconnect controls
    ...
    connect(bg_y_axis, SIGNAL(buttonReleased(int)), this,
            SLOT(select_y_axis(int)));
    connect(cb_curvtype, SIGNAL(activated(int)), this, SLOT(type_change()));
    connect(ct_lolimitx, SIGNAL(valueChanged(double)), this,
            SLOT(xlim_change()));
    connect(ct_uplimitx, SIGNAL(valueChanged(double)), this,
            SLOT(xlim_change()));
    connect(ct_lolimity, SIGNAL(valueChanged(double)), this,
            SLOT(ylim_change()));
    connect(ct_uplimity, SIGNAL(valueChanged(double)), this,
            SLOT(ylim_change()));
    ...
    connect(ct_cresolu, SIGNAL(valueChanged(double)), this,
            SLOT(reso_change()));
  }
...
void US_AnalysisControlPc::recompute_mrec() {
  ...
  DbgLv(1) << "AC:RM: mrec0 solsize" << mrec.isolutes.size() << "s0 x,y"
           << mrec.isolutes[0].x << mrec.isolutes[0].y << "sm x,y"
           << mrec.isolutes[mm].x << mrec.isolutes[mm].y << "sn x,y"
  ...
  if (ctype == CTYPE_SL) {
    ...
    for (int ll = 0; ll < nlpts;
         ll++) {  // Loop over points on a power law curve
      isol.x = xval * xscl;
  ...
  DbgLv(1) << "AC:RM: NEW mrec0 solsize" << mrec.isolutes.size() << "s0 x,y"
           << mrec.isolutes[0].x << mrec.isolutes[0].y << "sm x,y"
           << mrec.isolutes[mm].x << mrec.isolutes[mm].y << "sn x,y"
           << mrec.isolutes[nn].x << mrec.isolutes[nn].y;
  if (processor != 0) processor->put_mrec(mrec);
}
...
int US_AnalysisControlPc::memory_check() {
  ...
  DbgLv(1) << "MEMck: memtot,ava,use,safe,need" << memtot << memava << memuse
           << memsafe << memneed << "dsz msz ns nt" << idsz << imsz << nsols
           << nthrds;
...
void US_AnalysisControlPc::adjust_xyz(const int chg_ndx) {
  ...
  if (chg_ndx == 1) {  // X has just changed
    ...
    double zdef =
        (attr_z == US_ZSolute::ATTR_V) ? dsets[0]->vbar20 : vdefs[attr_z];
