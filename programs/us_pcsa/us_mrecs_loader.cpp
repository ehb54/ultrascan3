...
US_MrecsLoader::US_MrecsLoader(
   bool dbSrc, QString &search, VEC_MRECS &amrecs, QString &adescr, const QString eGUID, const QString arunID) :
    US_WidgetsDialog(0, 0), loadDB(dbSrc), dsearch(search), omrecs(amrecs), odescr(adescr) {
   editGUID = eGUID;
   runID = arunID;
   ...
   connect(pb_filtvmrecs, SIGNAL(clicked()), this, SLOT(list_vmrecs()));
   ...
   connect(le_mfilter, SIGNAL(returnPressed()), this, SLOT(list_vmrecs()));
   ...
   QGridLayout *lo_edit = us_checkbox(tr("Filter by Edit"), ck_edit, do_edit);
...
int US_MrecsLoader::load_mrecs(VEC_MRECS &mrecs, int index) {
   ...
   if (loadDB) {
      ...
      if (dbP->lastErrno() == US_DB2::OK && dbP->numRows() == 1) {
         ...
         analID = anedit.left(7) + "_" + anadat.left(7) + "_" + antype + "_" + anrqid;
         QString fname = "pcsa-mrs." + triple + "." + analID + ".xml";
         QString fpath = US_Settings::resultDir() + "/" + frunID + "/" + fname;
         QFile filemr(fpath);
         if (filemr.exists()) { // File exists:  check it size
            ...
            if (fcksm != dcksm || fsize != dsize) { // Differs in checksum or size:  so overwrite existing file
   ...
   nmrec = US_ModelRecord::load_modelrecs(xmlr, mrecs, mrdesc, ctype, xmin, xmax, ymin, ymax, stype);
   rc = (nmrec > 0) ? 0 : 1;
...
QString US_MrecsLoader::concat_description(int index) {
   ...
   QString cdesc = sep + mrecs_descriptions[ index ].description + sep + mrecs_descriptions[ index ].filename + sep
                   + mrecs_descriptions[ index ].mrecsGUID + sep + mrecs_descriptions[ index ].DB_id + sep
                   + mrecs_descriptions[ index ].editGUID;
...
void US_MrecsLoader::list_vmrecs() {
   ...
   qDebug() << "LM: desc edit" << listdesc << listedit << "editGUID" << editGUID;
   int kmmnew = 0;
   int kmmold = 0;
   int kmmmod = 0;
   ...
   if (listdesc) { // filter is not empty
      ...
      if (listedit && !can_edit) { // disallow edit filter if no edit GUID
         QMessageBox::information(this, tr("Edit GUID Problem"), tr("No EditGUID given.\nEdit filter turned off."));
         listedit = false;
         listdesc = listedit;
         listall = !listdesc;
      }
   ...
   if (loadDB) { // Mrecs list from DB
      ...
      qDebug() << "a_m size" << mrecs_descrs_recs.size() << "m_d size" << mrecs_descriptions.size();

      QApplication::restoreOverrideCursor();
   }

   else { // Mrecs list from local disk files
      QDir dir;
      ...
      if (runID.isEmpty()) { // Scan all results directories
         ...
         for (int jj = 0; jj < r_dirs.size(); jj++) {
            ...
            QStringList rf_names = rdir.entryList(filter, QDir::Files, QDir::Name);
      ...
      else { // Scan a specific results run directory
      ...
      if (f_paths.size() != mrecs_descrs_recs.size()) { // only re-fetch all-models list if we don't yet have it
         ...
         for (int ii = 0; ii < f_paths.size(); ii++) {
            ...
            while (!xml.atEnd()) { // Search XML elements for description and editGUID
               xml.readNext();
               ...
               if (xml.isStartElement() && xml.name() == "modelrecords") {
                  ...
                  attr = xml.attributes();
                  mrGUID = (!mrGUID.isEmpty()) ? mrGUID : attr.value("mrecGUID").toString();
                  edGUID = (!edGUID.isEmpty()) ? edGUID : attr.value("editGUID").toString();
                  moGUID = (!moGUID.isEmpty()) ? moGUID : attr.value("modelGUID").toString();
                  descript = (!descript.isEmpty()) ? descript : attr.value("description").toString();
               }
            ...
            if (descript.isEmpty()) {
               descript = QString(fpath).section("/", -2, -2) + ".triple." + QString(fname).replace(".xml", ".mrecs");
            }
            ...
            if (desc.description.simplified().length() < 2) {
               desc.description = " ( Fname " + QString(fname).left(20) + tr(" : empty description )");
            }
            //qDebug() << "   desc" << desc.description << "DB_id" << desc.DB_id;
   ...
   if (listall) { // No filtering or filter by edit already done
      ...
      for (int jj = 0; jj < mrecs_descrs_recs.size(); jj++) {
         if (mrecs_descrs_recs[ jj ].description.contains(mpart)) { // description filter matches
            mrecs_descriptions << mrecs_descrs_recs[ jj ];
   ...
   if (mrecs_descriptions.size() > 0) {
      for (int ii = 0; ii < mrecs_descriptions.size(); ii++) { // propagate list widget with descriptions
         lw_vmrecs->addItem(mrecs_descriptions[ ii ].description);
         maxlch = qMax(maxlch, mrecs_descriptions[ ii ].description.length());
      }
   ...
   connect(lw_vmrecs, SIGNAL(currentRowChanged(int)), this, SLOT(row_selected(int)));
   connect(le_mfilter, SIGNAL(textChanged(const QString &)), this, SLOT(msearch(const QString &)));
}
...
void US_MrecsLoader::accepted() {
   ...
   if ((sel_row = lw_vmrecs->currentRow()) >= 0) { // Get row of selection then index in original descriptions list
      ...
      QMessageBox::information(
         this, tr("No Mrecs Selected"), tr("You have not selected a model.\nSelect+Accept or Cancel"));
      return;
   }
...
void US_MrecsLoader::show_mrecs_info() {
   ...
   if ((sel_row = lw_vmrecs->currentRow()) >= 0) { // Row selected:  build information for single model records entry
      ...
      mrcid = frDisk ? mrecs_descriptions[ mdx ].filename : // ID is filename
                 mrecs_descriptions[ mdx ].DB_id; // ID is DB id
      mrcid = mrcid.length() < 50 ? mrcid : "*/" + mrcid.section("/", -3, -1); // short filename

      dtext = tr("Mrecs Information:") + tr("\n  Description:       ") + mdesc + tr("\n  Implied RunID:     ") + runid
              + tr("\n  Mrecs ID:          ") + mrcid + tr("\n  Mrecs Global ID:   ") + mrec.mrecGUID
              + tr("\n  Type:              ") + tdesc + "  (" + QString::number(( int ) mtype) + ")"
              + tr("\n  Model Global ID:   ") + mrec.modelGUID + tr("\n  Edit Global ID:    ") + mrec.editGUID
              + tr("\n  List Row:          ") + QString::number(row + 1) + tr("\n  Mrecs xmin:        ")
              + QString::number(mrec.xmin) + tr("\n  Mrecs xmax:        ") + QString::number(mrec.xmax)
              + tr("\n  Mrecs ymin:        ") + QString::number(mrec.ymin) + tr("\n  Mrecs ymax:        ")
              + QString::number(mrec.ymax) + tr("\n  Mrecs count:       ") + QString::number(mrecs.count())
              + tr("\n  Best Model Type:   ") + US_ModelRecord::ctype_text(mrec.ctype) + tr("\n  Best Model taskx:  ")
              + QString::number(mrec.taskx) + tr("\n  Best Model str_y:  ") + QString::number(mrec.str_y)
              + tr("\n  Best Model end_y:  ") + QString::number(mrec.end_y) + tr("\n  Best Model par1:   ")
              + QString::number(mrec.par1) + tr("\n  Best Model par2:   ") + QString::number(mrec.par2)
              + tr("\n  Best Model RMSD:   ") + QString::number(mrec.rmsd) + tr("\n  BM C_Solutes size: ")
              + QString::number(mrec.csolutes.size()) + tr("\n  BM I_Solutes size: ")
...
void US_MrecsLoader::delete_mrecs() {
   ...
   if (loadDB) { // Delete an entry from the database
      if (dbP == NULL) {
         US_Passwd pw;
         dbP = new US_DB2(pw.getPasswd());
      ...
      if (dbP->statusQuery(qry) == 0) {
         QMessageBox::information(
            this, tr("Successful DB Delete"),
            tr("The mrecs record '%1' has been deleted from the database.").arg(mdesc));
         ...
         QMessageBox::critical(
            this, tr("DB Delete Problem"),
            tr("There was an error deleting an entry from the database:\n") + dbP->lastError());
      }
   }

   else { // Delete a local file
      QString fpath = mrecs_descriptions[ mdx ].filename;
      QFile filemr(fpath);
      if (filemr.exists()) {
         if (filemr.remove()) {
            QMessageBox::information(
               this, tr("Successful Local File Delete"), tr("The mrecs file '%1' has been deleted.").arg(fpath));
            ...
            QMessageBox::critical(
               this, tr("Error in Local File Delete"),
               tr("The mrecs file '%1' was not deleted"
                  " due to a remove error.")
                  .arg(fpath));
         }
      }
