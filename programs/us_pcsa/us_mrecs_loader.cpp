...
US_MrecsLoader::US_MrecsLoader(bool dbSrc, QString& search, VEC_MRECS& amrecs,
...
int US_MrecsLoader::load_mrecs(VEC_MRECS& mrecs, int index) {
  ...
  if (loadDB) {
    ...
    if (dbP->lastErrno() == US_DB2::OK && dbP->numRows() == 1) {
      ...
      analID =
          anedit.left(7) + "_" + anadat.left(7) + "_" + antype + "_" + anrqid;
      ...
      if (filemr.exists()) {  // File exists:  check it size
        ...
        if (fcksm != dcksm || fsize != dsize) {  // Differs in checksum or size:
                                                 // so overwrite existing file
  ...
  nmrec = US_ModelRecord::load_modelrecs(xmlr, mrecs, mrdesc, ctype, xmin, xmax,
                                         ymin, ymax, stype);
  rc = (nmrec > 0) ? 0 : 1;
...
QString US_MrecsLoader::concat_description(int index) {
  ...
  QString cdesc = sep + mrecs_descriptions[index].description + sep +
                  mrecs_descriptions[index].filename + sep +
                  mrecs_descriptions[index].mrecsGUID + sep +
                  mrecs_descriptions[index].DB_id + sep +
...
void US_MrecsLoader::list_vmrecs() {
  ...
  qDebug() << "listall" << listall << "listdesc listedit" << listdesc
           << listedit;
  ...
  if (loadDB) {  // Mrecs list from DB
    ...
    qDebug() << "a_m size" << mrecs_descrs_recs.size() << "m_d size"
             << mrecs_descriptions.size();
    ...
    if (f_paths.size() !=
        mrecs_descrs_recs
            .size()) {  // only re-fetch all-models list if we don't yet have it
      ...
      for (int ii = 0; ii < f_paths.size(); ii++) {
        ...
        while (
            !xml.atEnd()) {  // Search XML elements for description and editGUID
          xml.readNext();
        ...
        if (do_edit && edGUID != editGUID) continue;

        if (descript.isEmpty()) {
          descript = QString(fpath).section("/", -2, -2) + ".triple." +
  ...
  if (listall) {  // No filtering or filter by edit already done
    ...
    for (int jj = 0; jj < mrecs_descrs_recs.size(); jj++) {
      if (mrecs_descrs_recs[jj].description.contains(
              mpart)) {  // description filter matches
        mrecs_descriptions << mrecs_descrs_recs[jj];
  ...
  if (mrecs_descriptions.size() > 0) {
    for (int ii = 0; ii < mrecs_descriptions.size();
  ...
  connect(lw_vmrecs, SIGNAL(currentRowChanged(int)), this,
          SLOT(row_selected(int)));
  connect(le_mfilter, SIGNAL(textChanged(const QString&)), this,
          SLOT(msearch(const QString&)));
}
...
void US_MrecsLoader::accepted() {
  ...
  if ((sel_row = lw_vmrecs->currentRow()) >=
      0) {  // Get row of selection then index in original descriptions list
...
void US_MrecsLoader::show_mrecs_info() {
  ...
  if ((sel_row = lw_vmrecs->currentRow()) >=
      0) {  // Row selected:  build information for single model records entry
    ...
    mrcid = mrcid.length() < 50
                ? mrcid
                : "*/" + mrcid.section("/", -3, -1);  // short filename

    dtext =
        tr("Mrecs Information:") + tr("\n  Description:       ") + mdesc +
        tr("\n  Implied RunID:     ") + runid + tr("\n  Mrecs ID:          ") +
        ...
        tr("\n  Best Model Type:   ") + US_ModelRecord::ctype_text(mrec.ctype) +
        ...
        tr("\n  BM C_Solutes size: ") + QString::number(mrec.csolutes.size()) +
        tr("\n  BM I_Solutes size: ") + QString::number(mrec.isolutes.size()) +
        "";
  }
...
void US_MrecsLoader::delete_mrecs() {
  if ((sel_row = lw_vmrecs->currentRow()) < 0) return;
  ...
  if (loadDB) {  // Delete an entry from the database
    ...
    if (filemr.exists()) {
      if (filemr.remove()) {
        QMessageBox::information(
            this, tr("Successful Local File Delete"),
            tr("The mrecs file '%1' has been deleted.").arg(fpath));

        list_vmrecs();
      }

      else {
        QMessageBox::critical(this, tr("Error in Local File Delete"),
