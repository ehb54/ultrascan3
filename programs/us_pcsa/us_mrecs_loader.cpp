...
int
US_MrecsLoader::load_mrecs (VEC_MRECS &mrecs, int index)
{
  int rc = 0;
  int nmrec = 0;
  QString xmlstr;
      ...
      if (dbP->lastErrno () == US_DB2::OK && dbP->numRows () == 1)
        {
          ...
          analID = anedit.left (7) + "_" + anadat.left (7) + "_" + antype + "_"
                   + anrqid;
          QString fname = "pcsa-mrs." + triple + "." + analID + ".xml";
          QString fpath
              = US_Settings::resultDir () + "/" + frunID + "/" + fname;
          QFile filemr (fpath);
          if (filemr.exists ())
  ...
  nmrec = US_ModelRecord::load_modelrecs (xmlr, mrecs, mrdesc, ctype, xmin,
                                          xmax, ymin, ymax, stype);
  rc = (nmrec > 0) ? 0 : 1;
...
QString
US_MrecsLoader::concat_description (int index)
{
...
US_MrecsLoader::list_vmrecs ()
{
  ...
  qDebug () << "LM: desc edit" << listdesc << listedit << "editGUID"
            << editGUID;
  int kmmnew = 0;
  int kmmold = 0;
  int kmmmod = 0;
          ...
          for (int jj = 0; jj < r_dirs.size (); jj++)
            {
              ...
              QStringList rf_names
                  = rdir.entryList (filter, QDir::Files, QDir::Name);
                  ...
                  if (xml.isStartElement () && xml.name () == "modelrecords")
                    {
                      ...
                      descript = attr.value ("description").toString ();
                           ...
                           && xml.name () == "modelrecord")
                    {
                      attr = xml.attributes ();
                      mrGUID = (!mrGUID.isEmpty ())
                                   ? mrGUID
                                   : attr.value ("mrecGUID").toString ();
                      edGUID = (!edGUID.isEmpty ())
                                   ? edGUID
                                   : attr.value ("editGUID").toString ();
                      moGUID = (!moGUID.isEmpty ())
                                   ? moGUID
...
int
US_MrecsLoader::mrecsIndex (QString mdesc, QList<MrecsDesc> mds)
{
  int mdx = 0;
...
void
US_MrecsLoader::show_mrecs_info ()
{
  int mtype;
...
void
US_MrecsLoader::msearch (const QString &search_string)
{
...
void
US_MrecsLoader::change_edit (bool ckedit)
{
  do_edit = ckedit;
...
void
US_MrecsLoader::row_selected (int row)
{
  sel_row = row;
