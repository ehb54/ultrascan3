...
void WorkerThreadPc::get_result(WorkPacketPc &workout) {
   ...
   if (depth == 0) {
      DbgLv(1) << "PC(WT): thr nn" << thrn << nn << "out sol0 solk soln" << workout.csolutes[ 0 ].c
               << workout.csolutes[ kk ].c << workout.csolutes[ nn - 1 ].c << "ni sol0 soln x" << ni
               << solutes_i[ 0 ].x * 1.e13 << solutes_i[ ni - 1 ].x * 1.e13 << "c" << solutes_i[ 0 ].c
               << solutes_i[ ni - 1 ].c;
   }
   else {
      DbgLv(1) << "PC(WT): thr nn" << thrn << nn << "ni sol0 soln x" << ni << solutes_i[ 0 ].x * 1.e13
               << solutes_i[ ni - 1 ].x * 1.e13 << "c" << solutes_i[ 0 ].c << solutes_i[ ni - 1 ].c;
   }
   //*DEBUG*
   mutex.unlock();
}
...
void WorkerThreadPc::calc_residuals() {
   ...
   if (depth == 0) { // Fit task:  do full compute of model
      ...
      for (int js = 0; js < ns; js++) {
         if (js < 4 || (js + 5) > ns)
            DbgLv(1) << phdr << "  soli: js" << js << " sol.x,y,z,c" << solutes_i[ js ].x << solutes_i[ js ].y
                     << solutes_i[ js ].z << solutes_i[ js ].c;
      }
      ...
      DbgLv(1) << phdr << "sim,res ptCounts" << sim_vals.sim_data.pointCount() << sim_vals.residuals.pointCount();
   }
      ...
      apply_alpha(alpha, psv_nnls_a, psv_nnls_b, nscans, npoints, nisols, variance, xnormsq);
...
void WorkerThreadPc::apply_alpha(
   ...
   DbgLv(1) << phdr << "  simdat fill ktot,kavl,naro" << ktot << kavl << narows << "rss_now" << US_Memory::rss_now();
   ...
   DbgLv(1) << phdr << "    ntot ncsols" << ntotal << ncsols << "varisum" << variance;

   // Return computed variance and xnorm-sq
   variance /= ( double ) ntotal;
   DbgLv(1) << phdr << " alpha" << alpha << "vari xnsq" << variance << xnormsq;
   int mm = npoints / 2;
   DbgLv(1) << phdr << " mm=" << mm << "a[m] b[m] s[m]" << (*psv_nnls_a)[ mm ] << (*psv_nnls_b)[ mm ] << simdat[ mm ]
