...
WorkerThreadPc::~WorkerThreadPc ()
{
  ...
  DbgLv (1) << "PC(WT):   Thread destroy - (1)finished?" << isFinished ()
            << thrn;
  if (!wait (2000))
    {
      DbgLv (1) << "Thread destroy wait timeout(2secs) : Thread" << thrn;
    }
  DbgLv (1) << "PC(WT):   Thread destroy - (2)finished?" << isFinished ()
            << thrn;
...
void
WorkerThreadPc::define_work (WorkPacketPc &workin)
{
  mutex.lock ();
  str_y = workin.str_y;
  end_y = workin.end_y;
...
void
WorkerThreadPc::get_result (WorkPacketPc &workout)
{
  mutex.lock ();
...
WorkerThreadPc::calc_residuals ()
{
  ...
  if (depth == 0)
    { // Fit task:  do full compute of model
      ...
      DbgLv (1) << phdr << "sim,res ptCounts"
                << sim_vals.sim_data.pointCount ()
...
void
WorkerThreadPc::forward_progress (int steps)
{
  emit work_progress (steps);
}

void
WorkerThreadPc::apply_alpha (const double alpha, QVector<double> *psv_nnls_a,
