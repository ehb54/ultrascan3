...
void
US_pcsaProcess::start_fit (double xll, double xul, double yll, double yul,
                           ...
                           int lmmxc, int gfits, double gfthr, double alf)
{
  ...
  DbgLv (1) << "PC: alf alpha lm fx scn" << alf << alpha << alpha_lm
            << alpha_fx << alpha_scn;
  ...
  DbgLv (1) << "STR_STR: nowmem" << memmb << "kthr ksol" << nthreads
            << cresolu;
  ...
  DbgLv (1) << "PC:   kstask nthreads" << kstask << nthreads
            << job_queue.size () << "(2)maxrss" << maxrss;
...
void
US_pcsaProcess::final_fit (double alf)
{
...
void
US_pcsaProcess::process_fxfinal (US_ModelRecord &mrec)
{
  if (abort)
    return;
  ...
  DbgLv (1) << "FIN_FIN:    c0 cn" << mrec.csolutes[0].c
            << mrec.csolutes[qMax (0, nsolutes - 1)].c << "  nsols"
            << nsolutes;
  nscans = edata->scanCount ();
  npoints = edata->pointCount ();
  ...
  DbgLv (1) << "done: vari bvol" << mrec.variance << bvol << "slun klun"
            << xlolim << xuplim << ylolim << yuplim << nypts << "ets"
            << ktimes;
  ...
  pmsg += tr ("   Maximum memory used:  ") + QString::number (memmb)
          + " MB\n\n"
  ...
  rd_frac = (fi_iter > 1) ? qAbs ((pfi_rmsd - cfi_rmsd) / pfi_rmsd)
                          : rd_thresh * 2;
  rmsds << cfi_rmsd;
...
bool
US_pcsaProcess::get_results (US_DataIO::RawData *da_sim,
                             US_DataIO::RawData *da_res, US_Model *da_mdl,
                             US_Noise *da_tin, US_Noise *da_rin, int &bm_ndx,
...
void
US_pcsaProcess::get_mrec (US_ModelRecord &p_mrec)
{
...
void
US_pcsaProcess::put_mrec (US_ModelRecord &a_mrec)
{
...
void
US_pcsaProcess::put_mrecs (QVector<US_ModelRecord> &a_mrecs)
{
  ...
  nypts = (curvtype != CTYPE_HL && curvtype != CTYPE_2O)
              ...
              : nypts;
  nypts = (curvtype == CTYPE_2O) ? (qRound (pow ((double)nypts, 0.3333333)))
                                 : nypts;
  alpha = 0.0;
...
// Submit a job
void
US_pcsaProcess::submit_job (WorkPacketPc &wtask, int thrx)
{
  wtask.thrn = thrx + 1;
...
void
US_pcsaProcess::process_job (WorkerThreadPc *wthrd)
{
  if (abort)
    return;
  ...
  if (variance < varimin)
    { // Handle a new minimum variance record
      if (minvarx < orecx)
        { // Clear vectors from the previous minimum
          ...
          DbgLv (1) << "PJ: CLEAR: VARI VMIN ORECX" << variance << varimin
                    << orecx;
        }
    ...
    { // Clear vectors from a model record that is not minimum-variance
      DbgLv (1) << "PJ:  CLEAR: VARI VMIN MVARX" << variance << varimin
                << minvarx;
      mrec.clear_data ();
    }

  mrecs << mrec; // Append to the vector of model records
  DbgLv (1) << "THR_FIN:  taskx minvarx varimin" << taskx << minvarx
            << varimin;
  ...
  int dfact = (nsoln + 4) * sizeof (double);
  int memneed = qRound ((double)dsize * (double)dfact / (1024. * 1024.));
  DbgLv (1) << "PC:MEM: (4)nsol,nscn,npt,dsize,dfact,memneed" << nsoln
            << nscann << npointn << dsize << dfact << memneed;
  memuse = memuse + memneed;
  memava = memtot - memuse;
  mempav = (memava * 100) / memtot;
...
void
US_pcsaProcess::queue_task (WorkPacketPc &wtask, double stry, double endy,
...
void
US_pcsaProcess::free_worker (int tx)
{
...
int
US_pcsaProcess::slmodels (int ctp, double xlo, double xup, double ylo,
...
int
US_pcsaProcess::sigmodels (int ctp, double xlo, double xup, double ylo,
...
int
US_pcsaProcess::pl2models (double xlo, double xup, double ylo, double yup,
                           int nyp, int nlpts)
{
  DbgLv (1) << "2OMO: xlo xup ylo yup nyp nlp" << xlo << xup << ylo << yup
            << nyp << nlpts;
  // double vbar20   = dsets[ 0 ]->vbar20;
  orig_sols.clear ();

  // Compute 2nd-order model records
  int nmodels = US_ModelRecord::compute_2ndorder (xlo, xup, ylo, yup, nyp,
                                                  nlpts, parlims, mrecs);
...
void
US_pcsaProcess::model_statistics (QVector<US_ModelRecord> &mrecs,
                                  QStringList &modstats)
{
  const char *ctp[] = { "None",
                        "Straight Line",
...
double
US_pcsaProcess::fit_function_SL (double t, double *par)
{
...
double
US_pcsaProcess::fit_function_IS (double t, double *par)
{
  static int ffcall = 0;  // Fit function call counter
...
double
US_pcsaProcess::fit_function_DS (double t, double *par)
{
  static int ffcall = 0;  // Fit function call counter
  static double epar[18]; // Static array for holding parameters
  static const int nepar = sizeof (epar) / sizeof (epar[0]);
...
double
US_pcsaProcess::fit_function_HL (double t, double *par)
{
  static int ffcall = 0;  // Fit function call counter
  static double epar[18]; // Static array for holding parameters
  static const int nepar = sizeof (epar) / sizeof (epar[0]);
...
maxp2=(maxp2+par2up)*0.5;
  ...
  for (int ii = 0; ii < nsol; ii++)
    {
      ...
      DbgLv (1) << "LMf:  ii" << ii << "x y c" << solute.x << solute.y
                << solute.c;
  ...
  DbgLv (0) << "LMf:insert-new: old par1 par2" << mrecs[0].par1
            << mrecs[0].par2 << "new par1 par2" << mrec.par1 << mrec.par2
            << "nmtasks mrec-size" << nmtasks << mrecs.size ();
  ...
  DbgLv (1) << "LMf: resids nsc npt" << resids->scanCount ()
            << resids->pointCount ();
  DbgLv (1) << "LMf: rdata  nsc npt" << rdata.scanCount ()
            << rdata.pointCount ();
  DbgLv (1) << "LMf: sdata  nsc npt" << sdata.scanCount ()
            << sdata.pointCount ();
      ...
      for (int rr = 0; rr < npoints; rr++)
        {
          ...
          if ((ss < 3 && rr < 3) || ((ss + 4) > nscans && (rr + 4) > npoints)
            ...
            DbgLv (1) << "LMf:  ss rr" << ss << rr << "edat sdat resv"
                      << edata->value (ss, rr) << sdata.value (ss, rr)
                      << resval;
          cvari += sq (resval);
        }
    }
...
void
US_pcsaProcess::elite_limits (QVector<US_ModelRecord> &mrecs, double &minyv,
...
double
US_pcsaProcess::evaluate_model (QList<US_SolveSim::DataSet *> &dsets,
                                US_SolveSim::Simulation &sim_vals)
{
  US_SolveSim::DataSet *dset = dsets[0];
  US_SimulationParameters *spar = &dset->simparams;
...
#endif
      US_ZSolute::set_mcomp_values (mcomp, sim_vals.zsolutes[ii], st_mask,
                                    true);
  ...
  if (sim_vals.noisflag != 0)
    {
      int ntin
          = ((sim_vals.noisflag & 1) == 0) ? 0 : sim_vals.ti_noise.size ();
      int nrin
          = ((sim_vals.noisflag & 2) == 0) ? 0 : sim_vals.ri_noise.size ();
...
void
US_pcsaProcess::timerEvent (QTimerEvent *event)
{
...
US_pcsaProcess::restart_fit ()
{
  ...
  max_rss ();
  kstask = nthreads; // count of started tasks is initially thread count
  DbgLv (1) << "RF:   kstask nthreads" << kstask << nthreads
            << job_queue.size ();
  int mempav = US_Memory::memory_profile ();
