...
US_pcsaProcess::US_pcsaProcess(QList<US_SolveSim::DataSet *> &dsets, QObject *parent /*=0*/) :
...
void US_pcsaProcess::start_fit(
   double xll, double xul, double yll, double yul, int nyp, int res, int typ, int nth, int noi, int lmmxc, int gfits,
   ...
   if (alpha < 0.0) { // Negative alpha acts as flag
      if (alpha == (-99.0)) { // This is an alpha scan run
         alpha_scn = true;
         alpha = 0.0;
      }

      else { // This is a run with regularize in earlier stage(s)
         alpha = -alpha;
   ...
   DbgLv(1) << "PC: alf alpha lm fx scn" << alf << alpha << alpha_lm << alpha_fx << alpha_scn;
   ...
   DbgLv(1) << "PC: sll sul" << xlolim << xuplim << " yll yul nyp" << ylolim << yuplim << nypts << " type reso nth noi"
            << curvtype << cresolu << nthreads << noisflag;
   ...
   if (curvtype == CTYPE_SL || curvtype == CTYPE_HL) { // Determine models for straight-line curves
      nmtasks = slmodels(curvtype, xlolim, xuplim, ylolim, yuplim, nypts, cresolu);
   }

   else if (curvtype == CTYPE_IS || curvtype == CTYPE_DS) { // Determine models for sigmoid curves
      nmtasks = sigmodels(curvtype, xlolim, xuplim, ylolim, yuplim, nypts, cresolu);
   }

   else if (curvtype == CTYPE_2O) { // Determine models for 2nd-order power law curves
   ...
   memneed *= nthreads;
   DbgLv(1) << "PC:MEM(1): pav" << mempav << "ns dsz dfac mneed" << nsoln << dsize << dfact << memneed;
   memuse = memuse + memneed;
   memava = memtot - memuse;
   mempav = (memava * 100) / memtot;
   DbgLv(1) << "PC:MEM(1): pav" << mempav << "ava tot use" << memava << memtot << memuse;
   ...
   DbgLv(1) << "PC:   kstask nthreads" << kstask << nthreads << job_queue.size() << "(2)maxrss" << maxrss;

   emit message_update(
      pmessage_head() + tr("Starting computations of %1 models\n using %2 threads ...").arg(nmtasks).arg(nthreads),
      false);
   max_rss();
   ...
   memava = memtot - memuse;
   mempav = (memava * 100) / memtot;
   DbgLv(1) << "PC:MEM(2): pav" << mempav << "ava tot use" << memava << memtot << memuse;
...
void US_pcsaProcess::stop_fit() {
   ...
   emit message_update(pmessage_head() + tr("All computations have been aborted."), false);
   abort = false;
}
...
void US_pcsaProcess::process_fxfinal(US_ModelRecord &mrec) {
   ...
   if ((noisflag & 1) != 0) { // copy TI noise to caller and internal vector
   ...
   if ((noisflag & 2) != 0) { // copy RI noise to caller and internal vector
...
#endif
      US_ZSolute::set_mcomp_values(mcomp, mrec.csolutes[ cc ], st_mask, true);
   ...
   DbgLv(1) << "FIN_FIN:    c0 cn" << mrec.csolutes[ 0 ].c << mrec.csolutes[ qMax(0, nsolutes - 1) ].c << "  nsols"
            << nsolutes;
   nscans = edata->scanCount();
   npoints = edata->pointCount();
   ...
   DbgLv(1) << "FIN_FIN: simdat nsc npt" << simdat->scanCount() << simdat->pointCount();
   DbgLv(1) << "FIN_FIN: resids nsc npt" << resids->scanCount() << resids->pointCount();
   DbgLv(1) << "FIN_FIN: rdata  nsc npt" << rdata.scanCount() << rdata.pointCount();
   DbgLv(1) << "FIN_FIN: sdata  nsc npt" << sdata.scanCount() << sdata.pointCount();
   ...
   int mms = nscans / 2;
   int mmr = npoints / 2;
   DbgLv(1) << "FIN_FIN: edatm sdatm rdatm" << edata->value(mms, mmr) << sdata.value(mms, mmr) << rdata.value(mms, mmr);
   ...
   DbgLv(1) << "done: vari bvol" << mrec.variance << bvol << "slun klun" << xlolim << xuplim << ylolim << yuplim
            << nypts << "ets" << ktimes;
   ...
   if (ktimeh > 0)
      pmsg += tr("%1 hr., %2 min., %3 sec.;").arg(ktimeh).arg(ktimem).arg(ktimes);

   else
      pmsg += tr("%1 min., %2 sec.;").arg(ktimem).arg(ktimes);
   ...
   pmsg += tr("   Maximum memory used:  ") + QString::number(memmb) + " MB\n\n"
           + tr("The best model (RMSD=%1, %2 solutes, index %3) is:\n").arg(mrec.rmsd).arg(nsolutes).arg(mrec.taskx);
   if (curvtype == CTYPE_SL) {
      double slopel = ( double ) (mrec.end_y - mrec.str_y) / ( double ) (xuplim - xlolim);
      ...
      pmsg += tr("  the curve with par1=%1 and par2=%2.").arg(mrec.par1).arg(mrec.par2);
      DbgLv(1) << "FIN_FIN: par1,par2" << mrec.par1 << mrec.par2;
   }

   else if (curvtype == CTYPE_HL) {
      pmsg += tr("  the line from x,y  %1, %2  to %3, %4.").arg(xlolim).arg(mrec.str_y).arg(xuplim).arg(mrec.end_y);
   }
   ...
   DbgLv(1) << "FIN_FIN: maxrss memmb nthr" << maxrss << memmb << nthreads << " nsubg noisf" << nmtasks << noisflag;
   DbgLv(1) << "FIN_FIN:   kcsteps nctotal" << kcsteps << nctotal;
   DbgLv(1) << "FIN_FIN:    alpha_fx" << alpha_fx;
   pfi_rmsd = cfi_rmsd;
   cfi_rmsd = mrecs[ 0 ].rmsd;
   rd_frac = (fi_iter > 1) ? qAbs((pfi_rmsd - cfi_rmsd) / pfi_rmsd) : rd_thresh * 2;
   rmsds << cfi_rmsd;
   ...
   if (alpha_scn) { // Signal analysis control that an alpha scan may begin
      emit process_complete(7);
      return;
   }

   if (alpha_lm == 0.0 && alpha != 0.0) { // L-M nonregularized, but need to do a final regularized computation
      compute_final();
   }
...
bool US_pcsaProcess::get_results(
   US_DataIO::RawData *da_sim, US_DataIO::RawData *da_res, US_Model *da_mdl, US_Noise *da_tin, US_Noise *da_rin,
   int &bm_ndx, QStringList &modstats, QVector<US_ModelRecord> &p_mrecs) {
   bool all_ok = true;
   model.alphaRP = alpha;
   mrecs[ 0 ].model = model;
   abort = false;
   ...
   *da_mdl = model; // copy model
   ...
   DbgLv(0) << " GET_RES:    VARI,RMSD" << mrecs[ 0 ].variance << mrecs[ 0 ].rmsd << "BM_NDX" << bm_ndx << "ALPHA"
            << alpha;
...
void US_pcsaProcess::put_mrecs(QVector<US_ModelRecord> &a_mrecs) {
   ...
   nmtasks = (mrecs[ 0 ].taskx == mrecs[ 1 ].taskx) ? (nmrecs - 1) : nmrecs;
   nmtasks = (mrecs[ 1 ].taskx == mrecs[ 2 ].taskx) ? (nmtasks - 1) : nmtasks;
   nypts = (v_ctype != CTYPE_ALL) ? nmtasks : (nmtasks / 3);
   nypts = (curvtype != CTYPE_HL && curvtype != CTYPE_2O) ? (qRound(sqrt(( double ) nypts))) : nypts;
   nypts = (curvtype == CTYPE_2O) ? (qRound(pow(( double ) nypts, 0.3333333))) : nypts;
   alpha = 0.0;
...
void US_pcsaProcess::submit_job(WorkPacketPc &wtask, int thrx) {
   ...
   connect(wthr, SIGNAL(work_complete(WorkerThreadPc *)), this, SLOT(process_job(WorkerThreadPc *)));
   DbgLv(1) << "SUBMIT_JOB taskx" << wtask.taskx << "sk ek" << wtask.str_y << wtask.end_y << "maxrss" << maxrss;
...
void US_pcsaProcess::process_job(WorkerThreadPc *wthrd) {
   ...
   //DBG-CONC
   if (dbg_level > 0)
      for (int mm = 0; mm < wresult.csolutes.size(); mm++) {
         if (wresult.csolutes[ mm ].c > 100.0) {
            DbgLv(1) << "PJ:  CONC=" << wresult.csolutes[ mm ].c << " x,y" << wresult.csolutes[ mm ].x
   ...
   if (variance < varimin) { // Handle a new minimum variance record
      if (minvarx < orecx) { // Clear vectors from the previous minimum
   ...
   else if (variance > varimin) { // Clear vectors from a model record that is not minimum-variance
   ...
   int dfact = (nsoln + 4) * sizeof(double);
   int memneed = qRound(( double ) dsize * ( double ) dfact / (1024. * 1024.));
   DbgLv(1) << "PC:MEM: (4)nsol,nscn,npt,dsize,dfact,memneed" << nsoln << nscann << npointn << dsize << dfact
            << memneed;
   memuse = memuse + memneed;
   memava = memtot - memuse;
   mempav = (memava * 100) / memtot;
   ...
   DbgLv(1) << "THR_FIN: thrn" << thrn << " taskx orecx" << taskx << orecx << " kct kst" << kctask << kstask;

   emit message_update(
      pmessage_head() + tr("Of %1 models, computations are complete for %2.").arg(nmtasks).arg(kctask), false);

   if (kctask >= nmtasks) { // All model tasks are now complete
      emit stage_complete(kcsteps, nctotal);

      emit message_update(pmessage_head() + tr("All models have now been completed. Evaluating..."), false);
   }

   if (kctask < nmtasks) { // Not the last:  add to the queue if necessary
   ...
   else { // We have done the last computation, so determine the low-rmsd result

      qSort(mrecs); // Sort model records by variance

      DbgLv(1) << "THR_FIN: thrn" << thrn << " mrecs size" << mrecs.size() << "mrec0 taskx,vari" << mrecs[ 0 ].taskx
               << mrecs[ 0 ].variance;
      process_fxfinal(mrecs[ 0 ]);
      //*DBG*
      if (dbg_level > 0) {
         for (int ii = 0; ii < mrecs.size(); ii++) {
            DbgLv(1) << "MREC:rank" << ii << "taskx" << mrecs[ ii ].taskx << "st_k en_k" << mrecs[ ii ].str_y
                     << mrecs[ ii ].end_y << "vari,rmsd" << mrecs[ ii ].variance << mrecs[ ii ].rmsd << "ncsols"
...
void US_pcsaProcess::queue_task(
   WorkPacketPc &wtask, double stry, double endy, int taskx, int noisf, QVector<US_ZSolute> isolutes) {
...
WorkPacketPc US_pcsaProcess::next_job() {
   ...
   if (jobx > 0) {
      DbgLv(1) << "NEXTJ: jobx taskx taskx0" << jobx << wtask.taskx << job_queue[ 0 ].taskx;
   }
   else {
...
int US_pcsaProcess::slmodels(int ctp, double xlo, double xup, double ylo, double yup, int nyp, int res) {
   DbgLv(1) << "SLMO: xlo xup ylo yup nyp res" << xlo << xup << ylo << yup << nyp << res;
   int nmodels = 0;
   //double vbar20   = dsets[ 0 ]->vbar20;
   orig_sols.clear();
   ...
   if (ctp == CTYPE_SL)
      nmodels = US_ModelRecord::compute_slines(xlo, xup, ylo, yup, nyp, res, parlims, mrecs);
   else if (ctp == CTYPE_HL)
      nmodels = US_ModelRecord::compute_hlines(xlo, xup, ylo, yup, nyp, res, parlims, mrecs);
...
int US_pcsaProcess::sigmodels(int ctp, double xlo, double xup, double ylo, double yup, int nyp, int nlpts) {
   DbgLv(1) << "SGMO: ctp xlo xup ylo yup nyp nlp" << ctp << xlo << xup << ylo << yup << nyp << nlpts;
   //double vbar20   = dsets[ 0 ]->vbar20;
   orig_sols.clear();

   // Compute sigmoid model records
   int nmodels = US_ModelRecord::compute_sigmoids(ctp, xlo, xup, ylo, yup, nyp, nlpts, parlims, mrecs);
...
int US_pcsaProcess::pl2models(double xlo, double xup, double ylo, double yup, int nyp, int nlpts) {
   DbgLv(1) << "2OMO: xlo xup ylo yup nyp nlp" << xlo << xup << ylo << yup << nyp << nlpts;
   //double vbar20   = dsets[ 0 ]->vbar20;
   orig_sols.clear();

   // Compute 2nd-order model records
   int nmodels = US_ModelRecord::compute_2ndorder(xlo, xup, ylo, yup, nyp, nlpts, parlims, mrecs);
...
void US_pcsaProcess::model_statistics(QVector<US_ModelRecord> &mrecs, QStringList &modstats) {
   const char *ctp[] = { "None",
                         "Straight Line",
                         "Increasing Sigmoid",
                         "Unknown-3",
   ...
   modstats << tr("Curve Type:") << QString(ctp[ curvtx ]);
   modstats << tr("Solute Type:") << US_ModelRecord::stype_text(soltype);
   modstats << tr("X Range:") << QString().sprintf("%10.4f  %10.4f", xlolim, xuplim);
   ...
   if (curvtype == CTYPE_SL || curvtype == CTYPE_HL) {
      ...
      modstats << tr("Y Range + delta:") << QString().sprintf("%10.4f  %10.4f  %10.4f", ylolim, yuplim, yincr);
      modstats << tr("Best curve Y end points + slope:")
               << QString().sprintf("%10.4f  %10.4f  %10.4f", str_y, end_y, slope);
      ...
      double par1 = exp(log(0.001) + (log(0.5) - log(0.001)) * p1off);
      double par2 = ( double ) p2ndx / krng;
      modstats << tr("Y Range:") << QString().sprintf("%10.4f  %10.4f", ylolim, yuplim);
      modstats << tr("Best curve par1 and par2:") << QString().sprintf("%10.4f  %10.4f", par1, par2);
      modstats << tr("Best curve Y end points:")
               << QString().sprintf("%10.4f  %10.4f", mrecs[ 0 ].str_y, mrecs[ 0 ].end_y);
   }
   else if (curvtype == CTYPE_2O) {
      modstats << tr("Y Range:") << QString().sprintf("%10.4f  %10.4f", ylolim, yuplim);
      modstats << tr("Best curve A, B, C:")
               << QString().sprintf("%10.4f  %10.4f  %10.4f", mrecs[ 0 ].par1, mrecs[ 0 ].par2, mrecs[ 0 ].par3);
      modstats << tr("Best curve Y end points:")
               << QString().sprintf("%10.4f  %10.4f", mrecs[ 0 ].str_y, mrecs[ 0 ].end_y);
   }
   else {
      ...
      modstats << tr("Best curve par1 and par2:")
               << QString().sprintf("%10.4f  %10.4f", mrecs[ 0 ].par1, mrecs[ 0 ].par2);
      modstats << tr("Best curve Y end points:")
               << QString().sprintf("%10.4f  %10.4f", mrecs[ 0 ].str_y, mrecs[ 0 ].end_y);
   }
   modstats << tr("Number of models:") << QString().sprintf("%5d", nmtasks);
   modstats << tr("Number of curve variations:") << QString().sprintf("%5d", nypts);
   modstats << tr("Solute points per curve:") << QString().sprintf("%5d", nlpts);
   if (nblpts != nlpts)
      modstats << tr("Best-curve points per curve:") << QString().sprintf("%5d", nblpts);
   modstats << tr("Index of best model:") << QString().sprintf("%5d", mrecs[ 0 ].taskx);
   modstats << tr("Best curve calculated solutes:") << QString().sprintf("%5d", mrecs[ 0 ].csolutes.size());
   modstats << tr("Minimum, Maximum calculated solutes:") << QString().sprintf("%5d  %5d", nsolmin, nsolmax);
   modstats << tr("Average calculated solutes:") << QString().sprintf("%5d", nsolavg);
   modstats << tr("Minimum variance:") << QString().sprintf("%12.6e", varimin);
   modstats << tr("Minimum, Maximum rmsd:") << QString().sprintf("%12.8f  %12.8f", rmsdmin, rmsdmax);
   modstats << tr("Average, Median rmsd:") << QString().sprintf("%12.8f  %12.8f", rmsdavg, rmsdmed);
   modstats << tr("Number of \"better\" models:") << QString().sprintf("%5d", nbmods);
   modstats << tr("%1 Best Min,Max calculated solutes:").arg(nbmods) << QString().sprintf("%5d  %5d", nbsomin, nbsomax);
   modstats << tr("%1 Best Average calculated solutes:").arg(nbmods) << QString().sprintf("%5d", nbsoavg);
   modstats << tr("%1 Best Minimum, Maximum rmsd:").arg(nbmods)
            << QString().sprintf("%12.8f  %12.8f", brmsmin, brmsmax);
   modstats << tr("%1 Best Average, Median rmsd:").arg(nbmods) << QString().sprintf("%12.8f  %12.8f", brmsavg, brmsmed);
   modstats << tr("Tikhonov regularization parameter:") << QString().sprintf("%12.3f", alpha);
   DbgLv(1) << "PC:MS:   RTN";
}
...
double US_pcsaProcess::fit_function_SL(double t, double *par) {
   ...
   if (t > 0.0) { // If not t[0], return immediately
      return 0.0;
   }

   if (t < 0.0) { // If t is special flag (-ve.), reset ffcall and return
      ffcall = 0;
      return 0.0;
   }
   ...
   if (ffcall == 0) { // On 1st call, copy par array to internal static one
   ...
   if (ffcall > 3) {
      ...
      if (
         par1 < p1lo || par2 < p2lo || par1 > p1hi || par2 > p2hi || ystart < ylow || yend < ylow || ystart > yhigh
         || yend > yhigh) {
         qDebug() << "ffSL: call" << ffcall << "par1 par2" << par1 << par2 << "ys ye" << ystart << yend
                  << "*OUT-OF-LIMITS*";
         return 1e+99;
      }
   }
   ...
   if (attr_x == ATTR_S) { // X is s:  scale by 1e-13
      xcurr *= 1e-13;
      xinc *= 1e-13;
   }
   if (attr_y == ATTR_S) { // Y is s:  scale by 1e-13
      ycurr *= 1e-13;
      yinc *= 1e-13;
   }
   ...
   for (int ii = 0; ii < nlpts; ii++) { // Fill the input solutes vector
   ...
   qDebug() << "ffSL: call" << ffcall << "par1 par2" << par1 << par2 << "rmsd" << rmsd << "eval time" << ktimms
            << "ms.";
   if (ffcall < 6)
      qDebug() << "ffSL:  epar0 epar1-9" << parP[ 0 ] << epar[ 1 ] << epar[ 2 ] << epar[ 3 ] << epar[ 4 ] << epar[ 5 ]
               << epar[ 6 ] << epar[ 7 ] << epar[ 8 ] << epar[ 9 ];
...
double US_pcsaProcess::fit_function_IS(double t, double *par) {
   ...
   if (ffcall > 3) {
      ...
      if (
         par1 < p1lo || par2 < p2lo || par1 > p1hi || par2 > p2hi || ystart < ylow || yend < ylow || ystart > yhigh
         || yend > yhigh) {
         qDebug() << "ffIS: call" << ffcall << "par1 par2" << par1 << par2 << "ys ye" << ystart << yend
                  << "*OUT-OF-LIMITS*";
         return 1e+99;
      }
   }
   ...
   epar[ 15 ] = rmsd;
   int ktimms = ftimer.elapsed();
   qDebug() << "ffIS: call" << ffcall << "par1 par2" << par1 << par2 << "rmsd" << rmsd << "eval time" << ktimms
            << "ms.";
   if (ffcall < 6)
      qDebug() << "ffIS:  epar0 epar1-9" << parP[ 0 ] << epar[ 1 ] << epar[ 2 ] << epar[ 3 ] << epar[ 4 ] << epar[ 5 ]
               << epar[ 6 ] << epar[ 7 ] << epar[ 8 ] << epar[ 9 ];
...
double US_pcsaProcess::fit_function_DS(double t, double *par) {
   ...
   if (ffcall > 3) {
      ...
      if (
         par1 < p1lo || par2 < p2lo || par1 > p1hi || par2 > p2hi || ystart < ylow || yend < ylow || ystart > yhigh
         || yend > yhigh) {
         qDebug() << "ffDS: call" << ffcall << "par1 par2" << par1 << par2 << "ys ye" << ystart << yend
                  << "*OUT-OF-LIMITS*";
         return 1e+99;
      }
   }
   ...
   epar[ 15 ] = rmsd;
   int ktimms = ftimer.elapsed();
   qDebug() << "ffDS: call" << ffcall << "par1 par2" << par1 << par2 << "rmsd" << rmsd << "eval time" << ktimms
            << "ms.";
   if (ffcall < 6)
      qDebug() << "ffDS:  epar0 epar1-9" << parP[ 0 ] << epar[ 1 ] << epar[ 2 ] << epar[ 3 ] << epar[ 4 ] << epar[ 5 ]
               << epar[ 6 ] << epar[ 7 ] << epar[ 8 ] << epar[ 9 ];
...
double US_pcsaProcess::fit_function_HL(double t, double *par) {
   ...
   if (ffcall > 3) {
      ...
      // If this record is beyond any limit, return now with it marked as bad
      if (par1 < p1lo || par1 > p1hi || yval < ylow || yval > yhigh) {
         qDebug() << "ffHL: call" << ffcall << "par1" << par1 << "yv" << yval << "*OUT-OF-LIMITS*";
         return 1e+99;
      }
   }
   ...
   epar[ 15 ] = rmsd;
   int ktimms = ftimer.elapsed();
   qDebug() << "ffHL: call" << ffcall << "par1 par2" << par1 << par2 << "rmsd" << rmsd << "eval time" << ktimms
            << "ms.";
   if (ffcall < 6)
      qDebug() << "ffHL:  epar0 epar1-9" << parP[ 0 ] << epar[ 1 ] << epar[ 2 ] << epar[ 3 ] << epar[ 4 ] << epar[ 5 ]
               << epar[ 6 ] << epar[ 7 ] << epar[ 8 ] << epar[ 9 ];
...
void US_pcsaProcess::LevMarq_fit(void) {
   ...
   static US_LM::LM_Control control(1.e-5, 1.e-5, 1.e-5, 1.e-5, 100., 100, 0, 3);
   if (lmmxcall < 1)
      return;
   ...
   kcsteps = 0;
   int stepms = 500;
   kctask = LnType ? (time_fg * nthreads * eslnc + nmtasks / 2) / nmtasks
...
#endif
#if 1
   US_ModelRecord::elite_limits(mrecs, curvtype, minyv, maxyv, minp1, maxp1, minp2, maxp2, minp3, maxp3);
#endif
#if 0
...
maxp2=(maxp2+par2up)*0.5;
   ...
   if (curvtype == CTYPE_SL) { // Fit with Levenberg-Marquardt for straight-line curves
      control.ftol = 1.e-5;
      control.xtol = 1.e-5;
      control.gtol = 1.e-5;
      control.epsilon = 1.e-5;
      DbgLv(0) << "lmcurve_fit (SL) with par1,par2" << par[ 0 ] << par[ 1 ] << "ftol,epsl" << control.ftol
               << control.epsilon;
      fit_function_SL(-1.0, par); // Make sure to reset eval. function

      US_LM::lmcurve_fit_rmsd(npar, par, m_dat, t, y, &(US_pcsaProcess::fit_function_SL), &control, &status);

      DbgLv(0) << "  lmcurve_fit (SL) return: par1,par2" << par[ 0 ] << par[ 1 ];
      DbgLv(0) << "   lmcfit status: fnorm nfev info" << status.fnorm << status.nfev << status.info
      ...
      DbgLv(0) << "   lmcfit xs,ys xe,ye rmsd" << xlolim << ys << xuplim << ye << rmsd << "ncsol" << nsol;
   }

   else if (curvtype == CTYPE_IS) { // Fit with Levenberg-Marquardt for increasing-sigmoid curves
      ...
      DbgLv(0) << "lmcurve_fit (IS) with par1,par2" << par[ 0 ] << par[ 1 ]
               << QString().sprintf("%14.8e %14.8e", par[ 0 ], par[ 1 ]) << "ftol,epsl" << control.ftol
               << control.epsilon;

      US_LM::lmcurve_fit_rmsd(npar, par, m_dat, t, y, &(US_pcsaProcess::fit_function_IS), &control, &status);
   ...
   else if (curvtype == CTYPE_DS) { // Fit with Levenberg-Marquardt for decreasing-sigmoid curves
      control.ftol = 1.e-16;
      control.xtol = 1.e-16;
      control.gtol = 1.e-16;
      ...
      US_LM::lmcurve_fit_rmsd(npar, par, m_dat, t, y, &(US_pcsaProcess::fit_function_DS), &control, &status);
   ...
   else if (curvtype == CTYPE_HL) { // Fit with Levenberg-Marquardt for horizontal-line curves
      control.ftol = 1.e-5;
      control.xtol = 1.e-5;
      control.gtol = 1.e-5;
      control.epsilon = 1.e-5;
      DbgLv(0) << "lmcurve_fit (HL) with par1" << par[ 0 ] << "ftol,epsl" << control.ftol << control.epsilon;
      fit_function_HL(-1.0, par); // Make sure to reset eval. function

      US_LM::lmcurve_fit_rmsd(npar, par, m_dat, t, y, &(US_pcsaProcess::fit_function_HL), &control, &status);
      ...
      double rmsd = sqrt(dsets[ 0 ]->model.variance);
      int nsol = dsets[ 0 ]->model.components.size();
      DbgLv(0) << "   lmcfit xs,yv xe,yv rmsd" << xlolim << yv << xuplim << yv << rmsd << "ncsol" << nsol;
   }
   ...
   DbgLv(0) << "     lmcfit  LM time(ms):  estimated" << kctask << "actual" << time_lm;
   QString fmsg = tr("The new best model has par1 %1,  par2 %2,\n"
                     ...
                     .arg(nfev);
   if (ktimeh == 0)
      fmsg = fmsg + tr("(%1 min., %2 sec.)").arg(ktimem).arg(ktimes);
   else
      fmsg = fmsg + tr("(%1 hr., %2 min., %3 sec.)").arg(ktimeh).arg(ktimem).arg(ktimes);
   if (alpha_lm != 0.0)
      fmsg = fmsg
   ...
   for (int ii = 0; ii < mrec.isolutes.size(); ii++) { // Replace x and y in top model input solutes
   ...
   for (int ii = 0; ii < nsol; ii++) {
      ...
      DbgLv(1) << "LMf:     s D mw" << model.components[ ii ].s << model.components[ ii ].D
   ...
   US_AstfemMath::initSimData(sdata, *edata, 0.0);
   US_AstfemMath::initSimData(rdata, *edata, 0.0);
   DbgLv(1) << "LMf:simparms: spts meni bott temp bpos" << dset->simparams.simpoints << dset->simparams.meniscus
            << dset->simparams.bottom << dset->simparams.temperature << dset->simparams.bottom_position;
   DbgLv(1) << "LMf:model: desc analys vari" << model.description << model.analysis << model.variance;
   ...
   DbgLv(0) << "LMf:insert-new: old par1 par2" << mrecs[ 0 ].par1 << mrecs[ 0 ].par2 << "new par1 par2" << mrec.par1
            << mrec.par2 << "nmtasks mrec-size" << nmtasks << mrecs.size();
   DbgLv(0) << "LMf: old01 x,y" << mrecs[ 0 ].isolutes[ 0 ].x << mrecs[ 0 ].isolutes[ 0 ].y
            << mrecs[ 0 ].isolutes[ 1 ].x << mrecs[ 0 ].isolutes[ 1 ].y;
   DbgLv(0) << "LMf: new01 x,y" << mrec.isolutes[ 0 ].x << mrec.isolutes[ 0 ].y << mrec.isolutes[ 1 ].x
            << mrec.isolutes[ 1 ].y;
   mrecs.insert(0, mrec);

   // Re-compute simulation and residuals
   DbgLv(0) << "LMf: tino rino" << tino << rino;
   DbgLv(0) << "LMf: simdat nsc npt" << simdat->scanCount() << simdat->pointCount();
   DbgLv(1) << "LMf: resids nsc npt" << resids->scanCount() << resids->pointCount();
   DbgLv(1) << "LMf: rdata  nsc npt" << rdata.scanCount() << rdata.pointCount();
   DbgLv(1) << "LMf: sdata  nsc npt" << sdata.scanCount() << sdata.pointCount();
   ...
   for (int ss = 0; ss < nscans; ss++) {
      ...
      for (int rr = 0; rr < npoints; rr++) {
         ...
         double resval = edata->value(ss, rr) - sdata.value(ss, rr) - tnois - rnois;
         ...
         if ((ss < 3 && rr < 3) || ((ss + 4) > nscans && (rr + 4) > npoints) || (rr == (npoints / 2)))
            DbgLv(1) << "LMf:  ss rr" << ss << rr << "edat sdat resv" << edata->value(ss, rr) << sdata.value(ss, rr)
                     << resval;
         cvari += sq(resval);
      }
   }
...
void US_pcsaProcess::elite_limits(
   QVector<US_ModelRecord> &mrecs, double &minyv, double &maxyv, double &minp1, double &maxp1, double &minp2,
   double &maxp2) {
   ...
   if (curvtype == CTYPE_SL || curvtype == CTYPE_HL) { // Possibly adjust initial par1,par2 limits for lines
      ln_type = true;
      m0p1 = mrecs[ 0 ].str_y;
      m0p2 = mrecs[ 0 ].end_y;
   ...
   if (curvtype == CTYPE_IS || curvtype == CTYPE_DS) { // Possibly adjust initial par1,par2 limits for sigmoids
      ...
      DbgLv(1) << " ElLim: ADJUST SIGM: m0p1 m0p1h" << m0p1 << m0p1h << "m0p1<0.500" << (m0p1 < 0.500) << 0.500
               << "(m0p1-0.5)" << (m0p1 - 0.5);
   }
   ...
   DbgLv(1) << " ElLim: in m0p1l,m0p1h,m0p2l,m0p2h" << m0p1l << m0p1h << m0p2l << m0p2h;
   ...
   for (int ii = 0; ii < nmr; ii++) {
      ...
      if (ii < 3 || (ii + 4) > nelite)
         DbgLv(1) << " ElLim:   ii" << ii << "par1 par2" << par1 << par2 << "str_y end_y" << str_y << end_y << "rmsd"
                  << mrecs[ ii ].rmsd;
      ...
      if (ii > nelite)
         DbgLv(1) << " ElLim:    minp1 maxp1 m0p1" << minp1 << maxp1 << m0p1 << "minp2 maxp2 m0p2" << minp2 << maxp2
                  << m0p2;
      if (ii > nelite && minp1 < m0p1l && maxp1 > m0p1h && minp2 < m0p2l && maxp2 > m0p2h)
         break;
   }
...
double US_pcsaProcess::evaluate_model(QList<US_SolveSim::DataSet *> &dsets, US_SolveSim::Simulation &sim_vals) {
   US_SolveSim::DataSet *dset = dsets[ 0 ];
   US_SimulationParameters *spar = &dset->simparams;
   spar->mesh_radius.clear();
...
#endif
      US_ZSolute::set_mcomp_values(mcomp, sim_vals.zsolutes[ ii ], st_mask, true);
   ...
   if (sim_vals.noisflag != 0) {
      int ntin = ((sim_vals.noisflag & 1) == 0) ? 0 : sim_vals.ti_noise.size();
      int nrin = ((sim_vals.noisflag & 2) == 0) ? 0 : sim_vals.ri_noise.size();
...
void US_pcsaProcess::timerEvent(QTimerEvent *event) {
   ...
   if (tm_id != lmtm_id) { // If other than L-M timing event, pass it on to the normal handler
   ...
   if (lm_done) { // If L-M is done, signal that with progress status
      nctotal = kcsteps;
      emit stage_complete(kcsteps, nctotal);
      killTimer(tm_id);
   }

   else if (kcsteps < nctotal) { // Normal less-than-100% progress
      emit stage_complete(kcsteps, nctotal);
   }

   else { // Not done with L-M but task count met, so adjust total
...
void US_pcsaProcess::compute_final() {
   ...
   DbgLv(1) << "CFin:   zsol0 x y z" << sim_vals.zsolutes[ 0 ].x << sim_vals.zsolutes[ 0 ].y
            << sim_vals.zsolutes[ 0 ].z;
   ...
   for (int ii = 0; ii < nmsol; ii++) {
      ...
      DbgLv(1) << "CFin:     s D mw" << model.components[ ii ].s << model.components[ ii ].D
               << model.components[ ii ].mw;
   }

   DbgLv(1) << "CFin:model: desc analys vari" << model.description << model.analysis << model.variance;
...
void US_pcsaProcess::restart_fit() {
   ...
   DbgLv(1) << "RF: sll sul" << xlolim << xuplim << " yll yul nyp" << ylolim << yuplim << nypts << " type reso nth noi"
            << curvtype << cresolu << nthreads << noisflag;
   ...
   US_ModelRecord::recompute_mrecs(curvtype, xlolim, xuplim, ylolim, yuplim, nypts, cresolu, parlims, mrecs);

   // Update the solutes list from the newly recomputed model records
   for (int ii = 0; ii < mrecs.size(); ii++) {
      orig_sols << mrecs[ ii ].isolutes;
   }
   DbgLv(1) << "SGMO:  orig_sols size" << orig_sols.size() << "nmodels" << mrecs.size();
