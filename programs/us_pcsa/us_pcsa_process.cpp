...
long int US_pcsaProcess::max_rss(void) { return US_Memory::rss_max(maxrss); }

// Start a specified PCSA fit run
void US_pcsaProcess::start_fit(double xll, double xul, double yll, double yul,
                               int nyp, int res, int typ, int nth, int noi,
                               int lmmxc, int gfits, double gfthr, double alf) {
  ...
  DbgLv(1) << "PC: alf alpha lm fx scn" << alf << alpha << alpha_lm << alpha_fx
           << alpha_scn;
  ...
  DbgLv(1) << "PC: sll sul" << xlolim << xuplim << " yll yul nyp" << ylolim
           << yuplim << nypts << " type reso nth noi" << curvtype << cresolu
           << nthreads << noisflag;
  ...
  if (curvtype == CTYPE_SL ||
      curvtype == CTYPE_HL) {  // Determine models for straight-line curves
    nmtasks =
        slmodels(curvtype, xlolim, xuplim, ylolim, yuplim, nypts, cresolu);
  }

  else if (curvtype == CTYPE_IS ||
           curvtype == CTYPE_DS) {  // Determine models for sigmoid curves
    nmtasks =
        sigmodels(curvtype, xlolim, xuplim, ylolim, yuplim, nypts, cresolu);
  }

  else if (curvtype ==
           CTYPE_2O) {  // Determine models for 2nd-order power law curves
  ...
  memneed *= nthreads;
  DbgLv(1) << "PC:MEM(1): pav" << mempav << "ns dsz dfac mneed" << nsoln
           << dsize << dfact << memneed;
  memuse = memuse + memneed;
  memava = memtot - memuse;
  mempav = (memava * 100) / memtot;
  DbgLv(1) << "PC:MEM(1): pav" << mempav << "ava tot use" << memava << memtot
           << memuse;
  ...
  memava = memtot - memuse;
  mempav = (memava * 100) / memtot;
  DbgLv(1) << "PC:MEM(2): pav" << mempav << "ava tot use" << memava << memtot
           << memuse;
...
void US_pcsaProcess::stop_fit() {
  ...
  emit message_update(
      pmessage_head() + tr("All computations have been aborted."), false);
  abort = false;
}
...
void US_pcsaProcess::process_fxfinal(US_ModelRecord& mrec) {
  ...
  DbgLv(1) << "FIN_FIN: simdat nsc npt" << simdat->scanCount()
           << simdat->pointCount();
  DbgLv(1) << "FIN_FIN: resids nsc npt" << resids->scanCount()
           << resids->pointCount();
  DbgLv(1) << "FIN_FIN: rdata  nsc npt" << rdata.scanCount()
           << rdata.pointCount();
  DbgLv(1) << "FIN_FIN: sdata  nsc npt" << sdata.scanCount()
           << sdata.pointCount();
  ...
  DbgLv(1) << "done: vari bvol" << mrec.variance << bvol << "slun klun"
           << xlolim << xuplim << ylolim << yuplim << nypts << "ets" << ktimes;
  ...
  if (curvtype == CTYPE_SL) {
    double slopel =
        (double)(mrec.end_y - mrec.str_y) / (double)(xuplim - xlolim);
  ...
  rd_frac =
      (fi_iter > 1) ? qAbs((pfi_rmsd - cfi_rmsd) / pfi_rmsd) : rd_thresh * 2;
  rmsds << cfi_rmsd;
  ...
  if (alpha_lm == 0.0 && alpha != 0.0) {  // L-M nonregularized, but need to do
                                          // a final regularized computation
    compute_final();
  }
...
bool US_pcsaProcess::get_results(US_DataIO::RawData* da_sim,
                                 US_DataIO::RawData* da_res, US_Model* da_mdl,
                                 US_Noise* da_tin, US_Noise* da_rin,
                                 int& bm_ndx, QStringList& modstats,
...
void US_pcsaProcess::put_mrecs(QVector<US_ModelRecord>& a_mrecs) {
  ...
  nypts = (curvtype != CTYPE_HL && curvtype != CTYPE_2O)
              ...
              : nypts;
  nypts =
      (curvtype == CTYPE_2O) ? (qRound(pow((double)nypts, 0.3333333))) : nypts;
  alpha = 0.0;
...
void US_pcsaProcess::submit_job(WorkPacketPc& wtask, int thrx) {
  ...
  connect(wthr, SIGNAL(work_complete(WorkerThreadPc*)), this,
          SLOT(process_job(WorkerThreadPc*)));
  DbgLv(1) << "SUBMIT_JOB taskx" << wtask.taskx << "sk ek" << wtask.str_y
           << wtask.end_y << "maxrss" << maxrss;
...
void US_pcsaProcess::process_job(WorkerThreadPc* wthrd) {
  ...
  if (dbg_level > 0)
    for (int mm = 0; mm < wresult.csolutes.size(); mm++) {
      if (wresult.csolutes[mm].c > 100.0) {
        DbgLv(1) << "PJ:  CONC=" << wresult.csolutes[mm].c << " x,y"
                 << wresult.csolutes[mm].x << wresult.csolutes[mm].y;
      }
    }
  // DBG-CONC
  ...
  } else if (variance > varimin) {  // Clear vectors from a model record that is
  ...
  int dfact = (nsoln + 4) * sizeof(double);
  int memneed = qRound((double)dsize * (double)dfact / (1024. * 1024.));
  DbgLv(1) << "PC:MEM: (4)nsol,nscn,npt,dsize,dfact,memneed" << nsoln << nscann
           << npointn << dsize << dfact << memneed;
  memuse = memuse + memneed;
  memava = memtot - memuse;
  mempav = (memava * 100) / memtot;
  ...
  emit message_update(
      pmessage_head() + tr("Of %1 models, computations are complete for %2.")
                            .arg(nmtasks)
                            .arg(kctask),
      false);
  ...
  if (kctask >= nmtasks) {  // All model tasks are now complete
    ...
    emit message_update(
        pmessage_head() +
            tr("All models have now been completed. Evaluating..."),
        false);
  }
...
void US_pcsaProcess::free_worker(int tx) {
  if (tx >= 0 && tx < wthreads.size()) {
    if (wthreads[tx] != 0) delete wthreads[tx];  // destroy thread
...
QString US_pcsaProcess::pmessage_head() {
  ...
  for (int ii = 1; ii < fi_iter; ii++)
    hmsg +=
        tr("Iteration %1 had a best RMSD of %2 ;\n").arg(ii).arg(rmsds[ii - 1]);
...
WorkPacketPc US_pcsaProcess::next_job() {
  ...
  if (jobx > 0) {
    DbgLv(1) << "NEXTJ: jobx taskx taskx0" << jobx << wtask.taskx
             << job_queue[0].taskx;
  } else {
...
int US_pcsaProcess::slmodels(int ctp, double xlo, double xup, double ylo,
                             double yup, int nyp, int res) {
  DbgLv(1) << "SLMO: xlo xup ylo yup nyp res" << xlo << xup << ylo << yup << nyp
           << res;
  int nmodels = 0;
  // double vbar20   = dsets[ 0 ]->vbar20;
  orig_sols.clear();
  ...
  if (ctp == CTYPE_SL)
    nmodels = US_ModelRecord::compute_slines(xlo, xup, ylo, yup, nyp, res,
                                             parlims, mrecs);
  else if (ctp == CTYPE_HL)
    nmodels = US_ModelRecord::compute_hlines(xlo, xup, ylo, yup, nyp, res,
                                             parlims, mrecs);
  ...
  DbgLv(1) << "SLMO:  orig_sols size" << orig_sols.size() << "nmodels"
           << nmodels;
...
int US_pcsaProcess::sigmodels(int ctp, double xlo, double xup, double ylo,
                              double yup, int nyp, int nlpts) {
  DbgLv(1) << "SGMO: ctp xlo xup ylo yup nyp nlp" << ctp << xlo << xup << ylo
           << yup << nyp << nlpts;
  // double vbar20   = dsets[ 0 ]->vbar20;
  orig_sols.clear();

  // Compute sigmoid model records
  int nmodels = US_ModelRecord::compute_sigmoids(ctp, xlo, xup, ylo, yup, nyp,
                                                 nlpts, parlims, mrecs);
  ...
  DbgLv(1) << "SGMO:  orig_sols size" << orig_sols.size() << "nmodels"
           << nmodels;
...
int US_pcsaProcess::pl2models(double xlo, double xup, double ylo, double yup,
                              int nyp, int nlpts) {
  DbgLv(1) << "2OMO: xlo xup ylo yup nyp nlp" << xlo << xup << ylo << yup << nyp
           << nlpts;
  // double vbar20   = dsets[ 0 ]->vbar20;
  orig_sols.clear();

  // Compute 2nd-order model records
  int nmodels = US_ModelRecord::compute_2ndorder(xlo, xup, ylo, yup, nyp, nlpts,
                                                 parlims, mrecs);
  ...
  DbgLv(1) << "2OMO:  orig_sols size" << orig_sols.size() << "nmodels"
           << nmodels;
...
void US_pcsaProcess::model_statistics(QVector<US_ModelRecord>& mrecs,
  ...
  double brmsmax = 0.0;
  double brmsavg = 0.0;
  DbgLv(1) << "PC:MS: nmtasks mrssiz nbmods" << nmtasks << mrecs.size()
           << nbmods;
  ...
  if (curvtype == CTYPE_SL || curvtype == CTYPE_HL) {
    ...
    modstats << tr("Y Range + delta:")
             << QString().sprintf("%10.4f  %10.4f  %10.4f", ylolim, yuplim,
                                  yincr);
    modstats << tr("Best curve Y end points + slope:")
             << QString().sprintf("%10.4f  %10.4f  %10.4f", str_y, end_y,
                                  slope);
    ...
    modstats << tr("Best curve Y end points:")
             << QString().sprintf("%10.4f  %10.4f", mrecs[0].str_y,
                                  mrecs[0].end_y);
  } else if (curvtype == CTYPE_2O) {
    modstats << tr("Y Range:")
             << QString().sprintf("%10.4f  %10.4f", ylolim, yuplim);
    modstats << tr("Best curve A, B, C:")
             << QString().sprintf("%10.4f  %10.4f  %10.4f", mrecs[0].par1,
                                  mrecs[0].par2, mrecs[0].par3);
    modstats << tr("Best curve Y end points:")
             << QString().sprintf("%10.4f  %10.4f", mrecs[0].str_y,
                                  mrecs[0].end_y);
  } else {
    modstats << tr("Y Range:")
             << QString().sprintf("%10.4f  %10.4f", ylolim, yuplim);
    modstats << tr("Best curve par1 and par2:")
             << QString().sprintf("%10.4f  %10.4f", mrecs[0].par1,
                                  mrecs[0].par2);
    modstats << tr("Best curve Y end points:")
             << QString().sprintf("%10.4f  %10.4f", mrecs[0].str_y,
                                  mrecs[0].end_y);
...
double US_pcsaProcess::fit_function_SL(double t, double* par) {
  ...
  if (ffcall == 0) {  // On 1st call, copy par array to internal static one
    for (int ii = 0; ii < nepar - 2; ii++) epar[ii] = par[ii + 2];
    parP[0] = iparP[px];
  }
  ...
  if (ffcall > 3) {
    ...
    if (par1 < p1lo || par2 < p2lo || par1 > p1hi || par2 > p2hi ||
        ystart < ylow || yend < ylow || ystart > yhigh || yend > yhigh) {
  ...
  qDebug() << "ffSL: call" << ffcall << "par1 par2" << par1 << par2 << "rmsd"
           << rmsd << "eval time" << ktimms << "ms.";
  if (ffcall < 6)
    qDebug() << "ffSL:  epar0 epar1-9" << parP[0] << epar[1] << epar[2]
             << epar[3] << epar[4] << epar[5] << epar[6] << epar[7] << epar[8]
             << epar[9];
...
double US_pcsaProcess::fit_function_IS(double t, double* par) {
  ...
  epar[15] = rmsd;
  int ktimms = ftimer.elapsed();
  qDebug() << "ffIS: call" << ffcall << "par1 par2" << par1 << par2 << "rmsd"
           << rmsd << "eval time" << ktimms << "ms.";
  if (ffcall < 6)
    qDebug() << "ffIS:  epar0 epar1-9" << parP[0] << epar[1] << epar[2]
             << epar[3] << epar[4] << epar[5] << epar[6] << epar[7] << epar[8]
             << epar[9];
...
double US_pcsaProcess::fit_function_DS(double t, double* par) {
  ...
  epar[15] = rmsd;
  int ktimms = ftimer.elapsed();
  qDebug() << "ffDS: call" << ffcall << "par1 par2" << par1 << par2 << "rmsd"
           << rmsd << "eval time" << ktimms << "ms.";
  if (ffcall < 6)
    qDebug() << "ffDS:  epar0 epar1-9" << parP[0] << epar[1] << epar[2]
             << epar[3] << epar[4] << epar[5] << epar[6] << epar[7] << epar[8]
             << epar[9];
...
double US_pcsaProcess::fit_function_HL(double t, double* par) {
  ...
  epar[15] = rmsd;
  int ktimms = ftimer.elapsed();
  qDebug() << "ffHL: call" << ffcall << "par1 par2" << par1 << par2 << "rmsd"
           << rmsd << "eval time" << ktimms << "ms.";
  if (ffcall < 6)
    qDebug() << "ffHL:  epar0 epar1-9" << parP[0] << epar[1] << epar[2]
             << epar[3] << epar[4] << epar[5] << epar[6] << epar[7] << epar[8]
             << epar[9];
...
#endif
#if 1
  US_ModelRecord::elite_limits(mrecs, curvtype, minyv, maxyv, minp1, maxp1,
                               minp2, maxp2, minp3, maxp3);
#endif
#if 0
...
maxp2=(maxp2+par2up)*0.5;
  ...
  if (curvtype ==
      CTYPE_SL) {  // Fit with Levenberg-Marquardt for straight-line curves
    control.ftol = 1.e-5;
    control.xtol = 1.e-5;
    control.gtol = 1.e-5;
    ...
    US_LM::lmcurve_fit_rmsd(npar, par, m_dat, t, y,
                            &(US_pcsaProcess::fit_function_SL), &control,
                            &status);

    DbgLv(0) << "  lmcurve_fit (SL) return: par1,par2" << par[0] << par[1];
    DbgLv(0) << "   lmcfit status: fnorm nfev info" << status.fnorm
             << status.nfev << status.info << US_LM::lm_statmsg(&status, false);
    double ys = par[0];
  ...
  else if (curvtype == CTYPE_IS) {  // Fit with Levenberg-Marquardt for
                                    // increasing-sigmoid curves
    ...
    US_LM::lmcurve_fit_rmsd(npar, par, m_dat, t, y,
                            &(US_pcsaProcess::fit_function_IS), &control,
                            &status);
  ...
  else if (curvtype == CTYPE_DS) {  // Fit with Levenberg-Marquardt for
                                    // decreasing-sigmoid curves
    control.ftol = 1.e-16;
    control.xtol = 1.e-16;
    control.gtol = 1.e-16;
    ...
    US_LM::lmcurve_fit_rmsd(npar, par, m_dat, t, y,
                            &(US_pcsaProcess::fit_function_DS), &control,
                            &status);

    DbgLv(0) << "  lmcurve_fit (DS) return: par1,par2" << par[0] << par[1]
             << QString().sprintf("%14.8e %14.8e", par[0], par[1]);
    DbgLv(0) << "   lmcfit status: fnorm nfev info" << status.fnorm
             << status.nfev << status.info;
    double rmsd = sqrt(dsets[0]->model.variance);
    int nsol = dsets[0]->model.components.size();
    DbgLv(0) << "   lmcfit rmsd" << rmsd << "#solutes" << nsol;
  }

  else if (curvtype == CTYPE_HL) {  // Fit with Levenberg-Marquardt for
                                    // horizontal-line curves
    control.ftol = 1.e-5;
    control.xtol = 1.e-5;
    control.gtol = 1.e-5;
    control.epsilon = 1.e-5;
    DbgLv(0) << "lmcurve_fit (HL) with par1" << par[0] << "ftol,epsl"
             << control.ftol << control.epsilon;
    fit_function_HL(-1.0, par);  // Make sure to reset eval. function

    US_LM::lmcurve_fit_rmsd(npar, par, m_dat, t, y,
                            &(US_pcsaProcess::fit_function_HL), &control,
                            &status);
  ...
  int ktimem = (ktimes - ktimeh * 3600) / 60;
  ktimes = ktimes - ktimeh * 3600 - ktimem * 60;
  DbgLv(0) << "     lmcfit time: " << ktimeh << "h" << ktimem << "m" << ktimes
           << "s";
  DbgLv(0) << "     lmcfit  LM time(ms):  estimated" << kctask << "actual"
           << time_lm;
  ...
  if (ktimeh == 0)
    ...
    fmsg = fmsg +
           tr("(%1 hr., %2 min., %3 sec.)").arg(ktimeh).arg(ktimem).arg(ktimes);
  if (alpha_lm != 0.0)
  ...
  for (int ii = 0; ii < mrec.isolutes.size();
       ii++) {  // Replace x and y in top model input solutes
  ...
  if (tino) {
    ...
    DbgLv(1) << "LMf: ti count size" << ti_noise.count
             << ti_noise.values.size();
  }
  ...
  if (rino) {
    ...
    DbgLv(1) << "LMf: ri count size" << ri_noise.count
             << ri_noise.values.size();
  }
  ...
  DbgLv(0) << "LMf:insert-new: old par1 par2" << mrecs[0].par1 << mrecs[0].par2
           ...
           << nmtasks << mrecs.size();
  DbgLv(0) << "LMf: old01 x,y" << mrecs[0].isolutes[0].x
           << mrecs[0].isolutes[0].y << mrecs[0].isolutes[1].x
  ...
  DbgLv(0) << "LMf: simdat nsc npt" << simdat->scanCount()
           << simdat->pointCount();
  DbgLv(1) << "LMf: resids nsc npt" << resids->scanCount()
           << resids->pointCount();
...
void US_pcsaProcess::elite_limits(QVector<US_ModelRecord>& mrecs, double& minyv,
                                  double& maxyv, double& minp1, double& maxp1,
  ...
  if (curvtype == CTYPE_SL ||
      curvtype ==
          CTYPE_HL) {  // Possibly adjust initial par1,par2 limits for lines
    ln_type = true;
    m0p1 = mrecs[0].str_y;
    m0p2 = mrecs[0].end_y;
  ...
  if (curvtype == CTYPE_IS ||
      curvtype ==
          CTYPE_DS) {  // Possibly adjust initial par1,par2 limits for sigmoids
    ...
    DbgLv(1) << " ElLim: ADJUST SIGM: m0p1 m0p1h" << m0p1 << m0p1h
             << "m0p1<0.500" << (m0p1 < 0.500) << 0.500 << "(m0p1-0.5)"
             << (m0p1 - 0.5);
  }
  ...
  DbgLv(1) << " ElLim: in m0p1l,m0p1h,m0p2l,m0p2h" << m0p1l << m0p1h << m0p2l
           << m0p2h;
  ...
  for (int ii = 0; ii < nmr; ii++) {
    ...
    if (ii > nelite && minp1 < m0p1l && maxp1 > m0p1h && minp2 < m0p2l &&
        maxp2 > m0p2h)
      break;
  }
...
void US_pcsaProcess::timerEvent(QTimerEvent* event) {
  ...
  if (tm_id != lmtm_id) {  // If other than L-M timing event, pass it on to the
                           // normal handler
...
void US_pcsaProcess::compute_final() {
  ...
  DbgLv(1) << "CFin:   zsol0 x y z" << sim_vals.zsolutes[0].x
           << sim_vals.zsolutes[0].y << sim_vals.zsolutes[0].z;
  ...
  for (int ii = 0; ii < nmsol; ii++) {
    ...
    US_ZSolute::set_solute_values(model.components[ii], solute, st_mask);
    mrec.csolutes << solute;
    DbgLv(1) << "CFin:  ii" << ii << "x y c" << solute.x << solute.y
             << solute.c;
...
void US_pcsaProcess::restart_fit() {
  ...
  DbgLv(1) << "RF: sll sul" << xlolim << xuplim << " yll yul nyp" << ylolim
           << yuplim << nypts << " type reso nth noi" << curvtype << cresolu
           << nthreads << noisflag;
