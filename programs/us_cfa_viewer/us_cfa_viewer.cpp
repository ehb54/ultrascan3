...
US_CfaDataViewer::US_CfaDataViewer() : US_Widgets()
{
   const QChar chlamb( 955 );

   setWindowTitle( tr( "CFA Raw Data Viewer" ) );
   ...
   QLabel*      lb_scntype  = us_label( tr( "Type of Raw Scans to Load:" ) );
                ...
                pb_loadCfa  = us_pushbutton( tr( "Load Raw CFA Data" ) );
   ...
   // Signals and Slots
   connect( pb_loadCfa,   SIGNAL( clicked()      ),
            this,         SLOT  ( load_cfa_raw() ) );
   connect( pb_loadAUC,   SIGNAL( clicked()      ),
            this,         SLOT  ( load_auc_cfa() ) );
   ...
   settings->addWidget( pb_loadCfa,    row,   0, 1, 4 );
...
void US_CfaDataViewer::reset( void )
{
...
//   pb_movie2d->setEnabled( false );
   ...
   last_ymin       = -1.0;
   last_ymax       = -1.0;
   cfa_data        = ( cfa_data == NULL ) ? new US_CfaData() : cfa_data;

   connect( cfa_data, SIGNAL( status_text  ( QString ) ),
...
void US_CfaDataViewer::resetAll( void )
{
   if ( allData.size() > 0 )
   {
...
void US_CfaDataViewer::enableControls( void )
{
   const QChar chlamb( 955 );
...
// Load CFA raw (.sqlite) data
void US_CfaDataViewer::load_cfa_raw( )
{
   // Ask for data directory
   QString dir = "";
   US_CfaRun* lddiag = new US_CfaRun( dir, true );
   ...
   le_status->setText( tr( "Reading Raw CFA data ..." ) );

   cfa_data->import_data( currentDir, sctype );

   // Build the AUC equivalent
   le_status->setText( tr( "Building AUC data ..." ) );

   cfa_data->build_rawData( allData );

   QApplication::restoreOverrideCursor();
   QApplication::restoreOverrideCursor();
   isRaw        = true;
   haveData     = true;
   cfa_fnames   = dirdir.entryList( QStringList( "*.sqlite" ),
                                  QDir::Files, QDir::Name );
   cfa_fnames.sort();
   ntriple       = cfa_fnames.size();
   runID         = new_runID;
   ncellch       = cfa_data->cellchannels( cellchans );
   radii.clear();
...
DbgLv(1) << "RDr:   rvS rvE" << radii[0] << radii[npoint-1];
   ...
   nlambda      = cfa_data->lambdas_raw( lambdas );
   int wvlo     = lambdas[ 0 ];
...
// Load US3 AUC CFA-derived data
void US_CfaDataViewer::load_auc_cfa( )
{
   int status        = 0;
   QStringList ifpaths;
...
DbgLv(1) << "RDa: dir" << currentDir;
   ...
   // Load the AUC data
   le_status->setText( tr( "Reading AUC Cfa data ..." ) );
   QApplication::setOverrideCursor( QCursor( Qt::WaitCursor) );
   QDir dirdir( dir, "*", QDir::Name, QDir::Files | QDir::Readable );
   dirdir.makeAbsolute();
   if ( dir.right( 1 ) != "/" ) dir += "/"; // Ensure trailing /
   cfa_fnames       = dirdir.entryList( QStringList( "*.auc" ),
                                        QDir::Files, QDir::Name );
   cfa_fnames.sort();
   ntriple           = cfa_fnames.size();
   cellchans.clear();
...
DbgLv(1) << "RDa: mwr ntriple" << ntriple;
   ...
   {
      QString mwrfname  = cfa_fnames.at( ii );
...
DbgLv(1) << "RDa:   rvS rvE" << radii[0] << radii[npoint-1];
   ...
   cfa_data->load_auc( allData, ifpaths );
...
void US_CfaDataViewer::plot_current( void )
{
   if ( allData.size() == 0 )
      return;
...
void US_CfaDataViewer::plot_titles( void )
{
...
void US_CfaDataViewer::changeRadius()
{
DbgLv(1) << "chgRadius";
   have_rngs  = false;
...
void US_CfaDataViewer::changeCellCh( void )
{
...
void US_CfaDataViewer::changeRecord( void )
{
...
void US_CfaDataViewer::prevPlot( void )
{
...
void US_CfaDataViewer::nextPlot( void )
{
   int pltrx      = cb_pltrec->currentIndex() + 1;
...
void US_CfaDataViewer::compute_ranges()
{
...
void US_CfaDataViewer::connect_ranges( bool conn )
{
   if ( conn )
...
void US_CfaDataViewer::export_auc()
{
   if ( ! isRaw  ||  allData.size() == 0 )
      return;

   int nfiles    = cfa_data->export_auc( allData );
   haveTmst     = QFile( currentDir + runID + ".time_state.tmst" ).exists();
...
void US_CfaDataViewer::exclude_from( double sfr )
{
...
void US_CfaDataViewer::exclude_to( double sto )
{
   int scan_to    = (int)sto;
   int scan_from  = (int)ct_from->value();
...
void US_CfaDataViewer::exclude_scans()
{
   int scan_from  = (int)ct_from->value();
   int scan_to    = (int)ct_to  ->value();
   int scan_knt   = 1;
...
void US_CfaDataViewer::include_scans()
{
   excludes.clear();
...
int US_CfaDataViewer::dvec_index( QVector< double >& dvec, const double dval )
{
...
void US_CfaDataViewer::showTimeState()
{
...
void US_CfaDataViewer::status_report( QString stat_text )
{
   qApp->processEvents();
