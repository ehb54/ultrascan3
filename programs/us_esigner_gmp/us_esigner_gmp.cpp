...
US_eSignaturesGMP::US_eSignaturesGMP() : US_Widgets() {
   ...
   te_inv_smry->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 1));
   ...
   te_grev_smry->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 1));
   ...
   te_gappr_smry->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 1));
   ...
   connect(le_inv_search, SIGNAL(textChanged(const QString &)), SLOT(limit_inv_names(const QString &)));
   connect(lw_inv_list, SIGNAL(itemClicked(QListWidgetItem *)), SLOT(get_inv_data(QListWidgetItem *)));

   connect(le_grev_search, SIGNAL(textChanged(const QString &)), SLOT(limit_grev_names(const QString &)));
   connect(lw_grev_list, SIGNAL(itemClicked(QListWidgetItem *)), SLOT(get_grev_data(QListWidgetItem *)));

   connect(le_gappr_search, SIGNAL(textChanged(const QString &)), SLOT(limit_gappr_names(const QString &)));
   connect(lw_gappr_list, SIGNAL(itemClicked(QListWidgetItem *)), SLOT(get_gappr_data(QListWidgetItem *)));
   ...
   te_operator_names->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 1));
   ...
   te_reviewer_names->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 1));
   ...
   te_appr_names->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 1));
...
void US_eSignaturesGMP::init_invs(void) {
   ...
   while (db.next()) {
      ...
      lw_inv_list->addItem(
         new QListWidgetItem("InvID: (" + QString::number(data.invID) + "), " + data.lastName + ", " + data.firstName));
   }
}
...
void US_eSignaturesGMP::limit_inv_names(const QString &s) {
   ...
   for (int i = 0; i < investigators.size(); i++) {
      if (
         investigators[ i ].lastName.contains(QRegExp(".*" + s + ".*", Qt::CaseInsensitive))
         || investigators[ i ].firstName.contains(QRegExp(".*" + s + ".*", Qt::CaseInsensitive)))
         lw_inv_list->addItem(new QListWidgetItem(
            "InvID: (" + QString::number(investigators[ i ].invID) + "), " + investigators[ i ].lastName + ", "
...
void US_eSignaturesGMP::limit_grev_names(const QString &s) {
   ...
   for (int i = 0; i < g_reviewers.size(); i++) {
      if (
         g_reviewers[ i ].lastName.contains(QRegExp(".*" + s + ".*", Qt::CaseInsensitive))
         || g_reviewers[ i ].firstName.contains(QRegExp(".*" + s + ".*", Qt::CaseInsensitive)))
         lw_grev_list->addItem(new QListWidgetItem(
            "InvID: (" + QString::number(g_reviewers[ i ].invID) + "), " + g_reviewers[ i ].lastName + ", "
...
void US_eSignaturesGMP::limit_gappr_names(const QString &s) {
   ...
   for (int i = 0; i < g_apprs.size(); i++) {
      if (
         g_apprs[ i ].lastName.contains(QRegExp(".*" + s + ".*", Qt::CaseInsensitive))
         || g_apprs[ i ].firstName.contains(QRegExp(".*" + s + ".*", Qt::CaseInsensitive)))
         lw_gappr_list->addItem(new QListWidgetItem(
            "InvID: (" + QString::number(g_apprs[ i ].invID) + "), " + g_apprs[ i ].lastName + ", "
            + g_apprs[ i ].firstName));
   }
}
...
void US_eSignaturesGMP::init_grevs(void) {
   ...
   while (db.next()) {
      ...
      if (!reassign_revs_sep)
         lw_grev_list->addItem(new QListWidgetItem(
            "InvID: (" + QString::number(data.invID) + "), " + data.lastName + ", " + data.firstName));

      if (!assign_revs_sep)
         cb_choose_rev->addItem(QString::number(data.invID) + ". " + data.lastName + ", " + data.firstName);
   }
}
...
void US_eSignaturesGMP::init_gapprs(void) {
   ...
   while (db.next()) {
      ...
      if (!assign_revs_sep)
         cb_choose_appr->addItem(QString::number(data.invID) + ". " + data.lastName + ", " + data.firstName);
   }
}
...
QMap<QString, QString> US_eSignaturesGMP::read_autoflow_record(int autoflowID, QString type) {
   ...
   qry.clear();
   QString xmlstr("");
   US_ProtocolUtil::read_record_auto(run_info[ "protocolName" ], run_info[ "invID_passed" ].toInt(), &xmlstr, NULL, db);
...
void US_eSignaturesGMP::assignOperRevs_sa(void) {
   ...
   qDebug() << "exsisting_oper_list, exsisting_rev_list, exsisting_appr_list: " << exsisting_oper_list
            << exsisting_rev_list << exsisting_appr_list;
   ...
   qDebug() << "[OLD] eSignStatusJson, eSignStatusAll, eSignID, eSigncreateUpdateLogJson -- " << eSignStatusJson
            << eSignStatusAll << eSignID << eSigncreateUpdateLogJson;
   ...
   if (msg_rev.clickedButton() == Accept) {
      ...
      QString eSignStatusJson_updated
         = compose_updated_eSign_Json_sa(u_ID, u_fname, u_lname, updated_to_sign_list, esigned_array);
      qDebug() << "eSignStatusJson_updated -- " << eSignStatusJson_updated;

      //2: updated admin log
      QString logJsonUpdateTime = compose_updated_admin_logJson(
         u_ID, u_fname, u_lname, old_opers, new_opers, old_revs, new_revs, old_apprs, new_apprs);
      qDebug() << "logJsonUpdateTimeJsonObject -- " << logJsonUpdateTime;


      qry.clear();
      qry << "update_gmp_review_record_by_admin" << eSign_details[ "ID" ] << gmp_run_details[ "autoflowID" ]
...
void US_eSignaturesGMP::compose_updated_ora_list(
   QString &jsonArrayList, QString &updated_list, QString &updated_to_sign_list, QString existing_ora, QString new_ora) {
  ...
  **/
   ...
   **/
   ...
   if (!isEsignRecord) //No eSignature Record exists, so create new one, with minimal status, createupdatelog
   {
      ...
      qry.clear();
      qry << "new_gmp_review_record" << gmp_run_details[ "autoflowID" ] << gmp_run_details[ "protocolName" ]
          << operListJsonArray << revListJsonArray << apprListJsonArray << smeListJsonArray << eSignStatusJson
          << logJsonFirstTime;
      ...
      qry.clear();
      qry << "update_autoflow_with_gmpReviewID" << gmp_run_details[ "autoflowID" ] << QString::number(eSignID_returned);
   ...
   if (esigned.isUndefined() || esigned_array.size() == 0 || !esigned_array.size()) {
      ...
      for (int i = 0; i < esigned_array.size(); ++i) {
         foreach (const QString &key, esigned_array[ i ].toObject().keys()) {
            ...
            qDebug() << "E-Signed - " << key << ": Comment, timeDate -- " << newObj[ "Comment" ].toString()
...
QString US_eSignaturesGMP::compose_updated_admin_logJson(
   int u_ID, QString u_fname, QString u_lname, QString old_opers, QString new_opers, QString old_revs, QString new_revs,
...
void US_eSignaturesGMP::loadGMPReportDB_assigned_auto(QString aID_passed) {
   ...
   int db_read_html = db.readBlobFromDB(filePath_db_html, "download_gmpReportData_html", gmpReport_id_selected.toInt());
...
int US_eSignaturesGMP::list_all_gmp_reports_db(QList<QStringList> &gmpReportsDBdata, US_DB2 *db) {
   ...
   while (db->next()) {
      ...
      qDebug() << "logged_user, operatorListJson, reviewersListJson, approversListJson -- " << logged_user << ", "
      ...
      if (
         jsonDocRevList.isArray() && jsonDocOperList.isArray() && !operatorListJson.isEmpty()
         && !reviewersListJson.isEmpty()) {
         ...
         if (auto_separate_status) {
            if (
               operatorListJson.contains(logged_user) || reviewersListJson.contains(logged_user)
...
QString US_eSignaturesGMP::check_revs_esign_status_sa(QString u_passed, QMap<QString, QString> eSign_stats) {
   ...
   qDebug() << "In check_eSign_status_for_gmpReport_auto(): eSignStatusJson, eSignStatusAll -- " << eSignStatusJson
            << eSignStatusAll;
   ...
   //to_sign:
   if (to_esign.isUndefined() || to_esign_array.size() == 0 || !to_esign_array.size() || eSignStatusAll == "YES") {
...
void US_eSignaturesGMP::esign_report(void) {
   ...
   qDebug() << "eSign: operatorListJson, reviewersListJson, approversListJson, eSignStatusAll -- " << operatorListJson
            << reviewersListJson << approversListJson << eSignStatusAll;
   ...
   if (!yesToSign) {
      ...
      QMessageBox::information(this, tr("Not Eligible for e-Signing"), msg_current_user);
   ...
   if (u_ID != current_esignee_in_order_id.toInt()) {
      qDebug() << "u_ID, current_esignee_in_order_id -- " << u_ID << current_esignee_in_order_id.toInt();
      qDebug() << "Current esignee, logged in user -- " << u_ID << ". " << u_lname << ", " << u_fname << ",    "
               << current_esignee_in_order;
      qDebug() << "E-Signees before the current one -- " << esignees_to_sign_before;
   ...
   gmp_esigning_map
      = pw_at.getPasswd_auditTrail("GMP e-Signing Form", "Please fill out GMP e-Signing form:", user_esignee);

   int gmp_esigning_map_size = gmp_esigning_map.keys().size();
   qDebug() << "e-Signing map: " << gmp_esigning_map.keys() << gmp_esigning_map.keys().size() << gmp_esigning_map_size
            << gmp_esigning_map.keys().isEmpty() << gmp_esigning_map[ "User:" ] << gmp_esigning_map[ "Comment:" ]
   ...
   QString eSignStatusJson_updated = compose_updated_eSign_Json(
      u_ID, u_fname, u_lname, to_esign_array, esigned_array, gmp_esigning_map[ "Comment:" ], eSignStatusAll_updated);

   qry.clear();
   qry << "update_gmp_review_record_by_esigner" << eSignID << gmpRunID_eSign << eSignStatusJson_updated
   ...
   QString html_eSignatures = write_pdf_eSignatures(
      filePath_db, eSignStatusJson_updated, operatorListJson, reviewersListJson, approversListJson);
   ...
   ***/
   ...
   int writeStatus
      = db.writeBlobToDB(tarFilename_t, QString("upload_gmpReportData"), autoflowGMPReport_id_selected.toInt());
   ...
   if (auto_mode) {
      ...
      qDebug() << "u_ID, u_lname, u_fname, obj_name -- " << u_ID << u_lname << u_fname << obj_name;
...
QMap<QString, QMap<QString, QString>> US_eSignaturesGMP::json_to_qmap(QString eSignStatusJson_updated) {
   ...
   for (int i = 0; i < esigned_array.size(); ++i) {
      foreach (const QString &key, esigned_array[ i ].toObject().keys()) {
         ...
         qDebug() << "E-Signed - " << key << ": Role, Comment, timeDate -- " << role << newObj[ "Comment" ].toString()
                  << newObj[ "timeDate" ].toString();
...
void US_eSignaturesGMP::printDocument(
   QPrinter &printer, QTextDocument *doc, QMap<QString, QMap<QString, QString>> eSigners_info) //, QWidget* parentWidget)
{
...
void US_eSignaturesGMP::paintPage(
   ...
   qreal footerHeight, QMap<QString, QMap<QString, QString>> eSigners_info) {
   ...
   footerRect.setHeight(
      eSigners_number * footerHeight); //will a parameter on #of lines (footer height, depending on # reviewers...)
...
QString US_eSignaturesGMP::compose_updated_eSign_Json_sa(
   int u_ID, QString u_fname, QString u_lname, QString updated_to_sign_list, QJsonArray esigned_array) {
   QString statusJson = "{";
...
QString US_eSignaturesGMP::compose_updated_eSign_Json(
   int u_ID, QString u_fname, QString u_lname, QJsonArray to_esign_array, QJsonArray esigned_array,
...
QString US_eSignaturesGMP::write_pdf_eSignatures(
   QString filePath, QString eSignStatusJson_upd, QString operList, QString revList, QString apprList) {
   ...
   for (int i = 0; i < esigned_array.size(); ++i) {
      foreach (const QString &key, esigned_array[ i ].toObject().keys()) {
         ...
         qDebug() << "write_pdf_eSign() -- " << key << ": Comment, timeDate, eSigner_role -- "
                  << newObj[ "Comment" ].toString() << newObj[ "timeDate" ].toString() << eSigner_role;

         QString eSigner = (key.split(".")[ 1 ]).trimmed();
         //eSigner
         html_assembled
            += tr("<h3 style=\"margin-left:10px\">%1. %2</h3>").arg(QString::number(eSigner_counter)).arg(eSigner);
...
void US_eSignaturesGMP::write_download_eSignatures_DB(QString filePath, QString proc_name) {
   ...
   qDebug() << "write_eSignatures_DB():: Writing Blob of final_fpath, eSignID -- " << final_fpath
   ...
   if (proc_name.contains("upload"))
      writeStatus = db.writeBlobToDB(final_fpath, proc_name, eSignID_global.toInt());
   if (proc_name.contains("download"))
      writeStatus = db.readBlobFromDB(final_fpath, proc_name, eSignID_global.toInt());
