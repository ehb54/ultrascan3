...
US_eSignaturesGMP::US_eSignaturesGMP(QStringList reassign) : US_Widgets() {
  ...
  QLabel* bn_revOperGMP = us_banner(
      tr("Reassign Operator(s), Reviewer(s), Approver(s) for GMP Report:"), 1);
  ...
  pb_set_operRev =
      us_pushbutton(tr("Reassign Operator(s), Reviewer(s) and Approver(s)"));
  pb_set_operRev->setEnabled(false);
...
void US_eSignaturesGMP::initPanel_auto(
    QMap<QString, QString>& protocol_details) {
...
void US_eSignaturesGMP::display_reviewers_auto(int& row,
  ...
  if (eSign_d.contains(JsonListName)) {
    ...
    for (int i = 0; i < jsonDocOperList_array.size(); ++i) {
      ...
      QLineEdit* le_stat =
          check_eSign_status_for_gmpReport_auto(current_reviewer, eSign_d);
...
US_eSignaturesGMP::US_eSignaturesGMP(QMap<QString, QString>& it_details)
  ...
  QLabel* bn_revGlobal =
      us_banner(tr("Create | Edit Global Lists of Reviewers & Approvers:"), 1);
  QFontMetrics m(bn_revGlobal->font());
  int RowHeight = m.lineSpacing();
  bn_revGlobal->setFixedHeight(1.5 * RowHeight);
...
void US_eSignaturesGMP::init_invs(void) {
  ...
  while (db.next()) {
    ...
    if (lev < 3 && inv != data.invID) continue;

    // Filter by userlevel: cannot be UL=1
    if (data.ulev < 2) continue;

    investigators << data;

    lw_inv_list->addItem(
        new QListWidgetItem("InvID: (" + QString::number(data.invID) + "), " +
...
QString US_eSignaturesGMP::get_inv_or_grev_smry(US_InvestigatorData p_info,
                                                QString p_type) {
  QString smry;
  QStringList mlines;
  ...
  for (int ii = 0; ii < mlines.count(); ii++) smry += mlines[ii] + "\n";
...
void US_eSignaturesGMP::init_grevs(void) {
  ...
  if (!reassign_revs_sep) lw_grev_list->clear();

  qDebug() << "init_grevs DEB1";

  if (!assign_revs_sep) cb_choose_rev->clear();
...
void US_eSignaturesGMP::init_gapprs(void) {
  g_apprs.clear();
  if (!reassign_revs_sep) lw_gappr_list->clear();

  qDebug() << "init_gapprs DEB1";

  if (!assign_revs_sep) cb_choose_appr->clear();
...
void US_eSignaturesGMP::set_greviewer() {
  ...
  if (!assign_revs_sep) setUnset_AddRemove_RevAppr_bttn("Reviewer");
...
void US_eSignaturesGMP::set_gappr() {
  ...
  if (!assign_revs_sep) setUnset_AddRemove_RevAppr_bttn("Approver");
...
void US_eSignaturesGMP::set_revOper_panel_gui_sa(void) {
  ...
  if (operators_info.contains("completed") &&
    ...
    te_opers_to_assign->setText(
        "<font color=\"Red\">All operators have eSigned, cannot be "
        "reassigned!</font>");
    pb_add_oper->setEnabled(false);
    pb_remove_oper->setEnabled(false);
  ...
  if (reviewers_info.contains("completed") &&
    ...
    te_revs_to_assign->setText(
        "<font color=\"Red\">All reviewers have eSigned, cannot be "
        "reassigned!</font>");
  ...
  if (approvers_info.contains("completed") &&
    ...
    te_apprs_to_assign->setText(
        "<font color=\"Red\">All approvers have eSigned, cannot be "
        "reassigned!</font>");
...
QString US_eSignaturesGMP::get_assigned_oper_revs_sa(
    QString role, QJsonDocument jsonDoc, QMap<QString, QString> esign_det) {
  QString smry;
  QStringList assigned_list;
  ...
  for (int ii = 0; ii < assigned_list.count(); ii++) {
    ...
    if (ii != assigned_list.count() - 1) smry += "<br>";

    if (!estatus.contains("NOT")) ++esigned_all;
  }
...
int US_eSignaturesGMP::list_all_autoflow_records(
    QList<QStringList>& autoflowdata, QString type) {
  int nrecs = 0;
  autoflowdata.clear();
  ...
  if (type == "CURRENT") qry << "get_autoflow_desc";
  db->query(qry);
...
QMap<QString, QString> US_eSignaturesGMP::read_autoflow_record(int autoflowID,
  ...
  if (type == "CURRENT") qry << "read_autoflow_record";
  ...
  US_ProtocolUtil::read_record_auto(run_info["protocolName"],
                                    run_info["invID_passed"].toInt(), &xmlstr,
                                    NULL, db);
...
QMap<QString, QString> US_eSignaturesGMP::read_autoflowGMPReportEsign_record(
    QString aID) {
  ...
  if (db->lastErrno() == US_DB2::OK)  // e-Sign record exists
  {
    ...
    // 1\",\"Operator 2\",\"Operator 3\"]" )); eSign_record[ "reviewersListJson"
    // ] = QString( tr( "[\"Reviewer 1\",\"Reviewer 2\",\"Reviewer 3\"]" ));
    // //END TEST ------------------------
  }
...
void US_eSignaturesGMP::assignOperRevs_sa(void) {
  ...
  qDebug() << "exsisting_oper_list, exsisting_rev_list, exsisting_appr_list: "
           << exsisting_oper_list << exsisting_rev_list << exsisting_appr_list;
  ...
  compose_updated_ora_list(operListJsonArray, opers_updated,
                           ...
                           oper_list);
  compose_updated_ora_list(revListJsonArray, revs_updated, updated_to_sign_list,
                           exsisting_rev_list, rev_list);
  ...
  if (old_opers == new_opers && old_revs == new_revs &&
      old_apprs == new_apprs) {
...
QString US_eSignaturesGMP::parse_new_ora(QString new_ora) {
  ...
  for (int i = 0; i < r_list_list.size(); ++i) {
    ...
    if (i != r_list_list.size() - 1) f_list += ", ";
  }
  return f_list;
}

// updated lists of o,r,a
void US_eSignaturesGMP::compose_updated_ora_list(QString& jsonArrayList,
  ...
  for (int i = 0; i < new_listList.size(); ++i) {
    ...
    updated_list +=
        new_listList[i] + "(<font color=\"Blue\"><b>NOT SIGNED<\b></font>)";
    updated_list += "<br>";
...
void US_eSignaturesGMP::assignOperRevs(void) {
  ...
  ***/
  ...
  if (is_eSignProcessBegan()) {
    QMessageBox::information(
        this, tr("Operator(s), Reviewer(s), Approver(s) CANNOT be Assigned"),
  ...
  **/
                       ...
                       // minimal status, createupdatelog
  {
    ...
    qry.clear();
    qry << "update_autoflow_with_gmpReviewID" << gmp_run_details["autoflowID"]
...
bool US_eSignaturesGMP::is_eSignProcessBegan(void) {
  ...
  if (!isEsignRecord) return isBegan;
...
QString US_eSignaturesGMP::compose_updated_admin_logJson(
  ...
  if (!jsonDoc.isObject()) {
    qDebug() << "compose_updated_admin_logJson(): createUpdateLogJson: NOT a "
                "JSON Doc !!";
    return e_logJson;
  }
  ...
  for (const auto& i : createUpdateKeys) {
    if (i.contains("Updated by")) ++update_count;
  }

  // Appended portion
  QString logJsonUpdateTime =
      ",\"Updated by (" + QString::number(update_count) + ")\":[{\"Person\":";
  ...
  // opers
  logJsonUpdateTime +=
      "Operator(s) replacement:" + compare_ora_lists(old_opers, new_opers) +
      ";";
  // revs
  logJsonUpdateTime +=
      "Reviewer(s) replacement:" + compare_ora_lists(old_revs, new_revs) + ";";
  // apprs
...
QString US_eSignaturesGMP::compare_ora_lists(QString old_ora, QString new_ora) {
  ...
  if (old_ora != new_ora) r_str = "(" + old_ora + ") to (" + new_ora + ")";
...
int US_eSignaturesGMP::list_all_gmp_reports_db(
  ...
  while (db->next()) {
    ...
    if (jsonDocRevList.isArray() && jsonDocOperList.isArray() &&
      ...
      // additionally filter by userID for {operator, reviewer, appr.} in case
      // of SEPARATE const.
      if (auto_separate_status) {
...
QString US_eSignaturesGMP::check_revs_esign_status_sa(
...
QLineEdit* US_eSignaturesGMP::check_eSign_status_for_gmpReport_auto(
    QString u_passed, QMap<QString, QString> eSign_stats) {
...
void US_eSignaturesGMP::remove_files_by_mask(QString dirPath,
...
void US_eSignaturesGMP::esign_report(void) {
  ...
  qDebug() << "eSign: operatorListJson, reviewersListJson, approversListJson, "
              "eSignStatusAll -- "
  ...
  if (to_esign.isUndefined() || to_esign_array.size() == 0 ||
      !to_esign_array.size() || eSignStatusAll == "YES") {
    qDebug() << "to_eSign(): All signatures have been collected; noone left to "
                "e-sign !!";
  ...
  if (u_ID != current_esignee_in_order_id.toInt()) {
    ...
    qDebug() << "Current esignee, logged in user -- " << u_ID << ". " << u_lname
             << ", " << u_fname << ",    " << current_esignee_in_order;
  ...
  if (gmp_esigning_map_size == 0 || gmp_esigning_map.keys().isEmpty()) return;
  ...
  QString html_eSignatures = write_pdf_eSignatures(
      filePath_db, eSignStatusJson_updated, operatorListJson, reviewersListJson,
      approversListJson);
  ...
  if (f_html.open(QFile::ReadOnly | QFile::Text)) return;
  ...
  if (!old_path.isEmpty()) html_assembled.replace(old_path, baseDir);
  ...
  // every time..]
  QDir w_dir(baseDir);
  if (w_dir.remove("html_string.html")) qDebug() << "HMTL removed !!";
  ...
  if (eSignStatusAll_updated == "YES")
    msg_f +=
        tr("<br><br>All parties e-Signed the report!"
...
// transform
QMap<QString, QMap<QString, QString>> US_eSignaturesGMP::json_to_qmap(
    QString eSignStatusJson_updated) {
  QMap<QString, QMap<QString, QString>> esigners_info;
...
QString US_eSignaturesGMP::get_role_by_name(QString u_name) {
  QJsonDocument jsonDocOperList =
      QJsonDocument::fromJson(eSign_details_auto["operatorListJson"].toUtf8());
  QString opers_a = get_assigned_oper_revs(jsonDocOperList);

  QJsonDocument jsonDocRevList =
      QJsonDocument::fromJson(eSign_details_auto["reviewersListJson"].toUtf8());
  QString revs_a = get_assigned_oper_revs(jsonDocRevList);

  QJsonDocument jsonDocApprList =
      QJsonDocument::fromJson(eSign_details_auto["approversListJson"].toUtf8());
  QString apprs_a = get_assigned_oper_revs(jsonDocApprList);
...
void US_eSignaturesGMP::printDocument(
    QPrinter& printer, QTextDocument* doc,
...
void US_eSignaturesGMP::paintPage(
  ...
  footerRect.setHeight(eSigners_number *
...
QString US_eSignaturesGMP::compose_updated_eSign_Json_sa(
    int u_ID, QString u_fname, QString u_lname, QString updated_to_sign_list,
  ...
  for (int i = 0; i < esigned_array.size(); ++i) {
    foreach (const QString& key, esigned_array[i].toObject().keys()) {
      ...
      qDebug() << "Updating E-Signed section - " << key
               << ": Comment, timeDate -- " << newObj["Comment"].toString()
               << newObj["timeDate"].toString();
  ...
  if (esigned_array.size() > 0) esigned_str.chop(1);
  esigned_str += "]";
...
QString US_eSignaturesGMP::compose_updated_eSign_Json(
    int u_ID, QString u_fname, QString u_lname, QJsonArray to_esign_array,
    QJsonArray esigned_array, QString comment_esignee,
    QString& eSignStatusAll_updated) {
  QString statusJson = "{";
...
QString US_eSignaturesGMP::write_pdf_eSignatures(QString filePath,
  ...
  if (!jsonDocEsign.isObject()) {
    qDebug()
        << "write_pdf_eSigns(): ERROR: eSignStatusJson_upd: NOT a JSON Doc !!";
  ...
  for (int i = 0; i < esigned_array.size(); ++i) {
    foreach (const QString& key, esigned_array[i].toObject().keys()) {
      ...
      qDebug() << "write_pdf_eSign() -- " << key
               ...
               << newObj["Comment"].toString() << newObj["timeDate"].toString()
               << eSigner_role;
...
void US_eSignaturesGMP::write_download_eSignatures_DB(QString filePath,
                                                      QString proc_name) {
  QString fpath = filePath;
  fpath.replace(filePath.section('/', -1), "");
