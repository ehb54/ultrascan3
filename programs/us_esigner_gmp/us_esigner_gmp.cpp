...
US_eSignaturesGMP::US_eSignaturesGMP () : US_Widgets ()
{
  ...
  QLabel *bn_revGlobal = us_banner (
      tr ("Assign Global Reviewers from the List of Investigators:"), 1);
  ...
  connect (pb_unset_global_rev, SIGNAL (clicked ()),
           SLOT (unset_greviewer ()));
  ...
  QLabel *bn_revOperGMP
      = us_banner (tr ("Assign Operator and Reviewer(s) for GMP Run:"), 1);
  bn_revOperGMP->setFixedHeight (1.5 * RowHeight);

  pb_selRun_operRev_set = us_pushbutton (tr ("Select GMP Run"));
  pb_set_operRev
      = us_pushbutton (tr ("Assign/Change Operators and Reviewers"));
  ...
  pb_loadreport_db = us_pushbutton (
      tr ("Load GMP Report from DB (.PDF) with Assigned Reviewers"));
  ...
  pb_esign_report->setStyleSheet (
      tr ("QPushButton::enabled {background: #556B2F; color: lightgray; }"
...
US_eSignaturesGMP::US_eSignaturesGMP (QStringList reassign) : US_Widgets ()
{
  ...
  setWindowTitle (
      tr ("GMP e-Signatures: Reassigning Operators, Reviewers, Approvers"));
  // setPalette( US_GuiSettings::frameColor() );
  setPalette (US_GuiSettings::normalColor ());
  ...
  QLabel *bn_revOperGMP = us_banner (
      tr ("Reassign Operator(s), Reviewer(s), Approver(s) for GMP Report:"),
      1);
  QFontMetrics m (bn_revOperGMP->font ());
  int RowHeight = m.lineSpacing ();
  bn_revOperGMP->setFixedHeight (1.5 * RowHeight);

  pb_selRun_operRev_set
      = us_pushbutton (tr ("Select Completed GMP Run with Generated Report"));
  pb_set_operRev = us_pushbutton (
      tr ("Reassign Operator(s), Reviewer(s) and Approver(s)"));
  pb_set_operRev->setEnabled (false);
  ...
  connect (pb_selRun_operRev_set, SIGNAL (clicked ()),
           SLOT (selectGMPRun_sa ()));
  ...
  if (a_mode == "SEPARATE")
    {
      ...
      QLabel *bn_loadGMPReport
          = us_banner (tr ("Select GMP Report to Review:"), 1);
      ...
      // QSpacerItem* spacer2 = new QSpacerItem( 20, 1*ihgt,
      // QSizePolicy::Expanding); topLayout_auto->addItem( spacer2 );

      connect (pb_loadreport_db, SIGNAL (clicked ()),
               SLOT (loadGMPReportDB_assigned_separate ()));
...
void
US_eSignaturesGMP::loadGMPReportDB_assigned_separate (void)
{
  US_Passwd pw;
...
void
US_eSignaturesGMP::reset_esign_panel (void)
{
  ...
  // same for other:
  if (eSignActionsGrid_auto != NULL
      && eSignActionsGrid_auto->layout () != NULL)
    {
      QLayoutItem *item;
...
void
US_eSignaturesGMP::initPanel_auto (QMap<QString, QString> &protocol_details)
{
  ...
  // //DEBUG
  // QMap<QString, QMap< QString, QString>> eSigned_so_fa = json_to_qmap(
  // eSign_details_auto[ "eSignStatusJson" ]); QStringList esigner_list =
  // eSigned_so_fa.keys(); qDebug() << "E-Signer's list -- " << esigner_list;
  // //DEBUG
...
void
US_eSignaturesGMP::display_reviewers_auto (int &row,
                                           QMap<QString, QString> eSign_d,
                                           QString JsonListName)
{
  if (eSign_d.contains (JsonListName))
    {
      QJsonDocument jsonDocOperList
          = QJsonDocument::fromJson (eSign_d[JsonListName].toUtf8 ());
      ...
      for (int i = 0; i < jsonDocOperList_array.size (); ++i)
        {
          ...
          QLineEdit *le_stat = check_eSign_status_for_gmpReport_auto (
              current_reviewer, eSign_d);
...
US_eSignaturesGMP::US_eSignaturesGMP (QMap<QString, QString> &it_details)
    : US_Widgets ()
{
  ...
  QLabel *bn_revGlobal = us_banner (
      tr ("Create | Edit Global Lists of Reviewers & Approvers:"), 1);
  QFontMetrics m (bn_revGlobal->font ());
  int RowHeight = m.lineSpacing ();
  bn_revGlobal->setFixedHeight (1.5 * RowHeight);
...
void
US_eSignaturesGMP::init_invs (void)
{
  investigators.clear ();
  ...
  while (db.next ())
    {
      ...
      lw_inv_list->addItem (new QListWidgetItem (
          "InvID: (" + QString::number (data.invID) + "), " + data.lastName
          + ", " + data.firstName));
    }
}

void
US_eSignaturesGMP::limit_inv_names (const QString &s)
{
  ...
  for (int i = 0; i < investigators.size (); i++)
    {
      if (investigators[i].lastName.contains (
        ...
        lw_inv_list->addItem (new QListWidgetItem (
            ...
            + investigators[i].lastName + ", " + investigators[i].firstName));
    }
}

void
US_eSignaturesGMP::limit_grev_names (const QString &s)
{
  lw_grev_list->clear ();
  ...
  for (int i = 0; i < g_reviewers.size (); i++)
    {
      if (g_reviewers[i].lastName.contains (
        ...
        lw_grev_list->addItem (new QListWidgetItem (
            ...
            + g_reviewers[i].lastName + ", " + g_reviewers[i].firstName));
    }
}

void
US_eSignaturesGMP::limit_gappr_names (const QString &s)
{
  lw_gappr_list->clear ();
  ...
  for (int i = 0; i < g_apprs.size (); i++)
    {
      if (g_apprs[i].lastName.contains (
        ...
        lw_gappr_list->addItem (new QListWidgetItem (
            ...
            + g_apprs[i].lastName + ", " + g_apprs[i].firstName));
    }
}

void
US_eSignaturesGMP::get_inv_data (QListWidgetItem *item)
{
...
QString
US_eSignaturesGMP::get_inv_or_grev_smry (US_InvestigatorData p_info,
                                         QString p_type)
{
  QString smry;
  QStringList mlines;
...
void
US_eSignaturesGMP::init_grevs (void)
{
  g_reviewers.clear ();
...
void
US_eSignaturesGMP::init_gapprs (void)
{
  g_apprs.clear ();
  if (!reassign_revs_sep)
    lw_gappr_list->clear ();
...
void
US_eSignaturesGMP::get_grev_data (QListWidgetItem *item)
{
  QString entry = item->text ();
...
void
US_eSignaturesGMP::get_gappr_data (QListWidgetItem *item)
{
  QString entry = item->text ();
...
void
US_eSignaturesGMP::set_greviewer ()
{
...
void
US_eSignaturesGMP::unset_greviewer ()
{
...
void
US_eSignaturesGMP::selectGMPRun (void)
{
...
void
US_eSignaturesGMP::selectGMPRun_sa (void)
{
  /* Here, we need to list all completed GMP runs (i.e. from autoflowHistory)
   */
  /* For HISTORY, check if GMP reports for these runs are NOT completely
   * eSigned */
  ...
  // Get detailed info on the autoflow record
  int autoflowID = autoflow_id_selected.toInt ();
  gmp_run_details = read_autoflow_record (
      autoflowID, "HISTORY"); //<== TODO: autoflowHistory should be read!!
...
void
US_eSignaturesGMP::reset_set_revOper_panel (void)
{
  le_run_name->clear ();
...
void
US_eSignaturesGMP::set_revOper_panel_gui (void)
{
  QString full_runname = gmp_run_details["filename"];
  QString FullRunName_auto
      = gmp_run_details["experimentName"] + "-run" + gmp_run_details["runID"];
  ...
  eSign_details
      = read_autoflowGMPReportEsign_record (gmp_run_details["autoflowID"]);
...
void
US_eSignaturesGMP::set_revOper_panel_gui_sa (void)
{
  QString full_runname = gmp_run_details["filename"];
  QString FullRunName_auto
      = gmp_run_details["experimentName"] + "-run" + gmp_run_details["runID"];
  ...
  QJsonDocument jsonDocOperList
      = QJsonDocument::fromJson (eSign_details["operatorListJson"].toUtf8 ());
  QString opers_a
      = get_assigned_oper_revs_sa ("operator", jsonDocOperList, eSign_details);

  QJsonDocument jsonDocRevList
      = QJsonDocument::fromJson (eSign_details["reviewersListJson"].toUtf8 ());
  QString revs_a
      = get_assigned_oper_revs_sa ("reviewer", jsonDocRevList, eSign_details);

  QJsonDocument jsonDocApprList
      = QJsonDocument::fromJson (eSign_details["approversListJson"].toUtf8 ());
  QString apprs_a
      = get_assigned_oper_revs_sa ("approver", jsonDocApprList, eSign_details);
  ...
  if (operators_info.contains ("completed")
      && operators_info["completed"] == "YES")
    {
      te_opers_to_assign->setText ("<font color=\"Red\">All operators have "
                                   "eSigned, cannot be reassigned!</font>");
      pb_add_oper->setEnabled (false);
      pb_remove_oper->setEnabled (false);
  ...
  if (reviewers_info.contains ("completed")
      && reviewers_info["completed"] == "YES")
    {
      te_revs_to_assign->setText ("<font color=\"Red\">All reviewers have "
                                  "eSigned, cannot be reassigned!</font>");
  ...
  if (approvers_info.contains ("completed")
      && approvers_info["completed"] == "YES")
    {
      te_apprs_to_assign->setText ("<font color=\"Red\">All approvers have "
                                   "eSigned, cannot be reassigned!</font>");
...
QString
US_eSignaturesGMP::get_assigned_oper_revs (QJsonDocument jsonDoc)
{
  QString smry;
...
QString
US_eSignaturesGMP::get_assigned_oper_revs_sa (QString role,
                                              QJsonDocument jsonDoc,
                                              QMap<QString, QString> esign_det)
{
  QString smry;
  QStringList assigned_list;
  ...
  for (int ii = 0; ii < assigned_list.count (); ii++)
    {
      // Check eSign status
      QString estatus
          = check_revs_esign_status_sa (assigned_list[ii], esign_det);
      QString stat_str;
      estatus.contains ("NOT")
          ? stat_str = "(<font color=\"Blue\"><b>" + estatus + "<\b></font>)"
...
int
US_eSignaturesGMP::list_all_autoflow_records (QList<QStringList> &autoflowdata,
                                              QString type)
{
  int nrecs = 0;
  autoflowdata.clear ();
  ...
  while (db->next ())
    {
      ...
      /* Check for HISTORY -- if autoflowGMPReportEsign where ID=devRecord
       * EXISTS!!! */
      /* If not - GMP report completely eSigned, so do not include in the list
       * (cont.) */
      if (type == "HISTORY")
        {
          QMap<QString, QString> eSign_rec
              = read_autoflowGMPReportEsign_record (id);
...
US_eSignaturesGMP::read_autoflow_record (int autoflowID, QString type)
{
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd ();
...
QStringList
US_eSignaturesGMP::read_operators (QString optima_id)
{
...
QMap<QString, QString>
US_eSignaturesGMP::read_autoflowGMPReportEsign_record (QString aID)
{
  ...
  if (db->lastErrno () == US_DB2::OK) // e-Sign record exists
  ...
  else
    {
      ...
      // //TEST ---------------------------
      // eSign_record[ "operatorListJson" ]  = QString( tr( "[\"Operator
      // 1\",\"Operator 2\",\"Operator 3\"]" )); eSign_record[
      // "reviewersListJson" ] = QString( tr( "[\"Reviewer 1\",\"Reviewer
      // 2\",\"Reviewer 3\"]" ));
      // //END TEST ------------------------
    }
...
// Disable all action buttons:
void
US_eSignaturesGMP::disableSetUnsetGButtons (void)
{
  pb_set_global_rev->setEnabled (false);
  pb_set_global_appr->setEnabled (false);
  pb_unset_global_rev->setEnabled (false);
...
void
US_eSignaturesGMP::setUnset_AddRemove_RevAppr_bttn (QString p_type)
{
  if (p_type == "Reviewer")
    {
...
void
US_eSignaturesGMP::setUnsetPb_operRev (void)
{
...
void
US_eSignaturesGMP::addOpertoList (void)
{
...
void
US_eSignaturesGMP::removeOperfromList (void)
{
...
void
US_eSignaturesGMP::addRevtoList (void)
{
...
void
US_eSignaturesGMP::removeRevfromList (void)
{
...
// Add reviewer to list
void
US_eSignaturesGMP::addApprtoList (void)
{
...
// Remove reviewer from list
void
US_eSignaturesGMP::removeApprfromList (void)
{
...
void
US_eSignaturesGMP::assignOperRevs_sa (void)
{
  ...
  qDebug () << "exsisting_oper_list, exsisting_rev_list, exsisting_appr_list: "
            << exsisting_oper_list << exsisting_rev_list
            << exsisting_appr_list;
  ...
  qDebug () << "new lists: " << oper_list << rev_list << appr_list;

  // Compose JSON arrays: QString( tr( "[\"Operator 1\",\"Operator
  // 2\",\"Operator 3\"]" ));
  ...
  /* if partially signed, insert in respective lists those who already signed,
   * then the rest (new ones) */

  QString opers_updated, revs_updated, apprs_updated;

  compose_updated_ora_list (operListJsonArray, opers_updated,
                            updated_to_sign_list, exsisting_oper_list,
                            oper_list);
  compose_updated_ora_list (revListJsonArray, revs_updated,
                            updated_to_sign_list, exsisting_rev_list,
                            rev_list);
  compose_updated_ora_list (apprListJsonArray, apprs_updated,
                            updated_to_sign_list, exsisting_appr_list,
                            appr_list);
  ...
  qDebug () << "[OLD] eSignStatusJson, eSignStatusAll, eSignID, "
               "eSigncreateUpdateLogJson -- "
            << eSignStatusJson << eSignStatusAll << eSignID
            << eSigncreateUpdateLogJson;

  QJsonDocument jsonDocEsign
      = QJsonDocument::fromJson (eSignStatusJson.toUtf8 ());
  ...
  msg_rev.setText (tr ("<font color='red'><b>ATTENTION:</b> </font><br>"
                       "You are about to change assigned operator(s) and/or "
                       "reviewer(s) and/or approver(s): <br><br>"
  ...
  QPushButton *Accept
      = msg_rev.addButton (tr ("Proceed"), QMessageBox::YesRole);
  QPushButton *Cancel
      = msg_rev.addButton (tr ("Cancel"), QMessageBox::RejectRole);
  ...
  if (msg_rev.clickedButton () == Accept)
    {
      ...
      QString eSignStatusJson_updated = compose_updated_eSign_Json_sa (
          u_ID, u_fname, u_lname, updated_to_sign_list, esigned_array);
      qDebug () << "eSignStatusJson_updated -- " << eSignStatusJson_updated;

      // 2: updated admin log
      QString logJsonUpdateTime = compose_updated_admin_logJson (
          u_ID, u_fname, u_lname, old_opers, new_opers, old_revs, new_revs,
          old_apprs, new_apprs);
      qDebug () << "logJsonUpdateTimeJsonObject -- " << logJsonUpdateTime;

      qry.clear ();
      qry << "update_gmp_review_record_by_admin" << eSign_details["ID"]
          << gmp_run_details["autoflowID"] << operListJsonArray
          << revListJsonArray << apprListJsonArray
          << eSign_details["smeListJson"] // do NOT change SME!
          << eSignStatusJson_updated << logJsonUpdateTime;
...
QString
US_eSignaturesGMP::parse_old_ora (QString ex_ora)
{
...
QString
US_eSignaturesGMP::parse_new_ora (QString new_ora)
{
...
void
US_eSignaturesGMP::compose_updated_ora_list (QString &jsonArrayList,
                                             QString &updated_list,
                                             QString &updated_to_sign_list,
                                             QString existing_ora,
                                             QString new_ora)
{
  ...
  if (!new_ora.contains (
          "cannot be reassigned")) // Not esigned, or partially esigned
    {
      for (int i = 0; i < existing_listList.size (); ++i)
        {
          if (!existing_listList[i].contains ("NOT SIGNED")) // eSigned already
            {
              jsonArrayList += "\""
                               + existing_listList[i].split ("(")[0].trimmed ()
                               + "\",";
              updated_list += existing_listList[i].split ("(")[0].trimmed ()
                              + "(<font color=\"Red\"><b>SIGNED<\b></font>)";
              updated_list += "<br>";
            }
        }
    }
  ...
  for (int i = 0; i < new_listList.size (); ++i)
    {
      ...
      updated_list += new_listList[i]
                      + "(<font color=\"Blue\"><b>NOT SIGNED<\b></font>)";
      updated_list += "<br>";
...
void
US_eSignaturesGMP::assignOperRevs (void)
{
  /**
      0. FIRST, check if e-Signature process already BEGAN !!!!         <-- 1st
  thing to check
        -- if YES, STOP!!!!
              ** if (isEsignRecord) BUT eSignStatusJson indicates it began
              ** above && || autoflowStatus indicates 7. e-Signature stage in
  the signing mode
        -- if NOT, PROCEED:
              ** if (!isEsignRecord)
              ** if (isEsignRecord) BUT eSignStatusJson HAS NOT began: at least
  1 signature put!!!
  ***/
  ...
  if (is_eSignProcessBegan ())
    {
      QMessageBox::information (
          this,
          tr ("Operator(s), Reviewer(s), Approver(s) CANNOT be Assigned"),
          tr ("<font color='red'><b>ATTENTION:</b> </font> Operator(s), "
              "Reviewer(s) and/or Approver(s)"
              "cannot be set/changed for the currently uploaded GMP run: "
              "<br><br>"
              "<font ><b>%1</b><br><br>"
  ...
  qDebug () << "revListJsonArray -- " << revListJsonArray;
  qDebug () << "apprListJsonArray -- " << apprListJsonArray;
  qDebug () << "smeListJsonArray -- "
            << apprListJsonArray; // Empty for now... [to be populated from
                                  // te_sem...]
  ...
  **/
  ...
  if (!isEsignRecord) // No eSignature Record exists, so create new one, with
                      // minimal status, createupdatelog
    {
      // Update te fileds
      ...
      qry.clear ();
      qry << "new_gmp_review_record" << gmp_run_details["autoflowID"]
          << gmp_run_details["protocolName"] << operListJsonArray
          << revListJsonArray << apprListJsonArray << smeListJsonArray
          << eSignStatusJson << logJsonFirstTime;
  ...
  else // exists eSign record: so update as ADMIN
    {
      ...
      // check if at least one of tehe operator OR reviewer lists has been
      // changed
      if (exsisting_oper_list == oper_list && exsisting_rev_list == rev_list
          && exsisting_appr_list == appr_list)
        {
          qDebug () << "Operators and Reviewers and Approvers are the same...";
          QMessageBox::information (
              this, tr ("Operator(s), Reviewer(s), Approver(s) NOT CHANGED"),
              tr ("Existing and to-be assigned "
                  "Operator(s), Reviewer(s), and Approver(s) "
                  "lists are the same.<br><br>"
                  "Nothing will be changed..."));
      ...
      QMessageBox msg_rev;
      msg_rev.setWindowTitle (
          tr ("Operator(s), Reviewer(s), Approver(s) Change"));
      msg_rev.setText (tr ("<font color='red'><b>ATTENTION:</b> </font><br>"
                           "You are about to change assigned operator(s) "
                           "and/or reviewer(s) and/or approver(s): <br><br>"
                           "Old Operators: <font ><b>%1</b><br>"
                           "New Operators: <font ><b>%2</b><br><br>"
                           "Old Reviewers: <font ><b>%3</b><br>"
                           "New Reviewers: <font ><b>%4</b><br><br>"
      ...
      if (msg_rev.clickedButton () == Accept)
        {
          ...
          qry << "update_gmp_review_record_by_admin" << eSign_details["ID"]
              << gmp_run_details["autoflowID"] << operListJsonArray
              << revListJsonArray << apprListJsonArray << smeListJsonArray
              << eSignStatusJson << logJsonUpdateTime;
...
bool
US_eSignaturesGMP::is_eSignProcessBegan (void)
{
  bool isBegan = false;
  eSign_details.clear ();
  eSign_details
  ...
  /****
     ...
     {
        "to_sign": ["Rev1","Rev2"],       <===== ORIGINALLY, full combined list
   of Oper(s) && Rev(s) i.e. ["Oper1","Oper2",..., "Rev1","Rev2",..] "signed" :
   [ { "Oper1":
                              {
                                "Comment"  : "Explanation",
                                "timeDate" : "timestamp"
     ...
     So, to understand if e-Signing started, investigate eSignStatusJson[
   "signed" ] JSON...
   ****/
  ...
  if (!jsonDoc.isObject ())
    {
      qDebug ()
          << "is_eSignProcessBegan(): eSignStatusJson: NOT a JSON Doc !!";
      return isBegan;
    }
  ...
  // signed
  if (esigned.isUndefined () || esigned_array.size () == 0
      || !esigned_array.size ())
    {
      qDebug () << "Nothing has been e-Signed yet !!! Oper(s), Rev(s) CAN BE "
                   "changed/assigned!!!";
      return false;
    }
  else
      ...
      for (int i = 0; i < esigned_array.size (); ++i)
        {
          foreach (const QString &key, esigned_array[i].toObject ().keys ())
            {
              QJsonObject newObj
                  = esigned_array[i].toObject ().value (key).toObject ();
...
QString
US_eSignaturesGMP::compose_updated_admin_logJson (
    int u_ID, QString u_fname, QString u_lname, QString old_opers,
    QString new_opers, QString old_revs, QString new_revs, QString old_apprs,
    QString new_apprs)
{
  ...
  if (!jsonDoc.isObject ())
    {
      qDebug () << "compose_updated_admin_logJson(): createUpdateLogJson: NOT "
                   "a JSON Doc !!";
      return e_logJson;
    }
  ...
  // Appended portion
  QString logJsonUpdateTime = ",\"Updated by ("
                              + QString::number (update_count)
                              + ")\":[{\"Person\":";

  QDateTime date = QDateTime::currentDateTime ();
  QString current_date = date.toString ("MM-dd-yyyy hh:mm:ss");

  logJsonUpdateTime += "\"" + QString::number (u_ID) + ". " + u_lname + ", "
                       + u_fname + "\",";
  logJsonUpdateTime += "\"timeDate\":\"" + current_date + "\",";
  logJsonUpdateTime += "\"Comment\":\"";

  // opers
  logJsonUpdateTime += "Operator(s) replacement:"
                       + compare_ora_lists (old_opers, new_opers) + ";";
  // revs
  logJsonUpdateTime += "Reviewer(s) replacement:"
                       + compare_ora_lists (old_revs, new_revs) + ";";
  // apprs
  logJsonUpdateTime += "Approver(s) replacement:"
                       + compare_ora_lists (old_apprs, new_apprs); // + ";";
...
// check for ora lists
QString
US_eSignaturesGMP::compare_ora_lists (QString old_ora, QString new_ora)
{
  QString r_str = "NONE";
...
void
US_eSignaturesGMP::loadGMPReportDB_assigned_auto (QString aID_passed)
{
  US_Passwd pw;
  US_DB2 db (pw.getPasswd ());
  ...
  filePath_db_html
      = dirName + "/" + gmpReport_runname_selected + "/" + GMPReportfname_html;

  int db_read_html
      = db.readBlobFromDB (filePath_db_html, "download_gmpReportData_html",
                           gmpReport_id_selected.toInt ());
...
// Load GMP Report form Db with assigned operator(s) && reviewer(s)
void
US_eSignaturesGMP::loadGMPReportDB_assigned (void)
{
  US_Passwd pw;
  US_DB2 db (pw.getPasswd ());
  ...
  qDebug () << "After download GMP Report, gmpRunID_eSign -- "
            << gmpRunID_eSign;
}

// Get .pdf GMP reports with assigned reviewers:
int
US_eSignaturesGMP::list_all_gmp_reports_db (
  ...
  while (db->next ())
    {
      ...
      QMap<QString, QString> eSign
          = read_autoflowGMPReportEsign_record (autoflowHistoryID);
      ...
      qDebug () << "logged_user, operatorListJson, reviewersListJson, "
                   "approversListJson -- "
                << logged_user << ", " << operatorListJson << ",   "
                << reviewersListJson << ", " << approversListJson;

      QJsonDocument jsonDocOperList
          = QJsonDocument::fromJson (operatorListJson.toUtf8 ());
      QJsonDocument jsonDocRevList
          = QJsonDocument::fromJson (reviewersListJson.toUtf8 ());
      QJsonDocument jsonDocApprList
          = QJsonDocument::fromJson (approversListJson.toUtf8 ());
      ...
      if (jsonDocRevList.isArray () && jsonDocOperList.isArray ()
          && !operatorListJson.isEmpty () && !reviewersListJson.isEmpty ())
        {
          gmpreportentry << id << autoflowHistoryName // << protocolName
                         ...
                         << filenamePdf << autoflowHistoryID;

          // additionally filter by userID for {operator, reviewer, appr.} in
          // case of SEPARATE const.
          if (auto_separate_status)
            {
...
QString
US_eSignaturesGMP::check_revs_esign_status_sa (
    QString u_passed, QMap<QString, QString> eSign_stats)
{
  QString eSignStatusJson = eSign_stats["eSignStatusJson"];
  QString eSignStatusAll = eSign_stats["eSignStatusAll"];

  qDebug () << "In check_eSign_status_for_gmpReport_auto(): eSignStatusJson, "
               "eSignStatusAll -- "
  ...
  if (to_esign.isUndefined () || to_esign_array.size () == 0
    ...
    {
      qDebug () << "check_eSign_status(): All signatures have been collected; "
                   "none left to e-sign !!";
  ...
  if (esigned.isUndefined () || esigned_array.size () == 0
    ...
    {
      qDebug ()
          << "check_eSign_status(): Some parties have e-Signed already !!!";
      // DEBUG
          ...
          foreach (const QString &key, esigned_array[i].toObject ().keys ())
            {
              ...
              QString current_reviewer_id
                  = current_reviewer.section (".", 0, 0);

              eSignees_current << key;
            }
        }
      // END DEBUG:
      qDebug () << "check_eSign_status(): so far, e-signed by: "
                << eSignees_current;
...
QLineEdit *
US_eSignaturesGMP::check_eSign_status_for_gmpReport_auto (
    QString u_passed, QMap<QString, QString> eSign_stats)
{
  ...
  if (to_esign.isUndefined () || to_esign_array.size () == 0
      || !to_esign_array.size () || eSignStatusAll == "YES")
    {
      ...
      le_stat->setStyleSheet (
          "QLineEdit { background-color:  rgb(50, 205, 50); }"); // green
  ...
  if (esigned.isUndefined () || esigned_array.size () == 0
      || !esigned_array.size ())
    {
      ...
      le_stat->setStyleSheet (
          "QLineEdit { background-color:  rgb(210, 0, 0); }"); // red
...
QString
US_eSignaturesGMP::check_eSign_status_for_gmpReport (void)
{
  QMap<QString, QString> stats;
  QMap<QString, QString> eSign
      = read_autoflowGMPReportEsign_record (gmpRunID_eSign);
  QString eSignStatusJson = eSign["eSignStatusJson"];
  ...
  if (esigned.isUndefined () || esigned_array.size () == 0
  ...
  else
    {
      ...
      le_eSign_status->setStyleSheet (
          "QLineEdit { background-color:  rgb(95, 152, 238); }"); // blue
...
void
US_eSignaturesGMP::remove_files_by_mask (QString dirPath,
...
// view report DB
void
US_eSignaturesGMP::view_report_db ()
{
...
void
US_eSignaturesGMP::view_eSignatures ()
{
  ...
  qDebug ()
      << "Opening PDF for e-Signatures (either downloaded OR generated) at -- "
      << filePath_to_eSigns;
  ...
  if (check_file.exists () && check_file.isFile ())
  ...
  else
    {
      QMessageBox::warning (this, tr ("Error: Cannot Open .PDF File"),
                            tr ("%1 \n\n"
                                "No such file or directory...")
                                .arg (filePath_to_eSigns));
    }
}

// Create a subdirectory if need be
bool
US_eSignaturesGMP::mkdir (const QString &baseDir, const QString &subdir)
{
  QDir folder (baseDir);
...
// eSign GMP Report
void
US_eSignaturesGMP::esign_report (void)
{

  qDebug () << "Extracted .PDF of ORIGINAL GMP Report filepath -- "
            << filePath_db;
  qDebug () << ".PDF of the current eSign_filepath -- " << filePath_eSign;
  qDebug () << "temp debug -- ";

  // first, check if you (as logged in user) are among listed operators &&
  // reviewers
  US_Passwd pw;
  US_DB2 db (pw.getPasswd ());
  ...
  qDebug () << "eSign: user_id, gmpRunID_eSign -- " << u_ID << gmpRunID_eSign;
  ...
  qDebug () << "eSign: operatorListJson, reviewersListJson, "
               "approversListJson, eSignStatusAll -- "
            << operatorListJson << reviewersListJson << approversListJson
            << eSignStatusAll;
  ...
  if (to_esign.isUndefined () || to_esign_array.size () == 0
      || !to_esign_array.size () || eSignStatusAll == "YES")
    {
      qDebug () << "to_eSign(): All signatures have been collected; noone "
                   "left to e-sign !!";

      QMessageBox::information (
          this, tr ("GMP Report e-Signed"),
  ...
  QString current_esignee_in_order_id
      = current_esignee_in_order.section (".", 0, 0);
          ...
          foreach (const QString &key, esigned_array[i].toObject ().keys ())
            {
              ...
              if (u_ID == current_reviewer_id.toInt ())
                {
                  qDebug ()
                      << "Current user, " << key << ", has already e-Signed!";
                  yesWasSigned = true;
                  break;
                }
            }
  ...
  if (u_ID != current_esignee_in_order_id.toInt ())
    {
      qDebug () << "u_ID, current_esignee_in_order_id -- " << u_ID
                << current_esignee_in_order_id.toInt ();
      qDebug () << "Current esignee, logged in user -- " << u_ID << ". "
                << u_lname << ", " << u_fname << ",    "
      ...
      QMessageBox::information (
          this, tr ("Cannot e-Sign: Wrong Order"),
          tr ("<font color='red'><b>WRONG E-SIGNING ORDER:</b> </font> <br>"
              "You have to wait for the following reviewers to e-Sign before "
              "proceeding: <br><br>"
              "<b>%1</b><br><br>"
  ...
  // ) ); QString default_text = QString(tr("e-Sign Comment: ")); QString
  // comment_t    = QInputDialog::getText( this, 						tr( "e-Sign Comment" ), 						msg,
  ...
  gmp_esigning_map = pw_at.getPasswd_auditTrail (
      "GMP e-Signing Form",
      "Please fill out GMP e-Signing form:", user_esignee);

  int gmp_esigning_map_size = gmp_esigning_map.keys ().size ();
  qDebug () << "e-Signing map: " << gmp_esigning_map.keys ()
            << gmp_esigning_map.keys ().size () << gmp_esigning_map_size
            << gmp_esigning_map.keys ().isEmpty () << gmp_esigning_map["User:"]
  ...
  qry.clear ();
  qry << "autoflow_esigning_status" << gmpRunID_eSign;
  ...
  if (!status_esign_unique)
    {
      QMessageBox::information (
          this, tr ("The e-Signature record can NOT be updated"),
  ...
  QString eSignStatusJson_updated = compose_updated_eSign_Json (
      u_ID, u_fname, u_lname, to_esign_array, esigned_array,
      gmp_esigning_map["Comment:"], eSignStatusAll_updated);

  qry.clear ();
  qry << "update_gmp_review_record_by_esigner" << eSignID << gmpRunID_eSign
      << eSignStatusJson_updated << eSignStatusAll_updated;
  ...
  QString html_eSignatures = write_pdf_eSignatures (
      filePath_db, eSignStatusJson_updated, operatorListJson,
      reviewersListJson, approversListJson);
  ...
  /***
       ...
       OR
       do we store eSignatures .PDF separately, in autoflowGMPReportEsign
   table??
   ***/
  // We need to paint new .PDF of the GMP report with e-Sigs out of
  // html_string.html file:
  ...
  qDebug () << "Captured regex, size(): " << rx.capturedTexts ()
            << rx.capturedTexts ().size ();

  QString old_path = QString ("");
  if (!rx.capturedTexts ()[0].isEmpty ())
    {
      QString path_t
          = rx.capturedTexts ()[0].split (".png\"")[0]; //.section('/', -1);
  ...
  QMap<QString, QMap<QString, QString> > eSigners_info
      = json_to_qmap (eSignStatusJson_updated);
  ...
  qDebug () << "ID to be updated GMPReport record: "
            << autoflowGMPReport_id_selected.toInt ();

  // //Also, remove eSignatures.pdf before archiving...[will be regenerated
  // every time..]
  QDir w_dir (baseDir);
  ...
  process->start ("tar", QStringList ()
                             << "-cvf" << tarFilename_t << folderRunName);
  process->waitForFinished ();
  process->close ();
  ...
  int writeStatus
      = db.writeBlobToDB (tarFilename_t, QString ("upload_gmpReportData"),
                          autoflowGMPReport_id_selected.toInt ());
  ...
  // Revert 'esigning' stage status back to DEFAULT for other reviewers to be
  // able to e-sign:
  qry.clear ();
  qry << "autoflow_esigning_status_revert" << gmpRunID_eSign;
  db.query (qry);
  /**********************************************************************************************/
  ...
  if (auto_mode)
    {
      QString current_reviewer_name
          = QString::number (u_ID) + ". " + u_lname + ", " + u_fname;
      QString obj_name = "status: " + current_reviewer_name;

      qDebug () << "u_ID, u_lname, u_fname, obj_name -- " << u_ID << u_lname
                << u_fname << obj_name;
      QLineEdit *lineedit_u_status = NULL;
      lineedit_u_status = mainWidget_auto->findChild<QLineEdit *> (
          obj_name, Qt::FindDirectChildrenOnly);
      ...
      if (lineedit_u_status != NULL)
        {
          ...
          lineedit_u_status->setStyleSheet (
              "QLineEdit { background-color:  rgb(50, 205, 50); }"); // green
  ...
  // concluding msg:
  QString msg_f
      = QString (tr ("<font color='red'><b>SUCCESS:</b> </font><br><br>"
  ...
  QMessageBox::information (this, tr ("Successful e-Signing"), msg_f);

  // Move to history
  if (eSignStatusAll_updated == "YES")
    {
      qry.clear ();
      // INSERT INTO autoflowGMPReportEsignHistory SELECT * FROM
      // autoflowGMPReportEsign WHERE ID=${ID}//
...
US_eSignaturesGMP::json_to_qmap (QString eSignStatusJson_updated)
{
  QMap<QString, QMap<QString, QString> > esigners_info;

  QJsonDocument jsonDoc
      = QJsonDocument::fromJson (eSignStatusJson_updated.toUtf8 ());
  if (!jsonDoc.isObject ())
    {
...
QString
US_eSignaturesGMP::get_role_by_name (QString u_name)
{
  QJsonDocument jsonDocOperList = QJsonDocument::fromJson (
      eSign_details_auto["operatorListJson"].toUtf8 ());
  QString opers_a = get_assigned_oper_revs (jsonDocOperList);

  QJsonDocument jsonDocRevList = QJsonDocument::fromJson (
      eSign_details_auto["reviewersListJson"].toUtf8 ());
  QString revs_a = get_assigned_oper_revs (jsonDocRevList);

  QJsonDocument jsonDocApprList = QJsonDocument::fromJson (
      eSign_details_auto["approversListJson"].toUtf8 ());
  QString apprs_a = get_assigned_oper_revs (jsonDocApprList);
...
double
US_eSignaturesGMP::mmToPixels (QPrinter &printer, int mm)
{
  return mm * 0.039370147 * printer.resolution ();
}

void
US_eSignaturesGMP::printDocument (QPrinter &printer, QTextDocument *doc,
                                  QMap<QString, QMap<QString, QString> >
                                      eSigners_info) //, QWidget* parentWidget)
{
  ...
  QProgressDialog *progress_msg = new QProgressDialog (
      "Preparing .PDF...", QString (), 0, pageCount, this);
...
void
US_eSignaturesGMP::paintPage (
    QPrinter &printer, int pageNumber, int pageCount, QPainter *painter,
    ...
    QMap<QString, QMap<QString, QString> > eSigners_info)
{
  ...
  for (int i = 0; i < eSigners_number; ++i)
    {
      ...
      // QString c_es = esigner.split(".")[1].trimmed() + "; ID: " +
      // esigner.split(".")[0].trimmed()
  ...
  footerRect.setHeight (
      eSigners_number * footerHeight); // will a parameter on #of lines (footer
  ...
  // painter->drawText(footerRect, Qt::AlignLeft, QObject::tr(
  // esigners_footer.join("\n") ) );
  painter->drawText (
...
QString
US_eSignaturesGMP::compose_updated_eSign_Json_sa (int u_ID, QString u_fname,
                                                  QString u_lname,
                                                  QString updated_to_sign_list,
                                                  QJsonArray esigned_array)
{
  QString statusJson = "{";
      ...
      foreach (const QString &key, esigned_array[i].toObject ().keys ())
        {
          ...
          qDebug () << "Updating E-Signed section - " << key
                    << ": Comment, timeDate -- "
                    << newObj["Comment"].toString ()
                    << newObj["timeDate"].toString ();

          existing_esignees += "{\"" + key + "\":{\"Comment\":\""
                               + newObj["Comment"].toString () + "\",";
          existing_esignees
              += "\"timeDate\":\"" + newObj["timeDate"].toString () + "\"}},";
        }
    }
...
QString
US_eSignaturesGMP::compose_updated_eSign_Json (int u_ID, QString u_fname,
                                               QString u_lname,
                                               QJsonArray to_esign_array,
                                               QJsonArray esigned_array,
                                               QString comment_esignee,
                                               QString &eSignStatusAll_updated)
{
  QString statusJson = "{";
...
QString
US_eSignaturesGMP::write_pdf_eSignatures (QString filePath,
                                          QString eSignStatusJson_upd,
                                          ...
                                          QString apprList)
{
  ...
  // Parse JSON
  QJsonDocument jsonDocEsign
      = QJsonDocument::fromJson (eSignStatusJson_upd.toUtf8 ());
  if (!jsonDocEsign.isObject ())
    {
      qDebug () << "write_pdf_eSigns(): ERROR: eSignStatusJson_upd: NOT a "
                   "JSON Doc !!";
      ...
      foreach (const QString &key, esigned_array[i].toObject ().keys ())
        {
          ...
          qDebug () << "write_pdf_eSign() -- " << key
                    ...
                    << newObj["Comment"].toString ()
                    << newObj["timeDate"].toString () << eSigner_role;
          ...
          // details
          html_assembled += tr ("<table style=\"margin-left:30px\">"
...
void
US_eSignaturesGMP::write_download_eSignatures_DB (QString filePath,
                                                  QString proc_name)
{
  QString fpath = filePath;
  fpath.replace (filePath.section ('/', -1), "");
  ...
  qDebug ()
      << "write_eSignatures_DB():: Writing Blob of final_fpath, eSignID -- "
  ...
  if (proc_name.contains ("upload"))
    writeStatus
        = db.writeBlobToDB (final_fpath, proc_name, eSignID_global.toInt ());
  if (proc_name.contains ("download"))
    writeStatus
        = db.readBlobFromDB (final_fpath, proc_name, eSignID_global.toInt ());
