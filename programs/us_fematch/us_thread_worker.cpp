...
ThreadWorker::ThreadWorker(
   US_Model &a_model, US_SimulationParameters &params, US_DataIO::RawData &simda, US_Buffer &a_buff, int thr) :
    QObject(), model(a_model), simparams(params), simdat(simda), buffer(a_buff) {
   thrn = thr;
...
void ThreadWorker::calc_simulation() {
   ...
   DbgLv(1) << " THRWRK:" << thrn << "simdat scans radii" << simdat.scanData.size() << simdat.xvalues.size();
   DbgLv(1) << " THRWRK:" << thrn << "model components" << model.components.size();
   DbgLv(1) << " THRWRK:" << thrn << "model-0 s k" << model.components[ 0 ].s * 1.e13 << model.components[ 0 ].f_f0;
   double compress = buffer.compressibility;

   if (model.components[ 0 ].sigma == 0.0 && model.components[ 0 ].delta == 0.0 && model.coSedSolute < 0.0 && compress == 0.0) {
      US_Astfem_RSA *astfem_rsa = new US_Astfem_RSA(model, simparams);

      connect(astfem_rsa, SIGNAL(current_component(int)), this, SLOT(forward_progress(int)));
      ...
      connect(astfvm, SIGNAL(comp_progress(int)), this, SLOT(forward_progress(int)));
