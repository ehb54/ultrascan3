...
ThreadWorker::ThreadWorker (US_Model &a_model, US_SimulationParameters &params,
                            US_DataIO::RawData &simda, US_Buffer &a_buff,
                            int thr)
    : QObject (), model (a_model), simparams (params), simdat (simda),
      buffer (a_buff)
{
  thrn = thr;
...
ThreadWorker::calc_simulation ()
{
  ...
  DbgLv (1) << " THRWRK:" << thrn << "model-0 s k"
            << model.components[0].s * 1.e13 << model.components[0].f_f0;
  ...
  if (model.components[0].sigma == 0.0 && model.components[0].delta == 0.0
  ...
  else
    {
      ...
      connect (astfvm, SIGNAL (comp_progress (int)), this,
               SLOT (forward_progress (int)));
...
void
ThreadWorker::forward_progress (int steps)
{
