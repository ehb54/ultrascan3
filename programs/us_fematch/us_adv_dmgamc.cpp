...
US_AdvDmgaMc::US_AdvDmgaMc(US_Model *amodel, QVector<US_Model> &aimodels, QMap<QString, QString> &adv_vals, QWidget *p) :
   ...
   plotLayout = new US_Plot(data_plot, tr("Attribute Distribution"), tr("Attribute Name"), tr("Frequency"));
   ...
   QLabel *lb_modelsim = us_banner(tr("Simulate data using parameters from "
                                      "Monte Carlo statistics or from "
   ...
   connect(pb_nextmodel, SIGNAL(clicked()), this, SLOT(next_model()));
   connect(ct_modelnbr, SIGNAL(valueChanged(double)), this, SLOT(change_model(double)));
   connect(rb_mean, SIGNAL(toggled(bool)), this, SLOT(set_model_type(bool)));
   connect(rb_median, SIGNAL(toggled(bool)), this, SLOT(set_model_type(bool)));
   connect(rb_mode, SIGNAL(toggled(bool)), this, SLOT(set_model_type(bool)));
   connect(rb_curmod, SIGNAL(toggled(bool)), this, SLOT(set_model_type(bool)));
   connect(pb_prevparm, SIGNAL(clicked()), this, SLOT(prev_param()));
   connect(pb_nextparm, SIGNAL(clicked()), this, SLOT(next_param()));
   connect(cb_params, SIGNAL(activated(int)), this, SLOT(plot_distrib()));

   connect(pb_help, SIGNAL(clicked()), this, SLOT(help()));
   connect(pb_simulate, SIGNAL(clicked()), this, SLOT(simulate()));
   connect(pb_close, SIGNAL(clicked()), this, SLOT(done()));

   adjustSize();
   QFontMetrics fmet(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() - 1));
   ...
   le_modtype->setText(tr("Mean model, %1 components, %2 reaction(s)").arg(ncomp).arg(nreac));
...
void US_AdvDmgaMc::done(void) {
   ...
   parmap[ "modelsim" ] = rb_mean->isChecked()
                             ? "mean"
                             : (rb_median->isChecked()
                                   ? "median"
                                   : (rb_mode->isChecked() ? "mode" : (rb_curmod->isChecked() ? "model" : "")));
...
void US_AdvDmgaMc::set_model_type(bool chekd) {
   ...
   le_modtype->setText(mtlabl + tr(" model,  %1 components, %2 reaction(s)").arg(ncomp).arg(nreac));
...
void US_AdvDmgaMc::plot_distrib() {
   ...
   for (int ii = 0; ii < nxi; ii++) { // Accumulate the input model X distribution values
      US_Model::SimulationComponent *sc;
      US_Model::Association *as;
      double xval = 0.0;

      if (jr < 0) { // Get a component attribute value
         ...
         xval = xtitle.contains(tr("Concentration")) ? sc->signal_concentration : xval;
      }

      else { // Get a reaction attribute value
   ...
   if (nxyp < 3) { // If only 1 or 2 points, expand to center points in X
      nxyp += 2;
      xvec_pl.resize(nxyp);
      yvec_pl.resize(nxyp);

      if (nxyp == 3) { // If there was 1, expand to 3
         xvec_pl[ 1 ] = xvec_pl[ 0 ];
         yvec_pl[ 1 ] = yvec_pl[ 0 ];
      ...
      else { // If there were 2, expand to 4
         xvec_pl[ 2 ] = xvec_pl[ 1 ];
         yvec_pl[ 2 ] = yvec_pl[ 1 ];
         xvec_pl[ 1 ] = xvec_pl[ 0 ];
         yvec_pl[ 1 ] = yvec_pl[ 0 ];
   ...
   if (dx < (ncomp * 6)) { // Remap from distribution index to model stats index
      int cx = (dx / 6) * 6;
      int ax = dx - cx;
      dx = (ax == 0) ? (cx + 3) : dx;
      dx = (ax == 1) ? (cx + 4) : dx;
   ...
   legend->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() - 2));
