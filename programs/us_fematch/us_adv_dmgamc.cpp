...
US_AdvDmgaMc::US_AdvDmgaMc(US_Model* amodel, QVector<US_Model>& aimodels,
  ...
  plotLayout = new US_Plot(data_plot, tr("Attribute Distribution"),
                           tr("Attribute Name"), tr("Frequency"));
  ...
  QLabel* lb_modelsim =
      us_banner(tr("Simulate data using parameters from "
  ...
  connect(ct_modelnbr, SIGNAL(valueChanged(double)), this,
          SLOT(change_model(double)));
  ...
  le_modtype->setText(
      tr("Mean model, %1 components, %2 reaction(s)").arg(ncomp).arg(nreac));
...
void US_AdvDmgaMc::set_model_type(bool chekd) {
  ...
  if (!ck_cmod) umodel.description = lddesc;
  int ncomp = umodel.components.size();
  int nreac = umodel.associations.size();
  ...
  le_modtype->setText(
      mtlabl +
      tr(" model,  %1 components, %2 reaction(s)").arg(ncomp).arg(nreac));
...
void US_AdvDmgaMc::plot_distrib() {
  ...
  for (int ii = 0; ii < nxi;
       ii++) {  // Accumulate the input model X distribution values
    ...
    if (jr < 0) {  // Get a component attribute value
      ...
      xval = xtitle.contains(tr("Concentration")) ? sc->signal_concentration
                                                  : xval;
    }
  ...
  legend->setFont(
      QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() - 2));
...
void US_AdvDmgaMc::simulate() {
  ...
  if (!rb_curmod->isChecked()) umodel.description = lddesc;
  *model = umodel;
