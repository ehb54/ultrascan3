...
US_ResidPlotFem::US_ResidPlotFem(QWidget *parent, const QString auto_mode) : US_WidgetsDialog(0, 0) {
   ...
   QLayout *lo_plteda = us_checkbox(tr("Plot Experimental Data"), ck_plteda, true);
   QLayout *lo_subtin = us_checkbox(tr("Subtract Time Invariant Noise"), ck_subtin);
   QLayout *lo_subrin = us_checkbox(tr("Subtract Radially Invariant Noise"), ck_subrin);
   QLayout *lo_pltsda = us_checkbox(tr("Plot Simulated/Modeled Data"), ck_pltsda, true);
   QLayout *lo_addtin = us_checkbox(tr("Add Time Invariant Noise"), ck_addtin);
   QLayout *lo_addrin = us_checkbox(tr("Add Radially Invariant Noise"), ck_addrin);
   QLayout *lo_pltres = us_checkbox(tr("Plot Residuals"), ck_pltres, true);
   QLayout *lo_plttin = us_checkbox(tr("Plot Time Invariant Noise"), ck_plttin);
   QLayout *lo_pltrin = us_checkbox(tr("Plot Radially Invariant Noise"), ck_pltrin);
   QLayout *lo_pltran = us_checkbox(tr("Plot Random Noise"), ck_pltran);
   QLayout *lo_shorbm = us_checkbox(tr("Show Residuals Bitmap"), ck_shorbm);
   ...
   plotLayout1 = new US_Plot(data_plot1, tr("Experimental Data"), tr("Radius (cm)"), tr("Absorbance"), true, "^Curve");

   plotLayout2 = new US_Plot(data_plot2, tr("Residuals"), tr("Radius (cm)"), tr("OD Difference"));
   ...
   connect(ck_plteda, SIGNAL(toggled(bool)), this, SLOT(pedaCheck(bool)));
   connect(ck_subtin, SIGNAL(toggled(bool)), this, SLOT(stinCheck(bool)));
   connect(ck_subrin, SIGNAL(toggled(bool)), this, SLOT(srinCheck(bool)));
   connect(ck_pltsda, SIGNAL(toggled(bool)), this, SLOT(psdaCheck(bool)));
   connect(ck_addtin, SIGNAL(toggled(bool)), this, SLOT(atinCheck(bool)));
   connect(ck_addrin, SIGNAL(toggled(bool)), this, SLOT(arinCheck(bool)));
   connect(ck_pltres, SIGNAL(toggled(bool)), this, SLOT(presCheck(bool)));
   connect(ck_plttin, SIGNAL(toggled(bool)), this, SLOT(ptinCheck(bool)));
   connect(ck_pltrin, SIGNAL(toggled(bool)), this, SLOT(prinCheck(bool)));
   connect(ck_pltran, SIGNAL(toggled(bool)), this, SLOT(pranCheck(bool)));
   connect(ck_shorbm, SIGNAL(toggled(bool)), this, SLOT(srbmCheck(bool)));
...
void US_ResidPlotFem::set_plot(int plotf) {
   if (plotf == 1) { // ti_noise plot
      ck_plttin->setChecked(true);
   }

   else if (plotf == 2) { // ri_noise plot
      ck_pltrin->setChecked(true);
   }

   else if (plotf == 0) { // residuals plot
...
void US_ResidPlotFem::pedaCheck(bool chkd) {
   if (chkd) { // box is being checked:  sub boxes enabled if data present
      ck_subtin->setEnabled(have_ti);
      ck_subrin->setEnabled(have_ri);
   }

   else { // box is being unchecked:  sub boxes disabled
...
void US_ResidPlotFem::presCheck(bool chkd) {
   ...
   if (chkd) { // alternate residual plots unchecked
...
void US_ResidPlotFem::srbmCheck(bool chkd) {
   ...
   {
      if (chkd) { // bitmap checked:  replot to possibly build new map
         have_bm = (resbmap != 0);

         if (have_bm) { // if bitmap exists already, detect when closed
            connect(resbmap, SIGNAL(destroyed()), this, SLOT(resids_closed()));
         }
      ...
      if (a_mode == "ANALYSIS") {
         ...
         if (chkd) {
            ...
            qDebug() << "ck_shorbm->isChecked(), have_ed, have_sd -- " << ck_shorbm->isChecked() << have_ed << have_sd;
...
void US_ResidPlotFem::plot_edata() {
   ...
   if (!do_plteda && !do_pltsda) { // no real experimental plot specified:  re-do plot and return
      data_plot1->replot();
      return;
   }
   ...
   if (do_plteda) { // set title and values count for experimental data
      data_plot1->setAxisTitle(QwtPlot::yLeft, tr("Absorbance at ") + edata->wavelength + tr(" nm"));
      points = edata->pointCount();
   }

   //qDebug() << "PLOT edata 4 ";

   if (do_pltsda) { // set title and values count for simulation data
      if (have_ed)
         data_plot1->setAxisTitle(QwtPlot::yLeft, tr("Absorbance at ") + edata->wavelength + tr(" nm"));
      else
   ...
   if (do_plteda) { // plot experimental curves
      points = edata->pointCount();
      ...
      for (int jj = 0; jj < points; jj++) { // get radii (x) just once
         rr[ jj ] = edata->radius(jj);
      }

      for (int ii = 0; ii < count; ii++) { // get readings (y) for each scan
         ...
         for (int jj = 0; jj < points; jj++) { // each y is reading, optionally minus some noise
            if (do_subtin)
   ...
   if (do_pltsda) { // plot simulation curves
      ...
      for (int ii = 0; ii < count; ii++) { // get readings (y) for each scan
         ...
         for (int jj = 0; jj < points; jj++) { // each y is reading, optionally plus some noise
            if (do_addtin)
               tinoi = ti_noise->values[ jj ];
...
void US_ResidPlotFem::plot_rdata() {
   ...
   if (!do_pltres && !do_plttin && !do_pltrin && !do_pltran && !do_shorbm) { // if no plots specified, just clean up plot and return
      data_plot2->replot();
      return;
   }
   ...
   if (do_pltres) { // plot residuals
      ...
      for (int ii = 0; ii < count; ii++) { // get readings (y) for each scan
         ...
         for (int jj = 0; jj < points; jj++) { // each residual is e-value minus s-value
            tinoi = 0.0;
            if (do_subtin)
               tinoi = ti_noise->values[ jj ];
            if (do_addtin)
            ...
            rvalu = edata->value(ii, jj) - sdata->value(ii, jj) - rinoi - tinoi;
   ...
   else if (do_plttin) { // plot time-invariant noise
      data_plot2->setTitle(tr("Time-Invariant Noise"));

      for (int jj = 0; jj < points; jj++) { // accumulate radii and noise values
         rr[ jj ] = edata->radius(jj);
   ...
   else if (do_pltrin) { // plot radially-invariant noise
      data_plot2->setTitle(tr("Radially-Invariant Noise"));

      for (int ii = 0; ii < count; ii++) { // accumulate scan numbers and noise values
         rr[ ii ] = ( double ) (ii + 1);
   ...
   else if (do_pltran) { // plot random noise
      ...
      for (int ii = 0; ii < count; ii++) { // get random noise (y) for each scan
         if (excllist->contains(ii))
            continue;

         rinoi = have_ri ? ri_noise->values[ ii ] : 0.0;

         for (int jj = 0; jj < points; jj++) { // each random value is e-value minus s-value with optional noise
   ...
   if (do_shorbm) { // show residuals bitmap (if not already shown)
      ...
      for (int ii = 0; ii < count; ii++) { // build a vector for each scan
         if (excllist->contains(ii)) {
            resscan.fill(0.0);
            resids[ ii ] = resscan;
            continue;
         ...
         for (int jj = 0; jj < points; jj++) { // build residual values within a scan
            tinoi = 0.0;
            if (do_subtin)
               tinoi = ti_noise->values[ jj ];
            if (do_addtin)
      ...
      if (resbmap) { // already have resbmap:  just replot residuals bitmap
      ...
      else { // pop up a little dialog with residuals bitmap
...
void US_ResidPlotFem::connect_pboxes(bool conn) {
   if (conn) { // Connect slots for plot boxes
      connect(ck_pltres, SIGNAL(toggled(bool)), this, SLOT(presCheck(bool)));
      connect(ck_plttin, SIGNAL(toggled(bool)), this, SLOT(ptinCheck(bool)));
      connect(ck_pltrin, SIGNAL(toggled(bool)), this, SLOT(prinCheck(bool)));
      connect(ck_pltran, SIGNAL(toggled(bool)), this, SLOT(pranCheck(bool)));
   ...
   else { // Disconnect slots for plot boxes
