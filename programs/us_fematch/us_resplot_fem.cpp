...
US_ResidPlotFem::US_ResidPlotFem(QWidget* parent, const QString auto_mode)
  ...
  plotLayout1 =
      new US_Plot(data_plot1, tr("Experimental Data"), tr("Radius (cm)"),
                  tr("Absorbance"), true, "^Curve");

  plotLayout2 = new US_Plot(data_plot2, tr("Residuals"), tr("Radius (cm)"),
                            tr("OD Difference"));
  ...
  if (parent) {
    ...
    if (a_mode.isEmpty()) {
      ...
      if (a_mode == "REPORT") {
        qDebug() << "THIS will be pointer to autoflow_report (US_ReporterGMP "
                    "for now !!!) ---";
...
void US_ResidPlotFem::stinCheck(bool) { plot_data(); }

// subtract-ri-noise box [un]checked
void US_ResidPlotFem::srinCheck(bool) { plot_data(); }
...
void US_ResidPlotFem::atinCheck(bool) { plot_data(); }

// add-ri-noise box [un]checked
void US_ResidPlotFem::arinCheck(bool) { plot_data(); }
...
void US_ResidPlotFem::plot_edata() {
  ...
  if (!do_plteda && !do_pltsda) {  // no real experimental plot specified: re-do
                                   // plot and return
    data_plot1->replot();
    return;
  }
  ...
  if (a_mode.isEmpty()) {
    ...
    if (a_mode == "ANALYSIS") {
      ...
      if (do_plteda && !do_pltsda) {
        ...
        QString expsim_data =
            QString(tr("Experimental and Simulated Data ")) + tripleInfo;
  ...
  if (do_plteda) {  // set title and values count for experimental data
    data_plot1->setAxisTitle(
        QwtPlot::yLeft, tr("Absorbance at ") + edata->wavelength + tr(" nm"));
    points = edata->pointCount();
  }
  ...
  if (do_plteda) {  // plot experimental curves
    ...
    for (int ii = 0; ii < count; ii++) {  // get readings (y) for each scan
      ...
      if (do_subrin) rinoi = ri_noise->values[ii];

      for (int jj = 0; jj < points;
           jj++) {  // each y is reading, optionally minus some noise
        if (do_subtin) tinoi = ti_noise->values[jj];
  ...
  if (do_pltsda) {  // plot simulation curves
    ...
    for (int ii = 0; ii < count; ii++) {  // get readings (y) for each scan
      ...
      if (do_addrin) rinoi = ri_noise->values[ii];

      for (int jj = 0; jj < points;
           jj++) {  // each y is reading, optionally plus some noise
        if (do_addtin) tinoi = ti_noise->values[jj];
...
void US_ResidPlotFem::plot_rdata() {
  ...
  if (!do_pltres && !do_plttin && !do_pltrin && !do_pltran &&
      !do_shorbm) {  // if no plots specified, just clean up plot and return
    data_plot2->replot();
    return;
  }
  ...
  if (do_pltres) {  // plot residuals
    ...
    for (int ii = 0; ii < count; ii++) {  // get readings (y) for each scan
      ...
      for (int jj = 0; jj < points;
           jj++) {  // each residual is e-value minus s-value
        tinoi = 0.0;
        if (do_subtin) tinoi = ti_noise->values[jj];
        if (do_addtin) tinoi += ti_noise->values[jj];
    ...
    for (int ii = 0; ii < count;
         ii++) {  // accumulate scan numbers and noise values
      rr[ii] = (double)(ii + 1);
    ...
    for (int ii = 0; ii < count; ii++) {  // get random noise (y) for each scan
      ...
      for (int jj = 0; jj < points;
           jj++) {  // each random value is e-value minus s-value with optional
                    // noise
  ...
  if (do_shorbm) {  // show residuals bitmap (if not already shown)
    ...
    for (int ii = 0; ii < count; ii++) {  // build a vector for each scan
      ...
      for (int jj = 0; jj < points;
           jj++) {  // build residual values within a scan
        tinoi = 0.0;
        if (do_subtin) tinoi = ti_noise->values[jj];
        if (do_addtin) tinoi += ti_noise->values[jj];
