  ...
  if (parent)
    {
      ...
      if (a_mode.isEmpty ())
      ...
      else
        {
          if (a_mode == "ANALYSIS")
            {
              ...
              qDebug () << "RP:ti_noise count"
                        << (have_ti ? ti_noise->count : 0);
              qDebug () << "RP:ri_noise count"
                        << (have_ri ? ri_noise->count : 0);
          ...
          if (a_mode == "REPORT")
            {
              qDebug () << "THIS will be pointer to autoflow_report "
                           "(US_ReporterGMP for now !!!) ---";
...
void
US_ResidPlotFem::set_plot (int plotf)
{
  if (plotf == 1)
    { // ti_noise plot
...
void
US_ResidPlotFem::pedaCheck (bool chkd)
{
  if (chkd)
...
void
US_ResidPlotFem::stinCheck (bool)
{
  plot_data ();
}
...
void
US_ResidPlotFem::srinCheck (bool)
{
  plot_data ();
}
...
void
US_ResidPlotFem::psdaCheck (bool chkd)
{
  if (chkd)
    { // box is being checked:  sub boxes enabled if data present
      ck_addtin->setEnabled (have_ti);
...
void
US_ResidPlotFem::atinCheck (bool)
{
  plot_data ();
}
...
void
US_ResidPlotFem::arinCheck (bool)
{
  plot_data ();
}
...
void
US_ResidPlotFem::presCheck (bool chkd)
{
  skip_plot = true;
...
void
US_ResidPlotFem::ptinCheck (bool chkd)
{
  skip_plot = true;
...
void
US_ResidPlotFem::prinCheck (bool chkd)
{
  skip_plot = true;
...
void
US_ResidPlotFem::pranCheck (bool chkd)
{
  skip_plot = true;
...
void
US_ResidPlotFem::srbmCheck (bool chkd)
{
        ...
        { // bitmap checked:  replot to possibly build new map
          ...
          if (have_bm)
            { // if bitmap exists already, detect when closed
              connect (resbmap, SIGNAL (destroyed ()), this,
                       SLOT (resids_closed ()));
            }
...
US_ResidPlotFem::plot_edata ()
{
      ...
      if (a_mode == "ANALYSIS")
        {
          ...
          if (do_plteda && !do_pltsda)
            {
              QString exp_data
                  = QString (tr ("Experimental Data ")) + tripleInfo;
              data_plot1->setTitle (exp_data);
            }
          else if (do_plteda && do_pltsda)
            {
              QString expsim_data
                  = QString (tr ("Experimental and Simulated Data "))
                    + tripleInfo;
  ...
  if (do_plteda)
    { // set title and values count for experimental data
      data_plot1->setAxisTitle (QwtPlot::yLeft, tr ("Absorbance at ")
                                                    + edata->wavelength
                                                    + tr (" nm"));
      points = edata->pointCount ();
    }
...
US_ResidPlotFem::plot_rdata ()
{
  ...
  if (!do_pltres && !do_plttin && !do_pltrin && !do_pltran && !do_shorbm)
        ...
        { // get readings (y) for each scan
          ...
          for (int jj = 0; jj < points; jj++)
            { // each residual is e-value minus s-value
              ...
              rvalu = edata->value (ii, jj) - sdata->value (ii, jj) - rinoi
                      - tinoi;
      ...
      if (a_mode == "ANALYSIS")
        {
          ...
          ck_shorbm->setChecked (false);
          connect (ck_shorbm, SIGNAL (toggled (bool)), this,
                   SLOT (srbmCheck (bool)));
        }
    }
}
...
void
US_ResidPlotFem::connect_pboxes (bool conn)
{
  if (conn)
    { // Connect slots for plot boxes
      connect (ck_pltres, SIGNAL (toggled (bool)), this,
               SLOT (presCheck (bool)));
      connect (ck_plttin, SIGNAL (toggled (bool)), this,
               SLOT (ptinCheck (bool)));
      connect (ck_pltrin, SIGNAL (toggled (bool)), this,
               SLOT (prinCheck (bool)));
      connect (ck_pltran, SIGNAL (toggled (bool)), this,
               SLOT (pranCheck (bool)));
    }
...
void
US_ResidPlotFem::closeEvent (QCloseEvent *event)
{
