...
void US_DmgaMcStats::build_rmsd_stats(int niters, QVector<US_Model> &imodels, QVector<double> &rstats) {
   const int stsiz = 23;
   ...
   qDebug() << "Dst:BRs:   iters" << niters << "RMSD min max mean median" << rstats[ 0 ] << rstats[ 1 ] << rstats[ 2 ]
            << rstats[ 3 ];
}

// Build Model attribute statistics from iteration models
int US_DmgaMcStats::build_model_stats(int niters, QVector<US_Model> &imodels, QVector<QVector<double>> &astats) {
   ...
   int kt = 0;
   qDebug() << "DmS:BMs:  ncomp" << ncomp << "nasso" << nasso << "ntatts" << ntatts << "niters" << niters;
   ...
   for (int ii = 0; ii < ncomp; ii++) {
      ...
      for (int jj = 0; jj < niters; jj++) {
         ...
         concs[ jj ] = sc->signal_concentration > 0.0 ? sc->signal_concentration : s0->signal_concentration;
         vbars[ jj ] = sc->vbar20;
         mwts[ jj ] = sc->mw;
         scos[ jj ] = sc->s;
         dcos[ jj ] = sc->D;
      ...
      qDebug() << "DmS:BMs:   ii" << ii << "mean c v w s d k" << astats[ kt - 6 ][ 2 ] << astats[ kt - 5 ][ 2 ]
               << astats[ kt - 4 ][ 2 ] << astats[ kt - 3 ][ 2 ] << astats[ kt - 2 ][ 2 ] << astats[ kt - 1 ][ 2 ];
   }
   ...
   for (int ii = 0; ii < nasso; ii++) {
      ...
      if (nrcs == 2) { // Single reactant and a product
         ...
         compute_statistics(niters, kds, concs, astats[ kt++ ]);
         compute_statistics(niters, koffs, concs, astats[ kt++ ]);
         qDebug() << "DmS:BMs:   ii" << ii << "mean kd ko" << astats[ kt - 2 ][ 2 ] << astats[ kt - 1 ][ 2 ];
      }
   }
...
bool US_DmgaMcStats::compute_statistics(
   const int nvals, QVector<double> &vals, QVector<double> &concs, QVector<double> &stats) {
   bool is_fixed = false;
   const int stsiz = 23;
   int nbins = 50;
   ...
   if (is_fixed) { // Values are the same, special statistics for fixed attribute
      vmedi = vmean;
...
// Build Model attribute statistics from iteration models
int US_DmgaMcStats::build_used_model(const QString smtype, const int iter, QVector<US_Model> &imodels, US_Model &umodel) {
   ...
   if (stx < 0) { // Used model is one of the iterations
      ...
      for (int ii = 0; ii < ncomp; ii++) { // Pick up mean|median|mode of any floating attributes
         ...
         //*DEBUG*
         qDebug() << "DmS:UajMd: Cii=" << ii << "c v w s d k" << umodel.components[ ii ].signal_concentration
                  << umodel.components[ ii ].vbar20 << umodel.components[ ii ].mw << umodel.components[ ii ].s
                  << umodel.components[ ii ].D << umodel.components[ ii ].f_f0 << "fixs,d,k" << fixs << fixd << fixk;
         umodel.calc_coefficients(umodel.components[ ii ]);
         qDebug() << "DmS:AdjMd:  Cii=" << ii << "c v w s d k" << umodel.components[ ii ].signal_concentration
                  << umodel.components[ ii ].vbar20 << umodel.components[ ii ].mw << umodel.components[ ii ].s
         ...
         umodel.components[ ii ].f = 0.0;
         umodel.calc_coefficients(umodel.components[ ii ]);
         qDebug() << "DmS:AdjMd:   fixS s d k" << umodel.components[ ii ].s << umodel.components[ ii ].D
                  << umodel.components[ ii ].f_f0;
         umodel.components[ ii ].s = 0.0;
         umodel.components[ ii ].f_f0 = 0.0;
         umodel.components[ ii ].f = 0.0;
         umodel.calc_coefficients(umodel.components[ ii ]);
         qDebug() << "DmS:AdjMd:   fixD s d k" << umodel.components[ ii ].s << umodel.components[ ii ].D
                  << umodel.components[ ii ].f_f0;
         umodel.components[ ii ].s = 0.0;
         umodel.components[ ii ].D = 0.0;
         umodel.components[ ii ].f = 0.0;
         umodel.calc_coefficients(umodel.components[ ii ]);
         qDebug() << "DmS:AdjMd:   fixK s d k" << umodel.components[ ii ].s << umodel.components[ ii ].D
                  << umodel.components[ ii ].f_f0;
         umodel.components[ ii ].mw = 0.0;
         umodel.components[ ii ].D = 0.0;
         umodel.components[ ii ].f = 0.0;
         umodel.calc_coefficients(umodel.components[ ii ]);
         qDebug() << "DmS:AdjMd:   fixD w d k" << umodel.components[ ii ].mw << umodel.components[ ii ].D
                  << umodel.components[ ii ].f_f0;
         umodel.components[ ii ].s = 0.0;
         umodel.components[ ii ].mw = 0.0;
         umodel.components[ ii ].f = 0.0;
         umodel.calc_coefficients(umodel.components[ ii ]);
         qDebug() << "DmS:AdjMd:   fixK s d w" << umodel.components[ ii ].s << umodel.components[ ii ].D
      ...
      for (int ii = 0; ii < nasso; ii++) {
         ...
         qDebug() << "DmS:AdjMd: Aii=" << ii << "d off" << umodel.associations[ ii ].k_d
                  << umodel.associations[ ii ].k_off;
