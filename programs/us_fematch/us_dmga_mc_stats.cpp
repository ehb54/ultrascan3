...
int
US_DmgaMcStats::build_imodels (US_Model &model, QVector<US_Model> &imodels)
{
  imodels.clear ();
  QStringList xmls;
  int niters = 0;
...
void
US_DmgaMcStats::build_rmsd_stats (int niters, QVector<US_Model> &imodels,
...
int
US_DmgaMcStats::build_model_stats (int niters, QVector<US_Model> &imodels,
...
bool
US_DmgaMcStats::compute_statistics (const int nvals, QVector<double> &vals,
                                    QVector<double> &concs,
                                    QVector<double> &stats)
{
  ...
  if (is_fixed)
    ...
    { // Values are not the same, so statistics need to be computed
      ...
      qDebug () << "DmS:cSt:     FLT  hx vmedi vhx" << hx << vmedi
                << wkvls[hx];
...
// Build Model attribute statistics from iteration models
int
US_DmgaMcStats::build_used_model (const QString smtype, const int iter,
