...
US_FeMatch::US_FeMatch() : US_Widgets() {
  ...
  QLabel* lb_experiment =
      us_banner(tr("Experimental Parameters (at 20") + DEGC + "):");
  ...
  connect(ct_model, SIGNAL(valueChanged(double)), this,
          SLOT(update_mc_model()));
...
void US_FeMatch::load(void) {
  ...
  int local =
      dkdb_cntrls->db() ? US_Disk_DB_Controls::DB : US_Disk_DB_Controls::Disk;
  // US_DataLoader* dialog =
  //   new US_DataLoader( dataLatest, local, rawList, dataList,
  //          triples, workingDir, QString( "velocity" ) );

  US_DataLoader* dialog =
      new US_DataLoader(dataLatest, local, rawList, dataList, triples,
                        workingDir, QString("none"));

  connect(dialog, SIGNAL(changed(bool)), this, SLOT(update_disk_db(bool)));
  connect(dialog, SIGNAL(progress(const QString)), this,
  ...
  runID = edata->runID;
  QString tmst_fpath =
      US_Settings::resultDir() + "/" + runID + "/" + runID + ".time_state.tmst";

  qDebug() << "RUNID --- " << runID;
  // Get speed steps from disk or DB experiment (and maybe timestate)

  if (local == US_Disk_DB_Controls::DB) {  // Fetch the speed steps for the
                                           // experiment from the database
    ...
    DbgLv(0) << "LD: newfile" << newfile << "idExp" << idExp << "tmst_fpath"
             << tmst_fpath;
  }
    ...
    QString expfpath =
        workingDir + "/" + runID + "." + edata->dataType + ".xml";
    QFile xfi(expfpath);

    if (xfi.open(QIODevice::ReadOnly)) {  // Read and parse "<speedstep>" lines
                                          // in the XML
  ...
  if (check_file.exists() &&
    ...
    // ALEXEY: #speed steps from TimeStamp can be bigger than what's read from
    // DB??
    for (int jj = 0; jj < simparams.speed_step.count(); jj++) {
      ...
      DbgLv(1) << "LD:    (s) tf tl wf wl scns" << speed_steps[jj].time_first
  ...
  if (nssp > 0) {
    for (int ii = 0; ii < nssp; ii++) {
      ...
      for (int ds = 0; ds < ntriples;
           ds++) {  // Scan data time ranges and compare to experiment speed
                    // steps
        edata = &dataList[ds];
        ...
        if (etm1 < stm1 || etm2 > stm2) {  // Data times beyond speed step
                                           // ranges, so flag use of data ranges
          nssp = 0;
          qDebug() << "Data is beyond range ex:" << etm1 << etm2
                   << "ss:" << stm1 << stm2;
          // speed_steps.clear();
          break;
        }
  ...
  exp_steps = (nssp > 0);
  dat_steps = !exp_steps;
  DbgLv(1) << "LD: after if block, nssp=" << nssp << "   exp_steps"
           << exp_steps;
  ...
  for (int ii = 0; ii < ntriples; ii++) lw_triples->addItem(triples.at(ii));
...
void US_FeMatch::data_plot(void) {
  ...
  data_plot2->setTitle(tr("Velocity Data for ") + triple + " of\n" +
                       edata->runID);
  data_plot2->setAxisTitle(
      QwtPlot::yLeft, dataType + tr(" at ") + edata->wavelength + tr(" nm"));
  ...
  for (int jj = point - 5; jj < point + 6; jj++) baseline += dscan->rvalues[jj];
  ...
  if (haveSim) {
    ...
    for (int ii = 0; ii < scanCount; ii++) {
      ...
      for (int jj = 0; jj < points;
           jj++) {  // accumulate coordinates of simulation curve
    ...
    for (int ii = 0; ii < scanCount; ii++) {
      if (excludedScans.contains(ii)) continue;
      for (int jj = 0; jj < points; jj++) vsum += sq(sdata->value(ii, jj));
    }
    DbgLv(1) << "VSUM=" << vsum;
    ...
    DbgLv(1) << "SDAT *-n" << sdata->value(ss - 4, hh)
             << sdata->value(ss - 3, hh) << sdata->value(ss - 2, hh)
             << sdata->value(ss - 1, hh);
    if (sdata->value(ss - 1, hh) == 0.0) {
      for (int js = 0; js < scanCount; js++)
        DbgLv(1) << "SDAT     js" << js << "sd" << sdata->value(js, hh) << "ed"
                 << edata->value(js, hh);
    }
    //*DEBUG
...
void US_FeMatch::save_data(void) {
  if (eplotcd == 0 ||
      eplotcd->data_3dplot() == 0) {  // if no 3d plot control up,  ask what
                                      // user wants for 3d plot save
    if (eplotcd == 0) eplotcd = new US_PlotControlFem(this, &model);
    ...
    mBox.setText(
        tr("You selected Save Data without a 3D Plot already displayed.<br>"
  ...
  QString dsinfFile = basename.section("/", 0, -2) + "/dsinfo." + tripnode +
                      ".dataset_info.html";

  // Write the main report file
  QFile rep_f(htmlFile);
  if (!rep_f.open(QIODevice::WriteOnly | QIODevice::Text)) return;
  ...
  QString umsg = tr("In directory ") +
                 basename.left(basename.lastIndexOf("/")) + tr(" ,\nwrote:\n");
...
void US_FeMatch::exclude(void) {
  ...
  for (int ii = 0; ii < totalScans; ii++) {
    ...
    if (displayedScan >= from && displayedScan <= to) excludedScans << ii;
...
void US_FeMatch::distrib_type() {
  const char* dptyp[] = {"s20,w distribution", "MW distribution",
                         "D20,w distribution",
                         // "f_f0 vs s20,w",
                         // "f_f0 vs MW",
                         // "vbar vs s20,w",
...
void US_FeMatch::distrib_plot_resids() {
  ...
  for (int jj = 0; jj < dsize;
       jj++) {  // accumulate x (radius) values and min,max
  ...
  for (int ii = 0; ii < scanCount;
       ii++) {  // accumulate min,max y (residual) values
    for (int jj = 0; jj < dsize; jj++) {
      yval = resids[ii][jj];
      ymin = min(yval, ymin);
      ymax = max(yval, ymax);
  ...
  for (int ii = 0; ii < scanCount;
       ii++) {  // draw residual dots a scan at a time
    ...
    if (nmcols > 0) pen_plot = QPen(mcolors[ii % nmcols]);
...
void US_FeMatch::load_model() {
  ...
  if (dialog.exec() != QDialog::Accepted) return;  // Cancel:  bail out now
  ...
  if (is_dmga_mc) {  // For DMGA-MC loaded model, build the vector of iteration
                     // models
...
void US_FeMatch::adjust_model() {
  ...
  DbgLv(1) << "Fem:Adj:  avgT" << avgTemp << "scorr dcorr" << scorrec
           << dcorrec;
  ...
  if (cnstvb && mc_vbar != sd.vbar20 &&
    ...
    DbgLv(1) << "Fem:Adj:   cnstvb" << cnstvb << "  scorr dcorr" << scorrec
             << dcorrec;
  }
  ...
  for (int jj = 0; jj < model.components.size(); jj++) {
    ...
    sc->s /= scorrec;
    sc->D /= dcorrec;
    DbgLv(1) << "Fem:Adj:  s20w D20w" << s20w << d20w << "s D" << sc->s << sc->D
...
void US_FeMatch::load_noise() {
  ...
  qDebug() << "load_noise(): mieGUIDs, nieGUIDs, editGUID, modelGUID -- "
           << mieGUIDs << nieGUIDs << editGUID << modelGUID;
  qDebug() << "load_noise(): !loadDB, noisdf, nenois -- " << !loadDB << noisdf
           << nenois;
      ...
      if (kenois == 2) {  // Second noise from same model:  g/et it, too
        ...
        if (typen == "ti") {
          ...
          qDebug() << "Noise 2: noiID, loaded ri--" << noiID;
             ...
             noisdf > 0) {  // more than 1:  get choice from noise loader dialog
      US_NoiseLoader* nldiag = new US_NoiseLoader(dbP, mieGUIDs, nieGUIDs,
                                                  ti_noise, ri_noise, edata);
...
void US_FeMatch::simulate_model() {
  ...
  DbgLv(1) << "SimMdl: simpoints" << simparams.simpoints << "rreso"
  ...
  if ((check_file.exists()) && (check_file.isFile())) {
    if (US_AstfemMath::timestate_onesec(
      ...
      simparams.simSpeedsFromTimeState(tmst_fpath);
      DbgLv(1) << "SimMdl: timestate file exists" << tmst_fpath
    ...
    if (!simparams.sim) {  // Compute acceleration rate for non-astfem_sim data
      ...
      DbgLv(1) << "SimMdl:  accel-calc:  t1 t2 w2t t_acc speed rate" << t1 << t2
  ...
  DbgLv(1) << "SimMdl: ssck: rspeed accel1 lo_ss_acc" << rspeed << accel1
  ...
  DbgLv(1) << "SimMdl: nthread" << nthread << "ncomp" << ncomp << "ntc" << ntc
  ...
  if (nthread < 2) {
    if (model.components[0].sigma == 0.0 && model.components[0].delta == 0.0 &&
      ...
      for (int ii = 0; ii < model.components.size(); ii++) {
        DbgLv(1) << "SimMdl:   comp" << ii << "s D v" << model.components[ii].s
      ...
      // connect( astfvm,  SIGNAL( comp_progress( int ) ), this,  SLOT(
      // update_progress(   int ) ) ); solution_rec.buffer.compressibility =
      // compress; solution_rec.buffer.manual          = manual;
...
QStringList US_FeMatch::last_edit_files(QStringList files) {
  ...
  for (int ii = 1; ii < nfi; ii++) {
    ...
    if (QString::compare(test, ptest) !=
        0) {  // differs by more than just edit, so output previous
      ofiles.append(pfile);
    }
  }
...
void US_FeMatch::comp_number(double cnbr) { component_values((int)cnbr - 1); }
...
void US_FeMatch::write_dset_report(QString& dsfname) {
  ...
  if (!f_rep.open(QIODevice::WriteOnly | QIODevice::Truncate)) return;
...
double US_FeMatch::calc_baseline(int drow) const {
  ...
  for (int jj = nn - 5; jj < nn + 6; jj++) bl += ss->rvalues[jj];
...
void US_FeMatch::calc_residuals() {
  ...
  rmsd = sqrt(rmsd);
  le_rmsd->setText(QString::number(rmsd));
  DbgLv(1) << "CALC_RESID: matchd" << matchd << "kexcls" << kexcls << "rmsd"
           << rmsd;
}

// Slot to make sure all windows and dialogs get closed
void US_FeMatch::close_all() {
  if (rbmapd) rbmapd->close();

  if (eplotcd) eplotcd->close();

  if (resplotd) resplotd->close();
...
QString US_FeMatch::run_details(void) {
  ...
  s += table_row(tr("Time Correction:"),
                 QString::number(minutes) + m + QString::number(seconds) + sec);
  ...
  s += table_row(tr("Run Duration:"), QString::number(hours) + h +
...
QString US_FeMatch::scan_info(void) const {
  ...
  s += table_row(tr("Scan"), tr("Corrected Time"), tr("Plateau Concentration"),
...
QString US_FeMatch::distrib_info() {
  ...
  if (cnstvb) {  // Normal constant-vbar distribution
    mstr += table_row(tr("Molec. Wt."), tr("S Apparent"), tr("S 20,W"),
                      tr("D Apparent"), tr("D 20,W"), tr("f / f0"),
                      tr("Concentration"));
    ...
    for (int ii = 0; ii < ncomp; ii++) {
      ...
      mstr +=
          table_row(QString().sprintf("%10.4e", model_used.components[ii].mw),
    ...
    mstr += table_row(tr("Molec. Wt."), tr("S Apparent"), tr("S 20,W"),
                      tr("D Apparent"), tr("D 20,W"), tr("Vbar20"),
                      tr("Concentration"));
  ...
  if (is_dmga_mc) {  // Compute DMGA-MC statistics and add them to the report
    ...
    mstr += indent(4) + "<table>\n";
    mstr += table_row(tr("Component"), tr("Attribute"), tr("Mean_Value"),
                      tr("95%_Confidence(low)"), tr("95%_Confidence(high)"));
    int kd = 0;
    QString fixd = tr("(Fixed)");
    QString blnk("");
    QStringList atitl;
    QStringList rtitl;
    atitl << tr("Concentration") << tr("Vbar20") << tr("Molecular Weight")
          << tr("Sedimentation Coefficient") << tr("Diffusion Coefficient")
          << tr("Frictional Ratio");
    ...
    for (int ii = 0; ii < ncomp; ii++) {
      ...
      for (int jj = 0; jj < 6; jj++) {
        ...
        QString strclo =
            is_fixed ? fixd : QString().sprintf("%10.4e", mstats[kd][9]);
        QString strchi =
            is_fixed ? blnk : QString().sprintf("%10.4e", mstats[kd][10]);
        mstr += table_row(compnum, atitl[jj],
                          QString().sprintf("%10.4e", mstats[kd][2]), strclo,
                          strchi);
        kd++;
      }
    }
    ...
    mstr += indent(4) + "</table>\n";
    mstr += indent(4) + "<table>\n";
    mstr += table_row(tr("Reaction"), tr("Attribute"), tr("Mean_Value"),
                      tr("95%_Confidence(low)"), tr("95%_Confidence(high)"));
    ...
    for (int ii = 0; ii < nreac; ii++) {
      ...
      QString strchi =
          is_fixed ? blnk : QString().sprintf("%10.4e", mstats[kd][10]);
      mstr +=
          table_row(reacnum, tr("K_dissociation"),
                    QString().sprintf("%10.4e", mstats[kd][2]), strclo, strchi);
      kd++;
      ...
      mstr +=
          table_row(reacnum, tr("K_off Rate"),
                    QString().sprintf("%10.4e", mstats[kd][2]), strclo, strchi);
      kd++;
    }
    ...
    mstr +=
        table_row(tr("Lower Mode:"), QString().sprintf("%10.4e", rstats[6]));
    mstr +=
        table_row(tr("Upper Mode:"), QString().sprintf("%10.4e", rstats[7]));
    mstr +=
        table_row(tr("Mode Center:"), QString().sprintf("%10.4e", rstats[8]));
    ...
    for (kd = 0; kd < ntatt; kd++) {
      ...
      if (is_fixed) {  // Fixed has limited lines
        ...
        mstr +=
            table_row(tr("Mean:"), QString().sprintf("%10.4e", mstats[kd][2]));
        ...
        mstr += table_row(tr("Median:"),
                          QString().sprintf("%10.4e", mstats[kd][3]));
        mstr +=
            table_row(tr("Skew:"), QString().sprintf("%10.4e", mstats[kd][4]));
...
#else
    QPixmap pixmap =
        QPixmap::grabWidget(rbmapd, 0, 0, rbmapd->width(), rbmapd->height());
#endif
...
void US_FeMatch::get_solution() {
  ...
  US_SolutionGui* soluInfo =
      new US_SolutionGui(expID, 1, true, dbdisk, solution_rec, false);

  connect(soluInfo, SIGNAL(updateSolutionGuiSelection(US_Solution)), this,
          SLOT(updateSolution(US_Solution)));
...
void US_FeMatch::updateSolution(US_Solution solution_sel) {
  ...
  if (dkdb_cntrls->db()) {
    US_Passwd pw;
    US_DB2 db(pw.getPasswd());
    US_SolutionVals::bufvals_db(&db, sbufID, bufGUID, bufDesc, bdens, bvisc,
                                bcmpr, bmanu, errmsg);
  }

  else {
    US_SolutionVals::bufvals_disk(sbufID, bufGUID, bufDesc, bdens, bvisc, bcmpr,
                                  bmanu, errmsg);
  }
...
void US_FeMatch::model_table(QString mdtFile) {
  ...
  if (ncomp == 0) return;
  QFile mdt_f(mdtFile);
  if (!mdt_f.open(QIODevice::WriteOnly | QIODevice::Text)) return;
  double sum_c = 0.0;
  ...
  ts << dquote + "Molec.Weight" + dquote + comma + dquote + "S_Apparent" +
            dquote + comma + dquote + "S_20_W" + dquote + comma + dquote +
            "D_Apparent" + dquote + comma + dquote + "D_20_W" + dquote + comma +
            dquote + "f/f0" + dquote + comma + dquote + "Vbar20" + dquote +
            comma + dquote + "Concentration" + dquote + comma + dquote +
            "Conc.Percent" + dquote + endln;

  for (int ii = 0; ii < ncomp; ii++) {  // Write each component line
    double conc = model_used.components[ii].signal_concentration;
    double perc = 100.0 * conc / sum_c;
    ts << dquote + QString().sprintf("%10.4e", model_used.components[ii].mw) +
              dquote + comma + dquote +
              QString().sprintf("%10.4e", model.components[ii].s) + dquote +
              comma + dquote +
              QString().sprintf("%10.4e", model_used.components[ii].s) +
              dquote + comma + dquote +
              QString().sprintf("%10.4e", model.components[ii].D) + dquote +
              comma + dquote +
              QString().sprintf("%10.4e", model_used.components[ii].D) +
              dquote + comma + dquote +
              QString().sprintf("%10.4e", model_used.components[ii].f_f0) +
              dquote + comma + dquote +
              QString().sprintf("%10.4e", model_used.components[ii].vbar20) +
              dquote + comma + dquote + QString().sprintf("%10.4e", conc) +
              dquote + comma + dquote + QString().sprintf("%5.2f %%", perc) +
              dquote + endln;
  }
  ...
  // Write the header line
  ts << dquote + "Reaction" + dquote + comma + dquote + "K_dissociation" +
            dquote + comma + dquote + "k_off_Rate" + dquote + endln;
  ...
  for (int ii = 0; ii < nreac; ii++) {  // Write each reaction line
    ...
    ts << dquote + QString().sprintf("%4d", ii) + dquote + comma + dquote +
              QString().sprintf("%10.4e", k_d) + dquote + comma + dquote +
              QString().sprintf("%10.4e", k_off) + dquote + endln;
  }
}
...
void US_FeMatch::update_mc_model() {
  if (rb_curmod->isChecked()) {  // The model is the currently selected MC
                                 // iteration model
  }

  else if (rb_mean->isChecked()) {  // The model is the mean of all iteration
                                    // models
  }

  else if (rb_median->isChecked()) {  // The model is the median of all
                                      // iteration models
  }

  else if (rb_mode->isChecked()) {  // The model is the mode of all iteration
                                    // models
  }
}

// Public slot that calls the private one to simulate a model
void US_FeMatch::simulate() { simulate_model(); }

// Public slot to mark residuals plot dialog closed
void US_FeMatch::resplot_done() { resplotd = 0; }
...
bool US_FeMatch::has_intensity_profile(const QString& runID, const bool in_db) {
  ...
  if (in_db) {  // Data from DB:  get any RIProfile from experiment record
    ...
    QString filename =
        US_Settings::resultDir() + "/" + runID + "/" + runID + ".RIProfile.xml";
...
void US_FeMatch::auto_load_simulate(US_DataIO::RawData i_rdata,
                                    ...
                                    US_Model i_model, int idExp, QPoint pos) {
  reset();
  QString file;
  QStringList files;
  QStringList parts;
  ...
  if (is_dmga_mc) {  // For DMGA-MC loaded model, build the vector of iteration
    ...
    if (US_Settings::advanced_level() >
        0) {  // For non dmga and users with advanced settings > 0, show the
              // advanced dialog
      model_used = model_loaded;
