...
US_FeMatch::US_FeMatch() : US_Widgets() {
   ...
   setWindowTitle(tr("Compare Experimental Data to Sums of Finite Element Solutions"));
   ...
   QLayout *lo_edit = us_checkbox(tr("Latest Data Edit"), ck_edit, true);
   ...
   connect(pb_distrib, SIGNAL(clicked()), this, SLOT(distrib_type()));
   connect(pb_loadmodel, SIGNAL(clicked()), this, SLOT(load_model()));
   connect(pb_simumodel, SIGNAL(clicked()), this, SLOT(simulate_model()));
   connect(pb_view, SIGNAL(clicked()), this, SLOT(view_report()));
   ...
   runInfoLayout->addWidget(te_desc, row, 0, 2, 4);
   row += 2;
   ...
   QLabel *lb_experiment = us_banner(tr("Experimental Parameters (at 20") + DEGC + "):");
   QLabel *lb_variance = us_label(tr("Variance:"));

   connect(pb_solution, SIGNAL(clicked()), this, SLOT(get_solution()));
   ...
   connect(pb_advanced, SIGNAL(clicked()), this, SLOT(advanced()));
   connect(pb_adv_dmga, SIGNAL(clicked()), this, SLOT(adv_dmga()));
   connect(pb_plot3d, SIGNAL(clicked()), this, SLOT(plot3d()));
   connect(pb_plotres, SIGNAL(clicked()), this, SLOT(plotres()));
   ...
   connect(ct_from, SIGNAL(valueChanged(double)), this, SLOT(exclude_from(double)));
   connect(ct_to, SIGNAL(valueChanged(double)), this, SLOT(exclude_to(double)));
   connect(pb_exclude, SIGNAL(clicked()), this, SLOT(exclude()));
   connect(pb_reset_exclude, SIGNAL(clicked()), this, SLOT(reset_excludes()));
   ...
   connect(pb_close, SIGNAL(clicked()), this, SLOT(close_all()));
   ...
   connect(rb_curmod, SIGNAL(toggled(bool)), this, SLOT(curmod_clicked(bool)));
   connect(rb_mean, SIGNAL(toggled(bool)), this, SLOT(modbtn_clicked(bool)));
   connect(rb_median, SIGNAL(toggled(bool)), this, SLOT(modbtn_clicked(bool)));
   connect(rb_mode, SIGNAL(toggled(bool)), this, SLOT(modbtn_clicked(bool)));
   connect(pb_nextm, SIGNAL(clicked()), this, SLOT(next_model()));
   connect(ct_model, SIGNAL(valueChanged(double)), this, SLOT(update_mc_model()));
...
void US_FeMatch::load(void) {
   ...
   int local = dkdb_cntrls->db() ? US_Disk_DB_Controls::DB : US_Disk_DB_Controls::Disk;
   // US_DataLoader* dialog =
   //   new US_DataLoader( dataLatest, local, rawList, dataList,
   //          triples, workingDir, QString( "velocity" ) );

   US_DataLoader *dialog = new US_DataLoader(dataLatest, local, rawList, dataList, triples, workingDir, QString("none"));

   connect(dialog, SIGNAL(changed(bool)), this, SLOT(update_disk_db(bool)));
   connect(dialog, SIGNAL(progress(const QString)), this, SLOT(set_progress(const QString)));
   ...
   if (local == US_Disk_DB_Controls::DB) { // Fetch the speed steps for the experiment from the database
      ...
      QString expID;
      int idExp = 0;
      query << "get_experiment_info_by_runID" << runID << QString::number(US_Settings::us_inv_ID());
      dbP->query(query);
      ...
      DbgLv(0) << "LD: newfile" << newfile << "idExp" << idExp << "tmst_fpath" << tmst_fpath;
   }

   else { // Read run experiment file and parse out speed steps
      workingDir = workingDir.section(workingDir.left(1), 4, 4);
      workingDir = workingDir.left(workingDir.lastIndexOf("/"));
      QString expfpath = workingDir + "/" + runID + "." + edata->dataType + ".xml";
      QFile xfi(expfpath);

      if (xfi.open(QIODevice::ReadOnly)) { // Read and parse "<speedstep>" lines in the XML
   ...
   if (check_file.exists() && check_file.isFile()) { // Get speed_steps from an existing timestate file
      ...
      for (int jj = 0; jj < qMin(essknt, tssknt); jj++) {
         DbgLv(1) << "LD:  jj" << jj << "(e) tf tl wf wl scns" << speed_steps[ jj ].time_first
                  << speed_steps[ jj ].time_last << speed_steps[ jj ].w2t_first << speed_steps[ jj ].w2t_last
                  << speed_steps[ jj ].scans;
         DbgLv(1) << "LD:    (t) tf tl wf wl scns" << simparams.speed_step[ jj ].time_first
      ...
      for (int jj = 0; jj < simparams.speed_step.count(); jj++) {
         ...
         DbgLv(1) << "LD:    (s) tf tl wf wl scns" << speed_steps[ jj ].time_first << speed_steps[ jj ].time_last
   ...
   if (nssp > 0) {
      for (int ii = 0; ii < nssp; ii++) {
         ...
         for (int ds = 0; ds < ntriples; ds++) { // Scan data time ranges and compare to experiment speed steps
            edata = &dataList[ ds ];
            int lesc = edata->scanCount() - 1;
            int etm1 = edata->scanData[ 0 ].seconds;
            int etm2 = edata->scanData[ lesc ].seconds;
            DbgLv(1) << "LD:  ds" << ds << "step" << ii << "stimes" << stm1 << stm2 << "etimes" << etm1 << etm2
                     << "lesc" << lesc;

            if (etm1 < stm1 || etm2 > stm2) { // Data times beyond speed step ranges, so flag use of data ranges
               nssp = 0;
   ...
   connect(lw_triples, SIGNAL(currentRowChanged(int)), SLOT(new_triple(int)));
...
void US_FeMatch::details(void) {
   US_RunDetails2 *dialog = new US_RunDetails2(rawList, runID, workingDir, triples);
...
void US_FeMatch::data_plot(void) {
   ...
   if (edata->dataType == "RI" && !has_intensity_profile(edata->runID, in_db))
      dataType = tr("Intensity");
   if (edata->dataType == "WI")
      dataType = tr("Intensity");
   if (edata->dataType == "IP")
      dataType = tr("Interference");
   if (edata->dataType == "FI")
      dataType = tr("Fluourescence");
   data_plot2->setTitle(tr("Velocity Data for ") + triple + " of\n" + edata->runID);
   data_plot2->setAxisTitle(QwtPlot::yLeft, dataType + tr(" at ") + edata->wavelength + tr(" nm"));
   ...
   int point = US_DataIO::index(dataList[ drow ].xvalues, dataList[ drow ].baseline);
   ...
   for (int ii = 0; ii < scanCount; ii++) {
      ...
      if (nmcols > 1) { // Get pen plot color from gradient
         int colx = ii % nmcols;
   ...
   if (haveSim) {
      ...
      for (int ii = 0; ii < scanCount; ii++) {
         ...
         for (int jj = 0; jj < points; jj++) { // accumulate coordinates of simulation curve
      ...
      DbgLv(1) << "SDAT 0-3" << sdata->value(0, hh) << sdata->value(1, hh) << sdata->value(2, hh)
               << sdata->value(3, hh);
      DbgLv(1) << "SDAT *-n" << sdata->value(ss - 4, hh) << sdata->value(ss - 3, hh) << sdata->value(ss - 2, hh)
               << sdata->value(ss - 1, hh);
...
void US_FeMatch::save_data(void) {
   if (eplotcd == 0 || eplotcd->data_3dplot() == 0) { // if no 3d plot control up,  ask what user wants for 3d plot save
      if (eplotcd == 0)
      ...
      QPushButton *bSkip = mBox.addButton(tr("Skip"), QMessageBox::NoRole);
      QPushButton *bEdit = mBox.addButton(tr("Edit"), QMessageBox::YesRole);
      mBox.setDefaultButton(bEdit);
      mBox.setWindowTitle(tr("3D Plot Save?"));
      mBox.setTextFormat(Qt::RichText);
      mBox.setText(tr("You selected Save Data without a 3D Plot already displayed.<br>"
   ...
   QString dsinfFile = basename.section("/", 0, -2) + "/dsinfo." + tripnode + ".dataset_info.html";
   ...
   if (p1type == 9)
      ...
      distrib_plot_stick(p1type);
   else
      distrib_plot_2d(p1type);
   ...
   QString umsg = tr("In directory ") + basename.left(basename.lastIndexOf("/")) + tr(" ,\nwrote:\n");

   for (int ii = 0; ii < files.length(); ii++) {
      QString fname = files[ ii ];
      umsg = umsg + "    " + fname.mid(fname.lastIndexOf("/") + 1) + "\n";
   }

   QDateTime time1 = QDateTime::currentDateTime();
   if (dkdb_cntrls->db()) { // Copy report files to the database
...
void US_FeMatch::view_report() {
   ...
   editd->e->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize()));
...
void US_FeMatch::distrib_type() {
   const char *dptyp[] = { "s20,w distribution", "MW distribution", "D20,w distribution",
                           // "f_f0 vs s20,w",
                           // "f_f0 vs MW",
                           // "vbar vs s20,w",
                           // "vbar vs MW",
...
void US_FeMatch::distrib_plot_resids() {
   ...
   for (int jj = 0; jj < dsize; jj++) { // accumulate x (radius) values and min,max
   ...
   for (int ii = 0; ii < scanCount; ii++) { // accumulate min,max y (residual) values
      for (int jj = 0; jj < dsize; jj++) {
         yval = resids[ ii ][ jj ];
         ymin = min(yval, ymin);
         ymax = max(yval, ymax);
   ...
   for (int ii = 0; ii < scanCount; ii++) { // draw residual dots a scan at a time

      for (int jj = 0; jj < dsize; jj++) { // get residuals for this scan
         yy[ jj ] = resids[ ii ][ jj ];
      }
...
void US_FeMatch::adv_dmga() {
   ...
   advddiag = new US_AdvDmgaMc(&model_used, imodels, adv_vals, ( QWidget * ) this);
   advddiag->show();
}
...
void US_FeMatch::load_model() {
   ...
   US_ModelLoader dialog(loadDB, mfilter, model, mdesc, dataList[ drow ].editGUID);
   ...
   DbgLv(1) << "post-Load m,e,r GUIDs" << model.modelGUID << model.editGUID << model.requestGUID;
   ...
   DbgLv(1) << "post-Load mC" << model.monteCarlo << "is_dmga_mc" << is_dmga_mc << "description" << model.description;
   ...
   if (is_dmga_mc) { // For DMGA-MC loaded model, build the vector of iteration models
...
void US_FeMatch::adjust_model() {
   ...
   for (int jj = 0; jj < model.components.size(); jj++) {
      ...
      sc->s /= scorrec;
      sc->D /= dcorrec;
      DbgLv(1) << "Fem:Adj:  s20w D20w" << s20w << d20w << "s D" << sc->s << sc->D << "  jj" << jj << "vb20 vb"
               << sc->vbar20 << sd.vbar;
...
void US_FeMatch::load_noise() {
   ...
   te_desc->setText(tr("<b>Scanning noise for %1 ...</b>").arg(triples[ lw_triples->currentRow() ]));
   qApp->processEvents();
   US_LoadableNoise lnoise;
   bool loadDB = dkdb_cntrls->db();
   int noisdf = US_Settings::noise_dialog();
   int nenois = lnoise.count_noise(!loadDB, edata, &model, mieGUIDs, nieGUIDs);

   qDebug() << "load_noise(): mieGUIDs, nieGUIDs, editGUID, modelGUID -- " << mieGUIDs << nieGUIDs << editGUID
            << modelGUID;
   ...
   if (nenois > 0) { // There is/are noise(s):  ask user if she wants to load
      ...
      else if (nenois > 1 && noisdf > 0) { // more than 1:  get choice from noise loader dialog
         US_NoiseLoader *nldiag = new US_NoiseLoader(dbP, mieGUIDs, nieGUIDs, ti_noise, ri_noise, edata);
...
void US_FeMatch::simulate_model() {
   ...
   DbgLv(1) << "SimMdl: 0) s D c" << model_used.components[ 0 ].s << model_used.components[ 0 ].D
            << model_used.components[ 0 ].signal_concentration << "  n" << lc;
   DbgLv(1) << "SimMdl: n) s D c" << model_used.components[ lc ].s << model_used.components[ lc ].D
            << model_used.components[ lc ].signal_concentration;
   adjust_model();
   DbgLv(1) << "SimMdl: 0) s D c" << model.components[ 0 ].s << model.components[ 0 ].D
            << model.components[ 0 ].signal_concentration;
   DbgLv(1) << "SimMdl: n) s D c" << model.components[ lc ].s << model.components[ lc ].D
   ...
   DbgLv(1) << "SimMdl: simpoints" << simparams.simpoints << "rreso" << simparams.radial_resolution << "bottom_sim"
   ...
   if ((check_file.exists()) && (check_file.isFile())) {
      if (US_AstfemMath::timestate_onesec(tmst_fpath, *sdata)) { // Load timestate that is already at 1-second-intervals
         simparams.simSpeedsFromTimeState(tmst_fpath);
         DbgLv(1) << "SimMdl: timestate file exists" << tmst_fpath << " timestateobject,count = " << simparams.tsobj
      ...
      if (!simparams.sim) { // Compute acceleration rate for non-astfem_sim data
         ...
         DbgLv(1) << "SimMdl:  accel-calc:  t1 t2 w2t t_acc speed rate" << t1 << t2 << w2t << t_acc
   ...
   DbgLv(1) << "SimMdl: ssck: rspeed accel1 lo_ss_acc" << rspeed << accel1 << lo_ss_acc << "tf_aend tf_scan" << tf_aend
            << tf_scan;
   //x0  1  2  3  4  5
   ...
   DbgLv(1) << "SimMdl: nthread" << nthread << "ncomp" << ncomp << "ntc" << ntc << "meshtype" << simparams.meshType;
   ...
   if (nthread < 2) {
      if (
         ...
         for (int ii = 0; ii < model.components.size(); ii++) {
            DbgLv(1) << "SimMdl:   comp" << ii << "s D v" << model.components[ ii ].s << model.components[ ii ].D
         ...
         connect(astfem_rsa, SIGNAL(current_component(int)), this, SIGNAL(astfem_cmp(int)));
...
QStringList US_FeMatch::last_edit_files(QStringList files) {
   ...
   for (int ii = 1; ii < nfi; ii++) {
      ...
      if (QString::compare(test, ptest) != 0) { // differs by more than just edit, so output previous
         ofiles.append(pfile);
      }
   }
...
QString US_FeMatch::table_row(const QString &s1, const QString &s2, const QString &s3) const {
   return (indent(6) + "<tr><td>" + s1 + "</td><td>" + s2 + "</td><td>" + s3 + "</td></tr>\n");
}

// Table row HTML with 5 columns
QString US_FeMatch::table_row(
...
QString US_FeMatch::html_header(QString title, QString head1, US_DataIO::EditedData *edata) {
...
QString US_FeMatch::run_details(void) {
   ...
   QString dataType = tr("Absorbance");
   if (d->dataType == "RI" && !has_intensity_profile(d->runID, in_db))
      dataType = tr("Intensity");
   if (d->dataType == "WI")
      dataType = tr("Intensity");
   if (d->dataType == "IP")
   ...
   QString s = "\n" + indent(4) + tr("<h3>Detailed Run Information:</h3>\n") + indent(4) + "<table>\n"
               + table_row(tr("Cell Description:"), d->description) + table_row(tr("Data Directory:"), workingDir)
               + table_row(tr("Rotor Speed:"), QString::number(( int ) d->scanData[ 0 ].rpm) + " rpm");
   ...
   if (maxTemp - minTemp <= US_Settings::tempTolerance())
      ...
      s += table_row(tr("Temperature Variation:"), tr("(!) OUTSIDE TOLERANCE (!)"));
   ...
   s += table_row(tr("Time Correction:"), QString::number(minutes) + m + QString::number(seconds) + sec);
   ...
   s += table_row(
      tr("Run Duration:"), QString::number(hours) + h + QString::number(minutes) + m + QString::number(seconds) + sec);

   // Wavelength, baseline, meniscus, range
   s += table_row(tr("Wavelength:"), d->wavelength + " nm")
        + table_row(tr("Baseline ") + dataType + ":", QString::number(baseline, 'f', 6) + " OD")
        + table_row(tr("Meniscus Position:     "), QString::number(d->meniscus, 'f', 3) + " cm");

   int rrx = d->xvalues.size() - 1;
   double left = d->xvalues[ 0 ];
   double right = d->xvalues[ rrx ];

   s += table_row(tr("Edited Data starts at: "), QString::number(left, 'f', 3) + " cm")
        + table_row(tr("Edited Data stops at:  "), QString::number(right, 'f', 3) + " cm");
   s += indent(4) + "</table>\n";
   return s;
}
...
QString US_FeMatch::hydrodynamics(void) const {
   ...
   s += table_row(tr("Viscosity corrected:"), QString::number(solution.viscosity, 'f', 5))
        + table_row(tr("Viscosity (absolute):"), QString::number(solution.viscosity_tb, 'f', 5))
        + table_row(tr("Density corrected:"), QString::number(solution.density, 'f', 6) + " g/ccm")
        + table_row(tr("Density (absolute):"), QString::number(solution.density_tb, 'f', 6) + " g/ccm")
        + table_row(tr("Vbar:"), QString::number(solution.vbar, 'f', 6) + " ccm/g")
        + table_row(tr("Vbar corrected for 20 ") + MLDEGC + ":", QString::number(solution.vbar20, 'f', 6) + " ccm/g")
        + table_row(tr("Buoyancy (Water, 20 ") + MLDEGC + "): ", QString::number(solution.buoyancyw, 'f', 6))
        + table_row(tr("Buoyancy (absolute)"), QString::number(solution.buoyancyb, 'f', 6))
        + table_row(tr("Correction Factor (s):"), QString::number(solution.s20w_correction, 'f', 6))
        + table_row(tr("Correction Factor (D):"), QString::number(solution.D20w_correction, 'f', 6)) + indent(4)
        + "</table>\n";
...
QString US_FeMatch::scan_info(void) const {
   ...
   s += table_row(tr("Scan"), tr("Corrected Time"), tr("Plateau Concentration"), tr("Seconds"), tr("Omega^2T"));
...
QString US_FeMatch::distrib_info() {
   ...
   mstr += table_row(tr("Residual RMS Deviation:"), le_rmsd->text());
   ...
   if (cnstvb) { // Normal constant-vbar distribution
      mstr += table_row(
         tr("Molec. Wt."), tr("S Apparent"), tr("S 20,W"), tr("D Apparent"), tr("D 20,W"), tr("f / f0"),
         tr("Concentration"));
      ...
      mstr += table_row(
         tr("Molec. Wt."), tr("S Apparent"), tr("S 20,W"), tr("D Apparent"), tr("D 20,W"), tr("Vbar20"),
         tr("Concentration"));
      ...
      mstr += table_row(
         tr("Molec. Wt."), tr("S Apparent"), tr("S 20,W"), tr("D 20,W"), tr("f / f0"), tr("Vbar20"),
         tr("Concentration"));
   ...
   if (is_dmga_mc) { // Compute DMGA-MC statistics and add them to the report
      ...
      mstr += indent(4) + "<table>\n";
      mstr += table_row(
         tr("Component"), tr("Attribute"), tr("Mean_Value"), tr("95%_Confidence(low)"), tr("95%_Confidence(high)"));
      int kd = 0;
      QString fixd = tr("(Fixed)");
      QString blnk("");
      QStringList atitl;
      QStringList rtitl;
      atitl << tr("Concentration") << tr("Vbar20") << tr("Molecular Weight") << tr("Sedimentation Coefficient")
            << tr("Diffusion Coefficient") << tr("Frictional Ratio");
      rtitl << tr("K_dissociation") << tr("K_off Rate");

      // Show summary of RMSDs
      mstr += table_row(
         tr("(All)"), tr("RMSD"), QString().sprintf("%10.4e", rstats[ 2 ]), QString().sprintf("%10.4e", rstats[ 9 ]),
      ...
      for (int ii = 0; ii < ncomp; ii++) {
         ...
         for (int jj = 0; jj < 6; jj++) {
            ...
            QString strclo = is_fixed ? fixd : QString().sprintf("%10.4e", mstats[ kd ][ 9 ]);
            QString strchi = is_fixed ? blnk : QString().sprintf("%10.4e", mstats[ kd ][ 10 ]);
            mstr += table_row(compnum, atitl[ jj ], QString().sprintf("%10.4e", mstats[ kd ][ 2 ]), strclo, strchi);
            kd++;
         }
      }
      ...
      mstr += indent(4) + "<table>\n";
      mstr += table_row(
         tr("Reaction"), tr("Attribute"), tr("Mean_Value"), tr("95%_Confidence(low)"), tr("95%_Confidence(high)"));
      ...
      for (int ii = 0; ii < nreac; ii++) {
         ...
         QString strchi = is_fixed ? blnk : QString().sprintf("%10.4e", mstats[ kd ][ 10 ]);
         mstr
            += table_row(reacnum, tr("K_dissociation"), QString().sprintf("%10.4e", mstats[ kd ][ 2 ]), strclo, strchi);
         kd++;
         is_fixed = (mstats[ kd ][ 0 ] == mstats[ kd ][ 1 ]);
         strclo = is_fixed ? fixd : QString().sprintf("%10.4e", mstats[ kd ][ 9 ]);
         strchi = is_fixed ? blnk : QString().sprintf("%10.4e", mstats[ kd ][ 10 ]);
         mstr += table_row(reacnum, tr("K_off Rate"), QString().sprintf("%10.4e", mstats[ kd ][ 2 ]), strclo, strchi);
         kd++;
      }
      ...
      mstr += indent(4) + "<table>\n";
      mstr += table_row(tr("Minimum:"), QString().sprintf("%10.4e", rstats[ 0 ]));
      mstr += table_row(tr("Maximum:"), QString().sprintf("%10.4e", rstats[ 1 ]));
      mstr += table_row(tr("Mean:"), QString().sprintf("%10.4e", rstats[ 2 ]));
      mstr += table_row(tr("Median:"), QString().sprintf("%10.4e", rstats[ 3 ]));
      mstr += table_row(tr("Skew:"), QString().sprintf("%10.4e", rstats[ 4 ]));
      mstr += table_row(tr("Kurtosis:"), QString().sprintf("%10.4e", rstats[ 5 ]));
      mstr += table_row(tr("Lower Mode:"), QString().sprintf("%10.4e", rstats[ 6 ]));
      mstr += table_row(tr("Upper Mode:"), QString().sprintf("%10.4e", rstats[ 7 ]));
      mstr += table_row(tr("Mode Center:"), QString().sprintf("%10.4e", rstats[ 8 ]));
      mstr += table_row(tr("95% Confidence Interval Low:"), QString().sprintf("%10.4e", rstats[ 9 ]));
      mstr += table_row(tr("95% Confidence Interval High:"), QString().sprintf("%10.4e", rstats[ 10 ]));
      mstr += table_row(tr("99% Confidence Interval Low:"), QString().sprintf("%10.4e", rstats[ 11 ]));
      mstr += table_row(tr("99% Confidence Interval High:"), QString().sprintf("%10.4e", rstats[ 12 ]));
      mstr += table_row(tr("Standard Deviation:"), QString().sprintf("%10.4e", rstats[ 13 ]));
      mstr += table_row(tr("Standard Error:"), QString().sprintf("%10.4e", rstats[ 14 ]));
      mstr += table_row(tr("Variance:"), QString().sprintf("%10.4e", rstats[ 15 ]));
      mstr += table_row(tr("Correlation Coefficient:"), QString().sprintf("%10.4e", rstats[ 16 ]));
      mstr += table_row(tr("Number of Bins:"), QString().sprintf("%10.0f", rstats[ 17 ]));
      mstr += table_row(tr("Distribution Area:"), QString().sprintf("%10.4e", rstats[ 18 ]));
      mstr += table_row(tr("95% Confidence Limit Low:"), QString().sprintf("%10.4e", rstats[ 19 ]));
      mstr += table_row(tr("95% Confidence Limit High:"), QString().sprintf("%10.4e", rstats[ 20 ]));
      mstr += table_row(tr("99% Confidence Limit Low:"), QString().sprintf("%10.4e", rstats[ 21 ]));
      mstr += table_row(tr("99% Confidence Limit High:"), QString().sprintf("%10.4e", rstats[ 22 ]));
      mstr += indent(4) + "</table>\n";
      ...
      for (kd = 0; kd < ntatt; kd++) {
         ...
         if (icomp <= ncomp) { // Title for component detail
            if (imodels[ 0 ].is_product(icomp - 1) && kk == 0)
               mstr += compnum + tr("(Product) Total Concentration") + ":</h4>\n";
            else
               mstr += compnum + atitl[ kk ] + ":</h4>\n";
         }
         else { // Title for reaction detail
         ...
         if (is_fixed) { // Fixed has limited lines
            mstr += table_row(tr("Minimum:"), QString().sprintf("%10.4e", mstats[ kd ][ 0 ]));
            mstr += table_row(tr("Maximum:"), QString().sprintf("%10.4e", mstats[ kd ][ 1 ]));
            mstr += table_row(tr("Mean:"), QString().sprintf("%10.4e", mstats[ kd ][ 2 ]));
            mstr += table_row(tr("Median (Fixed)"), QString().sprintf("%10.4e", mstats[ kd ][ 3 ]));
         }

         else { // Float has full set of statistics details
            mstr += table_row(tr("Minimum:"), QString().sprintf("%10.4e", mstats[ kd ][ 0 ]));
            mstr += table_row(tr("Maximum:"), QString().sprintf("%10.4e", mstats[ kd ][ 1 ]));
            mstr += table_row(tr("Mean:"), QString().sprintf("%10.4e", mstats[ kd ][ 2 ]));
            mstr += table_row(tr("Median:"), QString().sprintf("%10.4e", mstats[ kd ][ 3 ]));
            mstr += table_row(tr("Skew:"), QString().sprintf("%10.4e", mstats[ kd ][ 4 ]));
            mstr += table_row(tr("Kurtosis:"), QString().sprintf("%10.4e", mstats[ kd ][ 5 ]));
            mstr += table_row(tr("Lower Mode:"), QString().sprintf("%10.4e", mstats[ kd ][ 6 ]));
            mstr += table_row(tr("Upper Mode:"), QString().sprintf("%10.4e", mstats[ kd ][ 7 ]));
            mstr += table_row(tr("Mode Center:"), QString().sprintf("%10.4e", mstats[ kd ][ 8 ]));
            mstr += table_row(tr("95% Confidence Interval Low:"), QString().sprintf("%10.4e", mstats[ kd ][ 9 ]));
            mstr += table_row(tr("95% Confidence Interval High:"), QString().sprintf("%10.4e", mstats[ kd ][ 10 ]));
            mstr += table_row(tr("99% Confidence Interval Low:"), QString().sprintf("%10.4e", mstats[ kd ][ 11 ]));
            mstr += table_row(tr("99% Confidence Interval High:"), QString().sprintf("%10.4e", mstats[ kd ][ 12 ]));
            mstr += table_row(tr("Standard Deviation:"), QString().sprintf("%10.4e", mstats[ kd ][ 13 ]));
            mstr += table_row(tr("Standard Error:"), QString().sprintf("%10.4e", mstats[ kd ][ 14 ]));
            mstr += table_row(tr("Variance:"), QString().sprintf("%10.4e", mstats[ kd ][ 15 ]));
            mstr += table_row(tr("Correlation Coefficient:"), QString().sprintf("%10.4e", mstats[ kd ][ 16 ]));
            mstr += table_row(tr("Number of Bins:"), QString().sprintf("%10.0f", mstats[ kd ][ 17 ]));
            mstr += table_row(tr("Distribution Area:"), QString().sprintf("%10.4e", mstats[ kd ][ 18 ]));
            mstr += table_row(tr("95% Confidence Limit Low:"), QString().sprintf("%10.4e", mstats[ kd ][ 19 ]));
            mstr += table_row(tr("95% Confidence Limit High:"), QString().sprintf("%10.4e", mstats[ kd ][ 20 ]));
            mstr += table_row(tr("99% Confidence Limit Low:"), QString().sprintf("%10.4e", mstats[ kd ][ 21 ]));
            mstr += table_row(tr("99% Confidence Limit High:"), QString().sprintf("%10.4e", mstats[ kd ][ 22 ]));
         }
...
void US_FeMatch::write_plot(const QString &filename, const QwtPlot *plot) {
   if (filename.contains(".svg")) { // Save an SVG file and a PNG copy
   ...
   else if (filename.endsWith("rbitmap.png")) { // Special case of rbitmap PNG
      if (rbmapd == 0) { // if it is not currently displayed, display it
         rbmapd = new US_ResidsBitmap(resids);
         rbmapd->move(bmd_pos);
         rbmapd->show();
         rbmapd->raise();
      ...
      else { // if already displayed,  replot and re-activate
...
#else
      QPixmap pixmap = QPixmap::grabWidget(rbmapd, 0, 0, rbmapd->width(), rbmapd->height());
#endif
      ...
      if (eplotcd == 0) { // if no 3d plot control up,  create it now
         eplotcd = new US_PlotControlFem(this, &model);
         eplotcd->move(epd_pos);
         eplotcd->show();
         eplotcd->do_3dplot();
...
void US_FeMatch::get_solution() {
   ...
   int dbdisk = (dkdb_cntrls->db()) ? US_Disk_DB_Controls::DB : US_Disk_DB_Controls::Disk;
   int expID = 0;
   solution_rec.buffer.manual = manual;
   ...
   US_SolutionGui *soluInfo = new US_SolutionGui(expID, 1, true, dbdisk, solution_rec, false);

   connect(soluInfo, SIGNAL(updateSolutionGuiSelection(US_Solution)), this, SLOT(updateSolution(US_Solution)));
...
void US_FeMatch::updateSolution(US_Solution solution_sel) {
   ...
   if (dkdb_cntrls->db()) {
      US_Passwd pw;
      US_DB2 db(pw.getPasswd());
      US_SolutionVals::bufvals_db(&db, sbufID, bufGUID, bufDesc, bdens, bvisc, bcmpr, bmanu, errmsg);
   }

   else {
      US_SolutionVals::bufvals_disk(sbufID, bufGUID, bufDesc, bdens, bvisc, bcmpr, bmanu, errmsg);
   }
...
void US_FeMatch::reset(void) {
   ...
   connect(ct_from, SIGNAL(valueChanged(double)), SLOT(exclude_from(double)));

   connect(ct_to, SIGNAL(valueChanged(double)), SLOT(exclude_to(double)));
...
int US_FeMatch::type_distrib() {
   ...
   for (int ii = 0; ii < ndptyp; ii++) { // identify text of current push button
      if (curtxt == QString(dptyp[ ii ])) { // found:  save index and break
         itype = ii;
         break;
      }
   }
...
void US_FeMatch::reportFilesToDB(QStringList &files) {
   ...
   int st = freport.saveFileDocuments(pfdir, files, dbP, idEdit, tripdesc);
...
void US_FeMatch::model_table(QString mdtFile) {
   ...
   ts << dquote + "Molec.Weight" + dquote + comma + dquote + "S_Apparent" + dquote + comma + dquote + "S_20_W" + dquote
            + comma + dquote + "D_Apparent" + dquote + comma + dquote + "D_20_W" + dquote + comma + dquote + "f/f0"
            + dquote + comma + dquote + "Vbar20" + dquote + comma + dquote + "Concentration" + dquote + comma + dquote
            + "Conc.Percent" + dquote + endln;

   for (int ii = 0; ii < ncomp; ii++) { // Write each component line
      double conc = model_used.components[ ii ].signal_concentration;
      double perc = 100.0 * conc / sum_c;
      ts << dquote + QString().sprintf("%10.4e", model_used.components[ ii ].mw) + dquote + comma + dquote
               + QString().sprintf("%10.4e", model.components[ ii ].s) + dquote + comma + dquote
               + QString().sprintf("%10.4e", model_used.components[ ii ].s) + dquote + comma + dquote
               + QString().sprintf("%10.4e", model.components[ ii ].D) + dquote + comma + dquote
               + QString().sprintf("%10.4e", model_used.components[ ii ].D) + dquote + comma + dquote
               + QString().sprintf("%10.4e", model_used.components[ ii ].f_f0) + dquote + comma + dquote
               + QString().sprintf("%10.4e", model_used.components[ ii ].vbar20) + dquote + comma + dquote
               + QString().sprintf("%10.4e", conc) + dquote + comma + dquote + QString().sprintf("%5.2f %%", perc)
               + dquote + endln;
   }
   ...
   // Write the header line
   ts << dquote + "Reaction" + dquote + comma + dquote + "K_dissociation" + dquote + comma + dquote + "k_off_Rate"
            + dquote + endln;

   for (int ii = 0; ii < nreac; ii++) { // Write each reaction line
      double k_d = model_used.associations[ ii ].k_d;
      double k_off = model_used.associations[ ii ].k_off;
      ts << dquote + QString().sprintf("%4d", ii) + dquote + comma + dquote + QString().sprintf("%10.4e", k_d) + dquote
               + comma + dquote + QString().sprintf("%10.4e", k_off) + dquote + endln;
   }
}
...
void US_FeMatch::curmod_clicked(bool is_on) {
   ...
   if (is_on) { // Current Model turned on:  update model
      update_mc_model();
   }
}
...
void US_FeMatch::modbtn_clicked(bool is_on) {
   if (is_on) { // Button has been turned on:  update model
      update_mc_model();
   }
}
...
void US_FeMatch::update_mc_model() {
   if (rb_curmod->isChecked()) { // The model is the currently selected MC iteration model
   }

   else if (rb_mean->isChecked()) { // The model is the mean of all iteration models
   }

   else if (rb_median->isChecked()) { // The model is the median of all iteration models
   }

   else if (rb_mode->isChecked()) { // The model is the mode of all iteration models
   }
}
...
bool US_FeMatch::has_intensity_profile(const QString &runID, const bool in_db) {
   ...
   if (in_db) { // Data from DB:  get any RIProfile from experiment record
      US_Passwd pw;
      US_DB2 db(pw.getPasswd());
      QStringList query;
      query << "get_experiment_info_by_runID" << runID << QString::number(US_Settings::us_inv_ID());
   ...
   else { // Data local:  read in any RIProfile from an XML file
      QString filename = US_Settings::resultDir() + "/" + runID + "/" + runID + ".RIProfile.xml";
...
void US_FeMatch::auto_load_simulate(
   US_DataIO::RawData i_rdata, US_DataIO::EditedData i_edata, US_Model i_model, int idExp, QPoint pos) {
   reset();
   QString file;
   QStringList files;
   QStringList parts;
   ...
   rawList << i_rdata;
   dataList << i_edata;
   triples << tr("%1 / %2 / %3").arg(i_rdata.cell).arg(i_rdata.channel).arg(i_rdata.scanData.at(0).wavelength);
   ...
   if (is_dmga_mc) { // For DMGA-MC loaded model, build the vector of iteration models
      ...
      if (US_Settings::advanced_level() > 0) { // For non dmga and users with advanced settings > 0, show the advanced dialog
         model_used = model_loaded;
