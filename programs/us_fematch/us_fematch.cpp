...
US_FeMatch::US_FeMatch () : US_Widgets ()
{
  ...
  connect (dkdb_cntrls, SIGNAL (changed (bool)), this,
  ...
  connect (pb_reset_exclude, SIGNAL (clicked ()), this,
           SLOT (reset_excludes ()));
  ...
  connect (rb_curmod, SIGNAL (toggled (bool)), this,
           SLOT (curmod_clicked (bool)));
  connect (rb_mean, SIGNAL (toggled (bool)), this,
           SLOT (modbtn_clicked (bool)));
  connect (rb_median, SIGNAL (toggled (bool)), this,
           SLOT (modbtn_clicked (bool)));
  connect (rb_mode, SIGNAL (toggled (bool)), this,
           SLOT (modbtn_clicked (bool)));
...
US_FeMatch::load (void)
{
  ...
  edata = &dataList[0];
  runID = edata->runID;
  QString tmst_fpath = US_Settings::resultDir () + "/" + runID + "/" + runID
                       + ".time_state.tmst";
  ...
  if (check_file.exists () && check_file.isFile ())
    { // Get speed_steps from an existing timestate file
      ...
      // ALEXEY: #speed steps from TimeStamp can be bigger than what's read
      // from DB??
      ...
      for (int ii = 0; ii < nssp; ii++)
        {
          ...
          for (int ds = 0; ds < ntriples; ds++)
            { // Scan data time ranges and compare to experiment speed steps
              ...
              DbgLv (1) << "LD:  ds" << ds << "step" << ii << "stimes" << stm1
                        << stm2 << "etimes" << etm1 << etm2 << "lesc" << lesc;

              if (etm1 < stm1 || etm2 > stm2)
                { // Data times beyond speed step ranges, so flag use of data
                  // ranges
                  nssp = 0;
...
US_FeMatch::data_plot (void)
{
  ...
  int point
      = US_DataIO::index (dataList[drow].xvalues, dataList[drow].baseline);
      ...
      if (sdata->value (ss - 1, hh) == 0.0)
        {
          for (int js = 0; js < scanCount; js++)
            DbgLv (1) << "SDAT     js" << js << "sd" << sdata->value (js, hh)
                      << "ed" << edata->value (js, hh);
        }
      //*DEBUG
...
void
US_FeMatch::exclude_from (double from)
{
...
void
US_FeMatch::exclude_to (double to)
{
...
void
US_FeMatch::distrib_type ()
{
  const char *dptyp[]
      = { "s20,w distribution", "MW distribution", "D20,w distribution",
          // "f_f0 vs s20,w",
          // "f_f0 vs MW",
          // "vbar vs s20,w",
          // "vbar vs MW",
...
void
US_FeMatch::distrib_plot_stick (int type)
{
...
void
US_FeMatch::distrib_plot_2d (int type)
{
  QString pltitle = tr ("Run ") + edata->runID + tr (" :\nCell ") + edata->cell
                    + " (" + edata->wavelength + " nm)";
  QString yatitle;
...
void
US_FeMatch::distrib_plot_resids ()
{
  QString pltitle = tr ("Run ") + edata->runID + tr (" :\nCell ") + edata->cell
...
US_FeMatch::adv_dmga ()
{
  ...
  advddiag
      = new US_AdvDmgaMc (&model_used, imodels, adv_vals, (QWidget *)this);
  advddiag->show ();
}
...
US_FeMatch::adjust_model ()
{
  ...
  DbgLv (1) << "Fem:Adj:  avgT" << avgTemp << "vb20 vb" << sd.vbar20
            << sd.vbar;
  ...
  for (int jj = 0; jj < model.components.size (); jj++)
    {
      ...
      sc->s /= scorrec;
      sc->D /= dcorrec;
      DbgLv (1) << "Fem:Adj:  s20w D20w" << s20w << d20w << "s D" << sc->s
    ...
    { // There is/are noise(s):  ask user if she wants to load
      ...
      if (nenois > 1 && noisdf == 0)
        ...
        { // more than 1:  get choice from noise loader dialog
          US_NoiseLoader *nldiag = new US_NoiseLoader (
              dbP, mieGUIDs, nieGUIDs, ti_noise, ri_noise, edata);
      ...
      if (US_AstfemMath::timestate_onesec (tmst_fpath, *sdata))
        ...
        { // Replace timestate with a new one that is at 1-second-intervals
          if (drow == 0)
            {
              QString tmst_fdefs
                  = QString (tmst_fpath).replace (".tmst", ".xml");
  ...
  else
    {
      ...
      if (!simparams.sim)
        { // Compute acceleration rate for non-astfem_sim data
          ...
          DbgLv (1) << "SimMdl:  accel-calc:  t1 t2 w2t t_acc speed rate" << t1
...
QStringList
US_FeMatch::last_edit_files (QStringList files)
{
  QStringList ofiles;
  QStringList part;
  QString file;
...
void
US_FeMatch::component_values (int /*index*/)
{
#if 0
...
void
US_FeMatch::comp_number (double cnbr)
{
...
double
US_FeMatch::interp_sval (double xv, double *sx, double *sy, int ssize)
{
...
void
US_FeMatch::write_report (QTextStream &ts)
{
...
void
US_FeMatch::write_dset_report (QString &dsfname)
{
  QFile f_rep (dsfname);
...
double
US_FeMatch::calc_baseline (int drow) const
{
...
QString
US_FeMatch::text_model (US_Model model, int width)
{
...
US_FeMatch::calc_residuals ()
{
  ...
  int kpts = 0;
  DbgLv (1) << "CALC_RESID: matchd" << matchd << "dsize ssize" << dsize
            << ssize;
  int kexcls = 0;
...
QString
US_FeMatch::indent (const int spaces) const
{
...
QString
US_FeMatch::table_row (const QString &s1, const QString &s2) const
{
...
QString
US_FeMatch::table_row (const QString &s1, const QString &s2,
...
QString
US_FeMatch::table_row (const QString &s1, const QString &s2, const QString &s3,
                       const QString &s4, const QString &s5) const
{
  return (indent (6) + "<tr><td>" + s1 + "</td><td>" + s2 + "</td><td>" + s3
...
QString
US_FeMatch::html_header (QString title, QString head1,
...
US_FeMatch::run_details (void)
{
  ...
  s += table_row (tr ("Time Correction:"), QString::number (minutes) + m
...
QString
US_FeMatch::hydrodynamics (void) const
{
...
QString
US_FeMatch::scan_info (void) const
{
  int drow = lw_triples->currentRow ();
  const US_DataIO::EditedData *d = &dataList[drow];
          ...
          if (is_fixed)
            ...
            { // Float has full set of statistics details
              ...
              mstr += table_row (tr ("95% Confidence Interval Low:"),
                                 QString ().sprintf ("%10.4e", mstats[kd][9]));
              mstr
                  += table_row (tr ("95% Confidence Interval High:"),
                                QString ().sprintf ("%10.4e", mstats[kd][10]));
              mstr
                  += table_row (tr ("99% Confidence Interval Low:"),
                                QString ().sprintf ("%10.4e", mstats[kd][11]));
              mstr
                  += table_row (tr ("99% Confidence Interval High:"),
                                QString ().sprintf ("%10.4e", mstats[kd][12]));
              mstr
                  += table_row (tr ("Standard Deviation:"),
                                QString ().sprintf ("%10.4e", mstats[kd][13]));
              mstr
                  += table_row (tr ("Standard Error:"),
                                QString ().sprintf ("%10.4e", mstats[kd][14]));
              mstr
                  += table_row (tr ("Variance:"),
                                QString ().sprintf ("%10.4e", mstats[kd][15]));
              mstr
                  += table_row (tr ("Correlation Coefficient:"),
                                QString ().sprintf ("%10.4e", mstats[kd][16]));
              mstr
                  += table_row (tr ("Number of Bins:"),
                                QString ().sprintf ("%10.0f", mstats[kd][17]));
              mstr
                  += table_row (tr ("Distribution Area:"),
                                QString ().sprintf ("%10.4e", mstats[kd][18]));
              mstr
                  += table_row (tr ("95% Confidence Limit Low:"),
                                QString ().sprintf ("%10.4e", mstats[kd][19]));
              mstr
                  += table_row (tr ("95% Confidence Limit High:"),
                                QString ().sprintf ("%10.4e", mstats[kd][20]));
              mstr
                  += table_row (tr ("99% Confidence Limit Low:"),
                                QString ().sprintf ("%10.4e", mstats[kd][21]));
              mstr
                  += table_row (tr ("99% Confidence Limit High:"),
...
// Write out a plot
void
US_FeMatch::write_plot (const QString &filename, const QwtPlot *plot)
{
...
#else
      QPixmap pixmap = QPixmap::grabWidget (rbmapd, 0, 0, rbmapd->width (),
                                            rbmapd->height ());
#endif
...
bool
US_FeMatch::mkdir (const QString &baseDir, const QString &subdir)
{
  QDir folder (baseDir);
...
void
US_FeMatch::new_triple (int trow)
{
  haveSim = false;
...
// Set progress text
void
US_FeMatch::set_progress (const QString message)
{
...
void
US_FeMatch::update_disk_db (bool isDB)
{
...
void
US_FeMatch::updateSolution (US_Solution solution_sel)
{
  ...
  if (dkdb_cntrls->db ())
    ...
    {
      US_SolutionVals::bufvals_disk (sbufID, bufGUID, bufDesc, bdens, bvisc,
                                     bcmpr, bmanu, errmsg);
    }
...
void
US_FeMatch::update_progress (int icomp)
{
...
// Reset scan excludes
void
US_FeMatch::reset_excludes (void)
{
...
// Copy report files to the database
void
US_FeMatch::reportFilesToDB (QStringList &files)
{
  US_Passwd pw;
  US_DB2 db (pw.getPasswd ());
  US_DB2 *dbP = &db;
...
void
US_FeMatch::thread_progress (int thr, int icomp)
{
  int kcomp = 0;
  kcomps[thr] = icomp;
  for (int ii = 0; ii < nthread; ii++)
...
void
US_FeMatch::thread_complete (int thr)
{
  thrdone++;
...
void
US_FeMatch::model_table (QString mdtFile)
{
  ...
  ts << dquote + "Molec.Weight" + dquote + comma + dquote + "S_Apparent"
            ...
            + "D_Apparent" + dquote + comma + dquote + "D_20_W" + dquote
            + comma + dquote + "f/f0" + dquote + comma + dquote + "Vbar20"
            + dquote + comma + dquote + "Concentration" + dquote + comma
  ...
  for (int ii = 0; ii < ncomp; ii++)
    { // Write each component line
      ...
      ts << dquote
                ...
                + dquote + comma + dquote
                + QString ().sprintf ("%5.2f %%", perc) + dquote + endln;
    }
...
void
US_FeMatch::update_filelist (QStringList &flist, const QString fname)
{
  flist << fname;
...
void
US_FeMatch::curmod_clicked (bool is_on)
{
...
void
US_FeMatch::modbtn_clicked (bool is_on)
{
  if (is_on)
...
bool
US_FeMatch::has_intensity_profile (const QString &runID, const bool in_db)
{
  QString ripxml;
  ...
  if (in_db)
    ...
    { // Data local:  read in any RIProfile from an XML file
      QString filename = US_Settings::resultDir () + "/" + runID + "/" + runID
                         + ".RIProfile.xml";
...
void
US_FeMatch::auto_load_simulate (US_DataIO::RawData i_rdata,
  ...
  if (is_dmga_mc)
  ...
  else
    {
      if (US_Settings::advanced_level () > 0)
        { // For non dmga and users with advanced settings > 0, show the
          // advanced dialog
          model_used = model_loaded;
          // create advanced dialog and display it
          advdiag
              = new US_AdvancedFem (&model_used, adv_vals, (QWidget *)this);
          advdiag->show ();
...
void
US_FeMatch::closeEvent (QCloseEvent *event)
{
  if (eplotcd != 0)
    {
      epd_pos = eplotcd->pos ();
