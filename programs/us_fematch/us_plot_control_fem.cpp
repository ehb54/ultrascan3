...
US_PlotControlFem::US_PlotControlFem(QWidget* p, US_Model* amodel)
  ...
  if (cnstff0) {  // f/f0 constant, so disable f/f0 check boxes
    ck_xfra->setEnabled(false);
...
QWidget* US_PlotControlFem::caller(void) { return wparent; }

// public slot to force (re-)plot of currently specified 3-D plot
void US_PlotControlFem::do_3dplot() { plot3_btn(); }

// modified copy for autoflow
void US_PlotControlFem::do_3dplot_auto() { plot3_btn_auto(); }
...
// mw x box checked
void US_PlotControlFem::xmwtCheck(bool chkd) { checkSet(chkd, true, 0); }
// mw y box checked
void US_PlotControlFem::ymwtCheck(bool chkd) { checkSet(chkd, false, 0); }
// sedcoeff x box checked
void US_PlotControlFem::xsedCheck(bool chkd) { checkSet(chkd, true, 1); }
// sedcoeff y box checked
void US_PlotControlFem::ysedCheck(bool chkd) { checkSet(chkd, false, 1); }
// diffcoeff x box checked
void US_PlotControlFem::xdifCheck(bool chkd) { checkSet(chkd, true, 2); }
// diffcoeff y box checked
void US_PlotControlFem::ydifCheck(bool chkd) { checkSet(chkd, false, 2); }
// friccoeff x box checked
void US_PlotControlFem::xfcoCheck(bool chkd) { checkSet(chkd, true, 3); }
// friccoeff y box checked
void US_PlotControlFem::yfcoCheck(bool chkd) { checkSet(chkd, false, 3); }
// fricratio x box checked
void US_PlotControlFem::xfraCheck(bool chkd) { checkSet(chkd, true, 4); }
// fricratio y box checked
void US_PlotControlFem::yfraCheck(bool chkd) { checkSet(chkd, false, 4); }
// Vbar x box checked
void US_PlotControlFem::xvbaCheck(bool chkd) { checkSet(chkd, true, 5); }
// Vbar y box checked
void US_PlotControlFem::yvbaCheck(bool chkd) { checkSet(chkd, false, 5); }
...
void US_PlotControlFem::checkSet(bool chkd, bool isX, int row) {
  ...
  if (model->constant_ff0()) {  // f/f0 constant, so disable f/f0 check boxes
...
void US_PlotControlFem::zscal_value(double value) { zscale = value; }
// grid resolution changed
void US_PlotControlFem::gridr_value(double value) { gridres = value; }
// peak smoothing changed
void US_PlotControlFem::peaks_value(double value) { pksmooth = value; }
// peak width changed
void US_PlotControlFem::peakw_value(double value) { pkwidth = value; }
...
void US_PlotControlFem::close_all() {
  if (plot3d_w != 0) plot3d_w->close();

  close();
}

// plot window close has happened
void US_PlotControlFem::plot_close() { plot3d_w = 0; }
