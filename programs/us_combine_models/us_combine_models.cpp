...
void US_CombineModels::add_models() {
  ...
  if (dialog.exec() != QDialog::Accepted) return;  // No selection was made
...
void US_CombineModels::save() {
  ...
  for (int ii = 1; ii < nmodels;
       ii++) {  // Add components from the remainder of the models
    ...
    for (int jj = 0; jj < kcomps;
         jj++) {  // Add model components to the combo model
      bool dupc = false;
      ...
      if (cnstv) {  // Test for duplicate where vbar20 is constant
        for (int kk = 0; kk < ncomps;
             kk++) {  // See if this component already exists
          if (sc.s == cmodel.components[kk].s &&
              sc.f_f0 ==
                  cmodel.components[kk]
                      .f_f0) {  // This component is a duplicate, so break
            kksv = kk;
            dupc = true;
            break;
          }
        ...
        for (int kk = 0; kk < ncomps;
          ...
          if (sc.s == cmodel.components[kk].s &&
              sc.vbar20 ==
                  cmodel.components[kk]
                      .vbar20) {  // This component is a duplicate, so break
            kksv = kk;
            dupc = true;
            break;
          }
  ...
  qDebug() << "SAVE:    ncomps" << ncomps << cmodel.components.size() << "ndupc"
           << ndupc << "nrunIDs" << runIDs.size();
  ...
  QString mdlaanl =
      "a" + QDateTime::currentDateTime().toString("yyMMddhhmm") + "_";
  ...
  QString msg1 =
      tr("An output combined model has been created. "
  ...
  if (mbox.clickedButton() ==
    ...
    while (getruni) {
      ...
      QString msg2 =
          tr("The default run ID for the output combined"
             " model is <br/><b>") +
          runID + "</b>.<br/><br/>" +
