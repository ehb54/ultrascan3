...
US_CombineModels::US_CombineModels() : US_Widgets() {
   ...
   connect(pb_prefilt, SIGNAL(clicked()), this, SLOT(select_filt()));
   connect(pb_add, SIGNAL(clicked()), this, SLOT(add_models()));
   connect(pb_reset, SIGNAL(clicked()), this, SLOT(reset()));
   connect(pb_close, SIGNAL(clicked()), this, SLOT(close()));
   connect(pb_help, SIGNAL(clicked()), this, SLOT(help()));
   connect(pb_save, SIGNAL(clicked()), this, SLOT(save()));

   lw_models->setToolTip(tr("List of models to combine for a global model"));
   pb_prefilt->setToolTip(tr("Choose RunIDs to pre-filter the models list"));
   le_prefilt->setToolTip(tr("Chosen RunID pre-filter to the models list"));
   pb_add->setToolTip(tr("Add to the list of component models"));
   pb_reset->setToolTip(tr("Clear the models list to allow re-selection"));
   pb_help->setToolTip(tr("Display detailed US_CombineModels documentation text and images"));
   pb_close->setToolTip(tr("Close the US_CombineModels window and exit"));
   pb_save->setToolTip(tr("Create a global model from the listed component models"));
...
void US_CombineModels::save() {
   ...
   for (int ii = 1; ii < nmodels; ii++) { // Add components from the remainder of the models
      ...
      for (int jj = 0; jj < kcomps; jj++) { // Add model components to the combo model
         bool dupc = false;
         US_Model::SimulationComponent sc = imodel->components[ jj ];
         double conc = sc.signal_concentration;
         int kksv = 0;

         if (cnstv) { // Test for duplicate where vbar20 is constant
            for (int kk = 0; kk < ncomps; kk++) { // See if this component already exists
               if (sc.s == cmodel.components[ kk ].s && sc.f_f0 == cmodel.components[ kk ].f_f0) { // This component is a duplicate, so break
                  kksv = kk;
                  dupc = true;
                  break;
               }
         ...
         else if (cnstk) { // Test for duplicate where f/f0 is constant
            for (int kk = 0; kk < ncomps; kk++) { // See if this component already exists
               if (
                  sc.s == cmodel.components[ kk ].s
                  && sc.vbar20 == cmodel.components[ kk ].vbar20) { // This component is a duplicate, so break
                  kksv = kk;
                  dupc = true;
                  break;
               }
         ...
         else { // Test for duplicate where neither f/f0 nor vbar20 is constant
            for (int kk = 0; kk < ncomps; kk++) { // See if this component already exists
               if (
                  sc.s == cmodel.components[ kk ].s && sc.f_f0 == cmodel.components[ kk ].f_f0
                  && sc.vbar20 == cmodel.components[ kk ].vbar20) { // This component is a duplicate, so break
                  kksv = kk;
                  dupc = true;
                  break;
         ...
         if (dupc) { // Update concentration sum if duplicate component
   ...
   qDebug() << "SAVE:    ncomps" << ncomps << cmodel.components.size() << "ndupc" << ndupc << "nrunIDs"
            << runIDs.size();
      ...
      while (getruni) {
         ...
         QString msg2 = tr("The default run ID for the output combined"
                        ...
                        + tr("You may modify this part of the model description"
                             ...
                             .arg(curriln)
                             .arg(mxtotrl);
         runID = QInputDialog::getText(
            this, tr("Modify Global Model Description RunID"), msg2, QLineEdit::Normal, runID, &ok);
