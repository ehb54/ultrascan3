...
US_Admin::US_Admin(QWidget *w, Qt::WindowFlags flags) : US_Widgets(true, w, flags) {
...
void US_Admin::save(void) {
   ...
   if (!oldPW.isEmpty()) {
      QByteArray calcsha1 = QCryptographicHash::hash(le_oldPasswd->text().toLatin1(), QCryptographicHash::Sha1);

      if (calcsha1 != oldPW) {
         QMessageBox::information(this, tr("Attention:"), tr("The old password is incorrect. Please re-input.\n"));
         return;
      }
   }
   ...
   if (le_passwd1->text() != le_passwd2->text()) {
      ...
      QMessageBox::information(this, tr("Attention:"), tr("The entered passwords are not same. Please re-input.\n"));
      return;
   }
   ...
   if (newPW.isEmpty()) {
      QMessageBox::information(this, tr("Attention:"), tr("The new password cannot be empty. Please re-input.\n"));
      return;
   }

   /// Use settings

   QByteArray sha1string = QCryptographicHash::hash(newPW.toLatin1(), QCryptographicHash::Sha1);
   ...
   if (le_oldPasswd != NULL)
      oldPass = le_oldPasswd->text();

   if (defaultDB.size() > 0) {
      // Decrypt with old password
      // 4 = cipher; 5 = initialization vector
      QString db_password = US_Crypto::decrypt(defaultDB.at(4), oldPass, defaultDB.at(5));
   ...
   for (int i = 0; i < databases.size(); i++) {
      ...
      QString db_password = US_Crypto::decrypt(database.at(4), oldPass, database.at(5));
