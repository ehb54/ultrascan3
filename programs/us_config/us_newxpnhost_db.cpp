...
US_NewXpnHostDB::US_NewXpnHostDB () : US_Widgets ()
{
  ...
  connect (cb_type, SIGNAL (activated (int)), this, SLOT (changeType (int)));
  ...
  connect (le_description, SIGNAL (textChanged (QString)), this,
           SLOT (desc_changed (QString)));
  ...
  connect (pb_loadchromo, SIGNAL (clicked ()), this, SLOT (load_chromo ()));
  ...
  connect (pb_testconn, SIGNAL (clicked ()), this,
           SLOT (test_connectivity ()));
...
US_NewXpnHostDB::US_NewXpnHostDB (QMap<QString, QString> currentInstrument)
    : US_Widgets ()
{
...
bool
US_NewXpnHostDB::test_connectivity (void)
{
...
void
US_NewXpnHostDB::changeType (int ndx)
{
...
// Load Chromo Array
void
US_NewXpnHostDB::load_chromo (void)
{
  QStringList files;
  QFile f;
...
void
US_NewXpnHostDB::readingChromoArrayFile (const QString &fileName)
{
  QString str1, str2;
  float temp_x, temp_y;
  QStringList strl;
  ...
  qDebug ()
      << "Upload Finished, corr_lambda.size(), corr_lambda_current.size(): "
      << corr_lambda.size () << ", " << corr_lambda_current.size ();
}

// Read Existing Chromatic Aberration Array from DB
void
US_NewXpnHostDB::readingChromoArrayDB ()
{
  corr_lambda.clear ();
  corr_value.clear ();
  ...
  foreach (QString str, strl)
    {
      ...
      corr_lambda.push_back (
          double (str.split (",")[0].trimmed ().toFloat ()));
...
void
US_NewXpnHostDB::shiftChromoArray (double val)
{
  ...
  for (int i = 0; i < corr_value.size (); ++i)
    {
      corr_value[i] -= shift;
      ChromoArrayList += " (" + QString::number (corr_lambda[i]) + ","
                         + QString::number (corr_value[i]) + ") ";
    }

  // //If no data from file was uploaded, generate zero array  -------> DO we
  // need this ?? if( ChromoArrayList.isEmpty() )
  //   {
  //     for ( int i=190; i<801; ++i )
  // 	ChromoArrayList += " (" + QString::number( i ) + "," + QString::number(
  // 0 ) + ") ";
  //   }
}
...
void
US_NewXpnHostDB::desc_changed (QString text)
{
...
// Save New Instrument
void
US_NewXpnHostDB::save_new (void)
{
  if (!nonOptima_selected)
    {
      if (le_name->text ().isEmpty () || le_description->text ().isEmpty ()
          || le_host->text ().isEmpty () || le_port->text ().isEmpty ()
          || le_msgPort->text ().isEmpty () || le_user->text ().isEmpty ()
          || le_pasw->text ().isEmpty ()
          || le_serialNumber->text ()
                 .isEmpty ()) //|| le_chromofile->text().isEmpty() )
        {
          QMessageBox::warning (this,
                                tr ("Please provide the missing information:"),
  ...
  if (!nonOptima_selected)
      ...
      if (!rx_desc.exactMatch (le_description->text ()))
        {
          ...
          QString message_error
              = tr ("Syntax error for Optima Host Description!\n\nThe "
                    "description template is the following:\n  'Optima #' "
                    "(Optima|space|number)");
    ...
    {
      if (le_description->text ().contains ("Optima")
          || le_description->text ().contains ("optima"))
        {
          QString mtitle_error = tr ("Error");
          QString message_error = tr (
              "Syntax error for Instrument Description: non-Optima type "
              "cannot include words 'Optima' or 'optima' in its description!");
          QMessageBox::critical (this, mtitle_error, message_error);
  ...
  for (QStringList::iterator it = instrumentNames.begin ();
       it != instrumentNames.end (); ++it)
    {
      QString current = *it;
      ...
      if (current == le_description->text () && !update_instrument)
        {
          QMessageBox::critical (
              this, tr ("Duplicate Optima Machine Name:"),
              QString (tr ("The name selected (%1) is currently used by other "
                           "machine! Please select different name.")
                           .arg (current)));
          return;
        }
    }
  ...
  foreach (int ID, instrumentIDs)
    {
      ...
      if (optimaHost == le_host->text ()
          && optimaPort == le_port->text ().toInt () && !nonOptima_selected)
        {
          QMessageBox::critical (
              this, tr ("Duplicate Optima Machine Connection Info:"),
              QString (tr ("Specified combination of the host (%1) and port "
                           "(%2) is currently used by other machine!  Please "
                           "edit host and/or port and re-test the connection")
                           .arg (optimaHost)
                           .arg (QString::number (optimaPort))));
