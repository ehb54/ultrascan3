...
void US_NewXpnHostDB::load_chromo(void) {
  ...
  dialog.setNameFilter(
      tr("Text files (*.[Tt][Xx][Tt] *.[Cc][Ss][Vv] *.[Dd][Aa][Tt]);;All files "
         "(*)"));
  ...
  if (corr_lambda.size() != 0) le_chromofile->setText("Uploaded");
}
  ...
  if (!fileName.isEmpty()) {
    ...
    if (f.open(QIODevice::ReadOnly | QIODevice::Text)) {
      ...
      while (!ts.atEnd()) {
        ...
        if (str1.isEmpty() ||
            str1.startsWith("#"))  // <---- add more conditions if needed
          continue;
...
void US_NewXpnHostDB::shiftChromoArray(double val) {
  ...
  for (int i = 0; i < corr_lambda.size(); ++i)
    if (corr_lambda[i] == val) shift = corr_value[i];
...
void US_NewXpnHostDB::save_new(void) {
  ...
  if (!nonOptima_selected) {
    ...
    if (le_description->text().contains("Optima") ||
      ...
      QString mtitle_error = tr("Error");
      QString message_error =
          tr("Syntax error for Instrument Description: non-Optima type cannot "
             "include words 'Optima' or 'optima' in its description!");
      QMessageBox::critical(this, mtitle_error, message_error);
  ...
  foreach (int ID, instrumentIDs) {
    if (ID == instrumentedit["ID"].toInt() && update_instrument) continue;
    ...
    if (optimaHost == le_host->text() &&
      ...
      QMessageBox::critical(
          ...
          QString(tr("Specified combination of the host (%1) and port (%2) is "
                     "currently used by other machine!  Please edit host "
                     "and/or port and re-test the connection")
                      .arg(optimaHost)
