...
US_NewXpnHostDB::US_NewXpnHostDB() : US_Widgets() {
   ...
   connect(le_description, SIGNAL(textChanged(QString)), this, SLOT(desc_changed(QString)));
...
void US_NewXpnHostDB::readingChromoArrayFile(const QString &fileName) {
   ...
   qDebug() << "Upload Finished, corr_lambda.size(), corr_lambda_current.size(): " << corr_lambda.size() << ", "
            << corr_lambda_current.size();
}
...
void US_NewXpnHostDB::save_new(void) {
   if (!nonOptima_selected) {
      if (
         le_name->text().isEmpty() || le_description->text().isEmpty() || le_host->text().isEmpty()
         || le_port->text().isEmpty() || le_msgPort->text().isEmpty() || le_user->text().isEmpty()
         || le_pasw->text().isEmpty() || le_serialNumber->text().isEmpty()) //|| le_chromofile->text().isEmpty() )
      {
         QMessageBox::warning(this, tr("Please provide the missing information:"), tr("Fill out all fields!"));
         return;
      }
   }
   else {
   ...
   if (!nonOptima_selected) {
      ...
      if (le_description->text().contains("Optima") || le_description->text().contains("optima")) {
         QString mtitle_error = tr("Error");
         QString message_error = tr("Syntax error for Instrument Description: non-Optima type cannot include words "
                                    "'Optima' or 'optima' in its description!");
         QMessageBox::critical(this, mtitle_error, message_error);
      ...
      if (optimaHost == le_host->text() && optimaPort == le_port->text().toInt() && !nonOptima_selected) {
         QMessageBox::critical(
            ...
            QString(tr("Specified combination of the host (%1) and port (%2) is currently used by other machine!  "
                       "Please edit host and/or port and re-test the connection")
                       .arg(optimaHost)
