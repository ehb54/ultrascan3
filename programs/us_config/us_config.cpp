    ...
    : US_Widgets (true, parent, flags)
{
  ...
  QLabel *banner
      = us_banner (tr ("UltraScan %1 Configuration").arg (US_Version));
  ...
  QFont *f
      = new QFont (US_GuiSettings::fontFamily (), US_GuiSettings::fontSize ());
  ...
  connect (le_workDir, SIGNAL (editingFinished ()), this,
           SLOT (update_workDir ()));
  ...
  connect (le_importDir, SIGNAL (editingFinished ()), this,
           SLOT (update_importDir ()));
  ...
  connect (le_tmpDir, SIGNAL (editingFinished ()), this,
           SLOT (update_tmpDir ()));
  ...
  /* For DB change - check autoflow records & disable with message if there are
   * any records */
...
void
US_Config::set_data_location (bool)
{
  if (disk_db_control->db ())
    {
...
void
US_Config::update_password (void)
{
...
void
US_Config::update_advanced (void)
{
...
void
US_Config::open_browser (void)
{
  QString browser = QFileDialog::getOpenFileName (
      this, tr ("Select desired browser application"), le_browser->text ());
...
void
US_Config::open_workDir (void)
{
  QString dir = QFileDialog::getExistingDirectory (
      this, tr ("Select desired base work directory"), le_workDir->text ());
...
void
US_Config::open_importDir (void)
{
  QString dir = QFileDialog::getExistingDirectory (
      this, tr ("Select desired imports directory"), le_importDir->text ());
...
void
US_Config::open_tmpDir (void)
{
  QString dir = QFileDialog::getExistingDirectory (
      this, tr ("Select desired temporary directory"), le_tmpDir->text ());
...
void
US_Config::update_importDir (void)
{
