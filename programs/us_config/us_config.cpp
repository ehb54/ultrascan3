...
US_Config::US_Config(QWidget* parent, Qt::WindowFlags flags)
  ...
  if (!disk_db_control->db()) pb_xpnh->setEnabled(false);
...
void US_Config::help(void) { showhelp.show_help("config.html"); }
...
void US_Config::update_workDir(void) {
  ...
  if (!chg_dimport) {  // If imports directory wasn't changed already, default
                       // using work base
    le_importDir->setText(work_dir + "/imports");
  }

  if (!chg_dtmp) {  // If tmp directory wasn't changed already, default using
                    // work base
