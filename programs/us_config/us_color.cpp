...
US_Color::US_Color(QWidget *w, Qt::WindowFlags flags) : US_Widgets(true, w, flags) {
   ...
   cmbb_style->setCurrentIndex(cmbb_style->findText(current.guiStyle, Qt::MatchFixedString));

   connect(cmbb_style, SIGNAL(activated(const QString &)), SLOT(selectStyle(const QString &)));
   ...
   connect(elements, SIGNAL(currentRowChanged(int)), this, SLOT(selectedElement(int)));
...
void US_Color::selMargin(int index) {
   ...
   plot->setStyleSheet(QString("QwtPlot{ padding: %1px }").arg(current.plotMargin));
}
...
void US_Color::save_as(void) {
   ...
   if (custom.size() == 0) {
      QMessageBox::warning(this, tr("Save scheme"), tr("A scheme name must be specified."));
...
void US_Color::delete_scheme(void) {
   int result = QMessageBox::warning(
      this, tr("Attention:"),
      tr("Do you really want to delete the\n\"%1\" color scheme?").arg(schemes->currentItem()->text()),
