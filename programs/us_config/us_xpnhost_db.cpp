    ...
    : US_Widgets (true, w, flags)
{
  ...
  if (currentHash.isEmpty ())
    {
      QMessageBox::information (this, tr ("No master password"),
  ...
  connect (pb_edit, SIGNAL (clicked ()), this, SLOT (editDB ()));
  ...
  connect (pb_cancel, SIGNAL (clicked ()), this, SLOT (close_program ()));
...
void
US_XpnHostDB::readInstruments (US_DB2 *db)
{
  ...
  if (db->lastErrno () != US_DB2::OK)
    {
      QMessageBox::warning (
          this, tr ("Database Connection Problem!"),
      ...
      for (int ii = 0; ii < instruments.size (); ii++)
        {
          ...
          // ALEXEY: do not add any "default" as we won't enable settign
          // default (from this program)
          //  if ( instruments[ii].selected )
          //    {  // Select the matching entry in the list and flag it as
          //    selected
          //      desc.append( " (default)" );
      ...
      if (item == instruments[ii].name)
        {
          ...
          if (instruments[ii].radcalwvl.isEmpty ()
              || instruments[ii].chromoab.isEmpty ())
...
// When selected, fill GUI
void
US_XpnHostDB::optima_selected (QListWidgetItem *tmp_item)
{
  if (lw_entries->currentRow () < 0)
    {
      reset ();
...
// Select instrument
void
US_XpnHostDB::select_db (QListWidgetItem *entry, const bool showmsg)
{
      ...
      if (item == instruments[ii].name)
        {
          ...
          // QString encpw    = instruments[ii].optimaDBpassw;    // Encrypted
          // password QString cipher   = encpw.section( "^", 0, 0 ); // Cipher
          // QString initve   = encpw.section( "^", 1, 1 ); // initVector
          // QString decpw    = US_Crypto::decrypt( cipher, masterpw, initve );
          // le_pasw       ->setText( decpw );              // Decrypted
          // password

          qDebug () << "SELECT: Passw.: decrypted " << le_pasw->text ();

          // cb_os1        ->setCurrentIndex( cb_os1->findText(
          // instruments[ii].os1 ) ); cb_os2        ->setCurrentIndex(
          // cb_os2->findText( instruments[ii].os2 ) ); cb_os3
          // ->setCurrentIndex( cb_os3->findText( instruments[ii].os3 ) );
          ...
          // ALEXEY: set in DB selected to '1' for this entry and reset
          // selected to '0' for all others...
          US_Passwd pw;
...
void
US_XpnHostDB::Instrument::reset (void)
{
  ID = -1;
  name = "";
  serial = "";
...
US_XpnHostDB::check_user_level ()
{
  ...
  if (US_Settings::us_inv_level () < 3)
    {
      QMessageBox::warning (this, tr ("Check User Level"),
                            tr ("Low user level!\n This user cannot"
...
US_XpnHostDB::add_new (void)
{
  ...
  connect (new_xpnhost_db, SIGNAL (accepted (QMap<QString, QString> &)), this,
           SLOT (newHost (QMap<QString, QString> &)));
  connect (new_xpnhost_db, SIGNAL (editnew_cancelled ()), this,
           SLOT (editnew_cancelled ()));
  new_xpnhost_db->show ();
}

// Edit Entry
void
US_XpnHostDB::editDB (void)
{
  if (check_user_level ())
    return;
      ...
      if (desc == item)
        {
          ...
          currentInstrument["msgPort"]
              = QString::number (instruments[ii].msgPort);
  ...
  connect (edit_xpnhost_db, SIGNAL (accepted (QMap<QString, QString> &)), this,
           SLOT (editHost (QMap<QString, QString> &)));
  connect (edit_xpnhost_db, SIGNAL (editnew_cancelled ()), this,
           SLOT (editnew_cancelled ()));
  edit_xpnhost_db->show ();
}

void
US_XpnHostDB::editHost (QMap<QString, QString> &newInstrument)
{
  US_Passwd pw;
  QString masterpw = pw.getPasswd ();
  ...
  QStringList q ("");
  q.clear ();
  q << QString ("update_instrument_new") << QString::number (instID_toedit)
    << newInstrument["name"] << newInstrument["serialNumber"]
    << newInstrument["labID"] << newInstrument["optimaHost"]
    << newInstrument["optimaPort"] << newInstrument["optimaDBname"]
    << newInstrument["optimaDBusername"] << newInstrument["optimaDBpassw"]
    << newInstrument["os1"] << newInstrument["os2"] << newInstrument["os3"]
    << newInstrument["radCalWvl"] << newInstrument["chromoArray"]
  ...
  QString message_done
      = tr ("Instrument information has been successfully edited.");
...
void
US_XpnHostDB::editnew_cancelled ()
{
  update_lw ();
}
...
void
US_XpnHostDB::fillGui (QMap<QString, QString> &instrument)
{
...
void
US_XpnHostDB::newHost (QMap<QString, QString> &newInstrument)
{
  currentInstrument.reset ();

  qDebug () << "New Instrument: " << newInstrument["name"] << ", "
  ...
  // ALEXEY: we better use internal MySQL encryption with the same 'secret' for
  // all users... Use of default password encryption is a problem as it uses
  // masterpassword for secret to ecrypt/decrypt OptimaDB password

  // QString decpw    = newInstrument[ "optimaDBpassw" ];            //
  // Decrypted password QStringList pwl  = US_Crypto::encrypt( decpw, masterpw
  ...
  QStringList q ("");
  q.clear ();
  q << QString ("add_instrument_new") << newInstrument["name"]
    << newInstrument["serialNumber"] << newInstrument["labID"]
    << newInstrument["optimaHost"] << newInstrument["optimaPort"]
    << newInstrument["optimaDBname"] << newInstrument["optimaDBusername"]
    << newInstrument["optimaDBpassw"] << newInstrument["os1"]
    << newInstrument["os2"] << newInstrument["os3"]
    << newInstrument["radCalWvl"] << newInstrument["chromoArray"]
    << newInstrument["msgPort"];
  ...
  QString mtitle_done = tr ("Success");
  QString message_done
      = tr ("New instrument has been successfully added to DB.");
  QMessageBox::information (this, mtitle_done, message_done);
...
// Test Connection
bool
US_XpnHostDB::test_connect (void)
{
  ...
  if (xpnhost.isEmpty () || xpnport.isEmpty () || dbuser.isEmpty ()
      || dbpasw.isEmpty ())
    {
      QMessageBox::warning (
          this, tr ("Missing Data"),
  ...
  bool ok = xpn_data->connect_data (xpnhost, ixpport, dbname, dbuser, dbpasw);
  ...
  if (ok)
    {
      QMessageBox::information (this, tr ("OptimaHost Connection"),
    ...
    {
      QMessageBox::warning (this, tr ("OptimaHost Connection"),
  ...
  if (instID)
    {
      ...
      QStringList q ("");
      q.clear ();
      q << QString ("delete_instrument") << QString::number (instID);
      ...
      if (status == US_DB2::INSTRUMENT_IN_USE)
        {
          QMessageBox::warning (
              this, tr ("Instrument Not Deleted"),
      ...
      // ALEXEY: now check if the default DB was removed: if yes, reset default
      // to the first in the item list
      if (default_to_remove)
        {
          ...
          update_lw ();
          msg += QString ("\n NOTE: the default DB was reset to %1")
                     .arg ("Optima 1");
        }

      // reset();
      QMessageBox::information (this, tr ("XpnHost Removed"), msg);
...
void
US_XpnHostDB::close_program (void)
{
