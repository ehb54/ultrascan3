...
US_XpnHostDB::US_XpnHostDB(QWidget *w, Qt::WindowFlags flags) : US_Widgets(true, w, flags) {
   // Frame layout
...
void US_XpnHostDB::readInstruments(US_DB2 *db) {
   ...
   QStringList q("");
   q.clear();
   q << QString("get_instrument_names") << QString::number(1); //ALEXEY '1' for the (only) labID
...
void US_XpnHostDB::update_lw() {
   ...
   if (instruments.size() > 0) {
      ...
      if (!def_sel && instruments.size() > 0) { // Insure that *something* is selected!
...
void US_XpnHostDB::select_db(QListWidgetItem *entry, const bool showmsg) {
   ...
   for (int ii = 0; ii < instruments.size(); ii++) {
      if (item == instruments[ ii ].name) {
         ...
         if (db->lastErrno() != US_DB2::OK) {
            QMessageBox::warning(
               this, tr("Database Connection Problem!"), tr("You are not currently connected to a US3-LIMS DB!\n"));
            return;
         }
         ...
         for (int jj = 0; jj < instruments.size(); jj++) {
            ...
            q.clear();
            q << QString("update_instrument_set_unselected") << instruments[ jj ].name;
...
void US_XpnHostDB::editDB(void) {
   ...
   if (!instID) {
      QMessageBox::warning(this, tr("XpnHost Problem"), tr("The description does not match any in the database list.\n"));
...
void US_XpnHostDB::editHost(QMap<QString, QString> &newInstrument) {
   ...
   QStringList q("");
   q.clear();
   q << QString("update_instrument_new") << QString::number(instID_toedit) << newInstrument[ "name" ]
     << newInstrument[ "serialNumber" ] << newInstrument[ "labID" ] << newInstrument[ "optimaHost" ]
     << newInstrument[ "optimaPort" ] << newInstrument[ "optimaDBname" ] << newInstrument[ "optimaDBusername" ]
     << newInstrument[ "optimaDBpassw" ] << newInstrument[ "os1" ] << newInstrument[ "os2" ] << newInstrument[ "os3" ]
     << newInstrument[ "radCalWvl" ] << newInstrument[ "chromoArray" ] << newInstrument[ "msgPort" ];
...
void US_XpnHostDB::newHost(QMap<QString, QString> &newInstrument) {
   ...
   QStringList q("");
   q.clear();
   q << QString("add_instrument_new") << newInstrument[ "name" ] << newInstrument[ "serialNumber" ]
     << newInstrument[ "labID" ] << newInstrument[ "optimaHost" ] << newInstrument[ "optimaPort" ]
     << newInstrument[ "optimaDBname" ] << newInstrument[ "optimaDBusername" ] << newInstrument[ "optimaDBpassw" ]
     << newInstrument[ "os1" ] << newInstrument[ "os2" ] << newInstrument[ "os3" ] << newInstrument[ "radCalWvl" ]
     << newInstrument[ "chromoArray" ] << newInstrument[ "msgPort" ];
