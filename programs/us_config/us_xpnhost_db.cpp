...
US_XpnHostDB::US_XpnHostDB(QWidget* w, Qt::WindowFlags flags)
  ...
  if (instruments.size() > 0) update_lw();
  ...
  connect(lw_entries, SIGNAL(itemSelectionChanged()), SLOT(optima_selected()));
...
void US_XpnHostDB::update_lw() {
  ...
  if (instruments.size() > 0) {
    for (int ii = 0; ii < instruments.size(); ii++) {
      ...
      // ALEXEY: do not add any "default" as we won't enable settign default
      // (from this program)
    ...
    if (!def_sel &&
        instruments.size() > 0) {  // Insure that *something* is selected!
...
void US_XpnHostDB::select_db(QListWidgetItem* entry, const bool showmsg) {
  ...
  for (int ii = 0; ii < instruments.size(); ii++) {
    if (item == instruments[ii].name) {
      ...
      // instruments[ii].os2 ) ); cb_os3        ->setCurrentIndex(
      // cb_os3->findText( instruments[ii].os3 ) );
      ...
      // ALEXEY: set in DB selected to '1' for this entry and reset selected to
      // '0' for all others...
      US_Passwd pw;
      ...
      QStringList q("");
      q.clear();
      q << QString("update_instrument_set_selected") << instruments[ii].name;
      ...
      for (int jj = 0; jj < instruments.size(); jj++) {
        if (instruments[jj].name == item) continue;
...
void US_XpnHostDB::editnew_cancelled() { update_lw(); }
...
void US_XpnHostDB::deleteDB(void) {
  ...
  if (entry->text().contains("(default)")) default_to_remove = true;
  ...
  if (instID) {
    ...
    if (status == US_DB2::INSTRUMENT_IN_USE) {
      QMessageBox::warning(this, tr("Instrument Not Deleted"),
    ...
    // ALEXEY: now check if the default DB was removed: if yes, reset default to
    // the first in the item list
    if (default_to_remove) {
      ...
      QStringList q("");
      q.clear();
      q << QString("update_instrument_set_selected") << currentItem->text();
...
void US_XpnHostDB::close_program(void) { close(); }
