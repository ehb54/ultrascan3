...
US_XpnHost::US_XpnHost(QWidget *w, Qt::WindowFlags flags) : US_Widgets(true, w, flags) {
   // Frame layout
...
void US_XpnHost::select_db(QListWidgetItem *entry, const bool showmsg) {
   ...
   for (int ii = 0; ii < dblist.size(); ii++) {
      if (item == dblist.at(ii).at(0)) {
         ...
         if (elen < 9) { // Old entry has from 0 to 8 strings: use what you can
            QStringList dbentr;
            dbentr << ((elen > 0) ? dblist.at(ii).at(0) : le_description->text())
                   << ((elen > 1) ? dblist.at(ii).at(1) : le_host->text())
                   << ((elen > 2) ? dblist.at(ii).at(2) : le_port->text())
                   << ((elen > 3) ? dblist.at(ii).at(3) : le_name->text())
                   << ((elen > 4) ? dblist.at(ii).at(4) : le_user->text()) << ((elen > 5) ? dblist.at(ii).at(5) : encpw)
                   << ((elen > 6) ? dblist.at(ii).at(6) : cb_os1->currentText())
                   << ((elen > 7) ? dblist.at(ii).at(7) : cb_os2->currentText()) << cb_os3->currentText();
         ...
         cb_os1->setCurrentIndex(cb_os1->findText(dblist.at(ii).at(6)));
         cb_os2->setCurrentIndex(cb_os2->findText(dblist.at(ii).at(7)));
         cb_os3->setCurrentIndex(cb_os3->findText(dblist.at(ii).at(8)));
         ...
         if (showmsg) {
            QMessageBox::information(this, tr("Optima Host Selected"), tr("The default database host has been updated."));
         }
...
void US_XpnHost::check_add() {
   ...
   if (le_description->text().isEmpty()) {
      QMessageBox::information(this, tr("Attention"), tr("Please enter a Description before saving..."));
      return;
   }

   if (le_host->text().isEmpty()) {
      QMessageBox::information(this, tr("Attention"), tr("Please enter a Optima DB Host Address before saving..."));
      return;
   }
   ...
   if (!updated) {
      QStringList entry;
      entry << le_description->text() << le_host->text() << le_port->text() << le_name->text() << le_user->text()
            << encpw << cb_os1->currentText() << cb_os2->currentText() << cb_os3->currentText();
...
void US_XpnHost::update_lw(const QString &current) {
   ...
   if (dblist.size() == 0) { // First-time:  create an initial default entry
      QStringList entry;
      entry << def_desc << def_host << def_port << def_name << def_user << def_pasw << "UV/visible"
            << "Rayleigh Interference"
            << "(not installed)";

      dblist << entry;

      if (defaultDB.size() == 0) { // If we have no default, set the dummy entry as such
         defaultDB = entry;
      }
   }
   else { // Insure default entry has encrypted password
      if (defaultDB[ 5 ].length() < 20) {
         QStringList entry;
         entry << defaultDB[ 0 ] << defaultDB[ 1 ] << defaultDB[ 2 ] << defaultDB[ 3 ] << defaultDB[ 4 ]
               << dblist.at(0)[ 5 ] << defaultDB[ 6 ] << defaultDB[ 7 ] << defaultDB[ 8 ];
         defaultDB = entry;
   ...
   for (int ii = 0; ii < dblist.size(); ii++) { // Search entries for a match to the default
      ...
      if (desc == current) { // Select the matching entry in the list and flag it as selected
         lw_entries->setCurrentItem(widget, QItemSelectionModel::Select);
         def_sel = true; // Flag a default selection made
      }
   }

   if (!def_sel && dblist.size() > 0) { // Insure that *something* is selected!
...
void US_XpnHost::reset(void) {
   ...
   QString defaultDB;
   if (DB.size() > 0)
      defaultDB = US_Settings::defaultXpnHost().at(0);
   update_lw(defaultDB);
      ...
      if (desc == le_description->text()) {
         if (conn_stat[ desc ] != "connect_ok") {
            QMessageBox::information(
               this, tr("Default OptimaHost Problem"),
         ...
         QMessageBox::information(this, tr("Default OptimaHost"), tr("The default database has been updated."));
         return;
      }
   }
...
void US_XpnHost::deleteDB(void) {
   ...
   for (int ii = 0; ii < dblist.size(); ii++) {
      ...
      if (desc == item) {
         ...
         QMessageBox::information(this, tr("XpnHost Removed"), tr("The database has been removed."));
         return;
      }
   }
...
bool US_XpnHost::test_connect(void) {
   ...
   if (xpnhost.isEmpty() || xpnport.isEmpty() || dbuser.isEmpty() || dbpasw.isEmpty()) {
   ...
   if (ok) {
      QMessageBox::information(this, tr("OptimaHost Connection"), tr("The connection was successful."));

      pb_save->setEnabled(true);
      conn_stat[ xpndesc ] = "connect_ok";
   }
   else {
      QMessageBox::warning(this, tr("OptimaHost Connection"), tr("The connection failed.\n") + xpn_data->lastError());
