...
US_XpnHost::US_XpnHost(QWidget* w, Qt::WindowFlags flags)
  ...
  QLabel* banner2 =
      us_banner(tr("(Doubleclick for details and set the default)"), -1);
  ...
  QString defXpnHost;
  if (XpnHosts.size() > 0) defXpnHost = US_Settings::defaultXpnHost().at(0);
  ...
  if (dblist.size() == 0) update_lw(tr("place-holder"));
    ...
    if (item == dblist.at(ii).at(0)) {
      ...
      if (elen < 9) {  // Old entry has from 0 to 8 strings: use what you can
        ...
        dbentr << ((elen > 0) ? dblist.at(ii).at(0) : le_description->text())
               ...
               << ((elen > 4) ? dblist.at(ii).at(4) : le_user->text())
               << ((elen > 5) ? dblist.at(ii).at(5) : encpw)
               << ((elen > 6) ? dblist.at(ii).at(6) : cb_os1->currentText())
               << ((elen > 7) ? dblist.at(ii).at(7) : cb_os2->currentText())
               << cb_os3->currentText();
...
void US_XpnHost::check_add() {
  ...
  if (le_port->text().isEmpty()) le_port->setText(def_port);

  if (le_name->text().isEmpty()) le_name->setText(def_name);
  ...
  if (!updated) {
    QStringList entry;
    entry << le_description->text() << le_host->text() << le_port->text()
          << le_name->text() << le_user->text() << encpw
          << cb_os1->currentText() << cb_os2->currentText()
          << cb_os3->currentText();
...
void US_XpnHost::update_lw(const QString& current) {
  ...
  if (dblist.size() == 0) {  // First-time:  create an initial default entry
    QStringList entry;
    entry << def_desc << def_host << def_port << def_name << def_user
          << def_pasw << "UV/visible"
          << "Rayleigh Interference"
          << "(not installed)";
    ...
    if (defaultDB.size() ==
        0) {  // If we have no default, set the dummy entry as such
      defaultDB = entry;
    }
  } else {  // Insure default entry has encrypted password
    if (defaultDB[5].length() < 20) {
      QStringList entry;
      entry << defaultDB[0] << defaultDB[1] << defaultDB[2] << defaultDB[3]
            << defaultDB[4] << dblist.at(0)[5] << defaultDB[6] << defaultDB[7]
            << defaultDB[8];
      defaultDB = entry;
  ...
  for (int ii = 0; ii < dblist.size();
       ii++) {  // Search entries for a match to the default
    ...
    if (desc == current) {  // Select the matching entry in the list and flag it
                            // as selected
...
void US_XpnHost::save_default(void) {
  for (int ii = 0; ii < dblist.size(); ii++) {
    ...
    if (desc == le_description->text()) {
      ...
      QMessageBox::information(this, tr("Default OptimaHost"),
...
void US_XpnHost::deleteDB(void) {
  ...
  for (int ii = 0; ii < dblist.size(); ii++) {
    ...
    if (desc == item) {
      ...
      QMessageBox::information(this, tr("XpnHost Removed"),
