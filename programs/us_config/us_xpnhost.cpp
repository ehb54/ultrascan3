...
void
US_XpnHost::select_db (QListWidgetItem *entry, const bool showmsg)
{
...
void
US_XpnHost::update_lw (const QString &current)
{
  lw_entries->clear ();
      ...
      if (defaultDB[5].length () < 20)
        {
          ...
          entry << defaultDB[0] << defaultDB[1] << defaultDB[2] << defaultDB[3]
                << defaultDB[4] << dblist.at (0)[5] << defaultDB[6]
                << defaultDB[7] << defaultDB[8];
          defaultDB = entry;
