...
US_Database::US_Database(QWidget* w, Qt::WindowFlags flags)
  ...
  if (instances.count()) {
    QMessageBox::information(
        ...
        tr("Database preferences cannot be currently changed since there are "
           "Data Acquisition related processes ongoing. \n\nTo change DB "
           "preferences, Data Acquisiiton programs must be closed."));
    close();
  }
...
void US_Database::select_db(QListWidgetItem* entry) {
  ...
  for (int i = 0; i < dblist.size(); i++) {
    if (item == dblist.at(i).at(0)) {
      ...
      QMessageBox::information(this, tr("Database Selected"),
...
QString US_Database::validate_value(QLineEdit* line_edit,
...
void US_Database::check_add() {
  ...
  if (!updated) {
    ...
    entry << le_description->text() << le_username->text() << le_dbname->text()
          << le_host->text() << pw_list.at(0) << pw_list.at(1)
          << le_investigator_email->text() << inv_pw.at(0) << inv_pw.at(1)
          << uuid;
...
void US_Database::save_default() {
  for (const auto& i : dblist) {
    ...
    if (desc == le_description->text()) {
      ...
      QMessageBox::information(this, tr("Default Database"),
                               tr("The default database has been updated."));
      return;
    }
  }
...
bool US_Database::test_connect() {
  ...
  bool ok = db.test_secure_connection(le_host->text(), le_dbname->text(),
