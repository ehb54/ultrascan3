  ...
  if (instances.count ())
    {
      QMessageBox::information (
          ...
          tr ("Database preferences cannot be currently changed since there "
              "are Data Acquisition related processes ongoing. \n\nTo change "
              "DB preferences, Data Acquisiiton programs must be closed."));
      close ();
    }
...
int
US_Database::get_autoflow_records ()
{
  // Check DB connection
  US_Passwd pw;
...
void
US_Database::select_db (QListWidgetItem *entry)
{
      ...
      if (item == dblist.at (i).at (0))
        {
          ...
          //  << dblist.at(i).at(0) << dblist.at(i).at(6) <<
          //  dblist.at(i).at(9);
...
QString
US_Database::validate_value (QLineEdit *line_edit, const QString &property)
{
...
US_Database::check_add ()
{
  ...
  if (!updated)
    {
      QStringList entry;
      entry << le_description->text () << le_username->text ()
            << le_dbname->text () << le_host->text () << pw_list.at (0)
            << pw_list.at (1) << le_investigator_email->text ()
            << inv_pw.at (0) << inv_pw.at (1) << uuid;
...
void
US_Database::update_lw (const QString &current)
{
  lw_entries->clear ();
  ...
  for (const auto &i : dblist)
    {
      ...
      // qDebug() << "USCFG: UpdLw:   i desc email" << i << desc <<
      // dblist.at(i).at(6);
...
bool
US_Database::parse_database_url (const QString &database_url)
{
  ...
  // "mysql://{$username}:{$password}@{$host}/{$dbname}|{$_SESSION['email']}"
  ...
  if (le_description->text ().trimmed ().isEmpty () && !host.isEmpty ()
      && !dbname.isEmpty ())
    {
      ...
      if (desc == item)
        {
          ...
          QMessageBox::information (this, tr ("Database Removed"),
...
void
US_Database::values_updated () const
{
  QStringList problems;
...
bool
US_Database::test_connect ()
{
  if (le_host->text ().isEmpty () || le_dbname->text ().isEmpty ()
      || le_username->text ().isEmpty () || le_password->text ().isEmpty ()
  ...
  bool ok = db.test_secure_connection (
      le_host->text (), le_dbname->text (), le_username->text (),
      le_password->text (), le_investigator_email->text (),
  ...
  if (ok)
    {
      ...
      QMessageBox::information (this, tr ("Database Connection"),
  ...
  else
    {
      ...
      QMessageBox::warning (this, tr ("Database Connection"),
