...
class US_LegacyConverter : public US_Widgets {
 ...
 private:
  ...
  QHash<int, QHash<QString, QHash<QString, int> > >
      data_map;  //!< Map of data index (speed -> type -> CCW -> index)
  ...
  US_LineEdit_RE*
      le_runid;  //!< Line edit for run ID with regular expression validation
 ...
 private slots:
  ...
  //! \brief Slot to update text edit when a file extracted from the archive
  //! file
  //! \param relative path
  //! \param absolute path
