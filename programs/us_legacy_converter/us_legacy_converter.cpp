...
void US_LegacyConverter::save_auc() {
  ...
  foreach (int speed, speed_list) {
    foreach (QString rtype, data_map.value(speed).keys()) {
      ...
      int state = US_Convert::saveToDisk(data, triples, excludes, rtype, runid,
                                         path, false);
...
bool US_LegacyConverter::sort_files(const QStringList& flist,
                                    const QString& path) {
  QRegularExpression re;
  re.setPatternOptions(QRegularExpression::CaseInsensitiveOption);
  QRegularExpressionMatch match;
  ...
  // group5      (?:-w(\\d{3})-r_)?  =  wavelength (optional: some files include
  // it) group6                 -n(\\d)  =  scan repetition group7
  // (ra|ri|ip|fi|wa|wi)\\d  =  run type
  QString pattern =
      "^(.+)-s(\\d{4,6})-c(\\d)-s(\\d{4,6})(?:-w(\\d{3})-r_)?-n(\\d)[.](ra|ri|"
      "ip|fi|wa|wi)\\d$";
  ...
  foreach (QString fpath, flist) {
    ...
    if (match.hasMatch()) {
      ...
      QString key1 =
          tr("%1-%2-%3-%4-%5").arg(speed_id).arg(runtype, cell, wavl, replica);
  ...
  while (it.hasNext()) {
    ...
    for (int ii = 0; ii < scans.size(); ii++) {
      ...
      QString fname2 =
          tr("%1").arg(ii + 1).rightJustified(6, '0') + fpath1.right(4);
...
bool US_LegacyConverter::read_beckman_files(const QString& path,
                                            QString& status) {
  QDir dir(path);
  QStringList subdirs =
  ...
  if (!replicates.isEmpty()) {
    ...
    foreach (QString key, replicates) {
      ...
      details +=
          tr("Speed: %1, Type: %2, Triple: %3, Number of Replicates: %4\n")
              .arg(speed)
              .arg(runtype, ccw)
              .arg(n_replicas);
      for (int ss = 0; ss < nscans; ss++) {
    ...
    msgBox.setText(
        "Your experiment contains replicates. "
...
void US_LegacyConverter::itemExtracted(const QString& relative,
                                       const QString&) {
