...
US_LegacyConverter::US_LegacyConverter () : US_Widgets ()
{
  ...
  connect (le_runid, &US_LineEdit_RE::textUpdated, this,
           &US_LegacyConverter::runid_updated);
  connect (le_dir, &QLineEdit::textChanged, this,
           &US_LegacyConverter::runid_updated);
  connect (pb_save, &QPushButton::clicked, this,
           &US_LegacyConverter::save_auc);
  connect (archive, &US_Archive::itemExtracted, this,
           &US_LegacyConverter::itemExtracted);
}
...
US_LegacyConverter::save_auc ()
{
  ...
  if (exists)
    {
      int state = QMessageBox::question (this, "Warning!",
      ...
      foreach (QString rtype, data_map.value (speed).keys ())
        {
          ...
          int state = US_Convert::saveToDisk (data, triples, excludes, rtype,
                                              runid, path, false);
  ...
  if (!msg.isEmpty ())
    {
      QMessageBox::information (this, "Data Stored!",
                                "Run(s) Successfully Saved!\n\n" + msg);
    }
  pb_save->setEnabled (true);
}

void
US_LegacyConverter::reset (void)
{
  le_runid->clear ();
  lb_runid->setText ("Run ID:");
  le_runid->setStyleSheet ("color: black;");
...
void
US_LegacyConverter::load ()
{

  QString ext_str = "tar.gz Files ( *.tar.gz )";
  QString tar_fpath = QFileDialog::getOpenFileName (
      this, tr ("Beckman Optima tar.gz File"), QDir::homePath (), ext_str);
  ...
  if (tmp_dir.isValid ())
    ...
    {
      QMessageBox::warning (this, "Error!",
                            tr ("FAILED to Create a Temporary Directory!"));
      tar_fpath.clear ();
      qApp->restoreOverrideCursor ();
      pb_load->setEnabled (true);
      return;
...
void
US_LegacyConverter::list_files (const QString &path, QStringList &flist)
{
  QDir dir (path);
...
bool
US_LegacyConverter::sort_files (const QStringList &flist, const QString &path)
{
  QRegularExpression re;
  re.setPatternOptions (QRegularExpression::CaseInsensitiveOption);
  QRegularExpressionMatch match;
  ...
  // group5      (?:-w(\\d{3})-r_)?  =  wavelength (optional: some files
  // include it) group6                 -n(\\d)  =  scan repetition group7
  // (ra|ri|ip|fi|wa|wi)\\d  =  run type
  QString pattern = "^(.+)-s(\\d{4,6})-c(\\d)-s(\\d{4,6})(?:-w(\\d{3})-r_)?-n("
                    "\\d)[.](ra|ri|ip|fi|wa|wi)\\d$";
  re.setPattern (pattern);

  QString runid;
  QMap<QString, QVector<int> >
      scan_list; // speed-type-cell-wavelength-replica      -> scans
  QMap<QString, QString>
      fpath_list; // speed-type-cell-wavelength-replica-scan -> filepath
      ...
      if (match.hasMatch ())
        {
          ...
          QString key1 = tr ("%1-%2-%3-%4-%5")
                             .arg (speed_id)
                             .arg (runtype, cell, wavl, replica);
          ...
          if (fpath_list.contains (key2))
            {
              QMessageBox::warning (
                  this, "Error!",
                  tr ("Some of scans are redundant!\n\n%1").arg (fname));
              return false;
            }
          else
            {
      ...
      for (int ii = 0; ii < scans.size (); ii++)
        {
          ...
          QString fname2 = tr ("%1").arg (ii + 1).rightJustified (6, '0')
                           + fpath1.right (4);
...
bool
US_LegacyConverter::read_beckman_files (const QString &path, QString &status)
{
  QDir dir (path);
  QStringList subdirs
  ...
  foreach (QString path, subdirs)
    {
      ...
      QString runtype;
      US_Convert::readLegacyData (dir.absoluteFilePath (path), rawscan,
                                  runtype);
      if (rawscan.size () == 0)
        {
          continue;
      ...
      US_Convert::convertLegacyData (rawscan, rdata_list, triple_list, runtype,
                                     0.5);
      ...
      QDir subd
          = QDir (dir.absoluteFilePath (path), "*", QDir::Name, QDir::Files);
      QString log_1;
      log_1 += tr ("Speed : %1\n").arg (speed);
      ...
      QString msg (
          "Speed : %1, Run type : %2 (%3), Number of the processed files: %4");
      te_info->append (msg.arg (speed)
                           .arg (data_types.value (runtype), runtype)
                           .arg (subd.count ()));
      te_info->moveCursor (QTextCursor::End);
      qApp->processEvents ();
                  ...
                  if (data_map.value (speed).value (runtype).contains (ccw))
                    {
                      replicated = true;
                      int idx
                          = data_map.value (speed).value (runtype).value (ccw);
                          ...
                          for (int ss = 0; ss < nscans; ss++)
                            {
                              for (int pp = 0; pp < npoints; pp++)
                                {
                                  double val1
                                      = all_data[idx].rdata.value (ss, pp);
                                  double val2 = rdata.value (ss, pp);
                                  all_data[idx].rdata.setValue (ss, pp,
                                                                val1 + val2);
                                }
                            }
                          all_data[idx].n_replicates++;
                          QString rr
                              = tr ("%1-%2-%3").arg (speed).arg (runtype, ccw);
      ...
      foreach (QString key, replicates)
        {
          ...
          details += tr ("Speed: %1, Type: %2, Triple: %3, Number of "
                         "Replicates: %4\n")
                         .arg (speed)
                         .arg (runtype, ccw)
                         .arg (n_replicas);
...
void
US_LegacyConverter::itemExtracted (const QString &relative, const QString &)
{
