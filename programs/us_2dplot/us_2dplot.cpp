...
US_2dPlot::US_2dPlot () : US_Widgets ()
{
  ...
  connect (ct_par1, SIGNAL (valueChanged (double)), this,
           SLOT (update_par1 (double)));
  ...
  connect (ct_par2, SIGNAL (valueChanged (double)), this,
           SLOT (update_par2 (double)));
  ...
  connect (ct_par3, SIGNAL (valueChanged (double)), this,
           SLOT (update_par3 (double)));
  ...
  connect (ct_par4, SIGNAL (valueChanged (double)), this,
           SLOT (update_par4 (double)));
  ...
  connect (ct_resolution, SIGNAL (valueChanged (double)), this,
           SLOT (update_resolution (double)));
  ...
  connect (ct_model, SIGNAL (valueChanged (double)), this,
           SLOT (update_model (double)));
  top->addWidget (ct_model, row++, 0);

  // Plot layout on right side of window
  plot = new US_Plot (data_plot, tr ("Function Plot"),
        ...
        for (int i = 0; i < resolution; i++)
          {
            y[i] = f_min
                   + (f_max - f_min)
                         * (0.5 * erf ((x[i] - par2) / sqrt (2.0 * par1))
                            + 0.5);
          }
        break;
      }
...
void
US_2dPlot::update_par1 (double val)
{
  par1 = val;
  calculate ();
}
...
void
US_2dPlot::update_par2 (double val)
{
  par2 = val;
  calculate ();
}
...
void
US_2dPlot::update_par3 (double val)
{
  par3 = val;
  calculate ();
}
...
void
US_2dPlot::update_par4 (double val)
{
  par4 = val;
  calculate ();
}
...
void
US_2dPlot::update_model (double val)
{
  model = (int)val;
  switch (model)
    {
...
void
US_2dPlot::update_resolution (double val)
{
  resolution = val;
}
