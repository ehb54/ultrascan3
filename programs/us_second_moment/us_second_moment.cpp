...
void
US_SecondMoment::data_plot (void)
{
  ...
  if (count > 0)
    {
      curve = us_curve (data_plot1,
                        tr ("Non-cleared Sedimentation Coefficients"));
      xfsc = (int)x[0];
  ...
  if (count > 0)
    {
      ...
      qDebug () << "2ndm: cl: count" << count << "xfsc" << xfsc << "exclude"
                << exclude << "exSc k" << excludedScans.count ();
...
void
US_SecondMoment::write_report (QTextStream &ts)
{
  int index = lw_triples->currentRow ();
