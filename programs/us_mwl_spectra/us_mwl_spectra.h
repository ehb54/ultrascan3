...
class US_MwlSpectra : public US_Widgets {
 ...
 private slots:
  ...
  void sum_check(void);
  void save_data(void);
  void bld_stats(double, double, double, QVector<int>&, QVector<double>&,
                 QVector<double>&, QVector<double>&, QVector<double>&);
  void final_stats(QVector<int>&, QVector<double>&, QVector<double>&,
                   QVector<double>&, QVector<double>&);
  void changedPlotX(bool);
  double comp_value(const US_Model::SimulationComponent*, int);
  void help(void) { showHelp.show_help("mwl_spectra.html"); };
};
#endif
