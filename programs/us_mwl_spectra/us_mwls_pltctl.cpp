...
US_MwlSPlotControl::US_MwlSPlotControl(QWidget *p, QVector<QVector3D> *d) : US_WidgetsDialog(0, 0) {
   qDebug() << "PCtrl: IN";
   wparent = p;
   xyzdat = d;
   qDebug() << "PCtrl: xyzdat count" << xyzdat->count();

   setObjectName("US_MwlSPlotControl");
   ...
   QLayout *lo_yrevrs = us_checkbox(tr("Reverse Y"), ck_yrevrs, true);
...
US_Plot3D *US_MwlSPlotControl::widget_3dplot(void) {
   return plot3d_w;
}
...
void US_MwlSPlotControl::close_all() {
