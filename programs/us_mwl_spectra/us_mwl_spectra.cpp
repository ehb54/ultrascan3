...
US_MwlSpectra::US_MwlSpectra () : US_Widgets ()
{
  ...
  connect (cb_lend, SIGNAL (currentIndexChanged (int)), this,
           SLOT (changeLambda ()));
  connect (ct_smooth, SIGNAL (valueChanged (double)), this,
           SLOT (changeSmooth ()));
...
void
US_MwlSpectra::enableControls (void)
{
  if (mdlxyz.count () == 0)
...
US_MwlSpectra::load_distro ()
{
  ...
  connect (&lddiag, SIGNAL (changed (bool)), this,
           SLOT (update_disk_db (bool)));
          ...
          if (nconcs > 0)
            { // Normalize concentration by dividing sum by number of models
              ...
              if (nconcs > 5 || conc > 1000.0)
                DbgLv (1) << "LD: **nc_max cn_max" << nc_max << cn_max
                          << "nconcs conc" << nconcs << conc << "jj,ii" << jj
...
void
US_MwlSpectra::load_distro (const US_Model model, const QString mdescr)
{
...
void
US_MwlSpectra::plot_current (void)
{
  if (mdlxyz.count () == 0)
    return;
...
void
US_MwlSpectra::plot_titles (void)
{
...
void
US_MwlSpectra::changeRecord (void)
{
...
void
US_MwlSpectra::connect_ranges (bool conn)
{
  if (conn)
...
int
US_MwlSpectra::dvec_index (QVector<double> &dvec, const double dval)
{
...
bool
US_MwlSpectra::dvirt_equal (const double d1, const double d2)
{
...
void
US_MwlSpectra::update_disk_db (bool isDB)
{
  if (isDB)
    dkdb_cntrls->set_db ();
  else
...
void
US_MwlSpectra::bld_stats (double wlnv, double sedv, double conv,
...
void
US_MwlSpectra::final_stats (QVector<int> &istats, QVector<double> &dstats,
                            QVector<double> &lwlns, QVector<double> &lseds,
                            QVector<double> &lcons)
{
  ...
  for (int ii = 0; ii < nwavln; ii++)
    {
      ts << dquote + QString ().sprintf ("%d", (int)rr[ii]) + dquote + comma
                + dquote + QString ().sprintf ("%10.4e", vv[ii]) + dquote
                + endln;
    }
...
double
US_MwlSpectra::comp_value (const US_Model::SimulationComponent *sc, int type)
{
  double xval = sc->s;
...
void
US_MwlSpectra::changedPlotX (bool on_state)
{
  if (!on_state)
    return;
