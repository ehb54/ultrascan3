...
US_MwlSpectra::US_MwlSpectra() : US_Widgets() {
  ...
  QLabel* lb_smooth =
      us_label(tr("%1 Gaussian Smooth Points:").arg(chlamb), -1);
  ...
  QLayout* lo_srngsum =
      us_checkbox(tr("X Range Sum").arg(xaxis), ck_srngsum, false);
  ...
  connect(cb_sstart, SIGNAL(currentIndexChanged(int)), this,
          SLOT(changeSedcoeff()));
  connect(cb_send, SIGNAL(currentIndexChanged(int)), this,
          SLOT(changeSedcoeff()));
...
void US_MwlSpectra::enableControls(void) {
  ...
  for (int jj = 0; jj < nsedcos; jj++) slscos << QString::number(sedcoes[jj]);
...
void US_MwlSpectra::select_prefilt() {
  ...
  if (srdiag.exec() != QDialog::Accepted) pfilts.clear();
}
...
void US_MwlSpectra::load_distro() {
  ...
  if (lddiag.exec() != QDialog::Accepted) return;  // No selection made
  ...
  DbgLv(1) << "LD: nlambda" << nlambda << "nsedcos" << nsedcos << "nipoint"
           << nipoint;
  nc_max = 0;
  cn_max = 0.0;
    ...
    for (int ii = 0; ii < nlambda; ii++) {
      ...
      for (int kk = 0; kk < nipoint; kk++) {
        if (kk < 2 || (kk + 3) > nipoint)
          DbgLv(1) << "LD:      kk" << kk << "X,Y" << mdlxyz[kk].x()
                   << mdlxyz[kk].y();
      ...
      if (nconcs >
          0) {  // Normalize concentration by dividing sum by number of models
        conc = csum * scalen;
        ...
        if (nconcs > 5 || conc > 1000.0)
          DbgLv(1) << "LD: **nc_max cn_max" << nc_max << cn_max << "nconcs conc"
                   << nconcs << conc << "jj,ii" << jj << ii << "s wl" << sedco
                   << waveln;
      }
...
void US_MwlSpectra::load_distro(const US_Model model, const QString mdescr) {
  ...
  for (int jj = 0; jj < model.components.size(); jj++) {
    ...
    if (!sedcoes.contains(xc)) sedcoes << xc;
  }
}
...
void US_MwlSpectra::runDetails(void) {
  ...
  QString msg =
      tr("Multi-Wavelength Statistics for RunID %1 --\n\n").arg(runID);
...
void US_MwlSpectra::plot_all(void) {
  ...
  if (last_xmin <
      0.0) {  // If first time, set scales based on actual values present
...
void US_MwlSpectra::compute_ranges() {
  ...
  if (sedxp < 0 || recx < 0) {
    DbgLv(1) << "cmpR:  (1)sS sE sxS sxE" << sed_start << sed_end << sedxs
             << sedxe << "sxP rx sP" << sedxp << recx << sed_plot;
  ...
  for (int ii = lmbxs; ii < lmbxe; ii++) pltxvals << (double)lambdas[ii];

  if (!ck_srngsum->isChecked()) {  // Get concentrations from current sedcoeff
                                   // record, current lambda range
    for (int ii = lmbxs; ii < lmbxe; ii++) pltyvals << concdat[sedxp][ii];
  }
...
void US_MwlSpectra::plot_3d() {
  ...
  for (int jj = sedxs; jj < sedxe; jj++) {
    ...
    for (int ii = lmbxs; ii < lmbxe;
         ii++) {  // Create the concentration vector for this sed.coeff.
...
int US_MwlSpectra::dvec_index(QVector<double>& dvec, const double dval) {
  ...
  if (indx <
    ...
    for (int jj = 0; jj < dvec.size(); jj++) {  // Search doubles vector
      if (dvirt_equal(dvec[jj], dval)) {  // If vector value matches within
                                          // epsilon, break and return
        indx = jj;
        break;
      }
...
void US_MwlSpectra::final_stats(QVector<int>& istats, QVector<double>& dstats,
                                QVector<double>& lwlns, QVector<double>& lseds,
                                QVector<double>& lcons) {
...
void US_MwlSpectra::sum_check() {
  ...
  if (sumchkd) {  // Set S value to plot to midway point between S limits
    ...
    le_status->setText(
        lb_pltrec->text() + "  " +
        tr("sum for S from %1 to %2").arg(sed_start).arg(sed_end));
  }
...
void US_MwlSpectra::save_data() {
  ...
  QString dname =
      "wavelen-srangesum" + spart + "." + tpart + "." + apart + ".csv";
  ...
  QString msg = tr("In the results/run directory<br/>"
                   ...
                   " or use the default provided.<br/>")
                    .arg(rdir);
  ename =
      QInputDialog::getText(this, tr("File Name: Wavelength-vs-SRangeSum CSV"),
  ...
  const QString comma(",");
  const QString endln("\n");
  ts << dquote + "Wavelength(nm)" + dquote + comma + dquote +
            "S-Range-Concentration_Sum" + dquote + endln;
...
double US_MwlSpectra::comp_value(const US_Model::SimulationComponent* sc,
                                 int type) {
  double xval = sc->s;
...
void US_MwlSpectra::changedPlotX(bool on_state) {
  ...
  DbgLv(1) << "LD: loadedmodels:" << loadedmodels.count()
           << " mdescs:" << mdescs.count();
  for (int jj = 0; jj < loadedmodels.count();
       jj++) {  // Load each selected distribution model
  ...
  cn_max = 0.0;
  double scalen =
      1.0 / (double)loadedmodels.count();  // Normalizing scale factor
