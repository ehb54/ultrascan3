...
US_MwlSpectra::US_MwlSpectra() : US_Widgets() {
   ...
   QLabel *lb_smooth = us_label(tr("%1 Gaussian Smooth Points:").arg(chlamb), -1);
   ...
   QLayout *lo_srngsum = us_checkbox(tr("X Range Sum").arg(xaxis), ck_srngsum, false);
   ...
   connect(pb_loaddis, SIGNAL(clicked()), this, SLOT(load_distro()));
   connect(pb_reset, SIGNAL(clicked()), this, SLOT(resetAll()));
   connect(pb_details, SIGNAL(clicked()), this, SLOT(runDetails()));
   connect(cb_sstart, SIGNAL(currentIndexChanged(int)), this, SLOT(changeSedcoeff()));
   connect(cb_send, SIGNAL(currentIndexChanged(int)), this, SLOT(changeSedcoeff()));
   ...
   connect(ct_smooth, SIGNAL(valueChanged(double)), this, SLOT(changeSmooth()));
   ...
   connect(ck_srngsum, SIGNAL(clicked()), this, SLOT(sum_check()));
   connect(pb_svdata, SIGNAL(clicked()), this, SLOT(save_data()));
   connect(pb_plot2d, SIGNAL(clicked()), this, SLOT(changeRecord()));
   ...
   QBoxLayout *plot = new US_Plot(
      ...
      tr("S Record Spectrum Data"
         "\nS (x 10^13) :  1.12"),
      tr("Wavelength (nm)"), tr("Concentration (OD)"));
...
void US_MwlSpectra::enableControls(void) {
   if (mdlxyz.count() == 0) { // If no data yet, just reset
      reset();
      return;
   }
...
void US_MwlSpectra::load_distro() {
   ...
   connect(&lddiag, SIGNAL(changed(bool)), this, SLOT(update_disk_db(bool)));
   ...
   for (int jj = 0; jj < models.count(); jj++) { // Load each selected distribution model
   ...
   DbgLv(1) << "LD: nlambda" << nlambda << "nsedcos" << nsedcos << "nipoint" << nipoint;
   nc_max = 0;
   cn_max = 0.0;
   ...
   for (int jj = 0; jj < nsedcos; jj++) {
      ...
      for (int ii = 0; ii < nlambda; ii++) {
         ...
         for (int kk = 0; kk < nipoint; kk++) {
            ...
            if (dvirt_equal(mdlxyz[ kk ].y(), sedco) && dvirt_equal(mdlxyz[ kk ].x(), waveln)) {
               nconcs++;
               csum += mdlxyz[ kk ].z();
            }
         }

         if (nconcs > 0) { // Normalize concentration by dividing sum by number of models
            conc = csum * scalen;
            nc_max = qMax(nc_max, nconcs);
            cn_max = qMax(cn_max, conc);
            nnpoint++;
            if (nconcs > 5 || conc > 1000.0)
               DbgLv(1) << "LD: **nc_max cn_max" << nc_max << cn_max << "nconcs conc" << nconcs << conc << "jj,ii" << jj
                        << ii << "s wl" << sedco << waveln;
         }
   ...
   DbgLv(1) << "LD: nipoint" << nipoint << "nnpoint" << nnpoint << "ntpoint" << ntpoint << "nc_max" << nc_max
            << "cn_max" << cn_max;
   int kdx = 0;
...
void US_MwlSpectra::load_distro(const US_Model model, const QString mdescr) {
   ...
   if (lambda < 1) { // If model has no wavelength, get it from the description
   ...
   for (int jj = 0; jj < model.components.size(); jj++) {
      ...
      if (conc > 1000.0)
         DbgLv(1) << "LD: **co_max" << co_max << "conc" << conc << "jj" << jj << "lambda xc" << lambda << xc << "mdesc"
                  << mdesc;
...
void US_MwlSpectra::plot_all(void) {
   ...
   if (last_xmin < 0.0) { // If first time, set scales based on actual values present
...
void US_MwlSpectra::compute_ranges() {
   ...
   if (sedxp < 0 || recx < 0) {
      DbgLv(1) << "cmpR:  (1)sS sE sxS sxE" << sed_start << sed_end << sedxs << sedxe << "sxP rx sP" << sedxp << recx
               << sed_plot;
      sed_plot = qMax(sed_start, qMin(sed_end, sed_plot));
      sedxp = dvec_index(sedcoes, sed_plot);
      recx = sedxp - sedxs;
   }
   DbgLv(1) << "cmpR:  sS sE sxS sxE" << sed_start << sed_end << sedxs << sedxe << "sxP rx sP" << sedxp << recx
            << sed_plot;
   ...
   if (!ck_srngsum->isChecked()) { // Get concentrations from current sedcoeff record, current lambda range
      for (int ii = lmbxs; ii < lmbxe; ii++)
         pltyvals << concdat[ sedxp ][ ii ];
   }

   else { // Get sum of concentrations in sedcoeff range, current lambda range
      double ssum;
...
void US_MwlSpectra::plot_3d() {
   ...
   for (int jj = sedxs; jj < sedxe; jj++) {
      ...
      for (int ii = lmbxs; ii < lmbxe; ii++) { // Create the concentration vector for this sed.coeff.
...
int US_MwlSpectra::dvec_index(QVector<double> &dvec, const double dval) {
   ...
   if (indx < 0) { // If no exact match was found, look for a match within epsilon
      ...
      for (int jj = 0; jj < dvec.size(); jj++) { // Search doubles vector
         if (dvirt_equal(dvec[ jj ], dval)) { // If vector value matches within epsilon, break and return
            indx = jj;
            break;
         }

         double ddif = qAbs(dvec[ jj ] - dval);
         if (ddif < dmin) { // Save index to closest match so far, as a back-up
            indx = jj;
            dmin = ddif;
         }
      }
...
void US_MwlSpectra::bld_stats(
   double wlnv, double sedv, double conv, QVector<int> &istats, QVector<double> &dstats, QVector<double> &lwlns,
...
void US_MwlSpectra::final_stats(
   QVector<int> &istats, QVector<double> &dstats, QVector<double> &lwlns, QVector<double> &lseds,
...
void US_MwlSpectra::sum_check() {
   ...
   if (sumchkd) { // Set S value to plot to midway point between S limits
      ...
      cb_pltrec->setCurrentIndex(sedxp);
      connect(cb_pltrec, SIGNAL(currentIndexChanged(int)), this, SLOT(changeRecord()));
      le_status->setText(lb_pltrec->text() + "  " + tr("sum for S from %1 to %2").arg(sed_start).arg(sed_end));
   }
...
void US_MwlSpectra::save_data() {
   ...
   sed_start = cb_sstart->currentText().toDouble(); // Sedcoeff start
   sed_end = cb_send->currentText().toDouble(); // Sedcoeff end
   tpart = tpart.left(2) + QString("%1-%2").arg(lmb_start).arg(lmb_end);
   ...
   QString dname = "wavelen-srangesum" + spart + "." + tpart + "." + apart + ".csv";
   ...
   QString msg = tr("In the results/run directory<br/>"
                    ...
                    .arg(rdir);
   ename = QInputDialog::getText(this, tr("File Name: Wavelength-vs-SRangeSum CSV"), msg, QLineEdit::Normal, ename, &ok);
   ...
   if (!csvo_f.open(QIODevice::WriteOnly | QIODevice::Text)) {
      QMessageBox::critical(this, tr("CSV File Open ERROR"), tr("Output open error:\n%1").arg(dpath));
   ...
   const QString comma(",");
   const QString endln("\n");
   ts << dquote + "Wavelength(nm)" + dquote + comma + dquote + "S-Range-Concentration_Sum" + dquote + endln;

   for (int ii = 0; ii < nwavln; ii++) {
      ts << dquote + QString().sprintf("%d", ( int ) rr[ ii ]) + dquote + comma + dquote
               + QString().sprintf("%10.4e", vv[ ii ]) + dquote + endln;
   }

   csvo_f.close();

   QMessageBox::information(
      this, tr("CSV File Saved"),
...
void US_MwlSpectra::changedPlotX(bool on_state) {
   ...
   for (int jj = 0; jj < loadedmodels.count(); jj++) { // Load each selected distribution model
