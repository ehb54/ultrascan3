...
private:
  ...
  QMap<QString, QVector<double> >
      triple_name_to_Cdata; //!< C data for each triple.

  QMap<QString, QMap<double, QMap<int, QVector<double> > > >
      xfit_data_all_orders; //!< X fit data for all orders.
  QMap<QString, QMap<double, QMap<int, QVector<double> > > >
      yfit_data_all_orders; //!< Y fit data for all orders.
  QMap<QString, QMap<double, QMap<int, double> > >
      variance_triple_order_map; //!< Variance map for each triple and order.
  ...
  QMap<QString, QVector<double> >
      triple_name_to_peaks_map; //!< Peaks map for each triple.
  QMap<QString, double>
      triple_name_to_total_area; //!< Total area for each triple.
  QMap<QString, double>
      triple_name_to_total_area_uncorrected; //!< Uncorrected total area for
                                             //!< each triple.
  QMap<QString, double> triple_name_to_rmsd; //!< RMSD for each triple.
  QMap<QString, double *>
      triple_name_to_fit_parameters; //!< Fit parameters for each triple.
  QMap<QString, QVector<QwtPlotCurve *> >
      triple_name_to_peak_curves_map; //!< Peak curves map for each triple.
  QMap<QString, QVector<QwtPlotCurve *> >
      triple_name_to_peak_gauss_envelopes_map; //!< Gaussian envelope curves
                                               //!< map for each triple.
  QMap<QString, QMap<QString, QStringList> >
      triple_name_to_peak_to_parms_map; //!< Peak parameters map for each
                                        //!< triple.
  QMap<QString, bool>
      triple_report_saved_map;           //!< Report saved map for each triple.
  QMap<QString, bool> triple_fitted_map; //!< Fitted map for each triple.
  QMap<QString, bool>
      triple_peaks_defined_map; //!< Peaks defined map for each triple.
  QMap<QString, bool> do_fit_for_triple; //!< Fit flag for each triple.

  QMap<QString, double>
      alpha_centerpiece; //!< Alpha centerpiece for each triple.
  QMap<QString, double>
      data_left_to_triple_name_map; //!< Data left map for each triple.
  QMap<QString, double>
      data_right_to_triple_name_map; //!< Data right map for each triple.
  QMap<QString, double>
      meniscus_to_triple_name_map; //!< Meniscus map for each triple.
  QMap<QString, double>
      buffDensity_to_triple_name_map; //!< Buffer density map for each triple.
  QMap<QString, double>
      sigma_to_triple_name_map; //!< Sigma map for each triple.
  QMap<QString, double>
      gradMW_to_triple_name_map; //!< Gradient MW map for each triple.
  QMap<QString, double>
      gradVbar_to_triple_name_map; //!< Gradient Vbar map for each triple.
  QMap<QString, double>
      gradC0_to_triple_name_map; //!< Gradient C0 map for each triple.

  QMap<QString, int>
      gauss_order_minVariance; //!< Minimum variance order for each triple.
  QMap<QString, double>
      sigma_val_minVariance; //!< Minimum variance sigma value for each triple.
...
private slots:
  ...
  QMap<QString, double> find_closest_sigma_height (QString triple,
                                                   double value);
  ...
  bool isMaximum_y (QVector<double> data, int index, int left, int right,
                    QString triple);
  ...
  QMap<QString, double> get_data_conf_from_edit_profile (QString triple,
                                                         QString edit);

  //! \brief Process Y fit data.
  //! \param x The X fit data.
  //! \param y The Y fit data.
  void process_yfit (QVector<QVector<double> > &x,
  ...
  QMap<QString, double> calc_gauss_area (QString triple, double mean,
