...
US_Buoyancy::US_Buoyancy(QString auto_mode) : US_Widgets() {
  ...
  connect(ct_selectScan, SIGNAL(valueChanged(double)), SLOT(plot_scan(double)));
  ...
  connect(ck_auto_fit, SIGNAL(toggled(bool)), this, SLOT(enblFitBtn(bool)));
...
void US_Buoyancy::new_peak(int index) {
  ...
  double percent_of_total =
      curr_peak_parms[5].toDouble() / curr_peak_parms[6].toDouble() * 100.0;
...
void US_Buoyancy::calc_points(void) {
  ...
  if (tmp_dpoint.peakPosition != 0.0) {
    ...
    C0 = tmp_dpoint.gradientC0 -
         tmp_dpoint
             .bufferDensity;  // subtract buffer density from nycodenz density
    ...
    k2 = exp(
        k1 * (k4) *
        (pow(tmp_dpoint.peakPosition, 2.0) - pow(tmp_dpoint.meniscus, 2.0)));
    k3 = exp(k1 * (k4)*r2);
...
void US_Buoyancy::calc_points_auto(QString triple_n) {
  ...
  for (int rr = 0; rr < xfit_data[triple_n].size(); rr++) {
    ...
    C01 = gradC0 -
          buffDensity_to_triple_name_map[triple_n];  // subtract buffer density
                                                     // from nycodenz density
    ...
    k21 = exp(
        k11 * (k41) *
        (pow(curr_x, 2.0) - pow(meniscus_to_triple_name_map[triple_n], 2.0)));
    k31 = exp(k11 * (k41)*r21);
  ...
  for (int i = 0; i < peak_poss.size(); i++) {
    ...
    if (peak_poss[i] != 0.0) {
      ...
      // identify closest (position-wise) fitted Gaussian && its sigma, as well
      // as peak amplitude
      ...
      QMap<QString, double> gauss_areas =
          calc_gauss_area(triple_n, peak_poss[i], peak_sigma_height["sigma"],
  ...
  if (!peak_names.isEmpty()) {
    ...
    cb_peaks->addItems(peak_names);
    connect(cb_peaks, SIGNAL(currentIndexChanged(int)), SLOT(new_peak(int)));
...
QMap<QString, double> US_Buoyancy::calc_gauss_area(QString triple_n,
                                                   double pos_p, double sigma_p,
                                                   double height_p) {
  double gauss_area = 0;
  ...
  for (int ii = 0; ii < xfit_data[triple_n].size() - 1; ++ii) {
    ...
    if (sectoral)
      gauss_area +=
          height_p * exp(-(pow((x_val - pos_p), 2)) / (2 * pow(sigma_p, 2))) *
          jac * alpha / 360.0 * M_PI * (pow(point2, 2) - pow(point1, 2));
    else
      gauss_area += height_p *
...
QMap<QString, double> US_Buoyancy::find_closest_sigma_height(QString triple_n,
                                                             double peak_p) {
  ...
  for (unsigned int i = 0; i < gauss_order_minVariance[triple_n]; i++) {
    // qDebug() << tr("Peak position: ")      << fit_p[v_wavelength.size() + (i
    // * 3) + 1]  << " nm\n"; qDebug() << tr("Amplitude of peak: ")  <<
    // exp(fit_p[v_wavelength.size() + (i * 3)]) << tr(" extinction\n");
    // qDebug() << tr("Peak width: ")         << fit_p[v_wavelength.size() + (i
    // * 3) + 2]  << " nm\n";
...
void US_Buoyancy::load(void) {
  ...
  // for ( int ii=0; ii<triples.size(); ii++ )
  //   {
  //     QString rawGUID_test  = US_Util::uuid_unparse( (unsigned char*)allData[
  //     ii ].rawGUID ); qDebug() << " DataSet: " << ii << ", RawData GUID -- "
  //     << rawGUID_test; qDebug() << " DataSet: " << ii << ", Description --"
  //     << allData[ ii ].description;
  //   }
  // return;
  // //
  ...
  if (isLocal) {
    for (int ii = 0; ii < triples.size(); ii++) {  // Generate file names
...
QMap<QString, double> US_Buoyancy::get_data_conf_from_edit_profile(
    QString rawGUID_t, QString triple_n) {
  QMap<QString, double> data_conf;
  double meniscus_p = 0;
  double data_left = 0;
  double data_right = 0;
  ...
  if (editDataIDs.size() == 0) {
    QMessageBox::warning(this, tr("Edit data is not in DB"),
...
void US_Buoyancy::plot_scan(double scan_number) {
  ...
  //
  for (int ii = 0; ii < ssize; ii++) {
    if (us_buoyancy_auto_mode && ii != ssize - 1) continue;
    ...
    if (!us_buoyancy_auto_mode) {
      if (ii ==
          current_scan -
              1) {  // set the temperature to the currently highlighted scan:
  ...
  if (us_buoyancy_auto_mode && !triple_fitted_map[triple_n] &&
    ...
    double sigma_max = sigma + sigma_step * (double)(total_steps / 2.0) *
    ...
    for (int sigma_i = 0; sigma_i < totalSigmas; ++sigma_i) {
      ...
      for (int order_i = order_init; order_i < (totalOrders + order_init);
        ...
        pgb_progress->setFormat(
            triple_n + ", Orders:    " + QString::number(int(progress)) + "%");
        ...
        double R_step =
            (maxR - minR) /
            (order +
             1);  // create "order" peaks evenly distributed over the range
        QString projectName = QString("");

        qDebug() << "Positions: min, max, step: " << minR << ", " << maxR
                 << ", " << R_step;
        ...
        for (unsigned int i = 0; i < order; i++) {
          ...
          fitparameters[v_wavelength.size() + (i * 3) + 2] =
              current_sigma;  // Sigma: comes based on single Gauss fit of
                              // representative data (AVV) peak
        }

        // call US_Extinctfitter
        fitter = new US_ExtinctFitter(&v_wavelength, fitparameters, order,
                                      parameters, projectName, &fitting_widget,
                                      true);
  ...
  if (us_buoyancy_auto_mode && !triple_peaks_defined_map[triple_n] &&
    ...
    for (ss = curr_triple_sigma_order_vars.begin();
      ...
      for (mm = curr_triple_order_vars.begin();
        ...
        if (mm.value() == 0) continue;

        qDebug() << "Triple " << triple_n << ": sigma, order, variance -- "
                 << ss.key() << mm.key() << mm.value();
...
QVector<double> US_Buoyancy::identify_peaks(QString triple_n, double sigma_p) {
  ...
  for (int i = 0; i < xfit_data[triple_n].size(); i++) {
    ...
    // qDebug() << "Proceeding with point x, y: " << curr_x << curr_y << "; pm
    // sigma -- " << left_x << right_x ;
...
bool US_Buoyancy::isMaximum_y(QVector<double> ydata, int curr_i, int left_i,
  ...
  if (isPeak) {
    ...
    if ((curr_y - minimum) < od_threshold) isPeak = false;
  }
...
void US_Buoyancy::process_yfit(QVector<QVector<double> >& x,
...
void US_Buoyancy::add_peak(void) {
  QMessageBox::information(this, tr("Manual Peak Selection"),
...
void US_Buoyancy::draw_gauss_envelope(QMap<QString, QStringList> peak_p) {
  ...
  for (ss = peak_p.begin(); ss != peak_p.end(); ++ss) {
    ...
    double sigma =
        pp[3]
            .toDouble();  //<---- Alternatively, sigma is determined differently
    double height = pp[4].toDouble();

    qDebug() << "Peak: " << ss.key() << ", pos_x, sigma, height -- " << pos_x
             << sigma << height;
    ...
    for (int ii = 0; ii < xfit_data[triple_n].size(); ++ii) {
      double x_val = xfit_data[triple_n][ii];
      double y_val =
          height * exp(-(pow((x_val - pos_x), 2)) / (2 * pow(sigma, 2)));
...
void US_Buoyancy::write(void) {
  ...
  for (int i = 0; i < dpoint.size(); i++) {
    ...
    te->e->append("Rotor speed:\t" + str.setNum(dpoint[i].speed) +
                  " rpm, (Rotor stretch: " + str2.setNum(dpoint[i].stretch) +
                  " cm)");
...
void US_Buoyancy::write_auto(void) {
  ...
  for (ta = triple_name_to_total_area.begin();
    ...
    te->e->append(
        "Triple: " + ta.key() +
        ";\t Total OD, Volumetric Measure (corrected for centerpiece shape): " +
  ...
  for (int i = 0; i < dpoint.size(); i++) {
    ...
    te->e->append("Peak Gauss area UNCORRECTED (% of total):\t" +
