...
US_Buoyancy::US_Buoyancy(QString auto_mode) : US_Widgets() {
   ...
   connect(le_dens_0, SIGNAL(editingFinished(void)), this, SLOT(update_for_dens_0(void)));
   ...
   connect(le_vbar, SIGNAL(editingFinished(void)), this, SLOT(update_for_vbar(void)));
   ...
   connect(le_MW, SIGNAL(editingFinished(void)), this, SLOT(update_for_MW(void)));
   ...
   connect(le_sigma, SIGNAL(editingFinished(void)), this, SLOT(update_for_sigma(void)));
   ...
   connect(le_peakName, SIGNAL(editingFinished(void)), this, SLOT(update_peakName(void)));
   ...
   plot = new US_Plot(data_plot, tr("Buoyant Density Equilibrium"), tr("Radius (in cm)"), tr("Absorbance"));
...
US_Buoyancy::US_Buoyancy() : US_Widgets() {
   ...
   specs->addWidget(le_meniscus, s_row++, 2, 1, 2);
   connect(le_meniscus, SIGNAL(editingFinished(void)), this, SLOT(update_meniscus(void)));
   ...
   specs->addWidget(le_bottom, s_row++, 2, 1, 2);
   connect(le_bottom, SIGNAL(editingFinished(void)), this, SLOT(update_bottom(void)));

   QLabel *lbl_bottom_calc = us_label(tr("Speed-corrected Bottom (cm):"), -1);
   specs->addWidget(lbl_bottom_calc, s_row, 0, 1, 2);
   le_bottom_calc = us_lineedit(QString::number(bottom_calc));
   specs->addWidget(le_bottom_calc, s_row++, 2, 1, 2);
   connect(le_bottom_calc, SIGNAL(editingFinished(void)), this, SLOT(update_bottom_calc(void)));
   ...
   specs->addWidget(le_buffer_density, s_row++, 2, 1, 2);
   connect(le_buffer_density, SIGNAL(editingFinished(void)), this, SLOT(update_bufferDensity(void)));
   ...
   // Plot layout on right side of window
   plot = new US_Plot(data_plot, tr("Absorbance Data"), tr("Radius (in cm)"), tr("Absorbance"));
...
void US_Buoyancy::new_peak(int index) {
   ...
   //highligth peak line
   if (triple_name_to_peak_curves_map.contains(triple_n) && triple_name_to_peak_gauss_envelopes_map.contains(triple_n)) {
   ...
   for (int i = 0; i < peak_poss.size(); i++) {
      ...
      if (peak_poss[ i ] != 0.0) {
         ...
         QMap<QString, double> gauss_areas
            = calc_gauss_area(triple_n, peak_poss[ i ], peak_sigma_height[ "sigma" ], peak_sigma_height[ "height" ]);
         double gauss_area = gauss_areas[ "corrected" ];
         double gauss_area_uncorrected = gauss_areas[ "uncorrected" ];
...
QMap<QString, double> US_Buoyancy::calc_total_area(QString triple_n) {
   ...
   for (int ii = 0; ii < xfit_data[ triple_n ].size() - 1; ++ii) {
      ...
      if (sectoral)
         total_area += yfit_data[ triple_n ][ ii ] * jac * alpha / 360.0 * M_PI * (pow(point2, 2) - pow(point1, 2));
      else
...
QMap<QString, double> US_Buoyancy::calc_gauss_area(QString triple_n, double pos_p, double sigma_p, double height_p) {
   ...
   for (int ii = 0; ii < xfit_data[ triple_n ].size() - 1; ++ii) {
      ...
      if (sectoral)
         gauss_area += height_p * exp(-(pow((x_val - pos_p), 2)) / (2 * pow(sigma_p, 2))) * jac * alpha / 360.0 * M_PI
...
void US_Buoyancy::print_xy(US_DataIO::RawData curr_set, int set_num) {
   ...
   for (int ii = 0; ii < ssize; ii++) {
      ...
      for (int jj = 0; jj < rsize; jj++) {
         qDebug() << "SET [" << set_num << "] -- " << curr_set.xvalues[ jj ] << s->rvalues[ jj ];
      }
   }
}
...
QMap<QString, double> US_Buoyancy::get_data_conf_from_edit_profile(QString rawGUID_t, QString triple_n) {
   ...
   foreach (struct cellInfo cell, cells) {
      //DEBUG
      qDebug() << "Cell, Channel from tripleName -- " << tripleName[ 0 ].simplified() << tripleName[ 1 ].simplified();
      qDebug() << "Cell, Channel from Cell -- " << cell.cellName << cell.channelName;

      if (tripleName[ 0 ].simplified() == cell.cellName && tripleName[ 1 ].simplified() == cell.channelName) {
   ...
   qDebug() << "For rawDataID, eID: " << rawDataID_t << eID << ", Meniscus is -- " << data_conf[ "meniscus" ]
            << ", Data Left is -- " << data_conf[ "data_left" ] << ", Data Right is -- " << data_conf[ "data_right" ]
            << ", Buffer Density is -- " << data_conf[ "buffer_density" ] << ", Alpha Centerpiece is -- "
            << data_conf[ "alpha_centerpiece" ];
...
void US_Buoyancy::plot_scan(double scan_number) {
   ...
   for (int ii = 0; ii < ssize; ii++) {
      ...
      for (int jj = 0; jj < rsize; jj++) {
         ...
         if (temp_x >= data_left_to_triple_name_map[ triple_n ] && temp_x <= data_right_to_triple_name_map[ triple_n ])
      ...
      if (!us_buoyancy_auto_mode) {
         if (ii == current_scan - 1) { // set the temperature to the currently highlighted scan:
         ...
         for (int order_i = order_init; order_i < (totalOrders + order_init); ++order_i) {
            ...
            for (unsigned int i = 0; i < order; i++) {
               ...
               fitparameters[ v_wavelength.size() + (i * 3) + 2 ]
                  = current_sigma; // Sigma: comes based on single Gauss fit of representative data (AVV) peak
            }

            //call US_Extinctfitter
            fitter = new US_ExtinctFitter(
               &v_wavelength, fitparameters, order, parameters, projectName, &fitting_widget, true);
   ...
   if (us_buoyancy_auto_mode && !triple_peaks_defined_map[ triple_n ] && do_fit_for_triple[ triple_n ]) {
      ...
      qDebug() << "Smallest variance " << minVariance << " for triple " << triple_n << ", for Sigma val -- "
               << sigma_val_minVariance[ triple_n ] << ", for Gauss order -- " << gauss_order_minVariance[ triple_n ];
...
QVector<double> US_Buoyancy::identify_peaks(QString triple_n, double sigma_p) {
   ...
   for (int i = 0; i < xfit_data[ triple_n ].size(); i++) {
      ...
      if ((curr_x - sigma) < xfit_data[ triple_n ][ 0 ] || (curr_x + sigma) > xfit_data[ triple_n ][ last_index ])
         continue;

      double left_x = (curr_x - sigma) > xfit_data[ triple_n ][ 0 ] ? (curr_x - sigma) : xfit_data[ triple_n ][ 0 ];
      int left_i = index_of_data(xfit_data[ triple_n ], left_x);

      double right_x = (curr_x + sigma) < xfit_data[ triple_n ][ last_index ] ? (curr_x + sigma)
...
void US_Buoyancy::process_variance(double variance) {
   ...
   qDebug() << "For triple: " << triple_n << ", sigma " << current_sigma << ", order " << current_order
            << ": variance: " << variance;
}
...
void US_Buoyancy::add_peak(void) {
   ...
   connect(pick, SIGNAL(cMouseUp(const QwtDoublePoint &)), SLOT(mouse_peak(const QwtDoublePoint &)));
}
...
void US_Buoyancy::draw_gauss_envelope(QMap<QString, QStringList> peak_p) {
   ...
   for (ss = peak_p.begin(); ss != peak_p.end(); ++ss) {
      ...
      qDebug() << "Peak: " << ss.key() << ", pos_x, sigma, height -- " << pos_x << sigma << height;
...
void US_Buoyancy::save_auto(QString triple_n) {
   ...
   for (pp = peak_parms.begin(); pp != peak_parms.end(); ++pp) {
      ...
      tmp_dpoint.percentTotal_uncorrected = 100.0
...
void US_Buoyancy::write(void) {
   ...
   for (int i = 0; i < dpoint.size(); i++) {
      te->e->append(
         "Peak " + str.setNum(i + 1) + " (" + dpoint[ i ].name + " from experiment \"" + dpoint[ i ].dataset + "\"):");
      te->e->append("Sample location:\t" + dpoint[ i ].triple);
      te->e->append("Sample description:\t" + dpoint[ i ].description);
      te->e->append(
         "Rotor speed:\t" + str.setNum(dpoint[ i ].speed) + " rpm, (Rotor stretch: " + str2.setNum(dpoint[ i ].stretch)
         + " cm)");
      ...
      te->e->append(
         "Bottom of cell:\t" + str.setNum(dpoint[ i ].bottom)
...
void US_Buoyancy::write_auto(void) {
   ...
   for (ta = triple_name_to_total_area.begin(); ta != triple_name_to_total_area.end(); ++ta) {
      ...
      te->e->append(
         "Triple: " + ta.key()
         + ";\t Total OD, Volumetric Measure (corrected for centerpiece shape): " + QString::number(ta.value()));
      te->e->append(
         "Triple: " + ta.key() + ";\t Total OD, Area Measure (un-corrected for centerpiece shape): "
   ...
   for (int i = 0; i < dpoint.size(); i++) {
      ...
      te->e->append(
         "Peak Gauss area UNCORRECTED (% of total):\t" + QString::number(dpoint[ i ].percentTotal_uncorrected, 'f', 2));
...
double US_Buoyancy::calc_stretch() {
   return (
      simparams[ current_triple ].rotorcoeffs[ 0 ] * current_rpm
