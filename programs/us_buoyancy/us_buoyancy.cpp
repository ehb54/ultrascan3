...
US_Buoyancy::US_Buoyancy (QString auto_mode) : US_Widgets ()
{
  ...
  QGridLayout *box2
      = us_radiobutton (tr ("Select peak position"), rb_datapoint);
  ...
  le_buffer_density
      = us_lineedit (QString::number (tmp_dpoint.bufferDensity), -1, true);
  ...
  le_temperature
      = us_lineedit (QString::number (tmp_dpoint.temperature), -1, true);
  ...
  // Information on peaks: Listbox of to-be-determined (from automated fit)
  // size
  ...
  le_peakPosition
      = us_lineedit (QString::number (tmp_dpoint.peakPosition), -1, true);
  ...
  le_peakDensity
      = us_lineedit (QString::number (tmp_dpoint.peakDensity), -1, true);
  ...
  le_peakGaussArea
      = us_lineedit (QString::number (tmp_dpoint.peakGaussArea), -1, true);
  ...
  connect (ck_auto_fit, SIGNAL (toggled (bool)), this,
           SLOT (enblFitBtn (bool)));
  specs->addWidget (ck_auto_fit, s_row, 0, 1, 2);
  pb_fit_current_triple = us_pushbutton (tr ("Fit Current Triple"), false);
  connect (pb_fit_current_triple, SIGNAL (clicked ()),
           SLOT (fit_current_triple ()));
  ...
  plot = new US_Plot (data_plot, tr ("Buoyant Density Equilibrium"),
...
// Select a new peak
void
US_Buoyancy::new_peak (int index)
{
  QString triple_n = cb_triple->itemText (current_triple);
  QString peak_n = cb_peaks->itemText (index);

  QStringList curr_peak_parms
      = triple_name_to_peak_to_parms_map[triple_n][peak_n];

  double percent_of_total = curr_peak_parms[5].toDouble ()
                            / curr_peak_parms[6].toDouble () * 100.0;
  ...
  // QString::number(percent_of_total, 'f', 2) + "% of total)" );
  ...
  if (triple_name_to_peak_curves_map.contains (triple_n)
      && triple_name_to_peak_gauss_envelopes_map.contains (triple_n))
    {
      ...
      QVector<QwtPlotCurve *> v_line_vector
          = triple_name_to_peak_curves_map[triple_n];
      QVector<QwtPlotCurve *> gauss_vector
          = triple_name_to_peak_gauss_envelopes_map[triple_n];
      qDebug () << "v_line_vector size(), index -- " << v_line_vector.size ()
                << index;
...
void
US_Buoyancy::enblFitBtn (bool checked)
{
  if (checked)
...
// Fit current triple
void
US_Buoyancy::fit_current_triple (void)
{
...
// Select a new triple
void
US_Buoyancy::new_triple (int index)
{
...
void
US_Buoyancy::update_fields (void)
{
  QString str;
  QString triple_n = cb_triple->itemText (current_triple);
  ...
  tmp_dpoint.bottom
      = simparams[current_triple].bottom_position + current_stretch;
  ...
  if (!us_buoyancy_auto_mode)
  ...
  else
    {
      ...
      le_buffer_density->setText (
          QString::number (buffDensity_to_triple_name_map[triple_n]));
      le_sigma->setText (QString::number (sigma_to_triple_name_map[triple_n]));
      le_dens_0->setText (
          QString::number (gradC0_to_triple_name_map[triple_n]));
      le_vbar->setText (
          QString::number (gradVbar_to_triple_name_map[triple_n]));
  ...
  if (tmp_dpoint.peakPosition != 0.0)
    {
      ...
      C0 = tmp_dpoint.gradientC0
           - tmp_dpoint.bufferDensity; // subtract buffer density from nycodenz
                                       // density
      r2 = pow (tmp_dpoint.bottom, 2.0) - pow (tmp_dpoint.meniscus, 2.0);
      k1 = tmp_dpoint.gradientMW * omega_s
           / (2.0 * R_GC * (tmp_dpoint.temperature + 273.15));
      k4 = 1.0 - tmp_dpoint.gradientVbar * tmp_dpoint.bufferDensity;
      k2 = exp (k1 * (k4)
                * (pow (tmp_dpoint.peakPosition, 2.0)
                   - pow (tmp_dpoint.meniscus, 2.0)));
      k3 = exp (k1 * (k4)*r2);
...
void US_Buoyancy::calc_points_auto( QString triple_n )
{
  ...
  double tot_area_uncorrected  = triple_name_to_total_area_uncorrected[
triple_n ];
  ...
  for (int rr=0; rr < xfit_data[ triple_n ].size(); rr++ )
    {
      ...
      double omega_s, C, C0, r2, k1, k2, k3, k4;
      omega_s = pow( current_rpm * M_PI/30.0, 2.0 );
      C0 = gradC0 - buffDensity_to_triple_name_map[ triple_n ]; //subtract
buffer density from nycodenz density r2 = pow( tmp_dpoint.bottom, 2.0 ) - pow(
meniscus_to_triple_name_map[ triple_n ], 2.0); k1 = gradMW * omega_s/( 2.0 *
R_GC * (tmp_dpoint.temperature + 273.15) ); k4 = 1.0 - gradVbar *
buffDensity_to_triple_name_map[ triple_n ]; k2 = exp( k1 * ( k4 ) * (pow(
curr_x, 2.0 ) - pow( meniscus_to_triple_name_map[ triple_n ], 2.0 ) ) ); k3 =
exp( k1 * ( k4 ) * r2); C  = k1 * k4 * C0 *r2 * k2/( k3 - 1.0 ) +
buffDensity_to_triple_name_map[ triple_n ];
          ...
          if ( curr_x != 0.0 )
            {
              ...
              //identify closest (position-wise) fitted Gaussian && its sigma,
as well as peak amplitude QMap< QString, double > peak_sigma_height =
find_closest_sigma_height( triple_n, curr_x ); peak_sigma_height[ "sigma" ] =
sigma_gui;  // re-write sigma as fittted parm-sigma does not work...
              ...
              //add area under peak
              QMap< QString, double > gauss_areas = calc_gauss_area( triple_n,
curr_x, peak_sigma_height[ "sigma" ], peak_sigma_height[ "height" ] ); double
gauss_area             = gauss_areas[ "corrected" ]; double
gauss_area_uncorrected = gauss_areas[ "uncorrected" ];
              ...
              triple_name_to_peak_to_parms_map[ triple_n ][ peak_name ] =
curr_peak_parms;
          ...
          else
            {
              ...
              qDebug() << "Peak position #" <<  peak_number << " for triple "
<< triple_n << " is ZERO!";
            }
        }
    }
...
// cacl. poitns based on provided vector of peak positions
void
US_Buoyancy::calc_points_auto (QString triple_n)
{
  QVector<double> peak_poss = triple_name_to_peaks_map[triple_n];
  triple_name_to_peak_to_parms_map[triple_n].clear ();
  ...
  double gradVbar = gradVbar_to_triple_name_map[triple_n];
  double gradC0 = gradC0_to_triple_name_map[triple_n];
  double tot_area_uncorrected
      = triple_name_to_total_area_uncorrected[triple_n];
  ...
  for (int rr = 0; rr < xfit_data[triple_n].size (); rr++)
    {
      ...
      C01 = gradC0
            - buffDensity_to_triple_name_map
                [triple_n]; // subtract buffer density from nycodenz density
      r21 = pow (tmp_dpoint.bottom, 2.0)
            - pow (meniscus_to_triple_name_map[triple_n], 2.0);
      k11 = gradMW * omega_s1
            / (2.0 * R_GC * (tmp_dpoint.temperature + 273.15));
      ...
      k21 = exp (k11 * (k41)
                 * (pow (curr_x, 2.0)
                    - pow (meniscus_to_triple_name_map[triple_n], 2.0)));
      k31 = exp (k11 * (k41)*r21);
      C1 = k11 * k41 * C01 * r21 * k21 / (k31 - 1.0)
           + buffDensity_to_triple_name_map[triple_n];
  ...
  //   qDebug() << "C data for triple - " << triple_n << ": " <<
  //   triple_name_to_Cdata[ triple_n ][i];
      ...
      if (peak_poss[i] != 0.0)
        {
          ...
          // identify closest (position-wise) fitted Gaussian && its sigma, as
          // well as peak amplitude
          QMap<QString, double> peak_sigma_height
              = find_closest_sigma_height (triple_n, peak_poss[i]);
          peak_sigma_height["sigma"]
              = sigma_gui; // re-write sigma as fittted parm-sigma does not
                           // work...

          curr_peak_parms << QString::number (peak_sigma_height["sigma"]) // 3
                          << QString::number (
                                 peak_sigma_height["height"]); // 4

          // add area under peak
          QMap<QString, double> gauss_areas = calc_gauss_area (
              triple_n, peak_poss[i], peak_sigma_height["sigma"],
              peak_sigma_height["height"]);
      ...
      else
        {
          ...
          qDebug () << "Peak position #" << i << " for triple " << triple_n
                    << " is ZERO!";
        }
    }
...
QMap<QString, double>
US_Buoyancy::calc_total_area (QString triple_n)
{
  double total_area = 0;
  ...
  for (int ii = 0; ii < xfit_data[triple_n].size () - 1; ++ii)
    {
      ...
      if (sectoral)
        total_area += yfit_data[triple_n][ii] * jac * alpha / 360.0 * M_PI
                      * (pow (point2, 2) - pow (point1, 2));
      else
...
QMap<QString, double>
US_Buoyancy::calc_gauss_area (QString triple_n, double pos_p, double sigma_p,
                              double height_p)
{
  double gauss_area = 0;
  ...
  for (int ii = 0; ii < xfit_data[triple_n].size () - 1; ++ii)
    {
      ...
      if (sectoral)
        gauss_area
            += height_p
               * exp (-(pow ((x_val - pos_p), 2)) / (2 * pow (sigma_p, 2)))
               * jac * alpha / 360.0 * M_PI
               * (pow (point2, 2) - pow (point1, 2));
      else
        gauss_area
            += height_p
...
QMap<QString, double>
US_Buoyancy::find_closest_sigma_height (QString triple_n, double peak_p)
{
  ...
  for (unsigned int i = 0; i < gauss_order_minVariance[triple_n]; i++)
    {
      // qDebug() << tr("Peak position: ")      << fit_p[v_wavelength.size() +
      // (i * 3) + 1]  << " nm\n"; qDebug() << tr("Amplitude of peak: ")  <<
      // exp(fit_p[v_wavelength.size() + (i * 3)]) << tr(" extinction\n");
      // qDebug() << tr("Peak width: ")         << fit_p[v_wavelength.size() +
      // (i * 3) + 2]  << " nm\n";
...
US_Buoyancy::load (void)
{
  ...
  //     char*)allData[ ii ].rawGUID ); qDebug() << " DataSet: " << ii << ",
  //     RawData GUID -- " << rawGUID_test; qDebug() << " DataSet: " << ii <<
  //     ", Description --" << allData[ ii ].description;
  //   }
  // return;
  // //
  ...
  for (int ii = 0; ii < triples.size (); ii++)
    {
      ...
      if (us_buoyancy_auto_mode)
        {
          QString rawGUID_t
              = US_Util::uuid_unparse ((unsigned char *)allData[ii].rawGUID);

          // //DEBUG
          // /////////////////////////////////////////////////////////////////////////////////////////////////
          // QString rawGUID_t;
          // if ( ii == 0 )
          //   rawGUID_t  = US_Util::uuid_unparse( (unsigned char*)allData[ ii
          //   ].rawGUID );
          // else
          //   rawGUID_t  = US_Util::uuid_unparse( (unsigned char*)allData[
          //   triples_temp.size() - 1 ].rawGUID );
          // /////////////////////////////////////////////////////////////////////////////////////////////////////////

          QMap<QString, double> data_conf
              = get_data_conf_from_edit_profile (rawGUID_t, triples[ii]);
          meniscus[ii] = data_conf["meniscus"];

          if (meniscus[ii]
              == 0) // if no edit profile found, then HARD code something
          ...
          buffDensity_to_triple_name_map[triples[ii]]
              = data_conf["buffer_density"];
  ...
  if (isLocal)
    {
      for (int ii = 0; ii < triples.size (); ii++)
        { // Generate file names
          ...
          files << file;
          simparams[ii].initFromData (NULL, allData[ii], true, runID,
                                      dataType);
        }
    }
  else
  ...
  if (us_buoyancy_auto_mode)
    {
      ...
      else // fit each triple individually upon pressing 'Fit Current Triple':
           // disable checkbox
        {
        }
    }
...
// for debug
void
US_Buoyancy::print_xy (US_DataIO::RawData curr_set, int set_num)
{
  ...
  for (int ii = 0; ii < ssize; ii++)
    {
      ...
      for (int jj = 0; jj < rsize; jj++)
        {
          qDebug () << "SET [" << set_num << "] -- " << curr_set.xvalues[jj]
                    << s->rvalues[jj];
        }
    }
}
...
QMap<QString, double>
US_Buoyancy::get_data_conf_from_edit_profile (QString rawGUID_t,
                                              QString triple_n)
{
  QMap<QString, double> data_conf;
  double meniscus_p = 0;
  ...
  // now match cell's & channel name to those of triple && prescribe
  // centerpiece
  int centerpieceID = 0;
  ...
  foreach (struct cellInfo cell, cells)
    {
      ...
      qDebug () << "Cell, Channel from tripleName -- "
                << tripleName[0].simplified () << tripleName[1].simplified ();
      qDebug () << "Cell, Channel from Cell -- " << cell.cellName
                << cell.channelName;
  ...
  qDebug () << "Centerpiece name, angle -- " << centerpiece_name
            << centerpiece_angle;
      ...
      if (db.lastErrno () != US_DB2::OK)
        {
          ...
          QMessageBox::warning (this, tr ("Connection Problem"),
                                tr ("Unable to get buffer info from buffer ID")
                                    + bufId + "\n" + db.lastError ());
          return data_conf;
        }
...
void
US_Buoyancy::mouse (const QwtDoublePoint &p)
{
  ...
  if (rb_meniscus->isChecked ())
    {
      ...
      str.setNum (p.x ());
      le_meniscus->setText (str);
      // the internally stored meniscus position is adjusted to the theoretical
      // rest position
  ...
  QwtSymbol *sym = new QwtSymbol (QwtSymbol::Cross, brush, pen, QSize (8, 8));
...
void
US_Buoyancy::sel_investigator (void)
{
...
// changes it
void
US_Buoyancy::update_disk_db (bool isDB)
{
  if (isDB)
...
void
US_Buoyancy::plot_scan (double scan_number)
{
  float temp_x, temp_y;
  QString triple_n = cb_triple->itemText (current_triple);
  ...
  (wvl_n.length () == 3) ? change_y_axis_title = true
                         : change_y_axis_title = false;
  ...
  for (int ii = 0; ii < ssize; ii++)
    {
      ...
      // how many scans are included in the current set of speeds? Increment
      // maxscan...
      if (arpm == srpm)
        {
          maxscan++;
  ...
  if (us_buoyancy_auto_mode && triple_fitted_map[triple_n])
    {
      ...
      double *xx_ffit = (double *)xfit_data[triple_n]
                            .data (); // <- the only data set in xfit_data
      ...
      data_plot->setAxisScale (QwtPlot::yLeft, minV_final - padV,
                               maxV_final + padV);
      data_plot->setAxisScale (QwtPlot::xBottom, minR - padR, maxR + padR);
      ...
      double *xx_C = (double *)xfit_data[triple_n]
                         .data (); // <- the only data set in xfit_data
  ...
  if (us_buoyancy_auto_mode && !triple_fitted_map[triple_n]
      && do_fit_for_triple[triple_n])
    {
      // do fit of the current scan, identify peak positions
      // ///////////////////////////////////////////////////////////////////////
      qDebug () << "Size of v_wavelength array for triple : "
                << v_wavelength.last ().description << v_wavelength.size ();
      ...
      int total_steps = 0;
      while ((sigma - sigma_step * (int)(total_steps / 2.0))
             < 0.01) // set cap on minimum value
        --total_steps;

      double sigma_min = sigma - sigma_step * (double)(total_steps / 2.0);
      double sigma_max = sigma
                         + sigma_step * (double)(total_steps / 2.0)
                               * 1.5; // max value can go a bit higher
      ...
      for (int sigma_i = 0; sigma_i < totalSigmas; ++sigma_i)
        {
          ...
          for (int order_i = order_init; order_i < (totalOrders + order_init);
               ++order_i)
            {
              ...
              pgb_progress->setFormat (triple_n + ", Orders:    "
              ...
              double R_step
                  = (maxR - minR) / (order + 1); // create "order" peaks evenly
                                                 // distributed over the range
              QString projectName = QString ("");

              qDebug () << "Positions: min, max, step: " << minR << ", "
                        << maxR << ", " << R_step;

              for (unsigned int i = 0; i < order; i++)
                {
                  fitparameters[v_wavelength.size () + (i * 3)]
                      = 1; // Addition to the amplitude
                  // spread out the peaks
                  fitparameters[v_wavelength.size () + (i * 3) + 1]
                      = minR + R_step * long (i); // Position
                  fitparameters[v_wavelength.size () + (i * 3) + 2]
                      = current_sigma; // Sigma: comes based on single Gauss
                                       // fit of representative data (AVV) peak
                }

              // call US_Extinctfitter
              fitter = new US_ExtinctFitter (&v_wavelength, fitparameters,
                                             order, parameters, projectName,
                                             &fitting_widget, true);

              connect (fitter,
                       SIGNAL (get_yfit (QVector<QVector<double> > &,
                                         QVector<QVector<double> > &)),
                       this,
                       SLOT (process_yfit (QVector<QVector<double> > &,
                                           QVector<QVector<double> > &)));
              connect (fitter, SIGNAL (get_variance (double)), this,
                       SLOT (process_variance (double)));
  ...
  if (us_buoyancy_auto_mode && !triple_peaks_defined_map[triple_n]
      && do_fit_for_triple[triple_n])
    {
      ...
      QMap<double, QMap<int, double> > curr_triple_sigma_order_vars
          = variance_triple_order_map[triple_n];
      QMap<double, QMap<int, double> >::iterator ss;
      for (ss = curr_triple_sigma_order_vars.begin ();
           ss != curr_triple_sigma_order_vars.end (); ++ss)
        {
          QMap<int, double> curr_triple_order_vars = ss.value ();
          QMap<int, double>::iterator mm;

          for (mm = curr_triple_order_vars.begin ();
               mm != curr_triple_order_vars.end (); ++mm)
            {
              if (mm.value () == 0)
                continue;

              qDebug () << "Triple " << triple_n
                        << ": sigma, order, variance -- " << ss.key ()
                        << mm.key () << mm.value ();
      ...
      qDebug () << "Smallest variance " << minVariance << " for triple "
                << triple_n << ", for Sigma val -- "
                << sigma_val_minVariance[triple_n] << ", for Gauss order -- "
                << gauss_order_minVariance[triple_n];

      int g_order = gauss_order_minVariance[triple_n];
      double g_sigma = sigma_val_minVariance[triple_n];

      // finally, select the FIT data for the smallest variance to be plotted
      // alongside raw data:
      ...
      qDebug () << "[AUTO] peaks for triple: " << triple_n << " are -- "
                << peak_poss_auto;
      ...
      triple_name_to_total_area_uncorrected[triple_n]
          = tot_areas["uncorrected"];

      qDebug () << "Total area for triple " << triple_n
                << " is: " << triple_name_to_total_area[triple_n];
    }
...
double
US_Buoyancy::compute_rmsd (QString triple_n)
{
  double rmsd = 0;
  double mean = 0;
  ...
  for (int i = 0; i < xfit_data[triple_n].size (); i++)
    {
      mean += pow ((v_wavelength[0].v_readings[i].od - yfit_data[triple_n][i]),
                   2);
    }
...
QVector<double>
US_Buoyancy::identify_peaks (QString triple_n, double sigma_p)
{
...
bool
US_Buoyancy::isMaximum_y (QVector<double> ydata, int curr_i, int left_i,
                          int right_i, QString triple_n)
{
  bool isPeak = true;
  ...
  // maybe check for height estimate vs. OD threshold limit (to be considered a
  // peak)?
  if (isPeak)
    {
      double mean = 0;
      double minimum = 10000;
      // double od_threshold = 0.005;    // maybe should be calculated from
      // Gauss's height at [ x /pm sigma ]?
      double od_threshold = 3.5 * triple_name_to_rmsd[triple_n];

      qDebug () << "RMSD for triple: " << triple_n << ", "
                << triple_name_to_rmsd[triple_n];
...
int
US_Buoyancy::index_of_data (QVector<double> xdata, double val)
{
...
void
US_Buoyancy::process_yfit (QVector<QVector<double> > &x,
...
void
US_Buoyancy::process_variance (double variance)
{
  QString triple_n = cb_triple->itemText (current_triple);
  ...
  qDebug () << "For triple: " << triple_n << ", sigma " << current_sigma
            << ", order " << current_order << ": variance: " << variance;
}

// delete peak
void
US_Buoyancy::delete_peak (void)
{
  ...
  qDebug () << "Delete peak: triple_name_to_peaks_map[ triple ].size() -- "
            << triple_name_to_peaks_map[triple_n].size ();
  ...
  //      {
  // 	 qDebug() << "Removing dpoint entry for triple, peak -- " << dpoint[ i
  // ].triple << dpoint[ i ].name; 	 dpoint. remove( i ); 	 break;
  //      }
  //  }
}
...
void
US_Buoyancy::add_peak (void)
{
  QMessageBox::information (
      this, tr ("Manual Peak Selection"),
...
void
US_Buoyancy::mouse_peak (const QwtDoublePoint &p)
{
  QString triple_n = cb_triple->itemText (current_triple);
  triple_name_to_peaks_map[triple_n].push_back (p.x ());

  qDebug () << "Adding new peak, new size() -- "
            << triple_name_to_peaks_map[triple_n].size ();

  std::sort (triple_name_to_peaks_map[triple_n].begin (),
             triple_name_to_peaks_map[triple_n].end ());
  calc_points_auto (triple_n);
...
void
US_Buoyancy::draw_gauss_envelope (QMap<QString, QStringList> peak_p)
{
  QString triple_n = cb_triple->itemText (current_triple);
  ...
  for (ss = peak_p.begin (); ss != peak_p.end (); ++ss)
    {
      ...
      double sigma = pp[3].toDouble (); //<---- Alternatively, sigma is
      ...
      for (int ii = 0; ii < xfit_data[triple_n].size (); ++ii)
        {
          ...
          double y_val
              = height
                * exp (-(pow ((x_val - pos_x), 2)) / (2 * pow (sigma, 2)));
      ...
      ++peak_count;
      QString gauss_line_name
          = "Gauss Envelope" + QString::number (peak_count);
      ...
      triple_name_to_peak_gauss_envelopes_map[triple_n].push_back (
          peak_envelope);
    }
}
...
void
US_Buoyancy::draw_vline_auto (double radius, QString vline_number)
{
...
// Draw a vertical pick line
void
US_Buoyancy::draw_vline (double radius)
{
  double r[2];
...
void
US_Buoyancy::save_auto (QString triple_n)
{
  QMap<QString, QStringList> peak_parms
      = triple_name_to_peak_to_parms_map[triple_n];
  ...
  for (pp = peak_parms.begin (); pp != peak_parms.end (); ++pp)
    {
      ...
      tmp_dpoint.percentTotal = 100.0
                                * (curr_peak_parms_list[5].toDouble ()
                                   / curr_peak_parms_list[6].toDouble ());

      // TEMPORARY - for comparison; will be removed later
      tmp_dpoint.percentTotal_uncorrected
          = 100.0
            * (curr_peak_parms_list[7].toDouble ()
               / curr_peak_parms_list[8].toDouble ());
  ...
  for (int i = 0; i < dpoint.size (); i++)
    {
      ...
      te->e->append ("Rotor speed:\t" + str.setNum (dpoint[i].speed)
                     + " rpm, (Rotor stretch: "
                     + str2.setNum (dpoint[i].stretch) + " cm)");
      te->e->append ("Peak position:\t" + str.setNum (dpoint[i].peakPosition)
                     + " cm");
      te->e->append ("Peak density:\t" + str.setNum (dpoint[i].peakDensity)
                     + " g/ml");
      te->e->append ("Peak vbar:\t\t" + str.setNum (dpoint[i].peakVbar)
                     + " ml/g");
      te->e->append ("Buffer density:\t" + str.setNum (dpoint[i].bufferDensity)
                     + " g/ml");
      te->e->append ("Meniscus position:\t" + str.setNum (dpoint[i].meniscus)
                     + " cm");
      te->e->append ("Bottom of cell:\t" + str.setNum (dpoint[i].bottom)
                     + " cm (Centerpiece bottom at rest: "
                     + str2.setNum (dpoint[i].centerpiece) + " cm)");
      te->e->append ("Temperature:\t" + str.setNum (dpoint[i].temperature)
                     + " °C");
      te->e->append ("Gradient-forming\nmaterial details:");
      te->e->append ("Molecular weight:\t" + str.setNum (dpoint[i].gradientMW)
                     + " g/mol");
      te->e->append ("Loading density:\t" + str.setNum (dpoint[i].gradientC0)
                     + " g/mol");
      te->e->append ("vbar:\t\t" + str.setNum (dpoint[i].gradientVbar)
                     + " ml/g");
      te->e->append ("\n");
    }
...
US_Buoyancy::write_auto (void)
{
  ...
  for (ta = triple_name_to_total_area.begin ();
       ta != triple_name_to_total_area.end (); ++ta)
    {
      qDebug () << "triple, value -- " << ta.key () << ta.value ();
      te->e->append ("Triple: " + ta.key ()
                     + ";\t Total OD, Volumetric Measure (corrected for "
                       "centerpiece shape): "
                     + QString::number (ta.value ()));
      te->e->append (
          "Triple: " + ta.key ()
          + ";\t Total OD, Area Measure (un-corrected for centerpiece shape): "
          + QString::number (
              triple_name_to_total_area_uncorrected[ta.key ()]));
      te->e->append ("\n");
    }
  te->e->append ("\n\n");
  ...
  for (int i = 0; i < dpoint.size (); i++)
    {
      ...
      te->e->append ("Peak vbar:\t\t" + str.setNum (dpoint[i].peakVbar)
                     + " ml/g");
      te->e->append ("Peak Gauss area (% of total):\t\t"
                     + QString::number (dpoint[i].percentTotal, 'f', 2));
      te->e->append (
          "Peak Gauss area UNCORRECTED (% of total):\t"
          + QString::number (dpoint[i].percentTotal_uncorrected, 'f', 2));
      te->e->append ("Peak sigma value:\t\t" + str.setNum (dpoint[i].sigma)
                     + " cm");
...
void
US_Buoyancy::new_rpmval (int index)
{
...
// When sigma updated
void
US_Buoyancy::update_for_sigma (void)
{
  QString triple_n = cb_triple->itemText (current_triple);
...
// When MW updated
void
US_Buoyancy::update_for_MW (void)
{
  QString triple_n = cb_triple->itemText (current_triple);
...
// When Vbar updated
void
US_Buoyancy::update_for_vbar (void)
{
  QString triple_n = cb_triple->itemText (current_triple);
...
void
US_Buoyancy::update_for_dens_0 (void)
{
  QString triple_n = cb_triple->itemText (current_triple);
...
void
US_Buoyancy::update_meniscus (void)
{
  meniscus[current_triple]
      = (double)le_meniscus->text ().toDouble () - current_stretch;
}

void
US_Buoyancy::update_speedData (void)
{
  sData.clear ();
...
double
US_Buoyancy::calc_stretch ()
{
  return (simparams[current_triple].rotorcoeffs[0] * current_rpm
          + simparams[current_triple].rotorcoeffs[1] * pow (current_rpm, 2.0));
}

void
US_Buoyancy::closeEvent (QCloseEvent *event)
{
  emit widgetClosed ();
  event->accept ();
}
