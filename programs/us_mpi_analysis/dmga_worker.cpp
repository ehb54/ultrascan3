...
void
US_MPI_Analysis::dmga_worker (void)
{
      ...
      if (group_rank < 2)
        {
          ...
          for (int ii = 0; ii < nfvari; ii++)
            DbgLv (1) << my_rank << "  smask" << ii + 1 << "count"
                      << lfvari[ii];
        }
...
void
US_MPI_Analysis::dmga_worker_loop (void)
{
  // Initialize genes
  dgenes.clear ();
              ...
              for (int ii = 0; ii < nfloatc; ii++)
                {
                  ...
                  if (difv > NEAR_MATCH)
                    {
                      DbgLv (1) << "gw:" << my_rank
                                << ":  Dup NOT cleaned: f0 f1 fit0 fit1" << f0
                                << f1 << fitness[f0].fitness
                                << fitness[f1].fitness << "ii g0 g1 g0v g1v"
                                << ii << g0 << g1 << val0 << val1;
                      match = false;
                      f0 = f1;
                      break;
      ...
      if (generation > 0 && (generation % g_redo_inc) == 0)
        { // Set up to re-do initial genes after minimization
          ...
          for (int gg = 0; gg < population; gg++)
            old_genes[gg] = dgene;
          if ((group_rank % 10) == 1)
            DbgLv (0) << "gw:" << my_rank << ": re-do genes at gen"
                      << generation << "gen fitness" << fit0
...
DGene
US_MPI_Analysis::new_dmga_gene (void)
{
  ...
  for (int ii = 0; ii < nfloatc; ii++)
    {
      ...
      double vmin = cns_flt[ii].low;
      double vmax = cns_flt[ii].high;
      DbgLv (1) << my_rank << "dg:newg:  ii" << ii << "vmin vmax" << vmin
                << vmax;
...
void
US_MPI_Analysis::mutate_dgene (DGene &dgene)
{
  ...
  for (int ii = 0; ii < nfloatc; ii++)
    {
      ...
      DbgLv (1) << my_rank << "dg:mutg:  ii" << ii << "vl vh" << vl << vh
                << "vv" << vv;
...
void
US_MPI_Analysis::cross_dgene (DGene &dgene, QList<DGene> dgenes)
{
...
int
US_MPI_Analysis::migrate_dgenes (void)
{
...
double
US_MPI_Analysis::get_fitness_v_dmga (US_Vector &vv, US_Vector &zz)
{
  int vsize = vv.size ();
...
void
US_MPI_Analysis::lamm_gsm_df_dmga (US_Vector &vv, US_Vector &vd, US_Vector &zz)
{
...
double
US_MPI_Analysis::minimize_dmga (DGene &dgene, double fitness)
{
  ...
  for (int ii = 0; ii < vsize; ii++)
    {
      ...
      zz.assign (ii, vnorm);
      DbgLv (1) << my_rank << "dg:IHM:  ii" << ii << "vval vnorm" << vval
                << vnorm << "vpwr" << vpwr << "vvi" << vv[ii];
    }
...
double
US_MPI_Analysis::get_fitness_dmga (DGene &dgene)
{
...
QString
US_MPI_Analysis::dgene_key (DGene &dgene)
{
...
void
US_MPI_Analysis::calc_residuals_dmga (int offset, int dset_count,
      ...
      if (group_rank < 2 && dbg_level > 0)
        {
          ...
          DbgLv (1) << my_rank << "CR: dens visc scorr dcorr" << sd.density
                    << sd.viscosity << sd.s20w_correction
                    << sd.D20w_correction;
...
QString
US_MPI_Analysis::par_key_value (const QString kvtext, const QString key)
{
  QString value ("");
  int keyx = kvtext.indexOf (key);
  value = (keyx < 0) ? value : QString (kvtext).mid (keyx).section (" ", 1, 1);
  if (group_rank < 2)
    DbgLv (0) << my_rank << ": p_k_v  key" << key << "value" << value
              << "kvtext" << kvtext;
