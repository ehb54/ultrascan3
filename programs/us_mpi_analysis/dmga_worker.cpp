...
void US_MPI_Analysis::dmga_worker(void) {
   ...
   if (group_rank == 1)
      DbgLv(0) << my_rank << "dmga_worker: base_sig" << base_sig << "g_redo_inc" << g_redo_inc;
   ...
   DbgLv(1) << my_rank << "dmga_worker: nfloatc nfvari do_astfem" << nfloatc << nfvari << do_astfem;
   ...
   while (!finished) {
      ...
      DbgLv(1) << my_rank << "dmgw: job offs count len" << dataset << count << length << "tag" << status.MPI_TAG;
      int index = 0;
...
void US_MPI_Analysis::dmga_worker_loop(void) {
   ...
   for (generation = 0; generation < generations; generation++) {
      ...
      for (int ii = 0; ii < population; ii++) {
         ...
         DbgLv(1) << my_rank << "dg:getf:  ii" << ii << "  fitness" << fitness[ ii ].fitness;
      }
      ...
      if (generation == generations - 1) {
         ...
         fitness[ 0 ].fitness = minimize_dmga(dgenes[ fitness[ 0 ].index ], fitness[ 0 ].fitness);
      ...
      if ((deme_nbr % 10) == 1) {
         DbgLv(1) << "Best gene to master: gen" << generation << "worker" << deme_nbr << "fitness"
                  << fitness[ 0 ].fitness;
            ...
            for (int ii = 0; ii < nfloatc; ii++) {
               ...
               if (difv > NEAR_MATCH) {
                  DbgLv(1) << "gw:" << my_rank << ":  Dup NOT cleaned: f0 f1 fit0 fit1" << f0 << f1
                           << fitness[ f0 ].fitness << fitness[ f1 ].fitness << "ii g0 g1 g0v g1v" << ii << g0 << g1
                           << val0 << val1;
                  match = false;
                  f0 = f1;
                  break;
      ...
      if (generation > 0 && (generation % g_redo_inc) == 0) { // Set up to re-do initial genes after minimization
         ...
         for (int ii = 0; ii < nfloatc; ii++) { // Insure all the new gene attribute values are inside range
            ...
            if (aval < vmin || aval > vmax) {
               if ((group_rank % 10) == 1)
                  DbgLv(0) << "gw:" << my_rank << ": re-do : ii" << ii << "aval vmin vmax" << aval << vmin << vmax;
         ...
         if (nlim > 0) { // If adjustments due to limits, recompute fitness
            ...
            if ((group_rank % 10) == 1)
               DbgLv(0) << "gw:" << my_rank << ": re-do :     fit1a fit1b" << fitness[ 0 ].fitness << fit1;
         ...
         for (int gg = 0; gg < population; gg++)
            old_genes[ gg ] = dgene;
         if ((group_rank % 10) == 1)
            DbgLv(0) << "gw:" << my_rank << ": re-do genes at gen" << generation << "gen fitness" << fit0
      ...
      DbgLv(1) << "gw:" << my_rank << ": immigr_count" << immigr_count << "dgenes,old sizes" << dgenes.size()
               << old_genes.size() << population;
      ...
      for (int gg = elitism + immigr_count; gg < population; gg++) {
         ...
         DbgLv(1) << "gw:" << my_rank << ":  gg" << gg << "gene_index" << gene_index << "probability" << probability;
...
DGene US_MPI_Analysis::new_dmga_gene(void) {
   ...
   for (int ii = 0; ii < nfloatc; ii++) {
      ...
      DbgLv(1) << my_rank << "dg:newg:   ii" << ii << "p_grid" << p_grid << "delta" << delta << "aval" << aval;
...
void US_MPI_Analysis::mutate_dgene(DGene &dgene) {
   ...
   for (int ii = 0; ii < nfloatc; ii++) {
      ...
      DbgLv(1) << my_rank << "dg:mutg:  ii" << ii << "vl vh" << vl << vh << "vv" << vv;
      ...
      vv = qMin(vv, vh);
      vv = logscl ? exp(vv) : vv;
      DbgLv(1) << my_rank << "dg:mutg:  ii" << ii << "  xx" << xx << "delta" << delta << "vv" << vv;
...
int US_MPI_Analysis::migrate_dgenes(void) {
   ...
   for (int ii = 0; ii < migrate_count; ii++) { // Build the current list of emmigrating genes
      int gene_index = e_random();
   ...
   // MPI send emmigrants
   MPI_Send(
      emmigrants.data(), // to MPI #1
      doubles_count, MPI_DOUBLE, MPI_Job::MASTER, EMMIGRATE, my_communicator);
   ...
   MPI_Recv(
      immigres.data(), // to MPI #1
      doubles_count, MPI_DOUBLE, MPI_Job::MASTER, IMMIGRATE, my_communicator, &status);
...
double US_MPI_Analysis::get_fitness_v_dmga(US_Vector &vv, US_Vector &zz) {
   ...
   for (int ii = 0; ii < vsize; ii++) { // De-normalize values and put into marker QVector<double>
...
void US_MPI_Analysis::lamm_gsm_df_dmga(US_Vector &vv, US_Vector &vd, US_Vector &zz) {
...
double US_MPI_Analysis::minimize_dmga(DGene &dgene, double fitness) {
   ...
   for (int ii = 0; ii < vsize; ii++) {
      ...
      zz.assign(ii, vnorm);
      DbgLv(1) << my_rank << "dg:IHM:  ii" << ii << "vval vnorm" << vval << vnorm << "vpwr" << vpwr << "vvi"
               << vv[ ii ];
   }
   ...
   for (int ii = 0; ii < vsize; ii++) {
      dgmarker[ ii ] = vv[ ii ] / zz[ ii ];
      DbgLv(1) << my_rank << "dg:IHM: ii" << ii << "vvi zzi dgmi" << vv[ ii ] << zz[ ii ] << dgmarker[ ii ];
   }
...
double US_MPI_Analysis::get_fitness_dmga(DGene &dgene) {
   ...
   if (fitness_map.contains(fkey)) { // We already have a match to this key, so use its fitness value
      fitness_hits++;
...
void US_MPI_Analysis::calc_residuals_dmga(int offset, int dset_count, SIMULATION &sim_vals, DGene &dgene) {
   ...
   for (int ee = offset; ee < lim_offs; ee++) {
      ...
      for (int cc = 0; cc < model.components.size();
           cc++) { // Loop to perform data corrections to s and D (experimental space)
      ...
      if (group_rank < 2 && dbg_level > 0) {
         ...
         DbgLv(1) << my_rank << "CR: dens visc scorr dcorr" << sd.density << sd.viscosity << sd.s20w_correction
                  << sd.D20w_correction;
