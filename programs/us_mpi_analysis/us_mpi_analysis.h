...
#define DbTiming \
  if (dbg_timing) qDebug()  //!< debug-timing-conditioned qDebug()
...
class US_MPI_Analysis : public QObject {
 ...
 private:
  ...
  int max_iterations;       //!< Maximum number of iterations (Master only -
                            //!< Iterative)
  ...
  const double min_variance_improvement =
      1.0e-100;  //!< Minimum variance improvement
#else
  static const double min_variance_improvement =
      1.0e-100;  //!< Minimum variance improvement
#endif
  ...
  US_DataIO::RawData*
      res_data;  //!< Residual data (populated in calc_residuals)
  US_DataIO::RawData*
      sim_data;  //!< Simulation data (populated in calc_residuals)
  US_DataIO::RawData
      sim_data1;  //!< Simulation data for Monte Carlo iteration 1
...
#endif
  static const int solute_doubles =
      sizeof(US_Solute) / sizeof(double);  //!< Solute doubles
  static const int zsolut_doubles =
      sizeof(US_ZSolute) / sizeof(double);        //!< Z solute doubles
  QList<QVector<US_Solute> > calculated_solutes;  //!< Calculated solutes list
  QList<QVector<US_Solute> >
      ds_calc_solutes;                   //!< Dataset calculated solutes list
  QVector<US_Solute> dset_calc_solutes;  //!< Dataset calculated solutes
  QList<QVector<US_ZSolute> >
      calculated_zsolutes;  //!< Calculated Z solutes list
  ...
  QList<Fitness>
      best_fitness;  //!< Best fitness list (size is number of processors)

  //! \class MPI_GA_MSG
  //! \brief Class representing a message for genetic algorithm MPI.
  class MPI_GA_MSG {
   public:
    int generation;  //!< Generation count (from worker: -1 = final; from
                     //!< master: 1 = done)
    int size;  //!< Number of solutes in the following vector or genes requested
               //!< for emigration
  ...
  void vector_scaled_sum(US_Vector&, US_Vector&, double, US_Vector&,
                         double = 1.0);
  void solutes_from_gene(Gene&, int);
  void set_comp_attrib(US_Model::SimulationComponent&, double, int);
  void build_component(US_Model::SimulationComponent&, US_Math2::SolutionData&,
                       double, double);
  ...
  bool store_attr_value(double&, US_Model&, US_dmGA_Constraints::AttribType&,
                        int&);
  bool fetch_attr_value(double&, US_Model&, US_dmGA_Constraints::AttribType&,
                        int&);
