...
US_MPI_Analysis::US_MPI_Analysis(int nargs, QStringList &cmdargs) : QObject() {
   ...
   for (int jj = 1; jj < nargs; jj++) {
      ...
      if (cmdarg.startsWith("-")) { // Argument pair is keyed set (.e.g., "-maxwall <n>")
         QString cmdval = "";
         int valx = cmdarg.indexOf("=");

         if (valx > 0) { // Syntax is "-key=value"
            cmdval = cmdarg.mid(valx + 1).simplified();
         }

         else if ((jj + 1) < nargs) { // Syntax is "-key value"
            cmdval = cmdargs[ ++jj ];
         }

         if (cmdarg.contains(wallkey) || cmdarg.contains(wallkey2) || cmdarg.contains(wallkey3)) { // Get maximum wall time in minutes
            task_params[ "walltime" ] = cmdval;
         }

         else if (
            cmdarg.contains(pmgckey) || cmdarg.contains(pmgckey2)
            || cmdarg.contains(pmgckey3)) { // Get number of parallel masters groups
            task_params[ "mgroupcount" ] = cmdval;
         }
         else if (
            cmdarg.contains(sgszkey) || cmdarg.contains(sgszkey2)
            || cmdarg.contains(sgszkey3)) { // Get minimum subgrid size in solute points
      ...
      else if (tarfile.isEmpty()) { // First non-keyed argument is tar file path
         tarfile = cmdarg;
      }

      else { // Second non-keyed argument is jobxmlfile
         jxmlfili = cmdarg;
      }
   }
   if (my_rank == 0) {
   ...
   if (files.size() != 1)
      abort("Could not find unique hpc input file.");
   ...
   if (jxmlfile.isEmpty()) { // Not on command line: just look for its presence in the work directory
      ffilt.clear();
      ffilt << "*jobxmlfile.xml";
      ffilt << "jobxmlfile.xml";
      QStringList jfiles = QDir(input_dir).entryList(ffilt, QDir::Files);
      if (my_rank == 0)
         DbgLv(0) << "  jfiles size" << jfiles.size();

      if (jfiles.size() > 0) { // Files found in ./input
         jxmlfile = input_dir + "/" + jfiles[ 0 ];
      }

      else { // Files not found in ./input, so try base work directory
         ffilt << "us3.pbs";
   ...
   if (my_rank == 0) { // Save submit time
      submitTime = QFileInfo(tarfile).lastModified();
      DbgLv(0) << "submitTime " << submitTime << " mgroupcount" << task_params[ "mgroupcount" ].toInt() << " walltime"
               << task_params[ "walltime" ].toInt();
   ...
   if (my_rank == 0) {
      DbgLv(0) << "FMB: fit_mb_select" << fit_mb_select << "    fit_meni _bott _menbot" << fit_meni << fit_bott
               << fit_menbot;
   }

   if (fit_menbot) { // Meniscus-and-bottom fit iterations will be run
      bottom_range = meniscus_range;
      bottom_points = meniscus_points;
      menibott_count = meniscus_points * bottom_points;
   }

   else if (fit_meni) { // Meniscus-only fit iterations will be run
      bottom_range = 0.0;
      bottom_points = 1;
      menibott_count = meniscus_points;
   }

   else if (fit_bott) { // Bottom-only fit iterations will be run
      bottom_range = meniscus_range;
      bottom_points = meniscus_points;
      meniscus_range = 0.0;
      meniscus_points = 1;
   ...
   if (my_rank == 0) {
      DbgLv(0) << "FMB: meni range points" << meniscus_range << meniscus_points << "  bott range points" << bottom_range
               << bottom_points;
   }
   ...
   if (is_global_fit) {
      if (data_sets[ 0 ]->simparams.speed_step.count() > 1) { // Change concentrations back to 1.0 if multi-speed/global-fit
   ...
   bool noise = parameters[ "tinoise_option" ].toInt() > 0 || parameters[ "rinoise_option" ].toInt() > 0;

   if (!analysis_type.startsWith("PCSA") && mc_iterations > 1 && noise) {
   ...
   double meniscus_start = data_sets[ 0 ]->run_data.meniscus - meniscus_range / 2.0;

   double dmen = fit_meni ? (meniscus_range / (meniscus_points - 1)) : 0.0;
   ...
   if (bottom_ds == 0.0) {
      ...
      bottom_ds = US_AstfemMath::calc_bottom(rpm, ds->centerpiece_bottom, ds->rotor_stretch);
   }
   double bottom_start = bottom_ds - bottom_range / 2.0;

   double dbot = fit_bott ? (bottom_range / (bottom_points - 1)) : 0.0;
   ...
   for (int ee = 0; ee < data_sets.size(); ee++) {
      ...
      if (my_rank == 0)
         DbgLv(0) << "density/viscosity/comm vbar20/commvbar/compress" << sd.density << sd.viscosity << sd.vbar20
                  << sd.vbar << ds->compress;
      ...
      if (my_rank == 0)
         DbgLv(0) << "s20w_correction/D20w_correction" << sd.s20w_correction << sd.D20w_correction;
      ...
      // Use bottom from edited data if it is given
      double bottom_ds = (edata->bottom > 0.0) ? edata->bottom : ds->centerpiece_bottom;
      if (my_rank == 0)
         DbgLv(0) << "incl_speed" << incl_speed << "nssp" << nssp << "bottom_ds bottom_cp" << bottom_ds
                  << ds->centerpiece_bottom;

      if (!incl_speed && !redo_ss) { // If experiment speed step used, test against data set times
         ...
         if (etm1 < stm1 || etm2 > stm2) { // If data time range outside that of speed step, flag it
            redo_ss = true;
         }
      }
      ...
      if (!ds->tmst_file.isEmpty()) { // Dataset has non-empty timestate file string
         ntmsf_ne++;
         QString tmst_fpath = "../" + ds->tmst_file;
         QFileInfo check_file(tmst_fpath);

         if ((check_file.exists()) && (check_file.isFile())) { // Dataset timestate file exists
            ntmsf_ex++;
            US_DataIO::RawData simdat;
            US_AstfemMath::initSimData(simdat, *edata, 0.0);
            if (US_AstfemMath::timestate_onesec(tmst_fpath, simdat)) { // Timestate file is at 1-second-interval
               ntmsf_1s++;
            }
            else { // Existing timestate file is not at 1-second-interval
            }
         }
      }
      else { // Dataset's timestate file string is empty
         if (ee > 0 && !ds0->tmst_file.isEmpty()) { // Dataset 0's timestate file is not empty:  copy
   ...
   if (
      (ntmsf_ne == 0) || // No non-empty tmst_files
   ...
   DbgLv(0) << "rank" << my_rank << ": redo_tstate" << redo_tstate << "ntmsf_ne ntmsf_ex ntmsf_1s" << ntmsf_ne
            << ntmsf_ex << ntmsf_1s;
   //DbgLv(0) << "rank" << my_rank << ": barrier2 :TM:" << ELAPSED_SECS;

   if (redo_tstate) { // We need to re-do the timestate files

      if (my_rank == 0) { // Do any writing or re-writing of timestates in master process
         QStringList tmsf_fs;

         bool is_sim = (data_sets[ 0 ]->run_data.channel == "S");
         DbgLv(0) << "rank" << my_rank << ": is_sim" << is_sim << "chan" << data_sets[ 0 ]->run_data.channel;

         for (int ee = 0; ee < count_datasets; ee++) { // Examine each data set for associated timestate
            US_SolveSim::DataSet *ds = data_sets[ ee ];
            ...
            if (ntmsf_ne == 0) { // No tmst_file given:  create one
               tmst_fpath = "../" + ds->run_data.runID + ".time_state.tmst";
               tmst_fdefs = QString(tmst_fpath).replace(".tmst", ".xml");

               if (ee == 0) { // First (only) dataset:  create a tmst file set
                  US_DataIO::RawData simdat;
               ...
               else if (!tmsf_fs.contains(tmst_fpath)) { // Not-first dataset, but unique file:  create another file set
                  US_DataIO::RawData simdat;
                  US_AstfemMath::initSimData(simdat, ds->run_data, 0.0);
                  US_AstfemMath::writetimestate(tmst_fpath, ds->simparams, simdat);
                  tmsf_fs << tmst_fpath;
            ...
            else if (ntmsf_ex == 1 && ntmsf_1s == 0) { // Existing tmst_file given, but not at 1-second interval
            ...
            else if (ntmsf_ex > 1 && ntmsf_1s < ntmsf_ex) { // Existing tmst_files given, but not all at 1-second interval
               tmst_fpsav = tmst_fpath + "-orig";
               tmst_fdsav = tmst_fdefs + "-orig";
               QString tmst_fpold = tmst_fpath;
               QString tmst_fdold = tmst_fdefs;
               ...
               if (!tmsf_fs.contains(tmst_fpath) && !QFile(tmst_fpath).exists()) { // Unique data set timestate file that does not yet exist
                  QFile::rename(tmst_fpold, tmst_fpsav);
                  QFile::rename(tmst_fdold, tmst_fdsav);
                  // Create a new 1-second-interval file set
                  US_DataIO::RawData simdat;
   ...
   for (int ee = 0; ee < count_datasets; ee++) {
      ...
      ds->tmst_file = ds->tmst_file.isEmpty()
                         ...
                         : (ds->tmst_file.startsWith("../") ? ds->tmst_file : "../" + ds->tmst_file);
      ...
      DbgLv(0) << "rank" << my_rank << ": ee" << ee << "tmst_file" << ds->tmst_file << "ssp count"
               << ds->simparams.sim_speed_prof.count();
...
if ( my_rank == 0 )
 DbgLv(0) << "rank" << my_rank << ": ee" << ee << "bottom_pos" << bottom_pos
  ...
  << "adj.bottom_pos" << ds->simparams.bottom_position;
   ...
   if (redo_ss) { // If speed step re-do flagged, set all speed steps from data
      for (int ee = 0; ee < data_sets.size(); ee++) {
         US_SolveSim::DataSet *ds = data_sets[ ee ];

         ds->simparams.computeSpeedSteps(&ds->run_data.scanData, ds->simparams.speed_step);
         if (my_rank == 0)
   ...
   for (int ee = 0; ee < data_sets.size(); ee++) {
      ...
      if (my_rank == 0) {
         DbgLv(1) << "DSM: ssck: rspeed accel1 tf_aend tf_scan" << rspeed << accel1 << tf_aend << tf_scan << "accel2"
                  << accel2 << "lo_ss_acc" << lo_ss_acc;
      }
      ...
      if (accel1 < lo_ss_acc || tf_aend > (tf_scan + 1)) {
         DbgLv(0) << "rank: " << my_rank << "  Dataset " << ee << " Name: " << dset->run_data.runID
                  << " likely bad Timestate. Implied acceleration: " << accel1 << " accel end: " << tf_aend
                  << "s. First scan: " << tf_scan << "s.";
      }
   }
   ...
   if (analysis_type == "GA") {
      ...
      for (int i = 0; i < buckets.size(); i++) {
         ...
         bucket_rects << QRectF(
            QPointF(buckets[ i ].x_min, buckets[ i ].y_max), QPointF(buckets[ i ].x_max, buckets[ i ].y_min));
         ...
         for (int j = i + 1; j < bucket_rects.size(); j++) {
            if (bucket_rects[ i ].intersects(bucket_rects[ j ])) {
               ...
               if (my_rank == 0) {
                  ...
                  DbgLv(0) << " Overlap: st w h" << bukov.topLeft() << sdif << fdif;
                  DbgLv(0) << "  Bucket i" << bucket_rects[ i ].topLeft() << bucket_rects[ i ].bottomRight();
                  DbgLv(0) << "  Bucket j" << bucket_rects[ j ].topLeft() << bucket_rects[ j ].bottomRight();
                  DbgLv(0) << "  Bucket i right " << QString().sprintf("%22.18f", bucket_rects[ i ].right());
                  DbgLv(0) << "  Bucket j left  " << QString().sprintf("%22.18f", bucket_rects[ j ].left());
                  DbgLv(0) << "  Bucket i bottom" << QString().sprintf("%22.18f", bucket_rects[ i ].bottom());
                  DbgLv(0) << "  Bucket j top   " << QString().sprintf("%22.18f", bucket_rects[ j ].top());
               }

               if (qMin(sdif, fdif) < 1.e-6) { // Ignore the overlap if it is trivial
                  if (my_rank == 0)
   ...
   if (attr_x != ATTR_S) { // We will need to determine the maximum s value
      if (attr_y == ATTR_S) // s_max may be y_max
         s_max = y_max * 1.0e-13;
      else if (attr_z == ATTR_S) // s_max may be fixed value
         s_max = zval * 1.0e-13;
      else if (analysis_type == "GA") { // s_max may need computing
         s_max = 1e-39;
         ...
         for (int jj = 0; jj < buckets.size(); jj++) { // Compute s using given x,y,fixed and find the max
            ...
            if (my_rank == 0)
               DbgLv(0) << "ckGrSz:  buck" << jj << "xv yv zv" << xval << yval << zval << "mc s k w v d f" << mcomp.s
                        << mcomp.f_f0 << mcomp.mw << mcomp.vbar20 << mcomp.D << mcomp.f;
   ...
   if (my_rank == 0)
      DbgLv(0) << "ckGrSz: s_max" << s_max << "attr_x,y,z" << attr_x << attr_y << attr_z;
   ...
   if (my_rank == 0)
      DbgLv(0) << "ckGrSz: ssp count" << data_sets[ 0 ]->simparams.sim_speed_prof.count() << "g_too_big" << g_too_big;
   if (g_too_big) {
      if (my_rank == 0) {
         qDebug() << smsg;
...
void US_MPI_Analysis::send_udp(const QString &message) {
   ...
   if (my_rank == 0) { // Send UDP message from supervisor (or single-group master)
      ///////////////////////////////*DEBUG*
   ...
   else if (group_rank == 0) { // For pm group master, forward message to supervisor
      int super = 0;
      QString gpfix = QString("(pmg %1) ").arg(my_group);
      msg = QString(gpfix + message).toLatin1();
      int size = msg.size();

      MPI_Send(&size, sizeof(int), MPI_BYTE, super, UDPSIZE, MPI_COMM_WORLD);

      MPI_Send(msg.data(), size, MPI_BYTE, super, UDPMSG, MPI_COMM_WORLD);
   }
}
...
void US_MPI_Analysis::abort(const QString &message, int error) {
   if (my_rank == 0) { // Send abort message to both stdout and udp
   ...
   if (my_rank == 0) { // Create archive file of outputs and remove other output files
      update_outputs(true);
   }
...
void US_MPI_Analysis::stats_output(
   int walltime, int cputime, int maxrssmb, QDateTime submitTime, QDateTime startTime, QDateTime endTime) {
...
void US_MPI_Analysis::limitBucket(Bucket &buk) {
   if (buk.x_min > 0.0) { // All-positive s's start at 0.1 at least (other attribs are different)
      double xmin = 0.1;
   ...
   else if (buk.x_max <= 0.0) { // All-negative s's end at -0.1 at most
      buk.x_max = qMin(-0.1, buk.x_max);
      buk.x_min = qMin((buk.x_max - 0.0001), buk.x_min);
   }

   else if ((buk.x_min + buk.x_max) >= 0.0) { // Mostly positive clipped to all positive starting at 0.1
      buk.x_min = 0.1;
      buk.x_max = qMax(0.2, buk.x_max);
   }

   else { // Mostly negative clipped to all negative ending at -0.1
      buk.x_min = qMin(-0.2, buk.x_min);
      buk.x_max = -0.1;
   }

   if (attr_y == ATTR_K) { // If y-type is "ff0", insure minimum is at least 1.0
...
void US_MPI_Analysis::dset_matrices(
   int dsx, int *nsolutesP, QVector<double> &nnls_a, QVector<double> &nnls_b, QVector<int> &c_used) {
   double norm_cut = 1.0;
   ...
   DbgLv(0) << "DSM: dsx" << dsx << "rowx ndspts" << rowx << ndspts << "nsolutes" << nsolutes << "concen" << concen
            << "inccx" << inccx;
   ...
   for (int cc = 0; cc < nsolutes; cc++) {
      ...
      for (int pp = 0; pp < ndspts; pp++, ka++, jj++) { // Copy A column and accumulate column norm
   ...
   if (ksolutes < nsolutes) { // Norm tolerance has excluded some columns
...
void US_MPI_Analysis::calc_residuals(int offset, int dataset_count, SIMULATION &simu_values) {
   count_calc_residuals++;
   bool do_dbg = (dbg_level > 0 && (group_rank < 2 || group_rank == (proc_count / 2)));
   if (do_dbg)
      DbgLv(1) << "w:" << my_rank << ": CALC_RES : count" << count_calc_residuals << "offs dsknt" << offset
               << dataset_count;
   ...
   if (do_dbg)
      DbgLv(1) << "w:" << my_rank << ":nsoli" << nsoli << "nsolz" << nsolz;
   ...
   if (do_dbg)
      DbgLv(1) << "w:" << my_rank << ":nsoli" << nsoli << "nsolz" << nsolz;
   if (do_dbg)
      simu_values.dbg_level = qMax(simu_values.dbg_level, 1);
   //*DEBUG*
   ...
   for (int jj = 0; jj < simu_values.solutes.size(); jj++)
      if (simu_values.solutes[ jj ].c > 1.0)
         hicee = true;
   ...
   if (do_dbg || hicee) {
      ...
      for (int ss = 0; ss < nsc; ss++)
         for (int rr = 0; rr < nrp; rr++)
            dt += edat->scanData[ ss ].rvalues[ rr ];
      ...
      DbgLv(1) << "w:" << my_rank << ":  simpt men bott temp coef1" << dset->simparams.simpoints
               << dset->simparams.meniscus << dset->simparams.bottom << dset->simparams.temperature
               << dset->simparams.rotorcoeffs[ 0 ];
      DbgLv(1) << "w:" << my_rank << ":  vbar soltype manual visc dens" << dset->vbar20 << dset->solute_type
               << dset->manual << dset->viscosity << dset->density;
      DbgLv(1) << "w:" << my_rank << ":  noisf alpha" << simu_values.noisflag << simu_values.alpha
               << "s20w_c D20w_c vbar" << dset->s20w_correction << dset->D20w_correction << dset->vbar20;
      if (dataset_count > 1 && (offset + 1) < data_sets.size()) {
         dset = data_sets[ offset + 1 ];
         DbgLv(1) << "w:" << my_rank << ":  2)simpt men bott temp coef1" << dset->simparams.simpoints
                  << dset->simparams.meniscus << dset->simparams.bottom << dset->simparams.temperature
                  << dset->simparams.rotorcoeffs[ 0 ];
         DbgLv(1) << "w:" << my_rank << ":  2)vbar soltype manual visc dens" << dset->vbar20 << dset->solute_type
                  << dset->manual << dset->viscosity << dset->density;
         DbgLv(1) << "w:" << my_rank << ":  2)noisf alpha" << simu_values.noisflag << simu_values.alpha
                  << "s20w_c D20w_c vbar" << dset->s20w_correction << dset->D20w_correction << dset->vbar20;
      }
      int nn = isols.size() - 1;
      int mm = nn / 2;
      if (nn > mm)
         DbgLv(1) << "w:" << my_rank << ": sol0 solm soln" << isols[ 0 ].s << isols[ 0 ].k << isols[ 0 ].c
                  << isols[ mm ].s << isols[ mm ].k << isols[ mm ].c << isols[ nn ].s << isols[ nn ].k << isols[ nn ].c;
   }
   //*DEBUG*
}
...
void US_MPI_Analysis::write_output(void) {
   ...
   else if (mdl_type == US_Model::DMGA) { // Handle DMGA need for dummy solutes
   ...
   else if (mdl_type == US_Model::PCSA) { // PCSA: Order model records and pick best model
      max_depth = 0;
      qSort(mrecs);
      //*DEBUG*
      ...
      if (dbg_level > 0) {
         for (int jj = 0; jj < mrecs.size(); jj++) {
            DbgLv(1) << "M:wo: jj" << jj << "typ tx" << mrecs[ jj ].ctype << mrecs[ jj ].taskx << "isz csz"
                     ...
                     << "sy ey" << mrecs[ jj ].str_y << mrecs[ jj ].end_y << "p1 p2" << mrecs[ jj ].par1
                     << mrecs[ jj ].par2;
         }
      }
      //*DEBUG*
   ...
   if (mxdssz == 0) { // Handle the case of a zero-solute final model
      int simssz = simulation_values.zsolutes.size();
      int dm1ssz = max_depth > 0 ? calculated_solutes[ max_depth - 1 ].size() : 0;
      DbgLv(0) << "*WARNING* Final solutes size" << mxdssz << "max_depth" << max_depth << "Sim and Depth-1 solutes size"
               << simssz << dm1ssz;
      if (simssz > 0) { // Use the last result solutes if there are some
         sim.zsolutes = simulation_values.zsolutes;
         DbgLv(0) << "   SimValues solutes used";
      }
      else if (dm1ssz > 0) { // Else use the max-depth-minus-1 solutes if there are some
         //sim.zsolutes = calculated_solutes[ max_depth - 1 ];
         DbgLv(0) << "   CalcValue[mxdepth-1] solutes used";
      }
      else { // Else report the bad situation of no solutes available
         DbgLv(0) << "   *ERROR* No solutes will be used";
      }
   }

   DbgLv(1) << "WrO: meniscus_run" << meniscus_run << "mvsz" << meniscus_values.size();
   meniscus_value = meniscus_run < meniscus_values.size() ? meniscus_values[ meniscus_run ] : save_meniscus;
   bottom_value = bottom_run < bottom_values.size() ? bottom_values[ bottom_run ] : save_bottom;
   ...
   if (mdl_type == US_Model::PCSA) { // Output mrecs for PCSA, if we have a final mrecs vector
...
void US_MPI_Analysis::write_global(void) {
   ...
   DbgLv(0) << "WrGlob: mciter mxdepth" << mc_iteration + 1 << max_depth << "simvsols size" << nsolutes;
   if (nsolutes == 0) { // Handle the case of a zero-solute final model
   ...
   if (mdl_type == US_Model::TWODSA) {
      ...
      US_SolveSim solvesim(data_sets, my_rank, false);
      solvesim.calc_residuals(0, data_sets.size(), wksim_vals, false, &gl_nnls_a, &gl_nnls_b);
      DbgLv(0) << "WrGlob:  glob recompute nsols" << wksim_vals.solutes.size() << "globrec A,b sizes"
               << gl_nnls_a.size() << gl_nnls_b.size();
      ...
      for (int cc = 0; cc < nsolutes; cc++) {
         ...
         DbgLv(0) << "WrGlob:     cc" << cc << "conc-in" << gsim->solutes[ cc ].c << "SGconc-out"
                  << sim.solutes[ cc ].c;
      }

      write_superg(sim, mdl_type);

      DbgLv(0) << "WrGlob: dssize" << data_sets.size() << "concsize" << concentrations.size() << "simsolssz"
               << sim.solutes.size() << "gsimsolssz" << gsim->solutes.size() << nsolutes;
      ...
      for (int ee = 0; ee < data_sets.size(); ee++) {
         ...
         for (int cc = 0; cc < nsolutes; cc++) {
            ...
            DbgLv(0) << "WrGlob:     cc" << cc << "conc-in" << gsim->solutes[ cc ].c << "conc-out"
                     << sim.solutes[ cc ].c;
         }
         ...
         for (int cc = 0; cc < nsolutes; cc++) {
            ...
            if (soluval > 0.0) {
               ...
               if (soluval > 1.0) {
                  ...
                  DbgLv(0) << "WrGlob:      A lo,hi,av" << avlo << avhi << avav << "B lo,hi,av" << bvlo << bvhi << bvav;
               }
            }
         }
   ...
   else if (mdl_type == US_Model::GA) { // GA:  Compute and output each dataset model
      //      wksim_vals           = simulation_values;
      US_SolveSim solvesim(data_sets, my_rank, false);
      solvesim.calc_residuals(0, data_sets.size(), wksim_vals, false, &gl_nnls_a, &gl_nnls_b);
      DbgLv(1) << "WrGlob:  glob recompute nsols" << wksim_vals.solutes.size() << "globrec A,b sizes"
               << gl_nnls_a.size() << gl_nnls_b.size();
      ...
      for (int cc = 0; cc < nsolutes; cc++) {
         sim.solutes[ cc ].c = gsim->solutes[ cc ].c * avg_conc;
         DbgLv(1) << "WrGlob:     cc" << cc << "conc-in" << gsim->solutes[ cc ].c << "SGconc-out"
                  << sim.solutes[ cc ].c;
      }

      write_superg(sim, mdl_type);
      DbgLv(1) << "WrGlob: dssize" << data_sets.size() << "concsize" << concentrations.size() << "simsolssz"
               << sim.solutes.size() << "gsimsolssz" << gsim->solutes.size() << nsolutes;
      ...
      for (int ee = 0; ee < data_sets.size(); ee++) {
         ...
         for (int cc = 0; cc < nsolutes; cc++) {
            sim.solutes[ cc ].c = gsim->solutes[ cc ].c * concentration;
            DbgLv(1) << "WrGlob:     cc" << cc << "conc-in" << gsim->solutes[ cc ].c << "conc-out"
                     << sim.solutes[ cc ].c;
         }
   ...
   else if (mdl_type == US_Model::PCSA) { // PCSA: Recompute the global fit and save A and b matrices for later use
      US_SolveSim solvesim(data_sets, my_rank, false);
      solvesim.calc_residuals(0, data_sets.size(), wksim_vals, false, &gl_nnls_a, &gl_nnls_b);
      nsolutes = gsim->zsolutes.size();
      DbgLv(1) << "WrGlob:  glob recompute nzsols" << wksim_vals.zsolutes.size() << nsolutes << "globrec A,b sizes"
               << gl_nnls_a.size() << gl_nnls_b.size();
      double dsum_b = 0.0;
      for (int ii = 0; ii < gl_nnls_b.size(); ii++)
         dsum_b += gl_nnls_b[ ii ];
      ...
      for (int cc = 0; cc < nsolutes; cc++) {
         ...
         DbgLv(1) << "WrGlob:     cc" << cc << "conc-in" << gsim->zsolutes[ cc ].c << "SGconc-out"
                  << sim.zsolutes[ cc ].c;
      }
      ...
      DbgLv(1) << "WrGlob: dssize" << data_sets.size() << "concsize" << concentrations.size() << "simsolssz"
               << sim.zsolutes.size() << "gsimsolssz" << gsim->zsolutes.size() << nsolutes;
      ...
      for (int ee = 0; ee < data_sets.size(); ee++) {
         ...
         for (int cc = 0; cc < nsolutes; cc++) {
            ...
            DbgLv(1) << "WrGlob:     cc" << cc << "conc-in" << gsim->zsolutes[ cc ].c << "conc-out"
                     << sim.zsolutes[ cc ].c;
         }
         ...
         dset_matrices(ee, &nsolutes, nnls_a, nnls_b, c_used);
         double dsum_b = 0.0;
         for (int ii = 0; ii < narows; ii++)
            dsum_b += nnls_b[ ii ];
         DbgLv(1) << "WrGlob:      ee" << ee << "dsum_b" << dsum_b << "stx pts" << ds_startx[ ee ] << ds_points[ ee ]
                  << narows << "conc" << concentrations[ ee ];
         dsum_b_all += dsum_b;
         ...
         DbgLv(1) << "WrGlob:    currds" << ee << "nsol ksol" << nsolutes << ksolutes << sim.zsolutes.size()
                  << "sum_sol" << sum_sol;
   ...
   else if (mdl_type == US_Model::DMGA) { // DMGA:  Compute and output each dataset model
   }
...
void US_MPI_Analysis::shutdown_all(void) {
   ...
   for (int i = 1; i <= my_workers; i++) {
      MPI_Send(
         &job, sizeof(job), MPI_BYTE,
         i, // Send to each worker
         MPI_Job::MASTER, my_communicator);

      maxrss += work_rss[ i ];
      DbgLv(1) << "2dsa master shutdown : worker" << i << " upd. maxrss" << maxrss << "  wkrss" << work_rss[ i ];
   }
}

// Write model output at the end of an iteration
void US_MPI_Analysis::write_model(const SIMULATION &sim, US_Model::AnalysisType type, bool glob_sols) {
   ...
   DbgLv(1) << "wrMo: type" << type << "(DMGA=" << US_Model::DMGA << ") (PCSA=" << US_Model::PCSA
            << ") subtype=" << subtype;
   if (type == US_Model::DMGA) { // For discrete GA, get the already constructed model
      model = data_sets[ 0 ]->model;
      DbgLv(1) << "wrMo:  model comps" << model.components.size();
   }

   int mc_iter = (mgroup_count < 2 || is_composite_job) ? (mc_iteration + 1) : mc_iteration;
   model.monteCarlo = mc_iterations > 1;
   model.wavelength = edata->wavelength.toDouble();
   model.modelGUID = (!model.monteCarlo || mc_iter == 1) ? US_Util::new_guid() : modelGUID;
   DbgLv(1) << "wrMo:  mc mciter mGUID" << model.monteCarlo << mc_iter << model.modelGUID;
   ...
   DbgLv(0) << "wrMo:  is_glob glob_sols" << is_global_fit << glob_sols << "f_men f_bot f_mbo" << fit_meni << fit_bott
            << fit_menbot << "m.glob" << model.global;
   ...
   if (mc_iterations > 1)
      ...
      iterID
         .sprintf("i%02d-m%05db%05d", menibott_ndx + 1, ( int ) (meniscus_value * 10000), ( int ) (bottom_value * 10000));
   else if (fit_meni)
      iterID.sprintf("i%02d-m%05d", meniscus_run + 1, ( int ) (meniscus_value * 10000));
   else if (fit_bott)
      iterID.sprintf("i%02d-b%05d", bottom_run + 1, ( int ) (bottom_value * 10000));
   else
      iterID = "i01";
   ...
   if (type == US_Model::PCSA) { // For PCSA, construct the model from zsolutes
   ...
   else if (type != US_Model::DMGA) { // For other non-DMGA, construct the model from solutes
   ...
   DbgLv(1) << "wrMo: stype" << stype << QString().sprintf("0%o", stype) << "attr_z vbar20 mco0.v" << attr_z << vbar20
            << model.components[ 0 ].vbar20;
   ...
   if (lenfn > 96) { // Insure a model file name less than 100 characters in length (tar limit)
   ...
   tsout << fn << ";meniscus_value=" << meniscus_value << ";MC_iteration=" << mc_iter << ";variance=" << sim.variance
         << ";run=" << runstring << ";bottom_value=" << bottom_value << "\n";
   fileo.close();
}

// Write noise output at the end of an iteration
void US_MPI_Analysis::write_noise(US_Noise::NoiseType type, const QVector<double> &noise_data) {
   ...
   QString anType = "_" + data_sets[ current_dataset ]->model.typeText() + "_";
   if (analysis_type.contains("CG"))
   ...
   noise.description = data->runID + "." + tripleID + "." + analysID + "." + type_name + "_noise";
   ...
   for (int j = 0; j < noise_filenames.size(); j++) {
      ...
      input_noise.load(fn);
      if (input_noise.type == type)
         noise.sum_noise(input_noise);
   }
...
int US_MPI_Analysis::ready_worker() {
   ...
   int w1 = worker;
   int w1n = worknext;
   worker = (worker > 0) ? worker : worker_status.indexOf(READY, 1);
   ...
   if (w1 < 0)
      DbgLv(1) << "ready_worker  w1234...wmn" << worker_status[ 1 ] << worker_status[ 2 ] << worker_status[ 3 ]
               << worker_status[ 4 ] << worker_status[ my_workers - 1 ] << worker_status[ my_workers ];
...
void US_MPI_Analysis::update_outputs(bool is_final) {
   ...
   DbgLv(0) << my_rank << ": UpdOut : final" << is_final << "files size" << files.size();

   if (files.size() == 1 && files[ 0 ] == "analysis-results.tar") { // If the tar file exists alone, do nothing here
   ...
   if (mc_iterations > 1) {
      ...
      for (int ii = 0; ii < mtrips.size(); ii++) {
         ...
         if (mtfiles.count() < mc_iterations)
            continue;
         ...
         int f_iters = QString(cmfname).section(".", -3, -3).mid(3).toInt();
         if (!tfiles.contains(cmfname) && (is_final || f_iters == mc_iterations)) {
            US_Model model2;
            DbgLv(0) << my_rank << ":      model2.load(" << cmfname << ")";
            model2.load(cmfname);
            DbgLv(0) << my_rank << ":       model2.description" << model2.description;
            QString runstring = "Run: " + QString::number(ii + 1) + " " + tripleID;
            model2.variance = rmsd_combined_mc_models(model2);
            tsout << cmfname << ";meniscus_value=" << model2.meniscus << ";MC_iteration=" << mc_iterations
                  << ";variance=" << model2.variance << ";run=" << runstring << "\n";
   ...
   if (is_final) { // Remove the files we just put into the tar archive
...
double US_MPI_Analysis::rmsd_combined_mc_models(US_Model &model) {
   ...
   int kpts = 0;
   DbgLv(1) << "Combined MC Model: nscans:" << nscans << "npoints:" << npoints << "fit_ti:" << fit_ti
            << "fit_ri:" << fit_ri << "size ti_noise:" << ti_noise.size() << "size ri_noise:" << ri_noise.size();
   for (int ii = 0; ii < nscans; ii++) {
      double rin = fit_ri ? ri_noise.at(ii) : 0.0;
      for (int jj = 0; jj < npoints; jj++) { // Calculate the residuals and the RMSD
...
US_Model::AnalysisType US_MPI_Analysis::model_type(const QString a_type) {
   US_Model::AnalysisType m_type = US_Model::TWODSA;
...
void US_MPI_Analysis::write_superg(const SIMULATION &sim, US_Model::AnalysisType type) {
