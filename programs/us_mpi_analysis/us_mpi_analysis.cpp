...
int main(int argc, char* argv[]) {
  ...
  for (int jj = 0; jj < argc; jj++) cmdargs << argv[jj];
...
US_MPI_Analysis::US_MPI_Analysis(int nargs, QStringList& cmdargs) : QObject() {
  ...
  for (int jj = 1; jj < nargs; jj++) {
    ...
    if (my_rank == 0) DbgLv(0) << "CmdArg: jj" << jj << "cmdarg" << cmdarg;
    ...
    if (cmdarg.startsWith(
      ...
      if (cmdarg.contains(wallkey) || cmdarg.contains(wallkey2) ||
          cmdarg.contains(wallkey3)) {  // Get maximum wall time in minutes
        task_params["walltime"] = cmdval;
      }

      else if (cmdarg.contains(pmgckey) || cmdarg.contains(pmgckey2) ||
               cmdarg.contains(
                   pmgckey3)) {  // Get number of parallel masters groups
        task_params["mgroupcount"] = cmdval;
      } else if (cmdarg.contains(sgszkey) || cmdarg.contains(sgszkey2) ||
                 cmdarg.contains(
                     sgszkey3)) {  // Get minimum subgrid size in solute points
  ...
  if (my_rank == 0) socket = new QUdpSocket(this);
  ...
  if (files.size() == 0) {
    ...
    if (files.size() == 1) xmlfile = input_dir + "/" + files[0];
  } else
    xmlfile = files[0];
  ...
  if (jxmlfile.isEmpty()) {  // Not on command line: just look for its presence
  ...
  if (my_rank == 0) {  // Save submit time
    ...
    DbgLv(0) << "submitTime " << submitTime << " mgroupcount"
             << task_params["mgroupcount"].toInt() << " walltime"
  ...
  for (int ii = 0; ii < data_sets.size(); ii++) {
    ...
    try {
      ...
      QString msg = "Bad data file " + dset->auc_file + " " + dset->edit_file;
      DbgLv(0) << "BAD DATA. ioError" << error << "rank" << my_rank
               << proc_count;
      // if(proc_count!=16)
      abort(msg, error);
    }
    ...
    dset->vbartb =
        US_Math2::calcCommonVbar(dset->solution_rec, dset->temperature);
  ...
  if (my_rank == 0) {
    DbgLv(0) << "FMB: fit_mb_select" << fit_mb_select
             << "    fit_meni _bott _menbot" << fit_meni << fit_bott
             << fit_menbot;
  }
  ...
  if (is_global_fit) {
    if (data_sets[0]->simparams.speed_step.count() >
        1) {  // Change concentrations back to 1.0 if multi-speed/global-fit
  ...
  double meniscus_start =
      data_sets[0]->run_data.meniscus - meniscus_range / 2.0;
  ...
  if (bottom_ds == 0.0) {
    ...
    bottom_ds = US_AstfemMath::calc_bottom(rpm, ds->centerpiece_bottom,
                                           ds->rotor_stretch);
  }
  ...
  for (int ee = 0; ee < data_sets.size(); ee++) {
    ...
    // Use bottom from edited data if it is given
    double bottom_ds =
        (edata->bottom > 0.0) ? edata->bottom : ds->centerpiece_bottom;
    if (my_rank == 0)
      DbgLv(0) << "incl_speed" << incl_speed << "nssp" << nssp
               << "bottom_ds bottom_cp" << bottom_ds << ds->centerpiece_bottom;

    if (!incl_speed && !redo_ss) {  // If experiment speed step used, test
                                    // against data set times
      ...
      if (etm1 < stm1 ||
          etm2 >
              stm2) {  // If data time range outside that of speed step, flag it
        redo_ss = true;
      }
    }
    ...
    if (!ds->tmst_file
      ...
      if (ee > 0 &&
          !ds0->tmst_file
               .isEmpty()) {  // Dataset 0's timestate file is not empty:  copy
  ...
  if (redo_tstate) {  // We need to re-do the timestate files
    ...
    if (my_rank ==
        0) {  // Do any writing or re-writing of timestates in master process
      QStringList tmsf_fs;
      ...
      for (int ee = 0; ee < count_datasets;
           ee++) {  // Examine each data set for associated timestate
        US_SolveSim::DataSet* ds = data_sets[ee];
        ...
        if (ntmsf_ne == 0) {  // No tmst_file given:  create one
          ...
          else if (!tmsf_fs.contains(
                       tmst_fpath)) {  // Not-first dataset, but unique file:
          ...
          DbgLv(0) << "rank" << my_rank << ": ee" << ee << "(NE)tmst_file"
                   << ds->tmst_file;
        }

        else if (ntmsf_ex == 1 &&
                 ntmsf_1s == 0) {  // Existing tmst_file given, but not at
                                   // 1-second interval
          ...
          ds->tmst_file = tmst_fpath;  // Save the used timestate file set
          DbgLv(0) << "rank" << my_rank << ": ee" << ee << "(E1)tmst_file"
                   << ds->tmst_file;
        }

        else if (ntmsf_ex > 1 &&
                 ntmsf_1s < ntmsf_ex) {  // Existing tmst_files given, but not
                                         // all at 1-second interval
          tmst_fpsav = tmst_fpath + "-orig";
          tmst_fdsav = tmst_fdefs + "-orig";
          QString tmst_fpold = tmst_fpath;
          QString tmst_fdold = tmst_fdefs;
          ...
          if (!tmsf_fs.contains(tmst_fpath) &&
              !QFile(tmst_fpath).exists()) {  // Unique data set timestate file
          ...
          ds->tmst_file = tmst_fpath;  // Save the used timestate file set
          DbgLv(0) << "rank" << my_rank << ": ee" << ee << "(EX)tmst_file"
                   << ds->tmst_file;
        }
  ...
  for (int ee = 0; ee < count_datasets; ee++) {
    US_SolveSim::DataSet* ds = data_sets[ee];
    ds->tmst_file =
        ds->tmst_file.isEmpty()
            ? "../" + ds->run_data.runID + ".time_state.tmst"
            : (ds->tmst_file.startsWith("../") ? ds->tmst_file
                                               : "../" + ds->tmst_file);
    ...
    DbgLv(0) << "rank" << my_rank << ": ee" << ee << "tmst_file"
             << ds->tmst_file << "ssp count"
...
if ( my_rank == 0 )
 DbgLv(0) << "rank" << my_rank << ": ee" << ee << "bottom_pos" << bottom_pos
  ...
  //    }
  if (my_rank == 0) DbgLv(0) << "rank" << my_rank << ": redo_ss" << redo_ss;
  ...
  if (redo_ss) {  // If speed step re-do flagged, set all speed steps from data
    for (int ee = 0; ee < data_sets.size(); ee++) {
      ...
      if (my_rank == 0)
        DbgLv(0) << "rank" << my_rank << ": ee" << ee
                 << "speed_step RE-DONE from data";
      int nssp = ds->simparams.speed_step.count();
      int stm1 = ds->simparams.speed_step[0].time_first;
      int stm2 = ds->simparams.speed_step[nssp - 1].time_last;
      if (my_rank == 0) {
  ...
  for (int ee = 0; ee < data_sets.size(); ee++) {
    ...
    if (my_rank == 0) {
      DbgLv(1) << "DSM: ssck: rspeed accel1 tf_aend tf_scan" << rspeed << accel1
               << tf_aend << tf_scan << "accel2" << accel2 << "lo_ss_acc"
               << lo_ss_acc;
    }
    ...
    if (accel1 < lo_ss_acc || tf_aend > (tf_scan + 1)) {
      DbgLv(0) << "rank: " << my_rank << "  Dataset " << ee
               << " Name: " << dset->run_data.runID
               << " likely bad Timestate. Implied acceleration: " << accel1
               << " accel end: " << tf_aend << "s. First scan: " << tf_scan
               << "s.";
    }
  }
  ...
  // Check GA buckets
  if (analysis_type == "GA") {
    if (buckets.size() < 1) abort("No buckets defined");
    ...
    for (int i = 0; i < buckets.size(); i++) {
      ...
      bucket_rects << QRectF(QPointF(buckets[i].x_min, buckets[i].y_max),
    ...
    for (int i = 0; i < bucket_rects.size() - 1; i++) {
      for (int j = i + 1; j < bucket_rects.size(); j++) {
        if (bucket_rects[i].intersects(bucket_rects[j])) {
          ...
          if (qMin(sdif, fdif) <
              1.e-6) {  // Ignore the overlap if it is trivial
            if (my_rank == 0) DbgLv(0) << "Trivial overlap is ignored.";
            continue;
          }
  ...
  if (analysis_type.startsWith("PCSA")) zval = parameters["z_value"].toDouble();
  ...
  if (attr_x != ATTR_S) {  // We will need to determine the maximum s value
    if (attr_y == ATTR_S)  // s_max may be y_max
      ...
      for (int jj = 0; jj < buckets.size();
           jj++) {  // Compute s using given x,y,fixed and find the max
        ...
        if (my_rank == 0)
          DbgLv(0) << "ckGrSz:  buck" << jj << "xv yv zv" << xval << yval
                   << zval << "mc s k w v d f" << mcomp.s << mcomp.f_f0
                   << mcomp.mw << mcomp.vbar20 << mcomp.D << mcomp.f;
  ...
  if (my_rank == 0)
    DbgLv(0) << "ckGrSz: s_max" << s_max << "attr_x,y,z" << attr_x << attr_y
             << attr_z;
  ...
  if (my_rank == 0)
    DbgLv(0) << "ckGrSz: ssp count"
             << data_sets[0]->simparams.sim_speed_prof.count() << "g_too_big"
             << g_too_big;
  if (g_too_big) {
    if (my_rank == 0) {
      qDebug() << smsg;
  ...
  // Set some defaults
  if (!parameters.contains("mutate_sigma")) parameters["mutate_sigma"] = "2.0";

  if (!parameters.contains("p_mutate_s")) parameters["p_mutate_s"] = "20";

  if (!parameters.contains("p_mutate_k")) parameters["p_mutate_k"] = "20";

  if (!parameters.contains("p_mutate_sk")) parameters["p_mutate_sk"] = "20";
...
void US_MPI_Analysis::start(void) {
  ...
  if (my_rank == 0) {
    ...
    if (count_datasets < kc_iters) {
      ...
      DbgLv(0) << "Finished:  maxrss " << maxrssmb << "MB,  total run seconds "
               << cputime << "  (Reduced Datasets Count)";
    }
...
void US_MPI_Analysis::send_udp(const QString& message) {
  ...
  if (my_rank ==
      0) {  // Send UDP message from supervisor (or single-group master)
            ///////////////////////////////*DEBUG*
    ...
    socket->writeDatagram(msg.data(), msg.size(), server, port);
           ...
           0) {  // For pm group master, forward message to supervisor
    int super = 0;
...
void US_MPI_Analysis::abort(const QString& message, int error) {
  ...
  if (my_rank ==
      0) {  // Create archive file of outputs and remove other output files
    update_outputs(true);
  }
...
void US_MPI_Analysis::stats_output(int walltime, int cputime, int maxrssmb,
                                   QDateTime submitTime, QDateTime startTime,
                                   QDateTime endTime) {
...
void US_MPI_Analysis::limitBucket(Bucket& buk) {
  if (buk.x_min > 0.0) {  // All-positive s's start at 0.1 at least (other
                          // attribs are different)
    double xmin = 0.1;
  ...
  else if ((buk.x_min + buk.x_max) >=
           0.0) {  // Mostly positive clipped to all positive starting at 0.1
    buk.x_min = 0.1;
...
void US_MPI_Analysis::dset_matrices(int dsx, int* nsolutesP,
  ...
  DbgLv(0) << "DSM: dsx" << dsx << "rowx ndspts" << rowx << ndspts << "nsolutes"
           << nsolutes << "concen" << concen << "inccx" << inccx;
  ...
  for (int cc = 0; cc < nsolutes; cc++) {
    ...
    for (int pp = 0; pp < ndspts;
         pp++, ka++, jj++) {  // Copy A column and accumulate column norm
  ...
  if (ksolutes < nsolutes) {  // Norm tolerance has excluded some columns
    ...
    *nsolutesP = ksolutes;             // Save new solutes count
    DbgLv(0) << "DSM:   ASIZE" << nnls_a.size() << "c_used[n]"
             << c_used[kc - 1];
  }
...
void US_MPI_Analysis::calc_residuals(int offset, int dataset_count,
                                     SIMULATION& simu_values) {
  count_calc_residuals++;
  bool do_dbg =
      (dbg_level > 0 && (group_rank < 2 || group_rank == (proc_count / 2)));
  if (do_dbg)
  ...
  if (do_dbg || hicee) {
    ...
    if (dataset_count > 1 && (offset + 1) < data_sets.size()) {
      ...
      DbgLv(1) << "w:" << my_rank << ": s2(01hmn)" << s0 << s1 << sh << sm
               << sn;
    }
    DbgLv(1) << "w:" << my_rank << ":  nsoli nsolo" << nsoli << nsolo;
    DbgLv(1) << "w:" << my_rank << ":  simpt men bott temp coef1"
             << dset->simparams.simpoints << dset->simparams.meniscus
             << dset->simparams.bottom << dset->simparams.temperature
    ...
    DbgLv(1) << "w:" << my_rank << ":  noisf alpha" << simu_values.noisflag
             << simu_values.alpha << "s20w_c D20w_c vbar"
             << dset->s20w_correction << dset->D20w_correction << dset->vbar20;
    if (dataset_count > 1 && (offset + 1) < data_sets.size()) {
      ...
      DbgLv(1) << "w:" << my_rank << ":  2)noisf alpha" << simu_values.noisflag
               << simu_values.alpha << "s20w_c D20w_c vbar"
               << dset->s20w_correction << dset->D20w_correction
               << dset->vbar20;
    }
    int nn = isols.size() - 1;
    int mm = nn / 2;
    if (nn > mm)
      DbgLv(1) << "w:" << my_rank << ": sol0 solm soln" << isols[0].s
               << isols[0].k << isols[0].c << isols[mm].s << isols[mm].k
               << isols[mm].c << isols[nn].s << isols[nn].k << isols[nn].c;
  }
  //*DEBUG*
}
...
void US_MPI_Analysis::write_output(void) {
  ...
  if (mdl_type == US_Model::TWODSA || mdl_type == US_Model::GA) {
    ...
    DbgLv(1) << "MAST: wrout: mdl_type DMGA";
           ...
           US_Model::PCSA) {  // PCSA: Order model records and pick best model
    max_depth = 0;
    qSort(mrecs);
    //*DEBUG*
    ...
    if (dbg_level > 0) {
      for (int jj = 0; jj < mrecs.size(); jj++) {
        DbgLv(1) << "M:wo: jj" << jj << "typ tx" << mrecs[jj].ctype
                 << mrecs[jj].taskx << "isz csz" << mrecs[jj].isolutes.size()
                 << mrecs[jj].csolutes.size() << "rmsd" << mrecs[jj].rmsd
                 << "sy ey" << mrecs[jj].str_y << mrecs[jj].end_y << "p1 p2"
                 << mrecs[jj].par1 << mrecs[jj].par2;
      }
    }
    //*DEBUG*
  ...
  if (mxdssz == 0) {  // Handle the case of a zero-solute final model
    ...
    DbgLv(0) << "*WARNING* Final solutes size" << mxdssz << "max_depth"
             << max_depth << "Sim and Depth-1 solutes size" << simssz << dm1ssz;
    if (simssz > 0) {  // Use the last result solutes if there are some
      ...
      DbgLv(0) << "   SimValues solutes used";
               ...
               0) {  // Else use the max-depth-minus-1 solutes if there are some
  ...
  DbgLv(1) << "WrO: meniscus_run" << meniscus_run << "mvsz"
           << meniscus_values.size();
  meniscus_value = meniscus_run < meniscus_values.size()
                       ? meniscus_values[meniscus_run]
                       : save_meniscus;
  bottom_value = bottom_run < bottom_values.size() ? bottom_values[bottom_run]
                                                   : save_bottom;
  ...
  if (mdl_type == US_Model::PCSA) {  // Output mrecs for PCSA, if we have a
                                     // final mrecs vector
    int tikreg = parameters["tikreg_option"].toInt();
    int mc_iters = parameters["mc_iterations"].toInt();

    if (tikreg == 0 && mc_iters < 2) write_mrecs();
  }
}
...
void US_MPI_Analysis::write_global(void) {
  ...
  if (mdl_type == US_Model::TWODSA) {
    ...
    US_SolveSim solvesim(data_sets, my_rank, false);
    solvesim.calc_residuals(0, data_sets.size(), wksim_vals, false, &gl_nnls_a,
                            &gl_nnls_b);
    ...
    DbgLv(0) << "WrGlob: dssize" << data_sets.size() << "concsize"
             << concentrations.size() << "simsolssz" << sim.solutes.size()
             << "gsimsolssz" << gsim->solutes.size() << nsolutes;
    ...
    for (int ee = 0; ee < data_sets.size(); ee++) {
      ...
      DbgLv(0) << "WrGlob:    ks kp nar nav" << kscans << kpoints << narows
               << navals;

      dset_matrices(ee, &nsolutes, nnls_a, nnls_b, c_used);

      DbgLv(0) << "WrGlob:    mats built; calling NNLS";
      US_Math2::nnls(nnls_a.data(), narows, narows, nsolutes, nnls_b.data(),
                     nnls_x.data());
      ...
      for (int cc = 0; cc < nsolutes; cc++) {
        ...
        if (soluval > 0.0) {
          ...
          ksolutes++;
          if (soluval > 1.0) {
            DbgLv(0) << "WrGlob:     SOLUVAL" << soluval << "ee cc" << ee << cc
                     << "ksol" << ksolutes;
            double avlo = 1e+100;
            double avhi = 0.0;
            double avav = 0.0;
      ...
      DbgLv(0) << "WrGlob:    currds" << ee << "nsol ksol" << nsolutes
               << ksolutes;
      ...
      write_model(wksim_vals, mdl_type, false);
           ...
           US_Model::GA) {  // GA:  Compute and output each dataset model
                            //      wksim_vals           = simulation_values;
    US_SolveSim solvesim(data_sets, my_rank, false);
    solvesim.calc_residuals(0, data_sets.size(), wksim_vals, false, &gl_nnls_a,
                            &gl_nnls_b);
    ...
    write_superg(sim, mdl_type);
    DbgLv(1) << "WrGlob: dssize" << data_sets.size() << "concsize"
             << concentrations.size() << "simsolssz" << sim.solutes.size()
             << "gsimsolssz" << gsim->solutes.size() << nsolutes;
    ...
    for (int ee = 0; ee < data_sets.size(); ee++) {
      ...
      write_model(wksim_vals, mdl_type, false);
      DbgLv(1) << "WrGlob:    currds" << ee << "nsol ksol" << nsolutes
               << ksolutes;
    }
  }

  else if (mdl_type == US_Model::PCSA) {  // PCSA: Recompute the global fit and
                                          // save A and b matrices for later use
    US_SolveSim solvesim(data_sets, my_rank, false);
    solvesim.calc_residuals(0, data_sets.size(), wksim_vals, false, &gl_nnls_a,
                            &gl_nnls_b);
    nsolutes = gsim->zsolutes.size();
    DbgLv(1) << "WrGlob:  glob recompute nzsols" << wksim_vals.zsolutes.size()
             << nsolutes << "globrec A,b sizes" << gl_nnls_a.size()
             << gl_nnls_b.size();
    double dsum_b = 0.0;
    ...
    DbgLv(1) << "WrGlob: dssize" << data_sets.size() << "concsize"
             << concentrations.size() << "simsolssz" << sim.zsolutes.size()
             << "gsimsolssz" << gsim->zsolutes.size() << nsolutes;
    ...
    for (int ee = 0; ee < data_sets.size(); ee++) {
      ...
      DbgLv(1) << "WrGlob:  ee" << ee << "AA-dset(m)"
               << edata->value(ks / 2, kp / 2);
      current_dataset = ee;
      meniscus_value = edata->meniscus;
      sim.variance = sim.variances[ee];
      double concentration = concentrations[ee];
      ...
      QVector<double> nnls_x(nsolutes, 0.0);
      QVector<int> c_used(nsolutes, 0);
      DbgLv(1) << "WrGlob:    ks kp nar nav" << kscans << kpoints << narows
               << navals;

      dset_matrices(ee, &nsolutes, nnls_a, nnls_b, c_used);
      double dsum_b = 0.0;
      for (int ii = 0; ii < narows; ii++) dsum_b += nnls_b[ii];
      DbgLv(1) << "WrGlob:      ee" << ee << "dsum_b" << dsum_b << "stx pts"
               << ds_startx[ee] << ds_points[ee] << narows << "conc"
               << concentrations[ee];
      dsum_b_all += dsum_b;
      ...
      DbgLv(1) << "WrGlob:    currds" << ee << "nsol ksol" << nsolutes
               << ksolutes << sim.zsolutes.size() << "sum_sol" << sum_sol;
    ...
    DbgLv(1) << "WrGlob:     dsum_b_all" << dsum_b_all;
           ...
           US_Model::DMGA) {  // DMGA:  Compute and output each dataset model
  }
...
void US_MPI_Analysis::write_model(const SIMULATION& sim,
                                  US_Model::AnalysisType type, bool glob_sols) {
  ...
  DbgLv(1) << "wrMo: type" << type << "(DMGA=" << US_Model::DMGA
           << ") (PCSA=" << US_Model::PCSA << ") subtype=" << subtype;
  if (type ==
      US_Model::DMGA) {  // For discrete GA, get the already constructed model
    model = data_sets[0]->model;
    DbgLv(1) << "wrMo:  model comps" << model.components.size();
  }

  int mc_iter = (mgroup_count < 2 || is_composite_job) ? (mc_iteration + 1)
                                                       : mc_iteration;
  model.monteCarlo = mc_iterations > 1;
  model.wavelength = edata->wavelength.toDouble();
  model.modelGUID =
      (!model.monteCarlo || mc_iter == 1) ? US_Util::new_guid() : modelGUID;
  ...
  DbgLv(0) << "wrMo:  is_glob glob_sols" << is_global_fit << glob_sols
           << "f_men f_bot f_mbo" << fit_meni << fit_bott << fit_menbot
           << "m.glob" << model.global;
  ...
  // demo1.veloc. 1A999.
  // e201101171200_a201101171400_2DSA_us3-0000003_i01-m62345.ri_noise
  // demo1_veloc. 1A999. e201101171200_a201101171400_2DSA_us3-0000003_mc001
  // .model runID.tripleID.analysisID.recordType
  //    analysisID = editID_analysisDate_analysisType_requestID_iterID
  //    (underscores)
  //       editID:
  //       requestID: from lims or 'local'
  //       analysisType : 2DSA GA others
  //       iterID:       'i01-m62345' for meniscus, mc001 for monte carlo, i01
  //       default
  //
  ...
  if (mc_iterations > 1)
    ...
    iterID.sprintf("i%02d-m%05db%05d", menibott_ndx + 1,
                   (int)(meniscus_value * 10000), (int)(bottom_value * 10000));
  else if (fit_meni)
  ...
  if (analysis_type.contains("CG")) id = id.replace("2DSA", "2DSA-CG");
  if (max_iterations > 1 && mc_iterations == 1) id += "-IT";
  ...
  else if (type != US_Model::DMGA) {  // For other non-DMGA, construct the model
                                      // from solutes
  ...
  DbgLv(1) << "wrMo: stype" << stype << QString().sprintf("0%o", stype)
           << "attr_z vbar20 mco0.v" << attr_z << vbar20
  ...
  if (lenfn > 96) {  // Insure a model file name less than 100 characters in
                     // length (tar limit)
  ...
  tsout << fn << ";meniscus_value=" << meniscus_value
        << ";MC_iteration=" << mc_iter << ";variance=" << sim.variance
...
void US_MPI_Analysis::write_noise(US_Noise::NoiseType type,
  ...
  if (analysis_type.contains("CG")) anType = anType.replace("2DSA", "2DSA-CG");
...
int US_MPI_Analysis::ready_worker() {
  ...
  DbgLv(1) << "ready_worker  w1 w1n worker worknext" << w1 << w1n << worker
           << worknext;
  if (w1 < 0)
    DbgLv(1) << "ready_worker  w1234...wmn" << worker_status[1]
             << worker_status[2] << worker_status[3] << worker_status[4]
...
void US_MPI_Analysis::update_outputs(bool is_final) {
  ...
  DbgLv(0) << my_rank << ": UpdOut : final" << is_final << "files size"
           << files.size();

  if (files.size() == 1 &&
      files[0] == "analysis-results.tar") {  // If the tar file exists alone, do
                                             // nothing here
    DbgLv(0) << my_rank
  ...
  if (mc_iterations > 1) {
    ...
    for (int ii = 0; ii < mtrips.size(); ii++) {
      ...
      DbgLv(0) << my_rank << ": MC : ii" << ii << "mftrip" << mftrip
               << "mtf count" << mtfiles.count();
...
double US_MPI_Analysis::rmsd_combined_mc_models(US_Model& model) {
  ...
  for (int ii = 0; ii < model.components.size(); ii++) {
    ...
    if (component->extinction > 0.0)
      component->molar_concentration =
          component->signal_concentration / component->extinction;
  }
  ...
  DbgLv(1) << "Combined MC Model: nscans:" << nscans << "npoints:" << npoints
           << "fit_ti:" << fit_ti << "fit_ri:" << fit_ri
           << "size ti_noise:" << ti_noise.size()
