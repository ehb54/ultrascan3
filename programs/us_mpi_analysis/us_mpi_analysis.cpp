...
US_MPI_Analysis::US_MPI_Analysis (int nargs, QStringList &cmdargs) : QObject ()
{
      ...
      if (cmdarg.startsWith ("-"))
        { // Argument pair is keyed set (.e.g., "-maxwall <n>")
          ...
          if (cmdarg.contains (wallkey) || cmdarg.contains (wallkey2)
            ...
            DbgLv (0) << "CmdArg -- unknown cmdarg " << cmdarg << "cmdval"
                      << cmdval;
          if (my_rank == 0)
  ...
  if (jxmlfile.isEmpty ())
    { // Not on command line: just look for its presence in the work directory
      ...
      if (my_rank == 0)
        DbgLv (0) << "  jfiles size" << jfiles.size () << "jxmlfile"
                  << jxmlfile;
    }
  ...
  for (int ii = 0; ii < data_sets.size (); ii++)
    {
      ...
      try
        {
          int result
              = US_DataIO::loadData (".", dset->edit_file, dset->run_data);
        ...
        {
          QString msg
              = "Bad data file " + dset->auc_file + " " + dset->edit_file;
  ...
  if (my_rank == 0)
    {
      DbgLv (0) << "FMB: meni range points" << meniscus_range
                << meniscus_points << "  bott range points" << bottom_range
                << bottom_points;
    }
      ...
      if (concentrations.count () < count_datasets)
      ...
      else
        {
          ...
          if (concen_min <= 0.0)
            {
              abort (
                  "Unable to obtain all total_concentrations with global fit");
            }
        }
    }
  ...
  for (int ee = 0; ee < data_sets.size (); ee++)
    {
      ...
      if (!incl_speed && !redo_ss)
        { // If experiment speed step used, test against data set times
          ...
          if (my_rank == 0)
            DbgLv (0) << "etm1 stm1" << etm1 << stm1 << "stm2 etm2" << stm2
                      << etm2;
  ...
  DbgLv (0) << "rank" << my_rank << ": redo_tstate" << redo_tstate
            << "ntmsf_ne ntmsf_ex ntmsf_1s" << ntmsf_ne << ntmsf_ex
            << ntmsf_1s;
              ...
              if (ntmsf_ne == 0)
                { // No tmst_file given:  create one
                  ...
                  if (ee == 0)
                    { // First (only) dataset:  create a tmst file set
                      ...
                      US_AstfemMath::writetimestate (tmst_fpath, ds->simparams,
                                                     simdat);
                      tmsf_fs << tmst_fpath;
                    }

                  else if (!tmsf_fs.contains (tmst_fpath))
                    { // Not-first dataset, but unique file:  create another
                      // file set
                      US_DataIO::RawData simdat;
                      US_AstfemMath::initSimData (simdat, ds->run_data, 0.0);
                      US_AstfemMath::writetimestate (tmst_fpath, ds->simparams,
                  ...
                  ds->tmst_file
                      = tmst_fpath; // Save the used timestate file set
                  DbgLv (0) << "rank" << my_rank << ": ee" << ee
                            << "(NE)tmst_file" << ds->tmst_file;
                }
                ...
                { // Existing tmst_file given, but not at 1-second interval
                  ...
                  ds->tmst_file
                      = tmst_fpath; // Save the used timestate file set
                  DbgLv (0) << "rank" << my_rank << ": ee" << ee
                            << "(E1)tmst_file" << ds->tmst_file;
                }

              else if (ntmsf_ex > 1 && ntmsf_1s < ntmsf_ex)
                { // Existing tmst_files given, but not all at 1-second
                  // interval
                  tmst_fpsav = tmst_fpath + "-orig";
                  tmst_fdsav = tmst_fdefs + "-orig";
                  QString tmst_fpold = tmst_fpath;
                  ...
                  ds->tmst_file
                      = tmst_fpath; // Save the used timestate file set
                  DbgLv (0) << "rank" << my_rank << ": ee" << ee
                            << "(EX)tmst_file" << ds->tmst_file;
                }

              ds->simparams.simSpeedsFromTimeState (tmst_fpath);
            }
        }
      // DbgLv(0) << "rank" << my_rank << ": pre-barrier3 :TM:" <<
      // ELAPSED_SECS;
  ...
  for (int ee = 0; ee < data_sets.size (); ee++)
    {
      ...
      if (my_rank == 0)
        {
          DbgLv (1) << "DSM: ssck: rspeed accel1 tf_aend tf_scan" << rspeed
                    << accel1 << tf_aend << tf_scan << "accel2" << accel2
                    << "lo_ss_acc" << lo_ss_acc;
        }
      // check if the acceleration rate is low or the first scan was taken
      // before the acceleration ended Due to older, wrong timestate
      // calculation there might be a case, in which the calculated end of
      // acceleration can be up to 1 second later than expected, tf_scan + 1
      // accounts for this.
      if (accel1 < lo_ss_acc || tf_aend > (tf_scan + 1))
        {
          DbgLv (0) << "rank: " << my_rank << "  Dataset " << ee
                    << " Name: " << dset->run_data.runID
                    << " likely bad Timestate. Implied acceleration: "
                    << accel1 << " accel end: " << tf_aend
        ...
        { // s_max may need computing
          ...
          for (int jj = 0; jj < buckets.size (); jj++)
            { // Compute s using given x,y,fixed and find the max
              ...
              if (my_rank == 0)
                DbgLv (0) << "ckGrSz:  buck" << jj << "xv yv zv" << xval
                          << yval << zval << "mc s k w v d f" << mcomp.s
                          << mcomp.f_f0 << mcomp.mw << mcomp.vbar20 << mcomp.D
                          << mcomp.f;
  ...
  if (my_rank == 0)
    {
      ...
      stats_output (walltime, cputime, maxrssmb, submitTime, startTime,
                    endTime);
...
// Send udp
void
US_MPI_Analysis::send_udp (const QString &message)
{
  QByteArray msg;
...
void
US_MPI_Analysis::abort (const QString &message, int error)
{
  if (my_rank == 0)
...
void
US_MPI_Analysis::stats_output (int walltime, int cputime, int maxrssmb,
...
void
US_MPI_Analysis::limitBucket (Bucket &buk)
{
  if (buk.x_min > 0.0)
...
void
US_MPI_Analysis::dset_matrices (int dsx, int *nsolutesP,
                                QVector<double> &nnls_a,
                                QVector<double> &nnls_b, QVector<int> &c_used)
{
  double norm_cut = 1.0;
  ...
  DbgLv (0) << "DSM: dsx" << dsx << "rowx ndspts" << rowx << ndspts
            << "nsolutes" << nsolutes << "concen" << concen << "inccx"
            << inccx;
  ...
  for (int cc = 0; cc < nsolutes; cc++)
    {
      ...
      if (norm_a < norm_cut) // Test norm tolerance
        {                    // This column should be excluded
          ...
          DbgLv (0) << "DSM: NORM_CUT:  norm_a" << norm_a << "ksol"
                    << ksolutes;
        }
...
void
US_MPI_Analysis::calc_residuals (int offset, int dataset_count,
  ...
  if (do_dbg || hicee)
    {
      ...
      DbgLv (1) << "w:" << my_rank << ":d(01hmn)" << d0 << d1 << dh << dm
                << dn;
      double dt = 0.0;
      ...
      DbgLv (1) << "w:" << my_rank << ": s(01hmn)" << s0 << s1 << sh << sm
                << sn;
      if (dataset_count > 1 && (offset + 1) < data_sets.size ())
        {
          ...
          DbgLv (1) << "w:" << my_rank << ":d2(01hmn)" << d0 << d1 << dh << dm
                    << dn;
          dt = 0.0;
          for (int ss = 0; ss < nsc; ss++)
            for (int rr = 0; rr < nrp; rr++)
      ...
      if (dataset_count > 1 && (offset + 1) < data_sets.size ())
        {
          ...
          DbgLv (1) << "w:" << my_rank << ":  2)vbar soltype manual visc dens"
                    << dset->vbar20 << dset->solute_type << dset->manual
                    << dset->viscosity << dset->density;
          DbgLv (1) << "w:" << my_rank << ":  2)noisf alpha"
                    << simu_values.noisflag << simu_values.alpha
                    << "s20w_c D20w_c vbar" << dset->s20w_correction
...
void
US_MPI_Analysis::write_output (void)
{
          ...
          for (int jj = 0; jj < mrecs.size (); jj++)
            {
              DbgLv (1) << "M:wo: jj" << jj << "typ tx" << mrecs[jj].ctype
                        << mrecs[jj].taskx << "isz csz"
                        << mrecs[jj].isolutes.size ()
                        << mrecs[jj].csolutes.size () << "rmsd"
                        << mrecs[jj].rmsd << "sy ey" << mrecs[jj].str_y
                        << mrecs[jj].end_y << "p1 p2" << mrecs[jj].par1
                        << mrecs[jj].par2;
            }
        }
      //*DEBUG*
  ...
  if (mxdssz == 0)
    { // Handle the case of a zero-solute final model
      ...
      int dm1ssz
          = max_depth > 0 ? calculated_solutes[max_depth - 1].size () : 0;
      DbgLv (0) << "*WARNING* Final solutes size" << mxdssz << "max_depth"
                << max_depth << "Sim and Depth-1 solutes size" << simssz
                << dm1ssz;
      if (simssz > 0)
...
void
US_MPI_Analysis::write_global (void)
{
  US_SolveSim::Simulation sim = simulation_values;
  ...
  if (mdl_type == US_Model::TWODSA)
    {
      ...
      solvesim.calc_residuals (0, data_sets.size (), wksim_vals, false,
                               &gl_nnls_a, &gl_nnls_b);
      DbgLv (0) << "WrGlob:  glob recompute nsols"
                << wksim_vals.solutes.size () << "globrec A,b sizes"
      ...
      for (int cc = 0; cc < nsolutes; cc++)
        {
          ...
          DbgLv (0) << "WrGlob:     cc" << cc << "conc-in"
                    << gsim->solutes[cc].c << "SGconc-out"
                    << sim.solutes[cc].c;
        }
      ...
      for (int ee = 0; ee < data_sets.size (); ee++)
        {
          ...
          DbgLv (0) << "WrGlob:    ks kp nar nav" << kscans << kpoints
                    << narows << navals;
              ...
              if (soluval > 0.0)
                {
                  ...
                  if (soluval > 1.0)
                    {
                      DbgLv (0) << "WrGlob:     SOLUVAL" << soluval << "ee cc"
                                << ee << cc << "ksol" << ksolutes;
                      double avlo = 1e+100;
                      double avhi = 0.0;
                      double avav = 0.0;
                      ...
                      DbgLv (0)
                          << "WrGlob:      A lo,hi,av" << avlo << avhi << avav
    ...
    { // GA:  Compute and output each dataset model
      ...
      solvesim.calc_residuals (0, data_sets.size (), wksim_vals, false,
                               &gl_nnls_a, &gl_nnls_b);
      DbgLv (1) << "WrGlob:  glob recompute nsols"
                << wksim_vals.solutes.size () << "globrec A,b sizes"
                << gl_nnls_a.size () << gl_nnls_b.size ();
    ...
    { // PCSA: Recompute the global fit and save A and b matrices for later use
      ...
      DbgLv (1) << "WrGlob:  glob recompute nzsols"
                << wksim_vals.zsolutes.size () << nsolutes
      ...
      DbgLv (1) << "WrGlob: dssize" << data_sets.size () << "concsize"
                << concentrations.size () << "simsolssz"
                << sim.zsolutes.size () << "gsimsolssz"
                << gsim->zsolutes.size () << nsolutes;
      ...
      for (int ee = 0; ee < data_sets.size (); ee++)
        {
          ...
          QVector<double> nnls_x (nsolutes, 0.0);
          QVector<int> c_used (nsolutes, 0);
          DbgLv (1) << "WrGlob:    ks kp nar nav" << kscans << kpoints
                    << narows << navals;
          ...
          DbgLv (1) << "WrGlob:      ee" << ee << "dsum_b" << dsum_b
                    << "stx pts" << ds_startx[ee] << ds_points[ee] << narows
                    << "conc" << concentrations[ee];
          dsum_b_all += dsum_b;
          ...
          DbgLv (1) << "WrGlob:    currds" << ee << "nsol ksol" << nsolutes
                    << ksolutes << sim.zsolutes.size () << "sum_sol"
                    << sum_sol;
...
void
US_MPI_Analysis::shutdown_all (void)
{
  MPI_Job job;
  ...
  for (int i = 1; i <= my_workers; i++)
    {
      ...
      maxrss += work_rss[i];
      DbgLv (1) << "2dsa master shutdown : worker" << i << " upd. maxrss"
                << maxrss << "  wkrss" << work_rss[i];
    }
}

// Write model output at the end of an iteration
void
US_MPI_Analysis::write_model (const SIMULATION &sim,
                              US_Model::AnalysisType type, bool glob_sols)
{
  ...
  if (mc_iterations > 1)
    ...
    iterID.sprintf ("i%02d-b%05d", bottom_run + 1,
                    (int)(bottom_value * 10000));
  else
    iterID = "i01";
  ...
  if (lenfn > 96)
    { // Insure a model file name less than 100 characters in length (tar
      // limit)
...
void
US_MPI_Analysis::write_noise (US_Noise::NoiseType type,
...
void
US_MPI_Analysis::update_outputs (bool is_final)
{
      ...
      for (int ii = 0; ii < mtrips.size (); ii++)
        {
          ...
          int f_iters
              = QString (cmfname).section (".", -3, -3).mid (3).toInt ();
          if (!tfiles.contains (cmfname)
              && (is_final || f_iters == mc_iterations))
            {
              ...
              DbgLv (0) << my_rank << ":       model2.description"
                        << model2.description;
              QString runstring
                  = "Run: " + QString::number (ii + 1) + " " + tripleID;
...
double
US_MPI_Analysis::rmsd_combined_mc_models (US_Model &model)
{
  ...
  for (int ii = 0; ii < nscans; ii++)
    {
      ...
      for (int jj = 0; jj < npoints; jj++)
        { // Calculate the residuals and the RMSD
          ...
          double diff
              = edata.value (ii, jj) - sdata.value (ii, jj) - rin - tin;
          variance += sq (diff);
          kpts++;
        }
    }
...
US_MPI_Analysis::model_type (const QString a_type)
{
...
void
US_MPI_Analysis::write_superg (const SIMULATION &sim,
