...
void
US_MPI_Analysis::pcsa_worker (void)
{
  ...
  while (repeat_loop)
    {
      ...
      MPI_Recv (&job, // get masters' response
                sizeof (job), MPI_BYTE, MPI_Job::MASTER, MPI_Job::TAG0,
                my_communicator,
      ...
      switch (job.command)
        ...
        case MPI_Job::PROCESS: // Process solutes
          {
            ...
            DbgLv (1) << "w:" << my_rank << ":PM:Recv: 2:zsol"
                      << job_length * zsolut_doubles;
            MPI_Recv (simulation_values.zsolutes.data (), // Get solutes
                      job_length * zsolut_doubles, MPI_DOUBLE, MPI_Job::MASTER,
            ...
            // if(my_rank==1)
            {
              ...
              int mm = nn / 2;
              DbgLv (1) << "w:" << my_rank << ": offs dscnt" << offset
                        << dataset_count << "vbar s20wc bott"
              ...
              DbgLv (1) << "w:" << my_rank << ": sol0 solm soln"
                        << simulation_values.zsolutes[0].x
                        << simulation_values.zsolutes[0].y
                        << simulation_values.zsolutes[mm].x
                        << simulation_values.zsolutes[mm].y
                        << simulation_values.zsolutes[nn].x
            ...
            DbgLv (1) << "w:" << my_rank << ":   result sols size" << sizes[0]
                      << "max_rss" << sizes[3] << "sizes12" << sizes[1]
                      << sizes[2];
            //*DEBUG*
            if (dbg_level == 0 && my_rank == 1)
              {
                DbgLv (1) << "w:" << my_rank << ":   result sols size"
                          << sizes[0] << "nsscan"
                          << simulation_values.sim_data.scanCount ();
              }
            //*DEBUG*
            MPI_Send (sizes, 4, MPI_INT, MPI_Job::MASTER, MPI_Job::RESULTS,
                      my_communicator);

            // Send back to master all of simulation_values
            MPI_Send (simulation_values.zsolutes.data (),
                      sizes[0] * zsolut_doubles, MPI_DOUBLE, MPI_Job::MASTER,
                      MPI_Job::TAG0, my_communicator);

            MPI_Send (&simulation_values.variance, 1, MPI_DOUBLE,
                      MPI_Job::MASTER, MPI_Job::TAG0, my_communicator);

            MPI_Send (simulation_values.variances.data (), dataset_count,
                      MPI_DOUBLE, MPI_Job::MASTER, MPI_Job::TAG0,
                      my_communicator);

            MPI_Send (simulation_values.ti_noise.data (), sizes[1], MPI_DOUBLE,
                      MPI_Job::MASTER, MPI_Job::TAG0, my_communicator);

            MPI_Send (simulation_values.ri_noise.data (), sizes[2], MPI_DOUBLE,
                      MPI_Job::MASTER, MPI_Job::TAG0, my_communicator);
          }
        ...
        case MPI_Job::PROCESS_MC: // Process solutes for monte carlo
          {
            DbgLv (1) << "w:" << my_rank << ":Recv:PROCESS_MC" << "mc_iter"
                      << mc_iter << "is_glob" << is_global_fit << "jlen"
                      << job_length;
            double varrmsd = 0.0;
            ...
            DbgLv (1) << "w:" << my_rank << ":PM:Recv: 3:zsol"
                      << job_length * zsolut_doubles;
            MPI_Recv (simulation_values.zsolutes.data (), // Get solutes
                      job_length * zsolut_doubles, MPI_DOUBLE, MPI_Job::MASTER,
                      MPI_Job::TAG0, my_communicator, &status);
                    ...
                    for (int ss = 0; ss < scan_count; ss++, jsc++)
                      {
                        for (int rr = 0; rr < radius_points; rr++, index++)
                          {
                            double vari
                                = US_Math2::box_muller (0.0, sigmas[index]);
                            double datout = sim_data->value (jsc, rr) + vari;
                            varrmsd += sq (vari);
                            if (ss < 4 && rr < 4)
                              DbgLv (1) << "w:" << my_rank << ": ee ss rr"
                                        << ee << ss << rr << "index vari"
                                        << index << vari << "sigma vrmsd dato"
                                        << sigmas[index] << varrmsd << datout;
            ...
            //*DEBUG*
            // Tell master we are sending back results
            int sizes[4] = { simulation_values.zsolutes.size (), mc_iter, 0,
                             (int)max_rss () };
            ...
            MPI_Send (sizes, 4, MPI_INT, MPI_Job::MASTER, MPI_Job::RESULTS_MC,
                      my_communicator);
        ...
        case MPI_Job::NEWDATA: // Reset data for Monte Carlo or global fit
          {
            // if(my_rank==1)
            DbgLv (1) << "w:" << my_rank << ":Recv:NEWDATA  joblen"
                      << job_length;
            if (is_global_fit && mc_iter < 3 && my_rank < 3)
              { // For global fits, check the memory requirements
                ...
                if (parameters.contains ("ff0_grid_points"))
                  ...
                  ff0_pts
                      = parameters["ff0_resolution"].toDouble () * grid_reps;
                ...
                qDebug () << "++ Worker" << my_rank << ": MC iteration"
                          ...
                          << "\n    Maximum memory used to this point"
                          << memused << "\n    Composite data memory needed"
                          << memdata << "\n    Maximum subgrid solute count"
                          << maxsols << "\n    NNLS A matrix memory needed"
                          << memamatr << "\n    NNLS B matrix memory needed"
                          << membmatr << "\n    Total memory (GB) used"
                          << gb_used << "\n    Total memory (GB) needed"
                          << gb_need << "\n    Total memory (GB) available"
                          << gb_avail
                          << "\n    Memory (GB) currently available"
                          << gb_curav;
              }
            ...
            if (my_rank == 1 || my_rank == 11)
              DbgLv (1) << "newD:" << my_rank
                        << " scld/newdat rcv : offs dsknt" << offset
                        << dataset_count << "joblen" << job_length;
            double dsum = 0.0;
            // This is a receive
            DbgLv (1) << "w:" << my_rank << ":PM:Recv: 5:bcast-mcd"
                      << job_length;
            MPI_Bcast (mc_data.data (), job_length, MPI_DOUBLE,
            ...
            if (is_global_fit && dataset_count == 1)
              { // For global update to scaled data, extra value is new ODlimit
                ...
                if ((my_rank == 1 || my_rank == 11))
                  DbgLv (1) << "newD:" << my_rank << ":offset ODlimit"
                            << offset << data_sets[offset]->run_data.ODlimit;
              }
                    ...
                    for (int ee = 0; ee < dataset_count; ee++)
                      {
                        ...
                        DbgLv (1) << "newD:" << my_rank << ":    ee jda jre"
                                  << ee << jda << jre;
                      }
                    DbgLv (1) << "newD:" << my_rank << ":   jre mcsize" << jre
                              << mc_data.size () << "ksc" << ksc << "sigsize"
                              << sigmas.size () << dataset_count;
                  }

                else
                  { // Non-global (single-dataset) simdata,sigmas
                    US_DataIO::EditedData *edata
                        = &data_sets[offset]->run_data;
                    ...
                    DbgLv (1) << "newD:" << my_rank << ":   index mcsize"
                              << index << mc_data.size ();
                  }
                ...
                for (int ee = offset; ee < offset + dataset_count; ee++)
                  {
                    ...
                    int indxh = ((scan_count / 2) * radius_points)
                                + (radius_points / 2);
                    for (int ss = 0; ss < scan_count; ss++)
                      {
                        ...
                        for (int rr = 0; rr < radius_points; rr++, index++)
                          {
                            ...
                            if ((my_rank == 1 || my_rank == 11)
                                && (index < 5 || index > (job_length - 6)
                                    || (index > (indxh - 4)
                                        && index < (indxh + 3))))
                              DbgLv (1) << "newD:" << my_rank << ":index"
                                        << index << "edat"
                                        << edata->value (ss, rr) << "ee" << ee;
                          }
                      }
                  }
                if (my_rank == 1 || my_rank == 11)
                  DbgLv (1) << "newD:" << my_rank << "  length index"
                            << job_length << index << "dsum" << dsum;
              }
          }
