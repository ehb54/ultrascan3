  ...
  while (repeat_loop) {
    ...
    switch (job.command) {
      case MPI_Job::PROCESS:  // Process solutes
      {
        ...
        MPI_Send(simulation_values.zsolutes.data(), sizes[0] * zsolut_doubles,
                 MPI_DOUBLE, MPI_Job::MASTER, MPI_Job::TAG0, my_communicator);

        MPI_Send(&simulation_values.variance, 1, MPI_DOUBLE, MPI_Job::MASTER,
                 MPI_Job::TAG0, my_communicator);

        MPI_Send(simulation_values.variances.data(), dataset_count, MPI_DOUBLE,
      ...
      case MPI_Job::PROCESS_MC:  // Process solutes for monte carlo
      {
          ...
          for (int ee = 0; ee < dataset_count; ee++) {
            ...
            for (int ss = 0; ss < scan_count; ss++, jsc++) {
              for (int rr = 0; rr < radius_points; rr++, index++) {
                ...
                varrmsd += sq(vari);
                if (ss < 4 && rr < 4)
                  DbgLv(1) << "w:" << my_rank << ": ee ss rr" << ee << ss << rr
                           << "index vari" << index << vari
                           << "sigma vrmsd dato" << sigmas[index] << varrmsd
                           << datout;
        ...
        //*DEBUG*
        DbgLv(1) << "w:" << my_rank << ":Send:RESULTS_MC  sizes" << sizes[0]
      ...
      case MPI_Job::NEWDATA:  // Reset data for Monte Carlo or global fit
      {
        ...
        if (is_global_fit && mc_iter < 3 &&
            my_rank < 3) {  // For global fits, check the memory requirements
          ...
          qDebug() << "++ Worker" << my_rank << ": MC iteration" << mc_iter
                   << ": Memory Profile :"
        ...
        if (mc_data.size() != job_length) {
          DbgLv(0) << "*ERROR* mc_data.size() job_length" << mc_data.size()
                   << job_length;
        }
        ...
        MPI_Bcast(mc_data.data(), job_length, MPI_DOUBLE, MPI_Job::MASTER,
                  my_communicator);

        if (is_global_fit &&
            dataset_count == 1) {  // For global update to scaled data, extra
                                   // value is new ODlimit
          job_length--;
        ...
        if (is_simdat) {  // If simulation,residuals for MC; save and get sigmas
          ...
          if (is_global_fit) {  // Global-fit simdata,sigmas
            ...
            for (int ee = 0; ee < dataset_count; ee++) {
              ...
              DbgLv(1) << "newD:" << my_rank << ":    ee jda jre" << ee << jda
                       << jre;
            }
            ...
            DbgLv(1) << "newD:" << my_rank << ":   index mcsize" << index
                     << mc_data.size();
          }
          ...
          for (int ee = offset; ee < offset + dataset_count; ee++) {
            ...
            int indxh =
                ((scan_count / 2) * radius_points) + (radius_points / 2);
            for (int ss = 0; ss < scan_count; ss++) {
              for (int rr = 0; rr < radius_points; rr++, index++) {
                ...
                if ((my_rank == 1 || my_rank == 11) &&
                  ...
                  DbgLv(1) << "newD:" << my_rank << ":index" << index << "edat"
                           << edata->value(ss, rr) << "ee" << ee;
              }
            }
          }
          if (my_rank == 1 || my_rank == 11)
            DbgLv(1) << "newD:" << my_rank << "  length index" << job_length
                     << index << "dsum" << dsum;
        }
      }
