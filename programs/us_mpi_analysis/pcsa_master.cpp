...
void US_MPI_Analysis::pcsa_master(void) {
   ...
   while (true) {
      ...
      while (!job_queue.isEmpty() && worker_status.contains(READY)) { // There are jobs in the queue and workers ready
         ...
         DbgLv(1) << "pcsa_mast:   submit_pcsa kc" << kcurve << "ready worker" << worker << "depth" << job.mpi_job.depth
                  << "JQue size" << job_queue.size();

         jql = job_queue.size() - 1;
         if (jql >= 0) {
            DbgLv(1) << "pcsa_mast:MLOOP - (00)JQue size" << job_queue.size() << "JQ[n].zsol.siz"
                     << job_queue[ jql ].zsolutes.size();
         }
         submit_pcsa(job, worker); // Submit a job to the worker
      }
      jql = job_queue.size() - 1;
      if (jql >= 0) {
         DbgLv(1) << "pcsa_mast:MLOOP - (01)JQue size" << job_queue.size() << "JQ[n].zsol.siz"
                  << job_queue[ jql ].zsolutes.size();
      }
      else {
      ...
      if (job_queue.isEmpty() && !worker_status.contains(WORKING)) { // No jobs left in the queue and no workers still busy
         ...
         simulation_values.noisflag += parameters[ "rinoise_option" ].toInt() > 0 ? 2 : 0;
         ...
         DbgLv(1) << "final-mr0: csolsize1 csolsize2" << simulation_values.zsolutes.size()
                  << wksim_vals.zsolutes.size();
         ...
         int km = mrecs.size();
         int kg = 0;
         for (int jm = 0; jm < km; jm++)
            if (mrecs[ jm ].rmsd < 1.0)
               kg++;
         ...
         iterations = 1;
         max_iterations = 1;
         DbgLv(1) << "pcsa_mast: loop-BOT: dssize" << data_sets.size() << "ds_to_p" << datasets_to_process << "curr_ds"
                  << current_dataset;
         ...
         int ss = ks / 2;
         int rr = kp / 2;
         DbgLv(1) << "pcsa_mast: loop-BOT: ds" << current_dataset + 1 << "data l m h" << edat->value(10, 10)
                  << edat->value(ss, rr) << edat->value(ks, kp);
      ...
      jql = job_queue.size() - 1;
      if (jql >= 0) {
         DbgLv(1) << "pcsa_mast:MLOOP - (05)JQue size" << job_queue.size() << "JQ[n].zsol.siz"
                  << job_queue[ jql ].zsolutes.size();
      }
      else {
      ...
      DbgLv(1) << "PM:Recv:   sizes" << sizes[ 0 ] << sizes[ 1 ] << sizes[ 2 ] << sizes[ 3 ] << "statTAG"
               << status.MPI_TAG;

      worker = status.MPI_SOURCE;
      jql = job_queue.size() - 1;
      if (jql >= 0) {
         DbgLv(1) << "pcsa_mast:MLOOP - (06)JQue size" << job_queue.size() << "JQ[n].zsol.siz"
                  << job_queue[ jql ].zsolutes.size();
      }
      DbgLv(1) << "pcsa_mast:MLOOP -  status TAG" << status.MPI_TAG << "  source" << status.MPI_SOURCE;
      ...
      switch (status.MPI_TAG) {
         ...
         case MPI_Job::RESULTS: // Return solute data
            DbgLv(1) << "pcsa_mast: loop-BOTTOM:  RESULTS  worker" << worker << "sizes" << sizes[ 0 ] << sizes[ 1 ]
                     << sizes[ 2 ] << sizes[ 3 ];
         ...
         default: // Should never happen
            ...
            QString msg = "Master PCSA:  Received invalid status " + QString::number(status.MPI_TAG);
            abort(msg);
            break;
      }
      jql = job_queue.size() - 1;
      if (jql >= 0) {
         DbgLv(1) << "pcsa_mast:MLOOP - (09)JQue size" << job_queue.size() << "JQ[n].zsol.siz"
                  << job_queue[ jql ].zsolutes.size();
      }
...
void US_MPI_Analysis::init_pcsa_solutes(void) {
   ...
   if (ctype == CTYPE_SL) {
      ...
      US_ModelRecord::compute_slines(x_min, x_max, y_min, y_max, nkpts, nlpts, parlims, mrecs);
      ...
      US_ModelRecord::compute_sigmoids(ctype, x_min, x_max, y_min, y_max, nkpts, nlpts, parlims, mrecs);
      ...
      nkpto = nkpts;
      mrecs.reserve(nkpto);
      US_ModelRecord::compute_hlines(x_min, x_max, y_min, y_max, nkpts, nlpts, parlims, mrecs);
   }
      ...
      US_ModelRecord::compute_slines(x_min, x_max, y_min, y_max, nkpts, nlpts, parlims, mrecs);
      US_ModelRecord::compute_sigmoids(ctype1, x_min, x_max, y_min, y_max, nkpts, nlpts, parlims1, mrecs);
      US_ModelRecord::compute_sigmoids(ctype2, x_min, x_max, y_min, y_max, nkpts, nlpts, parlims2, mrecs);
   }
      ...
      DbgLv(1) << "  init_pcsa_sols: nkpts" << nkpts << "nkpto" << nkpto;
      mrecs.reserve(nkpto);
      US_ModelRecord::compute_2ndorder(x_min, x_max, y_min, y_max, nkpts, nlpts, parlims, mrecs);
   }
...
void US_MPI_Analysis::submit_pcsa(Sa_Job &job, int worker) {
   ...
   if (dd == 0) {
      DbgLv(1) << " submit_pcsa: worker" << worker << "  sols" << job.mpi_job.length << "mciter cds" << mc_iteration
               << current_dataset << " depth" << dd;
   }
   else {
      DbgLv(1) << " submit_pcsa:     worker" << worker << "  sols" << job.mpi_job.length << "mciter cds" << mc_iteration
               << current_dataset << " depth" << dd;
   }
   DbgLv(1) << "Mast: submit: len sol offs cnt" << job.mpi_job.length << job.mpi_job.solution
            << job.mpi_job.dataset_offset << job.mpi_job.dataset_count;

   // Tell worker that solutes are coming
   DbgLv(1) << " submit_pcsa-SEND PROCESS  worker" << worker << "isolsiz" << job.mpi_job.length << "depth"
            << job.mpi_job.depth;
   MPI_Send(
      &job.mpi_job, sizeof(MPI_Job), MPI_BYTE,
   ...
   // Send solutes
   MPI_Send(
      job.zsolutes.data(), job.mpi_job.length * zsolut_doubles,
      MPI_DOUBLE, // Pass solute vector as hw independent values
      worker, // to worker
      MPI_Job::MASTER, my_communicator);
   DbgLv(1) << " submit_pcsa: send #2  worker" << worker << "depth" << job.mpi_job.depth << "stat=WORKING";
...
void US_MPI_Analysis::process_pcsa_results(const int worker, const int *sizes) {
   ...
   MPI_Recv(
      simulation_values.zsolutes.data(), sizes[ 0 ] * zsolut_doubles, MPI_DOUBLE, worker, MPI_Job::TAG0,
      my_communicator, &status);

   DbgLv(1) << "PM:Recv: 3:vari" << 1;
   MPI_Recv(&simulation_values.variance, 1, MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator, &status);
   DbgLv(1) << "pcsa_mast:PPRes -   (03)JQue size" << job_queue.size();

   DbgLv(1) << "PM:Recv: 4:varis" << datasets_to_process;
   MPI_Recv(
      simulation_values.variances.data(), datasets_to_process, MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator,
      &status);

   DbgLv(1) << "PM:Recv: 5:tinoi" << sizes[ 1 ];
   MPI_Recv(simulation_values.ti_noise.data(), sizes[ 1 ], MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator, &status);
   DbgLv(1) << "pcsa_mast:PPRes -   (05)JQue size" << job_queue.size();

   DbgLv(1) << "PM:Recv: 6:rinoi" << sizes[ 2 ];
   MPI_Recv(simulation_values.ri_noise.data(), sizes[ 2 ], MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator, &status);
...
void US_MPI_Analysis::process_pcsa_solutes(Result &result) {
   ...
   US_ModelRecord *cMr = &mrecs[ jcurve ];
...
void US_MPI_Analysis::write_mrecs() {
   ...
   DbgLv(1) << "pcsa:wrmr:  editGUID=" << mrecs[ 0 ].editGUID << "modelGUID=" << mrecs[ 0 ].modelGUID;
   QString s_desc = QString(modelP->description).replace(".model", ".mrecs");
   QString s_ctyp = parameters[ "curve_type" ];
   int ctype = US_ModelRecord::ctype_flag(s_ctyp);
   QString s_styp = parameters[ "solute_type" ];
   int stype = US_ModelRecord::stype_flag(s_styp);
   ...
   if (fileo.open(QIODevice::WriteOnly | QIODevice::Text)) {
      ...
      US_ModelRecord::write_modelrecs(xmlo, mrecs, s_desc, ctype, x_min, x_max, y_min, y_max, stype);
   ...
   if (!filea.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Append)) {
   ...
   tsout << fnameo << ";meniscus_value=" << meniscus_value << ";MC_iteration=" << mc_iter
         << ";variance=" << simulation_values.variance << ";run=" << runstring << "\n";
   filea.close();
...
void US_MPI_Analysis::iterate_pcsa() {
   ...
   if (iterations > 1) { // If after first iteration, check RMSD difference
      double dr_rat = (rmsd_last - rmsd_curr) / rmsd_last;
      DbgLv(1) << "iter_p: rmsd_c rmsd_l" << rmsd_curr << rmsd_last << "dr_rat thr_dr_rat" << dr_rat << thr_dr_rat;

      // If difference in iteration RMSD less than threshold, we are done
      if (dr_rat < 0.0) { // Increase in RMSD
         qDebug() << "Increasing Last, Current RMSDs of" << rmsd_last << rmsd_curr << "truncate iterations at"
                  << iterations;
         max_iterations = iterations;
         return;
      }

      else if (dr_rat < thr_dr_rat) { // Equal RMSD (within threshold ratio)
         qDebug() << "Virtually identical RMSDs of" << rmsd_last << rmsd_curr << "truncate iterations at" << iterations;
         max_iterations = iterations;
         return;
      }
   }
   ...
   if (ctype != CTYPE_ALL) { // All non-combination records:  re-compute with narrower limits

      US_ModelRecord::recompute_mrecs(ctype, x_min, x_max, y_min, y_max, nkpts, nlpts, parlims, mrecs);
      DbgLv(1) << "iter_p: parlims" << parlims[ 0 ] << parlims[ 1 ] << parlims[ 2 ] << parlims[ 3 ] << parlims[ 4 ]
               << parlims[ 5 ] << parlims[ 6 ] << parlims[ 7 ] << parlims[ 8 ] << parlims[ 9 ];
   } // END: non-combo records

   else { // Records that are a mix of SL,IS,DS
      ...
      ctype = CTYPE_SL;
      US_ModelRecord::recompute_mrecs(ctype, x_min, x_max, y_min, y_max, nkpts, nlpts, plims_sl, mrecs_sl);

      ctype = CTYPE_IS;
      US_ModelRecord::recompute_mrecs(ctype, x_min, x_max, y_min, y_max, nkpts, nlpts, plims_is, mrecs_is);

      ctype = CTYPE_DS;
      US_ModelRecord::recompute_mrecs(ctype, x_min, x_max, y_min, y_max, nkpts, nlpts, plims_ds, mrecs_ds);
      ...
      DbgLv(1) << "iter_p: plims_sl" << plims_sl[ 0 ] << plims_sl[ 1 ] << plims_sl[ 2 ] << plims_sl[ 3 ]
               << plims_sl[ 4 ];
      DbgLv(1) << "iter_p: plims_is" << plims_is[ 0 ] << plims_is[ 1 ] << plims_is[ 2 ] << plims_is[ 3 ]
               << plims_is[ 4 ];
      DbgLv(1) << "iter_p: plims_ds" << plims_ds[ 0 ] << plims_ds[ 1 ] << plims_ds[ 2 ] << plims_ds[ 3 ]
               << plims_ds[ 4 ];
...
void US_MPI_Analysis::tikreg_pcsa() {
   ...
   alpha = (tikreg == 1) ? parameters[ "tikreg_alpha" ].toDouble() : alpha_scan();
   ...
   US_DataIO::EditedData *edata = &data_sets[ current_dataset ]->run_data;
   QString tripleID = edata->cell + edata->channel + edata->wavelength;
   QString progress = "Regularization: " + QString::number(wksim_vals.alpha);
...
void US_MPI_Analysis::montecarlo_pcsa() {
   ...
   if (is_global_fit) { // Global-fit:  grab sim+resids from all datasets
   ...
   else { // Single dataset:  construct sim+resids flat array
   ...
   for (worker = 1; worker <= my_workers; worker++) { // Tell each worker new data coming; worker expects a Send
      MPI_Job mjob;
      ...
      MPI_Send(&mjob, sizeof(MPI_Job), MPI_BYTE, worker, MPI_Job::MASTER, my_communicator);
   }
   ...
   MPI_Bcast(
      mc_data.data(), // Send simulation,residuals data
      mcd_size, MPI_DOUBLE, MPI_Job::MASTER, my_communicator);
   ...
   while (true) {
      ...
      while (!job_queue.isEmpty() && worker_status.contains(READY)) {
         ...
         // Tell worker that solutes are coming
         DbgLv(1) << " masterMC loop-SEND PROCESS_MC  worker" << worker << "kci_send" << kci_send;
         MPI_Send(&job.mpi_job, sizeof(MPI_Job), MPI_BYTE, worker, MPI_Job::MASTER, my_communicator);

         // Send solutes
         MPI_Send(
            job.zsolutes.data(), job.mpi_job.length * zsolut_doubles, MPI_DOUBLE, worker, MPI_Job::MASTER,
            my_communicator);
      ...
      if (worker < 1 || worker > my_workers) {
         ...
         QString msg = "Master PCSA:  Received invalid source (worker): " + QString::number(worker)
                       + ", beyond limit 1 to " + QString::number(my_workers);
         abort(msg);
      }
      DbgLv(1) << " masterMC loop-BOTTOM:   status TAG" << status.MPI_TAG << "  source" << status.MPI_SOURCE << "sizes"
      ...
      switch (status.MPI_TAG) {
         ...
         case MPI_Job::RESULTS_MC: // Return solute data
            ...
            MPI_Recv(
               wksim_vals.zsolutes.data(), sizes[ 0 ] * zsolut_doubles, MPI_DOUBLE, worker, MPI_Job::TAG0,
               my_communicator, &status);

            DbgLv(1) << "PM:Recv: 9:vari" << 1;
            MPI_Recv(&wksim_vals.variance, 1, MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator, &status);

            DbgLv(1) << "PM:Recv: 10:vairs" << datasets_to_process;
            MPI_Recv(
               wksim_vals.variances.data(), datasets_to_process, MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator,
               &status);
            ...
            if (count_datasets > 1) {
               ...
               progress = "Dataset: 1 of 1; MonteCarlo: " + QString::number(mc_iter);
            }
         ...
         case MPI_Job::RESULTS: // Return solute data (unused worker?)
            ...
            { // Receives just to consume messages
               ...
               MPI_Recv(wbuf, sizes[ 0 ] * zsolut_doubles, MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator, &status);
               DbgLv(1) << "PM:Recv: 12:wbuf" << 1;
               MPI_Recv(wbuf, 1, MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator, &status);
               DbgLv(1) << "PM:Recv: 13:wbuf" << datasets_to_process;
               MPI_Recv(wbuf, datasets_to_process, MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator, &status);
               DbgLv(1) << "PM:Recv: 14:wbuf" << sizes[ 1 ];
               MPI_Recv(wbuf, sizes[ 1 ], MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator, &status);
               DbgLv(1) << "PM:Recv: 15:wbuf" << sizes[ 2 ];
               MPI_Recv(wbuf, sizes[ 2 ], MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator, &status);
            }
            break;

         default: // Should never happen
            DbgLv(1) << " masterMC loop-RECV invalid  worker" << worker << "tag" << status.MPI_TAG;
            progress = "Master PCSA:  Received invalid status " + QString::number(status.MPI_TAG);
            abort(progress);
            break;
      }
   ...
   DbgLv(1) << " masterMC  loop-FALLTHRU mc_iter mc_iters kci_send kci_recv" << mc_iteration << mc_iterations
            << kci_send << kci_recv;
   ...
   if (nmfile == 1) {
      ...
      for (int jj = 1; jj < mlist.size(); jj++) { // Find the last of multiple files created
   ...
   for (int jj = 0; jj < wmodel.components.size(); jj++) {
      US_ZSolute::set_solute_values(wmodel.components[ jj ], mrec.csolutes[ jj ], stype);
   }
...
void US_MPI_Analysis::filter_mrecs(const int ctype, QVector<US_ModelRecord> &mrecs_a, QVector<US_ModelRecord> &mrecs_t) {
...
void US_MPI_Analysis::clean_mrecs(QVector<US_ModelRecord> &mrecs) {
   ...
   if (mrecs[ 0 ].v_ctype != CTYPE_ALL) { // If records of one type, just duplicate solutes from last good
   ...
   else { // If mixed records, duplicate solutes from last of each type
      int gx_sl = 0;
      int gx_is = 0;
      int gx_ds = 0;
      int ktype = 0;
...
void US_MPI_Analysis::pcsa_best_model() {
   ...
   DbgLv(1) << "bestm: CALC_RESID call" << current_dataset << datasets_to_process << "zsols size"
            << simulation_values.zsolutes.size() << "variance" << mrec.variance;
   calc_residuals(current_dataset, datasets_to_process, simulation_values);
   ...
   if ((tikreg != 0 || mc_iterations > 1) && (noiflg != 0)) { // Apply computed noise to data
      US_DataIO::EditedData *edata = &data_sets[ current_dataset ]->run_data;
      double vnoise = 0.0;
      ...
      if ((noiflg & 1) > 0) { // Apply ti noise
         DbgLv(1) << "bestm: noise flag" << noiflg << "TI_NOISE apply";
         for (int jj = 0; jj < rcount; jj++) { // Get constant noise value for each reading and apply to scans
            vnoise = -1.0 * mrec.ti_noise[ jj ];
            for (int ii = 0; ii < scount; ii++) { // Apply to all scans at reading position
               edata->scanData[ ii ].rvalues[ jj ] += vnoise;
            }
         }
      }

      if ((noiflg & 2) > 0) { // Apply ri noise
         DbgLv(1) << "bestm: noise flag" << noiflg << "RI_NOISE apply";
         for (int ii = 0; ii < scount; ii++) { // Get constant noise value for each scan and apply to readings
            vnoise = -1.0 * mrec.ri_noise[ ii ];
            for (int jj = 0; jj < rcount; jj++) { // Apply to all readings at scan position
               edata->scanData[ ii ].rvalues[ jj ] += vnoise;
            }
         }
      }
...
void US_MPI_Analysis::write_pcsa_aux_model(int iter) {
   ...
   if (is_global_fit) { // For Global Fit, de-scale solute signal concentrations
      double avg_conc = 0.0;

      for (int ee = 0; ee < data_sets.size(); ee++) { // Get average total concentration of data sets
      ...
      for (int ii = 0; ii < wksim_vals.zsolutes.size(); ii++) { // Scale concentration of solute points
   ...
   for (int ii = 0; ii < wksim_vals.zsolutes.size(); ii++) {
      ...
      US_ZSolute::set_mcomp_values(component, wksim_vals.zsolutes[ ii ], stype, true);
   ...
   tsout << fn << ";meniscus_value=" << meniscus_value << ";MC_iteration=" << iter << ";variance=" << wmodel.variance
         << ";run=Run: 1 " << tripID << "\n";

   fileo.close();

   // If final regularization or final MC iter model, write model records
   if ((iter == 0 && mc_iterations < 2) || (iter > 0 && iter >= mc_iterations)) {
...
double US_MPI_Analysis::alpha_scan() {
   ...
   for (int ja = 0; ja < nalpha; ja++) { // Loop to populate alpha scan arrays with preliminary values
      alphas << calpha;
      varias << v_vari;
      xnorms << v_xnsq;
      calpha += dalpha;
   ...
   for (int ja = 0; ja < nalpha; ja++) { // Loop to evaluate each alpha in the specified range
      calpha = alphas[ ja ];

      if (ja == 0) { // For the first one do a full compute and save the A,B matrices
         ...
         solvesim->calc_residuals(current_dataset, datasets_to_process, sim_vals, true, &sv_nnls_a, &sv_nnls_b);

         v_vari = sim_vals.variance;
         v_xnsq = sim_vals.xnormsq;
      }

      else { // After 1st, regularize by modifying A matrix diagonal, then NNLS
         apply_alpha(calpha, &sv_nnls_a, &sv_nnls_b, nscans, npoints, nisols, v_vari, v_xnsq);
      }
   ...
   for (int ja = 0; ja < nalpha; ja++) { // Scale the variance,normsq points
      varias[ ja ] *= vscl;
      xnorms[ ja ] *= xscl;
   }
   ...
   while (nlp < nalpha) {
      double avg = US_Math2::linefit(&xx, &yy, &slope, &intcp, &sigma, &corre, nlp);
      DbgLv(1) << "ASCN:H1:  avg" << avg << "nlp" << nlp;
      DbgLv(1) << "ASCN:H1:   sl" << slope << "in" << intcp << "sg" << sigma << "co" << corre;
   ...
   US_Math2::nearest_curve_point(xx, yy, nalpha, true, xlipt, ylipt, &xcipt, &ycipt, alphas.data(), &calpha);
   ...
   double xcvp2 = xx[ je ];
   double ycvp2 = yy[ je ];
   bool good_fit = (xlipt >= xcvp1 && ylipt <= ycvp1 && xlipt <= xcvp2 && ylipt >= ycvp2);
   DbgLv(1) << "ASCN:T4:   cv: x1,y1" << xcvp1 << ycvp1 << "x2,y2" << xcvp2 << ycvp2 << " good_fit" << good_fit;
...
void US_MPI_Analysis::apply_alpha(
   const double alpha, QVector<double> *psv_nnls_a, QVector<double> *psv_nnls_b, const int nscans, const int npoints,
   ...
   US_Math2::nnls(nnls_a.data(), narows, narows, nisols, nnls_b.data(), nnls_x.data());
...
void US_MPI_Analysis::fill_pcsa_queue(void) {
   ...
   for (int ii = 0; ii < orig_zsolutes.size(); ii++) {
      ...
      max_experiment_size = qMax(max_experiment_size, job.zsolutes.size());
   }
}
...
QString US_MPI_Analysis::shorter_filename(const QString fnamei) {
   ...
   if (fnlen > 99) { // Name too long:  break it down and shorten parts where possible
      ...
      if (parts[ nparts - 2 ] == "mrecs" && nchrmv < 11) { // If mrecs name, try to shorten analysis description part
         ...
         if (nchrmv < 9) { // Remove hhmm part of edit and analysis parts (- 8 chars.)
            anapart = anap0.left(7) + "_" + anap1.left(7) + "_" + anap2 + "_" + anap3 + "_" + anap4;
         }

         else { // Remove the edit part of the analysis section (- 11 chars.)
            anapart = anap1 + "_" + anap2 + "_" + anap3 + "_" + anap4;
         }

         fnameo = fnamei.section(".", 0, -4) + "." + anapart + "." + fnamei.section(".", -2, -1);
      }

      else { // Otherwise, just do brute-force name shortening
         fnameo = QString(fnamei).section(".", 0, -3);
         int kk = 97 - lnpm1 - lnpm2;
         kk = fnameo[ kk - 1 ] == '.' ? (kk - 1) : kk;
         fnameo = fnameo.left(kk) + "." + parts[ nparts - 2 ] + "." + parts[ nparts - 1 ];
      }
   }
