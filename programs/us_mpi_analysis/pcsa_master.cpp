...
void
US_MPI_Analysis::pcsa_master (void)
{
  current_dataset = 0;
  ...
  fill_pcsa_queue ();
  DbgLv (1) << "pcsa_mast: fill_pcsa_queue complete  qsize"
            << job_queue.size ();
  ...
  while (true)
    {
      ...
      while (!job_queue.isEmpty () && worker_status.contains (READY))
        { // There are jobs in the queue and workers ready
          ...
          DbgLv (1) << "pcsa_mast:   submit_pcsa kc" << kcurve
                    << "ready worker" << worker << "depth" << job.mpi_job.depth
                    << "JQue size" << job_queue.size ();

          jql = job_queue.size () - 1;
          if (jql >= 0)
            {
              DbgLv (1) << "pcsa_mast:MLOOP - (00)JQue size"
                        << job_queue.size () << "JQ[n].zsol.siz"
      ...
      if (job_queue.isEmpty () && !worker_status.contains (WORKING))
        { // No jobs left in the queue and no workers still busy
          ...
          DbgLv (1) << "final-mr0: variance1 variance2"
                    << simulation_values.variance << wksim_vals.variance;
          ...
          if (max_iterations > 1)
            {
              ...
              DbgLv (1) << "pcsa_mast:MLOOP - (02)JQue size"
                        << job_queue.size ();
              iterate_pcsa ();
              DbgLv (1) << "pcsa_mast:MLOOP - (03)JQue size"
                        << job_queue.size ();
            }
          ...
          iterations = 1;
          max_iterations = 1;
          DbgLv (1) << "pcsa_mast: loop-BOT: dssize" << data_sets.size ()
                    << "ds_to_p" << datasets_to_process << "curr_ds"
                    << current_dataset;
          ...
          if (is_global_fit)
            {
              ...
              calc_residuals (current_dataset, datasets_to_process,
                              simulation_values);
            }
      ...
      MPI_Recv (sizes, 4, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG,
...
void
US_MPI_Analysis::init_pcsa_solutes (void)
{
  ...
  DbgLv (1) << "  init_pcsa_sols: currds" << current_dataset << "vbar"
            << vbar20;
  ...
  if (ctype == CTYPE_SL)
    {
      ...
      US_ModelRecord::compute_slines (x_min, x_max, y_min, y_max, nkpts, nlpts,
                                      parlims, mrecs);
      DbgLv (1) << "  init_pcsa_sols:  compute_slines: mrecs sz"
                << mrecs.size ();
    }
  ...
  else if (ctype == CTYPE_IS || ctype == CTYPE_DS)
    {
      ...
      DbgLv (1) << "  init_pcsa_sols:  compute_sigmoids: mrecs sz"
                << mrecs.size ();
    }
  ...
  else if (ctype == CTYPE_2O)
    {
      ...
      DbgLv (1) << "  init_pcsa_sols: nkpts" << nkpts << "nkpto" << nkpto;
      mrecs.reserve (nkpto);
      US_ModelRecord::compute_2ndorder (x_min, x_max, y_min, y_max, nkpts,
                                        nlpts, parlims, mrecs);
    }
...
// Submit a PCSA job
void
US_MPI_Analysis::submit_pcsa (Sa_Job &job, int worker)
{
...
void
US_MPI_Analysis::process_pcsa_results (const int worker, const int *sizes)
{
  ...
  MPI_Recv (simulation_values.variances.data (), datasets_to_process,
            MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator, &status);
...
void
US_MPI_Analysis::process_pcsa_solutes (Result &result)
{
...
US_MPI_Analysis::write_mrecs ()
{
  ...
  DbgLv (0) << "pcsa:wrmr:  fnameo=" << fnameo << "  fn length"
            << fnameo.size ();
  QFile fileo (fnameo);
  ...
  if (fileo.open (QIODevice::WriteOnly | QIODevice::Text))
    {
      ...
      US_ModelRecord::write_modelrecs (xmlo, mrecs, s_desc, ctype, x_min,
                                       x_max, y_min, y_max, stype);
...
US_MPI_Analysis::iterate_pcsa ()
{
  ...
  if (iterations > 1)
    { // If after first iteration, check RMSD difference
      ...
      if (dr_rat < 0.0)
        { // Increase in RMSD
          qDebug () << "Increasing Last, Current RMSDs of" << rmsd_last
                    << rmsd_curr << "truncate iterations at" << iterations;
  ...
  if (ctype != CTYPE_ALL)
    ...
    { // Records that are a mix of SL,IS,DS
      ...
      DbgLv (1) << "iter_p: plims_sl" << plims_sl[0] << plims_sl[1]
                << plims_sl[2] << plims_sl[3] << plims_sl[4];
      DbgLv (1) << "iter_p: plims_is" << plims_is[0] << plims_is[1]
                << plims_is[2] << plims_is[3] << plims_is[4];
      DbgLv (1) << "iter_p: plims_ds" << plims_ds[0] << plims_ds[1]
                << plims_ds[2] << plims_ds[3] << plims_ds[4];
...
US_MPI_Analysis::tikreg_pcsa ()
{
  ...
  alpha
      = (tikreg == 1) ? parameters["tikreg_alpha"].toDouble () : alpha_scan ();
  ...
  DbgLv (1) << "tikr: RMSD" << mrec.rmsd << "csol size"
            << mrec.csolutes.size ();
...
void
US_MPI_Analysis::montecarlo_pcsa ()
{
  if (mc_iterations < 2)
    return;
  ...
  DbgLv (1) << " masterMC  mc_data size" << mcd_size << "glob"
            << is_global_fit;
      ...
      while (!job_queue.isEmpty () && worker_status.contains (READY))
        {
          ...
          DbgLv (1) << " masterMC loop-SEND PROCESS_MC  worker" << worker
                    << "kci_send" << kci_send;
          MPI_Send (&job.mpi_job, sizeof (MPI_Job), MPI_BYTE, worker,
                    MPI_Job::MASTER, my_communicator);
      ...
      switch (status.MPI_TAG)
        ...
        case MPI_Job::RESULTS_MC: // Return solute data
          ...
          MPI_Recv (wksim_vals.zsolutes.data (), sizes[0] * zsolut_doubles,
                    MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator,
                    &status);
          ...
          MPI_Recv (wksim_vals.variances.data (), datasets_to_process,
                    MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator,
                    &status);
          ...
          if (count_datasets > 1)
            {
              if (datasets_to_process == 1)
                ...
                {
                  progress
                      = "Datasets: " + QString::number (datasets_to_process)
        ...
        case MPI_Job::RESULTS: // Return solute data (unused worker?)
          DbgLv (1) << " masterMC loop-RECV RESULTS  worker" << worker
                    << "wstat" << worker_status[worker];
          { // Receives just to consume messages
            ...
            DbgLv (1) << "PM:Recv: 11:wbuf" << sizes[0] * zsolut_doubles
                      << mxsiz;
            MPI_Recv (wbuf, sizes[0] * zsolut_doubles, MPI_DOUBLE, worker,
                      MPI_Job::TAG0, my_communicator, &status);
            DbgLv (1) << "PM:Recv: 12:wbuf" << 1;
            MPI_Recv (wbuf, 1, MPI_DOUBLE, worker, MPI_Job::TAG0,
                      my_communicator, &status);
            DbgLv (1) << "PM:Recv: 13:wbuf" << datasets_to_process;
            MPI_Recv (wbuf, datasets_to_process, MPI_DOUBLE, worker,
                      MPI_Job::TAG0, my_communicator, &status);
...
void
US_MPI_Analysis::filter_mrecs (const int ctype,
...
void
US_MPI_Analysis::clean_mrecs (QVector<US_ModelRecord> &mrecs)
{
...
void
US_MPI_Analysis::pcsa_best_model ()
{
  US_Model mdummy;
  mrec = mrecs[0];
  //   simulation_values.dbg_level = dbg_level;
  ...
  DbgLv (1) << "bestm: CALC_RESID call" << current_dataset
            << datasets_to_process << "zsols size"
            << simulation_values.zsolutes.size () << "variance"
            << mrec.variance;
  ...
  if ((tikreg != 0 || mc_iterations > 1) && (noiflg != 0))
    { // Apply computed noise to data
      ...
      DbgLv (1) << "bestm: noise apply ns np sc rc" << nscan << npoint
                << scount << rcount;
...
void
US_MPI_Analysis::write_pcsa_aux_model (int iter)
{
...
void
US_MPI_Analysis::apply_alpha (const double alpha, QVector<double> *psv_nnls_a,
...
void
US_MPI_Analysis::fill_pcsa_queue (void)
{
...
QString
US_MPI_Analysis::shorter_filename (const QString fnamei)
{
        ...
        { // If mrecs name, try to shorten analysis description part
          ...
          if (nchrmv < 9)
            { // Remove hhmm part of edit and analysis parts (- 8 chars.)
              anapart = anap0.left (7) + "_" + anap1.left (7) + "_" + anap2
                        + "_" + anap3 + "_" + anap4;
            }
