...
void US_MPI_Analysis::_2dsa_master(void) {
  ...
  if (mc_iterations > 1) max_iterations = max_iters_all > 1 ? max_iters_all : 5;
  ...
  while (true) {
    ...
    if (job_queue.isEmpty() && !worker_status.contains(WORKING)) {
      ...
      if (mc_iterations > 1)
        ...
        progress += "; Meniscus: " + QString::number(meniscus_value, 'f', 4) +
                    "; Bottom: " + QString::number(bottom_value, 'f', 4) +
                    QString().sprintf("  ( m%2d b%2d )", (meniscus_run + 1),
                                      (bottom_run + 1));
      else if (fit_meni)
        progress +=
            "; Meniscus: " + QString::number(meniscus_value, 'f', 4) +
            tr(" (%1 of %2)").arg(meniscus_run + 1).arg(meniscus_points);
      else if (fit_bott)
        progress += "; Bottom: " + QString::number(bottom_value, 'f', 4) +
                    tr(" (%1 of %2)").arg(bottom_run + 1).arg(bottom_points);
      else
        progress +=
            "; RMSD: " + QString::number(sqrt(simulation_values.variance));
      ...
      DbgLv(1) << " master loop-BOT: ds" << current_dataset + 1 << "data l m h"
               << edat->value(10, 10) << edat->value(ss, rr)
               << edat->value(ks, kr);
      ...
      // Monte Carlo
      if (mc_iterations >
          1) {  // Recompute final fit to get simulation and residual
        mc_iteration++;
      ...
      if (is_composite_job) {  // Composite job:  update outputs in TAR and bump
                               // dataset count
        QString tripleID =
            QString(data_sets[current_dataset]->model.description)
                .section(".", -3, -3);
        current_dataset++;
        ...
        DbgLv(1) << " master loop-BOT:    cds kds" << current_dataset
                 << count_datasets;
        if (current_dataset < count_datasets) {
          ...
          if (menibott_count > 1) {  // Reset the range of fit-meniscus/bottom
                                     // points for this data set
            US_DataIO::EditedData* edata =
                &data_sets[current_dataset]->run_data;
            double dat_str = edata->radius(0);
            double men_dpt =
                (meniscus_points > 1) ? (double)(meniscus_points - 1) : 1;
            double bot_dpt =
                (bottom_points > 1) ? (double)(bottom_points - 1) : 1;
            ...
            if (men_end >= dat_str) {  // Adjust first meniscus so range remains
                                       // below data range
            ...
            DbgLv(0) << " master loop-BOT:     menpt" << meniscus_points
                     << "mv0 mvn" << meniscus_values[0]
                     << meniscus_values[meniscus_points - 1] << "botpt"
                     << bottom_points << "bv0 bvn" << bottom_values[0]
                     << bottom_values[bottom_points - 1] << "gcores_count"
                     << gcores_count;
          }
          ...
          for (int ii = 1; ii < gcores_count; ii++) worker_status[ii] = READY;
    ...
    if (max_depth > 0)
      DbgLv(1) << " master loop-BOTTOM:   status TAG" << status.MPI_TAG
               << MPI_Job::READY << MPI_Job::RESULTS << "  source"
               << status.MPI_SOURCE;
...
void US_MPI_Analysis::init_solutes(void) {
  ...
  if (model_filename.isEmpty()) {  // If no model file given, calculate initial
                                   // solutes in a fixed grid
    ...
    DbgLv(0) << "InSol: nss nks" << s_pts << ff0_pts << nsstep << nkstep
             << "grid_reps" << grid_reps;
    US_Solute::init_solutes(s_min, s_max, nsstep, ff0_min, ff0_max, nkstep,
                            grid_reps, cnstff0, orig_solutes);
    DbgLv(0) << "InSol:  s range" << s_min * 1.e+13 << s_max * 1.e+13
             << "k range" << ff0_min << ff0_max;
    int j0 = orig_solutes.count() - 1;
    int j1 = orig_solutes[0].count() - 1;
    int j2 = orig_solutes[j0].count() - 1;
    DbgLv(0) << "orig_solutes:" << orig_solutes[0][0].s * 1.e+13
             << orig_solutes[0][0].k << "  " << orig_solutes[0][j1].s * 1.e+13
             << orig_solutes[0][j1].k << "  " << orig_solutes[j0][0].s * 1.e+13
             << orig_solutes[j0][0].k << "  " << orig_solutes[j0][j2].s * 1.e+13
             << orig_solutes[j0][j2].k;
  }

  else {  // If a model file was given, use it to set the initial solutes
    US_Model model;
    ...
    if (nsubgrid < 1) abort("Custom grid model file has no subgrids", -1);

    if ((ncomps / nsubgrid) >
        150) {  // Subgrids too large:  adjust subgrid count and size
    ...
    for (int ii = 0; ii < nsubgrid; ii++) {
      ...
      for (int jj = ii; jj < ncomps; jj += nsubgrid) {
        ...
        US_Solute soli(model.components[jj].s, ffval, 0.0, vbval,
...
void US_MPI_Analysis::fill_queue(void) {
  ...
  for (int i = 0; i < orig_solutes.size(); i++) {
    max_experiment_size = qMax(max_experiment_size, orig_solutes[i].size());
    Sa_Job job;
...
void US_MPI_Analysis::global_fit(void) {
  ...
  if (mdl_type != US_Model::PCSA) {
    ...
    DbgLv(1) << ":gf: 2DSA: nsols" << simulation_values.solutes.size()
    ...
    DbgLv(1) << ":gf: PCSA: nzsols" << simulation_values.zsolutes.size()
             << "concen" << concentration;
  }

  qDebug() << "   == Dataset" << current_dataset + 1 << "Total Concentration"
           << concentration << "==";
  ...
  DbgLv(1) << ":gf:  ee" << current_dataset << "BB-dset(m)"
  ...
  DbgLv(0) << "ScaledData sum" << dsum << "iSum" << isum << "concen"
           << concentration;
  ...
  for (int worker = 1; worker <= my_workers; worker++) {
    job.solution = worker;
    job.depth = worker;
    MPI_Send(&job, sizeof(MPI_Job), MPI_BYTE, worker, MPI_Job::MASTER,
             my_communicator);
  }
  ...
  MPI_Bcast(scaled_data.data(), scaled_data.size(), MPI_DOUBLE, MPI_Job::MASTER,
            my_communicator);
  ...
  if (current_dataset >= count_datasets) {  // If all datasets have been scaled,
                                            // do all datasets from now on
    datasets_to_process = count_datasets;
    current_dataset = 0;
  }
...
void US_MPI_Analysis::set_meniscus(void) {
  ...
  if (fit_menbot) {
    ...
    DbgLv(0) << "FMB:set_meniscus: HUH????: mb m b:" << fit_menbot << fit_meni
             << fit_bott;
  }
  DbgLv(0) << "FMB:set_meniscus:  mb_ndx men_run bot_run" << menibott_ndx
           << meniscus_run << bottom_run << "mb m b mbc" << fit_menbot
           << fit_meni << fit_bott << menibott_count;
...
void US_MPI_Analysis::set_monteCarlo(void) {
  DbgLv(1) << "sMC: max_depth" << max_depth << "calcsols size"
           << calculated_solutes[max_depth].size() << "simvsols size"
  ...
  DbgLv(1) << "sMC:  sig-size" << sigmas.count() << "mcd-size"
           << mc_data.count() << "tot-scans" << sim_data1.scanCount();
  ...
  for (int ee = ds_start; ee < ds_end; ee++) {
    ...
    for (int ss = 0; ss < scan_count; ss++, scnx++) {
      for (int rr = 0; rr < radius_points; rr++) {
        if (scnx >= mcdscn)
          DbgLv(1) << "sMC:  *ERR* scnx" << scnx << "mcdscn" << mcdscn
                   << "ee,ss,rr" << ee << ss << rr;
        if (index >= sigsiz)
          DbgLv(1) << "sMC:  *ERR* index" << index << "sigsiz" << sigsiz
                   << "ee,ss,rr" << ee << ss << rr;
        if (index >= mcdsiz)
          DbgLv(1) << "sMC:  *ERR* index" << index << "mcdsiz" << mcdsiz
                   << "ee,ss,rr" << ee << ss << rr;
        ...
        if (index < 5 || index > (total_points - 6) ||
            (index > (indxh - 4) && index < (indxh + 3)))
          DbgLv(1) << "sMC:  index" << index << "sdat"
                   << sim_data1.value(scnx, rr) << "sigma" << sigmas[index]
                   << "vari" << variation << "mdat" << mcdata;
  ...
  qDebug() << "  Box_Muller Variation RMSD" << QString::number(varrmsd, 'f', 7)
  ...
  for (int worker = 1; worker <= my_workers; worker++) {
    MPI_Send(&newdata, sizeof(MPI_Job), MPI_BYTE, worker, MPI_Job::MASTER,
             my_communicator);
  }
  ...
  MPI_Bcast(mc_data.data(), ds_points, MPI_DOUBLE, MPI_Job::MASTER,
            my_communicator);
...
void US_MPI_Analysis::set_gaussians(void) {
  ...
  DbgLv(1) << "sGA: calcsols size mxdpth" << calculated_solutes.size()
           << max_depth;
  ...
  DbgLv(1) << "sGA:    edata scans points" << edata->scanCount()
           << edata->pointCount();
  ...
  int scnx = 0;
  DbgLv(1) << "sGA:  resids scans points" << res_data->scanCount()
           << res_data->pointCount();
  ...
  for (int ee = ds_start; ee < ds_end; ee++) {
    ...
    int scan_count = edata->scanCount();
    int radius_points = edata->pointCount();
    DbgLv(1) << "sGA:   ee" << ee << "scans points" << scan_count
             << radius_points;
...
void US_MPI_Analysis::iterate(void) {
  ...
  if (++iterations > max_iterations) {
    qDebug() << "++ Maximum refinement iterations has been reached:"
             << max_iterations;
    return;
  }
  ...
  if (iterations > 2) {
    if (diff > 0.0 && iterations > hiters) {  // Variance is getting worse
      iterations -= 2;
    ...
    if (nsols == simulation_values.solutes.size()) {
      ...
      for (int jj = 0; jj < nsols; jj++) {
        if (previous_values.solutes[jj] !=
            simulation_values.solutes[jj]) {  // Mismatch:  may need to iterate
          ssame = false;
          break;
        }

        else {  // Test if concentrations are almost the same
  ...
  if (ssame) {  // Solutes same as previous:  no more iterations
  ...
  for (int i = 0; i < orig_solutes.size(); i++) {
    ...
    max_experiment_size = qMax(max_experiment_size, job.solutes.size());
  }
...
void US_MPI_Analysis::submit(Sa_Job& job, int worker) {
  ...
  if (dd == 0) {
    DbgLv(1) << "Mast: submit: worker" << worker << "  sols"
  ...
  // Send solutes
  MPI_Send(job.solutes.data(), job.mpi_job.length * solute_doubles,
...
void US_MPI_Analysis::add_to_queue(Sa_Job& job) {
  ...
  for (int qq = 0; qq < job_queue.size(); qq++) {
    if (jdepth < job_queue[qq].mpi_job.depth) {  // Insert this job before any
...
void US_MPI_Analysis::process_results(int worker, const int* size) {
  ...
  MPI_Recv(simulation_values.solutes.data(), size[0] * solute_doubles,
  ...
  MPI_Recv(simulation_values.ti_noise.data(), size[1], MPI_DOUBLE, worker,
           MPI_Job::TAG0, my_communicator, &status);

  MPI_Recv(simulation_values.ri_noise.data(), size[2], MPI_DOUBLE, worker,
           MPI_Job::TAG0, my_communicator, &status);
  ...
  if (depth == 0) {
    DbgLv(1) << "Mast:  process_results: worker" << worker << " solsize"
             << size[0] << "depth" << depth;
  } else {
    DbgLv(1) << "Mast:  process_results:      worker" << worker << " solsize"
             << size[0] << "depth" << depth;
  ...
  while (cached_results.size() > 0 && cached_results[0].depth <= lwdepth) {
...
void US_MPI_Analysis::process_solutes(int& depth, int& worker,
  ...
  DbgLv(1) << "Mast:    process_solutes:      worker" << worker << " solsize"
           << result_solutes.size() << "depth" << depth;
  ...
  if (new_size > max_experiment_size) {
    ...
    DbgLv(1) << "Mast:   queue NEW DEPTH sols" << job.solutes.size()
             << " d=" << job.mpi_job.depth << " newsz mxesz" << new_size
             << max_experiment_size;
  ...
  if (next_depth > 1 &&
      new_size > max_experiment_size) {  // Adjust max_experiment_size if it is
  ...
  for (int d = 0; d < dcheck; d++) {
    ...
    if (!working && !queued &&
        remainder > 0) {  // Submit a job with remaining calculated solutes from
                          // an earlier depth
      int next_d = d + 1;
      Sa_Job job;
  ...
  if (depth == max_depth && job_queue.isEmpty() && !working &&
      csol_size > rsol_size) {
    Sa_Job job;
    job.solutes = calculated_solutes[depth];
    job.mpi_job.depth = next_depth;
    ...
    bottom_value = (bottom_points == 1)
                       ...
                       : bottom_values[bottom_run];
    qSort(job.solutes);
    DbgLv(1) << "Mast:   queue LAST ns=" << job.solutes.size()
             << "  d=" << depth + 1 << max_depth
    ...
    if (worker > 0) {  // Submit what should be the last job of this iteration
      ...
      if (calculated_solutes.size() > max_depth)
        ...
        calculated_solutes << QVector<US_Solute>();
    } else {  // Shouldn't happen, but put job in queue if no worker is yet
              // ready
...
int US_MPI_Analysis::low_working_depth() {
  ...
  for (int ii = 1; ii <= my_workers;
       ii++) {  // Test all worker statuses and depths
    int wdepth = worker_depth[ii];

    if (worker_status[ii] == WORKING &&
        wdepth < depth) {  // If working and low depth so far, save depth
      depth = wdepth;
    }
  }
...
void US_MPI_Analysis::cache_result(Result& result) {
  ...
  for (int ii = 0; ii < cached_results.size();
       ii++) {  // Examine all cached results
    int cdepth = cached_results[ii].depth;

    if (rdepth < cdepth) {  // Insert new result before next highest depth
