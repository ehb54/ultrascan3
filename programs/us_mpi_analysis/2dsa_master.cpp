...
void
US_MPI_Analysis::_2dsa_master (void)
{
  init_solutes ();
  fill_queue ();
      ...
      if (job_queue.isEmpty () && !worker_status.contains (WORKING))
        {
          ...
          iterations = 1;
          DbgLv (1) << " master loop-BOT: dssize" << data_sets.size ()
          ...
          int ss = ks / 2;
          int rr = kr / 2;
          DbgLv (1) << " master loop-BOT: ds" << current_dataset + 1
                    << "data l m h" << edat->value (10, 10)
                    << edat->value (ss, rr) << edat->value (ks, kr);
          DbgLv (1) << " master loop-BOT: GF job_queue empty"
                    << job_queue.isEmpty ();
              ...
              if (current_dataset < count_datasets)
                {
                  ...
                  if (menibott_count > 1)
                    { // Reset the range of fit-meniscus/bottom points for this
                      // data set
                      US_DataIO::EditedData *edata
                          = &data_sets[current_dataset]->run_data;
                      double dat_str = edata->radius (0);
                      double men_dpt = (meniscus_points > 1)
                                           ? (double)(meniscus_points - 1)
                                           : 1;
                      double bot_dpt = (bottom_points > 1)
                                           ? (double)(bottom_points - 1)
                                           : 1;
                      double men_str
                          = edata->meniscus - (meniscus_range * 0.5);
                      ...
                      if (men_end >= dat_str)
                        { // Adjust first meniscus so range remains below data
                          // range
                  ...
                  for (int ii = 1; ii < gcores_count; ii++)
                    worker_status[ii] = READY;
                  DbgLv (1)
                      << " master loop-BOT:      wkst1 wkstn"
                      << worker_status[1] << worker_status[gcores_count - 1];
...
void
US_MPI_Analysis::init_solutes (void)
{
    ...
    { // If a model file was given, use it to set the initial solutes
      ...
      if ((ncomps / nsubgrid) > 150)
        { // Subgrids too large:  adjust subgrid count and size
          ...
          DbgLv (0) << "InSol:  nsubgrid sbsize" << nsubgrid
                    << (ncomps / nsubgrid);
        }
...
void
US_MPI_Analysis::fill_queue (void)
{
  ...
  for (int i = 0; i < orig_solutes.size (); i++)
    {
      max_experiment_size
          = qMax (max_experiment_size, orig_solutes[i].size ());
      Sa_Job job;
...
//////////////////
void
US_MPI_Analysis::global_fit (void)
{
  ...
  if (mdl_type != US_Model::PCSA)
    {
      for (int solute = 0; solute < simulation_values.solutes.size ();
           solute++)
        {
    ...
    {
      for (int solute = 0; solute < simulation_values.zsolutes.size ();
           solute++)
        {
  ...
  MPI_Bcast (scaled_data.data (), scaled_data.size (), MPI_DOUBLE,
...
void
US_MPI_Analysis::set_meniscus (void)
{
  menibott_ndx++;
  ...
  if (fit_menbot)
    ...
    {
      DbgLv (0) << "FMB:set_meniscus: HUH????: mb m b:" << fit_menbot
                << fit_meni << fit_bott;
    }
...
void
US_MPI_Analysis::set_monteCarlo (void)
{
          ...
          for (int rr = 0; rr < radius_points; rr++)
            {
              ...
              if (index < 5 || index > (total_points - 6)
                ...
                DbgLv (1) << "sMC:  index" << index << "sdat"
                          << sim_data1.value (scnx, rr) << "sigma"
                          << sigmas[index] << "vari" << variation << "mdat"
                          << mcdata;
  ...
  qDebug () << "  Box_Muller Variation RMSD"
            << QString::number (varrmsd, 'f', 7) << "  for MC_Iteration"
            << mc_iteration + 1;
...
void
US_MPI_Analysis::set_gaussians (void)
{
  US_DataIO::EditedData *edata = &data_sets[current_dataset]->run_data;
  int ds_start = 0;
  int ds_end = count_datasets;
  ...
  if (iterations > 2)
    {
      ...
      if (adiff < min_variance_improvement)
        {
          qDebug () << "++ Refinement iteration improvement near-zero:"
                    << adiff;
          return;
        }
              ...
              if (previous_values.solutes[jj] != simulation_values.solutes[jj])
                ...
                { // Test if concentrations are almost the same
                  ...
                  if (cdiff > 1.0e-1) // Greater than 10% concentration diff?
                    { // Significant concentration mismatch:  may need to
                      // iterate
                      ssame = false;
                      break;
                    }
...
// Submit a queued job
void
US_MPI_Analysis::submit (Sa_Job &job, int worker)
{
...
void
US_MPI_Analysis::add_to_queue (Sa_Job &job)
{
...
void
US_MPI_Analysis::process_results (int worker, const int *size)
{
...
void
US_MPI_Analysis::process_solutes (int &depth, int &worker,
  ...
  for (int d = 0; d < dcheck; d++)
    {
      ...
      if (!working && !queued && remainder > 0)
        { // Submit a job with remaining calculated solutes from an earlier
          // depth
          int next_d = d + 1;
          Sa_Job job;
          ...
          qSort (job.solutes);
          add_to_queue (job);
          DbgLv (1) << "Mast:   queue REMAINDER" << remainder
                    << " d=" << d + 1;
...
int
US_MPI_Analysis::low_working_depth ()
{
  ...
  for (int ii = 1; ii <= my_workers; ii++)
    { // Test all worker statuses and depths
      ...
      if (worker_status[ii] == WORKING && wdepth < depth)
...
void
US_MPI_Analysis::cache_result (Result &result)
{
