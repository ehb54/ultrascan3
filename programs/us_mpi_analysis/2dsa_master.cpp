...
void US_MPI_Analysis::_2dsa_master(void) {
   ...
   while (true) {
      int worker;
      meniscus_value = (meniscus_points == 1) ? data_sets[ current_dataset ]->run_data.meniscus
                                              : meniscus_values[ meniscus_run ];
      bottom_value = (bottom_points == 1) ? data_sets[ current_dataset ]->run_data.bottom : bottom_values[ bottom_run ];
      //if ( max_depth > 1 )
      ...
      if (job_queue.isEmpty() && !worker_status.contains(WORKING)) {
         ...
         if (mc_iterations > 1)
            ...
            progress += "; Meniscus: " + QString::number(meniscus_value, 'f', 4)
                        ...
                        + QString().sprintf("  ( m%2d b%2d )", (meniscus_run + 1), (bottom_run + 1));
         else if (fit_meni)
            progress += "; Meniscus: " + QString::number(meniscus_value, 'f', 4)
         ...
         if (max_iterations > 1) {
            if (data_sets.size() > 1 && iterations == 1) {
               if (datasets_to_process == 1) {
                  ...
                  qDebug() << "   == Refinement Iterations for Datasets 1 to" << datasets_to_process << "==";
               }
            }
            qDebug() << "Iteration:" << iterations << " Variance:" << simulation_values.variance
         ...
         iterations = 1;
         DbgLv(1) << " master loop-BOT: dssize" << data_sets.size() << "ds_to_p" << datasets_to_process << "curr_ds"
                  << current_dataset;
         ...
         int ss = ks / 2;
         int rr = kr / 2;
         DbgLv(1) << " master loop-BOT: ds" << current_dataset + 1 << "data l m h" << edat->value(10, 10)
                  << edat->value(ss, rr) << edat->value(ks, kr);
         ...
         if (mc_iterations > 1) { // Recompute final fit to get simulation and residual
            ...
            qDebug() << "Base-Sim RMSD" << sqrt(simulation_values.variance) << "  Exp-Sim RMSD"
                     << sqrt(wksim_vals.variance) << "  of MC_Iteration" << mc_iteration;
         ...
         if (is_composite_job) { // Composite job:  update outputs in TAR and bump dataset count
            QString tripleID = QString(data_sets[ current_dataset ]->model.description).section(".", -3, -3);
            current_dataset++;
            ...
            if (simulation_values.noisflag == 0) {
               DbgLv(0) << my_rank << ": Dataset" << current_dataset << "(" << tripleID << ")"
                        << " :  model was output.";
            }
            else {
               DbgLv(0) << my_rank << ": Dataset" << current_dataset << "(" << tripleID << ")"
            ...
            if (current_dataset < count_datasets) {
               ...
               if (menibott_count > 1) { // Reset the range of fit-meniscus/bottom points for this data set
                  US_DataIO::EditedData *edata = &data_sets[ current_dataset ]->run_data;
                  double dat_str = edata->radius(0);
                  double men_dpt = (meniscus_points > 1) ? ( double ) (meniscus_points - 1) : 1;
                  double bot_dpt = (bottom_points > 1) ? ( double ) (bottom_points - 1) : 1;
                  ...
                  DbgLv(0) << " master loop-BOT:     menpt" << meniscus_points << "mv0 mvn" << meniscus_values[ 0 ]
                           << meniscus_values[ meniscus_points - 1 ] << "botpt" << bottom_points << "bv0 bvn"
                           << bottom_values[ 0 ] << bottom_values[ bottom_points - 1 ] << "gcores_count"
                           << gcores_count;
               }
...
void US_MPI_Analysis::init_solutes(void) {
   ...
   if (model_filename.isEmpty()) { // If no model file given, calculate initial solutes in a fixed grid
      ...
      US_Solute::init_solutes(s_min, s_max, nsstep, ff0_min, ff0_max, nkstep, grid_reps, cnstff0, orig_solutes);
      DbgLv(0) << "InSol:  s range" << s_min * 1.e+13 << s_max * 1.e+13 << "k range" << ff0_min << ff0_max;
      int j0 = orig_solutes.count() - 1;
      int j1 = orig_solutes[ 0 ].count() - 1;
      int j2 = orig_solutes[ j0 ].count() - 1;
      DbgLv(0) << "orig_solutes:" << orig_solutes[ 0 ][ 0 ].s * 1.e+13 << orig_solutes[ 0 ][ 0 ].k << "  "
      ...
      if ((ncomps / nsubgrid) > 150) { // Subgrids too large:  adjust subgrid count and size
      ...
      for (int ii = 0; ii < nsubgrid; ii++) {
         ...
         for (int jj = ii; jj < ncomps; jj += nsubgrid) {
            ...
            US_Solute soli(model.components[ jj ].s, ffval, 0.0, vbval, model.components[ jj ].D);
            solvec << soli;
         }
...
void US_MPI_Analysis::global_fit(void) {
   ...
   qDebug() << "   == Dataset" << current_dataset + 1 << "Total Concentration" << concentration << "==";
   ...
   for (int worker = 1; worker <= my_workers; worker++) {
      job.solution = worker;
      job.depth = worker;
      MPI_Send(&job, sizeof(MPI_Job), MPI_BYTE, worker, MPI_Job::MASTER, my_communicator);
   }

   // Get everybody synced up
   MPI_Barrier(my_communicator);

   MPI_Bcast(scaled_data.data(), scaled_data.size(), MPI_DOUBLE, MPI_Job::MASTER, my_communicator);
   ...
   if (current_dataset >= count_datasets) { // If all datasets have been scaled, do all datasets from now on
      datasets_to_process = count_datasets;
      current_dataset = 0;
   }
...
void US_MPI_Analysis::set_meniscus(void) {
   ...
   if (fit_menbot) {
      ...
      DbgLv(0) << "FMB:set_meniscus: HUH????: mb m b:" << fit_menbot << fit_meni << fit_bott;
   }
   DbgLv(0) << "FMB:set_meniscus:  mb_ndx men_run bot_run" << menibott_ndx << meniscus_run << bottom_run << "mb m b mbc"
...
void US_MPI_Analysis::set_monteCarlo(void) {
   ...
   if (mc_iteration == 1) {
      ...
      qDebug() << "  MC_Iteration 1 Simulation RMSD" << QString::number(fitrmsd, 'f', 7);
   }
   ...
   DbgLv(1) << "sMC:  sig-size" << sigmas.count() << "mcd-size" << mc_data.count() << "tot-scans"
            << sim_data1.scanCount();
      ...
      for (int ss = 0; ss < scan_count; ss++, scnx++) {
         for (int rr = 0; rr < radius_points; rr++) {
            ...
            if (index < 5 || index > (total_points - 6) || (index > (indxh - 4) && index < (indxh + 3)))
               DbgLv(1) << "sMC:  index" << index << "sdat" << sim_data1.value(scnx, rr) << "sigma" << sigmas[ index ]
   ...
   DbgLv(1) << "sMC:   variation  sum min max" << varisum << varimin << varimax << "mcdata sum" << datasum;
   ...
   for (int worker = 1; worker <= my_workers; worker++) {
      MPI_Send(&newdata, sizeof(MPI_Job), MPI_BYTE, worker, MPI_Job::MASTER, my_communicator);
   }
   ...
   MPI_Bcast(mc_data.data(), ds_points, MPI_DOUBLE, MPI_Job::MASTER, my_communicator);
...
void US_MPI_Analysis::set_gaussians(void) {
   ...
   DbgLv(1) << "sGA:   sol0.s solM.s" << simulation_values.solutes[ 0 ].s << simulation_values.solutes[ mm ].s
            << "  M=" << mm;
   ;
...
void US_MPI_Analysis::iterate(void) {
   ...
   if (iterations > 2) {
      if (diff > 0.0 && iterations > hiters) { // Variance is getting worse
         ...
         qDebug() << "++ Refinement iteration degrading" << diff << ". Reverting to iteration" << iterations;
         return;
      }
      ...
      if (nsols == simulation_values.solutes.size()) {
         ...
         for (int jj = 0; jj < nsols; jj++) {
            if (previous_values.solutes[ jj ] != simulation_values.solutes[ jj ]) { // Mismatch:  may need to iterate
               ssame = false;
               break;
            }
   ...
   if (ssame) { // Solutes same as previous:  no more iterations
      qDebug() << "++ Refinement iteration solutes same as previous:" << previous_values.solutes.size()
...
void US_MPI_Analysis::submit(Sa_Job &job, int worker) {
   ...
   if (dd == 0) {
      DbgLv(1) << "Mast: submit: worker" << worker << "  sols" << job.mpi_job.length << "mciter cds" << mc_iteration
   ...
   // Send solutes
   MPI_Send(
      job.solutes.data(), job.mpi_job.length * solute_doubles,
...
void US_MPI_Analysis::add_to_queue(Sa_Job &job) {
   ...
   for (int qq = 0; qq < job_queue.size(); qq++) {
      if (jdepth < job_queue[ qq ].mpi_job.depth) { // Insert this job before any with a greater depth
...
void US_MPI_Analysis::process_results(int worker, const int *size) {
   ...
   MPI_Recv(
      simulation_values.solutes.data(), size[ 0 ] * solute_doubles, MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator,
      &status);
   ...
   MPI_Recv(simulation_values.ti_noise.data(), size[ 1 ], MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator, &status);

   MPI_Recv(simulation_values.ri_noise.data(), size[ 2 ], MPI_DOUBLE, worker, MPI_Job::TAG0, my_communicator, &status);

   worker_status[ worker ] = INIT;
   int depth = worker_depth[ worker ];

   if (depth == 0) {
      DbgLv(1) << "Mast:  process_results: worker" << worker << " solsize" << size[ 0 ] << "depth" << depth;
   }
   else {
      DbgLv(1) << "Mast:  process_results:      worker" << worker << " solsize" << size[ 0 ] << "depth" << depth;
   }
...
void US_MPI_Analysis::process_solutes(int &depth, int &worker, QVector<US_Solute> &result_solutes) {
   DbgLv(1) << "Mast:    process_solutes:      worker" << worker << " solsize" << result_solutes.size() << "depth"
            << depth;
   ...
   if (new_size > max_experiment_size) {
      ...
      DbgLv(1) << "Mast:   queue NEW DEPTH sols" << job.solutes.size() << " d=" << job.mpi_job.depth << " newsz mxesz"
               << new_size << max_experiment_size;
   ...
   if (next_depth > 1 && new_size > max_experiment_size) { // Adjust max_experiment_size if it is only large enough for one output
      max_experiment_size = (new_size * 11 + 9) / 10; // 10% above
      DbgLv(1) << "Mast:    NEW max_exp_size" << max_experiment_size << "from new_size rsol_size" << new_size
               << rsol_size;
   }
   ...
   for (int d = 0; d < dcheck; d++) {
      ...
      if (!working && !queued && remainder > 0) { // Submit a job with remaining calculated solutes from an earlier depth
         int next_d = d + 1;
         Sa_Job job;
   ...
   if (depth == max_depth && job_queue.isEmpty() && !working && csol_size > rsol_size) {
      Sa_Job job;
      job.solutes = calculated_solutes[ depth ];
      job.mpi_job.depth = next_depth;
      meniscus_value = (meniscus_points == 1) ? data_sets[ current_dataset ]->run_data.meniscus
      ...
      else { // Shouldn't happen, but put job in queue if no worker is yet ready
...
int US_MPI_Analysis::low_working_depth() {
   ...
   for (int ii = 1; ii <= my_workers; ii++) { // Test all worker statuses and depths
      int wdepth = worker_depth[ ii ];

      if (worker_status[ ii ] == WORKING && wdepth < depth) { // If working and low depth so far, save depth
         depth = wdepth;
      }
   }
...
void US_MPI_Analysis::cache_result(Result &result) {
   ...
   for (int ii = 0; ii < cached_results.size(); ii++) { // Examine all cached results
