...
void
US_MPI_Analysis::pmasters_start ()
{
...
void
US_MPI_Analysis::task_parse (const QString &xmlfile)
{
  if (xmlfile.isEmpty ())
    {
      if (my_rank == 0)
...
void
US_MPI_Analysis::pmasters_supervisor ()
{
  ...
  while (true)
    {
      ...
      DbgLv (1) << "SUPER: mgr kgr jgr" << mgroup << kgroup << jgroup
                << "left iter iters" << nileft << mc_iteration
                << mc_iterations;
...
void
US_MPI_Analysis::pmasters_master ()
{
  MPI_Status status;
  QByteArray msg;
...
void
US_MPI_Analysis::pmasters_worker ()
{
  if (analysis_type.startsWith ("2DSA"))
...
void
US_MPI_Analysis::time_mc_iterations ()
{
  ...
  if (mc_iteration < (mgroup_count * 3))
    {
      if (show_mcit)
        DbgLv (0) << "TMI: mc_iter < pmgc*3" << mc_iteration
                  << (mgroup_count * 3);
  ...
  if (show_mcit)
    {
      DbgLv (0)
          << "TMI: NEAR-ALLOW_TIME test: mins_so_far max_wt mins_left_allow"
          << mins_so_far << max_walltime << mins_left_allow;
      DbgLv (0) << "TMI: NEAR-ALLOW_TIME test:  mc_iters_left mc_iters_estim "
                   "mc_iters"
...
void
US_MPI_Analysis::pm_2dsa_master (void)
{
...
void
US_MPI_Analysis::pm_ga_master (void)
{
  current_dataset = 0;
  datasets_to_process = data_sets.size ();
  max_depth = 0;
...
void
US_MPI_Analysis::pm_dmga_master (void)
{
  current_dataset = 0;
  datasets_to_process = data_sets.size ();
  max_depth = 0;
