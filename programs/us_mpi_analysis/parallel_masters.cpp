...
void US_MPI_Analysis::task_parse(const QString& xmlfile) {
  ...
  if (!file.open(QIODevice::ReadOnly |
                 QIODevice::Text)) {  // If no job xml or us3.pbs, return now
    if (my_rank == 0) DbgLv(0) << "Cannot open file " << xmlfile;

    if (proc_count > 64) {
      task_params["mgroupcount"] = parameters["req_mgroupcount"];
      if (my_rank == 0)
        DbgLv(0) << " Value of mgroupcount set to"
                 << parameters["req_mgroupcount"];
    }
    return;
  }

  if (xmlfile == "us3.pbs") {  // If no jobxmlfile.xml, but us3.pbs (-local),
                               // get values from it
    QTextStream ts(&file);
    ...
    file.close();
    if (my_rank == 0)
      DbgLv(0) << "walltime=" << task_params["walltime"]
  ...
  while (!xml.atEnd()) {  // Parse the *jobxmlfile.xml file  (mainly for
                          // walltime,mgroupcount)
    xml.readNext();
    ...
    if (xml.isEndElement() && xml.name() == "Header") break;
  }

  file.close();
  if (my_rank == 0)
    DbgLv(1) << "walltime=" << task_params["walltime"]
             << "mgroupcount=" << task_params["mgroupcount"];
}
...
void US_MPI_Analysis::pmasters_supervisor() {
  ...
  while (true) {
    ...
    DbgLv(1) << "SUPER:  Send next iter" << iter_wk << "gr ma tg" << jgroup
             << master << tag;
...
void US_MPI_Analysis::time_mc_iterations() {
  ...
  if (is_composite_job) {
    if (show_mcit) DbgLv(0) << "TMI: composite_job";
  ...
  if (show_mcit) {
    ...
    DbgLv(0)
        << "TMI: NEAR-ALLOW_TIME test:  mc_iters_left mc_iters_estim mc_iters"
        << mc_iters_left << mc_iters_estim << mc_iterations;
  }

  if (mc_iters_estim < mc_iterations &&
      mc_iters_left <
          4) {  // In danger of exceeding allowed time:   reduce MC iterations
...
void US_MPI_Analysis::pm_2dsa_master(void) {
  ...
  while (true) {
    ...
    if (job_queue.isEmpty() && !worker_status.contains(WORKING)) {
      ...
      // Write out the model
      max_rss();
      DbgLv(1) << "2dMast:    mc_iter" << mc_iteration << "variance"
...
void US_MPI_Analysis::pm_ga_master(void) {
  ...
  while (true) {
    ...
    if (mc_iteration < mc_iterations) {  // Before last iteration:  check if max
                                         // is reset based on time limit
