...
void
US_MPI_Analysis::pm_cjobs_start ()
{
...
void
US_MPI_Analysis::pm_cjobs_supervisor ()
{
  ...
  for (int ii = 0; ii < mgroup_count; ii++)
    {
      ...
      MPI_Send (&current_dataset, 1, MPI_INT, master, STARTITER,
                MPI_COMM_WORLD);
  ...
  while (true)
    {
      ...
      MPI_Recv (&iwork, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG,
      ...
      tag = status.MPI_TAG;
      QByteArray msg;
      DbgLv (1) << "SUPER:   wait recv'd iwork" << iwork << "master tag"
                << master << tag;
      ...
      switch (tag)
        ...
        case UDPSIZE: // UDP message size: get the following UDP message string
          ...
          MPI_Recv (msg.data (), isize, MPI_BYTE, master, UDPMSG,
                    MPI_COMM_WORLD, &status);
        ...
        case DONEITER: // Dataset done
          //            islast   = ( ittest >= count_datasets ) ? true :
          //            islast;
        ...
        { // Just finished was last dataset for that group
          ...
          if (nileft == 0)
            { // All are complete
              ...
              DbgLv (1) << "SUPER: nileft==0  count_datasets"
                        << count_datasets;
              break;
            }
...
void
US_MPI_Analysis::pm_cjobs_master ()
{
  MPI_Status status;
  QByteArray msg;
  ...
  MPI_Recv (msg.data (), isize, MPI_BYTE, super, ADATE, MPI_COMM_WORLD,
            &status);

  analysisDate = QString (msg);
  DbgLv (1) << "  MASTER:   Recv'd from super. (g r m)" << my_group
            << group_rank << my_rank << "analysisDate" << analysisDate
            << "atype" << analysis_type;
...
void
US_MPI_Analysis::pm_cjobs_worker ()
{
  if (analysis_type.startsWith ("2DSA"))
...
void
US_MPI_Analysis::time_datasets_left ()
{
...
void
US_MPI_Analysis::pm_2dsa_cjmast (void)
{
      ...
      if (job_queue.isEmpty () && !worker_status.contains (WORKING))
        {
          ...
          if (mc_iterations > 1)
            ...
            progress
                += "; Meniscus: " + QString::number (meniscus_value, 'f', 3)
                   + tr (" (%1 of %2)")
                         .arg (meniscus_run + 1)
                         .arg (menisc_size);
              ...
              if (ittest < count_datasets)
                {
                  ...
                  MPI_Recv (&iter, 1, MPI_INT, super, MPI_ANY_TAG,
                            MPI_COMM_WORLD, &status);
                  ...
                  if (meniscus_points > 1)
                    { // Reset the range of fit-meniscus points for this data
                      // set
                      US_DataIO::EditedData *edata
                          = &data_sets[current_dataset]->run_data;
                      double men_str = edata->meniscus - meniscus_range / 2.0;
                      double men_inc
                          = meniscus_range / (meniscus_points - 1.0);
                      double dat_str = edata->radius (0);
                      double men_end = men_str + meniscus_range - men_inc;
                      if (men_end >= dat_str)
...
void
US_MPI_Analysis::pm_ga_cjmast (void)
{
  current_dataset = 0;
  ...
  while (true)
    {
      ...
      int tag = status.MPI_TAG;
      DbgLv (1) << "  MASTER: iter" << iter << "gr" << my_group << "tag"
                << tag;
      switch (tag)
        {
        case STARTLAST:
      ...
      bool do_write
          = (mc_iteration > 1) || (mc_iteration == 1 && my_group == 0);
      DbgLv (1) << "2dMast:    do_write" << do_write << "mc_iter"
                << mc_iteration << "variance" << simulation_values.variance
                << "my_group" << my_group;
...
void
US_MPI_Analysis::pm_dmga_cjmast (void)
{
  current_dataset = 0;
  datasets_to_process = data_sets.size ();
  max_depth = 0;
...
void
US_MPI_Analysis::pm_pcsa_cjmast (void)
{
