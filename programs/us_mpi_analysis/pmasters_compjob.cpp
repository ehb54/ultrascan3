...
void US_MPI_Analysis::pm_cjobs_start() {
  ...
  else if (my_group == 0) {  // Group 0 Master(1) and workers(2,...): set group
                             // rank 0,   1,...
    group_rank--;
  }
  ...
  int sstat =
      MPI_Comm_split(MPI_COMM_WORLD, my_group, group_rank, &my_communicator);
  DbgLv(1) << "COMM_SPLIT (g r m)" << my_group << group_rank << my_rank
           << "stat" << sstat;

  if (my_rank ==
      0) {  // Run parallel-masters supervisor             (world rank 0)
    pm_cjobs_supervisor();
  }

  else if (group_rank ==
           0) {  // Run parallel-masters master within a group  (group rank 0)
    pm_cjobs_master();
  }
  ...
  DbgLv(1) << "Final-my_rank" << my_rank
           << " msecs=" << startTime.msecsTo(QDateTime::currentDateTime());
  MPI_Finalize();
  exit(0);
}
...
void US_MPI_Analysis::pm_cjobs_supervisor() {
  ...
  for (int ii = 0; ii < mgroup_count; ii++) {
    ...
    DbgLv(1) << "SUPER: master msgs" << master << "analysisDate"
             << analysisDate;
  ...
  while (true) {
    ...
    MPI_Recv(&iwork, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD,
             &status);
    bool udpmsg = false;
    bool islast = false;
    int isize = 0;
    ...
    tag = status.MPI_TAG;
    QByteArray msg;
    DbgLv(1) << "SUPER:   wait recv'd iwork" << iwork << "master tag" << master
             << tag;
    ...
    switch (tag) {
      case UDPSIZE:  // UDP message size: get the following UDP message string
        ...
        MPI_Recv(msg.data(), isize, MPI_BYTE, master, UDPMSG, MPI_COMM_WORLD,
                 &status);
      ...
      case DONEITER:  // Dataset done
        ...
        DbgLv(1) << "SUPER:     DONEITER  islast" << islast
                 << "ittest currds cntds" << ittest << current_dataset
                 << count_datasets;
        break;
    ...
    DbgLv(1) << "SUPER: mgr kgr jgr" << mgroup << kgroup << jgroup
             << "left dsets dset" << nileft << count_datasets
             << current_dataset;
    ...
    if (islast) {              // Just finished was last dataset for that group
      ...
      if (mgroup < 0) continue;  // Some iters are still working
    }
    ...
    DbgLv(1) << "SUPER:  Send curr_ds cnt_ds" << current_dataset
             << count_datasets << "gr ma tg" << jgroup << master << tag;
...
void US_MPI_Analysis::pm_cjobs_master() {
  ...
  DbgLv(1) << "  MASTER   Recv from super. my_rank" << my_rank << "group"
           << my_group;
  int super = 0;
  int isize = 0;
  ...
  DbgLv(1) << "  MASTER:   Recv'd from super. (g r m)" << my_group << group_rank
           << my_rank << "analysisDate" << analysisDate << "atype"
           << analysis_type;
...
void US_MPI_Analysis::time_datasets_left() {
  if (current_dataset < 4) return;  // Don't bother until current_dataset 4
  ...
  if (ds_iters_estim < count_datasets &&
      ds_iters_left < 4) {  // In danger of exceeding allowed time:   reduce
                            // count of datasets
...
void US_MPI_Analysis::pm_2dsa_cjmast(void) {
  ...
  MPI_Recv(&current_dataset, 1, MPI_INT, super, MPI_ANY_TAG, MPI_COMM_WORLD,
           &status);
  ...
  if (meniscus_points >
      1) {  // Reset the range of fit-meniscus points for this data set
    ...
    if (men_end >=
        dat_str) {  // Adjust first meniscus so range remains below data range
  ...
  while (true) {
    ...
    if (job_queue.isEmpty() && !worker_status.contains(WORKING)) {
      ...
      if (ittest >= count_datasets) {
        for (int jj = 1; jj <= my_workers; jj++) maxrss += work_rss[jj];
      }
      ...
      if (current_dataset < count_datasets) {
        if (my_group == 0 &&
            ittest < count_datasets) {  // If group 0 master, create an
        ...
        if (ittest < count_datasets) {
          ...
          MPI_Recv(&iter, 1, MPI_INT, super, MPI_ANY_TAG, MPI_COMM_WORLD,
                   &status);
...
void US_MPI_Analysis::pm_ga_cjmast(void) {
  ...
  while (true) {
    ...
    DbgLv(1) << "2dMast:    do_write" << do_write << "mc_iter" << mc_iteration
             << "variance" << simulation_values.variance << "my_group"
             << my_group;
    ...
    if (my_group ==
        0) {  // Update the tar file of outputs in case of an aborted run
      update_outputs();
    }

    if (current_dataset < count_datasets) {  // Before last iteration:  check if
                                             // max is reset based on time limit
    ...
    // Tell the supervisor that an iteration is done
    iter = (int)maxrss;
    DbgLv(1) << "GaMast:  iter done: maxrss" << iter << "tag" << tag
             << DONELAST;
...
void US_MPI_Analysis::pm_pcsa_cjmast(void) {
  ...
  DbgLv(1) << my_rank << ": recvd ds" << current_dataset << "mgc ittest"
           << mgroup_count << ittest;
  ...
  while (true) {
    ...
    // if ( max_depth > 0 )
    DbgLv(1) << my_rank << ": PMG master loop-BOTTOM:   status TAG"
             << status.MPI_TAG << MPI_Job::READY << MPI_Job::RESULTS
