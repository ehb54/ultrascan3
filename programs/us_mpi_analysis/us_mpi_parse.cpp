   ...
   while (!xml.atEnd()) {
      ...
      if (xml.isStartElement()) {
         ...
         if (xml.name() == "dataset") {
            ...
            if (parameters.contains("CG_model")) { // Flag Custom Grid model input
            ...
            else if (parameters.contains("DC_model")) { // Flag DMGA_Constraints model input
               d->model_file = parameters[ "DC_model" ];
            }

            else if (!parameters[ "ztype" ].isEmpty()) { // Flag generic x,y model with fixed "z"
               QStringList s_attrs;
            ...
            else if (parameters[ "ytype" ] == "vbar") { // Flag s,vbar model with fixed f_f0
               d->solute_type = 1;
               attr_x = ATTR_S;
               attr_y = ATTR_V;
               attr_z = ATTR_K;
            ...
            else if (!parameters[ "solute_type" ].isEmpty()) { // Flag explicit solute type
            ...
            else { // Flag standard s,k model with fixed vbar
               d->solute_type = 0;
               attr_x = ATTR_S;
               attr_y = ATTR_K;
               attr_z = ATTR_V;
            ...
            if (my_rank == 0)
               DbgLv(0) << "PF: solute_type" << d->solute_type << "attr_x _y _z" << attr_x << attr_y << attr_z;
            data_sets << d;
         }
      }
   }
   ...
   while (!xml.atEnd()) {
      ...
      if (xml.isStartElement()) {
         ...
         if (xname == "cluster") {
            ...
            QString ss;
            requestID = ss.sprintf("%06d", attr.value("id").toString().toInt());
            ...
            while (!xml.atEnd()) {
               ...
               if (xml.isEndElement() && name == "jobParameters")
                  break;

               if (xml.isStartElement()) {
                  attr = xml.attributes();

                  if (name == "bucket") { // Get bucket coords; try to forestall round-off problems
                     ...
                     if (my_rank == 0)
                        DbgLv(0) << "PF:   rnd xymnmx" << b.x_min << b.x_max << b.y_min << b.y_max;
                     ...
                     if (my_rank == 0)
                        DbgLv(0) << "PF:   DC_model" << parameters[ name ] << name;
                  }
                  ...
                  else if (name.startsWith("s_m") || name.startsWith("ff0_m")) {
      ...
      dbg_timing = (parameters.contains("debug_timings") && parameters[ "debug_timings" ].toInt() != 0);
   }
}
...
void US_MPI_Analysis::parse_dataset(QXmlStreamReader &xml, DATASET *dataset) {
   ...
   while (!xml.atEnd()) {
      ...
      if (xml.name() == "simpoints") {
         dataset->simparams.simpoints = a.value("value").toString().toInt();
      }

      if (xml.name() == "band_volume") {
         dataset->simparams.band_volume = a.value("value").toString().toDouble();
      }

      if (xml.name() == "radial_grid") {
         dataset->simparams.meshType = ( US_SimulationParameters::MeshType ) a.value("value").toString().toInt();
      }

      if (xml.name() == "time_grid") {
         dataset->simparams.gridType = ( US_SimulationParameters::GridType ) a.value("value").toString().toInt();
      }
      ...
      if (xml.name() == "rotor_stretch") {
         QStringList stretch = a.value("value").toString().split(" ", QString::SkipEmptyParts);
      ...
      if (xml.name() == "centerpiece_shape") {
         ...
         shapes << "sector"
                ...
                << "circular"
                << "synthetic";
         dataset->simparams.cp_sector = qMax(0, shapes.indexOf(shape));
      }

      if (xml.name() == "centerpiece_angle") {
         dataset->simparams.cp_angle = a.value("value").toString().toDouble();
      }

      if (xml.name() == "centerpiece_pathlength") {
         dataset->simparams.cp_pathlen = a.value("value").toString().toDouble();
      }

      if (xml.name() == "centerpiece_width") {
         dataset->simparams.cp_width = a.value("value").toString().toDouble();
      }
...
void US_MPI_Analysis::parse_files(QXmlStreamReader &xml, DATASET *dataset) {
   while (!xml.atEnd()) {
      ...
      if (xml.isEndElement() && xml.name() == "files")
         break;
      ...
      if (xml.isStartElement()) {
         ...
         if (type == "auc")
            dataset->auc_file = filename;
         if (type == "edit")
            dataset->edit_file = filename;
         if (type == "noise")
            dataset->noise_files << filename;
         if (type == "timestate")
            dataset->tmst_file = filename;
      }
   }

   QString clambda = dataset->edit_file.section(".", -2, -2);
   if (clambda.contains("-")) { // Modify edit file name for MWL case
      ...
      if (my_rank == 0)
         DbgLv(0) << "PF: MWL edit_file" << dataset->edit_file;
   }
}
