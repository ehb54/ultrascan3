      ...
      if (xml.name() == "dataset") {
        ...
        if (parameters.contains("CG_model")) {  // Flag Custom Grid model input
          ...
          attr_z = ATTR_K;
                     ...
                     "DC_model")) {  // Flag DMGA_Constraints model input
          d->model_file = parameters["DC_model"];
        }

        else if (!parameters["ztype"]
                      .isEmpty()) {  // Flag generic x,y model with fixed "z"
          QStringList s_attrs;
        ...
        else if (parameters["ytype"] ==
                 "vbar") {  // Flag s,vbar model with fixed f_f0
          d->solute_type = 1;
          attr_x = ATTR_S;
          attr_y = ATTR_V;
          attr_z = ATTR_K;
        ...
        else if (!parameters["solute_type"]
                      .isEmpty()) {  // Flag explicit solute type
        ...
        if (my_rank == 0)
          DbgLv(0) << "PF: solute_type" << d->solute_type << "attr_x _y _z"
...
void US_MPI_Analysis::parse_job(QXmlStreamReader& xml) {
  ...
  while (!xml.atEnd()) {
      ...
      if (xname == "cluster") {
        ...
        while (!xml.atEnd()) {
          ...
          if (xml.isStartElement()) {
            ...
            if (name == "bucket") {  // Get bucket coords; try to forestall
                                     // round-off problems
              ...
              if (my_rank == 0)
                DbgLv(0) << "PF:   rnd xymnmx" << b.x_min << b.x_max << b.y_min
                         << b.y_max;
    ...
    if (!parameters.contains("z_value")) parameters["z_value"] = "0.0";
...
void US_MPI_Analysis::parse_dataset(QXmlStreamReader& xml, DATASET* dataset) {
  ...
  while (!xml.atEnd()) {
    ...
    if (xml.name() == "files") parse_files(xml, dataset);

    if (xml.name() == "solution") parse_solution(xml, dataset);
    ...
    if (xml.name() == "radial_grid") {
      dataset->simparams.meshType =
          (US_SimulationParameters::MeshType)a.value("value")
              .toString()
              .toInt();
    }

    if (xml.name() == "time_grid") {
      dataset->simparams.gridType =
          (US_SimulationParameters::GridType)a.value("value")
              .toString()
              .toInt();
    }
...
void US_MPI_Analysis::parse_solution(QXmlStreamReader& xml, DATASET* dataset) {
  while (!xml.atEnd()) {
    ...
    if (xml.isStartElement() && xml.name() == "analyte") {
      ...
      if (atype == "DNA") aninfo.analyte.type = US_Analyte::DNA;
      if (atype == "RNA") aninfo.analyte.type = US_Analyte::RNA;
      if (atype == "Other") aninfo.analyte.type = US_Analyte::CARBOHYDRATE;
