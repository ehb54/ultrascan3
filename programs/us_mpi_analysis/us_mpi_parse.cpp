...
//  Parse XML routines
void
US_MPI_Analysis::parse (const QString &xmlfile)
{
  QFile file (xmlfile);
          ...
          if (xml.name () == "dataset")
            {
              ...
              if (my_rank == 0)
                DbgLv (0) << "PF: solute_type" << d->solute_type
...
void
US_MPI_Analysis::parse_job (QXmlStreamReader &xml)
{
                  ...
                  if (xml.isStartElement ())
                    {
                      ...
                      if (name == "bucket")
                        { // Get bucket coords; try to forestall round-off
                          // problems
                          ...
                          if (stxmx.isEmpty ())
                            {
                              xmin = attr.value ("s_min")
                                         .toString ()
                                         .toDouble ();
                              xmax = attr.value ("s_max")
                                         .toString ()
                                         .toDouble ();
                              if (!stfmx.isEmpty ())
                                {
                          ...
                          if (my_rank == 0)
                            DbgLv (0) << "PF: xymnmx" << xmin << xmax << ymin
                                      << ymax;
                          ...
                          if (my_rank == 0)
                            DbgLv (0) << "PF:  xp yp xi yi" << xpwr << ypwr
                                      << xinc << yinc;
                          ...
                          if (my_rank == 0)
                            DbgLv (0) << "PF:   rnd xymnmx" << b.x_min
                                      << b.x_max << b.y_min << b.y_max;
                      ...
                      else if (name == "CG_model" || name == "DC_model")
                        {
                          ...
                          if (my_rank == 0)
                            DbgLv (0) << "PF:   DC_model" << parameters[name]
                                      << name;
                        }
                      else if (name == "bucket_fixed")
                        {
                          ...
                          parameters["xtype"]
                              = attr.value ("xtype").toString ();
                          parameters["ytype"]
                              = attr.value ("ytype").toString ();
                          parameters["ztype"]
                              = attr.value ("fixedtype").toString ();
...
void
US_MPI_Analysis::parse_dataset (QXmlStreamReader &xml, DATASET *dataset)
{
  ...
  while (!xml.atEnd ())
    {
      ...
      if (xml.name () == "rotor_stretch")
        {
          QStringList stretch = a.value ("value").toString ().split (
...
void
US_MPI_Analysis::parse_files (QXmlStreamReader &xml, DATASET *dataset)
{
  while (!xml.atEnd ())
    {
      xml.readNext ();
...
void
US_MPI_Analysis::parse_solution (QXmlStreamReader &xml, DATASET *dataset)
{
  while (!xml.atEnd ())
    {
      xml.readNext ();
