...
void
US_MPI_Analysis::_2dsa_worker (void)
{
        ...
        case MPI_Job::PROCESS: // Process solutes
          {
            ...
            MPI_Recv (simulation_values.solutes.data (), // Get solutes
                      job_length * solute_doubles, MPI_DOUBLE, MPI_Job::MASTER,
            ...
            // if(my_rank==1)
            {
              ...
              DbgLv (1) << "w:" << my_rank << ": offs dscnt" << offset
                        << dataset_count << "vbar s20wc cpbott dabott"
              ...
              DbgLv (1) << "w:" << my_rank << ": sol0 solm soln"
                        ...
                        << simulation_values.solutes[mm].s
                        << simulation_values.solutes[mm].k
                        << simulation_values.solutes[nn].s
                        << simulation_values.solutes[nn].k;
            }
            //*DEBUG*
            ...
            //*DEBUG*
            MPI_Send (size, 4, MPI_INT, MPI_Job::MASTER, MPI_Job::RESULTS,
                      my_communicator);
            ...
            MPI_Send (simulation_values.ti_noise.data (),
                      simulation_values.ti_noise.size (), MPI_DOUBLE,
                      MPI_Job::MASTER, MPI_Job::TAG0, my_communicator);

            MPI_Send (simulation_values.ri_noise.data (),
                      simulation_values.ri_noise.size (), MPI_DOUBLE,
                      MPI_Job::MASTER, MPI_Job::TAG0, my_communicator);
          }
                        ...
                        if ((my_rank == 1 || my_rank == 11)
                            && (index < 5 || index > (job_length - 6)
                                || (index > (indxh - 4)
                                    && index < (indxh + 3))))
                          DbgLv (1)
                              << "newD:" << my_rank << ":index" << index
                              << "edat" << edata->value (ss, rr) << "ee" << ee;
                      }
                  }
              }
            if (my_rank == 1 || my_rank == 11)
