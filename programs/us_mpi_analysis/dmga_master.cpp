...
void
US_MPI_Analysis::dmga_master (void)
{
  current_dataset = 0;
  ...
  for (int mcompx = 0; mcompx < wmodel.components.size (); mcompx++)
    {
      ...
      // DbgLv(0) << "mcompx ncompc c0atype" << mcompx << ncompc <<
      // cnsv[0].atype
      ...
      for (int cx = 0; cx < ncompc; cx++)
        {
          ...
          attrtype = tr ("UNKNOWN");
          // DbgLv(0) << "  cx" << cx << "fl ls mn mx" << floats << logscl <<
          // vmin << vmax;
          ...
          if (floats)
            {
              ...
              if (logscl)
                {
                  // DbgLv(0) << "       LOGSCL";
                  DbgLv (0) << "    " << attrtype << "floats from " << vmin
                            << "to" << vmax << "(log scale)";
                }
              else
                {
      ...
      if (ncomp == 2)
      ...
      else
        {
          DbgLv (0) << "    Reactants are components"
                    << wmodel.associations[assocx].rcomps[0] + 1 << " and"
                    << wmodel.associations[assocx].rcomps[1] + 1
                    << ", Product is component"
  ...
  while (true)
    {
      ...
      if (mc_iterations > 1)
        ...
        {
          DbgLv (1) << "GaMast: FFrmsd: variance"
  ...
  DbgLv (0) << "SDAT 0-3" << simulation_values.sim_data.value (0, hh)
  ...
  DbgLv (0) << "MDL asoc1 Kd,koff" << fmodel->associations[0].k_d
            << fmodel->associations[0].k_off;
  DbgLv (0) << "DS dens visc manual temp" << data_sets[0]->density
            << data_sets[0]->viscosity << data_sets[0]->manual
            << data_sets[0]->temperature;
  US_SimulationParameters *simpar = &data_sets[0]->simparams;
  DbgLv (0) << "SIMP simpt mType gType rreso menis" << simpar->simpoints
            << simpar->meshType << simpar->gridType
            << simpar->radial_resolution << simpar->meniscus;
  DbgLv (0) << "SIMP bott temp rnoise tinoise rinoise" << simpar->bottom
            << simpar->temperature << simpar->rnoise << simpar->tinoise
            << simpar->rinoise;
  DbgLv (0) << "SIMP bform bvol bottpos rcoeffs" << simpar->band_forming
            << simpar->band_volume << simpar->bottom_position
            << simpar->rotorcoeffs[0] << simpar->rotorcoeffs[1];
  US_SimulationParameters::SpeedProfile *spstep = &simpar->speed_step[0];
  DbgLv (0) << "STEP0 durmin dlymin w2tf w2tl timf timl"
            << spstep->duration_minutes << spstep->delay_minutes
            << spstep->w2t_first << spstep->w2t_last << spstep->time_first
            << spstep->time_last;
  DbgLv (0) << "STEP0 speed accel accelf" << spstep->rotorspeed
            << spstep->acceleration << spstep->acceleration_flag;
  //*DEBUG
  ...
  for (int mcompx = 0; mcompx < fmodel->components.size (); mcompx++)
    {
      ...
      // DbgLv(0) << "   at[n-2] at[n-1]" << cnsv[ncompc-2].atype <<
      // cnsv[ncompc-1].atype;
      ...
      for (int cx = 0; cx < ncompc; cx++)
        {
          ...
          double aval
              = constraints.fetch_attrib (fmodel->components[mcompx], atype);
  ...
  for (int assocx = 0; assocx < fmodel->associations.size (); assocx++)
    {
      ...
      if (ncomp == 2)
        ...
        {
          DbgLv (0) << "    Reactants are components"
                    << fmodel->associations[assocx].rcomps[0] + 1 << " and"
                    << fmodel->associations[assocx].rcomps[1] + 1
                    << ", Product is component"
      ...
      for (int rx = 0; rx < nassoc; rx++)
        {
          ...
          double aval
              = constraints.fetch_attrib (fmodel->associations[assocx], atype);
          DbgLv (0) << "    " << attrtype << "has a value of" << aval;
        }
    }
...
void
US_MPI_Analysis::dmga_master_loop (void)
{
  ...
  DbgLv (0) << "dmga_master start loop:  nfloatc max_same_count" << nfloatc
            << max_same_count << "ks_base ks_step" << ks_base << ks_step;
      ...
      switch (status.MPI_TAG)
        ...
        case GENERATION:
          ...
          if (avg > avg_generation)
            {
              avg_generation = avg;
              int mc_iter
                  = mgroup_count < 2 ? (mc_iteration + 1) : mc_iteration;
          ...
          MPI_Recv (dgmarker.data (), // MPI #2
                    nfloatc, MPI_DOUBLE, worker, GENE, my_communicator,
                    MPI_STATUS_IGNORE);

          DbgLv (1) << "  MAST: work" << worker << " dgm size"
                    << dgmarker.size () << "bestdg size"
                    << best_dgenes.size ();
          ...
          fitness_round
              = (double)qRound64 (msg.fitness * fit_digit) / fit_digit;

          DbgLv (1) << "  MAST: work" << worker << "fit msg,round,bestw,besto"
                    << msg.fitness << fitness_round
          ...
          DbgLv (1) << "master: worker/fitness/best gene" << worker
                    << msg.fitness << dgene_key (best_dgenes[worker]);
          ...
          if (!early_termination)
            { // Handle normal pre-early-termination updates
              ...
              DbgLv (1) << "  MAST: work" << worker << "fit besto,round"
                        << best_overall_fitness << fitness_round
                        << "fit_power fit_digit msgfit" << fit_power
                        << fit_digit << msg.fitness;
          ...
          if ((worker % 10) == 1)
            DbgLv (1) << worker << ": best_overall_fitness"
                      << best_overall_fitness << "fitness_same_count"
                      << fitness_same_count << " early_term?"
                      << early_termination;

          // Tell the worker to either stop or continue
          tag = early_termination ? FINISHED : GENERATION;
          DbgLv (1) << "dgmast: Send#3 tag" << tag << "( FINISHED,GENERATION"
                    << FINISHED << GENERATION << " )";
        ...
        case FINISHED:
          DbgLv (1) << "dgmast: Recv FINISH msg.size rsstotal" << msg.size
                    << rsstotal;
        ...
        case EMMIGRATE:
          {
            ...
            DbgLv (1) << "dgmast: Recv#4 EMMIG: gene_count doubles_count"
                      << gene_count << doubles_count;

            MPI_Recv (emmigrants.data (), // MPI #4
                      doubles_count, MPI_DOUBLE, worker, EMMIGRATE,
                      my_communicator, MPI_STATUS_IGNORE);
            ...
            DbgLv (1) << "dgmast: Recv#4 EMMIG: emigres size"
                      << emigres.size ();
            ...
            if (doubles_count > 0)
              ...
              {
                DbgLv (1) << "dgmast: Send#5 IMMIG: count==0 migr data"
                          << immigrants.data ();
                immigrants.resize (1);
              }
            DbgLv (1) << "dgmast: Send#5 IMMIG: immigrants size"
                      << immigrants.size () << "doubles_count" << doubles_count
                      << "migr data" << immigrants.data ();

            MPI_Send (immigrants.data (), // MPI #5
                      doubles_count, MPI_DOUBLE, worker, IMMIGRATE,
                      my_communicator);
            DbgLv (1) << "dgmast:   Send#5 IMMIG: complete";
            //*DEBUG*
            // if(immigrants[0].s<0.0)
            // DbgLv(0) << "MAST: **GENE s" << immigrants[0].s << "
            // Immigrant-to-send";
            //*DEBUG*
            break;
          }
  ...
  if (early_termination)
    { // Report when we have reached early termination of generations
      ...
      DbgLv (0) << "Early termination at average generation" << avg << ", MC"
                << mc_iter;
    }
}

void
US_MPI_Analysis::dmga_global_fit (void)
{
  ...
  MPI_Bcast (scaled_data.data (), // MPI #8
             scaled_data.size (), MPI_DOUBLE, MPI_Job::MASTER,
             my_communicator);
...
void
US_MPI_Analysis::set_dmga_gaussians (void)
{
  sigmas.clear ();
...
void
US_MPI_Analysis::set_dmga_MonteCarlo (void)
{
  ...
  DbgLv (1) << "sdMC: mc_data set index" << index << "total_points"
            << total_points;
...
void
US_MPI_Analysis::marker_from_dgene (QVector<double> &dgm, DGene &dg)
{
...
void
US_MPI_Analysis::dgene_from_marker (QVector<double> &dgm, DGene &dg)
{
...
void
US_MPI_Analysis::dgenes_to_marker (QVector<double> &dgm, QList<DGene> &dgenes,
                                   const int stgx, const int ngenes)
{
...
void
US_MPI_Analysis::marker_to_dgenes (QVector<double> &dgm, QList<DGene> &dgenes,
                                   const int stgx, const int ngenes)
{
  int mx = 0;                  // Initial marker index
  int gx = stgx;               // Initial genes index
...
bool
US_MPI_Analysis::store_attr_value (double &aval, US_Model &model,
                                   US_dmGA_Constraints::AttribType &atype,
                                   int &mcompx)
{
  bool is_ok = true;
...
bool
US_MPI_Analysis::fetch_attr_value (double &aval, US_Model &model,
                                   US_dmGA_Constraints::AttribType &atype,
                                   int &mcompx)
{
  bool is_ok = true;
...
void
US_MPI_Analysis::model_from_dgene (US_Model &model, DGene &dgene)
{
  ...
  if (group_rank < 1)
    {
      DbgLv (2) << my_rank << "MFG:g-comp1 s,k,w,d,f" << dgene.components[0].s
                << dgene.components[0].f_f0 << dgene.components[0].mw
                << dgene.components[0].D << dgene.components[0].f;
      DbgLv (2) << my_rank << "MFG:m-comp1 s,k,w,d,f" << model.components[0].s
                << model.components[0].f_f0 << model.components[0].mw
                << model.components[0].D << model.components[0].f;
    }
  //*DEBUG*
  ...
  for (int ii = 0; ii < model.associations.size (); ii++)
    { // Modify some coefficients based on associations
      ...
      if (group_rank < 2)
        DbgLv (1) << my_rank << "MFG: ii" << ii << "nrco,rc1,rc2,rcp" << nrco
                  << rc1 << rc2 << rcp << "st1,st2,stp" << st1 << st2 << stp;
