...
void US_MPI_Analysis::dmga_master(void) {
  ...
  for (int mcompx = 0; mcompx < wmodel.components.size(); mcompx++) {
    ...
    for (int cx = 0; cx < ncompc; cx++) {
      ...
      attrtype = tr("UNKNOWN");
      // DbgLv(0) << "  cx" << cx << "fl ls mn mx" << floats << logscl << vmin
      // << vmax;
      ...
      if (floats) {
        ...
        if (logscl) {
          // DbgLv(0) << "       LOGSCL";
          DbgLv(0) << "    " << attrtype << "floats from " << vmin << "to"
                   << vmax << "(log scale)";
        } else {
          // DbgLv(0) << "       !LOGSCL";
          DbgLv(0) << "    " << attrtype << "floats from " << vmin << "to"
  ...
  while (true) {
    ...
    if (minimize_opt ==
        2) {  // If gradient search method for all terminations, minimize now
      double aval;
      in_gsm = true;
  ...
  DbgLv(0) << "SIMP simpt mType gType rreso menis" << simpar->simpoints
           << simpar->meshType << simpar->gridType << simpar->radial_resolution
           << simpar->meniscus;
...
void US_MPI_Analysis::dmga_master_loop(void) {
  ...
  while (workers > 0) {
    ...
    switch (status.MPI_TAG) {
      case GENERATION:
        ...
        sum = 0;
        for (int i = 1; i <= my_workers; i++) sum += v_generations[i];
        ...
        if (avg > avg_generation) {
          ...
          if (best_overall_fitness != LARGE && best_overall_fitness > 0.0)
            progress +=
                "; RMSD: " + QString::number(sqrt(best_overall_fitness));
        ...
        DbgLv(1) << "master: worker/fitness/best gene" << worker << msg.fitness
                 << dgene_key(best_dgenes[worker]);

        if (!early_termination) {  // Handle normal pre-early-termination
                                   // updates
          if (avg_generation == 1 && mc_iterations == 1 &&
              best_overall_fitness == LARGE) {  // Report first best-fit RMSD
            DbgLv(0) << "First Best Fit RMSD" << sqrt(fitness_round);
          }
          DbgLv(1) << "  MAST: work" << worker << "fit besto,round"
                   << best_overall_fitness << fitness_round
                   << "fit_power fit_digit msgfit" << fit_power << fit_digit
                   << msg.fitness;

          if (fitness_round <
              best_overall_fitness) {  // Update over-all best fitness value
                                       // (rounded)
            best_overall_fitness = fitness_round;
          ...
          if (fitness_same_count > max_same_count &&
              avg_generation >
                  min_generation) {  // Mark early termination at threshold
                                     // same-fitness count
      ...
      case EMMIGRATE: {
        ...
        MPI_Recv(emmigrants.data(),  // MPI #4
                 doubles_count, MPI_DOUBLE, worker, EMMIGRATE, my_communicator,
                 MPI_STATUS_IGNORE);
        ...
        DbgLv(1) << "dgmast: Send#5 IMMIG: immigrants size" << immigrants.size()
                 << "doubles_count" << doubles_count << "migr data"
                 << immigrants.data();
  ...
  if (early_termination) {  // Report when we have reached early termination of
                            // generations
    int mc_iter = mgroup_count < 2 ? (mc_iteration + 1) : mc_iteration;
...
void US_MPI_Analysis::dgenes_to_marker(QVector<double>& dgm,
                                       QList<DGene>& dgenes, const int stgx,
                                       const int ngenes) {
  int mx = 0;             // Initial marker index
  int gx = stgx;          // Initial genes index
  DGene wkgene = wmodel;  // Base gene

  for (int kg = 0; kg < ngenes;
       kg++) {  // Fetch each gene and store its float attribute values
    wkgene = dgenes[gx++];

    for (int ii = 0; ii < nfloatc; ii++) {  // Fetch each gene float attribute
                                            // value into the marker vector
...
void US_MPI_Analysis::marker_to_dgenes(QVector<double>& dgm,
  ...
  for (int kg = 0; kg < ngenes; kg++) {  // Build and store each gene
    ...
    for (int ii = 0; ii < nfloatc;
         ii++, gx++) {  // Store each float attribute value into the gene
...
void US_MPI_Analysis::model_from_dgene(US_Model& model, DGene& dgene) {
  ...
  for (int ii = 0; ii < model.associations.size();
       ii++) {  // Modify some coefficients based on associations
