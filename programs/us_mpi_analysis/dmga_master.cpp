...
void US_MPI_Analysis::dmga_master(void) {
   ...
   DbgLv(1) << "dmga_master: wmodel #comps" << wmodel.components.size() << "#assoc" << wmodel.associations.size();
   ...
   for (int mcompx = 0; mcompx < wmodel.components.size(); mcompx++) {
      ...
      for (int cx = 0; cx < ncompc; cx++) {
         ...
         if (floats) {
            ...
            if (logscl) {
               //DbgLv(0) << "       LOGSCL";
               DbgLv(0) << "    " << attrtype << "floats from " << vmin << "to" << vmax << "(log scale)";
            }
            else {
               //DbgLv(0) << "       !LOGSCL";
               DbgLv(0) << "    " << attrtype << "floats from " << vmin << "to" << vmax;
            }
         }
         else {
            //DbgLv(0) << "      !FLOATS";
            DbgLv(0) << "    " << attrtype << "is fixed at " << vmin;
         }
      }
   }
   ...
   for (int assocx = 0; assocx < wmodel.associations.size(); assocx++) {
      ...
      if (ncomp == 2) {
         DbgLv(0) << "    Reactant is component" << wmodel.associations[ assocx ].rcomps[ 0 ] + 1
                  << ", Product is component" << wmodel.associations[ assocx ].rcomps[ 1 ] + 1;
      }

      else {
         DbgLv(0) << "    Reactants are components" << wmodel.associations[ assocx ].rcomps[ 0 ] + 1 << " and"
                  << wmodel.associations[ assocx ].rcomps[ 1 ] + 1 << ", Product is component"
   ...
   DbgLv(0) << " wmodel as1 K_d k_off" << wmodel.associations[ 0 ].k_d << wmodel.associations[ 0 ].k_off;
   ...
   while (true) {
      ...
      qSort(best_fitness);
      DbgLv(1) << "GaMast: EOML: fitness sorted  bfx" << best_fitness[ 0 ].index << "bestdg size" << best_dgenes.size();

      Fitness *bfit = &best_fitness[ 0 ];
      DbgLv(0) << "Last generation best RMSD" << sqrt(bfit->fitness);

      if (minimize_opt == 2) { // If gradient search method for all terminations, minimize now
         double aval;
         in_gsm = true;
         ...
         for (int ii = 0; ii < nfloatc; ii++) { // Insure all the new gene attribute values are inside range
      ...
      mc_iteration++;
      if (mc_iterations > 1) {
         qDebug() << "Fit RMSD" << sqrt(simulation_values.variance) << " of MC_Iteration" << mc_iteration;
   ...
   DbgLv(0) << "SDAT 0-3" << simulation_values.sim_data.value(0, hh) << simulation_values.sim_data.value(1, hh)
            << simulation_values.sim_data.value(2, hh) << simulation_values.sim_data.value(3, hh);
   DbgLv(0) << "SDAT *-n" << simulation_values.sim_data.value(ss - 4, hh)
            << simulation_values.sim_data.value(ss - 3, hh) << simulation_values.sim_data.value(ss - 2, hh)
            << simulation_values.sim_data.value(ss - 1, hh);
   DbgLv(0) << "MDL comp1 s,k,w,D,f" << fmodel->components[ 0 ].s / data_sets[ 0 ]->s20w_correction
            << fmodel->components[ 0 ].f_f0 << fmodel->components[ 0 ].mw
            << fmodel->components[ 0 ].D / data_sets[ 0 ]->D20w_correction << fmodel->components[ 0 ].f;
   DbgLv(0) << "MDL comp2 s,k,w,D,f" << fmodel->components[ 1 ].s / data_sets[ 0 ]->s20w_correction
            << fmodel->components[ 1 ].f_f0 << fmodel->components[ 1 ].mw
            << fmodel->components[ 1 ].D / data_sets[ 0 ]->D20w_correction << fmodel->components[ 1 ].f;
   DbgLv(0) << "MDL asoc1 Kd,koff" << fmodel->associations[ 0 ].k_d << fmodel->associations[ 0 ].k_off;
   DbgLv(0) << "DS dens visc manual temp" << data_sets[ 0 ]->density << data_sets[ 0 ]->viscosity
            << data_sets[ 0 ]->manual << data_sets[ 0 ]->temperature;
   US_SimulationParameters *simpar = &data_sets[ 0 ]->simparams;
   DbgLv(0) << "SIMP simpt mType gType rreso menis" << simpar->simpoints << simpar->meshType << simpar->gridType
            << simpar->radial_resolution << simpar->meniscus;
   DbgLv(0) << "SIMP bott temp rnoise tinoise rinoise" << simpar->bottom << simpar->temperature << simpar->rnoise
            << simpar->tinoise << simpar->rinoise;
   DbgLv(0) << "SIMP bform bvol bottpos rcoeffs" << simpar->band_forming << simpar->band_volume
            << simpar->bottom_position << simpar->rotorcoeffs[ 0 ] << simpar->rotorcoeffs[ 1 ];
   US_SimulationParameters::SpeedProfile *spstep = &simpar->speed_step[ 0 ];
   DbgLv(0) << "STEP0 durmin dlymin w2tf w2tl timf timl" << spstep->duration_minutes << spstep->delay_minutes
            << spstep->w2t_first << spstep->w2t_last << spstep->time_first << spstep->time_last;
   DbgLv(0) << "STEP0 speed accel accelf" << spstep->rotorspeed << spstep->acceleration << spstep->acceleration_flag;
   //*DEBUG
   ...
   for (int mcompx = 0; mcompx < fmodel->components.size(); mcompx++) {
      ...
      for (int cx = 0; cx < ncompc; cx++) {
         ...
         double aval = constraints.fetch_attrib(fmodel->components[ mcompx ], atype);
         DbgLv(0) << "    " << attrtype << "has a value of" << aval;
      }
   }
   ...
   for (int assocx = 0; assocx < fmodel->associations.size(); assocx++) {
      ...
      if (ncomp == 2) {
         DbgLv(0) << "    Reactant is component" << fmodel->associations[ assocx ].rcomps[ 0 ] + 1
                  << ", Product is component" << fmodel->associations[ assocx ].rcomps[ 1 ] + 1;
      }

      else {
         DbgLv(0) << "    Reactants are components" << fmodel->associations[ assocx ].rcomps[ 0 ] + 1 << " and"
                  << fmodel->associations[ assocx ].rcomps[ 1 ] + 1 << ", Product is component"
      ...
      for (int rx = 0; rx < nassoc; rx++) {
         ...
         double aval = constraints.fetch_attrib(fmodel->associations[ assocx ], atype);
         DbgLv(0) << "    " << attrtype << "has a value of" << aval;
      }
   }
...
void US_MPI_Analysis::dmga_master_loop(void) {
   ...
   int max_same_count = (ks_base + (nfloatc / ks_step) * ks_step) * my_workers;
   ...
   DbgLv(1) << "dmga_master start loop:  gcores_count fitsize" << gcores_count << best_fitness.size() << "best_overall"
            << best_overall_fitness;
   DbgLv(0) << "dmga_master start loop:  nfloatc max_same_count" << nfloatc << max_same_count << "ks_base ks_step"
            << ks_base << ks_step;
   ...
   while (workers > 0) {
      ...
      MPI_Recv(
         &msg, // Get a message   MPI #1
         sizeof(msg), MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG, my_communicator, &status);
      ...
      switch (status.MPI_TAG) {
         case GENERATION:
            ...
            if (avg > avg_generation) {
               ...
               QString progress = "Avg. Generation: " + QString::number(avg_generation);

               if (data_sets.size() > 1) {
                  if (datasets_to_process == 1)
                     progress += "; Dataset: " + QString::number(current_dataset + 1) + " of "
               ...
               if (best_overall_fitness != LARGE && best_overall_fitness > 0.0)
                  progress += "; RMSD: " + QString::number(sqrt(best_overall_fitness));
            ...
            MPI_Recv(
               dgmarker.data(), // MPI #2
               nfloatc, MPI_DOUBLE, worker, GENE, my_communicator, MPI_STATUS_IGNORE);

            DbgLv(1) << "  MAST: work" << worker << " dgm size" << dgmarker.size() << "bestdg size"
                     << best_dgenes.size();
            ...
            fitness_round = ( double ) qRound64(msg.fitness * fit_digit) / fit_digit;

            DbgLv(1) << "  MAST: work" << worker << "fit msg,round,bestw,besto" << msg.fitness << fitness_round
                     << best_fitness[ worker ].fitness << best_overall_fitness;
            ...
            if (!early_termination) { // Handle normal pre-early-termination updates
               if (avg_generation == 1 && mc_iterations == 1 && best_overall_fitness == LARGE) { // Report first best-fit RMSD
               ...
               if (fitness_round < best_overall_fitness) { // Update over-all best fitness value (rounded)
                  best_overall_fitness = fitness_round;
                  fitness_same_count = 0;
               }
               else { // Bump the count of consecutive same best overall fitness
                  fitness_same_count++;
               }


               if (
                  fitness_same_count > max_same_count
                  && avg_generation > min_generation) { // Mark early termination at threshold same-fitness count
            ...
            if ((worker % 10) == 1)
               DbgLv(1) << worker << ": best_overall_fitness" << best_overall_fitness << "fitness_same_count"
            ...
            MPI_Send(
               ...
               MPI_BYTE, worker, tag, my_communicator);
            break;
         ...
         case EMMIGRATE: {
            ...
            MPI_Recv(
               emmigrants.data(), // MPI #4
               doubles_count, MPI_DOUBLE, worker, EMMIGRATE, my_communicator, MPI_STATUS_IGNORE);
            ...
            MPI_Send(
               immigrants.data(), // MPI #5
               doubles_count, MPI_DOUBLE, worker, IMMIGRATE, my_communicator);
   ...
   if (early_termination) { // Report when we have reached early termination of generations
      int mc_iter = mgroup_count < 2 ? (mc_iteration + 1) : mc_iteration;
      DbgLv(0) << "Early termination at average generation" << avg << ", MC" << mc_iter;
   }
}
...
void US_MPI_Analysis::dmga_global_fit(void) {
   ...
   for (int worker = 1; worker <= my_workers; worker++) {
      MPI_Send(
         &job, // MPI #7
         sizeof(MPI_Job), MPI_BYTE, worker, UPDATE, my_communicator);
   }
   ...
   MPI_Bcast(
      scaled_data.data(), // MPI #8
      scaled_data.size(), MPI_DOUBLE, MPI_Job::MASTER, my_communicator);
...
void US_MPI_Analysis::set_dmga_MonteCarlo(void) {
   ...
   DbgLv(1) << "sdMC: mc_data set index" << index << "total_points" << total_points;
   ...
   for (int worker = 1; worker <= my_workers; worker++) {
      MPI_Send(
         &job, // MPI #9
         sizeof(job), MPI_BYTE, worker, UPDATE, my_communicator);
   }
   ...
   MPI_Bcast(
      mc_data.data(), // MPI #10
      total_points, MPI_DOUBLE, MPI_Job::MASTER, my_communicator);
}
...
void US_MPI_Analysis::marker_from_dgene(QVector<double> &dgm, DGene &dg) {
   ...
   for (int ii = 0; ii < nfloatc; ii++) {
      fetch_attr_value(dgm[ ii ], dg, cns_flt[ ii ].atype, cns_flt[ ii ].mcompx);
   }
}
...
void US_MPI_Analysis::dgene_from_marker(QVector<double> &dgm, DGene &dg) {
   ...
   for (int ii = 0; ii < nfloatc; ii++) {
      store_attr_value(dgm[ ii ], dg, cns_flt[ ii ].atype, cns_flt[ ii ].mcompx);
   }
}

// Get a marker vector of doubles from multiple dmga genes (models)
void US_MPI_Analysis::dgenes_to_marker(QVector<double> &dgm, QList<DGene> &dgenes, const int stgx, const int ngenes) {
   int mx = 0; // Initial marker index
   int gx = stgx; // Initial genes index
   DGene wkgene = wmodel; // Base gene

   for (int kg = 0; kg < ngenes; kg++) { // Fetch each gene and store its float attribute values
      wkgene = dgenes[ gx++ ];

      for (int ii = 0; ii < nfloatc; ii++) { // Fetch each gene float attribute value into the marker vector
         fetch_attr_value(dgm[ mx++ ], wkgene, cns_flt[ ii ].atype, cns_flt[ ii ].mcompx);
      }
   }
}

// Get multiple dmga genes (models) from a marker vector of doubles
void US_MPI_Analysis::marker_to_dgenes(QVector<double> &dgm, QList<DGene> &dgenes, const int stgx, const int ngenes) {
   int mx = 0; // Initial marker index
   int gx = stgx; // Initial genes index
   int igsize = dgenes.size(); // Initial genes list size
   DGene wkgene = wmodel; // Base gene

   for (int kg = 0; kg < ngenes; kg++) { // Build and store each gene

      for (int ii = 0; ii < nfloatc; ii++, gx++) { // Store each float attribute value into the gene
         store_attr_value(dgm[ mx++ ], wkgene, cns_flt[ ii ].atype, cns_flt[ ii ].mcompx);
      }
...
bool US_MPI_Analysis::store_attr_value(
   double &aval, US_Model &model, US_dmGA_Constraints::AttribType &atype, int &mcompx) {
   bool is_ok = true;
...
bool US_MPI_Analysis::fetch_attr_value(
   double &aval, US_Model &model, US_dmGA_Constraints::AttribType &atype, int &mcompx) {
   bool is_ok = true;
   US_Model::SimulationComponent *sc = NULL;
   US_Model::Association *as = NULL;
...
void US_MPI_Analysis::model_from_dgene(US_Model &model, DGene &dgene) {
   ...
   //*DEBUG*
   if (group_rank < 1) {
      DbgLv(2) << my_rank << "MFG:g-comp1 s,k,w,d,f" << dgene.components[ 0 ].s << dgene.components[ 0 ].f_f0
               << dgene.components[ 0 ].mw << dgene.components[ 0 ].D << dgene.components[ 0 ].f;
      DbgLv(2) << my_rank << "MFG:m-comp1 s,k,w,d,f" << model.components[ 0 ].s << model.components[ 0 ].f_f0
               << model.components[ 0 ].mw << model.components[ 0 ].D << model.components[ 0 ].f;
   }
   //*DEBUG*

   for (int ii = 0; ii < model.associations.size(); ii++) { // Modify some coefficients based on associations
      ...
      if (group_rank < 2)
         DbgLv(1) << my_rank << "MFG: ii" << ii << "nrco,rc1,rc2,rcp" << nrco << rc1 << rc2 << rcp << "st1,st2,stp"
                  << st1 << st2 << stp;
      ...
      if (group_rank < 2)
         DbgLv(1) << my_rank << "MFG:   orig cval wval vval" << cval << model.components[ rcp ].mw
                  << model.components[ rcp ].vbar20;
      ...
      if (group_rank < 2)
         DbgLv(1) << my_rank << "MFG:     rcp" << rcp << "c.s c.k c.mw c.vb" << model.components[ rcp ].s
                  << model.components[ rcp ].f_f0 << model.components[ rcp ].mw << model.components[ rcp ].vbar20;
   }
}
