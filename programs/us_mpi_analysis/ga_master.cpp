...
void
US_MPI_Analysis::ga_master (void)
{
  current_dataset = 0;
  ...
  while (true)
    {
      ...
      DbgLv (1) << "GaMast: sols size" << nisols << "buck size"
                << buckets.size () << "dset size" << data_sets.size ();
      DbgLv (1) << "GaMast:  sol0.s .k .v .d" << simulation_values.solutes[0].s
                << simulation_values.solutes[0].k
                << simulation_values.solutes[0].v
                << simulation_values.solutes[0].d;

      solutes_from_gene (simulation_values.solutes, nisols);
      DbgLv (1) << "GaMast:   osol0.s .k .v .d"
                << simulation_values.solutes[0].s
                << simulation_values.solutes[0].k
                << simulation_values.solutes[0].v
                << simulation_values.solutes[0].d;
...
void
US_MPI_Analysis::ga_master_loop (void)
{
      ...
      switch (status.MPI_TAG)
        ...
        case GENERATION:
          ...
          MPI_Recv (best_genes[worker].data (), // MPI #2
                    buckets.size () * solute_doubles, MPI_DOUBLE, worker, GENE,
          ...
          for (int i = 0; i < buckets.size (); i++)
            g += s.sprintf ("(%.3f,%.3f)", best_genes[worker][i].s,
                            best_genes[worker][i].k);
        ...
        case EMMIGRATE:
          {
            ...
            if (doubles_count > 0)
              {
                // Prepare a vector of concatenated genes from the emmigrant
                // list
                for (int i = 0; i < gene_count; i++)
...
void
US_MPI_Analysis::ga_global_fit (void)
{
...
void
US_MPI_Analysis::set_gaMonteCarlo (void)
{
