...
void
US_MPI_Analysis::ga_worker (void)
{
  current_dataset = 0;
  ...
  while (!finished)
    {
      ...
      MPI_Recv (&job,         // Find out what to do next
                ...
                MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG, my_communicator,
                &status);
      ...
      switch (status.MPI_TAG)
        ...
        case UPDATE:
          ...
          DbgLv (1) << "Deme" << deme_nbr << "UPD ds cnt len" << dataset
                    << count << length;
          ...
          for (int ee = dataset; ee < dataset + count; ee++)
            {
              ...
              DbgLv (1) << "Deme" << deme_nbr << "  ee" << ee << "scnt rcnt"
                        << scan_count << radius_points;
              double dsumi = 0.0;
              double dsumo = 0.0;
              ...
              DbgLv (1) << "Deme" << deme_nbr << "  dsumi" << dsumi << "dsumo"
                        << dsumo;
            }
...
void
US_MPI_Analysis::ga_worker_loop (void)
{
  // Initialize genes
  genes.clear ();
  ...
  for (generation = 0; generation < generations; generation++)
    {
      ...
      if (generation == generations - 1)
        {
          ...
          fitness[0].fitness
              = minimize (genes[fitness[0].index], fitness[0].fitness);
          DbgLv (DL) << "Deme" << grp_nbr << deme_nbr
      ...
      MPI_Send (genes[fitness[0].index].data (), // to MPI #2
                solute_doubles * buckets.size (), MPI_DOUBLE, MPI_Job::MASTER,
                GENE, my_communicator);
          ...
          if (fitdiff < epsilon_f)
            {
              ...
              if (match)
                {
                  DbgLv (1) << "gw:" << my_rank
                            << ":  Dup cleaned: f0 f1 fit0 fit1" << f0 << f1
                            << fitness[f0].fitness << fitness[f1].fitness;
...
void
US_MPI_Analysis::align_gene (Gene &gene)
{
  ...
  for (int i = 0; i < gene.size (); i++)
    {
      ...
      // int grp_nbr  = ( my_rank / gcores_count );
      // int deme_nbr = my_rank - grp_nbr * gcores_count;
      // DbgLv(0) << "Dem :" << grp_nbr << deme_nbr << "gene.s" << gene[i].s <<
      // "ALIGN"
...
double
US_MPI_Analysis::get_fitness (const Gene &gene)
{
  ...
  if (data_sets.size () == 1)
    { // Count solutes whose concentration is at least the threshold
      ...
      DbgLv (2) << "get_fitness: sol_count conc_thresh" << solute_count
  ...
  if (dbg_level > 0 && fitness_map.size () == 20)
    {
      ...
      DbgLv (1) << "w:" << my_rank << generation << ": fmapsize fitness nsols"
                ...
                << "s0 s,k,v" << sim.solutes[0].s << sim.solutes[0].k
                << sim.solutes[0].v << "sn s,k,v" << sim.solutes[n].s
                << sim.solutes[n].k << sim.solutes[n].v;
    }
  //*DEBUG*
  ...
  if (dbg_level > 1 && datasets_to_process > 1 && generation < 11)
    {
      DbgLv (0) << "rank generation" << my_rank << generation << "fitness"
                << fitness << "vari0 vari1" << sim.variances[0]
                << sim.variances[1];
    }
  //*DEBUG*
  return fitness;
...
double
US_MPI_Analysis::get_fitness_v (const US_Vector &vv)
{
  ...
  if (gene[0].s < 0.0)
    {
      int grp_nbr = (my_rank / gcores_count);
      int deme_nbr = my_rank - grp_nbr * gcores_count;
      DbgLv (DL) << "Dem :" << grp_nbr << deme_nbr << "gene.s" << gene[0].s
                 << "FIT_V";
    }
  //*DEBUG*
...
int
US_MPI_Analysis::u_random (int modulo)
{
...
void
US_MPI_Analysis::mutate_gene (Gene &gene)
{
  ...
  // int grp_nbr  = ( my_rank / gcores_count );
  // int deme_nbr = my_rank - grp_nbr * gcores_count;
  // DbgLv(0) << "Dem :" << grp_nbr << deme_nbr << "gene.s" << gene[0].s <<
  // "MUTATE";
  //}
  //*DEBUG*
}
...
void
US_MPI_Analysis::mutate_s (US_Solute &solute, int b)
{
  ...
  // int grp_nbr  = ( my_rank / gcores_count );
  // int deme_nbr = my_rank - grp_nbr * gcores_count;
  // DbgLv(0) << "Dem :" << grp_nbr << deme_nbr << "  solute.s" << solute.s <<
  // "MUTATE_S";
  //}
  //*DEBUG*
}
...
void
US_MPI_Analysis::mutate_k (US_Solute &solute, int b)
{
  // static const double mutate_sigma = parameters[ "mutate_sigma"
  // ].toDouble();
...
void
US_MPI_Analysis::cross_gene (Gene &gene, QList<Gene> old_genes)
{
...
int
US_MPI_Analysis::migrate_genes (void)
{
  ...
  // int grp_nbr  = ( my_rank / gcores_count );
  // int deme_nbr = my_rank - grp_nbr * gcores_count;
  // DbgLv(0) << "Dem :" << grp_nbr << deme_nbr << "gene.s" << emmigres[0].s <<
  // "EMMIGRES";
  //}
  //*DEBUG*
  ...
  MPI_Send (&msg, // to MPI #1
            sizeof (msg), MPI_BYTE, MPI_Job::MASTER, EMMIGRATE,
            my_communicator);

  // MPI send emmigrants
  MPI_Send (emmigres.data (), // to MPI #4
            migrate_sols * solute_doubles, MPI_DOUBLE, MPI_Job::MASTER,
            EMMIGRATE, my_communicator);
  ...
  MPI_Recv (immigres.data (), // from MPI #5
            migrate_sols * solute_doubles, MPI_DOUBLE, MPI_Job::MASTER,
            IMMIGRATE, my_communicator, &status);
  ...
  if (mgenes_count > 0)
    {
      int grp_nbr = (my_rank / gcores_count);
      int deme_nbr = my_rank - grp_nbr * gcores_count;
      DbgLv (1) << "MG:Deme" << deme_nbr << ": solsent mg_count"
                << solutes_sent << mgenes_count << " elit" << elitism_count
      ...
      // int grp_nbr  = ( my_rank / gcores_count );
      // int deme_nbr = my_rank - grp_nbr * gcores_count;
      // DbgLv(0) << "Dem :" << grp_nbr << deme_nbr << "gene.s" <<
      // genes[elitism_count][0].s << "IMMIGRANT";
      //}
      //*DEBUG*
    }
...
US_MPI_Analysis::Gene
US_MPI_Analysis::new_gene (void)
{
  Gene gene;
...
double
US_MPI_Analysis::minimize (Gene &gene, double fitness)
{
#if 0
#define TIMING_MZ
#endif
  ...
  DbgLv (DL) << Phd << "vsize" << vsize << "fitness" << fitness << "gene0.s"
             << gene[0].s;
  int index = 0;
  ...
  while (uu.L2norm () >= epsilon_f && iteration < max_iterations)
    {
      ...
      while (((s2 - s1) > epsilon) && ((s3 - s2) > epsilon)
             && (reps++ < max_reps))
        {
          ...
          double bb = (g_s3 * (s2_2 - s1_2) + g_s2 * (s1_2 - s3_2)
                       + g_s1 * (s3_2 - s2_2))
                      * s1_s2 * s1_s3 * s2_s3;
          ...
          if (qAbs (aa) < max_a)
            {
              ...
              totms += insms;
              ncalls++;
              DRTiming << "MINIMIZE: msecs" << insms << "totmsecs calls"
                       << totms << ncalls;
              DRTiming << "   MMIZE:  t1 t2 t3 t4 t5 t6" << totT1 << totT2
                       << totT3 << totT4 << totT5 << totT6;
#endif
              return fitness;
            }
  ...
  totms += insms;
  ncalls++;
  DRTiming << "MINIMIZE: msecs" << insms << "totmsecs calls" << totms
           << ncalls;
  DRTiming << "   MMIZE:  t1 t2 t3 t4 t5 t6" << totT1 << totT2 << totT3
           << totT4 << totT5 << totT6;
#endif
  return fitness;
}
...
void
US_MPI_Analysis::vector_scaled_sum (US_Vector &cc, US_Vector &aa, double sa,
...
double
US_MPI_Analysis::update_fitness (int index, US_Vector &v)
{
  double fitness = 0.0;
  ...
  // qDebug() << "UF: vsize nsols ntotal navals" << vsize << nsols << ntotal <<
  // navals;
          ...
          for (int rr = 0; rr < npoints; rr++)
            {
              ...
              //   << ss << rr << edata->value(ss,rr) << simdat.value(ss,rr) <<
              //   resid;
            }
        }
...
void
US_MPI_Analysis::lamm_gsm_df (const US_Vector &vv, US_Vector &vd)
{
  ...
  DbgLv (DL) << "GDF: ...vdn" << vd[nn - 3] << vd[nn - 2] << vd[nn - 1]
             << vd[nn];
}

/////////////   Debug routines
void
US_MPI_Analysis::dump_buckets (void)
{
  if (my_rank != 1)
    return;
  DbgLv (1) << "Buckets:";
  ...
  for (int b = 0; b < buckets.size (); b++)
    {
      DbgLv (1) << buckets[b].x_min << buckets[b].x_max << buckets[b].y_min
                << buckets[b].y_max << buckets[b].ds << buckets[b].dk;
    }
}

void
US_MPI_Analysis::dump_genes (int gene)
{
  if (my_rank != 1)
    return;
...
void
US_MPI_Analysis::dump_fitness (const QList<Fitness> &fitness)
{
  if (my_rank != 1)
    return;
  ...
  for (int f = 0; f < fitness.size (); f++)
    {
      ...
      s += QString ().sprintf ("i, index, fitness: %3i, %3i, %.6e\n", f,
                               fitness[f].index, fitness[f].fitness);
    }
...
void
US_MPI_Analysis::set_comp_attrib (US_Model::SimulationComponent &mcomp,
...
void
US_MPI_Analysis::build_component (US_Model::SimulationComponent &mcomp,
                                  US_Math2::SolutionData &sd, double xval,
                                  double yval)
{
  US_SolveSim::DataSet *dset = data_sets[0];
...
void
US_MPI_Analysis::solutes_from_gene (Gene &solutes, int nsols)
{
  double fixval = (attr_z == ATTR_V) ? data_sets[0]->vbar20
  ...
  for (int cc = 0; cc < nsols; cc++)
    {
      ...
      solutes[cc].d
          = (attr_z == ATTR_W || attr_z == ATTR_D || attr_z == ATTR_F) ? fixval
                                                                       : 0.0;
