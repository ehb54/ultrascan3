...
void US_MPI_Analysis::ga_worker(void) {
   ...
   while (!finished) {
      ...
      msg.size = max_rss();
      DbgLv(0) << "Deme" << grp_nbr << deme_nbr << ":   Generations finished, second" << ELAPSEDSEC;

      MPI_Send(
         &msg, // This iteration is finished
         sizeof(msg), // to MPI #1
         MPI_BYTE, MPI_Job::MASTER, FINISHED, my_communicator);

      MPI_Recv(
         &job, // Find out what to do next
      ...
      switch (status.MPI_TAG) {
         case FINISHED:
            finished = true;
            DbgLv(0) << "    Deme" << grp_nbr << deme_nbr << ":" << fitness_hits << "fitness hits of" << fitness_count
         ...
         case UPDATE:
            ...
            // This is a receive
            MPI_Bcast(
               mc_data.data(), // from MPI #8, #10
               length, MPI_DOUBLE, MPI_Job::MASTER, my_communicator);

            if (is_global_fit && count == 1) { // For global update to scaled data, extra is new ODLimit
               length--;
...
void US_MPI_Analysis::ga_worker_loop(void) {
   ...
   for (generation = 0; generation < generations; generation++) {
      ...
      if (generation == generations - 1) {
         DbgLv(DL) << "Deme" << grp_nbr << deme_nbr << ": At last generation minimize.";
         DbTimMsg("Worker before gsm rank/generation/elapsed");

         fitness[ 0 ].fitness = minimize(genes[ fitness[ 0 ].index ], fitness[ 0 ].fitness);
         DbgLv(DL) << "Deme" << grp_nbr << deme_nbr << ":   last generation minimize fitness=" << fitness[ 0 ].fitness;
      ...
      MPI_Send(
         &msg, // to MPI #1
         sizeof(msg), MPI_BYTE, MPI_Job::MASTER, GENERATION, my_communicator);

      MPI_Send(
         genes[ fitness[ 0 ].index ].data(), // to MPI #2
         solute_doubles * buckets.size(), MPI_DOUBLE, MPI_Job::MASTER, GENE, my_communicator);
      ...
      if (status.MPI_TAG == FINISHED) {
         DbgLv(0) << "Deme" << grp_nbr << deme_nbr << ": Finish signalled at deme generation" << generation + 1;
         break;
      }
      ...
      DbgLv(1) << "gw:" << my_rank << ": Dup best-gene clean: fitness0 fitpwr epsilon_f" << fitness[ 0 ].fitness
               << fitpwr << epsilon_f;
         ...
         if (fitdiff < epsilon_f) {
            ...
            for (int ii = 0; ii < buckets.size(); ii++) {
               ...
               if (sdif > NEAR_MATCH || kdif > NEAR_MATCH) {
                  DbgLv(1) << "gw:" << my_rank << ":  Dup NOT cleaned: f0 f1 fit0 fit1" << f0 << f1
                           << fitness[ f0 ].fitness << fitness[ f1 ].fitness << "ii g0 g1 g0s g1s" << ii << g0 << g1
                           << genes[ g0 ][ ii ].s << genes[ f1 ][ ii ].s;
                  match = false;
                  f0 = f1;
                  break;
               }
            ...
            if (match) {
               DbgLv(1) << "gw:" << my_rank << ":  Dup cleaned: f0 f1 fit0 fit1" << f0 << f1 << fitness[ f0 ].fitness
                        << fitness[ f1 ].fitness;
...
double US_MPI_Analysis::get_fitness(const Gene &gene) {
   ...
   for (int cc = 0; cc < nisols; cc++) { // Concatenate all solute s,k values to form fitness key
      key += str.sprintf("%.5f%.5f", sim.solutes[ cc ].s, sim.solutes[ cc ].k);
   }

   DbgLv(2) << "get_fitness: nisols" << nisols << "key" << key;
   if (fitness_map.contains(key)) { // We already have a match to this key, so use its fitness value
      fitness_hits++;
   ...
   if (data_sets.size() == 1) { // Count solutes whose concentration is at least the threshold
      for (int cc = 0; cc < nosols; cc++) {
         if (sim.solutes[ cc ].c >= concentration_threshold)
            solute_count++;
      }
      DbgLv(2) << "get_fitness: sol_count conc_thresh" << solute_count << concentration_threshold;
   }

   else { // For multiple datasets, use a weighted average fitness
      double concen = 0.0;
      fitness = 0.0;
   ...
   if (dbg_level > 0 && fitness_map.size() == 20) {
      int n = nosols - 1;
      DbgLv(1) << "w:" << my_rank << generation << ": fmapsize fitness nsols" << fitness_map.size() << fitness << nisols
               << nosols << "s0 s,k,v" << sim.solutes[ 0 ].s << sim.solutes[ 0 ].k << sim.solutes[ 0 ].v << "sn s,k,v"
               << sim.solutes[ n ].s << sim.solutes[ n ].k << sim.solutes[ n ].v;
   }
   //*DEBUG*
   ...
   if (dbg_level > 1 && datasets_to_process > 1 && generation < 11) {
      DbgLv(0) << "rank generation" << my_rank << generation << "fitness" << fitness << "vari0 vari1"
               << sim.variances[ 0 ] << sim.variances[ 1 ];
   }
   //*DEBUG*
   return fitness;
}
...
int US_MPI_Analysis::e_random(void) {
   ...
   static const double beta = population / divisor;
...
int US_MPI_Analysis::migrate_genes(void) {
   ...
   // MPI send emmigrants
   MPI_Send(
      emmigres.data(), // to MPI #4
      migrate_sols * solute_doubles, MPI_DOUBLE, MPI_Job::MASTER, EMMIGRATE, my_communicator);
   ...
   MPI_Recv(
      immigres.data(), // from MPI #5
      migrate_sols * solute_doubles, MPI_DOUBLE, MPI_Job::MASTER, IMMIGRATE, my_communicator, &status);
   ...
   if (mgenes_count > 0) {
      int grp_nbr = (my_rank / gcores_count);
      int deme_nbr = my_rank - grp_nbr * gcores_count;
      DbgLv(1) << "MG:Deme" << deme_nbr << ": solsent mg_count" << solutes_sent << mgenes_count << " elit"
               << elitism_count << "sol_dbls" << solute_doubles;
...
#define TIMING_MZ
   ...
   DbgLv(DL) << Phd << "vsize" << vsize << "fitness" << fitness << "gene0.s" << gene[ 0 ].s;
   int index = 0;
   ...
   while (uu.L2norm() >= epsilon_f && iteration < max_iterations) {
      ...
      while (((s2 - s1) > epsilon) && ((s3 - s2) > epsilon) && (reps++ < max_reps)) {
         ...
         double bb = (g_s3 * (s2_2 - s1_2) + g_s2 * (s1_2 - s3_2) + g_s1 * (s3_2 - s2_2)) * s1_s2 * s1_s3 * s2_s3;
         ...
         if (qAbs(aa) < max_a) {
            ...
            DRTiming << "   MMIZE:  t1 t2 t3 t4 t5 t6" << totT1 << totT2 << totT3 << totT4 << totT5 << totT6;
#endif
            return fitness;
         }
         ...
         if (qAbs(prev_g_s2 - g_s2) < epsilon)
            break;
      } // end of inner loop
      ...
      if (fac > sqrt(epsilon * sumdg * sumxi)) {
         ...
         for (int ii = 0; ii < vsize; ii++) {
            for (int jj = ii; jj < vsize; jj++) {
               hessian[ ii ][ jj ] += fac * v_dx[ ii ] * v_dx[ jj ] - fad * v_hdg[ ii ] * v_hdg[ jj ]
                                      + fae * v_dg[ ii ] * v_dg[ jj ];
...
void US_MPI_Analysis::vector_scaled_sum(US_Vector &cc, US_Vector &aa, double sa, US_Vector &bb, double sb) {
...
double US_MPI_Analysis::update_fitness(int index, US_Vector &v) {
   ...
   if (index < 0) { // Do the initial population of A and B matrices
      ...
      for (int vv = 0; vv < vsize; vv += 2) { // Fit each solute and populate the A matrix with simulations
         double xval = v[ vv ];
         double yval = v[ vv + 1 ];
         US_Model model;
   ...
   else { // Calculate one column of the A matrix and re-do NNLS
      nnls_x.resize(nsols);
      ...
      for (int ss = 0; ss < nscans; ss++) {
         for (int rr = 0; rr < npoints; rr++) { // Save the column, then replace it with simulation values
            nnls_c[ jj++ ] = nnls_a[ kk ];
            nnls_a[ kk++ ] = simdat.value(ss, rr);
         }
      }
      ...
      US_Math2::nnls(w_nnls_a.data(), ntotal, ntotal, nsols, w_nnls_b.data(), nnls_x.data());
      ...
      for (int cc = 0; cc < nsols; cc++) { // Build a model using solutes where the concentration is positive
         ...
         if (soluval > 0.0) {
            ...
            if (soluval >= concentration_threshold)
               ksol++;
         }
      }
...
void US_MPI_Analysis::dump_buckets(void) {
   ...
   for (int b = 0; b < buckets.size(); b++) {
      DbgLv(1) << buckets[ b ].x_min << buckets[ b ].x_max << buckets[ b ].y_min << buckets[ b ].y_max
               << buckets[ b ].ds << buckets[ b ].dk;
   }
}
...
void US_MPI_Analysis::dump_genes(int gene) {
   ...
   if (gene > -1) {
      ...
      for (int b = 0; b < genes[ gene ].size(); b++) {
         s += QString("(%1, %2); ").arg(genes[ gene ][ b ].s).arg(genes[ gene ][ b ].k);
      }
      ...
      for (int g = 0; g < genes.size(); g++) {
         ...
         for (int b = 0; b < genes[ g ].size(); b++) {
            s += QString("(%1, %2)\n").arg(genes[ g ][ b ].s).arg(genes[ g ][ b ].k);
         }
...
void US_MPI_Analysis::set_comp_attrib(US_Model::SimulationComponent &mcomp, double covalue, int attribx) {
   switch (attribx) { // Set the coefficient value indicated by the attribute index
      default:
...
void US_MPI_Analysis::build_component(
   US_Model::SimulationComponent &mcomp, US_Math2::SolutionData &sd, double xval, double yval) {
   US_SolveSim::DataSet *dset = data_sets[ 0 ];
...
void US_MPI_Analysis::solutes_from_gene(Gene &solutes, int nsols) {
   double fixval = (attr_z == ATTR_V) ? data_sets[ 0 ]->vbar20 : parameters[ "bucket_fixed" ].toDouble();
   ...
   for (int cc = 0; cc < nsols; cc++) {
      ...
      solutes[ cc ].d = (attr_z == ATTR_W || attr_z == ATTR_D || attr_z == ATTR_F) ? fixval : 0.0;
