  ...
  while (!finished) {
    ...
    switch (status.MPI_TAG) {
      ...
      case UPDATE:
        ...
        DbgLv(1) << "Deme" << deme_nbr << "UPD ds cnt len" << dataset << count
                 << length;
        ...
        if (is_global_fit &&
            count ==
                1) {  // For global update to scaled data, extra is new ODLimit
          length--;
...
void US_MPI_Analysis::ga_worker_loop(void) {
  ...
  for (generation = 0; generation < generations; generation++) {
    ...
    MPI_Send(genes[fitness[0].index].data(),  // to MPI #2
             solute_doubles * buckets.size(), MPI_DOUBLE, MPI_Job::MASTER, GENE,
             my_communicator);
        ...
        for (int ii = 0; ii < buckets.size(); ii++) {
          ...
          if (sdif > NEAR_MATCH || kdif > NEAR_MATCH) {
            DbgLv(1) << "gw:" << my_rank
                     ...
                     << "ii g0 g1 g0s g1s" << ii << g0 << g1 << genes[g0][ii].s
                     << genes[f1][ii].s;
            match = false;
            f0 = f1;
            break;
    ...
    for (int g = 0; g < elitism; g++) genes[g] = old_genes[fitness[g].index];
...
double US_MPI_Analysis::get_fitness(const Gene& gene) {
  ...
  for (int cc = 0; cc < nisols;
       cc++) {  // Concatenate all solute s,k values to form fitness key
    key += str.sprintf("%.5f%.5f", sim.solutes[cc].s, sim.solutes[cc].k);
  }

  DbgLv(2) << "get_fitness: nisols" << nisols << "key" << key;
  if (fitness_map.contains(key)) {  // We already have a match to this key, so
  ...
  if (data_sets.size() ==
      1) {  // Count solutes whose concentration is at least the threshold
    ...
    for (int cc = 0; cc < nosols; cc++) concen += sim.solutes[cc].c;
  ...
  if (dbg_level > 0 && fitness_map.size() == 20) {
    ...
    DbgLv(1) << "w:" << my_rank << generation << ": fmapsize fitness nsols"
             << fitness_map.size() << fitness << nisols << nosols << "s0 s,k,v"
             << sim.solutes[0].s << sim.solutes[0].k << sim.solutes[0].v
             << "sn s,k,v" << sim.solutes[n].s << sim.solutes[n].k
             << sim.solutes[n].v;
  }
  //*DEBUG*
...
int US_MPI_Analysis::migrate_genes(void) {
  ...
  if (mgenes_count > 0) {
    ...
    DbgLv(1) << "MG:Deme" << deme_nbr << ": solsent mg_count" << solutes_sent
             << mgenes_count << " elit" << elitism_count << "sol_dbls"
             << solute_doubles;
...
#define TIMING_MZ
  ...
  DRTiming << "   MMIZE:  t1 t2 t3 t4 t5 t6" << totT1 << totT2 << totT3 << totT4
           << totT5 << totT6;
#endif
  return fitness;
}
...
void US_MPI_Analysis::vector_scaled_sum(US_Vector& cc, US_Vector& aa, double sa,
  ...
  if (sb == 1.0)
    for (int ii = 0; ii < sizec; ii++) cc.assign(ii, aa[ii] * sa + bb[ii]);
  else
    for (int ii = 0; ii < sizec; ii++) cc.assign(ii, aa[ii] * sa + bb[ii] * sb);
...
double US_MPI_Analysis::update_fitness(int index, US_Vector& v) {
  ...
  if (index < 0) {  // Do the initial population of A and B matrices
    ...
    for (int vv = 0; vv < vsize;
         vv +=
         2) {  // Fit each solute and populate the A matrix with simulations
      double xval = v[vv];
      double yval = v[vv + 1];
      US_Model model;
    ...
    for (int ss = 0; ss < nscans; ss++)
      for (int rr = 0; rr < npoints; rr++) nnls_b[kk++] = edata->value(ss, rr);
    ...
    for (int ss = 0; ss < nscans; ss++) {
      for (int rr = 0; rr < npoints;
           rr++) {  // Save the column, then replace it with simulation values
        nnls_c[jj++] = nnls_a[kk];
        nnls_a[kk++] = simdat.value(ss, rr);
      }
    }
    ...
    US_Math2::nnls(w_nnls_a.data(), ntotal, ntotal, nsols, w_nnls_b.data(),
                   nnls_x.data());
    ...
    for (int ss = 0; ss < nscans; ss++)
      for (int rr = 0; rr < npoints; rr++) nnls_a[kk++] = nnls_c[jj++];

    model.components.clear();
    kk = 0;
    int ksol = 0;

    for (int cc = 0; cc < nsols; cc++) {  // Build a model using solutes where
...
void US_MPI_Analysis::set_comp_attrib(US_Model::SimulationComponent& mcomp,
  ...
  switch (
      attribx) {  // Set the coefficient value indicated by the attribute index
    default:
...
void US_MPI_Analysis::solutes_from_gene(Gene& solutes, int nsols) {
  ...
  for (int cc = 0; cc < nsols; cc++) {
    ...
    solutes[cc].d = (attr_z == ATTR_W || attr_z == ATTR_D || attr_z == ATTR_F)
                        ? fixval
                        : 0.0;
