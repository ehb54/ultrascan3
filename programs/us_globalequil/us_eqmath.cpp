...
US_EqMath::US_EqMath (QVector<US_DataIO::EditedData> &dataList,
                      ...
                      EqRunFit &runfit)
    : QObject (), dataList (dataList), scedits (scedits), scanfits (scanfits),
      runfit (runfit)
{
...
void
US_EqMath::init_params (int modx, bool update_mw, QList<double> &ds_vbar20s,
...
// Initialize for fit
void
US_EqMath::init_fit (int modx, int methx, FitCtrlPar &fitpars)
{
  ...
  DbgLv (1) << "EM:IF: scan 1 ntpts ndsets nfpars" << ntpts << ndsets
            << nfpars;
  ...
  DbgLv (1) << "EM:IF: scan 2 ntpts ndsets nfpars" << ntpts << ndsets
            << nfpars;
  ...
  DbgLv (1) << "EM:IF: scan 3 ntpts ndsets nfpars" << ntpts << ndsets
            << nfpars;

  for (int jj = 0; jj < runfit.nbr_assocs; jj++)
    if (runfit.eq_fits[jj])
      nfpars++;
  DbgLv (1) << "EM:IF: scan 4 ntpts ndsets nfpars" << ntpts << ndsets
            << nfpars;
...
//
void
US_EqMath::guess_mapForward (double *vguess)
{
  int jpx = 0;
...
void
US_EqMath::parameter_mapBackward (double *vguess)
{
  int jpx = 0;
      ...
      for (int ii = 0; ii < scanfits.size (); ii++)
        {
          ...
          for (int kk = 0; kk < ncomps; kk++)
            {
              v_vbar[kk]
                  = US_Math2::adjust_vbar20 (runfit.vbar_vals[kk], tempera);
        ...
        for (int ii = 0; ii < scanfits.size (); ii++)
          {
            ...
            double dconst = omega_sq / (2.0 * R_GC * (K0 + tempera));
            double ODcorrec = log (
                stoich1 / pow (scnf->extincts[0] * scnf->pathlen, stoiexp));
            ...
            for (int jj = jstx; jj < jstx + v_setpts[jdx]; jj++)
              {
                ...
                double ufunc1
                    = exp (stoich1 * ampv + runfit.eq_vals[0] + ODcorrec
        ...
        for (int ii = 0; ii < scanfits.size (); ii++)
          {
            ...
            double ODcorec1 = log (
                stoich1 / pow (scnf->extincts[0] * scnf->pathlen, stoiexp));
            double ODcorec2 = log (
                stoich2 / pow (scnf->extincts[0] * scnf->pathlen, stoiex2));
            v_vbar[0] = US_Math2::adjust_vbar20 (runfit.vbar_vals[0], tempera);
            v_buoy[0] = (1.0 - v_vbar[0] * density);
            double buoy = v_buoy[0];
            ...
            for (int jj = jstx; jj < jstx + v_setpts[jdx]; jj++)
              {
                ...
                double ufunc0 = exp (ampv + dconst * mwv * buoy * xv);
                double ufunc1
                    = exp (stoich1 * ampv + runfit.eq_vals[0] + ODcorec1
                           + dconst * stoich1 * mwv * buoy * xv);
                double ufunc2
                    = exp (stoich2 * ampv + runfit.eq_vals[1] + ODcorec2
        ...
        for (int ii = 0; ii < scanfits.size (); ii++)
          {
            ...
            double ODcorec1 = log ((extinc0 + extinc0)
                                   / (scnf->pathlen * extinc0 * extinc1));
            double ODcorec2
                = log (stoich1 / pow (scnf->pathlen * extinc0, stoiexp));
            v_vbar[0] = US_Math2::adjust_vbar20 (runfit.vbar_vals[0], tempera);
            v_vbar[1] = US_Math2::adjust_vbar20 (runfit.vbar_vals[1], tempera);
            v_vbar[2] = (v_vbar[0] * mwv0 + v_vbar[1] * mwv1) / mw_ab;
            v_buoy[0] = (1.0 - v_vbar[0] * density);
            ...
            for (int jj = jstx; jj < jstx + v_setpts[jdx]; jj++)
              {
                ...
                double ufunc3
                    = exp (stoich1 * ampv0 + runfit.eq_vals[0] + ODcorec2
        ...
        for (int ii = 0; ii < scanfits.size (); ii++)
          {
            ...
            double extinc0 = scnf->extincts[0];
            double ODcorec1
                = log (stoich1 / pow (extinc0 * scnf->pathlen, stoiexp));
            v_vbar[0] = US_Math2::adjust_vbar20 (runfit.vbar_vals[0], tempera);
            v_buoy[0] = (1.0 - v_vbar[0] * density);
            double buoy0 = v_buoy[0];
            ...
            for (int jj = jstx; jj < jstx + v_setpts[jdx]; jj++)
              {
                ...
                double ufunc2
                    = exp (stoich1 * ampv0 + runfit.eq_vals[0] + ODcorec1
...
double
US_EqMath::calc_testParameter (double mwval)
{
  int points = 0;
  double chi_sq = 0.0;
  double x_temp = 0.0;
    ...
    { // Build up chi-squared for fitted scans
      ...
      for (int ii = 0; ii < points; ii++)
        { // Update the chi-squared value
          double chi
              = yrvec[ii]
                - (coeffs[0] + coeffs[1] * exp (mwval * xyvec[ii].x ()));
          chi_sq += sq (chi);
        }
...
US_EqMath::linesearch ()
{
  ...
  DbgLv (1) << "  x1 x2" << x1 << x2 << " f1 f2" << f1 << f2 << "errmx"
            << errmx;
  ...
  while (check_flag)
    {
      ...
      DbgLv (1) << "     test-0b";
      if ((qAbs (x0) < dflt_min) && (qAbs (x1) < dflt_min)
          && (qAbs (x2) < dflt_min))
        return (0.0);
      ...
      iter++;
      DbgLv (1) << "MinRes:         iter" << iter << " f0 f1 f2" << f0 << f1
                << f2;
    }
...
US_EqMath::calc_residuals ()
{
  ...
  if (residual < 0.0)
    qDebug () << "CalcResid: resid" << residual << "delta 0,1,m,n"
              << y_delta[0] << y_delta[1] << y_delta[ntpts - 2]
              << y_delta[ntpts - 1];
...
int
US_EqMath::calc_model (double *guess)
{
  int stat = 0;
...
void
US_EqMath::genLeastSquaresOrd2 (double **MM, int points, double *y_raw,
                                double **coeff)
{
...
bool
US_EqMath::Cholesky_DecompOrd2 (double **aa)
{
...
bool
US_EqMath::Cholesky_SolveSysOrd2 (double **LL, double *bb)
{
        ...
        for (int ii = 0; ii < scanfits.size (); ii++)
          {
            ...
            double ODcorrec
                = log (stoich1
                       / pow (scnf->extincts[0] * scnf->pathlen, stoiexp))
                  + eqval0;
            double dconst = omega_sq / (2.0 * R_GC * (K0 + tempera));

            double vbar
                = US_Math2::adjust_vbar20 (runfit.vbar_vals[0], tempera);
            double buoy = (1.0 - vbar * density);
            double xval = scnf->meniscus;
        ...
        for (int ii = 0; ii < scanfits.size (); ii++)
          {
            ...
            double ODcorre2
                = log (stoich2
                       / pow (scnf->extincts[0] * scnf->pathlen, stoiex2))
                  + eqval1;
            double dconst = omega_sq / (2.0 * R_GC * (K0 + tempera));
        ...
        for (int ii = 0; ii < scanfits.size (); ii++)
          {
            ...
            double vbar0
                = US_Math2::adjust_vbar20 (runfit.vbar_vals[0], tempera);
            double vbar1
                = US_Math2::adjust_vbar20 (runfit.vbar_vals[1], tempera);
            ...
            double ODcorrec
                = log (extin_ab / (extinc0 * extinc1 * scnf->pathlen))
                  + eqval0;
            xval = scnf->meniscus;
...
double
US_EqMath::calc_bottom (double rpm)
{
