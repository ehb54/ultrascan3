...
US_EqMath::US_EqMath(
   QVector<US_DataIO::EditedData> &dataList, QVector<ScanEdit> &scedits, QVector<EqScanFit> &scanfits,
   EqRunFit &runfit) : QObject(), dataList(dataList), scedits(scedits), scanfits(scanfits), runfit(runfit) {
   dbg_level = US_Settings::us_debug();
}

// Initialize parameters
void US_EqMath::init_params(int modx, bool update_mw, QList<double> &ds_vbar20s, QList<double> &aud_pars) {
   modelx = modx;
...
void US_EqMath::init_fit(int modx, int methx, FitCtrlPar &fitpars) {
   ...
   DbgLv(1) << "EM:IF: modelx nlsmeth nc" << modelx << nlsmeth << runfit.nbr_comps;
   ...
   for (int jj = 0; jj < runfit.nbr_comps; jj++) {
      if (runfit.mw_fits[ jj ] == true)
         nfpars++;

      if (runfit.vbar_fits[ jj ] == true)
         nfpars++;

      if (runfit.viri_fits[ jj ] == true)
         nfpars++;
   }
...
void US_EqMath::parameter_mapBackward(double *vguess) {
   ...
   for (int jj = 0; jj < runfit.nbr_comps; jj++) {
      if (runfit.mw_fits[ jj ])
         runfit.mw_vals[ jj ] = vguess[ jpx++ ];

      if (runfit.vbar_fits[ jj ])
         runfit.vbar_vals[ jj ] = vguess[ jpx++ ];

      if (runfit.viri_fits[ jj ])
         runfit.viri_vals[ jj ] = vguess[ jpx++ ];
   }
   ...
   for (int ii = 0; ii < scanfits.size(); ii++) {
      ...
      for (int jj = 0; jj < runfit.nbr_comps; jj++) {
         if (scnf->amp_fits[ jj ])
            scnf->amp_vals[ jj ] = vguess[ jpx++ ];

         if (scnf->baseln_fit)
            scnf->baseline = vguess[ jpx++ ];
      }
   }

   for (int jj = 0; jj < runfit.nbr_assocs; jj++)
      if (runfit.eq_fits[ jj ])
         runfit.eq_vals[ jj ] = vguess[ jpx++ ];
   ...
   switch (modelx) {
            ...
            for (int jj = jstx; jj < jstx + v_setpts[ jdx ]; jj++) {
               ...
               for (int kk = 0; kk < ncomps; kk++) {
                  ...
                  if (runfit.vbar_fits[ kk ])
                     m_jacobi[ jpx ][ runfit.vbar_ndxs[ kk ] ] = (-1.0) * dconst * mwv * xv * ufunc * density;

                  if (scnf->amp_fits[ kk ])
                     m_jacobi[ jpx ][ scnf->amp_ndxs[ kk ] ] = ufunc;
               }
      ...
      case 10: // 10: "User-Defined Monomer-Nmer Equilibrium"
      {
         ...
         for (int ii = 0; ii < scanfits.size(); ii++) {
            ...
            double density = scnf->density;
            double dconst = omega_sq / (2.0 * R_GC * (K0 + tempera));
            double ODcorrec = log(stoich1 / pow(scnf->extincts[ 0 ] * scnf->pathlen, stoiexp));
            ...
            for (int jj = jstx; jj < jstx + v_setpts[ jdx ]; jj++) {
               ...
               double ufunc1 = exp(
                  stoich1 * ampv + runfit.eq_vals[ 0 ] + ODcorrec + dconst * stoich1 * mwv * buoy * ufunc0);
               v_ufunc[ 0 ] = ufunc0;
               v_ufunc[ 1 ] = ufunc1;
               double dcoeff = dconst * xv * buoy;

               if (runfit.mw_fits[ 0 ])
                  m_jacobi[ jpx ][ runfit.mw_ndxs[ 0 ] ] = dcoeff * ufunc0 + dcoeff * ufunc1 * stoich1;

               dcoeff = dconst * mwv * xv * density;

               if (runfit.vbar_fits[ 0 ])
                  m_jacobi[ jpx ][ runfit.vbar_ndxs[ 0 ] ] = (-1.0) * dcoeff * ufunc0 - dcoeff * ufunc1 * stoich1;
      ...
      case 13: // 13: "User-Defined Monomer - N-mer - M-mer Equilibrium"
      {
         ...
         for (int ii = 0; ii < scanfits.size(); ii++) {
            ...
            double density = scnf->density;
            double dconst = omega_sq / (2.0 * R_GC * (K0 + tempera));
            double ODcorec1 = log(stoich1 / pow(scnf->extincts[ 0 ] * scnf->pathlen, stoiexp));
            double ODcorec2 = log(stoich2 / pow(scnf->extincts[ 0 ] * scnf->pathlen, stoiex2));
            v_vbar[ 0 ] = US_Math2::adjust_vbar20(runfit.vbar_vals[ 0 ], tempera);
            v_buoy[ 0 ] = (1.0 - v_vbar[ 0 ] * density);
            double buoy = v_buoy[ 0 ];
            ...
            for (int jj = jstx; jj < jstx + v_setpts[ jdx ]; jj++) {
               ...
               double ampv = scnf->amp_vals[ 0 ];
               double ufunc0 = exp(ampv + dconst * mwv * buoy * xv);
               double ufunc1 = exp(stoich1 * ampv + runfit.eq_vals[ 0 ] + ODcorec1 + dconst * stoich1 * mwv * buoy * xv);
               double ufunc2 = exp(stoich2 * ampv + runfit.eq_vals[ 1 ] + ODcorec2 + dconst * stoich2 * mwv * buoy * xv);
               v_ufunc[ 0 ] = ufunc0;
               v_ufunc[ 1 ] = ufunc1;
               v_ufunc[ 2 ] = ufunc2;
               double dcoeff = dconst * xv * buoy;
               ...
               if (scnf->amp_fits[ 0 ])
                  m_jacobi[ jpx ][ scnf->amp_ndxs[ 0 ] ] = ufunc0 + ufunc1 * stoich1 + ufunc2 * stoich2;
      ...
      case 14: // 14: "2-Component Hetero-Association: A + B <=> AB"
      {
         ...
         for (int ii = 0; ii < scanfits.size(); ii++) {
            ...
            double ODcorrec = log((extinc0 + extinc1) / (scnf->pathlen * extinc0 * extinc1));
            v_vbar[ 0 ] = US_Math2::adjust_vbar20(runfit.vbar_vals[ 0 ], tempera);
            ...
            for (int jj = jstx; jj < jstx + v_setpts[ jdx ]; jj++) {
               ...
               double ufunc2 = exp(ampv0 + ampv1 + runfit.eq_vals[ 0 ] + ODcorrec + constx * mw_ab * buoy2);
               v_ufunc[ 0 ] = ufunc0;
               v_ufunc[ 1 ] = ufunc1;
               v_ufunc[ 2 ] = ufunc2;

               if (runfit.mw_fits[ 0 ])
                  m_jacobi[ jpx ][ runfit.mw_ndxs[ 0 ] ] = constx * buoy0 * ufunc0 + constx * buoy2
                                                           + constx * (v_vbar[ 2 ] * density - v_vbar[ 0 ] * density)
                                                           + ufunc2;

               if (runfit.vbar_fits[ 0 ])
                  m_jacobi[ jpx ][ runfit.vbar_ndxs[ 0 ] ] = (-1.0) * constx * mwv1 * density * ufunc1
      ...
      case 15: // 15: "U-Defined self/Hetero-Assoc.: A + B <=> AB, nA <=> An"
      {
         ...
         for (int ii = 0; ii < scanfits.size(); ii++) {
            ...
            double extinc0 = scnf->extincts[ 0 ];
            double extinc1 = scnf->extincts[ 1 ];
            double ODcorec1 = log((extinc0 + extinc0) / (scnf->pathlen * extinc0 * extinc1));
            ...
            for (int jj = jstx; jj < jstx + v_setpts[ jdx ]; jj++) {
               ...
               double ufunc0 = exp(ampv0 + constx * mwv0 * buoy0);
               double ufunc1 = exp(ampv1 + constx * mwv1 * buoy1);
               double ufunc2 = exp(ampv0 + ampv1 + runfit.eq_vals[ 0 ] + ODcorec1 + constx * mw_ab * buoy2);
               double ufunc3 = exp(stoich1 * ampv0 + runfit.eq_vals[ 0 ] + ODcorec2 + constx * stoich1 * mw_ab * buoy0);
               v_ufunc[ 0 ] = ufunc0;
               v_ufunc[ 1 ] = ufunc1;
               v_ufunc[ 2 ] = ufunc2;
               v_ufunc[ 3 ] = ufunc3;
               ...
               if (runfit.mw_fits[ 1 ])
                  m_jacobi[ jpx ][ runfit.mw_ndxs[ 1 ] ] = constx * buoy1 * ufunc1 + constx * buoy2
                                                           + constx * (v_vbar[ 2 ] * density - v_vbar[ 1 ] * density)
                                                           + ufunc2;

               if (runfit.vbar_fits[ 0 ])
                  m_jacobi[ jpx ][ runfit.vbar_ndxs[ 0 ] ] = (-1.0) * constx * mwv0 * density * ufunc0
                                                             - constx * mwv0 * density * ufunc3 * stoich1
                                                             - constx * mwv0 * density * ufunc2;

               if (runfit.vbar_fits[ 1 ])
                  m_jacobi[ jpx ][ runfit.vbar_ndxs[ 1 ] ] = (-1.0) * constx * mwv1 * density * ufunc1
                                                             - constx * mwv1 * density * ufunc2;

               if (scnf->amp_fits[ 0 ])
                  m_jacobi[ jpx ][ scnf->amp_ndxs[ 0 ] ] = ufunc0 + ufunc3 * stoich1 + ufunc2;
      ...
      case 16: // 16: "U-Defined Monomer-Nmer, some monomer is incompetent"
      {
         ...
         for (int ii = 0; ii < scanfits.size(); ii++) {
            ...
            for (int jj = jstx; jj < jstx + v_setpts[ jdx ]; jj++) {
               ...
               double ufunc2 = exp(stoich1 * ampv0 + runfit.eq_vals[ 0 ] + ODcorec1 + constx * stoich1 * mwv0 * buoy0);
               v_ufunc[ 0 ] = ufunc0;
               v_ufunc[ 1 ] = ufunc1;
               v_ufunc[ 2 ] = ufunc2;

               if (runfit.mw_fits[ 0 ])
                  m_jacobi[ jpx ][ runfit.mw_ndxs[ 0 ] ] = constx * buoy0 * ufunc0 + constx * buoy0 * ufunc1
               ...
               if (scnf->amp_fits[ 1 ])
                  m_jacobi[ jpx ][ scnf->amp_ndxs[ 1 ] ] = ufunc1;
      ...
      case 17: // 17: "User-Defined Monomer-Nmer, some Nmer is incompetent"
      {
         ...
         for (int ii = 0; ii < scanfits.size(); ii++) {
            ...
            for (int jj = jstx; jj < jstx + v_setpts[ jdx ]; jj++) {
               ...
               if (runfit.mw_fits[ 0 ])
                  m_jacobi[ jpx ][ runfit.mw_ndxs[ 0 ] ] = constx * buoy0 * ufunc0 + constx * buoy0 * ufunc1 * stoich1
                                                           + constx * buoy0 * ufunc2 * stoich1;

               if (runfit.vbar_fits[ 0 ])
                  m_jacobi[ jpx ][ runfit.vbar_ndxs[ 1 ] ] = (-1.0) * constx * mwv0 * density * ufunc0
      ...
      case 18: // 18: "User-Defined irreversible Monomer-Nmer"
      {
         ...
         for (int ii = 0; ii < scanfits.size(); ii++) {
            ...
            for (int jj = jstx; jj < jstx + v_setpts[ jdx ]; jj++) {
               ...
               if (scnf->amp_fits[ 0 ])
                  m_jacobi[ jpx ][ scnf->amp_ndxs[ 0 ] ] = ufunc0;
      ...
      case 19: // 19: "User-Defined Monomer-Nmer plus contaminant"
      {
         ...
         for (int ii = 0; ii < scanfits.size(); ii++) {
            ...
            for (int jj = jstx; jj < jstx + v_setpts[ jdx ]; jj++) {
               ...
               if (runfit.mw_fits[ 0 ])
                  m_jacobi[ jpx ][ runfit.mw_ndxs[ 0 ] ] = constx * buoy0 * ufunc0 + constx * buoy0 * ufunc2 * stoich1;
...
double US_EqMath::calc_testParameter(double mwval) {
   ...
   for (int jes = 0; jes < scanfits.size(); jes++) { // Build up chi-squared for fitted scans
      ...
      for (int jxy = strtx; jxy < stopn; jxy++) { // Accumulate the X,Y values within the scan's start,stop range
         x_val = scnf->xvs[ jxy ];
         y_val = scnf->yvs[ jxy ];
      ...
      for (int ii = 0; ii < points; ii++) { // Build matrix and vector needed for LS
      ...
      for (int ii = 0; ii < points; ii++) { // Update the chi-squared value
         double chi = yrvec[ ii ] - (coeffs[ 0 ] + coeffs[ 1 ] * exp(mwval * xyvec[ ii ].x()));
         chi_sq += sq(chi);
      }
...
double US_EqMath::linesearch() {
   ...
   while (check_flag) {
      if ((isNan(f0) && isNan(f1)) || (isNan(f1) && isNan(f2)) || (isNan(f0) && isNan(f1)))
      ...
      if (((qAbs(f2 - f0) < dflt_min) && (qAbs(f1 - f0) < dflt_min)) || ((f0 > f1) && (qAbs(f2 - f1) < dflt_min)))
         return (0.0);
      ...
      DbgLv(1) << "     test-0c";
      if (((qAbs(f0 - f1) < dflt_min) && (qAbs(f1 - f2) < dflt_min)) || ((f2 > f1) && (qAbs(f0 - f1) < dflt_min)))
         return (0.0);
...
double US_EqMath::calc_residuals() {
   ...
   if (residual < 0.0)
      qDebug() << "CalcResid: resid" << residual << "delta 0,1,m,n" << y_delta[ 0 ] << y_delta[ 1 ]
               << y_delta[ ntpts - 2 ] << y_delta[ ntpts - 1 ];
...
void US_EqMath::genLeastSquaresOrd2(double **MM, int points, double *y_raw, double **coeff) {
   ...
   for (int ii = 0; ii < 2; ii++) { // Fill in the lower triangle of the 2 x 2 "A" matrix
   ...
   switch (modelx) {
      ...
      case 10: // 10: "User-Defined Monomer-Nmer Equilibrium"
      {
         ...
         for (int ii = 0; ii < scanfits.size(); ii++) {
            ...
            double tempera = scnf->tempera;
            double density = scnf->density;
            double ODcorrec = log(stoich1 / pow(scnf->extincts[ 0 ] * scnf->pathlen, stoiexp)) + eqval0;
            double dconst = omega_sq / (2.0 * R_GC * (K0 + tempera));
      ...
      case 13: // 13: "User-Defined Monomer - N-mer - M-mer Equilibrium"
      {
         ...
         for (int ii = 0; ii < scanfits.size(); ii++) {
            ...
            double tempera = scnf->tempera;
            double density = scnf->density;
            double ODcorre1 = log(stoich1 / pow(scnf->extincts[ 0 ] * scnf->pathlen, stoiexp)) + eqval0;
            double ODcorre2 = log(stoich2 / pow(scnf->extincts[ 0 ] * scnf->pathlen, stoiex2)) + eqval1;
            double dconst = omega_sq / (2.0 * R_GC * (K0 + tempera));
      ...
      case 14: // 14: "2-Component Hetero-Association: A + B <=> AB"
      {
         ...
         for (int ii = 0; ii < scanfits.size(); ii++) {
            ...
            double ODcorrec = log(extin_ab / (extinc0 * extinc1 * scnf->pathlen)) + eqval0;
            xval = scnf->meniscus;
...
double US_EqMath::calc_bottom(double rpm) {
   return (runfit.bottom_pos + runfit.rcoeffs[ 0 ] * rpm + runfit.rcoeffs[ 1 ] * sq(rpm));
}
