...
US_ModelSelect::US_ModelSelect(int &a_modelx, QStringList &a_models, QList<double> &a_udpars) :
    US_WidgetsDialog(0, 0), smodelx(a_modelx), models(a_models), aud_pars(a_udpars) {
...
void US_ModelSelect::selected() {
   aud_pars.clear();
   smodelx = lw_models->selectedItems().size() > 0 ? lw_models->currentRow() : -1;

   if (smodelx == (-1)) {
      QMessageBox::information(
         this, tr("No Data Selected"), tr("You have not selected any data.\nSelect Model or Cancel"));
      return;
   }
...
QString US_ModelSelect::function_equation(QStringList &comps) {
   ...
   switch (smodelx) { // Create unique function string and components (terms) list, ea. type
      case 0: //  0: "1-Component, Ideal"
         msg = msg + "exp[ (ln(A) + M * omega^2 * (1 - vbar * D)" + " * (X^2 - Xr^2)) / (2 * R_GC * T) ] + B";
         comps << "X" << "Xr" << "Aa" << "M" << "E" << "D" << "R" << "T"
               << "B";
         break;
      case 1: //  1: "2-Component, Ideal, Noninteracting"
         msg = msg + "exp[ ln(A[1]) + M[1] * omega^2 * (1 - vbar[1] * D)" + " * (X^2 - Xr^2) / (2 * R_GC * T) ]\n"
               + "  + exp[ (ln(A[2]) + M[2] * omega^2 * (1 - vbar[2] * D) * " + "(X^2 - Xr ^2) / (2 * R_GC * T) ] + B";
         comps << "X" << "Xr" << "Aa" << "M" << "D" << "R" << "T" << "B";
         break;
      case 2: //  2: "3-Component, Ideal, Noninteracting"
         msg = msg + "exp[ ln(A[1]) + M[1] * omega^2 * (1 - vbar[1] * D)" + " * (X^2 - Xr^2) / (2 * R_GC * T) ]\n"
               + "  + exp[ (ln(A[2]) + M[2] * omega^2 * (1 - vbar[2] * D) * " + "(X^2 - Xr^2) / (2 * R_GC * T) ]\n"
               + "  + exp[ (ln(A[3]) + M[3] * omega^2 * (1 - vbar[3] * D) * " + "(X^2 - Xr^2) / (2 * R_GC * T) ] + B";
         comps << "X" << "Xr" << "Aa" << "M" << "D" << "R" << "T" << "B";
         break;
      ...
      case 4: //  4: "Monomer-Dimer Equilibrium"
         msg = msg + "exp[ (ln(A) + M * omega^2 * (1 - vbar * D)" + " * (X^2 - Xr^2)) / (2 * R_GC * T) ]\n"
      ...
      case 14: // 14: "2-Component Hetero-Association: A + B <=> AB"
         msg = msg + "exp[ (ln(A) + M_A * omega^2 * (1 - vbar_A * D)" + " * (X^2 - Xr^2)) / (2 * R_GC * T) ]\n"
               + "  + exp[ (ln(B) + M_B * omega^2 * (1 - vbar_B * D) *" + " (X^2 - Xr^2)) / (2 * R_GC * T) ]\n"
               + "  + exp[ (ln(A) + ln(B) + ln(K_AB) + ln(E_AB/(E_A * E_B * L))"
               + " + (M_A + M_B) * omega^2 * (1 - vbar_AB * D) * (X^2 - Xr^2)) /" + " (2 * R_GC * T) ] + B";
      ...
      case 15: // 15: "U-Defined self/Hetero-Assoc.: A + B <=> AB, nA <=> An"
         msg = msg + "exp[ (ln(A) + M_A * omega^2 * (1 - vbar_A * D)" + " * (X^2 - Xr^2)) / (2 * R_GC * T) ]\n"
               + "  + exp[ (ln(B) + M_B * omega^2 * (1 - vbar_B * D) *" + " (X^2 - Xr^2)) / (2 * R_GC * T) ]\n"
               + "  + exp[ (ln(A) + ln(B) + ln(K_AB) + ln(E_AB/(E_A * E_B * L))"
               + " + (M_A + M_B) * omega^2 * (1 - vbar_AB * D) * (X^2 - Xr^2)) /" + " (2 * R_GC * T) ]\n"
               + "  + exp[ (N *ln(A) + ln(N/(E*L)^(N-1)) + ln(K1,N) + N * M_A"
               + " * omega^2 * (1 - vbar_A * D) * (X^2 - Xr^2)) /" + " (2 * R_GC * T) ] + B";
         comps << "X" << "Xr" << "Aa" << "Ab" << "E" << "L" << "KC" << "KN"
               << "MA" << "MB" << "MC" << "VA" << "VB" << "VC"
               << "D" << "R" << "T" << "B";
         break;
      case 16: // 16: "U-Defined Monomer-Nmer, some monomer is incompetent"
         msg = msg + "exp[ (ln(A1) + M * omega^2 * (1 - vbar * D)" + " * (X^2 - Xr^2)) / (2 * R_GC * T) ]\n"
               + "  + exp[ (N * ln(A1) + ln(N/(E * L)^(N - 1) + ln(K1,N) +"
               + " N * M * omega^2 * (1 - vbar * D) * (X^2 - Xr^2)) /" + " (2 * R_GC * T) ]\n"
               + "  + exp[ (ln(A2) + M * omega^2 * (1 - vbar * D) *" + " (X^2 - Xr^2)) / (2 * R_GC * T) ] + B";
      ...
      case 17: // 17: "User-Defined Monomer-Nmer, some Nmer is incompetent"
         msg = msg + "exp[ (ln(A1) + M * omega^2 * (1 - vbar * D)" + " * (X^2 - Xr^2)) / (2 * R_GC * T) ]\n"
               + "  + exp[ (N * ln(A1) + ln(N/(E * L)^(N - 1) + ln(K1,N) +"
               + " N * M * omega^2 * (1 - vbar * D) * (X^2 - Xr^2)) /" + " (2 * R_GC * T) ]\n"
               + "  + exp[ (ln(A2) + N * M * omega^2 * (1 - vbar * D) *" + " (X^2 - Xr^2)) / (2 * R_GC * T) ] + B";
         comps << "N" << "X" << "Xr" << "Aa" << "E" << "L" << "K1"
               << "M" << "A1" << "A2" << "D" << "R" << "T" << "B";
         break;
      case 18: // 18: "User-Defined irreversible Monomer-Nmer"
         msg = msg + "exp[ (ln(A1) + M * omega^2 * (1 - vbar * D)" + " * (X^2 - Xr^2)) / (2 * R_GC * T) ]\n"
               + "  + exp[ (N * ln(A1) + ln(N/(E * L)^(N - 1) + ln(K1,N) +"
               + " N * M * omega^2 * (1 - vbar * D) * (X^2 - Xr^2)) /" + " (2 * R_GC * T) ]\n"
               + "  + exp[ (ln(A2) + M * omega^2 * (1 - vbar * D) *" + " (X^2 - Xr^2)) / (2 * R_GC * T) ]\n"
               + "  + exp[ (ln(A3) + N * M * omega^2 * (1 - vbar * D) *" + " (X^2 - Xr^2)) / (2 * R_GC * T) ] + B";
         comps << "N" << "X" << "Xr" << "Aa" << "E" << "L" << "K1"
               << "M" << "A1" << "A2" << "A3" << "D" << "R" << "T" << "B";
         break;
      case 19: // 19: "User-Defined Monomer-Nmer plus contaminant"
         msg = msg + "exp[ (ln(A1) + M1 * omega^2 * (1 - vbar[1] * D)" + " * (X^2 - Xr^2)) / (2 * R_GC * T) ]\n"
               + "  + exp[ (N * ln(A1) + ln(N/(E * L)^(N - 1) + ln(K1,N) +"
               + " N * M * omega^2 * (1 - vbar[1] * D) * (X^2 - Xr^2)) /" + " (2 * R_GC * T) ]\n"
               + "  + exp[ (ln(A2) + M2 * omega^2 * (1 - vbar[2] * D) *" + " (X^2 - Xr^2)) / (2 * R_GC * T) ] + B";
         comps << "N" << "X" << "Xr" << "Aa" << "E" << "L" << "K1"
               << "M1" << "M2" << "A1" << "A2" << "D" << "R" << "T" << "B";
         break;
      default:
...
QString US_ModelSelect::function_components(QStringList &comps, int &notef) {
   ...
   if (comps.contains("X"))
      ms = ms + "\t" + tr("X  = Radius\n");
   if (comps.contains("Xr"))
      ms = ms + "\t" + tr("Xr = Reference Radius\n");
   if (comps.contains("Aa"))
      ms = ms + "\t" + tr("A  = Amplitude of component A *\n");
   if (comps.contains("Ab"))
      ms = ms + "\t" + tr("B  = Amplitude of component B *\n");
   if (comps.contains("N"))
      ms = ms + "\t" + tr("N  = User-Defined Association State\n");
   if (comps.contains("M"))
      ms = ms + "\t" + tr("M  = Molecular Weight *\n");
   if (comps.contains("E"))
      ms = ms + "\t" + tr("E  = Extinction Coefficient\n");
   if (comps.contains("L"))
      ms = ms + "\t" + tr("L  = Pathlength\n");
   if (comps.contains("Ai"))
      ms = ms + "\t" + tr("A[i] = Amplitude of component \"i\" *\n");
   if (comps.contains("Mi"))
      ms = ms + "\t" + tr("M[i] = Molecular Weight of component \"i\" *\n");
   if (comps.contains("N1"))
      ms = ms + "\t" + tr("N1 = Stoichiometry of first Association\n");
   if (comps.contains("N2"))
      ms = ms + "\t" + tr("N2 = Stoichiometry of second Association\n");
   if (comps.contains("K1"))
      ms = ms + "\t" + tr("K1 = Monomer-Dimer Equilibrium Constant *,**\n");
   if (comps.contains("KB"))
      ms = ms + "\t" + tr("K_AB = AB Equilibrium Constant *,**\n");
   if (comps.contains("KN"))
      ms = ms + "\t" + tr("K_AN = A monomer-nmer Equilibrium Constant *,**\n");
   if (comps.contains("MA"))
      ms = ms + "\t" + tr("M_A  = Molecular Weight of A *\n");
   if (comps.contains("MB"))
      ms = ms + "\t" + tr("M_B  = Molecular Weight of B *\n");
   if (comps.contains("MC"))
      ms = ms + "\t" + tr("M_AB = Molecular Weight of AB\n");
   if (comps.contains("M1"))
      ms = ms + "\t" + tr("M1 = Monomer Molecular Weight *\n");
   if (comps.contains("M2"))
      ms = ms + "\t" + tr("M2 = Contaminant Molecular Weight *\n");
   if (comps.contains("VA"))
      ms = ms + "\t" + tr("vbar_A  = vbar of A *\n");
   if (comps.contains("VB"))
      ms = ms + "\t" + tr("vbar_B  = vbar of B *\n");
   if (comps.contains("VC"))
      ms = ms + "\t" + tr("vbar_AB = vbar of AB\n");
   if (comps.contains("D"))
      ms = ms + "\t" + tr("D  = Density Coefficient\n");
   if (comps.contains("R"))
      ms = ms + "\t" + tr("R  = Gas Constant\n");
   if (comps.contains("T"))
      ms = ms + "\t" + tr("T  = Temperature\n");
   if (comps.contains("A1"))
      ms = ms + "\t" + tr("A1 = Reference Concentration of monomer *\n");
   if (comps.contains("A2"))
      ms = ms + "\t" + tr("A2 = Reference Concentration of incompetent monomer *\n");
   if (comps.contains("A3"))
      ms = ms + "\t" + tr("A3 = Reference Concentration of incompetent N-mer *\n");
   if (comps.contains("B"))
      ms = ms + "\t" + tr("B  = Baseline *\n");
