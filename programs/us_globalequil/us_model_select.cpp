...
US_ModelSelect::US_ModelSelect (int &a_modelx, QStringList &a_models,
    ...
    : US_WidgetsDialog (0, 0), smodelx (a_modelx), models (a_models),
      aud_pars (a_udpars)
{
...
void
US_ModelSelect::selected ()
{
  aud_pars.clear ();
  smodelx = lw_models->selectedItems ().size () > 0 ? lw_models->currentRow ()
                                                    : -1;
...
void
US_ModelSelect::function_dialog ()
{
...
QString
US_ModelSelect::function_equation (QStringList &comps)
{
  ...
  switch (smodelx)
    ...
    case 5: //  5: "Monomer-Trimer Equilibrium"
      msg = msg + "exp[ (ln(A) + M * omega^2 * (1 - vbar * D)"
            ...
            + " omega^2 * (1 - vbar * D) * (X^2 - Xr^2)) / (2 * R_GC * T) ] + "
              "B";
      comps << "X" << "Xr" << "Aa" << "E" << "L" << "K1" << "M" << "D"
            << "R" << "T" << "B";
      break;
...
QString
US_ModelSelect::function_components (QStringList &comps, int &notef)
{
  notef = 1;
  QString ms = tr ("\n\nwhere:\n");
