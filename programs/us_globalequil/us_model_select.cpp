...
US_ModelSelect::US_ModelSelect(int& a_modelx, QStringList& a_models,
  ...
  for (int ii = 0; ii < models.size(); ii++) lw_models->addItem(models.at(ii));
...
void US_ModelSelect::selected() {
  aud_pars.clear();
  smodelx =
      lw_models->selectedItems().size() > 0 ? lw_models->currentRow() : -1;
...
void US_ModelSelect::model_pars_0() { function_dialog(); }
...
QString US_ModelSelect::function_equation(QStringList& comps) {
  ...
  switch (smodelx) {  // Create unique function string and components (terms)
                      // list, ea. type
    ...
    case 13:  // 13: "User-Defined Monomer - N-mer - M-mer Equilibrium"
      msg =
          msg + "exp[ (ln(A) + M * omega^2 * (1 - vbar * D)" +
          " * (X^2 - Xr^2)) / (2 * R_GC * T) ]\n" +
...
QString US_ModelSelect::function_components(QStringList& comps, int& notef) {
  ...
  if (comps.contains("X")) ms = ms + "\t" + tr("X  = Radius\n");
  if (comps.contains("Xr")) ms = ms + "\t" + tr("Xr = Reference Radius\n");
  ...
  if (comps.contains("M")) ms = ms + "\t" + tr("M  = Molecular Weight *\n");
  if (comps.contains("E")) ms = ms + "\t" + tr("E  = Extinction Coefficient\n");
  if (comps.contains("L")) ms = ms + "\t" + tr("L  = Pathlength\n");
  ...
  if (comps.contains("VA")) ms = ms + "\t" + tr("vbar_A  = vbar of A *\n");
  if (comps.contains("VB")) ms = ms + "\t" + tr("vbar_B  = vbar of B *\n");
  if (comps.contains("VC")) ms = ms + "\t" + tr("vbar_AB = vbar of AB\n");
  if (comps.contains("D")) ms = ms + "\t" + tr("D  = Density Coefficient\n");
  if (comps.contains("R")) ms = ms + "\t" + tr("R  = Gas Constant\n");
  if (comps.contains("T")) ms = ms + "\t" + tr("T  = Temperature\n");
  ...
  if (comps.contains("A3"))
    ms =
        ms + "\t" + tr("A3 = Reference Concentration of incompetent N-mer *\n");
  if (comps.contains("B")) ms = ms + "\t" + tr("B  = Baseline *\n");
