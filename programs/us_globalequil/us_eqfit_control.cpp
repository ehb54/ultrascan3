...
US_EqFitControl::US_EqFitControl(
   QVector<EqScanFit> &a_scanfits, EqRunFit &a_runfit, US_DataIO::EditedData *a_edata, US_EqMath *a_emath,
   US_EqReporter *a_ereporter, int a_modelx, QStringList a_models, bool &a_fWidget, int &a_selscan) :
    US_WidgetsDialog(0, 0), scanfits(a_scanfits), runfit(a_runfit), edata(a_edata), emath(a_emath),
    ereporter(a_ereporter), modelx(a_modelx), models(a_models), fWidget(a_fWidget), selscan(a_selscan) {
   qDebug() << "EFC: IN";
   ...
   dplot
      = new US_Plot(data_plot, tr("Residuals"), tr("Radius^2 - Radius(ref)^2 (cm)"), tr("Optical Density Difference"));
   QGridLayout *statsLayout = new QGridLayout;

   // Fitting Control layout
   QLabel *lb_fbanner = us_banner(tr("Nonlinear Least Squares\n"
   ...
   connect(pb_strtfit, SIGNAL(clicked()), this, SLOT(start_fit()));
   connect(pb_pause, SIGNAL(clicked()), this, SLOT(pause_fit()));
   connect(pb_resume, SIGNAL(clicked()), this, SLOT(resume_fit()));
   connect(pb_savefit, SIGNAL(clicked()), this, SLOT(save_fit()));
   connect(pb_viewrep, SIGNAL(clicked()), this, SLOT(view_report()));
   connect(pb_resids, SIGNAL(clicked()), this, SLOT(plot_residuals()));
   connect(pb_ovrlays, SIGNAL(clicked()), this, SLOT(plot_overlays()));
   connect(pb_close, SIGNAL(clicked()), this, SLOT(closed()));
   ...
   QLabel *lb_gbanner = us_banner(tr("Graph Plotting Controls"));
   QGridLayout *lo_pltalld = us_radiobutton(tr("All Data"), rb_pltalld, true);
   QGridLayout *lo_pltgrp5 = us_radiobutton(tr("Groups of 5"), rb_pltgrp5, false);
   QGridLayout *lo_pltsscn = us_radiobutton(tr("Single Scan"), rb_pltsscn, false);
   ...
   QLayout *lo_monfitg = us_checkbox(tr("Monitor Fit Graphically"), ck_monfitg, true);
   ...
   QLabel *lb_tbanner = us_banner(tr("NLSQ Fit Tuning Controls"));
   ...
   connect(pb_lnvsr2, SIGNAL(clicked()), this, SLOT(plot_two()));
   connect(pb_mwvsr2, SIGNAL(clicked()), this, SLOT(plot_three()));
   connect(pb_mwvscv, SIGNAL(clicked()), this, SLOT(plot_four()));

   // Status layout
   QLabel *lb_status = us_label(tr("Status:"));
   le_status = us_lineedit(tr("Waiting for Input - Please click on \"Fit\" to start"
                              " the fitting process..."));
...
void US_EqFitControl::start_fit() {
   ...
   le_status->setText(tr("Equilibrium ") + QString(cmeth[ nlsmeth ]) + tr(" fitting for ") + models[ modelx ] + "...");
   ...
   connect(fitwork, SIGNAL(work_progress(int)), this, SLOT(new_progress(int)));
   connect(fitwork, SIGNAL(work_complete()), this, SLOT(fit_completed()));
...
void US_EqFitControl::view_report() {
   ...
   ereporter->fit_report(fitpars, showgui, writerf, filename);
...
void US_EqFitControl::plot_residuals() {
   ...
   data_plot->setAxisTitle(QwtPlot::yLeft, tr("Optical Density Difference"));
   data_plot->setAxisTitle(QwtPlot::xBottom, tr("Radius^2 - Radius(ref)^2 (cm)"));
   ...
   for (int ii = 0; ii < scanfits.size(); ii++) {
      ...
      xmin = min(xmin, -xpad);
      QwtPlotCurve *lcurve = us_curve(data_plot, QString("RLine-%1").arg(scnn));

      lcurve->setStyle(QwtPlotCurve::Lines);
      lcurve->setPen(lnpen);
      lcurve->setSamples(xplot, yplot, nspts);

      QwtPlotCurve *scurve = us_curve(data_plot, QString("RSymb-%1").arg(scnn));
   ...
   if (plotgrpf == 5) {
      ...
      while (yoffs < yofmx) {
         int scnn = sns[ ksn++ ];
         QwtPlotCurve *zcurve = us_curve(data_plot, QString("RZero-%1").arg(scnn));
         ypzero[ 0 ] = yoffs;
         ypzero[ 1 ] = yoffs;
...
void US_EqFitControl::plot_overlays() {
   ...
   dataPlotClear(data_plot);
   data_plot->setTitle(
      (npscns == 1) ? tr("Overlays for fitted Scan %1").arg(ipscnn)
...
void US_EqFitControl::new_progress(int step) {
   ...
   le_inform->setText(tr("Iteration %1 has completed.").arg(fitpars.k_iter));
...
void US_EqFitControl::prepare_data() {
   ...
   for (int ii = 0; ii < ntscns; ii++) {
      ...
      for (int jj = 0; jj < nspts; jj++) { // Insure we have freshly calculated deltas for scan data
   ...
   if (rb_pltalld->isChecked()) { // Plot all
