...
US_EqFitControl::US_EqFitControl (QVector<EqScanFit> &a_scanfits,
                                  ...
                                  US_EqMath *a_emath,
                                  US_EqReporter *a_ereporter, int a_modelx,
                                  QStringList a_models, bool &a_fWidget,
                                  int &a_selscan)
    : US_WidgetsDialog (0, 0), scanfits (a_scanfits), runfit (a_runfit),
      edata (a_edata), emath (a_emath), ereporter (a_ereporter),
      modelx (a_modelx), models (a_models), fWidget (a_fWidget),
      selscan (a_selscan)
{
  qDebug () << "EFC: IN";
  ...
  dplot = new US_Plot (data_plot, tr ("Residuals"),
  ...
  QGridLayout *lo_pltgrp5
      = us_radiobutton (tr ("Groups of 5"), rb_pltgrp5, false);
  QGridLayout *lo_pltsscn
      = us_radiobutton (tr ("Single Scan"), rb_pltsscn, false);
  ...
  QLayout *lo_monfitg
      = us_checkbox (tr ("Monitor Fit Graphically"), ck_monfitg, true);
  ...
  le_status
      = us_lineedit (tr ("Waiting for Input - Please click on \"Fit\" to start"
                         " the fitting process..."));
...
US_EqFitControl::start_fit ()
{
  ...
  connect (fitwork, SIGNAL (work_progress (int)), this,
           SLOT (new_progress (int)));
...
void
US_EqFitControl::save_fit ()
{
  qDebug () << "SAVE_FIT";
}
// View a report on the fit
void
US_EqFitControl::view_report ()
{
  qDebug () << "VIEW_REPORT";
  bool showgui = true;
  bool writerf = true;
...
void
US_EqFitControl::plot_residuals ()
{
  ...
  for (int ii = 0; ii < scanfits.size (); ii++)
    {
      ...
      // qDebug() << "RPlot: ii scnn" << ii << scnn << " ipsc lpsc" << ipscnn
      // << lpscnn;
      ...
      // qDebug() << "RPlot: ii scnn" << ii << scnn << " jdsx jvsy" << jdsx <<
      // jvxy
      //    << " nspts" << nspts;
      ...
      for (int jj = 0; jj < nspts; jj++)
        {
          ...
          // qDebug() << "RPlot:  jj jvxy" << jj << jvxy << "xval" <<
          // scnf->xvs[jvxy];
      ...
      xmin = min (xmin, -xpad);
      QwtPlotCurve *lcurve
          = us_curve (data_plot, QString ("RLine-%1").arg (scnn));
      ...
      QwtPlotCurve *scurve
          = us_curve (data_plot, QString ("RSymb-%1").arg (scnn));
      ...
      while (yoffs < yofmx)
        {
          int scnn = sns[ksn++];
          QwtPlotCurve *zcurve
              = us_curve (data_plot, QString ("RZero-%1").arg (scnn));
          ypzero[0] = yoffs;
          ypzero[1] = yoffs;
...
US_EqFitControl::plot_overlays ()
{
  ...
  data_plot->setTitle ((npscns == 1)
                           ...
                           : tr ("Overlays for fitted Scans %1 - %2")
                                 .arg (ipscnn)
                                 .arg (lpscnn));
  data_plot->setAxisTitle (QwtPlot::yLeft, tr ("Optical Density Difference"));
  data_plot->setAxisTitle (QwtPlot::xBottom,
  ...
  for (int ii = 0; ii < scanfits.size (); ii++)
    {
      ...
      // qDebug() << "OPlot: ii scnn" << ii << scnn << " ipsc lpsc" << ipscnn
      // << lpscnn;
      if (!scnf->scanFit || scnn < ipscnn || scnn > lpscnn)
        continue;
...
// Update progress bar
void
US_EqFitControl::new_progress (int step)
{
