...
US_EqFitControl::US_EqFitControl(QVector<EqScanFit>& a_scanfits,
                                 ...
                                 US_EqMath* a_emath, US_EqReporter* a_ereporter,
                                 int a_modelx, QStringList a_models,
                                 bool& a_fWidget, int& a_selscan)
  ...
  QLabel* lb_fbanner =
      us_banner(tr("Nonlinear Least Squares\n"
...
void US_EqFitControl::save_fit() { qDebug() << "SAVE_FIT"; }
...
void US_EqFitControl::plot_residuals() {
  ...
  for (int ii = 0; ii < scanfits.size(); ii++) {
    ...
    // lpscnn;
    if (!scnf->scanFit || scnn < ipscnn || scnn > lpscnn) continue;
...
void US_EqFitControl::prepare_data() {
  ...
  for (int ii = 0; ii < ntscns; ii++) {
    ...
    if (scann >= ipscnn) kpscns++;

    if (kpscns == 1) ipscnn = scann;  // Initial plot scan number

    if (kpscns > 0 && kpscns < 6) lpscnn = scann;  // Last plot scan number

    if (kpscns == 6) jpscnn = scann;  // Next group-of-5 start scan number
    // qDebug() << "PrD: sn ipsc lpsc jpsc kpsc" << scann << ipscnn << lpscnn
    //    << jpscnn << kpscns;

    for (int jj = 0; jj < nspts;
         jj++) {  // Insure we have freshly calculated deltas for scan data
