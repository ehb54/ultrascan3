...
void
US_LongMessageBox::setTitle (const QString &title)
{
  setWindowTitle (title);
}
...
// Set new plain text
void
US_LongMessageBox::setText (const QString &message)
{
  textbox->setPlainText (message);
  adjustSize ();
}
...
// Set new rich text
void
US_LongMessageBox::setHtml (const QString &message)
{
...
// Close
void
US_LongMessageBox::close_diag (void)
{
  accept ();
}
...
US_LongMessageBox::adjustSize ()
{
  ...
  for (int ii = 0; ii < nline; ii++)
    { // find the longest line and save its index
