...
US_EqModelControl::US_EqModelControl (QVector<EqScanFit> &a_scanfits,
                                      EqRunFit &a_runfit,
                                      US_DataIO::EditedData *a_edata,
                                      int a_modelx, QStringList a_models,
                                      bool &a_mWidget, int &a_selscan)
    : US_WidgetsDialog (0, 0), scanfits (a_scanfits), runfit (a_runfit),
      edata (a_edata), modelx (a_modelx), models (a_models),
      mWidget (a_mWidget), selscan (a_selscan)
{
  ...
  QLabel *lb_grunpar = us_label (tr ("Global Run Parameters for Component: "));
  ...
  QHBoxLayout *lo_mwflbox
      = radiobox (lo_mwfloat, rb_mwfloat, lo_mwlock, rb_mwlock);
  ...
  QHBoxLayout *lo_vbflbox
      = radiobox (lo_vbfloat, rb_vbfloat, lo_vblock, rb_vblock);
  ...
  QHBoxLayout *lo_l1flbox
      = radiobox (lo_l1float, rb_l1float, lo_l1lock, rb_l1lock);
  ...
  QHBoxLayout *lo_l2flbox
      = radiobox (lo_l2float, rb_l2float, lo_l2lock, rb_l2lock);
  ...
  QHBoxLayout *lo_l3flbox
      = radiobox (lo_l3float, rb_l3float, lo_l3lock, rb_l3lock);
  ...
  QHBoxLayout *lo_l4flbox
      = radiobox (lo_l4float, rb_l4float, lo_l4lock, rb_l4lock);
  ...
  connect (ct_grunpar, SIGNAL (valueChanged (double)), this,
           SLOT (global_comp_changed (double)));
  connect (pb_vbar20, SIGNAL (clicked ()), this, SLOT (set_vbar ()));
  connect (rb_mwfloat, SIGNAL (toggled (bool)), this, SLOT (update_floats ()));
  connect (rb_vbfloat, SIGNAL (toggled (bool)), this, SLOT (update_floats ()));
  connect (rb_l1float, SIGNAL (toggled (bool)), this, SLOT (update_floats ()));
  connect (rb_l2float, SIGNAL (toggled (bool)), this, SLOT (update_floats ()));
  connect (rb_l3float, SIGNAL (toggled (bool)), this, SLOT (update_floats ()));
  connect (rb_l4float, SIGNAL (toggled (bool)), this, SLOT (update_floats ()));

  // Local layout
  lb_lbanner
      = us_banner (tr ("Local Scan Parameters for Scan %1").arg (selscan));
  ...
  QHBoxLayout *lo_blflbox
      = radiobox (lo_blfloat, rb_blfloat, lo_bllock, rb_bllock);
  ...
  QLabel *lb_lrunpar = us_label (tr ("Local Scan Parameters for Component: "));
  ...
  QHBoxLayout *lo_amflbox
      = radiobox (lo_amfloat, rb_amfloat, lo_amlock, rb_amlock);
  ...
  QLayout *lo_inclfit
      = us_checkbox (tr ("Include this Scan in the Fit:"), ck_inclfit, false);
  ...
  connect (ct_lrunpar, SIGNAL (valueChanged (double)), this,
           SLOT (local_comp_changed (double)));
  connect (ct_scansel, SIGNAL (valueChanged (double)), this,
           SLOT (scan_changed (double)));
  connect (rb_blfloat, SIGNAL (toggled (bool)), this, SLOT (update_floats ()));
  connect (rb_amfloat, SIGNAL (toggled (bool)), this, SLOT (update_floats ()));
  connect (pb_plenapp, SIGNAL (clicked ()), this, SLOT (pathlen_applyto ()));
  connect (pb_density, SIGNAL (clicked ()), this, SLOT (set_density ()));
  connect (pb_densapp, SIGNAL (clicked ()), this, SLOT (density_applyto ()));
  connect (pb_extiapp, SIGNAL (clicked ()), this, SLOT (extinct_applyto ()));
...
void
US_EqModelControl::new_scan (int scannbr)
{
...
void
US_EqModelControl::new_components ()
{
...
void
US_EqModelControl::set_float (bool floated)
{
...
void
US_EqModelControl::scan_changed (double value)
{
...
void
US_EqModelControl::global_comp_changed (double value)
{
...
void
US_EqModelControl::local_comp_changed (double value)
{
...
void
US_EqModelControl::update_sigma (void)
{
...
void
US_EqModelControl::update_floats (void)
{
...
void
US_EqModelControl::update_gvals (void)
{
  int compx = (int)ct_grunpar->value () - 1;
...
void
US_EqModelControl::update_lvals (void)
{
  int scanx = selscan - 1;
  int compx = (int)ct_lrunpar->value () - 1;
...
void
US_EqModelControl::connect_global (void)
{
...
void
US_EqModelControl::connect_local (void)
{
...
void
US_EqModelControl::disconnect_global (void)
{
...
void
US_EqModelControl::disconnect_local (void)
{
...
void
US_EqModelControl::pathlen_applyto (void)
{
...
void
US_EqModelControl::density_applyto (void)
{
...
void
US_EqModelControl::extinct_applyto (void)
{
...
QHBoxLayout *
US_EqModelControl::radiobox (QGridLayout *lo_rb1, QRadioButton *rbtn1,
...
void
US_EqModelControl::set_vbar ()
{
  int dbdisk = runfit.dbdisk;
  US_AnalyteGui *adiag = new US_AnalyteGui (true, QString (), dbdisk);
  connect (adiag, SIGNAL (valueChanged (US_Analyte)), this,
           SLOT (assignVbar (US_Analyte)));
  adiag->exec ();
  qApp->processEvents ();
}
...
void
US_EqModelControl::set_density ()
{
  US_Buffer buff;
  int dbdisk = runfit.dbdisk;
  US_BufferGui *bdiag = new US_BufferGui (true, buff, dbdisk);
  connect (bdiag, SIGNAL (valueChanged (US_Buffer)), this,
           SLOT (assignDensity (US_Buffer)));
  bdiag->exec ();
  qApp->processEvents ();
}
...
void
US_EqModelControl::assignVbar (US_Analyte analyte)
{
  int compx = (int)ct_lrunpar->value () - 1;
...
void
US_EqModelControl::assignDensity (US_Buffer buffer)
{
  int scanx = selscan - 1;
