...
US_EqModelControl::US_EqModelControl(
   QVector<EqScanFit> &a_scanfits, EqRunFit &a_runfit, US_DataIO::EditedData *a_edata, int a_modelx,
   QStringList a_models, bool &a_mWidget, int &a_selscan) :
    US_WidgetsDialog(0, 0), scanfits(a_scanfits), runfit(a_runfit), edata(a_edata), modelx(a_modelx), models(a_models),
   ...
   QHBoxLayout *lo_mwflbox = radiobox(lo_mwfloat, rb_mwfloat, lo_mwlock, rb_mwlock);
   ...
   QHBoxLayout *lo_vbflbox = radiobox(lo_vbfloat, rb_vbfloat, lo_vblock, rb_vblock);
   ...
   QHBoxLayout *lo_l1flbox = radiobox(lo_l1float, rb_l1float, lo_l1lock, rb_l1lock);
   ...
   QHBoxLayout *lo_l2flbox = radiobox(lo_l2float, rb_l2float, lo_l2lock, rb_l2lock);
   ...
   QHBoxLayout *lo_l3flbox = radiobox(lo_l3float, rb_l3float, lo_l3lock, rb_l3lock);
   ...
   QHBoxLayout *lo_l4flbox = radiobox(lo_l4float, rb_l4float, lo_l4lock, rb_l4lock);
   ...
   connect(ct_grunpar, SIGNAL(valueChanged(double)), this, SLOT(global_comp_changed(double)));
   ...
   // Local layout
   lb_lbanner = us_banner(tr("Local Scan Parameters for Scan %1").arg(selscan));
   ...
   QHBoxLayout *lo_blflbox = radiobox(lo_blfloat, rb_blfloat, lo_bllock, rb_bllock);
   ...
   QHBoxLayout *lo_amflbox = radiobox(lo_amfloat, rb_amfloat, lo_amlock, rb_amlock);
   ...
   QLayout *lo_inclfit = us_checkbox(tr("Include this Scan in the Fit:"), ck_inclfit, false);
   ...
   connect(ct_lrunpar, SIGNAL(valueChanged(double)), this, SLOT(local_comp_changed(double)));
   connect(ct_scansel, SIGNAL(valueChanged(double)), this, SLOT(scan_changed(double)));
...
void US_EqModelControl::update_sigma(void) {
   ...
   double sigma = (1.0 - vbar * solution.density_tb) * omega_s * molecwt / (2.0 * R_GC * tempk);
...
QHBoxLayout *US_EqModelControl::radiobox(
   QGridLayout *lo_rb1, QRadioButton *rbtn1, QGridLayout *lo_rb2, QRadioButton *rbtn2) {
...
void US_EqModelControl::set_vbar() {
   ...
   connect(adiag, SIGNAL(valueChanged(US_Analyte)), this, SLOT(assignVbar(US_Analyte)));
   adiag->exec();
   qApp->processEvents();
}
...
void US_EqModelControl::set_density() {
   ...
   connect(bdiag, SIGNAL(valueChanged(US_Buffer)), this, SLOT(assignDensity(US_Buffer)));
   bdiag->exec();
   qApp->processEvents();
}
