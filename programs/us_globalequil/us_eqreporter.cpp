...
void
US_EqReporter::scan_diagnostics ()
{
  DbgLv (1) << "SCAN_DIAGS()";
  ...
  for (int jes = 0; jes < scedits.size (); jes++)
    {
      ...
      DbgLv (1) << "SDiag:  radlo radhi" << radlo << radhi << " rs re"
                ...
                << sdata->radius (0)
                << sdata->radius (sdata->pointCount () - 1);
...
// Check scan fit
bool
US_EqReporter::check_scan_fit (int modelx)
{
              ...
              if (runfit.nbr_assocs > 0)
                {
                  if (scanfits[jes].extincts[jj] == 0.0)
                    {
                      rs += tr ("The extinction coefficient for component %1 "
                                "is"
...
// Create fit report
QString
US_EqReporter::fit_report (FitCtrlPar &fitpars, bool opengui, bool wrreport,
                           QString &filename)
{
  QString rs;
  ...
  for (int ii = 0; ii < scanfits.size (); ii++)
    {
      ...
      rs += tr ("Density setting:                        %1 g/ccm\n")
                .arg (density);
      rs += tr ("Temperature setting:                    %1").arg (tempera)
            + DEGC + "\n";
      ...
      for (int jj = 0; jj < runfit.nbr_comps; jj++)
        {
          ...
          double vbar20
              = US_Math2::adjust_vbar20 (runfit.vbar_vals[jj], tempera);
...
int
US_EqReporter::index_radius (US_DataIO::EditedData *edat, double radius)
{
...
QString
US_EqReporter::centerInLine (const QString &titl_text, int linelen,
...
QString
US_EqReporter::scanInfoHeader (int jes, int jdx)
{
...
int
US_EqReporter::maxLineWidth (QFontMetrics &fm, const QString &repstr)
{
