...
US_EqReporter::US_EqReporter(QVector<US_DataIO::EditedData>& dataList,
                             ...
                             QVector<EqScanFit>& scanfits, EqRunFit& runfit,
                             QWidget* wparent)
    : QObject(),
      dataList(dataList),
      scedits(scedits),
...
void US_EqReporter::scan_diagnostics() {
  ...
  rs +=
      tr("Below is a listing of the ratios of slopes in the endpoints"
         " of each indicated\n");
  rs +=
      tr("scan. If the ratios are less than 30, then there is little"
         " information content\n");
  rs +=
      tr("in the scan and chances are that the experiment was"
         " improperly set up\nand should be repeated.\n\n");
  rs +=
      tr("Additional warnings will be generated if the scan does not"
         " contain enough\n");
  rs +=
      tr("data points or if the experimenter did not use the majority"
         " of the linear\n");
  rs +=
      tr("absorbance range available (at least 0.6 OD between 0.0 OD"
         " and 0.9 OD).\n\n");
  rs +=
      tr("These warnings are for your information only; they have no"
         " effect on the\n");
  rs +=
      tr("rest of the program, since there are valid exceptions to"
         " these warnings\n");
  rs +=
      tr("when including such scans is appropriate. Please refer to"
         " the global\n");
  ...
  for (int jes = 0; jes < scedits.size(); jes++) {
    ...
    if (npts > dimvs) {  // If need be (unlikely), resize the work x,y vectors
      dimvs = npts + 10;
      xvec.resize(dimvs);
      yvec.resize(dimvs);
      xx = xvec.data();
    ...
    DbgLv(1) << "SDiag: jes" << jes << "ivstx ivenx npts" << ivstx << ivenx
             << npts;
    DbgLv(1) << "SDiag:  radlo radhi" << radlo << radhi << " rs re"
             << sdata->radius(ivstx) << sdata->radius(ivenx) << " r0 rn"
             << sdata->radius(0) << sdata->radius(sdata->pointCount() - 1);

    int nwarns = 0;  // Initialize for scan analysis

    if (npts > 50) {  // If sufficient points, analyze slopes and ratios
      int nspts = npts / 5;
      int knt = 0;
      double slope1 = 0.0;
      double slope2 = 0.0;
      ...
      for (int jj = ivstx; jj < ivstx + nspts;
           jj++) {  // Accumulate work arrays of beginning points
      ...
      for (int jj = ivenn - nspts; jj < ivenn;
           jj++) {  // Accumulate work arrays of ending points
        xx[knt] = scanfits[jes].xvs[jj];
        yy[knt++] = scanfits[jes].yvs[jj];
      }
      ...
      DbgLv(1) << "SDiag:    y0 y1 ym yn" << yy[0] << yy[1] << yy[knt - 2]
               << yy[knt - 1];
      ...
      if (ratio > 0.0 && ratio < 1.5) {
        rs +=
            tr("Warning: The ratio is very small - there is"
               " probably not enough\n"
      ...
      if (slope1 < 0.0) {
        rs +=
            tr("Warning: The start point slope for this scan"
               " is negative!\n"
      ...
      if (slope2 < 0.0) {
        rs +=
            tr("Warning: The end point slope for this scan"
               " is negative!\n"
               "Possible reasons: excessive noise in the data,"
               " or time invariant noise\n"
               "from interference data has not been subtracted.\n\n");
      ...
      if (yend < 0.6) {
        rs += tr("Warning: This scan's maximum absorbance is only "
                 " %1 OD.\n")
                  .arg(yend);
        rs +=
            tr("This is lower than the linear range of the XL-A"
    ...
    if (nwarns > 2)
      rs +=
          tr("Please check the scan to make sure it is appropriate"
             " for inclusion in a global fit!\n");

    rs += "\n";

    if (nwarns == 0) {  // Mark scan as fit/non-excluded
  ...
  if (scprobs) {  // Pop up dialog warning of potential problems
    QMessageBox::warning(
        wparent, tr("Scan Problem(s)"),
  ...
  US_Editor* ediag = new US_Editor(US_Editor::DEFAULT, true, "*.res", wparent);
  ediag->setWindowTitle(tr("Scan Diagnostics"));
  QFont efont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize() - 2);
...
bool US_EqReporter::check_scan_fit(int modelx) {
  ...
  while (ffitx < scanfits.size() && !scanfits[ffitx].scanFit) ffitx++;

  DbgLv(1) << "  EqRep:CSF: ffitx" << ffitx;
  QString rs;

  rs +=
      tr("In order to assure that the proper parameters are used"
         " for the fitting process,\n");
  rs +=
      tr("the following information has been compiled about the"
         " components in your\n");
  rs +=
      tr("model and the scans included in the fit --\n\n"
      ...
      if (runfit.mw_vals[jj] == 0.0) {
        rs += tr("Although the molecular weight parameter for"
                 " component %1 has been floated,\n")
                  .arg(cnum);
        rs +=
            tr("the value for this parameter is equal to zero"
               " - fitting aborted!\n");
        critical = true;
      }
      ...
      if (runfit.mw_rngs[jj] == 0.0) {
        rs += tr("Although the molecular weight parameter for"
                 " component %1 has been floated,\n")
                  .arg(cnum);
        rs +=
            tr("the range for this parameter is equal to zero"
               " - fitting aborted!\n");
        critical = true;
      }
    }
    ...
    if ((int)(1000000.0 * runfit.vbar_vals[jj] + 0.5) == 720000) {
      rs += tr("The vbar value for component %1 has been"
               " left at 0.72,\n")
                .arg(cnum);
      rs +=
          tr("which is the default value - are you sure"
  ...
  if (modelx > 3) {
    ...
    DbgLv(1) << "  EqRep:CSF: fx extsz" << ffitx
    ...
    for (int jj = 0; jj < scanfits.size(); jj++) {
      if (scanfits[jj].scanFit) {
        if (scanfits[jj].wavelen != test_waveln) same_waveln = false;

        if (scanfits[jj].extincts[0] != test_extinc) same_extinc = false;
      }
    }
    ...
    if (scanfits[jes].scanFit) {
      for (int jj = 0; jj < runfit.nbr_comps; jj++) {
        ...
        if (scanfits[jes].amp_fits[jj]) {
          if (scanfits[jes].amp_vals[jj] == 0.0) {
            rs +=
                tr("Although the amplitude for component %1 has"
      ...
      if (scanfits[jes].baseln_fit) {
        if (scanfits[jes].baseln_rng == 0.0) {
          rs +=
              tr("Although the baseline for this scan has"
                 " been floated,\nthe range for this parameter"
                 " is equal to zero - fitting aborted!\n");
          critical = true;
        }
        ...
        rs +=
            tr("The baseline for this scan has been fixed -"
               " are you sure you want to do that?\n");
      }

      if ((int)(1000000.0 * scanfits[jes].density + 0.5) == 998234) {
        rs +=
            tr("The density setting corresponds to pure water -"
...
QString US_EqReporter::fit_report(FitCtrlPar& fitpars, bool opengui,
  ...
  runfit.runs_vari =
      (totprod2 * (totprod2 - totsum)) / (sq(totprod) * (totsum - 1.0));
  ...
  rs += tr("Number of Runs  (corrected):          %1 (%2 %)\n")
            .arg(runfit.nbr_runs)
            .arg(runfit.runs_percent);
  rs +=
      tr("Expected Number of Runs  (corrected): %1\n").arg(runfit.runs_expect);
  ...
  if (runfit.runs_percent <= 30.0) {
    ...
    rs +=
        tr("the experimental data, or the fitting process has not yet"
           " converged. Please try to reduce\n");
    rs +=
        tr("the variance by additional nonlinear least-squares"
    ...
    rs +=
        tr("This fit can be used for a Monte Carlo Analysis"
           " with reservations.\n");
  }
  ...
  for (int ii = 0; ii < scanfits.size(); ii++) {
    ...
    rs += tr("Temperature setting:                    %1").arg(tempera) + DEGC +
          "\n";
    ...
    for (int jj = 0; jj < runfit.nbr_comps; jj++) {
      ...
      double vbar20 = US_Math2::adjust_vbar20(runfit.vbar_vals[jj], tempera);
    ...
    double varirun =
        (runpro2 * (runpro2 - runsum)) / (sq(runsum) * (runsum - 1.0));
    rs +=
        tr("Expected Number of Runs  (corrected):   %1\n").arg(qRound(expectr));
    rs += tr("Run Variance  (corrected):              %1\n").arg(varirun);
  ...
  if (opengui) {
    US_Editor* ediag =
        new US_Editor(US_Editor::DEFAULT, true, "*.res", wparent);
  ...
  if (wrreport) {
    filename = US_Settings::reportDir() + "/" + dataList[0].runID + "/globeq." +
               runfit.projname + ".eqfit.rpt";
    QFile drf(filename);
...
QString US_EqReporter::scanInfoHeader(int jes, int jdx) {
  return ("\n" + asters + tr("Information for Scan ") +
          QString::number(jes + 1) + tr(",  Cell ") + dataList[jdx].cell +
          tr(", Channel ") + dataList[jdx].channel + tr(", Wavelength ") +
          dataList[jdx].wavelength + " nm,  " +
          QString::number(scanfits[jes].rpm) + tr(" rpm") + "\n   [  " +
