...
US_EqReporter::US_EqReporter(
   QVector<US_DataIO::EditedData> &dataList, QVector<ScanEdit> &scedits, QVector<EqScanFit> &scanfits, EqRunFit &runfit,
   QWidget *wparent) :
    QObject(), dataList(dataList), scedits(scedits), scanfits(scanfits), runfit(runfit), wparent(wparent) {
...
void US_EqReporter::scan_diagnostics() {
   ...
   for (int jes = 0; jes < scedits.size(); jes++) {
      ...
      DbgLv(1) << "SDiag:  radlo radhi" << radlo << radhi << " rs re" << sdata->radius(ivstx) << sdata->radius(ivenx)
      ...
      if (npts > 50) { // If sufficient points, analyze slopes and ratios
         ...
         for (int jj = ivstx; jj < ivstx + nspts; jj++) { // Accumulate work arrays of beginning points
         ...
         for (int jj = ivenn - nspts; jj < ivenn; jj++) { // Accumulate work arrays of ending points
            xx[ knt ] = scanfits[ jes ].xvs[ jj ];
            yy[ knt++ ] = scanfits[ jes ].yvs[ jj ];
         }
   ...
   if (!folder.exists(repdir)) {
      if (!folder.mkdir(repdir)) {
         QMessageBox::warning(wparent, tr("File Error"), tr("Could not create the directory:\n") + basedir + "/" + repdir);
         return;
      }
   }
   ...
   if (!drf.open(QIODevice::WriteOnly | QIODevice::Text)) {
      QMessageBox::warning(wparent, tr("File Error"), tr("Unable to open the file:\n") + filename);
      return;
   }
...
// Create fit report
QString US_EqReporter::fit_report(FitCtrlPar &fitpars, bool opengui, bool wrreport, QString &filename) {
   QString rs;
   ...
   for (int ii = 0; ii < runfit.nbr_comps; ii++) {
      ...
      for (int jj = 0; jj < runfit.nbr_assocs; jj++) {
         ...
         rs += tr("  Association (Dissociation) Constant %1: %2").arg(jj + 1).arg(eqval);
   ...
   runfit.runs_vari = (totprod2 * (totprod2 - totsum)) / (sq(totprod) * (totsum - 1.0));
   ...
   for (int ii = 0; ii < scanfits.size(); ii++) {
      ...
      double varirun = (runpro2 * (runpro2 - runsum)) / (sq(runsum) * (runsum - 1.0));
      rs += tr("Expected Number of Runs  (corrected):   %1\n").arg(qRound(expectr));
      rs += tr("Run Variance  (corrected):              %1\n").arg(varirun);
   ...
   if (opengui) {
      ...
      QFont efont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 2);
      ediag->e->setFont(efont);
      ediag->e->setText(rs);
      QFontMetrics fm(efont);
      int dwid = maxLineWidth(fm, rs) + fm.width("WWW");
   ...
   if (wrreport) {
      filename = US_Settings::reportDir() + "/" + dataList[ 0 ].runID + "/globeq." + runfit.projname + ".eqfit.rpt";
      QFile drf(filename);
...
QString US_EqReporter::centerInLine(const QString &titl_text, int linelen, bool rightPad, const QChar end_char) {
...
QString US_EqReporter::scanInfoHeader(int jes, int jdx) {
   return (
      "\n" + asters + tr("Information for Scan ") + QString::number(jes + 1) + tr(",  Cell ") + dataList[ jdx ].cell
      + tr(", Channel ") + dataList[ jdx ].channel + tr(", Wavelength ") + dataList[ jdx ].wavelength + " nm,  "
