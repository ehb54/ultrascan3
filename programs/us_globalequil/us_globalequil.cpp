...
DbgLv(1) << "EdataPlot: radl radr" << radl << radr
   ...
   double vlo  = 9e+10;
   double vhi  = -9e+10;
   int    krpt = std::min( nrpts, scanfits[ sscanx ].stop_ndx + 1 );
...
void US_GlobalEquil::assign_scanfit()
{
   ...
   EqScanFit   scanfit;
   QStringList channs;
   int         ncomp = std::max( 1, runfit.nbr_comps );
   int         mcomp = std::max( 4, ncomp );
...
int US_GlobalEquil::index_od_limit( EqScanFit& scanfit, double odlim )
{
   ...
   for ( int jj = scanfit.start_ndx; jj < scanfit.stop_ndx + 1; jj++ )
   {
      ...
      {
         stopx = std::max( jj - 1, scanfit.start_ndx );
         break;
      }
   }
