...
void
US_GlobalEquil::new_scan (int newscan)
{
  ...
  for (int jd = 0; jd < dataList.size (); jd++)
    {
      ...
      QString s_emsg;
      QString solID;
      US_SolutionVals::values (dbP, &dataList[jd], solID, s_vbar, s_dens,
                               s_visc, s_comp, s_manu, s_emsg);
...
void
US_GlobalEquil::view_report (void)
{
  DbgLv (1) << "VIEW_REPORT()";
}
...
void
US_GlobalEquil::scan_diags (void)
{
  DbgLv (1) << "SCAN_DIAGS()";
...
void
US_GlobalEquil::check_scan_fit (void)
{
  runfit.modlname = models[modelx];
  DbgLv (1) << "CHECK_SCAN_FIT()";
  if (ereporter == 0) // Create reporter (1st time)
    ereporter = new US_EqReporter (dataList, scedits, scanfits, runfit, this);
  DbgLv (1) << " CkScFit:  sz - sced scnf" << scedits.size ()
            << scanfits.size ();
...
void
US_GlobalEquil::conc_histogram (void)
{
...
void
US_GlobalEquil::reset_scan_lims (void)
{
...
void
US_GlobalEquil::load_model (void)
{
  DbgLv (1) << "LOAD_MODEL()";
}
void
US_GlobalEquil::new_project_name (const QString &newpname)
{
...
// Select the model
void
US_GlobalEquil::select_model (void)
{
...
void
US_GlobalEquil::model_control (void)
{
  ...
  if (model_widget)
  ...
  else
    {
      ...
      emodctrl = new US_EqModelControl (scanfits, runfit, edata, modelx,
                                        models, model_widget, sscann);

      connect (emodctrl, SIGNAL (update_scan (int)), this,
               SLOT (new_scan (int)));
...
void
US_GlobalEquil::fitting_control (void)
{
  ...
  if (fit_widget)
  ...
  else
    {
      ...
      if (ereporter == 0) // Create reporter (1st time)
        ereporter
            = new US_EqReporter (dataList, scedits, scanfits, runfit, this);

      efitctrl
          = new US_EqFitControl (scanfits, runfit, edata, emath, ereporter,
...
void
US_GlobalEquil::load_fit (void)
{
  DbgLv (1) << "LOAD_FIT()";
}
void
US_GlobalEquil::monte_carlo (void)
{
  DbgLv (1) << "MONTE_CARLO()";
}

void
US_GlobalEquil::float_params (void)
{
...
void
US_GlobalEquil::init_params (void)
{
  DbgLv (1) << "INIT_PARAMS()";
...
void
US_GlobalEquil::scan_select (double newscan)
{
  sscann = (int)newscan;
  sscanx = sscann - 1;
  ...
  DbgLv (1) << " GE:ClItem: signal_mc model_widget" << signal_mc
            << model_widget;
  if (signal_mc)
  ...
  DbgLv (1) << " GE:ScSel: signal_mc model_widget" << signal_mc
            << model_widget;
  if (signal_mc)
    { // Signalling of model control is enabled
      if (model_widget)
...
void
US_GlobalEquil::update_disk_db (bool dbaccess)
{
  if (dbaccess)
    { // Database
      dkdb_cntrls->set_db ();
...
void
US_GlobalEquil::doubleClickedItem (QTableWidgetItem *item)
{
  int row = item->row ();
...
bool
US_GlobalEquil::findData (QString trip, double drpm, int &jdx, int &jrx)
{
  bool found = false;
  jdx = -1;
  jrx = -1;
...
US_GlobalEquil::edata_plot ()
{
  ...
  equil_plot->setTitle (tr ("Run: ") + runID + tr ("  Edit: ") + editID + "\n"
                        ...
                        + tr (",  ") + QString::number (drpm)
                        + tr (" rpm,  Scan ") + QString::number (sscann));
...
void
US_GlobalEquil::edited_plot (void)
{
...
void
US_GlobalEquil::pMouseDown (const QwtDoublePoint &p)
{
  mMoved = false;
  mDown = true;
  sRadLo = p.x ();
...
void
US_GlobalEquil::pMouseUp (const QwtDoublePoint &p)
{
...
void
US_GlobalEquil::pMouseMoved (const QwtDoublePoint &p)
{
  if (!mDown)
    return;
...
int
US_GlobalEquil::index_radius (US_DataIO::EditedData *edat, double radius)
{
...
void
US_GlobalEquil::od_limit_changed (const QString &newlim)
{
...
void
US_GlobalEquil::update_limit (double odlim)
{
  od_limit = odlim;
...
int
US_GlobalEquil::index_od_limit (EqScanFit &scanfit, double odlim)
{
