...
US_GlobalEquil::US_GlobalEquil() : US_Widgets(true) {
  ...
  QLabel* lb_equiscns =
      us_banner(tr("List of available Equilibrium"
                   " Scans:"));
  ...
  // Model Fit elements
  QLabel* lb_mfitinfo =
      us_banner(tr("Model and Fitting"
                   " Information:"));
  ...
  eplot = new US_Plot(equil_plot, tr("Experiment Equilibrium Data"),
                      tr("Radius (cm)"), tr("Absorbance (280 nm)"));
  us_grid(equil_plot);
  ...
  te_status->setText(
      tr("Please select an edited Equilibrium"
  ...
  QLabel* lb_mxfnotes =
      us_label(tr("(set to zero to inactivate"
                  " high conc. limits)"));
...
void US_GlobalEquil::load(void) {
  ...
  US_DataLoader* dialog =
      new US_DataLoader(dataLatest, dbdisk, rawList, dataList, triples,
                        workingDir, QString("equilibrium"));
  ...
  for (int jd = 0; jd < dataList.size(); jd++) {
    ...
    QString s_emsg;
    QString solID;
    US_SolutionVals::values(dbP, &dataList[jd], solID, s_vbar, s_dens, s_visc,
                            s_comp, s_manu, s_emsg);
  ...
  for (int jd = 0; jd < dataList.size();
       jd++) {  // Loop to get scans from the data sets
    edata = &dataList[jd];
    QString triple = triples[jd];
    QString tdesc = edata->description;

    for (int jr = 0; jr < edata->speedData.size();
         jr++) {  // Add a table entry for each speed step of each triple
      ...
      for (int js = iscn; js < iscn + kscn; js++) {
        ...
        DbgLv(1) << "  jsscn jd js" << jsscn << jd << js << "  sRadLo sRadHi"
                 << sRadLo << sRadHi;
  ...
  connect(tw_equiscns, SIGNAL(itemDoubleClicked(QTableWidgetItem*)), this,
          SLOT(doubleClickedItem(QTableWidgetItem*)));
  connect(tw_equiscns, SIGNAL(itemSelectionChanged()), this,
          SLOT(itemRowChanged()));

  te_status->setText(
      tr("To edit (exclude points):  Ctrl-click-hold,"
...
void US_GlobalEquil::view_report(void) { DbgLv(1) << "VIEW_REPORT()"; }
...
void US_GlobalEquil::load_model(void) { DbgLv(1) << "LOAD_MODEL()"; }
...
void US_GlobalEquil::select_model(void) {
  ...
  if (modelx >= 0) {
    ...
    if (na == 4)
      DbgLv(1) << "   par1-4: " << aud_params[0] << aud_params[1]
    ...
    if (model_widget) emodctrl->new_components();
...
void US_GlobalEquil::model_control(void) {
  ...
  if (model_widget) {
    ...
    emodctrl = new US_EqModelControl(scanfits, runfit, edata, modelx, models,
                                     model_widget, sscann);
...
void US_GlobalEquil::fitting_control(void) {
  ...
  if (fit_widget) {
    ...
    if (emath == 0) emath = new US_EqMath(dataList, scedits, scanfits, runfit);
    ...
    efitctrl = new US_EqFitControl(scanfits, runfit, edata, emath, ereporter,
...
void US_GlobalEquil::load_fit(void) { DbgLv(1) << "LOAD_FIT()"; }
void US_GlobalEquil::monte_carlo(void) { DbgLv(1) << "MONTE_CARLO()"; }

void US_GlobalEquil::float_params(void) {
  DbgLv(1) << "FLOAT_PARAMS()";
  float_all();

  if (model_widget) emodctrl->set_float(true);
}
...
void US_GlobalEquil::init_params(void) {
  ...
  for (int ii = 0; ii < scanfits.size(); ii++)
    if (scanfits[ii].scanFit && fitx < 0) fitx = ii;
  ...
  if (runfit.mw_vals[0] <=
      0.0) {  // Calculate the molecular weight value the 1st time
...
bool US_GlobalEquil::findData(QString trip, double drpm, int& jdx, int& jrx) {
  ...
  while (++jdx < dataList.size()) {  // Search the data set list
    ...
    if (trip == triples[jdx]) {  // If we are at the right triple, examine the
                                 // speed data

      while (++jrx <
             dataList[jdx]
                 .speedData.size()) {  // Search the speed steps in this triple

        if (dataList[jdx].speedData[jrx].speed ==
            drpm) {  // Right speed in right triple:  get data and mark found
          edata = &dataList[jdx];
...
void US_GlobalEquil::edata_plot() {
  ...
  dataPlotClear(equil_plot);
  equil_plot->setTitle(tr("Run: ") + runID + tr("  Edit: ") + editID + "\n" +
  ...
  DbgLv(1) << "EdataPlot: radl radr" << radl << radr << " edited"
           << scedits[sscanx].edited;
  ...
  DbgLv(1) << "EdataPlot: dr0 drn" << edata->radius(0)
           << edata->radius(nrpts - 1);
...
void US_GlobalEquil::edited_plot(void) {
  ...
  for (int jj = 0; jj < vecknt; jj++) {
    if (ru[jj] > sRadMv) {  // Save count to position where radius is beyond
                            // mouse position
      countu = jj;
      break;
    }
  ...
  if (mLowerH) {  // Mouse in lower half:  edited at beginning; unedited at
                  // division
    counte = countu;
...
void US_GlobalEquil::pMouseMoved(const QwtDoublePoint& p) {
  ...
  if (sRadMv > cRadHi || sRadMv < cRadLo) return;
...
void US_GlobalEquil::assign_scanfit() {
  if (scanfits.size() > 0) return;  // Only create base scanfits array one time
  ...
  DbgLv(1) << "AsnSF: points xvs0 xvsN" << scanfit.points << scanfit.xvs[0]
           << scanfit.xvs[scanfit.points - 1];
}
...
void US_GlobalEquil::setup_runfit() {
  ...
  if (ds_vbar20s.size() > 0) dvval = ds_vbar20s[0];
...
int US_GlobalEquil::index_radius(US_DataIO::EditedData* edat, double radius) {
  ...
  while (++r_index < l_index) {
    if (radius <= edat->radius(r_index)) break;
  }
...
void US_GlobalEquil::float_all() {
  for (int jj = 0; jj < runfit.nbr_comps; jj++) runfit.mw_fits[jj] = true;

  for (int jj = 0; jj < runfit.nbr_assocs; jj++) runfit.eq_fits[jj] = true;
...
void US_GlobalEquil::fix_all() {
  for (int jj = 0; jj < runfit.nbr_comps; jj++) runfit.mw_fits[jj] = false;

  for (int jj = 0; jj < runfit.nbr_assocs; jj++) runfit.eq_fits[jj] = false;
