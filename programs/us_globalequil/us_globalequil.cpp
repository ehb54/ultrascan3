...
US_GlobalEquil::US_GlobalEquil() : US_Widgets(true) {
   ...
   QLayout *lo_edlast = us_checkbox(tr("Lastest Data Edit"), ck_edlast, true);
   ...
   connect(pb_loadExp, SIGNAL(clicked()), SLOT(load()));
   connect(pb_details, SIGNAL(clicked()), SLOT(details()));
   connect(pb_view, SIGNAL(clicked()), SLOT(view_report()));
   connect(pb_unload, SIGNAL(clicked()), SLOT(unload()));
   connect(pb_scdiags, SIGNAL(clicked()), SLOT(scan_diags()));
   connect(pb_ckscfit, SIGNAL(clicked()), SLOT(check_scan_fit()));
   connect(pb_conchist, SIGNAL(clicked()), SLOT(conc_histogram()));
   connect(pb_resetsl, SIGNAL(clicked()), SLOT(reset_scan_lims()));
   connect(le_prjname, SIGNAL(textChanged(const QString &)), SLOT(new_project_name(const QString &)));
   ...
   tw_equiscns->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize()));
   ...
   connect(pb_selModel, SIGNAL(clicked()), SLOT(select_model()));
   connect(pb_modlCtrl, SIGNAL(clicked()), SLOT(model_control()));
   connect(pb_fitcntrl, SIGNAL(clicked()), SLOT(fitting_control()));
   connect(pb_loadFit, SIGNAL(clicked()), SLOT(load_fit()));
   connect(pb_monCarlo, SIGNAL(clicked()), SLOT(monte_carlo()));
   ...
   connect(pb_floatPar, SIGNAL(clicked()), SLOT(float_params()));
   connect(pb_initPars, SIGNAL(clicked()), SLOT(init_params()));
   connect(pb_help, SIGNAL(clicked()), SLOT(help()));
   connect(pb_close, SIGNAL(clicked()), SLOT(close_all()));
   connect(ct_scselect, SIGNAL(valueChanged(double)), SLOT(scan_select(double)));
   ...
   eplot = new US_Plot(equil_plot, tr("Experiment Equilibrium Data"), tr("Radius (cm)"), tr("Absorbance (280 nm)"));
   us_grid(equil_plot);
   ...
   connect(le_mxfringe, SIGNAL(textChanged(const QString &)), SLOT(od_limit_changed(const QString &)));
...
void US_GlobalEquil::load(void) {
   ...
   dbdisk = dkdb_cntrls->db() ? US_Disk_DB_Controls::DB : US_Disk_DB_Controls::Disk;

   // Open a dialog to select and load data
   US_DataLoader *dialog
      = new US_DataLoader(dataLatest, dbdisk, rawList, dataList, triples, workingDir, QString("equilibrium"));
   ...
   for (int jd = 0; jd < dataList.size(); jd++) {
      ...
      QString s_emsg;
      QString solID;
      US_SolutionVals::values(dbP, &dataList[ jd ], solID, s_vbar, s_dens, s_visc, s_comp, s_manu, s_emsg);
   ...
   for (int jd = 0; jd < dataList.size(); jd++) { // Loop to get scans from the data sets
      edata = &dataList[ jd ];
      QString triple = triples[ jd ];
      QString tdesc = edata->description;

      for (int jr = 0; jr < edata->speedData.size(); jr++) { // Add a table entry for each speed step of each triple
         ...
         for (int js = iscn; js < iscn + kscn; js++) {
            ...
            DbgLv(1) << "  jsscn jd js" << jsscn << jd << js << "  sRadLo sRadHi" << sRadLo << sRadHi;
   ...
   connect(tw_equiscns, SIGNAL(itemDoubleClicked(QTableWidgetItem *)), this, SLOT(doubleClickedItem(QTableWidgetItem *)));
   connect(tw_equiscns, SIGNAL(itemSelectionChanged()), this, SLOT(itemRowChanged()));
...
void US_GlobalEquil::scan_diags(void) {
   ...
   if (ereporter == 0) // Create reporter (1st time)
      ereporter = new US_EqReporter(dataList, scedits, scanfits, runfit, this);
...
void US_GlobalEquil::select_model(void) {
   ...
   if (modelx >= 0) {
      ...
      if (na == 1)
         DbgLv(1) << "   par1: " << aud_params[ 0 ];
      if (na == 2)
         DbgLv(1) << "   par1-2: " << aud_params[ 0 ] << aud_params[ 1 ];
      if (na == 4)
         DbgLv(1) << "   par1-4: " << aud_params[ 0 ] << aud_params[ 1 ] << aud_params[ 2 ] << aud_params[ 3 ];
...
void US_GlobalEquil::model_control(void) {
   ...
   if (model_widget) {
      ...
      emodctrl = new US_EqModelControl(scanfits, runfit, edata, modelx, models, model_widget, sscann);

      connect(emodctrl, SIGNAL(update_scan(int)), this, SLOT(new_scan(int)));
...
void US_GlobalEquil::fitting_control(void) {
   ...
   if (fit_widget) {
      ...
      efitctrl = new US_EqFitControl(scanfits, runfit, edata, emath, ereporter, modelx, models, fit_widget, sscann);
...
void US_GlobalEquil::init_params(void) {
   ...
   DbgLv(1) << "IP:  points xvs0 xvsN" << scanfits[ 0 ].points << scanfits[ 0 ].xvs[ 0 ]
            << scanfits[ 0 ].xvs[ scanfits[ 0 ].points - 1 ];
   if (fitx < 0) {
      QMessageBox::warning(
         this, tr("Scan Fits"),
   ...
   if (runfit.mw_vals[ 0 ] <= 0.0) { // Calculate the molecular weight value the 1st time
      runfit.mw_vals[ 0 ] = emath->linesearch();
      runfit.mw_rngs[ 0 ] = runfit.mw_vals[ 0 ] * 0.2;
   }

   else { // Thereafter, ask if user wants to calculate a new MW
      QMessageBox msgBox;
      msgBox.setWindowTitle(tr("Molecular Weight"));
      msgBox.setText(tr("Do you want to use the currently defined molecular"
                        " weight for the parameter\n"
                        "initialization or calculate a newly initialized"
                        " molecular weight?"));
      msgBox.setStandardButtons(QMessageBox::Yes | QMessageBox::No | QMessageBox::Cancel);
      msgBox.setButtonText(QMessageBox::Yes, tr("New Molecular Weight"));
      msgBox.setButtonText(QMessageBox::No, tr("Current Molecular Weight"));
...
void US_GlobalEquil::scan_select(double newscan) {
   ...
   if (found)
      edata_plot(); // Change the plot to the newly selected scan
   ...
   if (signal_mc) { // Signalling of model control is enabled
      if (model_widget) { // If a model control is up, have it reset the scan
...
bool US_GlobalEquil::findData(QString trip, double drpm, int &jdx, int &jrx) {
   ...
   while (++jdx < dataList.size()) { // Search the data set list
      jrx = -1;

      if (trip == triples[ jdx ]) { // If we are at the right triple, examine the speed data

         while (++jrx < dataList[ jdx ].speedData.size()) { // Search the speed steps in this triple

            if (dataList[ jdx ].speedData[ jrx ].speed == drpm) { // Right speed in right triple:  get data and mark found
               edata = &dataList[ jdx ];
      ...
      if (found)
         break; // Break out of data list search when data found
   }
...
void US_GlobalEquil::edata_plot() {
   ...
   if (jscan < iscan || jscan >= (iscan + kscan)) {
      QMessageBox::warning(
         this, tr("Scan Problem"),
         tr("Scan %1 is not within speed data scan range: %2 for %3").arg(jscan).arg(iscan).arg(kscan));
      return;
   }
   ...
   dataPlotClear(equil_plot);
   equil_plot->setTitle(
      tr("Run: ") + runID + tr("  Edit: ") + editID + "\n" + tr("Cell ") + cell + tr(",  Channel ") + chan + tr(",  ")
      + QString::number(drpm) + tr(" rpm,  Scan ") + QString::number(sscann));
   equil_plot->setAxisTitle(QwtPlot::yLeft, tr("Absorbance (") + waveln + tr(" nm)"));
   ...
   connect(pick, SIGNAL(cMouseDown(const QwtDoublePoint &)), SLOT(pMouseDown(const QwtDoublePoint &)));
   connect(pick, SIGNAL(cMouseUp(const QwtDoublePoint &)), SLOT(pMouseUp(const QwtDoublePoint &)));
   connect(pick, SIGNAL(cMouseDrag(const QwtDoublePoint &)), SLOT(pMouseMoved(const QwtDoublePoint &)));
   ...
   DbgLv(1) << "EdataPlot: radl radr" << radl << radr << " edited" << scedits[ sscanx ].edited;
...
void US_GlobalEquil::edited_plot(void) {
   ...
   for (int jj = 0; jj < vecknt; jj++) {
      if (ru[ jj ] > sRadMv) { // Save count to position where radius is beyond mouse position
         countu = jj;
         break;
      }
   }
   ...
   if (mLowerH) { // Mouse in lower half:  edited at beginning; unedited at division
      counte = countu;
   ...
   else { // Mouse in upper half:  unedited at beginning; edited at division
...
void US_GlobalEquil::pMouseUp(const QwtDoublePoint &p) {
   ...
   if (mLowerH) { // Lower half:  range-to-keep is from current position to end
      sRadLo = p.x();
      sRadHi = cRadHi;
   }

   else { // Upper half:  range-to-keep is from beginning to current position
      sRadLo = cRadLo;
      sRadHi = p.x();
   }
...
void US_GlobalEquil::assign_scanfit() {
   ...
   DbgLv(1) << "AsnSF: points xvs0 xvsN" << scanfit.points << scanfit.xvs[ 0 ] << scanfit.xvs[ scanfit.points - 1 ];
}
