    ...
    : US_WidgetsDialog (0, 0), napars (a_napars), aud_pars (a_udpars)
{
  ...
  // Main banner
  QLabel *lb_mbanner
      = us_banner (tr ("Please provide additional information:"));
  int wwid = 0;
  int whgt = 0;
  int row = 0;

  if (napars == 1)
    { // Single parameter:  stoichiometry
      lb_mbanner->setText (tr ("Please provide the Stoichiometry\n"
    ...
    { // Dual parameters:  stoichiometries
      lb_mbanner->setText (
    ...
    { // 4 parameters; MW parameters and common vbar
...
void
US_ModelAdPars::selected ()
{
  aud_pars.clear ();

  if (napars == 1 || napars == 2)
    { // Get stoichiometr(y/ies) if 1 or 2 parameters
      aud_pars << le_stoich1->text ().toDouble ();

      if (napars == 2)
        aud_pars << le_stoich2->text ().toDouble ();
    }

  else if (napars == 4)
    { // Get MW/vbar parameters if 4 parameters
