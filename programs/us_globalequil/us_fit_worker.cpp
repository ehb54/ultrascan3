...
void US_FitWorker::flag_paused(bool pauseit) { paused = pauseit; }

// Flag that work should be aborted
void US_FitWorker::flag_abort() { abort = true; }
...
int US_FitWorker::fit_iter_LM() {
  ...
  for (int ii = 0; ii < nfpars; ii++) fitpars.info[ii][ii] += lambda;
  ...
  if (qAbs(dv) < dv_toler) old_vari = variance;
  ...
  if (variance < old_vari) {  // Reduction:  guess is test-guess; Lambda reduced
    ...
    fitpars.infomsg =
        tr("Variance change ") + ((dv == 0.0) ? tr("zero") : tr("near zero"));
  }
...
double US_FitWorker::linesearch(double* search, double f0) {
  ...
  while (true) {
    ...
    if (qAbs(f0 - f1 * 2.0 + f2) < dflt_min) break;
...
double US_FitWorker::calc_testParameter(double* search, double step) {
  ...
  if (emath->calc_model(fitpars.tguess) < 0.0) {
    for (int ii = 0; ii < nfpars; ii++) fitpars.tguess[ii] = fitpars.guess[ii];
