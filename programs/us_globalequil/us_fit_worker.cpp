...
US_FitWorker::~US_FitWorker () { wait (); }

// Redefine work
void
US_FitWorker::redefine_work ()
{
...
void
US_FitWorker::flag_paused (bool pauseit)
{
  paused = pauseit;
}
  ...
  while (k_iter < mxiters && variance > tolerance)
    {
      ...
      DbgLv (1) << "EOI improve v ov" << fitpars.improve << variance
                << old_vari;
      fitpars.lambda = lambda;
      ...
      nlsmeth = (autocnvg && qAbs (fitpars.improve) < fltolr
                 && (nlsmeth == 0 || nlsmeth == 3))
                    ? (3 - nlsmeth)
                    : nlsmeth;
      DbgLv (1) << "EOI  autocnvg" << autocnvg << " nlsmeth ftol" << nlsmeth
                << fltolr;
    }
...
US_FitWorker::fit_iter_LM ()
{
  ...
  DbgLv (1) << "FWk:   ChoSS stati" << stati << " LLtr0" << fitpars.LLtr[0][0]
            << "lambda" << lambda << fitpars.lambda << "lam start,step"
            << fitpars.lam_start << fitpars.lam_step;

  // Test guess vector is original guess + B vector
  US_Matrix::vsum (fitpars.guess, fitpars.BB, fitpars.tguess, nfpars);
  DbgLv (1) << "FWk:   ii tguess guess BB" << 0 << fitpars.tguess[0]
            << fitpars.guess[0] << fitpars.BB[0];
  DbgLv (1) << "FWk:   ii tguess guess BB" << 1 << fitpars.tguess[1]
            << fitpars.guess[1] << fitpars.BB[1];
  ...
  if (variance < 0.0)
    {
      ...
      DbgLv (1) << "FWk: *** VARI<0: lgd dlm dl1 dln" << lgdl << dlmx << dlt1
                << dltn;
      return -101;
    }
  ...
  DbgLv (1) << "FWk:    old,new vari" << old_vari << variance << dv
            << dv_toler;
  ...
  if (variance < old_vari)
    ...
    { // No change:  convergence!
      ...
      fitpars.improve = dv;
      DbgLv (1) << "CNV improve nv ov" << fitpars.improve << variance
                << old_vari;
...
// Line search
double
US_FitWorker::linesearch (double *search, double f0)
{
  double alpha = 0.0;
  ...
  while (f0 >= 10000.0 || f0 < 0.0 || f1 >= 10000.0 || f1 < 0.0
         || f2 >= 10000.0 || f2 < 0.0)
    {
      x1 /= 10.0;
      x2 /= 10.0;
...
double
US_FitWorker::calc_testParameter (double *search, double step)
{
  double resid = 0.0;
...
void
US_FitWorker::updateQN (double *gamma, double *delta)
{
...
bool
US_FitWorker::isNan (double value)
{
  if (value != value)
