...
// Main constructor
US_EqHistogram::US_EqHistogram(double od_limit, QVector<EqScanFit> &scanfits, QWidget *parent, Qt::WindowFlags f) :
   ...
   for (int ii = 0; ii < scanfits.size(); ii++) { // Accumulate information about wavelengths and extinctions
   ...
   for (int jj = 0; jj < ARRAY_SIZE; jj++) { // Build X-values ramp and initialize Y's to zero
      xplot[ jj ] = x_valu;
      x_valu += x_incr;
      yplot[ jj ] = 0.0;
   }

   for (int ii = 0; ii < scanfits.size(); ii++) { // Build histogram data for each scan
      ...
      int stopn = stopx + 1;
      double scl_ext = plot_conc ? (1.0 / (scnf->pathlen * scnf->extincts[ 0 ])) : 1.0;
      double xpres = xplot[ 0 ] - x_incr;
      double xlast;
      int nx = scnf->xvs.size();
      qDebug() << "UH:  ii" << ii << " strt,stop,nx" << strtx << stopx << nx << " t_odlim scex" << tmp_od_lim << scl_ext
               << " yvs,e" << scnf->yvs[ strtx ] << scnf->yvs[ stopx ];

      for (int jj = 0; jj < ARRAY_SIZE; jj++) { // Update histogram points for this scan
