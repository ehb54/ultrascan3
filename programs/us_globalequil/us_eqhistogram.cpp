...
// Main constructor
US_EqHistogram::US_EqHistogram(double od_limit, QVector<EqScanFit>& scanfits,
                               QWidget* parent, Qt::WindowFlags f)
    : US_WidgetsDialog(parent, f), od_limit(od_limit), scanfits(scanfits) {
  ...
  for (int ii = 0; ii < scanfits.size();
       ii++) {  // Accumulate information about wavelengths and extinctions
  ...
  if (ext_define) {  // Extinctions are defined
    QString msgstr;

    if (multi_lamd) {  // There are multiple wavelengths
      msgstr =
          tr("You appear to be fitting multiple wavelengths\n"
    ...
    else {  // There is a single wavelength
      msgstr =
          tr("You have extinction coefficients defined for\n"
  ...
  if (plot_conc) {  // initial X value and X increment for Concentration
    x_valu = bin_low;
    x_incr *= (bin_high - bin_low);
  }

  else {  // initial X value (as above) and X increment for Density
    x_incr = x_valu;
  }

  for (int jj = 0; jj < ARRAY_SIZE;
       jj++) {  // Build X-values ramp and initialize Y's to zero
    xplot[jj] = x_valu;
    x_valu += x_incr;
    yplot[jj] = 0.0;
  }
  ...
  for (int ii = 0; ii < scanfits.size();
       ii++) {  // Build histogram data for each scan
    ...
    int stopn = stopx + 1;
    double scl_ext =
        plot_conc ? (1.0 / (scnf->pathlen * scnf->extincts[0])) : 1.0;
    ...
    for (int jj = 0; jj < ARRAY_SIZE;
         jj++) {          // Update histogram points for this scan
      ...
      if (jj < 3 || (jj + 4) > ARRAY_SIZE)
        qDebug() << "UH:    jj" << jj << " xp" << xpres << " yplot[jj]"
                 << yplot[jj];
    }
  }
  ...
  qDebug() << "UH: x0 x1 xm xn" << xplot[0] << xplot[1] << xplot[48]
           << xplot[49];
  qDebug() << "UH: y0 y1 ym yn" << yplot[0] << yplot[1] << yplot[48]
           << yplot[49];
