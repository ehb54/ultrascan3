...
void
US_EquilTime::new_lowspeed (double speed)
{
...
void
US_EquilTime::new_highspeed (double speed)
{
...
void
US_EquilTime::new_speedstep (double count)
{
...
void
US_EquilTime::new_channel (int channel)
{
  cnt_top->setEnabled (false);
  cnt_bottom->setEnabled (false);
  ...
  switch (channel)
    ...
    case INNER:
      ...
      equilibrium_plot->setAxisScale (QwtPlot::xBottom, inner_top,
                                      inner_bottom);
      break;
    ...
    case OUTER:
      ...
      equilibrium_plot->setAxisScale (QwtPlot::xBottom, outer_top,
                                      outer_bottom);
      break;
...
double
US_EquilTime::rpmFromSigma (double sigma)
{
  ...
  double rpm
      = 30.0 / M_PI * sqrt (sigma * R_GC * T * 2 / (mw * (1 - vbar * rho)));
...
double
US_EquilTime::sigmaFromRpm (double rpm)
{
  ...
  double sigma
      = mw * (1 - vbar * rho) * sq (M_PI / 30.0 * rpm) / (2 * R_GC * T);

  return sigma;
}

void
US_EquilTime::update_speeds (int type)
{
  speed_steps.clear ();
  te_speedlist->clear ();
          ...
          for (int i = 0; i < speed_count; i++)
            {
              ...
              te_speedlist->append (
                  QString::number (i + 1)
                  + ": sigma = " + QString::number (speed_steps[i], 'f', 3));
            }
        }
      else
        {
          ...
          for (int i = 0; i < speed_count; i++)
            {
              ...
              rpm = floor (rpm / 100.0 + 0.5) * 100.0; // Round to closest 100
              speed_steps << rpm;
              te_speedlist->append (QString::number (i + 1)
                                    + ": rpm = " + QString::number (rpm));
            }
        }
      else
        {
...
void
US_EquilTime::change_model (void)
{
...
void
US_EquilTime::set_model (US_Model m)
{
  model = m;
...
void
US_EquilTime::init_simparams (void)
{
...
void
US_EquilTime::init_astfem_data (void)
{
  ...
  for (int i = 0; i < speed_steps.count (); i++)
    {
      ...
      QString results;
      results.sprintf ("%6.4f  %5d   %6.2f hours    %6.2f hours", sigma, rpm,
                       step_time / 3600.0, current_time / 3600.0);
...
void
US_EquilTime::check_equil (QVector<double> *x, double *c)
{
