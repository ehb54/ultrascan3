...
US_EquilTime::US_EquilTime() : US_Widgets(true) {
  ...
  QBoxLayout* plot =
      new US_Plot(equilibrium_plot, tr("Approach to Equilibrium Simulation"),
...
void US_EquilTime::simulate(void) {
  ...
  te_info->e->append(tr("(Note: All speeds have been adjusted to be rounded to "
                        "the nearest 100 RPM.)\n") +
...
void US_EquilTime::check_equil(QVector<double>* x, double* c) {
  ...
  for (int i = 0; i < radius_points; i++) diffs += sq(concentration[i] - c[i]);
