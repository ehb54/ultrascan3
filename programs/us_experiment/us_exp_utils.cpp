...
QString
US_ExperimentMain::childSValue (const QString child, const QString type)
{
  QString value ("");
  if (child == "general")
    {
      value = epanGeneral->getSValue (type);
    }
  else if (child == "rotor")
    {
      value = epanRotor->getSValue (type);
    }
  else if (child == "speeds")
    {
      value = epanSpeeds->getSValue (type);
    }
  else if (child == "cells")
    {
      value = epanCells->getSValue (type);
    }
  else if (child == "solutions")
    {
      value = epanSolutions->getSValue (type);
    }
  else if (child == "optical")
    {
      value = epanOptical->getSValue (type);
    }
  else if (child == "ranges")
    {
      value = epanRanges->getSValue (type);
    }
  else if (child == "aprofile")
    {
      value = epanAProfile->getSValue (type);
    }
  else if (child == "submit")
    {
      value = epanUpload->getSValue (type);
    }
  return value;
}
...
int
US_ExperimentMain::childIValue (const QString child, const QString type)
{
  int value = 0;
  if (child == "general")
    {
      value = epanGeneral->getIValue (type);
    }
  else if (child == "rotor")
    {
      value = epanRotor->getIValue (type);
    }
  else if (child == "speeds")
    {
      value = epanSpeeds->getIValue (type);
    }
  else if (child == "cells")
    {
      value = epanCells->getIValue (type);
    }
  else if (child == "solutions")
    {
      value = epanSolutions->getIValue (type);
    }
  else if (child == "optical")
    {
      value = epanOptical->getIValue (type);
    }
  else if (child == "ranges")
    {
      value = epanRanges->getIValue (type);
    }
  else if (child == "aprofile")
    {
      value = epanAProfile->getIValue (type);
    }
  else if (child == "submit")
    {
      value = epanUpload->getIValue (type);
    }
  return value;
}
...
double
US_ExperimentMain::childDValue (const QString child, const QString type)
{
  double value = 0.0;
  if (child == "general")
    {
      value = epanGeneral->getDValue (type);
    }
  else if (child == "rotor")
    {
      value = epanRotor->getDValue (type);
    }
  else if (child == "speeds")
    {
      value = epanSpeeds->getDValue (type);
    }
  else if (child == "cells")
    {
      value = epanCells->getDValue (type);
    }
  else if (child == "solutions")
    {
      value = epanSolutions->getDValue (type);
    }
  else if (child == "optical")
    {
      value = epanOptical->getDValue (type);
    }
  else if (child == "ranges")
    {
      value = epanRanges->getDValue (type);
    }
  else if (child == "aprofile")
    {
      value = epanAProfile->getDValue (type);
    }
  else if (child == "submit")
    {
      value = epanUpload->getDValue (type);
    }
  return value;
}
...
QStringList
US_ExperimentMain::childLValue (const QString child, const QString type)
{
  QStringList value;

  if (child == "general")
    {
      value = epanGeneral->getLValue (type);
    }
  else if (child == "rotor")
    {
      value = epanRotor->getLValue (type);
    }
  else if (child == "speeds")
    {
      value = epanSpeeds->getLValue (type);
    }
  else if (child == "cells")
    {
      value = epanCells->getLValue (type);
    }
  else if (child == "solutions")
    {
      value = epanSolutions->getLValue (type);
    }
  else if (child == "optical")
    {
      value = epanOptical->getLValue (type);
    }
  else if (child == "ranges")
    {
      value = epanRanges->getLValue (type);
    }
  else if (child == "aprofile")
    {
      value = epanAProfile->getLValue (type);
    }
  else if (child == "submit")
    {
      value = epanUpload->getLValue (type);
    }
...
void
US_ExperimentMain::newPanel (int panx)
{
  DbgLv (1) << "newPanel panx=" << panx << "prev.panx=" << curr_panx
            << "usmode" << usmode;
  // Save any changes in the old current panel
  if (curr_panx == panx)
    return; // No change in panel

  if (curr_panx == 0)
    epanGeneral->savePanel ();
  else if (curr_panx == 1)
    epanRotor->savePanel ();
  else if (curr_panx == 2)
    epanSpeeds->savePanel ();
  else if (curr_panx == 3)
    epanCells->savePanel ();
  else if (curr_panx == 4)
...
void
US_ExperimentMain::statUpdate ()
{
  DbgLv (1) << "statUpd: IN stat" << statflag;
  statflag = epanGeneral->status () + epanRotor->status ()
             + epanSpeeds->status () + epanCells->status ()
             + epanSolutions->status () + epanOptical->status ()
             + epanRanges->status () + epanAProfile->status ()
             + epanUpload->status ();
...
void
US_ExperimentMain::disable_tabs_buttons (void)
{
...
void
US_ExperimentMain::enable_tabs_buttons (void)
{
  DbgLv (1) << "ENABLING!!!";
  ...
  for (int i = 1; i < tabWidget->count (); i++)
    {
      ...
      tabWidget->tabBar ()->setTabTextColor (
          i, pal.color (QPalette::WindowText)); // Qt::black
    }
}

// Slot to ENABLE tabs and Next/Prev buttons, but make all Widgets read-only
void
US_ExperimentMain::enable_tabs_buttons_readonly (void)
{
  DbgLv (1) << "ENABLING!!!";
  pb_next->setEnabled (true);
  pb_prev->setEnabled (true);
  ...
  for (int i = 1; i < tabWidget->count (); i++)
    {
      ...
      QList<QPushButton *> allPButtons
          = pWidget->findChildren<QPushButton *> ();
      ...
      for (int i = 0; i < allPButtons.count (); i++)
        {
          if ((allPButtons[i]->text ()).contains ("View Solution Details")
              || (allPButtons[i]->text ())
                     .contains ("View Current Range Settings")
...
// Set all tabs their all Widgets (but 8. AProfile section) READ-ONLY for
// US_ProtocolDev
void
US_ExperimentMain::set_tabs_buttons_readonly (void)
{
  pb_next->setEnabled (true);
  pb_prev->setEnabled (true);
  ...
  for (int ii = 0; ii < tabWidget->count (); ii++)
    {
      ...
      // DbgLv(1) << "PALETTE: " << pal.color(QPalette::WindowText);
      tabWidget->tabBar ()->setTabTextColor (
          ii, pal.color (QPalette::WindowText)); // Qt::black
...
bool
US_ExperimentMain::centpInfo (const QString par1, US_AbstractCenterpiece &par2)
{
  return epanGeneral->centpInfo (par1, par2);
}

int
US_ExperimentMain::getProtos (QStringList &par1, QList<QStringList> &par2)
{
  return epanGeneral->getProtos (par1, par2);
}

void
US_ExperimentMain::setProtos (QStringList par1)
{
  epanGeneral->setProtos (par1);
}

bool
US_ExperimentMain::updateProtos (const QStringList par1)
{
...
US_ExperimentMain::initPanels ()
{
  ...
  DbgLv (1) << "main:  So:iP:";
  epanSolutions->initPanel ();
  for (int ii = 0; ii < epanSolutions->mxrow;
       ii++) // ALEXEY: reset channel comment while protocol re-loaded
    {
...
use_db=false;
#endif
  DbgLv (1) << "EGGe: inP: prn,prd counts" << protdata.count ()
            << pr_names.count ();
  ...
  // Debug
  qDebug () << "ul2_operator_for_optima.keys().size() -- "
            << ul2_operator_for_optima.keys ().size ();
  QMap<QString, bool>::iterator opd;
  for (opd = ul2_operator_for_optima.begin ();
       opd != ul2_operator_for_optima.end (); ++opd)
  ...
  if (inv_lev < 3 && mainw->automode)
    {
      ...
      for (op = ul2_operator_for_optima.begin ();
           op != ul2_operator_for_optima.end (); ++op)
        {
          if (op.value ())
            {
...
void
US_ExperGuiGeneral::check_empty_runname (const QString & /*str*/)
{
#if 0
   if ( str.isEmpty() )
...
void
US_ExperGuiGeneral::check_runname ()
{
#if 0
...
void
US_ExperGuiGeneral::update_inv (void)
{
  US_Passwd pw;
...
void
US_ExperGuiGeneral::check_user_level ()
{
  // update_inv(); //ALEXEY: this might be needed!!!
  //  MAYBE check for actual usr_level &&  US_Settings::set_us_inv_level( level
  //  ) ONLY!!! (NO inv_id && NO inv name)
  ...
  if (inv_lev > 2)
    ...
    { // Non-admin users enabled if they have instrument permit
      ...
      qDebug () << "check_user_level():  lablist.count() -- "
                << lablist.count ();

      for (int ii = 0; ii < lablist.count (); ii++)
        { // Look through lab info
          QList<US_Rotor::Instrument> instruments = lablist[ii].instruments;

          ul2_operator_for_optima.clear ();

          qDebug () << "check_user_level():  instruments.count() -- "
                    << instruments.count ();
          ...
          // Go over instruments, and determine if the current user an
          // operator: per instrument
...
QString
US_ExperGuiGeneral::getSValue (const QString type)
{
  QString value ("");

  if (type == "runID" || type == "runname")
    {
      value = currProto->runname;
    }
  else if (type == "protocol")
    {
      value = currProto->protoname;
    }
  else if (type == "project")
    {
      value = currProto->project;
    }
  else if (type == "investigator")
    {
      value = currProto->investigator;
    }
  else if (type == "dbdisk")
    {
...
int
US_ExperGuiGeneral::getIValue (const QString type)
{
  int value = 0;
  if (type == "dbdisk")
    {
...
double
US_ExperGuiGeneral::getDValue (const QString type)
{
  double value = 0.0;
  if (type == "temperature")
    {
      value = currProto->temperature;
    }
...
QStringList
US_ExperGuiGeneral::getLValue (const QString type)
{
  QStringList value ("");
...
QString
US_ExperGuiGeneral::sibSValue (const QString sibling, const QString type)
{
...
int
US_ExperGuiGeneral::sibIValue (const QString sibling, const QString type)
{
...
double
US_ExperGuiGeneral::sibDValue (const QString sibling, const QString type)
{
...
QStringList
US_ExperGuiGeneral::sibLValue (const QString sibling, const QString type)
{
...
int
US_ExperGuiGeneral::status ()
{
  bool is_done
      = !(currProto->runname.isEmpty () || currProto->project.isEmpty ());
  return (is_done ? 1 : 0);
}
...
US_ExperGuiRotor::initPanel ()
{
  ...
  QString rot_entr = QString::number (rpRotor->rotID) + ": " + rpRotor->rotor;
  ...
  // ALEXEY - set Optima name (with checking connection), THEN operator - ORDER
  // IMPORTANT!!!
  QString optima_name
      = QString::number (rpRotor->instID) + ": " + rpRotor->instrname;

  qDebug () << "OPTIMA NAME in initPANEL()  :" << optima_name;
  qDebug () << "OPTIMA INDEX in initPANEL() :"
            << cb_optima->findText (optima_name);

  // optima machine
  // IF optima_name is NOT present in the list of the available instruments
  // (from autoflow), then use 1st available instr. with informimng user that
  // Optima was changed
  if (cb_optima->findText (optima_name) < 0)
    {
      ...
      msg_instr_avail->setText (
          tr ("Loaded protocol specified <b>%1</b> as the instrument in "
              "use.<br><br>"
              "However, due to its current unavailability, the instrument in "
              "use will be changed to the first available intrument, "
              "<b>%2</b>.")
              .arg (rpRotor->instrname)
              .arg (mainw->currentInstrument["name"]));
    ...
    changeOptima (cb_optima->findText (
        optima_name)); //<-- Do actual connection test in changeLab();

  // setCbCurrentText( cb_optima,   optima_name );    // <-- NOT ENOUGH, no
  // connection check
  ...
  DbgLv (1) << "EGRo: inP:  rotID" << rpRotor->rotID << "rotor"
            << rpRotor->rotor << "cb_rotor text" << cb_rotor->currentText ();
  DbgLv (1) << "EGRo: inP:   calID" << rpRotor->calID << "calib"
            << rpRotor->calibration;
...
void
US_ExperGuiRotor::init_grevs (void)
{
  cb_choose_rev->clear ();
  ...
  while (db.next ())
    {
      ...
      // populate
      cb_choose_rev->addItem (QString::number (g_invID) + ". " + g_lastName
                              + ", " + g_firstName);
    }
}

void
US_ExperGuiRotor::init_gapprs (void)
{
  ...
  while (db.next ())
    {
      ...
      // populate
      cb_choose_appr->addItem (QString::number (g_invID) + ". " + g_lastName
                               + ", " + g_firstName);
    }
}

void
US_ExperGuiRotor::init_gsmes (void)
{
  cb_choose_sme->clear ();
  ...
  while (db.next ())
    {
      ...
      // populate
      cb_choose_sme->addItem (QString::number (g_invID) + ". " + g_lastName
                              + ", " + g_firstName);
    }
}
...
void
US_ExperGuiRotor::addOpertoList (void)
{
  ...
  if (e_operList.contains (c_oper))
    {
      QMessageBox::information (this, tr ("Cannot add Operator"),
                                tr ("<font color='red'><b>ATTENTION:</b> "
                                    "</font> Selected operator: <br><br>"
                                    "<font ><b>%1</b><br><br>"
  ...
  // also, check if to-be-added operator is in other role(s) already [rev,
  // appr]
  ...
  if (e_revList.contains (c_oper) || e_apprList.contains (c_oper))
    {
      QMessageBox::information (
          this, tr ("Cannot add Operator"),
          tr ("<font color='red'><b>ATTENTION:</b> </font> Selected operator: "
              "<br><br>"
              "<font ><b>%1</b><br><br>"
...
void
US_ExperGuiRotor::removeOperfromList (void)
{
  ...
  te_opers_to_assign->moveCursor (QTextCursor::StartOfLine,
...
void
US_ExperGuiRotor::addRevtoList (void)
{
...
void
US_ExperGuiRotor::removeRevfromList (void)
{
  ...
  te_revs_to_assign->moveCursor (QTextCursor::StartOfLine,
                                 QTextCursor::MoveAnchor);
...
void
US_ExperGuiRotor::addApprtoList (void)
{
...
// Remove reviewer from list
void
US_ExperGuiRotor::removeApprfromList (void)
{
  ...
  te_apprs_to_assign->moveCursor (QTextCursor::StartOfLine,
                                  QTextCursor::MoveAnchor);
...
// Add approver to list
void
US_ExperGuiRotor::addSmetoList (void)
{
...
// Remove reviewer from list
void
US_ExperGuiRotor::removeSmefromList (void)
{
  ...
  te_smes_to_assign->moveCursor (QTextCursor::StartOfLine,
                                 QTextCursor::MoveAnchor);
...
void
US_ExperGuiRotor::test_optima_connection ()
{
...
US_ExperGuiRotor::savePanel ()
{
  ...
  qDebug () << "Lists of o,r,a: " << oper_list << rev_list << appr_list;
  ...
  qDebug () << "NAME OF THE ROTOR IN SAVE: rot, rpRotor->rotor: " << rot
            << ", " << rpRotor->rotor;
  ...
  qDebug () << "OPERATORID / INSTRUMENT / ExpType in SAVE: " << rpRotor->operID
            << ", " << rpRotor->opername << " / " << rpRotor->instID << ", "
            << rpRotor->instrname << " / " << rpRotor->exptype;

  DbgLv (1) << "EGRo:  svP:  rotID" << rpRotor->rotID << "rotor"
            << rpRotor->rotor << "cb_rotor text" << rot;
      ...
      if (rpRotor->calID == calibs[ii].ID)
        {
          ...
          DbgLv (1) << "EGRo:  svP:  calndx" << ii << "calGUID"
                    << rpRotor->calGUID;
        }
    }
  ...
  qDebug ()
      << "[in Rotor: savePanel()]: exptype, expType_old,  expType_changed? "
...
// Get a specific panel string value
QString
US_ExperGuiRotor::getSValue (const QString type)
{
  QString value ("");
...
// Get a specific panel integer value
int
US_ExperGuiRotor::getIValue (const QString type)
{
  int value = 0;
  ...
  if (type == "nholes")
    {
      ...
      DbgLv (1) << "EGRo:gIV:  arotor" << getSValue ("arotor") << "value"
                << value;
    }
  else if (type == "maxrpm")
    {
...
// Get a specific panel double value
double
US_ExperGuiRotor::getDValue (const QString type)
{
  double value = 0;
  if (type == "maxrpm")
    {
      value = (double)getIValue (type);
    }
...
// Get specific panel list values
QStringList
US_ExperGuiRotor::getLValue (const QString type)
{
  QStringList value ("");
...
QString
US_ExperGuiRotor::sibSValue (const QString sibling, const QString type)
{
  return mainw->childSValue (sibling, type);
}
...
// Get a specific panel integer value from a sibling panel
int
US_ExperGuiRotor::sibIValue (const QString sibling, const QString type)
{
  return mainw->childIValue (sibling, type);
}
...
// Get a specific panel double value from a sibling panel
double
US_ExperGuiRotor::sibDValue (const QString sibling, const QString type)
{
  return mainw->childDValue (sibling, type);
}
...
// Get a specific panel list from a sibling panel
QStringList
US_ExperGuiRotor::sibLValue (const QString sibling, const QString type)
{
  return mainw->childLValue (sibling, type);
}
...
// Return status flag for the panel
int
US_ExperGuiRotor::status ()
{
  bool is_done = !(rpRotor->laboratory.isEmpty () || rpRotor->rotor.isEmpty ()
...
US_ExperGuiSpeeds::initPanel ()
{
  ...
  le_maxrpm->setText (
      tr ("Maximum speed for %1 rotor:  %2 rpm").arg (arotor).arg (speedmax));
  ...
  for (int ii = 0; ii < nspeed; ii++)
    {
      ...
      qDebug () << " DURATION SAVED IN  PROTOTCOL: speed " << ii
                << ", duration: " << rpSpeed->ssteps[ii].duration;
    }
}

// Get a specific panel value
QString
US_ExperGuiSpeeds::getSValue (const QString type)
{
  QString value ("");
...
// Get a specific panel integer value
int
US_ExperGuiSpeeds::getIValue (const QString type)
{
  int value = 0;
  if (type == "nspeeds")
    {
...
// Get a specific panel double value
double
US_ExperGuiSpeeds::getDValue (const QString type)
{
  double value = 0;
  if (type == "dbdisk")
    {
...
// Get specific panel list values
QStringList
US_ExperGuiSpeeds::getLValue (const QString type)
{
  QStringList value ("");
    ...
    { // Compose list of all speed-step values (4 lines per step)
      ...
      for (int ii = 0; ii < rpSpeed->nstep; ii++)
        { // Build list of QString forms of speed-step double
          ...
          double delaystagemin
              = qFloor (delay_stage - (delaystagehrs * 3600.0)) / 60.0;
          ...
          value << tr ("%1 h %2 m").arg (durathrs).arg (duratmin);
          value << tr ("%1 h %2 m ").arg (delayhrs).arg (delaymin);
          value << tr ("%1 h %2 m ").arg (delaystagehrs).arg (delaystagemin);
...
// Get a specific panel value from a sibling panel
QString
US_ExperGuiSpeeds::sibSValue (const QString sibling, const QString type)
{
  return mainw->childSValue (sibling, type);
}
...
// Get a specific panel integer value from a sibling panel
int
US_ExperGuiSpeeds::sibIValue (const QString sibling, const QString type)
{
  return mainw->childIValue (sibling, type);
}
...
// Get a specific panel double value from a sibling panel
double
US_ExperGuiSpeeds::sibDValue (const QString sibling, const QString type)
{
  return mainw->childDValue (sibling, type);
}
...
// Get a specific panel list from a sibling panel
QStringList
US_ExperGuiSpeeds::sibLValue (const QString sibling, const QString type)
{
  return mainw->childLValue (sibling, type);
}
...
void
US_ExperGuiCells::initPanel ()
{
  rpCells = &(mainw->currProto.rpCells);

  const int mxcels = 8;
  QStringList sl_bals; // Counterbalance choices
  sl_bals << tr ("empty (counterbalance)") << tr ("Beckman counterbalance")
  ...
  DbgLv (1) << "EGCe:inP: prb: nholes" << rpCells->ncell << "nused"
            << rpCells->nused;
  rebuild_Cells ();
  ...
  for (int ii = 0; ii < nholes; ii++)
    {                       //
      ...
      // qDebug() << "cell: " << ii+1 << ", text: " << cc_cenps[ ii
      // ]->currentText();

      if ((ii != icbal
           && cc_cenps[ii]->currentText ().contains (tr ("counterbalance")))
          || (ii != icbal
              && !cc_cenps[ii]->currentText ().contains (
                  tr ("counterbalance"))))
        { // Centerpiece when list is counterbalances: reset list

          // qDebug() << "CELL NOT LAST BUT counterbalance: text,  " <<
          // cc_cenps[ ii ]->currentText();
          cc_cenps[ii]->clear ();
        ...
        { // Counterbalance when list is centerpieces: reset list
          ...
          // qDebug() << "CELL LAST BUT not counterbalance: text,  " <<
          // cc_cenps[ ii ]->currentText();
          cc_cenps[ii]->clear ();
          cc_cenps[ii]->addItems (sl_bals); // Choose from counterbalances

          cc_cenps[ii]->addItems (
              cpnames); // ALEXEY: add to Choose from centerpieces  also
        }
        ...
        { // Search to see if current is among the used cells
          ...
          if (cell == rpCells->used[jj].cell)
            { // This is a used cell:  populate a table row
              // QString cenbal      =  ( ii != icbal ) //ALEXEY - change
              // needed!
              ...
              if (ii != icbal)
                {
                  // qDebug() << "POPULATING INNER CELL - cbalance   !!!!! :  "
                  // << rpCells->used[ jj ].cbalance; qDebug() << "POPULATING
                  // INNER CELL - centerpiece!!!!! :  " << rpCells->used[ jj
                  // ].centerpiece;
                  cenbal = rpCells->used[jj].centerpiece;
                }
              else
                {
                  // qDebug() << "POPULATING LAST CELL - cbalance   !!!!! :  "
                  // << rpCells->used[ jj ].cbalance; qDebug() << "POPULATING
                  // LAST CELL - centerpiece!!!!! :  " << rpCells->used[ jj
                  // ].centerpiece; if ( rpCells->used[ jj ].cbalance.contains(
                  // tr( "centerpiece" ) ) )
                  if (rpCells->used[jj].cbalance.isEmpty ()
                      || rpCells->used[jj].cbalance.contains (
                          tr ("centerpiece")))
                    cenbal = rpCells->used[jj].centerpiece;
                  else
      ...
      // cc_winds[ ii ]->setVisible( ii != icbal );                    //ALEXEY
      // - to change!!!
      if (ii != icbal)
      ...
      DbgLv (1) << "EGCe:inP:   ii" << ii << "kused" << kused << "is_used"
                << is_used;
...
US_ExperGuiCells::savePanel ()
{
  ...
  for (int ii = 0; ii < nholes; ii++)
    { // Count used (non-empty) cell rows
      ...
      DbgLv (1) << "EGCe:svP:  ii" << ii << "celnam" << celnam << "centp"
                << centp;
...
// Get a specific panel value
QString
US_ExperGuiCells::getSValue (const QString type)
{
...
// Get a specific panel integer value
int
US_ExperGuiCells::getIValue (const QString type)
{
  int value = 0;
...
// Get a specific panel double value
double
US_ExperGuiCells::getDValue (const QString type)
{
  int ncell = rpCells->ncell;
  double value = 0;
...
// Get specific panel list values
QStringList
US_ExperGuiCells::getLValue (const QString type)
{
  QStringList value;
      ...
      for (int ii = 0; ii < rpCells->nused; ii++)
        {
          ...
          // QString centry   = ( ( ii + 1 ) != nholes ) // ALEXEY: change
          // needed
          //                  ? celnm + " : " + centp + "  ( " + windo + " )"
          //                  : celnm + " : " + cobal;

          QString centry; // ALEXEY: if last cell and not counterbalance, treat
                          // as centerpiece
          if ((ii + 1) != nholes)
            {
    ...
    { // Return channel strings where a centerpiece is selected
      ...
      for (int ii = 0; ii < rpCells->nused; ii++)
        { // Examine the used cells
          ...
          // if ( icell >= icbal )   continue;                   // Skip
          // counterbal.      //ALEXEY bug

          if (icell > icbal
              && rpCells->used[ii].cbalance.contains (tr ("counterbalance")))
...
// Get a specific panel value from a sibling panel
QString
US_ExperGuiCells::sibSValue (const QString sibling, const QString type)
{
  return mainw->childSValue (sibling, type);
}
...
// Get a specific panel integer value from a sibling panel
int
US_ExperGuiCells::sibIValue (const QString sibling, const QString type)
{
  return mainw->childIValue (sibling, type);
}
...
// Get a specific panel double value from a sibling panel
double
US_ExperGuiCells::sibDValue (const QString sibling, const QString type)
{
  return mainw->childDValue (sibling, type);
}
...
// Get a specific panel list from a sibling panel
QStringList
US_ExperGuiCells::sibLValue (const QString sibling, const QString type)
{
  return mainw->childLValue (sibling, type);
}
...
void
US_ExperGuiSolutions::initPanel ()
{
  ...
  for (int ii = 0; ii < nchant; ii++)
    {
      ...
      qDebug () << "INIT SOLS, channel, ra_data_type -- " << channel
      ...
      qDebug () << "11: rpSolut->chsols.size(), ii: "
                << rpSolut->chsols.size () << ii;
  ...
  DbgLv (1) << "EGSo:inP: mxrow" << mxrow << "labls count"
            << cc_labls.count ();
  ...
  for (int ii = 0; ii < mxrow; ii++)
    {
      ...
      connect (cb_solution, SIGNAL (currentIndexChanged (int)), this,
               SLOT (changeSolu (int)));
    }
}
...
// Save panel controls when about to leave the panel
void
US_ExperGuiSolutions::savePanel ()
{
  ...
  for (int ii = 0; ii < mxrow; ii++)
    { // Fill panel lists and parameters from GUI elements
      ...
      qDebug () << "ii, rpSolut->chsols.size(): -- " << ii
                << rpSolut->chsols.size ();
      ...
      if (pro_comms.keys ().contains (iistr))
        {
          ...
          // ALEXEY - to remember changes to Soluton comments if manual
          // commnets was added while returning to "Solutons" tab
        ...
        {
          // commentStrings( solution, ch_comment, cs, ii ); //<-- incorrect as
          // always sets comment to solname!
          qDebug () << "sol save 2aB";
          ...
          if (rpSolut->chsols.size () == 0)
            ...
            {
              qDebug () << "sol save 2aBd, ii, rpSolut->chsols.size(): -- "
                        << ii << rpSolut->chsols.size ();
              if (ii < rpSolut->chsols.size ())
                ch_comment = rpSolut->chsols[ii].ch_comment;
              else
                ch_comment = QString (""); // are comments saved this way???
              qDebug () << "sol save 2aBd, ii, rpSolut->chsols.size(): after "
                           "reding comment-- ";
...
// Get a specific panel value
QString
US_ExperGuiSolutions::getSValue (const QString type)
{
  QString value ("");

  if (type == "nchant" || type == "nchanf" || type == "nusols"
      || type == "alldone" || type == "status")
...
// Get a specific panel integer value
int
US_ExperGuiSolutions::getIValue (const QString type)
{
  int value = 0;
...
// Get a specific panel double value
double
US_ExperGuiSolutions::getDValue (const QString type)
{
  double value = 0.0;
  if (type == "dbdisk")
    {
...
// Get specific panel list values
QStringList
US_ExperGuiSolutions::getLValue (const QString type)
{
...
// Get a specific panel value from a sibling panel
QString
US_ExperGuiSolutions::sibSValue (const QString sibling, const QString type)
{
  return mainw->childSValue (sibling, type);
}
...
// Get a specific panel integer value from a sibling panel
int
US_ExperGuiSolutions::sibIValue (const QString sibling, const QString type)
{
  return mainw->childIValue (sibling, type);
}
...
// Get a specific panel double value from a sibling panel
double
US_ExperGuiSolutions::sibDValue (const QString sibling, const QString type)
{
  return mainw->childDValue (sibling, type);
}
...
// Get a specific panel list from a sibling panel
QStringList
US_ExperGuiSolutions::sibLValue (const QString sibling, const QString type)
{
  return mainw->childLValue (sibling, type);
}
...
US_ExperGuiOptical::initPanel ()
{
  ...
  // ALEXEY: if last cell is used as centerpiece, disable Rayleigh Interference
  // (ckbox2/ckbox3)
  int nholes = sibIValue ("rotor", "nholes");
  ...
  for (int ii = 0; ii < nochan; ii++)
    {
      ...
      int cell_number
          = ((channel.split (QRegExp ("\\s+"), QString::SkipEmptyParts))[0])
                .toInt ();
  ...
  for (int ii = 0; ii < nochan; ii++)
    {
      ...
      DbgLv (1) << "EGOp:inP:  ii" << ii << "channel" << channel << "scan1-3"
                << scan1 << scan2 << scan3;
      ...
      // ALEXEY: set checkboxes checked based on the protocol loaded, rather
      // than by default
      if (!scan1.isEmpty ())
      ...
      // ckbox2->setChecked( prscans.contains( ckscan2 ) );   //ALEXEY do not
      // check Interference by default

      // ALEXEY: if last cell is used as centerpiece, disable Rayleigh
      // Interference (ckbox2|ckbox3]
  ...
  for (int ii = 0; ii < nochan; ii++)
    {
      ...
      DbgLv (1) << "EGOp:svP:     kchkd" << kchkd
                << "scans:" << rpOptic->chopts[ii].scan1
                << rpOptic->chopts[ii].scan2 << rpOptic->chopts[ii].scan3;
    }
}

// Get a specific panel value
QString
US_ExperGuiOptical::getSValue (const QString type)
{
  QString value ("");
...
// Get a specific panel integer value
int
US_ExperGuiOptical::getIValue (const QString type)
{
  int value = 0;
  int istat = status ();
...
// Get a specific panel double value
double
US_ExperGuiOptical::getDValue (const QString type)
{
  double value = 0;
  if (type == "dbdisk")
    {
...
// Get specific panel list values
QStringList
US_ExperGuiOptical::getLValue (const QString type)
{
  QStringList value ("");
...
// Get a specific panel value from a sibling panel
QString
US_ExperGuiOptical::sibSValue (const QString sibling, const QString type)
{
  return mainw->childSValue (sibling, type);
}
...
// Get a specific panel integer value from a sibling panel
int
US_ExperGuiOptical::sibIValue (const QString sibling, const QString type)
{
  return mainw->childIValue (sibling, type);
}
...
// Get a specific panel double value from a sibling panel
double
US_ExperGuiOptical::sibDValue (const QString sibling, const QString type)
{
  return mainw->childDValue (sibling, type);
}
...
// Get a specific panel list from a sibling panel
QStringList
US_ExperGuiOptical::sibLValue (const QString sibling, const QString type)
{
  return mainw->childLValue (sibling, type);
}
...
US_ExperGuiOptical::status ()
{
  ...
  for (int ii = 0; ii < nochan; ii++)
    { // Loop to count visible rows and ones with at least one check
      ...
      QString scan2 = rpOptic->chopts[ii].scan2;
      QString scan3 = rpOptic->chopts[ii].scan3;
      DbgLv (1) << "EGOp:st:  ii" << ii << "ch s1 s2 s3" << channel << scan1
                << scan2 << scan3;
...
US_ExperGuiRanges::initPanel ()
{
  ...
  for (int ii = 0; ii < nrnchan; ii++)
    {
      ...
      if (kswavl == 0)
        ...
        labwlr = tr ("1,  %1").arg (swvlens[ii][0]);
      else
        labwlr = tr ("%1,  %2 to %3")
                     .arg (kswavl)
                     .arg (swvlens[ii][0])
      ...
      qDebug () << "Ranges::abde_buffes for chann " << ii << " is -- "
                << abde_buff[ii];
      ...
      if (channel.contains (tr (
              "reference"))) // ALEXEY do not allow to set wavelengths/radial
                             // ranges for B channels (reference)
        {
        ...
        {
          if (US_Settings::us_inv_level ()
              > 2) // ALEXEY: if user level 3 or more
            {
  ...
  for (int i = 0; i < nrnchan; i++)
    {
      ...
      // Set check-state of  boxes in same-cell rows and reconnect : ALEXEY:
      // here set of channel A is copied to B
      for (int j = 0; j < ccrows.count (); j++)
        {
          int ccrow = ccrows[j];

          qDebug () << "::initPanel(), Ranges: same-cell rows -- " << clabl
                    << cc_labls[ccrow]->text ();
          ...
          for (int ii = 0; ii < rpRange->nranges; ii++)
            {
              ...
              if (channel.contains ("sample")
                ...
                {
                  DbgLv (1) << "EGRn:inP:   # of Ranges for cell " << j << ": "
                            << rpRange->chrngs[ii].wvlens.count ();
                  Total_wvl[i] += rpRange->chrngs[ii].wvlens.count ();

                  ++ncells_used[i];
                }
            }
          DbgLv (1) << "EGRn:inP:  #Wvl for cell: " << j
                    << " is: " << Total_wvl[i];
        }
    }
  ...
  for (int i = 0; i < nsp; i++)
    {
      ...
      qDebug () << "RANGES INIT: duration_sec , scanint_sec, scanint_sec_min, "
                   " Total_wvl[i], ncells_used[i] -- "
      ...
      if (has_absorbance)
        {
          ...
          QString scint_str = QString::number (hms_scanint[1]) + "h "
                              + QString::number (hms_scanint[2]) + "m "
                              + QString::number (hms_scanint[3]) + "s";
          ...
          QString scancount_stage
              = tr ("Stage %1. Number of Scans per Triple (UV/vis): %2 ")
                    .arg (i + 1)
                    .arg (scancount);
      ...
      else // hide uv/vis related widgets
        {
          ...
          // Update cb_scancount string:
          QString scancount_stage
              = tr ("Stage %1. Number of Scans per Triple (UV/vis): N/A ")
                    .arg (i + 1);
          cb_scancount->addItem (scancount_stage);
        }
      ...
      if (has_interference)
        {
          ...
          qDebug () << "RANGES INIT Interference: duration_sec , "
                       "scanint_sec_int, ncells_used -- "
          ...
          QString scint_str_int = QString::number (hms_scanint_int[1]) + "h "
          ...
          QString scancount_stage_int
              = tr ("Stage %1. Number of Scans per Cell (Interference): %2 ")
                    .arg (i + 1)
                    .arg (scancount_int);
      ...
      else // hide interference related widgets
        {
          ...
          QString scancount_stage_int
              = tr ("Stage %1. Number of Scans per Cell (Interference): N/A ")
                    .arg (i + 1);
          cb_scancount_int->addItem (scancount_stage_int);
        }
    }
  ...
  if (mainw->us_abde_mode)
      ...
      for (int ii = 0; ii < nrnchan; ii++)
        {
          ...
          if (kswavl > 1
              && iStwoOrMoreAnalytesSpectra_forChannel (chanName, msg_to_user,
                                                        "INIT", -1))
            {
  ...
  else
    {
      ...
      for (int ii = 0; ii < nrnchan; ii++)
        {
          // QString o_name = QString::number( ii ) + ":
          // ck_buff_spectrum_container";
          qDebug () << "[remove]o_name " << cc_buff_sp[ii]->objectName ();

          // QWidget* c_w = genL->findChild<QWidget *>( o_name,
          // Qt::FindChildrenRecursively ); //Qt::FindDirectChildrenOnly); if (
          // c_w != NULL )
          //   {
          //     qDebug() << "Widget found!";
...
// Save panel controls when about to leave the panel
void
US_ExperGuiRanges::savePanel ()
{
  DbgLv (1) << "EGwS:svP: nrnchan" << nrnchan << "nranges" << rpRange->nranges;
  rpRange->nranges = nrnchan; // Protocol channels               //ALEXEY not
                              // correct BUG-BUG
  ...
  for (int ii = 0; ii < nrnchan; ii++)
    {
      ...
      DbgLv (1) << "EGwS:svP:  ii" << ii << "wvl knt"
                << rpRange->chrngs[ii].wvlens.count ();
      qDebug () << "Ranges::save, hi, buff_bool -- "
                << rpRange->chrngs[ii].hi_rad
                << rpRange->chrngs[ii].abde_buffer_spectrum;
    }
}

// Get a specific panel value
QString
US_ExperGuiRanges::getSValue (const QString type)
{
  QString value ("");
...
// Get a specific panel integer value
int
US_ExperGuiRanges::getIValue (const QString type)
{
  int value = 0;
  if (type == "nrange")
    {
      value = rpRange->nranges;
    }
  else if (type == "alldone")
    {
      value = (status () > 0) ? 1 : 0;
...
// Get a specific panel double value
double
US_ExperGuiRanges::getDValue (const QString type)
{
  double value = 0.0;
  if (type == "dbdisk")
    {
...
// Get specific panel list values
QStringList
US_ExperGuiRanges::getLValue (const QString type)
{
  QStringList value ("");
...
// Get a specific panel value from a sibling panel
QString
US_ExperGuiRanges::sibSValue (const QString sibling, const QString type)
{
  return mainw->childSValue (sibling, type);
}
...
// Get a specific panel integer value from a sibling panel
int
US_ExperGuiRanges::sibIValue (const QString sibling, const QString type)
{
  return mainw->childIValue (sibling, type);
}
...
// Get a specific panel double value from a sibling panel
double
US_ExperGuiRanges::sibDValue (const QString sibling, const QString type)
{
  return mainw->childDValue (sibling, type);
}
...
// Get a specific panel list from a sibling panel
QStringList
US_ExperGuiRanges::sibLValue (const QString sibling, const QString type)
{
  return mainw->childLValue (sibling, type);
}
...
US_ExperGuiAProfile::initPanel ()
{
  ...
  qDebug () << "EXP_APROFILE_GUI -- initPanel(): sdiag->currProf.aprofname -- "
            << sdiag->currProf.aprofname;
}

void
US_ExperGuiAProfile::reset_sdiag (void)
{
  sdiag->reset ();

  qDebug () << "EXP_APROFILE_GUI RESET-- reset_sdiag(): "
               "sdiag->currProf.aprofname -- "
            << sdiag->currProf.aprofname;
}

void
US_ExperGuiAProfile::savePanel ()
{
...
QString
US_ExperGuiAProfile::getSValue (const QString type)
{
  QString value ("");

  if (type == "alldone" || type == "status")
    value = QString::number (getIValue (type));
...
// Get a specific panel integer value
int
US_ExperGuiAProfile::getIValue (const QString type)
{
  int value = 0;
  if (type == "alldone")
    {
...
// Get a specific panel double value
double
US_ExperGuiAProfile::getDValue (const QString type)
{
  double value = 0.;
  if (type == "dbdisk")
    {
...
// Get specific panel list values
QStringList
US_ExperGuiAProfile::getLValue (const QString type)
{
  QStringList value ("");
...
US_ExperGuiUpload::initPanel ()
{
  ...
  // finally, check for differences in currAProf's & loadAProf's nested  QMap<
  // QString, QMap < QString, US_ReportGMP > > ch_reports
  ...
  if (rps_differ)
    {
      ...
      DbgLv (1) << "EGUp:inP:  cPname" << cRP->protoname << "lPname"
                << lRP->protoname;
      DbgLv (1) << "EGUp:inP:  cInves" << cRP->investigator << "lInves"
                << lRP->investigator;
      DbgLv (1) << "EGUp:inP:  cPguid" << cRP->protoGUID << "lPguid"
                << lRP->protoGUID;
      DbgLv (1) << "EGUp:inP:  cOhost" << cRP->optimahost << "lOhost"
                << lRP->optimahost;
      DbgLv (1) << "EGUp:inP:  cTempe" << cRP->temperature << "lTempe"
                << lRP->temperature;
      DbgLv (1) << "EGUp:inP:   rpRotor diff"
                << (cRP->rpRotor != lRP->rpRotor);
      DbgLv (1) << "EGUp:inP:   rpSpeed diff"
                << (cRP->rpSpeed != lRP->rpSpeed);
      DbgLv (1) << "EGUp:inP:   rpCells diff"
                << (cRP->rpCells != lRP->rpCells);
      DbgLv (1) << "EGUp:inP:   rpSolut diff"
                << (cRP->rpSolut != lRP->rpSolut);
      DbgLv (1) << "EGUp:inP:   rpOptic diff"
                << (cRP->rpOptic != lRP->rpOptic);
      DbgLv (1) << "EGUp:inP:   rpRange diff"
                << (cRP->rpRange != lRP->rpRange);
      DbgLv (1) << "EGUp:inP:   rpAprof diff"
                << (cRP->rpAprof != lRP->rpAprof);
      DbgLv (1) << "EGUp:inP:   rpSubmt diff"
                << (cRP->rpSubmt != lRP->rpSubmt);
    }
  ...
  DbgLv (1) << "EGUp:inP: ck: run proj cent solu epro" << have_run << have_proj
            << have_cells << have_solus << have_range;
  proto_ena = (have_cells && have_solus && have_optic && have_range);
  ...
  if (mainw->automode)
    {
      subm_enab
          = (have_run && have_proj && proto_ena && mainw->connection_status
             && // ALEXEY: use top-level connection boolean!
    ...
    subm_enab = (have_run && have_proj && proto_ena
                 && mainw->connection_status); // ALEXEY: use top-level
                                               // connection boolean!
  ...
  pb_saverp->setEnabled (
      have_cells && have_solus && have_range
      && rps_differ); // ALEXEY: add check here is rps_differ == true
                      // (protocols differ)
      ...
      if (rpRotor->importData && !rpRotor->importDataDisk.isEmpty ())
        {
          ...
          connect (pb_submit, SIGNAL (clicked ()), this,
                   SLOT (submitExperiment_confirm_dataDisk ()));
        }
    }
  else
    {
      pb_submit->disconnect ();
      connect (pb_submit, SIGNAL (clicked ()), this,
               SLOT (submitExperiment_confirm_protDev ()));
  ...
  //[DataFromDisk] Check that channels & ranges correspond to those is
  //protocol:
  if (rpRotor->importData && !rpRotor->importDataDisk.isEmpty ())
    {
      ...
      if (!protocolToDataDisk (msg_to_user))
        {
          ...
          QMessageBox::critical (
              this, tr ("ATTENTION: Protocol Incorrectly Set"),
              msg_to_user.join ("\n")
                  + tr (
                      "\n\nProtocol does not correspond to the uploaded data! "
                      "\nIt appears the protocol was modified from it's "
                      "original state\n"
                      "corresponding to the uploaded from disk data. \n"
                      "Please re-upload data using 2. Lab/Rotor tab and do "
                      "not modify "
  ...
  //[ABDE] Here, check for existence of valid extinction profiles for analytes
  //(and optionally buffers) in the MWL-channels If not, inform user, disable
  // "Submit"/"Save Protocol" buttons
  if (mainw->us_abde_mode)
    {
      ...
      if (!ifMixedABDE (msg_to_user))
        {
          ...
          QMessageBox::critical (
              this, tr ("ATTENTION: Invalid Ranges Settings (ABDE)"),
              msg_to_user.join ("\n")
                  + tr ("\n\nCurrent Ranges settings do not correspond to "
                        "either multi-wavelength (MWL) or "
                        "single-wavelength (SWL) experiment."
                        "\nPlease modify wavelengths settings in the tab "
                        "7:Ranges, to ensure "
                        "all channels are either MWL or SWL."
                        "\n\nSaving protocol or run submission to the Optima "
                        "are not possible "
                        "until this problem is resolved."));
          return;
        }
          ...
          if (msg_to_user.join (",").contains ("Single Analyte Defined;"))
            {
              QMessageBox::critical (
                  ...
                  msg_to_user.join ("\n")
                      + tr ("\n\nThe solution for the above specified channel "
                            "has only "
                            "one analyte. At least two analytes with valid "
                            "extinction profiles "
                            "are required for the currently defined MWL-ABDE "
                            "experiment.\n"
                            "Please modify the solution, or select a "
                            "different one to satisfy these "
                            "requirements."
                            "\n\nSaving protocol or run submission to the "
                            "Optima are not possible "
                            "until this problem is resolved."));
            }
          else
            {
              QMessageBox::critical (
                  this, tr ("ATTENTION: Invalid Extinction Profiles (ABDE)"),
                  msg_to_user.join ("\n")
                      + tr ("\n\nPlease upload valid extinction profiles for "
                            "above specified analytes "
                            "and/or buffers using following UltraScan's "
          ...
          QMessageBox::critical (
              ...
              msg_to_user.join ("\n")
                  + tr ("\n\nPlease define references for all sample channels "
                        ...
                        "Saving protocol or run submission to the Optima are "
                        "not possible "
                        "until this problem is resolved."));
          return;
        }
      ...
      if (!samplesReferencesWvlsMatch (msg_to_user))
        {
          ...
          QMessageBox::critical (
              this,
              tr ("ATTENTION: Ranges Settings for Refs./Samples are Incorrect "
                  "(ABDE)"),
              msg_to_user.join ("\n")
                  + tr (
      ...
      if (oprof[kk].contains (rayleigh))
        {
          if (oprof[kk].section (":", 0, 0).contains ("sample"))
            {
              // qDebug() << "ITF channel name: " <<  oprof[ kk ].section( ":",
              // 0, 0 ).split(",")[0];
...
bool
US_ExperGuiUpload::protocolToDataDisk (QStringList &msg_to_user)
{
  bool all_matches = true;
  msg_to_user.clear ();

  // Cells, Optics  & Ranges
  QMap<QString, QStringList> runTypes_from_dataDisk,
      chann_ranges_from_dataDisk;
  ...
  if (chann_numbers_from_dataDisk.size () != rpCells->used.size ())
    {
      msg_to_user
          << "[Uploaded Data <-> Protocol] Numbers of Cells are mismatched!";
      return false;
    }
  for (int ii = 0; ii < rpCells->used.size (); ii++)
    {
      if (!chann_numbers_from_dataDisk.contains (
              QString::number (rpCells->used[ii].cell)))
        {
  ...
  int chopts_num = (!rpRotor->importData_absorbance_t)
                       ? chann_numbers_from_dataDisk.size () * 2
                       : chann_numbers_from_dataDisk.size ();
  if (chopts_num != rpOptic->chopts.size ())
    {
      msg_to_user << "[Uploaded Data <-> Protocol] Numbers of Optics cahnnels "
                     "are mismatched!";
      return false;
    }
  for (int ii = 0; ii < rpOptic->chopts.size (); ii++)
    {
      ...
      if (!chann_numbers_from_dataDisk.contains (ch_num))
        {
          msg_to_user << "[Uploaded Data <-> Protocol] Optics channels are "
                         "mismatched!";
          return false;
        }
      ...
      if (r_types != r_types_fromDisk)
        {
          msg_to_user
              << "[Uploaded Data <-> Protocol] Optics Types for channels "
                     + ch_num + " are mismatched!";
          return false;
        }
    }
  ...
  if (channames_from_dataDisk.size () != rpRange->nranges)
    {
      msg_to_user << "[Uploaded Data <-> Protocol] Numbers of Range channels "
                     "are mismatched!";
      return false;
    }
  ...
  for (int ii = 0; ii < rpRange->nranges; ii++)
    {
      ...
      if (!channames_from_dataDisk.contains (chan_red))
        {
          msg_to_user
              << "[Uploaded Data <-> Protocol] Range channels are mismatched!";
          return false;
        }
      ...
      if (nwavl != chann_ranges_from_dataDisk[chan_red].size ())
        {
          msg_to_user
              << "[Uploaded Data <-> Protocol] Wvls# for Range channel "
                     + chan_red + " are mismatched!";
          return false;
        }
      ...
      qDebug () << "DataDisk Wvl for chann: " << chan_red
                << chann_ranges_from_dataDisk[chan_red];
      qDebug () << "Protocol Wvl for chann: " << chan_red << all_wvls_list;

      if (all_wvls_list != chann_ranges_from_dataDisk[chan_red])
        {
          msg_to_user << "[Uploaded Data <-> Protocol] Wvls for channel "
                             + chan_red + " are mismatched!";
          return false;
        }
    }
...
bool
US_ExperGuiUpload::samplesReferencesWvlsMatch (QStringList &msg_to_user)
{
  bool all_matches = true;
  msg_to_user.clear ();
  ...
  qDebug () << "in samplesReferencesWvlsMatch(): chnns_names.size(), "
               "ref_chnns.size(), ref_use_chnns.size(), rpRange->nranges -- "
            << chnns_names.size () << ref_chnns.size ()
            << ref_use_chnns.size () << rpRange->nranges;

  // Create a QMap< QString(Reference), QStringList(Samples)>, to match Sample
  // channs to each Reference chan.
  ...
  for (rts = ref_to_samples.begin (); rts != ref_to_samples.end (); ++rts)
    {
      ...
      qDebug () << "in samplesReferencesWvlsMatch(): ref_chn_name, "
                   "sample_chn_names -- "
...
bool
US_ExperGuiUpload::matchRefSampleWvls (QString ref_chn_name, QString sn,
                                       QStringList &msg_to_user)
{
  bool matchedOK = true;
  ...
  for (int ii = 0; ii < rpRange->nranges; ii++)
    {
      ...
      qDebug () << "in matchRefSampleWvls(): over ranges: channel name, "
                   "ref_chn_name, sample_chn_name  -- "
                << channel << ref_chn_name << sn;
      qDebug () << "in matchRefSampleWvls(): over ranges: channel name, "
                   "ref_chn_name_red, sample_chn_name_red -- "
  ...
  qDebug ()
      << "in matchRefSampleWvls(): REF_chn_name, all_wvls_ref, nwavl_ref -- "
      << ref_chn_name << all_wvls_ref << nwavl_ref;
  qDebug () << "in matchRefSampleWvls(): SAMPLE_chn_name, all_wvls_sample, "
               "nwavl_sample -- "
...
bool
US_ExperGuiUpload::useReferenceNumbersSet (QStringList &msg_to_user)
{
  ...
  qDebug () << "in useReferenceNumbersSet(): chnns_names.size(), "
               "ref_chnns.size(), ref_use_chnns.size() -- "
            << chnns_names.size () << ref_chnns.size ()
            << ref_use_chnns.size ();
  ...
  for (int rn = 0; rn < ref_use_chnns.size (); ++rn)
    {
      ...
      if (ref_use_chnns[rn] < ref_chnn_min || ref_use_chnns[rn] > ref_chnn_max
          || ref_use_chnns[rn] == 0)
        {
  ...
  if (ref_use_chhs_number == 0)
    {
      msg_to_user << QString (
          tr ("All channels are set as \"Reference\"; Please define at least "
              "one non-reference channel..."));
      all_refs_set = false;
    }
...
bool
US_ExperGuiUpload::ifMixedABDE (QStringList &msg_to_user)
{
...
bool
US_ExperGuiUpload::extinctionProfilesExist (QStringList &msg_to_user)
{
  bool all_profiles_exist = true;
  msg_to_user.clear ();
  qDebug () << "Size rpSolut->nschan, rpRange->nranges -- " << rpSolut->nschan
            << rpRange->nranges;
  ...
  qDebug () << "in extinctionProfilesExist(): chnns_names.size(), "
               "ref_chnns.size(), ref_use_chnns.size(), rpRange->nranges -- "
            << chnns_names.size () << ref_chnns.size ()
            << ref_use_chnns.size () << rpRange->nranges;
      ...
      if (nwavl > 1)
        {
          if (!mwl_deconv)
            {
              // check if not a reference channel (that one can have a single
              // analyte)
              QString ch_name_m = channel.split (",")[0];
              ch_name_m.replace ("/", "");
              ch_name_m.simplified ();
              ch_name_m.replace (" ", "");
              qDebug () << "Ref_channs -- " << ref_channs << "ch_name_m -- "
                        << ch_name_m;
              ...
              for (int i = 0; i < num_analytes; ++i)
                {
                  ...
                  qDebug ()
                      << "Solution " << solution->solutionDesc
                  ...
                  // Check if ext. profile: (1) exists; (2) in range of specs
                  // channel-wvls.
                  QString a_desc = "ANALYTE: " + a_name;
                  if (!validExtinctionProfile (a_desc, all_wvls,
                                               analyte.extinction.keys (),
                                               msg_to_user))
              ...
              if (buff_req) // only if buffer spectrum required
                {
                  ...
                  if (!validExtinctionProfile (b_desc, all_wvls,
                                               buffer.extinction.keys (),
                                               msg_to_user))
                    all_profiles_exist = false;
                }
...
bool
US_ExperGuiUpload::validExtinctionProfile (QString desc,
                                           QList<double> all_wvls,
                                           QList<double> ext_prof,
                                           QStringList &msg_to_user)
{
  bool eprofile_ok;
  QString msg;
  ...
  if (!eprofile_ok)
    msg_to_user
        << QString (tr ("%1: Extinction profile %2")).arg (desc).arg (msg);

  return eprofile_ok;
}

bool
US_ExperGuiUpload::areReportMapsDifferent (US_AnaProfile aprof_curr,
                                           US_AnaProfile aprof_load)
{
  bool maps_different = false;

  QMap<QString, QMap<QString, US_ReportGMP> > aprof_curr_ch_reports
      = aprof_curr.ch_reports;
  QMap<QString, QMap<QString, US_ReportGMP> > aprof_load_ch_reports
      = aprof_load.ch_reports;
  ...
  qDebug () << "Top-level Report Maps Keys: curr, loaded -- "
            << aprof_curr_keys << aprof_load_keys;
  ...
  for (int i = 0; i < aprof_curr_keys.size (); i++)
    {
      QMap<QString, US_ReportGMP> triple_reports_curr
          = aprof_curr_ch_reports[aprof_curr_keys[i]];
      QMap<QString, US_ReportGMP> triple_reports_load
          = aprof_load_ch_reports[aprof_curr_keys[i]];
      ...
      qDebug () << "Triple's Report Maps Keys: curr, loaded -- "
                << triple_reports_curr_keys << triple_reports_load_keys;
      ...
      for (int j = 0; j < triple_reports_curr_keys.size (); j++)
        {
          US_ReportGMP report_curr
              = triple_reports_curr[triple_reports_curr_keys[j]];
          US_ReportGMP report_load
              = triple_reports_load[triple_reports_curr_keys[j]];
          ...
          if (report_curr.tot_conc != report_load.tot_conc
              ...
              || report_curr.experiment_duration
                     != report_load.experiment_duration
              || report_curr.experiment_duration_tol
                     != report_load.experiment_duration_tol)
            {
              maps_different = true;
              break;
            }
          ...
          if (report_curr.tot_conc_mask != report_load.tot_conc_mask
              ...
              || report_curr.experiment_duration_mask
                     != report_load.experiment_duration_mask
              || report_curr.integration_results_mask
                     != report_load.integration_results_mask
          ...
          if (report_curr.pseudo3d_mask != report_load.pseudo3d_mask
              || report_curr.pseudo3d_2dsait_s_ff0
                     != report_load.pseudo3d_2dsait_s_ff0
              || report_curr.pseudo3d_2dsait_s_d
                     != report_load.pseudo3d_2dsait_s_d
              || report_curr.pseudo3d_2dsait_mw_ff0
                     != report_load.pseudo3d_2dsait_mw_ff0
              || report_curr.pseudo3d_2dsait_mw_d
                     != report_load.pseudo3d_2dsait_mw_d
              || report_curr.pseudo3d_2dsamc_s_ff0
                     != report_load.pseudo3d_2dsamc_s_ff0
              || report_curr.pseudo3d_2dsamc_s_d
                     != report_load.pseudo3d_2dsamc_s_d
              || report_curr.pseudo3d_2dsamc_mw_ff0
                     != report_load.pseudo3d_2dsamc_mw_ff0
              || report_curr.pseudo3d_2dsamc_mw_d
                     != report_load.pseudo3d_2dsamc_mw_d
              || report_curr.pseudo3d_pcsa_s_ff0
                     != report_load.pseudo3d_pcsa_s_ff0
              || report_curr.pseudo3d_pcsa_s_d != report_load.pseudo3d_pcsa_s_d
              || report_curr.pseudo3d_pcsa_mw_ff0
                     != report_load.pseudo3d_pcsa_mw_ff0
              || report_curr.pseudo3d_pcsa_mw_d
                     != report_load.pseudo3d_pcsa_mw_d)
            {
              maps_different = true;
              break;
            }
          ...
          for (int k = 0; k < report_curr.reportItems.size (); k++)
            {
              US_ReportGMP::ReportItem curr_reportItem
                  = report_curr.reportItems[k];
              US_ReportGMP::ReportItem load_reportItem
                  = report_load.reportItems[k];
              ...
              if (curr_reportItem.type != load_reportItem.type
                  ...
                  || curr_reportItem.integration_val
                         != load_reportItem.integration_val
                  || curr_reportItem.tolerance != load_reportItem.tolerance
                  || curr_reportItem.total_percent
                         != load_reportItem.total_percent
                  || curr_reportItem.combined_plot
                         != load_reportItem.combined_plot
                  || curr_reportItem.ind_combined_plot
                         != load_reportItem.ind_combined_plot)
                {
                  maps_different = true;
                  break;
                }
...
// Save panel controls when about to leave the panel
void
US_ExperGuiUpload::savePanel ()
{
}

// Get a specific panel value
QString
US_ExperGuiUpload::getSValue (const QString type)
{
  QString value ("");

  if (type == "alldone" || type == "status" || type == "len_xml"
      || type == "len_json")
    value = QString::number (getIValue (type));
  else if (type == "xml")
...
// Get a specific panel integer value
int
US_ExperGuiUpload::getIValue (const QString type)
{
  int value = 0;
  if (type == "alldone")
    {
  ...
  else if (type == "len_xml")
    {
      value = rpSubmt->us_xml.length ();
    }
  else if (type == "len_json")
    {
      value = rpSubmt->op_json.length ();
    }
  return value;
}
...
// Get a specific panel double value
double
US_ExperGuiUpload::getDValue (const QString type)
{
  double value = 0;
  if (type == "dbdisk")
    {
...
// Get specific panel list values
QStringList
US_ExperGuiUpload::getLValue (const QString type)
{
  QStringList value ("");
...
// Get a specific panel value from a sibling panel
QString
US_ExperGuiUpload::sibSValue (const QString sibling, const QString type)
{
  return mainw->childSValue (sibling, type);
}
...
// Get a specific panel integer value from a sibling panel
int
US_ExperGuiUpload::sibIValue (const QString sibling, const QString type)
{
  return mainw->childIValue (sibling, type);
}
...
// Get a specific panel double value from a sibling panel
double
US_ExperGuiUpload::sibDValue (const QString sibling, const QString type)
{
  return mainw->childDValue (sibling, type);
}
...
// Get a specific panel list from a sibling panel
QStringList
US_ExperGuiUpload::sibLValue (const QString sibling, const QString type)
{
  return mainw->childLValue (sibling, type);
}
