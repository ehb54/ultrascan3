...
void US_ExperimentMain::statUpdate() {
   ...
   statflag = epanGeneral->status() + epanRotor->status() + epanSpeeds->status() + epanCells->status()
              + epanSolutions->status() + epanOptical->status() + epanRanges->status() + epanAProfile->status()
              + epanUpload->status();
...
void US_ExperimentMain::enable_tabs_buttons_readonly(void) {
   ...
   for (int i = 1; i < tabWidget->count(); i++) {
      ...
      for (int i = 0; i < allPButtons.count(); i++) {
         if (
            (allPButtons[ i ]->text()).contains("View Solution Details")
      ...
      for (int i = 0; i < allCBoxes.count(); i++) {
         if ((allCBoxes[ i ]->currentText()).contains("Speed Profile") || (allCBoxes[ i ]->currentText()).contains(": Optima"))
   ...
   for (int ii = 0; ii < tabWidget->count(); ii++) {
      ...
      for (int i = 0; i < allPButtons.count(); i++) {
         if (
            ...
            || (allPButtons[ i ]->text()).contains("View Experiment Details")
            || (allPButtons[ i ]->text()).contains("Test Connection")
            || (allPButtons[ i ]->text()).contains("Add to List") || (allPButtons[ i ]->text()).contains("Remove Last"))
            allPButtons[ i ]->setEnabled(true);
         else
            allPButtons[ i ]->setEnabled(false);
      }
      ...
      for (int i = 0; i < allCBoxes.count(); i++) {
         ...
         // else
         if (
            (allCBoxes[ i ]->objectName() == "ChooseOper") || (allCBoxes[ i ]->objectName() == "ChooseRev")
            || (allCBoxes[ i ]->objectName() == "ChooseAppr") || (allCBoxes[ i ]->objectName() == "ChooseSme")) {
            allCBoxes[ i ]->setEnabled(true);
         }
         else
            allCBoxes[ i ]->setEnabled(false);
...
bool US_ExperimentMain::centpInfo(const QString par1, US_AbstractCenterpiece &par2) {
...
void US_ExperGuiGeneral::check_user_level() {
   ...
   if (inv_lev > 2) { // All admin users and above are enabled
      ...
      qDebug() << "instr_opers.count(); check_user_level(): inv_lev -- " << instr_opers.count() << "; " << inv_lev;
      ...
      for (int ii = 0; ii < lablist.count(); ii++) { // Look through lab info
         ...
         for (int jj = 0; jj < instruments.count(); jj++) { // Look at instruments in the lab
            ...
            for (int kk = 0; kk < operators.count(); kk++) { // Look at operators permitted on the instrument
...
int US_ExperGuiGeneral::status() {
   bool is_done = !(currProto->runname.isEmpty() || currProto->project.isEmpty());
   return (is_done ? 1 : 0);
}
...
void US_ExperGuiRotor::initPanel() {
   ...
   QString cal_entr = QString::number(rpRotor->calID) + ": " + rpRotor->calibration;
   QString rot_entr = QString::number(rpRotor->rotID) + ": " + rpRotor->rotor;
   setCbCurrentText(cb_lab, QString::number(rpRotor->labID) + ": " + rpRotor->laboratory);
   setCbCurrentText(cb_rotor, rot_entr);

   DbgLv(1) << "EGRo: inP: calib_entr" << cal_entr;
   if (cb_calibr->findText(cal_entr) < 0) { // Repopulate calibration combo box to have current calibration
   ...
   QString optima_name = QString::number(rpRotor->instID) + ": " + rpRotor->instrname;
   ...
   if (cb_optima->findText(optima_name) < 0) {
      ...
      msg_instr_avail->setText(tr("Loaded protocol specified <b>%1</b> as the instrument in use.<br><br>"
                                  "However, due to its current unavailability, the instrument in use will be changed "
                                  "to the first available intrument, <b>%2</b>.")
   ...
   //operator
   qDebug() << "init Labs: operID + name -- " << QString::number(rpRotor->operID) + ": " + rpRotor->opername;

   setCbCurrentText(cb_operator, QString::number(rpRotor->operID) + ": " + rpRotor->opername);
   ...
   DbgLv(1) << "EGRo: inP:  rotID" << rpRotor->rotID << "rotor" << rpRotor->rotor << "cb_rotor text"
            << cb_rotor->currentText();
   ...
   qDebug() << "In Rotor: mainw->automode, mainw->usmode -- " << mainw->automode << ", " << mainw->usmode;
   if (mainw->usmode) {
...
void US_ExperGuiRotor::init_grevs(void) {
   ...
   while (db.next()) {
      ...
      //populate
      cb_choose_rev->addItem(QString::number(g_invID) + ". " + g_lastName + ", " + g_firstName);
   }
}
...
void US_ExperGuiRotor::init_gapprs(void) {
   ...
   while (db.next()) {
      ...
      //populate
      cb_choose_appr->addItem(QString::number(g_invID) + ". " + g_lastName + ", " + g_firstName);
   }
}
...
void US_ExperGuiRotor::init_gsmes(void) {
   ...
   while (db.next()) {
      ...
      //populate
      cb_choose_sme->addItem(QString::number(g_invID) + ". " + g_lastName + ", " + g_firstName);
   }
}
...
void US_ExperGuiRotor::test_optima_connection() {
   ...
   if (xpnhost.isEmpty() || QString::number(xpnport).isEmpty() || dbname.isEmpty() || dbuser.isEmpty() || dbpasw.isEmpty()) {
...
void US_ExperGuiRotor::savePanel() {
   ...
   qDebug() << "OPERATORID / INSTRUMENT / ExpType in SAVE: " << rpRotor->operID << ", " << rpRotor->opername << " / "
            << rpRotor->instID << ", " << rpRotor->instrname << " / " << rpRotor->exptype;

   DbgLv(1) << "EGRo:  svP:  rotID" << rpRotor->rotID << "rotor" << rpRotor->rotor << "cb_rotor text" << rot;
   ...
   qDebug() << "[in Rotor: savePanel()]: exptype, expType_old,  expType_changed? " << exptype << ", " << expType_old
            << ": " << expType_changed;
...
int US_ExperGuiRotor::status() {
   bool is_done = !(rpRotor->laboratory.isEmpty() || rpRotor->rotor.isEmpty() || rpRotor->calibration.isEmpty());
   return (is_done ? 2 : 0);
}
...
void US_ExperGuiSpeeds::initPanel() {
   ...
   le_maxrpm->setText(tr("Maximum speed for %1 rotor:  %2 rpm").arg(arotor).arg(speedmax));
   ...
   for (int ii = 0; ii < nspeed; ii++) {
      ...
      DbgLv(1) << "EGSp:inP:  ii" << ii << "speed accel durat delay scnint" << ssvals[ ii ][ "speed" ]
               << ssvals[ ii ][ "accel" ] << ssvals[ ii ][ "duration" ] << ssvals[ ii ][ "delay" ]
...
void US_ExperGuiSpeeds::savePanel() {
   ...
   for (int ii = 0; ii < nspeed; ii++) {
      ...
      DbgLv(1) << "EGSp:svP:  ii" << ii << "speed accel durat delay scnint" << ssvals[ ii ][ "speed" ]
               << ssvals[ ii ][ "accel" ] << ssvals[ ii ][ "duration" ] << ssvals[ ii ][ "delay" ]
...
QStringList US_ExperGuiSpeeds::getLValue(const QString type) {
   ...
   if (type == "profiles") { // Compose list of all speed-step values (4 lines per step)
      value.clear();

      for (int ii = 0; ii < rpSpeed->nstep; ii++) { // Build list of QString forms of speed-step double
...
void US_ExperGuiCells::initPanel() {
   ...
   sl_bals << tr("empty (counterbalance)") << tr("Beckman counterbalance") << tr("Titanium counterbalance")
   ...
   for (int ii = 0; ii < nholes; ii++) { //
      ...
      if (
         (ii != icbal && cc_cenps[ ii ]->currentText().contains(tr("counterbalance")))
         || (ii != icbal && !cc_cenps[ ii ]->currentText().contains(tr("counterbalance")))) { // Centerpiece when list is counterbalances: reset list
         ...
         && !cc_cenps[ ii ]->currentText().contains(
            tr("counterbalance"))) { // Counterbalance when list is centerpieces: reset list
      ...
      for (int jj = 0; jj < nused; jj++) { // Search to see if current is among the used cells
         DbgLv(1) << "EGCe:inP:     ii" << ii << "jj" << jj << "cell cellj" << cell << rpCells->used[ jj ].cell;
         if (cell == rpCells->used[ jj ].cell) { // This is a used cell:  populate a table row
...
void US_ExperGuiCells::savePanel() {
   ...
   for (int ii = 0; ii < nholes; ii++) {
      ...
      if (jj >= 0) {
         ...
         DbgLv(1) << "EGCe:svP:     cp cb wi" << rpCells->used[ jj ].centerpiece << rpCells->used[ jj ].cbalance
                  << rpCells->used[ jj ].windows;
      }
   }
}
...
int US_ExperGuiCells::getIValue(const QString type) {
   ...
   if (type == "ncells") {
      ...
      for (int ii = 0; ii < rpCells->nused; ii++) {
         ...
         if (chx > 0) {
            int ncchn = QString(centp).left(chx).section(" ", -1, -1).simplified().toInt();
            nchans += ncchn;
         }
      }
...
QStringList US_ExperGuiCells::getLValue(const QString type) {
   ...
   if (type == "centerpieces") { // Return row strings where a centerpiece is selected
      int nholes = sibIValue("rotor", "nholes");
   ...
   else if (type == "cpchannels") { // Return channel strings where a centerpiece is selected
      ...
      for (int ii = 0; ii < rpCells->nused; ii++) { // Examine the used cells
         ...
         if (chx > 0) { // Complete string for each channel in centerpiece
            int ncchn = QString(centp).left(chx).section(" ", -1, -1).simplified().toInt();
            for (int jj = 0; jj < ncchn; jj++) { // Save channel strings (e.g., "1 / A", "1 / B", "2 / A", ...)
...
void US_ExperGuiSolutions::initPanel() {
   ...
   for (int ii = 0; ii < nchant; ii++) {
      ...
      qDebug() << "INIT SOLS, channel, ra_data_type -- " << channel << rpRotor->importData_absorbance_t;
   ...
   for (int ii = 0; ii < mxrow; ii++) {
      ...
      connect(cb_solution, SIGNAL(currentIndexChanged(int)), this, SLOT(changeSolu(int)));
   }
}
...
void US_ExperGuiSolutions::savePanel() {
   ...
   for (int ii = 0; ii < mxrow; ii++) { // Fill panel lists and parameters from GUI elements
   ...
   for (int ii = 0; ii < nchanf; ii++) { // Now fill protocol from internal lists and parameters
...
QString US_ExperGuiSolutions::getSValue(const QString type) {
   ...
   if (
      type == "nchant" || type == "nchanf" || type == "nusols" || type == "alldone"
...
QStringList US_ExperGuiSolutions::getLValue(const QString type) {
   ...
   if (type == "solutions") { // Build a list of unique solutions
      ...
      for (int ii = 0; ii < rpSolut->nuniqs; ii++) {
         ...
         for (int jj = 0; jj < rpSolut->nschan; jj++) { // List channels that have this solution
   ...
   else if (type == "channel_solutions") { // Build a "channel : solution" list for used channels
      for (int ii = 0; ii < rpSolut->nschan; ii++) {
         value << (rpSolut->chsols[ ii ].channel + " : " + rpSolut->chsols[ ii ].solution);
      }
   }
...
int US_ExperGuiSolutions::status() {
   ...
   DbgLv(1) << "EGSo:st: nchant" << nchant << "nchanf" << nchanf << " is_done" << is_done;
   return (is_done ? 16 : 0);
}
...
void US_ExperGuiOptical::initPanel() {
   ...
   for (int ii = 0; ii < nochan; ii++) {
      ...
      DbgLv(1) << "EGOp:inP:  ii" << ii << "channel" << channel << "scan1-3" << scan1 << scan2 << scan3;
...
void US_ExperGuiOptical::savePanel() {
   ...
   for (int ii = 0; ii < nochan; ii++) {
      ...
      DbgLv(1) << "EGOp:svP:     kchkd" << kchkd << "scans:" << rpOptic->chopts[ ii ].scan1
...
QStringList US_ExperGuiOptical::getLValue(const QString type) {
   ...
   if (type == "profiles") {
      ...
      for (int ii = 0; ii < rpOptic->nochan; ii++) {
         ...
         QString scan2 = rpOptic->chopts[ ii ].scan2;
         QString scan3 = rpOptic->chopts[ ii ].scan3;
         DbgLv(1) << "EGOp: gPL: ii" << ii << "channel" << channel << "scan1-3" << scan1 << scan2 << scan3;
         QString pentry = channel;
         int noptis = 0;

         if (!scan1.isEmpty() && (scan1 != notinst)) { // Checked optical system:  add to profile entry
            pentry += (":" + scan1);
            noptis++;
         }

         if (!scan2.isEmpty() && (scan2 != notinst)) { // Checked optical system:  add to profile entry
            pentry += (":" + scan2);
            noptis++;
         }

         if (!scan3.isEmpty() && (scan3 != notinst)) { // Checked optical system:  add to profile entry
            pentry += (":" + scan3);
            noptis++;
         }
...
int US_ExperGuiOptical::status() {
   ...
   for (int ii = 0; ii < nochan; ii++) { // Loop to count visible rows and ones with at least one check
      QString channel = rpOptic->chopts[ ii ].channel;
      QString scan1 = rpOptic->chopts[ ii ].scan1;
      QString scan2 = rpOptic->chopts[ ii ].scan2;
      QString scan3 = rpOptic->chopts[ ii ].scan3;
      DbgLv(1) << "EGOp:st:  ii" << ii << "ch s1 s2 s3" << channel << scan1 << scan2 << scan3;
      ...
      if (have_uvvis)
         nuvvis++;
      DbgLv(1) << "EGOp:st:    hvu hvv nuch nuvv" << have_used << have_uvvis << nuchan << nuvvis;
   }
...
void US_ExperGuiRanges::initPanel() {
   ...
   for (int ii = 0; ii < nrnchan; ii++) {
      ...
      if (kswavl == 0)
         ...
         labwlr = tr("%1,  %2 to %3").arg(kswavl).arg(swvlens[ ii ][ 0 ]).arg(swvlens[ ii ][ kswavl - 1 ]);
      cc_labls[ ii ]->setText(channel);
      ...
      if (channel.contains(
             tr("reference"))) //ALEXEY do not allow to set wavelengths/radial ranges for B channels (reference)
      {
   ...
   for (int i = 0; i < nsp; i++) {
      ...
      if (has_absorbance) {
         ...
         QString scint_str = QString::number(hms_scanint[ 1 ]) + "h " + QString::number(hms_scanint[ 2 ]) + "m "
      ...
      if (has_interference) {
         ...
         qDebug() << "RANGES INIT Interference: duration_sec , scanint_sec_int, ncells_used -- " << duration_sec
                  << scanint_sec_int << ncells_used_int;
         ...
         QString scint_str_int = QString::number(hms_scanint_int[ 1 ]) + "h " + QString::number(hms_scanint_int[ 2 ])
                                 + "m " + QString::number(hms_scanint_int[ 3 ]) + "s";
         ...
         QString scancount_stage_int
            = tr("Stage %1. Number of Scans per Cell (Interference): %2 ").arg(i + 1).arg(scancount_int);
...
void US_ExperGuiRanges::savePanel() {
   ...
   for (int ii = 0; ii < nrnchan; ii++) {
      ...
      for (int jj = 0; jj < swvlens[ ii ].count(); jj++) { // Wavelength values from selected wavelengths
...
void US_ExperGuiAProfile::initPanel() {
   ...
   DbgLv(1) << "EGAp:inP:  sdiag inherit" << "proto GUID ID" << mainw->currProto.protoGUID << mainw->currProto.protoID;
...
void US_ExperGuiUpload::initPanel() {
   ...
   DbgLv(1) << "EGUp:inP: ck: run proj cent solu epro" << have_run << have_proj << have_cells << have_solus
            << have_range;
   ...
   if (mainw->automode) {
      ...
      subm_enab
         = (have_run && have_proj && proto_ena && mainw->connection_status); // ALEXEY: use top-level connection boolean!
   ...
   pb_saverp->setEnabled(
      ...
      && rps_differ); // ALEXEY: add check here is rps_differ == true (protocols differ)
   ...
   if (!mainw->us_prot_dev_mode) {
      ...
      if (rpRotor->importData && !rpRotor->importDataDisk.isEmpty()) {
         ...
         connect(pb_submit, SIGNAL(clicked()), this, SLOT(submitExperiment_confirm_dataDisk()));
      }
   }
   else {
      pb_submit->disconnect();
      connect(pb_submit, SIGNAL(clicked()), this, SLOT(submitExperiment_confirm_protDev()));
   ...
   if (mainw->us_abde_mode) {
      ...
      if (!samplesReferencesWvlsMatch(msg_to_user)) {
         ...
         QMessageBox::critical(
            this, tr("ATTENTION: Ranges Settings for Refs./Samples are Incorrect (ABDE)"),
            msg_to_user.join("\n")
...
bool US_ExperGuiUpload::protocolToDataDisk(QStringList &msg_to_user) {
   ...
   //Optics
   qDebug() << "chann_numbers_from_dataDisk, rpOptic->chopts.size() -- " << chann_numbers_from_dataDisk
            << rpOptic->chopts.size();
   int chopts_num = (!rpRotor->importData_absorbance_t) ? chann_numbers_from_dataDisk.size() * 2
   ...
   //Ranges
   qDebug() << "channames_from_dataDisk, rpRange->nranges -- " << channames_from_dataDisk << rpRange->nranges;
...
bool US_ExperGuiUpload::samplesReferencesWvlsMatch(QStringList &msg_to_user) {
   ...
   qDebug() << "in samplesReferencesWvlsMatch(): chnns_names.size(), ref_chnns.size(), ref_use_chnns.size(), "
               "rpRange->nranges -- "
   ...
   for (rts = ref_to_samples.begin(); rts != ref_to_samples.end(); ++rts) {
      ...
      qDebug() << "in samplesReferencesWvlsMatch(): ref_chn_name, sample_chn_names -- " << ref_chn_name
               << sample_chn_names;
...
bool US_ExperGuiUpload::matchRefSampleWvls(QString ref_chn_name, QString sn, QStringList &msg_to_user) {
   ...
   for (int ii = 0; ii < rpRange->nranges; ii++) {
      ...
      qDebug() << "in matchRefSampleWvls(): over ranges: channel name, ref_chn_name, sample_chn_name  -- " << channel
               << ref_chn_name << sn;
   ...
   qDebug() << "in matchRefSampleWvls(): REF_chn_name, all_wvls_ref, nwavl_ref -- " << ref_chn_name << all_wvls_ref
            << nwavl_ref;
   qDebug() << "in matchRefSampleWvls(): SAMPLE_chn_name, all_wvls_sample, nwavl_sample -- " << sn << all_wvls_sample
            << nwavl_sample;
   ...
   //messages
   if (!matchedOK)
      msg_to_user << QString(tr("REF channel: %1: Ranges: %2%3")).arg(ref_chn_name).arg(msg).arg(sn);
...
bool US_ExperGuiUpload::useReferenceNumbersSet(QStringList &msg_to_user) {
   ...
   for (int rn = 0; rn < ref_use_chnns.size(); ++rn) {
      ...
      if (ref_use_chnns[ rn ] < ref_chnn_min || ref_use_chnns[ rn ] > ref_chnn_max || ref_use_chnns[ rn ] == 0) {
         QString chnn_name = chnns_names[ rn ];
         msg_to_user << QString(tr("%1: \"Use Reference#\" field is %2")).arg(chnn_name).arg("not set, or out of range");
   ...
   if (ref_use_chhs_number == 0) {
      msg_to_user << QString(
         tr("All channels are set as \"Reference\"; Please define at least one non-reference channel..."));
      all_refs_set = false;
   }
...
bool US_ExperGuiUpload::extinctionProfilesExist(QStringList &msg_to_user) {
   ...
   msg_to_user.clear();
   qDebug() << "Size rpSolut->nschan, rpRange->nranges -- " << rpSolut->nschan << rpRange->nranges;
   ...
   qDebug() << "in extinctionProfilesExist(): chnns_names.size(), ref_chnns.size(), ref_use_chnns.size(), "
               "rpRange->nranges -- "
            << chnns_names.size() << ref_chnns.size() << ref_use_chnns.size() << rpRange->nranges;
   ...
   for (int ii = 0; ii < rpRange->nranges; ii++) {
      ...
      if (nwavl > 1) {
         if (!mwl_deconv) {
            ...
            ch_name_m.replace(" ", "");
            qDebug() << "Ref_channs -- " << ref_channs << "ch_name_m -- " << ch_name_m;
            ...
            for (int i = 0; i < num_analytes; ++i) {
               ...
               qDebug() << "Solution " << solution->solutionDesc << ", (GUID)Analyte " << "(" << a_GUID << ")" << a_name
               ...
               qDebug() << "[Analyte]Extinction Profile wvls: " << analyte.extinction.keys();
            ...
            if (buff_req) //only if buffer spectrum required
            {
               ...
               qDebug() << "Solution " << solution->solutionDesc << ", (ID)Buffer " << "(" << b_ID << ")" << b_name;

               buffer.extinction.clear();
               buffer.readFromDB(&db, b_ID);

               //QMap <double, double> extinction[ wavelength ] <=> value
               qDebug() << "[Buffer]Extinction Profile wvls: " << buffer.extinction.keys();
...
bool US_ExperGuiUpload::validExtinctionProfile(
   QString desc, QList<double> all_wvls, QList<double> ext_prof, QStringList &msg_to_user) {
   bool eprofile_ok;
   QString msg;
...
bool US_ExperGuiUpload::areReportMapsDifferent(US_AnaProfile aprof_curr, US_AnaProfile aprof_load) {
   ...
   qDebug() << "Top-level Report Maps Keys: curr, loaded -- " << aprof_curr_keys << aprof_load_keys;
   ...
   for (int i = 0; i < aprof_curr_keys.size(); i++) {
      ...
      qDebug() << "Triple's Report Maps Keys: curr, loaded -- " << triple_reports_curr_keys << triple_reports_load_keys;
      ...
      for (int j = 0; j < triple_reports_curr_keys.size(); j++) {
         ...
         if (
            report_curr.tot_conc != report_load.tot_conc || report_curr.tot_conc_tol != report_load.tot_conc_tol
            || report_curr.rmsd_limit != report_load.rmsd_limit || report_curr.av_intensity != report_load.av_intensity
         ...
         if (
            report_curr.tot_conc_mask != report_load.tot_conc_mask
         ...
         if (
            report_curr.pseudo3d_mask != report_load.pseudo3d_mask
         ...
         for (int k = 0; k < report_curr.reportItems.size(); k++) {
            ...
            if (
               curr_reportItem.type != load_reportItem.type || curr_reportItem.method != load_reportItem.method
...
QString US_ExperGuiUpload::getSValue(const QString type) {
   ...
   if (type == "alldone" || type == "status" || type == "len_xml" || type == "len_json")
      value = QString::number(getIValue(type));
   else if (type == "xml")
