...
QString US_ExperimentMain::childSValue(const QString child,
                                       const QString type) {
  QString value("");
  if (child == "general") {
...
QStringList US_ExperimentMain::childLValue(const QString child,
                                           const QString type) {
  QStringList value;
...
void US_ExperimentMain::newPanel(int panx) {
  DbgLv(1) << "newPanel panx=" << panx << "prev.panx=" << curr_panx << "usmode"
           << usmode;
...
void US_ExperimentMain::statUpdate() {
  ...
  statflag =
      epanGeneral->status() + epanRotor->status() + epanSpeeds->status() +
      epanCells->status() + epanSolutions->status() + epanOptical->status() +
      epanRanges->status() + epanAProfile->status() + epanUpload->status();
...
void US_ExperimentMain::enable_tabs_buttons_readonly(void) {
  ...
  for (int i = 1; i < tabWidget->count(); i++) {
    ...
    for (int i = 0; i < allSBoxes.count(); i++) allSBoxes[i]->setEnabled(false);
...
int US_ExperimentMain::getProtos(QStringList& par1, QList<QStringList>& par2) {
...
use_db=false;
  ...
  if (inv_lev < 3 && mainw->automode) {
    ...
    for (op = ul2_operator_for_optima.begin();
      ...
      if (!op.value()) mainw->instruments_no_permit << iname;
    }
  }
  //////////////////
}
...
void US_ExperGuiGeneral::check_empty_runname(const QString& /*str*/) {
   ...
   {
#endif
  if (!le_project->text().isEmpty()) emit set_tabs_buttons_active();
#if 0
   }
#endif
}
...
void US_ExperGuiGeneral::check_user_level() {
  ...
  if (inv_lev > 2) {  // All admin users and above are enabled
    usr_enab = true;
  }

  else {  // Non-admin users enabled if they have instrument permit
    ...
    for (int ii = 0; ii < lablist.count(); ii++) {  // Look through lab info
      ...
      for (int kk = 0; kk < instruments.count(); kk++) {
        ...
        // Skip any non-Optima
        if (!inname.startsWith("Optima")) continue;

        ul2_operator_for_optima[inname] = false;
      }

      // Go over instruments, and determine if the current user an operator: per
      // instrument
      for (int jj = 0; jj < instruments.count();
           jj++) {  // Look at instruments in the lab
        ...
        for (int kk = 0; kk < operators.count();
             kk++) {  // Look at operators permitted on the instrument
  ...
  if (!usr_enab) {  // User not enabled to set investigator
...
QString US_ExperGuiGeneral::sibSValue(const QString sibling,
                                      const QString type) {
...
double US_ExperGuiGeneral::sibDValue(const QString sibling,
                                     const QString type) {
  return mainw->childDValue(sibling, type);
}

// Get a specific panel list from a sibling panel
QStringList US_ExperGuiGeneral::sibLValue(const QString sibling,
                                          const QString type) {
...
void US_ExperGuiRotor::initPanel() {
  ...
  if (mainw->global_reset) mainw->global_reset = false;
  qDebug() << "GLOBAL RESET: " << mainw->global_reset;
  ...
  setCbCurrentText(
      cb_lab, QString::number(rpRotor->labID) + ": " + rpRotor->laboratory);
  setCbCurrentText(cb_rotor, rot_entr);

  DbgLv(1) << "EGRo: inP: calib_entr" << cal_entr;
  if (cb_calibr->findText(cal_entr) <
      0) {  // Repopulate calibration combo box to have current calibration
  ...
  if (cb_optima->findText(optima_name) < 0) {
    ...
    msg_instr_avail->setText(
        tr("Loaded protocol specified <b>%1</b> as the instrument in "
           "use.<br><br>"
           "However, due to its current unavailability, the instrument in use "
           "will be changed to the first available intrument, <b>%2</b>.")
  ...
  qDebug() << "In Rotor: mainw->automode, mainw->usmode -- " << mainw->automode
           << ", " << mainw->usmode;
  if (mainw->usmode) {
...
void US_ExperGuiRotor::savePanel() {
  ...
  for (int ii = 0; ii < labs.count(); ii++) {
    if (rpRotor->labID == labs[ii].ID) rpRotor->labGUID = labs[ii].GUID;
  }
  ...
  for (int ii = 0; ii < arotors.count(); ii++) {
    if (rpRotor->absID == arotors[ii].ID) rpRotor->absGUID = arotors[ii].GUID;
  }
  ...
  if (!first_time_init) {
    if (exptype == "Buoyancy") {
      ...
      if (expType_changed) mainw->abde_sv_mode_change_reset_reports("ABDE");
    } else {
      mainw->unset_abde_mode_aprofile();
      if (expType_changed) mainw->abde_sv_mode_change_reset_reports("SV");
    }
  } else {
    if (exptype == "Buoyancy") {
...
// Get a specific panel list from a sibling panel
QStringList US_ExperGuiRotor::sibLValue(const QString sibling,
                                        const QString type) {
  return mainw->childLValue(sibling, type);
}
...
QStringList US_ExperGuiSpeeds::getLValue(const QString type) {
  ...
  if (type ==
      "profiles") {  // Compose list of all speed-step values (4 lines per step)
    value.clear();

    for (int ii = 0; ii < rpSpeed->nstep;
         ii++) {  // Build list of QString forms of speed-step double
...
// Get a specific panel value from a sibling panel
QString US_ExperGuiSpeeds::sibSValue(const QString sibling,
                                     const QString type) {
  return mainw->childSValue(sibling, type);
}
...
// Get a specific panel list from a sibling panel
QStringList US_ExperGuiSpeeds::sibLValue(const QString sibling,
                                         const QString type) {
  return mainw->childLValue(sibling, type);
}
...
void US_ExperGuiCells::initPanel() {
  ...
  for (int ii = 0; ii < nholes; ii++) {  //
    ...
    if ((ii != icbal &&
        ...
        (ii != icbal &&
         !cc_cenps[ii]->currentText().contains(
             tr("counterbalance")))) {  // Centerpiece when list is
                                        // counterbalances: reset list

      // qDebug() << "CELL NOT LAST BUT counterbalance: text,  " << cc_cenps[ ii
      // ]->currentText();
      cc_cenps[ii]->clear();
      ...
      cc_cenps[ii]->addItems(cpnames);     // Choose from centerpieces
             ...
             !cc_cenps[ii]->currentText().contains(
                 tr("counterbalance"))) {  // Counterbalance when list is
                                           // centerpieces: reset list

      // qDebug() << "CELL LAST BUT not counterbalance: text,  " << cc_cenps[ ii
      // ]->currentText();
      cc_cenps[ii]->clear();
    ...
    for (int jj = 0; jj < nused;
         jj++) {  // Search to see if current is among the used cells
      DbgLv(1) << "EGCe:inP:     ii" << ii << "jj" << jj << "cell cellj" << cell
               << rpCells->used[jj].cell;
      if (cell == rpCells->used[jj]
                      .cell) {  // This is a used cell:  populate a table row
        ...
        if (ii != icbal) {
          ...
          // rpCells->used[ jj ].cbalance; qDebug() << "POPULATING INNER CELL -
          // centerpiece!!!!! :  " << rpCells->used[ jj ].centerpiece;
          cenbal = rpCells->used[jj].centerpiece;
        } else {
          // qDebug() << "POPULATING LAST CELL - cbalance   !!!!! :  " <<
          // rpCells->used[ jj ].cbalance; qDebug() << "POPULATING LAST CELL -
          // centerpiece!!!!! :  " << rpCells->used[ jj ].centerpiece; if (
          // rpCells->used[ jj ].cbalance.contains( tr( "centerpiece" ) ) )
...
void US_ExperGuiCells::savePanel() {
  ...
  for (int ii = 0; ii < nholes; ii++) {  // Count used (non-empty) cell rows
...
int US_ExperGuiCells::getIValue(const QString type) {
  ...
  if (type == "ncells") {
    ...
    for (int ii = 0; ii < rpCells->nused; ii++) {
      ...
      if (chx > 0) {
        int ncchn =
            QString(centp).left(chx).section(" ", -1, -1).simplified().toInt();
        nchans += ncchn;
      }
    }
...
QStringList US_ExperGuiCells::getLValue(const QString type) {
  ...
  if (type ==
      "centerpieces") {  // Return row strings where a centerpiece is selected
    int nholes = sibIValue("rotor", "nholes");
    ...
    for (int ii = 0; ii < rpCells->nused; ii++) {
      ...
      QString centry;  // ALEXEY: if last cell and not counterbalance, treat as
                       // centerpiece
      if ((ii + 1) != nholes) {
  ...
  else if (type == "cpchannels") {  // Return channel strings where a
                                    // centerpiece is selected
    int icbal = sibIValue("rotor", "nholes") - 1;
    QString rchans("ABCDEF");

    for (int ii = 0; ii < rpCells->nused; ii++) {  // Examine the used cells
      ...
      if (chx > 0) {  // Complete string for each channel in centerpiece
        int ncchn =
            QString(centp).left(chx).section(" ", -1, -1).simplified().toInt();
        for (int jj = 0; jj < ncchn;
             jj++) {  // Save channel strings (e.g., "1 / A", "1 / B", "2 / A",
                      // ...)
...
// Get a specific panel list from a sibling panel
QStringList US_ExperGuiCells::sibLValue(const QString sibling,
                                        const QString type) {
  return mainw->childLValue(sibling, type);
}
...
void US_ExperGuiSolutions::initPanel() {
  ...
  for (int ii = 0; ii < nchant; ii++) {
    ...
    qDebug() << "11: rpSolut->chsols.size(), ii: " << rpSolut->chsols.size()
             << ii;
...
void US_ExperGuiSolutions::savePanel() {
  ...
  for (int ii = 0; ii < mxrow;
       ii++) {  // Fill panel lists and parameters from GUI elements
    QString channel = cc_labls[ii]->text();
    DbgLv(1) << "EGSo: svP:  ii" << ii << "channel" << channel;

    if (channel == chn_none) break;
    ...
    if (pro_comms.keys().contains(iistr)) {
      ...
      // ALEXEY - to remember changes to Soluton comments if manual commnets was
      // added while returning to "Solutons" tab
  ...
  for (int ii = 0; ii < nchanf;
       ii++) {  // Now fill protocol from internal lists and parameters
...
QString US_ExperGuiSolutions::getSValue(const QString type) {
  ...
  if (type == "nchant" || type == "nchanf" || type == "nusols" ||
      type == "alldone" ||
      type == "status") {  // String representation of integer
...
QStringList US_ExperGuiSolutions::getLValue(const QString type) {
  ...
  if (type == "solutions") {  // Build a list of unique solutions
    QStringList chsolus;
    ...
    for (int ii = 0; ii < rpSolut->nuniqs; ii++) {
      ...
      for (int jj = 0; jj < rpSolut->nschan;
           jj++) {  // List channels that have this solution
  ...
  else if (type == "channel_solutions") {  // Build a "channel : solution" list
                                           // for used channels
    for (int ii = 0; ii < rpSolut->nschan; ii++) {
  ...
  else if (type == "sochannels") {  // Build a channels list for used channels
    for (int ii = 0; ii < rpSolut->nschan; ii++) {
      value << rpSolut->chsols[ii].channel;
    }
  }
...
// Get a specific panel value from a sibling panel
QString US_ExperGuiSolutions::sibSValue(const QString sibling,
                                        const QString type) {
  return mainw->childSValue(sibling, type);
}
...
// Get a specific panel double value from a sibling panel
double US_ExperGuiSolutions::sibDValue(const QString sibling,
                                       const QString type) {
  return mainw->childDValue(sibling, type);
}

// Get a specific panel list from a sibling panel
QStringList US_ExperGuiSolutions::sibLValue(const QString sibling,
                                            const QString type) {
  return mainw->childLValue(sibling, type);
}
...
void US_ExperGuiOptical::initPanel() {
  ...
  for (int ii = 0; ii < nochan; ii++) {
    ...
    int cell_number =
        ((channel.split(QRegExp("\\s+"), QString::SkipEmptyParts))[0]).toInt();
    DbgLv(1) << "EGOp:inP: CELL #" << cell_number;
    if (nholes == cell_number) ctrbal_is_centerpiece = true;
  }
  ...
  for (int ii = 0; ii < nochan; ii++) {
    ...
    // ALEXEY: set checkboxes checked based on the protocol loaded, rather than
    // by default
    if (!scan1.isEmpty())
...
QStringList US_ExperGuiOptical::getLValue(const QString type) {
  ...
  if (type == "profiles") {
    ...
    for (int ii = 0; ii < rpOptic->nochan; ii++) {
      ...
      QString scan2 = rpOptic->chopts[ii].scan2;
      QString scan3 = rpOptic->chopts[ii].scan3;
      DbgLv(1) << "EGOp: gPL: ii" << ii << "channel" << channel << "scan1-3"
               << scan1 << scan2 << scan3;
      ...
      if (!scan1.isEmpty() &&
          (scan1 !=
           notinst)) {  // Checked optical system:  add to profile entry
        pentry += (":" + scan1);
        noptis++;
      }
...
// Get a specific panel value from a sibling panel
QString US_ExperGuiOptical::sibSValue(const QString sibling,
                                      const QString type) {
  return mainw->childSValue(sibling, type);
}
...
// Get a specific panel double value from a sibling panel
double US_ExperGuiOptical::sibDValue(const QString sibling,
                                     const QString type) {
  return mainw->childDValue(sibling, type);
}

// Get a specific panel list from a sibling panel
QStringList US_ExperGuiOptical::sibLValue(const QString sibling,
                                          const QString type) {
  return mainw->childLValue(sibling, type);
}
...
int US_ExperGuiOptical::status() {
  ...
  for (int ii = 0; ii < nochan;
       ii++) {  // Loop to count visible rows and ones with at least one check
    QString channel = rpOptic->chopts[ii].channel;
    QString scan1 = rpOptic->chopts[ii].scan1;
    QString scan2 = rpOptic->chopts[ii].scan2;
    QString scan3 = rpOptic->chopts[ii].scan3;
    ...
    if (!scan1.isEmpty() && !scan1.contains(notinst)) {
      have_used = true;
      if (scan1.contains(l_uvvis)) have_uvvis = true;
    }

    if (!scan2.isEmpty() && !scan2.contains(notinst)) {
      have_used = true;
      if (scan2.contains(l_uvvis)) have_uvvis = true;
    }

    if (!scan3.isEmpty() && !scan3.contains(notinst)) {
      have_used = true;
      if (scan3.contains(l_uvvis)) have_uvvis = true;
    }

    if (have_used) nuchan++;

    if (have_uvvis) nuvvis++;
...
void US_ExperGuiRanges::initPanel() {
  ...
  for (int ii = 0; ii < nrnchan; ii++) {
    ...
    if (channel.contains(
            tr("reference")))  // ALEXEY do not allow to set wavelengths/radial
  ...
  for (int i = 0; i < nrnchan; i++) {
    ...
    if (!clabl.split(",")[0].contains("/ A")) continue;
    ...
    // Set check-state of  boxes in same-cell rows and reconnect : ALEXEY: here
    // set of channel A is copied to B
    for (int j = 0; j < ccrows.count(); j++) {
      ...
      swvlens[ccrow].clear();
      for (int jj = 0; jj < kswavl; jj++) swvlens[ccrow] << swvlens[i][jj];
    }
  }
  ///////////////////////////////////////////////////////////////
  ...
  for (int i = 0; i < nsp; i++) {
    ...
    qDebug() << "RANGES INIT: duration_sec , scanint_sec, scanint_sec_min,  "
             ...
             << duration_sec << scanint_sec << scanint_sec_min << Total_wvl[i]
             << ncells_used[i];
    ...
    } else  // hide uv/vis related widgets
    {
      ...
      // Update cb_scancount string:
      QString scancount_stage =
          tr("Stage %1. Number of Scans per Triple (UV/vis): N/A ").arg(i + 1);
      cb_scancount->addItem(scancount_stage);
    }
    ...
    if (has_interference) {
      ...
      qDebug() << "RANGES INIT Interference: duration_sec , scanint_sec_int, "
                  "ncells_used -- "
  ...
  if (mainw->us_abde_mode) {
    ...
    for (int ii = 0; ii < nrnchan; ii++) {
      ...
      if (kswavl > 1 && iStwoOrMoreAnalytesSpectra_forChannel(
                            chanName, msg_to_user, "INIT", -1)) {
    ...
    for (int ii = 0; ii < nrnchan; ii++) {
      ...
      // Qt::FindChildrenRecursively ); //Qt::FindDirectChildrenOnly); if ( c_w
      // != NULL )
      //   {
      //     qDebug() << "Widget found!";
...
void US_ExperGuiRanges::savePanel() {
  ...
  rpRange->nranges =
      nrnchan;  // Protocol channels               //ALEXEY not correct BUG-BUG
  ...
  for (int ii = 0; ii < nrnchan; ii++) {
    ...
    for (int jj = 0; jj < swvlens[ii].count();
         jj++) {  // Wavelength values from selected wavelengths
      rpRange->chrngs[ii].wvlens << swvlens[ii][jj];
    }
    DbgLv(1) << "EGwS:svP:  ii" << ii << "wvl knt"
             << rpRange->chrngs[ii].wvlens.count();
    qDebug() << "Ranges::save, hi, buff_bool -- " << rpRange->chrngs[ii].hi_rad
...
// Get a specific panel value from a sibling panel
QString US_ExperGuiRanges::sibSValue(const QString sibling,
                                     const QString type) {
  return mainw->childSValue(sibling, type);
}
...
// Get a specific panel list from a sibling panel
QStringList US_ExperGuiRanges::sibLValue(const QString sibling,
                                         const QString type) {
  return mainw->childLValue(sibling, type);
}
...
void US_ExperGuiAProfile::reset_sdiag(void) {
  ...
  qDebug()
      << "EXP_APROFILE_GUI RESET-- reset_sdiag(): sdiag->currProf.aprofname -- "
      << sdiag->currProf.aprofname;
}
...
void US_ExperGuiUpload::initPanel() {
  ...
  rps_differ = false;
  if (mainw->currProto != mainw->loadProto) rps_differ = true;

  if (aprof_curr != aprof_loaded) rps_differ = true;
  ...
  if (areReportMapsDifferent(aprof_curr, aprof_loaded)) rps_differ = true;
  ...
  if (mainw->automode) {
    subm_enab =
        (have_run && have_proj && proto_ena &&
         mainw->connection_status &&        // ALEXEY: use top-level connection
                                            // boolean!
    ...
    subm_enab =
        (have_run && have_proj && proto_ena &&
         mainw
             ->connection_status);  // ALEXEY: use top-level connection boolean!
  ...
  pb_saverp->setEnabled(have_cells && have_solus && have_range &&
                        rps_differ);  // ALEXEY: add check here is rps_differ ==
                                      // true (protocols differ)

  // Show/hide Submit and Save buttons based on RunId given
  if (!mainw->us_prot_dev_mode) {
    if (US_Settings::us_inv_level() > 2) {  // Can show/hide buttons as admin
      DbgLv(1) << "EGUp:inP: have_run" << have_run;
      if (have_run) {  // RunId given, so show submit
        DbgLv(1) << "EGUp:inP: have_run";
        pb_submit->show();
        pb_saverp->hide();
      } else {  // RunID not given, so show save
      ...
      QMessageBox::critical(
          ...
          msg_to_user.join("\n") +
              tr("\n\nProtocol does not correspond to the uploaded data! "
                 "\nIt appears the protocol was modified from it's original "
                 "state\n"
                 "corresponding to the uploaded from disk data. \n"
                 "Please re-upload data using 2. Lab/Rotor tab and do not "
                 "modify "
  ...
  if (mainw->us_abde_mode) {
    ...
    if (!ifMixedABDE(msg_to_user)) {
      ...
      QMessageBox::critical(
          ...
          msg_to_user.join("\n") +
              tr("\n\nCurrent Ranges settings do not correspond to either "
                 "multi-wavelength (MWL) or "
                 "single-wavelength (SWL) experiment."
                 "\nPlease modify wavelengths settings in the tab 7:Ranges, to "
                 "ensure "
                 "all channels are either MWL or SWL."
                 "\n\nSaving protocol or run submission to the Optima are not "
                 "possible "
                 "until this problem is resolved."));
      return;
    }
    ...
    if (!extinctionProfilesExist(msg_to_user)) {
      ...
      if (msg_to_user.join(",").contains("Single Analyte Defined;")) {
        QMessageBox::critical(
            this, tr("ATTENTION: Solution with a Single Analyte (MWL-ABDE)"),
            msg_to_user.join("\n") +
                tr("\n\nThe solution for the above specified channel has only "
                   "one analyte. At least two analytes with valid extinction "
                   "profiles "
                   "are required for the currently defined MWL-ABDE "
                   "experiment.\n"
                   "Please modify the solution, or select a different one to "
                   "satisfy these "
                   "requirements."
        ...
        QMessageBox::critical(
            ...
            msg_to_user.join("\n") +
                tr("\n\nPlease upload valid extinction profiles for above "
                   "specified analytes "
    ...
    if (!useReferenceNumbersSet(msg_to_user)) {
      ...
      QMessageBox::critical(
          this, tr("ATTENTION: Reference Channels NOT set Properly (ABDE)"),
          msg_to_user.join("\n") +
  ...
  for (int kk = 0; kk < oprof.count(); kk++) {
    if (oprof[kk].contains(rayleigh)) {
      if (oprof[kk].section(":", 0, 0).contains("sample")) {
        // qDebug() << "ITF channel name: " <<  oprof[ kk ].section( ":", 0, 0
        // ).split(",")[0];
...
bool US_ExperGuiUpload::protocolToDataDisk(QStringList& msg_to_user) {
  ...
  for (int ii = 0; ii < rpOptic->chopts.size(); ii++) {
    ...
    if (!chann_numbers_from_dataDisk.contains(ch_num)) {
      msg_to_user
          << "[Uploaded Data <-> Protocol] Optics channels are mismatched!";
      return false;
    }

    QStringList r_types;
    if (!rpOptic->chopts[ii].scan1.isEmpty()) r_types << "RI";
    if (!rpOptic->chopts[ii].scan2.isEmpty()) r_types << "IP";
    ...
    if (r_types != r_types_fromDisk) {
      msg_to_user << "[Uploaded Data <-> Protocol] Optics Types for channels " +
  ...
  if (channames_from_dataDisk.size() != rpRange->nranges) {
    msg_to_user << "[Uploaded Data <-> Protocol] Numbers of Range channels are "
                   "mismatched!";
    return false;
  }
  ...
  for (int ii = 0; ii < rpRange->nranges; ii++) {
    ...
    if (nwavl != chann_ranges_from_dataDisk[chan_red].size()) {
      msg_to_user << "[Uploaded Data <-> Protocol] Wvls# for Range channel " +
    ...
    for (const auto& c_wvl : all_wvls) all_wvls_list << QString::number(c_wvl);
...
bool US_ExperGuiUpload::samplesReferencesWvlsMatch(QStringList& msg_to_user) {
  ...
  for (rts = ref_to_samples.begin(); rts != ref_to_samples.end(); ++rts) {
    ...
    qDebug()
        << "in samplesReferencesWvlsMatch(): ref_chn_name, sample_chn_names -- "
...
bool US_ExperGuiUpload::matchRefSampleWvls(QString ref_chn_name, QString sn,
                                           QStringList& msg_to_user) {
  bool matchedOK = true;
...
bool US_ExperGuiUpload::useReferenceNumbersSet(QStringList& msg_to_user) {
  ...
  for (int rn = 0; rn < ref_chnns.size(); ++rn) {
    if (ref_chnns[rn] > ref_chnn_max) ref_chnn_max = ref_chnns[rn];
    if (ref_chnns[rn] < ref_chnn_min) ref_chnn_min = ref_chnns[rn];
  }
  ...
  if (ref_use_chhs_number == 0) {
    msg_to_user << QString(
        tr("All channels are set as \"Reference\"; Please define at least one "
...
bool US_ExperGuiUpload::ifMixedABDE(QStringList& msg_to_user) {
  ...
  if (all_wvl_types.size() != 1) all_chann_same_wvl_type = false;
      ...
      if (!mwl_deconv) {
        ...
        for (int i = 0; i < num_analytes; ++i) {
          ...
          if (!validExtinctionProfile(a_desc, all_wvls,
                                      analyte.extinction.keys(), msg_to_user))
        ...
        if (buff_req)  // only if buffer spectrum required
        {
          ...
          qDebug() << "Solution " << solution->solutionDesc << ", (ID)Buffer "
                   << "(" << b_ID << ")" << b_name;
          ...
          if (!validExtinctionProfile(b_desc, all_wvls,
                                      buffer.extinction.keys(), msg_to_user))
            all_profiles_exist = false;
        }
...
bool US_ExperGuiUpload::validExtinctionProfile(QString desc,
  ...
  // messages
  if (!eprofile_ok)
    msg_to_user << QString(tr("%1: Extinction profile %2")).arg(desc).arg(msg);

  return eprofile_ok;
}

bool US_ExperGuiUpload::areReportMapsDifferent(US_AnaProfile aprof_curr,
  ...
  qDebug() << "Top-level Report Maps Keys: curr, loaded -- " << aprof_curr_keys
           << aprof_load_keys;

  if (aprof_curr_keys.size() != aprof_load_keys.size()) return true;

  if (aprof_curr_keys != aprof_load_keys) return true;
...
// Get a specific panel value from a sibling panel
QString US_ExperGuiUpload::sibSValue(const QString sibling,
                                     const QString type) {
  return mainw->childSValue(sibling, type);
}
...
// Get a specific panel list from a sibling panel
QStringList US_ExperGuiUpload::sibLValue(const QString sibling,
                                         const QString type) {
  return mainw->childLValue(sibling, type);
}
