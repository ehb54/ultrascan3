...
US_ExperimentMain::US_ExperimentMain() : US_Widgets() {
  ...
  connect(tabWidget, SIGNAL(currentChanged(int)), this, SLOT(newPanel(int)));
  ...
  connect(epanUpload, SIGNAL(expdef_submitted(QMap<QString, QString>&)), this,
          SLOT(optima_submitted(QMap<QString, QString>&)));
...
void US_ExperimentMain::get_importDisk_data(
    QString d_type, QMap<QString, QStringList>& f_data) {
...
void US_ExperimentMain::exclude_used_instruments(
...
void US_ExperimentMain::accept_passed_protocol_details(
  ...
  QString xmlstr("");
  int protoID = US_ProtocolUtil::read_record_auto(pname, invID_passed, &xmlstr,
                                                  NULL, &db);
...
// When run submitted to Optima
void US_ExperimentMain::optima_submitted(
    QMap<QString, QString>& protocol_details) {
  tabWidget->setCurrentIndex(0);
...
void US_ExperimentMain::submitted_protDev(
    QMap<QString, QString>& protocol_details) {
  tabWidget->setCurrentIndex(0);
  emit to_live_update(protocol_details);
}
...
void US_ExperimentMain::submitted_dataDisk(
    QMap<QString, QString>& protocol_details) {
  tabWidget->setCurrentIndex(0);
...
US_ExperGuiGeneral::US_ExperGuiGeneral(QWidget* topw)
  ...
  connect(le_runid, SIGNAL(editingFinished()), this, SLOT(run_name_entered()));
  ...
  connect(pb_investigator, SIGNAL(clicked()), this, SLOT(sel_investigator()));
  ...
  for (int ii = 0; ii < protdata.count(); ii++) pr_names << protdata[ii][0];
...
bool US_ExperGuiGeneral::centpInfo(const QString cpname,
  ...
  for (int ii = 0; ii < acp_list.count();
       ii++) {  // Search abstract centerpieces for a name match
    if (acp_list[ii].name ==
        cpname) {  // Match found:  flag found and return entry
      is_found = true;
      cpEntry = acp_list[ii];
      break;
    }
...
void US_ExperGuiGeneral::label_name_entered(void) {
  ...
  lname.replace( rx,  "_" );   //ALEXEY - we may use alpha-numeric only, but not
  nessessarily..
  ...
  if ( lname != old_lname )
    {  // Report on invalid characters replaced
      QMessageBox::warning( this,
                            tr( "Label Changed" ),
                            tr( "The label name has been changed. It may consist
  ...
  */
  ...
  if (changed) {  // Replace runID in line edit box
...
void US_ExperGuiGeneral::run_name_entered(void) {
  ...
  if (rname != old_rname) {  // Report on invalid characters replaced
    QMessageBox::warning(
...
void US_ExperGuiGeneral::load_protocol(void) {
  ...
  US_SelectItem* pdiag =
      new US_SelectItem(protdata, hdrs, pdtitle, &prx, delete_button,
                        -2);  // ALEXEY <-- with Delete button and functionality

  connect(pdiag, SIGNAL(accept_deletion()), this, SLOT(update_protdata()));

  if (pdiag->exec() == QDialog::Accepted) {  // Accept in dialog:  get selected
                                             // protocol name and its XML
    DbgLv(1) << "EGGe:ldPro:  ACCEPT  prx" << prx << "sel proto"
             << protdata[prx][0] << "protID" << protdata[prx][2];

    qDebug() << "In load_protocol: before RESET, currProto->investigator: -- "
             << currProto->investigator;
    // ALEXEY: need to reset everything:
    mainw->reset();
    qDebug() << "In load_protocol: after RESET, currProto->investigator: -- "
             << currProto->investigator;
  ...
  DbgLv(1) << "EGGe:ldPro:    cPname" << mainw->currProto.protoname << "lPname"
           << mainw->loadProto.protoname << "pdbID" << mainw->loadProto.protoID;
  DbgLv(1) << "EGGe:ldPro:    cOhost" << mainw->currProto.optimahost << "lOhost"
           << mainw->loadProto.optimahost;
  DbgLv(1) << "EGGe:ldPro:    cTempe" << mainw->currProto.temperature
           << "lTempe" << mainw->loadProto.temperature;

  QString rname = le_runid->text();
  if (!rname.isEmpty()) mainw->currProto.runname = rname;
  loaded_proto = 1;
...
US_ExperGuiRotor::US_ExperGuiRotor(QWidget* topw) : US_WidgetsDialog(topw, 0) {
  ...
  connect(cb_calibr, SIGNAL(activated(int)), this, SLOT(changeCalib(int)));
  ...
  importDataPath = "";
  if (mainw->us_prot_dev_mode) ck_disksource->hide();
...
void US_ExperGuiRotor::get_chann_ranges_public(
    QString d_type, QMap<QString, QStringList>& f_data) {
  f_data.clear();
...
void US_ExperGuiRotor::importDisk(void) {
  ...
  QStringList files =
      readDir.entryList(nameFilters, QDir::Files | QDir::Readable, QDir::Name);
      ...
      if (qRound(rdata.scanData[0].wavelength) == 0) {
        QMessageBox::critical(
            ...
            tr("Uploaded simulated data not generated correctly:\n\n"
               "Wavelength: 0\n\n"
               "Please upload correctly generated simulated data, or raw data "
               "in .auc format."));
        return;
      }
  ...
  if (!ck_absorbance_t->isChecked()) ck_absorbance_t->setChecked(false);
  ...
  for (int i = 0; i < channs_ranges.keys().size(); ++i) {
    ...
    if (ra_data_type && ch_c.contains("B")) continue;
...
bool US_ExperGuiRotor::init_output_data() {
  ...
  if (!have_trip) all_tripinfo.clear();
  outData.clear();
  all_chaninfo.clear();
  all_triples.clear();
  all_channels.clear();
  ...
  for (int trx = 0; trx < allData.size(); trx++) {
    ...
    if (have_trip) tripinfo = all_tripinfo[trx];
  ...
  if (nspeed == 0) {  // Compute speed steps from all raw auc data
...
#define DSS_LO_ACC 250.0  // default SetSpeedLowAccel
    ...
    //    low_accel        = US_AstfemMath::low_acceleration( speedsteps,
    //    ss_lo_acc, rate );
    // }
  }
  ...
  if (low_accel) {
    ...
    msgBox.exec();
    if (msgBox.clickedButton() == bAbort) {  // Abort the import of this data
      QApplication::restoreOverrideCursor();
      qApp->processEvents();
      // reset();
      return false;
    } else {  // Modify times and omegas of this data, then proceed to import
  ...
  if (isMwl && (all_tripinfo.count() / all_chaninfo.count()) <
                   4) {  // If less than 4 wavelengths, treat as non-MWL
    isMwl = false;
    all_chaninfo = all_tripinfo;
    out_chaninfo = out_tripinfo;
    out_chandatx.clear();

    for (int jj = 0; jj < out_tripinfo.count(); jj++) out_chandatx << jj;
    DbgLv(1) << "CGui:IOD:    isMwl" << isMwl << "ac.count at.count oc.count"
             << all_chaninfo.count() << all_tripinfo.count()
             << out_chaninfo.count();
  }

  if (isMwl) {  // If MWL, update speed steps
    ...
    DbgLv(1) << "CGui:IOD:    updSS nspeed nstest nspeedc" << nspeed << nstest
             << nspeedc << "ss0  timf omgf" << speedsteps[0].time_first
             << speedsteps[0].w2t_first;
  }

  else {  // Else complete channel lists
...
void US_ExperGuiRotor::runDetails(void) {
  ...
  if (isMwl) {  // For MWL, only pass the 1st data set of each cell/channel
  ...
  else {  // For most data, pass all (non-excluded) triples
  ...
  for (int i = 0; i < scanCount_per_dataTypeList.size(); ++i) {
    ...
    if (dt_scans.contains("RI:") || dt_scans.contains("RA:"))
      rpSpeed->ssteps[curssx].scanintv = int(run_details["Time"].toDouble() /
                                             dt_scans.split(":")[1].toDouble());
    else if (dt_scans.contains("IP:"))
      rpSpeed->ssteps[curssx].scanintv_int = int(
          run_details["Time"].toDouble() / dt_scans.split(":")[1].toDouble());
  }
...
// for dataImport
QMap<QString, QStringList> US_ExperGuiRotor::build_protocol_for_data_import(
  ...
  for (int i = 0; i < all_tripinfo.size(); ++i) {
    ...
    QString channumber =
        all_tripinfo[i].tripleDesc.split(" / ")[0].simplified();
    QString channame =
        channumber + all_tripinfo[i].tripleDesc.split(" / ")[1].simplified();
    QString wvl = all_tripinfo[i].tripleDesc.split(" / ")[2].simplified();
    qDebug() << "Wvl, ra_data_sim: " << wvl << ra_data_sim;
    if (ra_data_sim && wvl.toInt() < 200) wvl = QString::number(280);

    chann_list << channumber;

    QString runType_t =
        QString(all_tripinfo[i].tripleFilename).section(".", -5, -5);
    if (runType_t == "RI" || runType_t == "RA") chann_to_wvls[channame] << wvl;
  }
  chann_list.removeDuplicates();
  ...
  for (int i = 0; i < chann_list.size(); ++i) {
    ...
    cu.cbalance = "";  // <-- put always empty for Cells initPanel not to treat
                       // as counterbalance!
    rpCells->used << cu;
  }
  ...
  for (int ii = 0; ii < rpSolut->nschan; ii++) {
    ...
    if (!rpSolut->solus.contains(
            sdesc)) {  // Update unique solutions list and correspond Id list
      rpSolut->solus << sdesc;
  ...
  for (int i = 0; i < chann_list.size(); ++i) {
    ...
    for (int j = 0; j < chann_to_wvls[chann_list[i] + "A"].size(); ++j) {
      ...
      rng_a.wvlens << wvl_c;
      qDebug() << "Wvl# " << j << " for channel, " << chann_list[i] + "A, is"
               << wvl_c;
    }
    ...
    for (int j = 0; j < chann_to_wvls[chann_list[i] + "B"].size(); ++j) {
      ...
      rng_b.wvlens << wvl_c;
      qDebug() << "Wvl# " << j << " for channel, " << chann_list[i] + "B, is"
               << wvl_c;
    }
...
void US_ExperGuiRotor::changeLab(int ndx) {
  ...
  connect(cb_optima, SIGNAL(activated(int)), this, SLOT(changeOptima(int)));
...
void US_ExperGuiRotor::changeOptima(int ndx) {
  ...
  foreach (US_Rotor::Operator oper, operators) {
    ...
    cb_choose_operator->addItem(QString::number(oper.ID) + ". " + oper.lname +
                                ", " + oper.fname);
  }
  cb_operator->clear();
  ...
  if (dbP != NULL) {
    ...
    for (int ii = 0; ii < calibs.count(); ii++) {
      if (ii < 4 || (ii + 4 > calibs.count()))
        DbgLv(1) << "EGR:  ii" << ii << "calibs ID,rotID,GUID" << calibs[ii].ID
                 << calibs[ii].rotorID << calibs[ii].GUID;
    }
    //*DEBUG*
  } else {
...
void US_ExperGuiRotor::advRotor() {
  ...
  calibr.ID =
      (calibx >= 0) ? cb_calibr->currentText().section(":", 0, 0).toInt() : 0;
  rotor.ID =
      (rotorx >= 0) ? cb_rotor->currentText().section(":", 0, 0).toInt() : 0;
  ...
  connect(rotorInfo,
          ...
          this,
          SLOT(advRotorChanged(US_Rotor::Rotor&, US_Rotor::RotorCalibration&)));
...
void US_ExperGuiRotor::advRotorChanged(US_Rotor::Rotor& crotor,
  ...
  for (int ii = 0; ii < sl_rotors.count();
       ii++) {  // Find and select the list item matching the accepted rotor
    int eID = sl_rotors[ii].section(":", 0, 0).toInt();
    if (eID == rID) {  // Match:  select item, set index, break from loop
  ...
  if (rx >= 0) changeRotor(rx);  // Rebuild calibrations for new rotor

  for (int ii = 0; ii < sl_calibs.count();
       ii++) {  // Find and select the list item matching the accepted
                // calibration
...
US_Rotor::AbstractRotor* US_ExperGuiRotor::abstractRotor(const QString rotor) {
  ...
  int arID = -1;
  for (int ii = 0; ii < rotors.count();
       ii++) {  // Search for a match to the rotor name
    DbgLv(1) << "EGR:  absR:   ii" << ii << "rname" << rotors[ii].name;
    if (rotors[ii].name.trimmed() ==
        rotor) {  // Match found:  break with abstractRotor ID value
  ...
  for (int ii = 0; ii < arotors.count();
       ii++) {  // Search for a match to the abstract rotor ID
    DbgLv(1) << "EGR:  absR:   ii" << ii << "rID" << arotors[ii].ID;
    if (arotors[ii].ID ==
        arID) {  // Match found:  return with pointer to abstract rotor object
      return (&arotors[ii]);
    }
  }
...
US_ExperGuiSpeeds::US_ExperGuiSpeeds(QWidget* topw)
  ...
  QLayout* lo_delay_stage_sync =
      us_checkbox(tr("Synchronize Stage Delay with the 1st Speed Profile: "),
                  ck_sync_delay, false);
  ...
  if (mainw->automode && !mainw->usmode) sb_count->setEnabled(false);
  ...
  QHBoxLayout* lo_duratlay =
      us_ddhhmmsslay(0, 0, 0, 0, 1, &sb_durat_dd, &sb_durat_hh, &sb_durat_mm,
  ...
  QHBoxLayout* lo_scnintlay =
      us_ddhhmmsslay(0, 0, 0, 0, 0, &sb_scnint_dd, &sb_scnint_hh, &sb_scnint_mm,
                     &sb_scnint_ss);
  ...
  QLabel* lb_wvl_per_cell =
      us_label(tr("Sum of all wavelengths (from all cells) to be scanned:"));
  ...
  connect(sb_count, SIGNAL(valueChanged(int)), this, SLOT(ssChangeCount(int)));
  connect(cb_prof, SIGNAL(activated(int)), this, SLOT(ssChangeProfx(int)));
...
QString US_ExperGuiSpeeds::speedp_description(int ssx) {
  ...
  double durtim =
      ssvals[ssx]
            ["duration"];  // Duration total minutes   //ALEXEY - no, in seconds
...
void US_ExperGuiSpeeds::ssChangeCount(int val) {
  ...
  if (new_nsp > nspeed) {  // Number of speed steps increases
    ...
    double ssdurtim =
        ssvals[kk]["duration"];  // ALEXEY - minutes ??? should be seconds ...
    ...
    for (int kkk = nspeed; kkk < new_nsp;
         kkk++) {  // Fill in new speed step description and values
      ssspeed += 1000.0;
      if (ssspeed > speedmax) ssspeed = speedmax;
  ...
  else {  // Number of speed steps decreases or remains the same
    profdesc.resize(new_nsp);
    ssvals.resize(new_nsp);
    ...
    cb_prof->clear();
    for (int ii = 0; ii < new_nsp; ii++) cb_prof->addItem(profdesc[ii]);
...
void US_ExperGuiSpeeds::ssChangeProfx(int ssp) {
  ...
  // ALEXEY: before minimums for delay/scanint counters must be set for selected
  // row (ssp -> curssx ), so minimums_hh/ss must be set to be arrays.
...
void US_ExperGuiSpeeds::ssChangeScInt(double val, int row) {
  ...
  double time_scint;
  if (val <= 14999) time_scint = a0[0] + qRound(a1[0] / val);
  if (val >= 15000 and val <= 32999) time_scint = a0[1] + qRound(a1[1] / val);
  if (val >= 33000 and val <= 50999) time_scint = a0[2] + qRound(a1[2] / val);
  if (val >= 51000 and val <= 60000) time_scint = a0[3] + qRound(a1[3] / val);
  ...
  // not needed here as these will overwrite current counters when another speed
  // profile is added. Values set in ChProfile function. sb_scnint_hh
...
void US_ExperGuiSpeeds::ssChgDelayTime_hh(int val) {
  ...
  if (val > 0) sb_delay_mm->setMinimum(0);

  if (val == 0) sb_delay_mm->setMinimum(minimum_mm);
...
void US_ExperGuiSpeeds::ssChgDelayDay(int val) {
  ...
  DbgLv(1) << "EGSp: chgDlyD: val" << val << "ssdly d h" << ssdlyday << ssdlyhr;
  // << ssdlyhr << ssdlymin << ssdlysec << "t" << ssdlytim;

  double ssdlytim =
      (val * 3600.0 * 24) + (ssdlyhr * 3600.0) + (ssdlymin * 60.0) + ssdlysec;
  ssvals[curssx]["delay"] = ssdlytim;  // Set Delay in step vals vector
}
/* END OF UV-vis DELAY
 * ***********************************************************************************/
...
void US_ExperGuiSpeeds::ssChgDelayTime_int_hh(int val) {
  ...
  if (val > 0) sb_delay_int_mm->setMinimum(0);

  if (val == 0) sb_delay_int_mm->setMinimum(minimum_mm);
...
#warning \
  ...
  double actual_delaylow_int =
      6 * 60 + delaylow;  // ALEXEY: Interference delay = 6 mins + speed/accel;
...
US_ExperGuiCells::US_ExperGuiCells(QWidget* topw) : US_WidgetsDialog(topw, 0) {
  ...
  for (int ii = 0; ii < mxcels; ii++) {
    ...
    connect(cb_wind, SIGNAL(activated(int)), this, SLOT(windowsChanged(int)));
...
void US_ExperGuiCells::rebuild_Cells(void) {
  ...
  if (nholes == rpCells->ncell) return;  // No rotor change means no rebuild

  if (rpCells->ncell ==
      0) {  // No existing Cells protocol, so initialize a brand-new one
  ...
  if (nholes <
      rpCells->ncell) {  // Decreasing cells count:  Recompute used count
    DbgLv(1) << "EGCe:rbC: H<C: nused" << rpCells->nused;
    int kused = 0;
    for (int ii = 0; ii < rpCells->nused; ii++) {
      if (rpCells->used[ii].cell > nholes) break;
      kused++;
  ...
  } else {  // If cells count increases, same as above: clear used, i.e. make
            // all fresh-empty, and counterbalance (empty-counterbalance)
    rpCells->nused = 0;     // ALEXEY: # used cells should be 0 when Rotor
    rpCells->used.clear();  // ALEXEY: no used cells when rotor is changed -
                            // otherwise cells are not populated correcly
  }
...
void US_ExperGuiCells::centerpieceChanged(int sel) {
  ...
  if (irow != icbal) {        // Not counterbalance:  change cross cell
    ...
    if (xrow == icbal) {
      if (cc_cenps[xrow]->currentText().contains(
              tr("counterbalance"))) {  // Cross cell is counterbalance
          ...
          // counterbalance
  {
    ...
    if (!cpname_counter.contains(tr("counterbalance"))) {
      ...
      int jsel_c = sel - 3;  // Use same centerpiece for cross (minus 3 since
                             // cent. list is longer in counterbalance rows)
...
US_ExperGuiSolutions::US_ExperGuiSolutions(QWidget* topw)
  ...
  for (int ii = 0; ii < mxrow;
       ii++) {  // Loop to build initial place-holder solution rows
    ...
    if (schan != chn_none) srchans << schan;
...
void US_ExperGuiSolutions::get_new_solutions_public() { allSolutions(); }
...
void US_ExperGuiSolutions::rebuild_Solut(void) {
  ...
  if (rpSolut->nschan ==
      0) {  // No existing Solutions protocol, so initialize a rudimentary one
    rpSolut->nuniqs = 0;
    rpSolut->chsols.clear();
    suchans.clear();
    nchanf = 0;
  ...
  if (nchans != nchanr || nchanr == 0) {  // Must set up total rows channels
    ...
    for (int ii = 0; ii < ncused; ii++) {
      ...
      if (chx > 0) {
        ...
        int nchan = centry.left(chx).section(" ", -1, -1).simplified().toInt();
...
void US_ExperGuiSolutions::detailSolutions() {
  ...
  for (int ii = 0; ii < cc_solus.count();
       ii++) {  // Build up information for each active solution row
    QComboBox* cbsolu = cc_solus[ii];
    ...
    if (!sdescrs.contains(sdescr)) {  // Add solution description to list and
                                      // begin channels-used list
      sdescrs << sdescr;
      chanuse[sdescr] = chanu;
      DbgLv(1) << "EGSo:detS:      chanu" << chanu;
    } else {  // Append to channels-used for solution
...
bool US_ExperGuiSolutions::solutionID(const QString sdescr, QString& solID) {
...
bool US_ExperGuiSolutions::solutionData(const QString sdescr,
  ...
  if (found) {
    if (solu_data.keys().contains(
            sdescr)) {  // Previously fetched and mapped:  just return it
      soludata = solu_data[sdescr];
      DbgLv(1) << "EGSo:solDat:    OLDfound descr" << soludata.solutionDesc;
    }

    else {  // Not fetched before:  do so now and map it
      US_Passwd pw;
      US_DB2* dbP = (sibSValue("general", "dbdisk") == "DB")
                        ? new US_DB2(pw.getPasswd())
                        : NULL;
      if (dbP != NULL) {
        ...
        DbgLv(1) << "EGSo:solDat:    NEWfound descr" << soludata.solutionDesc
                 << stat;
      } else {
...
int US_ExperGuiSolutions::allSolutions() {
  ...
  if (dbP != NULL) {  // Read all the solutions in the database
    soids << "-1";
    QString invID =
        sibSValue("general", "investigator").section(":", 0, 0).simplified();
  ...
  else {  // Read all the solutions on the local disk
    soids << "00000000";
    QString path;
    US_Solution solution;
    if (!solution.diskPath(path)) return 0;
    ...
    for (int ii = 0; ii < fnames.size(); ii++) {  // Examine each S*.xml file
      DbgLv(1) << "EGSo: allSo:  file" << (ii + 1) << "of" << fnames.size()
               << fnames[ii];
      QFile* s_file = new QFile(path + "/" + fnames[ii]);

      if (!s_file->open(QIODevice::ReadOnly | QIODevice::Text)) continue;
  ...
  for (int ii = 1; ii < sonames.count(); ii++) {
    ...
    if (lstx > ii) {  // At least one other with this name, make them unique
      ndup++;
      ...
      for (int jj = ii + 1; jj <= lstx; jj++) {
        ...
        if (sname == snbase) {  // This is a duplicate
          kk++;
          sname = snbase + QString().sprintf("  (%d)", kk);
  ...
  if (ndup > 0) {  // There were duplicates, so re-do the name-to-id mapping
    solu_ids.clear();
...
void US_ExperGuiSolutions::addComments() {
  ...
  if (!solution_comment_init[irow]) {
    ...
    // ALEXEY: Check if the solution descrition does not coinside with that of
    // the protocol's comment:
    //  I.e. if solution was changed from the GUI: then re-set manual_comment to
    //  empty string
    if (!protocol_comment.contains(sdescr))
      ...
      manual_comment[row_comment] =
          protocol_comment.trimmed();  // Initialize manual comment for solution
                                       // from protocol
    }
  ...
  for (int jj = 0; jj < ncc; jj++) {
    QString cc = comms[jj];
    if ((jj + 1) < ncc) {  // Not last component string
      if (cc.length() < 80) {
        msg += "   \"" + cc + ", \"<br/>";
      } else {
    ...
    else {                     // Last component string
      if (cc.length() < 80) {  // Append analyte
        msg += "   \"" + cc + "\"</b><br/><br/>";
      } else {  // Append analyte on two lines
        QString l1 = QString(cc).left(80);
        QString l2 = QString(cc).mid(80);
        msg += "   \"" + l1 + "\"<br/>";
        msg += "   \"" + l2 + "\"</b><br/><br/>";
  ...
  sufx = QInputDialog::getText(this, tr("Add to Experiment's Channel Comments"),
                               msg, QLineEdit::Normal,
                               manual_comment[row_comment], &ok);

  if (ok) {  // OK:  append suffix to channel comment
    chcomm += ", " + sufx;
...
US_ExperGuiOptical::US_ExperGuiOptical(QWidget* topw)
  ...
  QFont ckfont = QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(),
                       QFont::Bold);
  ...
  for (int ii = 0; ii < mxrow;
       ii++) {  // Loop to build initial place-holder optical rows
    QString scel;
    if (ii == 0)
      scel = QString("0/A");
    else if (ii == 1)
    ...
    connect(ck_osys1, SIGNAL(toggled(bool)), this, SLOT(opsysChecked(bool)));
    connect(ck_osys2, SIGNAL(toggled(bool)), this, SLOT(opsysChecked(bool)));
    connect(ck_osys3, SIGNAL(toggled(bool)), this, SLOT(opsysChecked(bool)));
...
void US_ExperGuiOptical::rebuild_Optic(void) {
  ...
  if (nchanf == nochan) {
    ...
    for (int ii = 0; ii < nochan; ii++) {
      if (rpOptic->chopts[ii].channel != ochans[ii]) ndiff++;
    }

    if (ndiff == 0) return;  // No solutions change means no rebuild
  }

  DbgLv(1) << "EGOp rbO: rp.nochan" << rpOptic->nochan;
  if (rpOptic->nochan ==
      0) {  // No existing Optic protocol, so initialize a rudimentary one
    nochan = ochans.count();
    nchanf = nochan;
    ...
    for (int jj = 0; jj < nochan_sv; jj++) {
      ...
      } else  // ALEXEY bug fixed
      {
        ...
        rpOptic->chopts[ii].scan3 = cc_osyss[ii]->button(3)->isChecked()
                                        ? cc_osyss[ii]->button(3)->text()
                                        : "";
        DbgLv(1) << "EGOp rbO:    scan1" << rpOptic->chopts[ii].scan1 << "CKD?"
...
void US_ExperGuiOptical::opsysChecked(bool checked) {
  ...
  // if ( optype != tr( "Rayleigh Interference" ) )  //ALEXEY: couple checkboxes
  // for all optics
  //     return;
  ...
  for (int ii = 0; ii < mxrow; ii++) {
    ...
    if (rlabl == labnone) break;
    // Compare the cell value to that of the one (un)checked
    QString rcell = rlabl.left(1);
    if (rcell == scell) {  // Save same-cell row and disconnect the checkbox
      ccrows << ii;
  ...
  for (int ii = 0; ii < ccrows.count(); ii++) {
    ...
    connect(ckbox, SIGNAL(toggled(bool)), this, SLOT(opsysChecked(bool)));
  }
}
...
US_ExperGuiAProfile::US_ExperGuiAProfile(QWidget* topw)
  ...
  panel->setContentsMargins(2, 2, 2, 2);
  QLabel* lb_panel =
      us_banner(tr("8: Review an Analysis"
                   " Profile"));
  panel->addWidget(lb_panel);
  ...
  if (protoname.isEmpty()) protoname = mainw->loadProto.protoname;
  QString aprofname = rpAprof->aprofname;
  if (aprofname.isEmpty()) aprofname = protoname;
...
US_ExperGuiUpload::US_ExperGuiUpload(QWidget* topw)
  ...
  panel->setContentsMargins(2, 2, 2, 2);
  QLabel* lb_panel =
      us_banner(tr("9: Submit an Experiment"
                   " to the Optima"));
  panel->addWidget(lb_panel);
  // QGridLayout* genL   = new QGridLayout();
  genL = new QGridLayout();
  ...
  QLayout* lo_rotor = us_checkbox(tr("Lab/Rotor/Calibration"), ck_rotor, true);
  QLayout* lo_rotor_ok =
  ...
  QLayout* lo_prot_svd = us_checkbox(tr("Protocol Saved"), ck_prot_svd, false);
  QLayout* lo_sub_enab = us_checkbox(tr("Submit Enabled"), ck_sub_enab, false);
  QLayout* lo_sub_done =
  ...
  connect(pb_submit, SIGNAL(clicked()), this, SLOT(submitExperiment_confirm()));
...
void US_ExperGuiUpload::detailExperiment() {
  ...
  for (int ii = 0; ii < rpRange->nranges; ii++) {
    ...
    dtext +=
        tr("    wavelength range  : %1  to  %2\n").arg(lo_wavl).arg(hi_wavl);
    dtext +=
        tr("    radius range      : %1  to  %2\n").arg(lo_radi).arg(hi_radi);
  }
  ...
  for (int ii = 0; ii < nchna_2dsa; ii++) {
    ...
    dtext += tr("      Custom Grid:            ") + cAP2->parms[ii].cgrid_name +
             "\n";
...
void US_ExperGuiUpload::testConnection() {
  ...
  if (connected) {  // Let the user know that connection is made and set flag
  ...
  else {  // Inform user of failure and give instructions
...
void US_ExperGuiUpload::saveReports(US_AnaProfile* aprof) {
  ...
  for (ri = aprof->ch_reports.begin(); ri != aprof->ch_reports.end(); ++ri) {
    ...
    for (tri = triple_reports.begin(); tri != triple_reports.end(); ++tri) {
      ...
      qDebug() << "Report ID/GUID: triple -- " << ri.key() << ": " << tri.key()
               << " -- " << reportID << " / " << curr_guid;

      ++triple_count;
      // For ABDE case, stop after recording the 1st-in-channel report:
      if (mainw->us_abde_mode && triple_count == 1) {
        qDebug() << "Exiting GMP Report DB writing after the 1st triple in a "
                    "channel...";
        break;
      }
    }
...
int US_ExperGuiUpload::writeReportToDB(QString reportGUID,
                                       US_ReportGMP report) {
  ...
  jsonMask +=
      QString("\"plots\":\"") + QString::number(plots_bool) + QString("\",");
  ...
  jsonMask += QString("\"pseudo3d_2dsait_mw_ff0\":\"") +
              QString::number(pseudo3d_2dsait_mw_ff0_bool) + QString("\",");
  ...
  jsonMask += QString("\"pseudo3d_2dsamc_mw_ff0\":\"") +
              QString::number(pseudo3d_2dsamc_mw_ff0_bool) + QString("\",");
  ...
  if (mainw->us_abde_mode) wvl_to_db = "-1";

  QStringList qry;
  qry << "new_report" << reportGUID << report.channel_name
      << QString::number(report.tot_conc) << QString::number(report.rmsd_limit)
...
int US_ExperGuiUpload::writeReportItemToDB(
    US_DB2* dbP, QString reportGUID, int reportID,
...
void US_ExperGuiUpload::saveAnalysisProfile() {
  ...
  for (ri = aprof->ch_reports.begin(); ri != aprof->ch_reports.end(); ++ri) {
    ...
    for (tri = triple_reports.begin(); tri != triple_reports.end(); ++tri) {
      ...
      qDebug() << "In saving: channel, wvl, Report --- " << chan_desc << wvl_cc;

      for (int ii = 0; ii < report_cc.reportItems.size(); ii++) {
        US_ReportGMP::ReportItem curr_reportItem = report_cc.reportItems[ii];
        qDebug() << "ReportItem # " << ii << "type -- " << curr_reportItem.type
                 << "method -- " << curr_reportItem.method << "range_low -- "
  ...
  if (new_file) {  // Write AProfile XML as a */ultrascan/data/projects/A*xml
                   // file
  ...
  if (dbP != NULL) {
    ...
    DbgLv(1) << "EGAp:svAP:  new DB:  ID" << aprof->aprofID << dbP->lastError();
  }
...
bool US_ExperGuiUpload::saveRunProtocol() {
  ...
  if (mainw->ScanCount_global_int > 1501) {
    QMessageBox::critical(this, tr("*ERROR* in Saving Protocol"),
  ...
  // DbgLv(1) << "EGUp:svRP:  protoname" << protoname << "prdats0" << prdats[0];
  // //ALEXEY: important: this statement caused bug when no protocols existed in
  // the DB

  if (prnames.contains(protoname) ||
      protoname.trimmed()
          .isEmpty()) {  // Cannot save until a new protocol name is given
    QString mtitle = tr("Protocol Name not New");
    QString message =
        tr("The current Run Protocol cannot be saved until\n"
  ...
  while (prnames.contains(newpname) || newpname.trimmed().isEmpty()) {
    ...
    if (!ok) {  // Cancel:  abort the save
      QMessageBox::critical(this, tr("Save Run Protocol Aborted"),
  ...
  if (currProto->protoGUID.isEmpty() || currProto->protoGUID.startsWith("0000"))
    currProto->protoGUID = US_Util::new_guid();  // Get a new GUID
  ...
  if (dbP != NULL) idProt = US_ProtocolUtil::write_record(rpSubmt->us_xml, dbP);
  ...
  QString pdate = US_Util::toUTCDatetimeText(
      QDateTime::currentDateTime().toUTC().toString("yyyy-MM-dd HH:mm"), true);
...
void US_ExperGuiUpload::submitExperiment_confirm_dataDisk() {
  ...
  if (rps_differ)
    message_protocol +=
        tr("A new protocol has been successfully saved to US-LIMS DB. \n\n");
  ...
  if (!mainw->usmode) {
    ...
    if (o_list.contains("MISSING") || r_list.contains("MISSING") ||
      ...
      info_text +=
          QString(tr("<br><br> <font color='red'><b> ATTENTION: </b></font>"
...
void US_ExperGuiUpload::submitExperiment_confirm() {
  ...
  // ALEXEY: here - check for connection to sys_data server and put error msg if
  // no connection, with suggesting to saveRunProtocol() into LIMS
  US_Passwd pw;
  US_DB2* dbP = new US_DB2(pw.getPasswd());
  if (dbP != NULL) read_optima_machines(dbP);
  ...
  if (optima_chromoAB.isEmpty()) {
    ...
    QString msg_chromo_text = QString(
                                  "Attention! Currently used instrument, %1, "
                                  "is missing chromatic aberration file.")
                                  .arg(alias);
    QString msg_chromo_text_info = QString(
        "To proceed with the job submission, the file must be uploaded using "
        "instrument configuraiton utility:");
    msg_chromo_text_info +=
        QString("\n Edit -> Preferences -> Instrument Preferences");
    msg_chromo_text_info +=
        QString(tr("\n\nIn the meantime, you may choose to save the protocol "
                   "into LIMS database."));

    msgBox_chromo.setText(msg_chromo_text);
    msgBox_chromo.setInformativeText(msg_chromo_text_info);

    QPushButton* Accept_chromo =
        msgBox_chromo.addButton(tr("Save Protocol"), QMessageBox::YesRole);
    ...
    if (msgBox_chromo.clickedButton() == Accept_chromo) {
      ...
      if (mainw->automode && rps_differ) {
        ...
        QMessageBox::warning(this, tr("No Changes in the Protocol"),
                             tr("The protocol was not saved because there were "
  ...
  if (!combined_check || daysToExpiration <= 0 || !certfile_exists) {
    ...
    QString msg_sys_text =
        QString(
            "Attention! UltraScan is not able to communicate with the data "
            "acquisition server on the %1.")
            .arg(alias);
    ...
    if (certfile_exists) {
      if (daysToExpiration > 0) {
        ...
        msg_sys_text_info +=
            QString(tr("Your license key is expired! Submission of the "
      ...
      msg_sys_text +=
          QString(tr("\n\nIt appears the license key for %1 is missing:"))
              .arg(alias);
      msg_sys_text_info +=
          QString(tr("Please check that your license key is stored in "
                     "$HOME/ultrascan/etc/optima and is not expired\n\n"
                     "Submission of the experimental protocol is suspended "
                     "until this condition is resolved."));
    }
    ...
    QPushButton* Accept_sys =
        msgBox_sys_data.addButton(tr("Save Protocol"), QMessageBox::YesRole);
  ...
  // Address separately the case when un-changed protocol is not re-saved before
  // submission to OptimaX: Ensure all (existing) autoflowReport records for
  // this protocol have 'tripleDropped' SET to 'none'!!!!!
  if (!rps_differ) {
    ...
    for (chan_rep = channels_report_t.begin();
      ...
      for (int i = 0; i < reportIDs_list.size(); ++i) {
        ...
        qry << "update_autoflow_report_at_import" << rID << QString("none");

        qDebug()
            << "Reverting 'tripleDropped' autoflowReport record: query, rID -- "
            << qry << rID;
        dbP->query(qry);
      }
    }
...
bool US_ExperGuiUpload::readAProfileBasicParms(
...
void US_ExperGuiUpload::submitExperiment_confirm_protDev() {
  ...
  QString
      info_text = QString(
                      tr("<b>Dev. Run Name:</b> %1 <br> <b>Data Location:</b> "
  ...
  if (msgBox.clickedButton() == Accept) {
    if (o_list.contains("MISSING") || r_list.contains("MISSING") ||
      ...
      if (submit_map_size > 0) {
        ...
        // Warn user that ALL data (editPRofiles, model, noises, reports) will
        // be deleted to this run
        QMessageBox msgBox_f;
        msgBox_f.setText(tr("You are about to reinitialize the run based on "
                            "the protocol:<br><br>") +
                         tr("<b>[OLD] Run Name:&emsp;</b>") +
                         protocol_details["protocolName"] + tr("<br>") +
                         tr("<b>[NEW] Run Name:&emsp;</b>") +
                         currProto->protoname + tr("<br>"));

        msgBox_f.setInformativeText(
            tr("<font color='red'><b>ATTENTION:</b></font> If you choose to "
               "Procceed, ") +
            tr("all existing edit profiles, models, and noises for this run "
               "will be deleted from DB, ") +
            tr("and the processing flow will reinitialize from the EDIT "
               "stage. ") +
            tr("<br><br><font color='red'><b>This action is not reversible. "
               "Proceed?</b></font>"));
        ...
        QPushButton* Cancel =
            msgBox_f.addButton(tr("Cancel"), QMessageBox::RejectRole);
        ...
        if (msgBox_f.clickedButton() == Cancel) {
          ...
          if (have_run && rps_differ) {
            ...
            // Now actually submit: make record in 'autoflow' table with 'DEV'
            // flag
...
void US_ExperGuiUpload::clearData_protDev() {
  ...
  for (int i = 0; i < fileNameList.size(); ++i) {
    ...
    status = db->statusQuery(qry);
    qDebug() << "Cleaning Data (del pcsa_recs) for Run PRotDev(): del_exp stat"
             << status;
    // deleteRunPcsaMrecs( db, protocol_details[ "invID_passed" ],
    // protocol_details[ "filename" ] );
    ...
    if (status != US_DB2::OK) {
      QMessageBox::information(
          ...
          db->lastError() + tr(" (error=%1, expID=%2)").arg(status).arg(expID));
    }
  }
...
void US_ExperGuiUpload::submitExperiment_dataDisk() {
  ...
  for (int i = 0; i < nstages_size; i++) {
    ...
    // ALEXEY: do we need also delays to first scans for abs/interference into
    // total duration?
  }
  ...
  for (int kk = 0; kk < oprof.count(); kk++) {
    ...
    if (oprof[kk].contains(uvvis)) ++nchannels_uvvis;
  }
  ...
  } else if (ncells_interference != 0 && nwavl_tot != 0)  // BOTH
  {
    ...
    QString triplenumber = QString("IP:") +
                           QString::number(1 * int(ncells_interference / 2)) +
...
void US_ExperGuiUpload::submitExperiment_protDev() {
  ...
  qDebug() << "Old, New protocolName: " << protocol_details["protocolName"]
           << ", " << currProto->protoname;
  qDebug() << "Old, New experimentName: " << protocol_details["experimentName"]
           << ", " << currProto->runname;
  ...
  qDebug() << "PROTCOL DETAILS at submission: -- "
           ...
           << protocol_details["invID_passed"]
           << protocol_details["correctRadii"] << protocol_details["expAborted"]
           << protocol_details["label"] << protocol_details["gmpRun"]
           << protocol_details["filename"] << protocol_details["aprofileguid"]
           << protocol_details["analysisIDs"] << protocol_details["intensityID"]
...
void US_ExperGuiUpload::submitExperiment() {
  ...
  if (dbxpn.open()) {  // dbxpn.open()
    ...
    // Add extra array QVector < QVector < QString >> RadialPath(nstages_size);
        ...
        for (int ii = 0; ii < rpRange->nranges; ii++) {
          ...
          if (channel.contains("sample") &&
              channel.startsWith(QString::number(
                  j + 1)))  // <-- Judge only by sample (channel A) for now
          {
            Total_wvl[i] +=
                rpRange->chrngs[ii].wvlens.count();  // <-- count wvl
          }
        }
    ...
    */
    ...
    for (int i = 0; i < nstages_size; i++) {
      ...
      qDebug() << "DURATION!!! duration_sec = rpSpeed->ssteps[ curr_stage "
                  "].duration: "
               << duration_sec << "=" << rpSpeed->ssteps[curr_stage].duration;

      // <-- Which delay should we substract ? (not a stage delay but due to
      // acceleration ONLY ? )
      ...
      for (int j = 0; j < ncells; j++) {
        ...
        if (has_absorbance) {
          ...
          // Create query VALUE strings and Make insertions into AbsScanParams
          // table
          if (is_wvl_range) {
            ...
            for (int r = 0; r < nwavl; r++) {
              ...
              scan_intervals +=
                  QString::number(int(ScanInt));  // <-- ScanInterval
              if (r != nwavl - 1) {
                wvl_array += ",";
                scan_inner_array += ",";
                scan_outer_array += ",";
            ...
            if (!query_abs_scan.prepare(
                    QString("INSERT INTO %1 "
                            ...
                            "\"ScanStarts\",\"ScanSteps\",\"ScanTypeFlag\","
                            "\"WavelengthCount\",\"Wavelengths\","
                            "\"ScanCounts\",\"ScanIntervals\",\"RadialPath\") "
            ...
            // qDebug() << "Stop here" << ", ScanInt: " << scan_intervals << ",
            // ScanCount: " << scan_counts; return;

            if (query_abs_scan.exec()) {
              qDebug() << "AbsorbaceScanParameters record created";

              query_abs_scan.next();
              AbsScanIds[i][j] = query_abs_scan.value(0)
              ...
              QString errmsg =
                  "Create record error: " + query_abs_scan.lastError().text();
              ;
              QMessageBox::critical(
    ...
    /* WHAT TO INSERT: fields
       ...
       scan interval "ScanTypeFlag": " ",        >> default 'P' "ScanTypeName":
       " ",        >> default 'Interference' "Wavelength": " ",          >>
       default 660 nm  - laser wvl
    ...
    */
    ...
    for (int i = 0; i < nstages_size; ++i) InterScanIds[i].resize(ncells);
    ...
    for (int i = 0; i < nstages_size; i++) {
      ...
      // 2)  ));  //ALEXEY: do NOT divide by #cells ?
      // //ScanCount = int( duration_sec / (scanint_sec  ));
      // ScanInt   = scanint_sec;

      // ALEXEY: use this algorithm for Interference (same as for UV/vis, but
      // scanint_int_min = 5 sec)
      ...
      for (int j = 0; j < ncells; j++) {
        ...
        if (has_interference) {
          ...
          if (ScanCount > 1500) {
            QMessageBox::critical(
                this, tr("*ERROR* in Submitting Protocol: Interference"),
                tr("Protocol cannot be submitted: \n"
                   "Number of scans per cell per wavelengths is %1. \n"
                   "It must not exceed 1500. \n\n"
                   "Please revise experiment parameters accordingly.")
          ...
          QString query_str =
              QString(
                  "INSERT INTO %1 (\"ScanCount\",\"ScanInterval\") VALUES (%2, "
                  "%3) RETURNING \"ScanId\"")
                  .arg(qrytab_inter)
                  .arg(scan_count)
                  .arg(scan_interval);
          ...
          if (query_inter_scan.exec()) {
            ...
            InterScanIds[i][j] = query_inter_scan.value(0)
                                     .toInt();  // <-- Save InterScanID inserted
                                                // [for given stage#/cell#]
            qDebug() << "ScanId: " << query_inter_scan.value(0).toInt();
          } else {
            QString errmsg =
                "Create record error: " + query_inter_scan.lastError().text();
            ;
            QMessageBox::critical(
                this, tr("*ERROR* in Submitting Protocol"),
                tr("An error occurred in the attempt to submit"
    ...
    */
    ...
    for (int i = 0; i < nstages_size; ++i) CellIds[i].resize(ncells);
    ...
    for (int i = 0; i < nstages_size; i++) {
      for (int j = 0; j < ncells; j++) {
        ...
        int cellsect;
        if (j != ncells - 1) {
          /* Check here if RadialPath[i][j] is DEFAULT (both channels) OR A/B:
           * if DEFAULT, CellSector=2, if A/B CellSector=1  */
        ...
        for (int ii = 0; ii < rpSolut->nschan; ii++) {
          ...
          if (channel_cell.startsWith(QString::number(j + 1))) {
            ...
            {
              // solname += ": A: " + sol_split[0] + ", "        // <-- solution
              // name
              // 	+ sol_split[sol_split.size()-1] + "; ";  // <-- solution
              // manual comment
              solname += ": A: " + solution + "; ";
            }
            ...
            {
              // solname += "B: " + sol_split[0] + ", "          // <-- solution
              // name
              //   + sol_split[sol_split.size()-1];             // <-- solution
              //   manual comment
              solname += "B: " + solution;
        ...
        if (i == 0 && tem_delay_sec) {
          cell_query_str =
              QString(
                  "INSERT INTO %1 "
                  "(\"CellPosition\",\"CellSectors\",\"SampleName\") VALUES "
                  "(%2, %3, %4) RETURNING \"CellParamId\"")
                  .arg(qrytab_cell)
                  .arg(cell_pos)
                  .arg(cell_sector)
                  .arg(solname);
        ...
        // <-- Active stages
          ...
          if (AbsScanIds[i][j] && !InterScanIds[i][j]) {
            cell_query_str =
                QString(
                    "INSERT INTO %1 "
                    "(\"CellPosition\",\"CellSectors\",\"AbsorbanceScan\","
                    "\"AbsorbanceScanId\",\"SampleName\") VALUES (%2, %3, %4, "
                    "%5, %6) RETURNING \"CellParamId\"")
                    .arg(qrytab_cell)
                    .arg(cell_pos)
                    .arg(cell_sector)
                    .arg("\'TRUE\'")
          ...
          // <-- Active Stage: ONLY InterferenceScan exists
            ...
            cell_query_str =
                QString(
                    "INSERT INTO %1 "
                    "(\"CellPosition\",\"CellSectors\",\"InterferenceScan\","
                    "\"InterferenceScanId\",\"SampleName\") VALUES (%2, %3, "
                    "%4, %5, %6) RETURNING \"CellParamId\"")
                    .arg(qrytab_cell)
                    .arg(cell_pos)
                    .arg(cell_sector)
                    .arg("\'TRUE\'")
          ...
          // <-- Active Stage: BOTH AbsScan && InterferenceScan exist - RARE
          // CASE
          else if (AbsScanIds[i][j] && InterScanIds[i][j]) {
            ...
            cell_query_str =
                QString(
                    "INSERT INTO %1 "
                    "(\"CellPosition\",\"CellSectors\",\"AbsorbanceScan\","
                    "\"AbsorbanceScanId\",\"InterferenceScan\","
                    "\"InterferenceScanId\",\"SampleName\") VALUES (%2, %3, "
                    "%4, %5, %6, %7, %8) RETURNING \"CellParamId\"")
                    .arg(qrytab_cell)
                    .arg(cell_pos)
                    .arg(cell_sector)
                    .arg("\'TRUE\'")
                    ...
                    .arg(solname);
            comment =
                "Active Stage --> AbsScan and InterferenceScan BOTH EXIST";
        ...
        if (query_cell.exec()) {
          ...
          query_cell.next();
          CellIds[i][j] =
              query_cell.value(0).toInt();  // <-- Save CellID inserted [for
                                            // given stage#/cell#]
          qDebug() << "CellId: " << query_cell.value(0).toInt();
        } else {
          QString errmsg =
              "Create record error: " + query_cell.lastError().text();
          ;
          QMessageBox::critical(this, tr("*ERROR* in Submitting Protocol"),
                                tr("An error occurred in the attempt to submit"
                                   " protocol to AUC DB\n  %1 table\n  %2 .")
                                    .arg(qrytab_cell)
                                    .arg(errmsg));
    ...
    /* WHAT TO INSERT: fields
       ...
       each stage
        ...
        "StageDuration"        : [0,0,0,..]            <-- alway zeroth -exact
       amount of time required for completion of all stage science "StageStart"
       : [1-3h, 0, 0, ..]      <-- Delay (in seconds) for stage science start
       after target RPM attainment ONLY for dummy stage "StageRPM"             :
       [] "Stages"               : " ",                  <-- # of stages
        "Temperature"          : " ",
        "SystemStatusInterval" : " "                   <--  interval in seconds
       [1s] between system status record insertions
    */
    ...
    for (int i = 0; i < nstages_size; i++) {
      ...
      if (i == 0 && tem_delay_sec) {
        stagestart +=
            QString::number(tem_delay_sec);  // <-- stagestart dummy stages
        ...
        // QString::SkipEmptyParts))[0].toInt() );       // <-- RPM Active
        // stages ss += 5;
        if (is_dummy_fuge)
          curr_stage_fuge = i - 1;
        else
        ...
        stagerpm += QString::number(
            rpSpeed->ssteps[curr_stage_fuge].speed);  // <-- RPM Active stages
      }
    ...
    if (!query_fuge.prepare(
            QString(
                "INSERT INTO %1 "
                "(\"StageCellParameterIds\",\"StageDuration\",\"StageStart\","
                "\"StageRPM\",\"Stages\",\"SystemStatusInterval\","
                "\"Temperature\",\"StageAccelRate\",\"HoldSpeedAfterFinal\") "
                "VALUES (%2, %3, %4, %5, %6, %7, %8, %9, %10) RETURNING "
                "\"FugeRunProfileId\"")
                .arg(qrytab_fuge)
                .arg(cellids)
                .arg(stagedur)
    ...
    /* WHAT TO INSERT: fields
       ...
       "FugeRunProfileId": " ",    >> reference to the profile used; must be in
       the database when this object is added "Name": " ", "Project": " ",
       "Researcher": " ",
    */
    ...
    if (!query_expdef.prepare(
            QString("INSERT INTO %1 "
                    "(\"CellCount\",\"Comments\",\"FugeRunProfileId\",\"Name\","
                    "\"Researcher\") VALUES (%2, %3, %4, %5, %6) RETURNING "
                    "\"ExperimentId\"")
                .arg(qrytab_expdef)
                .arg(cellcount)
                .arg(exp_comments)
    ...
    if (query_expdef.exec()) {
      ...
      protocol_details["experimentId"] =
          QString::number(ExpDefId);  // this should be put into new table
                                      // connceting protocol && experiment
      protocol_details["experimentName"] = runname;
      protocol_details["protocolName"] =
          currProto
              ->protoname;  // pass also to Live Update/PostProd protocol name
      ...
      {
        // protocol_details[ "CellChNumber" ]   =
        // QString::number(rpSolut->nschan); // this can be read from protocol
        // in US-lims DB
        protocol_details["CellChNumber"] = QString::number(nchannels_uvvis);
        qDebug() << "UV: CellChNumber: " << protocol_details["CellChNumber"];
      ...
      } else if (ncells_interference && nwavl_tot)  // BOTH
      {
        ...
        QString triplenumber =
            QString("IP:") + QString::number(1 * int(ncells_interference / 2)) +
            QString(",RI:") + QString::number(nwavl_tot);
        protocol_details["TripleNumber"] = triplenumber;
...
void US_ExperGuiUpload::do_accept_reviewers(
    QMap<QString, QString>& protocol_details) {
  emit expdef_submitted(protocol_details);
}

void US_ExperGuiUpload::cancel_reviewers(
    QMap<QString, QString>& protocol_details) {
  emit expdef_submitted(protocol_details);
}

// autoflow record for Data from Disk
void US_ExperGuiUpload::add_autoflow_record_dataDisk(
    QMap<QString, QString>& protocol_details) {
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd();
  ...
  if (db != NULL) {
    qry.clear();
    // first, check max(ID) in the autoflowHistory table && set AUTO_INCREMENT
    // in the autoflow table to: greater of:
    ...
    /* SET THE FOLLOWING:
       ...
       i.e. 3. IMPORT )
       3. optimaName = protocol_details[ "OptimaName" ]  (i chose 'dataDisk' but
       see how it affects... )
    ...
    qry << "add_autoflow_record_datadisk" << protocol_details["protocolName"]
        << protocol_details["CellChNumber"] << protocol_details["TripleNumber"]
  ...
  if (!mainw->usmode || protocol_details["gmpRun"] == "YES") {
    ...
    for (int i = 0; i < sme_listList.size(); ++i) {
      // oper_rev_joinedList << appr_listList[i];    // <----- do NOT include
      // SME!
    ...
    **/
    ...
    logJsonFirstTime +=
        "\"" + QString::number(u_ID) + ". " + u_lname + ", " + u_fname + "\",";
    ...
    if (eSignID_returned == 0) {
      QMessageBox::warning(this, tr("New eSign Record Problem"),
                           tr("autoflowGMPRecordEsign: There was a problem "
    ...
    qry.clear();
    qry << "update_autoflow_with_gmpReviewID" << protocol_details["autoflowID"]
    ...
    /********************************************************************************/
    // Create autoflowStatus record (gmp_submitter_map["User:"], ["Comment:"],
    // ["Master Password:"])
    /********************************************************************************/
    ...
    /** IF SWL or MWL **/
    /************** IF ABDE - add record to the new 'autoflowAnalysisABDEStages'
     * table *****/
    /** status 'unknown' ****/
    if (protocol_details["expType"] == "ABDE") {
      ...
      qry << "new_autoflowAnalysisABDE_record" << protocol_details["autoflowID"]
          << etype_abde;
...
void US_ExperGuiUpload::add_autoflow_record(
    QMap<QString, QString>& protocol_details) {
  ...
  if (db != NULL) {
    ...
    qry << "add_autoflow_record" << protocol_details["protocolName"]
        << protocol_details["CellChNumber"] << protocol_details["TripleNumber"]
        << protocol_details["duration"] << protocol_details["experimentName"]
  ...
  if (autoflowID_returned == 0) {
    QMessageBox::warning(this, tr("New Autoflow Record Problem"),
                         tr("autoflow: There was a problem with creating a new "
                            "autoflow record! \n"));
    return;
  }
  /*******************************************************************************/
...
void US_ExperGuiUpload::add_autoflow_record_protDev(
    QMap<QString, QString>& protocol_details) {
  ...
  if (db != NULL) {
    ...
    qry << "add_autoflow_record_dev" << protocol_details["protocolName"]
        ...
        << protocol_details["aprofileguid"] << protocol_details["operatorID"];
    ...
    if (new_autoflowID == 0) {
      QMessageBox::warning(this, tr("New Autoflow Record Problem"),
                           tr("autoflow: There was a problem with creating a "
                              "new autoflow record! \n"));
      return;
    }
  }
...
// autoflowIntensity record, copy it into new record & prescribe newly generated
// autoflowID!!!
...
// ), 				    tr( "ProtDev, autoflowIntensity: There was a problem with creating a
// record in autoflowIntensity table \n" ) + db->lastError() );
...
QString US_ExperGuiUpload::buildJson(void) {
  ...
  for (int ii = 0; ii < scentp.count(); ii++) {
    ...
    QString scell =
        QString(centp).section(":", 0, 0).section(" ", 1, 1).simplified();
