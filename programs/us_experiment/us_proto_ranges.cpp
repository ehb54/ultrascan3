...
US_ExperGuiRanges::US_ExperGuiRanges(QWidget *topw) : US_WidgetsDialog(topw, 0) {
   ...
   for (int ii = 0; ii < mxrow; ii++) { // Loop to build initial place-holder ranges rows
      QString scel;
      if (ii == 0)
         scel = QString("1 / A");
      else if (ii == 1)
      ...
      connect(ck_buff_spectrum, SIGNAL(toggled(bool)), this, SLOT(buffer_spectrum_checked(bool)));
      ...
      connect(
         pbwavln, SIGNAL(clicked()), //ALEXEY
         this, SLOT(Wavelengths_class()));
      connect(ctradfr, SIGNAL(valueChanged(double)), this, SLOT(changedLowRadius(double)));
      connect(ctradto, SIGNAL(valueChanged(double)), this, SLOT(changedHighRadius(double)));
#if 0
...
void US_ExperGuiRanges::rebuild_Ranges(void) {
   ...
   DbgLv(1) << "EGRn: rbR: nuvvis" << nuvvis << "nrange_sv" << nrange_sv << "nrnchan" << nrnchan
            << " rpRanges->nranges " << rpRange->nranges;

   //   if ( nrange_sv == nuvvis  &&  nuvvis != 0 )
   //      return;                           // No optical change means no rebuild

   DbgLv(1) << "EGRn: rbR:  nrnchan" << nrnchan;
   if (nrange_sv == 0) { // No existing Ranges protocol, so init with rudimentary one
      ...
      for (int ii = 0; ii < oprof.count(); ii++) {
         ...
         if (oprof[ ii ].contains(uvvis)) {
            ...
            DbgLv(1) << "Rn:CONTENT 11 inside: channel, wavelength: " << rpRange->chrngs[ kuv ].channel
      ...
      DbgLv(1) << "EGRn: rbR:  dummy proto  oprof count" << oprof.count() << "nuvvis" << nuvvis << " rpRanges->nranges "
               << rpRange->nranges;
      return;
   }
...
DbgLv(1) << "EGRn: rbR:  ii lorad hirad" << locrads[ii] << hicrads[ii];
   ...
   for (int ii = 0; ii < nochan; ii++) {
      ...
      for (int jj = 0; jj < nrange_sv; jj++) {
         if (chrngs_sv[ jj ].channel == channel) { // Match in old protocol:  save its index
            spx = jj;
            break;
         }
      }
   ...
   if (nrnchan_sv > 0) { // Use previous panel parameters
      for (int ii = 0; ii < nrnchan; ii++) {
         ...
         if (ppx >= 0) {
            DbgLv(1) << "EGRn: rbR:     sizes: rch swv lor hir" << rchans.count() << swvlens.count() << locrads.count()
                     << hicrads.count();
...
bool US_ExperGuiRanges::iStwoOrMoreAnalytesSpectra_forChannel(
   ...
   qDebug() << "In CHECK SPECTRA" << "[" << mode << "]: channel, channelName, nwavl, buff_req, solID -- " << channel
...
bool US_ExperGuiRanges::validExtinctionProfile(
...
void US_ExperGuiRanges::selectWavelengths_manual() {
   ...
   if (kswavl == 0)
      ...
      labwlr = QString::number(kswavl) + ",  " + wlselec[ 0 ] + tr(" to ") + wlselec[ lswx ];
   ...
   for (int i = 0; i < nsp; i++) {
      ...
      if (has_interference) {
         ...
         qDebug() << "RANGES SET MANUAL Interference: duration_sec , scanint_sec_int, ncells_used -- " << duration_sec
...
US_SelectWavelengths::US_SelectWavelengths(QStringList &orig_wavls, QStringList &select_wavls) :
    US_WidgetsDialog(0, 0), orig_wavls(orig_wavls), select_wavls(select_wavls) {
   ...
   le_original = us_lineedit(tr("%1 potential wavelengths").arg(nbr_poten), -1, true);
   le_selected = us_lineedit(tr("%1 selected wavelengths").arg(nbr_selec), -1, true);
   ...
   for (int ii = 0; ii < nbr_poten; ii++) { // Add items to the original (potential) list
   ...
   connect(ct_strwln, SIGNAL(valueChanged(double)), this, SLOT(new_wl_start(double)));
   connect(ct_endwln, SIGNAL(valueChanged(double)), this, SLOT(new_wl_end(double)));
   connect(ct_incwln, SIGNAL(valueChanged(double)), this, SLOT(new_wl_incr(double)));
...
US_SelectWavelengths_manual::US_SelectWavelengths_manual(QStringList &orig_wavls, QStringList &select_wavls) :
    US_WidgetsDialog(0, 0), orig_wavls(orig_wavls), select_wavls(select_wavls) {
   nbr_poten = orig_wavls.count();
   nbr_selec = select_wavls.count();
   DbgLv(1) << "SelWl: IN k_ori k_sel" << nbr_poten << nbr_selec;
   ...
   le_info->setText(
      tr("NOTES ON SYNTAX: \n The following wavelength(s) specification examples are accepted: \n   * 230-550:N (take "
         "every Nth wavelength, starting with 230 nm and go up to 550 nm);  \n   * 220,230,250 (measure at 220 nm, 230 "
         "nm, and at 250 nm); \n   * 220-230 (measure at 220, 221, 222, 223... in 1 nm increments when ':N' is not "
   ...
   connect(le_wrange, SIGNAL(textChanged(QString)), this, SLOT(wln_changed(QString)));
...
bool US_SelectWavelengths_manual::text_to_numbers(void) {
   ...
   if (selected.size() > 100) {
      ...
      QString mtitle_error = tr("Error");
      QString message_error = tr("Number of wavelengths is %1. <br> The number of selected wavelengths is more than "
                                 "100! <br> Make wavelength count less than or equal to 100.");
...
void US_SelectWavelengths::add_selections() {
   ...
   for (int ii = 0; ii < kntsel; ii++) { // Move selected items from original to selected
...
void US_SelectWavelengths::rmv_selections() {
   ...
   for (int ii = 0; ii < kntsel; ii++) { // Move selected items from selected to potential (if in range)
      QListWidgetItem *l_item = selitems.at(ii);
      QString waveln = l_item->text();
...
void US_SelectWavelengths::report(void) {
   // Report the new state of things
   le_original->setText(
      (nbr_poten == 1) ? tr("1 potential wavelength")
                       : tr("%1 potential wavelengths").arg(nbr_poten) + tr(" (of %1 in range)").arg(nbr_range));
   le_selected->setText((nbr_selec == 1) ? tr("1 selected wavelength") : tr("%1 selected wavelengths").arg(nbr_selec));
...
void US_SelectWavelengths::new_wl_range(const int wls, const int wle, const int wli) {
