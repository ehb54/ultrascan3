    ...
    : US_WidgetsDialog (topw, 0)
{
  ...
  QLabel *lb_panel
      = us_banner (tr ("7: Specify wavelength and radius ranges"));
  panel->addWidget (lb_panel);

  // QPushButton* pb_details  = us_pushbutton( tr( "View Current Range
  // Settings" ) );
  QPushButton *pb_details = us_pushbutton (tr ("View Ranges"));
  connect (pb_details, SIGNAL (clicked ()), this, SLOT (detailRanges ()));

  // Show also a scan interval (if it was updated - in RED !!!)
  QLabel *lb_scanint
      = us_label ("Scan Interval: UV/vis. (in red if updated): ");
  ...
  // !!!)
  QLabel *lb_scanint_int
      = us_label ("Scan Interval: Interference (in red if updated): ");
  le_scanint_int = us_lineedit ("", 0, true);

  // Show listbox with ScanCount per stage
  cb_scancount_int = new QComboBox;

  // cb_scancount->addItem( tr( "Stage %1. Number of Scans: %2 " ).arg(1).arg(
  // 0 ) );
  ...
  for (int ii = 0; ii < mxrow; ii++)
    { // Loop to build initial place-holder ranges rows
      ...
      // abde
      QLayout *lo_buff_spectrum
          = us_checkbox (tr ("Buffer Spectrum"), ck_buff_spectrum, false);
      ...
      containerW_buff_sp->setObjectName (strow
                                         + ": ck_buff_spectrum_container");
      connect (ck_buff_spectrum, SIGNAL (toggled (bool)), this,
               SLOT (buffer_spectrum_checked (bool)));
      ...
      connect (ctradfr, SIGNAL (valueChanged (double)), this,
               SLOT (changedLowRadius (double)));
      connect (ctradto, SIGNAL (valueChanged (double)), this,
               SLOT (changedHighRadius (double)));
#if 0
...
void
US_ExperGuiRanges::rebuild_Ranges (void)
{
  ...
  DbgLv (1) << "EGRn: rbR: nuvvis" << nuvvis << "nrange_sv" << nrange_sv
            << "nrnchan" << nrnchan << " rpRanges->nranges "
            << rpRange->nranges;

  //   if ( nrange_sv == nuvvis  &&  nuvvis != 0 )
  //      return;                           // No optical change means no
  //      rebuild
  ...
  if (nrange_sv == 0)
    { // No existing Ranges protocol, so init with rudimentary one
      ...
      // ALEXEY BUG fixed missed resizing/filling out following arrays: rchans,
      // swvlens, locrads, hicrads
          ...
          if (oprof[ii].contains (uvvis))
            {
              ...
              DbgLv (1) << "Rn:CONTENT 11 inside: channel, wavelength: "
                        << rpRange->chrngs[kuv].channel
                        << rpRange->chrngs[kuv].wvlens;
              ...
              for (int jj = 0; jj < nwavl; jj++)
                {
                  ...
                  swvlens[kuv] << wavelen;
                  DbgLv (1) << "EGRn: rbR:   kuv jj " << kuv << jj << "wavelen"
                            << wavelen;
                }
              ...
              abde_mwl_deconv[kuv]
                  = rpRange->chrngs[kuv].abde_mwl_deconvolution;

              if (++kuv >= nuvvis)
                break;
            }
        }
      DbgLv (1) << "EGRn: rbR:  dummy proto  oprof count" << oprof.count ()
                << "nuvvis" << nuvvis << " rpRanges->nranges "
                << rpRange->nranges;
      return;
    }
  ...
  DbgLv (1) << "EGRn: rbR:  pprotoname" << protoname << "cur_pname"
            << cur_pname;
...
DbgLv(1) << "EGRn: rbR:  ii lorad hirad" << locrads[ii] << hicrads[ii];
  ...
  for (int ii = 0; ii < nochan; ii++)
    {
      ...
      DbgLv (1) << "EGRn: rbR:   ii" << ii << "chan" << channel << "pentry"
                << pentry;

      if (!pentry.contains (uvvis))
        continue;
  ...
  if (nrnchan_sv > 0)
      ...
      for (int ii = 0; ii < nrnchan; ii++)
        {
          ...
          DbgLv (1) << "EGRn: rbR:    ii" << ii << "channel" << channel
                    << "ppx" << ppx;
          if (ppx >= 0)
            {
              DbgLv (1) << "EGRn: rbR:     sizes: rch swv lor hir"
                        << rchans.count () << swvlens.count ()
                        << locrads.count () << hicrads.count ();
    ...
    { // Create first shot at panel parameters
      ...
      for (int ii = 0; ii < nrnchan; ii++)
        {
          // DbgLv(1) << "EGRn: rbR:    ii" << ii << "channel" <<
          // rpRange->chrngs[ii].channel;  //ALEXEY potential bug here: causes
          // crash when protocol Re-loaded
          qDebug () << "Test ";
...
US_ExperGuiRanges::detailRanges ()
{
  ...
  dtext
      += tr ("Number of Protocol Channels Used:  %1\n").arg (rpRange->nranges);
...
void
US_ExperGuiRanges::Wavelengths_class ()
{
  QMessageBox msgBox;
  msgBox.setText (tr ("How do you want to specify wavelengths?"));
  msgBox.setInformativeText ("Wavelength(s) can be specified manually, or "
                             "with the use of wavelength selector");
  msgBox.setWindowTitle (tr ("Select Wavelength(s)"));
  QPushButton *Manual
      = msgBox.addButton (tr ("Manually"), QMessageBox::YesRole);
  QPushButton *Selector
      = msgBox.addButton (tr ("Use Selector"), QMessageBox::YesRole);
  QPushButton *Cancel
...
// Check for..
bool
US_ExperGuiRanges::iStwoOrMoreAnalytesSpectra_forChannel (
    QString channelName, QStringList &msg_to_user, QString mode, int chrow)
{
  ...
  qDebug () << "In CHECK SPECTRA" << "[" << mode
            << "]: channel, channelName, nwavl, buff_req, solID -- " << channel
            << channelName << nwavl << buff_req << sol_id;
      ...
      for (int i = 0; i < num_analytes; ++i)
        {
          ...
          if (!validExtinctionProfile (
                  a_desc, all_wvls, analyte.extinction.keys (), msg_to_user))
  ...
  //       //Check if ext. profile: (1) exists; (2) in range of specs
  //       channel-wvls. QString b_desc = "BUFFER: " + b_name; if (
  //       !validExtinctionProfile( b_desc, all_wvls, buffer.extinction.keys(),
  //       msg_to_user ) )
  // 	profiles_exist = false;
  //     }
...
bool
US_ExperGuiRanges::validExtinctionProfile (QString desc,
                                           QList<double> all_wvls,
...
// Slot to select wavelengths using a dialog
void
US_ExperGuiRanges::selectWavelengths_manual ()
{
  QObject *sobj = sender (); // Sender object
  QString sname = sobj->objectName ();
  chrow = sname.section (":", 0, 0).toInt ();
  ...
  // Open a wavelengths_MANUAL choice dialog and use to update selected
  // wavelengths

  US_SelectWavelengths_manual *swdiag
      = new US_SelectWavelengths_manual (wlpoten, wlselec);
  ...
  if (kswavl == 0)
    ...
    {
      labwlr = QString::number (kswavl) + ",  " + wlselec[0] + tr (" to ")
               + wlselec[lswx];

      // abde: setup buff_spectrum cks
      QStringList msg_to_user;
      if (mainw->us_abde_mode
          && iStwoOrMoreAnalytesSpectra_forChannel (cclabl, msg_to_user,
                                                    "SETWVLS", chrow))
        {
          genL->addWidget (cc_buff_sp[chrow], chrow, 16, 1, 2);
          cc_buff_sp[chrow]->setVisible (true);
          qDebug () << "Manual wvls setup: adding [add]o_name "
                    << cc_buff_sp[chrow]->objectName ();
  ...
  // ALEXEY: here set of channel A is copied to B
      ...
      if (mainw->us_abde_mode)
        {
          QStringList msg_to_user;
          if (kswavl > 1
              && iStwoOrMoreAnalytesSpectra_forChannel (cclabl_b, msg_to_user,
                                                        "SETWVLS", ccrow))
            {
              genL->addWidget (cc_buff_sp[ccrow], ccrow, 16, 1, 2);
              cc_buff_sp[ccrow]->setVisible (true);
              qDebug () << "Manual wvls setup: adding [add]o_name "
                        << cc_buff_sp[ccrow]->objectName ();
  ...
  for (int i = 0; i < nsp; i++)
    {
      ...
      qDebug () << "RANGES SET MANUAL: duration_sec , scanint_sec, "
                   "scanint_sec_min,  tot_wvl, ncells_used -- "
                << duration_sec << scanint_sec << scanint_sec_min << tot_wvl
                << ncells_used;
      ...
      if (has_interference)
        {
          ...
          qDebug () << "RANGES SET MANUAL Interference: duration_sec , "
...
// Slot to select wavelengths using a dialog
void
US_ExperGuiRanges::selectWavelengths ()
{
  QObject *sobj = sender (); // Sender object
  QString sname = sobj->objectName ();
  chrow = sname.section (":", 0, 0).toInt ();
  ...
  for (int i = 0; i < nsp; i++)
    {
      ...
      qDebug () << "RANGES SET SELECTOR: duration_sec , scanint_sec, "
                   "scanint_sec_min,  tot_wvl, ncells_used -- "
                << duration_sec << scanint_sec << scanint_sec_min << tot_wvl
                << ncells_used;
...
void
US_ExperGuiRanges::changedLowRadius (double val)
{
  QObject *sobj = sender (); // Sender object
  QString sname = sobj->objectName ();
  chrow = sname.section (":", 0, 0).toInt ();
  ...
  // ALEXEY - simulate the same counters coupling for same cell as for the
  // (Interference) checkboxes in Optics
  //  get a list of same-cell rows; disconnect
  QString clabl = cc_labls[chrow]->text ();
  QString scell = clabl.left (1);
  QString labnone = tr ("none");
...
void
US_ExperGuiRanges::changedHighRadius (double val)
{
  QObject *sobj = sender (); // Sender object
  QString sname = sobj->objectName ();
  chrow = sname.section (":", 0, 0).toInt ();
...
void
US_ExperGuiRanges::buffer_spectrum_checked (bool checked)
{
  QObject *sobj = sender (); // Sender object
  QString sname = sobj->objectName ();
  chrow = sname.section (":", 0, 0).toInt ();
...
US_SelectWavelengths::US_SelectWavelengths (QStringList &orig_wavls,
      ...
      select_wavls (select_wavls)
{
  ...
  le_original = us_lineedit (tr ("%1 potential wavelengths").arg (nbr_poten),
                             -1, true);
  le_selected
      = us_lineedit (tr ("%1 selected wavelengths").arg (nbr_selec), -1, true);
  ...
  DbgLv (1) << "SelWl:    k_pot k_sel" << potential.count ()
            << selected.count ();
  ...
  connect (ct_strwln, SIGNAL (valueChanged (double)), this,
           SLOT (new_wl_start (double)));
  connect (ct_endwln, SIGNAL (valueChanged (double)), this,
           SLOT (new_wl_end (double)));
  connect (ct_incwln, SIGNAL (valueChanged (double)), this,
           SLOT (new_wl_incr (double)));
      ...
      select_wavls (select_wavls)
{
  ...
  le_info = us_textedit ();
  le_info->setText (tr (
      "NOTES ON SYNTAX: \n The following wavelength(s) specification examples "
      "are accepted: \n   * 230-550:N (take every Nth wavelength, starting "
      "with 230 nm and go up to 550 nm);  \n   * 220,230,250 (measure at 220 "
      "nm, 230 nm, and at 250 nm); \n   * 220-230 (measure at 220, 221, 222, "
      "223... in 1 nm increments when ':N' is not specified); \n   * "
  ...
  connect (le_wrange, SIGNAL (textChanged (QString)), this,
           SLOT (wln_changed (QString)));
...
void
US_SelectWavelengths_manual::reset (void)
{
  le_wrange->setText ("");
  pb_accept->setEnabled (false);
  QPalette *palette = new QPalette ();
...
void
US_SelectWavelengths_manual::cancel (void)
{
  reset ();
...
void
US_SelectWavelengths_manual::wln_changed (QString text)
{
  QPalette *palette = new QPalette ();
  palette->setColor (QPalette::Text, Qt::black);
  palette->setColor (QPalette::Base, Qt::white);
...
// Entered wavelength(s):  check for syntax
bool
US_SelectWavelengths_manual::wln_entered (void)
{
  // CHECK for syntax
  ...
  // QRegExp
  // rx4("[(\\d{3}),]*(\\d{3})-(\\d{3})[,(\\d{3})]*(\\d{3})-(\\d{3})[,(\\d{3})]*");
  // QRegExp
  // rx5("[(\\d{3}),]*(\\d{3})-(\\d{3}):(\\d+)[,(\\d{3})]*(\\d{3})-(\\d{3}):(\\d+)[,(\\d{3})]*");
  // QRegExp
  // rx6("[(\\d{3}),]*(\\d{3})-(\\d{3}):(\\d+)[,(\\d{3})]*(\\d{3})-(\\d{3})[,(\\d{3})]*");
  // QRegExp
  // rx7("[(\\d{3}),]*(\\d{3})-(\\d{3})[,(\\d{3})]*(\\d{3})-(\\d{3}):(\\d+)[,(\\d{3})]*");
  // QRegExp rx8("[(\\d{3}-\\d{3})]*[(:\\d+)]*[(,)]*");
  // QRegExp rx9("[(\\d{3}-\\d{3}:\\d+),]+");
  // QRegExp rx_new("[\\d{3},]*[\\d{3}-\\d{3}]*[(:\\d+)]*[,(\\d{3})]*");
  // //working partially
  ...
  if (rx_new.exactMatch (text)
      ...
      //|| rx4.exactMatch(text) || rx5.exactMatch(text) || rx6.exactMatch(text)
      //||  rx7.exactMatch(text)
      //|| rx8.exactMatch(text)
      //|| rx9.exactMatch(text)
  )
...
bool
US_SelectWavelengths_manual::text_to_numbers (void)
{
  ...
  if (waveln_raw.contains (","))
    {
      ...
      for (QStringList::iterator it = wvllist.begin (); it != wvllist.end ();
           ++it)
        {
          QString current = *it;
          ...
          if (current.contains (":"))
            {
              ...
              QStringList wvllist_semicolon_dash
                  = wvllist_semicolon[0].split ("-");
  ...
  else if (waveln_raw.contains (":") && !waveln_raw.contains (","))
    {
      ...
      QStringList wvllist_semicolon_nocoma_dash
          = wvllist_semicolon_nocoma[0].split ("-");
  ...
  qDebug () << "First and Last element: " << selected[0].toDouble () << ", "
            << selected[selected.size () - 1].toDouble ();

  if (selected[0].toDouble () < 180.0
      || selected[selected.size () - 1].toDouble ()
             > 800.0) // Boundaries [180-800] nm
    {
      QPalette *palette = new QPalette ();
      palette->setColor (QPalette::Text, Qt::red);
      palette->setColor (QPalette::Base, Qt::white);
      ...
      QString mtitle_error = tr ("Error");
      QString message_error = tr ("Selected wavelengths are outside of "
                                  "accepted range of [180-800] nm!");
      QMessageBox::critical (this, mtitle_error, message_error);
  ...
  if (selected.size () > 100)
    {
      ...
      QString message_error
          = tr ("Number of wavelengths is %1. <br> The number of selected "
                "wavelengths is more than 100! <br> Make wavelength count "
                "less than or equal to 100.");
      QMessageBox::critical (this, mtitle_error,
                             message_error.arg (selected.size ()));
...
void
US_SelectWavelengths_manual::done (void)
{
  if (wln_entered ())
    {
      accept ();
...
void
US_SelectWavelengths::add_selections ()
{
...
void
US_SelectWavelengths::rmv_selections ()
{
...
void
US_SelectWavelengths::add_all_selects ()
{
...
void
US_SelectWavelengths::rmv_all_selects ()
{
...
void
US_SelectWavelengths::report (void)
{
  // Report the new state of things
...
// Reset the lists and buttons to their original state
void
US_SelectWavelengths::reset (void)
{
  lw_original->clear ();
  lw_selected->clear ();
  potential = orig_wavls;
...
// Cancel button clicked:  returned delete-selections is empty
void
US_SelectWavelengths::cancel (void)
{
  //   select_wavls.clear();
  reset ();
...
// Accept button clicked:  returned delete-selections list is the excluded list
void
US_SelectWavelengths::done (void)
{
  if (selected.size () > 100)
    {
      QString mtitle_error = tr ("Error");
...
void
US_SelectWavelengths::new_wl_start (double val)
{
  DbgLv (1) << "SelWl: neww_str: " << val;
  new_wl_range ((int)val, (int)ct_endwln->value (), (int)ct_incwln->value ());
}

// Slot for change in wavelength range end
void
US_SelectWavelengths::new_wl_end (double val)
{
  DbgLv (1) << "SelWl: neww_end: " << val;
  new_wl_range ((int)ct_strwln->value (), (int)val, (int)ct_incwln->value ());
}

// Slot for change in wavelength range increment
void
US_SelectWavelengths::new_wl_incr (double val)
{
  DbgLv (1) << "SelWl: neww_inc: " << val;
  new_wl_range ((int)ct_strwln->value (), (int)ct_endwln->value (), (int)val);
}

// Function for change in potential wavelength range
void
US_SelectWavelengths::new_wl_range (const int wls, const int wle,
                                    const int wli)
{
  ...
  for (int iwvl = wls; iwvl <= wle; iwvl += wli)
    {
      ...
      if (!potential.contains (wavel) && !selected.contains (wavel))
        {
  ...
  report (); // Report counts, enable buttons
  DbgLv (1) << "SelWl: newwr: @rtn: nbr_poten nbr_selec" << nbr_poten
            << nbr_selec;
}
