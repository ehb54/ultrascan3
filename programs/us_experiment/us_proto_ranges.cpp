...
US_ExperGuiRanges::US_ExperGuiRanges(QWidget* topw)
  ...
  for (int ii = 0; ii < mxrow;
       ii++) {  // Loop to build initial place-holder ranges rows
    QString scel;
    if (ii == 0)
      scel = QString("1 / A");
    else if (ii == 1)
...
void US_ExperGuiRanges::rebuild_Ranges(void) {
  ...
  if (nrange_sv ==
      0) {  // No existing Ranges protocol, so init with rudimentary one
    ...
    for (int ii = 0; ii < oprof.count(); ii++) {
      ...
      if (oprof[ii].contains(uvvis)) {
        ...
        DbgLv(1) << "Rn:CONTENT 11 inside: channel, wavelength: "
                 << rpRange->chrngs[kuv].channel << rpRange->chrngs[kuv].wvlens;
    ...
    DbgLv(1) << "EGRn: rbR:  dummy proto  oprof count" << oprof.count()
             << "nuvvis" << nuvvis << " rpRanges->nranges " << rpRange->nranges;
    return;
  }
...
DbgLv(1) << "EGRn: rbR:  ii lorad hirad" << locrads[ii] << hicrads[ii];
  ...
  for (int ii = 0; ii < nochan; ii++) {
    ...
    for (int jj = 0; jj < nrange_sv; jj++) {
      if (chrngs_sv[jj].channel ==
          channel) {  // Match in old protocol:  save its index
        spx = jj;
        break;
      }
    }
    ...
    if (spx < 0) {  // No such channel in old protocol:  use basic entry
    ...
    } else {  // Match to old protocol:  use that entry
  ...
  if (swvlens.count() < nrnchan) swvlens.resize(nrnchan);
  DbgLv(1) << "EGRn: rbR:  nrnchan" << nrnchan << "nrnchan_sv" << nrnchan_sv;

  // Rebuild panel parameters
  if (nrnchan_sv > 0) {  // Use previous panel parameters
    for (int ii = 0; ii < nrnchan; ii++) {
      QString channel = rpRange->chrngs[ii].channel;
      int ppx = rchans.indexOf(channel);
      DbgLv(1) << "EGRn: rbR:    ii" << ii << "channel" << channel << "ppx"
               << ppx;
      if (ppx >= 0) {
        DbgLv(1) << "EGRn: rbR:     sizes: rch swv lor hir" << rchans.count()
                 << swvlens.count() << locrads.count() << hicrads.count();
  ...
  } else {  // Create first shot at panel parameters
    ...
    for (int ii = 0; ii < nrnchan; ii++) {
      ...
      // rpRange->chrngs[ii].channel;  //ALEXEY potential bug here: causes crash
      // when protocol Re-loaded
      qDebug() << "Test ";
...
void US_ExperGuiRanges::detailRanges() {
  ...
  if (nrnchan != rpRange->nranges) {  // Should not occur!
    dtext += tr("\n*ERROR* Channel counts should be identical\n");
    nrnchan = 0;  // Skip channel details; too risky
  }

  for (int ii = 0; ii < nrnchan; ii++) {  // Show information for each channel
    dtext += tr("\n  Channel ") + rchans[ii] + " : \n";
    ...
    for (int jj = 0; jj < swvlens[ii].size(); jj++) {
      ...
      if (jj != swvlens[ii].size() - 1) dtext += tr(", ");
      if (counter % 8 == 0) dtext += tr("\n                                  ");
    }
    dtext += tr("\n");
...
void US_ExperGuiRanges::Wavelengths_class() {
  ...
  msgBox.setInformativeText(
      "Wavelength(s) can be specified manually, or with the use of wavelength "
      "selector");
...
// Check for..
bool US_ExperGuiRanges::iStwoOrMoreAnalytesSpectra_forChannel(
  ...
  if (num_analytes > 1) {
    ...
    for (int i = 0; i < num_analytes; ++i) {
      ...
      qDebug() << "Solution " << solution->solutionDesc << ", (GUID)Analyte "
               << "(" << a_GUID << ")" << a_name << ", (ID)Analyte " << "("
               << a_ID << ")" << a_name;
      ...
      if (!validExtinctionProfile(a_desc, all_wvls, analyte.extinction.keys(),
                                  msg_to_user))
...
bool US_ExperGuiRanges::validExtinctionProfile(QString desc,
                                               QList<double> all_wvls,
...
void US_ExperGuiRanges::selectWavelengths_manual() {
  ...
  for (int ii = 0; ii < nrnchan; ii++) {  // Show information for each channel
    ...
    for (int jj = 0; jj < kswavl; jj++) {
      ...
      if (!wlpoten.contains(swavl)) wlpoten << swavl;
    }
  }
  wlpoten.sort();
  ...
  for (int ii = 0; ii < kswavl; ii++) swvlens[chrow] << wlselec[ii].toDouble();
  ...
  if (kswavl == 0)
    ...
    QStringList msg_to_user;
    if (mainw->us_abde_mode && iStwoOrMoreAnalytesSpectra_forChannel(
                                   cclabl, msg_to_user, "SETWVLS", chrow)) {
  ...
  for (int ii = 0; ii < ccrows.count(); ii++) {
    ...
    if (mainw->us_abde_mode) {
      ...
      if (kswavl > 1 && iStwoOrMoreAnalytesSpectra_forChannel(
                            cclabl_b, msg_to_user, "SETWVLS", ccrow)) {
...
US_SelectWavelengths::US_SelectWavelengths(QStringList& orig_wavls,
  ...
  le_original =
      us_lineedit(tr("%1 potential wavelengths").arg(nbr_poten), -1, true);
  le_selected =
  ...
  for (int ii = 0; ii < nbr_poten;
       ii++) {  // Add items to the original (potential) list
    lw_original->addItem(potential[ii]);
  }
  for (int ii = 0; ii < nbr_selec; ii++) {  // Add items to the selected list
...
bool US_SelectWavelengths_manual::text_to_numbers(void) {
          ...
          800.0)  // Boundaries [180-800] nm
  {
    ...
    QString mtitle_error = tr("Error");
    QString message_error = tr(
        "Selected wavelengths are outside of accepted range of [180-800] nm!");
    QMessageBox::critical(this, mtitle_error, message_error);
  ...
  if (selected.size() > 100) {
    ...
    QString message_error =
        tr("Number of wavelengths is %1. <br> The number of selected "
           "wavelengths is more than 100! <br> Make wavelength count less than "
           "or equal to 100.");
...
void US_SelectWavelengths::add_selections() {
  ...
  for (int ii = 0; ii < kntsel;
       ii++) {  // Move selected items from original to selected
...
void US_SelectWavelengths::rmv_selections() {
  ...
  for (int ii = 0; ii < kntsel;
       ii++) {  // Move selected items from selected to potential (if in range)
    QListWidgetItem* l_item = selitems.at(ii);
    QString waveln = l_item->text();
...
void US_SelectWavelengths::rmv_all_selects() {
  ...
  for (int ii = 0; ii < nbr_selec; ii++) {
    ...
    if (rng_wavls.contains(selected[ii]) && !potential.contains(selected[ii]))
...
void US_SelectWavelengths::reset(void) {
  ...
  for (int ii = 0; ii < nbr_poten; ii++) lw_original->addItem(potential[ii]);

  for (int ii = 0; ii < nbr_selec; ii++) lw_selected->addItem(selected[ii]);
...
void US_SelectWavelengths::new_wl_range(const int wls, const int wle,
  ...
  for (int iwvl = wls; iwvl <= wle; iwvl += wli) {
    ...
    if (!potential.contains(wavel) && !selected.contains(wavel)) {
      ...
      if (iwvl < p_strwl || iwvl > p_endwl) potential << wavel;
