...
US_FDS_FileManager::US_FDS_FileManager() : US_Widgets() {
   ...
   connect(pb_save_first_and_last, SIGNAL(clicked()), SLOT(save_first_and_last()));
   ...
   plot = new US_Plot(data_plot, tr("Fluorescence Data"), tr("Radius (in cm)"), tr("Fluorescence Intensity"));
...
void US_FDS_FileManager::load(void) {
   ...
   source_dir = fd.getExistingDirectory(
   ...
   if (source_dir.right(1) != "/")
      source_dir += "/"; // Ensure trailing "/"
   files = QDir(readDir).entryList(QDir::Files | QDir::NoDotAndDotDot, QDir::Name);
   parse_files();
}
...
void US_FDS_FileManager::parse_files(void) {
   ...
   for (i = 0; i < files.size(); i++) {
      ...
      if (f.open(QFile::ReadOnly)) {
         ...
         tmp_scaninfo.triple = str1.setNum(tmp_scaninfo.cell) + " / " + tmp_scaninfo.channel + " / "
                               + str2.setNum(tmp_scaninfo.lambda);
         tmp_scaninfo.gainset = "V: " + str1.setNum(tmp_scaninfo.voltage) + ", G: " + str.setNum(tmp_scaninfo.gain)
   ...
   connect(cb_triple, SIGNAL(currentIndexChanged(int)), SLOT(select_triple(int)));
...
void US_FDS_FileManager::select_rpm(int index) {
   ...
   for (int i = 0; i < scaninfo.size(); i++) {
      if (
         scaninfo[ i ].triple == cb_triple->currentText() && str1.setNum(scaninfo[ i ].rpm) == cb_rpms->currentText()
         && scaninfo[ i ].include) {
...
void US_FDS_FileManager::select_gain(int index) {
   ...
   connect(ct_to, SIGNAL(valueChanged(double)), SLOT(focus_to(double)));
   le_info->setText(
      "Loaded " + str.setNum(scaninfo.size()) + " scans containing " + str1.setNum(cb_triple->count())
      + " triples. The current triple (" + cb_triple->currentText() + ") contains " + str2.setNum(cb_rpms->count())
...
void US_FDS_FileManager::plot_scans(void) {
   ...
   for (int i = 0; i < scaninfo.size(); i++) {
      if (
         scaninfo[ i ].triple == cb_triple->currentText() && scaninfo[ i ].rpm == cb_rpms->currentText().toInt()
...
void US_FDS_FileManager::write(void) {
   ...
   target_dir = fd.getExistingDirectory(
      this,
      tr("Please select or create an output directory for the ordered fluorescence data (ANY EXISITNG DATA WILL BE "
         "DELETED!!):"),
   ...
   if (target_dir.right(1) != "/")
      target_dir += "/"; // Ensure trailing "/"
   QStringList triples;
   ...
   for (i = 0; i < scaninfo.size(); i++) {
      if (scaninfo[ i ].include) { // increment filename and write to new location
         j = 0;
         while (scaninfo[ i ].triple != tripleCounts[ j ].name)
            j++;
         tripleCounts[ j ].index++;
         if (tripleCounts[ j ].index < 10) {
            filename = target_dir + prefix + scaninfo.at(i).channel + "0000" + str1.setNum(tripleCounts[ j ].index)
                       + ".FI" + str2.setNum(scaninfo.at(i).cell);
         }
         else if (tripleCounts[ j ].index > 9 && tripleCounts[ j ].index < 100) {
            filename = target_dir + prefix + scaninfo.at(i).channel + "000" + str1.setNum(tripleCounts[ j ].index)
                       + ".FI" + str2.setNum(scaninfo.at(i).cell);
         }
         else if (tripleCounts[ j ].index > 99 && tripleCounts[ j ].index < 1000) {
            filename = target_dir + prefix + scaninfo.at(i).channel + "00" + str1.setNum(tripleCounts[ j ].index)
                       + ".FI" + str2.setNum(scaninfo.at(i).cell);
         }
         else if (tripleCounts[ j ].index > 999 && tripleCounts[ j ].index < 10000) {
            filename = target_dir + prefix + scaninfo.at(i).channel + "0" + str1.setNum(tripleCounts[ j ].index) + ".FI"
                       + str2.setNum(scaninfo.at(i).cell);
         }
         else if (tripleCounts[ j ].index > 9999 && tripleCounts[ j ].index < 100000) {
            filename = target_dir + prefix + scaninfo.at(i).channel + str1.setNum(tripleCounts[ j ].index) + ".FI"
                       + str2.setNum(scaninfo.at(i).cell);
         }
