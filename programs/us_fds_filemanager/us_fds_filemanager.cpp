...
//! \brief Main program for US_FDS_FileManager. This program is intended to
//! help the
...
US_FDS_FileManager::US_FDS_FileManager () : US_Widgets ()
{
  ...
  pb_save_first_and_last
      = us_pushbutton (tr ("Save first and last Scan"), false);
  ...
  plot = new US_Plot (data_plot, tr ("Fluorescence Data"),
...
US_FDS_FileManager::load (void)
{
  ...
  files = QDir (readDir).entryList (QDir::Files | QDir::NoDotAndDotDot,
                                    QDir::Name);
  parse_files ();
}
...
void
US_FDS_FileManager::parse_files (void)
{
...
// Select a new triple
void
US_FDS_FileManager::select_triple (int index)
{
  cb_rpms->disconnect ();
  cb_rpms->clear ();
...
void
US_FDS_FileManager::select_rpm (int index)
{
  cb_gains->disconnect ();
  cb_gains->clear ();
  current_rpm = index;
...
void
US_FDS_FileManager::select_gain (int index)
{
  ...
  connect (ct_from, SIGNAL (valueChanged (double)),
           SLOT (focus_from (double)));
  connect (ct_to, SIGNAL (valueChanged (double)), SLOT (focus_to (double)));
  le_info->setText ("Loaded " + str.setNum (scaninfo.size ())
                    + " scans containing " + str1.setNum (cb_triple->count ())
                    + " triples. The current triple ("
                    + cb_triple->currentText () + ") contains "
...
void
US_FDS_FileManager::plot_scans (void)
{
  QString str1;
...
void
US_FDS_FileManager::activate_undo (void)
{
...
void
US_FDS_FileManager::activate_undo (QString triple)
{
  pb_undo->setEnabled (true);
  undo_triple = triple;
  plot_scans ();
...
void
US_FDS_FileManager::delete_all (void)
{
  for (int i = 0; i < scanindex.size (); i++)
    {
...
void
US_FDS_FileManager::save_first (void)
{
...
// equilibrium)
void
US_FDS_FileManager::save_last (void)
{
...
// equilibrium
void
US_FDS_FileManager::save_first_and_last (void)
{
...
void
US_FDS_FileManager::delete_triple (void)
{
  for (int i = 0; i < scaninfo.size (); i++)
    {
...
void
US_FDS_FileManager::exclude_scans (void)
{
  for (int i = 0; i < scanindex.size (); i++)
    {
      if (from == 0 && to == 0)
...
void
US_FDS_FileManager::focus_from (double scan)
{
  from = (int)scan;
  to = (int)ct_to->value ();
...
void
US_FDS_FileManager::focus_to (double scan)
{
  to = (int)scan;
...
void
US_FDS_FileManager::focus (int from, int to)
{
  if (from == 0 && to == 0)
    {
      pb_exclude->setEnabled (false);
...
void
US_FDS_FileManager::update_prefix (double val)
{
  QString str;
  ...
  for (i = 0; i < scaninfo.size (); i++)
    {
      if (scaninfo[i].include)
        { // increment filename and write to new location
          ...
          else if (tripleCounts[j].index > 999
                   && tripleCounts[j].index < 10000)
            {
          ...
          if (!f.copy (source_dir + scaninfo.at (i).filename, filename))
            {
              str1 = tr ("The file:\n\n   ") + filename
                     + tr ("\n\nalready exists in:\n\n   ") + target_dir
                     + "\n\n"
                     + tr ("Please use a different directory or prefix, or "
                           "create a new directory and try again.");
              QMessageBox::warning (this, tr ("Attention:"), str1,
                                    QMessageBox::Ok);
              return;
            }
        }
