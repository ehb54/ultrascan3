...
US_QueryRmsd::US_QueryRmsd () : US_Widgets ()
{
  ...
  connect (le_threshold, SIGNAL (editingFinished ()), this,
           SLOT (new_threshold ()));
  connect (fematch, SIGNAL (astfem_cmp (int)), SLOT (update_progress (int)));
}

bool
US_QueryRmsd::check_connection ()
{
...
bool
US_QueryRmsd::get_metadata (DataBundle &data, QString &mesg)
{
  if (!check_connection ())
    {
...
bool
US_QueryRmsd::load_data (int index, QString &mesg)
{
  if (!check_connection ())
    return false;
  ...
  if (rawData.contains (rdataId))
    ...
    {
      dbCon->readBlobFromDB (temp_dir.filePath (rdata_file),
                             "download_aucData", rdataId);
      if (dbCon->lastErrno () != US_DB2::OK)
        {
  ...
  if (state != US_DataIO::OK)
    {
      mesg = tr ("Error: US_DataIO::loadData:%1")
                 .arg (US_DataIO::errorString (state));
      return false;
    }
...
bool
US_QueryRmsd::check_combo_content (QComboBox *combo, QString &text)
{
  int cc = combo->count ();
  if (cc == 0)
    return false;
...
void
US_QueryRmsd::update_progress (int val)
{
  progress->setValue (val);
}

void
US_QueryRmsd::closeEvent (QCloseEvent *event)
{
  fematch->close ();
  event->accept ();
}
