...
US_QueryRmsd::US_QueryRmsd() : US_Widgets() {
  ...
  tw_rmsd->setHorizontalHeaderLabels(
      QStringList{"Triple_Method_Analysis", "RMSD"});
  tw_rmsd->setHorizontalHeaderLabels(
      QStringList{"Edit", "Analysis", "Method", "Triple", "RMSD"});
  hheader = tw_rmsd->horizontalHeader();
  tw_rmsd->setStyleSheet("background-color: white");
  QHeaderView *header = tw_rmsd->horizontalHeader();
  //   header->setSectionResizeMode(header->logicalIndexAt(0),
  //   QHeaderView::Stretch);
  //   header->setSectionResizeMode(header->logicalIndexAt(2),
  //   QHeaderView::Stretch);
  //   header->setSectionResizeMode(header->logicalIndexAt(3),
  //   QHeaderView::ResizeToContents);
  //   header->setSectionResizeMode(header->logicalIndexAt(4),
  //   QHeaderView::Stretch);
...
void US_QueryRmsd::load_runid() {
  ...
  for (int i = 0; i < modelIDs_tmp.size(); i++) {
    ...
    if (state == US_DB2::OK) {
      ...
      if (!editList.contains(data.edit)) editList << data.edit;
      allData << data;
  ...
  cb_edit->disconnect();
  cb_edit->clear();
  if (editList.size() > 1) cb_edit->addItem("ALL");
  foreach (QString item, editList) cb_edit->addItem(item);
  cb_edit->setCurrentIndex(0);
  connect(cb_edit, SIGNAL(currentIndexChanged(int)), this,
          SLOT(set_analysis(int)));
...
bool US_QueryRmsd::load_data(int index, QString &mesg) {
  ...
  if (!temp_dir.isValid()) return false;
  int state;
  ...
  dbCon->readBlobFromDB(temp_dir.filePath(edit_file), "download_editData",
                        editId);
  if (dbCon->lastErrno() != US_DB2::OK) {
  ...
  if (rawData.contains(rdataId)) {
    ...
    dbCon->readBlobFromDB(temp_dir.filePath(rdata_file), "download_aucData",
                          rdataId);
    if (dbCon->lastErrno() != US_DB2::OK) {
  ...
  if (state != US_DataIO::OK) {
    mesg =
        tr("Error: US_DataIO::loadData:%1").arg(US_DataIO::errorString(state));
    return false;
  }
...
void US_QueryRmsd::set_analysis(int) {
  ...
  for (int i = 0; i < allData.size(); i++) {
    if (!check_combo_content(cb_edit, edit)) continue;
    QString analysis = allData.at(i).analysis;
    if (!analysisList.contains(analysis)) analysisList << analysis;
  }
  analysisList.sort();
  cb_analysis->disconnect();
  cb_analysis->clear();
  if (analysisList.size() > 1) cb_analysis->addItem("ALL");
  foreach (QString item, analysisList) cb_analysis->addItem(item);
  cb_analysis->setCurrentIndex(0);
  connect(cb_analysis, SIGNAL(currentIndexChanged(int)), this,
          SLOT(set_method(int)));
  set_method(0);
}
...
void US_QueryRmsd::set_method(int) {
  ...
  for (int i = 0; i < allData.size(); i++) {
    ...
    QString analysis = allData.at(i).analysis;
    if (!check_combo_content(cb_edit, edit)) continue;
    if (!check_combo_content(cb_analysis, analysis)) continue;
    QString method = allData.at(i).method;
    if (!methodList.contains(method)) methodList << method;
  }
  methodList.sort();
  cb_method->disconnect();
  cb_method->clear();
  if (methodList.size() > 1) cb_method->addItem("ALL");
  foreach (QString item, methodList) cb_method->addItem(item);
  cb_method->setCurrentIndex(0);
  connect(cb_method, SIGNAL(currentIndexChanged(int)), this,
          SLOT(set_triple(int)));
  set_triple(0);
}
...
void US_QueryRmsd::set_triple(int) {
  ...
  for (int i = 0; i < allData.size(); i++) {
    ...
    if (!check_combo_content(cb_edit, edit)) continue;
    if (!check_combo_content(cb_analysis, analysis)) continue;
    if (!check_combo_content(cb_method, method)) continue;

    QString cell = allData.at(i).cell;
    QString channel = allData.at(i).channel;
    QString lambda = allData.at(i).lamda;
    if (!cellList.contains(cell)) cellList << cell;
    if (!channelList.contains(channel)) channelList << channel;
    if (!lambdaList.contains(lambda)) lambdaList << lambda;
  }
  cellList.sort();
  channelList.sort();
  lambdaList.sort();
  ...
  cb_channel->clear();
  cb_lambda->clear();
  if (cellList.size() > 1) cb_cell->addItem("ALL");
  if (channelList.size() > 1) cb_channel->addItem("ALL");
  if (lambdaList.size() > 1) cb_lambda->addItem("ALL");
  foreach (QString item, cellList) cb_cell->addItem(item);
  cb_cell->setCurrentIndex(0);

  foreach (QString item, channelList) cb_channel->addItem(item);
  cb_channel->setCurrentIndex(0);

  foreach (QString item, lambdaList) cb_lambda->addItem(item);
  cb_lambda->setCurrentIndex(0);

  connect(cb_cell, SIGNAL(currentIndexChanged(int)), this,
          SLOT(fill_table(int)));
  connect(cb_channel, SIGNAL(currentIndexChanged(int)), this,
          SLOT(fill_table(int)));
  connect(cb_lambda, SIGNAL(currentIndexChanged(int)), this,
          SLOT(fill_table(int)));
  fill_table(0);
}
...
void US_QueryRmsd::fill_table(int) {
  ...
  for (int i = 0; i < allData.size(); i++) {
    ...
    if (!check_combo_content(cb_analysis, analysis)) continue;
    if (!check_combo_content(cb_method, method)) continue;
    if (!check_combo_content(cb_cell, cell)) continue;
    if (!check_combo_content(cb_channel, channel)) continue;
    if (!check_combo_content(cb_lambda, lambda)) continue;
    selIndex << i;
  ...
  tw_rmsd->verticalHeader()->setSectionResizeMode(
      QHeaderView::ResizeToContents);
  ...
  le_file->setText(fname.arg(cb_edit->currentText(), cb_analysis->currentText(),
                             ...
                             cb_channel->currentText(),
                             cb_lambda->currentText()));
}
...
void US_QueryRmsd::save_data() {
  ...
  if (le_file->text().isEmpty()) {
    QMessageBox::warning(this, tr("Warning!"),
                         tr("Give a name to the file, then try again!"));
    return;
  }
...
void US_QueryRmsd::update_progress(int val) { progress->setValue(val); }
