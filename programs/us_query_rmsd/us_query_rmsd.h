...
/**
 * @class US_QueryRmsd
 * @brief The US_QueryRmsd class provides functionality to query and analyze
 * RMSD values from the database.
 */
...
private:
  ...
  QMap<int, US_DataIO::EditedData>
      editData;                          //!< Map of DB edit ID to EditedData
 ...
 * @brief The DoubleTableWidgetItem class provides a table widget item for
 * double values.
 */
...
public:
  ...
  DoubleTableWidgetItem (double value)
      : QTableWidgetItem (QString::number (value, 'f', 8)), m_value (value)
  {
  }
   ...
   */
  bool
  operator< (const QTableWidgetItem &other) const override
  {
