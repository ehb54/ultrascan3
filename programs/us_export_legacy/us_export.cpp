...
void US_ExportLegacy::load(void) {
  ...
  if (rawDtype == "RI") {  // Possibly convert Pseudo Absorbance to Intensity
    ...
    if (nrip == 0) {  // No RI Profile:  export RA
      ...
      QString mtxt =
          tr("The input is <b>RI</b> data,<br>&nbsp;&nbsp;"
      ...
      QString mtxt =
          tr("For <b>RI</b> data, you may export values as<br>"
             "<b>Intensity</b> or "
      ...
      if (msgBox.clickedButton() ==
          pb_abs) {  // RA chosen:  no conversion necessary
...
void US_ExportLegacy::data_plot(void) {
  ...
  for (int ii = 0; ii < scanCount; ii++) {
    for (int jj = 0; jj < valueCount;
         jj++) {  // accumulate coordinates of curve within baseline-to-plateau
...
void US_ExportLegacy::exp_mosttypes(QStringList& files) {
  ...
  if (htype == "R") {  // For RA, determine if pseudo absorbance needing channel
                       // subdirectories
  ...
  for (int drow = 0; drow < ntriples;
       drow++) {             // Output a set of files for each input triple
    ...
    for (int ii = 0; ii < nscan; ii++) {  // Output a file for each scan
      ...
      oline = htype + QString()
                          .sprintf("%2i%5.1f%6i %07i%11.4E", hcell, htemp, hrpm,
                                   hsecs, homeg)
                          .replace("E+", "E");
      // Complete header line, using radius if Wavelength data
      oline = oline + (wldata ? QString().sprintf("%6.3f %i\n", hradi, hcoun)
      ...
      for (int jj = 0; jj < nvalu; jj++) {  // Output a line for each data point
        ...
        QString oline = QString()
                            .sprintf("%9.4f %12.5E %13.5E\n", radi, valu, stdd)
                            .replace("E+", "E+00")
                            .replace("E-", "E-00");
        if (jj < 3 || jj > (nvalu - 4))
          DbgLv(1) << "  LINE:" << QString(oline).replace("\n", "");
...
void US_ExportLegacy::exp_intensity(QStringList& files) {
  ...
  for (int drow = 0; drow < ntriples;
    ...
    if (twofer) {           // We have channel A, test if we have a B next
      ...
      if (erow < ntriples && triples[erow].contains("B") &&
          QString(triples[erow]).replace("B", "A") ==
              tripa) {  // We have a B that goes with this A:  set up to do
                        // 2-at-a-time
        rdat2 = &rawList[erow];
        drow = erow;
      }
...
void US_ExportLegacy::rDataStrings(US_DataIO::RawData* rdata, QString& Dtype,
                                   QString& Cell, QString& Chann,
                                   QString& Waveln) {
...
double US_ExportLegacy::time_correction() {
  ...
  for (int ii = 1; ii < rawList.size(); ii++) size += rawList[ii].scanCount();
...
int US_ExportLegacy::getRIProfile(QVector<double>& RIProfile) {
  ...
  if (isDB) {  // Data from DB:  get any RIProfile from experiment record
    ...
    db.query(query);
    if (db.lastErrno() == US_DB2::NOROWS) return 0;
    db.next();
    ...
    if (!fi.open(QIODevice::ReadOnly | QIODevice::Text)) return 0;

    ripxml = fi.readAll();
    fi.close();
  }

  if (ripxml.isEmpty()) return 0;
...
void US_ExportLegacy::convertToIntensity(QVector<double>& RIProfile) {
  ...
  for (int kk = 0; kk < ntriples;
       kk++) {  // Loop to convert each data set to Intensity
    rdata = &rawList[kk];
    scanCount = rdata->scanCount();
    valueCount = rdata->pointCount();
    ...
    for (int ii = 0; ii < scanCount;
         ii++) {  // Convert each scan using a RIProfile term
      US_DataIO::Scan* rscan = &rdata->scanData[ii];
      double rip = RIProfile[qMin(ii, lrip)];
      DbgLv(1) << "CnvPA:   ii rip" << ii << rip;
      DbgLv(1) << "CnvPA:    aval0" << rscan->rvalues[0] << "pow"
               << pow(10.0, rscan->rvalues[0]);
