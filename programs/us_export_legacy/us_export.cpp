...
US_ExportLegacy::US_ExportLegacy() : US_Widgets() {
   ...
   connect(pb_save, SIGNAL(clicked()), this, SLOT(export_data()));
   ...
   runInfoLayout->addWidget(te_desc, row, 0, 3, 4);
   row += 3;
   runInfoLayout->addWidget(lb_triples, row++, 0, 1, 4);
   runInfoLayout->addWidget(lw_triples, row++, 0, 8, 4);
   row += 8;
   runInfoLayout->addWidget(lb_stat, row++, 0, 1, 4);
   runInfoLayout->addWidget(te_stat, row, 0, 1, 4);
   row += 3;

   // Plots
   plotLayout2 = new US_Plot(data_plot2, tr("Velocity Data"), tr("Radius (cm)"), tr("Absorbance"));
...
void US_ExportLegacy::load(void) {
   ...
   US_LoadAUC *dialog = new US_LoadAUC(isLocal, rawList, triples, workingDir);
   ...
   if (ntriples > 1)
      te_stat->setText(tr("%1 input %2 triples").arg(ntriples).arg(rawDtype));
   else
   ...
   if (rawDtype == "RI") { // Possibly convert Pseudo Absorbance to Intensity
      ...
      if (nrip == 0) { // No RI Profile:  export RA
      ...
      else { // RI Profile exists:  ask user RA/RI preference
         ...
         if (msgBox.clickedButton() == pb_abs) { // RA chosen:  no conversion necessary
            DbgLv(1) << "RI Export: YES : Absorbance";
            rawDtype = "RA";
         }

         else { // RI chosen:  convert data to Intensity
            DbgLv(1) << "RI Export: NO  : Intensity";
            convertToIntensity(RIProfile);
         }
      }

      if (rawDtype == "RA") { // Export type changed to RA:  modify data headers
...
void US_ExportLegacy::details(void) {
   QString workDir = dkdb_cntrls->db() ? tr("(database)") : workingDir;
   US_RunDetails2 *dialog = new US_RunDetails2(rawList, runID, workDir, triples);
...
void US_ExportLegacy::data_plot(void) {
   ...
   data_plot2->setTitle(tr("Velocity Data for ") + runID);
   data_plot2->setAxisTitle(QwtPlot::yLeft, dataType + tr(" at ") + rawWaveln + tr(" nm"));
   ...
   for (int ii = 0; ii < scanCount; ii++) {
      for (int jj = 0; jj < valueCount; jj++) { // accumulate coordinates of curve within baseline-to-plateau
...
void US_ExportLegacy::export_data(void) {
   ...
   if (rawDtype == "RI" || rawDtype == "WI") { // Export intensity
      exp_intensity(files);
   }
   else if (rawDtype == "IP") { // Export interference
      exp_interference(files);
   }
   else { // Export most types of data
      exp_mosttypes(files);
   }
...
void US_ExportLegacy::exp_mosttypes(QStringList &files) {
   ...
   if (htype == "R") { // For RA, determine if pseudo absorbance needing channel subdirectories
   ...
   for (int drow = 0; drow < ntriples; drow++) { // Output a set of files for each input triple
      ...
      if (channdir) { // For pseudo absorbance, output to channel subdirectory
      ...
      for (int ii = 0; ii < nscan; ii++) { // Output a file for each scan
         ...
         oline = htype
                 + QString().sprintf("%2i%5.1f%6i %07i%11.4E", hcell, htemp, hrpm, hsecs, homeg).replace("E+", "E");
         // Complete header line, using radius if Wavelength data
         oline = oline
                 + (wldata ? QString().sprintf("%6.3f %i\n", hradi, hcoun)
         ...
         for (int jj = 0; jj < nvalu; jj++) { // Output a line for each data point
            ...
            QString oline
               = QString().sprintf("%9.4f %12.5E %13.5E\n", radi, valu, stdd).replace("E+", "E+00").replace("E-", "E-00");
            if (jj < 3 || jj > (nvalu - 4))
               DbgLv(1) << "  LINE:" << QString(oline).replace("\n", "");
...
void US_ExportLegacy::exp_intensity(QStringList &files) {
   ...
   for (int drow = 0; drow < ntriples; drow++) { // Output a set of files for each input triple
      ...
      bfirst = false;
      if (twofer) { // We have channel A, test if we have a B next
         int erow = drow + 1; // Next triple index
         QString tripa = triples[ drow ]; // First triple in pair
         if (
            erow < ntriples && triples[ erow ].contains("B")
            && QString(triples[ erow ]).replace("B", "A")
                  == tripa) { // We have a B that goes with this A:  set up to do 2-at-a-time
            rdat2 = &rawList[ erow ];
            drow = erow;
         }
...
QString US_ExportLegacy::data_details(void) {
   ...
   if (rawDtype == "RA")
      dataType = tr("Radial Absorbance");
   if (rawDtype == "RI")
      dataType = tr("Radial Intensity");
   if (rawDtype == "WA")
      dataType = tr("Wavelength Absorbance");
   if (rawDtype == "WI")
      dataType = tr("Wavelength Intensity");
   if (rawDtype == "IP")
      dataType = tr("Interference");
   if (rawDtype == "FI")
      dataType = tr("Fluourescence Intensity");
   dataType = dataType + "  (" + rawDtype + ")";
   QString expDir = US_Settings::importDir() + "/" + runID;

   QString s = "\n" + indent(4) + tr("<h3>Detailed Run Information:</h3>\n") + indent(4) + "<table>\n"
               + table_row(tr("Cell Description:"), rdata->description) + table_row(tr("Data Directory:"), workingDir)
               + table_row(tr("Export Directory:"), expDir) + table_row(tr("Data Type:"), dataType)
               + table_row(tr("Rotor Speed:"), QString::number(( int ) rdata->scanData[ 0 ].rpm) + " rpm");
...
QString US_ExportLegacy::html_header(QString title, QString head1, US_DataIO::RawData *rdata) {
   rDataStrings(rdata, rawDtype, rawCell, rawChann, rawWaveln);
...
void US_ExportLegacy::rDataStrings(
   US_DataIO::RawData *rdata, QString &Dtype, QString &Cell, QString &Chann, QString &Waveln) {
   Dtype = QString(QChar(rdata->type[ 0 ])) + QString(QChar(rdata->type[ 1 ]));
...
double US_ExportLegacy::time_correction() {
   ...
   for (int ii = 0; ii < rawList.size(); ii++) {
      ...
      for (int jj = 0; jj < d->scanCount(); jj++) {
         if (d->scanData[ jj ].omega2t > 9.99999e10)
            break;
...
int US_ExportLegacy::getRIProfile(QVector<double> &RIProfile) {
   ...
   if (isDB) { // Data from DB:  get any RIProfile from experiment record
      US_Passwd pw;
...
void US_ExportLegacy::convertToIntensity(QVector<double> &RIProfile) {
   ...
   for (int kk = 0; kk < ntriples; kk++) { // Loop to convert each data set to Intensity
      rdata = &rawList[ kk ];
      scanCount = rdata->scanCount();
      valueCount = rdata->pointCount();
      DbgLv(1) << "CnvPA:  kk scns vals" << kk << scanCount << valueCount;

      for (int ii = 0; ii < scanCount; ii++) { // Convert each scan using a RIProfile term
         US_DataIO::Scan *rscan = &rdata->scanData[ ii ];
         double rip = RIProfile[ qMin(ii, lrip) ];
         DbgLv(1) << "CnvPA:   ii rip" << ii << rip;
         DbgLv(1) << "CnvPA:    aval0" << rscan->rvalues[ 0 ] << "pow" << pow(10.0, rscan->rvalues[ 0 ]);
...
void US_ExportLegacy::parseRIProfile(QString &ripxml, QVector<double> &RIProfile) {
   RIProfile.clear();
   ...
   if (nprof > 0)
      DbgLv(1) << "ParsRIProf: RIP0" << RIProfile[ 0 ];
   if (nprof > 1)
      DbgLv(1) << "ParsRIProf: RIPn" << RIProfile[ nprof - 1 ];
}
