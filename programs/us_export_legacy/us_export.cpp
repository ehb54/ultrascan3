...
US_ExportLegacy::US_ExportLegacy () : US_Widgets ()
{
  ...
  // Plots
  plotLayout2 = new US_Plot (data_plot2, tr ("Velocity Data"),
...
US_ExportLegacy::load (void)
{
  ...
  if (ntriples > 1)
    te_stat->setText (
        tr ("%1 input %2 triples").arg (ntriples).arg (rawDtype));
  else
...
// Details
void
US_ExportLegacy::details (void)
{
  QString workDir = dkdb_cntrls->db () ? tr ("(database)") : workingDir;
  US_RunDetails2 *dialog
      = new US_RunDetails2 (rawList, runID, workDir, triples);
...
void
US_ExportLegacy::update (int drow)
{
  rdata = &rawList[drow];
  scanCount = rdata->scanCount ();
  double avgTemp = rdata->average_temperature ();
...
// Data plot
void
US_ExportLegacy::data_plot (void)
{
...
void
US_ExportLegacy::export_data (void)
{
  QString rawDtyp2;
...
void
US_ExportLegacy::exp_mosttypes (QStringList &files)
{
  qDebug () << "Abs export";
    ...
    {                         // Output a set of files for each input triple
      ...
      for (int ii = 0; ii < nscan; ii++)
        { // Output a file for each scan
          ...
          // ofpath = odirname + ofname;            // Full path file name for
          // scan
          ...
          // DbgLv(1) << "  LINE: nv ii sizes" << nvalu << ii <<
          // rdata->scanCount()
...
void
US_ExportLegacy::exp_intensity (QStringList &files)
{
...
void
US_ExportLegacy::exp_interference (QStringList &files)
{
...
void
US_ExportLegacy::write_report (QTextStream &ts)
{
...
QString
US_ExportLegacy::indent (int spaces) const
{
...
QString
US_ExportLegacy::data_details (void)
{
...
bool
US_ExportLegacy::mkdir (QString &baseDir, QString &subdir)
{
...
void
US_ExportLegacy::new_triple (int trow)
{
  update (trow);
...
void
US_ExportLegacy::update_disk_db (bool isDB)
{
...
QString
US_ExportLegacy::table_row (const QString &s1, const QString &s2) const
{
...
QString
US_ExportLegacy::html_header (QString title, QString head1,
...
void
US_ExportLegacy::rDataStrings (US_DataIO::RawData *rdata, QString &Dtype,
                               QString &Cell, QString &Chann, QString &Waveln)
{
...
double
US_ExportLegacy::time_correction ()
{
...
int
US_ExportLegacy::getRIProfile (QVector<double> &RIProfile)
{
  QString ripxml;
...
void
US_ExportLegacy::convertToIntensity (QVector<double> &RIProfile)
{
...
void
US_ExportLegacy::parseRIProfile (QString &ripxml, QVector<double> &RIProfile)
{
  RIProfile.clear ();
