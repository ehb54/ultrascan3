...
US_ModelMetrics::US_ModelMetrics() : US_Widgets() {
   ...
   connect(le_plotxmin, SIGNAL(textChanged(const QString &)), this, SLOT(set_plotxmin(const QString &)));
   connect(le_plotxmax, SIGNAL(textChanged(const QString &)), this, SLOT(set_plotxmax(const QString &)));
   ...
   connect(le_name, SIGNAL(textChanged(const QString &)), this, SLOT(update_name(const QString &)));
   ...
   QBoxLayout *plot = new US_Plot(data_plot, tr(""), "Parameter Value", "Relative Concentration");
...
void US_ModelMetrics::load_model(void) {
   ...
   if (model.analysis != US_Model::COFS) {
      for (int jj = 0; jj < model.components.size(); jj++) {
         ...
         DbgLv(2) << "Solute jj s w k c d" << jj << sol_sk.s << sol_sk.w << sol_sk.k << sol_sk.c << sol_sk.d << " vb"
...
*/
   ...
   */
   ...
   if (HPd != fixed)
      rb_d->setEnabled(true);
   if (HPk != fixed)
      rb_k->setEnabled(true);
   if (HPf != fixed)
      rb_f->setEnabled(true);
   if (HPm != fixed)
      rb_m->setEnabled(true);
   if (HPv != fixed)
      rb_v->setEnabled(true);
   if (HPr != fixed)
      rb_r->setEnabled(true);
...
void US_ModelMetrics::set_dval_labels(bool update) {
   ...
   lbl_span->setText(
      "(D" + str1.setNum(( float ) dval3, 'g', 4) + "-D" + str2.setNum(( float ) dval1, 'g', 4) + ")/D"
      + str3.setNum(( float ) dval2, 'g', 4) + ": ");
   lbl_integral->setText(
      "D" + str1.setNum(( float ) dval1, 'g', 4) + ", D" + str2.setNum(( float ) dval3, 'g', 4) + " Integral: ");
   if (update) {
      disconnect(ct_dval1);
      disconnect(ct_dval2);
      disconnect(ct_dval3);
...
void US_ModelMetrics::plot_data() {
   ...
   median = (midx == (points / 2)) ? hp_distro[ midx ].parm
   ...
   if (calc_val == HPs || calc_val == HPk) {
      ...
      le_integral->setText(str1.setNum(integral, 'f', 3) + " (" + str2.setNum(percentage, 'f', 3) + "\%)");
      ...
      le_integral->setText(str1.setNum(integral, 'e', 3) + " (" + str2.setNum(percentage, 'f', 3) + "\%)");
...
void US_ModelMetrics::update_sigma(void) {
   ...
   DbgLv(2) << "In update_sigma: xmin: " << xmin << "t_xmin" << t_xmin << "xmax:" << xmax << "t_xmax:" << t_xmax
            << "range:" << range;
   if (t_xmin < 0.0)
      t_xmin = 0.0;
   t_xmax += range / 5.0;
...
void US_ModelMetrics::addReportItem(void) {
   ...
   report += "      <tr><td colspan='4' align='center' bgcolor='#FFFFFF'><img src='" + report_entry.filename
             + "'></td></tr>\n";
   report += table_row(6, "Investigator:", report_entry.investigator, "Distribution Type:", report_entry.parameter);
   if (report_entry.iterations == "1") {
      report += table_row(6, "Experiment:", report_entry.runID, "Analysis Method:", report_entry.analysis);
   }
   else {
      report += table_row(
         6, "Experiment:", report_entry.runID,
         "Analysis Method:", report_entry.analysis + " (" + report_entry.iterations + " MC Iterations)");
   }
   report += table_row(6, "Dataset:", report_entry.triple, "Edit and Model name:", report_entry.edit);
   report += indent(6) + "<tr><td colspan='4' align='center' bgcolor='#FFFFFF'>" + "<b>Statistics:</b></td></tr>\n";
   report += indent(6) + "<tr><td colspan='4' align='center' bgcolor='#FFFFFF'>\n";
   report += indent(8) + "<table border='1' bgcolor=#CCCCCC cellpadding='5'>\n";

   report += table_row(
      10, "D" + report_entry.d[ 0 ], report_entry.x[ 0 ], "D" + report_entry.d[ 1 ], report_entry.x[ 1 ],
      "D" + report_entry.d[ 2 ], report_entry.x[ 2 ]);
   report += table_row(
      10, report_entry.span_label, report_entry.span, "Minimum:", report_entry.minimum,
      "Maximum:", report_entry.maximum);
   report += table_row(
      10, "D" + report_entry.d[ 0 ] + ", " + "D" + report_entry.d[ 2 ] + " Integral:", report_entry.integral,
      "Total Concentration:", report_entry.totalc, "<a href=\"" + report_entry.csv + "\">CSV File</a>", "");
   report += table_row(10, "Mean:", report_entry.mean, "Mode:", report_entry.mode, "Median:", report_entry.median);
   report += table_row(10, "Sigma:", report_entry.sigma, "Skew:", report_entry.skew, "Kurtosis:", report_entry.kurtosis);
...
QString US_ModelMetrics::table_row(
   const int spaces, const QString &s1, const QString &s2, const QString &s3, const QString &s4) const {
   return (
      indent(spaces) + "<tr><td bgcolor='#CCCCFC'><b>" + s1 + "</b></td><td>" + s2 + "</td>\n" + indent(spaces)
      + "<td bgcolor='#CCCCFC'><b>" + s3 + "</b></td><td>" + s4 + "</td></tr>\n");
}
QString US_ModelMetrics::table_row(
   const int spaces, const QString &s1, const QString &s2, const QString &s3, const QString &s4, const QString &s5,
   const QString &s6) const {
   return (
      indent(spaces) + "<tr><td bgcolor='#CCCCFC'><b>" + s1 + "</b></td><td>" + s2 + "</td>\n" + indent(spaces)
      + "<td bgcolor='#CCCCFC'><b>" + s3 + "</b></td><td>" + s4 + "</td>\n" + indent(spaces)
      + "<td bgcolor='#CCCCFC'><b>" + s5 + "</b></td><td>" + s6 + "</td></tr>\n");
}
