...
void
US_ModelMetrics::select_prefilter (void)
{
  QString pfmsg;
  int nruns = 0;
  pfilts.clear ();
...
void
US_ModelMetrics::load_model (void)
{
  US_Model model;
  QString mdesc;
  S_Solute sol_sk;
  ...
  if (model.analysis != US_Model::COFS)
    {
      ...
      /*
            DbgLv(2) << "sk_distro.size() after reduction: " <<
         sk_distro.size(); for ( int jj=0;jj<sk_distro.size();jj++ )
            {
...
void
US_ModelMetrics::sel_investigator (void)
{
...
// changes it
void
US_ModelMetrics::update_disk_db (bool isDB)
{
  if (isDB)
...
void
US_ModelMetrics::write_report (void)
{
  report += "</body>\n</html>";
  report_ts << report;
  report_file.close ();
...
void
US_ModelMetrics::sort_distro (QList<S_Solute> &listsols, bool reduce)
{
...
bool
US_ModelMetrics::equivalent (double a, double b, double eps)
{
  return (qAbs ((a - b) / a) <= eps);
}

void
US_ModelMetrics::select_hp (int button)
{
  xautoscale = true;
  calc_val = button;
  calc ();
...
void
US_ModelMetrics::set_dval_labels (bool update)
{
  QString str1, str2, str3;
...
void
US_ModelMetrics::update_name (const QString &text)
{
  report_entry.name = text;
}

void
US_ModelMetrics::update_sigma (void)
{
      ...
      for (i = 0; i < points; i++)
        {
          tmp_hplist[i].conc
              += hp_distro[j].conc * amp
                 * exp (-0.5
                        * pow (tmp_hplist[i].parm - hp_distro[j].parm, 2.0)
                        / sigsqr);
        }
    }
  hp_distro.clear ();
...
void
US_ModelMetrics::set_dval1 (double val)
{
  dval1 = val;
  plot_data ();
}
...
void
US_ModelMetrics::set_dval2 (double val)
{
  dval2 = val;
  plot_data ();
}
...
void
US_ModelMetrics::set_dval3 (double val)
{
  dval3 = val;
  plot_data ();
}
...
void
US_ModelMetrics::set_sigma (double val)
{
  sigma = val;
  if (sigma == 0.0)
    {
...
void
US_ModelMetrics::addReportItem (void)
{
  QString str;
  QString timestamp
  ...
  if (!pb_write->isEnabled ()) // open a new report file in ultrascan's tmp dir
    {
      report_file.setFileName (US_Settings::tmpDir ()
                               + "/ModelStatistics.html");
  ...
  report += "  <center>\n    <table border='1' bgcolor=#CCCCCC "
            "cellpadding='5'>\n";
  report += "      <tr><td colspan='4' bgcolor='#FFFFFF' "
            "align='center'><b>Information for Model "
            + str.setNum (model_count) + " (" + report_entry.name
            + "):</b></td></tr>\n";
  report += "      <tr><td colspan='4' align='center' bgcolor='#FFFFFF'><img "
            "src='"
  ...
  report += indent (6)
            + "<tr><td colspan='4' align='center' bgcolor='#FFFFFF'>\n";
  report
      += indent (8) + "<table border='1' bgcolor=#CCCCCC cellpadding='5'>\n";
...
QString
US_ModelMetrics::table_row (const int spaces, const QString &s1,
...
QString
US_ModelMetrics::indent (const int spaces) const
{
  return QString (" ").leftJustified (spaces, ' ');
}

void
US_ModelMetrics::set_plotxmin (const QString &val)
{
...
void
US_ModelMetrics::set_plotxmax (const QString &val)
{
