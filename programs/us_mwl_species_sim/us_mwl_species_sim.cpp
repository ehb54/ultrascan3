...
US_MwlSpeciesSim::US_MwlSpeciesSim() : US_Widgets() {
   ...
   QBoxLayout *plot = new US_Plot(
      data_plot1, tr("MWL Species Simulation"), tr("Sedimentation Coefficient x 1e-13"), tr("Amplitude in OD"));

   data_plot1->setTitle(tr("MWL Species Simulation")), data_plot1->setMinimumSize(600, 300);
   ...
   connect(pb_prefilt, SIGNAL(clicked()), this, SLOT(pre_filt()));
   connect(pb_semodels, SIGNAL(clicked()), this, SLOT(select_models()));
   connect(pb_defbuff, SIGNAL(clicked()), this, SLOT(define_buffer()));
   connect(pb_simparms, SIGNAL(clicked()), this, SLOT(sim_params()));
   connect(pb_selrotor, SIGNAL(clicked()), this, SLOT(select_rotor()));
   connect(pb_strtsims, SIGNAL(clicked()), this, SLOT(start_sims()));
   connect(pb_stopsims, SIGNAL(clicked()), this, SLOT(stop_sims()));
   connect(pb_savesims, SIGNAL(clicked()), this, SLOT(save_sims()));
   connect(pb_prev, SIGNAL(clicked()), this, SLOT(prev_plot()));
   connect(pb_next, SIGNAL(clicked()), this, SLOT(next_plot()));
...
void US_MwlSpeciesSim::plot_data1(void) {
   ...
   QString wavl = QString().sprintf("%d", ( int ) rdata->scanData[ 0 ].wavelength);
   ...
   QString runid = "ISSF-" + mdesc.section(".", 0, -4) + "-" + chani;
...
void US_MwlSpeciesSim::pre_filt(void) {
   ...
   if (nruns == 1)
      ...
      pfmsg = tr("RunID prefilter - %1 runs:\n  ").arg(nruns) + QString(pfilts[ 0 ]).left(20) + "*, ...";
...
void US_MwlSpeciesSim::select_models(void) {
   ...
   DbgLv(1) << "  smdls: call ML dbload" << dbload << "mfilt" << mfilt << "pfilts" << pfilts;
   ...
   for (int jm = 0; jm < nmodels; jm++) {
      ...
      if (jm < 5 || (jm + 5) > nmodels) {
         DbgLv(1) << "  smdls: jm" << jm << "model.desc" << mdesc << "mdescs[jm]" << mdescs[ jm ];
      }
      //*DEBUG
   ...
   DbgLv(1) << "  smdls: nmodels" << nmodels << "nruns" << nruns << "nchans" << nchans << "nwavls" << nwavls;
   ...
   QString triple = chans[ 0 ].left(1) + "." + chans[ 0 ].mid(1, 1) + "." + wavelns[ 0 ] + "-" + wavelns[ nwavls - 1 ];
...
void US_MwlSpeciesSim::set_parameters(void) {
   ...
   simparams.meniscus = qRound(simparams.meniscus / rad_precis) * rad_precis;
   simparams.bottom_position = qRound(simparams.bottom_position / rad_precis) * rad_precis;
   ...
   while (aspeed < rspeed) { // Walk time and omega2t through acceleration zone
      aspeed += rpm_inc;
      tim_val += 1.0;
      w2t_val += sq(aspeed) * pi_fac;
   }

   while (tim_val < delay) { // Walk time and omega2t up to the first scan
      tim_val += 1.0;
      w2t_val += w2t_fac;
   }
   ...
   while (tim_val < duration) { // Walk time and omega2t up to the last scan
      tim_val += tim_inc;
      w2t_val += w2t_inc;
   }
...
void US_MwlSpeciesSim::select_rotor(void) {
   ...
   int dbdisk = dbload ? US_Disk_DB_Controls::DB : US_Disk_DB_Controls::Disk;

   US_RotorGui *rotorInfo = new US_RotorGui(true, dbdisk, rotor, calibration);

   connect(
      rotorInfo, SIGNAL(RotorCalibrationSelected(US_Rotor::Rotor &, US_Rotor::RotorCalibration &)), this,
      SLOT(assign_rotor(US_Rotor::Rotor &, US_Rotor::RotorCalibration &)));

   rotorInfo->exec();
}

// Set stretch coefficients based on a selected rotor
void US_MwlSpeciesSim::assign_rotor(US_Rotor::Rotor &arotor, US_Rotor::RotorCalibration &calibration) {
   rotor = arotor;
   rotor_calib = calibration;
   simparams.rotorcoeffs[ 0 ] = rotor_calib.coeff1;
   simparams.rotorcoeffs[ 1 ] = rotor_calib.coeff2;
   DbgLv(1) << "assign_rotor: rotor" << rotor.name << "coeffs" << simparams.rotorcoeffs[ 0 ]
...
void US_MwlSpeciesSim::start_sims(void) {
   ...
   for (int jm = 0; jm < nmodels; jm++) {
      ...
      smsg = tr("Of %1 models, building for: %2 (%3) ...").arg(nmodels).arg(jm + 1).arg(triple);
   ...
   if (kmodels == nmodels)
      smsg = tr("Raw Data have been built for all %1 models.").arg(nmodels);
   else
      smsg = tr("Raw Data builds stopped after %1 models.").arg(kmodels);
...
void US_MwlSpeciesSim::init_rawdata(void) {
   ...
   double stretch = speed0 * simparams.rotorcoeffs[ 0 ] + sq(speed0) * simparams.rotorcoeffs[ 1 ];
   ...
   int npoints = qRound((curr_bottom - curr_meniscus) / simparams.radial_resolution) + 1;
   ...
   for (int ii = 0; ii < lptx; ii++) { // Calculate the radial grid points
   ...
   double timeinc = (simparams.speed_step[ 0 ].time_last - simparams.speed_step[ 0 ].time_first)
                    / ( double ) (nscans - 1);
...
void US_MwlSpeciesSim::build_rawdata(void) {
   ...
   int hs = nscan / 2;
   int hp = npoint / 2;
   DbgLv(1) << "bldraw:   hs,hp" << hs << hp << "data0m" << rdata.value(0, hp) << "datamm" << rdata.value(hs, hp);
   ...
   for (int jr = npoint - 1; jr >= 0; jr--) { // Set shifted values, starting at data end
      int kr = jr - npad; // Old value index

      if (kr > 0) { // Move old values down by pad amount
      ...
      else if (kr == 0) { // Set the meniscus spike value for each scan
      ...
      else { // Set values to zero that are below the meniscus
         rdata.xvalues[ jr ] = radval;
         radval -= radinc;
         for (int js = 0; js < nscan; js++) {
...
int US_MwlSpeciesSim::writeTimeState(
   ...
   for (int ii = 0; ii < nscans; ii++) { // Accumulate the times at scans
