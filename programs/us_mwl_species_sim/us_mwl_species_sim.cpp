...
US_MwlSpeciesSim::US_MwlSpeciesSim() : US_Widgets() {
  ...
  QBoxLayout* plot = new US_Plot(data_plot1, tr("MWL Species Simulation"),
...
void US_MwlSpeciesSim::plot_data1(void) {
  ...
  if (tripx < 0 || tripx >= ndatas || !have_p1[tripx]) return;
...
void US_MwlSpeciesSim::select_models(void) {
  ...
  for (int jm = 0; jm < nmodels; jm++) {
    ...
    if (!wavelns.contains(waveln)) wavelns << waveln;
    //*DEBUG
    if (jm < 5 || (jm + 5) > nmodels) {
      DbgLv(1) << "  smdls: jm" << jm << "model.desc" << mdesc << "mdescs[jm]"
               << mdescs[jm];
    }
    //*DEBUG
  ...
  DbgLv(1) << "  smdls: nmodels" << nmodels << "nruns" << nruns << "nchans"
           << nchans << "nwavls" << nwavls;
  ...
  QString triple = chans[0].left(1) + "." + chans[0].mid(1, 1) + "." +
...
void US_MwlSpeciesSim::set_parameters(void) {
  ...
  simparams.bottom_position =
      qRound(simparams.bottom_position / rad_precis) * rad_precis;
...
void US_MwlSpeciesSim::assign_rotor(US_Rotor::Rotor& arotor,
  ...
  DbgLv(1) << "assign_rotor: rotor" << rotor.name << "coeffs"
...
void US_MwlSpeciesSim::init_rawdata(void) {
  ...
  double stretch =
      speed0 * simparams.rotorcoeffs[0] + sq(speed0) * simparams.rotorcoeffs[1];
  ...
  int npoints =
      qRound((curr_bottom - curr_meniscus) / simparams.radial_resolution) + 1;
  ...
  double timeinc =
      (simparams.speed_step[0].time_last - simparams.speed_step[0].time_first) /
      (double)(nscans - 1);
  double w2tval = simparams.speed_step[0].w2t_first;
  double w2tinc =
      timeinc * pow(simparams.speed_step[0].rotorspeed * M_PI / 30.0, 2.0);
  DbgLv(1) << "rdata0 tf tl" << simparams.speed_step[0].time_first
           << simparams.speed_step[0].time_last << "w2f w2l"
           << simparams.speed_step[0].w2t_first
           << simparams.speed_step[0].w2t_last << "timeinc w2tinc" << timeinc
           << w2tinc;
  ...
  for (int js = 0; js < nscans; js++) {
    ...
    w2tval += w2tinc;
    if (js == 0 || js == (nscans - 1))
      DbgLv(1) << "rdata0 sc seconds" << scan->seconds << "omg2t"
               << scan->omega2t << "js" << js;
  }
}
...
void US_MwlSpeciesSim::build_rawdata(void) {
  ...
  for (int js = 0; js < nscan; js++) rdata.scanData[js].wavelength = mwavelen;
  ...
  for (int jr = npoint - 1; jr >= 0;
       jr--) {           // Set shifted values, starting at data end
...
int US_MwlSpeciesSim::writeTimeState(const QString& tmst_fpath,
  ...
  if (spsim == false) {
    ...
    // DbgLv(1)<< " rate is given by user : t_acc from timestate" << t_acc <<
    // rate;
  }
