...
US_RasmolControl::US_RasmolControl() : US_Widgets() {
   ...
   connect(cb_commcmd, SIGNAL(activated(const QString &)), this, SLOT(choose_command(const QString &)));
   connect(pb_sendcmd, SIGNAL(clicked()), this, SLOT(send_command()));
   connect(pb_listints, SIGNAL(clicked()), this, SLOT(list_interps()));
...
void US_RasmolControl::send_command(void) {
   ...
   te_status->setPlainText(
      errcd == 0 ? tr("Successful Send to \"%1\"\n  of command \"%2\".").arg(iname).arg(scmnd)
...
void US_RasmolControl::list_interps(void) {
   ...
   if (nids != nnames) {
      qDebug() << "*ERROR* Number of IDs does not match number of Names" << nids << nnames;
      return;
   }
   ...
   if (nzomb > 0) { // If zombies exist, list them in the status box
      namtext += tr("\n\nZombie Interpreters:");
