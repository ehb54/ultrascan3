...
US_AnalysisProfileGui::US_AnalysisProfileGui() : US_Widgets() {
  ...
  connect(pb_apply, SIGNAL(clicked()), this, SLOT(apply_profile()));
  ...
  connect(apanGeneral, SIGNAL(set_tabs_buttons_inactive(void)), this,
          SLOT(disable_tabs_buttons(void)));
  connect(apanGeneral, SIGNAL(set_tabs_buttons_active_readonly(void)), this,
          SLOT(enable_tabs_buttons_readonly(void)));
  connect(apanGeneral, SIGNAL(set_tabs_buttons_active(void)), this,
...
void US_AnalysisProfileGui::auto_mode_passed(void) { automode = true; }

// Set auto protocol and aprofile names
void US_AnalysisProfileGui::auto_name_passed(QString& p_protname,
                                             QString& p_aproname) {
...
void US_AnalysisProfileGui::inherit_protocol(US_RunProtocol* iProto) {
  ...
  qDebug() << "In inherit: ScanCounts : " << iProto->scanCount
           << iProto->scanCount_int << iProto->rpSpeed.ssteps[0].scancount
  ...
  if (iProto->rpOptic.chopts.count() > 0)
    DbgLv(1) << "APG: ipro: 0)ch s1 s2 s3" << iProto->rpOptic.chopts[0].channel
             << iProto->rpOptic.chopts[0].scan1
             << iProto->rpOptic.chopts[0].scan2
             << iProto->rpOptic.chopts[0].scan3;
  if (iProto->rpOptic.chopts.count() > 1)
    DbgLv(1) << "APG: ipro: 1)ch s1 s2 s3" << iProto->rpOptic.chopts[1].channel
             << iProto->rpOptic.chopts[1].scan1
             << iProto->rpOptic.chopts[1].scan2
             << iProto->rpOptic.chopts[1].scan3;

  for (int ii = 0; ii < nchs; ii++) {  // Examine protocol's channel solutions
    QString chname = iProto->rpSolut.chsols[ii].channel;
    QString sodesc = iProto->rpSolut.chsols[ii].solution;

    chname = QString(chname).section(":", 0, 0).section(",", 0, 0).replace(
        " / ", "");
    sl_chns << chname;
    sl_sols << sodesc;
    DbgLv(1) << "APG: ipro:  s:ii" << ii << "chname" << chname << "sodesc"
             << sodesc;
  }

  for (int ii = 0; ii < ncho; ii++) {  // Examine protocol's channel optics
    ...
    if (scan1.isEmpty() && scan2.isEmpty() && scan3.isEmpty()) continue;

    QStringList ods;  // Optics descriptions, this channel
    chname = QString(chname).section(":", 0, 0).section(",", 0, 0).replace(
        " / ", "");
    if (!scan1.isEmpty()) ods << scan1;  // First optics
    if (!scan2.isEmpty()) ods << scan2;  // Additional optics
    if (!scan3.isEmpty()) ods << scan3;  // Additional optics
    ...
    for (int ch_r = 0; ch_r < iProto->rpRange.chrngs.size(); ++ch_r) {
      QString chdesc_from_ranges =
          (iProto->rpRange.chrngs[ch_r].channel).split(",")[0];
      chdesc_from_ranges.replace(" / ", "");

      qDebug() << " chdesc_from_ranges, chname_rpOptics  -- "
               << chdesc_from_ranges << chname;
    ...
    for (int jj = 0; jj < ods.count();
         jj++) {  // Create a channel entry for each optics type of this channel
      ...
      if (nchn < kchn) {  // Replace channel and channel description
        ...
        qDebug() << "Old wvl -- "
                 << currProf.ch_wvls[currProf.chndescs_alt[nchn]];
        ...
        for (ri = currProf.ch_reports.begin(); ri != currProf.ch_reports.end();
             ++ri) {
          ...
          for (tri = triple_reports.begin(); tri != triple_reports.end();
               ++tri) {
            old_wvl = tri.key();

            for (int ii = 0;
                 ii <
                 currProf.ch_reports[chan_desc][old_wvl].reportItems.size();
                 ++ii) {
              US_ReportGMP::ReportItem initItem =
                  currProf.ch_reports[chan_desc][old_wvl].reportItems[ii];
              qDebug() << "wvl: type, method, lo, hi -- " << old_wvl << ": "
                       << initItem.type << initItem.method << initItem.range_low
                       << initItem.range_high;
            }
          }
        }
        ...
        if (opdesc.contains("vis.")) currProf.ch_wvls[chentr_wvls] = wvls;
        ...
        qDebug() << "In inherit_prot: [new] wvl, wvls.size() for channel -- "
                 ...
                 << currProf.ch_wvls[chentr_wvls].size() << " for "
                 << chentr_wvls;

        // //ALEXEY: insert ch_reports here
        // //currProf.ch_reports[ chentr_wvls ] = reportGMP;
        // currProf.ch_reports[ chentr_wvls ] = currProf.ch_reports[
        // currProf.chndescs_alt[ nchn ] ]; currProf.ch_reports[ chentr_wvls
        // ].channel_name = chentr_wvls;

        // ALEXEY_NEW_REPORT: move these 2 lines below setting currProf.ch_wvls[
        // chentr_wvls ] Next, iterate over wvls & fill QMap< QString, QMap <
        // QString( wvls[i]), US_ReportGMP > > currProf.ch_reports[ chentr_wvls
        // ][wvl]
        ...
        for (int i = 0; i < ch_wavelengths.size(); ++i) {
          ...
          currProf.ch_reports[chentr_wvls][c_wvl] =
              currProf.ch_reports[currProf.chndescs_alt[nchn]][c_wvl];

          // currProf.ch_reports[ chentr_wvls ][ c_wvl ] = currProf.ch_reports[
          // old_desc ][ old_wvl ];
          ...
          if (chentr_wvls.contains("Interf.")) {
            if (currProf.ch_reports[chentr_wvls][c_wvl].DBread ||
                currProf.ch_reports[chentr_wvls][c_wvl].interf_report_changed) {
          ...
          double tot_time_exp = iProto->rpSpeed.ssteps[0].duration +
                                iProto->rpSpeed.ssteps[0].delay_stage;

          if (currProf.ch_reports[chentr_wvls][c_wvl].DBread ||
              currProf.ch_reports[chentr_wvls][c_wvl].exp_time_changed) {
          } else
            currProf.ch_reports[chentr_wvls][c_wvl].experiment_duration =
                tot_time_exp;  // <== 1st speed step!!!
        ...
        DbgLv(1) << "APG: ipro:     chx nchn dae" << chx << nchn << "dae size"
                 << currProf.data_ends.count() << "chentr" << chentr
                 << "currProf.analysis_run[ chx ]" << currProf.analysis_run[chx]
                 << "currProf.scan_excl_begin[ chx ]"
                 << currProf.scan_excl_begin[chx]
                 << "currProf.scan_excl_end[ chx ]"
                 << currProf.scan_excl_end[chx];

      } else {  // Append channel and channel description
        ...
        qDebug() << "In inherit_prot: wvls.size() for channel -- "
                 << currProf.ch_wvls[chentr_wvls].size() << " for "
                 << chentr_wvls;
        ...
        // //currProf.ch_reports[ chentr_wvls ] = reportGMP;
        // currProf.ch_reports[ chentr_wvls ] = currProf.ch_reports[
        // currProf.chndescs_alt.last() ]; currProf.ch_reports[ chentr_wvls
        // ].channel_name = chentr_wvls;
        ...
        for (int i = 0; i < ch_wavelengths.size(); ++i) {
          ...
          currProf.ch_reports[chentr_wvls][c_wvl] =
              currProf.ch_reports[currProf.chndescs_alt[nchn]][c_wvl];
          // currProf.ch_reports[ chentr_wvls ][ c_wvl ] = currProf.ch_reports[
          // currProf.chndescs_alt[ nchn - 1 ] ][ old_wvl ];
          currProf.ch_reports[chentr_wvls][c_wvl].channel_name = chentr_wvls;
        ...
        DbgLv(1) << "APG: ipro:     lch" << lch << "lv_tol da_end"
                 ...
                 << "currProf.scan_excl_begin[ lch ]"
                 << currProf.scan_excl_begin[lch]
                 << "currProf.scan_excl_end[ lch ]"
                 << currProf.scan_excl_end[lch];
      }
      nchn++;
    }
  ...
  // ALEXEY:Debug
  qDebug() << "In inherit_prot: size of currProf.ch_wvls QMap -- "
           << currProf.ch_wvls.count();
  //////////////

  kchn = currProf.pchans.count();
  qDebug() << "In inherit 1: pchans -- " << currProf.pchans;

  if (kchn > nchn) {  // Drop any Profile channel values left over from previous
  ...
  DbgLv(1) << "APG: ipro: name" << iProto->protoname << "cPname"
           << currProf.aprofname << "xml len" << ap_xml.length() << "GUID"
           << currProf.aprofGUID << iProto->rpAprof.aprofGUID;

  QString aprsrc("currProf");
  bool need_rec = ap_xml.isEmpty();
  need_rec =
      (currProf.aprofname == iProto->rpAprof.aprofname) ? need_rec : true;
  currProf.aprofGUID = iProto->rpAprof.aprofGUID = iProto->protoGUID;
  currProf.aprofname = iProto->rpAprof.aprofname = iProto->protoname;
  DbgLv(1) << "APG: ipro: name GUID" << currProf.aprofname
           << currProf.aprofGUID;

  qDebug() << "APG: ipro: currProf.aprofname, iProto->rpAprof.aprofname, "
              "need_rec -- "
           << currProf.aprofname << iProto->rpAprof.aprofname << need_rec;
  ...
  if (need_rec) {
    ...
    if (load_db) {  // Get AProfile XML from the database
                    // load_db=false;
      ...
      DbgLv(1) << "APG: ipro:  db stat nrow" << status << numrow << "lerr"
               << dbP->lastError();
    ...
    DbgLv(1) << "APG: ipro:  load_db" << load_db;
    if (!load_db) {  // Get AProfile XML from local disk
      bool new_file = false;
      QString dapath = US_Settings::dataDir() + "/projects";
      QString xmlipath =
          US_DataFiles::get_filename(dapath, currProf.aprofGUID, "A",
                                     "analysis_profile", "guid", new_file);

      DbgLv(1) << "APG: ipro:   new_file" << new_file << "xmlipath" << xmlipath
               << "guid" << currProf.aprofGUID;
      if (new_file) {  // No local file, so clear XML
        ap_xml.clear();
        currProf.aprofID = 0;
      } else {  // Read XML from local file
        ...
        if (xifile.open(QIODevice::ReadOnly | QIODevice::Text)) {
          ...
          currProf.aprofID = -(QString(xmlipath)
                                   .section("/", -1, -1)
                                   .section(".", -2, -2)
                                   .mid(1)
                                   .toInt());
    ...
    if (!reportMask_DB.isEmpty()) currProf.report_mask = reportMask_DB;
    ...
    if (!ap_xml.isEmpty()) {
      ...
      DbgLv(1) << "APG: ipro:   2kparm pkparm" << currProf.ap2DSA.parms.count()
               << currProf.apPCSA.parms.count() << "kpchan"
               << currProf.pchans.count();
      //         initPanels();

      // Check here if ch_report_ids[ channel_alt_desc ] and/or ch_report_guids[
      // channel_alt_desc ] are not empty Also check if ch_report_ids.values() -
      // the IDs - are not 0!!!
      ...
      //   {
      //     if( reportIDs_list[i] <= 0 ) //reportid is -1 or 0, so no report
      ...
      if (currProf.ch_report_ids.size()) {
        // fill in currProf->ch_reports[ chdesc_alt ] by reading ReportIDs from
        // ch_report_ids[ chdesc_alt ]

        // Also, clean  currProf->ch_reports first  (since it will be re-created
        // form DB here )
        currProf.ch_reports.clear();  // <-- Will this cause crash - as this
                                      // omits "B:Interf." cannels ??

        // iterate over QMap ch_report_ids[ chdesc_alt ]
        QMap<QString, QList<int>>::iterator ri;
        for (ri = currProf.ch_report_ids.begin();
             ri != currProf.ch_report_ids.end(); ++ri) {
          QString channel_alt_desc = ri.key();
          // int     reportID         = ri.value();

          QList<int> reportIDs = ri.value();

          // ALEXEY_NEW_REPORT: here ri.value() will be QList/QVector of
          // reportIDs for a given channel next will be cycle over this QList...

          QString wvl_read_abde;
          for (int rid = 0; rid < reportIDs.size(); ++rid) {
            int reportID = reportIDs[rid];
            qDebug() << "Reading reportItems form DB: channel_alt_desc, "
                        "reportID  -- "
                     << channel_alt_desc << ", " << reportID;

            // retrieve report from DB
            US_ReportGMP* reportFromDB = new US_ReportGMP();
            get_report_by_ID(reportFromDB, reportID);

            // ALEXEY_NEW_REPORT: here the QMap< QString, QMap < QString(wvl),
            // US_ReportGMP > > currProf.ch_reports will be filled Something
            // like currProf.ch_reports[ channel_alt_desc ][ wvl ] -- where wvl
            // == *reportFromDB->wavelength;
            //  OR  wvl is returned from get_report_by_ID( reportFromDB,
            //  reportID );

            QString wvl_read = QString::number(reportFromDB->wavelength);

            if (reportIDs.size() == 1) wvl_read_abde = wvl_read;

            // assign retieved report to currProf.ch_reports[ channel_alt_desc
            // ];
            currProf.ch_reports[channel_alt_desc][wvl_read] = *reportFromDB;

            qDebug() << "Filling currProf.ch_reports from DB: "
                        "channel_alt_desc, wvl_read -- "
                     << channel_alt_desc << ", " << wvl_read;
          }

          // In case of ABDE: the above loop would contain only 1 report (in DB:
          // 1 report per channel [1st wvl]) For internal consistency, we need
          // to populate (COPY) reports for the rest of wvls in the channel
          // (will not be used)
          QList<double> ch_wavelengths = currProf.ch_wvls[channel_alt_desc];
          if (abde_mode_aprofile && reportIDs.size() == 1 &&
              ch_wavelengths.size() > 1 && !wvl_read_abde.isEmpty()) {
            qDebug() << "AProfile[ABDE]: reading from DB: replicating reports "
                        "for MWL based on the 1st channel's wvl -- ";
            for (int i = 0; i < ch_wavelengths.size(); ++i) {
              QString c_wvl = QString::number(ch_wavelengths[i]);
              ...
              qDebug() << "Filling wvl = " << c_wvl
                       << ", with ref_wvl = " << wvl_read_abde;
              currProf.ch_reports[channel_alt_desc][c_wvl] =
                  currProf.ch_reports[channel_alt_desc][wvl_read_abde];
            }
          }
  ...
  // If currProf purely from upstream profile, insure proper channel
  // descriptions
  if (aprsrc == "currProf") {
    ...
    if (nchan_g == nchan_2 &&
        nchan_g == nchan_p) {  // Channels same size, so check contents
      for (int ii = 0; ii < nchan_g; ii++) {
        if (sl_chnsel_g[ii] != sl_chnsel_2[ii] ||
            sl_chnsel_g[ii] != sl_chnsel_p[ii])
          need_reset = true;
      }
    } else {  // Channels size difference, so mark need to reset
      need_reset = true;
      //   and effectively resize the string lists
      if (nchan_2 < nchan_g) {
        QString chann_2 = sl_chnsel_2[nchan_2 - 1];
        for (int ii = nchan_2; ii < nchan_g; ii++) sl_chnsel_2 << chann_2;
      } else if (nchan_2 > nchan_g) {
        for (int ii = nchan_g; ii < nchan_2; ii++) sl_chnsel_2.removeLast();
      }
      if (nchan_p < nchan_g) {
        QString chann_p = sl_chnsel_p[nchan_p - 1];
        for (int ii = nchan_p; ii < nchan_g; ii++) sl_chnsel_p << chann_p;
      } else if (nchan_p > nchan_g) {
        for (int ii = nchan_g; ii < nchan_p; ii++) sl_chnsel_p.removeLast();
      }
    }

    if (need_reset) {  // Reset channel descriptions for 2DSA,PCSA
      for (int ii = 0; ii < nchan_g; ii++) {
    ...
    need_reset = false;
    if (nchan_p2 == nchan_2 &&
        nchan_pp == nchan_p) {  // Channels same size, so check contents
      for (int ii = 0; ii < nchan_g; ii++) {
    ...
    if (need_reset) {  // Reset channel descriptions for 2DSA,PCSA parms
  ...
  // //ALEXEY: make copies of original currProf.ap2DSA && currProf.apPCSA
  // objects for further editing (analysis -- o run or not to run) qDebug() <<
  // "Inherit: BEFORE copying.."; currProf_copy.ap2DSA = currProf.ap2DSA;
  // currProf_copy.apPCSA = currProf.apPCSA;

  // qDebug() << "Inherit: AFTER copying..";

  qDebug() << "Size, Keys of currProf.ch_reports -- "
           << currProf.ch_reports.size() << ", " << currProf.ch_reports.keys();
}

// retrieve Report by ID from DB into ReportGMP structure
void US_AnalysisProfileGui::get_report_by_ID(US_ReportGMP* reportFromDB,
                                             int reportID) {
  US_DB2* db = NULL;
  US_Passwd pw;
  ...
  if (db->lastErrno() == US_DB2::OK) {
    while (db->next()) {
      ...
      reportFromDB->experiment_duration_tol =
          db->value(7).toString().toDouble();

      reportMaskJson = db->value(8).toString();
    }

    qDebug() << "report's channelname, wvl -- " << reportFromDB->channel_name
             << reportFromDB->wavelength;
    ...
    if (rootObj.contains("tot_conc") && rootObj.contains("rmsd_lim") &&
        rootObj.contains("intensity") && rootObj.contains("exp_dur") &&
        rootObj.contains("integration") && rootObj.contains("plots")) {
      rootObj["tot_conc"].toString().toInt()
          ? reportFromDB->tot_conc_mask = true
          : reportFromDB->tot_conc_mask = false;
      rootObj["rmsd_lim"].toString().toInt()
          ? reportFromDB->rmsd_limit_mask = true
          : reportFromDB->rmsd_limit_mask = false;
      rootObj["intensity"].toString().toInt()
          ? reportFromDB->av_intensity_mask = true
          : reportFromDB->av_intensity_mask = false;
      rootObj["exp_dur"].toString().toInt()
          ? reportFromDB->experiment_duration_mask = true
          : reportFromDB->experiment_duration_mask = false;
      rootObj["integration"].toString().toInt()
          ? reportFromDB->integration_results_mask = true
          : reportFromDB->integration_results_mask = false;
      rootObj["plots"].toString().toInt() ? reportFromDB->plots_mask = true
                                          : reportFromDB->plots_mask = false;
    } else {
      qDebug() << "No json mask set for report !\n";
    }
    if (rootObj.contains("pseudo3d_2dsait_s_ff0") &&
      ...
      rootObj["pseudo3d_2dsait_s_ff0"].toString().toInt()
          ? reportFromDB->pseudo3d_2dsait_s_ff0 = true
          : reportFromDB->pseudo3d_2dsait_s_ff0 = false;
      rootObj["pseudo3d_2dsait_s_d"].toString().toInt()
          ? reportFromDB->pseudo3d_2dsait_s_d = true
          : reportFromDB->pseudo3d_2dsait_s_d = false;
      rootObj["pseudo3d_2dsait_mw_ff0"].toString().toInt()
          ? reportFromDB->pseudo3d_2dsait_mw_ff0 = true
          : reportFromDB->pseudo3d_2dsait_mw_ff0 = false;
      rootObj["pseudo3d_2dsait_mw_d"].toString().toInt()
          ? reportFromDB->pseudo3d_2dsait_mw_d = true
          : reportFromDB->pseudo3d_2dsait_mw_d = false;

      rootObj["pseudo3d_2dsamc_s_ff0"].toString().toInt()
          ? reportFromDB->pseudo3d_2dsamc_s_ff0 = true
          : reportFromDB->pseudo3d_2dsamc_s_ff0 = false;
      rootObj["pseudo3d_2dsamc_s_d"].toString().toInt()
          ? reportFromDB->pseudo3d_2dsamc_s_d = true
          : reportFromDB->pseudo3d_2dsamc_s_d = false;
      rootObj["pseudo3d_2dsamc_mw_ff0"].toString().toInt()
          ? reportFromDB->pseudo3d_2dsamc_mw_ff0 = true
          : reportFromDB->pseudo3d_2dsamc_mw_ff0 = false;
      rootObj["pseudo3d_2dsamc_mw_d"].toString().toInt()
          ? reportFromDB->pseudo3d_2dsamc_mw_d = true
          : reportFromDB->pseudo3d_2dsamc_mw_d = false;

      rootObj["pseudo3d_pcsa_s_ff0"].toString().toInt()
          ? reportFromDB->pseudo3d_pcsa_s_ff0 = true
          : reportFromDB->pseudo3d_pcsa_s_ff0 = false;
      rootObj["pseudo3d_pcsa_s_d"].toString().toInt()
          ? reportFromDB->pseudo3d_pcsa_s_d = true
          : reportFromDB->pseudo3d_pcsa_s_d = false;
      rootObj["pseudo3d_pcsa_mw_ff0"].toString().toInt()
          ? reportFromDB->pseudo3d_pcsa_mw_ff0 = true
          : reportFromDB->pseudo3d_pcsa_mw_ff0 = false;
      rootObj["pseudo3d_pcsa_mw_d"].toString().toInt()
          ? reportFromDB->pseudo3d_pcsa_mw_d = true
          : reportFromDB->pseudo3d_pcsa_mw_d = false;

      rootObj["pseudo3d_mask"].toString().toInt()
          ? reportFromDB->pseudo3d_mask = true
          : reportFromDB->pseudo3d_mask = false;
  ...
  if (db->lastErrno() == US_DB2::OK)  // If not, no ReportItem(s) are associated
                                      // with the parent Report in DB!!
  {
    ...
    foreach (int ID, reportItemsIDs) {
      qry.clear();
      qry << QString("get_report_item_info_by_id") << QString::number(ID);
      db->query(qry);
      db->next();
  ...
  } else  // No reportItem(s) for report in the DB: push default rpeortItem
          // structure??
  {
...
void US_AnalysisProfileGui::setColumnStretches(QGridLayout* genL) {
  for (int ii = 0; ii < 12; ii++) genL->setColumnStretch(ii, 1);
}

// Panel for run and other general parameters
US_AnaprofPanGen::US_AnaprofPanGen(QWidget* topw) : US_WidgetsDialog(topw, 0) {
  ...
  QLabel* lb_panel =
      us_banner(tr("1: Specify OD range and other general parameters"));
...
void US_AnaprofPanGen::build_general_layout() {
  ...
  if (genL != NULL) {
    ...
    for (int ii = 0; ii < allObjects.count(); ii++) {
      ...
      if (objname.contains("GeneralLayout")) {  // Child is the general layout
        have_genl = true;
        DbgLv(1) << "APGe: bgL:    HAVE_GENL=true";
      }

      else if (have_genl) {  // Child is a widget after layout was given
  ...
  QPushButton* pb_gen_report_sttings =
      us_pushbutton(tr("General Report Settings"));
  ...
  if (!mainw->abde_mode_aprofile)
    ...
    pb_scan_excl->setVisible(false);
  row++;

  connect(pb_aproname, SIGNAL(clicked()), this, SLOT(apro_button_clicked()));
  connect(pb_protname, SIGNAL(clicked()), this, SLOT(prot_button_clicked()));
  connect(le_aproname, SIGNAL(editingFinished(void)), this,
          SLOT(apro_text_changed(void)));
  connect(le_protname, SIGNAL(editingFinished(void)), this,
          SLOT(prot_text_changed(void)));
  connect(pb_scan_excl, SIGNAL(clicked()), this, SLOT(set_scan_ranges()));
  connect(pb_gen_report_sttings, SIGNAL(clicked()), this,
          SLOT(set_gen_report_settings()));

  // Build channel lists and rows
  DbgLv(1) << "Ge:SL: nchn" << nchn << "sl_chnsel" << sl_chnsel;
  QLabel* lb_chann =
      us_label(tr("CellChannel:\n"
                  "Optics: Solution"));
  lb_chann->setAlignment(Qt::AlignVCenter | Qt::AlignLeft);
  lb_lcrat = us_label(tr("Loading\nRatio"));
  lb_lctol = us_label(tr("+/- %\nToler."));
  QLabel* lb_ldvol =
      us_label(tr("Loading\nVol. (") + QString(QChar(181)) + "l)");
  ...
  if (!mainw->abde_mode_aprofile) {
    genL->addWidget(lb_mwvprefs, row++, 12, 2, 1);
    row++;
  ...
  qDebug() << "Right.count(), gr_mwvbox.size() BEFORE deletion -- "
           << right->count() << gr_mwvbox.size();
  ...
  qDebug() << "Right.count(), gr_mwvbox.size() AFTER deletion -- "
           << right->count() << gr_mwvbox.size();
  // END clearing right
  ...
  for (int ii = 0; ii < nchn; ii++) {
    ...
    connect(ck_analysisrun, SIGNAL(toggled(bool)), this,
            SLOT(runChecked(bool)));
    ...
    connect(ck_reportrun, SIGNAL(toggled(bool)), this,
            SLOT(reportRunChecked(bool)));
    ...
    connect(pb_reportprefs, SIGNAL(clicked()), this, SLOT(setReport()));
    ...
    connect(ck_mwvprefs, SIGNAL(toggled(bool)), this, SLOT(mwvChecked(bool)));
    ...
    if (ii == 0) {
      ...
      connect(pb_applya, SIGNAL(clicked()), this, SLOT(applied_to_all()));
    } else {
      row++;
    }
    ...
    qDebug() << "In build_gen: curr_wvls.size() for schan -- "
             << curr_wvls.size() << " for " << schan;
    ...
    scrollArea_r->setMaximumWidth((pixelsWide_w + pixelsWide_w + pixelsWide_r) *
                                  3);
    ...
    qDebug() << "Right.count(), gr_mwvbox.size() AFTER insert -- "
             << right->count() << gr_mwvbox.size();
    ;
    // //END MWV dialog

    connect(le_lcrat, SIGNAL(editingFinished(void)), this,
            SLOT(lcrat_text_changed(void)));
    connect(le_lctol, SIGNAL(editingFinished(void)), this,
            SLOT(lctol_text_changed(void)));
    connect(le_ldvol, SIGNAL(editingFinished(void)), this,
            SLOT(ldvol_text_changed(void)));
    connect(le_lvtol, SIGNAL(editingFinished(void)), this,
            SLOT(lvtol_text_changed(void)));
    connect(le_daend, SIGNAL(editingFinished(void)), this,
            SLOT(daend_text_changed(void)));
  }
  DbgLv(1) << "Ge:SL: nchn" << nchn << "lcrat size" << le_lcrats.count();

  // Add button to set global combuned plot parameters
  QPushButton* pb_combplot_global =
      us_pushbutton(tr("Global Parameters for \nCombined Plots"));
  genL->addWidget(pb_combplot_global, row++, 9, 2, 3);
  connect(pb_combplot_global, SIGNAL(clicked()), this,
          SLOT(set_combplot_parms()));
  ...
  for (int i = 0; i < gr_mwvbox.size(); ++i) gr_mwvbox[i]->setVisible(false);
...
void US_AnaprofPanGen::mwvChecked(bool checked) {
  ...
  qDebug() << "oname, Channel name to MWL edit: Checked: " << oname
...
// create groupBox
QGroupBox* US_AnaprofPanGen::createGroup(QString& triple_name,
                                         QList<double>& wvls) {
  ...
  groupBox->setStyleSheet(
      ...
      "gray; border-radius: 10px; margin-top: 10px; margin-bottom: 10px; "
  ...
  for (int ii = 0; ii < wvls.size(); ii++) {
    ...
    // Create signalMapper, to pass argument to standard (argument-less)
    // signals:
    signalMapper = new QSignalMapper(this);
    connect(signalMapper, SIGNAL(mapped(QString)), this,
            SLOT(rbEditClicked(QString)));
...
void US_AnaprofPanGen::rbEditClicked(QString arg_passed) {
  ...
  if (sa != NULL) {
    ...
    if (rb_selected == NULL) qDebug() << "No RadioButton selected ...";
    ...
    foreach (QCheckBox* ckbox, sa->findChildren<QCheckBox*>()) {
      ...
      if (ck_row == irow) {
        if (rb_selected->isChecked()) {
          ...
          // //Update report.wavelength with the selected wvl for current
          // channel: QMap<QString, US_ReportGMP>::iterator ri; QString
          // chann_desc_short = triple_name; chann_desc_short.simplified();
          // chann_desc_short.replace( " ", "" );
          // for ( ri = internal_reports.begin(); ri != internal_reports.end();
          // ++ri )
          //   {
          //     qDebug() << "Key [chandesc] of internal_reports QMap VS
          //     triple_name:  " << ri.key() << " VS " << chann_desc_short; if (
          //     ri.key().contains( chann_desc_short ) )
          // 	 {
          // 	   qDebug() << "SETTING ref. wvl for channel -- " << wvl << "
          // for " << ri.key(); 	   internal_reports[ ri.key() ].wavelength =
          // wvl.toDouble(); 	   break;
          // 	 }
          //   }
          ///////////////////////////////////////////////////////
      ...
      // Enable the rest of checkboxes (associated with not-checked
      // radiobuttons)
      else {
        ckbox->setEnabled(true);
      }
...
void US_AnaprofPanGen::reportRunChecked(bool checked) {
  ...
  QString use_ch_oname =
      QString::number(irow) + ": RefChan --chann_name--" + channel_name;
  qDebug() << "use_ch_oname -- " << use_ch_oname;

  QString use_ref_ch_oname =
      QString::number(irow) + ": RefUseChan --chann_name--" + channel_name;
  ...
  if (!checked) {
    ...
    for (int i = 0; i < le_ref_chs.size(); ++i) {
      if (le_ref_chs[i]->objectName().contains(use_ch_oname)) {
        ...
        // 	 ref_number = ref_numbers_list[ ref_numbers_list.size() - 1 ] +
        // 1 : 	 ref_number = 1 ;
    ...
    qDebug() << "Old/Updated ref_numbers_list_old,  ref_numbers_list -- "
             << ref_numbers_list_old << ref_numbers_list << ", SAME? "
    ...
    for (int i = 0; i < sb_use_ref_chs.size(); ++i) {
      if (sb_use_ref_chs[i]->objectName().contains(use_ref_ch_oname)) {
        ...
        // break;
      } else  // check the rest of chnns: if runReport unchecked, set false,
              // otherwise true
      {
        (ck_report_runs[i]->isChecked()) ? sb_use_ref_chs[i]->setEnabled(true)
                                         : sb_use_ref_chs[i]->setEnabled(false);
        ...
        if (AProfIsIntiated) {
          ...
          // finnally, re-set to min (0) if there are more than 1
          // ref_numbers_list != 0
          //&& ref_numbers_list changed
          int ref_chann_number = 0;
          for (int rn = 0; rn < ref_numbers_list.size(); ++rn)
            if (ref_numbers_list[rn] > 0) ++ref_chann_number;
      ...
      if (le_ref_chs[i]->objectName().contains(use_ch_oname)) {
        ...
        for (int rn = 0; rn < ref_numbers_list.size(); ++rn)
          if (ref_numbers_list[rn] > curr_ref_number) {
            --ref_numbers_list[rn];
            le_ref_chs[rn]->setText("Ref:" +
                                    QString::number(ref_numbers_list[rn]));
          }
  ...
  // check if ALL runReport checkboxes ARE checked, then disable ALL "Use
  // Reference?"
...
void US_AnaprofPanGen::runChecked(bool checked) {
  ...
  QString channel_name = sl_chnsel[irow];
  qDebug() << "Channel name to RUN: Checked: " << channel_name << " : "
           << checked;
  ...
  QString use_ref_ch_oname =
      QString::number(irow) + ": RefUseChan --chann_name--";
  qDebug() << "use_ref_ch_oname -- " << use_ref_ch_oname;
  ...
  if (!checked) {
    ...
    // //Report: No need for Report btn since it's bound now to Run Report
    // checkbox... for ( int i=0; i < pb_reports.size(); ++i )
    // 	 {
    // 	   if ( pb_reports[ i ]->objectName().contains( report_oname ) )
    // 	     {
    // 	       pb_reports[ i ] -> setEnabled( false );
    ...
    for (int i = 0; i < ck_report_runs.size(); ++i) {
      if (ck_report_runs[i]->objectName().contains(run_report_oname)) {
        ...
        if (mainw->abde_mode_aprofile) ck_report_runs[i]->setChecked(true);
        break;
      }
    }
    ...
    // //Replicates: No need for Report btn since it's bound now to Run Report
    // checkbox... for ( int i=0; i < sb_repl_groups.size(); ++i )
    // 	 {
    // 	   if ( sb_repl_groups[ i ]->objectName().contains( replicate_oname ) )
    // 	     {
    // 	       sb_repl_groups[ i ] -> setEnabled( true );
  ...
  // ALEXEY: check what's unselected -- if all triples then disable 2DSA && PCSA
  // tabs
  int run_sum = 0;
  for (int ii = 0; ii < sl_chnsel.size(); ii++) {
...
void US_AnaprofPanGen::setReport(void) {
  ...
  // ALEXEY_NEW_REPORT: internal_reports[ chan_desc ] is now QMap < QString(
  // wvl), US_ReportGMP >
  //  OR should I create a small dialog with all channel's wvls, choose one &&
  //  pass to US_ReportGui ? (one-by-one)
  // US_ReportGMP* chan_report = &(internal_reports[ chan_desc ]);

  // //TESTING !!!!
  //    QMap < QString, US_ReportGMP* > channel_report_map = report_map[
  //    chan_desc ]; reportGui = new US_ReportGui( channel_report_map );
  // ///////////////

  QMap<QString, US_ReportGMP*> channel_report_map;
  QMap<QString, US_ReportGMP>::iterator ri;
  for (ri = internal_reports[chan_desc].begin();
       ri != internal_reports[chan_desc].end(); ++ri) {
    QString wvl = ri.key();
  ...
  reportGui->setWindowFlags(Qt::Dialog | Qt::WindowTitleHint |
                            Qt::WindowMinimizeButtonHint);
  reportGui->setWindowModality(Qt::ApplicationModal);

  connect(reportGui, SIGNAL(cancel_changes(QMap<QString, US_ReportGMP>&)), this,
          SLOT(restore_report(QMap<QString, US_ReportGMP>&)));

  // ALEXEY_NEW_REPORT: this signals may not be needed...
  //  connect( reportGui, SIGNAL( apply_to_all_reports( US_ReportGMP* ) ), this,
  //  SLOT( apply_to_other_reports( US_ReportGMP* )  ) );
  //  /////////////////////////////////////////////////////

  // abde
  if (mainw->abde_mode_aprofile) reportGui->abde_mode_passed();

  reportGui->show();
}

// ALEXEY_NEW_REPORT: this method may not be needed...
void US_AnaprofPanGen::restore_report(
    QMap<QString, US_ReportGMP>& orig_report_map) {
...
void US_AnaprofPanGen::apply_to_other_reports(US_ReportGMP* report) {
  ...
  //     qDebug() << "Applying report's " << primary_report_channel << "
  //     settings to channel " << ri.key();

  //     internal_reports[ ri.key() ].tot_conc            = report->tot_conc;
  //     internal_reports[ ri.key() ].rmsd_limit          = report->rmsd_limit;
  //     internal_reports[ ri.key() ].av_intensity        =
  //     report->av_intensity;
  //     //internal_reports[ ri.key() ].wavelength          =
  //     report->wavelength;
  //     //internal_reports[ ri.key() ].experiment_duration =
  ...
  // 	  internal_reports[ ri.key() ].reportItems.push_back(
  // report->reportItems[ ic ] );
  // 	}
  //   }
}
//////////////////////////////////////////////////////
...
int US_AnaprofPanGen::getProfiles(QStringList& prnames,
  ...
  DbgLv(1) << "APGe:  gP: prnames count" << prnames.count() << "prdat count"
           << prentries.count();
  return prnames.count();  // Return the current list count
}

bool US_AnaprofPanGen::updateProfiles(const QStringList) { return true; }
...
void US_AnaprofPanGen::pass_names(QString& protname, QString& aproname) {
  ...
  DbgLv(1) << "APGe:  pn: passed:" << protname << aproname
           << "McP names:" << mainw->currProf.protoname
           << mainw->currProf.aprofname << "GcP names:" << currProf->protoname
           << currProf->aprofname;
...
void US_AnaprofPanGen::set_gen_report_settings() {
  ...
  // reportMask_tmp = "{\"A. General Settings\":\"2\",\"B. Lab/Rotor
  // Parameters\":\"2\",\"C. Optima Machine Operator\":\"2\",\"D. Speed
  // Parameters\":\"2\",\"E. Cell Centerpiece Usage\":\"2\",\"F. Solutions for
  // Channels\":[{\"Solution Information\":\"0\"},{\"Analyte
  // Information\":\"0\"},{\"Buffer  Information\":\"0\"}],\"G.
  // Optics\":\"2\",\"H. Ranges\":\"0\",\"I. Scan Counts and Intervals for
  // Optics\":\"2\",\"J. Analysis Profile\":[{\"General Settings\":{\"Channel
  // General Settings\":\"0\",\"Report Item Parameters
  // (per-triple)\":\"0\",\"Report Parameters (per-triple)\":\"0\"}},{\"2DSA
  // Controls\":{\"Job Flow Summary\":\"2\",\"Per-Channel
  // Profiles\":\"2\"}},{\"PCSA Controls\":{\"Job Flow
  // Summary\":\"2\",\"Per-Channel Profiles\":\"2\"}}]}";

  // reportGenGui = new US_ReportGenGui( reportMask_tmp );
  reportGenGui = new US_ReportGenGui(currProf->report_mask);
  reportGenGui->setWindowFlags(Qt::Dialog | Qt::WindowTitleHint |
                               Qt::WindowMinimizeButtonHint);
  reportGenGui->setWindowModality(Qt::ApplicationModal);

  connect(reportGenGui, SIGNAL(update_details(QString&)), this,
          SLOT(update_gen_report_settings(QString&)));

  // abde
  if (mainw->abde_mode_aprofile) reportGenGui->abde_mode_passed();
...
void US_AnaprofPanGen::set_combplot_parms() {
  ...
  qDebug() << "loadProf->combPlots_parms --  "
           << mainw->loadProf.combPlots_parms;
  ...
  for (int i = 0; i < nchna; i++) {
    ...
    if (ck_runs[i]->isChecked()) {
      ...
      QString run_report;
      if (ck_report_runs[i]->isChecked()) triple_report = true;
    }

    if (triple_report) {
      QMap<QString, US_ReportGMP> chann_reports =
          internal_reports[channel_desc_alt];
  ...
  combPlotsGui =
      new US_CombPlotsGui(currProf->combPlots_parms, type_method_list);
  combPlotsGui->setWindowFlags(Qt::Dialog | Qt::WindowTitleHint |
                               Qt::WindowMinimizeButtonHint);
  combPlotsGui->setWindowModality(Qt::ApplicationModal);

  connect(combPlotsGui, SIGNAL(update_combplots_parms(QString&)), this,
          SLOT(update_combplots_settings(QString&)));
...
void US_AnaprofPanGen::set_scan_ranges() {
  qDebug() << "Set_scan_ranges 2: scanCount && scanCount_int -- "
           << mainw->scanCount << " && " << mainw->scanCount_int;

  qDebug() << "Set_scan_ranges: SIZES:  currProf->chndescs, "
              "currProf->scan_excl_begin, currProf->scan_excl_end -- "
           << currProf->chndescs.size() << currProf->scan_excl_begin.size()
           << currProf->scan_excl_end.size();

  // debug
  for (int i = 0; i < currProf->chndescs.size(); ++i)
    qDebug() << "Ch_desc, scan_beg, scan_end -- " << currProf->chndescs[i]
             << currProf->scan_excl_begin[i] << currProf->scan_excl_end[i];
  for (int i = 0; i < currProf->scan_excl_begin.size(); ++i)
    qDebug() << "scan_beg, scan_end -- " << currProf->scan_excl_begin[i]
             << currProf->scan_excl_end[i];

  // scanExclGui = new US_ScanExclGui( currProf->chndescs,
  // currProf->scan_excl_begin, currProf->scan_excl_end, mainw->scanCount,
  // mainw->scanCount_int );
  scanExclGui = new US_ScanExclGui(sl_chnsel, currProf->scan_excl_begin,
                                   currProf->scan_excl_end, mainw->scanCount,
                                   mainw->scanCount_int);
  scanExclGui->setWindowFlags(Qt::Dialog | Qt::WindowTitleHint |
                              Qt::WindowMinimizeButtonHint);
  scanExclGui->setWindowModality(Qt::ApplicationModal);

  connect(scanExclGui, SIGNAL(update_aprofile_scans(QStringList&)), this,
          SLOT(update_excl_scans(QStringList&)));
...
void US_AnaprofPanGen::applied_to_all() {
  ...
  for (int jj = 1; jj < sl_chnsel.count();
       jj++) {  // Replace values in all other rows where row 0 changed
    if (le_lcrats[jj]->text() != lcrat) le_lcrats[jj]->setText(lcrat);

    if (le_lctols[jj]->text() != lctol) le_lctols[jj]->setText(lctol);

    if (le_ldvols[jj]->text() != ldvol) le_ldvols[jj]->setText(ldvol);

    if (le_lvtols[jj]->text() != lvtol) le_lvtols[jj]->setText(lvtol);

    if (le_daends[jj]->text() != daend) le_daends[jj]->setText(daend);

    // abde
    if (le_dens0s[jj]->text() != lddens_c) le_dens0s[jj]->setText(lddens_c);

    if (le_vbars[jj]->text() != vbar_c) le_vbars[jj]->setText(vbar_c);

    if (le_MWs[jj]->text() != mw_c) le_MWs[jj]->setText(mw_c);
  }
}
...
US_AnaprofPan2DSA::US_AnaprofPan2DSA(QWidget* topw)
  ...
  connect(cb_chnsel, SIGNAL(activated(int)), this, SLOT(channel_selected(int)));
  connect(pb_nextch, SIGNAL(clicked()), this, SLOT(next_channel()));
  connect(le_smin, SIGNAL(editingFinished()), this, SLOT(smin_changed()));
  connect(le_smax, SIGNAL(editingFinished()), this, SLOT(smax_changed()));
  connect(le_sgrpts, SIGNAL(editingFinished()), this, SLOT(sgpoints_changed()));
  connect(le_kmin, SIGNAL(editingFinished()), this, SLOT(kmin_changed()));
  connect(le_kmax, SIGNAL(editingFinished()), this, SLOT(kmax_changed()));
  connect(le_kgrpts, SIGNAL(editingFinished()), this, SLOT(kgpoints_changed()));
  connect(le_grreps, SIGNAL(editingFinished()), this,
          SLOT(grid_reps_changed()));
  connect(pb_custmg, SIGNAL(clicked()), this, SLOT(cust_grid_clicked()));
  connect(le_custmg, SIGNAL(editingFinished()), this,
          SLOT(cust_grid_changed()));
  connect(ck_varyvb, SIGNAL(toggled(bool)), this,
          SLOT(vary_vbar_checked(bool)));
  connect(le_constk, SIGNAL(editingFinished()), this, SLOT(constk_changed()));
  connect(pb_applya, SIGNAL(clicked()), this, SLOT(apply_all_clicked()));
  connect(ck_j1run, SIGNAL(toggled(bool)), this, SLOT(job1_run_checked(bool)));
  connect(ck_j2run, SIGNAL(toggled(bool)), this, SLOT(job2_run_checked(bool)));
  connect(le_j2gpts, SIGNAL(editingFinished()), this, SLOT(mgpoints_changed()));
  connect(le_j2mrng, SIGNAL(editingFinished()), this, SLOT(mfrange_changed()));

  connect(le_j2iter, SIGNAL(editingFinished()), this, SLOT(mfiter_changed()));

  connect(ck_j3run, SIGNAL(toggled(bool)), this, SLOT(job3_run_checked(bool)));
  connect(ck_j3auto, SIGNAL(toggled(bool)), this, SLOT(autopick_checked(bool)));
  connect(ck_j4run, SIGNAL(toggled(bool)), this, SLOT(job4_run_checked(bool)));
  connect(le_j4iter, SIGNAL(editingFinished()), this, SLOT(rfiters_changed()));
  connect(ck_j5run, SIGNAL(toggled(bool)), this, SLOT(job5_run_checked(bool)));
  connect(le_j5iter, SIGNAL(editingFinished()), this, SLOT(mciters_changed()));
...
void US_AnaprofPan2DSA::channel_selected(int chnx) {
  ...
  // Enable/disable Next based on last index
  int lndx = 0;
  if (active_items_2dsa.size()) lndx = active_items_2dsa.last();
...
void US_AnaprofPan2DSA::gui_to_parms(int rowx) {
  ...
  parm1.have_custg =
      (!le_custmg->text().isEmpty() && le_custmg->text() != "(none)");
...
void US_AnaprofPan2DSA::parms_to_gui(int rowx) {
  US_AnaProfile* currProf = &mainw->currProf;
  QVector<US_AnaProfile::AnaProf2DSA::Parm2DSA>* parms =
      &currProf->ap2DSA.parms;
  int kchan = currProf->pchans.count();
  int kparm = currProf->ap2DSA.parms.size();
  ...
  US_AnaProfile::AnaProf2DSA::Parm2DSA parm1 = parms->at(rowx);
  ...
  if (chan != chan_l) {  // Parms channel wrong, so replace with correct string
  ...
  if (ixchns <
      0) {  // Channel string not found, so search for starting substring
    for (int ii = 0; ii < cb_chnsel->count(); ii++) {
      if (cb_chnsel->itemText(ii).startsWith(chan)) {
        ixchns = ii;
        DbgLv(1) << "2D:SL-pg:   ixchns(I)" << ixchns << chan
                 << cb_chnsel->itemText(ii);
        break;
      }
    }
  } else
    DbgLv(1) << "2D:SL-pg:   ixchns(X)" << ixchns << chan
             << cb_chnsel->itemText(ixchns);
...
// ALEXEY: Next Channel taking into account arbitrary selection of channels in
// Gen tab
void US_AnaprofPan2DSA::next_channel() {
  DbgLv(1) << "2D:SL: NEXT_CHAN";
  int lndx = 0;
...
// smin value changed
void US_AnaprofPan2DSA::smin_changed() { DbgLv(1) << "2D:SL: SMIN_CHG"; }
void US_AnaprofPan2DSA::smax_changed() { DbgLv(1) << "2D:SL: SMAX_CHG"; }
void US_AnaprofPan2DSA::sgpoints_changed() { DbgLv(1) << "2D:SL: SGPTS_CHG"; }
void US_AnaprofPan2DSA::kmin_changed() { DbgLv(1) << "2D:SL: KMIN_CHG"; }
void US_AnaprofPan2DSA::kmax_changed() { DbgLv(1) << "2D:SL: KMAX_CHG"; }
void US_AnaprofPan2DSA::kgpoints_changed() { DbgLv(1) << "2D:SL: KGPTS_CHG"; }
void US_AnaprofPan2DSA::grid_reps_changed() {
  DbgLv(1) << "2D:SL: GRDREPS_CHG";
}
void US_AnaprofPan2DSA::cust_grid_clicked() { DbgLv(1) << "2D:SL: CUSTG_CLK"; }
void US_AnaprofPan2DSA::cust_grid_changed() { DbgLv(1) << "2D:SL: CUSTG_CHG"; }
void US_AnaprofPan2DSA::vary_vbar_checked(bool chkd) {
  DbgLv(1) << "2D:SL: VVBAR_CKD" << chkd;
}
void US_AnaprofPan2DSA::constk_changed() { DbgLv(1) << "2D:SL: CONSTK_CHG"; }
...
void US_AnaprofPan2DSA::apply_all_clicked() {
  ...
  qDebug() << "Apply to All: 2DSA: Current chan index -- " << active_curr_ind
           << chnx;
...
void US_AnaprofPan2DSA::mgpoints_changed() { DbgLv(1) << "2D:SL: J2MGPT_CHG"; }
void US_AnaprofPan2DSA::mfrange_changed() { DbgLv(1) << "2D:SL: J2MRNG_CHG"; }
void US_AnaprofPan2DSA::mfiter_changed() { DbgLv(1) << "2D:SL: J2ITER_CHG"; }
...
void US_AnaprofPan2DSA::rfiters_changed() { DbgLv(1) << "2D:SL: J4RFITER_CHG"; }
...
US_AnaprofPanPCSA::US_AnaprofPanPCSA(QWidget* topw)
  ...
  sl_curvtype << tr("All (IS + DS + SL)") << tr("Increasing Sigmoid")
              << tr("Decreasing Sigmoid") << tr("Straight Line")
              << tr("Horizontal Line [ C(s) ]") << tr("Second Order Power Law");
  ...
  connect(ck_nopcsa, SIGNAL(toggled(bool)), this, SLOT(nopcsa_checked(bool)));
  connect(cb_chnsel, SIGNAL(activated(int)), this, SLOT(channel_selected(int)));
  connect(pb_nextch, SIGNAL(clicked()), this, SLOT(next_channel()));
  connect(cb_curvtype, SIGNAL(activated(int)), this,
          SLOT(curvtype_selected(int)));
  connect(pb_applya, SIGNAL(clicked()), this, SLOT(apply_all_clicked()));
  connect(cb_xaxistyp, SIGNAL(activated(int)), this, SLOT(xaxis_selected(int)));
  ...
  connect(le_xmin, SIGNAL(editingFinished()), this, SLOT(verify_xyz()));
  connect(le_xmax, SIGNAL(editingFinished()), this, SLOT(verify_xyz()));

  connect(cb_yaxistyp, SIGNAL(activated(int)), this, SLOT(yaxis_selected(int)));
  ...
  // 	       this,   SLOT  ( verify_xyz ( const QString& ) ) );
  connect(le_ymin, SIGNAL(editingFinished()), this, SLOT(verify_xyz()));
  connect(le_ymax, SIGNAL(editingFinished()), this, SLOT(verify_xyz()));

  connect(cb_zaxistyp, SIGNAL(activated(int)), this, SLOT(zaxis_selected(int)));

  // connect( le_zvalue,    SIGNAL( editingFinished  ( )      ),
  //          this,         SLOT  ( zvalue_changed   ( )      ) );
  // connect( le_zvalue, SIGNAL( textChanged ( const QString& ) ),
  // 	    this,   SLOT  ( verify_xyz ( const QString& ) ) );
  connect(le_zvalue, SIGNAL(editingFinished()), this, SLOT(verify_xyz()));

  connect(le_varcount, SIGNAL(editingFinished()), this,
          SLOT(varcount_changed()));
  connect(le_grfiters, SIGNAL(editingFinished()), this,
          SLOT(grfiters_changed()));
  connect(le_crpoints, SIGNAL(editingFinished()), this,
          SLOT(crpoints_changed()));
  connect(ck_tinoise, SIGNAL(toggled(bool)), this, SLOT(tinoise_checked(bool)));
  connect(ck_rinoise, SIGNAL(toggled(bool)), this, SLOT(rinoise_checked(bool)));
  connect(ck_tregspec, SIGNAL(toggled(bool)), this,
          SLOT(tregspec_checked(bool)));
  connect(ck_tregauto, SIGNAL(toggled(bool)), this,
          SLOT(tregauto_checked(bool)));
  connect(le_regalpha, SIGNAL(editingFinished()), this, SLOT(alpha_changed()));
  connect(le_mciters, SIGNAL(editingFinished()), this, SLOT(mciters_changed()));
...
void US_AnaprofPanPCSA::channel_selected(int chnx) {
  ...
  // int lndx    = sl_chnsel.count() - 1;
  int lndx = 0;
  if (active_items_pcsa.size()) lndx = active_items_pcsa.last();
...
void US_AnaprofPanPCSA::gui_to_parms(int rowx) {
  ...
  if (parm1.treg_flag == 0) parm1.treg_type = "none";
  if (parm1.treg_flag == 1) parm1.treg_type = "specified_alpha";
  if (parm1.treg_flag == 2) parm1.treg_type = "auto_computed_alpha";
  ...
  parm1.channel = sl_chnsel[rowx];
  DbgLv(1) << "PC:SL-gp:  curv_type" << parm1.curv_type << "channel"
           << parm1.channel;

  qDebug() << "In gui_to_parms:  ck_tregspec->isChecked(), parm1.treg_flag -- "
           << ck_tregspec->isChecked() << parm1.treg_flag;
...
void US_AnaprofPanPCSA::parms_to_gui(int rowx) {
  US_AnaProfile* currProf = &mainw->currProf;
  QVector<US_AnaProfile::AnaProfPCSA::ParmPCSA>* parms =
      &currProf->apPCSA.parms;
  int kchan = currProf->pchans.count();
  int kparm = currProf->apPCSA.parms.size();
  ...
  US_AnaProfile::AnaProfPCSA::ParmPCSA parm1 = parms->at(rowx);
  QString chan = parm1.channel;
  QString chan_g = cb_chnsel->itemText(rowx);
  QString chan_l = sl_chnsel[rowx];
  DbgLv(1) << "PC:SL-pg:  rowx" << rowx << "channel" << chan_g << chan
           << chan_l;

  int ixcurv = cb_curvtype->findText(parm1.curv_type);
  if (ixcurv < 0) {  // Curve type not found, so search for starting substring
    for (int ii = 0; ii < cb_curvtype->count(); ii++) {
      if (cb_curvtype->itemText(ii).startsWith(parm1.curv_type)) {
        ixcurv = ii;
        DbgLv(1) << "PC:SL-pg:   ixcurv" << ixcurv << parm1.curv_type
                 << cb_curvtype->itemText(ii);
        break;
      }
    }
  }

  if (chan != chan_g) {  // Parms channel wrong, so replace with correct string
  ...
  if (ixchns <
    ...
    for (int ii = 0; ii < cb_chnsel->count(); ii++) {
      if (cb_chnsel->itemText(ii).startsWith(chan)) {
        ixchns = ii;
        DbgLv(1) << "PC:SL-pg:   ixchns(I)" << ixchns << chan
                 << cb_chnsel->itemText(ii);
        break;
      }
    }
  } else
    DbgLv(1) << "PC:SL-pg:   ixchns(X)" << ixchns << chan
             << cb_chnsel->itemText(ixchns);
...
// X Min Changed
void US_AnaprofPanPCSA::xmin_changed() { DbgLv(1) << "PC:SL: XMIN_CHG"; }
// X Max Changed
void US_AnaprofPanPCSA::xmax_changed() { DbgLv(1) << "PC:SL: XMAX_CHG"; }
  ...
  if (oname == "xmax") {
    ...
    if (le_xmin->text().toDouble() < 0) {
      if (x < le_xmin->text().toDouble()) {
        ...
        if (x > -0.1) le_xmax->setText(QString::number(-0.1));
      }
    }
  }
...
// Y Min changed
void US_AnaprofPanPCSA::ymin_changed() { DbgLv(1) << "PC:SL: YMIN_CHG"; }
// Y Max changed
void US_AnaprofPanPCSA::ymax_changed() { DbgLv(1) << "PC:SL: YMAX_CHG"; }
// Z Axis type selected
void US_AnaprofPanPCSA::zaxis_selected(int zaxx) {
  DbgLv(1) << "PC:SL: ZAXIS_SEL" << zaxx;
}
// Z Value changed
void US_AnaprofPanPCSA::zvalue_changed() { DbgLv(1) << "PC:SL: ZVAL_CHG"; }
// Variations Count changed
void US_AnaprofPanPCSA::varcount_changed() { DbgLv(1) << "PC:SL: VCNT_CHG"; }
// Grid Fit Iterations changed
void US_AnaprofPanPCSA::grfiters_changed() { DbgLv(1) << "PC:SL: GFITER_CHG"; }
// Curve Resolution Points changed
void US_AnaprofPanPCSA::crpoints_changed() { DbgLv(1) << "PC:SL: CRPTS_CHG"; }
// TI Noise checked
...
void US_AnaprofPanPCSA::tregspec_checked(bool chkd) {
  ...
  if (chkd) {  // On-Specified checked, so uncheck On-Auto
    ck_tregauto->setChecked(false);
  }
...
void US_AnaprofPanPCSA::tregauto_checked(bool chkd) {
  ...
  if (chkd) {  // On-Auto checked, so uncheck On-Specified
    ck_tregspec->setChecked(false);
  }
  // Alpha read-only except when On-Specified is checked
  us_setReadOnly(le_regalpha, !ck_tregspec->isChecked());
...
void US_AnaprofPanPCSA::alpha_changed() { DbgLv(1) << "PC:SL: ALPHA_CHG"; }
// Monte-Carlo Iterations changed
void US_AnaprofPanPCSA::mciters_changed() { DbgLv(1) << "PC:SL: MCITER_CHG"; }
