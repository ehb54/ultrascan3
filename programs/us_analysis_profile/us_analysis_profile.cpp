...
US_AnalysisProfileGui::US_AnalysisProfileGui() : US_Widgets() {
   ...
   connect(apanGeneral, SIGNAL(set_tabs_buttons_inactive(void)), this, SLOT(disable_tabs_buttons(void)));
   connect(apanGeneral, SIGNAL(set_tabs_buttons_active_readonly(void)), this, SLOT(enable_tabs_buttons_readonly(void)));
   connect(apanGeneral, SIGNAL(set_tabs_buttons_active(void)), this, SLOT(enable_tabs_buttons(void)));
...
void US_AnalysisProfileGui::inherit_protocol(US_RunProtocol *iProto) {
   ...
   qDebug() << "In inherit: ScanCounts : " << iProto->scanCount << iProto->scanCount_int
            << iProto->rpSpeed.ssteps[ 0 ].scancount << iProto->rpSpeed.ssteps[ 0 ].scancount_int;
   ...
   if (iProto->rpOptic.chopts.count() > 0)
      DbgLv(1) << "APG: ipro: 0)ch s1 s2 s3" << iProto->rpOptic.chopts[ 0 ].channel << iProto->rpOptic.chopts[ 0 ].scan1
               << iProto->rpOptic.chopts[ 0 ].scan2 << iProto->rpOptic.chopts[ 0 ].scan3;
   if (iProto->rpOptic.chopts.count() > 1)
      DbgLv(1) << "APG: ipro: 1)ch s1 s2 s3" << iProto->rpOptic.chopts[ 1 ].channel << iProto->rpOptic.chopts[ 1 ].scan1
               << iProto->rpOptic.chopts[ 1 ].scan2 << iProto->rpOptic.chopts[ 1 ].scan3;
   ...
   for (int ii = 0; ii < ncho; ii++) { // Examine protocol's channel optics
      ...
      DbgLv(1) << "APG: ipro:  o.ii" << ii << "chname" << chname << "scan1 scan2 scan3" << scan1 << scan2 << scan3;
      ...
      for (int jj = 0; jj < ods.count(); jj++) { // Create a channel entry for each optics type of this channel
         ...
         if (nchn < kchn) { // Replace channel and channel description
            ...
            qDebug() << "currProf.chndescs_alt[ nchn ], chentr_wvls -- " << currProf.chndescs_alt[ nchn ]
                     << chentr_wvls;
            ...
            for (ri = currProf.ch_reports.begin(); ri != currProf.ch_reports.end(); ++ri) {
               ...
               for (tri = triple_reports.begin(); tri != triple_reports.end(); ++tri) {
                  ...
                  for (int ii = 0; ii < currProf.ch_reports[ chan_desc ][ old_wvl ].reportItems.size(); ++ii) {
                     ...
                     qDebug() << "wvl: type, method, lo, hi -- " << old_wvl << ": " << initItem.type << initItem.method
            ...
            qDebug() << "In inherit_prot: [new] wvl, wvls.size() for channel -- " << currProf.ch_wvls[ chentr_wvls ]
            ...
            for (int i = 0; i < ch_wavelengths.size(); ++i) {
               ...
               if (chentr_wvls.contains("Interf.")) {
                  if (
                     currProf.ch_reports[ chentr_wvls ][ c_wvl ].DBread
            ...
            DbgLv(1) << "APG: ipro:     chx nchn dae" << chx << nchn << "dae size" << currProf.data_ends.count()
                     << "chentr" << chentr << "currProf.analysis_run[ chx ]" << currProf.analysis_run[ chx ]
            ...
            qDebug() << "In inherit_prot: wvls.size() for channel -- " << currProf.ch_wvls[ chentr_wvls ].size()
                     << " for " << chentr_wvls;
            ...
            DbgLv(1) << "APG: ipro:     lch" << lch << "lv_tol da_end" << currProf.lv_tolers[ lch ]
                     << currProf.data_ends[ lch ] << "dae size" << currProf.data_ends.count()
   ...
   DbgLv(1) << "APG: ipro: name" << iProto->protoname << "cPname" << currProf.aprofname << "xml len" << ap_xml.length()
            << "GUID" << currProf.aprofGUID << iProto->rpAprof.aprofGUID;

   QString aprsrc("currProf");
   bool need_rec = ap_xml.isEmpty();
   need_rec = (currProf.aprofname == iProto->rpAprof.aprofname) ? need_rec : true;
   ...
   qDebug() << "APG: ipro: currProf.aprofname, iProto->rpAprof.aprofname, need_rec -- " << currProf.aprofname
            << iProto->rpAprof.aprofname << need_rec;
   ...
   if (need_rec) {
      ...
      if (load_db) { // Get AProfile XML from the database
         ...
         if (status != US_DB2::OK || numrow < 1) {
            ...
            DbgLv(1) << "APG: ipro:  ap ID,name,lnxml" << currProf.aprofID << currProf.aprofname << ap_xml.length();
         }
      }
      ...
      if (!load_db) { // Get AProfile XML from local disk
         ...
         QString xmlipath
            = US_DataFiles::get_filename(dapath, currProf.aprofGUID, "A", "analysis_profile", "guid", new_file);

         DbgLv(1) << "APG: ipro:   new_file" << new_file << "xmlipath" << xmlipath << "guid" << currProf.aprofGUID;
         if (new_file) { // No local file, so clear XML
            ...
            if (xifile.open(QIODevice::ReadOnly | QIODevice::Text)) {
               ...
               currProf.aprofID = -(QString(xmlipath).section("/", -1, -1).section(".", -2, -2).mid(1).toInt());
         ...
         if (currProf.ch_report_ids.size()) {
            ...
            for (ri = currProf.ch_report_ids.begin(); ri != currProf.ch_report_ids.end(); ++ri) {
               ...
               for (int rid = 0; rid < reportIDs.size(); ++rid) {
                  ...
                  qDebug() << "Reading reportItems form DB: channel_alt_desc, reportID  -- " << channel_alt_desc << ", "
                           << reportID;
                  ...
                  qDebug() << "Filling currProf.ch_reports from DB: channel_alt_desc, wvl_read -- " << channel_alt_desc
                           << ", " << wvl_read;
               }
               ...
               if (abde_mode_aprofile && reportIDs.size() == 1 && ch_wavelengths.size() > 1 && !wvl_read_abde.isEmpty()) {
                  qDebug() << "AProfile[ABDE]: reading from DB: replicating reports for MWL based on the 1st channel's "
                              "wvl -- ";
                  for (int i = 0; i < ch_wavelengths.size(); ++i) {
                     QString c_wvl = QString::number(ch_wavelengths[ i ]);
   ...
   if (aprsrc == "currProf") {
      ...
      if (nchan_g == nchan_2 && nchan_g == nchan_p) { // Channels same size, so check contents
         for (int ii = 0; ii < nchan_g; ii++) {
            if (sl_chnsel_g[ ii ] != sl_chnsel_2[ ii ] || sl_chnsel_g[ ii ] != sl_chnsel_p[ ii ])
               need_reset = true;
         }
      }
      ...
      need_reset = false;
      if (nchan_p2 == nchan_2 && nchan_pp == nchan_p) { // Channels same size, so check contents
         for (int ii = 0; ii < nchan_g; ii++) {
            if (
               currProf.ap2DSA.parms[ ii ].channel != sl_chnsel_2[ ii ]
   ...
   qDebug() << "Size, Keys of currProf.ch_reports -- " << currProf.ch_reports.size() << ", "
...
void US_AnalysisProfileGui::get_report_by_ID(US_ReportGMP *reportFromDB, int reportID) {
   ...
   if (db->lastErrno() != US_DB2::OK) {
      QMessageBox::warning(
         this, tr("Connection Problem"), tr("Reading Report: Could not connect to database: \n") + db->lastError());
      return;
   }
   ...
   if (db->lastErrno() == US_DB2::OK) {
      ...
      if (
         rootObj.contains("tot_conc") && rootObj.contains("rmsd_lim") && rootObj.contains("intensity")
         && rootObj.contains("exp_dur") && rootObj.contains("integration") && rootObj.contains("plots")) {
         rootObj[ "tot_conc" ].toString().toInt() ? reportFromDB->tot_conc_mask = true
                                                  : reportFromDB->tot_conc_mask = false;
         rootObj[ "rmsd_lim" ].toString().toInt() ? reportFromDB->rmsd_limit_mask = true
                                                  : reportFromDB->rmsd_limit_mask = false;
         rootObj[ "intensity" ].toString().toInt() ? reportFromDB->av_intensity_mask = true
                                                   : reportFromDB->av_intensity_mask = false;
         rootObj[ "exp_dur" ].toString().toInt() ? reportFromDB->experiment_duration_mask = true
                                                 : reportFromDB->experiment_duration_mask = false;
         rootObj[ "integration" ].toString().toInt() ? reportFromDB->integration_results_mask = true
      ...
      if (
         rootObj.contains("pseudo3d_2dsait_s_ff0") && rootObj.contains("pseudo3d_2dsait_s_d")
         && rootObj.contains("pseudo3d_2dsait_mw_ff0") && rootObj.contains("pseudo3d_2dsait_mw_d")
         && rootObj.contains("pseudo3d_2dsamc_s_ff0") && rootObj.contains("pseudo3d_2dsamc_s_d")
         && rootObj.contains("pseudo3d_2dsamc_mw_ff0") && rootObj.contains("pseudo3d_2dsamc_mw_d")
         && rootObj.contains("pseudo3d_pcsa_s_ff0") && rootObj.contains("pseudo3d_pcsa_s_d")
         && rootObj.contains("pseudo3d_pcsa_mw_ff0") && rootObj.contains("pseudo3d_pcsa_mw_d")
         && rootObj.contains("pseudo3d_mask")) {
         rootObj[ "pseudo3d_2dsait_s_ff0" ].toString().toInt() ? reportFromDB->pseudo3d_2dsait_s_ff0 = true
                                                               : reportFromDB->pseudo3d_2dsait_s_ff0 = false;
         rootObj[ "pseudo3d_2dsait_s_d" ].toString().toInt() ? reportFromDB->pseudo3d_2dsait_s_d = true
                                                             : reportFromDB->pseudo3d_2dsait_s_d = false;
         rootObj[ "pseudo3d_2dsait_mw_ff0" ].toString().toInt() ? reportFromDB->pseudo3d_2dsait_mw_ff0 = true
                                                                : reportFromDB->pseudo3d_2dsait_mw_ff0 = false;
         rootObj[ "pseudo3d_2dsait_mw_d" ].toString().toInt() ? reportFromDB->pseudo3d_2dsait_mw_d = true
                                                              : reportFromDB->pseudo3d_2dsait_mw_d = false;

         rootObj[ "pseudo3d_2dsamc_s_ff0" ].toString().toInt() ? reportFromDB->pseudo3d_2dsamc_s_ff0 = true
                                                               : reportFromDB->pseudo3d_2dsamc_s_ff0 = false;
         rootObj[ "pseudo3d_2dsamc_s_d" ].toString().toInt() ? reportFromDB->pseudo3d_2dsamc_s_d = true
                                                             : reportFromDB->pseudo3d_2dsamc_s_d = false;
         rootObj[ "pseudo3d_2dsamc_mw_ff0" ].toString().toInt() ? reportFromDB->pseudo3d_2dsamc_mw_ff0 = true
                                                                : reportFromDB->pseudo3d_2dsamc_mw_ff0 = false;
         rootObj[ "pseudo3d_2dsamc_mw_d" ].toString().toInt() ? reportFromDB->pseudo3d_2dsamc_mw_d = true
                                                              : reportFromDB->pseudo3d_2dsamc_mw_d = false;

         rootObj[ "pseudo3d_pcsa_s_ff0" ].toString().toInt() ? reportFromDB->pseudo3d_pcsa_s_ff0 = true
                                                             : reportFromDB->pseudo3d_pcsa_s_ff0 = false;
         rootObj[ "pseudo3d_pcsa_s_d" ].toString().toInt() ? reportFromDB->pseudo3d_pcsa_s_d = true
                                                           : reportFromDB->pseudo3d_pcsa_s_d = false;
         rootObj[ "pseudo3d_pcsa_mw_ff0" ].toString().toInt() ? reportFromDB->pseudo3d_pcsa_mw_ff0 = true
                                                              : reportFromDB->pseudo3d_pcsa_mw_ff0 = false;
         rootObj[ "pseudo3d_pcsa_mw_d" ].toString().toInt() ? reportFromDB->pseudo3d_pcsa_mw_d = true
                                                            : reportFromDB->pseudo3d_pcsa_mw_d = false;

         rootObj[ "pseudo3d_mask" ].toString().toInt() ? reportFromDB->pseudo3d_mask = true
 ...
 */
   ...
   connect(le_aproname, SIGNAL(editingFinished(void)), this, SLOT(apro_text_changed(void)));
   connect(le_protname, SIGNAL(editingFinished(void)), this, SLOT(prot_text_changed(void)));
   connect(pb_scan_excl, SIGNAL(clicked()), this, SLOT(set_scan_ranges()));
   connect(pb_gen_report_sttings, SIGNAL(clicked()), this, SLOT(set_gen_report_settings()));
   ...
   for (int ii = 0; ii < nchn; ii++) {
      ...
      connect(ck_analysisrun, SIGNAL(toggled(bool)), this, SLOT(runChecked(bool)));
      ...
      connect(ck_reportrun, SIGNAL(toggled(bool)), this, SLOT(reportRunChecked(bool)));
      ...
      connect(le_lcrat, SIGNAL(editingFinished(void)), this, SLOT(lcrat_text_changed(void)));
      connect(le_lctol, SIGNAL(editingFinished(void)), this, SLOT(lctol_text_changed(void)));
      connect(le_ldvol, SIGNAL(editingFinished(void)), this, SLOT(ldvol_text_changed(void)));
      connect(le_lvtol, SIGNAL(editingFinished(void)), this, SLOT(lvtol_text_changed(void)));
      connect(le_daend, SIGNAL(editingFinished(void)), this, SLOT(daend_text_changed(void)));
   }
   ...
   connect(pb_combplot_global, SIGNAL(clicked()), this, SLOT(set_combplot_parms()));
...
QGroupBox *US_AnaprofPanGen::createGroup(QString &triple_name, QList<double> &wvls) {
   ...
   groupBox->setStyleSheet(
      ...
      "#FFFFFF); border: 2px solid gray; border-radius: 10px; margin-top: 10px; margin-bottom: 10px; padding-top: 5px; "
...
void US_AnaprofPanGen::reportRunChecked(bool checked) {
   ...
   if (!checked) {
      ...
      qDebug() << "Old/Updated ref_numbers_list_old,  ref_numbers_list -- " << ref_numbers_list_old << ref_numbers_list
...
void US_AnaprofPanGen::setReport(void) {
   ...
   connect(
      reportGui, SIGNAL(cancel_changes(QMap<QString, US_ReportGMP> &)), this,
...
int US_AnaprofPanGen::getProfiles(QStringList &prnames, QList<QStringList> &prentries) {
   DbgLv(1) << "APGe: getProtos IN";
   //   prnames         = pr_names;   // Return a names list copy
   //   prentries       = protdata;   // Return an entries list copy
   DbgLv(1) << "APGe:  gP: prnames count" << prnames.count() << "prdat count" << prentries.count();
...
void US_AnaprofPanGen::pass_names(QString &protname, QString &aproname) {
   ...
   DbgLv(1) << "APGe:  pn: passed:" << protname << aproname << "McP names:" << mainw->currProf.protoname
            << mainw->currProf.aprofname << "GcP names:" << currProf->protoname << currProf->aprofname;
...
void US_AnaprofPanGen::apro_button_clicked() {
   ...
   //*TEMPORARY
   QMessageBox::information(this, "Under Development", "This will lead to an AnalysisProfileGui selection dialog");
   //*TEMPORARY
}
...
void US_AnaprofPanGen::set_scan_ranges() {
   qDebug() << "Set_scan_ranges 2: scanCount && scanCount_int -- " << mainw->scanCount << " && "
            << mainw->scanCount_int;

   qDebug() << "Set_scan_ranges: SIZES:  currProf->chndescs, currProf->scan_excl_begin, currProf->scan_excl_end -- "
            << currProf->chndescs.size() << currProf->scan_excl_begin.size() << currProf->scan_excl_end.size();

   //debug
   for (int i = 0; i < currProf->chndescs.size(); ++i)
      qDebug() << "Ch_desc, scan_beg, scan_end -- " << currProf->chndescs[ i ] << currProf->scan_excl_begin[ i ]
   ...
   scanExclGui = new US_ScanExclGui(
      sl_chnsel, currProf->scan_excl_begin, currProf->scan_excl_end, mainw->scanCount, mainw->scanCount_int);
...
void US_AnaprofPanGen::update_excl_scans(QStringList &scan_ranges_list) {
   ...
   qDebug() << "Scan update: init sizes scan_excl_beginn, scan_excl_end -- " << currProf->scan_excl_begin.size()
   ...
   qDebug() << "Scan update: final sizes scan_excl_beginn, scan_excl_end -- " << currProf->scan_excl_begin.size()
            << currProf->scan_excl_end.size();
}
...
void US_AnaprofPanGen::prot_button_clicked() {
   ...
   //*TEMPORARY
   QMessageBox::information(this, "Under Development", "This will lead to a Protocol selection dialog");
   //*TEMPORARY
}
...
void US_AnaprofPanGen::applied_to_all() {
   ...
   for (int jj = 1; jj < sl_chnsel.count(); jj++) { // Replace values in all other rows where row 0 changed
...
US_AnaprofPan2DSA::US_AnaprofPan2DSA(QWidget *topw) : US_WidgetsDialog(topw, 0) {
   mainw = ( US_AnalysisProfileGui * ) topw;
   dbg_level = US_Settings::us_debug();
   cchx = 0;
   ...
   connect(le_grreps, SIGNAL(editingFinished()), this, SLOT(grid_reps_changed()));
   connect(pb_custmg, SIGNAL(clicked()), this, SLOT(cust_grid_clicked()));
   connect(le_custmg, SIGNAL(editingFinished()), this, SLOT(cust_grid_changed()));
   connect(ck_varyvb, SIGNAL(toggled(bool)), this, SLOT(vary_vbar_checked(bool)));
...
void US_AnaprofPan2DSA::gui_to_parms(int rowx) {
   ...
   parm1.have_custg = (!le_custmg->text().isEmpty() && le_custmg->text() != "(none)");
...
void US_AnaprofPan2DSA::parms_to_gui(int rowx) {
   ...
   if (ixchns < 0) { // Channel string not found, so search for starting substring
...
US_AnaprofPanPCSA::US_AnaprofPanPCSA(QWidget *topw) : US_WidgetsDialog(topw, 0) {
   DbgLv(1) << "APpc: IN";
   mainw = ( US_AnalysisProfileGui * ) topw;
   dbg_level = US_Settings::us_debug();
   cchx = 0;
   ...
   sl_curvtype << tr("All (IS + DS + SL)") << tr("Increasing Sigmoid") << tr("Decreasing Sigmoid")
               << tr("Straight Line") << tr("Horizontal Line [ C(s) ]") << tr("Second Order Power Law");
   ...
   connect(cb_chnsel, SIGNAL(activated(int)), this, SLOT(channel_selected(int)));
   connect(pb_nextch, SIGNAL(clicked()), this, SLOT(next_channel()));
   connect(cb_curvtype, SIGNAL(activated(int)), this, SLOT(curvtype_selected(int)));
   connect(pb_applya, SIGNAL(clicked()), this, SLOT(apply_all_clicked()));
   ...
   connect(le_varcount, SIGNAL(editingFinished()), this, SLOT(varcount_changed()));
   connect(le_grfiters, SIGNAL(editingFinished()), this, SLOT(grfiters_changed()));
   connect(le_crpoints, SIGNAL(editingFinished()), this, SLOT(crpoints_changed()));
   ...
   connect(ck_tregspec, SIGNAL(toggled(bool)), this, SLOT(tregspec_checked(bool)));
   connect(ck_tregauto, SIGNAL(toggled(bool)), this, SLOT(tregauto_checked(bool)));
...
void US_AnaprofPanPCSA::gui_to_parms(int rowx) {
   ...
   qDebug() << "In gui_to_parms:  ck_tregspec->isChecked(), parm1.treg_flag -- " << ck_tregspec->isChecked()
            << parm1.treg_flag;
...
void US_AnaprofPanPCSA::parms_to_gui(int rowx) {
   ...
   DbgLv(1) << "PC:SL-pg:   curvtype ndx" << cb_curvtype->currentIndex() << "chnsel ndx" << cb_chnsel->currentIndex();
