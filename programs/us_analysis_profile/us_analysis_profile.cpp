...
void
US_AnalysisProfileGui::back_to_pcsa (void)
{
...
void
US_AnalysisProfileGui::reset (void)
{
...
void
US_AnalysisProfileGui::auto_mode_passed (void)
{
  automode = true;
}
...
void
US_AnalysisProfileGui::auto_name_passed (QString &p_protname,
                                         QString &p_aproname)
{
...
US_AnalysisProfileGui::inherit_protocol (US_RunProtocol *iProto)
{
  ...
  for (int ii = 0; ii < nchs; ii++)
    { // Examine protocol's channel solutions
      ...
      chname
          = QString (chname).section (":", 0, 0).section (",", 0, 0).replace (
              " / ", "");
      sl_chns << chname;
      sl_sols << sodesc;
  ...
  for (int ii = 0; ii < ncho; ii++)
    { // Examine protocol's channel optics
      ...
      if (scan1.isEmpty () && scan2.isEmpty () && scan3.isEmpty ())
        continue;
        ...
        { // Create a channel entry for each optics type of this channel
          ...
          if (nchn < kchn)
            { // Replace channel and channel description
              ...
              for (ri = currProf.ch_reports.begin ();
                   ri != currProf.ch_reports.end (); ++ri)
                {
                  ...
                  for (tri = triple_reports.begin ();
                       tri != triple_reports.end (); ++tri)
                    {
                      old_wvl = tri.key ();

                      for (int ii = 0;
                           ii < currProf.ch_reports[chan_desc][old_wvl]
                                    .reportItems.size ();
                           ++ii)
                        {
                          US_ReportGMP::ReportItem initItem
                              = currProf.ch_reports[chan_desc][old_wvl]
                                    .reportItems[ii];
                          qDebug ()
                              << "wvl: type, method, lo, hi -- " << old_wvl
                              << ": " << initItem.type << initItem.method
                              << initItem.range_low << initItem.range_high;
                        }
                    }
                }
              // END DEBUG
              ...
              qDebug ()
                  << "In inherit_prot: [new] wvl, wvls.size() for channel -- "
              ...
              // currProf.chndescs_alt[ nchn ] ]; currProf.ch_reports[
              // chentr_wvls ].channel_name = chentr_wvls;

              // ALEXEY_NEW_REPORT: move these 2 lines below setting
              // currProf.ch_wvls[ chentr_wvls ] Next, iterate over wvls & fill
              // QMap< QString, QMap < QString( wvls[i]), US_ReportGMP > >
              // currProf.ch_reports[ chentr_wvls ][wvl]
              ...
              for (int i = 0; i < ch_wavelengths.size (); ++i)
                {
                  ...
                  currProf.ch_reports[chentr_wvls][c_wvl]
                      = currProf
                            .ch_reports[currProf.chndescs_alt[nchn]][c_wvl];

                  // currProf.ch_reports[ chentr_wvls ][ c_wvl ] =
                  // currProf.ch_reports[ old_desc ][ old_wvl ];
                  currProf.ch_reports[chentr_wvls][c_wvl].channel_name
                      = chentr_wvls;

                  qDebug () << "nchn < kchn: Filling currProf.ch_reports -- ";
                  qDebug () << "chentr_wvls, c_wvl -- " << chentr_wvls << ", "
                            << c_wvl;
                  ...
                  if (chentr_wvls.contains ("Interf."))
                    {
                      if (currProf.ch_reports[chentr_wvls][c_wvl].DBread
                        ...
                        {
                          currProf.ch_reports[chentr_wvls][c_wvl].tot_conc
                              = 3.0;
                          currProf.ch_reports[chentr_wvls][c_wvl]
                              .reportItems.clear ();
                        }
                  ...
                  double tot_time_exp
                      = iProto->rpSpeed.ssteps[0].duration
              ...
              DbgLv (1) << "APG: ipro:     chx nchn dae" << chx << nchn
                        ...
                        << "chentr" << chentr << "currProf.analysis_run[ chx ]"
                        << currProf.analysis_run[chx]
            ...
            { // Append channel and channel description
              ...
              // //currProf.ch_reports[ chentr_wvls ] = reportGMP;
              // currProf.ch_reports[ chentr_wvls ] = currProf.ch_reports[
              // currProf.chndescs_alt.last() ]; currProf.ch_reports[
              // chentr_wvls ].channel_name = chentr_wvls;
              ...
              for (int i = 0; i < ch_wavelengths.size (); ++i)
                {
                  ...
                  currProf.ch_reports[chentr_wvls][c_wvl]
                      = currProf
                            .ch_reports[currProf.chndescs_alt[nchn]][c_wvl];
                  // currProf.ch_reports[ chentr_wvls ][ c_wvl ] =
                  // currProf.ch_reports[ currProf.chndescs_alt[ nchn - 1 ] ][
                  // old_wvl ];
                  currProf.ch_reports[chentr_wvls][c_wvl].channel_name
                      = chentr_wvls;
  ...
  if (need_rec)
    {
      ...
      if (!load_db)
        { // Get AProfile XML from local disk
          ...
          QString xmlipath = US_DataFiles::get_filename (
              dapath, currProf.aprofGUID, "A", "analysis_profile", "guid",
              new_file);

          DbgLv (1) << "APG: ipro:   new_file" << new_file << "xmlipath"
                    << xmlipath << "guid" << currProf.aprofGUID;
          if (new_file)
      ...
      if (!ap_xml.isEmpty ())
        {
          ...
          DbgLv (1) << "APG: ipro:   2kparm pkparm"
                    << currProf.ap2DSA.parms.count ()
                    << currProf.apPCSA.parms.count () << "kpchan"
                    << currProf.pchans.count ();
          //         initPanels();

          // Check here if ch_report_ids[ channel_alt_desc ] and/or
          // ch_report_guids[ channel_alt_desc ] are not empty Also check if
          // ch_report_ids.values() - the IDs - are not 0!!!
          ...
          //   {
          //     if( reportIDs_list[i] <= 0 ) //reportid is -1 or 0, so no
          //     report associated with the channel
          //       {
          // 	 null_reportID = true;
          // 	 break;
          //       }
          ...
          if (currProf.ch_report_ids.size ())
            {
              // fill in currProf->ch_reports[ chdesc_alt ] by reading
              // ReportIDs from ch_report_ids[ chdesc_alt ]

              // Also, clean  currProf->ch_reports first  (since it will be
              // re-created form DB here )
              currProf.ch_reports
                  .clear (); // <-- Will this cause crash - as this omits
                             // "B:Interf." cannels ??
              ...
              for (ri = currProf.ch_report_ids.begin ();
                   ri != currProf.ch_report_ids.end (); ++ri)
                {
                  ...
                  // ALEXEY_NEW_REPORT: here ri.value() will be QList/QVector
                  // of reportIDs for a given channel next will be cycle over
                  // this QList...
                  ...
                  for (int rid = 0; rid < reportIDs.size (); ++rid)
                    {
                      ...
                      qDebug () << "Reading reportItems form DB: "
                      ...
                      // ALEXEY_NEW_REPORT: here the QMap< QString, QMap <
                      // QString(wvl), US_ReportGMP > > currProf.ch_reports
                      // will be filled Something like currProf.ch_reports[
                      // channel_alt_desc ][ wvl ] -- where wvl ==
                      // *reportFromDB->wavelength;
                      //  OR  wvl is returned from get_report_by_ID(
                      //  reportFromDB, reportID );

                      QString wvl_read
                          = QString::number (reportFromDB->wavelength);

                      if (reportIDs.size () == 1)
                        wvl_read_abde = wvl_read;

                      // assign retieved report to currProf.ch_reports[
                      // channel_alt_desc ];
                      currProf.ch_reports[channel_alt_desc][wvl_read]
                          = *reportFromDB;
                  ...
                  // In case of ABDE: the above loop would contain only 1
                  // report (in DB: 1 report per channel [1st wvl]) For
                  // internal consistency, we need to populate (COPY) reports
                  // for the rest of wvls in the channel (will not be used)
                  QList<double> ch_wavelengths
                      = currProf.ch_wvls[channel_alt_desc];
                  if (abde_mode_aprofile && reportIDs.size () == 1
                      && ch_wavelengths.size () > 1
                      && !wvl_read_abde.isEmpty ())
                    {
                      qDebug () << "AProfile[ABDE]: reading from DB: "
                                   "replicating reports for MWL based on the "
                                   "1st channel's wvl -- ";
                      for (int i = 0; i < ch_wavelengths.size (); ++i)
                        {
                          QString c_wvl = QString::number (ch_wavelengths[i]);
                          ...
                          currProf.ch_reports[channel_alt_desc][c_wvl]
                              = currProf.ch_reports[channel_alt_desc]
                                                   [wvl_read_abde];
                        }
                    }
  ...
  qDebug () << "Size, Keys of currProf.ch_reports -- "
            << currProf.ch_reports.size () << ", "
            << currProf.ch_reports.keys ();
}

// retrieve Report by ID from DB into ReportGMP structure
void
US_AnalysisProfileGui::get_report_by_ID (US_ReportGMP *reportFromDB,
                                         int reportID)
{
  US_DB2 *db = NULL;
  US_Passwd pw;
  ...
  if (db->lastErrno () == US_DB2::OK)
    {
      while (db->next ())
        {
          ...
          reportFromDB->experiment_duration
              = db->value (4).toString ().toDouble ();
      ...
      qDebug () << "report's channelname, wvl -- "
                << reportFromDB->channel_name << reportFromDB->wavelength;
      qDebug () << "reportMaskJson form DB -- " << reportMaskJson;
      // process report Mask Json into US_ReportGMP fields:
      QJsonDocument jsonDoc
          = QJsonDocument::fromJson (reportMaskJson.toUtf8 ());
      if (!jsonDoc.isObject ())
        {
      ...
      if (rootObj.contains ("tot_conc") && rootObj.contains ("rmsd_lim")
          ...
          && rootObj.contains ("integration") && rootObj.contains ("plots"))
        {
          ...
          rootObj["plots"].toString ().toInt ()
              ? reportFromDB->plots_mask = true
  ...
  if (db->lastErrno ()
      == US_DB2::OK) // If not, no ReportItem(s) are associated with the parent
                     // Report in DB!!
    {
      ...
      foreach (int ID, reportItemsIDs)
        {
          ...
          reportItem_read.integration_val
              = db->value (4).toString ().toDouble ();
          reportItem_read.tolerance = db->value (5).toString ().toDouble ();
          reportItem_read.total_percent
              = db->value (6).toString ().toDouble ();
          reportItem_read.combined_plot = db->value (7).toString ().toInt ();
          reportItem_read.ind_combined_plot
              = db->value (8).toString ().toInt ();
...
// Reset parameters to their defaults
void
US_AnalysisProfileGui::close_program (void)
{
...
// Apply Profile
void
US_AnalysisProfileGui::apply_profile (void)
{
...
void
US_AnalysisProfileGui::setColumnStretches (QGridLayout *genL)
{
...
void
US_AnaprofPanGen::build_general_layout ()
{
      ...
      for (int ii = 0; ii < allObjects.count (); ii++)
        {
          ...
          if (objname.contains ("GeneralLayout"))
            ...
            {
              DbgLv (1) << "APGe: bgL:    CHILD *NOT* DELETED  have_genl"
                        << have_genl;
            }
        }
  ...
  connect (pb_aproname, SIGNAL (clicked ()), this,
           SLOT (apro_button_clicked ()));
  connect (pb_protname, SIGNAL (clicked ()), this,
           SLOT (prot_button_clicked ()));
  ...
  for (int ii = 0; ii < nchn; ii++)
    {
      ...
      // ); sb_ref_ch ->setMinimum( 0 ); sb_ref_ch ->setMaximum( int(nchn/2.0)
      ...
      sb_use_ref_ch->setObjectName (stchan + "RefUseChan --chann_name--"
                                    + schan);
      ...
      pb_reportprefs->setObjectName (strow + ": Report --chann_name--"
                                     + schan);
      ...
      sb_repl_group->setObjectName (strow + ": Replicate --chann_name--"
                                    + schan);
      ...
      connect (ck_mwvprefs, SIGNAL (toggled (bool)), this,
               SLOT (mwvChecked (bool)));
      ...
      if (ii == 0)
        {
          ...
          connect (pb_applya, SIGNAL (clicked ()), this,
                   SLOT (applied_to_all ()));
        }
      else
        {
...
void
US_AnaprofPanGen::mwvChecked (bool checked)
{
...
// create groupBox
QGroupBox *
US_AnaprofPanGen::createGroup (QString &triple_name, QList<double> &wvls)
{
  ...
  for (int ii = 0; ii < wvls.size (); ii++)
    {
      ...
      rb_edit->setObjectName (strow
                              + ":triple_edit:" + QString::number (wvls[ii]));
      ...
      ck_run->setObjectName (strow
                             + ":triple_run:" + QString::number (wvls[ii]));
...
void
US_AnaprofPanGen::rbEditClicked (QString arg_passed)
{
  // QObject* sobj       = sender();      // Sender object
  // QString oname       = sobj->objectName();
              ...
              if (rb_selected->isChecked ())
                {
                  ...
                  // //ALEXEY_NEW_REPORT: this setting wvl will not be
                  // needed...
                  // //Update report.wavelength with the selected wvl for
                  // current channel: QMap<QString, US_ReportGMP>::iterator ri;
                  // QString chann_desc_short = triple_name;
                  // chann_desc_short.simplified();
                  // chann_desc_short.replace( " ", "" );
                  // for ( ri = internal_reports.begin(); ri !=
                  // internal_reports.end(); ++ri )
                  //   {
                  //     qDebug() << "Key [chandesc] of internal_reports QMap
                  //     VS triple_name:  " << ri.key() << " VS " <<
                  //     chann_desc_short; if ( ri.key().contains(
                  //     chann_desc_short ) )
                  // 	 {
                  // 	   qDebug() << "SETTING ref. wvl for channel -- " <<
                  // wvl << " for " << ri.key(); 	   internal_reports[ ri.key()
                  // ].wavelength = wvl.toDouble(); 	   break;
                  // 	 }
                  //   }
                  ///////////////////////////////////////////////////////
                }
...
void
US_AnaprofPanGen::reportRunChecked (bool checked)
{
  QObject *sobj = sender (); // Sender object
  QString oname = sobj->objectName ();
  int irow = oname.section (":", 0, 0).toInt ();
          ...
          if (le_ref_chs[i]->objectName ().contains (use_ch_oname))
            {
              ...
              // 	 ref_number = ref_numbers_list[ ref_numbers_list.size()
              // - 1 ] + 1 : 	 ref_number = 1 ;
              ...
              if (AProfIsIntiated)
                {
                  ...
                  for (int rn = 0; rn < ref_numbers_list.size (); ++rn)
                    if (ref_numbers_list[rn] > 0)
                      ++ref_chann_number;
                  if (ref_chann_number > 1
                      && ref_numbers_list_old != ref_numbers_list)
                    sb_use_ref_chs[i]->setValue (
                        sb_use_ref_chs[i]->minimum ());
      ...
      for (int i = 0; i < le_ref_chs.size (); ++i)
        {
          if (le_ref_chs[i]->objectName ().contains (use_ch_oname))
            {
              int curr_ref_number
                  = le_ref_chs[i]->text ().split (":")[1].toInt ();
              // reset properly all other ref_numbers
              for (int rn = 0; rn < ref_numbers_list.size (); ++rn)
                if (ref_numbers_list[rn] > curr_ref_number)
                  {
                    --ref_numbers_list[rn];
                    le_ref_chs[rn]->setText (
                        "Ref:" + QString::number (ref_numbers_list[rn]));
                  }
...
void
US_AnaprofPanGen::runChecked (bool checked)
{
  QObject *sobj = sender (); // Sender object
  QString oname = sobj->objectName ();
  int irow = oname.section (":", 0, 0).toInt ();
  ...
  if (!checked)
    {
      ...
      // for ( int i=0; i<sb_use_ref_chs.size(); ++i)
      // 	 {
      // 	   if ( sb_use_ref_chs[ i ]->objectName().contains(
      // use_ref_ch_oname ) )
      // 	     {
      // 	       sb_use_ref_chs[ i ] -> setEnabled( false );
      // 	       break;
  ...
  else
    {
      ...
      // //Replicates: No need for Report btn since it's bound now to Run
      // Report checkbox... for ( int i=0; i < sb_repl_groups.size(); ++i )
      // 	 {
      // 	   if ( sb_repl_groups[ i ]->objectName().contains(
      // replicate_oname ) )
      // 	     {
      // 	       sb_repl_groups[ i ] -> setEnabled( true );
      // 	       break;
  ...
  // ALEXEY: check what's unselected -- if all triples then disable 2DSA &&
  // PCSA tabs
  int run_sum = 0;
  for (int ii = 0; ii < sl_chnsel.size (); ii++)
    {
...
// Report Btn clicked
void
US_AnaprofPanGen::setReport (void)
{
  QObject *sobj = sender (); // Sender object
  QString oname = sobj->objectName ();
  int irow = oname.section (":", 0, 0).toInt ();
  ...
  connect (reportGui, SIGNAL (cancel_changes (QMap<QString, US_ReportGMP> &)),
           this, SLOT (restore_report (QMap<QString, US_ReportGMP> &)));

  // ALEXEY_NEW_REPORT: this signals may not be needed...
  //  connect( reportGui, SIGNAL( apply_to_all_reports( US_ReportGMP* ) ),
  //  this, SLOT( apply_to_other_reports( US_ReportGMP* )  ) );
  //  /////////////////////////////////////////////////////
...
void
US_AnaprofPanGen::restore_report (QMap<QString, US_ReportGMP> &orig_report_map)
{
...
// ALEXEY_NEW_REPORT: this method may not be needed...
void
US_AnaprofPanGen::apply_to_other_reports (US_ReportGMP *report)
{
...
int
US_AnaprofPanGen::getProfiles (QStringList &prnames,
...
bool
US_AnaprofPanGen::updateProfiles (const QStringList)
{
  return true;
}

void
US_AnaprofPanGen::disable_name_buttons ()
{
...
void
US_AnaprofPanGen::pass_names (QString &protname, QString &aproname)
{
...
void
US_AnaprofPanGen::apro_button_clicked ()
{
...
void
US_AnaprofPanGen::set_gen_report_settings ()
{
...
void
US_AnaprofPanGen::update_gen_report_settings (QString &mask_updated)
{
...
void
US_AnaprofPanGen::set_combplot_parms ()
{
...
void
US_AnaprofPanGen::update_combplots_settings (QString &combPlots_updated)
{
...
void
US_AnaprofPanGen::set_scan_ranges ()
{
...
void
US_AnaprofPanGen::update_excl_scans (QStringList &scan_ranges_list)
{
...
void
US_AnaprofPanGen::prot_button_clicked ()
{
...
void
US_AnaprofPanGen::apro_text_changed ()
{
...
void
US_AnaprofPanGen::prot_text_changed ()
{
...
void
US_AnaprofPanGen::lcrat_text_changed ()
{
  QObject *sobj = sender ();
...
void
US_AnaprofPanGen::lctol_text_changed ()
{
  QObject *sobj = sender ();
  QString sname = sobj->objectName ();
  int chnx = sname.section (":", 0, 0).toInt ();
...
void
US_AnaprofPanGen::ldvol_text_changed ()
{
  QObject *sobj = sender ();
  QString sname = sobj->objectName ();
  int chnx = sname.section (":", 0, 0).toInt ();
...
void
US_AnaprofPanGen::lvtol_text_changed ()
{
  QObject *sobj = sender ();
  QString sname = sobj->objectName ();
  int chnx = sname.section (":", 0, 0).toInt ();
...
void
US_AnaprofPanGen::daend_text_changed ()
{
  QObject *sobj = sender ();
  QString sname = sobj->objectName ();
  int chnx = sname.section (":", 0, 0).toInt ();
...
void
US_AnaprofPanGen::applied_to_all ()
{
    ...
    : US_WidgetsDialog (topw, 0)
{
  ...
  connect (cb_chnsel, SIGNAL (activated (int)), this,
           SLOT (channel_selected (int)));
  ...
  connect (le_sgrpts, SIGNAL (editingFinished ()), this,
           SLOT (sgpoints_changed ()));
  ...
  connect (le_kgrpts, SIGNAL (editingFinished ()), this,
           SLOT (kgpoints_changed ()));
  ...
  connect (le_constk, SIGNAL (editingFinished ()), this,
           SLOT (constk_changed ()));
  connect (pb_applya, SIGNAL (clicked ()), this, SLOT (apply_all_clicked ()));
  connect (ck_j1run, SIGNAL (toggled (bool)), this,
           SLOT (job1_run_checked (bool)));
  connect (ck_j2run, SIGNAL (toggled (bool)), this,
           SLOT (job2_run_checked (bool)));
  connect (le_j2gpts, SIGNAL (editingFinished ()), this,
           SLOT (mgpoints_changed ()));
  connect (le_j2mrng, SIGNAL (editingFinished ()), this,
           SLOT (mfrange_changed ()));

  connect (le_j2iter, SIGNAL (editingFinished ()), this,
           SLOT (mfiter_changed ()));

  connect (ck_j3run, SIGNAL (toggled (bool)), this,
           SLOT (job3_run_checked (bool)));
  connect (ck_j3auto, SIGNAL (toggled (bool)), this,
           SLOT (autopick_checked (bool)));
  connect (ck_j4run, SIGNAL (toggled (bool)), this,
           SLOT (job4_run_checked (bool)));
  connect (le_j4iter, SIGNAL (editingFinished ()), this,
           SLOT (rfiters_changed ()));
  connect (ck_j5run, SIGNAL (toggled (bool)), this,
           SLOT (job5_run_checked (bool)));
  connect (le_j5iter, SIGNAL (editingFinished ()), this,
           SLOT (mciters_changed ()));
...
void
US_AnaprofPan2DSA::channel_selected (int chnx)
{
  int prvx = cchx;
  cchx = chnx;
  DbgLv (1) << "2D:SL: CHAN_SEL" << chnx << "prvx" << prvx;
...
void
US_AnaprofPan2DSA::gui_to_parms (int rowx)
{
  ...
  DbgLv (1) << "2D:SL-gp: rowx" << rowx << "kchan" << kchan << "kparm"
            << kparm;
  if (rowx >= kparm)
    {
...
void
US_AnaprofPan2DSA::parms_to_gui (int rowx)
{
  US_AnaProfile *currProf = &mainw->currProf;
  ...
  int kchan = currProf->pchans.count ();
  int kparm = currProf->ap2DSA.parms.size ();
  DbgLv (1) << "2D:SL-pg: rowx" << rowx << "kchan" << kchan << "kparm"
            << kparm;
  if (rowx >= kparm)
    {
...
// Gen tab
void
US_AnaprofPan2DSA::next_channel ()
{
  DbgLv (1) << "2D:SL: NEXT_CHAN";
  int lndx = 0;
  if (active_items_2dsa.size ())
...
// smin value changed
void
US_AnaprofPan2DSA::smin_changed ()
{
  DbgLv (1) << "2D:SL: SMIN_CHG";
}
void
US_AnaprofPan2DSA::smax_changed ()
{
  DbgLv (1) << "2D:SL: SMAX_CHG";
}
void
US_AnaprofPan2DSA::sgpoints_changed ()
{
  DbgLv (1) << "2D:SL: SGPTS_CHG";
}
void
US_AnaprofPan2DSA::kmin_changed ()
{
  DbgLv (1) << "2D:SL: KMIN_CHG";
}
void
US_AnaprofPan2DSA::kmax_changed ()
{
  DbgLv (1) << "2D:SL: KMAX_CHG";
}
void
US_AnaprofPan2DSA::kgpoints_changed ()
{
  DbgLv (1) << "2D:SL: KGPTS_CHG";
}
void
US_AnaprofPan2DSA::grid_reps_changed ()
{
  DbgLv (1) << "2D:SL: GRDREPS_CHG";
}
void
US_AnaprofPan2DSA::cust_grid_clicked ()
{
  DbgLv (1) << "2D:SL: CUSTG_CLK";
}
void
US_AnaprofPan2DSA::cust_grid_changed ()
{
  DbgLv (1) << "2D:SL: CUSTG_CHG";
}
void
US_AnaprofPan2DSA::vary_vbar_checked (bool chkd)
{
  DbgLv (1) << "2D:SL: VVBAR_CKD" << chkd;
}
void
US_AnaprofPan2DSA::constk_changed ()
{
  DbgLv (1) << "2D:SL: CONSTK_CHG";
}

void
US_AnaprofPan2DSA::apply_all_clicked ()
{
...
void
US_AnaprofPan2DSA::job1_run_checked (bool chkd)
{
  DbgLv (1) << "2D:SL: JOB1RUN_CKD" << chkd;
}
void
US_AnaprofPan2DSA::job2_run_checked (bool chkd)
{
  DbgLv (1) << "2D:SL: JOB2RUN_CKD" << chkd;
}
void
US_AnaprofPan2DSA::mgpoints_changed ()
{
  DbgLv (1) << "2D:SL: J2MGPT_CHG";
}
void
US_AnaprofPan2DSA::mfrange_changed ()
{
  DbgLv (1) << "2D:SL: J2MRNG_CHG";
}
void
US_AnaprofPan2DSA::mfiter_changed ()
{
  DbgLv (1) << "2D:SL: J2ITER_CHG";
}
void
US_AnaprofPan2DSA::job3_run_checked (bool chkd)
{
  DbgLv (1) << "2D:SL: JOB3RUN_CKD" << chkd;
}
void
US_AnaprofPan2DSA::autopick_checked (bool chkd)
{
  DbgLv (1) << "2D:SL: J3AUTO_CKD" << chkd;
}
void
US_AnaprofPan2DSA::job4_run_checked (bool chkd)
{
  DbgLv (1) << "2D:SL: JOB4RUN_CKD" << chkd;
}
void
US_AnaprofPan2DSA::rfiters_changed ()
{
  DbgLv (1) << "2D:SL: J4RFITER_CHG";
}
void
US_AnaprofPan2DSA::job5_run_checked (bool chkd)
{
  DbgLv (1) << "2D:SL: JOB5RUN_CKD" << chkd;
}
void
US_AnaprofPan2DSA::mciters_changed ()
{
    ...
    : US_WidgetsDialog (topw, 0)
{
  ...
  connect (ck_nopcsa, SIGNAL (toggled (bool)), this,
           SLOT (nopcsa_checked (bool)));
  connect (cb_chnsel, SIGNAL (activated (int)), this,
           SLOT (channel_selected (int)));
  connect (pb_nextch, SIGNAL (clicked ()), this, SLOT (next_channel ()));
  ...
  connect (pb_applya, SIGNAL (clicked ()), this, SLOT (apply_all_clicked ()));
  connect (cb_xaxistyp, SIGNAL (activated (int)), this,
           SLOT (xaxis_selected (int)));
  ...
  connect (cb_yaxistyp, SIGNAL (activated (int)), this,
           SLOT (yaxis_selected (int)));
  ...
  connect (cb_zaxistyp, SIGNAL (activated (int)), this,
           SLOT (zaxis_selected (int)));
  ...
  connect (ck_tinoise, SIGNAL (toggled (bool)), this,
           SLOT (tinoise_checked (bool)));
  connect (ck_rinoise, SIGNAL (toggled (bool)), this,
           SLOT (rinoise_checked (bool)));
  ...
  connect (le_regalpha, SIGNAL (editingFinished ()), this,
           SLOT (alpha_changed ()));
  connect (le_mciters, SIGNAL (editingFinished ()), this,
           SLOT (mciters_changed ()));
...
// NO PCSA Checked
void
US_AnaprofPanPCSA::nopcsa_checked (bool chkd)
{
...
void
US_AnaprofPanPCSA::channel_selected (int chnx)
{
  DbgLv (1) << "PC:SL: CHAN_SEL" << chnx;
  int prvx = cchx;
  cchx = chnx;
...
// Gen tab
void
US_AnaprofPanPCSA::next_channel ()
{
  DbgLv (1) << "PC:SL: NEXT_CHAN";
  int lndx = 0;
  if (active_items_pcsa.size ())
...
// Transfer gui elements to parms vector row
void
US_AnaprofPanPCSA::gui_to_parms (int rowx)
{
  US_AnaProfile *currProf = &mainw->currProf;
  US_AnaProfile::AnaProfPCSA::ParmPCSA parm1;
  int kchan = currProf->pchans.count ();
  int kparm = currProf->apPCSA.parms.size ();
  DbgLv (1) << "PC:SL-gp: rowx" << rowx << "kchan" << kchan << "kparm"
            << kparm;
  if (rowx >= kparm)
    {
  ...
  qDebug ()
      << "In gui_to_parms:  ck_tregspec->isChecked(), parm1.treg_flag -- "
...
// Transfer parms vector element to gui elements
void
US_AnaprofPanPCSA::parms_to_gui (int rowx)
{
  US_AnaProfile *currProf = &mainw->currProf;
  ...
  int kchan = currProf->pchans.count ();
  int kparm = currProf->apPCSA.parms.size ();
  DbgLv (1) << "PC:SL-pg: rowx" << rowx << "kchan" << kchan << "kparm"
            << kparm;
  if (rowx >= kparm)
    {
...
void
US_AnaprofPanPCSA::apply_all_clicked ()
{
...
// Curve Type Selected
void
US_AnaprofPanPCSA::curvtype_selected (int curx)
{
  DbgLv (1) << "PC:SL: CTYPE_SEL" << curx;
}
// X Axis Selected
void
US_AnaprofPanPCSA::xaxis_selected (int xaxx)
{
  DbgLv (1) << "PC:SL: XAXIS_SEL" << xaxx;
}
// X Min Changed
void
US_AnaprofPanPCSA::xmin_changed ()
{
  DbgLv (1) << "PC:SL: XMIN_CHG";
}
// X Max Changed
void
US_AnaprofPanPCSA::xmax_changed ()
{
...
void
US_AnaprofPanPCSA::bind_min_max (QString oname, double x)
{
  // x_min
  if (oname == "xmin")
    {
      ...
      if (le_xmin->text ().toDouble () > 0)
        {
          if (x < le_xmin->text ().toDouble ())
            le_xmax->setText (
                QString::number (le_xmin->text ().toDouble () + 0.01));
        }
      if (le_xmin->text ().toDouble () < 0)
        {
          if (x < le_xmin->text ().toDouble ())
      ...
      if (ytype == "f/f0")
        {
          if (le_ymax->text ().toDouble () < le_ymin->text ().toDouble ())
            le_ymax->setText (
                QString::number (le_ymin->text ().toDouble () + 0.01));
        }
    }
...
int
US_AnaprofPanPCSA::check_syntax_xyz (void)
{
  int syntax_errors = 0;
...
// Y Axis selected
void
US_AnaprofPanPCSA::yaxis_selected (int yaxx)
{
...
// Y Min changed
void
US_AnaprofPanPCSA::ymin_changed ()
{
  DbgLv (1) << "PC:SL: YMIN_CHG";
}
// Y Max changed
void
US_AnaprofPanPCSA::ymax_changed ()
{
  DbgLv (1) << "PC:SL: YMAX_CHG";
}
// Z Axis type selected
void
US_AnaprofPanPCSA::zaxis_selected (int zaxx)
{
  DbgLv (1) << "PC:SL: ZAXIS_SEL" << zaxx;
}
// Z Value changed
void
US_AnaprofPanPCSA::zvalue_changed ()
{
  DbgLv (1) << "PC:SL: ZVAL_CHG";
}
// Variations Count changed
void
US_AnaprofPanPCSA::varcount_changed ()
{
  DbgLv (1) << "PC:SL: VCNT_CHG";
}
// Grid Fit Iterations changed
void
US_AnaprofPanPCSA::grfiters_changed ()
{
  DbgLv (1) << "PC:SL: GFITER_CHG";
}
// Curve Resolution Points changed
void
US_AnaprofPanPCSA::crpoints_changed ()
{
  DbgLv (1) << "PC:SL: CRPTS_CHG";
}
// TI Noise checked
void
US_AnaprofPanPCSA::tinoise_checked (bool chkd)
{
  DbgLv (1) << "PC:SL: TNOI_CKD" << chkd;
}
// RI Noise checked
void
US_AnaprofPanPCSA::rinoise_checked (bool chkd)
{
  DbgLv (1) << "PC:SL: RNOI_CKD" << chkd;
}
// Regularization Type: On-Specified checked
void
US_AnaprofPanPCSA::tregspec_checked (bool chkd)
{
...
void
US_AnaprofPanPCSA::tregauto_checked (bool chkd)
{
...
void
US_AnaprofPanPCSA::alpha_changed ()
{
  DbgLv (1) << "PC:SL: ALPHA_CHG";
}
// Monte-Carlo Iterations changed
void
US_AnaprofPanPCSA::mciters_changed ()
{
