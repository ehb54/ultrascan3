...
QString
US_AnalysisProfileGui::childSValue (const QString child, const QString type)
{
  QString value ("");
  if (child == "general")
    {
      value = apanGeneral->getSValue (type);
    }
  else if (child == "2dsa")
    {
      value = apan2DSA->getSValue (type);
    }
  else if (child == "pcsa")
    {
      value = apanPCSA->getSValue (type);
    }
  return value;
}
...
int
US_AnalysisProfileGui::childIValue (const QString child, const QString type)
{
  int value = 0;
  if (child == "general")
    {
      value = apanGeneral->getIValue (type);
    }
  else if (child == "2dsa")
    {
      value = apan2DSA->getIValue (type);
    }
  else if (child == "pcsa")
    {
      value = apanPCSA->getIValue (type);
    }
  return value;
}
...
double
US_AnalysisProfileGui::childDValue (const QString child, const QString type)
{
  double value = 0.0;
  if (child == "general")
    {
      value = apanGeneral->getDValue (type);
    }
  else if (child == "2dsa")
    {
      value = apan2DSA->getDValue (type);
    }
  else if (child == "pcsa")
    {
      value = apanPCSA->getDValue (type);
    }
  return value;
}
...
QStringList
US_AnalysisProfileGui::childLValue (const QString child, const QString type)
{
  QStringList value;

  if (child == "general")
    {
      value = apanGeneral->getLValue (type);
    }
  else if (child == "2dsa")
    {
      value = apan2DSA->getLValue (type);
    }
  else if (child == "pcsa")
    {
      value = apanPCSA->getLValue (type);
    }
...
void
US_AnalysisProfileGui::newPanel (int panx)
{
...
void
US_AnalysisProfileGui::statUpdate ()
{
  DbgLv (1) << "statUpd: IN stat" << statflag;
  statflag
      = apanGeneral->status () + apan2DSA->status () + apanPCSA->status ();
  DbgLv (1) << "statUpd:  MOD stat" << statflag;
}

// Slot to advance to the next panel
void
US_AnalysisProfileGui::panelUp ()
{
...
void
US_AnalysisProfileGui::panelDown ()
{
  int newndx = tabWidget->currentIndex () - 1;
...
void
US_AnalysisProfileGui::help (void)
{
  if (curr_panx == 0)
    apanGeneral->help ();
  else if (curr_panx == 1)
...
void
US_AnalysisProfileGui::disable_tabs_buttons (void)
{
  // apan2DSA     ->initPanel();
  // apan2DSA     ->savePanel();
...
void
US_AnalysisProfileGui::enable_tabs_buttons (void)
{
  DbgLv (1) << "ENABLING!!!";
...
void
US_AnalysisProfileGui::enable_tabs_buttons_readonly (void)
{
      ...
      for (int jj = 0; jj < allPButtons.count (); jj++)
        {
          if ((allPButtons[jj]->text ()).contains ("View Solution Details")
              ...
              || (allPButtons[jj]->text ())
                     .contains ("View Experiment Details")
...
int
US_AnalysisProfileGui::getProfiles (QStringList &par1,
...
bool
US_AnalysisProfileGui::updateProfiles (const QStringList par1)
{
...
void
US_AnalysisProfileGui::initPanels ()
{
  ...
  qDebug () << "AProfile::initPanels(): abde_mode_aprofile ? "
            << abde_mode_aprofile;
...
void
US_AnalysisProfileGui::savePanels ()
{
...
use_db=false;
  ...
  for (ri = currProf->ch_reports.begin (); ri != currProf->ch_reports.end ();
       ++ri)
    {
      ...
      qDebug () << "[BEGIN]US_AnaprofPanGen::initPanel(): chan_desc:::  "
                << chan_desc;

      // if chan_desc contains "(unspecified)", save this ch_report as the
      // refrence one.. copy it to all other channels
      ...
      if (chan_desc.contains ("(unspecified)"))
        {
          ...
          qDebug () << "Reference report name, wvls, #wvl -- " << chan_desc
                    << triple_reports_ref.keys ()
                    << triple_reports_ref.keys ().size ();
          continue;
        }
      ...
      for (tri = triple_reports.begin (); tri != triple_reports.end (); ++tri)
        {
          ...
          // if ref_report was not replaced yet, substitute all channels'
          // reports witht he ref. one...
          if (!triple_reports_ref.isEmpty ())
            {
              currProf->ch_reports[chan_desc][c_wvl]
                  = triple_reports_ref[wvl_ref];
            }
          else // check if some channel's report still contain 's'/'2DSA' etc.
               // (caused by adding/removing wvl(s)/channels)
            {
              if (mainw->abde_mode_aprofile)
                {
                  for (int ii = 0; ii < currProf->ch_reports[chan_desc][c_wvl]
                                            .reportItems.size ();
                       ++ii)
                    {
                      US_ReportGMP::ReportItem initItem
                          = currProf->ch_reports[chan_desc][c_wvl]
                                .reportItems[ii];
                      if (initItem.type == 's'
                          && initItem.method.contains ("2DSA"))
                        {
                          // if attempts to insert reportItem from
                          // Velocity-type, replace with ABDE-type
                          currProf->ch_reports[chan_desc][c_wvl]
                              .reportItems[ii]
                              .type
                              = QString ("Radius");
          ...
          //&& debug
          for (int ii = 0;
               ii < currProf->ch_reports[chan_desc][c_wvl].reportItems.size ();
               ++ii)
            {

              US_ReportGMP::ReportItem initItem
                  = currProf->ch_reports[chan_desc][c_wvl].reportItems[ii];

              qDebug () << "wvl, type, method, lo, hi -- " << c_wvl
  ...
  for (wi = currProf->ch_wvls.begin (); wi != currProf->ch_wvls.end (); ++wi)
    {
      ...
      qDebug ()
          << "[BEGIN]US_AnaprofPanGen::initPanel(): chan_desc [ch_wvls]:::  "
          << chan_desc;
  ...
  DbgLv (1) << "APGe: inP: aname pname" << currProf->aprofname
            << currProf->protoname;
  ...
  for (int ii = 0; ii < nchan; ii++)
    {
      ...
      QString chname
          = QString (chdesc).section (":", 0, 0).section (",", 0, 0);
      ...
      if ((chopts.contains (tr ("(unspecified)")))
          ...
          || (chopts.contains ("nterf") && chname.contains ("B")))
        {
      ...
      sl_chnsel << chnsel;
      DbgLv (1) << "APGe: inP:  ch" << ii << "chdesc" << chdesc << "chnsel"
                << chnsel;
    }

  // //ALEXEY: now exclude channels for B:Interf. from the following
  // QLists/QMaps:
  ...
  DbgLv (1) << "APGe: inP: 0)tol,dae size" << currProf->lv_tolers.count ()
            << currProf->data_ends.count ();
  DbgLv (1) << "APGe: inP: 0)le_chn,lcr size" << le_channs.count ()
            << le_lcrats.count ();
  build_general_layout ();
  nchan = le_channs.count ();
  DbgLv (1) << "APGe: inP: 1)tol,dae size" << currProf->lv_tolers.count ()
            << currProf->data_ends.count ();
  DbgLv (1) << "APGe: inP: 1)le_chn,lcr size" << le_channs.count ()
            << le_lcrats.count () << "nchan_gui" << nchan;
  ...
  if (le_lcrats.count () == nchan)
    { // Reset General channel parameter gui elements
      for (int ii = 0; ii < nchan; ii++)
        {
          qDebug () << "currProf->analysis_run.size(): "
                    << currProf->analysis_run.count ();
          qDebug () << "currProf->scan_excl_begin.size(): "
                    << currProf->scan_excl_begin.count ();
          qDebug () << "currProf->scan_excl_end.size(): "
                    << currProf->scan_excl_end.count ();

          // ALEXEY: the next 5 following QLists/QMaps must be stripped of the
          // enties associated with B:Interf. channels!!!
          qDebug () << "currProf->wvl_edit.size(): "
                    << currProf->wvl_edit.count ();
          qDebug () << "currProf->wvl_not_run.size(): "
                    << currProf->wvl_not_run.count ();
          qDebug () << "currProf->ch_wvls.size(): "
                    << currProf->ch_wvls.count ();
          qDebug () << "currProf->ch_reports.size():   "
                    << currProf->ch_reports.count ();

          qDebug () << "currProf->chndescs_alt.size(): "
                    << currProf->chndescs_alt.count ();

          qDebug () << "nchan, sl_chnsel.size(): " << nchan
                    << sl_chnsel.count ();
          qDebug () << "ch_wvls for --  " << currProf->chndescs_alt[ii]
                    << currProf->ch_wvls[currProf->chndescs_alt[ii]];

          qDebug () << "scan_beg, scan_end for -- "
                    << currProf->chndescs_alt[ii]
                    << currProf->scan_excl_begin[ii]
                    << currProf->scan_excl_end[ii];
          ...
          (currProf->ref_channels[kk] > 0)
              ? le_ref_chs[ii]->setText (
                    "Ref:" + QString::number (currProf->ref_channels[kk]))
              : le_ref_chs[ii]->setText ("");
          ref_numbers_list << currProf->ref_channels[kk];
          sb_use_ref_chs[ii]->setValue (currProf->ref_use_channels[kk]);

          qDebug () << "Init APfor::Gen: currProf->ref_use_channels[ kk ] -- "
                    << currProf->ref_use_channels[kk];
          qDebug () << "Init APfor::Gen: ref_numbers_list -- "
                    << ref_numbers_list;

          DbgLv (1)
              << "APGe: inP:    ii kk" << ii << kk << "chann" << sl_chnsel[kk]
              << "lvtol daend dae[kk]" << currProf->lv_tolers[ii]
              << currProf->data_ends[ii] << currProf->data_ends[kk]
              << "currProf->analysis_run[ ii] currProf->analysis_run[ kk ]"
              << currProf->analysis_run[ii] << currProf->analysis_run[kk];

          // ALEXEY: also set info on wvl for edit
          kk = qMin (ii, currProf->wvl_edit.count () - 1);

          qDebug () << "ii kk currProf->wvl_edit[ ii] currProf->wvl_edit[ kk ]"
                    << ii << kk << currProf->wvl_edit[ii]
                    << currProf->wvl_edit[kk];

          QScrollArea *sa = gr_mwvbox[ii];
          foreach (QRadioButton *button, sa->findChildren<QRadioButton *> ())
            {
              int wvl_to_edit
                  = (button->objectName ()).split (":")[2].toInt ();

              qDebug () << "currProf->wvl_edit[ kk ] == wvl_to_edit -- "
                        << currProf->wvl_edit[kk] << " = " << wvl_to_edit;

              if (currProf->wvl_edit[kk]
                  == wvl_to_edit) // <-- NOT FULFILLED ????????
                {
                  button->setChecked (true);
                  button->click ();
                  qDebug () << "US_AnaprofPanGen::initPanel(): wvl_to_edit "
                            << wvl_to_edit << " for channel " << ii;
                  break;
                }
            }
          ...
          qDebug () << "US_AnaprofPanGen::initPanel(): ReportGMP kk, ii "
                       "currProf->chndescs_alt.count() "
                    << kk << ii << currProf->chndescs_alt.count ();

          // QString chdesc_alt = currProf->chndescs_alt[ kk ];
          QString chdesc_alt = chndescs_alt_copy[ii];
          qDebug () << "US_AnaprofPanGen::initPanel(): chdesc_alt "
                    << chdesc_alt;

          // ALEXEY_NEW_REPORT: there will be cycle over wvls:
          //  QList of currProf->ch_wvls[ chdesc_alt ] -> current_wvl:
          //  internal_reports[ chdesc_alt ][ wvl ] = currProf->ch_reports[
          //  chdesc_alt ] [ wvl ];
          ...
          for (int w = 0; w < ch_wavelengths.size (); ++w)
            {
              ...
              internal_reports[chdesc_alt][curr_w]
                  = currProf->ch_reports[chdesc_alt][curr_w];
              ...
              qDebug ()
                  << "US_AnaprofPanGen::initPanel(): ch_wavelengths[ w ] --  "
                  << ch_wavelengths[w];
            }

          // internal_reports[ chdesc_alt ] = currProf->ch_reports[ chdesc_alt
          // ];
          qDebug () << "US_AnaprofPanGen::initPanel(): internal_reports[ "
                       "chdesc_alt ].size() -- "
                    << internal_reports.size ();
          qDebug ()
              << "US_AnaprofPanGen::initPanel(): internal_reports[ chdesc_alt "
                 "][ QString::number ( ch_wavelengths[ 0 ] )] -- "
              << internal_reports[chdesc_alt]
                                 [QString::number (ch_wavelengths[0])]
                                     .tot_conc
              << internal_reports[chdesc_alt]
                                 [QString::number (ch_wavelengths[0])]
          ...
          // //ALEXEY_NEW_REPORT: this .wavelength needs to be properly set for
          // each triple internal_reports[ chdesc_alt ].wavelength =
          // currProf->wvl_edit[ kk ]; qDebug() <<
          // "US_AnaprofPanGen::initPanel(): internal_reports[ chdesc_alt
          // ].wavelength, currProf->wvl_edit.size() -- "
          ...
          // qDebug() << "currProf->wvl_not_run[ ii] currProf->wvl_not_run[ kk
          // ]" << currProf->wvl_not_run[ ii ] << currProf->wvl_not_run[ kk ];
              ...
              if (currProf->wvl_not_run[kk].contains (wvl_not_to_run))
                {
                  ...
                  qDebug () << "US_AnaprofPanGen::initPanel(): wvl_not_to_run "
                            << wvl_not_to_run << " for channel " << ii;
                }
            }
        }
    }
  else
    DbgLv (1) << "APGe: inP:  lcrats count, nchan" << le_lcrats.count ()
              << nchan;
  ...
  // Save to update Gui
  qDebug () << "US_AnaprofPanGen::initPanel(): before save: "
               "currProf->chndescs_alt, size() -- "
            << currProf->chndescs_alt << currProf->chndescs_alt.size ();
  savePanel ();
...
void
US_AnaprofPanGen::set_abde_panel ()
{
...
// Check the Run name
void
US_AnaprofPanGen::check_runname ()
{
#if 0
...
void
US_AnaprofPanGen::update_inv (void)
{
  US_Passwd pw;
...
void
US_AnaprofPanGen::check_user_level ()
{
  // update_inv();
  DbgLv (1) << "APGe:ckul: level" << US_Settings::us_inv_level ();
  //    if ( US_Settings::us_inv_level() < 3 )
...
US_AnaprofPanGen::savePanel ()
{
  ...
  DbgLv (1) << "APGe: svP:  kle cr,ct,dv,vt,de" << le_lcrats.count ()
            << le_lctols.count () << le_ldvols.count () << le_lvtols.count ()
            << le_daends.count () << "nchan" << nchan;

  // if ( currProf->pchans.count() == nchan )       <--- ALEXEY: BUG commented:
  // very important to re-generate GUI (e.g. when Optics chenged by
  // adding/removing Interfrence) Otherwise, with Interfence, no changes are
  // saved from the Geb tab GUI && no changes written to Aprofile DB !!
  { // Reset General channel parameter gui elements
    ...
    // ALEXEY: here -- for replicate groups -- we need to process
    // internal_reports:
    // 1. group channel_descriptions by replicate group #, e.g. QMap<
    // repl_group_number, QStringList ( chdesc_alt ) >;
    //    1a. if repl_group_number == 0 (default), OR QStringList.size() == 1 :
    //    not really a group, so continue / skip;
    // 2. internal_report corresponding to QStringList[ 0 ] - is a primary
    // ReportGMP; make it to be all other reports in a group
        ...
        if (sb_repl_groups[ii]->isEnabled ())
          {
            ...
            currProf->replicates_to_channdesc_main[replicate_group_number]
                << chdesc_alt;
          }
      }
    QMap<int, QStringList>::iterator rgi;
    for (rgi = replicates_to_channdesc.begin ();
         rgi != replicates_to_channdesc.end (); ++rgi)
      {
        ...
        //  internal_reports IS map-of-maps: QMap < chann_desc, QMap<
        //  wvl_number,  US_ReportGMP >>: So, what do we consider the primary
        //  ReportGMP?
        //           -- 1st channel | 1st wavelength ?
        //           -- e.g.: 1st wavelength:
        //                    QList < QString > ch_wavelengths =
        //                    internal_reports[ group_channels[0] ].keys();
        //           -- e.g.: US_ReportGMP primary_report = internal_reports[
        //           group_channels[0] ] [ ch_wavelengths[0] ] ?
        //

        // Go over all reference reports (triple reports in the 1st channel of
        // the replicate group):
        QList<QString> reference_ch_wvls
            = internal_reports[group_channels[0]].keys ();

        for (int rgj = 0; rgj < reference_ch_wvls.size (); ++rgj)
          {
            US_ReportGMP reference_group_report
                = internal_reports[group_channels[0]][reference_ch_wvls[rgj]];
            // Now, go over the reports for other channels in a group
            // (per-triple basis)
            for (int i = 1; i < group_channels.size (); ++i)
              {
                QList<QString> other_group_ch_wvls
                    = internal_reports[group_channels[i]].keys ();

                for (int j = 0; j < other_group_ch_wvls.size (); ++j)
                  {
                    // If other channel's wvl is the same to the reference
                    // channel's wvl, copy Report's params
                    if (other_group_ch_wvls[j] == reference_ch_wvls[rgj])
                      {
                        // QMap of the overlapping replicate group wavelengths
                        // (all overlapping triples):
                        QString ref_triple = group_channels[0].split (":")[0]
                                             + "." + reference_ch_wvls[rgj];
                        QString other_triple = group_channels[i].split (":")[0]
                                               + "." + other_group_ch_wvls[j];

                        channdesc_to_overlapping_wvls[group_channels[0]]
                            << ref_triple << other_triple;
                        currProf->channdesc_to_overlapping_wvls_main
                                [group_channels[0]]
                            << ref_triple << other_triple;

                        // Modify the report with the reference one:
                        US_ReportGMP *other_group_report_to_change
                            = &(internal_reports[group_channels[i]]
                                                [other_group_ch_wvls[j]]);

                        other_group_report_to_change->tot_conc
                            = reference_group_report.tot_conc;
                        other_group_report_to_change->tot_conc_tol
                            = reference_group_report.tot_conc_tol;

                        other_group_report_to_change->rmsd_limit
                            = reference_group_report.rmsd_limit;
                        other_group_report_to_change->av_intensity
                            = reference_group_report.av_intensity;

                        other_group_report_to_change->experiment_duration
                            = reference_group_report.experiment_duration;
                        other_group_report_to_change->experiment_duration_tol
                            = reference_group_report.experiment_duration_tol;

                        other_group_report_to_change->tot_conc_mask
                            = reference_group_report.tot_conc_mask;
                        other_group_report_to_change->rmsd_limit_mask
                            = reference_group_report.rmsd_limit_mask;
                        other_group_report_to_change->av_intensity_mask
                            = reference_group_report.av_intensity_mask;
                        other_group_report_to_change->experiment_duration_mask
                            = reference_group_report.experiment_duration_mask;
                        other_group_report_to_change->integration_results_mask
                            = reference_group_report.integration_results_mask;
                        other_group_report_to_change->plots_mask
                            = reference_group_report.plots_mask;

                        other_group_report_to_change->pseudo3d_2dsait_s_ff0
                            = reference_group_report.pseudo3d_2dsait_s_ff0;
                        other_group_report_to_change->pseudo3d_2dsait_s_d
                            = reference_group_report.pseudo3d_2dsait_s_d;
                        other_group_report_to_change->pseudo3d_2dsait_mw_ff0
                            = reference_group_report.pseudo3d_2dsait_mw_ff0;
                        other_group_report_to_change->pseudo3d_2dsait_mw_d
                            = reference_group_report.pseudo3d_2dsait_mw_d;
                        other_group_report_to_change->pseudo3d_2dsamc_s_ff0
                            = reference_group_report.pseudo3d_2dsamc_s_ff0;
                        other_group_report_to_change->pseudo3d_2dsamc_s_d
                            = reference_group_report.pseudo3d_2dsamc_s_d;
                        other_group_report_to_change->pseudo3d_2dsamc_mw_ff0
                            = reference_group_report.pseudo3d_2dsamc_mw_ff0;
                        other_group_report_to_change->pseudo3d_2dsamc_mw_d
                            = reference_group_report.pseudo3d_2dsamc_mw_d;
                        other_group_report_to_change->pseudo3d_pcsa_s_ff0
                            = reference_group_report.pseudo3d_pcsa_s_ff0;
                        other_group_report_to_change->pseudo3d_pcsa_s_d
                            = reference_group_report.pseudo3d_pcsa_s_d;
                        other_group_report_to_change->pseudo3d_pcsa_mw_ff0
                            = reference_group_report.pseudo3d_pcsa_mw_ff0;
                        other_group_report_to_change->pseudo3d_pcsa_mw_d
                            = reference_group_report.pseudo3d_pcsa_mw_d;

                        other_group_report_to_change->pseudo3d_mask
                            = reference_group_report.pseudo3d_mask;

                        // now, clear rerortItems && fill with reference
                        // ReportGMP:
                        other_group_report_to_change->reportItems.clear ();

                        for (int ic = 0;
                             ic < reference_group_report.reportItems.size ();
                             ++ic)
                          other_group_report_to_change->reportItems.push_back (
                              reference_group_report.reportItems[ic]);
                      }
                  }
              }
          }
    ...
    for (wvl_overlap = channdesc_to_overlapping_wvls.begin ();
         wvl_overlap != channdesc_to_overlapping_wvls.end (); ++wvl_overlap)
      {
        wvl_overlap.value ().removeDuplicates ();
        qDebug () << "Reference Channel: " << wvl_overlap.key ()
                  << ", list of overlapping wvls: " << wvl_overlap.value ();
      }
    // repeat duplicates removal for upper-level
    // currPof->channdesc_to_overlapping_wvls_main:
    for (wvl_overlap = currProf->channdesc_to_overlapping_wvls_main.begin ();
         wvl_overlap != currProf->channdesc_to_overlapping_wvls_main.end ();
         ++wvl_overlap)
      {
        wvl_overlap.value ().removeDuplicates ();
        qDebug () << "Reference Channel MAIN: " << wvl_overlap.key ()
                  << ", list of overlapping wvls: " << wvl_overlap.value ();
      }
    ...
    for (int ii = 0; ii < nchan; ii++)
      {
        ...
        (le_ref_chs[ii]->text ().isEmpty ())
            ? currProf->ref_channels << 0
            : currProf->ref_channels
                  << le_ref_chs[ii]->text ().split (":")[1].toInt ();
        currProf->ref_use_channels << sb_use_ref_chs[ii]->value ();
        qDebug () << "in SavePanel() AProf_Gen: chann, sb_use_ref_chs[ ii "
                     "]->value() -- "
                  << ii << sb_use_ref_chs[ii]->value ();

        // ALEXEY: add additional field for channels to be or not to be
        // analysed
        ...
        qDebug () << "APGR: SAVE: run channel analysis -- " << ii
                  << int (ck_runs[ii]->isChecked ());
        ...
        qDebug () << "APGR: SAVE: run channel report -- " << ii
                  << int (ck_report_runs[ii]->isChecked ());
            ...
            if (button->isChecked ())
              {
                int wvl_to_edit
                    = (button->objectName ()).split (":")[2].toInt ();
                qDebug () << "US_AnaprofPanGen::savePanel(): wvl_to_edit "
                          << wvl_to_edit << " for channel " << ii;
        ...
        QString chdesc_alt = currProf->chndescs_alt[ii];
        qDebug () << "US_AnaprofPanGen::savePanel(): chdesc_alt -- "
                  << chdesc_alt;

        // ALEXEY_NEW_REPORT: will be a cycle over wvl
        //  QMap internal_reports[ chdesc_alt ]:: keys(); --> all channel's
        //  wvls currProf->ch_reports[ chdesc_alt ] [ wvl ] = internal_reports[
        //  chdesc_alt ][ wvl ];
        QList<QString> ch_wavelengths = internal_reports[chdesc_alt].keys ();
        qDebug () << "internal_reports[ chdesc_alt ].keys()   -- "
                  << internal_reports[chdesc_alt].keys ();
        qDebug ()
            << "internal_reports[ chdesc_alt ][ ch_wavelengths[0] ] -- "
        ...
        // //ALEXEY: fill QMap relating chdesc_alt to future reportID or
        // reportGUID currProf->ch_report_ids[ chdesc_alt ]   = 0;
        // currProf->ch_report_guids[ chdesc_alt ] = QString(
        ...
        foreach (QCheckBox *ckbox, sa->findChildren<QCheckBox *> ())
          {
            if (!ckbox->isChecked ())
              {
                qDebug () << "US_AnaprofPanGen::savePanel(): wvl_not_to_run "
                          << (ckbox->objectName ()).split (":")[2]
                          << " for channel " << ii;
...
QString
US_AnaprofPanGen::getSValue (const QString type)
{
  QString value ("");

  if (type == "aprofname")
    {
      value = currProf->aprofname;
    }
  else if (type == "protoname")
    {
      value = currProf->protoname;
    }
  else if (type == "aprofGUID")
    {
      value = currProf->aprofGUID;
    }
  else if (type == "protoGUID")
    {
      value = currProf->protoGUID;
    }
...
int
US_AnaprofPanGen::getIValue (const QString type)
{
  int value = 0;
  if (type == "dbdisk")
    {
  ...
  else if (type == "aprofID")
    {
      value = currProf->aprofID;
    }
  else if (type == "protoID")
    {
      value = currProf->protoID;
    }
  else if (type == "nchan")
    {
      value = sl_chnsel.count ();
    }
  return value;
}
...
double
US_AnaprofPanGen::getDValue (const QString type)
{
  double value = 0.0;
...
QStringList
US_AnaprofPanGen::getLValue (const QString type)
{
  QStringList value ("");
  int nchan = currProf->lc_ratios.count ();
  ...
  if (type == "channels")
  ...
  else if (type == "analysisrun")
    {
      ...
      for (int ii = 0; ii < sl_chnsel.count (); ii++)
        {
          qDebug () << "Ana RUN for channel " << ii
                    << " is: " << currProf->analysis_run[ii];
  ...
  else if (type == "wvledit")
    {
      ...
      for (int ii = 0; ii < sl_chnsel.count (); ii++)
        {
          qDebug () << "WVL EDIT for channel " << ii
                    << " is: " << currProf->wvl_edit[ii];
...
QString
US_AnaprofPanGen::sibSValue (const QString sibling, const QString type)
{
...
int
US_AnaprofPanGen::sibIValue (const QString sibling, const QString type)
{
...
double
US_AnaprofPanGen::sibDValue (const QString sibling, const QString type)
{
...
QStringList
US_AnaprofPanGen::sibLValue (const QString sibling, const QString type)
{
...
int
US_AnaprofPanGen::status ()
{
  bool is_done
      = !(currProf->aprofname.isEmpty () || currProf->protoname.isEmpty ());
  return (is_done ? 1 : 0);
}
...
void
US_AnaprofPan2DSA::initPanel ()
{
  ap2DSA = &(mainw->currProf.ap2DSA);
  // ap2DSA             = &(mainw->currProf_copy.ap2DSA);
  qDebug () << "INIT 2DSA-begin: ap2DSA.parm SIZE -- "
            << ap2DSA->parms.size ();

  // Debug
  for (int j = 0; j < ap2DSA->parms.size (); j++)
    qDebug () << "Channels in COPY 2DCA parms vector --  "
              << ap2DSA->parms[j].channel;
  ...
  */
  ...
  QStandardItemModel *model
      = qobject_cast<QStandardItemModel *> (cb_chnsel->model ());
  for (int i = 0; i < chnls_to_run.size (); i++)
    {
      ...
      for ( int j = 0; j < ap2DSA->parms.size(); j++ )
        {
          if ( ap2DSA->parms[ j ].channel == chnls_to_remove[ i ] )
            {
              qDebug() << "2DSA parms for channel -- " << ap2DSA->parms[ j
  ].channel << " will be removed!"; ap2DSA->parms.remove( j ); break;
            }
        }
    }
  ...
  */
  ...
  for (int ii = kparm; ii < kchan; ii++)
    {
      ...
      DbgLv (1) << "AP2d:inP:    set-parm ii" << ii << "channel"
                << parm1.channel;
      ap2DSA->parms << parm1;
    }
  ...
  qDebug () << "In 2DSA's initPanel(): ap2DSA->job3auto -- "
            << ap2DSA->job3auto;
...
US_AnaprofPan2DSA::savePanel ()
{
  ...
  DbgLv (1) << "AP2d:svP:   runs:" << ap2DSA->job1run << ap2DSA->job2run
            << ap2DSA->job3run << ap2DSA->job4run << ap2DSA->job5run;

  // ALEXEY: here, process replicate groups defined in Gen tab:
  // 1. Identify primary channel & its parameters;
  // 2. Copy (replace) other's replicate group channel's parms with these
  // reference parameters

  US_AnaProfile *currProf = &mainw->currProf;
  QMap<int, QStringList> replicates_to_channdesc
      = currProf->replicates_to_channdesc_main;
  ...
  for (rgi = replicates_to_channdesc.begin ();
       rgi != replicates_to_channdesc.end (); ++rgi)
    {
      ...
      qDebug () << "Reference 2DSA parms for channel " << group_channels[0]
                << reference_2dsa_parms.channel;
...
QString
US_AnaprofPan2DSA::getSValue (const QString type)
{
  QString value ("");
...
// Get a specific panel integer value
int
US_AnaprofPan2DSA::getIValue (const QString type)
{
  int value = 0;
  if (type == "nchan")
    {
...
// Get a specific panel double value
double
US_AnaprofPan2DSA::getDValue (const QString type)
{
  double value = 0;
  if (type == "dbdisk")
    {
...
// Get specific panel list values
QStringList
US_AnaprofPan2DSA::getLValue (const QString type)
{
  QStringList value ("");
...
QString
US_AnaprofPan2DSA::sibSValue (const QString sibling, const QString type)
{
  return mainw->childSValue (sibling, type);
}
...
// Get a specific panel integer value from a sibling panel
int
US_AnaprofPan2DSA::sibIValue (const QString sibling, const QString type)
{
  return mainw->childIValue (sibling, type);
}
...
// Get a specific panel double value from a sibling panel
double
US_AnaprofPan2DSA::sibDValue (const QString sibling, const QString type)
{
  return mainw->childDValue (sibling, type);
}
...
// Get a specific panel list from a sibling panel
QStringList
US_AnaprofPan2DSA::sibLValue (const QString sibling, const QString type)
{
  return mainw->childLValue (sibling, type);
}
...
void
US_AnaprofPanPCSA::initPanel ()
{
  apPCSA = &(mainw->currProf.apPCSA);
  // apPCSA             = &(mainw->currProf_copy.apPCSA);
  qDebug () << "INIT PCSA-begin: apPCSA.parm SIZE -- "
            << apPCSA->parms.size ();
  // Debug
  for (int j = 0; j < apPCSA->parms.size (); j++)
    qDebug () << "Channels in COPY PSCA parms vector --  "
              << apPCSA->parms[j].channel;
  ...
  */
  ...
  // Populate GUI settings from protocol controls
  DbgLv (1) << "APpc:inP: prb: nchan" << apPCSA->nchan << "job_run"
            << apPCSA->job_run;
      ...
      for ( int j = 0; j < apPCSA->parms.size(); j++ )
        {
          if ( apPCSA->parms[ j ].channel == chnls_to_remove[ i ] )
            {
              qDebug() << "PCSA parms for channel -- " << apPCSA->parms[ j
  ].channel << " will be removed!"; apPCSA->parms.remove( j ); break;
            }
        }
    }
  ...
  for ( int j = 0; j < apPCSA->parms.size(); j++ )
    {
      qDebug() << "Channels in NEW PSCA parms vector --  " <<  apPCSA->parms[ j
  ].channel;
    }
  //End Debug
  ...
  */
  ...
  int kchan = sl_chnsel.size ();
  DbgLv (1) << "APpc:inP:   kparm kchan" << kparm << kchan << "  job_run"
            << apPCSA->job_run;
  ...
  for (int ii = kparm; ii < kchan; ii++)
    {
      parm1.channel = sl_chnsel[ii];
      DbgLv (1) << "APpc:inP:     set-parm ii" << ii << "channel"
                << parm1.channel;
      apPCSA->parms << parm1;
    }
...
US_AnaprofPanPCSA::savePanel ()
{
  ...
  if (syntax_errors != 0)
    {
      ...
      QString message_error
          = QString (tr ("There are %1 syntax errors in PCSA settings!"
                         "\n\nThe program will return to PCSA settings to fix "
                         "the errors (red fields) before proceeding..."))
  ...
  DbgLv (1) << "APpc:svP: nparm" << nparm << "cchx" << cchx << "  job_run"
            << apPCSA->job_run;
  ...
  for (rgi = replicates_to_channdesc.begin ();
       rgi != replicates_to_channdesc.end (); ++rgi)
    {
      ...
      qDebug () << "Reference PCSA parms for channel " << group_channels[0]
                << reference_pcsa_parms.channel;
      for (int ii = 1; ii < group_channels.size (); ++ii)
        {
          for (int jj = 0; jj < nparm; ++jj)
            {
...
// Get a specific panel value
QString
US_AnaprofPanPCSA::getSValue (const QString type)
{
...
// Get a specific panel integer value
int
US_AnaprofPanPCSA::getIValue (const QString type)
{
  int value = 0;
      ...
      for (int ii = 0; ii < apPCSA->nchan; ii++)
        {
          ...
          //         if ( chx > 0 )
          //         {
          //            int ncchn      = QString( centp ).left( chx ).section(
          //            " ", -1, -1 )
          //                                             .simplified().toInt();
          //            nchans        += ncchn;
          //         }
...
// Get a specific panel double value
double
US_AnaprofPanPCSA::getDValue (const QString type)
{
  int nchan = apPCSA->nchan;
  double value = 0;
...
// Get specific panel list values
QStringList
US_AnaprofPanPCSA::getLValue (const QString type)
{
  QStringList value;
...
// Get a specific panel value from a sibling panel
QString
US_AnaprofPanPCSA::sibSValue (const QString sibling, const QString type)
{
  return mainw->childSValue (sibling, type);
}
...
// Get a specific panel integer value from a sibling panel
int
US_AnaprofPanPCSA::sibIValue (const QString sibling, const QString type)
{
  return mainw->childIValue (sibling, type);
}
...
// Get a specific panel double value from a sibling panel
double
US_AnaprofPanPCSA::sibDValue (const QString sibling, const QString type)
{
  return mainw->childDValue (sibling, type);
}
...
// Get a specific panel list from a sibling panel
QStringList
US_AnaprofPanPCSA::sibLValue (const QString sibling, const QString type)
{
  return mainw->childLValue (sibling, type);
}
