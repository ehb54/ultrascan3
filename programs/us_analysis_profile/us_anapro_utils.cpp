...
QString US_AnalysisProfileGui::childSValue(const QString child,
                                           const QString type) {
  QString value("");
  if (child == "general") {
...
int US_AnalysisProfileGui::childIValue(const QString child,
                                       const QString type) {
  int value = 0;
  if (child == "general") {
...
double US_AnalysisProfileGui::childDValue(const QString child,
                                          const QString type) {
  double value = 0.0;
  if (child == "general") {
...
QStringList US_AnalysisProfileGui::childLValue(const QString child,
                                               const QString type) {
  QStringList value;
...
void US_AnalysisProfileGui::statUpdate() {
  ...
  statflag = apanGeneral->status() + apan2DSA->status() + apanPCSA->status();
...
use_db=false;
  ...
  for (ri = currProf->ch_reports.begin(); ri != currProf->ch_reports.end();
    ...
    if (chan_desc.contains("(unspecified)")) {
      ...
      qDebug() << "Reference report name, wvls, #wvl -- " << chan_desc
               << triple_reports_ref.keys() << triple_reports_ref.keys().size();
      continue;
    }
    ...
    for (tri = triple_reports.begin(); tri != triple_reports.end(); ++tri) {
      ...
      // if ref_report was not replaced yet, substitute all channels' reports
      // witht he ref. one...
        ...
        if (mainw->abde_mode_aprofile) {
          for (int ii = 0;
            ...
            if (initItem.type == 's' && initItem.method.contains("2DSA")) {
              // if attempts to insert reportItem from Velocity-type, replace
              // with ABDE-type
              currProf->ch_reports[chan_desc][c_wvl].reportItems[ii].type =
                  QString("Radius");
              currProf->ch_reports[chan_desc][c_wvl].reportItems[ii].method =
                  QString("raw");
              currProf->ch_reports[chan_desc][c_wvl].reportItems[ii].range_low =
                  5.8;
      ...
      for (int ii = 0;
        ...
        qDebug() << "wvl, type, method, lo, hi -- " << c_wvl << initItem.type
                 << initItem.method << initItem.range_low
                 << initItem.range_high;
      }
    }
  }
  ...
  for (wi = currProf->ch_wvls.begin(); wi != currProf->ch_wvls.end(); ++wi) {
    ...
    qDebug() << "[BEGIN]US_AnaprofPanGen::initPanel(): chan_desc [ch_wvls]:::  "
             << chan_desc;
  ...
  for (int ii = 0; ii < nchan; ii++) {
    ...
    QString chname = QString(chdesc).section(":", 0, 0).section(",", 0, 0);
  ...
  for (int ii = 0; ii < nchan; ii++) {
    ...
    if (chann_desc.contains("B:Interf")) continue;
  ...
  if (le_lcrats.count() ==
      nchan) {  // Reset General channel parameter gui elements
    for (int ii = 0; ii < nchan; ii++) {
      ...
      qDebug() << "scan_beg, scan_end for -- " << currProf->chndescs_alt[ii]
               << currProf->scan_excl_begin[ii] << currProf->scan_excl_end[ii];
      ...
      (currProf->ref_channels[kk] > 0)
          ? le_ref_chs[ii]->setText("Ref:" +
                                    QString::number(currProf->ref_channels[kk]))
      ...
      DbgLv(1) << "APGe: inP:    ii kk" << ii << kk << "chann" << sl_chnsel[kk]
      ...
      qDebug() << "ii kk currProf->wvl_edit[ ii] currProf->wvl_edit[ kk ]" << ii
               << kk << currProf->wvl_edit[ii] << currProf->wvl_edit[kk];
      ...
      for (int w = 0; w < ch_wavelengths.size(); ++w) {
        ...
        qDebug() << "US_AnaprofPanGen::initPanel(): ch_wavelengths[ w ] --  "
                 << ch_wavelengths[w];
      }

      // internal_reports[ chdesc_alt ] = currProf->ch_reports[ chdesc_alt ];
      qDebug() << "US_AnaprofPanGen::initPanel(): internal_reports[ chdesc_alt "
                  "].size() -- "
               << internal_reports.size();
      qDebug()
          << "US_AnaprofPanGen::initPanel(): internal_reports[ chdesc_alt ][ "
             "QString::number ( ch_wavelengths[ 0 ] )] -- "
          << internal_reports[chdesc_alt][QString::number(ch_wavelengths[0])]
                 .tot_conc
          << internal_reports[chdesc_alt][QString::number(ch_wavelengths[0])]
                 .wavelength;

      // //ALEXEY_NEW_REPORT: this .wavelength needs to be properly set for each
      // triple internal_reports[ chdesc_alt ].wavelength = currProf->wvl_edit[
      // kk ]; qDebug() << "US_AnaprofPanGen::initPanel(): internal_reports[
      // chdesc_alt ].wavelength, currProf->wvl_edit.size() -- "
  ...
  if (mainw->abde_mode_aprofile) {
    ...
    for (int rr = 0; rr < currProf->report_run.size(); ++rr)
      if (currProf->report_run[rr] == 0) all_reps_run = false;
...
void US_AnaprofPanGen::savePanel() {
  ...
  {  // Reset General channel parameter gui elements
    ...
    for (rgi = replicates_to_channdesc.begin();
      ...
      if (group_number == 0 || group_channels.size() == 1) continue;

      // do copying US_ReportGMPs here:
      //  internal_reports IS map-of-maps: QMap < chann_desc, QMap< wvl_number,
      //  US_ReportGMP >>: So, what do we consider the primary ReportGMP?
      //           -- 1st channel | 1st wavelength ?
      //           -- e.g.: 1st wavelength:
      //                    QList < QString > ch_wavelengths = internal_reports[
      //                    group_channels[0] ].keys();
      //           -- e.g.: US_ReportGMP primary_report = internal_reports[
      //           group_channels[0] ] [ ch_wavelengths[0] ] ?
      //

      // Go over all reference reports (triple reports in the 1st channel of the
      // replicate group):
      ...
      for (int rgj = 0; rgj < reference_ch_wvls.size(); ++rgj) {
        ...
        // Now, go over the reports for other channels in a group (per-triple
        // basis)
        for (int i = 1; i < group_channels.size(); ++i) {
          ...
          for (int j = 0; j < other_group_ch_wvls.size(); ++j) {
            // If other channel's wvl is the same to the reference channel's
            // wvl, copy Report's params
            if (other_group_ch_wvls[j] == reference_ch_wvls[rgj]) {
              // QMap of the overlapping replicate group wavelengths (all
              // overlapping triples):
              ...
              channdesc_to_overlapping_wvls[group_channels[0]] << ref_triple
                                                               << other_triple;
              currProf->channdesc_to_overlapping_wvls_main[group_channels[0]]
                  << ref_triple << other_triple;

              // Modify the report with the reference one:
              US_ReportGMP* other_group_report_to_change = &(
                  internal_reports[group_channels[i]][other_group_ch_wvls[j]]);
              ...
              for (int ic = 0; ic < reference_group_report.reportItems.size();
                   ++ic)
    ...
    for (int ii = 0; ii < nchan; ii++) {
      ...
      //  currProf->ch_reports[ chdesc_alt ] [ wvl ] = internal_reports[
      //  chdesc_alt ][ wvl ];
      ...
      foreach (QCheckBox* ckbox, sa->findChildren<QCheckBox*>()) {
        if (!ckbox->isChecked()) {
          qDebug() << "US_AnaprofPanGen::savePanel(): wvl_not_to_run "
                   << (ckbox->objectName()).split(":")[2] << " for channel "
                   << ii;
...
QStringList US_AnaprofPanGen::sibLValue(const QString sibling,
                                        const QString type) {
...
void US_AnaprofPan2DSA::initPanel() {
  ...
  */
  ...
  int first_avail = 0;
  if (active_items_2dsa.size()) first_avail = active_items_2dsa[0];
...
QString US_AnaprofPan2DSA::sibSValue(const QString sibling,
                                     const QString type) {
  return mainw->childSValue(sibling, type);
}
...
// Get a specific panel list from a sibling panel
QStringList US_AnaprofPan2DSA::sibLValue(const QString sibling,
                                         const QString type) {
  return mainw->childLValue(sibling, type);
}
...
void US_AnaprofPanPCSA::initPanel() {
  ...
  */
  ...
  int first_avail = 0;
  if (active_items_pcsa.size()) first_avail = active_items_pcsa[0];
  ...
  if (syntax_errors != 0) {
    ...
    QString message_error =
        QString(tr("There are %1 syntax errors in PCSA settings!"
                   "\n\nThe program will return to PCSA settings to fix the "
                   "errors (red fields) before proceeding..."))
...
QStringList US_AnaprofPanPCSA::getLValue(const QString type) {
  ...
  if (type == "channels") {
    value = sl_chnsel;
  } else if (type == "centerpieces") {  // Return row strings where a
...
// Get a specific panel value from a sibling panel
QString US_AnaprofPanPCSA::sibSValue(const QString sibling,
                                     const QString type) {
  return mainw->childSValue(sibling, type);
}
...
// Get a specific panel list from a sibling panel
QStringList US_AnaprofPanPCSA::sibLValue(const QString sibling,
                                         const QString type) {
  return mainw->childLValue(sibling, type);
}
