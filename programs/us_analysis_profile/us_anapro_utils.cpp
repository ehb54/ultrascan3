...
void US_AnalysisProfileGui::enable_tabs_buttons_readonly(void) {
   ...
   for (int ii = 0; ii < tabWidget->count(); ii++) {
      ...
      for (int jj = 0; jj < allPButtons.count(); jj++) {
         if (
            (allPButtons[ jj ]->text()).contains("View Solution Details")
...
int US_AnalysisProfileGui::getProfiles(QStringList &par1, QList<QStringList> &par2) {
...
use_db=false;
   ...
   for (ri = currProf->ch_reports.begin(); ri != currProf->ch_reports.end(); ++ri) {
      ...
      if (chan_desc.contains("(unspecified)")) {
         ...
         qDebug() << "Reference report name, wvls, #wvl -- " << chan_desc << triple_reports_ref.keys()
      ...
      for (tri = triple_reports.begin(); tri != triple_reports.end(); ++tri) {
         ...
         for (int ii = 0; ii < currProf->ch_reports[ chan_desc ][ c_wvl ].reportItems.size(); ++ii) {
            ...
            qDebug() << "wvl, type, method, lo, hi -- " << c_wvl << initItem.type << initItem.method
   ...
   for (int ii = 0; ii < nchan; ii++) {
      ...
      if (
         (chopts.contains(tr("(unspecified)"))) || (chopts.contains(tr("(not installed)")))
      ...
      sl_chnsel << chnsel;
      DbgLv(1) << "APGe: inP:  ch" << ii << "chdesc" << chdesc << "chnsel" << chnsel;
   }
   ...
   DbgLv(1) << "APGe: inP: 1)le_chn,lcr size" << le_channs.count() << le_lcrats.count() << "nchan_gui" << nchan;

   //Clear internal_reports QMap
   internal_reports.clear();
   ref_numbers_list.clear();

   if (le_lcrats.count() == nchan) { // Reset General channel parameter gui elements
      for (int ii = 0; ii < nchan; ii++) {
         ...
         qDebug() << "scan_beg, scan_end for -- " << currProf->chndescs_alt[ ii ] << currProf->scan_excl_begin[ ii ]
         ...
         DbgLv(1) << "APGe: inP:    ii kk" << ii << kk << "chann" << sl_chnsel[ kk ] << "lvtol daend dae[kk]"
                  ...
                  << "currProf->analysis_run[ ii] currProf->analysis_run[ kk ]" << currProf->analysis_run[ ii ]
                  << currProf->analysis_run[ kk ];


         //ALEXEY: also set info on wvl for edit
         kk = qMin(ii, currProf->wvl_edit.count() - 1);

         qDebug() << "ii kk currProf->wvl_edit[ ii] currProf->wvl_edit[ kk ]" << ii << kk << currProf->wvl_edit[ ii ]
         ...
         foreach (QRadioButton *button, sa->findChildren<QRadioButton *>()) {
            ...
            qDebug() << "currProf->wvl_edit[ kk ] == wvl_to_edit -- " << currProf->wvl_edit[ kk ] << " = "
                     << wvl_to_edit;
         ...
         qDebug() << "US_AnaprofPanGen::initPanel(): ReportGMP kk, ii currProf->chndescs_alt.count() " << kk << ii
         ...
         qDebug() << "US_AnaprofPanGen::initPanel(): internal_reports[ chdesc_alt ].size() -- "
                  << internal_reports.size();
         qDebug() << "US_AnaprofPanGen::initPanel(): internal_reports[ chdesc_alt ][ QString::number ( ch_wavelengths[ "
                     "0 ] )] -- "
   ...
   // Save to update Gui
   qDebug() << "US_AnaprofPanGen::initPanel(): before save: currProf->chndescs_alt, size() -- "
...
void US_AnaprofPanGen::savePanel() {
   ...
   DbgLv(1) << "APGe: svP:  kle cr,ct,dv,vt,de" << le_lcrats.count() << le_lctols.count() << le_ldvols.count()
   ...
   { // Reset General channel parameter gui elements
      ...
      for (wvl_overlap = channdesc_to_overlapping_wvls.begin(); wvl_overlap != channdesc_to_overlapping_wvls.end();
           ++wvl_overlap) {
      ...
      for (wvl_overlap = currProf->channdesc_to_overlapping_wvls_main.begin();
           wvl_overlap != currProf->channdesc_to_overlapping_wvls_main.end(); ++wvl_overlap) {
         wvl_overlap.value().removeDuplicates();
      ...
      for (int ii = 0; ii < nchan; ii++) {
         ...
         (le_ref_chs[ ii ]->text().isEmpty())
            ...
            : currProf->ref_channels << le_ref_chs[ ii ]->text().split(":")[ 1 ].toInt();
         currProf->ref_use_channels << sb_use_ref_chs[ ii ]->value();
         qDebug() << "in SavePanel() AProf_Gen: chann, sb_use_ref_chs[ ii ]->value() -- " << ii
                  << sb_use_ref_chs[ ii ]->value();
         ...
         foreach (QCheckBox *ckbox, sa->findChildren<QCheckBox *>()) {
            if (!ckbox->isChecked()) {
               qDebug() << "US_AnaprofPanGen::savePanel(): wvl_not_to_run " << (ckbox->objectName()).split(":")[ 2 ]
                        << " for channel " << ii;
...
int US_AnaprofPanGen::status() {
   bool is_done = !(currProf->aprofname.isEmpty() || currProf->protoname.isEmpty());
   return (is_done ? 1 : 0);
}
...
void US_AnaprofPan2DSA::savePanel() {
   ...
   DbgLv(1) << "AP2d:svP:   runs:" << ap2DSA->job1run << ap2DSA->job2run << ap2DSA->job3run << ap2DSA->job4run
            << ap2DSA->job5run;
   ...
   for (rgi = replicates_to_channdesc.begin(); rgi != replicates_to_channdesc.end(); ++rgi) {
      ...
      qDebug() << "In 2DSA's savePanel(): repl. number, channels -- " << group_number << group_channels;
...
void US_AnaprofPanPCSA::initPanel() {
   ...
   */
   ...
   int kchan = sl_chnsel.size();
   DbgLv(1) << "APpc:inP:   kparm kchan" << kparm << kchan << "  job_run" << apPCSA->job_run;
...
void US_AnaprofPanPCSA::savePanel() {
   ...
   if (syntax_errors != 0) {
      ...
      QString message_error
         = QString(
              tr("There are %1 syntax errors in PCSA settings!"
   ...
   DbgLv(1) << "APpc:svP: nparm" << nparm << "cchx" << cchx << "  job_run" << apPCSA->job_run;
   ...
   for (rgi = replicates_to_channdesc.begin(); rgi != replicates_to_channdesc.end(); ++rgi) {
      ...
      qDebug() << "In PCSA's savePanel(): repl. number, channels -- " << group_number << group_channels;
...
QStringList US_AnaprofPanPCSA::getLValue(const QString type) {
   ...
   else if (type == "centerpieces") { // Return row strings where a centerpiece is selected
   }
