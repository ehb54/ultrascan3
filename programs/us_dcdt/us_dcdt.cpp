...
US_Dcdt::US_Dcdt() : US_AnalysisBase2() {
   ...
   connect(ct_sValue, SIGNAL(valueChanged(double)), SLOT(sMaxChanged(double)));
   ...
   connect(this, SIGNAL(dataAreLoaded(void)), this, SLOT(smooth10(void)));
   qApp->processEvents();
}
...
void US_Dcdt::data_plot(void) {
   ...
   while (excludedScans.contains(skipped))
      skipped++;
   ...
   for (int i = skipped + 1; i < scanCount; i++) {
      ...
      for (int j = 0; j < points; j++) {
         ...
         sValues[ count ][ size ] = solution.s20w_correction * 1.0e13 * log(radius / meniscus)
   ...
   for (int j = 0; j < arrayLength; j++) {
      ...
      for (int i = 0; i < count; i++) {
         ...
         while (sValues[ i ][ k ] < avgS[ j ]) {
            k++;
            if (k >= arraySizes[ i ])
               goto next; // Skip rest of scans
         }

         // Interpolate and apply y = mx + b
         double m = (dcdt[ i ][ k ] - dcdt[ i ][ k - 1 ]) / (sValues[ i ][ k ] - sValues[ i ][ k - 1 ]);
   ...
   data_plot1->setTitle(
      tr("Time Derivative (dC/dt)\n") + tr("Run ") + d->runID + tr(": Cell ") + d->cell + " (" + d->wavelength
      + tr(" nm)"));
   ...
   switch (graphType) {
      case 0: // Radius Plot
         ...
         for (int i = 0; i < count; i++) {
            ...
            curve = us_curve(data_plot1, tr("Scan ") + QString::number(i + 1));
      ...
      case 1: // S-value plot
         for (int i = 0; i < count; i++) {
            ...
            curve->setSamples(sValues[ i ].data(), dcdt[ i ].data(), arraySizes[ i ]);
         }
      ...
      case 2: // Average dC/dt or g*(s) plot
         data_plot1->setAxisTitle(QwtPlot::yLeft, tr("Average g<sup>*</sup>(s)"));
...
void US_Dcdt::write_report(QTextStream &ts) {
   ...
   for (int ii = 0; ii < arrayLength; ii++) { // Find maximum Y and its index
   ...
   for (int ii = 0; ii < arrayLength; ii++) { // Find the start and end of zone where Y is 5% of maximum or more
      double avgv = avgDcdt[ ii ];
   ...
   QString xastr = table_row(tr("Average S at Maximum dCdt:"), QString::number(avgS[ asmxx ], 'f', 3));
   xastr += table_row(tr("Average S at interest zone Start:"), QString::number(avgS[ aslox ], 'f', 3));
   xastr += table_row(tr("Average S at interest zone End:"), QString::number(avgS[ ashix ], 'f', 3));
   xastr += table_row(tr("Interest zone percent of maximum:"), QString("5% and above"));
...
void US_Dcdt::save(void) {
   ...
   QString filebase = dir + "/" + d->runID + "/dcdt." + QString(triples.at(index)).replace(" / ", "") + ".";
   ...
   QString dsinfFile = QString(filebase).replace("/dcdt.", "/dsinfo.") + "dataset_info.html";
   ...
   if (disk_controls->db()) { // Write report files also to the database
