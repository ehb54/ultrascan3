...
void US_Dcdt::smooth10(void) { ct_smoothing->setValue(10); }
...
void US_Dcdt::sMaxChanged(double /* value */) { data_plot(); }
...
void US_Dcdt::data_plot(void) {
  ...
  for (int i = skipped + 1; i < scanCount; i++) {
    ...
    for (int j = 0; j < points; j++) {
      ...
      sValues[count][size] = solution.s20w_correction * 1.0e13 *
                             log(radius / meniscus) /
  ...
  for (int i = 0; i < arrayLength; i++) avgS[i] = s_min + i * increment;
  ...
  for (int j = 0; j < arrayLength; j++) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      double m =
          (dcdt[i][k] - dcdt[i][k - 1]) / (sValues[i][k] - sValues[i][k - 1]);
  ...
  QwtPlotCurve* curve;
  QwtText title =
      QwtText(tr("<b>Sedimentation Coefficient x "
                 "10<sup>13</sup> sec</b>"));
  ...
  switch (graphType) {
    case 0:  // Radius Plot
      ...
      for (int i = 0; i < count; i++) {
        ...
        for (int j = arrayStart[i]; j < end; j++) x[size++] = d->xvalues[j];
...
void US_Dcdt::write_report(QTextStream& ts) {
  ...
  for (int ii = 0; ii < arrayLength;
       ii++) {  // Find the start and end of zone where Y is 5% of maximum or
                // more
    double avgv = avgDcdt[ii];
...
void US_Dcdt::save(void) {
  ...
  QString dsinfFile =
      QString(filebase).replace("/dcdt.", "/dsinfo.") + "dataset_info.html";
