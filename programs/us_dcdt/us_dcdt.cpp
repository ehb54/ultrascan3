...
void
US_Dcdt::smooth10 (void)
{
  ct_smoothing->setValue (10);
}

void
US_Dcdt::exclude (void)
{
  if (!dataLoaded)
    return;
...
void
US_Dcdt::sMaxChanged (double /* value */)
{
  data_plot ();
}

void
US_Dcdt::set_graph (int button)
{
  if (graphType == button)
    return;
      ...
      for (int i = 0; i < count; i++)
        {
          ...
          curve
              = us_curve (data_plot1, tr ("Scan ") + QString::number (i + 1));
...
void
US_Dcdt::write_report (QTextStream &ts)
{
  int index = lw_triples->currentRow ();
