...
US_HelpDaemon::US_HelpDaemon (const QString &page, QObject *o) : QObject (o)
{
  ...
  args << QLatin1String ("-collectionFile") << location
       << QLatin1String ("-enableRemoteControl") << QLatin1String ("-showURL")
       << url;
...
void
US_HelpDaemon::close (int /*exitCode*/, QProcess::ExitStatus /*status*/)
{
  exit (0);
}

void
US_HelpDaemon::show (const QString &helpPage)
{
  debug ("IN show()\n");
  if (helpPage == "Quit")
    {
...
void
US_HelpDaemon::debug (const QString &message)
{
#ifdef Q_OS_WIN
