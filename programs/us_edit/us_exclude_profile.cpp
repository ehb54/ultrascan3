...
US_ExcludeProfile::US_ExcludeProfile(QList<int> includes) : US_WidgetsDialog(0, 0) {
   original = includes;
   finished = false;
   current.clear();
   ...
   connect(ct_start, SIGNAL(valueChanged(double)), SLOT(update_start(double)));
   ...
   connect(ct_stop, SIGNAL(valueChanged(double)), SLOT(update_stop(double)));
   ...
   if (finished) {
      if (remaining < 10) {
         int result = QMessageBox::question(
            this, tr("Exclude Error"), tr("Your profile should include\nat least 10 scans..."), QMessageBox::Cancel,
            QMessageBox::Ok);
