    ...
    : US_WidgetsDialog (0, 0)
{
  ...
  connect (ct_nth, SIGNAL (valueChanged (double)), SLOT (update (double)));
  ...
  le_remaining
      = us_lineedit (QString::number (scanCount) + tr (" scans"), -1, true);
...
void
US_ExcludeProfile::update_start (double v)
{
  if (ct_stop->value () < v)
    {
      ct_stop->disconnect ();
...
void
US_ExcludeProfile::update_stop (double v)
{
  if (ct_start->value () > v)
    {
      ct_start->disconnect ();
...
void
US_ExcludeProfile::apply (void)
{
  DbgLv (1) << "APL: origsize exclsize" << original.size ()
            << excludes.size ();
  current = excludes;
  ct_nth->setValue (1.0);
...
void
US_ExcludeProfile::update (double /* unused */)
{
  ...
  if (exclCount > 0)
    ...
    {
      for (int ii = 0; ii < original[scanCount - 1]; ii++)
        if (original.contains (ii))
          scnincl << ii;
    }

  stop = qMin (stop, scnincl.size ());
  DbgLv (1) << "UPD:  scninclsz" << scnincl.size () << "startstop" << start
            << stop;
...
void
US_ExcludeProfile::terminate (void)
{
