...
US_EditScan::US_EditScan(US_DataIO::Scan &s, const QVector<double> &r, double invertValue, double left, double right) :
    US_WidgetsDialog(0, 0), originalScan(s), allRadii(r), invert(invertValue), range_left(left), range_right(right) {
   dragging = false;
   ...
   // Plot Rows
   QBoxLayout *plot = new US_Plot(data_plot, tr("Edit Single Scan"), tr("Radius (cm)"), tr("Absorbance"));
   ...
   te_instructions->setText(tr("1. Use the zoom function as needed to focus on a point.\n"
   ...
   connect(pick, SIGNAL(cMouseDownRaw(QMouseEvent *)), SLOT(start_drag(QMouseEvent *)));

   connect(pick, SIGNAL(cMouseUp(const QwtDoublePoint &)), SLOT(end_drag(const QwtDoublePoint &)));

   connect(pick, SIGNAL(cMouseDrag(const QwtDoublePoint &)), SLOT(drag(const QwtDoublePoint &)));
}
