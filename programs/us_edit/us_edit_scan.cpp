...
US_EditScan::US_EditScan (US_DataIO::Scan &s, const QVector<double> &r,
                          double invertValue, double left, double right)
    : US_WidgetsDialog (0, 0), originalScan (s), allRadii (r),
      invert (invertValue), range_left (left), range_right (right)
{
  dragging = false;
  ...
  // Plot Rows
  QBoxLayout *plot = new US_Plot (data_plot, tr ("Edit Single Scan"),
...
void
US_EditScan::drag (const QwtDoublePoint &p)
{
...
void
US_EditScan::start_drag (QMouseEvent *e)
{
  dragging = true;
...
void
US_EditScan::end_drag (const QwtDoublePoint &p)
{
  dragging = false;
...
// qDebug() << " end_drag() point" << point << "values[point]" <<
// values[point];
#if QT_VERSION > 0x050000
  curve->setSamples (radii, values, count);
#endif
