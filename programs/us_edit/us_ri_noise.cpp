...
US_RiNoise::US_RiNoise(
   const US_DataIO::RawData &raw, const QList<int> &Includes, double left, double right, const QString &dataType,
   int &initial_order, QList<double> &r) :
    US_WidgetsDialog(0, 0), data(raw), includes(Includes), range_left(left), range_right(right), order(initial_order),
    residuals(r) {
   ...
   connect(ct_order, SIGNAL(valueChanged(double)), SLOT(draw_fit(double)));
   ...
   QBoxLayout *plot = new US_Plot(data_plot, tr("RI Noise Fit"), tr("Scan Time (seconds)"), y_axis);
...
void US_RiNoise::draw_fit(double new_order) {
   ...
   for (int i = 0; i < scan_count; i++) {
      ...
      double delta_r = (range_right - range_left) / (indexRight - indexLeft);
...
void US_RiNoise::calc_residuals(
   const US_DataIO::RawData &data, const QList<int> &includes, double range_left, double range_right, int order,
