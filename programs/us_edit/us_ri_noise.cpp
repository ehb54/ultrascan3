...
US_RiNoise::US_RiNoise (const US_DataIO::RawData &raw,
                        const QList<int> &Includes, double left, double right,
                        const QString &dataType, int &initial_order,
                        QList<double> &r)
    : US_WidgetsDialog (0, 0), data (raw), includes (Includes),
      range_left (left), range_right (right), order (initial_order),
      residuals (r)
{
  ...
  QBoxLayout *plot = new US_Plot (data_plot, tr ("RI Noise Fit"),
...
void
US_RiNoise::draw_fit (double new_order)
{
  order = (int)new_order;
...
void
US_RiNoise::calc_residuals (const US_DataIO::RawData &data,
                            const QList<int> &includes, double range_left,
                            double range_right, int order,
