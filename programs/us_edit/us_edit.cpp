...
US_Edit::US_Edit (QString auto_mode) : US_Widgets ()
{
  ...
  pb_bll_modify = us_pushbutton (
      tr ("Modify Baseline Correction for Selected Triple"), false);
  ...
  le_investigator
      = us_lineedit (number + US_Settings::us_inv_name (), 1, true);
  ...
  pb_emanual
      = us_pushbutton (tr ("Edit Currently Selected Profile Manually"), false);

  lo_writemwl
      = us_checkbox (tr ("Save to all Wavelengths"), ck_writemwl, true);
  ...
  connect (pb_bll_modify, SIGNAL (clicked ()),
           SLOT (correct_bll_for_triple_auto ()));

  connect (this, SIGNAL (process_next_optics ()),
           SLOT (process_optics_auto ()));
  ...
  */
  ...
  connect (pb_accept, SIGNAL (clicked ()), this, SLOT (close_edit ()));
  ...
  plot = new US_Plot (data_plot, tr ("Absorbance Data"), tr ("Radius (in cm)"),
  ...
  // // Interference Data WITH existing Aprofile corresponding to existing
  // protocol!!! details[ "invID_passed" ] = QString("6"); details[ "filename"
  // ]     = QString("Comproject-itf-031220-run1176"); details[ "protocolName"
  // ] = QString("Comproject-itf-031220");
  //   //
  //   /****************************************************************************************/

  // // Interference Data WITH existing Aprofile corresponding to existing
  // protocol!!! details[ "invID_passed" ] = QString("6"); details[ "filename"
  // ]     = QString("5-itf-test-run1179"); details[ "protocolName" ] =
  // QString("5-itf-test");
  //   //
  //   /****************************************************************************************/
  ...
  // // Interference Data WITH existing Aprofile corresponding to existing
  // protocol!!! details[ "invID_passed" ] = QString("6"); details[ "filename"
  // ]     = QString("001-Amy-MWL-if-test-3-run794-IP"); details[
  // "protocolName" ] = QString("001-Amy-MWL-if-test-3");
  // //
  // /****************************************************************************************/

  // details[ "invID_passed" ] = QString("40");
  // details[ "filename" ]     =
  // QString("KulkarniJ_NP025-D2O-0-20-17K_091220-run1285"); details[
  // "protocolName" ] = QString("KulkarniJ_NP025-D2O-0-20-17K_091220");
  ...
  // details[ "filename" ]     =
  // QString("DubnauD_ComEA-14pbDNA_5-1_100621-run1233"); details[
  // "protocolName" ] = QString("DubnauD_ComEA-14pbDNA_5-1_100621");

  // details[ "invID_passed" ] = QString("77");
  // details[ "filename" ]     =
  // QString("Johnson_C_EWS_10uM_20uM_NaPi_MES_01-13-22-run1242"); details[
  // "protocolName" ] = QString("Johnson_C_EWS_10uM_20uM_NaPi_MES_01-13-22");

  // //Borries: combined RI + IP run
  // details[ "invID_passed" ] = QString("2");
  // details[ "filename" ]     =
  // QString("MW-AUC-IF_test_031622-run1270-IP,MW-AUC-IF_test_031622-run1270-RI");
  ...
  // details[ "filename" ]     =
  // QString("BlazeB_AUC3_PRS4-3_Day7_082522-run1372"); details[ "protocolName"
  // ] = QString("BlazeB_AUC3_PRS4-3_Day7_082522"); details[ "statusID" ]     =
  // QString("46"); details[ "autoflowID" ]   = QString("671");
  ...
  // details[ "filename" ]     =
  // QString("AAV396_CsCl-46kRpm_21APRIL23-run1916"); details[ "protocolName" ]
  // = QString("AAV396_CsCl-46kRpm_21APRIL23"); details[ "statusID" ]     =
  // QString("148"); details[ "autoflowID" ]   = QString("866");
  ...
  // details[ "invID_passed" ] = QString("165");
  // details[ "filename" ]     =
  // QString("AAV_GMP_test_030325-run2366-dataDiskRun-1515"); details[
  // "protocolName" ] = QString("GMP-test-ABDE-fromDisk"); details[ "statusID"
  // ]     = QString("588"); details[ "autoflowID" ]   = QString("1515");
...
US_Edit::US_Edit (QVector<US_DataIO::RawData> allData, QStringList triples,
                  QString workingDir, int currenChtInd, int plotind,
                  QString exptype)
    : US_Widgets ()
{
  pb_removeAllbutLast = us_pushbutton (tr ("Remove All but Last"), true);
  ...
  connect (pb_baseline_correct, SIGNAL (clicked ()),
           SLOT (set_linear_baseline_corr ()));
  ...
  connect (pb_exclusion, SIGNAL (clicked ()), SLOT (exclusion ()));
  connect (pb_edit1, SIGNAL (clicked ()), SLOT (edit_scan ()));
  connect (pb_removeAllbutLast, SIGNAL (clicked ()),
           SLOT (exclude_all_but_last ()));
  ...
  connect (pb_pass, SIGNAL (clicked ()), this, SLOT (pass_values ()));
...
US_Edit::US_Edit (QVector<US_DataIO::RawData> allData, QStringList triples,
                  QString workingDir, int currenChtInd, int plotind,
                  QString exptype, QStringList editParams,
                  QList<int> editParams_includes)
    : US_Widgets ()
{
  pb_removeAllbutLast = us_pushbutton (tr ("Remove All but Last"), true);
  pb_baseline_correct = us_pushbutton (tr ("Correct Baseline"), false);
  ...
  connect (pb_cancel, SIGNAL (clicked ()), SLOT (close_manual_edit ()));
  connect (pb_pass, SIGNAL (clicked ()), this, SLOT (pass_values_bll ()));
  ...
  foreach (s, data.scanData)
    {
      ...
      if (!includes.contains (scanNumber))
        continue;
          ...
          if (!deleteAll)
            {
              ...
              gaps = tr ("Scan ") + QString::number (rawScanNumber)
                     + tr (" has a maximum reading gap of ")
                     + QString::number (maxGap) + tr (" starting at radius ")
              ...
              QPushButton *pb_delete
                  = box.addButton (tr ("Delete"), QMessageBox::YesRole);

              QPushButton *pb_deleteAll
                  = box.addButton (tr ("Delete All"), QMessageBox::AcceptRole);

              QPushButton *pb_skip
                  = box.addButton (tr ("Skip"), QMessageBox::NoRole);
...
void
US_Edit::load_auto (QMap<QString, QString> &details_at_editing)
{
  sdiag = NULL;
  sdiag_bll = NULL;
  triples_all_optics.clear ();
  ...
  if (filename_runID_passed.contains (",")
      && filename_runID_passed.contains ("IP")
      && filename_runID_passed.contains ("RI"))
    {
  ...
  if (runType_combined_IP_RI)
    {
      QString filename_runID_auto_base_temp
          = filename_runID_passed.split (",")[0];
      int pos = filename_runID_auto_base_temp.lastIndexOf (QChar ('-'));

      filename_runID_auto_base = filename_runID_auto_base_temp.left (pos);

      qDebug () << "IN EDIT - filename_base for combined runs: "
                << filename_runID_auto_base;
    }

  ///////////////////////////////////////////////////////////////////////

  qDebug () << "autoflowID, intensityID, autoflowStatusID : "
            << autoflowID_passed << details_at_editing["intensityID"]
            << autoflowStatusID;
  qDebug () << "AT EDIT_DATA: filename, idInv: " << filename_runID_passed
            << ", " << idInv_auto;
...
US_Edit::process_optics_auto ()
{
  ...
  data = allData[0];
  dataType = QString (QChar (data.type[0])) + QString (QChar (data.type[1]));
  ...
  if (expType.isEmpty ()) // if no experiment type, assume Velocity
    ...
    expType = expType.left (1).toUpper () + expType.mid (1).toLower ();
  ...
  DbgLv (1) << "LD(): isMwl" << isMwl << "nwaveln" << nwaveln
            << toti_wvlns.size () << "ntriple" << ntriple;
  ...
  if (isMwl)
    { // Set values related to MultiWaveLength
      ...
      // DbgLv(1) << "IS-MWL:   nrpoint nscan ndset ndpoint" << nrpoint <<
      // nscan
      //  << ndset << ndpoint;
      ...
      //          );
      //       }
      // DbgLv(1) << "IS-MWL:  expd_radii size" << expd_radii.size() <<
      // nrpoint;
      ...
      //       {  // Initialize the data vector that has wavelength as the
      //       x-axis
      //          rdata << wrdata;
      //       }
      // DbgLv(1) << "IS-MWL:  rdata size" << rdata.size() << ndset;

      //       // Update wavelength-x-axis data vector with amplitude data
      //       points
      ...
      //          lambdas_by_cell( ccx );                      // Lambdas in
      //          cell

      //          for ( int jwx = 0; jwx < nwaveln; jwx++ )
      //          {  // Each wavelength in the current cell
      //             edata         = &allData[ trx ];               // Triple
      //             data int iwavl     = rawi_wvlns[ jwx ];             //
      //             Wavelength value int wvx       = toti_wvlns.indexOf( iwavl
      //             );   // Wavelength index
      ...
      //                int odx       = ccx * nrpoint;         // Output
      //                dataset index int opx       = scx * maxwavl + wvx;   //
      //                Output point index
      // DbgLv(2) << "IS-MWL:    scx odx opx" << scx << odx << opx;
      //                for ( int rax = 0; rax < nrpoint; rax++ )
      //                {  // Store ea. radius data point as a wavelength point
      //                in a scan
      ...
      for (int ii = 0; ii < triple.scanData.size (); ii++)
        {
          ...
          qDebug () << "scan" << ii + 1 << "delta-r rpm seconds" << ds->delta_r
                    << rpmc << timec << "omega2t w2t-integ" << ds->omega2t
                    << w2ti;
          if (ii == 0)
            {
  ...
  connect (ct_odlim, SIGNAL (valueChanged (double)), this,
           SLOT (od_radius_limit (double)));
  ...
  qDebug () << "job1run, job2run, job3run, job3auto, job4run, job5run, "
               "job6run_pcsa: "
            << job1run << ", " << job2run << ", " << job3run << ", "
            << job3auto << ", " << job4run << ", " << job5run << ", "
            << job6run_pcsa;

  // Debug
  for (int i = 0; i < cb_triple->count (); i++)
    qDebug () << "Triple, AprofParms: " << cb_triple->itemText (i) << ", "
              << aprofileParameters[i];
  ...
  for (int trx = 0; trx < cb_triple->count (); trx++)
    {
      ...
      if (dataSource.contains ("DiskAUC:Absorbance")
          && channelname_.contains ("S"))
        {
      ...
      le_status->setText (
          tr ("Setting edit controls for channel %1").arg (triple_name));
      qApp->processEvents ();

      // Debug
      for (int g = 0; g < expi_wvlns.size (); ++g)
        qDebug () << "MWL wavelengths for triple: " << triple_name
                  << expi_wvlns[g];

      if (isMwl)
        {
          qDebug () << "#triples, #wavelns_i -- " << cb_triple->count ()
                    << wavelns_i.size ();
          qDebug () << "#wavelns in triple   -- " << triple_name
                    << wavelns_i[trx].size ();
          ...
          if (isSet_edit_info_for_channel (channel_desc, opsys))
            {
              ...
              for (jjj = triple_to_edit.begin (); jjj != triple_to_edit.end ();
                   ++jjj)
                {
                  qDebug () << "jjj.key(), channel_desc, opsys -- "
                            << jjj.key () << channel_desc << opsys;
                  if (jjj.key ().contains (channel_desc)
                      && jjj.key ().contains (opsys))
                    {
                      QString wvl_set_edit = jjj.value ();

                      iwavl_edit_ref[trx] = wvl_set_edit.toInt ();
                      qDebug () << "Reference wvl --  " << wvl_set_edit
                                << " was identified using AProfile";
          ...
          else
            {
              ...
              qDebug () << "Wvl index && value -- "
                        << iwavl_edit_ref_index[trx] << iwavl_edit_ref[trx];
            }
      ...
      // QString rawGUID_test          = US_Util::uuid_unparse( (unsigned
      // char*)data.rawGUID ); qDebug() << "Current rawData: rawGUID: " <<
      // rawGUID_test << ", filename: " << files[ trx ] << ", editGUID: " <<
      // editGUIDs[ trx ];

      // Find meniscus position based on the read parameters for centerpiece
      // channel (trx) && aprofile for each channel
      read_centerpiece_params (trx);

      // range_right = 7.0;
      aprofile_volume
          = aprofileParameters[trx][0].toDouble ();         //<-- From Aprofile
      range_right = aprofileParameters[trx][1].toDouble (); //<-- From Aprofile
      qDebug () << "Range_right && Loading volume FROM AProfile: "
                << triple_name << ", " << range_right << ", "
                << aprofile_volume;
      ...
      if (dataType != "IP")
        {
          ...
          // ALEXEY: here create a QMap to couple current triple AND meniscus,
          // ranges, plateau and baseline;
          //  then, use this QMap when plotting plot_range() in
          //  new_triple_auto()...
          ...
          QMap<QString, QStringList>::const_iterator ci
              = aprof_channel_to_scans.constBegin ();
          while (ci != aprof_channel_to_scans.constEnd ())
            {
              if (channelname_.contains (ci.key ()))
                {
                  qDebug () << "Channel, AProfile SCANSS: " << channelname_
                            << ", " << ci.key () << ": " << ci.value ();
      ...
      else
        {
          ...
          DbgLv (1) << "AGap: L R" << airGap_left << airGap_right
                    << " AdjIntf";
          ...
          QString wkstr;
          le_airGap->setText (
              wkstr.sprintf ("%.3f - %.3f", airGap_left, airGap_right));
          ...
          step = PLATEAU;
          // plot_range();              //ALEXEY <--- here applies the gap
          // removal
          next_step ();
...
void
US_Edit::reset_editdata_panel (void)
{
...
void
US_Edit::read_aprofile_data_from_aprofile ()
{
  ...
  for (int trx = 0; trx < cb_triple->count (); trx++)
    {
      ...
      if (dataSource.contains ("DiskAUC:Absorbance")
          && channelname.contains ("S"))
        channelname = channelname.replace ("S", "A");

      qDebug () << "channelname2: " << channelname;

      // iterate over aprofile_channel_to_parms QMap and check if channel name
      // is a part of triple_name:
      QMap<QString, QStringList>::const_iterator i
          = aprof_channel_to_parms.constBegin ();
      while (i != aprof_channel_to_parms.constEnd ())
        {
          if (channelname.contains (i.key ()))
            {
              qDebug () << "Channel, AProfile PARMS: " << channelname << ", "
                        << i.key () << ": " << i.value ();
...
void
US_Edit::read_centerpiece_names_from_protocol ()
{
  // centerpiece_names.clear();
  ...
  US_ProtocolUtil::read_record_auto (ProtocolName_auto, idInv_auto.toInt (),
                                     &xmlstr, NULL, &db);
  ...
  // DO  NOT FORGET TO populate centerpiece_names array by CHANNELS, NOT by
  // Cells only !!! E.g. if cells 2 & 4, then centerpiece_names must be put for
  // 2/A, 2/B, 4/A, 4/B Some channles of the same cell may NOT be used!!! e.g
  // 2/A, 4/A && 4/B
  ...
  for (int trx = 0; trx < cb_triple->count (); trx++)
    {
      ...
      // iterate over cell_to_centerpeice QMap and check if cellID is a part of
      // triple_name:
      QMap<QString, QString>::const_iterator i
          = cell_to_centerpiece.constBegin ();
      while (i != cell_to_centerpiece.constEnd ())
        {
          if (triple_name_cell.contains (i.key ()))
            {
              qDebug () << "triple_name_cell, cell, centerpiece: "
                        << triple_name_cell << ", " << i.key () << ": "
                        << i.value ();
...
bool
US_Edit::readProtocolCells_auto (QXmlStreamReader &xmli)
{
  while (!xmli.atEnd ())
    {
      QString ename = xmli.name ().toString ();
...
bool
US_Edit::readProtocolAProfile_auto (QXmlStreamReader &xmli)
{
  while (!xmli.atEnd ())
    {
      QString ename = xmli.name ().toString ();
...
bool
US_Edit::readAProfileBasicParms_auto (QXmlStreamReader &xmli)
{
  // while( ! xmli.atEnd() )
  //   {
  //     xmli.readNext();
  ...
  while (!xmli.atEnd ())
    {
      ...
      if (xmli.isStartElement ())
        {
          ...
          if (ename == "channel_parms")
              ...
              if (attr.hasAttribute ("load_volume"))
                {
                  ...
                  qDebug () << "READING aprof XML: " << channel_name << ", "
                            << aprof_parms;
                  ...
                  if (attr.hasAttribute ("run"))
                    {
                      QString channel_desc
                          = attr.value ("chandesc").toString ();
                      channels_to_analyse[channel_desc]
                  ...
                  if (attr.hasAttribute ("wvl_not_run"))
                    {
                      QString channel_desc
                          = attr.value ("chandesc").toString ();
                      triples_skip_analysis[channel_desc]
                          = attr.value ("wvl_not_run").toString ();
                    }
                  ...
                  if (attr.hasAttribute ("scan_excl_begin"))
                    {
                      ...
                      scan_excl_pairs
                          << attr.value ("scan_excl_begin").toString ()
                          << attr.value ("scan_excl_end").toString ();

                      qDebug ()
                          << "READING aprof XML, scans excl.: " << channel_name
                          << ", " << scan_excl_pairs;
          ...
          else if (ename == "job_2dsa_fm")
            {
              ...
              // fitrng         = attr.value( "fit_range"
              // ).toString().toDouble(); grpoints       = attr.value(
              // "grid_points" ).toString().toInt();
          ...
          else if (ename == "job_2dsa_it")
            {
              ...
              // rfiters        = attr.value( "max_iterations"
              // ).toString().toInt();
          ...
          else if (ename == "job_2dsa_mc")
            {
              ...
              // mciters        = attr.value( "mc_iterations"
              // ).toString().toInt();
      ...
      // else if ( ename == "p_2dsa" )       //Stop reading AProfile when 2DSA
      // section reached
      //  break;
...
bool
US_Edit::bool_flag (const QString xmlattr)
{
  return (!xmlattr.isEmpty () && (xmlattr == "1" || xmlattr == "T"));
}

void
US_Edit::read_centerpiece_params (int trx)
{
  QStringList query;
  ...
  centparms_info << centerpiece_info["name"] << centerpiece_info["bottom"]
                 << centerpiece_info["pathlen"] << centerpiece_info["angle"];

  centerpieceParameters[trx] = centparms_info;
}

QVector<double>
US_Edit::find_airGap_interference_auto ()
{
  ...
  // starting from meniscus position, go backwards and fit several points into
  // staight line.. then, look at max deviation from it and find the onset of
  // the airGap (in reverse order) basically, repeat finding meniscus
  // algorithm...
  ...
  for (int i = start_scan; i < end_scan; i++)
    {
      ...
      // qDebug() << "INTERFERENCE: left_fit, right_fit: " << left_fit << ", "
      // << right_fit;
      ...
      ***************/
      ...
      double Slope
          = (s->rvalues[indexRight_fit] * invert
             - s->rvalues[indexLeft_fit] * invert)
            / (data.xvalues[indexRight_fit] - data.xvalues[indexLeft_fit]);
      double YInt = s->rvalues[indexRight_fit] * invert
                    - Slope * data.xvalues[indexRight_fit];
      /****************************************************************************/
      ...
      for (int j = indexLeft_fit; j <= indexRight_fit; j++)
        {
          ...
          // qDebug() << "y_curr, x_curr, (Slope * x_curr + YInt), y_diff_abs:
          // " << y_curr << ", " << x_curr << ", " << y_fit << ", " <<
          // y_diff_abs;
      ...
      // reverse order: search for a y-deviation (much) bigger than from a
      // straight line fit based on selected points
      ...
      // qDebug() << "x_airGap_left, x_airGap_right: " << x_airGap_left << ", "
      // << x_airGap_right;
    }
...
double
US_Edit::find_meniscus_interference_auto ()
{
  ...
  // double aprofile_volume   = 460; // Just an example - to be read from
  // AProfile, will be global

  double meniscus_init
      = sqrt (bottom_db * bottom_db
              - (aprofile_volume * 360
                 / (1000 * pathlength_db * angle_db
                    * M_PI))); // Radians = Degrees * (M_PI/180.0)

  qDebug () << "Meniscus_init: INTERFERENCE " << meniscus_init << ", "
            << bottom_db << ", " << pathlength_db << ", " << angle_db << ", "
            << M_PI;
  ...
  qDebug () << "Scans for meniscus: start_scan, end_scan: " << start_scan
            << ", " << end_scan;
  ...
  for (int i = start_scan; i < end_scan; i++)
    {
      ...
      // qDebug() << "indexLeft_fit = " << indexLeft_fit << "; indexRight = "
      // <<  indexRight;
      ...
      **********************************************/
      ...
      // Alternative: derive a curve passing through Left/Right indecies
      double Slope
          = (s->rvalues[indexRight] * invert
             - s->rvalues[indexLeft_fit] * invert)
            / (data.xvalues[indexRight] - data.xvalues[indexLeft_fit]);
      double YInt
          = s->rvalues[indexRight] * invert - Slope * data.xvalues[indexRight];
      /****************************************************************************/
...
US_Edit::find_meniscus_auto ()
{
  ...
  qDebug () << "Meniscus_init: " << meniscus_init << ", " << bottom_db << ", "
            << pathlength_db << ", " << angle_db << ", " << M_PI;
  ...
  for (int i = start_scan; i < end_scan; i++)
    {
      ...
      int indexLeft = data.xindex (meniscus_init);
      // int indexRight  = data.xindex( meniscus_init + 0.5 );   // <---- OR
      // should it just be the end data from AProfile ?

      // int indexRight  = data.xindex( range_right );
      // ALEXEY: maybe to be on safer side, take indexRight = meniscus_init  +
      // ( aprofile_right - meniscus_init )/2.0 int indexRight = meniscus_init
      // + ( data.xindex( range_right ) - meniscus_init )/2.0;

      // int indexRight  = data.xindex( range_right );
      // ALEXEY: maybe to be on safer side, take indexRight = meniscus_init  +
      // ( aprofile_right - meniscus_init )/2.0!!!
      int indexRight
          = data.xindex (meniscus_init + (range_right - meniscus_init) / 1.5);

      qDebug () << "data.xindex( range_right ) = "
                << data.xindex (range_right);
      qDebug () << "indexLeft = " << indexLeft
      ...
      // Array of maximum values indexes for the last ~20% of scans in the
      // region [ meniscus_left -- meniscus_left + 0.1 ]
...
US_Edit::scan_db_auto (void)
{
  ...
  while (db.next ())
    { // Accumulate data description objects from database information
      ...
      QString idata = label + "^" + runID + "^" + tripID + "^" + filename + "^"
                      + rawGUID + "^" + rawDataID + "^" + date;
...
void
US_Edit::create_descs_auto (QStringList &runIDs, QStringList &infoDs,
                            int naucf)
{
...
void
US_Edit::load_db_auto (QList<DataDesc_auto> &sdescs)
{
  int nerr = 0;
  QString emsg;
          ...
          if (dcheck.isEmpty ())
            {
              ...
              db.next ();
              ddesc.dcheck
                  = db.value (8).toString () + " " + db.value (9).toString ();
              dcheck = ddesc.dcheck;
            }
...
US_Edit::load (void)
{
  ...
  connect (dialog, SIGNAL (progress (QString)), this,
           SLOT (progress_load (QString)));
  connect (dialog, SIGNAL (changed (bool)), this,
  ...
  if (isMwl)
    { // Set values related to MultiWaveLength
      ...
      //       {  // Handle each triple of AUC data
      //          lambdas_by_cell( ccx );                      // Lambdas in
      //          cell trx_b = trx; int min_xval = -100000000; int max_xval =
      //          100000000; int dx       = -1000; for ( int jwx = 0; jwx <
      //          nwaveln; jwx++ )
      //          {
      //             edata         = &allData[ trx ];               // Triple
      //             data min_xval = qMax(min_xval, static_cast<int>
      //             (qRound(edata->xvalues.first() * 1000))); max_xval =
      //             qMin(max_xval, static_cast<int>
      //             (qRound(edata->xvalues.last() * 1000))); double d =
      //             edata->xvalues.at(1) - edata->xvalues.at(0); dx = qMax(dx,
      //             static_cast<int> (qRound(d * 1000))); trx++;
      //          }
      ...
      //             {  // Each wavelength in the current cell
      //                 edata         = &allData[ trx ];               //
      //                 Triple data int iwavl     = rawi_wvlns[ jj ]; //
      //                 Wavelength value int wvx       = toti_wvlns.indexOf(
      //                 iwavl );   // Wavelength index DbgLv(1) << "IS-MWL:
      //                 trx ccx wvx" << trx << ccx << wvx;

      //                 int rpidx = -1;
      //                 for ( int kk = xvals_pos.at(jj); kk <
      //                 edata->pointCount(); kk++)
      //                 {
      //                     int cxval =
      //                     static_cast<int>(qRound(edata->xvalues.at(kk) *
      //                     1000)); if ( cxval == txval ) {
      //                         rpidx = kk;
      //                         xvals_pos[jj] = kk;
      //                         break;
      ...
      //                 {
      //                     double yval =
      //                     edata->scanData.at(ss).rvalues.at(rpidx); int
      //                     idx_ss = ss * nwaveln; int idx_wv = idx_ii +
      //                     idx_ss + jj; yvals[idx_wv] = yval;
      //                 }
      ...
      for (int jwx = 0; jwx < nwaveln; jwx++)
        {
          ...
          min_xval = qMax (min_xval, static_cast<int> (qRound (
                                         edata->xvalues.first () * 1000)));
          max_xval = qMin (max_xval, static_cast<int> (qRound (
                                         edata->xvalues.last () * 1000)));
          ...
          for (int jj = 0; jj < nwaveln; jj++)
            {                        // Each wavelength in the current cell
              ...
              for (int kk = xvals_pos.at (jj); kk < edata->pointCount (); kk++)
                {
                  int cxval = static_cast<int> (
                      qRound (edata->xvalues.at (kk) * 1000));
                  if (cxval == txval)
                    {
                      rpidx = kk;
                      xvals_pos[jj] = kk;
...
void
US_Edit::set_pbColors (QPushButton *pb)
{
...
void
US_Edit::plot_current (int index)
{
  if (isMwl)
    {
      plot_mwl ();
...
void
US_Edit::mouse (const QwtDoublePoint &p)
{
  ...
  switch (step)
    ...
    case MENISCUS:
      if (dataType == "IP")
      ...
      else
        {
          ...
          if (qAbs (p.x () - meniscus_left) < 0.005)
            return;
      ...
      if (dataType != "IP")
        {
          ...
          QPen pen (brush, 2.0);
          QwtSymbol *symb
              = new QwtSymbol (QwtSymbol::Cross, brush, pen, QSize (8, 8));
    ...
    case RANGE:
      qDebug () << "range_left, range_right -- " << range_left << range_right;
      if (range_left == 0.0)
        {
          if (v_line != NULL)
            {
      ...
      else
        {
          ...
          if (!expIsEquil)
            ...
            { // Equilibrium
              ...
              if (sData[jsd].meniscus == 0.0 && meniscus > 0.0)
                total_edits++;
          ...
          if (total_edits >= total_speeds)
            {
              qDebug () << "In RANGES: total_edits, total_speeds: "
                        << total_edits << ", " << total_speeds;
              all_edits = true;
              pb_write->setEnabled (true);
    ...
    case BASELINE:
      {
        ...
        double bl = sum / 11.0;
        baseline = p.x ();
        plateau = (plateau > 0.0) ? plateau : (range_right - _PLATEAU_OFFSET_);
    ...
    case BASELINE_LINEAR_CORR:
      qDebug () << "bl_corr_left, bl_corr_right {xx,yy} -- " << bl_corr_left_x
                << bl_corr_right_x << bl_corr_left_y << bl_corr_right_y;
      if (bl_corr_left_x == 0.0)
        {
          ...
          qDebug () << "[FIRST]bl_corr_left, bl_corr_right {xx,yy} -- "
                    << bl_corr_left_x << bl_corr_right_x << bl_corr_left_y
                    << bl_corr_right_y;

          fixedPoint = QPointF (p.x (), p.y ());
          qDebug () << "fixedPoint -- " << fixedPoint.x () << fixedPoint.y ();

          pick->setStateMachine (new QwtPickerTrackerMachine ());
          connect (pick, SIGNAL (moved (const QPointF &)), this,
                   SLOT (onMouseMoved (const QPointF &)));
      ...
      {
        bl_corr_slope = (bl_corr_right_y - bl_corr_left_y)
                        / (bl_corr_right_x - bl_corr_left_x);
...
void
US_Edit::onMouseMoved (const QPointF &mousePos)
{
  // Remove the left line
  if (line_to_mouse != NULL)
    {
...
void
US_Edit::draw_vline (double radius)
{
  double r[2];
      ...
      if (!isMwl)
        {
          ...
          qDebug () << "SETTING Baseline, Plateau : baseline bl" << baseline
                    << bl;
          le_plateau->setText (QString::number (plateau, 'f', 8));
        }
      else
...
void
US_Edit::set_linear_baseline_corr (void)
{
  le_bll_slope->setText ("");
  le_bll_intercept->setText ("");
...
void
US_Edit::set_fringe_tolerance (double ftvalue)
{
  DbgLv (1) << "   gap_fringe" << gap_fringe << "SFT: dataType" << dataType
            << "ftvalue" << ftvalue;
  ...
  if (step == MENISCUS || step == AIRGAP || step == RANGE)
    return;
  ...
  for (int i = 0; i < data.scanData.size (); i++)
    {
      ...
      QString title = tr ("Raw Data at ") + QString::number (s->seconds)
                      + tr (" seconds") + " #" + QString::number (i);
...
void
US_Edit::plot_range_and_blc (void)
{
  if (plot->btnZoom->isChecked ())
    plot->btnZoom->setChecked (false);
      ...
      if (expIsEquil)
        {
          ...
          QwtPlotCurve *c = us_curve (
              data_plot, tr ("Meniscus at") + QString::number (tScan));
      ...
      for (int j = indexLeft; j <= indexRight; j++)
        {
          ...
          v[count] = s->rvalues[j] * invert
                     - (r[count] * bl_corr_slope + bl_corr_yintercept);
...
US_Edit::plot_mwl (void)
{
  ...
  // ALEXEY: when all data loaded && processed for meniscus, plot reference wvl
  // (selected for edit)
  if (us_edit_auto_mode && all_loaded && !triple_plot_first_time[triple_index])
    {
      data_index
          = mwl_data.data_index (iwavl_edit_ref[triple_index], triple_index);
      index = data_index;

      recndx = iwavl_edit_ref_index[triple_index];

      qDebug () << "In PLOT_MWL when all loaded: triple_index, "
                   "iwavl_edit_ref[ triple_index ], iwavl_edit_ref_index[ "
                   "triple_index , data_index -- "
                << triple_index << ", " << iwavl_edit_ref[triple_index] << ", "
                << iwavl_edit_ref_index[triple_index] << ", " << data_index;
  ...
  if (xaxis_radius)
    {
      DbgLv (1) << "PlMwl:   x-r index cc nw rx" << index << ccx << nwavelo
                << recndx;
  ...
  else
    {
      ...
      // DbgLv(1) << "PlMwl:   x-w index cc nr rx" << index << ccx << nrpoint
      // << recndx;
      data = *outData[0];
      rectype = tr ("Radius");
  ...
  DbgLv (1) << "PlMwl: ccx index rtype rval" << ccx << index << rectype
            << recvalu;
  ...
  DbgLv (1) << "PlMwl:   xa_rad" << xaxis_radius << "nsc npt" << nscan
            << npoint;
  ...
  if (xaxis_radius)
    { // Build normal AUC data plot
      ...
      for (int ii = 0; ii < nscan; ii++)
        {
          ...
          QString ctitle = tr ("Raw Data at ") + QString::number (scn->seconds)
                           + tr (" seconds") + " #" + QString::number (ii);
      ...
      connect (pick, SIGNAL (cMouseUp (const QwtDoublePoint &)),
               SLOT (mouse (const QwtDoublePoint &)));
      DbgLv (1) << "PlMwl:      END xa_RAD  kodlim odlimit" << kodlim
                << odlimit;
    }
...
void
US_Edit::focus_from (double scan)
{
  int from = (int)scan;
...
// Set curve colors
void
US_Edit::set_colors (const QList<int> &focus)
{
  QList<QColor> mcolors;
  int nmcols = plot->map_colors (mcolors);
...
void
US_Edit::exclude_all_but_last (void)
{
  int scanStart = 0;
...
void
US_Edit::exclusion (void)
{
  reset_excludes ();
  US_ExcludeProfile *exclude = new US_ExcludeProfile (includes);

  connect (exclude, SIGNAL (update_exclude_profile (QList<int>)), this,
           SLOT (update_excludes (QList<int>)));

  connect (exclude, SIGNAL (cancel_exclude_profile (void)), this,
           SLOT (cancel_excludes (void)));

  connect (exclude, SIGNAL (finish_exclude_profile (QList<int>)), this,
           SLOT (finish_excludes (QList<int>)));
...
void
US_Edit::update_excludes (QList<int> scanProfile)
{
...
void
US_Edit::finish_excludes (QList<int> excludes)
{
  for (int i = 0; i < excludes.size (); i++)
    includes.removeAll (excludes[i]);

  DbgLv (1) << "FIN_EXCL: sizes excl incl" << excludes.size ()
            << includes.size ();
  replot ();
  reset_excludes ();
}
...
// Update scan points
void
US_Edit::update_scan (QList<QPointF> changes)
{
      ...
      for (int j = start; j < end; j++)
        {
          if (US_DataIO::spike_check (*s, data.xvalues, j, start, end,
                                      &smoothed_value))
            {
              ...
              while (c & (1 << (7 - index % 8)))
                {
                  if (US_DataIO::spike_check (*s, data.xvalues, index, start,
                                              end, &smoothed_value))
...
// Remove spikes
void
US_Edit::remove_spikes_auto (void)
{

  // ALEXEY: Apply data modifications like adjust_interference && calc_integral
  // for IP data
  if (dataType == "IP")
    {
  ...
  // // ALEXEY: resize up to (1) meniscus; (2) left_range; (3) right_range; (4)
  // plateau; (5) baseline; (6) baseline_od for (int i=0; i < editProfile[
  // cb_triple->currentText() ].count(); i++)
  //   {
  //     if (i > 5)
  ...
  qDebug () << cb_triple->currentText () << ", "
            << editProfile[cb_triple->currentText ()];
  ...
  if (us_edit_auto_mode && all_loaded)
    {
      ...
      if (isMwl)
      ...
      else
        {
          QString str;
          le_baseline->setText (
              str.sprintf ("%.3f (%.3e)", baseline, baseline_od));
        }
...
US_Edit::undo_auto (void)
{
  ...
  */
  ...
  if (all_loaded)
    {
      // // ALEXEY: resize up to (1) meniscus; (2) left_range; (3) right_range;
      // (4) plateau; (5) baseline; (6) baseline_od for (int i=0; i <
      // editProfile[ cb_triple->currentText() ].count(); i++)
      // 	 {
      // 	   if (i > 5)
      // 	     editProfile[ cb_triple->currentText() ].removeAt(i);
      // 	 }
...
{
  residuals.clear ();
  US_RiNoise *dialog = new US_RiNoise (data, includes, range_left, range_right,
                                       dataType, noise_order, residuals);
...
// Subtract residuals
void
US_Edit::subtract_residuals (void)
{
...
// Select a new triple
void
US_Edit::new_triple_auto (int index)
{
  triple_index = index;
  ...
  // qDebug() << "#triples, #wavelns_i -- " << cb_triple->count() <<
  // wavelns_i.size(); qDebug() << "#wavelns in triple   -- " << triple_name <<
  // wavelns_i[ triple_index ].size();

  qDebug () << "Triple_name, cb_triple->currentText() -- " << triple_name
  ...
  */
  ...
  // ALEXEY: if MVL plot triple for the reference wvl identified from the
  // AProfile
  //  if ( autoflow_expType != "ABDE" )
  //    {
  if (isMwl)
  ...
  QString rawGUID_test1
      = US_Util::uuid_unparse ((unsigned char *)data.rawGUID);
  ...
  DbgLv (1) << "EDT:NewTr:   sw tri dx" << swavl << triple << idax
            << "dataType" << dataType << "gap_fringe gap_thresh" << gap_fringe
            << gap_thresh;
  if (otdt != dataType)
  ...
  // qDebug() << "In new triple_auto: all_edits, isMw: " << all_edits << ", "
  // << isMwl;
  ...
  qDebug () << "In new_triple_auto: ref_wvl_index -- "
  ...
  int scanExcl_begin_ind
      = editProfile_scans_excl[cb_triple->currentText ()][0].toInt ();
  int scanExcl_end_ind
      = editProfile_scans_excl[cb_triple->currentText ()][1].toInt ();

  qDebug () << "IN new_Triple_auto(): triple -- " << cb_triple->currentText ()
            << "ScanBegin -- " << scanExcl_begin_ind << "ScanEnd -- "
            << scanExcl_end_ind;
  ...
  // end of the scan set
  for (int ii = data.scanData.size () - scanExcl_end_ind;
       ii < data.scanData.size (); ii++)
    {
      includes.removeLast ();
    }
  ...
  if (autoflow_expType == "ABDE"
      && edited_triples_abde[cb_triple->currentText ()])
  ...
  // qDebug() << "Triple: " << triple << ", cb_triple_text: " <<
  // cb_triple->currentText(); qDebug() << editProfile[ triple ] << ", " <<
  // editProfile[ cb_triple->currentText() ];
  ...
  QString wvl_value
      = (isMwl ? swavl : cb_triple->currentText ().split ("/")[2].trimmed ());
  ...
  if (all_loaded)
    {
      ...
      qDebug () << "NEW_TRIPLE_AUTO: editProfile.count(): "
                << editProfile.count ();
      ...
      //[ABDE]linear-baseline-correction
      // if ( autoflow_expType == "ABDE" && edited_triples_abde[
      // cb_triple->currentText() ] )
      if (autoflow_expType == "ABDE")
        {
          // QString wvl_value = ( isMwl ? swavl :
          // cb_triple->currentText().split("/")[2].trimmed() ); qDebug() <<
          // "wvl_value -- " << wvl_value;

          bl_corr_slope
              = editProfile_blc[cb_triple->currentText ()][wvl_value][0]
                    .toDouble ();
          bl_corr_yintercept
              = editProfile_blc[cb_triple->currentText ()][wvl_value][1]
                    .toDouble ();

          le_bll_slope->setText (QString::number (bl_corr_slope, 'f', 3));
          le_bll_intercept->setText (
              QString::number (bl_corr_yintercept, 'f', 3));
        }
      ...
      // ALEXEY: Apply data modifications like adjust_interference &&
      // calc_integral for IP data
      if (dataType == "IP")
        {
...
void
US_Edit::set_current_bll_abde (QString chann, QString swave)
{
...
US_Edit::write (void)
{
  ...
  pb_write->setEnabled (false);
  ck_writemwl->setEnabled (false);
  pb_nextChan->setEnabled (
      triples.size () > 1 && cb_triple->currentIndex () < triples.size () - 1);
}

// Check if EditedProfiles already saved/started to be saved into DB
bool
US_Edit::isSaved_auto (void)
{
  ...
  // Check if the edit profiles corresponding top filename/label already exist
  // in the DB
...
int
US_Edit::read_autoflow_stages_record (int autoflowID)
{
  int status = 0;
  ...
  // qDebug() << "BEFORE query ";
  QStringList qry;
  qry << "autoflow_edit_status" << QString::number (autoflowID);
...
void
US_Edit::revert_autoflow_stages_record (int autoflowID)
{
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd ();
  ...
  // qDebug() << "BEFORE query ";
  QStringList qry;
  qry << "autoflow_edit_status_revert" << QString::number (autoflowID);
...
US_Edit::read_autoflow_record (int autoflowID)
{
  // Check DB connection
  US_Passwd pw;
  QString masterpw = pw.getPasswd ();
...
void
US_Edit::correct_bll_for_triple_auto (void)
{
  // hide parent layouts
  upperWidget->hide ();
  leftWidget->hide ();
  ...
  int plotInd = plotndx;
  qDebug () << "IN correct_bll_for_triple_auto( void ), plotInd -- "
            << plotInd;

  QStringList editParams = editProfile[cb_triple->currentText ()];
  QList<int> editParams_includes
      = editProfile_includes[cb_triple->currentText ()];

  sdiag_bll = new US_Edit (allData, triples, workingDir, currChIndex, plotInd,
                           autoflow_expType, editParams, editParams_includes);
  ...
  // add slots
  connect (sdiag_bll,
           SIGNAL (pass_edit_params_blc (QMap<QString, QStringList> &)), this,
           SLOT (update_triple_edit_params_blc_modified (
  ...
  connect (sdiag_bll, SIGNAL (restore_main_view ()), this,
           SLOT (restore_view ()));
...
US_Edit::manual_edit_auto (void)
{
  ...
  // sdiag->setWindowFlags( Qt::Dialog | Qt::WindowTitleHint |
  ...
  connect (sdiag, SIGNAL (pass_edit_params (QMap<QString, QStringList> &)),
           this,
           SLOT (update_triple_edit_params (QMap<QString, QStringList> &)));

  connect (sdiag,
           SIGNAL (pass_edit_params_includes (QMap<QString, QList<int> > &)),
           this,
           SLOT (update_triple_edit_params_includes (
               QMap<QString, QList<int> > &)));

  connect (
      sdiag, SIGNAL (pass_edit_params_blc (QMap<QString, QStringList> &)),
      this,
      SLOT (update_triple_edit_params_blc (QMap<QString, QStringList> &)));
...
// [Modify-per-triple]Update triple's Linear-baseline-correction edit params
// with those obtained manually...
void
US_Edit::update_triple_edit_params_blc_modified (
    QMap<QString, QStringList> &edit_params_blc)
{
  pick->disconnect ();
  // show parent layouts
  upperWidget->show ();
  ...
  if (isMwl)
    {
      ...
      qDebug () << "[MWL:Modify-per-triple]Setting baseline coor for Wvl --  "
                << w_c;
  ...
  else
    {
      ...
      qDebug ()
          << "[Singe WL:Modify-per-triple]Setting baseline coor for Wvl --  "
          << w_c;
      editProfile_blc[t_name][w_c] = edit_params_blc[t_name];
    }
...
// Update triple's Linear-baseline-correction edit params with those obtained
// manually..
void
US_Edit::update_triple_edit_params_blc (
    QMap<QString, QStringList> &edit_params_blc)
{
  QString t_name = edit_params_blc.keys ()[0];
...
void
US_Edit::update_triple_edit_params_includes (
    QMap<QString, QList<int> > &edit_params_includes)
{
  pick->disconnect ();
  ...
  qDebug () << "In PASSING values manually: us_edit_auto_mode AFTER -- "
            << t_name << us_edit_auto_mode;
}

// Update triple's edit params with those obtained manually..
void
US_Edit::update_triple_edit_params (QMap<QString, QStringList> &edit_params)
{
  QString t_name = edit_params.keys ()[0];
  editProfile[t_name] = edit_params[t_name];

  qDebug () << "In update_triple_edit_params: t_name -- " << t_name;
  qDebug () << "In update_triple_edit_params: editProfile[ t_name ] -- "
            << editProfile[t_name];
  ...
  if (autoflow_expType != "ABDE")
    {
      ...
      QString comment_t
          = QInputDialog::getText (this, tr ("Reason for MANUAL editing"), msg,
                                   QLineEdit::Normal, default_text, &ok);
...
void
US_Edit::setUnsetSaveBttn_abde (void)
{
  bool all_processed = true;
  QMap<QString, bool>::iterator os;
  for (os = edited_triples_abde.begin (); os != edited_triples_abde.end ();
       ++os)
    {
      if (!os.value ())
        {
...
US_Edit::write_auto (void)
{
  ...
  if (!status_edit_unique)
    {
      ...
      QMessageBox::information (
          ...
          tr ("The program advanced or is advancing to the next stage!\n\n"
              ...
              "has already saved edit profiles into DB using different "
              "program "
  ...
  for ( int i = 0; i < triples.size(); ++i )
    {
      ...
      if ( dataType == "IP" )
        current_triple_name = triple_parts[0] + "." + triple_parts[1] + "." +
  ...
  */
  ...
  qDebug () << "autoflowID_passed, autoflow status, isSaved_auto(): "
            << autoflowID_passed << ", "
            << autoflow_details["status"]; // << ", " << isSaved_auto();
  ...
  // ALEXEY: before processing next optics system (if multiple), push triple
  // names to the general triple array: Also, push channel names into general
  // channel array
  for (int i = 0; i < triples.size (); ++i)
    {
      // qDebug() << "Triple name: " << triples[i];
          ...
          if (!all_processed)
            {
              QMessageBox::information (
                  this,
                  tr ("Edit Profiles for Current Optical System Already "
                      "Saved"),
                  tr ("It appears that edit profiles for the current optical "
                      "system are already saved!\n\n"
                      "The program will switch to processing data for the "
                      "next optical system... "));
          ...
          else
            {
              QMessageBox::information (
                  ...
                  tr ("The program advanced or is advancing to the next "
                      "stage!\n\n"
                      "This happened because you or different user "
                      "has already saved edit profiles into DB using "
                      "different program "
  ...
  gmp_submitter_map = pw_at.getPasswd_auditTrail (
      "GMP Run EDIT Form",
      "Please fill out GMP run EDIT form:", user_submitter);

  int gmp_submitter_map_size = gmp_submitter_map.keys ().size ();
  qDebug () << "Submitter map: " << gmp_submitter_map.keys ()
            << gmp_submitter_map.keys ().size () << gmp_submitter_map_size
  ...
  if (gmp_submitter_map_size == 0 || gmp_submitter_map.keys ().isEmpty ())
    {
      ...
      // DEBUG
      for (int i = 0; i < channels_all.size (); ++i)
        qDebug () << "[NO COMMMENT] BEFORE AUTOFLOW_ANALYSIS: channel name --"
                  << channels_all[i];
      for (int j = 0; j < triples_all_optics.size (); ++j)
        qDebug () << "[NO COMMNET] BEFORE AUTOFLOW_ANALYSIS: triple name -- "
                  << triples_all_optics[j];
      // END DEBUG
    ...
    { // non-Equilibrium:  write single current edit (if "all" unchecked)
      if (isMwl && ck_writemwl->isChecked ())
        { // Write edits for all triples in the current cell/channel
          ...
          for (int trx = 0; trx < cb_triple->count (); trx++)
            {
              qDebug () << "Writing MWL, channel: " << trx << ": "
                        << cb_triple->itemText (trx);
              write_mwl_auto (trx);
            }
        }
        ...
        { // Write single triple's edit
          ...
          for (int trx = 0; trx < cb_triple->count (); trx++)
            {
              qDebug () << "Writing non-MWL, channel: " << trx << ": "
                        << cb_triple->itemText (trx);
          ...
          // else
          //   {
          //     // <---- For now: for Interference data, do editing && save
          //     manually triple_index = cb_triple->currentIndex(); bottom =
          //     centerpieceParameters[ triple_index ][1].toDouble();  //Should
          //     be from centerpiece info from protocol write_triple();
          //   }
        }
    }
  ...
  // // ALEXEY: before processing next optics system (if multiple), push triple
  // names to the general triple array:
  ...
  qDebug () << "SAVING: Optics Type, all_processed:  " << filename_runID_auto
            << all_processed;
  ...
  ***/
  ...
  if (autoflow_expType == "ABDE")
    {
      ...
      // Just update status of autoflow record (to 'ANALYSIS' with no
      // analysisIDs) update_autoflow_record_atEditData_abde( dbP );
      QString AnalysisIDsstr = QString ("NULL");
      update_autoflow_record_atEditData (dbP, AnalysisIDsstr);

      QMessageBox::information (
          this, tr ("Saving of Edit Profiles is Complete."),
  ...
  // Now, remove duplicates from channels array, fill QMap keeping track on if
  // reference wavelength set for each channel (if MWL)
  channels_all.removeDuplicates ();
  triples_all_optics.removeDuplicates (); // Absence of this caused incorrect
                                          // analyses list: need to test!!!

  for (int i = 0; i < channels_all.size (); ++i)
    {
      qDebug () << "BEFORE AUTOFLOW_ANALYSIS: channel name --"
                << channels_all[i]; // RA:S:: "1.S"
      isSet_ref_wvl[channels_all[i]] = false;
    }

  // DEBUG
  for (int j = 0; j < triples_all_optics.size (); ++j)
    qDebug () << "BEFORE AUTOFLOW_ANALYSIS: triple name -- "
              << triples_all_optics[j]; // RA:S:: "1.S.280"

  // Process triples by channel, generate appropriate JSON (with or without
  // 2DSA_FM stage) for autoflowAnalysis record && create those records
      ...
      for (int j = 0; j < triples_all_optics.size (); ++j)
        {
          if (triples_all_optics[j].contains (channels_all[i]))
            {
              qDebug () << "AUTOFLOW_ANALYSIS: triple "
                        << triples_all_optics[j] << " ,containing channel "
                        << channels_all[i];
              ...
              if (triples_all_optics[j].contains ("Interference"))
                {
                  if (isSet_to_analyse (triples_all_optics[j],
                                        QString ("Interf"))
                      && isSet_to_analyse_triple (triples_all_optics[j],
                                                  QString ("Interf")))
                    {
                      // ALEXEY: here for combined run, the filename_runID_auto
                      // would be incorrectly set to that of the second type,
                      // the "-RI" Need to use "-IP"
                      if (runType_combined_IP_RI)
                      ...
                      ID = create_autoflowAnalysis_record (
                          dbP, triples_all_optics[j], json_status);
              ...
              // UV.vis
              else
                {
                  if (isSet_to_analyse (triples_all_optics[j],
                    ...
                    {
                      // ALEXEY: here for combined run, the filename_runID_auto
                      // must be "filename_base-RI" Need to use "-RI"
                      if (runType_combined_IP_RI)
                      ...
                      if (!isSet_ref_wvl[channels_all[i]])
                        {
                          // If there is an infortmaiton on specific triple to
                          // edit (for a current channel) in the AProfile?
                          if (isSet_edit_info_for_channel (
                                  triples_all_optics[j], QString ("UV/vis")))
                            {
                              qDebug ()
                                  << "isSet_edit_info_for_channel() TRUE: ";
                              // If this particular  triple set for FM fit (and
                              // edit): defined by "wvl_edit" attr in Aprofile
                              if (isSet_to_edit_triple (triples_all_optics[j],
                                                        QString ("UV/vis")))
                                {
                                  json_status = compose_json (true);
                                  qDebug ()
                                      << "isSet_to_edit_triple() TRUE: "
                                      << triples_all_optics[j] << json_status;

                                  ID = create_autoflowAnalysis_record (
                                      dbP, triples_all_optics[j], json_status);

                                  // So, reference wvl is defiend as the triple
                                  // specified in AProfile
                                  isSet_ref_wvl[channels_all[i]] = true;

                                  if (ID)
                                    create_autoflowAnalysisStages_record (dbP,
                                                                          ID);
                                }
                              else
                                {
                          ...
                          // No informaiton on specifi triple to edit -- select
                          // 1st triple in a channel as default
                          else
                            {
                              json_status = compose_json (true);
                              qDebug ()
                                  << "isSet_edit_info_for_channel() FALSE: "
                                  << triples_all_optics[j] << json_status;

                              ID = create_autoflowAnalysis_record (
                                  dbP, triples_all_optics[j], json_status);

                              // So, reference wvl is defiend as the 1st one in
                              // the channel domain
                              isSet_ref_wvl[channels_all[i]] = true;
  ...
  // If AnalysisIDs empty, STOP here and return: Nothing to process further;
  // emit signal to reset
  ...
  // Otherwise, proceed to analysis: Now we need to Update autoflow record,
  // reset GUI && send signal to switch to Analysis stage:
  ...
  QMessageBox::information (this, tr ("Saving of Edit Profiles is Complete."),
                            tr ("Edit profiles were saved successfully. \n\n"
                                "The program will switch to Analysis stage."));
  reset ();
  this->close ();
...
// Record statuses of the auto/manual meniscus determinaiton, on per-channel
// basis
void
US_Edit::record_edit_status (QMap<QString, bool> auto_meniscus, QString dtype)
{
  // DEBUG
  qDebug () << "Data Type: " << dtype;
  QMap<QString, bool>::iterator os;
  for (os = auto_meniscus.begin (); os != auto_meniscus.end (); ++os)
    qDebug () << "For channel " << os.key ()
              << ", meniscus determined automatically? " << os.value ();
  // END DEBUG
  ...
  qDebug () << "IN US_edit_AUTO, record status: ID,name,email,lev" << ID
  ...
  editRI_IP_Json
      += "\"Comment when SAVED\": \"" + gmp_submitter_map["Comment:"] + "\",";
  ...
  for (os = auto_meniscus.begin (); os != auto_meniscus.end (); ++os)
    {
      QString meniscus_method
          = os.value () ? QString ("automated") : QString ("manual");
      ...
      if (!os.value ())
        {
          editRI_IP_Json
              += ", Comment: " + manual_edit_comments[os.key ()] + "\",";
        }
      else
        editRI_IP_Json += "\",";
    }
  ...
  if (autoflowStatusID)
      ...
      if (dtype == "RI" || dtype == "RA")
        {
          qry << "update_autoflowStatusEditRI_record"
              ...
              << QString::number (autoflowID_passed) << editRI_IP_Json;
  ...
  else
    {
      QMessageBox::warning (
          ...
          tr ("autoflowStatus (EDIT {RI|RA,IP}): There was a problem with "
              "identifying "
...
void
US_Edit::delete_autoflow_record (void)
{
...
bool
US_Edit::isSet_to_analyse (QString triple_name, QString opsys)
{
  bool have_run = false;

  // In case anaprofile does not have "run" attr, so QMap<> channels_to_analyse
  // empty (older protocols):
  if (channels_to_analyse.isEmpty ())
    {
      qDebug () << "It looks like older protocol is in use: QMap "
                   "channels_to_analyse is EMPTY!";
  ...
  for (jj = channels_to_analyse.begin (); jj != channels_to_analyse.end ();
       ++jj)
    {
      if (jj.key ().contains (channel) && jj.key ().contains (opsys))
        {
          if (jj.value ())
            {
              qDebug () << "Triple " << triple_name << " of channel ("
                        << jj.key () << " will be analysed.";
              have_run = true;
              break;
            }
        }
...
bool
US_Edit::isSet_to_analyse_triple (QString triple_name, QString opsys)
{
  bool have_run = true;

  // In case anaprofile does not have "wvl_not_run" attr, so QMap<>
  // triples_skip_analysis empty (older protocols):
  if (triples_skip_analysis.isEmpty ())
    {
      qDebug () << "It looks like older protocol is in use: QMap "
                   "triples_skip_analysis is EMPTY!";
  ...
  for (jj = triples_skip_analysis.begin (); jj != triples_skip_analysis.end ();
       ++jj)
    {
      if (jj.key ().contains (channel) && jj.key ().contains (opsys))
        {
...
bool
US_Edit::isSet_edit_info_for_channel (QString triple_name, QString opsys)
{
  bool have_set = false;
      ...
      if (jj.key ().contains (channel) && jj.key ().contains (opsys))
        {
          ...
          if (!wvl_set_edit.isEmpty ())
            {
              qDebug () << "Edit Informaiton for channel " << channel
                        << " is SET.";
              have_set = true;
              break;
            }
...
bool
US_Edit::isSet_to_edit_triple (QString triple_name, QString opsys)
{
  bool have_run = false;
...
QString
US_Edit::compose_json (bool fm_stage)
{
  QString json;
...
void
US_Edit::update_autoflow_record_atEditData_abde (US_DB2 *db)
{
  QStringList qry;

  // using existing proc for switch from analysis stage...
  qry << "update_autoflow_at_analysis" << QString::number (autoflowID_passed);
...
void
US_Edit::update_autoflow_record_atEditData (US_DB2 *db,
                                            QString &AnalysisIDsString)
{
  QString runID_numeric = details_at_editing_local["runID"];
  QString OptimaName = details_at_editing_local["OptimaName"];
  ...
  //     << AnalysisIDsString
  //     << OptimaName;
  qry << "update_autoflow_at_edit_data" << AnalysisIDsString
      << QString::number (autoflowID_passed);
...
// Function to create a single autoflowAnalysis record: Pass some fields from
// autoflow table (set analysises IDs, filename etc.)
int
US_Edit::create_autoflowAnalysis_record (US_DB2 *db, QString &tripleName,
                                         QString &status_json)
{
  // create single record in autoflowAnalysis: return ID (auto-incremented), &&
  // update/push
  ...
  // first, check max(requestID) in the autoflowAnalysisHistory table && set
  // AUTO_INCREMENT in the autoflowAnalysis table to: greater of:
  ...
  QString status = QString ("Saving AutoflowAnalysis Profile for Triple %1")
                       .arg (tripleName);
  le_status->setText (status);
  qApp->processEvents ();

  qry.clear ();
  qry << "new_autoflow_analysis_record" << tripleName << filename_runID_auto
      << AProfileGUID << idInv_auto << status_json
      << QString::number (autoflowID_passed);
...
void
US_Edit::create_autoflowAnalysisStages_record (US_DB2 *db, int ID)
{
  QStringList qry;
  qry << "new_autoflow_analysis_stages_record" << QString::number (ID);
...
void
US_Edit::write_triple_auto (int trx)
{
  triple_index = trx;
  ...
  le_status->setText (
      tr ("Saving edit profile for channel %1").arg (triple_name));
  qApp->processEvents ();
  ...
  // ALEXEY: the bottom value affected by rotor stretch - time consuming !!! Do
  // we need it ? US_SimulationParameters simparams; simparams.initFromData(
  // dbP, data, false, runID, dataType ); bottom         = simparams.bottom;

  bottom = centerpieceParameters[trx][1]
               .toDouble (); // Should be from centerpiece info from protocol
  ...
  */
  ...
  if (meniscus >= range_left)
    {
      QMessageBox::critical (
          this, tr ("Meniscus/Data_Left Inconsistent"),
  ...
  if (plateau >= range_right)
    {
      QMessageBox::critical (
          this, tr ("Plateau/Data_Right Inconsistent"),
  ...
  QString now = QDateTime::currentDateTime ().toUTC ().toString ("yyMMddhhmm");
  ...
  QString rptfpath
      = QString (workingDir).replace ("/results", "/reports") + rptfname;
  ...
  while (editLabel.isEmpty ())
    {
      editGUIDs[idax].clear ();
      QString now
          = QDateTime::currentDateTime ().toUTC ().toString ("yyMMddhhmm");

      bool ok;
      QString msg
          = tr ("The base Edit Label for this edit session is <b>") + now
            + "</b> .<br/>"
      ...
      if (sufx != isufx)
        {
          QMessageBox::critical (
              this, tr ("Character content error"),
...
US_Edit::apply_prior (void)
{
  ...
  if (disk_controls->db ())
    {
      ...
      if (editDataIDs.size () == 0)
        {
          QMessageBox::warning (
              this, tr ("Edit data is not in DB"),
  ...
  else
    {
      ...
      filepath = QFileDialog::getOpenFileName (
          this, tr ("Select a saved edit file"), workingDir, filter);
    }
  ...
  QString triple = filename.section (".", -4, -2).replace (".", " / ");
  int idax = triples.indexOf (triple);
  ...
  if (nledits > 1)
    {
      ...
      mbox.setTextFormat (Qt::RichText);
      mbox.setText (msg);
      pb_selo = mbox.addButton (tr ("Selected Only"), QMessageBox::RejectRole);
      pb_allw
          = mbox.addButton (tr ("All Wavelengths"), QMessageBox::AcceptRole);
...
US_Edit::prior_equil (void)
{
  ...
  for (int ii = 0; ii < cefnames.size (); ii++)
    { // Read and apply edits from edit files
      ...
      le_dataRange->setText (
          wkstr.sprintf ("%.3f - %.3f", range_left, range_right));
      le_dataStart->setText (QString::number (range_left, 'f', 8));
      le_dataEnd->setText (QString::number (range_right, 'f', 8));
      pb_dataEnd->setIcon (check);
      pb_dataEnd->setEnabled (true);
  ...
  if ((row + 1) <= cb_triple->count ())
    {
      ...
      int dax = index_data ();
      // int dax = mwl_data.data_index( iwavl_edit_ref[ triple_index ],
      // triple_index );
      ...
      qDebug () << "NEXT Triple: row " << row << ", cb_triple->count() "
                << cb_triple->count ();
    }
  else
    pb_nextChan->setEnabled (row < cb_triple->count ());
...
// Advance to next triple and plot edited curves
void
US_Edit::prior_triple_auto (void)
{
  pb_nextChan->setEnabled (true);
  ...
  if (row >= 0)
    {
      ...
      // if ( le_edtrsp->isVisible() )
      // 	 {
      // 	   QString trsp = cb_triple->currentText() + " : " + trip_rpms[
      // 0 ]; 	   le_edtrsp->setText( trsp ); 	   cb_rpms  ->setCurrentIndex( 0 );
      // 	 }
      ...
      pb_nextChan->setEnabled (true);
      qDebug () << "PRIOR Triple: row " << row << ", cb_triple->count() "
                << cb_triple->count ();
    }
  else
    pb_priorChan->setEnabled (false);
...
US_Edit::all_edits_done (void)
{
  ...
  if (expIsEquil)
    ...
    {
      all_ed_done = (range_left != 0 && range_right != 0 && plateau != 0
                     && baseline != 0);
    }
...
void
US_Edit::update_disk_db (bool isDB)
{
  if (isDB)
...
void
US_Edit::progress_load (QString progress)
{
...
void
US_Edit::show_mwl_controls (bool show)
{
  if (!us_edit_auto_mode)
    {
...
void
US_Edit::connect_mwl_ctrls (bool conn)
{
  if (conn)
    {
      connect (rb_lrange, SIGNAL (toggled (bool)), this,
               SLOT (lselect_range_on (bool)));
      connect (rb_custom, SIGNAL (toggled (bool)), this,
               SLOT (lselect_custom_on (bool)));
      connect (ct_ldelta, SIGNAL (valueChanged (double)), this,
               SLOT (ldelta_value (double)));
      connect (cb_lstart, SIGNAL (currentIndexChanged (int)), this,
               SLOT (lambda_start_value (int)));
      connect (cb_lend, SIGNAL (currentIndexChanged (int)), this,
               SLOT (lambda_end_value (int)));
      connect (rb_radius, SIGNAL (toggled (bool)), this,
               SLOT (xaxis_radius_on (bool)));
      connect (rb_waveln, SIGNAL (toggled (bool)), this,
               SLOT (xaxis_waveln_on (bool)));
      connect (pb_custom, SIGNAL (clicked ()), this,
               SLOT (lambda_custom_list ()));
      connect (pb_incall, SIGNAL (clicked ()), this,
               SLOT (lambda_include_all ()));
      connect (cb_lplot, SIGNAL (currentIndexChanged (int)), this,
               SLOT (lambda_plot_value (int)));
      connect (pb_larrow, SIGNAL (clicked ()), this,
               SLOT (lambda_plot_prev ()));
      connect (pb_rarrow, SIGNAL (clicked ()), this,
               SLOT (lambda_plot_next ()));
    }
...
void
US_Edit::lselect_range_on (bool checked)
{
...
// Lambda selection has been changed to Range or Custom
void
US_Edit::lselect_custom_on (bool checked)
{
...
void
US_Edit::ldelta_value (double value)
{
...
void
US_Edit::lambda_start_value (int value)
{
  int new_slambda = cb_lstart->itemText (value).toInt ();
  if (new_slambda == slambda) // check if new start is same as current start ->
                              // skip every update
    {
      return;
    }
  else if (new_slambda > elambda) // check if new start is greater than end
    {
      ...
      slambda = elambda;
      elambda = new_slambda;
      cb_lstart->setCurrentIndex (
          cb_lstart->findText (QString::number (slambda)));
...
void
US_Edit::lambda_end_value (int value)
{
  int new_elambda = cb_lend->itemText (value).toInt ();
  if (new_elambda == elambda) // check if new end is same as current end ->
                              // skip every update
    {
      return;
    }
...
US_Edit::reset_plot_lambdas ()
{
  ...
  le_lxrng->setText (tr ("%1 MWL exports: %2 %3 to %4,")
                         ...
                         .arg (elambda)
                     + (lsel_range
                            ? tr (" raw index increment %1.").arg (dlambda)
...
// turn on and off scan control widgets and lambda widgets as switching to x
// axis wavelength
void
US_Edit::xaxis_wavl_wgts_on (bool on)
{
...
void
US_Edit::xaxis_radius_on (bool checked)
{
...
// X-axis has been changed to Radius or Wavelength
void
US_Edit::xaxis_waveln_on (bool checked)
{
...
void
US_Edit::lambda_plot_value (int value)
{
  if (value < 0)
    return;
...
void
US_Edit::lambda_plot_next ()
{
  DbgLv (1) << "lambda_plot_next  clicked";
  plotndx++;

  int lstx
      = xaxis_radius ? (expc_wvlns.size () - 1) : (expc_radii.size () - 1);
...
US_Edit::lambda_custom_list ()
{
  ...
  connect (sel_lambd, SIGNAL (new_lambda_list (QVector<int>)), this,
           SLOT (lambda_new_list (QVector<int>)));
...
void
US_Edit::lambda_new_list (QVector<int> newlams)
{
...
void
US_Edit::od_radius_limit (double value)
{
...
void
US_Edit::write_mwl_auto (int trx)
{
  ...
  */
  ...
  int     kwavelo  = mwl_data.lambdas( oldi_wvlns );      //ALEXEY: <-- current
  channel, needs to be looped over channels int     nwavelo  =
  expi_wvlns.count(); int     wvx; bool    chg_lamb = ( kwavelo != nwavelo );
  ...
  ***********/
  ...
  QString celchn = celchns.at (
      triple_index); // ALEXEY: <-- is it  lopped over triple indecies?
  QString scell = celchn.section ("/", 0, 0).simplified ();
  QString schan = celchn.section ("/", 1, 1).simplified ();
  QString tripbase = scell + " / " + schan + " / ";
  int idax = triples.indexOf (tripbase + current_wvlns_list[0]);
  // int     odax     = index_data_auto( trx, 0 ); //CORRECT! {first odax in a
  // triple}

  // qDebug() << "Write_MWL:  triple_index, #wvlns, odax, celchn" <<
  // triple_index << "," << curr_wvls_count << ", " << odax << "," << celchn;

  QString filebase = files[idax].section (".", 0, -6) + "." + editLabel + "."
                     + files[idax].section (".", -5, -5) + "." + scell + "."
                     + schan + ".";
  ...
  for (int wvx = 0; wvx < current_wvlns_list.size ();
       wvx++) // ALEXEY: needs to be looped over channels, not only current
              // channel
    {
      ...
      idax = triples.indexOf (triple);
      // odax             = index_data_auto( trx, wvx ); // Correct ? NOT!!!
      // Got you !!

      qDebug () << "EDT:WrMwl:  wvx triple" << wvx << triple << "filename"
                << filename << ", trx " << trx << ", wvx "
                << wvx; // << ", odax " << odax;

      // also, for ABDE, deal with includes / excludes && BAseline correction:
      // includes = ...
      if (autoflow_expType == "ABDE")
        {
          includes = editProfile_includes[triple_name];
          ...
          bl_corr_yintercept
              = editProfile_blc[triple_name][swavl][1].toDouble ();
        }
      ...
      //  select rawDataGUID from rawData where filename like
      //  '%MartinR_EcoRI_Digest_GMP_Optima1_23OCT23-run1985%2.A.260%';
      //  filename:
      //  MartinR_EcoRI_Digest_GMP_Optima1_23OCT23-run1985.23102502280.RI.2.A.260.xml
      QString triplename_qry_wvl = triplename_qry + swavl;

      QString rawGUID
          = get_rawDataGUID (dbP, filebase_qry, triplename_qry_wvl);
      ...
      if (disk_controls->db ())
        {
          ...
          // Output the edit database record
          wrstat = write_edit_db (dbP, filename, editGUID, editID, rawGUID);
          DbgLv (1) << "EDT:WrMwl:  dax fname" << idax << filename << "wrstat"
                    << wrstat;
...
QString
US_Edit::get_rawDataGUID (US_DB2 *db, QString filebase_qry,
                          QString triplename_qry)
{
...
US_Edit::write_mwl ()
{
      ...
      QString msg
          = tr ("The base Edit Label for this edit session is <b>") + now
            ...
            + tr ("You may add an optional suffix to further distinquish<br/>"
                  "the Edit Label. Use alphanumeric characters, "
                  "underscores,<br/>"
  ...
  DbgLv (1) << "EDT:WrMwl: files,wvlns.size" << files.size ()
            << expc_wvlns.size ();
  for (wvx = 0; wvx < expc_wvlns.size (); wvx++)
    {
      ...
      idax = triples.indexOf (triple);
      odax = index_data (wvx);
      DbgLv (1) << "EDT:WrMwl:  wvx triple" << wvx << triple << "filename"
                << filename;

      DbgLv (1) << "EDT:WrMwl:   idax,editGUIDs.size" << idax
                << editGUIDs.size ();
      QString editGUID = editGUIDs[idax];
      ...
      QString rawGUID
          = US_Util::uuid_unparse ((unsigned char *)outData[odax]->rawGUID);
...
// Write edit xml file
int
US_Edit::write_xml_file (QString &fname, QString &triple, QString &editGUID,
                         QString &rawGUID)
{
  ...
  if (!efo.open (QFile::WriteOnly | QFile::Text))
    {
      QMessageBox::information (
          this, tr ("File write error"),
          tr ("Could not open the file\n") + workingDir + fname
              + tr ("\n for writing.  Check your permissions."));
      return 1;
    }

  DbgLv (1) << "EDT:WrXml: IN: fname,triple,editGUID,rawGUID" << fname
            << triple << editGUID << rawGUID;
  ...
  QStringList parts
      = triple.contains (" / ") ? triple.split (" / ") : triple.split (".");
  ...
  if (us_edit_auto_mode) //<-- write excluded scans (beg|end) based on AProfile
    {
      ...
      for (int ii = 0; ii < data.scanData.size (); ii++)
        {
          if (!includes.contains (ii) && ii >= scanExcl_begin_ind
          ...
          for (int jj = 0; jj < e->changes.size (); jj++)
            {
              ...
              xml.writeAttribute (
                  "radius", QString::number (e->changes[jj].x (), 'f', 8));
              xml.writeAttribute (
                  "value", QString::number (e->changes[jj].y (), 'f', 8));
              xml.writeEndElement ();
            }
        }
  ...
  if (!expIsEquil)
    { // non-Equilibrium
      ...
      xml.writeAttribute ("radius", QString::number (bottom, 'f', 8));
      xml.writeEndElement ();

      if (dataType == "IP")
        {
          xml.writeStartElement ("air_gap");
          xml.writeAttribute ("left", QString::number (airGap_left, 'f', 8));
          xml.writeAttribute ("right", QString::number (airGap_right, 'f', 8));
      ...
      xml.writeAttribute ("left", QString::number (range_left, 'f', 8));
      xml.writeAttribute ("right", QString::number (range_right, 'f', 8));
      xml.writeEndElement ();

      xml.writeStartElement ("plateau");
      xml.writeAttribute ("radius", QString::number (plateau, 'f', 8));
      xml.writeEndElement ();

      xml.writeStartElement ("baseline");
      xml.writeAttribute ("radius", QString::number (baseline, 'f', 8));
      xml.writeEndElement ();

      xml.writeStartElement ("od_limit");
      xml.writeAttribute ("value", QString::number (odlimit, 'f', 8));
      xml.writeEndElement ();
  ...
  if (!pb_residuals->icon ().isNull () || !pb_spikes->icon ().isNull ()
      || invert == -1.0 || floatingData || is_spike_auto)
    {
      ...
      if (!us_edit_auto_mode)
        ...
        {
          if (is_spike_auto)
            // if ( is_spike_auto || dataType == "IP" ) //ALEXEY: for now, for
            // interference data which are processed manually....
            {
              xml.writeStartElement ("remove_spikes");
              xml.writeEndElement ();
            }
...
int
US_Edit::write_edit_db (US_DB2 *dbP, QString &fname, QString &editGUID,
                        QString &editID, QString &rawGUID)
{
  ...
  if (dbP->lastErrno () != US_DB2::OK)
    {
      QMessageBox::warning (this, tr ("AUC Data is not in DB"),
  ...
  if (editID.isEmpty ())
    {
      query << "new_editedData" << rawDataID << editGUID << runID << fname
            << "";
    ...
    {
      query << "update_editedData" << editID << rawDataID << editGUID << runID
            << fname << "";
      dbP->query (query);
...
int
US_Edit::index_data_auto (int trx, int wvx)
{
  ...
  qDebug () << "Index_data_auto: triple_name, isMwl -- " << triple_name
            << isMwl;
    ...
    { // For MWL, compute data index from wavelength and triple indexes
      if (wvx < 0)
        { // For the default case, use the current wavelength index
          plotndx = cb_lplot->currentIndex ();
          int iwavl = expi_wvlns[plotndx]; // ALEXEY: do we need to set data in
                                           // the middle of the triple set ??

          // int iwavl    = expi_wvlns[ 0 ];                      // ALEXEY: OR
  ...
  qDebug () << "Triple index in INDEX_DATA: trx, wvx, data_index: "
            << triple_index << ", " << wvx << ", " << data_index;
    ...
    { // For MWL, compute data index from wavelength and triple indexes
      if (wvx < 0)
        { // For the default case, use the current wavelength index
          ...
          data_index = mwl_data.data_index (iwavl, triple_index);
          odatx = data_index;
          DbgLv (1) << "IxDa: dx" << data_index << "plx wavl trx" << plotndx
                    << iwavl << triple_index;
        }
...
int
US_Edit::like_edit_files (QString filename, QStringList &editfiles,
                          US_DB2 *dbP)
{
  ...
  QStringList edfiles
      = QDir (workingDir).entryList (filter, QDir::Files, QDir::Name);
  QStringList ldfiles;
    ...
    { // Determine database like files and compare to local
      ...
      while (dbP->next ())
        { // Accumulate filenames and editIDs from the database
          ...
          QString efname = dbP->value (2).toString ().section ("/", -1, -1);
          QString triple = efname.section (".", -4, -2).replace (".", " / ");
          // DbgLv(1) << "LiEdFi:   db0,2" << idData << efname << "fbase" <<
          // filebase;
          ...
          if (efname.startsWith (filebase) && triples.contains (triple))
            {
              dbfiles << efname;
              dbEdIds << idData;
              DbgLv (1) << "LiEdFi:     id fn dbfsiz" << idData << efname
                        << dbfiles.count ();
            }
        }
...
int
US_Edit::apply_edits (US_DataIO::EditValues parameters)
{
  int status = 0;
...
int
US_Edit::lambdas_by_cell (int trx)
{
...
double
US_Edit::radius_indexed (const double radi)
{
...
void
US_Edit::create_report_auto (QString &ss, int trx)
{
  //*DEBUG*
...
// Create general data set information report file
void
US_Edit::create_report (QString &ss)
{
  //*DEBUG*
...
void
US_Edit::save_report_auto (const QString rtext, const QString rptfpath,
  ...
  if (dbP != NULL)
    {
      ...
      int st = freport.saveFileDocuments_auto (
          idInv_auto.toInt (), pfdir, rfiles, dbP, // include invID explicitly
          idEdit, tripdesc);
...
// Save data set report
void
US_Edit::save_report (const QString rtext, const QString rptfpath,
                      const int idEdit)
{
  ...
  if (dbP != NULL)
    {
      ...
      // Write the report record to the database
      int st
          = freport.saveFileDocuments (pfdir, rfiles, dbP, idEdit, tripdesc);
...
QString
US_Edit::indent (const int spaces)
{
...
QString
US_Edit::table_row (const QString s1, const QString s2)
{
...
QString
US_Edit::table_row (const QString s1, const QString s2, const QString s3)
{
...
QString
US_Edit::table_row (const QString s1, const QString s2, const QString s3,
                    const QString s4, const QString s5, const QString s6)
{
  return (indent (6) + "<tr><td>" + s1 + "</td><td>" + s2 + "</td><td>" + s3
...
QString
US_Edit::html_header (const QString title, const QString head1)
{
  int trx = index_data ();
...
// Compose HTML header string
QString
US_Edit::html_header_auto (const QString title, const QString head1,
                           const int trx_curr)
{
...
US_Edit::run_details (void)
{
  ...
  if (maxTemp - minTemp <= US_Settings::tempTolerance ())
    ss += table_row (tr ("Temperature Variation:"), tr ("Within tolerance"));
  else
  ...
  ss += table_row (tr ("Time Correction:"), QString::number (minutes) + mm
  ...
  if (ddType == "WI")
    dataType = tr ("Intensity:");
  if (ddType == "IP")
    dataType = tr ("Interference:");
  if (ddType == "FI")
    dataType = tr ("Fluorescence:");
  ...
  ss += table_row (tr ("Run Duration:"),
                   QString::number (hours) + hh + QString::number (minutes)
                       + mm + QString::number (seconds) + sec);

  // Wavelength, baseline, meniscus, range
  int iwvln = qRound (dd->scanData.last ().wavelength);
  QString bln_od
      = QString (le_baseline->text ()).section ("(", 1, 1).section (")", 0, 0)
        + " OD";
  QString left = le_dataStart->text ();
  QString right = le_dataEnd->text ();
  QString plat = le_plateau->text ();

  ss += table_row (tr ("Wavelength:"), QString::number (iwvln) + " nm")
        + table_row (tr ("Baseline ") + dataType, bln_od);
  table_row (tr ("Meniscus Position:"), le_meniscus->text () + " cm");
...
QString
US_Edit::run_details_auto (int trx)
{
  US_DataIO::RawData *dd = outData[index_data_auto (trx)];
  ...
  ss += table_row (tr ("Wavelength:"), QString::number (iwvln) + " nm")
        + table_row (tr ("Baseline ") + dataType, bln_od)
        + table_row (tr ("Meniscus Position:"), meniscus_r + " cm");
...
US_Edit::scan_info (void)
{
  ...
  ss += table_row (tr ("Scan"), tr ("Corrected Time"),
                   tr ("Plateau Concentration"), tr ("Seconds"),
                   tr ("Omega^2T"), tr ("Raw Speed"));
...
QString
US_Edit::scan_info_auto (int trx)
{
  US_DataIO::RawData *dd = outData[index_data_auto (trx)];
  double time_correction = US_Math2::time_correction (allData);
...
US_Edit::pass_values_bll (void)
{
  ...
  int status = QMessageBox::information (
      this, tr ("New Baseline Correction Parameters"),
...
void
US_Edit::pass_values (void)
{
  // collect menicsus | airGap | baseline | range | etc data && pass to main
  // edit stage
  ...
  triple_params << QString::number (meniscus) << QString::number (range_left)
                << QString::number (range_right) << QString::number (plateau)
                << QString::number (baseline) << QString::number (baseline_od)
                << QString ("spike_false");
  ...
  qDebug () << "In PASSING values manually: triple_name, parms -- "
            << triple_name << editProfile_triple[triple_name];
  qDebug () << "In PASSING values manually: includes -- " << triple_name
            << editProfile_triple_includes[triple_name];
  qDebug () << "In PASSING values manually: linear-baseline-corr -- "
            << triple_name << editProfile_triple_blc[triple_name];
  qDebug () << "In PASSING values manually: us_edit_auto_mode BEFORE -- "
            << triple_name << us_edit_auto_mode;

  int status = QMessageBox::information (
      this, tr ("New Edit Parameters"),
...
//
void
US_Edit::close_manual_edit (void)
{
...
US_Edit::close_edit (void)
{
  ...
  if (nunedit > 0)
    { // Some triples unedited:  give option to cancel close
      ...
      mbox.setTextFormat (Qt::RichText);
      mbox.setText (msg);
      pb_cancel = mbox.addButton (tr ("Cancel"), QMessageBox::RejectRole);
      pb_doclose = mbox.addButton (tr ("Close"), QMessageBox::AcceptRole);
...
//       //if (mainw->width() - offset > sdiag->width() || mainw->height() -
//       2*offset > sdiag->height()) { if ( new_main_w > sdiag->width() ||
//       new_main_h > sdiag->height()) {
...
//       //if (mainw->width() < sdiag->width() || mainw->height() <
//       sdiag->height()) { if ( new_main_w < sdiag->width() ||  new_main_h <
//       sdiag->height() ) {
