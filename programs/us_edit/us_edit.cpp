...
US_Edit::US_Edit(QString auto_mode) : US_Widgets() {
   ...
   int lmbdhi = 650;
   int lmbddl = 1;
   QString lrsmry = tr("%1 raw: %2 %3 to %4").arg(nlmbd).arg(chlamb).arg(lmbdlo).arg(lmbdhi);
   ...
   connect(cb_triple, SIGNAL(currentIndexChanged(int)), SLOT(new_triple_auto(int)));
   ...
   connect(ct_odlim, SIGNAL(valueChanged(double)), SLOT(od_radius_limit(double)));
...
*/
   ...
   plot = new US_Plot(data_plot, tr("Absorbance Data"), tr("Radius (in cm)"), tr("Absorbance"), true, "", "rainbow");
...
US_Edit::US_Edit(
   QVector<US_DataIO::RawData> allData, QStringList triples, QString workingDir, int currenChtInd, int plotind,
   QString exptype) : US_Widgets() {
   pb_removeAllbutLast = us_pushbutton(tr("Remove All but Last"), true);
   pb_baseline_correct = us_pushbutton(tr("Correct Baseline"), false);
   pb_bll_modify = us_pushbutton(tr("Modify Baseline Correction for Selected Triple"), false);
   lb_baseline_correct = us_banner(tr("Linear Baseline Correction"));
...
US_Edit::US_Edit(
   QVector<US_DataIO::RawData> allData, QStringList triples, QString workingDir, int currenChtInd, int plotind,
   QString exptype, QStringList editParams, QList<int> editParams_includes) : US_Widgets() {
   pb_removeAllbutLast = us_pushbutton(tr("Remove All but Last"), true);
   pb_baseline_correct = us_pushbutton(tr("Correct Baseline"), false);
   pb_bll_modify = us_pushbutton(tr("Modify Baseline Correction for Selected Triple"), false);
   lb_baseline_correct = us_banner(tr("Linear Baseline Correction"));
...
void US_Edit::sel_investigator(void) {
   ...
   QString inv_text = QString::number(investigator) + ": " + US_Settings::us_inv_name();
...
void US_Edit::details(void) {
   US_RunDetails2 *dialog = new US_RunDetails2(allData, runID, workingDir, triples);
   dialog->exec();
   qApp->processEvents();
   delete dialog;
}
   ...
   foreach (s, data.scanData) {
      ...
      if (maxGap >= threshold) {
         ...
         if (!deleteAll) {
            ...
            gaps = tr("Scan ") + QString::number(rawScanNumber) + tr(" has a maximum reading gap of ")
                   + QString::number(maxGap) + tr(" starting at radius ") + QString::number(radius, 'f', 3);

            box.setText(gaps);
            box.setInformativeText(tr("Delete?"));

            QPushButton *pb_delete = box.addButton(tr("Delete"), QMessageBox::YesRole);

            QPushButton *pb_deleteAll = box.addButton(tr("Delete All"), QMessageBox::AcceptRole);

            QPushButton *pb_skip = box.addButton(tr("Skip"), QMessageBox::NoRole);

            QPushButton *pb_cancel = box.addButton(tr("Cancel"), QMessageBox::RejectRole);
...
void US_Edit::load_auto(QMap<QString, QString> &details_at_editing) {
   ...
   qDebug() << "autoflowID_passed, dataSource, ProtocolName_auto, autoflow_expType : " << autoflowID_passed
            << dataSource << ProtocolName_auto << autoflow_expType;
   ...
   qDebug() << "autoflowID, intensityID, autoflowStatusID : " << autoflowID_passed
            << details_at_editing[ "intensityID" ] << autoflowStatusID;
...
void US_Edit::process_optics_auto() {
   ...
   if (dataType == "IP") {
      ...
      connect(ct_gaps, SIGNAL(valueChanged(double)), SLOT(set_fringe_tolerance(double)));
   }
   else {
      lb_gaps->setText(tr("Threshold for Scan Gaps"));
   ...
   for (int trx = 0; trx < triples.size(); trx++) { // Generate file names
      ...
      if (!rawc_wvlns.contains(swavl)) { // Accumulate wavelengths in case this is MWL
         nwaveln++;
         rawc_wvlns << swavl;
      }
      ...
      if (!celchns.contains(celchn)) { // Accumulate cell/channel values in case this is MWL
         ncelchn++;
         celchns << celchn;
      }
   }
   ...
   if (expType.isEmpty() && disk_controls->db()) { // no experiment type yet and data read from DB:  try for DB exp type
      if (dbP == NULL) {
         US_Passwd pw;
   ...
   if (expIsEquil) { // Equilibrium
      ...
      pick->disconnect();
      connect(pick, SIGNAL(cMouseUp(const QwtDoublePoint &)), SLOT(mouse(const QwtDoublePoint &)));
   ...
   DbgLv(1) << "LD(): isMwl" << isMwl << "nwaveln" << nwaveln << toti_wvlns.size() << "ntriple" << ntriple;
   ...
   if (isMwl) { // Set values related to MultiWaveLength
      ...
      for (int ccx = 0; ccx < ncelchn; ccx++) { // Save lambdas for each cell; flag if any cell-to-cell differences
         QVector<int> wvs;
      ...
      dlambda = 1;
      le_ltrng->setText(tr("%1 raw: %2 %3 to %4").arg(nwaveln).arg(chlamb).arg(slambda).arg(elambda));
      ...
      for (int ii = 0; ii < triple.scanData.size(); ii++) {
         ...
         qDebug() << "scan" << ii + 1 << "delta-r rpm seconds" << ds->delta_r << rpmc << timec << "omega2t w2t-integ"
                  << ds->omega2t << w2ti;
         if (ii == 0) {
   ...
   connect(ct_odlim, SIGNAL(valueChanged(double)), this, SLOT(od_radius_limit(double)));
   ...
   qDebug() << "job1run, job2run, job3run, job3auto, job4run, job5run, job6run_pcsa: " << job1run << ", " << job2run
            << ", " << job3run << ", " << job3auto << ", " << job4run << ", " << job5run << ", " << job6run_pcsa;
   ...
   for (int trx = 0; trx < cb_triple->count(); trx++) {
      ...
      qDebug() << "Range_right && Loading volume FROM AProfile: " << triple_name << ", " << range_right << ", "
               << aprofile_volume;
      ...
      if (dataType != "IP") {
         ...
         triple_info.clear();
         triple_info << QString::number(meniscus) << QString::number(range_left) << QString::number(range_right)
                     << QString::number(plateau) << QString::number(baseline) << QString::number(baseline_od)
                     << QString("spike_false");
         ...
         triple_info << QString::number(meniscus) << QString::number(range_left) << QString::number(range_right)
                     << QString::number(plateau) << QString::number(baseline) << QString::number(baseline_od)
                     << QString("spike_false") << QString::number(airGap_left) << QString::number(airGap_right);
   ...
   for (int trx = 0; trx < cb_triple->count(); trx++) {
      ...
      while (i != cell_to_centerpiece.constEnd()) {
         if (triple_name_cell.contains(i.key())) {
            qDebug() << "triple_name_cell, cell, centerpiece: " << triple_name_cell << ", " << i.key() << ": "
                     << i.value();
      ...
      if (xmli.isStartElement()) {
         ...
         if (ename == "channel_parms") {
            ...
            if (attr.hasAttribute("load_volume")) {
               ...
               aprof_parms << attr.value("load_volume").toString() << attr.value("data_end").toString();
               ...
               if (attr.hasAttribute("scan_excl_begin")) {
                  ...
                  scan_excl_pairs << attr.value("scan_excl_begin").toString() << attr.value("scan_excl_end").toString();
...
void US_Edit::read_centerpiece_params(int trx) {
   ...
   centparms_info << centerpiece_info[ "name" ] << centerpiece_info[ "bottom" ] << centerpiece_info[ "pathlen" ]
   ...
   for (int i = start_scan; i < end_scan; i++) {
	...
	++countPoints;
      ...
      ***************/
      ...
      double Slope = (s->rvalues[ indexRight_fit ] * invert - s->rvalues[ indexLeft_fit ] * invert)
...
double US_Edit::find_meniscus_interference_auto() {
   ...
   double meniscus_init = sqrt(
      bottom_db * bottom_db
      - (aprofile_volume * 360 / (1000 * pathlength_db * angle_db * M_PI))); //Radians = Degrees * (M_PI/180.0)

   qDebug() << "Meniscus_init: INTERFERENCE " << meniscus_init << ", " << bottom_db << ", " << pathlength_db << ", "
            << angle_db << ", " << M_PI;
   ...
   for (int i = start_scan; i < end_scan; i++) {
	...
	++countPoints;
      ...
      **********************************************/
      ...
      /********** TESTITNG *******************************************************/
      //Alternative: derive a curve passing through Left/Right indecies
      double Slope = (s->rvalues[ indexRight ] * invert - s->rvalues[ indexLeft_fit ] * invert)
...
double US_Edit::find_meniscus_auto() {
   ...
   qDebug() << "Meniscus_init: " << meniscus_init << ", " << bottom_db << ", " << pathlength_db << ", " << angle_db
            << ", " << M_PI;
...
int US_Edit::scan_db_auto(void) {
   ...
   while (db.next()) { // Accumulate data description objects from database information
      ...
      QString idata = label + "^" + runID + "^" + tripID + "^" + filename + "^" + rawGUID + "^" + rawDataID + "^"
                      + date;
...
void US_Edit::create_descs_auto(QStringList &runIDs, QStringList &infoDs, int naucf) {
   ...
   for (int ii = 0; ii < naucf; ii++) {
      ...
      if (datamap.contains(lkey)) { // Handle the case where the lkey already exists
...
void US_Edit::load_db_auto(QList<DataDesc_auto> &sdescs) {
   ...
   for (int ii = 0; ii < sdescs.count(); ii++) { // Loop to load selected data from the database
      ...
      if (QFile(filename).exists()) { // AUC file exists, do only download if checksum+size mismatch
...
void US_Edit::load(void) {
   ...
   connect(dialog, SIGNAL(progress(QString)), this, SLOT(progress_load(QString)));
...
void US_Edit::set_data_over_lamda() {
   ...
   for (int ccx = 0; ccx < ncelchn; ccx++) { // Handle each triple of AUC data
      ...
      for (int ii = 0; ii < xvals.size(); ii++) {
         trx = trx_b;
         int txval = static_cast<int>(qRound(xvals.at(ii) * 1000));
         for (int jj = 0; jj < nwaveln; jj++) { // Each wavelength in the current cell
            edata = &allData[ trx ]; // Triple data
            int iwavl = rawi_wvlns[ jj ]; // Wavelength value
            int wvx = toti_wvlns.indexOf(iwavl); // Wavelength index
            DbgLv(1) << "IS-MWL:   trx ccx wvx" << trx << ccx << wvx;
   ...
   switch (step) {
      case MENISCUS:
         ...
         if (dataType != "IP") {
            ...
            QPen pen(brush, 2.0);
            QwtSymbol *symb = new QwtSymbol(QwtSymbol::Cross, brush, pen, QSize(8, 8));
      ...
      case BASELINE: {
         ...
         if (pt - start < 5 || end - pt < 5) {
            QMessageBox::warning(this, tr("Position Error"), tr("The selected point is too close to the edge."));
            return;
         }
      ...
      case BASELINE_LINEAR_CORR:
         qDebug() << "bl_corr_left, bl_corr_right {xx,yy} -- " << bl_corr_left_x << bl_corr_right_x << bl_corr_left_y
                  << bl_corr_right_y;
         if (bl_corr_left_x == 0.0) {
            bl_corr_left_x = radius_indexed(p.x());
            bl_corr_left_y = p.y();

            qDebug() << "[FIRST]bl_corr_left, bl_corr_right {xx,yy} -- " << bl_corr_left_x << bl_corr_right_x
            ...
            qDebug() << "[SECOND]bl_corr_left, bl_corr_right {xx,yy} -- " << bl_corr_left_x << bl_corr_right_x
                     << bl_corr_left_y << bl_corr_right_y;
         }
...
void US_Edit::plot_all(void) {
   ...
   for (int i = 0; i < data.scanData.size(); i++) {
      ...
      QString title = tr("Raw Data at ") + QString::number(s->seconds) + tr(" seconds") + " #" + QString::number(i);
...
void US_Edit::plot_range_and_blc(void) {
   ...
   for (int i = 0; i < data.scanData.size(); i++) {
      ...
      if (expIsEquil) {
         ...
         QwtPlotCurve *c = us_curve(data_plot, tr("Meniscus at") + QString::number(tScan));
...
void US_Edit::plot_last(void) {
   ...
   QString title = tr("Raw Data at ") + QString::number(s->seconds) + tr(" seconds") + " #"
...
void US_Edit::plot_scan(void) {
   ...
   for (int ii = 0; ii < ssize; ii++) {
      ...
      QString title = tr("Raw Data at ") + QString::number(s->seconds) + tr(" seconds") + " #" + QString::number(ii);
...
void US_Edit::plot_mwl(void) {
   ...
   if (us_edit_auto_mode && all_loaded && !triple_plot_first_time[ triple_index ]) {
      ...
      qDebug() << "In PLOT_MWL when all loaded: triple_index, iwavl_edit_ref[ triple_index ], iwavl_edit_ref_index[ "
                  "triple_index , data_index -- "
               << triple_index << ", " << iwavl_edit_ref[ triple_index ] << ", " << iwavl_edit_ref_index[ triple_index ]
               << ", " << data_index;
   ...
   if (xaxis_radius) {
      ...
      DbgLv(1) << "PlMwl:     c triple" << scell << schan << svalu << "d triple" << dcell << dchan << dwavl;
   }
   ...
   if (xaxis_radius) { // Build normal AUC data plot
      ...
      for (int ii = 0; ii < nscan; ii++) {
         ...
         QString ctitle = tr("Raw Data at ") + QString::number(scn->seconds) + tr(" seconds") + " #"
                          + QString::number(ii);
...
void US_Edit::exclusion(void) {
   ...
   connect(exclude, SIGNAL(update_exclude_profile(QList<int>)), this, SLOT(update_excludes(QList<int>)));

   connect(exclude, SIGNAL(cancel_exclude_profile(void)), this, SLOT(cancel_excludes(void)));

   connect(exclude, SIGNAL(finish_exclude_profile(QList<int>)), this, SLOT(finish_excludes(QList<int>)));
...
void US_Edit::edit_scan(void) {
   ...
   US_EditScan *dialog = new US_EditScan(data.scanData[ scan ], data.xvalues, invert, range_left, range_right);
   connect(dialog, SIGNAL(scan_updated(QList<QPointF>)), SLOT(update_scan(QList<QPointF>)));
   dialog->exec();
   qApp->processEvents();
   delete dialog;
}
...
void US_Edit::remove_spikes(void) {
   ...
   for (int i = 0; i < data.scanData.size(); i++) {
      ...
      for (int j = start; j < end; j++) {
         if (US_DataIO::spike_check(*s, data.xvalues, j, start, end, &smoothed_value)) {
            ...
            while (c & (1 << (7 - index % 8))) {
               if (US_DataIO::spike_check(*s, data.xvalues, index, start, end, &smoothed_value))
...
void US_Edit::noise(void) {
   residuals.clear();
   US_RiNoise *dialog = new US_RiNoise(data, includes, range_left, range_right, dataType, noise_order, residuals);
...
void US_Edit::new_triple_auto(int index) {
   ...
   qDebug() << "Triple_name, cb_triple->currentText() -- " << triple_name << cb_triple->currentText();
...
*/
   ...
   dataType = QString(QChar(data.type[ 0 ])) + QString(QChar(data.type[ 1 ]));
   DbgLv(1) << "EDT:NewTr:   sw tri dx" << swavl << triple << idax << "dataType" << dataType << "gap_fringe gap_thresh"
            << gap_fringe << gap_thresh;
   ...
   if (expIsEquil) { // Equilibrium
      ...
      for (int ii = 0; ii < data.scanData.size(); ii++) { // build unique-rpm list for triple
         QString arpm = QString::number(data.scanData[ ii ].rpm);
   ...
   qDebug() << "IN new_Triple_auto(): triple -- " << cb_triple->currentText() << "ScanBegin -- " << scanExcl_begin_ind
...
void US_Edit::new_triple(int index) {
   ...
   if (changes_made) {
      ...
      mb.setIcon(QMessageBox::Question);
      mb.setText(tr("Ignore Edits?"));
      mb.setInformativeText(tr("Edits have been made.  If you want to keep them,\n"
                               "cancel and write the outputs first."));
      mb.addButton(tr("Ignore Edits"), QMessageBox::RejectRole);
      mb.addButton(tr("Return to previous selection"), QMessageBox::NoRole);
      int result = mb.exec();
...
void US_Edit::write(void) {
   ...
   if (!expIsEquil) { // non-Equilibrium:  write single current edit (if "all" unchecked)
      if (isMwl && ck_writemwl->isChecked()) { // Write edits for all triples in the current cell/channel
         write_mwl();
      }
...
void US_Edit::correct_bll_for_triple_auto(void) {
   ...
   sdiag_bll = new US_Edit(
      allData, triples, workingDir, currChIndex, plotInd, autoflow_expType, editParams, editParams_includes);
   ...
   //add slots
   connect(
      sdiag_bll, SIGNAL(pass_edit_params_blc(QMap<QString, QStringList> &)), this,
      SLOT(update_triple_edit_params_blc_modified(QMap<QString, QStringList> &)));
...
void US_Edit::manual_edit_auto(void) {
   ...
   connect(
      sdiag, SIGNAL(pass_edit_params(QMap<QString, QStringList> &)), this,
      SLOT(update_triple_edit_params(QMap<QString, QStringList> &)));

   connect(
      sdiag, SIGNAL(pass_edit_params_includes(QMap<QString, QList<int>> &)), this,
      SLOT(update_triple_edit_params_includes(QMap<QString, QList<int>> &)));

   connect(
      sdiag, SIGNAL(pass_edit_params_blc(QMap<QString, QStringList> &)), this,
...
void US_Edit::update_triple_edit_params(QMap<QString, QStringList> &edit_params) {
   ...
   if (autoflow_expType != "ABDE") {
      ...
      QString comment_t
         = QInputDialog::getText(this, tr("Reason for MANUAL editing"), msg, QLineEdit::Normal, default_text, &ok);
...
void US_Edit::write_auto(void) {
  ...
  */
   ...
   qDebug() << "autoflowID_passed, autoflow status, isSaved_auto(): " << autoflowID_passed << ", "
   ...
   US_Passwd pw_at;
   gmp_submitter_map
      = pw_at.getPasswd_auditTrail("GMP Run EDIT Form", "Please fill out GMP run EDIT form:", user_submitter);
   ...
   qDebug() << "Submitter map: " << gmp_submitter_map.keys() << gmp_submitter_map.keys().size()
            << gmp_submitter_map_size << gmp_submitter_map.keys().isEmpty() << gmp_submitter_map[ "User:" ]
   ...
   ***/
   ...
   for (int i = 0; i < channels_all.size(); ++i) {
      ...
      for (int j = 0; j < triples_all_optics.size(); ++j) {
         if (triples_all_optics[ j ].contains(channels_all[ i ])) {
            qDebug() << "AUTOFLOW_ANALYSIS: triple " << triples_all_optics[ j ] << " ,containing channel "
                     << channels_all[ i ];
            ...
            if (triples_all_optics[ j ].contains("Interference")) {
               if (
                  isSet_to_analyse(triples_all_optics[ j ], QString("Interf"))
            ...
            //UV.vis
               ...
               if (
                  isSet_to_analyse(triples_all_optics[ j ], QString("UV/vis"))
...
void US_Edit::record_edit_status(QMap<QString, bool> auto_meniscus, QString dtype) {
   ...
   if (autoflowStatusID) {
      ...
      if (dtype == "RI" || dtype == "RA") {
         qry << "update_autoflowStatusEditRI_record" << QString::number(autoflowStatusID)
      ...
      if (dtype == "IP") {
         qry << "update_autoflowStatusEditIP_record" << QString::number(autoflowStatusID)
             << QString::number(autoflowID_passed) << editRI_IP_Json;
...
void US_Edit::delete_autoflow_record(void) {
   ...
   if (status == US_DB2::NO_AUTOFLOW_RECORD) {
      QMessageBox::warning(
         this, tr("Autoflow Record Not Deleted"),
...
void US_Edit::update_autoflow_record_atEditData(US_DB2 *db, QString &AnalysisIDsString) {
   ...
   //     << AnalysisIDsString
   //     << OptimaName;
   qry << "update_autoflow_at_edit_data" << AnalysisIDsString << QString::number(autoflowID_passed);
...
int US_Edit::create_autoflowAnalysis_record(US_DB2 *db, QString &tripleName, QString &status_json) {
   ...
   qry.clear();
   qry << "new_autoflow_analysis_record" << tripleName << filename_runID_auto << AProfileGUID << idInv_auto
       << status_json << QString::number(autoflowID_passed);
...
void US_Edit::write_triple_auto(int trx) {
   ...
   */
   ...
   if (!ss.isEmpty()) {
      QMessageBox::information(
         this, tr("Data missing"), tr("You must define the ") + ss + tr(" before writing the edit profile."));
      return;
   }
   ...
   if (disk_controls->db()) {
      if (dbP == NULL) {
         ...
         if (dbP == NULL || dbP->lastErrno() != US_DB2::OK) {
            QMessageBox::warning(
               this, tr("Connection Problem"), tr("Could not connect to database \n") + dbP->lastError());
            return;
         }
      }
...
void US_Edit::write_triple(void) {
   ...
   if (expIsEquil) { // Equilibrium:  set baseline,plateau as flag that those are "done"
      int jsd = sd_offs[ triple_index ];
      meniscus = sData[ jsd ].meniscus;
      range_left = sData[ jsd ].dataLeft;
      range_right = sData[ jsd ].dataRight;
   ...
   while (editLabel.isEmpty()) {
      ...
      bool ok;
      QString msg = tr("The base Edit Label for this edit session is <b>") + now + "</b> .<br/>"
      ...
      sufx = QInputDialog::getText(this, tr("Create a unique session Edit Label"), msg, QLineEdit::Normal, sufx, &ok);
   ...
   QString filename = files[ idax ];
   DbgLv(1) << "EDT:WrTripl: tripindex" << triple_index << "idax" << idax << "filename" << filename;
   QString rpart = filename.section(".", 0, -6);
   QString tpart = filename.section(".", -5, -2);
   filename = rpart + "." + editLabel + "." + tpart + ".xml";
   QString wvpart = "";
...
void US_Edit::apply_prior(void) {
   ...
   if (disk_controls->db()) {
      ...
      // Error check
      if (dbP->lastErrno() != US_DB2::OK) {
         QMessageBox::warning(this, tr("AUC Data is not in DB"), tr("Cannot find the raw data in the database.\n"));
      ...
      if (editDataIDs.size() == 0) {
         QMessageBox::warning(this, tr("Edit data is not in DB"), tr("Cannot find any edit records in the database.\n"));
      ...
      // Ask for edit file
      filepath = QFileDialog::getOpenFileName(this, tr("Select a saved edit file"), workingDir, filter);
   }
   ...
   if (nledits > 1) {
      ...
      if (mbox.clickedButton() == pb_selo) { // Only apply for the selected file
         ...
         for (int ii = 0; ii < nledits; ii++) { // Save selected edit file name; use "same" for others in channel
   ...
   if (result != US_DataIO::OK) {
      QMessageBox::warning(
         this, tr("XML Error"), tr("An error occurred when reading edit file\n\n") + US_DataIO::errorString(result));
      return;
   }
   ...
   if (parameters.dataGUID != uuid) {
      QMessageBox::warning(this, tr("Data Error"), tr("The edit file was not created using the current data"));
...
void US_Edit::prior_equil(void) {
   ...
   if (disk_controls->db()) { // Get prior equilibrium edits from DB
      ...
      for (int ii = 1; ii < outData.size(); ii++) {
         data = *outData[ ii ];
         q.clear();
         q << "get_rawDataID_from_GUID" << US_Util::uuid_unparse(( uchar * ) data.rawGUID);
         db.query(q);
         db.next();
      ...
      // Ask for edit file
      filename = QFileDialog::getOpenFileName(this, tr("Select a saved edit file"), workingDir, filter);
      ...
      for (int ii = 0; ii < files.size(); ii++) {
         filename = files[ ii ];
         filename = runID + "." + editLabel + "." + filename.section(".", -5, -2) + ".xml";
         filename = workingDir + filename;
   ...
   for (int ii = 0; ii < cefnames.size(); ii++) { // Read and apply edits from edit files
      data = *outData[ ii ];
      ...
      le_dataRange->setText(wkstr.sprintf("%.3f - %.3f", range_left, range_right));
      le_dataStart->setText(QString::number(range_left, 'f', 8));
      le_dataEnd->setText(QString::number(range_right, 'f', 8));
      pb_dataEnd->setIcon(check);
      pb_dataEnd->setEnabled(true);
      ...
      if (noise_order > 0) {
         US_RiNoise::calc_residuals(data, includes, range_left, range_right, noise_order, residuals);
...
bool US_Edit::all_edits_done(void) {
   ...
   if (expIsEquil) {
      ...
      for (int jd = 0; jd < outData.size(); jd++) { // Examine each data set to evaluate whether edits complete
         int jsd = sd_offs[ jd ];
      ...
      all_ed_done = (range_left != 0 && range_right != 0 && plateau != 0 && baseline != 0);
   }
...
void US_Edit::connect_mwl_ctrls(bool conn) {
   if (conn) {
      connect(rb_lrange, SIGNAL(toggled(bool)), this, SLOT(lselect_range_on(bool)));
      connect(rb_custom, SIGNAL(toggled(bool)), this, SLOT(lselect_custom_on(bool)));
      connect(ct_ldelta, SIGNAL(valueChanged(double)), this, SLOT(ldelta_value(double)));
      connect(cb_lstart, SIGNAL(currentIndexChanged(int)), this, SLOT(lambda_start_value(int)));
      connect(cb_lend, SIGNAL(currentIndexChanged(int)), this, SLOT(lambda_end_value(int)));
      connect(rb_radius, SIGNAL(toggled(bool)), this, SLOT(xaxis_radius_on(bool)));
      connect(rb_waveln, SIGNAL(toggled(bool)), this, SLOT(xaxis_waveln_on(bool)));
      ...
      connect(cb_lplot, SIGNAL(currentIndexChanged(int)), this, SLOT(lambda_plot_value(int)));
...
void US_Edit::reset_plot_lambdas() {
   ...
   DbgLv(1) << "rpl: dl sl el px" << dlambda << slambda << elambda << plotx << "sx ex nr" << strtx << endx
            << rawc_wvlns.size();
   DbgLv(1) << "rpl: trx" << triple_index << cb_triple->currentIndex();
   DbgLv(1) << "rpl: rcw 0 1 m n" << rawc_wvlns[ 0 ] << rawc_wvlns[ 1 ] << rawc_wvlns[ nwaveln - 2 ]
            << rawc_wvlns[ nwaveln - 1 ];
   expc_wvlns.clear();
   expi_wvlns.clear();

   for (int ii = strtx; ii < endx; ii += dlambda) { // Accumulate new list of export lambdas by looking at all raw lambas
   ...
   le_lxrng->setText(
      tr("%1 MWL exports: %2 %3 to %4,").arg(nwavelo).arg(chlamb).arg(slambda).arg(elambda)
      + (lsel_range ? tr(" raw index increment %1.").arg(dlambda) : tr(" from custom selections.")));
...
void US_Edit::lambda_plot_value(int value) {
   ...
   else if (step != MENISCUS) { // New wavelength has same edit as others in channel:  make sure applied
...
void US_Edit::lambda_custom_list() {
   ...
   connect(sel_lambd, SIGNAL(new_lambda_list(QVector<int>)), this, SLOT(lambda_new_list(QVector<int>)));
...
void US_Edit::lambda_new_list(QVector<int> newlams) {
   ...
   expc_wvlns.clear();
   DbgLv(1) << "EDT:lnl: nnssee" << nwavelo << expi_wvlns.size() << slambda << newlams[ 0 ] << elambda
            << newlams[ nwavelo - 1 ];
...
void US_Edit::write_mwl_auto(int trx) {
   ...
   */
   ...
   if (!str.isEmpty()) {
      QMessageBox::information(
         this, tr("Data missing"), tr("You must define the ") + str + tr(" before writing the edit profile."));
      return;
   }
   //END check
   ...
   ***********/
   ...
   QString filebase = files[ idax ].section(".", 0, -6) + "." + editLabel + "." + files[ idax ].section(".", -5, -5)
                      + "." + scell + "." + schan + ".";
   ...
   for (int wvx = 0; wvx < current_wvlns_list.size();
        wvx++) //ALEXEY: needs to be looped over channels, not only current channel
   {
      ...
      qDebug() << "EDT:WrMwl:  wvx triple" << wvx << triple << "filename" << filename << ", trx " << trx << ", wvx "
               << wvx; // << ", odax " << odax;
...
void US_Edit::write_mwl() {
   ...
   if (!chg_lamb) { // If no change in number of wavelengths, check actual lists
      for (wvx = 0; wvx < nwavelo; wvx++) {
         if (oldi_wvlns[ wvx ] != expi_wvlns[ wvx ]) { // There is a difference in the lists:  mark as such
            chg_lamb = true;
            break;
         }
      }
   ...
   if (chg_lamb) { // If wavelengths have changed, save new list and rebuild some vectors
      mwl_data.set_lambdas(expi_wvlns); // Save new lambdas for channel
      std::sort(expi_wvlns.begin(), expi_wvlns.end());
      reset_outData();
   }
   ...
   for (wvx = 0; wvx < expc_wvlns.size(); wvx++) {
      ...
      QString rawGUID = US_Util::uuid_unparse(( unsigned char * ) outData[ odax ]->rawGUID);
...
// Write edit xml file
int US_Edit::write_xml_file(QString &fname, QString &triple, QString &editGUID, QString &rawGUID) {
   ...
   DbgLv(1) << "EDT:WrXml: IN: fname,triple,editGUID,rawGUID" << fname << triple << editGUID << rawGUID;
   ...
   QStringList parts = triple.contains(" / ") ? triple.split(" / ") : triple.split(".");
   ...
   if (us_edit_auto_mode) //<-- write excluded scans (beg|end) based on AProfile
   {
      ...
      for (int ii = 0; ii < data.scanData.size(); ii++) {
         if (!includes.contains(ii) && ii >= scanExcl_begin_ind && ii < data.scanData.size() - scanExcl_end_ind) {
            xml.writeStartElement("exclude");
            xml.writeAttribute("scan", QString::number(ii));
            xml.writeEndElement();
         }
   ...
   if (!changed_points.isEmpty()) {
      ...
      for (int ii = 0; ii < changed_points.size(); ii++) {
         ...
         for (int jj = 0; jj < e->changes.size(); jj++) {
            ...
            xml.writeAttribute("radius", QString::number(e->changes[ jj ].x(), 'f', 8));
            xml.writeAttribute("value", QString::number(e->changes[ jj ].y(), 'f', 8));
            xml.writeEndElement();
         }
      }
   ...
   if (!expIsEquil) { // non-Equilibrium
      ...
      if (dataType == "IP") {
         ...
         xml.writeAttribute("tolerance", QString::number(ct_gaps->value(), 'f', 8));
         xml.writeEndElement();
      }
      ...
      if (autoflow_expType == "ABDE") {
         ...
         xml.writeAttribute("y_intercept", QString::number(bl_corr_yintercept, 'f', 8));
         xml.writeEndElement();
      }
   }
   ...
   if (!pb_residuals->icon().isNull() || !pb_spikes->icon().isNull() || invert == -1.0 || floatingData || is_spike_auto) {
...
int US_Edit::write_edit_db(US_DB2 *dbP, QString &fname, QString &editGUID, QString &editID, QString &rawGUID) {
   int idEdit;
   ...
   if (editID.isEmpty()) {
      ...
      if (dbP->lastErrno() != US_DB2::OK) {
         QMessageBox::warning(
            this, tr("Database Problem"), tr("Could not insert metadata into the database\n") + dbP->lastError());
      ...
      query << "update_editedData" << editID << rawDataID << editGUID << runID << fname << "";
      dbP->query(query);

      if (dbP->lastErrno() != US_DB2::OK) {
         QMessageBox::warning(
            this, tr("Database Problem"), tr("Could not update metadata in the database \n") + dbP->lastError());
   ...
   if (dbP->lastErrno() != US_DB2::OK) {
      QMessageBox::warning(
         this, tr("Database Problem"), tr("Could not insert edit xml data into database \n") + dbP->lastError());
      return 5;
   }
...
int US_Edit::index_data_auto(int trx, int wvx) {
   ...
   if (isMwl) { // For MWL, compute data index from wavelength and triple indexes
      if (wvx < 0) { // For the default case, use the current wavelength index
         ...
         odatx = data_index;
         DbgLv(1) << "IxDa_AUTO: dx" << data_index << "plx wavl trx" << plotndx << iwavl << triple_index;
      }
   ...
   qDebug() << "Triple index in INDEX_DATA: trx, wvx, data_index: " << triple_index << ", " << wvx << ", "
            << data_index;
...
int US_Edit::index_data(int wvx) {
   ...
   if (isMwl) { // For MWL, compute data index from wavelength and triple indexes
      if (wvx < 0) { // For the default case, use the current wavelength index
         ...
         data_index = mwl_data.data_index(iwavl, triple_index);
         odatx = data_index;
         DbgLv(1) << "IxDa: dx" << data_index << "plx wavl trx" << plotndx << iwavl << triple_index;
      }
...
int US_Edit::like_edit_files(QString filename, QStringList &editfiles, US_DB2 *dbP) {
   ...
   QStringList edfiles = QDir(workingDir).entryList(filter, QDir::Files, QDir::Name);
   QStringList ldfiles;
   ...
   if (dbP != NULL) { // Determine database like files and compare to local
      ...
      if (nledits != nlocals) { // DB records are not matched locally, so download from DB
         if (nlocals == 0)
            QDir().mkpath(workingDir);

         dbfiles.sort();

         for (int ii = 0; ii < dbfiles.count(); ii++) { // Download each DB record to a local file
   ...
   if (nledits == 0) { // No files (should not happen):  return at least the specified file name
      editfiles << filename;
      nledits = 1;
   }
...
void US_Edit::reset_outData() {
   ...
   for (int ccx = 0; ccx < ncelchn; ccx++) {
      ...
      DbgLv(1) << "rsoD: ccx kwv" << ccx << kwvln << ex_wvlns.size() << "nwv ccoff" << nwaveln << ccoff;
   ...
   DbgLv(1) << "rsoD:  final ccoff" << ccoff << "aDa size" << allData.size() << "oDa size" << outData.size();
}
...
void US_Edit::create_report_auto(QString &ss, int trx) {
   ...
   for (int js = 0; js < dd->scanData.size(); js++) {
      DbgLv(1) << "CR: js" << js << "secs" << dd->scanData[ js ].seconds << "speed" << dd->scanData[ js ].rpm;
   }
   //*DEBUG*
...
void US_Edit::view_report(void) {
   ...
   tedit->e->setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize()));
   tedit->e->setText(rtext);
   tedit->show();
}

// Save data set report
void US_Edit::save_report_auto(const QString rtext, const QString rptfpath, const int idEdit, const int trx) {
...
// Save data set report
void US_Edit::save_report(const QString rtext, const QString rptfpath, const int idEdit) {
   // Write report text to the report file
   QFile f_rep(rptfpath);
...
QString US_Edit::table_row(const QString s1, const QString s2, const QString s3, const QString s4, const QString s5) {
   return (
...
QString US_Edit::html_header(const QString title, const QString head1) {
   ...
   QString editLbl = editLabel.isEmpty() ? editFnames[ trx ].section(".", -6, -6) : editLabel;
...
QString US_Edit::run_details(void) {
   ...
   QString ss = "\n" + indent(4) + tr("<h3>Detailed Run Information:</h3>\n") + indent(4) + "<table>\n"
                + table_row(tr("Cell Description:"), dd->description) + table_row(tr("Data Directory:"), workingDir)
   ...
   if (maxTemp - minTemp <= US_Settings::tempTolerance())
      ...
      ss += table_row(tr("Temperature Variation:"), tr("(!) OUTSIDE TOLERANCE (!)"));
   ...
   ss += table_row(tr("Time Correction:"), QString::number(minutes) + mm + QString::number(seconds) + sec);
   ...
   ss += table_row(
      tr("Run Duration:"), QString::number(hours) + hh + QString::number(minutes) + mm + QString::number(seconds) + sec);

   // Wavelength, baseline, meniscus, range
   int iwvln = qRound(dd->scanData.last().wavelength);
   QString bln_od = QString(le_baseline->text()).section("(", 1, 1).section(")", 0, 0) + " OD";
   QString left = le_dataStart->text();
   QString right = le_dataEnd->text();
   QString plat = le_plateau->text();

   ss += table_row(tr("Wavelength:"), QString::number(iwvln) + " nm") + table_row(tr("Baseline ") + dataType, bln_od);
   table_row(tr("Meniscus Position:"), le_meniscus->text() + " cm");


   ss += table_row(tr("Edited Data starts at:"), left + " cm") + table_row(tr("Edited Data stops at:"), right + " cm ")
...
QString US_Edit::scan_info(void) {
   ...
   ss += table_row(
      tr("Scan"), tr("Corrected Time"), tr("Plateau Concentration"), tr("Seconds"), tr("Omega^2T"), tr("Raw Speed"));
...
void US_Edit::pass_values_bll(void) {
   ...
   //Linear-baseline-correction
   triple_params_blc << QString::number(bl_corr_slope) << QString::number(bl_corr_yintercept);
...
void US_Edit::pass_values(void) {
   ...
   triple_params << QString::number(meniscus) << QString::number(range_left) << QString::number(range_right)
                 << QString::number(plateau) << QString::number(baseline) << QString::number(baseline_od)
                 << QString("spike_false");
   ...
   if (dataType == "IP") {
      triple_params << QString::number(airGap_left) << QString::number(airGap_right);
   }
   ...
   qDebug() << "In PASSING values manually: linear-baseline-corr -- " << triple_name
            << editProfile_triple_blc[ triple_name ];
