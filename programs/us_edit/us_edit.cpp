...
US_Edit::US_Edit(QString auto_mode) : US_Widgets() {
  ...
  if (US_Settings::us_inv_level() < 3) pb_investigator->setEnabled(false);
  ...
  int lmbddl = 1;
  QString lrsmry =
      tr("%1 raw: %2 %3 to %4").arg(nlmbd).arg(chlamb).arg(lmbdlo).arg(lmbdhi);
  ...
  lo_writemwl = us_checkbox(tr("Save to all Wavelengths"), ck_writemwl, true);
  ...
  */
  ...
  // // Interference Data WITH existing Aprofile corresponding to existing
  // protocol!!! details[ "invID_passed" ] = QString("6"); details[ "filename" ]
  // = QString("001-Amy-MWL-if-test-3-run794-IP"); details[ "protocolName" ] =
  // QString("001-Amy-MWL-if-test-3");
  // //
  // /****************************************************************************************/
  ...
  // = QString("588"); details[ "autoflowID" ]   = QString("1515"); details[
  // "runID" ]        = QString(""); details[ "OptimaName" ]   = QString("");
  // details[ "expType" ]      = QString("ABDE");
...
US_Edit::US_Edit(QVector<US_DataIO::RawData> allData, QStringList triples,
  ...
  connect(pb_investigator, SIGNAL(clicked()), SLOT(sel_investigator()));
  connect(pb_load, SIGNAL(clicked()), SLOT(load()));
  connect(cb_triple, SIGNAL(currentIndexChanged(int)), SLOT(new_triple(int)));
  connect(pb_exclusion, SIGNAL(clicked()), SLOT(exclusion()));
  connect(pb_edit1, SIGNAL(clicked()), SLOT(edit_scan()));
    ...
    if (maxGap >= threshold) {
      ...
      if (!deleteAll) {
        ...
        gaps = tr("Scan ") + QString::number(rawScanNumber) +
               ...
               tr(" starting at radius ") + QString::number(radius, 'f', 3);
        ...
        QPushButton* pb_skip = box.addButton(tr("Skip"), QMessageBox::NoRole);
        ...
        if (box.clickedButton() == pb_skip) continue;

        if (box.clickedButton() == pb_cancel) return;
      }
...
void US_Edit::process_optics_auto() {
  ...
  for (int trx = 0; trx < triples.size(); trx++) {  // Generate file names
    ...
    if (!rawc_wvlns.contains(
            swavl)) {  // Accumulate wavelengths in case this is MWL
      nwaveln++;
      rawc_wvlns << swavl;
    }
    ...
    if (!celchns.contains(
            celchn)) {  // Accumulate cell/channel values in case this is MWL
      ncelchn++;
      celchns << celchn;
    }
  }
  ...
  if (expType.isEmpty() &&
      disk_controls->db()) {  // no experiment type yet and data read from DB:
                              // try for DB exp type
    if (dbP == NULL) {
      US_Passwd pw;
  ...
  if (expIsEquil) {  // Equilibrium
    ...
    for (int jd = 0; jd < allData.size(); jd++) {
      ...
      if (jd > 0) sd_knts << (ksd - sd_offs[jd - 1]);
      ...
      if (jd == 0) cb_rpms->addItems(trip_rpms);
    ...
    connect(cb_rpms, SIGNAL(currentIndexChanged(int)), SLOT(new_rpmval(int)));
  }

  else {  // non-Equilibrium
  ...
  if (dt > US_Settings::tempTolerance()) {
    QMessageBox::warning(
        ...
        tr("The temperature in this run varied over the course\n"
           ...
           "current threshold (") +
            QString::number(US_Settings::tempTolerance(), 'f', 1) + " " + DEGC +
  ...
  if (isMwl) {  // Set values related to MultiWaveLength
    connect_mwl_ctrls(false);
    ...
    for (int ccx = 0; ccx < ncelchn;
         ccx++) {  // Save lambdas for each cell; flag if any cell-to-cell
                   // differences
      QVector<int> wvs;
    ...
    //             int iwavl     = rawi_wvlns[ jwx ];             // Wavelength
    //             value int wvx       = toti_wvlns.indexOf( iwavl );   //
    //             Wavelength index
    ...
    //                int odx       = ccx * nrpoint;         // Output dataset
    //                index int opx       = scx * maxwavl + wvx;   // Output
    //                point index
    // DbgLv(2) << "IS-MWL:    scx odx opx" << scx << odx << opx;
    //                for ( int rax = 0; rax < nrpoint; rax++ )
    //                {  // Store ea. radius data point as a wavelength point in
    //                a scan
  ...
  qDebug()
      << "job1run, job2run, job3run, job3auto, job4run, job5run, job6run_pcsa: "
      << job1run << ", " << job2run << ", " << job3run << ", " << job3auto
      << ", " << job4run << ", " << job5run << ", " << job6run_pcsa;
  ...
  for (int trx = 0; trx < cb_triple->count(); trx++) {
    ...
    if (isMwl) {
      ...
      if (isSet_edit_info_for_channel(channel_desc, opsys)) {
        ...
        for (jjj = triple_to_edit.begin(); jjj != triple_to_edit.end(); ++jjj) {
          qDebug() << "jjj.key(), channel_desc, opsys -- " << jjj.key()
                   << channel_desc << opsys;
        ...
        qDebug() << "Wvl index && value -- " << iwavl_edit_ref_index[trx]
                 << iwavl_edit_ref[trx];
      }
    ...
    qDebug() << "Range_right && Loading volume FROM AProfile: " << triple_name
             << ", " << range_right << ", " << aprofile_volume;
    ...
    if (dataType != "IP") {
      ...
      triple_info.clear();
      triple_info << QString::number(meniscus) << QString::number(range_left)
      ...
      // Un-zoom
      if (plot->btnZoom->isChecked()) plot->btnZoom->setChecked(false);
      ...
      triple_info << QString::number(meniscus) << QString::number(range_left)
                  << QString::number(range_right) << QString::number(plateau)
                  << QString::number(baseline) << QString::number(baseline_od)
                  << QString("spike_false") << QString::number(airGap_left)
  ...
  if (editProfile.count() == cb_triple->count()) all_loaded = true;
  ...
  if (us_edit_auto_mode) pick->disconnect();
}
...
void US_Edit::read_aprofile_data_from_aprofile() {
  ...
  for (int trx = 0; trx < cb_triple->count(); trx++) {
    ...
    // iterate over aprofile_channel_to_parms QMap and check if channel name is
    // a part of triple_name:
...
void US_Edit::read_centerpiece_names_from_protocol() {
  ...
  while (!xmli.atEnd()) {
    ...
    if (xmli.isStartElement()) {
      ...
      if (ename == "cells") readProtocolCells_auto(xmli);

      if (ename == "aprofile") readProtocolAProfile_auto(xmli);
    }
  }
  ...
  for (int trx = 0; trx < cb_triple->count(); trx++) {
    ...
    while (i != cell_to_centerpiece.constEnd()) {
      if (triple_name_cell.contains(i.key())) {
        qDebug() << "triple_name_cell, cell, centerpiece: " << triple_name_cell
                 << ", " << i.key() << ": " << i.value();
      ...
      if (ename == "channel_parms") {
        ...
        if (attr.hasAttribute("load_volume")) {
          ...
          if (attr.hasAttribute("scan_excl_begin")) {
            ...
            qDebug() << "READING aprof XML, scans excl.: " << channel_name
                     << ", " << scan_excl_pairs;
...
QVector<double> US_Edit::find_airGap_interference_auto() {
  ...
  for (int i = start_scan; i < end_scan; i++) {
    ...
    ***************/
    ...
    double Slope = (s->rvalues[indexRight_fit] * invert -
    ...
    for (int j = indexLeft_fit; j <= indexRight_fit; j++) {
      ...
      // y_curr << ", " << x_curr << ", " << y_fit << ", " << y_diff_abs;

      if (y_diff_abs > y_max_deviation) y_max_deviation = y_diff_abs;
    }
...
double US_Edit::find_meniscus_interference_auto() {
  ...
  double meniscus_init = sqrt(bottom_db * bottom_db -
                              (aprofile_volume * 360 /
  ...
  for (int i = start_scan; i < end_scan; i++) {
    ...
    **********************************************/
    ...
    // Alternative: derive a curve passing through Left/Right indecies
    double Slope =
        (s->rvalues[indexRight] * invert - s->rvalues[indexLeft_fit] * invert) /
...
double US_Edit::find_meniscus_auto() {
  ...
  if (simulated_data) meniscus_init = 5.79;
  ...
  for (int i = start_scan; i < end_scan; i++) {
    ...
    // Array of maximum values indexes for the last ~20% of scans in the region
    // [ meniscus_left -- meniscus_left + 0.1 ]
...
int US_Edit::scan_db_auto(void) {
  ...
  while (db.next()) {  // Accumulate data description objects from database
                       // information
  ...
  // type_to_process ****//
  if (runType_combined_IP_RI) runTypes_map[filename_runID_auto] = 0;

  return naucf;
}

// Create the data descriptions map with indecies and counts
void US_Edit::create_descs_auto(QStringList& runIDs, QStringList& infoDs,
                                int naucf) {
  ...
  for (int ii = 0; ii < naucf; ii++) {
    ...
    if (datamap.contains(
            lkey)) {  // Handle the case where the lkey already exists
      qDebug() << "*** DUPLICATE lkey" << lkey << "***";
      lkey = lkey + "(2)";

      if (datamap.contains(lkey)) {  // Handle two duplicates
...
void US_Edit::load_db_auto(QList<DataDesc_auto>& sdescs) {
  ...
  for (int ii = 0; ii < sdescs.count();
       ii++) {  // Loop to load selected data from the database
    ...
    if (QFile(filename).exists()) {  // AUC file exists, do only download if
      ...
      if (dcheck.isEmpty()) {
        ...
        db.query(query);
        db.next();
        ddesc.dcheck = db.value(8).toString() + " " + db.value(9).toString();
        dcheck = ddesc.dcheck;
      }
    ...
    if (dload_auc) {
      ...
      if (stat != US_DB2::OK) {
        nerr++;
        emsg +=
            tr("Error (%1) downloading to file %2\n").arg(stat).arg(filebase);
      }
    }
...
void US_Edit::load(void) {
  ...
  if (isMwl) {  // Set values related to MultiWaveLength
    ...
    //          {
    //             edata         = &allData[ trx ];               // Triple data
    //             min_xval = qMax(min_xval, static_cast<int>
    //             (qRound(edata->xvalues.first() * 1000))); max_xval =
    //             qMin(max_xval, static_cast<int> (qRound(edata->xvalues.last()
    //             * 1000))); double d = edata->xvalues.at(1) -
    //             edata->xvalues.at(0); dx = qMax(dx, static_cast<int>
    //             (qRound(d * 1000))); trx++;
    //          }
    ...
    //                 data int iwavl     = rawi_wvlns[ jj ];             //
    //                 Wavelength value int wvx       = toti_wvlns.indexOf(
    //                 iwavl );   // Wavelength index DbgLv(1) << "IS-MWL:   trx
    //                 ccx wvx" << trx << ccx << wvx;
    ...
    //                 {
    //                     double yval =
    //                     edata->scanData.at(ss).rvalues.at(rpidx); int idx_ss
    //                     = ss * nwaveln; int idx_wv = idx_ii + idx_ss + jj;
    //                     yvals[idx_wv] = yval;
    //                 }
...
void US_Edit::set_data_over_lamda() {
  ...
  int trx_b = 0;
  rdata.clear();
  for (int ccx = 0; ccx < ncelchn; ccx++) {  // Handle each triple of AUC data
    lambdas_by_cell(ccx);                    // Lambdas in cell
    trx_b = trx;
    int min_xval = -100000000;
    int max_xval = 100000000;
    ...
    for (int jwx = 0; jwx < nwaveln; jwx++) {
      edata = &allData[trx];  // Triple data
      min_xval = qMax(min_xval,
                      static_cast<int>(qRound(edata->xvalues.first() * 1000)));
      max_xval = qMin(max_xval,
                      static_cast<int>(qRound(edata->xvalues.last() * 1000)));
    ...
    for (int ii = 0; ii < xvals.size(); ii++) {
      trx = trx_b;
      int txval = static_cast<int>(qRound(xvals.at(ii) * 1000));
      for (int jj = 0; jj < nwaveln;
           jj++) {                   // Each wavelength in the current cell
        edata = &allData[trx];       // Triple data
...
void US_Edit::mouse(const QwtDoublePoint& p) {
  ...
  switch (step) {
    case MENISCUS:
      ...
      else if (expIsEquil || men_1click) {  // Equilibrium
    ...
    case AIRGAP:
      if (airGap_left == 0.0) {
        ...
        if (airGap_right < airGap_left) swap_double(airGap_left, airGap_right);
    ...
    case RANGE:
      ...
      if (range_left == 0.0) {
        ...
        if (range_right < range_left) swap_double(range_left, range_right);
        ...
        if (!expIsEquil) {  // non-Equilibrium
          ...
          if (sData[jsd].meniscus == 0.0 && meniscus > 0.0) total_edits++;
          ...
          if (++row >= cb_rpms->count()) {
            if (++index < cb_triple->count()) {
              ...
              for (int ii = 0; ii < data.scanData.size(); ii++) {
                ...
                if (!trip_rpms.contains(arpm)) trip_rpms << arpm;
              }
              ...
              if (total_edits >= total_speeds) review_edits();
            }
          }
            ...
            step = MENISCUS;
            QString trsp = cb_triple->currentText() + " : " + trip_rpms[row];
            le_edtrsp->setText(trsp);
        ...
        if (total_edits >= total_speeds) {
          qDebug() << "In RANGES: total_edits, total_speeds: " << total_edits
                   << ", " << total_speeds;
          all_edits = true;
          pb_write->setEnabled(true);
    ...
    case BASELINE: {
      ...
      for (int j = pt - 5; j <= pt + 5; j++) sum += scan->rvalues[j];
...
void US_Edit::next_step(void) {
  ...
  else {  // All values up to Plateau have been entered:  Finished
    step = FINISHED;
    pb = NULL;
    //      pb   = pb_dataEnd;
    double sum = 0.0;
...
void US_Edit::plot_all(void) {
  ...
  if (nmcols == 1) pen_plot = QPen(mcolors[0]);
  ...
  for (int i = 0; i < data.scanData.size(); i++) {
    ...
    if (expIsEquil) {
      ...
      for (int jj = jsd; jj < ksd; jj++) {
        ...
        if (tScan < sScan || tScan > eScan) continue;
      ...
      QwtPlotCurve* c =
          us_curve(data_plot, tr("Meniscus at") + QString::number(tScan));
...
void US_Edit::plot_last(void) {
  ...
  QString title = tr("Raw Data at ") + QString::number(s->seconds) +
                  tr(" seconds") + " #" + QString::number(includes.last());
...
void US_Edit::plot_mwl(void) {
  ...
  if (us_edit_auto_mode && all_loaded &&
      !triple_plot_first_time[triple_index]) {
    data_index =
    ...
    qDebug()
        << "In PLOT_MWL when all loaded: triple_index, iwavl_edit_ref[ "
           "triple_index ], iwavl_edit_ref_index[ triple_index , data_index -- "
  ...
  if (xaxis_radius) {
    ...
    DbgLv(1) << "PlMwl:     c triple" << scell << schan << svalu << "d triple"
             << dcell << dchan << dwavl;
  }
  ...
  if (xaxis_radius) {  // Build normal AUC data plot
  ...
  else {  // Build plot of radius record with wavelength points
...
void US_Edit::focus(int from, int to) {
  ...
  for (int ii = ifrom; ii < ito; ii++) focus << includes.at(ii);
...
void US_Edit::set_colors(const QList<int>& focus) {
  ...
  for (int i = 0; i < curves.size(); i++) {
    ...
    if (focus.contains(scnnbr)) {
      ...
      if (nmcols > 1) pen_plot = QPen(mcolors[scnnbr % nmcols]);
...
void US_Edit::exclude_all_but_last(void) {
  ...
  for (int i = scanEnd; i >= scanStart; i--) includes.removeAt(i - 1);
...
void US_Edit::finish_excludes(QList<int> excludes) {
  for (int i = 0; i < excludes.size(); i++) includes.removeAll(excludes[i]);
  ...
  for (int i = 0; i < data.scanData.size(); i++) {
    ...
    for (int j = start; j < end; j++) {
      if (US_DataIO::spike_check(*s, data.xvalues, j, start, end,
        ...
        while (c & (1 << (7 - index % 8))) {
          if (US_DataIO::spike_check(*s, data.xvalues, index, start, end,
                                     &smoothed_value))
...
void US_Edit::undo_auto(void) {
  ...
  if (step < PLATEAU) data = *outData[index_data()];
  ...
  */
  ...
  if (all_loaded) {
    ...
    // (4) plateau; (5) baseline; (6) baseline_od for (int i=0; i < editProfile[
    // cb_triple->currentText() ].count(); i++)
    // 	 {
    // 	   if (i > 5)
    // 	     editProfile[ cb_triple->currentText() ].removeAt(i);
...
void US_Edit::undo(void) {
  ...
  if (dataType == "IP") {
    ...
    if (step > AIRGAP) US_DataIO::adjust_interference(data, edits);

    if (step > RANGE) US_DataIO::calc_integral(data, edits);
  }
...
void US_Edit::new_triple_auto(int index) {
  ...
  */
  ...
  if (isMwl) {  // MWL:  restore the wavelengths for the newly selected triple
    if (lrng_bycell) {  // Restore raw lambdas for an individual cell
      connect_mwl_ctrls(false);
      lambdas_by_cell();
      cb_lstart->clear();
      cb_lend->clear();
  ...
  dataType = QString(QChar(data.type[0])) + QString(QChar(data.type[1]));
  DbgLv(1) << "EDT:NewTr:   sw tri dx" << swavl << triple << idax << "dataType"
           << dataType << "gap_fringe gap_thresh" << gap_fringe << gap_thresh;
  if (otdt != dataType) {  // Reset gap controls for new datatype
    if (dataType == "IP") {
      gap_thresh = gap_val;
  ...
  if (expIsEquil) {  // Equilibrium
    ...
    for (int ii = 0; ii < data.scanData.size();
         ii++) {  // build unique-rpm list for triple
      QString arpm = QString::number(data.scanData[ii].rpm);
  ...
  else {  // non-Equilibrium:  possibly re-do/un-do edits
    ...
    if (app_edit) {  // If editing chosen, apply it
    ...
    else {  // If no editing, be sure to turn off edits
            // set_meniscus();                                       // ALEXEY:
  ...
  if (all_loaded) {
    ...
    if (autoflow_expType == "ABDE") {
      ...
      bl_corr_slope =
          editProfile_blc[cb_triple->currentText()][wvl_value][0].toDouble();
      bl_corr_yintercept =
          editProfile_blc[cb_triple->currentText()][wvl_value][1].toDouble();
...
void US_Edit::set_current_bll_abde(QString chann, QString swave) {
  if (!isMwl || !edited_triples_abde[chann]) return;
...
void US_Edit::write(void) {
  ...
  if (!expIsEquil) {  // non-Equilibrium:  write single current edit (if "all"
                      // unchecked)
    if (isMwl && ck_writemwl->isChecked()) {  // Write edits for all triples in
                                              // the current cell/channel
      write_mwl();
    }

    else {  // Write single triple's edit
  ...
  else {  // Equilibrium:  loop to write all edits
  ...
  pb_nextChan->setEnabled(triples.size() > 1 &&
                          cb_triple->currentIndex() < triples.size() - 1);
}
...
bool US_Edit::isSaved_auto(void) {
  ...
  if (editProfile_count) isDataSaved = true;
...
void US_Edit::manual_edit_auto(void) {
  ...
  connect(sdiag, SIGNAL(pass_edit_params(QMap<QString, QStringList>&)), this,
          SLOT(update_triple_edit_params(QMap<QString, QStringList>&)));

  connect(
      sdiag, SIGNAL(pass_edit_params_includes(QMap<QString, QList<int> >&)),
      this,
      SLOT(update_triple_edit_params_includes(QMap<QString, QList<int> >&)));
...
void US_Edit::update_triple_edit_params_blc_modified(
  ...
  if (isMwl) {
    ...
    qDebug() << "[Singe WL:Modify-per-triple]Setting baseline coor for Wvl --  "
             << w_c;
    editProfile_blc[t_name][w_c] = edit_params_blc[t_name];
  }
...
// manually..
void US_Edit::update_triple_edit_params_blc(
    QMap<QString, QStringList>& edit_params_blc) {
  QString t_name = edit_params_blc.keys()[0];
...
void US_Edit::update_triple_edit_params_includes(
...
void US_Edit::update_triple_edit_params(
    QMap<QString, QStringList>& edit_params) {
...
void US_Edit::write_auto(void) {
  ...
  */
  ...
  if (autoflow_details["status"] != "EDIT_DATA" || isSaved_auto()) {
    if (runType_combined_IP_RI) {
      if (!all_processed) {
        QMessageBox::information(
            this, tr("Edit Profiles for Current Optical System Already Saved"),
            tr("It appears that edit profiles for the current optical system "
               "are already saved!\n\n"
               "The program will switch to processing data for the next "
               "optical system... "));
  ...
  qDebug() << "Submitter map: " << gmp_submitter_map.keys()
           ...
           << gmp_submitter_map.keys().isEmpty() << gmp_submitter_map["User:"]
  ...
  if (!expIsEquil) {  // non-Equilibrium:  write single current edit (if "all"
    ...
    if (isMwl && ck_writemwl->isChecked()) {  // Write edits for all triples in
      ...
      //     centerpieceParameters[ triple_index ][1].toDouble();  //Should be
      //     from centerpiece info from protocol write_triple();
      //   }
    }
  }
  ...
  ***/
  ...
  for (int i = 0; i < channels_all.size(); ++i) {
    ...
    for (int j = 0; j < triples_all_optics.size(); ++j) {
      if (triples_all_optics[j].contains(channels_all[i])) {
        qDebug() << "AUTOFLOW_ANALYSIS: triple " << triples_all_optics[j]
                 << " ,containing channel " << channels_all[i];
        ...
        if (triples_all_optics[j].contains("Interference")) {
          if (isSet_to_analyse(triples_all_optics[j], QString("Interf")) &&
              isSet_to_analyse_triple(triples_all_optics[j],
                                      QString("Interf"))) {
            // ALEXEY: here for combined run, the filename_runID_auto would be
            // incorrectly set to that of the second type, the "-RI" Need to use
            // "-IP"
            if (runType_combined_IP_RI)
            ...
            ID = create_autoflowAnalysis_record(dbP, triples_all_optics[j],
                                                json_status);

            if (ID) create_autoflowAnalysisStages_record(dbP, ID);
          }
        }
        // UV.vis
        else {
          if (isSet_to_analyse(triples_all_optics[j], QString("UV/vis")) &&
              isSet_to_analyse_triple(triples_all_optics[j],
                                      QString("UV/vis"))) {
            // ALEXEY: here for combined run, the filename_runID_auto must be
            // "filename_base-RI" Need to use "-RI"
            if (runType_combined_IP_RI)
              filename_runID_auto = filename_runID_auto_base + "-RI";

            // Was channel's reference wvl set?
            if (!isSet_ref_wvl[channels_all[i]]) {
              // If there is an infortmaiton on specific triple to edit (for a
              // current channel) in the AProfile?
              if (isSet_edit_info_for_channel(triples_all_optics[j],
                                              QString("UV/vis"))) {
                qDebug() << "isSet_edit_info_for_channel() TRUE: ";
                // If this particular  triple set for FM fit (and edit): defined
                // by "wvl_edit" attr in Aprofile
                if (isSet_to_edit_triple(triples_all_optics[j],
                                         QString("UV/vis"))) {
                  json_status = compose_json(true);
                  qDebug() << "isSet_to_edit_triple() TRUE: "
                           << triples_all_optics[j] << json_status;

                  ID = create_autoflowAnalysis_record(
                      dbP, triples_all_optics[j], json_status);

                  // So, reference wvl is defiend as the triple specified in
                  // AProfile
                  isSet_ref_wvl[channels_all[i]] = true;
                  ...
                  qDebug() << "isSet_to_edit_triple() FALSE: "
                           << triples_all_optics[j] << json_status;
              ...
              // No informaiton on specifi triple to edit -- select 1st triple
              // in a channel as default
              else {
                json_status = compose_json(true);
                qDebug() << "isSet_edit_info_for_channel() FALSE: "
                         << triples_all_optics[j] << json_status;

                ID = create_autoflowAnalysis_record(dbP, triples_all_optics[j],
                                                    json_status);

                // So, reference wvl is defiend as the 1st one in the channel
                // domain
                isSet_ref_wvl[channels_all[i]] = true;
        ...
        if (ID) AnalysisIDs << QString::number(ID);
      }
    }
  }
...
// basis
void US_Edit::record_edit_status(QMap<QString, bool> auto_meniscus,
                                 QString dtype) {
  // DEBUG
  ...
  qDebug() << "IN US_edit_AUTO, record status: ID,name,email,lev" << ID << fname
           << lname << email << level;
...
void US_Edit::delete_autoflow_record(void) {
  ...
  qry.clear();
  qry << "delete_autoflow_stages_record" << QString::number(autoflowID_passed);
...
bool US_Edit::isSet_to_analyse(QString triple_name, QString opsys) {
  ...
  for (jj = channels_to_analyse.begin(); jj != channels_to_analyse.end();
    ...
    if (jj.key().contains(channel) && jj.key().contains(opsys)) {
      if (jj.value()) {
        qDebug() << "Triple " << triple_name << " of channel (" << jj.key()
                 << " will be analysed.";
        have_run = true;
        break;
      }
...
bool US_Edit::isSet_to_edit_triple(QString triple_name, QString opsys) {
  ...
  if (triple_to_edit.isEmpty()) {
    qDebug() << "It looks like older protocol is in use: QMap triple_to_edit "
                "is EMPTY!";
  ...
  for (jj = triple_to_edit.begin(); jj != triple_to_edit.end(); ++jj) {
    if (jj.key().contains(channel) && jj.key().contains(opsys)) {
      ...
      if (wvl_set_edit == wvl) {
        qDebug() << "Triple " << triple_name << " for channel " << channel
                 << " is set for EDIT.";
        have_run = true;
        break;
      }
...
QString US_Edit::compose_json(bool fm_stage) {
  ...
  if (job1run) json += QString("\"2DSA\",");
  if (job2run && fm_stage) json += QString("\"2DSA_FM\",");
  if (job3run) {
    if (job3auto)
  ...
  if (job4run) json += QString("\"2DSA_IT\",");
  if (job5run) json += QString("\"2DSA_MC\",");
  if (job6run_pcsa) json += QString("\"PCSA\",");
...
void US_Edit::update_autoflow_record_atEditData(US_DB2* db,
...
int US_Edit::create_autoflowAnalysis_record(US_DB2* db, QString& tripleName,
                                            QString& status_json) {
  ...
  QString status =
      QString("Saving AutoflowAnalysis Profile for Triple %1").arg(tripleName);
...
void US_Edit::write_triple_auto(int trx) {
  ...
  if (isMwl) {  // For MultiWavelength, data index needs to be recomputed
    int wvx = cb_lplot->currentIndex();
  ...
  */
  ...
  if (!ss.isEmpty()) {
    QMessageBox::information(this, tr("Data missing"),
  ...
  if (runType_combined_IP_RI) cb_triple->disconnect();
}
...
void US_Edit::write_triple(void) {
  ...
  if (expIsEquil) {  // Equilibrium:  set baseline,plateau as flag that those
                     // are "done"
    int jsd = sd_offs[triple_index];
    meniscus = sData[jsd].meniscus;
    range_left = sData[jsd].dataLeft;
  ...
  while (editLabel.isEmpty()) {
    ...
    bool ok;
    QString msg = tr("The base Edit Label for this edit session is <b>") + now +
                  "</b> .<br/>" +
    ...
    sufx = QInputDialog::getText(this, tr("Create a unique session Edit Label"),
...
void US_Edit::apply_prior(void) {
  ...
  if (nledits > 1) {
    ...
    pb_allw = mbox.addButton(tr("All Wavelengths"), QMessageBox::AcceptRole);
    pb_defb = nledits > 2 ? pb_allw : pb_selo;
    mbox.setDefaultButton(pb_defb);
    mbox.exec();

    if (mbox.clickedButton() == pb_selo) {  // Only apply for the selected file
    ...
    else {  // Apply to all like-labeled wavelengths in the channel
      DbgLv(1) << "AppPri: AllWavl button clicked";
      for (int ii = 0; ii < nledits;
           ii++) {  // Save selected edit file name; use "same" for others in
                    // channel
        QString edfile = editfiles[ii];
        QString triple = edfile.section(".", -4, -2).replace(".", " / ");
        int jdax = triples.indexOf(triple);

        if (edfile != filename) {  // The selected file
          editFnames[jdax] = filename;
        }

        else {  // Non-selected file
  ...
  else {  // Save edit file name for the single triple
    editFnames[idax] = filename;
  }
...
void US_Edit::prior_equil(void) {
  ...
  if (disk_controls->db()) {  // Get prior equilibrium edits from DB
    US_Passwd pw;
    US_DB2 db(pw.getPasswd());
  ...
  else {  // Get prior equilibrium edits from Local Disk
    QString filter = files[cb_triple->currentIndex()];
    index1 = filter.indexOf('.') + 1;
    ...
    for (int ii = 0; ii < files.size(); ii++) {
      filename = files[ii];
      filename = runID + "." + editLabel + "." + filename.section(".", -5, -2) +
                 ".xml";
      filename = workingDir + filename;
  ...
  for (int ii = 0; ii < cefnames.size();
       ii++) {  // Read and apply edits from edit files
    data = *outData[ii];
...
void US_Edit::next_triple_auto(void) {
  ...
  if ((row + 1) <= cb_triple->count()) {
    ...
    if ((row + 1) == cb_triple->count()) pb_nextChan->setEnabled(false);
...
void US_Edit::prior_triple_auto(void) {
  ...
  if (row >= 0) {
    ...
    if (row == 0) pb_priorChan->setEnabled(false);
...
bool US_Edit::all_edits_done(void) {
  ...
  if (expIsEquil) {
    ...
    for (int jd = 0; jd < outData.size();
         jd++) {  // Examine each data set to evaluate whether edits complete
      int jsd = sd_offs[jd];
      ...
      for (int js = jsd; js < ksd; js++) {
        if (sData[js].meniscus > 0.0) total_edits++;
      }
      ...
      for (int js = 0; js < rawdat->scanData.size(); js++) {
        ...
        if (!drpms.contains(drpm)) drpms << drpm;
      }
    ...
    all_ed_done =
        (range_left != 0 && range_right != 0 && plateau != 0 && baseline != 0);
  }
...
void US_Edit::progress_load(QString progress) { le_info->setText(progress); }
...
void US_Edit::connect_mwl_ctrls(bool conn) {
  if (conn) {
    ...
    connect(pb_custom, SIGNAL(clicked()), this, SLOT(lambda_custom_list()));
    connect(pb_incall, SIGNAL(clicked()), this, SLOT(lambda_include_all()));
    connect(cb_lplot, SIGNAL(currentIndexChanged(int)), this,
            SLOT(lambda_plot_value(int)));
    connect(pb_larrow, SIGNAL(clicked()), this, SLOT(lambda_plot_prev()));
    connect(pb_rarrow, SIGNAL(clicked()), this, SLOT(lambda_plot_next()));
  }
...
void US_Edit::lambda_end_value(int value) {
  ...
  if (new_elambda ==
      elambda)  // check if new end is same as current end -> skip every update
  {
    return;
...
void US_Edit::reset_plot_lambdas() {
  ...
  for (int ii = strtx; ii < endx;
       ii += dlambda) {  // Accumulate new list of export lambdas by looking at
                         // all raw lambas
  ...
  if (xaxis_radius) {  // If x-axis is radius, reset wavelength-to-plot list
    cb_lplot->disconnect();
    cb_lplot->clear();
    cb_lplot->addItems(expc_wvlns);
    connect(cb_lplot, SIGNAL(currentIndexChanged(int)), this,
...
void US_Edit::lambda_plot_value(int value) {
  ...
  if (!xaxis_radius) {  // If plotting radius records, go straight to plotting
    plot_mwl();
    return;
  }
  ...
  if (fname == "none") {  // New wavelength has no edit:  turn off edits
    qDebug() << "Setting meniscus!!";
    set_meniscus();
  }

  else if (fname != "same") {  // New wavelength has its own edit:  apply it
  ...
  else if (step != MENISCUS) {  // New wavelength has same edit as others in
                                // channel:  make sure applied
...
void US_Edit::lambda_plot_next() {
  ...
  int lstx = xaxis_radius ? (expc_wvlns.size() - 1) : (expc_radii.size() - 1);
...
void US_Edit::write_mwl_auto(int trx) {
  ...
  ***********/
               ...
               // channel
  {
    ...
    idax = triples.indexOf(triple);
    // odax             = index_data_auto( trx, wvx ); // Correct ? NOT!!! Got
    // you !!
    ...
    //  '%MartinR_EcoRI_Digest_GMP_Optima1_23OCT23-run1985%2.A.260%'; filename:
...
QString US_Edit::get_rawDataGUID(US_DB2* db, QString filebase_qry,
  ...
  if (rawGUID.isEmpty()) qDebug() << "EMPTY rawID,rawGUID!!";
...
void US_Edit::write_mwl() {
  ...
  if (!chg_lamb) {  // If no change in number of wavelengths, check actual lists
    for (wvx = 0; wvx < nwavelo; wvx++) {
      if (oldi_wvlns[wvx] != expi_wvlns[wvx]) {  // There is a difference in the
                                                 // lists:  mark as such
        chg_lamb = true;
        break;
      }
  ...
  if (chg_lamb) {  // If wavelengths have changed, save new list and rebuild
                   // some vectors
    mwl_data.set_lambdas(expi_wvlns);  // Save new lambdas for channel
    std::sort(expi_wvlns.begin(), expi_wvlns.end());
    reset_outData();
...
// Write edit xml file
int US_Edit::write_xml_file(QString& fname, QString& triple, QString& editGUID,
                            QString& rawGUID) {
  ...
  DbgLv(1) << "EDT:WrXml: IN: fname,triple,editGUID,rawGUID" << fname << triple
           << editGUID << rawGUID;
  ...
  if (!expIsEquil) {  // non-Equilibrium
    ...
    if (autoflow_expType == "ABDE") {
      ...
      xml.writeAttribute("slope", QString::number(bl_corr_slope, 'f', 8));
...
int US_Edit::index_data_auto(int trx, int wvx) {
  ...
  if (isMwl) {      // For MWL, compute data index from wavelength and triple
                    // indexes
    if (wvx < 0) {  // For the default case, use the current wavelength index
      plotndx = cb_lplot->currentIndex();
      int iwavl = expi_wvlns[plotndx];  // ALEXEY: do we need to set data in the
                                        // middle of the triple set ??

      // int iwavl    = expi_wvlns[ 0 ];                      // ALEXEY: OR
      // beginning of the set? ??
      data_index = mwl_data.data_index(iwavl, triple_index);
      odatx = data_index;
      DbgLv(1) << "IxDa_AUTO: dx" << data_index << "plx wavl trx" << plotndx
               << iwavl << triple_index;
    }

    else {  // Use a specified wavelength index (and do not set internal value)
...
int US_Edit::index_data(int wvx) {
  ...
  if (isMwl) {      // For MWL, compute data index from wavelength and triple
    ...
    if (wvx < 0) {  // For the default case, use the current wavelength index
      ...
      data_index = mwl_data.data_index(iwavl, triple_index);
      odatx = data_index;
      DbgLv(1) << "IxDa: dx" << data_index << "plx wavl trx" << plotndx << iwavl
               << triple_index;
    }
...
int US_Edit::like_edit_files(QString filename, QStringList& editfiles,
  ...
  for (int ii = 0; ii < edfiles.count(); ii++) {
    ...
    if (triples.contains(triple)) ldfiles << fname;
  }

  int nledits = ldfiles.count();
  DbgLv(1) << "LiEdFi: nledits" << ldfiles.count() << filter;

  if (dbP != NULL) {  // Determine database like files and compare to local
    ...
    while (dbP->next()) {  // Accumulate filenames and editIDs from the database
    ...
    if (nledits !=
        nlocals) {  // DB records are not matched locally, so download from DB
      if (nlocals == 0) QDir().mkpath(workingDir);

      dbfiles.sort();

      for (int ii = 0; ii < dbfiles.count();
           ii++) {  // Download each DB record to a local file
  ...
  if (nledits == 0) {  // No files (should not happen):  return at least the
                       // specified file name
    editfiles << filename;
    nledits = 1;
  }

  else if (nledits > 1) {  // If multiple files, sort them
    editfiles.sort();
  }
...
int US_Edit::apply_edits(US_DataIO::EditValues parameters) {
  ...
  if (dataType == "IP") {
    US_DataIO::adjust_interference(data, parameters);
    US_DataIO::calc_integral(data, parameters);
    le_airGap->setText(wkstr.sprintf("%.3f - %.3f", airGap_left, airGap_right));
    pb_airGap->setIcon(check);
    pb_airGap->setEnabled(true);
  }
  ...
  // Average the value for +/- 5 points
  for (int jj = pt - 5; jj <= pt + 5; jj++) sum += scan.rvalues[jj];
  double bl = sum / 11.0;
...
void US_Edit::create_report_auto(QString& ss, int trx) {
  ...
  for (int js = 0; js < dd->scanData.size(); js++) {
    DbgLv(1) << "CR: js" << js << "secs" << dd->scanData[js].seconds << "speed"
             << dd->scanData[js].rpm;
  }
  //*DEBUG*
...
void US_Edit::save_report_auto(const QString rtext, const QString rptfpath,
  ...
  if (dbP != NULL) {
    ...
    int st = freport.saveFileDocuments_auto(idInv_auto.toInt(), pfdir, rfiles,
                                            dbP,  // include invID explicitly
                                            idEdit, tripdesc);
...
void US_Edit::save_report(const QString rtext, const QString rptfpath,
  ...
  if (dbP != NULL) {
    ...
    // Write the report record to the database
    int st = freport.saveFileDocuments(pfdir, rfiles, dbP, idEdit, tripdesc);
...
QString US_Edit::table_row(const QString s1, const QString s2, const QString s3,
...
QString US_Edit::html_header(const QString title, const QString head1) {
  ...
  QString editLbl =
      editLabel.isEmpty() ? editFnames[trx].section(".", -6, -6) : editLabel;
...
// Compose HTML header string
QString US_Edit::html_header_auto(const QString title, const QString head1,
                                  const int trx_curr) {
...
QString US_Edit::run_details(void) {
  ...
  ss += table_row(tr("Run Duration:"), QString::number(hours) + hh +
...
QString US_Edit::scan_info(void) {
  ...
  ss += table_row(tr("Scan"), tr("Corrected Time"), tr("Plateau Concentration"),
...
void US_Edit::close_edit(void) {
  ...
  if (nunedit > 0) {  // Some triples unedited:  give option to cancel close
    QMessageBox mbox;
    QPushButton* pb_cancel;
    ...
    mbox.exec();
    if (mbox.clickedButton() == pb_cancel) return;
  }
