...
US_SelectLambdas::US_SelectLambdas(QVector<int> lambdas) : US_WidgetsDialog(0, 0) {
   original = lambdas;
   nbr_select = 0;
   ...
   le_original = us_lineedit(tr("%1 original lambdas").arg(nbr_orig), -1, true);
   le_selected = us_lineedit(tr("%1 selected lambdas").arg(nbr_select), -1, true);
   ...
   for (int ii = 0; ii < nbr_orig; ii++) { // Add items to the original list
...
void US_SelectLambdas::add_selections() {
   ...
   le_selected->setText(nbr_select == 1 ? tr("1 selected lambda") : tr("%1 selected lambdas").arg(nbr_select));
