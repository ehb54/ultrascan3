...
US_Analysis_auto::US_Analysis_auto() : US_Widgets() {
   ...
   connect(
      this, SIGNAL(process_abde(QMap<QString, QString> &)), sdiag_norm_profile,
      SLOT(load_data_auto(QMap<QString, QString> &)));
   connect(
      sdiag_norm_profile, SIGNAL(abde_to_report(QMap<QString, QString> &)), this,
      SLOT(proceed_abde_to_report(QMap<QString, QString> &)));
   connect(sdiag_norm_profile, SIGNAL(back_to_runManager()), this, SLOT(back_to_initAutoflow()));
...
void US_Analysis_auto::initPanel(QMap<QString, QString> &protocol_details) {
   ...
   for (jj = Array_of_analysis.begin(); jj != Array_of_analysis.end(); ++jj) {
      ...
      qDebug() << "GUI: job1run, job2run, job3run, job4run, job5run -- " << job1run << job2run << job3run << job4run
               << job5run;
  ...
  */
   ...
   for (jj = Array_of_analysis.begin(); jj != Array_of_analysis.end(); ++jj) {
      ...
      if (!current_analysis_details.size()) {
         ...
         qDebug() << "FORM HISTORY: set Completed_triples[ " << triple_curr_key << " ] to "
         ...
         for (int i = 0; i < to_process_array.size(); ++i) {
            ...
            if (to_process_stage_groupbox != NULL) {
               ...
               QLineEdit *lineedit_runid
                  = to_process_stage_groupbox->findChild<QLineEdit *>("runID", Qt::FindDirectChildrenOnly);
               QLineEdit *lineedit_owner
                  = to_process_stage_groupbox->findChild<QLineEdit *>("owner", Qt::FindDirectChildrenOnly);
               QLineEdit *lineedit_lastmsg
                  = to_process_stage_groupbox->findChild<QLineEdit *>("lastmsg", Qt::FindDirectChildrenOnly);
               QLineEdit *lineedit_status
                  = to_process_stage_groupbox->findChild<QLineEdit *>("status", Qt::FindDirectChildrenOnly);
               QLineEdit *lineedit_anatype
                  = to_process_stage_groupbox->findChild<QLineEdit *>("anatype", Qt::FindDirectChildrenOnly);
               QLineEdit *lineedit_submit
                  = to_process_stage_groupbox->findChild<QLineEdit *>("submit", Qt::FindDirectChildrenOnly);
               QLineEdit *lineedit_cluster
                  = to_process_stage_groupbox->findChild<QLineEdit *>("cluster", Qt::FindDirectChildrenOnly);
               QLineEdit *lineedit_lastupd
                  = to_process_stage_groupbox->findChild<QLineEdit *>("lastupd", Qt::FindDirectChildrenOnly);

               QPushButton *pb_delete
                  = to_process_stage_groupbox->findChild<QPushButton *>("delete", Qt::FindDirectChildrenOnly);
               ...
               //owner
               QString investigator_text = investigator_details[ "email" ] + " (" + investigator_details[ "fname" ]
      ...
      if (processed.isUndefined())
         ...
         for (int i = 0; i < processed_array.size(); ++i) {
            ...
            foreach (const QString &key, processed_array[ i ].toObject().keys()) {
               ...
               qDebug() << "Processed stage - " << key << ": gfacID, status, statusMsg -- "
                        << newObj[ "gfacID" ].toString() << newObj[ "HPCAnalysisRequestID" ].toString()
                        << newObj[ "status" ].toString() << newObj[ "statusMsg" ].toString()
                        << newObj[ "createTime" ].toString() << newObj[ "updateTime" ].toString();
            ...
            if (stage_name.contains("FITMEN") && (stage_status == "CANCELED" || stage_status == "canceled")) {
               ...
               qDebug() << "CANCELED status for triple/stage (FITMEN) -- " << triple_curr_key << "/"
                        << submitted.toString();
            ...
            if (processed_stage_groupbox != NULL) {
               // update GUI
               QLineEdit *lineedit_runid
                  = processed_stage_groupbox->findChild<QLineEdit *>("runID", Qt::FindDirectChildrenOnly);
               QLineEdit *lineedit_owner
                  = processed_stage_groupbox->findChild<QLineEdit *>("owner", Qt::FindDirectChildrenOnly);
               QLineEdit *lineedit_lastmsg
                  = processed_stage_groupbox->findChild<QLineEdit *>("lastmsg", Qt::FindDirectChildrenOnly);
               QLineEdit *lineedit_status
                  = processed_stage_groupbox->findChild<QLineEdit *>("status", Qt::FindDirectChildrenOnly);
               QLineEdit *lineedit_anatype
                  = processed_stage_groupbox->findChild<QLineEdit *>("anatype", Qt::FindDirectChildrenOnly);
               QLineEdit *lineedit_submit
                  = processed_stage_groupbox->findChild<QLineEdit *>("submit", Qt::FindDirectChildrenOnly);
               QLineEdit *lineedit_cluster
                  = processed_stage_groupbox->findChild<QLineEdit *>("cluster", Qt::FindDirectChildrenOnly);
               QLineEdit *lineedit_lastupd
                  = processed_stage_groupbox->findChild<QLineEdit *>("lastupd", Qt::FindDirectChildrenOnly);

               QPushButton *pb_delete
                  = processed_stage_groupbox->findChild<QPushButton *>("delete", Qt::FindDirectChildrenOnly);
               QPushButton *pb_overlay
                  = processed_stage_groupbox->findChild<QPushButton *>("overlay", Qt::FindDirectChildrenOnly);

               //delete
               pb_delete->setEnabled(false);

               //runID
               QString runid_text = filename + " ( " + triple_curr + " )  " + defaultDB
               ...
               if (stage_status == "CANCELED" || stage_status == "canceled") {
                  ...
                  qDebug() << "CANCELED status( Processed ) for triple/stage -- " << triple_curr_key << "/"
                           << submitted.toString();
               }
               if (stage_status == "FAILED" || stage_status == "failed") {
                  ...
                  qDebug() << "FAILED status( Processed ) for triple/stage -- " << triple_curr_key << "/"
                           << submitted.toString();
               }
            }
         }
         ...
         if (
            !to_process_array.size() && History_read[ triple_curr_key ] && !status.contains("CANCELED")
            && !status.contains("canceled") && !status.contains("FAILED") && !status.contains("failed")) {
      ...
      if (submitted.isUndefined())
         ...
         if (submitted.toString() == "FITMEN_AUTO" && Process_2dsafm[ triple_curr_key ]) {
            ...
            if (
               nextWaitStatus == "COMPLETE" || nextWaitStatus == "complete" || nextWaitStatus == "CANCELED"
	      ...
	      ****/
            ...
            connect(FitMen, SIGNAL(editProfiles_updated_earlier()), this, SLOT(editProfiles_updated_earlier()));

            connect(FitMen, SIGNAL(triple_analysis_processed()), this, SLOT(triple_analysis_processed()));
         ...
         if (
            (submitted.toString() == "FITMEN" || submitted.toString() == "FITMEN_AUTO")
         ...
         if (current_stage_groupbox != NULL) {
            QLineEdit *lineedit_runid
               = current_stage_groupbox->findChild<QLineEdit *>("runID", Qt::FindDirectChildrenOnly);
            QLineEdit *lineedit_owner
               = current_stage_groupbox->findChild<QLineEdit *>("owner", Qt::FindDirectChildrenOnly);
            QLineEdit *lineedit_lastmsg
               = current_stage_groupbox->findChild<QLineEdit *>("lastmsg", Qt::FindDirectChildrenOnly);
            QLineEdit *lineedit_status
               = current_stage_groupbox->findChild<QLineEdit *>("status", Qt::FindDirectChildrenOnly);
            QLineEdit *lineedit_anatype
               = current_stage_groupbox->findChild<QLineEdit *>("anatype", Qt::FindDirectChildrenOnly);
            QLineEdit *lineedit_submit
               = current_stage_groupbox->findChild<QLineEdit *>("submit", Qt::FindDirectChildrenOnly);
            QLineEdit *lineedit_cluster
               = current_stage_groupbox->findChild<QLineEdit *>("cluster", Qt::FindDirectChildrenOnly);
            QLineEdit *lineedit_lastupd
               = current_stage_groupbox->findChild<QLineEdit *>("lastupd", Qt::FindDirectChildrenOnly);

            QPushButton *pb_delete
               = current_stage_groupbox->findChild<QPushButton *>("delete", Qt::FindDirectChildrenOnly);
            QPushButton *pb_overlay
               = current_stage_groupbox->findChild<QPushButton *>("overlay", Qt::FindDirectChildrenOnly);
...
US_DataIO::EditedData *US_Analysis_auto::aa_editdata() {
   return edata;
}

// public function to get pointer to list of excluded scans
QList<int> *US_Analysis_auto::aa_excllist() {
   return &excludedScans;
}

// public function to get pointer to sim data
US_DataIO::RawData *US_Analysis_auto::aa_simdata() {
   return sdata;
}

// public function to get pointer to load model
US_Model *US_Analysis_auto::aa_model() {
   return &model;
}

// public function to get pointer to TI noise
US_Noise *US_Analysis_auto::aa_ti_noise() {
   return &ti_noise;
}

// public function to get pointer to RI noise
US_Noise *US_Analysis_auto::aa_ri_noise() {
   return &ri_noise;
}

// public function to get pointer to resid bitmap diag
QPointer<US_ResidsBitmap> US_Analysis_auto::aa_resbmap() {
   return rbmapd;
}

QString US_Analysis_auto::aa_tripleInfo() {
   return tripleInfo;
}
   ...
   while (db->next()) {
      ...
      if (description.contains(triple_information[ "stage_name" ])) {
         if (triple_information[ "stage_name" ] == "2DSA") {
            if (
               !description.contains("-FM_") && !description.contains("-IT_") && !description.contains("-MC_")
...
int US_Analysis_auto::count_noise_auto(
...
void US_Analysis_auto::delete_jobs_at_fitmen(QMap<QString, QString> &triple_info) {
   ...
   if (!fitmen_bad_vals) {
      ...
      if (mwl_channel)
         msg_sys_text += QString("\n\nSince this is a multi-wavelength analysis, scheduled jobs for the following "
                                 "same-channel triples will be canceled as well: \n\n%1")
...
void US_Analysis_auto::delete_job(QString triple_stage) {
   ...
   //if ( mwl_channel && ( stage_n == "2DSA" || stage_n == "2DSA-FM" ) )
   if (
      Process_2dsafm[ triple_n ] && mwl_channel
      && (stage_n == "2DSA" || stage_n == "2DSA-FM")) // don't we have to do this ONLY fro representative triple??
      msg_sys_text += QString("\n\nSince this is a multi-wavelength analysis, ongoing jobs for the following "
                              "same-channel triples will be canceled as well: \n %1")
                         .arg(triple_list_affected.join(","));
   ...
   if (msg_delete.clickedButton() == Accept_sys) {
      ...
      QString comment_t
         = QInputDialog::getText(this, tr("Reason for job CANCELLATION"), msg, QLineEdit::Normal, default_text, &ok);
...
void US_Analysis_auto::update_autoflowAnalysis_uponDeletion(US_DB2 *db, const QString &requestID) {
   ...
   qry << "update_autoflow_analysis_record_at_deletion" << QString("Job scheduled for deletion, analysis stage")
       << requestID;
...
void US_Analysis_auto::update_autoflowAnalysis_uponDeletion_other_wvl(US_DB2 *db, const QStringList &requestID_list) {
   ...
   for (int i = 0; i < requestID_list.size(); ++i) {
      qry.clear();
      qry << "update_autoflow_analysis_record_at_deletion_other_wvl" << requestID_list[ i ];
...
void US_Analysis_auto::record_or_update_analysis_cancel_status(
   US_DB2 *db, QString triple_name, QString other_chan_triples, QString comment_t) {
   // Check DB connection
   if (db->lastErrno() != US_DB2::OK) {
      QMessageBox::warning(
         this, tr("Connection Problem"),
   ...
   if (autoflowStatusID) {
      ...
      qry << "update_autoflowStatusAnalysisCancel_record" << QString::number(autoflowStatusID)
...
void US_Analysis_auto::record_or_update_analysis_meniscus_status(US_DB2 *db, QString triple_name, QString FMB_changed) {
   if (FMB_changed.isEmpty()) {
      qDebug() << "FBM_changed EMPTY: It seems it's impossible to determine if meniscus| bottom was modified from the "
                  "best FIT...";
      return;
   }
   ...
   if (autoflowStatusID) {
      //update OR insert NEW Json
      qry << "update_autoflowStatusAnalysisFitmen_record" << QString::number(autoflowStatusID)
      ...
      QMessageBox::warning(
         this, tr("AutoflowStatus Record Problem"),
         tr("autoflowStatus (Analysis {FMB}): There was a problem with identifying a record in autoflowStatus table "
            "for a given run! \n"));
...
bool US_Analysis_auto::check_fitmen_status(const QString &requestID, const QString &stageName) {
   ...
   if (!analysis_details.size()) {
      ...
      QMessageBox::information(
         this, QString(tr("%1 | Triple Analysis already processed")).arg(stageName),
...
QGroupBox *US_Analysis_auto::createGroup(QString &triple_name) {
   ...
   groupBox->setStyleSheet(
      "QGroupBox { font: bold;  background-color: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 #E0E0E0, stop: 1 "
      "#FFFFFF); border: 2px solid gray; border-radius: 10px; margin-top: 20px; margin-bottom: 10px; padding-top: 5px; "
      "} QGroupBox::title { subcontrol-origin: margin; subcontrol-position: top left; left: 10px; margin: 0 5px; "
      "background-color: black; color: white; padding: 0 3px;}  QGroupBox::indicator { width: 13px; height: 13px; "
      "border: 1px solid grey; background-color: rgba(204, 204, 204, 255);} QGroupBox::indicator:hover "
      "{background-color: rgba(235, 235, 235, 255);} QLabel {background-color: rgb(105,105,105);}");
...
void US_Analysis_auto::gui_update_temp() {
   ...
   for (jj = Array_of_analysis.begin(); jj != Array_of_analysis.end(); ++jj) {
      ...
      QLineEdit *lineedit_2dsa_runID
         = groupbox_2DSA[ triple_curr ]->findChild<QLineEdit *>("runID", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_owner
         = groupbox_2DSA[ triple_curr ]->findChild<QLineEdit *>("owner", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_lastmsg
         = groupbox_2DSA[ triple_curr ]->findChild<QLineEdit *>("lastmsg", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_status
         = groupbox_2DSA[ triple_curr ]->findChild<QLineEdit *>("status", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_anatype
         = groupbox_2DSA[ triple_curr ]->findChild<QLineEdit *>("anatype", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_submit
         = groupbox_2DSA[ triple_curr ]->findChild<QLineEdit *>("submit", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_lastupd
         = groupbox_2DSA[ triple_curr ]->findChild<QLineEdit *>("lastupd", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_cluster
         = groupbox_2DSA[ triple_curr ]->findChild<QLineEdit *>("cluster", Qt::FindDirectChildrenOnly);
      ...
      QLineEdit *lineedit_2dsa_fm_runID
         = groupbox_2DSA_FM[ triple_curr ]->findChild<QLineEdit *>("runID", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_fm_owner
         = groupbox_2DSA_FM[ triple_curr ]->findChild<QLineEdit *>("owner", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_fm_lastmsg
         = groupbox_2DSA_FM[ triple_curr ]->findChild<QLineEdit *>("lastmsg", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_fm_status
         = groupbox_2DSA_FM[ triple_curr ]->findChild<QLineEdit *>("status", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_fm_anatype
         = groupbox_2DSA_FM[ triple_curr ]->findChild<QLineEdit *>("anatype", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_fm_submit
         = groupbox_2DSA_FM[ triple_curr ]->findChild<QLineEdit *>("submit", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_fm_lastupd
         = groupbox_2DSA_FM[ triple_curr ]->findChild<QLineEdit *>("lastupd", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_fm_cluster
         = groupbox_2DSA_FM[ triple_curr ]->findChild<QLineEdit *>("cluster", Qt::FindDirectChildrenOnly);
      ...
      QLineEdit *lineedit_2dsa_it_runID
         = groupbox_2DSA_IT[ triple_curr ]->findChild<QLineEdit *>("runID", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_it_owner
         = groupbox_2DSA_IT[ triple_curr ]->findChild<QLineEdit *>("owner", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_it_lastmsg
         = groupbox_2DSA_IT[ triple_curr ]->findChild<QLineEdit *>("lastmsg", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_it_status
         = groupbox_2DSA_IT[ triple_curr ]->findChild<QLineEdit *>("status", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_it_anatype
         = groupbox_2DSA_IT[ triple_curr ]->findChild<QLineEdit *>("anatype", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_it_submit
         = groupbox_2DSA_IT[ triple_curr ]->findChild<QLineEdit *>("submit", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_it_lastupd
         = groupbox_2DSA_IT[ triple_curr ]->findChild<QLineEdit *>("lastupd", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_it_cluster
         = groupbox_2DSA_IT[ triple_curr ]->findChild<QLineEdit *>("cluster", Qt::FindDirectChildrenOnly);
      ...
      QLineEdit *lineedit_2dsa_mc_runID
         = groupbox_2DSA_MC[ triple_curr ]->findChild<QLineEdit *>("runID", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_mc_owner
         = groupbox_2DSA_MC[ triple_curr ]->findChild<QLineEdit *>("owner", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_mc_lastmsg
         = groupbox_2DSA_MC[ triple_curr ]->findChild<QLineEdit *>("lastmsg", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_mc_status
         = groupbox_2DSA_MC[ triple_curr ]->findChild<QLineEdit *>("status", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_mc_anatype
         = groupbox_2DSA_MC[ triple_curr ]->findChild<QLineEdit *>("anatype", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_mc_submit
         = groupbox_2DSA_MC[ triple_curr ]->findChild<QLineEdit *>("submit", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_mc_lastupd
         = groupbox_2DSA_MC[ triple_curr ]->findChild<QLineEdit *>("lastupd", Qt::FindDirectChildrenOnly);
      QLineEdit *lineedit_2dsa_mc_cluster
         = groupbox_2DSA_MC[ triple_curr ]->findChild<QLineEdit *>("cluster", Qt::FindDirectChildrenOnly);
...
void US_Analysis_auto::scan_dbase_auto(QMap<QString, QString> &triple_information) {
   ...
   while (db.next()) {
      ...
      if (descript.length() == 80) { // Truncated description:  save ID and skip update for now
      ...
      DbgLv(1) << "DbSc:   modelID vari meni" << modelID << variance << meniscus << "ansysID" << ansysID << "iterID"
               << iterID;

      if (ansysID.contains("2DSA-F") || iterID.contains(fmIter)) { // Model from meniscus fit, so save information
         ...
         if (fittype == 2 || fittype == 3) { // Bottom or Meniscus+Bottom:  add to list of bottom redo's
         ...
         QString ftfname = runID + "/" + anType + "." + editLabel + "." + tripleID + fextn;
         mdescr.description = descript;
         mdescr.baseDescr = runID + "." + tripleID + "." + ansysID.section("-", 0, 3);
   ...
   DbgLv(1) << "DbSc: tmodels size" << tmodels.size() << "ted sizes" << tedGIDs.size() << tedIDs.size();
   ...
   for (int ii = 0; ii < tmodels.size(); ii++) {
      ...
      if (ansysID.contains("2DSA-F") || iterID.contains(fmIter)) { // Model from meniscus fit, so save information
         ...
         if (fittype != 1) { // Bottom or Meniscus+Bottom:  add to list of bottom redo's
            botredo << mDescrs.count();
         }
   ...
   for (int ii = 0; ii < botredo.count(); ii++) {
      ...
      if (bottom < 1.0) { // Bottom not reliable in model, get from model description
         ...
         if (fitVals.length() > 6)
            ...
            fittype = 1; // meniscus fit
         bottom = (fittype == 2) ? (fitVals.mid(1, 5).toDouble() / 10000.0) : (fitVals.mid(6, 5).toDouble() / 10000.0);
      }
   ...
   for (int ii = 0; ii < nfmods; ii++) { // Find unique file names in order to create sets
      QString ftfname = mDescrs[ ii ].fitfname;
      QString antime = mDescrs[ ii ].antime;

      if (!ufnams.contains(ftfname)) { // This is a new fit-file name, so new analysis set
         ufnams << ftfname;
         uantms << antime;
      }

      else if (!uantms.contains(antime)) { // Already seen fit-file, but new analysis time, so duplicate
         uantms << antime;
      }
   ...
   for (int ii = 0; ii < kfsets; ii++) { // Find out for each set whether a corresponding fit file exists
      QString ftfname = ufnams.at(ii);

      if (mfnams.count(ftfname) == 1) { // Not really a set; single fit model after previous fm run
         nfsets--;
      ...
      if (ftfile.exists()) { // File exists, so we must check the need to replace it
         QString ftfpath = rdir + ftfname;
         QDateTime fdate = QFileInfo(ftfile).lastModified().toUTC();
         int jj = mfnams.lastIndexOf(ftfname);
         QDateTime rdate = mDescrs[ jj ].lmtime;
         DbgLv(1) << " ii rdate fdate" << ii << rdate << fdate << "   ftfname" << ftfname << "  fdate.msecsTo(rdate)"
                  << fdate.msecsTo(rdate);
         DbgLv(1) << "   jj desc" << jj << mDescrs[ jj ].description << "antime meniscus" << mDescrs[ jj ].antime
                  << mDescrs[ jj ].meniscus;

         if (fdate.msecsTo(rdate) > 0) { // DB record date is later than file date, so must replace file
            nfrpls++;
            ftfile.remove();
         }
      ...
      for (int jj = jfirst; jj < jlast; jj++) { // First build the pairs (or triples) list
         ...
         QString mrpair = QString::number(meniscus, 'f', 6) + " " + QString::number(rmsd, 'e', 6);

         if (bottom > 1.0) { // Either Bottom or Meniscus+Bottom
            if (ftfname.contains("FB")) { // Bottom only
               mrpair = QString::number(bottom, 'f', 6) + " " + QString::number(rmsd, 'e', 6);
            }
            else { // Meniscus and Bottom
               mrpair = QString::number(meniscus, 'f', 6) + " " + QString::number(bottom, 'f', 6) + " "
...
bool US_Analysis_auto::file_loaded_auto(QMap<QString, QString> &triple_information) {
   ...
   QStringList fm_files
      = directory
           .entryList(QStringList() << "2DSA-FM*" + triple_name_cut + "*.fitmen.dat", QDir::Files | QDir::NoSymLinks);
   ...
   QString tripl = tripnode.left(1) + "." + tripnode.mid(1, 1) + "." + tripnode.mid(2);
   QStringList edtfilt;
...
void US_Analysis_auto::load_data_auto(const QString &text_content) {
   ...
   for (int ii = 0; ii < lines.size(); ii++) {
      ...
      if (valsize == 3) {
         ...
         parsed << QString().sprintf("%3d : ", count) + QString::number(rmeni, 'f', 5) + ", "
                      + QString::number(rbott, 'f', 5) + ", " + QString::number(rmsdv, 'f', 8);
      }
   ...
   DbgLv(1) << "LD:  was3val have3val" << was3val << have3val << "v_rmsd size" << v_rmsd.size() << "parsed length"
            << parsed.length();
...
void US_Analysis_auto::process_2d(void) {
   ...
   if (!US_Matrix::lsfit(c, radius_values, rmsd_values, count, order + 1)) {
      QMessageBox::warning(this, tr("Data Problem"), tr("The data is inadequate for this fit order"));
...
void US_Analysis_auto::process_3d(void) {
   ...
   DbgLv(1) << "pl3d: v_meni size" << v_meni.size() << "min,max x,yz" << min_x << max_x << min_y << max_y << min_z
            << max_z;
   ...
   QList<int> ixs;
   DbgLv(1) << "pl3d:  nmeni nbott" << nmeni << nbott << "ix_best" << ix_best << "ix_men ix_bot" << ix_men << ix_bot;
   ...
   for (int ii = 0; ii < ixs.count(); ii++) {
      ...
      w_rmsd += f_rmsd;
      DbgLv(1) << "pl3d:  ixs" << jx << "f_meni f_bott w_rmsd" << f_meni << f_bott << w_rmsd;
   }
   f_meni /= w_rmsd;
   f_bott /= w_rmsd;
...
int US_Analysis_auto::read_autoflowAnalysisStages(const QString &requestID) {
   ...
   if (db->lastErrno() != US_DB2::OK) {
      QMessageBox::warning(
         this, tr("Connection Problem"), tr("FitMeniscus: Could not connect to database \n") + db->lastError());
      return status;
   }
...
void US_Analysis_auto::revert_autoflow_analysis_stages_record(const QString &requestID) {
   ...
   if (db->lastErrno() != US_DB2::OK) {
      QMessageBox::warning(
         this, tr("Connection Problem"), tr("FitMen revert: Could not connect to database \n") + db->lastError());
      return;
   }
...
#define BOTT_LOWVAL 7.0
   ...
   qDebug() << "In FITMEN_AUTO's edit_update(): confirm, all_wvl, rmv_mdls, db_upd  ? " << confirm << all_wvl
            << rmv_mdls << db_upd;
   //*DEBUG*
   //db_upd=false;
   //*DEBUG*
   ...
   if (!all_wvl || nedtfs == 1) { // Apply to a single triple
      ...
      if (mennew >= lefval) //ALEXEY: HERE!!!!
      {
         ...
         QString reason_for_failure
            = QString("The selected Meniscus value, %1 , extends into the data range whose left-side value is %2")
                 .arg(mennew)
                 .arg(lefval);
      ...
      progress_msg_fmb->setWindowTitle(
         QString(tr("Updating Edit Profiles: %1")).arg(triple_information[ "triple_name" ]));
      ...
      for (int jj = 0; jj < nedtfs; jj++) { // Modify each Edit file corresponding to a wavelength
         ...
         if (!filei.open(QIODevice::ReadOnly | QIODevice::Text)) { // Skip any file we cannot read
...
void US_Analysis_auto::remove_models_auto(QString invID) {
   ...
   DbgLv(1) << "RmvMod: scn1  srchRun" << srchRun << "srchEdit" << srchEdit << "srchTrip" << srchTrip;
   ...
   QStringList modfiles = QDir(moddir).entryList(modfilt, QDir::Files, QDir::Name);
   ...
   for (int ii = 0; ii < modfiles.size(); ii++) {
      ...
      if (ii < 3 || (ii + 4) > modfiles.size() || ii == (modfiles.size() / 2))
         DbgLv(1) << "RmvMod:  scn1 ii runID editLabl tripID" << ii << runID << editLabl << tripID;
      ...
      if (!iterID.contains("-m") && !iterID.contains("-b")) {
         DbgLv(1) << "RmvMod:   ii" << ii << "iterID" << iterID << "has -m" << iterID.contains("-m") << "has -b"
                  << iterID.contains("-b");
         continue; // Can't use if not a fit-meniscus or fit-bottom type
      }

      if (itNum == "01" && msetBase.isEmpty()) { // When fit iteration is "01" and not yet saved, save model set base
         msetBase = descript.section(".", 0, -3) + "." + anRunID.section("_", 0, 3);
      ...
      if (arTime > lArTime) { // If first set or new one younger than previous, start lists
         lmodFnams.clear();
         lmodGUIDs.clear();
         lmodVaris.clear();
         lmodMenis.clear();
   ...
   for (int ii = 0; ii < nlmods; ii++) { // Scan to identify model in set with iteration index
      ...
      DbgLv(1) << "RmvMod: best ndx scan: ii vari" << ii << lmodd.variance << "iterID iters" << iterID << iterx;
   ...
   if (db_upd) {
      ...
      for (int ii = 0; ii < modIDs.size(); ii++) {
         ...
         if (arTime > dArTime) { // If first set or new one younger than previous, start lists
            dmodIDs.clear();
            dmodGUIDs.clear();
            dmodVaris.clear();
            dmodMenis.clear();
      ...
      for (int ii = 0; ii < ndmods; ii++) { // Scan to identify model in set with lowest variance
         ModelDesc dmodd = dMDescrs[ ii ];
         QString descrip = QString(dmodd.description);
         QString ansysID = descrip.section('.', -2, -2);
         QString iterID = ansysID.section('_', -1, -1);
         int iterx = iterID.section('-', 0, 0).mid(1).toInt() - 1;
         DbgLv(1) << "RmvMod: best ndx scan: ii vari" << ii << dmodd.variance << "iterID iters" << iterID << iterx;
      ...
      if (nlmods == ndmods || (ndmods > 0 && nlmods == 0)) {
         ...
         for (int jj = 0; jj < nlmods; jj++) {
            ...
            if (lmodd.modelGUID == dmodd.modelGUID && lmodd.description == dmodd.description)
               nmatch++;
      ...
      else if (nlmods == 0) { // It is OK if there are no local records, when DB ones were found
         DbgLv(1) << "++only dbase records exist";
      }

      else if (ndmods == 0) { // It is OK if there are only local records, when local ones found
   ...
   DbgLv(1) << "  nlmods ndmods" << nlmods << ndmods;
   if (ndmods > 0 || nlmods > 0) { // There are models to scan, so build a list of models,noises to remove
      ...
      for (int jj = 0; jj < ntmods; jj++) { // Build the list of model files and DB ids for removal
         if (jj != ikModx) {
            int itix;
            int irix;
            QString fname;
            ...
            DbgLv(1) << "RmvMod:    itix irix" << itix << irix << "tiDesc" << tiDesc << "riDesc" << riDesc;
      ...
      bool rmv_mdls = true;
      DbgLv(1) << "RmvMod: nlrmod ndrmod nlrnoi ndrnoi nlnois ndnois" << nlrmod << ndrmod << nlrnoi << ndrnoi << nlnois
               << ndnois;
      ...
      if (rmv_mdls) {
         ...
         for (int ii = 0; ii < rmodIDs.size(); ii++) { // Remove models and db noises
            recID = rmodIDs[ ii ];
            ...
            if (recID != "-1" && dbP != NULL) { // Delete model (and any child noise) from DB
               query.clear();
...
void US_Analysis_auto::index_model_setfit() {
   if (have3val) { // Fit Mensicus + Bottom:  find closest x,y in list
      ...
      for (int ii = 0; ii < v_meni.count(); ii++) { // Find lowest difference in X,Y list
      ...
      for (int ii = 0; ii < v_meni.count(); ii++) { // Find lowest difference in X list
...
void US_Analysis_auto::noises_in_edit(
   QString modDesc, QStringList &nieDescs, QStringList &nieIDs, QStringList &nieFnams, QString invID) {
   QString msetBase = modDesc.section(".", 0, -3) + "." + modDesc.section(".", -2, -2).section("_", 0, 3);
   QString srchTlab = msetBase.section(".", -2, -2);
   QString srchTrip = srchTlab.left(1) + "." + srchTlab.mid(1, 1) + "." + srchTlab.mid(2) + ".xml";
   QString srchRun = msetBase.section(".", 0, -3) + "." + msetBase.section(".", -1, -1).section("_", 0, 0).mid(1);
   ...
   QStringList noifiles = QDir(noidir).entryList(noifilt, QDir::Files, QDir::Name);
   noidir = noidir + "/";
   ...
   DbgLv(1) << "NIE: db_upd" << db_upd << "nlnois" << nlnois << "nieDescs-size" << nieDescs.size() << "nieIDs-size"
            << nieIDs.size();
