...
US_Analysis_auto::US_Analysis_auto() : US_Widgets() {
  ...
  connect(pb_show_all, SIGNAL(clicked()), this, SLOT(show_all()));

  connect(pb_hide_all, SIGNAL(clicked()), this, SLOT(hide_all()));
  ...
  connect(sdiag_norm_profile, SIGNAL(back_to_runManager()), this,
          SLOT(back_to_initAutoflow()));
  ...
  // QString("MartinR_RP12_EcoRI_Digest_Optima1_24823-v3"); protocol_details[
  // "aprofileguid" ] = QString("19a5ef33-0a1e-499e-95ef-45daac7bdcb7");
  // protocol_details[ "filename" ]     =
  // QString("MartinR_RP12_EcoRI_Digest_Optima1_24823-v3-run1963");
  ...
  // QString("6c376179-6eda-47e9-b699-3eef63c6fe6e"); protocol_details[
  // "filename" ]     = QString("AAV_GMP_test_030325-run2366-dataDiskRun-1515");
  ...
  // protocol_details[ "protocolName" ] =
  // QString("MartinR_alexey-dev-issue425-SWL-RI-T3_21JUN25"); protocol_details[
  // "aprofileguid" ] = QString("bc5641b1-27f2-4604-a3e2-5d24196fc9de");
  // protocol_details[ "filename" ]     =
  // QString("MartinR_alexey-dev-issue425-SWL-RI-T3_21JUN25-run2395");
...
void US_Analysis_auto::initPanel(QMap<QString, QString>& protocol_details) {
  ...
  if (autoflow_expType == "ABDE") {
    ...
    // check if MWL or SWL
    QMap<QString, QString> abde_analysis_details =
        get_abdeAnalysis_info(&db, QString::number(autoflowID_passed));
    ...
    if (abde_etype == "MWL") {
      ...
      // Pre-process data (decomposition for MWL); build alt. GUI (to integrate
      ...
      qDebug() << "Dir for GMP: "
               << protocol_details_at_analysis["directory_for_gmp"];
    }
  ...
  for (int i = 0; i < analysisIDs_list.size(); ++i) {
    ...
    if (!analysis_details.size()) {
      ...
      qDebug() << "analysis_details.size() FROM autoflowAnalysisHistory -- "
               << analysis_details.size();
    } else
      qDebug() << "analysis_details.size() FROM autoflowAnalysis -- "
               << analysis_details.size();
  ...
  for (int i = 0; i < channels_all.size(); ++i) {
    for (ti = Array_of_analysis.begin(); ti != Array_of_analysis.end(); ++ti) {
      ...
      if (triple_name.contains(channels_all[i]) &&
          !triple_name.contains("Interference")) {
  ...
  for (chanreq = Channel_to_requestIDs.begin();
       chanreq != Channel_to_requestIDs.end(); ++chanreq)
    qDebug() << "channel-to-requestIDs " << chanreq.key() << ": "
             << chanreq.value();
  ...
  for (jj = Array_of_analysis.begin(); jj != Array_of_analysis.end(); ++jj) {
    ...
    // TEST
    // json =
    // "{\"to_process\":[\"FITMEN\",\"2DSA_IT\",\"2DSA_MC\"],\"processed\":[\"2DSA\"],\"submitted\":\"2DSA_FM\"}"
    // ;

    if (json.contains("2DSA"))  // ALEXEY: maybe more strict condition here? (as
                                // 2DSA is in other stages...)
      job1run = true;
    if (json.contains("2DSA_FM")) job2run = true;
    if (json.contains("FITMEN")) job3run = true;
    if (json.contains("FITMEN_AUTO")) job3run = false;
    if (json.contains("2DSA_IT")) job4run = true;
    if (json.contains("2DSA_MC")) job5run = true;
    if (json.contains("PCSA")) job6run_pcsa = true;
    ...
    if (triple_name_width > max_width) max_width = triple_name_width;
    ...
    if (job1run)  // 2DSA
    {
      ...
      treeWidget->setItemWidget(childItem_2DSA[triple_curr], 1,
                                groupbox_2DSA[triple_curr]);
    }
    ...
    if (job2run)  // 2DSA-FM
    {
      ...
      treeWidget->setItemWidget(childItem_2DSA_FM[triple_curr], 1,
                                groupbox_2DSA_FM[triple_curr]);
    }
    ...
    if (job4run)  // 2DSA-IT
    {
      ...
      treeWidget->setItemWidget(childItem_2DSA_IT[triple_curr], 1,
                                groupbox_2DSA_IT[triple_curr]);
    }
    ...
    if (job5run)  // 2DSA-MC
    {
      ...
      treeWidget->setItemWidget(childItem_2DSA_MC[triple_curr], 1,
                                groupbox_2DSA_MC[triple_curr]);
    }
    ...
    if (job6run_pcsa)  // PCSA
    {
      ...
      treeWidget->setItemWidget(childItem_PCSA[triple_curr], 1,
                                groupbox_PCSA[triple_curr]);
    }
  }
...
void US_Analysis_auto::gui_update() {
  ...
  /*

    {
     ...
     "processed" :[
                    {"2DSA"
    :{"gfacID":"17","status":"COMPLETE","statusMsg":"Finished:  maxrss 441 MB,
    total run seconds 48"}},
                    {"2DSA_FM":{"gfacID":"18","status":"COMPLETE","statusMsg":"Finished:
    maxrss 462 MB,  total run seconds 32"}},
                    {"FITMEN"
    :{"gfacID":null,"status":"COMPLETE","statusMsg":"Waiting for manual stage
    FITMEN to complete."}}
                  ],
     "submitted":"2DSA_IT"
     }
  ...
  */
  ...
  for (jj = Array_of_analysis.begin(); jj != Array_of_analysis.end(); ++jj) {
    ...
    qDebug() << "triple_curr_key, triple_curr: " << triple_curr_key << ", "
             << triple_curr;
    ...
    if (!current_analysis_details.size()) {
      ...
      qDebug() << "FORM HISTORY: set Completed_triples[ " << triple_curr_key
               << " ] to " << Completed_triples[triple_curr_key];
    }
    ...
    QString curr_HPCAnalysisRequestID =
        current_analysis_details["currentHPCARID"];
    ...
    if (to_process.isUndefined())
      ...
      for (int i = 0; i < to_process_array.size(); ++i) {
        ...
        if (to_process_stage_groupbox != NULL) {
          // update GUI
          QLineEdit* lineedit_runid =
              to_process_stage_groupbox->findChild<QLineEdit*>(
                  "runID", Qt::FindDirectChildrenOnly);
          QLineEdit* lineedit_owner =
              to_process_stage_groupbox->findChild<QLineEdit*>(
                  "owner", Qt::FindDirectChildrenOnly);
          QLineEdit* lineedit_lastmsg =
              to_process_stage_groupbox->findChild<QLineEdit*>(
                  "lastmsg", Qt::FindDirectChildrenOnly);
          QLineEdit* lineedit_status =
              to_process_stage_groupbox->findChild<QLineEdit*>(
                  "status", Qt::FindDirectChildrenOnly);
          QLineEdit* lineedit_anatype =
              to_process_stage_groupbox->findChild<QLineEdit*>(
                  "anatype", Qt::FindDirectChildrenOnly);
          QLineEdit* lineedit_submit =
              to_process_stage_groupbox->findChild<QLineEdit*>(
                  "submit", Qt::FindDirectChildrenOnly);
          QLineEdit* lineedit_cluster =
              to_process_stage_groupbox->findChild<QLineEdit*>(
                  "cluster", Qt::FindDirectChildrenOnly);
          QLineEdit* lineedit_lastupd =
              to_process_stage_groupbox->findChild<QLineEdit*>(
                  "lastupd", Qt::FindDirectChildrenOnly);

          QPushButton* pb_delete =
              to_process_stage_groupbox->findChild<QPushButton*>(
                  "delete", Qt::FindDirectChildrenOnly);

          // delete button
          pb_delete->setEnabled(false);

          // runID
          QString runid_text = filename + " ( " + triple_curr + " )  " +
                               defaultDB + " (ID: N/A)";
          lineedit_runid->setText(runid_text);

          // owner
          QString investigator_text = investigator_details["email"] + " (" +
                                      investigator_details["fname"] + " " +
                                      investigator_details["lname"] + ")";
          ...
          lineedit_status->setStyleSheet(
              "QLineEdit { background-color:  rgb( 255, 255, 102 ); }");
    ...
    if (processed.isUndefined())
      ...
      for (int i = 0; i < processed_array.size(); ++i) {
        ...
        foreach (const QString& key, processed_array[i].toObject().keys()) {
          QJsonObject newObj =
              processed_array[i].toObject().value(key).toObject();

          qDebug() << "Processed stage - " << key
                   << ": gfacID, status, statusMsg -- "
        ...
        // IF Canceled by user:
        if (stage_name.contains("FITMEN") &&
            (stage_status == "CANCELED" || stage_status == "canceled")) {
          ...
          qDebug() << "CANCELED status for triple/stage (FITMEN) -- "
                   << triple_curr_key << "/" << submitted.toString();
        ...
        if (processed_stage_groupbox != NULL) {
          // update GUI
          QLineEdit* lineedit_runid =
              processed_stage_groupbox->findChild<QLineEdit*>(
                  "runID", Qt::FindDirectChildrenOnly);
          QLineEdit* lineedit_owner =
              processed_stage_groupbox->findChild<QLineEdit*>(
                  "owner", Qt::FindDirectChildrenOnly);
          ...
          QPushButton* pb_delete =
              processed_stage_groupbox->findChild<QPushButton*>(
                  "delete", Qt::FindDirectChildrenOnly);
          QPushButton* pb_overlay =
              processed_stage_groupbox->findChild<QPushButton*>(
                  "overlay", Qt::FindDirectChildrenOnly);
          ...
          lineedit_status->setStyleSheet(
              "QLineEdit { background-color:  rgb( 2, 88, 57 ); color : white; "
              "}");
          ...
          if (stage_status == "CANCELED" || stage_status == "canceled") {
            lineedit_status->setStyleSheet(
                "QLineEdit { background-color:  rgb(210, 0, 0); color : white; "
                "}");
            ...
            qDebug() << "CANCELED status( Processed ) for triple/stage -- "
                     << triple_curr_key << "/" << submitted.toString();
          }
          if (stage_status == "FAILED" || stage_status == "failed") {
            ...
            qDebug() << "FAILED status( Processed ) for triple/stage -- "
                     << triple_curr_key << "/" << submitted.toString();
          }
        }
      }
      ...
      if (!to_process_array.size() && History_read[triple_curr_key] &&
        ...
        qDebug() << "Setting triple to complete - read from History, status = "
                 << status;
    ...
    if (submitted.isUndefined())
      ...
      qDebug() << "Submitted stage - " << submitted.toString()
               << ", for triple " << triple_curr;
      ...
      if (submitted.toString() == "FITMEN_AUTO" &&
          Process_2dsafm[triple_curr_key]) {
        // What if submited stage still "FITMEN_AUTO" but
        // nextWaitStatus="COMPLETE" ? (the submission daemon didn't yet updated
        // submitted status...) I.e., FITMEN_AUTO was processed BUT submission
        // daemon still sees it in 'status' "FITMEN_AUTO" ?
        ...
        if (fitmen_auto_processed) {
          qDebug() << "CHECK on FITMEN_AUTO: already processed ? -- "
                   << fitmen_auto_processed;
          in_gui_update = false;
        ...
        in_gui_update = false;
        qDebug() << "Update stopped at FITMEN_AUTO for triple -- "
                 << triple_curr_key;
        ...
        triple_info_map_auto["filename"] =
            filename;  // ALEXEY -- NOT 'FileName' for combined runs;
      ...
      if (submitted.toString() == "FITMEN" && !Manual_update[triple_curr_key] &&
        ...
        QMessageBox::information(
            this, tr("ATTENTION: FITMEN stage reached"),
            tr("FITMEN stage for triple %1 will be processed manually.")
                .arg(triple_curr));

        //--- Check status of the FITMEN | Entire Analysis for the triple before
        //calling FitMen constructor:
        bool fitmen_processed;
        fitmen_processed = check_fitmen_status(requestID, stage_name);

        if (fitmen_processed) {
          Manual_update[triple_curr_key] = true;
          qDebug() << "CHECK on FITMEN: already processed ? -- "
                   << fitmen_processed;
        ...
        /** To FitMeniscus class -- pass:
            1. Name of the triple to title && and possibly to pass to scan_db()
        method
        ...
        ****/
        ...
        /** The following will block parent windows from closing BUT not from
         * continuing timer execution ***/
        FitMen->setWindowFlags(Qt::Dialog | Qt::WindowTitleHint |
                               Qt::WindowMinimizeButtonHint);
        FitMen->setWindowModality(Qt::ApplicationModal);
        /***************************************************************************************************/
        connect(
            FitMen, SIGNAL(editProfiles_updated(QMap<QString, QString>&)), this,
            SLOT(update_autoflowAnalysis_statuses(QMap<QString, QString>&)));

        connect(FitMen, SIGNAL(editProfiles_updated_earlier()), this,
                SLOT(editProfiles_updated_earlier()));

        connect(FitMen, SIGNAL(triple_analysis_processed()), this,
                SLOT(triple_analysis_processed()));

        // connect( FitMen, SIGNAL( bad_meniscus_values( QMap < QString, QString
        // > & ) ),  //This will take care of BAD values 	       this, SLOT(
        // delete_jobs_at_fitmen( QMap < QString, QString > & ) ) );
        ...
        qDebug() << "Update stopped at FITMEN for triple -- "
                 << triple_curr_key;
      ...
      // Special case: children triple of the channel, while parent triple
      // (selected wvl) is CANCELED
      if ((submitted.toString() == "FITMEN" ||
           submitted.toString() == "FITMEN_AUTO") &&
          (nextWaitStatus == "CANCELED" || nextWaitStatus == "canceled")) {
        if (groupbox_2DSA_IT.contains(triple_curr)) {
          current_stage_groupbox = groupbox_2DSA_IT[triple_curr];
          stage_name = "2DSA-IT";
        ...
        if (fitmen_bad_vals) status = "FAILED";
      ...
      if (current_stage_groupbox != NULL) {
        QLineEdit* lineedit_runid =
            current_stage_groupbox->findChild<QLineEdit*>(
                "runID", Qt::FindDirectChildrenOnly);
        QLineEdit* lineedit_owner =
            current_stage_groupbox->findChild<QLineEdit*>(
                "owner", Qt::FindDirectChildrenOnly);
        ...
        QPushButton* pb_delete =
            current_stage_groupbox->findChild<QPushButton*>(
                "delete", Qt::FindDirectChildrenOnly);
        QPushButton* pb_overlay =
            current_stage_groupbox->findChild<QPushButton*>(
                "overlay", Qt::FindDirectChildrenOnly);
        ...
        // delete button: if running && admin_user || user_owner, enable;
        // otherwise, disable (should be disabled earlier)
        if (US_Settings::us_inv_level() > 3 ||
            US_Settings::us_inv_ID() == invID)
        ...
        // runID
        QString runid_text = filename + " ( " + triple_curr + " )  " +
                             defaultDB + " (ID: " + curr_HPCAnalysisRequestID +
                             ")";
        lineedit_runid->setText(runid_text);
        ...
        if (status == "READY" || status == "SUBMITTED") {
          ...
          lineedit_status->setStyleSheet(
              "QLineEdit { background-color:  rgb(95, 152, 238); }");
        } else {
          lineedit_status->setText(status);
          lineedit_status->setStyleSheet(
              "QLineEdit { background-color:  rgb(50, 205, 50); }");
        }
        ...
        if (status == "COMPLETE") {
          ...
          if (!to_process_array.size()) {
            topItem[triple_curr]->setForeground(0, QBrush(colorGreen));
            /****
                 ALEXEY: maybe update QMap < triple_curr , bool >
            Completed_triples with TRUE (defined as all FALSES at the
            beginning): Completed_triples[ triple_curr ] = true; At the end of
            update cycle over triples, check if there is at least one FALSE
            value in the  Completed_triple QMap. If NO - stop timer, show Msg
            that everything completed.
            **/

            /** Comment the line BELOW as this MAY cause RACE CONDITIONS
             * !!!!!***************************/
            // Completed_triples[ triple_curr_key ] = true;  // May cause race
            // conditions!!!!
            /*******************************************************************************************/

            qDebug() << "set Completed_triples[ " << triple_curr_key << " ] to "
                     << Completed_triples[triple_curr_key];
          }
        }
        ...
        if (status == "FAILED" || status == "failed" || status == "CANCELED" ||
          ...
          if (status == "FAILED" || status == "failed") {
            Failed_triples[triple_curr_key] = true;
            qDebug() << "FAILED status for triple/stage -- " << triple_curr_key
                     << "/" << submitted.toString();
          }
          if (status == "CANCELED" || status == "canceled") {
            Canceled_triples[triple_curr_key] = true;
            qDebug() << "CANCELED status for triple/stage -- "
                     << triple_curr_key << "/" << submitted.toString();
          }
        }
      }
  ...
  for (td = Completed_triples.begin(); td != Completed_triples.end(); ++td) {
    qDebug() << "Key/Value of Completed_triples: " << td.key() << "/"
             << td.value();
    if (!td.value()) {
      all_processed = false;
      break;
  ...
  for (tdf = Failed_triples.begin(); tdf != Failed_triples.end(); ++tdf) {
    qDebug() << "Key/Value of Failed_triples: " << tdf.key() << "/"
             << tdf.value();
    if (tdf.value()) {
  ...
  qDebug() << "Failed_triples exist ? Name(s): " << failed_triples
           << Failed_triples_list;
  ...
  for (tdc = Canceled_triples.begin(); tdc != Canceled_triples.end(); ++tdc) {
    qDebug() << "Key/Value of Canceled_triples: " << tdc.key() << "/"
             << tdc.value();
    if (tdc.value()) {
  ...
  qDebug() << "Canceled_triples exist ? Name(s): " << canceled_triples
           << Canceled_triples_list;
  ...
  if (all_processed) {
    ...
    if (canceled_triples)
      msg_text += QString(
                      "\n\nNOTE: analyses for the following triples have been "
                      "CANCELED: \n\n%1")
    ...
    if (failed_triples || canceled_triples) {
      QMessageBox::information(this, tr("All Triples Processed !"), msg_text);
    }
  ...
  while (db->next()) {
    ...
    if (description.contains(triple_information["stage_name"])) {
      if (triple_information["stage_name"] == "2DSA") {
        if (!description.contains("-FM_") && !description.contains("-IT_") &&
            !description.contains("-MC_") && !description.contains("_mcN")) {
          int time_to_now = date.secsTo(now);
          if (time_to_now < latest_update_time) {
            latest_update_time = time_to_now;
            // qDebug() << "Edited profile MAX, NOW, DATE, sec-to-now -- " <<
            ...
            qDebug() << "Model 2DSA: ID, desc, timetonow -- " << modelID
                     << description << time_to_now;
        ...
        if (time_to_now < latest_update_time) {
          ...
          qDebug() << "Model NON-2DSA: ID, desc, timetonow -- " << modelID
                   << description << time_to_now;
...
double US_Analysis_auto::interp_sval(double xv, double* sx, double* sy,
                                     int ssize) {
...
void US_Analysis_auto::show_overlay(QString triple_stage) {
  ...
  progress_msg = new QProgressDialog("Downloading data and models...",
                                     QString(), 0, 5, this);
  ...
  // msg_sim->setWindowFlags ( Qt::CustomizeWindowHint | Qt::WindowTitleHint |
  ...
  // msg_sim->setText( QString( tr( "Downloading data and model and
  // simulating... \nPlease wait...") ) );
...
// Cancel all jobs if FITMEN for a channel was processed by other means: NO FM
// modles
void US_Analysis_auto::delete_jobs_at_fitmen(
    QMap<QString, QString>& triple_info) {
  qDebug() << "At delete_jobs_at_fitmen: triple_name: "
           << triple_info["triple_name_key"];
  ...
  qDebug() << "In delete_at_fitmen other WVLs: channel info -- "
           << requestID_list;
  ...
  if (requestID_list.size() > 1) {
    ...
    qDebug() << "In delete_at_fitmen other WVLs: MODIFIED channel info -- "
             << requestID_list;
    ...
    for (int i = 0; i < requestID_list.size(); ++i) {
      ...
      QMap<QString, QString> ana_details_affected =
          Array_of_analysis_by_requestID[requestID_affected];
  ...
  if (!fitmen_bad_vals) {
    ...
    QString msg_sys_text =
        QString(
            tr("ATTENTION!\n\nNo -FM models for triple %1 have been found. \n\n"
               ...
               "2. The data have been analyzed outside of the GMP "
               "framework.\n\n"
    ...
    if (mwl_channel)
      msg_sys_text += QString(
                          "\n\nSince this is a multi-wavelength analysis, "
                          "scheduled jobs for the following same-channel "
                          "triples will be canceled as well: \n\n%1")
    ...
    QPushButton* Accept_sys =
        msg_delete.addButton(tr("OK"), QMessageBox::YesRole);
    // QPushButton *Cancel_sys    = msg_delete.addButton(tr("Cancel"),
    ...
    if (msg_delete.clickedButton() == Accept_sys) {
      ...
      qDebug() << "[in delete_fitmen_job(no fitmen_bad_vals)] "
                  "status_fitmen_unique -- "
               << status_fitmen_unique;

      if (!status_fitmen_unique) {
        QMessageBox::information(
            this, tr("FITMEN | Triple Analysis already processed"),
            tr("It appears that FITMEN stage has already been OR being "
               "processed by "
      ...
      // ]; qDebug() << "Status && statusMsg to DETETE! -- " <<
      // current_analysis[ "status" ] << current_analysis[ "status_msg" ];
      /* **********/
    ...
    msg_delete_bad_vals.setWindowFlags(Qt::CustomizeWindowHint |
                                       Qt::WindowTitleHint);
    msg_delete_bad_vals.setWindowTitle(tr("Job Cancelation!"));

    QString msg_sys_text_bv =
        QString(
            tr("ATTENTION!\n\n The meniscus fit for tiple %1 resulted in a "
               "best-fit value that lies outside "
               "of the considered range for the meniscus. "
               "This should not happen, unless: \n\n"
               "1. The initial meniscus was not selected correctly; \n"
               "2. The experimental data are non-ideal and cannot be modeled "
               "with the standard finite element model;\n"
               "3. The radial calibration of the instrument is incorrect. \n\n"
               "The user should examine the data manually and determine the "
    ...
    QPushButton* Accept_sys =
        msg_delete_bad_vals.addButton(tr("OK"), QMessageBox::YesRole);
    // QPushButton *Cancel_sys    = msg_delete.addButton(tr("Cancel"),
    // QMessageBox::RejectRole);
    ...
    if (msg_delete_bad_vals.clickedButton() == Accept_sys) {
      ...
      qDebug() << "[in delete_fitmen_job(yes fitmen_bad_vals)] "
                  "status_fitmen_unique -- "
               << status_fitmen_unique;
  ...
  qDebug() << "Timer restarted after Canceling jobs at FITMEN (processed by "
              "other means) for channel -- "
           << channel_n;
}
...
void US_Analysis_auto::delete_job(QString triple_stage) {
  ...
  if (requestID_list.size() > 1) {
    ...
    // remove currently selected requestID -- for primary wvl
    requestID_list.removeOne(requestID);
    qDebug() << "In delete other WVLs: MODIFIED channel info -- "
             << requestID_list;
  ...
  QString msg_sys_text = QString(
                             "ATTENTION!\nYou have chosen to abort an ongoing "
                             "job for: \n triple %1, stage %2!")
                             .arg(triple_n)
                             .arg(stage_n);

  // if ( mwl_channel && ( stage_n == "2DSA" || stage_n == "2DSA-FM" ) )
  if (Process_2dsafm[triple_n] && mwl_channel &&
      (stage_n == "2DSA" ||
       stage_n == "2DSA-FM"))  // don't we have to do this ONLY fro
                               // representative triple??
    msg_sys_text += QString(
                        "\n\nSince this is a multi-wavelength analysis, "
                        "ongoing jobs for the following same-channel triples "
  ...
  QPushButton* Accept_sys =
      msg_delete.addButton(tr("YES"), QMessageBox::YesRole);
  QPushButton* Cancel_sys =
      msg_delete.addButton(tr("Cancel"), QMessageBox::RejectRole);
  ...
  if (msg_delete.clickedButton() == Accept_sys) {
    ...
    QString comment_t =
        QInputDialog::getText(this, tr("Reason for job CANCELLATION"), msg,
    ...
    // qDebug() << "GUID to DETETE! -- " << current_analysis[ "CurrentGfacID" ];
    // qDebug() << "Status && statusMsg to DETETE! -- " << current_analysis[
    // "status" ] << current_analysis[ "status_msg" ];
    /* **********/

    update_autoflowAnalysis_uponDeletion(&db, requestID);

    // Now, if 2DSA or 2DSA-FM of the representative wvl in the MWL case: DELETE
    // jobs for all other triples in a channel:
    QString other_chan_triples;
    if (Process_2dsafm[triple_n] &&
        (stage_n == "2DSA" || stage_n == "2DSA-FM") && mwl_channel) {
      update_autoflowAnalysis_uponDeletion_other_wvl(&db, requestID_list);
    ...
    record_or_update_analysis_cancel_status(&db, triple_n, other_chan_triples,
                                            comment_t);
...
void US_Analysis_auto::update_autoflowAnalysis_uponDeletion(
  ...
  if (db->lastErrno() != US_DB2::OK) {
    QMessageBox::warning(this, tr("Connection Problem"),
                         tr("Updating autoflowAnalysis at DELETEe (primary "
                            "wvl): Could not connect to database \n") +
                             db->lastError());
    return;
  }
  ...
  qry << "update_autoflow_analysis_record_at_deletion"
      << QString("Job scheduled for deletion, analysis stage") << requestID;
...
// slot to update autoflowAnalysis records when DELETE pressed for all other wvl
// in a channel
void US_Analysis_auto::update_autoflowAnalysis_uponDeletion_other_wvl(
    US_DB2* db, const QStringList& requestID_list) {
  if (db->lastErrno() != US_DB2::OK) {
    QMessageBox::warning(this, tr("Connection Problem"),
                         tr("Updating autoflowAnalysis at DELETE (other wvl): "
...
// slot to update autoflowAnalysis record at fitmen stage && register manually
// updated triple
void US_Analysis_auto::update_autoflowAnalysis_statuses(
    QMap<QString, QString>& triple_info) {
  QString triple_curr_key = triple_info["triple_name_key"];
  QString requestID = triple_info["requestID"];

  // Identify list of requestIDs needed to be updated at FITMEN (especcially for
  // MWL cases):
  ...
  QString FMB_changed = triple_info.contains("FMB_changed")
                            ? triple_info["FMB_changed"]
                            : QString("");
  ...
  1. If field is empty, first add initial record:
       UPDATE autoflowStatus set analysis='[{"2 / A / 260": "changed"}]' where
  ID=22;

  2. If not empty (then and after)
       UPDATE autoflowStatus set analysis=JSON_ARRAY_APPEND(analysis, '$',
  JSON_OBJECT('2 / B / 260', 'NO')) where ID=22;
  **********************************************/

  // Update autoflowStatus's 'analysis' field: per-channel (reference triple)
  // basis, by whom:
  US_Passwd pw;
  US_DB2* dbP = new US_DB2(pw.getPasswd());
  ...
  qDebug() << "Channel_name, requestIDs to update: " << channel_name
           << requestID_list;

  // --- Before updating autoflowAnalysis records ---
  // --- MAKE sure that ALL original 2DSA stages for other wavelength in a
  // channel (besides representative wvl) are completed
  ...
  qDebug() << "Timer restarted after updating EditProfiles for channel -- "
           << channel_name;
}

// Keep track on meniscus (or bottom) deviations in FITMEN from the best fit:
void US_Analysis_auto::record_or_update_analysis_cancel_status(
    US_DB2* db, QString triple_name, QString other_chan_triples,
    QString comment_t) {
  // Check DB connection
  if (db->lastErrno() != US_DB2::OK) {
    QMessageBox::warning(this, tr("Connection Problem"),
                         tr("Updating autoflowStatus's analysis at FITMEN: "
                            "Could not connect to database \n") +
                             db->lastError());
    return;
  }
  ...
  if (!other_chan_triples.isEmpty()) CancelTriples += "," + other_chan_triples;

  QString CancelAction =
      "CANCELED, by " + fname + ", " + lname + "; COMMENT, " + comment_t;

  if (autoflowStatusID) {
    // update OR insert NEW Json
    qry << "update_autoflowStatusAnalysisCancel_record"
        << QString::number(autoflowStatusID)
        << QString::number(autoflowID_passed) << CancelTriples << CancelAction;

    db->query(qry);
  } else {
    QMessageBox::warning(this, tr("AutoflowStatus Record Problem"),
                         tr("autoflowStatus (Analysis {CANCELED triples}): "
                            "There was a problem with identifying a record in "
                            "autoflowStatus table for a given run! \n"));
...
// Keep track on meniscus (or bottom) deviations in FITMEN from the best fit:
void US_Analysis_auto::record_or_update_analysis_meniscus_status(
    US_DB2* db, QString triple_name, QString FMB_changed) {
  if (FMB_changed.isEmpty()) {
    qDebug() << "FBM_changed EMPTY: It seems it's impossible to determine if "
                "meniscus| bottom was modified from the best FIT...";
    return;
  }
  ...
  qDebug() << "IN ANALYSIS, record autoflowStatus: ID,name,email,lev" << ID
  ...
  QString
      analysisJson;  //  [{"6 / B / 260": "fitted"}]','6 / B / 260', 'fitted'

  QString analysisTriple = triple_name;
  analysisTriple.replace(".", " / ");

  QString analysisAction =
      (FMB_changed == "YES") ? "modified" : "best fit selected";
  ...
  if (autoflowStatusID) {
    ...
    qry << "update_autoflowStatusAnalysisFitmen_record"
        ...
        // << analysisJson                            //ALEXEY: not needed with
        // new proc.
        << analysisTriple << analysisAction;

    db->query(qry);
  } else {
    QMessageBox::warning(
        this, tr("AutoflowStatus Record Problem"),
        tr("autoflowStatus (Analysis {FMB}): There was a problem with "
           "identifying a record in autoflowStatus table for a given run! \n"));
...
// slot to update autoflowAnalysis record at fitmen stage WHEN already treated
...
void US_Analysis_auto::reset_analysis_panel_public() { reset_analysis_panel(); }

// Check FITMEN | FITMEN_AUTO status before calling FITMEN constructor OR
...
bool US_Analysis_auto::check_fitmen_status(const QString& requestID,
  ...
  if (!analysis_details.size()) {
    // no record, so analysis completed/cancelled and already in the
    ...
    QMessageBox::information(
        ...
        QString(
            tr("It appears that %1 stage has already been processed by "
    ...
    qDebug() << "In check_fitmen 2: -- " << status_gen << "\n"
             << nextWaitStatus << "\n"
             << status_json;
    ...
    qDebug() << "In check_fitmen 3: -- " << submitted.toString() << stageName;
    ...
    if (submitted.toString() == stageName) {
      ...
      // FITMEN | FITMEN_AUTO  is no in "submitted" stages anymore, so it's
      // processed:
      QMessageBox::information(
          this, QString(tr("%1 already processed")).arg(stageName),
          QString(
...
void US_Analysis_auto::reset_analysis_panel() {
  if (autoflow_expType != "ABDE" || autoflow_expType == "VELOCITY") {
    ...
    // ALEXEY: now we should wait for completion of the last timer_update
    // shot...
...
void US_Analysis_auto::end_process() {
  qDebug() << "In the END process: in_reload_end_processes = "
           << in_reload_end_process;
  ...
  if (!in_gui_update) {
    ...
    disconnect(timer_end_process, SIGNAL(timeout()), 0,
               0);  // Disconnect timer from anything
...
QGroupBox* US_Analysis_auto::createGroup(QString& triple_name) {
  ...
  groupBox->setStyleSheet(
      "QGroupBox { font: bold;  background-color: qlineargradient(x1: 0, y1: "
      "0, x2: 0, y2: 1, stop: 0 #E0E0E0, stop: 1 #FFFFFF); border: 2px solid "
      "gray; border-radius: 10px; margin-top: 20px; margin-bottom: 10px; "
      "padding-top: 5px; } QGroupBox::title { subcontrol-origin: margin; "
      "subcontrol-position: top left; left: 10px; margin: 0 5px; "
      "background-color: black; color: white; padding: 0 3px;}  "
      "QGroupBox::indicator { width: 13px; height: 13px; border: 1px solid "
      "grey; background-color: rgba(204, 204, 204, 255);} "
      "QGroupBox::indicator:hover {background-color: rgba(235, 235, 235, "
      "255);} QLabel {background-color: rgb(105,105,105);}");
  ...
  connect(signalMapper_overlay, SIGNAL(mapped(QString)), this,
          SLOT(show_overlay(QString)));
...
// Unfold tree
void US_Analysis_auto::show_all() { treeWidget->expandAll(); }

// collapse tree
void US_Analysis_auto::hide_all() { treeWidget->collapseAll(); }

// Read AutoflowAnalysisRecord
QMap<QString, QString> US_Analysis_auto::read_autoflowAnalysis_record(
    US_DB2* db, const QString& requestID) {
  QMap<QString, QString> analysis_details;
...
QMap<QString, QString> US_Analysis_auto::read_autoflowAnalysisHistory_record(
    US_DB2* db, const QString& requestID) {
  QMap<QString, QString> analysis_details;
...
QMap<QString, QString> US_Analysis_auto::get_abdeAnalysis_info(
    US_DB2* db, const QString& ID) {
  QMap<QString, QString> abdeAnalysis_details;

  QStringList qry;
  qry << "read_autoflowAnalysisABDE_record" << ID;
...
QMap<QString, QString> US_Analysis_auto::get_investigator_info(
    US_DB2* db, const QString& ID) {
  QMap<QString, QString> inv_details;

  QStringList qry;
  qry << "get_person_info" << ID;
...
void US_Analysis_auto::update_autoflowAnalysis_status_at_fitmen(
    US_DB2* db, const QStringList& requestID_list) {
  // // Check DB connection
  // US_Passwd pw;
  // QString masterpw = pw.getPasswd();
  ...
  if (db->lastErrno() != US_DB2::OK) {
    QMessageBox::warning(this, tr("Connection Problem"),
                         tr("Updating autoflowAnalysis at FITMEN: Could not "
                            "connect to database \n") +
                             db->lastError());
    return;
  }
  ...
  for (int i = 0; i < requestID_list.size(); ++i) {
    qry.clear();
    qry << "update_autoflow_analysis_record_at_fitmen" << requestID_list[i];
...
void US_Analysis_auto::gui_update_temp() {
  ...
  for (jj = Array_of_analysis.begin(); jj != Array_of_analysis.end(); ++jj) {
    ...
    // 	{
    // 2DSA
    QLineEdit* lineedit_2dsa_runID =
        groupbox_2DSA[triple_curr]->findChild<QLineEdit*>(
            "runID", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_owner =
        groupbox_2DSA[triple_curr]->findChild<QLineEdit*>(
            "owner", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_lastmsg =
        groupbox_2DSA[triple_curr]->findChild<QLineEdit*>(
            "lastmsg", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_status =
        groupbox_2DSA[triple_curr]->findChild<QLineEdit*>(
            "status", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_anatype =
        groupbox_2DSA[triple_curr]->findChild<QLineEdit*>(
            "anatype", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_submit =
        groupbox_2DSA[triple_curr]->findChild<QLineEdit*>(
            "submit", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_lastupd =
        groupbox_2DSA[triple_curr]->findChild<QLineEdit*>(
            "lastupd", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_cluster =
        groupbox_2DSA[triple_curr]->findChild<QLineEdit*>(
            "cluster", Qt::FindDirectChildrenOnly);

    lineedit_2dsa_runID->setText(
        "demo1_interference (2/A/660) uslims3_CAUMA (ID: 8837)");
    ...
    lineedit_2dsa_status->setStyleSheet(
        "QLineEdit { background-color:  rgb(50, 205, 50); }");
    ...
    // 2DSA-FM
    QLineEdit* lineedit_2dsa_fm_runID =
        groupbox_2DSA_FM[triple_curr]->findChild<QLineEdit*>(
            "runID", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_fm_owner =
        groupbox_2DSA_FM[triple_curr]->findChild<QLineEdit*>(
            "owner", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_fm_lastmsg =
        groupbox_2DSA_FM[triple_curr]->findChild<QLineEdit*>(
            "lastmsg", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_fm_status =
        groupbox_2DSA_FM[triple_curr]->findChild<QLineEdit*>(
            "status", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_fm_anatype =
        groupbox_2DSA_FM[triple_curr]->findChild<QLineEdit*>(
            "anatype", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_fm_submit =
        groupbox_2DSA_FM[triple_curr]->findChild<QLineEdit*>(
            "submit", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_fm_lastupd =
        groupbox_2DSA_FM[triple_curr]->findChild<QLineEdit*>(
            "lastupd", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_fm_cluster =
        groupbox_2DSA_FM[triple_curr]->findChild<QLineEdit*>(
            "cluster", Qt::FindDirectChildrenOnly);

    lineedit_2dsa_fm_runID->setText(
        "demo1_interference (2/A/660) uslims3_CAUMA (ID: N/A)");
    ...
    lineedit_2dsa_fm_status->setStyleSheet(
        "QLineEdit { background-color:  rgb(255, 255, 102); }");
    ...
    // 2DSA-IT
    QLineEdit* lineedit_2dsa_it_runID =
        groupbox_2DSA_IT[triple_curr]->findChild<QLineEdit*>(
            "runID", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_it_owner =
        groupbox_2DSA_IT[triple_curr]->findChild<QLineEdit*>(
            "owner", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_it_lastmsg =
        groupbox_2DSA_IT[triple_curr]->findChild<QLineEdit*>(
            "lastmsg", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_it_status =
        groupbox_2DSA_IT[triple_curr]->findChild<QLineEdit*>(
            "status", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_it_anatype =
        groupbox_2DSA_IT[triple_curr]->findChild<QLineEdit*>(
            "anatype", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_it_submit =
        groupbox_2DSA_IT[triple_curr]->findChild<QLineEdit*>(
            "submit", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_it_lastupd =
        groupbox_2DSA_IT[triple_curr]->findChild<QLineEdit*>(
            "lastupd", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_it_cluster =
        groupbox_2DSA_IT[triple_curr]->findChild<QLineEdit*>(
            "cluster", Qt::FindDirectChildrenOnly);

    lineedit_2dsa_it_runID->setText(
        "demo1_interference (2/A/660) uslims3_CAUMA (ID: N/A)");
    ...
    lineedit_2dsa_it_status->setStyleSheet(
        "QLineEdit { background-color:  rgb(255, 255, 102); }");
    ...
    // 2DSA-MC
    QLineEdit* lineedit_2dsa_mc_runID =
        groupbox_2DSA_MC[triple_curr]->findChild<QLineEdit*>(
            "runID", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_mc_owner =
        groupbox_2DSA_MC[triple_curr]->findChild<QLineEdit*>(
            "owner", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_mc_lastmsg =
        groupbox_2DSA_MC[triple_curr]->findChild<QLineEdit*>(
            "lastmsg", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_mc_status =
        groupbox_2DSA_MC[triple_curr]->findChild<QLineEdit*>(
            "status", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_mc_anatype =
        groupbox_2DSA_MC[triple_curr]->findChild<QLineEdit*>(
            "anatype", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_mc_submit =
        groupbox_2DSA_MC[triple_curr]->findChild<QLineEdit*>(
            "submit", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_mc_lastupd =
        groupbox_2DSA_MC[triple_curr]->findChild<QLineEdit*>(
            "lastupd", Qt::FindDirectChildrenOnly);
    QLineEdit* lineedit_2dsa_mc_cluster =
        groupbox_2DSA_MC[triple_curr]->findChild<QLineEdit*>(
            "cluster", Qt::FindDirectChildrenOnly);

    lineedit_2dsa_mc_runID->setText(
        "demo1_interference (2/A/660) uslims3_CAUMA (ID: N/A)");
    ...
    lineedit_2dsa_mc_status->setStyleSheet(
        "QLineEdit { background-color:  rgb(255, 255, 102); }");
...
void US_Analysis_auto::scan_dbase_auto(
  ...
  QRegExp fmIter = QRegExp("i\?\?-[mb]*", Qt::CaseSensitive, QRegExp::Wildcard);
  ...
  while (db.next()) {
    ...
    if (descript.length() ==
        80) {  // Truncated description:  save ID and skip update for now
    ...
    if (ansysID.contains("2DSA-F") ||
        iterID.contains(
            fmIter)) {  // Model from meniscus fit, so save information
      ...
      QString fextn = (fittype != 2) ? ".fitmen.dat" : ".fitbot.dat";
      DbgLv(1) << "DbSc:     fittype" << fittype << "fextn" << fextn;
      if (fittype == 2 ||
          fittype ==
              3) {  // Bottom or Meniscus+Bottom:  add to list of bottom redo's
      ...
      DbgLv(1) << "DbSc:       anType" << anType << "editLabel" << editLabel
               << "ansysID" << ansysID;
      QString ftfname =
          runID + "/" + anType + "." + editLabel + "." + tripleID + fextn;
      mdescr.description = descript;
      mdescr.baseDescr =
          runID + "." + tripleID + "." + ansysID.section("-", 0, 3);
      ...
      mdescr.antime = descript.section('.', -2, -2).section('_', 1, 1).mid(1);
  ...
  for (int ii = 0; ii < tmodels.size(); ii++) {
    ...
    if (ansysID.contains("2DSA-F") ||
      ...
      if (fittype !=
          1) {  // Bottom or Meniscus+Bottom:  add to list of bottom redo's
        botredo << mDescrs.count();
      }
  ...
  for (int ii = 0; ii < botredo.count(); ii++) {
    ...
    if (bottom <
        1.0) {  // Bottom not reliable in model, get from model description
      ...
      if (fitVals.length() > 6)
        ...
        fittype = 1;  // meniscus fit
      bottom = (fittype == 2) ? (fitVals.mid(1, 5).toDouble() / 10000.0)
  ...
  for (int ii = 0; ii < nfmods;
       ii++) {  // Find unique file names in order to create sets
    QString ftfname = mDescrs[ii].fitfname;
    QString antime = mDescrs[ii].antime;

    if (!ufnams.contains(
            ftfname)) {  // This is a new fit-file name, so new analysis set
      ufnams << ftfname;
      uantms << antime;
    }

    else if (!uantms.contains(antime)) {  // Already seen fit-file, but new
                                          // analysis time, so duplicate
      uantms << antime;
    }
  ...
  for (int ii = 0; ii < kfsets;
       ii++) {  // Find out for each set whether a corresponding fit file exists
    QString ftfname = ufnams.at(ii);

    if (mfnams.count(ftfname) ==
        1) {  // Not really a set; single fit model after previous fm run
      nfsets--;
    ...
    if (ftfile.exists()) {  // File exists, so we must check the need to replace
                            // it
      QString ftfpath = rdir + ftfname;
      ...
      DbgLv(1) << "   jj desc" << jj << mDescrs[jj].description
               << "antime meniscus" << mDescrs[jj].antime
               << mDescrs[jj].meniscus;

      if (fdate.msecsTo(rdate) >
          0) {  // DB record date is later than file date, so must replace file
        nfrpls++;
        ftfile.remove();
      }

      else {  // DB record date is not later than file date, so leave file as is
        nfexss++;
        continue;
      }
    }

    else {  // File does not exist, so we definitely need to create it
      nfadds++;
    ...
    if (!ftfile.open(QIODevice::WriteOnly | QIODevice::Text)) {  // Problem!!!
    ...
    for (int jj = jfirst; jj < jlast;
         jj++) {  // First build the pairs (or triples) list
      ...
      if (bottom > 1.0) {              // Either Bottom or Meniscus+Bottom
        if (ftfname.contains("FB")) {  // Bottom only
          mrpair = QString::number(bottom, 'f', 6) + " " +
                   QString::number(rmsd, 'e', 6);
        } else {  // Meniscus and Bottom
      ...
      if (antime == antiml) mrpairs << mrpair;
    }
    ...
    for (int jj = 0; jj < mrpairs.size(); jj++) ts << mrpairs.at(jj) + "\n";
...
void US_Analysis_auto::get_editProfile_copy(
    QMap<QString, QString>& triple_information) {
  qDebug() << "In get_edit COPY: ";
  /* ALEXEY: we can use only triple_information[ "filename" ] (the editedData
     table's 'label' filed) to extract all editProfiles for this runID (which is
     triple_information[ "filename" ] )
   */
  ...
  // QMap below - establish correspondence btw EditProfile filename &&
  // EditDataID select filename, editedDataID from editedData where
  // label='ISSF-KulkarniJ_NP1-pDNA-D2O-0_091020-run822-2A';
  ...
  for (fn = EProfs_to_IDs.begin(); fn != EProfs_to_IDs.end(); ++fn) {
    qDebug() << "EditProfile filename / EditDataID: " << fn.key() << "/"
             << fn.value();
    ...
    QString dirpath =
        US_Settings::resultDir() + "/" + triple_information["filename"];
    QString filepath = US_Settings::resultDir() + "/" +
                       triple_information["filename"] + "/" + filename;
...
bool US_Analysis_auto::file_loaded_auto(
    QMap<QString, QString>& triple_information) {
  qDebug() << "In file_loaded_auto: ";
  QString file_directory =
      US_Settings::resultDir() + QString("/") + triple_information["filename"];
  ...
  QStringList fm_files = directory.entryList(
      QStringList() << "2DSA-FM*" + triple_name_cut + "*.fitmen.dat",
      QDir::Files | QDir::NoSymLinks);

  qDebug() << "In file_loaded_auto: 22 -- triple: " << triple_name_cut;

  // ALEXEY: if there is no files (since no "-FM" models produced for what ever
  // reason), issue a warning:
  if (!fm_files.size()) {
    QMessageBox::warning(
        this, QString(tr("FM models problem: %1")).arg(triple_name_cut),
  ...
  QString fn = directory.absoluteFilePath(
      fm_files[0]);  // ALEXEY: *should be* the only one fitmen.dat file
  ...
  QString tripl =
      tripnode.left(1) + "." + tripnode.mid(1, 1) + "." + tripnode.mid(2);
  QStringList edtfilt;
  DbgLv(1) << "edtLabel" << edtLabel;
  if (!edtLabel.contains("DSA-F")) {  // fitmen file name has edit label
    edtfilt << runID + "." + edtLabel + ".*." + tripl + ".xml";
  } else {  // fitmen file name has NO edit label (older form)
  ...
  if (nedtfs == 1) {  // Choose the single corresponding edit
    fname_edit = edtfiles.at(0);
    // pb_update->setEnabled( true );
  }

  else if (nedtfs > 0) {  // Choose the latest edit
    int min_ms = 999999;
    QString fname = edtfiles.at(0);
    QString fpath = filedir + "/" + fname;
    QDateTime cdate = QFileInfo(QFile(fpath)).lastModified().toUTC();

    for (int jj = 0; jj < nedtfs; jj++) {
      fname = edtfiles.at(jj);
      fpath = filedir + "/" + fname;
      QDateTime fdate = QFileInfo(QFile(fpath)).lastModified().toUTC();
  ...
  else {  // Could not find edit file, so inform the user
    // pb_update->setEnabled( false );

    QMessageBox::warning(
        this, tr("Missing Local Edit"),
  ...
  // Load data && process data depending on if it's 2d (Fit Men. || Fit Bott.)
  // OR 3d (Fit Men. && Bott.)
...
void US_Analysis_auto::load_data_auto(const QString& text_content) {
  ...
  DbgLv(1) << "LD:  was3val have3val" << was3val << have3val << "v_rmsd size"
           << v_rmsd.size() << "parsed length" << parsed.length();
...
void US_Analysis_auto::process_2d(void) {
  ...
  if (!US_Matrix::lsfit(c, radius_values, rmsd_values, count, order + 1)) {
    QMessageBox::warning(this, tr("Data Problem"),
  ...
  for (int ii = 0; ii < v_meni.count(); ii++) {
    ...
    if (diff_x < diff_min) {  // Running least difference and index to it
      diff_min = diff_x;
      ix_best = ii;
    }
  }
...
void US_Analysis_auto::process_3d(void) {
  ...
  for (int ii = 0; ii < v_meni.size(); ii++) {
    ...
    if (rrmsd < min_z) {  // Save best-rmsd meniscus,bottom
      min_z = rrmsd;
      b_meni = rmeni;
      b_bott = rbott;
      min_ix = ii + 1;
  ...
  DbgLv(1) << "pl3d: v_meni size" << v_meni.size() << "min,max x,yz" << min_x
  ...
  for (int ii = 0; ii < ixs.count(); ii++) {
    ...
    w_rmsd += f_rmsd;
    DbgLv(1) << "pl3d:  ixs" << jx << "f_meni f_bott w_rmsd" << f_meni << f_bott
             << w_rmsd;
  }
  f_meni /= w_rmsd;
  f_bott /= w_rmsd;
...
int US_Analysis_auto::read_autoflowAnalysisStages(const QString& requestID) {
  ...
  // qDebug() << "BEFORE query ";
  QStringList qry;
  qry << "fitmen_autoflow_analysis_status" << requestID;
...
void US_Analysis_auto::revert_autoflow_analysis_stages_record(
  ...
  // qDebug() << "BEFORE query ";
  QStringList qry;
  qry << "fitmen_autoflow_analysis_status_revert" << requestID;
...
void US_Analysis_auto::edit_update_auto(
    QMap<QString, QString>& triple_information) {
  /***/
  // ALEXEY: if autoflow: check if edit profiles already updated from other
  // FITMEN session
  ...
  if (!status_fitmen_unique) {
    QMessageBox::information(
        this, tr("FITMEN | Triple Analysis already processed"),
        tr("It appears that FITMEN stage has already been OR being processed "
           "by "
           "a different user from different session. \n\n"
           "The program will return to the autoflow runs dialog where "
           "you can re-attach to the actual current stage of the run. "));
...
#define BOTT_LOWVAL 7.0
  ...
  qDebug() << "In FITMEN_AUTO's edit_update(): confirm, all_wvl, rmv_mdls, "
              "db_upd  ? "
           << confirm << all_wvl << rmv_mdls << db_upd;
  //*DEBUG*
  // db_upd=false;
  //*DEBUG*
  ...
  if (have3val) {  // Fit is meniscus + bottom
                   // mennew         = le_men_fit->text().toDouble();
                   // botnew         = le_bot_fit->text().toDouble();
    mennew = f_meni;
    botnew = f_bott;
  } else if (!bott_fit) {  // Fit is meniscus only
                           // mennew         = le_men_sel->text().toDouble();
    mennew = fit_xvl;
  } else {  // Fit is bottom only
  ...
  if (mennew > MENI_HIGHVAL) {
    ...
    if (botnew != 0.0 && botnew < BOTT_LOWVAL) {
      ...
      mmsg = tr("The currently selected Mensicus value") + " ( " + s_meni +
             " )\n" +
             tr("is unreasonable !\n\nIt is recommended"
                " that you \"Cancel\" the \"Update Edit\"\n"
    ...
    bad_vals = true;
    mhdr = QString(tr("Unreasonable Bottom: %1"))
               .arg(triple_information["triple_name"]);
    mmsg = tr("The currently selected Bottom value") + " ( " + s_bott + " )\n" +
           tr("is unreasonable !\n\nIt is recommended"
              " that you \"Cancel\" the \"Update Edit\"\n"
              "and retry after setting a reasonable value.");
  }
  ...
  if (!all_wvl || nedtfs == 1) {  // Apply to a single triple
    QTextStream ts(&filei);       // Build up XML Edit text
    while (!ts.atEnd()) edtext += ts.readLine() + "\n";
    filei.close();
    ...
    DbgLv(1) << " eupd:  mennew" << mennew << "lefval" << lefval << "botnew"
             << botnew;
    DbgLv(1) << " eupd:   ixmlin ixblin" << ixmlin << ixblin << "ncmlin ncblin"
             << ncmlin << ncblin;
    ...
    if (mennew >= lefval)  // ALEXEY: HERE!!!!
    {
      // cancel job && restart timer();
      fitmen_bad_vals = true;
      QString reason_for_failure =
          QString(
              "The selected Meniscus value, %1 , extends into the data range "
              "whose left-side value is %2")
              .arg(mennew)
              .arg(lefval);
    ...
    if (have3val) {  // Replace meniscus and bottom values in edit
      edtext.replace(ixmval, ncmval, s_meni);
      ncmval += demval;
      ncmlin += demval;
      if (ixbval > 0) {  // Replace existing bottom value
        ixbval += demval;
        edtext.replace(ixbval, ncbval, s_bott);
        ixblin += demval;
        ncbval += debval;
        ncblin += debval;
      } else {  // Must insert an entirely new line for bottom
        QString bline = QString("\n            <bottom radius=\"") + s_bott +
                        QString("\"/>");
    ...
    } else if (!bott_fit) {  // Replace meniscus value in edit
      edtext.replace(ixmval, ncmval, s_meni);
      ncmval += demval;
      ncmlin += demval;
    } else {             // Replace/insert bottom value in edit
      if (ixbval > 0) {  // Replace existing bottom value
        edtext.replace(ixbval, ncbval, s_bott);
        ncbval += debval;
        ncblin += debval;
  ...
  else {  // Apply to all wavelengths in a cell/channel

    // ALEXEY: Set progressDialog
    progress_msg_fmb = new QProgressDialog("Updating edit profiles...",
                                           QString(), 0, nedtfs, this);
    progress_msg_fmb->setWindowFlags(Qt::Window | Qt::WindowTitleHint |
                                     Qt::CustomizeWindowHint);
    progress_msg_fmb->setWindowModality(Qt::WindowModal);
    progress_msg_fmb->setWindowTitle(
        QString(tr("Updating Edit Profiles: %1"))
            .arg(triple_information["triple_name"]));
    ...
    for (int jj = 0; jj < nedtfs;
         jj++) {  // Modify each Edit file corresponding to a wavelength
      ...
      if (!filei.open(QIODevice::ReadOnly |
                      QIODevice::Text)) {  // Skip any file we cannot read
      ...
      DbgLv(1) << " eupd:       ixmlin ixblin ixllin" << ixmlin << ixblin
               << ixllin;
      ...
      if (have3val) {  // Replace meniscus and bottom values in edit
        ...
        ncmval += demval;
        ncmlin += demval;
        if (ixbval > 0) {  // Replace an existing bottom line
          ixbval += demval;
          edtext.replace(ixbval, ncbval, s_bott);
          ixblin += demval;
          ncbval += debval;
      ...
      } else {  // Replace bottom value in edit
        if (ixbval > 0) {
          edtext.replace(ixbval, ncbval, s_bott);
          ncbval += debval;
          ncblin += debval;
      ...
      if (!fileo.open(QIODevice::WriteOnly | QIODevice::Text)) continue;
      ...
      if (db_upd) {
        ...
        if (edtfiles[jj] == fname_edit) idEdsv = idEdit;
      }
  ...
  // Set progress to max
  if (progress_msg_fmb != NULL) {
    progress_msg_fmb->setValue(
        progress_msg_fmb->maximum());  // ALEXEY -- bug fixed..
...
void US_Analysis_auto::remove_models_auto(QString invID) {
  ...
  QStringList modfiles =
      QDir(moddir).entryList(modfilt, QDir::Files, QDir::Name);
  ...
  for (int ii = 0; ii < modfiles.size(); ii++) {
    ...
    if (ii < 3 || (ii + 4) > modfiles.size() || ii == (modfiles.size() / 2))
      DbgLv(1) << "RmvMod:  scn1 ii runID editLabl tripID" << ii << runID
               << editLabl << tripID;
    ...
    if (!iterID.contains("-m") && !iterID.contains("-b")) {
      DbgLv(1) << "RmvMod:   ii" << ii << "iterID" << iterID << "has -m"
               << iterID.contains("-m") << "has -b" << iterID.contains("-b");
      continue;  // Can't use if not a fit-meniscus or fit-bottom type
    }

    if (itNum == "01" &&
        msetBase.isEmpty()) {  // When fit iteration is "01" and not yet saved,
                               // save model set base
      msetBase =
          descript.section(".", 0, -3) + "." + anRunID.section("_", 0, 3);
    ...
    if (arTime > lArTime) {  // If first set or new one younger than previous,
                             // start lists
      lmodFnams.clear();
      lmodGUIDs.clear();
      lmodVaris.clear();
    ...
    else if (arTime < lArTime) {  // If new one older than previous, skip it
      continue;
    }
  ...
  for (int ii = 0; ii < nlmods;
       ii++) {  // Scan to identify model in set with iteration index
    ...
    QString iterID = ansysID.section('_', -1, -1);
    int iterx = iterID.section('-', 0, 0).mid(1).toInt() - 1;
  ...
  if (db_upd) {
    ...
    for (int ii = 0; ii < modIDs.size(); ii++) {
      ...
      if (!msetBase.isEmpty() && !descript.startsWith(msetBase)) {
        DbgLv(1) << "RmvMod:  descript" << descript << "msetBase" << msetBase
                 << "ii" << ii;
        continue;
      }
      ...
      if (arTime > dArTime) {  // If first set or new one younger than previous,
                               // start lists
        dmodIDs.clear();
        dmodGUIDs.clear();
        dmodVaris.clear();
      ...
      else if (arTime < dArTime) {  // If new one older than previous, skip it
        continue;
      }
    ...
    for (int ii = 0; ii < ndmods;
         ii++) {  // Scan to identify model in set with lowest variance
    ...
    if (nlmods == ndmods || (ndmods > 0 && nlmods == 0)) {
      ...
      if (nmatch == nlmods) {  // OK if they match or local only
        DbgLv(1) << "++local/dbase match, or local only";
      }

      else {  // Not good if they do not match
    ...
    else if (nlmods == 0) {  // It is OK if there are no local records, when DB
                             // ones were found
      DbgLv(1) << "++only dbase records exist";
    }

    else if (ndmods == 0) {  // It is OK if there are only local records, when
                             // local ones found
      DbgLv(1) << "++only local records exist";
    }

    else {  // Non-zero local & DB, but they do not match
  ...
  DbgLv(1) << "  nlmods ndmods" << nlmods << ndmods;
  if (ndmods > 0 || nlmods > 0) {  // There are models to scan, so build a list
                                   // of models,noises to remove
    ...
    for (int jj = 0; jj < ntmods;
         jj++) {  // Build the list of model files and DB ids for removal
      if (jj != ikModx) {
        int itix;
        int irix;
        QString fname;
        ...
        tiDesc = QString(mDesc)
                     .replace(".model", ".ti_noise")
                     .replace("2DSA-FM-IT", "2DSA-FM")
                     .replace("2DSA-FB-IT", "2DSA-FB");
        riDesc = QString(mDesc)
                     .replace(".model", ".ri_noise")
                     .replace("2DSA-FM-IT", "2DSA-FM")
                     .replace("2DSA-FB-IT", "2DSA-FB");
        itix = nieDescs.indexOf(tiDesc);
        irix = nieDescs.indexOf(riDesc);
        DbgLv(1) << "RmvMod:    itix irix" << itix << irix << "tiDesc" << tiDesc
                 << "riDesc" << riDesc;

        if (itix >= 0) {  // There is a TI noise to remove
          noiID = nieIDs[itix];
        ...
        if (irix >= 0) {  // There is an RI noise to remove
          noiID = nieIDs[irix];
    ...
    bool rmv_mdls = true;
    DbgLv(1) << "RmvMod: nlrmod ndrmod nlrnoi ndrnoi nlnois ndnois" << nlrmod
             << ndrmod << nlrnoi << ndrnoi << nlnois << ndnois;
    ...
    if (rmv_mdls) {
      ...
      for (int ii = 0; ii < rmodIDs.size();
           ii++) {  // Remove models and db noises
        recID = rmodIDs[ii];
        recDesc = rmodDescs[ii];
        recFname = rmodFnams[ii];
        DbgLv(1) << "  Delete: " << recID << recFname.section("/", -1, -1)
                 << recDesc;

        if (!recFname.isEmpty()) {  // Delete local file model
          QFile recf(recFname);
          if (recf.exists()) {
            if (recf.remove()) {
              DbgLv(1) << "     local file removed";
            } else {
              qDebug() << "*ERROR* removing" << recFname;
            }
          } else {
            qDebug() << "*ERROR* does not exist:" << recFname;
          }
        }

        if (recID != "-1" &&
            dbP != NULL) {  // Delete model (and any child noise) from DB
          query.clear();
      ...
      for (int ii = 0; ii < rnoiIDs.size(); ii++) {  // Remove local noises
        recID = rnoiIDs[ii];
        ...
        if (!recFname.isEmpty()) {  // Delete local file noise
          QFile recf(recFname);
          if (recf.exists()) {
            if (recf.remove()) {
              DbgLv(1) << "     local file removed";
...
void US_Analysis_auto::index_model_setfit() {
  if (have3val) {  // Fit Mensicus + Bottom:  find closest x,y in list
    ...
    for (int ii = 0; ii < v_meni.count();
         ii++) {  // Find lowest difference in X,Y list
      double xydiff = sq((xmeni - v_meni[ii])) + sq((ybott - v_bott[ii]));
      xydiff = (xydiff == 0.0) ? 0.0 : sqrt(xydiff);

      if (xydiff < lowxyd) {  // Save lowest difference so far and its index
        ix_setfit = ii;
        lowxyd = xydiff;
  ...
  else {  // Fit Meniscus OR Fit Bottom:  find close X in list
    ...
    for (int ii = 0; ii < v_meni.count();
         ii++) {  // Find lowest difference in X list
      double xdiff = qAbs((xselec - v_meni[ii]));

      if (xdiff < lowxd) {  // Save lowest difference so far and its index
        ix_setfit = ii;
        lowxd = xdiff;
        DbgLv(1) << "IMS:    ii" << ii << "lowxd" << lowxd << "v_meni-ii"
                 << v_meni[ii];
      }
    }
  }
...
void US_Analysis_auto::noises_in_edit(QString modDesc, QStringList& nieDescs,
                                      QStringList& nieIDs,
                                      QStringList& nieFnams, QString invID) {
  ...
  QString srchRun = msetBase.section(".", 0, -3) + "." +
                    msetBase.section(".", -1, -1).section("_", 0, 0).mid(1);
  ...
  QStringList noifiles =
      QDir(noidir).entryList(noifilt, QDir::Files, QDir::Name);
  noidir = noidir + "/";
  ...
  for (int ii = 0; ii < noifiles.size(); ii++) {
    ...
    if (noise.load(noiPath) != US_DB2::OK) continue;  // Can't use if can't load
  ...
  if (db_upd) {
    ...
    while (db.next()) {
      ...
      if (edtName.startsWith(srchRun) && edtName.endsWith(srchTrip)) {
        edtIDs << edtID;
        edtNams << edtName;
    ...
    while (db.next()) {
      ...
      if (edtIDs.contains(edtID)) nIDs << noiID;
    }
  ...
  DbgLv(1) << "NIE: db_upd" << db_upd << "nlnois" << nlnois << "nieDescs-size"
           << nieDescs.size() << "nieIDs-size" << nieIDs.size();

  return;
}

void US_Analysis_auto::proceed_abde_to_report(
