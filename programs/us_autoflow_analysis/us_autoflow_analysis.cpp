...
US_Analysis_auto::US_Analysis_auto () : US_Widgets ()
{
  ...
  // QLabel* lb_hdr1          = us_banner( tr( "Analysis Stages for All
  // Triples" ) );
  ...
  connect (this, SIGNAL (process_abde (QMap<QString, QString> &)),
           sdiag_norm_profile,
           SLOT (load_data_auto (QMap<QString, QString> &)));
  connect (sdiag_norm_profile,
           SIGNAL (abde_to_report (QMap<QString, QString> &)), this,
           SLOT (proceed_abde_to_report (QMap<QString, QString> &)));
  ...
  // "filename" ]     =
  // QString("AAV_GMP_test_030325-run2366-dataDiskRun-1515"); protocol_details[
  // "analysisIDs"  ] = QString(""); protocol_details[ "expType" ]      =
  // QString("ABDE"); protocol_details[ "dataSource" ]   =
  // QString("dataDiskAUC"); protocol_details[ "statusID" ]     =
  // QString("588"); protocol_details[ "autoflowID" ]   = QString("1515");

  // //VELOCITY: failed JSON (bad fitmen-auto, "NAN")
  // protocol_details[ "invID_passed" ] = QString("11");
  // protocol_details[ "protocolName" ] =
  // QString("MartinR_alexey-dev-issue425-SWL-RI-T3_21JUN25");
  // protocol_details[ "aprofileguid" ] =
  // QString("bc5641b1-27f2-4604-a3e2-5d24196fc9de"); protocol_details[
  // "filename" ]     =
...
void
US_Analysis_auto::initPanel (QMap<QString, QString> &protocol_details)
{
  // Clear TreeWidget
  treeWidget->clear ();
  ...
  if (autoflow_expType == "ABDE")
    {
      ...
      qDebug () << "[in autoflow_analysis: ] norm_abde_open? "
                << norm_abde_open;
      if (norm_abde_open)
        {
      ...
      if (abde_etype == "MWL")
        {
          ...
          // Pre-process data (decomposition for MWL); build alt. GUI (to
          // integrate and manually normalize);
          sdiag = new US_MwlSpeciesFit (protocol_details_at_analysis);
          // sdiag -> show(); //for debug

          qDebug () << "SSF-Dir: "
                    << protocol_details_at_analysis["ssf_dir_name"];
      ...
      if (!analysis_details.size ())
        {
          ...
          qDebug ()
              << "analysis_details.size() FROM autoflowAnalysisHistory -- "
  ...
  for (int i = 0; i < channels_all.size (); ++i)
    {
      for (ti = Array_of_analysis.begin (); ti != Array_of_analysis.end ();
           ++ti)
        {
          ...
          if (triple_name.contains (channels_all[i])
              && !triple_name.contains ("Interference"))
            {
              ...
              Channel_to_requestIDs[channels_all[i]]
                  << ana_details["requestID"];
            }
        }
    }
  ...
  for (jj = Array_of_analysis.begin (); jj != Array_of_analysis.end (); ++jj)
    {
      ...
      if (json.contains ("2DSA")) // ALEXEY: maybe more strict condition here?
                                  // (as 2DSA is in other stages...)
        job1run = true;
      ...
      qDebug () << "Triple,  width:  " << triple_curr << ", "
                << triple_name_width;
...
QString
US_Analysis_auto::get_filename (QString triple_name)
{
  ...
  for (jj = Array_of_analysis.begin (); jj != Array_of_analysis.end (); ++jj)
    {
      ...
      if (!current_analysis_details.size ())
        {
          ...
          qDebug () << "FORM HISTORY: set Completed_triples[ "
                    << triple_curr_key << " ] to "
      ...
      QString nextWaitStatusMsg
          = current_analysis_details["nextWaitStatusMsg"];
              ...
              if (to_process_stage_groupbox != NULL)
                {
                  ...
                  // owner
                  QString investigator_text
                      = investigator_details["email"] + " ("
                  ...
                  lineedit_status->setText (
                      "Waiting for prior stage(s) to complete");
      ...
      if (processed.isUndefined ())
      ...
      else
        {
          for (int i = 0; i < processed_array.size (); ++i)
            {
              ...
              QString stage_name, stage_gfacID, stage_status, stage_statusMsg,
                  stage_createTime, stage_updateTime,
                  stage_HPCAnalysisRequestID;

              foreach (const QString &key,
                       processed_array[i].toObject ().keys ())
                {
                  QJsonObject newObj
              ...
              if (processed_stage_groupbox != NULL)
                {
                  ...
                  // runID
                  QString runid_text
                      = filename + " ( " + triple_curr + " )  " + defaultDB
                  ...
                  lineedit_status->setStyleSheet (
                      "QLineEdit { background-color:  rgb( 2, 88, 57 ); color "
                      ": white; }");
                  ...
                  if (stage_status == "CANCELED" || stage_status == "canceled")
                    {
                      lineedit_status->setStyleSheet (
                          "QLineEdit { background-color:  rgb(210, 0, 0); "
                          "color : white; }");
                      topItem[triple_curr]->setForeground (0,
                                                           QBrush (colorRed));
                      ...
                      qDebug ()
                          << "CANCELED status( Processed ) for triple/stage "
                             "-- "
                          << triple_curr_key << "/" << submitted.toString ();
                    }
                  if (stage_status == "FAILED" || stage_status == "failed")
                    {
                      ...
                      qDebug ()
                          << "FAILED status( Processed ) for triple/stage -- "
                          << triple_curr_key << "/" << submitted.toString ();
                    }
                }
            }
          ...
          if (!to_process_array.size () && History_read[triple_curr_key]
            ...
            {
              qDebug () << "Setting triple to complete - read from History, "
                           "status = "
                        << status;
      ...
      else
        {
          ...
          if (submitted.toString () == "FITMEN_AUTO"
              && Process_2dsafm[triple_curr_key])
            {
              ...
              // nextWaitStatus="COMPLETE" ? (the submission daemon didn't yet
              // updated submitted status...) I.e., FITMEN_AUTO was processed
              // BUT submission daemon still sees it in 'status' "FITMEN_AUTO"
              // ?
              qDebug () << "FITMEN_AUTO: nextWaitStatus -- " << nextWaitStatus;
              if (nextWaitStatus == "COMPLETE" || nextWaitStatus == "complete"
                  || nextWaitStatus == "CANCELED"
                  || nextWaitStatus == "canceled")
                continue;

              //--- Check status of the FITMEN_AUTO | Entire Analysis for the
              //triple
              bool fitmen_auto_processed;
              fitmen_auto_processed
                  = check_fitmen_status (requestID, stage_name);
          ...
          if (submitted.toString () == "FITMEN"
              ...
              && Process_2dsafm[triple_curr_key])
            {
              ...
              //--- Check status of the FITMEN | Entire Analysis for the triple
              //before calling FitMen constructor:
              ...
              /** To FitMeniscus class -- pass:
                  1. Name of the triple to title && and possibly to pass to
              scan_db() method
              ...
              ****/
              ...
              /** The following will block parent windows from closing BUT not
               * from continuing timer execution ***/
              ...
              connect (
                  FitMen,
                  SIGNAL (editProfiles_updated (QMap<QString, QString> &)),
                  this,
                  SLOT (update_autoflowAnalysis_statuses (
              ...
              // connect( FitMen, SIGNAL( bad_meniscus_values( QMap < QString,
              // QString > & ) ),  //This will take care of BAD values 	       this,
              // SLOT( delete_jobs_at_fitmen( QMap < QString, QString > & ) )
              // );
              ...
              // Stop timer here, and restart with the editProfile_updated
              // signal ??
              timer_update->stop ();
              disconnect (timer_update, SIGNAL (timeout ()), 0, 0);
          ...
          if (current_stage_groupbox != NULL)
            {
              ...
              // QString triple_stage = triple_curr_key + ":" +
              // submitted.toString(); delete button: if running && admin_user
              // || user_owner, enable; otherwise, disable (should be disabled
              // earlier)
              ...
              if (status == "COMPLETE")
                {
                  ...
                  if (!to_process_array.size ())
                    {
                      topItem[triple_curr]->setForeground (
                          0, QBrush (colorGreen));
                      /****
                           ALEXEY: maybe update QMap < triple_curr , bool >
                      Completed_triples with TRUE (defined as all FALSES at the
                      beginning): Completed_triples[ triple_curr ] = true; At
                      the end of update cycle over triples, check if there is
                      at least one FALSE value in the  Completed_triple QMap.
                           If NO - stop timer, show Msg that everything
                      completed.
                      **/

                      /** Comment the line BELOW as this MAY cause RACE
                       * CONDITIONS !!!!!***************************/
                      // Completed_triples[ triple_curr_key ] = true;  // May
                      // cause race conditions!!!!
                      /*******************************************************************************************/

                      qDebug ()
                          << "set Completed_triples[ " << triple_curr_key
              ...
              if (status == "FAILED" || status == "failed"
                  || status == "CANCELED" || status == "canceled")
                {
                  ...
                  pb_delete->setEnabled (false);
                  lineedit_status->setStyleSheet (
                      "QLineEdit { background-color:  rgb(210, 0, 0); color : "
                      "white; }");
                  ...
                  if (status == "FAILED" || status == "failed")
                    {
                      Failed_triples[triple_curr_key] = true;
                      qDebug ()
                          << "FAILED status for triple/stage -- "
                          << triple_curr_key << "/" << submitted.toString ();
                    }
                  if (status == "CANCELED" || status == "canceled")
                    {
                      Canceled_triples[triple_curr_key] = true;
                      qDebug ()
                          << "CANCELED status for triple/stage -- "
                          << triple_curr_key << "/" << submitted.toString ();
                    }
                }
            }
  ...
  if (all_processed)
    {
      ...
      if (failed_triples)
        msg_text += QString ("\n\nNOTE: analyses for the following triples "
                             "FAILED: \n\n%1")
      ...
      if (failed_triples || canceled_triples)
        {
          QMessageBox::information (this, tr ("All Triples Processed !"),
                                    msg_text);
        }
...
void
US_Analysis_auto::update_autoflow_record_atAnalysis (void)
{
  // Check DB connection
  US_Passwd pw;
...
US_Analysis_auto::aa_resbmap ()
{
  return rbmapd;
}

QString
US_Analysis_auto::aa_tripleInfo ()
{
  return tripleInfo;
}
...
bool
US_Analysis_auto::loadData (QMap<QString, QString> &triple_information)
{
  rawData.clear ();
  editedData.clear ();
  ...
  while (db->next ())
    {
      ...
      // 3 ).toDateTime().toString( "yyyy/MM/dd HH:mm" ), true );
...
// Load rawData/editedData
bool
US_Analysis_auto::loadModel (QMap<QString, QString> &triple_information)
{
  US_Passwd pw;
  US_DB2 *db = new US_DB2 (pw.getPasswd ());
          ...
          if (triple_information["stage_name"] == "2DSA")
            {
              if (!description.contains ("-FM_")
                  && !description.contains ("-IT_")
                  && !description.contains ("-MC_")
...
// Load Noises
bool
US_Analysis_auto::loadNoises (QMap<QString, QString> &triple_information)
{
  US_Passwd pw;
  US_DB2 *db = new US_DB2 (pw.getPasswd ());
...
void
US_Analysis_auto::loadNoises_whenAbsent ()
{
...
int
US_Analysis_auto::count_noise_auto (US_DataIO::EditedData *edata,
...
int
US_Analysis_auto::noises_in_model_auto (QString mGUID, QStringList &nGUIDs)
{
  QString xnGUID;
  QString xmGUID;
  QString xntype;
...
int
US_Analysis_auto::models_in_edit_auto (QString eGUID, QStringList &mGUIDs)
{
  QString xmGUID;
  QString xeGUID;
  QString xrGUID;
...
int
US_Analysis_auto::id_list_db_auto (QString daEditGUID)
{
  QStringList query;
...
// Simulate Model
void
US_Analysis_auto::simulateModel ()
{
...
void
US_Analysis_auto::update_progress (int icomp)
{
...
void
US_Analysis_auto::calc_residuals ()
{
...
double
US_Analysis_auto::interp_sval (double xv, double *sx, double *sy, int ssize)
{
...
void
US_Analysis_auto::thread_progress (int thr, int icomp)
{
...
void
US_Analysis_auto::thread_complete (int thr)
{
  thrdone++;
...
void
US_Analysis_auto::show_overlay (QString triple_stage)
{
...
// modles
void
US_Analysis_auto::delete_jobs_at_fitmen (QMap<QString, QString> &triple_info)
{
  ...
  if (!fitmen_bad_vals)
    {
      ...
      msg_delete.setWindowFlags (Qt::CustomizeWindowHint
                                 | Qt::WindowTitleHint);
      msg_delete.setWindowTitle (tr ("Job Cancelation!"));

      QString msg_sys_text
          = QString (
                tr ("ATTENTION!\n\nNo -FM models for triple %1 have been "
                    "found. \n\n"
      ...
      if (msg_delete.clickedButton () == Accept_sys)
        {
          ...
          // qDebug() << "GUID to DETETE! -- " << current_analysis[
          // "CurrentGfacID" ]; qDebug() << "Status && statusMsg to DETETE! --
  ...
  else
    {
      ...
      QString msg_sys_text_bv
          = QString (
                tr ("ATTENTION!\n\n The meniscus fit for tiple %1 resulted in "
                    "a best-fit value that lies outside "
                    "of the considered range for the meniscus. "
                    "This should not happen, unless: \n\n"
                    "1. The initial meniscus was not selected correctly; \n"
                    "2. The experimental data are non-ideal and cannot be "
                    "modeled with the standard finite element model;\n"
                    "3. The radial calibration of the instrument is "
                    "incorrect. \n\n"
                    "The user should examine the data manually and determine "
                    "the cause for this discrepancy."
                    "The automatic GMP processing for this triple cannot "
                    "continue.\n"
                    "All scheduled jobs will be canceled for this triple."))
                .arg (triple_name);
...
// Slot to delete Job
void
US_Analysis_auto::delete_job (QString triple_stage)
{
  QString tr_st = triple_stage.simplified ();
  tr_st.replace (" ", "");
  ...
  if (Process_2dsafm[triple_n] && mwl_channel
    ...
    msg_sys_text += QString ("\n\nSince this is a multi-wavelength analysis, "
                             "ongoing jobs for the following same-channel "
                             "triples will be canceled as well: \n %1")
                        .arg (triple_list_affected.join (","));
  ...
  if (msg_delete.clickedButton () == Accept_sys)
    {
      ...
      bool ok;
      QString msg
          = QString (tr ("Put a comment describing reason for a STOP:"));
      QString default_text = QString (tr ("Reason for job CANCELLATION: "));
      QString comment_t
          = QInputDialog::getText (this, tr ("Reason for job CANCELLATION"),
      ...
      // Now, if 2DSA or 2DSA-FM of the representative wvl in the MWL case:
      // DELETE jobs for all other triples in a channel:
      ...
      record_or_update_analysis_cancel_status (&db, triple_n,
                                               other_chan_triples, comment_t);
    }
...
void
US_Analysis_auto::update_autoflowAnalysis_uponDeletion (
  ...
  if (db->lastErrno () != US_DB2::OK)
    {
      QMessageBox::warning (
          this, tr ("Connection Problem"),
          tr ("Updating autoflowAnalysis at DELETEe (primary wvl): Could not "
              "connect to database \n")
              + db->lastError ());
      return;
    }
...
// slot to update autoflowAnalysis records when DELETE pressed for all other
// wvl in a channel
void
US_Analysis_auto::update_autoflowAnalysis_uponDeletion_other_wvl (
    US_DB2 *db, const QStringList &requestID_list)
{
  if (db->lastErrno () != US_DB2::OK)
    {
      QMessageBox::warning (this, tr ("Connection Problem"),
                            tr ("Updating autoflowAnalysis at DELETE (other "
...
// updated triple
void
US_Analysis_auto::update_autoflowAnalysis_statuses (
    QMap<QString, QString> &triple_info)
{
  ...
  // Identify list of requestIDs needed to be updated at FITMEN (especcially
  // for MWL cases):
  ...
  **********************************************/
  ...
  record_or_update_analysis_meniscus_status (dbP, triple_curr_key,
                                             FMB_changed);
...
void
US_Analysis_auto::record_or_update_analysis_cancel_status (
    ...
    QString comment_t)
{
  ...
  if (db->lastErrno () != US_DB2::OK)
    {
      QMessageBox::warning (this, tr ("Connection Problem"),
                            tr ("Updating autoflowStatus's analysis at "
                                "FITMEN: Could not connect to database \n")
                                + db->lastError ());
      return;
    }
  ...
  if (autoflowStatusID)
    {
      ...
      qry << "update_autoflowStatusAnalysisCancel_record"
          ...
          << QString::number (autoflowID_passed) << CancelTriples
          << CancelAction;
...
// Keep track on meniscus (or bottom) deviations in FITMEN from the best fit:
void
US_Analysis_auto::record_or_update_analysis_meniscus_status (
    US_DB2 *db, QString triple_name, QString FMB_changed)
{
  if (FMB_changed.isEmpty ())
    {
      qDebug () << "FBM_changed EMPTY: It seems it's impossible to determine "
                   "if meniscus| bottom was modified from the best FIT...";
      return;
    }
  ...
  if (autoflowStatusID)
    {
      ...
      qry << "update_autoflowStatusAnalysisFitmen_record"
          ...
          // << analysisJson                            //ALEXEY: not needed
          // with new proc.
  ...
  else
    {
      QMessageBox::warning (this, tr ("AutoflowStatus Record Problem"),
                            tr ("autoflowStatus (Analysis {FMB}): There was a "
                                "problem with identifying a record in "
                                "autoflowStatus table for a given run! \n"));
...
void
US_Analysis_auto::editProfiles_updated_earlier (void)
{
  // Restart timer:
  connect (timer_update, SIGNAL (timeout ()), this, SLOT (gui_update ()));
  timer_update->start (5000);
...
// from different session
void
US_Analysis_auto::triple_analysis_processed (void)
{
...
void
US_Analysis_auto::reset_analysis_panel_public ()
{
...
bool
US_Analysis_auto::check_fitmen_status (const QString &requestID,
  ...
  if (!analysis_details.size ())
    {
      ...
      QMessageBox::information (
          this,
          QString (tr ("%1 | Triple Analysis already processed"))
              .arg (stageName),
          QString (
  ...
  else
    {
      ...
      qDebug () << "In check_fitmen 3: -- " << submitted.toString ()
                << stageName;
          ...
          if (status_gen != "WAIT")
            {
              ...
              QMessageBox::information (
                  ...
                  QString (tr ("It appears that %1 stage has already been "
                               "processed by "
                               "a different user from different session.\n\n"
                               "The program will return to ANALYSIS tab where "
                               "you can continue to monitor the overall "
                               "analysis progress."))
                      .arg (stageName));
...
// reset Analysis GUI: stopping all update processes
void
US_Analysis_auto::reset_analysis_panel ()
{
  if (autoflow_expType != "ABDE" || autoflow_expType == "VELOCITY")
    {
      ...
      // shot...
      connect (timer_end_process, SIGNAL (timeout ()), this,
               SLOT (end_process ()));
...
// create groupBox
QGroupBox *
US_Analysis_auto::createGroup (QString &triple_name)
{
  ...
  connect (pb_overlay, SIGNAL (clicked ()), signalMapper_overlay,
           SLOT (map ()));
...
QMap<QString, QString>
US_Analysis_auto::read_autoflowAnalysis_record (US_DB2 *db,
...
QMap<QString, QString>
US_Analysis_auto::read_autoflowAnalysisHistory_record (
    US_DB2 *db, const QString &requestID)
{
  QMap<QString, QString> analysis_details;
...
QMap<QString, QString>
US_Analysis_auto::get_abdeAnalysis_info (US_DB2 *db, const QString &ID)
{
...
QMap<QString, QString>
US_Analysis_auto::get_investigator_info (US_DB2 *db, const QString &ID)
{
...
void
US_Analysis_auto::update_autoflowAnalysis_status_at_fitmen (
    US_DB2 *db, const QStringList &requestID_list)
{
  // // Check DB connection
  // US_Passwd pw;
  ...
  if (db->lastErrno () != US_DB2::OK)
    {
      QMessageBox::warning (this, tr ("Connection Problem"),
                            tr ("Updating autoflowAnalysis at FITMEN: Could "
...
void
US_Analysis_auto::gui_update_temp ()
{
  ...
  for (jj = Array_of_analysis.begin (); jj != Array_of_analysis.end (); ++jj)
    {
      ...
      lineedit_2dsa_it_status->setText (
          "Waiting for 2DSA-FM stage to complete");
      ...
      lineedit_2dsa_mc_status->setText (
          "Waiting for 2DSA-IT stage to complete");
...
void
US_Analysis_auto::scan_dbase_auto (QMap<QString, QString> &triple_information)
{
  ...
  QRegExp fmIter
      = QRegExp ("i\?\?-[mb]*", Qt::CaseSensitive, QRegExp::Wildcard);
  ...
  while (db.next ())
    {
      ...
      if (descript.length () == 80)
        { // Truncated description:  save ID and skip update for now
          DbgLv (1) << "DbSc:     TRUNC: modelID" << modelID << "descr"
                    << descript;
          tmodels << modelID;
          tedGIDs << editGUID;
          tedIDs << editID;
      ...
      DbgLv (1) << "DbSc:   modelID vari meni" << modelID << variance
                << meniscus << "ansysID" << ansysID << "iterID" << iterID;
      ...
      if (ansysID.contains ("2DSA-F") || iterID.contains (fmIter))
        { // Model from meniscus fit, so save information
          ...
          DbgLv (1) << "DbSc:       anType" << anType << "editLabel"
                    << editLabel << "ansysID" << ansysID;
          ...
          mdescr.antime
              = descript.section ('.', -2, -2).section ('_', 1, 1).mid (1);
      ...
      if (ftfile.exists ())
        { // File exists, so we must check the need to replace it
          ...
          DbgLv (1) << " ii rdate fdate" << ii << rdate << fdate
                    << "   ftfname" << ftfname << "  fdate.msecsTo(rdate)"
                    << fdate.msecsTo (rdate);
          ...
          if (fdate.msecsTo (rdate) > 0)
            ...
            { // DB record date is not later than file date, so leave file as
              // is
              nfexss++;
              continue;
            }
...
void
US_Analysis_auto::get_editProfile_copy (
    QMap<QString, QString> &triple_information)
{
  ...
  /* ALEXEY: we can use only triple_information[ "filename" ] (the editedData
     table's 'label' filed) to extract all editProfiles for this runID (which
     is triple_information[ "filename" ] )
   */
...
bool
US_Analysis_auto::file_loaded_auto (QMap<QString, QString> &triple_information)
{
  qDebug () << "In file_loaded_auto: ";
  QString file_directory = US_Settings::resultDir () + QString ("/")
      ...
      for (int jj = 0; jj < nedtfs; jj++)
        {
          ...
          DbgLv (1) << "     jj" << jj << "fdate" << fdate << "file_ms"
                    << file_ms;
          if (file_ms < min_ms)
            {
              min_ms = file_ms;
...
void
US_Analysis_auto::load_data_auto (const QString &text_content)
{
  int count = 0;
...
// process 2d data
void
US_Analysis_auto::process_2d (void)
{
  ...
  // // Add the marker label -- bold, font size default + 1, lines 3 pixels
  // wide QPen markerPen( QBrush( Qt::white ), 3.0 ); markerPen.setWidth( 3 );
...
// process 3d data
void
US_Analysis_auto::process_3d (void)
{
  bool auto_lim = false;
      ...
      for (int jbx = ix_bot - 1; jbx < ix_bot + 2; jbx++)
        {
          ...
          DbgLv (1) << "pl3d:   jmx jbx ixs" << jmx << jbx
                    << (jmx * nbott + jbx);
        }
    }
  ...
  for (int ii = 0; ii < ixs.count (); ii++)
    {
      ...
      w_rmsd += f_rmsd;
      DbgLv (1) << "pl3d:  ixs" << jx << "f_meni f_bott w_rmsd" << f_meni
                << f_bott << w_rmsd;
    }
  f_meni /= w_rmsd;
  f_bott /= w_rmsd;
...
int
US_Analysis_auto::read_autoflowAnalysisStages (const QString &requestID)
{
  int status = 0;
...
void
US_Analysis_auto::revert_autoflow_analysis_stages_record (
    const QString &requestID)
{
  // Check DB connection
  US_Passwd pw;
...
void
US_Analysis_auto::edit_update_auto (QMap<QString, QString> &triple_information)
{
  ...
  if (!all_wvl || nedtfs == 1)
    {                          // Apply to a single triple
      ...
      DbgLv (1) << " eupd:  mennew" << mennew << "lefval" << lefval << "botnew"
                << botnew;
      DbgLv (1) << " eupd:   ixmlin ixblin" << ixmlin << ixblin
                << "ncmlin ncblin" << ncmlin << ncblin;
      ...
      if (mennew >= lefval) // ALEXEY: HERE!!!!
        {
          ...
          QString reason_for_failure
              = QString ("The selected Meniscus value, %1 , extends into the "
                         "data range whose left-side value is %2")
                    .arg (mennew)
                    .arg (lefval);
    ...
    { // Apply to all wavelengths in a cell/channel
      ...
      for (int jj = 0; jj < nedtfs; jj++)
        { // Modify each Edit file corresponding to a wavelength
          ...
          if (mennew >= lefval) // ALEXEY: HERE !!!
            {
              ...
              fitmen_bad_vals = true;
              QString reason_for_failure
                  = QString ("The selected Meniscus value, %1 , extends into "
                             "the data range whose left-side value is %2")
                        .arg (mennew)
                        .arg (lefval);
          ...
          demval
              = s_meni.length () - ncmval; // Deltas in old,new value strings
          debval = s_bott.length () - ncbval;
...
void
US_Analysis_auto::update_db_edit (QString edtext, QString efilepath)
{
...
void
US_Analysis_auto::remove_models_auto (QString invID)
{
      ...
      for (int ii = 0; ii < modIDs.size (); ii++)
        {
          ...
          if (!msetBase.isEmpty () && !descript.startsWith (msetBase))
            {
              DbgLv (1) << "RmvMod:  descript" << descript << "msetBase"
                        << msetBase << "ii" << ii;
              continue;
            }
      ...
      if (nlmods == ndmods || (ndmods > 0 && nlmods == 0))
        {
          ...
          if (nmatch == nlmods)
            ...
            { // Not good if they do not match
              ...
              DbgLv (1) << "  nmatch ndmods nlmods" << nmatch << ndmods
                        << nlmods;
              return;
            }
        }
          ...
          if (jj != ikModx)
            {
              ...
              rmodFnams << fname;
              rmodDescs << mDesc;
              // DbgLv(1) << "RmvMod: jj" << jj << "mID" << mID << "mDesc" <<
              // mDesc;
              ...
              DbgLv (1) << "RmvMod:    itix irix" << itix << irix << "tiDesc"
                        << tiDesc << "riDesc" << riDesc;
      ...
      if (rmv_mdls)
        {
          ...
          for (int ii = 0; ii < rmodIDs.size (); ii++)
            { // Remove models and db noises
              ...
              DbgLv (1) << "  Delete: " << recID
                        << recFname.section ("/", -1, -1) << recDesc;
...
void
US_Analysis_auto::index_model_setfit ()
{
  if (have3val)
    { // Fit Mensicus + Bottom:  find closest x,y in list
      ...
      for (int ii = 0; ii < v_meni.count (); ii++)
        { // Find lowest difference in X,Y list
          double xydiff
              = sq ((xmeni - v_meni[ii])) + sq ((ybott - v_bott[ii]));
    ...
    { // Fit Meniscus OR Fit Bottom:  find close X in list
      ...
      DbgLv (1) << "IMS:  xselec" << xselec << "v_meni count"
                << v_meni.count ();
...
void
US_Analysis_auto::noises_in_edit (QString modDesc, QStringList &nieDescs,
  ...
  if (db_upd)
    {
      ...
      if (edtIDs.size () > 0)
        DbgLv (1) << "NIE: edtName0" << edtNams[0];
...
void
US_Analysis_auto::proceed_abde_to_report (
...
void
US_Analysis_auto::back_to_initAutoflow (void)
{
  emit analysis_back_to_initAutoflow ();
}
