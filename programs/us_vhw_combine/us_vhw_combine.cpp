...
US_vHW_Combine::US_vHW_Combine() : US_Widgets() {
  ...
  connect(lw_triples, SIGNAL(currentRowChanged(int)), this,
          SLOT(triple_select(int)));

  QBoxLayout* plot =
      new US_Plot(data_plot1, tr("G(s) Distributions"),
  ...
  qtitle.setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(),
                       QFont::Bold));
...
void US_vHW_Combine::load(void) {
  ...
  if (dkdb_cntrls->db()) {  // Plot reports are in the database
      ...
      for (int jj = 0; jj < ntripl; jj++) {
        ...
        for (int kk = 0; kk < ndocs; kk++) {
          ...
          if (fname.contains("s-c-distrib.csv") && edIDdoc > edIDdis) {
            ...
            DbgLv(1) << "      dpath" << dpath;
            int stat = db.readBlobFromDB(
                dpath, QString("download_reportContents"), ddoc->documentID);
            DbgLv(1) << "        readBlob stat" << stat << " edIDdis"
                     << edIDdis;
            ...
            DbgLv(1) << "      epath" << epath;
            int stat = db.readBlobFromDB(
                epath, QString("download_reportContents"), edoc->documentID);
            DbgLv(1) << "        readBlob stat" << stat << " edIDenv"
                     << edIDenv;
        ...
        if (havedis) {
          ...
          ddesc.runID = runid;
          ddesc.triple = trname;
          DbgLv(1) << " havedis run trip" << runid << tripl << "haveenv"
                   << haveenv;
          DbgLv(1) << "  dpath" << dpath;
          DbgLv(1) << "  epath" << epath;
          QFile fid(dpath);
    ...
    for (int ii = 0; ii < nruns; ii++) {
      ...
      QStringList dfiles =
          QDir(rundir).entryList(datfilt, QDir::Files, QDir::Name);
...
void US_vHW_Combine::plot_distr(DistrDesc ddesc, QString distrID) {
  ...
  if (dplot && dconc) {  // Build curves for integration/concentration plot
    ...
    for (int ii = 0; ii < ndispt; ii++) dconcs << (yy[ii] * fscl);
...
void US_vHW_Combine::save(void) {
  ...
  } else if (diplot && !edplot &&
             !dconc) {  // Distribution (Integral) only and Y=boundary
    fnamsvg = fnamsvg.replace("combo-distrib", "combdistr-integ");
    fnampng = fnampng.replace("combo-distrib", "combdistr-integ");
  } else if (diplot && !edplot &&
             dconc) {  // Distribution (Integral) only and Y=concentration
  ...
  while (iruns < nruns) {
    ...
    if (dkdb_cntrls->db()) {
      ...
      QString trip1 =
          US_Util::expanded_triple(trfirst, false).replace('/', '.');
...
void US_vHW_Combine::runid_select(int row) {
  ...
  for (int ii = 0; ii < distros.size(); ii++) {
    ...
    if (distros[ii].runID == runID) {
      ...
      QString distrID =
          ddesc.runID + " (" + ddesc.triple + ") : " + ddesc.tdescr;
...
void US_vHW_Combine::triple_select(int row) {
  ...
  for (int ii = 0; ii < distros.size(); ii++) {
    ...
    if (ddesc.runID == runID && ddesc.triple == triple) break;
  }
...
void US_vHW_Combine::setDSymbol(DistrDesc& ddesc, int distx) {
  ...
  while (js >= nsymbs) js -= nsymbs;

  while (jc >= ncolors) jc -= ncolors;
...
void US_vHW_Combine::possibleSymbols() {
  if (symbols.size() > 0 && colors.size() > 0) return;
...
QString US_vHW_Combine::expandedTriple(QString ctriple) {
  ...
  if (!etriple.contains(" / "))
    etriple =
        etriple.left(1) + " / " + etriple.mid(1, 1) + " / " + etriple.mid(2);
...
int US_vHW_Combine::envel_data(DistrDesc& ddesc) {
  ...
  for (int jj = 0; jj < ndpts;
       jj++) {  // get min,max intercept sedimentation coefficients
  ...
  if (nepts <=
      steps) {  // insure envelope array size bigger than histogram array size
    nepts = steps + 1;
  }
  ...
  for (int jj = 0; jj < steps;
       jj++) {  // calculate histogram values and envelope values based on them
    int kbin = 0;
    sed_lo = sed_bin * (double)jj;
    sed_hi = sed_lo + sed_bin;
    sval = (sed_lo + sed_hi) * 0.5;

    for (int kk = 0; kk < ndpts;
         kk++) {  // count sedcoeffs within current step range
      sedc = ddesc.dsedcs.at(kk);

      if (sedc >= sed_lo && sedc < sed_hi) kbin++;
    }
  ...
  for (int ii = fnz; ii < lnz;
       ii++) {  // Reduce array to Y's from first to last non-zero
    xval[nepts] = xval[ii];
    yval[nepts] = yval[ii];
    nepts++;
  }
...
void US_vHW_Combine::fill_in_desc(QTextStream& tsd, QTextStream& tse,
  ...
  if (haveenv) {  // Read in envelope data
    ...
    for (int jj = fnz; jj < lnz;
         jj++) {  // Reduce array to Y's from first to last non-zero
    ...
    for (int jj = 0; jj < nepts; jj++) ddesc.efreqs[jj] *= sclf;
...
void US_vHW_Combine::write_data(QString& dataFile, QString& listFile,
  ...
  for (int ii = 0; ii < nplots;
       ii++) {  // Accumulate long descriptions and build header line
  ...
  for (int jj = 0; jj < maxnvl;
       jj++) {  // Build and write svalue+boundary data line
    line = "";
    for (int ii = 0; ii < nplots; ii++) {  // Add each X,Y data pair
      ...
      QString dat = QString().sprintf("\"%12.5f\",\"%10.5f\",\"%12.4e\"", sval,
                                      boun, conc);
      dat.replace(" ", "");
      line += dat;
...
void US_vHW_Combine::plot_3d(void) {
  ...
  //                            : tr( "B.Frac." );
  QString zatitle =
      eplot ? tr("Concen.") : (icflag ? tr("BF*Conc.") : tr("Concen."));
  double xmin = 1e99;
  double ymin = 1e99;
  double zmin = 1e99;
  double xmax = -xmin;
  ...
  for (int ii = 0; ii < ndist; ii++) {
    ...
    for (int jj = 0; jj < ndpts; jj++) {
      ...
      DbgLv(0) << "Raw:xyzd: ii jj" << ii << jj << "xyz" << xval << yval
               << zval;
    }
  ...
  if (minpt !=
      maxpt) {  // Sed.Coeff. counts per wavelength vary:  create constant count
                //      nrow         = minpt;
    ...
    for (int ii = 0; ii < nrow; ii++, xval += xinc) xvals << xval;
    ...
    for (int ii = 0; ii < ncol; ii++) {
      ...
      DbgLv(0) << "     jrow" << jrow << "nrow" << nrow << "xv0,xvn" << xvsic[0]
               << xvsic[jrow - 1];
