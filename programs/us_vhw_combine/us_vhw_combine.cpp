...
US_vHW_Combine::US_vHW_Combine() : US_Widgets() {
   ...
   QLabel *lb_distrtype = us_banner(tr("Select Distribution Plot Type(s):"));
   ...
   QLayout *lo_intconc = us_checkbox(tr("Use Concentration"), ck_intconc, false);
   ...
   connect(ck_distrib, SIGNAL(stateChanged(int)), this, SLOT(plot_data()));
   connect(ck_envelope, SIGNAL(stateChanged(int)), this, SLOT(plot_data()));
   connect(ck_intconc, SIGNAL(stateChanged(int)), this, SLOT(plot_data()));

   connect(lw_runids, SIGNAL(currentRowChanged(int)), this, SLOT(runid_select(int)));
   connect(lw_triples, SIGNAL(currentRowChanged(int)), this, SLOT(triple_select(int)));

   QBoxLayout *plot = new US_Plot(
      data_plot1, tr("G(s) Distributions"), tr("Sedimentation Coefficient x 1e+13 (corr. for 20,W)"),
      tr("Boundary Fraction (%)"));

   QString etitle = tr("Signal Concentration");
   QwtText qtitle(etitle);
   qtitle.setFont(QFont(US_GuiSettings::fontFamily(), US_GuiSettings::fontSize(), QFont::Bold));
...
void US_vHW_Combine::load(void) {
   ...
   if (dkdb_cntrls->db()) { // Plot reports are in the database
      US_Passwd pw;
      ...
      for (int ii = 0; ii < nruns; ii++) {
         ...
         for (int jj = 0; jj < ntripl; jj++) {
            ...
            le_runid->setText(tr("loading triple %1 of %2 from run %3").arg(jj + 1).arg(ntripl).arg(runid));
            qApp->processEvents();
            bool havedis = false;
            bool haveenv = false;
            int edIDdis = 0;
            ...
            for (int kk = 0; kk < ndocs; kk++) {
               ...
               if (fname.contains("s-c-distrib.csv") && edIDdoc > edIDdis) {
                  edIDdis = edIDdoc;
                  ddoc = doc;
                  dpath = tmpdir + "/" + runid + "." + fname;
                  DbgLv(1) << "      dpath" << dpath;
                  int stat = db.readBlobFromDB(dpath, QString("download_reportContents"), ddoc->documentID);
               ...
               else if (fname.contains("s-c-envelope.csv") && edIDdoc > edIDenv) {
                  edIDenv = edIDdoc;
                  edoc = doc;
                  epath = tmpdir + "/" + runid + "." + fname;
                  DbgLv(1) << "      epath" << epath;
                  int stat = db.readBlobFromDB(epath, QString("download_reportContents"), edoc->documentID);
            ...
            if (havedis) {
               ...
               if (fid.open(QIODevice::ReadOnly | QIODevice::Text)) {
                  ...
                  QTextStream tse;
                  QFile fie(epath);
                  if (haveenv && fie.open(QIODevice::ReadOnly | QIODevice::Text))
                     tse.setDevice(&fie);
                  else
                     haveenv = false;
   ...
   else { // Plot reports are on local disk
      ...
      for (int ii = 0; ii < nruns; ii++) {
         ...
         QStringList dfiles = QDir(rundir).entryList(datfilt, QDir::Files, QDir::Name);
         ...
         for (int ii = 0; ii < ndfile; ii++) {
            ...
            QString epath = QString(fpath).replace("distrib.csv", "envelope.csv");
...
void US_vHW_Combine::plot_distr(DistrDesc ddesc, QString distrID) {
   ...
   if (dplot && !dconc) { // Build curves for distribution plot
   ...
   if (dplot && dconc) { // Build curves for integration/concentration plot
      dconcs.clear();
   ...
   if (eplot) { // Build curve for envelope plot
...
void US_vHW_Combine::save(void) {
   ...
   while (iruns < nruns) {
      ...
      if (dkdb_cntrls->db()) {
         ...
         QString trip1 = US_Util::expanded_triple(trfirst, false).replace('/', '.');
...
void US_vHW_Combine::triple_select(int row) {
   ...
   triple = item->text().section(":", 0, 0).simplified().section(" ", -1, -1).section("(", 1, 1).section(")", 0, 0);
...
QString US_vHW_Combine::expandedTriple(QString ctriple) {
   ...
   if (!etriple.contains(" / "))
      etriple = etriple.left(1) + " / " + etriple.mid(1, 1) + " / " + etriple.mid(2);
...
int US_vHW_Combine::envel_data(DistrDesc &ddesc) {
   ...
   for (int jj = 0; jj < ndpts; jj++) { // get min,max intercept sedimentation coefficients
   ...
   if (nepts <= steps) { // insure envelope array size bigger than histogram array size
      nepts = steps + 1;
   }
   ...
   for (int jj = 0; jj < nepts; jj++) { // initialize envelope values
   ...
   for (int jj = 0; jj < steps; jj++) { // calculate histogram values and envelope values based on them
      int kbin = 0;
      sed_lo = sed_bin * ( double ) jj;
      sed_hi = sed_lo + sed_bin;
      sval = (sed_lo + sed_hi) * 0.5;

      for (int kk = 0; kk < ndpts; kk++) { // count sedcoeffs within current step range
      ...
      if (kbin > 0) { // if non-empty bin, update envelope Y values
         for (int kk = 0; kk < nepts; kk++) {
            double xdif = (xval[ kk ] - sval) / sigma;
            yval[ kk ] += ((bink / (sigma * pisqr)) * exp(-(xdif * xdif) / 2.0));
         }
      }
   }

   for (int kk = 0; kk < nepts; kk++) { // accumulate the envelope values sum
      env_sum += yval[ kk ];
   }
   ...
   for (int kk = 0; kk < nepts; kk++) { // normalize Y values
      yval[ kk ] *= scale;
   ...
   for (int ii = fnz; ii < lnz; ii++) { // Reduce array to Y's from first to last non-zero
      xval[ nepts ] = xval[ ii ];
      yval[ nepts ] = yval[ ii ];
      nepts++;
   }
...
void US_vHW_Combine::fill_in_desc(QTextStream &tsd, QTextStream &tse, DistrDesc &ddesc, bool haveenv, int distx) {
   ...
   if (haveenv) { // Read in envelope data
      fline = tse.readLine();
      int fnz = -1;
      int lnz = 0;
      bool fev = true;
      ...
      while (!tse.atEnd()) {
         ...
         if (fev) {
            ...
            if (conc == 0.0) {
               QMessageBox::warning(
                  this, tr("No Total Concentration"),
      ...
      for (int jj = fnz; jj < lnz; jj++) { // Reduce array to Y's from first to last non-zero
   ...
   else { // Calculate envelope data from distribution data
      envel_data(ddesc);
   }
}

// Write data and list report files
void US_vHW_Combine::write_data(QString &dataFile, QString &listFile, int &irun) {
   ...
   for (int ii = 0; ii < nplots; ii++) { // Accumulate long descriptions and build header line
   ...
   for (int jj = 0; jj < maxnvl; jj++) { // Build and write svalue+boundary data line
      line = "";
      for (int ii = 0; ii < nplots; ii++) { // Add each X,Y data pair
         ...
         QString dat = QString().sprintf("\"%12.5f\",\"%10.5f\",\"%12.4e\"", sval, boun, conc);
         dat.replace(" ", "");
         line += dat;
...
void US_vHW_Combine::write_denv(QString &denvFile, int &irun) {
   if (irun > 0) { // After first/only time:  just make a copy of the file
...
int US_vHW_Combine::reportDocsFromFiles(
...
void US_vHW_Combine::plot_3d(void) {
   ...
   QString ptitle = eplot ? tr("g(s) Distributions") : tr("G(s) Distributions");

   QString xatitle = tr("Sed.C.(*e13)");
   QString yatitle = tr("Lambda(nm)");
   //   QString zatitle  = eplot ? tr( "Concen." )
   //                            : tr( "B.Frac." );
   QString zatitle = eplot ? tr("Concen.") : (icflag ? tr("BF*Conc.") : tr("Concen."));
   double xmin = 1e99;
   double ymin = 1e99;
   double zmin = 1e99;
   double xmax = -xmin;
   ...
   DbgLv(0) << "   xmin xmax" << xmin << xmax << "ymin ymax" << ymin << ymax << "zmin zmax" << zmin << zmax
            << "xyzd size" << xyzdat.size();

   if (minpt != maxpt) { // Sed.Coeff. counts per wavelength vary:  create constant count
      //      nrow         = minpt;
      ...
      for (int ii = 0; ii < ncol; ii++) {
         ...
         DbgLv(0) << "     jrow" << jrow << "nrow" << nrow << "xv0,xvn" << xvsic[ 0 ] << xvsic[ jrow - 1 ];
   ...
   if (p3d_ctld == NULL) {
      ...
      connect(p3d_ctld, SIGNAL(has_closed()), this, SLOT(control_closed()));
