...
US_vHW_Combine::US_vHW_Combine () : US_Widgets ()
{
  ...
  QLayout *lo_intconc
      = us_checkbox (tr ("Use Concentration"), ck_intconc, false);
  ...
  connect (ck_envelope, SIGNAL (stateChanged (int)), this,
           SLOT (plot_data ()));
      ...
      for (int ii = 0; ii < nruns; ii++)
        {
          ...
          DbgLv (1) << " ii,runid,ntrip,dists" << ii << runid << ntripl
                    << distx;
              ...
              for (int kk = 0; kk < ndocs; kk++)
                {
                  ...
                  DbgLv (1)
                      << "     kk,fname" << kk << fname << "edID" << edIDdoc;
                  ...
                  if (fname.contains ("s-c-distrib.csv") && edIDdoc > edIDdis)
                    {
                      ...
                      int stat = db.readBlobFromDB (
                          dpath, QString ("download_reportContents"),
                          ddoc->documentID);
                           ...
                           && edIDdoc > edIDenv)
                    {
                      ...
                      DbgLv (1) << "      epath" << epath;
                      int stat = db.readBlobFromDB (
                          epath, QString ("download_reportContents"),
                          edoc->documentID);
                      DbgLv (1) << "        readBlob stat" << stat
                                << " edIDenv" << edIDenv;
              ...
              if (havedis)
                {
                  ...
                  ddesc.runID = runid;
                  ddesc.triple = trname;
                  DbgLv (1) << " havedis run trip" << runid << tripl
                            << "haveenv" << haveenv;
                  DbgLv (1) << "  dpath" << dpath;
                  DbgLv (1) << "  epath" << epath;
                  QFile fid (dpath);
          ...
          for (int ii = 0; ii < ndfile; ii++)
            {
              ...
              QString epath
                  = QString (fpath).replace ("distrib.csv", "envelope.csv");
...
void
US_vHW_Combine::reset_data (void)
{
  distros.clear ();
  distIDs.clear ();
...
void
US_vHW_Combine::reset_plot (void)
{
...
void
US_vHW_Combine::plot_distr (DistrDesc ddesc, QString distrID)
{
...
// RunID selected
void
US_vHW_Combine::runid_select (int row)
{
...
// Triple selected
void
US_vHW_Combine::triple_select (int row)
{
...
void
US_vHW_Combine::setDSymbol (DistrDesc &ddesc, int distx)
{
...
void
US_vHW_Combine::possibleSymbols ()
{
...
QString
US_vHW_Combine::expandedTriple (QString ctriple)
{
  QString etriple = ctriple;

  if (!etriple.contains (" / "))
    etriple = etriple.left (1) + " / " + etriple.mid (1, 1) + " / "
              + etriple.mid (2);
...
QString
US_vHW_Combine::collapsedTriple (QString etriple)
{
...
int
US_vHW_Combine::envel_data (DistrDesc &ddesc)
{
  int nSensit = 50;
  int nSmooth = 30;
  int steps;
          ...
          for (int kk = 0; kk < nepts; kk++)
            {
              ...
              yval[kk]
                  += ((bink / (sigma * pisqr)) * exp (-(xdif * xdif) / 2.0));
            }
        }
    }
...
void
US_vHW_Combine::update_disk_db (bool isDB)
{
...
void
US_vHW_Combine::fill_in_desc (QTextStream &tsd, QTextStream &tse,
                              DistrDesc &ddesc, bool haveenv, int distx)
{
  ...
  DbgLv (1) << "Distro runid" << ddesc.runID << " triple" << ddesc.triple
            << kk;
...
void
US_vHW_Combine::write_data (QString &dataFile, QString &listFile, int &irun)
{
  if (irun > 0)
...
void
US_vHW_Combine::write_denv (QString &denvFile, int &irun)
{
  if (irun > 0)
    ...
    { // Build and write svalue+boundary data line
      ...
      for (int ii = 0; ii < nplots; ii++)
        { // Add each X,Y data pair
          ...
          QString dat
              = QString ().sprintf ("\"%12.5f\",\"%10.5f\"", sval, eval);
          dat.replace (" ", "");
          line += dat;
...
int
US_vHW_Combine::reportDocsFromFiles (QString &runID, QString &fdir,
...
US_vHW_Combine::plot_3d (void)
{
  ...
  QString ptitle
      = eplot ? tr ("g(s) Distributions") : tr ("G(s) Distributions");
      ...
      for (int ii = 0; ii < ncol; ii++)
        {
          ...
          DbgLv (0) << "     jrow" << jrow << "nrow" << nrow << "xv0,xvn"
                    << xvsic[0] << xvsic[jrow - 1];
      ...
      if (p3d_pltw != NULL)
        {
          ...
          DbgLv (0) << "p3db:N: titles" << ptitle << xatitle << yatitle
                    << zatitle;
        }
    }
  else
      ...
      if (p3d_pltw != NULL)
        {
          ...
          p3d_pltw->replot ();
          DbgLv (0) << "p3db:E: titles" << ptitle << xatitle << yatitle
                    << zatitle;
        }
