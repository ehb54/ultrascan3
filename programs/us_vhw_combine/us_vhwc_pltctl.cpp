...
US_VhwCPlotControl::US_VhwCPlotControl(QWidget *p, QVector<QVector3D> *d, int a_ptype) : US_WidgetsDialog(0, 0) {
   qDebug() << "PCtrl: IN";
   wparent = p;
   xyzdat = d;
   p_type = a_ptype;
...
void US_VhwCPlotControl::plot3_btn() {
   ...
   QString zatitle = envel ? tr("Concen.") : ((p_type == 1) ? tr("B.Frac.") : tr("BF*Conc."));
   ...
   for (int ii = 0; ii < nidpt; ii++) { // Accumulate unique Y values so that we can count columns
   ...
   plot3d_w->setParameters(ncol, nrow, rxscale, ryscale, zscale, pksmooth, pkwidth);
