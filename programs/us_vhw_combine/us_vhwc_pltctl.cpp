...
US_VhwCPlotControl::US_VhwCPlotControl(QWidget* p, QVector<QVector3D>* d,
  ...
  QLayout* lo_contour = us_checkbox(tr("Contour Plot"), ck_contour, true);
  ...
  connect(ck_contour, SIGNAL(toggled(bool)), this, SLOT(ptype_toggle(bool)));
...
QWidget* US_VhwCPlotControl::caller(void) { return wparent; }

// Public slot to force (re-)plot of currently specified 3-D plot
void US_VhwCPlotControl::do_3dplot() { plot3_btn(); }
...
US_Plot3D* US_VhwCPlotControl::widget_3dplot(void) { return plot3d_w; }
...
void US_VhwCPlotControl::plot3_btn() {
  ...
  QString zatitle =
      envel ? tr("Concen.") : ((p_type == 1) ? tr("B.Frac.") : tr("BF*Conc."));
  ...
  for (int ii = 0; ii < nidpt;
       ii++) {  // Accumulate unique Y values so that we can count columns
    double yval = xyzdat->at(ii).y();
    if (!yvals.contains(yval)) yvals << yval;
  }
  ...
  if (plot3d_w == 0) {  // If no 3-D plot window, bring it up now
  ...
  if (contour) {  // Contour plot scale adjustments Envel/Integ
    zscale *= (envel ? ZSCLF_CE : ZSCLF_CI);
    rxscale *= (envel ? XSCLF_CE : XSCLF_CI);
    ryscale *= (envel ? YSCLF_CE : YSCLF_CI);
  } else {  // Points plot scale adjustments Envel/Integ
  ...
  plot3d_w->setParameters(ncol, nrow, rxscale, ryscale, zscale, pksmooth,
                          pkwidth);
