...
US_SelectRunid::US_SelectRunid(bool dbase, QStringList &runIDs) : US_WidgetsDialog(0, 0), runIDs(runIDs) {
   sel_db = dbase;
   dbg_level = US_Settings::us_debug();

   setWindowTitle(tr("Select Run ID for vHW Distributions (%1)").arg(sel_db ? "DB" : "Local"));
...
void US_SelectRunid::list_data() {
   ...
   for (int ii = 0; ii < slabels.size(); ii++) { // Propagate list widget with labels sorted by date
   ...
   te_status->setText(tr("The list derives from %1 scanned run IDs.\n"
...
void US_SelectRunid::scan_local_runs(void) {
   ...
   for (int ii = 0; ii < aucdirs.count(); ii++) {
      ...
      datfilt << "vHW.*distrib.csv"
              << "vHW.*envelope.csv";
      QStringList datfiles = QDir(subdir).entryList(datfilt, QDir::Files, QDir::Name);
