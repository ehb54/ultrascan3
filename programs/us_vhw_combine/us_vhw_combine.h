...
class US_vHW_Combine : public US_Widgets {
      ...
      class DistrDesc {
         public:
            ...
            DistrDesc(const DistrDesc &dd) {
               ...
               color = dd.color;
               totconc = dd.totconc;
               symbol = new QwtSymbol(dd.symbol->style(), dd.symbol->brush(), dd.symbol->pen(), dd.symbol->size());
            };
   ...
   private slots:
      ...
      int reportDocsFromFiles(
         QString &, QString &, QStringList &, US_DB2 *, int &, QString &); //!< Report documents from files
