 ...
 * @brief The US_vHW_Combine class provides functionality for combining vHW
 * distributions.
 */
class US_vHW_Combine : public US_Widgets {
  ...
  class DistrDesc {
   public:
    ...
    DistrDesc(const DistrDesc& dd) {
      ...
      color = dd.color;
      totconc = dd.totconc;
      symbol = new QwtSymbol(dd.symbol->style(), dd.symbol->brush(),
                             dd.symbol->pen(), dd.symbol->size());
    };
 ...
 private slots:
  ...
  void fill_in_desc(QTextStream&, QTextStream&, DistrDesc&, bool,
                    int);                     //!< Fill in description
  ...
  int reportDocsFromFiles(QString&, QString&, QStringList&, US_DB2*, int&,
                          QString&);  //!< Report documents from files

  void help(void) { showHelp.show_help("vhw_combine.html"); }
};
