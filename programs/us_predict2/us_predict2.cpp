...
US_Predict2::US_Predict2() : US_Widgets() {
   ...
   connect(le_param1, SIGNAL(textChanged(const QString &)), SLOT(update_param1(const QString &)));
   ...
   connect(le_param2, SIGNAL(textChanged(const QString &)), SLOT(update_param2(const QString &)));
   ...
   pb_mw_s = us_pushbutton(tr("Molecular Weight + Sedimentation Coeff."));
   connect(pb_mw_s, SIGNAL(clicked()), SLOT(do_mw_s()));
   buttons->addWidget(pb_mw_s, b_row, 0);

   pb_mw_d = us_pushbutton(tr("Molecular Weight + Diffusion Coeff."));
   connect(pb_mw_d, SIGNAL(clicked()), SLOT(do_mw_d()));
   buttons->addWidget(pb_mw_d, b_row, 1);

   pb_s_d = us_pushbutton(tr("Sedimentation Coeff. + Diffusion Coeff."));
...
void US_Predict2::update(void) {
   if (le_param1->text().toDouble() == 0.0 || le_param2->text().toDouble() == 0.0) {
      le_prolate_a->clear();
      le_prolate_b->clear();
      le_prolate_ab->clear();
      le_oblate_a->clear();
...
double US_Predict2::root(int shape, double goal) {
   ...
   if (shape == PROLATE) {
      do {
         test = goal - pow(x, -1.0 / 3.0) * sqrt(sq(x) - 1.0) / log(x + sqrt(sq(x) - 1.0));
      ...
      do {
         test = goal - sqrt(sq(x) - 1.0) / (pow(x, 2.0 / 3.0) * atan(sqrt(sq(x) - 1.0)));
      ...
      do {
         test = goal - pow(2.0 / 3.0, 1.0 / 3.0) * pow(x, 2.0 / 3.0) / (log(2.0 * x) - 0.3);
...
void US_Predict2::get_buffer(void) {
   if (model == None || le_param1->text().toDouble() == 0.0 || le_param2->text().toDouble() == 0.0) {
   ...
   connect(buffer_dialog, SIGNAL(valueChanged(double, double)), SLOT(update_buffer(double, double)));
