...
bool
US_Predict2::check_valid (double f_f0)
{
  if (f_f0 < 1.0)
    {
...
double
US_Predict2::root (int shape, double goal)
{
  double test;
  ...
  if (shape == PROLATE)
    {
      do
        {
          test = goal
                 - pow (x, -1.0 / 3.0) * sqrt (sq (x) - 1.0)
                       / log (x + sqrt (sq (x) - 1.0));

          // Iterate until the difference between subsequent x value
          // evaluations is too small to be relevant
          ...
          // We adjust the new test x-value by adding half the stepsize from
          // the last step to the lower limit:
  ...
  else if (shape == ROD)
    {
      ...
      do
        {
          test = goal
                 - pow (2.0 / 3.0, 1.0 / 3.0) * pow (x, 2.0 / 3.0)
                       / (log (2.0 * x) - 0.3);
...
void
US_Predict2::update_param1 (const QString &s)
{
  if (model == SED_DIFF)
...
void
US_Predict2::update_param2 (const QString &s)
{
  if (model == MW_SEDIMENTATION)
    sed_coeff = s.toDouble ();
  else
...
void
US_Predict2::density (const QString &s)
{
...
void
US_Predict2::viscosity (const QString &s)
{
...
void
US_Predict2::vbar (const QString &s)
{
  d.vbar20 = s.toDouble ();
  d.vbar = d.vbar;
  update ();
...
void
US_Predict2::degC (const QString &s)
{
...
void
US_Predict2::update_buffer (double density, double viscosity)
{
  d.density = density;
...
void
US_Predict2::update_vbar (US_Analyte analyte)
{
...
void
US_Predict2::update_solution (US_Solution solution)
{
