...
US_Grid_Editor::US_Grid_Editor() : US_Widgets() {
  ...
  QGridLayout* lyt_x_ff0 =
      us_radiobutton(Attribute::symbol(Attribute::ATTR_K), rb_x_ff0, false);
  QGridLayout* lyt_x_mw =
      us_radiobutton(Attribute::symbol(Attribute::ATTR_M), rb_x_mw, false);
  QGridLayout* lyt_x_vbar =
      us_radiobutton(Attribute::symbol(Attribute::ATTR_V), rb_x_vbar, false);
  QGridLayout* lyt_x_D =
      us_radiobutton(Attribute::symbol(Attribute::ATTR_D), rb_x_D, false);
  QGridLayout* lyt_x_sr =
      us_radiobutton(Attribute::symbol(Attribute::ATTR_SR), rb_x_sr, false);
  QGridLayout* lyt_x_Dr =
      us_radiobutton(Attribute::symbol(Attribute::ATTR_DR), rb_x_Dr, false);

  QGridLayout* lyt_y_s =
      us_radiobutton(Attribute::symbol(Attribute::ATTR_S), rb_y_s, false);
  QGridLayout* lyt_y_ff0 =
      us_radiobutton(Attribute::symbol(Attribute::ATTR_K), rb_y_ff0, false);
  QGridLayout* lyt_y_mw =
      us_radiobutton(Attribute::symbol(Attribute::ATTR_M), rb_y_mw, false);
  QGridLayout* lyt_y_vbar =
      us_radiobutton(Attribute::symbol(Attribute::ATTR_V), rb_y_vbar, false);
  QGridLayout* lyt_y_D =
      us_radiobutton(Attribute::symbol(Attribute::ATTR_D), rb_y_D, false);
  QGridLayout* lyt_y_sr =
      us_radiobutton(Attribute::symbol(Attribute::ATTR_SR), rb_y_sr, false);
  QGridLayout* lyt_y_Dr =
      us_radiobutton(Attribute::symbol(Attribute::ATTR_DR), rb_y_Dr, false);
...
void US_Grid_Editor::plot_points() {
  ...
  for (int nn = 0; nn < grid_points.size(); nn++) {
    ...
    QwtSymbol* symbol = new QwtSymbol(QwtSymbol::Ellipse, QBrush(color_base),
...
void US_Grid_Editor::plot_subgrid(double subgrid_id) {
  ...
  QwtSymbol* symbol = new QwtSymbol(QwtSymbol::Ellipse, QBrush(color_subgrid),
...
void US_Grid_Editor::highlight(int id) {
  ...
  if (id != -1) {
    ...
    QwtSymbol* symbol =
        new QwtSymbol(QwtSymbol::Ellipse, QBrush(color_highlight), pen, size);
    point_curves[id]->setSymbol(symbol);

    GridInfo ginfo = grid_info.at(id);

    double x_min =
        correct_unit(ginfo.xMin, Attribute::from_symbol(ginfo.xType), true);
    double x_max =
        correct_unit(ginfo.xMax, Attribute::from_symbol(ginfo.xType), true);
    double y_min =
        correct_unit(ginfo.yMin, Attribute::from_symbol(ginfo.yType), true);
    double y_max =
        correct_unit(ginfo.yMax, Attribute::from_symbol(ginfo.yType), true);
    double z_val =
        correct_unit(ginfo.zVal, Attribute::from_symbol(ginfo.zType), true);
...
bool US_Grid_Editor::gen_points(double x1, double x2, int np, bool isLog,
...
bool US_Grid_Editor::gen_grid_points(const QVector<double>& x,
...
double US_Grid_Editor::correct_unit(double val, Attribute::Type type,
                                    bool h_flag) {
  double output = val;
...
bool US_Grid_Editor::check_overlap(double xMin, double xMax, double yMin,
...
void US_Grid_Editor::setup_grid() {
  if (!grid_points.isEmpty()) {
    int yes = QMessageBox::question(
        this, "Warning!",
        tr("Partial grid list is not empty and by changing the grid setting, "
           "it "
           "will be deleted.<br/>"
...
void US_Grid_Editor::save() {
  ...
  if (mbox.clickedButton() ==
      pb_edit) {  // Open another dialog to get a modified runID
    bool ok;
    QString newtext = "";
...
US_Grid_Preset::US_Grid_Preset(QWidget* parent, Attribute::Type x,
  ...
  QGridLayout* x_s =
      us_radiobutton(Attribute::long_desc(Attribute::ATTR_S), rb_x_s, true);
  QGridLayout* x_ff0 =
      us_radiobutton(Attribute::long_desc(Attribute::ATTR_K), rb_x_ff0, false);
  QGridLayout* x_mw =
      us_radiobutton(Attribute::long_desc(Attribute::ATTR_M), rb_x_mw, true);
  QGridLayout* x_vbar =
      us_radiobutton(Attribute::long_desc(Attribute::ATTR_V), rb_x_vbar, true);
  QGridLayout* x_D =
      us_radiobutton(Attribute::long_desc(Attribute::ATTR_D), rb_x_D, true);

  QGridLayout* y_s =
      us_radiobutton(Attribute::long_desc(Attribute::ATTR_S), rb_y_s, false);
  QGridLayout* y_ff0 =
      us_radiobutton(Attribute::long_desc(Attribute::ATTR_K), rb_y_ff0, true);
  QGridLayout* y_mw =
      us_radiobutton(Attribute::long_desc(Attribute::ATTR_M), rb_y_mw, true);
  QGridLayout* y_vbar =
      us_radiobutton(Attribute::long_desc(Attribute::ATTR_V), rb_y_vbar, true);
  QGridLayout* y_D =
      us_radiobutton(Attribute::long_desc(Attribute::ATTR_D), rb_y_D, true);
...
void US_Grid_Preset::parameters(Attribute::Type& x, Attribute::Type& y,
                                Attribute::Type& z) {
  x = x_param;
  y = y_param;
  z = z_param;
...
void US_Grid_Preset::apply() { accept(); }

void US_Grid_Preset::cancel() { reject(); }
...
bool GridPoint::set_param(const QVector<double>& values,
  ...
  for (int ii = 0; ii < 3; ii++) {
    ...
    if (types.at(ii) == Attribute::ATTR_S) {
      ...
      if (VBAR <= 0) {
        error =
            "Partial specific volume is less than or equal to zero. Please "
            "correct it!";
        return false;
      }
      ...
      if (D <= 0) {
        error =
            "Diffusion coefficient is less than or equal to zero. Please "
            "correct it!";
        return false;
      }
      ...
      if (F <= 0) {
        error =
            "Frictional coefficient is less than or equal to zero. Please "
            "correct it!";
        return false;
      }
    } else {
...
QString GridPoint::error_string() { return error; }
...
void GridPoint::set_id(int i) { id = i; }
...
int GridPoint::get_row() const { return row; }

int GridPoint::get_col() const { return col; }

int GridPoint::get_id() const { return id; }

double GridPoint::x_value() const { return value(x_param); }

double GridPoint::y_value() const { return value(y_param); }

double GridPoint::z_value() const { return value(z_param); }
...
void GridPoint::calculate_real() {
  ...
  DbgLv(2) << "Solution Data:" << QObject::tr("T=%1").arg(temperature)
           ...
           << QObject::tr("s*=%1").arg(S_real) << QObject::tr("D=%1").arg(D)
...
bool GridPoint::check_s_vbar() {
  ...
  if ((buoy > 0 && S < 0) || (buoy < 0 && S > 0)) {
    ...
    error +=
        QObject::tr("<b>Buoyancy = 1 - Density( 20°C water ) x vbar<b/><br/>");
  ...
  if (std::abs(S) < S_TRSHL) {
    error =
        QObject::tr("s = %1 [Sv]. s values less than %2 [Sv] are not allowed!")
            .arg(S * 1e13)
            .arg(S_TRSHL * 1e13);
    return false;
  }
...
bool GridPoint::contains(Attribute::Type p1, Attribute::Type p2,
                         Attribute::Type p3) {
...
QString Attribute::title(Type t) {
  ...
  if (t == Attribute::ATTR_S)
    ...
    str =
        "<p>Diffusion Coefficient ( 20,W ) [ cm<sup>2</sup> s<sup>-1</sup> "
        "]</p>";
  else if (t == Attribute::ATTR_F)
    ...
    str =
        "<p>Diffusion Coefficient ( T,Buffer ) [ cm<sup>2</sup> s<sup>-1</sup> "
        "]</p>";
  return str;
}
