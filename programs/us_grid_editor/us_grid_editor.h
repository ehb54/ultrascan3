...
public:
  ...
  //! \brief A static method to return the long-version description of the
  //! input Type.
  static QString long_desc (Type);

  //! \brief A static method to return the short-version description of the
  //! input Type.
...
public:
  ...
  bool set_param (const QVector<double> &values,
  ...
  //! \brief A method to set the grid row and column numbers where the grid
  //! point is placed.
  //! \param row.
  //! \param column
...
private:
  bool dvt_set;   //!< true if density, viscosity, and temperature are already
                  //!< set.
  ...
  //! \brief Calculate s and D apparent from s20w and D20w and buffer
  //! condition.
...
public:
  US_Grid_Preset (QWidget *, Attribute::Type, Attribute::Type,
                  Attribute::Type);
...
private:
  ...
  QPushButton *pb_default_plot; //!< plot grid points where x and y axis match
                                //!< x and y types.
