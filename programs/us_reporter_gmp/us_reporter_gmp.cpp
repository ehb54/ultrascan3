...
US_ReporterGMP::US_ReporterGMP(QMap<QString, QString> t_c) : US_Widgets() {
   ...
   write_gmp_report_DB_test(report_filepath, html_filePath, autolfowGMPReportID, autoStatusID, autoID);
}

//write GMP report to DB Test
void US_ReporterGMP::write_gmp_report_DB_test(
   QString report_filepath, QString html_filePath, int autolfowGMPReportID, QString autoStatusID, QString autoID) {
   ...
   int writeStatus_html = db.writeBlobToDB(html_filePath, QString("upload_gmpReportData_html"), autolfowGMPReportID);
   ...
   if (writeStatus_html == US_DB2::DBERROR) {
      ...
      QMessageBox::warning(
         this, "Error", "returned processing html file:\n" + html_filePath + "\n" + db.lastError() + "\n");
   ...
   int writeStatus = db.writeBlobToDB(report_filepath, QString("upload_gmpReportData"), autolfowGMPReportID);
   ...
   if (clear_GMP_report_record) {
      ...
      qry.clear();
      qry << "update_autoflowStatusReport_record" << autoStatusID << autoID << reporting_Json;
...
US_ReporterGMP::US_ReporterGMP() : US_Widgets() {
   ...
   connect(pb_loadreport_db, SIGNAL(clicked()), this, SLOT(load_gmp_report_db()));
   ...
   genTree->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() + 1));
   genTree->installEventFilter(this);
   genTreeLayout->addWidget(lb_gentree);
   genTreeLayout->addWidget(genTree);

   genTree->setStyleSheet(
      "QTreeWidget { font: bold; font-size: " + QString::number(sfont.pointSize())
      + "pt;}  QTreeView { alternate-background-color: yellow;} QTreeView::item:hover { border: black;  "
        "border-radius:1px;  background-color: rgba(0,128,255,95);}");
   ...
   miscTree->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() + 1));
   ...
   perChanTree->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() + 1));
   ...
   combPlotsTree->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() + 1));
...
QString US_ReporterGMP::missing_models_msg(void) {
   ...
   qDebug() << "Number of total missing triples; Number of dropped triples -- " << num_total_missed_triples << "; "
            << num_dropped_triples;
            ...
            for (int i = 0; i < json_array.size(); ++i) {
               foreach (const QString &array_key, json_array[ i ].toObject().keys()) {
                  ...
                  if (array_key == "modelID" && array_key == model_property) {
                     ...
                     qDebug() << "modelDescJson Map: -- model, property, value: " << key_mod << array_key
...
void US_ReporterGMP::load_gmp_report_db(void) {
   ...
   if (db.lastErrno() != US_DB2::OK) {
      QMessageBox::warning(
         this, tr("LIMS DB Connection Problem"), tr("Could not connect to database \n") + db.lastError());
      return;
   }
   ...
   int db_read = db.readBlobFromDB(GMPReportfpath, "download_gmpReportData", gmpReport_id_selected.toInt());
   ...
   msgBox.setInformativeText(tr("Report was downloaded form DB in .PDF format and saved at: \n%1\n\n"
                                "When this dialog is closed, the report can be re-opened by clicking \'View Downloaded "
                                "Report\' button on the left.")
                                .arg(filePath_db));
...
int US_ReporterGMP::list_all_gmp_reports_db(QList<QStringList> &gmpReportsDBdata, US_DB2 *db) {
   ...
   while (db->next()) {
      ...
      QString ptime_created = US_Util::toUTCDatetimeText(time_created.toString(Qt::ISODate), true).section(":", 0, 1)
                              + " UTC";
...
void US_ReporterGMP::load_gmp_run(void) {
   ...
   for (int i = 0; i < Array_of_tripleNames.size(); ++i) {
      ...
      for (it = tmap.begin(); it != tmap.end(); ++it) {
         qDebug() << "ModelsDesc: Triple, QMap -- " << Array_of_tripleNames[ i ] << it.key() << it.value();

         qDebug() << "ModelsDescGuid: Triple, QMap -- " << Array_of_tripleNames[ i ] << it.key()
                  << tmapguid[ it.key() ];
      }
   }
...
int US_ReporterGMP::list_all_autoflow_records(QList<QStringList> &autoflowdata) {
   ...
   while (db->next()) {
      ...
      if (time_started.toString().isEmpty())
         ...
         if (
            status == "EDITING" || status == "EDIT_DATA" || status == "ANALYSIS" || status == "REPORT"
            || status == "E-SIGNATURES")
      ...
      if (US_Settings::us_inv_level() < 3) {
         ...
         if (user_id && user_id == invID.toInt()) { //Do we allow operator as defined in autoflow record to also see reports??
...
void US_ReporterGMP::read_protocol_and_reportMasks(void) {
   ...
   //Debug: protocol
   qDebug() << "Protocols' details: -- " << currProto.investigator << currProto.runname << currProto.protoname
            << currProto.protoID << currProto.project << currProto.temperature << currProto.temeq_delay
            << currProto.exp_label;
   ...
   qDebug() << "Wavelengths ch_wvls[ channel_desc_alt ] " << ch_wvls[ channel_desc_alt ]
            << " for channel: " << channel_desc_alt;
   US_ReportGMP reportGMP = ch_reports[ channel_desc_alt ][ wvl ];

   if (reportGMP.reportItems.size() > 0)
      qDebug() << "AProfile's && ReportGMP's details: -- " << currAProf.aprofname << currAProf.protoname
               << currAProf.chndescs << currAProf.chndescs_alt << currAProf.lc_ratios << cAP2.parms[ 0 ].channel
               << cAPp.parms[ 0 ].channel << reportGMP.rmsd_limit << reportGMP.wavelength
               << reportGMP.reportItems[ 0 ].type << reportGMP.reportItems[ 0 ].ind_combined_plot;
   else
      qDebug() << "AProfile's && ReportGMP's details: -- " << currAProf.aprofname << currAProf.protoname
               << currAProf.chndescs << currAProf.chndescs_alt << currAProf.lc_ratios << cAP2.parms[ 0 ].channel
               << cAPp.parms[ 0 ].channel << reportGMP.rmsd_limit << reportGMP.wavelength << "No GMP_Report_Items!!!";
   ...
   qDebug() << "Pseudo3d pars: " << reportGMP.pseudo3d_2dsait_s_ff0 << reportGMP.pseudo3d_2dsait_s_d
            << reportGMP.pseudo3d_2dsait_mw_ff0 << reportGMP.pseudo3d_2dsait_mw_d;
      ...
      foreach (const QString &key, json_obj.keys()) {
         ...
         for (int i = 0; i < json_array.size(); ++i) {
            foreach (const QString &array_key, json_array[ i ].toObject().keys()) {
               ...
               qDebug() << "ComboPlots Map: -- type_method, parameter, value: " << key << array_key
                        << json_array[ i ].toObject().value(array_key).toString();
            }
         }
      }
...
void US_ReporterGMP::build_miscTree(void) {
   ...
   connect(miscTree, SIGNAL(itemChanged(QTreeWidgetItem *, int)), this, SLOT(changedItem(QTreeWidgetItem *, int)));
}

//build general report mask tree
void US_ReporterGMP::build_genTree(void) {
   QString indent("  ");
   QStringList topItemNameList, solutionItemNameList, analysisItemNameList, analysisGenItemNameList,
      analysis2DSAItemNameList, analysisPCSAItemNameList;
   int wiubase = ( int ) QTreeWidgetItem::UserType;
   ...
   for (int i = 0; i < topLevelItems.size(); ++i) {
      ...
      if (topItemName.contains("Solutions")) {
         ...
         for (int is = 0; is < solutionItems.size(); ++is) {
            ...
            solutionItem[ solutionItemName ]
               = new QTreeWidgetItem(topItem[ topItemName ], solutionItemNameList, wiubase);
      ...
      //Analysis Profile: add 2-levelchildren
         ...
         for (int ia = 0; ia < analysisItems.size(); ++ia) {
            ...
            analysisItem[ analysisItemName ]
               = new QTreeWidgetItem(topItem[ topItemName ], analysisItemNameList, wiubase);
            ...
            if (analysisItemName.contains("General")) {
               ...
               for (int iag = 0; iag < analysisGenItems.size(); ++iag) {
                  ...
                  analysisGenItem[ analysisGenItemName ]
                     = new QTreeWidgetItem(analysisItem[ analysisItemName ], analysisGenItemNameList, wiubase);
            ...
            if (analysisItemName.contains("2DSA")) {
               ...
               for (int ia2 = 0; ia2 < analysis2DSAItems.size(); ++ia2) {
                  ...
                  analysis2DSAItem[ analysis2DSAItemName ]
                     = new QTreeWidgetItem(analysisItem[ analysisItemName ], analysis2DSAItemNameList, wiubase);
            ...
            if (analysisItemName.contains("PCSA")) {
               ...
               for (int iap = 0; iap < analysisPCSAItems.size(); ++iap) {
                  ...
                  analysisPCSAItem[ analysisPCSAItemName ]
                     = new QTreeWidgetItem(analysisItem[ analysisItemName ], analysisPCSAItemNameList, wiubase);
   ...
   connect(genTree, SIGNAL(itemChanged(QTreeWidgetItem *, int)), this, SLOT(changedItem(QTreeWidgetItem *, int)));
}
...
void US_ReporterGMP::changedItem(QTreeWidgetItem *item, int col) {
   ...
   //reconnect
   connect(
      item->treeWidget(), SIGNAL(itemChanged(QTreeWidgetItem *, int)), this, SLOT(changedItem(QTreeWidgetItem *, int)));
}
...
void US_ReporterGMP::build_perChanTree_abde(void) {
   ...
   for (int ac = 0; ac < abde_channList.size(); ++ac) {
      ...
      tripleReportMasksList_vals << reportGMP.integration_results_mask << reportGMP.plots_mask;
   ...
   connect(perChanTree, SIGNAL(itemChanged(QTreeWidgetItem *, int)), this, SLOT(changedItem(QTreeWidgetItem *, int)));
}

//build perChanTree
void US_ReporterGMP::build_perChanTree(void) {
   QString indent("  ");
   QStringList chanItemNameList, tripleItemNameList, tripleItemModelNameList, tripleMaskItemNameList,
      ...
      if (triple_report) {
         ...
         for (int jj = 0; jj < chann_wvl_number; ++jj) {
            ...
            qDebug() << "For triple, " << tripleName << ", and models: " << Triple_to_Models[ tripleName ]
                     << reportGMP.tot_conc_mask << reportGMP.rmsd_limit_mask << reportGMP.av_intensity_mask
                     << reportGMP.experiment_duration_mask << reportGMP.integration_results_mask << reportGMP.plots_mask
            ...
            for (int mm = 0; mm < tripleReportModelsList.size(); ++mm) {
               ...
               tripleModelItem[ tripleModelName ]
                  = new QTreeWidgetItem(tripleItem[ tripleItemName ], tripleItemModelNameList, wiubase);
               ...
               tripleReportMasksList_vals.clear();
               tripleReportMasksList_vals << reportGMP.tot_conc_mask << reportGMP.rmsd_limit_mask
                                          << reportGMP.av_intensity_mask
                                          //<< reportGMP. experiment_duration_mask
                                          << true << reportGMP.integration_results_mask << reportGMP.plots_mask
                                          << reportGMP.pseudo3d_mask;
               ...
               for (int kk = 0; kk < tripleReportMasksList.size(); ++kk) {
                  ...
                  tripleMaskItem[ tripleModelName ]
                     = new QTreeWidgetItem(tripleModelItem[ tripleModelName ], tripleMaskItemNameList, wiubase);
                  ...
                  if (tripleMaskItemName.contains("Plots")) {
                     ...
                     for (int hh = 0; hh < tripleReportMasksPlotList.size(); ++hh) {
                        ...
                        tripleMaskPlotItem[ tripleModelName ]
                           = new QTreeWidgetItem(tripleMaskItem[ tripleModelName ], tripleMaskItemPlotNameList, wiubase);
                  ...
                  if (tripleMaskItemName.contains("Pseudo3d")) {
                     ...
                     if (tripleItemModelName.contains("2DSA-IT")) {
                        tripleReportMasksPseudoList_vals
                           << reportGMP.pseudo3d_2dsait_s_ff0 << reportGMP.pseudo3d_2dsait_s_d
                           << reportGMP.pseudo3d_2dsait_mw_ff0 << reportGMP.pseudo3d_2dsait_mw_d;
                        qDebug() << "In pseudo3d tree: tripleReportMasksPseudoList_vals (2DSA-IT) -- "
                                 << tripleReportMasksPseudoList_vals;
                     }
                     else if (tripleItemModelName.contains("2DSA-MC")) {
                        tripleReportMasksPseudoList_vals
                           << reportGMP.pseudo3d_2dsamc_s_ff0 << reportGMP.pseudo3d_2dsamc_s_d
                           << reportGMP.pseudo3d_2dsamc_mw_ff0 << reportGMP.pseudo3d_2dsamc_mw_d;
                        qDebug() << "In pseudo3d tree: tripleReportMasksPseudoList_vals (2DSA-MC) -- "
                                 << tripleReportMasksPseudoList_vals;
                     }
                     else if (tripleItemModelName.contains("PCSA")) {
                        tripleReportMasksPseudoList_vals << reportGMP.pseudo3d_pcsa_s_ff0 << reportGMP.pseudo3d_pcsa_s_d
                                                         << reportGMP.pseudo3d_pcsa_mw_ff0
                                                         << reportGMP.pseudo3d_pcsa_mw_d;
                        qDebug() << "In pseudo3d tree: tripleReportMasksPseudoList_vals (PCSA) -- "
                                 << tripleReportMasksPseudoList_vals;
                     }
                     ...
                     for (int ps = 0; ps < tripleReportMasksPseudoList.size(); ++ps) {
                        ...
                        tripleMaskPseudoItem[ tripleModelName ] = new QTreeWidgetItem(
                           tripleMaskItem[ tripleModelName ], tripleMaskItemPseudoNameList, wiubase);
                  ...
                  if (tripleMaskItemName.contains("Individual")) {
                     ...
                     for (int tm = 0; tm < types_methods_list.size(); tm++) {
                        ...
                        if (c_method == tripleReportModelsList[ mm ]) {
                           QMap<QString, bool> c_ranges_bool
                              = indComboPlotsMapTripleTypeRangeBool[ tripleName ][ types_methods_list[ tm ] ];
                     ...
                     for (int icp = 0; icp < tripleReportMasksIndividualPlotList.size(); ++icp) {
                        ...
                        tripleMaskIndComboPlotItem[ tripleModelName ]
                           = new QTreeWidgetItem(tripleMaskItem[ tripleModelName ], individualPlotNameList, wiubase);
                        //Checked/no
                        qDebug() << "To show/no Ind.ComboPlots -- " << tripleReportMasksList_vals[ kk ]
   ...
   for (ch = chanItem.begin(); ch != chanItem.end(); ++ch) {
      ...
      for (int i = 0; i < channel_children; ++i) {
         ...
         if (!child_triple->childCount()) {
            qDebug() << "Channel's " << ch.value()->text(1) << " triple: " << child_triple->text(1)
                     << " has NO children";
...
void US_ReporterGMP::build_combPlotsTree(void) {
   ...
   connect(combPlotsTree, SIGNAL(itemChanged(QTreeWidgetItem *, int)), this, SLOT(changedItem(QTreeWidgetItem *, int)));
}
...
void US_ReporterGMP::view_report(void) {
   ...
   if (check_file.exists() && check_file.isFile()) {
      ...
      QMessageBox::warning(
         this, tr("Error: Cannot Open .PDF File"),
         tr("%1 \n\n"
...
void US_ReporterGMP::generate_report(void) {
   ...
   else { //Will be modified for stand-alone GMP Reporter based on edited tree JSON
      for (int i = 0; i < Array_of_triples.size(); ++i) {
         currentTripleName = Array_of_triples[ i ];
         ...
         for (int j = 0; j < models_to_do.size(); ++j) {
            ...
            if (
               ...
               //Individual Combo plots
               qDebug() << "INDCOMBO, perChanMask_edited. has_tripleModelIndCombo_items[ triplename_alt ][ "
                           "models_to_do[ j ] ] -- "
   ...
   if (auto_mode) {
      ...
      ************************************************************/
      ...
      if (status == US_DB2::NO_AUTOFLOW_RECORD) {
         QMessageBox::warning(
            this, tr("Autoflow Record Not Updated"),
      ...
      qry.clear();
      qry << "update_gmp_review_record_by_esigner" << eSign_details[ "ID" ] << AutoflowID_auto << eSignStatusJson
          << "NO";
      ...
      msgBox_a.setInformativeText(tr("Report PDF was saved at: \n%1\n\n"
                                     "When this dialog is closed, the report can be re-opened by clicking \'View "
                                     "Generated Report\' button at the bottom."
                                     "\n\n"
      ...
      msgBox.setInformativeText(tr("Report PDF was saved at: \n%1\n\n"
                                   "When this dialog is closed, the report can be re-opened by clicking \'View "
                                   "Generated Report\' button on the left.")
                                   .arg(filePath));
...
void US_ReporterGMP::process_abde_plots(void) {
   ...
   connect(
      sdiag_norm_profile, SIGNAL(pass_rmsd_info(QMap<QString, double> &)), this,
      SLOT(get_abde_rmsds(QMap<QString, double> &)));
   connect(
      sdiag_norm_profile, SIGNAL(pass_menisc_info(QMap<QString, double> &)), this,
      SLOT(get_abde_menisc(QMap<QString, double> &)));
   connect(
      sdiag_norm_profile, SIGNAL(pass_percents_info(QMap<QString, QMap<QString, double>> &)), this,
      SLOT(get_abde_percents(QMap<QString, QMap<QString, double>> &)));
   connect(
      sdiag_norm_profile, SIGNAL(pass_data_per_channel(QMap<QString, QMap<QString, QVector<QVector<double>>>> &)), this,
      SLOT(get_abde_data_per_channel(QMap<QString, QMap<QString, QVector<QVector<double>>>> &)));
   ...
   for (int i = 0; i < abde_channList.size(); ++i) {
      ...
      qDebug() << "[ABDE proc.], channel_name_abde, abde_channList[i] -- " << channel_name_abde << abde_channList[ i ];
...
void US_ReporterGMP::simulate_triple(const QString triplesname, QString stage_model) {
   ...
   progress_msg = new QProgressDialog(
      QString("Downloading data and models for triple %1...").arg(triplesname), QString(), 0, 5, this);
   ...
   QString tmst_fpath = US_Settings::resultDir() + "/" + FileName_parsed + "/" + FileName_parsed + ".time_state.tmst";
   ...
   QString expID;
   int idExp = 0;
   query << "get_experiment_info_by_runID" << FileName_parsed << QString::number(invID);
   dbP->query(query);
   ...
   if (check_file.exists() && check_file.isFile()) { // Get speed_steps from an existing timestate file
      ...
      for (int jj = 0; jj < qMin(essknt, tssknt); jj++) {
         qDebug() << "LD:  jj" << jj << "(e) tf tl wf wl scns" << speed_steps[ jj ].time_first
                  << speed_steps[ jj ].time_last << speed_steps[ jj ].w2t_first << speed_steps[ jj ].w2t_last
                  << speed_steps[ jj ].scans;
         qDebug() << "LD:    (t) tf tl wf wl scns" << simparams.speed_step[ jj ].time_first
                  << simparams.speed_step[ jj ].time_last << simparams.speed_step[ jj ].w2t_first
                  << simparams.speed_step[ jj ].w2t_last << simparams.speed_step[ jj ].scans;
      }
      //*DEBUG*
      ...
      for (int jj = 0; jj < simparams.speed_step.count(); jj++) {
         ...
         qDebug() << "LD:    (s) tf tl wf wl scns" << speed_steps[ jj ].time_first << speed_steps[ jj ].time_last
                  << speed_steps[ jj ].w2t_first << speed_steps[ jj ].w2t_last << speed_steps[ jj ].scans;
      }
   }
   progress_msg->setValue(2);
   qApp->processEvents();
   ...
   bufvl = US_SolutionVals::values(dbP, edata, solID, svbar, bdens, bvisc, bcomp, bmanu, errmsg);
   progress_msg->setValue(3);
   ...
   if (bufvl) {
      ...
      if (solID.isEmpty()) {
         QMessageBox::warning(this, tr("Solution/Buffer Fetch"), tr("Empty solution ID value!"));
      }
...
US_DataIO::EditedData *US_ReporterGMP::rg_editdata() {
   return edata;
}

// public function to get pointer to list of excluded scans
QList<int> *US_ReporterGMP::rg_excllist() {
   return &excludedScans;
}

// public function to get pointer to sim data
US_DataIO::RawData *US_ReporterGMP::rg_simdata() {
   return sdata;
}

// public function to get pointer to load model
US_Model *US_ReporterGMP::rg_model() {
   return &model;
}

// public function to get pointer to TI noise
US_Noise *US_ReporterGMP::rg_ti_noise() {
   return &ti_noise;
}

// public function to get pointer to RI noise
US_Noise *US_ReporterGMP::rg_ri_noise() {
   return &ri_noise;
}

// public function to get pointer to resid bitmap diag
QPointer<US_ResidsBitmap> US_ReporterGMP::rg_resbmap() {
   return rbmapd;
}

QString US_ReporterGMP::rg_tripleInfo() {
   return tripleInfo;
}
...
bool US_ReporterGMP::loadData(QMap<QString, QString> &triple_information) {
   ...
   qDebug() << "In load Data: triple, eID (from modelsLink) -- " << triple_information[ "triple_name" ]
   ...
   rfilename = triple_information[ "filename" ] + "." + efilename_copy_list[ 2 ] + "." + efilename_copy_list[ 3 ] + "."
               + efilename_copy_list[ 4 ] + "." + efilename_copy_list[ 5 ] + ".auc";
...
bool US_ReporterGMP::loadModel(QMap<QString, QString> &triple_information) {
   ...
   int rc = 0;
   qDebug() << "ModelID to retrieve for triple, stage: -- " << triple_information[ "triple_name" ]
   ...
   is_dmga_mc = (model.monteCarlo && model.description.contains("DMGA") && model.description.contains("_mcN"));
   qDebug() << "post-Load mC" << model.monteCarlo << "is_dmga_mc" << is_dmga_mc << "description" << model.description;

   if (model.components.size() == 0) {
      QMessageBox::critical(this, tr("Empty Model"), tr("Loaded model has ZERO components!"));
      return false;
   }
...
bool US_ReporterGMP::loadNoises(QMap<QString, QString> &triple_information) {
   ...
   if (ntinois > 0 && ntinois < npoints) { // pad out ti noise values to radius count
      int jj = ntinois;
      while (jj++ < npoints)
         ti_noise.values << 0.0;
   ...
   if (nrinois > 0 && nrinois < nscans) { // pad out ri noise values to scan count
      int jj = nrinois;
      while (jj++ < nscans)
         ri_noise.values << 0.0;
...
void US_ReporterGMP::loadNoises_whenAbsent() {
   ...
   qDebug() << "load_noise_whenAbsent(): mieGUIDs, nieGUIDs, editGUID, modelGUID -- " << mieGUIDs << nieGUIDs
            << editGUID << modelGUID;
   ...
   if (nenois > 0) { // There is/are noise(s):  ask user if she wants to load
      ...
      if (nenois > 1 && noisdf == 0) { // Noise exists and noise-dialog flag set to "Auto-load"
...
int US_ReporterGMP::count_noise_auto(
   US_DataIO::EditedData *edata, US_Model *model, QStringList &mieGUIDs, QStringList &nieGUIDs) {
   ...
   if (nenois > 0) { // There is/are noise(s):  ask user if she wants to load
      ...
      if (nenois > 1) {
         msg = tr("There are noise files") + amsg + tr("Do you want to load some of them?");
      }
         ...
         if ((noimax - noimin) > (datmax - datmin)) { // Insert a warning if noise appears corrupt or unusual
      ...
      if (noidiag > 0) {
         ...
         if (msgBox.exec() != QMessageBox::Yes) { // user did not say "yes":  return zero count
...
int US_ReporterGMP::noises_in_model_auto(QString mGUID, QStringList &nGUIDs) {
   ...
   for (int ii = 0; ii < noiIDs.size(); ii++) { // Examine noises list; Save to this list if model GUID matches
      xnGUID = noiIDs.at(ii);
...
int US_ReporterGMP::models_in_edit_auto(QString eGUID, QStringList &mGUIDs) {
   ...
   for (int ii = 0; ii < modIDs.size(); ii++) { // Examine models list; Save to this list if edit GUID matches
      xmGUID = modIDs.at(ii);
...
int US_ReporterGMP::id_list_db_auto(QString daEditGUID) {
   ...
   while (db.next()) { // Accumulate from db desc entries matching noise model IDs
      QString modGUID = db.value(1).toString();
      QString modEdID = db.value(6).toString();

      if (noiMoIDs.contains(modGUID) && modEdID == daEditID) { // Only list models that have associated noise and match edit
         modIDs << modGUID;
   ...
   for (int ii = modIDs.size() - 1; ii >= 0; ii--) { // Work from the back so any removed records do not affect indexes
      ...
      if (mCarlo) { // Treat monte carlo in a special way (as single composite model)
         if (reqIDs.contains(reqID)) { // already have this request GUID, so remove this model
            modIDs.removeAt(ii);
...
void US_ReporterGMP::simulateModel(QMap<QString, QString> &tripleInfo) {
   progress_msg->setLabelText(
      QString("Simulating model %1 for triple %2...").arg(tripleInfo[ "stage_name" ]).arg(tripleInfo[ "triple_name" ]));
   progress_msg->setValue(0);
   qApp->processEvents();
   ...
   qDebug() << "SimMdl: 0) s D c" << model_used.components[ 0 ].s << model_used.components[ 0 ].D
            << model_used.components[ 0 ].signal_concentration << "  n" << lc;
   qDebug() << "SimMdl: n) s D c" << model_used.components[ lc ].s << model_used.components[ lc ].D
            << model_used.components[ lc ].signal_concentration;

   adjustModel();

   qDebug() << "SimMdl: 0) s D c" << model.components[ 0 ].s << model.components[ 0 ].D
            << model.components[ 0 ].signal_concentration;
   qDebug() << "SimMdl: n) s D c" << model.components[ lc ].s << model.components[ lc ].D
   ...
   qDebug() << "SimMdl: simpoints" << simparams.simpoints << "rreso" << simparams.radial_resolution << "bottom_sim"
            << simparams.bottom << "bottom_dat" << edata->bottom;
   ...
   if ((check_file.exists()) && (check_file.isFile())) {
      if (US_AstfemMath::timestate_onesec(tmst_fpath, *sdata)) { // Load timestate that is already at 1-second-intervals
         simparams.simSpeedsFromTimeState(tmst_fpath);
         qDebug() << "SimMdl: timestate file exists" << tmst_fpath << " timestateobject,count = " << simparams.tsobj
      ...
      if (!simparams.sim) { // Compute acceleration rate for non-astfem_sim data
         ...
         double rate = ( double ) (simparams.speed_step[ 0 ].rotorspeed) / ( double ) t_acc;
         qDebug() << "SimMdl:  accel-calc:  t1 t2 w2t t_acc speed rate" << t1 << t2 << w2t << t_acc
                  << simparams.speed_step[ 0 ].rotorspeed << rate;
   ...
   qDebug() << "SimMdl: ssck: rspeed accel1 lo_ss_acc" << rspeed << accel1 << lo_ss_acc << "tf_aend tf_scan" << tf_aend
            << tf_scan;
   //x0  1  2  3  4  5
  ...
  */
   ...
   qDebug() << "SimMdl: nthread" << nthread << "ncomp" << ncomp << "ntc" << ntc << "meshtype" << simparams.meshType;

   // Do simulation by several possible ways: 1-/Multi-thread, ASTFEM/ASTFVM
   //if ( nthread < 2 )
   if (nthread < 999) {
      if (
         model.components[ 0 ].sigma == 0.0 && model.components[ 0 ].delta == 0.0 && model.coSedSolute < 0.0
         && compress == 0.0) {
         qDebug() << "SimMdl: (fematch:)Finite Element Solver is called";
         //*DEBUG*
         for (int ii = 0; ii < model.components.size(); ii++) {
            qDebug() << "SimMdl:   comp" << ii << "s D v" << model.components[ ii ].s << model.components[ ii ].D
                     << model.components[ ii ].vbar20 << "  c" << model.components[ ii ].signal_concentration;
         }
      ...
      for (int ii = 0; ii < ncomp; ii++) {
         if (ii < nthread) { // First time through per thread:  get initial model and sim data
            tmodels << model;
      ...
      for (int ii = 0; ii < nthread; ii++) {
         ThreadWorker *tworker = new ThreadWorker(tmodels[ ii ], simparams, tsimdats[ ii ], solution_rec.buffer, ii);
         ...
         connect(tworker, SIGNAL(work_progress(int, int)), this, SLOT(thread_progress(int, int)));
         connect(tworker, SIGNAL(work_complete(int)), this, SLOT(thread_complete(int)));
...
bool US_ReporterGMP::modelGuidExistsForStage_ind(QString triple_n, QString model, QString mguid) {
   ...
   for (int i = 0; i < Array_of_tripleNames.size(); ++i) {
      ...
      qDebug() << "IN modelGuidExistsForStage_ind(): triple_n, c_triple_n, model  -- " << triple_n << c_triple_n
               << model;
      qDebug() << "IN modelGuidExistsForStage_ind(): mguid, Triple_to_ModelsDescGuid[ Array_of_tripleNames[ i ] ][ "
                  "model ] -- "
      ...
      if (c_triple_n == triple_n) {
         ...
         if (tmapguid[ model ] == mguid) {
            qDebug() << "For triple, stage: " << triple_n << model << ", there is modelGuid: " << mguid << " ("
...
void US_ReporterGMP::process_combined_plots_individual(QString triplesname_p, QString stage_model) {
   ...
   for (int ii = 0; ii < modelDescModified.size(); ii++) {
      ...
      //fiter by type|model
      if (
         modelDescModified[ ii ].contains(triplesname_mod) && modelDescModified[ ii ].contains(stage_model)
         ...
         QMap<QString, QString> ind_comboplots
            = perChanMask_edited.ShowTripleTypeModelRangeIndividualCombo[ triplesname ][ stage_model ];
         ...
         for (i_cp = ind_comboplots.begin(); i_cp != ind_comboplots.end(); ++i_cp) {
            ...
            qDebug() << "INDCOMBO_4: map's parms -- " << type_range << i_cp.value().toInt() << type << range;
         ...
         for (i_cpt = ind_compoplots_type_ranges.begin(); i_cpt != ind_compoplots_type_ranges.end(); ++i_cpt) {
            ...
            if (type == "s") {
               ...
               if (sim_ranges.contains("s_ranges")) {
                  ...
                  qDebug() << "s-type: sim_ranges.keys(), sim_ranges[\"s_ranges\"] -- " << sim_ranges.keys()
                           << sim_ranges[ "s_ranges" ];
               }
               ...
               CombPlotsParmsMap_Colors[ imgComb02File ]
                  = plotted_ids_colors_map_s_type[ plotted_ids_colors_map_s_type.firstKey() ];
            }
            //type: 1:MW
            else if (type == "MW") {
...
void US_ReporterGMP::process_combined_plots(QString filename_passed) {
   ...
   for (int m = 0; m < modelNames.size(); m++) {
      ...
      for (int ii = 0; ii < modelDescModified.size(); ii++) {
         //fiter by type|model
         if (
            modelDescModified[ ii ].contains(modelNames[ m ])
            ...
            plotted_ids_colors_map_s_type = sdiag_combplot->model_select_auto(
               modelDescModified[ ii ], c_params); //ALEXEY: here it plots s20 combPlot (xtype == 0)
         }
      }
      ...
      if (isModel) //TEMPORARY: will read a type-method combined plot QMap defined at the beginnig
      {
         ...
         bool show_combo_ff0 = (combPlotsMask_edited.ShowCombPlotParts[ "f/f0" ][ modelNames[ m ] ].toInt()) ? true
                                                                                                             : false;
         bool show_combo_MW = (combPlotsMask_edited.ShowCombPlotParts[ "MW" ][ modelNames[ m ] ].toInt()) ? true
                                                                                                          : false;
         ...
         for (int xt = 0; xt < xtype.size(); ++xt) {
            ...
            bool show_combo_plot_other_types
               = (combPlotsMask_edited.ShowCombPlotParts[ c_type ][ modelNames[ m ] ].toInt()) ? true : false;
...
void US_ReporterGMP::plot_pseudo3D(QString triple_name, QString stage_model) {
   ...
   //here identify what to show:
   bool show_s_ff0 = (perChanMask_edited
                         .ShowTripleModelPseudo3dParts[ t_name ][ stage_model ][ "Pseudo3d s-vs-f/f0 Distribution" ]
                         .toInt())
                        ? true
                        : false;
   bool show_s_d = (perChanMask_edited
                       .ShowTripleModelPseudo3dParts[ t_name ][ stage_model ][ "Pseudo3d s-vs-D Distribution" ]
                       .toInt())
                      ? true
                      : false;
   bool show_mw_ff0 = (perChanMask_edited
                          .ShowTripleModelPseudo3dParts[ t_name ][ stage_model ][ "Pseudo3d MW-vs-f/f0 Distribution" ]
                          .toInt())
                         ? true
                         : false;
   bool show_mw_d = (perChanMask_edited
                        .ShowTripleModelPseudo3dParts[ t_name ][ stage_model ][ "Pseudo3d MW-vs-D Distribution" ]
                        .toInt())
                       ? true
                       : false;
...
void US_ReporterGMP::calc_residuals() {
   ...
   for (int ii = 0; ii < scanCount; ii++) {
      ...
      for (int jj = 0; jj < dsize; jj++) { // Calculate the residuals and the RMSD
...
void US_ReporterGMP::assemble_run_details_html(void) {
   ...
   qDebug() << "[in assemble_run_details_html()], FileName,  fileNameList -- " << FileName << fileNameList;

   US_Passwd pw;
   US_DB2 *dbP = new US_DB2(pw.getPasswd());
   if (dbP->lastErrno() != US_DB2::OK) {
      QMessageBox::warning(
         this, tr("Connection Problem, run details"), tr("Could not connect to database: \n") + dbP->lastError());
      return;
   }
   ...
   for (int i = 0; i < fileNameList.size(); ++i) {
      qDebug() << "[in assemble_run_details_html()1], Filename -- " << fileNameList[ i ];

      if (fileNameList[ i ].endsWith("-IP"))
         continue;

      qDebug() << "[in assemble_run_details_html()2], Filename -- " << fileNameList[ i ];
      ...
      qDebug() << "[in assemble_run_details_html()], experimentID -- " << experimentID;

      if (experimentID == 0) {
         QMessageBox::warning(this, tr("ExpID zero, run details"), tr("Experiment ID zero!!!: \n"));
         return;
      }
      ...
      QString cksumd;
      QDateTime datedt;
      US_TimeState::dbExamine(dbP, &tmstID, &experimentID, &tfname, &xdefs, &cksumd, &datedt);

      qDebug() << "Assembling Run Details: expID tmstID tfname cksumd datedt" << experimentID << tmstID << tfname
               << cksumd << datedt;

      if (tmstID == 0) {
         QMessageBox::warning(this, tr("TimeStateID zero, run details"), tr("TimeState ID zero!!!: \n"));
         return;
      }
      ...
      QDir ddir(dirname);
      QStringList tmsfs = ddir.entryList(QStringList("*.time_state.*"), QDir::Files, QDir::Name);
      ...
      if (tmsfs.count() == 2) { // Looks like we have a TMST and corresponding Defs XML
         tmst_fnamei = tmsfs[ 0 ];
         defs_fnamei = tmsfs[ 1 ];
         qDebug() << "Assembling Run Details: tmsfs" << tmst_fnamei << defs_fnamei;

         if (tmst_fnamei.contains(".tmst") && defs_fnamei.contains(".xml")) { // Have both files, so save full path to TMST
            tmst_fnamei = tmst_dname + tmst_fnamei;
         }

         else if (
            tmsfs[ 0 ].contains(".xml")
            && tmsfs[ 1 ].contains(".tmst")) { // Have both files (in opposite order), so save full path to TMST
      ...
      html_assembled += table_row(
         tr("Parameter: "), tr("Target Value:"), tr("Tolerance:"), tr("Measured Value:"), tr("Std. Dev."),
         tr("PASSED ?"));
      ...
      for (int i = 0; i < dkeys.size(); ++i) {
         ...
         if (dkeys[ i ] == "Time") {
            val_target = currProto.rpSpeed.ssteps[ 0 ].duration
                         + ( double ) (currProto.rpSpeed.ssteps[ 0 ].speed / currProto.rpSpeed.ssteps[ 0 ].accel);
            qDebug() << "Assembling Run Details: exp. duration, speed, accel -- "
                     << currProto.rpSpeed.ssteps[ 0 ].duration << currProto.rpSpeed.ssteps[ 0 ].speed
            ...
            val_passed = (val_measured_sec >= (val_target * (1 - val_tol / 100.0))
                          && val_measured_sec <= (val_target * (1 + val_tol / 100.0)))
                            ? "YES"
                            : "NO";

            html_assembled += table_row(
               tr("Duration"), val_target_hh_mm, QString::number(val_tol) + "%", val_measured_hh_mm, "", val_passed);
         }
         ...
         if (dkeys[ i ].contains("Temperature")) {
            ...
            val_passed = (val_measured >= (val_target - val_tol) && val_measured <= (val_target + val_tol)) ? "YES"
                                                                                                            : "NO";

            val_measured_stdd = davgs_stdd_first_scan[ dkeys[ i ] ][ "Stdd" ];

            html_assembled += table_row(
               dkeys[ i ] + " (&#8451;)", QString::number(val_target), "&#177;" + QString::number(val_tol),
               QString::number(val_measured), QString::number(val_measured_stdd), val_passed);
         ...
         if (dkeys[ i ].contains("RawSpeed")) {
            ...
            html_assembled += table_row(
               "Speed (RPM)", QString::number(val_target), "&#177;" + QString::number(val_tol),
...
int US_ReporterGMP::get_expID_by_runID_invID(US_DB2 *dbP, QString runID_filename) {
   QStringList query;
   int idExp = 0;
   query << "get_experiment_info_by_runID" << runID_filename << QString::number(invID);
...
void US_ReporterGMP::assemble_user_inputs_html(void) {
   ...
   read_autoflowStatus_record(
      importRIJson, importRIts, importIPJson, importIPts, editRIJson, editRIts, editIPJson, editIPts, analysisJson,
      stopOptimaJson, stopOptimats, skipOptimaJson, skipOptimats, analysisCancelJson, createdGMPrunJson,
   ...
   QStringList dropped_triples_RI, dropped_triples_IP;
   read_reportLists_from_aprofile(dropped_triples_RI, dropped_triples_IP);
   qDebug() << "List of dropped triples (all OSs): " << dropped_triples_RI << dropped_triples_IP;
...
void US_ReporterGMP::user_interactions_analysis(QString analysisJson, QString analysisCancelJson) {
   ...
   {
      html_assembled += tr(
         "Meniscus positions have been determined automatically as best fit values for all channels.");
   }
...
void US_ReporterGMP::read_autoflowStatus_record(
   QString &importRIJson, QString &importRIts, QString &importIPJson, QString &importIPts, QString &editRIJson,
   QString &editRIts, QString &editIPJson, QString &editIPts, QString &analysisJson, QString &stopOptimaJson,
   QString &stopOptimats, QString &skipOptimaJson, QString &skipOptimats, QString &analysisCancelJson,
   QString &createdGMPrunJson, QString &createdGMPrunts, QString &analysisABDEJson, QString &analysisABDEts) {
   importRIJson.clear();
   importRIts.clear();
   importIPJson.clear();
   importIPts.clear();
   ...
   if (db.lastErrno() != US_DB2::OK) {
      QMessageBox::warning(
         this, tr("Connection Problem: Read autoflowStatus"), tr("Could not connect to database \n") + db.lastError());
      return;
   }
   ...
   qDebug() << "Read_autoflow_status: stopOptimaJson, skipOptimaJson, analysisJson -- " << stopOptimaJson
            << skipOptimaJson << analysisJson;
}
...
QString US_ReporterGMP::distrib_info_abde(QString &abde_channame) {
   ...
   if (mwl_abde) {
      mstr += "\n" + indent(2) + tr("<h3>Analytes and Buffer Used in MWL-Deconvolution:</h3>\n") + indent(2)
              + "<table>\n";
      ...
      mstr += table_row(tr("Residual RMS Deviation:"), QString::number(abde_rmsd[ abde_channame ]));
      mstr += indent(2) + "</table>\n";
   }
   ...
   if (mwl_abde)
      abde_dist_info
         = "Radial Pos., DNA Data (Norm.), Protein Data (Norm.), Integral DNA (Norm), Integral Protein (Norm.)";
   else
   ...
   for (int ii = 0; ii < xvals_size; ii++) {
      ...
      if (mwl_abde) {
         ...
         abde_dist_info = QString().sprintf("%10.4e", x_val) + ", " + QString().sprintf("%10.4e", y_dna_val) + ", "
   ...
   if (do_integration) {
      ...
      mstr += indent(2) + "<table>\n";
      mstr += table_row(
         tr("Type:"), tr("Range:"), tr("Fraction % from Model (target):"), tr("Tolerance, %:"), tr("PASSED ?"));
      for (int kk = 0; kk < report_items_number; ++kk) {
         US_ReportGMP::ReportItem curr_item = reportGMP->reportItems[ kk ];
         QString type = curr_item.type;
         QString method = curr_item.method;
         ...
         qDebug() << "In distrib_info(), fill simulated integration vals: for chann/wvl/type/method/low/high, "
                  << "Inter. val. Sim -- " << wvl_abde << curr_item.type << curr_item.method << curr_item.range_low
                  << curr_item.range_high << int_val_m;

         mstr += table_row(
            type, range, QString().sprintf("%5.2f%%", frac_tot_m) + " (" + QString::number(frac_tot_r) + "%)",
...
QMap<QString, QString> US_ReporterGMP::get_channels_analytes_mwl_abde(QString abde_channame) {
   ...
   if (db.lastErrno() != US_DB2::OK) {
      QMessageBox::warning(this, tr("Database Problem"), tr("Database returned the following error: \n") + db.lastError());
   ...
   for (int ii = 0; ii < currProto.rpRange.nranges; ii++) {
      ...
      channel_s.replace(" / ", "");
      qDebug() << "[in get_channels_analytes_mwl_abde()], channel, channel_s, abde_channame;" << channel << channel_s
               << abde_channame;
      ...
      if (channel_s == abde_channame) {
         ...
         if (nwavl > 1 && mwl_deconv) {
            ...
            if (status == US_DB2::NO_BUFFER) {
               ...
               QMessageBox::warning(
                  this, tr("Database Problem"), tr("Database returned the following error: \n") + db.lastError());
               return analytes_buffer_map;
            }
            ...
            for (int i = 0; i < num_analytes; ++i) {
               ...
               qDebug() << "[GMP REPORT] Solution " << solution->solutionDesc << ", (GUID)Analyte " << "(" << a_GUID
                        << ")" << a_name << ", (ID)Analyte " << "(" << a_ID << ")" << a_name;
            ...
            if (buff_req) //only if buffer spectrum required
            {
               ...
               qDebug() << "[GMP REPORT] Solution " << solution->solutionDesc << ", (ID)Buffer " << "(" << b_ID << ")"
                        << b_name;
...
void US_ReporterGMP::assemble_plots_html(QStringList PlotsFilenames, const QString plot_type) {
   ...
   for (int i = 0; i < PlotsFilenames.size(); ++i) {
      ...
      qreal qprinters_width
         = pageSize.width()
           * 0.75; //500 DEPENDS on QPrinter's constructor settings {QPrinter::PrinterResolution, 500; QPrinter::HighResolution, 9066}
      qreal qprinters_hight = pageSize.height() * 0.55;
      qDebug() << "qprinters_width, height [orig, scaled]: " << pageSize.width() << pageSize.height() << "; "
	 ...
	 <img style='max-height: 50%; max-width: 100%;'/>
      ...
      if (!plot_type.isEmpty() && plot_type == "CombPlots") // For Combined plots, scale down .png
      {
         ...
         html_assembled += " \"height=\"" + QString::number(scaled_i_height_cp) + "\"width=\""
                           + QString::number(scaled_i_width_cp);
      }
         ...
         html_assembled += " \"height=\"" + QString::number(scaled_rd_height) + "\"width=\""
                           + QString::number(scaled_rd_width);
      }
      else {
         html_assembled += " \"height=\"" + QString::number(scaled_i_height) + "\"width=\""
                           + QString::number(scaled_i_width);
      }

      qDebug() << "Image size html string: "
               << "\"height=\"" + QString::number(scaled_i_height) + "\"width=\"" + QString::number(scaled_i_width);
      ...
      if (!plot_type.isEmpty() && plot_type == "CombPlots") {
         ...
         qDebug() << "COMBOPLOT-parms/colors SZIES: combparms.size(), combparms_colors.size() --  " << combparms.size()
...
double US_ReporterGMP::interp_sval(double xv, double *sx, double *sy, int ssize) {
   for (int jj = 1; jj < ssize; jj++) {
      ...
      if (xv < sx[ jj ]) { // given x lower than array x: interpolate between point and previous
         int ii = jj - 1;
...
QString US_ReporterGMP::text_model(US_Model model, int width) {
   ...
   if (width != 0) { // long title:  add any suffixes and check need to center
      ...
      if (width > title.length()) { // long title centered:  center it in fixed-length string
...
QString US_ReporterGMP::html_header(QString title, QString head1, US_DataIO::EditedData *edata) {
...
QString US_ReporterGMP::html_header_abde(QString title, QString runName, QString abde_chanName) {
   qDebug() << "[in html_header_abde()], title, runName, abde_chanName -- " << title << runName << abde_chanName;
   ...
   for (chw = channdesc_to_overlapping_wvls.begin(); chw != channdesc_to_overlapping_wvls.end(); ++chw) {
      ...
      for (int j = 0; j < unique_wvls.size(); ++j) {
         ...
         html_str_replicate_av += table_row(
            tr("Type:"), tr("Method:"), tr("Range:"), tr("Av. Integration, Model (target):"), tr("St. Dev.:"),
            tr("Av. Fraction %, Model (target):"), tr("St. Dev.:"), tr("Tol.%:"), tr("PASSED?"));
         ...
         for (int kk = 0; kk < report_items_number; ++kk) {
            ...
            QMap<QString, double> replicate_g_results
               = get_replicate_group_results(ref_group_item, u_wvl, same_wvls_chann_map[ u_wvl ]);
...
QMap<QString, double> US_ReporterGMP::get_replicate_group_results(
   US_ReportGMP::ReportItem ref_report_item, QString u_wvl, QStringList channs_for_wvl) {
   ...
   for (int i = 0; i < channs_for_wvl.size(); ++i) //over channels for a given wvl in Replicate group:
   {
      ...
      for (int kk = 0; kk < report_items_number; ++kk) {
         ...
         if (ref_type == _type && ref_method == _method && ref_low == _low && ref_high == _high && ref_int_val == _int_val) {
            //can add an average fraction percent from model (if needed)

            qDebug() << "For Triple: " << channs_for_wvl[ i ] << "." << u_wvl << ", Type/Method: " << _type << ": "
                     << _method << ", Range: " << "[" << _low << " - " << _high << "]"
...
QString US_ReporterGMP::distrib_info(QMap<QString, QString> &tripleInfo) {
   ...
   qDebug() << "Distrib_info(): Model Name, time created, editUpdated -- " << model.description << model.timeCreated
   ...
   QString mdla = model_used.description.section(".", -2, -2).section("_", 1, -1);
   if (mdla.isEmpty())
   ...
   mstr += table_row(tr("Model-reported RMSD:"), (rmsd_m > 0.0) ? QString::number(rmsd_m) : "(none)");
   ...
   mstr += table_row(tr("Weight Average s20,W:"), QString().sprintf("%6.4e", (sum_s / sum_c)));
   mstr += table_row(tr("Weight Average D20,W:"), QString().sprintf("%6.4e", (sum_D / sum_c)));
   mstr += table_row(tr("W.A. Molecular Weight:"), QString().sprintf("%6.4e", (sum_mw / sum_c)));
   if (!cnstff)
      mstr += table_row(tr("Weight Average f/f0:"), QString::number((sum_k / sum_c)));
   if (!cnstvb)
      mstr += table_row(tr("Weight Average vbar20:"), QString::number((sum_v / sum_c)));
   mstr += table_row(tr("Total Concentration:"), QString().sprintf("%6.4e", sum_c));
   ...
   if (cnstvb) { // Normal constant-vbar distribution
      ...
      for (int ii = 0; ii < ncomp; ii++) {
         ...
         model_dist_info = QString().sprintf("%10.4e", model_used.components[ ii ].mw) + ", "
   ...
   */
   ...
   if (model_used.associations.size() > 0) {
      ...
      mstr += indent(2) + "<table>\n";
      mstr += table_row(tr("Reactant 1"), tr("Reactant 2"), tr("Product"), tr("K_dissociation"), tr("k_off Rate"));
      ...
      for (int ii = 0; ii < model_used.associations.size(); ii++) {
         ...
         mstr += table_row(reac1, reac2, prod, QString().sprintf("%10.4e", k_d), QString().sprintf("%10.4e", k_off));
      }
   }
   ...
   QString tot_conc_passed = (sum_c >= (tot_conc_r * (1 - tot_conc_tol_r / 100.0))
                              && sum_c <= (tot_conc_r * (1 + tot_conc_tol_r / 100.0)))
                                ? "YES"
                                : "NO";
   QString exp_dur_passed = (last_f >= (exp_dur_r * (1 - exp_dur_tol_r / 100.0))
   ...
   QMap<QString, QString> Model_parms_to_compare
      = perChanMask_edited.ShowTripleModelParts[ t_name ][ model_name ]; //2A660 => 2AInterference,
   ...
   if (show_comparison_section) {
      ...
      mstr
         += table_row(tr("Parameter: "), tr("Target Value:"), tr("Tolerance, %:"), tr("Measured Value:"), tr("PASSED ?"));

      if (show_tot_conc) {
         mstr += table_row(
            tr("Total Concentration"),
            QString::number(tot_conc_r) + " (factor " + QString::number(tot_conc_r_factor) + ")",
            QString::number(tot_conc_tol_r) + "%", QString().sprintf("%6.4e", sum_c), tot_conc_passed);
      }

      if (show_rmsd) {
         mstr += table_row(tr("RMSD limit"), QString::number(rmsd_r), QString(""), rmsd_global, rmsd_passed);
      }

      if (show_exp_dur) {
         mstr += table_row(
            tr("Experiment Duration"), exp_dur_r_hh_mm, QString::number(exp_dur_tol_r) + "%", wks, exp_dur_passed);
      }
      ...
      if (show_min_int && RIdata) {
         mstr += table_row(
            tr("Minimum Intensity"), QString::number(av_int_r), QString(""), QString::number(av_int_exp), av_int_passed);
      }

      if (show_loading_vol) {
         mstr += table_row(
            tr("Loading Volume"), QString::number(loading_volume_r), QString::number(loading_volume_tol_r) + "%",
            QString::number(loading_volume_measured), loading_volume_passed);
      }
   ...
   if (show_integration && method_type_combo_exists) {
      ...
      mstr += indent(2) + "<table>\n";
      mstr += table_row(
         tr("Type:"), tr("Range:"), tr("weight Avg.:"), tr("Integration from Model (target):"),
         tr("Fraction % from Model (target):"), tr("Tolerance, %:"), tr("PASSED ?"));
      for (int kk = 0; kk < report_items_number; ++kk) {
         US_ReportGMP::ReportItem curr_item = reportGMP->reportItems[ kk ];
         QString type = curr_item.type;
         QString method = curr_item.method;
         ...
         if (mdla.contains(method)) {
            ...
            qDebug() << "In distrib_info(), fill simulated integration vals: for chann/wvl/type/method/low/high, "
                     << "Inter. val. Sim -- " << wvl << curr_item.type << curr_item.method << curr_item.range_low
                     << curr_item.range_high << int_val_m;
...
double US_ReporterGMP::get_loading_volume(int cellID) {
   ...
   for (int i = 0; i < currProto.rpCells.used.size(); ++i) {
      if (currProto.rpCells.used[ i ].cell == cellID) {
         ...
         qDebug() << "In get_loading_volume(): cell, cenerpiece -- " << currProto.rpCells.used[ i ].cell
   ...
   qDebug() << "In get_loading_volume(): c_name, c_bottom, c_pathlength, c_angle -- " << c_name << c_bottom
            << c_pathlength << c_angle;
...
QString US_ReporterGMP::table_row(const QString &s1, const QString &s2, const QString &s3) const {
   return (indent(6) + "<tr><td>" + s1 + "</td><td>" + s2 + "</td><td>" + s3 + "</td></tr>\n");
}

// Table row HTML with 4 columns
QString US_ReporterGMP::table_row(const QString &s1, const QString &s2, const QString &s3, const QString &s4) const {
   return (indent(6) + "<tr><td>" + s1 + "</td><td>" + s2 + "</td><td>" + s3 + "</td><td>" + s4 + "</td></tr>\n");
}

// Table row HTML with 5 columns
QString US_ReporterGMP::table_row(
   const QString &s1, const QString &s2, const QString &s3, const QString &s4, const QString &s5) const {
   return (
      indent(6) + "<tr><td>" + s1 + "</td><td>" + s2 + "</td><td>" + s3 + "</td><td>" + s4 + "</td><td>" + s5
      + "</td></tr>\n");
}

// Table row HTML with 6 columns
QString US_ReporterGMP::table_row(
   const QString &s1, const QString &s2, const QString &s3, const QString &s4, const QString &s5,
...
QString US_ReporterGMP::table_row(
   const QString &s1, const QString &s2, const QString &s3, const QString &s4, const QString &s5, const QString &s6,
   const QString &s7) const {
   return (
      indent(6) + "<tr><td>" + s1 + "</td><td>" + s2 + "</td><td>" + s3 + "</td><td>" + s4 + "</td><td>" + s5
      + "</td><td>" + s6 + "</td><td>" + s7 + "</td></tr>\n");
}
...
QString US_ReporterGMP::table_row(
   ...
   return (
      indent(6) + "<tr><td>" + s1 + "</td><td>" + s2 + "</td><td>" + s3 + "</td><td>" + s4 + "</td><td>" + s5
      + "</td><td>" + s6 + "</td><td>" + s7 + "</td><td>" + s8 + "</td></tr>\n");
}

// Table row HTML with 9 columns
QString US_ReporterGMP::table_row(
   const QString &s1, const QString &s2, const QString &s3, const QString &s4, const QString &s5, const QString &s6,
   const QString &s7, const QString &s8, const QString &s9) const {
   return (
      indent(6) + "<tr><td>" + s1 + "</td><td>" + s2 + "</td><td>" + s3 + "</td><td>" + s4 + "</td><td>" + s5
      + "</td><td>" + s6 + "</td><td>" + s7 + "</td><td>" + s8 + "</td><td>" + s9 + "</td></tr>\n");
}
...
void US_ReporterGMP::plotres(QMap<QString, QString> &tripleInfo) {
   ...
   bool show_3d = false;
   bool show_d_mw = (perChanMask_edited.ShowTripleModelPlotParts[ t_name ][ s_name ][ "D-vs-MW 2D Model" ].toInt())
                       ? true
                       : false;
   bool show_d_s = (perChanMask_edited.ShowTripleModelPlotParts[ t_name ][ s_name ][ "D-vs-s 2D Model" ].toInt())
                      ? true
                      : false;
   bool show_dd
      = (perChanMask_edited.ShowTripleModelPlotParts[ t_name ][ s_name ][ "Diffusion Coefficient Distribution" ].toInt())
           ? true
           : false;
   bool show_res
      = (perChanMask_edited.ShowTripleModelPlotParts[ t_name ][ s_name ][ "Experiment-Simulation Residuals" ].toInt())
           ? true
           : false;
   ;
   bool show_vel
      = (perChanMask_edited
            .ShowTripleModelPlotParts[ t_name ][ s_name ][ "Experiment-Simulation Velocity Data (noise corrected)" ]
            .toInt())
           ? true
           : false;
   bool show_mw
      = (perChanMask_edited.ShowTripleModelPlotParts[ t_name ][ s_name ][ "Molecular Weight Distribution" ].toInt())
           ? true
           : false;
   bool show_sd = (perChanMask_edited
                      .ShowTripleModelPlotParts[ t_name ][ s_name ][ "Sedimentation Coefficient Distribution" ]
                      .toInt())
                     ? true
                     : false;
   bool show_f_mw = (perChanMask_edited.ShowTripleModelPlotParts[ t_name ][ s_name ][ "f/f0-vs-MW 2D Model" ].toInt())
                       ? true
                       : false;
   bool show_f_s = (perChanMask_edited.ShowTripleModelPlotParts[ t_name ][ s_name ][ "f/f0-vs-s 2D Model" ].toInt())
                      ? true
                      : false;
   ...
   //Debug
   qDebug() << "triple_name, stage_n: " << tripleInfo[ "triple_name" ] << tripleInfo[ "stage_name" ];
   ...
   plotLayout1 = new US_Plot(
      data_plot1, tr("Residuals"), tr("Radius (cm)"), tr("OD Difference"), true, "^resids [0-9].*", "rainbow");

   plotLayout2
      = new US_Plot(data_plot2, tr("Velocity Data"), tr("Radius (cm)"), tr("Absorbance"), true, ".*in range", "rainbow");
...
void US_ReporterGMP::distrib_plot_stick(int type) {
   ...
   for (int jj = 0; jj < dsize; jj++) {
      xval = (type == 0) ? model_used.components[ jj ].s
                         : ((type == 1) ? model_used.components[ jj ].mw : model_used.components[ jj ].D);
...
void US_ReporterGMP::distrib_plot_2d(int type) {
   ...
   for (int jj = 0; jj < dsize; jj++) {
      xval = ((type & 1) == 1) ? model_used.components[ jj ].s : model_used.components[ jj ].mw;
...
bool US_ReporterGMP::mkdir(const QString &baseDir, const QString &subdir) {
   ...
   QMessageBox::warning(this, tr("File error"), tr("Could not create the directory:\n") + baseDir + "/" + subdir);
...
void US_ReporterGMP::write_plot(const QString &filename, const QwtPlot *plot) {
   if (filename.contains(".svg")) { // Save an SVG file and a PNG copy
      if (US_GuiUtil::save_plot(filename, plot) != 0)
         QMessageBox::warning(this, tr("File Write Error"), tr("Unable to write file") + filename);
   }
...
void US_ReporterGMP::thread_complete(int thr) {
   ...
   if (thrdone >= nthread) { // All threads are done, so sum thread simulation data
...
void US_ReporterGMP::adjustModel() {
   ...
   if (cnstvb && mc_vbar != sd.vbar20 && mc_vbar != 0.0) { // Use vbar from the model component, instead of from the solution
      sd.vbar20 = mc_vbar;
   ...
   for (int jj = 0; jj < model.components.size(); jj++) {
      ...
      sc->s /= scorrec;
      sc->D /= dcorrec;
      qDebug() << "Fem:Adj:  s20w D20w" << s20w << d20w << "s D" << sc->s << sc->D << "  jj" << jj << "vb20 vb"
               << sc->vbar20 << sd.vbar;
...
void US_ReporterGMP::assemble_pdf(QProgressDialog *progress_msg) {
   ...
   QString html_title = tr("<h1 align=center>%1 Report for Run: <br><i>%2</i></h1>"
                           "<hr>")
                           .arg(report_type) //1
                           .arg(runName) //2
      ;
   ...
   if (!str_failed_stage_missing_models.isEmpty()) {
      html_failed = tr("<h3 style=\"color:red;\" align=left> ATTENTION: Analyses for Some Triples Failed, or Models "
                       "are Missing!</h3>"
                       "%1"
                       "<hr>")
   ...
   html_operator = tr("<h3 align=left>Optima Machine/Operator </h3>"
                      "<table>"
   ...
   html_speed = tr("<h3 align=left>Speed Parameters </h3>"
                   "<table>"
   ...
   if (has_uv) {
      html_speed += tr("<tr><td><br><b><i>UV-visible optics (total):</i></b>   </td>  "
      ...
      html_speed += tr("<tr><td><br><b><i>UV-visible optics:</i></b>   </td>  "
                       "<tr><td>Not Used  </td></tr>");
   }

   if (has_int) {
      html_speed += tr("<tr><td><br><b><i>Interference optics (per cell):</i></b></td>   "
      ...
      html_speed += tr("<tr><td><br><b><i>Interference optics:</i></b></td>   "
                       "<tr><td>Not Used </td></tr>");
   }
   ...
   html_cells += tr("# of Used Cells:  %1<br>").arg(QString::number(ncells_used)) //1
      ;
   ...
   for (int i = 0; i < ncells_used; ++i) {
      ...
      if (i == ncells_used - 1 && currProto.rpCells.used[ i ].cbalance.contains(tr("counterbalance"))) {
   ...
   html_solutions += tr("# of Solution Channels:  %1 <br>").arg(nsol_channels) //1
      ;
   ...
   nchan_optics = currProto.rpOptic.nochan;
   html_optical += tr("# of Channels With Optics:  %1<br>").arg(QString::number(nchan_optics)) //1
      ;
   ...
   for (int i = 0; i < nchan_optics; ++i) {
      ...
      if (!scan1.isEmpty()) {
         has_uvvis = true;
         html_optical += tr("<td> %1: </td> &nbsp;&nbsp;&nbsp;&nbsp;").arg(scan1); //1
      }
      else
      ...
      if (!scan2.isEmpty()) {
         ...
         html_optical += tr("<td> %1: </td> &nbsp;&nbsp;&nbsp;&nbsp;").arg(scan2); //1
      }
      else
         html_optical += tr("<td> &nbsp;&nbsp;&nbsp;&nbsp; </td>");
      ...
      if (!scan3.isEmpty()) {
         ...
         html_optical += tr("<td> %1: </td> &nbsp;&nbsp;&nbsp;&nbsp;").arg(scan3); //1
      }
      else
         html_optical += tr("<td> &nbsp;&nbsp;&nbsp;&nbsp; </td>");
   ...
   html_ranges += tr("# of Channels With Ranges:  %1<br>").arg(QString::number(nchan_ranges)) //1
      ;
   ...
   for (int i = 0; i < nchan_ranges; ++i) {
      ...
      if (w_count > 1) {
         html_ranges += tr("<tr><td> Selected Wavelength range: </td>  <td> %1 </td> </tr>").arg(w_range) //1
            ;
      }

      html_ranges += tr("<tr><td> Radius range:              </td>  <td> %1 </td> </tr>"
   ...
   //SCAN_COUNT: begin
   html_scan_count = tr("<h3 align=left> Scan Counts and Scan Intervals For Optics in Use </h3>"
                        "&nbsp;&nbsp;<br>");
   ...
   QString html_scan_count_uv = tr("<table>"
                                   ...
                                   "</tr>"
                                   "</table>");
   html_scan_count_uv += tr("<table style=\"margin-left:30px\">"
   ...
   //Interference
   QString html_scan_count_int = tr("<table>"
                                    "<tr>"
                                    "<td><b><i>Interference optics:</i></b></td>"
                                    "</tr>"
                                    "</table>");
   html_scan_count_int += tr("<table style=\"margin-left:30px\">"
                             "<tr><td> Scan Interval:             </td>  <td> %1 </td> </tr>"
   ...
   html_analysis_profile = tr("<h3 align=left> Analysis Profile: General Settings and Reports  </h3>"
                              "&nbsp;&nbsp;<br>");
   ...
   for (int i = 0; i < nchna; i++) {
      ...
      if (triple_report) {
         ...
         for (int jj = 0; jj < chann_wvl_number; ++jj) {
            ...
            QString exp_dur_str = QString::number(hms_tot[ 0 ]) + "d " + QString::number(hms_tot[ 1 ]) + "h "
            ...
            if (genMask_edited.ShowAnalysisGenParts[ "Report Parameters (per-triple)" ].toInt()) {
               ...
               if (expType == "VELOCITY") {
                  html_analysis_profile += tr("<table style=\"margin-left:70px\">"
            ...
            for (int kk = 0; kk < report_items_number; ++kk) {
               ...
               if (genMask_edited.ShowAnalysisGenParts[ "Report Item Parameters (per-triple)" ].toInt()) {
                  ...
                  if (expType == "VELOCITY") {
                     html_analysis_profile += tr("<table style=\"margin-left:110px\">"
   ...
   html_analysis_profile_2dsa = tr("<h3 align=left> Analysis Profile: 2DSA Controls </h3>"
                                   "&nbsp;&nbsp;<br>");
   ...
   html_analysis_profile_2dsa_flow += tr("<table>"
                                         ...
                                         "</tr>"
                                         "</table>");
   html_analysis_profile_2dsa_flow += tr("<table style=\"margin-left:10px\">"
                                         //2DSA
                                         "<tr>"
                                         "<td> \"2DSA  (TI Noise)\": </td>"
                                         "<td> Run?                  </td>"
   ...
   html_analysis_profile_2dsa_per_channel += tr("<table>"
                                                "<tr>"
   ...
   for (int i = 0; i < nchna_2dsa; i++) {
      ...
      QString s_data = QString::number(cAP2.parms[ i ].s_min) + ", " + QString::number(cAP2.parms[ i ].s_max) + ", "
                       + QString::number(cAP2.parms[ i ].s_grpts);

      QString ff0_data = QString::number(cAP2.parms[ i ].k_min) + ", " + QString::number(cAP2.parms[ i ].k_max) + ", "
      ...
      html_analysis_profile_2dsa_per_channel += tr("<table style=\"margin-left:30px\">"
   ...
   html_analysis_profile_pcsa = tr("<h3 align=left> Analysis Profile: PCSA Controls </h3>"
                                   "&nbsp;&nbsp;<br>");

   QString html_analysis_profile_pcsa_flow;
   html_analysis_profile_pcsa_flow += tr("<table style=\"margin-left:10px\">"
                                         //PCSA
                                         "<tr>"
                                         "<td> \"PCSA\":    </td>"
                                         "<td> Run?         </td>"
   ...
   if (cAPp.job_run) {
      ...
      for (int i = 0; i < nchna_pcsa; i++) {
         ...
         QString x_data = cAPp.parms[ i ].x_type + ", " + QString::number(cAPp.parms[ i ].x_min) + ", "
                          + QString::number(cAPp.parms[ i ].x_max);
         QString y_data = cAPp.parms[ i ].y_type + ", " + QString::number(cAPp.parms[ i ].y_min) + ", "
                          + QString::number(cAPp.parms[ i ].y_max);
         QString z_data = cAPp.parms[ i ].z_type + ", " + QString::number(cAPp.parms[ i ].z_value);

         html_analysis_profile_pcsa_per_channel += tr("<table style=\"margin-left:30px\">"
...
void US_ReporterGMP::paintPage(
   QPrinter &printer, int pageNumber, int pageCount, QPainter *painter, QTextDocument *doc, const QRectF &textRect,
   qreal footerHeight) {
   ...
   qDebug() << "Painter's settings: font, metrics -- " << painter->fontInfo().family()
            << painter->fontInfo().pointSizeF() << painter->fontInfo().pointSize();
   ...
   qDebug() << "Paint page: painter: pixelSize(), pointSize() -- " << pfont.pixelSize() << pfont.pointSize();
   ...
   //end test
   painter->drawText(
      footerRect, Qt::AlignVCenter | Qt::AlignRight, QObject::tr("Page %1/%2").arg(pageNumber + 1).arg(pageCount));
...
void US_ReporterGMP::write_gmp_report_DB(QString filename, QString filename_pdf) {
   ...
   qry.clear();
   qry << "new_autoflow_gmp_report_record" << AutoflowID_auto << FullRunName_auto << ProtocolName_auto << filename_pdf;
   ...
   qDebug() << "state_new_gmpReport, autolfowGMPReportID -- " << state_new_gmpReport << autolfowGMPReportID;
   ...
   if (state_new_gmpReport == US_DB2::OK && autolfowGMPReportID > 0) {
      ...
      if (writeStatus == US_DB2::DBERROR) {
         ...
         QMessageBox::warning(
            this, "Error", "returned processing file:\n" + report_filepath + "\n" + db.lastError() + "\n");
      ...
      if (clear_GMP_report_record) {
         ...
         qry.clear();
         qry << "update_autoflowStatusReport_record" << autoflowStatusID << AutoflowID_auto << reporting_Json;
   ...
   foreach (const QString &key, json.keys()) {
      ...
      if (key.contains("Solutions") || key.contains("Analysis")) {
         ...
         for (int i = 0; i < json_array.size(); ++i) {
            foreach (const QString &array_key, json_array[ i ].toObject().keys()) {
               if (key.contains("Solutions")) {
                  qDebug() << "Parse_editedJsonGen: Solution: array_key, value: " << array_key
                           << json_array[ i ].toObject().value(array_key).toString();
               ...
               if (key.contains("Analysis")) {
                  ...
                  foreach (const QString &n_key, newObj.keys()) {
                     ...
                     if (array_key.contains("General")) {
                        ...
                        qDebug() << "Parse_editedJsonGen: Analysis:Gen: n_key, value: " << n_key
                     ...
                     if (array_key.contains("2DSA")) {
                        ...
                        qDebug() << "Parse_editedJsonGen: Analysis:2DSA: n_key, value: " << n_key
                     ...
                     if (array_key.contains("PCSA")) {
                        ...
                        qDebug() << "Parse_editedJsonGen: Analysis:PCSA: n_key, value: " << n_key
...
void US_ReporterGMP::parse_edited_perChan_mask_json_abde(
   ...
   foreach (const QString &key, json.keys()) //over channels
   {
      ...
      foreach (const QString &n_key, tripleObj.keys()) //over single channel!
      {
         ...
         qDebug() << "channel's " << key << " feature: " << n_key << ", value: " << feature_value;
               ...
               foreach (const QString &j_key, modelObj.keys()) //over models' features (tot_conc, RMSD...)
               {
                  ...
                  if (feature_value.isString()) //deal with plots main characteristics ( tot_conc, RMSD)
                  {
                     ...
                     qDebug() << "Parse_editedJsonTriples: triple_name: " << triple_name << ": "
                              << "model_name, j_key, feature_value: " << model_name << j_key
                     ...
                     if (j_key.contains("Plots")) {
                        ...
                        foreach (const QString &p_key, plotObj.keys()) //over plots, per model/per triple/per channel
                        {
                           ...
                           qDebug() << "Parse_editedJsonTriples: triple_name: " << triple_name << ": "
                                    << "model_name, j_key, p_key, feature_PLOT_value: " << model_name << j_key << p_key
                                    << feature_plot_value;
                        }
                     }
                     //end of "Plots"
                     ...
                     //begin "Pseudo3D"
                        ...
                        foreach (const QString &p_key, pseudo3dObj.keys()) //over pseudo3d, per model/per triple/per channel
                        {
                           ...
                           QString model_name = n_key.split(" ")[ 0 ];
                           MaskStr.ShowTripleModelPseudo3dParts[ triple_name ][ model_name ][ p_key ]
                           ...
                           qDebug() << "Parse_editedJsonTriples: triple_name: " << triple_name << ": "
                                    << "model_name, j_key, p_key, feature_PSEUDO_value: " << model_name << j_key
                                    << p_key << feature_pseudo3d_value;
                        }
                     }
                     //end of "Pseudo3D"
                     ...
                     //begin "Individual"
                        ...
                        foreach (const QString &p_key, indComboObj.keys()) //over type[ranges], per model/per triple/per channel
                        {
                           ...
                           MaskStr.ShowTripleTypeModelRangeIndividualCombo[ triple_name ][ model_name ][ p_key ]
                              = feature_indCombo_value;
                           if (MaskStr.ShowTripleTypeModelRangeIndividualCombo[ triple_name ][ model_name ][ p_key ]
                                  .toInt())
                              ++MaskStr.has_tripleModelIndCombo_items[ triple_name ][ model_name ];

                           qDebug() << "Parse_editedJsonTriples: triple_name: " << triple_name << ": "
                                    << "model_name, j_key, p_key, feature_INDCOMBO_value: " << model_name << j_key
                                    << p_key << feature_indCombo_value;
                        }
                     }
                     //end of "Individual"
                  }
   ...
   foreach (const QString &key, json.keys()) //over types
   {
      ...
      for (int i = 0; i < json_array.size(); ++i) {
         foreach (const QString &array_key, json_array[ i ].toObject().keys()) //over models (aka methods)
         {
            ...
            qDebug() << "CombPlots: type, model, yes/no -- " << key << array_key_mod << show_plot.toString();
...
void US_ReporterGMP::format_needed_params() {
   ...
   duration_str = QString::number(hms_dur[ 0 ]) + "d " + QString::number(hms_dur[ 1 ]) + "h "
   ...
   US_RunProtocol::timeToList(total_time, hms_tot);
   total_time_str = QString::number(hms_tot[ 0 ]) + "d " + QString::number(hms_tot[ 1 ]) + "h "
                    + QString::number(hms_tot[ 2 ]) + "m ";
   ...
   delay_uvvis_str = QString::number(hms_delay_uvvis[ 0 ]) + "d " + QString::number(hms_delay_uvvis[ 1 ]) + "h "
   ...
   scanint_uvvis_str = //QString::number( hms_scanint_uvvis[ 0 ] ) + "d " +
      QString::number(hms_scanint_uvvis[ 1 ]) + "h " + QString::number(hms_scanint_uvvis[ 2 ]) + "m "
   ...
   delay_int_str = QString::number(hms_delay_int[ 0 ]) + "d " + QString::number(hms_delay_int[ 1 ]) + "h "
   ...
   scanint_int_str = //QString::number( hms_scanint_uvvis[ 0 ] ) + "d " +
      QString::number(hms_scanint_int[ 1 ]) + "h " + QString::number(hms_scanint_int[ 2 ]) + "m "
...
void US_ReporterGMP::add_solution_details(const QString sol_id, const QString sol_comment, QString &html_solutions) {
   ...
   html_solutions += tr("<table style=\"margin-left:5px\">"
   ...
   if (genMask_edited.ShowSolutionParts[ "Analyte Information" ].toInt()) {
      html_solutions += tr("<table style=\"margin-left:20px\">"
   ...
   for (int i = 0; i < num_analytes; ++i) {
      ...
      if (seqlen == 0)
         ...
         if (seqlen > 25) {
            seqsmry = QString(seqsmry).left(10) + " ... " + QString(seqsmry).mid(seqlen - 10);
         }
         //seqsmry          += "\n  ";
         seqsmry += "<br>  ";
         ...
         for (int ii = 0; ii < 26; ii++) {
            ...
            total += lcount;
            if (lcount > 0) {
               seqsmry += QString().sprintf("%d", lcount) + " " + letter.toUpper() + ", ";
               //seqsmry          += "\n  ";
            }
         }
      ...
      analyte_detailed_info += tr("<tr> <td> Type: </td> <td>%1</td>              </tr>"
      ...
      if (analyte.type == 0) {
         analyte_detailed_info += tr("<tr> <td> AAs absorbing at 280 nm: </td> <td>%1</td> </tr>"
   ...
   buffer_detailed_info += tr("<tr><td> Density (20&#8451;, g/cm<sup>3</sup>):  </td>   <td>%1</td>  </tr>"
   ...
   for (int i = 0; i < buffer.component.size(); ++i) {
      QString component_desc = buffer.component[ i ].name + " (" + QString::number(buffer.concentration[ i ]) + " "
                               + buffer.component[ i ].unit + ")";


      buffer_components_info += tr("<tr><td> Component Name:</td> &nbsp;&nbsp;  <td>%1</td>  </tr>").arg(component_desc);
   }
