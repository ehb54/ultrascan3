...
{
  QString report_filepath = "/home/alexey/ultrascan/reports/"
                            "eGFP-DNA-MW-08OCT23-run1981_GMP_DB.tar";
  QString html_filePath = "/home/alexey/ultrascan/reports/"
                          "eGFP-DNA-MW-08OCT23-run1981/html_string.html";
  int autolfowGMPReportID = 25;

  QString autoStatusID = QString::number (231);
  QString autoID = QString::number (1002);

  write_gmp_report_DB_test (report_filepath, html_filePath,
                            autolfowGMPReportID, autoStatusID, autoID);
}

// write GMP report to DB Test
void
US_ReporterGMP::write_gmp_report_DB_test (QString report_filepath,
                                          QString html_filePath,
                                          int autolfowGMPReportID,
                                          QString autoStatusID, QString autoID)
{
  ...
  int writeStatus_html
      = db.writeBlobToDB (html_filePath, QString ("upload_gmpReportData_html"),
                          autolfowGMPReportID);

  if (writeStatus_html == US_DB2::DBERROR)
    {
      QMessageBox::warning (this, "Error",
                            "Error processing html file:\n" + html_filePath
                                + "\n" + db.lastError () + "\n"
  ...
  int writeStatus = db.writeBlobToDB (
      report_filepath, QString ("upload_gmpReportData"), autolfowGMPReportID);
  ...
  if (clear_GMP_report_record)
    {
      ...
      // qry.clear();
      // qry << "clear_autoflowGMPReportRecord" << QString::number(
      // autolfowGMPReportID ); db.query( qry );
  ...
  else
    {
      ...
      qry.clear ();
      qry << "update_autoflowStatusReport_record" << autoStatusID << autoID
          << reporting_Json;
...
US_ReporterGMP::US_ReporterGMP () : US_Widgets ()
{
  ...
  QLabel *bn_actions = us_banner (
      tr ("Generate GMP or Custom Report from Completed GMP Run:"), 1);
  ...
  QLabel *bn_actions_db
      = us_banner (tr ("Download and View GMP Report from DB:"), 1);
  QLabel *lb_loaded_run_db = us_label (tr ("Loaded Run:"));
  le_loaded_run_db = us_lineedit (tr (""), 0, true);
  QPushButton *pb_loadreport_db
      = us_pushbutton (tr ("Load GMP Report from DB (.PDF)"));
  ...
  connect (pb_loadrun, SIGNAL (clicked ()), this, SLOT (load_gmp_run ()));
  connect (pb_gen_report, SIGNAL (clicked ()), this,
           SLOT (generate_report ()));
  connect (pb_view_report, SIGNAL (clicked ()), this, SLOT (view_report ()));
  connect (pb_select_all, SIGNAL (clicked ()), this, SLOT (select_all ()));
  connect (pb_unselect_all, SIGNAL (clicked ()), this, SLOT (unselect_all ()));
  connect (pb_expand_all, SIGNAL (clicked ()), this, SLOT (expand_all ()));
  connect (pb_collapse_all, SIGNAL (clicked ()), this, SLOT (collapse_all ()));

  connect (pb_loadreport_db, SIGNAL (clicked ()), this,
           SLOT (load_gmp_report_db ()));
  connect (pb_view_report_db, SIGNAL (clicked ()), this,
           SLOT (view_report_db ()));
  ...
  genTree->setStyleSheet (
      "QTreeWidget { font: bold; font-size: "
      + QString::number (sfont.pointSize ())
      + "pt;}  QTreeView { alternate-background-color: yellow;} "
        "QTreeView::item:hover { border: black;  border-radius:1px;  "
        "background-color: rgba(0,128,255,95);}");

  // rightLayout: miscTree
  QLabel *lb_misctree
      = us_banner (tr ("Miscellaneous Report Profile Settings:"), 1);
  ...
  QLabel *lb_chantree
      = us_banner (tr ("Per-Triple Report Profile Settings:"), 1);
  ...
  QLabel *lb_combplots
      = us_banner (tr ("Combined Plot Distributions Settings:"), 1);
...
US_ReporterGMP::US_ReporterGMP (QString a_mode) : US_Widgets ()
{
  ...
  QSpacerItem *spacer2
      = new QSpacerItem (20, 100 * ihgt, QSizePolicy::Expanding);
  ...
  connect (pb_view_report_auto, SIGNAL (clicked ()), this,
           SLOT (view_report ()));
...
void
US_ReporterGMP::loadRun_auto (QMap<QString, QString> &protocol_details)
{
  ...
  if (full_runname.contains (",") && full_runname.contains ("IP")
      && full_runname.contains ("RI"))
    {
  ...
  lb_hdr1->setText (
      QString (tr ("Report for run: %1")).arg (FullRunName_auto));
  lb_hdr1->setSizePolicy (QSizePolicy::Preferred, QSizePolicy::Fixed);

  // show progress dialog
  progress_msg = new QProgressDialog ("Accessing run's protocol...",
                                      QString (), 0, 12, this);
  progress_msg->setWindowFlags (Qt::Tool | Qt::WindowTitleHint
  ...
  if (!GMP_report)
    {
      QMessageBox::information (
          this, tr ("Report Profile Uploaded"),
          tr ("<font color='red'><b>ATTENTION:</b> There are missing models "
              "for certain triples: </font><br><br>"
              "%1<br><br>"
...
US_ReporterGMP::read_autoflowGMPReportEsign_record (US_DB2 *db)
{
...
QString
US_ReporterGMP::get_assigned_oper_revs (QJsonDocument jsonDoc)
{
  QString smry;
...
void
US_ReporterGMP::check_for_dropped_triples (void)
{
  ...
  qDebug () << "::check_for_dropped_triples(): List of ALL dropped triples : "
            << droppedTriplesList;
}

// check for failed triples
void
US_ReporterGMP::check_failed_triples (void)
{
  ...
  for (int i = 0; i < analysisIDs_list.size (); ++i)
    {
      ...
      if (!analysis_details.size ())
        {
          analysis_details
              = read_autoflowAnalysisHistory_record (&db, requestID);
          qDebug () << "GMP Report: analysis_details.size() FROM "
                       "autoflowAnalysisHistory -- "
                    << analysis_details.size ();
        }
      else
        qDebug ()
            << "GMP Report: analysis_details.size() FROM autoflowAnalysis -- "
            << analysis_details.size ();
      ...
      if (!jsonDoc.isObject ())
        {
          ...
          QMessageBox::warning (
              this, tr ("JSON Format Problem"),
              tr ("JSON message for status of the analysis performed on "
                  "triple %1 appears to be corrupted. "
                  "This may be an indicaiton of the problem with the analysis "
                  "run for this triple.\n\n"
                  "This problem will preclude status monitoring for other "
                  "triples. "
                  "Please check log messages, or contact administrator for "
                  "help.\n\n"
                  "The program will return to the autoflow runs dialog.")
                  .arg (triple_name));
          return;
        }
      ...
      if (status == "FAILED" || status == "CANCELED")
        {
          if (processed.isUndefined ())
          ...
          else
            {
              ...
              QString stage_failed
                  = processed_array.last ().toObject ().keys ().last ();

              qDebug () << "GMP Report: stage_failed for triple -- "
                        << triple_name << stage_failed;
...
QMap<QString, QString>
US_ReporterGMP::read_autoflowAnalysis_record (US_DB2 *db,
                                              const QString &requestID)
{
  ...
  QStringList qry;
  qry << "read_autoflowAnalysis_record" << requestID;
  ...
  // qDebug() << "In reading autoflwoAnalysis record: json: " <<
  // analysis_details[ "status_json" ] ;
...
QMap<QString, QString>
US_ReporterGMP::read_autoflowAnalysisHistory_record (US_DB2 *db,
                                                     const QString &requestID)
{
  QMap<QString, QString> analysis_details;

  QStringList qry;
  qry << "read_autoflowAnalysisHistory_record" << requestID;
...
void
US_ReporterGMP::check_for_missing_models (void)
{
  ...
  for (mm = Triple_to_Models.begin (); mm != Triple_to_Models.end (); ++mm)
    {
      ...
      qDebug () << "For triple -- " << mm.key ()
                << ", there are models: " << mm.value ();
  ...
  // debug
  for (mm = Triple_to_ModelsMissing.begin ();
       mm != Triple_to_ModelsMissing.end (); ++mm)
    qDebug () << "For triple -- " << mm.key ()
              << ", there are missing models: " << mm.value ();
}

// Compose a string of missing models
QString
US_ReporterGMP::missing_models_msg (void)
{
  QString models_str;
  ...
  for (mmm = Triple_to_ModelsMissing.begin ();
       mmm != Triple_to_ModelsMissing.end (); ++mmm)
    {
      if (!mmm.value ().isEmpty ())
        {
          ...
          models_str
              += mmm.key () + ", missing models: " + mmm.value ().join (", ");
          if (isDropped)
...
QString
US_ReporterGMP::compose_html_failed_stage_missing_models (void)
{
  QString failed_str;
  ...
  // for ( mm = Triple_to_FailedStage.begin(); mm !=
  // Triple_to_FailedStage.end(); ++mm )
  //   {
  //     if ( !mm.value().isEmpty() )
  // 	{
  // 	  areFailed = true;

  // 	  failed_str += "<tr><td style=\"color:red;\">" + mm.key() +
  // ",</td><td> analysis failed/canceled at stage: </td><td
  // style=\"color:red;\">"
  // 	                + mm.value() + ";</td>" +
  // 	                + "<td>Models missing: </td><td style=\"color:red;\">"
  // +  Triple_to_ModelsMissing[ mm.key() ].join(", ") + "</td></tr>";
  // 	}
  //   }
...
void
US_ReporterGMP::check_models (int autoflowID)
{
      ...
      for (int jj = 0; jj < chann_wvl_number; ++jj)
        {
          ...
          QString tripleName = channel_desc_alt.section (":", 0, 0)[0] + "."
                               + channel_desc_alt.section (":", 0, 0)[1];
  ...
  for (int i = 0; i < Array_of_tripleNames.size (); ++i)
    {
      ...
      QStringList query;
      query << "get_modelAnalysisInfo" << Array_of_tripleNames[i]
      ...
      while (db->next ())
        {
          ...
          qDebug () << "Triple, modelDesc -- " << Array_of_tripleNames[i]
                    << modelDescJson;
        }
      ///
      ...
      Triple_to_ModelsDesc[Array_of_tripleNames[i]]
          = parse_models_desc_json (modelDescJson, "modelID");
      Triple_to_ModelsDescGuid[Array_of_tripleNames[i]]
          = parse_models_desc_json (modelDescJson, "modelGUID");
      ...
      while (db->next ())
        {
          ...
          // db->value( 3 ).toDateTime().toString( "yyyy/MM/dd HH:mm" ), true
          // );
          ...
          if (description.contains ("2DSA-IT")
            ...
            {
              qDebug ()
                  << "2DSA-IT Ids: modelID, read from modelLink: " << modelID
                  << Triple_to_ModelsDesc[Array_of_tripleNames[i]]["2DSA-IT"];
              model_list << "2DSA-IT";
            }
          ...
          if (description.contains ("2DSA-MC")
            ...
            {
              qDebug ()
                  << "2DSA-MC Ids: modelID, read from modelLink: " << modelID
                  << Triple_to_ModelsDesc[Array_of_tripleNames[i]]["2DSA-MC"];
              model_list << "2DSA-MC";
            }

          if (description.contains ("PCSA")
              && modelID
                     == Triple_to_ModelsDesc[Array_of_tripleNames[i]]["PCSA"])
            {
              qDebug ()
                  << "PCSA Ids: modelID, read from modelLink: " << modelID
                  << Triple_to_ModelsDesc[Array_of_tripleNames[i]]["PCSA"];
              model_list << "PCSA";
            }
        }
...
// Create QMap for shorter desctiption of triple's models (2DSA-IT, ...) to
// modelIDs, OR to modelGUIDs
QMap<QString, QString>
US_ReporterGMP::parse_models_desc_json (QString modelDescJson,
                                        QString model_property)
{
  ...
  if (!modelDescJson.isEmpty ())
    {
      QJsonDocument jsonDoc
          = QJsonDocument::fromJson (modelDescJson.toUtf8 ());
          ...
          if (key == "2DSA_IT" || key == "2DSA_MC" || key == "PCSA")
            {
              ...
              for (int i = 0; i < json_array.size (); ++i)
                {
                  foreach (const QString &array_key,
                           json_array[i].toObject ().keys ())
                    {
                      // by modelID
                      if (array_key == "modelID"
                          && array_key == model_property)
                        {
                          // if ( !modelDesc_shortened.contains( key_mod ) )
                          // //Temporary, for PCSA (2 entries)
                          //   {
                          modelDesc_shortened[key_mod] = json_array[i]
                                                             .toObject ()
                                                             .value (array_key)
                                                             .toString ();
                          qDebug () << "modelDescJson Map: -- model, "
                                       "property, value: "
                                    << key_mod << array_key
                                    << json_array[i]
                                           .toObject ()
                      ...
                      // by modelGUID
                      if (array_key == "modelGUID"
                          && array_key == model_property)
                        {
                          if (!modelDesc_shortened.contains (
                                  key_mod)) // Temporary, for PCSA (2 entries)
                            {
                              modelDesc_shortened[key_mod]
                                  = json_array[i]
                                        .toObject ()
                                        .value (array_key)
                                        .toString ();
                      ...
                      if (array_key == "editeddataID")
                        {
                          modelDesc_shortened["eID"] = json_array[i]
                                                           .toObject ()
                                                           .value (array_key)
                                                           .toString ();
...
void
US_ReporterGMP::load_gmp_report_db (void)
{
  // //TESTING *************
  // QFile fin(
  // "/home/alexey/ultrascan/reports/SBird-DNA-EcoRI-101322-PD9-run1843_GMP_DB_bySysTar.tar.gz"
  ...
  pdiag_autoflow_db = new US_SelectItem (gmpReportsDBdata, hdrs, pdtitle, &prx,
                                         autoflow_btn, -3);
  ...
  if (gmpReport_runname_selected_c.contains ("combined"))
    {
      gmpReport_runname_selected
          = gmpReport_runname_selected_c.split ("(")[0].simplified ();
  ...
  // <---- TESTING: gZip algorithm NOT compatible (even for different Linux
  // distros...) **
  ...
  int db_read = db.readBlobFromDB (GMPReportfpath, "download_gmpReportData",
  ...
  // <--- TESTING: tried .tar.gz - NOT compatible (even for different Linux
  // distros...) ****
  ...
  filePath_db = dirName + "/" + gmpReport_runname_selected + "/"
                + gmpReport_filename_pdf;
  ...
  msgBox.setInformativeText (
      tr ("Report was downloaded form DB in .PDF format and saved at: \n%1\n\n"
          "When this dialog is closed, the report can be re-opened by "
          "clicking \'View Downloaded Report\' button on the left.")
          .arg (filePath_db));

  msgBox.setWindowTitle (tr ("Report Generation Complete"));
  QPushButton *Open
      = msgBox.addButton (tr ("View Report"), QMessageBox::YesRole);
  // QPushButton *Cancel  = msgBox.addButton(tr("Ignore Data"),
...
int
US_ReporterGMP::list_all_gmp_reports_db (QList<QStringList> &gmpReportsDBdata,
                                         US_DB2 *db)
{
  int nrecs = 0;
  ...
  while (db->next ())
    {
      ...
      QString ptime_created = US_Util::toUTCDatetimeText (
                                  time_created.toString (Qt::ISODate), true)
                                  .section (":", 0, 1)
                              + " UTC";
...
// load GMP run
void
US_ReporterGMP::load_gmp_run (void)
{
  ...
  pdiag_autoflow = new US_SelectItem (autoflowdata, hdrs, pdtitle, &prx,
                                      autoflow_btn, -4);
      ...
      for (it = tmap.begin (); it != tmap.end (); ++it)
        {
          qDebug () << "ModelsDesc: Triple, QMap -- "
                    << Array_of_tripleNames[i] << it.key () << it.value ();

          qDebug () << "ModelsDescGuid: Triple, QMap -- "
                    << Array_of_tripleNames[i] << it.key ()
                    << tmapguid[it.key ()];
        }
    }
  ...
  if (full_runname.contains (",") && full_runname.contains ("IP")
      && full_runname.contains ("RI"))
    {
      ...
      full_runname
          = runName
            + " (combined RI+IP)"; // Just use runName (captures ProtDev names)
  ...
  if (GMP_report)
    {
      QMessageBox::information (
          ...
          tr ("Report profile uploaded for GMP run:\n"
              "%1\n\n"
              "ATTENTION: Current profile configuration corresponds to GMP "
              "report settings.\n\n"
              "Any changes in the profile settings will result in generation "
              "of the non-GMP report!")
              .arg (full_runname));
    }
  else
    {
      QMessageBox::information (
          ...
          QString ("Report profile uploaded for GMP run:<br><br>"
                   "<b>%1</b><br><br>"
                   "<font color='red'><b>ATTENTION:</b> There are missing "
                   "models for certain triples: </font><br><br>"
                   "%2<br><br>"
...
// //public functions:
// int US_ReporterGMP::list_all_autoflow_records_pub( QList< QStringList >&
// autoflowdata )
// {
...
// Query autoflow (history) table for records
int
US_ReporterGMP::list_all_autoflow_records (QList<QStringList> &autoflowdata)
{
  int nrecs = 0;
  autoflowdata.clear ();
  ...
  while (db->next ())
    {
      ...
      // time_created.toString(); // << time_started.toString(); // <<
      // local.toString( Qt::ISODate );
      ...
      if (time_started.toString ().isEmpty ())
      ...
      else
        {
          ...
          if (status == "EDITING" || status == "EDIT_DATA"
              || status == "ANALYSIS" || status == "REPORT"
              || status == "E-SIGNATURES")
      ...
      // Check user level && GUID; if <3, check if the user is operator ||
      // investigator
      if (US_Settings::us_inv_level () < 3)
        {
          qDebug () << "User level low: " << US_Settings::us_inv_level ();
          qDebug () << "user_id, operatorID.toInt(), invID.toInt() -- "
                    << user_id << operatorID.toInt () << invID.toInt ();

          // if ( user_id && ( user_id == operatorID.toInt() || user_id ==
          // invID.toInt() ) )
          if (user_id && user_id == invID.toInt ())
            { // Do we allow operator as defined in autoflow record to also see
              // reports??
...
QMap<QString, QString>
US_ReporterGMP::read_autoflow_record (int autoflowID)
{
  // Check DB connection
  US_Passwd pw;
  ...
  QStringList qry;
  qry << "read_autoflow_history_record" << QString::number (autoflowID);
...
void
US_ReporterGMP::read_protocol_and_reportMasks (void)
{
  ...
  qApp->processEvents ();
  QString xmlstr ("");
  US_ProtocolUtil::read_record_auto (ProtocolName_auto, invID, &xmlstr, NULL,
                                     &db);
  ...
  // Debug: protocol
  qDebug () << "Protocols' details: -- " << currProto.investigator
            << currProto.runname << currProto.protoname << currProto.protoID
            << currProto.project << currProto.temperature
            << currProto.temeq_delay << currProto.exp_label;
  ...
  replicates_to_channdesc
      = currAProf.replicates_to_channdesc_main; // Empty ? (not needed?)
  ...
  qDebug () << "Wavelengths ch_wvls[ channel_desc_alt ] "
            << ch_wvls[channel_desc_alt]
            << " for channel: " << channel_desc_alt;
  ...
  if (reportGMP.reportItems.size () > 0)
    qDebug () << "AProfile's && ReportGMP's details: -- "
              << currAProf.aprofname << currAProf.protoname
              << currAProf.chndescs << currAProf.chndescs_alt
              << currAProf.lc_ratios << cAP2.parms[0].channel
              << cAPp.parms[0].channel << reportGMP.rmsd_limit
              << reportGMP.wavelength << reportGMP.reportItems[0].type
    ...
    qDebug () << "AProfile's && ReportGMP's details: -- "
              ...
              << currAProf.lc_ratios << cAP2.parms[0].channel
              << cAPp.parms[0].channel << reportGMP.rmsd_limit
              << reportGMP.wavelength << "No GMP_Report_Items!!!";

  qDebug () << "Number of wvls in channel: " << chndescs_alt[0] << ": "
            << ch_wvls[channel_desc_alt].size ();
  qDebug () << "Wvls in channel: " << chndescs_alt[0] << ": "
            << ch_wvls[channel_desc_alt];
  qDebug () << "Pseudo3d pars: " << reportGMP.pseudo3d_2dsait_s_ff0
...
US_ReporterGMP::parse_comb_plots_json (QString combPlots)
{
          ...
          for (int i = 0; i < json_array.size (); ++i)
            {
              foreach (const QString &array_key,
                       json_array[i].toObject ().keys ())
                {
                  iMap[key][array_key] = json_array[i]
                                             .toObject ()
                                             .value (array_key)
                                             .toString ();

                  qDebug ()
                      << "ComboPlots Map: -- type_method, parameter, value: "
                      << key << array_key
                      << json_array[i]
                             .toObject ()
                             .value (array_key)
...
// read Intensity info
QMap<QString, QString>
US_ReporterGMP::read_autoflowIntensity (QString ID, US_DB2 *db)
{
  QString iJson;
              ...
              for (int i = 0; i < json_array.size (); ++i)
                {
                  foreach (const QString &array_key,
                           json_array[i].toObject ().keys ())
                    {
                      iMap[array_key] = json_array[i]
                                            .toObject ()
                                            .value (array_key)
                                            .toString ();

                      qDebug ()
                          << "Intensity: MWL -- array_key, val: " << array_key
                          << json_array[i]
                                 .toObject ()
                                 .value (array_key)
                                 .toString ();
...
void
US_ReporterGMP::parse_gen_mask_json (const QString reportMask)
{
              ...
              foreach (const QString &array_key,
                       json_array[i].toObject ().keys ())
                {
                  if (key.contains ("Solutions"))
                    {
                      ...
                      solutionItems_vals << json_array[i]
                                                .toObject ()
                                                .value (array_key)
                                                .toString ();
                    }
                  if (key.contains ("Analysis"))
                    {
                      QJsonObject newObj = json_array[i]
                                               .toObject ()
                                               .value (array_key)
                                               .toObject ();
...
void
US_ReporterGMP::build_miscTree (void)
{
  QString indent ("  ");
  ...
  for (int i = 0; i < miscTopLevelItems.size (); ++i)
    {
      ...
      miscItem[topItemName]
          = new QTreeWidgetItem (miscTree, topItemNameList, wiubase);
  ...
  connect (miscTree, SIGNAL (itemChanged (QTreeWidgetItem *, int)), this,
           SLOT (changedItem (QTreeWidgetItem *, int)));
}

// build general report mask tree
void
US_ReporterGMP::build_genTree (void)
{
  QString indent ("  ");
  QStringList topItemNameList, solutionItemNameList, analysisItemNameList,
      analysisGenItemNameList, analysis2DSAItemNameList,
      analysisPCSAItemNameList;
  int wiubase = (int)QTreeWidgetItem::UserType;
  ...
  for (int i = 0; i < topLevelItems.size (); ++i)
    {
      ...
      topItemNameList << "" << indent + topItemName;
      topItem[topItemName]
          = new QTreeWidgetItem (genTree, topItemNameList, wiubase);
          ...
          for (int is = 0; is < solutionItems.size (); ++is)
            {
              ...
              solutionItemNameList << ""
                                   << indent.repeated (2) + solutionItemName;
              solutionItem[solutionItemName] = new QTreeWidgetItem (
                  topItem[topItemName], solutionItemNameList, wiubase);

              if (solutionItems_vals[is].toInt ())
                {
                  solutionItem[solutionItemName]->setCheckState (0,
                                                                 Qt::Checked);
                  ++checked_childs;
                }
              else
          ...
          for (int ia = 0; ia < analysisItems.size (); ++ia)
            {
              ...
              if (expType == "ABDE"
                  && (analysisItemName.contains ("2DSA")
                      || analysisItemName.contains ("PCSA")))
                continue;

              analysisItemNameList.clear ();
              analysisItemNameList << ""
                                   << indent.repeated (2) + analysisItemName;
              analysisItem[analysisItemName] = new QTreeWidgetItem (
                  topItem[topItemName], analysisItemNameList, wiubase);
              ...
              if (analysisItemName.contains ("General"))
                {
                  ...
                  for (int iag = 0; iag < analysisGenItems.size (); ++iag)
                    {
                      ...
                      analysisGenItemNameList
                          << "" << indent.repeated (3) + analysisGenItemName;
                      analysisGenItem[analysisGenItemName]
                          = new QTreeWidgetItem (
                              analysisItem[analysisItemName],
                              analysisGenItemNameList, wiubase);

                      if (analysisGenItems_vals[iag].toInt ())
                        {
                          analysisGenItem[analysisGenItemName]->setCheckState (
                              0, Qt::Checked);
                          ++checked_gen;
                        }
                      else
                  ...
                  if (checked_gen)
                    {
                      analysisItem[analysisItemName]->setCheckState (
                          0, Qt::Checked);
                      ++checked_childs;
                    }
                  else
                  ...
                  for (int ia2 = 0; ia2 < analysis2DSAItems.size (); ++ia2)
                    {
                      ...
                      analysis2DSAItemNameList
                          << "" << indent.repeated (3) + analysis2DSAItemName;
                      analysis2DSAItem[analysis2DSAItemName]
                          = new QTreeWidgetItem (
                              analysisItem[analysisItemName],
                              analysis2DSAItemNameList, wiubase);
                      ...
                      if (analysis2DSAItems_vals[ia2].toInt ())
                        ...
                        analysis2DSAItem[analysis2DSAItemName]->setCheckState (
                            0, Qt::Unchecked);
                    }
                  ...
                  for (int iap = 0; iap < analysisPCSAItems.size (); ++iap)
                    {
                      ...
                      analysisPCSAItemNameList
                          << "" << indent.repeated (3) + analysisPCSAItemName;
                      analysisPCSAItem[analysisPCSAItemName]
                          = new QTreeWidgetItem (
                              analysisItem[analysisItemName],
                              analysisPCSAItemNameList, wiubase);
                      ...
                      if (analysisPCSAItems_vals[iap].toInt ())
                        ...
                        analysisPCSAItem[analysisPCSAItemName]->setCheckState (
                            0, Qt::Unchecked);
                    }
  ...
  connect (genTree, SIGNAL (itemChanged (QTreeWidgetItem *, int)), this,
           SLOT (changedItem (QTreeWidgetItem *, int)));
}

// get Item's children
void
US_ReporterGMP::get_item_childs (QList<QTreeWidgetItem *> &children_list,
...
void
US_ReporterGMP::changedItem (QTreeWidgetItem *item, int col)
{
...
void
US_ReporterGMP::build_perChanTree_abde (void)
{
  // trees
  ...
  for (int ac = 0; ac < abde_channList.size (); ++ac)
    {
          ...
          if (channel_desc_alt.split (":")[0] == abde_channList[ac])
            {
              ...
              reportGMP
                  = ch_reports[channel_desc_alt][QString::number (chann_wvl)];
              break;
            }
        }
      ...
      chanItem[chanItemName]
          = new QTreeWidgetItem (perChanTree, chanItemNameList, wiubase);
      ...
      qDebug () << "For ABDE-channel, " << chanItemName
                << ", report parms. are -- "
                << reportGMP.integration_results_mask << reportGMP.plots_mask;
      ...
      for (int kk = 0; kk < tripleReportMasksList.size (); ++kk)
        {
          ...
          tripleMaskItemNameList << ""
                                 << indent.repeated (2) + tripleMaskItemName;
          tripleMaskItem[chanItemName] = new QTreeWidgetItem (
              chanItem[chanItemName], tripleMaskItemNameList, wiubase);
  ...
  qDebug () << "perChan Tree first time build ? "
            << first_time_perChan_tree_build;
  ...
  connect (perChanTree, SIGNAL (itemChanged (QTreeWidgetItem *, int)), this,
           SLOT (changedItem (QTreeWidgetItem *, int)));
}

// build perChanTree
void
US_ReporterGMP::build_perChanTree (void)
{
  QString indent ("  ");
  QStringList chanItemNameList, tripleItemNameList, tripleItemModelNameList,
      tripleMaskItemNameList, tripleMaskItemPlotNameList,
      tripleMaskItemPseudoNameList;
      ...
      if (triple_report)
        {
          ...
          QString chanItemName
              = "Channel "
                + channel_desc_alt.section (":", 0, 1).replace (":", "-");
          chanItemNameList.clear ();
          chanItemNameList << "" << indent + chanItemName;
          chanItem[chanItemName]
              = new QTreeWidgetItem (perChanTree, chanItemNameList, wiubase);

          // QList < double > chann_wvls                  = ch_wvls[
          // channel_desc ];
          QList<double> chann_wvls = ch_wvls[channel_desc_alt];
          QMap<QString, US_ReportGMP> chann_reports
              = ch_reports[channel_desc_alt];

          qDebug () << "channel_desc_alt , channel_desc: " << channel_desc_alt
                    << " , " << channel_desc;
          qDebug () << "chann_wvls [ channel_desc ] -- " << chann_wvls;
          qDebug () << "chann_wvls [ channel_desc_alt ] -- "
                    << ch_wvls[channel_desc_alt];
          ...
          for (int jj = 0; jj < chann_wvl_number; ++jj)
            {
              ...
              // QString tripleName = channel_desc_alt.section( ":", 0, 0 )[0]
              // + "." + channel_desc_alt.section( ":", 0, 0 )[1] + "." + wvl;
              QString tripleName = channel_desc_alt.section (":", 0, 0)[0]
                                   + "."
                                   + channel_desc_alt.section (":", 0, 0)[1];
              ...
              qDebug () << "[in build_perChanTree()]: TripleName -- "
                        << tripleName;
              ...
              tripleItem[tripleItemName] = new QTreeWidgetItem (
                  chanItem[chanItemName], tripleItemNameList, wiubase);
              ...
              for (int kk = 0; kk < report_items_number; ++kk)
                {
                  US_ReportGMP::ReportItem curr_item
                      = reportGMP.reportItems[kk];
                  ...
                  // Use ind_combined_plot here, to mark if ind. plots for
                  // triple's reportItem is to be plot
                  int ind_combPlot = curr_item.ind_combined_plot;
                  QString range_l_h = QString::number (curr_item.range_low)
                                      + ":"
                                      + QString::number (curr_item.range_high);

                  if (Triple_to_Models[tripleName].contains (method))
                    {
                      if (ind_combPlot)
                        {
                          // QMap< QString (triple), QMap< QString( type_method
                          // ), QStringList( ranges ) > >
                          indComboPlotsMapTripleTypeRangeBool[tripleName][t_m]
                                                             [range_l_h]
                              = true;
                        }
                      else
              ...
              qDebug () << "For triple, " << tripleName
                        << ", and models: " << Triple_to_Models[tripleName]
                        << reportGMP.tot_conc_mask << reportGMP.rmsd_limit_mask
                        << reportGMP.av_intensity_mask
                        << reportGMP.experiment_duration_mask
                        << reportGMP.integration_results_mask
                        << reportGMP.plots_mask << reportGMP.pseudo3d_mask;
              ...
              for (int mm = 0; mm < tripleReportModelsList.size (); ++mm)
                {
                  // Triple's mask params: child-level 3 in a perChanTree
                  QString tripleItemModelName
                      = tripleReportModelsList[mm] + " Model";
                  QString tripleModelName
                      = tripleItemName + "," + tripleItemModelName;
                  tripleItemModelNameList.clear ();
                  tripleItemModelNameList
                      << "" << indent.repeated (3) + tripleItemModelName;
                  tripleModelItem[tripleModelName]
                      = new QTreeWidgetItem (tripleItem[tripleItemName],
                                             tripleItemModelNameList, wiubase);
                  ...
                  tripleReportMasksList_vals.clear ();
                  tripleReportMasksList_vals
                      << reportGMP.tot_conc_mask << reportGMP.rmsd_limit_mask
                      << reportGMP.av_intensity_mask
                      //<< reportGMP. experiment_duration_mask
                      << true << reportGMP.integration_results_mask
                      << reportGMP.plots_mask << reportGMP.pseudo3d_mask;

                  // Here: add "Individual Distributions" to
                  // tripleReportMasksList IF there is/are type-method(s)
                  // matching current model:
                  QStringList type_methods_list
                      = indComboPlotsMapTripleTypeRangeBool[tripleName]
                            .keys ();
                  for (int tm = 0; tm < type_methods_list.size (); tm++)
                    {
                      // select type-method for current model only! [e.g.
                      // {s,D}-2DSA-IT for 2DSA-IT model]
                      QString c_method = type_methods_list[tm].split (",")[1];
                      if (c_method == tripleReportModelsList[mm])
                        {
                          qDebug ()
                              << "In Model, list, ind. : " << type_methods_list
                              << tripleReportModelsList[mm];
                          tripleReportMasksList
                              << "Individual Combined Distributions";
                  ...
                  for (int kk = 0; kk < tripleReportMasksList.size (); ++kk)
                    {
                      ...
                      tripleMaskItemNameList.clear ();
                      tripleMaskItemNameList
                          << "" << indent.repeated (4) + tripleMaskItemName;
                      tripleMaskItem[tripleModelName] = new QTreeWidgetItem (
                          tripleModelItem[tripleModelName],
                          tripleMaskItemNameList, wiubase);

                      if (tripleReportMasksList_vals[kk])
                        {
                          tripleMaskItem[tripleModelName]->setCheckState (
                              0, Qt::Checked);
                          ++checked_masks;
                        }
                      else
                      ...
                      if (tripleMaskItemName.contains ("Plots"))
                        {
                          ...
                          tripleReportMasksPlotList
                              << "Experiment-Simulation Velocity Data (noise "
                                 "corrected) "
                          ...
                          for (int hh = 0;
                               hh < tripleReportMasksPlotList.size (); ++hh)
                            {
                              QString tripleMaskItemPlotName
                                  = tripleReportMasksPlotList[hh];
                              tripleMaskItemPlotNameList.clear ();
                              tripleMaskItemPlotNameList
                                  << ""
                                  << indent.repeated (5)
                                         + tripleMaskItemPlotName;
                              tripleMaskPlotItem[tripleModelName]
                                  = new QTreeWidgetItem (
                                      tripleMaskItem[tripleModelName],
                                      tripleMaskItemPlotNameList, wiubase);
                      ...
                      if (tripleMaskItemName.contains ("Pseudo3d"))
                        {
                          ...
                          tripleReportMasksPseudoList
                              << "Pseudo3d s-vs-f/f0 Distribution"
                          ...
                          if (tripleItemModelName.contains ("2DSA-IT"))
                            {
                              tripleReportMasksPseudoList_vals
                                  << reportGMP.pseudo3d_2dsait_s_ff0
                                  << reportGMP.pseudo3d_2dsait_s_d
                                  << reportGMP.pseudo3d_2dsait_mw_ff0
                                  << reportGMP.pseudo3d_2dsait_mw_d;
                              qDebug () << "In pseudo3d tree: "
                                           "tripleReportMasksPseudoList_vals "
                                           "(2DSA-IT) -- "
                                        << tripleReportMasksPseudoList_vals;
                            }
                            ...
                            {
                              tripleReportMasksPseudoList_vals
                                  << reportGMP.pseudo3d_2dsamc_s_ff0
                          ...
                          for (int ps = 0;
                               ps < tripleReportMasksPseudoList.size (); ++ps)
                            {
                              QString tripleMaskItemPseudoName
                                  = tripleReportMasksPseudoList[ps];
                              tripleMaskItemPseudoNameList.clear ();
                              tripleMaskItemPseudoNameList
                                  << ""
                                  << indent.repeated (5)
                                         + tripleMaskItemPseudoName;
                              tripleMaskPseudoItem[tripleModelName]
                                  = new QTreeWidgetItem (
                                      tripleMaskItem[tripleModelName],
                                      tripleMaskItemPseudoNameList, wiubase);
                      ...
                      if (tripleMaskItemName.contains ("Individual"))
                        {
                          ...
                          // populate tripleReportMasksIndividualPlotList based
                          // on type-method entries in the reportItem:
                          QStringList types_methods_list
                              = indComboPlotsMapTripleTypeRangeBool[tripleName]
                                    .keys ();
                          for (int tm = 0; tm < types_methods_list.size ();
                               tm++)
                            {
                              // select type-method for current model only!
                              // [e.g. {s,D}-2DSA-IT for 2DSA-IT model]
                              QString c_method
                                  = types_methods_list[tm].split (",")[1];
                              QString c_type
                                  = types_methods_list[tm].split (",")[0];
                              if (c_method == tripleReportModelsList[mm])
                                {
                                  QMap<QString, bool> c_ranges_bool
                                      = indComboPlotsMapTripleTypeRangeBool
                                          [tripleName][types_methods_list[tm]];
                                  QMap<QString, bool>::iterator r_bool;
                                  for (r_bool = c_ranges_bool.begin ();
                                       r_bool != c_ranges_bool.end ();
                                       ++r_bool)
                                    {
                                      ...
                                      QString to_show_ind
                                          = r_value ? "YES" : "NO";
                                      QString c_type_range
                                          = c_type + "[" + r_key + "]" + "--"
                                            + to_show_ind;
                                      tripleReportMasksIndividualPlotList
                                          << c_type_range;
                                    }
                                }
                            }
                          ...
                          for (int icp = 0;
                               icp
                               < tripleReportMasksIndividualPlotList.size ();
                               ++icp)
                            {
                              QString individualPlotName
                                  = tripleReportMasksIndividualPlotList[icp];
                              QString individualPlotName_name
                                  = tripleReportMasksIndividualPlotList[icp]
                                        .split ("--")[0];
                              QString individualPlotName_show
                                  = tripleReportMasksIndividualPlotList[icp]
                                        .split ("--")[1];
                              QStringList individualPlotNameList;
                              individualPlotNameList
                                  << ""
                                  << indent.repeated (5)
                                         + individualPlotName_name;

                              tripleMaskIndComboPlotItem[tripleModelName]
                                  = new QTreeWidgetItem (
                                      tripleMaskItem[tripleModelName],
                          ...
                          // Now, set top-level, model-specific Ind. Comboplot
                          // item
                  ...
                  if (checked_masks)
                    tripleModelItem[tripleModelName]->setCheckState (
                        0, Qt::Checked);
                  else
                    tripleModelItem[tripleModelName]->setCheckState (
                        0, Qt::Unchecked);
  ...
  for (ch = chanItem.begin (); ch != chanItem.end (); ++ch)
    {
      ...
      if (channel_children == hidden_triples)
        {
          qDebug () << "We have to hide entire channel  "
                    << ch.value ()->text (1);
...
void
US_ReporterGMP::build_combPlotsTree (void)
{
  QString indent ("  ");
  ...
  for (cpt = comboPlotsMapTypes.begin (); cpt != comboPlotsMapTypes.end ();
       ++cpt)
    {
      if (cpt.value ())
        {
  ...
  for (tm = CombPlots_Type_to_Models.begin ();
       tm != CombPlots_Type_to_Models.end (); ++tm)
    {
      ...
      topItemCombPlots[tm.key ()]
          = new QTreeWidgetItem (combPlotsTree, topItemNameList, wiubase);
      ...
      for (int i = 0; i < unique_models.size (); ++i)
        {
          ...
          ItemCombPlots[modelName] = new QTreeWidgetItem (
              topItemCombPlots[tm.key ()], ItemNameList, wiubase);
  ...
  connect (combPlotsTree, SIGNAL (itemChanged (QTreeWidgetItem *, int)), this,
           SLOT (changedItem (QTreeWidgetItem *, int)));
}

// select all items in trees
void
US_ReporterGMP::select_all (void)
{
  ...
  QTreeWidgetItem *combPlotsTree_rootItem
      = combPlotsTree->invisibleRootItem ();
...
void
US_ReporterGMP::unselect_all (void)
{
  QTreeWidgetItem *getTree_rootItem = genTree->invisibleRootItem ();
  for (int i = 0; i < getTree_rootItem->childCount (); ++i)
    {
...
void
US_ReporterGMP::expand_all (void)
{
  genTree->expandAll ();
  perChanTree->expandAll ();
  combPlotsTree->expandAll ();
...
void
US_ReporterGMP::collapse_all (void)
{
  genTree->collapseAll ();
...
// view report
void
US_ReporterGMP::view_report (void)
{
...
// view report DB
void
US_ReporterGMP::view_report_db (void)
{
...
// reset
void
US_ReporterGMP::reset_report_panel (void)
{
  ...
  qDebug () << "Size Hint for combPlots Tree -- "
            << combPlotsTree->sizeHint ();
...
// Generate report
void
US_ReporterGMP::generate_report (void)
{
  ...
  fileNameList.clear ();
  if (FileName.contains (",") && FileName.contains ("IP")
      && FileName.contains ("RI"))
  ...
  if (auto_mode)
      ...
      for (int i = 0; i < Array_of_triples.size (); ++i)
        {
          ...
          // here should be cycle over triple's models ( 2DSA-IT, 2DSA-MC
          // etc..)
          ...
          for (int j = 0; j < models_to_do.size (); ++j)
            {
              ...
              process_combined_plots_individual (currentTripleName,
                                                 models_to_do[j]);
            }
        }
          ...
          for (int ac = 0; ac < abde_channList.size (); ++ac)
            {
              ...
              if (!perChanMask_edited_abde.ShowChannelParts[key_m])
                {
                  QString f_name_mask
                      = abde_plots_filenames[abde_channList[ac]];
              ...
              bool do_plots = true;
              QMap<QString, QString> channs_features
                  = perChanMask_edited_abde.ShowChannelItemParts[key_m];
              ...
              for (int fc = 0; fc < channs_features_keys.size (); ++fc)
                {
                  ...
                  bool do_feature
                      = (channs_features[fc_key].toInt ()) ? true : false;
              ...
              // display channel info: RMSD, exp. params, integration ranges &
              // percents
    ...
    { // Will be modified for stand-alone GMP Reporter based on edited tree
      // JSON
      for (int i = 0; i < Array_of_triples.size (); ++i)
        {
          currentTripleName = Array_of_triples[i];
          ...
          for (int j = 0; j < models_to_do.size (); ++j)
            {
              ...
              qDebug ()
                  << "Triple / Model " << triplename_alt << " / "
                  << models_to_do[j] << "has items ? "
                  << perChanMask_edited.has_tripleModel_items[triplename_alt]
                                                             [models_to_do[j]]
                  << perChanMask_edited
                         .has_tripleModelPlot_items[triplename_alt]
                                                   [models_to_do[j]];

              if (perChanMask_edited
                      .has_tripleModel_items[triplename_alt][models_to_do[j]]
                  || perChanMask_edited
                         .has_tripleModelPlot_items[triplename_alt]
                                                   [models_to_do[j]]
                  || perChanMask_edited
                         .has_tripleModelIndCombo_items[triplename_alt]
                                                       [models_to_do[j]])
                {
                  simulate_triple (currentTripleName, models_to_do[j]);
      ...
      if (expType == "VELOCITY")
      ...
      else if (expType == "ABDE")
        {
          ...
          // contruct basic channel description: RMDS, Comparison, Integration
          // (percents)
  ...
  if (auto_mode)
    {
      ...
      /******* NOTES *******************************************
               ...
               1. For assigned oper/rev/appr. cases (default):
                  -- inform user that they are assigned && those assignees will
      re-attach later;
      ...
      ************************************************************/
      ...
      // Compose "{to_sign:["","",...]}" out of operatorListJson,
      // reviewersListJson, approversListJson
      //&& update 'eSignStatusJson' of the autoflowGMPReportEsign record with
      //it
      QMap<QString, QString> eSign_details
          = read_autoflowGMPReportEsign_record (AutoflowID_auto);
      QStringList oper_rev_joinedList;

      QJsonDocument jsonDocOperList = QJsonDocument::fromJson (
          eSign_details["operatorListJson"].toUtf8 ());
      get_assigned_oper_revs (jsonDocOperList, oper_rev_joinedList);

      QJsonDocument jsonDocRevList = QJsonDocument::fromJson (
          eSign_details["reviewersListJson"].toUtf8 ());
      get_assigned_oper_revs (jsonDocRevList, oper_rev_joinedList);

      QJsonDocument jsonDocApprList = QJsonDocument::fromJson (
          eSign_details["approversListJson"].toUtf8 ());
      ...
      if (db->lastErrno () != US_DB2::OK)
        {
          QApplication::restoreOverrideCursor ();
          QMessageBox::information (this, tr ("DB Connection Problem"),
                                    tr ("AutoflowHistory: there was an error "
      ...
      // 	  << runID
      // 	  << optimaName;
      qry << "update_autoflow_at_report" << AutoflowID_auto;
      // db->query( qry );
      ...
      if (status == US_DB2::NO_AUTOFLOW_RECORD)
        {
          QMessageBox::warning (this, tr ("Autoflow Record Not Updated"),
      ...
      qry.clear ();
      qry << "update_gmp_review_record_by_esigner" << eSign_details["ID"]
          << AutoflowID_auto << eSignStatusJson << "NO";
      ...
      msgBox_a.setInformativeText (
          tr ("Report PDF was saved at: \n%1\n\n"
              "When this dialog is closed, the report can be re-opened by "
              "clicking \'View Generated Report\' button at the bottom."
              "\n\n"
      ...
      QPushButton *Open
          = msgBox_a.addButton (tr ("View Report"), QMessageBox::YesRole);
      // QPushButton *Cancel  = msgBox_a.addButton(tr("Ignore Data"),
      // QMessageBox::RejectRole);
  ...
  else
    {
      ...
      msgBox.setInformativeText (
          tr ("Report PDF was saved at: \n%1\n\n"
              "When this dialog is closed, the report can be re-opened by "
              "clicking \'View Generated Report\' button on the left.")
              .arg (filePath));
...
void
US_ReporterGMP::process_abde_plots (void)
{
  // read, parse
  ...
  connect (sdiag_norm_profile,
           SIGNAL (pass_rmsd_info (QMap<QString, double> &)), this,
           SLOT (get_abde_rmsds (QMap<QString, double> &)));
  connect (sdiag_norm_profile,
           SIGNAL (pass_menisc_info (QMap<QString, double> &)), this,
           SLOT (get_abde_menisc (QMap<QString, double> &)));
  connect (
      sdiag_norm_profile,
      SIGNAL (pass_percents_info (QMap<QString, QMap<QString, double> > &)),
      this,
      SLOT (get_abde_percents (QMap<QString, QMap<QString, double> > &)));
  connect (sdiag_norm_profile,
           SIGNAL (pass_data_per_channel (
               QMap<QString, QMap<QString, QVector<QVector<double> > > > &)),
           this,
           SLOT (get_abde_data_per_channel (
               QMap<QString, QMap<QString, QVector<QVector<double> > > > &)));
  ...
  for (int i = 0; i < abde_channList.size (); ++i)
    {
      QString channel_name_abde
          = sdiag_norm_profile->select_channel_public (i);
      qDebug () << "[ABDE proc.], channel_name_abde, abde_channList[i] -- "
                << channel_name_abde << abde_channList[i];
      QString img01File = dirName + "/" + "ABDE_norm." + channel_name_abde
                          + ".deconv" + svgext;
...
void
US_ReporterGMP::get_abde_channels (QStringList &abde_cl_p)
{
...
void
US_ReporterGMP::get_abde_rmsds (QMap<QString, double> &abde_rmsd_p)
{
  abde_rmsd = abde_rmsd_p;
}

void
US_ReporterGMP::get_abde_menisc (QMap<QString, double> &abde_menisc_p)
{
...
void
US_ReporterGMP::get_abde_percents (
    QMap<QString, QMap<QString, double> > &abde_perc_p)
{
  abde_ranges_percents = abde_perc_p;
}

void
US_ReporterGMP::get_abde_data_per_channel (
    QMap<QString, QMap<QString, QVector<QVector<double> > > > &data_per_chan)
{
  abde_data_per_channel = data_per_chan;

  // qDebug() << "Passed data_for_chan 2A: xvalues -- "   <<
  // abde_data_per_channel["2A"]["xvalues"]; qDebug() << "Passed data_for_chan
  // 2A: yvaluesN -- "  << abde_data_per_channel["2A"]["yvaluesN"]; qDebug() <<
  // "Passed data_for_chan 2A: integralN -- " <<
  // abde_data_per_channel["2A"]["integralN"];
}

// read eSign GMP record for assigned oper(s) && rev(s) && status
QMap<QString, QString>
US_ReporterGMP::read_autoflowGMPReportEsign_record (QString aID)
{
  QMap<QString, QString> eSign_record;
...
QStringList
US_ReporterGMP::scan_dbase_models (QStringList runIDs)
{
  QStringList wDescrs;
  ...
  while (db.next ())
    {
      ...
      DbgLv (1) << "ScMd: UNASSGN: mid,eid,mdesc,mrun" << mdlid << edtid
                << mdesc << mrunid;
  ...
  if (m > 1)
    {
      DbgLv (1) << "ScMd: 0: id,gid,eid,desc" << mmIDs[0] << mmGUIDs[0]
                << meIDs[0] << mmDescs[0];
      DbgLv (1) << "ScMd: m: id,gid,eid,desc" << mmIDs[m] << mmGUIDs[m]
                << meIDs[m] << mmDescs[m];
    }
...
// simulate triple
void
US_ReporterGMP::simulate_triple (const QString triplesname,
                                 QString stage_model)
{
  // Show msg while data downloaded and simulated
  progress_msg = new QProgressDialog (
      QString ("Downloading data and models for triple %1...")
          .arg (triplesname),
      QString (), 0, 5, this);
  progress_msg->setWindowFlags (Qt::Window | Qt::WindowTitleHint
                                | Qt::CustomizeWindowHint);
  ...
  qDebug () << "In SHOW OVERLAY: triple_stage / triple_name: " << stage_n
            << " / " << triple_n;
  ...
  QString expID;
  int idExp = 0;
  query << "get_experiment_info_by_runID" << FileName_parsed
      ...
      for (int jj = 0; jj < qMin (essknt, tssknt); jj++)
        {
          qDebug () << "LD:  jj" << jj << "(e) tf tl wf wl scns"
                    << speed_steps[jj].time_first << speed_steps[jj].time_last
                    << speed_steps[jj].w2t_first << speed_steps[jj].w2t_last
  ...
  bufvl = US_SolutionVals::values (dbP, edata, solID, svbar, bdens, bvisc,
                                   bcomp, bmanu, errmsg);
  progress_msg->setValue (3);
  ...
  if (bufvl)
    ...
    {
      QMessageBox::warning (this, tr ("Solution/Buffer Fetch"), errmsg);
  ...
  // Calculate basic parameters for other functions [ from us_fematch's
  // ::update()-> data_plot() ]
  ...
  // 	  if ( !loadModel( triple_info_map  ) && !model_exists )
  // 	    {
  // 	      qDebug() << "In loadModel(): No models (2DSA-IT, 2DSA-FM, 2DSA)
  // found for triple -- " << triple_info_map[ "triple_name" ];

  // 	      QMessageBox::critical( this, tr( "No Model Found" ),
  // 				     QString( tr( "In loadModel(): No models
  // (2DSA-IT, 2DSA-FM, 2DSA) found for triple %1" )) 				     .arg( triple_info_map[
  // "triple_name" ] ) );
  ...
  //resplotd->setWindowFlags( Qt::Dialog | Qt::WindowTitleHint |
...
QList<int> *
US_ReporterGMP::rg_excllist ()
{
  return &excludedScans;
}

// public function to get pointer to sim data
US_DataIO::RawData *
US_ReporterGMP::rg_simdata ()
{
  return sdata;
}
...
US_ReporterGMP::rg_resbmap ()
{
  return rbmapd;
}

QString
US_ReporterGMP::rg_tripleInfo ()
{
  return tripleInfo;
}
...
bool
US_ReporterGMP::loadData (QMap<QString, QString> &triple_information)
{
  rawData.clear ();
  editedData.clear ();
  ...
  QString rfilename;
  // int eID=0;
  int eID = Triple_to_ModelsDesc[triple_information["triple_name"]]["eID"]
                .toInt ();

  QString efilename;

  // get EditedData filename && editedDataID for current triple, then infer
  // rawDataID
  QStringList query;
  query << "get_editedDataFilenamesIDs_forReport"
        << triple_information["filename"] << QString::number (eID);
  db->query (query);

  qDebug () << "In loadData() Query: " << query;
  qDebug ()
      << "In loadData() Query: triple_information[ \"triple_name\" ]  -- "
      << triple_information["triple_name"];
  ...
  while (db->next ())
    {
      ...
      // 	      //qDebug() << "Edited profile MAX, NOW, DATE, sec-to-now
      // -- " << latest_update_time << now << date << date.secsTo(now);
  ...
  QString edirpath
      = US_Settings::resultDir () + "/" + triple_information["filename"];
  QDir edir (edirpath);
  if (!edir.exists ())
    edir.mkpath (edirpath);

  QString efilepath = US_Settings::resultDir () + "/"
                      + triple_information["filename"] + "/" + efilename;
  ...
  rfilename = triple_information["filename"] + "." + efilename_copy_list[2]
              + "." + efilename_copy_list[3] + "." + efilename_copy_list[4]
              + "." + efilename_copy_list[5] + ".auc";

  QString rfilepath = US_Settings::resultDir () + "/"
                      + triple_information["filename"] + "/" + rfilename;
  // do we need to check for existance ?
  db->readBlobFromDB (rfilepath, "download_aucData", rID);

  qApp->processEvents ();

  qDebug () << "Loading eData, rawData: efilepath, rfilepath, eID, rID --- "
            << efilepath << rfilepath << eID << rID;
...
// Load rawData/editedData
bool
US_ReporterGMP::loadModel (QMap<QString, QString> &triple_information)
{
  US_Passwd pw;
  US_DB2 *db = new US_DB2 (pw.getPasswd ());
  ...
  // //first, get ModelIDs corresponding to editedDataID AND triple_stage &&
  // select latest one QStringList query; query << "get_modelDescsIDs" <<
  // triple_information[ "eID" ]; db->query( query );
  ...
  // 	{
  // 	  //if contains, it matches & the model exists (e.g. 2DSA-IT); now find
  // the latest one 	  model_exists = true;

  // 	  if ( triple_information[ "stage_name" ] == "2DSA" )
  // 	    {
  // 	      if ( !description.contains("-FM_") &&
  // !description.contains("-IT_") && !description.contains("-MC_") &&
  // !description.contains("_mcN") )
  // 		{
  // 		  int time_to_now = date.secsTo(now);
  // 		  if ( time_to_now < latest_update_time )
  // 		    {
  ...
  // 		      qDebug() << "Model 2DSA: ID, desc, timetonow -- " <<
  // modelID << description << time_to_now;
  ...
  // 		  qDebug() << "Model NON-2DSA: ID, desc, timetonow -- " <<
  // modelID << description << time_to_now;
  ...
  //     // 			     QString (tr( "Triple %1 does not have  %2
  //     model !" ))
  ...
  //     progress_msg->setLabelText( QString("Model %1 is NOT found for triple
  //     %2.\n Trying other models...")
  // 				  .arg( triple_information[ "stage_name" ] )
  // 				  .arg( triple_information[ "triple_name" ] )
  // );
  ...
  int mID = Triple_to_ModelsDesc[triple_information["triple_name"]]
  ...
  qDebug () << "ModelID to retrieve for triple, stage: -- "
            << triple_information["triple_name"]
            << triple_information["stage_name"] << mID;
  ...
  is_dmga_mc = (model.monteCarlo && model.description.contains ("DMGA")
...
// Load Noises
bool
US_ReporterGMP::loadNoises (QMap<QString, QString> &triple_information)
{
  US_Passwd pw;
  US_DB2 *db = new US_DB2 (pw.getPasswd ());
  ...
  while (db->next ())
    {
      ...
      // QString  date                = US_Util::toUTCDatetimeText( db->value(
      // 2 ).toDateTime().toString( "yyyy/MM/dd HH:mm" ), true );
      QDateTime date = db->value (2).toDateTime ();

      QDateTime now = QDateTime::currentDateTime ();

      qDebug () << "Noises: noiseID, noiseType, date -- " << noiseID
                << noiseType << date;
  ...
  // ALEXEY: treat the case when model (like -MC does not possess its own
  // noises -- use latest available noises for prior model like -IT  ) int
  // US_LoadableNoise::count_noise() in ../../gui/us_loadable_noise.cpp void
  // US_FeMatch::load_noise( ) in us_fematch.cpp
  if (!nID_ti && !nID_ri)
  ...
  qDebug () << "ti_noise.count, ri_noise.count: " << ti_noise.count
            << ri_noise.count;
  qDebug () << "ti_noise.values.size(), ri_noise.values.size(): "
            << ti_noise.values.size () << ri_noise.values.size ();
...
void
US_ReporterGMP::loadNoises_whenAbsent ()
{
  ...
  qDebug ()
      << "load_noise_whenAbsent(): mieGUIDs, nieGUIDs, editGUID, modelGUID -- "
      << mieGUIDs << nieGUIDs << editGUID << modelGUID;
  qDebug () << "load_noise_whenAbsent(): noisdf, nenois -- " << noisdf
            << nenois;
          ...
          if (typen == "ti")
            {
              ...
              qDebug () << "load_noise_whenAbsent(): ti_noise created: ID -- "
                        << noiID;
              qDebug () << "load_noise_whenAbsent(): ti_noise.count: -- "
                        << ti_noise.count;
            }
          else
            {
              ri_noise.load (true, noiID, dbP);
              qDebug () << "load_noise_whenAbsent(): ri_noise created: ID -- "
                        << noiID;
              qDebug () << "load_noise_whenAbsent(): ri_noise.count: -- "
                        << ri_noise.count;
            }
              ...
              if (typen == "ti")
                {
                  ti_noise.load (true, noiID, dbP);
                  qDebug ()
                      << "load_noise_whenAbsent(): Noise 2: noiID, loaded ti--"
                      << noiID;
                  qDebug () << "load_noise_whenAbsent(): Noise 2: noiID, "
                               "loaded ti_noise.count --"
                            << ti_noise.count;
                }
              else
                {
                  ri_noise.load (true, noiID, dbP);
                  qDebug ()
                      << "load_noise_whenAbsent(): Noise 2: noiID, loaded ri--"
                      << noiID;
                  qDebug () << "load_noise_whenAbsent(): Noise 2: noiID, "
                               "loaded ri_noise.count --"
                            << ri_noise.count;
                }
            }
        }
...
int
US_ReporterGMP::count_noise_auto (US_DataIO::EditedData *edata,
                                  US_Model *model, QStringList &mieGUIDs,
    ...
    {                           // Search through models in edit
      ...
      if (kenois > 0)
        { // if we have 1 or 2 noises, add to noise-in-edit list
          ...
          lnoisGUID = tmpGUIDs.at (0).section (":", 0, 1) + ":" + modelIndx;
          nieGUIDs << lnoisGUID;
          if (kenois > 1)
            { // add a second noise to the list
              lnoisGUID
                  = tmpGUIDs.at (1).section (":", 0, 1) + ":" + modelIndx;
              nieGUIDs << lnoisGUID;
            }
        ...
        { // Single noise file: check its value range versus experiment
          ...
          if ((noimax - noimin) > (datmax - datmin))
            { // Insert a warning if noise appears corrupt or unusual
              amsg = amsg
                     + tr ("\nBUT THE NOISE HAS AN UNUSUALLY LARGE DATA "
                           "RANGE.\n\n");
            }
...
int
US_ReporterGMP::noises_in_model_auto (QString mGUID, QStringList &nGUIDs)
{
  QString xnGUID;
  QString xmGUID;
  QString xntype;
  ...
  for (int ii = 0; ii < noiIDs.size (); ii++)
    { // Examine noises list; Save to this list if model GUID matches
      ...
      xntype = xntype.contains ("ri_nois", Qt::CaseInsensitive) ? "ri" : "ti";
...
int
US_ReporterGMP::models_in_edit_auto (QString eGUID, QStringList &mGUIDs)
{
  QString xmGUID;
  QString xeGUID;
  QString xrGUID;
...
int
US_ReporterGMP::id_list_db_auto (QString daEditGUID)
{
  QStringList query;
  ...
  for (int ii = modIDs.size () - 1; ii >= 0; ii--)
    { // Work from the back so any removed records do not affect indexes
      ...
      bool mCarlo = (asysID.contains ("-MC") && asysID.contains ("_mc"));
...
// Simulate Model
void
US_ReporterGMP::simulateModel (QMap<QString, QString> &tripleInfo)
{
  ...
  qDebug () << "SimMdl: 0) s D c" << model_used.components[0].s
            << model_used.components[0].D
            << model_used.components[0].signal_concentration << "  n" << lc;
  qDebug () << "SimMdl: n) s D c" << model_used.components[lc].s
            << model_used.components[lc].D
            << model_used.components[lc].signal_concentration;

  adjustModel ();

  qDebug () << "SimMdl: 0) s D c" << model.components[0].s
            << model.components[0].D
            << model.components[0].signal_concentration;
  qDebug () << "SimMdl: n) s D c" << model.components[lc].s
            << model.components[lc].D
  ...
  qDebug () << "SimMdl: simpoints" << simparams.simpoints << "rreso"
            << simparams.radial_resolution << "bottom_sim" << simparams.bottom
            << "bottom_dat" << edata->bottom;
  ...
  if (model.components[0].sigma == 0.0 && model.components[0].delta == 0.0)
    simparams.meshType = US_SimulationParameters::ASTFEM;
  else
  ...
  US_AstfemMath::initSimData (*sdata, *edata,
                              concval1); // Gets experimental time grid set
  ...
  if ((check_file.exists ()) && (check_file.isFile ()))
    {
      if (US_AstfemMath::timestate_onesec (tmst_fpath, *sdata))
        { // Load timestate that is already at 1-second-intervals
          simparams.simSpeedsFromTimeState (tmst_fpath);
          qDebug () << "SimMdl: timestate file exists" << tmst_fpath
                    << " timestateobject,count = " << simparams.tsobj
                    << simparams.sim_speed_prof.count ();
          simparams.speedstepsFromSSprof ();
        }
  ...
  else
    {
      ...
      if (!simparams.sim)
        { // Compute acceleration rate for non-astfem_sim data
          ...
          double rate
              = (double)(simparams.speed_step[0].rotorspeed) / (double)t_acc;
          qDebug () << "SimMdl:  accel-calc:  t1 t2 w2t t_acc speed rate" << t1
                    << t2 << w2t << t_acc << simparams.speed_step[0].rotorspeed
                    << rate;
  ...
  qDebug () << "SimMdl: ssck: rspeed accel1 lo_ss_acc" << rspeed << accel1
            << lo_ss_acc << "tf_aend tf_scan" << tf_aend << tf_scan;
  // x0  1  2  3  4  5
  //  check if the acceleration rate is low or the first scan was taken before
  //  the acceleration ended Due to older, wrong timestate calculation there
  //  might be a case, in which the calculated end of acceleration can be up to
  //  1 second later than expected, tf_scan + 1 accounts for this.
  if (accel1 < lo_ss_acc || tf_aend > (tf_scan + 1))
    {
      QString wmsg = tr ("The TimeState computed/used is likely bad:<br/>"
                         ...
                         .arg (accel1)
                         .arg (QString::number (tf_aend))
                         .arg (QString::number (tf_scan));
      ...
      QPushButton *bAbort
          = msgBox.addButton (tr ("Abort"), QMessageBox::YesRole);
  ...
  */
  ...
  qDebug () << "SimMdl: nthread" << nthread << "ncomp" << ncomp << "ntc" << ntc
  ...
  if (nthread < 999)
    {
      if (model.components[0].sigma == 0.0 && model.components[0].delta == 0.0
          && model.coSedSolute < 0.0 && compress == 0.0)
        {
          ...
          for (int ii = 0; ii < model.components.size (); ii++)
            {
              qDebug () << "SimMdl:   comp" << ii << "s D v"
                        << model.components[ii].s << model.components[ii].D
          ...
          connect (astfem_rsa, SIGNAL (current_component (int)), this,
    ...
    { // Do multi-thread calculations
      ...
      for (int ii = 0; ii < nthread; ii++)
        {
          ThreadWorker *tworker = new ThreadWorker (
              tmodels[ii], simparams, tsimdats[ii], solution_rec.buffer, ii);
          ...
          connect (wthread, SIGNAL (started ()), tworker,
                   SLOT (calc_simulation ()));

          connect (tworker, SIGNAL (work_progress (int, int)), this,
                   SLOT (thread_progress (int, int)));
          connect (tworker, SIGNAL (work_complete (int)), this,
                   SLOT (thread_complete (int)));
...
void
US_ReporterGMP::update_progress (int icomp)
{
...
void
US_ReporterGMP::show_results (QMap<QString, QString> &tripleInfo)
{
  progress_msg->setValue (progress_msg->maximum ());
  qApp->processEvents ();
...
// Check if model exists for particular stage (2DSA-IT, or 2DSA-MC...) by its
// modelGUID (across all triples)
bool
US_ReporterGMP::modelGuidExistsForStage (QString model, QString mguid)
{
  ...
  for (int i = 0; i < Array_of_tripleNames.size (); ++i)
    {
      ...
      if (tmapguid[model] == mguid)
        {
          qDebug () << "For stage: " << model
                    << ", there is modelGuid: " << mguid << " ("
...
//[IND] Check if model exists for particular stage (2DSA-IT, or 2DSA-MC...) by
//its modelGUID (for specified triple)
bool
US_ReporterGMP::modelGuidExistsForStage_ind (QString triple_n, QString model,
                                             QString mguid)
{
  bool isModelGuid = false;
  ...
  for (int i = 0; i < Array_of_tripleNames.size (); ++i)
    {
      ...
      qDebug () << "IN modelGuidExistsForStage_ind(): triple_n, c_triple_n, "
                   "model  -- "
                << triple_n << c_triple_n << model;
      qDebug () << "IN modelGuidExistsForStage_ind(): mguid, "
                   "Triple_to_ModelsDescGuid[ Array_of_tripleNames[ i ] ][ "
                   "model ] -- "
                << mguid
                << Triple_to_ModelsDescGuid[Array_of_tripleNames[i]][model];
      ...
      if (c_triple_n == triple_n)
        {
          ...
          if (tmapguid[model] == mguid)
            {
              qDebug () << "For triple, stage: " << triple_n << model
                        << ", there is modelGuid: " << mguid << " ("
                        << Array_of_tripleNames[i] << ")";
...
void
US_ReporterGMP::process_combined_plots_individual (QString triplesname_p,
                                                   QString stage_model)
{
  QString triplesname_passed = triplesname_p;
  qDebug ()
      << "[in process_combined_plots_individual()]: triplesname_passed -- "
      << triplesname_passed;
  QString filename_passed = get_filename (triplesname_p);
  qDebug () << "[in process_combined_plots_individual()]: filename_passed -- "
            << filename_passed;
  QString triplesname = triplesname_p.replace (".", "");
  qDebug () << "[in process_combined_plots_individual()]: triplesname -- "
            << triplesname;
  ...
  // QStringList modelDescModified = sdiag_combplot->load_auto( runIDs_single,
  // aDescrs );
  QList<QStringList> modelDescModifiedList
      = sdiag_combplot->load_auto (runIDs_single, aDescrs);
  QStringList modelDescModified = modelDescModifiedList[0];
  QStringList modelDescModifiedGuid = modelDescModifiedList[1];

  qDebug () << "[IND] ComboPlots generation: modelDescModified -- "
            << modelDescModified;
  qDebug () << "[IND] ComboPlots generation: modelDescModifiedGuid -- "
            << modelDescModifiedGuid;
  ...
  // Choose model from modelDescModifiedGuid matching triplesname &&
  // stage_model: bool isModel = false; QString imgComb01File = basename +
  // "Ind_combined" + "." + triplesname + "." + stage_model + ".s20" + svgext;
  ...
  for (int ii = 0; ii < modelDescModified.size (); ii++)
    {
      ...
      if (triplesname.contains ("Interference"))
        {
          ...
          triplesname_chann = triplesname_passed.split (".")[0]
                              + triplesname_passed.split (".")[1] + ":Interf.";
        }
      else
        triplesname_chann = triplesname_passed.split (".")[0]
                            + triplesname_passed.split (".")[1] + ":UV/vis.";
      ...
      if (modelDescModified[ii].contains (triplesname_mod)
          ...
          && modelGuidExistsForStage_ind (triplesname, stage_model,
                                          modelDescModifiedGuid[ii]))
        {
          ...
          // compose map of [{"s_ranges","k_ranges"}, etc] from cAP2 & cAPp
          // (for given channel & model!!!)
          QMap<QString, QStringList> sim_ranges
              = find_sim_ranges (triplesname_chann, stage_model);

          QString t_m = "s," + stage_model;
          QMap<QString, QString> c_params = comboPlotsMap[t_m];
          // ALEXEY: here it plots s20 combPlot (xtype == 0) -- Need to do
          // first thing!!!
          plotted_ids_colors_map_s_type = sdiag_combplot->model_select_auto (
              modelDescModified[ii], c_params);

          // Check if to plot individual Combined distributons:
          // {s,D,f/f0,MW,Radius} {2DSA-IT,2DSA-MC,PCSA,raw} {p_key:
          // {s[3.2:3.7], D[11:15], etc.} }
          // MaskStr.ShowTripleTypeModelRangeIndividualCombo[ t_name ][ s_name
          // ][ p_key ] = feature_indCombo_value; // 1/0
          QMap<QString, QStringList> ind_compoplots_type_ranges;

          qDebug () << "BEFORE INDCOMBO_4: triplesname, stage_model -- "
                    << triplesname << stage_model;

          QMap<QString, QString> ind_comboplots
              = perChanMask_edited
                    .ShowTripleTypeModelRangeIndividualCombo[triplesname]
                                                            [stage_model];
          QMap<QString, QString>::iterator i_cp;
          for (i_cp = ind_comboplots.begin (); i_cp != ind_comboplots.end ();
               ++i_cp)
            {
              ...
              qDebug () << "INDCOMBO_4: map's parms -- " << type_range
                        << i_cp.value ().toInt () << type << range;
          ...
          // plot different types {s,D,MW...} types:  0: s20; 1: MW; 2: D; 3:
          // f/f0
          QMap<QString, QStringList>::iterator i_cpt;
          for (i_cpt = ind_compoplots_type_ranges.begin ();
               i_cpt != ind_compoplots_type_ranges.end (); ++i_cpt)
            {
              ...
              QString t_m;
              QString imgComb02File = basename + "Ind_combined" + "."
                                      + triplesname + "." + stage_model;

              qDebug () << "in the ind_compoplots_type_ranges QMap: type, "
                           "ranges --  "
                        << type << ranges;
                  ...
                  if (sim_ranges.contains ("s_ranges"))
                    {
                      ...
                      qDebug ()
                          << "s-type: sim_ranges.keys(), "
                             "sim_ranges[\"s_ranges\"] -- "
                          << sim_ranges.keys () << sim_ranges["s_ranges"];
                    }

                  // qDebug() << "over models: c_params -- " << c_params;

                  // ALEXEY: here it plots s20 combPlot (xtype == 0)
                  plotted_ids_colors_map_s_type
                      = sdiag_combplot->changedPlotX_auto (0, c_parms);

                  write_plot (
                      imgComb02File,
                      sdiag_combplot->rp_data_plot1 ()); //<-- rp_data_plot1()
                                                         //gives combined plot
                  imgComb02File.replace (svgext, pngext);
                  CombPlotsFileNames << imgComb02File;

                  CombPlotsParmsMap[imgComb02File]
                      = plotted_ids_colors_map_s_type.firstKey ();
                  CombPlotsParmsMap_Colors[imgComb02File]
                      = plotted_ids_colors_map_s_type
                          [plotted_ids_colors_map_s_type.firstKey ()];
                }
              // type: 1:MW
              else if (type == "MW")
                {
                  ...
                  plotted_ids_colors_map_s_type
                      = sdiag_combplot->changedPlotX_auto (1, c_parms);
              ...
              // type 2:D
              else if (type == "D")
                {
                  ...
                  plotted_ids_colors_map_s_type
                      = sdiag_combplot->changedPlotX_auto (2, c_parms);
              ...
              // type 3:f/f0
              else if (type == "f/f0")
                {
                  ...
                  plotted_ids_colors_map_s_type
                      = sdiag_combplot->changedPlotX_auto (3, c_parms);
...
US_ReporterGMP::find_sim_ranges (QString chann_desc, QString model)
{
  QMap<QString, QStringList> sim_ranges;
...
// Combined Plots
void
US_ReporterGMP::process_combined_plots (QString filename_passed)
{
  sdiag_combplot = new US_DDistr_Combine ("REPORT");
  QStringList runIDs_single;
  ...
  qDebug () << "ComboPlots generation: modelDescModified -- "
            << modelDescModified;
  qDebug () << "ComboPlots generation: modelDescModifiedGuid -- "
            << modelDescModifiedGuid;
  ...
  const QString csvext (".csv");
  QString basename = dirName + "/" + filename_passed + ".";
  // QString basename  = US_Settings::reportDir() + "/" + filename_passed + "/"
  // + filename_passed + ".";

  // estimate # of combined plots
  int combpl_number = 3 * 3;
  // Show msg while data downloaded and simulated
  progress_msg = new QProgressDialog (QString ("Generating combined plots..."),
                                      QString (), 0, combpl_number, this);
  progress_msg->setWindowFlags (Qt::Window | Qt::WindowTitleHint
                                | Qt::CustomizeWindowHint);
  progress_msg->setWindowModality (Qt::WindowModal);
  ...
  xtype << 1 << 2 << 3; // ALEXEY: 0: s20; 1: MW; 2: D; 3: f/f0
                        // Note: xtype==0 (s20) is the default, so iterate
                        // later starting from 1...
  QStringList CombPlotsFileNames;
  QStringList plottedIDs_s, plottedIDs_other_type;
  ...
  for (int m = 0; m < modelNames.size (); m++)
    {
      bool isModel = false;
      QString imgComb01File
          = basename + "combined" + "." + modelNames[m] + ".s20" + svgext;

      for (int ii = 0; ii < modelDescModified.size (); ii++)
        {
          // fiter by type|model
          if (modelDescModified[ii].contains (modelNames[m])
              && modelGuidExistsForStage (modelNames[m],
                                          modelDescModifiedGuid[ii]))
            {
              isModel = true;
              ...
              // qDebug() << "over models: c_params -- " << c_params;
              plotted_ids_colors_map_s_type
                  = sdiag_combplot->model_select_auto (
                      modelDescModified[ii],
                      c_params); // ALEXEY: here it plots s20 combPlot (xtype
                                 // == 0)
            }
        }
      ...
      // write plot
      if (isModel) // TEMPORARY: will read a type-method combined plot QMap
                   // defined at the beginnig
        {
          // here identify what to show:
          bool show_combo_s
              = (combPlotsMask_edited.ShowCombPlotParts["s"][modelNames[m]]
                     .toInt ())
                    ? true
                    : false;
          bool show_combo_D
              = (combPlotsMask_edited.ShowCombPlotParts["D"][modelNames[m]]
                     .toInt ())
                    ? true
                    : false;
          bool show_combo_ff0
              = (combPlotsMask_edited.ShowCombPlotParts["f/f0"][modelNames[m]]
                     .toInt ())
                    ? true
                    : false;
          bool show_combo_MW
              = (combPlotsMask_edited.ShowCombPlotParts["MW"][modelNames[m]]
                     .toInt ())
                    ? true
                    : false;

          qDebug () << "In process ComboPlots: Model: show_combo_s, "
                       "show_combo_D, show_combo_ff0, show_combo_MW -- "
                    << modelNames[m] << ": " << show_combo_s << show_combo_D
                    << show_combo_ff0 << show_combo_MW;

          // here writes a 's'-type IF it's to be included:
          //  QString t_m = "s," + modelNames[ m ];
          //  if ( comboPlotsMapTypes.contains( t_m ) && comboPlotsMapTypes[
          //  t_m ] != 0  )
          if (show_combo_s)
            {
              ...
              CombPlotsParmsMap[imgComb01File]
                  = plotted_ids_colors_map_s_type.firstKey ();
              CombPlotsParmsMap_Colors[imgComb01File]
                  = plotted_ids_colors_map_s_type[plotted_ids_colors_map_s_type
                                                      .firstKey ()];
            }
          ...
          for (int xt = 0; xt < xtype.size (); ++xt)
            {
              QString imgComb02File
                  = basename + "combined" + "." + modelNames[m];
              QMap<QString, QString> c_parms;
              QString t_m, c_type;
              ...
              // if ( comboPlotsMapTypes.contains( t_m ) && comboPlotsMapTypes[
              // t_m ] != 0  )
              bool show_combo_plot_other_types
                  = (combPlotsMask_edited
                         .ShowCombPlotParts[c_type][modelNames[m]]
                         .toInt ())
                        ? true
                        : false;
              if (show_combo_plot_other_types)
                {
                  ...
                  // plottedIDs_other_type = sdiag_combplot->
                  // changedPlotX_auto( xtype[ xt ], c_parms );
                  plotted_ids_colors_map_s_type
                      = sdiag_combplot->changedPlotX_auto (xtype[xt], c_parms);

                  // qDebug() << "PLOTTED_IDs_" << c_type << "_type -- " <<
                  // plottedIDs_other_type;
...
// Plot pseudo3d distr.
void
US_ReporterGMP::plot_pseudo3D (QString triple_name, QString stage_model)
{
  ...
  // Pseudo3D Plots Generation (after simulations? Actual simulations are NOT
  // needed for Pseudo3d plots?) FileName; //<-- a single-type runID (e.g. RI)
  // - NOT combined runs for now...
  ...
  // QString basename  = US_Settings::reportDir() + "/" + filename_returned +
  // "/" + filename_returned + ".";
  ...
  bool show_s_ff0
      = (perChanMask_edited
             .ShowTripleModelPseudo3dParts[t_name][stage_model]
                                          ["Pseudo3d s-vs-f/f0 Distribution"]
             .toInt ())
            ? true
            : false;
  ...
  if (show_s_ff0)
    {
      ...
      imgPseudo3d01File = basename + "pseudo3D" + "." + stage_model + "."
                          + t_name + ".sff0" + svgext; // [s-f/f0]
      write_plot (imgPseudo3d01File,
                  sdiag_pseudo3d->rp_data_plot ()); //<-- rp_data_plot() gives
                                                    //pointer to pseudo3D plot
...
US_ReporterGMP::calc_residuals ()
{
  ...
  int kpts = 0;
  qDebug () << "CALC_RESID: matchd" << matchd << "dsize ssize" << dsize
            << ssize;
  int kexcls = 0;
  ...
  qDebug () << "CALC_RESID: matchd" << matchd << "kexcls" << kexcls << "rmsd"
            << rmsd;
}

// output HTML string for run details
void
US_ReporterGMP::assemble_run_details_html (void)
{
  ...
  for (int i = 0; i < fileNameList.size (); ++i)
    {
      ...
      // html_assembled += tr( "<h3 align=left>TimeStamp Parameters for Run:
      // %1</h3>" ). 	arg( fileNameList[ i ] );
      ...
      QString cksumd;
      QDateTime datedt;
      US_TimeState::dbExamine (dbP, &tmstID, &experimentID, &tfname, &xdefs,
                               &cksumd, &datedt);
      ...
      if (tmst_dname.right (1) != "/")
        tmst_dname += "/";
      qDebug () << "Assembling Run Details: tmst_dname" << tmst_dname
      ...
      if (tmsfs.count () == 2)
        { // Looks like we have a TMST and corresponding Defs XML
          tmst_fnamei = tmsfs[0];
          defs_fnamei = tmsfs[1];
          qDebug () << "Assembling Run Details: tmsfs" << tmst_fnamei
                    << defs_fnamei;

          if (tmst_fnamei.contains (".tmst") && defs_fnamei.contains (".xml"))
            { // Have both files, so save full path to TMST
              tmst_fnamei = tmst_dname + tmst_fnamei;
            }

          else if (tmsfs[0].contains (".xml") && tmsfs[1].contains (".tmst"))
          ...
          qDebug () << "Assembling Run Details: tmst,defs fnamei"
                    << tmst_fnamei << defs_fnamei;
        }
      else // If file does not exist, clear name
        {
          qDebug () << "Assembling Run Details: TimeStamp NON_EXIST:"
                    << tmst_fnamei;
          tmst_fnamei.clear ();
        }
      ...
      // QMap< QString, double > davgs_first_scan = tsdiag ->
      // timestamp_data_avgs_first_scan();
      QMap<QString, QMap<QString, double> > davgs_stdd_first_scan
          = tsdiag->timestamp_data_avgs_stdd_first_scan ();

      html_assembled += tr ("<table style=\"margin-left:10px\">");

      html_assembled += table_row (tr ("Parameter: "), tr ("Target Value:"),
                                   tr ("Tolerance:"), tr ("Measured Value:"),
                                   tr ("Std. Dev."), tr ("PASSED ?"));

      // Iterate over Time Stamp params...
      for (int i = 0; i < dkeys.size (); ++i)
        {
          qDebug () << "Assembling Run Details: " << dkeys[i]
                    << dmins[dkeys[i]] << " to " << dmaxs[dkeys[i]]
                    << "; Avg: " << davgs[dkeys[i]] << "; Avg_1st_scan: "
                    << davgs_stdd_first_scan[dkeys[i]]["Avg"]
                    << "; Stdd_1st_scan: "
                    << davgs_stdd_first_scan[dkeys[i]]["Stdd"];
          ...
          if (dkeys[i] == "Time")
            {
              val_target = currProto.rpSpeed.ssteps[0].duration
                           + (double)(currProto.rpSpeed.ssteps[0].speed
                                      / currProto.rpSpeed.ssteps[0].accel);
              qDebug ()
                  << "Assembling Run Details: exp. duration, speed, accel -- "
                  << currProto.rpSpeed.ssteps[0].duration
                  << currProto.rpSpeed.ssteps[0].speed
                  << currProto.rpSpeed.ssteps[0].accel;
              int hours_r = (int)qFloor (val_target / 3600.0);
              int mins_r
                  = (int)qRound ((val_target - hours_r * 3600.0) / 60.0);
              QString hh_r = "h";
              QString val_target_hh_mm = QString ().sprintf (
                  "%d %s %02d m", hours_r, hh_r.toLatin1 ().data (), mins_r);
              ...
              mins_r
                  = (int)qRound ((val_measured_sec - hours_r * 3600.0) / 60.0);
              QString val_measured_hh_mm = QString ().sprintf (
                  "%d %s %02d m", hours_r, hh_r.toLatin1 ().data (), mins_r);

              val_passed
                  = (val_measured_sec >= (val_target * (1 - val_tol / 100.0))
                     && val_measured_sec
                            <= (val_target * (1 + val_tol / 100.0)))
                        ? "YES"
                        : "NO";

              html_assembled += table_row (tr ("Duration"), val_target_hh_mm,
                                           QString::number (val_tol) + "%",
                                           val_measured_hh_mm, "", val_passed);
            }
          ...
          if (dkeys[i].contains ("Temperature"))
            {
              ...
              val_passed = (val_measured >= (val_target - val_tol)
                            && val_measured <= (val_target + val_tol))
                               ? "YES"
                               : "NO";

              val_measured_stdd = davgs_stdd_first_scan[dkeys[i]]["Stdd"];

              html_assembled += table_row (
                  dkeys[i] + " (&#8451;)", QString::number (val_target),
                  "&#177;" + QString::number (val_tol),
                  QString::number (val_measured),
                  QString::number (val_measured_stdd), val_passed);

              // Plot Temp. plot
              QString img01File
                  = dirName + "/" + "RunDetails_Temperature" + svgext;
          ...
          if (dkeys[i].contains ("RawSpeed"))
            {
              ...
              html_assembled += table_row (
                  "Speed (RPM)", QString::number (val_target),
                  "&#177;" + QString::number (val_tol),
                  QString::number (val_measured), "", val_passed);
          ...
          // html_assembled += tr(
          // 		       "<tr><td>%1: </td> <td>%2 to %3</td> <td>Avg.:
          // %4</td></tr>"
          // 		       )
          //   .arg( dkeys[ i ] )                 //1
          //   .arg( dmins[ dkeys[ i ] ] )        //2
...
// get expID
int
US_ReporterGMP::get_expID_by_runID_invID (US_DB2 *dbP, QString runID_filename)
{
  QStringList query;
  int idExp = 0;
  query << "get_experiment_info_by_runID" << runID_filename
        << QString::number (invID);
...
void
US_ReporterGMP::assemble_user_inputs_html (void)
{
  html_assembled += "<p class=\"pagebreak \">\n";
  html_assembled += "<h2 align=left>User Interactions:\n GMP Run Initiation, "
                    "Live Update, Data Import, Editing, and Analysis</h2>";
  ...
  // importRIJson =
  //   "{ \"Person\":
  //   [{\"ID\":\"12\",\"fname\":\"Alexey\",\"lname\":\"Savelyev\",\"email\":\"oleksiy.savelyev@umontana.edu\",\"level\":\"4\"}],\"RefScan\":
  //   \"automated\"}";
  // importIPJson =
  //   "{ \"Person\":
  //   [{\"ID\":\"12\",\"fname\":\"Alexey\",\"lname\":\"Savelyev\",\"email\":\"oleksiy.savelyev@umontana.edu\",\"level\":\"4\"}],\"RefScan\":
  ...
  // editRIJson =
  //   "{ \"Person\":
  //   [{\"ID\":\"12\",\"fname\":\"Alexey\",\"lname\":\"Savelyev\",\"email\":\"oleksiy.savelyev@umontana.edu\",\"level\":\"4\"}],\"Meniscus\":
  //   [{\"1 / A\":\"automated\",\"1 / B\":\"manual\",\"2 /
  //   A\":\"automated\",\"2 / B\":\"automated\", \"3 / A\":\"automated\",\"3 /
  //   B\":\"manual\",\"4 / A\":\"automated\",\"4 / B\":\"automated\"}]}";
  // editIPJson = "{ \"Person\":
  // [{\"ID\":\"12\",\"fname\":\"Alexey\",\"lname\":\"Savelyev\",\"email\":\"oleksiy.savelyev@umontana.edu\",\"level\":\"4\"}],\"Meniscus\":
  // [{\"1 / A\":\"manual\"}]}"; editRIts = "2022-03-29 22:24:36"; editIPts =
  // "2022-03-30 23:26:36";
  // // END of DEBUG ////////////

  // read autoflowStatus record:
  read_autoflowStatus_record (
      importRIJson, importRIts, importIPJson, importIPts, editRIJson, editRIts,
      editIPJson, editIPts, analysisJson, stopOptimaJson, stopOptimats,
      skipOptimaJson, skipOptimats, analysisCancelJson, createdGMPrunJson,
      createdGMPrunts, analysisABDEJson, analysisABDEts);
  /////////////////////////////

  // 1. GMP run creation
  html_assembled += tr ("<hr>");
  html_assembled
      += tr ("<h3 align=left>GMP Run Initiation (1. EXPERIMENT)</h3>");
  QMap<QString, QMap<QString, QString> > status_map_c
      = parse_autoflowStatus_json (createdGMPrunJson, "");

  // html_assembled += tr("<br>");
  html_assembled
      += tr ("<table style=\"margin-left:10px\">"
  ...
  html_assembled += tr ("<table style=\"margin-left:10px\">"
                        "<caption align=left> <b><i>Time of GMP Run "
                        "Initiation: </i></b> </caption>"
                        "</table>"
  ...
  html_assembled += tr ("<table style=\"margin-left:10px\">"
                        "<caption align=left> <b><i>Comment at the Time of "
                        "GMP Run Initiation: </i></b> </caption>"
                        "</table>"
  ...
  if (!stopOptimaJson.isEmpty () || !skipOptimaJson.isEmpty ())
    html_assembled += tr ("<h3 align=left>Remote Stage Skipping, Stopping "
                          "Machine (2. LIVE_UPDATE)</h3>");

  for (im = operation_types_live_update.begin ();
       im != operation_types_live_update.end (); ++im)
    {
      ...
      QMap<QString, QMap<QString, QString> > status_map
          = parse_autoflowStatus_json (json_str, im.key ());
      ...
      html_assembled
          += tr ("<table style=\"margin-left:10px\">"
                 "<caption align=left> <b><i>Operation, Timestamp: </i></b> "
                 "</caption>"
                 "</table>"
      ...
      QString t_comment = status_map["Comment"]["comment"].isEmpty ()
                              ? "N/A"
                              : status_map["Comment"]["comment"];
      html_assembled += tr ("<table style=\"margin-left:10px\">"
                            "<caption align=left> <b><i>Reason for Operation: "
                            "</i></b> </caption>"
                            "</table>"
  ...
  QStringList dropped_triples_RI, dropped_triples_IP;
  read_reportLists_from_aprofile (dropped_triples_RI, dropped_triples_IP);
  qDebug () << "List of dropped triples (all OSs): " << dropped_triples_RI
            << dropped_triples_IP;

  html_assembled += tr ("<h3 align=left>Reference Scan Determination, Triples "
                        "Dropped, Data Saving (3. IMPORT)</h3>");
  ...
  for (im = data_types_import.begin (); im != data_types_import.end (); ++im)
    {
      ...
      QString ref_scan_method = (dataSource.contains ("Absorbance"))
                                    ? "N/A"
                                    : status_map["RefScan"]["type"];
      html_assembled += tr ("<table style=\"margin-left:10px\">"
                            "<caption align=left> <b><i>Reference Scan, Data "
                            "Saving: </i></b> </caption>"
                            "</table>"
      ...
      html_assembled += tr ("<table style=\"margin-left:10px\">"
                            "<caption align=left> <b><i>Comment at the Time "
                            "of Data Saving: </i></b> </caption>"
                            "</table>"
                            ...
                            "</tr>"
                            "</table>")
                            .arg (status_map["Comment when SAVED"]
                                            ["comment_when_saved"]) // 1
          ;
      ...
      if (!dtype_opt_dropped_triples.isEmpty ())
        {
          html_assembled
              += tr ("<table style=\"margin-left:10px\">"
                     "<caption style=\"color:red;\" align=left> <b><i>List of "
                     "Dropped Triples: </i></b> </caption>"
                     "</table>"
          ...
          for (int i = 0; i < dtype_opt_dropped_triples.size (); ++i)
            {
              html_assembled += tr ("<tr>"
                                    "<td> Triple Name: </td> <td "
                                    "style=\"color:red;\"> %1 </td> "
                                    "</tr>")
      ...
      if (status_map.contains ("Dropped"))
        {
          ...
          html_assembled
              += tr ("<table style=\"margin-left:10px\">"
                     "<caption align=left> <b><i>Comments on [triples | "
                     "channels | select channel] dropped: </i></b> </caption>"
                     "</table>"

                     "<table style=\"margin-left:25px\">");

          QMap<QString, QString>::iterator dr;
          for (dr = status_map["Dropped"].begin ();
               dr != status_map["Dropped"].end (); ++dr)
            {
              html_assembled += tr ("<tr>"
                                    "<td> Dropped:     %1 </td>"
                                    "<td> Comment:     %2 </td>"
  ...
  html_assembled += tr ("<h3 align=left>Meniscus Position Determination, Edit "
                        "Profiles Saving (4. EDITING)</h3>");
  ...
  for (im = data_types_edit.begin (); im != data_types_edit.end (); ++im)
    {
      ...
      html_assembled += tr ("<table style=\"margin-left:10px\">"
                            "<caption align=left> <b><i>Meniscus Position "
                            "Determination: </i></b> </caption>"
                            "</table>"

                            "<table style=\"margin-left:25px\">");

      QMap<QString, QString>::iterator mp;
      for (mp = status_map["Meniscus"].begin ();
           mp != status_map["Meniscus"].end (); ++mp)
        {
          ...
          if (expType == "ABDE")
            {
              ...
              QString menisc_val;
              if (mwl_abde)
                menisc_val = QString::number (
                    abde_menisc[chann_str.replace (" / ", "")]);
              else
                {
                  QString chann_swl = chann_str.split (" / ")[0]
                                      + chann_str.split (" / ")[1];
      ...
      // Edit Profiles Saved:
      html_assembled += tr ("<table style=\"margin-left:10px\">"
                            "<caption align=left> <b><i>Edit Profiles Saved "
                            "on: </i></b> </caption>"
                            "</table>"
...
void
US_ReporterGMP::user_interactions_analysis_abde (QString analysisABDEJson,
                                                 QString analysisABDEts)
{
  html_assembled
      += tr ("<h3 align=left>ABDE Profile Processing (5. ANALYSIS)</h3>");
  QMap<QString, QMap<QString, QString> > status_map_c
      = parse_autoflowStatus_json (analysisABDEJson, "");
  ...
  html_assembled += tr ("<table style=\"margin-left:10px\">"
                        "<caption align=left> <b><i>Time of ABDE profiles "
                        "processing: </i></b> </caption>"
                        "</table>"
  ...
  html_assembled += tr ("<table style=\"margin-left:10px\">"
                        "<caption align=left> <b><i>Comment at the Time of "
                        "ABDE Profile Processing: </i></b> </caption>"
                        "</table>"
...
void
US_ReporterGMP::user_interactions_analysis (QString analysisJson,
                                            QString analysisCancelJson)
{
  html_assembled += tr ("<h3 align=left>Meniscus Position from FITMEN Stage, "
                        "Job Cancellation (5. ANALYSIS)</h3>");

  QMap<QString, QString> analysis_status_map
      = parse_autoflowStatus_analysis_json (analysisJson);
  QMap<QString, QString> analysisCancel_status_map
      = parse_autoflowStatus_analysis_json (analysisCancelJson);
  ...
  if (!cAP2.job3auto) // interactive FITMEN (manual)
    {
      ...
      html_assembled
          += tr ("<table style=\"margin-left:10px\">"
                 "<caption align=left> <b><i>Meniscus Position Determination "
                 "from FITMEN_MANUAL stage: </i></b> </caption>"
                 "</table>"

                 "<table style=\"margin-left:25px\">");

      QMap<QString, QString>::iterator mfa;
      for (mfa = analysis_status_map.begin ();
           mfa != analysis_status_map.end (); ++mfa)
        {
    ...
    {
      html_assembled
          += tr ("Meniscus positions have been determined automatically as "
                 "best fit values for all channels.");
    }

  // Now add info on the CANCELED Jobs as captured in DB:
  html_assembled += tr ("<table style=\"margin-left:10px\">"
                        "<caption align=left> <b><i>Information on CANCELED "
                        "analysis jobs: </i></b> </caption>"
                        "</table>"
  ...
  if (!analysisCancelJson.isEmpty ())
    {
      ...
      for (cj = analysisCancel_status_map.begin ();
           cj != analysisCancel_status_map.end (); ++cj)
        {

          QString cj_value = cj.value ();
          QString performed_by_reason_time
              = cj_value.split ("CANCELED, by")[1];
...
void
US_ReporterGMP::read_reportLists_from_aprofile (
    QStringList &dropped_triples_RI, QStringList &dropped_triples_IP)
{
  ...
  if (!aprofile_xml.isEmpty ())
    {
      ...
      readReportLists (xmli, channame_to_reportIDs_RI,
                       channame_to_reportIDs_IP);
    }
...
QStringList
US_ReporterGMP::buildDroppedTriplesList (
    US_DB2 *dbP, QMap<QString, QString> channame_to_reportIDs)
{
  QStringList dropped_triples_list;

  QMap<QString, QString>::iterator chan_rep;
  for (chan_rep = channame_to_reportIDs.begin ();
       chan_rep != channame_to_reportIDs.end (); ++chan_rep)
    {
      QString chan_key = chan_rep.key ();
      QString reportIDs = chan_rep.value ();
      qDebug () << "Channel name -- " << chan_key << ", reportIDs -- "
                << reportIDs;
...
// Read AProfile's reportIDs per channel:
bool
US_ReporterGMP::readReportLists (
    QXmlStreamReader &xmli, QMap<QString, QString> &channame_to_reportIDs_RI,
    QMap<QString, QString> &channame_to_reportIDs_IP)
{
  while (!xmli.atEnd ())
    {
          ...
          if (ename == "channel_parms")
            {
              ...
              if (attr.hasAttribute (
                      "load_volume")) // ensure it reads upper-level
                                      // <channel_parms>
                {
                  ...
                  QString opsys
                      = channel_desc.split (":")[1]; // UV/vis. or Interf.

                  if (opsys.contains ("UV/vis")) // RI
                    {
                      // Read what reportID corresponds to channel:
                      if (attr.hasAttribute ("report_id"))
                        channame_to_reportIDs_RI[channel_name]
                            = attr.value ("report_id").toString ();
                    }
                  if (opsys.contains ("Interf")) // IP
                    {
                      // Read what reportID corresponds to channel:
                      if (attr.hasAttribute ("report_id"))
                        channame_to_reportIDs_IP[channel_name]
                            = attr.value ("report_id").toString ();
                    }
                }
            }
...
void
US_ReporterGMP::read_autoflowStatus_record (
    QString &importRIJson, QString &importRIts, QString &importIPJson,
    QString &importIPts, QString &editRIJson, QString &editRIts,
    QString &editIPJson, QString &editIPts, QString &analysisJson,
    QString &stopOptimaJson, QString &stopOptimats, QString &skipOptimaJson,
    QString &skipOptimats, QString &analysisCancelJson,
    QString &createdGMPrunJson, QString &createdGMPrunts,
    QString &analysisABDEJson, QString &analysisABDEts)
{
  ...
  if (db.lastErrno () != US_DB2::OK)
    {
      QMessageBox::warning (
          this, tr ("Connection Problem: Read autoflowStatus"),
          tr ("Could not connect to database \n") + db.lastError ());
      return;
    }
  ...
  qDebug () << "Read_autoflow_status: stopOptimaJson, skipOptimaJson, "
               "analysisJson -- "
            << stopOptimaJson << skipOptimaJson << analysisJson;
}

// Parse autoflowStatus Analysis Json
QMap<QString, QString>
US_ReporterGMP::parse_autoflowStatus_analysis_json (QString statusJson)
{
          ...
          foreach (const QString &key, json_array[i].toObject ().keys ())
            {
              ...
              qDebug () << "ANALYSIS_JSON [ARRAY]: key, value: " << key
                        << value.toString ();
...
QMap<QString, QMap<QString, QString> >
US_ReporterGMP::parse_autoflowStatus_json (QString statusJson, QString dtype)
{
  QMap<QString, QMap<QString, QString> > status_map;
  ...
  foreach (const QString &key, json_obj.keys ())
    {
          ...
          for (int i = 0; i < json_array.size (); ++i)
            {
              foreach (const QString &array_key,
                       json_array[i].toObject ().keys ())
                {
                  person_map[array_key] = json_array[i]
                                              .toObject ()
                                              .value (array_key)
                                              .toString ();
                  qDebug () << "Person Map: -- key, value: " << array_key
                            << json_array[i]
                                   .toObject ()
                                   .value (array_key)
                                   .toString ();
      ...
      if (key == "Dropped") // import: Dropped triples/channels/select channels
                            // operaitons
        {
          QJsonArray json_array = value.toArray ();
          ...
          for (int i = 0; i < json_array.size (); ++i)
            {
              foreach (const QString &array_key,
                       json_array[i].toObject ().keys ())
                {
                  dropped_map[array_key] = json_array[i]
                                               .toObject ()
                                               .value (array_key)
                                               .toString ();
                  qDebug () << "Dropped Map: -- key, value: " << array_key
                            << json_array[i]
                                   .toObject ()
                                   .value (array_key)
                                   .toString ();
          ...
          for (int i = 0; i < json_array.size (); ++i)
            {
              foreach (const QString &array_key,
                       json_array[i].toObject ().keys ())
                {
                  meniscus_map[array_key] = json_array[i]
                                                .toObject ()
                                                .value (array_key)
                                                .toString ();
                  qDebug () << "Meniscus Map: -- key, value: " << array_key
                            << json_array[i]
                                   .toObject ()
                                   .value (array_key)
                                   .toString ();
      ...
      if (key == "Remote Operation") // Live Update's remote operations [SKIP |
                                     // STOP]
        {
          status_map[key]["type"] = value.toString ();
        }
      if (key == "Comment") // GMP init || Live Update's remote operations
                            // [SKIP | STOP]
        {
...
void
US_ReporterGMP::assemble_distrib_html (QMap<QString, QString> &tripleInfo)
{
...
// output HTML string for Distributions for current triple:
void
US_ReporterGMP::assemble_distrib_ABDE_html (QString &abde_channame)
{
...
QString
US_ReporterGMP::distrib_info_abde (QString &abde_channame)
{
  // TimeStamps
  ...
  if (mwl_abde)
    {
      ...
      QMap<QString, QString> channs_analytes_buffers
          = get_channels_analytes_mwl_abde (abde_channame);

      QMap<QString, QString>::iterator ab;
      for (ab = channs_analytes_buffers.begin ();
           ab != channs_analytes_buffers.end (); ++ab)
  ...
  if (mwl_abde)
    abde_dist_info = "Radial Pos., DNA Data (Norm.), Protein Data (Norm.), "
                     "Integral DNA (Norm), Integral Protein (Norm.)";
  else
    abde_dist_info
        = "Radial Pos., Sample Data (Norm.), Sample Integral (Norm.)";
  html_abde_s += abde_dist_info + "<br>";

  QMap<QString, QVector<QVector<double> > > abde_data
      = abde_data_per_channel[abde_channame];
  ...
  for (int ii = 0; ii < xvals_size; ii++)
    {
      ...
      if (mwl_abde)
        ...
        {
          abde_dist_info = QString ().sprintf ("%10.4e", x_val) + ", "
                           + QString ().sprintf ("%10.4e", y_dna_val) + ", "
                           + QString ().sprintf ("%10.4e", yp_int_dna_val);
        }
  ...
  // create a .pdf
  QString f_model_path
      = dirName + "/" + "ABDE_distro_" + abde_channame + ".pdf";
  ...
  mstr += "<a href=\"./" + f_model_path_str_only
          + "\">View Model Distributions</a>";
  ...
  if (do_integration)
    {
      ...
      mstr += "\n" + indent (2)
              + tr ("<h3>Integration Results: Fraction of Total "
                    "Concentration:</h3>\n");
      mstr += indent (2) + "<table>\n";
      mstr += table_row (tr ("Type:"), tr ("Range:"),
                         tr ("Fraction % from Model (target):"),
                         tr ("Tolerance, %:"), tr ("PASSED ?"));
      for (int kk = 0; kk < report_items_number; ++kk)
        {
          ...
          QString range = "[" + QString::number (low) + " - "
                          + QString::number (high) + "]";
          QString range_alt
              = QString::number (low) + "-" + QString::number (high);
          ...
          QString tot_frac_passed
              = (qAbs (frac_tot_m - frac_tot_r) <= frac_tot_tol_r) ? "YES"
                                                                   : "NO";

          // reportGMP-> reportItems[ kk ]. integration_val_sim = int_val_m;
          // reportGMP-> reportItems[ kk ]. total_percent_sim   = frac_tot_m;
          // reportGMP-> reportItems[ kk ]. passed              =
          // tot_frac_passed;

          qDebug () << "In distrib_info(), fill simulated integration vals: "
                       "for chann/wvl/type/method/low/high, "
                    << "Inter. val. Sim -- " << wvl_abde << curr_item.type
                    << curr_item.method << curr_item.range_low
                    << curr_item.range_high << int_val_m;

          mstr
              += table_row (type, range,
                            QString ().sprintf ("%5.2f%%", frac_tot_m) + " ("
                                + QString::number (frac_tot_r) + "%)",
                            QString::number (frac_tot_tol_r), tot_frac_passed);
        }
      mstr += indent (2) + "</table>\n";
...
QMap<QString, QString>
US_ReporterGMP::get_channels_analytes_mwl_abde (QString abde_channame)
{
  ...
  for (int ii = 0; ii < currProto.rpRange.nranges; ii++)
    {
      ...
      channel_s.replace (" / ", "");
      qDebug () << "[in get_channels_analytes_mwl_abde()], channel, "
                   "channel_s, abde_channame;"
      ...
      if (channel_s == abde_channame)
        {
          ...
          bool mwl_deconv
              = currProto.rpRange.chrngs[ii].abde_mwl_deconvolution;
          ...
          if (nwavl > 1 && mwl_deconv)
            {
              ...
              if (status == US_DB2::NO_BUFFER)
              ...
              else if (status == US_DB2::NO_ANALYTE)
                {
                  QMessageBox::information (
                      this, tr ("Attention"),
                      tr ("One of the analytes this solution refers to was "
                          "not found.\n"
                          "Please restore and try again.\n"));
                  return analytes_buffer_map;
                }
              ...
              for (int i = 0; i < num_analytes; ++i)
                {
                  ...
                  qDebug ()
                      << "[GMP REPORT] Solution " << solution->solutionDesc
                      << ", (GUID)Analyte " << "(" << a_GUID << ")" << a_name
                      << ", (ID)Analyte " << "(" << a_ID << ")" << a_name;

                  QString ana_name
                      = "Analyte #" + QString::number (i + 1) + ":";
...
void
US_ReporterGMP::assemble_replicate_av_integration_html (void)
{
  // QString html_distibutions = distrib_info();
  html_assembled += "<p class=\"pagebreak \">\n";
...
void
US_ReporterGMP::assemble_plots_html (QStringList PlotsFilenames,
                                     const QString plot_type)
{
  qDebug () << "[in assemble_plots_html()], PlotsFilenames -- "
            << PlotsFilenames;
  ...
  for (int i = 0; i < PlotsFilenames.size (); ++i)
    {
      ...
      // QString filename = "./" + PlotsFilenames[ i ]. section('/', -1); //try
      // relative path!
      ...
      qDebug () << "Image, " << filename << "width, height: " << i_width
                << i_height;

      // QPrinter below must be the same as defined just prior painting
      // QTextDocument in ::write_pdf
      QPrinter printer_t (
          QPrinter::
              PrinterResolution); //(QPrinter::HighResolution);//(QPrinter::PrinterResolution);
      ...
      qreal qprinters_width
          = pageSize.width () * 0.75; // 500 DEPENDS on QPrinter's constructor
                                      // settings {QPrinter::PrinterResolution,
                                      // 500; QPrinter::HighResolution, 9066}
      ...
      double scaled_i_height
          = double (i_height) * (scaled_i_width / double (i_width));
      // int scaled_i_height = i_height * i_scale_factor;

      qDebug () << "Image scaled, " << filename
                << "scaled_width, scaledheight: " << scaled_i_width
                << scaled_i_height;
      ...
      // html_assembled   += "    <div><img style=\"height: 50px; width:
      // 100px\" src=\"" + filename
      // 	+ "\" alt=\"" + label + "\"/></div>\n\n";

      html_assembled
          += "    <div><img src=\"" + filename + "\" alt=\"" + label;

      if (!plot_type.isEmpty ()
          && plot_type == "CombPlots") // For Combined plots, scale down .png
        {
          // reduce comb. plots somewhat
          double i_scale_factor_h = double (qprinters_hight / scaled_i_height);
          double scaled_i_height_cp
              = double (scaled_i_height * i_scale_factor_h);
          double scaled_i_width_cp
              = double (scaled_i_width)
                * (scaled_i_height_cp / double (scaled_i_height));

          // html_assembled  += " \"height=\"" + QString::number(
          // scaled_i_height ) + "\"width=\"" + QString::number(
          // scaled_i_width);
          html_assembled
              += " \"height=\"" + QString::number (scaled_i_height_cp)
                 + "\"width=\"" + QString::number (scaled_i_width_cp);
        }
      else if (!plot_type.isEmpty () && plot_type == "RunDetails")
        {
          double scaled_rd_width = double (scaled_i_width * 0.9);
          double scaled_rd_height
              = double (scaled_i_height)
                * (scaled_rd_width / double (scaled_i_width));
          html_assembled += " \"height=\"" + QString::number (scaled_rd_height)
                            + "\"width=\"" + QString::number (scaled_rd_width);
        }
      else
        {
          html_assembled += " \"height=\"" + QString::number (scaled_i_height)
                            + "\"width=\"" + QString::number (scaled_i_width);
        }

      qDebug () << "Image size html string: "
                << "\"height=\"" + QString::number (scaled_i_height)
                       + "\"width=\"" + QString::number (scaled_i_width);
      ...
      if (!plot_type.isEmpty () && plot_type == "CombPlots")
        {
          ...
          qDebug () << "COMBOPLOT-parms/colors SZIES: combparms.size(), "
                       "combparms_colors.size() --  "
                    << combparms.size () << combparms_colors.size ();

          for (int i = 0; i < combparms.size (); ++i)
            {
              if (!combparms_colors.isEmpty ()
                  && combparms_colors.size () == combparms.size ())
                {
                  qDebug ()
                      << "COMBOPLOT-COLORS -- " << combparms_colors[i].name ();
                  // html_assembled   += "<br><span
                  // style='color:blue'>&#9726;</span>";
                  html_assembled += "<br><span style='color:"
                                    + combparms_colors[i].name ()
                                    + "'>&#9726;</span>";
                }
              else
                html_assembled
                    += "<br><span style='color:blue'>&#9726;</span>"; // SOME
                                                                      // default
                                                                      // color!!!
...
double
US_ReporterGMP::interp_sval (double xv, double *sx, double *sy, int ssize)
{
...
QString
US_ReporterGMP::text_model (US_Model model, int width)
{
...
QString
US_ReporterGMP::html_header (QString title, QString head1,
...
QString
US_ReporterGMP::html_header_abde (QString title, QString runName,
...
QString
US_ReporterGMP::calc_replicates_averages (void)
{
  /*
...
=====================================================================================
    ...
    Replicate group #2: [channels 2A, 4A, 6B]
       Subgroup 1:  ----- Same: wvl, type/method, ranges, integraiton value
         triples:           2A.290, 6B.290 (overlapping wvls)  --> IT Maybe
that 4A does not have wvl 290!!! type/method:       [s / 2DSA-IT] ranges: [3.2
- 3.7] integration value: 0.57

        Subgroup 2:  ----- Same: wvl, type/method, ranges, integraiton value
         triples:           2A.290, 6B.290 (overlapping wvls)  --> IT Maybe
that 4A does not have wvl 290!!! type/method:       [D / 2DSA-IT] ranges: [3.4
- 3.9] integration value: 0.51

=====================================================================================

 Now, the 1st triple in a given subgroup (same-wvl triples) is the REFERENCE
one:
    - e.g., 1A.280, in Replicate #1, subgroup 1,2;
            1A.320, in Replicate #1, subgroup 3;
            2A.290, in Replicate #2, subgroup 1,2;

    **  Retrieve US_ReportGMP for these REFERENCE triples && go over theirs
ReportItems:
...
channel_desc_alt ] [ QStringList(
"2A.260","2B.260","4A.260","2A.275","2B.275","4A.275","2A.280","2B.280","4A.280","4B.280","6A.280","6B.280"
)]

          IMPORTANT: in QStringList, group triples by same-wvl (subgroups),
then [0] in each group will be REFERENCE wvl! So, reference triples in the
above QStringList example will be: 2A.260, 2A.275 && 2A.280

       -- Start by iterating over QMap< QString, QStringList >
channdesc_to_overlapping_wvls: iterator::key()   - channel_desc_at;
                       iterator::value() - access to wvl; IMPORTANT, break into
same-wvl groups, then take 1st to access wvl of the REFERNECE triple

       -- Iterate over ReportItems in ref_report.reportItems;
       -- For each unique ReportItem, identify type-method, ranges, int.value;
       -- Retrive earlier processed (in ::distrib_info() ) integration results
for subgroup triples && AVERAGE

       -- BEFORE: in ::distrib_info() STORE integration results into respective
reportItem[ kk ]. { integration_val_sim; ...}
  ...
  if (!channdesc_to_overlapping_wvls.isEmpty ())
    html_str_replicate_av
        += tr ("<h2>Replicate Groups Averaging Results:</h2>\n");

  QMap<QString, QStringList>::iterator chw;
  for (chw = channdesc_to_overlapping_wvls.begin ();
       chw != channdesc_to_overlapping_wvls.end (); ++chw)
    {
      ...
      for (int i = 0; i < all_wvls.size (); ++i)
        {
          ...
          // here, add to list FULL channel desc, e.g. "1A:Iterf.", or
          // "1A:UV/vis." same_wvls_chann_map[ curr_wvl ] << curr_chann;
          // //BEFORE
          same_wvls_chann_map[curr_wvl]
              << curr_chann + ":" + o_type; // Will this work?
        }

      unique_wvls.removeDuplicates ();
      unique_channels.removeDuplicates ();

      QString replicate_group_number
          = get_replicate_group_number (ch_alt_desc);

      html_str_replicate_av
          += "\n" + indent (2)
             + tr ("<h3>Replicate Group #%1: [Channels: %2 (%3)] </h3>\n")
      ...
      for (int j = 0; j < unique_wvls.size (); ++j)
        {
          ...
          for (int jj = 0; jj < same_wvls_chann_map[u_wvl].size (); ++jj)
            replicate_subgroup_triples
                += same_wvls_chann_map[u_wvl][jj].split (":")[0] + "." + u_wvl
                   + ",";
          ...
          html_str_replicate_av
              += "\n" + indent (2)
                 + tr ("<h3>Subgroup #%1: [Triples: %2] </h3>\n")
                       .arg (QString::number (j + 1))
                       .arg (replicate_subgroup_triples);

          html_str_replicate_av
              += indent (2) + "<table style=\"font-size:80%\">\n";
          html_str_replicate_av += table_row (
              tr ("Type:"), tr ("Method:"), tr ("Range:"),
              tr ("Av. Integration, Model (target):"), tr ("St. Dev.:"),
              tr ("Av. Fraction %, Model (target):"), tr ("St. Dev.:"),
              tr ("Tol.%:"), tr ("PASSED?"));

          // Reference GMP Report, ReportItems for 1st channel in a replicate
          // group && over each wavelength:
          US_ReportGMP ref_group_report = ch_reports[ch_alt_desc][u_wvl];
          int report_items_number = ref_group_report.reportItems.size ();
          for (int kk = 0; kk < report_items_number; ++kk)
            {
              US_ReportGMP::ReportItem ref_group_item
                  = ref_group_report.reportItems[kk];
              ...
              if (method.contains ("PCSA"))
                method = "PCSA";
              QString int_val_r
                  = QString::number (ref_group_item.integration_val);
              ...
              QMap<QString, double> replicate_g_results
                  = get_replicate_group_results (ref_group_item, u_wvl,
                                                 same_wvls_chann_map[u_wvl]);

              // print results into HTML report:
              /*
                 Replicate Group #: [channles: 1A, 2A, 5A]
                    Sub-Group #: [wvl: 280]
                      type,  method,   range,    integration_AV (target),
                 St.Dev.   Fraction of Total AV. (target)   PASSED? s
                 2DSA-MC   3.2-3.7   integration_sim_av         st_dev      XXX
                 YES/NO
              */

              double frac_tot_m_av = replicate_g_results["tot_percent_av"];
              QString tot_av_frac_passed
                  = (frac_tot_m_av
                         >= (frac_tot_r * (1 - frac_tot_tol_r / 100.0))
                     && frac_tot_m_av
                            <= (frac_tot_r * (1 + frac_tot_tol_r / 100.0)))
                        ? "YES"
                        : "NO";
              ...
              html_str_replicate_av += table_row (
                  type, method, range,
                  QString ().sprintf ("%10.4e", replicate_g_results["int_av"])
                      + " (" + int_val_r + ")",
                  QString ().sprintf ("%10.2e",
                                      replicate_g_results["int_st_dev"]),
                  QString ().sprintf ("%5.2f%%",
                                      replicate_g_results["tot_percent_av"])
                      + " (" + QString ().sprintf ("%5.2f%%", frac_tot_r)
                      + ")",
                  QString ().sprintf (
                      "%5.2f%%", replicate_g_results["tot_percent_st_dev"]),
                  QString::number (frac_tot_tol_r), tot_av_frac_passed);
            }
...
QString
US_ReporterGMP::get_replicate_group_number (QString ch_alt_desc)
{
  ...
  qDebug () << "Replicate Group # for channel: " << ch_alt_desc << ", IS -- "
            << replicate_gn;

  return replicate_gn;
}

// Get Integration Results from same Replicate subGroup | same-wvl |
// same-report-items:
QMap<QString, double>
US_ReporterGMP::get_replicate_group_results (
    US_ReportGMP::ReportItem ref_report_item, QString u_wvl,
    QStringList channs_for_wvl)
{
  ...
  // iterate over UR_ReportsGMPs && pick ones correspondning to
  // 'chan_desc_alt.contains("ch_wvls[i]")' && wavelength == u_wvl;
  int nchna = currAProf.pchans.count ();
  for (int i = 0; i < channs_for_wvl.size ();
       ++i) // over channels for a given wvl in Replicate group:
    {
      US_ReportGMP reportGMP;
      for (int j = 0; j < nchna; j++) // over all channels
        {
          ...
          if (channel_desc_alt.contains (channs_for_wvl[i]))
            {
              qDebug () << "In get_replicate_group_results(): "
                           "channel_desc_alt, channs_for_wvl[ i ], wvl -- "
                        << channel_desc_alt << channs_for_wvl[i] << u_wvl;

              // Select US_ReportGMP for channel in a Replicate group &&
              // representative wvl!
      ...
      // then pick report's ReportItem corresponding to the passed
      // ref_report_item:
      int report_items_number = reportGMP.reportItems.size ();
      for (int kk = 0; kk < report_items_number; ++kk)
        {
          ...
          QString ref_int_val
              = QString::number (ref_report_item.integration_val);
          ...
          // Comparison nased on: type/method [s-2DSA-IT], ranges [3.2 - 3.7]
          // && integration value: CAN BE EXTENDED if needed
          if (ref_type == _type && ref_method == _method && ref_low == _low
              && ref_high == _high && ref_int_val == _int_val)
            {
              ...
              if (curr_item.integration_val_sim >= 0)
                {
                  ...
                  tot_percent_sim_vector.push_back (
                      curr_item.total_percent_sim);
  ...
  if (same_item_counter)
    {
      ...
      for (int i = 0; i < int_res_sim_vector.size (); ++i)
        {
          st_dev_int_1 += (int_res_sim_av - int_res_sim_vector[i])
                          * (int_res_sim_av - int_res_sim_vector[i]);
          st_dev_tot_percent_1
              += (tot_percent_sim_av - tot_percent_sim_vector[i])
                 * (tot_percent_sim_av - tot_percent_sim_vector[i]);
        }

      st_dev_int_final
          = sqrt (st_dev_int_1) / (sqrt (int_res_sim_vector.size ()));
      st_dev_tot_percent_final = sqrt (st_dev_tot_percent_1)
                                 / (sqrt (tot_percent_sim_vector.size ()));
    }
...
QString
US_ReporterGMP::distrib_info (QMap<QString, QString> &tripleInfo)
{
  ...
  qDebug () << "Distrib_info(): Model Name, time created, editUpdated -- "
            << model.description << model.timeCreated << model.editDataUpdated;
  ...
  if (is_dmga_mc)
    ...
    { // Normal non-DMGA-MC model
      msim = "";
      if (model_used.monteCarlo && !model_used.description.contains ("_mcN"))
        msim = "<b>&nbsp;&nbsp;( single iteration )</b>";
    }

  QString mdla
      = model_used.description.section (".", -2, -2).section ("_", 1, -1);
  if (mdla.isEmpty ())
  ...
  mstr += table_row (tr ("Number of Components:"), QString::number (ncomp));
  mstr += table_row (tr ("Residual RMS Deviation:"), rmsd_global);
  ...
  if (cnstvb)
    mstr += table_row (tr ("Constant vbar20:"), QString::number (minv));
  else if (cnstff)
    mstr += table_row (tr ("Constant f/f0:"), QString::number (mink));
  mstr += indent (2) + "</table>\n";
  ...
  // QString fileName_str      = dirName + "/" + model_desc_edited +
  // "_csv.txt"; QString fileName_str_only = model_desc_edited + "_csv.txt";
  ...
  if (cnstvb)
    { // Normal constant-vbar distribution
      model_dist_info = "Molec. Wt., S Apparent, S 20 (W), D Apparent, D 20 "
                        "(W), f/f0, Concentration";
    ...
    { // Neither vbar nor f/f0 are constant
      ...
      model_dist_info = "Molec. Wt., S Apparent, S 20 (W), D 20 (W), f/f0, "
                        "Vbar20, Concentration";
  ...
  // mstr += "<a href=\"./" + fileName_str_only + "\">View Model
  // Distributions</a>"; mstr += "<br>";
  mstr += "<a href=\"./" + f_model_path_str_only
          + "\">View Model Distributions</a>";
  ...
  */
  ...
  if (model_used.associations.size () > 0)
    {
      ...
      mstr += indent (2) + "<table>\n";
      mstr += table_row (tr ("Reactant 1"), tr ("Reactant 2"), tr ("Product"),
                         tr ("K_dissociation"), tr ("k_off Rate"));
  ...
  qDebug () << "In DISTRIB: triple, cellID, wvl -- " << t_name << cellID
            << wvl;

  qDebug () << "Model description, mdla, msim -- " << model_used.description
            << mdla << msim;
  QString model_name = mdla.split ("_")[1];
  if (model_name.contains ("PCSA"))
    model_name = "PCSA";

  double tot_conc_r, tot_conc_r_factor, tot_conc_tol_r, rmsd_r, av_int_r,
      exp_dur_r, exp_dur_tol_r;
  ...
  hh = "h";
  QString wks = QString ().sprintf ("%d %s %02d m", hours,
                                    hh.toLatin1 ().data (), mins);
  ...
  for (int i = 0; i < nchna; i++)
    {
      ...
      if (t_name.contains (channelNameProt))
        {
          qDebug () << "So, what are channel_desc_alt, wvl ? "
                    << channel_desc_alt << wvl;
  ...
  hh_r = "h";
  QString exp_dur_r_hh_mm = QString ().sprintf (
      "%d %s %02d m", hours_r, hh_r.toLatin1 ().data (), mins_r);
  ...
  if (intensity_keys.size () == 1) // single wvl
    {
      ...
      qDebug () << "single wvl: key, value -- " << intensityRIMap.firstKey ()
                << intensityRIMap.first ();
    }
  else // MWL
    {
  ...
  QString tot_conc_passed
      = (sum_c >= (tot_conc_r * (1 - tot_conc_tol_r / 100.0))
         && sum_c <= (tot_conc_r * (1 + tot_conc_tol_r / 100.0)))
            ? "YES"
            : "NO";
  QString exp_dur_passed
      = (last_f >= (exp_dur_r * (1 - exp_dur_tol_r / 100.0))
         && last_f <= (exp_dur_r * (1 + exp_dur_tol_r / 100.0)))
            ? "YES"
            : "NO";
  QString rmsd_passed = (rmsd_global.toDouble () <= rmsd_r) ? "YES" : "NO";
  QString av_int_passed = (av_int_exp > av_int_r) ? "YES" : "NO";
  QString loading_volume_passed
      = (loading_volume_measured
             >= (loading_volume_r * (1 - loading_volume_tol_r / 100.0))
         && loading_volume_measured
                <= (loading_volume_r * (1 + loading_volume_tol_r / 100.0)))
            ? "YES"
            : "NO";
  ...
  QMap<QString, QString> Model_parms_to_compare
      = perChanMask_edited
            .ShowTripleModelParts[t_name]
                                 [model_name]; // 2A660 => 2AInterference,
  ...
  for (sh = Model_parms_to_compare.begin ();
       sh != Model_parms_to_compare.end (); ++sh)
    {
  ...
  if (show_comparison_section)
    {
      mstr += "\n" + indent (2)
              + tr ("<h3>Comparison Between Run/Simulation Results and Report "
                    "Parameters:</h3>\n");
      mstr += indent (2) + "<table>\n";

      mstr += table_row (tr ("Parameter: "), tr ("Target Value:"),
                         tr ("Tolerance, %:"), tr ("Measured Value:"),
                         tr ("PASSED ?"));
      ...
      if (show_tot_conc)
        {
          mstr += table_row (tr ("Total Concentration"),
                             QString::number (tot_conc_r) + " (factor "
                                 + QString::number (tot_conc_r_factor) + ")",
      ...
      if (show_rmsd)
        {
          mstr += table_row (tr ("RMSD limit"), QString::number (rmsd_r),
                             QString (""), rmsd_global, rmsd_passed);
        }

      if (show_exp_dur)
        {
          mstr += table_row (tr ("Experiment Duration"), exp_dur_r_hh_mm,
                             QString::number (exp_dur_tol_r) + "%", wks,
                             exp_dur_passed);
        }

      // Show only if not Absorbance || not Interference
      qDebug () << "Show_INTENSITY: dataSource -- " << dataSource;
      qDebug () << "Show_INTENSITY: tripleInfo[ \"triple_name\" ] -- "
      ...
      if (show_min_int && RIdata)
        {
          mstr += table_row (tr ("Minimum Intensity"),
                             QString::number (av_int_r), QString (""),
                             QString::number (av_int_exp), av_int_passed);
        }
  ...
  if (show_integration && method_type_combo_exists)
    {
      mstr += "\n" + indent (2)
              + tr ("<h3>Integration Results: Fraction of Total "
                    "Concentration:</h3>\n");
      mstr += indent (2) + "<table>\n";
      mstr += table_row (tr ("Type:"), tr ("Range:"), tr ("weight Avg.:"),
      ...
      for (int kk = 0; kk < report_items_number; ++kk)
        {
          ...
          // QString tot_frac_passed = ( frac_tot_m >= ( frac_tot_r * (1 -
          // frac_tot_tol_r/100.0)  )
          // 			       && frac_tot_m <= ( frac_tot_r * (1 +
          // frac_tot_tol_r/100.0)  ) ) ? "YES" : "NO";

          QString tot_frac_passed
              = (qAbs (frac_tot_m - frac_tot_r) <= frac_tot_tol_r) ? "YES"
                                                                   : "NO";

          double wav_variable_within_range
              = double (variable_val_within_range / int_val_m);
          ...
          if (mdla.contains (method))
            {
              ...
              qDebug () << "In distrib_info(), fill simulated integration "
                           "vals: for chann/wvl/type/method/low/high, "
                        << "Inter. val. Sim -- " << wvl << curr_item.type
                        << curr_item.method << curr_item.range_low
                        << curr_item.range_high << int_val_m;
...
double
US_ReporterGMP::get_loading_volume (int cellID)
{
  double vol = 0;

  double speed = currProto.rpSpeed.ssteps[0].speed;
  double stretch = simparams.rotorcoeffs[0] * speed
                   + simparams.rotorcoeffs[1] * sq (speed);
  ...
  if (centerpieceID_read.isEmpty ())
    {
      qDebug ()
          << "In get_loading_volume(): Couldn't read centerpiece name ! ";
  ...
  qDebug ()
      << "In get_loading_volume(): c_name, c_bottom, c_pathlength, c_angle -- "
  ...
  vol = (c_bottom * c_bottom - meniscus * meniscus)
        * (1000 * c_pathlength * c_angle * M_PI) / 360.0;
...
QString
US_ReporterGMP::indent (const int spaces) const
{
...
QString
US_ReporterGMP::table_row (const QString &s1, const QString &s2) const
{
...
QString
US_ReporterGMP::table_row (const QString &s1, const QString &s2,
                           ...
                           const QString &s9) const
{
  return (indent (6) + "<tr><td>" + s1 + "</td><td>" + s2 + "</td><td>" + s3
          + "</td><td>" + s4 + "</td><td>" + s5 + "</td><td>" + s6
          + "</td><td>" + s7 + "</td><td>" + s8 + "</td><td>" + s9
          + "</td></tr>\n");
}

// Open a residual plot dialog
void
US_ReporterGMP::plotres (QMap<QString, QString> &tripleInfo)
{
  ...
  // bool show_3d   = ( perChanMask_edited. ShowTripleModelPlotParts[ t_name ][
  // s_name ][ "3D Model Plot" ].toInt() ) ? true : false ;
  bool show_3d = false;
  bool show_d_mw
      = (perChanMask_edited
             .ShowTripleModelPlotParts[t_name][s_name]["D-vs-MW 2D Model"]
             .toInt ())
            ? true
            : false;
  bool show_d_s
      = (perChanMask_edited
             .ShowTripleModelPlotParts[t_name][s_name]["D-vs-s 2D Model"]
             .toInt ())
            ? true
            : false;
  bool show_dd
      = (perChanMask_edited
             .ShowTripleModelPlotParts[t_name][s_name]
                                      ["Diffusion Coefficient Distribution"]
             .toInt ())
            ? true
            : false;
  ...
  bool show_vel
      = (perChanMask_edited
             .ShowTripleModelPlotParts
                 [t_name][s_name]
                 ["Experiment-Simulation Velocity Data (noise corrected)"]
             .toInt ())
            ? true
            : false;
  bool show_mw
  ...
  bool show_sd
      = (perChanMask_edited
             .ShowTripleModelPlotParts
                 [t_name][s_name]["Sedimentation Coefficient Distribution"]
             .toInt ())
            ? true
            : false;
  bool show_f_mw
      = (perChanMask_edited
             .ShowTripleModelPlotParts[t_name][s_name]["f/f0-vs-MW 2D Model"]
             .toInt ())
            ? true
            : false;
  bool show_f_s
      = (perChanMask_edited
             .ShowTripleModelPlotParts[t_name][s_name]["f/f0-vs-s 2D Model"]
             .toInt ())
            ? true
            : false;

  // // Check if to plot individual Combined distributons:
  // // {s,D,f/f0,MW,Radius} {2DSA-IT,2DSA-MC,PCSA,raw} {p_key: {s[3.2:3.7],
  // D[11:15], etc.} }
  // // MaskStr.ShowTripleTypeModelRangeIndividualCombo[ t_name ][ s_name ][
  // p_key ] = feature_indCombo_value; // 1/0 bool show_ind_combo_s; bool
  // show_ind_combo_D; bool show_ind_combo_f_f0; bool show_ind_combo_MW; bool
  // show_ind_combo_Radius;

  // Debug
  qDebug () << "triple_name, stage_n: " << tripleInfo["triple_name"]
  ...
  QString basename
      = dirName + "/" + text_model (model, 0) + "." + tripnode + ".";
  ...
  if (show_vel)
    {
      write_plot (img01File,
                  resplotd->rp_data_plot1 ()); //<-- rp_data_plot1() gives
                                               //overlay (Exp/Simulations) plot
  ...
  if (show_res)
    {
      write_plot (
          img02File,
          resplotd
              ->rp_data_plot2 ()); //<-- rp_data_plot2() gives residuals plot
      PlotsFileNames << img02File;
    }

  // Stick Plots for S-, MW-, D- distributions
  plotLayout1
      = new US_Plot (data_plot1, tr ("Residuals"), tr ("Radius (cm)"),
                     tr ("OD Difference"), true, "^resids [0-9].*", "rainbow");

  plotLayout2
      = new US_Plot (data_plot2, tr ("Velocity Data"), tr ("Radius (cm)"),
                     tr ("Absorbance"), true, ".*in range", "rainbow");
...
void
US_ReporterGMP::distrib_plot_stick (int type)
{
...
void
US_ReporterGMP::distrib_plot_2d (int type)
{
  QString pltitle = tr ("Run ") + edata->runID + tr (" :\nCell ") + edata->cell
                    + " (" + edata->wavelength + " nm)";
  QString yatitle;
  ...
  for (int jj = 0; jj < dsize; jj++)
    {
      ...
      if (type < 5)
        yval = model_used.components[jj].f_f0;
      else if (type < 7)
        yval = model_used.components[jj].vbar20;
      else
        yval = model_used.components[jj].D;
...
bool
US_ReporterGMP::mkdir (const QString &baseDir, const QString &subdir)
{
  QDir folder (baseDir);

  if (folder.exists (subdir))
    return true;

  if (folder.mkdir (subdir))
    return true;
...
// Write out a plot
void
US_ReporterGMP::write_plot (const QString &filename, const QwtPlot *plot)
{
  ...
  //    }
    ...
    { // Special case of 3dplot PNG
      ...
      QPixmap pixmap
          = dataw->renderPixmap (dataw->width (), dataw->height (), true);
...
void
US_ReporterGMP::thread_progress (int thr, int icomp)
{
  qDebug () << "Updating progress multiple threads, thr, icomp -- " << thr
            << icomp;
  int kcomp = 0;
  kcomps[thr] = icomp;
  for (int ii = 0; ii < nthread; ii++)
...
void
US_ReporterGMP::thread_complete (int thr)
{
  thrdone++;
...
US_ReporterGMP::adjustModel ()
{
  ...
  qDebug () << "Fem:Adj:  avgT" << avgTemp << "scorr dcorr" << scorrec
            << dcorrec;
  ...
  if (cnstvb && mc_vbar != sd.vbar20 && mc_vbar != 0.0)
    { // Use vbar from the model component, instead of from the solution
      ...
      qDebug () << "Fem:Adj:   cnstvb" << cnstvb << "  scorr dcorr" << scorrec
                << dcorrec;
    }
  qDebug () << "Fem:Adj:  avgT" << avgTemp << "vb20 vb" << sd.vbar20
            << sd.vbar;
  ...
  for (int jj = 0; jj < model.components.size (); jj++)
    {
      ...
      sc->s /= scorrec;
      sc->D /= dcorrec;
      qDebug () << "Fem:Adj:  s20w D20w" << s20w << d20w << "s D" << sc->s
                << sc->D << "  jj" << jj << "vb20 vb" << sc->vbar20 << sd.vbar;
...
QString
US_ReporterGMP::get_filename (QString triple_name)
{
...
void
US_ReporterGMP::assemble_pdf (QProgressDialog *progress_msg)
{
  ...
  // rptpage  += "  div.footer { display: block; text-align: center;  position:
  // running(footer);"; rptpage  += "  @page { @bottom-center { content:
  // element(footer) }}";

  rptpage += "  </style>\n";
  rptpage += "  </head>\n  <body>\n";

  // rptpage  += "  <footer> This is the text that goes at the bottom of every
  // page. </footer>\n"; rptpage  += " <div class='footer'>Footer</div>";

  // HEADER: begin
  QString us_version
      = "Version " + US_Version + " ( " REVISION " ) for " OS_TITLE;
  QString html_header = QString ("");
  html_header += rptpage;
  html_header += tr ("<div align=left>"
                     "Created, %1<br>"
  ...
  QString html_title
      = tr ("<h1 align=center>%1 Report for Run: <br><i>%2</i></h1>"
            "<hr>")
            .arg (report_type) // 1
            .arg (runName)     // 2
      ;
  ...
  QString html_paragraph_open = tr ("<p align=justify>");

  // Failed Triples' Analyses, Missing Models info (if any):
  QString str_failed_stage_missing_models
      = compose_html_failed_stage_missing_models ();
  qDebug () << "STR_on_failed: " << str_failed_stage_missing_models;
  if (!str_failed_stage_missing_models.isEmpty ())
    {
      html_failed
          = tr ("<h3 style=\"color:red;\" align=left> ATTENTION: Analyses for "
                "Some Triples Failed, or Models are Missing!</h3>"
                "%1"
                "<hr>")
  ...
  html_general = tr (
                     ...
                     "<td>%1</td></tr>"
                     "<tr><td>Protocol Name: </td>                          "
                     "<td>%2</td></tr>"
                     "<tr><td>Project:</td>                                 "
                     "<td>%3</td></tr>"
                     "<tr><td>Run Temperature (&#8451;):</td>               "
                     "<td>%4</td></tr>"
                     "<tr><td>Temperature-Equilibration Delay (mins):</td>  "
                     "<td>%5</td></tr>"
                     "</table>"
                     "<hr>")
  ...
  // ROTOR/LAB: begin
  html_lab_rotor = tr ("<h3 align=left>Lab/Rotor Parameters</h3>"
                       "<table>"
  ...
  QJsonDocument jsonDocOperList
      = QJsonDocument::fromJson (eSign_details["operatorListJson"].toUtf8 ());
  QString opers_a = get_assigned_oper_revs (jsonDocOperList);

  QJsonDocument jsonDocRevList
      = QJsonDocument::fromJson (eSign_details["reviewersListJson"].toUtf8 ());
  QString revs_a = get_assigned_oper_revs (jsonDocRevList);

  QJsonDocument jsonDocApprList
      = QJsonDocument::fromJson (eSign_details["approversListJson"].toUtf8 ());
  QString apprs_a = get_assigned_oper_revs (jsonDocApprList);

  QString run_id = (dataSource == "INSTRUMENT") ? runID : "N/A";
  QString instr_name = (dataSource == "INSTRUMENT")
                           ? currProto.rpRotor.instrname
                           : "dataDisk";

  html_operator
      = tr ("<h3 align=left>Optima Machine/Operator </h3>"
            "<table>"
  ...
  html_speed
      = tr ("<h3 align=left>Speed Parameters </h3>"
            "<table>"
            "<tr><td>Rotor Speed  (RPM):    </td>                   "
            "<td>%1</td></tr>"
            "<tr><td>Acceleration (RMP/sec): </td>                  "
            "<td>%2</td></tr>"
            "<tr><td>Active Scaning Time:    </td>                  "
            "<td>%3</td></tr>"
            "<tr><td>Stage Delay:          </td>                    "
            "<td>%4</td></tr>"
            "<tr><td>Total Time (without equilibration):  </td>     "
            "<td>%5</td></tr>")
  ...
  if (has_uv)
    {
      html_speed += tr ("<tr><td><br><b><i>UV-visible optics (total):</i></b> "
                        "  </td>  "
                        "<tr><td>Delay to First Scan:            </td>        "
                        "  <td>%6</td></tr>"
                        "<tr><td>Scan Interval:                  </td>        "
                        "  <td>%7</td></tr>")
    ...
    {
      html_speed
          += tr ("<tr><td><br><b><i>UV-visible optics:</i></b>   </td>  "
                 "<tr><td>Not Used  </td></tr>");
    }

  if (has_int)
    {
      html_speed += tr ("<tr><td><br><b><i>Interference optics (per "
                        "cell):</i></b></td>   "
                        "<tr><td>Delay to First Scan:  </td>                  "
                        "  <td>%8</td></tr>"
                        "<tr><td>Scan Interval:        </td>                  "
    ...
    {
      html_speed
          += tr ("<tr><td><br><b><i>Interference optics:</i></b></td>   "
                 "<tr><td>Not Used </td></tr>");
    }
  ...
  // CELLS: begin
  html_cells = tr ("<h3 align=left>Cell Centerpiece Usage </h3>");

  ncells_used = currProto.rpCells.nused;
  html_cells += tr ("# of Used Cells:  %1<br>")
  ...
  for (int i = 0; i < ncells_used; ++i)
    {
      ...
      if (i == ncells_used - 1
          && currProto.rpCells.used[i].cbalance.contains (
              tr ("counterbalance")))
        {
  ...
  // SOLUTIONS: begin
  html_solutions = tr ("<h3 align=left>Solutions for Channels</h3>");

  nsol_channels = currProto.rpSolut.nschan;
  html_solutions
      += tr ("# of Solution Channels:  %1 <br>").arg (nsol_channels) // 1
      ;

  for (int i = 0; i < nsol_channels; ++i)
    {
      html_solutions += tr ("<table>"
                            "<tr>"
                            "<td><b>Cell/Channel:</b> "
                            "&nbsp;&nbsp;&nbsp;&nbsp; </td> <td><b>%1</b></td>"
                            "</tr>"
  ...
  // OPTICS: begin
  html_optical = tr ("<h3 align=left>Optical Systems </h3>");
  nchan_optics = currProto.rpOptic.nochan;
  html_optical += tr ("# of Channels With Optics:  %1<br>")
                      .arg (QString::number (nchan_optics)) // 1
      ;
  html_optical += tr ("<table>");
  ...
  // RANGES: begin
  html_ranges = tr ("<h3 align=left> Ranges </h3>");
  nchan_ranges = currProto.rpRange.nranges;
  html_ranges += tr ("# of Channels With Ranges:  %1<br>")
                     .arg (QString::number (nchan_ranges)) // 1
      ;

  for (int i = 0; i < nchan_ranges; ++i)
    {
      html_ranges += tr ("<table>"
                         "<tr>"
                         "<td><b>Cell/Channel:</b> &nbsp;&nbsp;&nbsp;&nbsp; "
                         "</td> <td><b>%1</b></td>"
                         "</tr>"
      ...
      QString w_range
          = QString::number (w_min) + tr (" to ") + QString::number (w_max);
      QString r_range
          = QString::number (r_min) + tr (" to ") + QString::number (r_max);
      ...
      html_ranges += tr ("<table style=\"margin-left:30px\">"
                         "<tr><td> Selected Wavelength count: </td>  <td> %1 "
                         "</td> </tr>")
      ...
      if (w_count > 1)
        {
          html_ranges += tr ("<tr><td> Selected Wavelength range: </td>  <td> "
                             "%1 </td> </tr>")
                             .arg (w_range) // 1
              ;
        }

      html_ranges += tr ("<tr><td> Radius range:              </td>  <td> %1 "
                         "</td> </tr>"
                         "<tr><td> Selected Wavelengths:      </td>  <td> %2 "
                         "</td> </tr>"
                         "</table>")
                         .arg (r_range) // 1
                         .arg (all_wvl) // 2
  ...
  // UV-vis
  QString html_scan_count_uv = tr ("<table>"
                                   "<tr>"
                                   "<td><b><i>UV-visible optics:</i></b></td>"
                                   "</tr>"
                                   "</table>");
  html_scan_count_uv
      += tr ("<table style=\"margin-left:30px\">"
  ...
  for (int i = 0; i < nchna; i++)
    {
      ...
      if (expType == "VELOCITY")
        {
          ...
          html_analysis_gen += tr ("<table style=\"margin-left:30px\">"
                                   "<tr><td> Loading Ratio:              "
                                   "</td>  <td> %1 </td> </tr>"
                                   "<tr><td> Ratio Tolerance (&#177;%):  "
                                   "</td>  <td> %2 </td> </tr>"
                                   "<tr><td> Loading Volume (&#181;l):   "
                                   "</td>  <td> %3 </td> </tr>"
                                   "<tr><td> Volume Tolerance (&#177;%): "
                                   "</td>  <td> %4 </td> </tr>"
                                   "<tr><td> Data End (cm):              "
                                   "</td>  <td> %5 </td> </tr>"
                                   "</table>")
      ...
      else if (expType == "ABDE")
        {
          ...
          html_analysis_gen += tr ("<table style=\"margin-left:30px\">"
                                   "<tr><td> Loading Density (g/ml):     "
                                   "</td>  <td> %1 </td> </tr>"
                                   "<tr><td> Gradient Mat. vbar (ml/g):  "
                                   "</td>  <td> %2 </td> </tr>"
                                   "<tr><td> Gradient Mat. MW (g/mol):   "
                                   "</td>  <td> %3 </td> </tr>"
                                   "<tr><td> Loading Volume (&#181;l):   "
                                   "</td>  <td> %4 </td> </tr>"
                                   "<tr><td> Volume Tolerance (&#177;%): "
          ...
          if (expType == "VELOCITY")
            {
              ...
              html_analysis_gen
                  += tr ("<table style=\"margin-left:50px\">"
                         "<tr><td> Wavelength for Edit, 2DSA-FM & Fitmen "
                         "Stages (nm):   </td>  <td> %1 </td> </tr>"
                         "</table>")
                         .arg (rep_wvl) // 1
                  ;
            }
      ...
      // For ABDE, show if channel is reference one; if not, what reference is
      // used
      if (expType == "ABDE")
        {
          QString ref_channel;
          ...
          if (is_ref_channel)
          ...
          else
            {
              html_analysis_gen += tr ("<table style=\"margin-left:30px\">"
                                       ...
                                       "<td> <i> Reference Channel? </i> "
                                       "</td>  <td> %1 </td>"
                                       "<td> <i> Using Reference #: </i> "
                                       "</td>  <td> %2 </td>"
                                       "</tr>"
                                       "</table>")
                                       .arg (ref_channel) // 1
                                       .arg (QString::number (
                                           currAProf.ref_use_channels[i])) // 2
                  ;
            }
        }

      if (genMask_edited.ShowAnalysisGenParts["Channel General Settings"]
              .toInt ())
        {
      ...
      if (triple_report)
        {
          ...
          // for ABDE, identify only 1st wvl in the cahnnel, and proceed with
          // it only!
          ...
          for (int jj = 0; jj < chann_wvl_number; ++jj)
            {
              ...
              QString exp_dur_str = QString::number (hms_tot[0]) + "d "
                                    + QString::number (hms_tot[1]) + "h "
                                    + QString::number (hms_tot[2]) + "m ";

              if (genMask_edited
                      .ShowAnalysisGenParts["Report Parameters (per-triple)"]
                      .toInt ())
                {
                  html_analysis_profile
                      += tr ("<table style=\"margin-left:50px\">"
                             "<tr><td> <b><i> Report Parameters for Triple: "
                             "</i> &nbsp;&nbsp;&nbsp; %1 </b></td> </tr>"
                             "</table>")
                             .arg (triple_name) // 1
                      ;
                  ...
                  if (expType == "VELOCITY")
                    {
                      html_analysis_profile
                          += tr ("<table style=\"margin-left:70px\">"
                                 "<tr><td> Total Concentration:           "
                                 "</td>  <td> %1 </td> </tr>"
                                 "<tr><td> Total Concentration Tolerance: "
                                 "</td>  <td> %2 </td> </tr>"
                                 "<tr><td> RMSD (upper limit):            "
                                 "</td>  <td> %3 </td> </tr>"
                                 "<tr><td> Average Intensity:             "
                                 "</td>  <td> %4 </td> </tr>"
                                 "<tr><td> Experiment Duration:           "
                                 "</td>  <td> %5 </td> </tr>"
                                 "<tr><td> Experiment Duration Tolerance: "
                                 "</td>  <td> %6 </td> </tr>"
                                 "</table>")
                                 .arg (
                                     QString::number (reportGMP.tot_conc)) // 1
                                 .arg (QString::number (
                                     reportGMP.tot_conc_tol)) // 2
                                 .arg (QString::number (
                                     reportGMP.rmsd_limit)) // 3
                                 .arg (QString::number (
                                     reportGMP.av_intensity)) // 4
                                 .arg (exp_dur_str)           // 5
                                 .arg (QString::number (
                                     reportGMP.experiment_duration_tol)) // 6
                          ;
                    }
                  else if (expType == "ABDE")
                    {
                      html_analysis_profile
                          += tr ("<table style=\"margin-left:70px\">"
                                 ...
                                 .arg (exp_dur_str) // 1
                                 .arg (QString::number (
                                     reportGMP.experiment_duration_tol)) // 2
                          ;
                    }
                }
              ...
              for (int kk = 0; kk < report_items_number; ++kk)
                {
                  ...
                  if (genMask_edited
                          .ShowAnalysisGenParts
                              ["Report Item Parameters (per-triple)"]
                          .toInt ())
                    {
                      html_analysis_profile
                          += tr ("<table style=\"margin-left:90px\">"
                                 "<tr><td> <b><i> Report Item #%1: </i> "
                      ...
                      if (expType == "VELOCITY")
                        {
                          html_analysis_profile
                              += tr ("<table style=\"margin-left:110px\">"
                                     "<tr><td> Type:                  </td>  "
                                     "<td> %1 </td> </tr>"
                                     "<tr><td> Method:                </td>  "
                                     "<td> %2 </td> </tr>"
                                     ...
                                     "<tr><td> Range High:            </td>  "
                                     "<td> %4 </td> </tr>"
                                     "<tr><td> Integration Value:     </td>  "
                                     "<td> %5 </td> </tr>"
                                     "<tr><td> Tolerance (%):         </td>  "
                                     "<td> %6 </td> </tr>"
                                     "<tr><td> Fraction of Total (%): </td>  "
                                     "<td> %7 </td> </tr>"
                                     "</table>")
                                     .arg (curr_item.type)   // 1
                                     .arg (curr_item.method) // 2
                                     .arg (QString::number (
                                         curr_item.range_low)) // 3
                                     .arg (QString::number (
                                         curr_item.range_high)) // 4
                                     .arg (QString::number (
                                         curr_item.integration_val)) // 5
                                     .arg (QString::number (
                                         curr_item.tolerance)) // 6
                                     .arg (QString::number (
                                         curr_item.total_percent)) // 7
                              ;
                        }
                      else if (expType == "ABDE")
                        {
                          html_analysis_profile
                              += tr ("<table style=\"margin-left:110px\">"
                                     ...
                                     .arg (QString::number (
                                         curr_item.range_high)) // 4
                                     .arg (QString::number (
                                         curr_item.tolerance)) // 5
                                     .arg (QString::number (
                                         curr_item.total_percent)) // 6
                              ;
                        }
                    }
                }
  ...
  html_analysis_profile_2dsa
      = tr ("<h3 align=left> Analysis Profile: 2DSA Controls </h3>"
            "&nbsp;&nbsp;<br>");

  // Job Flow Summary:
  QString html_analysis_profile_2dsa_flow;
  html_analysis_profile_2dsa_flow += tr ("<table>"
                                         "<tr>"
                                         "<td><b>Job Flow Summary:</b></td>"
                                         "</tr>"
                                         "</table>");
  html_analysis_profile_2dsa_flow
      += tr ("<table style=\"margin-left:10px\">"
             // 2DSA
             "<tr>"
             "<td> \"2DSA  (TI Noise)\": </td>"
  ...
  html_analysis_profile_2dsa_per_channel
      += tr ("<table>"
             "<tr>"
  ...
  for (int i = 0; i < nchna_2dsa; i++)
    {
      ...
      html_analysis_profile_2dsa_per_channel
          += tr ("<table style=\"margin-left:30px\">"
                 "<tr><td> s Min, Max, Grid Points:     </td>  <td> %1 </td> "
                 "</tr>"
                 "<tr><td> f/f0 Min, Max, Grid Points:  </td>  <td> %2 </td> "
                 "</tr>"
                 "<tr><td> Grid Repetitions:            </td>  <td> %3 </td> "
                 "</tr>"
                 "<tr><td> Custom Grid:                 </td>  <td> %4 </td> "
                 "</tr>"
                 "<tr><td> Varying Vbar:                </td>  <td> %5 </td> "
                 "</tr>"
                 "<tr><td> Constant f/f0:               </td>  <td> %6 </td> "
                 "</tr>"
                 "</table>")
                 .arg (s_data)      // 1
                 .arg (ff0_data)    // 2
  ...
  html_analysis_profile_pcsa
      = tr ("<h3 align=left> Analysis Profile: PCSA Controls </h3>"
            "&nbsp;&nbsp;<br>");

  QString html_analysis_profile_pcsa_flow;
  html_analysis_profile_pcsa_flow
      += tr ("<table style=\"margin-left:10px\">"
             // PCSA
             "<tr>"
             "<td> \"PCSA\":    </td>"
  ...
  if (cAPp.job_run)
    {
      ...
      for (int i = 0; i < nchna_pcsa; i++)
        {
          html_analysis_profile_pcsa_per_channel
              += tr ("<table>"
                     "<tr>"
                     "<td><b>Channel:</b> &nbsp;&nbsp;&nbsp;&nbsp; </td> "
                     "<td><b>%1</b></td>"
          ...
          html_analysis_profile_pcsa_per_channel
              += tr ("<table style=\"margin-left:30px\">"
                     "<tr><td> Curve Type:                </td>  <td> %1 "
                     "</td> </tr>"
                     "<tr><td> X Axis Type, Min, Max:     </td>  <td> %2 "
                     "</td> </tr>"
                     "<tr><td> Y Axis Type, Min, Max:     </td>  <td> %3 "
                     "</td> </tr>"
                     "<tr><td> Z Axis Type, Value:        </td>  <td> %4 "
                     "</td> </tr>"
                     "<tr><td> Variations Count:          </td>  <td> %5 "
                     "</td> </tr>"
                     "<tr><td> Grid Fit Iterations:       </td>  <td> %6 "
                     "</td> </tr>"
                     "<tr><td> Curve Resolution Points:   </td>  <td> %7 "
                     "</td> </tr>"
                     "<tr><td> Noise Type:                </td>  <td> %8 "
                     "</td> </tr>"
                     "<tr><td> Tikhonov Regularization:   </td>  <td> %9 "
                     "</td> </tr>"
                     "<tr><td> Tikhonov Alpha:            </td>  <td> %10 "
                     "</td> </tr>"
                     "<tr><td> MonteCarlo Iterations:     </td>  <td> %11 "
                     "</td> </tr>"
                     "</table>")
  ...
  html_assembled += html_header + html_title + html_paragraph_open;
...
// write PDF Report
void
US_ReporterGMP::write_pdf_report (void)
{
  QString html_paragraph_close = tr ("</p>");

  QString html_footer = tr ("<div align=right>"
  ...
  QString fileName = ProtocolName_auto
                     + ".pdf"; // Use unique protocol name for .pdf filename
  ...
  // /**************************************************************************************/
  //  //TEST !!! Write HTML file to BLOB instead SEPARATELY for hard-coded GMP
  //  Report IDs:
  //  //
  //  //
  //  // Choose one of the below!
  //  //int r_ID = 1;  // < --- [FOR SBird-DNA-EcoRI-101322-PD9 ..
  //  autoflowID=838] int r_ID = 12;   // < --- [FOR OkaK_MW-AAV8_17-19_062823
  //  .. autoflowID=930]
  ...
  //    					  QString(
  //    "upload_gmpReportData_html" ), 					  r_ID );
  ...
  //    else if ( writeStatus_html != US_DB2::OK )
  //      {
  //        QMessageBox::warning(this, "Error", "returned processing html
  //        file:\n" +
  //    			   html_filePath + "\n" + db.lastError() +
  //    "\n");
  ...
  //    //file_exts << "*.png" << "*.svgz";
  //    file_exts << "*.svgz" << "*.html";                // retain *pngs (BUT
  //    remove *html) for further assembly at e-Signing remove_files_by_mask(
  //    dirName, file_exts );
  ...
  //    process->start("tar", QStringList() << "-cvf" << tarFilename_t <<
  //    subDirName );
  //    //process->start("rar", QStringList() << "a" << tarFilename_t <<
  //    subDirName );
  //    //sleep( 5 );                     //Maybe implement something like a
  //    timer to check on completion!!! process -> waitForFinished(); process
  //    -> close();

  //    qDebug() << "tar command: " << "tar " << "-cvf " << tarFilename_t << ",
  //    " << subDirName;
  ...
  if (auto_mode)
    {
      ...
      // file_exts << "*.png" << "*.svgz";
      file_exts << "*.svgz" << "*.html"
                << "*.txt"; // retain *pngs (BUT remove *html) for further
                            // assembly at e-Signing
      remove_files_by_mask (dirName, file_exts);

      // Archive using US_Tar [does NOT work for filename lengths >=100 char]
      /****************************************************************************
      QDir odir( dirName );
      QStringList fileList = odir.entryList( QStringList( "*.pdf" ),
      QDir::Files );
      ...
      int result;
       ...
       if ( result != TAR_OK )
         {
           QString errormsg = QString("Error: Problem creating tar archive
      %1").arg( tarFilename );
      ...
      ***************************************************************************/
      ...
      //  QProcess *process = new QProcess(this);
      //  process->setWorkingDirectory( US_Settings::reportDir() );
      //  process->start("tar", QStringList() << "-czvf" << tarFilename_t <<
      //  subDirName );
      // END TESTING
      // ***********************************************************************
...
double
US_ReporterGMP::mmToPixels (QPrinter &printer, int mm)
{
  return mm * 0.039370147 * printer.resolution ();
}

void
US_ReporterGMP::printDocument (QPrinter &printer,
                               QTextDocument *doc) //, QWidget* parentWidget)
{
  ...
  qDebug () << "textRect=, width, height: " << textRect << textRect.width ()
            << textRect.height ();
  qDebug () << "footerHeigh: " << footerHeight;
  doc->setPageSize (textRect.size ());

  const int pageCount = doc->pageCount ();
  // QProgressDialog dialog( QObject::tr( "Printing" ), QObject::tr( "Cancel"
  // ), 0, pageCount, parentWidget ); dialog.setWindowModality(
  // Qt::ApplicationModal );
  progress_msg = new QProgressDialog ("Preparing .PDF...", QString (), 0,
                                      pageCount, this);
  progress_msg->setWindowFlags (Qt::Tool | Qt::WindowTitleHint
                                | Qt::CustomizeWindowHint);
  progress_msg->setModal (true);
  ...
  for (int pageIndex = 0; pageIndex < pageCount; ++pageIndex)
    {
      ...
      paintPage (printer, pageIndex, pageCount, &painter, doc, textRect,
                 footerHeight);
      firstPage = false;
    }
...
void
US_ReporterGMP::paintPage (QPrinter &printer, int pageNumber, int pageCount,
                           ...
                           const QRectF &textRect, qreal footerHeight)
{
  ...
  // const QRectF borderRect(bm, bm, pageSize.width() - 2 * bm,
  // pageSize.height() - 2 * bm);
  // painter->drawRect(borderRect);

  painter->save ();

  // textPageRect is the rectangle in the coordinate system of the
  // QTextDocument, in pixels, and starting at (0,0) for the first page. Second
  // page is at y=doc->pageSize().height().
  const QRectF textPageRect (0, pageNumber * doc->pageSize ().height (),
                             doc->pageSize ().width (),
                             doc->pageSize ().height ());
  // Clip the drawing so that the text of the other pages doesn't appear in the
  // margins
  ...
  doc->drawContents (painter,
                     textPageRect); // add second params, OR it appears to draw
                                    // full QTextDoc.... (huge sizes)
  painter->restore ();

  // Footer: e-Signer comment && page number
  QRectF footerRect = textRect;
  footerRect.setTop (textRect.bottom ());
  footerRect.setHeight (
      1 * footerHeight); // will a parameter on #of lines (footer height,
                         // depending on # reviewers...)
  // test below
  // footerRect.setHeight( 4*footerHeight ); //will a parameter on #of lines
  // (footer height, depending on # reviewers...) end test

  painter->setPen (Qt::blue);
  QFont pfont = painter->font ();
  qDebug () << "Paint page: painter: pixelSize(), pointSize() -- "
            << pfont.pixelSize () << pfont.pointSize ();
  ...
  // painter->drawText(footerRect, Qt::AlignLeft, QObject::tr("Not e-Signed/Not
  // Reviewed..." ));
  painter->drawText (footerRect, Qt::AlignLeft, init_footer);
  // test below
  // painter->drawText(footerRect, Qt::AlignLeft, QObject::tr("Not e-Signed/Not
  // Reviewed...\nNot e-Signed/Not Reviewed...\nNot e-Signed/Not
  // Reviewed...\nNot e-Signed/Not Reviewed..." )); end test
  painter->drawText (
      footerRect, Qt::AlignVCenter | Qt::AlignRight,
      QObject::tr ("Page %1/%2").arg (pageNumber + 1).arg (pageCount));

  // Footer: page number or "end"
  // if (pageNumber == pageCount - 1)
  //   painter->drawText(footerRect, Qt::AlignLeft, QObject::tr("Footer to be
  //   passed by e-Signers"));
  // else
...
void
US_ReporterGMP::write_gmp_report_DB (QString filename, QString filename_pdf)
{
  ...
  // BEFORE writing, check if writing has been initiated, or completed from
  // different session:
  int status_report_unique = 0;
  qry << "autoflow_report_status" << AutoflowID_auto;
  ...
  if (!status_report_unique || status_report_unique < 0)
    {
      QMessageBox::information (
          this, tr ("The Program State Updated / Being Updated"),
  ...
  qry.clear ();
  qry << "new_autoflow_gmp_report_record" << AutoflowID_auto
      << FullRunName_auto << ProtocolName_auto << filename_pdf;
  ...
  qDebug () << "state_new_gmpReport, autolfowGMPReportID -- "
            << state_new_gmpReport << autolfowGMPReportID;
  ...
  if (state_new_gmpReport == US_DB2::OK && autolfowGMPReportID > 0)
    {
      ...
      int writeStatus_html = db.writeBlobToDB (
          html_filePath, QString ("upload_gmpReportData_html"),
          autolfowGMPReportID);
      ...
      if (clear_GMP_report_record)
        {
          qry.clear ();
          qry << "clear_autoflowGMPReportRecord"
              << QString::number (autolfowGMPReportID);
          db.query (qry);
      ...
      else
        {
          ...
          qry.clear ();
          qry << "update_autoflowStatusReport_record" << autoflowStatusID
              << AutoflowID_auto << reporting_Json;
    ...
    {
      QMessageBox::warning (this, "Error returned processing file",
                            "File: " + report_filepath + db.lastError ());
    }
}

// remove files by extension from dirPath
void
US_ReporterGMP::remove_files_by_mask (QString dirPath, QStringList file_exts)
{
  QDir dir (dirPath);
...
void
US_ReporterGMP::gui_to_parms (void)
{
  ...
  if (expType == "VELOCITY")
    ...
    parse_edited_perChan_mask_json_abde (editedMask_perChan,
                                         perChanMask_edited_abde);

  // tree-to-json: combPlotsTree
  QString editedMask_combPlots = tree_to_json (topItemCombPlots);
  parse_edited_combPlots_mask_json (editedMask_combPlots,
                                    combPlotsMask_edited);
  ...
  if (auto_mode)
    ...
    {
      if (editedMask_gen != JsonMask_gen_loaded
          || editedMask_perChan != JsonMask_perChan_loaded)
        GMP_report = false;
    }
  // DEBUG
  // exit(1);
...
void
US_ReporterGMP::parse_edited_misc_mask_json (const QString maskJson,
                                             MiscReportMaskStructure &MaskStr)
{
...
void
US_ReporterGMP::parse_edited_gen_mask_json (const QString maskJson,
                                            GenReportMaskStructure &MaskStr)
{
  QJsonDocument jsonDoc = QJsonDocument::fromJson (maskJson.toUtf8 ());
  QJsonObject json = jsonDoc.object ();
                  ...
                  if (key.contains ("Solutions"))
                    {
                      ...
                      qDebug () << "Parse_editedJsonGen: Solution: array_key, "
                                   "value: "
                                << array_key
                                << json_array[i]
                                       .toObject ()
                      ...
                      MaskStr.ShowSolutionParts[array_key]
                          = json_array[i]
                                .toObject ()
                                .value (array_key)
                                .toString ();
                  ...
                  if (key.contains ("Analysis"))
                    {
                      ...
                      foreach (const QString &n_key, newObj.keys ())
                        {
                          QString analysis_cathegory_item_value
                              = newObj.value (n_key).toString ();
                          ...
                          if (array_key.contains ("General"))
                            {
                              MaskStr.ShowAnalysisGenParts[n_key]
                                  = analysis_cathegory_item_value;
                              if (MaskStr.ShowAnalysisGenParts[n_key].toInt ())
                                ++MaskStr.has_anagen_items;

                              qDebug () << "Parse_editedJsonGen: "
                                           "Analysis:Gen: n_key, value: "
                                        << n_key
                                        << MaskStr.ShowAnalysisGenParts[n_key];
                            }
                          ...
                          if (array_key.contains ("2DSA"))
                            {
                              MaskStr.ShowAnalysis2DSAParts[n_key]
                                  = analysis_cathegory_item_value;
                              if (MaskStr.ShowAnalysis2DSAParts[n_key]
                                      .toInt ())
                                ++MaskStr.has_ana2dsa_items;

                              qDebug ()
                                  << "Parse_editedJsonGen: Analysis:2DSA: "
                                     "n_key, value: "
                                  << n_key
                                  << MaskStr.ShowAnalysis2DSAParts[n_key];
                            }
                          ...
                          if (array_key.contains ("PCSA"))
                            {
                              MaskStr.ShowAnalysisPCSAParts[n_key]
                                  = analysis_cathegory_item_value;
                              if (MaskStr.ShowAnalysisPCSAParts[n_key]
                                      .toInt ())
                                ++MaskStr.has_anapcsa_items;

                              qDebug ()
                                  << "Parse_editedJsonGen: Analysis:PCSA: "
                                     "n_key, value: "
                                  << n_key
                                  << MaskStr.ShowAnalysisPCSAParts[n_key];
                            }
...
void
US_ReporterGMP::parse_edited_perChan_mask_json_abde (
    const QString maskJson, PerChanReportMaskStructureABDE &MaskStr)
{
      ...
      foreach (const QString &n_key, tripleObj.keys ()) // over single channel!
        {
          ...
          qDebug () << "channel's " << key << " feature: " << n_key
                    << ", value: " << feature_value;

          if (feature_value.isString ()) // all strings by default; may be more
                                         // complex if JSON is complexer...
            {
              if (feature_value.toString ().toInt ())
                ++has_channel_items;

              MaskStr.ShowChannelItemParts[key][n_key]
...
void
US_ReporterGMP::parse_edited_perChan_mask_json (
    const QString maskJson, PerChanReportMaskStructure &MaskStr)
{
  QJsonDocument jsonDoc = QJsonDocument::fromJson (maskJson.toUtf8 ());
  QJsonObject json = jsonDoc.object ();
  ...
  foreach (const QString &key, json.keys ()) // over channels
    {
      ...
      for (int i = 0; i < json_array.size (); ++i)
        {
          foreach (const QString &array_key,
                   json_array[i].toObject ().keys ()) // over triples
            {
              QJsonObject tripleObj
                  = json_array[i].toObject ().value (array_key).toObject ();

              foreach (const QString &n_key, tripleObj.keys ()) // over models
                {
                  QJsonObject modelObj = tripleObj.value (n_key).toObject ();

                  foreach (const QString &j_key,
                           modelObj.keys ()) // over models' features
                                             // (tot_conc, RMSD...)
                    {
                      QJsonValue feature_value = modelObj.value (j_key);

                      if (feature_value.isString ()) // deal with plots main
                                                     // characteristics (
                                                     // tot_conc, RMSD)
                        {
                          if (feature_value.toString ().toInt ())
                          ...
                          // QString triple_name = key.split("
                          // ")[1].split("-")[0] + array_key.split(" ")[0];
                          QString triple_name
                              = key.split (" ")[1].split ("-")[0];
                          ...
                          MaskStr.ShowTripleModelParts[triple_name][model_name]
                                                      [j_key]
                              = feature_value.toString ();
                          if (MaskStr
                                  .ShowTripleModelParts[triple_name]
                                                       [model_name][j_key]
                                  .toInt ())
                            ++MaskStr.has_tripleModel_items[triple_name]
                                                           [model_name];

                          qDebug () << "Parse_editedJsonTriples: triple_name: "
                                    << triple_name << ": "
                      ...
                      else
                        {
                          ...
                          if (j_key.contains ("Plots"))
                            {
                              QJsonObject plotObj
                                  = modelObj.value (j_key).toObject ();

                              foreach (
                                  const QString &p_key,
                                  plotObj.keys ()) // over plots, per model/per
                                                   // triple/per channel
                                {
                                  QString feature_plot_value
                                      = plotObj.value (p_key).toString ();

                                  // QString triple_name = key.split("
                                  // ")[1].split("-")[0]  + array_key.split("
                                  // ")[0];
                                  QString triple_name
                                      = key.split (" ")[1].split ("-")[0];
                                  ...
                                  QString model_name = n_key.split (" ")[0];
                                  MaskStr.ShowTripleModelPlotParts[triple_name]
                                                                  [model_name]
                                                                  [p_key]
                                      = feature_plot_value;
                                  if (MaskStr
                                          .ShowTripleModelPlotParts
                                              [triple_name][model_name][p_key]
                                          .toInt ())
                                    ++MaskStr.has_tripleModelPlot_items
                                          [triple_name][model_name];

                                  qDebug () << "Parse_editedJsonTriples: "
                                               "triple_name: "
                                            << triple_name << ": "
                                            << "model_name, j_key, p_key, "
                                               "feature_PLOT_value: "
                                            << model_name << j_key << p_key
                                            << feature_plot_value;
                                }
                          ...
                          // begin "Pseudo3D"
                            ...
                            {
                              QJsonObject pseudo3dObj
                                  = modelObj.value (j_key).toObject ();

                              foreach (
                                  const QString &p_key,
                                  pseudo3dObj
                                      .keys ()) // over pseudo3d, per model/per
                                                // triple/per channel
                                {
                                  QString feature_pseudo3d_value
                                      = pseudo3dObj.value (p_key).toString ();
                                  ...
                                  QString model_name = n_key.split (" ")[0];
                                  MaskStr.ShowTripleModelPseudo3dParts
                                      [triple_name][model_name][p_key]
                                      = feature_pseudo3d_value;
                                  if (MaskStr
                                          .ShowTripleModelPseudo3dParts
                                              [triple_name][model_name][p_key]
                                          .toInt ())
                                    ++MaskStr.has_tripleModelPseudo3d_items
                                          [triple_name][model_name];
                          ...
                          // begin "Individual"
                            ...
                            {
                              QJsonObject indComboObj
                                  = modelObj.value (j_key).toObject ();

                              foreach (
                                  const QString &p_key,
                                  indComboObj
                                      .keys ()) // over type[ranges], per
                                                // model/per triple/per channel
                                {
                                  QString feature_indCombo_value
                                      = indComboObj.value (p_key).toString ();
                                  ...
                                  MaskStr
                                      .ShowTripleTypeModelRangeIndividualCombo
                                          [triple_name][model_name][p_key]
                                  ...
                                  if (MaskStr
                                          .ShowTripleTypeModelRangeIndividualCombo
                                              [triple_name][model_name][p_key]
                                          .toInt ())
                                    ++MaskStr.has_tripleModelIndCombo_items
                                          [triple_name][model_name];
...
void
US_ReporterGMP::parse_edited_combPlots_mask_json (
    const QString maskJson, CombPlotsReportMaskStructure &MaskStr)
{
  QJsonDocument jsonDoc = QJsonDocument::fromJson (maskJson.toUtf8 ());
  QJsonObject json = jsonDoc.object ();
  ...
  foreach (const QString &key, json.keys ()) // over types
    {
      ...
      for (int i = 0; i < json_array.size (); ++i)
        {
          foreach (
              const QString &array_key,
              json_array[i].toObject ().keys ()) // over models (aka methods)
            {
              QJsonValue show_plot
                  = json_array[i].toObject ().value (array_key);

              QString array_key_mod = array_key;
              if (array_key.contains ("PCSA"))
                array_key_mod = "PCSA";

              MaskStr.ShowCombPlotParts[key][array_key_mod]
                  = show_plot.toString ();

              qDebug () << "CombPlots: type, model, yes/no -- " << key
                        << array_key_mod << show_plot.toString ();
...
void
US_ReporterGMP::get_children_to_json (QString &mask_edited,
                                      QTreeWidgetItem *item)
{
  for (int i = 0; i < item->childCount (); ++i)
    {
      ...
      if (!children_lev2)
        {
          mask_edited
              += "\"" + QString::number (int (item->child (i)->checkState (0)))
                 + "\"";
  ...
  // ALEXEY: <-- little trick to enable super-fast recursive over arbitrary
  // tree:))
  mask_edited.replace (",},", "},");
}
...
QString
US_ReporterGMP::tree_to_json (QMap<QString, QTreeWidgetItem *> topLevItems)
{
  QString mask_edited;
  mask_edited += "{";
  ...
  for (top = topLevItems.begin (); top != topLevItems.end (); ++top)
    {
      ...
      if (!children_lev1)
        {
          mask_edited += "\""
                         + QString::number (int (top.value ()->checkState (0)))
                         + "\",";
        }
      else
        {
  ...
  QString new_substr = "}]";
  if (!mask_edited
           .mid (mask_edited.lastIndexOf (to_replace) + to_replace.size (),
                 mask_edited.length ())
           .contains ("\""))
    mask_edited.replace (mask_edited.lastIndexOf (to_replace),
                         to_replace.size (), new_substr);
...
QString US_ReporterGMP::tree_to_json( QMap < QString, QTreeWidgetItem * >
topLevItems )
{
  QString mask_edited;
  mask_edited += "{";
  ...
  for ( top = topLevItems.begin(); top != topLevItems.end(); ++top )
    {
      qDebug() << "Top item " << top.key() << " is " <<
int(top.value()->checkState(0)) << "\n";
...
int(top.value()->checkState(0)) ) + "\",";
      ...
      else
        {
          ...
          for( int i = 0; i < top.value()->childCount(); ++i )
            {
              qDebug() << "\tThe child's " << top.value()->child(i)->text(1) <<
", state is: " << top.value()->child(i)->checkState(0);
...
"\":";
              ...
              if ( !children_lev2 )
                {
                  mask_edited += "\"" + QString::number(
int(top.value()->child(i)->checkState(0)) ) + "\"}"; if ( i !=
top.value()->childCount()-1 ) mask_edited += ",";
                }
              else
                {
                  mask_edited += "{";
                  for( int ii = 0; ii < top.value()->child(i)->childCount();
++ii )
                    {
                      qDebug() << "\t\tThe child's " <<
top.value()->child(i)->child(ii)->text(1) << ", state is: " <<
top.value()->child(i)->child(ii)->checkState(0);

                      mask_edited += "\"" +
top.value()->child(i)->child(ii)->text(1).trimmed() + "\":"; int children_lev3
= top.value()->child(i)->child(ii)->childCount(); if ( !children_lev3 )
                        {
                          mask_edited += "\"" + QString::number(
int(top.value()->child(i)->child(ii)->checkState(0)) ) + "\""; if ( ii !=
top.value()->child(i)->childCount()-1 ) mask_edited += ",";
                        }
                      else
                        {
                          //Here 3th level of nestedness may be considered if
needed... and so on...
                        }
...
// Format times
void
US_ReporterGMP::format_needed_params ()
{
  // Duration
  QList<int> hms_dur;
  double duration = currProto.rpSpeed.ssteps[0].duration;
  US_RunProtocol::timeToList (duration, hms_dur);
  duration_str = QString::number (hms_dur[0]) + "d "
                 + QString::number (hms_dur[1]) + "h "
                 + QString::number (hms_dur[2]) + "m ";
  ...
  delay_stage_str = // QString::number( hms_delay_stage[ 0 ] ) + "d " +
      QString::number (hms_delay_stage[1]) + "h "
      + QString::number (hms_delay_stage[2]) + "m ";

  // Total Time
  QList<int> hms_tot;
  double total_time = currProto.rpSpeed.ssteps[0].total_time;
  US_RunProtocol::timeToList (total_time, hms_tot);
  total_time_str = QString::number (hms_tot[0]) + "d "
                   + QString::number (hms_tot[1]) + "h "
                   + QString::number (hms_tot[2]) + "m ";
  ...
  delay_uvvis_str = QString::number (hms_delay_uvvis[0]) + "d "
                    + QString::number (hms_delay_uvvis[1]) + "h "
                    + QString::number (hms_delay_uvvis[2]) + "m ";
  ...
  scanint_uvvis_str = // QString::number( hms_scanint_uvvis[ 0 ] ) + "d " +
      QString::number (hms_scanint_uvvis[1]) + "h "
      + QString::number (hms_scanint_uvvis[2]) + "m "
      + QString::number (hms_scanint_uvvis[3]) + "s ";
  ...
  delay_int_str = QString::number (hms_delay_int[0]) + "d "
                  + QString::number (hms_delay_int[1]) + "h "
                  + QString::number (hms_delay_int[2]) + "m ";
  ...
  scanint_int_str = // QString::number( hms_scanint_uvvis[ 0 ] ) + "d " +
      QString::number (hms_scanint_int[1]) + "h "
      + QString::number (hms_scanint_int[2]) + "m "
      + QString::number (hms_scanint_int[3]) + "s ";
  qDebug () << "ScanInt Interference str: " << scanint_int_str;
}

// get current date
void
US_ReporterGMP::get_current_date ()
{
...
void
US_ReporterGMP::assemble_parts (QString &html_str)
{
  // info on failed analyses
  html_str += html_failed;

  QMap<QString, bool>::iterator top;
  for (top = genMask_edited.ShowReportParts.begin ();
       top != genMask_edited.ShowReportParts.end (); ++top)
    {
...
void
US_ReporterGMP::add_solution_details (const QString sol_id,
                                      const QString sol_comment,
                                      QString &html_solutions)
{
  ...
  html_solutions
      += tr ("<table style=\"margin-left:5px\">"
             "<caption align=left> <b><i>Solution Information </i></b> "
             "</caption>"
             "</table>"
  ...
  for (int i = 0; i < num_analytes; ++i)
    {
      ...
      if (seqlen == 0)
        ...
        {
          seqsmry = seqsmry.toLower ().remove (QRegExp ("[\\s0-9]"));
      ...
      if (all_abs > 0)
        {
          ...
          if (cys > 0)
            absorbing_residues
                += QString ().sprintf ("%d", cys) + " " + "Cys" + ", ";
          if (hao > 0)
            absorbing_residues
                += QString ().sprintf ("%d", hao) + " " + "Hao" + ", ";
          if (orn > 0)
            absorbing_residues
                += QString ().sprintf ("%d", orn) + " " + "Orn" + ", ";
          if (trp > 0)
            absorbing_residues
                += QString ().sprintf ("%d", trp) + " " + "Trp" + ", ";
          if (tyr > 0)
            absorbing_residues
                += QString ().sprintf ("%d", tyr) + " " + "Tyr" + ", ";
      ...
      analyte_detailed_info
          += tr ("<tr> <td> Type: </td> <td>%1</td>              </tr>"
      ...
      if (analyte.type == 0)
        {
          analyte_detailed_info
              += tr ("<tr> <td> AAs absorbing at 280 nm: </td> <td>%1</td> "
                     "</tr>"
                     "<tr> <td> E280: </td>  <td> %2</td>                  "
                     "</tr>"
                     "<tr> <td> Extinction count: </td>  <td> %3</td>      "
                     "</tr>")
                     .arg (absorbing_residues)
                     .arg (QString::number (analyte.extinction[280.0]))
                     .arg (QString::number (
                         analyte.extinction.keys ().count ()));
        }

      // add info on the current analyte to html_solutions string
      if (genMask_edited.ShowSolutionParts["Analyte Information"].toInt ())
        {
          html_solutions += tr ("<table style=\"margin-left:40px\">"
                                "%1"
                                "</table>"
  ...
  buffer_detailed_info
      += tr ("<tr><td> Density (20&#8451;, g/cm<sup>3</sup>):  </td>   "
             "<td>%1</td>  </tr>"
             "<tr><td> Viscosity (20&#8451;, cP): </td>   <td>%2</td>         "
             "      </tr>"
             "<tr><td> pH:       </td>   <td>%3 </td>                         "
             "     </tr>"
             "<tr><td> Compressibility:</td>   <td>%4</td>                    "
             "     </tr>")
  ...
  for (int i = 0; i < buffer.component.size (); ++i)
    {
      QString component_desc = buffer.component[i].name + " ("
                               + QString::number (buffer.concentration[i])
                               + " " + buffer.component[i].unit + ")";

      buffer_components_info += tr ("<tr><td> Component Name:</td> "
                                    "&nbsp;&nbsp;  <td>%1</td>  </tr>")
                                    .arg (component_desc);
    }
  ...
  if (genMask_edited.ShowSolutionParts["Buffer Information"].toInt ())
    {
      html_solutions += tr ("<table style=\"margin-left:30px\">"
                            "<caption align=left> <b><i>Buffer "
                            "Information</i></b> </caption>"
                            "</table>"
