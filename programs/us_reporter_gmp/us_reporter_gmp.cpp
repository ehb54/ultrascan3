...
US_ReporterGMP::US_ReporterGMP(QMap<QString, QString> t_c) : US_Widgets() {
  QString report_filepath =
      "/home/alexey/ultrascan/reports/eGFP-DNA-MW-08OCT23-run1981_GMP_DB.tar";
  QString html_filePath =
      "/home/alexey/ultrascan/reports/eGFP-DNA-MW-08OCT23-run1981/"
      "html_string.html";
...
void US_ReporterGMP::write_gmp_report_DB_test(QString report_filepath,
                                              QString html_filePath,
                                              int autolfowGMPReportID,
                                              QString autoStatusID,
                                              QString autoID) {
  ...
  int writeStatus_html = db.writeBlobToDB(
      html_filePath, QString("upload_gmpReportData_html"), autolfowGMPReportID);
...
US_ReporterGMP::US_ReporterGMP() : US_Widgets() {
  ...
  QLabel* bn_actions =
      us_banner(tr("Generate GMP or Custom Report from Completed GMP Run:"), 1);
  ...
  connect(pb_gen_report, SIGNAL(clicked()), this, SLOT(generate_report()));
  ...
  connect(pb_view_report_db, SIGNAL(clicked()), this, SLOT(view_report_db()));
...
US_ReporterGMP::US_ReporterGMP(QString a_mode) : US_Widgets() {
  ...
  connect(pb_view_report_auto, SIGNAL(clicked()), this, SLOT(view_report()));
...
void US_ReporterGMP::loadRun_auto(QMap<QString, QString>& protocol_details) {
  ...
  progress_msg = new QProgressDialog("Accessing run's protocol...", QString(),
                                     0, 12, this);
  ...
  if (!GMP_report) {
    QMessageBox::information(
        ...
        tr("<font color='red'><b>ATTENTION:</b> There are missing models for "
           "certain triples: </font><br><br>"
           "%1<br><br>"
...
QMap<QString, QString> US_ReporterGMP::read_autoflowGMPReportEsign_record(
    US_DB2* db) {
...
QString US_ReporterGMP::get_assigned_oper_revs(QJsonDocument jsonDoc) {
  ...
  for (int ii = 0; ii < assigned_list.count(); ii++) {
    ...
    if (ii != assigned_list.count() - 1) smry += "; ";
  }
  ...
  for (int i = 0; i < analysisIDs_list.size(); ++i) {
    ...
    if (!jsonDoc.isObject()) {
      ...
      QMessageBox::warning(
          ...
          tr("JSON message for status of the analysis performed on triple %1 "
             "appears to be corrupted. "
             "This may be an indicaiton of the problem with the analysis run "
             "for this triple.\n\n"
    ...
    if (status == "FAILED" || status == "CANCELED") {
      if (processed.isUndefined())
        ...
        qDebug() << "GMP Report: stage_failed for triple -- " << triple_name
                 << stage_failed;
...
QMap<QString, QString> US_ReporterGMP::read_autoflowAnalysis_record(
    US_DB2* db, const QString& requestID) {
  QMap<QString, QString> analysis_details;
...
QMap<QString, QString> US_ReporterGMP::read_autoflowAnalysisHistory_record(
    US_DB2* db, const QString& requestID) {
  QMap<QString, QString> analysis_details;
...
void US_ReporterGMP::check_for_missing_models(void) {
  ...
  for (mm = Triple_to_Models.begin(); mm != Triple_to_Models.end(); ++mm) {
    ...
    if (hasIT && !mm.value().contains("2DSA-IT")) missing_models << "2DSA-IT";
    if (hasMC && !mm.value().contains("2DSA-MC")) missing_models << "2DSA-MC";
    if (hasPCSA && !mm.value().contains("PCSA")) missing_models << "PCSA";
...
QString US_ReporterGMP::missing_models_msg(void) {
  ...
  for (mmm = Triple_to_ModelsMissing.begin();
    ...
    if (!mmm.value().isEmpty()) {
      ...
      if (isDropped) models_str += "<font color='red'> [triple dropped]</font>";
...
QString US_ReporterGMP::compose_html_failed_stage_missing_models(void) {
  ...
  // 	  failed_str += "<tr><td style=\"color:red;\">" + mm.key() + ",</td><td>
  // analysis failed/canceled at stage: </td><td style=\"color:red;\">"
  // 	                + mm.value() + ";</td>" +
  ...
  for (mmm = Triple_to_ModelsMissing.begin();
    ...
    if (!mmm.value().isEmpty()) {
      ...
      failed_str += "<tr><td style=\"color:red;\">" + mmm.key() + ":</td>" +
                    +"<td>Models missing: </td><td style=\"color:red;\">" +
                    mmm.value().join(", ") + "</td>";
      if (isDropped) failed_str += "<td>[Triple dropped]</td>";
...
void US_ReporterGMP::check_models(int autoflowID) {
  ...
  for (int i = 0; i < nchna; i++) {
    ...
    if (analysis_to_be_run == 0 || report_to_be_run == 0) continue;
  ...
  for (int i = 0; i < Array_of_tripleNames.size(); ++i) {
    ...
    while (db->next()) {
      ...
      // QString  date                = US_Util::toUTCDatetimeText( db->value( 3
      // ).toDateTime().toString( "yyyy/MM/dd HH:mm" ), true );
      ...
      if (description.contains("2DSA-IT") &&
        ...
        qDebug() << "2DSA-IT Ids: modelID, read from modelLink: " << modelID
      ...
      if (description.contains("2DSA-MC") &&
        ...
        qDebug() << "2DSA-MC Ids: modelID, read from modelLink: " << modelID
                 << Triple_to_ModelsDesc[Array_of_tripleNames[i]]["2DSA-MC"];
        model_list << "2DSA-MC";
      }

      if (description.contains("PCSA") &&
          modelID == Triple_to_ModelsDesc[Array_of_tripleNames[i]]["PCSA"]) {
        qDebug() << "PCSA Ids: modelID, read from modelLink: " << modelID
...
QMap<QString, QString> US_ReporterGMP::parse_models_desc_json(
    QString modelDescJson, QString model_property) {
        ...
        for (int i = 0; i < json_array.size(); ++i) {
          foreach (const QString& array_key, json_array[i].toObject().keys()) {
            ...
            if (array_key == "modelID" && array_key == model_property) {
              // if ( !modelDesc_shortened.contains( key_mod ) )  //Temporary,
              // for PCSA (2 entries)
              //   {
            ...
            if (array_key == "modelGUID" && array_key == model_property) {
                      ...
                      key_mod))  // Temporary, for PCSA (2 entries)
              {
                modelDesc_shortened[key_mod] =
                    json_array[i].toObject().value(array_key).toString();
                qDebug()
                    << "modelDescJson Map: -- model, property, value: "
                    << key_mod << array_key
                    << json_array[i].toObject().value(array_key).toString();
              }
            }
            ...
            if (array_key == "editeddataID") {
              modelDesc_shortened["eID"] =
                  json_array[i].toObject().value(array_key).toString();
              qDebug() << "modelDescJson Map: -- meID, value: " << array_key
                       << json_array[i].toObject().value(array_key).toString();
            }
          }
        }
...
void US_ReporterGMP::load_gmp_report_db(void) {
  ...
  if (db.lastErrno() != US_DB2::OK) {
    QMessageBox::warning(
        this, tr("LIMS DB Connection Problem"),
        tr("Could not connect to database \n") + db.lastError());
    return;
  }
  ...
  filePath_db =
      dirName + "/" + gmpReport_runname_selected + "/" + gmpReport_filename_pdf;
  ...
  msgBox.setInformativeText(
      tr("Report was downloaded form DB in .PDF format and saved at: \n%1\n\n"
         "When this dialog is closed, the report can be re-opened by clicking "
         "\'View Downloaded Report\' button on the left.")
          .arg(filePath_db));
...
int US_ReporterGMP::list_all_gmp_reports_db(
    QList<QStringList>& gmpReportsDBdata, US_DB2* db) {
  int nrecs = 0;
  ...
  while (db->next()) {
    ...
    QString ptime_created =
        US_Util::toUTCDatetimeText(time_created.toString(Qt::ISODate), true)
            .section(":", 0, 1) +
        " UTC";
...
void US_ReporterGMP::load_gmp_run(void) {
  ...
  pdiag_autoflow =
      new US_SelectItem(autoflowdata, hdrs, pdtitle, &prx, autoflow_btn, -4);
  ...
  for (int i = 0; i < Array_of_tripleNames.size(); ++i) {
    ...
    for (it = tmap.begin(); it != tmap.end(); ++it) {
      qDebug() << "ModelsDesc: Triple, QMap -- " << Array_of_tripleNames[i]
               << it.key() << it.value();

      qDebug() << "ModelsDescGuid: Triple, QMap -- " << Array_of_tripleNames[i]
               << it.key() << tmapguid[it.key()];
    }
  }
  ...
  if (GMP_report) {
    QMessageBox::information(this, tr("Report Profile Uploaded"),
                             tr("Report profile uploaded for GMP run:\n"
                                "%1\n\n"
                                "ATTENTION: Current profile configuration "
                                "corresponds to GMP report settings.\n\n"
                                "Any changes in the profile settings will "
                                "result in generation of the non-GMP report!")
                                 .arg(full_runname));
  } else {
    QMessageBox::information(
        this, tr("Report Profile Uploaded"),
        QString("Report profile uploaded for GMP run:<br><br>"
                "<b>%1</b><br><br>"
                "<font color='red'><b>ATTENTION:</b> There are missing models "
...
// Query autoflow (history) table for records
int US_ReporterGMP::list_all_autoflow_records(
  ...
  if (db->next()) user_id = db->value(0).toInt();
  ...
  while (db->next()) {
    ...
    if (devRecord == "Processed") continue;
    ...
    if (time_started.toString().isEmpty())
      ...
      if (status == "LIVE_UPDATE") autoflowentry << QString(tr("RUNNING"));
    ...
    if (status == "EDITING") status = "LIMS_IMPORT";
    ...
    if (US_Settings::us_inv_level() < 3) {
      ...
      qDebug() << "user_id, operatorID.toInt(), invID.toInt() -- " << user_id
               << operatorID.toInt() << invID.toInt();

      // if ( user_id && ( user_id == operatorID.toInt() || user_id ==
      // invID.toInt() ) )
      if (user_id &&
          user_id == invID.toInt()) {  // Do we allow operator as defined in
                                       // autoflow record to also see reports??
...
void US_ReporterGMP::read_protocol_and_reportMasks(void) {
  ...
  qDebug() << "Wavelengths ch_wvls[ channel_desc_alt ] "
           << ch_wvls[channel_desc_alt] << " for channel: " << channel_desc_alt;
  US_ReportGMP reportGMP = ch_reports[channel_desc_alt][wvl];

  if (reportGMP.reportItems.size() > 0)
    qDebug() << "AProfile's && ReportGMP's details: -- " << currAProf.aprofname
             << currAProf.protoname << currAProf.chndescs
             << currAProf.chndescs_alt << currAProf.lc_ratios
             << cAP2.parms[0].channel << cAPp.parms[0].channel
             << reportGMP.rmsd_limit << reportGMP.wavelength
  ...
  qDebug() << "Pseudo3d pars: " << reportGMP.pseudo3d_2dsait_s_ff0
           << reportGMP.pseudo3d_2dsait_s_d << reportGMP.pseudo3d_2dsait_mw_ff0
...
// combPlots parms
QMap<QString, QMap<QString, QString>> US_ReporterGMP::parse_comb_plots_json(
    QString combPlots) {
...
// read Intensity info
QMap<QString, QString> US_ReporterGMP::read_autoflowIntensity(QString ID,
                                                              US_DB2* db) {
  QString iJson;
      ...
      } else if (value.isArray())  // MWL
      {
        ...
        for (int i = 0; i < json_array.size(); ++i) {
          foreach (const QString& array_key, json_array[i].toObject().keys()) {
            ...
            qDebug() << "Intensity: MWL -- array_key, val: " << array_key
                     << json_array[i].toObject().value(array_key).toString();
          }
        }
      }
  ...
  for (int i = 0; i < topLevelItems.size(); ++i) {
    ...
    // Analysis Profile: add 2-levelchildren
      ...
      for (int ia = 0; ia < analysisItems.size(); ++ia) {
        ...
        if (expType == "ABDE" && (analysisItemName.contains("2DSA") ||
        ...
        if (analysisItemName.contains("General")) {
          ...
          for (int iag = 0; iag < analysisGenItems.size(); ++iag) {
            ...
            analysisGenItem[analysisGenItemName] =
                new QTreeWidgetItem(analysisItem[analysisItemName],
                                    analysisGenItemNameList, wiubase);

            if (analysisGenItems_vals[iag].toInt()) {
              analysisGenItem[analysisGenItemName]->setCheckState(0,
                                                                  Qt::Checked);
              ++checked_gen;
            } else
...
// get Item's children
void US_ReporterGMP::get_item_childs(QList<QTreeWidgetItem*>& children_list,
...
void US_ReporterGMP::changedItem(QTreeWidgetItem* item, int col) {
  ...
  for (int j = 0; j < parents_list.size(); ++j) {
    ...
    for (int jj = 0; jj < parent_item_children; ++jj) {
      if (int(parents_list[j]->child(jj)->checkState(0))) ++checked_children;
    }
  ...
  // reconnect
  connect(item->treeWidget(), SIGNAL(itemChanged(QTreeWidgetItem*, int)), this,
          SLOT(changedItem(QTreeWidgetItem*, int)));
}
...
void US_ReporterGMP::build_perChanTree_abde(void) {
  ...
  for (int ac = 0; ac < abde_channList.size(); ++ac) {
    ...
    qDebug() << "For ABDE-channel, " << chanItemName
             << ", report parms. are -- " << reportGMP.integration_results_mask
...
void US_ReporterGMP::build_perChanTree(void) {
  ...
  for (int i = 0; i < nchna; i++) {
    ...
    if (currAProf.analysis_run[i]) {
      ...
      QString run_report;
      if (currAProf.report_run[i]) triple_report = true;
    }

    if (triple_report) {
      // Channel Name: topItem in a perChanTree
      QString chanItemName =
          "Channel " + channel_desc_alt.section(":", 0, 1).replace(":", "-");
      chanItemNameList.clear();
      chanItemNameList << "" << indent + chanItemName;
      chanItem[chanItemName] =
          new QTreeWidgetItem(perChanTree, chanItemNameList, wiubase);
      ...
      for (int jj = 0; jj < chann_wvl_number; ++jj) {
        ...
        for (int kk = 0; kk < report_items_number; ++kk) {
          ...
          if (method.contains("PCSA")) method = "PCSA";
          int combPlot = curr_item.combined_plot;

          QString t_m = type + "," + method;
          if (combPlot) comboPlotsMapTypes[t_m] = 1;

          // Use ind_combined_plot here, to mark if ind. plots for triple's
          // reportItem is to be plot
          ...
          if (Triple_to_Models[tripleName].contains(method)) {
            if (ind_combPlot) {
              ...
              // QStringList( ranges ) > >
              indComboPlotsMapTripleTypeRangeBool[tripleName][t_m][range_l_h] =
                  true;
            } else
              indComboPlotsMapTripleTypeRangeBool[tripleName][t_m][range_l_h] =
                  false;
        ...
        for (int mm = 0; mm < tripleReportModelsList.size(); ++mm) {
          ...
          tripleModelItem[tripleModelName] = new QTreeWidgetItem(
              tripleItem[tripleItemName], tripleItemModelNameList, wiubase);
          ...
          // Here: add "Individual Distributions" to tripleReportMasksList IF
          // there is/are type-method(s) matching current model:
          QStringList type_methods_list =
              indComboPlotsMapTripleTypeRangeBool[tripleName].keys();
          for (int tm = 0; tm < type_methods_list.size(); tm++) {
            // select type-method for current model only! [e.g. {s,D}-2DSA-IT
            // for 2DSA-IT model]
            QString c_method = type_methods_list[tm].split(",")[1];
            if (c_method == tripleReportModelsList[mm]) {
              qDebug() << "In Model, list, ind. : " << type_methods_list
          ...
          for (int kk = 0; kk < tripleReportMasksList.size(); ++kk) {
            ...
            tripleMaskItem[tripleModelName] =
                new QTreeWidgetItem(tripleModelItem[tripleModelName],
            ...
            if (tripleMaskItemName.contains("Plots")) {
              ...
              for (int hh = 0; hh < tripleReportMasksPlotList.size(); ++hh) {
                ...
                tripleMaskItemPlotNameList
                    << "" << indent.repeated(5) + tripleMaskItemPlotName;
                tripleMaskPlotItem[tripleModelName] =
                    new QTreeWidgetItem(tripleMaskItem[tripleModelName],
                                        tripleMaskItemPlotNameList, wiubase);

                if (tripleReportMasksList_vals[kk]) {
                  tripleMaskPlotItem[tripleModelName]->setCheckState(
                      0, Qt::Checked);
                  ++checked_masks;
                } else
                  tripleMaskPlotItem[tripleModelName]->setCheckState(
            ...
            if (tripleMaskItemName.contains("Pseudo3d")) {
              ...
              if (tripleItemModelName.contains("2DSA-IT")) {
                ...
                qDebug() << "In pseudo3d tree: "
                            "tripleReportMasksPseudoList_vals (2DSA-IT) -- "
                         << tripleReportMasksPseudoList_vals;
                ...
                qDebug() << "In pseudo3d tree: "
                            "tripleReportMasksPseudoList_vals (2DSA-MC) -- "
                         << tripleReportMasksPseudoList_vals;
                ...
                qDebug() << "In pseudo3d tree: "
                            "tripleReportMasksPseudoList_vals (PCSA) -- "
                         << tripleReportMasksPseudoList_vals;
              }
            ...
            if (tripleMaskItemName.contains("Individual")) {
              ...
              // populate tripleReportMasksIndividualPlotList based on
              // type-method entries in the reportItem:
              ...
              for (int icp = 0;
                   icp < tripleReportMasksIndividualPlotList.size(); ++icp) {
                QString individualPlotName =
                    tripleReportMasksIndividualPlotList[icp];
                QString individualPlotName_name =
                    tripleReportMasksIndividualPlotList[icp].split("--")[0];
                QString individualPlotName_show =
                    tripleReportMasksIndividualPlotList[icp].split("--")[1];
                QStringList individualPlotNameList;
                individualPlotNameList
                    << "" << indent.repeated(5) + individualPlotName_name;
                ...
                if (tripleReportMasksList_vals[kk] &&
                  ...
                  tripleMaskIndComboPlotItem[tripleModelName]->setCheckState(
                      0, Qt::Checked);
                  ++checked_masks;
              ...
              if (top_ind_comboPlot)
                ...
                tripleMaskItem[tripleModelName]->setCheckState(0,
                                                               Qt::Unchecked);
            }
    ...
    for (int i = 0; i < channel_children; ++i) {
      ...
      if (!child_triple->childCount()) {
        qDebug() << "Channel's " << ch.value()->text(1)
                 << " triple: " << child_triple->text(1) << " has NO children";
...
void US_ReporterGMP::build_combPlotsTree(void) {
  ...
  for (tm = CombPlots_Type_to_Models.begin();
    ...
    for (int i = 0; i < unique_models.size(); ++i) {
      ...
      ItemCombPlots[modelName] = new QTreeWidgetItem(topItemCombPlots[tm.key()],
                                                     ItemNameList, wiubase);
...
void US_ReporterGMP::reset_report_panel(void) {
  ...
  if (auto_mode) pb_view_report_auto->setVisible(false);
...
void US_ReporterGMP::generate_report(void) {
  ...
  if (miscMask_edited.ShowMiscParts["User Input"]) assemble_user_inputs_html();

  // here, add Run Details based on timestamp info (OR timestapms of IP+RI?)
  if (miscMask_edited.ShowMiscParts["Run Details"]) assemble_run_details_html();
  ...
  } else {  // Will be modified for stand-alone GMP Reporter based on edited
            // tree JSON
    for (int i = 0; i < Array_of_triples.size(); ++i) {
      currentTripleName = Array_of_triples[i];
      ...
      for (int j = 0; j < models_to_do.size(); ++j) {
        ...
        qDebug()
            << "Triple / Model " << triplename_alt << " / " << models_to_do[j]
            << "has items ? "
            << perChanMask_edited
                   .has_tripleModel_items[triplename_alt][models_to_do[j]]
            << perChanMask_edited
                   .has_tripleModelPlot_items[triplename_alt][models_to_do[j]];

        if (perChanMask_edited
                .has_tripleModel_items[triplename_alt][models_to_do[j]] ||
            perChanMask_edited
                .has_tripleModelPlot_items[triplename_alt][models_to_do[j]] ||
            perChanMask_edited.has_tripleModelIndCombo_items[triplename_alt]
                                                            [models_to_do[j]]) {
          simulate_triple(currentTripleName, models_to_do[j]);
          ...
          // Individual Combo plots
          qDebug()
              << "INDCOMBO, perChanMask_edited. has_tripleModelIndCombo_items[ "
...
void US_ReporterGMP::process_abde_plots(void) {
  ...
  connect(sdiag_norm_profile, SIGNAL(pass_channels_info(QStringList&)), this,
          SLOT(get_abde_channels(QStringList&)));
  ...
  for (int i = 0; i < abde_channList.size(); ++i) {
    ...
    QString img01File =
        dirName + "/" + "ABDE_norm." + channel_name_abde + ".deconv" + svgext;
...
void US_ReporterGMP::get_abde_percents(
    QMap<QString, QMap<QString, double>>& abde_perc_p) {
  abde_ranges_percents = abde_perc_p;
}

void US_ReporterGMP::get_abde_data_per_channel(
...
// form a string of opers/revs out of jsonDoc
void US_ReporterGMP::get_assigned_oper_revs(QJsonDocument jsonDoc,
...
QStringList US_ReporterGMP::scan_dbase_models(QStringList runIDs) {
  ...
  while (db.next()) {
    ...
    DbgLv(1) << "ScMd: UNASSGN: mid,eid,mdesc,mrun" << mdlid << edtid << mdesc
             << mrunid;
  ...
  while (db.next()) {
    QString mdlid = db.value(0).toString();
    if (mmIDs.contains(mdlid)) continue;
  ...
  int m = nmodel - 1;
  if (m > 1) {
    DbgLv(1) << "ScMd: 0: id,gid,eid,desc" << mmIDs[0] << mmGUIDs[0] << meIDs[0]
             << mmDescs[0];
    DbgLv(1) << "ScMd: m: id,gid,eid,desc" << mmIDs[m] << mmGUIDs[m] << meIDs[m]
             << mmDescs[m];
  }
  ...
  for (int mm = nmodel - 1; mm >= 0; mm--) {
    ...
    if (kk > 0) {  // We have found the data description, so map it
...
void US_ReporterGMP::simulate_triple(const QString triplesname,
  ...
  progress_msg = new QProgressDialog(
      QString("Downloading data and models for triple %1...").arg(triplesname),
      QString(), 0, 5, this);
  ...
  if (check_file.exists() &&
      check_file.isFile()) {  // Get speed_steps from an existing timestate file
    ...
    for (int jj = 0; jj < simparams.speed_step.count(); jj++) {
      ...
      qDebug() << "LD:    (s) tf tl wf wl scns" << speed_steps[jj].time_first
               << speed_steps[jj].time_last << speed_steps[jj].w2t_first
               << speed_steps[jj].w2t_last << speed_steps[jj].scans;
    }
  }
  progress_msg->setValue(2);
  qApp->processEvents();
  ...
  bufvl = US_SolutionVals::values(dbP, edata, solID, svbar, bdens, bvisc, bcomp,
                                  bmanu, errmsg);
  progress_msg->setValue(3);
...
bool US_ReporterGMP::loadData(QMap<QString, QString>& triple_information) {
  ...
  // int eID=0;
  int eID =
      Triple_to_ModelsDesc[triple_information["triple_name"]]["eID"].toInt();
  ...
  qDebug() << "In loadData() Query: triple_information[ \"triple_name\" ]  -- "
           << triple_information["triple_name"];
  ...
  QDir edir(edirpath);
  if (!edir.exists()) edir.mkpath(edirpath);
...
bool US_ReporterGMP::loadModel(QMap<QString, QString>& triple_information) {
  ...
  //     3 ).toDateTime().toString( "yyyy/MM/dd HH:mm" ), true ); QDateTime date
  //     = db->value( 2 ).toDateTime();
...
bool US_ReporterGMP::loadNoises(QMap<QString, QString>& triple_information) {
  ...
  while (db->next()) {
    ...
    // QString  date                = US_Util::toUTCDatetimeText( db->value( 2
    // ).toDateTime().toString( "yyyy/MM/dd HH:mm" ), true );
    QDateTime date = db->value(2).toDateTime();

    QDateTime now = QDateTime::currentDateTime();

    qDebug() << "Noises: noiseID, noiseType, date -- " << noiseID << noiseType
             << date;
  ...
  // ALEXEY: treat the case when model (like -MC does not possess its own noises
  // -- use latest available noises for prior model like -IT  ) int
  // US_LoadableNoise::count_noise() in ../../gui/us_loadable_noise.cpp void
  // US_FeMatch::load_noise( ) in us_fematch.cpp
  if (!nID_ti && !nID_ri) loadNoises_whenAbsent();
  ...
  if (ntinois > 0 &&
      ntinois < npoints) {  // pad out ti noise values to radius count
    int jj = ntinois;
    while (jj++ < npoints) ti_noise.values << 0.0;
    ti_noise.count = ti_noise.values.size();
    npadded++;
  }

  if (nrinois > 0 &&
      nrinois < nscans) {  // pad out ri noise values to scan count
    int jj = nrinois;
    while (jj++ < nscans) ri_noise.values << 0.0;
    ri_noise.count = ri_noise.values.size();
    npadded++;
  }

  if (npadded > 0) {  // let user know that padding occurred
    QString pmsg;

    if (npadded == 1)
      pmsg =
          tr("The noise file was padded out with zeroes\n"
             "in order to match the data range.");
    else
      pmsg =
          tr("The noise files were padded out with zeroes\n"
...
void US_ReporterGMP::loadNoises_whenAbsent() {
  ...
  if (nenois > 0) {  // There is/are noise(s):  ask user if she wants to load
    US_Passwd pw;
    US_DB2* dbP = new US_DB2(pw.getPasswd());

    if (nenois > 1 &&
        noisdf == 0) {  // Noise exists and noise-dialog flag set to "Auto-load"
      ...
      if (kenois == 2) {  // Second noise from same model:  g/et it, too
        noiID = descn.section(":", 0, 0);
        typen = descn.section(":", 1, 1);
        if (typen == "ti") {
          ti_noise.load(true, noiID, dbP);
          qDebug() << "load_noise_whenAbsent(): Noise 2: noiID, loaded ti--"
                   << noiID;
          qDebug() << "load_noise_whenAbsent(): Noise 2: noiID, loaded "
                      "ti_noise.count --"
                   << ti_noise.count;
        } else {
          ri_noise.load(true, noiID, dbP);
          qDebug() << "load_noise_whenAbsent(): Noise 2: noiID, loaded ri--"
                   << noiID;
          qDebug() << "load_noise_whenAbsent(): Noise 2: noiID, loaded "
                      "ri_noise.count --"
                   << ri_noise.count;
...
int US_ReporterGMP::count_noise_auto(US_DataIO::EditedData* edata,
  ...
  if (model == 0) modelGUID = mieGUIDs[latemx];
  ...
  if (nmnois > 0) {        // If loaded model has noise, put noise in list
  ...
  for (int ii = 1; ii < nemods; ii++) {  // Search through models in edit
    ...
    if (kenois > 0) {  // if we have 1 or 2 noises, add to noise-in-edit list
      nenois += qMin(2, kenois);
      // adjust entry to have the right model-in-edit index
      lnoisGUID = tmpGUIDs.at(0).section(":", 0, 1) + ":" + modelIndx;
      nieGUIDs << lnoisGUID;
      if (kenois > 1) {  // add a second noise to the list
        lnoisGUID = tmpGUIDs.at(1).section(":", 0, 1) + ":" + modelIndx;
        nieGUIDs << lnoisGUID;
      }
  ...
  if (nenois > 0) {  // There is/are noise(s):  ask user if she wants to load
    ...
    if (nenois > 1) {
    ...
    else {  // Single noise file: check its value range versus experiment
      ...
      if ((noimax - noimin) >
          (datmax -
           datmin)) {  // Insert a warning if noise appears corrupt or unusual
        amsg =
            amsg + tr("\nBUT THE NOISE HAS AN UNUSUALLY LARGE DATA RANGE.\n\n");
      }

      msg = tr("There is a noise file") + amsg + tr("Do you want to load it?");
    }
    ...
    if (noidiag > 0) {
      ...
      if (msgBox.exec() !=
          QMessageBox::Yes) {  // user did not say "yes":  return zero count
        nenois = 0;            // number of edited-data-related noises
      }
    }

    if (kk < nemods) {  // Models with noise were found, so truncate models list
      for (int ii = 0; ii < (nemods - kk); ii++) mieGUIDs.removeLast();
    }
  }
...
int US_ReporterGMP::noises_in_model_auto(QString mGUID, QStringList& nGUIDs) {
  ...
  for (int ii = 0; ii < noiIDs.size();
       ii++) {  // Examine noises list; Save to this list if model GUID matches
    xnGUID = noiIDs.at(ii);
    ...
    if (mGUID == xmGUID) nGUIDs << xnGUID + ":" + xntype + ":0000";
  }
...
int US_ReporterGMP::models_in_edit_auto(QString eGUID, QStringList& mGUIDs) {
  ...
  for (int ii = 0; ii < modIDs.size();
       ii++) {  // Examine models list; Save to this list if edit GUID matches
    xmGUID = modIDs.at(ii);
...
int US_ReporterGMP::id_list_db_auto(QString daEditGUID) {
  ...
  if (db.lastErrno() != US_DB2::OK) return 0;
  ...
  while (db.next()) {  // Accumulate lists from noise records
  ...
  while (
      db.next()) {  // Accumulate from db desc entries matching noise model IDs
    QString modGUID = db.value(1).toString();
    QString modEdID = db.value(6).toString();

    if (noiMoIDs.contains(modGUID) &&
        modEdID == daEditID) {  // Only list models that have associated noise
                                // and match edit
      modIDs << modGUID;
  ...
  for (int ii = modIDs.size() - 1; ii >= 0;
       ii--) {  // Work from the back so any removed records do not affect
                // indexes
    ...
    if (mCarlo) {  // Treat monte carlo in a special way (as single composite
                   // model)
      if (reqIDs.contains(
              reqID)) {  // already have this request GUID, so remove this model
        modIDs.removeAt(ii);
        modDescs.removeAt(ii);
        modEdIDs.removeAt(ii);
      }

      else {  // This is the first time for this request, so save it in a list
        reqIDs << reqID;
      }
    }
  }
...
void US_ReporterGMP::simulateModel(QMap<QString, QString>& tripleInfo) {
  ...
  if (exp_steps) simparams.speed_step = speed_steps;
  ...
  if ((check_file.exists()) && (check_file.isFile())) {
    if (US_AstfemMath::timestate_onesec(
            tmst_fpath,
            *sdata)) {  // Load timestate that is already at 1-second-intervals
      simparams.simSpeedsFromTimeState(tmst_fpath);
    ...
    else {  // Replace timestate with a new one that is at 1-second-intervals
      // if ( drow == 0 )
      //   {
    ...
    if (!simparams.sim) {  // Compute acceleration rate for non-astfem_sim data
      ...
      qDebug() << "SimMdl:  accel-calc:  t1 t2 w2t t_acc speed rate" << t1 << t2
               << w2t << t_acc << simparams.speed_step[0].rotorspeed << rate;
  ...
  */
  ...
  if (nthread < 999) {
    if (model.components[0].sigma == 0.0 && model.components[0].delta == 0.0 &&
      ...
      for (int ii = 0; ii < model.components.size(); ii++) {
        qDebug() << "SimMdl:   comp" << ii << "s D v" << model.components[ii].s
                 << model.components[ii].D << model.components[ii].vbar20
                 << "  c" << model.components[ii].signal_concentration;
      }
      ...
      for (int ss = 0; ss < sdata->scanCount(); ss++) {
        ...
        for (int rr = 0; rr < sdata->pointCount(); rr++) {
          ...
          double rval =
              edata->value(ss, rr) - sdata->value(ss, rr) - rnoi - tnoi;
          trmsd += sq(rval);
          kpts++;
        }
      }
  ...
  else {  // Do multi-thread calculations
    ...
    for (int ii = 0; ii < ncomp; ii++) {
      if (ii < nthread) {  // First time through per thread:  get initial model
                           // and sim data
        tmodels << model;
    ...
    for (int ii = 0; ii < nthread; ii++) {
      ...
      connect(wthread, SIGNAL(started()), tworker, SLOT(calc_simulation()));
...
bool US_ReporterGMP::modelGuidExistsForStage(QString model, QString mguid) {
  ...
  for (int i = 0; i < Array_of_tripleNames.size(); ++i) {
    ...
    if (tmapguid[model] == mguid) {
      qDebug() << "For stage: " << model << ", there is modelGuid: " << mguid
...
bool US_ReporterGMP::modelGuidExistsForStage_ind(QString triple_n,
                                                 QString model, QString mguid) {
  bool isModelGuid = false;
  ...
  for (int i = 0; i < Array_of_tripleNames.size(); ++i) {
    ...
    qDebug()
        << "IN modelGuidExistsForStage_ind(): triple_n, c_triple_n, model  -- "
        << triple_n << c_triple_n << model;
    qDebug()
        << "IN modelGuidExistsForStage_ind(): mguid, Triple_to_ModelsDescGuid[ "
           "Array_of_tripleNames[ i ] ][ model ] -- "
...
void US_ReporterGMP::process_combined_plots_individual(QString triplesname_p,
                                                       QString stage_model) {
  QString triplesname_passed = triplesname_p;
  qDebug() << "[in process_combined_plots_individual()]: triplesname_passed -- "
           << triplesname_passed;
  ...
  for (int ii = 0; ii < modelDescModified.size(); ii++) {
    ...
    if (modelDescModified[ii].contains(triplesname_mod) &&
      ...
      // compose map of [{"s_ranges","k_ranges"}, etc] from cAP2 & cAPp (for
      // given channel & model!!!)
      ...
      // ALEXEY: here it plots s20 combPlot (xtype == 0) -- Need to do first
      // thing!!!
      plotted_ids_colors_map_s_type =
          sdiag_combplot->model_select_auto(modelDescModified[ii], c_params);

      // Check if to plot individual Combined distributons:
      // {s,D,f/f0,MW,Radius} {2DSA-IT,2DSA-MC,PCSA,raw} {p_key: {s[3.2:3.7],
      // D[11:15], etc.} } MaskStr.ShowTripleTypeModelRangeIndividualCombo[
      // t_name ][ s_name ][ p_key ] = feature_indCombo_value; // 1/0
      ...
      for (i_cpt = ind_compoplots_type_ranges.begin();
        ...
        QString imgComb02File =
            basename + "Ind_combined" + "." + triplesname + "." + stage_model;

        qDebug() << "in the ind_compoplots_type_ranges QMap: type, ranges --  "
                 << type << ranges;
        ...
        if (type == "s") {
          ...
          if (sim_ranges.contains("s_ranges")) {
            ...
            qDebug()
                << "s-type: sim_ranges.keys(), sim_ranges[\"s_ranges\"] -- "
          ...
          write_plot(
              imgComb02File,
              sdiag_combplot
                  ->rp_data_plot1());  //<-- rp_data_plot1() gives combined plot
...
QMap<QString, QStringList> US_ReporterGMP::find_sim_ranges(QString chann_desc,
                                                           QString model) {
  QMap<QString, QStringList> sim_ranges;
...
void US_ReporterGMP::process_combined_plots(QString filename_passed) {
  ...
  for (int m = 0; m < modelNames.size(); m++) {
    ...
    for (int ii = 0; ii < modelDescModified.size(); ii++) {
      ...
      if (modelDescModified[ii].contains(modelNames[m]) &&
        ...
        plotted_ids_colors_map_s_type = sdiag_combplot->model_select_auto(
            ...
            c_params);  // ALEXEY: here it plots s20 combPlot (xtype == 0)
      }
    }
                  ...
                  // defined at the beginnig
    {
      ...
      bool show_combo_s =
          (combPlotsMask_edited.ShowCombPlotParts["s"][modelNames[m]].toInt())
              ? true
              : false;
      bool show_combo_D =
          (combPlotsMask_edited.ShowCombPlotParts["D"][modelNames[m]].toInt())
              ? true
              : false;
      bool show_combo_ff0 =
          (combPlotsMask_edited.ShowCombPlotParts["f/f0"][modelNames[m]]
              ...
              : false;
      bool show_combo_MW =
          (combPlotsMask_edited.ShowCombPlotParts["MW"][modelNames[m]].toInt())
              ? true
              : false;

      qDebug() << "In process ComboPlots: Model: show_combo_s, show_combo_D, "
      ...
      //  QString t_m = "s," + modelNames[ m ];
      //  if ( comboPlotsMapTypes.contains( t_m ) && comboPlotsMapTypes[ t_m ]
      //  != 0  )
      if (show_combo_s) {
      ...
      for (int xt = 0; xt < xtype.size(); ++xt) {
        ...
        bool show_combo_plot_other_types =
            (combPlotsMask_edited.ShowCombPlotParts[c_type][modelNames[m]]
                 .toInt())
                ? true
                : false;
        if (show_combo_plot_other_types) {
          // plottedIDs_other_type = sdiag_combplot-> changedPlotX_auto( xtype[
          // xt ], c_parms );
          plotted_ids_colors_map_s_type =
...
void US_ReporterGMP::plot_pseudo3D(QString triple_name, QString stage_model) {
  ...
  if (t_name.contains("Interference")) t_name.replace("Interference", "660");
...
void US_ReporterGMP::calc_residuals() {
  ...
  for (int ii = 0; ii < scanCount; ii++) {
    ...
    for (int jj = 0; jj < dsize;
         jj++) {  // Calculate the residuals and the RMSD
...
void US_ReporterGMP::assemble_run_details_html(void) {
  ...
  if (dbP->lastErrno() != US_DB2::OK) {
    QMessageBox::warning(
        this, tr("Connection Problem, run details"),
        tr("Could not connect to database: \n") + dbP->lastError());
    return;
  }
  ...
  for (int i = 0; i < fileNameList.size(); ++i) {
    ...
    if (fileNameList[i].endsWith("-IP")) continue;
    ...
    QDir edir(dirname);
    if (!edir.exists()) edir.mkpath(dirname);
    ...
    QDir ddir(dirname);
    QStringList tmsfs =
        ddir.entryList(QStringList("*.time_state.*"), QDir::Files, QDir::Name);
    QString tmst_dname = dirname;
    if (tmst_dname.right(1) != "/") tmst_dname += "/";
    qDebug() << "Assembling Run Details: tmst_dname" << tmst_dname
             << "tmsfs count" << tmsfs.count();

    if (tmsfs.count() ==
        2) {  // Looks like we have a TMST and corresponding Defs XML
      tmst_fnamei = tmsfs[0];
      defs_fnamei = tmsfs[1];
      qDebug() << "Assembling Run Details: tmsfs" << tmst_fnamei << defs_fnamei;

      if (tmst_fnamei.contains(".tmst") &&
          defs_fnamei.contains(
              ".xml")) {  // Have both files, so save full path to TMST
        tmst_fnamei = tmst_dname + tmst_fnamei;
      }

      else if (tmsfs[0].contains(".xml") &&
               tmsfs[1].contains(
                   ".tmst")) {  // Have both files (in opposite order), so save
                                // full path to TMST
        tmst_fnamei = tmst_dname + tmsfs[1];
      }

      else {  // Do not have both files, so clear TMST file path
        tmst_fnamei.clear();
      }

      qDebug() << "Assembling Run Details: tmst,defs fnamei" << tmst_fnamei
               << defs_fnamei;
    ...
    html_assembled +=
        table_row(tr("Parameter: "), tr("Target Value:"), tr("Tolerance:"),
                  tr("Measured Value:"), tr("Std. Dev."), tr("PASSED ?"));

    // Iterate over Time Stamp params...
    for (int i = 0; i < dkeys.size(); ++i) {
      qDebug() << "Assembling Run Details: " << dkeys[i] << dmins[dkeys[i]]
               << " to " << dmaxs[dkeys[i]] << "; Avg: " << davgs[dkeys[i]]
      ...
      if (dkeys[i].contains("Temperature")) {
        ...
        html_assembled += table_row(
            ...
            "&#177;" + QString::number(val_tol), QString::number(val_measured),
      ...
      if (dkeys[i].contains("RawSpeed")) {
        ...
        html_assembled +=
            table_row("Speed (RPM)", QString::number(val_target),
                      "&#177;" + QString::number(val_tol),
...
// get expID
int US_ReporterGMP::get_expID_by_runID_invID(US_DB2* dbP,
                                             QString runID_filename) {
  QStringList query;
  int idExp = 0;
...
void US_ReporterGMP::assemble_user_inputs_html(void) {
  html_assembled += "<p class=\"pagebreak \">\n";
  html_assembled +=
      "<h2 align=left>User Interactions:\n GMP Run Initiation, Live Update, "
      "Data Import, Editing, and Analysis</h2>";
  ...
  html_assembled += tr("<table style=\"margin-left:10px\">"
                       "<caption align=left> <b><i>Time of GMP Run Initiation: "
                       "</i></b> </caption>"
                       "</table>"
  ...
  html_assembled += tr("<table style=\"margin-left:10px\">"
                       "<caption align=left> <b><i>Comment at the Time of GMP "
                       "Run Initiation: </i></b> </caption>"
                       "</table>"
  ...
  if (!stopOptimaJson.isEmpty() || !skipOptimaJson.isEmpty())
    html_assembled +=
        tr("<h3 align=left>Remote Stage Skipping, Stopping Machine (2. "
           "LIVE_UPDATE)</h3>");
  ...
  for (im = operation_types_live_update.begin();
    ...
    if (json_str.isEmpty()) continue;

    QString dtype_opt;

    if (im.key() == "STOP") dtype_opt = "Stopping Optima";

    if (im.key() == "SKIP") dtype_opt = "Skipping Stage";
    ...
    html_assembled += tr("<table style=\"margin-left:10px\">"
                         "<caption align=left> <b><i>Operation, Timestamp: "
                         "</i></b> </caption>"
                         "</table>"
  ...
  html_assembled +=
      tr("<h3 align=left>Reference Scan Determination, Triples Dropped, Data "
         "Saving (3. IMPORT)</h3>");
  ...
  for (im = data_types_import.begin(); im != data_types_import.end(); ++im) {
    ...
    html_assembled +=
        tr("<table style=\"margin-left:10px\">"
           "<caption align=left> <b><i>Comment at the Time of Data Saving: "
           "</i></b> </caption>"
           "</table>"
    ...
    if (!dtype_opt_dropped_triples.isEmpty()) {
      html_assembled +=
          tr("<table style=\"margin-left:10px\">"
             "<caption style=\"color:red;\" align=left> <b><i>List of Dropped "
             "Triples: </i></b> </caption>"
             "</table>"
    ...
    if (status_map.contains("Dropped")) {
      ...
      html_assembled +=
          tr("<table style=\"margin-left:10px\">"
             "<caption align=left> <b><i>Comments on [triples | channels | "
             "select channel] dropped: </i></b> </caption>"
             "</table>"
  ...
  if (expType == "ABDE") editing_time_abde = editRIts;

  html_assembled +=
      tr("<h3 align=left>Meniscus Position Determination, Edit Profiles Saving "
         "(4. EDITING)</h3>");
  ...
  for (im = data_types_edit.begin(); im != data_types_edit.end(); ++im) {
    ...
    if (im.key() == "RI") dtype_opt = "RI (UV/vis.)";
    if (im.key() == "IP") dtype_opt = "IP (Interf.)";
    ...
    for (mp = status_map["Meniscus"].begin();
      ...
      if (expType == "ABDE") {
        ...
        if (mwl_abde)
          ...
          QString chann_swl =
              chann_str.split(" / ")[0] + chann_str.split(" / ")[1];
    ...
    // Edit Profiles Saved:
    html_assembled += tr("<table style=\"margin-left:10px\">"
                         "<caption align=left> <b><i>Edit Profiles Saved on: "
                         "</i></b> </caption>"
                         "</table>"
...
void US_ReporterGMP::user_interactions_analysis_abde(QString analysisABDEJson,
  ...
  html_assembled += tr("<table style=\"margin-left:10px\">"
                       "<caption align=left> <b><i>Comment at the Time of ABDE "
                       "Profile Processing: </i></b> </caption>"
                       "</table>"
...
void US_ReporterGMP::user_interactions_analysis(QString analysisJson,
                                                QString analysisCancelJson) {
  html_assembled +=
      tr("<h3 align=left>Meniscus Position from FITMEN Stage, Job Cancellation "
         "(5. ANALYSIS)</h3>");
  ...
  if (!cAP2.job3auto)  // interactive FITMEN (manual)
  {
    html_assembled +=
        tr("<table style=\"margin-left:10px\">"
           "<caption align=left> <b><i>Meniscus Position Determination from "
           "FITMEN_MANUAL stage: </i></b> </caption>"
           "</table>"

           "<table style=\"margin-left:25px\">");

    QMap<QString, QString>::iterator mfa;
    for (mfa = analysis_status_map.begin(); mfa != analysis_status_map.end();
         ++mfa) {
  ...
  {
    html_assembled +=
        tr("Meniscus positions have been determined automatically as best fit "
  ...
  html_assembled +=
      tr("<table style=\"margin-left:10px\">"
         "<caption align=left> <b><i>Information on CANCELED analysis jobs: "
         "</i></b> </caption>"
         "</table>"
...
void US_ReporterGMP::read_reportLists_from_aprofile(
...
QStringList US_ReporterGMP::buildDroppedTriplesList(
...
// Read AProfile's reportIDs per channel:
bool US_ReporterGMP::readReportLists(
    ...
    if (xmli.isStartElement()) {
      if (ename == "channel_parms") {
        ...
        if (attr.hasAttribute(
                "load_volume"))  // ensure it reads upper-level <channel_parms>
        {
...
void US_ReporterGMP::read_autoflowStatus_record(
...
QMap<QString, QString> US_ReporterGMP::parse_autoflowStatus_analysis_json(
    QString statusJson) {
...
QMap<QString, QMap<QString, QString>> US_ReporterGMP::parse_autoflowStatus_json(
    QString statusJson, QString dtype) {
  QMap<QString, QMap<QString, QString>> status_map;
  ...
  foreach (const QString& key, json_obj.keys()) {
    ...
    if (key ==
        "Remote Operation")  // Live Update's remote operations [SKIP | STOP]
    {
      status_map[key]["type"] = value.toString();
    }
    if (key ==
        "Comment")  // GMP init || Live Update's remote operations [SKIP | STOP]
    {
...
QString US_ReporterGMP::distrib_info_abde(QString& abde_channame) {
  ...
  if (mwl_abde)
    abde_dist_info =
        "Radial Pos., DNA Data (Norm.), Protein Data (Norm.), Integral DNA "
        "(Norm), Integral Protein (Norm.)";
  else
    abde_dist_info =
  ...
  if (do_integration) {
    ...
    mstr +=
        "\n" + indent(2) +
        tr("<h3>Integration Results: Fraction of Total Concentration:</h3>\n");
    mstr += indent(2) + "<table>\n";
    ...
    for (int kk = 0; kk < report_items_number; ++kk) {
      ...
      QString tot_frac_passed =
          (qAbs(frac_tot_m - frac_tot_r) <= frac_tot_tol_r) ? "YES" : "NO";
      ...
      qDebug() << "In distrib_info(), fill simulated integration vals: for "
                  "chann/wvl/type/method/low/high, "
               << "Inter. val. Sim -- " << wvl_abde << curr_item.type
               << curr_item.method << curr_item.range_low
               << curr_item.range_high << int_val_m;

      mstr += table_row(type, range,
...
QMap<QString, QString> US_ReporterGMP::get_channels_analytes_mwl_abde(
  ...
  for (int ii = 0; ii < currProto.rpRange.nranges; ii++) {
    ...
    channel_s.replace(" / ", "");
    qDebug() << "[in get_channels_analytes_mwl_abde()], channel, channel_s, "
                "abde_channame;"
  ...
  for (int i = 0; i < PlotsFilenames.size(); ++i) {
    ...
    qreal qprinters_width =
        pageSize.width() * 0.75;  // 500 DEPENDS on QPrinter's constructor
                                  // settings {QPrinter::PrinterResolution, 500;
                                  // QPrinter::HighResolution, 9066}
    qreal qprinters_hight = pageSize.height() * 0.55;
    qDebug() << "qprinters_width, height [orig, scaled]: " << pageSize.width()
             << pageSize.height() << "; " << qprinters_width << qprinters_hight;
    ...
    // html_assembled   += "    <div><img style=\"height: 50px; width: 100px\"
    // src=\"" + filename
    // 	+ "\" alt=\"" + label + "\"/></div>\n\n";

    html_assembled += "    <div><img src=\"" + filename + "\" alt=\"" + label;
        ...
        plot_type == "CombPlots")  // For Combined plots, scale down .png
    {
      ...
      double scaled_i_width_cp = double(scaled_i_width) *
      ...
      html_assembled += " \"height=\"" + QString::number(scaled_i_height_cp) +
      ...
      double scaled_rd_height =
          double(scaled_i_height) * (scaled_rd_width / double(scaled_i_width));
    ...
    if (!plot_type.isEmpty() && plot_type == "CombPlots") {
      ...
      for (int i = 0; i < combparms.size(); ++i) {
        if (!combparms_colors.isEmpty() &&
          ...
          html_assembled +=
              "<br><span style='color:" + combparms_colors[i].name() +
              "'>&#9726;</span>";
        } else
          html_assembled +=
              "<br><span style='color:blue'>&#9726;</span>";  // SOME default
                                                              // color!!!
...
double US_ReporterGMP::interp_sval(double xv, double* sx, double* sy,
                                   int ssize) {
  for (int jj = 1; jj < ssize; jj++) {
    ...
    if (xv < sx[jj]) {  // given x lower than array x: interpolate between point
                        // and previous
      int ii = jj - 1;
...
QString US_ReporterGMP::text_model(US_Model model, int width) {
  ...
  if (width != 0) {  // long title:  add any suffixes and check need to center
    ...
    if (stitle.contains("-SL"))
      if (model.associations.size() > 1) title = title + " (RA)";
    ...
    if (model.monteCarlo) title = title + " (MC)";

    if (width > title.length()) {  // long title centered:  center it in
                                   // fixed-length string
      int lent = title.length();
      int lenl = (width - lent) / 2;
      int lenr = width - lent - lenl;
      title = QString(" ").repeated(lenl) + title + QString(" ").repeated(lenr);
    }
  }
...
=====================================================================================
    ...
    Replicate group #2: [channels 2A, 4A, 6B]
       Subgroup 1:  ----- Same: wvl, type/method, ranges, integraiton value
         triples:           2A.290, 6B.290 (overlapping wvls)  --> IT Maybe that
4A does not have wvl 290!!! type/method:       [s / 2DSA-IT] ranges: [3.2 - 3.7]
         integration value: 0.57

        Subgroup 2:  ----- Same: wvl, type/method, ranges, integraiton value
         triples:           2A.290, 6B.290 (overlapping wvls)  --> IT Maybe that
4A does not have wvl 290!!! type/method:       [D / 2DSA-IT] ranges: [3.4 - 3.9]
         integration value: 0.51
...
"2A.260","2B.260","4A.260","2A.275","2B.275","4A.275","2A.280","2B.280","4A.280","4B.280","6A.280","6B.280"
          ...
          IMPORTANT: in QStringList, group triples by same-wvl (subgroups), then
[0] in each group will be REFERENCE wvl! So, reference triples in the above
QStringList example will be: 2A.260, 2A.275 && 2A.280
...
reportItem[ kk ]. { integration_val_sim; ...}
  ...
  for (chw = channdesc_to_overlapping_wvls.begin();
    ...
    for (int j = 0; j < unique_wvls.size(); ++j) {
      ...
      html_str_replicate_av +=
          table_row(tr("Type:"), tr("Method:"), tr("Range:"),
                    tr("Av. Integration, Model (target):"), tr("St. Dev.:"),
                    tr("Av. Fraction %, Model (target):"), tr("St. Dev.:"),
                    tr("Tol.%:"), tr("PASSED?"));

      // Reference GMP Report, ReportItems for 1st channel in a replicate group
      // && over each wavelength:
      ...
      for (int kk = 0; kk < report_items_number; ++kk) {
        ...
        QMap<QString, double> replicate_g_results = get_replicate_group_results(
            ref_group_item, u_wvl, same_wvls_chann_map[u_wvl]);

        // print results into HTML report:
        /*
           Replicate Group #: [channles: 1A, 2A, 5A]
              Sub-Group #: [wvl: 280]
                type,  method,   range,    integration_AV (target),   St.Dev.
           Fraction of Total AV. (target)   PASSED? s      2DSA-MC   3.2-3.7
           integration_sim_av         st_dev      XXX YES/NO
        */

        double frac_tot_m_av = replicate_g_results["tot_percent_av"];
        QString tot_av_frac_passed =
            (frac_tot_m_av >= (frac_tot_r * (1 - frac_tot_tol_r / 100.0)) &&
             frac_tot_m_av <= (frac_tot_r * (1 + frac_tot_tol_r / 100.0)))
                ? "YES"
                : "NO";
...
// same-report-items:
QMap<QString, double> US_ReporterGMP::get_replicate_group_results(
       ...
       ++i)  // over channels for a given wvl in Replicate group:
  {
    ...
    for (int j = 0; j < nchna; j++)  // over all channels
    {
      ...
      if (channel_desc_alt.contains(channs_for_wvl[i])) {
        qDebug() << "In get_replicate_group_results(): channel_desc_alt, "
                    "channs_for_wvl[ i ], wvl -- "
  ...
  if (same_item_counter) {
    ...
    for (int i = 0; i < int_res_sim_vector.size(); ++i) {
      ...
      st_dev_tot_percent_1 += (tot_percent_sim_av - tot_percent_sim_vector[i]) *
                              (tot_percent_sim_av - tot_percent_sim_vector[i]);
    }

    st_dev_int_final = sqrt(st_dev_int_1) / (sqrt(int_res_sim_vector.size()));
    st_dev_tot_percent_final =
        sqrt(st_dev_tot_percent_1) / (sqrt(tot_percent_sim_vector.size()));
  }
...
QString US_ReporterGMP::distrib_info(QMap<QString, QString>& tripleInfo) {
  ...
  if (is_dmga_mc) {
    if (msim == "model") {  // Use DMGA-MC single-iteration model
      msim = "<b>&nbsp;&nbsp;( single iteration )</b>";
    } else {  // Use mean|median|mode model
      msim = "<b>&nbsp;&nbsp;( " + msim + " )</b>";
    }
  } else {  // Normal non-DMGA-MC model
    msim = "";
  ...
  if (mdla.isEmpty()) mdla = model_used.description.section(".", 0, -2);
  ...
  if (!cnstff)
    mstr +=
        table_row(tr("Weight Average f/f0:"), QString::number((sum_k / sum_c)));
  if (!cnstvb)
    mstr += table_row(tr("Weight Average vbar20:"),
                      QString::number((sum_v / sum_c)));
  mstr +=
      table_row(tr("Total Concentration:"), QString().sprintf("%6.4e", sum_c));
  ...
  if (cnstvb) {  // Normal constant-vbar distribution
    model_dist_info =
        "Molec. Wt., S Apparent, S 20 (W), D Apparent, D 20 (W), f/f0, "
        "Concentration";
  ...
  else if (cnstff) {  // Constant-f/f0, varying vbar
    model_dist_info =
        "Molec. Wt., S Apparent, S 20 (W), D Apparent, D 20 (W), Vbar20, "
        "Concentration";
  ...
  else {  // Neither vbar nor f/f0 are constant

    model_dist_info =
        "Molec. Wt., S Apparent, S 20 (W), D 20 (W), f/f0, Vbar20, "
        "Concentration";
  ...
  */
  ...
  if (model_used.associations.size() > 0) {
    ...
    for (int ii = 0; ii < model_used.associations.size(); ii++) {
      ...
      mstr += table_row(reac1, reac2, prod, QString().sprintf("%10.4e", k_d),
  ...
  if (model_name.contains("PCSA")) model_name = "PCSA";
  ...
  hh = "h";
  QString wks =
      QString().sprintf("%d %s %02d m", hours, hh.toLatin1().data(), mins);
  ...
  for (int i = 0; i < nchna; i++) {
    ...
    if (t_name.contains(channelNameProt)) {
      qDebug() << "So, what are channel_desc_alt, wvl ? " << channel_desc_alt
               << wvl;
  ...
  hh_r = "h";
  QString exp_dur_r_hh_mm = QString().sprintf("%d %s %02d m", hours_r,
                                              hh_r.toLatin1().data(), mins_r);
  ...
  for (sh = Model_parms_to_compare.begin(); sh != Model_parms_to_compare.end();
       ++sh) {
    if (sh.key().contains("Concentration") && sh.value().toInt())
      show_tot_conc = true;

    if (sh.key().contains("RMSD") && sh.value().toInt()) show_rmsd = true;
  ...
  if (show_comparison_section) {
    ...
    if (show_tot_conc) {
      mstr += table_row(tr("Total Concentration"),
                        ...
                        QString().sprintf("%6.4e", sum_c), tot_conc_passed);
    }

    if (show_rmsd) {
      mstr += table_row(tr("RMSD limit"), QString::number(rmsd_r), QString(""),
                        rmsd_global, rmsd_passed);
    }

    if (show_exp_dur) {
      mstr +=
          table_row(tr("Experiment Duration"), exp_dur_r_hh_mm,
                    QString::number(exp_dur_tol_r) + "%", wks, exp_dur_passed);
    }
    ...
    if (show_min_int && RIdata) {
      mstr +=
          table_row(tr("Minimum Intensity"), QString::number(av_int_r),
                    QString(""), QString::number(av_int_exp), av_int_passed);
    }

    if (show_loading_vol) {
      mstr += table_row(tr("Loading Volume"), QString::number(loading_volume_r),
  ...
  for (int kk = 0; kk < report_items_number; ++kk) {
    ...
    if (mdla.contains(method)) method_type_combo_exists = true;
  }
  ...
  if (show_integration && method_type_combo_exists) {
    ...
    for (int kk = 0; kk < report_items_number; ++kk) {
      ...
      if (mdla.contains(method)) {
        ...
        mstr += table_row(
            type, range, QString().sprintf("%10.4e", wav_variable_within_range),
...
double US_ReporterGMP::get_loading_volume(int cellID) {
  ...
  double stretch =
      simparams.rotorcoeffs[0] * speed + simparams.rotorcoeffs[1] * sq(speed);
...
QString US_ReporterGMP::table_row(const QString& s1, const QString& s2,
  ...
  return (indent(6) + "<tr><td>" + s1 + "</td><td>" + s2 + "</td><td>" + s3 +
          "</td><td>" + s4 + "</td><td>" + s5 + "</td><td>" + s6 + "</td><td>" +
          s7 + "</td></tr>\n");
}
...
void US_ReporterGMP::plotres(QMap<QString, QString>& tripleInfo) {
  ...
  bool show_vel =
      (perChanMask_edited
          ...
          ? true
          : false;
  bool show_mw = (perChanMask_edited
                      .ShowTripleModelPlotParts[t_name][s_name]
                                               ["Molecular Weight Distribution"]
                      .toInt())
                     ? true
  ...
  if (show_vel) {
    write_plot(img01File,
               resplotd->rp_data_plot1());  //<-- rp_data_plot1() gives overlay
                                            //(Exp/Simulations) plot
  ...
  if (show_res) {
    write_plot(
        img02File,
        resplotd->rp_data_plot2());  //<-- rp_data_plot2() gives residuals plot
  ...
  plotLayout2 = new US_Plot(data_plot2, tr("Velocity Data"), tr("Radius (cm)"),
...
void US_ReporterGMP::distrib_plot_stick(int type) {
  ...
  data_grid->setMajorPen(QPen(US_GuiSettings::plotMinGrid(), 0, Qt::DotLine));
...
void US_ReporterGMP::write_plot(const QString& filename, const QwtPlot* plot) {
  if (filename.contains(".svg")) {  // Save an SVG file and a PNG copy
  ...
  //    }
  ...
  else if (filename.endsWith("3dplot.png")) {  // Special case of 3dplot PNG
                                               // if ( eplotcd == 0 )
                                               // {  // if no 3d plot control
                                               // up,  create it now
    ...
    QPixmap pixmap = dataw->renderPixmap(dataw->width(), dataw->height(), true);
  ...
  else if (filename.endsWith(".png")) {  // General case of PNG
...
void US_ReporterGMP::thread_progress(int thr, int icomp) {
  ...
  int kcomp = 0;
  kcomps[thr] = icomp;
  for (int ii = 0; ii < nthread; ii++) kcomp += kcomps[ii];
...
void US_ReporterGMP::thread_complete(int thr) {
  ...
  if (thrdone >=
      nthread) {  // All threads are done, so sum thread simulation data
    for (int ii = 0; ii < sdata->scanData.size(); ii++) {
      for (int jj = 0; jj < sdata->xvalues.size(); jj++) {
        // double conc = 0.0;
        double conc = sdata->value(ii, jj);

        for (int kk = 0; kk < nthread; kk++) conc += tsimdats[kk].value(ii, jj);
...
void US_ReporterGMP::adjustModel() {
  ...
  if (cnstvb && mc_vbar != sd.vbar20 &&
      mc_vbar != 0.0) {  // Use vbar from the model component, instead of from
                         // the solution
    sd.vbar20 = mc_vbar;
  ...
  for (int jj = 0; jj < model.components.size(); jj++) {
    ...
    if (!cnstvb) {  // Set s,D corrections based on component vbar
      sd.vbar20 = sc->vbar20;
      sd.vbar = US_Math2::adjust_vbar20(sd.vbar20, avgTemp);
      US_Math2::data_correction(avgTemp, sd);
      scorrec = sd.s20w_correction;
    ...
    sc->s /= scorrec;
    sc->D /= dcorrec;
    qDebug() << "Fem:Adj:  s20w D20w" << s20w << d20w << "s D" << sc->s << sc->D
             << "  jj" << jj << "vb20 vb" << sc->vbar20 << sd.vbar;
...
void US_ReporterGMP::assemble_pdf(QProgressDialog* progress_msg) {
  ...
  if (!str_failed_stage_missing_models.isEmpty()) {
    html_failed = tr("<h3 style=\"color:red;\" align=left> ATTENTION: Analyses "
                     "for Some Triples Failed, or Models are Missing!</h3>"
                     "%1"
                     "<hr>")
  ...
  QString instr_name =
      (dataSource == "INSTRUMENT") ? currProto.rpRotor.instrname : "dataDisk";
  ...
  for (int i = 0; i < nchan_optics; ++i) {
    ...
    if (!scan1.isEmpty()) has_uv = true;
    if (!scan2.isEmpty()) has_int = true;
    if (!scan3.isEmpty()) has_fluo = true;
  }
  ...
  if (has_uv) {
    html_speed +=
        tr("<tr><td><br><b><i>UV-visible optics (total):</i></b>   </td>  "
  ...
  if (has_int) {
    html_speed +=
        tr("<tr><td><br><b><i>Interference optics (per cell):</i></b></td>   "
           "<tr><td>Delay to First Scan:  </td>                    "
           "<td>%8</td></tr>"
           "<tr><td>Scan Interval:        </td>                    "
           "<td>%9</td></tr>")
  ...
  html_cells +=
      tr("# of Used Cells:  %1<br>").arg(QString::number(ncells_used))  // 1
      ;
  ...
  for (int i = 0; i < nchan_ranges; ++i) {
    ...
    for (int jj = 0; jj < w_count; jj++) {
      ...
      if (jj != wvl_list.size() - 1) all_wvl += tr(", ");
      if (counter % 8 == 0) all_wvl += tr("<br>");
    }
    ...
    if (w_count > 1) {
      html_ranges +=
          tr("<tr><td> Selected Wavelength range: </td>  <td> %1 </td> </tr>")
              .arg(w_range)  // 1
          ;
    }

    html_ranges +=
        tr("<tr><td> Radius range:              </td>  <td> %1 </td> </tr>"
  ...
  if (has_uvvis) html_scan_count += html_scan_count_uv;
  if (has_interference) html_scan_count += html_scan_count_int;
  ...
  html_analysis_profile =
      tr("<h3 align=left> Analysis Profile: General Settings and Reports  </h3>"
         "&nbsp;&nbsp;<br>");
  ...
  for (int i = 0; i < nchna; i++) {
      ...
      if (expType == "VELOCITY") {
        ...
        html_analysis_gen +=
            tr("<table style=\"margin-left:50px\">"
               "<tr><td> Wavelength for Edit, 2DSA-FM & Fitmen Stages (nm):   "
               "</td>  <td> %1 </td> </tr>"
               "</table>")
                .arg(rep_wvl)  // 1
            ;
    ...
    if (triple_report) {
      ...
      // for ABDE, identify only 1st wvl in the cahnnel, and proceed with it
      // only!
      ...
      for (int jj = 0; jj < chann_wvl_number; ++jj) {
        ...
        // abde
        if (expType == "ABDE" && wvl != wvl_abde) continue;
        ...
        if (genMask_edited
          ...
          html_analysis_profile +=
              tr("<table style=\"margin-left:50px\">"
                 "<tr><td> <b><i> Report Parameters for Triple: </i> "
                 "&nbsp;&nbsp;&nbsp; %1 </b></td> </tr>"
                 "</table>")
                  .arg(triple_name)  // 1
              ;
          ...
          if (expType == "VELOCITY") {
            html_analysis_profile +=
                tr("<table style=\"margin-left:70px\">"
                   "<tr><td> Total Concentration:           </td>  <td> %1 "
                   "</td> </tr>"
                   "<tr><td> Total Concentration Tolerance: </td>  <td> %2 "
                   "</td> </tr>"
                   "<tr><td> RMSD (upper limit):            </td>  <td> %3 "
                   "</td> </tr>"
                   "<tr><td> Average Intensity:             </td>  <td> %4 "
                   "</td> </tr>"
                   "<tr><td> Experiment Duration:           </td>  <td> %5 "
                   "</td> </tr>"
                   "<tr><td> Experiment Duration Tolerance: </td>  <td> %6 "
                   "</td> </tr>"
                   "</table>")
            ...
            html_analysis_profile +=
                tr("<table style=\"margin-left:70px\">"
                   "<tr><td> Experiment Duration:           </td>  <td> %1 "
                   "</td> </tr>"
                   "<tr><td> Experiment Duration Tolerance: </td>  <td> %2 "
                   "</td> </tr>"
                   "</table>")
                    .arg(exp_dur_str)  // 1
                    .arg(QString::number(
        ...
        for (int kk = 0; kk < report_items_number; ++kk) {
          ...
          if (genMask_edited
                  .ShowAnalysisGenParts["Report Item Parameters (per-triple)"]
                  .toInt()) {
            html_analysis_profile +=
                tr("<table style=\"margin-left:90px\">"
                   "<tr><td> <b><i> Report Item #%1: </i> &nbsp;&nbsp; Type | "
                   "Method Parameters </b></td> </tr>"
                   "</table>")
            ...
            if (expType == "VELOCITY") {
              html_analysis_profile +=
                  tr("<table style=\"margin-left:110px\">"
                     "<tr><td> Type:                  </td>  <td> %1 </td> "
                     "</tr>"
                     "<tr><td> Method:                </td>  <td> %2 </td> "
                     "</tr>"
                     "<tr><td> Range Low:             </td>  <td> %3 </td> "
                     "</tr>"
                     "<tr><td> Range High:            </td>  <td> %4 </td> "
                     "</tr>"
                     "<tr><td> Integration Value:     </td>  <td> %5 </td> "
                     "</tr>"
                     "<tr><td> Tolerance (%):         </td>  <td> %6 </td> "
                     "</tr>"
                     "<tr><td> Fraction of Total (%): </td>  <td> %7 </td> "
                     "</tr>"
                     "</table>")
              ...
              html_analysis_profile +=
                  tr("<table style=\"margin-left:110px\">"
                     ...
                     "<tr><td> Range High:            </td>  <td> %4 </td> "
                     "</tr>"
                     "<tr><td> Tolerance (%):         </td>  <td> %5 </td> "
                     "</tr>"
                     "<tr><td> Fraction of Total (%): </td>  <td> %6 </td> "
                     "</tr>"
                     "</table>")
                      .arg(curr_item.type)                            // 1
                      .arg(curr_item.method)                          // 2
  ...
  if (cAPp.job_run) {
    ...
    for (int i = 0; i < nchna_pcsa; i++) {
      ...
      QString y_data = cAPp.parms[i].y_type + ", " +
                       ...
                       QString::number(cAPp.parms[i].y_max);
      QString z_data =
          cAPp.parms[i].z_type + ", " + QString::number(cAPp.parms[i].z_value);
...
void US_ReporterGMP::write_pdf_report(void) {
  ...
  QString fileName =
      ProtocolName_auto + ".pdf";  // Use unique protocol name for .pdf filename
...
void US_ReporterGMP::printDocument(
    QPrinter& printer, QTextDocument* doc)  //, QWidget* parentWidget)
{
  ...
  // Qt::ApplicationModal );
  progress_msg =
      new QProgressDialog("Preparing .PDF...", QString(), 0, pageCount, this);
  progress_msg->setWindowFlags(Qt::Tool | Qt::WindowTitleHint |
                               Qt::CustomizeWindowHint);
  progress_msg->setModal(true);
...
void US_ReporterGMP::paintPage(QPrinter& printer, int pageNumber, int pageCount,
  ...
  qDebug() << "Painter's settings: font, metrics -- "
           << painter->fontInfo().family() << painter->fontInfo().pointSizeF()
  ...
  footerRect.setHeight(1 *
                       footerHeight);  // will a parameter on #of lines (footer
                                       // height, depending on # reviewers...)
  // test below
...
void US_ReporterGMP::write_gmp_report_DB(QString filename,
  ...
  qry.clear();
  qry << "new_autoflow_gmp_report_record" << AutoflowID_auto << FullRunName_auto
      << ProtocolName_auto << filename_pdf;
...
void US_ReporterGMP::remove_files_by_mask(QString dirPath,
                                          QStringList file_exts) {
  QDir dir(dirPath);
...
void US_ReporterGMP::parse_edited_misc_mask_json(
    const QString maskJson, MiscReportMaskStructure& MaskStr) {
...
void US_ReporterGMP::parse_edited_gen_mask_json(
    const QString maskJson, GenReportMaskStructure& MaskStr) {
  QJsonDocument jsonDoc = QJsonDocument::fromJson(maskJson.toUtf8());
  QJsonObject json = jsonDoc.object();
        ...
        foreach (const QString& array_key, json_array[i].toObject().keys()) {
          if (key.contains("Solutions")) {
            ...
            MaskStr.ShowSolutionParts[array_key] =
                json_array[i].toObject().value(array_key).toString();
            if (MaskStr.ShowSolutionParts[array_key].toInt()) ++has_sol_items;
          }
          if (key.contains("Analysis")) {
            QJsonObject newObj =
                json_array[i].toObject().value(array_key).toObject();
            ...
            foreach (const QString& n_key, newObj.keys()) {
              ...
              if (analysis_cathegory_item_value.toInt()) ++has_analysis_items;
              ...
              if (array_key.contains("General")) {
                ...
                qDebug() << "Parse_editedJsonGen: Analysis:Gen: n_key, value: "
                         << n_key << MaskStr.ShowAnalysisGenParts[n_key];
              }
              ...
              if (array_key.contains("2DSA")) {
                ...
                qDebug() << "Parse_editedJsonGen: Analysis:2DSA: n_key, value: "
                         << n_key << MaskStr.ShowAnalysis2DSAParts[n_key];
              }
              ...
              if (array_key.contains("PCSA")) {
                ...
                qDebug() << "Parse_editedJsonGen: Analysis:PCSA: n_key, value: "
                         << n_key << MaskStr.ShowAnalysisPCSAParts[n_key];
              }
            }
          }
        }
...
void US_ReporterGMP::parse_edited_perChan_mask_json_abde(
  ...
  foreach (const QString& key, json.keys())  // over channels
  {
    ...
    foreach (const QString& n_key, tripleObj.keys())  // over single channel!
    {
      ...
      {
        if (feature_value.toString().toInt()) ++has_channel_items;

        MaskStr.ShowChannelItemParts[key][n_key] = feature_value.toString();
      }
    }

    if (!has_channel_items) MaskStr.ShowChannelParts[key] = false;
  }
}

// Pasre reportMask JSON: perChan
void US_ReporterGMP::parse_edited_perChan_mask_json(
        ...
        foreach (const QString& n_key, tripleObj.keys())  // over models
        {
              ...
              modelObj.keys())  // over models' features (tot_conc, RMSD...)
          {
                                           ...
                                           // characteristics ( tot_conc, RMSD)
            {
              ...
              MaskStr.ShowTripleModelParts[triple_name][model_name][j_key] =
                  feature_value.toString();
              if (MaskStr.ShowTripleModelParts[triple_name][model_name][j_key]
                      .toInt())
                ++MaskStr.has_tripleModel_items[triple_name][model_name];

              qDebug() << "Parse_editedJsonTriples: triple_name: "
                       << triple_name << ": "
                       << "model_name, j_key, feature_value: " << model_name
                       << j_key << feature_value.toString();
            } else {
              // begin "Plots"
              if (j_key.contains("Plots")) {
                QJsonObject plotObj = modelObj.value(j_key).toObject();

                foreach (const QString& p_key,
                                          ...
                                          // triple/per channel
                {
                  ...
                  QString model_name = n_key.split(" ")[0];
                  MaskStr.ShowTripleModelPlotParts[triple_name][model_name]
                                                  [p_key] = feature_plot_value;
                  if (MaskStr
                          .ShowTripleModelPlotParts[triple_name][model_name]
                                                   [p_key]
                          .toInt())
                    ++MaskStr
                          .has_tripleModelPlot_items[triple_name][model_name];
              ...
              // begin "Pseudo3D"
                ...
                foreach (const QString& p_key,
                         pseudo3dObj.keys())  // over pseudo3d, per model/per
                                              // triple/per channel
                {
                  ...
                  QString model_name = n_key.split(" ")[0];
                  MaskStr.ShowTripleModelPseudo3dParts[triple_name][model_name]
                                                      [p_key] =
                      feature_pseudo3d_value;
                  if (MaskStr
                          .ShowTripleModelPseudo3dParts[triple_name][model_name]
                                                       [p_key]
                          .toInt())
                    ++MaskStr.has_tripleModelPseudo3d_items[triple_name]
                                                           [model_name];
              ...
              // begin "Individual"
                ...
                foreach (const QString& p_key,
                         indComboObj.keys())  // over type[ranges], per
                                              // model/per triple/per channel
                {
                  ...
                  MaskStr.ShowTripleTypeModelRangeIndividualCombo[triple_name]
                                                                 [model_name]
                                                                 [p_key] =
                      feature_indCombo_value;
                  if (MaskStr
                          .ShowTripleTypeModelRangeIndividualCombo[triple_name]
                                                                  [model_name]
                                                                  [p_key]
                          .toInt())
                    ++MaskStr.has_tripleModelIndCombo_items[triple_name]
                                                           [model_name];

                  qDebug()
                      << "Parse_editedJsonTriples: triple_name: " << triple_name
                      << ": "
...
void US_ReporterGMP::parse_edited_combPlots_mask_json(
  ...
  foreach (const QString& key, json.keys())  // over types
  {
    ...
    for (int i = 0; i < json_array.size(); ++i) {
               ...
               json_array[i].toObject().keys())  // over models (aka methods)
      {
        ...
        if (array_key.contains("PCSA")) array_key_mod = "PCSA";

        MaskStr.ShowCombPlotParts[key][array_key_mod] = show_plot.toString();

        qDebug() << "CombPlots: type, model, yes/no -- " << key << array_key_mod
                 << show_plot.toString();
...
void US_ReporterGMP::get_children_to_json(QString& mask_edited,
                                          QTreeWidgetItem* item) {
  for (int i = 0; i < item->childCount(); ++i) {
    ...
    if (!children_lev2) {
      ...
      if (i != item->childCount() - 1) mask_edited += ",";
    } else {
      mask_edited += "{";
...
QString US_ReporterGMP::tree_to_json(
    QMap<QString, QTreeWidgetItem*> topLevItems) {
  QString mask_edited;
  mask_edited += "{";
  ...
  if (!mask_edited
           .mid(mask_edited.lastIndexOf(to_replace) + to_replace.size(),
                mask_edited.length())
           .contains("\""))
    mask_edited.replace(mask_edited.lastIndexOf(to_replace), to_replace.size(),
                        new_substr);
...
void US_ReporterGMP::assemble_parts(QString& html_str) {
  ...
  for (top = genMask_edited.ShowReportParts.begin();
    ...
    if (top.key().contains("General") && top.value()) html_str += html_general;
    if (top.key().contains("Rotor") && top.value()) html_str += html_lab_rotor;
    if (top.key().contains("Operator") && top.value())
      html_str += html_operator;
    if (top.key().contains("Speed") && top.value()) html_str += html_speed;
    if (top.key().contains("Cell") && top.value()) html_str += html_cells;
    ...
    if (top.key().contains("Ranges") && top.value()) html_str += html_ranges;
    ...
    if (top.key().contains("Analysis Profile") && top.value()) {
      if (genMask_edited.has_anagen_items) html_str += html_analysis_profile;
      if (expType == "VELOCITY") {
...
void US_ReporterGMP::add_solution_details(const QString sol_id,
  ...
  if (genMask_edited.ShowSolutionParts["Analyte Information"].toInt()) {
    html_solutions +=
        tr("<table style=\"margin-left:20px\">"
  ...
  for (int i = 0; i < num_analytes; ++i) {
    ...
    if (seqlen == 0)
      ...
      for (int ii = 0; ii < 26; ii++) {
        ...
        if (lcount > 0) {
          seqsmry +=
              QString().sprintf("%d", lcount) + " " + letter.toUpper() + ", ";
          // seqsmry          += "\n  ";
        }
      }
    ...
    if (analyte.type == 0) {
      analyte_detailed_info +=
          tr("<tr> <td> AAs absorbing at 280 nm: </td> <td>%1</td> </tr>"
  ...
  buffer_detailed_info +=
      tr("<tr><td> Density (20&#8451;, g/cm<sup>3</sup>):  </td>   <td>%1</td> "
         " </tr>"
  ...
  for (int i = 0; i < buffer.component.size(); ++i) {
    ...
    buffer_components_info +=
        tr("<tr><td> Component Name:</td> &nbsp;&nbsp;  <td>%1</td>  </tr>")
            .arg(component_desc);
  }
