 ...
 * @brief The US_ReporterGMP class provides functionality for generating GMP
 * reports.
 */
...
public:
   ...
   */
  void write_gmp_report_DB_test (QString runID, QString protocolID, int invID,
                                 QString protocolName, QString esign);
  ...
  bool first_time_misc_tree_build; //!< Flag for first time miscellaneous tree
                                   //!< build
  bool first_time_perChan_tree_build; //!< Flag for first time per-channel tree
                                      //!< build
  ...
  QMap<QString, QMap<QString, US_ReportGMP> >
      ch_reports_internal;               //!< Internal channel reports
  QMap<QString, QList<double> > ch_wvls; //!< Channel wavelengths

  QList<int> replicates; //!< Replicate list
  QMap<int, QStringList>
      replicates_to_channdesc; //!< Replicate to channel descriptions map
  QMap<QString, QStringList>
      channdesc_to_overlapping_wvls; //!< Channel descriptions to overlapping
                                     //!< wavelengths map
  ...
  struct GenReportMaskStructure
  {
    ...
    QMap<QString, QString>
        ShowSolutionParts; //!< Map of solution parts to show
    QMap<QString, QString>
        ShowAnalysisGenParts; //!< Map of general analysis parts to show
    QMap<QString, QString>
        ShowAnalysis2DSAParts; //!< Map of 2DSA analysis parts to show
    QMap<QString, QString>
        ShowAnalysisPCSAParts; //!< Map of PCSA analysis parts to show
  ...
  GenReportMaskStructure
      genMask_edited; //!< Edited general report mask structure
  ...
  struct PerChanReportMaskStructure
  {
    ...
    // triple_name -> model -> feature -> value [Exp. Duration, Intensity,
    // RMSD, Integration Results, Tot. Conc.]
    QMap<QString, QMap<QString, QMap<QString, QString> > >
        ShowTripleModelParts; //!< Map of triple model parts to show
    QMap<QString, QMap<QString, int> >
        has_tripleModel_items; //!< Map of triple model items

    // triple_name -> model -> feature -> value [All types of plots]
    QMap<QString, QMap<QString, QMap<QString, QString> > >
        ShowTripleModelPlotParts; //!< Map of triple model plot parts to show
    QMap<QString, QMap<QString, int> >
        has_tripleModelPlot_items; //!< Map of triple model plot items

    // triple_name -> model -> feature -> value [All types of pseudo distr.]
    QMap<QString, QMap<QString, QMap<QString, QString> > >
        ShowTripleModelPseudo3dParts; //!< Map of triple model pseudo3d parts
                                      //!< to show
    QMap<QString, QMap<QString, int> >
        has_tripleModelPseudo3d_items; //!< Map of triple model pseudo3d items

    // Individual Combined Plots
    QMap<QString, QMap<QString, QMap<QString, QString> > >
        ShowTripleTypeModelRangeIndividualCombo; //!< Map of individual
                                                 //!< combined plots
    QMap<QString, QMap<QString, int> >
        has_tripleModelIndCombo_items; //!< Map of triple model individual
                                       //!< combined items
  };

  PerChanReportMaskStructureABDE perChanMask_edited_abde;
  PerChanReportMaskStructure
      perChanMask_edited; //!< Edited per-channel report mask structure

  struct CombPlotsReportMaskStructure
  {
    QMap<QString, int>
        ShowCombPlotsTypes; //!< Map of combined plots types to show

    // type (s,D,..) -> model -> yes/no
    QMap<QString, QMap<QString, QString> >
        ShowCombPlotParts; //!< Map of combined plot parts to show

    int has_combo_plots; //!< Flag for combined plots
  };

  CombPlotsReportMaskStructure
      combPlotsMask_edited; //!< Edited combined plots report mask structure
  ...
  MiscReportMaskStructure
      miscMask_edited; //!< Edited miscellaneous report mask structure
...
private:
  ...
  QMap<QString, QTreeWidgetItem *>
      analysisGenItem; //!< Map of general analysis items
  QMap<QString, QTreeWidgetItem *>
      analysis2DSAItem; //!< Map of 2DSA analysis items
  QMap<QString, QTreeWidgetItem *>
      analysisPCSAItem;      //!< Map of PCSA analysis items
  ...
  QMap<QString, QTreeWidgetItem *>
      tripleModelItem; //!< Map of triple model items
  QMap<QString, QTreeWidgetItem *>
      tripleMaskItem; //!< Map of triple mask items
  QMap<QString, QTreeWidgetItem *>
      tripleMaskPlotItem; //!< Map of triple mask plot items
  QMap<QString, QTreeWidgetItem *>
      tripleMaskPseudoItem; //!< Map of triple mask pseudo items
  QMap<QString, QTreeWidgetItem *>
      tripleMaskIndComboPlotItem; //!< Map of triple mask individual combined
                                  //!< plot items

  // Combined plots masks
  QMap<QString, QTreeWidgetItem *>
      topItemCombPlots; //!< Map of top items for combined plots
  QMap<QString, QTreeWidgetItem *>
      ItemCombPlots; //!< Map of items for combined plots
  ...
  QMap<QString, QStringList>
      Triple_to_ModelsMissing; //!< Triple to missing models map
  QMap<QString, QString> Triple_to_FailedStage; //!< Triple to failed stage map
  QMap<QString, QMap<QString, QString> >
      Triple_to_ModelsDesc; //!< Triple to models description map
  QMap<QString, QMap<QString, QString> >
      Triple_to_ModelsDescGuid; //!< Triple to models description GUID map
  ...
  QMap<QString, QMap<QString, QMap<QString, bool> > >
      indComboPlotsMapTripleTypeRangeBool; //!< Individual combined plots map
  QMap<QString, QStringList>
      CombPlots_Type_to_Models;         //!< Combined plots type to models map
  QMap<QString, QString> eSign_details; //!< Electronic signature details

  QMap<QString, QStringList>
      CombPlotsParmsMap; //!< Combined plots parameters map
  QMap<QString, QList<QColor> >
      CombPlotsParmsMap_Colors; //!< Combined plots parameters colors map
  ...
  void add_solution_details (const QString, const QString,
                             QString &); //!< Add solution details
  void assemble_parts (QString &);       //!< Assemble parts
  int list_all_gmp_reports_db (QList<QStringList> &,
                               US_DB2 *); //!< List all GMP reports from DB
  int list_all_autoflow_records (
      QList<QStringList> &); //!< List all autoflow records
  ...
  int get_expID_by_runID_invID (
      US_DB2 *, QString); //!< Get experiment ID by run ID and investigator ID
  ...
  void read_autoflowStatus_record (QString &, QString &, QString &, QString &,
                                   ...
                                   QString &, QString &, QString &, QString &,
                                   QString &,
                                   QString &); //!< Read autoflow status record
  QMap<QString, QMap<QString, QString> >
  parse_autoflowStatus_json (const QString,
                             const QString); //!< Parse autoflow status JSON
  QMap<QString, QString> parse_autoflowStatus_analysis_json (
      const QString); //!< Parse autoflow status analysis JSON

  void read_reportLists_from_aprofile (
      QStringList &,
      QStringList &); //!< Read report lists from analysis profile
  bool readReportLists (QXmlStreamReader &, QMap<QString, QString> &,
                        QMap<QString, QString> &); //!< Read report lists
  QStringList buildDroppedTriplesList (
      US_DB2 *, QMap<QString, QString>); //!< Build dropped triples list

  void
  read_protocol_and_reportMasks (void); //!< Read protocol and report masks
  QMap<QString, QString>
  read_autoflowIntensity (QString, US_DB2 *); //!< Read autoflow intensity
  QMap<QString, QString> read_autoflowGMPReportEsign_record (
      US_DB2 *); //!< Read autoflow GMP report electronic signature record
  QString get_assigned_oper_revs (
      QJsonDocument); //!< Get assigned operator revisions
  void parse_gen_mask_json (const QString); //!< Parse general mask JSON
  QMap<QString, QMap<QString, QString> >
  parse_comb_plots_json (const QString); //!< Parse combined plots JSON
  QMap<QString, QString>
  parse_models_desc_json (const QString,
                          const QString); //!< Parse models description JSON

  void get_item_childs (QList<QTreeWidgetItem *> &,
                        QTreeWidgetItem *); //!< Get item children
  ...
  void get_children_to_json (QString &,
                             QTreeWidgetItem *); //!< Get children to JSON
  QString tree_to_json (
      QMap<QString, QTreeWidgetItem *>); //!< Convert tree to JSON
  void parse_edited_gen_mask_json (
      const QString,
      GenReportMaskStructure &); //!< Parse edited general mask JSON
  void parse_edited_perChan_mask_json (
      const QString,
      PerChanReportMaskStructure &); //!< Parse edited per-channel mask JSON
  void parse_edited_perChan_mask_json_abde (const QString,
                                            PerChanReportMaskStructureABDE &);
  void parse_edited_combPlots_mask_json (
      const QString, CombPlotsReportMaskStructure
                         &); //!< Parse edited combined plots mask JSON
  void parse_edited_misc_mask_json (
      const QString,
      MiscReportMaskStructure &); //!< Parse edited miscellaneous mask JSON
  ...
  public:
     ...
     */
    bool
    operator< (const ModelDesc &md) const
    {
  ...
  public:
     ...
     */
    bool
    operator< (const NoiseDesc &nd) const
    {
  ...
  int count_noise_auto (US_DataIO::EditedData *, US_Model *, QStringList &,
                        QStringList &); //!< Count noise automatically

  int id_list_db_auto (QString); //!< Get ID list from DB automatically
  int
  models_in_edit_auto (QString,
                       QStringList &); //!< Get models in edit automatically
  int
  noises_in_model_auto (QString,
                        QStringList &); //!< Get noises in model automatically
  ...
  QString html_header (QString, QString,
                       US_DataIO::EditedData *); //!< Generate HTML header
  QString html_header_abde (QString, QString,
                            QString); //!< Generate HTML header
  QString distrib_info (
      QMap<QString, QString> &);         //!< Generate distribution information
  ...
  QMap<QString, double> get_replicate_group_results (
      US_ReportGMP::ReportItem, QString,
      QStringList); //!< Get replicate group results
  void assemble_replicate_av_integration_html (
      void); //!< Assemble replicate average integration HTML
...
private slots:
  ...
  QMap<QString, QString> read_autoflowAnalysis_record (
      US_DB2 *, const QString &); //!< Read autoflow analysis record
  QMap<QString, QString> read_autoflowAnalysisHistory_record (
      US_DB2 *, const QString &); //!< Read autoflow analysis history record
  ...
  QString compose_html_failed_stage_missing_models (
      void); //!< Compose HTML for failed stage with missing models
  ...
  void assemble_distrib_html (
      QMap<QString, QString> &);               //!< Assemble distribution HTML
  void assemble_distrib_ABDE_html (QString &); //!< Assemble distribution HTML
  void assemble_plots_html (QStringList,
                            QString = QString ("")); //!< Assemble plots HTML
  double interp_sval (double, double *, double *,
                      int);                //!< Interpolate s-value
  void plotres (QMap<QString, QString> &); //!< Plot residuals
  void plot_pseudo3D (QString, QString);   //!< Plot pseudo 3D
  bool modelGuidExistsForStage (
      QString, QString); //!< Check if model GUID exists for stage
  bool modelGuidExistsForStage_ind (
      QString, QString,
      QString); //!< Check if model GUID exists for stage (individual)
  void process_combined_plots (QString); //!< Process combined plots
  void process_combined_plots_individual (
      QString, QString); //!< Process combined plots (individual)
  ...
  void get_abde_data_per_channel (
      QMap<QString, QMap<QString, QVector<QVector<double> > > > &);

  QMap<QString, QString> read_autoflowGMPReportEsign_record (
      QString); //!< Read autoflow GMP report electronic signature record
  void
  get_assigned_oper_revs (QJsonDocument,
                          QStringList &); //!< Get assigned operator revisions

  QString indent (int) const; //!< Generate indent
  QString
  table_row (const QString &,
             const QString &) const; //!< Generate table row with 2 columns
  QString
  table_row (const QString &, const QString &,
             const QString &) const; //!< Generate table row with 3 columns
  QString
  table_row (const QString &, const QString &, const QString &,
             const QString &) const; //!< Generate table row with 4 columns
  QString
  table_row (const QString &, const QString &, const QString &,
             const QString &,
             const QString &) const; //!< Generate table row with 5 columns
  QString
  table_row (const QString &, const QString &, const QString &,
             const QString &, const QString &,
             const QString &) const; //!< Generate table row with 6 columns
  QString
  table_row (const QString &, const QString &, const QString &,
             const QString &, const QString &, const QString &,
             const QString &) const; //!< Generate table row with 7 columns
  QString
  table_row (const QString &, const QString &, const QString &,
             const QString &, const QString &, const QString &,
             const QString &,
             const QString &) const; //!< Generate table row with 8 columns
  QString
  table_row (const QString &, const QString &, const QString &,
             const QString &, const QString &, const QString &,
             const QString &, const QString &,
             const QString &) const; //!< Generate table row with 9 columns
