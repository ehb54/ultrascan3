    ...
    : US_WidgetsDialog (0, 0), runID (run)
{
  ...
  QString invval
      = QString::number (personID) + ": " + US_Settings::us_inv_name ();
  ...
  while (db.next ())
    {
      ...
      rr.date = US_Util::toUTCDatetimeText (
  ...
  for (int ii = 0; ii < rdirs.count (); ii++)
    {
      ...
      QStringList efiles
          = QDir (wdir).entryList (efilt, QDir::Files, QDir::Name);
  ...
  for (int ii = 0; ii < runids.count (); ii++)
    {
      ...
      // qDebug() << "LdDk:   ii" << ii << "ef" << efilt << "efcount" <<
      // efiles.count()
      //  << "  run" << runID;
      if (efiles.count () != 1)
        continue;
      ...
      QString date = US_Util::toUTCDatetimeText (
                         QFileInfo (rfn).lastModified ().toUTC ().toString (
                             Qt::ISODate),
                         true)
                         .section (" ", 0, 0)
                         .simplified ();
      ...
      rr.runID = runID;
      rr.label = label;
      // qDebug() << "LdDk:   ii" << ii << "   expID" << expID << "ID date
      // runID label"
      //  << rr.ID << rr.date << rr.runID << rr.label;
...
bool
US_GetRun::check_filename_for_autoflow (QString rFilename)
{
...
US_GetRun::deleteRun (void)
{
  ...
  qDebug () << "GetRun::check for GMP: ndx_1 -- " << ndx_1 << ", rFilename -- "
            << rFilename << ", expID  -- " << rExpID;

  bool isRequired = check_filename_for_autoflow (rFilename);
  if (isRequired)
    {
      QMessageBox::information (
          this, tr ("Data Cannot be Deleted"),
...
void
US_GetRun::update_disk_db (bool isDB)
{
...
void
US_GetRun::limit_data (const QString &sfilt)
{
...
void
US_GetRun::sel_investigator (void)
{
  if (!dkdb_cntrls->db ())
...
void
US_GetRun::assign_investigator (int invID)
{
  personID = invID;
