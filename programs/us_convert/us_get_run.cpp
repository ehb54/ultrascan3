  ...
  for (int ii = 0; ii < runids.count(); ii++) {
    ...
    QString date =
        US_Util::toUTCDatetimeText(
            QFileInfo(rfn).lastModified().toUTC().toString(Qt::ISODate), true)
            .section(" ", 0, 0)
            .simplified();

    // qDebug() << "LdDk:   ii" << ii << "  rfn" << rfn;
    QFile xfi(rfn);
    if (!xfi.open(QIODevice::ReadOnly | QIODevice::Text)) continue;
    ...
    rr.runID = runID;
    rr.label = label;
    // qDebug() << "LdDk:   ii" << ii << "   expID" << expID << "ID date runID
    // label"
    //  << rr.ID << rr.date << rr.runID << rr.label;
...
void US_GetRun::update_disk_db(bool isDB) {
  ...
  if (!sfilt.isEmpty()) limit_data(sfilt);
