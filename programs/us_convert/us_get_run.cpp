...
US_GetRun::US_GetRun(QString &run, bool inDB) : US_WidgetsDialog(0, 0), runID(run) {
   int ddstate;
   if (inDB) {
   ...
   QString invval = QString::number(personID) + ": " + US_Settings::us_inv_name();
...
void US_GetRun::load_db(void) {
   ...
   while (db.next()) {
      ...
      rr.date = US_Util::toUTCDatetimeText(db.value(5).toDateTime().toString(Qt::ISODate), true)
                   .section(" ", 0, 0)
                   .simplified();
...
void US_GetRun::load_disk(void) {
   ...
   QStringList rdirs = QDir(resdir).entryList(QDir::AllDirs | QDir::NoDotAndDotDot, QDir::Name);
   ...
   for (int ii = 0; ii < runids.count(); ii++) {
      ...
      while (!xml.atEnd()) { // Read experiment elements
         QString xname;
         QString attv;
...
void US_GetRun::deleteRun(void) {
   ...
   qDebug() << "GetRun::check for GMP: ndx_1 -- " << ndx_1 << ", rFilename -- " << rFilename << ", expID  -- "
            << rExpID;
