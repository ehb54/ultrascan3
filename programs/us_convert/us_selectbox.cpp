...
US_SelectBox::US_SelectBox (QWidget *parent)
    : QComboBox (parent), US_SelectBase ()
{
...
void
US_SelectBox::addOption (listInfo &option)
{
  this->widgetList << option;
}

void
US_SelectBox::addOptions (QList<listInfo> &options)
{
...
void
US_SelectBox::setLogicalIndex (int ID)
{
...
int
US_SelectBox::getLogicalID (void)
{
...
US_ListwidgetBox::US_ListwidgetBox (QWidget *parent, int fontAdjust)
    : QListWidget (parent), US_SelectBase ()
{
  this->setAutoFillBackground (true);
...
void
US_ListwidgetBox::addOption (listInfo &option)
{
  this->widgetList << option;
}

void
US_ListwidgetBox::addOptions (QList<listInfo> &options)
{
...
// Function to update a combobox so that the current choice is selected
void
US_ListwidgetBox::setLogicalIndex (int ID)
{
  for (int i = 0; i < this->widgetList.size (); i++)
    {
      if (this->widgetList[i].ID.toInt () == ID)
...
int
US_ListwidgetBox::getLogicalID (void)
{
