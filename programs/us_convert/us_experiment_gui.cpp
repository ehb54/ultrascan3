...
US_ExperimentGui::US_ExperimentGui(bool signal_wanted, US_Experiment &dataIn, int select_db_disk) :
...
void US_ExperimentGui::setOperatorList(void) {
   qDebug() << "ExpG:sOL:   cLab cInstr" << currentLab << currentInstrument << "llsz" << labList.size();
   QList<listInfo> options;
   QList<US_Rotor::Instrument> instruments = labList[ currentLab ].instruments;
   int insz = (currentInstrument < labList[ currentLab ].instruments.size()) ? labList[ currentLab ].instruments.size()
                                                                             : 0;
...
void US_ExperimentGui::change_instrument(int) {
   ...
   expInfo.operatorID = (cb_operator->getLogicalID() == -1) ? expInfo.operatorID : cb_operator->getLogicalID();
