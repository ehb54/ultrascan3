...
void US_ExperimentGui::reset(void) {
  ...
  if (etndx >= 0) cb_expType->setCurrentIndex(etndx);
  ...
  if (expInfo.invID > 0 || !disk_controls->db()) {
    ...
    if (disk_controls->db()) {
      ...
      if (db.lastErrno() == US_DB2::OK) runIDStatus = expInfo.checkRunID(&db);
    ...
    if (expInfo.label.isEmpty()) pb_accept->setEnabled(false);
  }
...
bool US_ExperimentGui::load(void) {
  ...
  if (!expInfo.label.isEmpty()) le_label->setText(expInfo.label);

  if (expInfo.project.projectID > 0)
    le_project->setText(expInfo.project.projectDesc);

  if (!expInfo.comments.isEmpty()) te_comment->setText(expInfo.comments);
  ...
  if (disk_controls->db()) {
    ...
    if (db.lastErrno() == US_DB2::OK) US_Rotor::readLabsDB(labList, &db);
    ...
    if (linssize == 0) {  // If empty instrument table, warn and exit
      ...
      QMessageBox::warning(this, tr("No Instruments"),
...
void US_ExperimentGui::reload(void) {
  ...
  if (lab_changed && labList.size() > 0) {
    ...
    for (int i = 0; i < labList.size(); i++) {
      if (labList[i].ID == expInfo.labID && labList[i].instruments.size() > 0) {
        found = true;
        currentLab = i;
        break;
      }
...
void US_ExperimentGui::getInvestigatorInfo(void) {
  ...
  if (disk_controls->db()) {
    ...
    if (db.lastErrno() == US_DB2::OK) {
      ...
      if (db.next()) expInfo.invGUID = db.value(9).toString();
    }
  }
}
...
void US_ExperimentGui::cancelProject(void) { reset(); }
...
void US_ExperimentGui::setOperatorList(void) {
  ...
  if (options.size() > 0) {
    ...
    if (!disk_controls->db()) {
      ...
      for (int i = 0; i < options.size(); i++) {
        qDebug() << "ExpG:sOL:    i" << i << "eoID" << expInfo.operatorID
                 << "oID" << options[i].ID;
...
void US_ExperimentGui::cancelRotor(void) { reset(); }
