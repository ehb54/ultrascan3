...
US_ExperimentGui::US_ExperimentGui (bool signal_wanted, US_Experiment &dataIn,
                                    int select_db_disk)
    : US_WidgetsDialog (0, 0), expInfo (dataIn)
{
  ...
  QPushButton *pb_rotor
      = us_pushbutton (tr ("Select Lab / Rotor / Calibration"));
  ...
  if (expInfo.invID > 0 || !disk_controls->db ())
    {
      ...
      if (expInfo.project.projectID == 0
          && expInfo.project.projectGUID.isEmpty ())
        pb_accept->setEnabled (false);
...
void
US_ExperimentGui::syncHardware (void)
{
}

void
US_ExperimentGui::selectInvestigator (void)
{
...
void
US_ExperimentGui::assignInvestigator (int invID)
{
  expInfo.invID = invID;
...
void
US_ExperimentGui::getInvestigatorInfo (void)
{
...
void
US_ExperimentGui::source_changed (bool db)
{
  QStringList DB = US_Settings::defaultDB ();
...
void
US_ExperimentGui::update_disk_db (bool db)
{
...
void
US_ExperimentGui::selectProject (void)
{
  ...
  connect (projInfo, SIGNAL (updateProjectGuiSelection (US_Project &)),
           SLOT (assignProject (US_Project &)));

  connect (projInfo, SIGNAL (cancelProjectGuiSelection ()),
           SLOT (cancelProject ()));
...
void
US_ExperimentGui::assignProject (US_Project &project)
{
...
void
US_ExperimentGui::cancelProject (void)
{
  reset ();
}
...
void
US_ExperimentGui::saveLabel (void)
{
...
QComboBox *
US_ExperimentGui::us_expTypeComboBox (void)
{
...
void
US_ExperimentGui::setInstrumentList (void)
{
...
void
US_ExperimentGui::setOperatorList (void)
{
  ...
  int insz = (currentInstrument < labList[currentLab].instruments.size ())
                 ? labList[currentLab].instruments.size ()
                 : 0;
  qDebug () << "ExpG:sOL:    insz" << insz;
  QList<US_Rotor::Operator> operators
      = instruments[currentInstrument].operators;
  ...
  if (options.size () > 0)
    {
      ...
      if (!disk_controls->db ())
      ...
      else
        {
          ...
          for (int i = 0; i < options.size (); i++)
            {
              qDebug () << "ExpG:sOL:    i" << i << "eoID"
                        << expInfo.operatorID << "oID" << options[i].ID;
      ...
      qDebug () << "ExpG:sOL: eI ID GUID" << expInfo.operatorID
                << expInfo.operatorGUID;
    }
}
...
void
US_ExperimentGui::change_instrument (int)
{
...
void
US_ExperimentGui::selectRotor (void)
{
  // Save other elements on the page first
  expInfo.label = le_label->text ();
  expInfo.comments = te_comment->toPlainText ();
...
void
US_ExperimentGui::assignRotor (US_Rotor::Rotor &rotor,
                               US_Rotor::RotorCalibration &calibration)
{
...
void
US_ExperimentGui::cancelRotor (void)
{
  reset ();
}

void
US_ExperimentGui::accept (void)
{
...
void
US_ExperimentGui::connect_error (const QString &error)
{
