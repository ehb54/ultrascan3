...
class US_ConvertIO {
  ...
  */
  ...
  /*! \brief Writes a new DB rawData record for each triple
      ...
      \param db      An opened db connection
  */
  static QString writeRawDataToDB(US_Experiment&,
  ...
  /*! \brief Checks some info that was read from disk with values from DB
      ...
      \param db      An opened db connection
  */
  static int checkDiskData(US_Experiment&, QList<US_Convert::TripleInfo>&,
                           US_DB2* = 0);
  static int checkDiskData_auto(US_Experiment&, QList<US_Convert::TripleInfo>&,
                                US_DB2* = 0);

 private:
  static QString readRawDataFromDB(US_Experiment&,
                                   QList<US_Convert::TripleInfo>&, QString&,
                                   US_DB2* = 0);
};
#endif
