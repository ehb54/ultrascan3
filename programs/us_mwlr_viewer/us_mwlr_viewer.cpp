...
US_MwlRawViewer::US_MwlRawViewer () : US_Widgets ()
{
  ...
  connect (cb_lend, SIGNAL (currentIndexChanged (int)), this,
           SLOT (changeLambda ()));
  connect (ct_recavg, SIGNAL (valueChanged (double)), this,
           SLOT (changeAverage ()));
  connect (cb_pltrec, SIGNAL (currentIndexChanged (int)), this,
           SLOT (changeRecord ()));
  connect (ck_xwavlen, SIGNAL (toggled (bool)), this,
           SLOT (changeRectype (bool)));
...
void
US_MwlRawViewer::enableControls (void)
{
  if (allData.size () == 0)
...
US_MwlRawViewer::load_mwl_raw ()
{
  ...
  QApplication::restoreOverrideCursor ();
  mwl_fnames
      = dirdir.entryList (QStringList ("*.mwrs"), QDir::Files, QDir::Name);
  mwl_fnames.sort ();
...
void
US_MwlRawViewer::runDetails (void)
{
  ...
  int wvx = 0;
  int rpx = 0;
  DbgLv (1) << "DDet: nlambda ntriple ntpoint" << nlambda << ntriple
            << ntpoint;
...
void
US_MwlRawViewer::changeCellCh (void)
{
...
#else
      QwtScaleDiv *sdx
          = (QwtScaleDiv *)&data_plot->axisScaleDiv (QwtPlot::xBottom);
      QwtScaleDiv *sdy
...
void
US_MwlRawViewer::plot_current (void)
{
  if (allData.size () == 0)
    return;
...
void
US_MwlRawViewer::plot_titles (void)
{
  QString cellch = cb_cellchn->currentText ();
...
void
US_MwlRawViewer::changeRecord (void)
{
...
void
US_MwlRawViewer::changeRectype (bool wlnrec)
{
...
void
US_MwlRawViewer::build_cmp_data ()
{
...
void
US_MwlRawViewer::build_avg_data ()
{
...
void
US_MwlRawViewer::build_rec_data (const int recx, QVector<double> &rdata)
{
      ...
      for (int scnx = 0; scnx < nscan; scnx++)
        { // Examine all scans, but only store for included ones
          ...
          for (int wavx = wavxs; wavx < wavxe; wavx++)
            { // Save a data point for each wavelength (radius=record is
              // constant)
...
void
US_MwlRawViewer::compute_ranges ()
{
...
void
US_MwlRawViewer::connect_ranges (bool conn)
{
  if (conn)
...
void
US_MwlRawViewer::exclude_from (double sfr)
{
...
void
US_MwlRawViewer::exclude_to (double sto)
{
  int scan_to = (int)sto;
  int scan_from = (int)ct_from->value ();
  ...
  if (p3d_ctld == NULL)
    {
      ...
      int cy = 40;
      p3d_ctld->move (cx, cy);
      connect (p3d_ctld, SIGNAL (has_closed ()), this,
               SLOT (p3dctrl_closed ()));
    }
      ...
      if (p3d_pltw != NULL)
        {
          ...
          QString ptitle
              = tr ("MWL 3-D Plot, Scan ") + QString::number (scan_nbr);
...
void
US_MwlRawViewer::save_plot ()
{
  DbgLv (1) << "Save Plot";
  QString savedir = US_Settings::reportDir () + "/" + runID;
  QDir ().mkpath (savedir);
  savedir = savedir.replace ("\\", "/") + "/";
  QString fname2d = runID
                    + (ck_xwavlen->isChecked () ? ".radRec_RRRRR_2D.png"
                                                : ".lmbRec_RRRRR_2D.png");
  ...
  for (int prx = 0; prx < krecs; prx++)
    {
      ...
      QString fname = QString (bfname)
                          .replace ("RRRRR", rec_str)
...
int
US_MwlRawViewer::dvec_index (QVector<double> &dvec, const double dval)
{
...
int
US_MwlRawViewer::build_xyz_data (QVector<QVector3D> &xyzd, int scan)
{
  ...
  DbgLv (1) << "cmpR:  pxS pxE" << pltxvals[0]
            << pltxvals[pltxvals.count () - 1];
...
void
US_MwlRawViewer::p3dctrl_closed ()
{
  p3d_ctld = NULL;
  p3d_pltw = NULL;
...
int
US_MwlRawViewer::live_scan (int *fsP, int *lsP, int *ksP)
{
  int fscnx = -1;
  int lscnx = -1;
  int krscan = 0;
