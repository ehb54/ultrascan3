...
US_MwlRawViewer::US_MwlRawViewer() : US_Widgets() {
   ...
   // Signals and Slots
   connect(pb_loadMwl, SIGNAL(clicked()), this, SLOT(load_mwl_raw()));
   connect(pb_loadAUC, SIGNAL(clicked()), this, SLOT(load_auc_mwl()));
   ...
   connect(cb_lstart, SIGNAL(currentIndexChanged(int)), this, SLOT(changeLambda()));
   connect(cb_lend, SIGNAL(currentIndexChanged(int)), this, SLOT(changeLambda()));
   connect(ct_recavg, SIGNAL(valueChanged(double)), this, SLOT(changeAverage()));
   connect(cb_pltrec, SIGNAL(currentIndexChanged(int)), this, SLOT(changeRecord()));
   connect(ck_xwavlen, SIGNAL(toggled(bool)), this, SLOT(changeRectype(bool)));
   ...
   connect(pb_movie2d, SIGNAL(clicked()), this, SLOT(show_2d_movie()));
   connect(pb_plot3d, SIGNAL(clicked()), this, SLOT(plot_3d()));
   connect(pb_movie3d, SIGNAL(clicked()), this, SLOT(show_3d_movie()));
   connect(pb_svplot, SIGNAL(clicked()), this, SLOT(save_plot()));
   connect(pb_svmovie, SIGNAL(clicked()), this, SLOT(save_movie()));
   ...
   QBoxLayout *plot = new US_Plot(data_plot, tr("Intensity Data"), tr("Radius (in cm)"), tr("Intensity"));
...
void US_MwlRawViewer::load_mwl_raw() {
   ...
   QApplication::restoreOverrideCursor();
   mwl_fnames = dirdir.entryList(QStringList("*.mwrs"), QDir::Files, QDir::Name);
   mwl_fnames.sort();
...
void US_MwlRawViewer::load_auc_mwl() {
   ...
   if (dir.right(1) != "/")
      dir += "/"; // Ensure trailing /
   mwl_fnames = dirdir.entryList(QStringList("*.auc"), QDir::Files, QDir::Name);
   mwl_fnames.sort();
   ntriple = mwl_fnames.size();
   runID = new_runID;
   cellchans.clear();
   ...
   npoint = allData[ 0 ].pointCount();
   ntpoint = nscan * npoint;
   DbgLv(1) << "RD: mwr ncellch nlambda nscan npoint" << ncellch << nlambda << nscan << npoint;
   DbgLv(1) << "RD:   rvS rvE" << radii[ 0 ] << radii[ npoint - 1 ];
...
void US_MwlRawViewer::changeCellCh(void) {
   ...
   if (last_xmin < 0.0) { // If first time plotting any data, detect actual data bounds
      ...
      if (is_wrecs) { // For wavelength records, X bounds are radius bounds
      ...
      else { // For radius records, X bounds are lambda bounds
...
void US_MwlRawViewer::changeRadius() {
   ...
   if (!is_wrecs) { // For radius records, we need to reset the plot record list
      cb_pltrec->disconnect();
      cb_pltrec->clear();

      for (int rdx = radxs; rdx < radxe; rdx++) { // Build the list of radii that are within range
...
void US_MwlRawViewer::changeLambda() {
   ...
   if (is_wrecs) { // For wavelength records, we need to reset the plot record list
      cb_pltrec->disconnect();
      cb_pltrec->clear();

      for (int wvx = lmbxs; wvx < lmbxe; wvx++) { // Build the list of wavelengths that are within range
...
void US_MwlRawViewer::changeRectype(bool wlnrec) {
   ...
   if (is_wrecs) { // Change record button title and reset for lambda record
      lb_pltrec->setText(tr("Plot (W nm)"));
      changeLambda(); // Change to lambda records
      last_xmin = rad_start; // Change X limits
      last_xmax = rad_end;
   }
   else { // Change record button title and reset for radius record
...
void US_MwlRawViewer::build_cmp_data() {
   ...
   for (int krx = arxs; krx < arxe; krx++, jrx++) { // Build or move component records
      int prx = prev_recxs.indexOf(krx);
      DbgLv(1) << "BldC:   krx" << krx << "prx" << prx;

      if (prx < 0) { // Previous components missing this one, so build a new one
         build_rec_data(krx, curr_cdata[ jrx ]);
      }
      else { // Previous components had this one, so just copy to new location
...
void US_MwlRawViewer::build_avg_data() {
   ...
   for (int ptx = 0; ptx < ktpoint; ptx++) { // Compute the average at each data point
      double dsum = 0.0;

      for (int rcx = 0; rcx < kavgc; rcx++) { // Build a data point sum across all components
         dsum += curr_cdata[ rcx ][ ptx ];
         if (ptx == (ktpoint / 2))
            DbgLv(1) << "BldA:   ptx rcx" << ptx << rcx << "cdat dsum" << curr_cdata[ rcx ][ ptx ] << dsum;
      }
...
void US_MwlRawViewer::build_rec_data(const int recx, QVector<double> &rdata) {
   ...
   if (is_wrecs) { // Build data values for wavelength record with x-axis radius
      ...
      for (int scnx = 0; scnx < nscan; scnx++) { // Examine all scans, but only store for included ones
         if (excludes.contains(scnx))
            continue;

         for (int radx = radxs; radx < radxe; radx++) { // Save a data point for each radius (input=triple is constant)
   ...
   else { // Build data values for radius record with x-axis wavelength
      ...
      for (int scnx = 0; scnx < nscan; scnx++) { // Examine all scans, but only store for included ones
         ...
         for (int wavx = wavxs; wavx < wavxe;
              wavx++) { // Save a data point for each wavelength (radius=record is constant)
...
void US_MwlRawViewer::plot_3d() {
   ...
   if (p3d_ctld == NULL) {
      ...
      int cy = 40;
      p3d_ctld->move(cx, cy);
      connect(p3d_ctld, SIGNAL(has_closed()), this, SLOT(p3dctrl_closed()));
   }
      ...
      if (p3d_pltw != NULL) {
         ...
         QString ptitle = tr("MWL 3-D Plot, Scan ") + QString::number(scan_nbr);
...
void US_MwlRawViewer::show_3d_movie() {
   ...
   if (p3d_ctld == NULL) { // This should not happen, but disallow Show 3D Movie if no plot opened
      return;
   }

   p3d_pltw = p3d_ctld->widget_3dplot(); // Main 3D plot window

   if (p3d_pltw == NULL) { // If the dialog is opened
      QMessageBox::warning(
         this, tr("3-D Plot Window Not Opened"),
   ...
   QString statmsg = (nscan == krscan) ? tr("Of %1 scans, showing:   Scan ").arg(nscan)
...
void US_MwlRawViewer::save_plot() {
   ...
   QString fname2d = runID + (ck_xwavlen->isChecked() ? ".radRec_RRRRR_2D.png" : ".lmbRec_RRRRR_2D.png");
   QString fname3d = runID + ".Scan_SSSS_3D.png";
   p3d_pltw = (p3d_ctld == NULL) ? NULL : p3d_ctld->widget_3dplot();
   int nfiles = (p3d_pltw != NULL) ? 2 : 1;

   if (nfiles == 2) { // If there is a 3D window, first save a PNG of that window
   ...
   QString mtitle = (nfiles == 1) ? tr("Plot File Saved") : tr("Plot Files Saved");
   QString msg = tr("In the directory\n     %1,\n\n").arg(savedir);
   if (nfiles == 1)
      msg += tr("File\n    %1 was saved.").arg(fname2d);
   else
      msg += tr("Files\n    %1 ;  and\n    %2\nwere saved.").arg(fname3d).arg(fname2d);
...
void US_MwlRawViewer::save_movie() {
   ...
   DbgLv(1) << "Save Movie";
   if (save_3d) { // If there is a 3D window, ask the user which movies to run
      QMessageBox msgBox(this);
      msgBox.setWindowTitle(tr("Multiple Possible Movie Saves"));
      msgBox.setTextFormat(Qt::RichText);
      msgBox.setText(tr("A 3-D plot window is opened, so you may save both a "
...
void US_MwlRawViewer::save_2d_movie() {
   ...
   QDir().mkpath(savedir);
   savedir = savedir.replace("\\", "/") + "/";
   QString bfname = runID
                    + (ck_xwavlen->isChecked() ? ".radRec_RRRRR_2D_frame_XXXXX.png"
   ...
   for (int prx = 0; prx < krecs; prx++) {
      ...
      QString fname = QString(bfname).replace("RRRRR", rec_str).replace("XXXXX", frm_str);
...
void US_MwlRawViewer::save_3d_movie() {
   ...
   if (p3d_ctld == NULL) { // This should not happen, but disallow Save 3D Movie if no plot opened
      return;
   }

   p3d_pltw = p3d_ctld->widget_3dplot(); // Main 3D plot window

   if (p3d_pltw == NULL) { // If the plot window is not opened, explain what to do
      QMessageBox::warning(
         this, tr("3-D Plot Window Not Opened"),
   ...
   for (int scnx = fscnx; scnx <= lscnx; scnx++) {
      ...
      QString fname = QString(bfname).replace("SSSS", s_scan).replace("XXXX", s_frame);
      QString fpath = savedir + fname;
...
int US_MwlRawViewer::build_xyz_data(QVector<QVector3D> &xyzd, int scan) {
   ...
   if (scan < 0) { // If no scan given, compute index of first included scan in given range
   ...
   else { // If scan given, insure it is included and in given range
      for (scnx = 0; scnx < nscan; scnx++) {
         if (excludes.contains(scnx))
            continue;
   ...
   DbgLv(1) << "Bxyz: lambda count" << k3dlamb << "range" << lmb3d[ 0 ] << lmb3d[ k3dlamb - 1 ];
   DbgLv(1) << "Bxyz: radius count" << k3drads << "range" << rad3d[ 0 ] << rad3d[ k3drads - 1 ];
   ...
   for (int klx = 0; klx < k3dlamb; klx++, wvx++) { // Outer loop is lambdas
...
int US_MwlRawViewer::live_scan(int *fsP, int *lsP, int *ksP) {
   ...
   for (int scnx = 0; scnx < nscan; scnx++) {
      ...
      if (scan_knt < scan_fr || scan_knt > scan_to)
         continue;
