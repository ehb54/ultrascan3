...
US_MwlRawViewer::US_MwlRawViewer() : US_Widgets() {
  ...
  QBoxLayout* plot = new US_Plot(data_plot, tr("Intensity Data"),
...
void US_MwlRawViewer::load_auc_mwl() {
  ...
  npoint = allData[0].pointCount();
  ntpoint = nscan * npoint;
  DbgLv(1) << "RD: mwr ncellch nlambda nscan npoint" << ncellch << nlambda
           << nscan << npoint;
  DbgLv(1) << "RD:   rvS rvE" << radii[0] << radii[npoint - 1];
...
void US_MwlRawViewer::changeRadius() {
  ...
  if (!is_wrecs) {  // For radius records, we need to reset the plot record list
    ...
    for (int rdx = radxs; rdx < radxe;
         rdx++) {  // Build the list of radii that are within range
...
void US_MwlRawViewer::changeLambda() {
  ...
  if (is_wrecs) {  // For wavelength records, we need to reset the plot record
                   // list
    cb_pltrec->disconnect();
    cb_pltrec->clear();

    for (int wvx = lmbxs; wvx < lmbxe;
         wvx++) {  // Build the list of wavelengths that are within range
...
void US_MwlRawViewer::build_cmp_data() {
  ...
  for (int krx = arxs; krx < arxe;
       krx++, jrx++) {  // Build or move component records
...
void US_MwlRawViewer::build_avg_data() {
  ...
  for (int ptx = 0; ptx < ktpoint;
       ptx++) {  // Compute the average at each data point
    double dsum = 0.0;

    for (int rcx = 0; rcx < kavgc;
         rcx++) {  // Build a data point sum across all components
...
void US_MwlRawViewer::build_rec_data(const int recx, QVector<double>& rdata) {
  ...
  if (is_wrecs) {  // Build data values for wavelength record with x-axis radius
    ...
    DbgLv(1) << "BldR:      trx rxs rxe nscan" << trpx << radxs << radxe
             << nscan;

    for (int scnx = 0; scnx < nscan;
         scnx++) {  // Examine all scans, but only store for included ones
      if (excludes.contains(scnx)) continue;

      for (int radx = radxs; radx < radxe;
           radx++) {  // Save a data point for each radius (input=triple is
                      // constant)
    ...
    for (int scnx = 0; scnx < nscan;
      ...
      for (int wavx = wavxs; wavx < wavxe;
           wavx++) {  // Save a data point for each wavelength (radius=record is
                      // constant)
...
void US_MwlRawViewer::show_3d_movie() {
  ...
  if (p3d_ctld == NULL) {  // This should not happen, but disallow Show 3D Movie
                           // if no plot opened
    return;
  }
...
void US_MwlRawViewer::save_plot() {
  ...
  QString fname2d = runID + (ck_xwavlen->isChecked() ? ".radRec_RRRRR_2D.png"
                                                     : ".lmbRec_RRRRR_2D.png");
  ...
  if (nfiles ==
      2) {  // If there is a 3D window, first save a PNG of that window
  ...
  QString mtitle =
      (nfiles == 1) ? tr("Plot File Saved") : tr("Plot Files Saved");
...
void US_MwlRawViewer::save_movie() {
  ...
  if (save_3d) save_3d_movie();

  if (save_2d) save_2d_movie();
}
...
void US_MwlRawViewer::save_2d_movie() {
  ...
  savedir = savedir.replace("\\", "/") + "/";
  QString bfname =
      runID + (ck_xwavlen->isChecked() ? ".radRec_RRRRR_2D_frame_XXXXX.png"
  ...
  for (int prx = 0; prx < krecs; prx++) {
    ...
    QString fname =
        QString(bfname).replace("RRRRR", rec_str).replace("XXXXX", frm_str);
...
void US_MwlRawViewer::save_3d_movie() {
  ...
  if (p3d_ctld == NULL) {  // This should not happen, but disallow Save 3D Movie
                           // if no plot opened
    return;
  }
  ...
  if (p3d_pltw ==
      NULL) {  // If the plot window is not opened, explain what to do
    QMessageBox::warning(this, tr("3-D Plot Window Not Opened"),
  ...
  for (int scnx = fscnx; scnx <= lscnx; scnx++) {
    ...
    QString fname =
        QString(bfname).replace("SSSS", s_scan).replace("XXXX", s_frame);
    QString fpath = savedir + fname;
...
int US_MwlRawViewer::build_xyz_data(QVector<QVector3D>& xyzd, int scan) {
  ...
  if (scan < 0) {  // If no scan given, compute index of first included scan in
                   // given range
    for (scnx = 0; scnx < nscan; scnx++) {
      if (excludes.contains(scnx)) continue;

      if (scan_knt >= scan_fr) break;
    ...
    for (scnx = 0; scnx < nscan; scnx++) {
      ...
      if (scan_knt >= scan_fr && scan_knt <= scan_to && scnx == scan) break;
  ...
  for (int lmbx = lmbxs; lmbx < lmbxe; lmbx++) lmb3d << lambdas[lmbx];

  // Build the list of radii to plot
  for (int radx = radxs; radx < radxe; radx++) rad3d << radii[radx];

  k3dlamb = lmb3d.count();  // Lambda plot count
  k3drads = rad3d.count();  // Radius plot count
  DbgLv(1) << "Bxyz: lambda count" << k3dlamb << "range" << lmb3d[0]
           << lmb3d[k3dlamb - 1];
  DbgLv(1) << "Bxyz: radius count" << k3drads << "range" << rad3d[0]
           << rad3d[k3drads - 1];
  ...
  for (int klx = 0; klx < k3dlamb; klx++, wvx++) {  // Outer loop is lambdas
    ...
    for (int krx = 0; krx < k3drads; krx++, rdx++) {
      ...
      DbgLv(2) << "Bxyz:    k" << xyzd.count() << "x y z" << xval << yval
               << zval;
    }
  }
...
int US_MwlRawViewer::live_scan(int* fsP, int* lsP, int* ksP) {
  ...
  for (int scnx = 0; scnx < nscan; scnx++) {
    ...
    if (fscnx < 0) fscnx = scnx;  // First in-range
