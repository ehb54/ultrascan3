...
US_MwlRun::US_MwlRun(QString &runID, bool isRawMwl) : US_WidgetsDialog(0, 0), runID(runID), isRawMwl(isRawMwl) {
   if (isRawMwl) {
...
void US_MwlRun::load_files(void) {
   ...
   qDebug() << "LdDk:  RawMwl" << isRawMwl << "efilt" << efilt << "rdirscount" << rdirs.count();
   resdir = resdir + "/";
   QStringList runids;
   ...
   for (int ii = 0; ii < rdirs.count(); ii++) {
      ...
      if (!isRawMwl) { // For US3 openAUC, count wavelengths
         QStringList wavelns;
         ...
         qDebug() << "LdDk:   ii" << ii << "run" << rdirs[ ii ] << "nwaveln" << wavelns.count();
...
void US_MwlRun::populate_list() {
   ...
   QStringList headers;
   headers << tr("Run") << tr("Date") << tr("Count of ") + (isRawMwl ? ".mwrs" : ".auc");
