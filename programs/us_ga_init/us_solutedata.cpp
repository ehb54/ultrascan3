...
bool
buck_vx_lessthan (const bucket &buck1, const bucket &buck2)
...
int
US_SoluteData::sortBuckets (QList<bucket> *buks)
{
  int rc = 0;
...
int
US_SoluteData::appendBucket (bucket &buk)
{
  int rc = 0;
...
int
US_SoluteData::appendBucket (QRectF brect, QPointF bpnt, qreal bconc,
                             int bstat)
{
...
int
US_SoluteData::setBucket (int ix, QRectF brect, QPointF bpnt, qreal bconc,
                          int bstat)
{
  bucket buk = createBucket (brect, bpnt, bconc, bstat);
  int rc = 0;
...
int
US_SoluteData::bucketsCount ()
{
  return allbucks.size ();
}

bucket
US_SoluteData::createBucket (QRectF brect, QPointF bpnt, qreal bconc,
                             int bstat)
{
  bucket buk;
...
bucket
US_SoluteData::minBucket ()
{
  bucket buk;
  return buk;
}
bucket
US_SoluteData::maxBucket ()
{
  bucket buk;
  return buk;
}
...
bucket
US_SoluteData::firstBucket ()
{
  return bucketAt (0);
}

bucket
US_SoluteData::lastBucket ()
{
  return bucketAt (-1);
}

bucket
US_SoluteData::nextBucket ()
{
  return bucketAt (-2);
}

bucket
US_SoluteData::currentBucket ()
{
  return bucketAt (-3);
}

QRectF
US_SoluteData::bucketRect (int ix)
{
...
QPointF
US_SoluteData::bucketPoint (int ix, bool nearest)
{
...
QPointF
US_SoluteData::bucketPoint (int ix)
{
  return bucketPoint (ix, true); // solute point near bucket
}

QSizeF
US_SoluteData::bucketSize (int ix)
{
  bucket buk = bucketAt (ix);
...
QString
US_SoluteData::bucketLine (int ix)
{
...
void
US_SoluteData::setDistro (QList<S_Solute> *a_distro, int a_ax, int a_ay,
                          int a_az)
{
  distro = a_distro;
  attr_x = a_ax;
...
int
US_SoluteData::findNearestPoint (QPointF &midpt)
{
  qreal xm = midpt.x ();
  qreal ym = midpt.y ();
  qreal xn = xm;
...
int
US_SoluteData::removeBucketAt (int ix)
{
...
int
US_SoluteData::autoCalcBins (int mxsols, qreal wsbuck, qreal hfbuck)
{
                      ...
                      if (vertr < horzr)
                        { // split vertically
                          ...
                          if (horzr > _MIN_VHR_ && vertr > _MIN_VHR_)
                            ...
                            DbgLv (2) << "BUCKET TOO THIN H,V " << horzr << ","
                                      << vertr;
...
int
US_SoluteData::saveGAdata (QString &fname, int xtype, int ytype, int ztype,
                           double fixval)
{
  int rc = 0;
...
int
US_SoluteData::loadGAdata (QString &fname, int *xtype, int *ytype, int *ztype,
                           double *fixval)
{
  int rc = 0;
  int nbuk = allbucks.size ();
...
int
US_SoluteData::reportDataMC (QString &fname, int mc_iters)
{
  int rc = 0;
...
void
US_SoluteData::outputStats (QTextStream &ts, QList<qreal> &vals,
...
void
US_SoluteData::limitBucket (bucket &buk)
{
      ...
      for (int jj = ii + 1; jj < nbuks; jj++)
        {
          if (bukrect.intersects (bucket_rects[jj]))
            { // Buckets overlap:  test if overlap is virtually a line
              ...
              DbgLv (1) << "   buki left-right" << bwid << "  bottom-top"
                        << bhgt;
...
void
US_SoluteData::bucketSeparate (int ii, int jj, QList<QRectF> &bucket_rects)
{
...
int
US_SoluteData::countFullestBucket ()
{
  int mxsol = 0;
