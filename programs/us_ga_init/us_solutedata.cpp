...
bool buck_vx_lessthan(const bucket &buck1, const bucket &buck2) { // TRUE iff  (sx1<sx2) ||  (sx1==sx2 && sy1<sy2)
   return (buck1.x_min < buck2.x_min) || ((buck1.x_min == buck2.x_min) && (buck1.y_min < buck2.y_min));
}
...
int US_SoluteData::appendBucket(QRectF brect, QPointF bpnt, qreal bconc, int bstat) {
...
int US_SoluteData::setBucket(int ix, QRectF brect, QPointF bpnt, qreal bconc, int bstat) {
   bucket buk = createBucket(brect, bpnt, bconc, bstat);
   int rc = 0;
...
bucket US_SoluteData::createBucket(QRectF brect, QPointF bpnt, qreal bconc, int bstat) {
   bucket buk;
...
bucket US_SoluteData::bucketAt(int ix) {
   ...
   if (ix > (-1) && ix < bsiz) { // get bucket at specified index and save the index
      buk = allbucks.at(ix);
      bndx = ix;
   }
...
void US_SoluteData::setDistro(QList<S_Solute> *a_distro, int a_ax, int a_ay, int a_az) {
   distro = a_distro;
   attr_x = a_ax;
   attr_y = a_ay;
   attr_z = a_az;
...
#define _MIN_VHR_ 0.01
   ...
   while (overs) {
      ...
      for (int jj = 1; jj < nssols; jj++) { // examine each of the current buckets
         qreal horzr;
         qreal vertr;
         buk = buks1->at(jj);
         ...
         for (int kk = 0; kk < jj; kk++) { // compare current to each previous bucket
            ...
            if (cx1 < px2) { // possible horizontal overlap
               ...
               else if (cy2 > py1 && cy1 < py1) { // current overlaps in its upper left
                  novls++;
                  buk.status = 1;
                  buk2 = buk;
                  horzr = (px2 - cx1) / wsbuck;
               ...
               else if (cy1 >= py1 && cy2 <= py2) { // current overlaps in its middle left
                  novls++;
                  buk.status = 1;
                  buk.x_min = qMin(px2, cx2);
                  buk.x_max = qMax(cx2, px2);
               ...
               else if (cy2 > py2 && cy1 < py1) { // current overlaps in both upper and lower left
                  novls++;
                  buk.status = 1;
                  buk2 = buk;
                  horzr = (px2 - cx1) / wsbuck;
      ...
      if (novls > 0) { // if there were overlaps, re-sort buckets by vertex for next pass
         qSort(buks1->begin(), buks1->end(), buck_vx_lessthan);
      }

      else if (tstat == 1) { // start new set of passes where reduced buckets can be reduced more
         tstat = 4;
      }
...
int US_SoluteData::saveGAdata(QString &fname, int xtype, int ytype, int ztype, double fixval) {
   int rc = 0;
   ...
   if (fileo.open(QIODevice::WriteOnly | QIODevice::Text)) {
      ...
      line += " # buckets=" + QString::number(nbuk) + " x=" + cts[ xtype ] + " y=" + cts[ ytype ]
              + " fixed=" + cts[ ztype ] + "="
              + ((ztype == US_GA_Initialize::ATTR_V) ? "0.0=(data_set_vbar)" : QString().sprintf(cffmt, fixval));
      ts << line << endl;
...
int US_SoluteData::loadGAdata(QString &fname, int *xtype, int *ytype, int *ztype, double *fixval) {
   int rc = 0;
   int nbuk = allbucks.size();
   int xt, yt, zt;
   double fxv = 0.0;
...
int US_SoluteData::buildDataMC() {
   ...
   for (int jj = 0; jj < nbuk; jj++) {
      ...
      for (int kk = 0; kk < nsol; kk++) { // add solute points that fall within bin dimensions
         ssval = distro->at(kk).s; // "x,y" of solute point
         sfval = distro->at(kk).k;
         if (ssval >= bsmin && ssval <= bsmax && sfval >= bkmin && sfval <= bkmax) { // solute is in this bin:  save component for bin
...
int US_SoluteData::reportDataMC(QString &fname, int mc_iters) {
   ...
   if (fileo.open(QIODevice::WriteOnly | QIODevice::Text)) { // output Monte Carlo statistics to a report file
      QList<qreal> valus;
      QList<qreal> concs;
      QList<qreal> csums;
      QTextStream ts(&fileo);
      ...
      for (int kk = 0; kk < nbuk; kk++) { // accumulate statistics for each bin
         ...
         if (ksol < 1) { // Summary print for a bin with no solutes
         ...
         else if (ksol < 3) { // Summary prints for a bin that has only a point or two
            ...
            csums.append(vtotal);
            ts << tr("Partial concentration:     ") << str1.sprintf(" %6.4e\n", vtotal);
         }
      }
      ...
      for (int jj = 0; jj < csums.size(); jj++) {
         ...
         ts << tr("Relative percentage of Solute ") << (jj + 1) << ": " << QString().sprintf("%7.3f", pcconc) << " %\n";
      }

      ts << tr("\n\nDetailed Results:\n");

      for (int kk = 0; kk < nbuk; kk++) { // output detailed statistics for all the bins
         bcomp = MC_solute.at(kk);
         ts << "\n*****************************************"
               "**********************************";
         ts << "\nSolute " << (kk + 1) << ":";
...
void US_SoluteData::outputStats(QTextStream &ts, QList<qreal> &vals, QList<qreal> &concs, bool details, QString title) {
   QString str1;
   QString str2;
   int nvals = vals.size();
   int nbins = 50;
   ...
   if (details && is_constant) {
      ...
      ts << tr("Mode Center:               ") << str1.sprintf("%6.4e\n", mode_cen);
...
void US_SoluteData::limitBucket(bucket &buk) {
   if (attr_x != US_GA_Initialize::ATTR_D && attr_x != US_GA_Initialize::ATTR_F) {
   ...
   if (attr_y != US_GA_Initialize::ATTR_D && attr_y != US_GA_Initialize::ATTR_F) {
...
int US_SoluteData::countOverlaps() {
   ...
   for (int ii = 0; ii < nbuks; ii++) {
      ...
      for (int jj = ii + 1; jj < nbuks; jj++) {
         if (bukrect.intersects(bucket_rects[ jj ])) { // Buckets overlap:  test if overlap is virtually a line
            ...
            DbgLv(1) << "   buck i" << bukrect.left() << bukrect.right() << bukrect.top() << bukrect.bottom();
            QRectF brj = bucket_rects[ jj ];
            DbgLv(1) << "   buck j" << brj.left() << brj.right() << brj.top() << brj.bottom();
            DbgLv(1) << "   buki" << buki.left() << buki.right() << buki.top() << buki.bottom();
            DbgLv(1) << "   buki left-right" << bwid << "  bottom-top" << bhgt;
            if (bwid < 1e-10 || bhgt < 1e-10) { // Buckets only share a line:  move a bit
...
void US_SoluteData::bucketSeparate(int ii, int jj, QList<QRectF> &bucket_rects) {
