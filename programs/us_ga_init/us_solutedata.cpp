...
bool buck_vx_lessthan(
    const bucket& buck1,
    const bucket& buck2) {  // TRUE iff  (sx1<sx2) ||  (sx1==sx2 && sy1<sy2)
...
int US_SoluteData::setBucket(int ix, QRectF brect, QPointF bpnt, qreal bconc,
                             int bstat) {
  bucket buk = createBucket(brect, bpnt, bconc, bstat);
  int rc = 0;
...
int US_SoluteData::bucketsCount() { return allbucks.size(); }

bucket US_SoluteData::createBucket(QRectF brect, QPointF bpnt, qreal bconc,
                                   int bstat) {
  bucket buk;
...
bucket US_SoluteData::bucketAt(int ix) {
  ...
  if (ix > (-1) &&
      ix < bsiz) {  // get bucket at specified index and save the index
    buk = allbucks.at(ix);
    bndx = ix;
  } else if (ix == (-1)) {  // get last bucket and set its index
    buk = allbucks.at(lx);
    bndx = lx;
  } else if (ix == (-2)) {  // get next bucket and set its index
    buk = allbucks.at(jx);
    bndx = jx;
  } else if (ix == (-3)) {  // get current bucket
...
bucket US_SoluteData::firstBucket() { return bucketAt(0); }

bucket US_SoluteData::lastBucket() { return bucketAt(-1); }

bucket US_SoluteData::nextBucket() { return bucketAt(-2); }

bucket US_SoluteData::currentBucket() { return bucketAt(-3); }
...
QString US_SoluteData::bucketLine(int ix) {
  ...
  if (attr_x < attr_y)
    return QString("Solute Bin %1: %2=%3, %4=%5, %6=%7, %8=%9")
        .arg(kx)
        .arg(slo)
        .arg(buk.x_min)
        .arg(shi)
...
void US_SoluteData::setDistro(QList<S_Solute>* a_distro, int a_ax, int a_ay,
                              int a_az) {
  distro = a_distro;
  attr_x = a_ax;
  attr_y = a_ay;
...
#define _MIN_VHR_ 0.01
  ...
  while (overs) {
    ...
    for (int jj = 1; jj < nssols;
         jj++) {  // examine each of the current buckets
      qreal horzr;
      qreal vertr;
      buk = buks1->at(jj);
      ...
      for (int kk = 0; kk < jj;
           kk++) {  // compare current to each previous bucket
        ...
        if (cx1 < px2) {  // possible horizontal overlap

          if (cy1 < py2 && cy2 > py2) {  // current overlaps in its lower left
            novls++;
            buk.status = 1;
            buk2 = buk;
            horzr = (px2 - cx1) / wsbuck;
            vertr = (py2 - cy1) / hfbuck;
            if (vertr < horzr) {  // split vertically
              ...
              if (horzr > _MIN_VHR_ && vertr > _MIN_VHR_)
                ...
                DbgLv(2) << "BUCKET TOO THIN H,V " << horzr << "," << vertr;
            ...
            break;
                   ...
                   cy1 < py1) {  // current overlaps in its upper left
            novls++;
            buk.status = 1;
            buk2 = buk;
            horzr = (px2 - cx1) / wsbuck;
            ...
            break;
                   ...
                   cy2 <= py2) {  // current overlaps in its middle left
            novls++;
            buk.status = 1;
            buk.x_min = qMin(px2, cx2);
            buk.x_max = qMax(cx2, px2);
            ...
            break;
                   ...
                   cy1 <
                       py1) {  // current overlaps in both upper and lower left
            novls++;
            buk.status = 1;
            buk2 = buk;
            horzr = (px2 - cx1) / wsbuck;
            vertr = (py2 - py1) / hfbuck;
            if (vertr < horzr) {  // split vertically (into 3 total parts)
            ...
            else {  // split horizontally (into 3 total parts)
        ...
        else if (cx1 == px1) {  // possible pure vertical overlap

          if (cy1 < py1 && cy2 > py1) {  // current overlaps in its upper part
            novls++;
            buk.status = 1;
            buk.y_min = qMin(cy1, py1);
            buk.y_max = qMax(py1, cy1);
    ...
    if (novls >
        0) {  // if there were overlaps, re-sort buckets by vertex for next pass
      qSort(buks1->begin(), buks1->end(), buck_vx_lessthan);
    }

    else if (tstat == 1) {  // start new set of passes where reduced buckets can
                            // be reduced more
      tstat = 4;
    }

    else {  // otherwise, we must be done!
      overs = false;
    }
  }
...
int US_SoluteData::saveGAdata(QString& fname, int xtype, int ytype, int ztype,
                              double fixval) {
  int rc = 0;
  ...
  if (fileo.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    line += " # buckets=" + QString::number(nbuk) + " x=" + cts[xtype] +
            " y=" + cts[ytype] + " fixed=" + cts[ztype] + "=" +
...
int US_SoluteData::buildDataMC() {
  ...
  for (int jj = 0; jj < nbuk; jj++) {
    ...
    for (int kk = 0; kk < nsol;
         kk++) {  // add solute points that fall within bin dimensions
      ssval = distro->at(kk).s;  // "x,y" of solute point
      sfval = distro->at(kk).k;
      if (ssval >= bsmin && ssval <= bsmax && sfval >= bkmin &&
          sfval <= bkmax) {  // solute is in this bin:  save component for bin
...
int US_SoluteData::reportDataMC(QString& fname, int mc_iters) {
  ...
  if (fileo.open(
          QIODevice::WriteOnly |
          QIODevice::Text)) {  // output Monte Carlo statistics to a report file
    QList<qreal> valus;
    QList<qreal> concs;
    QList<qreal> csums;
    QTextStream ts(&fileo);
    ...
    for (int kk = 0; kk < nbuk; kk++) {  // accumulate statistics for each bin
      bcomp = MC_solute.at(kk);
      ts << "\nSolute " << (kk + 1) << ":\n";
      int ksol = bcomp.size();
      vsiz = (double)ksol;

      if (ksol < 1) {  // Summary print for a bin with no solutes
        ts << tr(
            "This solute bin has no solute points at all, "
            "\nso no statistics are available.\n");
      }

      else if (ksol <
               3) {  // Summary prints for a bin that has only a point or two
        ts << tr(
            "This solute bin does not have sufficient points to"
            "\ncalculate meaningful statistics.\n");

        double tconc = 0.0;
        for (int jj = 0; jj < ksol; jj++) tconc += bcomp.at(jj).c;
      ...
      else {  // Summary prints for the typical bin with many points
        qreal vtotal = 0.0;

        concs.clear();
        for (int jj = 0; jj < ksol; jj++) concs.append(bcomp.at(jj).c);

        valus.clear();
        for (int jj = 0; jj < ksol; jj++) valus.append(bcomp.at(jj).w);
        outputStats(ts, valus, concs, false, tr("Molecular weight:          "));

        valus.clear();
        for (int jj = 0; jj < ksol; jj++) valus.append(bcomp.at(jj).s);
        outputStats(ts, valus, concs, false, tr("Sedimentation coefficient: "));

        valus.clear();
        for (int jj = 0; jj < ksol; jj++) valus.append(bcomp.at(jj).d);
        outputStats(ts, valus, concs, false, tr("Diffusion coefficient:     "));

        valus.clear();
        for (int jj = 0; jj < ksol; jj++) valus.append(bcomp.at(jj).f);
        outputStats(ts, valus, concs, false, tr("Frictional ratio:          "));

        valus.clear();
        for (int jj = 0; jj < ksol; jj++) valus.append(bcomp.at(jj).v);
        outputStats(ts, valus, concs, false, tr("Partial specific volume:   "));

        for (int jj = 0; jj < ksol; jj++) vtotal += bcomp.at(jj).c;
        csums.append(vtotal);
        ts << tr("Partial concentration:     ")
           << str1.sprintf(" %6.4e\n", vtotal);
      }
    ...
    for (int jj = 0; jj < distro->size(); jj++) concsum += distro->at(jj).c;

    ts << tr("\nMonte Carlo iterations:   ") << mc_iters;
    if (mc_iters < 2) ts << tr("  (Global or High-Solutes-per-Bucket)");
    ...
    for (int kk = 0; kk < nbuk;
         kk++) {  // output detailed statistics for all the bins
      bcomp = MC_solute.at(kk);
      ts << "\n*****************************************"
            "**********************************";
      ts << "\nSolute " << (kk + 1) << ":";
      ...
      if (ksol < 3) {  // just print the values for a sparse bin
        ts << tr(
            "\nThis solute bin does not have sufficient points to"
            "\ncalculate a meaningful distribution.\n\n");

        ts << tr("Molecular weight:          ");
        for (int jj = 0; jj < ksol; jj++) ts << bcomp.at(jj).w << "  ";
        ts << endl;

        ts << tr("Sedimentation coefficient: ");
        for (int jj = 0; jj < ksol; jj++) ts << bcomp.at(jj).s << "  ";
        ts << endl;

        ts << tr("Diffusion coefficient:     ");
        for (int jj = 0; jj < ksol; jj++) ts << bcomp.at(jj).d << "  ";
        ts << endl;

        ts << tr("Frictional ratio:          ");
        for (int jj = 0; jj < ksol; jj++) ts << bcomp.at(jj).f << "  ";
        ts << endl;

        ts << tr("Partial specific volume:   ");
        for (int jj = 0; jj < ksol; jj++) ts << bcomp.at(jj).v << "  ";
        ts << endl;

        ts << tr("Concentration:             ");
        for (int jj = 0; jj < ksol; jj++) ts << bcomp.at(jj).c << "  ";
        ts << endl;
      }

      else {  // calculate and output detailed statistics for the bin
        concs.clear();
        for (int jj = 0; jj < ksol; jj++) concs.append(bcomp.at(jj).c);

        valus.clear();
        for (int jj = 0; jj < ksol; jj++) valus.append(bcomp.at(jj).w);
        outputStats(ts, valus, concs, true, tr("Molecular Weight"));

        valus.clear();
        for (int jj = 0; jj < ksol; jj++) valus.append(bcomp.at(jj).s);
        outputStats(ts, valus, concs, true, tr("Sedimentation Coefficient"));

        valus.clear();
        for (int jj = 0; jj < ksol; jj++) valus.append(bcomp.at(jj).d);
        outputStats(ts, valus, concs, true, tr("Diffusion Coefficient"));

        valus.clear();
        for (int jj = 0; jj < ksol; jj++) valus.append(bcomp.at(jj).f);
        outputStats(ts, valus, concs, true, tr("Frictional Ratio"));

        valus.clear();
        for (int jj = 0; jj < ksol; jj++) valus.append(bcomp.at(jj).v);
        outputStats(ts, valus, concs, true, tr("Partial Specific Volume"));
      }
    }
...
void US_SoluteData::outputStats(QTextStream& ts, QList<qreal>& vals,
                                QList<qreal>& concs, bool details,
                                QString title) {
  QString str1;
  QString str2;
  int nvals = vals.size();
  ...
  if (details && is_constant) {
    ...
    if (vhi == vlo) {
      ts << tr("Constant Value:            ") << str1.sprintf("%6.4e\n", vhi);
    }

    else {
      ts << tr("Maximum Value:             ") << str1.sprintf("%6.4e\n", vhi);
      ts << tr("Minimum Value:             ") << str1.sprintf("%6.4e\n", vlo);
      ts << tr("(Nearly) Constant Value:   ") << str1.sprintf("%6.4e\n", vmean);
    }
  }
    ...
    ts << tr("Maximum Value:             ") << str1.sprintf("%6.4e\n", vhi);
    ts << tr("Minimum Value:             ") << str1.sprintf("%6.4e\n", vlo);
    ts << tr("Mean Value:                ") << str1.sprintf("%6.4e\n", vmean);
    ts << tr("Median Value:              ") << str1.sprintf("%6.4e\n", vmedi);
    ts << tr("Skew Value:                ") << str1.sprintf("%6.4e\n", skew);
    ts << tr("Kurtosis Value:            ") << str1.sprintf("%6.4e\n", kurto);
    ts << tr("Lower Mode Value:          ") << str1.sprintf("%6.4e\n", mode_lo);
    ts << tr("Upper Mode Value:          ") << str1.sprintf("%6.4e\n", mode_hi);
    ts << tr("Mode Center:               ")
       << str1.sprintf("%6.4e\n", mode_cen);
    ...
    ts << tr("99% Confidence Limits:     ")
       << str1.sprintf("%6.4e, -%6.4e\n", (conf99hi - mode_cen),
                       (mode_cen - conf99lo));
    ts << tr("Standard Deviation:        ") << str1.sprintf("%6.4e\n", sdevi);
    ts << tr("Standard Error:            ") << str1.sprintf("%6.4e\n", sderr);
    ts << tr("Variance:                  ") << str1.sprintf("%6.4e\n", vari);
    ts << tr("Correlation Coefficent:    ") << str1.sprintf("%6.4e\n", corr);
    ts << tr("Number of Bins:            ") << qRound(binsz) << "\n";
    ts << tr("Distribution Area:         ") << str1.sprintf("%6.4e\n", area);
  ...
  else if (is_constant) {  // Summary (where value is constant)
    ts << title << str1.sprintf(" %6.4e (**constant**)\n", vmean);
  }

  else {  // Summary
    ts << title
       << str1.sprintf(" %6.4e (%6.4e, %6.4e)\n", vmean, conf95lo, conf95hi);
  }
}
...
void US_SoluteData::limitBucket(bucket& buk) {
  if (attr_x != US_GA_Initialize::ATTR_D &&
    ...
    if (buk.x_min > 0.0) {  // All-positive s's start at 0.1 at least
...
int US_SoluteData::countOverlaps() {
  ...
  for (int ii = 0; ii < nbuks; ii++) bucket_rects << bucketRect(ii);
  ...
  for (int ii = 0; ii < nbuks; ii++) {
    ...
    for (int jj = ii + 1; jj < nbuks; jj++) {
      if (bukrect.intersects(
              bucket_rects[jj])) {  // Buckets overlap:  test if overlap is
                                    // virtually a line
        ...
        DbgLv(1) << "   buck j" << brj.left() << brj.right() << brj.top()
                 << brj.bottom();
        DbgLv(1) << "   buki" << buki.left() << buki.right() << buki.top()
                 << buki.bottom();
        DbgLv(1) << "   buki left-right" << bwid << "  bottom-top" << bhgt;
        if (bwid < 1e-10 ||
            bhgt < 1e-10) {  // Buckets only share a line:  move a bit
          bucketSeparate(ii, jj, bucket_rects);
        }

        else {  // Overlap is significant:  mark it;
          novlps++;
        }
      }
    }
...
void US_SoluteData::bucketSeparate(int ii, int jj,
  ...
  if (xdiff < 1e-10) {  // Overlap by just vertical line
    double xleft = bukrecj.left() + 1e-6;
    bukrecj.setLeft(xleft);
  }

  if (ydiff < 1e-10) {  // Overlap by just horizontal line
