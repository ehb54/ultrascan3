...
bool
distro_lessthan (const S_Solute &solu1, const S_Solute &solu2)
...
US_GA_Initialize::US_GA_Initialize () : US_Widgets ()
{
  ...
  connect (ct_nisols, SIGNAL (valueChanged (double)), this,
           SLOT (update_nisols (double)));
  ...
  connect (ct_wxbuck, SIGNAL (valueChanged (double)), this,
           SLOT (update_wxbuck (double)));
  ...
  connect (ct_hybuck, SIGNAL (valueChanged (double)), this,
           SLOT (update_hybuck (double)));
  ...
  connect (ct_resolu, SIGNAL (valueChanged (double)), this,
           SLOT (update_resolu (double)));
  ...
  connect (ct_xreso, SIGNAL (valueChanged (double)), this,
           SLOT (update_xreso (double)));
  ...
  connect (ct_yreso, SIGNAL (valueChanged (double)), this,
           SLOT (update_yreso (double)));
  ...
  connect (ct_plxmin, SIGNAL (valueChanged (double)), this,
           SLOT (update_plxmin (double)));

  lb_plxmax = us_label (tr ("Plot Limit s Max:"));
  ct_plxmax = us_counter (3, -100.0, 10000.0, 0.0);
  ct_plxmax->setSingleStep (1);
  connect (ct_plxmax, SIGNAL (valueChanged (double)), this,
           SLOT (update_plxmax (double)));
  ...
  connect (ct_plymin, SIGNAL (valueChanged (double)), this,
           SLOT (update_plymin (double)));
  ...
  connect (ct_plymax, SIGNAL (valueChanged (double)), this,
           SLOT (update_plymax (double)));

  lw_sbin_data = us_listwidget ();
  connect (lw_sbin_data, SIGNAL (pressed (const QModelIndex &)), this,
           SLOT (sclick_sbdata (const QModelIndex &)));
  connect (lw_sbin_data, SIGNAL (doubleClicked (const QModelIndex &)), this,
           SLOT (dclick_sbdata (const QModelIndex &)));
  connect (lw_sbin_data, SIGNAL (currentRowChanged (int)), this,
           SLOT (newrow_sbdata (int)));
  ...
  QBoxLayout *plot
      = new US_Plot (data_plot, tr ("Distribution Data"), xa_title, ya_title);
  rght->addLayout (plot);
...
void
US_GA_Initialize::manDrawSb (void)
{
  ...
  connect (pick, SIGNAL (mouseDown (const QwtDoublePoint &)), this,
           SLOT (getMouseDown (const QwtDoublePoint &)));
  connect (pick, SIGNAL (mouseUp (const QwtDoublePoint &)), this,
           SLOT (getMouseUp (const QwtDoublePoint &)));
...
void
US_GA_Initialize::checkOverlaps (void)
{
  int novlps = soludata->countOverlaps ();
  ...
  if (novlps == 0)
    {
      ...
      pb_save->setEnabled (attr_x == ATTR_V || attr_y == ATTR_V
                           || attr_z == ATTR_V);
      pb_view->setEnabled (true);
    }
...
void
US_GA_Initialize::autoAssignSb (void)
{
...
// plot data 1-D
void
US_GA_Initialize::plot_1dim (void)
{
  dataPlotClear (data_plot);
  data_plot->replot ();
  if (pick != NULL)
...
// plot data 2-D
void
US_GA_Initialize::plot_2dim (void)
{
  dataPlotClear (data_plot);
  data_plot->replot ();
  if (pick != NULL)
  ...
  data_plot->axisTitle (QwtPlot::yRight)
      .setFont (data_plot->axisTitle (QwtPlot::yLeft).font ());
  data_plot->setAxisScale (QwtPlot::xBottom, smin, smax);
...
// plot data 3-D
void
US_GA_Initialize::plot_3dim (void)
{
  QColor cblack (Qt::black);
  QColor cwhite (Qt::white);
  dataPlotClear (data_plot);
  ...
  if (auto_lim)
    ...
    {
      drect = QwtDoubleRect (plxmin, plymin, (plxmax - plxmin),
                             (plymax - plymin));
    }
  ...
  data_plot->axisTitle (QwtPlot::yRight)
      .setFont (data_plot->axisTitle (QwtPlot::yLeft).font ());
  data_plot->setAxisScale (QwtPlot::yRight, spec_dat->range ().minValue (),
...
void
US_GA_Initialize::update_resolu (double dval)
{
  resolu = dval;
}
...
void
US_GA_Initialize::update_xreso (double dval)
{
  xreso = dval;
}
...
void
US_GA_Initialize::update_yreso (double dval)
{
  yreso = dval;
}
...
void
US_GA_Initialize::update_zfloor (double dval)
{
  zfloor = dval;
}
...
void
US_GA_Initialize::update_nisols (double dval)
{
  nisols = qRound (dval);
}
...
void
US_GA_Initialize::update_wxbuck (double dval)
{
  wxbuck = dval;
}
...
void
US_GA_Initialize::update_hybuck (double dval)
{
  hybuck = dval;
}
...
void
US_GA_Initialize::update_plxmin (double dval)
{
  plxmin = dval;
}
...
void
US_GA_Initialize::update_plxmax (double dval)
{
  plxmax = dval;
...
void
US_GA_Initialize::update_plymin (double dval)
{
  plymin = dval;
}
...
void
US_GA_Initialize::update_plymax (double dval)
{
  plymax = dval;
...
void
US_GA_Initialize::select_autolim ()
{
...
void
US_GA_Initialize::select_plot1d ()
{
  plot_dim = 1;
...
void
US_GA_Initialize::select_plot2d ()
{
  plot_dim = 2;
...
void
US_GA_Initialize::select_plot3d ()
{
  plot_dim = 3;
  ck_1dplot->disconnect ();
  ck_2dplot->disconnect ();
...
US_GA_Initialize::load_distro ()
{
  ...
  method
      = mdesc.contains ("-CG") ? method.replace ("2DSA", "2DSA-CG") : method;
      ...
      for (int jj = 0; jj < model.components.size (); jj++)
        {
          ...
          // DbgLv(2) << "Solute jj s w k c d" << jj << sol_s.s << sol_w.s <<
          // sol_s.k
      ...
      for (int jj = 0; jj < sk_distro.size (); jj++)
        {
          DbgLv (2) << " jj" << jj << " s k" << sk_distro[jj].s
                    << sk_distro[jj].k << " w v" << sk_distro[jj].w
                    << sk_distro[jj].v;
        }
    }
...
US_GA_Initialize::set_limits ()
{
  ...
  kmin -= kinc;
  kmax += kinc;
  DbgLv (1) << "SL: adj smin,max" << smin << smax << "kmin,max" << kmin
            << kmax;
  ...
  DbgLv (1) << "SL: autoassn xmin,xmax,ymin,ymax" << plxmin << plxmax << plymin
            << plymax;
  pb_autassb->setEnabled (!monte_carlo);
...
void
US_GA_Initialize::sort_distro (QList<S_Solute> &listsols, bool reduce)
{
...
void
US_GA_Initialize::setBucketPens ()
{
...
void
US_GA_Initialize::highlight_solute (QwtPlotCurve *bc1)
{
  if (bc1 == NULL)
    return;
...
void
US_GA_Initialize::highlight_solute (int sx)
{
...
void
US_GA_Initialize::getMouseDown (const QwtDoublePoint &p)
{
...
void
US_GA_Initialize::getMouseUp (const QwtDoublePoint &p)
{
  double tx[2];
  double ty[2];
  QwtPlotCurve *bc1;
...
QwtPlotCurve *
US_GA_Initialize::drawBucketRect (int sx, QPointF pt1, QPointF pt2)
{
  double tx[5];
  double ty[5];
  QwtPlotCurve *bc1;
...
QwtPlotCurve *
US_GA_Initialize::drawBucketRect (int sx, QRectF rect)
{
...
void
US_GA_Initialize::newrow_sbdata (int /*row*/)
{
...
void
US_GA_Initialize::sclick_sbdata (const QModelIndex &mx)
{
...
// solute bin list row clicked: highlight bucket
void
US_GA_Initialize::sclick_sbdata (const int mrow)
{
  int sx = mrow;
  ...
  if (rtbtn)
    { // Handle right-mouse-button click:  delete bin
      ...
      QMessageBox msgBox;
      QString msg
          = tr ("Are you sure you want to delete solute bin %1 ?").arg (binx);
...
void
US_GA_Initialize::dclick_sbdata (const QModelIndex &mx)
{
  int sx = mx.row ();
  bool global = monte_carlo || manbuks;
  QwtPlotCurve *bc1;
...
void
US_GA_Initialize::changeBucketRect (int sx, QRectF &rect)
{
...
QwtPlotCurve *
US_GA_Initialize::bucketCurveAt (int sx)
{
...
void
US_GA_Initialize::erase_buckets (bool delflag)
{
...
void
US_GA_Initialize::erase_buckets ()
{
  erase_buckets (false);
  return;
}
...
void
US_GA_Initialize::removeSoluteBin (int sx)
{
...
void
US_GA_Initialize::resetPlotAndList (int hlx)
{
...
bool
US_GA_Initialize::equivalent (double a, double b, double eps)
{
...
void
US_GA_Initialize::select_x_axis (int ival)
{
...
void
US_GA_Initialize::select_y_axis (int ival)
{
...
void
US_GA_Initialize::build_xy_distro ()
{
  S_Solute sol_sk;
  S_Solute sol_xy;
  xy_distro.clear ();
...
QString
US_GA_Initialize::anno_title (int pltndx)
{
  QString a_title;
...
void
US_GA_Initialize::load_bins (void)
{
  int hlx = 0;
  pc1 = NULL;
  ...
  QString filter = tr ("Bin GaDistro files (*gadistro.dat);;")
                   + tr ("Any DAT files (*.dat);;") + tr ("Any files (*)");
  QString fname = QFileDialog::getOpenFileName (
      this, tr ("Load Solute Bin (GaDistro) File"), binfpath, filter, 0, 0);
  ...
  DbgLv (1) << "gain:  ld_b : fname attr_x attr_y attr_z" << fname << attr_x
            << attr_y << attr_z;
...
QwtLinearColorMap *
US_GA_Initialize::ColorMapCopy (QwtLinearColorMap *colormap)
{
