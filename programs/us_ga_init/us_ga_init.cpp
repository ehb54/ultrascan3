...
void US_GA_Initialize::save(void) {
  ...
  if (attr_x != ATTR_V && attr_y != ATTR_V &&
      attr_z !=
          ATTR_V) {  // None of X,Y,fixed is vbar:  there can be no output file
  ...
  if (runid.startsWith("Global-")) runid = runid.mid(7);
  ...
  if (!dirp.exists(fdir)) dirp.mkpath(fdir);

  if (!dirp.exists(fdir2)) dirp.mkpath(fdir2);
...
void US_GA_Initialize::checkOverlaps(void) {
  ...
  if (novlps == 0) {
    QMessageBox::information(this, tr("No Bins Overlap"),
...
void US_GA_Initialize::autoAssignSb(void) {
  ...
  for (int jj = 0; jj < nibuks;
       jj++) {  // draw the auto-assigned buckets and add lines to list widget
...
void US_GA_Initialize::replot_data() {
  if (sdistro->isEmpty() || sdistro->size() == 0) return;
...
void US_GA_Initialize::plot_1dim(void) {
  ...
  for (int jj = 1; jj < dsize; jj++) {
    ...
    if (equivalent(sval, svpr,
                   epsilon)) {  // effectively equal s values: sum c values
      cval += cvpr;
...
void US_GA_Initialize::update_resolu(double dval) { resolu = dval; }

// update raster x resolution
void US_GA_Initialize::update_xreso(double dval) { xreso = dval; }

// update raster y resolution
void US_GA_Initialize::update_yreso(double dval) { yreso = dval; }

// update Z (frequency) floor percent
void US_GA_Initialize::update_zfloor(double dval) { zfloor = dval; }

// update number of initial solutes
void US_GA_Initialize::update_nisols(double dval) { nisols = qRound(dval); }

// update width in s of buckets
void US_GA_Initialize::update_wxbuck(double dval) { wxbuck = dval; }

// update height in f/f0 of buckets
void US_GA_Initialize::update_hybuck(double dval) { hybuck = dval; }

// update plot limit s min
void US_GA_Initialize::update_plxmin(double dval) { plxmin = dval; }
...
void US_GA_Initialize::update_plymin(double dval) { plymin = dval; }
...
void US_GA_Initialize::load_distro() {
  ...
  if (attr_x != ATTR_V && attr_y != ATTR_V &&
      attr_z !=
          ATTR_V) {  // No attribute is vbar, so use vbar as the default X or Y
    int attrv = ATTR_V;
...
void US_GA_Initialize::set_limits() {
  ...
  QString hmsg =
      tr("Now either auto-assign the solute bins, or manually select bins"
...
QwtPlotCurve* US_GA_Initialize::drawBucketRect(int sx, QPointF pt1,
                                               QPointF pt2) {
  double tx[5];
  double ty[5];
  QwtPlotCurve* bc1;
...
QwtPlotCurve* US_GA_Initialize::bucketCurveAt(int sx) {
  ...
  for (int jj = 0; jj < ilist.size();
       jj++) {  // test each item for Curve type and matching title
    QwtPlotCurve* bc1 = (QwtPlotCurve*)ilist.at(jj);

    if (bc1->rtti() ==
        QwtPlotItem::Rtti_PlotCurve) {  // right type,  so check title
      QString itext = bc1->title().text();

      if (itext.compare(ctext) ==
          0) {  // this is the one we want, return a pointer to the curve
        return bc1;
      }
    }
  }
...
void US_GA_Initialize::select_prefilt(void) {
  ...
  if (nruns == 0)
    ...
    pfmsg =
        tr("RunID prefilter - 1 run: ") + QString(pfilts[0]).left(8) + "...";
