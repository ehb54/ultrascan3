...
class bucket {
   public:
      ...
      bool operator==(const bucket &objIn) {
         return (
            x == objIn.x && x_min == objIn.x_min && x_max == objIn.x_max && y == objIn.y && y_min == objIn.y_min
            && y_max == objIn.y_max && conc == objIn.conc && status == objIn.status);
      }
      bool operator!=(const bucket &objIn) {
         return (
            x != objIn.x || x_min != objIn.x_min || x_max != objIn.x_max || y != objIn.y || y_min != objIn.y_min
...
class US_SoluteData : public QObject {
   ...
   public slots:
      ...
      int loadGAdata(QString &, int * = NULL, int * = NULL, int * = NULL, double * = NULL);
