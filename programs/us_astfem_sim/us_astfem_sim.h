...
public:
  ...
  //! \returns int: 0 on success no gui needed, 1 on success but gui needed, -1
  //! on error
...
private:
  ...
  bool time_correctionFlag; //!< Flag to apply time correction - Currently not
  ...
  QString imageName; //!< Full path template for movie frames
                     //!< ({imagedir}/frame{image_count}.png)
  QString tmst_tfpath; //!< Path to the timestate of the simulation in a
                       //!< temporary location
  ...
  QCheckBox
      *ck_timeCorr; //!< Pointer to QCheckbox for applying time correction
  ...
  US_AstfemMath::AstFemParameters
      af_params; //!< Fixed grid simulation parameters used for exporting
  QVector<US_DataIO::RawData>
      sim_datas; //!< Vector of simulated data for each speedstep
