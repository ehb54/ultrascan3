...
US_auditTrailGMP::US_auditTrailGMP () : US_Widgets ()
{
  ...
  QLabel *bn_loadGMPReport
      = us_banner (tr ("Select GMP Report for Audit Trail:"), 1);
...
// view report
void
US_auditTrailGMP::viewAPDF (void)
{
...
// Load GMP Run
void
US_auditTrailGMP::loadGMPReport (void)
{
  US_Passwd pw;
  ...
  QMap<QString, QString> p_details
      = read_autoflow_record (gmpRunID_eSign.toInt ());
...
int
US_auditTrailGMP::list_all_gmp_reports_db (
  ...
  while (db->next ())
    {
      ...
      if (jsonDocRevList.isArray () && jsonDocOperList.isArray ()
          && !operatorListJson.isEmpty () && !reviewersListJson.isEmpty ())
        {
          ...
          // case of SEPARATE const. if ( operatorListJson.contains(
          // logged_user ) ||
...
US_auditTrailGMP::read_autoflowGMPReportEsign_record (QString aID)
{
...
// slot to..
void
US_auditTrailGMP::initPanel_auto (QMap<QString, QString> &protocol_details)
{
  ...
  // QLabel* bn_viewAPDF = us_banner( tr( "View .PDF of the Audit Trail for the
  // Currenlty Loaded GMP Run:" ), 1 );
  pb_viewAPDF = us_pushbutton (
      tr ("View .PDF of the Audit Trail for the Currenlty Loaded GMP Run:"));
  ...
  eSignTree->setStyleSheet (
      "QTreeWidget { font: bold; font-size: "
      + QString::number (2 * sfont.pointSize ())
  ...
  uInteractionsTree->setStyleSheet (
      "QTreeWidget { font: bold; font-size: "
      + QString::number (2 * sfont.pointSize ())
      + "pt;}  QTreeView { alternate-background-color: yellow;} "
        "QTreeView::item:hover { border: black;  border-radius:1px;  "
...
QGroupBox *
US_auditTrailGMP::createGroup_eSign (QString name)
{
...
QVector<QGroupBox *>
US_auditTrailGMP::createGroup_stages (QString name, QString s_name)
{
  ...
  if (s_name == "GMP Run Initiation")
    {
      ...
      QLineEdit *le_name = us_lineedit (status_map["Person"]["lname"] + ","
                                            + status_map["Person"]["fname"],
                                        0, true);
      QLineEdit *le_email
          = us_lineedit (status_map["Person"]["email"], 0, true);
      QLineEdit *le_level
          = us_lineedit (status_map["Person"]["level"], 0, true);
      ...
      // QSpacerItem* spacer2 = new QSpacerItem( 20, 3*ihgt,
      // QSizePolicy::Expanding); genL2->setRowStretch( 1, 1 ); genL2->addItem(
      // spacer2,  row++,  0, 1, 6 );
      ...
      groupBox->setStyleSheet (
          "QGroupBox { font: bold;  background-color: qlineargradient(x1: 0, "
          "y1: 0, x2: 0, y2: 1, stop: 0 #E0E0E0, stop: 1 #FFFFFF); border: "
          "2px solid gray; border-radius: 10px; margin-top: 20px; "
          "margin-bottom: 10px; padding-top: 5px; } QGroupBox::title { "
          "subcontrol-origin: margin; subcontrol-position: top left; left: "
          "10px; margin: 0 5px; background-color: black; color: white; "
          "padding: 0 3px;}  QGroupBox::indicator { width: 13px; height: "
          "13px; border: 1px solid grey; background-color: rgba(204, 204, "
          "204, 255);} QGroupBox::indicator:hover {background-color: "
          "rgba(235, 235, 235, 255);} QLabel {background-color: "
          "rgb(105,105,105);}");
      ...
      if (stopOptimaJson.isEmpty () && skipOptimaJson.isEmpty ())
        {
          ...
          html_assembled
              += tr ("<tr><td> There were NO remote operations. </td></tr>");
          html_assembled += tr ("</table>");
          ...
          QLabel *lb_remote = us_label (tr (
              "Remote Operations on Optima Instrument: SKIP stage, STOP:"));
          lb_remote->setSizePolicy (QSizePolicy::Preferred,
                                    QSizePolicy::Fixed);
          row = 0;
          ...
          groupBox->setStyleSheet (
              "QGroupBox { font: bold;  background-color: qlineargradient(x1: "
              "0, y1: 0, x2: 0, y2: 1, stop: 0 #E0E0E0, stop: 1 #FFFFFF); "
              "border: 2px solid gray; border-radius: 10px; margin-top: 20px; "
              "margin-bottom: 10px; padding-top: 5px; } QGroupBox::title { "
              "subcontrol-origin: margin; subcontrol-position: top left; "
              "left: 10px; margin: 0 5px; background-color: black; color: "
              "white; padding: 0 3px;}  QGroupBox::indicator { width: 13px; "
              "height: 13px; border: 1px solid grey; background-color: "
              "rgba(204, 204, 204, 255);} QGroupBox::indicator:hover "
              "{background-color: rgba(235, 235, 235, 255);} QLabel "
      ...
      else
        {
          ...
          for (im = operation_types_live_update.begin ();
               im != operation_types_live_update.end (); ++im)
            {
              ...
              QLabel *lb_level = us_label (tr ("Level:"));
              QLineEdit *le_ID
                  = us_lineedit (status_map["Person"]["ID"], 0, true);
              QLineEdit *le_name
                  = us_lineedit (status_map["Person"]["lname"] + ","
                                     + status_map["Person"]["fname"],
                                 0, true);
              QLineEdit *le_email
                  = us_lineedit (status_map["Person"]["email"], 0, true);
              ...
              lb_time_o->setSizePolicy (QSizePolicy::Preferred,
                                        QSizePolicy::Fixed);
              QLabel *lb_time_o1 = us_label (tr ("Type:"));
              QLineEdit *le_time_o1 = us_lineedit (
                  status_map["Remote Operation"]["type"], 0, true);
              QLabel *lb_time_o2 = us_label (tr ("Performed on:"));
              QLineEdit *le_time_o2 = us_lineedit (
                  operation_types_live_update_ts[im.key ()] + " (UTC)", 0,
                  true);
              ...
              lb_comm->setSizePolicy (QSizePolicy::Preferred,
                                      QSizePolicy::Fixed);
              QLabel *lb_comm1 = us_label (tr ("Comment:"));
              ...
              groupBox->setStyleSheet (
                  "QGroupBox { font: bold;  background-color: "
                  "qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 "
                  "#E0E0E0, stop: 1 #FFFFFF); border: 2px solid gray; "
                  "border-radius: 10px; margin-top: 20px; margin-bottom: "
                  "10px; padding-top: 5px; } QGroupBox::title { "
                  "subcontrol-origin: margin; subcontrol-position: top left; "
                  "left: 10px; margin: 0 5px; background-color: black; color: "
                  "white; padding: 0 3px;}  QGroupBox::indicator { width: "
                  "13px; height: 13px; border: 1px solid grey; "
                  "background-color: rgba(204, 204, 204, 255);} "
                  "QGroupBox::indicator:hover {background-color: rgba(235, "
                  "235, 235, 255);} QLabel {background-color: "
                  "rgb(105,105,105);}");
    ...
    {
      html_assembled += tr ("<h3 align=left>Reference Scan Determination, "
                            "Triples Dropped, Data Saving (3. IMPORT)</h3>");
      ...
      for (im = data_types_import.begin (); im != data_types_import.end ();
           ++im)
        {
          QString json_str = im.value ();
          ...
          lb_time_ref->setSizePolicy (QSizePolicy::Preferred,
                                      QSizePolicy::Fixed);
          ...
          QLineEdit *le_time_ref2 = us_lineedit (
              data_types_import_ts[im.key ()] + " (UTC)", 0, true);
          ...
          us_setReadOnly (te_comm1, true);
          QString t_comment
              = status_map["Comment when SAVED"]["comment_when_saved"]
                        .isEmpty ()
                    ? "N/A"
                    : status_map["Comment when SAVED"]["comment_when_saved"];
          te_comm1->setText (t_comment);
          ...
          if (!dtype_opt_dropped_triples.isEmpty ())
            {
              html_assembled
                  += tr ("<table style=\"margin-left:10px\">"
                         "<caption style=\"color:red;\" align=left> "
                         "<b><i>List of Dropped Triples: </i></b> </caption>"
                         "</table>"
              ...
              lb_drop->setSizePolicy (QSizePolicy::Preferred,
                                      QSizePolicy::Fixed);
              row = 0;
              ...
              te_dropped_tr->setFont (
                  QFont (US_Widgets::fixedFont ().family (),
                         US_GuiSettings::fontSize () - 1));
          ...
          if (status_map.contains ("Dropped"))
            {
              ...
              QLabel *lb_drop_c = us_label (
                  tr ("Comments [Dropped Triple(s) | Channel(s)]:"));
              lb_drop_c->setSizePolicy (QSizePolicy::Preferred,
                                        QSizePolicy::Fixed);
              row = 0;
    ...
    {
      html_assembled += tr ("<h3 align=left>Meniscus Position Determination, "
                            "Edit Profiles Saving (4. EDITING)</h3>");
      ...
      for (im = data_types_edit.begin (); im != data_types_edit.end (); ++im)
        {
          ...
          QLineEdit *le_ts1 = us_lineedit (
              data_types_edit_ts[im.key ()] + " (UTC)", 0, true);
    ...
    {
      if (expType == "VELOCITY")
        user_interactions_analysis (name, analysisJson, analysisCancelJson,
                                    groupBoxes);
      else if (expType == "ABDE")
        user_interactions_analysis_abde (name, analysisABDEJson,
                                         analysisABDEts, groupBoxes);
    }

  else if (s_name == "E-SIGNATURES")
    {
      html_assembled += tr ("<h3 align=left>Information on Reassigning "
                            "Reviewers (E-SIGNING)</h3>");

      // take all info from  eSign_details_auto[ "createUpdateLogJson" ];
      status_map = parse_autoflowStatus_json (
          eSign_details_auto["createUpdateLogJson"], "");
      ...
      if (status_map.isEmpty ())
        {
          ...
          QLabel *lb_remote = us_label (
              tr ("Reassignments of Operator(s), Reviewer(s), Approver(s):"));
          lb_remote->setSizePolicy (QSizePolicy::Preferred,
                                    QSizePolicy::Fixed);
          row = 0;
          genL1->addWidget (lb_remote, row++, 0, 1, 22);
          ...
          for (int i = 0; i < s_map_keys.size (); ++i)
            {
              ...
              if (c_repls.contains ("replacement:"))
                {
                  ...
                  for (int j = 0; j < c_repls_list.size (); ++j)
                    c_repls_str
                        += c_repls_list[j].replace ("replacement:", ": ")
                           + "<br>";
                }
              else
                c_repls_str = c_repls;
              ...
              lb_time_o->setSizePolicy (QSizePolicy::Preferred,
                                        QSizePolicy::Fixed);
              QLabel *lb_time_o2 = us_label (tr ("Performed on:"));
              QLineEdit *le_time_o2
                  = us_lineedit (admin_map["timeDate"], 0, true);
              ...
              // Set GroupBox
              QString gBox_name
                  = "Reviewers Update: " + QString::number (i + 1);
              QGroupBox *groupBox = new QGroupBox (gBox_name);
              QPalette p = groupBox->palette ();
              p.setColor (QPalette::Dark, Qt::white);
              groupBox->setPalette (p);
...
void
US_auditTrailGMP::user_interactions_analysis_abde (
    QString name, QString analysisABDEJson, QString analysisABDEts,
    QVector<QGroupBox *> &groupBoxes)
{
  html_assembled
  ...
  QLabel *lb_comm
      = us_label (tr ("Comment at the Time of ABDE Profile Processing:"));
  lb_comm->setSizePolicy (QSizePolicy::Preferred, QSizePolicy::Fixed);
  QLabel *lb_comm1 = us_label (tr ("Comment:"));
...
void
US_auditTrailGMP::user_interactions_analysis (QString name,
                                              ...
                                              QVector<QGroupBox *> &groupBoxes)
{
  ...
  QLabel *lb_men
      = us_label (tr ("Meniscus Position Determination at FITMEN Stage:"));
  lb_men->setSizePolicy (QSizePolicy::Preferred, QSizePolicy::Fixed);
  int row = 0;
  ...
  if (!p_2dsa_auto_fitmen) // interactive FITMEN (manual)
  ...
  else // automatic mode
    {
      ...
      te_fitmen->setText (
          "Meniscus positions have been determined automatically as best fit "
  ...
  // assemble html
  assemble_GMP_analysis_cancelled (analysisCancel_status_map,
                                   analysisCancelJson);
  html_assembled += tr ("<hr>");
}
...
QMap<QString, QString>
US_auditTrailGMP::read_autoflow_record (int autoflowID)
{
  // Check DB connection
  US_Passwd pw;
...
void
US_auditTrailGMP::read_reportLists_from_aprofile (
...
QStringList
US_auditTrailGMP::buildDroppedTriplesList (
...
// Read AProfile's reportIDs per channel:
bool
US_auditTrailGMP::readReportLists (
...
bool
US_auditTrailGMP::read_2dsa_settings (QXmlStreamReader &xmli)
{
  while (!xmli.atEnd ())
    {
      QString ename = xmli.name ().toString ();
          ...
          if (ename == "job_fitmen" || ename == "job_fitmen_auto")
            {
              QXmlStreamAttributes attr = xmli.attributes ();
              p_2dsa_run_fitmen
                  = US_Util::bool_flag (attr.value ("run").toString ());
              p_2dsa_auto_fitmen
                  = attr.value ("interactive").toString ().toInt () == 0;

              qDebug () << "In 2DSA's Settings run_fitmen, auto_fitmen -- "
                        << p_2dsa_run_fitmen << p_2dsa_auto_fitmen;
            }
        }
...
void
US_auditTrailGMP::read_autoflowStatus_record (
    ...
    QString &analysisABDEJson, QString &analysisABDEts)
{
  ...
  qDebug () << "Read_autoflow_status: stopOptimaJson, skipOptimaJson -- "
            << stopOptimaJson << skipOptimaJson;
}

// Parse autoflowStatus RI/IP Json
QMap<QString, QMap<QString, QString> >
US_auditTrailGMP::parse_autoflowStatus_json (QString statusJson, QString dtype)
{
  QMap<QString, QMap<QString, QString> > status_map;
          ...
          for (int i = 0; i < json_array.size (); ++i)
            {
              foreach (const QString &array_key,
                       json_array[i].toObject ().keys ())
                {
                  admin_info_map[array_key] = json_array[i]
                                                  .toObject ()
                                                  .value (array_key)
                                                  .toString ();
                  qDebug () << "Admin update Map: -- key, value: " << array_key
                            << json_array[i]
                                   .toObject ()
                                   .value (array_key)
                                   .toString ();
...
QMap<QString, QString>
US_auditTrailGMP::parse_autoflowStatus_analysis_json (QString statusJson)
{
...
void
US_auditTrailGMP::display_reviewers_auto (int &row,
  ...
  if (eSign_d.contains (JsonListName))
    {
      ...
      QJsonDocument jsonDoc_signed
          = QJsonDocument::fromJson (eSign_d["eSignStatusJson"].toUtf8 ());
      if (!jsonDoc_signed.isObject ())
        {
          qDebug () << "json_to_qmap in display_reviewers_aut(): "
                       "eSignStatusJson: NOT a JSON Doc !!";
          return;
        }
          ...
          for (int i = 0; i < esigned_array.size (); ++i)
            {
              foreach (const QString &key,
                       esigned_array[i].toObject ().keys ())
                {
                  if (key == current_reviewer)
                    {
                      QJsonObject newObj = esigned_array[i]
                                               .toObject ()
                                               .value (key)
                                               .toObject ();
...
QLineEdit *
US_auditTrailGMP::check_eSign_status_for_gmpReport_auto (
...
void
US_auditTrailGMP::reset_panel (void)
{
...
bool
US_auditTrailGMP::mkdir (const QString &baseDir, const QString &subdir)
{
  QDir folder (baseDir);
...
// GMP init html
void
US_auditTrailGMP::assemble_GMP_init (
    QMap<QString, QMap<QString, QString> > status_map_c,
    QString createdGMPrunts)
{
  // html_assembled += tr("<hr>");
  html_assembled
...
// LIVE UPDATE html
void
US_auditTrailGMP::assemble_GMP_live_update (
    QMap<QString, QMap<QString, QString> > status_map, QString dtype_opt,
    QString oper_ts)
{
  html_assembled += tr ("<table>"
                        "<tr>"
...
// IMPORT html
void
US_auditTrailGMP::assemble_GMP_import (
    QMap<QString, QMap<QString, QString> > status_map, QString dtype_opt,
    QString oper_ts)
{
...
// EDITING html
void
US_auditTrailGMP::assemble_GMP_editing (
    QMap<QString, QMap<QString, QString> > status_map, QString dtype_opt,
    QString oper_ts)
{
  ...
  // Edit Profiles Saved:
  html_assembled += tr ("<table style=\"margin-left:10px\">"
                        "<caption align=left> <b><i>Edit Profiles Saved at: "
                        "</i></b> </caption>"
                        "</table>"
...
void
US_auditTrailGMP::assemble_GMP_analysis_fitmen (
    QMap<QString, QString> analysis_status_map)
{
  html_assembled
      += tr ("<table style=\"margin-left:10px\">"
...
void
US_auditTrailGMP::assemble_GMP_analysis_cancelled (
    QMap<QString, QString> analysisCancel_status_map,
    QString analysisCancelJson)
{
  html_assembled += tr ("<table style=\"margin-left:10px\">"
...
void
US_auditTrailGMP::assemble_GMP_esign_reassign (
    QMap<QString, QMap<QString, QString> > status_map)
{
  if (status_map.isEmpty ())
    {
      html_assembled += tr ("<table>");
      html_assembled += tr (
          "<tr><td> There were NO reviewer(s) reassignments. </td></tr>");
      html_assembled += tr ("</table>");
      ...
      if (c_repls.contains ("replacement:"))
        {
          ...
          for (int j = 0; j < c_repls_list.size (); ++j)
            c_repls_str
                += c_repls_list[j].replace ("replacement:", ": ") + "<br>";
        }
      else
        c_repls_str = c_repls;
...
// initHTML
void
US_auditTrailGMP::initHTML (void)
{
  QString rptpage;
  ...
  QString html_title
      = tr ("<h2 align=center>Audit Trail for GMP Run: <br><i>%1</i></h2>"
            "<hr>")
            .arg (gmpRunName_passed) // 1
      ;

  html_assembled += rptpage + html_title;
}

// close HTML
void
US_auditTrailGMP::closeHTML (void)
{
...
// eSigs HTML
void
US_auditTrailGMP::assemble_esigs (QMap<QString, QString> esigs_html)
{
  ...
  html_assembled_esigs
      += tr ("<h3 style=\"margin-left:10px\">%1</h3>").arg (name_c);

  QString eStatus;
  if (esigs_html["Status"].contains ("NOT"))
    eStatus = "<td style=\"color:red;\"><b><i>" + esigs_html["Status"]
              + "</i></b></td>";
  else
    eStatus = "<td style=\"color:green;\"><b><i>" + esigs_html["Status"]
              + "</i></b></td>";

  html_assembled_esigs += tr ("<table style=\"margin-left:30px\">"
