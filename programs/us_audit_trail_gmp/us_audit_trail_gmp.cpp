...
void US_auditTrailGMP::initPanel_auto(QMap<QString, QString> &protocol_details) {
   ...
   eSignTree->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() + 1));
   ...
   eSignTree->setStyleSheet(
      "QTreeWidget { font: bold; font-size: " + QString::number(2 * sfont.pointSize())
   ...
   uInteractionsTree->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() + 1));
...
QVector<QGroupBox *> US_auditTrailGMP::createGroup_stages(QString name, QString s_name) {
   ...
   if (s_name == "GMP Run Initiation") {
      ...
      QLineEdit *le_name
         = us_lineedit(status_map[ "Person" ][ "lname" ] + "," + status_map[ "Person" ][ "fname" ], 0, true);
      ...
      te_comm1->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 1));
      ...
      groupBox->setStyleSheet(
         "QGroupBox { font: bold;  background-color: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 #E0E0E0, "
         "stop: 1 #FFFFFF); border: 2px solid gray; border-radius: 10px; margin-top: 20px; margin-bottom: 10px; "
         "padding-top: 5px; } QGroupBox::title { subcontrol-origin: margin; subcontrol-position: top left; left: 10px; "
         "margin: 0 5px; background-color: black; color: white; padding: 0 3px;}  QGroupBox::indicator { width: 13px; "
         "height: 13px; border: 1px solid grey; background-color: rgba(204, 204, 204, 255);} "
         "QGroupBox::indicator:hover {background-color: rgba(235, 235, 235, 255);} QLabel {background-color: "
         "rgb(105,105,105);}");
      ...
      if (stopOptimaJson.isEmpty() && skipOptimaJson.isEmpty()) {
         ...
         te_remote->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 1));
         ...
         groupBox->setStyleSheet(
            "QGroupBox { font: bold;  background-color: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 #E0E0E0, "
            "stop: 1 #FFFFFF); border: 2px solid gray; border-radius: 10px; margin-top: 20px; margin-bottom: 10px; "
            "padding-top: 5px; } QGroupBox::title { subcontrol-origin: margin; subcontrol-position: top left; left: "
            "10px; margin: 0 5px; background-color: black; color: white; padding: 0 3px;}  QGroupBox::indicator { "
            "width: 13px; height: 13px; border: 1px solid grey; background-color: rgba(204, 204, 204, 255);} "
            "QGroupBox::indicator:hover {background-color: rgba(235, 235, 235, 255);} QLabel {background-color: "
            "rgb(105,105,105);}");
         ...
         for (im = operation_types_live_update.begin(); im != operation_types_live_update.end(); ++im) {
            ...
            lb_comm->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Fixed);
            QLabel *lb_comm1 = us_label(tr("Comment:"));
            QString t_comment = status_map[ "Comment" ][ "comment" ].isEmpty() ? "N/A"
            ...
            groupBox->setStyleSheet(
               "QGroupBox { font: bold;  background-color: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 "
               "#E0E0E0, stop: 1 #FFFFFF); border: 2px solid gray; border-radius: 10px; margin-top: 20px; "
               "margin-bottom: 10px; padding-top: 5px; } QGroupBox::title { subcontrol-origin: margin; "
               "subcontrol-position: top left; left: 10px; margin: 0 5px; background-color: black; color: white; "
               "padding: 0 3px;}  QGroupBox::indicator { width: 13px; height: 13px; border: 1px solid grey; "
               "background-color: rgba(204, 204, 204, 255);} QGroupBox::indicator:hover {background-color: rgba(235, "
               "235, 235, 255);} QLabel {background-color: rgb(105,105,105);}");
      ...
      html_assembled += tr(
         "<h3 align=left>Reference Scan Determination, Triples Dropped, Data Saving (3. IMPORT)</h3>");
      ...
      for (im = data_types_import.begin(); im != data_types_import.end(); ++im) {
         ...
         if (!dtype_opt_dropped_triples.isEmpty()) {
            ...
            te_dropped_tr->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 1));
         ...
         if (status_map.contains("Dropped")) {
            html_assembled += tr("<table style=\"margin-left:10px\">"
                                 "<caption align=left> <b><i>Comments on [triples | channels | select channel] "
                                 "dropped: </i></b> </caption>"
                                 "</table>"
            ...
            te_drop_c1->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 1));
...
void US_auditTrailGMP::user_interactions_analysis_abde(
   QString name, QString analysisABDEJson, QString analysisABDEts, QVector<QGroupBox *> &groupBoxes) {
...
void US_auditTrailGMP::user_interactions_analysis(
   QString name, QString analysisJson, QString analysisCancelJson, QVector<QGroupBox *> &groupBoxes) {
   ...
   else //automatic mode
   {
      ...
      te_fitmen->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 1));
   ...
   else //no cancelled jobs
   {
      ...
      te_canceled->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 1));
   ...
   while (!xmli.atEnd()) {
      ...
      if (xmli.isStartElement()) {
         if (ename == "job_fitmen" || ename == "job_fitmen_auto") {
            ...
            qDebug() << "In 2DSA's Settings run_fitmen, auto_fitmen -- " << p_2dsa_run_fitmen << p_2dsa_auto_fitmen;
         }
      }
...
void US_auditTrailGMP::read_autoflowStatus_record(
   ...
   qDebug() << "Read_autoflow_status: stopOptimaJson, skipOptimaJson -- " << stopOptimaJson << skipOptimaJson;
}
...
void US_auditTrailGMP::display_reviewers_auto(
   int &row, QMap<QString, QString> eSign_d, QString JsonListName, QGridLayout *genL) {
   if (eSign_d.contains(JsonListName)) {
      ...
      for (int i = 0; i < jsonDocOperList_array.size(); ++i) {
         ...
         te_comment->setFont(QFont(US_Widgets::fixedFont().family(), US_GuiSettings::fontSize() - 1));
...
void US_auditTrailGMP::assemble_GMP_live_update(
   QMap<QString, QMap<QString, QString>> status_map, QString dtype_opt, QString oper_ts) {
   html_assembled += tr("<table>"
                        "<tr>"
...
void US_auditTrailGMP::assemble_GMP_analysis_cancelled(
   QMap<QString, QString> analysisCancel_status_map, QString analysisCancelJson) {
   html_assembled += tr("<table style=\"margin-left:10px\">"
...
void US_auditTrailGMP::initHTML(void) {
   ...
   QString html_title = tr("<h2 align=center>Audit Trail for GMP Run: <br><i>%1</i></h2>"
                           "<hr>")
...
void US_auditTrailGMP::assemble_esigs(QMap<QString, QString> esigs_html) {
   ...
   html_assembled_esigs += tr("<h3 style=\"margin-left:10px\">%1</h3>").arg(name_c);
