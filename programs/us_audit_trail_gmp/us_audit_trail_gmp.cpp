...
void US_auditTrailGMP::printAPDF(void) {
  ...
  filePath_pdf =
      US_Settings::reportDir() + "/" + subDirName + "/" + subDirName + ".pdf";
...
int US_auditTrailGMP::list_all_gmp_reports_db(
  ...
  while (db->next()) {
    ...
    if (jsonDocRevList.isArray() && jsonDocOperList.isArray() &&
      ...
      // of SEPARATE const. if ( operatorListJson.contains( logged_user ) ||
...
QMap<QString, QString> US_auditTrailGMP::read_autoflowGMPReportEsign_record(
    QString aID) {
...
// slot to..
void US_auditTrailGMP::initPanel_auto(
...
QVector<QGroupBox*> US_auditTrailGMP::createGroup_stages(QString name,
                                                         QString s_name) {
  ...
  if (s_name == "GMP Run Initiation") {
    ...
    QLineEdit* le_name = us_lineedit(
        status_map["Person"]["lname"] + "," + status_map["Person"]["fname"], 0,
        true);
    ...
    if (stopOptimaJson.isEmpty() && skipOptimaJson.isEmpty()) {
      ...
      QLabel* lb_remote = us_label(
          tr("Remote Operations on Optima Instrument: SKIP stage, STOP:"));
      ...
      groupBox->setStyleSheet(
          ...
          "y1: 0, x2: 0, y2: 1, stop: 0 #E0E0E0, stop: 1 #FFFFFF); border: 2px "
          "solid gray; border-radius: 10px; margin-top: 20px; margin-bottom: "
          "10px; padding-top: 5px; } QGroupBox::title { subcontrol-origin: "
          "margin; subcontrol-position: top left; left: 10px; margin: 0 5px; "
          "background-color: black; color: white; padding: 0 3px;}  "
          "QGroupBox::indicator { width: 13px; height: 13px; border: 1px solid "
          "grey; background-color: rgba(204, 204, 204, 255);} "
          "QGroupBox::indicator:hover {background-color: rgba(235, 235, 235, "
      ...
      for (im = operation_types_live_update.begin();
        ...
        QLineEdit* le_ID = us_lineedit(status_map["Person"]["ID"], 0, true);
        QLineEdit* le_name = us_lineedit(
            status_map["Person"]["lname"] + "," + status_map["Person"]["fname"],
            0, true);
        QLineEdit* le_email =
        ...
        QLineEdit* le_time_o1 =
            us_lineedit(status_map["Remote Operation"]["type"], 0, true);
        QLabel* lb_time_o2 = us_label(tr("Performed on:"));
        QLineEdit* le_time_o2 = us_lineedit(
            operation_types_live_update_ts[im.key()] + " (UTC)", 0, true);
    ...
    for (im = data_types_import.begin(); im != data_types_import.end(); ++im) {
      ...
      QLineEdit* le_time_ref2 =
          us_lineedit(data_types_import_ts[im.key()] + " (UTC)", 0, true);
      ...
      us_setReadOnly(te_comm1, true);
      QString t_comment =
          status_map["Comment when SAVED"]["comment_when_saved"].isEmpty()
              ? "N/A"
      ...
      if (status_map.contains("Dropped")) {
        ...
        QLabel* lb_drop_c =
            us_label(tr("Comments [Dropped Triple(s) | Channel(s)]:"));
      ...
      if (genL41 != NULL) genL_sec_row->addLayout(genL41);
      if (genL51 != NULL) genL_sec_row->addLayout(genL51);
    ...
    html_assembled +=
        tr("<h3 align=left>Meniscus Position Determination, Edit Profiles "
           "Saving (4. EDITING)</h3>");
    ...
    for (im = data_types_edit.begin(); im != data_types_edit.end(); ++im) {
      ...
      QLineEdit* le_ts1 =
          us_lineedit(data_types_edit_ts[im.key()] + " (UTC)", 0, true);
    ...
    if (expType == "VELOCITY")
      ...
      user_interactions_analysis_abde(name, analysisABDEJson, analysisABDEts,
                                      groupBoxes);
  }

  else if (s_name == "E-SIGNATURES") {
    html_assembled += tr(
        "<h3 align=left>Information on Reassigning Reviewers (E-SIGNING)</h3>");
...
void US_auditTrailGMP::user_interactions_analysis_abde(
...
void US_auditTrailGMP::user_interactions_analysis(
    QString name, QString analysisJson, QString analysisCancelJson,
    QVector<QGroupBox*>& groupBoxes) {
  html_assembled +=
...
void US_auditTrailGMP::read_reportLists_from_aprofile(
...
QStringList US_auditTrailGMP::buildDroppedTriplesList(
...
// Read AProfile's reportIDs per channel:
bool US_auditTrailGMP::readReportLists(
...
void US_auditTrailGMP::read_autoflowStatus_record(
...
QMap<QString, QString> US_auditTrailGMP::parse_autoflowStatus_analysis_json(
    QString statusJson) {
...
void US_auditTrailGMP::display_reviewers_auto(int& row,
  ...
  if (eSign_d.contains(JsonListName)) {
    ...
    if (!jsonDoc_signed.isObject()) {
      qDebug() << "json_to_qmap in display_reviewers_aut(): eSignStatusJson: "
                  "NOT a JSON Doc !!";
      return;
    }
...
QLineEdit* US_auditTrailGMP::check_eSign_status_for_gmpReport_auto(
...
// GMP init html
void US_auditTrailGMP::assemble_GMP_init(
...
// LIVE UPDATE html
void US_auditTrailGMP::assemble_GMP_live_update(
...
// IMPORT html
void US_auditTrailGMP::assemble_GMP_import(
    QMap<QString, QMap<QString, QString>> status_map, QString dtype_opt,
    QString oper_ts) {
  html_assembled += tr("<table>"
                       "<tr>"
...
// EDITING html
void US_auditTrailGMP::assemble_GMP_editing(
    QMap<QString, QMap<QString, QString>> status_map, QString dtype_opt,
    QString oper_ts) {
  html_assembled += tr("<table>"
                       "<tr>"
  ...
  html_assembled +=
      tr("<table style=\"margin-left:10px\">"
         "<caption align=left> <b><i>Meniscus Position Determination: </i></b> "
         "</caption>"
         "</table>"
  ...
  QMap<QString, QString>::iterator mp;
  for (mp = status_map["Meniscus"].begin(); mp != status_map["Meniscus"].end();
       ++mp) {
    html_assembled += tr("<tr>"
                         "<td> Channel:  %1 </td>"
                         "<td> Type:     %2 </td>"
...
void US_auditTrailGMP::assemble_GMP_analysis_fitmen(
    QMap<QString, QString> analysis_status_map) {
  html_assembled +=
      tr("<table style=\"margin-left:10px\">"
...
void US_auditTrailGMP::assemble_GMP_analysis_cancelled(
    QMap<QString, QString> analysisCancel_status_map,
...
void US_auditTrailGMP::assemble_GMP_esign_reassign(
    QMap<QString, QMap<QString, QString>> status_map) {
  if (status_map.isEmpty()) {
    html_assembled += tr("<table>");
    html_assembled +=
        tr("<tr><td> There were NO reviewer(s) reassignments. </td></tr>");
    html_assembled += tr("</table>");
