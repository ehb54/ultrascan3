...
//! \brief Contains the declaration of the US_auditTrailGMP class and its
//! members.
...
private:
  ...
  US_SelectItem
      *pdiag_autoflow_db;    //!< Dialog for selecting autoflow from database.
  ...
  QMap<QString, QString>
      eSign_details_auto;  //!< Details of eSign for auto section.
...
private slots:
  ...
  void user_interactions_analysis (QString, QString, QString,
                                   QVector<QGroupBox *> &);
  void user_interactions_analysis_abde (QString, QString, QString,
                                        QVector<QGroupBox *> &);
  ...
  void display_reviewers_auto (int &index, QMap<QString, QString> details,
                               QString role, QGridLayout *layout);
  ...
  QLineEdit *
  check_eSign_status_for_gmpReport_auto (QString id,
  ...
  void read_autoflowStatus_record (
      QString &status, QString &runID, QString &protocolName,
      QString &investigatorName, QString &expType, QString &date,
      QString &filename, QString &projectName, QString &solutionName,
      QString &bufferName, QString &analyteName, QString &gradientName,
      QString &hardwareName, QString &aprofileName, QString &aprofileGUID,
      QString &dataSource, QString &, QString &);
  ...
  parse_autoflowStatus_json (const QString json, const QString key);
  ...
  void read_reportLists_from_aprofile (QStringList &reportList,
  ...
  bool readReportLists (QXmlStreamReader &xml, QMap<QString, QString> &reports,
                        QMap<QString, QString> &methods);

  //! \brief Read the 2DSA settings from XML.
  //! \param xml The XML stream reader.
  //! \return True if the 2DSA settings were read successfully, false
  //! otherwise.
  ...
  QStringList buildDroppedTriplesList (US_DB2 *db,
                                       QMap<QString, QString> details);

  //! \brief Assemble the initial GMP data.
  //! \param statusMap The map of the status.
  //! \param runID The run ID.
  void assemble_GMP_init (QMap<QString, QMap<QString, QString> > statusMap,
                          QString runID);
  ...
  void
  assemble_GMP_live_update (QMap<QString, QMap<QString, QString> > statusMap,
                            QString runID, QString status);

  //! \brief Assemble the GMP import data.
  //! \param statusMap The map of the status.
  //! \param runID The run ID.
  //! \param status The status of the GMP.
  void assemble_GMP_import (QMap<QString, QMap<QString, QString> > statusMap,
                            QString runID, QString status);
  ...
  //! \param runID The run ID.
  //! \param status The status of the GMP.
  void assemble_GMP_editing (QMap<QString, QMap<QString, QString> > statusMap,
                             QString runID, QString status);
  ...
  //! \param details The details map.
  //! \param status The status of the GMP.
  void assemble_GMP_analysis_cancelled (QMap<QString, QString> details,
                                        QString status);

  //! \brief Assemble the GMP eSign reassign data.
  //! \param statusMap The map of the status.
  void assemble_GMP_esign_reassign (
      QMap<QString, QMap<QString, QString> > statusMap);
