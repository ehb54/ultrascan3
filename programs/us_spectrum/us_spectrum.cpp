...
US_Spectrum::US_Spectrum () : US_Widgets ()
{
  ...
  pb_reset_basis
      = us_pushbutton (tr ("Reset Basis Spectra / Reset Fit Results"));
  ...
  connect (lw_basis, SIGNAL (itemDoubleClicked (QListWidgetItem *)),
           SLOT (deleteBasisCurve ()));
  ...
  plotLayout1 = new US_Plot (data_plot, tr (""), tr ("Wavelength(nm)"),
                             tr ("Extinction"));
  ...
  plotLayout2 = new US_Plot (residuals_plot, tr (""), tr ("Wavelength(nm)"),
                             tr ("Extinction"));
...
US_Spectrum::load_basis ()
{
  ...
  dialog.setNameFilter (
      tr ("Text Files (*.txt *.csv *.dat *.wa *.dsp);;All Files (*)"));
      ...
      for (int ii = 0; ii < files.size (); ii++)
        {
          ...
          US_CSV_Loader *csv_loader
              = new US_CSV_Loader (filepath, note, editing, this);
          int state = csv_loader->exec ();
          if (state == QDialog::Rejected)
            {
              int check = QMessageBox::question (
                  this, "Warning!",
                  "You canceled parsing a file.\n" + filepath
                      + "\nDo you want to continue loading the rest of the "
                        "file(s)?");
              ...
              if (csv_data.columnCount () < 2)
                {
                  int check = QMessageBox::question (
                      ...
                      "This file does not have two data columns:\n" + filepath
                          + "\nDo you want to continue loading the rest of "
                            "the file(s)?");
                  if (check == QMessageBox::No)
                    {
                      return;
          ...
          for (int jj = 1; jj < data_list.at (ii).columnCount (); jj++)
            {
              ...
              std::pair<double *, double *> minmax
                  = std::minmax_element (xvals.begin (), xvals.end ());
...
// Takes the information in the basis vector to plot all of the curves for the
// basis spectrums
void
US_Spectrum::plot_basis ()
{
  // QStringList names;
...
void
US_Spectrum::load_target ()
{
  QString filter
      = tr ("Text Files (*.txt *.csv *.dat *.wa *.dsp);;All Files (*)");
  QString fpath
      = QFileDialog::getOpenFileName (this, "Load The Target Spectrum",
  ...
  if (csv_data.columnCount () < 2)
    {
      QMessageBox::warning (
          this, "Error!",
          "Data files must have two columns of wavelength and OD values!");
      return;
    }
...
void
US_Spectrum::new_value (const QwtDoublePoint &p)
{
      ...
      for (int m = 0; m < v_basis.size (); m++)
        {
          if (lw_basis->currentItem () == NULL)
            return;
          if (v_basis.at (m).header.compare (lw_basis->currentItem ()->text ())
              == 0)
            {
              basisIndex = m;
            }
        }
      if (specified_wavelength < v_basis.at (basisIndex).lambda_max
          && specified_wavelength > v_basis.at (basisIndex).lambda_min)
        le_extinction->setText (QString::number (
            v_basis.at (basisIndex)
                .extinction.at (specified_wavelength
                                - v_basis.at (basisIndex).lambda_min)));
    }
  else if (cb_spectrum_type->currentText ().compare ("target") == 0)
    {
      if (specified_wavelength < w_target.lambda_max
          && specified_wavelength > w_target.lambda_min)
        le_extinction->setText (QString::number (w_target.extinction.at (
            specified_wavelength - w_target.lambda_min)));
    }
  else
    {
      if (specified_wavelength < w_solution.lambda_max
          && specified_wavelength > w_solution.lambda_min)
        le_extinction->setText (QString::number (w_solution.extinction.at (
            specified_wavelength - w_solution.lambda_min)));
    }
}
void
US_Spectrum::findExtinction ()
{
  unsigned int specified_wavelength = 0;
...
US_Spectrum::fit ()
{
  ...
  for (i = 0; i < (unsigned int)v_basis.size (); i++)
    {
      if (v_basis[i].lambda_min != min_lambda
          || v_basis[i].lambda_max != max_lambda
          || v_basis[i].wvl.size () != w_target.wvl.size ())
        {
  ...
  nnls_a
      = new double[points * order]; // contains the model functions, end-to-end
  ...
  nnls_wp = new double[order]; // pre-allocated working space for nnls, On
                               // exit, wp[] will contain the dual solution
                               // vector, wp[i]=0.0 for all i in set p and
                               // wp[i]<=0.0 for all i in set z.
  ...
  US_Math2::nnls (nnls_a, points, points, order, nnls_b, nnls_x, &nnls_rnorm,
                  nnls_wp, nnls_zzp, nnls_indexp);
  ...
  for (i = 0; i < (unsigned int)v_basis.size (); i++)
    {
      ...
      // str.sprintf((v_basis[i].filenameBasis +": %3.2f%%
      // (%6.4e)").toLocal8Bit().data(), results[i], nnls_x[i]);
      // lw_basis->item((int)i)->setText(str);
      lw_basis->item ((int)i)->setText (str.arg (v_basis[i].header)
                                            .arg (results.at (i), 0, 'f', 2)
                                            .arg (nnls_x[i], 0, 'e'));
...
bool
US_Spectrum::deleteBasisCurve (void)
{
  QMessageBox mBox;
  mBox.setText (
...
US_Spectrum::overlap ()
{
  ...
  qDebug () << "Mins: " << lambdaMins
            << ", highest_lambda_min: " << highest_lambda_min;
  qDebug () << "Maxs: " << lambdaMaxs
            << ", lowest_lambda_max : " << lowest_lambda_max;

  // we need a vector of all wavelengths which will be decimated based on
  // overlap with target/basis
...
US_Spectrum::findAngles ()
{
  ...
  if (v_basis[indexOne].extinction.size ()
      != v_basis[indexTwo].extinction.size ())
    {
      QMessageBox::warning (this, tr ("UltraScan Warning"), str,
                            QMessageBox::Ok, QMessageBox::NoButton,
                            QMessageBox::NoButton);
  ...
  for (int i = 0; i < v_basis[indexOne].extinction.size (); i++)
    {
      dotproduct
          += v_basis[indexOne].extinction[i] * v_basis[indexTwo].extinction[i];
...
US_Spectrum::save ()
{
  QString basename = QFileDialog::getSaveFileName (
      this, "Set the Base Name for the 'CSV' and 'DAT' Files",
  ...
  if (!csv_data.setData (csvfile, header, columns))
    {
      QMessageBox::warning (this, "Error!",
                            "Error in making the csv data:\n\n"
                                + csv_data.error ());
    }
  if (!csv_data.writeFile (seprtr))
    {
      QMessageBox::warning (this, "Error!",
                            "Error in writing the csv file:\n\n"
                                + csv_data.error ());
    }
  if (flag)
    {
...
US_Spectrum::load ()
{
  ...
  // "/home/minji/ultrascan/results", "*.spectrum_fit");

  QString filename = QFileDialog::getOpenFileName (
      this, "Load File", US_Settings::resultDir (), "*.spectrum_fit");
  if (filename.isEmpty ())
    return;
