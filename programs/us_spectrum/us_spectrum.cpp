...
US_Spectrum::US_Spectrum() : US_Widgets() {
  ...
  plotLayout1 =
      new US_Plot(data_plot, tr(""), tr("Wavelength(nm)"), tr("Extinction"));
...
void US_Spectrum::new_value(const QwtDoublePoint& p) {
  ...
  if (cb_spectrum_type->currentText().compare("basis") == 0) {
    ...
    for (int m = 0; m < v_basis.size(); m++) {
      if (lw_basis->currentItem() == NULL) return;
    ...
    if (specified_wavelength < w_target.lambda_max &&
      ...
      le_extinction->setText(QString::number(
          w_target.extinction.at(specified_wavelength - w_target.lambda_min)));
  } else {
...
void US_Spectrum::fit() {
  ...
  for (i = 0; i < (unsigned int)v_basis.size(); i++) {
    if (v_basis[i].lambda_min != min_lambda ||
      ...
      QMessageBox::warning(this, tr("UltraScan Warning"), str, QMessageBox::Ok,
...
void US_Spectrum::overlap() {
  ...
  for (int i = 0; i < allRange.size(); ++i) {
    ...
    for (int j = 0; j < w_target.wvl.size(); ++j) {
      if (allRange[i] == w_target.wvl.at(j)) exists_target = 1;
    }
    if (!exists_target) {
      allRange.remove(i);
      --i;
  ...
  for (int j = 0; j < v_basis.size(); ++j) {
    for (int i = 0; i < allRange.size(); ++i) {
      ...
      for (int m = 0; m < v_basis.at(j).wvl.size(); ++m) {
        if (allRange[i] == v_basis.at(j).wvl.at(m)) exists_basis = 1;
      }
      if (!exists_basis) {
        allRange.remove(i);
        --i;
  ...
  for (int i = 0; i < v_basis.size(); ++i) {
    for (int j = 0; j < v_basis.at(i).wvl.size(); ++j) {
      ...
      for (int m = 0; m < allRange.size(); ++m) {
        if (v_basis.at(i).wvl.at(j) == allRange[m]) exists_basis = 1;
      }
      if (!exists_basis) {
  ...
  for (int j = 0; j < w_target.wvl.size(); ++j) {
    ...
    exists_target = 0;
    for (int m = 0; m < allRange.size(); ++m) {
      if (w_target.wvl.at(j) == allRange[m]) exists_target = 1;
    }
    if (!exists_target) {
...
void US_Spectrum::findAngles() {
  ...
  for (int k = 0; k < v_basis.size(); k++) {
    if (firstProf.compare(v_basis[k].header) == 0) indexOne = k;
    if (secondProf.compare(v_basis[k].header) == 0) indexTwo = k;
  }
...
void US_Spectrum::save() {
  ...
  if (!csv_data.setData(csvfile, header, columns)) {
    QMessageBox::warning(
        this, "Error!", "Error in making the csv data:\n\n" + csv_data.error());
  }
