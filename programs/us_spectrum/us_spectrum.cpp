   ...
   if (cb_spectrum_type->currentText().compare("basis") == 0) {
      ...
      if (specified_wavelength < v_basis.at(basisIndex).lambda_max && specified_wavelength > v_basis.at(basisIndex).lambda_min)
         le_extinction->setText(QString::number(
            v_basis.at(basisIndex).extinction.at(specified_wavelength - v_basis.at(basisIndex).lambda_min)));
   }
...
void US_Spectrum::fit() {
   ...
   for (i = 0; i < ( unsigned int ) v_basis.size(); i++) {
      if (
         v_basis[ i ].lambda_min != min_lambda || v_basis[ i ].lambda_max != max_lambda
         || v_basis[ i ].wvl.size() != w_target.wvl.size()) {
         QMessageBox::warning(
            this, tr("UltraScan Warning"), str, QMessageBox::Ok, QMessageBox::NoButton, QMessageBox::NoButton);
         return;
      }
   }
   ...
   nnls_wp = new double
      [ order ]; // pre-allocated working space for nnls, On exit, wp[] will contain the dual solution vector, wp[i]=0.0 for all i in set p and wp[i]<=0.0 for all i in set z.
   ...
   for (i = 0; i < ( unsigned int ) v_basis.size(); i++) {
      ...
      lw_basis->item(( int ) i)->setText(
         str.arg(v_basis[ i ].header).arg(results.at(i), 0, 'f', 2).arg(nnls_x[ i ], 0, 'e'));
...
void US_Spectrum::save() {
   QString basename = QFileDialog::getSaveFileName(
      this, "Set the Base Name for the 'CSV' and 'DAT' Files", US_Settings::resultDir(), "All Files (*)");
   if (basename.isEmpty()) {
      return;
   }
