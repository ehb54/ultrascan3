...
US_2dsa::US_2dsa() : US_AnalysisBase2() {
   ...
   te_status->setText(tr("Solution not initiated...\n"
                         "RMSD:  0.000000,\n"
...
void US_2dsa::analysis_done(int updflag) {
   if (updflag == (-1)) { // fit has been aborted or reset for new fit
      pb_view->setEnabled(false);
      pb_save->setEnabled(false);
      pb_plt3d->setEnabled(false);
      pb_pltres->setEnabled(false);
   ...
   if (updflag == (-2)) { // update model,noise lists and RMSD
      models << model;
      ...
      DbgLv(0) << "Analysis Done VARI" << vari << "model,noise counts" << models.count()
               << (ti_noises.count() + ri_noises.count()) << "menisc bott"
   ...
   DbgLv(1) << "  edat0 sdat0 rdat0 tnoi0" << edata->value(0, 0) << sdata.value(0, 0) << rdata.value(0, 0)
   ...
   if (plotdata) { // Plot 3D and Residuals
      open_3dplot();
      open_resplot();
   }

   else if (savedata) { // Save the data and reports
      save();
   }
   ...
   if (nmodels > 1 && fitMeni) {
      ...
      for (int ii = 0; ii < nmodels; ii++) {
         ...
         double vari = mdesc.mid(mdesc.indexOf("VARI=") + 5).section(" ", 0, 0).toDouble();
         double rmsd = sqrt(vari);

         if (rmsd < b_rmsd) {
            b_rmsd = rmsd;
            b_meni = mdesc.mid(mdesc.indexOf("MENISCUS=") + 9).section(" ", 0, 0).toDouble();
            b_bott = mdesc.mid(mdesc.indexOf("BOTTOM=") + 7).section(" ", 0, 0).toDouble();
            DbgLv(1) << "FitMens Done:    b_ rmsd,meni,bott" << b_rmsd << b_meni << b_bott << "  ix" << (ii + 1);
         }
      }
      DbgLv(1) << "FitMens Done: BEST rmsd,meniscus,bottom" << b_rmsd << b_meni << b_bott;
   }
}
...
void US_2dsa::load(void) {
   ...
   if (loadDB) { // Fetch the speed steps for the experiment from the database
      US_Passwd pw;
      ...
      if (dbP->lastErrno() == US_DB2::OK) {
         ...
         if (speed_steps.count() > 0)
            DbgLv(1) << "2dsa_load:  speed step_ w2tfirst w2tlast timefirst timelast" << speed_steps[ 0 ].w2t_first
                     << speed_steps[ 0 ].w2t_last << speed_steps[ 0 ].time_first << speed_steps[ 0 ].time_last;
         ...
         DbgLv(0) << "2DS:LD: newfile" << newfile << "idExp" << idExp << "tmst_fpath" << tmst_fpath;
      }
   }

   else { // Read run experiment file and parse out speed steps
      QString expfpath = directory + "/" + runID + "." + edata->dataType + ".xml";
   ...
   DbgLv(1) << "2dsa_no. of speeds " << nssp << "no of scans " << nssc << " triple_count= " << lw_triples->count();
   ...
   for (int js = 0; js < edata->scanCount(); js++) {
      DbgLv(1) << " 2dsa: js" << js << "secs" << edata->scanData[ js ].seconds << "speed" << edata->scanData[ js ].rpm;
   }
   //*DEBUG*
}
...
void US_2dsa::data_plot(void) {
   ...
   if (!dataLoaded || sdata.scanData.size() != edata->scanData.size()) {
   ...
   for (int ii = 0; ii < nscans; ii++) {
      ...
      for (int jj = 0; jj < npoints; jj++) {
         ...
         va[ jj ] = edata->value(ii, jj) - sdata.value(ii, jj) - tinoi - rinoi;
         vari += sq(va[ jj ]);
         kntva++;
      }
...
void US_2dsa::save(void) {
   QString analysisDate = QDateTime::currentDateTime().toUTC().toString("yyMMddhhmm");
   QString reqGUID = US_Util::new_guid();
   QString runID = edata->runID;
   QString editID = (edata->editID.length() == 12) ? edata->editID.mid(2) : edata->editID;
   ...
   QString analysisType = QString(cusGrid ? "2DSA-CG" : "2DSA") + QString(fitMeni ? "-FM" : "")
                          + QString(fitBott ? "-FB" : "") + QString(fitMeBo ? "-FMB" : "")
                          + QString(refIter ? "-IT" : "") + QString(montCar ? "-MC" : "");
   ...
   int knois = min(ti_noise.count, 1) + min(ri_noise.count, 1); // noise files per model
   ...
   while (indx > 0) { // build a list of available model file names
      mname = "M" + QString().sprintf("%07i", indx++) + ".xml";
      if (!mdnams.contains(mname)) { // no name with this index exists, so add it new-name list
         mnames << mname;
         if (++kmodels >= nmodels)
            break;
      }
   ...
   while (indx > 0) { // build a list of available noise file names
      nname = "N" + QString().sprintf("%07i", indx++) + ".xml";
      if (!ndnams.contains(nname)) { // add to the list of new-name noises
         nnames << nname;
         if (++knoises >= nnoises)
            break;
      }
   ...
   for (int jj = 0; jj < nmodels; jj++) { // loop to output models and noises
      model = models[ jj ]; // model to output
      QString mdesc = model.description; // description from processor
      double variance = mdesc.mid(mdesc.indexOf("VARI=") + 5).section(' ', 0, 0).toDouble();
      ...
      if (montCar)
         ...
         meniscus = mdesc.mid(mdesc.indexOf("MENISCUS=") + 9).section(' ', 0, 0).toDouble();
         bottom = mdesc.mid(mdesc.indexOf("BOTTOM=") + 7).section(' ', 0, 0).toDouble();
         if (bottom > 0.0)
            iterID.sprintf("i%02d-m%05db%05d", iterNum, qRound(meniscus * 10000), qRound(bottom * 10000));
         else
      ...
      mname = montCar ? tmppath + mname.replace(".model", ".mdl.tmp") : mdlpath + mnames[ jj ];
   ...
   if (montCar) { // For Monte Carlo, create a composite of MC iteration files
      QString tname = US_Model::composite_mc_file(tnames, true);
      tnames[ 0 ] = tname;
      QFile(tname).rename(mname); // Move/rename to model directory

      if (dbP != NULL) { // If DB, load and store in the database
         model.load(mname);
         ...
         for (int ii = 0; ii < sdata.scanCount(); ii++) {
            ...
            for (int jj = 0; jj < sdata.pointCount(); jj++) { // Calculate the residuals and the RMSD
   ...
   else { // For non-MC, save the model name for message to follow
      tnames.clear();
      tnames << mname;
   }
   ...
   QString fitFile = filebase + (fitBott ? "fitbot.dat" : "fitmen.dat");
   QString fresFile = respath + QString(fitFile).section("/", -1, -1);
   QString dsinfFile = QString(filebase).replace(analynode, "/dsinfo.") + "dataset_info.html";
   ...
   if (nmodels > 1 && !montCar) {
      ...
      wmsg = wmsg + " ...  ( " + QString::number(kk) + tr(" files unshown )\n");
      kk = nmodels - 1;
      mname = mdlpath + mnames[ kk ]; // list last model file
      wmsg = wmsg + mname + "\n";

      if (knois > 0) { // list last noise file(s)
         kk *= knois;
         nname = noipath + nnames[ kk++ ];
         wmsg = wmsg + nname + "\n";
...
QVector<double> *US_2dsa::mw_Anorm() {
   return &normvA;
}
...
void US_2dsa::open_fitcntl() {
   ...
   sd.manual = manual;
   US_Math2::data_correction(avTemp, sd);
   DbgLv(0) << "2DSA s_corr D_corr" << sd.s20w_correction << sd.D20w_correction << "manual" << sd.manual << "vbar20"
            << vbar20;
   DbgLv(0) << "2DSA d_corr v vW vT d dW dT" << sd.viscosity << sd.viscosity_wt << sd.viscosity_tb << sd.density
            << sd.density_wt << sd.density_tb;
   US_Passwd pw;
   loadDB = disk_controls->db();
   US_DB2 *dbP = loadDB ? new US_DB2(pw.getPasswd()) : NULL;
   ...
   edata->bottom = (edata->bottom > 0.0) ? edata->bottom : 0.0;
   ...
   if (check_file.exists() && check_file.isFile()) {
      ...
      DbgLv(1) << "2dsa : timestate file exists" << tmst_fpath << " timestateobject = " << dset.simparams.tsobj
   ...
   if (need_tsfile) {
      ...
      dset.simparams.simSpeedsFromTimeState(tmst_fpath);
      DbgLv(1) << "2dsa : timestate newly created.  timestateobject = " << dset.simparams.tsobj
               << "exp_steps=" << exp_steps << "sspknt" << dset.simparams.sim_speed_prof.count();
   }
   ...
   DbgLv(1) << "2dsa : ssck: rspeed accel1 tf_aend tf_scan" << rspeed << accel1 << tf_aend << tf_scan << "accel2"
   ...
   dset.manual = manual;
   DbgLv(1) << "Bottom" << dset.simparams.bottom << "rotorcoeffs" << dset.simparams.rotorcoeffs[ 0 ]
...
QString US_2dsa::distrib_info() {
   ...
   if (maDesc.startsWith(runID)) { // Saved model:  get analysis description from model description
      maDesc = maDesc.section(".", -2, -2).section("_", 1, -1);
   }

   else { // No saved model yet:  compose analysis description
      QString adate = "a" + QDateTime::currentDateTime().toUTC().toString("yyMMddhhmm");
      bool cusGrid = model.description.contains("CUSTOMGRID");
      bool refIter = model.description.contains("REFITERS");
      bool fitMeni = (model.global == US_Model::MENISCUS);
      bool fitBott = (model.global == US_Model::BOTTOM);
      ...
      bool montCar = model.monteCarlo;
      DbgLv(1) << "2DSA:SV: cusGrid" << cusGrid << "desc" << model.description;
      QString anType = QString(cusGrid ? "2DSA-CG" : "2DSA") + QString(fitMeni ? "-FM" : "")
                       + QString(fitBott ? "-FB" : "") + QString(fitMeBo ? "-FMB" : "") + QString(refIter ? "-IT" : "")
                       + QString(montCar ? "-MC" : "");
   ...
   mstr += table_row(tr("Model Analysis:"), maDesc);
   mstr += table_row(tr("Number of Components:"), QString::number(ncomp));
   mstr += table_row(tr("Residual RMS Deviation:"), QString::number(rmsd));
   ...
   bool cnstvb = ((maxk - mink) / qAbs(maxk) > (maxv - minv) / qAbs(maxv));
   ...
   if (cnstvb)
      mstr += table_row(tr("Constant Vbar at 20") + DEGC + ":", QString().number(maxv));
   else
      mstr += table_row(tr("Constant f/f0:"), QString().number(maxk));
   ...
   if (cnstvb)
      mstr += table_row(
         tr("Molecular Wt."), tr("S Apparent"), tr("S 20,W"), tr("D Apparent"), tr("D 20,W"), tr("f / f0"),
         tr("Concentration"));
   else
      mstr += table_row(
         tr("Molecular Wt."), tr("S Apparent"), tr("S 20,W"), tr("D Apparent"), tr("D 20,W"), tr("Vbar20"),
         tr("Concentration"));
   ...
   sd.vbar = vbartb;
   sd.manual = manual;
   DbgLv(1) << "Data_Corr manual" << sd.manual << "avTemp" << avTemp << "vbar20" << sd.vbar20 << "vbart" << sd.vbar;
   US_Math2::data_correction(avTemp, sd);
   ...
   for (int ii = 0; ii < ncomp; ii++) {
      ...
      mstr += table_row(
         QString().sprintf("%10.4e", model.components[ ii ].mw), QString().sprintf("%10.4e", s_ap),
         QString().sprintf("%10.4e", model.components[ ii ].s), QString().sprintf("%10.4e", D_ap),
         QString().sprintf("%10.4e", model.components[ ii ].D), QString().sprintf("%10.4e", f_f0),
...
QString US_2dsa::iteration_info() {
   ...
   QString mstr = "\n" + indent(4) + tr("<h3>Multiple Model Settings:</h3>\n") + indent(4) + "<table>\n";

   mstr += table_row(tr("Number of Model Iterations:"), QString::number(nmodels));
   ...
   if (fitMeni) {
      ...
      mstr += table_row(tr("Meniscus Range:"), QString::number(rmenis));
      mstr += table_row(tr("Base Experiment Meniscus:"), QString::number(bmenis));
      mstr += table_row(tr("Start Fit Meniscus:"), QString::number(smenis));
      mstr += table_row(tr("End Fit Meniscus:"), QString::number(emenis));
   }
   ...
   if (montCar)
      mstr += table_row(tr("Iteration"), tr("Iteration ID"), tr("RMSD"));

   else
      mstr += table_row(tr("Iteration"), tr("Meniscus"), tr("Bottom"), tr("RMSD"));
   ...
   for (int ii = 0; ii < nmodels; ii++) {
      ...
      if (montCar) {
         ...
         QString ameni = mdesc.mid(mdesc.indexOf("MENISCUS=") + 9).section(" ", 0, 0);
         QString abott = mdesc.mid(mdesc.indexOf("BOTTOM=") + 7).section(" ", 0, 0);
...
void US_2dsa::write_report(QTextStream &ts) {
   ts << html_header(QString("US_2dsa"), tr("2-Dimensional Spectrum Analysis"), edata);
   ts << distrib_info();
   ts << iteration_info();
   ts << indent(2) + "</body>\n</html>\n";
}
...
void US_2dsa::write_bmap(const QString plotFile) {
   ...
   for (int ii = 0; ii < nscans; ii++) {
      ...
      for (int jj = 0; jj < npoints; jj++) {
         ...
         resscn[ jj ] = edata->value(ii, jj) - sdata.value(ii, jj) - rnoi - tnoi;
      }
...
#else
   QPixmap pixmap = QPixmap::grabWidget(resbmap, 0, 0, resbmap->width(), resbmap->height());
#endif
...
QString US_2dsa::fit_meniscus_data() {
   ...
   bool usemen = (QString(models[ 0 ].description).indexOf("MENISCUS=") > 0);
   bool usebot = (QString(models[ 0 ].description).indexOf("BOTTOM=") > 0);
   ...
   for (int ii = 0; ii < nmodels; ii++) {
      ...
      if (usemen && usebot) {
         ...
         DbgLv(1) << "fitmdat:  ii" << ii << "meni bott rmsd" << ameni << abott << armsd;
      }
      else if (usemen) {
         QString ameni = mdesc.mid(mdesc.indexOf("MENISCUS=") + 9).section(" ", 0, 0);
         mstr += (ameni + " " + armsd + "\n");
         DbgLv(1) << "fitmdat:  ii" << ii << "meni rmsd" << ameni << armsd;
      }
      else if (usebot) {
         QString abott = mdesc.mid(mdesc.indexOf("BOTTOM=") + 7).section(" ", 0, 0);
         mstr += (abott + " " + armsd + "\n");
         DbgLv(1) << "fitbdat:  ii" << ii << "bott rmsd" << abott << armsd;
      }
   }
