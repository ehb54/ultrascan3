...
void
US_2dsa::analysis_done (int updflag)
{
  if (updflag == (-1))
  ...
  if (updflag == (-2))
    { // update model,noise lists and RMSD
      ...
      DbgLv (0)
          << "Analysis Done VARI" << vari << "model,noise counts"
          ...
          if (rmsd < b_rmsd)
            {
              ...
              b_bott = mdesc.mid (mdesc.indexOf ("BOTTOM=") + 7)
                           .section (" ", 0, 0)
                           .toDouble ();
              DbgLv (1) << "FitMens Done:    b_ rmsd,meni,bott" << b_rmsd
                        << b_meni << b_bott << "  ix" << (ii + 1);
            }
        }
      DbgLv (1) << "FitMens Done: BEST rmsd,meniscus,bottom" << b_rmsd
                << b_meni << b_bott;
    }
}
...
US_2dsa::load (void)
{
  ...
  if (loadDB)
      ...
      if (dbP->lastErrno () == US_DB2::OK)
        {
          ...
          if (speed_steps.count () > 0)
            DbgLv (1) << "2dsa_load:  speed step_ w2tfirst w2tlast timefirst "
                         "timelast"
    ...
    { // Read run experiment file and parse out speed steps
      QString expfpath
          = directory + "/" + runID + "." + edata->dataType + ".xml";
          ...
          for (int ds = 0; ds < lw_triples->count (); ds++)
            {
              ...
              if (etm1 < stm1 || etm2 > stm2)
                { // Data times beyond speed step ranges, so flag use of data
                  // ranges
                  DbgLv (1) << "2dsa: Data is beyond range"
                            << "limits from scans" << etm1 << etm2 << ds;
                  DbgLv (1)
                      << "2dsa: Data is beyond range"
                      << "limits from speed_profs " << stm1 << stm2 << ds;
                  // nssp = 0 ;
                  // speed_steps.clear();
                  break;
                }
      ...
      for (int jj = 0; jj < npoints; jj++)
        {
          ...
          va[jj]
              = edata->value (ii, jj) - sdata.value (ii, jj) - tinoi - rinoi;
          vari += sq (va[jj]);
          kntva++;
        }
...
US_2dsa::save (void)
{
  ...
  QString editID = (edata->editID.length () == 12) ? edata->editID.mid (2)
                                                   : edata->editID;
  ...
  for (int jj = 0; jj < nmodels; jj++)
    {                                    // loop to output models and noises
      ...
      // e.g.,"i01" normally; "i03-m60190" for meniscus; "mc017" for monte
      // carlo
      QString iterID = "i01";
      int iterNum = jj + 1;
      ...
      if (montCar)
      ...
      else if (fitMeni)
        {
          ...
          bottom = mdesc.mid (mdesc.indexOf ("BOTTOM=") + 7)
                       ...
                       .toDouble ();
          if (bottom > 0.0)
            iterID.sprintf ("i%02d-m%05db%05d", iterNum,
                            qRound (meniscus * 10000),
                            qRound (bottom * 10000));
          else
          ...
          for (int ii = 0; ii < sdata.scanCount (); ii++)
            {
              ...
              for (int jj = 0; jj < sdata.pointCount (); jj++)
                { // Calculate the residuals and the RMSD
                  ...
                  double diff = edata->value (ii, jj) - sdata.value (ii, jj)
                                - rin - tin;
                  variance += sq (diff);
                  kpts++;
                }
...
US_2dsa::distrib_info ()
{
  ...
  if (maDesc.startsWith (runID))
    ...
    { // No saved model yet:  compose analysis description
      ...
      bool fitMeBo = (model.global == US_Model::MENIBOTT);
      bool montCar = model.monteCarlo;
      DbgLv (1) << "2DSA:SV: cusGrid" << cusGrid << "desc"
                << model.description;
      QString anType
          = QString (cusGrid ? "2DSA-CG" : "2DSA")
            + QString (fitMeni ? "-FM" : "") + QString (fitBott ? "-FB" : "")
            + QString (fitMeBo ? "-FMB" : "") + QString (refIter ? "-IT" : "")
            + QString (montCar ? "-MC" : "");
  ...
  sd.vbar = vbartb;
  sd.manual = manual;
  DbgLv (1) << "Data_Corr manual" << sd.manual << "avTemp" << avTemp
            << "vbar20" << sd.vbar20 << "vbart" << sd.vbar;
  US_Math2::data_correction (avTemp, sd);
  ...
  for (int ii = 0; ii < ncomp; ii++)
    {
      ...
      mstr
          += table_row (QString ().sprintf ("%10.4e", model.components[ii].mw),
...
US_2dsa::iteration_info ()
{
  ...
  if (montCar)
    ...
    mstr += table_row (tr ("Iteration"), tr ("Meniscus"), tr ("Bottom"),
                       tr ("RMSD"));
...
// Write HTML report file
void
US_2dsa::write_report (QTextStream &ts)
{
  ts << html_header (QString ("US_2dsa"),
                     tr ("2-Dimensional Spectrum Analysis"), edata);
  ts << distrib_info ();
  ts << iteration_info ();
  ts << indent (2) + "</body>\n</html>\n";
}
...
void
US_2dsa::write_bmap (const QString plotFile)
{
      ...
      for (int jj = 0; jj < npoints; jj++)
        {
          ...
          resscn[jj]
              = edata->value (ii, jj) - sdata.value (ii, jj) - rnoi - tnoi;
        }
...
#else
  QPixmap pixmap = QPixmap::grabWidget (resbmap, 0, 0, resbmap->width (),
                                        resbmap->height ());
#endif
      ...
      if (usemen && usebot)
        {
          ...
          DbgLv (1) << "fitmdat:  ii" << ii << "meni bott rmsd" << ameni
                    << abott << armsd;
        }
      else if (usemen)
        {
