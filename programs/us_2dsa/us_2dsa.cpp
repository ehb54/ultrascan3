...
void US_2dsa::analysis_done(int updflag) {
  ...
  if (updflag == (-2)) {  // update model,noise lists and RMSD
    ...
    if (ti_noise.count > 0) ti_noises << ti_noise;

    if (ri_noise.count > 0) ri_noises << ri_noise;
  ...
  DbgLv(1) << "  edat0 sdat0 rdat0 tnoi0" << edata->value(0, 0)
           << sdata.value(0, 0) << rdata.value(0, 0)
  ...
  if (nmodels > 1 && fitMeni) {
    ...
    for (int ii = 0; ii < nmodels; ii++) {
      ...
      double vari =
          mdesc.mid(mdesc.indexOf("VARI=") + 5).section(" ", 0, 0).toDouble();
      double rmsd = sqrt(vari);
      ...
      if (rmsd < b_rmsd) {
        ...
        b_bott = mdesc.mid(mdesc.indexOf("BOTTOM=") + 7)
                     .section(" ", 0, 0)
                     .toDouble();
        DbgLv(1) << "FitMens Done:    b_ rmsd,meni,bott" << b_rmsd << b_meni
                 << b_bott << "  ix" << (ii + 1);
      }
    }
    DbgLv(1) << "FitMens Done: BEST rmsd,meniscus,bottom" << b_rmsd << b_meni
             << b_bott;
  }
}
...
void US_2dsa::load(void) {
  ...
  if (loadDB) {  // Fetch the speed steps for the experiment from the database
    ...
    if (dbP->lastErrno() == US_DB2::OK) {
      ...
      DbgLv(1) << "2dsa_load: speed step count" << speed_steps.count()
               << "idExp" << idExp;
      if (speed_steps.count() > 0)
        DbgLv(1)
            << "2dsa_load:  speed step_ w2tfirst w2tlast timefirst timelast"
    ...
    if (xfi.open(QIODevice::ReadOnly)) {  // Read and parse "<speedstep>" lines
      ...
      while (!xmli.atEnd()) {
        ...
        if (xmli.isStartElement() && xmli.name() == "speedstep") {
          ...
          speed_steps << sp;
          DbgLv(1) << "LD:  sp: rotspeed" << sp.rotorspeed << "t1"
                   << sp.time_first;
        }
      }
    ...
    for (int ii = 0; ii < nssp; ii++) {
      ...
      for (int ds = 0; ds < lw_triples->count(); ds++) {
        ...
        if (etm1 < stm1 || etm2 > stm2) {  // Data times beyond speed step
          ...
          DbgLv(1) << "2dsa: Data is beyond range" << "limits from scans"
                   << etm1 << etm2 << ds;
          DbgLv(1) << "2dsa: Data is beyond range" << "limits from speed_profs "
                   << stm1 << stm2 << ds;
          // nssp = 0 ;
          // speed_steps.clear();
          break;
...
void US_2dsa::data_plot(void) {
  ...
  if (!dataLoaded || sdata.scanData.size() != edata->scanData.size()) {
    DbgLv(1) << "2dsa: from data plot : data are different  edata:"
             << edata->scanData.size() << "sdata:" << sdata.scanData.size();
    return;
  }
  ...
  for (int jj = 0; jj < npoints; jj++) ra[jj] = sdata.radius(jj);
...
void US_2dsa::save(void) {
  ...
  QString editID =
      (edata->editID.length() == 12) ? edata->editID.mid(2) : edata->editID;
  ...
  QString analysisType =
      QString(cusGrid ? "2DSA-CG" : "2DSA") + QString(fitMeni ? "-FM" : "") +
  ...
  int knois =
      min(ti_noise.count, 1) + min(ri_noise.count, 1);  // noise files per model
  ...
  while (indx > 0) {  // build a list of available model file names
    ...
    if (!mdnams.contains(mname)) {  // no name with this index exists, so add it
                                    // new-name list
      mnames << mname;
      if (++kmodels >= nmodels) break;
    }
  }
  ...
  // DbgLv(1) << "  Pre-sum tno tni" << tino << tini << "rno rni" << rino <<
  // rini;
  ...
  for (int jj = 0; jj < nmodels; jj++) {  // loop to output models and noises
    ...
    if (montCar)
      ...
      meniscus = mdesc.mid(mdesc.indexOf("MENISCUS=") + 9)
                     ...
                     .toDouble();
      bottom =
          mdesc.mid(mdesc.indexOf("BOTTOM=") + 7).section(' ', 0, 0).toDouble();
      if (bottom > 0.0)
    ...
    mname = montCar ? tmppath + mname.replace(".model", ".mdl.tmp")
                    : mdlpath + mnames[jj];
    ...
    if (have_ti) {  // output a TI noise
      ...
      if (dbP != NULL) ti_noise.write(dbP);

      if (nicount > 0)  // Remove input noise in case re-plotted
      {
        US_Noise noise_rmv = ti_noise_in;

        for (int kk = 0; kk < nicount; kk++) noise_rmv.values[kk] *= -1.0;
    ...
    if (have_ri) {  // output an RI noise
      ...
      if (dbP != NULL) ri_noise.write(dbP);
  ...
  if (montCar) {  // For Monte Carlo, create a composite of MC iteration files
    ...
    if (dbP != NULL) {  // If DB, load and store in the database
      ...
      qDebug() << "::::: nmodel=" << model.components.size()
               << " nmodel2=" << model2.components.size();
      ...
      for (int ii = 0; ii < sdata.scanCount(); ii++) {
        ...
        for (int jj = 0; jj < sdata.pointCount();
  ...
  QString dsinfFile =
      QString(filebase).replace(analynode, "/dsinfo.") + "dataset_info.html";
...
void US_2dsa::open_fitcntl() {
  ...
  if (need_tsfile) {
    ...
    DbgLv(1) << "2dsa : timestate newly created.  timestateobject = "
             << dset.simparams.tsobj << "exp_steps=" << exp_steps << "sspknt"
  ...
  DbgLv(1) << "2dsa : ssck: rspeed accel1 tf_aend tf_scan" << rspeed << accel1
  ...
  if (accel1 < lo_ss_acc || tf_aend > (tf_scan - 3)) {
    ...
    if (msgBox.clickedButton() == bAbort) return;
  }
...
QString US_2dsa::distrib_info() {
  ...
  if (maDesc.startsWith(runID)) {  // Saved model:  get analysis description
    ...
    QString adate =
        "a" + QDateTime::currentDateTime().toUTC().toString("yyMMddhhmm");
    bool cusGrid = model.description.contains("CUSTOMGRID");
    bool refIter = model.description.contains("REFITERS");
    bool fitMeni = (model.global == US_Model::MENISCUS);
    bool fitBott = (model.global == US_Model::BOTTOM);
  ...
  if (cnstvb)
    mstr += table_row(tr("Molecular Wt."), tr("S Apparent"), tr("S 20,W"),
  ...
  sd.vbar = vbartb;
  sd.manual = manual;
  DbgLv(1) << "Data_Corr manual" << sd.manual << "avTemp" << avTemp << "vbar20"
           << sd.vbar20 << "vbart" << sd.vbar;
  US_Math2::data_correction(avTemp, sd);
  ...
  for (int ii = 0; ii < ncomp; ii++) {
    ...
    mstr += table_row(QString().sprintf("%10.4e", model.components[ii].mw),
...
QString US_2dsa::iteration_info() {
  ...
  mstr +=
      table_row(tr("Number of Model Iterations:"), QString::number(nmodels));
  ...
  if (montCar)
    ...
    mstr +=
        table_row(tr("Iteration"), tr("Meniscus"), tr("Bottom"), tr("RMSD"));
...
void US_2dsa::write_report(QTextStream& ts) {
  ts << html_header(QString("US_2dsa"), tr("2-Dimensional Spectrum Analysis"),
                    edata);
  ts << distrib_info();
  ts << iteration_info();
  ts << indent(2) + "</body>\n</html>\n";
...
#else
  QPixmap pixmap =
      QPixmap::grabWidget(resbmap, 0, 0, resbmap->width(), resbmap->height());
#endif
...
QString US_2dsa::fit_meniscus_data() {
  ...
  for (int ii = 0; ii < nmodels; ii++) {
    ...
    if (usemen && usebot) {
      ...
      DbgLv(1) << "fitmdat:  ii" << ii << "meni bott rmsd" << ameni << abott
               << armsd;
    } else if (usemen) {
      QString ameni =
          mdesc.mid(mdesc.indexOf("MENISCUS=") + 9).section(" ", 0, 0);
...
void US_2dsa::resplot_done() { resplotd = 0; }
