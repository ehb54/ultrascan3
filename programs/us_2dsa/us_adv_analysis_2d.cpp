...
US_AdvAnalysis2D::US_AdvAnalysis2D(US_SimulationParameters* sim_par,
  ...
  QLabel* lb_bandload =
      us_label(tr("Band loading volume (") + QString(QChar(181)) + "l):");
  ...
  QLayout* lo_stndcp = us_radiobutton(tr("Standard Centerpiece"), rb_stndcp,
                                      !sparms->band_forming);
  QLayout* lo_bandcp = us_radiobutton(tr("Band-forming Centerpiece"), rb_bandcp,
                                      sparms->band_forming);
...
void US_AdvAnalysis2D::get_parameters(int& rtype, double& rtpar1,
                                      double& rtpar2, double& rtpar3,
                                      US_Model& modpar, bool& reg,
                                      double& repar1) {
...
void US_AdvAnalysis2D::load_model() {
  ...
  if (dialog.exec() ==
      QDialog::Accepted) {  // Enable Accept button if a model was loaded
