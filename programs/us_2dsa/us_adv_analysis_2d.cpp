...
US_AdvAnalysis2D::US_AdvAnalysis2D(US_SimulationParameters *sim_par, bool &loadDB, QWidget *p) :
   ...
   QLabel *lb_bandload = us_label(tr("Band loading volume (") + QString(QChar(181)) + "l):");
   ...
   QLayout *lo_stndcp = us_radiobutton(tr("Standard Centerpiece"), rb_stndcp, !sparms->band_forming);
   QLayout *lo_bandcp = us_radiobutton(tr("Band-forming Centerpiece"), rb_bandcp, sparms->band_forming);
   ...
   QLayout *lo_clipcs = us_checkbox(tr("Clip Lowest Concentration Solutes"), ck_clipcs);
   QLayout *lo_regulz = us_checkbox(tr("Regularization"), ck_regulz);
   QLayout *lo_mdgrid = us_checkbox(tr("Model-Defined Grid"), ck_mdgrid);
   QLayout *lo_mdrati = us_checkbox(tr("Model-Defined Ratios and Grid"), ck_mdrati);
   ...
   connect(rb_bandcp, SIGNAL(toggled(bool)), this, SLOT(checkBandForm(bool)));
   connect(ck_unifgr, SIGNAL(toggled(bool)), this, SLOT(checkUniGrid(bool)));
   connect(ck_locugr, SIGNAL(toggled(bool)), this, SLOT(checkLocalUni(bool)));
   connect(ck_ranlgr, SIGNAL(toggled(bool)), this, SLOT(checkRandLoc(bool)));
   connect(ck_soluco, SIGNAL(toggled(bool)), this, SLOT(checkSoluCoal(bool)));
   connect(ck_clipcs, SIGNAL(toggled(bool)), this, SLOT(checkClipLow(bool)));
   ...
   connect(ck_mdgrid, SIGNAL(toggled(bool)), this, SLOT(checkMdGrid(bool)));
   connect(ck_mdrati, SIGNAL(toggled(bool)), this, SLOT(checkMdRatios(bool)));
   connect(ck_regulz, SIGNAL(toggled(bool)), this, SLOT(checkRegular(bool)));
...
void US_AdvAnalysis2D::get_parameters(
   int &rtype, double &rtpar1, double &rtpar2, double &rtpar3, US_Model &modpar, bool &reg, double &repar1) {
...
void US_AdvAnalysis2D::select() {
   ...
   sparms->meshType = ( US_SimulationParameters::MeshType ) cmb_mesh->currentIndex();
   sparms->gridType = ( US_SimulationParameters::GridType ) cmb_moving->currentIndex();
...
void US_AdvAnalysis2D::load_model() {
   ...
   if (dialog.exec() == QDialog::Accepted) { // Enable Accept button if a model was loaded
