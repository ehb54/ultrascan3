...
US_AdvAnalysis2D::US_AdvAnalysis2D (US_SimulationParameters *sim_par,
    ...
    : US_WidgetsDialog (p, 0), loadDB (loadDB)
{
  sparms = sim_par;
  parentw = p;
  ...
  QLayout *lo_stndcp = us_radiobutton (tr ("Standard Centerpiece"), rb_stndcp,
                                       !sparms->band_forming);
  ...
  QLayout *lo_locugr = us_checkbox (tr ("Local Uniform Grid"), ck_locugr);
  QLayout *lo_ranlgr = us_checkbox (tr ("Random Local Grid"), ck_ranlgr);
  QLayout *lo_soluco = us_checkbox (tr ("Solute Coalescing"), ck_soluco);
  QLayout *lo_clipcs
      = us_checkbox (tr ("Clip Lowest Concentration Solutes"), ck_clipcs);
  QLayout *lo_regulz = us_checkbox (tr ("Regularization"), ck_regulz);
  QLayout *lo_mdgrid = us_checkbox (tr ("Model-Defined Grid"), ck_mdgrid);
  QLayout *lo_mdrati
  ...
  connect (rb_bandcp, SIGNAL (toggled (bool)), this,
           SLOT (checkBandForm (bool)));
  connect (ck_unifgr, SIGNAL (toggled (bool)), this,
           SLOT (checkUniGrid (bool)));
  connect (ck_locugr, SIGNAL (toggled (bool)), this,
           SLOT (checkLocalUni (bool)));
  connect (ck_ranlgr, SIGNAL (toggled (bool)), this,
           SLOT (checkRandLoc (bool)));
  connect (ck_soluco, SIGNAL (toggled (bool)), this,
           SLOT (checkSoluCoal (bool)));
  connect (ck_clipcs, SIGNAL (toggled (bool)), this,
           SLOT (checkClipLow (bool)));
  ...
  connect (ck_mdgrid, SIGNAL (toggled (bool)), this,
           SLOT (checkMdGrid (bool)));
  connect (ck_mdrati, SIGNAL (toggled (bool)), this,
           SLOT (checkMdRatios (bool)));
  connect (ck_regulz, SIGNAL (toggled (bool)), this,
           SLOT (checkRegular (bool)));
...
void
US_AdvAnalysis2D::get_parameters (int &rtype, double &rtpar1, double &rtpar2,
                                  double &rtpar3, US_Model &modpar, bool &reg,
                                  double &repar1)
{
...
void
US_AdvAnalysis2D::optimize_options ()
{
...
void
US_AdvAnalysis2D::uncheck_optimize (int ckflag)
{
  if (ckflag != 1)
...
void
US_AdvAnalysis2D::checkBandForm (bool checked)
{
...
void
US_AdvAnalysis2D::checkUniGrid (bool checked)
{
  if (checked)
    {
      uncheck_optimize (1);
...
void
US_AdvAnalysis2D::checkLocalUni (bool checked)
{
  if (checked)
    {
      uncheck_optimize (2);
...
void
US_AdvAnalysis2D::checkRandLoc (bool checked)
{
  if (checked)
    {
      uncheck_optimize (3);
...
void
US_AdvAnalysis2D::checkSoluCoal (bool checked)
{
  if (checked)
    {
      uncheck_optimize (4);
...
void
US_AdvAnalysis2D::checkClipLow (bool checked)
{
  if (checked)
    {
      uncheck_optimize (5);
...
void
US_AdvAnalysis2D::checkMeniscus (bool checked)
{
...
void
US_AdvAnalysis2D::checkMonteCar (bool checked)
{
...
void
US_AdvAnalysis2D::checkMdGrid (bool checked)
{
...
void
US_AdvAnalysis2D::checkMdRatios (bool checked)
{
...
void
US_AdvAnalysis2D::checkRegular (bool checked)
{
...
void
US_AdvAnalysis2D::select ()
{
  sparms->band_forming = rb_bandcp->isChecked ();
  sparms->band_volume = ct_bandload->value () / 1000.0;
  sparms->simpoints = (int)ct_spoints->value ();
  sparms->meshType
      = (US_SimulationParameters::MeshType)cmb_mesh->currentIndex ();
  sparms->gridType
      = (US_SimulationParameters::GridType)cmb_moving->currentIndex ();
