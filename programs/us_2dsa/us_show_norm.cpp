...

{
  ...
  QBoxLayout* plot =
      new US_Plot(data_plot, tr("Norms on grids"), xa_title, ya_title);
...
void US_show_norm::plot_data(void) {
  ...
  if (zpcent) {
    data_plot->setAxisTitle(QwtPlot::yRight, tr("Norms on grids"));
    sol_d = &xy_distro_zp;
  }
...
void US_show_norm::plot_data(int) { plot_data(); }

void US_show_norm::update_resolu(double dval) { resolu = dval; }

void US_show_norm::update_xreso(double dval) { xreso = dval; }

void US_show_norm::update_yreso(double dval) { yreso = dval; }

void US_show_norm::update_zfloor(double dval) { zfloor = dval; }
...
void US_show_norm::update_plot_kmin(double dval) { plt_kmin = dval; }

void US_show_norm::update_plot_kmax(double dval) { plt_kmax = dval; }
...
void US_show_norm::set_limits() {
  ...
  if (model->components.size() < 1) return;
...
void US_show_norm::sort_distro(QList<S_Solute>& listsols, bool reduce) {
