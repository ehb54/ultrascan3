...
void US_AnalysisControl2D::start() {
  if (parentw) {  // Get pointers to needed objects from the main
    ...
    DbgLv(1) << "AnaC: edata scans, baserss" << edata->scanData.size()
             << baserss;
  ...
  if (grtype == US_2dsaProcess::UGRID) {
    ...
    if ((ct_uplimits->value() - ct_lolimits->value()) < 0.0 ||
      ...
      if (ck_varvbar->isChecked()) msg = msg.replace("f/f0", "vbar");
    ...
    if (buoy == 0) {
      ...
      QMessageBox::critical(
          ...
          tr("The current vbar20 value (%1) implies a positive buoyancy\n"
             "value (%2), while the selected sedimentation values are "
             "negative.\n\n"
  ...
  int noif =
      (ck_tinoise->isChecked() ? 1 : 0) + (ck_rinoise->isChecked() ? 2 : 0);
  ...
  connect(processor, SIGNAL(progress_update(int)), this,
          SLOT(update_progress(int)));
  connect(processor, SIGNAL(message_update(QString, bool)), this,
          SLOT(progress_message(QString, bool)));
  connect(processor, SIGNAL(stage_complete(int, int)), this,
          SLOT(reset_steps(int, int)));
  connect(processor, SIGNAL(process_complete(int)), this,
          SLOT(completed_process(int)));

  int mxiter = (int)ct_iters->value();
  int mniter = (ck_menisc->isChecked() || ck_bottom->isChecked())
                   ? (int)ct_menispts->value()
                   : 0;
  int fittype = 0;
...
void US_AnalysisControl2D::load_model() {
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    if (sgsize < minsgsz ||
        sgsize >
            maxsgsz) {  // Implied subgrid size too large:  change subgrid count
      int ksubg = nsubg;
      int kssiz = sgsize;
    ...
    QString amsg = tr("Grid from loaded model\n  ( ") + QString::number(nsol) +
                   tr(" solutes, ") + QString::number(nsubg) +
                   tr(" subgrids )");
    ...
    if (cnst_ff0) ct_constff0->setValue(cusmodel.components[0].f_f0);
  }
}
...
void US_AnalysisControl2D::close_all() { close(); }
...
void US_AnalysisControl2D::grid_change() {
  ...
  if (nsteps != ksteps) {  // S grid points just changed:  adjust K points
    ...
    DbgLv(1) << "GC: 3)ngrrep" << ngrrep << "nss nsk" << nsteps << nstepk;
             ...
             kstepk) {  // K grid points just changed:  adjust S points
  ...
  if (parentw) {  // Get the starting base rss memory of this dataset and
                  // parameters
    US_2dsa* mainw = (US_2dsa*)parentw;
    mw_baserss = mainw->mw_base_rss();
    baserss = *mw_baserss;
  ...
  DbgLv(1) << "GC:  baserss tdata mdata ndatas nthrd" << baserss
           << qRound(tdata) << qRound(mdata) << ndatas << nthrd << "memneed"
           << memneed;

  int memava, memtot;
  US_Memory::memory_profile(&memava, &memtot);

  le_estmemory->setText(
      tr("%1 MB  (of %2 MB total real)").arg(memneed).arg(memtot));
  ...
  le_gridreps->setText(
      tr("%1  -> %2 %3-point subgrids").arg(ngrrep).arg(nsubg).arg(nspts));
...
void US_AnalysisControl2D::kstep_change() {
  ...
  else if (ct_uplimitk->value() ==
           ct_lolimitk->value()) {  // Set up for normal 2dsa parameters
    ct_uplimitk->setValue(4.0);
    int nthr = US_Settings::threads();
    nthr = (nthr > 1) ? nthr : QThread::idealThreadCount();
    ct_thrdcnt->setValue(nthr);
...
void US_AnalysisControl2D::completed_process(int stage) {
  ...
  if (dbg_level > 0) {
    ...
    for (int jj = 0; jj < knoi; jj++) ntot += ti_noise->values[jj];
  ...
  DbgLv(1) << "AC:cp inum mmit vari meni bott" << iternum << mmitnum << varinew
           << meniscus << bottom;
  ...
  if (mmitnum == 0) {  // simple refinement iteration (no MC/Meniscus)
    ...
    model->description =
        QString("MMITER=%1 VARI=%2 ").arg(mmitnum).arg(varinew);
  }
  ...
  if (alldone) {
    ...
    pb_save->setEnabled(true);
           ...
           stage > 0) {  // signal main to update lists of models,noises
    mainw->analysis_done(-2);
  }
}
...
void US_AnalysisControl2D::calculate_norms() {
  ...
  double cff0 =
      ck_varvbar->isChecked() ? ct_constff0->value() : 0.0;  // constant vbar

  DbgLv(1) << "CN: slo=" << slo << "sup=" << sup << "nss=" << nss
           << "klo=" << klo << "kup=" << kup << "nks=" << nks
           << "cff0=" << cff0;
  ...
  DbgLv(1) << "CN: nprs nprk" << nprs << nprk << "s_step" << s_step
           << "ff0_step" << ff0_step;

  solutes =
      US_Solute::create_solutes(slo, sup, s_step, klo, kup, ff0_step, cff0);
  ...
  if (check_file.exists() && check_file.isFile()) {
    ...
    DbgLv(1) << "adv_anal_control_2d : timestate file exists" << tmst_fpath
             << " timestateobject=" << simparms.tsobj << solutes.size()
             << simparms.speed_step.size() << simparms.speed_step[0].rotorspeed
             << simparms.speed_step[0].time_first
             << simparms.speed_step[0].time_last;
  } else {
    DbgLv(1) << "adv_anal_control_2d: timestate file does not exist"
             << solutes.size();
  }
  ...
  for (int ii = 0; ii < nthrd; ii++) {
    ...
    connect(wthr, SIGNAL(work_complete(WorkerThreadCalcNorm*)), this,
            SLOT(norm_complete(WorkerThreadCalcNorm*)));
...
void US_AnalysisControl2D::advanced() {
  ...
  if (aadiag->exec() == QDialog::Accepted) {
    ...
    DbgLv(1) << "Adv BANDVOL" << sparms->band_volume << " MUL"
             << sparms->cp_width;
    if (grtype < 0) {
      ...
      if (sgsize >
          150) {  // Implied subgrid size too large:  change subgrid count
        int ksubg = nsubg;
        int kssiz = sgsize;
        nsubg = (nsol / 100 + 1) | 1;
        sgsize = nsol / nsubg;
      ...
      QString amsg =
          ...
          QString::number(nsol) + tr(" solutes, ") + QString::number(nsubg) +
          tr(" subgrids )");
      te_status->setText(amsg);
...
int US_AnalysisControl2D::memory_check() {
  ...
  if (memneed > memsafe || mempca < 20) {
    ...
    if (memneed > memtot) {
      ...
      if (msgBox.exec() == QMessageBox::No) status = 2;
    }
  }
...
void US_AnalysisControl2D::set_comp_attr(
...
void US_AnalysisControl2D::norm_complete(WorkerThreadCalcNorm* wthr) {
  ...
  DbgLv(1) << "norm_complete is called " << workout.nwsols
           << workout.csolutes.size();
  ...
  if (kthrdr == 0) {
    ...
    for (int ii = 0; ii < model2.components.size();
         ii++) {  // For plotting purposes, scale sedimentation coefficients
      model2.components[ii].s *= 1.0e+13;
      DbgLv(1) << "model2_values_from_norm_complete"
               << "  s,k" << model2.components[ii].s
               << model2.components[ii].f_f0 << "  norm"
