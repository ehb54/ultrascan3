    ...
    : US_WidgetsDialog (p, 0), dsets (dsets), loadDB (loadDB)
{
  ...
  connect (ck_unifgr, SIGNAL (toggled (bool)), this,
           SLOT (checkUniGrid (bool)));
  connect (ck_custgr, SIGNAL (toggled (bool)), this,
           SLOT (checkCusGrid (bool)));
  connect (ck_menisc, SIGNAL (toggled (bool)), this,
           SLOT (checkMeniscus (bool)));
  connect (ck_bottom, SIGNAL (toggled (bool)), this,
           SLOT (checkMeniscus (bool)));
  connect (ck_mcarlo, SIGNAL (toggled (bool)), this,
           SLOT (checkMonteCar (bool)));
  connect (ck_iters, SIGNAL (toggled (bool)), this,
           SLOT (checkIterate (bool)));
  connect (ck_varvbar, SIGNAL (toggled (bool)), this,
           SLOT (checkVaryVbar (bool)));

  connect (ct_nstepss, SIGNAL (valueChanged (double)), this,
           SLOT (grid_change ()));
  connect (ct_nstepsk, SIGNAL (valueChanged (double)), this,
           SLOT (grid_change ()));
  connect (ct_thrdcnt, SIGNAL (valueChanged (double)), this,
           SLOT (grid_change ()));
  connect (ct_lolimits, SIGNAL (valueChanged (double)), this,
           SLOT (slim_change ()));
  connect (ct_uplimits, SIGNAL (valueChanged (double)), this,
           SLOT (slim_change ()));
  connect (ct_lolimitk, SIGNAL (valueChanged (double)), this,
           SLOT (klim_change ()));
  connect (ct_nstepsk, SIGNAL (valueChanged (double)), this,
           SLOT (kstep_change ()));
...
void
US_AnalysisControl2D::optimize_options ()
{
...
void
US_AnalysisControl2D::uncheck_optimize (int ckflag)
{
  if (ckflag > 3)
...
void
US_AnalysisControl2D::checkUniGrid (bool checked)
{
  if (checked)
    uncheck_optimize (1);
...
void
US_AnalysisControl2D::checkCusGrid (bool checked)
{
...
void
US_AnalysisControl2D::checkMeniscus (bool checked)
{
  if (checked)
    uncheck_optimize (2);
...
void
US_AnalysisControl2D::checkMonteCar (bool checked)
{
  if (checked)
    {
      uncheck_optimize (3);
...
void
US_AnalysisControl2D::checkLocalUni (bool checked)
{
  if (checked)
    {
      uncheck_optimize (4);
...
void
US_AnalysisControl2D::checkRandLoc (bool checked)
{
  if (checked)
    {
      uncheck_optimize (5);
...
void
US_AnalysisControl2D::checkSoluCoal (bool checked)
{
  if (checked)
    {
      uncheck_optimize (6);
...
void
US_AnalysisControl2D::checkClipLow (bool checked)
{
  if (checked)
    {
      uncheck_optimize (7);
...
void
US_AnalysisControl2D::checkRegular (bool checked)
{
  if (checked)
    {
      uncheck_optimize (8);
...
void
US_AnalysisControl2D::checkIterate (bool checked)
{
...
void
US_AnalysisControl2D::checkVaryVbar (bool checked)
{
...
US_AnalysisControl2D::start ()
{
  ...
  DbgLv (1) << "AnaC:St:MEM (1)rssnow,proc" << US_Memory::rss_now ()
            << processor;
...
void
US_AnalysisControl2D::load_model ()
{
...
void
US_AnalysisControl2D::close_all ()
{
  close ();
}
...
void
US_AnalysisControl2D::grid_change ()
{
  static int ksteps = 0;
  static int kstepk = 0;
...
void
US_AnalysisControl2D::slim_change ()
{
...
void
US_AnalysisControl2D::klim_change ()
{
...
void
US_AnalysisControl2D::kstep_change ()
{
  if (ct_nstepsk->value () == 1.0)
...
void
US_AnalysisControl2D::update_progress (int ksteps)
{
  ncsteps += ksteps;
...
void
US_AnalysisControl2D::progress_message (QString pmsg, bool append)
{
  QString amsg;
...
void
US_AnalysisControl2D::reset_steps (int kcs, int nct)
{
...
void
US_AnalysisControl2D::completed_process (int stage)
{
  ...
  DbgLv (1) << "AC:cp inum mmit vari meni bott" << iternum << mmitnum
            << varinew << meniscus << bottom;
  ...
  if (mmitnum == 0)
    ...
    { // Meniscus (or Meniscus,Bottom)
      ...
      if (ck_bottom->isChecked ())
        { // Meniscus,Bottom-fit
          model->description
              = QString ("MMITER=%1 VARI=%2 MENISCUS=%3 BOTTOM=%4")
                    .arg (mmitnum)
                    .arg (varinew)
                    .arg (meniscus)
                    .arg (bottom);
...
void
US_AnalysisControl2D::calculate_norms ()
{
  ...
  double cff0 = ck_varvbar->isChecked () ? ct_constff0->value ()
                                         : 0.0; // constant vbar
...
void
US_AnalysisControl2D::advanced ()
{
  US_SimulationParameters *sparms = &dsets[0]->simparams;
  DbgLv (1) << "Adv sparms.bf sect" << sparms->band_forming
            << sparms->cp_sector;
  ...
  if (aadiag->exec () == QDialog::Accepted)
    {
      ...
      aadiag->get_parameters (grtype, grpar1, grpar2, grpar3, modpar, reg,
                              repar1);
      DbgLv (1) << "Adv ACCEPT";
      ...
      if (grtype < 0)
        {
          ...
          if (sgsize > 150)
            { // Implied subgrid size too large:  change subgrid count
              ...
              nsubg = (nsol / 100 + 1) | 1;
              sgsize = nsol / nsubg;
              DbgLv (0) << "Subgrid count adjusted from" << ksubg << "to"
                        << nsubg;
              DbgLv (0) << "Subgrid size adjusted from" << kssiz << "to"
                        << sgsize;
...
int
US_AnalysisControl2D::memory_check ()
{
  const int pc_ava = 90;
  int status = 0;
...
void
US_AnalysisControl2D::set_comp_attr (US_Model::SimulationComponent &component,
...
void
US_AnalysisControl2D::norm_progress (int kstep)
{
  normstep += kstep;
...
void
US_AnalysisControl2D::norm_complete (WorkerThreadCalcNorm *wthr)
{
