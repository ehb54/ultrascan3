...
US_AnalysisControl2D::US_AnalysisControl2D(QList<SS_DATASET *> &dsets, bool &loadDB, QWidget *p) :
    US_WidgetsDialog(p, 0), dsets(dsets), loadDB(loadDB) {
   parentw = p;
   processor = 0;
   ...
   QLayout *lo_tinois = us_checkbox(tr("Fit Time-Invariant Noise"), ck_tinoise);
   QLayout *lo_rinois = us_checkbox(tr("Fit Radially-Invariant Noise"), ck_rinoise);
   QLayout *lo_autoplt = us_checkbox(tr("Automatically Plot"), ck_autoplt);
   QLayout *lo_varvbar = us_checkbox(tr("Vary Vbar with Constant f/f0"), ck_varvbar);
   ...
   le_gridreps = us_lineedit(tr("8  -> 64 64-point subgrids"), -1, true);
   le_gridreps->setAlignment(Qt::AlignVCenter | Qt::AlignLeft);

   b_progress = us_progressBar(0, 100, 0);

   QLayout *lo_iters = us_checkbox(tr("Use Iterative Method"), ck_iters, false);
   QLayout *lo_unifgr = us_checkbox(tr("Uniform Grid"), ck_unifgr, true);
   QLayout *lo_custgr = us_checkbox(tr("Custom Grid"), ck_custgr, false);
   QLayout *lo_menisc = us_checkbox(tr("Float Meniscus"), ck_menisc, false);
   QLayout *lo_bottom = us_checkbox(tr("Float Bottom"), ck_bottom, false);
   QLayout *lo_mcarlo = us_checkbox(tr("Monte Carlo Iterations"), ck_mcarlo, false);
   ...
   connect(ck_unifgr, SIGNAL(toggled(bool)), this, SLOT(checkUniGrid(bool)));
   connect(ck_custgr, SIGNAL(toggled(bool)), this, SLOT(checkCusGrid(bool)));
   connect(ck_menisc, SIGNAL(toggled(bool)), this, SLOT(checkMeniscus(bool)));
   connect(ck_bottom, SIGNAL(toggled(bool)), this, SLOT(checkMeniscus(bool)));
   connect(ck_mcarlo, SIGNAL(toggled(bool)), this, SLOT(checkMonteCar(bool)));
   connect(ck_iters, SIGNAL(toggled(bool)), this, SLOT(checkIterate(bool)));
   connect(ck_varvbar, SIGNAL(toggled(bool)), this, SLOT(checkVaryVbar(bool)));

   connect(ct_nstepss, SIGNAL(valueChanged(double)), this, SLOT(grid_change()));
   connect(ct_nstepsk, SIGNAL(valueChanged(double)), this, SLOT(grid_change()));
   connect(ct_thrdcnt, SIGNAL(valueChanged(double)), this, SLOT(grid_change()));
   connect(ct_lolimits, SIGNAL(valueChanged(double)), this, SLOT(slim_change()));
   connect(ct_uplimits, SIGNAL(valueChanged(double)), this, SLOT(slim_change()));
   connect(ct_lolimitk, SIGNAL(valueChanged(double)), this, SLOT(klim_change()));
   connect(ct_nstepsk, SIGNAL(valueChanged(double)), this, SLOT(kstep_change()));

   connect(pb_strtfit, SIGNAL(clicked()), this, SLOT(start()));
   connect(pb_stopfit, SIGNAL(clicked()), this, SLOT(stop_fit()));
   connect(pb_ldmodel, SIGNAL(clicked()), this, SLOT(load_model()));
   connect(pb_plot, SIGNAL(clicked()), this, SLOT(plot()));
   connect(pb_save, SIGNAL(clicked()), this, SLOT(save()));
   connect(pb_help, SIGNAL(clicked()), this, SLOT(help()));
   connect(pb_close, SIGNAL(clicked()), this, SLOT(close_all()));
   connect(pb_advance, SIGNAL(clicked()), this, SLOT(advanced()));
   connect(pb_anorm, SIGNAL(clicked()), this, SLOT(calculate_norms()));
...
void US_AnalysisControl2D::optimize_options() {
   ct_menisrng->setEnabled(ck_menisc->isChecked() || ck_bottom->isChecked());
   ct_menispts->setEnabled(ck_menisc->isChecked() || ck_bottom->isChecked());
...
void US_AnalysisControl2D::start() {
   if (parentw) { // Get pointers to needed objects from the main
   ...
   if (grtype == US_2dsaProcess::UGRID) {
      ...
      if ((ct_uplimits->value() - ct_lolimits->value()) < 0.0 || (ct_uplimitk->value() - ct_lolimitk->value()) < 0.0) {
         QString msg = tr("The \"s\" or \"f/f0\" ranges are inconsistent.\n"
      ...
      if (buoy == 0) {
         ...
         QMessageBox::critical(
            this, tr("Positive Buoyancy Implied"),
         ...
         QMessageBox::critical(
            this, tr("Negative Buoyancy Implied"),
   ...
   if (US_SolveSim::checkGridSize(dsets, s_max, smsg)) {
      QMessageBox::critical(this, tr("Implied Grid Size is Too Large!"), smsg);
      //      return;
   }
   ...
   int noif = (ck_tinoise->isChecked() ? 1 : 0) + (ck_rinoise->isChecked() ? 2 : 0);

   int ngrr = (grtype < 0) ? grtype : US_Math2::best_grid_reps(nss, nks);
   ...
   connect(processor, SIGNAL(progress_update(int)), this, SLOT(update_progress(int)));
   connect(processor, SIGNAL(message_update(QString, bool)), this, SLOT(progress_message(QString, bool)));
   connect(processor, SIGNAL(stage_complete(int, int)), this, SLOT(reset_steps(int, int)));
   connect(processor, SIGNAL(process_complete(int)), this, SLOT(completed_process(int)));

   int mxiter = ( int ) ct_iters->value();
   int mniter = (ck_menisc->isChecked() || ck_bottom->isChecked()) ? ( int ) ct_menispts->value() : 0;
   int fittype = 0;
   fittype |= (ck_menisc->isChecked() ? 1 : 0);
   fittype |= (ck_bottom->isChecked() ? 2 : 0);
   int mciter = ck_mcarlo->isChecked() ? ( int ) ct_mciters->value() : 0;
   ...
   // Begin the fit
   processor->set_iters(mxiter, mciter, mniter, vtoler, menrng, cff0, ngrr, fittype);

   processor->start_fit(slo, sup, nss, klo, kup, nks, ngrr, nthr, noif);
...
void US_AnalysisControl2D::load_model() {
   ...
   if (dialog.exec() == QDialog::Accepted) {
      ...
      if (sgsize < minsgsz || sgsize > maxsgsz) { // Implied subgrid size too large:  change subgrid count
         int ksubg = nsubg;
         int kssiz = sgsize;
      ...
      QString amsg = tr("Grid from loaded model\n  ( ") + QString::number(nsol) + tr(" solutes, ")
...
void US_AnalysisControl2D::grid_change() {
   ...
   if (nsteps != ksteps) { // S grid points just changed:  adjust K points
      nstepk = (nstepk / ngrrep) * ngrrep;
      ngrrep = US_Math2::best_grid_reps(nsteps, nstepk);
      DbgLv(1) << "GC: 3)ngrrep" << ngrrep << "nss nsk" << nsteps << nstepk;
   }
   else if (nstepk != kstepk) { // K grid points just changed:  adjust S points
   ...
   if (parentw) { // Get the starting base rss memory of this dataset and parameters
      US_2dsa *mainw = ( US_2dsa * ) parentw;
      mw_baserss = mainw->mw_base_rss();
      baserss = *mw_baserss;
   ...
   DbgLv(1) << "GC:  baserss tdata mdata ndatas nthrd" << baserss << qRound(tdata) << qRound(mdata) << ndatas << nthrd
            << "memneed" << memneed;

   int memava, memtot;
   US_Memory::memory_profile(&memava, &memtot);

   le_estmemory->setText(tr("%1 MB  (of %2 MB total real)").arg(memneed).arg(memtot));
   ...
   le_gridreps->setText(tr("%1  -> %2 %3-point subgrids").arg(ngrrep).arg(nsubg).arg(nspts));
...
void US_AnalysisControl2D::slim_change() {
   ...
   if (loval < -1.e5 || upval > 1.e5) { // For larger magnitudes, adjust ranges
...
void US_AnalysisControl2D::kstep_change() {
   if (ct_nstepsk->value() == 1.0) { // Set up for C(s) parameters
      ct_uplimitk->setValue(ct_lolimitk->value());
   ...
   else if (ct_uplimitk->value() == ct_lolimitk->value()) { // Set up for normal 2dsa parameters
      ct_uplimitk->setValue(4.0);
      int nthr = US_Settings::threads();
      nthr = (nthr > 1) ? nthr : QThread::idealThreadCount();
      ct_thrdcnt->setValue(nthr);
...
void US_AnalysisControl2D::progress_message(QString pmsg, bool append) {
   ...
   if (append) { // append to existing progress message
      amsg = te_status->toPlainText() + "\n" + pmsg;
   }

   else { // create a new progress message
      amsg = pmsg;
   }
...
void US_AnalysisControl2D::completed_process(int stage) {
   ...
   if (stage == 6) { // If stopped because of memory usage, execute Stop Fit
      stop_fit();
      return;
   }
   ...
   if (dbg_level > 0) {
      ...
      DbgLv(1) << "AC:cp DTOT" << dtot << "edata" << edata << "NTOT" << ntot << "nnoi knoi" << nnoi << knoi << "STOT"
               << stot;
   }
   //DBG-DATA
   ...
   DbgLv(1) << "AC:cp inum mmit vari meni bott" << iternum << mmitnum << varinew << meniscus << bottom;
   ...
   if (mmitnum == 0) { // simple refinement iteration (no MC/Meniscus)
      ...
      model->description = QString("MMITER=%1 VARI=%2 ").arg(mmitnum).arg(varinew);
   }

   else if (ck_menisc->isChecked()) { // Meniscus (or Meniscus,Bottom)
      model->global = US_Model::MENISCUS;
      if (ck_bottom->isChecked()) { // Meniscus,Bottom-fit
         model->description
            = QString("MMITER=%1 VARI=%2 MENISCUS=%3 BOTTOM=%4").arg(mmitnum).arg(varinew).arg(meniscus).arg(bottom);
         le_iteration->setText(
            QString::number(iternum) + " , Model " + QString::number(mmitnum) + " , Meniscus "
            + QString::number(meniscus) + " , Bottom " + QString::number(bottom));
      }
      else { // Meniscus-fit
         model->description = QString("MMITER=%1 VARI=%2 MENISCUS=%3").arg(mmitnum).arg(varinew).arg(meniscus);
         le_iteration->setText(
            QString::number(iternum) + " , Model " + QString::number(mmitnum) + " , Meniscus "
   ...
   else if (ck_bottom->isChecked()) { // Bottom-fit (only)
      model->global = US_Model::BOTTOM;
      model->description = QString("MMITER=%1 VARI=%2 BOTTOM=%4").arg(mmitnum).arg(varinew).arg(bottom);
      le_iteration->setText(
         QString::number(iternum) + " , Model " + QString::number(mmitnum) + " , Bottom " + QString::number(bottom));
   }

   else { // Monte Carlo
      model->monteCarlo = true;
      model->description = QString("MMITER=%1 VARI=%2 ").arg(mmitnum).arg(varinew);
      le_iteration->setText(QString::number(iternum) + "   ( MC Model " + QString::number(mmitnum) + " )");
   }
   ...
   else if (mmitnum > 0 && stage > 0) { // signal main to update lists of models,noises
      mainw->analysis_done(-2);
   }
}
...
void US_AnalysisControl2D::calculate_norms() {
   ...
   DbgLv(1) << "CN: slo=" << slo << "sup=" << sup << "nss=" << nss << "klo=" << klo << "kup=" << kup << "nks=" << nks
            << "cff0=" << cff0;
   ...
   if (check_file.exists() && check_file.isFile()) {
      ...
      DbgLv(1) << "adv_anal_control_2d : timestate file exists" << tmst_fpath << " timestateobject=" << simparms.tsobj
               << solutes.size() << simparms.speed_step.size() << simparms.speed_step[ 0 ].rotorspeed
   ...
   for (int ii = 0; ii < nthrd; ii++) {
      ...
      connect(wthr, SIGNAL(work_progress(int)), this, SLOT(norm_progress(int)));

      connect(wthr, SIGNAL(work_complete(WorkerThreadCalcNorm *)), this, SLOT(norm_complete(WorkerThreadCalcNorm *)));
...
void US_AnalysisControl2D::advanced() {
   ...
   if (aadiag->exec() == QDialog::Accepted) {
      ...
      aadiag->get_parameters(grtype, grpar1, grpar2, grpar3, modpar, reg, repar1);
      DbgLv(1) << "Adv ACCEPT";
      ...
      if (grtype < 0) {
         ...
         if (sgsize > 150) { // Implied subgrid size too large:  change subgrid count
            int ksubg = nsubg;
            int kssiz = sgsize;
            nsubg = (nsol / 100 + 1) | 1;
            sgsize = nsol / nsubg;
         ...
         QString amsg = (grtype == (-1) ? tr("Grid from loaded model\n  ( ")
                                        : tr("Grid and signal ratios from loaded model\n\n  ( "))
                        + QString::number(nsol) + tr(" solutes, ") + QString::number(nsubg) + tr(" subgrids )");
         te_status->setText(amsg);
...
int US_AnalysisControl2D::memory_check() {
   ...
   if (memneed > memsafe || mempca < 20) {
      ...
      if (memneed > memtot) {
         QMessageBox::critical(
            this, title,
            tr("Memory needed for this fit exceeds total available.") + memp
               + tr("This fit will not proceed.\n"
         ...
         msgBox.setText(
            tr("Memory needed for this fit is a\n"
...
void US_AnalysisControl2D::set_comp_attr(US_Model::SimulationComponent &component, US_Solute &solute, int attr_type) {
   switch (attr_type) {
      default:
...
void US_AnalysisControl2D::norm_complete(WorkerThreadCalcNorm *wthr) {
   ...
   if (kthrdr == 0) {
      ...
      for (int ii = 0; ii < model2.components.size(); ii++) { // For plotting purposes, scale sedimentation coefficients
         model2.components[ ii ].s *= 1.0e+13;
         DbgLv(1) << "model2_values_from_norm_complete"
                  << "  s,k" << model2.components[ ii ].s << model2.components[ ii ].f_f0 << "  norm"
                  << model2.components[ ii ].signal_concentration;
      }
