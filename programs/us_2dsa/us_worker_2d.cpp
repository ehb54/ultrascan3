...
void WorkerThread2D::define_work(WorkPacket2D &workin) {
   ...
   DbgLv(1) << "WT:DWk: "
            << "dsi:tsobj" << workin.dsets[ 0 ]->simparams.tsobj << "dsw:tsobj" << dset_wk.simparams.tsobj << "dsi:sspk"
...
void WorkerThread2D::get_result(WorkPacket2D &workout) {
   ...
   DbgLv(1) << "2P(WT): thr nn" << thrn << nn << "out sol0 solk soln" << workout.csolutes[ 0 ].c
            << workout.csolutes[ kk ].c << workout.csolutes[ nn - 1 ].c << "in sol0 soln" << ni
            << solutes_i[ 0 ].s * 1.e13 << solutes_i[ ni - 1 ].s * 1.e13 << solutes_i[ 0 ].c << solutes_i[ ni - 1 ].c;
}
...
void WorkerThread2D::calc_residuals() {
   ...
   connect(solvesim, SIGNAL(work_progress(int)), this, SLOT(forward_progress(int)));
...
void WorkerThread2D::calc_resids_ratio() {
   ...
   US_Math2::nnls(nnls_a.data(), ntotal, ntotal, 1, nnls_b.data(), nnls_x.data());
