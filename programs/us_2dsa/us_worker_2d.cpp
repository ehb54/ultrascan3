...
WorkerThread2D::WorkerThread2D (QObject *parent) : QThread (parent)
{
...
WorkerThread2D::~WorkerThread2D ()
{
  ...
  DbgLv (1) << "2P(WT):   Thread destroy - (1)finished?" << isFinished ()
            << thrn;
  if (!wait (2000))
    {
      qDebug () << "Thread destroy wait timeout(2secs) : Thread" << thrn;
    }
  DbgLv (1) << "2P(WT):   Thread destroy - (2)finished?" << isFinished ()
            << thrn;
...
void
WorkerThread2D::define_work (WorkPacket2D &workin)
{
  ...
  DbgLv (1) << "WT:DWk: "
            << "dsi:tsobj" << workin.dsets[0]->simparams.tsobj << "dsw:tsobj"
            << dset_wk.simparams.tsobj << "dsi:sspk"
            << workin.dsets[0]->simparams.sim_speed_prof.count () << "dsw:sspk"
            << dset_wk.simparams.sim_speed_prof.count ();
...
void
WorkerThread2D::get_result (WorkPacket2D &workout)
{
  workout.ll_s = llim_s;
  workout.ll_k = llim_k;
  workout.thrn = thrn;
  ...
  DbgLv (1) << "2P(WT): thr nn" << thrn << nn << "out sol0 solk soln"
            ...
            << workout.csolutes[nn - 1].c << "in sol0 soln" << ni
            << solutes_i[0].s * 1.e13 << solutes_i[ni - 1].s * 1.e13
...
WorkerThread2D::calc_residuals ()
{
  ...
  connect (solvesim, SIGNAL (work_progress (int)), this,
           SLOT (forward_progress (int)));
...
void
WorkerThread2D::forward_progress (int steps)
{
...
void
WorkerThread2D::calc_resids_ratio ()
{
  dset_wk = *dsets[0];
  ...
  US_Math2::nnls (nnls_a.data (), ntotal, ntotal, 1, nnls_b.data (),
                  nnls_x.data ());
