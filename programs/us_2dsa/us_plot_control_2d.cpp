...
US_PlotControl2D::US_PlotControl2D(QWidget *p, US_Model *amodel) : US_WidgetsDialog(p, 0) {
   model = amodel;
   ...
   connect(ct_zscalefac, SIGNAL(valueChanged(double)), this, SLOT(zscal_value(double)));
   connect(ct_gridreso, SIGNAL(valueChanged(double)), this, SLOT(gridr_value(double)));
   connect(ct_peaksmoo, SIGNAL(valueChanged(double)), this, SLOT(peaks_value(double)));
   connect(ct_peakwidth, SIGNAL(valueChanged(double)), this, SLOT(peakw_value(double)));
   ...
   if (model->constant_vbar()) { // Vbar constant, so disable vbar X,Y check boxes
...
void US_PlotControl2D::checkSet(bool chkd, bool isX, int row) {
   ...
   if (model->constant_ff0()) { // f/f0 constant, so disable f/f0 X,Y check boxes
...
void US_PlotControl2D::plot_close(void) {
   plot3d_w = 0;
}
