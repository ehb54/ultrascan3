...
US_PlotControl2D::US_PlotControl2D(QWidget* p, US_Model* amodel)
  ...
  setObjectName("US_PlotControl2D");
  ...
  connect(ck_xmwt, SIGNAL(toggled(bool)), this, SLOT(xmwtCheck(bool)));
  connect(ck_ymwt, SIGNAL(toggled(bool)), this, SLOT(ymwtCheck(bool)));
  connect(ck_xsed, SIGNAL(toggled(bool)), this, SLOT(xsedCheck(bool)));
  connect(ck_ysed, SIGNAL(toggled(bool)), this, SLOT(ysedCheck(bool)));
  connect(ck_xdif, SIGNAL(toggled(bool)), this, SLOT(xdifCheck(bool)));
  connect(ck_ydif, SIGNAL(toggled(bool)), this, SLOT(ydifCheck(bool)));
  connect(ck_xfco, SIGNAL(toggled(bool)), this, SLOT(xfcoCheck(bool)));
  connect(ck_yfco, SIGNAL(toggled(bool)), this, SLOT(yfcoCheck(bool)));
  connect(ck_xfra, SIGNAL(toggled(bool)), this, SLOT(xfraCheck(bool)));
  connect(ck_yfra, SIGNAL(toggled(bool)), this, SLOT(yfraCheck(bool)));
  connect(ck_xvba, SIGNAL(toggled(bool)), this, SLOT(xvbaCheck(bool)));
  connect(ck_yvba, SIGNAL(toggled(bool)), this, SLOT(yvbaCheck(bool)));

  connect(ct_zscalefac, SIGNAL(valueChanged(double)), this,
          SLOT(zscal_value(double)));
  connect(ct_gridreso, SIGNAL(valueChanged(double)), this,
          SLOT(gridr_value(double)));
  connect(ct_peaksmoo, SIGNAL(valueChanged(double)), this,
          SLOT(peaks_value(double)));
  connect(ct_peakwidth, SIGNAL(valueChanged(double)), this,
          SLOT(peakw_value(double)));

  connect(pb_plot3d, SIGNAL(clicked()), this, SLOT(plot3_btn()));
  ...
  if (cnstff0) {  // f/f0 constant, so disable f/f0 X,Y check boxes
    ck_xfra->setEnabled(false);
  ...
  if (model->constant_vbar()) {  // Vbar constant, so disable vbar X,Y check
                                 // boxes
...
// mw x box checked
void US_PlotControl2D::xmwtCheck(bool chkd) { checkSet(chkd, true, 0); }
// mw y box checked
void US_PlotControl2D::ymwtCheck(bool chkd) { checkSet(chkd, false, 0); }
// sedcoeff x box checked
void US_PlotControl2D::xsedCheck(bool chkd) { checkSet(chkd, true, 1); }
// sedcoeff y box checked
void US_PlotControl2D::ysedCheck(bool chkd) { checkSet(chkd, false, 1); }
// diffcoeff x box checked
void US_PlotControl2D::xdifCheck(bool chkd) { checkSet(chkd, true, 2); }
// diffcoeff y box checked
void US_PlotControl2D::ydifCheck(bool chkd) { checkSet(chkd, false, 2); }
// friccoeff x box checked
void US_PlotControl2D::xfcoCheck(bool chkd) { checkSet(chkd, true, 3); }
// friccoeff y box checked
void US_PlotControl2D::yfcoCheck(bool chkd) { checkSet(chkd, false, 3); }
// fricratio x box checked
void US_PlotControl2D::xfraCheck(bool chkd) { checkSet(chkd, true, 4); }
// fricratio y box checked
void US_PlotControl2D::yfraCheck(bool chkd) { checkSet(chkd, false, 4); }
// Vbar x box checked
void US_PlotControl2D::xvbaCheck(bool chkd) { checkSet(chkd, true, 5); }
// Vbar y box checked
void US_PlotControl2D::yvbaCheck(bool chkd) { checkSet(chkd, false, 5); }

// handle any x or y box checked
void US_PlotControl2D::checkSet(bool chkd, bool isX, int row) {
  if (!chkd)  // only respond if checked
    return;

  if (isX) {                          // this is a X box
    for (int ii = 0; ii < 6; ii++) {  // review all check boxes
      if (ii != row) {                // in other rows, uncheck X, enable Y
        xCheck[ii]->setChecked(false);
        yCheck[ii]->setEnabled(true);
      }

      else {  // in this row, disable the Y
        yCheck[ii]->setEnabled(false);
      }
    }
  }

  else {                              // this is a Y box
    for (int ii = 0; ii < 6; ii++) {  // review all check boxes
      if (ii != row) {                // in other rows, uncheck Y, enable X
        yCheck[ii]->setChecked(false);
        xCheck[ii]->setEnabled(true);
      }

      else {  // in this row, disable the X
        xCheck[ii]->setEnabled(false);
      }
    }
  }
  if (model
          ->constant_ff0()) {  // f/f0 constant, so disable f/f0 X,Y check boxes
...
void US_PlotControl2D::zscal_value(double value) { zscale = value; }
// grid resolution changed
void US_PlotControl2D::gridr_value(double value) { gridres = value; }
// peak smoothing changed
void US_PlotControl2D::peaks_value(double value) { pksmooth = value; }
// peak width changed
void US_PlotControl2D::peakw_value(double value) { pkwidth = value; }
...
void US_PlotControl2D::plot3_btn() {
  ...
  if (plot3d_w == 0) {
    if (model == 0 || model->components.size() < 1) return;

    plot3d_w = new US_Plot3D(this, model);
    connect(plot3d_w, SIGNAL(has_closed()), this, SLOT(plot_close()));
  }
...
void US_PlotControl2D::close_all() { close(); }
...
// 3d plot window closed
void US_PlotControl2D::plot_close(void) { plot3d_w = 0; }
