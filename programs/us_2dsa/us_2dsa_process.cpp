...
long int US_2dsaProcess::max_rss(void) { return US_Memory::rss_max(maxrss); }

// Start a specified 2DSA fit run
void US_2dsaProcess::start_fit(double sll, double sul, int nss, double kll,
                               double kul, int nks, int ngr, int nthr,
                               int noif) {
  ...
  if (mmtype >
      0) {  // for meniscus or monte carlo, make a working copy of the base data
    wdata = *bdata;
    ...
    if (mmtype == 1) {  // if meniscus, use the start meniscus,bottom values
      ...
      DbgLv(1) << "2P:SF: MENISC: mm_iter" << mm_iter << "bmeniscus bbottom"
               << bmeniscus << bbottom << "emeniscus ebottom" << edata->meniscus
               << edata->bottom;
  ...
  int kksubg =
      nksteps * nssteps - (kgref + kgrefsq) * (nsubp_s + nsubp_k) + kgrefsq;
  ...
  DbgLv(1) << "2P:SF:   nsubgrid nctotal nthreads maxtsols" << nsubgrid
           << nctotal << nthreads << maxtsols;
  max_rss();
  ...
  if (jgrefine > 0) {
    US_Solute::init_solutes(ssllim, ssulim, nssteps, klolim, kuplim, nksteps,
    ...
    if (model.analysis == US_Model::CUSTOMGRID && nsubgrid > 0) {
      for (int ii = 0; ii < nsubgrid; ii++) {
        ...
        for (int jj = ii; jj < ncomps; jj += nsubgrid) {
          US_Solute soli(model.components[jj].s, model.components[jj].f_f0, 0.0,
                         model.components[jj].vbar20, model.components[jj].D);
      ...
      for (int ii = 0; ii < ncomps; ii++) {
        US_Solute soli(model.components[ii].s, model.components[ii].f_f0, 0.0,
    ...
    for (int ii = 0; ii < ncomps; ii++) {
      US_Solute soli(model.components[ii].s, model.components[ii].f_f0,
  ...
  DbgLv(1) << "2P:SF: orig_sols: " << orig_sols[0][0].s * 1.e+13
           << orig_sols[0][0].k << "  " << orig_sols[0][k1].s * 1.e+13
           << orig_sols[0][k1].k << "  " << orig_sols[k0][0].s * 1.e+13
           << orig_sols[k0][0].k << "  " << orig_sols[k0][k2].s * 1.e+13
           << orig_sols[k0][k2].k;
  ...
  DbgLv(1) << "2P:SF:   kstask nthreads" << kstask << nthreads
           << job_queue.size();
...
void US_2dsaProcess::set_iters(int mxiter, int mciter, int mniter,
  ...
  if (jgrefine > 0) {
    if (cnstff0 > 0.0) stype = 1;  // Constant f/f0, varying vbar
  } else
    stype = 2;  // Custom grid
...
void US_2dsaProcess::final_computes() {
  ...
  for (int ii = 0; ii < c_solutes[depth].size(); ii++) {
    if (c_solutes[depth][ii].c > 0.0) wtask.isolutes << c_solutes[depth][ii];
  }
  // DbgLv(1) << "norm_size_final_compute" << wtask.isolutes.size() <<
  // wtask.Anorm.size();
  ...
  connect(wthr, SIGNAL(work_complete(WorkerThread2D*)), this,
          SLOT(process_final(WorkerThread2D*)));
...
void US_2dsaProcess::process_final(WorkerThread2D* wthrd) {
  ...
  if (c_solutes.size() < (maxdepth + 1)) c_solutes << QVector<US_Solute>();
  ...
  DbgLv(1) << "FIN_FIN:    wresult menisc bott" << dset->simparams.meniscus
  ...
  if (dset->solute_type == 0) {  // Normal case of varying f/f0
    DbgLv(1) << "FIN_FIN: solute_type" << dset->solute_type << "nsols"
             << nsolutes << "maxdepth" << maxdepth << "csol size"
             << c_solutes.size() << "csol[m]size" << c_solutes[maxdepth].size();
  ...
  DbgLv(1) << "FIN_FIN:    c0 cn" << c_solutes[maxdepth][0].c
           << c_solutes[maxdepth][qMax(0, nsolutes - 1)].c << "  nsols"
           << nsolutes;
...
stdspc=true;
    ...
    int lc = nsolutes - 1;
    DbgLv(1) << "FIN_FIN:  0) s D c" << s_model.components[0].s
             << s_model.components[0].D
             << s_model.components[0].signal_concentration;
    DbgLv(1) << "FIN_FIN:  n) s D c" << s_model.components[lc].s
             << s_model.components[lc].D
  ...
  int mms = nscans / 2;
  int mmr = npoints / 2;
  DbgLv(1) << "FIN_FIN: edatm" << edata->value(mms, mmr) << "sdatm"
           << sdata.value(mms, mmr) << swdat.value(mms, mmr) << "rdatm"
           << rdata.value(mms, mmr) << rwdat.value(mms, mmr);
  ...
  DbgLv(1) << "FIN_FIN: vari" << vari << "vari2" << vari2 << "rmsd2" << rmsd2
           << "s20w,D20w_corr" << dset->s20w_correction
           << dset->D20w_correction;
  itvaris << vari;
  ...
  DbgLv(1) << "FIN_FIN: vari riter miter menisc bott" << s_variance << s_rfiter
           << s_mmiter << s_meniscus << s_bottom;
  ...
  DbgLv(1) << "done: vari bvol" << vari << bvol << "slun klun ngr" << slolim
           << suplim << nssteps << klolim << kuplim << nksteps << ngrefine
           << "ets" << ktimes;

  ktimes = ktimes - ktimeh * 3600 - ktimem * 60;

  // compose final iteration status message
  QString pmsg = pmessage_head() + tr("The Solution has converged...\n"
                                      "Iterations:  %1\n"
  ...
  DbgLv(1) << "FIN_FIN: maxrss memmb nthr" << maxrss << memmb << nthreads
           << " nsubg nsst nkst noisf" << nsubgrid << nssteps << nksteps
           << noisflag;
  ...
  if ((r_iter + 1) <
      maxiters) {  // possibly iterate if not yet at maximum iterations

    if (r_iter < 1) {  // if max not 2 or more, we must do at least 2 iterations
                       // to compare
      neediter = true;
    }
      ...
      if (nccsol == npcsol) {  // determine if calculated solutes match previous
                               // in s and k
        sdiffs = false;
        for (int jj = 0; jj < nccsol; jj++) {
          if (ical_sols[jc][jj] !=
              ical_sols[jp][jj]) {  // if any mismatch, we may need to iterate
            sdiffs = true;
            break;
          }
        }
  ...
  if (mmtype > 0 &&
      ++mm_iter < mtiters) {  // doing meniscus or monte carlo and more to do
    ...
    if (mtiters > 50)
      DbgLv(1) << "MENISC: mtiters mmiters" << mtiters << mmiters
               << "ff_: omeni" << ff_omeni << "obott" << ff_obott << "menbot"
               << ff_menbot << "meni" << ff_meni << "bott" << ff_bott;
    ...
    DbgLv(1) << "MENISC: k_iter mm_iter mtiter mmtype" << k_iter << mm_iter
             << mtiters << mmtype;
...
bool US_2dsaProcess::get_results(US_DataIO::RawData* da_sim,
  ...
  *da_mdl = model;  // copy model
  ...
  double vari2 = 0.0;
  for (int ss = 0; ss < nscans; ss++)
    for (int rr = 0; rr < npoints; rr++) vari2 += sq(rdata.value(ss, rr));
  vari2 /= (double)(nscans * npoints);
  int mms = nscans / 2;
  int mmr = npoints / 2;
...
bool US_2dsaProcess::get_values(QMap<QString, QString>& mp_val) {
  bool all_ok = true;
  DbgLv(1) << "2P:GV: variance" << s_variance << vari_curr << "iter"
           << s_mmiter;
  ...
  DbgLv(1) << " GET_VAL: rfit mcit vari meni bott" << s_rfiter << s_mmiter
           << s_variance << s_meniscus << s_bottom;
...
void US_2dsaProcess::submit_job(WorkPacket2D& wtask, int thrx) {
  ...
  connect(wthr, SIGNAL(work_complete(WorkerThread2D*)), this,
          SLOT(process_job(WorkerThread2D*)));
  connect(wthr, SIGNAL(work_progress(int)), this, SLOT(step_progress(int)));
...
void US_2dsaProcess::process_job(WorkerThread2D* wthrd) {
  ...
  DbgLv(1) << "PROCESS_JOB thrn" << thrn << "taskx" << taskx << "depth"
           << wresult.depth;
  ...
  if (dbg_level > 0)
    for (int mm = 0; mm < wresult.csolutes.size(); mm++) {
      if (wresult.csolutes[mm].c > 100.0) {
        DbgLv(1) << "PJ:  CONC=" << wresult.csolutes[mm].c << " s,ff0"
                 << wresult.csolutes[mm].s * 1.0e+13 << wresult.csolutes[mm].k;
      }
    }
  // DBG-CONC
  ...
  if (dbg_level > 0) {
    ...
    for (int ii = 0; ii < nscans; ii++) {
      ...
      DbgLv(1) << "PJ:DA DTOT" << dtot << "thr,tsk,ncso" << thrn << taskx
               << nrcso << "edata" << edata << "nti,NTOT" << nnoi << ntot;
    }
  }
  // DBG-DATA
  //
  max_rss();                              // Compute max memory used
  if (taskx <= nthreads) memory_check();  // Check for memory use too high
  ...
  while (c_solutes.size() < (depth + 1)) c_solutes << QVector<US_Solute>();
  ...
  if (depthn > 4 && nextc > maxtsols &&
      ((cs_size / wr_size) == 1 ||
       (wr_size / cs_size) == 1)) {  // Adjust max solutes per task if it is
                                     // only large enough for one output
    maxtsols = (nextc * 11 + 9) / 10;
  }

  if (nextc > maxtsols) {  // if new solutes push count over limit, queue a job
                           // at next depth
  ...
  if (depth ==
      0) {     // this result is from depth 0, it is from initial subgrids pass
    kctask++;  // bump count of completed subgrid tasks
    DbgLv(1) << "THR_FIN: thrn" << thrn << " taskx" << taskx << " kct kst"
             << kctask << kstask << "csols size" << c_solutes.size();
    emit message_update(
        pmessage_head() + tr("Computations for %1 of %2 subgrids are complete")
                              .arg(kctask)
                              .arg(nsubgrid),
        false);
    ...
    if (kctask == nsubgrid) {  // all subgrid tasks are now complete
      ...
      emit message_update(
          pmessage_head() + tr("Computing depth 1 solutions and beyond ..."),
          false);
  ...
  for (int dd = 0; dd < depth; dd++) {
    if (jobs_at_depth(dd) == 0 &&
        c_solutes[dd].size() >
            0) {  // queue a task to handle last solutes at this depth
      WorkPacket2D wtask = wresult;
      int taskx = tkdepths.size();
      int depthn = dd + 1;
      DbgLv(1) << "THR_FIN:    QT: taskx depth solsz" << taskx << depth
               << c_solutes[dd].size();
  ...
  if (depth == maxdepth && job_queue.isEmpty() && no_working &&
  ...
  while (!job_queue.isEmpty() && (thrx = wkstates.indexOf(READY)) >= 0) {
    ...
    if (wtask.depth > 1 &&
        wtask.taskx ==
            tkdepths.indexOf(
                wtask.depth)) {  // submitting the first job of a depth pass
  ...
  if (kstsksv == kstask) {  // No new tasks got started:  what's going on?
    ...
    if (depth < maxdepth) {  // If done at depth less than max, see need to
                             // queue new task
      int dd = depth + 1;
      ...
      if (jobs_at_depth(dd) == 0 &&
          c_solutes[dd].size() >
              0) {  // queue a task to handle remaining solutes at next depth
        depth = dd;
        WorkPacket2D wtask = wresult;
        int taskx = tkdepths.size();
      ...
      if (jobs_at_depth(dd) == 0 &&
          c_solutes[dd].size() > 0) {  // queue a task to handle remaining
                                       // solutes at next depth plus one
        depth = dd;
        WorkPacket2D wtask = wresult;
        int taskx = tkdepths.size();
        DbgLv(1) << "THR_FIN:    QT: /NONEW++/taskx depth solsz" << taskx
                 << depth << c_solutes[dd].size();
        queue_task(wtask, slolim, klolim, taskx, depth, jnois,
                   c_solutes[depth]);
...
void US_2dsaProcess::queue_task(WorkPacket2D& wtask, double llss, double llsk,
  ...
  ntisols += nrisols;
  if (taskx < 9 || taskx > (nsubgrid - 4))
    DbgLv(1) << "QT: taskx" << taskx << " isolutes size tot" << nrisols
             << ntisols;
  ...
  if (tkdepths.count(depth) == 1) {  // if first task at this depth, report it
    emit message_update(
        tr("Submitting first depth %1 calculations...").arg(depth), true);
...
void US_2dsaProcess::iterate() {
  ...
  DbgLv(1) << "ITER:    nsubg ktask kstep knois" << nsubgrid << ktask << kstep
           << knois;
  ...
  for (int jj = 0; jj < ncsol; jj++)
    DbgLv(1) << "ITER:     csol" << jj << "  s k c" << csolutes[jj].s * 1.e13
             << csolutes[jj].k << csolutes[jj].c;
  int ktadd = 0;
  //*DEBUG
  ...
  for (ktask = 0; ktask < nsubgrid; ktask++) {
    ...
    for (int cc = 0; cc < ncsol; cc++) {
      if (!isolutes.contains(csolutes[cc])) isolutes << csolutes[cc];
    }
    //*DEBUG
    ...
    ktadd += (ncsol - kadd);
    if (kadd < ncsol)
      DbgLv(1) << "ITER: kt" << ktask << "iterate nisol o a c +" << kosz << kasz
               << ncsol << kadd << "ktadd" << ktadd << "nsubg" << nsubgrid;
    //*DEBUG
    ...
    for (int kt = 0; kt < nsubgrid; kt++)
      for (int cc = 0; cc < orig_sols[kt].size(); cc++)
        DbgLv(1) << "ITER          kt cc" << kt << cc << " s k c"
                 << orig_sols[kt][cc].s * 1.e13 << orig_sols[kt][cc].k
                 << orig_sols[kt][cc].c;
  }
  //*DEBUG

  // Make sure calculated solutes are cleared out for new iteration
  for (int ii = 0; ii < c_solutes.size(); ii++) c_solutes[ii].clear();
  ...
  emit message_update(tr("Starting iteration %1 computations of %2 subgrids\n"
                         "  using %3 threads ...")
                          .arg(r_iter + 1)
                          .arg(nsubgrid)
                          .arg(nthreads),
...
int US_2dsaProcess::estimate_steps(int ncsol) {
  ...
  DbgLv(1) << "ES: ncsol ktisol ktcsol ktstep" << ncsol << ktisol << ktcsol
           << ktstep;
  ...
  while (n1task >
         1) {  // Sum in steps for depth 3 and following until just 1 task left
    n1task = (n1csol + ktisol / 2) / ktisol + 1;
    ntasks += n1task;
    n1csol = n1task * ktcsol;
  ...
  int szdata =
      sizeof(US_DataIO::RawData) + (nscns * szscnd) + (nscns * nrpts * szrdng);
  int szsimu =
      sizeof(US_SolveSim::Simulation) + (2 * szdata) + (szsols * ktisol);
...
int US_2dsaProcess::queued_at_depth(int depth) {
  ...
  for (int ii = 0; ii < job_queue.size(); ii++) {
    if (job_queue[ii].depth == depth) count++;
  }
...
void US_2dsaProcess::set_meniscus() {
  ...
  DbgLv(1) << "SET_MEN: k_iter m_iter b_iter" << k_iter << m_iter << b_iter
           << "meniscus bottom" << edata->meniscus << simparms->bottom << "bbot"
           << bbottom;
...
void US_2dsaProcess::set_monteCarlo() {
  ...
  edata = &wdata;
  DbgLv(1) << "MCARLO: mm_iter" << mm_iter << " sigma0 c0 v0 cn vn" << sigmas[0]
           << bdata->value(0, 0) << edata->value(0, 0)
           << bdata->value(nscans - 1, npoints - 1)
           << edata->value(nscans - 1, npoints - 1);
...
void US_2dsaProcess::set_gaussians() {
  ...
  sigmas.clear();
  DbgLv(1) << "MCARLO:setgau ns np" << nscans << npoints << "gausmoo"
           << gausmoo;

  if (!gausmoo) {  // Construct sigmas from iteration 1 residuals
    for (int ss = 0; ss < nscans; ss++)
      for (int rr = 0; rr < npoints; rr++) sigmas << qAbs(rdata.value(ss, rr));
  }
    ...
    for (int rr = 0; rr < ntpoints; rr++) rmsds += sq(sigmas[rr]);
...
WorkPacket2D US_2dsaProcess::next_job() {
  ...
  if (depth >
      0) {  // If first is beyond depth 0, insure what we get is lowest depth
  ...
  if (jobx > 0) {
    DbgLv(1) << "NEXTJ: jobx depth depth0 taskx taskx0" << jobx << depth
             << job_queue[0].depth << wtask.taskx << job_queue[0].taskx;
  } else {
