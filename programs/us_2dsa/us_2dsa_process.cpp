...
US_2dsaProcess::US_2dsaProcess(QList<SS_DATASET *> &dsets, QObject *parent /*=0*/) : QObject(parent), dsets(dsets) {
...
void US_2dsaProcess::start_fit(
   double sll, double sul, int nss, double kll, double kul, int nks, int ngr, int nthr, int noif) {
   ...
   if (jgrefine < 0) { // Special model-grid or model-ratio grid refinement
      ngrefine = 1;
      model = dsets[ 0 ]->model;
   ...
   DbgLv(1) << "2P:SF: sll sul nss" << slolim << suplim << nssteps << " kll kul nks" << klolim << kuplim << nksteps
   ...
   if (mmtype > 0) { // for meniscus or monte carlo, make a working copy of the base data
      wdata = *bdata;

      if (mmtype == 1) { // if meniscus, use the start meniscus,bottom values
         edata = &wdata;
         ...
         DbgLv(1) << "2P:SF: MENISC: mm_iter" << mm_iter << "bmeniscus bbottom" << bmeniscus << bbottom
   ...
   int kksubg = nksteps * nssteps - (kgref + kgrefsq) * (nsubp_s + nsubp_k) + kgrefsq;
   ...
   DbgLv(1) << "2P:SF:   gdelta_s gdelta_k" << gdelta_s << gdelta_k << " sdelta_s sdelta_k" << sdelta_s << sdelta_k;
   DbgLv(1) << "2P:SF:   nsubgrid nctotal nthreads maxtsols" << nsubgrid << nctotal << nthreads << maxtsols;
   max_rss();
   ...
   if (jgrefine > 0) {
      US_Solute::init_solutes(ssllim, ssulim, nssteps, klolim, kuplim, nksteps, ngrefine, cnstff0, orig_sols);
      ...
      if (model.analysis == US_Model::CUSTOMGRID && nsubgrid > 0) {
         for (int ii = 0; ii < nsubgrid; ii++) {
            ...
            for (int jj = ii; jj < ncomps; jj += nsubgrid) {
               US_Solute soli(
                  model.components[ jj ].s, model.components[ jj ].f_f0, 0.0, model.components[ jj ].vbar20,
         ...
         for (int ii = 0; ii < ncomps; ii++) {
            US_Solute soli(
               model.components[ ii ].s, model.components[ ii ].f_f0, 0.0, model.components[ ii ].vbar20,
   ...
   else if (jgrefine == (-2)) { // model-ratio
      QVector<US_Solute> solvec;

      for (int ii = 0; ii < ncomps; ii++) {
         US_Solute soli(
            model.components[ ii ].s, model.components[ ii ].f_f0, model.components[ ii ].signal_concentration,
            model.components[ ii ].vbar20, model.components[ ii ].D);
         DbgLv(1) << "ii" << ii << "soli" << soli.s << soli.k << soli.c;
         solvec << soli;
      }
   ...
   DbgLv(1) << "2P:SF: orig_sols: " << orig_sols[ 0 ][ 0 ].s * 1.e+13 << orig_sols[ 0 ][ 0 ].k << "  "
            << orig_sols[ 0 ][ k1 ].s * 1.e+13 << orig_sols[ 0 ][ k1 ].k << "  " << orig_sols[ k0 ][ 0 ].s * 1.e+13
            << orig_sols[ k0 ][ 0 ].k << "  " << orig_sols[ k0 ][ k2 ].s * 1.e+13 << orig_sols[ k0 ][ k2 ].k;
   ...
   emit message_update(
      pmessage_head() + tr("Starting computations of %1 subgrids\n using %2 threads ...").arg(nsubgrid).arg(nthreads),
      false);
...
void US_2dsaProcess::set_iters(
   int mxiter, int mciter, int mniter, double vtoler, double menrng, double cff0, int jgref, int fittyp) {
   maxiters = mxiter;
   ...
   DbgLv(1) << "2PSI: fittyp" << fit_type << "ff_omeni obott menbot meni bott" << ff_omeni << ff_obott << ff_menbot
            << ff_meni << ff_bott;
...
void US_2dsaProcess::final_computes() {
   ...
   connect(wthr, SIGNAL(work_complete(WorkerThread2D *)), this, SLOT(process_final(WorkerThread2D *)));
   connect(wthr, SIGNAL(work_progress(int)), this, SLOT(step_progress(int)));

   emit message_update(pmessage_head() + tr("Computing final NNLS ..."), false);
...
void US_2dsaProcess::process_final(WorkerThread2D *wthrd) {
   ...
   DbgLv(1) << "FIN_FIN: get_results: wr.isol.sz" << wresult.isolutes.size() << "wr.csol.sz" << wresult.csolutes.size()
            << nsolutes;
   ...
   DbgLv(1) << "FIN_FIN: s20w,D20w_corr" << dset->s20w_correction << dset->D20w_correction << "sfac dfac" << sfactor
            << dfactor;
   DbgLv(1) << "FIN_FIN:    wresult menisc bott" << dset->simparams.meniscus << dset->simparams.bottom;
   ...
   if (dset->solute_type == 0) { // Normal case of varying f/f0
      DbgLv(1) << "FIN_FIN: solute_type" << dset->solute_type << "nsols" << nsolutes << "maxdepth" << maxdepth
               << "csol size" << c_solutes.size() << "csol[m]size" << c_solutes[ maxdepth ].size();
      for (int cc = 0; cc < nsolutes; cc++) {
         ...
         mcomp.signal_concentration = c_solutes[ maxdepth ][ cc ].c;

         // Complete other coefficients in standard-space
         model.calc_coefficients(mcomp);
         DbgLv(1) << "FIN_FIN:  Bcc comp D" << mcomp.D << "comp ff0" << mcomp.f_f0 << "comp vb20" << mcomp.vbar20;
   ...
   else if (dset->solute_type == 1) { // Special case of varying vbar
   ...
   DbgLv(1) << "FIN_FIN:    c0 cn" << c_solutes[ maxdepth ][ 0 ].c << c_solutes[ maxdepth ][ qMax(0, nsolutes - 1) ].c
            << "  nsols" << nsolutes;
...
stdspc=true;
      ...
      int lc = nsolutes - 1;
      DbgLv(1) << "FIN_FIN:  0) s D c" << s_model.components[ 0 ].s << s_model.components[ 0 ].D
               << s_model.components[ 0 ].signal_concentration;
      DbgLv(1) << "FIN_FIN:  n) s D c" << s_model.components[ lc ].s << s_model.components[ lc ].D
   ...
   int mms = nscans / 2;
   int mmr = npoints / 2;
   DbgLv(1) << "FIN_FIN: edatm" << edata->value(mms, mmr) << "sdatm" << sdata.value(mms, mmr) << swdat.value(mms, mmr)
   ...
   DbgLv(1) << "FIN_FIN: vari" << vari << "vari2" << vari2 << "rmsd2" << rmsd2 << "s20w,D20w_corr"
   ...
   DbgLv(1) << "FIN_FIN: vari riter miter menisc bott" << s_variance << s_rfiter << s_mmiter << s_meniscus << s_bottom;
   ...
   DbgLv(1) << "done: vari bvol" << vari << bvol << "slun klun ngr" << slolim << suplim << nssteps << klolim << kuplim
   ...
   pmsg += tr("Maximum memory used:  ") + QString::number(qRound(memmb)) + " MB";
   ...
   DbgLv(1) << "FIN_FIN: maxrss memmb nthr" << maxrss << memmb << nthreads << " nsubg nsst nkst noisf" << nsubgrid
            << nssteps << nksteps << noisflag;
   DbgLv(1) << "FIN_FIN:   kcsteps nctotal" << kcsteps << nctotal;

   bool neediter = false; // need-more-iterations false by default

   if ((r_iter + 1) < maxiters) { // possibly iterate if not yet at maximum iterations

      if (r_iter < 1) { // if max not 2 or more, we must do at least 2 iterations to compare
         neediter = true;
      }

      else { // otherwise, we must compare solutes and variance difference
         int jc = r_iter;
         int jp = r_iter - 1;
         ...
         if (nccsol == npcsol) { // determine if calculated solutes match previous in s and k
            sdiffs = false;
            for (int jj = 0; jj < nccsol; jj++) {
               if (ical_sols[ jc ][ jj ] != ical_sols[ jp ][ jj ]) { // if any mismatch, we may need to iterate
                  sdiffs = true;
                  break;
               }
            }
         ...
         DbgLv(1) << "FIN_FIN: neediter" << neediter << "  sdiffs" << sdiffs << " dvari varitol" << dvari << varitol
                  << " iter" << r_iter;
      }
   }

   if (neediter) { // we must iterate
   ...
   if (dset->solute_type == 0) { // Constant vbar
      for (int cc = 0; cc < nsolutes; cc++) {
   ...
   if (mmtype > 0 && ++mm_iter < mtiters) { // doing meniscus or monte carlo and more to do
      int m_iter = ff_menbot ? (k_iter / mmiters) : (ff_meni ? k_iter : 0);
      int b_iter = ff_menbot ? (k_iter % mmiters) : (ff_bott ? k_iter : 0);
      double bmeniscus = bdata->meniscus;
      double bbottom = bdata->bottom;
      double emeniscus = bmeniscus;
      double ebottom = bbottom;
      if (ff_meni) {
         emeniscus = (bmeniscus - menrange * 0.5) + (( double ) m_iter * mendelta);
      }
      if (ff_bott) {
         ebottom = (bbottom - menrange * 0.5) + (( double ) b_iter * mendelta);
      }
      ...
      DbgLv(1) << "MENISC: k_iter m_iter b_iter" << k_iter << m_iter << b_iter << "meniscus bottom" << edata->meniscus
               << simparms->bottom << "bbottom mtiters" << bbottom << mtiters;
      if (mtiters > 50)
         DbgLv(1) << "MENISC: mtiters mmiters" << mtiters << mmiters << "ff_: omeni" << ff_omeni << "obott" << ff_obott
...
bool US_2dsaProcess::get_results(
   ...
   *da_mdl = model; // copy model
   ...
   int mms = nscans / 2;
   int mmr = npoints / 2;
   DbgLv(1) << " GET_RES:    VARI" << vari_curr << vari2 << s_variance << "rdatmm" << rdata.value(mms, mmr);
   return all_ok;
}
...
bool US_2dsaProcess::get_values(QMap<QString, QString> &mp_val) {
   ...
   DbgLv(1) << " GET_VAL: rfit mcit vari meni bott" << s_rfiter << s_mmiter << s_variance << s_meniscus << s_bottom;
...
void US_2dsaProcess::submit_job(WorkPacket2D &wtask, int thrx) {
   ...
   connect(wthr, SIGNAL(work_complete(WorkerThread2D *)), this, SLOT(process_job(WorkerThread2D *)));
   connect(wthr, SIGNAL(work_progress(int)), this, SLOT(step_progress(int)));
...
void US_2dsaProcess::process_job(WorkerThread2D *wthrd) {
   ...
   DbgLv(1) << "PROCESS_JOB thrn" << thrn << "taskx" << taskx << "depth" << wresult.depth;
   ...
   if (dbg_level > 0)
      for (int mm = 0; mm < wresult.csolutes.size(); mm++) {
         if (wresult.csolutes[ mm ].c > 100.0) {
            DbgLv(1) << "PJ:  CONC=" << wresult.csolutes[ mm ].c << " s,ff0" << wresult.csolutes[ mm ].s * 1.0e+13
   ...
   if (dbg_level > 0) {
      ...
      for (int ii = 0; ii < nscans; ii++) {
         ...
         DbgLv(1) << "PJ:DA DTOT" << dtot << "thr,tsk,ncso" << thrn << taskx << nrcso << "edata" << edata << "nti,NTOT"
                  << nnoi << ntot;
      }
   }
   //DBG-DATA
   ...
   if (
      depthn > 4 && nextc > maxtsols
      && ((cs_size / wr_size) == 1 || (wr_size / cs_size) == 1)) { // Adjust max solutes per task if it is only large enough for one output
      maxtsols = (nextc * 11 + 9) / 10;
   }

   if (nextc > maxtsols) { // if new solutes push count over limit, queue a job at next depth
      WorkPacket2D wtask = wresult;
      int taskx = tkdepths.size();

      queue_task(wtask, slolim, klolim, taskx, depthn, jnois, c_solutes[ depth ]);

      maxdepth = qMax(maxdepth, depthn);
      c_solutes[ depth ].clear();
      DbgLv(1) << "THR_FIN: depth" << wtask.depth << " #solutes" << wtask.isolutes.size() << " nextc maxtsols" << nextc
               << maxtsols << "wres#sols" << wresult.csolutes.size();
   }

   if (depth == 0) { // this result is from depth 0, it is from initial subgrids pass
      kctask++; // bump count of completed subgrid tasks
      DbgLv(1) << "THR_FIN: thrn" << thrn << " taskx" << taskx << " kct kst" << kctask << kstask << "csols size"
               << c_solutes.size();
      emit message_update(
         pmessage_head() + tr("Computations for %1 of %2 subgrids are complete").arg(kctask).arg(nsubgrid), false);

      if (kctask == nsubgrid) { // all subgrid tasks are now complete
         if (r_iter == 0) { // in 1st iteration, re-estimate total progress steps
            ...
            DbgLv(1) << "THR_FIN:   (est)kcst ncto" << kcsteps << nctotal << " nsol ntodo" << nsolest << todoest;
            // adjust the estimate of total progress steps
            nctotal = kcsteps + todoact;
            DbgLv(1) << "THR_FIN:   (new)kcst ncto" << kcsteps << nctotal << " nsol ntodo" << nsolact << todoact;
         }

         emit stage_complete(kcsteps, nctotal);

         emit message_update(pmessage_head() + tr("Computing depth 1 solutions and beyond ..."), false);
   ...
   for (int dd = 0; dd < depth; dd++) {
      if (jobs_at_depth(dd) == 0 && c_solutes[ dd ].size() > 0) { // queue a task to handle last solutes at this depth
         WorkPacket2D wtask = wresult;
         int taskx = tkdepths.size();
         int depthn = dd + 1;
         DbgLv(1) << "THR_FIN:    QT: taskx depth solsz" << taskx << depth << c_solutes[ dd ].size();
         queue_task(wtask, slolim, klolim, taskx, depthn, jnois, c_solutes[ dd ]);
         c_solutes[ dd ].clear();
      }
   }
   ...
   DbgLv(1) << "THR_FIN: nowk dep mxdp cssz wrsz" << no_working << depth << maxdepth << c_solutes[ depth ].size()
            << wresult.csolutes.size();

   // Submit one last time with all solutes if necessary
   if (depth == maxdepth && job_queue.isEmpty() && no_working && c_solutes[ depth ].size() >= wresult.csolutes.size()) {
      final_computes();
      return;
   }
   DbgLv(1) << "THR_FIN: jqempty" << job_queue.isEmpty() << "ReadyWorkerNdx" << wkstates.indexOf(READY);
   int kstsksv = kstask;
   ...
   while (!job_queue.isEmpty() && (thrx = wkstates.indexOf(READY)) >= 0) {
      ...
      if (wtask.depth > 1 && wtask.taskx == tkdepths.indexOf(wtask.depth)) { // submitting the first job of a depth pass
         QString pmsg = tr("Computing depth %1 solutions...").arg(wtask.depth);
   ...
   thrx = wkstates.indexOf(READY);
   if (kstsksv == kstask) { // No new tasks got started:  what's going on?
      DbgLv(1) << "THR_FIN: *NONEW* jqempty" << job_queue.isEmpty() << " ReadyWorkerNdx" << wkstates.indexOf(READY);
      if (depth < maxdepth) { // If done at depth less than max, see need to queue new task
         int dd = depth + 1;
         if ((dd + 1) > c_solutes.size())
            c_solutes << QVector<US_Solute>();
         DbgLv(1) << "THR_FIN:  dd" << dd << "jad(dd)" << jobs_at_depth(dd) << "csize" << c_solutes[ dd ].size();

         if (jobs_at_depth(dd) == 0 && c_solutes[ dd ].size() > 0) { // queue a task to handle remaining solutes at next depth
            depth = dd;
            WorkPacket2D wtask = wresult;
            int taskx = tkdepths.size();
            DbgLv(1) << "THR_FIN:    QT: /NONEW/taskx depth solsz" << taskx << depth << c_solutes[ dd ].size();
            queue_task(wtask, slolim, klolim, taskx, depth, jnois, c_solutes[ depth ]);
         ...
         if ((dd + 1) > c_solutes.size())
            c_solutes << QVector<US_Solute>();
         DbgLv(1) << "THR_FIN:  dd2" << dd << "jad(dd)" << jobs_at_depth(dd) << "csize" << c_solutes[ dd ].size();

         if (jobs_at_depth(dd) == 0 && c_solutes[ dd ].size() > 0) { // queue a task to handle remaining solutes at next depth plus one
            depth = dd;
            WorkPacket2D wtask = wresult;
            int taskx = tkdepths.size();
            DbgLv(1) << "THR_FIN:    QT: /NONEW++/taskx depth solsz" << taskx << depth << c_solutes[ dd ].size();
...
void US_2dsaProcess::queue_task(
   WorkPacket2D &wtask, double llss, double llsk, int taskx, int depth, int noisf, QVector<US_Solute> isolutes) {
   ...
   if (tkdepths.count(depth) == 1) { // if first task at this depth, report it
      emit message_update(tr("Submitting first depth %1 calculations...").arg(depth), true);
...
void US_2dsaProcess::iterate() {
   ...
   for (int jj = 0; jj < ncsol; jj++)
      DbgLv(1) << "ITER:     csol" << jj << "  s k c" << csolutes[ jj ].s * 1.e13 << csolutes[ jj ].k
               << csolutes[ jj ].c;
   int ktadd = 0;
   //*DEBUG
   ...
   for (ktask = 0; ktask < nsubgrid; ktask++) {
      ...
      ktadd += (ncsol - kadd);
      if (kadd < ncsol)
         DbgLv(1) << "ITER: kt" << ktask << "iterate nisol o a c +" << kosz << kasz << ncsol << kadd << "ktadd" << ktadd
                  << "nsubg" << nsubgrid;
      //*DEBUG
   ...
   if (ktadd < ncsol) {
      for (int kt = 0; kt < nsubgrid; kt++)
         for (int cc = 0; cc < orig_sols[ kt ].size(); cc++)
            DbgLv(1) << "ITER          kt cc" << kt << cc << " s k c" << orig_sols[ kt ][ cc ].s * 1.e13
                     << orig_sols[ kt ][ cc ].k << orig_sols[ kt ][ cc ].c;
   }
   //*DEBUG
...
int US_2dsaProcess::estimate_steps(int ncsol) {
   ...
   DbgLv(1) << "ES: ncsol ktisol ktcsol ktstep" << ncsol << ktisol << ktcsol << ktstep;
   ...
   while (n1task > 1) { // Sum in steps for depth 3 and following until just 1 task left
      n1task = (n1csol + ktisol / 2) / ktisol + 1;
      ntasks += n1task;
      n1csol = n1task * ktcsol;
...
int US_2dsaProcess::running_at_depth(int depth) {
   ...
   for (int ii = 0; ii < nthreads; ii++) {
      if (wkstates[ ii ] == WORKING && wkdepths[ ii ] == depth && wthreads[ ii ] != 0)
         count++;
   }
...
void US_2dsaProcess::set_meniscus() {
   ...
   DbgLv(1) << "SET_MEN: k_iter m_iter b_iter" << k_iter << m_iter << b_iter << "meniscus bottom" << edata->meniscus
            << simparms->bottom << "bbot" << bbottom;
...
void US_2dsaProcess::set_monteCarlo() {
   ...
   edata = &wdata;
   DbgLv(1) << "MCARLO: mm_iter" << mm_iter << " sigma0 c0 v0 cn vn" << sigmas[ 0 ] << bdata->value(0, 0)
            << edata->value(0, 0) << bdata->value(nscans - 1, npoints - 1) << edata->value(nscans - 1, npoints - 1);
...
void US_2dsaProcess::set_gaussians() {
   ...
   if (!gausmoo) { // Construct sigmas from iteration 1 residuals
      for (int ss = 0; ss < nscans; ss++)
         for (int rr = 0; rr < npoints; rr++)
            sigmas << qAbs(rdata.value(ss, rr));
   }

   else { // Construct sigmas from residuals smoothed for each scan
...
WorkPacket2D US_2dsaProcess::next_job() {
   ...
   if (depth > 0) { // If first is beyond depth 0, insure what we get is lowest depth
   ...
   if (jobx > 0) {
      DbgLv(1) << "NEXTJ: jobx depth depth0 taskx taskx0" << jobx << depth << job_queue[ 0 ].depth << wtask.taskx
               << job_queue[ 0 ].taskx;
   }
   else {
