    ...
    : QObject (parent), dsets (dsets)
{
  ...
  // mintsols         = 100;            // minimum solutes per task, depth 1
  // ff.
...
void
US_2dsaProcess::start_fit (double sll, double sul, int nss, double kll,
                           double kul, int nks, int ngr, int nthr, int noif)
{
  ...
  if (jgrefine > 0)
    {
      US_Solute::init_solutes (ssllim, ssulim, nssteps, klolim, kuplim,
                               nksteps, ngrefine, cnstff0, orig_sols);
      ...
      if (model.analysis == US_Model::CUSTOMGRID && nsubgrid > 0)
          ...
          for (int ii = 0; ii < nsubgrid; ii++)
            {
              ...
              for (int jj = ii; jj < ncomps; jj += nsubgrid)
                {
                  US_Solute soli (
                      model.components[jj].s, model.components[jj].f_f0, 0.0,
                      model.components[jj].vbar20, model.components[jj].D);
                  DbgLv (1) << "ii" << ii << "soli" << soli.s << soli.k
                            << soli.c << soli.v;
                  solvec << soli;
                }
      ...
      else
        {
          for (int ii = 0; ii < ncomps; ii++)
            {
              US_Solute soli (
                  model.components[ii].s, model.components[ii].f_f0, 0.0,
...
void
US_2dsaProcess::set_iters (int mxiter, int mciter, int mniter, double vtoler,
                           double menrng, double cff0, int jgref, int fittyp)
{
  maxiters = mxiter;
...
void
US_2dsaProcess::step_progress (int ksteps)
{
  max_rss ();
...
US_2dsaProcess::final_computes ()
{
  ...
  connect (wthr, SIGNAL (work_progress (int)), this,
           SLOT (step_progress (int)));
...
void
US_2dsaProcess::process_final (WorkerThread2D *wthrd)
{
  ...
  if (dset->solute_type == 0)
    { // Normal case of varying f/f0
      DbgLv (1) << "FIN_FIN: solute_type" << dset->solute_type << "nsols"
                ...
                << c_solutes.size () << "csol[m]size"
                << c_solutes[maxdepth].size ();
      for (int cc = 0; cc < nsolutes; cc++)
        {
          ...
          DbgLv (1) << "FIN_FIN:  Bcc comp D" << mcomp.D << "comp ff0"
                    << mcomp.f_f0 << "comp vb20" << mcomp.vbar20;
...
stdspc=true;
  ...
  pmsg += tr ("Maximum memory used:  ") + QString::number (qRound (memmb))
          + " MB";
      ...
      if (r_iter < 1)
        ...
        { // otherwise, we must compare solutes and variance difference
          ...
          DbgLv (1) << "FIN_FIN: neediter" << neediter << "  sdiffs" << sdiffs
                    << " dvari varitol" << dvari << varitol << " iter"
                    << r_iter;
        }
    }
  ...
  if (mmtype > 0 && ++mm_iter < mtiters)
    { // doing meniscus or monte carlo and more to do
      ...
      if (ff_meni)
        {
          emeniscus
              = (bmeniscus - menrange * 0.5) + ((double)m_iter * mendelta);
        }
      if (ff_bott)
        {
      ...
      if (mtiters > 50)
        DbgLv (1) << "MENISC: mtiters mmiters" << mtiters << mmiters
                  ...
                  << "menbot" << ff_menbot << "meni" << ff_meni << "bott"
                  << ff_bott;
...
bool
US_2dsaProcess::get_results (US_DataIO::RawData *da_sim,
...
bool
US_2dsaProcess::get_values (QMap<QString, QString> &mp_val)
{
  bool all_ok = true;
...
// Submit a job
void
US_2dsaProcess::submit_job (WorkPacket2D &wtask, int thrx)
{
  wtask.thrn = thrx + 1;
...
void
US_2dsaProcess::process_job (WorkerThread2D *wthrd)
{
  WorkPacket2D wresult;
  ...
  if (nextc > maxtsols)
    { // if new solutes push count over limit, queue a job at next depth
      ...
      DbgLv (1) << "THR_FIN: depth" << wtask.depth << " #solutes"
                << wtask.isolutes.size () << " nextc maxtsols" << nextc
    ...
    { // this result is from depth 0, it is from initial subgrids pass
      ...
      if (kctask == nsubgrid)
        { // all subgrid tasks are now complete
          ...
          emit message_update (
              pmessage_head ()
                  + tr ("Computing depth 1 solutions and beyond ..."),
              false);
  ...
  while (!job_queue.isEmpty () && (thrx = wkstates.indexOf (READY)) >= 0)
    {
      ...
      if (wtask.depth > 1 && wtask.taskx == tkdepths.indexOf (wtask.depth))
        { // submitting the first job of a depth pass
          QString pmsg
              = tr ("Computing depth %1 solutions...").arg (wtask.depth);
    ...
    { // No new tasks got started:  what's going on?
      ...
      if (depth < maxdepth)
        { // If done at depth less than max, see need to queue new task
          ...
          if (jobs_at_depth (dd) == 0 && c_solutes[dd].size () > 0)
            { // queue a task to handle remaining solutes at next depth plus
              // one
              depth = dd;
              WorkPacket2D wtask = wresult;
              int taskx = tkdepths.size ();
              DbgLv (1) << "THR_FIN:    QT: /NONEW++/taskx depth solsz"
                        << taskx << depth << c_solutes[dd].size ();
              queue_task (wtask, slolim, klolim, taskx, depth, jnois,
                          c_solutes[depth]);
...
void
US_2dsaProcess::queue_task (WorkPacket2D &wtask, double llss, double llsk,
  ...
  for (ktask = 0; ktask < nsubgrid; ktask++)
    {
      ...
      ktadd += (ncsol - kadd);
      if (kadd < ncsol)
        DbgLv (1) << "ITER: kt" << ktask << "iterate nisol o a c +" << kosz
                  << kasz << ncsol << kadd << "ktadd" << ktadd << "nsubg"
                  << nsubgrid;
      //*DEBUG
...
void
US_2dsaProcess::free_worker (int tx)
{
  if (tx >= 0 && tx < nthreads)
    {
      if (wthreads[tx] != 0)
...
// Estimate progress steps after depth 0 from given total of calculated
// solutes.
...
int
US_2dsaProcess::estimate_steps (int ncsol)
{
  ...
  int szdata = sizeof (US_DataIO::RawData) + (nscns * szscnd)
               + (nscns * nrpts * szrdng);
  int szsimu
...
int
US_2dsaProcess::queued_at_depth (int depth)
{
  int count = 0;
...
int
US_2dsaProcess::running_at_depth (int depth)
{
  int count = 0;

  for (int ii = 0; ii < nthreads; ii++)
    {
      if (wkstates[ii] == WORKING && wkdepths[ii] == depth
          && wthreads[ii] != 0)
        count++;
    }
...
int
US_2dsaProcess::jobs_at_depth (int depth)
{
...
US_2dsaProcess::set_meniscus ()
{
  ...
  edata->meniscus = emeniscus;
  edata->bottom = ebottom;
  DbgLv (1) << "SET_MEN: emeniscus ebottom" << edata->meniscus
            << edata->bottom;
