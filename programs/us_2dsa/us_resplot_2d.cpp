...
US_ResidPlot2D::US_ResidPlot2D(QWidget* parent) : US_WidgetsDialog(0, 0) {
  ...
  plotLayout1 = new US_Plot(data_plot1, tr("Experimental Data"),
                            tr("Radius (cm)"), tr("Absorbance"));
...
void US_ResidPlot2D::stinCheck(bool) { plot_data(); }

// subtract-ri-noise box [un]checked
void US_ResidPlot2D::srinCheck(bool) { plot_data(); }
...
void US_ResidPlot2D::atinCheck(bool) { plot_data(); }

// add-ri-noise box [un]checked
void US_ResidPlot2D::arinCheck(bool) { plot_data(); }
  ...
  if (do_shorbm) {  // show residuals bitmap (if not already shown)
    ...
    for (int ii = 0; ii < count; ii++) {  // build a vector for each scan
      ...
      for (int jj = 0; jj < points;
        ...
        resscan[jj] =
            edata->value(ii, jj) - sdata->value(ii, jj) - rinoi - tinoi;
        rmsd += sq(resscan[jj]);
        kntva++;
      }
...
QwtPlot* US_ResidPlot2D::rp_data_plot1() { return data_plot1; }

// Return a QwtPlot pointer to the lower plot
QwtPlot* US_ResidPlot2D::rp_data_plot2() { return data_plot2; }
