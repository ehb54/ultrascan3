...
void
US_ResidPlot2D::pedaCheck (bool chkd)
{
  if (chkd)
...
void
US_ResidPlot2D::psdaCheck (bool chkd)
{
  if (chkd)
    { // box is being checked:  sub boxes enabled if data present
      ck_addtin->setEnabled (have_ti);
...
void
US_ResidPlot2D::presCheck (bool chkd)
{
  skip_plot = true;
...
void
US_ResidPlot2D::ptinCheck (bool chkd)
{
  skip_plot = true;
...
void
US_ResidPlot2D::prinCheck (bool chkd)
{
  skip_plot = true;
...
void
US_ResidPlot2D::pranCheck (bool chkd)
{
  skip_plot = true;
...
void
US_ResidPlot2D::srbmCheck (bool chkd)
{
  if (chkd)
        ...
        { // get readings (y) for each scan
          ...
          for (int jj = 0; jj < points; jj++)
            { // each residual is e-value minus s-value
              ...
              vv[jj] = edata->value (ii, jj) - sdata->value (ii, jj) - rinoi
                       - tinoi;
              rmsd += sq (vv[jj]);
              kntva++;
            }
...
void
US_ResidPlot2D::connect_pboxes (bool conn)
{
  if (conn)
