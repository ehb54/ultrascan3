...
US_ResidPlot2D::US_ResidPlot2D(QWidget *parent) : US_WidgetsDialog(0, 0) {
   // lay out the GUI
   ...
   plotLayout1 = new US_Plot(data_plot1, tr("Experimental Data"), tr("Radius (cm)"), tr("Absorbance"));
   ...
   if (do_pltres) { // plot residuals
      ...
      for (int ii = 0; ii < count; ii++) { // get readings (y) for each scan
         ...
         for (int jj = 0; jj < points; jj++) { // each residual is e-value minus s-value
            ...
            vv[ jj ] = edata->value(ii, jj) - sdata->value(ii, jj) - rinoi - tinoi;
            rmsd += sq(vv[ jj ]);
            kntva++;
         }
   ...
   if (do_shorbm) { // show residuals bitmap (if not already shown)
      ...
      for (int ii = 0; ii < count; ii++) { // build a vector for each scan
         ...
         for (int jj = 0; jj < points; jj++) { // build residual values within a scan
            ...
            resscan[ jj ] = edata->value(ii, jj) - sdata->value(ii, jj) - rinoi - tinoi;
            rmsd += sq(resscan[ jj ]);
            kntva++;
         }
...
void US_ResidPlot2D::connect_pboxes(bool conn) {
