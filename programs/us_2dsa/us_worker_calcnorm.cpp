...
{
  DbgLv (1) << "CN(WT):   Thread destroy - (1)finished?" << isFinished ()
            << thrn;
  if (!wait (2000))
    {
      qDebug () << "Thread destroy wait timeout(2secs) : Thread" << thrn;
    }
  DbgLv (1) << "CN(WT):   Thread destroy - (2)finished?" << isFinished ()
            << thrn;
...
void
WorkerThreadCalcNorm::define_work (WorkPacketCN &workin)
{
...
void
WorkerThreadCalcNorm::get_result (WorkPacketCN &workout)
{
  workout.thrn = thrn;
  workout.nthrd = nthrd;
  workout.dset = dset;
...
void
WorkerThreadCalcNorm::calc_norms ()
{
  ...
  DbgLv (1) << "nwsols_" << nwsols << "solx0" << solxs[0] << "solx1"
            << solxs[1] << "solxn" << solxs[nwsols - 1];
...
void
WorkerThreadCalcNorm::set_comp_attr (US_Model::SimulationComponent &component,
