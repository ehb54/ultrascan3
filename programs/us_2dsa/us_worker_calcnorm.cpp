...
void WorkerThreadCalcNorm::calc_norms() {
   ...
   for (int ii = (thrn - 1); ii < nsolutes; ii += nthrd) { // fill list with this worker's solute point indecies
      solxs << ii;
      DbgLv(1) << "solxs_values" << solxs;
   }

   nwsols = solxs.count(); // count of solutes for worker
   DbgLv(1) << "nwsols_" << nwsols << "solx0" << solxs[ 0 ] << "solx1" << solxs[ 1 ] << "solxn" << solxs[ nwsols - 1 ];

   solutes_c.resize(nwsols); // computed solute points

   for (int ii = 0; ii < nwsols; ii++) { // computed solutes initialized from full input list
...
void WorkerThreadCalcNorm::set_comp_attr(US_Model::SimulationComponent &component, US_Solute &solute, int attr_type) {
   switch (attr_type) {
      default:
