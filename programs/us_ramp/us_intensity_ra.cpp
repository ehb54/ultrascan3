...
US_IntensityRa::US_IntensityRa(
   const QString runID, const QString triple, const QVector<double> &data, const QVector<double> &scan) :
   ...
   // Plot Rows
   QBoxLayout *plot = new US_Plot(data_plot, tr("Intensity Profile"), tr("Scan Number"), tr("Intensity"));
   ...
   if (triple.contains("-")) { // Handle special MWL triple with wavelength range
      data_plot->setTitle(
         tr("Intensity Profile\n"
            "for wavelengths: ")
         + ctriple.mid(2));
      data_plot->setAxisTitle(QwtPlot::xBottom, tr("Wavelength.Scan Number"));
   }
...
void US_IntensityRa::draw_plot(const QVector<double> &scanData, const QVector<double> &scanNbrs) {
