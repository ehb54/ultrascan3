...
US_SelectTriplesRa::US_SelectTriplesRa(QStringList &triples) : US_WidgetsDialog(0, 0), triples(triples) {
   ...
   le_selected = us_lineedit(tr("%1 excluded triples").arg(nbr_select), -1, true);
   ...
   for (int ii = 0; ii < nbr_orig; ii++) { // Add items to the original (included) list
...
void US_SelectTriplesRa::add_selections() {
   ...
   for (int ii = 0; ii < nbr_select; ii++) { // Move selected items from included to excluded
   ...
   le_original->setText(
      (nbr_incl == 1) ? tr("1 included triple")
                      : tr("%1 included triples").arg(nbr_incl) + tr(" (of %1 original)").arg(nbr_orig));
   le_selected->setText((nbr_excl == 1) ? tr("1 excluded triple") : tr("%1 excluded triples").arg(nbr_excl));
...
void US_SelectTriplesRa::rmv_selections() {
   ...
   for (int ii = 0; ii < nbr_select; ii++) { // Move selected items from excluded to included
      QListWidgetItem *l_item = selitems.at(ii);
      QString triple = l_item->text();
