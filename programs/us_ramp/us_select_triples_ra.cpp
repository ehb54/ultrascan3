   ...
   : US_WidgetsDialog( nullptr, Qt::WindowFlags() ), triples( triples )
{
   ...
   le_original   = us_lineedit( tr( "%1 original triples" )
                      .arg( nbr_orig ),   -1, true );
   le_selected   = us_lineedit( tr( "%1 excluded triples" )
   ...
   {  // Add items to the original (included) list
  ...
  // Connections
   connect( pb_add,    &QAbstractButton::clicked, this, &US_SelectTriplesRa::add_selections );
   connect( pb_remove, &QAbstractButton::clicked, this, &US_SelectTriplesRa::rmv_selections );
   connect( pb_reset,  &QAbstractButton::clicked, this, &US_SelectTriplesRa::reset );
   connect( pb_help,   &QAbstractButton::clicked, this, &US_SelectTriplesRa::help );
   connect( pb_cancel, &QAbstractButton::clicked, this, &US_SelectTriplesRa::cancel );
   connect( pb_accept, &QAbstractButton::clicked, this, &US_SelectTriplesRa::done );
...
DbgLv(0) << "AddSelections";
   ...
   {  // Move selected items from included to excluded
...
DbgLv(0) << "RemoveSelections";
   ...
   for ( int ii = 0; ii < nbr_select; ii++ )
   {  // Move selected items from excluded to included
      QListWidgetItem* l_item   = selitems.at( ii );
      QString          triple   = l_item->text();
