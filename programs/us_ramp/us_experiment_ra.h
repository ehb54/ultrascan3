...
class US_ExperimentRa {
 public:
  ...
  struct RotorInfo {
    ...
    QList<CalibrationInfo>
        calibrationInfo;  //!< All available calibration profiles for this rotor
  };
  ...
  //       QList< double >  rpms;               //!< A list of rotor speeds
  ...
  //       QVector< double > RIProfile;         //!< If RI data, the intensity
  //       profile QVector< int >    RIwvlns;           //!< For RI+MWL,
  //       wavelengths present int               RI_nscans;         //!< RI+MWL
  //       scans per profile int               RI_nwvlns;         //!< RI+MWL
  //       wavelengths (profiles count)
  ...
  */
  int saveToDisk(QVector<US_mwlRamp::RampRawData*>&,
                 QList<US_Ramp::TripleInfo>&, QString, QString, QString);
 ...
 private:
  void readExperiment(QXmlStreamReader&, QList<US_Ramp::TripleInfo>&, QString,
                      QString);
