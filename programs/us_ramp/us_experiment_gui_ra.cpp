...
bool US_ExperimentGuiRa::load(void) {
  ...
  if (disk_controls->db()) {
    ...
    if (labList.size() > 0 &&
        labList[0].instruments.size() ==
            0) {  // If empty instrument table, warn and exit
...
void US_ExperimentGuiRa::cancelProject(void) { reset(); }
...
void US_ExperimentGuiRa::cancelRotor(void) { reset(); }
