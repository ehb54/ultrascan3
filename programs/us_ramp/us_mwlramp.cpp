...
bool US_mwlRamp::import_data(
   QString &mwldir, QLineEdit *lestat, QVector<US_mwlRamp::RampRawData> &allData, QString &runID_xml_mwl) {
   bool status = true;
   cur_dir = mwldir;
   le_status = lestat;
   //    qDebug() << cur_dir;
   ...
   // Read in the data
   QStringList mwrfs = ddir.entryList(QStringList("*.mwsr"), QDir::Files, QDir::Name);
   QStringList chans;
   cells.clear();
   mwrfs.sort();
   nfile = mwrfs.size();
   ...
   chans.sort();
   cellchans.clear();
   le_status->setText(QString("%1 cell(s) and %2 channel(s) ...").arg(ncell).arg(nchan));
   qApp->processEvents();
   ...
   for (int cc = 0; cc < cellchans.size(); cc++) {
      ...
      qDebug() << "sizeintarray" << raw.intarray.size() << "raw.intarray[cc].size()" << raw.intarray[ cc ].size();
      for (int wl = 0; wl < n_wl; wl++) {
      ...
      allData[ cc ] = raw;
      qDebug() << "alldat" << allData[ cc ].chan << "temp" << allData[ cc ].temperature;
   }
...
void US_mwlRamp::read_runxml(QDir ddir, QString curdir) {
   QStringList mwrfs = ddir.entryList(QStringList("*.mwsr.xml"), QDir::Files, QDir::Name);
