...
//    qDebug() << "before make absolute" << ddir;
    ...
    // Read in the data
    QStringList mwrfs = ddir.entryList( QStringList( "*.mwsr" ),
...
//     qDebug() << "MwDa Ramp: ncell nchan" << ncell << nchan;
    ...
    chans    .sort();
    cellchans.clear();
    le_status->setText( QString( "%1 cell(s) and %2 channel(s) ..." )
                        .arg( ncell ).arg( nchan ) );
    qApp->processEvents();
...
{
   QStringList mwrfs = ddir.entryList( QStringList( "*.mwsr.xml" ),
         QDir::Files, QDir::Name );
