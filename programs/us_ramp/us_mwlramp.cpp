...
bool
US_mwlRamp::import_data (QString &mwldir, QLineEdit *lestat,
                         ...
                         QString &runID_xml_mwl)
{
  ...
  qDebug () << "ri" << ri_wavelns.size () << "raw"
            << allData[0].wl_array.size ();
...
void
US_mwlRamp::read_runxml (QDir ddir, QString curdir)
{
  QStringList mwrfs
          ...
          if (xml.name () == "runID")
            {
              ...
              radius
                  = att.value ("radius_measurement").toString ().toDouble ();
            }
