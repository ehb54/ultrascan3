...
DbgLv(1) << "XpDa:LdMw:   wlns size" << wavelns.size();
   ...
   // Write time state files if need be
   QString tpath = cur_dir + runID + "time_state.tmst";
   if ( ! QFile( tpath ).exists() )
   {
      ...
      US_SimulationParameters::computeSpeedSteps( &allData[ 0 ].scanData,
                                                  speedsteps );
      write_timestate( allData, speedsteps, cur_dir );
   }
}
...
//*DEBUG*
   ...
   tsobj.open_write_data( tspath, 1.0, 0.0 );  // Initialize TMST creation
...
DbgLv(1) << "expA:   ii" << ii << "scan" << scannbr << "stage" << istagen
 << "speed" << irSpeed << isSpeed << "time" << time;
         ...
         tsobj.set_value( fkeys[ 1 ], irSpeed );       // Raw speed
...
DbgLv(1) << "expA:  ii" << ii << "sctime" << time;
      ...
      tsobj.set_value( fkeys[ 0 ], time_n  );    // Time in seconds
      tsobj.set_value( fkeys[ 1 ], speed_n );    // Raw speed
      tsobj.set_value( fkeys[ 2 ], 0       );    // Set (stage) speed
      tsobj.set_value( fkeys[ 3 ], omg2t_n );    // Omega-Squared-T
      tsobj.set_value( fkeys[ 4 ], tempe_n );    // Temperature
      tsobj.set_value( fkeys[ 5 ], 1       );    // Stage (speed step)
      tsobj.set_value( fkeys[ 6 ], 0       );    // Scan
      tsobj.flush_record();
...
DbgLv(1) << "expA:      jj" << jj << "scan" << scannbr
 ...
 << "omg2t(rec)" << omega2t << "omg2t(sum)" << omg2t_sm;
   ...
   {
      tsobj.write_defs( 1.0, "Optima" );
      nfiles        += 2;
DbgLv(1) << "expA: TMST files written.";
   }
#endif
#if 1
   // Write time state files
   write_timestate( allData, speedsteps, cur_dir );
   nfiles        += 2;
#endif
...
// Write time state files
void US_XpnData::write_timestate( QVector< US_DataIO::RawData >& allData,
   QVector< US_SimulationParameters::SpeedProfile >& speedsteps,
   QString& cur_dir )
{
   // Find the index to the first dataset with maximum scans
   int mxscans       = 0;
   int iiuse         = 0;
   int ntrips        = allData.count();
   ...
   US_DataIO::RawData* udata = &allData[ iiuse ];
   QString runID     = QString( cur_dir ).section( "/", -2, -2 );
...
DbgLv(1) << "expA:  ii" << ii << "sctime" << time;
      ...
      ntimes            = ntssda;
DbgLv(1) << "expA:  ntimes" << ntimes << "etimoff" << etimoff;
      int time_n        = 0;                        // Initial values
      double speed_n    = 0.0;
      double omg2t_n    = 0.0;
      double tempe_n    = tSydata[ ftx ].tempera;
...
void US_XpnData::update_ATable( QSqlQuery& sqry, QList< int >& cxs )
{
   ...
   if ( asdrow.speed == 0.0 )
   {  // While speed is zero, only update first row of output data
      
      mdx1             = ( strow >= 0 ) ? 0 : -1;
      mdx2             = ( strow >= 0 ) ? 1 : -1;
   }
   else
   {  // Once into non-zero speeds do normal add/substitution
