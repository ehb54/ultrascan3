...
int main( int argc, char **argv )
{
   ...
   */
   ...
   do 
   {
      ...
      case PM_NEW_PM :
         {
            ...
            pm = new PM_WORKER( msg.grid_conversion_factor,
                                msg.max_dimension,
                                msg.max_harmonics,
                                F,
                                q,
      ...
      case PM_NEW_GRID_SIZE:
         {
            ...
            if ( !pm )
            {
               cout << myrank << ":MPI PM_NEW_GRID_SIZE Receive called before PM_NEW_PM  pm_mpi_worker()\n"  << flush;
               MPI_Abort( MPI_COMM_WORLD, errorno - myrank );
               exit( errorno - myrank );
            }
      ...
      case PM_CALC_FITNESS:
         {
            ...
            if ( !pm )
            {
               cout << myrank << ":MPI PM_CALC_FITNESS Receive called before PM_NEW_PM  pm_mpi_worker()\n"  << flush;
               MPI_Abort( MPI_COMM_WORLD, errorno - myrank );
               exit( errorno - myrank );
            }
            ...
            if ( MPI_SUCCESS != MPI_Recv( &(params[ 0 ]),
                                          ...
                                          &mpi_status ) )
            {
               cout << myrank << ":MPI PM_CALC_FITNESS Receive failed in pm_mpi_worker()\n"  << flush;
               MPI_Abort( MPI_COMM_WORLD, errorno - myrank );
               exit( errorno - myrank );
            }         
            ...
            if ( MPI_SUCCESS != MPI_Send( &msg,
                                          ...
                                          MPI_COMM_WORLD ) )
            {
               cout << myrank << ":MPI PM_CALC_FITNESS Send 1 failed in pm_mpi_worker()\n"  << flush;
               MPI_Abort( MPI_COMM_WORLD, errorno - myrank );
               exit( errorno - myrank );
            }         
            ...
            if ( MPI_SUCCESS != MPI_Send( &(vmodel[0]),
                                          ...
                                          MPI_COMM_WORLD ) )
            {
               cout << myrank << ":MPI PM_CALC_FITNESS Send 2 failed in pm_mpi_worker()\n"  << flush;
               MPI_Abort( MPI_COMM_WORLD, errorno - myrank );
               exit( errorno - myrank );
            }         
         }
      ...
      default:
         {
            cout << myrank << ":MPI Receive unknown message type in pm_mpi_worker() " << msg.type << endl << flush;
            MPI_Abort( MPI_COMM_WORLD, errorno - myrank );
            exit( errorno - myrank );
         }         
      }
