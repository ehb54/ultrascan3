...
bool US_Saxs_Util::c2check( QString a, QString b )
{
   ...
   for ( unsigned int i = 0; i < model_vector.size(); i++ )
   {
      ...
      for (unsigned int j = 0; j < model_vector[current_model].molecule.size(); j++)
      {
         for (unsigned int k = 0; k < model_vector[current_model].molecule[j].atom.size(); k++)
         {
                 ...
                 this_atom->chainID == b )
            {
               ...
               if ( new_atom.atom_name == "CA" || !control_parameters.count( "c2checkcaonly" ) )
               {
                  this_atom->chainID == a ? atomsa.push_back( new_atom ) : atomsb.push_back( new_atom );
               }
            }
         }
      }
      ...
      {
         errormsg = QString( "Error: chain %1 has %2 atoms and chain %3 has %4 atoms inputfile %5" )
            .arg( a ).arg( atomsa.size() )
            .arg( b ).arg( atomsb.size() )
      ...
      for ( int k = 0; k < (int) atomsa.size(); ++k )
      {
         ...
         {
            errormsg = QString( "Error: name mismatch %1:%2:%3 vs %4:%5:%6 inputfile %7" )
               .arg( a ).arg( atomsa[ k ].atom_name ).arg( atomsa[ k ].residue_name )
               .arg( b ).arg( atomsb[ k ].atom_name ).arg( atomsb[ k ].residue_name )
               .arg( control_parameters[ "inputfile" ] )
               ;
            return false;
         }
      ...
      {
         QFile f( QString( "%1_%2_%3_%4.txt" )
                  .arg( inputbase )
                  .arg( i + 1 )
                  .arg( a )
                  .arg( b ) );
